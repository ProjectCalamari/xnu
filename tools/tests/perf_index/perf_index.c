...
void start_timer(struct timeval *tp) { gettimeofday(tp, NULL); }
...
static void *thread_setup(void *arg) {
  ...
  test.execute(my_index, args.num_threads, work_size, args.test_argc,
               args.test_argv);
  return NULL;
}
...
int main(int argc, char **argv) {
  ...
  for (thread_index = 0; thread_index < args.num_threads; thread_index++) {
    retval = pthread_create(&threads[thread_index], NULL, thread_setup,
