...
int cleanup_tempdir(char *path) { return rmdir(path); }

int test_file_create(char *path, int thread_id, int num_threads,
                     long long length) {
  long long i;
  int fd;
  int retval;
  ...
  for (i = 0; i < length; i++) {
    snprintf(filepath, MAXPATHLEN, "%s/file_create-%d-%lld", path, thread_id,
             i);
...
int test_file_read_setup(char *path, int num_threads, long long length,
...
int test_file_read(char *path, int thread_id, int num_threads, long long length,
                   long long max_file_size) {
  long long left;
...
int test_file_write(char *path, int thread_id, int num_threads,
                    long long length, long long max_file_size) {
  long long left;
  size_t file_offset = 0;
  int writelen;
  int retval;
