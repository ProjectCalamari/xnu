...
int test_fault_setup() {
  ...
  memblock = (char *)mmap(NULL, MEMSIZE, PROT_READ | PROT_WRITE,
...
int test_fault_helper(int thread_id, int num_threads, long long length,
                      testtype_t testtype) {
  char *ptr;
  int pgsz = getpagesize();
  int retval;
  ...
  while (1) {
    for (ptr = memblock + region_start; ptr < memblock + region_end;
         ptr += pgsz) {
      *ptr = 1;
      left--;
      if (left == 0) {
    ...
    if (testtype == TESTFAULT) {
      ...
      VERIFY(retval == 0, "mprotect failed");
      retval = mprotect(memblock + region_start, region_len,
                        PROT_READ | PROT_WRITE) == 0;
      VERIFY(retval == 0, "mprotect failed");
    } else if (testtype == TESTZFOD) {
      retval = munmap(memblock + region_start, region_len) == 0;
      VERIFY(retval == 0, "munmap failed");
      ptr = mmap(memblock + region_start, region_len, PROT_READ | PROT_WRITE,
