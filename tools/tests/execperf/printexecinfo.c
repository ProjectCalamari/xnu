...
__attribute__((constructor)) void init(int argc, const char *argv[],
                                       const char *envp[], const char *appl[],
                                       void *vars __attribute__((unused))) {
  int i;
...
void printexecinfo(void) {
  ...
  const NXArchInfo *arch = NXGetArchInfoFromCpuType(
      ...
      _mh_execute_header.cpusubtype & ~CPU_SUBTYPE_MASK);
  ...
  printf("executable cputype 0x%08x cpusubtype 0x%08x (%s:%s)\n",
         ...
         arch ? arch->name : "unknown", arch ? arch->description : "unknown");
