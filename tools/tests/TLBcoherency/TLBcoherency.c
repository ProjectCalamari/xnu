 ...
 * With a custom kernel, it has the option of creating a remapping of the page
 * in the kernel's address space to exercise shared kernel mapping coherency.
 * Each thread subsequently loops around on the set of mappings. One thread is
 * designated the observer thread. The thread acquires a lock on the arena
 * element, verifies that the mapping has the expected pattern (Address | PID),
 * if the element is in the MAPPED state. Can optionally tell the kernel to
 * check its alias as well. If it notices a mismatch, it has the option to issue
 * a syscall to  stop kernel tracing. If the -f option is supplied, the test is
 * terminated. If the page has lingered beyond -l microseconds, non-observer
 * threads will unmap the page, optionally calling into the kernel to unmap its
 * alias, and repopulate the element. After this sequence, the thread will
 * optionally usleep for -p microseconds, to allow for idle power management to
 * engage if possible (errata might exist in those areas), or context switches
 * to occur. Created Derek Kumar, 2011.
 */
...
void initialize_arena_element(int i) {
  ...
  parray[i].taddr = (uintptr_t)mmap(hint, mapping_size, PROT_READ | PROT_WRITE,
  ...
  for (;;) {
    for (int i = 0; i < arenasize; i++) {
      ...
      if (parray[i].tstate != UNMAPPED) {
        ...
        if ((role != OBSERVER) &&
            ((mach_absolute_time() - parray[i].tctime) > page_linger_time)) {
          ...
          if (dosyscall) {
            sysret =
                syscall(MYSYS, MDOUNMAP, parray[i].taddr, ad, i, mapping_size);
          }
        }
      } else {
        if (role != OBSERVER) {
...
int main(int argc, char **argv) {
  ...
  printf("page_linger_time: 0x%llx, nthreads: %u, arenasize: %u sleepus: %d "
         "reuse_addrs: %u, stop_on_failure: %u, dosyscall: %u, mappingsize: "
         "0x%x\n",
         page_linger_time, nthreads, arenasize, sleepus, reuse_addrs,
         (unsigned)stop_on_failure, dosyscall, mapping_size);
