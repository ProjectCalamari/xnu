...
#define assert(truth, label)                                                   \
  do {                                                                         \
    if (!(truth)) {                                                            \
      printf("Thread %p: failure on line %d\n", pthread_self(), __LINE__);     \
      goto label;                                                              \
    }                                                                          \
  } while (0)
...
void print_usage() {
  printf("Usage: jitter [-w] [-s <random seed>] [-n <min sleep, ns>] [-m <max "
         "sleep, ns>] <realtime | timeshare | fixed> <num iterations> "
         "<traceworthy jitter, ns>\n");
}
...
int thread_setup(my_policy_type_t pol) {
  int res;
  ...
  case MY_POLICY_REALTIME: {
    ...
    res = thread_policy_set(mach_thread_self(), THREAD_TIME_CONSTRAINT_POLICY,
                            (thread_policy_t)&pol,
...
uint64_t get_random_sleep_length_abs_ns(uint64_t min_sleep_ns,
...
void compute_stats(double *values, uint64_t count, double *average_magnitudep,
                   double *maxp, double *minp, double *stddevp) {
  uint64_t i;
  double _sum = 0;
  double _max = 0;
...
void print_stats_us(const char *label, double avg, double max, double min,
                    double stddev) {
  printf("Max %s: %.1lfus\n", label,
         max / 1000.0 * (((double)g_mti.numer) / ((double)g_mti.denom)));
  printf("Min %s: %.1lfus\n", label,
         min / 1000.0 * (((double)g_mti.numer) / ((double)g_mti.denom)));
  printf("Avg magnitude of %s: %.1lfus\n", label,
         avg / 1000.0 * (((double)g_mti.numer) / ((double)g_mti.denom)));
  printf("Stddev: %.1lfus\n",
         stddev / 1000.0 * (((double)g_mti.numer) / ((double)g_mti.denom)));
  putchar('\n');
}

void print_stats_fract(const char *label, double avg, double max, double min,
                       double stddev) {
...
int main(int argc, char **argv) {
  ...
  if (wakeup_second_thread) {
    ...
    wakeup_second_jitter_arr =
        (double *)malloc(sizeof(*jitter_arr) * iterations);
    if (wakeup_second_jitter_arr == NULL) {
      printf("Couldn't allocate array to store results.\n");
      exit(1);
    }

    kret = semaphore_create(mach_task_self(), &wakeup_semaphore,
                            SYNC_POLICY_FIFO, 0);
    ...
    kret = semaphore_create(mach_task_self(), &return_semaphore,
                            SYNC_POLICY_FIFO, 0);
    if (kret != KERN_SUCCESS) {
      printf("Couldn't allocate semaphore %d\n", kret);
      exit(1);
  ...
  for (i = 0; i < iterations; i++) {
    sleep_length_abs =
        (uint64_t)(get_random_sleep_length_abs_ns(min_sleep_ns, max_sleep_ns) *
                   (((double)g_mti.denom) / ((double)g_mti.numer)));
  ...
  compute_stats(fraction_arr, iterations, &avg_fract, &max_fract, &min_fract,
                &stddev_fract);
  ...
  if (wakeup_second_thread) {
    ...
    compute_stats(wakeup_second_jitter_arr, iterations, &avg, &max, &min,
                  &stddev);
    ...
    printf("%llu/%llu (%.1f%%) wakeups on same CPU\n", secargs.woke_on_same_cpu,
           iterations, 100.0 * ((double)secargs.woke_on_same_cpu) / iterations);
  }
...
void *second_thread(void *args) {
  ...
  for (i = 0; i < secargs->iterations; i++) {
    ...
    if (wake_time < secargs->last_poke_time) {
      ...
      errx(1, "woke in past %llu (%d) < %llu (%d)", wake_time, cpuno,
           secargs->last_poke_time, secargs->cpuno);
    }
    ...
    secargs->wakeup_second_jitter_arr[i] =
        (double)(wake_time - secargs->last_poke_time);
