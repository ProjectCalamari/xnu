...
typedef enum wake_type {
  WAKE_BROADCAST_ONESEM,
  WAKE_BROADCAST_PERTHREAD,
  WAKE_CHAIN,
  WAKE_HOP
} wake_type_t;
typedef enum my_policy_type {
  MY_POLICY_REALTIME,
  MY_POLICY_TIMESHARE,
...
#define mach_assert_zero(error)                                                \
  do {                                                                         \
    if ((error) != 0) {                                                        \
      fprintf(stderr, "[FAIL] error %d (%s) ", (error),                        \
              mach_error_string(error));                                       \
      assert(error == 0);                                                      \
    }                                                                          \
  } while (0)
#define mach_assert_zero_t(tid, error)                                         \
  do {                                                                         \
    if ((error) != 0) {                                                        \
      fprintf(stderr, "[FAIL] Thread %d error %d (%s) ", (tid), (error),       \
              mach_error_string(error));                                       \
      assert(error == 0);                                                      \
    }                                                                          \
  } while (0)
#define assert_zero_t(tid, error)                                              \
  do {                                                                         \
    if ((error) != 0) {                                                        \
      fprintf(stderr, "[FAIL] Thread %d error %d ", (tid), (error));           \
      assert(error == 0);                                                      \
    }                                                                          \
  } while (0)
...
static boolean_t g_churn_random =
    FALSE; /* churn threads randomly sleep and wake */
 ...
 * will not change the work interval's start or deadline. Useful if churn
 * threads are meant to pre-warm the workgroup. */
static boolean_t g_rt_churn_same_wg = FALSE;

/* On SMT machines, test whether realtime threads are scheduled on the correct
 * CPUs */
...
/* Each thread spins for a certain duration after waking up before blocking
 * again. */
...
static uint64_t abs_to_nanos(uint64_t abstime) {
  return (uint64_t)(abstime * (((double)g_mti.numer) / ((double)g_mti.denom)));
}

static uint64_t nanos_to_abs(uint64_t ns) {
...
static bool workload_config_load(void) {
  /* Try to load the test workload config plist. */
  size_t len = 0;
  int result = sysctlbyname("kern.workload_config", NULL, &len,
                            (void *)(const void *)workload_config_plist,
                            strlen(workload_config_plist));
  if (result != 0) {
...
static void workload_config_unload(void) {
...
static void *churn_thread(__unused void *arg) {
  ...
  uint64_t sleep_at_time =
      ctime + nanos_to_abs(arc4random_uniform(sleep_us * NSEC_PER_USEC) + 1);
  while ((g_churn_stop == FALSE) &&
         (ctime < (g_starttime_abs + NSEC_PER_SEC))) {
    spin_count++;
    yield();
    ...
    if (g_churn_random && (ctime > sleep_at_time)) {
      ...
      sleep_at_time =
          ctime +
          nanos_to_abs(arc4random_uniform(sleep_us * NSEC_PER_USEC) + 1);
    }
  }

  /* This is totally racy, but only here to detect if anyone stops early */
  atomic_fetch_add_explicit(&g_churn_stopped_at, spin_count,
                            memory_order_relaxed);

  return NULL;
}

static void create_churn_threads() {
  if (g_churn_count == 0) {
...
static void join_churn_threads(void) {
 ...
 * Set policy
 */
static int rt_churn_thread_setup(void) {
  kern_return_t kr;
...
static void *rt_churn_thread(__unused void *arg) {
  ...
  for (uint32_t i = 0; i < g_iterations; i++) {
    kern_return_t kr =
        semaphore_wait_signal(g_rt_churn_start_sem, g_rt_churn_sem);
    mach_assert_zero_t(0, kr);
...
static void wait_for_rt_churn_threads(void) {
...
static void start_rt_churn_threads(void) {
  for (uint32_t i = 0; i < g_rt_churn_count; i++) {
...
static void create_rt_churn_threads(void) {
  ...
  /* Array for churn threads */
  g_rt_churn_threads =
      (pthread_t *)valloc(sizeof(pthread_t) * g_rt_churn_count);
...
static void join_rt_churn_threads(void) {
 ...
 */
static wake_type_t parse_wakeup_pattern(const char *str) {
...
time_value_t get_thread_runtime(void) {
  thread_basic_info_data_t info;
  mach_msg_type_number_t info_count = THREAD_BASIC_INFO_COUNT;
  thread_info(pthread_mach_thread_np(pthread_self()), THREAD_BASIC_INFO,
...
static void *worker_thread(void *arg) {
  ...
  for (uint32_t i = 0; i < g_iterations; i++) {
    if (my_id == 0) {
      ...
      /* Give the worker threads undisturbed time to finish before waiting on
       * them */
      if (g_do_sleep) {
       ...
       * Is there a better way to do this that won't interfere with the rest of
       * the chain?
      ...
      if (i > 0) {
        for (int cpuid = 0; cpuid < g_maxcpus; cpuid++) {
          if (g_cpu_histogram[cpuid].current == 1) {
            atomic_fetch_or_explicit(&g_cpu_map[i - 1], (1UL << cpuid),
                                     memory_order_relaxed);
      ...
      assert_zero_t(
          my_id, atomic_load_explicit(&g_done_threads, memory_order_relaxed));

      if (g_rt_workgroup_interval) {
        uint64_t interval_start = mach_absolute_time();
        uint64_t constraint_nanos =
            g_rt_ll ? LL_CONSTRAINT_NANOS : CONSTRAINT_NANOS;
        uint64_t deadline = interval_start + nanos_to_abs(constraint_nanos);
        debug_log("Starting work interval %u at %llu, deadline %llu\n", i,
                  interval_start, deadline);
        kr = os_workgroup_interval_start(g_rt_workgroup, interval_start,
                                         deadline, NULL);
        if (kr != 0) {
          printf("WARN: os_workgroup_interval_start returned %d; overlapping "
                 "intervals?\n",
                 kr);
        }
      }
    ...
    if (g_do_each_spin) {
      /* Each thread spins for a certain duration after waking up before
       * blocking again. */
    ...
    done_threads =
        atomic_fetch_add_explicit(&g_done_threads, 1, memory_order_relaxed) + 1;
    ...
    if (g_do_all_spin) {
      ...
      while (atomic_load_explicit(&g_done_threads, memory_order_relaxed) <
             g_numthreads) {
        y = y + 1.5 + x;
        x = sqrt(y);
      }
  ...
  if (my_id == 0) {
    ...
    for (int cpuid = 0; cpuid < g_maxcpus; cpuid++) {
      if (g_cpu_histogram[cpuid].current == 1) {
        atomic_fetch_or_explicit(&g_cpu_map[g_iterations - 1], (1UL << cpuid),
                                 memory_order_relaxed);
        g_cpu_histogram[cpuid].current = 0;
      }
    }
...
/*
 * Given an array of uint64_t values, compute average, max, min, and standard
 * deviation
 */
static void compute_stats(uint64_t *values, uint64_t count, float *averagep,
                          uint64_t *maxp, uint64_t *minp, float *stddevp) {
  uint32_t i;
  uint64_t _sum = 0;
  uint64_t _max = 0;
...
void record_cpu_time(cpu_time_t *cpu_time) {
...
static int set_recommended_cluster(char cluster_char) {
  char buff[4];
  buff[1] = '\0';
  ...
  int ret =
      sysctlbyname("kern.sched_task_set_cluster_type", NULL, NULL, buff, 1);
  if (ret != 0) {
...
int main(int argc, char **argv) {
  ...
  if (g_rt_workgroup_interval) {
    ...
    /* Pretend to be an audio client so that os_workgroup_max_parallel_threads
     * is accurate. */
    ret = os_workgroup_attr_set_interval_type(
        &attr, OS_WORKGROUP_INTERVAL_TYPE_AUDIO_CLIENT);
    if (ret != 0) {
      errx(EX_OSERR, "os_workgroup_attr_set_interval_type(OS_WORKGROUP_"
                     "INTERVAL_TYPE_AUDIO_CLIENT)");
    }
    g_rt_workgroup = os_workgroup_interval_create_with_workload_id(
        "zero-to-n", "com.apple.test.zero-to-n.audio",
        OS_CLOCK_MACH_ABSOLUTE_TIME, &attr);
    if (g_rt_workgroup == NULL) {
      errx(EX_OSERR, "Failed to create zero-to-n workgroup interval.");
    }
  } else if (g_policy == MY_POLICY_REALTIME) {
    os_workgroup_attr_s attr = OS_WORKGROUP_ATTR_INITIALIZER_DEFAULT;
    g_rt_workgroup = os_workgroup_create_with_workload_id(
        "zero-to-n", "com.apple.test.zero-to-n.default", &attr);
    if (g_rt_workgroup == NULL) {
  ...
  if (g_rt_churn && !g_rt_churn_same_wg) {
    os_workgroup_attr_s attr = OS_WORKGROUP_ATTR_INITIALIZER_DEFAULT;
    g_rt_churn_workgroup = os_workgroup_create_with_workload_id(
        "churn", "com.apple.test.zero-to-n.churn", &attr);
  ...
  ret = sysctlbyname("hw.perflevel0.logicalcpu", &g_numcpus, &numcpu_size, NULL,
                     0);
  if (ret) {
  ...
  ret = sysctlbyname("hw.perflevel0.physicalcpu", &g_nphysicalcpu,
                     &physicalcpu_size, NULL, 0);
  if (ret) {
    /* hw.perflevel0.physicalcpu failed so falling back to hw.physicalcpu */
    ret = sysctlbyname("hw.physicalcpu", &g_nphysicalcpu, &physicalcpu_size,
                       NULL, 0);
    if (ret) {
  ...
  ret =
      sysctlbyname("hw.logicalcpu", &g_nlogicalcpu, &logicalcpu_size, NULL, 0);
  if (ret) {
  ...
  if (g_test_rt) {
    if (g_numthreads == 0) {
      ...
      if ((g_numthreads == 1) &&
          ((g_waketype == WAKE_CHAIN) || (g_waketype == WAKE_HOP))) {
        g_numthreads = 2;
      }
    }
    ...
    g_histogram = true;
    /* Don't change g_traceworthy_latency_ns if it's explicity been set to
     * something other than the default */
    if (g_traceworthy_latency_ns == TRACEWORTHY_NANOS) {
      g_traceworthy_latency_ns =
          g_rt_ll ? TRACEWORTHY_NANOS_LL : TRACEWORTHY_NANOS_TEST;
    }
  ...
  if (g_do_each_spin) {
    g_iteration_sleeptime_us +=
        threads_per_core * (g_each_spin_duration_ns / NSEC_PER_USEC);
  }
  if (g_do_one_long_spin) {
  ...
  /* Ensure the allocation is pre-faulted */
  ret = memset_s(worst_latencies_from_first_ns, latencies_size, 0,
                 latencies_size);
  if (ret) {
  ...
  /* Ensure the allocation is pre-faulted */
  ret = memset_s(worst_latencies_from_previous_ns, latencies_size, 0,
                 latencies_size);
  if (ret) {
  ...
  if (g_waketype == WAKE_CHAIN || g_waketype == WAKE_BROADCAST_PERTHREAD ||
    ...
    for (uint32_t i = 0; i < g_numthreads; i++) {
      kr =
          semaphore_create(mach_task_self(), &g_semarr[i], SYNC_POLICY_FIFO, 0);
      mach_assert_zero(kr);
    }

    g_leadersem = g_semarr[0];
  } else {
    kr = semaphore_create(mach_task_self(), &g_broadcastsem, SYNC_POLICY_FIFO,
                          0);
    mach_assert_zero(kr);
  ...
  kr = semaphore_create(mach_task_self(), &g_rt_churn_start_sem,
                        SYNC_POLICY_FIFO, 0);
  mach_assert_zero(kr);
  ...
  for (uint32_t i = 0; i < g_iterations; i++) {
    ...
    assert(atomic_load_explicit(&g_done_threads, memory_order_relaxed) ==
           g_numthreads);
    ...
    ret = sysctlbyname("kern.sched_recommended_cores", &recommended_cores_map,
                       &map_size, NULL, 0);
    if ((ret == 0) &&
        (recommended_cores_map & mask(g_maxcpus)) != mask(g_maxcpus)) {
      if (g_test_rt) {
     ...
     */
    uint64_t worst_latency_ns =
        ((g_waketype == WAKE_CHAIN) || (g_waketype == WAKE_HOP))
            ? worst_latencies_from_previous_ns[i]
            : worst_latencies_ns[i];
    if (worst_latency_ns > g_traceworthy_latency_ns) {
      g_traceworthy_count++;
      /* Ariadne's ad-hoc test signpost */
      kdebug_trace(ARIADNEDBG_CODE(0, 0), worst_latency_ns,
                   g_traceworthy_latency_ns, 0, 0);
  ...
  uint32_t worker_threads_runtime =
      worker_threads_total_runtime.seconds * 1000 +
      worker_threads_total_runtime.microseconds / 1000;
  ...
  compute_stats(worst_latencies_from_first_ns, g_iterations, &avg, &max, &min,
                &stddev);
  ...
  if ((g_waketype == WAKE_CHAIN) || (g_waketype == WAKE_HOP)) {
    ...
    compute_stats(worst_latencies_from_previous_ns, g_iterations, &avg, &max,
                  &min, &stddev);
...
#define SECONDARY 0xaaaaaaaaaaaaaaaaULL
    ...
    int fail_count = 0;
    uint64_t *sched_latencies_ns =
        ((g_waketype == WAKE_CHAIN) || (g_waketype == WAKE_HOP))
            ? worst_latencies_from_previous_ns
            : worst_latencies_ns;
    ...
    for (uint32_t i = 0; i < g_iterations; i++) {
      ...
      if (g_test_rt_smt) {
        /* Test for one or more threads running on secondary cores unexpectedly
         * (WARNING) */
        secondary = (map & SECONDARY);
        /* Test for threads running on both primary and secondary cpus of the
         * same core (FAIL) */
      ...
      if (warn || secondary || fail) {
        printf("Iteration %d: 0x%llx worst latency %.2fus%s%s%s\n", i, map,
               sched_latencies_ns[i] / 1000.0, warn ? " WARNING" : "",
               secondary ? " SECONDARY" : "", fail ? " FAIL" : "");
      }
    ...
    if (test_fail && !g_test_strict_fail && (g_iterations >= 100) &&
        (fail_count <= g_iterations / 100)) {
...
static void selfexec_with_apptype(int argc, char *argv[]) {
  ...
  ret = posix_spawnattr_setprocesstype_np(&attr,
...
static void __attribute__((noreturn)) usage() {
  errx(EX_USAGE,
       "Usage: %s <threads> <chain | hop | broadcast-single-sem | "
       ...
       "[--verbose] [--spin-one] [--spin-all] [--spin-time <nanos>] "
       "[--affinity]\n\t\t"
...
static int32_t read_signed_dec_arg() {
  char *cp;
  /* char* optarg is a magic global */
...
static char read_cluster_type_arg() {
...
static void parse_args(int argc, char *argv[]) {
  ...
  static struct option longopts[] = {
      ...
      {NULL, 0, NULL, 0} /* END IGNORE CODESTYLE */
  };
  ...
  if (argc != 4) {
    warnx("Missing required <threads> <waketype> <policy> <iterations> "
          "arguments");
    usage();
  }
  ...
  if (cp == argv[0] || *cp) {
    errx(EX_USAGE, "numthreads requires a decimal number, found \"%s\"",
         argv[0]);
  }
