...
#define err(fmt, ...)                                                          \
  do {                                                                         \
    ...
    fprintf(stderr, "[%4d] [ERROR(%d:%s)] %s:%d: " fmt "\n", getuid(), errno,  \
            strerror(errno), __func__, __LINE__, ##__VA_ARGS__);               \
    fflush(stderr);                                                            \
    exit(ERR_SYSTEM);                                                          \
  } while (0)
...
static inline void _dump_kpersona(const char *msg, uint32_t flags,
                                  const struct kpersona_info *ki) {
  if (msg) {
    info("%s", msg);
  }
  info("\t kpersona_info (v%d) {", ki->persona_info_version);
  info("\t\t     %cid:  %d", flags & PA_HAS_ID ? '+' : '-', ki->persona_id);
  info("\t\t     %ctype:  %d", flags & PA_HAS_TYPE ? '+' : '-',
       ki->persona_type);
  ...
  info("\t\t  %clogin: \"%s\"", flags & PA_HAS_LOGIN ? '+' : '-',
       ki->persona_name);
  info("\t }");
}

#define dump_kpersona(msg, ki) _dump_kpersona(msg, 0xffffffff, ki)

static inline void dump_persona_args(const char *msg,
                                     const struct persona_args *pa) {
