...
static pid_t spawn_child(int argc, char **argv, struct persona_args *pa) {
  ...
  ret = posix_spawnattr_set_persona_np(&attr, pa->kinfo.persona_id,
                                       persona_flags);
  if (ret != 0) {
  ...
  if (pa->flags & PA_HAS_GROUPS) {
    ret = posix_spawnattr_set_persona_groups_np(
        &attr, pa->kinfo.persona_ngroups, pa->kinfo.persona_groups,
        KAUTH_UID_NONE);
    if (ret != 0) {
      err_print("");
      ret = -ERR_SPAWN_ATTR;
...
static void usage_main(const char *progname, int verbose) {
  ...
  printf("%s v%d.%d\n", PERSONA_TEST_NAME, PERSONA_TEST_VMAJOR,
         PERSONA_TEST_VMINOR);
  ...
  printf("\t%-10s\tVerify persona parameters against existing persona (given "
         "by -I)\n",
         "-V");
...
int main(int argc, char **argv) {
  ...
  if (geteuid() != 0) {
    err("%s must be run as root",
        argv[0] ? basename(argv[0]) : PERSONA_TEST_NAME);
  }
  ...
  if (g.wait_for_children) {
    ...
    if (WIFEXITED(status)) {
      ...
      if (status != 0) {
        errc(ERR_CHILD_FAIL, "Child exited with status: %d", status);
      }
    }
  }
