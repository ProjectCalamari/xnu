...
static int persona_op_lookup(struct kpersona_info *ki, pid_t pid, uid_t uid) {
  ...
  info("Looking up persona (login:%s, pid:%d, uid:%d)", ki->persona_name, pid,
       uid);
  if (pid > 0) {
    ki->persona_info_version = PERSONA_INFO_V1;
    ...
    while (np--) {
      ...
      if (ret < 0) {
        err("kpersona_info failed (errno:%d) for persona[%d]", errno,
            personas[np]);
      }
      dump_kpersona(NULL, ki);
    }
...
static void usage_main(const char *progname, const char *msg, int verbose) {
  ...
  printf("usage: %s [op] [-v] [-i id] [-t type] [-p pid] [-u uid] [-g gid] [-l "
         "login] [-G {groupspec}] [-m gmuid]\n",
         nm);
  if (!verbose) {
    exit(1);
  }
