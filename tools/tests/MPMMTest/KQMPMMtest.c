...
void setup_server_ports(struct port_args *ports) {
  ...
  ret = mach_port_insert_member(mach_task_self(), ports->port, ports->pset);
  if (KERN_SUCCESS != ret) {
...
void *server(void *serverarg) {
  ...
  EV_SET64(&kev[0], args.pset, EVFILT_MACHPORT,
           (EV_ADD | EV_CLEAR | EV_DISPATCH),
#if DIRECT_MSG_RCV
           MACH_RCV_MSG | MACH_RCV_LARGE, 0, 0, (mach_vm_address_t)args.req_msg,
           args.req_size);
#else
           0, 0, 0, 0, 0);
#endif
  ...
  retry:
...
#if DIRECT_MSG_RCV
             MACH_RCV_MSG | MACH_RCV_LARGE, 0, 0,
             (mach_vm_address_t)args.req_msg, args.req_size);
#else
             0, 0, 0, 0, 0);
#endif
    ...
    ret = mach_msg(
        ...
        args.req_size, args.pset, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
