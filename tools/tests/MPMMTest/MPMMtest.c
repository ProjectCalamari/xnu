...
enum { msg_type_trivial = 0, msg_type_inline = 1, msg_type_complex = 2 };
...
void usage(const char *progname) {
  ...
  fprintf(stderr, "    -verbose\t\tbe verbose (use multiple times to increase "
                  "verbosity)\n");
  ...
  fprintf(stderr, "    -set nset num\tcreate [nset] portsets and [num] ports "
                  "in each server.\n");
  fprintf(stderr, "                 \tEach port is connected to each set.\n");
  fprintf(stderr, "    -prepost\t\tstress the prepost system (implies "
                  "-threaded, requires -set X Y)\n");
...
void setup_server_ports(struct port_args *ports) {
  ...
  ports->req_size =
      MAX(sizeof(ipc_inline_message) + sizeof(u_int32_t) * num_ints,
          sizeof(ipc_complex_message));
  ports->reply_size = sizeof(ipc_trivial_message) - sizeof(mach_msg_trailer_t);
  ...
  for (int i = 0; i < portcount; i++) {
    ...
    if (useset) {
      ...
      for (int ns = 0; ns < setcount; ns++) {
        ...
        ret = mach_port_insert_member(mach_task_self(), port, ports->set[ns]);
        if (KERN_SUCCESS != ret) {
  ...
  if (stress_prepost) {
    ...
    for (int i = 0; i < portcount; i++) {
      ret =
          mach_port_insert_right(mach_task_self(), ports->port_list[i],
                                 ports->port_list[i], MACH_MSG_TYPE_MAKE_SEND);
      if (KERN_SUCCESS != ret) {
    ...
    ret = mach_port_insert_right(mach_task_self(), ports->port, ports->port,
  ...
  ret = bootstrap_register(bsport, server_port_name[ports->server_num],
                           ports->port);
  if (KERN_SUCCESS != ret) {
...
void setup_client_ports(struct port_args *ports) {
  ...
  case msg_type_inline:
    ports->req_size = sizeof(ipc_inline_message) + sizeof(u_int32_t) * num_ints;
    break;
  ...
  if (verbose) {
    printf("Client sending %d %s IPC messages to port '%s' in %s mode\n",
           ...
           (msg_type == msg_type_inline)
               ? "inline"
               : ((msg_type == msg_type_complex) ? "complex" : "trivial"),
           server_port_name[ports->server_num], (oneway ? "oneway" : "rpc"));
  }
}

static void thread_setup(int tag) {
  kern_return_t ret;
  ...
  if (!timeshare) {
    ...
    ret = thread_policy_set(mach_thread_self(), THREAD_EXTENDED_POLICY,
  ...
  if (affinity) {
    ...
    ret = thread_policy_set(mach_thread_self(), THREAD_AFFINITY_POLICY,
...
void *server(void *serverarg) {
  ...
  for (idx = 0; idx < totalmsg; idx++) {
    ...
    ret = mach_msg(
        args->req_msg, MACH_RCV_MSG | MACH_RCV_INTERRUPT | MACH_RCV_LARGE, 0,
        args->req_size, recv_port, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
    ...
    if (args->req_msg->msgh_bits & MACH_MSGH_BITS_COMPLEX) {
      ret = vm_deallocate(
          mach_task_self(),
          (vm_address_t)((ipc_complex_message *)args->req_msg)
              ->descriptor.address,
    ...
    if (1 == args->req_msg->msgh_id) {
      ...
      ret = mach_msg(args->reply_msg, MACH_SEND_MSG, args->reply_size, 0,
  ...
  for (int ns = 0; ns < setcount; ns++) {
    ...
    ret = mach_port_mod_refs(mach_task_self(), args->set[ns],
...
static inline void client_spin_loop(unsigned count, void(fn)(void)) {
  while (count--) {
    fn();
  }
}
...
static void client_work_atom(void) {
  static int i;
...
static void *calibrate_client_work(void) {
  long dummy;
...
static void *client_work(void) {
  if (client_spin) {
    client_spin_loop(calibration_count * client_spin, client_work_atom);
  }
...
void *client(void *threadarg) {
  ...
  ret = bootstrap_look_up(bsport, server_port_name[server_num], &servport);
  if (KERN_SUCCESS != ret) {
  ...
  for (idx = 0; idx < num_msgs; idx++) {
    ...
    if (verbose > 2) {
      printf("client sending message %d to port %#x\n", idx,
             req->msgh_remote_port);
    }
    starttm = mach_absolute_time();
    ret = mach_msg(req, MACH_SEND_MSG, args.req_size, 0, MACH_PORT_NULL,
    ...
    if (!oneway) {
      ...
      ret = mach_msg(args.reply_msg, MACH_RCV_MSG | MACH_RCV_INTERRUPT, 0,
                     args.reply_size, args.port, MACH_MSG_TIMEOUT_NONE,
                     MACH_PORT_NULL);
      if (MACH_MSG_SUCCESS != ret) {
        mach_error("mach_msg (receive): ", ret);
        fprintf(stderr, "bailing after %u iterations\n", idx);
...
static void thread_spawn(thread_id_t *thread, void *(fn)(void *), void *arg) {
  if (threaded) {
    kern_return_t ret;
    ret = pthread_create(&thread->tid, NULL, fn, arg);
    if (ret != 0) {
...
static void wait_for_servers(void) {
  int i;
  int retry_count = 10;
  mach_port_t bsport, servport;
  kern_return_t ret;
  ...
  while (retry_count-- > 0) {
    for (i = 0; i < num_servers; i++) {
      ret = bootstrap_look_up(bsport, server_port_name[i], &servport);
      if (ret != KERN_SUCCESS) {
        break;
      }
    }
...
int main(int argc, char *argv[]) {
  ...
  server_port_args =
      (struct port_args *)calloc(sizeof(struct port_args), num_servers);
  ...
  printf("%d server%s, %d client%s per server (%d total) %u messages...",
         num_servers, (num_servers > 1) ? "s" : "", num_clients,
         (num_clients > 1) ? "s" : "", totalclients, totalmsg);
  fflush(stdout);
  ...
  for (i = 0; i < num_servers; i++) {
    for (j = 0; j < num_clients; j++) {
      thread_spawn(&client_id[(i * num_clients) + j], client, (void *)(long)i);
    }
  }
  ...
  double dsecs = (double)deltatv.tv_sec + 1.0E-6 * (double)deltatv.tv_usec;
  ...
  double time_in_sec =
      (double)deltatv.tv_sec + (double)deltatv.tv_usec / 1000.0;
