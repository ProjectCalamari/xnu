...
clock_t testt(boolean_t superpages, int mode, int write, int kb) {
  static int sum;
  char *data;
  ...
  int pages = kb / 4;
  mach_vm_size_t size =
      SUPERPAGE_ROUND_UP(pages * PAGE_SIZE); /* allocate full superpages */
  int kr;

  kr = mach_vm_allocate(
      mach_task_self(), &addr, size,
      VM_FLAGS_ANYWHERE |
          (superpages ? VM_FLAGS_SUPERPAGE_SIZE_2MB : VM_FLAGS_SUPERPAGE_NONE));
...
int main(int argc, char **argv) {
  ...
  printf("; m0 r s; m0 r b; m0 w s; m0 w b; m1 r s; m1 r b; m1 w s; m1 w b; m2 "
         "r s; m2 r b; m2 w s; m2 w b\n");
