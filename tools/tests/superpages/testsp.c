...
#ifdef __LP64__
#define FIXED_ADDRESS1                                                         \
  (0x100000000ULL + 500 * 1024 * 1024) /* at 4 GB + 500 MB virtual */
#define FIXED_ADDRESS2                                                         \
  (0x100000000ULL + 502 * 1024 * 1024 +                                        \
   4 * 1024) /* at 4 GB + 502 MB + 4 KB virtual */
#else
#define FIXED_ADDRESS1 (500 * 1024 * 1024) /* at 500 MB virtual */
#define FIXED_ADDRESS2                                                         \
  (502 * 1024 * 1024 + 4 * 1024) /* at 502 MB + 4 KB virtual */
#endif

char error[100];

jmp_buf resume;
void test_signal_handler(int signo) { longjmp(resume, signo); }

char *signame[32] = {[SIGBUS] "SIGBUS", [SIGSEGV] "SIGSEGV"};
...
boolean_t check_kr(int kr, char *fn) {
  if (kr) {
...
boolean_t check_addr0(mach_vm_address_t addr, char *fn) {
  if (!addr) {
...
boolean_t check_addr(mach_vm_address_t addr1, mach_vm_address_t addr2,
                     char *fn) {
  if (addr1 != addr2) {
    sprintf(error, "%s() returned address %llx instead of %llx", fn, addr1,
            addr2);
    return FALSE;
  }
  return TRUE;
...
boolean_t check_align(mach_vm_address_t addr) {
...
boolean_t check_r(mach_vm_address_t addr, mach_vm_size_t size, int *res) {
...
boolean_t check_nr(mach_vm_address_t addr, mach_vm_size_t size, int *res) {
  int i;
  boolean_t ret;
...
boolean_t check_w(mach_vm_address_t addr, mach_vm_size_t size) {
...
boolean_t check_nw(mach_vm_address_t addr, mach_vm_size_t size) {
  int i;
  boolean_t ret;
...
boolean_t check_rw(mach_vm_address_t addr, mach_vm_size_t size) {
  int ret;
  int res;
 ...
 */
boolean_t test_allocate() {
  int kr, ret;

  global_addr = 0;
  global_size = SUPERPAGE_SIZE;

  kr = mach_vm_allocate(mach_task_self(), &global_addr, global_size,
...
/*
 * If we allocate a superpage of any size read-write without specifying an
 * address
 * - the call should succeed
 * - not return 0
 * - the memory should be readable and writable
 ...
 * - make the memory inaccessible
 */
boolean_t test_allocate_size_any() {
  int kr;
  int ret;
  mach_vm_address_t addr = 0;
  mach_vm_size_t size =
      2 * PAGE_SIZE; /* will be rounded up to some superpage size */
 ...
 */
boolean_t test_allocateunalignedfixed() {
  int kr;
  int ret;
 ...
 * - the call should fail
 */
boolean_t test_allocateoddsize() {
  int kr;
  int ret;
  mach_vm_address_t addr = FIXED_ADDRESS1;
 ...
 */
boolean_t test_deallocatesubpage() {
  int kr;
  int ret;
  mach_vm_address_t addr = 0;
  mach_vm_size_t size = SUPERPAGE_SIZE;
...
boolean_t test_wire() {
  ...
  kr = mach_vm_wire(mach_host_self(), mach_task_self(), addr, size,
 ...
 */
boolean_t test_readonlysubpage() {
  int kr;
  int ret;
  mach_vm_address_t addr = 0;
  mach_vm_size_t size = SUPERPAGE_SIZE;
  ...
  mach_vm_protect(mach_task_self(), addr + PAGE_SIZE, PAGE_SIZE, 0,
                  VM_PROT_READ);
  if (!(ret = check_kr(kr, "mach_vm_protect"))) {
    return ret;
  }
...
boolean_t test_fileio() {
  ...
  kr = mach_vm_allocate(mach_task_self(), &addr1, size,
                        VM_FLAGS_ANYWHERE | VM_FLAGS_SUPERPAGE_SIZE_2MB);
  ...
  if ((bytes = read(fd, (void *)(uintptr_t)addr1, SUPERPAGE_SIZE)) <
      SUPERPAGE_SIZE) {
  ...
  if ((bytes = read(fd, (void *)(uintptr_t)addr2, SUPERPAGE_SIZE)) <
      SUPERPAGE_SIZE) {
...
boolean_t test_mmap() {
  ...
  addr = (uintptr_t)mmap((void *)addr, size, PROT_READ, MAP_ANON | MAP_PRIVATE,
...
test_t test[] = {
    ...
    {"allocate one page at an unaligned fixed address",
