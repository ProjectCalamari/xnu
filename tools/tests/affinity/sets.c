  ...
  do {
    ...
    while (1) {
      ...
      DBG("    %s[%d,%d] iteration %d waiting for buffer\n", sp->name,
          lp->setnum, sp->stagenum, iteration);
...
static void auto_config(int npages, int *nbufs, int *nsets) {
  size_t len;
  int ncpu;
  int llc;
  ...
  if (sysctlbyname("hw.cachesize", &cachesize[0], &len, NULL, 0) != 0) {
...
int main(int argc, char *argv[]) {
  ...
  mutter("%d buffer%s per set ", buffers, s_if_plural(buffers));
  ...
  for (i = 0; i < sets; i++) {
    ...
    for (j = 0; j < stages; j++) {
      if (ret = pthread_create(&lp->stage[j]->thread, NULL, &manager_fn,
                               (void *)lp->stage[j])) {
