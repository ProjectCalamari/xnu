...
static void usage() {
  fprintf(stderr, "usage: pool [-a]    Turn affinity on (off)\n"
...
void writer_fn(int *data, int isize) {
  int i;
...
void reader_fn(int *data, int isize) {
  int i;
  int datum;
...
void reader_writer_fn(int *data, int isize) {
  int i;
...
void *manager_fn(void *arg) {
  ...
  do {
    ...
    DBG("  %s[%d,%d] todo %d work %p data %p\n", sp->name, wp->setnum,
        sp->stagenum, iteration, workp, workp->data);
...
int main(int argc, char *argv[]) {
  ...
  mutter("Launching %d producer%s with %d stage%s of %d consumer%s\n"
         ...
         producers, s_if_plural(producers), stages - 1, s_if_plural(stages - 1),
         consumers, s_if_plural(consumers), affinity ? "" : "no ",
  ...
  mutter("%d buffer%s per producer ", buffers, s_if_plural(buffers));
  ...
  for (i = 0; i < producers; i++) {
    ...
    if (ret = pthread_create(&wp->thread, NULL, &manager_fn, (void *)wp)) {
