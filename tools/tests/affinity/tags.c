...
static void usage() {
  fprintf(stderr, "usage: tags [-i]   interactive/input\n"
...
void thread_tag_set(thread_t thread, int tag) {
  kern_return_t ret;
...
int thread_tag_get(thread_t thread) {
  kern_return_t ret;
  boolean_t get_default = FALSE;
  thread_affinity_policy_data_t policy;
  mach_msg_type_number_t count = THREAD_AFFINITY_POLICY_COUNT;

  ret = thread_policy_get(thread, THREAD_AFFINITY_POLICY,
...
int main(int argc, char *argv[]) {
  ...
  while (interactive) {
    ...
    for (i = 0; i < num_threads; i++) {
      ...
      printf(" %d: thread 0x%08x tag %d: ", i, thread_array[i], tag);
      fflush(stdout);
