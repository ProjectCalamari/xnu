...
bool testthreadcall::start(IOService *provider) {
  boolean_t ret;
  uint64_t deadline;
  int sleepret;
  ...
  IOLog("%s: Diagnostic API %s\n", __PRETTY_FUNCTION__,
        (kernel_configuration & kPEICanHasDiagnosticAPI) ? "enabled"
                                                         : "disabled");
  ...
  IOLog("%d sec deadline is %llu\n", 3, deadline);
  sleepret =
      IOLockSleepDeadline(tlock2, &my_event, deadline, THREAD_INTERRUPTIBLE);
