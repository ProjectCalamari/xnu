/* quick and dirty hack to grab credential backtrace info from kernel via
 * sysctl. sysctl is only defined if xnu is built with DEBUG_CRED defined. The
 * current version of this is used to target a specific credential and gather
