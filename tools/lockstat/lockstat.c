 ...
 *	Usage:
 *	lockstat [all, spin, mutex, rw, <lock group name>] {<repeat interval>}
 * {abs}
 *
...
void usage() {
  fprintf(stderr,
          "Usage: %s [all, spin, mutex, rw, <lock group name>] {<repeat "
          "interval>} {abs}\n",
          pgmname);
  exit(EXIT_FAILURE);
}
...
#if defined(__i386__) || defined(__x86_64__)
    printf("%10lld %10lld %10lld   ", curptr->lock_mtx_miss_cnt,
           curptr->lock_mtx_wait_cnt, curptr->lock_mtx_held_cnt);
#else
    printf("%16lld %16lld   ", curptr->lock_mtx_miss_cnt,
           curptr->lock_mtx_wait_cnt);
#endif
    printf("%-14s\n", curptr->lockgroup_name);
  }
}
...
void print_rw(int requested, lockgroup_info_t *lockgroup) {
  ...
  if (curptr->lock_rw_cnt != 0 && curptr->lock_rw_util_cnt != 0) {
    ...
    printf("%16lld %16lld   ", curptr->lock_rw_miss_cnt,
           curptr->lock_rw_wait_cnt);
    printf("%-14s\n", curptr->lockgroup_name);
  }
}
...
void get_lockgroup_deltas(void) {
  ...
  for (i = 0; i < count; i++) {
    ...
    lockgroup_deltas[i].lock_rw_util_cnt = lockgroup_info[i].lock_rw_util_cnt -
                                           lockgroup_start[i].lock_rw_util_cnt;
    lockgroup_deltas[i].lock_rw_miss_cnt = lockgroup_info[i].lock_rw_miss_cnt -
                                           lockgroup_start[i].lock_rw_miss_cnt;
    lockgroup_deltas[i].lock_rw_wait_cnt = lockgroup_info[i].lock_rw_wait_cnt -
