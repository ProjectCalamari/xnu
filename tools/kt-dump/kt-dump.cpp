...
template <typename T> struct macho_section {
  ...
  size_t elem_count() const { return section.size / elem_size(); }
};

int printf_with_indent(const char *indent, const char *format, ...) {
  int n = 0;
...
static inline const char *decode_string(const macho_section<char> &sec_cstring,
                                        const char *string) {
  /*
...
class image {
  ...
  std::set<std::tuple<const char *, const char *, const char *>>
      dedup_entries_var;
  ...
  void dump_types(const char *indent) {
    const char *sep = "\n";
  ...
  void dump_types_var(const char *indent) {
    const char *sep = "\n";
    ...
    for (auto &ktv : sec_types_var->contents) {
      ...
      /* Only output the equal entries (same name/signature) once */
      if (!dedup_entries_var.insert(std::make_tuple(name, sig_hdr, sig_type))
               .second) {
        continue;
      }
      ...
      case out_fmt_type::SIMPLE:
        if (sig_hdr) {
          std::cout << indent << name << " [" << sig_hdr << ", " << sig_type
                    << "]\n";
        } else {
          std::cout << indent << name << " [, " << sig_type << "]\n";
        }
        break;
      case out_fmt_type::JSON:
        std::cout << sep << indent << "    { \"name\": \"" << name << "\", ";
  ...
  const mach_header_64 *mh_hdr() const {
    return reinterpret_cast<const mach_header_64 *>(slice_contents.data() +
                                                    slice_mh_offs);
  }
...
public:
  image(std::span<uint8_t> contents, size_t mh_offs = 0)
    ...
    for (size_t i = 0; i < hdr->ncmds; i++) {
      ...
      const std::span<section_64> sections((section_64 *)(seg_cmd + 1),
                                           seg_cmd->nsects);
      ...
      for (auto &sec : sections) {
        ...
        if (sectname == "__kalloc_type") {
          ...
          sec_types_var =
              macho_section<kalloc_type_var_view>(sec, slice_contents);
  ...
  void dump(const std::string &imgname, const char *indent = "") {
    if (out_fmt == out_fmt_type::JSON) {
      std::cout << indent << "{\n"
    ...
    if (out_fmt == out_fmt_type::STATS) {
      if (auto &sec = *sec_types; sec_types) {
        ...
        std::cout << indent << "  total structs:    " << sec.elem_count()
                  << std::endl;
        std::cout << indent << "  unique structs:   " << ucount << std::endl;
        std::cout << indent << "  total  size:      " << sec.section.size
                  << std::endl;
      ...
      std::cout << indent << "names strings:      " << stats.names_sz
                << std::endl;
      std::cout << indent << "signatures strings: " << stats.sig_sz
                << std::endl;
    }
...
static int do_simple_macho(const std::string filename,
...
static int do_fat_macho(const std::string filename,
                        std::span<uint8_t> contents) {
...
static int do_fileset(std::span<uint8_t> contents) {
  ...
  for (size_t i = 0; i < hdr->ncmds; i++) {
    ...
    const char *name =
        reinterpret_cast<const char *>(cmd) + fec->entry_id.offset;
...
void read_file(std::filesystem::path &path, std::vector<uint8_t> &contents) {
...
static file_kind recognize_file(const std::vector<uint8_t> &contents) {
...
static int call_cmd_silent(const char *const *args) {
  ...
  posix_spawn_file_actions_addopen(&facts, STDIN_FILENO, "/dev/null", O_RDONLY,
                                   0777);
  posix_spawn_file_actions_addopen(&facts, STDOUT_FILENO, "/dev/null", O_WRONLY,
                                   0777);
  posix_spawn_file_actions_addopen(&facts, STDERR_FILENO, "/dev/null", O_WRONLY,
                                   0777);
  rc = posix_spawnp(&pid, args[0], &facts, nullptr, (char *const *)args,
                    environ);
...
static int do_file(const std::filesystem::path &path,
  ...
  static const char *const img4args[] = {
      "img4utility", "--copyBinary",        "--input", path.c_str(),
      "--output",    compressed_kc.c_str(), NULL,
  };
