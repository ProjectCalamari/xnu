...
int rsa_make_pub(rsa_pub_ctx *pub, size_t exp_nbytes, const uint8_t *exp,
  ...
  return g_crypto_funcs->ccrsa_make_pub_fn(pub->key, exp_nbytes, exp,
                                           mod_nbytes, mod);
}
...
int rsa_verify_pkcs1v15(rsa_pub_ctx *pub, const uint8_t *oid, size_t digest_len,
  ...
  return g_crypto_funcs->ccrsa_verify_pkcs1v15_fn(pub->key, oid, digest_len,
                                                  digest, sig_len, sig, valid);
}
