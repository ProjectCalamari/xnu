...
aes_rval aes_encrypt_key(const unsigned char *key, int key_len,
...
aes_rval aes_encrypt_cbc(const unsigned char *in_blk,
                         const unsigned char *in_iv, unsigned int num_blk,
  ...
  return cccbc_update(cbc, cx[0].ctx, ctx_iv, num_blk, in_blk,
                      out_blk); // Actually cbc encrypt.
}

#if defined(__i386__) || defined(__x86_64__) || defined(__arm64__)
/* This does one block of ECB, using the CBC implementation - this allow to use
 * the same context for both CBC and ECB */
aes_rval aes_encrypt(const unsigned char *in_blk, unsigned char *out_blk,
                     aes_encrypt_ctx cx[1]) {
  return aes_encrypt_cbc(in_blk, NULL, 1, out_blk, cx);
}
#endif

aes_rval aes_decrypt_key(const unsigned char *key, int key_len,
...
aes_rval aes_decrypt_cbc(const unsigned char *in_blk,
                         const unsigned char *in_iv, unsigned int num_blk,
  ...
  return cccbc_update(cbc, cx[0].ctx, ctx_iv, num_blk, in_blk,
                      out_blk); // Actually cbc decrypt.
}

#if defined(__i386__) || defined(__x86_64__) || defined(__arm64__)
/* This does one block of ECB, using the CBC implementation - this allow to use
 * the same context for both CBC and ECB */
aes_rval aes_decrypt(const unsigned char *in_blk, unsigned char *out_blk,
                     aes_decrypt_ctx cx[1]) {
...
aes_rval aes_encrypt_key_gcm(const unsigned char *key, int key_len,
                             ccgcm_ctx *ctx) {
...
aes_rval aes_encrypt_key_with_iv_gcm(const unsigned char *key, int key_len,
...
aes_rval aes_encrypt_set_iv_gcm(const unsigned char *in_iv, unsigned int len,
                                ccgcm_ctx *ctx) {
  const struct ccmode_gcm *gcm = g_crypto_funcs->ccaes_gcm_encrypt;
  if (!gcm) {
    return aes_error;
...
aes_rval aes_encrypt_aad_gcm(const unsigned char *aad, unsigned int aad_bytes,
                             ccgcm_ctx *ctx) {
  const struct ccmode_gcm *gcm = g_crypto_funcs->ccaes_gcm_encrypt;
  if (!gcm) {
    return aes_error;
...
aes_rval aes_encrypt_gcm(const unsigned char *in_blk, unsigned int num_bytes,
  ...
  return ccgcm_update(gcm, ctx, num_bytes, in_blk,
                      out_blk); // Actually gcm encrypt.
}

aes_rval aes_encrypt_finalize_gcm(unsigned char *tag, size_t tag_bytes,
                                  ccgcm_ctx *ctx) {
  const struct ccmode_gcm *gcm = g_crypto_funcs->ccaes_gcm_encrypt;
  if (!gcm) {
    return aes_error;
...
aes_rval aes_decrypt_key_gcm(const unsigned char *key, int key_len,
                             ccgcm_ctx *ctx) {
...
aes_rval aes_decrypt_key_with_iv_gcm(const unsigned char *key, int key_len,
                                     const unsigned char *in_iv,
                                     ccgcm_ctx *ctx) {
  const struct ccmode_gcm *gcm = g_crypto_funcs->ccaes_gcm_decrypt;
  if (!gcm) {
...
aes_rval aes_decrypt_set_iv_gcm(const unsigned char *in_iv, size_t len,
                                ccgcm_ctx *ctx) {
  const struct ccmode_gcm *gcm = g_crypto_funcs->ccaes_gcm_decrypt;
  if (!gcm) {
    return aes_error;
...
aes_rval aes_decrypt_aad_gcm(const unsigned char *aad, unsigned int aad_bytes,
                             ccgcm_ctx *ctx) {
  const struct ccmode_gcm *gcm = g_crypto_funcs->ccaes_gcm_decrypt;
  if (!gcm) {
    return aes_error;
...
aes_rval aes_decrypt_gcm(const unsigned char *in_blk, unsigned int num_bytes,
  ...
  return ccgcm_update(gcm, ctx, num_bytes, in_blk,
                      out_blk); // Actually gcm decrypt.
}

aes_rval aes_decrypt_finalize_gcm(unsigned char *tag, size_t tag_bytes,
                                  ccgcm_ctx *ctx) {
  const struct ccmode_gcm *gcm = g_crypto_funcs->ccaes_gcm_decrypt;
  if (!gcm) {
    return aes_error;
