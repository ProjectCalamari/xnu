...
uint32_t xts_start(uint32_t cipher
                       __unused, // ignored - we're doing this for xts-aes only
                   const uint8_t *IV __unused, // ignored
                   const uint8_t *key1, int keylen, const uint8_t *key2,
                   int tweaklen __unused, // both keys are the same size for xts
  ...
  if (!enc || !dec) {
    panic("%s: xts mode not registered? enc=%p, dec=%p", __FUNCTION__, enc,
          dec);
  }
...
int xts_encrypt(
    const uint8_t *pt, unsigned long ptlen, uint8_t *ct,
...
int xts_decrypt(
    const uint8_t *ct, unsigned long ptlen, uint8_t *pt,
    const uint8_t *iv, // this can be considered the sector IV for this use
    symmetric_xts *xts) {
