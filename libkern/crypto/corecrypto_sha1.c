...
static uint64_t getCount(SHA1_CTX *ctx) { return ctx->c.b64[0]; }

static void setCount(SHA1_CTX *ctx, uint64_t count) { ctx->c.b64[0] = count; }

/* Copy a ccdigest ctx into a legacy SHA1 context */
static void DiToSHA1(const struct ccdigest_info *di,
                     struct ccdigest_ctx *di_ctx, SHA1_CTX *sha1_ctx) {
...
static void SHA1ToDi(const struct ccdigest_info *di, SHA1_CTX *sha1_ctx,
...
typedef kern_return_t (*InKernelPerformSHA1Func)(
    void *ref, const void *data, size_t dataLen, u_int32_t *inHash,
    u_int32_t options, u_int32_t *outHash, Boolean usePhysicalAddress);
void sha1_hardware_hook(Boolean option, InKernelPerformSHA1Func func,
                        void *ref);
static void *SHA1Ref;
...
void sha1_hardware_hook(Boolean option, InKernelPerformSHA1Func func,
  ...
  if (option) {
    ...
    if (!OSCompareAndSwapPtr((void *)NULL, (void *)func,
