...
template <typename queue_t, typename entry_t> struct pqueue_entry_traits {
  /*
   * Explain how to compare two elements in the natural order.
   */
  static inline int compare(queue_t que, entry_t a, entry_t b);
};

template <typename queue_t>
struct pqueue_entry_traits<queue_t, priority_queue_entry_t> {
  static inline int compare(queue_t que, priority_queue_entry_t e1,
...
struct pqueue_entry_traits<queue_t, priority_queue_entry_deadline_t> {
  static inline int compare(queue_t que __unused,
                            priority_queue_entry_deadline_t e1,
...
struct pqueue_entry_traits<queue_t, priority_queue_entry_sched_t> {
  static inline int compare(queue_t que __unused,
                            priority_queue_entry_sched_t e1,
...
struct pqueue_entry_traits<queue_t, priority_queue_entry_stable_t> {
  static inline int compare(queue_t que __unused,
                            priority_queue_entry_stable_t e1,
 ...
 */
template <typename queue_t, typename entry_t> struct pqueue {
  using entry_traits = pqueue_entry_traits<queue_t, entry_t>;

  static inline void pack_child(entry_t e, const entry_t child) {
  ...
  static inline entry_t unpack_child(entry_t e) {
#if CONFIG_KERNEL_TAGGING
...
private:
  static inline bool merge_parent_is_subtree_b(queue_t que, entry_t subtree_a,
                                               entry_t subtree_b) {
  ...
  static inline entry_t merge_pair_inline(queue_t que, entry_t subtree_a,
                                          entry_t subtree_b) {
  ...
  OS_NOINLINE
  static entry_t merge_pair(queue_t que, entry_t subtree_a, entry_t subtree_b) {
    return merge_pair_inline(que, subtree_a, subtree_b);
  }

  OS_NOINLINE
  static entry_t meld_pair(queue_t que, entry_t elt) {
    ...
    /* Hook these pairs onto an intermediary list in reverse order of
     * traversal.*/
  ...
  static inline void list_clear(entry_t e) { e->next = e->prev = NULL; }

  static inline void list_remove(entry_t elt) {
  ...
  static inline bool sift_down(queue_t que, entry_t elt) {
  ...
  static inline bool sift_up(queue_t que, entry_t elt) {
    if (elt == que->pq_root) {
      return true;
    }
  ...
  static inline entry_t remove_non_root(queue_t que, entry_t elt) {
...
public:
  ...
  OS_NOINLINE
  static void destroy(queue_t que, uintptr_t offset,
  ...
  static inline bool insert(queue_t que, entry_t elt, bool clear = true) {
    if (clear) {
      list_clear(elt);
      pack_child(elt, nullptr);
    }
  ...
  static inline entry_t remove_root(queue_t que, entry_t old_root) {
  ...
  static inline bool remove(queue_t que, entry_t elt) {
    if (elt == que->pq_root) {
      remove_root(que, elt);
      return true;
    } else {
  ...
  static inline bool entry_increased(queue_t que, entry_t elt) {
  ...
  static inline bool entry_decreased(queue_t que, entry_t elt) {
...
#define PRIORITY_QUEUE_MAKE_IMPL(pqueue_t, queue_t, entry_t)                   \
  ...
  extern "C" {                                                                 \
                                                                               \
  __pqueue_overloadable void _priority_queue_destroy(queue_t que,              \
                                                     uintptr_t offset,         \
                                                     void (^cb)(void *e)) {    \
    pqueue_t::destroy(que, offset, cb);                                        \
  }                                                                            \
                                                                               \
  __pqueue_overloadable extern bool priority_queue_insert(queue_t que,         \
                                                          entry_t elt) {       \
  ...
  _priority_queue_remove_root(queue_t que) {                                   \
    return pqueue_t::remove_root(que, que->pq_root);                           \
                                                                               ...
                                                                               \
  __pqueue_overloadable extern bool priority_queue_remove(queue_t que,         \
                                                          entry_t elt) {       \
...
PRIORITY_QUEUE_MAKE_IMPL(pqueue_min_t, struct priority_queue_min *,
                         priority_queue_entry_t);
PRIORITY_QUEUE_MAKE_IMPL(pqueue_max_t, struct priority_queue_max *,
                         priority_queue_entry_t);

PRIORITY_QUEUE_MAKE_IMPL(pqueue_sched_min_t, struct priority_queue_sched_min *,
                         priority_queue_entry_sched_t);
PRIORITY_QUEUE_MAKE_IMPL(pqueue_sched_max_t, struct priority_queue_sched_max *,
                         priority_queue_entry_sched_t);
