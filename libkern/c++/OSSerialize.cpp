...
static inline kmem_guard_t OSSerialize_guard() {
  kmem_guard_t guard = {
...
char *OSSerialize::text() const { return data; }
...
bool OSSerialize::addXMLStartTag(const OSMetaClassBase *o,
...
bool OSSerialize::addChar(const char c) {
  ...
  if (length >= capacity &&
      length >= ensureCapacity(capacity + capacityIncrement)) {
    return false;
  }
...
bool OSSerialize::initWithCapacity(unsigned int inCapacity) {
  ...
  kmr = kmem_alloc_guard(kernel_map, inCapacity, /* mask */ 0,
                         (kma_flags_t)(KMA_ZERO | KMA_DATA_SHARED),
                         OSSerialize_guard());
...
OSSharedPtr<OSSerialize> OSSerialize::withCapacity(unsigned int inCapacity) {
...
unsigned int OSSerialize::getCapacityIncrement() const {
...
unsigned int OSSerialize::ensureCapacity(unsigned int newCapacity) {
  ...
  kmr = kmem_realloc_guard(kernel_map, (vm_offset_t)data, capacity, newCapacity,
                           (kmr_flags_t)(KMR_ZERO | KMR_DATA | KMR_FREEOLD),
                           OSSerialize_guard());
...
void OSSerialize::free() {
  if (capacity) {
    kmem_free_guard(kernel_map, (vm_offset_t)data, capacity, KMF_NONE,
                    OSSerialize_guard());
...
OSDefineMetaClassAndStructors(OSSerializer, OSObject)
    ...
    OSSharedPtr<OSSerializer> OSSerializer::forTarget(
        void *target, OSSerializerCallback callback, void *ref) {
...
OSSharedPtr<OSSerializer> OSSerializer::withBlock(OSSerializerBlock callback) {
  ...
  serializer =
      (OSSerializer::forTarget(NULL, &OSSerializer::callbackToBlock, block));
