...
int OSObject::getRetainCount() const { return (int)((UInt16)retainCount); }

bool OSObject::taggedTryRetain(const void *tag) const {
  ...
  do {
    ...
    newCount = origCount + inc;
      ...
      !OSCompareAndSwap(origCount, newCount, const_cast<UInt32 *>(countP)));
...
void OSObject::taggedRelease(const void *tag) const { taggedRelease(tag, 1); }
...
void OSObject::release() const { taggedRelease(NULL); }

void OSObject::retain() const { taggedRetain(NULL); }

extern "C" void osobject_retain(void *object) {
  ((OSObject *)object)->retain();
}

extern "C" void osobject_release(void *object) {
  ((OSObject *)object)->release();
}

void OSObject::release(int when) const { taggedRelease(NULL, when); }
...
void *OSObject_typed_operator_new(kalloc_type_view_t ktv, vm_size_t size) {
#if IOTRACKING
  if (kIOTracking & gIOKitDebug) {
    return OSMetaClass::trackedNew(size);
  }
...
void OSObject_typed_operator_delete(kalloc_type_view_t ktv, void *mem,
                                    vm_size_t size) {
  if (!mem) {
    return;
  }
...
bool OSObject::iterateObjects(void *refcon,
...
bool OSObject::iterateObjects(bool (^block)(OSObject *object)) {
  OSCollection *col;
  if ((col = OSDynamicCast(OSCollection, this))) {
