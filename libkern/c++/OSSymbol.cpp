...
class OSSymbolPool {
  ...
  static inline smrh_key_t OSSymbol_get_key(const OSSymbol *sym) {
    return {.smrk_string = sym->string, .smrk_len = (size_t)(sym->length - 1)};
  }

  static uint32_t OSSymbol_obj_hash(const struct smrq_slink *link,
                                    uint32_t seed) {
  ...
  static bool OSSymbol_obj_equ(const struct smrq_slink *link, smrh_key_t key) {
    OSSymbol *sym = __container_of(link, OSSymbol, hashlink);

    return smrh_key_equ_str(OSSymbol_get_key(sym), key);
  }

  static bool OSSymbol_obj_try_get(void *obj) {
    OSSymbol *sym = (OSSymbol *)obj;

    return (sym->flags & kOSSSymbolPermanent) || sym->taggedTryRetain(nullptr);
  }

  SMRH_TRAITS_DEFINE_STR(hash_traits, OSSymbol, hashlink, .domain = &smr_iokit,
...
private:
  inline void lock() const { lck_mtx_lock(&_mutex); }

  inline void unlock() const { lck_mtx_unlock(&_mutex); }

  inline bool shouldShrink() const {
...
public:
  ...
  OSSymbolPool() {
    ...
    _tcall = thread_call_allocate_with_options(
        rehash, this, THREAD_CALL_PRIORITY_KERNEL, THREAD_CALL_OPTIONS_ONCE);
  }
  ...
  void insertSymbol(OSSharedPtr<OSSymbol> &sym, smrh_key_t key,
...
OSSymbolPool &OSSymbolPool::instance() {
  return reinterpret_cast<OSSymbolPool &>(OSSymbolPoolStorage);
}

static inline bool OSSymbol_is_huge(size_t size) {
  return size > KALLOC_SAFE_ALLOC_SIZE;
}

OSSharedPtr<const OSSymbol> OSSymbolPool::findSymbol(smrh_key_t key) const {
  OSSymbol *sym;
  ...
  if (!OSSymbol_is_huge(key.smrk_len)) {
    ...
    if (key.smrk_len <= sizeof(tmp_buf)) {
      ...
      copy_s = (char *)kalloc_data(key.smrk_len, Z_WAITOK_ZERO_NOFAIL);
...
void OSSymbolPool::insertSymbol(OSSharedPtr<OSSymbol> &symToInsert,
                                smrh_key_t key, bool make_permanent) {
  OSSymbol *sym;
  ...
  if (!OSSymbol_is_huge(key.smrk_len)) {
    sym = smr_hash_serialized_get_or_insert(&_hash, key, &symToInsert->hashlink,
                                            &hash_traits);
    ...
    sym = (OSSymbol *)__smr_hash_serialized_find(&_huge_head, key,
                                                 &hash_traits.smrht);
    if (!sym || !OSSymbol_obj_try_get(sym)) {
      smrq_serialized_insert_head(&_huge_head, &symToInsert->hashlink);
      _hugeCount++;
      sym = NULL;
    }
  }
...
void OSSymbolPool::checkForPageUnload(void *startAddr, void *endAddr) {
  OSSymbol *sym;
  char *s;
  bool mustSync = false;
  ...
  smr_hash_foreach(sym, &_hash, &hash_traits) {
    if (sym->string >= startAddr && sym->string < endAddr) {
      ...
      s = (char *)kalloc_data(sym->length, Z_WAITOK_ZERO);
      if (s) {
...
static void OSSymbol_smr_free(void *sym, vm_size_t size __unused) {
...
OSSharedPtr<const OSSymbol> OSSymbol::withString(const OSString *aString) {
...
OSSharedPtr<const OSSymbol> OSSymbol::withCString(const char *cString) {
...
OSSharedPtr<const OSSymbol> OSSymbol::withCStringNoCopy(const char *cString) {
  auto &pool = OSSymbolPool::instance();
  smrh_key_t key = {
      .smrk_string = cString,
      .smrk_len = strnlen(cString, kMaxStringLength),
...
void OSSymbol::taggedRetain(const void *tag) const {
...
void *OSSymbol::operator new(size_t size __unused) {
  return zalloc_smr(OSSymbol_zone, Z_WAITOK_ZERO_NOFAIL);
}

void OSSymbol::operator delete(void *mem, size_t size) {
  /*
...
unsigned int OSSymbol::bsearch(const void *key, const void *array,
                               unsigned int arrayCount, size_t memberSize) {
  const void **p;
...
static int iokit_symbol_basic_test(int64_t size, int64_t *out) {
