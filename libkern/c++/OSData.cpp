...
bool OSData::initWithData(const OSData *inData, unsigned int start,
...
OSSharedPtr<OSData> OSData::withCapacity(unsigned int inCapacity) {
...
OSSharedPtr<OSData> OSData::withBytes(const void *bytes,
                                      unsigned int inLength) {
  OSSharedPtr<OSData> me = OSMakeShared<OSData>();
...
OSSharedPtr<OSData> OSData::withBytesNoCopy(void *bytes,
                                            unsigned int inLength) {
  OSSharedPtr<OSData> me = OSMakeShared<OSData>();
...
OSSharedPtr<OSData> OSData::withData(const OSData *inData) {
  OSSharedPtr<OSData> me = OSMakeShared<OSData>();
...
OSSharedPtr<OSData> OSData::withData(const OSData *inData, unsigned int start,
                                     unsigned int inLength) {
  OSSharedPtr<OSData> me = OSMakeShared<OSData>();
...
unsigned int OSData::getCapacityIncrement() const { return capacityIncrement; }
...
bool OSData::clipForCopyout() {
  ...
  if (length >= msg_ool_size_small && newCapacity < capacity) {
    kr = krealloc_ext(GET_KEXT_KHEAP_DATA(), data, capacity, newCapacity,
                      Z_VM_TAG_BT(Z_WAITOK_ZERO | Z_FULLSIZE | Z_MAY_COPYINMAP,
                                  VM_KERN_MEMORY_LIBKERN),
                      (void *)&this->data);
    assert(kr.addr == data);
...
bool OSData::isEqualTo(const OSString *obj) const {
  ...
  if (stringLen != checkLen) {
    ...
    if ((checkLen - stringLen) == 1) {
      if (dataPtr[checkLen - 1] !=
          0) { // non-zero means not a terminator and thus not likely the same
        return false;
      }
      checkLen--;
    } else {
...
static const char __CFPLDataEncodeTable[] =
...
void OSData::setDeallocFunction(DeallocFunction func) {
  if (!reserved) {
    reserved = (typeof(reserved))kalloc_type(
        ExpansionData, (zalloc_flags_t)(Z_WAITOK | Z_ZERO));
    if (!reserved) {
      return;
    }
  }
