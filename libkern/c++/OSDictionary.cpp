...
int OSDictionary::dictEntry::compare(const void *_e1, const void *_e2) {
...
void OSDictionary::sortBySymbol(void) {
...
OSSharedPtr<OSDictionary> OSDictionary::withCapacity(unsigned int capacity) {
...
OSSharedPtr<OSDictionary> OSDictionary::withObjects(const OSObject *objects[],
                                                    const OSSymbol *keys[],
                                                    unsigned int count,
...
OSSharedPtr<OSDictionary> OSDictionary::withDictionary(const OSDictionary *dict,
                                                       unsigned int capacity) {
  OSSharedPtr<OSDictionary> me = OSMakeShared<OSDictionary>();
...
unsigned int OSDictionary::ensureCapacity(unsigned int newCapacity) {
  ...
  newDict = kreallocp_type_container(dictEntry, dictionary, capacity,
...
bool OSDictionary::setObject(const OSSymbol *aKey,
                             const OSMetaClassBase *anObject, bool onlyAdd) {
  unsigned int i;
  bool exists;
...
bool OSDictionary::setObject(
    const OSString *aKey, OSSharedPtr<const OSMetaClassBase> const &anObject) {
  return setObject(aKey, anObject.get());
}

bool OSDictionary::setObject(
    const char *aKey, OSSharedPtr<const OSMetaClassBase> const &anObject) {
  return setObject(aKey, anObject.get());
}
...
void OSDictionary::removeObject(const OSSymbol *aKey) {
  ...
  if (exists) {
    ...
    count--;
    bcopy(&dictionary[i + 1], &dictionary[i],
          (count - i) * sizeof(dictionary[0]));
...
bool OSDictionary::merge(const OSDictionary *srcDict) {
  ...
  iter =
      OSCollectionIterator::withCollection(const_cast<OSDictionary *>(srcDict));
  if (!iter) {
    return false;
  }
  ...
  if (count > 4 && (fOptions & kSort)) {
    while (l < r) {
      ...
      if (aKey == dictionary[i].key) {
        return const_cast<OSObject *>(
            (const OSObject *)dictionary[i].value.get());
      }
...
bool OSDictionary::setObject(const OSString *aKey,
                             const OSMetaClassBase *anObject)
    OBJECT_WRAP_2(
        setObject, OSSymbol::withString(aKey),
        anObject) bool OSDictionary::setObject(const char *aKey,
                                               const OSMetaClassBase *anObject)
    ...
    OBJECT_WRAP_1(getObject, OSSymbol::existingSymbolForCString(aKey))
        ...
        void OSDictionary::removeObject(const OSString *aKey) OBJECT_WRAP_3(
            removeObject,
            OSSymbol::existingSymbolForString(
                aKey)) void OSDictionary::removeObject(const char *aKey)
            OBJECT_WRAP_3(removeObject,
                          OSSymbol::existingSymbolForCString(aKey))

                bool OSDictionary::isEqualTo(const OSDictionary *srcDict,
...
unsigned int OSDictionary::iteratorSize() const { return sizeof(unsigned int); }
...
bool OSDictionary::getNextObjectForIterator(void *inIterator,
                                            OSObject **ret) const {
  unsigned int *iteratorP = (unsigned int *)inIterator;
...
OSDictionary::copyCollection(OSDictionary *cycleDict) {
  ...
  do {
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      OSTaggedSharedPtr<OSCollection, OSCollection> coll(
          OSDynamicCast(OSCollection, EXT_CAST(obj)), OSNoRetain);
...
OSSharedPtr<OSArray> OSDictionary::copyKeys(void) {
...
bool OSDictionary::iterateObjects(void *refcon,
  ...
  for (unsigned int i = 0; i < count; i++) {
    done = callback(refcon, dictionary[i].key.get(),
                    EXT_CAST(dictionary[i].value.get()));
    if (done) {
      break;
    }
...
static bool OSDictionaryIterateObjectsBlock(void *refcon, const OSSymbol *key,
                                            OSObject *object) {
  bool (^block)(const OSSymbol *key, OSObject *object) = (typeof(block))refcon;
  return block(key, object);
}

bool OSDictionary::iterateObjects(bool (^block)(const OSSymbol *key,
                                                OSObject *object)) {
