...
#include <ptrauth.h>
#define PTRAUTH_STRIP_STRUCTOR(x)                                              \
  ((uintptr_t)ptrauth_strip(ptrauth_nop_cast(void *, (x)),                     \
...
#if __GNUC__ >= 3
void __dead2 __cxa_pure_virtual(void) { panic("%s", __FUNCTION__); }
#else
void __dead2 __pure_virtual(void) { panic("%s", __FUNCTION__); }
#endif
...
static bool OSRuntimeCallStructorsInSection(
    OSKext *theKext, kmod_info_t *kmodInfo, void *metaHandle,
    kernel_segment_command_t *segment, const char *sectionName,
    uintptr_t textStart, uintptr_t textEnd) {
  kernel_section_t *section;
  bool result = TRUE;

  for (section = firstsect(segment); section != NULL;
       section = nextsect(segment, section)) {
    if (strncmp(section->sectname, sectionName,
                sizeof(section->sectname) - 1)) {
      continue;
    }
    if (section->size == 0) {
      continue;
    ...
    for (; (firstIndex < num_structors) &&
      ...
      if ((structor = structors[firstIndex])) {
        ...
        OSRuntimeLog(theKext, kOSRuntimeLogSpec,
                     "Null structor in kext %s segment %s!", kmodInfo->name,
                     section->segname);
      }
    }
    if (metaHandle) {
...
/*********************************************************************
*********************************************************************/
kern_return_t OSRuntimeFinalizeCPP(OSKext *theKext) {
  ...
  if (OSMetaClass::modHasInstance(kmodInfo->name)) {
    ...
    OSRuntimeLog(
        theKext, kOSRuntimeLogSpec,
        "Can't tear down kext %s C++; classes have instances:", kmodInfo->name);
  ...
  for (segment = firstsegfromheader(header); segment != NULL;
    ...
    OSRuntimeCallStructorsInSection(theKext, kmodInfo, NULL, segment,
                                    sectionNames[kOSSectionNameFinalizer],
                                    textStart, textEnd);
  }
...
#endif
static void OSRuntimeSignStructorsInSegment(kernel_segment_command_t *segment) {
  kernel_section_t *section;
  ...
  for (section = firstsect(segment); section != NULL;
    ...
    for (idx = 0; idx < num_structors; idx++) {
      ...
      structor = ptrauth_sign_unauthenticated(
...
/*********************************************************************
*********************************************************************/
void OSRuntimeSignStructors(kernel_mach_header_t *header __unused) {
#if defined(HAS_APPLE_PAC)
...
/*********************************************************************
*********************************************************************/
void OSRuntimeSignStructorsInFileset(
    kernel_mach_header_t *fileset_header __unused) {
#if defined(HAS_APPLE_PAC)
  struct load_command *lc;

  lc = (struct load_command *)((uintptr_t)fileset_header +
                               sizeof(*fileset_header));
  for (uint32_t i = 0; i < fileset_header->ncmds;
...
/*********************************************************************
*********************************************************************/
kern_return_t OSRuntimeInitializeCPP(OSKext *theKext) {
  kern_return_t result = KMOD_RETURN_FAILURE;
  ...
  for (segment = firstsegfromheader(header); segment != NULL && load_success;
    ...
    load_success = OSRuntimeCallStructorsInSection(
        ...
        sectionNames[kOSSectionNameInitializer], textStart, textEnd);
  } /* for (segment...) */
  ...
  if (load_success && result != KMOD_RETURN_SUCCESS) {
    (void)OSRuntimeFinalizeCPP(
        theKext); // kmodInfo, sectionNames, textStart, textEnd);
  }
...
void OSRuntimeUnloadCPPForSegment(kernel_segment_command_t *segment) {
  OSRuntimeCallStructorsInSection(
      NULL, &g_kernel_kmod_info, NULL, segment,
      gOSStructorSectionNames[kOSSectionNamesDefault][kOSSectionNameFinalizer],
      0, 0);
}
...
__typed_allocators_ignore_pop
    ...
    /* PR-6481964 - The compiler is going to check for size overflows in calls
     * to new[], and if there is an overflow, it will call __throw_length_error.
     * This is an unrecoverable error by the C++ standard, so we must panic
     * here.
     *
     ...
     */
    namespace std {
      void __dead2 __throw_length_error(const char *msg __unused){
