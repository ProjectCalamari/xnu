...
extern ppnum_t pmap_find_phys(pmap_t pmap,
                              addr64_t va); /* osfmk/machine/pmap.h */
extern int dtrace_keep_kernel_symbols(void);

#if defined(__x86_64__) || defined(__i386__)
extern kern_return_t i386_slide_individual_kext(kernel_mach_header_t *mh,
                                                uintptr_t slide);
extern kern_return_t
i386_slide_kext_collection_mh_addrs(kernel_mach_header_t *mh, uintptr_t slide,
                                    bool adjust_mach_headers);
extern void *ubc_getobject_from_filename(const char *filename, struct vnode **,
                                         off_t *file_size);
static void *allocate_kcfileset_map_entry_list(void);
static void add_kcfileset_map_entry(void *map_entry_list, vm_map_offset_t start,
                                    vm_map_offset_t size);
static void deallocate_kcfileset_map_entry_list_and_unmap_entries(
    void *map_entry_list, boolean_t unmap_entries, bool pageable);
int vnode_put(struct vnode *vp);
kern_return_t vm_map_kcfileset_segment(vm_map_offset_t *start,
                                       vm_map_offset_t size, void *control,
                                       vm_object_offset_t fileoffset,
                                       vm_prot_t max_prot);
kern_return_t vm_unmap_kcfileset_segment(vm_map_offset_t *start,
                                         vm_map_offset_t size);
...
static OSObject *_OSKextGetRequestArgument(OSDictionary *requestDict,
                                           const char *argName);
static bool _OSKextSetRequestArgument(OSDictionary *requestDict,
                                      const char *argName,
                                      OSMetaClassBase *value);
template <typename T>
static T *_OSKextExtractPointer(OSValueObject<T *> *wrapper);
static OSKextRequestResourceCallback _OSKextExtractCallbackPointer(
    OSValueObject<OSKextRequestResourceCallback> *wrapper);
static OSReturn _OSDictionarySetCStringValue(OSDictionary *dict,
                                             const char *key,
                                             const char *value);
...
// We really should add containsObject() & containsCString to OSCollection &
// subclasses. So few pad slots, though....
static bool _OSArrayContainsCString(OSArray *array, const char *cString);
static void OSKextLogKextInfo(OSKext *aKext, uint64_t address, uint64_t size,
                              firehose_tracepoint_code_t code);

static const char *getDextUniqueIDCString(OSData *dextUniqueID,
                                          unsigned int *size);
...
#define KOSBundleDextUniqueIdentifierMaxStringLength                           \
  (KOSBundleDextUniqueIdentifierMaxLength * 2 + 1)
...
/* Set up a fake kmod_info struct for statically linked kexts that don't have
 * one. */

kmod_info_t invalid_kmod_info = {.next = NULL,
                                 .info_version = KMOD_INFO_VERSION,
                                 .id = UINT32_MAX,
                                 .name = "invalid",
                                 .version = "0",
...
extern "C" {
// symbol 'kmod' referenced in: model_dep.c, db_trace.c, symbols.c,
// db_low_trace.c, dtrace.c, dtrace_glue.h, OSKext.cpp, locore.s,
// lowmem_vectors.s, misc_protos.h, db_low_trace.c, kgmacros 'kmod' is a
// holdover from the old kmod system, we can't rename it.
...
#if CONFIG_KXLD
static thread_call_t sDestroyLinkContextThread =
    NULL;                          // one-shot, one-at-a-time thread
#endif                             // CONFIG_KXLD
...
void (*const sLoadedKextSummariesUpdated)(void) =
    OSKextLoadedKextSummariesUpdated;
OSKextLoadedKextSummaryHeader *gLoadedKextSummaries __attribute__((used)) =
    NULL;
...
static const OSKextLogSpec kDefaultKernelLogFilter =
    kOSKextLogBasicLevel | kOSKextLogVerboseFlagsMask;
static OSKextLogSpec sKernelLogFilter = kDefaultKernelLogFilter;
static bool sBootArgLogFilterFound = false;
SYSCTL_UINT(_debug, OID_AUTO, kextlog, CTLFLAG_RW | CTLFLAG_LOCKED,
            &sKernelLogFilter, 0, "kernel kext logging");
...
OSDefineValueObjectForDependentType(void *)
    ...
    TUNABLE(uint32_t, kMaxDextCrashesInOneDay, "daily_max_dext_crashes",
...
static int OSKextGrabPgoDataLocked(OSKext *kext, bool metadata,
                                   uuid_t instance_uuid, uint64_t *pSize,
                                   char *pBuffer, uint64_t bufferSize);
 ...
 *********************************************************************/
kxld_addr_t kern_allocate(u_long size, KXLDAllocateFlags *flags,
                          void *user_data) {
  ...
  if (round_page(size) > UINT_MAX) {
    OSKextLog(theKext, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
  ...
  OSKextLog(theKext,
            kOSKextLogProgressLevel | kOSKextLogLoadFlag | kOSKextLogLinkFlag,
            "Allocated link buffer for kext %s at %p (%lu bytes).",
            theKext->getIdentifierCString(), (void *)result,
...
/*********************************************************************
*********************************************************************/
void kxld_log_callback(KXLDLogSubsystem subsystem, KXLDLogLevel level,
                       const char *format, va_list argList, void *user_data) {
  OSKext *theKext = (OSKext *)user_data;
 ...
 */
/* static */
OSKext *OSKext::allocAndInitFakeKext(kmod_info_t *kmod_info) {
  ...
  if (kmod_info->id == kOSKextKernelLoadTag) {
    ...
    load_address =
        (vm_offset_t)SPTMArgs->debug_header->image[DEBUG_HEADER_ENTRY_SPTM];
    bundle_name = "sptm";
    ...
    load_address =
        (vm_offset_t)SPTMArgs->debug_header->image[DEBUG_HEADER_ENTRY_TXM];
    bundle_name = "txm";
  ...
  binaryLength -=
      (macho_is_unslid) ? ml_static_unslide(load_address) : load_address;
  ...
  OSSharedPtr<OSData> executable =
      OSData::withBytesNoCopy((void *)load_address, (unsigned int)binaryLength);
  assert(executable);
  ...
  if (set_custom_path) {
     ...
     */
    fakeKext->path =
        OSString::withCStringNoCopy("/usr/appleinternal/standalone/platform");

    /**
     * Each SPTM/TXM Mach-O should contain a __TEXT,__binname section which
     * contains a character array representing the name of the Mach-O
     * executable.
     */
    kernel_section_t *binname_sect = getsectbynamefromheader(
        (kernel_mach_header_t *)load_address, "__TEXT", "__binname");
    ...
    if (binname_sect != NULL) {
      ...
      fakeKext->executableRelPath =
          OSString::withCStringNoCopy(executable_fallback_name);
    }
  }
#endif /* CONFIG_SPTM */

  fakeKext->kmod_info = kmod_info;
  strlcpy(kmod_info->version, osrelease, sizeof(kmod_info->version));
  ...
  assert(setResult);
  setResult =
      fakeKext->infoDict->setObject(kOSKernelResourceKey, kOSBooleanTrue);
  assert(setResult);

  {
    ...
    setResult =
        fakeKext->infoDict->setObject(kCFBundleVersionKey, scratchString.get());
    assert(setResult);
  }

  {
    OSSharedPtr<OSString> scratchString(
        OSString::withCStringNoCopy(bundle_name));
    assert(scratchString);
    setResult =
        fakeKext->infoDict->setObject(kCFBundleNameKey, scratchString.get());
    assert(setResult);
  }
...
void OSKext::initialize(void) {
  ...
  sDriverKitToUpgradeByID =
      OSDictionary::withCapacity(kOSKextTypicalUpgradeCount);

  assert(sKextsByID && sLoadedKexts && sLoadedDriverKitKexts &&
         sKernelRequests && sPostedKextLoadIdentifiers &&
         sAllKextLoadIdentifiers && sRequestCallbackRecords &&
         sUnloadedPrelinkedKexts && sDriverKitToUpgradeByID);
  ...
  if (PE_parse_boot_argn("kextlog", &bootLogFilter, sizeof(bootLogFilter))) {
    ...
    OSKextLog(/* kext */ NULL, kOSKextLogBasicLevel | kOSKextLogFlagsMask,
   ...
   */
  sSafeBoot = PE_parse_boot_argn("-x", bootArgBuffer, sizeof(bootArgBuffer))
                  ? true
                  : false;
#endif /* defined(__arm__) && defined(__arm64__) */

  if (sSafeBoot) {
    OSKextLog(/* kext */ NULL, kOSKextLogWarningLevel | kOSKextLogGeneralFlag,
              "SAFE BOOT DETECTED - "
  ...
  kernelCPUType =
      OSNumber::withNumber((long long unsigned int)_mh_execute_header.cputype,
  ...
  gBuiltinKmodsSectionInfo =
      getsectbyname(kPrelinkInfoSegment, kBuiltinInfoSection);
  if (gBuiltinKmodsSectionInfo) {
    ...
    gBuiltinKmodsCount =
        (unsigned int)(gBuiltinKmodsSectionInfo->size / sizeof(kmod_info_t *));

    gBuiltinKmodsSectionStart =
        getsectbyname(kPrelinkInfoSegment, kBuiltinStartSection);
    ...
    vm_kernel_builtinkmod_text =
        ((uintptr_t *)gBuiltinKmodsSectionStart->addr)[0];
    vm_kernel_builtinkmod_text_end =
        ((uintptr_t *)gBuiltinKmodsSectionStart->addr)[count - 1];
  }
  ...
  *timestamp = 0;
  ...
  OSKextLog(/* kext */ NULL, kOSKextLogProgressLevel | kOSKextLogGeneralFlag,
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::removeKextBootstrap(void) {
   ...
   */
  dt_result = IODTGetLoaderInfo(dt_kernel_header_name, (void **)&dt_mach_header,
                                &dt_mach_header_size);
  ...
  dt_result = IODTGetLoaderInfo(dt_kernel_symtab_name, (void **)&dt_symtab,
                                &dt_symtab_size);
  ...
  if (0 ==
      IODTGetLoaderInfo(dt_segment_name, &segment_paddress, &segment_size)) {
...
#if !CONFIG_SPTM
    ml_static_mfree(
        ml_static_ptovirt(seg_klddata->vmaddr - gVirtBase + gPhysBase),
        seg_klddata->vmsize);
#else
...
#error arch
  ...
  if (!sKeepSymbols && kc_format != KCFormatFileset) {
     ...
     */
    mem_result = kmem_alloc(kernel_map, (vm_offset_t *)&seg_copy, seg_length,
                            KMA_ZERO, VM_KERN_MEMORY_KEXT);
     ...
     */
    mem_result =
        mach_vm_map_kernel(kernel_map, &seg_offset, seg_length, /* mask */ 0,
                           VM_MAP_KERNEL_FLAGS_FIXED(.vmf_overwrite = true),
                           (ipc_port_t)NULL, (vm_object_offset_t)0,
                           /* copy */ FALSE,
    ...
    if ((mem_result != KERN_SUCCESS) ||
      ...
      OSKextLog(
          /* kext */ NULL,
          kOSKextLogErrorLevel | kOSKextLogGeneralFlag | kOSKextLogArchiveFlag,
    ...
    kernel_mach_header_t *mh =
        (kernel_mach_header_t *)PE_get_kc_header(KCKindPrimary);
  ...
  if (!sKeepSymbols && kc_format != KCFormatFileset) {
    ...
    OSKextLog(
        /* kext */ NULL, kOSKextLogBasicLevel | kOSKextLogGeneralFlag,
...
/*********************************************************************
*********************************************************************/
void OSKext::flushNonloadedKexts(Boolean flushPrelinkedKexts) {
   ...
   */
  {
    sKextsByID->iterateObjects(
        ^bool(const OSSymbol *thisID __unused, OSObject *obj) {
          ...
          if (!flushPrelinkedKexts && thisKext->isPrelinked()) {
            ...
            /*
             * Don't unload codeless kexts, because they never appear in the
             * loadedKexts array. Requesting one from the IOKit daemon will load
             * it and then immediately remove it by calling
             * flushNonloadedKexts().
             * And adding one to loadedKexts breaks code assuming they have
             * kmod_info etc.
             */
            keepKexts->setObject(thisKext);
...
void OSKext::flushNonloadedKexts(Boolean flushPrelinkedKexts __unused) {
  IORecursiveLockLock(sKextLock);
...
*********************************************************************/
/* static */
void OSKext::setIOKitDaemonActive(bool active) {
...
/* static */
OSReturn OSKext::pingIOKitDaemon(void) {
  OSReturn result = kOSReturnError;
#if !NO_KEXTD
...
bool OSKext::driverkitEnabled(void) {
  ...
  if (kSuccess !=
      SecureDTGetProperty(entry, "supports-driverkit", &prop, &prop_size)) {
    return false;
  }
...
*********************************************************************/
/* static */
bool OSKext::iokitDaemonAvailable(void) {
...
*********************************************************************/
/* static */
void OSKext::setDeferredLoadSucceeded(Boolean succeeded) {
  IORecursiveLockLock(sKextLock);
...
void OSKext::willShutdown(void) {
  ...
  checkResult =
      _OSKextCreateRequest(kKextRequestPredicateRequestDaemonExit, exitRequest);
...
void OSKext::setWillUserspaceReboot(void) {
  OSKext::willShutdown();
  IOService::setWillUserspaceReboot();
  gIOCatalogue->terminateDriversForUserspaceReboot();
}

void OSKext::resetAfterUserspaceReboot(void) {
  OSSharedPtr<OSArray> arr = OSArray::withCapacity(1);
  IOService::updateConsoleUsers(arr.get(), 0,
                                true /* after_userspace_reboot */);
 ...
 * next time the daemon starts up it will immediately exit as it gets the old
 * exit request.
 *
 * This removes exit requests so that this does not happen.
 */
void OSKext::removeDaemonExitRequests(void) {
  OSDictionary *current = NULL;
  OSString *predicate = NULL;
  size_t index = 0;
  OSSharedPtr<const OSSymbol> predicateKey =
      OSSymbol::withCString(kKextRequestPredicateKey);

  while (index < sKernelRequests->getCount()) {
    current = OSDynamicCast(OSDictionary, sKernelRequests->getObject(index));
    if (current) {
      predicate =
          OSDynamicCast(OSString, current->getObject(predicateKey.get()));
      if (predicate &&
          predicate->isEqualTo(kKextRequestPredicateRequestDaemonExit)) {
...
*********************************************************************/
/* static */
bool OSKext::getLoadEnabled(void) {
  bool result;
...
*********************************************************************/
/* static */
bool OSKext::setLoadEnabled(bool flag) {
  bool result;
...
*********************************************************************/
/* static */
bool OSKext::getUnloadEnabled(void) {
  bool result;
...
*********************************************************************/
/* static */
bool OSKext::setUnloadEnabled(bool flag) {
  bool result;
 ...
 *********************************************************************/
/* static */
bool OSKext::getAutounloadEnabled(void) {
#if XNU_TARGET_OS_OSX
  bool result;
 ...
 *********************************************************************/
/* static */
bool OSKext::setAutounloadsEnabled(bool flag) {
#if XNU_TARGET_OS_OSX
  bool result;
...
bool OSKext::setAutounloadEnabled(bool flag) {
  ...
  if (result != (flag ? true : false)) {
    OSKextLog(this,
              ...
              "Autounloading for kext %s now %sabled.", getIdentifierCString(),
...
*********************************************************************/
/* static */
bool OSKext::setKernelRequestsEnabled(bool flag) {
  bool result;
  ...
  if (sKernelRequestsEnabled != result) {
    OSKextLog(/* kext */ NULL, kOSKextLogBasicLevel | kOSKextLogGeneralFlag,
...
*********************************************************************/
/* static */
bool OSKext::getKernelRequestsEnabled(void) {
  bool result;
...
static bool segmentIsMutable(kernel_segment_command_t *seg) {
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSKext> OSKext::withPrelinkedInfoDict(OSDictionary *anInfoDict,
...
OSData *OSKext::parseDextUniqueID(OSDictionary *anInfoDict,
                                  const char *dextIDCS) {
  OSData *ret = NULL;
  OSData *data_duid = OSDynamicCast(
      OSData, anInfoDict->getObject(kOSBundleDextUniqueIdentifierKey));
  if (data_duid != NULL) {
    if (data_duid->getLength() > KOSBundleDextUniqueIdentifierMaxLength) {
      OSKextLog(NULL, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                "Dext %s DextUniqueIdentifier too long.", dextIDCS);
    } else {
      /*
    ...
    OSKextLog(NULL, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
              "Dext %s does not have a DextUniqueIdentifier", dextIDCS);
  }
  return ret;
}

void OSKext::setDextUniqueIDInPersonalities(OSDictionary *anInfoDict,
                                            OSData *dextUniqueID) {
  OSDictionary *dextPersonalities = NULL;
  OSSharedPtr<OSCollectionIterator> personalitiesIterator;
  OSString *personalityName = NULL;

  dextPersonalities = OSDynamicCast(
      OSDictionary, anInfoDict->getObject(kIOKitPersonalitiesKey));
  ...
  while ((personalityName = OSDynamicCast(
              OSString, personalitiesIterator->getNextObject()))) {
    OSDictionary *personality = OSDynamicCast(
        OSDictionary, dextPersonalities->getObject(personalityName));
    if (personality) {
...
/*********************************************************************
*********************************************************************/
bool OSKext::initWithPrelinkedInfoDict(OSDictionary *anInfoDict,
                                       bool doCoalescedSlides, kc_kind_t type) {
  bool result = false;
   ...
   */
  kextPath =
      OSDynamicCast(OSString, anInfoDict->getObject(kPrelinkBundlePathKey));
  ...
  userExecutableRelPath.reset(
      OSDynamicCast(OSString,
                    anInfoDict->getObject(kCFBundleDriverKitExecutableKey)),
      OSRetain);
  ...
  scratchBool = OSDynamicCast(
      OSBoolean, getPropertyForHostArch(kOSBundleRequireExplicitLoadKey));
   ...
   */
  addressNum =
      OSDynamicCast(OSNumber, anInfoDict->getObject(kPrelinkExecutableLoadKey));
  if (addressNum &&
      addressNum->unsigned64BitValue() != kOSKextCodelessKextLoadAddr) {
    lengthNum = OSDynamicCast(OSNumber,
                              anInfoDict->getObject(kPrelinkExecutableSizeKey));
    if (!lengthNum) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
    ...
    data =
        (void *)(((uintptr_t)(addressNum->unsigned64BitValue())) + kext_slide);
     ...
     */
    addressNum = OSDynamicCast(
        OSNumber, anInfoDict->getObject(kPrelinkExecutableSourceKey));
    if (addressNum) {
      srcData = (void *)(((uintptr_t)(addressNum->unsigned64BitValue())) +
                         kext_slide);
      ...
      if (data != srcData) {
        ...
        alloc_result =
            kext_alloc((vm_offset_t *)&data, length, /* fixed */ TRUE);
        if (alloc_result != KERN_SUCCESS) {
          OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
    ...
    addressNum =
        OSDynamicCast(OSNumber, anInfoDict->getObject(kPrelinkKmodInfoKey));
    if (!addressNum) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
    ...
    if (addressNum->unsigned64BitValue() != 0) {
      kmod_info =
          (kmod_info_t *)(((uintptr_t)(addressNum->unsigned64BitValue())) +
                          kext_slide);
  ...
  if ((addressNum =
           OSDynamicCast(OSNumber, anInfoDict->getObject("ModuleIndex")))) {
    ...
    builtinTextStart =
        ((uintptr_t *)gBuiltinKmodsSectionStart->addr)[builtinKmodIdx];
    builtinTextEnd =
        ((uintptr_t *)gBuiltinKmodsSectionStart->addr)[builtinKmodIdx + 1];

    kmod_info =
        ((kmod_info_t **)gBuiltinKmodsSectionInfo->addr)[builtinKmodIdx];
  ...
  if (isInterface()) {
    interfaceUUID.reset(
        OSDynamicCast(OSData, anInfoDict->getObject(kPrelinkInterfaceUUIDKey)),
        OSRetain);
    if (interfaceUUID) {
  ...
  if (flags.resetSegmentsFromImmutableCopy) {
    ...
    for (seg = firstsegfromheader(k_mh); seg;
         seg = nextsegfromheader(k_mh, seg)) {
      ...
      OSKextLog(this, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
                ...
                getIdentifierCString(),
                (int)strnlen(seg->segname, sizeof(seg->segname)), seg->segname,
                unslid_vmaddr, unslid_vmaddr + vmsize - 1);
      OSSharedPtr<OSKextSavedMutableSegment> savedSegment =
          OSKextSavedMutableSegment::withSegment(seg);
      if (!savedSegment) {
        OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
                  "Kext %s failed to save mutable segment %llx->%llx.",
                  getIdentifierCString(), unslid_vmaddr,
                  unslid_vmaddr + vmsize - 1);
        result = false;
        goto finish;
      }
  ...
  if (doCoalescedSlides == false && !flags.resetSegmentsFromVnode) {
     ...
     */
    result = (kOSReturnSuccess ==
              setVMAttributes(true, (type == KCKindAuxiliary) ? true : false));
    if (!result) {
      goto finish;
    }
  }
  ...
  if (isDriverKit()) {
    ...
    dextUniqueID.reset(parseDextUniqueID(anInfoDict, getIdentifierCString()),
                       OSRetain);
    dextLaunchedCount = 0;
  }
...
*********************************************************************/
/* static */
OSSharedPtr<OSKext> OSKext::withCodelessInfo(OSDictionary *anInfoDict,
                                             OSKextInitResult *result) {
  OSSharedPtr<OSKext> newKext = OSMakeShared<OSKext>();
  if (!newKext) {
    return NULL;
  }
...
/*********************************************************************
*********************************************************************/
OSKextInitResult OSKext::initWithCodelessInfo(OSDictionary *anInfoDict) {
   ...
   * Get the path. Don't look for an arch-specific path property.
   */
  kextPath = OSDynamicCast(
      OSString,
      anInfoDict->getObject(kKextRequestArgumentCodelessInfoBundlePathKey));
  if (!kextPath) {
    OSKextLog(
        NULL, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
  ...
  if (declaresExecutable() || isKernelComponent()) {
    OSKextLog(NULL, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Refusing to register codeless kext that declares an "
              "executable/kernel component: %s",
              getIdentifierCString());
    goto finish;
  }
  ...
  if (strcmp(getIdentifierCString(), kIOExcludeListBundleID) == 0) {
    ...
    if (updated) {
      OSKextLog(this, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
                "KextExcludeList was updated to version: %lld",
                sExcludeListVersion);
    }
  }
...
void OSKext::setAllVMAttributes(void) {
  ...
  while ((thisID = OSDynamicCast(OSSymbol, kextIterator->getNextObject()))) {
    ...
    if (!thisKext || thisKext->isInterface() ||
        !thisKext->declaresExecutable()) {
      continue;
    }
    ...
    if (!thisKext->flags.resetSegmentsFromVnode) {
       ...
       * wire pages for the rest of the KC types at load time.
       */
      thisKext->setVMAttributes(
          true, (thisKext->kc_type == KCKindAuxiliary) ? true : false);
    }
  }
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSKext> OSKext::withBooterData(OSString *deviceTreeName,
                                           OSData *booterData) {
  OSSharedPtr<OSKext> newKext(OSMakeShared<OSKext>());
...
bool OSKext::initWithBooterData(OSString *deviceTreeName, OSData *booterData) {
  bool result = false;
  ...
  if (!theInfoDict) {
    ...
    OSKextLog(
        this, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
  ...
  if (kextFileInfo->executablePhysAddr && kextFileInfo->executableLength) {
    ...
    executable =
        OSData::withBytesNoCopy(executableAddr, kextFileInfo->executableLength);
    if (!executable) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
  ...
  if (isDriverKit()) {
    dextStatistics = OSDextStatistics::create();
    dextUniqueID.reset(parseDextUniqueID(theInfoDict, getIdentifierCString()),
                       OSRetain);
    dextLaunchedCount = 0;
  }
...
/*********************************************************************
*********************************************************************/
OSKextInitResult OSKext::registerIdentifier(void) {
  OSKextInitResult result = kOSKextInitFailure;
  ...
  OSKextVersionGetString(existingVersion, existingVersionCString,
                         kOSKextVersionMaxLength);
  ...
  if (isDriverKit() && existingIsDext) {
    ...
    if (!newDextUID) {
      OSKextLog(
          this, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
          "New dext %s, v%s requested does not have a unique dext identifier\n",
          getIdentifierCString(), newVersionCString);
      goto finish;
    }
    newDextUniqueIDCString =
        getDextUniqueIDCString(newDextUID, &newDextUniqueIDCStringSize);
    ...
    existingDextUniqueIDCString = getDextUniqueIDCString(
        existingDextUID, &existingDextUniqueIDCStringSize);
     ...
     */
    if (newDextUID->isEqualTo(existingDextUID) &&
        existingKext->flags.dextToReplace == 0) {
      OSKextLog(this, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
                "Refusing new dext %s, v%s:"
                "a dext v %s with the same unique dext identifier (%s) already "
                "exists\n",
                getIdentifierCString(), newVersionCString,
    ...
    if (upgraded) {
      ...
      OSKextLog(this, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
                ...
                getIdentifierCString(), existingVersionCString,
                existingDextUniqueIDCString, newVersionCString,
                newDextUniqueIDCString);
      result = kOSKextInitialized;
    } else {
      OSKextLog(this, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
  ...
  if ((!existingIsCodeless || existingIsDext) &&
    ...
    if (existingIsDext && !isDriverKit()) {
      OSKextLog(this, kOSKextLogWarningLevel | kOSKextLogKextBookkeepingFlag,
    ...
    goto finish;
  } /* if ((!existingIsCodeless || existingIsDext) && (existingIsLoaded ||
       existingIsPrelinked)) */
  ...
  /* We have two nonloaded/nonprelinked kexts, so our decision depends on
   * whether user loads are happening or if we're still in early boot. User
   * agents are supposed to resolve dependencies topside and include only the
   * exact kexts needed; so we always accept the new kext (in fact we should
   * never see an older unloaded copy hanging around).
   */
  if (sUserLoadsActive) {
    sKextsByID->setObject(bundleID.get(), this);
    result = kOSKextInitialized;

    OSKextLog(this, kOSKextLogStepLevel | kOSKextLogKextBookkeepingFlag,
  ...
  if (newVersion > existingVersion) {
    ...
    OSKextLog(
        this, kOSKextLogStepLevel | kOSKextLogKextBookkeepingFlag,
        "Dropping lower version (v%s) of registered kext %s for higher (v%s).",
        existingVersionCString, getIdentifierCString(), newVersionCString);
  } else {
    OSKextLog(this, kOSKextLogStepLevel | kOSKextLogKextBookkeepingFlag,
 ...
 **********************************************************************/
bool OSKext::setInfoDictionaryAndPath(OSDictionary *aDictionary,
                                      OSString *aPath) {
  bool result = false;
   ...
   */
  scratchBool = OSDynamicCast(
      OSBoolean, getPropertyForHostArch(kOSBundleEnableKextLoggingKey));
  if (scratchBool == kOSBooleanTrue) {
   ...
   */
  bundleIDString =
      OSDynamicCast(OSString, getPropertyForHostArch(kCFBundleIdentifierKey));
  if (!bundleIDString) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogValidationFlag,
  ...
  versionString =
      OSDynamicCast(OSString, getPropertyForHostArch(kCFBundleVersionKey));
  if (!versionString) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogValidationFlag,
  ...
  compatibleVersionString = OSDynamicCast(
      OSString, getPropertyForHostArch(kOSBundleCompatibleVersionKey));
  if (compatibleVersionString) {
    ...
    if (compatibleVersion > version) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogValidationFlag,
                ...
                getIdentifierCString(), kOSBundleCompatibleVersionKey,
                compatibleVersionCString, kCFBundleVersionKey, versionCString);
      goto finish;
    }
  }
   ...
   */
  scratchBool =
      OSDynamicCast(OSBoolean, getPropertyForHostArch(kOSBundleIsInterfaceKey));
  if (scratchBool == kOSBooleanTrue) {
    flags.interface = 1;
  }

  scratchBool =
      OSDynamicCast(OSBoolean, getPropertyForHostArch(kOSKernelResourceKey));
  if (scratchBool == kOSBooleanTrue) {
 ...
 * executable.
 *********************************************************************/
bool OSKext::setExecutable(OSData *anExecutable, OSData *externalData,
...
/*********************************************************************
*********************************************************************/
static void uniqueStringPlistProperty(OSDictionary *dict, const char *key) {
...
/*********************************************************************
*********************************************************************/
static void uniqueStringPlistProperty(OSDictionary *dict, const OSString *key) {
  OSObject *value = NULL;       // do not release
  OSString *stringValue = NULL; // do not release
  OSSharedPtr<const OSSymbol> symbolValue;
...
void OSKext::uniquePersonalityProperties(OSDictionary *personalityDict) {
 ...
 *********************************************************************/
/* static */
void OSKext::uniquePersonalityProperties(
    OSDictionary *personalityDict, bool defaultAddKernelBundleIdentifier) {
  ...
  if (personalityDict->getObject(kCFBundleIdentifierKernelKey)) {
    ...
    personalityDict->setObject(
        kCFBundleIdentifierKernelKey,
        personalityDict->getObject(kCFBundleIdentifierKey));
  }
...
/*********************************************************************
*********************************************************************/
OSReturn OSKext::readMkextArchive(OSData *mkextData, uint32_t *checksumPtr) {
  ...
  if (MKEXT_GET_LENGTH(mkextHeader) != mkextLength) {
    OSKextLog(
        /* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::readMkext2Archive(OSData *mkextData,
  ...
  crc_buffer_size = (uintptr_t)mkextHeader + MKEXT_GET_LENGTH(mkextHeader) -
  ...
  if (MKEXT_GET_CPUTYPE(mkextHeader) == (UInt32)CPU_TYPE_ANY) {
    ...
    if ((UInt32)_mh_execute_header.cputype != MKEXT_GET_CPUTYPE(mkextHeader)) {
      OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
  ...
  mkextPlistEnd =
      (char *)mkextHeader + mkextPlistOffset + mkextPlistCompressedSize;
  ...
  mkextInfoDictArray =
      OSDynamicCast(OSArray, mkextPlist->getObject(kMKEXTInfoDictionariesKey));
  ...
  for (i = 0; i < count; i++) {
    ...
    infoDict = OSDynamicCast(OSDictionary, mkextInfoDictArray->getObject(i));
    ...
    if (infoDict) {
      ...
      if (kc_format == KCFormatFileset && newKext &&
          !(newKext->isPrelinked()) && newKext->declaresExecutable()) {
        result = kOSReturnError;
        printf("Kext LOG: Dynamic loading of kext denied for kext %s\n",
               newKext->getIdentifier() ? newKext->getIdentifierCString()
                                        : "unknown kext");
...
OSReturn OSKext::readMkext2Archive(OSData *mkextData,
  ...
  if (kOSReturnSuccess ==
      (ret = readMkext2Archive(mkextData, &mkextPlist, checksumPtr))) {
...
*********************************************************************/
/* static */
OSSharedPtr<OSKext> OSKext::withMkext2Info(OSDictionary *anInfoDict,
                                           OSData *mkextData) {
  OSSharedPtr<OSKext> newKext = OSMakeShared<OSKext>();
...
/*********************************************************************
*********************************************************************/
bool OSKext::initWithMkext2Info(OSDictionary *anInfoDict, OSData *mkextData) {
  bool result = false;
  OSString *kextPath = NULL;            // do not release
   ...
   */
  kextPath =
      OSDynamicCast(OSString, anInfoDict->getObject(kMKEXTBundlePathKey));
  ...
  executableOffsetNum =
      OSDynamicCast(OSNumber, infoDict->getObject(kMKEXTExecutableKey));
  if (executableOffsetNum) {
    executable =
        createMkext2FileEntry(mkextData, executableOffsetNum, "executable");
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSData> OSKext::createMkext2FileEntry(OSData *mkextData,
                                                  OSNumber *offsetNum,
                                                  const char *name) {
...
void *z_alloc(void *notused __unused, u_int num_items, u_int size) {
  ...
  if (total > UINT32_MAX) {
    panic("z_alloc(%p, %x, %x): overflow caused by %x * %x", notused, num_items,
          size, num_items, size);
  }
  ...
  if (allocSize64 > UINT32_MAX) {
    panic("z_alloc(%p, %x, %x): overflow caused by %x + %lx", notused,
          num_items, size, (uint32_t)total, sizeof(zmem));
  }
  uint32_t allocSize = (uint32_t)allocSize64;

  zmem = (z_mem *)kalloc_data_tag(allocSize, Z_WAITOK, VM_KERN_MEMORY_OSKEXT);
  if (!zmem) {
    goto finish;
  }
...
OSSharedPtr<OSData> OSKext::extractMkext2FileData(UInt8 *data, const char *name,
  ...
  if (KERN_SUCCESS !=
      kmem_alloc(kernel_map, (vm_offset_t *)&uncompressedDataBuffer, fullSize,
  ...
  if (isKernel()) {
    OSKextLog(this, kOSKextLogDetailLevel | kOSKextLogArchiveFlag,
              "Kernel extracted %s from mkext - compressed size %d, "
              "uncompressed size %d.",
              name, compressedSize, fullSize);
  } else {
    OSKextLog(this, kOSKextLogDetailLevel | kOSKextLogArchiveFlag,
              "Kext %s extracted %s from mkext - compressed size %d, "
              "uncompressed size %d.",
  ...
  if (zlib_result == Z_STREAM_END || zlib_result == Z_OK) {
    ...
    if (isKernel()) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
                "Mkext error; zlib inflate failed (%d) for %s.", zlib_result,
                name);
    } else {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
...
*********************************************************************/
/* static */
OSReturn OSKext::loadFromMkext(OSKextLogSpec clientLogFilter, char *mkextBuffer,
                               uint32_t mkextBufferLength, char **logInfoOut,
   ...
   * the buffer.
   */
  mkextData = OSData::withBytesNoCopy(mkextBuffer, mkextBufferLength);
  if (!mkextData) {
    OSKextLog(/* kext */ NULL,
  ...
  startKextExcludeNum = OSDynamicCast(
      OSNumber, requestArgs->getObject(kKextRequestArgumentStartExcludeKey));
  startMatchingExcludeNum = OSDynamicCast(
      OSNumber,
      requestArgs->getObject(kKextRequestArgumentStartMatchingExcludeKey));
  delayAutounloadBool = OSDynamicCast(
      OSBoolean,
      requestArgs->getObject(kKextRequestArgumentDelayAutounloadKey));
  personalityNames = OSDynamicCast(
      OSArray, requestArgs->getObject(kKextRequestArgumentPersonalityNamesKey));
  ...
  OSKextLog(/* kext */ NULL, kOSKextLogProgressLevel | kOSKextLogIPCFlag,
  ...
  result = OSKext::loadKextWithIdentifier(
      kextIdentifier,
      /* kextRef */ NULL,
      /* allowDefer */ false, delayAutounload, startKextExcludeLevel,
      startMatchingExcludeLevel, personalityNames);
  if (result != kOSReturnSuccess) {
    goto finish;
  }
  /* If the load came down from the IOKit daemon, it will shortly inform
   * IOCatalogue for matching via a separate IOKit calldown.
   */
...
finish:
  ...
  if (logInfoArray && logInfoOut && logInfoLengthOut) {
    tempResult = OSKext::serializeLogInfo(logInfoArray.get(), logInfoOut,
                                          logInfoLengthOut);
...
*********************************************************************/
/* static */
OSReturn OSKext::serializeLogInfo(OSArray *logInfoArray, char **logInfoOut,
                                  uint32_t *logInfoLengthOut) {
  OSReturn result = kOSReturnError;
  char *buffer = NULL;
  ...
  if (!logInfoArray || !logInfoOut || !logInfoLengthOut) {
    OSKextLog(
        /* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogIPCFlag,
  ...
  if (!serializer) {
    ...
    if (!logInfoArray->serialize(serializer.get())) {
      ...
      kmem_result = kmem_alloc(kernel_map, (vm_offset_t *)&buffer,
                               round_page(logInfoLength), KMA_DATA_SHARED,
...
OSKext::lookupKextWithIdentifier(const char *kextIdentifier) {
  ...
  IORecursiveLockLock(sKextLock);
  foundKext.reset(OSDynamicCast(OSKext, sKextsByID->getObject(kextIdentifier)),
                  OSRetain);
  IORecursiveLockUnlock(sKextLock);
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSKext> OSKext::lookupKextWithIdentifier(OSString *kextIdentifier) {
...
*********************************************************************/
OSSharedPtr<OSKext>
OSKext::lookupDextWithIdentifier(OSString *dextIdentifier,
                                 OSData *dextUniqueIdentifier) {
  OSSharedPtr<OSKext> foundDext;
  foundDext.reset();

  IORecursiveLockLock(sKextLock);
  OSKext *dext = OSDynamicCast(
      OSKext, sKextsByID->getObject(dextIdentifier->getCStringNoCopy()));
  if (dext != NULL && dext->isDriverKit()) {
    if (dext->dextUniqueID == NULL ||
        dext->dextUniqueID->isEqualTo(dextUniqueIdentifier)) {
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSKext> OSKext::lookupKextWithLoadTag(uint32_t aTag) {
  OSSharedPtr<OSKext> foundKext; // returned
  uint32_t i, j;
  OSArray *list[2] = {sLoadedKexts.get(), sLoadedDriverKitKexts.get()};
  uint32_t count[2] = {sLoadedKexts->getCount(),
                       sLoadedDriverKitKexts->getCount()};
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSKext> OSKext::lookupKextWithAddress(vm_address_t address) {
  OSSharedPtr<OSKext> foundKext; // returned
  uint32_t count, i;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (thisKext->kmod_info && thisKext->kmod_info->address) {
...
#if defined(__arm64__)
      textExecBase = (uintptr_t)getsegdatafromheader(
          (kernel_mach_header_t *)kmod_info->address, "__TEXT_EXEC",
          &textExecSize);
      ...
      if ((textExecBase <= address) &&
          (address < textExecBase + textExecSize)) {
        foundKext.reset(thisKext, OSRetain);
        goto finish;
      }
#endif /* defined (__arm64__) */
   ...
   * so we "fake" their address range with the LoadTag. We cannot use the
   * ptrauth-stripped address here, so use the original address passed to this
   * method.
   *
   * This is supposed to be used for logging reasons only. When logd
   * calls this function it ors the address with
   * FIREHOSE_TRACEPOINT_PC_KERNEL_MASK, so we remove it here before checking it
   * against the LoadTag. Also we need to remove
   * FIREHOSE_TRACEPOINT_PC_DYNAMIC_BIT set when emitting the log line.
   */

  address = originalAddress & ~(FIREHOSE_TRACEPOINT_PC_KERNEL_MASK |
                                FIREHOSE_TRACEPOINT_PC_DYNAMIC_BIT);
  count = sLoadedDriverKitKexts->getCount();
  for (i = 0; i < count; i++) {
    OSKext *thisKext =
        OSDynamicCast(OSKext, sLoadedDriverKitKexts->getObject(i));
...
OSSharedPtr<OSData> OSKext::copyKextUUIDForAddress(OSNumber *address) {
  ...
  uintptr_t slidAddress =
      ml_static_slide((uintptr_t)address->unsigned64BitValue());
  if (slidAddress != 0) {
  ...
  if (!uuid) {
    /*
     * If we still don't have a UUID, then we failed to match the slid +
     * stripped address with a kext. This might have happened because the log
     * message came from a dext.
     *
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSKext> OSKext::lookupKextWithUUID(uuid_t wanted) {
  OSSharedPtr<OSKext> foundKext; // returned
  uint32_t j, i;
  OSArray *list[2] = {sLoadedKexts.get(), sLoadedDriverKitKexts.get()};
  uint32_t count[2] = {sLoadedKexts->getCount(),
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::removeKext(OSKext *aKext,
#if CONFIG_EMBEDDED
                            __unused
#endif
  ...
  if (!aKext->isDriverKit()) {
    OSKextLog(aKext, kOSKextLogErrorLevel | kOSKextLogKextBookkeepingFlag,
              "removeKext() called for %s, only supported on embedded for "
              "DriverKit dexts",
              aKext->getIdentifier() ? aKext->getIdentifierCString()
                                     : "unknown kext");
  ...
  checkKext =
      OSDynamicCast(OSKext, sKextsByID->getObject(aKext->getIdentifier()));
  if (checkKext != aKext) {
  ...
  if (aKext->isLoaded() || aKext->isDriverKit()) {
    ...
    if (current_task() != kernel_task) {
      cred = kauth_cred_get_with_ref();
      macCheckResult =
          mac_kext_check_unload(cred, aKext->getIdentifierCString());
      kauth_cred_unref(&cred);
    }
  ...
  if (aKext->isInFileset()) {
    OSKextLog(aKext, kOSKextLogProgressLevel | kOSKextLogKextBookkeepingFlag,
              "Fileset kext %s unloaded.", aKext->getIdentifierCString());
  } else {
    OSKextLog(aKext, kOSKextLogProgressLevel | kOSKextLogKextBookkeepingFlag,
              "Removing kext %s.", aKext->getIdentifierCString());
...
*********************************************************************/
/* static */
OSReturn OSKext::removeKextWithIdentifier(
  ...
  OSKext *aKext = OSDynamicCast(OSKext, sKextsByID->getObject(kextIdentifier));
  if (!aKext) {
    result = kOSKextReturnNotFound;
    OSKextLog(/* kext */ NULL,
              kOSKextLogErrorLevel | kOSKextLogKextBookkeepingFlag,
              "Can't remove kext %s - not found.", kextIdentifier);
    goto finish;
  }

  result =
      OSKext::removeKext(aKext, terminateServicesAndRemovePersonalitiesFlag);
...
*********************************************************************/
/* static */
OSReturn OSKext::removeKextWithLoadTag(
    OSKextLoadTag loadTag, bool terminateServicesAndRemovePersonalitiesFlag) {
  OSReturn result = kOSReturnError;
  ...
  if (!foundKext) {
    ...
    OSKextLog(/* kext */ NULL,
              kOSKextLogErrorLevel | kOSKextLogLoadFlag |
                  kOSKextLogKextBookkeepingFlag,
              "Can't remove kext with load tag %d - not found.", loadTag);
    goto finish;
  }
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSDictionary> OSKext::copyKexts(void) {
 ...
 *********************************************************************/
/* static */
void OSKext::createExcludeListFromBooterData(
    OSDictionary *theDictionary, OSCollectionIterator *theIterator) {
  ...
  while ((deviceTreeName =
    ...
    if (strncmp(devTreeNameCString, BOOTER_KEXT_PREFIX,
      ...
      OSKextLog(NULL, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
                "\"%s\" not a kext", devTreeNameCString);
      continue;
    }

    deviceTreeBuffer =
        (const _DeviceTreeBuffer *)deviceTreeEntry->getBytesNoCopy(
            0, sizeof(deviceTreeBuffer));
    if (!deviceTreeBuffer) {
      continue;
    }
    ...
    myBundleID =
        OSDynamicCast(OSString, theInfoDict->getObject(kCFBundleIdentifierKey));
    if (myBundleID &&
 ...
 *********************************************************************/
/* static */
void OSKext::createExcludeListFromPrelinkInfo(OSArray *theInfoArray) {
  ...
  for (i = 0; i < theInfoArray->getCount(); i++) {
    ...
    myBundleID =
        OSDynamicCast(OSString, myInfoDict->getObject(kCFBundleIdentifierKey));
    if (myBundleID &&
        strcmp(myBundleID->getCStringNoCopy(), kIOExcludeListBundleID) == 0) {
...
/* static */
boolean_t OSKext::updateExcludeList(OSDictionary *infoDict) {
  ...
  myTempDict =
      OSDynamicCast(OSDictionary, infoDict->getObject("OSKextExcludeList"));
  if (!myTempDict) {
    return false;
  }

  myTempString =
      OSDynamicCast(OSString, infoDict->getObject(kCFBundleVersionKey));
  if (!myTempString) {
    return false;
  }
...
/*********************************************************************
*********************************************************************/
const OSObject *OSKext::getBundleExecutable(void) {
  return infoDict->getObject(kCFBundleExecutableKey);
}

/*********************************************************************
*********************************************************************/
const OSSymbol *OSKext::getIdentifier(void) { return bundleID.get(); }

/*********************************************************************
 * A kext must have a bundle identifier to even survive initialization;
 * this is guaranteed to exist past then.
 *********************************************************************/
const char *OSKext::getIdentifierCString(void) {
  return bundleID->getCStringNoCopy();
}

/*********************************************************************
*********************************************************************/
OSKextVersion OSKext::getVersion(void) { return version; }

/*********************************************************************
*********************************************************************/
OSKextVersion OSKext::getCompatibleVersion(void) { return compatibleVersion; }

/*********************************************************************
*********************************************************************/
bool OSKext::isLibrary(void) { return getCompatibleVersion() > 0; }

/*********************************************************************
*********************************************************************/
bool OSKext::isCompatibleWithVersion(OSKextVersion aVersion) {
...
/*********************************************************************
*********************************************************************/
bool OSKext::declaresExecutable(void) {
  if (isDriverKit()) {
    return false;
  }
...
/*********************************************************************
*********************************************************************/
OSData *OSKext::getExecutable(void) {
  OSData *result = NULL;
  ...
  mkextExecutableRef = OSDynamicCast(
      OSData, getPropertyForHostArch(_kOSKextMkextExecutableReferenceKey));

  if (mkextExecutableRef) {
    MkextEntryRef *mkextEntryRef =
        (MkextEntryRef *)mkextExecutableRef->getBytesNoCopy();
    ...
    if (mkextVersion == MKEXT_VERS_2) {
      ...
      extractedExecutable =
          extractMkext2FileData(MKEXT2_GET_ENTRY_DATA(fileinfo), "executable",
...
/*********************************************************************
*********************************************************************/
bool OSKext::isInterface(void) { return flags.interface; }

/*********************************************************************
*********************************************************************/
bool OSKext::isKernel(void) { return this == sKernelKext; }

/*********************************************************************
*********************************************************************/
bool OSKext::isKernelComponent(void) {
  return flags.kernelComponent ? true : false;
}

/*********************************************************************
*********************************************************************/
bool OSKext::isExecutable(void) {
  return !isKernel() && !isInterface() && declaresExecutable();
}

/*********************************************************************
*********************************************************************/
bool OSKext::isSpecialKernelBinary(void) {
#if CONFIG_SPTM
  return (this->kmod_info) && ((this->kmod_info->id == kOSKextSPTMLoadTag) ||
 ...
 * use a "kx" boot-arg!
 *********************************************************************/
bool OSKext::isLoadableInSafeBoot(void) {
  bool result = false;
  ...
  required =
      OSDynamicCast(OSString, getPropertyForHostArch(kOSBundleRequiredKey));
  if (!required) {
    goto finish;
  }
...
/*********************************************************************
*********************************************************************/
bool OSKext::isPrelinked(void) { return flags.prelinked ? true : false; }

/*********************************************************************
*********************************************************************/
bool OSKext::isLoaded(void) { return flags.loaded ? true : false; }

/*********************************************************************
*********************************************************************/
bool OSKext::isStarted(void) { return flags.started ? true : false; }

/*********************************************************************
*********************************************************************/
bool OSKext::isCPPInitialized(void) { return flags.CPPInitialized; }

/*********************************************************************
*********************************************************************/
void OSKext::setCPPInitialized(bool initialized) {
  flags.CPPInitialized = initialized;
}

/*********************************************************************
*********************************************************************/
uint32_t OSKext::getLoadTag(void) { return loadTag; }
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSData> OSKext::copyUUID(void) {
  OSSharedPtr<OSData> result;
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSData> OSKext::copyTextUUID(void) {
  if (flags.builtin) {
...
/*********************************************************************
*********************************************************************/
OSSharedPtr<OSData> OSKext::copyMachoUUID(const kernel_mach_header_t *header) {
  OSSharedPtr<OSData> result;
  ...
  if (header->magic != MH_MAGIC_KERNEL) {
    OSKextLog(NULL, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
              "%s: bad header %p", __func__, header);
    goto finish;
  }
...
void OSKext::setDriverKitUUID(OSData *uuid) {
  if (!OSCompareAndSwapPtr(nullptr, uuid, &driverKitUUID)) {
    OSSafeReleaseNULL(uuid);
  }
}

OSData *OSKext::getDextUniqueID(void) {
 ...
 */
void OSKext::replaceDextInternal(OSKext *olddext, OSKext *newdext) {
  OSReturn result;
  ...
  // The old dext will be unloaded and release dextID, so we need to retain
  // dextID here
  dextID->retain();
   ...
   */
  if (newdext != NULL) {
    result = notifyDextUpgrade(OSDynamicCast(OSString, dextID),
                               newdext->getDextUniqueID());
    if (result != kOSReturnSuccess) {
      OSKextLog(NULL, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
    ...
    queueKextNotification(kKextRequestPredicateUnloadNotification,
                          OSDynamicCast(OSString, dextID),
                          oldDextUniqueIdentifier);
  }
 ...
 */
bool OSKext::upgradeDext(OSKext *olddext, OSKext *newdext) {
 ...
 */
bool OSKext::removeDext(OSKext *dext) {
  ...
  result = gIOCatalogue->terminateDriversForModule(dext->getIdentifierCString(),
                                                   /* unload */ false,
                                                   /* asynchronous */ true);
  if (result != kOSReturnSuccess) {
    OSKextLog(dext, kOSKextLogErrorLevel | kOSKextLogKextBookkeepingFlag,
...
bool OSKext::incrementDextLaunchCount(OSKext *dext,
                                      OSData *dextUniqueIDToMatch) {
  bool ret = false;
  ...
  if (!myDextUniqueID || !dextUniqueIDToMatch) {
    OSKextLog(
        dext, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
  ...
  const char *dextUniqueIDCString =
      getDextUniqueIDCString(myDextUniqueID, &dextUniqueIDCStringSize);
  const char *dextUniqueIDToMatchCString = getDextUniqueIDCString(
      dextUniqueIDToMatch, &dextUniqueIDToMatchCStringSize);
  ...
  if (myDextUniqueID->isEqualTo(dextUniqueIDToMatch)) {
    if (dext->flags.dextToReplace == 0 || dext->dextLaunchedCount > 0) {
      if (dext->dextLaunchedCount == kOSKextMaxDextLaunchedCount) {
        OSKextLog(dext, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
        ...
        OSKextLog(dext, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
                  "New dext launched for  %s UniqueID %s", dextIDCS,
                  dextUniqueIDCString);
      }
    } else {
      OSKextLog(
          dext, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
          "Dext %s UniqueID %s requires update, cannot launch a new dext\n",
          dextIDCS, dextUniqueIDCString);
    }
  } else {
    OSKextLog(dext, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
              "Dext %s: UniqueID %s does not match UniqueID looked for %s, "
              "cannot launch a new dext\n",
...
bool OSKext::decrementDextLaunchCount(OSString *bundleID) {
  bool ret = false;
  const char *dextIDCS;
  ...
  OSKextLog(dext, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
            "Dext terminated for %s UniqueID %s", dextIDCS,
            dextUniqueIDCString);
  ...
  if (dext->dextLaunchedCount == 0 && dext->flags.dextToReplace == 1) {
     ...
     */
    OSKext *newdext =
        OSDynamicCast(OSKext, sDriverKitToUpgradeByID->getObject(dextIDCS));
    if (newdext) {
      OSKextLog(dext, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
                "Dext upgrade for %s UniqueID %s", dextIDCS,
                dextUniqueIDCString);
      ...
      OSKextLog(dext, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
                "Dext unload for %s UniqueID %s", dextIDCS,
                dextUniqueIDCString);
      replaceDextInternal(dext, NULL);
    }
...
static char *makeHostArchKey(const char *key, size_t *keySizeOut) {
  char *result = NULL;
...
/*********************************************************************
*********************************************************************/
OSObject *OSKext::getPropertyForHostArch(const char *key) {
   ...
   * - OSKernelResource.
   */
  if (STRING_HAS_PREFIX(key, "OS") || STRING_HAS_PREFIX(key, "IO")) {
 ...
 *********************************************************************/
bool OSKext::isInExcludeList(void) {
  OSString *versionString = NULL; // do not release
  ...
  if (!sExcludeListByID) {
     ...
     */
    versionString =
        OSDynamicCast(OSString, sExcludeListByID->getObject(bundleID.get()));
    if (versionString == NULL ||
        versionString->getLength() > (sizeof(myBuffer) - 1)) {
      isInExcludeList = false;
    }
  }
...
bool OSKext::isLoadable(void) {
  ...
  if (sNonLoadableKextsByID) {
    ...
    loadableVal = OSDynamicCast(
        OSBoolean, sNonLoadableKextsByID->getObject(bundleID.get()));
...
*********************************************************************/
/* static */
OSReturn OSKext::loadKextWithIdentifier(const char *kextIdentifierCString,
  ...
  result = OSKext::loadKextWithIdentifier(
      kextIdentifier.get(), NULL /* kextRef */, allowDeferFlag,
      delayAutounloadFlag, startOpt, startMatchingOpt, personalityNames);

finish:
  return result;
}

OSReturn OSKext::loadKextWithIdentifier(OSString *kextIdentifier,
  ...
  result = loadKextWithIdentifier(kextIdentifier, &kextRefRaw, allowDeferFlag,
                                  delayAutounloadFlag, startOpt,
                                  startMatchingOpt, personalityNames);
...
/*********************************************************************
*********************************************************************/
OSReturn OSKext::loadKextWithIdentifier(
    OSString *kextIdentifier, OSObject **kextRef, Boolean allowDeferFlag,
    Boolean delayAutounloadFlag, OSKextExcludeLevel startOpt,
    OSKextExcludeLevel startMatchingOpt, OSArray *personalityNames) {
  OSReturn result = kOSReturnError;
  ...
  if (!theKext) {
    ...
    if (!sKernelRequestsEnabled) {
      OSKextLog(theKext, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
    ...
    if (!sPostedKextLoadIdentifiers->containsObject(
            kextIdentifierSymbol.get())) {
      result =
          _OSKextCreateRequest(kKextRequestPredicateRequestLoad, loadRequest);
      if (result != kOSReturnSuccess) {
        goto finish;
      }
      ...
      OSKextLog(theKext, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
    ...
    if (pingResult == kOSKextReturnDisabled) {
      OSKextLog(/* kext */ NULL,
                ((sPrelinkBoot) ? kOSKextLogDebugLevel : kOSKextLogErrorLevel) |
                    kOSKextLogLoadFlag,
                "Kext %s might not load - " kIOKitDaemonName
...
*********************************************************************/
/* static */
OSReturn OSKext::loadKextFromKC(OSKext *theKext, OSDictionary *requestDict) {
  OSReturn result = kOSReturnError;
  ...
  delayAutounloadBool = OSDynamicCast(
      OSBoolean, _OSKextGetRequestArgument(
                     requestDict, kKextRequestArgumentDelayAutounloadKey));
  startKextExcludeNum = OSDynamicCast(
      OSNumber, _OSKextGetRequestArgument(requestDict,
                                          kKextRequestArgumentStartExcludeKey));
  startMatchingExcludeNum = OSDynamicCast(
      OSNumber, _OSKextGetRequestArgument(
                    requestDict, kKextRequestArgumentStartMatchingExcludeKey));
  personalityNames = OSDynamicCast(
      OSArray, _OSKextGetRequestArgument(
                   requestDict, kKextRequestArgumentPersonalityNamesKey));
  ...
  OSKext::recordIdentifierRequest(
      OSDynamicCast(OSString, theKext->getIdentifier()));

  /*
   * Load the kext
   */
  result = theKext->load(startKextExcludeLevel, startMatchingExcludeLevel,
                         personalityNames);
  ...
  if (result != kOSReturnSuccess) {
    ...
    OSKextLog(theKext, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
...
*********************************************************************/
/* static */
OSReturn OSKext::loadCodelessKext(OSString *kextIdentifier,
                                  OSDictionary *requestDict) {
  OSReturn result = kOSReturnError;
  OSDictionary *anInfoDict = NULL; // do not release

  anInfoDict = OSDynamicCast(
      OSDictionary, _OSKextGetRequestArgument(
                        requestDict, kKextRequestArgumentCodelessInfoKey));
  if (anInfoDict == NULL) {
    OSKextLog(/* kext */ NULL,
              kOSKextLogErrorLevel | kOSKextLogGeneralFlag | kOSKextLogLoadFlag,
              "Missing 'Codeless Kext Info' dictionary in codeless kext load "
              "request of %s.",
              kextIdentifier->getCStringNoCopy());
...

  {
    ...
    OSKext::recordIdentifierRequest(
        OSDynamicCast(OSString, newKext->getIdentifier()));
...
*********************************************************************/
/* static */
void OSKext::dropMatchingReferences(OSSet *kexts) {
  IORecursiveLockLock(sKextLock);
...
*********************************************************************/
/* static */
void OSKext::recordIdentifierRequest(OSString *kextIdentifier) {
  OSSharedPtr<const OSSymbol> kextIdentifierSymbol;
  bool fail = false;
...
finish:
  ...
  if (fail) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
              "Failed to record kext %s as a candidate for inclusion in "
              "prelinked kernel.",
              kextIdentifier->getCStringNoCopy());
  }
  return;
...
/*********************************************************************
*********************************************************************/
OSReturn OSKext::load(OSKextExcludeLevel startOpt,
                      OSKextExcludeLevel startMatchingOpt,
                      OSArray *personalityNames) {
  OSReturn result = kOSReturnError;
  ...
  if (isInExcludeList()) {
    OSKextLog(
        this, kOSKextLogErrorLevel | kOSKextLogGeneralFlag | kOSKextLogLoadFlag,
        "Kext %s is in exclude list, not loadable", getIdentifierCString());
  ...
  if (!isLoadable()) {
    OSKextLog(this,
              kOSKextLogErrorLevel | kOSKextLogGeneralFlag | kOSKextLogLoadFlag,
              "Kext %s is not loadable", getIdentifierCString());
  ...
  if (isLoaded()) {
    ...
    OSKextLog(this,
              kOSKextLogDebugLevel | kOSKextLogLoadFlag |
                  kOSKextLogKextBookkeepingFlag,
              "Kext %s is already loaded.", getIdentifierCString());
    goto loaded;
  }
  ...
  OSKextLog(this, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
            "Loading kext %s.", getIdentifierCString());
  ...
  if (!sKxldContext) {
    ...
    kxldResult = kxld_create_context(
        &sKxldContext, &kern_allocate, &kxld_log_callback,
        /* Flags */ (KXLDFlags)0,
        /* cputype */ 0, /* cpusubtype */ 0, /* page size */ 0);
    if (kxldResult) {
      OSKextLog(this,
                kOSKextLogErrorLevel | kOSKextLogLoadFlag | kOSKextLogLinkFlag,
  ...
  for (i = 0; i < count; i++) {
     ...
     */
    result =
        dependency->load(dependenciesStartOpt, dependenciesStartMatchingOpt,
    ...
    if (result != KERN_SUCCESS) {
      OSKextLog(this,
                ...
                dependency->getIdentifierCString(), getIdentifierCString());
...
loaded:
  if (isExecutable() && !flags.started) {
    if (startOpt == kOSKextExcludeNone) {
      ...
      if (result != kOSReturnSuccess) {
        OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                  "Kext %s start failed (result 0x%x).", getIdentifierCString(),
                  result);
...
finish:
  ...
  if (result != kOSReturnSuccess) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Kext %s failed to load (0x%x).", getIdentifierCString(),
              (int)result);
  } else if (!alreadyLoaded) {
    OSKextLog(this, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
              "Kext %s loaded.", getIdentifierCString());
...
kernel_section_t *OSKext::lookupSection(const char *segname,
                                        const char *secname) {
  ...
  for (seg = firstsegfromheader(mh); seg != NULL;
       seg = nextsegfromheader(mh, seg)) {
...
OSReturn OSKext::slidePrelinkedExecutable(bool doCoalescedSlides) {
  OSReturn result = kOSKextReturnBadData;
  kernel_mach_header_t *mh = NULL;
  kernel_segment_command_t *seg = NULL;
  ...
  segmentSplitInfo = (struct linkedit_data_command *)getcommandfromheader(
      mh, LC_SEGMENT_SPLIT_INFO);
    ...
    if (dysymtab->nlocrel > 0) {
      if (!haveLinkeditBase) {
        OSKextLog(
            this,
            kOSKextLogErrorLevel | kOSKextLogLoadFlag | kOSKextLogLinkFlag,
            "Sliding kext %s: No linkedit segment.", getIdentifierCString());
        goto finish;
      }
      ...
      for (i = 0; i < dysymtab->nlocrel; i++) {
        if (reloc[i].r_extern != 0 || reloc[i].r_type != 0 ||
...
#define MAX_DYSYMS_TO_LOG 5
        if (i < MAX_DYSYMS_TO_LOG) {
          IOLog(
              "kaslr: LC_DYSYMTAB unslid 0x%lx slid 0x%lx \n",
      ...
      /* We should free these relocations, not just delete the reference to
       * them. <rdar://problem/10535549> Free relocations from PIE kexts.
       *
      ...
      if (new_kextsize > UINT_MAX) {
        OSKextLog(
            this,
            kOSKextLogErrorLevel | kOSKextLogLoadFlag | kOSKextLogLinkFlag,
            "Kext %s: new kext size is too large.", getIdentifierCString());
        goto finish;
      }
      if (((kmod_info->size - new_kextsize) > PAGE_SIZE) &&
        ...
        vm_offset_t endofrelocInfo =
            (vm_offset_t)(((uint8_t *)reloc) + reloc_size);
        ...
        new_osdata = OSData::withBytesNoCopy((void *)kmod_info->address,
        ...
        if (new_osdata) {
           ...
           * Doing so may result in panics like rdar://85419651
           */
          if (PE_get_kc_format(kc_type, &format) &&
 ...
 * called only by load()
 *********************************************************************/
OSReturn OSKext::loadExecutable() {
  OSReturn result = kOSReturnError;
    ...
    if (STRING_HAS_PREFIX(versCString, KERNEL_LIB_PREFIX)) {
      if (strncmp(versCString, KERNEL6_VERSION, strlen(KERNEL6_VERSION))) {
        OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                  ...
                  "expected %s.",
                  getIdentifierCString(), versCString, KERNEL6_VERSION);
        result = kOSKextReturnInternalError;
        goto finish;
        ...
        OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                  "Kernel component %s has incorrect version %s; "
                  "expected %s.",
                  getIdentifierCString(), versCString, osrelease);
        result = kOSKextReturnInternalError;
        goto finish;
      }
    }
  ...
  if (flags.resetSegmentsFromVnode) {
    ...
    if (i386_slide_individual_kext(mh, PE_get_kc_slide(kc_type)) !=
        KERN_SUCCESS) {
  ...
  if (FALSE ==
    ...
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Not entitled to link kext '%s'", getIdentifierCString());
  ...
  if (flags.hasBleedthrough) {
    ...
    for (i = 0; i < numDirectDependencies; ++i) {
      OSKext *dependencyKext =
          OSDynamicCast(OSKext, dependencies->getObject(i));
  ...
  kxlddeps = kalloc_type_tag(KXLDDependency, num_kxlddeps, Z_WAITOK,
                             VM_KERN_MEMORY_OSKEXT);
  if (!kxlddeps) {
    OSKextLog(
        this, kOSKextLogErrorLevel | kOSKextLogLoadFlag | kOSKextLogLinkFlag,
        "Can't allocate link context to load kext %s.", getIdentifierCString());
    goto finish;
  }
  ...
  for (i = 0; i < num_kxlddeps; ++i) {
    ...
    if (dependency->isInterface()) {
      ...
      if (dependency->isKernelComponent()) {
        ...
        interfaceTargetKext =
            OSDynamicCast(OSKext, dependency->dependencies->getObject(0));
      ...
      kxlddeps[i].kext_name =
          strdup(interfaceTargetKext->getIdentifierCString());

      if (dependency->linkedExecutable != NULL) {
        kxlddeps[i].interface =
            (u_char *)dependency->linkedExecutable->getBytesNoCopy();
...
#if DEBUG
  OSKextLog(this,
            kOSKextLogExplicitLevel | kOSKextLogLoadFlag | kOSKextLogLinkFlag,
            ...
            theExecutable->getBytesNoCopy(), theExecutable->getLength(), this,
            kxlddeps, num_kxlddeps, kxldHeaderPtr, &kmod_info);
#endif

  /* After this call, the linkedExecutable instance variable
   * should exist.
   */
  kxldResult = kxld_link_file(
      sKxldContext, (u_char *)theExecutable->getBytesNoCopy(),
      theExecutable->getLength(), getIdentifierCString(), this, kxlddeps,
      num_kxlddeps, (u_char **)kxldHeaderPtr, (kxld_addr_t *)&kmod_info);

  if (kxldResult != KERN_SUCCESS) {
    // xxx - add kxldResult here?
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Can't load kext %s - link failed.", getIdentifierCString());
...
register_kmod:
  ...
  if (num_kmod_refs) {
    kmod_info->reference_list = kalloc_type_tag(
        kmod_reference_t, num_kmod_refs, Z_WAITOK_ZERO, VM_KERN_MEMORY_OSKEXT);
    ...
    for (uint32_t refIndex = 0; refIndex < num_kmod_refs; refIndex++) {
      ...
      OSKext *refKext =
          OSDynamicCast(OSKext, dependencies->getObject(refIndex));
...
#endif
              kmod_info->name, kmod_info->hdr_size);
    result = KERN_FAILURE;
    goto finish;
  }
  ...
  /* VM protections and wiring for the Aux KC are done at collection loading
   * time */
  if (kc_type != KCKindAuxiliary || flags.resetSegmentsFromVnode) {
    /* if prelinked and primary KC, VM protections are already set */
    result =
        setVMAttributes(!isPrelinked() || flags.resetSegmentsFromVnode, true);
    if (result != KERN_SUCCESS) {
      goto finish;
    }
  }
...
finish:
  ...
  if (result != kOSReturnSuccess) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Failed to load executable for kext %s.", getIdentifierCString());

    if (kmod_info && kmod_info->reference_list) {
      kfree_type(kmod_reference_t, num_kmod_refs, kmod_info->reference_list);
    }
    if (isInterface()) {
      kfree_type(kmod_info_t, kmod_info);
      kmod_info = NULL;
...
/* static */
void OSKext::jettisonFileSetLinkeditSegment(kernel_mach_header_t *mh) {
  kernel_segment_command_t *linkeditseg = NULL;
  ...
  OSKextLog(
      /* kext */ NULL, kOSKextLogProgressLevel | kOSKextLogGeneralFlag,
...
#if !CONFIG_SPTM
  vm_offset_t linkedit_vmaddr = ml_static_ptovirt(
      (vm_offset_t)(linkeditseg->vmaddr - gVirtBase + gPhysBase));
#else
  ...
  OSKextLog(
      /* kext */ NULL, kOSKextLogProgressLevel | kOSKextLogGeneralFlag,
      "Jettisoning fileset Linkedit segments from vmaddr %llx with size %llu",
      (unsigned long long)linkedit_vmaddr,
      (unsigned long long)linkeditseg->vmsize);
#endif /* VM_MAPPED_KEXTS */
}
 ...
 *********************************************************************/
void OSKext::jettisonLinkeditSegment(void) {
 ...
 * them
 *********************************************************************/
void OSKext::jettisonDATASegmentPadding(void) {
  kernel_mach_header_t *mh;
...
/*********************************************************************
*********************************************************************/
void OSKext::setLinkedExecutable(OSData *anExecutable) {
  if (linkedExecutable) {
 ...
 *********************************************************************/
/* static */
void OSKext::registerKextsWithDTrace(void) {
...
/*********************************************************************
*********************************************************************/
void OSKext::registerWithDTrace(void) {
  ...
  if (!flags.dtraceInitialized && (dtrace_modload != NULL)) {
    ...
    if (!sKeepSymbols && kc_type == KCKindPrimary) {
      if (forceInit == kOSBooleanTrue) {
        OSKextLog(this, kOSKextLogBasicLevel | kOSKextLogGeneralFlag,
                  "Ignoring OSBundleForceDTraceInit for Boot KC Kext %s",
                  getIdentifierCString());
        forceInit = kOSBooleanFalse;
      }
      /* Linkedit segment of the Boot KC is gone, make sure fbt_provide_module
       * don't use kernel symbols */
...
/*********************************************************************
*********************************************************************/
void OSKext::unregisterWithDTrace(void) {
...
OSKext_protect(kernel_mach_header_t *kext_mh, vm_map_t map,
               vm_map_offset_t start, vm_map_size_t size, vm_prot_t new_prot,
               boolean_t set_max, kc_kind_t kc_type) {
  vm_map_offset_t end = start + size;

#pragma unused(kext_mh, map, kc_type)
  assert(map == kernel_map); // we can handle KEXTs arising from the PRELINK
                             // segment and no others
  assert(start <= end);
  if (start >= end) {
    return KERN_SUCCESS; // Punt segments of length zero (e.g., headers) or less
                         // (i.e., blunders)
  } else if (set_max) {
    return KERN_SUCCESS; // Punt set_max, as there's no mechanism to record that
                         // state
  } else {
...
static inline kern_return_t
OSKext_wire(kernel_mach_header_t *kext_mh, vm_map_t map, vm_map_offset_t start,
            vm_map_offset_t end, vm_prot_t access_type, boolean_t user_wire,
            kc_kind_t kc_type) {
#pragma unused(kext_mh, map, start, end, access_type, user_wire, kc_type)
  return KERN_SUCCESS; // No-op as PRELINK kexts are cemented into physical
                       // memory at boot
}
#else
...
OSKext_wire(kernel_mach_header_t *kext_mh, vm_map_t map, vm_map_offset_t start,
  ...
  return vm_map_wire_kernel(map, start, end, access_type, VM_KERN_MEMORY_KEXT,
                            user_wire);
}
#endif

OSReturn OSKext::setVMAttributes(bool protect, bool wire) {
...
#if !VM_MAPPED_KEXTS
  if (getcommandfromheader((kernel_mach_header_t *)kmod_info->address,
  ...
  result =
      (protect)
          ? OSKext_protect((kernel_mach_header_t *)kmod_info->address, kext_map,
                           kmod_info->address, kmod_info->hdr_size,
                           VM_PROT_READ, TRUE, kc_type)
          : KERN_SUCCESS;
  if (result != KERN_SUCCESS) {
    goto finish;
  }
  ...
  while (seg) {
     ...
     * sure OSKext_protect is not called twice on same page, if the page is
     * shared between two segments.
     *
     * Wiring: The non page aligned data at the start or at the end of the
     * segment is included in the wiring range, this inclusion is needed to make
     * sure all the data of the segment is wired.
     */
    ...
    if (protect && (end_protect > start_protect) &&
      ...
      result = OSKext_protect((kernel_mach_header_t *)kmod_info->address,
                              kext_map, start_protect, size_protect,
                              seg->maxprot, TRUE, kc_type);
      if (result != KERN_SUCCESS) {
        OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
    ...
    if (segmentShouldBeWired(seg) && wire) {
      result = OSKext_wire((kernel_mach_header_t *)kmod_info->address, kext_map,
                           start_wire, end_wire, seg->initprot, FALSE, kc_type);
      if (result != KERN_SUCCESS) {
        goto finish;
      }
    }
...
/*********************************************************************
*********************************************************************/
boolean_t OSKext::segmentShouldBeWired(kernel_segment_command_t *seg) {
  return sKeepSymbols ||
         (strncmp(seg->segname, SEG_LINKEDIT, sizeof(seg->segname)) &&
          strncmp(seg->segname, SEG_LINKINFO, sizeof(seg->segname)));
}

/*********************************************************************
*********************************************************************/
OSReturn OSKext::validateKextMapping(bool startFlag) {
  OSReturn result = kOSReturnError;
  ...
  if (!kmod_info) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Kext %s - NULL kmod_info pointer.", getIdentifierCString());
    result = kOSKextReturnBadData;
    goto finish;
  }
  ...
  if (!address) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Kext %s - NULL module %s pointer.", getIdentifierCString(),
              whichOp);
    result = kOSKextReturnBadData;
    goto finish;
  }
  ...
  if (isInFileset()) {
    ...
    address = (mach_vm_address_t)ptrauth_auth_data(
        (void *)address, ptrauth_key_function_pointer, 0);
  ...
  if (getcommandfromheader((kernel_mach_header_t *)kmod_info->address,
    ...
    for (seg = firstsegfromheader((kernel_mach_header_t *)kmod_info->address);
         seg != NULL; seg = nextsegfromheader(
                          (kernel_mach_header_t *)kmod_info->address, seg)) {
    ...
    if (!seg) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                ...
                getIdentifierCString(), whichOp, whichOp,
  ...
  if (startFlag) {
    if (!isInFileset() || kc_type != KCKindPrimary) {
       ...
       */
      kern_result =
          mach_vm_region_recurse(kernel_map, &address, &size, &depth,
                                 (vm_region_recurse_info_t)&info, &count);
      if (kern_result != KERN_SUCCESS) {
        OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                  "Kext %s - bad %s pointer %p.", getIdentifierCString(),
    ...
    if (((!isInFileset() || kc_type != KCKindPrimary) &&
        ...
        ((isInFileset() && kc_type == KCKindPrimary) &&
         ml_static_verify_page_protections(kext_segbase, kext_segsize,
...
/*********************************************************************
*********************************************************************/
boolean_t OSKext::verifySegmentMapping(kernel_segment_command_t *seg) {
  mach_vm_address_t address = 0;
  ...
  for (address = seg->vmaddr; address < round_page(seg->vmaddr + seg->vmsize);
       address += PAGE_SIZE) {
    if (!pmap_find_phys(kernel_pmap, (vm_offset_t)address)) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                "Kext %s - page %p is not backed by physical memory.",
                getIdentifierCString(), (void *)address);
      return false;
    }
  }
...
static void OSKextLogKextInfo(OSKext *aKext, uint64_t address, uint64_t size,
  ...
  trace_id.ftid_value =
      FIREHOSE_TRACE_ID_MAKE(firehose_tracepoint_namespace_metadata,
                             ...
                             (firehose_tracepoint_flags_t)0, code);

  uuid_data = aKext->copyTextUUID();
  if (uuid_data) {
    memcpy(uuid_info->ftui_uuid, uuid_data->getBytesNoCopy(),
           sizeof(uuid_info->ftui_uuid));
  }
...
void OSKext::OSKextLogDriverKitInfoLoad(OSKext *kext) {
  OSKextLogKextInfo(kext, kext->getLoadTag(), 1, firehose_tracepoint_code_load);
}

/*********************************************************************
*********************************************************************/
OSReturn OSKext::start(bool startDependenciesFlag) {
  OSReturn result = kOSReturnError;
  ...
  if (!isLoaded()) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Attempt to start nonloaded kext %s.", getIdentifierCString());
    result = kOSKextReturnInvalidArgument;
    goto finish;
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!dependency->isStarted()) {
      if (startDependenciesFlag) {
        OSReturn dependencyResult = dependency->start(startDependenciesFlag);
        if (dependencyResult != KERN_SUCCESS) {
          OSKextLog(
              this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Kext %s start - dependency %s failed to start (error 0x%x).",
              getIdentifierCString(), dependency->getIdentifierCString(),
              dependencyResult);
          goto finish;
        }
      } else {
  ...
  OSKextLog(this, kOSKextLogDetailLevel | kOSKextLogLoadFlag,
            "Kext %s calling module start function.", getIdentifierCString());

  flags.starting = 1;

  // Drop a log message so logd can grab the needed information to decode this
  // kext
  OSKextLogKextInfo(this, kmod_info->address, kmod_info->size,
  ...
  if (result == KERN_SUCCESS) {
    ...
    OSKextLog(this, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
              "Kext %s is now started.", getIdentifierCString());
  } else {
    invokeOrCancelRequestCallbacks(
        /* result not actually used */ kOSKextReturnStartStopError,
        /* invokeFlag */ false);
    OSKextLog(this, kOSKextLogWarningLevel | kOSKextLogLoadFlag,
...
*********************************************************************/
/* static */
bool OSKext::canUnloadKextWithIdentifier(OSString *kextIdentifier,
...
OSReturn OSKext::stop(void) {
  ...
  if (!isLoaded()) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Attempt to stop nonloaded kext %s.", getIdentifierCString());
    result = kOSKextReturnInvalidArgument;
    goto finish;
  }
  ...
  if (getRetainCount() > kOSKextMinLoadedRetainCount) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Kext %s - C++ instances; can't stop.", getIdentifierCString());
    result = kOSKextReturnInUse;
    goto finish;
  }
  ...
  if (stopfunc) {
    OSKextLog(this, kOSKextLogDetailLevel | kOSKextLogLoadFlag,
              "Kext %s calling module stop function.", getIdentifierCString());
...
OSReturn OSKext::unload(void) {
  ...
  if (!sUnloadEnabled) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Kext unloading is disabled (%s).", this->getIdentifierCString());
  ...
  if (getRetainCount() > kOSKextMinLoadedRetainCount) {
    ...
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogKextBookkeepingFlag,
              "Can't unload kext %s; outstanding references %d (linkage or "
              "tracking object).",
  ...
  if (metaClasses && !OSMetaClass::removeClasses(metaClasses.get())) {
    OSKextLog(this,
              ...
              getIdentifierCString());
    reportOSMetaClassInstances(kOSKextLogErrorLevel | kOSKextLogLoadFlag |
                               kOSKextLogKextBookkeepingFlag);
    result = kOSKextReturnInUse;
    goto finish;
  }
  ...
  OSKextLog(this, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
            "Kext %s unloading.", getIdentifierCString());

  {
    ...
    for (p = pendingPgoHead.next; p != &pendingPgoHead; p = next) {
      ...
      s->err = OSKextGrabPgoDataLocked(this, s->metadata, instance_uuid,
                                       s->pSize, s->pBuffer, s->bufferSize);
      prev = p->prev;
      next = p->next;
      prev->next = next;
      next->prev = prev;
  ...
  if (index != (unsigned int)-1) {
    ...
    OSKext *nextKext = OSDynamicCast(OSKext, sLoadedKexts->getObject(index));

    if (nextKext) {
      if (index > 0) {
        OSKext *gapKext =
            OSDynamicCast(OSKext, sLoadedKexts->getObject(index - 1));
  ...
  if (linkedExecutable) {
#if KASAN
    kasan_unload_kext((vm_offset_t)linkedExecutable->getBytesNoCopy(),
                      linkedExecutable->getLength());
#endif
    ...
    if (!isInterface() && (!in_fileset || flags.resetSegmentsFromVnode)) {
      ...
      while (seg) {
        if (segmentShouldBeWired(seg)) {
          ...
          result = vm_map_unwire(kext_map, start_wire, end_wire, FALSE);
          if (result != KERN_SUCCESS) {
            OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                      "Failed to unwire kext %s.", getIdentifierCString());
            result = kOSKextReturnInternalError;
            goto finish;
          }
        }
        ...
        seg =
            nextsegfromheader((kernel_mach_header_t *)kmod_info->address, seg);
      }
#if defined(__x86_64__) || defined(__i386__)
    ...
    if (flags.resetSegmentsFromImmutableCopy) {
      ...
      if (result != kOSReturnSuccess) {
        OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
                  "Failed to reset kext %s.", getIdentifierCString());
        result = kOSKextReturnInternalError;
        goto finish;
      }
    }
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::queueKextNotification(const char *notificationName,
  ...
  if (NULL != dextUniqueIdentifier) {
    if (!_OSKextSetRequestArgument(loadRequest.get(),
                                   kKextRequestArgumentDriverUniqueIdentifier,
                                   dextUniqueIdentifier)) {
      result = kOSKextReturnNoMemory;
      goto finish;
    }
  }
...
*********************************************************************/
static void
_OSKextConsiderDestroyingLinkContext(__unused thread_call_param_t p0,
 ...
 *********************************************************************/
/* static */
void OSKext::considerDestroyingLinkContext(void) {
  IORecursiveLockLock(sKextInnerLock);
   ...
   */
  sDestroyLinkContextThread =
      thread_call_allocate(&_OSKextConsiderDestroyingLinkContext, NULL);
  if (!sDestroyLinkContextThread) {
    OSKextLog(/* kext */ NULL,
...
/* static */
void OSKext::considerDestroyingLinkContext(void) { return; }
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::autounloadKext(OSKext *aKext) {
  OSReturn result = kOSKextReturnInUse;
   ...
   */
  if ((aKext->getRetainCount() > kOSKextMinLoadedRetainCount) ||
      !aKext->flags.autounloadEnabled || aKext->isKernelComponent()) {
    goto finish;
  }
  ...
  if (aKext->hasOSMetaClassInstances() || aKext->countRequestCallbacks()) {
    goto finish;
  }
...
/*********************************************************************
*********************************************************************/
void _OSKextConsiderUnloads(__unused thread_call_param_t p0,
                            __unused thread_call_param_t p1) {
  ...
  OSKextLog(/* kext */ NULL, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
  ...
  if (count) {
    ...
    do {
      OSDictionary *callbackRecord =
          OSDynamicCast(OSDictionary, sRequestCallbackRecords->getObject(i));
      if (callbackRecord) {
        OSBoolean *stale = OSDynamicCast(
            OSBoolean, callbackRecord->getObject(kKextRequestStaleKey));

        if (stale == kOSBooleanTrue) {
          OSKext::invokeRequestCallback(callbackRecord, kOSKextReturnTimeout);
        } else {
          callbackRecord->setObject(kKextRequestStaleKey, kOSBooleanTrue);
        }
      }
    } while (i--);
  }
 ...
 *********************************************************************/
/* static */
void OSKext::considerRebuildOfPrelinkedKernel(void) {
  ...
  /* We need to wait for the IOKit daemon to get up and running with unloads
   * already done and any new startup kexts loaded.
   */
  ...
  if (!_OSKextInPrelinkRebuildWindow()) {
    OSKextLog(/* kext */ NULL, kOSKextLogArchiveFlag,
              "%s prebuild rebuild has expired", __FUNCTION__);
  ...
  checkResult =
      _OSKextCreateRequest(kKextRequestPredicateRequestPrelink, prelinkRequest);
  if (checkResult != kOSReturnSuccess) {
    goto finish;
  }
...
/*********************************************************************
*********************************************************************/
bool OSKext::resolveDependencies(OSArray *loopStack) {
  bool result = false;
  ...
  if (loopStack) {
    if (loopStack->getNextIndexOfObject(this, 0) != (unsigned int)-1) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogDependenciesFlag,
                "Kext %s has a dependency loop; can't resolve dependencies.",
                getIdentifierCString());
      goto finish;
    }
  } else {
    OSKextLog(this, kOSKextLogStepLevel | kOSKextLogDependenciesFlag,
              "Kext %s resolving dependencies.", getIdentifierCString());

    localLoopStack = OSArray::withCapacity(6); // any small capacity will do
    if (!localLoopStack) {
      OSKextLog(
          this, kOSKextLogErrorLevel | kOSKextLogDependenciesFlag,
  ...
  while (
      (libraryID = OSDynamicCast(OSString, libraryIterator->getNextObject()))) {
    const char *library_id = libraryID->getCStringNoCopy();

    OSString *libraryVersion =
        OSDynamicCast(OSString, libraries->getObject(libraryID));
    ...
    if (libraryVers == -1) {
      OSKextLog(this,
                kOSKextLogErrorLevel | kOSKextLogValidationFlag |
                    kOSKextLogDependenciesFlag,
                "Kext %s - invalid library version %s.", getIdentifierCString(),
    ...
    if (libraryKext == NULL) {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogDependenciesFlag,
                "Kext %s - library kext %s not found.", getIdentifierCString(),
                library_id);
      goto finish;
    }
     ...
     */
    if (this->isPrelinked() && libraryKext->declaresExecutable() &&
    ...
    if (libraryKext->declaresExecutable() || libraryKext->isInterface()) {
      if (dependencies->getNextIndexOfObject(libraryKext, 0) == (unsigned)-1) {
        dependencies->setObject(libraryKext);

        OSKextLog(this, kOSKextLogDetailLevel | kOSKextLogDependenciesFlag,
                  "Kext %s added dependency %s.", getIdentifierCString(),
      ...
      if (numLibDependencies) {
        ...
        OSKextLog(this, kOSKextLogDetailLevel | kOSKextLogDependenciesFlag,
                  ...
                  getIdentifierCString(), numLibDependencies,
                  libraryKext->getIdentifierCString());
      }
      for (index = 0; index < numLibDependencies; index++) {
        OSKext *thisLibDependency =
            OSDynamicCast(OSKext, libraryDependencies->getObject(index));
        if (dependencies->getNextIndexOfObject(thisLibDependency, 0) ==
            (unsigned)-1) {
  ...
  if (!hasKPIDependency) {
    OSKextLog(this, kOSKextLogWarningLevel | kOSKextLogDependenciesFlag,
  ...
  if (!hasKernelDependency && !hasKPIDependency) {
    ...
    OSKext *kernelKext =
        OSDynamicCast(OSKext, sKextsByID->getObject(KERNEL6_LIB));
    if (kernelKext) {
      dependencies->setObject(kernelKext);
    } else {
      OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogDependenciesFlag,
                "Error - Library %s not found for %s.", KERNEL6_LIB,
                getIdentifierCString());
    }
  }
  ...
  if (hasPrivateKPIDependency) {
    ...
    hasApplePrefix =
        STRING_HAS_PREFIX(getIdentifierCString(), APPLE_KEXT_PREFIX);
    ...
    readableString = OSDynamicCast(
        OSString, getPropertyForHostArch("NSHumanReadableCopyright"));
    if (readableString) {
...
/*********************************************************************
*********************************************************************/
bool OSKext::addBleedthroughDependencies(OSArray *anArray) {
  bool result = false;
  unsigned int dependencyIndex, dependencyCount;

  dependencyCount = getNumDependencies();

  for (dependencyIndex = 0; dependencyIndex < dependencyCount;
       dependencyIndex++) {
    OSKext *dependency =
        OSDynamicCast(OSKext, dependencies->getObject(dependencyIndex));
    if (!dependency) {
      OSKextLog(
          this, kOSKextLogErrorLevel | kOSKextLogDependenciesFlag,
...
/*********************************************************************
*********************************************************************/
bool OSKext::flushDependencies(bool forceFlag) {
  bool result = false;
  ...
  if (!isLoaded() || forceFlag) {
    if (dependencies) {
      // xxx - check level
      OSKextLog(this, kOSKextLogProgressLevel | kOSKextLogDependenciesFlag,
                "Kext %s flushing dependencies.", getIdentifierCString());
      dependencies.reset();
    }
    if (!isKernelComponent()) {
...
/*********************************************************************
*********************************************************************/
uint32_t OSKext::getNumDependencies(void) {
  if (!dependencies) {
    return 0;
  }
...
/*********************************************************************
*********************************************************************/
OSArray *OSKext::getDependencies(void) { return dependencies.get(); }

bool OSKext::hasDependency(const OSSymbol *depID) {
  bool result __block;
...
/*********************************************************************
*********************************************************************/
OSReturn OSKext::addClass(OSMetaClass *aClass, uint32_t numClasses) {
  ...
  if (metaClasses->containsObject(aClass)) {
    OSKextLog(this, kOSKextLogWarningLevel | kOSKextLogLoadFlag,
              ...
              getIdentifierCString(), aClass->getClassName());
    result = kOSReturnSuccess;
    goto finish;
  }
  ...
  if (!metaClasses->setObject(aClass)) {
    ...
    OSKextLog(this, kOSKextLogDetailLevel | kOSKextLogLoadFlag,
              "Kext %s registered class %s.", getIdentifierCString(),
              aClass->getClassName());
  }
...
finish:
  if (result != kOSReturnSuccess) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Kext %s failed to register class %s.", getIdentifierCString(),
              aClass->getClassName());
  }
...
/*********************************************************************
*********************************************************************/
OSReturn OSKext::removeClass(OSMetaClass *aClass) {
  ...
  OSKextLog(this, kOSKextLogDetailLevel | kOSKextLogLoadFlag,
            "Kext %s unregistering class %s.", getIdentifierCString(),
            aClass->getClassName());
...
finish:
  if (result != kOSReturnSuccess) {
    OSKextLog(this, kOSKextLogErrorLevel | kOSKextLogLoadFlag,
              "Failed to unregister kext %s class %s.", getIdentifierCString(),
              aClass->getClassName());
  }
  return result;
}
...
/*********************************************************************
*********************************************************************/
OSSet *OSKext::getMetaClasses(void) { return metaClasses.get(); }

/*********************************************************************
*********************************************************************/
bool OSKext::hasOSMetaClassInstances(void) {
  bool result = false;
...
*********************************************************************/
/* static */
void OSKext::reportOSMetaClassInstances(const char *kextIdentifier,
...
/*********************************************************************
*********************************************************************/
void OSKext::reportOSMetaClassInstances(OSKextLogSpec msgLogSpec) {
  OSSharedPtr<OSCollectionIterator> classIterator;
  OSMetaClass *checkClass = NULL; // do not release
  ...
  while ((checkClass = (OSMetaClass *)classIterator->getNextObject())) {
    if (checkClass->getInstanceCount()) {
      OSKextLog(this, msgLogSpec, "    Kext %s class %s has %d instance%s.",
                getIdentifierCString(), checkClass->getClassName(),
...
static kern_return_t patchDextLaunchRequests(task_t calling_task,
                                             OSArray *requests) {
  OSReturn result = kOSReturnSuccess;
  for (uint32_t requestIndex = 0; requestIndex < requests->getCount();
       requestIndex++) {
    ...
    if (!request) {
      OSKextLog(/* kext */ NULL, kOSKextLogGeneralFlag | kOSKextLogErrorLevel,
    ...
    if (requestPredicate->isEqualTo(kKextRequestPredicateRequestDaemonLaunch)) {
      token = OSDynamicCast(
          ...
          _OSKextGetRequestArgument(request, kKextRequestArgumentCheckInToken));
      if (!token) {
        OSKextLog(/* kext */ NULL, kOSKextLogGeneralFlag | kOSKextLogErrorLevel,
                  "Could not find a IOUserServerCheckInToken in daemon launch "
                  "request.");
        result = kOSKextReturnInternalError;
        goto finish;
      }
      ...
      portNameNumber =
          OSNumber::withNumber(portName, CHAR_BIT * sizeof(portName));
      if (!portNameNumber) {
        OSKextLog(/* kext */ NULL, kOSKextLogGeneralFlag | kOSKextLogErrorLevel,
      ...
      if (!_OSKextSetRequestArgument(request, kKextRequestArgumentCheckInToken,
                                     portNameNumber.get())) {
        OSKextLog(/* kext */ NULL, kOSKextLogGeneralFlag | kOSKextLogErrorLevel,
                  "Could not set OSNumber object as "
                  "request " kKextRequestArgumentCheckInToken);
        result = kOSKextReturnNoMemory;
        goto finish;
      }
    }
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::handleRequest(host_priv_t hostPriv,
                               OSKextLogSpec clientLogFilter,
                               char *requestBuffer, uint32_t requestLength,
                               char **responseOut, uint32_t *responseLengthOut,
                               char **logInfoOut, uint32_t *logInfoLengthOut) {
  OSReturn result = kOSReturnError;
  kern_return_t kmem_result = KERN_FAILURE;
  ...
  if (!requestDict) {
    ...
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogIPCFlag,
              "Error unserializing request from user space: %s.", errorCString);
  ...
  if (predicate->isEqualTo(kKextRequestPredicateUnload) ||
    ...
    if (!iokitDaemonAvailable()) {
      panic("Received unexpected request in environment where " kIOKitDaemonName
            " is unavailable");
    }
    ...
    taskCanManageAllKCs = IOCurrentTaskHasEntitlement(
                              kOSKextCollectionManagementEntitlement) == TRUE;
    taskOnlyManagesBootKC = IOCurrentTaskHasEntitlement(
                                kOSKextOnlyBootKCManagementEntitlement) == TRUE;
   ...
   */
  kextIdentifier = OSDynamicCast(
      OSString, _OSKextGetRequestArgument(
  ...
  boolArg = OSDynamicCast(
      OSBoolean,
      _OSKextGetRequestArgument(requestDict, kKextRequestArgumentValueKey));

  if (taskOnlyManagesBootKC && theKext && theKext->isInFileset() &&
      theKext->kc_type != KCKindPrimary) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogIPCFlag,
              "Access Failure - client not entitled to manage kext in "
              "non-primary KC");
    result = kOSKextReturnNotPrivileged;
    goto finish;
  }
  ...
  if (predicate->isEqualTo(kKextRequestPredicateStart)) {
    ...
    if (!kextIdentifier) {
      OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogIPCFlag,
                "Invalid arguments to codeless kext load interface (missing "
                "identifier).");
    } else {
    ...
    if (!kextIdentifier) {
      ...
      if (theKext->isDriverKit()) {
        ...
        OSBoolean *terminateFlag = OSDynamicCast(
            OSBoolean,
            _OSKextGetRequestArgument(
                requestDict, kKextRequestArgumentTerminateIOServicesKey));
    ...
    infoKeysRaw =
        _OSKextGetRequestArgument(requestDict, kKextRequestArgumentInfoKeysKey);
    ...
    if (predicate->isEqualTo(kKextRequestPredicateGetLoaded)) {
      ...
      responseObject =
          OSKext::copyLoadedKextInfoByUUID(kextIdentifiers, infoKeys);
    } else if (predicate->isEqualTo(
                   kKextRequestPredicateGetKextsInCollection)) {
  ...
  if (responseOut && responseObject) {
     ...
     * that the value for kKextRequestArgumentCheckInToken is a mach port name
     * for the IOUserServerCheckInToken kernel object.
     */
    if (predicate->isEqualTo(kKextRequestPredicateGetKernelRequests)) {
      ...
      if (!requests) {
        OSKextLog(/* kext */ NULL, kOSKextLogGeneralFlag | kOSKextLogErrorLevel,
                  "responseObject should be an OSArray if predicate "
                  "is " kKextRequestPredicateGetKernelRequests);
        result = kOSKextReturnInternalError;
        goto finish;
      }
  ...
  if (responseOut && response) {
     ...
     */
    kmem_result = kmem_alloc(kernel_map, (vm_offset_t *)&buffer,
                             round_page(responseLength), KMA_DATA_SHARED,
                             VM_KERN_MEMORY_OSKEXT);
    if (kmem_result != KERN_SUCCESS) {
      OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogIPCFlag,
...
finish:
  ...
  if (logInfoArray && logInfoOut && logInfoLengthOut) {
    (void)OSKext::serializeLogInfo(logInfoArray.get(), logInfoOut,
                                   logInfoLengthOut);
  }
...
static int __whereIsAddr(vm_offset_t theAddr, unsigned long *segSizes,
                         vm_offset_t *segAddrs, int segCount) {
...
static void __slideOldKaslrOffsets(kernel_mach_header_t *mh,
                                   kernel_segment_command_t *kextTextSeg,
                                   OSData *kaslrOffsets) {
  static const char *plk_segNames[] = {
      "__TEXT",          "__TEXT_EXEC",    "__DATA",
      "__DATA_CONST",    "__LINKEDIT",     "__PRELINK_TEXT",
      "__PLK_TEXT_EXEC", "__PRELINK_DATA", "__PLK_DATA_CONST",
      "__PLK_LLVM_COV",  "__PLK_LINKEDIT", "__PRELINK_INFO"};
  static const size_t num_plk_seg =
      (size_t)(sizeof(plk_segNames) / sizeof(plk_segNames[0]));
  ...
  for (size_t i = 0; i < num_plk_seg; i++) {
    plk_segSizes[i] = 0;
    plk_segAddrs[i] = (vm_offset_t)getsegdatafromheader(mh, plk_segNames[i],
                                                        &plk_segSizes[i]);
  }
  ...
  for (uint32_t j = 0; j < myOffsets->count; j++) {
    ...
    vm_offset_t *slideAddr =
        (vm_offset_t *)((uint64_t)kextTextStart + slideOffset);
    int slideAddrSegIndex = -1;
    int addrToSlideSegIndex = -1;

    slideAddrSegIndex = __whereIsAddr((vm_offset_t)slideAddr, &plk_segSizes[0],
                                      &plk_segAddrs[0], num_plk_seg);
    if (slideAddrSegIndex >= 0) {
      addrToSlideSegIndex =
          __whereIsAddr(ml_static_slide(*slideAddr), &plk_segSizes[0],
                        &plk_segAddrs[0], num_plk_seg);
 ...
 *
 ********************************************************************/
bool OSKext::addKextsFromKextCollection(kernel_mach_header_t *mh,
                                        OSDictionary *infoDict,
  ...
  infoDictKCUUID =
      OSDynamicCast(OSData, infoDict->getObject(kPrelinkInfoKCIDKey));
  if (infoDictKCUUID) {
    if (infoDictKCUUID->getLength() != sizeof(uuid_t)) {
      panic("kcUUID length is %d, expected %lu", infoDictKCUUID->getLength(),
            sizeof(uuid_t));
    }
  }
  ...
  kextArray =
      OSDynamicCast(OSArray, infoDict->getObject(kPrelinkInfoDictionaryKey));
  if (!kextArray) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
   ...
   */
  kaslrOffsets =
      OSDynamicCast(OSData, infoDict->getObject(kPrelinkLinkKASLROffsetsKey));
  ...
  for (int i = 0; i < (int)kextArray->getCount(); ++i) {
    ...
    if (!kextDict) {
      OSKextLog(/* kext */ NULL,
                kOSKextLogErrorLevel | kOSKextLogDirectoryScanFlag |
                    kOSKextLogArchiveFlag,
  ...
  if (kaslrOffsets && vm_kernel_slide > 0) {
    ...
    /* All kexts covered by the old-style kaslr relocation list are now slid,
     * set VM protections for them */
    setAllVMAttributes();
  }
  ...
  kcKextCount = OSNumber::withNumber((unsigned long long)infoDict->getCount(),
                                     8 * sizeof(uint32_t));
  assert(kcKextCount);
  if (kcKextCount) {
    OSSharedPtr<OSObject> prop =
        registryRoot->copyProperty(kOSPrelinkKextCountKey);
    OSNumber *num;
  ...
  OSKextLog(/* kext */ NULL,
            kOSKextLogProgressLevel | kOSKextLogGeneralFlag |
                kOSKextLogKextBookkeepingFlag | kOSKextLogDirectoryScanFlag |
                kOSKextLogArchiveFlag,
...
bool OSKext::addKextsFromKextCollection(kernel_mach_header_t *mh,
                                        OSDictionary *infoDict,
                                        const char *text_seg_name,
                                        OSSharedPtr<OSData> &kcUUID,
                                        kc_kind_t type) {
  OSData *result = NULL;
  bool success =
      addKextsFromKextCollection(mh, infoDict, text_seg_name, &result, type);
  if (success) {
...
bool OSKext::registerDeferredKextCollection(kernel_mach_header_t *mh,
                                            OSSharedPtr<OSObject> &parsedXML,
                                            kc_kind_t type) {
...
OSSharedPtr<OSObject> OSKext::consumeDeferredKextCollection(kc_kind_t type) {
...
uint64_t __llvm_profile_get_size_for_buffer_internal(
    const char *DataBegin, const char *DataEnd, const char *CountersBegin,
    const char *CountersEnd, const char *BitmapBegin, const char *BitmapEnd,
    const char *NamesBegin, const char *NamesEnd, const char *VTableBegin,
    const char *VTableEnd, const char *VNamesBegin, const char *VNamesEnd);
...
static void OSKextPgoMetadataPut(char *pBuffer, size_t *position,
                                 size_t bufferSize, uint32_t *num_pairs,
  ...
  *position += len;
  if (pBuffer && bufferSize && *position <= bufferSize) {
    memcpy(pos, key, strlen_key);
    pos += strlen_key;
    *(pos++) = '=';
    memcpy(pos, value, strlen_value);
    pos += strlen_value;
    *(pos++) = 0;
    if (num_pairs) {
      (*num_pairs)++;
...
static void OSKextPgoMetadataPutMax(size_t *position, const char *key,
                                    size_t value_max) {
  *position += strlen(key) + 1 + value_max + 1;
}

static void OSKextPgoMetadataPutAll(OSKext *kext, uuid_t instance_uuid,
                                    char *pBuffer, size_t *position,
                                    size_t bufferSize, uint32_t *num_pairs) {
  ...
  if (!pBuffer) {
    ...
    OSKextPgoMetadataPut(pBuffer, position, bufferSize, num_pairs, "INSTANCE",
    ...
    if (uuid_data) {
      ...
      OSKextPgoMetadataPut(pBuffer, position, bufferSize, num_pairs, "UUID",
                           uuid_string);
    }
    ...
    snprintf(timestamp, sizeof(timestamp), "%lu.%06d", (unsigned long)secs,
             (int)usecs);
    OSKextPgoMetadataPut(pBuffer, position, bufferSize, num_pairs, "TIMESTAMP",
                         timestamp);
  }

  OSKextPgoMetadataPut(pBuffer, position, bufferSize, num_pairs, "NAME",
                       kext->getIdentifierCString());

  char versionCString[kOSKextVersionMaxLength];
  OSKextVersionGetString(kext->getVersion(), versionCString,
                         kOSKextVersionMaxLength);
  OSKextPgoMetadataPut(pBuffer, position, bufferSize, num_pairs, "VERSION",
                       versionCString);
}

static size_t OSKextPgoMetadataSize(OSKext *kext) {
  size_t position = 0;
  uuid_t fakeuuid = {};
...
int OSKextGrabPgoDataLocked(OSKext *kext, bool metadata, uuid_t instance_uuid,
                            uint64_t *pSize, char *pBuffer,
                            uint64_t bufferSize) {
  int err = 0;
  ...
  if (!sect_prf_name) {
    ...
    // <rdar://problem/52080551> 16 character section name is truncated to 15
    // characters by kextcache
  ...
  size = __llvm_profile_get_size_for_buffer_internal(
      (const char *)sect_prf_data->addr,
      (const char *)sect_prf_data->addr + sect_prf_data->size,
      (const char *)sect_prf_cnts->addr,
      (const char *)sect_prf_cnts->addr + sect_prf_cnts->size,
      unused_section /* bits */, unused_section /* bits end */,
      (const char *)sect_prf_name->addr,
      (const char *)sect_prf_name->addr + sect_prf_name->size,
  ...
  if (pBuffer && bufferSize) {
    ...
    err = __llvm_profile_write_buffer_internal(
        pBuffer, (const char *)sect_prf_data->addr,
        (const char *)sect_prf_data->addr + sect_prf_data->size,
        (const char *)sect_prf_cnts->addr,
        (const char *)sect_prf_cnts->addr + sect_prf_cnts->size,
        unused_section /* bits */, unused_section /* bits end */,
    ...
    if (metadata) {
      ...
      struct pgo_metadata_footer *footerp =
          (struct pgo_metadata_footer *)(end_of_buffer -
      ...
      OSKextPgoMetadataPutAll(kext, instance_uuid, metadata_buffer,
                              &metadata_position, metadata_size, &num_pairs);
...
int OSKextGrabPgoData(uuid_t uuid, uint64_t *pSize, char *pBuffer,
  ...
  if (wait_for_unload) {
    ...
    err = OSKextGrabPgoDataLocked(kext.get(), metadata, kext->instance_uuid,
                                  pSize, pBuffer, bufferSize);
  }
...
void OSKextResetPgoCountersLock() { IORecursiveLockLock(sKextLock); }

void OSKextResetPgoCountersUnlock() { IORecursiveLockUnlock(sKextLock); }
...
void OSKextResetPgoCounters() {
  ...
  for (uint32_t i = 0; i < count; i++) {
    ...
    kernel_section_t *sect_prf_cnts =
        kext->lookupSection("__DATA", "__llvm_prf_cnts");
    if (!sect_prf_cnts) {
      continue;
    }
...
OSKext::copyLoadedKextInfoByUUID(OSArray *kextIdentifiers, OSArray *infoKeys) {
  OSSharedPtr<OSDictionary> result;
  ...
  for (j = 0; j < (sizeof(list) / sizeof(list[0])); j++) {
    for (i = 0; i < count[j]; i++) {
      ...
      memcpy(&thisKextTextUUID, uuid_data->getBytesNoCopy(),
             sizeof(thisKextTextUUID));
      ...
      if (kextIdentifiers) {
        ...
        for (idIndex = 0; idIndex < idCount; idIndex++) {
          const OSString *wantedUUID =
              OSDynamicCast(OSString, kextIdentifiers->getObject(idIndex));
...
/* static */
OSSharedPtr<OSDictionary>
OSKext::copyKextCollectionInfo(OSDictionary *requestDict, OSArray *infoKeys) {
  OSSharedPtr<OSDictionary> result;
  ...
  collectionType = OSDynamicCast(
      OSString, _OSKextGetRequestArgument(
                    requestDict, kKextRequestArgumentCollectionTypeKey));
  if (!collectionType) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogIPCFlag,
  ...
  rawLoadedState = _OSKextGetRequestArgument(
      requestDict, kKextRequestArgumentLoadedStateKey);
  if (rawLoadedState) {
  ...
  { // start block scope
    sKextsByID->iterateObjects(
        ^bool(const OSSymbol *thisKextID, OSObject *obj) {
          OSKext *thisKext = NULL; // do not release
          OSSharedPtr<OSDictionary> kextInfo;
           ...
           */
          if ((kc_request_kind != KCKindUnknown) &&
              (thisKext->kc_type != kc_request_kind)) {
            return false;
          }
           ...
           */
          if (onlyLoaded &&
              (-1U == sLoadedKexts->getNextIndexOfObject(thisKext, 0))) {
            return false;
          }
          if (onlyUnloaded &&
              (-1U != sLoadedKexts->getNextIndexOfObject(thisKext, 0))) {
...
/* static */
OSSharedPtr<OSArray> OSKext::copyDextsInfo(OSArray *kextIdentifiers,
                                           OSArray *infoKeys) {
   ...
   */
  if (infoKeys &&
      _OSArrayContainsCString(infoKeys, kOSBundleDextStateActiveKey)) {
    getActive = true;
  }
  if (infoKeys &&
      _OSArrayContainsCString(infoKeys, kOSBundleDextStateActiveLoadedKey)) {
    getLoaded = true;
  }
  if (infoKeys &&
      _OSArrayContainsCString(infoKeys, kOSBundleDextStateActiveUnloadedKey)) {
    getUnloaded = true;
  }
  if (infoKeys &&
      _OSArrayContainsCString(infoKeys, kOSBundleDextStatePendingUpgradeKey)) {
   ...
   */
  avgDextCount =
      sLoadedDriverKitKexts->getCount() + sDriverKitToUpgradeByID->getCount();
  ...
  { // start block scope
    if (getActive || getLoaded || getUnloaded) {
      sKextsByID->iterateObjects(^bool(const OSSymbol *thisKextID,
                                       OSObject *obj) {
        OSKext *thisKext = NULL; // do not release
        OSSharedPtr<OSDictionary> kextInfo;
        Boolean includeThis = true;
    ...
    if (getPendingUpgrade) {
      sDriverKitToUpgradeByID->iterateObjects(^bool(const OSSymbol *thisKextID,
                                                    OSObject *obj) {
        OSKext *thisKext = NULL; // do not release
        OSSharedPtr<OSDictionary> kextInfo;
        Boolean includeThis = true;
        ...
        if (kextInfo) {
          OSSharedPtr<OSString> state =
              OSString::withCString(kOSBundleDextStatePendingUpgradeKey);
          kextInfo->setObject(kOSBundleDextStateKey, state.get());
          result->setObject(kextInfo.get());
        }
        return false;
...
*********************************************************************/
/* static */
OSSharedPtr<OSDictionary> OSKext::copyLoadedKextInfo(OSArray *kextIdentifiers,
                                                     OSArray *infoKeys) {
  OSSharedPtr<OSDictionary> result;
  uint32_t idCount = 0;
  bool onlyLoaded;
  ...
  onlyLoaded = (!infoKeys ||
                !_OSArrayContainsCString(infoKeys, kOSBundleAllPrelinkedKey));
...
OSSharedPtr<OSDictionary> OSKext::copyInfo(OSArray *infoKeys) {
  OSSharedPtr<OSDictionary> result;
  bool success = false;
  ...
  if (!infoKeys ||
    ...
    if (linkedExecutable && !isInterface()) {
      kernel_mach_header_t *kext_mach_hdr =
          (kernel_mach_header_t *)linkedExecutable->getBytesNoCopy();
      ...
      if (!infoKeys ||
        ...
        temp_kext_mach_hdr =
            (kernel_mach_header_t *)headerData->getBytesNoCopy();
        if (temp_kext_mach_hdr == NULL) {
          goto finish;
        }

        lcp = (struct load_command *)(temp_kext_mach_hdr + 1);
        for (i = 0; (i < temp_kext_mach_hdr->ncmds) && !flags.unslidMachO;
             i++) {
          if (lcp->cmd == LC_SEGMENT_KERNEL) {
            ...
            if (flags.jettisonLinkeditSeg) {
              if (strncmp(segp->segname, SEG_LINKEDIT, sizeof(segp->segname)) ==
                  0) {
                segp->vmsize = 0;
                segp->fileoff = 0;
                segp->filesize = 0;
...
#if __arm__ || __arm64__
            // iBoot disregards zero-size segments, just set their addresses to
            // gVirtBase and unslide them to avoid vm assertion failures /
            // kernel logging breakage.
            if (segp->vmsize == 0 && segp->vmaddr < gVirtBase) {
              segp->vmaddr = gVirtBase;
              for (secp = firstsect(segp); secp != NULL;
                   secp = nextsect(segp, secp)) {
      ...
      if (_OSArrayContainsCString(infoKeys, kOSBundleLogStringsKey)) {
        ...
        char headerBytes[offsetof(osLogDataHeaderRef, sections) +
                         NUM_OS_LOG_SECTIONS * sizeof(header->sections[0])];
        ...
        os_log_data = getsectdatafromheader(kext_mach_hdr, "__TEXT", "__os_log",
                                            &os_log_size);
        cstring_data = getsectdatafromheader(kext_mach_hdr, "__TEXT",
                                             "__cstring", &cstring_size);
        asan_cstring_data = getsectdatafromheader(
            kext_mach_hdr, "__TEXT", "__asan_cstring", &asan_cstring_size);
        ...
        if (flags.unslidMachO) {
          os_log_data = (os_log_data != nullptr)
                            ? (void *)ml_static_slide((vm_offset_t)os_log_data)
                            : nullptr;
          cstring_data =
              (cstring_data != nullptr)
                  ? (void *)ml_static_slide((vm_offset_t)cstring_data)
                  : nullptr;
          asan_cstring_data =
              (asan_cstring_data != nullptr)
                  ? (void *)ml_static_slide((vm_offset_t)asan_cstring_data)
                  : nullptr;
        }
        ...
        asan_cstring_offset =
            (uintptr_t)asan_cstring_data - (uintptr_t)kext_mach_hdr;
        ...
        header->sections[ASAN_CSTRING_SECT_IDX].sect_offset =
            asan_cstring_offset;
        ...
        res = logData->appendBytes(
            &(header->sections[0]),
            (u_int)(header->sect_count * sizeof(header->sections[0])));
        if (!res) {
          goto finish;
        }
        if (os_log_data) {
          res = logData->appendBytes(
              os_log_data, (u_int)header->sections[OS_LOG_SECT_IDX].sect_size);
          if (!res) {
            goto finish;
          }
        }
        if (cstring_data) {
          res = logData->appendBytes(
              cstring_data,
              (u_int)header->sections[CSTRING_SECT_IDX].sect_size);
          if (!res) {
            goto finish;
          }
        }
      ...
      if (!infoKeys || _OSArrayContainsCString(infoKeys, kOSBundleCPUTypeKey)) {
        cpuTypeNumber =
            OSNumber::withNumber((uint64_t)kext_mach_hdr->cputype,
      ...
      if (!infoKeys ||
          _OSArrayContainsCString(infoKeys, kOSBundleCPUSubtypeKey)) {
        cpuSubtypeNumber =
            OSNumber::withNumber((uint64_t)kext_mach_hdr->cpusubtype,
  ...
  bundleType = infoDict
                   ? OSDynamicCast(OSString,
                                   infoDict->getObject(kCFBundlePackageTypeKey))
                   : NULL;
  if (bundleType) {
   ...
   */
  if (!infoKeys ||
      _OSArrayContainsCString(infoKeys, kOSBundleCompatibleVersionKey)) {
    versionString = OSDynamicCast(
        OSString, getPropertyForHostArch(kOSBundleCompatibleVersionKey));
    if (versionString) {
   ...
   */
  if (!infoKeys ||
      _OSArrayContainsCString(infoKeys, kOSBundleExecutablePathKey)) {
    if (path && executableRelPath) {
      uint32_t pathLength = path->getLength(); // gets incremented below

      // +1 for slash, +1 for \0
      executablePathCStringSize =
          pathLength + executableRelPath->getLength() + 2;

      executablePathCString =
          (char *)kalloc_data_tag(executablePathCStringSize, Z_WAITOK,
                                  VM_KERN_MEMORY_OSKEXT); // +1 for \0
      ...
      if (path) {
        ...
        executablePathCString = (char *)kalloc_data_tag(
   ...
   */
  if (!infoKeys ||
      _OSArrayContainsCString(infoKeys, kOSKextInfoPlistDigestKey)) {
    OSData *digest;
    digest = infoDict
                 ? OSDynamicCast(OSData,
                                 infoDict->getObject(kOSKextInfoPlistDigestKey))
                 : NULL;
    if (digest) {
   ...
   */
  if (!infoKeys ||
      _OSArrayContainsCString(infoKeys, kOSKextBundleCollectionTypeKey)) {
    result->setObject(kOSKextBundleCollectionTypeKey,
   ...
   */
  if (!infoKeys ||
      _OSArrayContainsCString(infoKeys, kOSKextAuxKCAvailabilityKey)) {
   ...
   */
  if (!infoKeys ||
      _OSArrayContainsCString(infoKeys, kOSBundleAllowUserLoadKey)) {
    OSBoolean *allowUserLoad = OSDynamicCast(
        OSBoolean, getPropertyForHostArch(kOSBundleAllowUserLoadKey));
    if (allowUserLoad) {
  ...
  if (!infoKeys || _OSArrayContainsCString(infoKeys, kOSBundleLibrariesKey)) {
    OSDictionary *libraries = OSDynamicCast(
        OSDictionary, getPropertyForHostArch(kOSBundleLibrariesKey));
    if (libraries) {
  ...
  if (!infoKeys || _OSArrayContainsCString(infoKeys, kOSBundleLoadTagKey)) {
    OSSharedPtr<OSNumber> scratchNumber =
  ...
  if (!infoKeys || _OSArrayContainsCString(infoKeys, kOSBundleLoadAddressKey) ||
    ...
    if (isInterface() || flags.builtin || linkedExecutable || is_dext) {
      ...
      if (flags.builtin || linkedExecutable) {
        ...
        for (seg = firstsegfromheader(mh); seg != NULL;
          ...
          if (seg->initprot & VM_PROT_EXECUTE) {
            execLoadAddress = (flags.unslidMachO)
                                  ? seg->vmaddr
                                  : ml_static_unslide(seg->vmaddr);
        ...
        /*
         * DriverKit userspace executables do not have a kernel
         * linkedExecutable, so we "fake" their address range with the LoadTag.
         */
        if (loadTag) {
      ...
      if (!infoKeys ||
        ...
        OSSharedPtr<OSNumber> scratchNumber =
            OSNumber::withNumber((unsigned long long)(loadAddress),
      ...
      if (kcformat == KCFormatStatic || kcformat == KCFormatKCGEN) {
        if ((!infoKeys ||
             _OSArrayContainsCString(infoKeys, kOSBundleCacheLoadAddressKey)) &&
            loadAddress && loadSize) {
        ...
        if ((!infoKeys || _OSArrayContainsCString(
                              infoKeys, kOSBundleKextsInKernelTextKey)) &&
      ...
      if (!infoKeys ||
          _OSArrayContainsCString(infoKeys, kOSBundleExecLoadAddressKey)) {
        OSSharedPtr<OSNumber> scratchNumber =
            OSNumber::withNumber((unsigned long long)(execLoadAddress),
      ...
      if (!infoKeys ||
          _OSArrayContainsCString(infoKeys, kOSBundleLoadSizeKey)) {
        OSSharedPtr<OSNumber> scratchNumber =
            OSNumber::withNumber((unsigned long long)(loadSize),
      ...
      if (!infoKeys ||
          _OSArrayContainsCString(infoKeys, kOSBundleExecLoadSizeKey)) {
        OSSharedPtr<OSNumber> scratchNumber =
            OSNumber::withNumber((unsigned long long)(execLoadSize),
      ...
      if (!infoKeys ||
          _OSArrayContainsCString(infoKeys, kOSBundleWiredSizeKey)) {
        OSSharedPtr<OSNumber> scratchNumber =
            OSNumber::withNumber((unsigned long long)(wiredSize),
   ...
   */
  if (!infoKeys ||
      _OSArrayContainsCString(infoKeys, kOSBundleDependenciesKey)) {
    if ((count = getNumDependencies())) {
      ...
      do {
        ...
        OSSharedPtr<OSNumber> scratchNumber =
            OSNumber::withNumber((unsigned long long)dependency->getLoadTag(),
                                 /* numBits*/ 8 * sizeof(loadTag));
        if (!scratchNumber) {
          goto finish;
        }
  ...
  if (!infoKeys || _OSArrayContainsCString(infoKeys, kOSBundleClassesKey)) {
    if (metaClasses && metaClasses->getCount()) {
      metaClassIterator =
          OSCollectionIterator::withCollection(metaClasses.get());
      ...
      while ((thisMetaClass = OSDynamicCast(
                  OSMetaClass, metaClassIterator->getNextObject()))) {
        ...
        OSSharedPtr<OSNumber> scratchNumber = OSNumber::withNumber(
            thisMetaClass->getInstanceCount(), 8 * sizeof(unsigned int));

        /* Bail if any of the essentials is missing. The root class lacks a
        ...
        metaClassDict->setObject(kOSMetaClassTrackingCountKey,
                                 scratchNumber.get());
      }
    }
  }
  ...
  if (!infoKeys || _OSArrayContainsCString(infoKeys, kOSBundleRetainCountKey)) {
    {
      ...
      OSSharedPtr<OSNumber> scratchNumber =
          OSNumber::withNumber((int)kextRetainCount,
...
*********************************************************************/
/* static */
bool OSKext::copyUserExecutablePath(const OSSymbol *bundleID, char *pathResult,
                                    size_t pathSize) {
  bool ok;
  OSSharedPtr<OSKext> kext;
...
*********************************************************************/
/* static */
OSReturn OSKext::requestResource(const char *kextIdentifierCString,
  ...
  if (!sKernelRequestsEnabled) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogIPCFlag,
              "Can't request resource %s for %s - requests to user space are "
              "disabled.",
  ...
  result =
      _OSKextCreateRequest(kKextRequestPredicateRequestResource, requestDict);
  if (result != kOSReturnSuccess) {
    goto finish;
  }
  ...
  requestTagNum = OSNumber::withNumber((long long unsigned int)requestTag,
                                       8 * sizeof(requestTag));
  if (!kextIdentifier || !resourceName || !requestTagNum ||
      !_OSKextSetRequestArgument(requestDict.get(),
                                 kKextRequestArgumentBundleIdentifierKey,
                                 kextIdentifier.get()) ||
      !_OSKextSetRequestArgument(requestDict.get(), kKextRequestArgumentNameKey,
                                 resourceName.get()) ||
      !_OSKextSetRequestArgument(requestDict.get(),
  ...
  callbackRecord =
      OSDynamicPtrCast<OSDictionary>(requestDict->copyCollection());
  if (!callbackRecord) {
    result = kOSKextReturnNoMemory;
    goto finish;
  }
  ...
  if (!callbackWrapper ||
      !_OSKextSetRequestArgument(callbackRecord.get(),
...
finish:
  ...
  if (result != kOSReturnSuccess) {
    ...
    index =
        sRequestCallbackRecords->getNextIndexOfObject(callbackRecord.get(), 0);
    if (index != (unsigned int)-1) {
...
OSReturn OSKext::requestDaemonLaunch(OSString *kextIdentifier,
                                     OSString *serverName, OSNumber *serverTag,
                                     OSBoolean *reslide,
  ...
  OSKextLog(
      /* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogGeneralFlag,
      "Requesting daemon launch for %s %s with serverName %s and tag %llu%s",
      kextIdentifier->getCStringNoCopy(),
      (dextUniqueIDCString != NULL) ? dextUniqueIDCString : "",
      serverName->getCStringNoCopy(), serverTag->unsigned64BitValue(),
      reslide == kOSBooleanTrue ? " with reslid shared cache" : "");

  result = _OSKextCreateRequest(kKextRequestPredicateRequestDaemonLaunch,
                                requestDict);
  if (result != kOSReturnSuccess) {
    goto finish;
  }
  ...
  if (!_OSKextSetRequestArgument(requestDict.get(),
                                 ...
                                 kextIdentifier) ||
      !_OSKextSetRequestArgument(requestDict.get(),
                                 kKextRequestArgumentDriverExtensionServerName,
                                 serverName) ||
      !_OSKextSetRequestArgument(requestDict.get(),
      ...
      !_OSKextSetRequestArgument(
          requestDict.get(), kKextRequestArgumentCheckInToken, checkInToken)) {
    result = kOSKextReturnNoMemory;
    goto finish;
  }

  if (serverDUI) {
    if (!_OSKextSetRequestArgument(
            requestDict.get(), kOSBundleDextUniqueIdentifierKey, serverDUI)) {
      result = kOSKextReturnNoMemory;
      goto finish;
    }
  }
...
OSReturn OSKext::notifyDextUpgrade(OSString *kextIdentifier,
                                   OSData *dextUniqueIdentifier) {
  OSReturn result = kOSReturnError;
  OSSharedPtr<OSDictionary> requestDict;
  unsigned int size = 0;
  const char *dextUniqueIDCString =
      getDextUniqueIDCString(dextUniqueIdentifier, &size);
  assert(dextUniqueIDCString != NULL);
  ...
  result = _OSKextCreateRequest(
      kKextRequestPredicateRequestDaemonUpgradeNotification, requestDict);
  if (result != kOSReturnSuccess) {
    goto finish;
  }
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::dequeueCallbackForRequestTag(
    OSKextRequestTag requestTag, OSSharedPtr<OSDictionary> &callbackRecordOut) {
  OSDictionary *callbackRecordOutRaw = NULL;
  OSReturn result;

  result = dequeueCallbackForRequestTag(requestTag, &callbackRecordOutRaw);
...
OSReturn
OSKext::dequeueCallbackForRequestTag(OSKextRequestTag requestTag,
...
/* static */
OSReturn OSKext::dequeueCallbackForRequestTag(
    OSNumber *requestTagNum, OSSharedPtr<OSDictionary> &callbackRecordOut) {
  OSDictionary *callbackRecordOutRaw = NULL;
  OSReturn result;

  result = dequeueCallbackForRequestTag(requestTagNum, &callbackRecordOutRaw);
...
OSReturn
OSKext::dequeueCallbackForRequestTag(OSNumber *requestTagNum,
                                     OSDictionary **callbackRecordOut) {
  ...
  for (i = 0; i < count; i++) {
     ...
     */
    callbackTagNum = OSDynamicCast(
        OSNumber, _OSKextGetRequestArgument(callbackRecord,
                                            kKextRequestArgumentRequestTagKey));
    if (!callbackTagNum) {
      goto finish;
    }
 ...
 *********************************************************************/
/* static */
bool OSKext::pendingIOKitDaemonRequests(void) {
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::loadFileSetKexts(OSDictionary *requestDict __unused) {
  ...
  pageable_filepath = OSDynamicCast(
      OSString, requestArgs->getObject(kKextRequestArgumentPageableKCFilename));
  ...
  if (allow_fileset_load && pageable_filepath != NULL) {
    ...
    ret = OSKext::loadKCFileSet(pageable_filepath->getCStringNoCopy(),
                                KCKindPageable);
    if (ret) {
      OSKextLog(/* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogIPCFlag,
...
try_auxkc:
  ...
  if (akc_mh) {
     ...
     */
    size_t receipt_sz = 0;
    if (getsectdatafromheader(akc_mh, kReceiptInfoSegment, kAuxKCReceiptSection,
                              &receipt_sz) == NULL ||
        receipt_sz == 0) {
      OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
                "KextLog: WARNING: Failed to load AuxKC from memory: missing "
                "receipt");
      ret = kOSKextReturnKCLoadFailure;
      goto try_codeless;
    }
    ...
    if (infoDict) {
      ...
      added = OSKext::addKextsFromKextCollection(
          akc_mh, infoDict, kPrelinkTextSegment, loaded_kcUUID,
          KCKindAuxiliary);
      if (!loaded_kcUUID) {
        OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
      ...
      if (daemon_ready) {
         ...
         * hit this case because we've already consumed the deferred
         * personalities. We return success here so that a call to this function
         * from a restarted daemon with no codeless kexts will succeed.
         */
        OSKextLog(/* kext */ NULL, kOSKextLogBasicLevel | kOSKextLogIPCFlag,
                  "KextLog: can't re-parse deferred AuxKC personalities on "
                  "IOKit daemon restart");
        if (ret != kOSKextReturnKCLoadFailure) {
          ret = kOSReturnSuccess;
        }
      } else {
        ...
        OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogIPCFlag,
                  "KextLog: ERROR loading deferred AuxKC: PRELINK_INFO wasn't "
                  "an OSDictionary");
        printf("KextLog: ERROR loading deferred AuxKC: PRELINK_INFO wasn't an "
               "OSDictionary\n");
        ret = kOSKextReturnKCLoadFailure;
  ...
  aux_filepath = OSDynamicCast(
      OSString, requestArgs->getObject(kKextRequestArgumentAuxKCFilename));
  if (allow_fileset_load && aux_filepath != NULL) {
    printf("KextLog: Loading Aux KC from file %s\n",
           aux_filepath->getCStringNoCopy());

    ret = OSKext::loadKCFileSet(aux_filepath->getCStringNoCopy(),
                                KCKindAuxiliary);
    if (ret) {
      OSKextLog(/* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogIPCFlag,
...
try_codeless:
  ...
  if (codeless_kexts != NULL) {
    ...
    for (uint32_t i = 0; i < count; i++) {
      OSDictionary *infoDict;
      infoDict = OSDynamicCast(OSDictionary, codeless_kexts->getObject(i));
      if (!infoDict) {
        continue;
      }
      // instantiate a new kext, and don't hold a reference
  ...
  if (ret == kOSReturnSuccess || start_matching ||
      sOSKextWasResetAfterUserspaceReboot) {
    OSKext::sendAllKextPersonalitiesToCatalog(true);
    /*
     * This request necessarily came from the IOKit daemon (kernelmanagerd), so
     * mark things as active and start all the delayed matching: the dext and
     * codeless kext personalities should have all been delivered via this one
     * call.
     */
    if (!daemon_ready) {
...
OSReturn OSKext::resetMutableSegments(void) {
  kernel_segment_command_t *seg = NULL;
  kernel_mach_header_t *k_mh = (kernel_mach_header_t *)kmod_info->address;
  u_int index = 0;
  ...
  for (seg = firstsegfromheader(k_mh), index = 0; seg;
       seg = nextsegfromheader(k_mh, seg)) {
    if (!segmentIsMutable(seg)) {
      continue;
    }
    ...
    for (index = 0; index < savedMutableSegments->getCount(); index++) {
      savedSegment = OSDynamicCast(OSKextSavedMutableSegment,
                                   savedMutableSegments->getObject(index));
      assert(savedSegment);
      if (savedSegment->getVMAddr() == seg->vmaddr &&
          savedSegment->getVMSize() == seg->vmsize) {
        OSKextLog(this, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
                  "Resetting kext %s, mutable segment %.*s %llx->%llx.",
                  getIdentifierCString(),
                  (int)strnlen(seg->segname, sizeof(seg->segname)),
                  seg->segname, unslid_vmaddr, unslid_vmaddr + vmsize - 1);
        err = savedSegment->restoreContents(seg);
        if (err != kOSReturnSuccess) {
          panic("Kext %s cannot be reset, mutable segment %llx->%llx could not "
                "be restored.",
                getIdentifierCString(), unslid_vmaddr,
                unslid_vmaddr + vmsize - 1);
        }
    ...
    if (err != kOSReturnSuccess) {
      panic("Kext %s cannot be reset, could not find saved mutable segment for "
            "%llx->%llx.",
            getIdentifierCString(), unslid_vmaddr, unslid_vmaddr + vmsize - 1);
    }
  }
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::loadKCFileSet(const char *filepath, kc_kind_t type) {
#if VM_MAPPED_KEXTS
  ...
  if ((pageable && pageableKCloaded) || (!pageable && auxKCloaded)) {
    OSKextLog(/* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogIPCFlag,
              "KC FileSet of type %s is already loaded",
              (pageable ? "Pageable" : "Aux"));
  ...
  err = OSKext::mapKCFileSet(fileset_control, (vm_size_t)fsize, &mh, 0, &slide,
                             pageable, NULL);
  if (err) {
...
finish:
#endif
  /* Drop the vnode ref returned by ubc_getobject_from_filename if mapKCFileSet
   * failed */
  assert(vp != NULL);
  if (err == kOSReturnSuccess) {
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::mapKCFileSet(void *control, vm_size_t fsize,
                              kernel_mach_header_t **mhp, off_t file_offset,
                              uintptr_t *slidep, bool pageable,
                              void *map_entry_list) {
  ...
  if (err) {
    if (fileset_load) {
      deallocate_kcfileset_map_entry_list_and_unmap_entries(map_entry_list,
                                                            TRUE, pageable);
    }
    return err;
  }
  ...
  for (unsigned int i = 0; i < (*mhp)->ncmds;
       i++, lcp = (struct load_command *)((uintptr_t)lcp + lcp->cmdsize)) {
    ...
    if (lcp->cmd == LC_SEGMENT_KERNEL) {
      ...
      err = OSKext::mapKCFileSet(control, 0, &k_mh, fse->fileoff, slidep,
                                 pageable, map_entry_list);
      if (err) {
        deallocate_kcfileset_map_entry_list_and_unmap_entries(map_entry_list,
                                                              TRUE, pageable);
        return kOSKextReturnInvalidArgument;
    ...
    if (fileset_load) {
      ...
      /* Only map __PRELINK_INFO, __BRANCH_STUBS, __BRANCH_GOTS and __LINKEDIT
       * sections */
      if (strncmp(seg->segname, kPrelinkInfoSegment, sizeof(seg->segname)) !=
              0 &&
    ...
    ret = vm_map_kcfileset_segment(&start, seg->vmsize,
                                   (memory_object_control_t)control,
                                   seg->fileoff, seg->maxprot);
   ...
   */
  bool adjust_mach_header =
      (pageable ? true : ((resetAuxKCSegmentOnUnload) ? true : false));
  ret = i386_slide_kext_collection_mh_addrs(*mhp, *slidep, adjust_mach_header);
  if (ret != KERN_SUCCESS) {
    deallocate_kcfileset_map_entry_list_and_unmap_entries(map_entry_list, TRUE,
                                                          pageable);
    return kOSKextReturnInvalidArgument;
  }
  ...
  infoPlistSection =
      getsectbynamefromheader(*mhp, kPrelinkInfoSegment, kPrelinkInfoSection);
  parsedXML =
      OSUnserializeXML((const char *)infoPlistSection->addr, errorString);
  if (parsedXML) {
    infoDict = OSDynamicCast(OSDictionary, parsedXML.get());
  }
  ...
  err = OSKext::validateKCFileSetUUID(infoDict, pageable ? KCKindPageable
                                                         : KCKindAuxiliary);
  if (err) {
    deallocate_kcfileset_map_entry_list_and_unmap_entries(map_entry_list, TRUE,
                                                          pageable);
  ...
  OSKext::addKextsFromKextCollection(
      *mhp, infoDict, kPrelinkTextSegment, loaded_kcUUID,
      pageable ? KCKindPageable : KCKindAuxiliary);

  /* Copy in the KC UUID */
  if (!loaded_kcUUID) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
              "WARNING: did not find UUID in prelinked %s KC!",
              pageable ? "Pageable" : "Aux");
  } else if (pageable) {
    pageablekc_uuid_valid = TRUE;
    memcpy((void *)&pageablekc_uuid,
           (const void *)loaded_kcUUID->getBytesNoCopy(),
           loaded_kcUUID->getLength());
    uuid_unparse_upper(pageablekc_uuid, pageablekc_uuid_string);
  } else {
    auxkc_uuid_valid = TRUE;
    memcpy((void *)&auxkc_uuid, (const void *)loaded_kcUUID->getBytesNoCopy(),
           loaded_kcUUID->getLength());
    uuid_unparse_upper(auxkc_uuid, auxkc_uuid_string);
  }

  deallocate_kcfileset_map_entry_list_and_unmap_entries(map_entry_list, FALSE,
                                                        pageable);
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::mapKCTextSegment(void *control, kernel_mach_header_t **mhp,
                                  off_t file_offset, uintptr_t *slidep,
                                  void *map_entry_list) {
  kern_return_t ret;
  vm_map_offset_t mach_header_map_size =
      vm_map_round_page(sizeof(kernel_mach_header_t), PAGE_MASK);
  ...
  ret = vm_map_kcfileset_segment((vm_map_offset_t *)&base_mh,
                                 mach_header_map_size,
                                 (memory_object_control_t)control, file_offset,
                                 (VM_PROT_READ | VM_PROT_WRITE));

  if (ret != KERN_SUCCESS) {
    printf("Kext Log: mapKCTextSegment failed to map mach header of fileset %x",
           ret);
  ...
  if (slidep) {
    ...
    if (base_mh->filetype != MH_FILESET) {
      printf("Kext Log: mapKCTextSegment mach header filetype"
             " is not an MH_FILESET, it is %x",
             base_mh->filetype);

      OSKextLog(/* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogIPCFlag,
                "mapKCTextSegment mach header filetype is not an MH_FILESET, "
                "it is %x",
                base_mh->filetype);

      /* Unmap the mach header */
      vm_unmap_kcfileset_segment((vm_map_offset_t *)&base_mh,
                                 mach_header_map_size);
      return kOSKextReturnInvalidArgument;
    }
  }
  ...
  if (base_mh->sizeofcmds > mach_header_map_size) {
    vm_map_offset_t load_command_addr =
    ...
    ret = vm_map_kcfileset_segment(&load_command_addr, load_command_map_size,
                                   (memory_object_control_t)control,
                                   file_offset + mach_header_map_size,
                                   (VM_PROT_READ | VM_PROT_WRITE));

    if (ret != KERN_SUCCESS) {
      printf(
          "KextLog: mapKCTextSegment failed to map load commands of fileset %x",
          ret);
      OSKextLog(/* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogIPCFlag,
  ...
  ret = vm_unmap_kcfileset_segment((vm_map_offset_t *)&base_mh,
                                   mach_header_map_size);
  ...
  ret = vm_map_kcfileset_segment((vm_map_offset_t *)mhp, text_segment_size,
                                 (memory_object_control_t)control,
                                 text_segment_fileoff, text_maxprot);
  if (ret != KERN_SUCCESS) {
    OSKextLog(/* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogIPCFlag,
  ...
  add_kcfileset_map_entry(map_entry_list, (vm_map_offset_t)*mhp,
                          text_segment_size);
  return 0;
}
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::protectKCFileSet(kernel_mach_header_t *mh, kc_kind_t type) {
  ...
  while (seg) {
    ...
    if (strncmp(seg->segname, kKCBranchGots, sizeof(seg->segname)) == 0 ||
        ...
        (type == KCKindAuxiliary && !resetAuxKCSegmentOnUnload &&
         strncmp(seg->segname, SEG_LINKEDIT, sizeof(seg->segname)) == 0)) {
      ret = OSKext_protect((kernel_mach_header_t *)mh, kext_map, start, size,
                           seg->maxprot, TRUE, type);
      if (ret != KERN_SUCCESS) {
      ...
      ret = OSKext_protect((kernel_mach_header_t *)mh, kext_map, start, size,
                           seg->initprot, FALSE, type);
      if (ret != KERN_SUCCESS) {
        printf("OSKext protect failed with error %d", ret);
        return kOSKextReturnInvalidArgument;
      }

      ret = OSKext_wire((kernel_mach_header_t *)mh, kext_map, start, end,
                        seg->initprot, FALSE, type);
      if (ret != KERN_SUCCESS) {
 ...
 *********************************************************************/
/* static */
void OSKext::freeKCFileSetcontrol(void) { PE_reset_all_kc_vp(); }
 ...
 *********************************************************************/
OSReturn OSKext::resetKCFileSetSegments(void) {
  kernel_segment_command_t *seg = NULL;
  kernel_segment_command_t *text_seg;
  ...
  if (vp == NULL) {
    OSKextLog(this, kOSKextLogProgressLevel | kOSKextLogLoadFlag,
              "Kext %s could not be reset, since reboot released the vnode ref",
              getIdentifierCString());
  ...
  text_seg = getsegbynamefromheader((kernel_mach_header_t *)kmod_info->address,
                                    SEG_TEXT);
  ...
  err = OSKext::mapKCFileSet(fileset_control, 0, &k_mh, text_fileoff, &slide,
                             pageable, NULL);
  if (err) {
...
static void *allocate_kcfileset_map_entry_list(void) {
  ...
  entry_list =
      kalloc_type(struct kcfileset_map_entry_list, struct kcfileset_map_entry,
                  KCFILESET_MAP_ENTRY_MAX, Z_WAITOK_ZERO);
...
static void add_kcfileset_map_entry(void *map_entry_list, vm_map_offset_t start,
  ...
  struct kcfileset_map_entry_list *entry_list =
      (struct kcfileset_map_entry_list *)map_entry_list;
...
static void deallocate_kcfileset_map_entry_list_and_unmap_entries(
  ...
  if (unmap_entries) {
    for (int i = 0; i < entry_list->kme_list_index; i++) {
      kern_return_t ret;
      ret = vm_unmap_kcfileset_segment(&(entry_list->kme_list[i].me_start),
...
kern_return_t vm_map_kcfileset_segment(vm_map_offset_t *start,
  ...
  ret = vm_map_enter_mem_object_control(
      g_kext_map, start, size, (mach_vm_offset_t)0, vmk_flags,
      (memory_object_control_t)control, fileoffset, TRUE, /* copy */
      (VM_PROT_READ | VM_PROT_WRITE), max_prot, VM_INHERIT_NONE);
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::validateKCFileSetUUID(OSDictionary *infoDict, kc_kind_t type) {
  ...
  ret = OSKext::validateKCUUIDfromPrelinkInfo(&kernelcache_uuid, type, infoDict,
  ...
  if (type == KCKindAuxiliary) {
    if (!pageablekc_uuid_valid) {
      OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
                "validateKCFileSetUUID Pageable KC UUID was not set while "
                "loading Pageable KC.");
      ret = kOSKextReturnInvalidArgument;
      goto finish;
    }
    ret = OSKext::validateKCUUIDfromPrelinkInfo(
        &pageablekc_uuid, type, infoDict, kPrelinkInfoPageableKCIDKey);
    if (ret != 0) {
      goto finish;
    }
  }
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::validateKCUUIDfromPrelinkInfo(uuid_t *loaded_kcuuid,
                                               kc_kind_t type,
                                               OSDictionary *infoDict,
                                               const char *uuid_key) {
  /* extract the UUID from the dictionary */
  OSData *prelinkinfoKCUUID =
      OSDynamicCast(OSData, infoDict->getObject(uuid_key));
  ...
  if (prelinkinfoKCUUID->getLength() != sizeof(uuid_t)) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
              "validateKCUUID %s KC UUID has wrong length: %d.", uuid_key,
              prelinkinfoKCUUID->getLength());
    return kOSKextReturnInvalidArgument;
  }

  if (memcmp((void *)loaded_kcuuid,
             (const void *)prelinkinfoKCUUID->getBytesNoCopy(),
             prelinkinfoKCUUID->getLength())) {
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::dispatchResource(OSDictionary *requestDict) {
  OSReturn result = kOSReturnError;
  OSSharedPtr<OSDictionary> callbackRecord;
  ...
  OSValueObject<OSKextRequestResourceCallback> *callbackWrapper =
      nullptr; // do not release
   ...
   */
  requestTag = OSDynamicCast(
      OSNumber, _OSKextGetRequestArgument(requestDict,
                                          kKextRequestArgumentRequestTagKey));
  requestResult = OSDynamicCast(
      OSNumber,
      _OSKextGetRequestArgument(requestDict, kKextRequestArgumentResultKey));
   ...
   */
  contextWrapper =
      OSDynamicCast(OSValueObject<void *>,
  ...
  callbackWrapper =
      OSDynamicCast(OSValueObject<OSKextRequestResourceCallback>,
                    _OSKextGetRequestArgument(callbackRecord.get(),
                                              kKextRequestArgumentCallbackKey));
  ...
  (void)callback(requestTag->unsigned32BitValue(),
                 (OSReturn)requestResult->unsigned32BitValue(), dataPtr,
                 dataLength, context);
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::setMissingAuxKCBundles(OSDictionary *requestDict) {
  OSSharedPtr<OSDictionary> missingIDs;
  OSArray *bundleIDList = NULL; // do not release

  bundleIDList = OSDynamicCast(
      OSArray, _OSKextGetRequestArgument(requestDict,
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::setAuxKCBundleAvailable(OSString *kextIdentifier,
                                         OSDictionary *requestDict) {
  bool loadable = true;
  if (!kextIdentifier) {
    return kOSKextReturnInvalidArgument;
  ...
  sNonLoadableKextsByID->setObject(kextIdentifier,
...
*********************************************************************/
/* static */
void OSKext::invokeRequestCallback(OSDictionary *callbackRecord,
 ...
 *********************************************************************/
/* static */
OSReturn OSKext::cancelRequest(OSKextRequestTag requestTag, void **contextOut) {
  OSReturn result = kOSKextReturnNoMemory;
  OSSharedPtr<OSDictionary> callbackRecord;
  OSValueObject<void *> *contextWrapper = nullptr; // do not release

  IORecursiveLockLock(sKextLock);
  result = OSKext::dequeueCallbackForRequestTag(requestTag, callbackRecord);
  IORecursiveLockUnlock(sKextLock);

  if (result == kOSReturnSuccess && contextOut) {
    contextWrapper = OSDynamicCast(
        OSValueObject<void *>,
        _OSKextGetRequestArgument(callbackRecord.get(),
                                  kKextRequestArgumentContextKey));
 ...
 * Assumes sKextLock is held.
 *********************************************************************/
void OSKext::invokeOrCancelRequestCallbacks(OSReturn callbackResult,
                                            bool invokeFlag) {
  unsigned int count, i;
  ...
  i = count - 1;
  do {
    OSDictionary *request =
        OSDynamicCast(OSDictionary, sRequestCallbackRecords->getObject(i));

    if (!request) {
      continue;
    }
    auto *callbackWrapper = OSDynamicCast(
        OSValueObject<OSKextRequestResourceCallback>,
        _OSKextGetRequestArgument(request, kKextRequestArgumentCallbackKey));
    ...
    vm_address_t callbackAddress = (vm_address_t)ptrauth_strip(
        _OSKextExtractPointer(callbackWrapper), ptrauth_key_function_pointer);
 ...
 * Assumes sKextLock is held.
 *********************************************************************/
uint32_t OSKext::countRequestCallbacks(void) {
  uint32_t result = 0;
  unsigned int count, i;
...
static OSReturn _OSKextCreateRequest(const char *predicate,
  ...
  result = _OSDictionarySetCStringValue(request.get(), kKextRequestPredicateKey,
                                        predicate);
  if (result != kOSReturnSuccess) {
    goto finish;
  }
...
static OSObject *_OSKextGetRequestArgument(OSDictionary *requestDict,
                                           const char *argName) {
  OSDictionary *args = OSDynamicCast(
      OSDictionary, requestDict->getObject(kKextRequestArgumentsKey));
  if (args) {
...
*********************************************************************/
/* static */
OSSharedPtr<OSArray> OSKext::copyAllKextPersonalities(bool filterSafeBootFlag) {
  ...
  while ((kextID = OSDynamicCast(OSString, kextIterator->getNextObject()))) {
    ...
    if (theKext->flags.requireExplicitLoad) {
      OSKextLog(theKext, kOSKextLogDebugLevel | kOSKextLogLoadFlag,
                ...
                theKext->getIdentifierCString());
    } else if (!sSafeBoot || !filterSafeBootFlag ||
               theKext->isLoadableInSafeBoot()) {
      ...
      OSKextLog(theKext, kOSKextLogWarningLevel | kOSKextLogLoadFlag,
...
*********************************************************************/
/* static */
void OSKext::sendAllKextPersonalitiesToCatalog(bool startMatching) {
  int numPersonalities = 0;

  OSKextLog(/* kext */ NULL, kOSKextLogStepLevel | kOSKextLogLoadFlag,
            "Sending all eligible registered kexts' personalities "
            "to the IOCatalogue %s.",
            startMatching ? "and starting matching"
 ...
 *********************************************************************/
OSSharedPtr<OSArray> OSKext::copyPersonalitiesArray(void) {
  OSSharedPtr<OSArray> result;
  ...
  while ((personalityName = OSDynamicCast(
    ...
    OSDictionary *personality =
        OSDynamicCast(OSDictionary, personalities->getObject(personalityName));
    ...
    if (personality) {
      ...
      personalityBundleIdentifier = OSDynamicCast(
          OSString, personality->getObject(kCFBundleIdentifierKey));
 ...
 *********************************************************************/
OSReturn OSKext::sendPersonalitiesToCatalog(bool startMatching,
                                            OSArray *personalityNames) {
  OSReturn result = kOSReturnSuccess;
  ...
  if (!personalityNames || !personalityNames->getCount()) {
    ...
    kextPersonalities = OSDynamicCast(
        OSDictionary, getPropertyForHostArch(kIOKitPersonalitiesKey));
    ...
    for (i = 0; i < count; i++) {
      OSString *name = OSDynamicCast(OSString, personalityNames->getObject(i));
      if (!name) {
        continue;
      }
      OSDictionary *personality =
          OSDynamicCast(OSDictionary, kextPersonalities->getObject(name));
      if (personality) {
  ...
  if (personalitiesToSend) {
    ...
    OSKextLog(this, kOSKextLogStepLevel | kOSKextLogLoadFlag,
              "Kext %s sending %d personalit%s to the IOCatalogue%s.",
              getIdentifierCString(), numPersonalities,
 ...
 *********************************************************************/
void OSKext::removePersonalitiesFromCatalog(void) {
  ...
  OSKextLog(
      this, kOSKextLogStepLevel | kOSKextLogLoadFlag,
...
void OSKext::updatePersonalitiesInCatalog(OSArray *upgradedPersonalities) {
  ...
  gIOCatalogue->exchangeDrivers(personalityToRemove.get(),
                                upgradedPersonalities, true);
}
 ...
 *********************************************************************/
/* static */
OSKextLogSpec OSKext::setUserSpaceLogFilter(OSKextLogSpec newUserLogFilter,
                                            bool captureFlag) {
  ...
  if (newUserLogFilter && captureFlag && !sUserSpaceLogSpecArray &&
  ...
  if (result != newUserLogFilter) {
    OSKextLog(/* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogGeneralFlag,
              "User-space log flags changed from 0x%x to 0x%x.", result,
              newUserLogFilter);
  }
  if (allocError) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
 ...
 *********************************************************************/
/* static */
OSSharedPtr<OSArray> OSKext::clearUserSpaceLogFilter(void) {
  OSSharedPtr<OSArray> result;
  ...
  if (oldLogFilter != newLogFilter) {
    OSKextLog(/* kext */ NULL, kOSKextLogDebugLevel | kOSKextLogGeneralFlag,
              "User-space log flags changed from 0x%x to 0x%x.", oldLogFilter,
              newLogFilter);
  }
 ...
 *********************************************************************/
/* static */
OSKextLogSpec OSKext::getUserSpaceLogFilter(void) {
  OSKextLogSpec result;
...
inline bool logSpecMatch(OSKextLogSpec msgLogSpec, OSKextLogSpec logFilter) {
...
void OSKextVLog(OSKext *aKext, OSKextLogSpec msgLogSpec, const char *format,
                va_list srcArgList) {
  ...
  if (length + 1 >= sizeof(stackBuffer)) {
    allocBuffer =
        (char *)kalloc_data_tag(length + 1, Z_WAITOK, VM_KERN_MEMORY_OSKEXT);
    if (!allocBuffer) {
      goto finish;
    }
  ...
  if (logForKernel) {
    ...
    if (sBootArgLogFilterFound) {
      ...
      printf("%s%s%s\n", colorForFlags(msgLogSpec), buffer,
             color[0] ? VTRESET : "");
    } else {
      printf("%s\n", buffer);
    }
...
bool ScanForAddrInObject(OSObject *theObject, int indent);
  ...
  if (myTypeID == OSTypeID(OSDictionary)) {
    ...
    if (myPtrPtr && myLen && myLen > 7) {
      ...
      for (i = 0; i < myPtrCount; i++) {
        ...
        if (kext_alloc_max != 0 && numberValue >= kext_alloc_base &&
          ...
          if (myKext) {
            IOLog("found addr %p from an OSData obj within kext \"%s\"  \n",
                  *(myPtrPtr), myKext->getIdentifierCString());
          }
          myResult = true;
        }
        if (vm_kernel_etext != 0 && numberValue >= vm_kernel_stext &&
            numberValue < vm_kernel_etext) {
          IOLog("found addr %p from an OSData obj within kernel text segment "
                "%p to %p  \n",
                *(myPtrPtr), (void *)vm_kernel_stext, (void *)vm_kernel_etext);
          myResult = true;
        }
        myPtrPtr++;
      }
    ...
    if (vm_kernel_etext != 0 && numberValue >= vm_kernel_stext &&
      ...
      IOLog("found OSNumber in kernel text segment %p to %p  \n",
            (void *)vm_kernel_stext, (void *)vm_kernel_etext);
      IOLog("OSNumber 0x%08llx (%llu) \n", numberValue, numberValue);
      myResult = true;
    }
  }
 ...
 *********************************************************************/
/* static */
void OSKext::printKextsInBacktrace(vm_offset_t *addr __unused,
  ...
  for (; summary_page < last_summary_page; summary_page += PAGE_SIZE) {
    if (pmap_find_phys(kernel_pmap, summary_page) == 0) {
      (*printf_func)("         can't perform kext scan: "
                     "missing kext summary page %p",
                     summary_page);
      goto finish;
    }
  }

  foreachKextInBacktrace(
      addr, cnt, 0, ^(OSKextLoadedKextSummary *summary, uint32_t index) {
...
void OSKext::foreachKextInBacktrace(
    vm_offset_t *addr, uint32_t cnt, uint32_t flags,
 ...
 *********************************************************************/
/* static */
boolean_t OSKext::summaryIsInBacktrace(OSKextLoadedKextSummary *summary,
                                       vm_offset_t *addr, unsigned int cnt) {
  u_int i = 0;
...
/*
 * Get the kext summary object for the kext where 'addr' lies. Must be called
 * with sKextSummariesLock held.
 */
OSKextLoadedKextSummary *OSKext::summaryForAddress(uintptr_t addr) {
#if __has_feature(ptrauth_calls)
  ...
  for (unsigned i = 0; i < gLoadedKextSummaries->numSummaries; ++i) {
    ...
    if ((addr >= summary->address) &&
        (addr < (summary->address + summary->size))) {
      return summary;
    }
#else
...
/* static */
void *OSKext::kextForAddress(const void *address) {
  ...
  if (sKextAccountsCount) {
    ...
    for (baseIdx = 0, lim = sKextAccountsCount; lim; lim >>= 1) {
      ...
      if ((addr >= active->address) && (addr < active->address_end)) {
        if (active->account && (kext = active->account->kext) &&
            kext->kmod_info) {
          lck_ticket_unlock(sKextAccountsLock);
...
/* static */
kern_return_t OSKext::summaryForAddressExt(const void *address,
                                           OSKextLoadedKextSummary *summary) {
 ...
 */
static OSKextLoadedKextSummary *findSummary(uint32_t tagID) {
  OSKextLoadedKextSummary *summary;
 ...
 * This function must be safe to call in panic context.
 *********************************************************************/
void OSKext::printSummary(OSKextLoadedKextSummary *summary,
                          int (*printf_func)(const char *fmt, ...),
                          uint32_t flags) {
  ...
  (*printf_func)("%s%s(%s)[%s]@0x%llx->0x%llx\n",
                 (kPrintKextsTerse & flags) ? "" : "         ", summary->name,
                 version, uuid, tmpAddr, tmpAddr + tmpSize - 1);
  ...
  for (kmod_ref = (kmod_reference_t *)summary->reference_list; kmod_ref;
    ...
    if (pmap_find_phys(kernel_pmap, (addr64_t)((uintptr_t)kmod_ref)) == 0) {
      (*printf_func)("            kmod dependency scan stopped "
                     ...
                     (kPrintKextsUnslide & flags)
                         ? (void *)ml_static_unslide((vm_offset_t)kmod_ref)
                         : kmod_ref);
      break;
    }
    rinfo = kmod_ref->info;
    ...
    if (pmap_find_phys(kernel_pmap, (addr64_t)((uintptr_t)rinfo)) == 0) {
      (*printf_func)("            kmod dependency scan stopped "
                     ...
                     (kPrintKextsUnslide & flags)
                         ? (void *)ml_static_unslide((vm_offset_t)rinfo)
                         : rinfo);
      break;
    }
    ...
    (*printf_func)("            dependency: %s(%s)[%s]@%p->%p\n", rinfo->name,
                   rinfo->version, uuid, tmpAddr, tmpAddr + tmpSize - 1);
  }
  return;
}
 ...
 *******************************************************************************/
static int substitute(const char *scan_string, char *string_out,
                      uint32_t *to_index, uint32_t *from_index,
                      const char *substring, char marker, char substitution);
 ...
 *******************************************************************************/
static void compactIdentifier(const char *identifier, char *identifier_out,
...
static void compactIdentifier(const char *identifier, char *identifier_out,
  ...
  for (subs_entry = &kext_identifier_prefix_subs[0];
       subs_entry->substring && !did_sub; subs_entry++) {
    did_sub = substitute(identifier, identifier_out, &scan_to_index,
                         &scan_from_index, subs_entry->substring,
                         /* marker */ '\0', subs_entry->substitute);
  }
  did_sub = 0;
  ...
  for (/* see above */;
    ...
    if (scan_from_index) {
      for (subs_entry = &kext_identifier_substring_subs[0];
           subs_entry->substring && !did_sub; subs_entry++) {
        did_sub = substitute(scan_string, identifier_out, &scan_to_index,
                             &scan_from_index, subs_entry->substring, '!',
                             subs_entry->substitute);
      }
    }
 ...
 */
static size_t assemble_identifier_and_version(kmod_info_t *kmod_info,
                                              char *identPlusVers,
                                              size_t bufSize);
 ...
 *******************************************************************************/
/* static */
int OSKext::saveLoadedKextPanicListTyped(const char *prefix, int invertFlag,
                                         int libsFlag, char *paniclist,
                                         uint32_t list_size) {
  int result = -1;
  unsigned int count, i;
  ...
  do {
    ...
    identPlusVersLength = assemble_identifier_and_version(
        kmod_info, identPlusVers, sizeof(identPlusVers));
...
*********************************************************************/
/* static */
void OSKext::saveLoadedKextPanicList(void) {
  char *newlist = NULL;
  uint32_t newlist_size = 0;

  newlist_size = KEXT_PANICLIST_SIZE;
  newlist =
      (char *)kalloc_data_tag(newlist_size, Z_WAITOK, VM_KERN_MEMORY_OSKEXT);
 ...
 * Assumes sKextLock is held.
 *********************************************************************/
void OSKext::savePanicString(bool isLoading) {
  u_long len;
  ...
  len = assemble_identifier_and_version(
      kmod_info, (isLoading) ? last_loaded_str_buf : last_unloaded_str_buf,
      (isLoading) ? sizeof(last_loaded_str_buf)
...
*********************************************************************/
/* static */
void OSKext::printKextPanicLists(int (*printf_func)(const char *fmt, ...)) {
  if (last_loaded_strlen) {
    printf_func("last started kext at %llu: %.*s (addr %p, size %lu)\n",
                AbsoluteTime_to_scalar(&last_loaded_timestamp),
                last_loaded_strlen, last_loaded_str_buf, last_loaded_address,
                last_loaded_size);
  }
 ...
 *********************************************************************/
/* static */
void OSKext::updateLoadedKextSummaries(void) {
  kern_return_t result = KERN_FAILURE;
  ...
  uint32_t idx, accountingListAlloc, accountingListCount,
  ...
  for (i = 0, maxKexts = 0; i < count; ++i) {
    ...
    maxKexts +=
        (aKext && (aKext->isExecutable() || aKext->isSpecialKernelBinary()));
  }
  ...
  if (gLoadedKextSummaries == NULL || sLoadedKextSummariesAllocSize < size) {
    if (gLoadedKextSummaries) {
      kmem_free(kernel_map, (vm_offset_t)gLoadedKextSummaries,
    ...
    result =
        mach_vm_protect(kernel_map, start, summarySize, false, VM_PROT_DEFAULT);
    if (result != KERN_SUCCESS) {
      goto finish;
    }
  }
  ...
  for (i = 0, j = 0; i < count && j < maxKexts; ++i) {
    ...
    bcopy(&accountingList[idx], &accountingList[idx + 1],
          (accountingListCount - idx) * sizeof(accountingList[0]));
...
finish:
  ...
  if (prevAccountingList) {
    IODelete(prevAccountingList, typeof(accountingList[0]),
             prevAccountingListCount);
  }
...
/*********************************************************************
*********************************************************************/
void OSKext::updateLoadedKextSummary(OSKextLoadedKextSummary *summary) {
  OSSharedPtr<OSData> uuid;

  strlcpy(summary->name, getIdentifierCString(), sizeof(summary->name));
  ...
  summary->text_exec_address =
      (uint64_t)getsegdatafromheader((kernel_mach_header_t *)summary->address,
                                     "__TEXT_EXEC", &summary->text_exec_size);
  if (summary->text_exec_address == 0) {
    // Fallback to __TEXT
    summary->text_exec_address =
        (uint64_t)getsegdatafromheader((kernel_mach_header_t *)summary->address,
                                       "__TEXT", &summary->text_exec_size);
  }
  ...
  if (flags.unslidMachO) {
    summary->text_exec_address =
        (uint64_t)ml_static_slide((vm_offset_t)summary->text_exec_address);
  }
...
void OSKext::updateActiveAccount(OSKextActiveAccount *accountp) {
  ...
  if (getcommandfromheader(hdr, LC_SEGMENT_SPLIT_INFO) || isInFileset()) {
     ...
     */
    for (seg = firstsegfromheader(hdr); seg != NULL;
         seg = nextsegfromheader(hdr, seg)) {
      if (seg->initprot & VM_PROT_EXECUTE) {
        break;
      }
    }
...
bool OSKext::isDriverKit(void) {
  OSString *bundleType;

  if (infoDict) {
    bundleType =
        OSDynamicCast(OSString, infoDict->getObject(kCFBundlePackageTypeKey));
    if (bundleType &&
        bundleType->isEqualTo(kOSKextBundlePackageTypeDriverKit)) {
      return TRUE;
    }
  }
  return FALSE;
...
OSSharedPtr<OSDextStatistics> OSKext::copyDextStatistics(void) {
  return dextStatistics;
}

bool OSKextSavedMutableSegment::initWithSegment(kernel_segment_command_t *seg) {
...
OSKextSavedMutableSegment::withSegment(kernel_segment_command_t *seg) {
  OSSharedPtr<OSKextSavedMutableSegment> me =
      OSMakeShared<OSKextSavedMutableSegment>();
...
void OSKextSavedMutableSegment::free(void) {
  if (data) {
    kmem_free(kernel_map, (vm_offset_t)data, vmsize);
  }
}

vm_offset_t OSKextSavedMutableSegment::getVMAddr() const { return vmaddr; }

vm_offset_t OSKextSavedMutableSegment::getVMSize() const { return vmsize; }
...
extern "C" kern_return_t OSKextSetReceiptQueried(void) {
  OSKextLog(/* kext */ NULL, kOSKextLogStepLevel | kOSKextLogGeneralFlag,
...
#if DEVELOPMENT || DEBUG
extern "C" void OSKextGetRefGrpForCaller(uintptr_t address,
                                         void (^cb)(struct os_refgrp *)) {
  OSKextActiveAccount *active;
...
extern "C" uint32_t OSKextGetKmodIDForSite(const vm_allocation_site_t *site,
...
extern "C" void OSKextFreeSite(vm_allocation_site_t *site) {
...
OSSharedPtr<OSDextCrash> OSDextCrash::withTimestamp(uint64_t timestamp) {
...
bool OSDextCrash::initWithTimestamp(uint64_t timestamp) {
  if (!OSObject::init()) {
    return false;
  }
...
uint64_t OSDextCrash::getTimestamp() { return fTimestamp; }

OSSharedPtr<OSDextStatistics> OSDextStatistics::create() {
...
OSDextCrashPolicy OSDextStatistics::recordCrash() {
  size_t i = 0;
...
size_t OSDextStatistics::getCrashCount() {
  size_t result = 0;
  IOLockLock(lock);
...
static int sysctl_willuserspacereboot(__unused struct sysctl_oid *oidp,
