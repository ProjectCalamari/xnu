...
OSSerialize *OSSerialize::binaryWithCapacity(unsigned int inCapacity,
  ...
  bcopy(kOSSerializeBinarySignature, &me->data[0],
...
void OSSerialize::setIndexed(bool index __unused) {
...
void OSSerialize::endBinaryCollection(uint32_t startCollection) {
  uint32_t clength;
...
bool OSSerialize::binarySerializeInternal(const OSMetaClassBase *o) {
  OSDictionary *dict;
  OSArray *array;
  OSSet *set;
  OSNumber *num;
...
#define setAtIndex(v, idx, o)                                                  \
  ...
  if (!ok && v##Capacity < v##CapacityMax) {                                   \
    ...
    typeof(v##Array) nbuf = kreallocp_type_container(                          \
        OSObject *, v##Array, v##Capacity, &ncap, Z_WAITOK_ZERO);              \
    if (nbuf) {                                                                \
      ok = true;                                                               \
      v##Array = nbuf;                                                         \
      v##Capacity = ncap;                                                      \
...
OSObject *OSUnserializeXML(const char *buffer,
...
OSObject *OSUnserialize(const char *buffer,
                        OSSharedPtr<OSString> &errorString) {
  OSString *errorStringRaw = NULL;
