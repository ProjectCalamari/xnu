...
OSSharedPtr<OSOrderedSet> OSOrderedSet::withCapacity(unsigned int capacity,
...
static SInt32 OSOrderedSetBlockToFunc(const OSMetaClassBase *obj1,
...
OSSharedPtr<OSOrderedSet> OSOrderedSet::withCapacity(unsigned int capacity,
                                                     OSOrderBlock ordering) {
  auto me = OSMakeShared<OSOrderedSet>();

  if (me &&
      !me->initWithCapacity(capacity, &OSOrderedSetBlockToFunc, ordering)) {
    return nullptr;
  }
...
unsigned int OSOrderedSet::ensureCapacity(unsigned int newCapacity) {
  ...
  newArray = kreallocp_type_container(_Element, array, capacity, &finalCapacity,
                                      Z_WAITOK_ZERO);
  if (newArray) {
...
/* internal */
bool OSOrderedSet::setObject(unsigned int index,
...
bool OSOrderedSet::setFirstObject(
...
bool OSOrderedSet::setLastObject(
    OSSharedPtr<const OSMetaClassBase> const &anObject) {
  return setLastObject(anObject.get());
}

#define ORDER(obj1, obj2)                                                      \
  (ordering ? ((*ordering)((const OSObject *)obj1, (const OSObject *)obj2,     \
                           orderingRef))                                       \
            : 0)
...
bool OSOrderedSet::setObject(const OSMetaClassBase *anObject) {
  ...
  for (i = 0; (i < count) && (ORDER(array[i].obj.get(), anObject) >= 0); i++) {
  }
...
void OSOrderedSet::removeObject(
    OSSharedPtr<const OSMetaClassBase> const &anObject) {
...
bool OSOrderedSet::member(const OSMetaClassBase *anObject) const {
  ...
  for (i = 0; (i < count) && (array[i].obj != anObject); i++) {
  }
...
void *OSOrderedSet::getOrderingRef() { return orderingRef; }
...
unsigned int OSOrderedSet::iteratorSize() const { return sizeof(unsigned int); }
...
bool OSOrderedSet::getNextObjectForIterator(void *inIterator,
                                            OSObject **ret) const {
  unsigned int *iteratorP = (unsigned int *)inIterator;
