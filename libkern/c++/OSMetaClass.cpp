...
OSKextLogSpec kOSMetaClassLogSpec =
    kOSKextLogErrorLevel | kOSKextLogLoadFlag | kOSKextLogKextBookkeepingFlag;
...
/*
 *  IHI0059A "C++ Application Binary Interface Standard for the ARM 64 - bit
 * Architecture":
 *
 *  3.2.1 Representation of pointer to member function The generic C++ ABI
 * [GC++ABI] specifies that a pointer to member function is a pair of words
 * <ptr, adj>. The least significant bit of ptr discriminates between (0) the
 * address of a non- virtual member function and (1) the offset in the class's
 * virtual table of the address of a virtual function. This encoding cannot work
 * for the AArch64 instruction set where the architecture reserves all bits of
 * code addresses. This ABI specifies that adj contains twice the this
 * adjustment, plus 1 if the member function is virtual. The least significant
 * bit of adj then makes exactly the same discrimination as the least
 * significant bit of ptr does for Itanium. A pointer to member function is NULL
 * when ptr = 0 and the least significant bit of adj is zero.
 */
...
#endif
{
  ...
  // It can also be NULL.
  if (pfn) {
    pfn = ptrauth_auth_and_resign(
        pfn, ptrauth_key_function_pointer,
  ...
  if (map.pTMF.delta & 1) {
    ...
    const _ptf_t *vtablep =
        ptrauth_auth_data(*u.vtablep, ptrauth_key_cxx_vtable_pointer, 0);
#endif
    ...
    pfn = ptrauth_auth_and_resign(
        pfn, ptrauth_key_function_pointer, auth_data,
        ptrauth_key_function_pointer,
        ptrauth_function_pointer_type_discriminator(_ptf_t));
...
/*********************************************************************
*********************************************************************/
OSMetaClassBase *OSMetaClassBase::safeMetaCast(const OSMetaClassBase *me,
                                               const OSMetaClass *toType) {
  return (me) ? me->metaCast(toType) : NULL;
}

/// A helper function to crash with a kernel panic.
__attribute__((cold, not_tail_called, noreturn)) static inline void
panic_crash_fail_cast(const OSMetaClassBase *me, const OSMetaClass *toType) {
  panic("Unexpected cast fail: from %p to %p", me, toType);
  __builtin_unreachable();
}

OSMetaClassBase *OSMetaClassBase::requiredMetaCast(const OSMetaClassBase *me,
                                                   const OSMetaClass *toType) {
  if (!me) {
    return NULL;
  }
...
/*********************************************************************
*********************************************************************/
bool OSMetaClassBase::checkTypeInst(const OSMetaClassBase *inst,
...
/*********************************************************************
*********************************************************************/
void OSMetaClassBase::initialize() {
...
OSObject *OSMetaClassMeta::alloc() const { return NULL; }
...
/*********************************************************************
*********************************************************************/
static void OSMetaClassLogErrorForKext(OSReturn error, OSKext *aKext) {
  ...
  case kOSMetaClassNoDicts:
    message = "OSMetaClass: Allocation failure for OSMetaClass internal "
              "dictionaries.";
    break;
  case kOSMetaClassNoKModSet:
    message = "OSMetaClass: Allocation failure for internal kext recording "
              "set/set missing.";
    break;
...
void OSMetaClass::logError(OSReturn error) {
...
OSMetaClass::OSMetaClass(const char *inClassName,
  ...
  if ((this == &OSSymbol ::gMetaClass) || (this == &OSString ::gMetaClass) ||
      (this == &OSNumber ::gMetaClass) || (this == &OSString ::gMetaClass) ||
      (this == &OSData ::gMetaClass) || (this == &OSDictionary::gMetaClass) ||
      (this == &OSArray ::gMetaClass) || (this == &OSSet ::gMetaClass)) {
    numSiteQs = 27;
  }

  reserved->tracking =
      IOTrackingQueueAlloc(inClassName, (uintptr_t)this, inClassSize, 0,
                           kIOTrackingQueueTypeAlloc, numSiteQs);
#endif
  ...
  if (!sStalled) {
    ...
    if (sStalled->count >= sStalled->capacity) {
      ...
      memmove(sStalled->classes, oldStalled, sizeof(OSMetaClass *) * oldCount);
...
OSMetaClass::OSMetaClass(const char *inClassName,
                         const OSMetaClass *inSuperClass,
                         unsigned int inClassSize, zone_t *inZone,
                         const char *zone_name, zone_create_flags_t zflags)
    : OSMetaClass(inClassName, inSuperClass, inClassSize) {
...
void OSMetaClass::taggedRelease(__unused const void *tag,
                                __unused const int when) const {}
...
/*********************************************************************
*********************************************************************/
const OSSymbol *OSMetaClass::getClassNameSymbol() const { return className; }
/*********************************************************************
*********************************************************************/
unsigned int OSMetaClass::getClassSize() const { return classSize; }

/*********************************************************************
*********************************************************************/
void *OSMetaClass::preModLoad(const char *kextIdentifier) {
  IOLockLock(sStalledClassesLock);

  assert(sStalled == NULL);
  sStalled = kalloc_type(StalledData, Z_WAITOK_ZERO_NOFAIL);

  sStalled->classes = kalloc_type_tag(OSMetaClass *, kKModCapacityIncrement,
                                      Z_WAITOK_ZERO, VM_KERN_MEMORY_OSKEXT);
  if (!sStalled->classes) {
...
OSReturn OSMetaClass::postModLoad(void *loadHandle) {
  ...
  if (sStalled->result) {
    ...
    case kCompletedBootstrap: {
      unsigned int i;
      myKextName = const_cast<OSSymbol *>(
          OSSymbol::withCStringNoCopy(sStalled->kextIdentifier));
      ...
      for (i = 0; i < sStalled->count; i++) {
        ...
        OSMetaClass *orig = OSDynamicCast(
            OSMetaClass,
...
finish:
   ...
   */
  if (result != kOSReturnSuccess && result != kOSMetaClassNoInsKModSet &&
      result != kOSMetaClassDuplicateClass && result != kOSMetaClassNoKext) {
  ...
  if (sStalled) {
    OSMETA_ACCUMSIZE(
        -(sStalled->capacity * sizeof(OSMetaClass *) + sizeof(*sStalled)));
...
void OSMetaClass::instanceConstructed() const {
  // if ((0 == OSIncrementAtomic(&(((OSMetaClass *) this)->instanceCount))) &&
  // superClassLink)
...
void OSMetaClass::instanceDestructed() const {
  ...
  if (((int)instanceCount) < 0) {
    ...
    OSKextLog(myKext, kOSMetaClassLogSpec,
              ...
              "OSMetaClass: Class %s - bad retain (%d)", getClassName(),
              instanceCount);
  }
}
...
/*********************************************************************
*********************************************************************/
bool OSMetaClass::modHasInstance(const char *kextIdentifier) {
  bool result = false;
...
/*********************************************************************
*********************************************************************/
void OSMetaClass::reportModInstances(const char *kextIdentifier) {
  OSKext::reportOSMetaClassInstances(kextIdentifier, kOSKextLogExplicitLevel);
  return;
}
/*********************************************************************
*********************************************************************/

void OSMetaClass::addInstance(const OSObject *instance, bool super) const {
  if (!super) {
...
void OSMetaClass::removeInstance(const OSObject *instance, bool super) const {
  if (!super) {
    IOLockLock(sInstancesLock);
  }
...
void OSMetaClass::applyToInstances(OSOrderedSet *set,
...
void OSMetaClass::applyToInstances(OSMetaClassInstanceApplierFunction applier,
                                   void *context) const {
  IOLockLock(sInstancesLock);
  if (reserved->instances) {
...
void OSMetaClass::applyToInstancesOfClassName(
    const OSSymbol *name, OSMetaClassInstanceApplierFunction applier,
    void *context) {
  OSMetaClass *meta;
...
/*********************************************************************
*********************************************************************/
void OSMetaClass::considerUnloads() { OSKext::considerUnloads(); }

/*********************************************************************
*********************************************************************/
bool OSMetaClass::removeClasses(OSCollection *metaClasses) {
  ...
  do {
    while ((checkClass = (OSMetaClass *)classIterator->getNextObject()) &&
           !checkClass->getInstanceCount() && !checkClass->reserved->retain) {
    }
    if (checkClass) {
      break;
    }
...
/*********************************************************************
*********************************************************************/
const OSMetaClass *OSMetaClass::copyMetaClassWithName(const OSSymbol *name) {
...
/*********************************************************************
*********************************************************************/
void OSMetaClass::releaseMetaClass() const {
...
/*********************************************************************
*********************************************************************/
OSMetaClassBase *OSMetaClass::checkMetaCastWithName(const OSSymbol *name,
...
/*********************************************************************
*********************************************************************/
OSMetaClassBase *OSMetaClass::checkMetaCastWithName(const OSString *name,
                                                    const OSMetaClassBase *in) {
  const OSSymbol *tmpKey = OSSymbol::withString(name);
...
/*********************************************************************
*********************************************************************/
OSMetaClassBase *OSMetaClass::checkMetaCastWithName(const char *name,
                                                    const OSMetaClassBase *in) {
  const OSSymbol *tmpKey = OSSymbol::withCStringNoCopy(name);
  OSMetaClassBase *result = checkMetaCastWithName(tmpKey, in);
 ...
 *********************************************************************/
OSMetaClassBase *
OSMetaClass::checkMetaCast(const OSMetaClassBase *check) const {
...
/*********************************************************************
*********************************************************************/
__dead2 void OSMetaClass::reservedCalled(int ind) const {
...
/*********************************************************************
*********************************************************************/
const OSMetaClass *OSMetaClass::getSuperClass() const { return superClassLink; }
...
/*********************************************************************
*********************************************************************/
OSKext *OSMetaClass::getKext() const {
  return reserved ? reserved->kext : NULL;
}

/*********************************************************************
*********************************************************************/
unsigned int OSMetaClass::getInstanceCount() const { return instanceCount; }
...
void OSMetaClass::printInstanceCounts() {
  ...
  while ((className = (OSSymbol *)classes->getNextObject())) {
    ...
    printf("%24s count: %03d x 0x%03x = 0x%06x\n",
           className->getCStringNoCopy(), meta->getInstanceCount(),
           meta->getClassSize(),
...
void OSMetaClass::trackedDelete(void *instance, size_t size) {
  IOTracking *mem = (typeof(mem))instance;
  mem--;
...
IOTrackingQueue *OSMetaClass::getTracking() const { return reserved->tracking; }
