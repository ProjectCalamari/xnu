...
OSDefineMetaClassAndStructorsWithZone(OSSet, OSCollection, ZC_ZFREE_CLEARMEM)
...
bool OSSet::initWithObjects(const OSObject *inObjects[], unsigned int inCount,
...
bool OSSet::initWithArray(const OSArray *inArray, unsigned int inCapacity) {
  if (!inArray) {
    return false;
  }

  return initWithObjects((const OSObject **)inArray->array, inArray->count,
                         inCapacity);
}

bool OSSet::initWithSet(const OSSet *inSet, unsigned int inCapacity) {
  return initWithArray(inSet->members.get(), inCapacity);
}

OSSharedPtr<OSSet> OSSet::withCapacity(unsigned int capacity) {
...
OSSharedPtr<OSSet> OSSet::withObjects(const OSObject *objects[],
                                      unsigned int count,
...
OSSharedPtr<OSSet> OSSet::withArray(const OSArray *array,
                                    unsigned int capacity) {
  OSSharedPtr<OSSet> me = OSMakeShared<OSSet>();
...
OSSharedPtr<OSSet> OSSet::withSet(const OSSet *set, unsigned int capacity) {
  OSSharedPtr<OSSet> me = OSMakeShared<OSSet>();
...
unsigned int OSSet::getCount() const { return members->count; }

unsigned int OSSet::getCapacity() const { return members->capacity; }
...
bool OSSet::setObject(OSSharedPtr<const OSMetaClassBase> const &anObject) {
...
bool OSSet::merge(const OSSet *set) { return merge(set->members.get()); }
...
void OSSet::removeObject(OSSharedPtr<const OSMetaClassBase> const &anObject) {
...
OSObject *OSSet::getAnyObject() const { return members->getObject(0); }
...
unsigned int OSSet::iteratorSize() const { return sizeof(unsigned int); }
...
OSSharedPtr<OSCollection> OSSet::copyCollection(OSDictionary *cycleDict) {
