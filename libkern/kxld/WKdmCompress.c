...
unsigned int WKdm_compress(WK_word *src_buf, WK_word *dest_buf,
  ...
  printf(" i.e., there are %u full patterns\n",
         next_full_patt -
             (dest_buf + TAGS_AREA_OFFSET + (num_input_words / 16)));
  fflush(stdout);
...

  {
    ...
    unsigned int num_packed_words =
        (num_bytes_to_pack + 7) >> 3; // ceil((double) num_bytes_to_pack / 8);
    ...
    printf("dest is range %u to %u\n", next_full_patt,
           next_full_patt + num_packed_words);
    {
      int i;
...

  {
    ...
    unsigned int num_packed_words =
        (num_tenbits_to_pack + 2) /
        3; // ceil((double) num_tenbits_to_pack / 3);
