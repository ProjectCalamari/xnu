...
static kern_return_t array_init(KXLDArray *array, size_t itemsize,
                                u_int nitems);
...
static kern_return_t array_init(KXLDArray *array, size_t itemsize,
  ...
  if (pool) {
    require_action(itemsize * nitems < array->pool_capacity, finish,
                   rval = KERN_FAILURE);
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_array_copy(KXLDArray *dstarray, const KXLDArray *srcarray) {
  kern_return_t rval = KERN_FAILURE;
...
void kxld_array_reset(KXLDArray *array) {
  ...
  if (array) {
    STAILQ_FOREACH(pool, &array->pools, entries) { pool->nitems = 0; }
    array->nitems = 0;
  }
}
