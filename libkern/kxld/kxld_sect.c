...
static kern_return_t export_macho(const KXLDSect *sect, u_char *buf,
                                  u_long offset, u_long bufsize);
#if KXLD_USER_OR_ILP32
static kern_return_t sect_export_macho_header_32(const KXLDSect *sect,
                                                 u_char *buf,
...
static kern_return_t sect_export_macho_header_64(const KXLDSect *sect,
                                                 u_char *buf,
                                                 u_long *header_offset,
                                                 u_long header_size,
                                                 u_long data_offset);
...
kern_return_t kxld_sect_init_from_macho_32(KXLDSect *sect, u_char *macho,
                                           u_long *sect_offset, u_int sectnum,
  ...
  rval = kxld_reloc_create_macho(&sect->relocs, relocator, relocs, src->nreloc);
...
kern_return_t kxld_sect_init_from_macho_64(KXLDSect *sect, u_char *macho,
                                           ...
                                           const KXLDRelocator *relocator) {
  kern_return_t rval = KERN_FAILURE;
  struct section_64 *src =
      (struct section_64 *)((void *)(macho + *sect_offset));
  struct relocation_info *relocs = NULL;
...
void kxld_sect_init_zerofill(KXLDSect *sect, const char *segname,
                             const char *sectname, kxld_size_t size,
                             u_int align) {
  check(sect);
  check(segname);
  check(sectname);
...
/*******************************************************************************
*******************************************************************************/
u_int kxld_sect_get_ngots(const KXLDSect *sect, const KXLDRelocator *relocator,
 ...
 * alignment, we mask for the low bits that may need to be adjusted.  If they
 * are non zero, we then subtract them from the target alignment to find the
 * offset, and then add that offset to the link address.
 *******************************************************************************/
...
kxld_sect_export_macho_to_file_buffer(const KXLDSect *sect, u_char *buf,
   ...
   */
  if (!sect->data) {
    KXLD_3264_FUNC(is_32_bit, rval, sect_export_macho_header_32,
                   sect_export_macho_header_64, sect, buf, header_offset,
                   header_size, /* data_offset */ 0);
    require_noerr(rval, finish);
  } else {
...
static kern_return_t export_macho(const KXLDSect *sect, u_char *buf,
  ...
  if (!isSplitKext) {
    ...
    if (kxld_sect_align_address(sect, offset) != offset) {
      kxld_log(kKxldLogLinking, kKxldLogErr,
               ...
               kxld_sect_align_address(sect, offset), offset, sect->segname,
               sect->sectname, __func__);
      goto finish;
    }
  }
  ...
  if (buf + offset + sect->size > buf + bufsize) {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             ...
             offset, sect->size, bufsize, sect->segname, sect->sectname);
    goto finish;
  }
  ...
  case S_SYMBOL_STUBS:
    ...
    kxld_log(kKxldLogLinking, kKxldLogErr,
             " sectname %s copy from %p (sect->data) for %llu bytes "
             "(sect->size) to %p (buf %p + offset %lu <%s>",
             sect->sectname[0] ? sect->sectname : "none", (void *)sect->data,
             sect->size, (void *)(buf + offset), (void *)buf, offset, __func__);

    kxld_log(kKxldLogLinking, kKxldLogErr,
             " %p >>> Start of %s section data (sect->size %llu) <%s>",
             (void *)(buf + offset),
             sect->sectname[0] ? sect->sectname : "none", sect->size, __func__);
    kxld_log(kKxldLogLinking, kKxldLogErr,
             " %p <<< End of %s section data <%s>",
             (void *)(buf + offset + sect->size),
             sect->sectname[0] ? sect->sectname : "none", __func__);
#endif
  ...
  default:
    rval = KERN_FAILURE;
    kxld_log(kKxldLogLinking, kKxldLogErr,
             kKxldLogMalformedMachO "Invalid section type: %u.",
  ...
  *header_offset += sizeof(*secthdr);
...
#if SPLIT_KEXTS_DEBUG
  {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             ...
             sect->sectname[0] ? sect->sectname : "none", (void *)secthdr,
             (void *)((uint64_t)secthdr->addr), secthdr->size, secthdr->size,
             secthdr->offset, secthdr->offset, __func__);
  }
#endif
...
#if SPLIT_KEXTS_DEBUG
  kxld_log(kKxldLogLinking, kKxldLogErr,
           " %p >>> Start of %s secthdr (size %lu) <%s>", (void *)secthdr,
           sect->sectname[0] ? sect->sectname : "none", sizeof(*secthdr),
           __func__);
  kxld_log(kKxldLogLinking, kKxldLogErr, " %p <<< End of %s secthdr <%s>",
           (void *)((u_char *)secthdr + sizeof(*secthdr)),
           sect->sectname[0] ? sect->sectname : "none", __func__);
  kxld_log(kKxldLogLinking, kKxldLogErr,
           " secthdr: addr %p size %llu offset %u sectname %s <%s>",
           (void *)secthdr->addr, secthdr->size, secthdr->offset,
           sect->sectname[0] ? sect->sectname : "none", __func__);
#endif
...
void kxld_sect_relocate(KXLDSect *sect, kxld_addr_t link_addr) {
#if SPLIT_KEXTS_DEBUG
  {
    kxld_log(
        ...
        sect->sectname[0] ? sect->sectname : "none", sect->size, __func__);
    kxld_log(
        kKxldLogLinking, kKxldLogErr, "%p <<< End of %s section <%s>",
        (void *)(kxld_sect_align_address(sect, sect->link_addr + link_addr) +
                 sect->size),
        sect->sectname[0] ? sect->sectname : "none", __func__);
  }
  ...
  sect->link_addr = kxld_sect_align_address(sect, sect->link_addr + link_addr);
}

#if KXLD_USER_OR_GOT
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_sect_populate_got(KXLDSect *sect, KXLDSymtab *symtab,
