...
static kern_return_t export_symbols_through_interface(
    ...
    KXLDDict *defined_symbols_by_name, KXLDDict *defined_cxx_symbol_by_value,
...
static kern_return_t get_vtable_syms_from_smcp(KXLDKext *kext,
                                               const KXLDDict *defined_symbols,
                                               KXLDSym *super_metaclass_ptr_sym,
                                               KXLDSym **vtable_sym_out,
...
/*******************************************************************************
*******************************************************************************/
size_t kxld_kext_sizeof(void) { return sizeof(KXLDKext); }

/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_kext_init(KXLDKext *kext, KXLDObject *kext_object,
...
kern_return_t kxld_kext_export_symbols(const KXLDKext *kext,
  ...
  if (kext->interface) {
    rval = export_symbols_through_interface(
        kext->kext, kext->interface, defined_symbols_by_name,
        obsolete_symbols_by_name, defined_cxx_symbols_by_value);
    require_noerr(rval, finish);
  } else {
...
/*******************************************************************************
*******************************************************************************/
kern_return_t export_symbols_through_interface(
    const KXLDObject *kext, const KXLDObject *interface,
    KXLDDict *defined_symbols_by_name, KXLDDict *obsolete_symbols_by_name,
    KXLDDict *defined_cxx_symbols_by_value) {
  kern_return_t rval = KERN_FAILURE;
  ...
  if (defined_symbols_by_name) {
    ...
    while ((interface_sym = kxld_symtab_iterator_get_next(&iter))) {
      kext_sym = kxld_symtab_get_locally_defined_symbol_by_name(
          kext_symtab, interface_sym->name);
      if (!kext_sym) {
      ...
      rval =
          kxld_dict_insert(defined_symbols_by_name, kext_sym->name, kext_sym);
      require_noerr(rval, finish);
    }
    ...
    while ((interface_sym = kxld_symtab_iterator_get_next(&iter))) {
      kext_sym = kxld_symtab_get_locally_defined_symbol_by_name(
          kext_symtab, interface_sym->alias);
      if (!kext_sym) {
        kxld_log(
      ...
      rval = kxld_dict_insert(defined_symbols_by_name, interface_sym->name,
                              kext_sym);
      require_noerr(rval, finish);
    }
  }
  ...
  if (obsolete_symbols_by_name) {
    ...
    while ((kext_sym = kxld_symtab_iterator_get_next(&iter))) {
      rval =
          kxld_dict_insert(obsolete_symbols_by_name, kext_sym->name, kext_sym);
      require_noerr(rval, finish);
    }
  }
  ...
  if (defined_cxx_symbols_by_value) {
    (void)kxld_symtab_iterator_init(&iter, kext_symtab, kxld_sym_is_cxx, FALSE);
    while ((kext_sym = kxld_symtab_iterator_get_next(&iter))) {
...
/*******************************************************************************
*******************************************************************************/
kern_return_t export_symbols(const KXLDObject *kext,
                             KXLDDict *defined_symbols_by_name,
                             KXLDDict *defined_cxx_symbols_by_value) {
  kern_return_t rval = KERN_FAILURE;
  KXLDSymtabIterator iter;
  ...
  while ((sym = kxld_symtab_iterator_get_next(&iter))) {
    ...
    if (kxld_sym_is_cxx(sym) && defined_cxx_symbols_by_value) {
      rval =
          kxld_dict_insert(defined_cxx_symbols_by_value, &sym->link_addr, sym);
      require_noerr(rval, finish);
    }
  }
...
void kxld_kext_get_vmsize_for_seg_by_name(const KXLDKext *kext,
                                          const char *segname, u_long *vmsize) {
...
static kern_return_t create_vtables(KXLDKext *kext,
  ...
  while ((sym = kxld_symtab_iterator_get_next(&iter))) {
    if (kxld_object_is_linked(kext->kext)) {
      ...
      rval = get_vtable_syms_from_smcp(kext, defined_symbols, sym, &vtable_sym,
                                       &meta_vtable_sym);
      require_noerr(rval, finish);
    }
    ...
    vtable = kxld_array_get_item(&kext->vtables, i++);
    rval =
        kxld_vtable_init(vtable, vtable_sym, kext->kext, defined_cxx_symbols);
    require_noerr(rval, finish);
    ...
    if (!kxld_object_is_linked(kext->kext)) {
      if (meta_vtable_sym) {
        ...
        rval = kxld_vtable_init(meta_vtable, meta_vtable_sym, kext->kext,
                                defined_cxx_symbols);
        require_noerr(rval, finish);
      } else {
  ...
  require_action(i == kext->vtables.nitems, finish, rval = KERN_FAILURE);
...
static kern_return_t get_vtable_syms_from_smcp(KXLDKext *kext,
  ...
  rval = kxld_sym_get_vtable_name_from_class_name(class_name, vtable_name,
                                                  sizeof(vtable_name));
  require_noerr(rval, finish);
  ...
  if (defined_symbols) {
    ...
    vtable_sym =
        kxld_symtab_get_locally_defined_symbol_by_name(symtab, vtable_name);
  }
  ...
  rval = kxld_sym_get_meta_vtable_name_from_class_name(
      class_name, meta_vtable_name, sizeof(meta_vtable_name));
  require_noerr(rval, finish);
  ...
  if (defined_symbols) {
    ...
    meta_vtable_sym = kxld_symtab_get_locally_defined_symbol_by_name(
        symtab, meta_vtable_name);
  }
  if (!meta_vtable_sym) {
    if (kxld_object_target_supports_strict_patching(kext->kext)) {
      ...
      kxld_log(
          kKxldLogPatching, kKxldLogErr, "Warning: " kKxldLogMissingVtable,
          kxld_demangle(meta_vtable_name, &demangled_name1, &demangled_length1),
          kxld_demangle(class_name, &demangled_name2, &demangled_length2));
    }
  }
...
static kern_return_t resolve_symbols(KXLDKext *kext,
  ...
  kxld_symtab_iterator_init(&iter, symtab, kxld_sym_is_unresolved, FALSE);
  while ((sym = kxld_symtab_iterator_get_next(&iter))) {
    ...
    if (kxld_sym_is_common(sym)) {
      ...
      if (defined_sym) {
         ...
         */
        require_action(
            tests_for_weak, finish, rval = KERN_FAILURE; kxld_log(
...
static kern_return_t patch_vtables(KXLDKext *kext, KXLDDict *patched_vtables,
  ...
  kxld_symtab_iterator_init(&iter, symtab, kxld_sym_is_super_metaclass_pointer,
                            FALSE);
  nvtables = kxld_symtab_iterator_get_num_remaining(&iter);
    ...
    while ((super_metaclass_pointer = kxld_symtab_iterator_get_next(&iter))) {
      ...
      require_action(vtable, finish, rval = KERN_FAILURE;
                     kxld_log(kKxldLogPatching, kKxldLogErr,
                              kKxldLogMissingVtable, vtable_name, class_name));
      ...
      if (!vtable->is_patched) {
        ...
        rval = kxld_sym_get_class_name_from_metaclass(
            metaclass, super_class_name, sizeof(super_class_name));
        require_noerr(rval, finish);

        /* Get the super vtable name from the class name */
        rval = kxld_sym_get_vtable_name_from_class_name(
            super_class_name, super_vtable_name, sizeof(super_vtable_name));
        require_noerr(rval, finish);
        ...
        if (failure) {
          ...
          unpatched_super_vtable =
              kxld_dict_find(&kext->vtable_index, super_vtable_name);
        ...
        rval = kxld_sym_get_final_sym_name_from_class_name(
            super_class_name, final_sym_name, sizeof(final_sym_name));
        require_noerr(rval, finish);
        ...
        if (final_sym) {
          kxld_log(
              kKxldLogPatching, kKxldLogErr,
              "Class '%s' is a subclass of final class '%s'.",
              kxld_demangle(class_name, &demangled_name1, &demangled_length1),
        ...
        /* Get the meta vtable name from the class name */
        rval = kxld_sym_get_meta_vtable_name_from_class_name(
            class_name, vtable_name, sizeof(vtable_name));
        require_noerr(rval, finish);
        ...
        require_action(len == const_strlen(kOSMetaClassVTableName), finish,
                       rval = KERN_FAILURE);

        /* Get the super meta vtable */
        super_vtable = kxld_dict_find(patched_vtables, super_vtable_name);
        require_action(super_vtable && super_vtable->is_patched, finish,
                       rval = KERN_FAILURE);
...
static const KXLDSym *get_metaclass_symbol_from_super_meta_class_pointer_symbol(
  ...
  reloc =
      kxld_object_get_reloc_at_symbol(kext->kext, super_metaclass_pointer_sym);
...
finish:
  if (metaclass == NULL) {
    kxld_log(kKxldLogLinking, kKxldLogErr, "metaclass == NULL kxld_sym %s <%s>",
