...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_seg_init_from_macho_32(KXLDSeg *seg,
                                          struct segment_command *src) {
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_seg_init_from_macho_64(KXLDSeg *seg,
                                          struct segment_command_64 *src) {
  kern_return_t rval = KERN_FAILURE;
  check(seg);
  check(src);
 ...
 * matching sections, placing them in the segment's section index as we find
 * them. If this function must exit early, the segment's section index is left
...
static kern_return_t reorder_sections(KXLDSeg *seg, KXLDArray *section_order) {
  ...
  reorder_buffer = kxld_alloc(seg->sects.nitems * sizeof(*reorder_buffer));
  ...
  while (legacy_index < section_order->nitems) {
    ...
    for (i = legacy_start; i < legacy_end; ++i) {
      ...
      if (sect) {
        (void)reorder_section(&seg->sects, &sect_reorder_index, reorder_buffer,
                              j);
      }
    }
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_seg_export_macho_to_file_buffer(
    const KXLDSeg *seg, u_char *buf, u_long *header_offset, u_long header_size,
    u_long *data_offset, u_long data_size, boolean_t is_32_bit) {
  kern_return_t rval = KERN_FAILURE;
  KXLDSect *sect = NULL;
  ...
  KXLD_3264_FUNC(is_32_bit, rval, seg_export_macho_header_32,
                 seg_export_macho_header_64, seg, buf, header_offset,
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_seg_export_macho_to_vm(const KXLDSeg *seg, u_char *buf,
                                          u_long *header_offset,
                                          u_long header_size, u_long data_size,
  ...
  for (i = 0; i < seg->sects.nitems; ++i) {
    ...
    rval = kxld_sect_export_macho_to_vm(sect, buf, header_offset, header_size,
                                        file_link_addr, data_size, is_32_bit);
    require_noerr(rval, finish);
  }
  ...
  *header_offset += sizeof(*seghdr);
...
#if SPLIT_KEXTS_DEBUG
  {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             "segname %s seghdr %p vmaddr %p vmsize 0x%02X %u fileoff 0x%02X "
             "%u <%s>",
             seg->segname[0] ? seg->segname : "none", (void *)seghdr,
             (void *)((uint64_t)seghdr->vmaddr), seghdr->vmsize, seghdr->vmsize,
             seghdr->fileoff, seghdr->fileoff, __func__);
  }
#endif
...
static kern_return_t seg_export_macho_header_64(const KXLDSeg *seg, u_char *buf,
...
#if SPLIT_KEXTS_DEBUG
  {
    ...
    if (mach->magic != MH_MAGIC_64) {
      kxld_log(kKxldLogLinking, kKxldLogErr, "bad macho header at %p <%s>",
               (void *)mach, __func__);
      goto finish;
    }
  }
  ...
  *header_offset += sizeof(*seghdr);
  ...
  {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             "%p >>> Start of %s seghdr (size %lu) <%s>", (void *)seghdr,
             seg->segname[0] ? seg->segname : "none", sizeof(*seghdr),
             __func__);
    kxld_log(kKxldLogLinking, kKxldLogErr, "%p <<< End of %s seghdr <%s>",
             (void *)((u_char *)seghdr + sizeof(*seghdr)),
             seg->segname[0] ? seg->segname : "none", __func__);

    kxld_log(
        kKxldLogLinking, kKxldLogErr,
        "%s seghdr, cmdsize %d vmaddr %p vmsize %p %llu fileoff %p %llu <%s>",
        seg->segname[0] ? seg->segname : "none", seghdr->cmdsize,
        (void *)seghdr->vmaddr, (void *)seghdr->vmsize, seghdr->vmsize,
        (void *)seghdr->fileoff, seghdr->fileoff, __func__);
  }
#endif
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_seg_add_section(KXLDSeg *seg, KXLDSect *sect) {
  kern_return_t rval = KERN_FAILURE;
  KXLDSect **sectp = NULL;
  u_int i;
...
kern_return_t kxld_seg_finish_init(KXLDSeg *seg) {
  ...
  if ((seg->vmsize == 0) && (seg->sects.nitems)) {
    ...
    seg->vmsize =
        kxld_round_page_cross_safe(maxaddr + maxsize - seg->base_addr);
  }
...
void kxld_seg_relocate(KXLDSeg *seg, kxld_addr_t link_addr) {
...
#if SPLIT_KEXTS_DEBUG
  {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             ...
             (void *)seg->link_addr, seg->segname[0] ? seg->segname : "none",
             seg->vmsize, __func__);
    kxld_log(kKxldLogLinking, kKxldLogErr, "%p <<< End of %s segment <%s>",
...
/*******************************************************************************
*******************************************************************************/
void kxld_seg_populate_linkedit(KXLDSeg *seg, const KXLDSymtab *symtab,
                                boolean_t is_32_bit
#if KXLD_PIC_KEXTS
                                ,
