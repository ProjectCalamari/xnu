...
void kxld_versionmin_init_from_macho(KXLDversionmin *versionmin,
  ...
  check(versionmin);
  check(src);
  check((src->cmd == LC_VERSION_MIN_MACOSX) ||
        (src->cmd == LC_VERSION_MIN_IPHONEOS) ||
        (src->cmd == LC_VERSION_MIN_TVOS) ||
        (src->cmd == LC_VERSION_MIN_WATCHOS));
...
/*******************************************************************************
*******************************************************************************/
u_long kxld_versionmin_get_macho_header_size(
    __unused const KXLDversionmin *versionmin) {
...
kern_return_t kxld_versionmin_export_macho(const KXLDversionmin *versionmin,
  ...
  require_action(sizeof(*versionminhdr) <= header_size - *header_offset, finish,
                 rval = KERN_FAILURE);
  versionminhdr =
      (struct version_min_command *)((void *)(buf + *header_offset));
