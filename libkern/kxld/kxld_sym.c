...
static kern_return_t init_predicates(KXLDSym *sym, u_char n_type,
                                     u_short n_desc) __attribute__((nonnull));
...
kern_return_t kxld_sym_init_from_macho64(KXLDSym *sym, char *strtab,
  ...
  if (!strcmp("__ZN15OSMetaClassBase25_RESERVEDOSMetaClassBase3Ev",
              sym->name)) {
  ...
  if (n_type & N_STAB) {
    ...
    default:
      ...
      kxld_log(kKxldLogLinking, kKxldLogErr,
               kKxldLogMalformedMachO "Invalid N_STAB symbol type: %u.",
               n_type);
      goto finish;
    }
    ...
    default:
      rval = KERN_FAILURE;
      kxld_log(kKxldLogLinking, kKxldLogErr,
               kKxldLogMalformedMachO "Invalid symbol type: %u.", type);
      goto finish;
    }
    ...
    if (!strncmp(CXX_PREFIX, sym->name, const_strlen(CXX_PREFIX))) {
      ...
      if (streq_safe(sym->name, METACLASS_VTABLE_PREFIX,
        ...
        sym->is_metaclass = 1;
      } else if (strnstr(sym->name, SUPER_METACLASS_POINTER_TOKEN,
                         strlen(sym->name))) {
...
/*******************************************************************************
*******************************************************************************/
void kxld_sym_deinit(KXLDSym *sym __unused) { check(sym); }
...
/*******************************************************************************
*******************************************************************************/
u_int kxld_sym_get_section_offset(const KXLDSym *sym, const KXLDSect *sect) {
  check(sym);
...
kern_return_t kxld_sym_get_class_name_from_super_metaclass_pointer(
  ...
  rval = extract_inner_string(sym->name, OSOBJ_PREFIX,
                              SUPER_METACLASS_POINTER_TOKEN, class_name,
                              class_name_len);
  require_noerr(rval, finish);
...
kern_return_t kxld_sym_get_class_name_from_vtable(const KXLDSym *sym,
  ...
  rval = kxld_sym_get_class_name_from_vtable_name(sym->name, class_name,
                                                  class_name_len);
  require_noerr(rval, finish);
...
kern_return_t kxld_sym_get_class_name_from_vtable_name(const char *vtable_name,
  ...
  rval = extract_inner_string(vtable_name, VTABLE_PREFIX, NULL, class_name,
                              class_name_len);
  require_noerr(rval, finish);
...
kern_return_t kxld_sym_get_vtable_name_from_class_name(const char *class_name,
  ...
  require_action(outlen < vtable_name_len, finish, rval = KERN_FAILURE);
...
kxld_sym_get_meta_vtable_name_from_class_name(const char *class_name,
  ...
  outlen =
      strlcpy(meta_vtable_name, METACLASS_VTABLE_PREFIX, meta_vtable_name_len);
  require_action(outlen < meta_vtable_name_len, finish, rval = KERN_FAILURE);

  outlen = strlcat(meta_vtable_name, class_name, meta_vtable_name_len);
  require_action(outlen < meta_vtable_name_len, finish, rval = KERN_FAILURE);

  outlen =
      strlcat(meta_vtable_name, METACLASS_VTABLE_SUFFIX, meta_vtable_name_len);
  require_action(outlen < meta_vtable_name_len, finish, rval = KERN_FAILURE);
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_sym_get_final_sym_name_from_class_name(
    const char *class_name, char final_sym_name[], u_long final_sym_name_len) {
  kern_return_t rval = KERN_FAILURE;
  u_long outlen = 0;
  ...
  require_action(outlen < final_sym_name_len, finish, rval = KERN_FAILURE);
...
/*******************************************************************************
*******************************************************************************/
void kxld_sym_set_got(KXLDSym *sym) { sym->is_got = 1; }
#endif /* KXLD_USER_OR_GOT */

/*******************************************************************************
*******************************************************************************/
void kxld_sym_relocate(KXLDSym *sym, const KXLDSect *sect) {
  if (kxld_sym_is_section(sym)) {
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_sym_export_macho_32(const KXLDSym *sym, u_char *_nl,
                                       char *strtab, u_long *stroff,
                                       u_long strsize) {
  kern_return_t rval = KERN_FAILURE;
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_sym_export_macho_64(const KXLDSym *sym, u_char *_nl,
                                       char *strtab, u_long *stroff,
                                       u_long strsize) {
  kern_return_t rval = KERN_FAILURE;
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_sym_resolve(KXLDSym *sym, kxld_addr_t addr) {
  kern_return_t rval = KERN_FAILURE;
...
kern_return_t kxld_sym_resolve_common(KXLDSym *sym, u_int sectnum,
  ...
  require_action(kxld_sym_is_common(sym), finish, rval = KERN_FAILURE);
