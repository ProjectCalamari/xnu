...
#define streq_safe(str1, str2, len)                                            \
  (((str1) && (str2)) ? !strncmp((str1), (str2), (len)) : 0)
...
#define kKxldLogArchNotSupported                                               \
  "The target architecture (cputype 0x%x) is not supported by kxld."
#define kKxldLogArchNotFound                                                   \
  "The kext does not contain a fat slice for the target architecture."
#define kKxldLogFiletypeNotSupported                                           \
  "The Mach-O filetype 0x%x is not supported on the target architecture."
#define kKxldLogTruncatedMachO                                                 \
  "The Mach-O file has been truncated. Make sure the Mach-O header "           \
  "structures are correct."
#define kKxldLogMalformedMachO "The Mach-O file is malformed: "
#define kKxldLogMalformedVTable                                                \
  "The vtable '%s' is malformed. Make sure your kext has been built against "  \
  "the correct headers."
#define kKxldLogMissingVtable                                                  \
  "Cannot find the vtable '%s' for class '%s'. This vtable symbol is "         \
  "required for binary compatibility, and it may have been stripped."
#define kKxldLogDirectPureVirtualCall                                          \
  "This kext calls a pure virtual function. Make sure your kext's "            \
  "OSObject-derived classes implement all pure virtual functions."
#define kKxldLogParentOutOfDate                                                \
  "The super class vtable '%s' for vtable '%s' is out of date. Make sure "     \
  "your kext has been built against the correct headers."
#define kKxldLogNoKmodInfo "The kext is missing its kmod_info structure."
#define kKxldLogRelocationOverflow                                             \
  "A relocation entry has overflowed. The kext may be too far from one "       \
  "of its dependencies. Check your kext's load address."
#define kKxldLogRelocatingPatchedSym                                           \
  "Warning: relocation failed because some class in this kext "                \
  "didn't use the OSDeclareDefaultStructors and "                              \
  "OSDefineMetaClassAndStructors, "                                            \
  "or didn't export all vtable symbols, so it still "                          \
  "references %s, which has been patched with another symbol for binary "      \
  "compatibility. "                                                            \
...
void *kxld_calloc(size_t size) __attribute__((malloc, visibility("hidden")));

void *kxld_alloc(size_t size) __attribute__((malloc, visibility("hidden")));
...
void kxld_free(void *ptr, size_t size) __attribute__((visibility("hidden")));
...
void kxld_print_memory_report(void) __attribute__((visibility("hidden")));
