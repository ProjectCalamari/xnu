...
kern_return_t kxld_splitinfolc_export_macho(const KXLDsplitinfolc *splitinfolc,
  ...
  *header_offset += sizeof(*splitinfolc_hdr);
  ...
  memcpy(buf + *data_offset,
         linked_object->kextExecutable + splitinfolc->dataoff,
  ...
  kxld_log(kKxldLogLinking, kKxldLogErr,
           ...
           (void *)buf, (void *)(buf + size), size, (void *)(dataPtr),
           (void *)(dataPtr + splitinfolc->datasize), splitinfolc->datasize,
           __func__);

  if (*(dataPtr + 0) != 0x7F) {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             "\n\n bad LC_SEGMENT_SPLIT_INFO: 0x%02X %02X %02X %02X %02X %02X "
             "%02X %02X at %p (buf %p + %lu) <%s>",
             *(dataPtr + 0), *(dataPtr + 1), *(dataPtr + 2), *(dataPtr + 3),
             *(dataPtr + 4), *(dataPtr + 5), *(dataPtr + 6), *(dataPtr + 7),
             (void *)dataPtr, (void *)buf, *data_offset, __func__);
  }
#endif
