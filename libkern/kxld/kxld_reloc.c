...
static boolean_t generic_reloc_has_pair(u_int _type) __attribute__((const));
static u_int generic_reloc_get_pair_type(u_int _prev_type)
    __attribute__((const));
static boolean_t generic_reloc_has_got(u_int _type) __attribute__((const));
static kern_return_t
generic_process_reloc(const KXLDRelocator *relocator, u_char *instruction,
                      u_int length, u_int pcrel, kxld_addr_t base_pc,
                      kxld_addr_t link_pc, kxld_addr_t link_disp, u_int type,
                      kxld_addr_t target, kxld_addr_t pair_target,
                      boolean_t swap);
#endif /* KXLD_USER_OR_I386 */

#if KXLD_USER_OR_X86_64
static boolean_t x86_64_reloc_has_pair(u_int _type) __attribute__((const));
static u_int x86_64_reloc_get_pair_type(u_int _prev_type)
    __attribute__((const));
static boolean_t x86_64_reloc_has_got(u_int _type) __attribute__((const));
static kern_return_t
x86_64_process_reloc(const KXLDRelocator *relocator, u_char *instruction,
                     u_int length, u_int pcrel, kxld_addr_t base_pc,
                     kxld_addr_t link_pc, kxld_addr_t link_disp, u_int type,
                     kxld_addr_t target, kxld_addr_t pair_target,
                     boolean_t swap);
...
#if KXLD_USER_OR_ARM
static boolean_t arm_reloc_has_pair(u_int _type) __attribute__((const));
static u_int arm_reloc_get_pair_type(u_int _prev_type) __attribute__((const));
static boolean_t arm_reloc_has_got(u_int _type) __attribute__((const));
static kern_return_t
arm_process_reloc(const KXLDRelocator *relocator, u_char *instruction,
                  u_int length, u_int pcrel, kxld_addr_t base_pc,
                  kxld_addr_t link_pc, kxld_addr_t link_disp, u_int type,
...
#if KXLD_USER_OR_ARM64
static boolean_t arm64_reloc_has_pair(u_int _type) __attribute__((const));
static u_int arm64_reloc_get_pair_type(u_int _prev_type) __attribute__((const));
static boolean_t arm64_reloc_has_got(u_int _type) __attribute__((const));
static kern_return_t
arm64_process_reloc(const KXLDRelocator *relocator, u_char *instruction,
                    u_int length, u_int pcrel, kxld_addr_t base_pc,
                    kxld_addr_t link_pc, kxld_addr_t link_disp, u_int type,
                    kxld_addr_t target, kxld_addr_t pair_target,
                    boolean_t swap);
...
static u_int count_relocatable_relocs(const KXLDRelocator *relocator,
                                      ...
                                      u_int nrelocs) __attribute__((pure));
...
static kern_return_t
check_for_direct_pure_virtual_call(const KXLDRelocator *relocator,
                                   u_long offset);
...
kern_return_t kxld_relocator_init(KXLDRelocator *relocator, u_char *file,
  ...
  default:
    rval = KERN_FAILURE;
    kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogArchNotSupported, cputype);
    goto finish;
  }
...
kern_return_t kxld_reloc_create_macho(KXLDArray *relocarray,
                                      ...
                                      const struct relocation_info *srcs,
                                      u_int nsrcs) {
  kern_return_t rval = KERN_FAILURE;
  ...
  if (nrelocs) {
    ...
    for (i = 0; i < nsrcs; ++i) {
      ...
      if (relocator->reloc_has_pair(reloc->reloc_type)) {
        ...
        if (relocator->may_scatter && (src->r_address & R_SCATTERED)) {
          require_action(relocator->reloc_get_pair_type(reloc->reloc_type) ==
                             scatsrc->r_type,
...
static u_int count_relocatable_relocs(const KXLDRelocator *relocator,
  ...
  for (i = 1; i < nrelocs; ++i) {
    ...
    if (reloc->r_address & R_SCATTERED) {
      ...
      num_nonpair_relocs += !relocator->reloc_has_pair(sreloc->r_type);
    } else {
...
/*******************************************************************************
*******************************************************************************/
boolean_t kxld_relocator_has_pair(const KXLDRelocator *relocator,
                                  u_int r_type) {
  check(relocator);
...
/*******************************************************************************
*******************************************************************************/
KXLDSym *kxld_reloc_get_symbol(const KXLDRelocator *relocator,
  ...
  case KXLD_TARGET_SECTNUM:
    if (data) {
      value =
          kxld_relocator_get_pointer_at_addr(relocator, data, reloc->address);
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_reloc_get_reloc_index_by_offset(const KXLDArray *relocs,
                                                   kxld_size_t offset,
                                                   u_int *idx) {
  kern_return_t rval = KERN_FAILURE;
  KXLDReloc *reloc = NULL;
...
/*******************************************************************************
*******************************************************************************/
KXLDReloc *kxld_reloc_get_reloc_by_offset(const KXLDArray *relocs,
                                          kxld_addr_t offset) {
  kern_return_t rval = KERN_FAILURE;
  KXLDReloc *reloc = NULL;
  u_int i = 0;
...
/*******************************************************************************
*******************************************************************************/
u_long kxld_reloc_get_macho_data_size(const KXLDArray *locrelocs,
...
kern_return_t kxld_reloc_export_macho(const KXLDRelocator *relocator,
                                      ...
                                      const KXLDArray *extrelocs, u_char *buf,
  ...
  require_action(sizeof(*dysymtabhdr) <= header_size - *header_offset, finish,
                 rval = KERN_FAILURE);
  ...
  *header_offset += sizeof(*dysymtabhdr);
  ...
  require_action((*data_offset + data_size) <= size, finish,
                 rval = KERN_FAILURE);
  ...
  if (kaslr_offsets == NULL) {
    ...
    kaslr_offsets =
        (uint32_t *)calloc(kaslr_offsets_count, sizeof(*kaslr_offsets));
  }
  ...
  *data_offset += count * sizeof(struct relocation_info);
  ...
  kxld_log(kKxldLogLinking, kKxldLogErr,
           "%p >>> Start of dysymtabhdr (size %lu) <%s> ", (void *)dysymtabhdr,
           sizeof(*dysymtabhdr), __func__);
  kxld_log(kKxldLogLinking, kKxldLogErr, "%p <<< End of dysymtabhdr <%s> ",
           (void *)((u_char *)dysymtabhdr + sizeof(*dysymtabhdr)), __func__);

  kxld_log(kKxldLogLinking, kKxldLogErr,
           "dysymtabhdr at %p: cmdsize %u indirectsymoff %u nindirectsyms %u "
           "extreloff %u nextrel %u locreloff %u nlocrel %u <%s>",
           (void *)dysymtabhdr, dysymtabhdr->cmdsize,
           dysymtabhdr->indirectsymoff, dysymtabhdr->nindirectsyms,
           dysymtabhdr->extreloff, dysymtabhdr->nextrel, dysymtabhdr->locreloff,
           dysymtabhdr->nlocrel, __func__);
#endif
...
kxld_addr_t kxld_relocator_get_pointer_at_addr(const KXLDRelocator *relocator,
  ...
  KXLD_3264_FUNC(relocator->is_32_bit, value, get_pointer_at_addr_32,
                 get_pointer_at_addr_64, relocator, data, offset);
...
kern_return_t kxld_relocator_process_sect_reloc(KXLDRelocator *relocator,
  ...
  rval = relocator->process_reloc(
      relocator, instruction, reloc->length, reloc->pcrel, base_pc, link_pc,
      link_disp, reloc->reloc_type, target, pair_target, relocator->swap);
  require_noerr(rval, finish);
...
kern_return_t kxld_reloc_update_symindex(KXLDReloc *reloc, u_int symindex) {
  ...
  require_action(reloc->target_type == KXLD_TARGET_SYMBOLNUM, finish,
                 rval = KERN_FAILURE);
...
kern_return_t kxld_relocator_process_table_reloc(KXLDRelocator *relocator,
  ...
  rval = relocator->process_reloc(
      relocator, instruction, reloc->length, reloc->pcrel, base_pc, link_pc,
      link_addr, reloc->reloc_type, target, pair_target, relocator->swap);
  require_noerr(rval, finish);
  ...
  *_pair_target = 0;
  ...
  case KXLD_TARGET_LOOKUP:
    ...
    if (reloc->pair_target_type == KXLD_TARGET_LOOKUP) {
      rval = get_target_by_address_lookup(&pair_target, reloc->pair_target,
                                          relocator->sectarray);
      require_noerr(rval, finish);
...
check_for_direct_pure_virtual_call(const KXLDRelocator *relocator,
  ...
  if (relocator->current_vtable) {
    entry = kxld_vtable_get_entry_for_offset(relocator->current_vtable, offset,
                                             relocator->is_32_bit);
    require_action(
        !entry || !entry->patched.name ||
            !kxld_sym_name_is_pure_virtual(entry->patched.name),
        finish, rval = KERN_FAILURE;
        kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogDirectPureVirtualCall));
  }
...
static kern_return_t export_macho_for_array(const KXLDRelocator *relocator,
  ...
  for (i = 0; i < relocs->nitems; ++i) {
    ...
    case KXLD_TARGET_LOOKUP:
      if (kaslr_offsets) {
        if (kaslr_offsets_index >= kaslr_offsets_count) {
          kxld_log(kKxldLogLinking, kKxldLogErr,
                   "kaslr_offsets overflow %d > %d <%s> ", kaslr_offsets_index,
                   kaslr_offsets_count, __func__);
          abort();
        }
    ...
    case KXLD_TARGET_SECTNUM:
      if (kaslr_offsets) {
        if (kaslr_offsets_index >= kaslr_offsets_count) {
          kxld_log(kKxldLogLinking, kKxldLogErr, "kaslr_offsets overflow <%s> ",
                   __func__);
          abort();
        }
        // reloc->address is really an offset from the start of the kext
...
static boolean_t generic_reloc_has_pair(u_int _type) {
  ...
  return type == GENERIC_RELOC_SECTDIFF || type == GENERIC_RELOC_LOCAL_SECTDIFF;
}

/*******************************************************************************
*******************************************************************************/
static u_int generic_reloc_get_pair_type(u_int _prev_type __unused) {
  return GENERIC_RELOC_PAIR;
}

/*******************************************************************************
*******************************************************************************/
static boolean_t generic_reloc_has_got(u_int _type __unused) { return FALSE; }

/*******************************************************************************
*******************************************************************************/
static kern_return_t
generic_process_reloc(const KXLDRelocator *relocator, u_char *instruction,
                      u_int length, u_int pcrel, kxld_addr_t _base_pc,
                      kxld_addr_t _link_pc, kxld_addr_t _link_disp __unused,
                      u_int _type, kxld_addr_t _target,
...
/*******************************************************************************
*******************************************************************************/
static u_int x86_64_reloc_get_pair_type(u_int _prev_type __unused) {
...
*******************************************************************************/
static kern_return_t
x86_64_process_reloc(const KXLDRelocator *relocator __unused,
                     u_char *instruction, u_int length, u_int pcrel,
                     kxld_addr_t _base_pc __unused, kxld_addr_t _link_pc,
                     kxld_addr_t _link_disp, u_int _type, kxld_addr_t _target,
                     kxld_addr_t _pair_target, boolean_t swap __unused) {
  kern_return_t rval = KERN_FAILURE;
  enum reloc_type_x86_64 type = _type;
...
static kern_return_t calculate_displacement_x86_64(uint64_t target,
  ...
  require_action(
      difference < X86_64_RIP_RELATIVE_LIMIT, finish, rval = KERN_FAILURE;
...
/*******************************************************************************
*******************************************************************************/
static u_int arm_reloc_get_pair_type(u_int _prev_type __unused) {
  return ARM_RELOC_PAIR;
}

/*******************************************************************************
*******************************************************************************/
static boolean_t arm_reloc_has_got(u_int _type __unused) { return FALSE; }
...
arm_process_reloc(const KXLDRelocator *relocator __unused, u_char *instruction,
                  ...
                  kxld_addr_t _link_disp __unused, u_int _type __unused,
...
/*******************************************************************************
*******************************************************************************/
boolean_t arm64_reloc_has_pair(u_int _type) {
  return _type == ARM64_RELOC_SUBTRACTOR;
}

/*******************************************************************************
*******************************************************************************/
u_int arm64_reloc_get_pair_type(u_int _prev_type __unused) {
...
/*******************************************************************************
*******************************************************************************/
boolean_t arm64_reloc_has_got(u_int _type) {
...
/*******************************************************************************
*******************************************************************************/
kern_return_t arm64_process_reloc(
    const KXLDRelocator *relocator __unused, u_char *instruction, u_int length,
    u_int pcrel, kxld_addr_t _base_pc __unused, kxld_addr_t _link_pc,
  ...
  if (length == 2) {
    ...
    case ARM64_RELOC_BRANCH26:
      ...
      require_action(
          difference < (128 * 1024 * 1024), finish, rval = KERN_FAILURE;
          kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogRelocationOverflow));
