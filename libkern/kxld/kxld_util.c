...
void kxld_log(KXLDLogSubsystem subsystem, KXLDLogLevel level,
  ...
  if (s_logging_callback) {
    ...
    if (length >= sizeof(stack_buffer)) {
      ...
      snprintf(alloc_buffer, length, "kxld[%s]: %s", s_callback_name,
               in_format);
      format = alloc_buffer;
    }
...
/*******************************************************************************
*******************************************************************************/
void kxld_free(void *ptr, size_t size __unused) {
#if DEBUG
  ++num_frees;
  bytes_freed += size;
#endif
...
/*******************************************************************************
*******************************************************************************/
void kxld_page_free_untracked(void *ptr, size_t size __unused) {
#if KERNEL
...
kern_return_t validate_and_swap_macho_32(u_char *file, u_long size
  ...
  require_action(
      size >= sizeof(*mach_hdr), finish, rval = KERN_FAILURE;
  ...
  require_action(mach_hdr->magic == MH_MAGIC, finish, rval = KERN_FAILURE;
                 kxld_log(kKxldLogLinking, kKxldLogErr,
                          kKxldLogMalformedMachO "Invalid magic number: 0x%x.",
                          mach_hdr->magic));
  ...
  for (i = 0; i < mach_hdr->ncmds; ++i, offset += cmdsize) {
    ...
    require_action(
        size >= offset + cmdsize, finish, rval = KERN_FAILURE;
        kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogTruncatedMachO));
    ...
    case LC_SEGMENT:
      ...
      require_action(
          size >= seg_hdr->fileoff + seg_hdr->filesize, finish,
          rval = KERN_FAILURE;
          kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogTruncatedMachO));
      ...
      for (j = 0; j < seg_hdr->nsects; ++j) {
         ...
         */
        require_action(
            (sects[j].flags & S_ZEROFILL) ||
                (size >= sects[j].offset + sects[j].size),
         ...
         */
        require_action(
            size >= sects[j].reloff + sects[j].nreloc * sizeof(*relocs), finish,
            rval = KERN_FAILURE;
            kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogTruncatedMachO));
...
#if !KERNEL
        if (swap) {
          swap_relocation_info(relocs, sects[j].nreloc, host_order);
        }
#endif /* !KERNEL */
      }
    ...
    case LC_SYMTAB:
      ...
      require_action(
          size >= symtab_hdr->symoff + symtab_hdr->nsyms * sizeof(*symtab),
          finish, rval = KERN_FAILURE;
          kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogTruncatedMachO));

      /* Verify that the file is big enough for the string table */
      require_action(
          size >= symtab_hdr->stroff + symtab_hdr->strsize, finish,
          rval = KERN_FAILURE;
          kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogTruncatedMachO));
...
kern_return_t validate_and_swap_macho_64(u_char *file, u_long size
  ...
  /* Validate the mach_header's magic number */
  require_action(mach_hdr->magic == MH_MAGIC_64, finish, rval = KERN_FAILURE;
                 kxld_log(kKxldLogLinking, kKxldLogErr,
                          kKxldLogMalformedMachO "Invalid magic number: 0x%x.",
                          mach_hdr->magic));
...
/*******************************************************************************
*******************************************************************************/
boolean_t kxld_is_32_bit(cpu_type_t cputype) {
...
#if DEBUG
  kxld_log(kKxldLogLinking, kKxldLogExplicit,
...
void kxld_show_split_info(splitKextLinkInfo *info) {
  kxld_log(kKxldLogLinking, kKxldLogErr,
           ...
           (void *)(info->kextExecutable + info->kextSize), info->kextSize,
           (void *)info->linkedKext,
           (void *)(info->linkedKext + info->linkedKextSize),
           info->linkedKextSize, (void *)info->vmaddr_TEXT,
           (void *)info->vmaddr_TEXT_EXEC, (void *)info->vmaddr_DATA,
           (void *)info->vmaddr_DATA_CONST, (void *)info->vmaddr_LLVM_COV,
