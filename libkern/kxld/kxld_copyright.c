...
CFStringRef passes[] = {
    ...
    CFSTR("Copyright © 2004 , 2006-2008 Apple Inc. All rights reserved."),
    CFSTR("Copyright © 1998,2000-2002,2004,2006-2008 Apple Inc. All rights "
          "reserved."),
    CFSTR("IOPCIFamily 2.1; Copyright © 2004,2006-2008 Apple Inc. All rights "
          "reserved."),
    CFSTR("Copyright © 2004,2006-2008 Apple Inc. All rights reserved.  The "
          "quick brown fox jumped over the lazy dog."),
    CFSTR("IOPCIFamily 2.1; Copyright © 2004,2006-2008 Apple Inc. All rights "
          "reserved.  The quick brown fox jumped over the lazy dog.")};
...
static boolean_t is_space(const char c) __attribute__((const));
static boolean_t is_token_delimiter(const char c) __attribute__((const));
static boolean_t is_token_break(const char *str) __attribute__((pure, nonnull));
static boolean_t token_is_year(const char *str) __attribute__((pure, nonnull));
...
static boolean_t dates_are_valid(const char *str, const u_long len) {
  ...
  for (token_ptr = str; token_ptr < str + len; ++token_ptr) {
    ...
    if (is_token_break(token_ptr)) {
      ...
      if (!token_is_year(token_buffer) && !token_is_yearRange(token_buffer)) {
        goto finish;
      }
...
int main(int argc __unused, char *argv[] __unused) {
  ...
  for (i = 0; i < const_array_len(passes); ++i) {
    ...
    printf("%s: %s\n", (kxld_validate_copyright_string(str)) ? "pass" : "fail",
           str);
  }
