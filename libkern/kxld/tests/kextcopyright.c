...
static void printFormat(void) {
  fprintf(stderr,
          "The copyright string should be contained in the "
          "NSHumanReadableCopyright key.\n"
          "It should be of the format:\n"
          "\tCopyright © [year(s) of publication] Apple Inc. All rights "
          "reserved.\n\n"
          "where [year(s) of publication] is a comma-separated list of years "
          "and/or\n"
          "year ranges, e.g., 2004, 2006-2008.  Years must be four digits.  "
          "Year ranges\n"
          ...
          "\tCopyright © 1998,2000-2002,2004,2006-2008 Apple Inc. All rights "
          "reserved.\n");
}

/******************************************************************************
******************************************************************************/
char *convert_cfstring(CFStringRef the_string) {
  char *result = NULL;
  ...
  the_data = CFStringCreateExternalRepresentation(
      kCFAllocatorDefault, the_string, kCFStringEncodingUTF8, 0);
  if (!the_data) {
...
int main(int argc, const char *argv[]) {
  ...
  anURL = CFURLCreateFromFileSystemRepresentation(
      kCFAllocatorDefault, (const UInt8 *)argv[1], strlen(argv[1]),
      /* isDirectory */ FALSE);
  if (!anURL) {
  ...
  readableCopyrightString =
      CFDictionaryGetValue(aDict, kNSHumanReadableCopyrightKey);
  ...
  if (infoCopyrightString) {
    fprintf(
        ...
        "It has been replaced by CFBundleVersion and "
  ...
  if (!readableCopyrightIsValid) {
    if (infoCopyrightIsValid) {
      fprintf(stderr, "Warning: The copyright string in "
                      "NSHumanReadableCopyright is invalid,\n"
                      "but the string in CFBundleGetInfoString is valid.  "
                      "CFBundleGetInfoString is\n"
                      "obsolete.  Please migrate your copyright string to "
                      "NSHumanReadableCopyright.\n\n");
    } else {
      fprintf(stderr,
