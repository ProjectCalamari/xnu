...
static kern_return_t get_target_machine_info(KXLDObject *object,
                                             cpu_type_t cputype,
                                             cpu_subtype_t cpusubtype);
static kern_return_t get_macho_slice_for_arch(KXLDObject *object, u_char *file,
                                              u_long size);
...
static KXLDSeg *get_seg_by_base_addr(KXLDObject *object, kxld_addr_t base_addr);
...
static kern_return_t export_macho_header(const KXLDObject *object, u_char *buf,
                                         u_int ncmds, u_long *header_offset,
                                         u_long header_size);
...
static KXLDSeg *kxld_object_get_seg_by_name(const KXLDObject *object,
                                            const char *segname);
static KXLDSect *kxld_object_get_sect_by_name(const KXLDObject *object,
                                              const char *segname,
                                              const char *sectname);

/*******************************************************************************
*******************************************************************************/
size_t kxld_object_sizeof(void) { return sizeof(KXLDObject); }

/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_object_init_from_macho(KXLDObject *object, u_char *file,
                                          u_long size, const char *name,
  ...
  object->include_kaslr_relocs =
      ((flags & kKXLDFlagIncludeRelocs) == kKXLDFlagIncludeRelocs);
#endif
  ...
  rval =
      kxld_relocator_init(&object->relocator, my_file, object->symtab,
                          &object->sects, object->cputype, object->cpusubtype,
  ...
  if (kxld_object_is_32_bit(object)) {
    ...
    struct mach_header_64 *mach_hdr =
        (struct mach_header_64 *)((void *)my_file);
    object->filetype = mach_hdr->filetype;
  }
  ...
  default:
    rval = KERN_FAILURE;
    kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogFiletypeNotSupported,
             object->filetype);
    goto finish;
  }
  ...
  if (!kxld_object_is_kernel(object)) {
    ...
    if (seg) {
      (void)kxld_seg_populate_linkedit(
          seg, object->symtab, kxld_object_is_32_bit(object)
#if KXLD_PIC_KEXTS
                                   ,
...
/*******************************************************************************
*******************************************************************************/
kern_return_t get_target_machine_info(KXLDObject *object,
 ...
 * __AVX2__ is a temporary solution until this is available. */
  ...
  return KERN_SUCCESS;
#else
  kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogArchNotSupported,
...
static kern_return_t get_macho_slice_for_arch(KXLDObject *object, u_char *file,
...
#if !KERNEL
  require_action(size >= sizeof(*fat), finish, rval = KERN_FAILURE; kxld_log(
                     kKxldLogLinking, kKxldLogErr, kKxldLogTruncatedMachO));
  ...
  if (fat->magic == FAT_MAGIC) {
    ...
    u_long arch_size;
    boolean_t ovr = os_mul_and_add_overflow(fat->nfat_arch, sizeof(*archs),
                                            sizeof(*fat), &arch_size);

    require_action(
        !ovr && size >= arch_size, finish, rval = KERN_FAILURE;
    ...
    arch = NXFindBestFatArch(object->cputype, object->cpusubtype, archs,
                             fat->nfat_arch);
    require_action(arch, finish, rval = KERN_FAILURE; kxld_log(
                       kKxldLogLinking, kKxldLogErr, kKxldLogArchNotFound));
    require_action(
        size >= arch->offset + arch->size, finish, rval = KERN_FAILURE;
        kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogTruncatedMachO));
  ...
  require_action(
      object->cputype == mach_hdr->cputype, finish, rval = KERN_FAILURE;
      kxld_log(kKxldLogLinking, kKxldLogErr, kKxldLogTruncatedMachO));
...
init_from_final_linked_image(KXLDObject *object, u_int *filetype_out,
  ...
  KXLD_3264_FUNC(kxld_object_is_32_bit(object), base_offset,
                 get_macho_cmd_data_32, get_macho_cmd_data_64, my_file, offset,
                 &filetype, &ncmds);
...
#if KXLD_USER_OR_ILP32
    case LC_SEGMENT: {
      struct segment_command *seg_hdr = (struct segment_command *)cmd_hdr;
  ...
  for (i = 0; i < ncmds; ++i, offset += cmd_hdr->cmdsize) {
    ...
    case LC_BUILD_VERSION:
      ...
      kxld_versionmin_init_from_build_cmd(&object->versionmin,
                                          build_version_hdr);
      break;
    ...
    case LC_DYSYMTAB:
      ...
      rval = kxld_reloc_create_macho(
          &object->extrelocs, &object->relocator,
          (struct relocation_info *)((void *)(my_file +
                                              object->dysymtab_hdr->extreloff)),
          object->dysymtab_hdr->nextrel);
      require_noerr(rval, finish);

      rval = kxld_reloc_create_macho(
          &object->locrelocs, &object->relocator,
          (struct relocation_info *)((void *)(my_file +
                                              object->dysymtab_hdr->locreloff)),
    ...
    case LC_MAIN:
      ...
      require_action(
          kxld_object_is_kernel(object), finish, rval = KERN_FAILURE;
    ...
    default:
      ...
      kxld_log(kKxldLogLinking, kKxldLogErr,
               kKxldLogMalformedMachO
               "Invalid load command type in MH_KEXT_BUNDLE kext: %u.",
               cmd_hdr->cmd);
      goto finish;
    }
    ...
    if (seg) {
      ...
      for (j = 0; j < seg->sects.nitems; ++j, ++secti) {
        ...
        KXLD_3264_FUNC(kxld_object_is_32_bit(object), rval,
                       kxld_sect_init_from_macho_32,
                       kxld_sect_init_from_macho_64, sect, my_file,
                       &sect_offset, secti, &object->relocator);
        require_noerr(rval, finish);
...
static kern_return_t init_from_execute(KXLDObject *object) {
  ...
  require_action(kernel_linkedit_seg, finish, rval = KERN_FAILURE; kxld_log(
                     kKxldLogLinking, kKxldLogErr, kKxldLogMalformedMachO));
#endif
...
/*******************************************************************************
*******************************************************************************/
static boolean_t target_supports_bundle(const KXLDObject *object __unused) {
  return TRUE;
}
...
static kern_return_t init_from_bundle(KXLDObject *object) {
  ...
  require_action(target_supports_bundle(object), finish, rval = KERN_FAILURE;
                 kxld_log(kKxldLogLinking, kKxldLogErr,
  ...
  require_action(filetype == MH_KEXT_BUNDLE, finish, rval = KERN_FAILURE);
...
static kern_return_t init_from_object(KXLDObject *object) {
  ...
  require_action(target_supports_object(object), finish, rval = KERN_FAILURE;
                 kxld_log(kKxldLogLinking, kKxldLogErr,
                          kKxldLogFiletypeNotSupported, MH_OBJECT));

  KXLD_3264_FUNC(kxld_object_is_32_bit(object), offset, get_macho_cmd_data_32,
                 get_macho_cmd_data_64, my_file, offset, &filetype, &ncmds);
  ...
  for (; i < ncmds; ++i, offset += cmd_hdr->cmdsize) {
    ...
    case LC_SEGMENT: {
      ...
      require_action(!has_segment, finish, rval = KERN_FAILURE; kxld_log(
          kKxldLogLinking, kKxldLogErr,
          kKxldLogMalformedMachO "Multiple segments in an MH_OBJECT kext."));
    ...
    case LC_SEGMENT_64: {
      ...
      require_action(
          !kxld_object_is_32_bit(object), finish, rval = KERN_FAILURE;
          kxld_log(kKxldLogLinking, kKxldLogErr,
                   kKxldLogMalformedMachO "LC_SEGMENT_64 in a 32-bit kext."));
      require_action(!has_segment, finish, rval = KERN_FAILURE; kxld_log(
          kKxldLogLinking, kKxldLogErr,
          kKxldLogMalformedMachO "Multiple segments in an MH_OBJECT kext."));
    ...
    case LC_SYMTAB:
      ...
      KXLD_3264_FUNC(
          kxld_object_is_32_bit(object), rval, kxld_symtab_init_from_macho_32,
          kxld_symtab_init_from_macho_64, object->symtab, symtab_hdr, my_file,
          /* kernel_linkedit_seg */ NULL);
      require_noerr(rval, finish);
      break;
    case LC_UUID:
    ...
    default:
      ...
      kxld_log(kKxldLogLinking, kKxldLogErr,
               kKxldLogMalformedMachO
               "Invalid load command type in MH_OBJECT kext: %u.",
               cmd_hdr->cmd);
      goto finish;
    }
  }
  ...
  if (has_segment) {
    ...
    rval = kxld_seg_finalize_object_segment(
        &object->segs, object->section_order, get_macho_header_size(object));
    require_noerr(rval, finish);
...
static u_long get_macho_cmd_data_32(u_char *file, u_long offset,
  ...
  struct mach_header *mach_hdr =
      (struct mach_header *)((void *)(file + offset));
...
static u_long get_macho_cmd_data_64(u_char *file, u_long offset,
                                    u_int *filetype, u_int *ncmds) {
  struct mach_header_64 *mach_hdr =
      (struct mach_header_64 *)((void *)(file + offset));
...
static u_long get_macho_header_size(const KXLDObject *object) {
  ...
  header_size += object_is_32_bit ? sizeof(struct mach_header)
...
/*******************************************************************************
*******************************************************************************/
static u_long get_macho_data_size(const KXLDObject *object) {
  KXLDSeg *seg = NULL;
  u_long data_size = 0;
  u_int i = 0;
...
#if KXLD_PIC_KEXTS
  {
     ...
     */
    symtab_size = kxld_symtab_get_macho_data_size(
        object->symtab, kxld_object_is_32_bit(object));

    if (target_supports_slideable_kexts(object)) {
      /* get size of __DYSYMTAB relocation entries */
      reloc_size = kxld_reloc_get_macho_data_size(&object->locrelocs,
                                                  &object->extrelocs);
    }
...
/*******************************************************************************
*******************************************************************************/
boolean_t kxld_object_target_needs_swap(const KXLDObject *object __unused) {
#if KERNEL
  return FALSE;
#else
...
/*******************************************************************************
*******************************************************************************/
KXLDSeg *kxld_object_get_seg_by_name(const KXLDObject *object,
                                     const char *segname) {
  KXLDSeg *seg = NULL;
  u_int i = 0;
...
/*******************************************************************************
*******************************************************************************/
const KXLDRelocator *kxld_object_get_relocator(const KXLDObject *object) {
  check(object);
...
/*******************************************************************************
*******************************************************************************/
KXLDSect *kxld_object_get_sect_by_name(const KXLDObject *object,
                                       const char *segname,
                                       const char *sectname) {
  KXLDSect *sect = NULL;
  u_int i = 0;
...
/*******************************************************************************
*******************************************************************************/
const KXLDReloc *kxld_object_get_reloc_at_symbol(const KXLDObject *object,
                                                 const KXLDSym *sym) {
  ...
  if (kxld_object_is_final_image(object)) {
    reloc = kxld_reloc_get_reloc_by_offset(&object->extrelocs, sym->base_addr);
    if (!reloc) {
      reloc =
          kxld_reloc_get_reloc_by_offset(&object->locrelocs, sym->base_addr);
    }
  } else {
...
/*******************************************************************************
*******************************************************************************/
const KXLDSym *kxld_object_get_symbol_of_reloc(const KXLDObject *object,
...
/*******************************************************************************
*******************************************************************************/
const KXLDSect *kxld_object_get_section_by_index(const KXLDObject *object,
                                                 u_int sectnum) {
  KXLDSect *sect = NULL;
...
/*******************************************************************************
*******************************************************************************/
const KXLDArray *kxld_object_get_extrelocs(const KXLDObject *object) {
...
/*******************************************************************************
*******************************************************************************/
const KXLDSymtab *kxld_object_get_symtab(const KXLDObject *object) {
  check(object);
...
static kern_return_t resolve_common_symbols(KXLDObject *object) {
  ...
  if (total_size) {
    ...
    if (sect) {
      ...
      kxld_sect_init_zerofill(sect, SEG_DATA, SECT_COMMON, total_size,
                              max_align);
    }
...
/*******************************************************************************
*******************************************************************************/
static boolean_t target_has_got(const KXLDObject *object) { return FALSE: }
...
static kern_return_t create_got(KXLDObject *object) {
  ...
  for (i = 0; i < object->sects.nitems; ++i) {
    sect = kxld_array_get_item(&object->sects, i);
    ngots += kxld_sect_get_ngots(sect, &object->relocator, object->symtab);
  }
...
static boolean_t target_supports_protected_segments(const KXLDObject *object) {
  return object->is_final_image &&
         (object->cputype == CPU_TYPE_X86_64 ||
          object->cputype == CPU_TYPE_ARM || object->cputype == CPU_TYPE_ARM64);
}
...
void kxld_object_get_vmsize(const KXLDObject *object, u_long *header_size,
  ...
  *header_size =
      (object->is_final_image)
          ? 0
          : (u_long)kxld_round_page_cross_safe(get_macho_header_size(object));
...
kern_return_t kxld_object_export_linked_object(const KXLDObject *object,
  ...
  rval = export_macho_header(object, my_linked_object, ncmds, &header_offset,
                             header_size);
  require_noerr(rval, finish);
  ...
  rval = kxld_symtab_export_macho(object->symtab, my_linked_object,
                                  &header_offset, header_size, &data_offset,
                                  size, is_32bit_object);
  require_noerr(rval, finish);
...
#if KXLD_PIC_KEXTS
  if (target_supports_slideable_kexts(object)) {
    rval = kxld_reloc_export_macho(
        &object->relocator, &object->locrelocs, &object->extrelocs,
        my_linked_object, &header_offset, header_size, &data_offset, size);
    require_noerr(rval, finish);
  }
#endif /* KXLD_PIC_KEXTS */

  if (object->uuid.has_uuid) {
    rval = kxld_uuid_export_macho(&object->uuid, my_linked_object,
                                  &header_offset, header_size);
    require_noerr(rval, finish);
  }

  if (object->versionmin.has_versionmin) {
    rval = kxld_versionmin_export_macho(&object->versionmin, my_linked_object,
                                        &header_offset, header_size);
    require_noerr(rval, finish);
  }

  if (object->srcversion.has_srcversion) {
    rval = kxld_srcversion_export_macho(&object->srcversion, my_linked_object,
                                        &header_offset, header_size);
    require_noerr(rval, finish);
  }

  if (isSplitKext && object->splitinfolc.has_splitinfolc) {
    rval = kxld_splitinfolc_export_macho(&object->splitinfolc, linked_object,
                                         &header_offset, header_size,
                                         &data_offset, size);
    require_noerr(rval, finish);
  }
...
static kern_return_t export_macho_header(const KXLDObject *object, u_char *buf,
  ...
  KXLD_3264_FUNC(kxld_object_is_32_bit(object), rval, export_macho_header_32,
                 export_macho_header_64, object, buf, ncmds, header_offset,
                 header_size);
  require_noerr(rval, finish);
...
static kern_return_t export_macho_header_64(const KXLDObject *object,
  ...
  *header_offset += sizeof(*mach);
  ...
  {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             " %p >>> Start of macho header (size %lu) <%s>", (void *)mach,
             sizeof(*mach), __func__);
    kxld_log(kKxldLogLinking, kKxldLogErr, " %p <<< End of macho header <%s>",
             (void *)((u_char *)mach + sizeof(*mach)), __func__);
  }
#endif
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_object_relocate(KXLDObject *object,
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_object_resolve_symbol(KXLDObject *object, const KXLDSym *sym,
                                         kxld_addr_t addr) {
  kern_return_t rval = KERN_FAILURE;
...
static kern_return_t process_symbol_pointers(KXLDObject *object) {
  ...
  require_action(object->is_final_image && object->dysymtab_hdr, finish,
                 rval = KERN_FAILURE);
  ...
  if (isOldInterface) {
    symidx = (int32_t *)((void *)(object->file +
                                  object->dysymtab_hdr->indirectsymoff));
  } else {
    symidx = (int32_t *)((void *)(object->split_info.kextExecutable +
                                  object->dysymtab_hdr->indirectsymoff));
  }
  ...
  for (i = 0; i < nsyms; ++i, ++symidx, symptr += symptrsize) {
    if (*symidx & INDIRECT_SYMBOL_LOCAL) {
      ...
      if (isOldInterface) {
        ...
        add_to_ptr(symptr, object->split_info.vmaddr_TEXT,
...
static kern_return_t process_relocs_from_tables(KXLDObject *object) {
  ...
  for (i = 0; i < object->extrelocs.nitems; ++i) {
    ...
    if (isOldInterface) {
      rval = kxld_relocator_process_table_reloc(&object->relocator, reloc, seg,
                                                object->link_addr);
    } else {
...
static kern_return_t populate_kmod_info(KXLDObject *object) {
  ...
  kmodsym = kxld_symtab_get_locally_defined_symbol_by_name(
      object->symtab, KXLD_KMOD_INFO_SYMBOL);
  ...
  if (kxld_object_is_32_bit(object)) {
    ...
    {
      kxld_log(kKxldLogLinking, kKxldLogErr,
               " kmodsect %p kmod_info %p = kmodsect->data %p + kmod_offset "
               "%lu <%s>",
               (void *)kmodsect, (void *)kmod_info, (void *)kmodsect->data,
               kmod_offset, __func__);

      kxld_log(kKxldLogLinking, kKxldLogErr,
               " kmod_info data: address %p size %llu hdr_size %llu start_addr "
               "%p stop_addr %p <%s>",
               (void *)kmod->address, kmod->size, kmod->hdr_size,
               (void *)kmod->start_addr, (void *)kmod->stop_addr, __func__);
    }
#endif
  }
