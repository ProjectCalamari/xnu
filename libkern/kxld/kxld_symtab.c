...
static kern_return_t init_syms_32(KXLDSymtab *symtab, u_char *macho,
                                  u_long offset, u_int nsyms);
#endif
#if KXLD_USER_OR_LP64
static kern_return_t init_syms_64(KXLDSymtab *symtab, u_char *macho,
                                  u_long offset, u_int nsyms);
#endif
...
/*******************************************************************************
*******************************************************************************/
size_t kxld_symtab_sizeof(void) { return sizeof(KXLDSymtab); }

#if KXLD_USER_OR_ILP32
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_symtab_init_from_macho_32(KXLDSymtab *symtab,
                                             struct symtab_command *src,
                                             u_char *macho,
                                             KXLDSeg *kernel_linkedit_seg) {
...
static kern_return_t init_macho(KXLDSymtab *symtab, struct symtab_command *src,
  ...
  KXLD_3264_FUNC(is_32_bit, rval, init_syms_32, init_syms_64, symtab,
                 macho_or_linkedit, symoff, src->nsyms);
  require_noerr(rval, finish);
...
static void restrict_private_symbols(KXLDSymtab *symtab) {
  const char *private_symbols[] = {
      KXLD_KMOD_INFO_SYMBOL, KXLD_OPERATOR_NEW_SYMBOL,
      KXLD_OPERATOR_NEW_ARRAY_SYMBOL, KXLD_OPERATOR_DELETE_SYMBOL,
...
kern_return_t kxld_symtab_get_sym_index(const KXLDSymtab *symtab,
                                        const KXLDSym *sym, u_int *symindex) {
  kern_return_t rval = KERN_FAILURE;
...
u_long kxld_symtab_get_macho_data_size(const KXLDSymtab *symtab,
  ...
  kxld_symtab_iterator_init(&iter, symtab, kxld_sym_is_defined_locally, FALSE);
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_symtab_export_macho(const KXLDSymtab *symtab, u_char *buf,
                                       u_long *header_offset,
                                       u_long header_size, u_long *data_offset,
  ...
  require_action(sizeof(*symtabhdr) <= header_size - *header_offset, finish,
                 rval = KERN_FAILURE);
  ...
  *header_offset += sizeof(*symtabhdr);
  ...
  symtabhdr->stroff =
      (uint32_t)(symtabhdr->symoff + (symtabhdr->nsyms * nlistsize));
  ...
  while ((sym = kxld_symtab_iterator_get_next(&iter))) {
    KXLD_3264_FUNC(is_32_bit, rval, kxld_sym_export_macho_32,
                   kxld_sym_export_macho_64, sym, nl, strtab, &stroff,
                   symtabhdr->strsize);
    require_noerr(rval, finish);
  ...
  *data_offset = (*data_offset + 7) & ~7;
...
#if SPLIT_KEXTS_DEBUG
  {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             " %p to %p (size %lu) symtabhdr <%s>", (void *)symtabhdr,
             (void *)((u_char *)symtabhdr + sizeof(*symtabhdr)),
             sizeof(*symtabhdr), __func__);

    kxld_log(
        kKxldLogLinking, kKxldLogErr,
        " symtabhdr %p cmdsize %u symoff %u nsyms %u stroff %u strsize %u <%s>",
        (void *)symtabhdr, symtabhdr->cmdsize, symtabhdr->symoff,
        symtabhdr->nsyms, symtabhdr->stroff, symtabhdr->strsize, __func__);
  }
#endif
