...
static KXLDObject *get_object_for_file(KXLDContext *context, u_char *file,
                                       u_long size, const char *name);
static kern_return_t allocate_split_kext(KXLDContext *context,
                                         splitKextLinkInfo *link_info);
static u_char *allocate_kext(KXLDContext *context, void *callback_data,
                             kxld_addr_t *vmaddr, u_long *vmsize,
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_create_context(KXLDContext **_context,
                                  KXLDAllocateCallback allocate_callback,
                                  KXLDLoggingCallback logging_callback,
                                  KXLDFlags flags, cpu_type_t cputype,
                                  cpu_subtype_t cpusubtype,
...
/*******************************************************************************
*******************************************************************************/
void kxld_destroy_context(KXLDContext *context) {
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_link_split_file(KXLDContext *context,
                                   splitKextLinkInfo *link_info,
                                   const char *name, void *callback_data,
                                   KXLDDependency *dependencies,
                                   u_int ndependencies,
  ...
  rval =
      init_kext_objects(context, link_info->kextExecutable, link_info->kextSize,
                        name, dependencies, ndependencies);
  require_noerr(rval, finish);

  kext_object = get_object_for_file(context, link_info->kextExecutable,
                                    link_info->kextSize, name);
  ...
  rval = kxld_kext_relocate(
      context->kext, (kxld_addr_t)my_link_info, &context->vtables_by_name,
      &context->defined_symbols_by_name, &context->obsolete_symbols_by_name,
      &context->defined_cxx_symbols_by_value);
  require_noerr(rval, finish);

  rval = kxld_kext_export_linked_object(context->kext, (void *)my_link_info,
                                        kmod_info_kern);
  require_noerr(rval, finish);
  ...
  if (kaslr_offsets_count != kaslr_offsets_index) {
    kxld_log(kKxldLogLinking, kKxldLogErr,
             "[ERROR] %s: KASLR pointers: count=%d, but only populated %d!",
             name, kaslr_offsets_count, kaslr_offsets_index);
    rval = KERN_FAILURE;
    goto finish;
  }
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_link_file(KXLDContext *context, u_char *file, u_long size,
                             const char *name, void *callback_data,
                             KXLDDependency *dependencies, u_int ndependencies,
  ...
  rval =
      init_kext_objects(context, file, size, name, dependencies, ndependencies);
  require_noerr(rval, finish);

  linked_object = allocate_kext(context, callback_data, &vmaddr, &vmsize,
                                &linked_object_alloc);
  require_action(linked_object, finish, rval = KERN_RESOURCE_SHORTAGE);

  rval = kxld_kext_relocate(context->kext, vmaddr, &context->vtables_by_name,
                            &context->defined_symbols_by_name,
                            &context->obsolete_symbols_by_name,
                            &context->defined_cxx_symbols_by_value);
  require_noerr(rval, finish);

  rval = kxld_kext_export_linked_object(context->kext, (void *)linked_object,
                                        kmod_info_kern);
  require_noerr(rval, finish);
...
static kern_return_t init_context(KXLDContext *context, u_int ndependencies) {
  ...
  rval = kxld_array_init(&context->objects, kxld_object_sizeof(),
                         2 * ndependencies + 1);
  require_noerr(rval, finish);

  rval = kxld_array_init(&context->dependencies, kxld_kext_sizeof(),
                         ndependencies);
  require_noerr(rval, finish);
...
static kern_return_t init_kext_objects(KXLDContext *context, u_char *file,
  ...
  for (i = 0; i < ndependencies; ++i) {
    ...
    kext_object = get_object_for_file(context, dependencies[i].kext,
                                      dependencies[i].kext_size,
                                      dependencies[i].kext_name);
    require_action(kext_object, finish, rval = KERN_FAILURE);

    if (dependencies[i].interface) {
      interface_object = get_object_for_file(context, dependencies[i].interface,
                                             dependencies[i].interface_size,
  ...
  for (i = 0; i < context->dependencies.nitems; ++i) {
    ...
    rval = kxld_kext_export_vtables(
        kext, &context->defined_cxx_symbols_by_value,
        &context->defined_symbols_by_name, &context->vtables_by_name);
    require_noerr(rval, finish);
  }
...
static KXLDObject *get_object_for_file(KXLDContext *context, u_char *file,
  ...
  for (i = 0; i < context->objects.nitems; ++i) {
    ...
    if (!kxld_object_get_file(object)) {
      result = kxld_object_init_from_macho(
          object, file, size, name, context->section_order, context->cputype,
          context->cpusubtype, context->flags);
