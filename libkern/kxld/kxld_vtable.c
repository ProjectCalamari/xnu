...
static kern_return_t init_by_relocs(KXLDVTable *vtable,
                                    const KXLDSym *vtable_sym,
                                    const KXLDSect *sect,
...
/*******************************************************************************
*******************************************************************************/
kern_return_t kxld_vtable_init(KXLDVTable *vtable, const KXLDSym *vtable_sym,
  ...
  vtable_sect = kxld_object_get_section_by_index(object, vtable_sym->sectnum);
  require_action(vtable_sect, finish, rval = KERN_FAILURE);

  vtable->name = vtable_sym->name;
  vtable->vtable =
      vtable_sect->data + kxld_sym_get_section_offset(vtable_sym, vtable_sect);
  ...
  if (kxld_object_is_linked(object)) {
    ...
    if (kxld_object_is_final_image(object)) {
      ...
      require_action(
          extrelocs, finish, rval = KERN_FAILURE; kxld_log(
              kKxldLogPatching, kKxldLogErr, kKxldLogMalformedVTable,
              kxld_demangle(vtable->name, &demangled_name, &demangled_length)));

      rval = init_by_entries_and_relocs(vtable, vtable_sym, relocator,
                                        extrelocs, defined_cxx_symbols);
      require_noerr(rval, finish);
    } else {
      require_action(kxld_sect_get_num_relocs(vtable_sect) > 0, finish,
                     ...
                     kxld_log(kKxldLogPatching, kKxldLogErr,
                              ...
                              kxld_demangle(vtable->name, &demangled_name,
                                            &demangled_length)));
...
static kern_return_t init_by_relocs(KXLDVTable *vtable,
  ...
  (void)get_vtable_base_sizes(relocator->is_32_bit, &vtable_entry_size,
                              &vtable_header_size);

  vtable_base_offset =
      kxld_sym_get_section_offset(vtable_sym, sect) + vtable_header_size;

  /* Find the relocation entry at the start of the vtable */

  rval = kxld_reloc_get_reloc_index_by_offset(&sect->relocs, vtable_base_offset,
                                              &base_reloc_index);
  require_noerr(rval, finish);
...
static kern_return_t init_by_entries(KXLDVTable *vtable,
  ...
  while (1) {
    entry_value = kxld_relocator_get_pointer_at_addr(relocator, vtable->vtable,
                                                     entry_offset);
    if (!entry_value) {
      break;
    }
  ...
  for (i = 0, entry_offset = vtable_header_size; i < vtable->entries.nitems;
...
init_by_entries_and_relocs(KXLDVTable *vtable, const KXLDSym *vtable_sym,
  ...
  while (1) {
    entry_value = kxld_relocator_get_pointer_at_addr(relocator, vtable->vtable,
                                                     entry_offset);
    if (!entry_value) {
      reloc = kxld_reloc_get_reloc_by_offset(relocs, vtable_sym->base_addr +
                                                         entry_offset);
      if (!reloc) {
        break;
      }
  ...
  for (i = 0, entry_offset = vtable_header_size; i < vtable->entries.nitems;
    ...
    if (entry_value) {
      ...
      require_action(reloc, finish, rval = KERN_FAILURE; kxld_log(
                         kKxldLogPatching, kKxldLogErr, kKxldLogMalformedVTable,
...
KXLDVTableEntry *kxld_vtable_get_entry_for_offset(const KXLDVTable *vtable,
  ...
  (void)get_vtable_base_sizes(is_32_bit, &vtable_entry_size,
                              &vtable_header_size);
...
kern_return_t kxld_vtable_patch(KXLDVTable *vtable,
  ...
  for (i = 0; i < super_vtable->entries.nitems; ++i) {
    ...
    if (streq(child_entry->unpatched.sym->name, parent_entry->patched.name)) {
      continue;
    }
    ...
    if (kxld_object_target_supports_strict_patching(object) &&
      ...
      rval = kxld_sym_get_class_name_from_vtable_name(vtable->name, class_name,
                                                      sizeof(class_name));
      require_noerr(rval, finish);

      function_prefix_len = kxld_sym_get_function_prefix_from_class_name(
          class_name, function_prefix, sizeof(function_prefix));
      require(function_prefix_len, finish);

      if (!strncmp(child_entry->unpatched.sym->name, function_prefix,
                   function_prefix_len)) {
        failure = TRUE;
        kxld_log(
            ...
            kxld_demangle(child_entry->unpatched.sym->name, &demangled_name2,
                          &demangled_length2));
        continue;
      }
    }
    ...
    sym = kxld_symtab_get_locally_defined_symbol_by_name(
        symtab, parent_entry->patched.name);
    if (!sym) {
