  ...
  if (s->status == INIT_STATE) {
    ...
    if (s->wrap == 2) {
      ...
      if (s->gzhead == NULL) {
        ...
        if (s->gzhead->hcrc)
          strm->adler = z_crc32(strm->adler, s->pending_buf, s->pending);
        s->gzindex = 0;
      ...
      while (s->gzindex < (s->gzhead->extra_len & 0xffff)) {
        if (s->pending == s->pending_buf_size) {
          if (s->gzhead->hcrc && s->pending > beg)
            strm->adler =
                z_crc32(strm->adler, s->pending_buf + beg, s->pending - beg);
          flush_pending(strm);
          beg = s->pending;
          if (s->pending == s->pending_buf_size)
            break;
...
uLong zlib_deflate_memory_size(int wbits, int memlevel) {
  return (31 + sizeof(deflate_state) + (1 << (wbits + 2)) +
          (1 << (memlevel + 9)));
}
