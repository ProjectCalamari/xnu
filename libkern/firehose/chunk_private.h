...
OS_ALWAYS_INLINE
static inline bool firehose_chunk_pos_fits(firehose_chunk_pos_u *pos,
                                           uint16_t size) {
  return pos->fcp_next_entry_offs + size <= pos->fcp_private_offs;
}

OS_ALWAYS_INLINE
static inline firehose_chunk_t firehose_chunk_for_address(void *addr) {
...
OS_ALWAYS_INLINE
static inline long firehose_chunk_tracepoint_try_reserve(
    firehose_chunk_t fc, uint64_t stamp, firehose_stream_t stream, uint8_t qos,
  ...
  os_atomic_rmw_loop(
      &fc->fc_pos.fcp_atomic_pos, orig.fcp_pos, pos.fcp_pos, relaxed, {
        if (orig.fcp_pos == 0) {
        ...
        if (!firehose_chunk_pos_fits(&orig, ft_size + pubsize + privsize) ||
            !stamp_delta_fits) {
          ...
          pos.fcp_pos +=
              roundup(ft_size + pubsize, 8) * FIREHOSE_CHUNK_POS_ENTRY_OFFS_INC;
          ...
          if (!firehose_chunk_pos_fits(
                  &pos, roundup(ft_size + minimum_payload_size, 8))) {
...
firehose_chunk_tracepoint_begin(firehose_chunk_t fc, uint64_t stamp,
                                uint16_t pubsize, uint64_t thread_id,
                                long offset) {
  firehose_tracepoint_t ft =
      (firehose_tracepoint_t)__builtin_assume_aligned(fc->fc_start + offset, 8);
...
OS_ALWAYS_INLINE
static inline bool
firehose_chunk_tracepoint_end(firehose_chunk_t fc, firehose_tracepoint_t ft,
  ...
  pos.fcp_pos = os_atomic_std(atomic_fetch_sub_explicit)(
      &fc->fc_pos.fcp_atomic_pos, FIREHOSE_CHUNK_POS_REFCNT_INC,
      os_atomic_std(memory_order_relaxed));
