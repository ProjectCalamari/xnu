...
static lck_mtx_t *sOSReportLock =
...
void OSReportWithBacktrace(const char *str, ...) {
  ...
  lck_mtx_lock(sOSReportLock);
  {
    ...
    printf("%s\nBacktrace 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx 0x%lx\n", buf,
           (unsigned long)VM_KERNEL_UNSLIDE(bt[2]),
           (unsigned long)VM_KERNEL_UNSLIDE(bt[3]),
           (unsigned long)VM_KERNEL_UNSLIDE(bt[4]),
           (unsigned long)VM_KERNEL_UNSLIDE(bt[5]),
           (unsigned long)VM_KERNEL_UNSLIDE(bt[6]),
           (unsigned long)VM_KERNEL_UNSLIDE(bt[7]),
...
static unsigned int x86_64_validate_raddr(vm_offset_t raddr) {
  return (raddr > VM_MIN_KERNEL_AND_KEXT_ADDRESS) &&
         (raddr < VM_MAX_KERNEL_ADDRESS);
}
static unsigned int x86_64_validate_stackptr(vm_offset_t stackptr) {
...
#error Unknown architecture.
  ...
  do {
    // check bounds
    if ((fp == 0) || (!OSBacktraceFrameAlignOK(fp)) ||
        (fp > VM_MAX_KERNEL_ADDRESS) || (fp < VM_MIN_KERNEL_AND_KEXT_ADDRESS)) {
      break;
    }
    // safely read frame
#ifdef __arm64__
    ...
    bt[i] = ptrauth_strip((void *)frameb[1],
                          ptrauth_key_return_address); // link register
#else
