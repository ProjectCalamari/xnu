...
#define ALIGN_TEST(p, t)                                                       \
  do {                                                                         \
    if ((uintptr_t)p & (sizeof(t) - 1))                                        \
      panic("Unaligned atomic pointer %p", p);                                 \
  } while (0)
#else
#define ALIGN_TEST(p, t)                                                       \
  do {                                                                         \
...
/*
 * atomic operations
 *	These are _the_ atomic operations, now implemented via compiler
 * built-ins. It is expected that this C implementation is a candidate for
 * Link-Time- Optimization inlining, whereas the assembler implementations they
 * replace were not.
 */
...
Boolean OSCompareAndSwap16(UInt16 oldValue, UInt16 newValue,
...
long OSAddAtomicLong(long theAmount, volatile long *address) {
...
static Boolean OSTestAndSetClear(UInt32 bit, bool wantSet,
