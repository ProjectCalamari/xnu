...
SECURITY_READ_ONLY_LATE(const apple_encrypted_archive_t *)
apple_encrypted_archive = NULL;
...
void apple_encrypted_archive_interface_register(
...
void apple_encrypted_archive_interface_set_registration_callback(
  ...
  if (callback && registration_callback) {
    panic("apple_encrypted_archive interface registration callback is already "
          "set");
  }
