...
/*********************************************************************
*********************************************************************/
static const char *__OSKextVersionStringForStage(OSKextVersion stage) {
  switch (stage) {
  default:
    OS_FALLTHROUGH;
  case kOSKextVersionStageInvalid:
    return NULL;
  case kOSKextVersionStageDevelopment:
    return "d";
  case kOSKextVersionStageAlpha:
    return "a";
  case kOSKextVersionStageBeta:
    return "b";
  case kOSKextVersionStageCandidate:
    return "f";
  case kOSKextVersionStageRelease:
    return "";
  }
}
...
finish:
  ...
  result = (vers_major * VERS_MAJOR_MULT) + (vers_minor * VERS_MINOR_MULT) +
           (vers_revision * VERS_REVISION_MULT) +
           (vers_stage * VERS_STAGE_MULT) + vers_stage_level;
...
Boolean OSKextVersionGetString(OSKextVersion aVersion, char *buffer,
  ...
  vers_revision = aVersion - ((vers_major * VERS_MAJOR_MULT) +
                              (vers_minor * VERS_MINOR_MULT));
  vers_revision /= VERS_REVISION_MULT;

  vers_stage = aVersion - ((vers_major * VERS_MAJOR_MULT) +
                           (vers_minor * VERS_MINOR_MULT) +
                           (vers_revision * VERS_REVISION_MULT));
  ...
  cpos++;
  cpos +=
      scnprintf(buffer + cpos, bufferLength - cpos, "%u", (uint32_t)vers_minor);
  ...
  if (vers_stage < kOSKextVersionStageRelease) {
    snprintf(buffer + cpos, bufferLength - cpos, "%u",
             (uint32_t)vers_stage_level);
  }
...
OSKextVersion OSKextParseVersionCFString(CFStringRef versionString) {
  ...
  if (CFStringGetCString(versionString, versBuffer, sizeof(versBuffer),
