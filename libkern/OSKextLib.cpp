...
OSReturn OSKextRetainKextWithLoadTag(uint32_t loadTag) {
  ...
  if (theKext) {
    ...
    OSKextLog(theKext, kOSKextLogDebugLevel | kOSKextLogKextBookkeepingFlag,
              "Kext %s (load tag %d) has been retained.",
              theKext->getIdentifierCString(), loadTag);

    /* Call this after so a log message about autounload comes second.
     */
    theKext->setAutounloadEnabled(true);
  } else {
    OSKextLog(theKext, kOSKextLogErrorLevel | kOSKextLogKextBookkeepingFlag,
...
OSReturn OSKextReleaseKextWithLoadTag(uint32_t loadTag) {
  ...
  // xxx - should I check that the refcount of the OSKext is above the lower
  // bound? xxx - do we want a OSKextGetRetainCountOfKextWithLoadTag()?
finish:
  return result;
}
#endif // __clang_analyzer__
...
OSReturn OSKextUnloadKextWithLoadTag(uint32_t loadTag) {
  return OSKext::removeKextWithLoadTag(
      loadTag,
...
OSReturn OSKextRequestResource(const char *kextIdentifier,
  ...
  return OSKext::requestResource(kextIdentifier, resourceName, callback,
                                 context, requestTagOut);
}
...
kext_request(host_priv_t hostPriv,
  ...
  *logDataLengthOut = 0;
  ...
  result = vm_map_copyout_size(kernel_map, &map_addr, (vm_map_copy_t)requestIn,
                               requestLengthIn);
  ...
  if (isMkext) {
    ...
    *op_result =
        OSKext::loadFromMkext((OSKextLogSpec)clientLogSpec, request,
                              requestLengthIn, &logData, &logDataLength);
     ...
     */
    *op_result = OSKext::handleRequest(
        hostPriv, (OSKextLogSpec)clientLogSpec, request, requestLengthIn,
        &response, &responseLength, &logData, &logDataLength);
  }
  ...
  if (response && responseLength > 0) {
    ...
    copyin_result =
        vm_map_copyin(kernel_map, CAST_USER_ADDR_T(response), responseLength,
  ...
  if (logData && logDataLength > 0) {
    ...
    copyin_result =
        vm_map_copyin(kernel_map, CAST_USER_ADDR_T(logData), logDataLength,
...
vm_map_t kext_get_vm_map(kmod_info_t *info) {
  ...
  if (PE_get_primary_kc_format(&kcformat) && kcformat == KCFormatFileset) {
    ...
    if ((info->address >= segPRELINKTEXTB) && (info->address < segLINKB)) {
      kext_map = kernel_map;
    } else {
      kext_map = g_kext_map;
    }
...
*********************************************************************/
void kmod_panic_dump(vm_offset_t *addr, unsigned int cnt) {
...
void telemetry_backtrace_add_kexts(char *buf, size_t buflen, uintptr_t *frames,
                                   uint32_t framecnt) {
  __block size_t pos = 0;

  OSKext::foreachKextInBacktrace(
      frames, framecnt, OSKext::kPrintKextsLock,
