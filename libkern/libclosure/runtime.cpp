...
KALLOC_TYPE_VAR_DEFINE(KT_BLOCK_LAYOUT, struct Block_layout, void *,
                       KT_DEFAULT);
KALLOC_TYPE_VAR_DEFINE(KT_BLOCK_BYREF, struct Block_byref, void *, KT_DEFAULT);

static inline struct Block_layout *block_layout_alloc(size_t size) {
  return (struct Block_layout *)kalloc_type_var_impl(
      KT_BLOCK_LAYOUT, size, Z_WAITOK_ZERO_NOFAIL, NULL);
}

static inline void block_layout_free(Block_layout *ptr, size_t size) {
  kfree_type_var_impl(KT_BLOCK_LAYOUT, ptr, size);
}

static inline struct Block_byref *block_byref_alloc(size_t size) {
  return (struct Block_byref *)kalloc_type_var_impl(KT_BLOCK_BYREF, size,
                                                    Z_WAITOK_ZERO_NOFAIL, NULL);
}

static inline void block_byref_free(Block_byref *ptr, size_t size) {
...
#else
#define OSAtomicCompareAndSwapLong(_Old, _New, _Ptr)                           \
  os_atomic_cmpxchg(_Ptr, _Old, _New, relaxed)
...
#endif
#if KERNEL
static inline void _Block_retain_object(const void *ptr __unused) {}

static inline void _Block_release_object(const void *ptr __unused) {}

static inline void _Block_destructInstance(const void *aBlock __unused) {}
...
template <class T> static T *unwrap_relative_pointer(int32_t &offset) {
  if (offset == 0) {
    return nullptr;
  }
...
// been copied to the heap, and if so, increment and return it. Otherwise we
// need to copy it and update the stack forwarding pointer
 ...
 *
 *
 *  A Block can reference four different kinds of things that require help when
 * the Block is copied to the heap. 1) C++ stack based objects 2) References to
 ...
 * _Block_object_assign.  The dispose helper has a call to the C++ destructor
 * for case 1 and a call into _Block_object_dispose for the rest.
 *
 *  The flags parameter of _Block_object_assign and _Block_object_dispose is set
 * to BLOCK_FIELD_IS_OBJECT (3), for the case of an Objective-C Object,
 * BLOCK_FIELD_IS_BLOCK (7), for the case of another Block, and
 * BLOCK_FIELD_IS_BYREF (8), for the case of a __block variable.
 *  If the __block variable is marked weak the compiler also or's in
 * BLOCK_FIELD_IS_WEAK (16)
 *
 *  So the Block copy/dispose helpers should only ever generate the four flag
 * values of 3, 7, 8, and 24.
 *
 ...
 * one though!) and the dispose helper will do the destructor.  And similarly
 * the helpers will call into the same two support functions with the same
 * values for objects and Blocks with the additional BLOCK_BYREF_CALLER (128)
...
// this entry point to help dispose of the contents
...
__attribute__((used)) static int let_there_be_data = 42;
