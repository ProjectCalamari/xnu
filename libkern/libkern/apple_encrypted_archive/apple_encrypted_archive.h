...
typedef ssize_t (*apple_encrypted_archive_pwrite_proc)(void *arg,
                                                       const void *buf,
                                                       size_t nbyte,
                                                       off_t offset);

typedef ssize_t (*apple_encrypted_archive_pread_proc)(void *arg, void *buf,
                                                      size_t nbyte,
                                                      off_t offset);
 ...
 * @param state_size Size allocated in \p state, must be at least
 * apple_encrypted_archive_get_state_size()
 * @param recipient_public_key x9.63 encoded public key, must be on the P256
 * elliptic curve
...
typedef int (*apple_encrypted_archive_initialize_state)(
    void *state, size_t state_size, const uint8_t *recipient_public_key,
    size_t recipient_public_key_size);

/**
 * @abstract Open encryption stream
 *
 * @discussion State must have been initialized with
 * apple_encrypted_archive_initialize_state()
 *
 * @param state Encryption state buffer, \p state_size bytes
 * @param state_size Size allocated in \p state, must be at least
 * apple_encrypted_archive_get_state_size()
 * @param callback_arg Value passed as first argument to the pwrite/pread
 * callbacks
...
typedef int (*apple_encrypted_archive_open)(
    void *state, size_t state_size, void *callback_arg,
 ...
 *
 * @return Number of bytes written on success, and a negative error code on
 * failure
 */
typedef ssize_t (*apple_encrypted_archive_write)(void *state, size_t state_size,
 ...
 * @return 0 on success, and a negative error code on failure
 */
typedef int (*apple_encrypted_archive_close)(void *state, size_t state_size);
...
/**
 * @abstract Registers the AppleEncryptedArchive kext interface for use within
 * the kernel proper.
 *
 ...
 */
OS_EXPORT OS_NONNULL1 void apple_encrypted_archive_interface_register(
    const apple_encrypted_archive_t *aea);
