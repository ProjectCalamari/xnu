...
/* digests */
typedef void (*ccdigest_init_fn_t)(const struct ccdigest_info *di,
                                   ccdigest_ctx_t ctx);
typedef void (*ccdigest_update_fn_t)(const struct ccdigest_info *di,
                                     ccdigest_ctx_t ctx, unsigned long len,
                                     const void *data);
typedef void (*ccdigest_final_fn_t)(const struct ccdigest_info *di,
...
/* hmac */
typedef void (*cchmac_init_fn_t)(const struct ccdigest_info *di,
                                 cchmac_ctx_t ctx, unsigned long key_len,
                                 const void *key);
typedef void (*cchmac_update_fn_t)(const struct ccdigest_info *di,
                                   cchmac_ctx_t ctx, unsigned long data_len,
                                   const void *data);
typedef void (*cchmac_final_fn_t)(const struct ccdigest_info *di,
                                  cchmac_ctx_t ctx, unsigned char *mac);

typedef void (*cchmac_fn_t)(const struct ccdigest_info *di,
                            unsigned long key_len, const void *key,
                            unsigned long data_len, const void *data,
                            unsigned char *mac);

/* gcm */
typedef int (*ccgcm_init_with_iv_fn_t)(const struct ccmode_gcm *mode,
                                       ccgcm_ctx *ctx, size_t key_nbytes,
                                       const void *key, const void *iv);
typedef int (*ccgcm_inc_iv_fn_t)(const struct ccmode_gcm *mode, ccgcm_ctx *ctx,
                                 void *iv);

typedef const struct ccchacha20poly1305_fns {
  const struct ccchacha20poly1305_info *(*info)(void);
  int (*init)(const struct ccchacha20poly1305_info *info,
              ccchacha20poly1305_ctx *ctx, const uint8_t *key);
  int (*reset)(const struct ccchacha20poly1305_info *info,
               ccchacha20poly1305_ctx *ctx);
  int (*setnonce)(const struct ccchacha20poly1305_info *info,
                  ccchacha20poly1305_ctx *ctx, const uint8_t *nonce);
  int (*incnonce)(const struct ccchacha20poly1305_info *info,
                  ccchacha20poly1305_ctx *ctx, uint8_t *nonce);
  int (*aad)(const struct ccchacha20poly1305_info *info,
             ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *aad);
  int (*encrypt)(const struct ccchacha20poly1305_info *info,
                 ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *ptext,
                 void *ctext);
  int (*finalize)(const struct ccchacha20poly1305_info *info,
                  ccchacha20poly1305_ctx *ctx, uint8_t *tag);
  int (*decrypt)(const struct ccchacha20poly1305_info *info,
                 ccchacha20poly1305_ctx *ctx, size_t nbytes, const void *ctext,
                 void *ptext);
  int (*verify)(const struct ccchacha20poly1305_info *info,
                ccchacha20poly1305_ctx *ctx, const uint8_t *tag);
} *ccchacha20poly1305_fns_t;

/* pbkdf2 */
typedef void (*ccpbkdf2_hmac_fn_t)(const struct ccdigest_info *di,
                                   unsigned long passwordLen,
                                   const void *password, unsigned long saltLen,
                                   const void *salt, unsigned long iterations,
...
typedef size_t (*ccpad_cts3_crypt_fn_t)(const struct ccmode_cbc *cbc,
                                        cccbc_ctx *cbc_key, cccbc_iv *iv,
...
/* rsa */
typedef int (*ccrsa_make_pub_fn_t)(ccrsa_pub_ctx_t pubk, size_t exp_nbytes,
                                   const uint8_t *exp, size_t mod_nbytes,
                                   const uint8_t *mod);

typedef int (*ccrsa_verify_pkcs1v15_fn_t)(ccrsa_pub_ctx_t key,
                                          const uint8_t *oid, size_t digest_len,
                                          const uint8_t *digest, size_t sig_len,
                                          const uint8_t *sig, bool *valid);

__enum_decl(crypto_digest_alg_t, unsigned int,
            {CRYPTO_DIGEST_ALG_NONE, CRYPTO_DIGEST_ALG_MD5,
             CRYPTO_DIGEST_ALG_SHA1, CRYPTO_DIGEST_ALG_SHA256,
             CRYPTO_DIGEST_ALG_SHA384, CRYPTO_DIGEST_ALG_SHA512});

typedef size_t (*crypto_digest_ctx_size_fn_t)(crypto_digest_alg_t alg);

typedef void (*crypto_digest_init_fn_t)(crypto_digest_alg_t alg, void *ctx,
                                        size_t ctx_size);

typedef void (*crypto_digest_update_fn_t)(crypto_digest_alg_t alg, void *ctx,
                                          size_t ctx_size, const void *data,
                                          size_t data_size);

typedef void (*crypto_digest_final_fn_t)(crypto_digest_alg_t alg, void *ctx,
                                         size_t ctx_size, void *digest,
                                         size_t digest_size);

typedef void (*crypto_digest_fn_t)(crypto_digest_alg_t alg, const void *data,
                                   size_t data_size, void *digest,
                                   size_t digest_size);

typedef size_t (*crypto_hmac_ctx_size_fn_t)(crypto_digest_alg_t alg);

typedef void (*crypto_hmac_init_fn_t)(crypto_digest_alg_t alg, void *ctx,
                                      size_t ctx_size, const void *key,
                                      size_t key_size);

typedef void (*crypto_hmac_update_fn_t)(crypto_digest_alg_t alg, void *ctx,
                                        size_t ctx_size, const void *data,
                                        size_t data_size);

typedef void (*crypto_hmac_final_generate_fn_t)(crypto_digest_alg_t alg,
                                                void *ctx, size_t ctx_size,
                                                void *tag, size_t tag_size);

typedef bool (*crypto_hmac_final_verify_fn_t)(crypto_digest_alg_t alg,
                                              void *ctx, size_t ctx_size,
                                              const void *tag, size_t tag_size);

typedef void (*crypto_hmac_generate_fn_t)(crypto_digest_alg_t alg,
                                          const void *key, size_t key_size,
                                          const void *data, size_t data_size,
                                          void *tag, size_t tag_size);

typedef bool (*crypto_hmac_verify_fn_t)(crypto_digest_alg_t alg,
                                        const void *key, size_t key_size,
                                        const void *data, size_t data_size,
                                        const void *tag, size_t tag_size);
