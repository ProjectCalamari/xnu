 ...
 * The first argument is the event name. This will be prefixed with
 * "com.apple.xnu.". The following arguments define the event type. Fields are
 * defined as a type followed by a name. For example, say we've defined an event
 * called "com.apple.xnu.my_event". It has the following fields count Integer
 ...
 *
 * This defines a struct that we can use to create the event. To get the
 * struct's typename, use the CA_EVENT_TYPE macro. To send the struct, it needs
 * to be allocated via the CoreAnalytics subsystem. So to create and send
 * "my_event" we can do: ca_event_t event = CA_EVENT_ALLOCATE(my_event);
 ...
 * CA_EVENT_SEND(event);
 *
 * Note that CA_EVENT_SEND just enqueues the event on a lock free queue.
 * Serializing & sending the event to userspace happens on another thread.
 */

#define CA_EVENT(name, ...)                                                    \
  const char *_CA_EVENT_NAME_PREFIX(name) =                                    \
      _CA_EVENT_ORG #name "\0" _CA_FOREACH(_CA_STRINGIFY, _CA_NULL_TERMINATOR, \
                                           _CA_STRINGIFY, ##__VA_ARGS__) "\0"; \
  _Static_assert(sizeof(_CA_EVENT_ORG #name) <= 64,                            \
                 "CoreAnalytics event name ('" _CA_EVENT_ORG #name             \
                 "') is too long");                                            \
  CA_EVENT_TYPE(name) {                                                        \
    _CA_FOREACH(_CA_TYPE_DECLARATION, _CA_NULL_EPSILON,                        \
                _CA_VARIABLE_DECLARATION, ##__VA_ARGS__)                       \
  }                                                                            \
 ...
 * May block (use CA_EVENT_ALLOCATE_FLAGS with Z_NOWAIT for an allocation which
 * can't block but may fail).
 */
#define CA_EVENT_ALLOCATE(name)                                                \
  core_analytics_allocate_event(                                               \
      sizeof(CA_EVENT_TYPE(name)), _CA_EVENT_NAME_PREFIX(name),                \
...
#define CA_EVENT_ALLOCATE_FLAGS(name, flags)                                   \
  core_analytics_allocate_event(sizeof(CA_EVENT_TYPE(name)),                   \
                                _CA_EVENT_NAME_PREFIX(name), flags)
...
#define CA_EVENT_DEALLOCATE(event)                                             \
  ({                                                                           \
    kfree_data(event->data, core_analytics_event_size(event->format_str));     \
...
/*
 * Same as CA_EVENT_SEND but the caller takes responsibility for disabling
 * preemption.
 */
#define CA_EVENT_SEND_PREEMPTION_DISABLED(event)                               \
  core_analytics_send_event_preemption_disabled(event)
