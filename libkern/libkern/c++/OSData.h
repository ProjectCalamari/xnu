...
public:
   ...
   * for which a nonzero initial capacity is a hard limit).
   */
  static OSPtr<OSData> withBytes(const void *bytes,
   ...
   * for which a nonzero initial capacity is a hard limit).
   */
  template <typename T> static OSPtr<OSData> withValue(const T &value) {
   ...
   */
  static OSPtr<OSData> withBytesNoCopy(void *bytes, unsigned int numBytes);
   ...
   * modify bytes within the shared buffer.
   */
  template <typename T> static OSPtr<OSData> withValueNoCopy(T &value) {
   ...
   * for which a nonzero initial capacity is a hard limit).
   */
  static OSPtr<OSData> withData(const OSData *inData, unsigned int start,
   ...
   * for which a nonzero initial capacity is a hard limit).
   */
  template <typename T> bool initWithValue(const T &value) {
    validateValueType<T, kValueCopy>();
   ...
   * modify bytes within the shared buffer.
   */
  template <typename T> bool initWithValueNoCopy(T &value) {
    validateValueType<T, kValueNoCopy>();
  ...
  /* rvalue overload is deleted for the NoCopy variation to
   * disallow holding a dangling pointer to a temporary value */
  template <typename T> bool initWithValueNoCopy(T &&value) = delete;
#endif
#endif // KERNEL_PRIVATE
   ...
   * for which a nonzero initial capacity is a hard limit).
   */
  virtual bool initWithData(const OSData *inData, unsigned int start,
                            unsigned int numBytes);
   ...
   *
   * @abstract
   * Returns the total number of bytes the OSData can store without
   * reallocating.
   *
   * @result
   ...
   * to be appended.
   */
  template <typename T> bool appendValue(const T &value) {
    validateValueType<T, kValueCopy>();
   ...
   */
  __xnu_returns_data_pointer virtual const void *getBytesNoCopy() const;
   ...
   * rendering invalid an extrated pointer to that storage.
   */
  __xnu_returns_data_pointer virtual const void *
  getBytesNoCopy(unsigned int start, unsigned int numBytes) const;
...
private:
  enum ValueAcquisition { kValueCopy, kValueNoCopy };

#if KERNEL_PRIVATE
#if __cplusplus >= 201103L
  template <typename T, ValueAcquisition acquisition>
  static constexpr int validateValueType() {
    static_assert(sizeof(T) <= size_t(UINT32_MAX),
                  "value type's size is too large");
    ...
    static_assert(!XNU_HAS_KALLOC_TYPE_SUMMARY || KALLOC_TYPE_IS_DATA_ONLY(T) ||
                      (acquisition == kValueNoCopy),
                  "only plain data types can be contained (consider using "
                  "OSValueObject)");
    return 0; // C++11 does not support void-returning constexpr functions
  }
#else
  template <typename T, ValueAcquisition> static void validateValueType() {}
#endif
#endif // KERNEL_PRIVATE
