...
kern_return_t OSRuntimeInitializeCPP(OSKext *kext);
kern_return_t OSRuntimeFinalizeCPP(OSKext *kext);
void OSRuntimeUnloadCPPForSegment(kernel_segment_command_t *segment);
void OSRuntimeSignStructors(kernel_mach_header_t *header);
void OSRuntimeSignStructorsInFileset(kernel_mach_header_t *fileset_header);

kern_return_t is_io_catalog_send_data(mach_port_t masterPort, uint32_t flag,
                                      io_buf_ptr_t inData,
...
public:
  static OSPtr<OSKextSavedMutableSegment>
  withSegment(kernel_segment_command_t *seg);
...
/**************************************/
  ...
  friend int OSKextGrabPgoData(uuid_t uuid, uint64_t *pSize, char *pBuffer,
                               uint64_t bufferSize, int wait_for_unload,
                               int metadata);

#ifdef XNU_KERNEL_PRIVATE
  friend void OSKextVLog(OSKext *aKext, OSKextLogSpec msgLogSpec,
                         const char *format, va_list srcArgList)
      __printflike(3, 0);
  ...
  kext_request(host_priv_t hostPriv,
  ...
  friend kxld_addr_t kern_allocate(u_long size, KXLDAllocateFlags *flags,
                                   void *user_data);

  friend void kxld_log_shim(KXLDLogSubsystem subsystem, KXLDLogLevel level,
                            const char *format, va_list argList,
                            void *user_data);

  friend void _OSKextConsiderUnloads(__unused thread_call_param_t p0,
                                     __unused thread_call_param_t p1);

  friend kern_return_t OSRuntimeInitializeCPP(OSKext *kext);
  friend kern_return_t OSRuntimeFinalizeCPP(OSKext *kext);
  friend void OSRuntimeUnloadCPPForSegment(kernel_segment_command_t *segment);

  friend kern_return_t is_io_catalog_send_data(
      mach_port_t masterPort, uint32_t flag, io_buf_ptr_t inData,
      mach_msg_type_number_t inDataCount, kern_return_t *result);
...
public:
  ...
  static void reportOSMetaClassInstances(const char *kextIdentifier,
...
private:
  ...
  /* Instance life cycle.
   */
  static OSData *parseDextUniqueID(OSDictionary *anInfoDict,
                                   const char *dextIDCS);
  static void setDextUniqueIDInPersonalities(OSDictionary *anInfoDict,
                                             OSData *dextUniqueID);

  static OSPtr<OSKext> withBooterData(OSString *deviceTreeName,
                                      OSData *booterData);
  virtual bool initWithBooterData(OSString *deviceTreeName, OSData *booterData);

  static OSPtr<OSKext> withPrelinkedInfoDict(OSDictionary *infoDict,
                                             bool doCoalesedSlides,
                                             kc_kind_t type);
  virtual bool initWithPrelinkedInfoDict(OSDictionary *infoDict,
                                         bool doCoalesedSlides, kc_kind_t type);
  static OSSharedPtr<OSKext> withCodelessInfo(OSDictionary *infoDict,
                                              OSKextInitResult *result);

  virtual OSKextInitResult initWithCodelessInfo(OSDictionary *infoDict);

  static void setAllVMAttributes(void);

  virtual bool setInfoDictionaryAndPath(OSDictionary *aDictionary,
                                        OSString *aPath);
  virtual bool setExecutable(OSData *anExecutable, OSData *externalData = NULL,
  ...
  static OSReturn
  removeKext(OSKext *aKext,
             bool terminateServicesAndRemovePersonalitiesFlag = false);

  virtual bool isInExcludeList(void);
  virtual bool isLoadable(void);

  static OSKext *allocAndInitFakeKext(kmod_info_t *kmod_info);

/* Mkexts.
 */
#if CONFIG_KXLD
  static OSPtr<OSKext> withMkext2Info(OSDictionary *anInfoDict,
                                      OSData *mkextData);
  virtual bool initWithMkext2Info(OSDictionary *anInfoDict, OSData *mkextData);

  static OSReturn readMkextArchive(OSData *mkextData,
                                   uint32_t *checksumPtr = NULL);
  static OSReturn readMkext2Archive(OSData *mkextData,
  ...
  virtual OSPtr<OSData> createMkext2FileEntry(OSData *mkextData,
                                              OSNumber *offsetNum,
                                              const char *entryName);
  virtual OSPtr<OSData> extractMkext2FileData(UInt8 *data, const char *name,
  ...
  /* Dependencies.
   */
  virtual bool resolveDependencies(OSArray *loopStack = NULL); // priv/prot
   ...
   */
  static OSReturn loadFromMkext(OSKextLogSpec clientLogSpec, char *mkextBuffer,
                                uint32_t mkextBufferLength, char **logInfoOut,
                                uint32_t *logInfoLengthOut);
  static OSReturn handleRequest(host_priv_t hostPriv,
  ...
  static OSReturn loadCodelessKext(OSString *kextIdentifier,
                                   OSDictionary *requestDict);
  static OSReturn serializeLogInfo(OSArray *logInfoArray, char **logInfoOut,
                                   uint32_t *logInfoLengthOut);
  ...
  virtual OSReturn load(OSKextExcludeLevel startOpt = kOSKextExcludeNone,
                        OSKextExcludeLevel startMatchingOpt = kOSKextExcludeAll,
                        OSArray *personalityNames = NULL); // priv/prot
  virtual OSReturn unload(void);
  static OSReturn queueKextNotification(const char *notificationName,
                                        OSString *kextIdentifier,
                                        OSData *dextUniqueIdentifier);

  static void recordIdentifierRequest(OSString *kextIdentifier);
  ...
  static OSPtr<OSArray>
  copyAllKextPersonalities(bool filterSafeBootFlag = false);
  ...
  virtual OSReturn sendPersonalitiesToCatalog(bool startMatching = false,
                                              OSArray *personalityNames = NULL);

  static bool canUnloadKextWithIdentifier(OSString *kextIdentifier,
   ...
   */
  static OSPtr<OSDictionary> copyLoadedKextInfo(OSArray *kextIdentifiers = NULL,
                                                OSArray *keys = NULL);
  static OSPtr<OSDictionary>
  copyLoadedKextInfoByUUID(OSArray *kextIdentifiers = NULL,
                           OSArray *keys = NULL);
  static OSPtr<OSDictionary> copyKextCollectionInfo(OSDictionary *requestDict,
                                                    OSArray *infoKeys = NULL);
  static OSPtr<OSData> copyKextUUIDForAddress(OSNumber *address = NULL);
  static OSPtr<OSArray> copyDextsInfo(OSArray *kextIdentifiers = NULL,
                                      OSArray *keys = NULL);
  virtual OSPtr<OSDictionary> copyInfo(OSArray *keys = NULL);

  /* Logging to user space.
   */
  static OSKextLogSpec setUserSpaceLogFilter(OSKextLogSpec userLogSpec,
  ...
  virtual void reportOSMetaClassInstances(OSKextLogSpec msgLogSpec);
  ...
  static OSReturn mapKCFileSet(void *control, vm_size_t fsize,
                               kernel_mach_header_t **mh, off_t file_offset,
                               uintptr_t *slide, bool pageable,
                               void *map_entry_buffer);
  static OSReturn protectKCFileSet(kernel_mach_header_t *mh, kc_kind_t type);
  static OSReturn mapKCTextSegment(void *control, kernel_mach_header_t **mhp,
                                   off_t file_offset, uintptr_t *slide,
                                   void *map_entry_list);
  ...
  static OSReturn validateKCFileSetUUID(OSDictionary *infoDict, kc_kind_t type);

  static OSReturn validateKCUUIDfromPrelinkInfo(uuid_t *loaded_kcuuid,
                                                kc_kind_t type,
                                                OSDictionary *infoDict,
                                                const char *uuid_key);
  ...
  static OSReturn
  dequeueCallbackForRequestTag(OSKextRequestTag requestTag,
                               OSSharedPtr<OSDictionary> &callbackRecordOut);
  static OSReturn
  dequeueCallbackForRequestTag(OSNumber *requestTagNum,
                               OSSharedPtr<OSDictionary> &callbackRecordOut);

  static void invokeRequestCallback(OSDictionary *callbackRecord,
                                    OSReturn requestResult);
  virtual void invokeOrCancelRequestCallbacks(OSReturn callbackResult,
  ...
  static bool upgradeDext(OSKext *olddext, OSKext *newdext);
  static bool removeDext(OSKext *dext);
  static void replaceDextInternal(OSKext *olddext, OSKext *newdext);
  /* panic() support.
   */
public:
  enum {
  ...
  static void printKextsInBacktrace(vm_offset_t *addr, unsigned int cnt,
...
private:
  ...
  static kern_return_t summaryForAddressExt(const void *addr,
                                            OSKextLoadedKextSummary *summary);
  static void *kextForAddress(const void *addr);
  static boolean_t summaryIsInBacktrace(OSKextLoadedKextSummary *summary,
                                        vm_offset_t *addr, unsigned int cnt);
  static void printSummary(OSKextLoadedKextSummary *summary,
                           int (*printf_func)(const char *fmt, ...),
                           uint32_t flags);

  static int saveLoadedKextPanicListTyped(const char *prefix, int invertFlag,
...
public:
  ...
  static OSPtr<OSKext> lookupDextWithIdentifier(OSString *dextIdentifier,
                                                OSData *dextUniqueIdentifier);
  ...
  static OSReturn loadKextWithIdentifier(
      const char *kextIdentifier, Boolean allowDeferFlag = true,
  ...
  static OSReturn loadKextWithIdentifier(
      ...
      Boolean allowDeferFlag = true, Boolean delayAutounloadFlag = false,
      OSKextExcludeLevel startOpt = kOSKextExcludeNone,
      OSKextExcludeLevel startMatchingOpt = kOSKextExcludeAll,
      OSArray *personalityNames = NULL);

  static OSReturn loadKextWithIdentifier(
      OSString *kextIdentifier, OSSharedPtr<OSObject> &kextRef,
      Boolean allowDeferFlag = true, Boolean delayAutounloadFlag = false,
      OSKextExcludeLevel startOpt = kOSKextExcludeNone,
      OSKextExcludeLevel startMatchingOpt = kOSKextExcludeAll,
      OSArray *personalityNames = NULL);
  ...
  static void dropMatchingReferences(OSSet *kexts);
  ...
  static OSReturn removeKextWithLoadTag(
      ...
      bool terminateServicesAndRemovePersonalitiesFlag = false);
  static OSReturn
  requestDaemonLaunch(OSString *kextIdentifier, OSString *serverName,
                      OSNumber *serverTag, OSBoolean *reslide,
                      class IOUserServerCheckInToken *checkInToken,
                      OSData *serverDUI);
  static OSReturn notifyDextUpgrade(OSString *kextIdentifier,
                                    OSData *dextUniqueIdentifier);
  static OSReturn requestResource(const char *kextIdentifier,
                                  const char *resourceName,
                                  OSKextRequestResourceCallback callback,
                                  void *context,
                                  OSKextRequestTag *requestTagOut);
  static OSReturn cancelRequest(OSKextRequestTag requestTag, void **contextOut);
  ...
  static void
  createExcludeListFromBooterData(OSDictionary *theDictionary,
  ...
  static bool copyUserExecutablePath(const OSSymbol *bundleID, char *pathResult,
                                     size_t pathSize);
  virtual void setDriverKitUUID(LIBKERN_CONSUMED OSData *uuid);
  static bool incrementDextLaunchCount(OSKext *dext,
   ...
   * not all personalities defined by the kext (IOPersonalityPublisher or
   * CFBundleIdentifier).
   */
  virtual void removePersonalitiesFromCatalog(void);
  /*
   * This removes the personalities naming the kext (by CFBundleIdentifier), and
   * atomically adds the new personalities upgradedPersonalities.
   */
  virtual void updatePersonalitiesInCatalog(OSArray *upgradedPersonalities);

  /* Converts common string-valued properties to OSSymbols for lower memory
   * consumption.
   */
  static void uniquePersonalityProperties(OSDictionary *personalityDict);
#ifdef XNU_KERNEL_PRIVATE
  static void
  uniquePersonalityProperties(OSDictionary *personalityDict,
  ...
  const char *getKCTypeString(void) {
    switch (kc_type) {
    case KCKindPrimary:
