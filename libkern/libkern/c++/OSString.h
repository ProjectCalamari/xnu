...
protected:
  unsigned int flags : 14, length : 18;
...
public:
   ...
   * When the caller determines that the OSString object has actually been
   * freed, it can safely dispose of the data buffer. Conversely, if it frees
   * the shared data buffer, it must not attempt to use the OSString object and
   * should release it.
   *
   ...
   *
   * @param cString   The C string to copy into the new OSString.
   * @param length    Number of characters to copy from cString. If the actual
   * length of the C string is less than this length, then the length of the
   * resulting OSString will be the same as that of the C cstring.
   *
   * @result
   * An instance of OSString representing
   ...
   *
   * @result
   * <code>true</code> if the two objects are equivalent, <code>false</code>
   * otherwise.
   *
   * @discussion
