...
__enum_closed_decl(
    OSCollectionIteratorStorageType, uint8_t,
    {
        OSCollectionIteratorStorageUnallocated = 0,
        OSCollectionIteratorStorageInvalid = 1,
        OSCollectionIteratorStorageInline = 2,
        OSCollectionIteratorStoragePointer =
            3, /* two bits set, matches top bits of kernel pointer */
    });
...
private:
  ...
  /*
   * Set the storage type. The expectation is that this is called AFTER the
   * conditions for the storage type have been set. For example, collIterator
   * must be set to a valid pointer before calling
