...
struct IOKit_allocator {
  static void *allocate(size_t bytes) { return IOMalloc(bytes); }

  static void *allocate_zero(size_t bytes) { return IOMallocZero(bytes); }

  static void deallocate(void *p, size_t bytes) { IOFree(p, bytes); }
};

#ifdef KERNEL_PRIVATE
struct IOKit_data_allocator {
  static void *allocate(size_t bytes) { return IOMallocData(bytes); }

  static void *allocate_zero(size_t bytes) { return IOMallocZeroData(bytes); }

  static void deallocate(void *p, size_t bytes) { IOFreeData(p, bytes); }
};
...
template <typename T> struct IOKit_typed_allocator<T, false> {
  static inline kalloc_type_var_view_t kt_view(void) {
  ...
  static void *allocate_zero(size_t bytes) {
    return IOMallocTypeVarImpl(kt_view(), bytes);
  }
...
template <typename T,
          ...
          >
using OSAllocation =
    libkern::safe_allocation<T, Allocator, os_detail::panic_trapping_policy>;

#ifdef KERNEL_PRIVATE
// obsolete: just use the determination that OSAllocation<> does already.
//           (this works around incorrect adoptions like 104478984).
template <typename T> using OSDataAllocation = OSAllocation<T>;
#endif
