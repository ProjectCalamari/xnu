...
public:
   ...
   * for which the initial capacity is a hard limit).
   *
   * The objects in <code>objects</code> are retained for storage in the new
   * set, not copied.
   */
  static OSPtr<OSSet> withObjects(const OSObject *objects[], unsigned int count,
   ...
   * equal to the number of objects in <code>array</code>.
   * @result
   ...
   * not copied.
   */
  static OSPtr<OSSet> withArray(const OSArray *array,
                                unsigned int capacity = 0);
   ...
   * not copied.
   */
  static OSPtr<OSSet> withSet(const OSSet *set, unsigned int capacity = 0);
   ...
   * <code>count</code>. The new array will grow as needed to accommodate more
   * key-object pairs
   * (<i>unlike</i> @link //apple_ref/doc/uid/20001503 CFMutableSet@/link,
   * for which the initial capacity is a hard limit).
   *
   ...
   *                  otherwise <code>capacity</code> must be greater than or
   * equal to the number of objects in <code>set</code>.
   *
   * @result
   * <code>true</code> on success, <code>false</code> on failure.
   *
   ...
   * This is no longer the case, so this function correctly returns
   * <code>true</code> when the semantic of merging is met.
   */
