 ...
 *  overriden using either LIBKERN_RETURNS_RETAINED or
 * LIBKERN_RETURNS_NOT_RETAINED attribute applied to a function. In the former
 * case, it stipulates that the function is returning at "+1", and in the latter
 * case "+0".
 *
...
/*
 *  <rdar://problem/44872498> AppleUSBAudio builds xnu's libkern headers in user
 * space
 */
...
#define APPLE_KEXT_WSHADOW_PUSH                                                \
 ...
 * <ul>
 * <li><code>@link OSTypeAlloc OSTypeAlloc@/link</code> - allocation of new
 * instances</li> <li><code>@link OSDynamicCast OSDynamicCast@/link</code> -
 * safe type casting</li> <li><code>@link OSCheckTypeInst
 * OSCheckTypeInst@/link</code> - checking for inheritance/derivation</li>
 ...
 * and
 * <code>@link //apple_ref/cpp/macro/OSCheckTypeInst
 * OSCheckTypeInst@/link</code>.
 */
...
#define OSSafeRelease(inst)                                                    \
  do {                                                                         \
    int OSSafeRelease __attribute__((deprecated("Use OSSafeReleaseNULL")));    \
    (OSSafeRelease);                                                           \
    if (inst)                                                                  \
      (inst)->release();                                                       \
  } while (0)
 ...
 *  @abstract Release an object if not <code>NULL</code>, then set it to
 * <code>NULL</code>.
...
#define OSSafeReleaseNULL(inst)                                                \
  ...
  static _ptf_t _ptmf2ptf(const OSMetaClassBase *self,
  ...
  static inline _ptf_t _ptmf2ptf(const OSMetaClassBase *self,
                                 void (OSMetaClassBase::*func)(void)) {
    union {
 ...
 *                 (typically provided as a <code>typedef</code> by  I/O KitKit
 * classes).
...
#define OSMemberFunctionCast(cptrtype, self, func)                             \
  ((cptrtype)__builtin_load_member_function_pointer(*self, func)               \
       ?: (cptrtype)OSMetaClassBase::_ptmf2ptf(                                \
              self, (void (OSMetaClassBase::*)(void))func))
#else
#define OSMemberFunctionCast(cptrtype, self, func)                             \
  (cptrtype)                                                                   \
      OSMetaClassBase::_ptmf2ptf(self, (void (OSMetaClassBase::*)(void))func)
#endif
...
public:
   ...
   * <code>@link OSDynamicCast OSDynamicCast@/link</code>.
   */
  static OSMetaClassBase *safeMetaCast(const OSMetaClassBase *anObject,
   ...
   */
  static OSMetaClassBase *requiredMetaCast(const OSMetaClassBase *anObject,
                                           const OSMetaClass *toMeta);
   ...
   */
  static bool checkTypeInst(const OSMetaClassBase *inst,
 ...
 * <ul>
 * <li><code>@link OSDeclareDefaultStructors
 * OSDeclareDefaultStructors@/link</code> - for classes with no abstract member
 * function declarations</li> <li><code>@link OSDeclareAbstractStructors
 * OSDeclareAbstractStructors@/link</code> - for classes with at least one
 * abstract member function declaration</li> <li><code>@link
 * OSDeclareFinalStructors OSDeclareFinalStructors@/link</code> - for classes
 * that should not be subclassable by another kext</li>
 * </ul>
 *
...
protected:
   ...
   */
  OSMetaClass(const char *className, const OSMetaClass *superclass,
              unsigned int classSize, zone_t *zone, const char *zone_name,
...
public:
   ...
   * <code>@link OSCheckTypeInst OSCheckTypeInst@/link</code>.
   */
  static OSMetaClassBase *checkMetaCastWithName(const OSSymbol *className,
   ...
   * <code>@link OSCheckTypeInst OSCheckTypeInst@/link</code>.
   */
  static OSMetaClassBase *checkMetaCastWithName(const OSString *className,
                                                const OSMetaClassBase *object);
   ...
   * <code>@link OSCheckTypeInst OSCheckTypeInst@/link</code>.
   */
  static OSMetaClassBase *checkMetaCastWithName(const char *className,
                                                const OSMetaClassBase *object);
  ...
  void applyToInstances(OSMetaClassInstanceApplierFunction applier,
                        void *context) const;
  static void
  applyToInstancesOfClassName(const OSSymbol *name,
...
#define _OS_ADD_METHODS_dispatch(className)                                    \
  className##_Methods className##_KernelMethods
...
OSDeclareCommonStructors(className, dispatch) private                          \
...
#define OSDeclareAbstractStructors(className)                                  \
  _OSDeclareAbstractStructors(className, ) _OS_ADD_OPERATOR_PROTO

#define OSDeclareAbstractStructorsWithDispatch(className)                      \
  _OSDeclareAbstractStructors(className, dispatch) _OS_ADD_OPERATOR_PROTO

#define OSDeclareAbstractStructorsWithDispatchAndNoOperators(className)        \
...
_OSDeclareDefaultStructors(className, dispatch) private : void __OSFinalClass( \
                                                              void);           \
                                                                               \
protected:
 ...
 */
#define OSMetaClassConstructorInit(className, superclassName, init)            \
...
#define declareZone(className) static zone_t className##_zone;
#endif /* KERNEL_PRIVATE */

#ifdef KERNEL_PRIVATE
#define OSMetaClassConstructorInitWithZone(className, superclassName, init,    \
                                           zflags)                             \
  declareZone(className) /* The ::MetaClass constructor */                     \
      className ::MetaClass::MetaClass()                                       \
      : OSMetaClass(#className, className::superClass, sizeof(className),      \
                    &(className##_zone), "iokit." #className, zflags) {        \
    init;                                                                      \
  }
#endif /* KERNEL_PRIVATE */
...
#define OSDefineMetaClassWithInit(className, superclassName, init)             \
  ...
  const OSMetaClass *const className ::metaClass = &className ::gMetaClass;    \
  const OSMetaClass *const className ::superClass =                            \
      &superclassName ::gMetaClass;                                            \
  /* Class member functions */                                                 \
  className ::className(const OSMetaClass *meta) : superclassName(meta) {}     \
  className ::~className() {}                                                  \
  const OSMetaClass *className ::getMetaClass() const { return &gMetaClass; }
...
#define OSDefineOperatorMethods(className)                                     \
  static KALLOC_TYPE_DEFINE(className##_ktv, className, KT_DEFAULT);           \
  void *className::operator new(size_t size) {                                 \
    return OSObject_typed_operator_new(className##_ktv, size);                 \
  }                                                                            \
  void className::operator delete(void *mem, size_t size) {                    \
    return OSObject_typed_operator_delete(className##_ktv, mem, size);         \
  }
#else
#define OSDefineOperatorMethods(className)                                     \
  void *className::operator new(size_t size) {                                 \
...
#define OSDefineFinalStructors(className, superclassName)                      \
  OSDefineBasicStructors(className, superclassName) void                       \
  className ::__OSFinalClass(void) {}
 ...
 */
#define OSDefineMetaClassAndStructorsWithInitAndZone(                          \
    className, superclassName, init, zflags)                                   \
  OSDefineMetaClassWithInit(className, superclassName, init)                   \
      OSMetaClassConstructorInitWithZone(className, superclassName, init,      \
                                         zflags)                               \
          OSDefineBasicStructors(className, superclassName)                    \
 ...
 *                       of the class's OSMetaClass.
 */
#define OSDefineMetaClassAndAbstractStructorsWithInit(className,               \
                                                      superclassName, init)    \
  OSDefineMetaClassWithInit(className, superclassName, init)                   \
      OSMetaClassConstructorInit(className, superclassName, init)              \
 ...
 *                       of the class's OSMetaClass.
 */
#define OSDefineMetaClassAndFinalStructorsWithInit(className, superclassName,  \
                                                   init)                       \
  OSDefineMetaClassWithInit(className, superclassName, init)                   \
      OSMetaClassConstructorInit(className, superclassName, init)              \
 ...
 * own zone rather than share VA with other objects.
 */
#define OSDefineMetaClassAndStructorsWithZone(className, superclassName,       \
                                              zflags)                          \
  OSDefineMetaClassAndStructorsWithInitAndZone(className, superclassName, ,    \
                                               zflags)
#endif
 ...
 * own zone rather than share VA with other objects.
 */
#define OSDefineMetaClassAndFinalStructorsWithZone(className, superclassName,  \
                                                   zflags)                     \
  OSDefineMetaClassAndFinalStructorsWithInitAndZone(className, superclassName, \
                                                    , zflags)
#endif /* KERNEL_PRIVATE */
