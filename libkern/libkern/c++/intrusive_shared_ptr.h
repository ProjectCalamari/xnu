...
struct __attribute__((trivial_abi)) intrusive_shared_ptr {
  ...
  constexpr intrusive_shared_ptr(isp_detail::nullptr_t) noexcept
      : ptr_(nullptr) {}
  ...
  explicit intrusive_shared_ptr(pointer p, retain_t) noexcept : ptr_(p) {
    if (ptr_ != nullptr) {
  ...
  explicit constexpr intrusive_shared_ptr(
      __attribute__((os_consumed)) pointer p, no_retain_t) noexcept
      : ptr_(p) {}
  ...
  // refcount is increased. Unlike for move operations, the source
  // pointer is left untouched.
  template <typename U, typename = isp_detail::enable_if_t<
                            isp_detail::is_convertible_v<U *, T *>>>
  ...
  constexpr intrusive_shared_ptr(intrusive_shared_ptr &&other) noexcept
      : ptr_(other.ptr_) {
    other.ptr_ = nullptr;
  }
  ...
  template <typename U, typename = isp_detail::enable_if_t<
                            isp_detail::is_convertible_v<U *, T *>>>
  constexpr intrusive_shared_ptr(
      intrusive_shared_ptr<U, RefcountPolicy> &&other) noexcept
      : ptr_(other.ptr_) {
    other.ptr_ = nullptr;
  }
  ...
  // the assignment, the refcount is decremented on that object after
  // the assignment is performed.
  intrusive_shared_ptr &operator=(isp_detail::nullptr_t) noexcept {
    reset();
    return *this;
  }
  ...
  intrusive_shared_ptr &reset() noexcept {
    if (ptr_ != nullptr) {
  ...
  intrusive_shared_ptr &reset(__attribute__((os_consumed)) pointer p,
                              no_retain_t) noexcept {
    if (ptr_ != nullptr) {
      RefcountPolicy::release(*ptr_);
    }
    ptr_ = p;
  ...
  // If the shared pointer is pointing to an object before calling `reset`,
  // the refcount is decremented on that object.
  intrusive_shared_ptr &reset(pointer p, retain_t) noexcept {
  ...
  constexpr pointer get() const && noexcept = delete;
...
private:
  friend constexpr void swap(intrusive_shared_ptr &a,
                             intrusive_shared_ptr &b) noexcept {
    pointer tmp = a.ptr_;
    a.ptr_ = b.ptr_;
    b.ptr_ = tmp;
  }
  ...
  template <typename U, typename Policy> friend struct intrusive_shared_ptr;
...
static_pointer_cast(intrusive_shared_ptr<From, R> &&ptr) {
  return intrusive_shared_ptr<To, R>(static_cast<To *>(ptr.detach()),
                                     no_retain);
}
...
reinterpret_pointer_cast(intrusive_shared_ptr<From, R> &&ptr) noexcept {
  return intrusive_shared_ptr<To, R>(reinterpret_cast<To *>(ptr.detach()),
                                     no_retain);
}
...
template <typename T, typename U, typename R,
          typename = isp_detail::WhenComparable<T *, U *>>
bool operator==(intrusive_shared_ptr<T, R> const &x,
                intrusive_shared_ptr<U, R> const &y) {
  return x.get() == y.get();
}
...
bool operator==(intrusive_shared_ptr<T, R> const &x,
                isp_detail::nullptr_t) noexcept {
  return x.get() == nullptr;
}

template <typename T, typename R>
bool operator==(isp_detail::nullptr_t,
                intrusive_shared_ptr<T, R> const &x) noexcept {
