...
class OSArray : public OSCollection {
  ...
  typedef OSTaggedSharedPtr<const OSMetaClassBase, OSCollection>
      ArraySharedPtrType;
...
public:
   ...
   *
   * @discussion
   * <code>objects</code> must be non-<code>NULL</code>, and <code>count</code>
   * must be nonzero. If <code>capacity</code> is nonzero, it must be greater
   * than or equal to <code>count</code>. The new array will grow as needed to
   * accommodate more objects
   * (<i>unlike</i> @link //apple_ref/doc/uid/20001502 CFMutableArray@/link,
   * for which the initial capacity is a hard limit).
   */
  static OSPtr<OSArray> withObjects(const OSObject *objects[],
                                    unsigned int count,
   ...
   *
   * @abstract
   * Creates and initializes an OSArray populated with the contents of another
   * array.
   *
   ...
   * not copied.
   */
  static OSPtr<OSArray> withArray(const OSArray *array,
                                  unsigned int capacity = 0);
   ...
   * <code>@link
   * //apple_ref/cpp/clm/OSArray/withCapacity/staticOSArray*\/(unsignedint)
   ...
   * for which the initial capacity is a hard limit).
   */
  virtual bool initWithObjects(const OSObject *objects[], unsigned int count,
                               unsigned int capacity = 0);
   ...
   * Not for general use. Use the static instance creation method
   * <code>@link
   * //apple_ref/cpp/clm/OSArray/withArray/staticOSArray*\/(constOSArray*,unsignedint)
   ...
   * for which the initial capacity is a hard limit).
   *
   * The objects in <code>array</code> are retained for storage in the new
   * OSArray, not copied.
   */
  ...
  bool setObject(unsigned int index,
   ...
   */
  virtual void replaceObject(unsigned int index,
                             const OSMetaClassBase *anObject);

  void replaceObject(unsigned int index,
                     OSSharedPtr<const OSMetaClassBase> const &anObject);
