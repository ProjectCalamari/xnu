...
void *
operator new(size_t,
             void *) noexcept; // forward declaration needed for placement-new
#endif
...
// 2. Check that the allocation is accessed within its bounds on indexing
// operations
// 3. Act as a source for obtaining (non-owning) `bounded_ptr`s to the
// underlying memory
//
// In fact, `safe_allocation` should be the primary source of `bounded_ptr`s to
// heap-allocated memory, via its `.begin()` and `.end()` methods.
// `safe_allocation` is optimized for use cases where simple scratch space is
// needed for calculation and deallocated once the calculation is done. As such,
// it is not a full-blown container class, which drives many design choices
// behind `safe_allocation`:
//
// 1. It can't be copied or compared for equality -- `safe_allocation` is not a
// proper value type
// 2. It can't be resized -- this keeps the design extremely simple and free of
// overhead
...
struct safe_allocation {
  static_assert(
      sa_detail::is_empty_v<Allocator>,
  ...
  constexpr safe_allocation(sa_detail::nullptr_t) noexcept
      : safe_allocation() {}
  ...
  explicit safe_allocation(size_t n, allocate_memory_zero_t) {
    static_assert(
        __is_scalar(T) || __is_aggregate(T),
        "Creating objects via zero-allocation requires those objects to be "
        "scalars or aggregates (more broadly implicit lifetime types)");
    size_t bytes;
    if (__improbable(os_mul_overflow(n, sizeof(T), &bytes) ||
                     (n > UINT32_MAX))) {
      data_ = nullptr;
  ...
  // goes out of scope.
  safe_allocation(safe_allocation &&other) noexcept
  ...
  safe_allocation &operator=(sa_detail::nullptr_t) {
    if (data_ != nullptr) {
  ...
  OS_ALWAYS_INLINE T &operator[](ptrdiff_t n) {
    return begin()[n]; // trap happens in `bounded_ptr` if null or OOB
  }
  OS_ALWAYS_INLINE T const &operator[](ptrdiff_t n) const {
    return begin()[n]; // trap happens in `bounded_ptr` if null or OOB
  }

private:
  // Swap support
  friend void swap(safe_allocation &a, safe_allocation &b) noexcept {
...
bool operator==(safe_allocation<T, A, P> const &x, sa_detail::nullptr_t) {
...
template <typename T, typename A, typename P>
bool operator==(sa_detail::nullptr_t, safe_allocation<T, A, P> const &x) {
  return x == nullptr;
}
