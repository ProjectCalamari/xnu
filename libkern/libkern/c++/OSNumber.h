...
public:
   ...
   */
  static OSPtr<OSNumber> withNumber(unsigned long long value,
   ...
   */
  static OSPtr<OSNumber> withDouble(double value);
  /*!
   * @function withFloat
   * @abstract
   * Creates and initializes an instance of OSNumber
   * with an float value.
   */
  static OSPtr<OSNumber> withFloat(float value);
   ...
   *
   * @discussion
   * This function does not work in I/O Kit versions prior to 8.0 (Mac OS
   * X 10.4). In I/O Kit version 8.0 and later, it works but is limited to
   * parsing unsigned 32 bit quantities. The format of the C string may be
   * decimal, hexadecimal ("0x" prefix), binary ("0b" prefix), or octal ("0"
   * prefix).
   *
   ...
   * but you can't change the bit size.
   */
  static OSPtr<OSNumber> withNumber(const char *valueString,
                                    unsigned int numberOfBits);
