 ...
 * I/O Kit driver objects are either designed for use within thread-safe
 * contexts or designed to inherently be thread-safe. Always check the
 * individual class documentation to see what steps are necessary for concurrent
 * use of instances.
 */
...
protected:
   ...
   * If the subclass encounters a failure then it should return
   * <code>false</code>.
   */
  virtual bool init();
...
public:
  ...
  // xx-review: used to say, "Remove a reference on this object with this tag,
  // if an attempt is made to remove a reference that isn't associated with this
  // tag the kernel will panic immediately", but I don't see that in the
  // implementation
  ...
  bool iterateObjects(void *refcon,
                      bool (*callback)(void *refcon, OSObject *object));
#ifdef __BLOCKS__
