...
protected:
  ...
  virtual bool
  getNextObjectForIterator(void *iterator,
                           OSObject **ret) const APPLE_KEXT_OVERRIDE;
...
public:
   ...
   *
   * @param  capacity  The initial storage capacity of the new dictionary
   * object.
   *
   * @result
   ...
   * (<i>unlike</i> @link //apple_ref/doc/uid/20001497
   * CFMutableDictionary@/link, for which the initial capacity is a hard limit).
   */
   ...
   */
  static OSPtr<OSDictionary> withObjects(const OSObject *objects[],
                                         const OSSymbol *keys[],
                                         unsigned int count,
   ...
   * <code>objects</code> and <code>keys</code> must be non-<code>NULL</code>,
   * and <code>count</code> must be nonzero.
   * If <code>capacity</code> is nonzero, it must be greater than or equal to
   * <code>count</code>. The new dictionary will grow as needed to accommodate
   * more key/object pairs
   * (<i>unlike</i>
   * @link //apple_ref/doc/uid/20001497 CFMutableDictionary@/link,
   * for which the initial capacity is a hard limit).
   */
   ...
   *                  in <code>dict</code>;
   *                  otherwise <code>capacity</code> must be greater than or
   * equal to the number of key/value pairs in <code>dict</code>.
   *
   * @result
   * An instance of OSDictionary
   ...
   * not copied.
   */
  static OSPtr<OSDictionary> withDictionary(const OSDictionary *dict,
                                            unsigned int capacity = 0);
   ...
   * <code>@link
   * //apple_ref/cpp/clm/OSDictionary/withCapacity/staticOSDictionary*\/(unsignedint)
  ...
  virtual bool initWithObjects(const OSObject *objects[],
                               const OSSymbol *keys[], unsigned int count,
                               unsigned int capacity = 0);
   ...
   */
  virtual bool initWithObjects(const OSObject *objects[],
                               const OSString *keys[], unsigned int count,
                               unsigned int capacity = 0);
   ...
   *
   * @abstract
   * Returns the number of objects the dictionary can store without
   * reallocating.
   *
   * @result
  ...
  setCapacityIncrement(unsigned increment) APPLE_KEXT_OVERRIDE;
   ...
   */
  virtual unsigned int
  ensureCapacity(unsigned int newCapacity) APPLE_KEXT_OVERRIDE;
  ...
  bool setObject(OSSharedPtr<const OSSymbol> const &aKey,
  ...
  bool setObject(const OSString *aKey,
                 OSSharedPtr<const OSMetaClassBase> const &anObject);
  ...
  bool setObject(const char *aKey,
                 OSSharedPtr<const OSMetaClassBase> const &anObject);
   ...
   *                     (as @link //apple_ref/cpp/cl/OSString OSStrings@/link
   * or
   ...
   */
  virtual bool
  isEqualTo(const OSMetaClassBase *anObject) const APPLE_KEXT_OVERRIDE;
   ...
   */
  virtual unsigned setOptions(unsigned options, unsigned mask,
   ...
   */
  OSPtr<OSCollection>
  copyCollection(OSDictionary *cycleDict = NULL) APPLE_KEXT_OVERRIDE;

#if XNU_KERNEL_PRIVATE
  bool setObject(const OSSymbol *aKey, const OSMetaClassBase *anObject,
                 bool onlyAdd);
   ...
   */
  bool iterateObjects(void *refcon,
                      bool (*callback)(void *refcon, const OSSymbol *key,
                                       OSObject *object));
