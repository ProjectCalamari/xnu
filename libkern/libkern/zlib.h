 ...
 *  is on the return value of inflate(), as noted below, or when it returns
 ...
 *  total_out bytes) and returns Z_OK, Z_STREAM_END or an error code as
 * described below. At the end of the stream, inflate() checks that its computed
 ...
 * check value), Z_STREAM_ERROR if the stream structure was inconsistent (for
 ...
 * room in the output buffer when Z_FINISH is used. Note that Z_BUF_ERROR is not
 * fatal, and inflate() can be called again with more input and more output
 * space to continue decompressing. If Z_DATA_ERROR is returned, the application
 * may then call inflateSync() to look for a good compression block if a partial
...
ZEXTERN int ZEXPORT deflateResetWithIO(z_streamp strm, z_input_func zinput,
 ...
 * which indicates where valid compressed data was found. In the error case, the
...
ZEXTERN uLong ZEXPORT z_crc32_combine OF((uLong crc1, uLong crc2,
                                          z_off_t len2));
