...
#define MKEXT2_GET_PLIST_COMPSIZE(hdr)                                         \
  MKEXT_SWAP(MKEXT2_HDR_CAST(hdr)->plist_compressed_size)
#define MKEXT2_GET_PLIST_FULLSIZE(hdr)                                         \
  MKEXT_SWAP(MKEXT2_HDR_CAST(hdr)->plist_full_size)
...
#define MKEXT1_GET_ENTRY_OFFSET(ptr)                                           \
  (MKEXT_SWAP(MKEXT1_ENTRY_CAST(ptr)->offset))
#define MKEXT1_GET_ENTRY_COMPSIZE(ptr)                                         \
  (MKEXT_SWAP(MKEXT1_ENTRY_CAST(ptr)->compsize))
#define MKEXT1_GET_ENTRY_FULLSIZE(ptr)                                         \
  (MKEXT_SWAP(MKEXT1_ENTRY_CAST(ptr)->realsize))
#define MKEXT1_GET_ENTRY_MODTIME(ptr)                                          \
  ((time_t)MKEXT_SWAP(MKEXT1_ENTRY_CAST(ptr)->modifiedsecs))
#define MKEXT1_ENTRY_EXISTS(ptr)                                               \
  (MKEXT1_GET_ENTRY_OFFSET(ptr) || MKEXT1_GET_ENTRY_FULLSIZE(ptr) ||           \
   MKEXT_GET_ENTRY_COMPSIZE(ptr) || MKEXT_GET_ENTRY_COMPSIZE(ptr))

#define MKEXT1_GET_KEXT(hdr, i)                                                \
  ((mkext_kext *)&(MKEXT1_HDR_CAST(hdr)->kext[(i)]))
#define MKEXT1_GET_KEXT_PLIST(hdr, i)                                          \
  (MKEXT1_ENTRY_CAST(&(MKEXT1_GET_KEXT((hdr), (i))->plist)))
...
int decompress_lzss(u_int8_t *dst, u_int32_t dstlen, u_int8_t *src,
                    u_int32_t srclen);
