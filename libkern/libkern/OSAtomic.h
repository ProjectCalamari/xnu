...
#define __SAFE_CAST_PTR(type, var)                                             \
  ({                                                                           \
    _Static_assert(sizeof(*(var)) == sizeof(*(type)NULL), "size mismatch");    \
    ((type)(var));                                                             \
  })
#else
 ...
 * See OSAddAtomic.
 */
extern long OSAddAtomicLong(long theAmount, volatile long *address);
 ...
 *
 * @abstract
 * Compare and swap operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSCompareAndSwap8 function compares the value at the specified address
 * with oldVal. The value of newValue is written to the address only if oldValue
 * and the value at the address are equal. OSCompareAndSwap returns true if
 * newValue is written to the address; otherwise, it returns false.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 * devices; this function cannot guarantee atomicity, for example, on memory
 * mapped from a PCI device. Additionally, this function incorporates a memory
 * barrier on systems with weakly-ordered memory architectures.
 *
 ...
 */
extern Boolean OSCompareAndSwap8(UInt8 oldValue, UInt8 newValue,
 ...
 *
 * @discussion
 * The OSCompareAndSwap16 function compares the value at the specified address
 * with oldVal. The value of newValue is written to the address only if oldValue
 * and the value at the address are equal. OSCompareAndSwap returns true if
 * newValue is written to the address; otherwise, it returns false.
 *
 ...
 * @result true if newValue was written to the address.
 */
extern Boolean OSCompareAndSwap16(UInt16 oldValue, UInt16 newValue,
 ...
 *
 * @discussion
 * The OSCompareAndSwap function compares the value at the specified address
 * with oldVal. The value of newValue is written to the address only if oldValue
 * and the value at the address are equal. OSCompareAndSwap returns true if
 * newValue is written to the address; otherwise, it returns false.
 *
 ...
 *
 * @discussion
 * The OSCompareAndSwapPtr function compares the pointer-sized value at the
 * specified address with oldVal. The value of newValue is written to the
 * address only if oldValue and the value at the address are equal.
 * OSCompareAndSwapPtr returns true if newValue is written to the address;
 ...
 * @param newValue The pointer value to write to address if oldValue compares
 * true.
 * @param address The pointer-size aligned address of the data to update
 * atomically.
 * @result true if newValue was written to the address.
 */
extern Boolean OSCompareAndSwapPtr(void *oldValue, void *newValue,
 ...
 *
 * @abstract
 * 32-bit add operation, performed atomically with respect to all devices that
 * participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSAddAtomic function adds the specified amount to the value at the
 * specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 * devices; this function cannot guarantee atomicity, for example, on memory
 * mapped from a PCI device. Previous incarnations of this function incorporated
 * a memory barrier on systems with weakly-ordered memory architectures, but
 * current versions contain no barriers.
 ...
 *
 * @abstract
 * 16-bit add operation, performed atomically with respect to all devices that
 * participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSAddAtomic16 function adds the specified amount to the value at the
 * specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 8-bit add operation, performed atomically with respect to all devices that
 * participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSAddAtomic8 function adds the specified amount to the value at the
 * specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 32-bit increment operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSIncrementAtomic function increments the value at the specified address
 * by one and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 * devices; this function cannot guarantee atomicity, for example, on memory
 * mapped from a PCI device.
 * @param address The 4-byte aligned address of the value to update atomically.
 ...
 *
 * @abstract
 * 16-bit increment operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSIncrementAtomic16 function increments the value at the specified
 * address by one and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 * devices; this function cannot guarantee atomicity, for example, on memory
 ...
 *
 * @abstract
 * 8-bit increment operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSIncrementAtomic8 function increments the value at the specified address
 * by one and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 32-bit decrement operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSDecrementAtomic function decrements the value at the specified address
 * by one and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 16-bit decrement operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSDecrementAtomic16 function decrements the value at the specified
 * address by one and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 8-bit decrement operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSDecrementAtomic8 function decrements the value at the specified address
 * by one and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 32-bit logical and operation, performed atomically with respect to all
 * devices that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSBitAndAtomic function logically ands the bits of the specified mask
 * into the value at the specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 * devices; this function cannot guarantee atomicity, for example, on memory
 ...
 *
 * @abstract
 * 16-bit logical and operation, performed atomically with respect to all
 * devices that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSBitAndAtomic16 function logically ands the bits of the specified mask
 * into the value at the specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 8-bit logical and operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSBitAndAtomic8 function logically ands the bits of the specified mask
 * into the value at the specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 32-bit logical or operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSBitOrAtomic function logically ors the bits of the specified mask into
 * the value at the specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 * devices; this function cannot guarantee atomicity, for example, on memory
 ...
 *
 * @abstract
 * 16-bit logical or operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSBitOrAtomic16 function logically ors the bits of the specified mask
 * into the value at the specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 8-bit logical or operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @discussion
 * The OSBitOrAtomic8 function logically ors the bits of the specified mask into
 * the value at the specified address and returns the original value.
 * @param mask The mask to logically or with the value.
 * @param address The address of the value to update atomically.
 * @result The value before the bitwise operation.
 ...
 *
 * @abstract
 * 32-bit logical xor operation, performed atomically with respect to all
 * devices that participate in the coherency architecture of the platform.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @discussion
 * The OSBitXorAtomic function logically xors the bits of the specified mask
 * into the value at the specified address and returns the original value.
 * @param mask The mask to logically or with the value.
 * @param address The 4-byte aligned address of the value to update atomically.
 * @result The value before the bitwise operation.
 ...
 *
 * @abstract
 * 16-bit logical xor operation, performed atomically with respect to all
 * devices that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSBitXorAtomic16 function logically xors the bits of the specified mask
 * into the value at the specified address and returns the original value.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @abstract
 * 8-bit logical xor operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @discussion
 * The OSBitXorAtomic8 function logically xors the bits of the specified mask
 * into the value at the specified address and returns the original value.
 * @param mask The mask to logically or with the value.
 * @param address The address of the value to update atomically.
 * @result The value before the bitwise operation.
 ...
 *
 * @abstract
 * Bit test and set operation, performed atomically with respect to all devices
 * that participate in the coherency architecture of the platform.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 ...
 *
 * @discussion
 * The OSTestAndSet function sets a single bit in a byte at a specified address.
 * It returns true if the bit was already set, false otherwise.
 * @param bit The bit number in the range 0 through 7. Bit 0 is the most
 * significant.
 ...
 *
 * @abstract
 * Bit test and clear operation, performed atomically with respect to all
 * devices that participate in the coherency architecture of the platform.
 *
 * @discussion
 * The OSTestAndClear function clears a single bit in a byte at a specified
 * address. It returns true if the bit was already clear, false otherwise.
 *
 * This function guarantees atomicity only with main system memory. It is
 * specifically unsuitable for use on noncacheable memory such as that in
 * devices; this function cannot guarantee atomicity, for example, on memory
 ...
 *
 * @discussion
 * Multiprocessor locks used within the shared memory area between the kernel
 * and event system.  These must work in both user and kernel mode.
 *
 * @result
 * Returns false if the lock was already held by another thread, true if it took
 ...
 *
 * @abstract
 * The OSSynchronizeIO routine ensures orderly load and store operations to
 * noncached memory mapped I/O devices.
 *
 * @discussion
 * The OSSynchronizeIO routine ensures orderly load and store operations to
 * noncached memory mapped I/O devices. It executes the eieio instruction on
 * PowerPC processors.
 */
