...
static bool _os_trace_addr_in_text_segment_32(const void *dso,
                                              const void *addr) {
  const struct mach_header *mhp = (const struct mach_header *)dso;
  const struct segment_command *sgp =
      (const struct segment_command *)(const void *)((const char *)mhp +
                                                     sizeof(
                                                         struct mach_header));
  ...
  for (uint32_t i = 0; i < mhp->ncmds; i++) {
    if (sgp->cmd == LC_SEGMENT) {
      if (strncmp(sgp->segname, SEG_TEXT, sizeof(sgp->segname)) == 0) {
        return (uintptr_t)addr >= (sgp->vmaddr) &&
               (uintptr_t)addr < (sgp->vmaddr + sgp->vmsize);
      }
    }
    sgp = (const struct segment_command *)(const void *)((const char *)sgp +
                                                         sgp->cmdsize);
  }
...
static bool _os_trace_addr_in_text_segment_64(const void *dso,
                                              const void *addr) {
  ...
  const struct segment_command_64 *sgp =
      (const struct segment_command_64
           *)(const void *)((const char *)mhp + sizeof(struct mach_header_64));
  ...
  for (uint32_t i = 0; i < mhp->ncmds; i++) {
    ...
    sgp = (const struct segment_command_64 *)(const void *)((const char *)sgp +
                                                            sgp->cmdsize);
  }
