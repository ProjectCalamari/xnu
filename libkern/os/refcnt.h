 ...
 *
 * Two flavors are provided: atomic and locked. Atomic internally uses C11
 * atomic operations and requires no external synchronization, whereas the
 * locked flavor assumes the refcnt object is locked by the caller. It is NOT
 * safe to mix-and-match locked and atomic calls.
 *
 ...
 * reference count of member refcounts, and the "rlog=" boot-arg is used to
 * enable refcount logging by group name. Named groups can be created explicitly
 * with os_refgrp_decl(), or implicitly by passing NULL for the refgrp when
 * initializing a refcnt object. In the latter case, the group name is the same
 * as the function enclosing the init call. Groups are only available on DEV or
 * DEBUG builds, and are otherwise compiled out.
 */
 ...
 *
 * Static initializers that create refcnt objects with safe initial values for
 * use between declaration and initialization (os_ref*_init()). Equivalent to
 * zeroing.
 */
...
static void os_ref_init_count(struct os_refcnt *, struct os_refgrp *,
...
/*
 *
 * os_ref_retain: acquire a reference (increment reference count by 1)
 * atomically.
 *
 * os_ref_release: release a reference (decrement reference count) atomically
 * and return the new count. Memory is synchronized such that the dealloc block
 *		(i.e. code handling the final release() == 0 call) sees
 * up-to-date memory with respect to all prior release()s on the same refcnt
 * object. This memory ordering is sufficient for most use cases.
 *
 * os_ref_release_relaxed: same as release() but with weaker relaxed memory
 * ordering. This can be used when the dealloc block is already synchronized
 * with other accesses to the object (for example, with a lock).
 *
 * os_ref_release_live: release a reference that is guaranteed not to be the
 * last one.
 */
 ...
 *		zero reference count. The caller must therefore ensure that the
 * object remains alive for any possible retain_try() caller, usually by using a
 *		lock protecting both the retain and dealloc paths. This variant
 * is useful for objects stored in a collection, because no lock is required on
 * the release() side until the object is deallocated.
 */
 ...
 *		lock. The caller must ensure mutual exclusivity of
 * retain_locked() and release_locked() calls on the same object.
 *
 ...
 *
 * This returns the resulting refcount value, like @c os_pcpu_ref_release()
 * does.
 */
extern os_ref_count_t
os_pcpu_ref_kill(os_pcpu_ref_t ref, struct os_refgrp *grp) __result_use_check;
 ...
 * the refcount hit 0.
 */
extern bool os_pcpu_ref_retain_try(os_pcpu_ref_t ref,
                                   struct os_refgrp *grp) __result_use_check;
 ...
 */
extern os_ref_count_t
os_pcpu_ref_release(os_pcpu_ref_t ref,
                    struct os_refgrp *grp) __result_use_check;
...
static void os_ref_init_count_raw(os_ref_atomic_t *, struct os_refgrp *,
                                  os_ref_count_t count)
    os_error_if(count == 0, "Reference count must be non-zero initialized");
static void os_ref_retain_floor(struct os_refcnt *, os_ref_count_t f)
    os_error_if(!__builtin_constant_p(f) || f == 0,
                "refcount floor must be >= 1");
static void os_ref_retain_raw(os_ref_atomic_t *, struct os_refgrp *);
static void os_ref_retain_floor_raw(os_ref_atomic_t *, os_ref_count_t f,
                                    struct os_refgrp *)
    os_error_if(!__builtin_constant_p(f) || f == 0,
                "refcount floor must be >= 1");
static os_ref_count_t os_ref_release_raw(os_ref_atomic_t *,
                                         struct os_refgrp *) OS_WARN_RESULT;
static os_ref_count_t
os_ref_release_raw_relaxed(os_ref_atomic_t *,
                           struct os_refgrp *) OS_WARN_RESULT;
static void os_ref_release_live_raw(os_ref_atomic_t *, struct os_refgrp *);
static bool os_ref_retain_try_raw(os_ref_atomic_t *,
                                  struct os_refgrp *) OS_WARN_RESULT;
static bool os_ref_retain_floor_try_raw(os_ref_atomic_t *, os_ref_count_t f,
                                        struct os_refgrp *) OS_WARN_RESULT
    os_error_if(!__builtin_constant_p(f) || f == 0,
                "refcount floor must be >= 1");
static void os_ref_retain_locked_raw(os_ref_atomic_t *, struct os_refgrp *);
static void os_ref_retain_floor_locked_raw(os_ref_atomic_t *, os_ref_count_t f,
                                           struct os_refgrp *)
    os_error_if(!__builtin_constant_p(f) || f == 0,
                "refcount floor must be >= 1");
static os_ref_count_t
os_ref_release_locked_raw(os_ref_atomic_t *, struct os_refgrp *) OS_WARN_RESULT;
static os_ref_count_t os_ref_get_count_raw(os_ref_atomic_t *rc);

/*
 * Bitwise API: like the raw API, but allows some bits in the refcount value to
 * be reserved for other purposes. 'b' defines the number of trailing (LSB)
 * reserved bits, which the refcnt_raw API will never modify (except at init()).
 *
 * It is assumed that users of this API always use atomic ops on the
 * os_ref_atomic_t (or hold a lock for the locked variants), and never modify
 * the top (32 - 'b') bits.
 *
...
#define os_ref_init_mask(rc, b, grp, bits)                                     \
  os_ref_init_count_mask((rc), (b), (grp), 1, bits)
void os_ref_init_count_mask(os_ref_atomic_t *rc, uint32_t b,
                            struct os_refgrp *grp, os_ref_count_t init_count,
                            uint32_t init_bits)
...
static void os_ref_retain_mask(os_ref_atomic_t *rc, uint32_t b,
                               struct os_refgrp *grp);
static void os_ref_retain_acquire_mask(os_ref_atomic_t *rc, uint32_t b,
                                       struct os_refgrp *grp);
static uint32_t os_ref_retain_try_mask(os_ref_atomic_t *, uint32_t b,
                                       uint32_t reject_mask,
                                       struct os_refgrp *grp) OS_WARN_RESULT;
static bool
os_ref_retain_try_acquire_mask(os_ref_atomic_t *, uint32_t b,
                               uint32_t reject_mask,
                               struct os_refgrp *grp) OS_WARN_RESULT;

static uint32_t os_ref_release_raw_mask(os_ref_atomic_t *rc, uint32_t b,
                                        struct os_refgrp *grp) OS_WARN_RESULT;
static uint32_t
os_ref_release_raw_relaxed_mask(os_ref_atomic_t *rc, uint32_t b,
                                struct os_refgrp *grp) OS_WARN_RESULT;
static os_ref_count_t os_ref_release_mask(os_ref_atomic_t *rc, uint32_t b,
                                          struct os_refgrp *grp) OS_WARN_RESULT;
static os_ref_count_t
os_ref_release_relaxed_mask(os_ref_atomic_t *rc, uint32_t b,
                            struct os_refgrp *grp) OS_WARN_RESULT;
static uint32_t os_ref_release_live_raw_mask(os_ref_atomic_t *rc, uint32_t b,
                                             struct os_refgrp *grp);
static void os_ref_release_live_mask(os_ref_atomic_t *rc, uint32_t b,
                                     struct os_refgrp *grp);
