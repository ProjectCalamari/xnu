...
#define log_context_cursor(ctx)                                                \
  &(ctx)->ctx_hdr->hdr_data[(ctx)->ctx_content_off]
...
static bool is_kernel_pointer(void *arg, size_t arg_len) {
...
static void log_context_cursor_advance(os_log_context_t ctx, size_t amount) {
  ctx->ctx_content_off += amount;
  assert(log_context_cursor(ctx) <= (ctx->ctx_buffer + ctx->ctx_buffer_sz));
}

static bool log_fits(os_log_context_t ctx, size_t data_size) {
  return (ctx->ctx_content_off + data_size) <= ctx->ctx_content_sz;
}

static bool log_fits_cmd(os_log_context_t ctx, size_t data_size) {
  return log_fits(ctx, sizeof(*ctx->ctx_hdr) + data_size);
}

static void log_range_update(os_log_fmt_range_t range, uint16_t offset,
                             uint16_t length) {
  range->offset = offset;
  /*
 ...
 */
static void log_add_cmd(os_log_context_t ctx, os_log_fmt_cmd_type_t type,
                        uint8_t flags, void *arg, size_t arg_size) {
 ...
 * section.
 */
static void log_collect_public_range_data(os_log_context_t ctx,
                                          os_log_fmt_range_t range, void *arg) {
  ctx->ctx_pubdata[ctx->ctx_pubdata_cnt++] = (char *)arg;
  ctx->ctx_pubdata_sz += range->length;
}

static void log_add_range_data(os_log_context_t ctx, os_log_fmt_range_t range,
                               void *arg) {
...
static struct os_log_fmt_range_s log_create_range(os_log_context_t ctx,
                                                  size_t arg_len) {
  const size_t final_arg_len = MIN(arg_len, UINT16_MAX);

  return (struct os_log_fmt_range_s){.offset = ctx->ctx_pubdata_sz,
                                     .length = (uint16_t)final_arg_len,
                                     .truncated = (final_arg_len < arg_len)};
}

static int log_add_range_arg(os_log_context_t ctx, os_log_fmt_cmd_type_t type,
                             os_log_fmt_cmd_flags_t flags, void *arg,
                             size_t arg_len) {
 ...
 */
static int log_add_arg(os_log_context_t ctx, os_log_fmt_cmd_type_t type,
...
static void log_encode_public_data(os_log_context_t ctx) {
  ...
  for (int i = 0, pub_i = 0; i < hdr->hdr_cmd_cnt;
       i++, cmd = (os_log_fmt_cmd_t)(cmd->cmd_data + cmd->cmd_size)) {
    if (cmd->cmd_type != OSLF_CMD_TYPE_STRING) {
      continue;
    }

    os_log_fmt_range_t const range __attribute__((aligned(8))) =
        (os_log_fmt_range_t)&cmd->cmd_data;

    // Fix offset and length of the argument data in the hdr.
    log_range_update(
        range, ctx->ctx_content_off - orig_content_off,
...
static bool log_expand(os_log_context_t ctx, size_t new_size) {
  ...
  const size_t copy_size =
      hdr_size + sizeof(*ctx->ctx_hdr) + ctx->ctx_content_sz;
  ...
  ctx->ctx_content_sz =
      (uint16_t)(ctx->ctx_buffer_sz - hdr_size - sizeof(*ctx->ctx_hdr));
...
static int log_encode_fmt_arg(void *arg, size_t arg_len,
...
static int log_encode_fmt(os_log_context_t ctx, const char *format,
                          va_list args) {
  ...
  while ((position = strchr(position, '%'))) {
    ...
    for (bool done = false; !done; position++) {
      ...
      case '.': // precision
        ...
        err = log_encode_fmt_arg(&precision, sizeof(precision),
                                 OSLF_CMD_TYPE_COUNT, ctx);
      ...
      case 'X': // upper-hex
        ...
        case OST_CHAR:
          ...
          err = log_encode_fmt_arg(&value.ch, sizeof(value.ch),
                                   OSLF_CMD_TYPE_SCALAR, ctx);
          break;

        case OST_SHORT:
          value.s = (short)va_arg(args, int);
          err = log_encode_fmt_arg(&value.s, sizeof(value.s),
                                   OSLF_CMD_TYPE_SCALAR, ctx);
          break;

        case OST_INT:
          value.i = va_arg(args, int);
          err = log_encode_fmt_arg(&value.i, sizeof(value.i),
                                   OSLF_CMD_TYPE_SCALAR, ctx);
          break;

        case OST_LONG:
          value.l = va_arg(args, long);
          err = log_encode_fmt_arg(&value.l, sizeof(value.l),
                                   OSLF_CMD_TYPE_SCALAR, ctx);
          break;

        case OST_LONGLONG:
          value.ll = va_arg(args, long long);
          err = log_encode_fmt_arg(&value.ll, sizeof(value.ll),
                                   OSLF_CMD_TYPE_SCALAR, ctx);
          break;

        case OST_SIZE:
          value.z = va_arg(args, size_t);
          err = log_encode_fmt_arg(&value.z, sizeof(value.z),
                                   OSLF_CMD_TYPE_SCALAR, ctx);
          break;

        case OST_INTMAX:
          value.im = va_arg(args, intmax_t);
          err = log_encode_fmt_arg(&value.im, sizeof(value.im),
                                   OSLF_CMD_TYPE_SCALAR, ctx);
          break;

        case OST_PTRDIFF:
          value.pd = va_arg(args, ptrdiff_t);
          err = log_encode_fmt_arg(&value.pd, sizeof(value.pd),
                                   OSLF_CMD_TYPE_SCALAR, ctx);
          break;
      ...
      case 'p': // pointer
        ...
        err = log_encode_fmt_arg(&value.p, sizeof(value.p),
                                 OSLF_CMD_TYPE_SCALAR, ctx);
        done = true;
        break;
      ...
      case 's': // string
        ...
        err = log_encode_fmt_arg(value.pch, str_length, OSLF_CMD_TYPE_STRING,
                                 ctx);
        done = true;
        break;
      ...
      case 'f':
        ...
        err = log_encode_fmt_arg(&value.f, sizeof(value.f),
                                 OSLF_CMD_TYPE_SCALAR, ctx);
        done = true;
        break;
...
OS_ALWAYS_INLINE
static inline void tracepoint_buf_add(tracepoint_buf_t *tp, const void *data,
                                      size_t size) {
...
static void tracepoint_buf_location(tracepoint_buf_t *tpb, uintptr_t loc,
                                    size_t loc_size) {
...
static void os_log_context_prepare_header(os_log_context_t ctx,
                                          size_t hdr_size) {
 ...
 */
bool os_log_context_encode(os_log_context_t ctx, const char *fmt, va_list args,
                           uintptr_t loc, size_t loc_size,
                           uint16_t subsystem_id) {
  tracepoint_buf_t tpb = {.tp_buf = ctx->ctx_buffer, .tp_size = 0};
  ...
  if (!log_fits(ctx, ctx->ctx_pubdata_sz) && logmem_ready(ctx->ctx_logmem)) {
    size_t space_needed =
        log_context_cursor(ctx) + ctx->ctx_pubdata_sz - ctx->ctx_buffer;
...
void os_log_context_init(os_log_context_t ctx, logmem_t *logmem,
...
void os_log_context_free(os_log_context_t ctx) {
  if (ctx->ctx_allocated) {
    logmem_free_locked(ctx->ctx_logmem, ctx->ctx_buffer, ctx->ctx_buffer_sz);
  }
}
