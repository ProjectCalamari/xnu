...
#error                                                                         \
    "Do not include <os/atomic_private_impl.h> directly, use <os/atomic_private.h>"
#endif
...
static inline int _os_atomic_mo_has_acquire(OS_ATOMIC_STD memory_order ord) {
  switch (ord) {
...
static inline int _os_atomic_mo_has_release(OS_ATOMIC_STD memory_order ord) {
  switch (ord) {
...
#define _os_atomic_value_cast(p, v)                                            \
  ({                                                                           \
    __typeof__(*os_cast_to_nonatomic_pointer(p)) ___v = (v);                   \
    ___v;                                                                      \
  })
...
#define _os_atomic_c11_op_orig(p, v, m, o)                                     \
  ({                                                                           \
    ...
    __auto_type _r = os_atomic_std(atomic_##o##_explicit)(                     \
        os_cast_to_atomic_pointer(p), _os_atomic_value_cast(p, v),             \
...
#define _os_atomic_clang_op_orig(p, v, m, o)                                   \
  ({                                                                           \
    _os_compiler_barrier_before_atomic(m);                                     \
    __auto_type _r =                                                           \
        __atomic_##o(os_cast_to_nonatomic_pointer(p),                          \
                     _os_atomic_value_cast(p, v), _os_atomic_mo_##m##_smp);    \
    _os_compiler_barrier_after_atomic(m);                                      \
    _r;                                                                        \
  })
