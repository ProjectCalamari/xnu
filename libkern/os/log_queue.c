...
OS_ENUM(log_queue_entry_state, uint8_t, LOG_QUEUE_ENTRY_STATE_INVALID = 0,
        LOG_QUEUE_ENTRY_STATE_STORED, LOG_QUEUE_ENTRY_STATE_DISPATCHED,
        LOG_QUEUE_ENTRY_STATE_SENT, LOG_QUEUE_ENTRY_STATE_FAILED);

OS_ENUM(lq_mem_state, uint8_t, LQ_MEM_STATE_READY = 0, LQ_MEM_STATE_ALLOCATING,
        LQ_MEM_STATE_RELEASING);

OS_ENUM(lq_req_state, uint8_t, LQ_REQ_STATE_INVALID = 0,
        LQ_REQ_STATE_ALLOCATING, LQ_REQ_STATE_RELEASING, LQ_REQ_STATE_READY);
...
SYSCTL_UINT(_debug, OID_AUTO, log_queue_max_slots, CTLFLAG_RW,
            (unsigned int *)&lq_max_slots, 0, "");
#endif
...
static void *log_queue_buffer_alloc(size_t amount) {
  return kalloc_data_tag(amount, Z_WAITOK_ZERO, VM_KERN_MEMORY_LOG);
}

static void log_queue_buffer_free(void *addr, size_t amount) {
  kfree_data(addr, amount);
}

static void log_queue_increment_mem_avail(const log_queue_t lq, size_t idx,
                                          size_t size) {
...
static void log_queue_decrement_mem_avail(const log_queue_t lq, size_t idx,
                                          size_t size) {
  lq->lq_cnt_mem_avail -= size;
  if (idx < LQ_OTHER_START_SLOT) {
    lq->lq_cnt_mem_meta_avail -= size;
...
#define read_dependent(v, t)                                                   \
  os_atomic_load_with_dependency_on((v), (uintptr_t)(t))
...
static log_queue_entry_t log_queue_entry_alloc(log_queue_t lq, size_t lqe_size,
                                               firehose_stream_t stream_type) {
...
static void log_queue_entry_free(log_queue_t lq, log_queue_entry_t lqe) {
...
static bool log_queue_add_entry(log_queue_t lq, log_payload_t lp,
                                const uint8_t *lp_data) {
  log_queue_entry_t lqe =
      log_queue_entry_alloc(lq, log_queue_entry_size(lp), lp->lp_stream);
  if (!lqe) {
  ...
  lqe->lqe_payload = *lp;
  (void)memcpy((uint8_t *)lqe + sizeof(*lqe), lp_data,
               lqe->lqe_payload.lp_data_size);
 ...
 */
static size_t dispatch_list_cleanup(log_queue_t lq) {
 ...
 * logs.
 */
static size_t log_dispatch_prepare(log_queue_t lq, size_t requested,
                                   log_queue_entry_t *buf) {
  log_queue_entry_t lqe, lqe_tmp;
  size_t collected = 0;
 ...
 * order.
 */
static void log_queue_dispatch_logs(size_t logs_count,
                                    log_queue_entry_t *logs) {
  for (size_t i = 0; i < logs_count; i++) {
    ...
    if (lqe_state == LOG_QUEUE_ENTRY_STATE_DISPATCHED) {
      ...
      log_payload_s lp = {
          ...
          .lp_pub_data_size =
              read_dependent(&lqe_lp->lp_pub_data_size, lqe_state),
...
static bool log_queue_empty(const log_queue_t lq) {
  return STAILQ_EMPTY(&lq->lq_log_list) && STAILQ_EMPTY(&lq->lq_dispatch_list);
}

static boolean_t log_queue_low_mem(const log_queue_t lq) {
  size_t mem_avail = lq->lq_cnt_mem_avail - lq->lq_cnt_mem_meta_avail;
  size_t low_mem_threshold =
      (lq->lq_cnt_mem_active - LQ_OTHER_START_SLOT) * lq_low_mem_limit;
  return mem_avail < low_mem_threshold;
}

static lq_req_state_t log_queue_request_state(log_queue_t lq) {
  lq_req_state_t req_state = read_dependency(&lq->lq_req_state);
  return req_state;
}

static void log_queue_mem_init(log_queue_t lq, size_t idx, void *buf,
                               size_t buflen) {
  assert(buf);
  assert(buflen > 0);
...
void log_queue_set_max_slots(size_t max_slots) {
  ...
  atomic_store_explicit((atomic_size_t *)&lq_max_slots, max_slots,
                        memory_order_relaxed);
}

static size_t log_queue_mem_max_slots(void) {
  size_t max_slots = atomic_load_explicit((atomic_size_t *)&lq_max_slots,
                                          memory_order_relaxed);
  max_slots = MAX(max_slots, LQ_MIN_ALLOCATED_LM_SLOTS);
  assert(max_slots <= LQ_MAX_LM_SLOTS);
  assert(max_slots >= LQ_MIN_ALLOCATED_LM_SLOTS);
...
static int log_queue_mem_free_slot(log_queue_t lq) {
...
static void log_queue_memory_handler(thread_call_param_t a0,
                                     __unused thread_call_param_t a1) {
...
static void log_queue_order_memory(log_queue_t lq) {
...
static void log_queue_release_memory(log_queue_t lq, void *buf) {
  boolean_t __assert_only running;
...
static void log_queue_mem_enable(log_queue_t lq, size_t i) {
...
static void log_queue_mem_disable(log_queue_t lq, size_t i) {
  logmem_t *lm = &lq->lq_mem[i];
  assert(LQ_MEM_ENABLED(lq, i));
...
static void *log_queue_mem_reclaim(log_queue_t lq) {
...
static void log_queue_mem_reconfigure(log_queue_t lq) {
...
static boolean_t log_queue_needs_memory(log_queue_t lq, boolean_t new_suspend) {
...
static boolean_t log_queue_can_release_memory(log_queue_t lq) {
...
static boolean_t detect_new_suspend(log_queue_t lq) {
...
static void log_queue_dispatch(void) {
  ...
  size_t logs_count =
      log_dispatch_prepare(lq, LQ_BATCH_SIZE, (log_queue_entry_t *)&logs);
...
static bool log_queue_add(log_payload_t lp, const uint8_t *lp_data) {
...
__startup_func static size_t log_queue_init_memory(log_queue_t lq) {
  lq->lq_cnt_mem_max = log_queue_mem_max_slots();
  assert(lq->lq_cnt_mem_max <= LQ_MAX_LM_SLOTS);
...
__startup_func static void oslog_init_log_queues(void) {
  if (os_log_disabled()) {
  ...
  percpu_foreach(lq, oslog_queue) {
    ...
    lq->lq_mem_size = round_page(
        logmem_required_size(lq->lq_mem_size_order, LQ_MIN_LOG_SZ_ORDER));
    lq->lq_mem_handler =
        thread_call_allocate(log_queue_memory_handler, (thread_call_param_t)lq);
  ...
  printf("Log queues configured: slot count: %u, per-slot size: %u, total "
         "size: %u\n",
...
bool log_queue_log(log_payload_t lp, const void *lp_data, bool stream) {
  assert(lp);
