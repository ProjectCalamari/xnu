...
extern void oslog_stream(bool, firehose_tracepoint_id_u, uint64_t, const void *,
                         size_t);
extern void *OSKextKextForAddress(const void *);

static bool os_log_safe(void) {
  return oslog_is_safe() || startup_phase < STARTUP_SUB_EARLY_BOOT;
}

static bool os_log_turned_off(void) {
  return oslog_disabled || (atm_get_diagnostic_config() & ATM_TRACE_OFF);
}

bool os_log_info_enabled(os_log_t log __unused) { return !os_log_turned_off(); }

bool os_log_debug_enabled(os_log_t log __unused) {
  return !os_log_turned_off();
}

bool os_log_disabled(void) { return oslog_disabled; }

static inline size_t log_payload_priv_data_size(const log_payload_s *lp) {
  assert3u(lp->lp_pub_data_size, <=, lp->lp_data_size);
  return lp->lp_data_size - lp->lp_pub_data_size;
}

static inline const uint8_t *log_payload_priv_data(const log_payload_s *lp,
                                                   const uint8_t *lp_data) {
...
static void log_payload_init(log_payload_t lp, firehose_stream_t stream,
                             firehose_tracepoint_id_u ftid, uint64_t timestamp,
                             size_t data_size, size_t pub_data_size) {
...
static firehose_stream_t firehose_stream(os_log_type_t type) {
  return (type == OS_LOG_TYPE_INFO || type == OS_LOG_TYPE_DEBUG)
             ? firehose_stream_memory
             : firehose_stream_persist;
}

static firehose_tracepoint_id_t firehose_ftid(os_log_type_t type,
                                              const char *fmt,
  ...
  if (driverKit) {
    ...
    off = _os_trace_offset(dso, fmt,
                           (_firehose_tracepoint_flags_activity_t)flags);
  }

  return FIREHOSE_TRACE_ID_MAKE(firehose_tracepoint_namespace_log, type, flags,
                                off);
}

static firehose_tracepoint_flags_t firehose_ftid_flags(const void *dso,
                                                       bool driverKit) {
...
static void *resolve_dso(const char *fmt, void *dso, void *addr,
                         bool driverKit) {
  kc_format_t kcformat = KCFormatUnknown;
...
static inline uintptr_t resolve_location(firehose_tracepoint_flags_t flags,
                                         uintptr_t dso, uintptr_t addr,
...
__startup_func static void oslog_init(void) {
  /*
...
__startup_func static void oslog_init_logmem(void) {
  if (os_log_disabled()) {
    printf("Long logs support disabled: Logging disabled by ATM\n");
    return;
  }

  const size_t logmem_size =
      logmem_required_size(OS_LOGMEM_BUF_ORDER, OS_LOGMEM_MIN_LOG_ORDER);
  vm_offset_t addr;

  if (kmem_alloc(kernel_map, &addr, logmem_size + ptoa(2),
                 KMA_KOBJECT | KMA_PERMANENT | KMA_ZERO | KMA_GUARD_FIRST |
                     KMA_GUARD_LAST,
                 VM_KERN_MEMORY_LOG) == KERN_SUCCESS) {
    logmem_init(&os_log_mem, (void *)(addr + PAGE_SIZE), logmem_size,
                OS_LOGMEM_BUF_ORDER, OS_LOGMEM_MIN_LOG_ORDER,
                OS_LOGMEM_MAX_LOG_ORDER);
...
bool os_log_encoded_metadata(firehose_tracepoint_id_u ftid, uint64_t ts,
  ...
  log_payload_s log;
  log_payload_init(&log, firehose_stream_metadata, ftid, ts, msg_size,
                   msg_size);
...
bool os_log_encoded_log(firehose_stream_t stream, firehose_tracepoint_id_u ftid,
                        uint64_t ts, const void *msg, size_t msg_size,
...
bool os_log_encoded_signpost(firehose_stream_t stream,
                             firehose_tracepoint_id_u ftid, uint64_t ts,
                             const void *msg, size_t msg_size,
                             size_t pub_data_size) {
...
static inline size_t os_log_subsystem_name_size(const char *name) {
  return MIN(strlen(name) + 1, OS_LOG_SUBSYSTEM_NAME_MAX_LEN);
}

bool os_log_subsystem_id_valid(uint16_t sid) {
...
static void os_log_subsystem_init(struct os_log_subsystem_s *ols, uint16_t sid,
                                  const char *sub, const char *cat) {
  ...
  strlcpy(&ols->ols_name[ols->ols_sub_size], cat,
          OS_LOG_SUBSYSTEM_NAME_MAX_LEN);
  ols->ols_id = sid;
}

static void os_log_subsystem_register(const struct os_log_subsystem_s *ols,
                                      uint64_t stamp) {
  assert(os_log_subsystem_id_valid(ols->ols_id));

  firehose_tracepoint_id_u trace_id;
  trace_id.ftid_value = FIREHOSE_TRACE_ID_MAKE(
...
static bool os_log_valid(const struct os_log_s *log) {
  return os_log_subsystem_id_valid(log->ol_subsystem.ols_id);
}
#endif // DEVELOPMENT || DEBUG

static smrh_key_t os_log_key(const struct os_log_subsystem_s *sub) {
  return (smrh_key_t){.smrk_opaque = sub->ols_name,
                      .smrk_len = os_log_subsystem_id_length(sub)};
}

static uint32_t os_log_key_hash(smrh_key_t key, uint32_t seed) {
  return smrh_key_hash_mem(key, seed);
}

static bool os_log_key_equ(smrh_key_t k1, smrh_key_t k2) {
  return k1.smrk_len == k2.smrk_len ? smrh_key_equ_mem(k1, k2) : false;
}

static uint32_t os_log_hash(const struct smrq_slink *link, uint32_t seed) {
...
static bool os_log_equ(const struct smrq_slink *link, smrh_key_t key) {
  const os_log_t h = __container_of(link, struct os_log_s, ol_hash_link);
  return os_log_key_equ(os_log_key(&h->ol_subsystem), key);
}

static bool oslog_try_get(void *oslog __unused) { return true; }

SMRH_TRAITS_DEFINE(os_log_cache_traits, struct os_log_s, ol_hash_link,
                   .domain = &smr_oslog, .key_hash = os_log_key_hash,
                   .key_equ = os_log_key_equ, .obj_hash = os_log_hash,
                   .obj_equ = os_log_equ, .obj_try_get = oslog_try_get);

static os_log_t os_log_cache_find_by_sub(const struct os_log_subsystem_s *sub) {
  os_log_t log =
      smr_hash_get(&os_log_cache, os_log_key(sub), &os_log_cache_traits);
  assert(!log || os_log_valid(log));
  return log;
}

static os_log_t os_log_cache_insert(os_log_t log) {
  assert(os_log_subsystem_id_valid(log->ol_subsystem.ols_id));

  os_log_t found = smr_hash_serialized_get_or_insert(
      &os_log_cache, os_log_key(&log->ol_subsystem), &log->ol_hash_link,
      &os_log_cache_traits);
  assert(!found || os_log_valid(found));
  return found;
}

static os_log_t os_log_allocate(const struct os_log_subsystem_s *subsystem) {
...
static uint16_t os_log_subsystem_id_next(void) {
...
static void os_log_subsystem_id_revert(uint16_t __assert_only sid) {
  assert(os_log_subsystem_id_valid(os_log_subsystem_id));
  assert3u(os_log_subsystem_id, ==, sid + 1);
  os_log_subsystem_id--;
}

static os_log_t os_log_create_impl(const char *subsystem, const char *category,
                                   uint64_t stamp) {
  if (os_log_disabled()) {
...
static void _os_log_to_msgbuf_internal(const char *format, va_list args,
                                       uint64_t timestamp, bool safe,
                                       bool logging, bool addcr) {
  /*
...
static void _os_log_to_log_internal(uint16_t sid, os_log_type_t type,
                                    const char *fmt, va_list args, uint64_t ts,
                                    void *addr, void *dso, bool driverKit) {
  ...
  __attribute__((uninitialized,
                 aligned(8))) uint8_t buffer[OS_LOG_BUFFER_MAX_SIZE];
...
static void _os_log_with_args_internal(os_log_t oslog, os_log_type_t type,
                                       const char *fmt, va_list args,
                                       uint64_t ts, void *addr, void *dso,
                                       bool driverKit, bool addcr) {
  counter_inc(&oslog_p_total_msgcount);
...
__attribute__((noinline, not_tail_called)) void
_os_log_at_time(void *dso, os_log_t log, uint8_t type, uint64_t ts,
                const char *fmt, ...) {
  void *addr = __builtin_return_address(0);
  va_list args;
...
_os_log_internal_driverKit(void *dso, os_log_t log, uint8_t type,
                           const char *fmt, ...) {
  uint64_t ts = firehose_tracepoint_time(firehose_activity_flags_default);
  void *addr = __builtin_return_address(0);
  ...
  va_start(args, fmt);
  _os_log_with_args_internal(log, type, fmt, args, ts, addr, dso, driverKitLog,
                             true);
  va_end(args);
...
os_log_with_args(os_log_t oslog, os_log_type_t type, const char *fmt,
  ...
  _os_log_with_args_internal(oslog, type, fmt, args, ts, addr, NULL, FALSE,
                             FALSE);
}
...
bool os_log_coprocessor(void *buff, uint64_t buff_len, os_log_type_t type,
  ...
  __attribute__((uninitialized)) uint8_t pubdata[OS_LOG_BUFFER_MAX_SIZE];
  size_t wr_pos = 0;
  ...
  log_payload_s log;
  log_payload_init(&log, firehose_stream(type), trace_id, timestamp, wr_pos,
                   wr_pos);
...
firehose_trace_start(const log_payload_s *lp, uint8_t **ft_priv_data) {
  ...
  assert3u(offsetof(struct firehose_tracepoint_s, ft_data) + pub_data_size +
               priv_data_size,
           <=, sizeof(((firehose_chunk_t)0)->fc_data));

  firehose_tracepoint_t ft = __firehose_buffer_tracepoint_reserve(
      lp->lp_timestamp, lp->lp_stream, pub_data_size, priv_data_size,
      ft_priv_data);
  ...
  long offset = firehose_chunk_tracepoint_try_reserve(
      &firehose_boot_chunk, lp->lp_timestamp, firehose_stream_persist, 0,
      pub_data_size, priv_data_size, ft_priv_data);
  if (offset <= 0) {
    counter_inc(&oslog_p_boot_dropped_msgcount);
    return 0;
  }

  return firehose_chunk_tracepoint_begin(&firehose_boot_chunk, lp->lp_timestamp,
                                         pub_data_size,
                                         thread_tid(current_thread()), offset);
}

static inline void firehose_trace_finish(firehose_tracepoint_t ft,
                                         firehose_tracepoint_id_u ftid) {
...
static inline void firehose_trace_save_payload(const log_payload_s *lp,
                                               const uint8_t *lp_data,
                                               firehose_tracepoint_t ft,
                                               uint8_t *ft_priv_data) {
...
static inline void log_stats_update_saved(firehose_stream_t stream) {
...
static inline firehose_tracepoint_id_t firehose_trace(const log_payload_s *lp,
                                                      const uint8_t *lp_data) {
...
void os_log_coprocessor_register_with_type(const char *uuid,
  ...
  __attribute__((uninitialized)) union {
...
bool log_payload_send(log_payload_t lp, const void *lp_data, bool use_stream) {
  if (use_stream) {
    bool is_metadata = (lp->lp_stream == firehose_stream_metadata);
    oslog_stream(is_metadata, lp->lp_ftid, lp->lp_timestamp, lp_data,
                 lp->lp_data_size);
  }
  return firehose_trace(lp, lp_data) != 0;
}

void __firehose_buffer_push_to_logd(firehose_buffer_t fb __unused,
                                    bool for_io __unused) {
  oslogwakeup();
  return;
}

void __firehose_allocate(vm_offset_t *addr, vm_size_t size __unused) {
  ...
  // Now that we are done adding logs to this chunk, set the number of writers
  // to 0 Without this, logd won't flush when the page is full
  firehose_boot_chunk.fc_pos.fcp_refcnt = 0;
  memcpy(&kernel_buffer[FIREHOSE_BUFFER_KERNEL_CHUNK_COUNT - 1],
         (const void *)&firehose_boot_chunk, FIREHOSE_CHUNK_SIZE);
  return;
}
...
#define GENOSLOGHELPER(fname, ident, callout_f)                                \
  static void fname(uint32_t uniqid, uint64_t count, log_stats_t *before,      \
                    log_stats_t *after) {                                      \
    uint32_t checksum = 0;                                                     \
    ...
    for (uint64_t i = 0; i < count; i++) {                                     \
      (void)save_pattern(pattern, &checksum, TESTOSLOGFMT(ident), uniqid,      \
                         i + 1, count);                                        \
      callout_f(OS_LOG_DEFAULT, TESTOSLOG(ident), checksum, uniqid, i + 1,     \
                count);                                                        \
    }                                                                          \
    log_stats_get(after);                                                      \
  }
...
static void log_stats_get(log_stats_t *stats) {
  if (!stats) {
    return;
  }
...
static void log_stats_diff(const log_stats_t *before, const log_stats_t *after,
                           log_stats_t *diff) {
  log_stats_t d = {};
...
static void log_stats_check_errors(const log_stats_t *before,
                                   const log_stats_t *after,
                                   const log_stats_t *expected,
                                   const char *test) {
  LOG_STAT_EQ(before, after, expected, errors,
              "%s: Expected %lu encoding errors", test);
  LOG_STAT_EQ(before, after, expected, errors_kc, "%s: Expected %lu DSO errors",
              test);
  LOG_STAT_EQ(before, after, expected, errors_fmt,
              "%s: Expected %lu bad format errors", test);
  LOG_STAT_EQ(before, after, expected, errors_max_args,
              "%s: Expected %lu max arguments errors", test);
}

static void log_stats_check(const log_stats_t *before, const log_stats_t *after,
                            const log_stats_t *expected, const char *test) {
  /*
   ...
   */
  LOG_STAT_GE(before, after, expected, total, "%s: Expected %lu logs total",
              test);
  LOG_STAT_GE(before, after, expected, saved, "%s: Expected %lu saved logs",
              test);
  LOG_STAT_EQ(before, after, expected, dropped, "%s: Expected %lu logs dropped",
              test);
  log_stats_check_errors(before, after, expected, test);
}

static void log_stats_report(const log_stats_t *before,
                             const log_stats_t *after, const char *test) {
  log_stats_t diff = {};
  ...
  T_LOG(
      "\n%s: Logging stats:\n\ttotal: %u\n\tsaved: %u\n\tdropped: %u\n"
      "\terrors: %u\n\terrors_kc: %u\n\terrors_fmt: %u\n\terrors_max_args: %u",
      test, diff.total, diff.saved, diff.dropped, diff.errors, diff.errors_kc,
      diff.errors_fmt, diff.errors_max_args);
}

static int save_pattern(char buf[static TESTBUFLEN], uint32_t *crc,
                        const char *fmt, ...) {
  va_list va;
...
kern_return_t test_printf(void) {
  ...
  __attribute__((uninitialized)) char pattern[TESTBUFLEN];
  log_stats_t before = {}, after = {};
  uint32_t checksum = 0;

  log_stats_get(&before);

  (void)save_pattern(pattern, &checksum, TESTOSLOGFMT("printf_only"), uniqid,
                     1LL, 2LL);
  printf(TESTOSLOG("printf_only") "mat%llu\n", checksum, uniqid, 1LL, 2LL,
         stamp);

  (void)save_pattern(pattern, &checksum, TESTOSLOGFMT("printf_only"), uniqid,
                     2LL, 2LL);
  printf(TESTOSLOG("printf_only") "mat%llu\n", checksum, uniqid, 2LL, 2LL,
         stamp);

  size_t saved =
      save_pattern(pattern, NULL, "kernel^0^test^printf_only#mat%llu", stamp);
...
static void verify_os_log(struct os_log_s *h, char *sub, char *cat) {
  ...
  T_ASSERT_EQ_ULONG(s->ols_sub_size, sub_len, "Log subsystem size is %lu",
                    sub_len);
  T_ASSERT_EQ_ULONG(strlen(s->ols_name), sub_len - 1,
                    "Log subsystem length is %lu", sub_len - 1);

  T_ASSERT_EQ_STR(&s->ols_name[sub_len], cat, "Log category name is %s", cat);
  T_ASSERT_EQ_ULONG(s->ols_cat_size, cat_len, "Log category length is %lu",
                    cat_len);
  T_ASSERT_EQ_ULONG(strlen(&s->ols_name[sub_len]), cat_len - 1,
                    "Log category size is %lu", cat_len - 1);
}

static os_log_t create_verified_os_log(char *sub, char *cat) {
...
kern_return_t test_os_log_handles(void) {
  os_log_t h1 =
      create_verified_os_log("xnu_post_subsystem1", "xnu_post_category1");
  if (os_log_disabled()) {
    T_ASSERT_EQ_PTR(h1, OS_LOG_DISABLED,
                    "Disabled logging uses OS_LOG_DISABLED");
    return KERN_SUCCESS;
  }
  T_ASSERT_NE_PTR(h1, OS_LOG_DEFAULT,
                  "Custom log handle is not OS_LOG_DEFAULT");

  os_log_t h2 =
      create_verified_os_log("xnu_post_subsystem1", "xnu_post_category1");
  T_ASSERT_EQ_PTR(h1, h2,
                  "os_log_create() finds an existing log handle in the cache");
  ...
  T_ASSERT_NE_PTR(h1, h2,
                  "Different subsystem and category yield a different handle");
  ...
  char super_long_sub[name_size] = {[0 ... sizeof(super_long_sub) - 1] = 'X'};

  char super_long_cat[name_size] = {[0 ... sizeof(super_long_sub) - 1] = 'Y'};
...
kern_return_t test_os_log(void) {
  ...
  T_ASSERT_NE_PTR(OS_LOG_DEFAULT, log_handle,
                  "Log handle is not OS_LOG_DEFAULT");

  const bool enabled = !os_log_turned_off();
  T_ASSERT_EQ_INT(enabled, os_log_info_enabled(log_handle),
                  "Info log level is enabled");
  T_ASSERT_EQ_INT(enabled, os_log_debug_enabled(log_handle),
                  "Debug log level is enabled");

  __attribute__((uninitialized)) char pattern[TESTBUFLEN];
  uint32_t checksum = 0;

  (void)save_pattern(pattern, &checksum, TESTOSLOGFMT("oslog_info"), uniqid,
                     1LL, 1LL);
  os_log_info(log_handle, TESTOSLOG("oslog_info") "mat%llu", checksum, uniqid,
              1LL, 1LL, stamp);
  size_t saved =
      save_pattern(pattern, NULL, "kernel^0^test^oslog_info#mat%llu", stamp);
  size_t match_count = find_pattern_in_buffer(pattern, saved, 1LL);
  T_EXPECT_EQ_ULONG(match_count, 1LL,
                    "oslog_info() logs to system message buffer");

  const size_t n = 10;

  log_stats_t expected = {.total = enabled ? n : 0, .saved = enabled ? n : 0};
...
static void _test_log_loop(void *arg __unused, wait_result_t wres __unused) {
...
kern_return_t test_os_log_parallel(void) {
  ...
  T_ASSERT_EQ_INT(kr, KERN_SUCCESS,
...
static kern_return_t test_stresslog_dropmsg(const uint32_t uniqid) {
  if (os_log_turned_off()) {
    T_LOG("Logging disabled, skipping tests.");
    return KERN_SUCCESS;
  }
