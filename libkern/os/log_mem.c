...
#define BITMAP_BIT(i)                                                          \
  (uint8_t)(1 << (BITMAP_BUCKET_SIZE - ((i) % BITMAP_BUCKET_SIZE) - 1))

#define logmem_lock(lock, lm)                                                  \
  if ((lock))                                                                  \
  lck_spin_lock(&(lm)->lm_lock)
#define logmem_unlock(lock, lm)                                                \
  if ((lock))                                                                  \
  lck_spin_unlock(&(lm)->lm_lock)

static LCK_GRP_DECLARE(logmem_lck_grp, "logmem_lck_grp");

static bool bitmap_get(logmem_t *lm, size_t block) {
  return lm->lm_mem_map[BITMAP_BUCKET(block)] & BITMAP_BIT(block);
}

static void bitmap_set(logmem_t *lm, size_t block) {
  lm->lm_mem_map[BITMAP_BUCKET(block)] |= BITMAP_BIT(block);
}

static void bitmap_clear(logmem_t *lm, size_t block) {
  lm->lm_mem_map[BITMAP_BUCKET(block)] &= ~BITMAP_BIT(block);
}

static void bitmap_reserve_root(logmem_t *lm, size_t block) {
  const size_t top_block =
      BLOCK_LEVEL_BASE(lm->lm_cap_order - lm->lm_max_order);

  for (ssize_t next = BLOCK_PARENT(block); next >= top_block;
       next = BLOCK_PARENT(next)) {
    /*
...
static void bitmap_release_root(logmem_t *lm, size_t block) {
  const size_t top_block =
      BLOCK_LEVEL_BASE(lm->lm_cap_order - lm->lm_max_order);
...
static void bitmap_update_subtree(logmem_t *lm, size_t level, size_t block,
                                  void (*fun)(logmem_t *, size_t)) {
...
static void bitmap_release_subtree(logmem_t *lm, size_t level, size_t block) {
  bitmap_update_subtree(lm, level, block, bitmap_clear);
}

static void bitmap_reserve_subtree(logmem_t *lm, size_t level, size_t block) {
  bitmap_update_subtree(lm, level, block, bitmap_set);
}

static size_t block_size_level(logmem_t *lm, size_t amount) {
...
static size_t block_locate(logmem_t *lm, void *addr, size_t amount,
                           size_t *block) {
...
static size_t block_reserve(logmem_t *lm, size_t level) {
...
static void *logmem_alloc_impl(logmem_t *lm, size_t *amount, bool lock_lm) {
  assert(amount);
...
void *logmem_alloc_locked(logmem_t *lm, size_t *amount) {
  return logmem_alloc_impl(lm, amount, true);
}

void *logmem_alloc(logmem_t *lm, size_t *amount) {
  return logmem_alloc_impl(lm, amount, false);
}

static void logmem_free_impl(logmem_t *lm, void *addr, size_t amount,
                             bool lock_lm) {
  assert(addr);
...
void logmem_free_locked(logmem_t *lm, void *addr, size_t amount) {
  logmem_free_impl(lm, addr, amount, true);
}

void logmem_free(logmem_t *lm, void *addr, size_t amount) {
  logmem_free_impl(lm, addr, amount, false);
}

size_t logmem_required_size(size_t size_order, size_t min_order) {
  return round_page(BLOCK_SIZE(size_order)) +
         round_page(MAP_SIZE(size_order, min_order));
}

size_t logmem_max_size(const logmem_t *lm) {
  return BLOCK_SIZE(lm->lm_max_order);
}

bool logmem_empty(const logmem_t *lm) {
  return lm->lm_cnt_free == BLOCK_SIZE(lm->lm_cap_order);
}

bool logmem_ready(const logmem_t *lm) { return lm->lm_mem != NULL; }

void logmem_init(logmem_t *lm, void *lm_mem, size_t lm_mem_size,
                 size_t size_order, size_t min_order, size_t max_order) {
  ...
  lm->lm_mem = lm_mem;
  lm->lm_mem_map =
      (uint8_t *)((uintptr_t)lm_mem + round_page(BLOCK_SIZE(size_order)));
