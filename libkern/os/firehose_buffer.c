...
static firehose_chunk_t firehose_chunk_for_stream(firehose_stream_t stream) {
  if (stream >= _firehose_stream_max || !firehose_chunks ||
      !firehose_chunk_count) {
    return NULL;
  }

  /* One chunk per stream avoids changing a chunk's stream while a writer owns
   * it. */
...
firehose_buffer_t __firehose_buffer_create(size_t *size) {
  ...
  if (!size || !chunks ||
      !__firehose_kernel_configuration_valid(count,
