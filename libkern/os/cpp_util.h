...
template <class _T> struct remove_pointer<_T *> {
  typedef _T type;
};
template <class _T> struct remove_pointer<_T *const> {
  typedef _T type;
};
template <class _T> struct remove_pointer<_T *volatile> {
  typedef _T type;
};
template <class _T> struct remove_pointer<_T *const volatile> {
  typedef _T type;
};
...
template <class _T> struct remove_volatile<volatile _T> {
  typedef _T type;
};
template <class _T>
...
template <class _T> struct remove_extent<_T[]> {
  typedef _T type;
};
template <class _T, size_t N> struct remove_extent<_T[N]> {
  typedef _T type;
};
...
template <class _T> inline typename remove_reference<_T>::type &&move(_T &&_t) {
  typedef typename os::remove_reference<_T>::type _U;
  return static_cast<_U &&>(_t);
}

template <class T> T *move(T *first, T *last, T *d_first) {
...
template <class T> T *move_backward(T *first, T *last, T *d_last) {
  while (first != last) {
    *(--d_last) = os::move(*(--last));
  }
  return d_last;
...
template <class T> T *uninitialized_move(T *first, T *last, T *d_first) {
  for (; first != last; ++d_first, (void)++first) {
...
template <class T> void uninitialized_value_construct(T *first, T *last) {
  for (; first != last; ++first) {
