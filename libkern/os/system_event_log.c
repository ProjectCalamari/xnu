...
static const char *convert_subsystem_to_string(uint8_t subsystem) {
  switch (subsystem) {
...
static const char *convert_type_to_string(uint8_t type) {
  switch (type) {
 ...
 * A suggested improvement is adding events to an MPSC queue to log later on
 * (rdar://84678724).
 */
static bool is_recording_allowed(void) {
  return ml_get_interrupts_enabled() || panic_active();
}

static void _record_system_event_internal(uint8_t type, uint8_t subsystem,
                                          const char *event,
                                          const char *payload) {
  ...
  printf("[System Event] [%llu] [%s] [Subsystem: %s] [Event: %.*s] %s\n",
         timestamp, type_string, subsystem_string, SYSTEM_EVENT_EVENT_MAX,
         event, payload);
}
