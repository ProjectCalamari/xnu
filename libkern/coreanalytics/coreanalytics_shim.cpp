...
core_analytics_family_service_t *core_analytics_family_match() {
  OSSharedPtr<OSDictionary> dict =
      IOService::serviceMatching(kCoreAnalyticsMatchingClassName);
...
void core_analytics_family_release(core_analytics_family_service_t *service) {
  service->release();
}
 ...
 * We want to support both since the event may have been defined in C or C++
 * code.
 */
extern const char *core_analytics_ca_bool_c_stringified;
const char *core_analytics_ca_bool_cpp_stringified =
    _CA_STRINGIFY_EXPAND(CA_BOOL);

static OSSharedPtr<OSObject>
serialize_event_field(const char **field_spec, ptrdiff_t *event,
...
static int core_analytics_serialize_event(
  ...
  while (strlen(spec_curr) != 0) {
    ...
    OSSharedPtr<OSObject> object =
        serialize_event_field(&spec_curr, &event_curr, &key);
    if (!object) {
      return ENOMEM;
    }
...
int core_analytics_send_event_lazy(
  ...
  for (size_t i = 0; i < kMaxRetries; i++) {
    success = core_analytics_hub_functions->analytics_send_event_lazy(
        core_analytics_hub, serialized.case_event_name.get(),
        serialized.case_event.get());
    if (success) {
      break;
    } else {
      if (i != kMaxRetries - 1) {
...
void core_analytics_hub_register(core_analytics_hub_functions_t *fns) {
  if (fns->version != CORE_ANALYTICS_FUNCTIONS_TABLE_VERSION) {
    panic("CoreAnalyticsHub is out of sync with xnu. CoreAnalyticsHub table "
          "version: %d. xnu table version: %d",
          fns->version, CORE_ANALYTICS_FUNCTIONS_TABLE_VERSION);
  }
