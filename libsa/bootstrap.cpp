...
static void bootstrapDestroy(void) { sBootstrapObject.~KLDBootstrap(); }

__attribute__((
    used,
    section("__DATA,__mod_term_func"))) static void (*const
                                                         bootstrapDestructor)(
    void) = bootstrapDestroy;
#endif
...
/*********************************************************************
*********************************************************************/
void KLDBootstrap::readStartupExtensions(void) {
   ...
   */
  prelinkInfoSect =
      getsectbynamefromheader(mh, kPrelinkInfoSegment, kPrelinkInfoSection);
...
/*********************************************************************
*********************************************************************/
void KLDBootstrap::readPrelinkedExtensions(kernel_mach_header_t *mh,
                                           kc_kind_t type) {
  bool ret;
  ...
  if (type == KCKindAuxiliary) {
    if (OSKext::validateKCFileSetUUID(infoDict, KCKindAuxiliary) != 0) {
      OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
                "Early boot AuxKC  doesn't appear to be linked against the "
                "loaded BootKC.");
      return;
    }
     ...
     * the segment.
     */
    if (!OSKext::registerDeferredKextCollection(mh, parsedXML,
                                                KCKindAuxiliary)) {
      OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
                "Error deferring AuxKC kext processing: Kexts in this "
   ...
   */
  ret = OSKext::addKextsFromKextCollection(
      mh, infoDict, kPrelinkTextSegment, loaded_kcUUID,
      (mh->filetype == MH_FILESET) ? type : KCKindUnknown);
  ...
  if (!loaded_kcUUID) {
    OSKextLog(/* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogArchiveFlag,
              "WARNING: did not find UUID in %s KC!",
              (type == KCKindAuxiliary) ? "Aux" : "Primary");
  } else if (type != KCKindAuxiliary) {
    kernelcache_uuid_valid = TRUE;
    memcpy((void *)&kernelcache_uuid,
...
skip_adding_kexts:
  ...
  if (mh->filetype != MH_FILESET) {
    ...
    ml_static_mfree((vm_offset_t)prelinkTextSegment->vmaddr,
                    prelinkTextSegment->vmsize);
  }
...
void KLDBootstrap::readBooterExtensions(void) {
  ...
  OSKext::createExcludeListFromBooterData(propertyDict.get(),
                                          keyIterator.get());
#endif
  ...
  while ((deviceTreeName =
    ...
    OSData *deviceTreeEntry =
        OSDynamicCast(OSData, propertyDict->getObject(deviceTreeName));
    ...
    if (!deviceTreeBuffer) {
      ...
      OSKextLog(/* kext */ NULL,
                kOSKextLogErrorLevel | kOSKextLogDirectoryScanFlag,
                "Device tree entry %s has NULL pointer.", devTreeNameCString);
     ...
     */
    booterData =
        OSData::withBytesNoCopy(booterDataPtr, deviceTreeBuffer->length);
    if (!booterData) {
      OSKextLog(
          /* kext */ NULL, kOSKextLogErrorLevel | kOSKextLogGeneralFlag,
     ...
     */
    OSSharedPtr<OSKext> newKext =
        OSKext::withBooterData(deviceTreeName, booterData.get());
...
void KLDBootstrap::loadSecurityExtensions(void) {
 ...
 *********************************************************************/
OSReturn KLDBootstrap::loadKernelComponentKexts(void) {
  ...
  for (kextIDPtr = &sKernelComponentNames[0]; *kextIDPtr; kextIDPtr++) {
    ...
    if (theKext) {
      if (kOSReturnSuccess !=
          OSKext::loadKextWithIdentifier(*kextIDPtr, /* allowDefer */ false)) {
...
void KLDBootstrap::loadKernelExternalComponents(void) {
  OSSharedPtr<OSDictionary> extensionsDict;
  OSSharedPtr<OSCollectionIterator> keyIterator;
  OSString *bundleID = NULL;                   // don't release
  OSKext *theKext = NULL;                      // don't release
  ...
  while ((bundleID = OSDynamicCast(OSString, keyIterator->getNextObject()))) {
    ...
    isKernelExternalComponent = OSDynamicCast(
        OSBoolean,
    ...
    if (isKernelExternalComponent && isKernelExternalComponent->isTrue()) {
      OSKextLog(/* kext */ NULL, kOSKextLogStepLevel | kOSKextLogLoadFlag,
                "Loading kernel external component %s.",
...
/*********************************************************************
*********************************************************************/
void KLDBootstrap::readBuiltinPersonalities(void) {
  OSSharedPtr<OSObject> parsedXML;
  ...
  parsedXML =
      OSUnserializeXML((const char *)(uintptr_t)infosect->addr, errorString);
  if (parsedXML) {
  ...
  for (i = 0; i < count; i++) {
    ...
    infoDict = OSDynamicCast(OSDictionary, builtinExtensions->getObject(i));
    if (!infoDict) {
      continue;
    }

    moduleName =
        OSDynamicCast(OSString, infoDict->getObject(kCFBundleIdentifierKey));
    if (!moduleName) {
      continue;
    }
    ...
    personalities =
        OSDynamicCast(OSDictionary, infoDict->getObject("IOKitPersonalities"));
    if (!personalities) {
      continue;
    }
    ...
    while ((personalityName = OSDynamicCast(
      ...
      OSDictionary *personality = OSDynamicCast(
          OSDictionary, personalities->getObject(personalityName));

      OSKextLog(/* kext */ NULL, kOSKextLogDetailLevel | kOSKextLogLoadFlag,
