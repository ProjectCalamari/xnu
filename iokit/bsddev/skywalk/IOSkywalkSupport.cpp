...
static vm_tag_t getVMTagForMap(vm_map_t map) {
  return (map == kernel_map) ? VM_KERN_MEMORY_SKYWALK : VM_MEMORY_SKYWALK;
}
...
public:
  bool overwriteMappingInTask(task_t intoTask, mach_vm_address_t *startAddr,
...
public:
  bool initWithSpec(task_t inTask, mach_vm_size_t capacity,
                    mach_vm_address_t alignment,
                    const IOSKMemoryBufferSpec *spec);

  virtual void *getBytesNoCopy(void) APPLE_KEXT_OVERRIDE;

  virtual void *getBytesNoCopy(vm_size_t start,
                               vm_size_t withLength) APPLE_KEXT_OVERRIDE;

  bool isWired(void) const { return _wireCount != 0; }
...
public:
  ...
  kern_return_t mapOverwrite(vm_map_offset_t addr, vm_map_size_t size,
                             vm_prot_t prot);
...
public:
  ...
  mach_vm_address_t getMapAddress(mach_vm_size_t *size) const {
    if (size) {
      *size = fMapSize;
    }
    return fMapAddr;
  ...
  IOSKArena *getArena(void) const { return fArena; }
  bool isRedirected(void) const { return fRedirected; }
  void redirectMap(void) { fRedirected = true; }
...
public:
  bool initWithRegions(IOSKRegion **regions, IOSKCount regionCount);

  IOReturn createMapperForTask(task_t task,
                               LIBKERN_RETURNS_RETAINED IOSKMapper **mapper);
  void redirectMap(IOSKMapper *mapper);

  IOSKSize getArenaSize(void) const { return fArenaSize; }
  IOSKCount getRegionCount(void) const { return fRegions->getCount(); }
  IOSKRegion *getRegion(IOSKIndex regIndex) const;

  IOReturn map(const IOSKRegion *region, IOSKOffset regionOffset,
               IOSKIndex regionIndex, IOSKIndex segmentIndex,
               IOSKMemoryBuffer *buffer);

  void unmap(const IOSKRegion *region, IOSKOffset regionOffset,
             IOSKIndex regionIndex, IOSKIndex segmentIndex, vm_prot_t prot,
             bool isRedirected, const void *context);
...
public:
  bool initWithSpec(const IOSKRegionSpec *spec, IOSKSize segSize,
                    IOSKCount segCount);
  ...
  bool attachArena(IOSKArena *arena, IOSKOffset regionOffset,
  ...
  IOSKCount getSegmentCount(void) const { return fSegmentCount; }
  IOSKSize getSegmentSize(void) const { return fSegmentSize; }
  IOSKSize getRegionSize(void) const { return fSegmentCount * fSegmentSize; }
...
private:
  ...
  void _clearSegmentBuffer(const IOSKIndex index,
                           IOSKMemoryBufferRef *prevBuffer);
...
OSDefineMetaClassAndFinalStructors(IOSKRegionMapper, OSObject)
    ...
    bool IOSKRegionMapper::initWithMapper(IOSKMapper *mapper,
                                          IOSKRegion *region,
                                          IOSKOffset regionOffset) {
  ...
  DLOG("SKRegionMapper %p mapper %p region %p offset 0x%x", this, mapper,
       region, regionOffset);
  return true;
}

void IOSKRegionMapper::free(void) {
...
IOReturn IOSKRegionMapper::map(IOSKIndex segIndex, IOSKBuffer *buffer) {
  ...
  SK_DF(ret == kIOReturnSuccess ? SK_VERB_IOSK : SK_VERB_ERROR,
        "%p buffer %p index %u map %p offset 0x%x size 0x%x", this, buffer,
        segIndex, fMemoryMaps[segIndex], (uint32_t)offset,
        fRegion->getSegmentSize());

  return ret;
}

void IOSKRegionMapper::unmap(IOSKIndex segIndex, vm_prot_t prot) {
  mach_vm_address_t addr;
  mach_vm_offset_t offset;
  IOMemoryMap *map;
  kern_return_t kr;
  ...
  // by more than one mapper.
  if ((segIndex >= fMemoryMapCount) ||
      ((map = fMemoryMaps[segIndex]) == NULL)) {
    return;
  }
  ...
  DLOG("SKRegionMapper %p index %u offset 0x%x size 0x%x", this, segIndex,
       (uint32_t)offset, fRegion->getSegmentSize());
}

kern_return_t IOSKRegionMapper::mapOverwrite(vm_map_offset_t addr,
                                             vm_map_size_t size,
                                             vm_prot_t prot) {
  ...
  kr = mach_vm_map_kernel(fMapper->fTaskMap, &addr, size, (vm_map_offset_t)0,
                          vmk_flags, IPC_PORT_NULL, (vm_object_offset_t)0,
                          FALSE, prot, VM_PROT_DEFAULT, VM_INHERIT_NONE);

  SK_DF(kr == KERN_SUCCESS ? SK_VERB_IOSK : SK_VERB_ERROR,
        "SKRegionMapper %p addr 0x%llx size 0x%llx prot 0x%x "
        "kr 0x%x",
        this, (uint64_t)addr, (uint64_t)size, prot, kr);
  return kr;
}
...
OSDefineMetaClassAndFinalStructors(IOSKMapper, OSObject)
    ...
    bool IOSKMapper::initWithTask(task_t task, IOSKArena *arena) {
  ...
  kr = mach_vm_map_kernel(fTaskMap, &addr, size, (vm_map_offset_t)0, vmk_flags,
                          IPC_PORT_NULL, (vm_object_offset_t)0, FALSE,
                          VM_PROT_NONE, VM_PROT_DEFAULT, VM_INHERIT_NONE);
  ...
  SK_DF(kr == KERN_SUCCESS ? SK_VERB_IOSK : SK_VERB_ERROR,
        ...
        "kr 0x%x",
        this, (uint64_t)task, fTaskMap, (uint64_t)addr, (uint64_t)size,
        fSubMaps->getCount(), kr);

  return ok;
}

void IOSKMapper::free(void) {
...
IOReturn IOSKMapper::map(IOSKIndex regionIndex, IOSKIndex segmentIndex,
                         IOSKBuffer *buffer) {
  IOSKRegionMapper *subMap;
...
void IOSKMapper::unmap(IOSKIndex regionIndex, IOSKIndex segmentIndex,
                       vm_prot_t prot) {
  IOSKRegionMapper *subMap;
...
OSDefineMetaClassAndFinalStructors(IOSKArena, OSObject)
    ...
    bool IOSKArena::initWithRegions(IOSKRegion **regions,
                                    IOSKCount regionCount) {
  IOSKRegion *region;
  IOSKSize regionSize;
  IOSKOffset regionOffset = 0;
  ...
  DLOG("SKArena %p regions %u size 0x%x ok %d", this, regionCount, fArenaSize,
       ok);
  return ok;
}

void IOSKArena::free(void) {
...
IOReturn IOSKArena::createMapperForTask(task_t task, IOSKMapper **outMapper) {
  IOSKRegion *region;
  OSObject *object;
  IOSKMapper *mapper;
...
IOReturn IOSKArena::map(const IOSKRegion *region __unused,
                        IOSKOffset regionOffset __unused, IOSKIndex regionIndex,
                        IOSKIndex segmentIndex, IOSKBuffer *buffer) {
  IOSKMapper *mapper;
  OSIterator *iter;
  IOReturn result, ret = kIOReturnSuccess;
...
void IOSKArena::unmap(const IOSKRegion *region __unused,
                      IOSKOffset regionOffset __unused, IOSKIndex regionIndex,
                      IOSKIndex segmentIndex, vm_prot_t prot, bool redirecting,
                      const void *context) {
  IOSKMapper *mapper;
...
void IOSKArena::redirectMap(IOSKMapper *mapper) {
  OSObject *object;
  IOSKRegion *region;
  IOReturn ret;
...
IOSKRegion *IOSKArena::getRegion(IOSKIndex regionIndex) const {
  assert(regionIndex < getRegionCount());
  return OSDynamicCast(IOSKRegion, fRegions->getObject(regionIndex));
}

bool IOSKArena::addMapper(const IOSKMapper *mapper) {
  bool ok = false;
...
void IOSKArena::removeMapper(const IOSKMapper *mapper) {
  assert(mapper != NULL);
  if (!mapper) {
    return;
  }
...
OSDefineMetaClassAndFinalStructors(IOSKRegion, OSObject)
    ...
    bool IOSKRegion::initWithSpec(const IOSKRegionSpec *spec,
                                  IOSKSize segmentSize,
                                  IOSKCount segmentCount) {
  bool ok = false;
  ...
  SK_DF(ok ? SK_VERB_IOSK : SK_VERB_ERROR,
        "SKRegion %p segment size 0x%x count %u ok %d", this, segmentSize,
        segmentCount, ok);

  return ok;
}

void IOSKRegion::free(void) {
...
IOReturn IOSKRegion::_setSegmentBuffer(const IOSKIndex segmentIndex,
                                       IOSKBuffer *buffer) {
  Segment *seg;
  IOReturn ret = kIOReturnSuccess;
  ...
  if (seg->fBuffer == NULL) {
    ...
    SLIST_FOREACH(entry, &fArenaHead, link) {
      if (entry->fArena != NULL) {
        ret = entry->fArena->map(this, entry->fRegionOffset,
                                 entry->fRegionIndex, segmentIndex, buffer);
        assert(kIOReturnSuccess == ret);
  ...
  SK_DF(ret == kIOReturnSuccess ? SK_VERB_IOSK : SK_VERB_ERROR,
        "SKRegion %p set segment[%u] buffer %p ret 0x%x", this, segmentIndex,
        buffer, ret);

  return ret;
}

void IOSKRegion::_clearSegmentBuffer(const IOSKIndex segmentIndex,
                                     IOSKMemoryBufferRef *prevBuffer) {
  Segment *seg;
  bool cleared = false;
  ...
  if (seg->fBuffer != NULL) {
    ...
    SLIST_FOREACH(entry, &fArenaHead, link) {
      if (entry->fArena != NULL) {
        entry->fArena->unmap(this, entry->fRegionOffset, entry->fRegionIndex,
  ...
  DLOG("SKRegion %p clear segment[%u] ok %d", this, segmentIndex, cleared);
}

IOReturn IOSKRegion::setSegmentBuffer(IOSKIndex index,
                                      IOSKMemoryBuffer *buffer) {
  IOReturn ret;
...
void IOSKRegion::clearSegmentBuffer(IOSKIndex index,
                                    IOSKMemoryBufferRef *prevBuffer) {
  IOLockLock(fRegionLock);
...
IOSKRegion::ArenaEntry *IOSKRegion::findArenaEntry(const IOSKArena *arena) {
...
bool IOSKRegion::attachArena(IOSKArena *arena, IOSKOffset regionOffset,
                             IOSKIndex regionIndex) {
  bool ok = false;
  ...
  SK_DF(ok ? SK_VERB_IOSK : SK_VERB_ERROR,
        "SKRegion %p attach arena %p offset 0x%x index %u ok %d", this, arena,
        regionOffset, regionIndex, ok);
  return ok;
}

void IOSKRegion::detachArena(const IOSKArena *arena) {
  ArenaEntry *entry;
  bool detached = false;
...
IOReturn IOSKRegion::updateMappingsForArena(IOSKArena *arena, bool redirect,
                                            const void *context) {
  ArenaEntry *entry;
  Segment *seg;
  vm_prot_t prot;
  ...
  if (entry != NULL) {
    ...
    for (uint32_t index = 0; index < fSegmentCount; index++) {
      ...
      if ((seg->fBuffer == NULL) || redirect) {
        ...
        arena->unmap(this, entry->fRegionOffset, entry->fRegionIndex, index,
                     prot, redirect, context);
      } else {
        result = arena->map(this, entry->fRegionOffset, entry->fRegionIndex,
                            index, seg->fBuffer);
      }
    }
  }
  ...
  SK_DF(result == kIOReturnSuccess ? SK_VERB_IOSK : SK_VERB_ERROR,
        "%p update arena %p redirect %d ret 0x%x", this, arena, redirect,
        result);
  return result;
}
...
OSDefineMetaClassAndFinalStructors(IOSKMemoryArray, IOMultiMemoryDescriptor)
    ...
    bool IOSKMemoryArray::overwriteMappingInTask(task_t intoTask,
  ...
  for (uint32_t i = 0; i < _descriptorsCount; i++) {
    ...
    if (mb) {
      ...
      if (mb->fSpec.user_writable) {
        ...
        rwMap = mb->createMappingInTask(intoTask, *startAddr,
                                        options | kIOMapOverwrite);
        if (rwMap) {
          DLOG("map_rw %d: addr 0x%llx, size 0x%x", i, *startAddr,
               (uint32_t)iomd->getLength());
          rwMap->release();
        } else {
        ...
        DLOG("map_ro %d: addr 0x%llx, size 0x%x", i, *startAddr,
             (uint32_t)iomd->getLength());
      }
...
OSDefineMetaClassAndFinalStructorsWithZone(IOSKMemoryBuffer,
    ...
    bool IOSKMemoryBuffer::initWithSpec(task_t inTask, mach_vm_size_t capacity,
                                        mach_vm_address_t alignment,
                                        const IOSKMemoryBufferSpec *spec) {
  bool ok = true;
  ...
  if (fSpec.kernel_writable) {
    if (fSpec.puredata) {
      ...
      ok = initControlWithPhysicalMask(inTask, options, capacity, alignment, 0);
    }
    if (!ok) {
      return false;
    }
    ...
    if (fSpec.puredata) {
      /* purely data; use data buffers heap */
      ok = initWithPhysicalMask(TASK_NULL, options, capacity, alignment, 0);
    } else {
      /* may have pointers; use default heap */
      ok = initControlWithPhysicalMask(TASK_NULL, options, capacity, alignment,
                                       0);
    }
    if (!ok) {
      return false;
    ...
    fKernelReadOnlyMapping =
        super::createMappingInTask(kernel_task, 0, options);
...
void IOSKMemoryBuffer::taggedRelease(const void *tag) const {
  /*
...
void IOSKMemoryBuffer::free(void) {
  if (!fSpec.kernel_writable && fKernelReadOnlyMapping != NULL) {
...
void *IOSKMemoryBuffer::getBytesNoCopy(void) { return fKernelAddr; }

void *IOSKMemoryBuffer::getBytesNoCopy(vm_size_t start, vm_size_t withLength) {
...
static IOSKMemoryBuffer *RefToMemoryBuffer(IOSKMemoryRef inRef) {
  IOSKMemoryBuffer *mb = OSDynamicCast(IOSKMemoryBuffer, inRef);
  return mb;
}

static IOSKMemoryArray *RefToMemoryArray(IOSKMemoryRef inRef) {
...
IOSKMemoryArrayRef IOSKMemoryArrayCreate(const IOSKMemoryRef refs[],
  ...
  if (ma && !ma->initWithDescriptors((IOMemoryDescriptor **)refs, count,
...
IOSKMemoryMapRef IOSKMemoryMapToTask(IOSKMemoryRef reference, task_t intoTask,
...
IOReturn IOSKMemoryDiscard(IOSKMemoryRef reference) {
  ...
  return mb->setPurgeable(
      kIOMemoryPurgeableEmpty | kIOMemoryPurgeableFaultOnAccess, NULL);
}
...
static void IOSKObjectDestroy(const OSObject *object) {
  assert(object != NULL);
  if (object) {
    object->release();
  }
...
IOSKArenaRef IOSKArenaCreate(IOSKRegionRef *regionList, IOSKCount regionCount) {
  IOSKArenaRef arena;
...
void IOSKArenaDestroy(IOSKArenaRef arena) { IOSKObjectDestroy(arena); }

void IOSKArenaRedirect(IOSKArenaRef arena) {
  assert(arena != NULL);
  if (arena != NULL) {
...
IOSKRegionRef IOSKRegionCreate(const IOSKRegionSpec *regionSpec,
                               IOSKSize segSize, IOSKCount segCount) {
  IOSKRegionRef region;

  region = new IOSKRegion;
  if ((region != NULL) &&
      !region->initWithSpec(regionSpec, segSize, segCount)) {
    region->release();
    region = NULL;
  }
  return region;
...
void IOSKRegionDestroy(IOSKRegionRef region) { IOSKObjectDestroy(region); }

IOReturn IOSKRegionSetBuffer(IOSKRegionRef region, IOSKIndex segmentIndex,
...
void IOSKRegionClearBuffer(IOSKRegionRef region, IOSKIndex segmentIndex) {
  assert(region != NULL);
  if (region != NULL) {
...
void IOSKRegionClearBufferDebug(IOSKRegionRef region, IOSKIndex segmentIndex,
...
IOSKMapperRef IOSKMapperCreate(IOSKArenaRef arena, task_t task) {
...
void IOSKMapperDestroy(IOSKMapperRef mapper) {
  assert(mapper != NULL);
  if (mapper != NULL) {
...
void IOSKMapperRedirect(IOSKMapperRef mapper) {
  assert(mapper != NULL);
  if (mapper != NULL) {
    IOSKArena *arena = mapper->getArena();
    assert(arena != NULL);
...
IOReturn IOSKMapperGetAddress(IOSKMapperRef mapper, mach_vm_address_t *address,
                              mach_vm_size_t *size) {
  assert(mapper != NULL);
...
boolean_t IOSKBufferIsWired(IOSKMemoryBufferRef buffer) {
  assert(buffer != NULL);
...
int IOSkywalkSupportTest(int newValue) {
  ...
  for (int i = 0; i < kNumMappers; i++) {
    kprintf("mapper[%d] %p map address 0x%llx size 0x%x\n", i, mappers[i],
            (uint64_t)addrs[i], (uint32_t)size);
  }
