 ...
 *	Merge of branch "chardonnay" to pick up all chardonnay changes in
 * Leopard as of xnu-792.7.4
 *
 ...
 *	Merged PR-2957314 into Jaguar (siegmund: netboot kernel code needs to
 * set com.apple.AppleDiskImageController.load to boolean Yes)
 *
 ...
 *	Merged PR-2909558 into Jaguar (siegmund POST WWDC: add support for
 * NetBoot over IOHDIXController)
 *
...
static IOService *di_load_controller(void) {
  OSIterator *controllerIterator = NULL;
  OSDictionary *matchDictionary = NULL;
  IOService *controller = NULL;

  do {
    IOService::getResourceService()->publishResource(
        "com.apple.AppleDiskImageController.load", kOSBooleanTrue);
 ...
 */
static int di_add_properties(IOService *controller, bool removable) {
  if (!controller->setProperty(kDIRootImageRemovableKey,
                               removable ? kOSBooleanTrue : kOSBooleanFalse)) {
...
int di_root_image_ext(const char *path, char *devname, size_t devsz,
  ...
  if (!controller->setProperty(kDIRootImageKey, pathString)) {
    IOLog("IOHDIXController::setProperty(%s, %s) failed.\n", kDIRootImageKey,
          pathString->getCStringNoCopy());
  }

  myResult =
      OSDynamicCast(OSNumber, controller->getProperty(kDIRootImageResultKey));
  res = kIOReturnError;
  if (myResult) {
  ...
  // success - grab
  myDevT =
      OSDynamicCast(OSNumber, controller->getProperty(kDIRootImageDevTKey));
  if (myDevT) {
  ...
  myDevName =
      OSDynamicCast(OSString, controller->getProperty(kDIRootImageDevNameKey));
  if (myDevName) {
  ...
  /*
   * NOTE: The attached disk image may trigger IOKit matching. At the very
   * least, an IOMedia must claim it.  More complex scenarios might include a
   * GPT containing a partition mapping to an APFS container, both of which need
   * to probe and claim their respective media devices.
   *
   * After the attach is complete, we should quiesce the disk image controller
   * before returning from this function successfully.  If we failed to quiesce,
   * then we should treat it as a hard failure, to make it more obvious to
   * triage.
   */
 ...
 *                               devname	<-		dev node used to
 * set the rootdevice global variable dev_p	<-		device number
 * generated from major/minor numbers Comments: This is an exported function.
 * Changing this will break API.
 */
...
int di_root_ramfile_buf(void *buf, size_t bufsz, char *devname, size_t devsz,
                        dev_t *dev_p) {
  IOReturn res = 0;
  IOService *controller = NULL;
  OSNumber *myResult = NULL;
...
void di_root_ramfile(IORegistryEntry *entry) {
  ...
  do {
    ...
    __typed_allocators_ignore_push extentList =
        (IOAddressRange *)extentData->getBytesNoCopy();
    ...
    mem = IOMemoryDescriptor::withAddressRanges(
        extentList, extentCount, kIODirectionOut | kIOMemoryMapperNone, NULL);
