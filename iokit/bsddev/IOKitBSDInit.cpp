...
extern int IODTGetDefault(const char *key, void *infoAddr,
...
static void IOOpenPolledCoreFile(thread_call_param_t __unused,
...
#if IOPOLLED_COREFILE
  corefile_open_call = thread_call_allocate_with_options(
      IOOpenPolledCoreFile, NULL, THREAD_CALL_PRIORITY_KERNEL,
...
OSDictionary *IONetworkNamePrefixMatching(const char *prefix) {
  ...
  do {
    ...
    if (matching->setObject(gIOPropertyMatchKey, (OSObject *)propDict) !=
        true) {
      continue;
    }
...
static bool IORegisterNetworkInterface(IOService *netif) {
  ...
  do {
    stack =
        IOService::waitForService(IOService::serviceMatching("IONetworkStack"));
    if (stack == NULL) {
      break;
    }
    ...
    len = kMaxPathLen;
    if (netif->getPath(pathBuf.data(), &len, gIOServicePlane) == false) {
      break;
    }
...
bool IOGetApfsPrebootUUID(char *uuid) {
  IORegistryEntry *entry;
  OSData *uuid_data = NULL;
  bool result = false;
...
bool IOGetBootObjectsPath(char *path_prefix) {
  IORegistryEntry *entry;
  ...
  if ((entry = IORegistryEntry::fromPath("/chosen", gIODTPlane))) {
    ...
    if (path_prefix_data) {
      ...
      if (length <= MAXPATHLEN) {
        ...
        /* copy the content of path_prefix_data->getBytesNoCopy() into
         * path_prefix */
...
bool IOGetBootManifestHash(char *hash_data, size_t *hash_data_size) {
  ...
  if ((entry = IORegistryEntry::fromPath("/chosen", gIODTPlane))) {
    ...
    if (manifest_hash_data) {
      ...
      /* hashed with SHA2-384 or SHA1, the boot manifest hash should be 48 Bytes
       * or less */
      if ((length <= CCSHA384_OUTPUT_SIZE) &&
          (*hash_data_size >= CCSHA384_OUTPUT_SIZE)) {
        /* ensure caller's buffer is fully initialized: */
        bzero(hash_data, CCSHA384_OUTPUT_SIZE);
        /* copy the content of manifest_hash_data->getBytesNoCopy() into
         * hash_data */
 ...
 */
boolean_t IOSetRecoveryBoot(bsd_bootfail_mode_t mode, uuid_t volume_uuid,
                            boolean_t reboot) {
  ...
  if (mode == BSD_BOOTFAIL_SEAL_BROKEN) {
    ...
    if (!PEWriteNVRAMProperty(SYSTEM_NVRAM_PREFIX "recovery-boot-mode",
                              boot_mode,
    ...
    boot_command_sym =
        OSSymbol::withCStringNoCopy(SYSTEM_NVRAM_PREFIX "boot-command");
    ...
    nvram = OSDynamicCast(
        IODTNVRAM,
        IOService::waitForService(IOService::serviceMatching("IODTNVRAM")));
    if (nvram == NULL) {
...
kern_return_t IOFindBSDRoot(char *rootName, unsigned int rootNameSize,
  ...
  // stall here for anyone matching on the IOBSD resource to finish
  // (filesystems)
  ...
  if ((service =
           IOService::waitForMatchingService(matching, 30ULL * kSecondScale))) {
  ...
  if ((regEntry = IORegistryEntry::fromPath("/chosen", gIODTPlane))) {
    do {
      ...
      if (data) {
        unserializedContainer =
            OSUnserializeXML((char *)data->getBytesNoCopy());
  ...
  //
  //	See if we have a RAMDisk property in /chosen/memory-map.  If so, make it
  //into a device. 	It will become /dev/mdx, where x is 0-f.
  //

  if (!didRam) { /* Have we already build this ram disk? */
    didRam = 1;  /* Remember we did this */
    if ((regEntry = IORegistryEntry::fromPath(
             "/chosen/memory-map", gIODTPlane))) { /* Find the map node */
      data = (OSData *)regEntry->getProperty(
          "RAMDisk"); /* Find the ram disk, if there */
      if (data) {     /* We found one */
        uintptr_t *ramdParms;
        /* BEGIN IGNORE CODESTYLE */
        __typed_allocators_ignore_push ramdParms =
            (uintptr_t *)data
...
#define MAX_PHYS_RAM (((uint64_t)UINT_MAX) << 12)
...
#endif /* __LP64__ */
        (void)mdevadd(-1, ml_static_ptovirt(ramdParms[0]) >> 12,
                      (unsigned int)(ramdParms[1] >> 12),
                      0); /* Initialize it and pass back the device number */
      }
      ...
      *root = mdevlookup(xchar);   /* Find the device number */
      if (*root >= 0) {            /* Did we find one? */
        ...
        *oflags = 0; /* Show that this is not network */
        ...
        if (mdevgetrange(xchar, &kdp_core_ramdisk_addr,
      ...
      panic("IOFindBSDRoot: specified root memory device, %s, has not been "
            "configured",
            rdBootVar); /* Not there */
    }
  }
  ...
  char namep[8];
  if (needNetworkKexts || PE_parse_boot_argn("-s", namep, sizeof(namep))) {
  ...
  if (service && service->metaCast("IONetworkInterface") &&
...
void IOSecureBSDRoot(const char *rootName) {
  ...
  assert(matching);
  pe = (IOPlatformExpert *)IOService::waitForMatchingService(
      matching, 30ULL * kSecondScale);
  matching->release();
  assert(pe);
  ...
  result = pe->callPlatformFunction(functionName, false, (void *)rootName,
...
kern_return_t IOBSDGetPlatformUUID(uuid_t uuid, mach_timespec_t timeout) {
  ...
  resources = IOService::waitForService(
      IOService::resourceMatching(kIOPlatformUUIDKey),
      (timeout.tv_sec || timeout.tv_nsec) ? &timeout : NULL);
  if (resources == NULL) {
...
IOPolledCoreFileMode_t gIOPolledCoreFileMode =
...
static IOPolledCoreFileMode_t GetCoreFileMode() {
...
static void IOResolveCoreFilePath() {
  ...
  if (kSuccess != SecureDTGetProperty(node, "kernel-core-dump-location",
  ...
  // The kdp_corefile_path is allowed to be one of 2 options to working
  // locations. This value is set on EARLY_BOOT since we need to know it before
  // any volumes are mounted. The mount event triggers IOOpenPolledCoreFile()
  // which opens the file. Once we commit to using the path from EDT we can't
  // back out since a different path may reside in a different volume. In case
  // the path from EDT can't be opened, there will not be a kernel core-dump
  if (strlcmp(value, "preboot", size) == 0) {
    kdp_corefile_path = kIOCoreDumpPrebootPath;
  } else if (strlcmp(value, "default", size) != 0) {
    IOLog("corefile path selection in device-tree is not one of the allowed "
          "values: %s, Using default %s\n",
...
static void IOCoreFileGetSize(uint64_t *ideal_size, uint64_t *fallback_size,
  ...
  *ideal_size = *fallback_size = *free_space_to_leave = 0;
  ...
  int error = VNOP_IOCTL(rootvnode, HFS_IOCTL_VOLUME_STATUS,
                         (caddr_t)&status_flags, 0, vfs_context_kernel());
  if (!error) {
  ...
  if (mode == kIOPolledCoreFileModeCoredump) {
    *ideal_size = *fallback_size = kIOCoreDumpMinSize;
    if (kIOCoreDumpIncrementalSize != 0 && max_mem > (32 * 1024ULL * ONE_MB)) {
      *ideal_size = ((ROUNDUP(max_mem, (32 * 1024ULL * ONE_MB)) /
                      (32 * 1024ULL * ONE_MB)) *
                     kIOCoreDumpIncrementalSize);
    }
    *free_space_to_leave = kIOCoreDumpFreeSize;
...
static IOReturn IOAccessCoreFileData(void *context, boolean_t write,
  ...
  vnode_error = vn_rdwr(
      write ? UIO_WRITE : UIO_READ, vnode_ptr, (caddr_t)buffer, length, offset,
      UIO_SYSSPACE, IO_SWAP_DISPATCH | IO_SYNC | IO_NOCACHE | IO_UNIT,
...
static void IOOpenPolledCoreFile(thread_call_param_t __unused,
  ...
  uint64_t corefile_size_bytes = 0, corefile_fallback_size_bytes = 0,
           free_space_to_leave_bytes = 0;
  ...
  // We'll overwrite this once we open the file, we update this to mark that we
  // have made it past initialization
  gIOPolledCoreFileMode = kIOPolledCoreFileModeClosed;

  IOCoreFileGetSize(&corefile_size_bytes, &corefile_fallback_size_bytes,
                    &free_space_to_leave_bytes, mode_to_init);
  ...
  do {
    // This file reference remains open long-term in case we need to write a
    // core-dump
    err = IOPolledFileOpen(filename, kIOPolledFileCreate, 0 /*setFileSizeMin*/,
                           corefile_size_bytes, free_space_to_leave_bytes, NULL,
                           0, &gIOPolledCoreFileVars, NULL, NULL, NULL);
    ...
    err = IOPolledFileOpen(filename, kIOPolledFileCreate, 0 /*setFileSizeMin*/,
                           ...
                           free_space_to_leave_bytes, NULL, 0,
                           &gIOPolledCoreFileVars, NULL, NULL, NULL);
    if (kIOReturnSuccess != err) {
      IOLog("Failed to open corefile of size %llu MB (returned error 0x%x)\n",
  ...
  gIOPolledCoreFileOpenRet = IOPolledFilePollersSetup(
      gIOPolledCoreFileVars, kIOPolledPreflightCoreDumpState);
  ...
  // Provide the "polled file available" callback with a temporary way to read
  // from the file
...
kern_return_t IOProvideCoreFileAccess(IOCoreFileAccessRecipient recipient,
  ...
  vfs_context = vfs_context_kernel();
  vnode_error = vnode_open(kdp_corefile_path, (FREAD | FWRITE | O_NOFOLLOW),
                           0600, 0, &vnode_ptr, vfs_context);
  if (vnode_error) {
...
static void IOClosePolledCoreFile(void) {
...
static void IOUnlinkPolledCoreFile(void) {
  // Notify kdp core that the corefile is no longer available
  (void)kdp_core_polled_io_polled_file_unavailable();
...
extern "C" void IOBSDMountChange(struct mount *mp, uint32_t op) {
#if IOPOLLED_COREFILE
  uint64_t flags;
  char path[128];
  int pathLen;
  ...
  case kIOMountChangeDidResize:
    ...
    // want to match with /private/var because /private/var/internal is often
    // mounted over /private/var
  ...
  case kIOMountChangeWillResize:
    if (gIOPolledCoreFileVars &&
        (mp == kern_file_mount(gIOPolledCoreFileVars->fileRef))) {
...
extern "C" void IOBSDLowSpaceUnlinkKernelCore(void) {
#if IOPOLLED_COREFILE
  lck_mtx_lock(&gIOPolledCoreFileMtx);
  if (gIOPolledCoreFileVars) {
    thread_call_cancel_wait(corefile_open_call);
...
static char *copyOSStringAsCString(OSString *string) {
  ...
  c_string = (char *)kalloc_data(
      string_length, (zalloc_flags_t)(Z_ZERO | Z_WAITOK | Z_NOFAIL));
...
extern "C" boolean_t IOTaskHasStringEntitlement(task_t task,
...
extern "C" boolean_t IOTaskHasEntitlement(task_t task,
                                          const char *entitlement) {
  if (task == NULL) {
    task = current_task();
  }
  ...
  kern_return_t ret =
      amfi->OSEntitlements.queryEntitlementBooleanWithProc(proc, entitlement);
...
extern "C" boolean_t IOTaskGetIntegerEntitlement(task_t task,
                                                 const char *entitlement,
                                                 uint64_t *value) {
...
extern "C" char *IOTaskGetEntitlement(task_t task, const char *entitlement) {
  void *entitlement_object = NULL;
...
extern "C" boolean_t IOVnodeHasEntitlement(vnode_t vnode, int64_t off,
                                           const char *entitlement) {
  OSObject *obj;
 ...
 */
extern "C" boolean_t IOVnodeGetBooleanEntitlement(vnode_t vnode, int64_t off,
                                                  const char *entitlement,
                                                  bool *value) {
  OSObject *obj;
  off_t offset = (off_t)off;
...
extern "C" char *IOVnodeGetEntitlement(vnode_t vnode, int64_t off,
                                       const char *entitlement) {
  OSObject *obj = NULL;
  OSString *str = NULL;
  size_t len;
