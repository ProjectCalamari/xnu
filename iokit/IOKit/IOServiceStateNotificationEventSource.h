 ...
 *   @abstract Event source for IOService state notification delivery to
 * work-loop based drivers.
 *   @discussion For use with the IOService.iig IOService::StateNotification*
 * APIs.
 */
...
public:
  /*!
   * @brief       Create an IOServiceStateNotificationEventSource for
   * notification of IOService state events sent by the StateNotificationSet()
   * api.
   * @param       service The object hosting state, typically returned by
   * IOService::CopySystemStateNotificationService().
   * @param       items Array of state item names to be notified about.
   * @param       action Handler block to be invoked on the event source
   * workloop when the notification fires.
   * @return      IOServiceStateNotificationEventSource with +1 retain, or NULL
   * on failure.
   */
  static OSPtr<IOServiceStateNotificationEventSource>
  serviceStateNotificationEventSource(IOService *service, OSArray *items,
                                      ActionBlock action);
