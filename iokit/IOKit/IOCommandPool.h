...
protected:
  ...
  OSPtr<IOCommandGate>
      fSerializer; /* command gate used for serializing pool access */
...
public:
   ...
   */
  virtual bool init(IOService *inOwner, IOWorkLoop *inWorkLoop,
   ...
   */
  static OSPtr<IOCommandPool>
  commandPool(IOService *inOwner, IOWorkLoop *inWorkLoop,
              UInt32 inSize = kIOCommandPoolDefaultSize);

  /*!
   * @function getCommand
   * @discussion The getCommand method is used to get a pointer to an object of
   * type IOCommand from the pool.
   ...
   * the caller passes false, s/he is responsible for checking whether a
   * non-NULL pointer was returned.
   */

  virtual OSPtr<IOCommand> getCommand(bool blockForCommand = true);
...
protected:
   ...
   */
  virtual IOReturn
  gatedGetCommand(LIBKERN_RETURNS_NOT_RETAINED IOCommand **command,
