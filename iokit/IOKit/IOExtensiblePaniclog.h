...
public:
  /*!
   * @brief       This function is to be called to create IOExtensiblePaniclog
   * object.
   * @discussion  First function to be called.
   *
   * @param       uuid  The UUID of the handle.
   * @param       data_id The pointer to a string describing the handle. MAX
   * length of 32.
   * @param       max_len The maximum length of the buffer.
   * @param       options Options to be passed while creating the handle
   * @param       out The pointer to the created IOExtensiblePaniclog object.
   * NULL in case of an error.
   * @return      True in case of success. False in case of an error.
   */
  static bool createWithUUID(uuid_t uuid, const char *data_id, uint32_t max_len,
                             ext_paniclog_create_options_t options,
                             IOExtensiblePaniclog **out);

  /*!
   * @brief       This function is called to set the IOExtensiblePaniclog object
   * active.
   * @discussion  When it is set active, it is picked up and added to the
   * extensible paniclog in case of a panic.
   *
   ...
   * @brief       This function is called to set the IOExtensiblePaniclog object
   * inactive.
   * @discussion  When it is set inactive, this buffer is not picked up in case
   * of a panic
   *
   * @return      True in case of success. False in case of an error.
   */
   ...
   * @discussion  This function overwrites the data in the buffer. The write
   * starts from offset 0 and continues until 'len'
   *
   ...
   * @brief       This function is called to insert data into the buffer.
   * @discussion  This function overwrites the data in the buffer. The write
   * starts from last written byte and continues until 'len'
   *
   * @param       addr The address of the source buffer
   * @param       len The length to be copied.
   *
  ...
  /*!
   * @brief       This function is called to get a pointer to the ext paniclog
   * buffer
   * @discussion  After this function is called, the user is responsible for
   * copying data into the buffer. The entire buffer is copied when a system
   * panics. After claiming the buffer, yieldBuffer() has to be called to set
   * the used_len of the buffer before calling insertData() or appendData()
   *
  ...
  /*!
   * @brief       This function is called to yield the buffer and set the
   * used_len for the buffer
   * @discussion  After this function call, insertData() and appendData() can be
   * called.
   *
