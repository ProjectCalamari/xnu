...
enum {
   ...
   * of a major system event (between the event's initiation and its "done
   * event".) e.g. chronologically kIOPMEventTypeSleep may be followed by one or
   * more intermediate events, which then must be followed by
   * kIOPMEventTypeSleepDone.
   *
   * The intermediate events below will always occur in a Sleep or Wake event,
   * and may or may not occur for any of the other events.
   */
  kIOPMEventTypeAppNotificationsFinished = 501 | kIOPMEventTypeIntermediateFlag,
  kIOPMEventTypeDriverNotificationsFinished =
 ...
 * @abstract The potential causes for system sleep as logged in the system event
 * record.
 */
enum {
 ...
 * @discussion This type can be passed as arguments to
 * registerPMSettingController() to receive callbacks.
 */
...
enum {
   ...
   * @abstract User has been presented with the option to shutdown or restart.
   * Shutdown may be cancelled.
 ...
 * @discussion This key can be passed as an argument to
 * registerPMSettingController() and also identifies the type of PMSetting
 ...
 */
enum { kIOPMSilentRunningModeOn = 0x00000001 };

/* @constant kIOPMSettingLowLatencyAudioModeKey
 * @abstract Notification about low latency activity in the system available to
 * kexts.
 * @discussion This type can be passed as arguments to
 * registerPMSettingController() to receive callbacks.
 */
...
// PM Statistics: potential values for the key
// kIOPMStatsApplicationResponseTypeKey entry in the application results array.
 ...
 * we expect sleep to continue in a monotonically increasing order of
 * tracepoints to kIOPMTracePointSystemLoginwindowPhase. After trace point
 * SystemLoginWindowPhase, the system will return to kIOPMTracePointSystemUp.
 *
 * If the trace point decreases (instead of increasing) before reaching
 * kIOPMTracePointSystemUp, that indicates that the sleep process was cancelled.
 * The cancel reason shall be indicated in the cancel tracepoint. (TBD)
 */
...
enum {
   ...
   *      notifications, and also kIOMessageCanSystemSleep for the idle sleep
   * case. Phase ends: When PM has received all application responses.
   */
   ...
   *
   * Phase begins: Synchronous delivery of kIOMessageSystemWillSleep
   * notifications. Phase ends: All notified clients have acknowledged.
   */
  kIOPMTracePointSleepPriorityClients = 0x12,

  /* When kIOPMTracePointSleepWillChangeInterests is the latest tracePoint,
   * PM is calling powerStateWillChangeTo() on interested drivers of root
   * domain.
   *
   ...
   * Phase ends: Control returns to the OS on wake, but before recording the
   * first wake trace point.
   */
   ...
   *
   * Phase begins: Inform capability clients that system has gained
   * capabilities. Phase ends: All notified clients have acknowledged.
   */
   ...
   * This phase represents a several minute window after the system has powered
   * on. Higher levels of system diagnostics are in a heightened state of alert
   * in this phase, in case any user errors occurred that we could not detect in
   * software.
   *
   * Phase begins: After IOPMrootDomain sends kIOMessageSystemHasPoweredOn
   * message. Phase ends: When loginwindow calls IOPMSleepWakeSetUUID(NULL) the
   * system shall be considered awake and usable. The next phase shall be
...
/*
 * kIOPMLoginWindowProgressKey - identifies PM debug data specific to
 * LoginWindow for use with IOPMrootDomain. Only 4 bits of data are allotted.
 */
#define kIOPMLoginWindowProgressKey "LoginWindowProgress"

/*
 * kIOPMCoreDisplayProgressKey - identifies PM debug data specific to
 * CoreDisplay for use with IOPMrootDomain. Only 8 bits of data are allotted.
 */
#define kIOPMCoreDisplayProgressKey "CoreDisplayProgress"

/*
 * kIOPMCoreGraphicsProgressKey - identifies PM debug data specific to
 * CoreGraphics for use with IOPMrootDomain. Only 8 bits of data are allotted.
 */
#define kIOPMCoreGraphicsProgressKey "CoreGraphicsProgress"

// For PM internal use only - key to locate sleep failure results within
// SCDynamicStore.
...
#define kIOPMWakeEventAOTFlags                                                 \
  (kIOPMWakeEventAOTExit | kIOPMWakeEventAOTPossibleExit |                     \
...
#define kIOPMWakeEventAOTPossibleFlags                                         \
  (kIOPMWakeEventAOTPossibleExit | kIOPMWakeEventAOTConfirmedPossibleExit |    \
   kIOPMWakeEventAOTRejectedPossibleExit |                                     \
   kIOPMWakeEventAOTExpiredPossibleExit)

#define kIOPMWakeEventAOTPerCycleFlags                                         \
  (kIOPMWakeEventAOTPossibleExit | kIOPMWakeEventAOTRejectedPossibleExit |     \
   kIOPMWakeEventAOTExpiredPossibleExit)

#define kIOPMWakeEventAOTExitFlags                                             \
  (kIOPMWakeEventAOTExit | kIOPMWakeEventAOTConfirmedPossibleExit)
...
enum {
  ...
  kIOPMAOTModeDefault = (kIOPMAOTModeEnable | kIOPMAOTModeAddEventFlags |
                         kIOPMAOTModeRespectTimers)
};

enum { kIOPMAOTMetricsKernelWakeCountMax = 24 };
...
enum { kIOPMSleepPhase0 = 0, kIOPMSleepPhase1, kIOPMSleepPhase2 };
...
enum { kIOPMPerformanceNormal = 0, kIOPMPerformanceWarning = 100 };
