 ...
 *
 *  Only Kernel IOKit power management should reference the IOPowerConnection
 * class.
 */
...
protected:
  ...
  /*! @field currentPowerFlags	power flags which describe  the current state of
   * the power domain used by child */
  IOPMPowerFlags currentPowerFlags;

  /*! @field desiredDomainState	state number which corresponds to the child's
   * desire used by parent */
  unsigned long desiredDomainState;

  /*! @field requestFlag		set to true when desiredDomainState is
   * set */
  bool requestFlag;

  /*! @field preventIdleSleepFlag	true if child has this bit set in its
   * desired state used by parent */
  unsigned long preventIdleSleepFlag;

  /*! @field preventSystemSleepFlag	true if child has this bit set in its
   * desired state used by parent */
  unsigned long preventSystemSleepFlag;

  /*! @field awaitingAck		true if child has not yet acked our
   * notification used by parent */
  bool awaitingAck;

  /*! @field readyFlag		true if the child has been added as a power
   * child used by parent */
  bool readyFlag;
...
public:
   ...
   *   @discussion Called by the parent when the object is created and called by
   * the child when it discovers that the parent now knows its state. */
  void setParentKnowsState(bool);

  /*! @function setParentCurrentPowerFlags
   *   @abstract Sets the currentPowerFlags variable.
   *   @discussion Called by the parent when the object is created and called by
   * the child when it discovers that the parent state is changing. */
   ...
   *   @abstract Set the flag that says that the child has called
   * requestPowerDomainState.
   *  @discussion Called by the parent. */
  void setChildHasRequestedPower(void);

  /*! @function childHasRequestedPower
   *   @abstract Return the flag that says whether the child has called
   * requestPowerDomainState.
   *  @discussion Called by the PCI Aux Power Supply Driver to see if a device
   * driver is power managed. */
