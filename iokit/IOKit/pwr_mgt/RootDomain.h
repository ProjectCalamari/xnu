 ...
 * A bitfield describing a set of assertions. May be used to specify which
 * assertions to set with <link>IOPMrootDomain::createPMAssertion</link>; or to
 * query which assertions are set with
 * <link>IOPMrootDomain::releasePMAssertion</link>.
 */
typedef uint64_t IOPMDriverAssertionType;

/* IOPMDriverAssertionID
 * Drivers may create PM assertions to request system behavior (keep the system
 * awake, or keep the display awake). When a driver creates an assertion via
 ...
 * Possible values for IOPMDriverAssertionLevel are
 * <link>kIOPMDriverAssertionLevelOff</link> and
 * <link>kIOPMDriverAssertionLevelOn</link>
 */
...
/*
 * String constants to use as keys for a dictionary passed to
...
typedef IOReturn (*IOPMSettingControllerCallback)(OSObject *target,
                                                  const OSSymbol *type,
                                                  OSObject *val,
                                                  uintptr_t refcon);

__BEGIN_DECLS
IONotifier *registerSleepWakeInterest(IOServiceInterestHandler, void *,
                                      void * = NULL);

IONotifier *registerPrioritySleepWakeInterest(IOServiceInterestHandler handler,
...
public:
  ...
  copyProperty(const char *aKey) const APPLE_KEXT_OVERRIDE;

  /*! @function systemPowerEventOccurred
   *    @abstract Other drivers may inform IOPMrootDomain of system PM events
   *    @discussion systemPowerEventOccurred is a richer alternative to
   * receivePowerNotification() Only Apple-owned kexts should have reason to
   * call systemPowerEventOccurred.
  ...
  IOReturn systemPowerEventOccurred(const OSSymbol *event, OSObject *value);
 ...
 *   @discussion IOKit drivers may call claimSystemWakeEvent() during system
 * wakeup to provide human readable debug information describing the event(s)
 * that caused the system to wake.
 *
 ...
 *
 *               - This call tracks wake events from a non-S0 state (S0i, S3,
 * S4) into S0.
 *               - This call does not track wake events from DarkWake(S0) to
 * FullWake(S0).
 *
 *       Examples:
 *               (reason  = "WiFi.TCPData",
 ...
 *                   aggregating wake events from multiple sources, e.g. an USB
 * or Thunderbolt host controller.
 *
 *   @param reason   Caller should pass a human readable C string describing the
 *                   wake reason. Please use a string from the list below, or
 * create your own string matching this format: [Hardware].[Event]
 *                             WiFi.MagicPacket
 *                             WiFi.ScanOffload
 *                             WiFi.mDNSConflict
 *                             WiFi.mDNSService
 ...
 */
#endif
  void claimSystemWakeEvent(IOService *device, IOOptionBits flags,
                            const char *reason, OSObject *details = NULL);

  void claimSystemBootEvent(IOService *device, IOOptionBits flags,
                            const char *reason, OSObject *details = NULL);

  void claimSystemShutdownEvent(IOService *device, IOOptionBits flags,
                                const char *reason, OSObject *details = NULL);
  ...
  void publishFeature(const char *feature, uint32_t supportedWhere,
   ...
   *       setting that the caller is interested in and wants to get callbacks
   * for.
   ...
   *   @param handle Caller should keep the OSObject * returned here. If
   * non-NULL, handle will have a retain count of 1 on return. To deregister,
   * pass to unregisterPMSettingController()
   *   @result kIOReturnSuccess on success. */

  IOReturn registerPMSettingController(const OSSymbol *settings[],
                                       IOPMSettingControllerCallback callout,
                                       OSObject *target, uintptr_t refcon,
   ...
   *       setting that the caller is interested in and wants to get callbacks
   * for.
   *   @param supportedPowerSources bitfield indicating which power sources
   * these settings are supported for (kIOPMSupportedOnAC, etc.)
   *   @param callout C function ptr or member function cast as such.
   *   @param target The target of the callback, usually 'this'
   *   @param refcon Will be passed to caller in callback; for caller's use.
   *   @param handle Caller should keep the OSObject * returned here. If
  ...
  virtual OSPtr<IONotifier>
  registerInterest(const OSSymbol *typeOfInterest,
                   IOServiceInterestHandler handler, void *target,
                   void *ref = NULL) APPLE_KEXT_OVERRIDE;
   ...
   *   @param whichAssertionsBits A bitfield specify the assertion that the
   * caller requests.
   *   @param assertionLevel An integer detailing the initial assertion level,
   * kIOPMDriverAssertionLevelOn or kIOPMDriverAssertionLevelOff.
   *   @param ownerService A pointer to the caller's IOService class, for
   * tracking.
   *   @param ownerDescription A reverse-DNS string describing the caller's
   * identity and reason.
   *   @result On success, returns a new assertion of type IOPMDriverAssertionID
   */
  IOPMDriverAssertionID
  createPMAssertion(IOPMDriverAssertionType whichAssertionsBits,
                    IOPMDriverAssertionLevel assertionLevel,
                    IOService *ownerService, const char *ownerDescription);

  /* @function setPMAssertionLevel
   *  @abstract Modify the level of a pre-existing assertion.
   *  @discussion Change the value of a PM assertion to influence system
   * behavior, without undergoing the work required to create or destroy an
   * assertion. Suggested for clients who will assert and de-assert needs for PM
   * behavior several times over their lifespan.
   *  @param assertionID An assertion ID previously returned by
   * <link>createPMAssertion</link>
   *  @param assertionLevel The new assertion level.
   *  @result kIOReturnSuccess if it worked; kIOReturnNotFound or other IOReturn
   * error on failure.
   */
  IOReturn setPMAssertionLevel(IOPMDriverAssertionID assertionID,
                               IOPMDriverAssertionLevel assertionLevel);
   ...
   *       <link>kIOPMDriverAssertionLevelOn</link>. If multiple assertions are
   * specified in the bitfield, only returns
   * <link>kIOPMDriverAssertionLevelOn</link> if all assertions are active.
   *   @param whichAssertionBits Bits defining the assertion or assertions the
   * caller is interested in the level of. If in doubt, pass
   * <link>kIOPMDriverAssertionCPUBit</link> as the argument.
   *   @result Returns <link>kIOPMDriverAssertionLevelOff</link> or
   *       <link>kIOPMDriverAssertionLevelOn</link> indicating the specified
   * assertion's levels, if available. If the assertions aren't supported on
   * this machine, or aren't recognized by the OS, the result is undefined.
   */
  IOPMDriverAssertionLevel
   ...
   *   @result Return kIOReturnSuccess if it work, kIOReturnError if the service
   * is not available.
   */
  ...
  IOReturn acquireDriverKitSyncedAssertion(IOService *from,
...
private:
  ...
  virtual IOReturn
  changePowerStateTo(unsigned long ordinal) APPLE_KEXT_COMPATIBILITY_OVERRIDE;
  virtual IOReturn changePowerStateToPriv(unsigned long ordinal);
  virtual IOReturn requestPowerDomainState(IOPMPowerFlags, IOPowerConnection *,
                                           unsigned long) APPLE_KEXT_OVERRIDE;
  ...
  IOReturn updateReportGated(uint64_t ch_id, void *result,
                             IOBufferMemoryDescriptor *dest);

#ifdef XNU_KERNEL_PRIVATE
  /* Root Domain internals */
public:
  void tagPowerPlaneService(IOService *service, IOPMActions *actions,
                            IOPMPowerStateIndex maxPowerState);

  void overrideOurPowerChange(IOService *service, IOPMActions *actions,
                              const IOPMRequest *request,
                              IOPMPowerStateIndex *inOutPowerState,
                              IOPMPowerChangeFlags *inOutChangeFlags);

  void handleOurPowerChangeStart(IOService *service, IOPMActions *actions,
                                 const IOPMRequest *request,
                                 IOPMPowerStateIndex powerState,
                                 IOPMPowerChangeFlags *inOutChangeFlags);

  void handleOurPowerChangeDone(IOService *service, IOPMActions *actions,
                                const IOPMRequest *request,
                                IOPMPowerStateIndex powerState,
                                IOPMPowerChangeFlags changeFlags);

  void overridePowerChangeForService(IOService *service, IOPMActions *actions,
                                     const IOPMRequest *request,
                                     IOPMPowerStateIndex *inOutPowerState,
                                     IOPMPowerChangeFlags *inOutChangeFlags);

  void handleActivityTickleForDisplayWrangler(IOService *service,
                                              IOPMActions *actions);
  ...
  bool shouldDelayChildNotification(IOService *service);
  ...
  void handlePowerChangeDoneForPCIDevice(IOService *service,
                                         IOPMActions *actions,
                                         const IOPMRequest *request,
                                         IOPMPowerStateIndex powerState,
                                         IOPMPowerChangeFlags changeFlags);

  void askChangeDownDone(IOPMPowerChangeFlags *inOutChangeFlags, bool *cancel);

  void handlePublishSleepWakeUUID(bool shouldPublish);

  void handleQueueSleepWakeUUID(OSObject *obj);
  ...
  void kdebugTrace(uint32_t event, uint64_t regId, uintptr_t param1,
                   uintptr_t param2, uintptr_t param3 = 0);
  void tracePoint(uint8_t point);
  void traceDetail(uint32_t msgType, uint32_t msgIndex, uint32_t delay);
  void traceNotification(OSObject *notifier, bool start, uint64_t ts = 0,
                         uint32_t msgIndex = UINT_MAX);
  void traceNotificationAck(OSObject *notifier, uint32_t delay_ms);
  void traceNotificationResponse(OSObject *object, uint32_t delay_ms,
  ...
  bool systemMessageFilter(void *object, void *arg1, void *arg2, void *arg3);

  bool updatePreventIdleSleepList(IOService *service, bool addNotRemove);
  void updatePreventSystemSleepList(IOService *service, bool addNotRemove);

  bool updatePreventIdleSleepListInternal(IOService *service, bool addNotRemove,
                                          unsigned int oldCount);
  unsigned int idleSleepPreventersCount();

  void publishPMSetting(const OSSymbol *feature, uint32_t where,
                        uint32_t *featureID);

  void pmStatsRecordEvent(int eventIndex, AbsoluteTime timestamp);

  void pmStatsRecordApplicationResponse(const OSSymbol *response,
                                        const char *name, int messageType,
                                        uint32_t delay_ms, uint64_t id,
                                        OSObject *object,
...
#if HIBERNATION
  bool getHibernateSettings(uint32_t *hibernateMode,
  ...
  IOReturn createPMAssertionSafe(IOPMDriverAssertionID *assertionID,
...
private:
  ...
  uint32_t
      sleepDelaysClientCnt; // Number of interested clients in sleepDelaysReport
  uint64_t ts_sleepStart;
  uint64_t
      wake2DarkwakeDelay; // Time taken to change from full wake -> Dark wake

  void *assertOnWakeReport; // report to track time spent without any assertions
                            // held after wake
  uint32_t assertOnWakeClientCnt; // Number of clients interested in
                                  // assertOnWakeReport
  ...
  // Update the current systemTransitionType and wakeup any waiters blocking on
  // transitions.
  void setSystemTransitionTypeGated(SystemTransitionType type);

  // Block until no system transitions are in progress and the current power
  // state has reached at a minimum state.
...
private:
  ...
  bool checkSystemSleepAllowed(IOOptionBits options, uint32_t sleepReason);
  ...
  IOReturn setAggressiveness(unsigned long type, unsigned long value,
                             IOOptionBits options);

  void synchronizeAggressives(queue_head_t *services,
                              const AggressivesRecord *array, int count);

  void broadcastAggressives(const AggressivesRecord *array, int count);
  ...
  void tracePhase2String(uint32_t tracePhase, const char **phaseString,
  ...
  void copySleepPreventersList(OSArray **idleSleepList,
                               OSArray **systemSleepList);
  void copySleepPreventersListWithID(OSArray **idleSleepList,
                                     OSArray **systemSleepList);
  void recordRTCAlarm(const OSSymbol *type, OSObject *object);
  void scheduleImmediateDebugWake(void);

  // Used to inform interested clients about low latency audio activity in the
  // system
