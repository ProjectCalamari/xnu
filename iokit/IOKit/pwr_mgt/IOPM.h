...
/*! @header IOPM.h
 *   @abstract Defines power management constants and keys used by both
 * in-kernel and user space power management.
 *   @discussion IOPM.h defines a range of power management constants used in
 * several in-kernel and user space APIs. Most significantly, the IOPMPowerFlags
 * used to specify the fields of an IOPMPowerState struct are defined here.
 *
 *   Most of the constants defined in IOPM.h are deprecated or for Apple
 * internal use only, and are not elaborated on in headerdoc.
 */

enum { kIOPMMaxPowerStates = 10, IOPMMaxPowerStates = kIOPMMaxPowerStates };

/*! @enum IOPMPowerFlags
 *   @abstract Bits are used in defining capabilityFlags,
 * inputPowerRequirements, and outputPowerCharacter in the IOPMPowerState
 * structure.
 *   @discussion These bits may be bitwise-OR'd together in the IOPMPowerState
 * capabilityFlags field, the outputPowerCharacter field, and/or the
 * inputPowerRequirement field.
 *
 *   The comments clearly mark whether each flag should be used in the
 * capabilityFlags field, outputPowerCharacter field, and inputPowerRequirement
 * field, or all three.
 *
 *   The value of capabilityFlags, inputPowerRequirement or outputPowerCharacter
 * may be 0. Most drivers implement their 'OFF' state, used when asleep, by
 * defininf each of the 3 fields as 0.
 *
 *   The bits listed below are only the most common bits used to define a
 * device's power states. Your device's IO family may require that your device
 * specify other input or output power flags to interact properly. Consult
 * family-specific documentation to determine if your IOPower plane parents or
 * children require other power flags; they probably don't.
 *
 *   @constant kIOPMPowerOn Indicates the device is on, requires power, and
 * provides power. Useful as a: Capability, InputPowerRequirement,
 * OutputPowerCharacter
 *
 *   @constant kIOPMDeviceUsable Indicates the device is usable in this state.
 ...
 *
 *   Useful only as a Capability, although USB drivers should consult USB family
 * documentation for other valid circumstances to use the kIOPMLowPower bit.
 *
 *   @constant kIOPMPreventIdleSleep
 *   In the capability field of a power state, disallows idle system sleep while
 * the device is in that state.
 *
 *   For example, displays and disks set this capability for their ON power
 * state; since the system may not idle sleep while the display (and thus
 * keyboard or mouse) or the disk is active.
 *
 *   Useful only as a Capability.
 *
 *   @constant kIOPMSleepCapability
 *   Used only by certain IOKit Families (USB). Not defined or used by generic
 * Power Management. Read your family documentation to see if you should define
 * a powerstate using these capabilities.
 *
 ...
 * <code>initialPowerStateForDomainState()</code> returns a power state with
 * this flag set in the capability field, then the initial power change is
 * performed without calling the driver's <code>setPowerState()</code>.
 *
...
enum { kIOPMBroadcastAggressiveness = 1, kIOPMUnidleDevice };
...
enum {
   ...
   * When set, the system will immediately do a full wakeup after going to
   * sleep.
   */
 ...
 *   indicates that a driver (or drivers) have asserted a need for CPU and
 * video.
 */
 ...
 * property kIOPMAssertionsDriverKey.
 *
 */
 ...
 */
enum { kClamshellStateBit = (1 << 0), kClamshellSleepBit = (1 << 1) };
 ...
 */
enum { kInflowForciblyEnabledBit = (1 << 0) };
...
// kIOPMPSBatteryChargeStatusKey may have one of the following values, or may
// have no value. If kIOPMBatteryChargeStatusKey has a NULL value (or no value)
// associated with it then charge is proceeding normally. If one of these
// battery charge status reasons is listed, then the charge may have been
// interrupted.
...
enum { kIOPMPSLocationLeft = 1001, kIOPMPSLocationRight = 1002 };
...
/* kIOPMCPUPowerLimitSchedulerTimeKey represents the percentage (0-100) of CPU
 * time available. 100% at normal operation. The OS may limit this time for a
 ...
 *      kIOPMThermalLevelCritical - thermal conditions may cause imminent
 * shutdown
 *
...
enum { kIOPMInternalPower = 1, kIOPMExternalPower };
...
#define kIOPMMessageBatteryStatusHasChanged                                    \
  iokit_family_msg(sub_iokit_pmu, 0x100)

// Apple private Legacy messages for re-routing AutoWake and AutoPower messages
// to the PMU through newer user space IOPMSchedulePowerEvent API
...
enum { kIOPowerEmergencyLevel = 1000 };
 ...
 *   @discussion A system capability change is a system level transition from a
 * set of system capabilities to a new set of system capabilities. Power
 * management sends a <code>kIOMessageSystemCapabilityChange</code> message and
 * provides this structure as the message data (by reference) to
 *       <code>gIOPriorityPowerStateInterest</code> clients when system
 * capability changes.
 *   @field notifyRef An identifier for this message notification. Clients with
 * pending I/O can signal completion by calling <code>allowPowerChange()</code>
 * with this value as the argument. Clients that are able to process the
 * notification synchronously should ignore this field.
 *   @field maxWaitForReply A return value to the caller indicating the maximum
 * time in microseconds to wait for the <code>allowPowerChange()</code> call.
 * The default value is zero, which indicates the client processing has
 * finished, and power management should not wait for an
 * <code>allowPowerChange()</code> call.
 *   @field changeFlags Flags will be set to indicate whether the notification
 * precedes the capability change
 * (<code>kIOPMSystemCapabilityWillChange</code>), or after the capability
 * change has occurred (<code>kIOPMSystemCapabilityDidChange</code>).
 *   @field __reserved1 Set to zero.
 *   @field fromCapabilities The system capabilities at the start of the
 * transition.
 ...
 *   @constant kIOPMSystemCapabilityWillChange Indicates the system capability
 * will change.
 *   @constant kIOPMSystemCapabilityDidChange Indicates the system capability
 * has changed.
 */
enum {
