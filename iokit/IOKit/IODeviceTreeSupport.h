...
bool IODTCompareNubName(
    const IORegistryEntry *regEntry, OSString *name,
...
typedef SInt32 (*IODTCompareAddressCellFunc)(UInt32 cellCount, UInt32 left[],
                                             UInt32 right[]);
#else
typedef SInt64 (*IODTCompareAddressCellFunc)(UInt32 cellCount, UInt32 left[],
                                             UInt32 right[]);
#endif

typedef void (*IODTNVLocationFunc)(IORegistryEntry *entry, UInt8 *busNum,
                                   UInt8 *deviceNum, UInt8 *functionNum);
...
bool IODTResolveAddressCell(IORegistryEntry *regEntry, UInt32 cellsIn[],
...
IODTResolveAddressing(IORegistryEntry *regEntry,
                      const char *addressPropertyName, IODeviceMemory *parent);
...
LIBKERN_RETURNS_NOT_RETAINED OSData *IODTFindSlotName(IORegistryEntry *regEntry,
                                                      UInt32 deviceNumber);
...
enum { kIODTInterruptShared = 0x00000001 };
