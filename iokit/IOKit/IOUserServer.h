...
public:
  /*
   * Launch a dext with the specified bundle ID, server name, and server tag. If
   * reuseIfExists is true, this will attempt to find an existing IOUserServer
   * instance or a pending dext launch with the same server name.
   *
   * Returns a IOUserServer instance if one was found, or a token to track the
   * pending dext launch. If both are NULL, then launching the dext failed.
   */
  static IOUserServer *launchUserServer(OSString *bundleID,
  ...
  static IOReturn externalMethodStart(OSObject *target, void *reference,
                                      IOExternalMethodArguments *arguments);
  static IOReturn
  externalMethodRegisterClass(OSObject *target, void *reference,
                              IOExternalMethodArguments *arguments);

  virtual IOExternalTrap *
  getTargetAndTrapForIndex(IOService **targetP,
                           UInt32 index) APPLE_KEXT_OVERRIDE;
  ...
  static void serviceWillTerminate(IOService *client, IOService *provider,
                                   IOOptionBits options);
  static void serviceDidTerminate(IOService *client, IOService *provider,
  ...
  IOReturn serviceSetPowerState(IOService *controllingDriver,
                                IOService *service, IOPMPowerFlags flags,
                                IOPMPowerStateIndex powerState);
  IOReturn serviceCreatePMAssertion(IOService *service, uint32_t assertionBits,
                                    uint64_t *assertionID, bool synced);
  IOReturn serviceReleasePMAssertion(IOService *service, uint64_t assertionID);
  IOReturn serviceNewUserClient(IOService *service, task_t owningTask,
                                void *securityID, uint32_t type,
  ...
  bool checkEntitlements(LIBKERN_CONSUMED OSObject *prop, IOService *provider,
                         IOService *dext);
  static bool checkEntitlements(OSDictionary *entitlements,
                                LIBKERN_CONSUMED OSObject *prop,
  ...
  IOReturn setPowerState(unsigned long state,
                         IOService *service) APPLE_KEXT_OVERRIDE;
  IOReturn powerStateWillChangeTo(IOPMPowerFlags flags, unsigned long state,
                                  IOService *service) APPLE_KEXT_OVERRIDE;
  IOReturn powerStateDidChangeTo(IOPMPowerFlags flags, unsigned long state,
                                 IOService *service) APPLE_KEXT_OVERRIDE;
  ...
  IOReturn registerClass(OSClassDescription *desc, uint32_t size,
                         OSUserMetaClass **cls);
  IOReturn registerClass(OSClassDescription *desc, uint32_t size,
                         OSSharedPtr<OSUserMetaClass> &cls);
  IOReturn setRootQueue(IODispatchQueue *queue);

  OSObjectUserVars *varsForObject(OSObject *obj);
  LIBKERN_RETURNS_NOT_RETAINED IODispatchQueue *queueForObject(OSObject *obj,
                                                               uint64_t msgid);

  static ipc_port_t copySendRightForObject(OSObject *object,
                                           ipc_kobject_type_t type);
  static OSObject *copyObjectForSendRight(ipc_port_t port,
                                          ipc_kobject_type_t type);
  ...
  IOReturn consumeObjects(IORPCMessageMach *mach, IORPCMessage *message,
                          size_t messageSize);
  ...
  IOReturn server(ipc_kmsg_t requestkmsg, IORPCMessage *message,
                  ipc_kmsg_t *preply);
  kern_return_t waitInterruptTrap(void *p1, void *p2, void *p3, void *p4,
                                  void *p5, void *p6);
...
typedef void (*IOUserServerCheckInCancellationHandler)(
    class IOUserServerCheckInToken *, void *);
...
public:
  ...
  /*
   * Set handler to be invoked when launch is cancelled. Returns an wrapper
   * object for the handler to be released by the caller. The handler always
   * runs under the lock for this IOUserServerCheckInToken. The returned object
   * can be used with removeCancellationHandler().
   */
  _IOUserServerCheckInCancellationHandler *
  setCancellationHandler(IOUserServerCheckInCancellationHandler handler,
                         void *handlerArgs);
   ...
   */
  void
  removeCancellationHandler(_IOUserServerCheckInCancellationHandler *handler);
...
private:
  static IOUserServerCheckInToken *
  findExistingToken(const OSSymbol *serverName);
  bool init(const OSSymbol *userServerName, OSNumber *serverTag,
            OSKext *driverKext, OSData *serverDUI);
...
extern "C" kern_return_t IOUserServerUEXTTrap(OSObject *object, void *p1,
...
public:
  ...
  IOReturn eventlinkConfigurationTrap(void *p1, void *p2, void *p3, void *p4,
                                      void *p5, void *p6);
  IOReturn workgroupConfigurationTrap(void *p1, void *p2, void *p3, void *p4,
                                      void *p5, void *p6);
  ...
  virtual IOReturn
  clientMemoryForType(UInt32 type, IOOptionBits *options,
