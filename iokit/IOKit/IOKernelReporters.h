...
protected:
  ...
  virtual bool init(IOService *reportingService,
                    IOReportChannelType channelType, IOReportUnit unit);
...
protected:
   ...
   */
  IOReturn updateReportChannel(int channel_index, int *nElements,
   ...
   *   @param  values - IORepoterElementValues to replace those at
   * _elements[idx]
   ...
   *
   *   @discussion Locking: Caller must ensure that the reporter (data) lock is
   * held. The returned pointer is only valid until unlockReporter() is called.
   */
   ...
   *   Locking: Caller must ensure that the reporter (data) lock is held.
   */
  virtual IOReturn getChannelIndices(uint64_t channel_id, int *channel_index,
                                     int *element_index);
   ...
   *   @param  channelCount - number of channels, and size of channelIDs and
   * channelNames
   ...
   *       raw arrays for the channelIDs and channelNames. It supports the
   * static createLegend() methods for the IOReporter subclasses.
   *
...
private:
   ...
   *
   *   @discussion
   *       This static method is the main legend creation function. It is called
   * by IOReporter sub-classes and is responsible for building an
...
public:
   ...
   *
   *   @param  reportingService - IOService associated with all channels
   *   @param  categories - The category in which the report should be
   * classified
   ...
   *
   *   @param  channel_id - ID of the channel for which the value needs to be
   * set
   ...
   *   Locking: same-instance concurrency SAFE, WILL NOT BLOCK
   */
  IOReturn setValue(uint64_t channel_id, int64_t value);
   ...
   *   Locking: same-instance concurrency SAFE, WILL NOT BLOCK
   */
  IOReturn incrementValue(uint64_t channel_id, int64_t increment);
   ...
   *
   *   @discussion
   *       This static method supports creating a legend entry for an
   * IOSimpleReporter without actually having an IOSimpleReporter instance. This
   * can be used to lazily create IOReporters in IOService::configureReport()
...
protected:
   ...
   *   @abstract   instance method implementation called by
   * IOSimpleReporter::with
   *
   *   @discussion
...
public:
   ...
   *
   *   @param  reportingService - The I/O Kit service for this reporter's
   * channels
  ...
  static OSPtr<IOStateReporter> with(IOService *reportingService,
                                     IOReportCategories categories, int nstates,
   ...
   *   Locking: same-instance concurrency SAFE, WILL NOT BLOCK
   */
  IOReturn setState(uint64_t new_state_id, uint64_t last_intransition,
                    uint64_t prev_state_residency) __deprecated;
   ...
   *   Locking: same-instance concurrency SAFE, WILL NOT BLOCK
   */
  IOReturn overrideChannelState(uint64_t channel_id, uint64_t state_id,
                                uint64_t time_in_state, uint64_t intransitions,
   ...
   *   Locking: same-instance concurrency SAFE, WILL NOT BLOCK
   */
  IOReturn incrementChannelState(uint64_t channel_id, uint64_t state_id,
                                 uint64_t time_in_state, uint64_t intransitions,
                                 uint64_t last_intransition = 0);
   ...
   *
   *   @discussion
   *       Some clients may need to consume internally the data aggregated by
   * the reporter object. This method allows a client to retrieve the count of
   ...
   *   @discussion
   *       This static method supports creating a legend entry for an
   * IOStateReporter without actually having an IOStateReporter instance. This
   * can be used to lazily create IOReporters in IOService::configureReport()
   *       rather than during driver initialization.
   */
  static OSPtr<IOReportLegendEntry> createLegend(const uint64_t *channelIDs,
...
protected:
   ...
   */
  virtual bool initWith(IOService *reportingService,
                        IOReportCategories categories, int16_t nstates,
                        IOReportUnit unit);
   ...
   *
   *   @discussion
   *       Locked version of IOReporter::setStateByIndices().  This method may
   ...
   *   Locking: Caller must ensure that the reporter (data) lock is held.
   */
  virtual IOReturn handleSetStateID(uint64_t channel_id, int state_index,
                                    uint64_t state_id);
   ...
   *   Locking: Caller must ensure that the reporter (data) lock is held.
   */
  virtual IOReturn handleOverrideChannelStateByIndices(
      int channel_index, int state_index, uint64_t time_in_state,
      uint64_t intransitions, uint64_t last_intransition = 0);
   ...
   *   Locking: Caller must ensure that the reporter (data) lock is held.
   */
  virtual IOReturn handleIncrementChannelStateByIndices(
      int channel_index, int state_index, uint64_t time_in_state,
      uint64_t intransitions, uint64_t last_intransition = 0);
...
private:
  ...
  uint64_t _getStateValue(uint64_t channel_id, uint64_t state_id,
                          enum valueSelector value);

  IOReturn _getStateIndices(uint64_t channel_id, uint64_t state_id,
                            int *channel_index, int *state_index);
};
...
public:
   ...
   *   @abstract   Initializes the IOHistogramReporter instance variables and
   * data structures
   *
   *   @param  reportingService - The I/O Kit service for this reporter's
   * channels
   *   @param  categories - The categories in which the report should be
   * classified
   *   @param  channelID - uint64_t channel identifier
   *   @param  channelName - rich channel name as char*
   *   @param  unit - The unit for the quantity recorded by the reporter object
   *   @param  nSegments - Number of segments to be extracted from the config
   * data structure
   *   @param  config - Histograms require the caller to pass a configuration by
   * segments
  ...
  with(IOService *reportingService, IOReportCategories categories,
       uint64_t channelID, const char *channelName, IOReportUnit unit,
       int nSegments, IOHistogramSegmentConfig *config);
   ...
   */
  IOReturn addChannel(__unused uint64_t channelID,
                      __unused const char *channelName = NULL) {
   ...
   *
   *   @discussion
   *       Replaces data in the bucket at the specified index with the data
   * pointed to by bucket. No sanity check is performed on the data. If the
   * index is out of bounds, kIOReturnBadArgument is returned.
   *
   *   Locking: same-instance concurrency SAFE, WILL NOT BLOCK
   */

  IOReturn overrideBucketValues(unsigned int index, uint64_t bucket_hits,
                                int64_t bucket_min, int64_t bucket_max,
                                int64_t bucket_sum);
   ...
   *   @param  channelID - uint64_t channel identifier
   *   @param  channelName - rich channel name as char*
   *   @param  segmentCount - Number of segments to be extracted from the config
   * data structure
   *   @param  config - Histograms require the caller to pass a configuration by
   * segments
   *   @param  categories - The category in which the report should be
  ...
  createLegend(const uint64_t channelID, const char *channelName,
               int segmentCount, IOHistogramSegmentConfig *config,
               IOReportCategories categories, IOReportUnit unit);
...
protected:
   ...
   */
  virtual bool initWith(IOService *reportingService,
                        IOReportCategories categories, uint64_t channelID,
                        const OSSymbol *channelName, IOReportUnit unit,
                        int nSegments, IOHistogramSegmentConfig *config);

  /*! @function   IOHistogramReporter::handleCreateLegend
   *   @abstract   Builds an IOReporting legend entry representing the channels
   * of this reporter.
   *
...
public:
   ...
   *
   *   @param  legend - OSArray of the legend possibly already present in
   * registry
  ...
  IOReturn addLegendEntry(IOReportLegendEntry *legendEntry,
                          const char *groupName, const char *subGroupName);
   ...
   */
  IOReturn addReporterLegend(IOReporter *reporter, const char *groupName,
                             const char *subGroupName);

  /*! @function   IOReportLegend::addReporterLegend
   *   @abstract   Add a legend entry from a reporter object
   *
   *   @param  reportingService - IOService data provider into the reporter
   * object
   *   @param  reporter - IOReporter to use to extract and append the legend
   *   @param  groupName - primary group name for this entry
   *   @param  subGroupName - secondary group name for this entry
  ...
  static IOReturn addReporterLegend(IOService *reportingService,
                                    IOReporter *reporter, const char *groupName,
                                    const char *subGroupName);

  /*! @function   IOReportLegend::getLegend
   *   @abstract   Accessor method to get the legend array
   *
   *   @result     Returns the OSObject holding the legend to be published by
   * the driver
   *   @discussion
...
private:
   ...
   *   @abstract   Sets up the legend entry, organizing it with group and
   * sub-group names
   *
   ...
   *
   *   @param  legend - OSArray legend object containing the description of all
   * reporters the driver is able to address
   ...
   *       Convenience method to create all the driver's reporter objects from a
   * legend. Can be used when a legend is made public through the IORegistry but
   * IOReporter objects have not yet been created to save memory, waiting for
   * observers. Upon a call to configureReport via the IOService method, a
   * driver could create all reporter objects on the fly using this function.
   */
