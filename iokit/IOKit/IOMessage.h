...
#define iokit_common_msg(message)                                              \
  (UInt32)(sys_iokit | sub_iokit_common | message)
#define iokit_family_msg(sub, message) (UInt32)(sys_iokit | sub | message)

/*!
 * @defined         iokit_vendor_specific_msg
 * @discussion      iokit_vendor_specific_msg passes messages in the
 * sub_iokit_vendor_specific subsystem. It can be used to generate messages that
 * are used for private communication between vendor specific code with the
 * IOService::message() etc. APIs.
 */
#define iokit_vendor_specific_msg(message)                                     \
  (UInt32)(sys_iokit | sub_iokit_vendor_specific | message)
 ...
 * @discussion      Indicates the device is about to move to a lower power
 * state. Sent to IOKit interest notification clients of type
 * <code>kIOAppPowerStateInterest</code> and <code>kIOGeneralInterest</code>.
 */
 ...
 *                  Sent to IOKit interest notification clients of type
 * <code>kIOAppPowerStateInterest</code> and <code>kIOGeneralInterest</code>.
 */
 ...
 * @discussion      Indicates an imminent system shutdown. Recipients have a
 * limited amount of time to respond, otherwise the system will timeout and
 *                  shutdown even without a response.
 *                  Delivered to in-kernel IOKit drivers via
 * <code>IOService::systemWillShutdown()</code>, and to clients of
 * <code>registerPrioritySleepWakeInterest()</code>. Never delivered to user
 * space notification clients.
 */
#define kIOMessageSystemWillPowerOff iokit_common_msg(0x250)

/*!
 * @defined         kIOMessageSystemWillRestart
 * @discussion      Indicates an imminent system restart. Recipients have a
 * limited amount of time to respond, otherwise the system will timeout and
 ...
 * @discussion      Indicates an imminent system shutdown, paging device now
 * unavailable. Recipients have a limited amount of time to respond, otherwise
 * the system will timeout and shutdown even without a response. Delivered to
 * clients of <code>registerPrioritySleepWakeInterest()</code>. Never delivered
 * to user space notification clients.
 */
 ...
 *                  Delivered to in-kernel IOKit drivers via
 * <code>kIOGeneralInterest</code> and
 * <code>kIOPriorityPowerStateInterest</code>. Delivered to user clients of
 * <code>IORegisterForSystemPower</code>.
 */
#define kIOMessageCanSystemSleep iokit_common_msg(0x270)

/*!
 * @defined         kIOMessageSystemWillNotSleep
 * @discussion      Announces that the system has retracted a previous attempt
 * to sleep; it follows <code>kIOMessageCanSystemSleep</code>. Delivered to
 * in-kernel IOKit drivers via <code>kIOGeneralInterest</code> and
 * <code>kIOPriorityPowerStateInterest</code>. Delivered to user clients of
 * <code>IORegisterForSystemPower</code>.
 */
 ...
 * @discussion      Announces that the system is beginning to power the device
 * tree; most devices are unavailable at this point.. Delivered to in-kernel
 * IOKit drivers via <code>kIOGeneralInterest</code> and
 * <code>kIOPriorityPowerStateInterest</code>. Delivered to user clients of
 * <code>IORegisterForSystemPower</code>.
 */
 ...
 * @discussion      Delivered to <code>kIOAppPowerStateInterest</code> clients
 * of devices that implement their own idle timeouts. This message type is
 * almost never used.
 */
 ...
 * @discussion      IOService power mgt does not send
