...
typedef struct IOWorkLoopCounter {
  ...
  RB_PROTOTYPE_SC(static, DependencyTree, IOWorkLoopDependency, dependencyLink,
                  KextTagCompare);
} IOWorkLoopCounter;
...
class IOStatistics {
  ...
  RB_PROTOTYPE_SC(static, KextAddressTree, KextNode, addressLink,
  ...
  static uint32_t copyUserClientStatistics(IOStatisticsUserClientHeader *stats,
                                           uint32_t loadTag);

  static void updateAllocationCounter(vm_offset_t address, uint32_t index,
                                      vm_size_t size);

  static void storeUserClientCallInfo(IOUserClient *userClient,
...
public:
   ...
   *
   * NOTE: counter access is not expressly guarded here so as not to incur
   * performance penalties in the instrumented parent objects. Writes are
   * arranged so as to be protected by pre-existing locks in the parent where
   * appropriate, but reads have no such guarantee. Counters should therefore be
   * regarded as providing an indication of current state, rather than precisely
   * accurate statistics.
   */

  static inline void setCounterType(IOEventSourceCounter *counter,
                                    IOStatisticsCounterType type) {
    if (counter) {
      counter->type = type;
    }
  ...
  /* WorkLoop */
  static void attachWorkLoopEventSource(IOWorkLoopCounter *wlc,
                                        IOEventSourceCounter *esc);
  static void detachWorkLoopEventSource(IOWorkLoopCounter *wlc,
                                        IOEventSourceCounter *esc);
