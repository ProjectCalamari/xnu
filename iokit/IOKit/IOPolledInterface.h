...
enum { kIOPolledWrite = 1, kIOPolledRead = 2, kIOPolledFlush = 3 };

typedef void (*IOPolledCompletionAction)(void *target, void *parameter,
                                         IOReturn status,
...
public:
  ...
  virtual IOReturn startIO(uint32_t operation, uint32_t bufferOffset,
...
enum { kIOPolledFileSSD = 0x00000001 };
...
// a file up to length set_file_size_max is created leaving fsFreeSize bytes
// free, but length no smaller than set_file_size_min regardless of disk free
// space.

IOReturn IOPolledFileOpen(const char *filename, uint32_t flags,
                          uint64_t setFileMinSize, uint64_t setFileMaxSize,
...
extern __C IOReturn IOPolledFileSeek(IOPolledFileIOVars *vars,
                                     uint64_t position);
...
extern __C IOReturn IOPolledFileRead(IOPolledFileIOVars *vars, uint8_t *bytes,
                                     IOByteCount size,
                                     IOPolledFileCryptVars *cryptvars);

extern __C IOReturn IOPolledFileFlush(IOPolledFileIOVars *vars);

extern __C IOReturn IOPolledFilePollersOpen(IOPolledFileIOVars *vars,
                                            uint32_t state, bool abortable);

extern __C IOReturn IOPolledFilePollersClose(IOPolledFileIOVars *vars,
                                             uint32_t state);

extern __C IOReturn IOPolledFilePollersSetEncryptionKey(
    IOPolledFileIOVars *vars, const uint8_t *key, size_t keySize);
...
typedef void (*kern_get_file_extents_callback_t)(void *ref, uint64_t start,
                                                 uint64_t size);

struct kern_direct_file_io_ref_t *kern_open_file_for_direct_io(
    const char *name, uint32_t flags, kern_get_file_extents_callback_t callback,
    void *callback_ref, off_t set_file_size_min, off_t set_file_size_max,
    off_t fs_free_size, off_t write_file_offset, void *write_file_addr,
    size_t write_file_len, dev_t *partition_device_result,
    dev_t *image_device_result, uint64_t *partitionbase_result,
    uint64_t *maxiocount_result, uint32_t *oflags);
