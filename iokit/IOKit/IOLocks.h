...
/*! @var IOLockGroup
 *   Global lock group used by all IOKit locks.  To simplify kext debugging and
 * lock-heat analysis, consider using lck_* locks with a per-driver lock group,
 * as defined in kern/locks.h.
 */
 ...
 *   @discussion Allocates a mutex in general purpose memory, and initializes
 * it. Mutexes are general purpose blocking mutual exclusion locks, supplied by
 * libkern/locks.h. This function may block and so should not be called from
 * interrupt level or while a spin lock is held.  IOLocks use the global IOKit
 * lock group, IOLockGroup.  To simplify kext debugging and lock-heat analysis,
 * consider using lck_* locks with a per-driver lock group, as defined in
 * kern/locks.h.
 ...
 *   @discussion Frees a lock allocated with IOLockAlloc. Mutex should be
 ...
 *   @discussion Lock the mutex. If the lock is held by any thread, block
 * waiting for its unlock. This function may block and so should not be called
 * from interrupt level or while a spin lock is held. Locking the mutex
 ...
 *   @discussion Lock the mutex if it is currently unlocked, and return true. If
 * the lock is held by any thread, return false.
 *   @param lock Pointer to the allocated lock.
 *   @result True if the mutex was unlocked and is now locked by the caller,
 * otherwise false. */
 ...
 *  @discussion Unlock the mutex and wake any blocked waiters. Results are
 * undefined if the caller has not locked the mutex. This function may block and
 * so should not be called from interrupt level or while a spin lock is held.
 *   @param lock Pointer to the allocated lock. */
 ...
 *  @discussion Prepare to sleep,unlock the mutex, and re-acquire it on wakeup.
 * Results are undefined if the caller has not locked the mutex. This function
 * may block and so should not be called from interrupt level or while a spin
 * lock is held.
...
int IOLockSleepDeadline(IOLock *lock, void *event, AbsoluteTime deadline,
                        UInt32 interType) __DARWIN14_ALIAS(IOLockSleepDeadline);
...
int IOLockSleepWithInheritor(IOLock *lock, UInt32 lck_sleep_action, void *event,
                             thread_t inheritor, UInt32 interType,
                             uint64_t deadline);
 ...
 * kIOLockAssertOwned, and vice-versa.
 */
...
typedef enum { kIOLockStateUnlocked = 0, kIOLockStateLocked = 1 } IOLockState;
 ...
 *   @discussion Allocates a recursive lock in general purpose memory, and
 * initializes it. Recursive locks function identically to mutexes but allow one
 * thread to lock more than once, with balanced unlocks.  IORecursiveLocks use
 * the global IOKit lock group, IOLockGroup.  To simplify kext debugging and
 * lock-heat analysis, consider using lck_* locks with a per-driver lock group,
 * as defined in kern/locks.h.
 *   @result Pointer to the allocated lock, or zero on failure. */
 ...
 *   @discussion Frees a lock allocated with IORecursiveLockAlloc. Lock should
 * be unlocked with no waiters.
 *   @param lock Pointer to the allocated lock. */
 ...
 *   @discussion Lock the recursive lock. If the lock is held by another thread,
 * block waiting for its unlock. This function may block and so should not be
 * called from interrupt level or while a spin lock is held. The lock may be
 * taken recursively by the same thread, with a balanced number of calls to
 ...
 *   @discussion Lock the lock if it is currently unlocked, or held by the
 * calling thread, and return true. If the lock is held by another thread,
 * return false. Successful calls to IORecursiveLockTryLock should be balanced
 * with calls to IORecursiveLockUnlock.
 *   @param lock Pointer to the allocated lock.
 ...
 *  @discussion Undo one call to IORecursiveLockLock, if the lock is now
 * unlocked wake any blocked waiters. Results are undefined if the caller does
 * not balance calls to IORecursiveLockLock with IORecursiveLockUnlock. This
 * function may block and so should not be called from interrupt level or while
 * a spin lock is held.
 *   @param lock Pointer to the allocated lock. */
 ...
 *   @discussion If the lock is held by the calling thread, return true,
 * otherwise the lock is unlocked, or held by another thread and false is
 * returned.
 *   @param lock Pointer to the allocated lock.
...
extern int IORecursiveLockSleep(IORecursiveLock *_lock, void *event,
                                UInt32 interType);
extern int IORecursiveLockSleepDeadline(IORecursiveLock *_lock, void *event,
                                        AbsoluteTime deadline,
                                        UInt32 interType);
extern void IORecursiveLockWakeup(IORecursiveLock *_lock, void *event,
                                  bool oneThread);
 ...
 *   @discussion Allocates and initializes a read/write lock in general purpose
 * memory. Read/write locks provide for multiple readers, one exclusive writer,
 * and are supplied by libkern/locks.h. This function may block and so should
 * not be called from interrupt level or while a spin lock is held.  IORWLocks
 * use the global IOKit lock group, IOLockGroup.  To simplify kext debugging and
 * lock-heat analysis, consider using lck_* locks with a per-driver lock group,
 * as defined in kern/locks.h.
 *   @result Pointer to the allocated lock, or zero on failure. */
 ...
 *  @discussion Frees a lock allocated with IORWLockAlloc. Lock should be
 * unlocked with no waiters.
 *   @param lock Pointer to the allocated lock. */
 ...
 *  @discussion Lock the lock for read, allowing multiple readers when there are
 * no writers. If the lock is held for write, block waiting for its unlock. This
 * function may block and so should not be called from interrupt level or while
 * a spin lock is held. Locking the lock recursively from one thread, for read
 * or write, can result in deadlock.
 *   @param lock Pointer to the allocated lock. */
 ...
 *  @discussion Lock the lock for read, allowing multiple readers when there are
 * no writers. If the lock is held for write, return false. Return true
 * otherwise.
 *   @param lock Pointer to the allocated lock. */
 ...
 *   @discussion Lock the lock for write, allowing one writer exlusive access.
 * If the lock is held for read or write, block waiting for its unlock. This
 * function may block and so should not be called from interrupt level or while
 * a spin lock is held. Locking the lock recursively from one thread, for read
 * or write, can result in deadlock.
 *   @param lock Pointer to the allocated lock. */
 ...
 *   @discussion Lock the lock for write, allowing one writer exlusive access.
 * If the lock is held for read or write, return false. Return true otherwise.
 *   @param lock Pointer to the allocated lock. */
 ...
 *   @discussion Undo one call to IORWLockRead or IORWLockWrite. Results are
 * undefined if the caller has not locked the lock. This function may block and
 * so should not be called from interrupt level or while a spin lock is held.
 *   @param lock Pointer to the allocated lock. */
 ...
 *   @discussion Allocates and initializes a spin lock in general purpose
 * memory. Spin locks provide non-blocking mutual exclusion for synchronization
 * between thread context and interrupt context, or for multiprocessor
 * synchronization, and are supplied by libkern/locks.h. This function may block
 * and so should not be called from interrupt level or while a spin lock is
 * held.  IOSimpleLocks use the global IOKit lock group, IOLockGroup.  To
 * simplify kext debugging and lock-heat analysis, consider using lck_* locks
 * with a per-driver lock group, as defined in kern/locks.h.
 *   @result Pointer to the allocated lock, or zero on failure. */
 ...
 *   @discussion Initialize a non heap allocated spin lock to the unlocked
 * state. Use this function when your lock is, for example, a member variable.
 * You will need to call IOSimpleLockDestroy when you are finished with the lock
 * to avoid lock group refcount leaks.
 *   @param lock Pointer to the lock. */

void IOSimpleLockInit(IOSimpleLock *lock);

/*! @function IOSimpleLockDestroy
 *   @abstract De-initializes (destroys) a spin lock initialized with
 * IOSimpleLockInit
 *   @discussion Destroy / De-initialize a non heap allocated spin lock,
 * releasing any system resources such as lock group refcounts.
 *   @param lock Pointer to the lock. */
 ...
 *  @discussion Lock the spin lock. If the lock is held, spin waiting for its
 * unlock. Spin locks disable preemption, cannot be held across any blocking
 * operation, and should be held for very short periods. When used to
 * synchronize between interrupt context and thread context they should be
 * locked with interrupts disabled - IOSimpleLockLockDisableInterrupt() will do
 * both. Locking the lock recursively from one thread will result in deadlock.
 *   @param lock Pointer to the lock. */
 ...
 *  @discussion Lock the spin lock if it is currently unlocked, and return true.
 * If the lock is held, return false. Successful calls to IOSimpleLockTryLock
 * should be balanced with calls to IOSimpleLockUnlock.
 *   @param lock Pointer to the lock.
 *   @result True if the lock was unlocked and is now locked by the caller,
 * otherwise false. */
 ...
 *   @discussion Unlock the lock, and restore preemption. Results are undefined
 * if the caller has not locked the lock.
 *   @param lock Pointer to the lock. */
 ...
 *  @abstract   Assert that spinlock is either held or not held by current
 * thread.
 *  @discussion Call with either kIOSimpleLockAssertOwned or
 * kIOSimpleLockAssertNotOwned. Panics the kernel if the lock is not owned if
 * called with kIOSimpleLockAssertOwned and vice-versa.
 */
 ...
 *   @abstract Lock a spin lock.
 *   @discussion Lock the spin lock. If the lock is held, spin waiting for its
 * unlock. Simple locks disable preemption, cannot be held across any blocking
 * operation, and should be held for very short periods. When used to
 * synchronize between interrupt context and thread context they should be
 * locked with interrupts disabled - IOSimpleLockLockDisableInterrupt() will do
 * both. Locking the lock recursively from one thread will result in deadlock.
...
static __inline__ IOInterruptState
 ...
 *   @discussion Unlock the lock, and restore preemption and interrupts to the
 * state as they were when the lock was taken. Results are undefined if the
 * caller has not locked the lock.
 *   @param lock Pointer to the lock.
 *   @param state The interrupt state returned by
 * IOSimpleLockLockDisableInterrupt() */

static __inline__ void
IOSimpleLockUnlockEnableInterrupt(IOSimpleLock *lock, IOInterruptState state) {
