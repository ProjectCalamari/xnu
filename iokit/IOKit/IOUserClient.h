...
/*! @enum
 *   @abstract Constant to denote a variable length structure argument to
 * IOUserClient.
 *   @constant kIOUCVariableStructureSize Use in the structures
 * IOExternalMethod, IOExternalAsyncMethod, IOExternalMethodDispatch to specify
 * the size of the structure is variable.
 */
enum { kIOUCVariableStructureSize = 0xffffffff };

typedef IOReturn (IOService::*IOMethod)(void *p1, void *p2, void *p3, void *p4,
                                        void *p5, void *p6);
...
typedef IOReturn (IOService::*IOTrap)(void *p1, void *p2, void *p3, void *p4,
                                      void *p5, void *p6);
...
enum { kIOUserNotifyMaxMessageSize = 64 };
...
/*! @enum
 *   @abstract Constants to specify the maximum number of scalar arguments in
 * the IOExternalMethodArguments structure. These constants are documentary
 * since the scalarInputCount, scalarOutputCount fields reflect the actual
 * number passed.
 *   @constant kIOExternalMethodScalarInputCountMax The maximum number of
 * scalars able to passed on input.
 *   @constant kIOExternalMethodScalarOutputCountMax The maximum number of
 * scalars able to passed on output.
 */
enum {
...
typedef IOReturn (*io_filter_resolver_t)(task_t task, IOUserClient *client,
                                         uint32_t type,
                                         io_filter_policy_t *filterp);
typedef IOReturn (*io_filter_applier_t)(IOUserClient *client,
...
/*!
 *   @class IOUserClient
 *   @abstract   Provides a basis for communication between client applications
 * and I/O Kit objects.
 */
...
public:
  ...
  UInt8 closed;
  UInt8 __ipcFinal;
  UInt8 messageAppSuspended : 1, uc2022 : 1, defaultLocking : 1,
      defaultLockingSingleThreadExternalMethod : 1,
      defaultLockingSetProperties : 1, opened : 1, __reservedA : 2;
...
public:
  ...
  MIG_SERVER_ROUTINE virtual IOReturn
  registerNotificationPort(mach_port_t port, UInt32 type,
...
public:
  ...
  OSPtr<IOMemoryMap> mapClientMemory64(IOOptionBits type, task_t task,
  ...
  io_filter_policy_t filterForTask(task_t task,
...
public:
  static IOReturn
  registerFilterCallbacks(const struct io_filter_callbacks *callbacks,
                          size_t size);
#endif /* PRIVATE */

protected:
  static IOReturn sendAsyncResult(OSAsyncReference reference, IOReturn result,
                                  void *args[], UInt32 numArgs);
  static void setAsyncReference(OSAsyncReference asyncRef, mach_port_t wakePort,
                                void *callback, void *refcon);

  static IOReturn sendAsyncResult64(OSAsyncReference64 reference,
                                    IOReturn result, io_user_reference_t args[],
                                    UInt32 numArgs);

  /*!
   *   @function sendAsyncResult64WithOptions
   *   @abstract Send a notification as with sendAsyncResult, but with finite
   * queueing.
   *   @discussion IOUserClient::sendAsyncResult64() will infitely queue
   * messages if the client is not processing them in a timely fashion.  This
   * variant will not, for simple handling of situations where clients may be
   * expected to stop processing messages.
   */
  ...
  static void setAsyncReference64(OSAsyncReference64 asyncRef,
                                  mach_port_t wakePort,
                                  mach_vm_address_t callback,
                                  io_user_reference_t refcon, task_t task);

public:
  static IOReturn clientHasAuthorization(task_t task, IOService *service);

  static IOReturn clientHasPrivilege(void *securityToken,
                                     const char *privilegeName);

  static OSPtr<OSObject> copyClientEntitlement(task_t task,
                                               const char *entitlement);
  static OSPtr<OSObject> copyClientEntitlementVnode(struct vnode *vnode,
                                                    off_t offset,
                                                    const char *entitlement);

  static OSPtr<OSDictionary> copyClientEntitlements(task_t task);
  static OSPtr<OSDictionary> copyClientEntitlementsVnode(struct vnode *vnode,
                                                         off_t offset);

  /*!
   *   @function releaseAsyncReference64
   *   @abstract Release the mach_port_t reference held within the
   * OSAsyncReference64 structure.
   *   @discussion The OSAsyncReference64 structure passed to async methods
   * holds a reference to the wakeup mach port, which should be released to
   * balance each async method call. Behavior is undefined if these calls are
   * not correctly balanced.
   *   @param reference The reference passed to the subclass IOAsyncMethod, or
   * externalMethod() in the IOExternalMethodArguments.asyncReference field.
   ...
   *   @discussion The mach_port_t passed to the registerNotificationPort()
   * methods should be released to balance each call to
   * registerNotificationPort(). Behavior is undefined if these calls are not
   * correctly balanced.
   *   @param port The mach_port_t argument previously passed to the subclass
   * implementation of registerNotificationPort().
   *   @result A return code.
   */
  ...
  MIG_SERVER_ROUTINE virtual IOReturn
  registerNotificationPort(mach_port_t port, UInt32 type, UInt32 refCon);

  MIG_SERVER_ROUTINE virtual IOReturn
  getNotificationSemaphore(UInt32 notification_type, semaphore_t *semaphore);
...
private:
  ...
  OSPtr<IOMemoryMap> mapClientMemory(IOOptionBits type, task_t task,
                                     IOOptionBits mapFlags = kIOMapAnywhere,
  ...
  static IOReturn _sendAsyncResult64(OSAsyncReference64 reference,
                                     IOReturn result,
                                     io_user_reference_t args[], UInt32 numArgs,
                                     IOOptionBits options);

public:
  /*!
   *   @function removeMappingForDescriptor
   *   Remove the first mapping created from the memory descriptor returned by
   * clientMemoryForType() from IOUserClient's list of mappings. If such a
   * mapping exists, it is retained and the reference currently held by
   * IOUserClient is returned to the caller.
   *   @param memory The memory descriptor instance previously returned by the
   * implementation of clientMemoryForType().
   *   @result A reference to the first IOMemoryMap instance found in the list
   * of mappings created by IOUserClient from that passed memory descriptor is
   * returned, or zero if none exist. The caller should release this reference.
   */
   ...
   *   The port holds a reference on the object, this function does not consume
   * any reference on the object.
   *   @param port_name Returned value is the task's port name. It has one send
   * right created by this function.
   *   @result A return code.
   */
   ...
   *   Look up an OSObject given a task's port name created with
   * copyPortNameForObjectInTask().
   *   @param task The task.
   *   @param port_name The task's port name. This function does not consume any
   * reference on the port name.
   *   @param object If the port name is valid, a reference to the object is
   * returned. It should be released by the caller.
   *   @result A return code.
   */
  static IOReturn copyObjectForPortNameInTask(task_t task,
                                              mach_port_name_t port_name,
                                              OSObject **object);
   ...
   *   Adjust the send rights for a port name created with
   * copyPortNameForObjectInTask().
   *   @param task The task.
   ...
   *   @result A return code.
   */
  static IOReturn adjustPortNameReferencesInTask(task_t task,
                                                 mach_port_name_t port_name,
...
#define IOUC_COPYPORTNAMEFOROBJECTINTASK 1
  ...
  getExternalMethodForIndex(UInt32 index) APPLE_KEXT_DEPRECATED;
  virtual IOExternalAsyncMethod *
  getExternalAsyncMethodForIndex(UInt32 index) APPLE_KEXT_DEPRECATED;

  // Methods for accessing method vector.
  virtual IOExternalMethod *
  getTargetAndMethodForIndex(LIBKERN_RETURNS_NOT_RETAINED IOService **targetP,
                             UInt32 index);
  virtual IOExternalAsyncMethod *getAsyncTargetAndMethodForIndex(
      LIBKERN_RETURNS_NOT_RETAINED IOService **targetP, UInt32 index);
  IOExternalMethod *getTargetAndMethodForIndex(OSSharedPtr<IOService> &targetP,
                                               UInt32 index);
  IOExternalAsyncMethod *
  getAsyncTargetAndMethodForIndex(OSSharedPtr<IOService> &targetP,
                                  UInt32 index);

  // Methods for accessing trap vector - old and new style
  virtual IOExternalTrap *
  getExternalTrapForIndex(UInt32 index) APPLE_KEXT_DEPRECATED;

  virtual IOExternalTrap *
  getTargetAndTrapForIndex(LIBKERN_RETURNS_NOT_RETAINED IOService **targetP,
                           UInt32 index);
};
...
/*
 *  IOUserClient2022 is a new superclass for an IOUserClient implementation to
 * opt-in to several security related best practices. The changes in behavior
 * are:
 *  - these properties must be present after ::start completes to control
 * default single threading calls to the IOUC from clients. It is recommended to
 ...
 *  Entitlements required for a process to open the IOUC (see the key
 * description). It is recommended to require an entitlement if all calling
 * processes are from Apple.
 *
 *  - the externalMethod override is required and must call
 * dispatchExternalMethod() to do basic argument checking before calling
 * subclass code to implement the method. dispatchExternalMethod() is called
 * with an array of IOExternalMethodDispatch2022 elements and will index into
 * the array for the given selector. The selector should be adjusted
 * accordingly, if needed, in the subclass' externalMethod(). The allowAsync
 * field of IOExternalMethodDispatch2022 must be true to allow the
 *  IOConnectCallAsyncMethod(etc) APIs to be used with the method.
 *  If the checkEntitlement field of IOExternalMethodDispatch2022 is non-NULL,
 * then the calling process must have the named entitlement key, with a value of
 * boolean true, or kIOReturnNotPrivileged is returned. This should be used when
 * per-selector entitlement checks are required.  If you only need to check at
 * the time the connection is created, use kIOUserClientEntitlementsKey instead.
 */
...
private:
  ...
  externalMethod(uint32_t selector, IOExternalMethodArguments *arguments,
                 ...
                 void *reference = NULL) APPLE_KEXT_OVERRIDE;

protected:
  IOReturn dispatchExternalMethod(
...
public:
  ...
  externalMethod(uint32_t selector,
                 IOExternalMethodArgumentsOpaque *arguments) = 0;
