...
private:
  ...
  bool handleSpecialVariables(const char *name, const uuid_t guid,
  ...
  OSSharedPtr<OSObject> copyPropertyWithGUIDAndName(const uuid_t guid,
                                                    const char *name) const;
  IOReturn removePropertyWithGUIDAndName(const uuid_t guid, const char *name);
  IOReturn setPropertyWithGUIDAndName(const uuid_t guid, const char *name,
                                      OSObject *anObject);
...
public:
  virtual bool init(IORegistryEntry *old,
  ...
  virtual OSPtr<OSDictionary>
  dictionaryWithProperties(void) const APPLE_KEXT_OVERRIDE;
