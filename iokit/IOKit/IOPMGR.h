...
public:
   ...
   * @param info                 Pointer to the ml_processor_info_t struct that
   * is being initialized (and hasn't been registered yet).
   */
  virtual void initCPUIdle(ml_processor_info_t *info) = 0;

  /*!
   * @function                   enterCPUIdle
   * @abstract                   Called from cpu_idle() prior to entering the
   * idle state on the current CPU.
   * @param newIdleTimeoutTicks  If non-NULL, will be overwritten with a new
   * idle timeout value, in ticks.  If the value is 0, XNU will disable the idle
   * timer.
   */
   ...
   * @abstract                   Called from cpu_idle_exit() after leaving the
   * idle state on the current CPU.
   * @param newIdleTimeoutTicks  If non-NULL, will be overwritten with a new
   * idle timeout value, in ticks.  If the value is 0, XNU will disable the idle
   * timer.
   ...
   * @abstract                   Called from timer_intr() to ask when to
   * schedule the next idle timeout on the current CPU.
   * @param newIdleTimeoutTicks  If non-NULL, will be overwritten with a new
   * idle timeout value, in ticks.  If the value is 0, XNU will disable the idle
   * timer.
   */
