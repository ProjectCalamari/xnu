...
enum {
  ...
  kIOLogTracePower =
      0x00000400ULL, // Obsolete: Use iotrace=0x00000400ULL to enable now
  ...
  kOSLogRegistryMods =
      0x00010000ULL, // Log attempts to modify registry collections
  kIOLogPMRootDomain = 0x00020000ULL,
  kOSRegistryModsMode = 0x00040000ULL, // Change default registry modification
                                       // handling - panic vs. log
  //    kIOTraceIOService   =         0x00080000ULL,  // Obsolete: Use
  //    iotrace=0x00080000ULL to enable now
  ...
  //    kIOLogSynchronous   =         0x00200000ULL,  // IOLog completes
  //    synchronously -- obsolete
...
enum {
  kIOKitDebugUserOptions =
      0 | kIOLogAttach | kIOLogProbe | kIOLogStart | kIOLogRegister |
      kIOLogMatch | kIOLogConfig | kIOLogYield | kIOLogPower | kIOLogMapping |
      kIOLogCatalogue | kIOLogTracePower | kIOLogDebugPower |
      kOSLogRegistryMods | kIOLogPMRootDomain | kOSRegistryModsMode |
      kIOLogHibernate | kIOSleepWakeWdogOff | kIOKextSpinDump |
      kIOWaitQuietPanics | kIOLogExclaves
};
...
#define DKLOG(fmt, args...)                                                    \
  {                                                                            \
    IOLog("DK: " fmt, ##args);                                                 \
  }
#define DKS "%s-0x%qx"
#define DKN(s)                                                                 \
  s ? s->getName() : "NO-NAME", s ? s->getRegistryEntryID() : UINT64_MAX
...
enum { kIOKitDiagnosticsClientType = 0x99000002 };
...
void IOTrackingAdd(IOTrackingQueue *head, IOTracking *mem, size_t size,
                   bool address, vm_tag_t tag);
void IOTrackingRemove(IOTrackingQueue *head, IOTracking *mem, size_t size);
void IOTrackingRemoveAddress(IOTrackingQueue *head, IOTrackingAddress *mem,
                             size_t size);
...
kern_return_t IOTrackingDebug(uint32_t selector, uint32_t options,
                              const char *names, size_t namesLen, size_t size,
                              OSObject **result);
