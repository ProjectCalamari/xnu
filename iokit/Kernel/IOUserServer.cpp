...
static IORPCMessage *IORPCMessageFromMachReply(IORPCMessageMach *msg);
...
OSDefineMetaClassAndStructors(_IOUserServerCheckInCancellationHandler,
                              OSObject);

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * * * * */

bool IOUserService::start(IOService *provider) {
  bool ok = true;
  IOReturn ret;
...
kern_return_t IOService::GetRegistryEntryID_Impl(uint64_t *registryEntryID) {
...
kern_return_t IOService::SetName_Impl(const char *name) {
  IOReturn ret = kIOReturnSuccess;
...
kern_return_t IOService::CopyName_Impl(OSString **name) {
  const OSString *str = copyName();
  *name = __DECONST(OSString *, str);
  return str ? kIOReturnSuccess : kIOReturnError;
}

kern_return_t IOService::Start_Impl(IOService *provider) {
  IOReturn ret = kIOReturnSuccess;
  return ret;
}

IOReturn IOService::UpdateReport_Impl(OSData *channels, uint32_t action,
...
IOReturn IOService::ConfigureReport_Impl(OSData *channels, uint32_t action,
                                         uint32_t *outCount) {
  return kIOReturnUnsupported;
}

// adapt old signature of configureReport to the iig-friendly signature of
// ConfigureReport
IOReturn IOService::_ConfigureReport(IOReportChannelList *channelList,
                                     IOReportConfigureAction action,
                                     void *result, void *destination) {
  if (action != kIOReportEnable && action != kIOReportGetDimensions &&
      action != kIOReportDisable) {
    return kIOReturnUnsupported;
  }
  ...
  bool overflow =
      os_mul_and_add_overflow(channelList->nchannels, sizeof(IOReportChannel),
                              sizeof(IOReportChannelList), &size_of_channels);
  if (overflow) {
    return kIOReturnOverrun;
  }
  OSSharedPtr<OSData> sp_channels(
      OSData::withBytesNoCopy(channelList, size_of_channels),
      libkern::no_retain);
  if (!sp_channels) {
...
// adapt old signature of updateReport to the iig-friendly signature of
// UpdateReport
IOReturn IOService::_UpdateReport(IOReportChannelList *channelList,
  ...
  IOReturn r = UpdateReport(sp_channels.get(), action, &count, length,
                            buffer->getCapacity() - length, buffer);
  int new_result;
  overflow = os_add_overflow(*resultp, count, &new_result);
  size_t new_length;
  overflow = overflow || os_mul_and_add_overflow(count, sizeof(IOReportElement),
                                                 length, &new_length);
...
IOReturn IOService::SetLegend_Impl(OSArray *legend, bool is_public) {
...
kern_return_t IOService::RegisterService_Impl() {
  IOReturn ret = kIOReturnSuccess;
  bool started;
...
kern_return_t IOService::CopyDispatchQueue_Impl(const char *name,
                                                IODispatchQueue **queue) {
  ...
  if (!strcmp("Default", name)) {
    ...
    index = reserved->uvars->userServer->stringArrayIndex(
        reserved->uvars->userMeta->queueNames, name);
    if (index != -1U) {
      index++;
    }
  }
...
kern_return_t
IOService::CreateDefaultDispatchQueue_Impl(IODispatchQueue **queue) {
  return kIOReturnError;
}

kern_return_t
IOService::CoreAnalyticsSendEvent_Impl(uint64_t options, OSString *eventName,
...
kern_return_t IOService::SetDispatchQueue_Impl(const char *name,
                                               IODispatchQueue *queue) {
  IOReturn ret = kIOReturnSuccess;
  uint32_t index;
...
IOService *IOService::GetProvider() const { return getProvider(); }

kern_return_t IOService::SetProperties_Impl(OSDictionary *properties) {
  IOUserServer *us;
  OSDictionary *dict;
  IOReturn ret;
  ...
  if (kIOReturnUnsupported == ret) {
    if (dict && reserved->uvars && (reserved->uvars->userServer == us)) {
      ret = runPropertyActionBlock(^IOReturn(void) {
        ...
        userProps = OSDynamicCast(OSDictionary,
...
kern_return_t IOService::RemoveProperty_Impl(OSString *propertyName) {
  ...
  if (reserved && reserved->uvars && reserved->uvars->userServer == us) {
    ret = runPropertyActionBlock(^IOReturn(void) {
      OSDictionary *userProps;
      userProps =
          OSDynamicCast(OSDictionary, getProperty(gIOUserServicePropertiesKey));
      if (userProps) {
...
kern_return_t IOService::CopyProperties_Local(OSDictionary **properties) {
  OSDictionary *props;
  OSDictionary *userProps;

  props = dictionaryWithProperties();
  userProps = OSDynamicCast(OSDictionary,
                            props->getObject(gIOUserServicePropertiesKey));
  if (userProps) {
...
kern_return_t IOService::CopyProperties_Impl(OSDictionary **properties) {
  return CopyProperties_Local(properties);
}

kern_return_t IOService::RequireMaxBusStall_Impl(uint64_t u64ns) {
  IOReturn ret;
  UInt32 ns;
...
kern_return_t IOService::UserSetProperties_Impl(OSContainer *properties) {
  return kIOReturnUnsupported;
}

kern_return_t IOService::SendIOMessageServicePropertyChange_Impl(void) {
...
kern_return_t IOMemoryDescriptor::_CopyState_Impl(_IOMDPrivateState *state) {
  IOReturn ret;
...
kern_return_t IOMemoryDescriptor::GetLength(uint64_t *returnLength) {
...
kern_return_t
IOMemoryDescriptor::CreateMapping_Impl(uint64_t options, uint64_t address,
                                       uint64_t offset, uint64_t length,
                                       uint64_t alignment, IOMemoryMap **map) {
  IOReturn ret;
  ...
  if (kIOReturnSuccess == ret) {
    resultMap = createMappingInTask(current_task(), atAddress, koptions, offset,
                                    length);
    if (!resultMap) {
      ret = kIOReturnError;
    }
...
kern_return_t IOMemoryDescriptor::CreateSubMemoryDescriptor_Impl(
    uint64_t memoryDescriptorCreateOptions, uint64_t offset, uint64_t length,
    IOMemoryDescriptor *ofDescriptor, IOMemoryDescriptor **memory) {
  IOReturn ret;
  ...
  iomd = IOSubMemoryDescriptor::withSubRange(
      ofDescriptor, mdOffset, mdLength,
      (IOOptionBits)memoryDescriptorCreateOptions);
...
kern_return_t IOMemoryDescriptor::CreateWithMemoryDescriptors_Impl(
    uint64_t memoryDescriptorCreateOptions, uint32_t withDescriptorsCount,
    IOMemoryDescriptor **const withDescriptors, IOMemoryDescriptor **memory) {
  IOReturn ret;
  IOMemoryDescriptor *iomd;
  ...
  iomd = IOMultiMemoryDescriptor::withDescriptors(
      withDescriptors, withDescriptorsCount,
...
kern_return_t IOUserClient::CreateMemoryDescriptorFromClient_Impl(
    uint64_t memoryDescriptorCreateOptions, uint32_t segmentsCount,
    const IOAddressSegment segments[32], IOMemoryDescriptor **memory) {
  IOReturn ret;
  IOMemoryDescriptor *iomd;
  ...
  iomd = IOMemoryDescriptor::withAddressRanges(ranges, segmentsCount, mdOptions,
                                               me->fTask);
...
kern_return_t IOMemoryMap::_CopyState_Impl(_IOMemoryMapPrivateState *state) {
  IOReturn ret;
...
kern_return_t
IOBufferMemoryDescriptor::Create_Impl(uint64_t options, uint64_t capacity,
                                      uint64_t alignment,
  ...
  bmdOptions = (options & kIOMemoryDirectionOutIn) | kIOMemoryKernelUserShared |
               kIOMemoryThreadSafe;
  bmd = IOBufferMemoryDescriptor::inTaskWithOptions(kernel_task, bmdOptions,
                                                    capacity, alignment);
...
kern_return_t IOBufferMemoryDescriptor::SetLength_Impl(uint64_t length) {
  setLength(length);
  return kIOReturnSuccess;
}
...
kern_return_t
IODMACommand::Create_Impl(IOService *device, uint64_t options,
  ...
  if (os_convert_overflow(specification->maxAddressBits,
  ...
  dma = IODMACommand::withSpecification(
      kIODMACommandOutputHost64, &segmentOptions,
      kIODMAMapOptionDextOwner | kIODMAMapOptionMapped, mapper, NULL);
...
kern_return_t IODMACommand::PrepareForDMA_Impl(uint64_t options,
...
kern_return_t IODMACommand::CompleteDMA_Impl(uint64_t options) {
...
kern_return_t IODMACommand::GetPreparation_Impl(uint64_t *offset,
                                                uint64_t *length,
                                                IOMemoryDescriptor **memory) {
  IOReturn ret;
  IOMemoryDescriptor *md;
...
kern_return_t IODMACommand::PerformOperation_Impl(uint64_t options,
                                                  uint64_t dmaOffset,
                                                  uint64_t length,
                                                  uint64_t dataOffset,
  ...
  if (options & ~((uint64_t)(kIODMACommandPerformOperationOptionRead |
...
OSActionCreateWithTypeNameInternal(OSObject *target, uint64_t targetmsgid,
                                   uint64_t msgid, size_t referenceSize,
                                   OSString *typeName, bool fromKernel,
                                   OSAction **action) {
  OSAction *inst = NULL;
  ...
  if (fromKernel && typeName) {
    ...
    if (actionMetaClass &&
        actionMetaClass->getSuperClass() == OSTypeID(OSAction)) {
      ...
      assert(inst); // obj is a subclass of OSAction so the dynamic cast should
                    // always work
    } else {
      DKLOG("Attempted to create action object with type \"%s\" which does not "
            "inherit from OSAction\n",
...
kern_return_t OSAction::Create(OSAction_Create_Args) {
  return OSAction::CreateWithTypeName(target, targetmsgid, msgid, referenceSize,
                                      NULL, action);
}

kern_return_t OSAction::CreateWithTypeName(OSAction_CreateWithTypeName_Args) {
  return OSActionCreateWithTypeNameInternal(
      target, targetmsgid, msgid, referenceSize, typeName, true, action);
}

kern_return_t OSAction::Create_Impl(OSObject *target, uint64_t targetmsgid,
                                    uint64_t msgid, size_t referenceSize,
                                    OSAction **action) {
  return OSAction::CreateWithTypeName_Impl(target, targetmsgid, msgid,
                                           referenceSize, NULL, action);
}

kern_return_t
OSAction::CreateWithTypeName_Impl(OSObject *target, uint64_t targetmsgid,
                                  uint64_t msgid, size_t referenceSize,
                                  OSString *typeName, OSAction **action) {
  return OSActionCreateWithTypeNameInternal(
      target, targetmsgid, msgid, referenceSize, typeName, false, action);
}
...
kern_return_t OSAction::SetAbortedHandler(OSActionAbortedHandler handler) {
  ivars->abortedHandler = Block_copy(handler);
  return kIOReturnSuccess;
}

void OSAction::Aborted_Impl(void) {
...
void IODispatchSource::free() {
  IOFreeType(ivars, IODispatchSource_IVars);
  super::free();
}

kern_return_t IODispatchSource::SetEnable_Impl(bool enable) {
...
void IOInterruptDispatchSourceInterrupt(OSObject *target, void *refCon,
...
kern_return_t IOInterruptDispatchSource::Create_Impl(
    IOService *provider, uint32_t indexAndFlags, IODispatchQueue *queue,
  ...
  ret = provider->registerInterrupt(
      index, inst, IOInterruptDispatchSourceInterrupt, inst->ivars);
  if (kIOReturnSuccess == ret) {
...
kern_return_t IOInterruptDispatchSource::GetInterruptType_Impl(
    IOService *provider, uint32_t index, uint64_t *interruptType) {
  IOReturn ret;
  int type;
...
bool IOInterruptDispatchSource::init() {
  if (!super::init()) {
    return false;
  }
...
void IOInterruptDispatchSource::free() {
...
kern_return_t IOInterruptDispatchSource::SetHandler_Impl(OSAction *action) {
  IOReturn ret;
  OSAction *oldAction;
...
kern_return_t IOInterruptDispatchSource::SetEnableWithCompletion_Impl(
    bool enable, IODispatchSourceCancelHandler handler) {
  IOReturn ret;
  IOInterruptState is;
...
kern_return_t
IOInterruptDispatchSource::Cancel_Impl(IODispatchSourceCancelHandler handler) {
  IOInterruptState is;
  IOService *provider;
...
kern_return_t IOInterruptDispatchSource::CheckForWork_Impl(const IORPC rpc,
                                                           bool synchronous) {
  ...
  do {
    ...
    if (!canceled) {
      ...
      if (willWait && (kIOInterruptTypeLevel & ivars->interruptType) &&
          ivars->enable) {
        IOSimpleLockUnlockEnableInterrupt(ivars->lock, is);
...
void IOInterruptDispatchSource::InterruptOccurred_Impl(OSAction *action,
                                                       uint64_t count,
                                                       uint64_t time) {}

kern_return_t
IOInterruptDispatchSource::GetLastInterrupt_Impl(uint64_t *pCount,
                                                 uint64_t *pTime) {
  IOInterruptState is;
  uint64_t count, time;
...
kern_return_t IOServiceNotificationDispatchSource::Create_Impl(
    OSDictionary *matching, uint64_t options, IODispatchQueue *queue,
  ...
  inst->ivars->notifier = IOService::addMatchingNotification(
      gIOMatchedNotification, matching, 0 /*priority*/,
      ^bool(IOService *newService, IONotifier *notifier) {
        ...
        okToUse =
            (serverName && inst->ivars->serverName->isEqualTo(serverName));
        OSSafeReleaseNULL(serverName);
        if (!okToUse) {
          OSObject *prop;
          OSObject *str;

          if (!newService->reserved->uvars ||
              !newService->reserved->uvars->userServer) {
            return false;
          }
          ...
          okToUse = newService->reserved->uvars->userServer->checkEntitlements(
              str, NULL, NULL);
          if (!okToUse) {
            if (kIODKLogSetup & gIODKDebug) {
        ...
        notifyReady =
            (0 == inst->ivars->pending[kIOServiceNotificationTypeMatched]
                      ->getCount());
        inst->ivars->pending[kIOServiceNotificationTypeMatched]->setObject(
            newService);
        bool needInterest = (NULL == inst->ivars->interestNotifiers->getObject(
                                         (const OSSymbol *)newService));
        IOLockUnlock(inst->ivars->lock);

        if (needInterest) {
          interest = newService->registerInterest(
              gIOGeneralInterest,
              ^IOReturn(uint32_t messageType, IOService *provider,
                ...
                // after the notifier remove,
                // IOServiceNotificationDispatchSource::free will not wait for
                ...
                case kIOMessageServiceIsTerminated:
                  ...
                  notifyReady =
                      (0 == inst->ivars
                                ->pending[kIOServiceNotificationTypeTerminated]
                                ->getCount());
                  inst->ivars->pending[kIOServiceNotificationTypeTerminated]
                      ->setObject(provider);
                  if (inst->ivars->interestNotifiers != NULL) {
                    interest = (typeof(interest))inst->ivars->interestNotifiers
                                   ->getObject((const OSSymbol *)newService);
                    assert(interest);
                    interest->remove();
                    inst->ivars->interestNotifiers->removeObject(
                        (const OSSymbol *)newService);
                  }
                  IOLockUnlock(inst->ivars->lock);
                  break;
        ...
        if (interest) {
          IOLockLock(inst->ivars->lock);
          inst->ivars->interestNotifiers->setObject(
              (const OSSymbol *)newService, interest);
          IOLockUnlock(inst->ivars->lock);
        }
        if (notifyReady) {
...
kern_return_t IOServiceNotificationDispatchSource::CopyNextNotification_Impl(
...
bool IOServiceNotificationDispatchSource::init() {
  if (!super::init()) {
    return false;
  }
...
void IOServiceNotificationDispatchSource::free() {
  if (ivars) {
    if (ivars->notifier) {
    ...
    if (ivars->interestNotifiers) {
      ...
      savedInterestNotifiers->iterateObjects(
          ^bool(const OSSymbol *key, OSObject *object) {
...
kern_return_t
IOServiceNotificationDispatchSource::SetHandler_Impl(OSAction *action) {
  IOReturn ret;
  bool notifyReady;
...
kern_return_t IOServiceNotificationDispatchSource::SetEnableWithCompletion_Impl(
    bool enable, IODispatchSourceCancelHandler handler) {
  if (enable == ivars->enable) {
    return kIOReturnSuccess;
  }
...
kern_return_t IOServiceNotificationDispatchSource::Cancel_Impl(
    IODispatchSourceCancelHandler handler) {
  return kIOReturnUnsupported;
}

kern_return_t
IOServiceNotificationDispatchSource::CheckForWork_Impl(const IORPC rpc,
                                                       bool synchronous) {
  return kIOReturnNotReady;
}

kern_return_t IOServiceNotificationDispatchSource::DeliverNotifications(
...
OSDictionary *
IOService::CreatePropertyMatchingDictionary(const char *key, OSObjectPtr value,
...
OSDictionary *IOService::CreatePropertyMatchingDictionary(
    const char *key, const char *stringValue, OSDictionary *matching) {
  OSDictionary *result;
  OSString *value;
...
OSDictionary *
IOService::CreateKernelClassMatchingDictionary(OSString *className,
                                               OSDictionary *matching) {
  if (!className) {
    return NULL;
  }
...
OSDictionary *
IOService::CreateKernelClassMatchingDictionary(const char *className,
                                               OSDictionary *matching) {
  OSDictionary *result;
  OSString *string;
...
OSDictionary *
IOService::CreateUserClassMatchingDictionary(OSString *className,
                                             OSDictionary *matching) {
  return CreatePropertyMatchingDictionary(kIOUserClassKey, className, matching);
}

OSDictionary *
IOService::CreateUserClassMatchingDictionary(const char *className,
                                             OSDictionary *matching) {
  return CreatePropertyMatchingDictionary(kIOUserClassKey, className, matching);
}

OSDictionary *IOService::CreateNameMatchingDictionary(OSString *serviceName,
                                                      OSDictionary *matching) {
  if (!serviceName) {
    return NULL;
  }
...
OSDictionary *IOService::CreateNameMatchingDictionary(const char *serviceName,
                                                      OSDictionary *matching) {
  OSDictionary *result;
  OSString *string;
...
kern_return_t IOUserServer::waitInterruptTrap(void *p1, void *p2, void *p3,
  ...
  object = iokit_lookup_object_with_port_name((mach_port_name_t)(uintptr_t)p1,
                                              IKOT_UEXT_OBJECT, current_task());
...
kern_return_t IOUserServer::Create_Impl(const char *name, uint64_t tag,
                                        uint64_t options, OSString *bundleID,
                                        IOUserServer **server) {
  IOReturn ret;
  IOUserServer *us;
  const OSSymbol *sym;
  ...
  if (bundleID) {
    ...
    if (kext) {
      ...
      DKLOG(DKS "::Create(" DKS "): could not find OSKext for %s\n", DKN(us),
            name, tag, bundleID->getCStringNoCopy());
    }

    us->fAllocationName =
        kern_allocation_name_allocate(bundleID->getCStringNoCopy(), 0);
...
kern_return_t IOUserServer::RegisterService_Impl() {
  kern_return_t ret = IOService::RegisterService_Impl();

  return ret;
}

kern_return_t IOUserServer::Exit_Impl(const char *reason) {
  return kIOReturnUnsupported;
}

kern_return_t IOUserServer::LoadModule_Impl(const char *path) {
  return kIOReturnUnsupported;
}
...
kern_return_t IODispatchQueue::Create_Impl(const char *name, uint64_t options,
                                           uint64_t priority,
                                           IODispatchQueue **queue) {
  IODispatchQueue *result;
  IOUserServer *us;
...
kern_return_t IODispatchQueue::SetPort_Impl(mach_port_t port) {
...
bool IODispatchQueue::OnQueue() { return false; }

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * * * * */

kern_return_t OSMetaClassBase::Dispatch(IORPC rpc) {
  return kIOReturnUnsupported;
}

kern_return_t OSMetaClassBase::Invoke(IORPC rpc) {
  IOReturn ret = kIOReturnUnsupported;
...
kern_return_t OSUserMetaClass::Dispatch(IORPC rpc) {
  if (meta) {
...
void OSUserMetaClass::free() {
  if (queueNames) {
    IOFreeData(queueNames,
               sizeof(IOPStrings) + queueNames->dataSize * sizeof(char));
    queueNames = NULL;
  }
  if (description) {
 ...
 */
void IOUserServer::setTaskLoadTag(OSKext *kext) {
  task_t owningTask;
 ...
 * dext executable.
 */
void IOUserServer::setDriverKitUUID(OSKext *kext) {
  task_t task;
  proc_t p;
...
void IOUserServer::setDriverKitStatistics(OSKext *kext) {
...
IOReturn IOUserServer::setCheckInToken(IOUserServerCheckInToken *token) {
...
bool IOUserServer::serviceMatchesCheckInToken(IOUserServerCheckInToken *token) {
  if (token != NULL) {
...
//                                   - if any top level array succeeds return
//                                   true
// consumes one reference of prop
bool IOUserServer::checkEntitlements(OSDictionary *entitlements, OSObject *prop,
...
bool IOUserServer::checkEntitlements(OSObject *prop, IOService *provider,
                                     IOService *dext) {
  return checkEntitlements(fEntitlements, prop, provider, dext);
}

bool IOUserServer::checkEntitlements(IOService *provider, IOService *dext) {
  OSObject *prop;
  bool ok;
...
IOReturn IOUserServer::exit(const char *reason) {
  DKLOG(DKS "::exit(%s)\n", DKN(this), reason);
  Exit(reason);
  return kIOReturnSuccess;
}

IOReturn IOUserServer::kill(const char *reason) {
  IOReturn ret = kIOReturnError;
  if (fOwningTask != NULL) {
    DKLOG(DKS "::kill(%s)\n", DKN(this), reason);
...
OSObjectUserVars *IOUserServer::varsForObject(OSObject *obj) {
  IOService *service;
...
IOPStrings *IOUserServer::copyInStringArray(const char *string,
                                            uint32_t userSize) {
  IOPStrings *array;
  vm_size_t alloc;
  size_t len;
...
uint32_t IOUserServer::stringArrayIndex(IOPStrings *array, const char *look) {
  uint32_t idx;
  size_t len, llen;
  OSBoundedPtr<const char> cstr;
  const char *end;
...
IODispatchQueue *IOUserServer::queueForObject(OSObject *obj, uint64_t msgid) {
  IODispatchQueue *queue;
  ...
  if (uvars->userMeta && uvars->userMeta->methods) {
...
IOReturn IOUserServer::objectInstantiate(OSObject *obj, IORPC rpc,
                                         IORPCMessage *message) {
  IOReturn ret;
  OSString *str;
  OSObject *prop;
  ...
  if (!service) {
    ...
    if (service->lockForArbitration()) {
      if (service->isInactive() &&
          (service->__state[1] & kIOServiceStartState) == 0) {
    ...
    if (service->reserved->uvars && service->reserved->uvars->userServer) {
      ...
      userMeta =
          (typeof(userMeta))
              service->reserved->uvars->userServer->fClasses->getObject(str);
    ...
    if (userMeta) {
      if (kOSObjectRPCRemote & resultFlags) {
        if (!action) {
          ...
          while (userMeta &&
                 !(kOSClassCanRemote & userMeta->description->flags)) {
        ...
        service->reserved->uvars->queueArray =
            OSBoundedArrayRef<IODispatchQueue *>(unboundedQueueArray,
                                                 queueAlloc);
  ...
  if (methodCount) {
    bcopy(methods, &reply->methods[0],
          methodCount * 2 * sizeof(reply->methods[0]));
  }
...
IOReturn IOUserServer::kernelDispatch(OSObject *obj, IORPC rpc) {
  IOReturn ret;
  IORPCMessage *message;
...
OSObject *IOUserServer::target(OSAction *action, IORPCMessage *message) {
  OSObject *object;
  ...
  if (kIORPCMessageRemote & message->flags) {
    ...
    // non-standard contract of this method, which is that it releases 'action'
    // when the message flags have kIORPCMessageRemote set.
    action->release();
#endif
  }
  if (kIODKLogIPC & gIODKDebug) {
...
kern_return_t uext_server(ipc_port_t receiver, ipc_kmsg_t requestkmsg,
                          ipc_kmsg_t *pReply) {
  kern_return_t ret;
  OSObject *object;
  IOUserServer *server;
...
kern_return_t IOUserServer::server(ipc_kmsg_t requestkmsg,
                                   IORPCMessage *message, ipc_kmsg_t *pReply) {
  kern_return_t ret;
  ...
  if (msgin->msgh.msgh_size <
      (sizeof(IORPCMessageMach) + sizeof(IORPCMessage))) {
    if (kIODKLogIPC & gIODKDebug) {
      DKLOG("UEXT notify %o\n", msgin->msgh.msgh_id);
    }
  ...
  if (replyAlloc) {
     ...
     * IPC_KMSG_ALLOC_LINEAR | IPC_KMSG_ALLOC_NOFAIL);
     */
    replykmsg = ipc_kmsg_alloc_uext_reply(MAX_UEXT_REPLY_SIZE);
    msgout = (typeof(msgout))ikm_header(replykmsg);
  }

  IORPC rpc = {.message = msgin,
               .reply = msgout,
               .sendSize = msgin->msgh.msgh_size,
               .replySize = replyAlloc,
               .kernelContent = message};
  ...
  if (!oneway) {
    if (kIOReturnSuccess == ret) {
      ...
      if (!reply) {
        ...
        ret = copyOutObjects(msgout, reply, replySize,
                             (kIORPCVersionCurrentReply ==
                              msgout->msgh.msgh_id) /* =>!InvokeReply */);
      }
    }
    if (kIOReturnSuccess != ret) {
    ...
    msgout->msgh.msgh_bits =
        MACH_MSGH_BITS_COMPLEX |
        MACH_MSGH_BITS_SET(
            MACH_MSGH_BITS_LOCAL(msgin->msgh.msgh_bits) /*remote*/, 0 /*local*/,
            0, 0);
...
static inline uint32_t MAX_OBJECT_COUNT(IORPCMessageMach *mach, size_t size,
                                        IORPCMessage *message __unused) {
  assert(mach->msgh.msgh_size == size);
  size_t used_size;
  size_t remaining_size;
  if (os_mul_and_add_overflow(mach->msgh_body.msgh_descriptor_count,
                              sizeof(mach_msg_port_descriptor_t),
                              sizeof(mach->msgh) + sizeof(mach->msgh_body) +
                                  offsetof(IORPCMessage, objects[0]),
                              &used_size)) {
    return 0;
  }
...
kern_return_t IOUserServerUEXTTrap(OSObject *object, void *p1, void *p2,
...
#pragma pack()
  ...
  mach->msgh.msgh_size =
      (mach_msg_size_t)(sizeof(IORPCMessageMach) + refs * sizeof(*descs) +
                        inSize); // totalSize was checked
  ...
  rpc.replySize =
      ((uint32_t)(sizeof(buffer.buffer) - inSize)); // inSize was checked
  ...
  if ((action = OSDynamicCast(OSAction, object))) {
    ...
    if (kIOReturnSuccess == ret) {
      ...
      copySize = rpc.reply->msgh.msgh_size -
                 (((uintptr_t)reply) - ((uintptr_t)rpc.reply)) +
                 sizeof(uint64_t);
      if (copySize > outSize) {
        return kIOReturnIPCError;
      }
...
IOReturn IOUserServer::rpc(IORPC rpc) {
  ...
  if (oneway) {
    ret = kernel_mach_msg_send(&mach->msgh, sendSize, MACH_SEND_KERNEL_DEFAULT,
                               0, &message_moved);
  } else {
    assert(replySize >= (sizeof(IORPCMessageMach) + sizeof(IORPCMessage)));
    ret = kernel_mach_msg_rpc(&mach->msgh, sendSize, replySize, FALSE,
                              &message_moved);
  }
  ...
  if ((KERN_SUCCESS == ret) && !oneway) {
    if (kIORPCVersionCurrentReply != mach->msgh.msgh_id) {
      ret = (MACH_NOTIFY_SEND_ONCE == mach->msgh.msgh_id) ? MIG_SERVER_DIED
                                                          : MIG_REPLY_MISMATCH;
    } else if ((replySize = mach->msgh.msgh_size) <
               (sizeof(IORPCMessageMach) + sizeof(IORPCMessage))) {
...
IORPCMessage *IORPCMessageFromMach(IORPCMessageMach *msg, bool reply) {
  ...
  upgrade = (msg->msgh.msgh_id !=
             (reply ? kIORPCVersionCurrentReply : kIORPCVersionCurrent));
...
ipc_port_t IOUserServer::copySendRightForObject(OSObject *object,
                                                ipc_kobject_type_t type) {
  return iokit_port_make_send_for_object(object, type);
}

OSObject *IOUserServer::copyObjectForSendRight(ipc_port_t port,
                                               ipc_kobject_type_t type) {
...
static kern_return_t copyoutkdata(const void *data, vm_size_t len, void **buf) {
  kern_return_t err;
  vm_map_copy_t copy;
...
IOReturn IOUserServer::copyOutObjects(IORPCMessageMach *mach,
                                      IORPCMessage *message, size_t size,
                                      bool consume) {
  uint64_t refs;
      ...
      if (object) {
        ...
        if (kIODKLogIPC & gIODKDebug) {
          ...
          if (iomd != NULL && (iomd->getFlags() & kIOMemoryThreadSafe) == 0) {
            OSReportWithBacktrace("IOMemoryDescriptor %p was created without "
                                  "kIOMemoryThreadSafe flag",
                                  iomd);
          }
        }
...
IOReturn IOUserServer::copyInObjects(IORPCMessageMach *mach,
                                     IORPCMessage *message, size_t size,
                                     bool copyObjects, bool consumePorts) {
  uint64_t refs;
  uint32_t idx, maxObjectCount;
  ipc_port_t port;
  OSObject *object;
  ...
  for (idx = 0; idx < mach->msgh_body.msgh_descriptor_count; idx++) {
    ...
    case MACH_MSG_OOL_DESCRIPTOR:
      ...
      if (isObjectPort && copyObjects && ool->size && ool->address) {
        kr = vm_map_copyout(kernel_map, &copyoutdata,
                            (vm_map_copy_t)ool->address);
        if (KERN_SUCCESS == kr) {
...
IOReturn IOUserServer::consumeObjects(IORPCMessageMach *mach,
                                      IORPCMessage *message,
                                      size_t messageSize) {
  uint64_t refs, idx;
  OSObject *object;
...
static kern_return_t acknowledgeSetPowerState(IOService *service);

bool IOUserServer::finalize(IOOptionBits options) {
  OSArray *services;
  ...
  IOOptionBits terminateFlags =
      kIOServiceTerminateNeedWillTerminate | kIOServiceTerminateWithRematch;
  if (fCheckInToken) {
    ...
    if (can_rematch) {
      ...
      DKLOG(DKS
            "::finalize(%p) dext was replaced, do not rematch current dext\n",
            DKN(this), this);
    }
  } else {
    terminateFlags |= kIOServiceTerminateWithRematchCurrentDext;
  ...
  if (services) {
    services->iterateObjects(^bool(OSObject *obj) {
      ...
      if (nextService->reserved->uvars) {
        ...
        if (provider != NULL &&
            (terminateFlags & kIOServiceTerminateWithRematchCurrentDext) == 0) {
      ...
      if (!started || !instantiated) {
        DKLOG(DKS "::terminate(" DKS
                  ") server exit before start() instantiated %d\n",
...
OSDefineMetaClassAndStructors(IOUserServer, IOUserClient2022)
  ...
  if (!(kIODKDisableEntitlementChecking & gIODKDebug)) {
    ...
    if (!IOCurrentTaskHasEntitlement(
            gIODriverKitEntitlementKey->getCStringNoCopy())) {
  ...
  if (csproc_get_validation_category(
          current_proc(), &inst->fCSValidationCategory) != KERN_SUCCESS) {
  ...
  inst->setProperty(kIOUserClientDefaultLockingSetPropertiesKey,
                    kOSBooleanTrue);
  inst->setProperty(kIOUserClientDefaultLockingSingleThreadExternalMethodKey,
                    kOSBooleanTrue);
  // requirement for gIODriverKitEntitlementKey is enforced elsewhere
  // conditionally
...
bool IOUserServer::shouldLeakObjects() { return gIOUserServerLeakObjects; }

void IOUserServer::beginLeakingObjects() { gIOUserServerLeakObjects = true; }

bool IOUserServer::isPlatformDriver() { return fPlatformDriver; }

int IOUserServer::getCSValidationCategory() { return fCSValidationCategory; }
...
static bool IOUserServerRecordExitReasonMatch(const OSObject *obj,
                                              void *context) {
  IOUserServerRecordExitReasonContext *ctx =
      (IOUserServerRecordExitReasonContext *)context;
...
extern "C" void IOUserServerRecordExitReason(task_t task, os_reason_t reason) {
  IOUserServerRecordExitReasonContext ctx{task, reason};
  IOUserServer::gMetaClass.applyToInstances(IOUserServerRecordExitReasonMatch,
                                            &ctx);
}

IOReturn IOUserServer::clientClose(void) {
  OSArray *services;
  ...
  // if this was a an expected exit, termination and stop should have detached
  // at this point, so send any provider still attached and not owned by this
  // user server the ClientCrashed() notification
  if (services) {
    services->iterateObjects(^bool(OSObject *obj) {
      int service __unused; // hide outer defn
      IOService *nextService;
      ...
      if (nextService->reserved && nextService->reserved->uvars &&
          nextService->reserved->uvars->started) {
        unexpectedExit = true;
      }
      provider = nextService->getProvider();
      if (provider && (!provider->reserved->uvars ||
                       (provider->reserved->uvars->userServer != this))) {
        if (kIODKLogSetup & gIODKDebug) {
  ...
  if (unexpectedExit && !gInUserspaceReboot &&
      (fTaskCrashReason != OS_REASON_NULL &&
       fTaskCrashReason->osr_namespace != OS_REASON_JETSAM &&
       fTaskCrashReason->osr_namespace != OS_REASON_RUNNINGBOARD) &&
      fStatistics != NULL) {
    OSDextCrashPolicy policy = fStatistics->recordCrash();
    bool allowPanic;
#if DEVELOPMENT || DEBUG
    allowPanic = !restore_boot && fPlatformDriver &&
                 fEntitlements->getObject(
                     gIODriverKitTestDriverEntitlementKey) != kOSBooleanTrue &&
                 !disable_dext_crash_reboot;
#else
    allowPanic = !restore_boot && fPlatformDriver;
#endif /* DEVELOPMENT || DEBUG */
...
IOReturn IOUserServer::setProperties(OSObject *properties) {
  IOReturn kr = kIOReturnUnsupported;
  return kr;
}

void IOUserServer::stop(IOService *provider) {
  if (fOwningTask) {
...
IOWorkLoop *IOUserServer::getWorkLoop() const { return fWorkLoop; }

void IOUserServer::free() {
...
IOReturn IOUserServer::registerClass(OSClassDescription *desc, uint32_t size,
                                     OSUserMetaClass **pCls) {
  OSUserMetaClass *cls;
  const OSSymbol *sym;
  ...
  if (kIODKLogSetup & gIODKDebug) {
    DKLOG(DKS "::registerClass %s, %d, %d\n", DKN(this), desc->name,
          desc->queueNamesSize, desc->methodNamesSize);
  }
  ...
  if (os_add_overflow(desc->queueNamesOffset, desc->queueNamesSize,
                      &queueNamesEnd)) {
    assert(false);
    return kIOReturnBadArgument;
  }
  ...
  if (os_add_overflow(desc->methodOptionsOffset, desc->methodOptionsSize,
                      &methodOptionsEnd)) {
    assert(false);
    return kIOReturnBadArgument;
  }
  ...
  // overlaps?
  if ((desc->queueNamesOffset >= desc->methodOptionsOffset) &&
      (desc->queueNamesOffset < methodOptionsEnd)) {
    assert(false);
    return kIOReturnBadArgument;
  }
  if ((queueNamesEnd >= desc->methodOptionsOffset) &&
      (queueNamesEnd < methodOptionsEnd)) {
    assert(false);
    return kIOReturnBadArgument;
  }
  ...
  if (sizeof(desc->superName) ==
      strnlen(desc->superName, sizeof(desc->superName))) {
    assert(false);
    return kIOReturnBadArgument;
  }
  ...
  methodOptions =
      (typeof(methodOptions))(((uintptr_t)desc) + desc->methodOptionsOffset);
  ...
  IOLockLock(fLock);
  cls->superMeta =
      OSDynamicCast(OSUserMetaClass, fClasses->getObject(desc->superName));
...
IOReturn IOUserServer::registerClass(OSClassDescription *desc, uint32_t size,
                                     OSSharedPtr<OSUserMetaClass> &pCls) {
...
IOReturn IOUserServer::setRootQueue(IODispatchQueue *queue) {
  assert(!fRootQueue);
  if (fRootQueue) {
...
IOReturn IOUserServer::externalMethod(uint32_t selector,
  ...
  return dispatchExternalMethod(
      selector, args, dispatchArray,
      sizeof(dispatchArray) / sizeof(dispatchArray[0]), this, NULL);
}

IOReturn
IOUserServer::externalMethodRegisterClass(OSObject *target, void *reference,
  ...
  ret = me->registerClass((OSClassDescription *)args->structureInput,
                          args->structureInputSize, &cls);
  if (kIOReturnSuccess == ret) {
...
IOReturn IOUserServer::externalMethodStart(OSObject *target, void *reference,
                                           IOExternalMethodArguments *args) {
  ...
  if (!(kIODKDisableCheckInTokenVerification & gIODKDebug)) {
    mach_port_name_t checkInPortName =
        ((typeof(checkInPortName))args->scalarInput[0]);
    OSObject *obj = iokit_lookup_object_with_port_name(
        checkInPortName, IKOT_IOKIT_IDENT, me->fOwningTask);
    IOUserServerCheckInToken *retrievedToken =
        OSDynamicCast(IOUserServerCheckInToken, obj);
...
IOExternalTrap *IOUserServer::getTargetAndTrapForIndex(IOService **targetP,
                                                       UInt32 index) {
...
void IOUserServer::pageout() { fPageout = 1; }

IOReturn IOUserServer::serviceAttach(IOService *service, IOService *provider) {
  IOReturn ret;
  ...
  if (-1U == fServices->getNextIndexOfObject(service, 0)) {
    ...
    userMeta =
        (typeof(userMeta))
            service->reserved->uvars->userServer->fClasses->getObject(str2);
    while (str2 && userMeta) {
  ...
  if (bundleID) {
    execPath[0] = 0;
    bool ok =
        OSKext::copyUserExecutablePath(bundleID, execPath, sizeof(execPath));
    if (ok) {
...
IOReturn IOUserServer::serviceNewUserClient(IOService *service,
  ...
  if (entitlements) {
    if (kIOReturnSuccess ==
        clientHasPrivilege((void *)owningTask,
                           kIOClientPrivilegeAdministrator)) {
      entitlements->setObject(kIODriverKitUserClientEntitlementAdministratorKey,
                              kOSBooleanTrue);
    }
  ...
  *handler = NULL;
  ...
  if (!(kIODKDisableEntitlementChecking & gIODKDebug)) {
    do {
      ...
      // check if client has com.apple.private.driverkit.driver-access and the
      // required entitlements match the driver's entitlements
      if (entitlements && (prop = entitlements->getObject(
                               gIODriverKitRequiredEntitlementsKey))) {
        prop->retain();
        ok = checkEntitlements(fEntitlements, prop, NULL, NULL);
        if (ok) {
          break;
      ...
      if (checkiOS3pEntitlements) {
        ...
        ok = entitlements &&
             entitlements->getObject(
                 gIODriverKitUserClientEntitlementCommunicatesWithDriversKey) ==
                 kOSBooleanTrue;
        if (ok) {
          // check team ID
          const char *clientTeamID = csproc_get_teamid(current_proc());
          bool sameTeam = fTeamIdentifier != NULL && clientTeamID != NULL &&
                          strncmp(fTeamIdentifier->getCStringNoCopy(),
                                  clientTeamID, CS_MAX_TEAMID_LEN) == 0;

          if (sameTeam) {
            ok = true;
          } else {
            // different team IDs, dext must have
          ...
          if (!ok) {
            DKLOG(DKS
                  ":UC failed team ID check. client team=%s, driver team=%s\n",
                  DKN(userUC), clientTeamID ? clientTeamID : "(null)",
                  fTeamIdentifier ? fTeamIdentifier->getCStringNoCopy()
                                  : "(null)");
          }
        } else {
          DKLOG(DKS ":UC entitlement check failed, app does not have %s "
                    "entitlement\n",
                DKN(userUC),
                gIODriverKitUserClientEntitlementCommunicatesWithDriversKey
                    ->getCStringNoCopy());
        ...
        // When checking iOS 3rd party entitlements, do not fall through to
        // other entitlement checks
        break;
      }
      ...
      if (fEntitlements && fEntitlements->getObject(
                               gIODriverKitUserClientEntitlementAllowAnyKey)) {
        ok = true;
        break;
      }

      // check if client has com.apple.developer.driverkit.userclient-access and
      // its value matches the bundle ID of the service
      bundleID = service->copyProperty(gIOModuleIdentifierKey);
      ok = (entitlements && bundleID &&
            (prop = entitlements->getObject(
      ...
      if (ok) {
        ...
        DKLOG(DKS ":UC failed userclient-access check, needed bundle ID %s\n",
              DKN(userUC),
              bundleIDStr ? bundleIDStr->getCStringNoCopy() : "(null)");
      }
...
IOReturn IOUserServer::serviceNewUserClient(
    IOService *service, task_t owningTask, void *securityID, uint32_t type,
    OSDictionary *properties, OSSharedPtr<IOUserClient> &handler) {
  IOUserClient *handlerRaw = NULL;
  IOReturn result = serviceNewUserClient(service, owningTask, securityID, type,
                                         properties, &handlerRaw);
...
enum { kUserServerMaxPowerState = 3 };

IOReturn IOUserServer::serviceJoinPMTree(IOService *service) {
  IOReturn ret;
  ...
  if (!fRootNotifier) {
    ret = registerPowerDriver(this, sPowerStates,
                              sizeof(sPowerStates) / sizeof(sPowerStates[0]));
  ...
  if (!(kIODKDisablePM & gIODKDebug) && !service->pm_vars) {
    ...
    if (kIOReturnSuccess == kr) {
      ...
      if (!(kIODKDisableIOPMSystemOffPhase2Allow & gIODKDebug)) {
        fSystemOffPhase2Allow |=
            (NULL != props->getObject(kIOPMSystemOffPhase2AllowKey));
      }
    ...
    service->PMinit();
    ret = service->registerPowerDriver(
        this, sPowerStates, sizeof(sPowerStates) / sizeof(sPowerStates[0]));
    assert(kIOReturnSuccess == ret);
    joinTree = true;
  }
  ...
  if (!(kIODKDisablePM & gIODKDebug) && pmProvider) {
    ...
    if (joinTree) {
      ...
      service->reserved->uvars->resetPowerOnWake =
          service->propertyExists(kIOPMResetPowerStateOnWakeKey);
    }
  }
...
IOReturn IOUserServer::setPowerState(unsigned long state, IOService *service) {
  ...
  if (sendIt) {
    ...
    if (pageout) {
      DKLOG(DKS " state %ld pageins %qd\n", DKN(service), state,
            vm_task_pageins(fOwningTask) - pageincount);
    }
#endif /* DEBUG || DEVELOPMENT */
...
IOReturn IOUserServer::powerStateWillChangeTo(IOPMPowerFlags flags,
                                              unsigned long state,
                                              IOService *service) {
  return kIOPMAckImplied;
}

IOReturn IOUserServer::powerStateDidChangeTo(IOPMPowerFlags flags,
                                             unsigned long state,
                                             IOService *service) {
  unsigned int idx;
  bool pmAck;
  ...
  if (kIODKLogPM & gIODKDebug) {
    DKLOG(DKS "::powerStateDidChangeTo(%ld) %d, %d\n", DKN(service), state,
          allPowerStates, fSystemPowerAck);
  }
...
IOReturn IOUserServer::serviceCreatePMAssertion(IOService *service,
  ...
  *assertionID = kIOPMUndefinedDriverAssertionID;
  ...
  if (!service->reserved->uvars ||
      service->reserved->uvars->userServer != this) {
    return kIOReturnError;
  }
  ...
  uint32_t userAllowedBits =
      kIOPMDriverAssertionCPUBit | kIOPMDriverAssertionForceFullWakeupBit;
  if (synced) {
    userAllowedBits = kIOPMDriverAssertionCPUBit;
  }
  if (0 == (assertionBits & ~userAllowedBits)) {
    if (synced) {
      ret = getPMRootDomain()->acquireDriverKitSyncedAssertion(service,
                                                               assertionID);
      assert(ret != kIOReturnSuccess ||
             *assertionID != kIOPMUndefinedDriverAssertionID);
    } else {
      *assertionID = getPMRootDomain()->createPMAssertion(
          assertionBits, kIOPMDriverAssertionLevelOn, getPMRootDomain(),
          service->getName());
      if (!*assertionID) {
  ...
  if (*assertionID != kIOPMUndefinedDriverAssertionID) {
    ...
    OSArray **pmAssertions =
        (synced ? &service->reserved->uvars->pmAssertionsSynced
                : &service->reserved->uvars->pmAssertions);
    if (!*pmAssertions) {
...
IOReturn
IOUserServer::serviceReleasePMAssertion(IOService *service,
  ...
  if (findAndRemoveAssertionID(service->reserved->uvars->pmAssertionsSynced)) {
    synced = true;
  } else if (!findAndRemoveAssertionID(
                 service->reserved->uvars->pmAssertions)) {
    ret = kIOReturnNotFound;
  }
  IOLockUnlock(fLock);
...
kern_return_t IOService::JoinPMTree_Impl(void) {
...
static kern_return_t acknowledgeSetPowerState(IOService *service) {
  if (service->reserved->uvars && service->reserved->uvars->userServer &&
    ...
    IOReturn ret;
    service->reserved->uvars->willPower = false;
    ret = service->reserved->uvars->controllingDriver->setPowerState(
        service->reserved->uvars->willPowerState, service);
...
kern_return_t IOService::SetPowerState_Impl(uint32_t powerFlags) {
  if (kIODKLogPM & gIODKDebug) {
...
kern_return_t IOService::ChangePowerState_Impl(uint32_t powerFlags) {
  switch (powerFlags) {
...
kern_return_t IOService::SetPowerOverride_Impl(bool enable) {
  kern_return_t ret;
...
kern_return_t IOService::_ClaimSystemWakeEvent_Impl(IOService *device,
                                                    uint64_t flags,
                                                    const char *reason,
                                                    OSContainer *details) {
...
kern_return_t IOService::Create_Impl(IOService *provider,
  ...
  userServerName =
      reserved->uvars->userServer->copyProperty(gIOUserServerNameKey);
  if (userServerName) {
...
kern_return_t IOService::Terminate_Impl(uint64_t options) {
  IOUserServer *us;
  ...
  us = (typeof(us))thread_iokit_tls_get(0);
  if (us && (!reserved->uvars || (reserved->uvars->userServer != us))) {
    return kIOReturnNotPermitted;
  }
...
kern_return_t IOService::NewUserClient_Impl(uint32_t type,
                                            IOUserClient **userClient) {
  return kIOReturnError;
}

kern_return_t IOService::_NewUserClient_Impl(uint32_t type,
                                             OSDictionary *entitlements,
                                             IOUserClient **userClient) {
  return kIOReturnError;
}

kern_return_t IOService::SearchProperty_Impl(const char *name,
                                             const char *plane,
                                             uint64_t options,
  ...
  if (NULL == object) {
    for (provider = this; provider; provider = provider->getProvider()) {
      provider->runPropertyActionBlock(^IOReturn(void) {
        ...
        if (!object && (userProps = OSDynamicCast(
                            OSDictionary, provider->getProperty(
...
kern_return_t IOService::StringFromReturn_Impl(IOReturn retval,
                                               OSString **str) {
...
#if PRIVATE_WIFI_ONLY
const char *IOService::StringFromReturn(IOReturn retval) {
  return stringFromReturn(retval);
}
#endif /* PRIVATE_WIFI_ONLY */

kern_return_t IOService::CopyProviderProperties_Impl(OSArray *propertyKeys,
                                                     OSArray **properties) {
  IOReturn ret;
  OSArray *result;
  IOService *provider;
  ...
  for (provider = this; provider; provider = provider->getProvider()) {
    ...
    if (addClass) {
      ...
      for (const OSMetaClass *meta = provider->getMetaClass(); meta;
           meta = meta->getSuperClass()) {
...
IOReturn IOService::AdjustBusy_Impl(int32_t delta) {
  adjustBusy(delta);
  return kIOReturnSuccess;
}

IOReturn IOService::GetBusyState_Impl(uint32_t *busyState) {
  *busyState = getBusyState();
  return kIOReturnSuccess;
}

kern_return_t IOService::CreatePMAssertion_Impl(uint32_t assertionBits,
                                                uint64_t *assertionID,
                                                bool synced) {
  *assertionID = kIOPMUndefinedDriverAssertionID;
  ...
  return reserved->uvars->userServer->serviceCreatePMAssertion(
      this, assertionBits, assertionID, synced);
}

kern_return_t IOService::ReleasePMAssertion_Impl(uint64_t assertionID) {
  if (!reserved->uvars || !reserved->uvars->userServer) {
    return kIOReturnError;
  }

  return reserved->uvars->userServer->serviceReleasePMAssertion(this,
                                                                assertionID);
}

void IOUserServer::serverAck(void) { IOServicePH::serverAck(this); }

void IOUserServer::systemSuspend() {
  if (fSystemOff && !fSuspended) {
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SUSPEND_DRIVERKIT_USERSPACE) |
             DBG_FUNC_START,
...
void IOUserServer::systemPower(uint8_t systemState, bool hibernate) {
  OSArray *services;
  {
    ...
    if (sleepDescription) {
      ...
      if (prop) {
        sleepDescription->setObject(gIOSystemStateSleepDescriptionReasonKey,
                                    prop);
        OSSafeReleaseNULL(prop);
      }
      prop = getPMRootDomain()->copyProperty(kIOHibernateStateKey);
      if (prop) {
        sleepDescription->setObject(
            gIOSystemStateSleepDescriptionHibernateStateKey, prop);
        OSSafeReleaseNULL(prop);
      }
      if (hibernate) {
        uint32_t correctHibernateState =
            kIOSystemStateSleepDescriptionHibernateStateHibernating;
        OSData *correctHibernateStateData =
            OSData::withValue(correctHibernateState);
        assert(correctHibernateStateData != NULL);
        sleepDescription->setObject(
            gIOSystemStateSleepDescriptionHibernateStateKey,
            correctHibernateStateData);
        OSSafeReleaseNULL(correctHibernateStateData);
      }
      getSystemStateNotificationService()->StateNotificationItemSet(
          gIOSystemStateSleepDescriptionKey, sleepDescription);
  ...
  if (!IsIOServiceSystemStateOff(systemState)) {
    ...
    if (wakeDescription) {
      ...
      getPMRootDomain()->copyWakeReasonString(wakeReasonString,
                                              sizeof(wakeReasonString));

      if (wakeReasonString[0]) {
        prop = OSString::withCString(&wakeReasonString[0]);
        wakeDescription->setObject(gIOSystemStateWakeDescriptionWakeReasonKey,
                                   prop);
        OSSafeReleaseNULL(prop);
      }
#if defined(__arm__) || defined(__arm64__)
      prop = OSNumber::withNumber(ml_get_conttime_offset(),
                                  sizeof(uint64_t) * CHAR_BIT);
      wakeDescription->setObject(
          gIOSystemStateWakeDescriptionContinuousTimeOffsetKey, prop);
      OSSafeReleaseNULL(prop);
#endif /* defined(__arm__) || defined(__arm64__) */
      getSystemStateNotificationService()->StateNotificationItemSet(
          gIOSystemStateWakeDescriptionKey, wakeDescription);
  ...
  effectiveOff |= ((kIOServiceSystemStateOffPhase1 == systemState) &&
                   !fSystemOffPhase2Allow);
  ...
  DKLOG(DKS "::systemPower(0x%x) effective %d current %d\n", DKN(this),
        systemState, !effectiveOff, allPowerStates != 0);
    ...
    if (!fSystemPowerAck) {
      ...
      if (services) {
        services->iterateObjects(^bool(OSObject *obj) {
          ...
          nextService->reserved->uvars->powerOverride =
              nextService->reserved->uvars->userServerPM
                  ? kUserServerMaxPowerState
                  : nextService->getPowerState();
        ...
        if (-1U != nextService->reserved->uvars->powerOverride) {
          if (kIODKLogPM & gIODKDebug) {
            DKLOG("%schangePowerStateWithOverrideTo(" DKS ", %d)\n",
                  nextService->reserved->uvars->resetPowerOnWake ? "!" : "",
                  DKN(nextService),
                  nextService->reserved->uvars->powerOverride);
          }
          if (!nextService->reserved->uvars->resetPowerOnWake) {
            nextService->changePowerStateWithOverrideTo(
                nextService->reserved->uvars->powerOverride, 0);
          }
...
void IOUserServer::systemHalt(int howto) {
  OSArray *services;
...

  {
    ...
    if (haltDescription) {
      ...
      haltDescription->setObject(gIOSystemStateHaltDescriptionHaltStateKey,
                                 state);
      getSystemStateNotificationService()->StateNotificationItemSet(
          gIOSystemStateHaltDescriptionKey, haltDescription);
  ...
  if (services) {
    services->iterateObjects(^bool(OSObject *obj) {
      ...
      root = (NULL ==
              provider->getProperty(gIOUserServerNameKey, gIOServicePlane));
      if (true || (kIODKLogPM & gIODKDebug)) {
      ...
      terminateOptions =
          kIOServiceRequired | kIOServiceTerminateNeedWillTerminate;
      if (!nextService->terminate(terminateOptions)) {
        IOLog("failed to terminate service %s-0x%llx\n", nextService->getName(),
              nextService->getRegistryEntryID());
      }
      return false;
    });
  }
...
void IOUserServer::powerSourceChanged(bool acAttached) {
  ...
  powerSourceDescription->setObject(
      gIOSystemStatePowerSourceDescriptionACAttachedKey,
      acAttached ? kOSBooleanTrue : kOSBooleanFalse);
  getSystemStateNotificationService()->StateNotificationItemSet(
      gIOSystemStatePowerSourceDescriptionKey, powerSourceDescription);

  OSSafeReleaseNULL(powerSourceDescription);
}

IOReturn IOUserServer::serviceStarted(IOService *service, IOService *provider,
                                      bool result) {
  IOReturn ret;
  bool needStop = false;

  DKLOG(DKS "::start(" DKS ") %s\n", DKN(service), DKN(provider),
        result ? "ok" : "fail");

  if (!result) {
    if (!service->reserved->uvars->instantiated && provider) {
      // Object instantiation did not happen. This can happen if,
      // 1. Dext crashed, in which case the user server has been terminated when
      // the task is marked as corpse
  ...
  if (service->reserved->uvars->deferredRegisterService) {
    service->registerService(kIOServiceAsynchronous |
                             kIOServiceDextRequirePowerForMatching);
...
IOReturn IOUserServer::serviceOpen(IOService *provider, IOService *client) {
  OSObjectUserVars *uvars;
  IOReturn ret;
  ...
  if (uvars->willTerminate || uvars->stopped) {
    ...
    if (!uvars->openProviders) {
      uvars->openProviders =
          OSArray::withObjects((const OSObject **)&provider, 1);
...
IOReturn IOUserServer::serviceClose(IOService *provider, IOService *client) {
  OSObjectUserVars *uvars;
  unsigned int idx;
  IOReturn ret;
...
finish:
  ...
  if (kIODKLogSetup & gIODKDebug) {
    DKLOG(DKS "::serviceClose(" DKS ", " DKS ") -> %x\n", DKN(this),
          DKN(provider), DKN(client), ret);
  }

  return ret;
}

IOReturn IOUserServer::serviceStop(IOService *service, IOService *provider) {
  IOReturn ret;
  uint32_t idx;
  bool pmAck;
  bool deferred = false;
  ...
  if (-1U != idx) {
    ...
    OSNumber *__assert_only registryEntryID =
        OSDynamicCast(OSNumber, serviceArray->getObject(idx));
  ...
  if (uvars->userServerPM) {
    ...
    if (uvars->pmAssertionsSynced) {
      uvars->pmAssertionsSynced->iterateObjects(^(OSObject *obj) {
        rootDomain->releaseDriverKitSyncedAssertion(
            ((OSNumber *)obj)->unsigned64BitValue());
        return false;
      });
...
void IOUserServer::serviceFree(IOService *service) {
  OSObjectUserVars *uvars;
...
void IOUserServer::serviceWillTerminate(IOService *client, IOService *provider,
...
void IOUserServer::serviceDidTerminate(IOService *client, IOService *provider,
...
void IOUserServer::serviceDidStop(IOService *client, IOService *provider) {
  bool complete;
  OSArray *closeArray;
  ...
  IOLockLock(client->reserved->uvars->uvarsLock);
  if (client->reserved->uvars && client->reserved->uvars->willTerminate &&
      !client->reserved->uvars->stopped) {
...
kern_return_t IOService::ClientCrashed_Impl(IOService *client,
                                            uint64_t options) {
  return kIOReturnUnsupported;
}

kern_return_t IOService::Stop_Impl(IOService *provider) {
  IOUserServer::serviceDidStop(this, provider);

  return kIOReturnSuccess;
}

void IOService::Stop_async_Impl(IOService *provider) {}
...
OSDefineMetaClassAndStructors(IOUserUserClient, IOUserClient)
    ...
    bool IOUserUserClient::init(OSDictionary *properties) {
...
IOReturn IOUserUserClient::setTask(task_t task) {
  task_reference(task);
  fTask = task;
...
void IOUserUserClient::stop(IOService *provider) {
  if (fTask) {
...
IOReturn IOUserUserClient::clientClose(void) {
  terminate(kIOServiceTerminateNeedWillTerminate);
  return kIOReturnSuccess;
}

IOReturn IOUserUserClient::setProperties(OSObject *properties) {
  IOReturn ret = kIOReturnUnsupported;
  return ret;
}
...
kern_return_t IOUserUserClient::eventlinkConfigurationTrap(void *p1, void *p2,
                                                           void *p3, void *p4,
                                                           void *p5, void *p6) {
  ...
  ret = copyinstr(userObjectName, &eventlinkName[0], sizeof(eventlinkName),
                  &eventLinkNameLen);
  if (ret != kIOReturnSuccess) {
    goto finish;
  }
  ...
  IOLockLock(fLock);
  eventLink =
      OSDynamicCast(IOEventLink, fEventLinks->getObject(eventlinkNameStr));
  if (eventLink) {
    eventLink->retain();
  }
  IOLockUnlock(fLock);
  ...
  if (ret != kIOReturnSuccess) {
    if (kIODKLogSetup & gIODKDebug) {
      DKLOG(DKS " %s SetEventlinkPort() returned %x\n", DKN(this),
            eventlinkNameStr->getCStringNoCopy(), ret);
    }
    goto finish;
  }
...
kern_return_t IOUserUserClient::workgroupConfigurationTrap(void *p1, void *p2,
                                                           void *p3, void *p4,
                                                           void *p5, void *p6) {
  user_addr_t userObjectName = (user_addr_t)p1;
  mach_port_name_t portName = (mach_port_name_t)(uintptr_t)p3;
  ...
  ret = copyinstr(userObjectName, &workgroupName[0], sizeof(workgroupName),
                  &workgroupNameLen);
  if (ret != kIOReturnSuccess) {
    goto finish;
  }
  ...
  IOLockLock(fLock);
  workgroup =
      OSDynamicCast(IOWorkGroup, fWorkGroups->getObject(workgroupNameStr));
  if (workgroup) {
    workgroup->retain();
  }
  IOLockUnlock(fLock);
  ...
  if (ret != kIOReturnSuccess) {
    if (kIODKLogSetup & gIODKDebug) {
      DKLOG(DKS " %s SetWorkGroupPort() returned %x\n", DKN(this),
            workgroupNameStr->getCStringNoCopy(), ret);
    }
    goto finish;
  }
...
IOExternalTrap *IOUserUserClient::getTargetAndTrapForIndex(IOService **targetP,
                                                           UInt32 index) {
...
void IOUserClient::KernelCompletion_Impl(OSAction *action, IOReturn status,
  ...
  IOUserClient::sendAsyncResult64(
      ref->asyncRef, status, (io_user_reference_t *)asyncData, asyncDataCount);
}

kern_return_t IOUserClient::_ExternalMethod_Impl(
    uint64_t selector, const unsigned long long *scalarInput,
    uint32_t scalarInputCount, OSData *structureInput,
    IOMemoryDescriptor *structureInputDescriptor,
    unsigned long long *scalarOutput, uint32_t *scalarOutputCount,
    uint64_t structureOutputMaximumSize, OSData **structureOutput,
    IOMemoryDescriptor *structureOutputDescriptor, OSAction *completion) {
  return kIOReturnUnsupported;
}

IOReturn IOUserUserClient::clientMemoryForType(UInt32 type,
                                               IOOptionBits *koptions,
...
IOReturn IOUserUserClient::externalMethod(uint32_t selector,
                                          IOExternalMethodArguments *args,
  ...
  if (args->structureInputSize) {
    structureInput = OSData::withBytesNoCopy((void *)args->structureInput,
  ...
  if (MACH_PORT_NULL != args->asyncWakePort) {
    ...
    kr = CreateActionKernelCompletion(sizeof(IOUserUserClientActionRef),
                                      &action);
    assert(KERN_SUCCESS == kr);
    ref = (typeof(ref))action->GetReference();
    bcopy(args->asyncReference, &ref->asyncRef[0],
          args->asyncReferenceCount * sizeof(ref->asyncRef[0]));
  ...
  kr = _ExternalMethod(selector, &args->scalarInput[0], args->scalarInputCount,
                       ...
                       structureOutputSize, &structureOutput,
                       args->structureOutputDescriptor, action);
    ...
    if (args->structureVariableOutputData) {
      *args->structureVariableOutputData = structureOutput;
      ...
      if (copylen > args->structureOutputSize) {
        ...
        bcopy((const void *)structureOutput->getBytesNoCopy(),
              args->structureOutput, copylen);
  ...
  // We must never return error after this point in order to preserve MIG
  // ownership semantics
...
IOUserServerCheckInToken::setCancellationHandler(
    IOUserServerCheckInCancellationHandler handler, void *handlerArgs) {
  _IOUserServerCheckInCancellationHandler *handlerObj =
      _IOUserServerCheckInCancellationHandler::withHandler(handler,
                                                           handlerArgs);
  if (!handlerObj) {
    goto finish;
  }
...
void IOUserServerCheckInToken::removeCancellationHandler(
    _IOUserServerCheckInCancellationHandler *handler) {
  IORecursiveLockLock(gDriverKitLaunchLock);
...
void IOUserServerCheckInToken::cancel() {
  ...
  if (fState == kIOUserServerCheckInPending) {
    ...
    fHandlers->iterateObjects(^bool(OSObject *obj) {
      _IOUserServerCheckInCancellationHandler *handlerObj =
          OSDynamicCast(_IOUserServerCheckInCancellationHandler, obj);
      if (handlerObj) {
        handlerObj->call(this);
      }
      return false;
...
IOReturn IOUserServerCheckInToken::complete() {
  IOReturn ret;
  IORecursiveLockLock(gDriverKitLaunchLock);
...
bool IOUserServerCheckInToken::init(const OSSymbol *serverName,
                                    OSNumber *serverTag, OSKext *driverKext,
                                    OSData *serverDUI) {
  if (!OSObject::init()) {
    return false;
  }
  ...
  if (driverKext) {
    fExecutableName =
        OSDynamicCast(OSSymbol, driverKext->getBundleExecutable());
 ...
 * for matching.
 */
bool IOUserServerCheckInToken::dextTerminate(void) {
  bool ret = true;
...
void IOUserServerCheckInToken::free() {
...
const OSSymbol *IOUserServerCheckInToken::copyServerName() const {
  fServerName->retain();
  return fServerName;
}

OSNumber *IOUserServerCheckInToken::copyServerTag() const {
  fServerTag->retain();
  return fServerTag;
}

IOUserServer *
IOUserServer::launchUserServer(OSString *bundleID, const OSSymbol *serverName,
                               OSNumber *serverTag, bool reuseIfExists,
                               IOUserServerCheckInToken **resultToken,
                               OSData *serverDUI) {
  ...
  if (driverKext != NULL) {
    ...
    IOLog("Driver %s has crashed %zu time(s)\n", bundleID->getCStringNoCopy(),
          driverStatistics->getCrashCount());
  ...
  if (reuseIfExists) {
    ...
    (endOrgCStr = strchr(bundleIDCStr, '.')) &&
        (endOrgCStr = strchr(endOrgCStr + 1, '.'));
    reuseIfExists = endOrgCStr && (0 == strncmp(bundleIDCStr, serverNameCStr,
                                                endOrgCStr + 1 - bundleIDCStr));
    if (!reuseIfExists) {
      IOLog(kIOUserServerNameKey
            " \"%s\" not correct organization for bundleID \"%s\"\n",
  ...
  OSKext::requestDaemonLaunch(bundleID, (OSString *)serverName, serverTag,
                              reslide ? kOSBooleanTrue : kOSBooleanFalse, token,
                              serverDUI);
...
/*
 * IOUserServerCheckInTokens are used to track dext launches. They have three
 * possible states:
 *
 ...
 *
 * A token can be shared among multiple IOServices that are waiting for dexts if
 * the IOUserServerName is the same. This allows dexts to be reused and host
 * multiple services. All pending tokens are stored in gDriverKitLaunches and we
 * check here before creating a new token when launching a dext.
 *
 * A token starts in the pending state with a pending count of 1. When we reuse
 * a token, we increase the pending count of the token.
 *
 * The token is sent to userspace as a mach port through
 * kernelmanagerd/driverkitd to the dext. The dext then uses that token to check
 * in to the kernel. If any part of the dext launch failed (dext crashed, kmd
 * crashed, etc.) we get a no-senders notification for the token in the kernel
 * and the token goes into the Canceled state.
 *
 * Once the dext checks in to the kernel, we decrement the pending count for the
 * token. When the pending count reaches 0, the token goes into the Complete
 * state. So if the token is in the Complete state, there are no kernel matching
 * threads waiting on the dext to check in.
 */
...
void _IOUserServerCheckInCancellationHandler::call(
    IOUserServerCheckInToken *token) {
  fHandler(token, fHandlerArgs);
}

_IOUserServerCheckInCancellationHandler *
_IOUserServerCheckInCancellationHandler::withHandler(
    IOUserServerCheckInCancellationHandler handler, void *args) {
...
kern_return_t IOServiceStateNotificationDispatchSource::Create_Impl(
    IOService *service, OSArray *items, IODispatchQueue *queue,
    IOServiceStateNotificationDispatchSource **outSource) {
  kern_return_t kr;
  ...
  kr = service->stateNotificationListenerAdd(
      items, &source->ivars->fListener, ^kern_return_t() {
        OSAction *action;
...
bool IOServiceStateNotificationDispatchSource::init() {
...
void IOServiceStateNotificationDispatchSource::free() {
  if (ivars) {
    if (ivars->fListener) {
      ivars->fStateNotification->stateNotificationListenerRemove(
          ivars->fListener);
    }
    if (ivars->fLock) {
...
kern_return_t IOServiceStateNotificationDispatchSource::Cancel_Impl(
    IODispatchSourceCancelHandler handler) {
  return kIOReturnUnsupported;
}
...
OSDefineMetaClassAndStructors(IOServiceStateNotificationEventSource,
                              IOEventSource)
...
IOServiceStateNotificationEventSource::serviceStateNotificationEventSource(
    IOService *service, OSArray *items, ActionBlock inAction) {
  kern_return_t kr;
  ...
  kr = service->stateNotificationListenerAdd(
      items, &source->fListener, ^kern_return_t() {
        if (!source->workLoop) {
          return kIOReturnSuccess;
        }
...
void IOServiceStateNotificationEventSource::free() {
  if (fListener) {
...
void IOServiceStateNotificationEventSource::enable() { fEnable = true; }

void IOServiceStateNotificationEventSource::disable() { fEnable = false; }

void IOServiceStateNotificationEventSource::setWorkLoop(
    IOWorkLoop *inWorkLoop) {
  IOEventSource::setWorkLoop(inWorkLoop);
}

bool IOServiceStateNotificationEventSource::checkForWork() {
...
bool IOStateNotificationItem::init() { return OSObject::init(); }

bool IOStateNotificationListener::init() { return OSObject::init(); }

void IOStateNotificationListener::free() {
  if (fHandler) {
...
IOService *IOSystemStateNotification::initialize(void) {
  ...
  vars->fItems = OSDictionary::withCapacity(16);
  {
    ...
    gIOSystemStateSleepDescriptionKey = (OSString *)OSSymbol::withCStringNoCopy(
        kIOSystemStateSleepDescriptionKey);
    gIOSystemStateSleepDescriptionHibernateStateKey =
        OSSymbol::withCStringNoCopy(
            kIOSystemStateSleepDescriptionHibernateStateKey);
    gIOSystemStateSleepDescriptionReasonKey =
        OSSymbol::withCStringNoCopy(kIOSystemStateSleepDescriptionReasonKey);

    ret = me->StateNotificationItemCreate(gIOSystemStateSleepDescriptionKey,
                                          NULL);
    assert(kIOReturnSuccess == ret);

    gIOSystemStateWakeDescriptionKey = (OSString *)OSSymbol::withCStringNoCopy(
        kIOSystemStateWakeDescriptionKey);
    gIOSystemStateWakeDescriptionWakeReasonKey =
        OSSymbol::withCStringNoCopy(kIOSystemStateWakeDescriptionWakeReasonKey);
    gIOSystemStateWakeDescriptionContinuousTimeOffsetKey =
        OSSymbol::withCStringNoCopy(
    ...
    OSObject *prop = OSNumber::withNumber(ml_get_conttime_offset(),
                                          sizeof(uint64_t) * CHAR_BIT);
    wakeDescription->setObject(
        gIOSystemStateWakeDescriptionContinuousTimeOffsetKey, prop);
    ...
    ret =
        me->StateNotificationItemCreate(gIOSystemStateWakeDescriptionKey, NULL);
    assert(kIOReturnSuccess == ret);
#endif /* defined(__arm__) || defined(__arm64__) */

    gIOSystemStateHaltDescriptionKey = (OSString *)OSSymbol::withCStringNoCopy(
        kIOSystemStateHaltDescriptionKey);
    gIOSystemStateHaltDescriptionHaltStateKey =
        OSSymbol::withCStringNoCopy(kIOSystemStateHaltDescriptionHaltStateKey);

    ret =
        me->StateNotificationItemCreate(gIOSystemStateHaltDescriptionKey, NULL);
    assert(kIOReturnSuccess == ret);

    gIOSystemStatePowerSourceDescriptionKey =
        (OSString *)OSSymbol::withCStringNoCopy(
            kIOSystemStatePowerSourceDescriptionKey);
    gIOSystemStatePowerSourceDescriptionACAttachedKey =
        OSSymbol::withCStringNoCopy(
            kIOSystemStatePowerSourceDescriptionACAttachedKey);

    ret = me->StateNotificationItemCreate(
        gIOSystemStatePowerSourceDescriptionKey, NULL);
    assert(kIOReturnSuccess == ret);
  }
...
bool IOSystemStateNotification::serializeProperties(OSSerialize *s) const {
...
kern_return_t IOSystemStateNotification::setProperties(OSObject *properties) {
  kern_return_t kr;
  OSDictionary *dict;
  OSDictionary *value;
  OSString *itemName;
  ...
  if ((value = OSDynamicCast(
           OSDictionary, dict->getObject(kIOStateNotificationItemCreateKey)))) {
    itemName =
        OSDynamicCast(OSString, value->getObject(kIOStateNotificationNameKey));
    itemName->retain();
    value->removeObject(kIOStateNotificationNameKey);
    kr = StateNotificationItemCreate(itemName, value);
    itemName->release();
  } else if ((value = OSDynamicCast(
                  OSDictionary,
                  dict->getObject(kIOStateNotificationItemSetKey)))) {
    itemName =
        OSDynamicCast(OSString, value->getObject(kIOStateNotificationNameKey));
    itemName->retain();
    value->removeObject(kIOStateNotificationNameKey);
...
IOService::stateNotificationItemCopy(OSString *itemName,
                                     OSDictionary *initialValue) {
  IOServiceStateChangeVars *ivars = reserved->svars;
...
kern_return_t IOService::StateNotificationItemCreate_Impl(OSString *itemName,
                                                          OSDictionary *value) {
  IOStateNotificationItem *item;
...
kern_return_t IOService::StateNotificationItemSet_Impl(OSString *itemName,
                                                       OSDictionary *value) {
  kern_return_t ret = kIOReturnSuccess;
  IOServiceStateChangeVars *ivars = reserved->svars;
...
kern_return_t
IOService::StateNotificationItemCopy_Impl(OSString *itemName,
                                          OSDictionary **outValue) {
  IOServiceStateChangeVars *ivars = reserved->svars;
...
kern_return_t IOWorkGroup::Create_Impl(OSString *name, IOUserClient *userClient,
...
kern_return_t IOWorkGroup::InvalidateKernel_Impl(IOUserClient *client) {
...
kern_return_t IOWorkGroup::SetWorkGroupPort_Impl(mach_port_t port) {
  return kIOReturnUnsupported;
}

bool IOWorkGroup::init() {
  if (!OSObject::init()) {
    return false;
  }
...
kern_return_t IOEventLink::Create_Impl(OSString *name, IOUserClient *userClient,
...
kern_return_t IOEventLink::InvalidateKernel_Impl(IOUserClient *client) {
  IOUserUserClient *uc = OSDynamicCast(IOUserUserClient, client);
...
kern_return_t IOEventLink::SetEventlinkPort_Impl(mach_port_t port __unused) {
  return kIOReturnUnsupported;
}
