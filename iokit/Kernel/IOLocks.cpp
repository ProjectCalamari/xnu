...
void IOLockInlineInit(IOLock *lock) {
  lck_mtx_init(lock, IOLockGroup, LCK_ATTR_NULL);
}

void IOLockInlineDestroy(IOLock *lock) { lck_mtx_destroy(lock, IOLockGroup); }

void IOLockFree(IOLock *lock) { lck_mtx_free(lock, IOLockGroup); }

lck_mtx_t *IOLockGetMachLock(IOLock *lock) { return (lck_mtx_t *)lock; }

int IOLockSleep(IOLock *lock, void *event, UInt32 interType) {
  return (int)lck_mtx_sleep(lock, LCK_SLEEP_PROMOTED_PRI, (event_t)event,
                            (wait_interrupt_t)interType);
}

int IOLockSleepDeadline(IOLock *lock, void *event, AbsoluteTime deadline,
                        UInt32 interType) {
  return (int)lck_mtx_sleep_deadline(
...
int IOLockSleepWithInheritor(IOLock *lock, UInt32 lck_sleep_action, void *event,
                             ...
                             uint64_t deadline) {
  return (int)lck_mtx_sleep_with_inheritor(
      lock, (lck_sleep_action_t)lck_sleep_action, (event_t)event, inheritor,
...
#if defined(__x86_64__)
/*
 * For backwards compatibility, kexts built against pre-Darwin 14 headers will
 * bind at runtime to this function, which supports a NULL event,
 */
int IOLockSleep_legacy_x86_64(IOLock *lock, void *event,
                              UInt32 interType) __asm("_IOLockSleep");
int IOLockSleepDeadline_legacy_x86_64(
    IOLock *lock, void *event, AbsoluteTime deadline,
    UInt32 interType) __asm("_IOLockSleepDeadline");
void IOLockWakeup_legacy_x86_64(IOLock *lock, void *event,
                                bool oneThread) __asm("_IOLockWakeup");
...
int IORecursiveLockSleep(IORecursiveLock *_lock, void *event,
                         UInt32 interType) {
  _IORecursiveLock *lock = (_IORecursiveLock *)_lock;
  ...
  lock->count = 0;
  lock->thread = NULL;
  res = lck_mtx_sleep(&lock->mutex, LCK_SLEEP_PROMOTED_PRI, (event_t)event,
                      (wait_interrupt_t)interType);
...
int IORecursiveLockSleepDeadline(IORecursiveLock *_lock, void *event,
  ...
  lock->count = 0;
  lock->thread = NULL;
  res = lck_mtx_sleep_deadline(&lock->mutex, LCK_SLEEP_PROMOTED_PRI,
...
void IORWLockInlineInit(IORWLock *lock) {
  lck_rw_init(lock, IOLockGroup, LCK_ATTR_NULL);
}

void IORWLockInlineDestroy(IORWLock *lock) {
  lck_rw_destroy(lock, IOLockGroup);
}

void IORWLockFree(IORWLock *lock) { lck_rw_free(lock, IOLockGroup); }

lck_rw_t *IORWLockGetMachLock(IORWLock *lock) { return (lck_rw_t *)lock; }
...
void IOSimpleLockFree(IOSimpleLock *lock) { lck_spin_free(lock, IOLockGroup); }
