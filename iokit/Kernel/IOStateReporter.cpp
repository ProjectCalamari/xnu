...
IOStateReporter::with(IOService *reportingService,
  ...
  if (!reporter->initWith(reportingService, categories, (int16_t)nstates,
                          unit)) {
    return nullptr;
  }
...
bool IOStateReporter::initWith(IOService *reportingService,
...
void IOStateReporter::free(void) {
  if (_currentStates) {
...
IOReturn IOStateReporter::handleSwapPrepare(int newNChannels) {
...
IOReturn IOStateReporter::handleAddChannelSwap(uint64_t channelID,
  ...
  if (_currentStates) {
    ...
    if (!_lastUpdateTimes) {
      panic("IOStateReporter::handleAddChannelSwap _lastUpdateTimes unset "
            "despite non-NULL _currentStates");
    }
  ...
  // Channel added successfully, initialize the new channel's state_ids to
  // 0..nStates-1
...
void IOStateReporter::handleSwapCleanup(int swapNChannels) {
  IOREPORTER_CHECK_CONFIG_LOCK();
...
IOReturn IOStateReporter::_getStateIndices(uint64_t channel_id,
                                           uint64_t state_id,
                                           int *channel_index,
                                           int *state_index) {
  IOReturn res = kIOReturnError;
  ...
  if (getChannelIndices(channel_id, channel_index, &element_index) !=
      kIOReturnSuccess) {
...
IOReturn IOStateReporter::setChannelState(uint64_t channel_id,
  ...
  if (_getStateIndices(channel_id, new_state_id, &channel_index,
                       &new_state_index) == kIOReturnSuccess) {
...
IOReturn IOStateReporter::overrideChannelState(
    uint64_t channel_id, uint64_t state_id, uint64_t time_in_state,
  ...
  if (_getStateIndices(channel_id, state_id, &channel_index, &state_index) ==
      kIOReturnSuccess) {
...
IOReturn IOStateReporter::handleOverrideChannelStateByIndices(
  ...
  if (channel_index < 0 ||
      channel_index > (_nElements - state_index) / _channelDimension) {
  ...
  kerr =
      copyElementValues(element_index, (IOReportElementValues *)&state_values);
  if (kerr) {
    result = kerr;
    goto finish;
  }
  ...
  kerr =
      setElementValues(element_index, (IOReportElementValues *)&state_values);
  if (kerr) {
    result = kerr;
    goto finish;
  }
...
IOReturn IOStateReporter::incrementChannelState(
    uint64_t channel_id, uint64_t state_id, uint64_t time_in_state,
    uint64_t intransitions, uint64_t last_intransition /*=0*/) {
  IOReturn res = kIOReturnError;
  int channel_index, state_index;
...
IOReturn IOStateReporter::setState(uint64_t new_state_id) {
  uint64_t last_intransition = 0;
  uint64_t prev_state_residency = 0;
  IOReturn res = kIOReturnError;
  IOStateReportValues *values;
...
IOReturn IOStateReporter::setStateID(uint64_t channel_id, int state_index,
                                     uint64_t state_id) {
  IOReturn res = kIOReturnError;
...
IOReturn IOStateReporter::handleSetStateID(uint64_t channel_id, int state_index,
                                           uint64_t state_id) {
  IOReturn res = kIOReturnError;
  IOStateReportValues state_values;
  int element_index = 0;
...
IOReturn IOStateReporter::setStateByIndices(int channel_index,
                                            int new_state_index) {
  IOReturn res = kIOReturnError;
  uint64_t last_intransition = 0;
  uint64_t prev_state_residency = 0;
...
IOReturn
IOStateReporter::handleSetStateByIndices(int channel_index, int new_state_index,
                                         uint64_t last_intransition,
                                         uint64_t prev_state_residency) {
  IOReturn res = kIOReturnError;
  ...
  if (new_state_index != curr_state_index) {
    // fetch element data
    if (channel_index < 0 ||
        channel_index > (_nElements - new_state_index) / _channelDimension) {
      res = kIOReturnOverrun;
      goto finish;
    }
  ...
  if (last_ch_update_time != 0) {
    if (channel_index < 0 ||
        channel_index > (_nElements - curr_state_index) / _channelDimension) {
      res = kIOReturnOverrun;
      goto finish;
    }
...
uint64_t IOStateReporter::getStateInTransitions(uint64_t channel_id,
                                                uint64_t state_id) {
  return _getStateValue(channel_id, state_id, kInTransitions);
}

uint64_t IOStateReporter::getStateResidencyTime(uint64_t channel_id,
                                                uint64_t state_id) {
  return _getStateValue(channel_id, state_id, kResidencyTime);
}

uint64_t IOStateReporter::getStateLastTransitionTime(uint64_t channel_id,
                                                     uint64_t state_id) {
  return _getStateValue(channel_id, state_id, kLastTransitionTime);
}

uint64_t IOStateReporter::_getStateValue(uint64_t channel_id, uint64_t state_id,
...
uint64_t IOStateReporter::getStateLastChannelUpdateTime(uint64_t channel_id) {
  int channel_index;
  uint64_t result = kIOReportInvalidValue;
 ...
 */
IOReturn IOStateReporter::updateChannelValues(int channel_index) {
  IOReturn kerr, result = kIOReturnError;
  ...
  // and store the values
  kerr = setElementValues(element_idx, (IOReportElementValues *)&state_values,
                          currentTime);
  if (kerr) {
    result = kerr;
    goto finish;
...
/* static */ OSPtr<IOReportLegendEntry> IOStateReporter::createLegend(
    const uint64_t *channelIDs, const char **channelNames, int channelCount,
    int nstates, IOReportCategories categories, IOReportUnit unit) {
  IOReportChannelType channelType = {.categories = categories,
                                     .report_format = kIOReportFormatState,
                                     .nelements =
                                         static_cast<uint16_t>(nstates),
