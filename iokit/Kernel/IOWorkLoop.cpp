...
static inline void SETP(void *addr, unsigned int flag) {
  unsigned char *num = (unsigned char *)addr;
  *num |= flag;
}
...
#define IOStatisticsCloseGate()                                                \
  do {                                                                         \
    ...
    if (reserved->lockInterval)                                                \
      reserved->lockTime = mach_absolute_time();                               \
  } while (0)

#define IOStatisticsAttachEventSource()                                        \
  do {                                                                         \
    IOStatistics::attachWorkLoopEventSource(reserved->counter,                 \
                                            inEvent->reserved->counter);       \
  } while (0)

#define IOStatisticsDetachEventSource()                                        \
  do {                                                                         \
    IOStatistics::detachWorkLoopEventSource(reserved->counter,                 \
                                            inEvent->reserved->counter);       \
  } while (0)
...
bool IOWorkLoop::init() {
  ...
  if (controlG == NULL) {
    controlG = IOCommandGate::commandGate(
        this, OSMemberFunctionCast(IOCommandGate::Action, this,
  ...
  if (workThread == NULL) {
    thread_continue_t cptr =
        OSMemberFunctionCast(thread_continue_t, this, &IOWorkLoop::threadMain);
...
IOWorkLoop *IOWorkLoop::workLoop() {
  return IOWorkLoop::workLoopWithOptions(0);
}

IOWorkLoop *IOWorkLoop::workLoopWithOptions(IOOptionBits options) {
...
void IOWorkLoop::releaseEventChain(LIBKERN_CONSUMED IOEventSource *eventChain) {
...
IOReturn IOWorkLoop::addEventSource(IOEventSource *newEvent) {
  if ((workThread) && !thread_has_thread_name(workThread) &&
      (newEvent->owner) && !OSDynamicCast(IOCommandPool, newEvent->owner)) {
    thread_set_thread_name(workThread,
                           newEvent->owner->getMetaClass()->getClassName());
  }
...
/* virtual */ bool IOWorkLoop::runEventSources() {
  ...
  if (traceWL) {
    IOTimeStampStartConstant(IODBG_WORKLOOP(IOWL_WORK),
  ...
  do {
    ...
    workToDo = false;
    IOSimpleLockUnlockEnableInterrupt(workToDoLock, is);
    /* NOTE: only loop over event sources in eventChain. Bypass "passive" event
     * sources for performance */
    for (IOEventSource *evnt = eventChain; evnt; evnt = evnt->getNext()) {
      if (traceES) {
        IOTimeStampStartConstant(IODBG_WORKLOOP(IOWL_CLIENT),
                                 VM_KERNEL_ADDRHIDE(this),
                                 VM_KERNEL_ADDRHIDE(evnt));
      }

      more |= evnt->checkForWork();

      if (traceES) {
        IOTimeStampEndConstant(IODBG_WORKLOOP(IOWL_CLIENT),
                               VM_KERNEL_ADDRHIDE(this),
                               VM_KERNEL_ADDRHIDE(evnt));
      }
  ...
  do {
    ...
    if (!ISSETP(&fFlags, kLoopTerminate) && !workToDo) {
      ...
      if (!reserved || !(kPreciousStack & reserved->options)) {
        cptr = OSMemberFunctionCast(thread_continue_t, this,
...
IOThread IOWorkLoop::getThread() const { return workThread; }

bool IOWorkLoop::onThread() const { return IOThreadSelf() == workThread; }

bool IOWorkLoop::inGate() const { return IORecursiveLockHaveLock(gateLock); }
...
int IOWorkLoop::sleepGate(void *event, AbsoluteTime deadline,
                          UInt32 interuptibleType) {
  int res;
  IOStatisticsOpenGate();
  res =
      IORecursiveLockSleepDeadline(gateLock, event, deadline, interuptibleType);
  IOStatisticsCloseGate();
  return res;
}
...
static IOReturn IOWorkLoopActionToBlock(OSObject *owner, void *arg0, void *arg1,
                                        void *arg2, void *arg3) {
...
IOReturn IOWorkLoop::runAction(Action inAction, OSObject *target, void *arg0,
    ...
    if (!inEvent->getWorkLoop()) {
      ...
      if (eventSourcePerformsWork(inEvent)) {
        ...
        if (!passiveEventChain) {
          ...
          for (event = passiveEventChain; (next = event->getNext());
               event = next) {
            ;
          }
          event->setNext(inEvent);
...
bool IOWorkLoop::eventSourcePerformsWork(IOEventSource *inEventSource) {
  bool result = true;

  /*
   * The idea here is to see if the subclass of IOEventSource has overridden
   * checkForWork(). The assumption is that if you override checkForWork(), you
   * need to be active and not passive.
   *
   ...
   *
   * We do a pointer comparison on the offset in the vtable for inNewEvent
   * against the offset in the vtable for inReferenceEvent. This works because
  ...
  if (IOEventSource::kPassive & inEventSource->flags) {
    ...
    ptr1 = OSMemberFunctionCast(void *, inEventSource,
...
void IOWorkLoop::lockTime(void) {
  ...
  if (time > reserved->lockInterval) {
    ...
    if (kTimeLockPanics & reserved->options) {
      ...
      OSReportWithBacktrace("IOWorkLoop %p lock time %qd us", this,
                            time / 1000ULL);
    }
  }
}

void IOWorkLoop::setMaximumLockTime(uint64_t interval, uint32_t options) {
  IORecursiveLockLock(gateLock);
  reserved->lockInterval = interval;
  reserved->options =
      (reserved->options & ~kTimeLockPanics) | (options & kTimeLockPanics);
  IORecursiveLockUnlock(gateLock);
}
