...
__inline__ void IOTimerEventSource::invokeAction(IOEventSource::Action _action,
  ...
  if (kActionBlock & flags) {
    address = ptrauth_nop_cast(
        void *, _Block_get_invoke_fn((struct Block_layout *)actionBlock));
  } else {
...
#if CONFIG_DTRACE
  DTRACE_TMR3(iotescallout__expire, Action, address, OSObject, _owner, void,
              _workLoop);
#endif
...
void IOTimerEventSource::timeoutAndRelease(void *self, void *c) {
  IOTimerEventSource *me = (IOTimerEventSource *)self;
  /* The second parameter (a pointer) gets abused to carry an SInt32, so on
   * LP64, "count" must be cast to "long" before, in order to tell GCC we're not
   * truncating a pointer. */
  SInt32 count = (SInt32)(long)c;

  IOStatisticsTimeout();

  if (me->enabled && me->action) {
    IOWorkLoop *wl = me->reserved->workLoop;
    if (wl) {
      IOEventSource::Action doit;
      wl->closeGate();
      IOStatisticsCloseGate();
...
void IOTimerEventSource::setTimeoutFunc() {
  ...
  calloutEntry = (void *)thread_call_allocate_with_options(
      func, (thread_call_param_t)this, pri, tcoptions);
  assert(calloutEntry);
}
...
bool IOTimerEventSource::init(uint32_t options, OSObject *inOwner,
                              Action inAction) {
  // make use of an existing ivar for parameter passing
  abstime = options;
...
IOTimerEventSource *IOTimerEventSource::timerEventSource(uint32_t inOptions,
                                                         OSObject *inOwner,
                                                         Action inAction) {
...
IOTimerEventSource *IOTimerEventSource::timerEventSource(uint32_t options,
                                                         OSObject *inOwner,
                                                         ActionBlock _action) {
...
#define _thread_call_cancel(tc)                                                \
  ((kActive & flags) ? thread_call_cancel_wait((tc)) : thread_call_cancel((tc)))

IOTimerEventSource *IOTimerEventSource::timerEventSource(OSObject *inOwner,
                                                         Action inAction) {
  return IOTimerEventSource::timerEventSource(
      kIOTimerEventSourceOptionsPriorityKernelHigh, inOwner, inAction);
}
...
IOReturn IOTimerEventSource::setTimeout(mach_timespec_t interval) {
  ...
  clock_interval_to_absolutetime_interval(interval.tv_nsec, kNanosecondScale,
                                          &nsecs);
  clock_interval_to_deadline(interval.tv_sec, NSEC_PER_SEC, &end);
...
IOReturn IOTimerEventSource::setTimeout(uint32_t options, AbsoluteTime abstime,
                                        AbsoluteTime leeway) {
  AbsoluteTime end;
...
IOReturn IOTimerEventSource::wakeAtTime(mach_timespec_t inAbstime) {
  ...
  clock_interval_to_absolutetime_interval(inAbstime.tv_nsec, kNanosecondScale,
                                          &nsecs);
  clock_interval_to_absolutetime_interval(inAbstime.tv_sec, kSecondScale, &end);
  ADD_ABSOLUTETIME(&end, &nsecs);
...
IOReturn IOTimerEventSource::wakeAtTime(uint32_t options,
  ...
  abstime = inAbstime;
  if (enabled && AbsoluteTime_to_scalar(&inAbstime) &&
      AbsoluteTime_to_scalar(&abstime) && workLoop) {
    ...
    if (reserved) {
      ...
      if (thread_call_enter_delayed_with_leeway(
              (thread_call_t)calloutEntry,
              (void *)(uintptr_t)reserved->calloutGeneration, inAbstime, leeway,
              tcoptions) &&
          (kPassive & flags)) {
        release();
        workLoop->release();
      ...
      thread_call_enter_delayed_with_leeway((thread_call_t)calloutEntry, NULL,
                                            inAbstime, leeway, tcoptions);
    }
  }
