...
// FIXME: These are in <kern/misc_protos.h> but that file has other deps that
...
static void processor_idle_wrapper(cpu_id_t /*cpu_id*/, boolean_t enter,
...
static void idle_timer_wrapper(void * /*refCon*/, uint64_t *new_timeout_ticks) {
  gPMGR->updateCPUIdle(new_timeout_ticks);
}

static OSDictionary *matching_dict_for_cpu_id(unsigned int cpu_id) {
...
static void register_aic_handlers(const ml_topology_cpu *cpu_info,
  ...
  if (irqcount == 3) {
    ...
    if (cpu->registerInterrupt(0, NULL, (IOInterruptAction)ipi_handler, NULL) !=
            kIOReturnSuccess ||
        cpu->enableInterrupt(0) != kIOReturnSuccess ||
        cpu->registerInterrupt(2, NULL, (IOInterruptAction)ipi_handler, NULL) !=
            kIOReturnSuccess ||
  ...
  // Conditional, because on Skye and later, we use an FIQ instead of an
  // external IRQ.
  if (pmi_handler && irqcount == 1) {
    if (cpu->registerInterrupt(1, NULL,
                               (IOInterruptAction)(void (*)(void))pmi_handler,
...
static void cpu_boot_thread(void * /*unused0*/, wait_result_t /*unused1*/) {
  ...
  gPMGR = OSDynamicCast(
      IOPMGR, IOService::waitForMatchingService(matching, UINT64_MAX));
  matching->release();

  const size_t array_size =
      (topology_info->max_cpu_id + 1) * sizeof(*machProcessors);
  machProcessors =
      static_cast<processor_t *>(zalloc_permanent(array_size, ZALIGN_PTR));
  ...
  for (unsigned int cpu = 0; cpu < topology_info->num_cpus; cpu++) {
    ...
    kern_return_t result =
        ml_processor_register(&this_processor_info, &machProcessors[cpu_id],
                              &ipi_handler, &pmi_handler);
...
static unsigned int target_to_cpu_id(cpu_id_t in) {
...
PE_cpu_start_from_kext(cpu_id_t target, __unused vm_offset_t start_paddr,
...
void PE_cpu_start_internal(cpu_id_t target, __unused vm_offset_t start_paddr,
                           __unused vm_offset_t arg_paddr) {
  unsigned int cpu_id = target_to_cpu_id(target);

  assert(cpu_id !=
         cpu_number()); /* we can't be already on the CPU to be started */

#if APPLEVIRTUALPLATFORM
  /* When running virtualized, the reset vector address must be passed to PMGR
   * explicitly */
 ...
 * This is IOKit KPI, but not used by anyone today.
 */
void __abortlike PE_cpu_halt(cpu_id_t target) {
...
static bool is_cluster_powering_down(int cpu_id) {
  // Don't kill the cluster power if any other CPUs in this cluster are still
  // awake
  unsigned int target_cluster_id = topology_info->cpus[cpu_id].cluster_id;
  for (int i = 0; i < topology_info->num_cpus; i++) {
    if (topology_info->cpus[i].cluster_id == target_cluster_id && cpu_id != i &&
...
// Returns true if the platform code should go into deep sleep WFI, false
// otherwise.
bool PE_cpu_down(cpu_id_t target) {
  unsigned int cpu_id = target_to_cpu_id(target);
  if (ml_is_quiescing()) {
...
void PE_handle_ext_interrupt(void) { gCPUIC->externalInterrupt(); }
...
bool PE_cpu_power_check_kdp(int cpu_id) {
...
void IOCPUSleepKernel(void) {
   ...
   * target CPU.  Note that realtime threads have their own mechanism to
   * eventually demote their priority below MAXPRI_KERNEL if they hog the CPU
   * for too long.
   */
