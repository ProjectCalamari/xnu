...
#define SHOULD_COPY_DIR(op, direction)                                         \
  ((kIODirectionNone == (direction)) || (kWalkSyncAlways & (op)) ||            \
   (((kWalkSyncIn & (op)) ? kIODirectionIn : kIODirectionOut) & (direction)))
...
OSSharedPtr<IODMACommand> IODMACommand::withRefCon(void *refCon) {
...
OSSharedPtr<IODMACommand> IODMACommand::withSpecification(
    SegmentFunction outSegFunc, const SegmentOptions *segmentOptions,
    uint32_t mappingOptions, IOMapper *mapper, void *refCon) {
  OSSharedPtr<IODMACommand> me = OSMakeShared<IODMACommand>();

  if (me && !me->initWithSpecification(outSegFunc, segmentOptions,
                                       mappingOptions, mapper, refCon)) {
    return nullptr;
  }
...
OSSharedPtr<IODMACommand> IODMACommand::withSpecification(
    ...
    MappingOptions mappingOptions, UInt64 maxTransferSize, UInt32 alignment,
    IOMapper *mapper, void *refCon) {
  OSSharedPtr<IODMACommand> me = OSMakeShared<IODMACommand>();

  if (me && !me->initWithSpecification(
...
OSSharedPtr<IODMACommand> IODMACommand::cloneCommand(void *refCon) {
  ...
  return IODMACommand::withSpecification(
      fOutSeg, &segmentOptions, fMappingOptions, fMapper.get(), refCon);
}

#define kLastOutputFunction ((SegmentFunction)kLastOutputFunction)

bool IODMACommand::initWithRefCon(void *refCon) {
  if (!super::init()) {
    return false;
  }
...
bool IODMACommand::initWithSpecification(SegmentFunction outSegFunc,
  ...
  if (kIOReturnSuccess !=
      setSpecification(outSegFunc, segmentOptions, mappingOptions, mapper)) {
    return false;
  }
...
bool IODMACommand::initWithSpecification(
    SegmentFunction outSegFunc, UInt8 numAddressBits, UInt64 maxSegmentSize,
    MappingOptions mappingOptions, UInt64 maxTransferSize, UInt32 alignment,
    IOMapper *mapper, void *refCon) {
  SegmentOptions segmentOptions = {.fStructSize = sizeof(segmentOptions),
  ...
  return initWithSpecification(outSegFunc, &segmentOptions, mappingOptions,
                               mapper, refCon);
}

IOReturn IODMACommand::setSpecification(SegmentFunction outSegFunc,
                                        const SegmentOptions *segmentOptions,
                                        uint32_t mappingOptions,
                                        IOMapper *mapper) {
  ...
  is32Bit = ((OutputHost32 == outSegFunc) || (OutputBig32 == outSegFunc) ||
...
IOReturn IODMACommand::setMemoryDescriptor(const IOMemoryDescriptor *mem,
                                           bool autoPrepare) {
  ...
  if (mem) {
    ...
    err = mem->dmaCommandOperation(kIOMDGetCharacteristics |
                                       (kMapped == MAPTYPE(fMappingOptions)),
    ...
    ppnum_t highPage =
        fMDSummary.fHighestPage ? fMDSummary.fHighestPage : gIOLastPage;

    if ((kMapped == MAPTYPE(fMappingOptions)) && fMapper) {
      fInternalState->fCheckAddressing = false;
    } else {
      fInternalState->fCheckAddressing =
          (fNumAddressBits &&
           (highPage >= (1UL << (fNumAddressBits - PAGE_SHIFT))));
    }
...
IOReturn IODMACommand::clearMemoryDescriptor(bool autoComplete) {
...
const IOMemoryDescriptor *IODMACommand::getMemoryDescriptor() const {
  return fMemory.get();
}

IOMemoryDescriptor *IODMACommand::getIOMemoryDescriptor() const {
...
IOReturn IODMACommand::segmentOp(void *reference, IODMACommand *target,
  ...
  if (target->fNumAddressBits && (target->fNumAddressBits < 64) &&
      (state->fLocalMapperAllocValid || !target->fMapper)) {
  ...
  if (!state->fMisaligned) {
    mask = (segmentIndex ? target->fAlignMaskInternalSegments
      ...
      for (IOItemCount idx = 0; idx < numPages; idx++) {
        if (SHOULD_COPY_DIR(op, target->fMDSummary.fDirection)) {
          ...
          DEBG("cpv: 0x%qx %s 0x%qx, 0x%qx, 0x%02lx\n", remapAddr,
               (kWalkSyncIn & op) ? "->" : "<-", address, chunk, op);
...
IODMACommand::createCopyBuffer(IODirection direction, UInt64 length) {
  ...
  return IOBufferMemoryDescriptor::inTaskWithPhysicalMask(
      kernel_task, direction, length, mask);
}

IOReturn IODMACommand::walkAll(uint32_t op) {
  ...
  if (kWalkPreflight & op) {
    ...
    if (!(kWalkDoubleBuffer & op)) {
      offset = 0;
      numSegments = 0 - 1;
      ret = genIOVMSegments(op, segmentOp, (void *)(uintptr_t)op, &offset,
                            state, &numSegments);
    }
    ...
    if (state->fDoubleBuffer) {
      state->fCopyPageCount = (typeof(state->fCopyPageCount))(atop_64(
          round_page(state->fPreparedLength)));
    }
    ...
    if (state->fCopyPageCount) {
      ...
      if (!fMapper && !state->fDoubleBuffer) {
        ...
        kr = vm_page_alloc_list(state->fCopyPageCount,
                                (kma_flags_t)(KMA_LOMEM | KMA_NOPAGEWAIT),
                                &mapBase);
        if (KERN_SUCCESS != kr) {
      ...
      if (mapBase) {
        ...
        DEBG("alloc IOBMD\n");
        state->fCopyMD =
            createCopyBuffer(fMDSummary.fDirection, state->fPreparedLength);
    ...
    if (state->fCopyPageCount) {
      ...
      if (state->fCopyPageAlloc) {
        ...
        if (SHOULD_COPY_DIR(op, fMDSummary.fDirection)) {
          ...
          DEBG("fCopyMD %s %lx bytes\n", (kWalkSyncIn & op) ? "wrote" : "read",
               bytes);
          ret = (bytes == state->fPreparedLength) ? kIOReturnSuccess
                                                  : kIOReturnUnderrun;
        } else {
          ret = kIOReturnSuccess;
        }
...
UInt8 IODMACommand::getNumAddressBits(void) { return (UInt8)fNumAddressBits; }

UInt32 IODMACommand::getAlignment(void) { return fAlignMask + 1; }

uint32_t IODMACommand::getAlignmentLength(void) { return fAlignMaskLength + 1; }

uint32_t IODMACommand::getAlignmentInternalSegments(void) {
  return fAlignMaskInternalSegments + 1;
}

IOReturn IODMACommand::prepareWithSpecification(
    SegmentFunction outSegFunc, const SegmentOptions *segmentOptions,
    uint32_t mappingOptions, IOMapper *mapper, UInt64 offset, UInt64 length,
    bool flushCache, bool synchronize) {
  IOReturn ret;
...
IOReturn IODMACommand::prepareWithSpecification(
    SegmentFunction outSegFunc, UInt8 numAddressBits, UInt64 maxSegmentSize,
    MappingOptions mappingOptions, UInt64 maxTransferSize, UInt32 alignment,
    IOMapper *mapper, UInt64 offset, UInt64 length, bool flushCache,
    bool synchronize) {
  SegmentOptions segmentOptions = {.fStructSize = sizeof(segmentOptions),
                                   .fNumAddressBits = numAddressBits,
                                   .fMaxSegmentSize = maxSegmentSize,
  ...
  return prepareWithSpecification(outSegFunc, &segmentOptions, mappingOptions,
                                  mapper, offset, length, flushCache,
                                  synchronize);
}

IOReturn IODMACommand::prepare(UInt64 offset, UInt64 length, bool flushCache,
                               bool synchronize) {
  IODMACommandInternal *state = fInternalState;
  IOReturn ret = kIOReturnSuccess;
  ...
  if (fActive++) {
    ...
    state->fCursor =
        ...
        (!state->fCheckAddressing &&
         (!state->fSourceAlignMask ||
          ((fMDSummary.fPageAlign & (1 << 31)) &&
           (0 == (fMDSummary.fPageAlign & state->fSourceAlignMask)))));
    ...
    if (fMapper) {
      ...
      mapArgs.fMapSpec.numAddressBits =
          fNumAddressBits ? ((UInt8)fNumAddressBits) : 64;
      ...
      if ((kIOReturnSuccess == ret) && mapArgs.fAllocLength &&
          (mapArgs.fAllocLength != mapArgs.fLength)) {
        do {
          // multisegment case
          IOMDDMAWalkSegmentState walkState;
          IOMDDMAWalkSegmentArgs *walkArgs =
              (IOMDDMAWalkSegmentArgs *)(void *)&walkState;
          IOOptionBits mdOp;
          uint64_t index;
          ...
          for (insertOffset = 0, index = 0; index < length; segCount++) {
            ...
            if (LOGTAG == fMemory->getTag()) {
              IOLog(
                  "DMA[%p] runs[%d] dmaoff 0x%qx, mapoff 0x%qx, align 0x%qx\n",
                  this, segCount, index, insertOffset, align);
            }
#endif /* defined(LOGTAG) */

            assert(segCount < state->fMapSegmentsCount);
            state->fMapSegments[segCount].fDMAOffset =
                state->fPreparedOffset + index;
...
IOReturn IODMACommand::complete(bool invalidateCache, bool synchronize) {
  IODMACommandInternal *state = fInternalState;
  IOReturn ret = kIOReturnSuccess;
  ...
  if (!--fActive) {
    ...
    if (IS_NONCOHERENT(fMappingOptions) && invalidateCache) {
      if (copyMD) {
        copyMD->performOperation(kIOMemoryIncoherentIOFlush, 0,
                                 state->fPreparedLength);
      } else {
        OSSharedPtr<IOMemoryDescriptor> md = fMemory;
        md->performOperation(kIOMemoryIncoherentIOFlush, state->fPreparedOffset,
                             state->fPreparedLength);
      }
    }
    ...
    if (state->fLocalMapperAllocValid) {
      ...
      state->fLocalMapperAllocLength = 0;
      if (state->fMapSegments) {
        IODeleteData(state->fMapSegments, IODMACommandMapSegment,
                     state->fMapSegmentsCount);
...
IOReturn IODMACommand::getPreparedOffsetAndLength(UInt64 *offset,
                                                  UInt64 *length) {
  IODMACommandInternal *state = fInternalState;
  if (fActive < 1) {
    return kIOReturnNotReady;
...
IOReturn IODMACommand::synchronize(IOOptionBits options) {
  IODMACommandInternal *state = fInternalState;
  IOReturn ret = kIOReturnSuccess;
  IOOptionBits op;
  ...
  if (kForceDoubleBuffer & options) {
    ...
    ret = prepare(state->fPreparedOffset, state->fPreparedLength,
...
IOReturn IODMACommand::transferSegment(void *reference, IODMACommand *target,
                                       Segment64 segment, void *segments,
                                       UInt32 segmentIndex) {
  IODMACommandTransferContext *context =
      (IODMACommandTransferContext *)reference;
  ...
  while (length) {
    ...
    if ((kMapped == MAPTYPE(target->fMappingOptions)) && target->fMapper) {
    ...
    case kIODMACommandTransferOpReadBytes:
      copypv(cpuAddr, context->bufferOffset + (addr64_t)context->buffer,
             (unsigned int)copyLen,
             cppvPsrc | cppvNoRefSrc | cppvFsnk | cppvKmap);
      break;
    case kIODMACommandTransferOpWriteBytes:
      copypv(context->bufferOffset + (addr64_t)context->buffer, cpuAddr,
             (unsigned int)copyLen,
...
UInt64 IODMACommand::transfer(IOOptionBits transferOp, UInt64 offset,
  ...
  (void)genIOVMSegments(kWalkClient, transferSegment, &context, &offset,
                        &segments[0], &numSegments);

  return length - context.remaining;
}

UInt64 IODMACommand::readBytes(UInt64 offset, void *bytes, UInt64 length) {
  return transfer(kIODMACommandTransferOpReadBytes, offset, bytes, length);
}

UInt64 IODMACommand::writeBytes(UInt64 offset, const void *bytes,
                                UInt64 length) {
  return transfer(kIODMACommandTransferOpWriteBytes, offset,
                  const_cast<void *>(bytes), length);
}

IOReturn IODMACommand::genIOVMSegments(UInt64 *offsetP, void *segmentsP,
                                       UInt32 *numSegmentsP) {
  return genIOVMSegments(kWalkClient, clientOutputSegment, (void *)fOutSeg,
                         offsetP, segmentsP, numSegmentsP);
}

IOReturn IODMACommand::genIOVMSegments(uint32_t op,
                                       InternalSegmentFunction outSegFunc,
                                       void *reference, UInt64 *offsetP,
  ...
  IOMDDMAWalkSegmentArgs *state = (IOMDDMAWalkSegmentArgs *)(void *)fState;

  UInt64 offset = *offsetP + internalState->fPreparedOffset;
  UInt64 memLength =
      internalState->fPreparedOffset + internalState->fPreparedLength;

  if (offset >= memLength) {
    return kIOReturnOverrun;
  }

  if ((offset == internalState->fPreparedOffset) ||
      (offset != state->fOffset) || internalState->fNewMD) {
    state->fOffset = 0;
  ...
  while (internalState->fIOVMAddrValid || (state->fOffset < memLength)) {
    ...
    if (!internalState->fIOVMAddrValid) {
        ...
        if (!internalState->fMapSegmentsCount) {
          ...
          if (LOGTAG == fMemory->getTag()) {
            IOLog("DMA[%p] offsets 0x%qx, 0x%qx, 0x%qx ind %qd\n", this, offset,
                  internalState->fPreparedOffset, internalState->fNextOffset,
                  ind);
          }
#endif /* defined(LOGTAG) */

          // Scan through iopl info blocks looking for block containing offset
          while (ind < internalState->fMapSegmentsCount &&
                 offset >= internalState->fMapSegments[ind].fDMAOffset) {
            ind++;
          }
          ...
          address = internalState->fLocalMapperAllocBase +
                    internalState->fMapSegments[ind].fMapOffset + runOffset;
#if defined(LOGTAG)
          if (LOGTAG == fMemory->getTag()) {
      ...
      if (!done) {
        IOMemoryDescriptor *memory = internalState->fCopyMD
                                         ? internalState->fCopyMD.get()
                                         : fMemory.get();
      ...
      if (rtn == kIOReturnSuccess) {
        ...
        if (curSegValid &&
            ((curSeg.fIOVMAddr + curSeg.fLength) == state->fIOVMAddr)) {
        ...
        internalState->fIOVMAddrValid = state->fIOVMAddr = state->fLength =
            0; // At end
      } else {
        return rtn;
      }
    ...
    if (!internalState->fIOVMAddrValid) {
      // maxPhys
      if ((kWalkClient & op) &&
          (curSeg.fIOVMAddr + curSeg.fLength - 1) > maxPhys) {
        if (internalState->fCursor) {
          ...
          curSeg.fIOVMAddr =
              ptoa_64(vm_page_get_phys_page(remap)) + (addr & PAGE_MASK);
          curSegValid = true;
      ...
      if (leftover) {
        DEBG("reduce seg by 0x%llx @ 0x%llx [0x%llx, 0x%llx]\n", leftover,
             offset, curSeg.fIOVMAddr, curSeg.fLength);
      ...
      if (internalState->fCursor) {
        ...
        mask = (segIndex ? fAlignMaskInternalSegments
                         : internalState->fSourceAlignMask);
...
IOReturn IODMACommand::clientOutputSegment(void *reference,
                                           IODMACommand *target,
...
IOReturn IODMACommand::genIOVMSegments(SegmentFunction segmentFunction,
                                       UInt64 *offsetP, void *segmentsP,
                                       UInt32 *numSegmentsP) {
  return genIOVMSegments(kWalkClient, clientOutputSegment,
                         (void *)segmentFunction, offsetP, segmentsP,
                         numSegmentsP);
}

bool IODMACommand::OutputHost32(IODMACommand *, Segment64 segment,
                                void *vSegList, UInt32 outSegIndex) {
...
bool IODMACommand::OutputBig32(IODMACommand *, Segment64 segment,
                               void *vSegList, UInt32 outSegIndex) {
...
bool IODMACommand::OutputLittle32(IODMACommand *, Segment64 segment,
                                  void *vSegList, UInt32 outSegIndex) {
  const UInt offAddr = outSegIndex * sizeof(Segment32);
  const UInt offLen = offAddr + sizeof(UInt32);
...
bool IODMACommand::OutputHost64(IODMACommand *, Segment64 segment,
                                void *vSegList, UInt32 outSegIndex) {
...
bool IODMACommand::OutputBig64(IODMACommand *, Segment64 segment,
                               void *vSegList, UInt32 outSegIndex) {
...
bool IODMACommand::OutputLittle64(IODMACommand *, Segment64 segment,
                                  void *vSegList, UInt32 outSegIndex) {
  const UInt offAddr = outSegIndex * sizeof(Segment64);
  const UInt offLen = offAddr + sizeof(UInt64);
