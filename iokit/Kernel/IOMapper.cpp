...
OSSharedPtr<IOMapper> IOMapper::copyMapperForDevice(IOService *device) {
...
IOMapper::copyMapperForDeviceWithIndex(IOService *device, unsigned int index) {
  ...
  if (matching) {
    mapper = OSDynamicPtrCast<IOMapper>(
        IOService::waitForMatchingService(matching.get()));
  }
...
ppnum_t IOMapperIOVMAlloc(unsigned pages) {
  ...
  if (IOMapper::gSystem) {
    ret = IOMapper::gSystem->iovmMapMemory(
        NULL, 0, ptoa_64(pages), (kIODMAMapReadAccess | kIODMAMapWriteAccess),
        NULL, NULL, NULL, &dmaAddress, &dmaLength);
  }
...
void IOMapperIOVMFree(ppnum_t addr, unsigned pages) {
  if (IOMapper::gSystem) {
    IOMapper::gSystem->iovmUnmapMemory(NULL, NULL, ptoa_64(addr),
                                       ptoa_64(pages));
  }
}
...
ppnum_t IOMapperInsertPage(ppnum_t addr, unsigned offset, ppnum_t page) {
  ...
  IOMapper::gSystem->iovmInsert((kIODMAMapReadAccess | kIODMAMapWriteAccess),
                                ptoa_64(addr), ptoa_64(offset), ptoa_64(page),
                                ptoa_64(1));
  return addr + offset;
}
