...
void IOCatalogue::initialize(void) {
  ...
  array = OSDynamicPtrCast<OSArray>(
  ...
  assert(array && gIOClassKey && gIOProbeScoreKey && gIOModuleIdentifierKey);
...
void IOCatalogue::addPersonality(OSDictionary *dict) {
  ...
  if (arr) {
    ...
    OSSharedPtr<OSArray> sharedArr =
        OSArray::withObjects((const OSObject **)&dict, 1, 2);
 ...
 *********************************************************************/
void IOCatalogue::free(void) { panic(""); }

/*********************************************************************
*********************************************************************/
OSPtr<OSOrderedSet> IOCatalogue::findDrivers(IOService *service,
  ...
  while (meta) {
    ...
    if (array) {
      for (idx = 0; (nextTable = (OSDictionary *)array->getObject(idx));
           idx++) {
 ...
 *********************************************************************/
OSPtr<OSOrderedSet> IOCatalogue::findDrivers(OSDictionary *matching,
                                             SInt32 *generationCount) {
...
bool IOCatalogue::exchangeDrivers(OSDictionary *matchingForRemove,
  ...
  iter_all_personalities =
      OSCollectionIterator::withCollection(personalities.get());
   ...
   * We get a dictionary that has only some keys that could belong to a
   * personality. Every personality that will match those keys will be removed.
   */
  const OSSymbol *key;
...
bool IOCatalogue::addDrivers(OSArray *drivers, bool doNubMatching) {
  bool result = false;
  OSSharedPtr<OSOrderedSet> set;
  OSSharedPtr<OSCollectionIterator> iter;
...
bool IOCatalogue::removeDrivers(
    bool doNubMatching, bool (^shouldRemove)(OSDictionary *personality)) {
  OSSharedPtr<OSOrderedSet> set;
  OSSharedPtr<OSCollectionIterator> iter;
  OSDictionary *dict;
  OSArray *array;
 ...
 *********************************************************************/
bool IOCatalogue::removeDrivers(OSDictionary *matching, bool doNubMatching) {
  if (!matching) {
    return false;
  }
...
SInt32 IOCatalogue::getGenerationCount(void) const { return generation; }
/*********************************************************************
*********************************************************************/
/* static */

bool IOCatalogue::personalityIsBoot(OSDictionary *match) {
  OSString *moduleName;
  OSSharedPtr<OSKext> theKext;

  moduleName =
      OSDynamicCast(OSString, match->getObject(gIOModuleIdentifierKey.get()));
  if (!moduleName) {
    return true;
  }
...
bool IOCatalogue::isModuleLoaded(OSDictionary *driver,
                                 OSObject **kextRef) const {
   ...
   */
  publisherName =
      OSDynamicCast(OSString, driver->getObject(kIOPersonalityPublisherKey));
  OSKext::recordIdentifierRequest(publisherName);

  moduleName = OSDynamicCast(
      OSString, driver->getObject(gIOModuleIdentifierKernelKey.get()));
  if (moduleName) {
    ...
    OSString *moduleDextName = OSDynamicCast(
        OSString, driver->getObject(gIOModuleIdentifierKey.get()));
  ...
  /* If a personality doesn't hold the "CFBundleIdentifier" or
   * "CFBundleIdentifierKernel" key it is assumed to be an "in-kernel" driver.
   */
  return true;
}
 ...
 */
void IOCatalogue::moduleHasLoaded(const OSSymbol *moduleName) {
...
IOReturn IOCatalogue::terminateDrivers(OSDictionary *matching,
                                       io_name_t className, bool asynchronous) {
  OSDictionary *dict;
  ...
  do {
    ...
    while ((service = (IOService *)iter->getNextObject())) {
      ...
      for (okToTerminate = true;;) {
        if (isDext) {
          dextBundleID = OSDynamicPtrCast<OSString>(
              service->copyProperty(gIOModuleIdentifierKey.get()));
          if (!dextBundleID) {
            break;
          }
...
IOReturn IOCatalogue::terminateDriversForUserspaceReboot() {
  IOReturn ret = kIOReturnSuccess;
  ...
  do {
    ...
    while ((service = (IOService *)iter->getNextObject())) {
      isDeferredMatch =
          service->propertyHasValue(gIOMatchDeferKey, kOSBooleanTrue);
      isDext = service->hasUserServer();
      if (isDeferredMatch || isDext) {
        OSSharedPtr<OSObject> prop = service->copyProperty(
            gIOUserServerPreserveUserspaceRebootKey, gIOServicePlane,
        ...
        if (isDext) {
          OSSharedPtr<OSString> name = OSDynamicPtrCast<OSString>(
              service->copyProperty(gIOUserServerNameKey));
          ...
          IOLog("terminating service %s-0x%llx [dext %s]\n", service->getName(),
                ...
                userServerName ? userServerName : "(null)");
        } else {
          ...
          IOLog("terminating service %s-0x%llx [kext %s]\n", service->getName(),
                service->getRegistryEntryID(), bundleID ? bundleID : "(null)");
        }
...
IOReturn IOCatalogue::resetAfterUserspaceReboot(void) {
  OSSharedPtr<OSIterator> iter;
  IOService *service;
...
IOReturn IOCatalogue::terminateDriversForModule(OSString *moduleName,
                                                bool unload,
                                                bool asynchronous) {
  IOReturn ret;
  ...
  if (isDext) {
    /* Force rematching after removing personalities. Dexts are never considered
     * to be "loaded" (from OSKext), so we can't call unloadModule() to remove
     * personalities and start rematching. */
...
IOReturn IOCatalogue::terminateDriversForModule(const char *moduleName,
                                                bool unload,
                                                bool asynchronous) {
...
bool IOCatalogue::startMatching(const OSSymbol *moduleName) {
  OSSharedPtr<OSOrderedSet> set;
  OSSharedPtr<OSKext> kext;
  ...
  if (kext && kext->isDriverKit()) {
    /* We're here because kernelmanagerd called IOCatalogueModuleLoaded after
     * launching a dext. Determine what providers the dext would match against.
     * If there's something already attached to the provider, terminate it.
     *
    ...
    dextPersonalities->iterateObjects(^bool(OSObject *obj) {
      ...
      if (personality) {
        category = OSDynamicCast(OSSymbol,
                                 personality->getObject(gIOMatchCategoryKey));
        if (!category) {
        ...
        while (iter &&
               (provider = OSDynamicCast(IOService, iter->getNextObject()))) {
          if (provider->metaCast(gIOHIDInterfaceClassName.get()) != NULL) {
            service.reset(provider->copyClientWithCategory(category),
                          OSNoRetain);
            if (service) {
  ...
  personalities->iterateObjects(^bool(const OSSymbol *key, OSObject *value) {
    ...
    for (idx = 0; (dict = (OSDictionary *)array->getObject(idx)); idx++) {
      moduleIdentifierKernel =
          dict->getObject(gIOModuleIdentifierKernelKey.get());
      moduleIdentifier = dict->getObject(gIOModuleIdentifierKey.get());
      if ((moduleIdentifierKernel &&
           moduleName->isEqualTo(moduleIdentifierKernel)) ||
  ...
  if (servicesToTerminate) {
    servicesToTerminate->iterateObjects(^bool(OSObject *obj) {
      ...
      if (service) {
        ...
        if (!service->terminate(terminateOptions)) {
          IOLog("%s: failed to terminate service %s-0x%qx with options %08llx "
                "for new dext %s\n",
                __FUNCTION__, service->getName(), service->getRegistryEntryID(),
                (long long)terminateOptions, moduleName->getCStringNoCopy());
        }
      }
      return false;
    });
...
bool IOCatalogue::resetAndAddDrivers(OSArray *drivers, bool doNubMatching) {
  ...
  while ((key = (const OSSymbol *)iter->getNextObject())) {
    ...
    for (idx = 0; (thisOldPersonality = (OSDictionary *)array->getObject(idx));
         idx++) {
      ...
      if (newPersonalities) {
        for (newIdx = 0;
             (thisNewPersonality =
                  (OSDictionary *)newPersonalities->getObject(newIdx));
             newIdx++) {
          /* Unlike in other functions, this comparison must be exact!
           * The catalogue must be able to contain personalities that
           * are proper supersets of others.
      ...
      if (thisNewPersonality) {
        ...
        if (isModuleLoadedNoOSKextLock(myKexts.get(), thisOldPersonality) ==
            false) {
          if (matchSet) {
  ...
  // add new
  if (newPersonalities) {
    for (newIdx = 0; (thisNewPersonality =
                          (OSDictionary *)newPersonalities->getObject(newIdx));
         newIdx++) {
      if (OSDynamicCast(OSDictionary, thisNewPersonality) == NULL) {
        /* skip thisNewPersonality if it is not an OSDictionary */
...
static bool isModuleLoadedNoOSKextLock(OSDictionary *theKexts,
  ...
  myBundleID = OSDynamicCast(
      OSString, theModuleDict->getObject(gIOModuleIdentifierKey.get()));
  if (myBundleID == NULL) {
    return myResult;
  }

  myKext = OSDynamicCast(OSKext,
                         theKexts->getObject(myBundleID->getCStringNoCopy()));
  if (myKext) {
