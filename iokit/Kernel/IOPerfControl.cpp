...
static IOPerfControlClient::IOPerfControlClientShared
    *_Atomic gIOPerfControlClientShared;

bool IOPerfControlClient::init(IOService *driver, uint64_t maxWorkCapacity) {
  // TODO: Remove this limit and implement dynamic table growth if workloads are
  // found that exceed this
  ...
  shared =
      atomic_load_explicit(&gIOPerfControlClientShared, memory_order_acquire);
  if (shared == nullptr) {
    ...
    shared->interface = PerfControllerInterface{
        ...
        .registerDevice = [](IOService *device) { return kIOReturnSuccess; },
        .unregisterDevice = [](IOService *device) { return kIOReturnSuccess; },
        .workCanSubmit = [](IOService *device,
                            PerfControllerInterface::WorkState *state,
                            WorkSubmitArgs *args) { return false; },
        .workSubmit = [](IOService *device, uint64_t token,
                         PerfControllerInterface::WorkState *state,
                         WorkSubmitArgs *args) {},
        .workBegin = [](IOService *device, uint64_t token,
                        PerfControllerInterface::WorkState *state,
                        WorkBeginArgs *args) {},
        .workEnd = [](IOService *device, uint64_t token,
                      PerfControllerInterface::WorkState *state,
                      WorkEndArgs *args, bool done) {},
        .workUpdate = [](IOService *device, uint64_t token,
                         PerfControllerInterface::WorkState *state,
                         WorkUpdateArgs *args) {},
    };
    ...
    if (!atomic_compare_exchange_strong_explicit(
            &gIOPerfControlClientShared, &expected, shared,
  ...
  // Note: driverIndex is not guaranteed to be unique if maxDriverIndex wraps
  // around. It is intended for debugging only.
  driverIndex = atomic_fetch_add_explicit(&shared->maxDriverIndex, 1,
                                          memory_order_relaxed) +
                1;

  // + 1 since index 0 is unused for kIOPerfControlClientWorkUntracked
  workTableLength = maxWorkCapacity + 1;
  assertf(workTableLength <= kWorkTableMaxSize,
...
void IOPerfControlClient::setDeviceType(IOPCDeviceType newDeviceType) {
...
IOPerfControlClient *IOPerfControlClient::copyClientForDeviceType(
    IOService *driver, uint64_t maxWorkCapacity, IOPCDeviceType deviceType) {
...
IOPerfControlClient *IOPerfControlClient::copyClient(IOService *driver,
                                                     uint64_t maxWorkCapacity) {
  return copyClientForDeviceType(driver, maxWorkCapacity,
                                 IOPCDeviceTypeUnknown);
}
 ...
 * drivers.
 */
inline uint64_t IOPerfControlClient::tokenToGlobalUniqueToken(uint64_t token) {
 ...
 */
void IOPerfControlClient::accountResources(
    coalition_t coal, PerfControllerInterface::PerfDeviceID device_type,
    PerfControllerInterface::ResourceAccounting *resources) {
  switch (device_type) {
  case PerfControllerInterface::PerfDeviceID::kANE:
    if (coal != nullptr) {
      coalition_update_ane_stats(coal, resources->mach_time_delta,
                                 resources->energy_nj_delta);
    }
    break;

  default:
    assertf(false,
            "Unexpected device type for IOPerfControlClient::accountResources: "
            "%llu",
...
uint64_t IOPerfControlClient::allocateToken(thread_group *thread_group) {
  ...
  if (token == kIOPerfControlClientWorkUntracked) {
    /* When investigating a panic here, first check that the driver is not
     * leaking tokens. If the driver is not leaking tokens and maximum is less
     * than kMaxWorkTableNumEntries, the driver should be modified to pass a
     * larger value to copyClient. If the driver is not leaking tokens and
     * maximum is equal to kMaxWorkTableNumEntries, this code will have to be
     * modified to support dynamic table growth to support larger numbers of
     * tokens.
     */
...
void IOPerfControlClient::deallocateToken(uint64_t token) {
#if CONFIG_THREAD_GROUPS
  assertf(token != kIOPerfControlClientWorkUntracked,
          "Attempt to deallocate token kIOPerfControlClientWorkUntracked\n");
  assertf(token <= workTableLength,
          "Attempt to deallocate token %llu which is greater than the table "
          "size of %zu\n",
  ...
  // This can call into the performance controller if the last reference is
  // dropped here. Are we sure the driver isn't holding any locks? If not, we
  // may want to async this to another context.
...
static struct thread_group *threadGroupForDextService(IOService *device) {
  assert(device);
...
IOReturn IOPerfControlClient::registerDevice(IOService *driver,
                                             IOService *device) {
  ...
  if (device) {
    ...
    if (dext_thread_group) {
      ...
      clientData.driverState.target_thread_group_id =
          thread_group_get_id(dext_thread_group);
      clientData.driverState.target_thread_group_data =
          thread_group_get_machine_data(dext_thread_group);
  ...
  if (shared->interface.version >= PERFCONTROL_INTERFACE_VERSION_3) {
    ret = shared->interface.registerDriverDevice(driver, device,
                                                 &clientData.driverState);
...
void IOPerfControlClient::unregisterDevice(IOService *driver,
                                           IOService *device) {
#if CONFIG_THREAD_GROUPS
  IOLockLock(shared->interfaceLock);

  if (shared->interface.version >= PERFCONTROL_INTERFACE_VERSION_3) {
    shared->interface.unregisterDriverDevice(driver, device,
                                             &clientData.driverState);
  } else if (shared->interface.version >= PERFCONTROL_INTERFACE_VERSION_1) {
...
uint64_t IOPerfControlClient::workSubmit(IOService *device,
                                         WorkSubmitArgs *args) {
#if CONFIG_THREAD_GROUPS
  ...
  if (token != kIOPerfControlClientWorkUntracked) {
    ...
    shared->interface.workSubmit(device, tokenToGlobalUniqueToken(token),
                                 &state, args);
  }
  return token;
#else
...
uint64_t IOPerfControlClient::workSubmitAndBegin(IOService *device,
                                                 WorkSubmitArgs *submitArgs,
                                                 WorkBeginArgs *beginArgs) {
#if CONFIG_THREAD_GROUPS
  auto *thread_group = thread_group_get(current_thread());
  if (!thread_group) {
    return kIOPerfControlClientWorkUntracked;
  ...
  if (token != kIOPerfControlClientWorkUntracked) {
    ...
    shared->interface.workSubmit(device, tokenToGlobalUniqueToken(token),
                                 &state, submitArgs);
    state.started = true;
    shared->interface.workBegin(device, tokenToGlobalUniqueToken(token), &state,
                                beginArgs);
...
void IOPerfControlClient::workBegin(IOService *device, uint64_t token,
                                    WorkBeginArgs *args) {
#if CONFIG_THREAD_GROUPS
...
void IOPerfControlClient::workEnd(IOService *device, uint64_t token,
                                  WorkEndArgs *args, bool done) {
#if CONFIG_THREAD_GROUPS
  WorkTableEntry *entry = getEntryForToken(token);
  if (entry == nullptr) {
  ...
  if (shared->interface.version >= PERFCONTROL_INTERFACE_VERSION_4) {
    PerfControllerInterface::ResourceAccounting resources;
    shared->interface.workEndWithResources(device,
                                           tokenToGlobalUniqueToken(token),
                                           &state, args, &resources, done);
    if (clientData.driverState.resource_accounting) {
      accountResources(workTable[token].coal,
                       clientData.driverState.device_type, &resources);
    }
  } else {
    shared->interface.workEnd(device, tokenToGlobalUniqueToken(token), &state,
                              args, done);
  }
...
bool IOPerfControlWorkContext::init() {
  if (!super::init()) {
    return false;
  }
  id = atomic_fetch_add_explicit(&unique_work_context_id, 1,
                                 memory_order_relaxed) +
       1;
  reset();
  return true;
...
void IOPerfControlWorkContext::reset() {
...
void IOPerfControlWorkContext::free() {
  assertf(thread_group == nullptr,
          "IOPerfControlWorkContext ID %llu being released without calling "
          "workEnd!\n",
          id);
  assertf(coal == nullptr,
          "IOPerfControlWorkContext ID %llu being released without calling "
          "workEnd!\n",
          id);
  super::free();
...
OSObject *IOPerfControlClient::copyWorkContext() {
...
bool IOPerfControlClient::workSubmitAndBeginWithContext(
    IOService *device, OSObject *context, WorkSubmitArgs *submitArgs,
    WorkBeginArgs *beginArgs) {
#if CONFIG_THREAD_GROUPS
  ...
  IOPerfControlWorkContext *work_context =
      OSDynamicCast(IOPerfControlWorkContext, context);

  PerfControllerInterface::WorkState state{
      .thread_group_id = thread_group_get_id(work_context->thread_group),
      .thread_group_data =
          thread_group_get_machine_data(work_context->thread_group),
...
bool IOPerfControlClient::workSubmitWithContext(IOService *device,
                                                OSObject *context,
                                                WorkSubmitArgs *args) {
#if CONFIG_THREAD_GROUPS
  IOPerfControlWorkContext *work_context =
  ...
  assertf(!work_context->started,
          "IOPerfControlWorkContext ID %llu was already started",
          work_context->id);
  assertf(work_context->thread_group == nullptr,
          "IOPerfControlWorkContext ID %llu has already taken a refcount on TG "
          "0x%p \n",
...
void IOPerfControlClient::workUpdateWithContext(IOService *device,
                                                OSObject *context,
                                                WorkUpdateArgs *args) {
#if CONFIG_THREAD_GROUPS
  IOPerfControlWorkContext *work_context =
...
void IOPerfControlClient::workBeginWithContext(IOService *device,
                                               OSObject *context,
                                               WorkBeginArgs *args) {
#if CONFIG_THREAD_GROUPS
  IOPerfControlWorkContext *work_context =
  ...
  assertf(!work_context->started,
          "IOPerfControlWorkContext %llu was already started",
          work_context->id);
...
void IOPerfControlClient::workEndWithContext(IOService *device,
                                             OSObject *context,
                                             WorkEndArgs *args, bool done) {
#if CONFIG_THREAD_GROUPS
  IOPerfControlWorkContext *work_context =
  ...
  if (shared->interface.version >= PERFCONTROL_INTERFACE_VERSION_4) {
    PerfControllerInterface::ResourceAccounting resources;
    shared->interface.workEndWithResources(device, work_context->id, &state,
                                           args, &resources, done);
    if (clientData.driverState.resource_accounting) {
      accountResources(work_context->coal, clientData.driverState.device_type,
                       &resources);
    }
  } else {
...
IOReturn IOPerfControlClient::querySubmitterRole(IOService *device,
                                                 task_t clientTask,
                                                 uint32_t *role_out) {
...
IOReturn IOPerfControlClient::registerPerformanceController(
    PerfControllerInterface *pci) {
  ...
  if (shared->interface.version == PERFCONTROL_INTERFACE_VERSION_NONE) {
    ...
    if (pci->version >= PERFCONTROL_INTERFACE_VERSION_1) {
      assert(pci->registerDevice && pci->unregisterDevice &&
             pci->workCanSubmit && pci->workSubmit && pci->workBegin &&
             pci->workEnd);
    ...
    while ((obj = shared->deviceRegistrationList->getAnyObject())) {
      ...
      if (clientData && clientData->device) {
        if (pci->version >= PERFCONTROL_INTERFACE_VERSION_3) {
          pci->registerDriverDevice(clientData->device->getProvider(),
                                    clientData->device,
                                    &(clientData->driverState));
