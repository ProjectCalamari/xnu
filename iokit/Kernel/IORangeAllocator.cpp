...
#define LOCK()                                                                 \
  if (options & kLocking)                                                      \
  lck_mtx_lock(&gIORangeAllocatorLock)
#define UNLOCK()                                                               \
  if (options & kLocking)                                                      \
  lck_mtx_unlock(&gIORangeAllocatorLock)

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

bool IORangeAllocator::init(IORangeScalar endOfRange,
                            IORangeScalar _defaultAlignment, UInt32 _capacity,
...
IORangeAllocator *IORangeAllocator::withRange(IORangeScalar endOfRange,
  ...
  if (thingy &&
      !thingy->init(endOfRange, defaultAlignment, capacity, options)) {
    thingy->release();
    thingy = NULL;
  }
...
UInt32 IORangeAllocator::getFragmentCount(void) { return numElements; }

UInt32 IORangeAllocator::getFragmentCapacity(void) { return capacity; }
...
bool IORangeAllocator::allocElement(UInt32 index) {
  ...
  if (((numElements == capacity) && capacityIncrement) || (!elements)) {
    ...
    if (elements) {
      bcopy(elements, newElements, index * sizeof(IORangeAllocatorElement));
      bcopy(elements + index, newElements + index + 1,
    ...
    bcopy(elements + index, elements + index + 1,
          (numElements - index) * sizeof(IORangeAllocatorElement));
  }
  numElements++;
...
bool IORangeAllocator::allocateRange(IORangeScalar data, IORangeScalar size) {
  IORangeScalar thisStart, thisEnd;
  ...
  for (index = 0; (!found) && (index < numElements); index++) {
    thisStart = elements[index].start;
    thisEnd = elements[index].end;
...
void IORangeAllocator::deallocate(IORangeScalar data, IORangeScalar size) {
  IORangeScalar dataEnd;
  UInt32 index;
  ...
  if (headContig) {
    if (tailContig) {
      ...
      if (dataEnd > elements[index - 1].end) {
