...
static bool IODTMapInterruptsSharing(IORegistryEntry *regEntry,
...
IORegistryEntry *IODeviceTreeAlloc(void *dtTop) {
  ...
  gIODTAssociatedServiceKey = OSData::withBytesNoCopy(
      (void *)kIODTAssociatedServiceKey, sizeof(kIODTAssociatedServiceKey));

  assert(gIODTPlane && gIODTCompatibleKey && gIODTTypeKey && gIODTModelKey &&
         gIODTSizeCellKey && gIODTAddressCellKey && gIODTRangeKey &&
         gIODTPersistKey);

  gIODTDefaultInterruptController =
      OSSymbol::withCStringNoCopy("IOPrimaryInterruptController");
  gIODTNWInterruptMappingKey = OSSymbol::withCStringNoCopy("IONWInterrupts");

  gIODTAAPLInterruptsKey = OSSymbol::withCStringNoCopy("AAPL,interrupts");
  gIODTPHandleKey = OSSymbol::withCStringNoCopy("AAPL,phandle");

  gIODTInterruptParentKey = OSSymbol::withCStringNoCopy("interrupt-parent");
  ...
  gIODTInterruptCellKey = OSSymbol::withCStringNoCopy("#interrupt-cells");

  assert(gIODTDefaultInterruptController && gIODTNWInterruptMappingKey &&
         gIODTAAPLInterruptsKey && gIODTPHandleKey && gIODTInterruptParentKey &&
         gIODTPHandles && gIODTPHandleMap && gIODTInterruptCellKey &&
         gIODTResolvers && gIODTResolvers->lock && gIODTResolvers->resolvers);

  foundDTNode =
      (kSuccess ==
       SecureDTLookupEntry(NULL, "/chosen/memory-map", &mapEntry)) &&
      (kSuccess == SecureDTGetProperty(mapEntry, "DeviceTree",
                                       (void const **)&dtMap, &propSize)) &&
  ...
  if (freeDT) {
    ...
    SecureDTInit(NULL, 0);
    IODTFreeLoaderInfo("DeviceTree", (void *)dtMap[0],
                       (int)round_page(dtMap[1]));
  }
  ...
  if (regIter) {
    while ((child = regIter->getNextObject())) {
      OSArray *array =
          OSDynamicCast(OSArray, child->getProperty(gIOInterruptSpecifiersKey));
...
static IORegistryEntry *MakeReferenceTable(DTEntry dtEntry, bool copy) {
  ...
  if (regEntry &&
    ...
    kernelOnly = (kSuccess == SecureDTGetProperty(dtEntry, "kernel-only", &prop,
                                                  &propSize));
...
FindPHandle(UInt32 phandle) {
  ...
  for (i = 0; (data = (OSData *)gIODTPHandles->getObject(i)); i++) {
    if (phandle == *((UInt32 *)data->getBytesNoCopy())) {
      regEntry = (IORegistryEntry *)gIODTPHandleMap->getObject(i);
      break;
    }
  }
...
static bool GetUInt32(IORegistryEntry *regEntry, const OSSymbol *name,
  ...
  result = ((data = OSDynamicCast(OSData, obj)) &&
            (sizeof(UInt32) == data->getLength()));
  if (result) {
...
static IORegistryEntry *IODTFindInterruptParent(IORegistryEntry *regEntry,
                                                IOItemCount index) {
  IORegistryEntry *parent;
  UInt32 phandle;
  OSData *data;
  ...
  if ((data = OSDynamicCast(OSData,
                            regEntry->getProperty(gIODTInterruptParentKey))) &&
...
#define unexpected(a)                                                          \
  {                                                                            \
    kprintf("unexpected %s:%d\n", __FILE__, __LINE__);                         \
    a;                                                                         \
  }

static void IODTGetICellCounts(IORegistryEntry *regEntry, UInt32 *iCellCount,
                               UInt32 *aCellCount) {
...
IODTMapOneInterrupt(IORegistryEntry *regEntry, UInt32 *intSpec, UInt32 index,
  ...
  do {
    ...
    if (parent && (data = OSDynamicCast(OSData, regEntry->getProperty(
                                                    "interrupt-controller")))) {
      // found a controller - don't want to follow cascaded controllers
      parent = NULL;
      *spec = OSData::withBytesNoCopy((void *)intSpec, icells * sizeof(UInt32));
      *controller = IODTInterruptControllerName(regEntry);
      ok = (*spec && *controller);
    } else if (parent && (data = OSDynamicCast(OSData, regEntry->getProperty(
                                                           "interrupt-map")))) {
      // interrupt-map
      ...
      if (maskCmp) {
        ...
        for (i = 0; i < acells + icells; i++) {
          ...
          kprintf("0x%08X ", ((i < acells) ? addrCmp[i] : intSpec[i - acells]) &
                                 maskCmp[i]);
        }
        kprintf("\n");
      } else {
...
IOReturn IODTGetInterruptOptions(IORegistryEntry *regEntry, int source,
                                 IOOptionBits *options) {
  OSArray *controllers;
  OSArray *specifiers;
  OSArray *shared;
  ...
  *options = 0;
  ...
  controllers =
      OSDynamicCast(OSArray, regEntry->getProperty(gIOInterruptControllersKey));
  specifiers =
      OSDynamicCast(OSArray, regEntry->getProperty(gIOInterruptSpecifiersKey));
  ...
  for (unsigned int i = 0;
       (oneSpec = shared->getObject(i)) && (!oneSpec->isEqualTo(spec)); i++) {
  }
...
static bool IODTMapInterruptsSharing(IORegistryEntry *regEntry,
  ...
  nw = (NULL == (local = OSDynamicCast(
                     OSData, regEntry->getProperty(gIODTAAPLInterruptsKey))));
  if (nw && (NULL == (local = OSDynamicCast(
                          OSData, regEntry->getProperty("interrupts"))))) {
  ...
  if (ok) {
    do {
      if (nw) {
        skip =
            IODTMapOneInterrupt(regEntry, localBits, index, &map, &controller);
        if (0 == skip) {
      ...
      if (allInts) {
        ...
        if (controllerInts) {
          for (unsigned int i = 0; (oneMap = controllerInts->getObject(i));
               i++) {
            if (map->isEqualTo(oneMap)) {
              controllerInts =
                  (OSArray *)gIODTSharedInterrupts->getObject(controller);
              if (controllerInts) {
                controllerInts->setObject(map);
              } else {
                controllerInts =
                    OSArray::withObjects((const OSObject **)&map, 1, 4);
                if (controllerInts) {
...
static bool CompareKey(OSString *key, const IORegistryEntry *table,
  ...
  if (names) {
    ...
    do {
      ...
      if (wild) {
        matched =
            ((nlen >= (keyLen - 1)) && (0 == strncmp(ckey, names, keyLen - 1)));
      } else {
...
bool IODTCompareNubName(const IORegistryEntry *regEntry, OSString *name,
...
bool IODTMatchNubWithKeys(IORegistryEntry *regEntry, const char *keys) {
  OSObject *obj;
  bool result = false;
...
IODTFindMatchingEntries(IORegistryEntry *from, IOOptionBits options,
                        const char *keys) {
  OSSet *result = NULL;
...
void IODTGetCellCounts(IORegistryEntry *regEntry, UInt32 *sizeCount,
                       UInt32 *addressCount) {
...
bool IODTResolveAddressCell(IORegistryEntry *startEntry, UInt32 cellsIn[],
  ...
  if (childCells > sizeof(cell) / sizeof(cell[0])) {
    panic("IODTResolveAddressCell: Invalid device tree (%u,%u)",
          (uint32_t)childAddressCells, (uint32_t)childSizeCells);
  }
    ...
    if ((propLen = prop->getLength())) {
      ...
      for (ok = false; range < endRanges;
        ...
        if (childAddressCells > sizeof(endCell) / sizeof(endCell[0])) {
          panic("IODTResolveAddressCell: Invalid device tree (%u)",
                (uint32_t)childAddressCells);
        }

        bcopy(range, endCell, childAddressCells * sizeof(UInt32));

        rangeLen = CellsValue(childSizeCells,
                              range + childAddressCells + addressCells);
        AddLengthToCells(childAddressCells, endCell, rangeLen);

        diff2 = (*compare)(childAddressCells, cell, endCell);

        // if start of cell < start of range, or end of range >= start of cell,
        // skip
        if ((diff < 0) || (diff2 >= 0)) {
          continue;
        }
        ...
        if (!ok) {
          ...
          for (lookRange = startRange; lookRange < endRanges;
            ...
            UInt64 rangeStart =
                CellsValue(addressCells, range + childAddressCells);
            UInt64 lookRangeStart =
                CellsValue(addressCells, lookRange + childAddressCells);
      ...
      if (addressCells + sizeCells > sizeof(cell) / sizeof(cell[0])) {
        panic("IODTResolveAddressCell: Invalid device tree (%u, %u)",
              (uint32_t)addressCells, (uint32_t)sizeCells);
      }
  ...
  do {
    ...
    for (i = 0; i < num; i++) {
      if (IODTResolveAddressCell(parentEntry, reg, &phys, &len)) {
        ...
        range = NULL;
        if (parent) {
          range = IODeviceMemory::withSubRange(
              parent,
              phys - parent->getPhysicalSegment(0, NULL, kIOMemoryMapperNone),
              len);
        }
        if (NULL == range) {
