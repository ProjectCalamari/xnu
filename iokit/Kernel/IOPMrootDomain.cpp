...
const char *processor_to_datastring(const char *prefix,
...
#define CAP_LOSS(c)                                                            \
  (((_pendingCapability & (c)) == 0) && ((_currentCapability & (c)) != 0))

#define CAP_GAIN(c)                                                            \
  (((_currentCapability & (c)) == 0) && ((_pendingCapability & (c)) != 0))

#define CAP_CHANGE(c) (((_currentCapability ^ _pendingCapability) & (c)) != 0)

#define CAP_CURRENT(c) ((_currentCapability & (c)) != 0)

#define CAP_HIGHEST(c) ((_highestCapability & (c)) != 0)

#define CAP_PENDING(c) ((_pendingCapability & (c)) != 0)
...
static int IOPMConvertSecondsToCalendar(clock_sec_t secs,
                                        IOPMCalendarStruct *dt);
static clock_sec_t IOPMConvertCalendarToSeconds(const IOPMCalendarStruct *dt);
#define YMDTF "%04d/%02d/%d %02d:%02d:%02d"
#define YMDT(cal)                                                              \
  ((int)(cal)->year), (cal)->month, (cal)->day, (cal)->hour, (cal)->minute,    \
      (cal)->second
...
// Minimum time in milliseconds after AP wake that we allow idle timer to
// expire. We impose this minimum to avoid race conditions in the AP wake path
// where userspace clients are not able to acquire power assertions before the
// idle timer expires.
...
const char *getPowerStateString(uint32_t state) {
#define POWER_STATE(x)                                                         \
  {                                                                            \
    (uint32_t)x, #x                                                            \
  }

  static const IONamedValue powerStates[] = {
      POWER_STATE(OFF_STATE),   POWER_STATE(RESTART_STATE),
      POWER_STATE(SLEEP_STATE), POWER_STATE(AOT_STATE),
      POWER_STATE(ON_STATE),    {0, NULL}};
...
#define kRootDomainEntitlementSetProperty                                      \
...
const char *getAggressivenessTypeString(uint32_t type) {
#define AGGRESSIVENESS_TYPE(x)                                                 \
  {                                                                            \
    (uint32_t)x, #x                                                            \
  }
...
enum { kAggressivesStateBusy = 0x01, kAggressivesStateQuickSpindown = 0x02 };
...
enum { kAggressivesRequestTypeService = 1, kAggressivesRequestTypeRecord };
...
const char *getSystemSleepPreventerString(uint32_t preventer) {
#define SYSTEM_SLEEP_PREVENTER(x)                                              \
  {                                                                            \
    (int)x, #x                                                                 \
  }
...
static IOReturn
defaultSleepPolicyHandler(void *ctx, const IOPMSystemSleepPolicyVariables *vars,
...
static IOPMSystemSleepPolicyHandler gSleepPolicyHandler =
...
enum { kInformAC = 0, kInformLid = 1, kInformableCount = 2 };
...
public:
  static PMSettingObject *
  pmSettingObject(IOPMrootDomain *parent_arg,
                  IOPMSettingControllerCallback handler_arg,
                  OSObject *target_arg, uintptr_t refcon_arg,
                  uint32_t supportedPowerSources, const OSSymbol *settings[],
                  OSObject **handle_obj);
...
typedef void (*IOPMTracePointHandler)(void *target, uint32_t code,
                                      uint32_t data);
...
public:
  ...
  IOReturn createAssertion(IOPMDriverAssertionType, IOPMDriverAssertionLevel,
                           IOService *, const char *, IOPMDriverAssertionID *);
  ...
  IOReturn handleSetAssertionLevel(IOPMDriverAssertionID,
...
#define super IOService
    OSDefineMetaClassAndFinalStructors(IOPMrootDomain, IOService)
        ...
        boolean_t IOPMRootDomainGetWillShutdown(void) {
...
extern "C" IONotifier *
registerSleepWakeInterest(IOServiceInterestHandler handler, void *self,
                          void *ref) {
...
extern "C" IONotifier *
registerPrioritySleepWakeInterest(IOServiceInterestHandler handler, void *self,
                                  void *ref) {
  return gRootDomain
      ->registerInterest(gIOPriorityPowerStateInterest, handler, self, ref)
      .detach();
}
...
extern "C" void _doprnt_log(const char *fmt, va_list *argp, void (*putc)(char),
                            int radix);

static int halt_log(const char *fmt, ...) {
  va_list listp;
...
extern "C" void halt_log_enter(const char *what, const void *pc,
                               uint64_t time) {
...
extern "C" void IOSystemShutdownNotification(int howto, int stage) {
  uint64_t startTime;
  ...
  halt_log_enter("IOPMRootDomainWillShutdown", NULL,
                 mach_absolute_time() - startTime);
#if HIBERNATION
  startTime = mach_absolute_time();
  IOHibernateSystemPostWake(true);
  halt_log_enter("IOHibernateSystemPostWake", NULL,
                 mach_absolute_time() - startTime);
#endif
 ...
 *  of the power state provided by its parent.  The driver expresses its desire
 * by calling changePowerStateTo(), the policy-maker expresses its desire by
 * calling changePowerStateToPriv(), and the children express their desires by
 * calling requestPowerDomainState().
 *
 *  The Root Power Domain owns the policy for idle and demand sleep for the
 * system. It is a power-managed IOService just like the others in the system.
 *  It implements several power states which map to what we see as Sleep and On.
 *
 *  The sleep policy is as follows:
 *  1. Sleep is prevented if the case is open so that nobody will think the
 * machine is off and plug/unplug cards.
 *  2. Sleep is prevented if the sleep timeout slider in the prefs panel is
 * zero.
 ...
 *  kIOPMPreventIdleSleep.  When this is true, the root's children are not
 * holding the root on, so it sets the "policy-maker clamp" by calling
 *  changePowerStateToPriv(ON_STATE) to hold itself on until the sleep timer
 * expires. This timer is set for the difference between the sleep timeout
 * slider and the display dim timeout slider. When the timer expires, it
 * releases its clamp and now nothing is holding it awake, so it falls asleep.
 *
...
static void updateConsoleUsersCallout(thread_call_param_t p0,
...
bool IOPMrootDomain::updateTasksSuspend(int newTasksSuspended,
                                        int newAOTTasksSuspended) {
  bool newSuspend;
...
static int sysctl_sleepwaketime SYSCTL_HANDLER_ARGS {
...
SYSCTL_QUAD(_kern, OID_AUTO, wake_abs_time, CTLFLAG_RD | CTLFLAG_LOCKED,
            &gIOLastWakeAbsTime, "");
SYSCTL_QUAD(_kern, OID_AUTO, sleep_abs_time, CTLFLAG_RD | CTLFLAG_LOCKED,
            &gIOLastSleepAbsTime, "");
SYSCTL_QUAD(_kern, OID_AUTO, useractive_abs_time, CTLFLAG_RD | CTLFLAG_LOCKED,
            &gUserActiveAbsTime, "");
SYSCTL_QUAD(_kern, OID_AUTO, userinactive_abs_time, CTLFLAG_RD | CTLFLAG_LOCKED,
            &gUserInactiveAbsTime, "");

static int sysctl_willshutdown SYSCTL_HANDLER_ARGS {
  ...
  error =
      sysctl_io_number(req, gWillShutdown, sizeof(int), &new_value, &changed);
  if (changed) {
...
static int sysctl_progressmeterenable(__unused struct sysctl_oid *oidp,
                                      __unused void *arg1, __unused int arg2,
                                      struct sysctl_req *req) {
  int error;
  int new_value, changed;

  error = sysctl_io_number(req, vc_progressmeter_enable, sizeof(int),
                           &new_value, &changed);
...
static int sysctl_progressmeter(__unused struct sysctl_oid *oidp,
                                __unused void *arg1, __unused int arg2,
                                struct sysctl_req *req) {
  int error;
  int new_value, changed;

  error = sysctl_io_number(req, vc_progressmeter_value, sizeof(int), &new_value,
                           &changed);
...
static int sysctl_consoleoptions(__unused struct sysctl_oid *oidp,
                                 __unused void *arg1, __unused int arg2,
                                 struct sysctl_req *req) {
  int error, changed;
  uint32_t new_value;

  error = sysctl_io_number(req, vc_user_options.options, sizeof(uint32_t),
                           &new_value, &changed);
...
static int sysctl_progressoptions SYSCTL_HANDLER_ARGS {
...
static SYSCTL_PROC(_kern, OID_AUTO, progressoptions,
                   CTLTYPE_STRUCT | CTLFLAG_RW | CTLFLAG_KERN | CTLFLAG_LOCKED |
                       CTLFLAG_ANYBODY,
                   NULL, 0, sysctl_progressoptions,
                   "S,vc_progress_user_options", "");

static int sysctl_wakereason SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern, OID_AUTO, wakereason,
            CTLTYPE_STRING | CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_LOCKED, NULL,
            0, sysctl_wakereason, "A", "wakereason");

static int sysctl_bootreason SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern, OID_AUTO, bootreason,
            CTLTYPE_STRING | CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_LOCKED, NULL,
            0, sysctl_bootreason, "A", "");

static int sysctl_shutdownreason SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern, OID_AUTO, shutdownreason,
            CTLTYPE_STRING | CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_LOCKED, NULL,
            0, sysctl_shutdownreason, "A", "shutdownreason");
...
static int sysctl_shutdowntime SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern, OID_AUTO, shutdowntime,
            CTLTYPE_QUAD | CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_LOCKED, NULL, 0,
            sysctl_shutdowntime, "Q", "shutdowntime");

static int sysctl_targettype SYSCTL_HANDLER_ARGS {
  IOService *root;
...
SYSCTL_PROC(_hw, OID_AUTO, targettype,
            CTLTYPE_STRING | CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_LOCKED, NULL,
            0, sysctl_targettype, "A", "targettype");

static SECURITY_READ_ONLY_LATE(char *)
    jetsam_properties_product_type_string = NULL;
static SECURITY_READ_ONLY_LATE(size_t)
    jetsam_properties_product_type_string_len = 0;

/*
 * SecureDTLookupEntry() is only guaranteed to work before PE_init_iokit(),
 * so we load the jetsam_properties_product_type string (if available) in a
 * startup handler.
 */
__startup_func static void sysctl_load_jetsam_properties_product_type(void) {
  DTEntry node;
  ...
  if (kSuccess != SecureDTGetProperty(node, "jetsam-properties-product-type",
  ...
  jetsam_properties_product_type_string =
      (char *)zalloc_permanent(size, ZALIGN_NONE);
...
STARTUP(SYSCTL, STARTUP_RANK_MIDDLE,
        sysctl_load_jetsam_properties_product_type);

static int sysctl_jetsam_properties_product_type SYSCTL_HANDLER_ARGS {
  if (jetsam_properties_product_type_string != NULL) {
    return SYSCTL_OUT(req, jetsam_properties_product_type_string,
                      jetsam_properties_product_type_string_len);
  }
...
SYSCTL_PROC(_hw, OID_AUTO, jetsam_properties_product_type,
            CTLTYPE_STRING | CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_LOCKED, NULL,
            0, sysctl_jetsam_properties_product_type, "A",
            "jetsam_properties_product_type");

static SYSCTL_INT(_debug, OID_AUTO, noidle, CTLFLAG_RW, &gNoIdleFlag, 0, "");
static SYSCTL_INT(_debug, OID_AUTO, swd_sleep_timeout, CTLFLAG_RW,
                  &gSwdSleepTimeout, 0, "");
static SYSCTL_INT(_debug, OID_AUTO, swd_wake_timeout, CTLFLAG_RW,
                  &gSwdWakeTimeout, 0, "");
static SYSCTL_INT(_debug, OID_AUTO, swd_timeout, CTLFLAG_RW,
                  &gSwdSleepWakeTimeout, 0, "");
static SYSCTL_INT(_debug, OID_AUTO, swd_panic, CTLFLAG_RW, &gSwdPanic, 0, "");
#if DEVELOPMENT || DEBUG
static SYSCTL_INT(_debug, OID_AUTO, swd_panic_phase, CTLFLAG_RW,
                  &swd_panic_phase, 0, "");
#if defined(XNU_TARGET_OS_OSX)
...
static int sysctl_aotmetrics SYSCTL_HANDLER_ARGS {
  if (NULL == gRootDomain) {
    return ENOENT;
  }
  ...
  return sysctl_io_opaque(req, gRootDomain->_aotMetrics, sizeof(IOPMAOTMetrics),
                          NULL);
}

TUNABLE_DT_WRITEABLE(uint32_t, gAOTMode, "/product/iopm", "aot-mode",
                     "aot_mode", 0, TUNABLE_DT_NONE);
static SYSCTL_PROC(_kern, OID_AUTO, aotmetrics,
                   CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_LOCKED |
                       CTLFLAG_ANYBODY,
                   NULL, 0, sysctl_aotmetrics, "S,IOPMAOTMetrics", "");

static int update_aotmode(uint32_t mode) {
  int result;
...
static int sysctl_aotmodebits(__unused struct sysctl_oid *oidp,
                              __unused void *arg1, __unused int arg2,
                              struct sysctl_req *req) {
  int error, changed;
  uint32_t new_value;
  ...
  error = sysctl_io_number(req, gRootDomain->_aotMode, sizeof(uint32_t),
                           &new_value, &changed);
...
static int sysctl_aotmode(__unused struct sysctl_oid *oidp, __unused void *arg1,
...
TUNABLE_DT(uint32_t, gAOTLingerTimeMS, "/product/iopm", "aot-linger-time-ms",
           "aot_linger_time_ms", 800, TUNABLE_DT_NONE);

// Low Power Wake tunables
TUNABLE_DT_WRITEABLE(uint64_t, gLPWFlags, "/product/iopm", "low-power-wake",
                     "low_power_wake", false, TUNABLE_DT_NONE);
...
bool IOPMrootDomain::start(IOService *nub) {
  ...
  PE_parse_boot_argn("swd_sleeptimeout", &gSwdSleepTimeout,
                     sizeof(gSwdSleepTimeout));
  PE_parse_boot_argn("swd_waketimeout", &gSwdWakeTimeout,
                     sizeof(gSwdWakeTimeout));
  PE_parse_boot_argn("swd_timeout", &gSwdSleepWakeTimeout,
                     sizeof(gSwdSleepWakeTimeout));
  PE_parse_boot_argn("haltmspanic", &gHaltTimeMaxPanic,
  ...
  aggressivesData = OSData::withCapacity(sizeof(AggressivesRecord) *
                                         (kPMLastAggressivenessType + 4));
  ...
  extraSleepTimer =
      thread_call_allocate(idleSleepTimerExpired, (thread_call_param_t)this);

  powerButtonDown =
      thread_call_allocate(powerButtonDownCallout, (thread_call_param_t)this);

  powerButtonUp =
      thread_call_allocate(powerButtonUpCallout, (thread_call_param_t)this);

  diskSyncCalloutEntry =
      thread_call_allocate(&disk_sync_callout, (thread_call_param_t)this);
  updateConsoleUsersEntry = thread_call_allocate(&updateConsoleUsersCallout,
  ...
  _currentCapability =
      ...
      kIOPMSystemCapabilityAudio | kIOPMSystemCapabilityNetwork;
  ...
  allowedPMSettings = OSArray::withObjects((const OSObject **)settingsArr,
                                           kRootDomainSettingsCount, 0);

  // List of PM settings that should not automatically publish itself
  // as a feature when registered by a listener.
  noPublishPMSettings =
      OSArray::withObjects((const OSObject **)noPublishSettingsArr,
                           kRootDomainNoPublishSettingsCount, 0);
  ...
  _aotTimerES = IOTimerEventSource::timerEventSource(
      this, OSMemberFunctionCast(IOTimerEventSource::Action, this,
                                 &IOPMrootDomain::aotEvaluate));
  ...
  OSSharedPtr<OSNumber> wranglerIdlePeriod =
      OSNumber::withNumber(kDefaultWranglerIdlePeriod, 32);
  ...
  if (lowLatencyAudioNotifierDict && lowLatencyAudioNotifyStateSym &&
      lowLatencyAudioNotifyTimestampSym && lowLatencyAudioNotifyStateVal &&
      lowLatencyAudioNotifyTimestampVal) {
    lowLatencyAudioNotifierDict->setObject(lowLatencyAudioNotifyStateSym.get(),
                                           lowLatencyAudioNotifyStateVal.get());
    lowLatencyAudioNotifierDict->setObject(
        lowLatencyAudioNotifyTimestampSym.get(),
        lowLatencyAudioNotifyTimestampVal.get());
  }

  OSSharedPtr<const OSSymbol> ucClassName =
      OSSymbol::withCStringNoCopy("RootDomainUserClient");
  setProperty(
      gIOUserClientClassKey,
      const_cast<OSObject *>(static_cast<const OSObject *>(ucClassName.get())));
...
IOReturn IOPMrootDomain::setProperties(OSObject *props_obj) {
  ...
  {
    OSSharedPtr<OSObject> obj = IOUserClient::copyClientEntitlement(
        current_task(), kRootDomainEntitlementSetProperty);
  ...
  if (!clientEntitled) {
    ...
    if ((dict->getCount() == 1) &&
      ...
      return_value = IOUserClient::clientHasPrivilege(
    ...
    if (return_value != kIOReturnSuccess) {
      OSSharedPtr<OSString> procName(IOCopyLogNameForPID(proc_selfpid()),
                                     OSNoRetain);
  ...
  OSSharedPtr<const OSSymbol> publish_simulated_battery_string =
      OSSymbol::withCString("SoftwareSimulatedBatteries");
  OSSharedPtr<const OSSymbol> boot_complete_string =
      OSSymbol::withCString("System Boot Complete");
  OSSharedPtr<const OSSymbol> sys_shutdown_string =
      OSSymbol::withCString("System Shutdown");
  OSSharedPtr<const OSSymbol> stall_halt_string =
      OSSymbol::withCString("StallSystemAtHalt");
  OSSharedPtr<const OSSymbol> battery_warning_disabled_string =
      OSSymbol::withCString("BatteryWarningsDisabled");
  OSSharedPtr<const OSSymbol> idle_seconds_string =
      OSSymbol::withCString("System Idle Seconds");
  OSSharedPtr<const OSSymbol> idle_milliseconds_string =
      OSSymbol::withCString("System Idle Milliseconds");
  OSSharedPtr<const OSSymbol> sleepdisabled_string =
      OSSymbol::withCString("SleepDisabled");
  OSSharedPtr<const OSSymbol> ondeck_sleepwake_uuid_string =
      OSSymbol::withCString(kIOPMSleepWakeUUIDKey);
  OSSharedPtr<const OSSymbol> loginwindow_progress_string =
      OSSymbol::withCString(kIOPMLoginWindowProgressKey);
  OSSharedPtr<const OSSymbol> coredisplay_progress_string =
      OSSymbol::withCString(kIOPMCoreDisplayProgressKey);
  OSSharedPtr<const OSSymbol> coregraphics_progress_string =
  ...
  OSSharedPtr<const OSSymbol> clamshell_open_string =
      OSSymbol::withCString("IOPMTestClamshellOpen");
  OSSharedPtr<const OSSymbol> ac_detach_string =
      OSSymbol::withCString("IOPMTestACDetach");
  OSSharedPtr<const OSSymbol> ac_attach_string =
      OSSymbol::withCString("IOPMTestACAttach");
  OSSharedPtr<const OSSymbol> desktopmode_set_string =
      OSSymbol::withCString("IOPMTestDesktopModeSet");
  OSSharedPtr<const OSSymbol> desktopmode_remove_string =
      OSSymbol::withCString("IOPMTestDesktopModeRemove");
#endif

#if HIBERNATION
  OSSharedPtr<const OSSymbol> hibernatemode_string =
      OSSymbol::withCString(kIOHibernateModeKey);
  OSSharedPtr<const OSSymbol> hibernatefile_string =
      OSSymbol::withCString(kIOHibernateFileKey);
  OSSharedPtr<const OSSymbol> hibernatefilemin_string =
      OSSymbol::withCString(kIOHibernateFileMinSizeKey);
  OSSharedPtr<const OSSymbol> hibernatefilemax_string =
      OSSymbol::withCString(kIOHibernateFileMaxSizeKey);
  OSSharedPtr<const OSSymbol> hibernatefreeratio_string =
      OSSymbol::withCString(kIOHibernateFreeRatioKey);
  OSSharedPtr<const OSSymbol> hibernatefreetime_string =
  ...
  while ((key = (const OSSymbol *)iter->getNextObject()) &&
    ...
    if (key->isEqualTo(publish_simulated_battery_string.get())) {
      ...
      if ((b = OSDynamicCast(OSBoolean, obj))) {
        pmPowerStateQueue->submitPowerEvent(kPowerEventSystemShutdown,
                                            (void *)b);
      }
      ...
      if ((b = OSDynamicCast(OSBoolean, obj))) {
        setProperty(key, b);
        pmPowerStateQueue->submitPowerEvent(kPowerEventUserDisabledSleep,
                                            (void *)b);
      }
    } else if (key->isEqualTo(ondeck_sleepwake_uuid_string.get())) {
      obj->retain();
      pmPowerStateQueue->submitPowerEvent(kPowerEventQueueSleepWakeUUID,
                                          (void *)obj);
      ...
      if (pmTracer && (n = OSDynamicCast(OSNumber, obj))) {
        ...
        kdebugTrace(kPMLogComponentWakeProgress, 0, kIOPMLoginWindowProgress,
                    data);
      }
      ...
      if (pmTracer && (n = OSDynamicCast(OSNumber, obj))) {
        ...
        kdebugTrace(kPMLogComponentWakeProgress, 0, kIOPMCoreDisplayProgress,
                    data);
      }
      ...
      if (pmTracer && (n = OSDynamicCast(OSNumber, obj))) {
        ...
        kdebugTrace(kPMLogComponentWakeProgress, 0, kIOPMCoreGraphicsProgress,
                    data);
      }
      ...
      pmPowerStateQueue->submitPowerEvent(kPowerEventReceivedPowerNotification,
                                          (void *)(uintptr_t)msg);
    ...
    else if ((allowedPMSettings->getNextIndexOfObject(key, 0) !=
              (unsigned int)-1)) {
...
// ***************************************************************************
void IOPMrootDomain::setLockdownModeHibernation(uint32_t status) {
...
IOReturn IOPMrootDomain::setAggressiveness(unsigned long type,
                                           unsigned long value) {
...
IOReturn IOPMrootDomain::setAggressiveness(unsigned long type,
  ...
  if (type == kPMMinutesToDim || type == kPMMinutesToSleep) {
    DLOG("setAggressiveness(%x) %s = %u\n", (uint32_t)options,
         getAggressivenessTypeString((uint32_t)type), (uint32_t)value);
  } else {
...
IOReturn IOPMrootDomain::getAggressiveness(unsigned long type,
...
IOReturn IOPMrootDomain::joinAggressiveness(IOService *service) {
  AggressivesRequest *request;
  ...
  request->data.service.reset(service,
                              OSRetain); // released by synchronizeAggressives()
...
static void handleAggressivesFunction(thread_call_param_t param1,
...
void IOPMrootDomain::handleAggressivesRequests(void) {
  ...
  do {
    ...
    do {
      ...
      queue_remove_first(&aggressivesQueue, request, AggressivesRequest *,
                         chain);
      ...
      case kAggressivesRequestTypeRecord:
        ...
        if (!found && ((request->options &
                        kAggressivesOptionQuickSpindownDisable) == 0)) {
...
unlock_done:
  ...
  if (pingSelf && pmPowerStateQueue) {
    pmPowerStateQueue->submitPowerEvent(kPowerEventPolicyStimulus,
...
void IOPMrootDomain::synchronizeAggressives(queue_head_t *joinedQueue,
...
void IOPMrootDomain::broadcastAggressives(const AggressivesRecord *array,
                                          int count) {
  ...
  iter = IORegistryIterator::iterateOver(this, gIOPowerPlane,
      ...
      while ((entry = iter->getNextObject())) {
        ...
        if (child) {
          if ((service = OSDynamicCast(IOService, child.get()))) {
            if (service->assertPMDriverCall(
                    &callEntry, kIOPMDriverCallMethodSetAggressive)) {
              for (i = 0, record = array; i < count; i++, record++) {
                if (record->flags & kAggressivesRecordFlagModified) {
                  value = record->value;
                  if (record->flags & kAggressivesRecordFlagMinValue) {
...
// ***************************************
static void powerButtonDownCallout(thread_call_param_t us,
                                   thread_call_param_t) {
...
static void powerButtonUpCallout(thread_call_param_t us, thread_call_param_t) {
...
void IOPMrootDomain::startIdleSleepTimer(uint32_t inMilliSeconds) {
  ...
  if (inMilliSeconds) {
    if (inMilliSeconds < kMinimumTimeBeforeIdleSleep) {
      ...
      // Adjust idle timer so it will not expire until atleast
      // kMinimumTimeBeforeIdleSleep milliseconds after the most recent AP wake.
      ...
      if (msec_since_wake < kMinimumTimeBeforeIdleSleep) {
        uint32_t newIdleTimer =
            kMinimumTimeBeforeIdleSleep - (uint32_t)msec_since_wake;
...
static void idleSleepTimerExpired(thread_call_param_t us, thread_call_param_t) {
...
void IOPMrootDomain::setQuickSpinDownTimeout(void) {
  ASSERT_GATED();
  setAggressiveness(kPMMinutesToSpinDown, 0,
...
/* public */
IOReturn IOPMrootDomain::sleepSystem(void) { return sleepSystemOptions(NULL); }
...
void IOPMrootDomain::powerChangeDone(unsigned long previousPowerState) {
  ...
  case SLEEP_STATE: {
    if (kIOPMDriverAssertionLevelOn ==
        getPMAssertionLevel(kIOPMDriverAssertionForceWakeupBit)) {
      IOLog("accelerate wake for assertion\n");
      setWakeTime(mach_continuous_time());
    }
    if (kIOPMDriverAssertionLevelOn ==
        getPMAssertionLevel(kIOPMDriverAssertionForceFullWakeupBit)) {
    ...
    if (kPMCalendarTypeInvalid != _aotWakeTimeCalendar.selector) {
      ...
      if ((kIOPMAOTModeRespectTimers & _aotMode) &&
          (_calendarWakeAlarmUTC < _aotWakeTimeUTC)) {
        ...
        adjWakeTime = secs;
      } else if (kIOPMDriverAssertionLevelOn ==
    ...
    if (_aotLastWakeTime) {
      ...
      if (_aotMetrics->sleepCount &&
          (_aotMetrics->sleepCount <= kIOPMAOTMetricsKernelWakeCountMax)) {
    ...
    if (wake2DarkwakeDelay && sleepDelaysReport) {
      ...
      // Update 'wake2DarkwakeDelay' histogram if this is a fullwake->sleep
      // transition

      SUB_ABSOLUTETIME(&now, &ts_sleepStart);
      absolutetime_to_microtime(now, &darkwake2SleepSecs, &microsecs);
      absolutetime_to_microtime(wake2DarkwakeDelay, &wake2DarkwakeSecs,
                                &microsecs);
...
#if (DEVELOPMENT || DEBUG)
    record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_PMRD,
                        "System State",
    ...
    if (thermalWarningState) {
      OSSharedPtr<const OSSymbol> event =
    ...
    if (g_should_log_clock_adjustments) {
      ...
      os_log(OS_LOG_DEFAULT,
             "%s PMU before going to sleep %lu s %d u %llu abs_b_PEG %llu "
             "abs_a_PEG \n",
    ...
    if (_aotWakeTimeCalendar.selector != kPMCalendarTypeInvalid) {
      ...
      if (_aotMetrics->sleepCount <= kIOPMAOTMetricsKernelWakeCountMax) {
        _aotMetrics->kernelSleepTime[_aotMetrics->sleepCount - 1] =
            (((uint64_t)gIOLastSleepTime.tv_sec) << 10) +
            (gIOLastSleepTime.tv_usec / 1000);
        _aotMetrics->kernelWakeTime[_aotMetrics->sleepCount - 1] =
            (((uint64_t)gIOLastWakeTime.tv_sec) << 10) +
            (gIOLastWakeTime.tv_usec / 1000);
      }
    ...
    OSSharedPtr<OSObject> wakeTypeProp =
        copyProperty(kIOPMRootDomainWakeTypeKey);
    OSSharedPtr<OSObject> wakeReasonProp =
...
#if HIBERNATION
      OSSharedPtr<OSObject> hibOptionsProp =
          copyProperty(kIOHibernateOptionsKey);
      OSNumber *hibOptions = OSDynamicCast(OSNumber, hibOptionsProp.get());
      if (hibernateAborted ||
          ((hibOptions && !(hibOptions->unsigned32BitValue() &
                            kIOHibernateOptionDarkWake)))) {
        ...
        if (hibernateAborted) {
          ...
          DLOG(
              "EFI brought up graphics. Going to full wake. HibOptions: 0x%x\n",
              hibOptions->unsigned32BitValue());
        }
      } else
#endif
          if (wakeType && (wakeType->isEqualTo(kIOPMRootDomainWakeTypeUser) ||
    ...
    if (darkWakeExit) {
      ...
      if ((gDarkWakeFlags & kDarkWakeFlagPromotionMask) !=
    ...
    kdebugTrace(kPMLogSystemWake, 0, (uintptr_t)(timeSinceReset >> 32),
                (uintptr_t)timeSinceReset);
    ...
    if ((gDarkWakeFlags & kDarkWakeFlagPromotionMask) ==
      ...
      requestUserActive(this, "Full wake on dark wake promotion boot-arg");
               ...
               !(gDarkWakeFlags & kDarkWakeFlagAlarmIsDark)) {
      isRTCAlarmWake = true;
      fullWakeReason = kFullWakeReasonLocalUser;
  ...
  case AOT_STATE: {
    ...
    /* Force re-evaluate the aggressiveness values to set appropriate idle sleep
     * timer */
    pmPowerStateQueue->submitPowerEvent(kPowerEventPolicyStimulus,
...
IOReturn
IOPMrootDomain::requestPowerDomainState(IOPMPowerFlags childDesire,
...
static void makeSleepPreventersListLog(const OSSharedPtr<OSSet> &preventers,
  ...
  OSSharedPtr<OSCollectionIterator> iterator =
      OSCollectionIterator::withCollection(preventers.get());
  OSObject *obj = NULL;
...
bool IOPMrootDomain::updatePreventIdleSleepList(IOService *service,
                                                bool addNotRemove) {
  unsigned int oldCount;
...
bool IOPMrootDomain::updatePreventIdleSleepListInternal(IOService *service,
                                                        bool addNotRemove,
                                                        unsigned int oldCount) {
...
#if defined(XNU_TARGET_OS_OSX)
  if (addNotRemove && (service == wrangler) &&
...
void IOPMrootDomain::startSpinDump(uint32_t spindumpKind) {
  messageClients(kIOPMMessageLaunchBootSpinDump,
...
void IOPMrootDomain::updatePreventSystemSleepList(IOService *service,
                                                  bool addNotRemove) {
  ...
  messageClient(kIOPMMessageSystemSleepPreventers,
                systemCapabilityNotifier.get(), &newCount, sizeof(newCount));
}

void IOPMrootDomain::copySleepPreventersList(OSArray **idleSleepList,
                                             OSArray **systemSleepList) {
  OSSharedPtr<OSCollectionIterator> iterator;
  ...
  if (systemSleepList && preventSystemSleepList &&
      (preventSystemSleepList->getCount() != 0)) {
    iterator =
        OSCollectionIterator::withCollection(preventSystemSleepList.get());
    array = OSArray::withCapacity(5);
...
void IOPMrootDomain::copySleepPreventersListWithID(OSArray **idleSleepList,
                                                   OSArray **systemSleepList) {
  OSSharedPtr<OSCollectionIterator> iterator;
  OSObject *object = NULL;
  OSSharedPtr<OSArray> array;
    ...
    if (iterator && array) {
      while ((object = iterator->getNextObject())) {
        ...
        if (service) {
          ...
          OSSharedPtr<const OSSymbol> name = service->copyName();
          OSSharedPtr<OSNumber> id =
              OSNumber::withNumber(service->getRegistryEntryID(), 64);
          if (dict && name && id) {
...
bool IOPMrootDomain::tellChangeDown(unsigned long stateNum) {
  DLOG("tellChangeDown %s->%s\n",
       getPowerStateString((uint32_t)getPowerState()),
       getPowerStateString((uint32_t)stateNum));
...
void IOPMrootDomain::askChangeDownDone(IOPMPowerChangeFlags *inOutChangeFlags,
                                       bool *cancel) {
  DLOG("askChangeDownDone(0x%x, %u) type %x, cap %x->%x\n", *inOutChangeFlags,
       *cancel, _systemTransitionType, _currentCapability, _pendingCapability);
...
void IOPMrootDomain::systemDidNotSleep(void) {
  ...
  if (toldPowerdCapWillChange && systemCapabilityNotifier &&
    ...
    messageClient(kIOMessageSystemCapabilityChange,
                  systemCapabilityNotifier.get(), &params, sizeof(params));
  }
}
...
void IOPMrootDomain::tellChangeUp(unsigned long stateNum) {
  ...
  if (stateNum == ON_STATE) {
    ...
    getPlatform()->callPlatformFunction(
        ...
        (void *)(uintptr_t)kIOMessageSystemHasPoweredOn, NULL, NULL, NULL);
...
IOReturn IOPMrootDomain::sysPowerDownHandler(void *target, void *refCon,
  ...
  if (messageType == kIOMessageSystemCapabilityChange) {
    ...
    DLOG("sysPowerDownHandler cap %x -> %x (flags %x)\n",
         params->fromCapabilities, params->toCapabilities, params->changeFlags);
    ...
    if (CAP_WILL_CHANGE_TO_OFF(params, kIOPMSystemCapabilityCPU)) {
      ...
      getPlatform()->callPlatformFunction(
          sleepMessagePEFunction.get(), false,
          (void *)(uintptr_t)kIOMessageSystemWillSleep, NULL, NULL, NULL);
      ...
      if (!OSCompareAndSwap(0, 1, &gSleepOrShutdownPending)) {
        ...
        thread_call_enter1_delayed(
            ...
            (thread_call_param_t)(uintptr_t)params->notifyRef, deadline);
      } else {
        thread_call_enter1(gRootDomain->diskSyncCalloutEntry,
...
extern "C" bool IOPMCopySleepWakeUUIDKey(char *buffer, size_t buf_len) {
  if (!gSleepWakeUUIDIsSet) {
    return false;
  }

  if (buffer != NULL) {
    OSSharedPtr<OSString> string = OSDynamicPtrCast<OSString>(
...
void IOPMrootDomain::lowLatencyAudioNotify(uint64_t time, boolean_t state) {
  if (lowLatencyAudioNotifierDict && lowLatencyAudioNotifyStateSym &&
      lowLatencyAudioNotifyTimestampSym && lowLatencyAudioNotifyStateVal &&
      lowLatencyAudioNotifyTimestampVal) {
    lowLatencyAudioNotifyTimestampVal->setValue(time);
    lowLatencyAudioNotifyStateVal->setValue(state);
    setPMSetting(gIOPMSettingLowLatencyAudioModeKey.get(),
                 lowLatencyAudioNotifierDict.get());
  } else {
...
extern "C" void IOPMrootDomainRTNotifier(uint64_t time, boolean_t state) {
...
static uint32_t nextRequestTag(IOPMRequestTag tag) {
...
IOReturn
IOPMrootDomain::changePowerStateWithOverrideTo(IOPMPowerStateIndex ordinal,
                                               IOPMRequestTag reason) {
  uint32_t tag = nextRequestTag(reason);
  DLOG("%s(%s, %x)\n", __FUNCTION__, getPowerStateString((uint32_t)ordinal),
       tag);

  if ((ordinal != ON_STATE) && (ordinal != AOT_STATE) &&
      (ordinal != SLEEP_STATE)) {
    return kIOReturnUnsupported;
  }
...
IOReturn IOPMrootDomain::changePowerStateWithTagTo(IOPMPowerStateIndex ordinal,
                                                   IOPMRequestTag reason) {
  uint32_t tag = nextRequestTag(reason);
  DLOG("%s(%s, %x)\n", __FUNCTION__, getPowerStateString((uint32_t)ordinal),
       tag);
...
IOReturn
IOPMrootDomain::changePowerStateWithTagToPriv(IOPMPowerStateIndex ordinal,
                                              IOPMRequestTag reason) {
  uint32_t tag = nextRequestTag(reason);
  DLOG("%s(%s, %x)\n", __FUNCTION__, getPowerStateString((uint32_t)ordinal),
       tag);
...
extern "C" int hibernate_should_abort(void) {
  if (gRootDomain) {
...
void IOPMrootDomain::scheduleImmediateDebugWake(void) {
...
// Dispatched though a thread call, so it is safe to perform work that might
// block on a sleeping disk. PM state machine (not thread) will block w/o
// timeout until this function returns.
//******************************************************************************

void IOPMrootDomain::willNotifyInterested(IOPMPowerStateIndex newPowerState) {
  if (SLEEP_STATE == newPowerState) {
    ...
    if (!_aotMode) {
      ...
      if (kIOPMAOTModeCycle & _aotMode) {
        clock_interval_to_absolutetime_interval(10, kSecondScale,
                                                &_aotTestInterval);
        _aotTestTime = mach_continuous_time() + _aotTestInterval;
      ...
      clock_interval_to_absolutetime_interval(2000, kMillisecondScale,
                                              &_aotWakePreWindow);
      clock_interval_to_absolutetime_interval(1100, kMillisecondScale,
                                              &_aotWakePostWindow);
    }
...
void IOPMrootDomain::willNotifyPowerChildren(
  ...
  if (SLEEP_STATE == newPowerState) {
    ...
    // On AS, hibernation cannot be aborted. Resetting RTC to 1s during
    // hibernation upon detecting user activity is pointless (we are likely to
    // spend >1s hibernating). It also clears existing alarms, which can mess
    // with cycler tools.
    if (gRootDomain->activitySinceSleep() &&
        gIOHibernateState == kIOHibernateStateInactive) {
#else  /* defined(__arm64__) && HIBERNATION */
    // On non-AS, hibernation can be aborted if user activity is detected. So
    // continue to reset the RTC alarm (even during hibernation) so we can
    // immediately wake from regular S2R if needed.
    if (gRootDomain->activitySinceSleep()) {
...
void IOPMrootDomain::willTellSystemCapabilityDidChange(void) {
...
bool IOPMrootDomain::shouldSleepOnRTCAlarmWake(void) {
  ...
  DLOG("shouldSleepOnRTCAlarmWake: clamshell closed %d, disabled %d/%x, "
       "desktopMode %d, ac %d\n",
       clamshellClosed, clamshellDisabled, clamshellSleepDisableMask,
       desktopMode, acAdaptorConnected);
...
void IOPMrootDomain::sendClientClamshellNotification(void) {
  ...
  setProperty(kAppleClamshellCausesSleepKey, shouldSleepOnClamshellClosed()
                                                 ? kOSBooleanTrue
                                                 : kOSBooleanFalse);
  ...
  messageClients(
      ...
      (void *)(uintptr_t)((clamshellClosed ? kClamshellStateBit : 0) |
                          (shouldSleepOnClamshellClosed() ? kClamshellSleepBit
                                                          : 0)));
}
...
void IOPMrootDomain::setClamShellSleepDisable(bool disable, uint32_t bitmask) {
  uint32_t oldMask;
  ...
  if (gIOPMWorkLoop->inGate() == false) {
    gIOPMWorkLoop->runAction(
        ...
        (OSObject *)this, (void *)disable, (void *)(uintptr_t)bitmask);
    return;
  }
...
void IOPMrootDomain::recordRTCAlarm(const OSSymbol *type, OSObject *object) {
...
void IOPMrootDomain::publishFeature(const char *feature,
  ...
  OSSharedPtr<OSObject> origFeaturesProp =
      copyProperty(kRootDomainSupportedFeatures);
  OSDictionary *origFeatures =
      OSDynamicCast(OSDictionary, origFeaturesProp.get());
  ...
  new_feature_data =
      OSNumber::withNumber((unsigned long long)feature_value, 32);
  ...
  if ((osObj = features->getObject(feature))) {
    if ((existing_feature = OSDynamicCast(OSNumber, osObj))) {
      ...
      existing_feature_arr =
          OSArray::withObjects((const OSObject **)&existing_feature, 1, 2);
      ...
      existing_feature_arr = OSArray::withArray(
          existing_feature_arr_raw, existing_feature_arr_raw->getCount() + 1);
    }
...
void IOPMrootDomain::publishPMSetting(const OSSymbol *feature, uint32_t where,
                                      uint32_t *featureID) {
  if (noPublishPMSettings && (noPublishPMSettings->getNextIndexOfObject(
                                  feature, 0) != (unsigned int)-1)) {
    // Setting found in noPublishPMSettings array
    *featureID = kBadPMFeatureID;
    return;
  }

  publishFeature(feature->getCStringNoCopy(), where, featureID);
}
...
IOReturn IOPMrootDomain::setPMSetting(const OSSymbol *type, OSObject *object) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ret == kIOReturnSuccess) {
      ...
      OSSharedPtr<const OSSymbol> kextName =
          copyKextIdentifierWithAddress((vm_address_t)pmso->func);
      if (kextName) {
        DLOG("PMSetting(%s) error 0x%x from %s\n", type->getCStringNoCopy(),
             ret, kextName->getCStringNoCopy());
      }
#endif
    }
  }
...
OSSharedPtr<OSObject> IOPMrootDomain::copyPMSetting(OSSymbol *whichSetting) {
  OSSharedPtr<OSObject> obj;
...
IOReturn IOPMrootDomain::registerPMSettingController(
    const OSSymbol *settings[], IOPMSettingControllerCallback func,
    OSObject *target, uintptr_t refcon, OSObject **handle) {
  return registerPMSettingController(
      settings,
      (kIOPMSupportedOnAC | kIOPMSupportedOnBatt | kIOPMSupportedOnUPS), func,
      target, refcon, handle);
}
...
IOReturn IOPMrootDomain::registerPMSettingController(
    const OSSymbol *settings[], uint32_t supportedPowerSources,
    IOPMSettingControllerCallback func, OSObject *target, uintptr_t refcon,
    OSObject **handle) {
  PMSettingObject *pmso = NULL;
  OSObject *pmsh = NULL;
  int i;

  if (NULL == settings || NULL == func || NULL == handle) {
    return kIOReturnBadArgument;
  }

  pmso = PMSettingObject::pmSettingObject((IOPMrootDomain *)this, func, target,
                                          refcon, supportedPowerSources,
                                          settings, &pmsh);
  ...
  for (i = 0; settings[i]; i++) {
    ...
    OSArray *list =
        OSDynamicCast(OSArray, settingsCallbacks->getObject(settings[i]));
    if (!list) {
...
void IOPMrootDomain::informCPUStateChange(uint32_t type, uint32_t value) {
#if defined(__i386__) || defined(__x86_64__)
  ...
  strlcpy((char *)varInfoStruct.varName, (const char *)varNameStr,
  ...
  if ((0 == pmCPUret) && (*varIndex == kCPUUnknownIndex)) {
...
static uint32_t getSleepTypeAttributes(uint32_t sleepType) {
  static const uint32_t sleepTypeAttributes[kIOPMSleepTypeLast] = {
      /* invalid   */ 0,
      /* abort     */ 0,
      /* normal    */ 0,
      /* safesleep */ kIOPMSleepAttributeHibernateSetup,
      /* hibernate */ kIOPMSleepAttributeHibernateSetup |
          kIOPMSleepAttributeHibernateSleep,
      /* standby   */ kIOPMSleepAttributeHibernateSetup |
          kIOPMSleepAttributeHibernateSleep,
      /* poweroff  */ kIOPMSleepAttributeHibernateSetup |
          kIOPMSleepAttributeHibernateSleep,
      /* deepidle  */ 0};
...
#define SLEEP_FACTOR(x)                                                        \
  {                                                                            \
    (uint32_t)kIOPMSleepFactor##x, #x                                          \
  }
  ...
  powerOffEnabled =
      (getSleepOption(kIOPMAutoPowerOffDelayKey, &powerOffDelay) &&
  ...
  DLOG("phase %d, standby %d delay %u timer %u, poweroff %d delay %u timer %u, "
       "hibernate 0x%x\n",
       sleepPhase, standbyEnabled, standbyDelay, standbyTimer, powerOffEnabled,
       powerOffDelay, powerOffTimer, *hibMode);
...
#define TCPKEEPALIVE 1
  ...
  for (int factorBit = 0; factorBit < (8 * sizeof(uint32_t)); factorBit++) {
    ...
    if (factor & currentFactors) {
      ...
      strlcat(currentFactorsBuf, IOFindNameForValue(factor, factorValues),
  ...
  if (gSleepPolicyHandler) {
    ...
    gSleepPolicyVars->scheduledAlarms =
        _scheduledAlarmMask | _userScheduledAlarmMask;
    ...
    DLOG("sleep params v%u, type %u, flags 0x%x, wake 0x%x, timer %u, poweroff "
         "%u\n",
  ...
  for (uint32_t i = 0; i < pt->entryCount; i++) {
    ...
    mismatch =
        (((uint32_t)currentFactors ^ entry->factorBits) & entry->factorMask);

    DLOG("mask 0x%08x, bits 0x%08x, flags 0x%08x, wake 0x%08x, mismatch "
         "0x%08x\n",
         entry->factorMask, entry->factorBits, entry->sleepFlags,
         entry->wakeEvents, mismatch);
    if (mismatch) {
      continue;
    }
...
void IOPMrootDomain::evaluateSystemSleepPolicyFinal(void) {
  ...
  if (evaluateSystemSleepPolicy(&params, kIOPMSleepPhase2, &hibernateMode)) {
    if ((kIOPMSleepTypeStandby == params.sleepType) &&
        ...
        (!((kIOPMSleepFactorStandbyForced | kIOPMSleepFactorAutoPowerOffForced |
            kIOPMSleepFactorHibernateForced) &
           gSleepPolicyVars->sleepFactors))) {
      standbyNixed = true;
      wakeNow = true;
    }
    if (wakeNow || ((hibernateDisabled || hibernateAborted) &&
...
bool IOPMrootDomain::getHibernateSettings(uint32_t *hibernateModePtr,
...
IOReturn IOPMrootDomain::getSystemSleepType(uint32_t *sleepType,
  ...
  if (gIOPMWorkLoop->inGate() == false) {
    IOReturn ret = gIOPMWorkLoop->runAction(
        ...
        (OSObject *)this, (void *)sleepType, (void *)standbyTimer);
    return ret;
  }
...
static void platformHaltRestartApplier(OSObject *object, void *context) {
  ...
  if ((deltaTime > kPMHaltTimeoutMS) && notifier) {
    LOG("%s handler %p took %u ms\n", ctx->LogString,
        OBFUSCATE(notifier->handler), deltaTime);
    halt_log_enter("PowerOff/Restart message to priority client",
                   (const void *)notifier->handler, elapsedTime);
  }
...
void IOPMrootDomain::handlePlatformHaltRestart(UInt32 pe_type) {
  ...
  applyToInterested(gIOPriorityPowerStateInterest, platformHaltRestartApplier,
                    &gHaltRestartCtx);

  // For normal shutdown, turn off File Server Mode.
  if (kPEHaltCPU == pe_type) {
    OSSharedPtr<const OSSymbol> setting =
        OSSymbol::withCString(kIOPMSettingRestartOnPowerLossKey);
...
void IOPMrootDomain::panicWithShutdownLog(uint32_t timeoutInMs) {
  if (gHaltLog) {
    if ((gHaltRestartCtx.phase == kNotifyPriorityClients) &&
        gHaltRestartCtx.handler) {
      halt_log_enter("Blocked on priority client",
                     (void *)gHaltRestartCtx.handler,
                     mach_absolute_time() - gHaltStartTime);
    }
    panic("%s timed out in phase '%s'. Total %d ms:%s",
          gHaltRestartCtx.LogString,
          shutdownPhase2String(gHaltRestartCtx.phase), timeoutInMs, gHaltLog);
  } else {
    panic("%s timed out in phase \'%s\'. Total %d ms",
          gHaltRestartCtx.LogString,
          shutdownPhase2String(gHaltRestartCtx.phase), timeoutInMs);
  }
}
...
IOReturn IOPMrootDomain::restartSystem(void) { return kIOReturnUnsupported; }
...
void IOPMrootDomain::tagPowerPlaneService(IOService *service,
                                          IOPMActions *actions,
  ...
  if (service == this) {
    actions->actionPowerChangeStart =
        OSMemberFunctionCast(IOPMActionPowerChangeStart, this,
                             &IOPMrootDomain::handleOurPowerChangeStart);

    actions->actionPowerChangeDone =
        OSMemberFunctionCast(IOPMActionPowerChangeDone, this,
                             &IOPMrootDomain::handleOurPowerChangeDone);

    actions->actionPowerChangeOverride =
        OSMemberFunctionCast(IOPMActionPowerChangeOverride, this,
  ...
  if (NULL != service->metaCast("IODisplayWrangler")) {
    ...
    wrangler->registerInterest(gIOGeneralInterest, &displayWranglerNotification,
                               this, NULL);

    // found the display wrangler, check for any display assertions already
    // created
    if (pmAssertions->getActivatedAssertions() &
  ...
  if (flags) {
    ...
    while (child != this) {
      if (child->propertyHasValue("IOPCITunnelled", kOSBooleanTrue)) {
        // Skip delaying notifications and clamping power on external graphics
        // and audio devices.
      ...
      if ((parent == pciHostBridgeDriver) || (parent == this)) {
  ...
  OSSharedPtr<OSObject> prop =
      service->copyProperty(kIOPMDarkWakeMaxPowerStateKey);
  if (prop) {
  ...
  if (flags) {
    ...
    if (flags & kPMActionsFlagIsDisplayWrangler) {
      actions->actionActivityTickle = OSMemberFunctionCast(
  ...
  if (pciHostBridgeDevice && service->metaCast("IOPCIDevice")) {
    ...
    if ((parent == pciHostBridgeDevice) &&
        service->propertyExists("acpi-device")) {
      ...
      if (bit >= 0) {
        ...
        actions->actionPowerChangeStart = OSMemberFunctionCast(
...
void IOPMrootDomain::overrideOurPowerChange(
    IOService *service, IOPMActions *actions, const IOPMRequest *request,
  ...
  if (desiredPowerState < currentPowerState) {
    if (CAP_CURRENT(kIOPMSystemCapabilityGraphics)) {
      ...
      _desiredCapability &=
          ~(kIOPMSystemCapabilityGraphics | kIOPMSystemCapabilityAudio);
...
void IOPMrootDomain::handleOurPowerChangeStart(
    IOService *service, IOPMActions *actions, const IOPMRequest *request,
    IOPMPowerStateIndex newPowerState, IOPMPowerChangeFlags *inOutChangeFlags) {
  ...
  else if (!systemBooting && (changeFlags & kIOPMSelfInitiated) &&
    ...
    if (kFullWakeReasonNone != fullWakeReason) {
      ...
      if (fullWakeReason == kFullWakeReasonLocalUser) {
        ...
        setProperty(kIOPMRootDomainWakeTypeKey,
                    isRTCAlarmWake ? kIOPMRootDomainWakeTypeAlarm
  ...
  if (kSystemTransitionSleep == _systemTransitionType) {
    ...
    _pendingCapability = _desiredCapability | kIOPMSystemCapabilityCPU |
  ...
  if (kSystemTransitionCapability == _systemTransitionType) {
    ...
    if (CAP_GAIN(kIOPMSystemCapabilityGraphics)) {
      ...
      if (checkSystemCanSustainFullWake()) {
        changePowerStateWithOverrideTo(getRUN_STATE(),
                                       kCPSReasonSustainFullWake);
      }
#endif
    ...
    if (CAP_LOSS(kIOPMSystemCapabilityGraphics)) {
      ...
      *inOutChangeFlags |= kIOPMSyncTellPowerDown;
      _systemMessageClientMask =
          kSystemMessageClientPowerd | kSystemMessageClientLegacyApp;
  ...
  if (publishSleepReason) {
    static const char *IOPMSleepReasons[] = {kIOPMClamshellSleepKey,
  ...
  if ((kSystemTransitionNone != _systemTransitionType) &&
    ...
    DLOG("=== START (%s->%s, %x->%x, 0x%x) gen %u, msg %x, tag %x\n",
         getPowerStateString(currentPowerState),
         getPowerStateString((uint32_t)newPowerState), _currentCapability,
         _pendingCapability, *inOutChangeFlags, _systemStateGeneration,
         _systemMessageClientMask, requestTag);
#if DEVELOPMENT || DEBUG
    if (currentPowerState != (uint32_t)newPowerState) {
      record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_PMRD,
                          ...
                          *inOutChangeFlags, _systemStateGeneration,
                          _systemMessageClientMask, requestTag);
    }
#endif /* DEVELOPMENT || DEBUG */
  }
...
void IOPMrootDomain::setSystemTransitionTypeGated(SystemTransitionType type) {
  assert(gIOPMWorkLoop->inGate());
  _systemTransitionType = type;
  commandGate->commandWakeup(&_systemTransitionType);
}

void IOPMrootDomain::waitForSystemTransitionToMinPowerState(
    IOPMRootDomainPowerState state) {
  while (true) {
...
void IOPMrootDomain::handleOurPowerChangeDone(
    IOService *service, IOPMActions *actions, const IOPMRequest *request,
    IOPMPowerStateIndex oldPowerState, IOPMPowerChangeFlags changeFlags) {
  ...
  if (_systemTransitionType != kSystemTransitionNone) {
    ...
    if (changeFlags & kIOPMNotDone) {
      ...
      if (!CAP_CURRENT(kIOPMSystemCapabilityGraphics) &&
        ...
        pmPowerStateQueue->submitPowerEvent(
            kPowerEventPolicyStimulus, (void *)kStimulusDarkWakeActivityTickle);
#endif /* !defined(XNU_TARGET_OS_OSX) */
      }
    ...
    if (currentPowerState != (uint32_t)oldPowerState) {
      record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_PMRD,
                          ...
                          getPowerStateString(currentPowerState),
                          _currentCapability, _pendingCapability, changeFlags,
                          _systemStateGeneration, _systemMessageClientMask,
                          request->getTag());
    }
#endif /* DEVELOPMENT || DEBUG */
    ...
    DLOG("=== FINISH (%s->%s, %x->%x, 0x%x) gen %u, msg %x, tag %x\n",
         getPowerStateString((uint32_t)oldPowerState),
         getPowerStateString(currentPowerState), _currentCapability,
         _pendingCapability, changeFlags, _systemStateGeneration,
         _systemMessageClientMask, request->getTag());
    ...
    if (_pendingCapability & kIOPMSystemCapabilityGraphics) {
      ...
      if (clamshellExists && fullWakeThreadCall) {
        AbsoluteTime deadline;
        clock_interval_to_deadline(DARK_TO_FULL_EVALUATE_CLAMSHELL_DELAY,
                                   kSecondScale, &deadline);
    ...
    } else if ((fullWakeReason == kFullWakeReasonDisplayOn) &&
...
void IOPMrootDomain::overridePowerChangeForService(
    IOService *service, IOPMActions *actions, const IOPMRequest *request,
    IOPMPowerStateIndex *inOutPowerState,
    IOPMPowerChangeFlags *inOutChangeFlags) {
  ...
  if (actions->state & kPMActionsStatePowerClamped) {
    ...
    if (powerState > maxPowerState) {
      DLOG("power clamped %s %qx, ps %u->%u, cflags 0x%x)\n",
           service->getName(), service->getRegistryEntryID(), powerState,
           maxPowerState, changeFlags);
...
void IOPMrootDomain::handleActivityTickleForDisplayWrangler(
    IOService *service, IOPMActions *actions) {
#if DISPLAY_WRANGLER_PRESENT
...
void IOPMrootDomain::handleUpdatePowerClientForDisplayWrangler(
    IOService *service, IOPMActions *actions, const OSSymbol *powerClient,
    IOPMPowerStateIndex oldPowerState, IOPMPowerStateIndex newPowerState) {
#if DISPLAY_WRANGLER_PRESENT
  assert(service == wrangler);
  ...
  DLOG("wrangler %s (ps %u, %u->%u)\n", powerClient->getCStringNoCopy(),
       (uint32_t)service->getPowerState(), (uint32_t)oldPowerState,
...
bool IOPMrootDomain::shouldDelayChildNotification(IOService *service) {
  if ((kFullWakeReasonNone == fullWakeReason) &&
      (kSystemTransitionWake == _systemTransitionType)) {
...
void IOPMrootDomain::handlePowerChangeStartForPCIDevice(
    IOService *service, IOPMActions *actions, const IOPMRequest *request,
    IOPMPowerStateIndex powerState, IOPMPowerChangeFlags *inOutChangeFlags) {
  pmTracer->tracePCIPowerChange(PMTraceWorker::kPowerChangeStart, service,
                                *inOutChangeFlags,
...
void IOPMrootDomain::handlePowerChangeDoneForPCIDevice(
    IOService *service, IOPMActions *actions, const IOPMRequest *request,
    IOPMPowerStateIndex powerState, IOPMPowerChangeFlags changeFlags) {
  pmTracer->tracePCIPowerChange(PMTraceWorker::kPowerChangeCompleted, service,
                                changeFlags,
                                (actions->flags & kPMActionsPCIBitNumberMask));
}
...
OSDefineMetaClassAndStructors(IOPMServiceInterestNotifier,
                              _IOServiceInterestNotifier)

    OSSharedPtr<IONotifier> IOPMrootDomain::registerInterest(
        const OSSymbol *typeOfInterest, IOServiceInterestHandler handler,
  ...
  if (notifier->init()) {
    rc = super::registerInterestForNotifier(notifier, typeOfInterest, handler,
                                            target, ref);
  }
  ...
  OSSharedPtr<OSKext> kext =
      OSKext::lookupKextWithAddress((vm_address_t)handler);
  if (kext) {
    data = kext->copyUUID();
  }
  if (data && (data->getLength() == sizeof(uuid_t))) {
    ...
    notifier->uuid0 =
        ((uint64_t)(uuid[0]) << 56) | ((uint64_t)(uuid[1]) << 48) |
        ((uint64_t)(uuid[2]) << 40) | ((uint64_t)(uuid[3]) << 32) |
        ((uint64_t)(uuid[4]) << 24) | ((uint64_t)(uuid[5]) << 16) |
        ((uint64_t)(uuid[6]) << 8) | (uuid[7]);
    notifier->uuid1 =
        ((uint64_t)(uuid[8]) << 56) | ((uint64_t)(uuid[9]) << 48) |
        ((uint64_t)(uuid[10]) << 40) | ((uint64_t)(uuid[11]) << 32) |
        ((uint64_t)(uuid[12]) << 24) | ((uint64_t)(uuid[13]) << 16) |
...
bool IOPMrootDomain::systemMessageFilter(void *object, void *arg1, void *arg2,
                                         void *arg3) {
...
IOReturn
IOPMrootDomain::setMaintenanceWakeCalendar(const IOPMCalendarStruct *calendar) {
  OSSharedPtr<OSData> data;
  IOReturn ret = 0;
  ...
  if (kPMCalendarTypeMaintenance == calendar->selector) {
    ret =
        setPMSetting(gIOPMSettingMaintenanceWakeCalendarKey.get(), data.get());
  } else if (kPMCalendarTypeSleepService == calendar->selector) {
    ret =
        setPMSetting(gIOPMSettingSleepServiceWakeCalendarKey.get(), data.get());
  }
...
void IOPMrootDomain::updateUserActivity(void) {
#if defined(XNU_TARGET_OS_OSX) && !DISPLAY_WRANGLER_PRESENT
  clock_get_uptime(&userActivityTime);
...
void IOPMrootDomain::requestUserActive(IOService *device, const char *reason) {
#if DISPLAY_WRANGLER_PRESENT
  if (wrangler) {
    wrangler->activityTickle(0, 0);
  }
  ...
  messageClient(kIOPMMessageRequestUserActive, systemCapabilityNotifier.get(),
...
bool IOPMrootDomain::checkSystemSleepAllowed(IOOptionBits options,
  ...
  do {
    ...
    if (systemBooting || systemShutdown || gWillShutdown) {
      err = kPMSystemRestartBootingInProgress; // 2. restart or shutdown in
                                               // progress
      break;
    }
    ...
    if (_driverKitMatchingAssertionCount != 0 ||
        _driverKitSyncedAssertionCount != 0) {
      err = kPMCPUAssertion;
      break;
    }
    ...
    if (!IOServicePH::checkPMReady()) {
      ...
      if (!(lowBatteryCondition || thermalWarningState ||
        ...
        // 116893363: kPMDKNotReady sleep cancellations often leaves embedded
        // devices in dark wake for long periods of time, which causes issues as
        // apps were already informed of sleep during the f->9 transition. As a
        // temporary measure, always full wake if we hit this specific
        // condition.
        pmPowerStateQueue->submitPowerEvent(
            kPowerEventPolicyStimulus, (void *)kStimulusDarkWakeActivityTickle);
      }
    ...
    if (lowBatteryCondition || thermalWarningState || thermalEmergencyState) {
      break; // always sleep on low battery or when in thermal warning/emergency
             // state
    }
    ...
    if (pciCantSleepValid) {
      ...
      ret = getPlatform()->callPlatformFunction(sleepSupportedPEFunction.get(),
...
bool IOPMrootDomain::checkSystemCanAbortIdleSleep(void) {
  bool abortableSleepType =
      ((lastSleepReason == kIOPMSleepReasonIdle) || (lastSleepReason == 0));
...
int32_t IOPMrootDomain::considerRunMode(IOService *service,
  ...
  if (promote > 0) {
    IOLog("IOPMRD: %s 0x%llx runmode to %s\n", service->getName(),
          pmDriverClass, (promote < 0) ? "OFF" : "ON");
  }
  return promote;
}
...
bool IOPMrootDomain::attemptIdleSleepAbort(void) {
  if (!gIOPMWorkLoop->inGate()) {
...
void IOPMrootDomain::setIdleSleepRevertible(bool revertible) {
...
static const unsigned int daysbymonth[] = {0,   31,  59,  90,  120, 151, 181,
                                           212, 243, 273, 304, 334, 365};

static const unsigned int lydaysbymonth[] = {0,   31,  60,  91,  121, 152, 182,
                                             213, 244, 274, 305, 335, 366};

static int __unused IOPMConvertSecondsToCalendar(clock_sec_t secs,
                                                 IOPMCalendarStruct *dt) {
...
unsigned long IOPMrootDomain::getRUN_STATE(void) {
  return (_aotNow && !(kIOPMWakeEventAOTExitFlags & _aotPendingFlags))
             ? AOT_STATE
             : ON_STATE;
}

bool IOPMrootDomain::isAOTMode() { return _aotNow; }

bool IOPMrootDomain::isLPWMode() {
  return gLPWFlags && currentOrPendingPowerState(AOT_STATE);
}

bool IOPMIsAOTMode(void) {
  return gIOPMRootDomain && gIOPMRootDomain->isAOTMode();
}
bool IOPMIsLPWMode(void) {
  return gIOPMRootDomain && gIOPMRootDomain->isLPWMode();
}

void IOPMNetworkStackFullWake(uint64_t flags, const char *reason) {
  assert(kIOPMNetworkStackFullWakeFlag == flags);
  assert(gIOPMRootDomain);
  gIOPMRootDomain->claimSystemWakeEvent(gIOPMRootDomain, kIOPMWakeEventAOTExit,
                                        reason, NULL);
}

IOReturn IOPMrootDomain::setWakeTime(uint64_t wakeContinuousTime) {
  if (kIOPMAOTModeCycle & _aotMode) {
    return kIOReturnSuccess;
  }
...
IOReturn IOPMrootDomain::_setWakeTime(uint64_t wakeContinuousTime) {
...
bool IOPMrootDomain::aotShouldExit(bool software) {
  bool exitNow = false;
  const char *reason = "";
  ...
  if (exitNow) {
    ...
    IOLog(LOG_PREFIX
          ...
          reason, _aotMetrics->sleepCount, _aotMetrics->possibleCount,
          _aotMetrics->confirmedPossibleCount,
          _aotMetrics->rejectedPossibleCount, _aotMetrics->expiredPossibleCount,
          _aotMetrics->noTimeSetCount, _aotMetrics->rtcAlarmsCount);
  }
  return exitNow;
}

void IOPMrootDomain::aotExit(bool cps) {
...
void IOPMrootDomain::aotEvaluate(IOTimerEventSource *timer) {
  bool exitNow;
...
void IOPMrootDomain::adjustPowerState(bool sleepASAP) {
  DEBUG_LOG("adjustPowerState %s, asap %d, idleSleepEnabled %d, _aotNow %d\n",
            getPowerStateString((uint32_t)getPowerState()), sleepASAP,
            idleSleepEnabled, _aotNow);
  ...
  if (_aotNow) {
    ...
    WAKEEVENT_LOCK();
    exitNow = aotShouldExit(false);
    if (!exitNow && !_aotTimerScheduled &&
        (kIOPMWakeEventAOTPossibleExit ==
         (kIOPMWakeEventAOTPossibleFlags & _aotPendingFlags))) {
    ...
    if (exitNow) {
      ...
      if (!_aotTimerScheduled) {
        if (kIOPMDriverAssertionLevelOn ==
            getPMAssertionLevel(kIOPMDriverAssertionCPUBit)) {
          // Don't try to force sleep during AOT while IOMobileFramebuffer is
          // holding a power assertion. Doing so will result in the sleep being
          // cancelled anyway, but this check avoids unnecessary thrashing in
          // the power state engine.
          return;
        }
        privateSleepSystem(kIOPMSleepReasonSoftware);
      }
...
void IOPMrootDomain::handleSetDisplayPowerOn(bool powerOn) {
  if (powerOn) {
    if (!checkSystemCanSustainFullWake()) {
...
void IOPMrootDomain::dispatchPowerEvent(uint32_t event, void *arg0,
                                        uint64_t arg1) {
  ASSERT_GATED();
  ...
  case kPowerEventSystemBootCompleted:
    ...
    if (systemBooting) {
      ...
      if (PE_get_default("sleep-disabled", &gSleepDisabledFlag,
                         sizeof(gSleepDisabledFlag))) {
        DLOG("Setting gSleepDisabledFlag to %u from device tree\n",
             gSleepDisabledFlag);
        if (test_sleep_in_vm && gSleepDisabledFlag) {
          DLOG(
              "Clearing gSleepDisabledFlag due to test_sleep_in_vm boot-arg\n");
  ...
  case kPowerEventAssertionCreate:
    DMSG("power event %u args %p 0x%llx\n", event, OBFUSCATE(arg0), arg1);
    if (pmAssertions) {
      pmAssertions->handleCreateAssertion(
          (OSValueObject<PMAssertStruct> *)arg0);
    }
    break;
  ...
  case kPowerEventAssertionSetLevel:
    DMSG("power event %u args %p 0x%llx\n", event, OBFUSCATE(arg0), arg1);
    if (pmAssertions) {
      pmAssertions->handleSetAssertionLevel(
          arg1, (IOPMDriverAssertionLevel)(uintptr_t)arg0);
    }
    break;
...
IOReturn IOPMrootDomain::systemPowerEventOccurred(const OSSymbol *event,
                                                  uint32_t intValue) {
  ...
  attempt =
      systemPowerEventOccurred(event, static_cast<OSObject *>(newNumber.get()));
...
void IOPMrootDomain::setThermalState(OSObject *value) {
  ...
  if (gIOPMWorkLoop->inGate() == false) {
    gIOPMWorkLoop->runAction(
        OSMemberFunctionCast(IOWorkLoop::Action, this,
                             &IOPMrootDomain::setThermalState),
        (OSObject *)this, (void *)value);

    return;
  }
  if (value && (num = OSDynamicCast(OSNumber, value))) {
    thermalWarningState =
        ((num->unsigned32BitValue() == kIOPMThermalLevelWarning) ||
         (num->unsigned32BitValue() == kIOPMThermalLevelTrap))
            ? 1
            : 0;
  }
}
...
IOReturn IOPMrootDomain::systemPowerEventOccurred(const OSSymbol *event,
  ...
  // the very infrequently used publish/remove feature mechanism; so there's
  // zero rsk of stepping on that lock.
  if (featuresDictLock) {
    IOLockLock(featuresDictLock);
  }

  OSSharedPtr<OSObject> origThermalsProp =
      copyProperty(kIOPMRootDomainPowerStatusKey);
  OSDictionary *origThermalsDict =
      OSDynamicCast(OSDictionary, origThermalsProp.get());
...
exit:
  ...
  if (shouldUpdate) {
    if (event && event->isEqualTo(kIOPMThermalLevelWarningKey)) {
...
IOReturn IOPMrootDomain::receivePowerNotification(UInt32 msg) {
  if (msg & kIOPMPowerButton) {
    ...
    if (currentPhase != kIOPMTracePointSystemUp &&
        currentPhase > kIOPMTracePointSystemSleep) {
...
void IOPMrootDomain::handlePowerNotification(UInt32 msg) {
  ...
  if (msg & kIOPMPowerEmergency) {
    ...
    if (!ml_is_secure_hib_supported() || ldmHibernateDisable) {
      ...
      // in transition.
      if ((_systemTransitionType == kSystemTransitionNone) &&
          CAP_CURRENT(kIOPMSystemCapabilityCPU) && !systemBooting &&
          !systemShutdown && !gWillShutdown) {
        // Setting lowBatteryCondition will prevent system sleep
        lowBatteryCondition = true;

        // Notify userspace to initiate system shutdown
        DLOG("Initiating userspace shutdown ml_is_secure_hib_supported %d "
             "lockdownMode %d",
             ml_is_secure_hib_supported(), ldmHibernateDisable);
  ...
  if (msg & kIOPMProModeEngaged) {
    int newState = 1;
    DLOG("ProModeEngaged\n");
    messageClient(kIOPMMessageProModeStateChange,
                  systemCapabilityNotifier.get(), &newState, sizeof(newState));
  }
...
void IOPMrootDomain::evaluatePolicy(int stimulus, uint32_t arg) {
  ...
  case kStimulusAggressivenessChanged: {
    ...
    DLOG("aggressiveness changed: system %u->%u, display %u\n", sleepSlider,
         minutesToIdleSleep, minutesToDisplayDim);

    DLOG("idle time -> %d ms (ena %d)\n", idleMilliSeconds,
         (minutesToIdleSleep != 0));
  ...
  if (flags.bit.evaluateDarkWake && (kFullWakeReasonNone == fullWakeReason)) {
    DLOG("DarkWake: sleepASAP %d, clamshell closed %d, disabled %d/%x, "
         "desktopMode %d, ac %d\n",
         darkWakeToSleepASAP, clamshellClosed, clamshellDisabled,
         clamshellSleepDisableMask, desktopMode, acAdaptorConnected);
    if (darkWakeToSleepASAP ||
      ...
      if (checkSystemCanSleep(kIOPMSleepReasonIdle)) {
        ...
        changePowerStateWithTagToPriv(getRUN_STATE(),
...
unsigned int IOPMrootDomain::idleSleepPreventersCount() {
  if (_aotMode) {
...
void IOPMrootDomain::requestFullWake(FullWakeReason reason) {
  ...
  if (!CAP_HIGHEST(kIOPMSystemCapabilityGraphics) || _pendingCapability == 0) {
...
void IOPMrootDomain::willEnterFullWake(void) {
  ...
  if ((_highestCapability & kIOPMSystemCapabilityGraphics) == 0) {
    ...
    setProperty(gIOPMUserTriggeredFullWakeKey.get(),
                (kFullWakeReasonLocalUser == fullWakeReason) ? kOSBooleanTrue
                                                             : kOSBooleanFalse);
  }
#if HIBERNATION
...
void IOPMrootDomain::fullWakeDelayedWork(void) {
  ...
  DLOG("fullWakeDelayedWork cap cur %x pend %x high %x, clamshell disable "
       "%x/%x\n",
       _currentCapability, _pendingCapability, _highestCapability,
       clamshellDisabled, clamshellSleepDisableMask);

  if (clamshellExists && CAP_CURRENT(kIOPMSystemCapabilityGraphics) &&
...
void IOPMrootDomain::evaluateAssertions(IOPMDriverAssertionType newAssertions,
  ...
  if (changedBits & kIOPMDriverAssertionPreventDisplaySleepBit) {
    if (wrangler) {
      bool value = (newAssertions & kIOPMDriverAssertionPreventDisplaySleepBit)
                       ? true
                       : false;
  ...
  if (changedBits & kIOPMDriverAssertionCPUBit) {
    ...
    evaluatePolicy(_aotNow ? kStimulusNoIdleSleepPreventers
  ...
  if (changedBits & NO_IDLE_SLEEP_ASSERTIONS_MASK) {
    if ((newAssertions & NO_IDLE_SLEEP_ASSERTIONS_MASK) != 0) {
      if ((oldAssertions & NO_IDLE_SLEEP_ASSERTIONS_MASK) == 0) {
        ...
        if (ok &&
            (changedBits & kIOPMDriverAssertionPreventSystemIdleSleepBit)) {
...
void IOPMrootDomain::pmStatsRecordEvent(int eventIndex,
...
void IOPMrootDomain::pmStatsRecordApplicationResponse(
    ...
    uint32_t delay_ms, uint64_t id, OSObject *object,
    IOPMPowerStateIndex powerState, bool async) {
  ...
  if (responseDescription) {
    ...
    if (response->isEqualTo(gIOPMStatsDriverPSChangeSlow.get())) {
      ...
      if (messageType <
          (sizeof(driverCallTypes) / sizeof(driverCallTypes[0]))) {
        DLOG("%s[0x%qx]::%s(%u) %stook %d ms\n", name, id,
             driverCallTypes[messageType], (uint32_t)powerState,
    ...
    if (_systemTransitionType == kSystemTransitionSleep) {
      responseDescription->setObject(kIOPMStatsSystemTransitionKey,
                                     sleep.get());
...
IOReturn IOPMrootDomain::callPlatformFunction(const OSSymbol *functionName,
  ...
  if (pmTracer && functionName &&
    ...
    if ((tracePointPhases & 0xff) == kIOPMTracePointSystemSleep) {
      OSSharedPtr<IORegistryEntry> node =
      ...
      if (node) {
        ...
        if (data && data->getLength() == sizeof(bootFailureCode)) {
          ...
          memcpy(&bootFailureCode, data->getBytesNoCopy(),
  ...
  return super::callPlatformFunction(functionName, waitForFunction, param1,
                                     param2, param3, param4);
}

void IOPMrootDomain::kdebugTrace(uint32_t event, uint64_t id, uintptr_t param1,
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE, code, (uintptr_t)regId, param1,
                            param2, param3, 0);
}
...
static void kext_log_putc(char c) {
...
static int kext_log(const char *fmt, ...) {
  va_list listp;
...
copyKextIdentifierWithAddress(vm_address_t address) {
  ...
  OSKext::printKextsInBacktrace(&address, 1, kext_log,
                                OSKext::kPrintKextsLock |
                                    OSKext::kPrintKextsTerse);
  gKextNameBuf[sizeof(gKextNameBuf) - 1] = 0;
  identifer = OSSymbol::withCString(
      (gKextNameBuf[0] != 0) ? gKextNameBuf : kOSKextKernelIdentifier);
...
const char *IOPMrootDomain::getNotificationClientName(OSObject *object) {
  ...
  if (!notifier->clientName) {
    ...
    if (systemCapabilityNotifier &&
        (((IOPMServiceInterestNotifier *)systemCapabilityNotifier.get())
             ->handler == notifier->handler)) {
      OSNumber *clientID = NULL;
      messageClient(kIOMessageCopyClientID, object, &clientID);
      if (clientID) {
        OSSharedPtr<OSString> string(
            IOCopyLogNameForPID(clientID->unsigned32BitValue()), OSNoRetain);
        if (string) {
...
void IOPMrootDomain::traceNotification(OSObject *object, bool start,
                                       uint64_t timestamp, uint32_t msgIndex) {
  IOPMServiceInterestNotifier *notifier;
  ...
  if (start) {
    ...
    kdebugTrace(kPMLogSleepWakeMessage, pmTracer->getTracePhase(),
                (uintptr_t)notifier->msgType, (uintptr_t)notifier->uuid0,
                (uintptr_t)notifier->uuid1);
    ...
    if (msgIndex != UINT_MAX) {
      DLOG("%s[%u] to %s\n", getIOMessageString(notifier->msgType), msgIndex,
           getNotificationClientName(notifier));
    } else {
      DLOG("%s to %s\n", getIOMessageString(notifier->msgType),
           getNotificationClientName(notifier));
    }
...
void IOPMrootDomain::traceNotificationAck(OSObject *object, uint32_t delay_ms) {
  if (systemBooting) {
    return;
  }
  IOPMServiceInterestNotifier *notifier =
      OSDynamicCast(IOPMServiceInterestNotifier, object);
  if (!notifier) {
    return;
  }
  ...
  DLOG("%s[%u] ack from %s took %d ms\n", getIOMessageString(notifier->msgType),
       notifier->msgIndex, getNotificationClientName(notifier), delay_ms);
  if (delay_ms > notifier->maxAckDelayMS) {
    notifier->maxAckDelayMS = delay_ms;
  }
}

void IOPMrootDomain::traceNotificationResponse(OSObject *object,
                                               uint32_t delay_ms,
  ...
  kdebugTrace(kPMLogDrvResponseDelay, notifier->uuid0,
              (uintptr_t)notifier->uuid1, (uintptr_t)(ack_time_us / 1000),
              (uintptr_t)delay_ms);

  if (ack_time_us == 0) {
    // Client work is done and ack will not be forthcoming
    DLOG("%s[%u] response from %s took %d ms\n",
         getIOMessageString(notifier->msgType), notifier->msgIndex,
         getNotificationClientName(notifier), delay_ms);
  } else {
    // Client needs more time and it must ack within ack_time_us
    DLOG("%s[%u] response from %s took %d ms (ack in %d us)\n",
         getIOMessageString(notifier->msgType), notifier->msgIndex,
         getNotificationClientName(notifier), delay_ms, ack_time_us);
  }
}

void IOPMrootDomain::traceFilteredNotification(OSObject *object) {
...
void IOPMrootDomain::traceDetail(uint32_t msgType, uint32_t msgIndex,
                                 uint32_t delay) {
  if (!systemBooting) {
    uint32_t detail = ((msgType & 0xffff) << 16) | (delay & 0xffff);
    pmTracer->traceDetail(detail);
    kdebugTrace(kPMLogSleepWakeTracePoint, pmTracer->getTracePhase(), msgType,
                delay);
...
void IOPMrootDomain::configureReportGated(uint64_t channel_id, uint64_t action,
                                          void *result) {
  size_t reportSize;
  void **report = NULL;
  uint32_t bktCnt;
...
IOReturn IOPMrootDomain::configureReport(IOReportChannelList *channelList,
  ...
  for (cnt = 0; cnt < channelList->nchannels; cnt++) {
    if ((channelList->channels[cnt].channel_id == kSleepCntChID) ||
      ...
      gIOPMWorkLoop->runAction(
          OSMemberFunctionCast(IOWorkLoop::Action, this,
                               &IOPMrootDomain::configureReportGated),
...
IOReturn IOPMrootDomain::updateReportGated(uint64_t ch_id, void *result,
...
IOReturn IOPMrootDomain::updateReport(IOReportChannelList *channelList,
  ...
  IOBufferMemoryDescriptor *dest =
      OSDynamicCast(IOBufferMemoryDescriptor, (OSObject *)destination);
  unsigned cnt;
  uint64_t ch_id;
  ...
  for (cnt = 0; cnt < channelList->nchannels; cnt++) {
    ...
    if ((ch_id == kAssertDelayChID) || (ch_id == kSleepDelaysChID)) {
      gIOPMWorkLoop->runAction(
          OSMemberFunctionCast(IOWorkLoop::Action, this,
                               &IOPMrootDomain::updateReportGated),
          (OSObject *)this, (void *)ch_id, (void *)result, (void *)dest);
      continue;
    } else if ((ch_id == kSleepCntChID) || (ch_id == kDarkWkCntChID) ||
               (ch_id == kUserWkCntChID)) {
      SIMPLEREPORT_INIT(buf, sizeof(buf), getRegistryEntryID(), ch_id,
...
#define kPMMaxRTCBitfieldSize 32
    ...
    OSPtr<PMTraceWorker> PMTraceWorker::tracer(IOPMrootDomain *owner) {
...
int PMTraceWorker::recordTopLevelPCIDevice(IOService *pciDevice) {
  ...
  if ((deviceName = pciDevice->copyName()) &&
      (pciDeviceBitMappings->getNextIndexOfObject(deviceName.get(), 0) ==
       (unsigned int)-1) &&
...
void PMTraceWorker::traceComponentWakeProgress(uint32_t component,
                                               uint32_t data) {
  switch (component) {
...
void PMTraceWorker::tracePCIPowerChange(change_t type, IOService *service,
                                        uint32_t changeFlags, uint32_t bitNum) {
  uint32_t bitMask;
  ...
  if (bitNum < kPMMaxRTCBitfieldSize) {
    ...
    if (kPowerChangeStart == type) {
      ...
      owner->kdebugTrace(kPMLogPCIDevChangeStart, service->getRegistryEntryID(),
                         traceData32, 0);
    } else {
      traceData32 &= ~bitMask;
      _LOG("PMTrace: Device %s finished - bit %2d mask 0x%08x => 0x%08x\n",
           service->getName(), bitNum, bitMask, traceData32);
      owner->kdebugTrace(kPMLogPCIDevChangeDone, service->getRegistryEntryID(),
                         traceData32, 0);
    }
...
uint8_t PMTraceWorker::getTracePhase() { return tracePhase; }

uint32_t PMTraceWorker::getTraceData() { return traceData32; }
...
PMHaltWorker *PMHaltWorker::worker(void) {
  ...
  do {
    ...
    if (KERN_SUCCESS !=
        kernel_thread_start(&PMHaltWorker::main, (void *)me, &thread)) {
      me->release();
      break;
    }
  ...
  while (true) {
    ...
    if ((deltaTime > kPMHaltTimeoutMS) || timeout) {
      LOG("%s driver %s (0x%llx) took %u ms\n",
          (gPMHaltMessageType == kIOMessageSystemWillPowerOff) ? "PowerOff"
                                                               : "Restart",
          service->getName(), service->getRegistryEntryID(),
          (uint32_t)deltaTime);
      halt_log_enter("PowerOff/Restart handler completed",
                     OSMemberFunctionCast(const void *, service.get(),
  ...
  if (me->service && !me->timeout) {
    ...
    if (nano > 3000000000ULL) {
      ...
      halt_log_enter("PowerOff/Restart still waiting on handler",
                     OSMemberFunctionCast(const void *, me->service,
                                          &IOService::systemWillShutdown),
                     endTime);
...
#define PLACEHOLDER ((OSSet *)gPMHaltArray.get())
  ...
  if (!gPMHaltClientAcknowledgeKey) {
    gPMHaltClientAcknowledgeKey = OSSymbol::withCStringNoCopy("PMShutdown");
    if (!gPMHaltClientAcknowledgeKey) {
      goto done;
    }
  }
  ...
  iter = IORegistryIterator::iterateOver(root, gIOPowerPlane,
                                         kIORegistryIterateRecursively);
  ...
  DLOG("PM nodes %u, maxDepth %u, workers %u\n", totalNodes,
       gPMHaltArray->getCount(), numWorkers);
  ...
  while (gPMHaltDepth >= 0) {
    ...
    waitResult =
        IOLockSleepDeadline(gPMHaltLock, &gPMHaltDepth, deadline, THREAD_UNINT);
...
IOPMrootDomain::createPMAssertion(IOPMDriverAssertionType whichAssertionBits,
  ...
  ret = pmAssertions->createAssertion(whichAssertionBits, assertionLevel,
                                      ownerService, ownerDescription,
                                      &newAssertion);

  if (kIOReturnSuccess == ret) {
#if (DEVELOPMENT || DEBUG)
    if (_aotNow || (kIOLogPMRootDomain & gIOKitDebug)) {
      const char *serviceName = (ownerService && ownerService->reserved)
                                    ? ownerService->getName()
                                    : NULL;
      OSReportWithBacktrace("PMRD: createPMAssertion(0x%qx) %s (%s)",
                            newAssertion, serviceName, ownerDescription);
    }
#endif /* (DEVELOPMENT || DEBUG) */

    const bool waitForWakeup =
        (whichAssertionBits & kIOPMDriverAssertionForceWakeupBit);
    if (waitForWakeup) {
...
#if (DEVELOPMENT || DEBUG)
  if (_aotNow || (kIOLogPMRootDomain & gIOKitDebug)) {
    PMAssertStruct *details =
        pmAssertions->detailsForID(releaseAssertion, NULL);
    if (details) {
      const char *serviceName =
          (details->ownerService && details->ownerService->reserved)
              ? details->ownerService->getName()
              : NULL;
      const char *ownerString = details->ownerString
                                    ? details->ownerString->getCStringNoCopy()
                                    : NULL;
      OSReportWithBacktrace("PMRD: releasePMAssertion(0x%qx) %s (%s)",
                            releaseAssertion, serviceName, ownerString);
    } else {
      OSReportWithBacktrace("PMRD: releasePMAssertion(0x%qx)",
                            releaseAssertion);
    }
  }
#endif /* (DEVELOPMENT || DEBUG) */
...
IOReturn
IOPMrootDomain::setPMAssertionLevel(IOPMDriverAssertionID assertionID,
                                    IOPMDriverAssertionLevel assertionLevel) {
...
IOReturn IOPMrootDomain::acquireDriverKitMatchingAssertion() {
  return gIOPMWorkLoop->runActionBlock(^{
    if (_driverKitMatchingAssertionCount != 0) {
      ...
      if (kSystemTransitionSleep == _systemTransitionType &&
        ...
        // we must also set _driverKitMatchingAssertionCount under the PM
        // workloop lock so that we can cancel sleep immediately The assertion
        // is used so that on release, we reevaluate all assertions
        _driverKitMatchingAssertion =
            createPMAssertion(kIOPMDriverAssertionCPUBit,
                              kIOPMDriverAssertionLevelOn, this, "DK matching");
...
void IOPMrootDomain::releaseDriverKitMatchingAssertion() {
...
IOReturn IOPMrootDomain::acquireDriverKitSyncedAssertion(
    IOService *from, IOPMDriverAssertionID *assertionID) {
  return gIOPMWorkLoop->runActionBlock(^{
    if (kSystemTransitionSleep == _systemTransitionType &&
        !idleSleepRevertible) {
      // system going to sleep
    ...
    // createPMAssertion is asynchronous.
    // we must also set _driverKitSyncedAssertionCount under the PM workloop
    // lock so that we can cancel sleep immediately only
    // kIOPMDriverAssertionCPUBit is used for "synced" assertion
    *assertionID =
        createPMAssertion(kIOPMDriverAssertionCPUBit,
                          kIOPMDriverAssertionLevelOn, this, from->getName());
...
void IOPMrootDomain::releaseDriverKitSyncedAssertion(
...
IOReturn IOPMrootDomain::createPMAssertionSafe(
    IOPMDriverAssertionID *assertionID,
    IOPMDriverAssertionType whichAssertionBits,
    IOPMDriverAssertionLevel assertionLevel, IOService *ownerService,
    const char *ownerDescription) {
  IOReturn ret;
  ...
  ret = gIOPMWorkLoop->runActionBlock(^{
    ...
    id = createPMAssertion(whichAssertionBits, assertionLevel, ownerService,
                           ownerDescription);
...
OSSharedPtr<OSObject> IOPMrootDomain::copyProperty(const char *aKey) const {
  OSSharedPtr<OSObject> obj;
  ...
  /*
   * XXX: We should get rid of "DesktopMode" property  when
   * 'kAppleClamshellCausesSleepKey' is set properly in darwake from sleep. For
   * that, kIOPMEnableClamshell msg has to be issued by DisplayWrangler on
   * darkwake.
   */
  ...
  if (!strcmp(aKey, kIOPMDriverWakeEventsKey)) {
    ...
    if (_systemWakeEventsArray && _systemWakeEventsArray->getCount()) {
      OSSharedPtr<OSCollection> collection =
          _systemWakeEventsArray->copyCollection();
      if (collection) {
  ...
  if (!strcmp(aKey, kIOPMSleepStatisticsAppsKey)) {
    ...
    if (pmStatsAppResponses && pmStatsAppResponses->getCount()) {
      OSSharedPtr<OSCollection> collection =
          pmStatsAppResponses->copyCollection();
      if (collection) {
        array = OSDynamicPtrCast<OSArray>(collection);
      }
    }
...
void IOPMrootDomain::copyShutdownReasonString(char *outBuf, size_t bufSize) {
  WAKEEVENT_LOCK();
  strlcpy(outBuf, gShutdownReasonString, bufSize);
  WAKEEVENT_UNLOCK();
}

void IOPMrootDomain::copyShutdownTime(uint64_t *time) {
  WAKEEVENT_LOCK();
  *time = gShutdownTime;
  WAKEEVENT_UNLOCK();
}
...
void IOPMrootDomain::acceptSystemWakeEvents(uint32_t control) {
  ...
  case kAcceptSystemWakeEvents_Disable:
    ...
    if ((panic_allowed == -1) &&
        (PE_parse_boot_argn("swd_wakereason_panic", &panic_allowed,
                            sizeof(panic_allowed)) == false)) {
      panic_allowed = 0;
    }
...
void IOPMrootDomain::claimSystemWakeEvent(IOService *device, IOOptionBits flags,
                                          const char *reason,
                                          OSObject *details) {
  OSSharedPtr<const OSSymbol> deviceName;
  ...
  kdebugTrace(kPMLogClaimSystemWake, args[0], args[1], args[2],
  ...
  if ((kIOPMAOTModeAddEventFlags & _aotMode) &&
      (!flags || (flags == kIOPMWakeEventSource))) {
    flags |= kIOPMWakeEventAOTExit;

    // Only allow lingering in AOT_STATE for the two wake reasons used for the
    // wrist raise gesture.
    if (!strcmp("AOP.OutboxNotEmpty", reason) ||
        !strcmp("spu_gesture", reason)) {
      flags &= ~kIOPMWakeEventAOTExit;
    }
  }
  ...
  if (!dict || !deviceName || !deviceRegId || !claimTime || !flagsData ||
      !reasonString) {
    goto done;
  }
  ...
  if (_aotMode) {
    IOLog("claimSystemWakeEvent(%s, %s, 0x%x) 0x%x %d\n", reason,
          deviceName->getCStringNoCopy(), (int)flags, _aotPendingFlags,
          _aotReadyToFullWake);
  }
  ...
  if (_aotNow && aotFlags) {
    ...
    addWakeReason = _aotNow && _systemWakeEventsArray &&
                    ((kIOPMWakeEventAOTExitFlags & aotFlags));
    needAOTEvaluate = _aotReadyToFullWake;
  }
  DMSG("claimSystemWakeEvent(%s, 0x%x, %s, 0x%llx) aot %d phase 0x%x add %d\n",
       reason, (int)flags, deviceName->getCStringNoCopy(),
       device->getRegistryEntryID(), _aotNow, pmTracer->getTracePhase(),
       addWakeReason);

#if DEVELOPMENT || DEBUG
  if (addWakeReason) {
    record_system_event(
        SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_PMRD,
        "Report System Wake Event",
        "Reason: %s Flags: 0x%x Device: %s, DeviceRegEntry: 0x%llx\n", reason,
        (int)flags, deviceName->getCStringNoCopy(),
        device->getRegistryEntryID());
  }
#endif /* DEVELOPMENT || DEBUG */
...
void IOPMrootDomain::claimSystemBootEvent(IOService *device, IOOptionBits flags,
                                          const char *reason,
  ...
  record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_PMRD,
                      ...
                      "Reason: %s Flags: 0x%x Device: %s", reason, (int)flags,
                      device->getName());
#endif /* DEVELOPMENT || DEBUG */
  WAKEEVENT_LOCK();
...
void IOPMrootDomain::claimSystemShutdownEvent(IOService *device,
                                              IOOptionBits flags,
                                              const char *reason,
                                              OSObject *details) {
  if (!device || !reason) {
  ...
  if (details) {
    ...
    if (dict) {
      OSSharedPtr<OSString> sharedKey =
          OSString::withCString(kIOPMRootDomainShutdownTime);
      if (sharedKey) {
        OSNumber *num =
            OSDynamicCast(OSNumber, dict->getObject(sharedKey.get()));
        if (num) {
...
IOReturn IOPMrootDomain::requestRunMode(uint64_t runModeMask) {
  // We only support requesting full wake at the moment
  if (runModeMask == kIOPMRunModeFullWake) {
    pmPowerStateQueue->submitPowerEvent(kPowerEventRunModeRequest, NULL,
                                        runModeMask);
    return kIOReturnSuccess;
  }
  return kIOReturnUnsupported;
...
IOReturn IOPMrootDomain::handleRequestRunMode(uint64_t runModeMask) {
...
OSDefineMetaClassAndFinalStructors(PMSettingObject, OSObject)
    ...
    PMSettingObject *PMSettingObject::pmSettingObject(
        IOPMrootDomain *parent_arg, IOPMSettingControllerCallback handler_arg,
        OSObject *target_arg, uintptr_t refcon_arg,
        uint32_t supportedPowerSources, const OSSymbol *settings[],
        OSObject **handle_obj) {
  ...
  pmso->publishedFeatureID =
      OSDataAllocation<uint32_t>(settingCount, OSAllocateMemory);
  if (pmso->publishedFeatureID) {
    for (unsigned int i = 0; i < settingCount; i++) {
      ...
      parent_arg->publishPMSetting(settings[i], supportedPowerSources,
                                   &pmso->publishedFeatureID[i]);
    }
  }
...
IOReturn PMSettingObject::dispatchPMSetting(const OSSymbol *type,
                                            OSObject *object) {
...
void PMAssertionsTracker::tabulate(void) {
  ...
  if ((count = assertionsArray->getCount())) {
    for (i = 0; i < count; i++) {
      _d = OSDynamicCast(OSValueObject<PMAssertStruct>,
                         assertionsArray->getObject(i));
      if (_d) {
  ...
  if ((assertionsKernel != oldKernel) || (assertionsCombined != oldCombined)) {
    owner->evaluateAssertions(assertionsCombined, oldCombined);
  }
}

void PMAssertionsTracker::updateCPUBitAccounting(PMAssertStruct *assertStruct) {
  AbsoluteTime now;
  uint64_t nsec;
...
void PMAssertionsTracker::reportCPUBitAccounting(void) {
  const PMAssertStruct *_a = nullptr;
  OSValueObject<PMAssertStruct> *_d = nullptr;
  int i, count;
  AbsoluteTime now;
...
void PMAssertionsTracker::publishProperties(void) {
  ...
  if (tabulateConsumerCount != tabulateProducerCount) {
    ...
    if (assertionsSummary) {
      owner->setProperty(kIOPMAssertionsDriverDetailedKey,
                         assertionsSummary.get());
    } else {
...
PMAssertStruct *PMAssertionsTracker::detailsForID(IOPMDriverAssertionID _id,
                                                  int *index) {
  ...
  if (assertionsArray && (count = assertionsArray->getCount())) {
    for (i = 0; i < count; i++) {
      _d = OSDynamicCast(OSValueObject<PMAssertStruct>,
                         assertionsArray->getObject(i));
      if (_d) {
 ...
 */
IOReturn PMAssertionsTracker::handleCreateAssertion(
    OSValueObject<PMAssertStruct> *newAssertion) {
 ...
 * createAssertion allocates memory for a new PM assertion, and affects system
 * behavior, if appropiate.
 */
IOReturn PMAssertionsTracker::createAssertion(IOPMDriverAssertionType which,
                                              IOPMDriverAssertionLevel level,
                                              IOService *serviceID,
                                              const char *whoItIs,
  ...
  // Warning: trillions and trillions of created assertions may overflow the
  // unique ID.
  ...
  track.ownerString =
      whoItIs ? OSSymbol::withCString(whoItIs).detach() : nullptr;
  ...
  *outID = track.id;
  ...
  if (owner && owner->pmPowerStateQueue) {
    ...
    owner->pmPowerStateQueue->submitPowerEvent(kPowerEventAssertionCreate,
                                               (void *)dataStore.detach());
  }
 ...
 */
IOReturn
PMAssertionsTracker::handleReleaseAssertion(IOPMDriverAssertionID _id) {
  ASSERT_GATED();
 ...
 */
IOReturn PMAssertionsTracker::releaseAssertion(IOPMDriverAssertionID _id) {
  if (owner && owner->pmPowerStateQueue) {
    owner->pmPowerStateQueue->submitPowerEvent(kPowerEventAssertionRelease,
                                               NULL, _id);
  }
  return kIOReturnSuccess;
}
 ...
 */
IOReturn
PMAssertionsTracker::handleSetAssertionLevel(IOPMDriverAssertionID _id,
 ...
 */
IOReturn
PMAssertionsTracker::setAssertionLevel(IOPMDriverAssertionID _id,
                                       IOPMDriverAssertionLevel _level) {
  if (owner && owner->pmPowerStateQueue) {
...
IOReturn PMAssertionsTracker::setUserAssertionLevels(
  ...
  if (gIOPMWorkLoop) {
    gIOPMWorkLoop->runAction(
        ...
        this, (void *)&new_user_levels, NULL, NULL, NULL);
  }

  return kIOReturnSuccess;
}

OSSharedPtr<OSArray> PMAssertionsTracker::copyAssertionsArray(void) {
  int count;
  int i;
...
IOPMDriverAssertionLevel
PMAssertionsTracker::getAssertionLevel(IOPMDriverAssertionType type) {
...
OSSharedPtr<OSObject> IORootParent::copyProperty(const char *aKey) const {
...
void IOPMrootDomain::tracePhase2String(uint32_t tracePhase,
  ...
  case kIOPMTracePointSleepWillChangeInterests:
    ...
    *description = "creating hibernation file or while calling rootDomain's "
                   "clients about upcoming rootDomain's state changes";
    break;
  ...
  case kIOPMTracePointWakeWillPowerOnClients:
    ...
    *description = "sending kIOMessageSystemWillPowerOn message to kernel and "
                   "userspace clients";
    break;

  case kIOPMTracePointWakeWillChangeInterests:
    *phaseString = "kIOPMTracePointWakeWillChangeInterests";
    *description = "calling rootDomain's clients about upcoming rootDomain's "
                   "state changes";
    break;

  case kIOPMTracePointWakeDidChangeInterests:
    *phaseString = "kIOPMTracePointWakeDidChangeInterests";
    *description = "calling rootDomain's clients about completed rootDomain's "
                   "state changes";
    break;
  ...
  case kIOPMTracePointWakeApplications:
    ...
    *description = "sending asynchronous kIOMessageSystemHasPoweredOn message "
                   "to userspace clients";
    break;
...
void IOPMrootDomain::saveFailureData2File() {
  ...
  OSSharedPtr<OSObject> statusCodeProp =
  ...
  if (statusCode) {
    ...
    if ((phaseData & 0xFF) == kIOPMTracePointSystemSleep) {
      ...
      efiFailure = true;
      failureStr[0] = 0;
      snprintf(failureStr, sizeof(failureStr),
  ...
  if (failureStr[0] != 0) {
    error =
        sleepWakeDebugSaveFile(kSleepWakeFailureStringFile, failureStr, len);
    if (error) {
  ...
  if (swd_buffer) {
    ...
    OSBoundedArrayRef<char> boundedOutBuf(outbuf, hdr->alloc_size -
                                                      hdr->spindump_offset);

    for (int i = 0; i < 8; i++) {
      snprintf(nvram_var_name_buffer, sizeof(nvram_var_name_buffer), "%s%02d",
               SWD_STACKSHOT_VAR_PREFIX, i + 1);
      if (!PEReadNVRAMProperty(nvram_var_name_buffer, NULL, &len)) {
        LOG("No SleepWake blob to read beyond chunk %d\n", i);
        break;
      }
      if (PEReadNVRAMProperty(nvram_var_name_buffer,
                              boundedOutBuf.slice(concat_len, len).data(),
                              &len) == FALSE) {
    ...
    if (concat_len) {
      error =
          sleepWakeDebugSaveFile(kSleepWakeStacksFilename, outbuf, concat_len);
      if (error) {
...
void IOPMrootDomain::getFailureData(thread_t *thread, char *failureStr,
                                    size_t strLen) {
  OSSharedPtr<IORegistryIterator> iter;
  ...
  IOPMServiceInterestNotifier *notifier =
      OSDynamicCast(IOPMServiceInterestNotifier, notifierObject.get());
  uint32_t tracePhase = pmTracer->getTracePhase();

  *thread = NULL;

  delta = get_watchdog_elapsed_time();
  if ((tracePhase < kIOPMTracePointSystemSleep) ||
      (tracePhase == kIOPMTracePointDarkWakeEntry)) {
    snprintf(failureStr, strLen, "Sleep transition timed out after %qd seconds",
             delta);
  } else {
    snprintf(failureStr, strLen, "Wake transition timed out after %qd seconds",
             delta);
  }
  ...
  if (notifierThread) {
    ...
    *thread = notifierThread;
    ...
    iter = IORegistryIterator::iterateOver(getPMRootDomain(), gIOPowerPlane,
                                           kIORegistryIterateRecursively);
  ...
  if (*thread) {
    char threadName[40];
    snprintf(threadName, sizeof(threadName), " Thread 0x%llx.",
             thread_tid(*thread));
...
static int swd_compress(char *inPtr, char *outPtr, size_t numBytes) {
  ...
  if (!swd_zs.zalloc) {
    ...
    if (deflateInit2(&swd_zs, Z_BEST_SPEED, Z_DEFLATED, wbits + 16, memlevel,
                     Z_DEFAULT_STRATEGY)) {
      // allocation failed
  ...
  while (swd_zip_var.error >= 0) {
    if (!zs->avail_in) {
      zs->next_in = (unsigned char *)inPtr ? (Bytef *)inPtr
                                           : (Bytef *)zs; /* zero marker? */
...
void IOPMrootDomain::deleteStackshot() {
  if (!OSCompareAndSwap(0, 1, &gRootDomain->swd_lock)) {
  ...
  for (int i = 0; i < 8; i++) {
    snprintf(nvram_var_name_buf, sizeof(nvram_var_name_buf), "%s%02d",
             SWD_STACKSHOT_VAR_PREFIX, i + 1);
  ...
  // force NVRAM sync
  if (PEWriteNVRAMProperty(
          kIONVRAMSyncNowPropertyKey, kIONVRAMSyncNowPropertyKey,
          (unsigned int)strlen(kIONVRAMSyncNowPropertyKey)) == false) {
    DLOG("Failed to force nvram sync\n");
  }
  gRootDomain->swd_lock = 0;
}

void IOPMrootDomain::takeStackshot(bool wdogTrigger) {
  swd_hdr *hdr = NULL;
  int cnt = 0;
  int max_cnt;
  pid_t pid = 0;
  ...
  failureStr[0] = 0;
  if ((kIOSleepWakeWdogOff & gIOKitDebug) || systemBooting || systemShutdown ||
      gWillShutdown) {
    return;
  }
  ...
  flags = STACKSHOT_KCDATA_FORMAT | STACKSHOT_NO_IO_STATS |
  ...
  while (success == 0 && cnt < max_cnt) {
    ...
    size = bytesRemaining;
    kr = stack_snapshot_from_kernel(pid, dstAddr, size, flags, 0, 0,
                                    &bytesWritten);
    DLOG("stack_snapshot_from_kernel returned 0x%x. pid: %d bufsize:0x%x "
         "flags:0x%llx bytesWritten: %d\n",
    ...
    if (kr == KERN_SUCCESS) {
      ...
      // Compress stackshot and save to NVRAM
      {
        ...
        outlen = swd_compress((char *)hdr + hdr->spindump_offset, outbuf,
                              bytesWritten);
        ...
        if (outlen) {
          ...
          if (max_chunks < 8) {
            for (num_chunks = 0; num_chunks < max_chunks; num_chunks++) {
              snprintf(nvram_var_name_buffer, sizeof(nvram_var_name_buffer),
                       "%s%02d", SWD_STACKSHOT_VAR_PREFIX, num_chunks + 1);
              if (PEWriteNVRAMPropertyWithCopy(
                      nvram_var_name_buffer,
                      (outbuf + (num_chunks * (2096 - 200))),
                      (2096 - 200)) == FALSE) {
            ...
            if (pid == -1) {
              ...
              LOG("Compressed failure stackshot of kernel+dexts is too large "
                  "size=%d bytes\n",
                  outlen);
              flags = flags & ~STACKSHOT_INCLUDE_DRIVER_THREADS_IN_KERNEL;
            } else {
              LOG("Compressed failure stackshot of only kernel is too large "
                  "size=%d bytes\n",
                  outlen);
              break;
            }
  ...
  if (failureStr[0]) {
    ...
    if (PEWriteNVRAMProperty(
            kIOSleepWakeFailureString, failureStr,
            (unsigned int)strnlen(failureStr, sizeof(failureStr))) == false) {
...
skip_stackshot:
  if (wdogTrigger) {
    if (PEGetCoprocessorVersion() < kCoprocessorVersion2) {
      if (swd_flags & SWD_BOOT_BY_SW_WDOG) {
        // If current boot is due to this watch dog trigger restart in previous
        // boot, then don't trigger again until at least 1 successful sleep &
        // wake.
    ...
    if (!concise &&
        (PEWriteNVRAMProperty(kIOSleepWakeFailurePanic, swfPanic,
                              (unsigned int)strlen(swfPanic)) == false)) {
      DLOG("Failed to write SleepWake failure panic key\n");
    }
#if defined(__x86_64__)
    if (thread) {
      panic_with_thread_context(0, NULL,
                                DEBUGGER_OPTION_ATTEMPTCOREDUMPANDREBOOT,
                                thread, "%s", failureStr);
    } else
#endif /* defined(__x86_64__) */
    {
      panic_with_options(0, NULL, DEBUGGER_OPTION_ATTEMPTCOREDUMPANDREBOOT,
                         "%s", failureStr);
    }
  } else {
    gRootDomain->swd_lock = 0;
...
void IOPMrootDomain::sleepWakeDebugMemAlloc() {
  vm_size_t size =
      SWD_STACKSHOT_SIZE + SWD_COMPRESSED_BUFSIZE + SWD_ZLIB_BUFSIZE;
  ...
  memDesc = IOBufferMemoryDescriptor::inTaskWithOptions(
      kernel_task, kIODirectionIn | kIOMemoryMapperNone, size);
  if (memDesc == NULL) {
...
void IOPMrootDomain::sleepWakeDebugSpinDumpMemAlloc() {
  ...
  memDesc = IOBufferMemoryDescriptor::inTaskWithOptions(
      kernel_task, kIODirectionIn | kIOMemoryMapperNone, SWD_SPINDUMP_SIZE);
...
errno_t IOPMrootDomain::sleepWakeDebugSaveFile(const char *name, char *buf,
                                               int len) {
  ...
  if (vnode_open(name, (O_CREAT | FWRITE | O_NOFOLLOW),
                 S_IRUSR | S_IRGRP | S_IROTH, VNODE_LOOKUP_NOFOLLOW, &vp,
                 ctx) != 0) {
  ...
  if (vp->v_type != VREG || vnode_getattr(vp, &va, ctx) || va.va_nlink != 1) {
  ...
  if (buf != NULL) {
    error = vn_rdwr(UIO_WRITE, vp, buf, len, 0, UIO_SYSSPACE,
                    IO_NODELOCKED | IO_UNIT, cred, (int *)NULL,
...
void IOPMrootDomain::takeStackshot(bool restart) {
...
bool IOPMrootDomain::sleepWakeDebugIsWdogEnabled() { return false; }
