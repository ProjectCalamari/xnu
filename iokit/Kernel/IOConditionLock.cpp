...
bool IOConditionLock::getInterruptible() const { return interruptible; }

int IOConditionLock::getCondition() const { return condition; }
...
void IOConditionLock::unlock() {
  ...
  if (waiting) {
    waiting = false;
    IOLockWakeup(sleep_interlock, this,
                 /* one-thread */ false); // Wakeup everybody
  }
