...
bool IOEventSource::init(OSObject *inOwner, Action inAction) {
  if (!inOwner) {
    return false;
  }
...
void IOEventSource::setRefcon(void *newrefcon) { refcon = newrefcon; }

void *IOEventSource::getRefcon() const { return refcon; }
...
void IOEventSource::setNext(IOEventSource *inNext) { eventChainNext = inNext; }
...
void IOEventSource::disable() { enabled = false; }

bool IOEventSource::isEnabled() const { return enabled; }
...
IOWorkLoop *IOEventSource::getWorkLoop() const { return workLoop; }
