 ...
 * bsd level, grabs its extents and searches for a polling driver willing to
 * work with that IOMedia. The BSD code makes an ioctl to the storage driver to
 * get the partition base offset to the disk, and other ioctls to get the
 * transfer constraints If successful, the file is written to make sure its
 * initially not bootable (in case of later failure) and nvram set to point to
 * the first block of the file. (Has to be done here so blocking is possible in
 * nvram support). hibernate_setup() in osfmk is called to allocate page bitmaps
 * for all dram, and page out any pages it wants to (currently zero, but
 * probably some percentage of memory). Its assumed just allocating pages will
 * cause the VM system to naturally select the best pages for eviction. It also
 * copies processor flags needed for the restore path and sets a flag in the
 * boot processor proc info. gIOHibernateState = kIOHibernateStateHibernating.
 *  - Regular sleep progresses - some drivers may inspect the root domain
 * property kIOHibernateStateKey to modify behavior. The platform driver saves
 * state to memory as usual but leaves motherboard I/O on.
 *  - Eventually the platform calls ml_ppc_sleep() in the shutdown context on
 * the last cpu, at which point memory is ready to be saved.
 * mapping_hibernate_flush() is called to get all ppc RC bits out of the hash
 * table and caches into the mapping structures.
 *  - hibernate_write_image() is called (still in shutdown context, no blocking
 * or preemption). hibernate_page_list_setall() is called to get a bitmap of
 * dram pages that need to be saved. All pages are assumed to be saved (as part
 * of the wired image) unless explicitly subtracted by
 * hibernate_page_list_setall(), avoiding having to find arch dependent low
 * level bits. The image header and block list are written. The header includes
 * the second file extent so only the header block is needed to read the file,
 * regardless of filesystem. The kernel segment "__HIB" is written uncompressed
 * to the image. This segment of code and data (only) is used to decompress the
 * image during wake/boot. Some additional pages are removed from the bitmaps -
 * the buffers used for hibernation. The bitmaps are written to the image. More
 * areas are removed from the bitmaps (after they have been written to the
 * image) - the segment "__HIB" pages and interrupt stack. Each wired page is
 * compressed and written and then each non-wired page. Compression and disk
 * writes are in parallel. The image header is written to the start of the file
 * and the polling driver closed. The machine powers down (or sleeps).
 *
 *  Boot/Wake:
 *
 *  - BootX sees the boot-image nvram variable containing the device and block
 * number of the image, reads the header and if the signature is correct
 * proceeds. The boot-image variable is cleared.
 *  - BootX reads the portion of the image used for wired pages, to memory. Its
 * assumed this will fit in the OF memory environment, and the image is
 * decrypted. There is no decompression in BootX, that is in the kernel's __HIB
 * section.
 *  - BootX copies the "__HIB" section to its correct position in memory,
 * quiesces and calls its entry hibernate_kernel_entrypoint(), passing the
 * location of the image in memory. Translation is off, only code & data in that
 * section is safe to call since all the other wired pages are still compressed
 * in the image.
 *  - hibernate_kernel_entrypoint() removes pages occupied by the raw image from
 * the page bitmaps. It uses the bitmaps to work out which pages can be
 * uncompressed from the image to their final location directly, and copies
 * those that can't to interim free pages. When the image has been completed,
 * the copies are uncompressed, overwriting the wired image pages.
 *  hibernate_restore_phys_page() (in osfmk since its arch dependent, but part
 * of the "__HIB" section) is used to get pages into place for 64bit.
 *  - the reset vector is called (at least on ppc), the kernel proceeds on a
 * normal wake, with some changes conditional on the per proc flag - before VM
 * is turned on the boot cpu, all mappings are removed from the software
 * strutures, and the hash table is reinitialized.
 ...
 * other threads can run or any devices are turned on. This reduces the memory
 * usage for BootX and allows decompression in parallel with disk reads, for the
 * remaining non wired pages.
 *  - The polling driver is closed down and regular wake proceeds. When the
 * kernel calls iokit to wake (normal execution context) hibernate_teardown() in
 * osmfk is called to release any memory, the file is closed via bsd.
 *
 *  Polled Mode I/O:
 *
 *  IOHibernateSystemSleep() finds a polled mode interface to the ATA controller
 * via a property in the registry, specifying an object of calls
 * IOPolledInterface.
 *
 *  Before the system goes to sleep it searches from the IOMedia object (could
 * be a filesystem or partition) that the image is going to live, looking for
 * polled interface properties. If it finds one the IOMedia object is passed to
 * a "probe" call for the interface to accept or reject. All the interfaces
 * found are kept in an ordered list.
 *
 *  There is an Open/Close pair of calls made to each of the interfaces at
 * various stages since there are few different contexts things happen in:
 *
 *  - there is an Open/Close (Preflight) made before any part of the system has
 * slept (I/O is all up and running) and after wake - this is safe to allocate
 * memory and do anything. The device ignores sleep requests from that point
 * since its a waste of time if it goes to sleep and immediately wakes back up
 * for the image write.
 *
 *  - there is an Open/Close (BeforeSleep) pair made around the image write
 * operations that happen immediately before sleep. These can't block or
 * allocate memory - the I/O system is asleep apart from the low level bits
 * (motherboard I/O etc). There is only one thread running. The close can be
 *  used to flush and set the disk to sleep.
 *
 *  - there is an Open/Close (AfterSleep) pair made around the image read
 * operations that happen immediately after sleep. These can't block or allocate
 * memory. This is happening after the platform expert has woken the low level
 * bits of the system, but most of the I/O system has not. There is only one
 * thread running.
 *
 *  For the actual I/O, all the ops are with respect to a single
 * IOMemoryDescriptor that was passed (prepared) to the Preflight Open() call.
 * There is a read/write op, buffer offset to the IOMD for the data, an offset
 * to the disk and length (block aligned 64 bit numbers), and completion
 * callback. Each I/O is async but only one is ever outstanding. The polled
 * interface has a checkForWork call that is called for the hardware to check
 * for events, and complete the I/O via the callback. The hibernate path uses
 * the same transfer constraints the regular cluster I/O path in BSD uses to
 * restrict I/O ops.
 */
...
uint32_t gIOHibernateFreeTime =
    0 * 1000; // max time to spend freeing pages (ms)

enum {
  HIB_COMPR_RATIO_ARM64 = (0xa5), // compression ~65%. Since we don't support
                                  // retries we start higher.
...
static IOReturn IOWriteExtentsToFile(IOPolledFileIOVars *vars,
                                     uint32_t signature);
...
#if CONFIG_SPTM
/**
 * Copies the handoff pages in the order passed in, into the
 * already-IOKit-allocated handoff region memory pages.
 *
 * @param page_array The source page array to use that contains the handoff
 * region's pages.
 * @param page_count The number of pages to copy from the page array.
 */
void HibernationCopyHandoffRegionFromPageArray(uint32_t page_array[],
                                               uint32_t page_count) {
  ...
  for (unsigned i = 0; i < page_count; i++) {
     ...
     */
    memcpy(&copyDest[i * PAGE_SIZE], (void *)phystokv(ptoa_64(page_array[i])),
           PAGE_SIZE);
  }
}
#endif /* CONFIG_SPTM */
...
static IOReturn IOMemoryDescriptorWriteFromPhysical(IOMemoryDescriptor *md,
                                                    IOByteCount offset,
                                                    addr64_t bytes,
                                                    IOByteCount length) {
...
static IOReturn IOMemoryDescriptorReadToPhysical(IOMemoryDescriptor *md,
                                                 IOByteCount offset,
                                                 addr64_t bytes,
                                                 IOByteCount length) {
...
void hibernate_set_page_state(hibernate_page_list_t *page_list,
                              hibernate_page_list_t *page_list_wired,
                              vm_offset_t ppnum, vm_offset_t count,
                              uint32_t kind) {
  count += ppnum;
...
static void hibernate_set_descriptor_page_state(IOHibernateVars *vars,
  ...
  IOByteCount segLen;
  if (descriptor) {
    for (count = 0; (phys64 = descriptor->getPhysicalSegment(
                         count, &segLen, kIOMemoryMapperNone));
         count += segLen) {
      hibernate_set_page_state(vars->page_list, vars->page_list_wired,
                               atop_64(phys64), atop_32(segLen), kind);
      *pageCount -= atop_32(segLen);
    }
  }
}

static vm_offset_t hibernate_page_list_iterate(hibernate_page_list_t *list,
                                               ppnum_t *pPage) {
...
IOReturn IOHibernateSystemSleep(void) {
  ...
  if (!gIOHibernateTrimCalloutEntry) {
    gIOHibernateTrimCalloutEntry =
        thread_call_allocate(&IOHibernateSystemPostWakeTrim, &gFSLock);
  }
  ...
  swapPinned = false;
  do {
    vars->srcBuffer = IOBufferMemoryDescriptor::withOptions(
        kIODirectionOutIn, HIBERNATION_SRC_BUFFER_SIZE, page_size);

    vars->handoffBuffer = IOBufferMemoryDescriptor::withOptions(
        kIODirectionOutIn, ptoa_64(gIOHibernateHandoffPageCount), page_size);
    ...
    if ((obj = IOService::getPMRootDomain()->copyProperty(
             kIOHibernateFileMinSizeKey))) {
    ...
    vmflush = ((kOSBooleanTrue == IOService::getPMRootDomain()->getProperty(
                                      kIOPMDeepSleepEnabledKey)));
    err = hibernate_alloc_page_lists(&vars->page_list, &vars->page_list_wired,
                                     &vars->page_list_pal);
    if (KERN_SUCCESS != err) {
    ...
    if (vars->fileMinSize || (kIOHibernateModeFileResize & gIOHibernateMode)) {
      hibernate_page_list_setall(vars->page_list, vars->page_list_wired,
                                 vars->page_list_pal, true /* preflight */,
                                 vmflush /* discard */, &pageCount);
      ...
      setFileSizeMin =
          ((ptoa_64((106 * pageCount) / 100) * gIOHibernateCompression) >> 8) +
...
#endif
      HIBLOG("hibernate_page_list_setall preflight pageCount %d est comp %qd "
             "setfilemin %qd setfilemax %qd min %qd\n",
             pageCount, (100ULL * gIOHibernateCompression) >> 8, setFileSizeMin,
             setFileSizeMax, vars->fileMinSize);
...
#if TEST_XXX
      {
        vm_size_t extraAlloc =
            2ULL * 1024ULL * 1024ULL * 1024ULL / sizeof(uint64_t);
    ...
    err = IOPolledFileOpen(
        gIOHibernateFilename, (kIOPolledFileCreate | kIOPolledFileHibernate),
        setFileSizeMin, setFileSizeMax, kIOHibernateDiskFreeSpace,
        gIOHibernateCurrentHeader, sizeof(gIOHibernateCurrentHeader),
        &vars->fileVars, &nvramData, &vars->volumeCryptKey[0],
        &vars->volumeCryptKeySize);
    ...
    err = hibernate_setup(gIOHibernateCurrentHeader, vmflush, vars->page_list,
                          vars->page_list_wired, vars->page_list_pal);
    ...
    HIBLOG("hibernate_setup(%d) took %qd ms, swapPin(%d) ssd(%d)\n", err,
           nsec / 1000000ULL, haveSwapPin, hibFileSSD);
    if (KERN_SUCCESS != err) {
      OSSafeReleaseNULL(nvramData);
      break;
    }
    ...
    dsSSD = ((0 != (kIOPolledFileSSD & vars->fileVars->flags)) &&
             (kOSBooleanTrue == IOService::getPMRootDomain()->getProperty(
                                    kIOPMDeepSleepEnabledKey)));

    if (dsSSD) {
      gIOHibernateCurrentHeader->options |=
          kIOHibernateOptionSSD | kIOHibernateOptionColor;
    } else {
    ...
    if (vars->volumeCryptKeySize &&
      ...
      OSData *smcData;
      smcData = OSData::withBytesNoCopy(&gIOHibernateVars.volumeCryptKey[0],
                                        (unsigned int)vars->volumeCryptKeySize);
      if (smcData) {
        smcData->setSerializable(false);
        IOService::getPMRootDomain()->setProperty(kIOHibernateSMCVariablesKey,
                                                  smcData);
        smcData->release();
      }
    }
    ...
    if (kIOHibernateOptionProgress & gIOHibernateCurrentHeader->options) {
      ...
      if (KERN_SUCCESS !=
          kmem_alloc(kernel_map, &vars->videoMapping, vars->videoAllocSize,
                     (kma_flags_t)(KMA_PAGEABLE | KMA_DATA_SHARED),
...
#if defined(__i386__) || defined(__x86_64__)
    {
      ...
      rtcVars.revision = 1;
      bcopy(&vars->wiredCryptKey[0], &rtcVars.wiredCryptKey[0],
            sizeof(rtcVars.wiredCryptKey));

      if (gIOChosenEntry &&
          (data = OSDynamicCast(OSData, gIOChosenEntry->getProperty(
                                            gIOHibernateBootSignatureKey))) &&
          (sizeof(rtcVars.booterSignature) <= data->getLength())) {
        bcopy(data->getBytesNoCopy(), &rtcVars.booterSignature[0],
              sizeof(rtcVars.booterSignature));
      } else if (gIOHibernateBootSignature[0]) {
        char c;
        uint8_t value = 0;
        uint32_t in, out, digits;
        for (in = out = digits = 0; (c = gIOHibernateBootSignature[in]) &&
                                    (in < sizeof(gIOHibernateBootSignature));
             in++) {
          if ((c >= 'a') && (c <= 'f')) {
            c -= 'a' - 10;
          } else if ((c >= 'A') && (c <= 'F')) {
      ...
      if (kIOLogHibernate & gIOKitDebug) {
        IOKitKernelLogBuffer("H> rtc:", &rtcVars, sizeof(rtcVars), &kprintf);
      }
      ...
      if (data) {
        if (gIOHibernateRTCVariablesKey) {
          IOService::getPMRootDomain()->setProperty(gIOHibernateRTCVariablesKey,
                                                    data);
        }
        data->release();
      }
      if (gIOChosenEntry && gIOOptionsEntry) {
        ...
        if (data) {
          gIOHibernateCurrentHeader->machineSignature =
        ...
        if (!gIOHibernateBoot0082Data) {
          ...
          data = OSDynamicCast(OSData,
                               gIOChosenEntry->getProperty("boot-device-path"));
          if (data && data->getLength() >= 4) {
            fileData = OSDynamicCast(
                OSData, gIOChosenEntry->getProperty("boot-file-path"));
          }
          if (data && (data->getLength() <= UINT16_MAX)) {
            ...
            gIOHibernateBoot0082Data = OSData::withCapacity(
                sizeof(loadOptionHeader) + loadOptionHeader.FilePathLength);
            if (gIOHibernateBoot0082Data) {
              gIOHibernateBoot0082Data->appendValue(loadOptionHeader);
              if (fileData) {
                gIOHibernateBoot0082Data->appendBytes(data->getBytesNoCopy(),
                                                      data->getLength() - 4);
...
#if DEBUG || DEVELOPMENT
        if (kIOLogHibernate & gIOKitDebug) {
          IOKitKernelLogBuffer(
              "H> bootnext:", gIOHibernateBoot0082Data->getBytesNoCopy(),
              gIOHibernateBoot0082Data->getLength(), &kprintf);
        }
#endif /* DEBUG || DEVELOPMENT */
        if (gIOHibernateBoot0082Key && gIOHibernateBoot0082Data &&
            gIOHibernateBootNextKey && gIOHibernateBootNextData) {
          gIOHibernateBootNextSave =
              gIOOptionsEntry->copyProperty(gIOHibernateBootNextKey);
          gIOOptionsEntry->setProperty(gIOHibernateBoot0082Key,
  ...
  if (kIOReturnSuccess == err) {
    ...
    if (kIOLogHibernate & gIOKitDebug) {
      OSData *data =
          OSDynamicCast(OSData, IOService::getPMRootDomain()->getProperty(
                                    kIOHibernateSMCVariablesKey));
      if (data) {
        IOKitKernelLogBuffer("H> smc:", data->getBytesNoCopy(),
                             data->getLength(), &kprintf);
      }
    }
#endif /* defined(__i386__) || defined(__x86_64__) */
#endif /* DEBUG || DEVELOPMENT */
    ...
    IOPolledFileClose(&fileVars,
                      ...
                      0, NULL, 0, 0, 0, false);
#else
                      0, NULL, 0, sizeof(IOHibernateImageHeader),
                      setFileSizeMin, false);
#endif
    gFSState = kFSIdle;
  }
  IOLockUnlock(gFSLock);
  ...
  if (gIOOptionsEntry && gIOHibernateBootImageKey) {
    if (data) {
...
#if DEBUG || DEVELOPMENT
      if (kIOLogHibernate & gIOKitDebug) {
        IOKitKernelLogBuffer("H> boot-image:", data->getBytesNoCopy(),
                             data->getLength(), &kprintf);
      }
#endif /* DEBUG || DEVELOPMENT */
    } else {
      ...
      if (gIOHibernateState == kIOHibernateStateWakingFromHibernate) {
        // if we woke from hibernation, the booter may have changed the state of
        // NVRAM, so force a sync
        gIOOptionsEntry->sync();
      }
#endif
    }
...
static IOReturn IOWriteExtentsToFile(IOPolledFileIOVars *vars,
  ...
  if (hdr.fileExtentMapSize > sizeof(hdr.fileExtentMap)) {
    ...
    rc = kern_write_file(
        vars->fileRef, vars->blockSize,
        (caddr_t)(((uint8_t *)fileExtents) + sizeof(hdr.fileExtentMap)), count,
        IO_SKIP_ENCRYPTION);
    if (rc != 0) {
  ...
  rc = kern_write_file(vars->fileRef, 0, (char *)&hdr, sizeof(hdr),
                       IO_SKIP_ENCRYPTION);
  if (rc != 0) {
    HIBLOG("kern_write_file returned %d\n", rc);
    err = kIOReturnIOError;
...
static void ProgressInit(hibernate_graphics_t *display, uint8_t *screen,
                         uint8_t *saveunder, uint32_t savelen) {
  ...
  screen += ((display->width -
              kIOHibernateProgressCount *
                  (kIOHibernateProgressWidth + kIOHibernateProgressSpacing))
             << (pixelShift - 1)) +
            (display->height - kIOHibernateProgressOriginY -
      ...
      for (x = 0; x < kIOHibernateProgressWidth; x++) {
        ...
        if (alpha) {
          if (0xff != alpha) {
            ...
            saveunder[blob * kIOHibernateProgressSaveUnderSize +
                      saveindex[blob]++] = in;
          ...
          if (1 == pixelShift) {
            result >>= 3;
            *((uint16_t *)out) =
                ((uint16_t)((result << 10) | (result << 5) | result)); // 16
          } else {
...
static void ProgressUpdate(hibernate_graphics_t *display, uint8_t *screen,
  ...
  lastBlob = (select < kIOHibernateProgressCount)
                 ? select
                 : (kIOHibernateProgressCount - 1);

  screen +=
      (firstBlob * (kIOHibernateProgressWidth + kIOHibernateProgressSpacing))
      << pixelShift;

  for (y = 0; y < kIOHibernateProgressHeight; y++) {
    out = screen + y * rowBytes;
    for (blob = firstBlob; blob <= lastBlob; blob++) {
      color = (blob < select) ? kIOHibernateProgressLightGray
...
IOReturn IOHibernateSystemHasSlept(void) {
  ...
  if (gIOHibernateMode) {
    obj = IOService::getPMRootDomain()->copyProperty(
        kIOHibernatePreviewBufferKey);
  }
  ...
  if (vars->previewBuffer &&
      (kIOReturnSuccess != vars->previewBuffer->prepare())) {
  ...
  if ((kIOHibernateOptionProgress & gIOHibernateCurrentHeader->options) &&
      vars->previewBuffer &&
      (data = OSDynamicCast(OSData, IOService::getPMRootDomain()->getProperty(
    ...
    if (kIOHibernatePreviewUpdates & flags) {
      ...
      if (vars->consoleMapping) {
        ProgressInit(graphicsInfo, vars->consoleMapping,
                     &graphicsInfo->progressSaveUnder[0][0],
                     sizeof(graphicsInfo->progressSaveUnder));
      }
    }
  }
...
static const DeviceTreeNode *MergeDeviceTree(const DeviceTreeNode *entry,
                                             IORegistryEntry *regEntry,
                                             OSSet *entriesToUpdate,
  ...
  for (idx = 0; idx < entry->nProperties; idx++) {
    if (updateEntry && (0 != strcmp("name", prop->name))) {
      regEntry->setProperty((const char *)prop->name, (void *)(prop + 1),
                            prop->length);
      //	    HIBPRINT("%s: %s, %d\n", regEntry->getName(), prop->name,
      //prop->length);
    }
    prop = (DeviceTreeNodeProperty *)(((uintptr_t)(prop + 1)) +
                                      ((prop->length + 3) & ~3));
  }
  ...
  for (idx = 0; idx < entry->nChildren; idx++) {
    if (kSuccess !=
        SecureDTGetPropertyRegion(child, "name", (void const **)&nameProp,
                                  &propLen, region_start, region_size)) {
      panic("no name");
    }
    childRegEntry =
        regEntry ? regEntry->childFromPath(nameProp, gIODTPlane) : NULL;
    //	HIBPRINT("%s == %p\n", nameProp, childRegEntry);
    child = MergeDeviceTree(child, childRegEntry, entriesToUpdate, region_start,
                            region_size);
    OSSafeReleaseNULL(childRegEntry);
    if (!child) {
      // the recursive call updated the last entry we cared about, so we can
      // stop
      break;
    }
  }
...
IOReturn IOHibernateSystemWake(void) {
  if (kFSOpened == gFSState) {
    IOPolledFilePollersClose(gIOHibernateVars.fileVars,
  ...
  if (gIOOptionsEntry && gIOHibernateBootImageKey) {
    ...
    // we don't need to sync immediately; the booter should have already removed
    // this entry we just want to make sure that if anyone syncs nvram after
    // this point, we don't re-write a stale boot-image value
    gIOOptionsEntry->removeProperty(gIOHibernateBootImageKey);
  }
...
static IOReturn IOHibernateDone(IOHibernateVars *vars) {
  ...
  if (vars->videoMapping) {
    ...
    if (vars->videoAllocSize) {
      // dealloc range
      kmem_free(kernel_map, trunc_page(vars->videoMapping),
                vars->videoAllocSize);
    }
  }
  ...
  if (kIOHibernateStateWakingFromHibernate == gIOHibernateState) {
    IOService::getPMRootDomain()->setProperty(
        kIOHibernateOptionsKey, gIOHibernateCurrentHeader->options, 32);
  } else {
    IOService::getPMRootDomain()->removeProperty(kIOHibernateOptionsKey);
  }
  ...
  if ((kIOHibernateStateWakingFromHibernate == gIOHibernateState) &&
    ...
    IOService::getPMRootDomain()->setProperty(
        kIOHibernateGfxStatusKey, &gIOHibernateGraphicsInfo->gfxStatus,
  ...
  bzero(&gIOHibernateHandoffPages[0],
        gIOHibernateHandoffPageCount * sizeof(gIOHibernateHandoffPages[0]));
  if (vars->handoffBuffer) {
    if (kIOHibernateStateWakingFromHibernate == gIOHibernateState) {
      IOHibernateHandoff *handoff;
      bool done = false;
      for (handoff =
               (IOHibernateHandoff *)vars->handoffBuffer->getBytesNoCopy();
           !done;
        ...
        case kIOHibernateHandoffTypeDeviceTree: {
          ...
          const size_t mergePathCount =
              sizeof(mergePaths) / sizeof(mergePaths[0]);
          ...
          MergeDeviceTree((DeviceTreeNode *)data, IOService::getServiceRoot(),
                          entriesToUpdate, (vm_offset_t)data,
                          (vm_size_t)handoff->bytecount);
...
#if defined(__i386__) || defined(__x86_64__)
        {
          IOBufferMemoryDescriptor *md = IOBufferMemoryDescriptor::withBytes(
              data, handoff->bytecount, kIODirectionOutIn);
          if (md) {
...
#if defined(__i386__) || defined(__x86_64__)
      if (vars->volumeCryptKeySize) {
        IOBufferMemoryDescriptor *bmd = IOBufferMemoryDescriptor::withBytes(
  ...
  if (gIOChosenEntry &&
      (data = OSDynamicCast(
           OSData, gIOChosenEntry->getProperty(gIOBridgeBootSessionUUIDKey))) &&
  ...
  //    gIOHibernateState = kIOHibernateStateInactive;       // leave it for
  //    post wake code to see
  gIOHibernateCount++;
...
static void IOHibernateSystemPostWakeTrim(void *p1, void *p2) {
  ...
  if (kFSTrimDelay == gFSState) {
    ...
    IOPolledFileClose(&vars,
                      ...
                      sizeof(IOHibernateImageHeader),
                      gIOHibernateCurrentHeader->imageSize, false);
#endif
    gFSState = kFSIdle;
  }
  if (p1) {
...
uint32_t IOHibernateWasScreenLocked(void) {
  ...
  if (gIOChosenEntry) {
    if (kIOHibernateStateWakingFromHibernate == gIOHibernateState) {
      OSData *data = OSDynamicCast(
          OSData, gIOChosenEntry->getProperty(kIOScreenLockStateKey));
      if (data) {
...
SYSCTL_STRING(_kern, OID_AUTO, hibernatefile,
              CTLFLAG_RW | CTLFLAG_KERN | CTLFLAG_LOCKED, gIOHibernateFilename,
              sizeof(gIOHibernateFilename), "");
...
SYSCTL_UINT(_kern, OID_AUTO, hibernatemode,
            CTLFLAG_RW | CTLFLAG_KERN | CTLFLAG_LOCKED, &gIOHibernateMode, 0,
            "");
...
SYSCTL_OID_MANUAL(_kern_bridge, OID_AUTO, bootsessionuuid,
                  CTLTYPE_STRING | CTLFLAG_RD | CTLFLAG_NOAUTO | CTLFLAG_KERN |
                      CTLFLAG_LOCKED,
                  gIOHibernateBridgeBootSessionUUIDString,
                  sizeof(gIOHibernateBridgeBootSessionUUIDString),
...
SYSCTL_UINT(_kern, OID_AUTO, hibernatecount,
            CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_ANYBODY, &gIOHibernateCount, 0,
            "");

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

static int hibernate_set_preview SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  ...
  IOBufferMemoryDescriptor *md = IOBufferMemoryDescriptor::withOptions(
      kIODirectionOutIn, rounded_size, page_size);
  if (!md) {
    return ENOMEM;
  }
...
SYSCTL_PROC(_kern, OID_AUTO, hibernatepreview,
            CTLTYPE_OPAQUE | CTLFLAG_WR | CTLFLAG_LOCKED | CTLFLAG_ANYBODY,
            NULL, 0, hibernate_set_preview, "S", "");
...
void IOHibernateSystemInit(IOPMrootDomain *rootDomain) {
  ...
  if (PE_parse_boot_argn("hfile", gIOHibernateFilename,
                         sizeof(gIOHibernateFilename))) {
...
static IOReturn IOHibernatePolledFileWrite(IOHibernateVars *vars,
                                           const uint8_t *bytes,
                                           IOByteCount size,
...
extern "C" uint32_t hibernate_write_image(void) {
  ...
  // The panic must occur after boot-image is set but before the image is
  // written.
  if ((panic_test_case & PANIC_TEST_CASE_HIBERNATION_ENTRY) &&
      (panic_test_failure_mode & PANIC_TEST_FAILURE_MODE_PANIC)) {
  ...
  if (kIOHibernateModeEncrypt & gIOHibernateMode) {
    ...
    aes_encrypt_key(vars->cryptKey, kIOHibernateAESKeySize,
                    &cryptvars->ctx.encrypt);
    aes_decrypt_key(vars->cryptKey, kIOHibernateAESKeySize,
  ...
  hibernate_page_list_setall(
      ...
      ((0 == (kIOHibernateModeSleep & gIOHibernateMode)) &&
       (0 != ((kIOHibernateModeDiscardCleanActive |
  ...
  fileExtents =
      (IOPolledFileExtent *)vars->fileVars->fileExtents->getBytesNoCopy();
  ...
  do {
    ...
    err =
        IOPolledFilePollersOpen(vars->fileVars, kIOPolledBeforeSleepState,
    ...
    if (vars->volumeCryptKeySize) {
      err = IOPolledFilePollersSetEncryptionKey(
          vars->fileVars, &vars->volumeCryptKey[0], vars->volumeCryptKeySize);
    ...
    if (count > sizeof(header->fileExtentMap)) {
      ...
      err = IOHibernatePolledFileWrite(
          vars, ((uint8_t *)&fileExtents[0]) + sizeof(header->fileExtentMap),
          count, cryptvars);
      if (kIOReturnSuccess != err) {
        break;
      }
    ...
    for (count = 0; (phys64 = vars->handoffBuffer->getPhysicalSegment(
                         count, &segLen, kIOMemoryMapperNone));
         count += segLen) {
      for (pagesDone = 0; pagesDone < atop_32(segLen); pagesDone++) {
        gIOHibernateHandoffPages[atop_32(count) + pagesDone] =
            atop_64_ppnum(phys64) + pagesDone;
      }
    }
    ...
    uintptr_t entrypoint =
        ((uintptr_t)&hibernate_machine_entrypoint) - hibernateBase;
    uintptr_t stack =
        ((uintptr_t)&gIOHibernateRestoreStackEnd[0]) - 64 - hibernateBase;
    ...
    if (uuid_parse(&gIOHibernateBridgeBootSessionUUIDString[0],
                   &header->bridgeBootSessionUUID[0])) {
      bzero(&header->bridgeBootSessionUUID[0],
            sizeof(header->bridgeBootSessionUUID));
    }

    // sum __HIB seg, with zeros for the stack
    src = (uint8_t *)trunc_page(hibernateBase);
    for (page = 0; page < count; page++) {
      if ((src < &gIOHibernateRestoreStack[0]) ||
          (src >= &gIOHibernateRestoreStackEnd[0])) {
        restore1Sum += hibernate_sum_page(
            src, (uint32_t)(header->restore1CodeVirt + page));
      } else {
    ...
    src = (uint8_t *)trunc_page(hibernateBase);
    count =
        ((uintptr_t)&gIOHibernateRestoreStack[0]) - trunc_page(hibernateBase);
    if (count) {
    ...
    err = IOHibernatePolledFileWrite(vars, (uint8_t *)NULL,
                                     &gIOHibernateRestoreStackEnd[0] -
                                         &gIOHibernateRestoreStack[0],
                                     cryptvars);
    if (kIOReturnSuccess != err) {
      break;
    }
    ...
    if (!vars->hwEncrypt && (kIOHibernateModeEncrypt & gIOHibernateMode)) {
      vars->fileVars->encryptStart =
          (vars->fileVars->position & ~(AES_BLOCK_SIZE - 1));
    ...
    if (vars->previewBuffer) {
      ...
      count = 0;
      do {
        phys64 = vars->previewBuffer->getPhysicalSegment(count, &segLen,
                                                         kIOMemoryMapperNone);
        pageAndCount[0] = atop_64_ppnum(phys64);
        pageAndCount[1] = atop_64_ppnum(segLen);
        err = IOHibernatePolledFileWrite(vars, (const uint8_t *)&pageAndCount,
                                         sizeof(pageAndCount), cryptvars);
        if (kIOReturnSuccess != err) {
          break;
        }
        count += segLen;
      ...
      src = (uint8_t *)vars->previewBuffer->getPhysicalSegment(
          0, NULL, _kIOMemorySourceSegment);
      ...
      for (page = 0; page < count; page += page_size) {
        phys64 = vars->previewBuffer->getPhysicalSegment(page, NULL,
                                                         kIOMemoryMapperNone);
    ...
    hibernate_set_descriptor_page_state(vars,
                                        IOPolledFileGetIOBuffer(vars->fileVars),
    ...
    hibernate_page_list_set_volatile(vars->page_list, vars->page_list_wired,
                                     &pageCount);
    ...
    hibernate_set_page_state(vars->page_list, vars->page_list_wired, page,
                             count, kIOHibernatePageStateFree);
    pageCount -= count;
#elif defined(__arm64__)
    // the segments described in IOHibernateHibSegInfo are stored directly in
    // the hibernation file, so they don't need to be saved again
...
#error unimplemented
    ...
    hibernate_set_page_state(vars->page_list, vars->page_list_wired,
                             atop_64(shadow_pnext), shadow_pages_free,
    ...
    HIBLOG("bitmap_size 0x%x, previewSize 0x%x, writing %d pages @ 0x%llx\n",
           bitmap_size, header->previewSize, pageCount,
    ...
    for (pageType = kWiredEncrypt; pageType >= kUnwiredEncrypt; pageType--) {
      if (kUnwiredEncrypt == pageType) {
        ...
        if (!vars->hwEncrypt && (kIOHibernateModeEncrypt & gIOHibernateMode)) {
          vars->fileVars->encryptStart =
              (vars->fileVars->position & ~(((uint64_t)AES_BLOCK_SIZE) - 1));
          vars->fileVars->encryptEnd = UINT64_MAX;
          HIBLOG("encryptStart %qx\n", vars->fileVars->encryptStart);
        }
        bcopy(&cryptvars->aes_iv[0], &gIOHibernateCryptWakeContext.aes_iv[0],
      ...
      for (iterDone = false, ppnum = 0; !iterDone;) {
        if (cpuAES && (pageType == kWiredClear)) {
          ...
          count = hibernate_page_list_iterate(
              (kWired & pageType) ? vars->page_list_wired : vars->page_list,
              &ppnum);
          if (count > UINT_MAX) {
            count = UINT_MAX;
          }
        ...
        if (!cpuAES) {
          if (count && (kWired & pageType) && needEncrypt) {
            ...
            for (checkIndex = 0;
                 (checkIndex < count) &&
                 (((kEncrypt & pageType) == 0) ==
                  pmap_is_noencrypt(((ppnum_t)(ppnum + checkIndex))));
                 checkIndex++) {
            }
            if (!checkIndex) {
              ppnum++;
        ...
        switch (pageType) {
        case kWiredEncrypt:
          wiredPagesEncrypted += count;
          break;
        case kWiredClear:
          wiredPagesClear += count;
          break;
        case kUnwiredEncrypt:
          dirtyPagesEncrypted += count;
          break;
        }

        if (iterDone &&
            (kWiredEncrypt == pageType)) { /* not yet end of wired list */
        } else {
        ...
        for (page = ppnum; page < (ppnum + count); page++) {
          ...
          err = IOMemoryDescriptorWriteFromPhysical(vars->srcBuffer, 0,
                                                    ptoa_64(page), page_size);
          if (err) {
          ...
          clock_get_uptime(&startTime);
          wkresult =
              WKdm_compress_new((const WK_word *)src, (WK_word *)compressed,
                                (WK_word *)scratch, (uint32_t)(page_size - 4));
          ...
          err = IOHibernatePolledFileWrite(vars, (const uint8_t *)&tag,
                                           sizeof(tag), cryptvars);
          if (kIOReturnSuccess != err) {
            break;
          }

          err = IOHibernatePolledFileWrite(
              vars, data, (pageCompressedSize + 3) & ~3, cryptvars);
          if (kIOReturnSuccess != err) {
            break;
          }
          ...
          if (vars->consoleMapping && (0 == (1023 & pagesDone))) {
            ...
            if (blob != lastBlob) {
              ProgressUpdate(gIOHibernateGraphicsInfo, vars->consoleMapping,
                             lastBlob, blob);
              lastBlob = blob;
            }
          }
      ...
      if ((kEncrypt & pageType) && vars->fileVars->encryptStart) {
        vars->fileVars->encryptEnd =
            ((vars->fileVars->position + 511) & ~511ULL);
    ...
    header->compression =
        ((uint32_t)((compressedSize << 8) / uncompressedSize));
#if defined(__arm64__)
    /*
    ...
    /* Compute the "mem slide" -- difference between the virtual base and the
     * physical base */
    ...
    err = IOHibernatePolledFileWrite(vars, (uint8_t *)header,
                                     sizeof(IOHibernateImageHeader), cryptvars);
    if (kIOReturnSuccess != err) {
#if DEVELOPMENT || DEBUG
  ...
  HIBLOG("crypt bytes: %qd time: %qd ms %qd Mb/s, ", vars->fileVars->cryptBytes,
         nsec / 1000000ULL,
         nsec ? (((vars->fileVars->cryptBytes * 1000000000ULL) / 1024 / 1024) /
                 nsec)
              : 0);
  ...
  HIBLOG(
      ...
      uncompressedSize ? ((int)((compressedSize * 100ULL) / uncompressedSize))
                       : 0);

  HIBLOG("\nsum1 %x, sum2 %x\n", sum1, sum2);

  HIBLOG("svPageCount %d, zvPageCount %d, wiredPagesEncrypted %d, "
         "wiredPagesClear %d, dirtyPagesEncrypted %d\n",
         svPageCount, zvPageCount, wiredPagesEncrypted, wiredPagesClear,
         dirtyPagesEncrypted);

  if (pollerOpen) {
    IOPolledFilePollersClose(vars->fileVars,
  ...
  if (vars->consoleMapping) {
    ProgressUpdate(gIOHibernateGraphicsInfo, vars->consoleMapping, 0,
                   kIOHibernateProgressCount);
  }
  ...
  if (kIOReturnSuccess == err) {
    ...
    vm_panic_hibernate_write_image_failed(err, vars->fileVars->fileSizeMin,
                                          vars->fileVars->fileSizeMax,
                                          vars->fileVars->fileSize);
...
extern "C" void hibernate_machine_init(void) {
  IOReturn err;
  uint32_t sum;
  uint32_t pagesDone;
  ...
  HIBPRINT("diag %x %x %x %x\n", gIOHibernateCurrentHeader->diag[0],
           gIOHibernateCurrentHeader->diag[1],
           gIOHibernateCurrentHeader->diag[2],
           gIOHibernateCurrentHeader->diag[3]);

#if defined(__i386__) || defined(__x86_64__)
#define t40ms(x)                                                               \
  ((uint32_t)((tmrCvt((((uint64_t)(x)) << 8), tscFCvtt2n) / 1000000)))
#else /* defined(__i386__) || defined(__x86_64__) */
#define t40ms(x) x
#endif /* defined(__i386__) || defined(__x86_64__) */
#define tStat(x, y) gIOHibernateStats->x = t40ms(gIOHibernateCurrentHeader->y);
  ...
  HIBLOG("booter start at %d ms smc %d ms, [%d, %d, %d] total %d ms, dsply %d, "
         "%d ms, tramp %d ms\n",
         gIOHibernateStats->booterStart, gIOHibernateStats->smcStart,
         gIOHibernateStats->booterDuration0, gIOHibernateStats->booterDuration1,
         gIOHibernateStats->booterDuration2, gIOHibernateStats->booterDuration,
         gIOHibernateStats->booterConnectDisplayDuration,
         gIOHibernateStats->booterSplashDuration,
         gIOHibernateStats->trampolineDuration);

  HIBLOG("hibernate_machine_init: state %d, image pages %d, sum was %x, "
         "imageSize 0x%qx, image1Size 0x%qx, conflictCount %d, nextFree %x\n",
         gIOHibernateState, pagesDone, sum, gIOHibernateStats->imageSize,
         gIOHibernateStats->image1Size,
         gIOHibernateCurrentHeader->conflictCount,
         gIOHibernateCurrentHeader->nextFree);
  ...
  if (vars->hwEncrypt) {
    ...
    cryptvars = (kIOHibernateModeEncrypt & gIOHibernateMode)
                    ? &gIOHibernateCryptWakeContext
                    : NULL;
  }

  if (gIOHibernateCurrentHeader->handoffPageCount >
  ...
  const uint8_t *handoffStart =
      (const uint8_t *)vars->handoffBuffer->getBytesNoCopy();
  ...
  for (handoff = (IOHibernateHandoff *)vars->handoffBuffer->getBytesNoCopy();
    ...
    case kIOHibernateHandoffTypeGraphicsInfo:
      if (handoff->bytecount == sizeof(*gIOHibernateGraphicsInfo)) {
        bcopy(data, gIOHibernateGraphicsInfo,
    ...
    case kIOHibernateHandoffTypeCryptVars:
      if (cryptvars) {
        hibernate_cryptwakevars_t *wakevars =
            (hibernate_cryptwakevars_t *)&handoff->data[0];
        if (handoff->bytecount == sizeof(*wakevars)) {
          bcopy(&wakevars->aes_iv[0], &cryptvars->aes_iv[0],
                sizeof(cryptvars->aes_iv));
        } else {
  ...
  HIBPRINT("video 0x%llx %d %d %d status %x\n",
           ...
           gIOHibernateGraphicsInfo->depth, gIOHibernateGraphicsInfo->width,
           gIOHibernateGraphicsInfo->height,
  ...
  if (vars->videoMapping && gIOHibernateGraphicsInfo->physicalAddress) {
    ...
    if (vars->videoMapSize > vars->videoAllocSize) {
      ...
      IOMapPages(kernel_map, vars->videoMapping,
                 gIOHibernateGraphicsInfo->physicalAddress, vars->videoMapSize,
                 kIOMapInhibitCache);
    }
  }

  if (vars->videoMapSize) {
    ProgressUpdate(gIOHibernateGraphicsInfo, (uint8_t *)vars->videoMapping, 0,
                   kIOHibernateProgressCount);
  }
  ...
  HIBLOG("IOPolledFilePollersOpen(), ml_get_interrupts_enabled %d\n",
         ml_get_interrupts_enabled());
  err =
      IOPolledFilePollersOpen(vars->fileVars, kIOPolledAfterSleepState, false);
  ...
  while (kIOReturnSuccess == err) {
    ...
    for (page = 0; page < count; page++) {
      ...
      err = IOPolledFileRead(vars->fileVars, src, (compressedSize + 3) & ~3,
                             cryptvars);
      if (kIOReturnSuccess != err) {
        panic("Hibernate restore error %x", err);
      }
      ...
      if (compressedSize < page_size) {
        ...
        if (compressedSize == 4) {
          ...
          pal_hib_decompress_page(src, compressed, scratch,
      ...
      err = IOMemoryDescriptorReadToPhysical(vars->srcBuffer, decoOffset,
                                             ptoa_64(ppnum), page_size);
      if (err) {
  ...
  HIBLOG("hibernate_machine_init pagesDone %d sum2 %x, time: %d ms, disk(0x%x) "
         "%qd Mb/s, ",
         pagesDone, sum, gIOHibernateStats->kernelImageReadDuration,
         kDefaultIOSize,
         nsecIO ? ((((gIOHibernateCurrentHeader->imageSize -
                      gIOHibernateCurrentHeader->image1Size) *
                     1000000000ULL) /
                    1024 / 1024) /
                   nsecIO)
                : 0);
...
void IOHibernateSystemRestart(void) {
  ...
  element =
      (element & page_mask) | ptoa_64(pmap_find_phys(kernel_pmap, element));
  noteProp->appendValue(element);
