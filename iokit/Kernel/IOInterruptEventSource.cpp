...
#define IOStatisticsCheckForWork()                                             \
  do {                                                                         \
    IOStatistics::countInterruptCheckForWork(                                  \
...
bool IOInterruptEventSource::init(OSObject *inOwner, Action inAction,
                                  IOService *inProvider, int inIntIndex) {
  bool res = true;
  ...
  if (inProvider) {
    ...
    res =
        (kIOReturnSuccess == registerInterruptHandler(inProvider, inIntIndex));
...
IOReturn IOInterruptEventSource::registerInterruptHandler(IOService *inProvider,
   ...
   * init()), so we don't have to worry that we will invoke
   * addInterruptStatistics erroneously.
   */
...
void IOInterruptEventSource::unregisterInterruptHandler(IOService *inProvider,
                                                        int inIntIndex) {
...
IOInterruptEventSource::interruptEventSource(OSObject *inOwner, Action inAction,
                                             IOService *inProvider,
                                             int inIntIndex) {
  OSSharedPtr<IOInterruptEventSource> me =
      OSMakeShared<IOInterruptEventSource>();
...
IOInterruptEventSource::interruptEventSource(OSObject *inOwner,
  ...
  ies = IOInterruptEventSource::interruptEventSource(inOwner, (Action)NULL,
                                                     inProvider, inIntIndex);
  if (ies) {
...
void IOInterruptEventSource::setWorkLoop(IOWorkLoop *inWorkLoop) {
  ...
  if (provider) {
    if (!inWorkLoop) {
      if (intIndex >= 0) {
        /*
         * It isn't necessarily safe to wait until free() to unregister the
         * interrupt; our provider may disappear.
         */
        unregisterInterruptHandler(provider, intIndex);
        intIndex = ~intIndex;
      }
               ...
               (kIOReturnSuccess ==
                registerInterruptHandler(provider, ~intIndex))) {
      intIndex = ~intIndex;
    }
  }
...
int IOInterruptEventSource::getIntIndex() const { return intIndex; }

bool IOInterruptEventSource::getAutoDisable() const { return autoDisable; }
...
bool IOInterruptEventSource::checkForWork() {
  ...
  if (kActionBlock & flags) {
    address = ptrauth_nop_cast(
        void *, _Block_get_invoke_fn((struct Block_layout *)intActionBlock));
  } else {
  ...
  if (numInts > 0) {
    if (trace) {
      IOTimeStampStartConstant(
          IODBG_INTES(IOINTES_ACTION), VM_KERNEL_ADDRHIDE(address),
          VM_KERNEL_ADDRHIDE(owner), VM_KERNEL_ADDRHIDE(this),
          VM_KERNEL_ADDRHIDE(workLoop));
    }

    if (reserved->statistics) {
      if (IA_GET_STATISTIC_ENABLED(
    ...
    if (reserved->statistics) {
      if (IA_GET_STATISTIC_ENABLED(kInterruptAccountingSecondLevelCountIndex)) {
        IA_ADD_VALUE(&reserved->statistics->interruptStatistics
...
void IOInterruptEventSource::normalInterruptOccurred(void * /*refcon*/,
                                                     IOService * /*prov*/,
                                                     int /*source*/) {
  bool trace = (gIOKitTrace & kIOTraceIntEventSource) ? true : false;
  ...
  if (trace) {
    IOTimeStampStartConstant(IODBG_INTES(IOINTES_SEMA),
  ...
  if (reserved->statistics) {
    ...
    if (IA_GET_STATISTIC_ENABLED(kInterruptAccountingFirstLevelCountIndex)) {
      IA_ADD_VALUE(
          &reserved->statistics
               ->interruptStatistics[kInterruptAccountingFirstLevelCountIndex],
          1);
    }
  }
  ...
  if (trace) {
    IOTimeStampEndConstant(IODBG_INTES(IOINTES_SEMA), VM_KERNEL_ADDRHIDE(this),
                           VM_KERNEL_ADDRHIDE(owner));
  }
}

void IOInterruptEventSource::disableInterruptOccurred(void * /*refcon*/,
                                                      IOService *prov,
                                                      int source) {
  bool trace = (gIOKitTrace & kIOTraceIntEventSource) ? true : false;
...
void IOInterruptEventSource::interruptOccurred(void *_refcon, IOService *prov,
                                               int source) {
  if (autoDisable && prov) {
...
IOReturn IOInterruptEventSource::warmCPU(uint64_t abstime) {
  return ml_interrupt_prewarm(abstime);
}

void IOInterruptEventSource::enablePrimaryInterruptTimestamp(bool enable) {
  if (reserved->statistics) {
    reserved->statistics->enablePrimaryTimestamp = enable;
  }
}

uint64_t IOInterruptEventSource::getPrimaryInterruptTimestamp() {
