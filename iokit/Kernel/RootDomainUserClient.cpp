...
OSDefineMetaClassAndStructors(RootDomainUserClient, IOUserClient2022)
    ...
    bool RootDomainUserClient::initWithTask(task_t owningTask,
...
IOReturn RootDomainUserClient::secureSleepSystemOptions(
    const void *inOptions, IOByteCount inOptionsSize, uint32_t *returnCode) {
  int local_priv = 0;
  int admin_priv = 0;
  ...
  if ((local_priv || admin_priv) && fOwner) {
    ...
    if (inOptions) {
      unserializedObject = OSUnserializeXML(
          (const char *)inOptions, inOptionsSize, &unserializeErrorString);
      ...
      if (!sleepOptionsDict) {
        IOLog("IOPMRootDomain SleepSystem unserialization failure: %s\n",
              unserializeErrorString
                  ? unserializeErrorString->getCStringNoCopy()
                  : "Unknown");
      }
    }
...
IOReturn RootDomainUserClient::secureSetAggressiveness(unsigned long type,
...
IOReturn RootDomainUserClient::secureSetMaintenanceWakeCalendar(
    IOPMCalendarStruct *inCalendar, uint32_t *returnCode) {
  int admin_priv = 0;
  IOReturn ret = kIOReturnNotPrivileged;
...
IOReturn
RootDomainUserClient::secureSetUserAssertionLevels(uint32_t assertionBitfield) {
  int admin_priv = 0;
  IOReturn ret = kIOReturnNotPrivileged;
...
IOReturn RootDomainUserClient::secureGetSystemSleepType(uint32_t *outSleepType,
                                                        uint32_t *sleepTimer) {
  int admin_priv = 0;
  IOReturn ret;
...
IOReturn
RootDomainUserClient::secureAttemptIdleSleepAbort(uint32_t *outReverted) {
  int admin_priv = 0;
  IOReturn ret;
...
IOReturn
RootDomainUserClient::secureSetLockdownModeHibernation(uint32_t status) {
#if HIBERNATION
  int admin_priv = 0;
  IOReturn ret;
...
IOReturn RootDomainUserClient::externalMethodDispatched(
  ...
  case kPMSetAggressiveness:
    ret = me->secureSetAggressiveness((unsigned long)arguments->scalarInput[0],
  ...
  case kPMSleepSystem:
    ret = me->secureSleepSystem((uint32_t *)&arguments->scalarOutput[0]);
    break;

  case kPMAllowPowerChange:
    ret = me->fOwner->allowPowerChange(arguments->scalarInput[0]);
    break;

  case kPMCancelPowerChange:
    ret = me->fOwner->cancelPowerChange(arguments->scalarInput[0]);
    break;
  ...
  case kPMSleepSystemOptions:
    ret = me->secureSleepSystemOptions(arguments->structureInput,
  ...
  case kPMSetUserAssertionLevels:
    ret = me->secureSetUserAssertionLevels((uint32_t)arguments->scalarInput[0]);
    break;

  case kPMActivityTickle:
    if (me->fOwner->checkSystemCanSustainFullWake()) {
      me->fOwner->reportUserInput();
      me->fOwner->setProperty(kIOPMRootDomainWakeTypeKey,
  ...
  case kPMSetClamshellSleepState:
    me->fOwner->setClamShellSleepDisable(
        arguments->scalarInput[0] ? true : false,
        IOPMrootDomain::kClamshellSleepDisablePowerd);
    ret = kIOReturnSuccess;
    break;

  case kPMGetSystemSleepType:
    ret = me->secureGetSystemSleepType((uint32_t *)&arguments->scalarOutput[0],
  ...
  case kPMSetLDMHibernationDisable:
    ret = me->secureSetLockdownModeHibernation(
 ...
 */
IOExternalMethod *
RootDomainUserClient::getTargetAndMethodForIndex(IOService **targetP,
                                                 UInt32 index) {
  // DO NOT EDIT
 ...
 */
void RootDomainUserClient::setPreventative(UInt32 on_off,
