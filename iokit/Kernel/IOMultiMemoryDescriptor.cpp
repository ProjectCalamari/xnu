...
OSDefineMetaClassAndStructors(IOMultiMemoryDescriptor, IOMemoryDescriptor)
    ...
    IOMultiMemoryDescriptor *IOMultiMemoryDescriptor::withDescriptors(
        IOMemoryDescriptor **descriptors, UInt32 withCount,
        IODirection withDirection, bool asReference) {
  //
  ...
  do {
    ...
    if (kIOMapOverwrite & options) {
      ...
      if ((ref.map == kernel_map) && (kIOMemoryBufferPageable & _flags)) {
        err =
            IOIteratePageableMaps(ref.size, &IOMemoryDescriptorMapAlloc, &ref);
      } else {
    ...
    for (unsigned index = 0; bytesRemaining && (index < _descriptorsCount);
         index++) {
      ...
      IOMemoryMap *subMap;
      subMap = _descriptors[index]->createMappingInTask(
          mapping->fAddressTask, nextAddress, subOptions, mapOffset, chunk);
      if (!subMap) {
        break;
      }
...
IOReturn IOMultiMemoryDescriptor::setOwnership(task_t newOwner,
                                               int newLedgerTag,
  ...
  err = kIOReturnSuccess;
  for (unsigned index = 0; index < _descriptorsCount; index++) {
    err = _descriptors[index]->setOwnership(newOwner, newLedgerTag,
                                            newLedgerOptions);
    if (kIOReturnSuccess != err) {
      break;
    }
...
IOMultiMemoryDescriptor::getPageCounts(IOByteCount *pResidentPageCount,
  ...
  for (unsigned index = 0; index < _descriptorsCount; index++) {
    err = _descriptors[index]->getPageCounts(
        &residentPageCount, &dirtyPageCount, &swappedPageCount);
    if (kIOReturnSuccess != err) {
      break;
    }
