...
private:
  ...
  IOReturn secureSetAggressiveness(unsigned long type, unsigned long newLevel,
                                   int *return_code);

  IOReturn secureSetMaintenanceWakeCalendar(IOPMCalendarStruct *inCalendar,
                                            uint32_t *returnCode);
...
public:
  ...
  static IOReturn externalMethodDispatched(OSObject *target, void *reference,
  ...
  getTargetAndMethodForIndex(IOService **targetP,
