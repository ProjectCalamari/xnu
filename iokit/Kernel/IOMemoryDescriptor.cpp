...
enum { kIOMapGuardSizeLarge = 65536 };
...
#define getNumIOPL(osd, d)                                                     \
  ((UInt)(((osd)->getLength() - ((char *)getIOPLList(d) - (char *)d)) /        \
          sizeof(ioPLBlock)))
#define getPageList(d) (&(d->fPageList[0]))
#define computeDataSize(p, u)                                                  \
  (offsetof(ioGMDData, fPageList) + p * sizeof(upl_page_info_t) +              \
   u * sizeof(ioPLBlock))
...
extern "C" {
kern_return_t device_data_action(uintptr_t device_handle,
                                 ipc_port_t device_pager, vm_prot_t protection,
                                 vm_object_offset_t offset, vm_size_t size) {
  kern_return_t kr;
...
kern_return_t device_close(uintptr_t device_handle) {
  IOMemoryDescriptorReserved *ref = (IOMemoryDescriptorReserved *)device_handle;
...
static inline void getAddrLenForInd(mach_vm_address_t &addr,
                                    mach_vm_size_t &len, // Output variables
                                    UInt32 type,
                                    IOGeneralMemoryDescriptor::Ranges r,
                                    UInt32 ind, task_t task __unused) {
  assert(kIOMemoryTypeUIO == type || kIOMemoryTypeVirtual == type ||
         kIOMemoryTypeVirtual64 == type || kIOMemoryTypePhysical == type ||
         kIOMemoryTypePhysical64 == type);
  if (kIOMemoryTypeUIO == type) {
    user_size_t us;
    user_addr_t ad;
    uio_getiov((uio_t)r.uio, ind, &ad, &us);
    addr = ad;
    len = us;
  }
#ifndef __LP64__
  else if ((kIOMemoryTypeVirtual64 == type) ||
           (kIOMemoryTypePhysical64 == type)) {
...
static IOReturn purgeableControlBits(IOOptionBits newState,
...
static IOReturn purgeableStateBits(int *state) {
  IOReturn err = kIOReturnSuccess;
...
static const iokit_memtype_entry iomd_mem_types[] = {
    ...
    [kIOPostedCombinedReordered] = {VM_WIMG_POSTED_COMBINED_REORDERED,
                                    MAP_MEM_POSTED_COMBINED_REORDERED},
};

static vm_prot_t vmProtForCacheMode(IOOptionBits cacheMode) {
...
static unsigned int pagerFlagsForCacheMode(IOOptionBits cacheMode) {
  assert(cacheMode < (sizeof(iomd_mem_types) / sizeof(iomd_mem_types[0])));
  if (cacheMode >= (sizeof(iomd_mem_types) / sizeof(iomd_mem_types[0]))) {
    cacheMode = kIODefaultCache;
  }
...
static IOOptionBits cacheModeForPagerFlags(unsigned int pagerFlags) {
  pagerFlags &= VM_WIMG_MASK;
  IOOptionBits cacheMode = kIODefaultCache;
  for (IOOptionBits i = 0;
       i < (sizeof(iomd_mem_types) / sizeof(iomd_mem_types[0])); ++i) {
...
IOMemoryReference *
IOGeneralMemoryDescriptor::memoryReferenceAlloc(uint32_t capacity,
  ...
  ref = krealloc_type(IOMemoryReference, IOMemoryEntry, oldCapacity, capacity,
                      realloc, Z_WAITOK_ZERO);
  if (ref) {
    if (oldCapacity == 0) {
      ref->refCount = 1;
...
void IOGeneralMemoryDescriptor::memoryReferenceFree(IOMemoryReference *ref) {
...
void IOGeneralMemoryDescriptor::memoryReferenceRelease(IOMemoryReference *ref) {
  if (1 == OSDecrementAtomic(&ref->refCount)) {
    memoryReferenceFree(ref);
  }
}

IOReturn IOGeneralMemoryDescriptor::memoryReferenceCreate(
    IOOptionBits options, IOMemoryReference **reference) {
  ...
  if (_task) {
    ...
    if (kIODefaultCache ==
        ((_flags & kIOMemoryBufferCacheMask) >> kIOMemoryBufferCacheShift)) {
  ...
  if (_task) {
    ...
    while (remain) {
      ...
      if (vm_map_page_mask(get_task_map(_task)) < PAGE_MASK) {
        DEBUG4K_IOKIT(
            "IOMemRef %p _flags 0x%x prot 0x%x _ranges[%d]: 0x%llx 0x%llx\n",
      ...
      do {
        ...
        err = mach_make_memory_entry_internal(map, &actualSize, entryAddrForVm,
                                              prot, vmne_kflags, &entry,
                                              cloneEntry);

        if (KERN_SUCCESS != err) {
          DEBUG4K_ERROR("make_memory_entry(map %p, addr 0x%llx, size 0x%llx, "
                        "prot 0x%x) err 0x%x\n",
        ...
        memory_entry_check_for_adjustment(map, entry, &overmap_start,
                                          &overmap_end);
    ...
    pager = device_pager_setup((memory_object_t)NULL, (uintptr_t)reserved, size,
                               pagerFlags);
    assert(pager);
    if (!pager) {
      ...
      srcAddr = nextAddr;
      entryAddr = trunc_page_64(srcAddr);
      err = mach_memory_object_memory_entry_64(
          (host_t)1, false /*internal*/, size, VM_PROT_READ | VM_PROT_WRITE,
          pager, &entry);
  ...
  if (_task && (KERN_SUCCESS == err) && (kIOMemoryMapCopyOnWrite & _flags) &&
...
static mach_vm_size_t IOMemoryDescriptorMapGuardSize(vm_map_t map,
...
static kern_return_t IOMemoryDescriptorMapDealloc(IOOptionBits options,
                                                  vm_map_t map,
  ...
  actualSize =
      vm_map_round_page(addr + size, vm_map_page_mask(map)) - actualAddr;
...
kern_return_t IOMemoryDescriptorMapAlloc(vm_map_t map, void *_ref) {
  ...
  err = mach_vm_map_kernel(
      map, &addr, size,
#if __ARM_MIXED_PAGE_SIZE__
      // TODO4K this should not be necessary...
      (vm_map_offset_t)((ref->options & kIOMapAnywhere)
                            ? max(PAGE_MASK, vm_map_page_mask(map))
                            : 0),
#else  /* __ARM_MIXED_PAGE_SIZE__ */
      (vm_map_offset_t)0,
#endif /* __ARM_MIXED_PAGE_SIZE__ */
      vmk_flags, IPC_PORT_NULL, (memory_object_offset_t)0, false, /* copy */
      ref->prot, ref->prot, VM_INHERIT_NONE);
  if (KERN_SUCCESS == err) {
    ref->mapped = (mach_vm_address_t)addr;
    ref->map = map;
    if (kIOMapGuardedMask & ref->options) {
      vm_map_offset_t lastpage =
          vm_map_trunc_page(addr + size - guardSize, vm_map_page_mask(map));

      err = mach_vm_protect(map, addr, guardSize, false /*set max*/,
                            VM_PROT_NONE);
      assert(KERN_SUCCESS == err);
      err = mach_vm_protect(map, lastpage, guardSize, false /*set max*/,
                            VM_PROT_NONE);
      assert(KERN_SUCCESS == err);
...
IOReturn IOGeneralMemoryDescriptor::memoryReferenceMap(
    IOMemoryReference *ref, vm_map_t map, mach_vm_size_t inoffset,
    mach_vm_size_t size, IOOptionBits options, mach_vm_address_t *inaddr) {
  IOReturn err;
  ...
  DEBUG4K_IOKIT(
      "ref %p map %p inoffset 0x%llx size 0x%llx options 0x%x *inaddr 0x%llx\n",
  ...
  if (kIODefaultCache != cacheMode) {
    ...
    memEntryCacheMode =
        (MAP_MEM_ONLY | VM_PROT_WRITE | prot | vmProtForCacheMode(cacheMode));
  }
  ...
  if (remain >= nextLen) {
    DEBUG4K_ERROR("map %p inoffset 0x%llx size 0x%llx options 0x%x inaddr "
                  "0x%llx remain 0x%llx nextLen 0x%llx\n",
                  map, inoffset, size, (uint32_t)options, *inaddr,
                  (uint64_t)remain, nextLen);
    return kIOReturnBadArgument;
  }
  ...
  if (!(options & kIOMapAnywhere)) {
    ...
    if (pageOffset != (vm_map_page_mask(map) & addr)) {
      DEBUG4K_ERROR("map %p inoffset 0x%llx size 0x%llx options 0x%x inaddr "
                    "0x%llx addr 0x%llx page_mask 0x%llx pageOffset 0x%llx\n",
                    map, inoffset, size, (uint32_t)options, *inaddr,
                    (uint64_t)addr, (uint64_t)page_mask, (uint64_t)pageOffset);
    }
    addr -= pageOffset;
  }
   ...
   * it will be immediately faulted in through the pager via
   * populateDevicePager(). kIOMapPrefault is redundant in that case, so don't
   * try to use it for UPL operations.
   */
  ...
  if (options & kIOMapPrefault) {
    ...
    if ((_wireCount == 0) || (_memoryEntries == NULL)) {
      DEBUG4K_ERROR(
          "map %p inoffset 0x%llx size 0x%llx options 0x%x inaddr 0x%llx\n",
          map, inoffset, size, (uint32_t)options, *inaddr);
      return kIOReturnBadArgument;
    }
     ...
     */
    UInt ioplIndex = 0;
    while ((ioplIndex < numIOPLs) &&
           (((uint64_t)offset) >= ioplList[ioplIndex].fIOMDOffset)) {
      ioplIndex++;
    }
    ioplIndex--;
    ...
    mach_vm_size_t offsetInIOPL =
        offset - ioplInfo.fIOMDOffset + ioplInfo.fPageOffset;
  ...
  while (remain && (KERN_SUCCESS == err)) {
    ...
    if ((min(vm_map_page_mask(map), page_mask) & entryOffset) != pageOffset) {
      ...
      DEBUG4K_ERROR("map %p inoffset 0x%llx size 0x%llx options 0x%x inaddr "
                    "0x%llx entryOffset 0x%llx pageOffset 0x%llx\n",
                    map, inoffset, size, (uint32_t)options, *inaddr,
                    (uint64_t)entryOffset, (uint64_t)pageOffset);
      break;
    }
    ...
    if (chunk) {
      ...
      if (options & kIOMapPrefault) {
        ...
        err = vm_map_enter_mem_object_prefault(
            map, &mapAddr, chunk, 0 /* mask */, vmk_flags, entry->entry,
            entryOffset,
            prot, // cur
            prot, // max
            &pageList[currentPageIndex], nb_pages);

        if (err || vm_map_page_mask(map) < PAGE_MASK) {
          DEBUG4K_IOKIT("IOMemRef %p mapped in map %p (pgshift %d) at 0x%llx "
                        "size 0x%llx err 0x%x\n",
                        ref, map, vm_map_page_shift(map), (uint64_t)mapAddr,
                        (uint64_t)chunk, err);
        }
        ...
        err = mach_vm_map_kernel(map, &mapAddr, chunk, 0 /* mask */, vmk_flags,
      ...
      if (KERN_SUCCESS != err) {
        DEBUG4K_ERROR("IOMemRef %p mapped in map %p (pgshift %d) at 0x%llx "
                      "size 0x%llx err 0x%x\n",
                      ref, map, vm_map_page_shift(map), (uint64_t)mapAddr,
                      (uint64_t)chunk, err);
        break;
    ...
    if (entryIdx >= ref->count) {
      ...
      DEBUG4K_ERROR("map %p inoffset 0x%llx size 0x%llx options 0x%x inaddr "
                    "0x%llx entryIdx %d ref->count %d\n",
                    map, inoffset, size, (uint32_t)options, *inaddr, entryIdx,
                    ref->count);
      break;
    }
  }
  ...
  *inaddr = addr;
  ...
  if (err /* || vm_map_page_mask(map) < PAGE_MASK */) {
    DEBUG4K_ERROR("map %p (%d) inoffset 0x%llx size 0x%llx options 0x%x inaddr "
                  "0x%llx err 0x%x\n",
                  map, vm_map_page_shift(map), inoffset, size,
                  (uint32_t)options, *inaddr, err);
  }
  return err;
}

#define LOGUNALIGN 0
IOReturn IOGeneralMemoryDescriptor::memoryReferenceMapNew(
    IOMemoryReference *ref, vm_map_t map, mach_vm_size_t inoffset,
    mach_vm_size_t size, IOOptionBits options, mach_vm_address_t *inaddr) {
  IOReturn err;
  int64_t offset = inoffset;
  ...
  // find first entry for offset
  for (firstEntryIdx = 0; (firstEntryIdx < ref->count) &&
                          (offset >= ref->entries[firstEntryIdx].offset);
       firstEntryIdx++) {
  }
  firstEntryIdx--;
  ...
  while (remain) {
    ...
    printf("[%d] size %qx offset %qx start %qx iter %qx\n", entryIdx,
           entry->size, entry->offset, entry->start, iteroffset);
#endif
    ...
    if (chunk) {
      ...
      err = mach_memory_entry_map_size(entry->entry, map, entryOffset, chunk,
                                       &entrySize);
      assert(KERN_SUCCESS == err);
      mapSize += entrySize;
  ...
  while (remain && (KERN_SUCCESS == err)) {
    ...
    if (chunk) {
      ...
      if (options & kIOMapPrefault) {
        ...
        err = vm_map_enter_mem_object_prefault(
            map, &mapAddrOut, chunk, 0 /* mask */, vmk_flags, entry->entry,
            entryOffset,
            prot, // cur
            prot, // max
            &pageList[currentPageIndex], nb_pages);
...
#endif
        err = mach_vm_map_kernel(map, &mapAddrOut, chunk, 0 /* mask */,
                                 vmk_flags, entry->entry, entryOffset,
                                 false, // copy
                                 prot,  // cur
                                 prot,  // max
                                 VM_INHERIT_NONE);
...
uint64_t IOGeneralMemoryDescriptor::memoryReferenceGetDMAMapLength(
    IOMemoryReference *ref, uint64_t *offset) {
  kern_return_t kr;
...
IOReturn IOGeneralMemoryDescriptor::memoryReferenceGetPageCounts(
    IOMemoryReference *ref, IOByteCount *residentPageCount,
...
IOReturn IOGeneralMemoryDescriptor::memoryReferenceSetPurgeable(
    IOMemoryReference *ref, IOOptionBits newState, IOOptionBits *oldState) {
  IOReturn err;
  IOMemoryEntry *entries;
  ...
  while (entries > &ref->entries[0]) {
    ...
    err = memory_entry_purgeable_control_internal(entries->entry, control,
                                                  &state);
    if (KERN_SUCCESS != err) {
      break;
    }
...
IOReturn IOGeneralMemoryDescriptor::memoryReferenceSetOwnership(
    IOMemoryReference *ref, task_t newOwner, int newLedgerTag,
  ...
  while (entries > &ref->entries[0]) {
    ...
    err = mach_memory_entry_ownership(entries->entry, newOwner, newLedgerTag,
                                      newLedgerOptions);
    if (KERN_SUCCESS != err) {
      totalErr = err;
    }
...
IOMemoryDescriptor::withAddress(void *address, IOByteCount length,
                                IODirection direction) {
  return IOMemoryDescriptor::withAddressRange((IOVirtualAddress)address, length,
                                              direction | kIOMemoryAutoPrepare,
                                              kernel_task);
}

#ifndef __LP64__
OSSharedPtr<IOMemoryDescriptor>
IOMemoryDescriptor::withAddress(IOVirtualAddress address, IOByteCount length,
                                IODirection direction, task_t task) {
  OSSharedPtr<IOGeneralMemoryDescriptor> that =
      OSMakeShared<IOGeneralMemoryDescriptor>();
  if (that) {
...
OSSharedPtr<IOMemoryDescriptor> IOMemoryDescriptor::withPhysicalAddress(
    IOPhysicalAddress address, IOByteCount length, IODirection direction) {
  return IOMemoryDescriptor::withAddressRange(address, length, direction,
                                              TASK_NULL);
}

#ifndef __LP64__
OSSharedPtr<IOMemoryDescriptor>
IOMemoryDescriptor::withRanges(IOVirtualRange *ranges, UInt32 withCount,
                               IODirection direction, task_t task,
                               bool asReference) {
  OSSharedPtr<IOGeneralMemoryDescriptor> that =
      OSMakeShared<IOGeneralMemoryDescriptor>();
...
IOMemoryDescriptor::withAddressRange(mach_vm_address_t address,
                                     ...
                                     IOOptionBits options, task_t task) {
  IOAddressRange range = {address, length};
  return IOMemoryDescriptor::withAddressRanges(&range, 1, options, task);
}

OSSharedPtr<IOMemoryDescriptor>
IOMemoryDescriptor::withAddressRanges(IOAddressRange *ranges, UInt32 rangeCount,
                                      IOOptionBits options, task_t task) {
  OSSharedPtr<IOGeneralMemoryDescriptor> that =
      OSMakeShared<IOGeneralMemoryDescriptor>();
  if (that) {
    ...
    if (that->initWithOptions(ranges, rangeCount, 0, task, options,
                              /* mapper */ NULL)) {
      return os::move(that);
    }
  }
 ...
 */
OSSharedPtr<IOMemoryDescriptor>
IOMemoryDescriptor::withOptions(void *buffers, UInt32 count, UInt32 offset,
                                task_t task, IOOptionBits opts,
                                IOMapper *mapper) {
...
bool IOMemoryDescriptor::initWithOptions(void *buffers, UInt32 count,
                                         UInt32 offset, task_t task,
                                         IOOptionBits options,
                                         IOMapper *mapper) {
  return false;
}
...
IOMemoryDescriptor::withPhysicalRanges(IOPhysicalRange *ranges,
                                       UInt32 withCount, IODirection direction,
                                       bool asReference) {
  OSSharedPtr<IOGeneralMemoryDescriptor> that =
      OSMakeShared<IOGeneralMemoryDescriptor>();
  if (that) {
    if (that->initWithPhysicalRanges(ranges, withCount, direction,
                                     asReference)) {
      return os::move(that);
    }
  }
...
OSSharedPtr<IOMemoryDescriptor>
IOMemoryDescriptor::withSubRange(IOMemoryDescriptor *of, IOByteCount offset,
                                 IOByteCount length, IODirection direction) {
...
IOMemoryDescriptor::withPersistentMemoryDescriptor(
  ...
  if (origGenMD) {
    return IOGeneralMemoryDescriptor::withPersistentMemoryDescriptor(origGenMD);
  } else {
    return nullptr;
  }
}
...
IOGeneralMemoryDescriptor::withPersistentMemoryDescriptor(
  ...
  if (kIOReturnSuccess !=
      originalMD->memoryReferenceCreate(kIOMemoryReferenceReuse, &memRef)) {
    return nullptr;
  }
  ...
  if (self && !self->initWithOptions(&initData, 1, 0, NULL,
                                     kIOMemoryTypePersistentMD, NULL)) {
    return nullptr;
  }
  return os::move(self);
}
...
#ifndef __LP64__
bool IOGeneralMemoryDescriptor::initWithAddress(void *address,
...
bool IOGeneralMemoryDescriptor::initWithAddress(IOVirtualAddress address,
                                                IOByteCount withLength,
                                                IODirection withDirection,
                                                task_t withTask) {
...
bool IOGeneralMemoryDescriptor::initWithPhysicalAddress(
    IOPhysicalAddress address, IOByteCount withLength,
    IODirection withDirection) {
...
bool IOGeneralMemoryDescriptor::initWithPhysicalRanges(IOPhysicalRange *ranges,
                                                       UInt32 count,
                                                       IODirection direction,
                                                       bool reference) {
...
bool IOGeneralMemoryDescriptor::initWithRanges(IOVirtualRange *ranges,
                                               UInt32 count,
                                               IODirection direction,
                                               task_t task, bool reference) {
...
bool IOGeneralMemoryDescriptor::initWithOptions(void *buffers, UInt32 count,
                                                UInt32 offset, task_t task,
                                                IOOptionBits options,
                                                IOMapper *mapper) {
...
#ifndef __LP64__
  if (task && (kIOMemoryTypeVirtual == type) &&
      vm_map_is_64bit(get_task_map(task)) &&
      ((IOVirtualRange *)buffers)->address) {
    OSReportWithBacktrace("IOMemoryDescriptor: attempt to create 32b virtual "
                          "in 64b task, use ::withAddressRange()");
    return false;
  }
#endif /* !__LP64__ */
  ...
  if (kIOMemoryTypeUPL == type) {
    ...
    if (options & kIOMemoryAsReference) {
      ...
      case kIOMemoryTypePhysical64:
        if (count == 1
#ifndef __arm__
            && (((IOAddressRange *)buffers)->address +
                ((IOAddressRange *)buffers)->length) <= 0x100000000ULL
#endif
        ) {
    ...
    for (ind = 0; ind < count; ind++) {
      ...
      if (_task) {
        ...
        kern_return_t kret;
        kret = vm_map_range_physical_size(get_task_map(_task), addr, len,
                                          &phys_size);
        ...
        if (os_add_overflow(pages, (atop_64(endAddr) - atop_64(addr)),
                            &pages)) {
          break;
        }
      }
      ...
      if ((kIOMemoryTypePhysical == type) ||
        ...
        if ((highPage > _highestPage) && (highPage <= UINT_MAX)) {
          ...
          DEBUG4K_IOKIT(
              "offset 0x%x task %p options 0x%x -> _highestPage 0x%x\n",
              (uint32_t)offset, task, (uint32_t)options, _highestPage);
        }
      }
    }
    if ((ind < count) || (totalLength != ((IOByteCount)totalLength))) {
    ...
    if ((kIOMemoryTypePhysical == type) || (kIOMemoryTypePhysical64 == type)) {
      ...
      if ((kIOMemoryPersistent & _flags) && !_memRef) {
        IOReturn err = memoryReferenceCreate(0, &_memRef);
        if (kIOReturnSuccess != err) {
          return false;
        }
      }

      if ((_flags & kIOMemoryAutoPrepare) && prepare() != kIOReturnSuccess) {
        return false;
      }
    }
  }
...
void IOGeneralMemoryDescriptor::free() {
  ...
  if ((kIOMemoryTypePhysical == type) || (kIOMemoryTypePhysical64 == type)) {
    ...
    if (_memoryEntries && (dataP = getDataP(_memoryEntries)) &&
      ...
      dmaUnmap(dataP->fMapper, NULL, 0, dataP->fMappedBase,
               dataP->fMappedLength);
 ...
 */
IOByteCount IOMemoryDescriptor::getLength() const { return _length; }

void IOMemoryDescriptor::setTag(IOOptionBits tag) { _tag = tag; }

IOOptionBits IOMemoryDescriptor::getTag(void) { return _tag; }

uint64_t IOMemoryDescriptor::getFlags(void) { return _flags; }

OSObject *IOMemoryDescriptor::copyContext(const OSSymbol *key) const {
...
OSObject *IOMemoryDescriptor::copyContext(const char *key) const {
  OSSharedPtr<const OSSymbol> sym = OSSymbol::withCString(key);
  return copyContext(sym.get());
}

OSObject *IOMemoryDescriptor::copySharingContext(const char *key) const {
...
void IOMemoryDescriptor::setContext(const OSSymbol *key, OSObject *obj) {
...
void IOMemoryDescriptor::setContext(const char *key, OSObject *obj) {
  OSSharedPtr<const OSSymbol> sym = OSSymbol::withCString(key);
  setContext(sym.get(), obj);
}

OSObject *IOMemoryDescriptor::copyContext(void) const {
...
void IOMemoryDescriptor::setSharingContext(const char *key, OSObject *obj) {
  OSSharedPtr<const OSSymbol> sym = OSSymbol::withCString(key);
...
bool IOMemoryDescriptor::hasSharingContext(void) {
  return 0 != (kIOMemoryDescriptorInternalFlagsSharing & _internalIOMDFlags);
}

void IOMemoryDescriptor::setContext(OSObject *obj) {
...
IOByteCount IOMemoryDescriptor::readBytes(IOByteCount offset, void *bytes,
                                          IOByteCount length) {
  ...
  if ((offset > _length) || os_add_overflow(length, offset, &endoffset) ||
      (endoffset > _length)) {
    assertf(false, "readBytes exceeds length (0x%lx, 0x%lx) > 0x%lx",
            (long)offset, (long)length, (long)_length);
    return 0;
  }
  if (offset >= _length) {
    return 0;
  ...
  while (remaining) { // (process another target segment?)
    ...
    kern_return_t copy_ret =
        copypv(srcAddr64, dstAddr, (unsigned int)srcLen, options);
...
IOByteCount IOMemoryDescriptor::writeBytes(IOByteCount inoffset,
                                           const void *bytes,
                                           IOByteCount length) {
  ...
  if ((offset > _length) || os_add_overflow(length, offset, &endoffset) ||
      (endoffset > _length)) {
    assertf(false, "writeBytes exceeds length (0x%lx, 0x%lx) > 0x%lx",
            (long)inoffset, (long)length, (long)_length);
    return 0;
  }
  ...
  while (remaining) { // (process another target segment?)
    ...
    if (!srcAddr) {
      ...
      kern_return_t copy_ret =
          copypv(srcAddr, (addr64_t)dstAddr64, (unsigned int)dstLen, options);
#pragma unused(copy_ret)
      srcAddr += dstLen;
    }
    offset += dstLen;
...
static volatile SInt64 gIOMDPreparationID __attribute__((aligned(8))) =
    (1ULL << 32);
static volatile SInt64 gIOMDDescriptorID __attribute__((aligned(8))) =
    (kIODescriptorIDInvalid + 1ULL);

uint64_t IOGeneralMemoryDescriptor::getPreparationID(void) {
  ioGMDData *dataP;
  ...
  if (kIOPreparationIDUnprepared == dataP->fPreparationID) {
    ...
    OSCompareAndSwap64(kIOPreparationIDUnprepared, newID,
                       &dataP->fPreparationID);
  }
  return dataP->fPreparationID;
}

void IOMemoryDescriptor::cleanKernelReserved(
...
void IOMemoryDescriptor::setPreparationID(void) {
  if (getKernelReserved() &&
      (kIOPreparationIDUnprepared == reserved->preparationID)) {
    SInt64 newID = OSIncrementAtomic64(&gIOMDPreparationID);
    OSCompareAndSwap64(kIOPreparationIDUnprepared, newID,
                       &reserved->preparationID);
  }
}
...
void IOMemoryDescriptor::setDescriptorID(void) {
  if (getKernelReserved() &&
      (kIODescriptorIDInvalid == reserved->descriptorID)) {
    SInt64 newID = OSIncrementAtomic64(&gIOMDDescriptorID);
    OSCompareAndSwap64(kIODescriptorIDInvalid, newID, &reserved->descriptorID);
  }
}

uint64_t IOMemoryDescriptor::getDescriptorID(void) {
  setDescriptorID();
...
IOReturn IOMemoryDescriptor::ktraceEmitPhysicalSegments(void) {
  ...
  IOTimeStampConstant(IODBG_IOMDPA(IOMDPA_MAPPED), descriptorID,
  ...
  for (IOByteCount offset = 0; offset < getLength();
       offset += physical_segment.length) {
    physical_segment.address =
        getPhysicalSegment(offset, &physical_segment.length);
    ...
    /**
     * Most IOMemoryDescriptors are made up of many individual physically
     * discontiguous pages.  To optimize for trace buffer memory, pack segment
     * events according to the following.
     *
     * Mappings must be emitted in ascending order starting from offset 0.
     * Mappings can be associated with the previous IOMDPA_MAPPED event emitted
     * on by the current thread_id.
     *
     * IOMDPA_SEGMENTS_PAGE        = up to 8 virtually contiguous page aligned
     * mappings of PAGE_SIZE length
     * - (ppn_0 << 32 | ppn_1), ..., (ppn_6 << 32 | ppn_7)
     * - unmapped pages will have a ppn of MAX_INT_32
     * IOMDPA_SEGMENTS_LONG	= up to 2 virtually contiguous mappings of
     * variable length
     * - address_0, length_0, address_0, length_1
     * - unmapped pages will have an address of 0
     *
     * During each iteration do the following depending on the length of the
     * mapping:
     * 1. add the current segment to the appropriate queue of pending segments
     * 1. check if we are operating on the same type of segment (PAGE/LONG) as
     * the previous pass 1a. if FALSE emit and reset all events in the previous
     * queue
    ...
    if ((physical_segment.address & PAGE_MASK) == 0 &&
        physical_segment.length == PAGE_SIZE) {
      ...
      if (os_unlikely(emit_page)) {
        ...
        IOTimeStampConstant(IODBG_IOMDPA(IOMDPA_SEGMENTS_PAGE),
                            ((uintptr_t)atop_64(segments_page[0]) << 32) |
                                (ppnum_t)atop_64(segments_page[1]),
                            ((uintptr_t)atop_64(segments_page[2]) << 32) |
                                (ppnum_t)atop_64(segments_page[3]),
                            ((uintptr_t)atop_64(segments_page[4]) << 32) |
                                (ppnum_t)atop_64(segments_page[5]),
                            ((uintptr_t)atop_64(segments_page[6]) << 32) |
                                (ppnum_t)atop_64(segments_page[7]));
#else
        IOTimeStampConstant(IODBG_IOMDPA(IOMDPA_SEGMENTS_PAGE),
...
#else
    IOTimeStampConstant(
        IODBG_IOMDPA(IOMDPA_SEGMENTS_PAGE), (ppnum_t)atop_32(segments_page[1]),
        (ppnum_t)atop_32(segments_page[2]), (ppnum_t)atop_32(segments_page[3]),
        (ppnum_t)atop_32(segments_page[4]));
#endif
...
void IOMemoryDescriptor::setVMTags(uint32_t kernelTag, uint32_t userTag) {
  _kernelTag = (vm_tag_t)kernelTag;
  _userTag = (vm_tag_t)userTag;
}

uint32_t IOMemoryDescriptor::getVMTag(vm_map_t map) {
...
IOReturn IOGeneralMemoryDescriptor::dmaCommandOperation(DMACommandOps op,
                                                        void *vData,
                                                        UInt dataSize) const {
  IOReturn err = kIOReturnSuccess;
  ...
  if (kIOMDDMAMap == op) {
    ...
    if (_memoryEntries && data->fMapper) {
      ...
      remap |= (dataP->fDMAMapNumAddressBits < 64) &&
               ((dataP->fMappedBase + _length) >
                (1ULL << dataP->fDMAMapNumAddressBits));
      remap |= (dataP->fDMAMapAlignment > page_size);

      if (remap || !dataP->fMappedBaseValid) {
        err = md->dmaMap(data->fMapper, md, data->fCommand, &data->fMapSpec,
                         data->fOffset, data->fLength, &data->fAlloc,
                         &data->fAllocLength);
  ...
  if (kIOMDDMAUnmap == op) {
    ...
    if (_pages) {
      err = md->dmaUnmap(data->fMapper, data->fCommand, data->fOffset,
                         data->fAlloc, data->fAllocLength);
    }
  ...
  if (IOMapper::gSystem && mapped && (!(kIOMemoryHostOnly & _flags)) &&
    ...
    if (dataP->fMapper) {
      ...
      err = md->dmaMap(dataP->fMapper, md, NULL, &mapSpec, 0, _length,
                       &dataP->fMappedBase, &dataP->fMappedLength);
      if (kIOReturnSuccess != err) {
        return err;
      }
      dataP->fMappedBaseValid = true;
  ...
  if (mapped) {
    if (IOMapper::gSystem && (!(kIOMemoryHostOnly & _flags)) &&
        _memoryEntries && (dataP = getDataP(_memoryEntries)) &&
        dataP->fMappedBaseValid) {
  ...
  if (!params && offset && (offset == isP->fNextOffset || off2Ind <= offset)) {
    ind = isP->fIndex;
  } else {
  ...
  if ((_flags & kIOMemoryTypeMask) == kIOMemoryTypePhysical) {
    ...
    do {
      ...
      if (mapped) {
        ...
        address =
            trunc_page_64(mappedBase) + ptoa_64(ioplInfo.fMappedPage) + offset;
...
addr64_t IOGeneralMemoryDescriptor::getPhysicalSegment(
    IOByteCount offset, IOByteCount *lengthOfSegment, IOOptionBits options) {
  IOReturn ret;
  ...
  // IOMemoryDescriptor::doMap() cannot use getPhysicalSegment() to obtain the
  // page offset, since it must support the unwired memory case in
  // IOGeneralMemoryDescriptor, and hibernate_write_image() cannot use
  // map()->getVirtualAddress() to obtain the kernel pointer, since it must
  // prevent the memory allocation due to IOMemoryMap, so
  // _kIOMemorySourceSegment is a necessary evil until all of this gets cleaned
  // up
  ...
  if ((options & _kIOMemorySourceSegment) && (kIOMemoryTypeUPL != type)) {
    ...
    state->fMapped = (0 == (options & kIOMemoryMapperNone)) &&
                     !(_flags & kIOMemoryHostOrRemote);

    ret = dmaCommandOperation(kIOMDFirstSegment, _state, sizeof(_state));

    if ((kIOReturnSuccess != ret) && (kIOReturnOverrun != ret)) {
      DEBG("getPhysicalSegment dmaCommandOperation(%lx), %p, offset %qx, addr "
           "%qx, len %qx\n",
           ret, this, state->fOffset, state->fIOVMAddr, state->fLength);
    }
    ...
    // dmaCommandOperation() does not distinguish between "mapped" and
    // "unmapped" physical memory, even with fMapped set correctly, so we must
    // handle the transformation here until this gets cleaned up

    if (mapper && ((kIOMemoryTypePhysical == type) ||
                   (kIOMemoryTypePhysical64 == type))) {
      if ((options & kIOMemoryMapperNone) && !(_flags & kIOMemoryMapperNone)) {
        ...
        while ((length < origLen) &&
               ((address + length) ==
                mapper->mapToPhysicalAddress(origAddr + length))) {
          length += page_size;
        }
        if (length > origLen) {
          length = origLen;
...
addr64_t IOMemoryDescriptor::getPhysicalSegment(IOByteCount offset,
...
addr64_t
IOGeneralMemoryDescriptor::getPhysicalSegment64(IOByteCount offset,
                                                IOByteCount *lengthOfSegment) {
...
IOPhysicalAddress
IOMemoryDescriptor::getPhysicalSegment(IOByteCount offset,
                                       IOByteCount *lengthOfSegment) {
...
IOReturn IOMemoryDescriptor::dmaCommandOperation(DMACommandOps op, void *vData,
                                                 UInt dataSize) const {
  ...
  if (kIOMDGetCharacteristics == op) {
    ...
    if (data->fMapped) {
      panic("fMapped %p %s %qx", this, getMetaClass()->getClassName(),
            (uint64_t)getLength());
    }
    ...
    while (offset < getLength()) {
      nextAddr =
          md->getPhysicalSegment(offset, &nextLength, kIOMemoryMapperNone);
...
IOReturn IOGeneralMemoryDescriptor::setPurgeable(IOOptionBits newState,
...
IOReturn IOMemoryDescriptor::setPurgeable(IOOptionBits newState,
  ...
  if (_memRef) {
    err = IOGeneralMemoryDescriptor::memoryReferenceSetPurgeable(
        _memRef, newState, oldState);
  }
  if (kIOMemoryThreadSafe & _flags) {
    UNLOCK;
  }
...
IOReturn
IOGeneralMemoryDescriptor::setOwnership(task_t newOwner, int newLedgerTag,
                                        IOOptionBits newLedgerOptions) {
  IOReturn err = kIOReturnSuccess;
...
IOReturn IOMemoryDescriptor::setOwnership(task_t newOwner, int newLedgerTag,
                                          IOOptionBits newLedgerOptions) {
  IOReturn err = kIOReturnNotReady;
  ...
  if (_memRef) {
    err = IOGeneralMemoryDescriptor::memoryReferenceSetOwnership(
        _memRef, newOwner, newLedgerTag, newLedgerOptions);
  } else {
...
uint64_t IOMemoryDescriptor::getDMAMapLength(uint64_t *offset) {
  uint64_t length;

  if (_memRef) {
    length = IOGeneralMemoryDescriptor::memoryReferenceGetDMAMapLength(_memRef,
                                                                       offset);
  } else {
    ...
    length = 0;
    iterate = 0;
    while ((sourceAddr = getPhysicalSegment(iterate, &segLen,
                                            _kIOMemorySourceSegment))) {
...
IOReturn IOMemoryDescriptor::getPageCounts(IOByteCount *residentPageCount,
  ...
  if (_memRef) {
    err = IOGeneralMemoryDescriptor::memoryReferenceGetPageCounts(
        _memRef, residentPageCount, dirtyPageCount, swappedPageCount);
  } else {
    IOMultiMemoryDescriptor *mmd;
    IOSubMemoryDescriptor *smd;
    if ((smd = OSDynamicCast(IOSubMemoryDescriptor, this))) {
...
IOReturn IOMemoryDescriptor::performOperation(IOOptionBits options,
...
#if defined(__arm64__)
  void (*func_ext)(addr64_t pa, unsigned int count, unsigned int remaining,
  ...
  while (remaining) {
    ...
    if (func_ext) {
      (*func_ext)(dstAddr64, (unsigned int)dstLen, (unsigned int)remaining,
                  &res);
      if (res != 0x0UL) {
        remaining = 0;
        break;
...
/* XXX: By extending io_kernel_static_end to the highest virtual address in the
 * KC, we're opening up this path to IOMemoryDescriptor consumers who can now
 * create UPLs to kernel non-text data -- should we just add another range
 * instead?
 */
#define io_kernel_static_start vm_kernel_stext
#define io_kernel_static_end                                                   \
  (kc_highest_nonlinkedit_vmaddr ? kc_highest_nonlinkedit_vmaddr               \
                                 : vm_kernel_etext)
...
io_get_kernel_static_upl(vm_map_t /* map */, uintptr_t offset,
                         upl_size_t *upl_size, unsigned int *page_offset,
                         upl_t *upl, upl_page_info_array_t page_list,
                         unsigned int *count, ppnum_t *highest_page) {
...
IOReturn IOGeneralMemoryDescriptor::wireVirtual(IODirection forDirection) {
  ...
  dataP = getDataP(_memoryEntries);
  upl_control_flags_t
      uplFlags; // This Mem Desc's default flags for upl creation
  ...
  if (_wireCount) {
    ...
    IOTimeStampIntervalConstantFiltered traceInterval(
        IODBG_MDESC(IOMDESC_WIRE), VM_KERNEL_ADDRHIDE(this), forDirection);
    IOMapper *mapper;
    ...
    for (UInt range = 0; mdOffset < _length; range++) {
      ...
      while (numBytes) {
        ...
        if (_memRef) {
          error =
              mach_memory_entry_map_size(memRefEntry->entry, NULL /*physical*/,
                                         0, memRefEntry->size, &ioplPhysSize);
          DEBUG4K_IOKIT("_memRef %p memRefEntry %p entry %p startPage 0x%llx "
                        "numBytes 0x%llx ioplPhysSize 0x%llx\n",
                        _memRef, memRefEntry, memRefEntry->entry, startPage,
                        numBytes, ioplPhysSize);
        } else {
          error = vm_map_range_physical_size(theMap, startPage, numBytes,
                                             &ioplPhysSize);
          DEBUG4K_IOKIT("_memRef %p theMap %p startPage 0x%llx numBytes 0x%llx "
                        "ioplPhysSize 0x%llx\n",
                        _memRef, theMap, startPage, numBytes, ioplPhysSize);
        }
        if (error != KERN_SUCCESS) {
          if (_memRef) {
            DEBUG4K_ERROR("_memRef %p memRefEntry %p entry %p theMap %p "
                          "startPage 0x%llx numBytes 0x%llx error 0x%x\n",
                          _memRef, memRefEntry, memRefEntry->entry, theMap,
                          startPage, numBytes, error);
          } else {
            DEBUG4K_ERROR("_memRef %p theMap %p startPage 0x%llx numBytes "
                          "0x%llx error 0x%x\n",
        ...
        ioplPhysSize = (ioplPhysSize <= MAX_UPL_SIZE_BYTES)
                           ? ioplPhysSize
                           : MAX_UPL_SIZE_BYTES;
        ...
        if (_memRef) {
          ...
          if (byteAlignUPL) {
            ...
            entryOffset =
                (entryOffset - iopl.fPageOffset - memRefEntry->offset);
          }
          if (ioplSize > (memRefEntry->size - entryOffset)) {
            ioplSize = ((typeof(ioplSize))(memRefEntry->size - entryOffset));
          }
          error = memory_object_iopl_request(memRefEntry->entry, entryOffset,
                                             &ioplSize, &iopl.fIOPL, baseInfo,
                                             &numPageInfo, &ioplFlags, tag);
        } else if ((theMap == kernel_map) &&
                   (kernelStart >= io_kernel_static_start) &&
                   (kernelStart < io_kernel_static_end)) {
          error = io_get_kernel_static_upl(theMap, kernelStart, &ioplSize,
                                           &iopl.fPageOffset, &iopl.fIOPL,
                                           baseInfo, &numPageInfo, &highPage);
        } else {
          assert(theMap);
          error = vm_map_create_upl(theMap, startPage, (upl_size_t *)&ioplSize,
                                    &iopl.fIOPL, baseInfo, &numPageInfo,
                                    &ioplFlags, tag);
        }

        if (error != KERN_SUCCESS) {
          traceInterval.setEndArg2(error);
          DEBUG4K_ERROR("UPL create error 0x%x theMap %p (kernel:%d) _memRef "
                        "%p startPage 0x%llx ioplSize 0x%x\n",
                        error, theMap, (theMap == kernel_map), _memRef,
                        startPage, ioplSize);
          goto abortExit;
        }
        ...
        if (byteAlignUPL) {
          if (iopl.fIOPL) {
            DEBUG4K_UPL("startPage 0x%llx numBytes 0x%llx iopl.fPageOffset "
                        "0x%x upl_get_data_offset(%p) 0x%llx\n",
                        startPage, numBytes, iopl.fPageOffset, iopl.fIOPL,
                        upl_get_data_offset(iopl.fIOPL));
            iopl.fPageOffset =
                (typeof(iopl.fPageOffset))upl_get_data_offset(iopl.fIOPL);
          }
...
bool IOGeneralMemoryDescriptor::initMemoryEntries(size_t size,
                                                  IOMapper *mapper) {
  ioGMDData *dataP;
...
IOReturn IOMemoryDescriptor::dmaMap(IOMapper *mapper,
  ...
  err = mapper->iovmMapMemory(memory, offset, length, mapOptions, mapSpec,
                              command, NULL, mapAddress, mapLength);
...
void IOMemoryDescriptor::dmaMapRecord(IOMapper *mapper, IODMACommand *command,
                                      uint64_t mapLength) {
  IOTimeStampIntervalConstantFiltered traceInterval(
      IODBG_MDESC(IOMDESC_DMA_MAP), VM_KERNEL_ADDRHIDE(this));
...
IOReturn IOMemoryDescriptor::dmaUnmap(IOMapper *mapper, IODMACommand *command,
                                      uint64_t offset, uint64_t mapAddress,
                                      uint64_t mapLength) {
  IOTimeStampIntervalConstantFiltered traceInterval(
      IODBG_MDESC(IOMDESC_DMA_UNMAP), VM_KERNEL_ADDRHIDE(this));
  IOReturn ret;
  kern_allocation_name_t alloc;
...
IOReturn IOGeneralMemoryDescriptor::dmaMap(
    IOMapper *mapper, IOMemoryDescriptor *memory, IODMACommand *command,
    const IODMAMapSpecification *mapSpec, uint64_t offset, uint64_t length,
    uint64_t *mapAddress, uint64_t *mapLength) {
  IOReturn err = kIOReturnSuccess;
  ioGMDData *dataP;
  IOOptionBits type = _flags & kIOMemoryTypeMask;
  ...
  *mapAddress = 0;
  ...
  if ((type == kIOMemoryTypePhysical) || (type == kIOMemoryTypePhysical64) ||
    ...
    err = super::dmaMap(mapper, memory, command, mapSpec, offset, length,
                        mapAddress, mapLength);
...
IOReturn IOGeneralMemoryDescriptor::prepare(IODirection forDirection) {
  ...
  IOOptionBits type = _flags & kIOMemoryTypeMask;
  IOTimeStampIntervalConstantFiltered traceInterval(
      IODBG_MDESC(IOMDESC_PREPARE), VM_KERNEL_ADDRHIDE(this), forDirection);
  ...
  if (kIOMemoryTypeVirtual == type || kIOMemoryTypeVirtual64 == type ||
      kIOMemoryTypeUIO == type) {
    if ((forDirection & kIODirectionPrepareAvoidThrottling) &&
...
IOReturn IOGeneralMemoryDescriptor::complete(IODirection forDirection) {
  ...
  ioGMDData *dataP;
  IOTimeStampIntervalConstantFiltered traceInterval(
      IODBG_MDESC(IOMDESC_COMPLETE), VM_KERNEL_ADDRHIDE(this), forDirection);
    ...
    if (!_wireCount || (kIODirectionCompleteWithDataValid & forDirection)) {
      ...
      if (_wireCount) {
...
#if IOTRACKING
        if (dataP->fWireTracking.link.next) {
          IOTrackingRemove(gIOWireTracking, &dataP->fWireTracking,
                           ptoa(_pages));
        }
#endif /* IOTRACKING */
        ...
        if (kdebug_debugid_explicitly_enabled(IODBG_IOMDPA(IOMDPA_UNMAPPED))) {
          IOTimeStampConstantFiltered(IODBG_IOMDPA(IOMDPA_UNMAPPED),
                                      getDescriptorID(),
...
IOOptionBits
IOGeneralMemoryDescriptor::memoryReferenceCreateOptions(IOOptionBits options,
                                                        IOMemoryMap *mapping) {
  ...
  if (!(kIOMapReadOnly & options)) {
    ...
    if ((kIODirectionOut == (kIODirectionOutIn & _flags)) &&
      ...
      OSReportWithBacktrace("warning: creating writable mapping from "
                            "IOMemoryDescriptor(kIODirectionOut) - use "
                            "kIOMapReadOnly or change direction");
    }
#endif
  }
...
/*
 * Attempt to create any kIOMemoryMapCopyOnWrite named entry needed ahead of the
 * global lock taken in IOMemoryDescriptor::makeMapping() since it may allocate
 * real pages on creation.
 */

IOMemoryMap *IOGeneralMemoryDescriptor::makeMapping(
    IOMemoryDescriptor *owner, task_t __intoTask, IOVirtualAddress __address,
    IOOptionBits options, IOByteCount __offset, IOByteCount __length) {
  IOReturn err = kIOReturnSuccess;
  IOMemoryMap *mapping;

  if ((kIOMemoryMapCopyOnWrite & _flags) && _task && !_memRef) {
    struct IOMemoryReference *newRef;
    err = memoryReferenceCreate(
        memoryReferenceCreateOptions(options, (IOMemoryMap *)__address),
        &newRef);
    if (kIOReturnSuccess == err) {
  ...
  mapping = IOMemoryDescriptor::makeMapping(owner, __intoTask, __address,
                                            options, __offset, __length);

#if IOTRACKING
  if ((mapping == (IOMemoryMap *)__address) &&
      (0 == (kIOMapStatic & mapping->fOptions)) &&
      (NULL == mapping->fSuperMap) && ((kIOTracking & gIOKitDebug) || _task)) {
...
IOReturn IOGeneralMemoryDescriptor::doMap(vm_map_t __addressMap,
                                          IOVirtualAddress *__address,
                                          IOOptionBits options,
                                          IOByteCount __offset,
                                          IOByteCount __length) {
  IOTimeStampIntervalConstantFiltered traceInterval(
      IODBG_MDESC(IOMDESC_MAP), VM_KERNEL_ADDRHIDE(this),
      VM_KERNEL_ADDRHIDE(*__address), __length);
  traceInterval.setEndArg1(kIOReturnSuccess);
#ifndef __LP64__
  if (!(kIOMap64Bit & options)) {
  ...
  if ((offset >= _length) || ((offset + length) > _length)) {
    ...
    DEBUG4K_ERROR("map %p offset 0x%llx length 0x%llx _length 0x%llx "
  ...
  if (_task && (mapping->fAddressTask == _task) &&
      (mapping->fAddressMap == get_task_map(_task)) &&
      (options & kIOMapAnywhere) && (!(kIOMapUnique & options)) &&
      (!(kIOMapGuardedMask & options)) && (1 == _rangesCount) &&
      (0 == offset) && range0Addr && (length <= range0Len)) {
  ...
  if (!_memRef) {
    err = memoryReferenceCreate(memoryReferenceCreateOptions(options, mapping),
                                &_memRef);
    if (kIOReturnSuccess != err) {
  ...
  // <upl_transpose //
  if ((kIOMapReference | kIOMapUnique) ==
      ((kIOMapReference | kIOMapUnique) & options)) {
    do {
      upl_t redirUPL2;
      upl_size_t size;
      ...
      flags = UPL_COPYOUT_FROM | UPL_SET_INTERNAL | UPL_SET_LITE |
              UPL_SET_IO_WIRE | UPL_BLOCK_ACCESS;

      if (KERN_SUCCESS !=
          memory_object_iopl_request(_memRef->entries[0].entry, 0, &size,
                                     &redirUPL2, NULL, NULL, &flags,
                                     (vm_tag_t)getVMTag(kernel_map))) {
        redirUPL2 = NULL;
      }

      for (lock_count = 0; IORecursiveLockHaveLock(gIOMemoryLock);
           lock_count++) {
        UNLOCK;
      }
      ...
      if (pager) {
        err = populateDevicePager(pager, mapping->fAddressMap,
                                  mapping->fAddress, offset, length, options);
      }
    } while (false);
  }
  // upl_transpose> //
  else {
    err = memoryReferenceMap(_memRef, mapping->fAddressMap, offset, length,
                             options, &mapping->fAddress);
    if (err) {
      DEBUG4K_ERROR("map %p err 0x%x\n", mapping->fAddressMap, err);
    }
    if ((err == KERN_SUCCESS) && pager) {
      err = populateDevicePager(pager, mapping->fAddressMap, mapping->fAddress,
...
#if IOTRACKING
IOReturn IOMemoryMapTracking(IOTrackingUser *tracking, task_t *task,
                             mach_vm_address_t *address, mach_vm_size_t *size) {
#define iomap_offsetof(type, field) ((size_t)(&((type *)NULL)->field))

  IOMemoryMap *map = (typeof(map))(((uintptr_t)tracking) -
                                   iomap_offsetof(IOMemoryMap, fTracking));

  if (!map->fAddressMap ||
      (map->fAddressMap != get_task_map(map->fAddressTask))) {
...
IOReturn IOGeneralMemoryDescriptor::doUnmap(vm_map_t addressMap,
                                            IOVirtualAddress __address,
                                            IOByteCount __length) {
  IOTimeStampIntervalConstantFiltered traceInterval(
      IODBG_MDESC(IOMDESC_UNMAP), VM_KERNEL_ADDRHIDE(this),
      VM_KERNEL_ADDRHIDE(__address), __length);
  IOReturn ret;
...
bool IOMemoryMap::init(task_t intoTask, mach_vm_address_t toAddress,
                       IOOptionBits _options, mach_vm_size_t _offset,
...
bool IOMemoryMap::setMemoryDescriptor(IOMemoryDescriptor *_memory,
...
IOReturn IOMemoryDescriptor::doMap(vm_map_t __addressMap,
                                   IOVirtualAddress *__address,
                                   IOOptionBits options, IOByteCount __offset,
                                   IOByteCount __length) {
  return kIOReturnUnsupported;
}

IOReturn IOMemoryDescriptor::handleFault(void *_pager,
...
IOReturn IOMemoryDescriptor::populateDevicePager(
    void *_pager, vm_map_t addressMap, mach_vm_address_t address,
    mach_vm_size_t sourceOffset, mach_vm_size_t length, IOOptionBits options) {
  IOReturn err = kIOReturnSuccess;
  ...
  do {
    ...
    if ((kIOMemoryTypeUPL != type) &&
        pmap_has_managed_page((ppnum_t)atop_64(physAddr),
                              (ppnum_t)atop_64(physAddr + segLen - 1))) {
      OSReportWithBacktrace(
          "IOMemoryDescriptor physical with managed page 0x%qx:0x%qx", physAddr,
          (uint64_t)segLen);
    }
#endif /* DEBUG || DEVELOPMENT */

    chunk = (reserved->dp.pagerContig ? round_page(segLen) : page_size);
    for (page = 0; (page < segLen) && (KERN_SUCCESS == err); page += chunk) {
    ...
    if ((addressMap == kernel_map) && !(kIOMemoryRedirected & _flags)) {
      err = vm_fault(addressMap, (vm_map_offset_t)trunc_page_64(address),
                     options & kIOMapReadOnly ? VM_PROT_READ
                                              : VM_PROT_READ | VM_PROT_WRITE,
                     FALSE, VM_KERN_MEMORY_NONE, THREAD_UNINT, NULL,
                     (vm_map_offset_t)0);
    ...
    bytes -= segLen;
    pageOffset = 0;
  } while (bytes && (physAddr = getPhysicalSegment(sourceOffset, &segLen,
                                                   kIOMemoryMapperNone)));
...
IOReturn IOMemoryDescriptor::doUnmap(vm_map_t addressMap,
                                     IOVirtualAddress __address,
                                     IOByteCount __length) {
  IOReturn err;
  IOMemoryMap *mapping;
...
#endif
    err = IOMemoryDescriptorMapDealloc(mapping->fOptions, addressMap, address,
                                       length);
  ...
  do {
    if ((iter = OSCollectionIterator::withCollection(_mappings.get()))) {
      ...
      while ((mapping = (IOMemoryMap *)iter->getNextObject())) {
        ...
        if (!doRedirect && !safeTask && pager &&
            (kernel_map == mapping->fAddressMap)) {
          err = populateDevicePager(pager, mapping->fAddressMap,
                                    mapping->fAddress, mapping->fOffset,
                                    mapping->fLength, kIOMapDefaultCache);
        }
      }
    ...
    do {
      ...
      if ((!safeTask || (get_task_map(safeTask) != fAddressMap)) &&
        ...
        IOLog("IOMemoryMap::redirect(%d, %p) 0x%qx:0x%qx from %p\n", doRedirect,
              this, fAddress, fLength, fAddressMap);
#endif
      } else if (kIOMapWriteCombineCache == (fOptions & kIOMapCacheMask)) {
        IOOptionBits newMode;
        newMode = (fOptions & ~kIOMapCacheMask) |
                  (doRedirect ? kIOMapInhibitCache : kIOMapWriteCombineCache);
...
IOByteCount IOMemoryMap::getLength() { return fLength; }
...
IOVirtualAddress IOMemoryMap::getVirtualAddress() {
  ...
  if (fSuperMap) {
    ...
    OSReportWithBacktrace("IOMemoryMap::getVirtualAddress(0x%qx) called on 64b "
                          "map; use ::getAddress()",
                          fAddress);
  }
#endif /* !__LP64__ */
...
#ifndef __LP64__
mach_vm_address_t IOMemoryMap::getAddress() { return fAddress; }

mach_vm_size_t IOMemoryMap::getSize() { return fLength; }
#endif /* !__LP64__ */
...
IOOptionBits IOMemoryMap::getMapOptions() { return fOptions; }

IOMemoryDescriptor *IOMemoryMap::getMemoryDescriptor() { return fMemory.get(); }

IOMemoryMap *IOMemoryMap::copyCompatible(IOMemoryMap *newMapping) {
...
IOReturn IOMemoryMap::wireRange(uint32_t options, mach_vm_size_t offset,
                                mach_vm_size_t length) {
  IOReturn kr;
  ...
  if (prot) {
    kr = vm_map_wire_kernel(fAddressMap, start, end, prot,
                            (vm_tag_t)fMemory->getVMTag(kernel_map), FALSE);
  } else {
...
IOMemoryDescriptor::setMapping(task_t intoTask, IOVirtualAddress mapAddress,
                               IOOptionBits options) {
  return createMappingInTask(intoTask, mapAddress, options | kIOMapStatic, 0,
                             getLength());
}

OSSharedPtr<IOMemoryMap> IOMemoryDescriptor::map(IOOptionBits options) {
  return createMappingInTask(kernel_task, 0, options | kIOMapAnywhere, 0,
                             getLength());
}

#ifndef __LP64__
OSSharedPtr<IOMemoryMap> IOMemoryDescriptor::map(task_t intoTask,
                                                 IOVirtualAddress atAddress,
                                                 IOOptionBits options,
                                                 IOByteCount offset,
                                                 IOByteCount length) {
  if ((!(kIOMapAnywhere & options)) &&
      vm_map_is_64bit(get_task_map(intoTask))) {
    OSReportWithBacktrace(
        "IOMemoryDescriptor::map() in 64b task, use ::createMappingInTask()");
    return NULL;
  }

  return createMappingInTask(intoTask, atAddress, options, offset, length);
}
#endif /* !__LP64__ */

OSSharedPtr<IOMemoryMap> IOMemoryDescriptor::createMappingInTask(
    task_t intoTask, mach_vm_address_t atAddress, IOOptionBits options,
    mach_vm_size_t offset, mach_vm_size_t length) {
  IOMemoryMap *result;
  IOMemoryMap *mapping;
  ...
  if (mapping && !mapping->init(intoTask, atAddress, options, offset, length)) {
    mapping->release();
    mapping = NULL;
  }

  if (mapping) {
    result = makeMapping(this, intoTask, (IOVirtualAddress)mapping,
                         options | kIOMap64Bit, 0, 0);
  } else {
    result = nullptr;
  }
...
#if DEBUG
  if (!result) {
    IOLog("createMappingInTask failed desc %p, addr %qx, options %x, offset "
          "%qx, length %llx\n",
...
IOReturn IOMemoryMap::redirect(IOMemoryDescriptor *newBackingMemory,
                               IOOptionBits options, mach_vm_size_t offset) {
  IOReturn err = kIOReturnSuccess;
    ...
    do {
      ...
      if (!fRedirUPL && fMemory->_memRef && (1 == fMemory->_memRef->count)) {
        ...
        upl_control_flags_t flags = UPL_COPYOUT_FROM | UPL_SET_INTERNAL |
                                    UPL_SET_LITE | UPL_SET_IO_WIRE |
                                    UPL_BLOCK_ACCESS;
        if (KERN_SUCCESS != memory_object_iopl_request(
                                fMemory->_memRef->entries[0].entry, 0, &size,
                                &fRedirUPL, NULL, NULL, &flags,
                                (vm_tag_t)fMemory->getVMTag(kernel_map))) {
          fRedirUPL = NULL;
        }
      ...
      if (newBackingMemory) {
        if (newBackingMemory != fMemory) {
          fOffset = 0;
          if (this !=
              newBackingMemory->makeMapping(
                  newBackingMemory, fAddressTask, (IOVirtualAddress)this,
...
IOMemoryMap *IOMemoryDescriptor::makeMapping(
  ...
  do {
    ...
    IOReturn kr =
        mapDesc->doMap(NULL, (IOVirtualAddress *)&mapping, options, 0, 0);
...
void IOMemoryDescriptor::addMapping(IOMemoryMap *mapping) {
  if (mapping) {
    if (NULL == _mappings) {
...
void IOMemoryDescriptor::removeMapping(IOMemoryMap *mapping) {
  if (_mappings) {
    _mappings->removeObject(mapping);
  }
}

void IOMemoryDescriptor::setMapperOptions(uint16_t options) {
  _iomapperOptions = options;
}

uint16_t IOMemoryDescriptor::getMapperOptions(void) { return _iomapperOptions; }

#ifndef __LP64__
// obsolete initializers
// - initWithOptions is the designated initializer
bool IOMemoryDescriptor::initWithAddress(void *address, IOByteCount length,
                                         IODirection direction) {
  return false;
}

bool IOMemoryDescriptor::initWithAddress(IOVirtualAddress address,
                                         IOByteCount length,
                                         IODirection direction, task_t task) {
  return false;
}

bool IOMemoryDescriptor::initWithPhysicalAddress(IOPhysicalAddress address,
                                                 IOByteCount length,
                                                 IODirection direction) {
  return false;
}

bool IOMemoryDescriptor::initWithRanges(IOVirtualRange *ranges,
                                        UInt32 withCount, IODirection direction,
                                        task_t task, bool asReference) {
  return false;
}

bool IOMemoryDescriptor::initWithPhysicalRanges(IOPhysicalRange *ranges,
                                                UInt32 withCount,
                                                IODirection direction,
                                                bool asReference) {
  return false;
...
bool IOGeneralMemoryDescriptor::serialize(OSSerialize *s) const {
  ...
  for (index = 0; index < vcopy.size(); index++) {
    ...
    OSSharedPtr<OSDictionary> dict = OSDictionary::withObjects(
        (const OSObject **)values, (const OSSymbol **)keys, 2);
    if (dict == NULL) {
      return false;
    }
...
KALLOC_TYPE_VAR_DEFINE(KT_IOMD_MIXED_DATA, struct ioGMDData, struct ioPLBlock,
                       KT_DEFAULT);
...
OSDefineMetaClassAndStructors(_IOMemoryDescriptorMixedData, OSObject)
    ...
    OSPtr<_IOMemoryDescriptorMixedData> _IOMemoryDescriptorMixedData::
        withCapacity(size_t capacity) {
  OSSharedPtr<_IOMemoryDescriptorMixedData> me =
      OSMakeShared<_IOMemoryDescriptorMixedData>();
...
bool _IOMemoryDescriptorMixedData::initWithCapacity(size_t capacity) {
  ...
  if (!_data && capacity) {
    _data = kalloc_type_var_impl(
        KT_IOMD_MIXED_DATA, capacity,
...
void _IOMemoryDescriptorMixedData::free() {
  freeMemory();
  OSObject::free();
}

void _IOMemoryDescriptorMixedData::freeMemory() {
  kfree_type_var_impl(KT_IOMD_MIXED_DATA, _data, _capacity);
  _data = nullptr;
  _capacity = _length = 0;
}

bool _IOMemoryDescriptorMixedData::appendBytes(const void *bytes,
                                               size_t length) {
...
bool _IOMemoryDescriptorMixedData::setLength(size_t length) {
  if (!_data || (length > _capacity)) {
    void *newData;

    newData =
        __krealloc_type(KT_IOMD_MIXED_DATA, _data, _capacity, length,
                        Z_VM_TAG_BT(Z_WAITOK_ZERO, VM_KERN_MEMORY_IOKIT), NULL);
    if (!newData) {
      return false;
    }
...
const void *_IOMemoryDescriptorMixedData::getBytes() const {
  return _length ? _data : nullptr;
}

size_t _IOMemoryDescriptorMixedData::getLength() const {
