...
#define NVRAM_CHRP_LENGTH_BLOCK_SIZE                                           \
  0x10 // CHRP length field is in 16 byte blocks

typedef struct chrp_nvram_header { // 16 bytes
  uint8_t sig;
  uint8_t cksum; // checksum on sig, len, and name
  uint16_t len; // total length of the partition in 16 byte blocks starting with
                // the signature
  // and ending with the last byte of data area, ie len includes its own header
  // size
  char name[12];
  uint8_t data[0];
} chrp_nvram_header_t;
...
class IONVRAMCHRPHandler : public IODTNVRAMFormatHandler,
                           IOTypedOperatorsMixin<IONVRAMCHRPHandler> {
private:
  bool _newData;
  bool _reload;
  ...
  static OSSharedPtr<OSData> unescapeBytesToData(const uint8_t *bytes,
                                                 uint32_t length);
  static OSSharedPtr<OSData> escapeDataToData(OSData *value);

  static bool
  convertPropToObject(const uint8_t *propName, uint32_t propNameLength,
...
public:
  virtual ~IONVRAMCHRPHandler() APPLE_KEXT_OVERRIDE;
  IONVRAMCHRPHandler();
  static bool isValidImage(const uint8_t *image, IOByteCount length);
  static IONVRAMCHRPHandler *init(IODTNVRAM *provider, const uint8_t *image,
                                  IOByteCount length);
...
static const char *get_bank_version_string(int version) {
  switch (version) {
  case kNVRAMVersion1:
...
static uint32_t adler32(const uint8_t *buffer, size_t length) {
  uint32_t offset;
...
static uint32_t nvram_get_adler(uint8_t *buf, int version) {
  return ((struct apple_nvram_header *)buf)->adler;
}

static uint32_t adler32_with_version(const uint8_t *buf, size_t len,
                                     int version) {
  size_t offset;
...
static uint8_t chrp_checksum(const struct chrp_nvram_header *hdr) {
  ...
  const uint8_t *p;
  const uint8_t *begin =
      (const uint8_t *)hdr + offsetof(struct chrp_nvram_header, len);
  const uint8_t *end =
      (const uint8_t *)hdr + offsetof(struct chrp_nvram_header, data);
...
static IOReturn nvram_validate_header_v1v2(const uint8_t *buf,
                                           uint32_t *generation, int version) {
  ...
  // <rdar://problem/73454488> Recovery Mode [Internal Build] 18D52-->18E141
  // [J307/308 Only] we can only compare the first "nvram" parts of the name as
  // some devices have additional junk from a previous build likely copying past
  // bounds of the "nvram" name in the const section
  if (memcmp(header_string, chrp_header->name, strlen(header_string)) == 0) {
    ...
    DEBUG_INFO("invalid bank for \"%s\", name = %#02x %#02x %#02x %#02x\n",
               header_string, chrp_header->name[0], chrp_header->name[1],
               chrp_header->name[2], chrp_header->name[3]);
  }
...
static void nvram_set_apple_header(uint8_t *buf, size_t len,
                                   uint32_t generation, int version) {
  if (version == kNVRAMVersion1 || version == kNVRAMVersion2) {
...
static NVRAMVersion validateNVRAMVersion(const uint8_t *buf, size_t len,
                                         uint32_t *generation) {
  NVRAMVersion version = kNVRAMVersionUnknown;

  if (nvram_validate_header_v1v2(buf, generation, kNVRAMVersion1) ==
      kIOReturnSuccess) {
    version = kNVRAMVersion1;
    goto exit;
  }

  if (nvram_validate_header_v1v2(buf, generation, kNVRAMVersion2) ==
      kIOReturnSuccess) {
...
bool IONVRAMCHRPHandler::isValidImage(const uint8_t *image,
                                      IOByteCount length) {
  return validateNVRAMVersion(image, length, nullptr) != kNVRAMVersionUnknown;
}

IONVRAMCHRPHandler *IONVRAMCHRPHandler::init(IODTNVRAM *provider,
                                             const uint8_t *image,
                                             IOByteCount length) {
  bool propertiesOk;
  ...
  require_action(length == handler->_bankSize, exit,
                 DEBUG_ERROR("length 0x%llx != _bankSize 0x%x\n", length,
                             handler->_bankSize));
...
IONVRAMCHRPHandler::IONVRAMCHRPHandler() : _commonPartitionSize(0x800) {}

IOReturn IONVRAMCHRPHandler::flush(const uuid_t guid, IONVRAMOperation op) {
...
IOReturn IONVRAMCHRPHandler::reloadInternal(void) {
...
void IONVRAMCHRPHandler::reload(void) {
  _reload = true;

  DEBUG_INFO("reload marked\n");
}

IOReturn IONVRAMCHRPHandler::unserializeImage(const uint8_t *image,
                                              IOByteCount length) {
  ...
  while (currentOffset < _bankSize) {
    ...
    const chrp_nvram_header_t *header =
        (chrp_nvram_header_t *)(_nvramImage + currentOffset);
    const uint8_t common_v1_name[sizeof(header->name)] = {
        NVRAM_CHRP_PARTITION_NAME_COMMON_V1};
    const uint8_t common_v2_name[sizeof(header->name)] = {
        NVRAM_CHRP_PARTITION_NAME_COMMON_V2};
    const uint8_t system_v1_name[sizeof(header->name)] = {
        NVRAM_CHRP_PARTITION_NAME_SYSTEM_V1};
    const uint8_t system_v2_name[sizeof(header->name)] = {
    ...
    common_partition =
        (memcmp(header->name, common_v1_name, sizeof(header->name)) == 0) ||
        (memcmp(header->name, common_v2_name, sizeof(header->name)) == 0);
    system_partition =
        (memcmp(header->name, system_v1_name, sizeof(header->name)) == 0) ||
...
exit:
  ...
  DEBUG_ALWAYS("NVRAM : commonPartitionOffset - %#x, commonPartitionSize - "
               "%#x, systemPartitionOffset - %#x, systemPartitionSize - %#x\n",
               _commonPartitionOffset, _commonPartitionSize,
               _systemPartitionOffset, _systemPartitionSize);

  return ret;
}

IOReturn IONVRAMCHRPHandler::unserializeVariables(void) {
  ...
  NVRAMRegionInfo variableRegions[] = {
      {kIONVRAMPartitionCommon, _commonPartitionOffset, _commonPartitionSize},
      {kIONVRAMPartitionSystem, _systemPartitionOffset, _systemPartitionSize}};

  DEBUG_INFO("...\n");

  for (regionIndex = 0; regionIndex < ARRAY_SIZE(variableRegions);
       regionIndex++) {
    ...
    while (cnt < currentRegion->size) {
      ...
      if (convertPropToObject(propName, propNameLength, propData,
                              propDataLength, propSymbol, propObject)) {
        ...
        if (currentRegion->type == kIONVRAMPartitionCommon) {
          ...
          canonicalKey =
              keyWithGuidAndCString(gAppleSystemVariableGuid, varName);
        }
  ...
  DEBUG_INFO("%s _varDict=%p\n", __FUNCTION__,
             _varDict ? _varDict.get() : nullptr);

  return kIOReturnSuccess;
}

IOReturn IONVRAMCHRPHandler::serializeVariables(void) {
  IOReturn ret;
  bool ok = false;
  ...
  require_action(
      _nvramController != nullptr, exit,
      (ret = kIOReturnNotReady, DEBUG_ERROR("No _nvramController\n")));
  require_action(_newData == true, exit,
                 (ret = kIOReturnSuccess, DEBUG_INFO("No _newData to sync\n")));
  require_action(_bankSize != 0, exit,
                 (ret = kIOReturnSuccess, DEBUG_INFO("No nvram size info\n")));
  require_action(_nvramImage != nullptr, exit,
                 (ret = kIOReturnSuccess, DEBUG_INFO("No nvram image info\n")));

  nvramImage = IONewZeroData(uint8_t, _bankSize);
  require_action(nvramImage != nullptr, exit,
                 (ret = kIOReturnNoMemory,
                  DEBUG_ERROR("Can't create NVRAM image copy\n")));
...
IOReturn IONVRAMCHRPHandler::setVariableInternal(const uuid_t varGuid,
  ...
  if (remove == false) {
    ...
    if (systemVar) {
      if ((newSize + _systemUsed - existingSize) > _systemPartitionSize) {
        DEBUG_ERROR("No space left in system partition, need=%#x, "
                    "_systemUsed=%#x, _systemPartitionSize=%#x\n",
      ...
      if ((newSize + _commonUsed - existingSize) > _commonPartitionSize) {
        DEBUG_ERROR("No space left in common partition, need=%#x, "
                    "_commonUsed=%#x, _commonPartitionSize=%#x\n",
...
IOReturn IONVRAMCHRPHandler::setVariable(const uuid_t varGuid,
                                         const char *variableName,
                                         OSObject *object) {
  uuid_t destGuid;
...
uint32_t IONVRAMCHRPHandler::findCurrentBank(uint32_t *gen) {
  ...
  for (unsigned int i = 0; i < _bankCount; i++) {
    ...
    bankVer = validateNVRAMVersion((uint8_t *)&storeHeader, sizeof(storeHeader),
                                   &bankGen);
...
bool IONVRAMCHRPHandler::setController(IONVRAMController *controller) {
  IOReturn ret;
...
IOReturn IONVRAMCHRPHandler::sync(void) {
  IOReturn ret;
  ...
  require_noerr_action(ret, exit,
                       DEBUG_ERROR("serializeVariables failed, ret=%#x", ret));
...
uint32_t IONVRAMCHRPHandler::getGeneration(void) const { return _generation; }

uint32_t IONVRAMCHRPHandler::getVersion(void) const { return _version; }

uint32_t IONVRAMCHRPHandler::getSystemUsed(void) const { return _systemUsed; }

uint32_t IONVRAMCHRPHandler::getCommonUsed(void) const { return _commonUsed; }

bool IONVRAMCHRPHandler::getSystemPartitionActive(void) const {
...
OSSharedPtr<OSData> IONVRAMCHRPHandler::escapeDataToData(OSData *value) {
  ...
  while (wherePtr < endPtr) {
    ...
    if ((byte == 0x00) || (byte == 0xFF)) {
      for (; ((wherePtr - startPtr) < 0x7F) && (wherePtr < endPtr) &&
             (byte == *wherePtr);
           wherePtr++) {
      }
...
bool IONVRAMCHRPHandler::convertPropToObject(const uint8_t *propName,
                                             uint32_t propNameLength,
                                             const uint8_t *propData,
                                             uint32_t propDataLength,
                                             const OSSymbol **propSymbol,
...
bool IONVRAMCHRPHandler::convertPropToObject(
  ...
  bool ok = convertPropToObject(propName, propNameLength, propData,
                                propDataLength, &propSymbolRaw, &propObjectRaw);
...
bool IONVRAMCHRPHandler::convertObjectToProp(uint8_t *buffer, uint32_t *length,
                                             const OSSymbol *propSymbol,
                                             OSObject *propObject) {
  return convertObjectToProp(buffer, length, propSymbol->getCStringNoCopy(),
                             propObject);
}
