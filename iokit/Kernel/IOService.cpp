...
enum { kIOUserServerCheckInTimeoutSecs = 120ULL };
...
OSDefineMetaClassAndStructors(IOService, IORegistryEntry)
    ...
    OSDefineMetaClassAndStructors(
        _IOServiceNotifier,
        IONotifier) OSDefineMetaClassAndStructors(_IOServiceNullNotifier,
                                                  IONotifier)
            ...
            OSDefineMetaClassAndStructors(_IOConfigThread, OSObject)
                ...
                OSDefineMetaClassAndStructors(_IOServiceJob, OSObject)
                    ...
                    OSDefineMetaClassAndStructors(IOResources, IOService)
                        OSDefineMetaClassAndStructors(IOUserResources,
                                                      IOService)
                            OSDefineMetaClassAndStructors(IOExclaveProxy,
                                                          IOService)

                                OSDefineMetaClassAndStructors(
                                    _IOOpenServiceIterator, OSIterator)

                                    OSDefineMetaClassAndStructors(
                                        _IOServiceStateNotification, IOService)
...
#define LOCKREADNOTIFY() IORecursiveLockLock(gNotificationLock)
#define LOCKWRITENOTIFY() IORecursiveLockLock(gNotificationLock)
#define LOCKWRITE2READNOTIFY()
#define UNLOCKNOTIFY() IORecursiveLockUnlock(gNotificationLock)
...
#define SLEEPNOTIFYTO(event, deadline)                                         \
  IORecursiveLockSleepDeadline(gNotificationLock, (void *)(event), deadline,   \
                               THREAD_UNINT)
#define WAKEUPNOTIFY(event)                                                    \
  IORecursiveLockWakeup(gNotificationLock, (void *)(event),                    \
                        /* wake one */ false)
...
#define queue_element(entry, element, type, field)                             \
  do {                                                                         \
...
#define iterqueue(que, elt)                                                    \
  for (queue_entry_t elt = queue_first(que); !queue_end(que, elt);             \
       elt = queue_next(elt))
...
struct IOInterruptAccountingReporter {
  IOSimpleReporter
      *reporter; /* Reporter responsible for communicating the statistics */
  IOInterruptAccountingData
      *statistics; /* The live statistics values, if any */
};
...
static void requireMaxCpuDelay(IOService *service, UInt32 ns, UInt32 delayType);
static IOReturn setLatencyHandler(UInt32 delayType, IOService *target,
                                  bool enable);
...
void systemPowerChange(uint8_t newState, IOService *ackTo, uint32_t ackRef,
                       uint32_t *pMaxWaitForReply);
IOReturn systemPowerChange(void *target, void *refCon, UInt32 messageType,
                           IOService *service, void *messageArgument,
                           vm_size_t argSize);
...
void IOService::initialize(void) {
  ...
  gIODefaultMatchCategoryKey =
      OSSymbol::withCStringNoCopy(kIODefaultMatchCategoryKey);
  gIOMatchedAtBootKey = OSSymbol::withCStringNoCopy(kIOMatchedAtBootKey);

  gIOMatchedServiceCountKey =
      OSSymbol::withCStringNoCopy(kIOMatchedServiceCountKey);
  gIOMatchedPersonalityKey =
      OSSymbol::withCStringNoCopy(kIOMatchedPersonalityKey);
  gIORematchPersonalityKey =
      OSSymbol::withCStringNoCopy(kIORematchPersonalityKey);
  gIORematchCountKey = OSSymbol::withCStringNoCopy(kIORematchCountKey);
  gIODEXTMatchCountKey = OSSymbol::withCStringNoCopy(kIODEXTMatchCountKey);

#if defined(XNU_TARGET_OS_OSX)
  gIOServiceLegacyMatchingRegistryIDKey =
      OSSymbol::withCStringNoCopy(kIOServiceLegacyMatchingRegistryIDKey);
#endif /* defined(XNU_TARGET_OS_OSX) */

  if (!PE_parse_boot_argn("dextrelaunch", &gIODextRelaunchMax,
  ...
  PE_parse_boot_argn("iocthreads", &gMaxConfigThreads,
  ...
  gIOUserServerOneProcessKey =
      OSSymbol::withCStringNoCopy(kIOUserServerOneProcessKey);

  gIOUserServerPreserveUserspaceRebootKey =
      OSSymbol::withCStringNoCopy(kIOUserServerPreserveUserspaceRebootKey);
  ...
  gIOPublishNotification = OSSymbol::withCStringNoCopy(kIOPublishNotification);
  gIOFirstPublishNotification =
      OSSymbol::withCStringNoCopy(kIOFirstPublishNotification);
  gIOMatchedNotification = OSSymbol::withCStringNoCopy(kIOMatchedNotification);
  gIOFirstMatchNotification =
      OSSymbol::withCStringNoCopy(kIOFirstMatchNotification);
  gIOTerminatedNotification =
      OSSymbol::withCStringNoCopy(kIOTerminatedNotification);
  gIOWillTerminateNotification =
      OSSymbol::withCStringNoCopy(kIOWillTerminateNotification);
  ...
  gIOUserClientEntitlementsKey =
      OSSymbol::withCStringNoCopy(kIOUserClientEntitlementsKey);
  ...
  gIODriverKitRequiredEntitlementsKey =
      OSSymbol::withCStringNoCopy(kIODriverKitRequiredEntitlementsKey);
  gIODriverKitTestDriverEntitlementKey =
      OSSymbol::withCStringNoCopy(kIODriverKitTestDriverEntitlementKey);
  ...
  gIOServiceMatchDeferredKey =
      OSSymbol::withCStringNoCopy(kIOServiceMatchDeferredKey);
  gIOServiceNotificationUserKey =
      OSSymbol::withCStringNoCopy(kIOServiceNotificationUserKey);
  ...
  gIOPrimaryDriverTerminateOptionsKey =
      OSSymbol::withCStringNoCopy(kIOPrimaryDriverTerminateOptionsKey);
  gIOMediaKey = OSSymbol::withCStringNoCopy("IOMedia");
  gIOBlockStorageDriverKey =
      OSSymbol::withCStringNoCopy("IOBlockStorageDriver");
  gPhysicalInterconnectKey =
      OSSymbol::withCStringNoCopy("Physical Interconnect");
...
#if defined(__x86_64__)
  sCPULatencyFunctionName[kCpuDelayInterrupt] =
  ...
  assert(gIOServicePlane && gIODeviceMemoryKey && gIOInterruptControllersKey &&
         gIOInterruptSpecifiersKey && gIOResourcesKey && gNotifications &&
         gNotificationLock && gIOProviderClassKey && gIONameMatchKey &&
         gIONameMatchedKey && gIOMatchCategoryKey &&
         gIODefaultMatchCategoryKey && gIOPublishNotification &&
         gIOMatchedNotification && gIOTerminatedNotification && gIOServiceKey &&
  ...
  gIOConsoleLockCallout =
      thread_call_allocate(&IOService::consoleLockTimer, NULL);

  IORegistryEntry::getRegistryRoot()->setProperty(gIOConsoleLockedKey,
                                                  kOSBooleanTrue);
...
#endif
  assert(gIOTerminatePhase2List && gIOStopList && gIOStopProviderList &&
         gIOFinalizeList);

  gDriverKitLaunches = OSSet::withCapacity(0);
  gDriverKitLaunchLock = IORecursiveLockAlloc();
  gIOAssociatedServicesKey =
      OSSymbol::withCStringNoCopy("IOAssociatedServices");
#if CONFIG_EXCLAVES
...
IOService *IOService::probe(IOService *provider, SInt32 *score) { return this; }

bool IOService::start(IOService *provider) { return true; }
...
bool IOService::init(IORegistryEntry *from, const IORegistryPlane *inPlane) {
  bool ret;
...
void IOService::free(void) {
  ...
  if (reserved) {
    if (reserved->interruptStatisticsArray) {
      ...
      IODelete(reserved->interruptStatisticsArray,
               IOInterruptAccountingReporter,
               reserved->interruptStatisticsArrayCount);
    }
  ...
  if (provider) {
    ...
    do {
      ...
      if (wait) {
        ...
        assert_wait_deadline((event_t)&provider->__provider, THREAD_UNINT,
                             deadline);
...
IOService *IOService::getServiceRoot(void) { return gIOServiceRoot; }
...
void IOService::detach(IOService *provider) {
  ...
  IOServiceTrace(IOSERVICE_DETACH, (uintptr_t)regID1, (uintptr_t)(regID1 >> 32),
                 (uintptr_t)regID2, (uintptr_t)(regID2 >> 32));
  ...
  if (busy) {
    ...
    if (busy && (__state[1] & kIOServiceTermPhase3State) &&
        (NULL == newProvider)) {
      _adjustBusy(-busy);
    }
  }
...
void IOService::registerService(IOOptionBits options) {
  ...
  if ((this != gIOResources) && (kIOLogRegister & gIOKitDebug)) {
...
void IOService::startMatching(IOOptionBits options) {
  ...
  if (options & kIOServiceDextRequirePowerForMatching) {
    bool ok = gIOPMRootDomain->acquireDriverKitMatchingAssertion() ==
              kIOReturnSuccess;
    if (!ok) {
  ...
  needConfig = (0 == (__state[1] &
                      (kIOServiceNeedConfigState | kIOServiceConfigRunning))) &&
...
void IOService::startDeferredMatches(void) {
#if !NO_KEXTD
  OSArray *array;
...
void IOService::iokitDaemonLaunched(void) {
#if !NO_KEXTD
  if (!OSKext::iokitDaemonAvailable()) {
    panic(kIOKitDaemonName
          " is unavailable in this environment, but it was launched");
  }
...
IOReturn IOService::catalogNewDrivers(OSOrderedSet *newTables) {
  ...
  while ((table = (OSDictionary *)newTables->getFirstObject())) {
    LOCKWRITENOTIFY();
    set = (OSSet *)copyExistingServices(table, kIOServiceRegisteredState,
...
bool IOService::matchPropertyTable(OSDictionary *table) { return true; }
...
IOReturn IOService::getResources(void) { return kIOReturnSuccess; }
...
bool _IOOpenServiceIterator::isValid() { return iter->isValid(); }
...
IOReturn IOService::callPlatformFunction(const OSSymbol *functionName,
  ...
  if (functionName == gIOPlatformQuiesceActionKey ||
    ...
    /*
     * Services which register for IOPlatformQuiesceAction /
     * IOPlatformActiveAction / IOPlatformPanicAction must consume that event
     * themselves, without passing it up to super/IOService.
     */
...
IOReturn IOService::callPlatformFunction(const char *functionName,
  ...
  if (functionSymbol != NULL) {
    result = callPlatformFunction(functionSymbol, waitForFunction, param1,
...
IOPlatformExpert *IOService::getPlatform(void) { return gIOPlatform; }

class IOPMrootDomain *IOService::getPMRootDomain(void) {
  return gIOPMRootDomain;
}

IOService *IOService::getResourceService(void) { return gIOResources; }

IOService *gIOSystemStateNotificationService;

IOService *IOService::getSystemStateNotificationService(void) {
...
void IOService::setPlatform(IOPlatformExpert *platform) {
  ...
  // Test object that will be terminated for dext to match
  {
    ...
    ios->setProperty(gIOMatchCategoryKey->getCStringNoCopy(),
                     "com.apple.iokit.test");
    ios->setProperty(gIOModuleIdentifierKey->getCStringNoCopy(),
                     "com.apple.kpi.iokit");
  ...
  static const char *keys[kCpuNumDelayTypes] = {kIOPlatformMaxBusDelay,
#if defined(__x86_64__)
...
void IOService::publishPMRootDomain(void) {
...
bool IOService::lockForArbitration(bool isSuccessRequired) {
  ...
  if (!queue_empty(&gArbitrationLockQueueFree)) {
    queue_remove_first(&gArbitrationLockQueueFree, element,
                       ArbitrationLockQueueElement *, link);
  } else {
  ...
  found = false;
  queue_iterate(&gArbitrationLockQueueActive, owner,
                ArbitrationLockQueueElement *, link) {
  ...
  if (found) { // this object is already locked
    ...
    if (active->thread != element->thread) { // it is a different thread
      ...
      while (1) {
        ...
        found = false;
        queue_iterate(&gArbitrationLockQueueWaiting, waiting,
                      ArbitrationLockQueueElement *, link) {
          ...
          if (active->thread == element->thread) {
            ...
            if (false == element->required) { // willing to fail?
              ...
              if (victim) {
                ...
                queue_remove(&gArbitrationLockQueueWaiting, victim,
                             ArbitrationLockQueueElement *, link);

                // wake the victim
                wakeup_thread_with_inheritor(
                    &victim->service->__machPortHoldDestroy, // event
      ...
      if (success) { // put the request on the waiting queue?
        ...
        queue_enter(&gArbitrationLockQueueWaiting, element,
                    ArbitrationLockQueueElement *, link);
      ...
      restart_sleep:
        ...
        wait_result = IOLockSleepWithInheritor(
            gArbitrationLockQueueLock, LCK_SLEEP_UNLOCK,
        ...
        if (THREAD_INTERRUPTED == wait_result) {
          ...
          if (found) { // yes, we're still on the waiting queue
            ...
            if (false == element->required) {
              ...
              queue_remove(&gArbitrationLockQueueWaiting, element,
                           ArbitrationLockQueueElement *, link);
  ...
  if (kPutOnActiveQueue == action) {
    queue_enter(&gArbitrationLockQueueActive, element,
                ArbitrationLockQueueElement *, link);
  } else if (kPutOnFreeQueue == action) {
    queue_enter(&gArbitrationLockQueueFree, element,
                ArbitrationLockQueueElement *, link);
  } else {
...
void IOService::unlockForArbitration(void) {
  ...
  found = false;
  queue_iterate(&gArbitrationLockQueueActive, element,
                ArbitrationLockQueueElement *, link) {
  ...
  if (element->count > 1) {
    ...
    queue_remove(&gArbitrationLockQueueActive, element,
                 ArbitrationLockQueueElement *, link);
    ...
    if (KERN_SUCCESS == kr) {
      found = false;
      queue_iterate(&gArbitrationLockQueueWaiting, element,
                    ArbitrationLockQueueElement *, link) {
...
uint32_t IOService::isLockedForArbitration(IOService *service) {
  ...
  // determine whether this object is already locked (ie. on active queue)
  count = 0;
  queue_iterate(&gArbitrationLockQueueActive, active,
                ArbitrationLockQueueElement *, link) {
...
void IOService::setMachPortHoldDestroy(bool holdDestroy) {
  __machPortHoldDestroy = holdDestroy;
}

bool IOService::machPortHoldDestroy() { return __machPortHoldDestroy; }

void IOService::applyToProviders(IOServiceApplierFunction applier,
                                 void *context) {
  applyToParents((IORegistryEntryApplierFunction)applier, context,
                 gIOServicePlane);
}

void IOService::applyToClients(IOServiceApplierFunction applier,
                               void *context) {
  applyToChildren((IORegistryEntryApplierFunction)applier, context,
                  gIOServicePlane);
}

static void IOServiceApplierToBlock(IOService *next, void *context) {
  IOServiceApplierBlock block = (IOServiceApplierBlock)context;
  block(next);
}

void IOService::applyToProviders(IOServiceApplierBlock applier) {
  applyToProviders(&IOServiceApplierToBlock, applier);
}

void IOService::applyToClients(IOServiceApplierBlock applier) {
...
IOReturn IOService::messageClient(UInt32 type, OSObject *client, void *argument,
                                  vm_size_t argSize) {
  IOReturn ret;
  IOService *service;
  ...
  if ((service = OSDynamicCast(IOService, client))) {
    ...
    if (willNotify) {
      ret = (*notify->handler)(notify->target, notify->ref, type, this,
                               argument, argSize);
...
static void applyToInterestNotifiers(const IORegistryEntry *target,
...
static void messageClientsApplier(OSObject *object, void *ctx) {
  ...
  ret = context->service->messageClient(context->type, object,
                                        context->argument, context->argSize);
...
IOReturn IOService::messageClients(UInt32 type, void *argument,
                                   vm_size_t argSize) {
  ...
  applyToInterested(gIOGeneralInterest, &messageClientsApplier, &context);
...
IONotifier *IOService::registerInterest(const OSSymbol *typeOfInterest,
  ...
  if (notify->init()) {
    rc = registerInterestForNotifier(notify, typeOfInterest, handler, target,
                                     ref);
  }
...
IOServiceInterestHandlerToBlock(void *target __unused, void *refCon,
  ...
  return ((IOServiceInterestHandlerBlock)refCon)(messageType, provider,
...
IONotifier *IOService::registerInterest(const OSSymbol *typeOfInterest,
  ...
  notify = registerInterest(typeOfInterest, &IOServiceInterestHandlerToBlock,
                            NULL, handler);
  return notify;
}

IOReturn IOService::registerInterestForNotifier(
    IONotifier *svcNotify, const OSSymbol *typeOfInterest,
    IOServiceInterestHandler handler, void *target, void *ref) {
  IOReturn rc = kIOReturnSuccess;
  _IOServiceInterestNotifier *notify = NULL;

  if (!svcNotify ||
      !(notify = OSDynamicCast(_IOServiceInterestNotifier, svcNotify)) ||
      !handler) {
...
void _IOServiceInterestNotifier::wait() {
  ...
  do {
    doWait = false;
    queue_iterate(&handlerInvocations, next, _IOServiceNotifierInvocation *,
                  link) {
...
#define TLOG(fmt, args...)                                                     \
  {                                                                            \
    if (kIOLogYield & gIOKitDebug) {                                           \
      IOLog("[%llx] ", thread_tid(current_thread()));                          \
      IOLog(fmt, ##args);                                                      \
    }                                                                          \
  }

static void _workLoopAction(IOWorkLoop::Action action, IOService *service,
...
bool IOService::terminatePhase1(IOOptionBits options) {
  ...
  IOServiceTrace(IOSERVICE_TERMINATE_PHASE1, (uintptr_t)regID,
                 (uintptr_t)(regID >> 32), (uintptr_t)this, (uintptr_t)options);
  ...
  while (victim) {
    ...
    if (didInactive) {
      ...
      IOServiceTrace(IOSERVICE_TERM_SET_INACTIVE, (uintptr_t)regID1,
                     (uintptr_t)(regID1 >> 32), (uintptr_t)victim->__state[1],
                     (uintptr_t)0);
      ...
      if (!didInactive) {
          ...
          do {
            ...
            if (wait) {
              TLOG("%s[0x%qx]::waitPhase1(%s[0x%qx])\n", getName(),
                   getRegistryEntryID(), victim->getName(),
                   victim->getRegistryEntryID());
        ...
        victim->__state[0] &=
            ~(kIOServiceRegisteredState | kIOServiceMatchedState |
        ...
        if ((options & kIOServiceTerminateWithRematch) && (victim == this)) {
          if ((options & kIOServiceTerminateWithRematchCurrentDext)) {
            ...
            if (rematchProvider) {
              ...
              rematchProvider->setProperty(gIORematchPersonalityKey,
                                           rematchProps);
    ...
    if (didInactive) {
      OSArray *notifiers;
      notifiers =
          victim->copyNotifiers(gIOTerminatedNotification, 0, 0xffffffff);
      ...
      if (iter) {
        while ((client = (IOService *)iter->getNextObject())) {
          ...
          IOServiceTrace((ok ? IOSERVICE_TERMINATE_REQUEST_OK
                             : IOSERVICE_TERMINATE_REQUEST_FAIL),
                         (uintptr_t)regID1, (uintptr_t)(regID1 >> 32),
                         (uintptr_t)regID2, (uintptr_t)(regID2 >> 32));
...
void IOService::waitToBecomeTerminateThread(void) {
  ...
  do {
    ...
    if (wait) {
      IOLockSleepWithInheritor(gJobsLock, LCK_SLEEP_DEFAULT,
                               &gIOTerminateThread, gIOTerminateThread,
...
void IOService::scheduleTerminatePhase2(IOOptionBits options) {
  ...
  IOServiceTrace(IOSERVICE_TERM_SCHED_PHASE2, (uintptr_t)regID1,
                 (uintptr_t)(regID1 >> 32), (uintptr_t)__state[1],
                 (uintptr_t)options);
  ...
  if ((options & kIOServiceSynchronous) &&
    ...
    do {
      ...
      if (wait) {
        ...
        waitResult = IOLockSleepDeadline(gJobsLock, &gIOTerminateWork, deadline,
                                         THREAD_UNINT);
        if (__improbable(waitResult == THREAD_TIMED_OUT)) {
          IOLog(
              "%s[0x%qx]::terminate(kIOServiceSynchronous): THREAD_TIMED_OUT. "
    ...
    if (0 == gIOTerminateWork++) {
      assert(gIOTerminateWorkerThread);
      IOLockWakeup(gJobsLock, (event_t)&gIOTerminateWork,
                   /* one-thread */ false);
    }
  }
...
void IOService::scheduleStop(IOService *provider) {
  ...
  TLOG("%s[0x%qx]::scheduleStop(%s[0x%qx])\n", getName(), regID1,
       provider->getName(), regID2);
  IOServiceTrace(IOSERVICE_TERMINATE_SCHEDULE_STOP, (uintptr_t)regID1,
                 (uintptr_t)(regID1 >> 32), (uintptr_t)regID2,
                 (uintptr_t)(regID2 >> 32));
...
void IOService::scheduleFinalize(bool now) {
  ...
  IOServiceTrace(IOSERVICE_TERMINATE_SCHEDULE_FINALIZE, (uintptr_t)regID1,
                 (uintptr_t)(regID1 >> 32), 0, 0);
...
bool IOService::didTerminate(IOService *provider, IOOptionBits options,
                             bool *defer) {
  if (reserved->uvars) {
  ...
  if (false == *defer) {
    if (lockForArbitration(true)) {
      ...
      // -- compat
        ...
        message(kIOMessageServiceIsRequestingClose, provider,
...
void IOService::actionWillTerminate(IOService *victim, IOOptionBits options,
                                    OSArray *doPhase2List, bool user,
  ...
  if (iter) {
    while ((client = (IOService *)iter->getNextObject())) {
      ...
      TLOG("%s[0x%qx]::willTerminate(%s[0x%qx], %08llx)\n", client->getName(),
           regID1, victim->getName(), regID2, (long long)options);
      IOServiceTrace(IOSERVICE_TERMINATE_WILL, (uintptr_t)regID1,
                     (uintptr_t)(regID1 >> 32), (uintptr_t)regID2,
                     (uintptr_t)(regID2 >> 32));
...
void IOService::actionDidTerminate(IOService *victim, IOOptionBits options,
  ...
  victim->messageClients(kIOMessageServiceIsTerminated,
                         (void *)(uintptr_t)options);
  ...
  if (iter) {
    while ((client = (IOService *)iter->getNextObject())) {
      ...
      IOServiceTrace(
          (defer ? IOSERVICE_TERMINATE_DID_DEFER : IOSERVICE_TERMINATE_DID),
          (uintptr_t)regID1, (uintptr_t)(regID1 >> 32), (uintptr_t)regID2,
          (uintptr_t)(regID2 >> 32));

      TLOG("%s[0x%qx]::didTerminate(%s[0x%qx], defer %d)\n", client->getName(),
           regID1, victim->getName(), regID2, defer);
    }
    iter->release();
  }
}
...
void IOService::actionWillStop(IOService *victim, IOOptionBits options,
  ...
  if (iter) {
    while ((provider = (IOService *)iter->getNextObject())) {
      ...
      TLOG("%s[0x%qx]::willTerminate(%s[0x%qx], %08llx)\n", victim->getName(),
           regID2, provider->getName(), regID1, (long long)options);
      IOServiceTrace(IOSERVICE_TERMINATE_WILL, (uintptr_t)regID2,
                     (uintptr_t)(regID2 >> 32), (uintptr_t)regID1,
                     (uintptr_t)(regID1 >> 32));
...
void IOService::actionDidStop(IOService *victim, IOOptionBits options,
  ...
  if (iter) {
    while ((provider = (IOService *)iter->getNextObject())) {
      ...
      IOServiceTrace(
          (defer ? IOSERVICE_TERMINATE_DID_DEFER : IOSERVICE_TERMINATE_DID),
          (uintptr_t)regID2, (uintptr_t)(regID2 >> 32), (uintptr_t)regID1,
          (uintptr_t)(regID1 >> 32));

      TLOG("%s[0x%qx]::didTerminate(%s[0x%qx], defer %d)\n", victim->getName(),
           regID2, provider->getName(), regID1, defer);
    }
    iter->release();
  }
}
...
void IOService::actionFinalize(IOService *victim, IOOptionBits options,
                               ...
                               void *unused3 __unused) {
  uint64_t regID1 = victim->getRegistryEntryID();
  TLOG("%s[0x%qx]::finalize(%08llx)\n", victim->getName(), regID1,
       (long long)options);
  IOServiceTrace(IOSERVICE_TERMINATE_FINALIZE, (uintptr_t)regID1,
                 (uintptr_t)(regID1 >> 32), 0, 0);
...
void IOService::actionStop(IOService *provider, IOService *client,
  ...
  TLOG("%s[0x%qx]::stop(%s[0x%qx])\n", client->getName(), regID2,
       provider->getName(), regID1);
  IOServiceTrace(IOSERVICE_TERMINATE_STOP, (uintptr_t)regID1,
                 (uintptr_t)(regID1 >> 32), (uintptr_t)regID2,
                 (uintptr_t)(regID2 >> 32));
...
void IOService::terminateWorker(IOOptionBits options) {
  ...
  do {
    ...
    while ((victim = (IOService *)gIOTerminatePhase2List->getObject(0))) {
      ...
      uint64_t regID1 = victim->getRegistryEntryID();
      IOServiceTrace(IOSERVICE_TERM_START_PHASE2, (uintptr_t)regID1,
                     (uintptr_t)(regID1 >> 32), (uintptr_t)0, (uintptr_t)0);
      ...
      while (victim) {
        ...
        if (doPhase2) {
          ...
          if (doPhase2) {
            ...
            uint64_t regID1 = victim->getRegistryEntryID();
            IOServiceTrace(IOSERVICE_TERM_TRY_PHASE2, (uintptr_t)regID1,
                           (uintptr_t)(regID1 >> 32),
                           (uintptr_t)victim->__state[1], (uintptr_t)0);

            doPhase2 = (0 == (victim->__state[1] & (kIOServiceTermPhase1State |
                                                    kIOServiceTermPhase2State |
                                                    kIOServiceConfigState)));

            if (doPhase2 && (iter = victim->getClientIterator())) {
              while (doPhase2 &&
                     (client = (IOService *)iter->getNextObject())) {
                doPhase2 = (0 == (client->__state[1] & kIOServiceStartState));
                if (!doPhase2) {
                  uint64_t regID1 = client->getRegistryEntryID();
                  IOServiceTrace(IOSERVICE_TERM_UC_DEFER, (uintptr_t)regID1,
                                 (uintptr_t)(regID1 >> 32),
                                 (uintptr_t)client->__state[1], (uintptr_t)0);
        ...
        if (doPhase2) {
          if (kIOServiceNeedWillTerminate & victim->__state[1]) {
            if (NULL == victim->reserved->uvars) {
              _workLoopAction((IOWorkLoop::Action)&actionWillStop, victim,
                              (void *)(uintptr_t)options);
            } else {
          ...
          OSArray *notifiers;
          notifiers = victim->copyNotifiers(gIOWillTerminateNotification, 0,
                                            0xffffffff);
          victim->invokeNotifiers(&notifiers);

          _workLoopAction((IOWorkLoop::Action)&actionWillTerminate, victim,
                          (void *)(uintptr_t)options,
                          (void *)(uintptr_t)doPhase2List,
                          (void *)(uintptr_t)false);

          actionWillTerminate(victim, options, doPhase2List, true, NULL);
      ...
      while ((victim = (IOService *)didPhase2List->getObject(0))) {
        ...
        _workLoopAction((IOWorkLoop::Action)&actionDidTerminate, victim,
                        (void *)(uintptr_t)options);
        if (kIOServiceNeedWillTerminate & victim->__state[1]) {
          _workLoopAction((IOWorkLoop::Action)&actionDidStop, victim,
    ...
    do {
      ...
      while ((victim = (IOService *)gIOFinalizeList->getObject(0))) {
        ...
        if (sendFinal) {
          _workLoopAction((IOWorkLoop::Action)&actionFinalize, victim,
                          (void *)(uintptr_t)options);
        }
        IOLockLock(gJobsLock);
        // hold off free
      ...
      for (idx = 0; (!doPhase3) &&
                    (client = (IOService *)gIOStopList->getObject(idx));) {
        ...
        if (!provider->isChild(client, gIOServicePlane)) {
          ...
          TLOG("%s[0x%qx]::nop stop(%s[0x%qx])\n", client->getName(), regID2,
               provider->getName(), regID1);
          IOServiceTrace(IOSERVICE_TERMINATE_STOP_NOP, (uintptr_t)regID1,
                         (uintptr_t)(regID1 >> 32), (uintptr_t)regID2,
                         (uintptr_t)(regID2 >> 32));
        } else {
          // a terminated client is not ready for stop if it has clients, skip
          // it
          bool deferStop =
              (0 != (kIOServiceInactiveState & client->__state[0]));
          IOLockUnlock(gJobsLock);
          if (deferStop && client->lockForArbitration(true)) {
            ...
            if (deferStop) {
              ...
              IOServiceTrace(IOSERVICE_TERMINATE_STOP_DEFER, (uintptr_t)regID1,
                             (uintptr_t)(regID1 >> 32), (uintptr_t)regID2,
                             (uintptr_t)(regID2 >> 32));
          ...
          _workLoopAction((IOWorkLoop::Action)&actionStop, provider,
                          (void *)client);
          IOLockLock(gJobsLock);
    ...
    if (!moreToDo) {
      ...
      IOServiceTrace(IOSERVICE_TERMINATE_DONE,
...
static void serviceOpenMessageApplier(OSObject *object, void *ctx) {
  ...
  if (object != context->excludeClient) {
    context->service->messageClient(context->type, object,
                                    (void *)(uintptr_t)context->options);
  }
}

bool IOService::open(IOService *forClient, IOOptionBits options, void *arg) {
  bool ok;
  ...
  if (ok && forClient && forClient->reserved->uvars &&
      forClient->reserved->uvars->userServer) {
...
void IOService::close(IOService *forClient, IOOptionBits options) {
  ...
  if (wasClosed) {
    ...
    if (forClient && forClient->reserved->uvars &&
        forClient->reserved->uvars->userServer) {
      forClient->reserved->uvars->userServer->serviceClose(this, forClient);
    }
  }
...
bool IOService::handleOpen(IOService *forClient, IOOptionBits options,
                           void *arg) {
  bool ok;
...
void IOService::handleClose(IOService *forClient, IOOptionBits options) {
 ...
 * Probing & starting
 */
static SInt32 IONotifyOrdering(const OSMetaClassBase *inObj1,
...
IOServiceOrdering(const OSMetaClassBase *inObj1, const OSMetaClassBase *inObj2,
                  void *ref) {
...
IOService *IOService::copyClientWithCategory(const OSSymbol *category) {
  ...
  if (iter) {
    while ((service = (IOService *)iter->getNextObject())) {
      ...
      nextCat = (const OSSymbol *)OSDynamicCast(
          OSSymbol, service->getProperty(gIOMatchCategoryKey));
      if (category == nextCat) {
        service->retain();
        break;
      }
...
IOService *IOService::getClientWithCategory(const OSSymbol *category) {
  IOService *service = copyClientWithCategory(category);
  if (service) {
    service->release();
  }
  return service;
...
bool IOService::invokeNotifiers(OSArray *willSend[]) {
  ...
  *willSend = NULL;
  ...
  for (unsigned int idx = 0;
       (notify = (_IOServiceNotifier *)array->getObject(idx)); idx++) {
...
TUNABLE(bool, iokit_print_verbose_match_logs, "iokit_print_verbose_match_logs",
        false);
...
void IOService::probeCandidates(OSOrderedSet *matches) {
  ...
  while (!needReloc && (nextMatch = matches->getFirstObject())) {
    ...
    do {
      ...
      category = OSDynamicCast(OSSymbol, match->getObject(gIOMatchCategoryKey));
      if (NULL == category) {
      ...
      if (false ==
          matchPassive(props, kIOServiceChangesOK | kIOServiceClassDone)) {
        break;
      }
      if (isReplacementCandidate) {
        if (canTerminateForReplacement(client)) {
          client->terminate(kIOServiceTerminateNeedWillTerminate |
      ...
      if (NULL == familyMatches) {
        familyMatches = OSOrderedSet::withCapacity(1, IOServiceOrdering,
...
#endif
      do {
        symbol = OSDynamicCast(OSSymbol, props->getObject(gIOClassKey));
        if (!symbol) {
          continue;
        }

        // IOLog("%s alloc (symbol %p props %p)\n", symbol->getCStringNoCopy(),
        // IOSERVICE_OBFUSCATE(symbol), IOSERVICE_OBFUSCATE(props));
        ...
        category =
            OSDynamicCast(OSSymbol, props->getObject(gIOMatchCategoryKey));
        if (NULL == category) {
          category = gIODefaultMatchCategoryKey;
        }
        ...
        if (debugFlags & kIOLogProbe) {
          LOG("%s[0x%qx]::probe(%s)\n", inst->getMetaClass()->getClassName(),
              inst->getRegistryEntryID(), getName());
        }
#endif
        ...
        if (NULL == startList) {
          startList = OSOrderedSet::withCapacity(1, IOServiceOrdering,
                                                 (void *)gIOProbeScoreKey);
  ...
  if ((debugFlags & kIOLogMatch) && iokit_print_verbose_match_logs &&
      startDict != NULL) {
    ...
    startDict->iterateObjects(^(const OSSymbol *key, OSObject *value) {
      ...
      if (startList) {
        IOLog("%s(0x%qx): category %s, %u matches\n", getName(),
              getRegistryEntryID(), key->getCStringNoCopy(),
              startList->getCount());
        startList->iterateObjects(^(OSObject *obj) {
          IOService *match = OSDynamicCast(IOService, obj);
          OSNumber *probeScore =
              OSDynamicCast(OSNumber, match->getProperty(gIOProbeScoreKey));

          if (match && probeScore) {
            IOLog("%s(0x%qx): category %s: matched %s, probe score %qd\n",
                  getName(), getRegistryEntryID(), key->getCStringNoCopy(),
                  match->getName(), probeScore->unsigned64BitValue());
          }
          return false;
        });
      }
      ...
      while (true // (!started)
        ...
        if (debugFlags & kIOLogStart) {
          ...
          LOG("%s[0x%qx]::start(%s) <%d>\n", inst->getName(),
              inst->getRegistryEntryID(), getName(), inst->getRetainCount());
        }
#endif
        if (false == started) {
#if !NO_KEXTD
          ...
          if (matchDeferred &&
              (-1U == gIOMatchDeferList->getNextIndexOfObject(this, 0))) {
          ...
          if (!matchDeferred) {
            ...
            if ((debugFlags & kIOLogStart) && (false == started)) {
              LOG("%s[0x%qx]::start(%s[0x%qx]) <%d> failed\n", inst->getName(),
                  inst->getRegistryEntryID(), getName(), getRegistryEntryID(),
                  inst->getRetainCount());
            }
#endif
            if (!started) {
              if (inst->propertyExists(gIOServiceMatchDeferredKey)) {
                matchDeferred = true;
              } else if (inst->reserved->uvars &&
                         inst->reserved->uvars->userServer &&
                         !inst->reserved->uvars->instantiated &&
                ...
                // Do not start the next candidate, otherwise the rematched dext
                // might not be able to replace it
                OSString *bundleID = OSDynamicCast(
                    OSString, inst->getProperty(gIOModuleIdentifierKey));
  ...
  if (needReloc) {
    ...
    if (adjBusy) {
      IOServiceTrace(IOSERVICE_MODULESTALL, (uintptr_t)regID,
                     (uintptr_t)(regID >> 32), (uintptr_t)this, 0);

      __state[1] |= kIOServiceModuleStallState;
    }
  } else if (__state[1] & kIOServiceModuleStallState) {
    IOServiceTrace(IOSERVICE_MODULEUNSTALL, (uintptr_t)regID,
                   (uintptr_t)(regID >> 32), (uintptr_t)this, 0);
...
static __attribute__((noinline, not_tail_called)) IOUserServer *
__WAITING_FOR_USER_SERVER__(IOUserServerCheckInToken *token) {
  ...
  server = IOService::waitForMatchingServiceWithToken(
      matching, kIOUserServerCheckInTimeoutSecs * NSEC_PER_SEC, token);
...
bool IOService::getWillUserspaceReboot() {
  return os_atomic_load(&gInUserspaceReboot, relaxed);
}

void IOService::setWillUserspaceReboot() {
  IOLockLock(gJobsLock);
#if !NO_KEXTD
  IOService *provider;
  IOService *service;
  ...
  if (gIOMatchDeferList) {
    ...
    if (iter) {
      do {
        iter->reset();
        while ((service = (IOService *)iter->getNextObject())) {
          /* Rematch providers of services that will be terminated on userspace
           * reboot, after the userspace reboot is complete. This normally
           * happens automatically as the IOKit daemon sends personalities to
           * the kernel which triggers rematching. But if this doesn't happen
           * (for example, if a feature flag is turned off), then these services
           * will never get rematched.
           */
          if (service->propertyHasValue(gIOMatchDeferKey, kOSBooleanTrue) ||
              service->hasUserServer()) {
            provider = service->getProvider();
            IOLog("deferring %s-%llx (provider of %s-%llx) matching after "
                  "userspace reboot\n",
                  provider->getName(), provider->getRegistryEntryID(),
...
void IOService::userSpaceDidReboot() {
  os_atomic_store(&gInUserspaceReboot, false, relaxed);
}

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

void IOServicePH::init(IOPMrootDomain *root) {
  ...
  fRootNotifier =
      root->registerInterest(gIOPriorityPowerStateInterest,
                             &IOServicePH::systemPowerChange, NULL, NULL);
  ...
  fUserServerAckTimer = thread_call_allocate(
      &IOServicePH::userServerAckTimerExpired, (thread_call_param_t)NULL);
}

void IOServicePH::lock() { IOLockLock(gJobsLock); }

void IOServicePH::unlock() { IOLockUnlock(gJobsLock); }

void IOServicePH::serverAdd(IOUserServer *server) {
  uint32_t idx;
...
void IOServicePH::serverRemove(IOUserServer *server) {
  uint32_t idx;
...
void IOServicePH::serverAck(IOUserServer *server) {
  uint32_t idx;
  IOService *ackTo;
  uint32_t ackToRef;
  ...
  if (fUserServersWait && fUserServersWait->getCount() > 0 && fMatchingWork &&
      fMatchingWork->getCount() > 0) {
...
bool IOServicePH::matchingStart(IOService *service) {
  uint32_t idx;
  bool assertionActive =
      gIOPMRootDomain->acquireDriverKitMatchingAssertion() == kIOReturnSuccess;
  ...
  if (matchNow) {
    ...
    if (!fMatchingDelayed) {
      fMatchingDelayed =
          OSArray::withObjects((const OSObject **)&service, 1, 1);
    } else {
...
void IOServicePH::matchingEnd(IOService *service) {
  uint32_t idx;
  OSArray *notifyServers;
  ...
  if (notifyServers) {
    ...
    if (IsIOServiceSystemStateOff(fSystemState) &&
        IOService::getPMRootDomain()->getSystemSleepType(
            &sleepType, &standbyTimer) == kIOReturnSuccess) {
...
#endif
TUNABLE(bool, dk_panic_on_setpowerstate_hang, "dk_panic_on_setpowerstate_hang",
        DK_SETPOWERSTATE_HANG);

void IOServicePH::userServerAckTimerExpired(void *, void *) {
...
void IOServicePH::systemHalt(int howto) {
  OSArray *notifyServers;
  uint64_t deadline;
  ...
  lock();
  clock_interval_to_deadline(dk_shutdown_timeout_ms, kMillisecondScale,
                             &deadline);
  while (0 < fUserServers->getCount()) {
    fWaitingUserServers = true;
    __assert_only int waitResult = IOLockSleepDeadline(
        gJobsLock, &fWaitingUserServers, deadline, THREAD_UNINT);
...
bool IOServicePH::serverSlept(void) {
  bool ret;
...
TUNABLE(uint32_t, dk_power_state_timeout_ms, "dk_power_state_timeout_ms",
        30000);
...
void IOServicePH::systemPowerChange(uint8_t newState, IOService *ackTo,
                                    uint32_t ackRef,
                                    uint32_t *pMaxWaitForReply) {
  AbsoluteTime deadline;
  ...
  case kIOServiceSystemStateOffPhase2:
    ...
    clock_interval_to_deadline(dk_power_state_timeout_ms, kMillisecondScale,
                               &deadline);
...
IOReturn IOServicePH::systemPowerChange(void *target, void *refCon,
  ...
  case kIOMessageSystemCapabilityChange:
    ...
    if ((params->changeFlags & kIOPMSystemCapabilityWillChange) &&
        ...
        ((params->toCapabilities &
          (kIOPMSystemCapabilityCPU | kIOPMSystemCapabilityAOT)) == 0)) {
      systemPowerChange(kIOServiceSystemStateOffPhase1, service,
                        params->notifyRef, &params->maxWaitForReply);
      ret = kIOReturnSuccess;
    } else if ((params->changeFlags & kIOPMSystemCapabilityWillChange) &&
               (0 != ((params->fromCapabilities ^ params->toCapabilities) &
                      (kIOPMSystemCapabilityCPU | kIOPMSystemCapabilityAOT)))) {
      systemPowerChange((params->toCapabilities & kIOPMSystemCapabilityCPU)
                            ? kIOServiceSystemStateOn
                            : kIOServiceSystemStateAOT,
...
IOReturn IOServicePH::rootWillChangeTo(IOPMPowerFlags flags,
                                       unsigned long state) {
  IOReturn ret;
  uint32_t maxWaitForReply = kIOPMAckImplied;

  if (kIOPMSleepCapability & flags) {
    systemPowerChange(kIOServiceSystemStateOffPhase2, gIOUserResources, 0,
                      &maxWaitForReply);
...
bool IOSystemStateAOT(void) {
  return kIOServiceSystemStateAOT == IOServicePH::fSystemState;
}

bool IOServicePH::checkPMReady(void) {
...
void IOService::__patchProperties(void) {
#if 0
  ...
  ph = false;
  {
    ...
    if ((serverName = OSDynamicCast(OSString, obj))) {
      ...
      if (propertyExists(gIOUserServerNameKey)) {
        reuseRequired = reuse && service->propertyHasValue(
                                     gIOUserServerNameKey,
                                     getProperty(gIOUserServerNameKey));
      }
      serverDUI = OSDynamicCast(
          OSData, service->getProperty(kOSBundleDextUniqueIdentifierKey));
      userServer = IOUserServer::launchUserServer(bundleID, sym, serverTag,
                                                  reuse, &token, serverDUI);
      ...
      if (userServer) {
        ...
        DKLOG(DKS " waiting for server %s-%llx\n", DKN(service),
              tokenServerName->getCStringNoCopy(),
              tokenServerTag->unsigned64BitValue());
      ...
      // Prevent third party drivers from matching IOUserResources/IOResources,
      // except when signed for development
      if (!userServer->isPlatformDriver() &&
          userServer->getCSValidationCategory() !=
    ...
    if (CMP_ABSOLUTETIME(&endTime, &startTime) > 0) {
      ...
      if (nano > 500000000ULL) {
        IOLog("%s[0x%qx]::start took %ld ms\n", service->getName(),
              service->getRegistryEntryID(), (long)(UInt32)(nano / 1000000ULL));
      }
    }
  }
  if (userServer) {
...
void IOService::publishUserResource(const OSSymbol *key, OSObject *value) {
  if (NULL == value) {
    value = (OSObject *)gIOServiceKey;
  }
...
void _IOConfigThread::configThread(const char *name) {
  ...
  do {
    ...
    thread_t thread;
    if (KERN_SUCCESS !=
        kernel_thread_start(&_IOConfigThread::main, inst, &thread)) {
      continue;
    }
...
/*
 * To support driver replacement of boot matched drivers later in boot, drivers
 * can opt-in to be being terminated if a non-boot driver matches their
 * provider, by setting the gIOPrimaryDriverTerminateOptionsKey property. The
 * driver providing the root disk media may not be terminated. IOMedia objects
 * are hidden from user space until all drivers are available, but any
...
bool IOService::hasParent(IOService *parent) {
  IOService *service;

  for (service = this; service && (service != parent);
...
bool IOService::publishHiddenMediaApplier(const OSObject *entry,
                                          void *context) {
...
void IOService::publishHiddenMedia(IOService *parent) {
  ...
  if (ctx.services) {
    ...
    for (idx = 0; (service = (IOService *)ctx.services->getObject(idx));
         idx++) {
      ...
      notifiers[0] =
          service->copyNotifiers(gIOFirstPublishNotification, 0, 0xffffffff);
      if (kIOServiceMatchedState & service->__state[0]) {
        notifiers[1] =
            service->copyNotifiers(gIOMatchedNotification, 0, 0xffffffff);
      }
      if (kIOServiceFirstMatchState & service->__state[0]) {
        notifiers[2] =
            service->copyNotifiers(gIOFirstMatchNotification, 0, 0xffffffff);
      }
      service->unlockForArbitration();
...
// Find the block storage driver providing the root disk, or NULL if not booting
// from a block device

void IOService::setRootMedia(IOService *root) {
  const OSMetaClass *ioblockstoragedriverClass;
  bool unhide;

  ioblockstoragedriverClass =
      OSMetaClass::getMetaClassWithName(gIOBlockStorageDriverKey);
...
// Check if the driver may be terminated when a later driver could be used
// instead

bool IOService::canTerminateForReplacement(IOService *client) {
  IOService *parent;
...
void IOService::doServiceMatch(IOOptionBits options) {
  ...
  while (keepGuessing) {
    ...
    if (matches) {
      ...
      if (gIOServiceHideIOMedia && metaCast(gIOMediaKey) &&
          !(kIOServiceUserUnhidden & __state[1]) && gIOServiceRootMediaParent &&
          !hasParent(gIOServiceRootMediaParent) &&
          propertyExists(gIOPrimaryDriverTerminateOptionsKey,
                         gIOServicePlane)) {
      ...
      if (reRegistered && keepGuessing) {
        iter = OSCollectionIterator::withCollection(
            (OSOrderedSet *)gNotifications->getObject(gIOPublishNotification));
        if (iter) {
      ...
      if (keepGuessing && matches->getCount() &&
          (kIOReturnSuccess == getResources())) {
        if ((this == gIOResources) || (this == gIOUserResources)) {
          if (resourceKeys) {
    ...
    keepGuessing = (reRegistered || (catalogGeneration !=
  ...
  if ((0 == (__state[0] & kIOServiceInactiveState)) &&
    ...
    notifiers[0] = copyNotifiers(gIOMatchedNotification, kIOServiceMatchedState,
                                 0xffffffff);
...
UInt32 IOService::_adjustBusy(SInt32 delta, bool unlock) {
  IOService *next;
  ...
  if (delta) {
    do {
      ...
      wasQuiet = (0 == count);
      if (((delta < 0) && wasQuiet) ||
          ((delta > 0) && (kIOServiceBusyMax == count))) {
        OSReportWithBacktrace("%s: bad busy count (%d,%d)\n", next->getName(),
                              (uint32_t)count, (int)delta);
      } else {
        count += delta;
      }
      ...
      if ((wasQuiet || nowQuiet)) {
        ...
        IOServiceTrace(
            ...
            (uintptr_t)regID, (uintptr_t)(regID >> 32), (uintptr_t)next, 0);
        ...
        applyToInterestNotifiers(next, gIOBusyInterest, &messageClientsApplier,
                                 &context);
...
IOReturn IOService::waitForState(UInt32 mask, UInt32 value, uint64_t timeout) {
  bool wait, done;
  int waitResult = THREAD_AWAKENED;
  ...
  do {
    ...
    if (wait) {
      ...
      if (timeout != UINT64_MAX) {
        ...
        assert_wait_deadline((event_t)this, THREAD_UNINT,
...
IOReturn IOService::waitQuietWithOptions(uint64_t timeout,
                                         IOOptionBits options) {
  IOReturn ret;
  ...
  for (loops = 0; loops < timeoutExtensions; loops++) {
    ...
    if (loops && (kIOReturnSuccess == ret)) {
      ...
      IOLog("busy extended ok[%d], (%llds, %llds)\n", loops,
            timeout / 1000000000ULL, nano / 1000000000ULL);
      break;
...

    {
      ...
      if (leaves && set) {
          ...
          if (next->getBusyState()) {
            ...
            if ((prop = next->copyProperty(kIOServiceBusyTimeoutExtensionsKey,
              ...
              if ((num = OSDynamicCast(OSNumber, prop))) {
                ...
                if (value && (value <= kIOServiceBusyTimeoutExtensionsMax) &&
        ...
        registryRootBusy =
            leaves->getCount() == 1 && leaves->getObject(0) == getServiceRoot();
        multipleEntries = leaves->getCount() > 1;
        s = busyEntriesString;
        busyEntriesStringRemaining = busyEntriesStringLen;

        if (registryRootBusy) {
          snprintf(s, busyEntriesStringRemaining,
                   "registry root held busy, " kIOKitDaemonName
                   " %s checked in",
                   OSKext::iokitDaemonActive() ? "has" : "has not");
        } else {
          while ((next = (IOService *)leaves->getLastObject())) {
            l = snprintf(s, busyEntriesStringRemaining, "%s'%s' (%x,%x)",
                         ((s == busyEntriesString) ? "" : ", "),
                         next->getName(), (uint32_t)next->__state[0],
                         (uint32_t)next->__state[1]);
    ...
    dopanic = (kIOWaitQuietPanics & gIOKitDebug) &&
              (options & kIOWaitQuietPanicOnFailure) && !gIOKitWillTerminate;
    ...
    if (multipleEntries) {
      snprintf(panicString, panicStringLen,
               "%s[%d], (%llds): multiple entries holding the registry busy, "
               "IOKit termination queue depth %u: %s",
               pendingRequests ? "IOKit Daemon (" kIOKitDaemonName ") stall"
                               : "busy timeout",
               loops, timeout / 1000000000ULL, (uint32_t)gIOTerminateWork,
               busyEntriesString ? busyEntriesString : "");
    } else {
      snprintf(panicString, panicStringLen, "%s[%d], (%llds): %s",
               pendingRequests ? "IOKit Daemon (" kIOKitDaemonName ") stall"
                               : "busy timeout",
...
void IOService::resetRematchProperties() {
...
void _IOConfigThread::main(void *arg, wait_result_t result) {
  ...
  kr = thread_policy_set(current_thread(), THREAD_PRECEDENCE_POLICY,
    ...
    if (job) {
      ...
      if (gIOKitDebug & kIOLogConfig) {
        LOG("config(%p): starting on %s, %d\n",
            IOSERVICE_OBFUSCATE(IOThreadSelf()), job->nub->getName(),
            job->type);
      }
      ...
      //		gNumConfigThreads++;
        ...
        if (0 == --gNumConfigThreads) {
          //                    IOLog("MATCH IDLE\n");
          IOLockWakeup(gJobsLock, (event_t)&gNumConfigThreads,
                       /* one-thread */ false);
        }
      }
      IOUnlock(gJobsLock);
...
IOReturn IOService::waitMatchIdle(UInt32 msToWait) {
  ...
  do {
    ...
    if (wait) {
      if (msToWait) {
        if (computeDeadline) {
          clock_interval_to_deadline(msToWait, kMillisecondScale, &deadline);
          computeDeadline = false;
        }
        waitResult = IOLockSleepDeadline(gJobsLock, &gNumConfigThreads,
                                         deadline, THREAD_UNINT);
      } else {
        waitResult = IOLockSleep(gJobsLock, &gNumConfigThreads, THREAD_UNINT);
      }
    }
  } while (wait && (waitResult != THREAD_TIMED_OUT));
  IOLockUnlock(gJobsLock);
...
void IOService::cpusRunning(void) { gCPUsRunning = true; }
...
void _IOServiceJob::pingConfig(_IOServiceJob *job) {
  ...
  //    if( gNumConfigThreads) count++;// assume we're called from a config
  //    thread

  create =
      ((gOutstandingJobs > count) && ((gNumConfigThreads < gMaxConfigThreads) ||
                                      (nub == gIOResources) || !gCPUsRunning));
  if (create) {
    gNumConfigThreads++;
    gNumWaitingThreads++;
...
OSObject *IOService::copyExistingServices(OSDictionary *matching,
  ...
  if ((obj = matching->getObject(gIOProviderClassKey)) && gIOResourcesKey &&
      gIOResourcesKey->isEqualTo(obj) && (service = gIOResources)) {
        ...
        while ((service = (IOService *)iter->getNextObject())) {
          if ((inState == (service->__state[0] & inState)) &&
            ...
            if (_current) {
              ...
              _current = OSSet::withObjects((const OSObject **)&service, 1, 1);
            }
          }
        }
      } while (!service && !iter->isValid());
...
OSIterator *IOService::getMatchingServices(OSDictionary *matching) {
  ...
  iter = (OSIterator *)copyExistingServices(matching, kIOServiceMatchedState);
...
IOService *IOService::copyMatchingService(OSDictionary *matching) {
  ...
  service = (IOService *)copyExistingServices(matching, kIOServiceMatchedState,
                                              kIONotifyOnce);
...
static bool _IOServiceMatchingNotificationHandler(void *target, void *refCon,
                                                  IOService *newService,
                                                  IONotifier *notifier) {
  return (*((_IOServiceNotifier *)notifier)->compatHandler)(target, refCon,
                                                            newService);
}

// internal - call with gNotificationLock
IONotifier *
IOService::setNotification(const OSSymbol *type, OSDictionary *matching,
  ...
  if (notify) {
    ...
    if (handler == &_IOServiceMatchingNotificationHandler) {
      notify->compatHandler =
          ((_IOServiceMatchingNotificationHandlerRef *)ref)->handler;
    ...
    if (NULL == (set = (OSOrderedSet *)gNotifications->getObject(type))) {
      set = OSOrderedSet::withCapacity(1, IONotifyOrdering, NULL);
      if (set) {
...
// internal - call with gNotificationLock
IONotifier *
IOService::doInstallNotification(const OSSymbol *type, OSDictionary *matching,
                                 IOServiceMatchingNotificationHandler handler,
                                 void *target, void *ref, SInt32 priority,
                                 OSIterator **existing) {
  OSIterator *exist;
...
#if !defined(__LP64__)
IONotifier *IOService::installNotification(const OSSymbol *type,
  ...
  result = (_IOServiceNotifier *)installNotification(
      type, matching, &_IOServiceMatchingNotificationHandler, target, &ref,
      priority, existing);
  if (result) {
    matching->release();
  }
...
IOService::installNotification(const OSSymbol *type, OSDictionary *matching,
  ...
  notify = doInstallNotification(type, matching, handler, target, ref, priority,
                                 existing);
...
IONotifier *IOService::addNotification(const OSSymbol *type,
                                       OSDictionary *matching,
                                       IOServiceNotificationHandler handler,
                                       void *target, void *refCon,
                                       SInt32 priority) {
  ...
  result = addMatchingNotification(type, matching,
...
IONotifier *
IOService::addMatchingNotification(const OSSymbol *type, OSDictionary *matching,
                                   IOServiceMatchingNotificationHandler handler,
                                   void *target, void *ref, SInt32 priority) {
  ...
  ret = notify = (_IOServiceNotifier *)installNotification(
      type, matching, handler, target, ref, priority, &existing);
  if (!ret) {
...
static bool IOServiceMatchingNotificationHandlerToBlock(void *target __unused,
                                                        ...
                                                        IOService *newService,
                                                        IONotifier *notifier) {
  return ((IOServiceMatchingNotificationHandlerBlock)refCon)(newService,
                                                             notifier);
}

IONotifier *IOService::addMatchingNotification(
    const OSSymbol *type, OSDictionary *matching, SInt32 priority,
...
void IOService::userServerCheckInTokenCancellationHandler(
    __unused IOUserServerCheckInToken *token, void *ref) {
...
bool IOService::syncNotificationHandler(void * /* target */, void *ref,
                                        IOService *newService,
                                        IONotifier *notifier) {
  LOCKWRITENOTIFY();
  if (!*((IOService **)ref)) {
...
IOService *IOService::waitForMatchingServiceWithToken(
    OSDictionary *matching, uint64_t timeout,
  ...
  if (thread_has_thread_name(current_thread())) {
    ...
    if ((str = OSDynamicCast(OSString, obj))) {
      ...
      snprintf(newName, sizeof(newName), "Waiting_'%s'",
  ...
  if (checkInToken) {
    cancellationHandler = checkInToken->setCancellationHandler(
        &IOService::userServerCheckInTokenCancellationHandler, &cancelArgs);
  }
  ...
  do {
    ...
    result = (IOService *)copyExistingServices(matching, kIOServiceMatchedState,
                                               kIONotifyOnce);
    if (result) {
      break;
    }
...
__dead2 void IOService::deliverNotification(const OSSymbol *type,
                                            IOOptionBits orNewState,
                                            IOOptionBits andNewState) {
  panic("deliverNotification");
}

OSArray *IOService::copyNotifiers(const OSSymbol *type, IOOptionBits orNewState,
                                  IOOptionBits andNewState) {
  _IOServiceNotifier *notify;
  OSIterator *iter;
  ...
  if ((0 == (__state[0] & kIOServiceInactiveState)) ||
    ...
    iter = OSCollectionIterator::withCollection(
        (OSOrderedSet *)gNotifications->getObject(type));
...
IOOptionBits IOService::getState(void) const { return __state[0]; }
...
OSSharedPtr<OSDictionary>
IOService::nameMatching(const OSString *name, OSSharedPtr<OSDictionary> table) {
...
OSDictionary *IOService::nameMatching(const char *name, OSDictionary *table) {
  const OSString *str;
...
OSSharedPtr<OSDictionary>
IOService::nameMatching(const char *name, OSSharedPtr<OSDictionary> table) {
  OSDictionary *result = nameMatching(name, table.get());
  if (table) {
    return OSSharedPtr<OSDictionary>(result, OSRetain);
  } else {
...
OSSharedPtr<OSDictionary>
IOService::resourceMatching(const char *name, OSSharedPtr<OSDictionary> table) {
...
OSDictionary *IOService::propertyMatching(const OSSymbol *key,
...
void _IOServiceNullNotifier::taggedRelease(const void *tag,
...
static bool IOResourcesMatchPropertyTable(IOService *resources,
                                          OSDictionary *table) {
  OSObject *prop;
...
IOService *IOUserResources::resources(void) {
...
bool IOUserResources::init(OSDictionary *dictionary) {
  // Do super init first
  if (!IOService::init()) {
    return false;
  }
...
IOReturn IOUserResources::newUserClient(task_t owningTask, void *securityID,
                                        UInt32 type, OSDictionary *properties,
                                        IOUserClient **handler) {
  return kIOReturnUnsupported;
}

IOWorkLoop *IOUserResources::getWorkLoop() const {
  return getPlatform()->getWorkLoop();
}

bool IOUserResources::matchPropertyTable(OSDictionary *table) {
  return IOResourcesMatchPropertyTable(this, table);
}

IOReturn IOUserResources::powerStateWillChangeTo(IOPMPowerFlags flags,
                                                 unsigned long state,
                                                 IOService *service) {
...
void IOService::consoleLockTimer(thread_call_param_t p0,
                                 thread_call_param_t p1) {
  IOService::updateConsoleUsers(NULL, 0);
}

void IOService::updateConsoleUsers(OSArray *consoleUsers,
  ...
  if (consoleUsers) {
    ...
    for (idx = 0;
         ...
         idx++) {
      gIOConsoleLoggedIn |=
          ((kOSBooleanTrue == user->getObject(gIOConsoleSessionOnConsoleKey)) &&
           (kOSBooleanTrue == user->getObject(gIOConsoleSessionLoginDoneKey)));

      loginLocked &= (kOSBooleanTrue ==
                      user->getObject(gIOConsoleSessionScreenIsLockedKey));
      if (!num) {
        num = OSDynamicCast(
            OSNumber, user->getObject(gIOConsoleSessionScreenLockedTimeKey));
      }
    }
#if HIBERNATION
  ...
  publish =
      (consoleUsers || (locked != regEntry->getProperty(gIOConsoleLockedKey)));
  if (publish) {
  ...
  if (gIOChosenEntry) {
    ...
    gIOChosenEntry->setProperty(kIOScreenLockStateKey, &gIOScreenLockState,
                                sizeof(gIOScreenLockState));

    IOLog("IOConsoleUsers: gIOScreenLockState %d, hs %d, bs %d, now %ld, sm "
          "0x%x\n",
          gIOScreenLockState, gIOHibernateState,
          (gIOConsoleBooterLockState != NULL), now, systemMessage);
  }
#endif /* HIBERNATION */
...
IOReturn IOResources::setProperties(OSObject *properties) {
  ...
  if (!IOCurrentTaskHasEntitlement(kIOResourcesSetPropertyKey)) {
    err = IOUserClient::clientHasPrivilege(current_task(),
...
bool IOService::compareProperty(OSDictionary *matching, const char *key) {
  OSObject *value;
  OSObject *prop;
  bool ok;
...
bool IOService::compareProperty(OSDictionary *matching, const OSString *key) {
  OSObject *value;
  OSObject *prop;
  bool ok;
...
bool IOService::compareProperties(OSDictionary *matching, OSCollection *keys) {
  OSCollectionIterator *iter;
  const OSString *key;
  bool ok = true;
...
OSDictionary *IOService::_copyPropertiesForMatching(void) {
  OSDictionary *matchProps;

  matchProps = dictionaryWithProperties();
  if (matchProps) {
    // merge will check the OSDynamicCast
    matchProps->merge((const OSDictionary *)matchProps->getObject(
        gIOUserServicePropertiesKey));
  }
  return matchProps;
}

bool IOService::matchInternal(OSDictionary *table, uint32_t options,
                              uint32_t *did) {
  OSString *matched;
  OSObject *obj;
  OSString *str;
  ...
  do {
    ...
    if (obj) {
      ...
      if (matchProps) {
        ...
        while (nextDict ||
               (iter && (NULL != (nextDict = OSDynamicCast(
                                      OSDictionary, iter->getNextObject()))))) {
    ...
    if (obj) {
      ...
      if (matchProps) {
        ...
        while (nextKey ||
               (iter && (NULL != (nextKey = OSDynamicCast(
                                      OSString, iter->getNextObject()))))) {
...
#define propMatch(key)                                                         \
  ...
  if (obj) {                                                                   \
    ...
    propMatch(gIOBSDNameKey) propMatch(gIOBSDMajorKey) propMatch(gIOBSDMinorKey)
...
bool IOService::matchPassive(OSDictionary *table, uint32_t options) {
  ...
  do {
    do {
      ...
      nextTable =
          OSDynamicCast(OSDictionary, table->getObject(gIOParentMatchKey));
      if (nextTable) {
      ...
      table =
          OSDynamicCast(OSDictionary, table->getObject(gIOLocationMatchKey));
      if (table) {
    ...
    if (matchParent == true) {
#if defined(XNU_TARGET_OS_OSX)
      // check if service has an alias to search its other "parents" if a parent
      // match isn't found
      OSObject *prop =
          where->copyProperty(gIOServiceLegacyMatchingRegistryIDKey);
...
#if defined(XNU_TARGET_OS_OSX)
    if (where == NULL) {
      // there were no matching parent services, check to see if there are
      // aliased services that have a matching parent
      if (aliasServiceRegIds != NULL) {
        unsigned int numAliasedServices = aliasServiceRegIds->getCount();
        if (numAliasedServices != 0) {
          OSNumber *alternateRegistryID = OSDynamicCast(
              OSNumber, aliasServiceRegIds->getObject(numAliasedServices - 1));
          if (alternateRegistryID != NULL) {
            OSDictionary *alternateMatchingDict =
                IOService::registryEntryIDMatching(
                    alternateRegistryID->unsigned64BitValue());
...
IOReturn IOService::newUserClient(task_t owningTask, void *securityID,
  ...
  if (reserved && reserved->uvars && reserved->uvars->userServer) {
    return reserved->uvars->userServer->serviceNewUserClient(
        this, owningTask, securityID, type, properties, handler);
  }

  if (kIOReturnSuccess ==
      newUserClient(owningTask, securityID, type, handler)) {
    return kIOReturnSuccess;
  }
  ...
  if (prop) {
    if (OSDynamicCast(OSSymbol, prop)) {
      ...
      OSSafeReleaseNULL(prop);
      if (userClientClass) {
        setProperty(gIOUserClientClassKey, (OSObject *)userClientClass);
      }
    } else {
      OSSafeReleaseNULL(prop);
    }
...
IOReturn IOService::newUserClient(task_t owningTask, void *securityID,
  ...
  IOReturn result =
      newUserClient(owningTask, securityID, type, properties, &handlerRaw);
...
IOReturn IOService::newUserClient(task_t owningTask, void *securityID,
  ...
  IOReturn result =
      IOService::newUserClient(owningTask, securityID, type, &handlerRaw);
  handler.reset(handlerRaw, OSNoRetain);
  return result;
}
...
bool IOService::hasUserServer() const {
...
IOReturn IOService::message(UInt32 type, IOService *provider, void *argument) {
  /*
...
static void requireMaxCpuDelay(IOService *service, UInt32 ns,
                               UInt32 delayType) {
  ...
  __typed_allocators_ignore_push CpuDelayEntry *entries =
      (CpuDelayEntry *)sCpuDelayData->getBytesNoCopy();
  __typed_allocators_ignore_pop IOService *holder = NULL;
  ...
  if (ns) {
    ...
    setCpuDelay = true;
    if (kNoReplace == replace) {
      __typed_allocators_ignore_push sCpuDelayData->appendBytes(&ne,
                                                                sizeof(ne));
  ...
  if (setCpuDelay) {
    if (holder && debug_boot_arg) {
      strlcpy(sCPULatencyHolderName[delayType], holder->getName(),
              sizeof(sCPULatencyHolderName[delayType]));
    }
...
#endif /* defined(__x86_64__) */
    sCPULatencyHolder[delayType]->setValue(holder ? holder->getRegistryEntryID()
                                                  : 0);
    ...
    if (handlers) {
      for (unsigned int idx = 0;
           (target = (IOService *)handlers->getObject(idx)); idx++) {
        target->callPlatformFunction(sCPULatencyFunctionName[delayType], false,
                                     (void *)(uintptr_t)ns, holder, NULL, NULL);
      }
    }
  }
...
static IOReturn setLatencyHandler(UInt32 delayType, IOService *target,
  ...
  do {
    ...
    if (!enable) {
      ...
      __typed_allocators_ignore_push CpuDelayEntry *entries =
          (CpuDelayEntry *)sCpuDelayData->getBytesNoCopy();
      __typed_allocators_ignore_pop UInt32 ns =
          -1U; // Set to max unsigned, i.e. no restriction
      IOService *holder = NULL;

      for (UInt i = 0; i < count; i++) {
        if (entries[i].fService && (delayType == entries[i].fDelayType) &&
...
IOReturn IOService::requireMaxBusStall(UInt32 ns) {
#if !defined(__x86_64__)
  switch (ns) {
...
IOReturn IOService::requireMaxInterruptDelay(uint32_t ns) {
#if defined(__x86_64__)
...
IOReturn
IOService::lookupInterrupt(int source, bool resolve,
                           IOInterruptController **interruptController) {
  IOReturn ret;
...
IOReturn IOService::registerInterrupt(int source, OSObject *target,
  ...
  /* Register the source */
  return interruptController->registerInterrupt(
      this, source, target, (IOInterruptHandler)handler, refCon);
}
...
IOReturn IOService::registerInterruptBlock(int source, OSObject *target,
  ...
  ret = registerInterrupt(source, target, &IOServiceInterruptActionToBlock,
                          block);
  if (kIOReturnSuccess != ret) {
    Block_release(block);
    return ret;
...
void IOService::unregisterAllInterrupts(void) {
...
IOReturn
IOService::addInterruptStatistics(IOInterruptAccountingData *statistics,
                                  int source) {
  ...
  if (source > IA_INDEX_MAX) {
    panic("addInterruptStatistics called for an excessively large index (%d)",
          source);
  }
   ...
   */
  if (!reserved->interruptStatisticsArray) {
    reserved->interruptStatisticsArray =
        IONew(IOInterruptAccountingReporter, 1);
    assert(reserved->interruptStatisticsArray);
    reserved->interruptStatisticsArrayCount = 1;
    bzero(reserved->interruptStatisticsArray,
          sizeof(*reserved->interruptStatisticsArray));
  }
  ...
  if (source >= reserved->interruptStatisticsArrayCount) {
    ...
    memcpy(newArray, reserved->interruptStatisticsArray,
           reserved->interruptStatisticsArrayCount * sizeof(*newArray));
    IODelete(reserved->interruptStatisticsArray, IOInterruptAccountingReporter,
             reserved->interruptStatisticsArrayCount);
  ...
  if (!reserved->interruptStatisticsArray[source].reporter) {
     ...
     */
    reserved->interruptStatisticsArray[source].reporter =
        IOSimpleReporter::with(this, kIOReportCategoryPower, kIOReportUnitNone);
    ...
    for (i = 0; i < IA_NUM_INTERRUPT_ACCOUNTING_STATISTICS; i++) {
      ...
      if (IA_GET_STATISTIC_ENABLED(i)) {
        reserved->interruptStatisticsArray[source].reporter->addChannel(
            IA_GET_CHANNEL_ID(source, i),
            kInterruptAccountingStatisticNameArray[i]);
      }
    }
    ...
    legend->addReporterLegend(
        reserved->interruptStatisticsArray[source].reporter,
        kInterruptAccountingGroupName, subgroupName);
  ...
  if (oldValue) {
    panic("addInterruptStatistics call for index %d would have clobbered "
          "existing statistics",
          source);
  }
   ...
   */
  interruptAccountingDataInheritChannels(
      reserved->interruptStatisticsArray[source].statistics,
      reserved->interruptStatisticsArray[source].reporter);
...
IOReturn IOService::removeInterruptStatistics(int source) {
   ...
   */
  if (source >= reserved->interruptStatisticsArrayCount) {
    panic("removeInterruptStatistics called for index %d, which was never "
          "registered",
          source);
  }
...
IOReturn IOService::configureReport(IOReportChannelList *channelList,
  ...
  /* The array count is signed (because the interrupt indices are signed), hence
   * the cast */
  for (cnt = 0; cnt < (unsigned)reserved->interruptStatisticsArrayCount;
       cnt++) {
    if (reserved->interruptStatisticsArray[cnt].reporter) {
      ...
      if (reserved->interruptStatisticsArray[cnt].statistics) {
        interruptAccountingDataUpdateChannels(
            ...
            reserved->interruptStatisticsArray[cnt].reporter);
      }

      reserved->interruptStatisticsArray[cnt].reporter->configureReport(
          channelList, action, result, destination);
    }
  }
...
IOReturn IOService::updateReport(IOReportChannelList *channelList,
  ...
  for (cnt = 0; cnt < (unsigned)reserved->interruptStatisticsArrayCount;
    ...
    if (reserved->interruptStatisticsArray[cnt].reporter) {
      ...
      reserved->interruptStatisticsArray[cnt].reporter->updateReport(
          channelList, action, result, destination);
    }
  }
...
IOReturn IOService::setAuthorizationID(uint64_t authorizationID) {
  ...
  entitlement = IOUserClient::copyClientEntitlement(
      current_task(), "com.apple.private.iokit.IOServiceSetAuthorizationID");
