...
bool IOPlatformExpert::start(IOService *provider) {
  ...
  // Override the mapper present flag is requested by boot arguments, if SIP
  // disabled.
#if CONFIG_CSR
  if (csr_check(CSR_ALLOW_UNRESTRICTED_FS) == 0)
#endif /* CONFIG_CSR */
  {
    if (PE_parse_boot_argn("dart", &debugFlags, sizeof(debugFlags)) &&
        (debugFlags == 0)) {
  ...
  busFrequency = OSData::withBytesNoCopy(
      (void *)&gPEClockFrequencyInfo.bus_clock_rate_hz, 4);
  provider->setProperty("clock-frequency", busFrequency);
  busFrequency->release();

  gPlatformInterruptControllerName =
      (OSSymbol *)OSSymbol::withCStringNoCopy("IOPlatformInterruptController");
  ...
  PE_parse_boot_argn("enforce_platform_action_safety",
                     &gEnforcePlatformActionSafety,
...
bool IOPlatformExpert::configure(IOService *provider) {
  ...
  if (topLevel) {
    while ((dict = OSDynamicCast(OSDictionary, topLevel->getAnyObject()))) {
      dict->retain();
...
bool IOPlatformExpert::compareNubName(const IOService *nub, OSString *name,
...
long IOPlatformExpert::getBootROMType(void) { return _peBootROMType; }

long IOPlatformExpert::getChipSetType(void) { return _peChipSetType; }

long IOPlatformExpert::getMachineType(void) { return _peMachineType; }
...
bool IOPlatformExpert::getTargetName(char * /*name*/, int /*maxLength*/) {
  return false;
}

bool IOPlatformExpert::getProductName(char * /*name*/, int /*maxLength*/) {
  return false;
}
...
IORangeAllocator *IOPlatformExpert::getPhysicalRangeAllocator(void) {
  return OSDynamicCast(IORangeAllocator, getProperty("Platform Memory Ranges"));
}
...
long IOPlatformExpert::getGMTTimeOfDay(void) { return 0; }
...
IOReturn IOPlatformExpert::registerInterruptController(
    OSSymbol *name, IOInterruptController *interruptController) {
  IOLockLock(gIOInterruptControllersLock);

  gIOInterruptControllers->setObject(name, interruptController);

  IOLockWakeup(gIOInterruptControllersLock, gIOInterruptControllers,
               /* one-thread */ false);
...
IOPlatformExpert::lookUpInterruptController(OSSymbol *name) {
  ...
  while (1) {
    ...
    IOLockSleep(gIOInterruptControllersLock, gIOInterruptControllers,
                THREAD_UNINT);
  }
...
void IOPlatformExpert::setCPUInterruptProperties(IOService *service) {
  ...
  matching = propertyMatching(gPlatformInterruptControllerName, kOSBooleanTrue,
                              matching);
...
void IOPlatformExpert::setUTCTimeOfDay(clock_sec_t secs,
...
void IOPlatformExpert::PMLog(const char *who, unsigned long event,
  ...
  kprintf("pm%u %p %.30s %d %lx %lx\n", nowus, OBFUSCATE(current_thread()),
          who, // Identity
...
// PMRegisterDevice
//
// In this vanilla implementation, all callers are made children of the root
// power domain. Where this is inappropriate, PMRegisterDevice is overridden in
// a platform-specific subclass.
//*********************************************************************************

void IOPlatformExpert::PMRegisterDevice(IOService *theNub,
                                        IOService *theDevice) {
...
// provides an XML power tree description against which a device is matched
// based on class and provider. If a match is found this routine returns true in
// addition to flagging the description tree at the appropriate node that a
// device has been registered for the given service.
//*********************************************************************************

bool IOPlatformExpert::CheckSubTree(OSArray *inSubTree, IOService *theNub,
                                    IOService *theDevice,
                                    OSDictionary *theParent) {
  unsigned int i;
  ...
  for (i = 0; i < numPowerTreeNodes; i++) {
    ...
    deviceMatch = true; // if no matching dictionary, this is not a criteria and
                        // so must match
    if (matchingDictionary) {
      deviceMatch = false;
      if (NULL != (deviceDictionary = theDevice->dictionaryWithProperties())) {
        deviceMatch =
            deviceDictionary->isEqualTo(matchingDictionary, matchingDictionary);
    ...
    if (theNub && providerDictionary) {
      ...
      if (NULL != (nubDictionary = theNub->dictionaryWithProperties())) {
        providerMatch =
            nubDictionary->isEqualTo(providerDictionary, providerDictionary);
    ...
    if (deviceMatch && providerMatch) {
      if (NULL != multipleParentKeyValue) {
        ...
        multiParentMatch = (NULL != aNumber)
                               ? multipleParentKeyValue->isEqualTo(aNumber)
                               : false;
      }
    }
    ...
    if (nodeFound) {
      if (RegisterServiceInTree(theDevice, entry, theParent, theNub)) {
        ...
        multipleParentKeyValue =
            (OSNumber *)entry->getObject("multiple-parent");
      } else {
        nodeFound = false;
      }
    }
    ...
    if (continueSearch &&
        (NULL != (children = (OSArray *)entry->getObject("children")))) {
      nodeFound = CheckSubTree(children, theNub, theDevice, entry);
      continueSearch =
          ((false == nodeFound) || (NULL != multipleParentKeyValue));
    }
...
bool IOPlatformExpert::RegisterServiceInTree(IOService *theService,
  ...
  if (NULL == theTreeNode->getObject("service")) {
    if (theTreeNode->setObject("service",
                               OSDynamicCast(OSObject, theService))) {
      // 1. CHILDREN ------------------
      ...
      if (theTreeParentNode) {
        if (NULL !=
            (aService = (IOService *)theTreeParentNode->getObject("service"))) {
...
__abortlike
#endif
    static void IOShutdownNotificationsTimedOut(thread_call_param_t p0,
  ...
  if (pmRootDomain) {
    if ((PEGetCoprocessorVersion() >= kCoprocessorVersion2) ||
        pmRootDomain->checkShutdownTimeout()) {
...
force_hard_hang_if_transaction_pending(void) {
   ...
   * so that we get an AP watchdog hang *here*  instead of later in the panic
   * flow.
   */
  arm64_sync_tlb(true);
}
...
int PEHaltRestartInternal(unsigned int type, uint32_t details) {
  ...
  if (type == kPEHaltCPU || type == kPERestartCPU ||
      type == kPEUPSDelayHaltCPU) {
    /* If we're in the panic path, the locks and memory allocations required
     * below could fail. So just try to reboot instead of risking a nested
     * panic.
     */
...
#define RESTART_NODE_PATH "/chosen"
    ...
    if (PE_parse_boot_argn("halt_restart_timeout", &boot_arg_val,
                           sizeof(boot_arg_val))) {
    ...
    if (timeout) {
      ...
      thread_call_enter1_delayed(
          shutdown_hang, (thread_call_param_t)(uintptr_t)timeout, deadline);
    }
...
#endif
    {
      ...
      if (!panic_begin_called) {
        ...
        IOCPURunPlatformPanicActions(type, details);
             ...
             type == kPEPanicDiagnosticsInProgress) {
    IOCPURunPlatformPanicActions(type, details);
  }
...
skip_to_haltRestart:
  if (gIOPlatform) {
    ...
    // Implementors of this function will check the type and take action
    // accordingly
...
int PEHaltRestart(unsigned int type) { return PEHaltRestartInternal(type, 0); }
...
static boolean_t PEWriteNVRAMPropertyInternal(const char *symbol,
...
boolean_t PEWriteNVRAMPropertyWithCopy(const char *symbol, const void *value,
                                       const unsigned int len) {
...
void PESetGMTTimeOfDay(long secs) { PESetUTCTimeOfDay(secs, 0); }
...
coprocessor_type_t PEGetCoprocessorVersion(void) {
  ...
  platform_entry =
      IORegistryEntry::fromPath(kIODeviceTreePlane ":/efi/platform");
  if (platform_entry != NULL) {
    coprocessor_version_obj = OSDynamicCast(
        OSData, platform_entry->getProperty("apple-coprocessor-version"));
    if ((coprocessor_version_obj != NULL) &&
        (coprocessor_version_obj->getLength() <= sizeof(uint64_t))) {
      memcpy(&coprocessor_version, coprocessor_version_obj->getBytesNoCopy(),
             coprocessor_version_obj->getLength());
    }
...
void IOPlatformExpert::publishPlatformUUIDAndSerial(void) {
  if (!gIOPlatformUUIDAndSerialDone) {
    ...
    IOPlatformExpertDevice *provider =
        OSDynamicCast(IOPlatformExpertDevice, getProvider());
    assert(provider != NULL);
    provider->generatePlatformUUID();
  }

  if (gIOPlatformUUIDAndSerialDone) {
    publishResource(kIOPlatformUUIDKey,
                    getProvider()->getProperty(kIOPlatformUUIDKey));
  }
}

void IOPlatformExpert::publishNVRAM(void) {
  if (init_gIOOptionsEntry() < 0) {
    IOPlatformExpertDevice *provider =
        OSDynamicCast(IOPlatformExpertDevice, getProvider());
    assert(provider != NULL);
...
void IOPlatformExpert::registerNVRAMController(IONVRAMController *caller) {
  ...
  /*
   * If we have panic debugging enabled WITHOUT behavior to reboot after any
   * crash (DB_REBOOT_ALWAYS) and we are on a co-processor system that has the
   * panic SoC watchdog enabled, disable cross panics so that the co-processor
   * doesn't cause the system to reset when we enter the debugger or hit a panic
   * on the x86 side.
   */
  if (panicDebugging && !(debug_boot_arg & DB_REBOOT_ALWAYS)) {
    ...
    if (entry) {
      data = OSDynamicCast(OSData,
                           entry->getProperty(APPLE_VENDOR_VARIABLE_GUID
...
IOReturn IOPlatformExpert::callPlatformFunction(const OSSymbol *functionName,
  ...
  if (functionName == gIOPlatformQuiesceActionKey ||
     ...
     * themselves, without passing it up to super/IOPlatformExpert.
     */
  ...
  ret = service->callPlatformFunction(functionName, waitForFunction, param1,
...
IOService *IODTPlatformExpert::probe(IOService *provider, SInt32 *score) {
...
bool IODTPlatformExpert::createNubs(IOService *parent, OSIterator *iter) {
  ...
  if (iter) {
    while ((next = (IORegistryEntry *)iter->getNextObject())) {
      ...
      /*
       * If the following condition triggers, it means that a CPU that was
       * present in the DT was ignored by XNU at topology parsing time. This can
       * happen currently when using the cpus=N boot-arg; for example, cpus=1
       * will cause XNU to parse and enable a single CPU.
       *
       * Note that this condition will not trigger for harvested cores because
       * these do not show up in the DT/IORegistry in the first place.
       */
      if (logical_cpu_id < 0) {
        nub->registerService();
        continue;
      ...
      __assert_only bool logical_id_added_to_ioreg =
          nub->setProperty("logical-cpu-id", logical_cpu_id, 32U);
      assert(logical_id_added_to_ioreg == true);
      logical_id_added_to_ioreg =
          nub->setProperty("logical-cluster-id", logical_cluster_id, 32U);
      assert(logical_id_added_to_ioreg == true);
#endif
      nub->registerService();
    }
...
void IODTPlatformExpert::processTopLevel(IORegistryEntry *rootEntry) {
  ...
  publishNVRAM();
  assert(gIOOptionsEntry != NULL); // subclasses that do their own NVRAM
                                   // initialization shouldn't be calling this
  ...
  createNubs(this,
             IODTFindMatchingEntries(rootEntry, kIODTExclusive, excludeList()));
}
...
bool IODTPlatformExpert::compareNubName(const IOService *nub, OSString *name,
                                        OSString **matched) const {
...
bool IODTPlatformExpert::getTargetName(char *name, int maxLength) {
#if __x86_64__
  OSData *prop;
...
bool IODTPlatformExpert::getProductName(char *name, int maxLength) {
#if __x86_64__
  return getModelName(name, maxLength);
#else
...
IOReturn IODTPlatformExpert::readNVRAMProperty(IORegistryEntry *entry,
                                               const OSSymbol **name,
                                               OSData **value) {
  if (dtNVRAM) {
...
IOReturn
IODTPlatformExpert::readNVRAMProperty(IORegistryEntry *entry,
...
IOReturn IODTPlatformExpert::writeNVRAMProperty(IORegistryEntry *entry,
                                                const OSSymbol *name,
                                                OSData *value) {
  if (dtNVRAM) {
...
IOReturn IODTPlatformExpert::readNVRAMPartition(const OSSymbol *partitionID,
                                                ...
                                                IOByteCount length) {
  if (dtNVRAM) {
    return dtNVRAM->readNVRAMPartition(partitionID, offset, buffer, length);
  } else {
    return kIOReturnNotReady;
  }
}
...
IOReturn IODTPlatformExpert::writeNVRAMPartition(const OSSymbol *partitionID,
                                                 ...
                                                 IOByteCount length) {
  if (dtNVRAM) {
    return dtNVRAM->writeNVRAMPartition(partitionID, offset, buffer, length);
  } else {
    return kIOReturnNotReady;
  }
}

IOByteCount IODTPlatformExpert::savePanicInfo(UInt8 *buffer,
                                              IOByteCount length) {
...
IODTPlatformExpert::createSystemSerialNumberString(OSData *myProperty) {
  ...
  if (myProperty != NULL) {
    ...
    if (serialNumberSize > 0) {
      ...
      if (pos <
          serialNumberSize) { // there was a hyphen, so it's a CTO serial number
...
bool IOPlatformExpertDevice::init(void *dtRoot) {
...
bool IOPlatformExpertDevice::startIOServiceMatching(void) {
...
IOWorkLoop *IOPlatformExpertDevice::getWorkLoop() const { return workLoop; }

IOReturn IOPlatformExpertDevice::setProperties(OSObject *properties) {
  return kIOReturnUnsupported;
}

IOReturn IOPlatformExpertDevice::newUserClient(task_t owningTask,
...
void IOPlatformExpertDevice::configureDefaults(void) {
  createNVRAM();
  // Parse the serial-number data and publish a user-readable string
  OSData *mydata = (OSData *)(getProperty("serial-number"));
  if (mydata != NULL) {
    OSString *serNoString =
        OSString::withCString((const char *)mydata->getBytesNoCopy());
    if (serNoString != NULL) {
...
void IOPlatformExpertDevice::createNVRAM(void) {
  /*
...
void IOPlatformExpertDevice::generatePlatformUUID(void) {
  IORegistryEntry *entry;
  ...
  if (entry) {
    ...
    if (data1 && data1->getLength() == 8) {
      ...
      if (data2 && data2->getLength() == 4) {
        ...
        const uuid_t space = {0xA6, 0xDD, 0x4C, 0xCB, 0xB5, 0xE8, 0x4A, 0xF5,
                              0xAC, 0xDD, 0xB6, 0xDC, 0x6A, 0x05, 0x42, 0xB8};
...
bool IOPlatformDevice::compareName(OSString *name, OSString **matched) const {
...
bool IOPanicPlatform::start(IOService *provider) {
  ...
  panic("Unable to find driver for this platform: \"%s\".", platform_name);
