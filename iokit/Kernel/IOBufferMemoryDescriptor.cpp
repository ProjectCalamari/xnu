...
static uintptr_t IOBMDPageProc(kalloc_heap_t kheap, iopa_t *a) {
  kern_return_t kr;
  ...
  kr = kmem_alloc(kernel_map, &vmaddr, page_size, kma_flags,
...
#ifndef __LP64__
bool IOBufferMemoryDescriptor::initWithOptions(IOOptionBits options,
                                               vm_size_t capacity,
...
IOBufferMemoryDescriptor::withCopy(task_t inTask, IOOptionBits options,
                                   vm_map_t sourceMap, mach_vm_address_t source,
                                   mach_vm_size_t size) {
  ...
  do {
    ...
    err =
        vm_map_copyin(sourceMap, source, size, false /* src_destroy */, &copy);
    if (KERN_SUCCESS != err) {
      break;
    }
...
bool IOBufferMemoryDescriptor::initWithPhysicalMask(
    task_t inTask, IOOptionBits options, mach_vm_size_t capacity,
    mach_vm_address_t alignment, mach_vm_address_t physicalMask) {
  task_t mapTask = NULL;
  ...
  if ((options &
       (kIOMemorySharingTypeMask | kIOMapCacheMask | kIOMemoryClearEncrypt)) &&
      (alignment < page_size)) {
    alignment = page_size;
  }
  ...
  if (highestMask && mapped) {
    if (highestMask <= 0xFFFFFFFF) {
      mapSpec.numAddressBits =
          (uint8_t)(32 - __builtin_clz((unsigned int)highestMask));
    } else {
      mapSpec.numAddressBits =
          (uint8_t)(64 - __builtin_clz((unsigned int)(highestMask >> 32)));
    }
    highestMask = 0;
  }

  // set memory entry cache mode, pageable, purgeable
  iomdOptions |= ((options & kIOMapCacheMask) >> kIOMapCacheShift)
  ...
  if (options & kIOMemoryPageable) {
    ...
    if (contig || highestMask || (alignment > page_size)) {
      if (_internalFlags & kInternalFlagGuardPages) {
        printf("IOBMD: Unsupported use of guard pages with physical mask or "
               "contiguous memory.\n");
        return false;
      }
      ...
      _buffer = (void *)IOKernelAllocateWithPhysicalRestrict(
          kheap, capacity, highestMask, alignment, contig, noSoftLimit);
    } else if (_internalFlags & kInternalFlagGuardPages) {
      vm_offset_t address = 0;
      kern_return_t kr;
      uintptr_t alignMask;
      kma_flags_t kma_flags =
          (kma_flags_t)(KMA_GUARD_FIRST | KMA_GUARD_LAST | KMA_ZERO);
      ...
      kr =
          kernel_memory_allocate(kernel_map, &address, capacity + page_size * 2,
                                 alignMask, kma_flags, IOMemoryTag(kernel_map));
      ...
      _buffer = (void *)iopa_alloc(&gIOBMDPageAllocator, &IOBMDPageProc, kheap,
                                   capacity, alignment);
      if (_buffer) {
      ...
      __typed_allocators_ignore_push if (alignment > 1) {
        _buffer = IOMallocAligned_internal(kheap, capacity, alignment, zflags);
      }
      else {
  ...
  if (!super::initWithOptions(
          ...
          // Since we handle all "unmapped" behavior internally and our
          // superclass requires a task, default all unbound IOBMDs to the
          // kernel task.
  ...
  if (kIOReturnSuccess !=
      dmaCommandOperation(kIOMDAddDMAMapSpec, &mapSpec, sizeof(mapSpec))) {
    return false;
  }
  ...
  if (mapTask) {
    ...
    reserved->map =
        createMappingInTask(mapTask, 0,
                            kIOMapAnywhere | (options & kIOMapPrefault) |
                                (options & kIOMapCacheMask),
                            0, 0)
            .detach();
    if (!reserved->map) {
      _buffer = NULL;
...
bool IOBufferMemoryDescriptor::initControlWithPhysicalMask(
    task_t inTask, IOOptionBits options, mach_vm_size_t capacity,
    mach_vm_address_t alignment, mach_vm_address_t physicalMask) {
...
bool IOBufferMemoryDescriptor::initWithGuardPages(task_t inTask,
...
OSSharedPtr<IOBufferMemoryDescriptor>
IOBufferMemoryDescriptor::inTaskWithOptions(task_t inTask, IOOptionBits options,
                                            vm_size_t capacity,
                                            vm_offset_t alignment) {
  OSSharedPtr<IOBufferMemoryDescriptor> me =
      OSMakeShared<IOBufferMemoryDescriptor>();

  if (me &&
      !me->initWithPhysicalMask(inTask, options, capacity, alignment, 0)) {
    me.reset();
  }
  return me;
}
...
IOBufferMemoryDescriptor::inTaskWithPhysicalMask(
  ...
  if (me &&
      !me->initWithPhysicalMask(inTask, options, capacity, 1, physicalMask)) {
    me.reset();
  }
  return me;
}
...
OSSharedPtr<IOBufferMemoryDescriptor>
IOBufferMemoryDescriptor::inTaskWithGuardPages(task_t inTask,
                                               IOOptionBits options,
                                               mach_vm_size_t capacity) {
  OSSharedPtr<IOBufferMemoryDescriptor> me =
      OSMakeShared<IOBufferMemoryDescriptor>();
...
bool IOBufferMemoryDescriptor::initWithOptions(IOOptionBits options,
                                               vm_size_t capacity,
                                               vm_offset_t alignment) {
  return initWithPhysicalMask(kernel_task, options, capacity, alignment,
                              (mach_vm_address_t)0);
}
#endif /* !__LP64__ */

OSSharedPtr<IOBufferMemoryDescriptor>
IOBufferMemoryDescriptor::withOptions(IOOptionBits options, vm_size_t capacity,
                                      vm_offset_t alignment) {
  OSSharedPtr<IOBufferMemoryDescriptor> me =
      OSMakeShared<IOBufferMemoryDescriptor>();

  if (me &&
      !me->initWithPhysicalMask(kernel_task, options, capacity, alignment, 0)) {
    me.reset();
  }
  return me;
}
 ...
 * capacity.
 */
OSSharedPtr<IOBufferMemoryDescriptor> IOBufferMemoryDescriptor::withCapacity(
    vm_size_t inCapacity, IODirection inDirection, bool inContiguous) {
 ...
 */
OSSharedPtr<IOBufferMemoryDescriptor>
IOBufferMemoryDescriptor::withBytes(const void *inBytes, vm_size_t inLength,
                                    IODirection inDirection,
                                    bool inContiguous) {
  OSSharedPtr<IOBufferMemoryDescriptor> me =
      OSMakeShared<IOBufferMemoryDescriptor>();
...
void IOBufferMemoryDescriptor::free() {
...
#if IOTRACKING
  if (!(options & kIOMemoryPageable) && buffer &&
  ...
  if (options & kIOMemoryPageable) {
    ...
    if (kInternalFlagPhysical & internalFlags) {
      ...
      /* BEGIN IGNORE CODESTYLE */
      __typed_allocators_ignore_push IOFreeAligned_internal(kheap, buffer,
                                                            size);
    } else {
 ...
 */
vm_size_t IOBufferMemoryDescriptor::getCapacity() const { return _capacity; }
 ...
 */
bool IOBufferMemoryDescriptor::appendBytes(const void *bytes,
 ...
 */
void *IOBufferMemoryDescriptor::getBytesNoCopy(vm_size_t start,
                                               vm_size_t withLength) {
