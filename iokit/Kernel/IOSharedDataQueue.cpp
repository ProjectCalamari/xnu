...
OSDefineMetaClassAndStructors(IOSharedDataQueue, IODataQueue)
    ...
    OSSharedPtr<IOSharedDataQueue> IOSharedDataQueue::withCapacity(
        UInt32 size) {
...
Boolean IOSharedDataQueue::initWithCapacity(UInt32 size) {
  ...
  kr = kmem_alloc(kernel_map, (vm_offset_t *)&dataQueue, allocSize,
                  (kma_flags_t)(KMA_DATA_SHARED | KMA_ZERO),
...
OSSharedPtr<IOMemoryDescriptor> IOSharedDataQueue::getMemoryDescriptor() {
...
IODataQueueEntry *IOSharedDataQueue::peek() {
  ...
  tailOffset =
      __c11_atomic_load((_Atomic UInt32 *)&dataQueue->tail, __ATOMIC_ACQUIRE);
...
Boolean IOSharedDataQueue::dequeue(void *data, UInt32 *dataSize) {
  ...
  __c11_atomic_store((_Atomic UInt32 *)&dataQueue->head, newHeadOffset,
                     __ATOMIC_RELEASE);
