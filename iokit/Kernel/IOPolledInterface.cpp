...
OSDefineMetaClassAndStructors(IOPolledFilePollers, OSObject)
    ...
    IOPolledFilePollers *IOPolledFilePollers::copyPollers(IOService *media) {
  ...
  do {
    ...
    do {
      ...
      if ((service = OSDynamicCast(IOService, next)) &&
          service->getDeviceMemory() && !vars->pollers->getCount()) {
        break;
      }
...
static IOReturn IOPolledFilePollersIODone(IOPolledFilePollers *vars,
                                          bool abortable);

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

static IOReturn IOPolledFilePollersProbe(IOPolledFilePollers *vars) {
...
IOReturn IOPolledFilePollersOpen(IOPolledFileIOVars *filevars, uint32_t state,
                                 bool abortable) {
  ...
  if ((kIOPolledPreflightState == state) ||
      (kIOPolledPreflightCoreDumpState == state)) {
    ioBuffer = vars->ioBuffer;
    if (!ioBuffer) {
      vars->ioBuffer = ioBuffer = IOBufferMemoryDescriptor::withOptions(
          kIODirectionInOut, kDefaultIONumBuffers * kDefaultIOSize, page_size);
      if (!ioBuffer) {
...
IOReturn IOPolledFilePollersClose(IOPolledFileIOVars *filevars,
                                  uint32_t state) {
  IOPolledFilePollers *vars = filevars->pollers;
  IOPolledInterface *poller;
  IORegistryEntry *next;
  ...
  if ((kIOPolledPostflightState == state) ||
      (kIOPolledPostflightCoreDumpState == state)) {
    vars->openCount--;
  }

  for (idx = 0, err = kIOReturnSuccess;
       (poller = (IOPolledInterface *)vars->pollers->getObject(idx)); idx++) {
    err = poller->close(state);
    if ((kIOReturnSuccess != err) &&
        (kIOPolledBeforeSleepStateAborted == state)) {
...
IOReturn IOPolledInterface::setEncryptionKey(const uint8_t *key,
                                             size_t keySize) {
...
IOReturn IOPolledFilePollersSetEncryptionKey(IOPolledFileIOVars *filevars,
                                             const uint8_t *key,
                                             size_t keySize) {
  ...
  for (idx = 0; (poller = (IOPolledInterface *)vars->pollers->getObject(idx));
       idx++) {
    poller = (IOPolledInterface *)vars->pollers->getObject(idx);
...
static void IOPolledIOComplete(void *target, void *parameter, IOReturn status,
...
static IOReturn IOStartPolledIO(IOPolledFilePollers *vars, uint32_t operation,
                                uint32_t bufferOffset, uint64_t deviceOffset,
                                uint64_t length) {
  IOReturn err;
  IOPolledInterface *poller;
  ...
  err = poller->startIO(operation, bufferOffset, deviceOffset, length,
                        completion);
  if (err) {
    if (kernel_debugger_entry_count) {
      ...
      HIBLOGFROMPANIC(
          "IOPolledInterface::IOStartPolledIO(0x%p, %d, 0x%x, 0x%llx, %llu) : "
          "poller->startIO(%d, 0x%x, 0x%llx, %llu, completion) returned 0x%x",
          vars, operation, bufferOffset, deviceOffset, length, operation,
          bufferOffset, deviceOffset, length, err);
    }
  }
  return err;
}
...
static void file_extent_callback(void *ref, uint64_t start, uint64_t length) {
...
static IOService *IOCopyMediaForDev(dev_t device) {
...
IOGetVolumeCryptKey(dev_t block_dev,
  ...
  {
    uuid_t volUuid = {0};
    err = part->callPlatformFunction(APFSMEDIA_GETHIBERKEY, false, &volUuid,
                                     volumeCryptKey, keySize, keySize);
  ...
  // Then old CS path
  err = part->callPlatformFunction(
      PLATFORM_FUNCTION_GET_MEDIA_ENCRYPTION_KEY_UUID, false, (void *)&keyUUID,
      (void *)&keyStoreUUID, NULL, NULL);
  if ((kIOReturnSuccess == err) && keyUUID && keyStoreUUID) {
    //        IOLog("got volume key %s\n", keyStoreUUID->getCStringNoCopy());

    if (!sKeyStore) {
      sKeyStore = (IOService *)IORegistryEntry::fromPath(AKS_SERVICE_PATH,
                                                         gIOServicePlane);
    }
    if (sKeyStore) {
    ...
    if (kIOReturnSuccess == err) {
      err = sKeyStore->callPlatformFunction(gAKSGetKey, true, volumeKeyUUID,
                                            &vek, NULL, NULL);
    }
    if (kIOReturnSuccess != err) {
...
#if defined(__arm64__)
static IOReturn IOGetHibernationCryptKey(uint8_t *hibernationKey,
                                         size_t *keySize, uint32_t *swSeed) {
  return kIOReturnNotFound;
}
#endif /* defined(__arm64__) */
...
IOReturn IOPolledFileOpen(const char *filename, uint32_t flags,
                          uint64_t setFileSizeMin, uint64_t setFileSizeMax,
                          uint64_t fsFreeSize, void *write_file_addr,
                          size_t write_file_len, IOPolledFileIOVars **fileVars,
  ...
  do {
    ...
    vars->fileRef = kern_open_file_for_direct_io(
        filename, flags, &file_extent_callback, &ctx, setFileSizeMin,
        setFileSizeMax, fsFreeSize,
        // write file:
        0, write_file_addr, write_file_len,
        // results
        &block_dev, &image_dev, &vars->block0, &vars->maxiobytes, &vars->flags);
#if 0
    ...
    if ((num = OSDynamicCast(
             OSNumber, part->getProperty(kIOMediaPreferredBlockSizeKey)))) {
    ...
    *fileVars = vars;
    ...
    if (imagePath) {
      ...
      if (keyUUID) {
        snprintf(str2, sizeof(str2), "%qx:%s", vars->extentMap[0].start,
                 keyUUID->getCStringNoCopy());
      } else {
        snprintf(str2, sizeof(str2), "%qx", vars->extentMap[0].start);
      }

      err = IOService::getPlatform()->callPlatformFunction(
          gIOCreateEFIDevicePathSymbol, false, (void *)part, (void *)str2,
...
IOReturn IOPolledFileClose(IOPolledFileIOVars **pVars, off_t write_offset,
                           void *addr, size_t write_length,
  ...
  if (vars->fileRef) {
    kern_close_file_for_direct_io(vars->fileRef, write_offset, addr,
                                  write_length, discard_offset, discard_end,
                                  vars->fileSizeMin, unlink);
    vars->fileRef = NULL;
  }
  if (vars->fileExtents) {
...
IOReturn IOPolledFilePollersSetup(IOPolledFileIOVars *vars,
                                  uint32_t openState) {
  IOReturn err;
  ...
  do {
    ...
    if ((kIOPolledPreflightState == openState) ||
        (kIOPolledPreflightCoreDumpState == openState)) {
    ...
    vars->bufferSize =
        (typeof(vars->bufferSize))(vars->pollers->ioBuffer->getLength() >> 1);
...
IOReturn IOPolledFileSeek(IOPolledFileIOVars *vars, uint64_t position) {
  ...
  if (position > vars->fileSize) {
    HIBLOG("IOPolledFileSeek: called to seek to 0x%llx greater than file size "
           "of 0x%llx\n",
...
IOReturn IOPolledFileWrite(IOPolledFileIOVars *vars, const uint8_t *bytes,
                           IOByteCount size, IOPolledFileCryptVars *cryptvars) {
  IOReturn err = kIOReturnSuccess;
  ...
  do {
    ...
    if (bytes) {
#if KASAN
      /* Since this may copy mach-o segments in bulk, use the nosan variants of
       * bcopy to avoid triggering global redzone sanitizer violations when
       * accessing interstices between 'C' structures
       */
      __nosan_bcopy(bytes, vars->buffer + vars->bufferHalf + vars->bufferOffset,
                    copy);
#else
    ...
    if (flush && vars->bufferOffset) {
      ...
      if (cryptvars && vars->encryptStart &&
        ...
        bcopy(vars->buffer + vars->bufferHalf + encryptStart + encryptLen -
                  AES_BLOCK_SIZE,
      ...
      // if (length != vars->bufferSize) HIBLOG("short write of %qx ends@
      // %qx\n", length, offset + length);

      err = IOStartPolledIO(vars->pollers, kIOPolledWrite, vars->bufferHalf,
                            offset, length);
      if (kIOReturnSuccess != err) {
        HIBLOGFROMPANIC(
            "IOPolledFileWrite(0x%p, 0x%p, %llu, 0x%p) : IOStartPolledIO(0x%p, "
            "kIOPolledWrite, %llu, 0x%llx, %d) returned 0x%x\n",
            vars, bytes, (uint64_t)original_size, cryptvars, vars->pollers,
            (uint64_t)vars->bufferHalf, offset, length, err);
        break;
      }
...
IOReturn IOPolledFileFlush(IOPolledFileIOVars *vars) {
  // Only supported by the underlying polled mode driver on embedded currently
  // (expect kIOReturnUnsupported on other platforms)
  IOReturn err = kIOReturnSuccess;
  ...
  if (kIOReturnSuccess != err) {
    HIBLOGFROMPANIC("IOPolledFileFlush(0x%p) : IOStartPolledIO(0x%p, "
                    "kIOPolledFlush, 0, 0, 0) returned 0x%x\n",
...
IOReturn IOPolledFileRead(IOPolledFileIOVars *vars, uint8_t *bytes,
                          IOByteCount size, IOPolledFileCryptVars *cryptvars) {
  IOReturn err = kIOReturnSuccess;
  IOByteCount copy;
  ...
  do {
    ...
    if (bytes) {
#if KASAN
      __nosan_bcopy(vars->buffer + vars->bufferHalf + vars->bufferOffset, bytes,
                    copy);
#else
    ...
    if ((vars->bufferOffset == vars->bufferLimit) &&
      ...
      uint64_t offset =
          (vars->position - vars->extentPosition + vars->currentExtent->start);
      ...
      if (length) {
        // if (length != vars->bufferSize) HIBLOG("short read of %qx ends@
        // %qx\n", length, offset + length);
        err = IOStartPolledIO(vars->pollers, kIOPolledRead, vars->bufferHalf,
                              offset, length);
        if (kIOReturnSuccess != err) {
          break;
        }
      ...
      if (cryptvars) {
        ...
        aes_decrypt_cbc(vars->buffer + vars->bufferHalf, &thisVector[0],
