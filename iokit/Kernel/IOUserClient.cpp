...
#define FAKE_STACK_FRAME_END() __frameptr[1] = __retaddr;
...
#define ASYNC_REF64_COUNT                                                      \
  (sizeof(io_async_ref64_t) / sizeof(io_user_reference_t))
...
public:
  ...
  static LIBKERN_RETURNS_NOT_RETAINED IOMachPort *
  portForObjectInBucket(IOMachPortHashList *bucket, OSObject *obj,
                        ipc_kobject_type_t type);

  static IOMachPort *noMoreSenders(ipc_port_t port, ipc_kobject_type_t type,
                                   mach_port_mscount_t mscount);
  static void releasePortForObject(OSObject *obj, ipc_kobject_type_t type);

  static mach_port_name_t makeSendRightForTask(task_t task, io_object_t obj,
                                               ipc_kobject_type_t type);

  virtual void free() APPLE_KEXT_OVERRIDE;

  void makePort(ipc_kobject_type_t type) {
    port = iokit_alloc_object_port(this, type);
  }

  void adoptPort(IOMachPort *other, ipc_kobject_type_t type) {
    port = other->port;
    ipc_kobject_enable(port, this, IKOT_IOKIT_CONNECT);
    other->port = NULL;
  }

  void disablePort(ipc_kobject_type_t type) {
...
SECURITY_READ_ONLY_LATE(const struct io_filter_callbacks *)
...
void IOMachPortInitialize(void) {
  for (size_t i = 0; i < PORT_HASH_SIZE; i++) {
    SLIST_INIT(&gIOMachPortHash[i]);
  }
}

IOMachPortHashList *IOMachPort::bucketForObject(OSObject *obj) {
  return &gIOMachPortHash[os_hash_kernel_pointer(obj) % PORT_HASH_SIZE];
}

IOMachPort *IOMachPort::portForObjectInBucket(IOMachPortHashList *bucket,
                                              OSObject *obj,
                                              ipc_kobject_type_t type) {
  IOMachPort *machPort;
...
IOMachPort *IOMachPort::withObject(OSObject *obj) {
...
IOMachPort *IOMachPort::noMoreSenders(ipc_port_t port, ipc_kobject_type_t type,
                                      mach_port_mscount_t mscount) {
...
void IOMachPort::releasePortForObject(OSObject *obj, ipc_kobject_type_t type) {
  ...
  if (machPort && ((type != IKOT_IOKIT_OBJECT) ||
...
mach_port_name_t IOMachPort::makeSendRightForTask(task_t task, io_object_t obj,
                                                  ipc_kobject_type_t type) {
...
static bool IOTaskRegistryCompatibility(task_t task) { return false; }

static void IOTaskRegistryCompatibilityMatching(task_t task,
...
OSDefineMetaClassAndStructors(IOUserIterator, OSIterator)
    ...
    IOUserIterator *IOUserIterator::withIterator(OSIterator *iter) {
  IOUserIterator *me;
...
OSObject *IOUserIterator::getNextObject() {
  assert(false);
  return NULL;
}

OSObject *IOUserIterator::copyNextObject() {
  OSObject *ret = NULL;
...
void iokit_port_object_description(io_object_t obj,
  ...
  if ((regEntry = OSDynamicCast(IORegistryEntry, obj))) {
    snprintf(desc, KOBJECT_DESCRIPTION_LENGTH, "%s(0x%qx)",
             obj->getMetaClass()->getClassName(),
             regEntry->getRegistryEntryID());
#if DEVELOPMENT || DEBUG
  } else if ((noti = OSDynamicCast(IOUserNotification, obj))) {
    // serviceNoti->matching may become NULL if the port gets a no-senders
    // notification, so we have to lock gIOObjectPortLock
    ...
    if (matching) {
      ...
      if (s && matching->serialize(s)) {
        snprintf(desc, KOBJECT_DESCRIPTION_LENGTH, "%s(%s)",
                 obj->getMetaClass()->getClassName(), s->text());
      }
      OSSafeReleaseNULL(s);
    ...
    snprintf(desc, KOBJECT_DESCRIPTION_LENGTH, "%s",
             obj->getMetaClass()->getClassName());
  }
}
...
void iokit_remove_reference(io_object_t obj) {
  if (obj) {
    obj->release();
  }
}
...
void iokit_remove_connect_reference(LIBKERN_CONSUMED io_object_t obj) {
  if (!obj) {
    return;
  }
  obj->release();
...
enum { kIPCLockNone = 0, kIPCLockRead = 1, kIPCLockWrite = 2 };

void IOUserClient::ipcEnter(int locking) {
  switch (locking) {
  case kIPCLockWrite:
    IORWLockWrite(&lock);
    break;
...
void IOUserClient::ipcExit(int locking) {
  bool finalize = false;
...
void iokit_kobject_retain(io_kobject_t machPort) {
...
bool IOUserClient::finalizeUserReferences(OSObject *obj) {
  IOUserClient *uc;
  bool ok = true;
...
ipc_port_t iokit_port_make_send_for_object(io_object_t obj,
                                           ipc_kobject_type_t type) {
  IOMachPort *machPort = NULL;
...
void iokit_ident_no_senders(ipc_port_t port, mach_port_mscount_t mscount) {
  IOMachPort *machPort;
...
void iokit_object_no_senders(ipc_port_t port, mach_port_mscount_t mscount) {
  IOMachPort *machPort;
...
void iokit_connect_no_senders(ipc_port_t port, mach_port_mscount_t mscount) {
  IOMachPort *machPort;
...
void iokit_uext_no_senders(ipc_port_t port, mach_port_mscount_t mscount) {
  IOMachPort *machPort;
...
public:
  virtual bool init(mach_port_t port, natural_t type, void *reference,
                    vm_size_t referenceSize, bool clientIs64);
  virtual void free() APPLE_KEXT_OVERRIDE;
  void invalidatePort(void);

  static bool _handler(void *target, void *ref, IOService *newService,
                       IONotifier *notifier);
...
public:
  ...
  static IOReturn _handler(void *target, void *ref, UInt32 messageType,
                           IOService *provider, void *messageArgument,
                           vm_size_t argSize);
  virtual IOReturn handler(void *ref, UInt32 messageType, IOService *provider,
...
void IOUserNotification::setNotification(IONotifier *notify) {
   ...
   * Two threads may race in setNotification(). If one thread sets a new
   * notifier while the other thread sets the notifier to NULL, it is possible
   * for the second thread to call release() before the first thread calls
   * retain(). Without the retain here, this thread interleaving would cause the
   * object to get released and freed before it is retained by the first thread,
   * which is a UaF.
   */
  retain();
  ...
  if (previousNotify) {
    ...
    // new IONotifier, retain the object. release() will happen in
...
bool IOUserNotification::isValid() { return true; }
...
bool IOServiceUserNotification::_handler(void *target, void *ref,
...
bool IOServiceUserNotification::handler(void *ref, IOService *newService) {
  unsigned int count;
  kern_return_t kr;
  ipc_port_t port = NULL;
  bool sendPing = false;
  ...
  if (sendPing) {
    ...
    kr = kernel_mach_msg_send_with_builder_internal(
        0, payloadSize, MACH_SEND_KERNEL_IMPORTANCE, MACH_MSG_TIMEOUT_NONE,
        NULL,
        ^(mach_msg_header_t *hdr, __assert_only mach_msg_descriptor_t *descs,
          void *payload) {
          PingMsgUdata *udata = (PingMsgUdata *)payload;

          hdr->msgh_remote_port = remotePort;
          hdr->msgh_local_port = port;
          hdr->msgh_bits = MACH_MSGH_BITS(MACH_MSG_TYPE_COPY_SEND /*remote*/,
          ...
          assert((char *)udata->notifyHeader.reference + msgReferenceSize <=
...
OSDefineMetaClassAndStructors(IOServiceMessageUserNotification,
                              IOUserNotification)

    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

    bool IOServiceMessageUserNotification::init(mach_port_t port,
                                                natural_t type, void *reference,
                                                vm_size_t referenceSize,
                                                bool client64) {
  if (!super::init()) {
    return false;
...
IOReturn IOServiceMessageUserNotification::_handler(void *target, void *ref,
                                                    ...
                                                    void *argument,
                                                    vm_size_t argSize) {
  IOServiceMessageUserNotification *targetObj =
      (IOServiceMessageUserNotification *)target;
  IOReturn ret;

  targetObj->retain();
  ret = targetObj->handler(ref, messageType, provider, argument, argSize);
  targetObj->release();
  return ret;
}
...
IOReturn IOServiceMessageUserNotification::handler(void *ref,
  ...
  type |= ((argSize & kIOKitNoticationMsgSizeMask)
           << kIOKitNoticationTypeSizeAdjShift);
  argSize =
      (argSize + kIOKitNoticationMsgSizeMask) & ~kIOKitNoticationMsgSizeMask;

  mach_msg_size_t extraSize =
      kIOUserNotifyMaxMessageSize + sizeof(IOServiceInterestContent64);
  mach_msg_size_t msgSize =
      (mach_msg_size_t)(sizeof(PingMsgKdata) + sizeof(PingMsgUdata) -
                        sizeof(OSAsyncReference64) + msgReferenceSize);

  if (os_add3_overflow(msgSize,
                       offsetof(IOServiceInterestContent64, messageArgument),
                       argSize, &thisMsgSize)) {
  ...
  kr = kernel_mach_msg_send_with_builder_internal(
      1, payloadSize, MACH_SEND_KERNEL_IMPORTANCE, MACH_MSG_TIMEOUT_NONE, NULL,
      ^(mach_msg_header_t *hdr, mach_msg_descriptor_t *descs, void *payload) {
        ...
        hdr->msgh_bits = MACH_MSGH_BITS_SET(MACH_MSG_TYPE_COPY_SEND /*remote*/,
                                            MACH_MSG_TYPE_MOVE_SEND /*local*/,
        ...
        dataOffset = sizeof(PingMsgUdata) - sizeof(OSAsyncReference64) +
                     msgReferenceSize;
        data = (IOServiceInterestContent64 *)(((uint8_t *)udata) + dataOffset);
        data->messageType = messageType;

        if (callerArgSize == 0) {
          assert((char *)data->messageArgument + argSize <=
                 (char *)payload + payloadSize);
          ...
          assert((char *)data->messageArgument + callerArgSize <=
                 (char *)payload + payloadSize);
...
OSObject *IOServiceMessageUserNotification::getNextObject() { return NULL; }

OSObject *IOServiceMessageUserNotification::copyNextObject() { return NULL; }
...
static TUNABLE(bool, gEnforcePowerEntitlement, "enforce-power-entitlement",
               false);

static_assert(offsetof(IOUserClient, __opaque_end) -
                      offsetof(IOUserClient, __opaque_start) ==
                  sizeof(void *) * 9,
...
void IOUserClient::initialize(void) {
  ...
  IOTrackingQueueCollectUser(
      IOServiceMessageUserNotification::gMetaClass.getTracking());
  IOTrackingQueueCollectUser(
      IOServiceUserNotification::gMetaClass.getTracking());
...
void
    ...
    IOUserClient::setAsyncReference(OSAsyncReference asyncRef,
                                    mach_port_t wakePort, void *callback,
                                    void *refcon) {
#if __LP64__
...
static OSDictionary *CopyConsoleUser(UInt32 uid) {
  ...
  if ((array = OSDynamicCast(OSArray, ioProperty))) {
    for (unsigned int idx = 0;
         (user = OSDynamicCast(OSDictionary, array->getObject(idx))); idx++) {
      OSNumber *num;

      if ((num = OSDynamicCast(OSNumber,
                               user->getObject(gIOConsoleSessionUIDKey))) &&
...
IOReturn IOUserClient::clientHasAuthorization(task_t task, IOService *service) {
  proc_t p;
...
IOReturn IOUserClient::clientHasPrivilege(void *securityToken,
  ...
  if (!strncmp(privilegeName, kIOClientPrivilegeConsoleSession,
    ...
    if (p && (cred = kauth_cred_proc_ref(p))) {
      ...
      if (user) {
        OSNumber *num;
        if ((num = OSDynamicCast(
                 OSNumber, user->getObject(gIOConsoleSessionAuditIDKey))) &&
            (cred->cr_audit.as_aia_p->ai_asid ==
             (au_asid_t)num->unsigned32BitValue())) {
          kr = kIOReturnSuccess;
        }
        user->release();
      }
  ...
  if ((secureConsole =
           !strncmp(privilegeName, kIOClientPrivilegeSecureConsoleProcess,
  ...
  if (KERN_SUCCESS != kr) {
    ...
    if (user) {
      ...
      kr = kIOReturnNotPrivileged;
             ...
             !strncmp(privilegeName, kIOClientPrivilegeConsoleUser,
    ...
    if (user) {
      if (user->getObject(gIOConsoleSessionOnConsoleKey) != kOSBooleanTrue) {
        ...
        OSNumber *pid = OSDynamicCast(
            OSNumber, user->getObject(gIOConsoleSessionSecureInputPIDKey));
        if (pid && pid->unsigned32BitValue() !=
                       ((IOUCProcessToken *)securityToken)->pid) {
          kr = kIOReturnNotPrivileged;
        }
      }
      user->release();
...
OSDictionary *IOUserClient::copyClientEntitlementsVnode(vnode_t vnode,
                                                        off_t offset) {
  OSDictionary *entitlements = NULL;

  if (cs_entitlements_dictionary_copy_vnode(vnode, offset,
                                            (void **)&entitlements) != 0) {
    return NULL;
  }
  return entitlements;
...
OSObject *IOUserClient::copyClientEntitlement(task_t task,
  ...
  kern_return_t ret = amfi->OSEntitlements.copyEntitlementAsOSObjectWithProc(
      proc, entitlement, &entitlement_object);
...
OSObject *IOUserClient::copyClientEntitlementVnode(struct vnode *vnode,
                                                   off_t offset,
                                                   const char *entitlement) {
...
bool IOUserClient::initWithTask(task_t owningTask, void *securityID,
                                UInt32 type) {
  if (getPropertyTable() || super::init()) {
    return reserve();
  }
...
IOReturn IOUserClient::registerOwner(task_t task) {
  ...
  if (newOwner) {
    ...
    queue_enter_first(task_io_user_clients(task), owner, IOUserClientOwner *,
                      taskLink);
...
void IOUserClient::noMoreSenders(void) {
  IOUserClientOwner *owner;
...
extern "C" void iokit_task_app_suspended_changed(task_t task) {
  queue_head_t *taskque;
  IOUserClientOwner *owner;
  OSSet *set;
...
static kern_return_t iokit_task_terminate_phase1(task_t task) {
  queue_head_t *taskque;
  IOUserClientOwner *iter;
  ...
  if (userServers) {
    ...
    while ((userServer =
                OSRequiredCast(IOUserServer, userServers->getAnyObject()))) {
...
static kern_return_t iokit_task_terminate_phase2(task_t task) {
  queue_head_t *taskque;
  IOUserClientOwner *owner;
  IOUserClient *dead;
  IOUserClient *uc;
...
extern "C" kern_return_t iokit_task_terminate(task_t task, int phase) {
  switch (phase) {
  case 1:
...
IOUserClient::filterForTask(task_t task, io_filter_policy_t addFilterPolicy) {
  ...
  for (elem = reserved->filterPolicies; elem && (elem->task != task);
       elem = elem->next) {
  }
...
IOReturn IOUserClient::clientClose(void) { return kIOReturnUnsupported; }

IOService *IOUserClient::getService(void) { return NULL; }

IOReturn IOUserClient::registerNotificationPort(mach_port_t /* port */,
                                                UInt32 /* type */,
                                                UInt32 /* refCon */) {
  return kIOReturnUnsupported;
}

IOReturn IOUserClient::registerNotificationPort(mach_port_t port, UInt32 type,
                                                io_user_reference_t refCon) {
...
IOReturn IOUserClient::clientMemoryForType(UInt32 type, IOOptionBits *options,
                                           IOMemoryDescriptor **memory) {
  return kIOReturnUnsupported;
}

IOReturn
IOUserClient::clientMemoryForType(UInt32 type, IOOptionBits *options,
...
#if !__LP64__
IOMemoryMap *IOUserClient::mapClientMemory(IOOptionBits type, task_t task,
...
IOMemoryMap *IOUserClient::mapClientMemory64(IOOptionBits type, task_t task,
                                             IOOptionBits mapFlags,
  ...
  if ((kIOReturnSuccess == err) && memory && !memory->hasSharingContext()) {
    ...
    options =
        (options & ~kIOMapUserOptionsMask) | (mapFlags & kIOMapUserOptionsMask);
...
IOReturn IOUserClient::exportObjectToClient(task_t task, OSObject *obj,
...
IOReturn
IOUserClient::copyPortNameForObjectInTask(task_t task, OSObject *obj,
...
IOReturn IOUserClient::copyObjectForPortNameInTask(task_t task,
                                                   mach_port_name_t port_name,
                                                   OSObject **obj) {
  OSObject *object;

  object =
      iokit_lookup_object_with_port_name(port_name, IKOT_IOKIT_IDENT, task);
...
IOReturn IOUserClient::adjustPortNameReferencesInTask(
    task_t task, mach_port_name_t port_name, mach_port_delta_t delta) {
...
// Suppressing the deprecated-declarations warning. Avoiding the use of
// deprecated functions can break clients of kexts implementing
// getExternalMethodForIndex()
IOExternalMethod *IOUserClient::getTargetAndMethodForIndex(IOService **targetP,
                                                           UInt32 index) {
...
IOExternalMethod *
IOUserClient::getTargetAndMethodForIndex(OSSharedPtr<IOService> &targetP,
                                         UInt32 index) {
...
IOExternalAsyncMethod *
IOUserClient::getAsyncTargetAndMethodForIndex(IOService **targetP,
                                              UInt32 index) {
...
IOExternalAsyncMethod *
IOUserClient::getAsyncTargetAndMethodForIndex(OSSharedPtr<IOService> &targetP,
                                              UInt32 index) {
  IOService *targetPRaw = NULL;
  IOExternalAsyncMethod *result =
      getAsyncTargetAndMethodForIndex(&targetPRaw, index);
  targetP.reset(targetPRaw, OSRetain);
  return result;
}

IOExternalTrap *IOUserClient::getTargetAndTrapForIndex(IOService **targetP,
                                                       UInt32 index) {
...
IOReturn IOUserClient::sendAsyncResult64WithOptions(
    OSAsyncReference64 reference, IOReturn result, io_user_reference_t args[],
    UInt32 numArgs, IOOptionBits options) {
...
IOReturn IOUserClient::_sendAsyncResult64(OSAsyncReference64 reference,
  ...
  replyMsg.msgHdr.msgh_bits =
      MACH_MSGH_BITS(MACH_MSG_TYPE_COPY_SEND /*remote*/, 0 /*local*/);
  ...
  if (kIOUCAsync64Flag & reference[0]) {
    ...
    /* Copy reference except for reference[0], which is left as 0 from the
     * earlier bzero */
    bcopy(&reference[1], &replyMsg.m.msg64.notifyHdr.reference[1],
          sizeof(OSAsyncReference64) - sizeof(reference[0]));
    ...
    replyMsg.msgHdr.msgh_size = sizeof(replyMsg.msgHdr) +
                                sizeof(replyMsg.m.msg32) -
                                (kMaxAsyncArgs - numArgs) * sizeof(uint32_t);

    replyMsg.m.msg32.notifyHdr.size =
        sizeof(IOAsyncCompletionContent) + numArgs * sizeof(uint32_t);
  ...
  if ((options & kIOUserNotifyOptionCanDrop) != 0) {
    kr = mach_msg_send_from_kernel_with_options(
        &replyMsg.msgHdr, replyMsg.msgHdr.msgh_size, MACH64_SEND_TIMEOUT,
        MACH_MSG_TIMEOUT_NONE);
  } else {
    /* Fail on full queue. */
    kr = mach_msg_send_from_kernel(&replyMsg.msgHdr, replyMsg.msgHdr.msgh_size);
  }
  if ((KERN_SUCCESS != kr) && (MACH_SEND_TIMED_OUT != kr) &&
      !(kIOUCAsyncErrorLoggedFlag & reference[0])) {
...
kern_return_t is_io_server_version(mach_port_t main_port, uint64_t *version) {
  *version = IOKIT_SERVER_VERSION;
  return kIOReturnSuccess;
}

/* Routine io_object_get_class */
kern_return_t is_io_object_get_class(io_object_t object, io_name_t className) {
...
kern_return_t is_io_object_get_superclass(mach_port_t main_port,
                                          io_name_t obj_name,
...
kern_return_t is_io_object_get_bundle_identifier(mach_port_t main_port,
                                                 io_name_t obj_name,
...
kern_return_t is_io_object_conforms_to(io_object_t object, io_name_t className,
                                       boolean_t *conforms) {
  if (!object) {
    return kIOReturnBadArgument;
  }
...
kern_return_t is_io_object_get_retain_count(io_object_t object,
...
kern_return_t is_io_iterator_next(io_object_t iterator, io_object_t *object) {
  IOReturn ret;
  OSObject *obj;
  OSIterator *iter;
...
kern_return_t is_io_iterator_reset(io_object_t iterator) {
...
kern_return_t is_io_iterator_is_valid(io_object_t iterator,
                                      boolean_t *is_valid) {
  CHECK(OSIterator, iterator, iter);
...
static kern_return_t internal_io_service_match_property_table(
    io_service_t _service, const char *matching,
    mach_msg_type_number_t matching_size, boolean_t *matches) {
...
kern_return_t is_io_service_match_property_table(io_service_t service,
...
kern_return_t is_io_service_match_property_table_ool(
    io_object_t service, io_buf_ptr_t matching,
    mach_msg_type_number_t matchingCnt, kern_return_t *result,
    boolean_t *matches) {
  kern_return_t kr;
  vm_offset_t data;
  ...
  if (KERN_SUCCESS == kr) {
    ...
    *result = internal_io_service_match_property_table(
        service, (const char *)data, matchingCnt, matches);
...
kern_return_t is_io_service_match_property_table_bin(
    io_object_t service, io_struct_inband_t matching,
    mach_msg_type_number_t matchingCnt, boolean_t *matches) {
...
static kern_return_t internal_io_service_get_matching_services(
    mach_port_t main_port, const char *matching,
    mach_msg_type_number_t matching_size, io_iterator_t *existing) {
  kern_return_t kr;
  OSObject *obj;
  OSDictionary *dict;
...
kern_return_t is_io_service_get_matching_services(mach_port_t main_port,
                                                  io_string_t matching,
...
kern_return_t is_io_service_get_matching_services_ool(
    mach_port_t main_port, io_buf_ptr_t matching,
    mach_msg_type_number_t matchingCnt, kern_return_t *result,
  ...
  if (KERN_SUCCESS == kr) {
    ...
    *existing = NULL;
    *result = internal_io_service_get_matching_services(
        main_port, (const char *)data, matchingCnt, existing);
    vm_deallocate(kernel_map, data, matchingCnt);
  }
...
kern_return_t is_io_service_get_matching_services_bin(
    mach_port_t main_port, io_struct_inband_t matching,
    mach_msg_type_number_t matchingCnt, io_object_t *existing) {
  return internal_io_service_get_matching_services(main_port, matching,
                                                   matchingCnt, existing);
}

static kern_return_t internal_io_service_get_matching_service(
    mach_port_t main_port, const char *matching,
    mach_msg_type_number_t matching_size, io_service_t *service) {
  kern_return_t kr;
  OSObject *obj;
  OSDictionary *dict;
...
kern_return_t is_io_service_get_matching_service(mach_port_t main_port,
                                                 io_string_t matching,
...
kern_return_t is_io_service_get_matching_service_ool(
  ...
  if (KERN_SUCCESS == kr) {
    ...
    *result = internal_io_service_get_matching_service(
        main_port, (const char *)data, matchingCnt, service);
    vm_deallocate(kernel_map, data, matchingCnt);
  }
...
kern_return_t is_io_service_get_matching_service_bin(
    mach_port_t main_port, io_struct_inband_t matching,
    mach_msg_type_number_t matchingCnt, io_object_t *service) {
  return internal_io_service_get_matching_service(main_port, matching,
                                                  matchingCnt, service);
}

static kern_return_t internal_io_service_add_notification(
    mach_port_t main_port, io_name_t notification_type, const char *matching,
    size_t matching_size, mach_port_t port, void *reference,
    vm_size_t referenceSize, bool client64, io_object_t *notification) {
  ...
  do {
    ...
    if (userNotify && !userNotify->init(port, userMsgType, reference,
                                        referenceSize, client64)) {
      userNotify->release();
      userNotify = NULL;
    }
    if (!userNotify) {
    ...
    notify = IOService::addMatchingNotification(
        sym, dict, &userNotify->_handler, userNotify);
    if (notify) {
...
kern_return_t is_io_service_add_notification(
    mach_port_t main_port, io_name_t notification_type, io_string_t matching,
    mach_port_t port, io_async_ref_t reference,
    mach_msg_type_number_t referenceCnt, io_object_t *notification) {
  return kIOReturnUnsupported;
}
...
kern_return_t is_io_service_add_notification_64(
    mach_port_t main_port, io_name_t notification_type, io_string_t matching,
    mach_port_t wake_port, io_async_ref64_t reference,
    mach_msg_type_number_t referenceCnt, io_object_t *notification) {
  return kIOReturnUnsupported;
}
...
kern_return_t is_io_service_add_notification_bin(
    mach_port_t main_port, io_name_t notification_type,
    io_struct_inband_t matching, mach_msg_type_number_t matchingCnt,
    mach_port_t wake_port, io_async_ref_t reference,
    mach_msg_type_number_t referenceCnt, io_object_t *notification) {
  ...
  bzero(&zreference[referenceCnt],
        (ASYNC_REF_COUNT - referenceCnt) * sizeof(zreference[0]));

  return internal_io_service_add_notification(
      main_port, notification_type, matching, matchingCnt, wake_port,
      &zreference[0], sizeof(io_async_ref_t), false, notification);
}
...
kern_return_t is_io_service_add_notification_bin_64(
  ...
  bcopy(&reference[0], &zreference[0], referenceCnt * sizeof(zreference[0]));
  bzero(&zreference[referenceCnt],
        (ASYNC_REF64_COUNT - referenceCnt) * sizeof(zreference[0]));

  return internal_io_service_add_notification(
      main_port, notification_type, matching, matchingCnt, wake_port,
      &zreference[0], sizeof(io_async_ref64_t), true, notification);
}

static kern_return_t internal_io_service_add_notification_ool(
    mach_port_t main_port, io_name_t notification_type, io_buf_ptr_t matching,
    mach_msg_type_number_t matchingCnt, mach_port_t wake_port, void *reference,
    vm_size_t referenceSize, bool client64, kern_return_t *result,
  ...
  if (KERN_SUCCESS == kr) {
    ...
    // and mig will copy out objects on success
    *notification = NULL;
    *result = internal_io_service_add_notification(
        main_port, notification_type, (char *)data, matchingCnt, wake_port,
        reference, referenceSize, client64, notification);
    vm_deallocate(kernel_map, data, matchingCnt);
  }
...
kern_return_t is_io_service_add_notification_ool(
    mach_port_t main_port, io_name_t notification_type, io_buf_ptr_t matching,
    mach_msg_type_number_t matchingCnt, mach_port_t wake_port,
    io_async_ref_t reference, mach_msg_type_number_t referenceCnt,
    kern_return_t *result, io_object_t *notification) {
  io_async_ref_t zreference;
  ...
  return internal_io_service_add_notification_ool(
      main_port, notification_type, matching, matchingCnt, wake_port,
      &zreference[0], sizeof(io_async_ref_t), false, result, notification);
}
...
kern_return_t is_io_service_add_notification_ool_64(
    mach_port_t main_port, io_name_t notification_type, io_buf_ptr_t matching,
    mach_msg_type_number_t matchingCnt, mach_port_t wake_port,
    io_async_ref64_t reference, mach_msg_type_number_t referenceCnt,
    kern_return_t *result, io_object_t *notification) {
  io_async_ref64_t zreference;
  ...
  return internal_io_service_add_notification_ool(
      main_port, notification_type, matching, matchingCnt, wake_port,
      &zreference[0], sizeof(io_async_ref64_t), true, result, notification);
}
...
kern_return_t is_io_service_add_notification_old(
    ...
    natural_t ref, io_object_t *notification) {
  return is_io_service_add_notification(main_port, notification_type, matching,
                                        port, &ref, 1, notification);
}

static kern_return_t internal_io_service_add_interest_notification(
    io_object_t _service, io_name_t type_of_interest, mach_port_t port,
    ...
    do {
      ...
      if (sym == gIOAppPowerStateInterest &&
          !(IOCurrentTaskHasEntitlement(
                "com.apple.private.power.notifications") ||
            IOCurrentTaskHasEntitlement(
                "com.apple.private.power.notifications-temp"))) {
        OSString *taskName = IOCopyLogNameForPID(proc_selfpid());
        IOLog("IORegisterForSystemPower called by %s without "
              "\"com.apple.private.power.notifications\" entitlement\n",
      ...
      if (userNotify &&
          !userNotify->init(port, kIOServiceMessageNotificationType, reference,
                            referenceSize, client64)) {
        userNotify->release();
        userNotify = NULL;
      }
      ...
      notify =
          service->registerInterest(sym, &userNotify->_handler, userNotify);
      if (notify) {
        *notification = userNotify;
        userNotify->setNotification(notify);
        err = kIOReturnSuccess;
...
kern_return_t is_io_service_add_interest_notification(
    io_object_t service, io_name_t type_of_interest, mach_port_t port,
    io_async_ref_t reference, mach_msg_type_number_t referenceCnt,
    io_object_t *notification) {
  io_async_ref_t zreference;
  ...
  return internal_io_service_add_interest_notification(
      service, type_of_interest, port, &zreference[0], sizeof(io_async_ref_t),
      false, notification);
}

/* Routine io_service_add_interest_notification_64 */
kern_return_t is_io_service_add_interest_notification_64(
    io_object_t service, io_name_t type_of_interest, mach_port_t wake_port,
    io_async_ref64_t reference, mach_msg_type_number_t referenceCnt,
    io_object_t *notification) {
  io_async_ref64_t zreference;
  ...
  return internal_io_service_add_interest_notification(
      service, type_of_interest, wake_port, &zreference[0],
      sizeof(io_async_ref64_t), true, notification);
}

/* Routine io_service_acknowledge_notification */
kern_return_t is_io_service_acknowledge_notification(io_object_t _service,
                                                     natural_t notify_ref,
                                                     natural_t response) {
  CHECK(IOService, _service, service);

  return service->acknowledgeNotification(
      (IONotificationRef)(uintptr_t)notify_ref, (IOOptionBits)response);
}

/* Routine io_connect_get_semaphore */
kern_return_t
is_io_connect_get_notification_semaphore(io_connect_t connection,
  ...
  ret = client->getNotificationSemaphore((UInt32)notification_type, semaphore);
...
kern_return_t is_io_registry_get_root_entry(mach_port_t main_port,
                                            io_object_t *root) {
...
kern_return_t is_io_registry_create_iterator(mach_port_t main_port,
                                             io_name_t plane, uint32_t options,
                                             io_object_t *iterator) {
  if (main_port != main_device_port) {
    return kIOReturnNotPrivileged;
  }

  *iterator = IOUserIterator::withIterator(IORegistryIterator::iterateOver(
...
kern_return_t is_io_registry_entry_create_iterator(io_object_t registry_entry,
                                                   io_name_t plane,
                                                   uint32_t options,
                                                   io_object_t *iterator) {
  ...
  *iterator = IOUserIterator::withIterator(IORegistryIterator::iterateOver(
      entry, IORegistryEntry::getPlane(plane), options));

  return *iterator ? kIOReturnSuccess : kIOReturnBadArgument;
}

/* Routine io_registry_iterator_enter */
kern_return_t is_io_registry_iterator_enter_entry(io_object_t iterator) {
...
kern_return_t is_io_registry_iterator_exit_entry(io_object_t iterator) {
  bool didIt;
...
kern_return_t is_io_registry_entry_from_path(mach_port_t main_port,
                                             io_string_t path,
...
kern_return_t is_io_registry_entry_from_path_ool(
    mach_port_t main_port, io_string_inband_t path, io_buf_ptr_t path_ool,
    mach_msg_type_number_t path_oolCnt, kern_return_t *result,
    io_object_t *registry_entry) {
  IORegistryEntry *entry;
  vm_map_offset_t map_data;
  const char *cpath;
  ...
  if (path[0]) {
    ...
    if (KERN_SUCCESS == err) {
      // must return success to mig after vm_map_copyout() succeeds, so result
      // is actual
...
kern_return_t is_io_registry_entry_in_plane(io_object_t registry_entry,
                                            io_name_t plane,
                                            boolean_t *inPlane) {
  CHECK(IORegistryEntry, registry_entry, entry);
...
kern_return_t is_io_registry_entry_get_path(io_object_t registry_entry,
                                            io_name_t plane, io_string_t path) {
  int length;
  CHECK(IORegistryEntry, registry_entry, entry);
...
kern_return_t is_io_registry_entry_get_path_ool(
    io_object_t registry_entry, io_name_t plane, io_string_inband_t path,
    io_buf_ptr_t *path_ool, mach_msg_type_number_t *path_oolCnt) {
  enum { kMaxPath = 16384 };
  IOReturn err;
  int length;
  char *buf;
  ...
  *path_oolCnt = 0;
  ...
  if (entry->getPath(path, &length, IORegistryEntry::getPlane(plane))) {
    ...
    } else if (!entry->getPath(buf, &length,
...
kern_return_t is_io_registry_entry_get_name(io_object_t registry_entry,
                                            io_name_t name) {
  CHECK(IORegistryEntry, registry_entry, entry);
...
kern_return_t is_io_registry_entry_get_name_in_plane(io_object_t registry_entry,
                                                     io_name_t planeName,
                                                     io_name_t name) {
...
kern_return_t
is_io_registry_entry_get_registry_entry_id(io_object_t registry_entry,
                                           uint64_t *entry_id) {
  CHECK(IORegistryEntry, registry_entry, entry);
...
static OSObject *IOCopyPropertyCompatible(IORegistryEntry *regEntry,
                                          const char *name) {
  OSObject *obj;
  ...
  if (!compatProperties && IOTaskRegistryCompatibility(current_task())) {
...
kern_return_t is_io_registry_entry_get_property_bytes(
    io_object_t registry_entry, io_name_t property_name, io_struct_inband_t buf,
...
#if CONFIG_MACF
  if (0 !=
      mac_iokit_check_get_property(kauth_cred_get(), entry, property_name)) {
    return kIOReturnNotPermitted;
  }
#endif
  ...
  if ((data = OSDynamicCast(OSData, obj))) {
    ...
    bytes = (const void *)(((UInt32)bytes) + (sizeof(UInt64) - len));
#endif
  } else {
...
kern_return_t is_io_registry_entry_get_property(
    io_object_t registry_entry, io_name_t property_name,
    io_buf_ptr_t *properties, mach_msg_type_number_t *propertiesCnt) {
  kern_return_t err;
  unsigned int len;
  OSObject *obj;
...
kern_return_t is_io_registry_entry_get_property_recursively(
    ...
    uint32_t options, io_buf_ptr_t *properties,
  ...
  obj = entry->copyProperty(property_name, IORegistryEntry::getPlane(plane),
                            options);
  if (!obj) {
    return kIOReturnNotFound;
  }
...
kern_return_t
is_io_registry_entry_get_properties(io_object_t registry_entry,
...
GetPropertiesEditor(void *reference, OSSerialize *s, OSCollection *container,
                    const OSSymbol *name, const OSMetaClassBase *value) {
  ...
  if (ref->root == container) {
    if (0 != mac_iokit_check_get_property(ref->cred, ref->entry,
...
kern_return_t is_io_registry_entry_get_properties_bin_buf(
    io_object_t registry_entry, mach_vm_address_t buf, mach_vm_size_t *bufsize,
    io_buf_ptr_t *properties, mach_msg_type_number_t *propertiesCnt) {
...
kern_return_t
is_io_registry_entry_get_properties_bin(io_object_t registry_entry,
                                        io_buf_ptr_t *properties,
                                        mach_msg_type_number_t *propertiesCnt) {
  return is_io_registry_entry_get_properties_bin_buf(registry_entry, 0, NULL,
                                                     properties, propertiesCnt);
}

/* Routine io_registry_entry_get_property_bin_buf */
kern_return_t is_io_registry_entry_get_property_bin_buf(
    io_object_t registry_entry, io_name_t plane, io_name_t property_name,
    uint32_t options, mach_vm_address_t buf, mach_vm_size_t *bufsize,
    io_buf_ptr_t *properties, mach_msg_type_number_t *propertiesCnt) {
  kern_return_t err;
  unsigned int len;
  OSObject *obj;
      ...
      if (obj == NULL) {
        ...
        if (iter) {
          while ((NULL == obj) && (entry = iter->getNextObject())) {
            OSObject *currentObj =
                IOCopyPropertyCompatible(entry, property_name);
#if CONFIG_MACF
            if (currentObj != NULL &&
                0 != mac_iokit_check_get_property(kauth_cred_get(), entry,
                                                  property_name)) {
              // Record that MAC hook blocked this entry and property, and
...
kern_return_t is_io_registry_entry_get_property_bin(
    ...
    uint32_t options, io_buf_ptr_t *properties,
    mach_msg_type_number_t *propertiesCnt) {
  return is_io_registry_entry_get_property_bin_buf(
      registry_entry, plane, property_name, options, 0, NULL, properties,
      propertiesCnt);
}

/* Routine io_registry_entry_set_properties */
kern_return_t is_io_registry_entry_set_properties(
    io_object_t registry_entry, io_buf_ptr_t properties,
    mach_msg_type_number_t propertiesCnt, kern_return_t *result) {
  OSObject *obj;
  kern_return_t err;
  IOReturn res;
  vm_offset_t data;
  ...
  if (KERN_SUCCESS == err) {
    ...
    if (!obj) {
      ...
      OSObject *allowable =
          entry->copyProperty(gIORegistryEntryAllowableSetPropertiesKey);
      ...
      if (!allowable) {
        ...
        if (!props) {
          ...
          props->iterateObjects(^(const OSSymbol *key, OSObject *value) {
            ...
            allFound &= found;
            if (!found) {
              IOLog("IORegistryEntrySetProperties(%s, %s) disallowed due "
                    "to " kIORegistryEntryAllowableSetPropertiesKey "\n",
      ...
      if (kIOReturnSuccess == res) {
        IOUserClient *client = OSDynamicCast(IOUserClient, entry);

        if (client && client->defaultLockingSetProperties) {
          IORWLockWrite(&client->lock);
        }

        if (!client && (kOSBooleanTrue ==
                        entry->getProperty(
...
kern_return_t is_io_registry_entry_get_child_iterator(
    io_object_t registry_entry, io_name_t plane, io_object_t *iterator) {
  CHECK(IORegistryEntry, registry_entry, entry);

  *iterator = IOUserIterator::withIterator(
      entry->getChildIterator(IORegistryEntry::getPlane(plane)));
...
kern_return_t is_io_registry_entry_get_parent_iterator(
  ...
  *iterator = IOUserIterator::withIterator(
      entry->getParentIterator(IORegistryEntry::getPlane(plane)));

  return kIOReturnSuccess;
}

/* Routine io_service_get_busy_state */
kern_return_t is_io_service_get_busy_state(io_object_t _service,
                                           uint32_t *busyState) {
  CHECK(IOService, _service, service);
...
kern_return_t is_io_service_get_state(io_object_t _service, uint64_t *state,
                                      uint32_t *busy_state,
...
kern_return_t is_io_service_wait_quiet(io_object_t _service,
...
kern_return_t is_io_service_wait_quiet_with_options(io_object_t _service,
                                                    mach_timespec_t wait_time,
                                                    uint32_t options) {
  uint64_t timeoutNS;
  ...
  if ((options & kIOWaitQuietPanicOnFailure) &&
      !IOCurrentTaskHasEntitlement(kIOWaitQuietPanicsEntitlement)) {
    OSString *taskName = IOCopyLogNameForPID(proc_selfpid());
...
kern_return_t is_io_service_request_probe(io_object_t _service,
                                          uint32_t options) {
  CHECK(IOService, _service, service);
...
kern_return_t is_io_service_get_authorization_id(io_object_t _service,
...
kern_return_t is_io_service_set_authorization_id(io_object_t _service,
                                                 uint64_t authorization_id) {
  CHECK(IOService, _service, service);
...
kern_return_t
is_io_service_open_extended(io_object_t _service, task_t owningTask,
                            uint32_t connect_type, NDR_record_t ndr,
                            io_buf_ptr_t properties,
                            mach_msg_type_number_t propertiesCnt,
                            kern_return_t *result, io_object_t *connection) {
...
#if CONFIG_MACF
  if (mac_iokit_check_open_service(kauth_cred_get(), service, connect_type) !=
      0) {
    return kIOReturnNotPermitted;
  }
#endif
  do {
...
#endif
    res = service->newUserClient(owningTask, (void *)owningTask, connect_type,
                                 propertiesDict, &client);
    ...
    if (res == kIOReturnSuccess &&
        OSDynamicCast(IOUserClient, client) == NULL) {
    ...
    if (res == kIOReturnSuccess) {
      ...
      const char *creatorNameCStr =
          creatorName ? creatorName->getCStringNoCopy() : "<unknown>";
      client->sharedInstance =
          (NULL != client->getProperty(kIOUserClientSharedInstanceKey));
      ...
      if (!client->opened) {
        ...
        client->messageAppSuspended =
            (NULL != client->getProperty(kIOUserClientMessageAppSuspendedKey));
        {
          OSObject *obj;
          ...
          if (obj) {
            hasProps = true;
            client->defaultLocking =
                (kOSBooleanFalse !=
                 client->getProperty(kIOUserClientDefaultLockingKey));
          } else if (client->uc2022) {
            res = kIOReturnError;
          }
          obj =
              client->getProperty(kIOUserClientDefaultLockingSetPropertiesKey);
          if (obj) {
            hasProps = true;
            client->defaultLockingSetProperties =
                (kOSBooleanFalse !=
                 client->getProperty(
          ...
          obj = client->getProperty(
              kIOUserClientDefaultLockingSingleThreadExternalMethodKey);
          if (obj) {
            hasProps = true;
            client->defaultLockingSingleThreadExternalMethod =
                (kOSBooleanFalse !=
                 client->getProperty(
                     kIOUserClientDefaultLockingSingleThreadExternalMethodKey));
          } else if (client->uc2022) {
          ...
          if (kIOReturnSuccess != res) {
            IOLog("IOUC %s requires kIOUserClientDefaultLockingKey, "
                  "kIOUserClientDefaultLockingSetPropertiesKey, "
                  "kIOUserClientDefaultLockingSingleThreadExternalMethodKey\n",
          ...
          if (!hasProps) {
            ...
            if (!kext || !kext->hasDependency(gIOSurfaceIdentifier)) {
              ...
              client->setProperty(kIOUserClientDefaultLockingKey,
                                  kOSBooleanTrue);
            }
          }
        }
      ...
      OSObject *requiredEntitlement =
          client->copyProperty(gIOUserClientEntitlementsKey);
      OSString *requiredEntitlementString =
          OSDynamicCast(OSString, requiredEntitlement);
      // If this is an IOUserClient2022, having kIOUserClientEntitlementsKey is
      // mandatory. If it has kIOUserClientEntitlementsKey, the value must be
      // either kOSBooleanFalse or an OSString If the value is kOSBooleanFalse,
      // we allow access. If the value is an OSString, we allow access if the
      // task has the named entitlement
      if (client->uc2022) {
        if (!requiredEntitlement) {
          IOLog("IOUC %s missing " kIOUserClientEntitlementsKey " property\n",
                client->getMetaClass()->getClassName());
          disallowAccess = true;
        } else if (!requiredEntitlementString &&
                   requiredEntitlement != kOSBooleanFalse) {
          IOLog("IOUC %s had " kIOUserClientEntitlementsKey
                "with value not boolean false or string\n",
                client->getMetaClass()->getClassName());
          disallowAccess = true;
        }
      }
      ...
      if (requiredEntitlement && disallowAccess == false) {
        if (kOSBooleanFalse == requiredEntitlement) {
          ...
          disallowAccess = !IOTaskHasEntitlement(
              owningTask, requiredEntitlementString->getCStringNoCopy());
          if (disallowAccess) {
      ...
      if (disallowAccess) {
        res = kIOReturnNotPrivileged;
               ...
               mac_iokit_check_open(kauth_cred_get(), client, connect_type)) {
      ...
      if ((kIOReturnSuccess == res) && gIOUCFilterCallbacks &&
          gIOUCFilterCallbacks->io_filter_resolver) {
        io_filter_policy_t filterPolicy;
        filterPolicy = client->filterForTask(owningTask, 0);
        if (!filterPolicy) {
          res = gIOUCFilterCallbacks->io_filter_resolver(
              owningTask, client, connect_type, &filterPolicy);
...
kern_return_t is_io_service_close(io_connect_t connection) {
  OSSet *mappings;
...
kern_return_t is_io_connect_get_service(io_connect_t connection,
                                        io_object_t *service) {
...
kern_return_t is_io_connect_set_notification_port(io_connect_t connection,
...
kern_return_t is_io_connect_set_notification_port_64(
    io_connect_t connection, uint32_t notification_type, mach_port_t port,
    io_user_reference_t reference) {
  kern_return_t ret;
  CHECK(IOUserClient, connection, client);
  ...
  client->ipcEnter(kIPCLockWrite);
  ret = client->registerNotificationPort(port, notification_type, reference);
  client->ipcExit(kIPCLockWrite);
...
kern_return_t is_io_connect_map_shared_memory(
    io_connect_t connection, uint32_t memory_type, task_t into_task,
    mach_vm_address_t *address, mach_vm_size_t *size, uint32_t map_flags,
    io_name_t property_name, io_struct_inband_t inband_output,
  ...
  if (client->sharedInstance || (into_task != current_task())) {
    return kIOReturnUnsupported;
  }
  ...
  if (memory && (kIOReturnSuccess == err)) {
    ...
    if (kIOReturnSuccess == err) {
      ...
      options = (options & ~kIOMapUserOptionsMask) |
                (map_flags & kIOMapUserOptionsMask) | kIOMapAnywhere;
...
kern_return_t is_io_connect_map_memory_into_task(
    io_connect_t connection, uint32_t memory_type, task_t into_task,
    mach_vm_address_t *address, mach_vm_size_t *size, uint32_t flags) {
  IOReturn err;
  IOMemoryMap *map;
    ...
    *address = map->getAddress();
    ...
    if (client->sharedInstance || (into_task != current_task())) {
      ...
      mach_port_name_t name __unused =
          IOMachPort::makeSendRightForTask(into_task, map, IKOT_IOKIT_OBJECT);
      map->release();
    } else {
      // keep it with the user client
      IOLockLock(gIOObjectPortLock);
...
kern_return_t is_io_connect_map_memory(io_object_t connect, uint32_t type,
                                       task_t task, uint32_t *mapAddr,
                                       uint32_t *mapSize, uint32_t flags) {
  IOReturn err;
  ...
  err = is_io_connect_map_memory_into_task(connect, type, task, &address, &size,
                                           flags);
...
kern_return_t is_io_connect_unmap_memory_from_task(io_connect_t connection,
  ...
  if (memory && (kIOReturnSuccess == err)) {
    options =
        (options & ~kIOMapUserOptionsMask) | kIOMapAnywhere | kIOMapReference;
    ...
    if (map) {
      ...
      bool is_shared_instance_or_from_current_task =
          from_task != current_task() || client->sharedInstance;
      if (is_shared_instance_or_from_current_task) {
        name =
            IOMachPort::makeSendRightForTask(from_task, map, IKOT_IOKIT_OBJECT);
        map->release();
      }
...
kern_return_t is_io_connect_unmap_memory(io_object_t connect, uint32_t type,
                                         task_t task, uint32_t mapAddr) {
  IOReturn err;
  mach_vm_address_t address;
...
kern_return_t is_io_connect_add_client(io_connect_t connection,
...
kern_return_t is_io_connect_set_properties(io_connect_t connection,
                                           io_buf_ptr_t properties,
                                           mach_msg_type_number_t propertiesCnt,
                                           kern_return_t *result) {
  return is_io_registry_entry_set_properties(connection, properties,
                                             propertiesCnt, result);
}

/* Routine io_user_client_method */
kern_return_t is_io_connect_method_var_output(
    io_connect_t connection, uint32_t selector,
    io_scalar_inband64_t scalar_input, mach_msg_type_number_t scalar_inputCnt,
    io_struct_inband_t inband_input, mach_msg_type_number_t inband_inputCnt,
    mach_vm_address_t ool_input, mach_vm_size_t ool_input_size,
    io_struct_inband_t inband_output, mach_msg_type_number_t *inband_outputCnt,
    io_scalar_inband64_t scalar_output,
    mach_msg_type_number_t *scalar_outputCnt, io_buf_ptr_t *var_output,
  ...
  if (ool_input) {
    inputMD = IOMemoryDescriptor::withAddressRange(
        ool_input, ool_input_size, kIODirectionOut | kIOMemoryMapCopyOnWrite,
        current_task());
  }
  ...
  if (filterPolicy && gIOUCFilterCallbacks->io_filter_applier) {
    ret = gIOUCFilterCallbacks->io_filter_applier(
        client, filterPolicy, io_filter_type_external_method, selector);
  }
...
kern_return_t is_io_connect_method(
  ...
  if (ool_output && ool_output_size) {
    outputMD = IOMemoryDescriptor::withAddressRange(
        ool_output, *ool_output_size, kIODirectionIn, current_task());
  }
...
kern_return_t is_io_connect_async_method(
    io_connect_t connection, mach_port_t wake_port, io_async_ref64_t reference,
    mach_msg_type_number_t referenceCnt, uint32_t selector,
    io_scalar_inband64_t scalar_input, mach_msg_type_number_t scalar_inputCnt,
    io_struct_inband_t inband_input, mach_msg_type_number_t inband_inputCnt,
    mach_vm_address_t ool_input, mach_vm_size_t ool_input_size,
    io_struct_inband_t inband_output, mach_msg_type_number_t *inband_outputCnt,
  ...
  if (filterPolicy && gIOUCFilterCallbacks->io_filter_applier) {
    ret = gIOUCFilterCallbacks->io_filter_applier(
        client, filterPolicy, io_filter_type_external_async_method, selector);
  }
  if (kIOReturnSuccess == ret) {
    ret = client->callExternalMethod(selector, &args);
  }
...
kern_return_t is_io_connect_method_scalarI_scalarO(
    io_object_t connect, uint32_t index, io_scalar_inband_t input,
    mach_msg_type_number_t inputCount, io_scalar_inband_t output,
  ...
  err = is_io_connect_method(connect, index, _input, inputCount, NULL, 0, 0, 0,
                             NULL, &struct_outputCnt, _output, outputCount, 0,
                             &ool_output_size);
...
kern_return_t shim_io_connect_method_scalarI_scalarO(
    IOExternalMethod *method, IOService *object, const io_user_scalar_t *input,
    mach_msg_type_number_t inputCount, io_user_scalar_t *output,
    mach_msg_type_number_t *outputCount) {
  IOMethod func;
  ...
  do {
    if (inputCount != method->count0) {
      IOLog("%s:%d %s: IOUserClient inputCount count mismatch 0x%llx 0x%llx\n",
            ...
            (uint64_t)method->count0);
      DTRACE_IO2(iokit_count_mismatch, uint64_t, (uint64_t)inputCount, uint64_t,
                 (uint64_t)method->count0);
      continue;
    }
    if (*outputCount != method->count1) {
      IOLog("%s:%d %s: IOUserClient outputCount count mismatch 0x%llx 0x%llx\n",
            __FUNCTION__, __LINE__, object->getName(), (uint64_t)*outputCount,
            (uint64_t)method->count1);
      DTRACE_IO2(iokit_count_mismatch, uint64_t, (uint64_t)*outputCount,
                 uint64_t, (uint64_t)method->count1);
      continue;
    }
    ...
    case 5:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), ARG32(input[2]),
                            ARG32(input[3]), ARG32(input[4]), &_output[0]);
      break;
    case 4:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), ARG32(input[2]),
                            ARG32(input[3]), &_output[0], &_output[1]);
      break;
    case 3:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), ARG32(input[2]),
      ...
      break;
    case 2:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), &_output[0],
                            &_output[1], &_output[2], &_output[3]);
      break;
    case 1:
      err = (object->*func)(ARG32(input[0]), &_output[0], &_output[1],
                            &_output[2], &_output[3], &_output[4]);
      break;
    case 0:
      err = (object->*func)(&_output[0], &_output[1], &_output[2], &_output[3],
                            &_output[4], &_output[5]);
      break;
...
kern_return_t is_io_async_method_scalarI_scalarO(
    io_object_t connect, mach_port_t wake_port, io_async_ref_t reference,
    mach_msg_type_number_t referenceCnt, uint32_t index,
    io_scalar_inband_t input, mach_msg_type_number_t inputCount,
    io_scalar_inband_t output, mach_msg_type_number_t *outputCount) {
  IOReturn err;
  uint32_t i;
  io_scalar_inband64_t _input;
  io_scalar_inband64_t _output;
  ...
  bzero(&_reference[referenceCnt],
        (ASYNC_REF64_COUNT - referenceCnt) * sizeof(_reference[0]));
  ...
  err = is_io_connect_async_method(connect, wake_port, _reference, referenceCnt,
                                   index, _input, inputCount, NULL, 0, 0, 0,
                                   NULL, &struct_outputCnt, _output,
                                   outputCount, 0, &ool_output_size);
...
kern_return_t is_io_async_method_scalarI_structureO(
    ...
    mach_msg_type_number_t referenceCnt, uint32_t index,
    io_scalar_inband_t input, mach_msg_type_number_t inputCount,
    io_struct_inband_t output, mach_msg_type_number_t *outputCount) {
  uint32_t i;
  io_scalar_inband64_t _input;
  io_async_ref64_t _reference;
  ...
  return is_io_connect_async_method(connect, wake_port, _reference,
                                    referenceCnt, index, _input, inputCount,
                                    NULL, 0, 0, 0, output, outputCount, NULL,
                                    &scalar_outputCnt, 0, &ool_output_size);
}

/* Routine io_async_method_scalarI_structureI */
kern_return_t is_io_async_method_scalarI_structureI(
    io_connect_t connect, mach_port_t wake_port, io_async_ref_t reference,
    mach_msg_type_number_t referenceCnt, uint32_t index,
    io_scalar_inband_t input, mach_msg_type_number_t inputCount,
    io_struct_inband_t inputStruct, mach_msg_type_number_t inputStructCount) {
  uint32_t i;
  io_scalar_inband64_t _input;
  io_async_ref64_t _reference;
  ...
  return is_io_connect_async_method(
      connect, wake_port, _reference, referenceCnt, index, _input, inputCount,
      inputStruct, inputStructCount, 0, 0, NULL, &inband_outputCnt, NULL,
      &scalar_outputCnt, 0, &ool_output_size);
}
...
kern_return_t is_io_async_method_structureI_structureO(
    io_object_t connect, mach_port_t wake_port, io_async_ref_t reference,
    mach_msg_type_number_t referenceCnt, uint32_t index,
    io_struct_inband_t input, mach_msg_type_number_t inputCount,
    io_struct_inband_t output, mach_msg_type_number_t *outputCount) {
  uint32_t i;
  mach_msg_type_number_t scalar_outputCnt = 0;
  mach_vm_size_t ool_output_size = 0;
  ...
  return is_io_connect_async_method(connect, wake_port, _reference,
                                    referenceCnt, index, NULL, 0, input,
                                    inputCount, 0, 0, output, outputCount, NULL,
                                    &scalar_outputCnt, 0, &ool_output_size);
}

kern_return_t shim_io_async_method_scalarI_scalarO(
    IOExternalAsyncMethod *method, IOService *object, mach_port_t asyncWakePort,
    io_user_reference_t *asyncReference, uint32_t asyncReferenceCount,
    const io_user_scalar_t *input, mach_msg_type_number_t inputCount,
    io_user_scalar_t *output, mach_msg_type_number_t *outputCount) {
  IOAsyncMethod func;
  uint32_t i;
  io_scalar_inband_t _output;
  IOReturn err;
  ...
  do {
    ...
    switch (inputCount) {
    case 6:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            ARG32(input[2]), ARG32(input[3]), ARG32(input[4]),
                            ARG32(input[5]));
      break;
    case 5:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
    ...
    case 4:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            ARG32(input[2]), ARG32(input[3]), &_output[0],
                            &_output[1]);
      break;
    case 3:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            ARG32(input[2]), &_output[0], &_output[1],
                            &_output[2]);
      break;
    case 2:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            &_output[0], &_output[1], &_output[2], &_output[3]);
      break;
    case 1:
      err = (object->*func)(reference, ARG32(input[0]), &_output[0],
                            &_output[1], &_output[2], &_output[3], &_output[4]);
      break;
    case 0:
      err = (object->*func)(reference, &_output[0], &_output[1], &_output[2],
...
kern_return_t is_io_connect_method_scalarI_structureO(
    io_object_t connect, uint32_t index, io_scalar_inband_t input,
    mach_msg_type_number_t inputCount, io_struct_inband_t output,
    mach_msg_type_number_t *outputCount) {
  uint32_t i;
  io_scalar_inband64_t _input;
  ...
  return is_io_connect_method(connect, index, _input, inputCount, NULL, 0, 0, 0,
                              output, outputCount, NULL, &scalar_outputCnt, 0,
                              &ool_output_size);
}
  ...
  do {
    ...
    if ((kIOUCVariableStructureSize != method->count1) &&
      ...
      IOLog("%s:%d %s: IOUserClient outputCount count mismatch 0x%llx 0x%llx "
            "0x%llx\n",
            __FUNCTION__, __LINE__, object->getName(), (uint64_t)*outputCount,
            (uint64_t)method->count1, (uint64_t)kIOUCVariableStructureSize);
      DTRACE_IO2(iokit_count_mismatch, uint64_t, (uint64_t)*outputCount,
                 uint64_t, (uint64_t)method->count1);
      continue;
    }
    ...
    case 5:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), ARG32(input[2]),
                            ARG32(input[3]), ARG32(input[4]), output);
      break;
    case 4:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), ARG32(input[2]),
                            ARG32(input[3]), output, (void *)outputCount);
      break;
    case 3:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), ARG32(input[2]),
      ...
      break;
    case 2:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), output,
                            (void *)outputCount, NULL, NULL);
      break;
    case 1:
      err = (object->*func)(ARG32(input[0]), output, (void *)outputCount, NULL,
                            NULL, NULL);
      break;
    case 0:
      err =
          (object->*func)(output, (void *)outputCount, NULL, NULL, NULL, NULL);
      break;
...
kern_return_t shim_io_async_method_scalarI_structureO(
  ...
  do {
    ...
    case 4:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            ARG32(input[2]), ARG32(input[3]), output,
                            (void *)outputCount);
      break;
    case 3:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            ARG32(input[2]), output, (void *)outputCount, NULL);
      break;
    case 2:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]), output,
                            (void *)outputCount, NULL, NULL);
      break;
    case 1:
      err = (object->*func)(reference, ARG32(input[0]), output,
                            (void *)outputCount, NULL, NULL, NULL);
      break;
    case 0:
      err = (object->*func)(reference, output, (void *)outputCount, NULL, NULL,
                            NULL, NULL);
      break;
...
kern_return_t is_io_connect_method_scalarI_structureI(
    io_connect_t connect, uint32_t index, io_scalar_inband_t input,
    mach_msg_type_number_t inputCount, io_struct_inband_t inputStruct,
  ...
  return is_io_connect_method(connect, index, _input, inputCount, inputStruct,
                              inputStructCount, 0, 0, NULL, &inband_outputCnt,
                              NULL, &scalar_outputCnt, 0, &ool_output_size);
}
...
kern_return_t shim_io_connect_method_scalarI_structureI(
  ...
  do {
    ...
    if ((kIOUCVariableStructureSize != method->count1) &&
      ...
      IOLog("%s:%d %s: IOUserClient outputCount count mismatch 0x%llx 0x%llx "
            "0x%llx\n",
            __FUNCTION__, __LINE__, object->getName(),
            (uint64_t)inputStructCount, (uint64_t)method->count1,
            (uint64_t)kIOUCVariableStructureSize);
      DTRACE_IO2(iokit_count_mismatch, uint64_t, (uint64_t)inputStructCount,
                 uint64_t, (uint64_t)method->count1);
      continue;
    }
    ...
    case 5:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), ARG32(input[2]),
                            ARG32(input[3]), ARG32(input[4]), inputStruct);
      break;
    case 4:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), (void *)input[2],
                            ARG32(input[3]), inputStruct,
    ...
    case 3:
      ...
      break;
    case 2:
      err = (object->*func)(ARG32(input[0]), ARG32(input[1]), inputStruct,
                            (void *)(uintptr_t)inputStructCount, NULL, NULL);
      break;
    case 1:
      err = (object->*func)(ARG32(input[0]), inputStruct,
                            (void *)(uintptr_t)inputStructCount, NULL, NULL,
                            NULL);
      break;
    case 0:
...
kern_return_t shim_io_async_method_scalarI_structureI(
  ...
  do {
    ...
    case 4:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            ARG32(input[2]), ARG32(input[3]), inputStruct,
                            (void *)(uintptr_t)inputStructCount);
      break;
    case 3:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            ARG32(input[2]), inputStruct,
                            (void *)(uintptr_t)inputStructCount, NULL);
      break;
    case 2:
      err = (object->*func)(reference, ARG32(input[0]), ARG32(input[1]),
                            inputStruct, (void *)(uintptr_t)inputStructCount,
      ...
      break;
    case 1:
      err = (object->*func)(reference, ARG32(input[0]), inputStruct,
                            (void *)(uintptr_t)inputStructCount, NULL, NULL,
                            NULL);
      break;
    case 0:
      err = (object->*func)(reference, inputStruct,
                            (void *)(uintptr_t)inputStructCount, NULL, NULL,
                            NULL, NULL);
      break;
...
kern_return_t is_io_connect_method_structureI_structureO(
    io_object_t connect, uint32_t index, io_struct_inband_t input,
    mach_msg_type_number_t inputCount, io_struct_inband_t output,
    mach_msg_type_number_t *outputCount) {
  mach_msg_type_number_t scalar_outputCnt = 0;
  mach_vm_size_t ool_output_size = 0;

  return is_io_connect_method(connect, index, NULL, 0, input, inputCount, 0, 0,
                              output, outputCount, NULL, &scalar_outputCnt, 0,
                              &ool_output_size);
}

kern_return_t shim_io_connect_method_structureI_structureO(
    IOExternalMethod *method, IOService *object, io_struct_inband_t input,
    mach_msg_type_number_t inputCount, io_struct_inband_t output,
    IOByteCount *outputCount) {
  IOMethod func;
  IOReturn err = kIOReturnBadArgument;
  ...
  do {
    if ((kIOUCVariableStructureSize != method->count0) &&
      ...
      IOLog("%s:%d %s: IOUserClient inputCount count mismatch 0x%llx 0x%llx "
            "0x%llx\n",
            __FUNCTION__, __LINE__, object->getName(), (uint64_t)inputCount,
            (uint64_t)method->count0, (uint64_t)kIOUCVariableStructureSize);
      DTRACE_IO2(iokit_count_mismatch, uint64_t, (uint64_t)inputCount, uint64_t,
                 (uint64_t)method->count0);
      continue;
    }
    ...
    if (method->count1) {
      if (method->count0) {
        err = (object->*func)(input, output, (void *)(uintptr_t)inputCount,
                              outputCount, NULL, NULL);
      } else {
        err = (object->*func)(output, outputCount, NULL, NULL, NULL, NULL);
      }
    } else {
      err = (object->*func)(input, (void *)(uintptr_t)inputCount, NULL, NULL,
                            NULL, NULL);
    }
  } while (false);
...
kern_return_t shim_io_async_method_structureI_structureO(
  ...
  do {
    ...
    if (method->count1) {
      if (method->count0) {
        err = (object->*func)(reference, input, output,
                              (void *)(uintptr_t)inputCount, outputCount, NULL,
                              NULL);
      } else {
        err = (object->*func)(reference, output, outputCount, NULL, NULL, NULL,
                              NULL);
      }
    } else {
      err = (object->*func)(reference, input, (void *)(uintptr_t)inputCount,
                            NULL, NULL, NULL, NULL);
    }
  } while (false);
...
kern_return_t is_io_catalog_send_data(mach_port_t main_port, uint32_t flag,
                                      io_buf_ptr_t inData,
                                      mach_msg_type_number_t inDataCount,
                                      kern_return_t *result) {
  // Allow sending catalog data if there is no kextd and the kernel is
  // DEVELOPMENT || DEBUG
  ...
  if (!IOCurrentTaskHasEntitlement(kIOCatalogManagementEntitlement)) {
    ...
    IOLog("IOCatalogueSendData(%s): Not entitled\n",
          taskName ? taskName->getCStringNoCopy() : "");
    OSSafeReleaseNULL(taskName);
    // For now, fake success to not break applications relying on this function
    // succeeding. See <rdar://problem/32554970> for more details.
    return kIOReturnSuccess;
  }
  ...
  case kIOCatalogAddDriversNoMatch: {
    ...
    array = OSDynamicCast(OSArray, obj);
    if (array) {
      if (!gIOCatalogue->addDrivers(array, flag == kIOCatalogAddDrivers)) {
        kr = kIOReturnError;
      }
    } else {
      kr = kIOReturnBadArgument;
  ...
  case kIOCatalogRemoveDriversNoMatch: {
    ...
    dict = OSDynamicCast(OSDictionary, obj);
    if (dict) {
      if (!gIOCatalogue->removeDrivers(dict, flag == kIOCatalogRemoveDrivers)) {
        kr = kIOReturnError;
      }
    } else {
      kr = kIOReturnBadArgument;
...
kern_return_t is_io_catalog_terminate(mach_port_t main_port, uint32_t flag,
                                      io_name_t name) {
  kern_return_t kr;
  ...
  case kIOCatalogModuleTerminate:
    kr = gIOCatalogue->terminateDriversForModule(
        name, flag == kIOCatalogModuleUnload);
    break;
#endif
...
kern_return_t is_io_catalog_get_data(mach_port_t main_port, uint32_t flag,
...
kern_return_t is_io_catalog_get_gen_count(mach_port_t main_port,
                                          uint32_t *genCount) {
  if (main_port != main_device_port) {
    return kIOReturnNotPrivileged;
  }
 ...
 * Is invoked from IOKitLib's IOCatalogueModuleLoaded(). Doesn't seem to be
 * used.
 */
kern_return_t is_io_catalog_module_loaded(mach_port_t main_port,
                                          io_name_t name) {
  if (main_port != main_device_port) {
    return kIOReturnNotPrivileged;
  }
...
kern_return_t is_io_catalog_reset(mach_port_t main_port, uint32_t flag) {
  if (main_port != main_device_port) {
    return kIOReturnNotPrivileged;
  }
...
kern_return_t iokit_user_client_trap(struct iokit_user_client_trap_args *args) {
  ...
  if (((1ULL << 32) & ref) || !(1 & ref)) {
    ...
    if (object) {
      result = IOUserServerUEXTTrap(object, args->p1, args->p2, args->p3,
                                    args->p4, args->p5, args->p6);
    }
    OSSafeReleaseNULL(object);
  } else {
    io_object_t ref_current_task =
        iokit_lookup_connect_ref_current_task((mach_port_name_t)ref);
    if ((userClient = OSDynamicCast(IOUserClient, ref_current_task))) {
      ...
      io_filter_policy_t filterPolicy =
          userClient->filterForTask(current_task(), 0);
      if (filterPolicy && gIOUCFilterCallbacks->io_filter_applier) {
        result = gIOUCFilterCallbacks->io_filter_applier(
            userClient, filterPolicy, io_filter_type_trap, args->index);
      }
      ...
      if (trap && target) {
        ...
        if (func) {
          result = (target->*func)(args->p1, args->p2, args->p3, args->p4,
                                   args->p5, args->p6);
        }
      }
...
kern_return_t is_io_device_tree_entry_exists_with_name(mach_port_t main_port,
                                                       io_name_t name,
                                                       boolean_t *exists) {
  ...
  iter = IODTFindMatchingEntries(IORegistryEntry::getRegistryRoot(),
                                 kIODTRecursive, entryname);
...
IOReturn IOUserClient::callExternalMethod(uint32_t selector,
                                          IOExternalMethodArguments *args) {
  IOReturn ret;

  ipcEnter(defaultLocking
               ? (defaultLockingSingleThreadExternalMethod ? kIPCLockWrite
                                                           : kIPCLockRead)
               : kIPCLockNone);
  if (uc2022) {
    ret =
        ((IOUserClient2022 *)this)
            ->externalMethod(selector, (IOExternalMethodArgumentsOpaque *)args);
  } else {
    ret = externalMethod(selector, args);
  }
  ipcExit(defaultLocking
              ? (defaultLockingSingleThreadExternalMethod ? kIPCLockWrite
                                                          : kIPCLockRead)
              : kIPCLockNone);
...
MIG_SERVER_ROUTINE IOReturn IOUserClient2022::externalMethod(
    uint32_t selector, IOExternalMethodArguments *arguments,
    IOExternalMethodDispatch *dispatch, OSObject *target, void *reference) {
  panic("wrong externalMethod for IOUserClient2022");
}

IOReturn IOUserClient2022::dispatchExternalMethod(
    uint32_t selector, IOExternalMethodArgumentsOpaque *arguments,
    const IOExternalMethodDispatch2022 dispatchArray[],
    size_t dispatchArrayCount, OSObject *target, void *reference) {
  IOReturn err;
...
IOReturn IOUserClient::externalMethod(uint32_t selector,
  ...
  if (args->asyncWakePort) {
    ...
    case kIOUCScalarIStructI:
      err = shim_io_async_method_scalarI_structureI(
          method, object, args->asyncWakePort, args->asyncReference,
          args->asyncReferenceCount, args->scalarInput, args->scalarInputCount,
          (char *)args->structureInput, args->structureInputSize);
      break;

    case kIOUCScalarIScalarO:
      err = shim_io_async_method_scalarI_scalarO(
          method, object, args->asyncWakePort, args->asyncReference,
          args->asyncReferenceCount, args->scalarInput, args->scalarInputCount,
    ...
    case kIOUCScalarIStructO:
      err = shim_io_async_method_scalarI_structureO(
          method, object, args->asyncWakePort, args->asyncReference,
          args->asyncReferenceCount, args->scalarInput, args->scalarInputCount,
    ...
    case kIOUCStructIStructO:
      err = shim_io_async_method_structureI_structureO(
          method, object, args->asyncWakePort, args->asyncReference,
          args->asyncReferenceCount, (char *)args->structureInput,
          args->structureInputSize, (char *)args->structureOutput,
          &args->structureOutputSize);
      break;
    ...
    switch (method->flags & kIOUCTypeMask) {
    case kIOUCScalarIStructI:
      err = shim_io_connect_method_scalarI_structureI(
          method, object, args->scalarInput, args->scalarInputCount,
          (char *)args->structureInput, args->structureInputSize);
      break;

    case kIOUCScalarIScalarO:
      err = shim_io_connect_method_scalarI_scalarO(
          method, object, args->scalarInput, args->scalarInputCount,
          args->scalarOutput, &args->scalarOutputCount);
      break;

    case kIOUCScalarIStructO:
      err = shim_io_connect_method_scalarI_structureO(
          method, object, args->scalarInput, args->scalarInputCount,
          (char *)args->structureOutput, &structureOutputSize);
      break;

    case kIOUCStructIStructO:
      err = shim_io_connect_method_structureI_structureO(
          method, object, (char *)args->structureInput,
          args->structureInputSize, (char *)args->structureOutput,
          &structureOutputSize);
      break;
  ...
  args->structureOutputSize =
      ((typeof(args->structureOutputSize))structureOutputSize);

  return err;
}

IOReturn IOUserClient::registerFilterCallbacks(
    const struct io_filter_callbacks *callbacks, size_t size) {
  if (size < sizeof(*callbacks)) {
    return kIOReturnBadArgument;
  }
  if (!OSCompareAndSwapPtr(NULL, __DECONST(void *, callbacks),
                           &gIOUCFilterCallbacks)) {
