...
OSDefineMetaClassAndStructors(IOExtensiblePaniclog, OSObject)
    ...
    bool IOExtensiblePaniclog::init(void) {
...
bool IOExtensiblePaniclog::createWithUUID(uuid_t uuid, const char *data_id,
                                          uint32_t len,
  ...
  inst->extPaniclogHandle =
      ext_paniclog_handle_alloc_with_uuid(uuid, data_id, len, options);
...
void IOExtensiblePaniclog::free(void) {
...
kern_return_t IOExtensiblePaniclog::Create_Impl(OSData *uuid, OSString *data_id,
                                                uint32_t max_len,
                                                uint32_t options,
                                                IOExtensiblePaniclog **out) {
  ...
  mem_options =
      kIOMemoryKernelUserShared | kIOMemoryThreadSafe | kIODirectionInOut;
  ...
  inst->extPaniclogHandle = ext_paniclog_handle_alloc_with_buffer(
      uuid_copy, data_id->getCStringNoCopy(), max_len,
      inst->iomd->getBytesNoCopy(),
      (ext_paniclog_create_options_t)(options |
                                      EXT_PANICLOG_OPTIONS_WITH_BUFFER));
  if (inst->extPaniclogHandle == NULL) {
    OSSafeReleaseNULL(inst);
    return kIOReturnNoMemory;
  }
...
kern_return_t IOExtensiblePaniclog::SetActive_Impl() {
...
kern_return_t IOExtensiblePaniclog::SetInactive_Impl() {
...
kern_return_t IOExtensiblePaniclog::InsertData_Impl(OSData *data) {
  ...
  if (ext_paniclog_insert_data(extPaniclogHandle, addr, data->getLength()) !=
      0) {
    return kIOReturnBadArgument;
  }
...
kern_return_t IOExtensiblePaniclog::AppendData_Impl(OSData *data) {
  ...
  if (ext_paniclog_append_data(extPaniclogHandle, addr, data->getLength()) !=
      0) {
    return kIOReturnBadArgument;
  }
...
kern_return_t IOExtensiblePaniclog::CopyMemoryDescriptor_Impl(
...
kern_return_t IOExtensiblePaniclog::SetUsedLen_Impl(uint32_t used_len) {
  return ext_paniclog_set_used_len(extPaniclogHandle, used_len);
}
