  ...
  if (wasAlreadyRegisterd || shouldBeShared) {
    ...
    if (vector->sharedController == NULL) {
      ...
      if (wasAlreadyRegisterd) {
        ...
        // Physically disable the interrupt, but mark it as being enabled in the
        // hardware. The interruptDisabledSoft now indicates the driver's
      ...
      error =
          vector->sharedController->initInterruptController(this, vectorData);
      ...
      // If there was an original consumer try to register it on the shared
      // controller.
      if (wasAlreadyRegisterd) {
        error = vector->sharedController->registerInterrupt(
            originalNub, originalSource, vector->target, vector->handler,
            vector->refCon);
      ...
      vector->handler =
          (IOInterruptHandler)
              vector->sharedController->getInterruptHandlerAddress();
      ...
      // Do any specific initalization for this vector if it has not yet been
      // used.
      if (!wasAlreadyRegisterd) {
...
IOReturn IOInterruptController::unregisterInterrupt(IOService *nub,
                                                    int source) {
  IOInterruptSource *interruptSources;
  IOInterruptVectorNumber vectorNumber;
  IOInterruptVector *vector;
...
bool IOInterruptController::vectorCanBeShared(
    IOInterruptVectorNumber /*vectorNumber*/, IOInterruptVector * /*vector*/) {
  return false;
}
...
int IOInterruptController::getVectorType(
    IOInterruptVectorNumber /*vectorNumber*/, IOInterruptVector * /*vector*/) {
  return kIOInterruptTypeEdge;
}

void IOInterruptController::disableVectorHard(
    IOInterruptVectorNumber /*vectorNumber*/, IOInterruptVector * /*vector*/) {}

void IOInterruptController::enableVector(
    IOInterruptVectorNumber /*vectorNumber*/, IOInterruptVector * /*vector*/) {}

void IOInterruptController::causeVector(
    IOInterruptVectorNumber /*vectorNumber*/, IOInterruptVector * /*vector*/) {}

void IOInterruptController::setCPUInterruptProperties(IOService * /*service*/) {
}

void IOInterruptController::sendIPI(unsigned int /*cpu_id*/,
                                    bool /*deferred*/) {}

void IOInterruptController::cancelDeferredIPI(unsigned int /*cpu_id*/) {}
...
void IOInterruptController::timeStampInterruptHandlerInternal(
    bool isStart, IOInterruptVectorNumber vectorNumber,
    IOInterruptVector *vector) {
  uint64_t providerID = 0;
  ...
  if (isStart) {
    ...
    IOTimeStampStartConstant(IODBG_INTC(IOINTC_HANDLER),
                             (uintptr_t)vectorNumber, (uintptr_t)unslidHandler,
                             (uintptr_t)unslidTarget, (uintptr_t)providerID);
  } else {
    IOTimeStampEndConstant(IODBG_INTC(IOINTC_HANDLER), (uintptr_t)vectorNumber,
                           (uintptr_t)unslidHandler, (uintptr_t)unslidTarget,
...
void IOInterruptController::timeStampInterruptHandlerStart(
    IOInterruptVectorNumber vectorNumber, IOInterruptVector *vector) {
  timeStampInterruptHandlerInternal(true, vectorNumber, vector);
}

void IOInterruptController::timeStampInterruptHandlerEnd(
    IOInterruptVectorNumber vectorNumber, IOInterruptVector *vector) {
...
OSDefineMetaClassAndStructors(IOSharedInterruptController,
...
IOReturn IOSharedInterruptController::initInterruptController(
    IOInterruptController *parentController, OSData *parentSource) {
  ...
  numVectors =
      kIOSharedInterruptControllerDefaultVectors; // For now a constant number.
...
IOReturn IOSharedInterruptController::registerInterrupt(
...
IOReturn IOSharedInterruptController::unregisterInterrupt(IOService *nub,
  ...
  for (vectorNumber = 0;
    ...
    // Return success if it is not already registered
    if (!vector->interruptRegistered || (vector->nub != nub) ||
        (vector->source != source)) {
      IOLockUnlock(vector->interruptLock);
      continue;
    }
  ...
  for (vectorNumber = 0; vectorNumber < numVectors; vectorNumber++) {
    ...
    if (!vector->interruptDisabledSoft) {
      ...
      if (vector->interruptRegistered) {
        ...
        // Call handler.
        vector->handler(vector->target, vector->refCon, vector->nub,
                        vector->source);
