...
OSDefineMetaClassAndStructors(IODataQueue, OSObject)
    ...
    OSSharedPtr<IODataQueue> IODataQueue::withCapacity(UInt32 size) {
...
OSSharedPtr<IODataQueue> IODataQueue::withEntries(UInt32 numEntries,
                                                  UInt32 entrySize) {
  OSSharedPtr<IODataQueue> dataQueue = OSMakeShared<IODataQueue>();
...
Boolean IODataQueue::initWithEntries(UInt32 numEntries, UInt32 entrySize) {
  // Checking overflow for (numEntries + 1)*(entrySize +
  ...
  if ((entrySize > UINT32_MAX - DATA_QUEUE_ENTRY_HEADER_SIZE) ||
      ...
      (entrySize + DATA_QUEUE_ENTRY_HEADER_SIZE >
       UINT32_MAX / (numEntries + 1))) {
    return false;
  }

  return initWithCapacity((numEntries + 1) *
                          (DATA_QUEUE_ENTRY_HEADER_SIZE + entrySize));
}
...
Boolean IODataQueue::enqueue(void *data, UInt32 dataSize) {
  ...
  tail =
      __c11_atomic_load((_Atomic UInt32 *)&dataQueue->tail, __ATOMIC_RELAXED);
  head =
      __c11_atomic_load((_Atomic UInt32 *)&dataQueue->head, __ATOMIC_ACQUIRE);
  ...
  if (tail >= head) {
    ...
    if ((entrySize <= UINT32_MAX - tail) && ((tail + entrySize) <= queueSize)) {
      ...
      if ((queueSize - tail) >= DATA_QUEUE_ENTRY_HEADER_SIZE) {
        ((IODataQueueEntry *)((UInt8 *)dataQueue->queue + tail))->size =
            dataSize;
      }
  ...
  __c11_atomic_store((_Atomic UInt32 *)&dataQueue->tail, newTail,
                     __ATOMIC_RELEASE);
  ...
  if (tail != head) {
    ...
    head =
        __c11_atomic_load((_Atomic UInt32 *)&dataQueue->head, __ATOMIC_RELAXED);
  }
...
void IODataQueue::sendDataAvailableNotification() {
  ...
  if (msgh->msgh_remote_port) {
    kr = mach_msg_send_from_kernel_with_options(
        msgh, msgh->msgh_size, MACH64_SEND_TIMEOUT, MACH_MSG_TIMEOUT_NONE);
    switch (kr) {
...
OSSharedPtr<IOMemoryDescriptor> IODataQueue::getMemoryDescriptor() {
  ...
  if (dataQueue != NULL) {
    descriptor = IOMemoryDescriptor::withAddress(
        dataQueue, queueSize + DATA_QUEUE_MEMORY_HEADER_SIZE,
        kIODirectionOutIn);
  }
