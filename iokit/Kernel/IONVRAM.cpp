...
#define OP_RST                                                                 \
  ((1 << kIONVRAMOperationObliterate) | (1 << kIONVRAMOperationReset))
...
enum NVRAMVersion {
  ...
  kNVRAMVersion2, // V1 but with (0x2000 - sizeof(struct apple_nvram_header) -
                  // sizeof(struct chrp_nvram_header)) common region
...
UUID_DEFINE(gAppleSystemVariableGuid, 0x40, 0xA0, 0xDD, 0xD2, 0x77, 0xF8, 0x43,
            0x92, 0xB4, 0xA3, 0x1E, 0x73, 0x04, 0x20, 0x65, 0x16);

// Apple NVRAM Variable namespace (APPLE_VENDOR_OS_VARIABLE_GUID)
// 7C436110-AB2A-4BBB-A880-FE41995C9F82
UUID_DEFINE(gAppleNVRAMGuid, 0x7C, 0x43, 0x61, 0x10, 0xAB, 0x2A, 0x4B, 0xBB,
            0xA8, 0x80, 0xFE, 0x41, 0x99, 0x5C, 0x9F, 0x82);

// Wifi NVRAM namespace
// 36C28AB5-6566-4C50-9EBD-CBB920F83843
UUID_DEFINE(gAppleWifiGuid, 0x36, 0xC2, 0x8A, 0xB5, 0x65, 0x66, 0x4C, 0x50,
...
// "force-lock-bits" used by fwam over ssh nvram to device so they are unable to
// use entitlements "stress-rack" used in SEG stress-rack restore prdocs
#define IONVRAMSystemVariableListInternal                                      \
  IONVRAMSystemVariableList, "force-lock-bits", "stress-rack"

// allowlist variables from macboot that need to be set/get from system region
// if present
static const char *const gNVRAMSystemList[] = {IONVRAMSystemVariableList,
                                               nullptr};
static const char *const gNVRAMSystemListInternal[] = {
    IONVRAMSystemVariableListInternal, nullptr};
...
static const VariableTypeEntry gVariableTypes[] = {
...
static const VariablePermissionEntry gVariablePermissions[] = {
    ...
    {"fm-activation-locked", .p.Bits.RootRequired = 1,
     .p.Bits.NeverAllowedToDelete = 1},
    {"fm-spkeys", .p.Bits.RootRequired = 1, .p.Bits.NeverAllowedToDelete = 1},
    {"fm-spstatus", .p.Bits.RootRequired = 1, .p.Bits.NeverAllowedToDelete = 1},
    {"policy-nonce-digests",
     .p.Bits.ResetNVRAMOnlyDelete = 1}, // Deleting this via user triggered
                                        // obliterate leave J273a unable to boot
    ...
    {"testKernelOnly", .p.Bits.KernelOnly = 1, .p.Bits.TestingOnly = 1},
    {"testResetOnlyDel", .p.Bits.ResetNVRAMOnlyDelete = 1,
     .p.Bits.TestingOnly = 1},
    {"testNeverDel", .p.Bits.NeverAllowedToDelete = 1, .p.Bits.TestingOnly = 1},
    {"testUserWrite", .p.Bits.UserWrite = 1, .p.Bits.TestingOnly = 1},
    {"testRootReq", .p.Bits.RootRequired = 1, .p.Bits.TestingOnly = 1},
...
// variable-guid pair entries that require entitlement check to do specified
// nvram operations
static const VariableEntitlementEntry gVariableEntitlements[] = {
    {OP_MOD_RST, &gAppleNVRAMGuid, "ownership-warning",
     "com.apple.private.iokit.ddl-write"},
    {OP_MOD, &gAppleSystemVariableGuid, "BluetoothInfo",
     "com.apple.private.iokit.nvram-bluetooth"},
    {OP_MOD, &gAppleSystemVariableGuid, "BluetoothUHEDevices",
     "com.apple.private.iokit.nvram-bluetooth"},
    {OP_MOD, &gAppleNVRAMGuid, "bluetoothExternalDongleFailed",
     "com.apple.private.iokit.nvram-bluetooth"},
    {OP_MOD, &gAppleNVRAMGuid, "bluetoothInternalControllerInfo",
     "com.apple.private.iokit.nvram-bluetooth"},
    {OP_RD, &gAppleSystemVariableGuid, "current-network",
     "com.apple.private.security.nvram.wifi-psks"},
    {OP_RD, &gAppleWifiGuid, "current-network",
     "com.apple.private.security.nvram.wifi-psks"},
    {OP_RD, &gAppleSystemVariableGuid, "preferred-networks",
     "com.apple.private.security.nvram.wifi-psks"},
    {OP_RD, &gAppleWifiGuid, "preferred-networks",
     "com.apple.private.security.nvram.wifi-psks"},
    {OP_RD, &gAppleSystemVariableGuid, "preferred-count",
     "com.apple.private.security.nvram.wifi-psks"},
    {OP_RD, &gAppleWifiGuid, "preferred-count",
     "com.apple.private.security.nvram.wifi-psks"},
    {OP_MOD_RD, &gAppleSystemVariableGuid, "fmm-mobileme-token-FMM",
     "com.apple.private.security.nvram.fmm"},
    {OP_MOD_RD, &gAppleNVRAMGuid, "fmm-mobileme-token-FMM",
     "com.apple.private.security.nvram.fmm"},
    {OP_MOD_RD, &gAppleSystemVariableGuid,
     "fmm-mobileme-token-FMM-BridgeHasAccount",
     "com.apple.private.security.nvram.fmm"},
    {OP_MOD_RD, &gAppleNVRAMGuid, "fmm-mobileme-token-FMM-BridgeHasAccount",
     "com.apple.private.security.nvram.fmm"},
    {OP_MOD_RD, &gAppleSystemVariableGuid, "fmm-computer-name",
     "com.apple.private.security.nvram.fmm"},
    {OP_MOD_RD, &gAppleNVRAMGuid, "fmm-computer-name",
     "com.apple.private.security.nvram.fmm"},
    // Variables used for testing entitlement
    {OP_MOD_RST, &gAppleNVRAMGuid, "testEntModRst",
     "com.apple.private.iokit.testEntModRst"},
    {OP_MOD_RST, &gAppleSystemVariableGuid, "testEntModRstSys",
     "com.apple.private.iokit.testEntModRst"},
    {OP_RST, &gAppleNVRAMGuid, "testEntRst",
     "com.apple.private.iokit.testEntRst"},
    {OP_RST, &gAppleSystemVariableGuid, "testEntRstSys",
     "com.apple.private.iokit.testEntRst"},
    {OP_RD, &gAppleNVRAMGuid, "testEntRd", "com.apple.private.iokit.testEntRd"},
    {OP_RD, &gAppleSystemVariableGuid, "testEntRdSys",
     "com.apple.private.iokit.testEntRd"},
    {OP_DEL, &gAppleNVRAMGuid, "testEntDel",
     "com.apple.private.iokit.testEntDel"},
    {OP_DEL, &gAppleSystemVariableGuid, "testEntDelSys",
     "com.apple.private.iokit.testEntDel"},
    {0, &UUID_NULL, nullptr, nullptr}};

static NVRAMPartitionType getPartitionTypeForGUID(const uuid_t guid) {
...
static IONVRAMVariableType getVariableType(const char *propName) {
...
static VariablePermission getVariablePermission(const char *propName) {
...
static bool variableInAllowList(const char *varName) {
  unsigned int i = 0;
  const char *const *list =
      gInternalBuild ? gNVRAMSystemListInternal : gNVRAMSystemList;
...
static bool verifyWriteSizeLimit(const uuid_t varGuid, const char *variableName,
                                 size_t propDataSize) {
...
#if defined(DEBUG) || defined(DEVELOPMENT)
static const char *getNVRAMOpString(IONVRAMOperation op) {
  switch (op) {
 ...
 */
static bool parseVariableName(const char *key, uuid_t *guidResult,
  ...
  if (keyLen > sizeof(temp)) {
    ...
    if ((uuid_parse(temp, guid) == 0) && (key[sizeof(temp) - 1] == ':')) {
...
static bool parseVariableName(const OSSymbol *key, uuid_t *guidResult,
                              const char **nameResult) {
 ...
 */
static void translateGUID(const uuid_t varGuid, const char *variableName,
                          uuid_t destGuid, bool systemActive, bool log) {
...
/**
 * @brief Checks if the variable-guid(translated) pair is present in
 * gVariableEntitlements and if so, does it have the required entitlement for
 * the NVRAM operation passed in
 *
 * @param varGuid       guid for the variable to be checked, this gets
 * translated by translateGUID
 * @param varName       variable name
 * @param op            NVRAM operation
 * @param systemActive  used to pass into translateGUID to get the correct guid
 * to check against
 ...
 *                      entitlement check wasn't required for operation passed
 * in, or if entitlement check returned true
 * @return false        if varName/varGuid/veChecked was NULL or if entitlement
 * check returned false
 */
static bool verifyVarEntitlement(const uuid_t varGuid, const char *varName,
                                 IONVRAMOperation op, bool systemActive,
                                 bool *veChecked, bool log) {
  ...
  *veChecked = false;
  ...
  while ((entry != nullptr) && (entry->varName != nullptr)) {
    if ((strcmp(entry->varName, varName) == 0) &&
        (uuid_compare(translatedGuid, *(entry->varGuid)) == 0)) {
...
static bool kernelOnlyVar(const char *varName) {
  if (strncmp(varName, KERNEL_ONLY_VAR_NAME_PREFIX,
              sizeof(KERNEL_ONLY_VAR_NAME_PREFIX) - 1) == 0) {
    return true;
  }
...
static bool verifyPermission(IONVRAMOperation op, const uuid_t varGuid,
                             const char *varName, const bool systemActive,
                             bool log) {
  VariablePermission perm;
  bool kernel, varEntitled, writeEntitled = false, readEntitled = false,
                            allowList, systemGuid = false,
                            systemEntitled = false,
                            systemInternalEntitled = false, systemAllow,
                            systemReadHiddenAllow = false;
  bool admin = false;
  bool ok = false;

  if (verifyVarEntitlement(varGuid, varName, op, systemActive, &varEntitled,
                           log) == false) {
    goto exit;
  }
  ...
  admin = IOUserClient::clientHasPrivilege(current_task(), kIONVRAMPrivilege) ==
          kIOReturnSuccess;
  ...
  systemInternalEntitled =
      IOCurrentTaskHasEntitlement(kIONVRAMSystemInternalAllowKey);
  systemReadHiddenAllow =
      IOCurrentTaskHasEntitlement(kIONVRAMSystemHiddenAllowKey);

  systemAllow =
      systemEntitled || (systemInternalEntitled && gInternalBuild) || kernel;
    ...
    if (kernel || perm.Bits.UserWrite || admin || writeEntitled ||
      ...
      if (systemGuid) {
        if (allowList) {
          if (!systemAllow) {
            DEBUG_ERROR(
                "Allowed write to system region when NOT entitled for %s\n",
                varName);
          }
        } else if (varEntitled) {
          DEBUG_INFO_IF(log,
                        "Allowed write to system region using variable "
                        "specific entitlement for %s\n",
                        varName);
        } else if (!systemAllow) {
  ...
  case kIONVRAMOperationReset:
    if (perm.Bits.NeverAllowedToDelete) {
      ...
      break;
    } else if ((op == kIONVRAMOperationObliterate) &&
               perm.Bits.ResetNVRAMOnlyDelete) {
      DEBUG_INFO_IF(log, "Not allowed to obliterate %s\n", varName);
      break;
    } else if ((op == kIONVRAMOperationDelete) &&
               perm.Bits.ResetNVRAMOnlyDelete) {
    ...
    if (kernel || perm.Bits.UserWrite || admin || writeEntitled ||
      ...
      if (systemGuid) {
        if (allowList) {
          if (!systemAllow) {
            DEBUG_ERROR(
                "Allowed delete to system region when NOT entitled for %s\n",
                varName);
          }
        } else if (varEntitled) {
          DEBUG_INFO_IF(log,
                        "Allowed delete to system region using variable "
                        "specific entitlement for %s\n",
                        varName);
        } else if (!systemAllow) {
...
exit:
  DEBUG_INFO_IF(log,
                "Permission for %s of %s %s: I=%d R=%d kern=%d, adm=%d, wE=%d, "
                "rE=%d, sG=%d, sEd=%d, sIEd=%d, sRHA=%d, UW=%d, vE=%d\n",
                getNVRAMOpString(op), varName, ok ? "granted" : "denied",
                gInternalBuild, gRestoreBoot, kernel, admin, writeEntitled,
                readEntitled, systemGuid, systemEntitled,
                systemInternalEntitled, systemReadHiddenAllow,
                perm.Bits.UserWrite, varEntitled);

  return ok;
}

static bool verifyPermission(IONVRAMOperation op, const OSSymbol *canonicalKey,
...
static bool skipKey(const OSSymbol *aKey) {
  return aKey->isEqualTo(kIORegistryEntryAllowableSetPropertiesKey) ||
         ...
         aKey->isEqualTo(kIOBSDNameKey) || aKey->isEqualTo(kIOBSDNamesKey) ||
         aKey->isEqualTo(kIOBSDMajorKey) || aKey->isEqualTo(kIOBSDMinorKey) ||
...
static OSSharedPtr<const OSSymbol> keyWithGuidAndCString(const uuid_t guid,
                                                         const char *cstring) {
  size_t length;
  ...
  strlcpy(&canonicalString[sizeof(uuid_string_t)], cstring,
          length - sizeof(uuid_string_t));
...
static void dumpDict(OSSharedPtr<OSDictionary> dict) {
  const OSSymbol *key;
...
static void dumpData(const char *name, OSSharedPtr<OSData> propData) {
...
// **************************** private IOService based class for passing
// notifications to IODTNVRAM

class IODTNVRAMPlatformNotifier : public IOService {
OSDeclareDefaultStructors(IODTNVRAMPlatformNotifier) private
    : IODTNVRAM *_provider;
...
OSDefineMetaClassAndStructors(IODTNVRAMPlatformNotifier, IOService)
    ...
    bool IODTNVRAMPlatformNotifier::start(IOService *provider) {
...
static const OSSharedPtr<const OSSymbol> gIOHibernateStateKey =
    OSSymbol::withCString(kIOHibernateStateKey);

static uint32_t hibernateState(void) {
  OSSharedPtr<OSData> data =
      OSDynamicPtrCast<OSData>(IOService::getPMRootDomain()->copyProperty(
          gIOHibernateStateKey.get()->getCStringNoCopy()));
...
IOReturn IODTNVRAMPlatformNotifier::callPlatformFunction(
    const OSSymbol *functionName, bool waitForFunction, void *param1,
...
OSDeclareDefaultStructors(IODTNVRAMDiags) private : IODTNVRAM *_provider;
  IORWLock *_diagLock;
...
public:
  bool start(IOService *provider) APPLE_KEXT_OVERRIDE;
  void logVariable(NVRAMPartitionType region, IONVRAMOperation op,
                   const char *name, void *data);
};

OSDefineMetaClassAndStructors(IODTNVRAMDiags, IOService)

    bool IODTNVRAMDiags::start(IOService *provider) {
  OSSharedPtr<OSSerializer> serializer;
  ...
  serializer = OSSerializer::forTarget(
      this, OSMemberFunctionCast(OSSerializerCallback, this,
                                 &IODTNVRAMDiags::serializeStats));
...
void IODTNVRAMDiags::logVariable(NVRAMPartitionType region, IONVRAMOperation op,
                                 const char *name, void *data) {
  // "Stats"        : OSDictionary
  // - "XX:varName" : OSDictionary, XX is the region value prefix to distinguish
  // which dictionary the variable is in
  //   - "Init"     : OSBoolean True/present if variable present at
  //   initialization
  //   - "Read"     : OSNumber count
  ...
  existingEntry.reset(OSDynamicCast(OSDictionary, _stats->getObject(entryKey)),
                      OSRetain);
  ...
  if (opCountKey) {
    currentCount.reset(
        OSDynamicCast(OSNumber, existingEntry->getObject(opCountKey)),
        OSRetain);
...
bool IODTNVRAMDiags::serializeStats(void *, OSSerialize *serializer) {
  bool ok;
...
// private IOService based class for publishing distinct dictionary properties
// on for easy ioreg access since the serializeProperties call is overloaded and
// is used as variable access
class IODTNVRAMVariables : public IOService {
OSDeclareDefaultStructors(IODTNVRAMVariables) private : IODTNVRAM *_provider;
  uuid_t _guid;
  bool _systemActive;
...
public:
  ...
  copyProperty(const OSSymbol *aKey) const APPLE_KEXT_OVERRIDE;
...
OSDefineMetaClassAndStructors(IODTNVRAMVariables, IOService)
    ...
    bool IODTNVRAMVariables::init(const uuid_t guid, const bool systemActive) {
...
bool IODTNVRAMVariables::start(IOService *provider) {
  _provider = OSDynamicCast(IODTNVRAM, provider);
...
bool IODTNVRAMVariables::serializeProperties(OSSerialize *s) const {
  const OSSymbol *key;
  ...
  require_noerr_action(status, exit,
                       DEBUG_ERROR("Failed to get variable dictionary\n"));
...
OSPtr<OSObject> IODTNVRAMVariables::copyProperty(const OSSymbol *aKey) const {
  if (_provider && !skipKey(aKey)) {
    DEBUG_INFO("aKey=%s\n", aKey->getCStringNoCopy());

    return _provider->copyPropertyWithGUIDAndName(_guid,
                                                  aKey->getCStringNoCopy());
  } else {
    return nullptr;
  }
...
OSObject *IODTNVRAMVariables::getProperty(const OSSymbol *aKey) const {
  OSSharedPtr<OSObject> theObject = copyProperty(aKey);

  return theObject.get();
}

bool IODTNVRAMVariables::setProperty(const OSSymbol *aKey, OSObject *anObject) {
  if (_provider) {
    return _provider->setPropertyWithGUIDAndName(
               _guid, aKey->getCStringNoCopy(), anObject) == kIOReturnSuccess;
  } else {
    return false;
  }
}

IOReturn IODTNVRAMVariables::setProperties(OSObject *properties) {
  ...
  while (ret == kIOReturnSuccess) {
    ...
    ret = _provider->setPropertyWithGUIDAndName(_guid, key->getCStringNoCopy(),
                                                object);
  }
...
void IODTNVRAMVariables::removeProperty(const OSSymbol *aKey) {
...
public:
  virtual ~IODTNVRAMFormatHandler();
  virtual bool getNVRAMProperties(void);
  virtual IOReturn unserializeVariables(void) = 0;
  virtual IOReturn setVariable(const uuid_t varGuid, const char *variableName,
                               OSObject *object) = 0;
...
bool IODTNVRAMFormatHandler::getNVRAMProperties() {
  bool ok = false;
  ...
  require_action(
      prop, exit,
      DEBUG_ERROR("Unable to find %s property\n", kNVRAMBankSizeKey));
...
bool IODTNVRAM::start(IOService *provider) {
  ...
  // This is not an error we just need to successfully exit this function to
  // allow AppleEFIRuntime to proceed and take over operation
  ...
  // IODTNVRAM for the variable sets which will also update the system/common
  // services
  initImageFormat();
  ...
  if (_format->getSystemUsed()) {
    ...
    if (!_systemService ||
        !_systemService->init(gAppleSystemVariableGuid,
                              _format->getSystemPartitionActive())) {
...
no_system:
  ...
  if (!_commonService ||
      !_commonService->init(gAppleNVRAMGuid,
                            _format->getSystemPartitionActive())) {
...
void IODTNVRAM::initImageFormat(void) {
  OSSharedPtr<IORegistryEntry> entry;
  OSSharedPtr<OSObject> prop;
  ...
  require_action(size != 0, skip,
                 panic("NVRAM size is 0 bytes, possibly due to bad config with "
                       "iBoot + xnu mismatch"));
...
skip:
  if (IONVRAMV3Handler::isValidImage(image, size)) {
    ...
    require_action(_format, skip,
                   panic("IONVRAMCHRPHandler creation failed\n"));
  }

  status = _format->unserializeVariables();
  verify_noerr_action(
      status,
      DEBUG_ERROR("Failed to unserialize variables, status=%08x\n", status));

  status = _format->getVarDict(localVariables);
  verify_noerr_action(
      status,
      DEBUG_ERROR("Failed to get variable dictionary, status=%08x\n", status));
...
void IODTNVRAM::registerNVRAMController(IONVRAMController *controller) {
  DEBUG_INFO("setting controller\n");

  require_action(_format != nullptr, exit,
                 DEBUG_ERROR("Handler not initialized yet\n"));
...
bool IODTNVRAM::safeToSync(void) {
  ...
  if ((delta_secs > (_lastDeviceSync + MIN_SYNC_NOW_INTERVAL)) ||
      _freshInterval) {
...
IOReturn IODTNVRAM::syncInternal(bool rateLimit) {
  IOReturn ret = kIOReturnSuccess;
  ...
  require_action(
      _format != nullptr, exit,
      (ret = kIOReturnNotReady, DEBUG_ERROR("Handler not initialized yet\n")));
  ...
  record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_NVRAM,
                      "sync", "triggered");

  ret = _format->sync();

  record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_NVRAM,
                      "sync", "completed with ret=%08x", ret);

  if (_diags) {
    OSSharedPtr<OSNumber> generation =
        OSNumber::withNumber(_format->getGeneration(), 32);
...
IOReturn IODTNVRAM::sync(void) { return syncInternal(false); }

void IODTNVRAM::reload(void) { _format->reload(); }

IOReturn IODTNVRAM::getVarDict(OSSharedPtr<OSDictionary> &varDictCopy) {
  return _format->getVarDict(varDictCopy);
}

OSPtr<OSDictionary> IODTNVRAM::dictionaryWithProperties(void) const {
  ...
  require_action(returnDict, exit,
                 DEBUG_ERROR("Failed to create return dict\n"));
  ...
  while ((canonicalKey = OSDynamicCast(OSSymbol, iter->getNextObject()))) {
    ...
    if ((uuid_compare(varGuid, gAppleSystemVariableGuid) == 0) &&
        verifyPermission(kIONVRAMOperationRead, varGuid, varName,
                         _format->getSystemPartitionActive(), false)) {
      OSSharedPtr<const OSSymbol> returnKey = OSSymbol::withCString(varName);
      returnDict->setObject(returnKey.get(),
                            localVarDict->getObject(canonicalKey));
    }
  }
  ...
  while ((canonicalKey = OSDynamicCast(OSSymbol, iter->getNextObject()))) {
    ...
    if (uuid_compare(varGuid, gAppleNVRAMGuid) == 0) {
      ...
      if (verifyPermission(kIONVRAMOperationRead, varGuid, varName,
                           _format->getSystemPartitionActive(), false)) {
        OSSharedPtr<const OSSymbol> returnKey = OSSymbol::withCString(varName);
        returnDict->setObject(returnKey.get(),
                              localVarDict->getObject(canonicalKey));
...
IOReturn IODTNVRAM::flushGUID(const uuid_t guid, IONVRAMOperation op) {
  IOReturn ret = kIOReturnSuccess;
  ...
  if (_format->getSystemPartitionActive() &&
      (uuid_compare(guid, gAppleSystemVariableGuid) == 0)) {
...
bool IODTNVRAM::handleSpecialVariables(const char *name, const uuid_t guid,
                                       const OSObject *obj, IOReturn *error) {
  IOReturn ret = kIOReturnSuccess;
  bool special = false;
...
IODTNVRAM::copyPropertyWithGUIDAndName(const uuid_t guid,
  ...
  if (!verifyPermission(kIONVRAMOperationRead, guid, name,
                        _format->getSystemPartitionActive(), true)) {
  ...
  if (_diags) {
    _diags->logVariable(getPartitionTypeForGUID(newGuid), kIONVRAMOperationRead,
                        name, NULL);
  }
...
OSSharedPtr<OSObject> IODTNVRAM::copyProperty(const OSSymbol *aKey) const {
...
OSSharedPtr<OSObject> IODTNVRAM::copyProperty(const char *aKey) const {
...
IOReturn IODTNVRAM::clearTestVars(const uuid_t guid) {
  const VariablePermissionEntry *entry;
  IOReturn ret = kIOReturnSuccess;
  uuid_t newGuid;
  ...
  while ((entry != nullptr) && (entry->name != nullptr)) {
    if (entry->p.Bits.TestingOnly) {
      translateGUID(guid, entry->name, newGuid,
                    _format->getSystemPartitionActive(), true);
...
IOReturn IODTNVRAM::setPropertyWithGUIDAndName(const uuid_t guid,
                                               const char *name,
                                               OSObject *anObject) {
  IOReturn ret = kIOReturnSuccess;
  bool remove = false;
  ...
  if (strncmp(name, kNVRAMClearTestVarKey, sizeof(kNVRAMClearTestVarKey)) ==
      0) {
    ret = clearTestVars(guid);
    goto exit;
  }

  deletePropertyKey = strncmp(name, kIONVRAMDeletePropertyKey,
                              sizeof(kIONVRAMDeletePropertyKey)) == 0;
  deletePropertyKeyWRet = strncmp(name, kIONVRAMDeletePropertyKeyWRet,
                                  sizeof(kIONVRAMDeletePropertyKeyWRet)) == 0;
  syncNowPropertyKey = strncmp(name, kIONVRAMSyncNowPropertyKey,
                               sizeof(kIONVRAMSyncNowPropertyKey)) == 0;
  forceSyncNowPropertyKey =
      strncmp(name, kIONVRAMForceSyncNowPropertyKey,
              sizeof(kIONVRAMForceSyncNowPropertyKey)) == 0;
  ...
  if (deletePropertyKey || deletePropertyKeyWRet) {
    ...
    if (tmpString != nullptr) {
      ...
      guidProvided = parseVariableName(tmpString->getCStringNoCopy(),
                                       &valueVarGuid, &variableName);
      ...
      // that will come into this function as (gAppleNVRAMGuid, varname,
      // nullptr) if we provide the "-z" flag to the nvram tool this function
      // will come in as (gAppleSystemVariableGuid, varname, nullptr). We are
      // reparsing the value string, if there is a GUID provided with the value
      // then use that GUID otherwise use the guid that was provided via the
      // node selection or default.
      ...
      DEBUG_INFO("%s found, removeRet=%#08x\n",
                 deletePropertyKeyWRet ? "deletePropertyKeyWRet"
                                       : "deletePropertyKey",
                 removeRet);
    } else {
    ...
    if (tmpString != nullptr) {
      // We still want to throttle NVRAM commit rate for SyncNow. ForceSyncNow
      // is provided as a really big hammer.
  ...
  case kOFVariableTypeBoolean:
    ...
    if (propObject) {
      record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_NVRAM,
                          "write", "%s as bool to %d", name,
                          ((OSBoolean *)propObject.get())->getValue());
    }
    break;
  ...
  case kOFVariableTypeNumber:
    ...
    if (propObject) {
      record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_NVRAM,
                          "write", "%s as number to %#llx", name,
                          ((OSNumber *)propObject.get())->unsigned64BitValue());
    }
    break;
  ...
  case kOFVariableTypeString:
    ...
    if (propObject != nullptr) {
      record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_NVRAM,
                          "write", "%s as string to %s", name,
                          ((OSString *)propObject.get())->getCStringNoCopy());

      propDataSize = (OSDynamicPtrCast<OSString>(propObject))->getLength();

      if ((strncmp(name, kIONVRAMBootArgsKey, sizeof(kIONVRAMBootArgsKey)) ==
           0) &&
          (propDataSize >= BOOT_LINE_LENGTH)) {
        DEBUG_ERROR(
  ...
  case kOFVariableTypeData:
    ...
    if (propObject != nullptr) {
      ...
      record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_NVRAM,
                          "write", "%s as data with size %zu", name,
                          propDataSize);
    ...
    if ((propObject != nullptr) &&
        ((OSDynamicPtrCast<OSData>(propObject))->getLength() == 0)) {
      remove = true;
    }
...
IOReturn IODTNVRAM::setPropertyInternal(const OSSymbol *aKey,
                                        OSObject *anObject) {
  const char *variableName;
  uuid_t varGuid;
...
IOReturn IODTNVRAM::removePropertyWithGUIDAndName(const uuid_t guid,
                                                  const char *name) {
  IOReturn ret;
  uuid_t newGuid;
...
IOReturn IODTNVRAM::removePropertyInternal(const OSSymbol *aKey) {
  IOReturn ret;
  const char *variableName;
  uuid_t varGuid;
...
IOReturn IODTNVRAM::readXPRAM(IOByteCount offset, uint8_t *buffer,
                              IOByteCount length) {
  return kIOReturnUnsupported;
}

IOReturn IODTNVRAM::writeXPRAM(IOByteCount offset, uint8_t *buffer,
                               IOByteCount length) {
  return kIOReturnUnsupported;
}
...
OSDictionary *IODTNVRAM::getNVRAMPartitions(void) { return NULL; }
...
IOByteCount IODTNVRAM::savePanicInfo(uint8_t *buffer, IOByteCount length) {
  return 0;
}
