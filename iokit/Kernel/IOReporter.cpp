...
/***         STATIC METHODS         ***/
/**************************************/
IOReturn IOReporter::configureAllReports(OSSet *reporters,
...
IOReturn IOReporter::updateAllReports(OSSet *reporters,
                                      IOReportChannelList *channelList,
                                      IOReportConfigureAction action,
                                      void *result, void *destination) {
  IOReturn rval = kIOReturnError;
...
bool IOReporter::init(IOService *reportingService,
...
void IOReporter::initialize(void) {
 ...
 *  } while (0);
 */
IOReturn IOReporter::addChannel(uint64_t channelID,
...
OSSharedPtr<IOReportLegendEntry> IOReporter::createLegend(void) {
...
IOReturn IOReporter::configureReport(IOReportChannelList *channelList,
                                     IOReportConfigureAction action,
                                     void *result, void *destination) {
...
IOReturn IOReporter::updateReport(IOReportChannelList *channelList,
...
IOReturn IOReporter::handleAddChannelSwap(uint64_t channel_id,
  ...
  if (_elements) {
    ...
    memcpy(_swapEnableCounts, _enableCounts, (size_t)_nChannels * sizeof(int));
  }
...
void IOReporter::handleSwapCleanup(int swapNChannels) {
  int swapNElements;
...
IOReturn IOReporter::handleConfigureReport(IOReportChannelList *channelList,
                                           IOReportConfigureAction action,
                                           void *result, void *destination) {
  IOReturn res = kIOReturnError;
  int channel_index = 0;
  ...
  for (chIdx = 0; chIdx < channelList->nchannels; chIdx++) {
    if (getChannelIndex(channelList->channels[chIdx].channel_id,
      ...
      // IORLOG("reporter %p recognizes channel %lld", this,
...
IOReturn IOReporter::handleUpdateReport(IOReportChannelList *channelList,
                                        IOReportConfigureAction action,
                                        void *result, void *destination) {
  IOReturn res = kIOReturnError;
...
OSSharedPtr<IOReportLegendEntry> IOReporter::handleCreateLegend(void) {
  ...
  if (channelIDs) {
    legendEntry = IOReporter::legendWith(channelIDs.get(), _channelNames.get(),
                                         _channelType, _unit);
  }
...
IOReturn IOReporter::setElementValues(int element_index,
  ...
  memcpy(&_elements[element_index].values, values,
...
const IOReportElementValues *IOReporter::getElementValues(int element_index) {
...
IOReturn IOReporter::updateChannelValues(int channel_index) {
  return kIOReturnSuccess;
}

IOReturn
IOReporter::updateReportChannel(int channel_index, int *nElements,
  ...
  if (size2cpy > (destination->getCapacity() - destination->getLength())) {
    IORLOG("CRITICAL ERROR: Report Buffer Overflow (buffer cap %luB, length "
           "%luB, size2cpy %luB",
           (unsigned long)destination->getCapacity(),
           (unsigned long)destination->getLength(), (unsigned long)size2cpy);
    goto finish;
  }
...
IOReturn IOReporter::copyElementValues(int element_index,
  ...
  memcpy(elementValues, &_elements[element_index].values,
         sizeof(IOReportElementValues));
  res = kIOReturnSuccess;
...
IOReturn IOReporter::getFirstElementIndex(uint64_t channel_id, int *index) {
  IOReturn res = kIOReturnError;
...
IOReturn IOReporter::getChannelIndex(uint64_t channel_id, int *index) {
  IOReturn res = kIOReturnError;
  int channel_index = 0, element_index = 0;
...
IOReturn IOReporter::getChannelIndices(uint64_t channel_id, int *channel_index,
                                       int *element_index) {
...
OSSharedPtr<OSArray> IOReporter::copyChannelIDs() {
  int cnt, cnt2;
  OSSharedPtr<OSArray> channelIDs;
...
IOReporter::legendWith(OSArray *channelIDs, OSArray *channelNames,
                       IOReportChannelType channelType, IOReportUnit unit) {
  ...
  for (cnt = 0; cnt < chCnt; cnt++) {
    ...
    // Encapsulate the Channel ID in OSNumber
    tmpChannelArray->setObject(kIOReportChannelIDIdx,
                               channelIDs->getObject(cnt));
  ...
  if (channelLegendArray->getCount() != 0) {
    ...
    if (legendEntry) {
      legendEntry->setObject(kIOReportLegendChannelsKey,
                             channelLegendArray.get());
...
/*static */ OSPtr<IOReportLegendEntry>
IOReporter::legendWith(const uint64_t *channelIDs, const char **channelNames,
                       int channelCount, IOReportChannelType channelType,
                       IOReportUnit unit) {
  ...
  return legendWith(channelIDsArray.get(), channelNamesArray.get(), channelType,
                    unit);
}
