...
static size_t nvram_v3_var_container_size(const struct v3_var_header *header) {
  return sizeof(struct nvram_v3_var_entry) + header->nameSize +
         header->dataSize;
}

static size_t variable_length(const struct v3_var_header *header) {
  return sizeof(struct v3_var_header) + header->nameSize + header->dataSize;
}

static bool valid_store_header(const struct v3_store_header *header) {
  return (header->name == VARIABLE_STORE_SIGNATURE) &&
         (header->version == VARIABLE_STORE_VERSION);
}

static bool valid_variable_header(const struct v3_var_header *header,
                                  size_t buf_len) {
...
static uint32_t find_active_var_in_image(const struct v3_var_header *var,
                                         const uint8_t *image, uint32_t offset,
                                         uint32_t len) {
  ...
  while ((offset + sizeof(struct v3_var_header) < len)) {
    ...
    if (valid_variable_header(store_var, len - offset)) {
      if ((store_var->state == VAR_ADDED) &&
          ...
          (memcmp(var->name_data_buf, store_var->name_data_buf,
                  var->nameSize) == 0)) {
        var_offset = offset;
        break;
      }
...
static IOReturn find_current_offset_in_image(const uint8_t *image, uint32_t len,
                                             uint32_t *newOffset) {
  uint32_t offset = 0;
  ...
  while (offset < len) {
    const struct v3_var_header *store_var =
        (const struct v3_var_header *)(image + offset);
    uuid_string_t uuidString;

    if (valid_variable_header(store_var, len - offset)) {
      uuid_unparse(store_var->guid, uuidString);
      DEBUG_INFO("Valid var @ %#08x, state=%#02x, length=%#08zx, %s:%s\n",
                 offset, store_var->state, variable_length(store_var),
                 uuidString, store_var->name_data_buf);
      offset += variable_length(store_var);
    } else {
      break;
    }
...
class IONVRAMV3Handler : public IODTNVRAMFormatHandler,
                         IOTypedOperatorsMixin<IONVRAMV3Handler> {
private:
  ...
  static bool convertObjectToProp(uint8_t *buffer, uint32_t *length,
                                  const char *propSymbol, OSObject *propObject);
  static bool convertPropToObject(const uint8_t *propName,
                                  uint32_t propNameLength,
                                  const uint8_t *propData,
                                  uint32_t propDataLength,
                                  OSSharedPtr<const OSSymbol> &propSymbol,
                                  OSSharedPtr<OSObject> &propObject);

  IOReturn reloadInternal(void);
  IOReturn setVariableInternal(const uuid_t varGuid, const char *variableName,
                               OSObject *object);

  void setEntryForRemove(struct nvram_v3_var_entry *v3Entry, bool system);
  void findExistingEntry(const uuid_t varGuid, const char *varName,
                         struct nvram_v3_var_entry **existing,
...
public:
  virtual ~IONVRAMV3Handler() APPLE_KEXT_OVERRIDE;
  IONVRAMV3Handler();
  static bool isValidImage(const uint8_t *image, IOByteCount length);
  static IONVRAMV3Handler *init(IODTNVRAM *provider, const uint8_t *image,
                                IOByteCount length);
  ...
  virtual IOReturn setVariable(const uuid_t varGuid, const char *variableName,
                               OSObject *object) APPLE_KEXT_OVERRIDE;
  ...
  virtual IOReturn flush(const uuid_t guid,
                         IONVRAMOperation op) APPLE_KEXT_OVERRIDE;
  ...
  virtual IOReturn
  getVarDict(OSSharedPtr<OSDictionary> &varDictCopy) APPLE_KEXT_OVERRIDE;
};
...
bool IONVRAMV3Handler::isValidImage(const uint8_t *image, IOByteCount length) {
...
IONVRAMV3Handler *IONVRAMV3Handler::init(IODTNVRAM *provider,
                                         const uint8_t *image,
                                         IOByteCount length) {
  ...
  require_action(propertiesOk, exit,
                 DEBUG_ERROR("Unable to get NVRAM properties\n"));

  require_action(length == handler->_bankSize, exit,
                 DEBUG_ERROR("length %#llx != _bankSize %#x\n", length,
                             handler->_bankSize));
...
bool IONVRAMV3Handler::getNVRAMProperties() {
  bool ok = false;
  ...
  require_action(IODTNVRAMFormatHandler::getNVRAMProperties(), exit,
                 DEBUG_ERROR("parent getNVRAMProperties failed\n"));

  entry = IORegistryEntry::fromPath("/chosen", gIODTPlane);
  require_action(entry, exit, DEBUG_ERROR("Unable to find chosen node\n"));

  prop = entry->copyProperty(rawControllerKey);
  require_action(prop != nullptr, exit,
                 DEBUG_ERROR("No %s entry\n", rawControllerKey));
...
IOReturn IONVRAMV3Handler::flush(const uuid_t guid, IONVRAMOperation op) {
  IOReturn ret = kIOReturnSuccess;
  bool flushSystem;
  bool flushCommon;

  flushSystem = getSystemPartitionActive() &&
  ...
  if (flushSystem || flushCommon) {
    ...
    while ((canonicalKey = OSDynamicCast(OSSymbol, iter->getNextObject()))) {
      ...
      clear = ((flushSystem &&
                (uuid_compare(varGuid, gAppleSystemVariableGuid) == 0)) ||
               (flushCommon &&
                (uuid_compare(varGuid, gAppleSystemVariableGuid) != 0))) &&
...
IOReturn IONVRAMV3Handler::reloadInternal(void) {
  IOReturn ret;
  ...
  require_action(isValidImage(controllerImage, _bankSize), exit,
                 (ret = kIOReturnInvalid,
                  DEBUG_ERROR("Invalid image at bank %d\n", _currentBank)));
  ...
  // We must sync any existing variables offset on the controller image with our
  // internal representation If we find an existing entry and the data is still
  // the same we record the existing offset and mark it as VAR_NEW_STATE_NONE
  // meaning no action needed Otherwise if the data is different or it is not
  // found on the controller image we mark it as VAR_NEW_STATE_APPEND which will
  // have us invalidate the existing entry if there is one and append it on the
  // next save
  ...
  for (unsigned int i = 0; i < _varEntries->getCount(); i++) {
    ...
    while ((latestOffset = find_active_var_in_image(
                &v3Entry->header, controllerImage, offset, _bankSize))) {
      ...
      if (prevOffset) {
        ...
        struct v3_var_header *prevVarHeader =
            (struct v3_var_header *)(controllerImage + prevOffset);
        uint8_t state =
            prevVarHeader->state & VAR_DELETED & VAR_IN_DELETED_TRANSITION;

        ret = _nvramController->write(prevOffset +
                                          offsetof(struct v3_var_header, state),
                                      &state, sizeof(state));
        require_noerr_action(
            ret, unlock, DEBUG_ERROR("existing state w fail, ret=%#x\n", ret));
      }
    ...
    if (v3Entry->existing_offset == 0) {
      ...
      storeVar = (const struct v3_var_header
                      *)&controllerImage[v3Entry->existing_offset];

      if (v3Entry->new_state != VAR_NEW_STATE_REMOVE) {
        // Verify that the existing data matches the store data
        if ((variable_length(&v3Entry->header) == variable_length(storeVar)) &&
            (memcmp(v3Entry->header.name_data_buf, storeVar->name_data_buf,
                    storeVar->nameSize + storeVar->dataSize) == 0)) {
          DEBUG_INFO("Store var data for %s matches, marking new state none\n",
                     v3Entry->header.name_data_buf);
          v3Entry->new_state = VAR_NEW_STATE_NONE;
        } else {
          DEBUG_INFO(
              "Store var data for %s differs, marking new state append\n",
              v3Entry->header.name_data_buf);
          v3Entry->new_state = VAR_NEW_STATE_APPEND;
        }
      } else {
        // Store has entry but it has been removed from our collection, keep it
        // marked for delete but with updated existing_offset for coherence
  ...
  ret =
      find_current_offset_in_image(controllerImage, _bankSize, &_currentOffset);
  require_noerr_action(ret, unlock,
                       DEBUG_ERROR("Unidentified bytes in image\n"));
...
void IONVRAMV3Handler::reload(void) {
  _reload = true;

  DEBUG_INFO("reload marked\n");
}

void IONVRAMV3Handler::setEntryForRemove(struct nvram_v3_var_entry *v3Entry,
                                         bool system) {
  ...
  require_action(v3Entry != nullptr, exit,
                 DEBUG_INFO("remove with no entry\n"));
  ...
  if (v3Entry->new_state == VAR_NEW_STATE_REMOVE) {
    ...
    if (_provider->_diags) {
      _provider->_diags->logVariable(
          getPartitionTypeForGUID(v3Entry->header.guid),
          kIONVRAMOperationDelete, variableName, nullptr);
    }
  }
...
void IONVRAMV3Handler::findExistingEntry(const uuid_t varGuid,
                                         const char *varName,
                                         struct nvram_v3_var_entry **existing,
                                         unsigned int *existingIndex) {
    ...
    if ((v3Entry->header.nameSize == nameLen) &&
      ...
      if (varGuid) {
        if (uuid_compare(varGuid, v3Entry->header.guid) == 0) {
          uuid_string_t uuidString;
          uuid_unparse(varGuid, uuidString);
          DEBUG_INFO("found existing entry for %s:%s, e_off=%#lx, len=%#lx, "
                     "new_state=%#x\n",
                     uuidString, varName, v3Entry->existing_offset,
                     variable_length(&v3Entry->header), v3Entry->new_state);
          break;
        }
      } else {
        DEBUG_INFO("found existing entry for %s, e_off=%#lx, len=%#lx\n",
                   varName, v3Entry->existing_offset,
                   variable_length(&v3Entry->header));
        break;
      }
    }
...
IOReturn IONVRAMV3Handler::unserializeImage(const uint8_t *image,
                                            IOByteCount length) {
...
static ephDMAllowListEntry ephDMEntries[] = {
...
IOReturn IONVRAMV3Handler::handleEphDM(void) {
  OSSharedPtr<IORegistryEntry> entry;
  OSData *data;
  OSSharedPtr<OSObject> prop;
  uint32_t ephDM = 0;
  ...
  require_action(ephDM != 0, exit,
                 DEBUG_ALWAYS("ephemeral-data-mode not supported\n"));
  require_action(_systemSize != 0, exit,
                 DEBUG_ALWAYS("No system region, no need to clear\n"));

  if (PE_parse_boot_argn("epdm-skip-nvram", &skip, sizeof(skip))) {
    require_action(!(gInternalBuild && (skip == 1)), exit,
                   DEBUG_ALWAYS("Internal build + epdm-skip-nvram set to true, "
                                "skip nvram clearing\n"));
  }
  ...
  for (uint32_t entry = 0; entry < ARRAY_SIZE(ephDMEntries); entry++) {
    canonicalKey =
        keyWithGuidAndCString(gAppleNVRAMGuid, ephDMEntries[entry].name);
    ephDMEntries[entry].value.reset(
        OSDynamicCast(OSData, _varDict->getObject(canonicalKey.get())),
        OSRetain);
  }
  ...
  require_noerr_action(
      ret, exit,
      DEBUG_ERROR("Flushing common region failed, ret=%#08x\n", ret));
  ...
  for (uint32_t entry = 0; entry < ARRAY_SIZE(ephDMEntries); entry++) {
    ...
    ret = setVariableInternal(gAppleNVRAMGuid, ephDMEntries[entry].name,
                              ephDMEntries[entry].value.get());
    require_noerr_action(
        ret, exit,
        DEBUG_ERROR("Setting allowlist variable %s failed, ret=%#08x\n",
...
IOReturn IONVRAMV3Handler::unserializeVariables(void) {
  IOReturn ret = kIOReturnSuccess;
  OSSharedPtr<const OSSymbol> propSymbol;
  OSSharedPtr<OSObject> propObject;
  ...
  while ((offset + sizeof(struct v3_var_header)) < _bankSize) {
    ...
    DEBUG_INFO("Valid var @ %#08zx, state=%#02x, length=%#08zx, %s:%s\n",
               offset, header->state, variable_length(header), uuidString,
    ...
    v3Entry = (struct nvram_v3_var_entry *)IOMallocZeroData(
        nvram_v3_var_container_size(header));
    __nochk_memcpy(&v3Entry->header, _nvramImage + offset,
                   variable_length(header));

    // It is assumed that the initial image being unserialized here is going to
    // be the proxy data from EDT and not the image read from the controller,
    // which for various reasons due to the setting of states and saves from
    // iBoot, can be different. We will have an initial existing_offset of 0 and
    // once the controller is set we will read out the image there and update
    // the existing offset with what is present on the NOR image
    v3Entry->existing_offset = 0;
    v3Entry->new_state = VAR_NEW_STATE_NONE;

    // safe guard for any strange duplicate entries in the store
    findExistingEntry(v3Entry->header.guid,
                      (const char *)v3Entry->header.name_data_buf,
                      &existingEntry, &existingIndex);

    if (existingEntry != nullptr) {
      existingSize = variable_length(&existingEntry->header);

      entryContainer = OSData::withBytes(
          v3Entry, (uint32_t)nvram_v3_var_container_size(header));
    ...
    system =
        (_systemSize != 0) &&
        (uuid_compare(v3Entry->header.guid, gAppleSystemVariableGuid) == 0);
    if (system) {
      _systemUsed = _systemUsed + (uint32_t)variable_length(header) -
                    (uint32_t)existingSize;
    } else {
      _commonUsed = _commonUsed + (uint32_t)variable_length(header) -
                    (uint32_t)existingSize;
    }

    if (convertPropToObject(
            v3Entry->header.name_data_buf, v3Entry->header.nameSize,
            v3Entry->header.name_data_buf + v3Entry->header.nameSize,
            v3Entry->header.dataSize, propSymbol, propObject)) {
      OSSharedPtr<const OSSymbol> canonicalKey = keyWithGuidAndCString(
          v3Entry->header.guid, (const char *)v3Entry->header.name_data_buf);

      DEBUG_INFO("adding %s, dataLength=%u, system=%d\n",
                 canonicalKey->getCStringNoCopy(), v3Entry->header.dataSize,
                 system);
...
IOReturn IONVRAMV3Handler::setVariableInternal(const uuid_t varGuid,
                                               const char *variableName,
                                               OSObject *object) {
  struct nvram_v3_var_entry *v3Entry = nullptr;
  ...
  // Anyone calling setVariableInternal should've already held the lock for
  // write.
  NVRAMRWLOCKASSERTEXCLUSIVE(_variableLock);
  ...
  DEBUG_INFO("setting %s:%s, system=%d, current var count=%u\n", uuidString,
             variableName, system, _varEntries->getCount());
  ...
  if (unset == true) {
    ...
    } else if (_commonUsed - existingVariableSize + newVariableSize >
               _commonSize) {
    ...
    DEBUG_INFO("creating new entry for %s, existingVariableSize=%#zx, "
               "newVariableSize=%#zx\n",
    ...
    convertObjectToProp(newV3Entry->header.name_data_buf + entryNameLen,
                        &dataSize, variableName, object);
    ...
    newV3Entry->header.crc =
        crc32(0, newV3Entry->header.name_data_buf + entryNameLen, dataSize);
    ...
    if (system) {
      _systemUsed = _systemUsed + (uint32_t)newVariableSize -
                    (uint32_t)existingVariableSize;
    } else {
      _commonUsed = _commonUsed + (uint32_t)newVariableSize -
                    (uint32_t)existingVariableSize;
    }
...
IOReturn IONVRAMV3Handler::setVariable(const uuid_t varGuid,
                                       const char *variableName,
                                       OSObject *object) {
  uuid_t destGuid;
  ...
  if (getSystemPartitionActive()) {
    // System region case, if they're using the GUID directly or it's on the
    // system allow list force it to use the System GUID
    if ((uuid_compare(varGuid, gAppleSystemVariableGuid) == 0) ||
        variableInAllowList(variableName)) {
...
uint32_t IONVRAMV3Handler::findCurrentBank(void) {
  ...
  for (unsigned int i = 0; i < _bankCount; i++) {
    ...
    if (valid_store_header(&storeHeader) &&
        (storeHeader.generation >= maxGen)) {
      currentBank = i;
...
bool IONVRAMV3Handler::setController(IONVRAMController *controller) {
  IOReturn ret = kIOReturnSuccess;
...
reclaim:
  ...
  require_noerr_action(
      ret, exit,
      DEBUG_ERROR(
          "Reclaim recovery failed, invalid controller state!!! ret=%#x\n",
          ret));
exit:
  NVRAMUNLOCK(_controllerLock);
...
IOReturn IONVRAMV3Handler::reclaim(void) {
  IOReturn ret;
  ...
  verify_noerr_action(ret,
                      DEBUG_INFO("select of bank %#08x failed\n", next_bank));
  ...
  for (unsigned int i = 0; i < _varEntries->getCount(); i++) {
    ...
    DEBUG_INFO("entry %u %s, new_state=%#x, e_offset=%#lx, state=%#x\n", i,
               varEntry->header.name_data_buf, varEntry->new_state,
               varEntry->existing_offset, varHeader->state);

    if ((varEntry->new_state == VAR_NEW_STATE_NONE) ||
        (varEntry->new_state == VAR_NEW_STATE_APPEND)) {
      varHeader->state = VAR_ADDED;

      memcpy(bankData + new_bank_offset, (uint8_t *)varHeader,
             variable_length(varHeader));
  ...
  require_noerr_action(
      ret, unlock, DEBUG_ERROR("reclaim bank write failed, ret=%08x\n", ret));

  _currentOffset = (uint32_t)new_bank_offset;

  DEBUG_INFO(
      "Reclaim complete, _currentBank=%u _generation=%u, _currentOffset=%#x\n",
...
size_t IONVRAMV3Handler::getAppendSize(void) {
  struct nvram_v3_var_entry *varEntry;
  struct v3_var_header *varHeader;
  OSData *entryContainer;
  size_t appendSize = 0;
...
IOReturn IONVRAMV3Handler::syncRaw(void) {
  IOReturn ret = kIOReturnSuccess;
  struct nvram_v3_var_entry *varEntry;
  struct v3_var_header *varHeader;
  OSData *entryContainer;
  ...
  require_action(_nvramController != nullptr, exit,
                 DEBUG_INFO("No _nvramController\n"));
  ...
  if (getAppendSize() + _currentOffset < _bankSize) {
    ...
    invalidateOffsets =
        (size_t *)IOMallocData(invalidateOffsetsCount * sizeof(size_t));
    ...
    for (unsigned int i = 0; i < _varEntries->getCount(); i++) {
      ...
      DEBUG_INFO(
          "entry %s, new_state=%#02x state=%#02x, existing_offset=%#zx\n",
          varEntry->header.name_data_buf, varEntry->new_state,
          varEntry->header.state, varEntry->existing_offset);
      ...
      if (varEntry->new_state == VAR_NEW_STATE_APPEND) {
        ...
        DEBUG_INFO("Appending %s in append buffer offset %#zx, actual offset "
                   "%#zx, prevOffset %#zx, varsize=%#zx\n",
                   varEntry->header.name_data_buf, appendBufferOffset,
                   varEntry->existing_offset, prevOffset, varSize);

        // Write to append buffer
        memcpy(appendBuffer + appendBufferOffset, (uint8_t *)varHeader,
               varSize);
        ...
        if (prevOffset) {
          ...
          invalidatedSize +=
              variable_length((struct v3_var_header *)prevOffset);
        }
        ...
        if (varEntry->existing_offset) {
          ...
          invalidateOffsets[invalidateOffsetIndex++] =
              varEntry->existing_offset;
          invalidatedSize += variable_length(
              (struct v3_var_header *)varEntry->existing_offset);
        } else {
    ...
    if (appendBufferOffset > 0) {
      ...
      ret = _nvramController->write(_currentOffset, appendBuffer,
                                    appendBufferOffset);
      require_noerr_action(ret, unlock,
                           DEBUG_ERROR("could not re-append, ret=%#x\n", ret));
    ...
    if (invalidateOffsetIndex > 0) {
      ...
      for (unsigned int i = 0; i < invalidateOffsetIndex; i++) {
        ...
        ret = _nvramController->write(invalidateOffsets[i] +
                                          offsetof(struct v3_var_header, state),
                                      &state, sizeof(state));
        require_noerr_action(
            ret, unlock,
            DEBUG_ERROR("unable to invalidate at offset %#zx, ret=%#x\n",
...
IOReturn IONVRAMV3Handler::syncBlock(void) {
  IOReturn ret = kIOReturnSuccess;
  struct v3_store_header newStoreHeader;
  struct v3_var_header *varHeader;
  struct nvram_v3_var_entry *varEntry;
  ...
  ret = _nvramController->select(next_bank);
  verify_noerr_action(ret,
                      DEBUG_INFO("select of bank %#x failed\n", next_bank));
  ...
  for (unsigned int i = 0; i < _varEntries->getCount(); i++) {
    ...
    if (varEntry->new_state != VAR_NEW_STATE_REMOVE) {
      ...
      memcpy(block + new_bank_offset, (uint8_t *)varHeader,
             variable_length(varHeader));
...
IOReturn IONVRAMV3Handler::sync(void) {
  IOReturn ret;
  ...
  if (_reload) {
    ...
    if (ret != kIOReturnSuccess) {
      ...
      ret = reclaim();
      require_noerr_action(
          ret, exit, DEBUG_ERROR("Reclaim recovery failed, ret=%#x\n", ret));
    }
    _reload = false;
  }
...
uint32_t IONVRAMV3Handler::getGeneration(void) const { return _generation; }

uint32_t IONVRAMV3Handler::getVersion(void) const { return kNVRAMVersion3; }

uint32_t IONVRAMV3Handler::getSystemUsed(void) const { return _systemUsed; }

uint32_t IONVRAMV3Handler::getCommonUsed(void) const { return _commonUsed; }

bool IONVRAMV3Handler::getSystemPartitionActive(void) const {
  return _systemSize != 0;
}

bool IONVRAMV3Handler::convertObjectToProp(uint8_t *buffer, uint32_t *length,
                                           const char *propName,
                                           OSObject *propObject) {
  uint32_t offset;
  ...
  case kOFVariableTypeNumber:
    ...
    if (tmpNumber != nullptr) {
      ...
      if (tmpValue == 0xFFFFFFFF) {
        ...
        end_buf +=
            snprintf(end_buf, sizeof(num_buf), "%#x", (uint32_t)tmpValue);
      }
...
bool IONVRAMV3Handler::convertPropToObject(
    const uint8_t *propName, uint32_t propNameLength, const uint8_t *propData,
    uint32_t propDataLength, OSSharedPtr<const OSSymbol> &propSymbol,
    OSSharedPtr<OSObject> &propObject) {
  ...
  case kOFVariableTypeNumber:
    tmpNumber =
        OSNumber::withNumber(strtol((const char *)propData, nullptr, 0), 32);
    if (tmpNumber != nullptr) {
      tmpObject = tmpNumber;
    }
    break;
...
IOReturn IONVRAMV3Handler::getVarDict(OSSharedPtr<OSDictionary> &varDictCopy) {
