...
SYSCTL_UINT(_kern, OID_AUTO, pmtimeout, CTLFLAG_RW | CTLFLAG_LOCKED,
            &gSleepAckTimeout, 0, "Power Management Timeout");
SYSCTL_UINT(_kern, OID_AUTO, pmcallouttimer, CTLFLAG_RW | CTLFLAG_LOCKED,
            &gDriverCalloutTimer, 0,
            "Power Management Driver Callout Log Timer");
...
#define RD_LOG(x...)                                                           \
  do {                                                                         \
    if ((kIOLogPMRootDomain & gIOKitDebug) && (getPMRootDomain() == this)) {   \
      IOLog("PMRD: " x);                                                       \
    }                                                                          \
  } while (false)
...
#define PM_LOCK_SLEEP(event, dl)                                               \
  IOLockSleepDeadline(fPMLock, event, dl, THREAD_UNINT)
...
#define OUR_PMLogFuncStart(t, a, b)                                            \
  do {                                                                         \
    if (pwrMgt) {                                                              \
      if (gIOKitDebug & kIOLogPower)                                           \
        pwrMgt->pmPrint(t, a, b);                                              \
...
#define IS_POWER_DROP                                                          \
  (StateOrder(fHeadNotePowerState) < StateOrder(fCurrentPowerState))
#define IS_POWER_RISE                                                          \
  (StateOrder(fHeadNotePowerState) > StateOrder(fCurrentPowerState))
...
enum { kReserveDomainPower = 1 };

#define MS_PUSH(n)                                                             \
  do {                                                                         \
    assert(kIOPM_BadMachineState == fSavedMachineState);                       \
    assert(kIOPM_BadMachineState != n);                                        \
    fSavedMachineState = n;                                                    \
  } while (false)
...
#define PM_ACTION_CHANGE(a, x, y)                                              \
  do {                                                                         \
    if (fPMActions.a) {                                                        \
      (fPMActions.a)(fPMActions.target, this, &fPMActions, gIOPMRequest, x,    \
                     y);                                                       \
    }                                                                          \
  } while (false)
...
static OSNumber *copyClientIDForNotification(OSObject *object,
                                             IOPMInterestContext *context);

static void logClientIDForNotification(OSObject *object,
                                       IOPMInterestContext *context,
...
void IOService::allocPMInitLock(void) {
...
void IOService::PMinit(void) {
  if (!initialized) {
    ...
    if (!gIOPMInitialized) {
      ...
      if (gIOPMWorkLoop) {
        ...
        gIOPMRequestQueue = IOPMRequestQueue::create(
            this, OSMemberFunctionCast(IOPMRequestQueue::Action, this,
                                       &IOService::actionPMRequestQueue));

        gIOPMReplyQueue = IOPMRequestQueue::create(
            this, OSMemberFunctionCast(IOPMRequestQueue::Action, this,
                                       &IOService::actionPMReplyQueue));
        ...
        gIOPMCompletionQueue = IOPMCompletionQueue::create(
            this, OSMemberFunctionCast(IOPMCompletionQueue::Action, this,
        ...
        if (gIOPMWorkLoop->addEventSource(gIOPMWorkQueue) != kIOReturnSuccess) {
      ...
      if (PE_parse_boot_argn("setpowerstate_log", &setPowerStateLogMS,
    ...
    if (!gIOPMRootNode &&
        (getParentEntry(gIOPowerPlane) == getRegistryRoot())) {
      gIOPMRootNode = this;
    ...
    if (IS_ROOT_DOMAIN) {
      fWatchdogTimer = thread_call_allocate(&IOService::watchdog_timer_expired,
    ...
    fAckTimer = thread_call_allocate(&IOService::ack_timer_expired,
                                     (thread_call_param_t)this);
#if USE_SETTLE_TIMER
    fSettleTimer =
        thread_call_allocate(&settle_timer_expired, (thread_call_param_t)this);
#endif
    fIdleTimer =
        thread_call_allocate(&idle_timer_expired, (thread_call_param_t)this);
    fDriverCallTimer = thread_call_allocate(&IOService::pmDriverCalloutTimer,
                                            (thread_call_param_t)this);
    ...
    if (OSMemberFunctionCast(void (*)(void), getResourceService(),
                             &IOService::powerChangeDone) !=
        OSMemberFunctionCast(void (*)(void), this,
                             &IOService::powerChangeDone)) {
...
IOReturn IOService::youAreRoot(void) { return IOPMNoErr; }
#endif /* !__LP64__ */
...
void IOService::addPowerChild2(IOPMRequest *request) {
  ...
  if (fControllingDriver && fParentsKnowState) {
    fMaxPowerState = fControllingDriver->driverMaxCapabilityForDomainState(
        fParentsCurrentPowerFlags);
    // initially change into the state we are already in
    tempDesire = fControllingDriver->driverInitialPowerStateForDomainState(
        fParentsCurrentPowerFlags);
  ...
  if (child) {
    ...
    if ((num = OSDynamicCast(OSNumber, obj))) {
      ...
      if (child->reserved->uvars && child->reserved->uvars->userServer) {
        child->reserved->uvars->userServer->addPMDriverClass(
...
bool IOService::currentOrPendingPowerState(uint32_t state) {
  return (fCurrentPowerState == state) || (fHeadNotePowerState == state);
}

void IOService::addPMDriverClass(uint64_t driverClass) {
  if (pwrMgt) {
...
IOReturn IOService::setPowerParent(IOPowerConnection *theParent,
                                   bool stateKnown, IOPMPowerFlags powerFlags) {
...
IOReturn IOService::registerPowerDriver(IOService *powerDriver,
...
void IOService::handleRegisterPowerDriver(IOPMRequest *request) {
  ...
  if (!fNumberOfPowerStates) {
    ...
    assert(root);
    if (!root ||
        ((OSMemberFunctionCast(void (*)(void), root,
...
bool IOService::handleAcknowledgePowerChange(IOPMRequest *request) {
  ...
  if (fHeadNotePendingAcks != 0) {
    ...
    if (informee != NULL) {
      ...
      if (informee->timer != 0) {
        SOCD_TRACE_XNU(
            PM_INFORM_POWER_CHANGE_ACK, SOCD_TRACE_MODE_NONE,
            ADDR(informee->whatObject->getMetaClass()),
            ADDR(this->getMetaClass()),
            PACK_2X32(VALUE(this->getRegistryEntryID()),
                      VALUE(informee->whatObject->getRegistryEntryID())),
        ...
        if (informee->timer > 0) {
          ...
          if (nsec > gIOPMSetPowerStateLogNS) {
            getPMRootDomain()->pmStatsRecordApplicationResponse(
                ...
                fDriverCallReason, NS_TO_MS(nsec),
                informee->whatObject->getRegistryEntryID(), NULL,
                fHeadNotePowerState, true);
          }
        }
...
bool IOService::handleAcknowledgeSetPowerState(IOPMRequest *request __unused) {
  ...
  if (fDriverTimer == -1) {
    ...
    if (nsec > gIOPMSetPowerStateLogNS) {
      getPMRootDomain()->pmStatsRecordApplicationResponse(
          gIOPMStatsDriverPSChangeSlow, fName, kDriverCallSetPowerState,
          NS_TO_MS(nsec), getRegistryEntryID(), NULL, fHeadNotePowerState,
          true);
    }
  ...
  if (trace_this_ack) {
    if (fControllingDriver) {
      ...
      controllingDriverRegistryEntryID =
          (uint32_t)fControllingDriver->getRegistryEntryID();
    }

    SOCD_TRACE_XNU(PM_SET_POWER_STATE_ACK, SOCD_TRACE_MODE_NONE,
                   ADDR(controllingDriverMetaClass), ADDR(this->getMetaClass()),
                   PACK_2X32(VALUE(this->getRegistryEntryID()),
                             VALUE(controllingDriverRegistryEntryID)),
...
void IOService::adjustPowerState(IOPMPowerStateIndex clamp) {
  PM_ASSERT_IN_GATE();
...
IOReturn IOService::synchronizePowerTree(IOOptionBits options,
  ...
  if (notifyRoot) {
    ...
    nr = acquirePMRequest(getPMRootDomain(),
...
void IOService::handleSynchronizePowerTree(IOPMRequest *request) {
  ...
  if (fControllingDriver && fParentsKnowState && inPlane(gIOPowerPlane) &&
    ...
    IOPMPowerChangeFlags options =
        (IOPMPowerChangeFlags)(uintptr_t)request->fArg0;
...
IOReturn IOService::powerDomainWillChangeTo(IOPMPowerFlags newPowerFlags,
...
void IOService::handlePowerDomainWillChangeTo(IOPMRequest *request) {
  ...
  IOPMPowerChangeFlags parentChangeFlags =
      (IOPMPowerChangeFlags)(uintptr_t)request->fArg2;
  ...
  if (!inPlane(gIOPowerPlane) || !whichParent ||
      !whichParent->getAwaitingAck()) {
...
//
// Called by the power-hierarchy parent after the power state of the power
// domain has settled at a new level. We enqueue a parent power-change to our
// queue of power changes. This may or may not cause us to change power,
// depending on what kind of change is occuring in the domain.
//*********************************************************************************

IOReturn IOService::powerDomainDidChangeTo(IOPMPowerFlags newPowerFlags,
                                           IOPowerConnection *whichParent) {
  assert(false);
  return kIOReturnUnsupported;
}
...
//
// Set our connection data for one specific parent, and then combine all the
// parent data together.
//*********************************************************************************

void IOService::setParentInfo(IOPMPowerFlags newPowerFlags,
                              IOPowerConnection *whichParent, bool knowsState) {
  OSIterator *iter;
  OSObject *next;
...
void IOService::trackSystemSleepPreventers(
  ...
  if ((oldCapability ^ newCapability) & kIOPMPreventIdleSleep) {
    ...
    bool idleCancelAllowed =
        getPMRootDomain()->updatePreventIdleSleepList(this, enablePrevention);
  ...
  if ((oldCapability ^ newCapability) & kIOPMPreventSystemSleep) {
    getPMRootDomain()->updatePreventSystemSleepList(
        this, ((oldCapability & kIOPMPreventSystemSleep) == 0));
  }
}
...
IOReturn
IOService::requestPowerDomainState(IOPMPowerFlags childRequestPowerFlags,
    ...
    if (IS_POWER_DROP && !IS_ROOT_DOMAIN) {
      ...
      if ((fMachineState == kIOPM_OurChangeTellClientsPowerDown) ||
        ...
        PM_LOG1("%s: power drop cancelled in state %u by %s\n", getName(),
                fMachineState, child->getName());
      } else {
        // Beyond cancellation point, report the impending state.
        outputPowerFlags = fPowerStates[fHeadNotePowerState].outputPowerFlags;
      }
  ...
  if (adjustPower) {
    PM_LOG("requestPowerDomainState[%s]: %s, init %d, %u->%u\n", getName(),
           child->getName(), !childConnection->childHasRequestedPower(),
  ...
  if (adjustPower && !fDeviceOverrideEnabled &&
    ...
    subRequest =
        acquirePMRequest(this, kIOPMRequestTypeAdjustPowerState, gIOPMRequest);
    if (subRequest) {
...
// power state is computed and compared against the current power state. If
// those power states differ, then a power state change is initiated.
//*********************************************************************************
...
IOReturn IOService::changePowerStateWithTagTo(IOPMPowerStateIndex ordinal,
                                              IOPMRequestTag tag) {
...
IOReturn IOService::changePowerStateWithTagToPriv(unsigned long ordinal,
                                                  IOPMRequestTag tag) {
...
IOReturn IOService::quiescePowerTree(void *target, IOPMCompletionAction action,
                                     void *param) {
  IOPMRequest *request;
...
IOReturn IOService::requestPowerState(const OSSymbol *client,
...
void IOService::handleRequestPowerState(IOPMRequest *request) {
  ...
  if ((state == kPowerStateZero) && (client != gIOPMPowerClientDevice) &&
...
void IOService::updatePowerClient(const OSSymbol *client,
                                  IOPMPowerStateIndex powerState) {
  ...
  if (fPowerClients && client) {
    ...
    PM_ACTION_CLIENT(actionUpdatePowerClient, client, oldPowerState,
                     powerState);
  }
}
...
IOPMPowerStateIndex IOService::getPowerStateForClient(const OSSymbol *client) {
...
void IOService::computeDesiredState(unsigned long localClamp,
  ...
  if (iter) {
    while ((next = iter->getNextObject())) {
      if ((connection = OSDynamicCast(IOPowerConnection, next))) {
        ...
        desiredState =
            StateMax(connection->getDesiredDomainState(), desiredState);
      }
    }
    iter->release();
  }
  ...
  if (getPMRootDomain()->isAOTMode()) {
    if ((kIOPMPreventIdleSleep & fPowerStates[newPowerState].capabilityFlags) &&
        !(kIOPMPreventIdleSleep &
          fPowerStates[fDesiredPowerState].capabilityFlags)) {
      getPMRootDomain()->claimSystemWakeEvent(this, kIOPMWakeEventAOTExit,
                                              getName(), NULL);
    }
  }
  ...
  PM_LOG1("  temp %u, clamp %u, current %u, new %u\n", (uint32_t)localClamp,
          (uint32_t)fTempClampPowerState, (uint32_t)fCurrentPowerState,
...
static void applyToPowerChildren(IOService *service,
...
static void applyToPowerParent(IOService *service,
                               IOServiceApplierFunction applier, void *context,
...
void IOService::handleDeferredActivityTickle(IOPMRequest *request) {
...
bool IOService::_activityTickle(unsigned long type, unsigned long stateNumber) {
  IOPMRequest *request;
...
void IOService::handleActivityTickle(IOPMRequest *request) {
  ...
  if (tickleFlags & kTickleTypeActivity) {
    ...
    IOPMPowerStateIndex idleTimerGeneration =
        ticklePowerState; // kTickleTypePowerDrop
...
SInt32 IOService::nextIdleTimeout(AbsoluteTime currentTime,
...
void IOService::start_PM_idle_timer(void) {
  ...
  idle_in = nextIdleTimeout(uptime, fDeviceActiveTimestamp,
                            (unsigned int)fCurrentPowerState);

  // Check for out-of range responses
  if (idle_in > maxTimeout) {
    // use standard implementation
    idle_in = IOService::nextIdleTimeout(uptime, fDeviceActiveTimestamp,
                                         (unsigned int)fCurrentPowerState);
...
static void idle_timer_expired(thread_call_param_t arg0,
                               thread_call_param_t arg1) {
  IOService *me = (IOService *)arg0;

  if (gIOPMWorkLoop) {
    gIOPMWorkLoop->runAction(OSMemberFunctionCast(IOWorkLoop::Action, me,
...
//
// Pass on the input parameters to all power domain children. All those which
// are power domains will pass it on to their children, etc.
//*********************************************************************************

IOReturn IOService::setAggressiveness(unsigned long type,
...
IOReturn IOService::getAggressiveness(unsigned long type,
...
UInt32 IOService::getDesiredPowerState(void) {
  if (!initialized) {
    return kPowerStateZero;
  }
...
IOReturn IOService::startPowerChange(IOPMPowerChangeFlags changeFlags,
  ...
  if (!fAdjustPowerScheduled && ((changeFlags & kIOPMSelfInitiated) == 0) &&
  ...
  for (IOItemCount i = 0, arrayIdx = 0; i < count; i++) {
    if (fInitialSetPowerState ||
      ...
      if ((this == informee->whatObject) &&
          (fHeadNotePowerArrayEntry->capabilityFlags &
...
void IOService::notifyInterestedDriversDone(void) {
  ...
  if (param && count) {
    ...
    if (fHeadNotePendingAcks) {
      ...
      start_ack_timer();
      getPMRootDomain()->reset_watchdog_timer(this,
                                              maxTimeout / USEC_PER_SEC + 1);
    }
  }
  ...
  if (iter) {
    while ((next = iter->getNextObject())) {
      if ((connection = OSDynamicCast(IOPowerConnection, next))) {
        ...
        if (!fIsPreChange && connection->delayChildNotification &&
...
IOReturn IOService::notifyAll(uint32_t nextMS) {
  ...
  fMachineState = kIOPM_DriverThreadCallDone;
  fDriverCallReason =
      fIsPreChange ? kDriverCallInformPreChange : kDriverCallInformPostChange;
...
IOReturn IOService::actionDriverCalloutDone(OSObject *target, void *arg0,
                                            void *arg1, void *arg2,
                                            void *arg3) {
...
void IOService::driverSetPowerState(void) {
  ...
  if (assertPMDriverCall(&callEntry, kIOPMDriverCallMethodSetPowerState)) {
    SOCD_TRACE_XNU_START(PM_SET_POWER_STATE, ADDR(controllingDriverMetaClass),
                         ADDR(this->getMetaClass()),
                         PACK_2X32(VALUE(this->getRegistryEntryID()),
                                   VALUE(controllingDriverRegistryEntryID)),
    ...
    if (reserved && reserved->uvars && reserved->uvars->userServer) {
      result = reserved->uvars->userServer->serviceSetPowerState(
          fControllingDriver, this, fHeadNotePowerArrayEntry->capabilityFlags,
          powerState);
    } else {
    ...
    SOCD_TRACE_XNU_END(PM_SET_POWER_STATE, ADDR(controllingDriverMetaClass),
                       ADDR(this->getMetaClass()),
                       PACK_2X32(VALUE(this->getRegistryEntryID()),
                                 VALUE(controllingDriverRegistryEntryID)),
    ...
    if (result < 0) {
      PM_LOG("%s::setPowerState(%p, %lu -> %lu) returned 0x%x\n", fName,
             OBFUSCATE(this), fCurrentPowerState, powerState, result);
    }
    ...
    if ((result == IOPMAckImplied) || (result < 0)) {
      ...
      if (nsec > gIOPMSetPowerStateLogNS) {
        getPMRootDomain()->pmStatsRecordApplicationResponse(
            gIOPMStatsDriverPSChangeSlow, fName, kDriverCallSetPowerState,
            NS_TO_MS(nsec), getRegistryEntryID(), NULL, powerState);
      }
    }
  } else {
...
void IOService::driverInformPowerChange(void) {
  ...
  IOOptionBits callMethod = (fDriverCallReason == kDriverCallInformPreChange)
                                ? kIOPMDriverCallMethodWillChange
  ...
  for (IOItemCount i = 0; i < count; i++) {
    ...
    if (assertPMDriverCall(&callEntry, callMethod, informee)) {
      SOCD_TRACE_XNU_START(
          PM_INFORM_POWER_CHANGE, ADDR(driver->getMetaClass()),
          ADDR(this->getMetaClass()),
          PACK_2X32(VALUE(this->getRegistryEntryID()),
                    VALUE(driver->getRegistryEntryID())),
          PACK_2X32(VALUE(powerState), VALUE(fDriverCallReason)));

      if (fDriverCallReason == kDriverCallInformPreChange) {
        OUR_PMLogFuncStart(kPMLogInformDriverPreChange, (uintptr_t)this,
                           powerState);
        ...
        OUR_PMLogFuncStart(kPMLogInformDriverPostChange, (uintptr_t)this,
                           powerState);
        clock_get_uptime(&informee->startTime);
      ...
      SOCD_TRACE_XNU_END(PM_INFORM_POWER_CHANGE, ADDR(driver->getMetaClass()),
                         ADDR(this->getMetaClass()),
                         PACK_2X32(VALUE(this->getRegistryEntryID()),
                                   VALUE(driver->getRegistryEntryID())),
...
void IOService::startDriverCalloutTimer(void) {
...
void IOService::stopDriverCalloutTimer(void) {
  boolean_t pending;
...
void IOService::pmDriverCalloutTimer(thread_call_param_t arg0,
...
void IOService::all_done(void) {
  ...
  if ((fHeadNoteChangeFlags & kIOPMSynchronize) &&
    ...
    PM_ACTION_CHANGE(actionPowerChangeDone, fHeadNotePowerState,
  ...
  if (fHeadNoteChangeFlags & kIOPMSelfInitiated) {
    ...
    if ((fHeadNoteChangeFlags & kIOPMNotDone) == 0) {
      trackSystemSleepPreventers(fCurrentPowerState, fHeadNotePowerState,
                                 fHeadNoteChangeFlags);
      ...
      PM_ACTION_CHANGE(actionPowerChangeDone, prevPowerState,
                       fHeadNoteChangeFlags);
      actionCalled = true;
      ...
      if (fPCDFunctionOverride && fParentsKnowState &&
          assertPMDriverCall(&callEntry, kIOPMDriverCallMethodChangeDone, NULL,
                             kIOPMDriverCallNoInactiveCheck)) {
        powerChangeDone(prevPowerState);
        deassertPMDriverCall(&callEntry);
      }
    } else if (getPMRequestType() ==
               kIOPMRequestTypeRequestPowerStateOverride) {
...
void IOService::OurChangeStart(void) {
  ...
  if (!IS_PM_ROOT &&
      (StateOrder(fMaxPowerState) < StateOrder(fHeadNotePowerState))) {
  ...
  PM_ACTION_CHANGE(actionPowerChangeStart, fHeadNotePowerState,
                   &fHeadNoteChangeFlags);
...
static void requestDomainPowerApplier(IORegistryEntry *entry, void *inContext) {
  IOPowerConnection *connection;
  IOService *parent;
  ...
  if (connection->parentKnowsState() && connection->getReadyFlag()) {
    parent->requestPowerDomainState(context->requestPowerFlags, connection,
                                    IOPMLowestState);
  }
...
IOReturn IOService::requestDomainPower(IOPMPowerStateIndex ourPowerState,
                                       IOOptionBits options) {
...
void IOService::OurChangeFinish(void) { all_done(); }
...
//
// All (legacy) applications and kernel clients have acknowledged, extra stage
// for root domain to notify apps and drivers about the system capability
// change.
//*********************************************************************************
...
#if USE_SETTLE_TIMER
static void settle_timer_expired(thread_call_param_t arg0,
                                 thread_call_param_t arg1) {
  IOService *me = (IOService *)arg0;
    ...
    if (fDriverTimer > 0) {
      ...
      if (fDriverTimer == 0) {
        ...
        if (panic_allowed) {
          ...
          if (strncmp(fName, allowlist, strlen(allowlist))) {
            panic("%s::setPowerState(%p, %lu -> %lu) timed out after %d ms",
                  fName, this, fCurrentPowerState, fHeadNotePowerState,
                  NS_TO_MS(nsec));
          }
        } else {
#ifdef CONFIG_XNUPOST
          ...
          if (setpowerstate_panic != 0) {
            PM_ERROR("setPowerState panic disabled by setpowerstate_panic "
                     "boot-arg\n");
          }
        }
        ...
        if (gIOKitDebug & kIOLogDebugPower) {
          ...
          OSKext *kext = NULL;
          function_addr = OSMemberFunctionCast(const void *, fControllingDriver,
          ...
          if (kext) {
            ...
            if (strncmp(bundleID, apple_prefix, strlen(apple_prefix)) == 0 ||
                strncmp(bundleID, kernel_prefix, strlen(kernel_prefix)) == 0) {
              // first party client
              panic("%s::setPowerState(%p : %p, %lu -> %lu) timed out after %d "
                    "ms",
                    fName, this, function_addr, fCurrentPowerState,
                    fHeadNotePowerState, NS_TO_MS(nsec));
            }
            kext->release();
          }
      ...
      while (nextObject != NULL) {
        if (nextObject->timer > 0) {
          ...
          if (nextObject->timer == 0) {
            ...
            nextObject->whatObject->setProperty(kIOPMTardyAckPSCKey,
                                                kOSBooleanTrue);
            PM_ERROR("%s::powerState%sChangeTo(%p, %s, %lu -> %lu) timed out "
                     "after %d ms\n",
                     nextObject->whatObject->getName(),
                     (fDriverCallReason == kDriverCallInformPreChange) ? "Will"
                                                                       : "Did",
                     OBFUSCATE(nextObject->whatObject), fName,
                     fCurrentPowerState, fHeadNotePowerState, NS_TO_MS(nsec));
...
void IOService::reset_watchdog_timer(IOService *blockedObject,
  ...
  if (pendingResponseTimeout == 0) {
    ...
    clock_interval_to_deadline(pendingResponseTimeout + 2, kSecondScale,
                               &deadline);

    if (i != (unsigned int)-1) {
      PM_ERROR("WDOG:Object %s is already blocked for responses. Ignoring "
               "timeout %d\n",
...
void IOService::reset_watchdog_timer(int timeout) {
  uint64_t deadline;
...
uint64_t IOService::get_watchdog_elapsed_time(void) {
  uint64_t delta;
...
void IOService::watchdog_timer_expired(thread_call_param_t arg0,
                                       thread_call_param_t arg1) {
  IOService *me = (IOService *)arg0;
  bool expired;
  ...
  expired =
      me->fWatchdogDeadline && (me->fWatchdogDeadline <= mach_absolute_time());
...
IOWorkLoop *IOService::getIOPMWorkloop(void) { return gIOPMWorkLoop; }
...
IOReturn IOService::actionAckTimerExpired(OSObject *target, void *arg0,
...
void IOService::ack_timer_expired(thread_call_param_t arg0,
                                  thread_call_param_t arg1) {
  IOService *me = (IOService *)arg0;
  ...
  if (OSDynamicCast(_IOServiceInterestNotifier, object)) {
    ...
    if ((clientIndex != (unsigned int)-1) &&
      ...
      IOService::getPMRootDomain()->pmStatsRecordApplicationResponse(
          gIOPMStatsResponseTimedOut, name, 0, (30 * 1000), pid, object);
    }
  }
}

static void logClientTimeouts(OSObject *object, void *arg) {
  IOPMInterestContext *context = (IOPMInterestContext *)arg;
  unsigned int clientIndex, startIndex = 0;
  OSObject *flag;
  bool isPriorityClient;

  isPriorityClient = (context->notifyType == kNotifyPriority) ||
                     (context->notifyType == kNotifyCapabilityChangePriority);

  // notifyClients can contain multiple instances of a client if we have
  // notified them multiple times in one tellClientsWithResponse cycle.
  while ((clientIndex = context->notifyClients->getNextIndexOfObject(
              object, startIndex)) != (unsigned int)-1) {
    // Check for client timeouts
    bool timeout = (flag = context->responseArray->getObject(clientIndex)) &&
                   (flag != kOSBooleanTrue);
    if (timeout) {
      if (context->us == IOService::getPMRootDomain()) {
        ...
        PM_ERROR("PM %snotification timeout (service: %s, client: %s)\n",
                 isPriorityClient ? "priority " : "", context->us->getName(),
                 id);
      }
    }
  ...
  if (logErrors && fResponseArray) {
    if (fNotifyClientArray) {
      ...
      case kNotifyApps:
        ...
        applyToInterested(gIOGeneralInterest, logClientTimeouts,
                          (void *)&context);
        OS_FALLTHROUGH;
      ...
      case kNotifyCapabilityChangePriority:
        applyToInterested(gIOPriorityPowerStateInterest, logClientTimeouts,
                          (void *)&context);
        break;
      default:
        break;
...
bool IOService::tellClientsWithResponse(int messageType) {
  ...
  context.messageFilter =
      (isRootDomain)
          ? OSMemberFunctionCast(IOPMMessageFilter, (IOPMrootDomain *)this,
                                 &IOPMrootDomain::systemMessageFilter)
          : NULL;

  switch (fOutOfBandParameter) {
  case kNotifyApps:
    applyToInterested(gIOAppPowerStateInterest, pmTellAppWithResponse,
                      (void *)&context);
    ...
    applyToInterested(gIOGeneralInterest, pmTellClientWithResponse,
                      (void *)&context);
  ...
  case kNotifyPriority:
    context.enableTracing = isRootDomain;
    applyToInterested(gIOPriorityPowerStateInterest, pmTellClientWithResponse,
                      (void *)&context);
  ...
  case kNotifyCapabilityChangeApps:
    context.enableTracing = isRootDomain;
    applyToInterested(gIOAppPowerStateInterest, pmTellCapabilityAppWithResponse,
                      (void *)&context);
    if (context.messageType == kIOMessageCanSystemSleep) {
      maxTimeOut = kCanSleepMaxTimeReq;
      if (gSleepAckTimeout) {
  ...
  if (context.skippedInDark) {
    IOLog("tellClientsWithResponse(%s, %s) %d of %d skipped in dark\n",
          ...
          context.skippedInDark + context.notSkippedInDark);
  }
  ...
  if (!checkForDone()) {
    ...
    if (context.enableTracing) {
      getPMRootDomain()->traceDetail(context.messageType, 0,
                                     context.maxTimeRequested / 1000);
      getPMRootDomain()->reset_watchdog_timer(
          this, context.maxTimeRequested / USEC_PER_SEC + 1);
    }
...
//
// We send a message to an application, and we expect a response, so we compute
// a cookie we can identify the response with.
//*********************************************************************************
...
void IOService::pmTellAppWithResponse(OSObject *object, void *arg) {
  ...
  if (context->us == getPMRootDomain()) {
    if ((clientID = copyClientIDForNotification(object, context))) {
      ...
      if (proc) {
        ...
        if (proc_suspended) {
          logClientIDForNotification(object, context,
                                     "PMTellAppWithResponse - Suspended");
        } else if (getPMRootDomain()->isAOTMode() &&
                   get_task_suspended((task_t)proc_task(proc))) {
          proc_suspended = true;
          context->skippedInDark++;
        }
        proc_rele(proc);
  ...
  if (context->messageFilter &&
      !context->messageFilter(context->us, object, context, NULL,
                              &waitForReply)) {
    if (kIOLogDebugPower & gIOKitDebug) {
  ...
  if (waitForReply == kOSBooleanTrue) {
    OSNumber *num;
    clock_get_uptime(&now);
    num = OSNumber::withNumber(AbsoluteTime_to_scalar(&now),
                               sizeof(uint64_t) * 8);
    if (num) {
...
void IOService::pmTellClientWithResponse(OSObject *object, void *arg) {
  ...
  // Besides interest notifiers this applier function can also be invoked
  // against IOService clients of context->us, so notifier can be NULL. But for
  // tracing purposes the IOService clients can be ignored but each will still
  // consume an entry in the responseArray and also advance msgIndex.
  ...
  if (kIOReturnSuccess == retCode) {
      ...
      if (notify.returnValue > context->maxTimeRequested) {
        if (notify.returnValue > kPriorityClientMaxWait) {
          ...
          PM_ERROR("%s: client %p returned %llu for %s\n",
                   context->us->getName(),
                   notifier ? (void *)OBFUSCATE(notifier->handler)
                            : OBFUSCATE(object),
                   (uint64_t)notify.returnValue, getIOMessageString(msgType));
        } else {
          context->maxTimeRequested =
              (typeof(context->maxTimeRequested))notify.returnValue;
        }
      }
    ...
    if (enableTracing) {
      ...
      if ((nsec > LOG_KEXT_RESPONSE_TIMES) || (notify.returnValue != 0)) {
        getPMRootDomain()->traceNotificationResponse(
            notifier, NS_TO_MS(nsec), (uint32_t)notify.returnValue);
      }
    }
  } else {
...
void IOService::pmTellCapabilityAppWithResponse(OSObject *object, void *arg) {
  ...
  if (context->messageFilter &&
      !context->messageFilter(context->us, object, context, &msgArg,
                              &waitForReply)) {
    return;
  }

  if (context->us == getPMRootDomain() && getPMRootDomain()->isAOTMode()) {
    OSNumber *clientID = NULL;
    boolean_t proc_suspended = FALSE;
    proc_t proc = NULL;
    if ((clientID = copyClientIDForNotification(object, context))) {
      ...
      if (proc) {
        ...
        if (proc_suspended) {
          logClientIDForNotification(
              object, context, "PMTellCapablityAppWithResponse - Suspended");
...
void IOService::pmTellCapabilityClientWithResponse(OSObject *object,
                                                   void *arg) {
  IOPMSystemCapabilityChangeParameters msgArg;
  IOPMInterestContext *context = (IOPMInterestContext *)arg;
  OSObject *replied = kOSBooleanTrue;
  ...
  retCode = context->us->messageClient(msgType, object, (void *)&msgArg,
                                       sizeof(msgArg));
  ...
  if (kIOReturnSuccess == retCode) {
    if (0 == msgArg.maxWaitForReply) {
      ...
      if (msgArg.maxWaitForReply > context->maxTimeRequested) {
        if (msgArg.maxWaitForReply > kCapabilityClientMaxWait) {
          context->maxTimeRequested = ackTimeRequested =
              kCapabilityClientMaxWait;
          PM_ERROR("%s: client %p returned %u for %s\n", context->us->getName(),
                   notifier ? (void *)OBFUSCATE(notifier->handler)
                            : OBFUSCATE(object),
                   msgArg.maxWaitForReply, getIOMessageString(msgType));
        } else {
    ...
    if (enableTracing) {
      ...
      if ((nsec > LOG_KEXT_RESPONSE_TIMES) || (msgArg.maxWaitForReply != 0)) {
        getPMRootDomain()->traceNotificationResponse(notifier, NS_TO_MS(nsec),
...
void IOService::tellClients(int messageType) {
  ...
  applyToInterested(gIOPriorityPowerStateInterest, tellKernelClientApplier,
                    (void *)&context);

  context.notifyType = kNotifyApps;
  applyToInterested(gIOAppPowerStateInterest, tellAppClientApplier,
                    (void *)&context);

  applyToInterested(gIOGeneralInterest, tellKernelClientApplier,
                    (void *)&context);
}
  ...
  if (context->us == IOService::getPMRootDomain()) {
    if ((clientID = copyClientIDForNotification(object, context))) {
      ...
      if (proc) {
        ...
        } else if (IOService::getPMRootDomain()->isAOTMode() &&
                   get_task_suspended((task_t)proc_task(proc))) {
          proc_suspended = true;
          context->skippedInDark++;
        }
...
bool IOService::responseValid(uint32_t refcon, int pid) {
  ...
  if ((num = OSDynamicCast(OSNumber, theFlag)) ||
      (ack = OSDynamicCast(IOPMClientAck, theFlag))) {
    AbsoluteTime now;
    AbsoluteTime start;
    uint64_t nsec;
    char name[128];
    ...
    clock_get_uptime(&now);
    AbsoluteTime_to_scalar(&start) =
        num ? num->unsigned64BitValue() : ack->completionTimestamp;
    SUB_ABSOLUTETIME(&now, &start);
    absolutetime_to_nanoseconds(now, &nsec);
    ...
    if (pid != 0) {
      ...
      if (nsec > LOG_APP_RESPONSE_TIMES) {
        IOLog("PM response took %d ms (%d, %s)\n", NS_TO_MS(nsec), pid, name);
      }

      if (nsec > LOG_APP_RESPONSE_MSG_TRACER) {
        // TODO: populate the messageType argument
        getPMRootDomain()->pmStatsRecordApplicationResponse(
            gIOPMStatsResponseSlow, name, 0, NS_TO_MS(nsec), pid, object);
      } else {
        getPMRootDomain()->pmStatsRecordApplicationResponse(
            gIOPMStatsResponsePrompt, name, 0, NS_TO_MS(nsec), pid, object);
      }
    } else {
    ...
    if (kIOLogDebugPower & gIOKitDebug) {
      PM_LOG("Ack(%u) %u ms\n", (uint32_t)ordinalComponent, NS_TO_MS(nsec));
    }
    theFlag = kOSBooleanFalse;
  } else if (object) {
    getPMRootDomain()->pmStatsRecordApplicationResponse(
        gIOPMStatsResponsePrompt, NULL, 0, 0, pid, object);
  }
...
//
// Called upon every client acknowledgement to scan through the response array
// and update the ack timer based on which clients have yet to acknowledge the
// power change. If a client hasn't acknowledged by their requested time, make
// sure not to wait on that client.
//*********************************************************************************

OSDefineMetaClassAndStructors(IOPMClientAck, OSObject);

void IOService::updateClientResponses(void) {
  int i = 0;
  ...
  for (i = 0;; i++) {
    ...
    if ((ack = OSDynamicCast(IOPMClientAck, obj))) {
      ...
      if (nsec >= timeRequestedNS) {
        ...
        maxTimeToAckMS =
            timeToAckMS > maxTimeToAckMS ? timeToAckMS : maxTimeToAckMS;
      }
    }
  }
...
// and kernel clients, and one of them has acknowledged.  If this is the last to
// do so, and all acknowledgements are positive, we continue with the power
// change.
//*********************************************************************************
...
//
// Our power state is about to lower, and we have notified applications
// and kernel clients, and one of them has vetoed the change.  If this is the
// last client to respond, we abandon the power change.
//*********************************************************************************
...
void IOService::cancelIdlePowerDown(IOService *service) {
  IOPMRequest *request;
...
//
// Internal method to cancel sleep synchronously to avoid races on power down
// path
//*********************************************************************************

void IOService::cancelIdlePowerDownSync(void) { handleCancelIdlePowerDown(); }

//*********************************************************************************
// [private] handleCancelIdlePowerDown
//*********************************************************************************

bool IOService::handleCancelIdlePowerDown(void) {
  bool more = false;
  if ((fMachineState == kIOPM_OurChangeTellClientsPowerDown) ||
      (fMachineState == kIOPM_OurChangeTellUserPMPolicyPowerDown) ||
      (fMachineState == kIOPM_OurChangeTellPriorityClientsPowerDown) ||
...
//*********************************************************************************
IOReturn IOService::configurePowerStatesReport(IOReportConfigureAction action,
                                               void *result) {
  ...
  if (!fNumberOfPowerStates) {
    return kIOReturnSuccess; // For drivers which are in power plane, but
                             // haven't called registerPowerDriver()
  }
  ...
  case kIOReportEnable:
    ...
    STATEREPORT_INIT((uint16_t)fNumberOfPowerStates, fReportBuf, reportSize,
                     getRegistryEntryID(), kPMPowerStatesChID,
    ...
    for (i = 0; i < fNumberOfPowerStates; i++) {
      ...
      STATEREPORT_SETSTATEID(fReportBuf, i,
                             ((bits & 0xff) << 8) |
                                 ((StateOrder(fMaxPowerState) & 0xf) << 4) |
                                 (StateOrder(i) & 0xf));
    }
    ts = mach_absolute_time();
    STATEREPORT_SETSTATE(fReportBuf, (uint16_t)fCurrentPowerState, ts);
...
//*********************************************************************************
IOReturn IOService::updatePowerStatesReport(IOReportConfigureAction action,
...
//*********************************************************************************
IOReturn IOService::configureSimplePowerReport(IOReportConfigureAction action,
                                               void *result) {
  IOReturn rc = kIOReturnSuccess;
...
//*********************************************************************************
IOReturn IOService::updateSimplePowerReport(IOReportConfigureAction action,
                                            void *result, void *destination) {
  uint32_t size2cpy;
  void *data2cpy;
  uint64_t buf[SIMPLEREPORT_BUFSIZE / sizeof(uint64_t) +
               1]; // Force a 8-byte alignment
  IOBufferMemoryDescriptor *dest =
      OSDynamicCast(IOBufferMemoryDescriptor, (OSObject *)destination);
  IOReturn rc = kIOReturnSuccess;
  unsigned bits = 0;
  ...
  case kIOReportCopyChannelData:
    ...
    SIMPLEREPORT_INIT(buf, sizeof(buf), getRegistryEntryID(), kPMCurrStateChID,
                      kIOReportCategoryPower);
    ...
    SIMPLEREPORT_SETVALUE(buf, ((bits & 0xff) << 8) |
                                   ((StateOrder(fMaxPowerState) & 0xf) << 4) |
...
IOReturn IOService::setPowerState(unsigned long powerStateOrdinal,
...
IOService::driverMaxCapabilityForDomainState(IOPMPowerFlags domainState) {
  ...
  if (!assertPMDriverCall(&callEntry,
                          kIOPMDriverCallMethodMaxCapabilityForDomainState)) {
    return kPowerStateZero;
  }

  if ((promote > 0) && (0 != (kIOPMPowerOn & domainState))) {
    IOPMPowerFlags newDomainState =
        (domainState & ~kIOPMPowerOn) | kIOPMAOTPower;
  ...
  if ((promote > 0) && (kPowerStateZero == powerState) &&
    ...
    IOPMPowerFlags newDomainState =
        (domainState & ~kIOPMAOTPower) | kIOPMPowerOn;
    powerState = maxCapabilityForDomainState(newDomainState);
  }
  deassertPMDriverCall(&callEntry);
...
IOService::driverInitialPowerStateForDomainState(IOPMPowerFlags domainState) {
  ...
  if (assertPMDriverCall(
          &callEntry, kIOPMDriverCallMethodInitialPowerStateForDomainState)) {
...
bool IOService::didYouWakeSystem(void) { return false; }
#endif /* !__LP64__ */
...
IOReturn IOService::powerStateWillChangeTo(IOPMPowerFlags, unsigned long,
                                           IOService *) {
...
IOReturn IOService::powerStateDidChangeTo(IOPMPowerFlags, unsigned long,
                                          IOService *) {
  return kIOPMAckImplied;
}
...
IOPMRequest *IOService::acquirePMRequest(IOService *target,
...
void IOService::submitPMRequest(IOPMRequest *request) {
  ...
  PM_LOG1("[+ %02lx] %p [%p %s] %p %p %p\n", (long)request->getType(),
          OBFUSCATE(request), OBFUSCATE(request->getTarget()),
          request->getTarget()->getName(), OBFUSCATE(request->fArg0),
...
void IOService::submitPMRequests(IOPMRequest **requests, IOItemCount count) {
  ...
  for (IOItemCount i = 0; i < count; i++) {
    ...
    PM_LOG1("[+ %02lx] %p [%p %s] %p %p %p\n", (long)req->getType(),
            OBFUSCATE(req), OBFUSCATE(req->getTarget()),
            req->getTarget()->getName(), OBFUSCATE(req->fArg0),
...
bool IOService::actionPMRequestQueue(IOPMRequest *request,
  ...
  if (initialized) {
    ...
    PM_LOG1("[- %02x] %p [%p %s] !initialized\n", request->getType(),
            OBFUSCATE(request), OBFUSCATE(this), getName());
...
bool IOService::actionPMCompletionQueue(IOPMRequest *request,
...
//
// IOPMWorkQueue::checkForWork() passing a retired request to the request
// target.
//*********************************************************************************

bool IOService::actionPMWorkQueueRetire(IOPMRequest *request,
                                        IOPMWorkQueue *queue) {
  assert(request && queue);

  PM_LOG1("[- %02x] %p [%p %s] state %d, busy %d\n", request->getType(),
          OBFUSCATE(request), OBFUSCATE(this), getName(), fMachineState,
...
bool IOService::actionPMWorkQueueInvoke(IOPMRequest *request,
                                        IOPMWorkQueue *queue) {
  bool done = false;
  int loop = 0;
  ...
  while (isPMBlocked(request, loop++) == false) {
    ...
    case kIOPM_OurChangeTellClientsPowerDown:
      ...
      if (IS_ROOT_DOMAIN) {
        ...
        getPMRootDomain()->askChangeDownDone(&fHeadNoteChangeFlags, &cancel);
      ...
      if (!fDoNotPowerDown) {
        ...
        record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_PMRD,
                            "Idle Sleep", "%s idle cancel, state %u", fName,
                            fMachineState);
#endif /* DEVELOPMENT || DEBUG */
        if (IS_ROOT_DOMAIN) {
    ...
    case kIOPM_DriverThreadCallDone:
      ...
      case kRootDomainInformPreChange:
        ...
        break;
      default:
        panic("%s: bad call reason %x", getName(), fDriverCallReason);
      }
      break;
    ...
    default:
      panic("PMWorkQueueInvoke: unknown machine state %x", fMachineState);
    }
...
bool IOService::actionPMReplyQueue(IOPMRequest *request,
                                   IOPMRequestQueue *queue) {
  bool more = false;
    ...
    if (responseValid((uint32_t)(uintptr_t)request->fArg0,
      ...
      if (kIOPMRequestTypeCancelPowerChange == request->getType()) {
        ...
        if ((fMachineState == kIOPM_OurChangeTellUserPMPolicyPowerDown) ||
          ...
          getPMRootDomain()->pmStatsRecordApplicationResponse(
              gIOPMStatsResponseCancel, name ? name->getCStringNoCopy() : "", 0,
...
bool IOService::assertPMDriverCall(IOPMDriverCallEntry *entry,
                                   IOOptionBits method,
  ...
  case kIOPMDriverCallMethodSetPowerState:
    entry->callMethod = OSMemberFunctionCast(const void *, fControllingDriver,
                                             &IOService::setPowerState);
    break;
  case kIOPMDriverCallMethodWillChange:
    entry->callMethod = OSMemberFunctionCast(
        const void *, target, &IOService::powerStateWillChangeTo);
    break;
  case kIOPMDriverCallMethodDidChange:
    entry->callMethod = OSMemberFunctionCast(const void *, target,
                                             &IOService::powerStateDidChangeTo);
    break;
...
bool IOService::getBlockingDriverCall(thread_t *thread,
...
void IOService::waitForPMDriverCall(IOService *target) {
  ...
  do {
    wait = false;
    queue_iterate(&fPMDriverCallQueue, entry, const IOPMDriverCallEntry *,
                  link) {
      ...
      if (entry->thread == thread) {
        if (log) {
          ...
          OSReportWithBacktrace("%s: %s(%s) on PM thread\n", fName,
                                __FUNCTION__, target ? target->getName() : "");
          log = false;
        }
        continue;
      }
...
bool IOPMRequest::init(IOService *target, IOOptionBits type) {
  ...
  fIsQuiesceBlocker =
      ((fTarget != gIOPMRootNode) && (fTarget != IOService::getPMRootDomain()));
...
bool IOPMRequest::attachNextRequest(IOPMRequest *next) {
  ...
  if (!fRequestNext) {
    ...
    PM_LOG("Attached next: %p [0x%x] -> %p [0x%x, %u] %s\n", OBFUSCATE(this),
           fRequestType, OBFUSCATE(fRequestNext), fRequestNext->fRequestType,
           (uint32_t)fRequestNext->fWorkWaitCount, fTarget->getName());
#endif
    ok = true;
  }
  return ok;
...
bool IOPMRequest::attachRootRequest(IOPMRequest *root) {
  ...
  if (!fRequestRoot) {
    ...
    PM_LOG("Attached root: %p [0x%x] -> %p [0x%x, %u] %s\n", OBFUSCATE(this),
           (uint32_t)fType, OBFUSCATE(fRequestRoot),
           (uint32_t)fRequestRoot->fType,
           (uint32_t)fRequestRoot->fFreeWaitCount, fTarget->getName());
#endif
    ok = true;
  }
  return ok;
...
IOPMRequestQueue *IOPMRequestQueue::create(IOService *inOwner,
                                           Action inAction) {
...
void IOPMRequestQueue::queuePMRequestChain(IOPMRequest **requests,
                                           IOItemCount count) {
  IOPMRequest *next;
  uint64_t now = mach_continuous_time();
...
IOPMWorkQueue *IOPMWorkQueue::create(IOService *inOwner, Action invoke,
                                     Action retire) {
...
void IOPMWorkQueue::incrementProducerCount(void) { fProducerCount++; }
...
void IOPMWorkQueue::attachQuiesceRequest(IOPMRequest *quiesceRequest) {
  ...
  queue_iterate(&fWorkQueue, entry, typeof(entry), WorkChain) {
    queue_iterate(&entry->RequestHead, request, typeof(request),
                  fCommandChain) {
...
IOPMCompletionQueue *IOPMCompletionQueue::create(IOService *inOwner,
                                                 Action inAction) {
...
OSDefineMetaClassAndStructors(IOServicePM, OSObject)
    ...
    static void setPMProperty(OSDictionary *dict, const char *key,
                              uint64_t value) {
...
IOReturn IOServicePM::gatedSerialize(OSSerialize *s) const {
  ...
  if (PowerClients) {
    dict = OSDictionary::withDictionary(PowerClients,
                                        PowerClients->getCount() + dictSize);
  } else {
  ...
  if (dict) {
    ...
    if (gIOPMRootNode == Owner) {
      setPMProperty(dict, "WQ-CheckForWork", gIOPMWorkQueue->fStatCheckForWork);
      setPMProperty(dict, "WQ-ScanEntries", gIOPMWorkQueue->fStatScanEntries);
      setPMProperty(dict, "WQ-QueueEmpty", gIOPMWorkQueue->fStatQueueEmpty);
      setPMProperty(dict, "WQ-NoWorkDone", gIOPMWorkQueue->fStatNoWorkDone);
    }
#endif
...
bool IOServicePM::serialize(OSSerialize *s) const {
  ...
  if (gIOPMWatchDogThread == current_thread()) {
    // Calling without lock as this data is collected for debug purpose, before
    // reboot. The workloop is probably already hung in state machine.
    ret = gatedSerialize(s);
  } else if (gIOPMWorkLoop) {
    ret = gIOPMWorkLoop->runAction(
        OSMemberFunctionCast(IOWorkLoop::Action, this,
                             &IOServicePM::gatedSerialize),
...
void IOServicePM::pmPrint(uint32_t event, uintptr_t param1,
                          uintptr_t param2) const {
  gPlatform->PMLog(Name, event, param1, param2);
}

void IOServicePM::pmTrace(uint32_t event, uint32_t eventFunc, uintptr_t param1,
                          uintptr_t param2) const {
...
#if defined(XNU_TARGET_OS_OSX)
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE, IODBG_POWER(event) | eventFunc,
                            nameAsArg, (uintptr_t)Owner->getRegistryEntryID(),
                            (uintptr_t)(OBFUSCATE(param1)),
                            (uintptr_t)(OBFUSCATE(param2)), 0);
#else
  IOTimeStampConstant(IODBG_POWER(event) | eventFunc, nameAsArg,
                      (uintptr_t)Owner->getRegistryEntryID(),
                      (uintptr_t)(OBFUSCATE(param1)),
