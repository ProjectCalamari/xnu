...
enum { kParentSetIndex = 0, kChildSetIndex = 1, kNumSetIndex };
enum { kIOMaxPlaneName = 32 };
...
#define WLOCK                                                                  \
  lck_rw_lock_exclusive(&gIORegistryLock);                                     \
...
IORegistryEntry *IORegistryEntry::initialize(void) {
  ...
  if (!gRegistryRoot) {
    ...
    gIORegistryEntryAllowableSetPropertiesKey =
...
SInt32 IORegistryEntry::getRegistryEntryParentGenerationCount(void) const {
...
const IORegistryPlane *IORegistryEntry::makePlane(const char *name) {
  ...
  if (plane && plane->init() && nameKey && parentKey && childKey &&
...
bool IORegistryEntry::init(IORegistryEntry *old, const IORegistryPlane *plane) {
  OSArray *all;
  ...
  fRegistryTable->setObject(gIONameKey,
                            old->fRegistryTable->getObject(gIONameKey));
  fRegistryTable->setObject(gIOLocationKey,
                            old->fRegistryTable->getObject(gIOLocationKey));
  fRegistryTable->setObject(plane->nameKey,
                            old->fRegistryTable->getObject(plane->nameKey));
  fRegistryTable->setObject(plane->pathNameKey,
                            old->fRegistryTable->getObject(plane->pathNameKey));
  fRegistryTable->setObject(
      plane->pathLocationKey,
      old->fRegistryTable->getObject(plane->pathLocationKey));
  fRegistryTable->setObject(
      plane->keys[kParentSetIndex],
      old->fRegistryTable->getObject(plane->keys[kParentSetIndex]));
  fRegistryTable->setObject(
      plane->keys[kChildSetIndex],
      old->fRegistryTable->getObject(plane->keys[kChildSetIndex]));
#endif /* IOREGSPLITTABLES */
  ...
  if (all) {
    for (index = 0; (next = (IORegistryEntry *)all->getObject(index));
         index++) {
...
#define wrap2(type, constant)                                                  \
  OSObject *IORegistryEntry::copyProperty(type *aKey) constant {               \
    OSObject *obj;                                                             \
                                                                               \
    PLOCK;                                                                     \
...
#define wrap4(type, constant)                                                  \
  OSObject *IORegistryEntry::getProperty(type *aKey,                           \
                                         const IORegistryPlane *plane,         \
...
#define wrap5(type, constant)                                                  \
  OSObject *IORegistryEntry::copyProperty(type *aKey,                          \
                                          const IORegistryPlane *plane,        \
                                          IOOptionBits options) constant {     \
...
wrap2(const OSSymbol, const)     // copyProperty() definition
    ...
    bool IORegistryEntry::propertyExists(const OSSymbol *aKey) {
  return NULL != getProperty(aKey);
}

bool IORegistryEntry::propertyExists(const OSString *aKey) {
  return NULL != getProperty(aKey);
}

bool IORegistryEntry::propertyExists(const char *aKey) {
  return NULL != getProperty(aKey);
}

bool IORegistryEntry::propertyHasValue(const OSSymbol *aKey,
...
bool IORegistryEntry::propertyHasValue(const OSString *aKey,
                                       const OSObject *value) {
  const OSObject *found;
  bool result;
...
bool IORegistryEntry::propertyHasValue(const char *aKey,
                                       const OSObject *value) {
  const OSObject *found;
  bool result;
...
bool IORegistryEntry::propertyHasValue(const char *aKey, const OSObject *value,
                                       const IORegistryPlane *plane,
                                       uint32_t options) const {
  const OSObject *found;
  bool result;
...
OSObject *IORegistryEntry::getProperty(const OSSymbol *aKey) const {
  OSObject *obj;
...
void IORegistryEntry::removeProperty(const OSSymbol *aKey) {
  PLOCK;
...
bool IORegistryEntry::setProperty(const OSSymbol *aKey, OSObject *anObject) {
  bool ret = false;
  ...
  if (anObject && strcmp(kIOKitDiagnosticsKey, aKey->getCStringNoCopy()) != 0) {
    if (ScanForAddrInObject(anObject, 0)) {
      IOLog("%s: IORegistryEntry name %s with key \"%s\" \n", __FUNCTION__,
            getName(0), aKey->getCStringNoCopy());
    }
  }
#endif
...
IOReturn IORegistryEntry::runPropertyAction(Action inAction, OSObject *target,
...
static IOReturn IORegistryEntryActionToBlock(OSObject *target, void *arg0,
                                             void *arg1, void *arg2,
                                             void *arg3) {
...
IOReturn IORegistryEntry::runPropertyActionBlock(ActionBlock block) {
  IOReturn res;
...
OSObject *IORegistryEntry::getProperty(const OSString *aKey) const {
...
OSObject *IORegistryEntry::getProperty(const char *aKey) const {
...
void IORegistryEntry::removeProperty(const OSString *aKey) {
  const OSSymbol *tmpKey = OSSymbol::withString(aKey);
  removeProperty(tmpKey);
  tmpKey->release();
}

void IORegistryEntry::removeProperty(const char *aKey) {
  const OSSymbol *tmpKey = OSSymbol::withCString(aKey);
  removeProperty(tmpKey);
  tmpKey->release();
}

bool IORegistryEntry::setProperty(const OSString *aKey, OSObject *anObject) {
  const OSSymbol *tmpKey = OSSymbol::withString(aKey);
...
bool IORegistryEntry::setProperty(const char *aKey, OSObject *anObject) {
  const OSSymbol *tmpKey = OSSymbol::withCString(aKey);
  bool ret = setProperty(tmpKey, anObject);
...
bool IORegistryEntry::setProperty(const char *aKey, const char *aString) {
  bool ret = false;
...
bool IORegistryEntry::setProperty(const char *aKey, bool aBoolean) {
  bool ret = false;
...
bool IORegistryEntry::setProperty(const char *aKey, unsigned long long aValue,
...
bool IORegistryEntry::setProperty(const char *aKey, void *bytes,
                                  unsigned int length) {
  bool ret = false;
...
OSObject *IORegistryEntry::setIndexedProperty(uint32_t index,
                                              OSObject *anObject) {
  OSObject **array;
  OSObject *prior;
  ...
  if (!array) {
    ...
    if (!os_atomic_cmpxchg(&reserved->fIndexedProperties, NULL, array,
                           release)) {
      IODelete(array, OSObject *, kIORegistryEntryIndexedPropertyCount);
      array = os_atomic_load(&reserved->fIndexedProperties, acquire);
    }
...
const OSSymbol *IORegistryEntry::copyName(const IORegistryPlane *plane) const {
  OSSymbol *sym = NULL;
...
const OSSymbol *
IORegistryEntry::copyLocation(const IORegistryPlane *plane) const {
  OSSymbol *sym = NULL;
...
void IORegistryEntry::setName(const OSSymbol *name,
  ...
  if (name) {
    ...
    if (gIOKitTrace && reserved && reserved->fRegistryEntryID) {
      ...
      kernel_debug_string(IODBG_IOREGISTRY(IOREGISTRYENTRY_NAME_STRING),
                          &str_id, name->getCStringNoCopy());
      KERNEL_DEBUG_CONSTANT(IODBG_IOREGISTRY(IOREGISTRYENTRY_NAME),
                            (uintptr_t)regID, (uintptr_t)(regID >> 32),
                            (uintptr_t)str_id, (uintptr_t)(str_id >> 32), 0);
    }
...
void IORegistryEntry::setName(const char *name, const IORegistryPlane *plane) {
...
void IORegistryEntry::setName(const OSString *name,
                              const IORegistryPlane *plane) {
...
bool IORegistryEntry::compareName(OSString *name, OSString **matched) const {
...
bool IORegistryEntry::compareNames(OSObject *names, OSString **matched) const {
  OSString *string;
  ...
  } while ((false == result) && iter &&
           (string = OSDynamicCast(OSString, iter->getNextObject())));
...
bool IORegistryEntry::compareName(OSString *name,
...
bool IORegistryEntry::compareNames(OSObject *names,
                                   OSSharedPtr<OSString> &matched) const {
  OSString *matchedRaw = NULL;
...
bool IORegistryEntry::getPath(char *path, int *length,
  ...
  strlcpy(nextComp.discard_bounds(), plane->nameKey->getCStringNoCopy(),
          len + 1);
  nextComp[len++] = ':';
  nextComp += len;
      ...
      *nextComp = 0;
      ...
      while (ok && ((--index) >= 0)) {
        ...
        if ((alias = entry->hasAlias(plane))) {
          ...
          // pointer is to the first argument, with next 2 arguments describing
          ...
          ok = entry->getPathComponent(nextComp.discard_bounds() + 1, &compLen,
                                       plane);
...
IORegistryEntry::getChildFromComponent(const char **opath,
  ...
  if (set) {
    ...
    for (index = 0; (entry = (IORegistryEntry *)set->getObject(index));
         index++) {
      cmp = path;
...
const OSSymbol *IORegistryEntry::hasAlias(const IORegistryPlane *plane,
  ...
  if (entry) {
    RLOCK;
    if ((iter =
             OSCollectionIterator::withCollection(entry->getPropertyTable()))) {
      while ((key = (OSSymbol *)iter->getNextObject())) {
        data = (OSData *)entry->getProperty(key);
        path = (const char *)data->getBytesNoCopy();
        if ((entry2 = IORegistryEntry::fromPath(path, plane, opath, length))) {
          if (this == entry2) {
            if (!bestKey || (bestKey->getLength() > key->getLength())) {
...
const char *IORegistryEntry::dealiasPath(const char **opath,
                                         const IORegistryPlane *plane) {
  IORegistryEntry *entry;
  OSData *data;
...
IORegistryEntry *IORegistryEntry::fromPath(const char *path,
  ...
  if ((alias = dealiasPath(&end, plane))) {
    ...
    aliasEntry =
        IORegistryEntry::fromPath(alias, plane, opath, &len, fromEntry);
    where = aliasEntry;
    if (where) {
      path = end;
    } else {
...
IORegistryEntry *IORegistryEntry::childFromPath(const char *path,
                                                const IORegistryPlane *plane,
                                                char *opath, int *len) {
...
bool IORegistryEntry::makeLink(IORegistryEntry *to, unsigned int relation,
                               const IORegistryPlane *plane) const {
  OSArray *links;
  bool result = false;

  if ((links = (OSArray *)registryTable()->getObject(plane->keys[relation]))) {
    ...
    result = (links != NULL);
    if (result) {
      result = registryTable()->setObject(plane->keys[relation], links);
      links->release();
    }
  }
...
void IORegistryEntry::breakLink(IORegistryEntry *to, unsigned int relation,
                                const IORegistryPlane *plane) const {
  OSArray *links;
  unsigned int index;
...
OSArray *
IORegistryEntry::getParentSetReference(const IORegistryPlane *plane) const {
  if (plane) {
    return (OSArray *)registryTable()->getObject(plane->keys[kParentSetIndex]);
  } else {
    return NULL;
  }
}
...
OSIterator *
IORegistryEntry::getParentIterator(const IORegistryPlane *plane) const {
  OSArray *links;
  OSIterator *iter;
...
IORegistryEntry::getChildSetReference(const IORegistryPlane *plane) const {
  if (plane) {
    return (OSArray *)registryTable()->getObject(plane->keys[kChildSetIndex]);
  } else {
    return NULL;
  }
}
...
IORegistryEntry *
IORegistryEntry::copyChildEntry(const IORegistryPlane *plane) const {
  IORegistryEntry *entry = NULL;
  OSArray *links;
...
IORegistryEntry *
IORegistryEntry::getChildEntry(const IORegistryPlane *plane) const {
  IORegistryEntry *entry;
...
void IORegistryEntry::applyToChildren(IORegistryEntryApplierFunction applier,
  ...
  UNLOCK;
  if (array) {
    for (index = 0; (next = (IORegistryEntry *)array->getObject(index));
         index++) {
    ...
    if (iter) {
      ...
      while ((key = (OSSymbol *)iter->getNextObject())) {
        ...
        if (!strncmp(key->getCStringNoCopy() + keysuffix,
                     kIORegPlaneParentSuffix, kIORegPlaneParentSuffixLen + 1)) {
          ret = true;
          break;
        }
      }
...
bool IORegistryEntry::attachToParent(IORegistryEntry *parent,
  ...
  if (traceName) {
    uint64_t str_id = 0;
    uint64_t __unused regID = getRegistryEntryID();
    kernel_debug_string(IODBG_IOREGISTRY(IOREGISTRYENTRY_NAME_STRING), &str_id,
                        getName());
    KERNEL_DEBUG_CONSTANT(IODBG_IOREGISTRY(IOREGISTRYENTRY_NAME),
                          (uintptr_t)regID, (uintptr_t)(regID >> 32),
                          (uintptr_t)str_id, (uintptr_t)(str_id >> 32), 0);
  ...
  OSCollectionIterator *iter = OSCollectionIterator::withCollection(ptable);
  if (iter) {
    const OSSymbol *key;

    while ((key = (OSSymbol *)iter->getNextObject())) {
      // Is object for key a collection?
      OSCollection *coll = OSDynamicCast(OSCollection, ptable->getObject(key));
...
unsigned int IORegistryEntry::getDepth(const IORegistryPlane *plane) const {
  ...
  while ((parents = next->getParentSetReference(plane))) {
    ...
    if (1 == count) {
      ...
      // painful
      maxParentDepth = 0;
      for (index = 0; (one = (IORegistryEntry *)parents->getObject(index));
           index++) {
...
OSDefineMetaClassAndStructors(IORegistryIterator, OSIterator)
    ...
    enum {
      kIORegistryIteratorInvalidFlag = 0x80000000
    };

/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */

IORegistryIterator *IORegistryIterator::iterateOver(
    IORegistryEntry *root, const IORegistryPlane *plane, IOOptionBits options) {
...
void IORegistryIterator::enterEntry(void) { enterEntry(plane); }
