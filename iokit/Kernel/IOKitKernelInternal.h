...
kern_return_t IOMemoryDescriptorMapAlloc(vm_map_t map, void *ref);
...
void IOKernelFreePhysical(kalloc_heap_t kheap, mach_vm_address_t address,
                          mach_vm_size_t size);
...
extern void bcopy_phys_with_options(addr64_t from, addr64_t to,
                                    vm_size_t nbytes, int options);
#endif /* __arm64__ */
...
enum { kIOPageAllocSignature = 'iopa' };

extern "C" void iopa_init(iopa_t *a);
extern "C" uintptr_t iopa_alloc(iopa_t *a, iopa_proc_t alloc,
                                kalloc_heap_t kheap, vm_size_t bytes,
                                vm_size_t balign);
...
extern "C" void IOKitKernelLogBuffer(const char *title, const void *buffer,
                                     size_t size,
...
extern "C" void
IOSetAPFSKeyStoreData(LIBKERN_CONSUMED IOMemoryDescriptor *data);
#endif
...
public:
  ...
  virtual bool
  serializeProperties(OSSerialize *serialize) const APPLE_KEXT_OVERRIDE;
};
