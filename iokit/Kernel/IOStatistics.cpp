...
IOStatistics::KextTreeHead IOStatistics::kextHead =
...
IOStatistics::KextAddressTreeHead IOStatistics::kextAddressHead =
...
RB_GENERATE(IOStatistics::KextAddressTree, KextNode, addressLink,
            kextAddressNodeCompare);

/* Class tree */

IOStatistics::ClassTreeHead IOStatistics::classHead =
...
int IOWorkLoopCounter::loadTagCompare(IOWorkLoopDependency *e1,
                                      IOWorkLoopDependency *e2) {
...
RB_GENERATE(IOWorkLoopCounter::DependencyTree, IOWorkLoopDependency, link,
            IOWorkLoopCounter::loadTagCompare);

/* sysctl stuff */

static int oid_sysctl(__unused struct sysctl_oid *oidp, __unused void *arg1,
...
SYSCTL_NODE(_debug, OID_AUTO, iokit_statistics, CTLFLAG_RW | CTLFLAG_LOCKED,
            NULL, "IOStatistics");
...
void IOStatistics::onKextLoad(OSKext *kext, kmod_info_t *kmod_info) {
  ...
  LOG(1,
      "IOStatistics::onKextLoad: %s, tag %d, address 0x%llx, address end "
      "0x%llx\n",
      kext->getIdentifierCString(), kmod_info->id, (uint64_t)kmod_info->address,
      (uint64_t)(kmod_info->address + kmod_info->size));
...
void IOStatistics::onKextUnload(OSKext *kext) {
  ...
  if (found) {
    ...
    panic("IOStatistics::onKextUnload: cannot find kext: %s",
          kext->getIdentifierCString());
  }
...
void IOStatistics::onClassAdded(OSKext *parentKext, OSMetaClass *metaClass) {
  ...
  if (foundKext) {
    ...
    panic("IOStatistics::onClassAdded: cannot find parent kext: %s",
          parentKext->getIdentifierCString());
  }
...
void IOStatistics::onClassRemoved(OSKext *parentKext, OSMetaClass *metaClass) {
  ...
  if (found) {
    ...
    panic("IOStatistics::onClassRemoved: cannot find class: %s",
          metaClass->getClassName());
  }
...
IOEventSourceCounter *IOStatistics::registerEventSource(OSObject *inOwner) {
  ...
  counter =
      kalloc_type(IOEventSourceCounter, (zalloc_flags_t)(Z_WAITOK | Z_ZERO));
  if (!counter) {
    return NULL;
  }
  ...
  /* Workaround for <rdar://problem/7158117> - create a dummy counter when
   * inOwner is bad. We use retainCount here as our best indication that the
   * pointer is awry.
   */
  ...
  if (!(createDummyCounter || found)) {
    panic("IOStatistics::registerEventSource: cannot find parent class: %s",
          inOwner->getMetaClass()->getClassName());
  }
...
void IOStatistics::unregisterEventSource(IOEventSourceCounter *counter) {
  ...
  if (counter->parentClass) {
    SLIST_REMOVE(&counter->parentClass->counterList, counter,
                 IOEventSourceCounter, link);
    registeredCounters--;
  }
...
void IOStatistics::unregisterWorkLoop(IOWorkLoopCounter *counter) {
  ...
  if (counter->parentKext) {
    SLIST_REMOVE(&counter->parentKext->workLoopList, counter, IOWorkLoopCounter,
                 link);
  }
...
IOStatistics::registerUserClient(IOUserClient *userClient) {
  ...
  counter =
      kalloc_type(IOUserClientCounter, (zalloc_flags_t)(Z_WAITOK | Z_ZERO));
  if (!counter) {
    return NULL;
  }
  ...
  if (found) {
    ...
    panic("IOStatistics::registerUserClient: cannot find parent class: %s",
          sought.metaClass->getClassName());
  }
...
void IOStatistics::unregisterUserClient(IOUserClientCounter *counter) {
  ...
  SLIST_REMOVE(&counter->parentClass->userClientList, counter,
               IOUserClientCounter, link);
  kfree_type(IOUserClientCounter, counter);

  IORWLockUnlock(lock);
}

void IOStatistics::attachWorkLoopEventSource(IOWorkLoopCounter *wlc,
                                             IOEventSourceCounter *esc) {
  if (!wlc) {
    return;
  }
  ...
  if (NULL == RB_INSERT(IOWorkLoopCounter::DependencyTree, &wlc->dependencyHead,
...
void IOStatistics::detachWorkLoopEventSource(IOWorkLoopCounter *wlc,
                                             IOEventSourceCounter *esc) {
  ...
  found =
      RB_FIND(IOWorkLoopCounter::DependencyTree, &wlc->dependencyHead, &sought);
  if (found) {
...
int IOStatistics::getStatistics(sysctl_req *req) {
  ...
  /* Work out how much we need to allocate. IOStatisticsKext is of variable
   * size. */
  calculatedSize = sizeof(IOStatisticsHeader) + sizeof(IOStatisticsGlobal) +
                   (sizeof(IOStatisticsKext) * loadedKexts) +
  ...
  buffer =
      (char *)kalloc_data(calculatedSize, (zalloc_flags_t)(Z_WAITOK | Z_ZERO));
  if (!buffer) {
    error = ENOMEM;
    goto exit;
  }
  ...
  LOG(2,
      "IOStatistics::getStatistics - calculatedSize 0x%x, kexts 0x%x, classes "
      "0x%x.\n",
...
int IOStatistics::getUserClientStatistics(sysctl_req *req) {
  ...
  calculatedSize = sizeof(IOStatisticsUserClientHeader) +
                   sizeof(IOStatisticsUserClientCall) *
                       IOKIT_STATISTICS_RECORDED_USERCLIENT_PROCS * loadedKexts;
  ...
  LOG(2,
      "IOStatistics::getUserClientStatistics - requesting kext w/load tag: "
      "%d\n",
      requestedLoadTag);
  ...
  if (size) {
    error =
        SYSCTL_OUT(req, buffer, sizeof(IOStatisticsUserClientHeader) + size);
  } else {
    error = EINVAL;
  }
...
uint32_t IOStatistics::copyKextStatistics(IOStatisticsKext *stats) {
  ...
  RB_FOREACH(ke, KextTree, &kextHead) {
    ...
    stats =
        (IOStatisticsKext *)((void *)((char *)stats + sizeof(IOStatisticsKext) +
                                      (ke->classes * sizeof(uint32_t))));
  }

  return sizeof(IOStatisticsKext) * loadedKexts +
         sizeof(uint32_t) * registeredClasses;
}
...
uint32_t IOStatistics::copyMemoryStatistics(IOStatisticsMemory *stats) {
  ...
  RB_FOREACH(ke, KextTree, &kextHead) {
    ...
    stats->allocatedAlignedSize =
        ke->memoryCounters[kIOStatisticsMallocAligned];
    stats->freedAlignedSize = ke->memoryCounters[kIOStatisticsFreeAligned];
    stats->allocatedContiguousSize =
        ke->memoryCounters[kIOStatisticsMallocContiguous];
    stats->freedContiguousSize =
        ke->memoryCounters[kIOStatisticsFreeContiguous];
    stats->allocatedPageableSize =
        ke->memoryCounters[kIOStatisticsMallocPageable];
...
uint32_t IOStatistics::copyCounterStatistics(IOStatisticsCounter *stats) {
  ...
  RB_FOREACH(ke, KextTree, &kextHead) {
    SLIST_FOREACH(ce, &ke->classList, lLink) {
      ...
      IOStatisticsInterruptEventSources *iec =
          &stats->interruptEventSourceStatistics;
      IOStatisticsInterruptEventSources *fiec =
          &stats->filterInterruptEventSourceStatistics;
      ...
      IOStatisticsDerivedEventSources *dec =
          &stats->derivedEventSourceStatistics;
...
IOStatistics::copyKextIdentifiers(IOStatisticsKextIdentifier *kextIDs) {
  ...
  RB_FOREACH(ke, KextTree, &kextHead) {
    strncpy(kextIDs->identifier, ke->kext->getIdentifierCString(),
...
uint32_t IOStatistics::copyClassNames(IOStatisticsClassName *classNames) {
  ...
  RB_FOREACH(ke, KextTree, &kextHead) {
    SLIST_FOREACH(ce, &ke->classList, lLink) {
      strncpy(classNames->name, ce->metaClass->getClassName(),
...
uint32_t IOStatistics::copyWorkLoopStatistics(IOStatisticsWorkLoop *stats) {
  ...
  RB_FOREACH(ke, KextTree, &kextHead) {
    SLIST_FOREACH(wlc, &ke->workLoopList, link) {
      ...
      RB_FOREACH(dependentNode, IOWorkLoopCounter::DependencyTree,
                 &wlc->dependencyHead) {
        stats->dependentKextLoadTags[stats->dependentKexts] =
            dependentNode->loadTag;
        stats->dependentKexts++;
      }

      size = sizeof(IOStatisticsWorkLoop) +
             (sizeof(uint32_t) * stats->dependentKexts);
...
uint32_t
IOStatistics::copyUserClientStatistics(IOStatisticsUserClientHeader *stats,
                                       uint32_t loadTag) {
  ...
  TAILQ_FOREACH(processEntry, &found->userClientCallList, link) {
    strncpy(stats->userClientCalls[procs].processName,
            processEntry->processName, kIOStatisticsProcessNameLength);
...
void IOStatistics::storeUserClientCallInfo(IOUserClient *userClient,
  ...
  obj = userClient->copyProperty("IOUserClientCreator", gIOServicePlane,
