...
#define kDisableWorkloadClassThreadPolicyValue                                 \
...
static inline IOReturn stringToFlags(const OSString &str, UInt32 &flags,
                                     const struct FlagMap *map, size_t mapLen) {
...
static inline IOReturn flagsToString(const UInt32 flags,
...
static IOReturn unparseWorkIntervalType(const UInt32 createFlags,
                                        OSSharedPtr<OSString> &typeStr) {
  IOReturn ret =
      flagsToString(createFlags, typeStr, typeMap, ARRAY_LEN(typeMap));
...
static IOReturn parseWorkIntervalType(const OSSymbol &id,
                                      const OSObject *typeObj,
                                      UInt32 &createFlags) {
  ...
  IOReturn ret =
      stringToFlags(*typeStr, createFlags, typeMap, ARRAY_LEN(typeMap));
  if (ret != kIOReturnSuccess) {
...
static IOReturn parseWorkloadClass(const OSSymbol &id,
                                   const OSObject *wlClassObj,
                                   wi_class_t &wiClass) {
  ...
  WLC_LOG("%s: unknown %s: \"%s\"\n", id.getCStringNoCopy(), kWorkloadClassKey,
          wlClass->getCStringNoCopy());
  return kIOReturnError;
}

static IOReturn parseCriticalityOffset(const OSSymbol &id,
                                       const wi_class_t wiClass,
                                       const OSObject *cOffsetObj,
                                       uint8_t &criticalityOffset) {
  if (wiClass != WI_CLASS_SYSTEM_CRITICAL &&
      wiClass != WI_CLASS_REALTIME_CRITICAL &&
      wiClass != WI_CLASS_BEST_EFFORT && wiClass != WI_CLASS_APP_SUPPORT &&
...
static IOReturn parseFlags(const OSSymbol &id, const OSObject *flagsObj,
                           UInt32 &threadGroupFlags,
                           UInt32 &workIntervalFlags) {
...
static IOReturn parsePhases(workload_config_ctx_t *ctx, const OSSymbol &id,
                            OSObject *phasesObj) {
  ...
  phases->iterateObjects(^bool(const OSSymbol *phase, OSObject *value) {
    ...
    ret = parseCriticalityOffset(
        id, wiClass, dict->getObject(kCriticalityOffsetKey), criticalityOffset);
    if (ret != kIOReturnSuccess) {
      return true;
    }
    ...
    ret = parseFlags(id, dict->getObject(kFlagsKey), threadGroupFlags,
                     workIntervalFlags);
    if (ret != kIOReturnSuccess) {
      return true;
    }
    ...
    ret = workload_config_insert(ctx, id.getCStringNoCopy(),
                                 phase->getCStringNoCopy(), &config);
    if (ret != kIOReturnSuccess) {
      WLC_LOG("%s: failed to add \"%s\" phase\n", id.getCStringNoCopy(),
              phase->getCStringNoCopy());
      return true;
    }
...
static IOReturn parseRoot(const OSSymbol &id, const OSObject *rootDict,
                          OSString *&defaultPhase) {
  ...
  if (defaultPhase == nullptr) {
    WLC_LOG("%s: failed to find \"" kDefaultPhaseKey "\" in \"" kRootKey
            "\" dictionary\n",
            id.getCStringNoCopy());
    return kIOReturnError;
  }
  ...
  if (defaultPhase->getLength() == 0) {
    WLC_LOG("%s: \"" kDefaultPhaseKey " \" is empty in \"" kRootKey
            "\" dictionary\n",
            id.getCStringNoCopy());
    return kIOReturnError;
  }
...
static IOReturn parseWorkloadIDTable(workload_config_ctx_t *ctx,
                                     OSDictionary *IDTable) {
  /*
...
static IOReturn
parseWorkloadIDConfigurationFlags(workload_config_ctx_t *ctx,
                                  const OSObject *idTableFlagsObj) {
  /* Optional, so just carry on if not found. */
  ...
  if (idTableFlags == nullptr) {
    WLC_LOG("failed to parse \"" kWorkloadIDConfigurationFlagsKey "\"\n");
    return kIOReturnError;
  }
...
unparseWorkloadIDConfigurationFlags(OSSharedPtr<OSDictionary> &plist) {
  ...
  OSSharedPtr<OSString> flag =
      OSString::withCString(kDisableWorkloadClassThreadPolicyValue);
...
extern "C" {
extern IOReturn IOParseWorkloadConfig(workload_config_ctx_t *, const char *,
                                      size_t);
extern IOReturn IOUnparseWorkloadConfig(char *, size_t *);
}

/* Called locked. */
IOReturn IOParseWorkloadConfig(workload_config_ctx_t *ctx, const char *buffer,
                               size_t size) {
  ...
  ret = parseWorkloadIDConfigurationFlags(
      ctx, dict->getObject(kWorkloadIDConfigurationFlagsKey));
  if (ret != kIOReturnSuccess) {
    goto out;
  }
 ...
 */
IOReturn IOUnparseWorkloadConfig(char *buffer, size_t *size) {
  assert(size != nullptr);
  ...
  workload_config_iterate(^(const char *id_str, const void *config) {
    ...
    workload_config_phases_iterate(config, ^(const char *phase_str,
      ...
      if (wc->wc_class != WI_CLASS_NONE) {
        ...
        OSSharedPtr<OSString> wClass =
            OSString::withCString(wlClassData[wc->wc_class].name);
        if (wClass == nullptr ||
            !phaseData->setObject(kWorkloadClassKey, wClass)) {
          ret = kIOReturnError;
          return true;
        }
      }
      ...
      if (wc->wc_class_offset > 0) {
        OSSharedPtr<OSNumber> criticalityOffset =
            OSNumber::withNumber(wc->wc_class_offset, 8);
      ...
      if (unparseWorkIntervalType(wc->wc_create_flags, type) !=
              kIOReturnSuccess ||
      ...
      if ((wc->wc_thread_group_flags & THREAD_GROUP_FLAGS_STRICT_TIMERS) != 0) {
        OSSharedPtr<OSString> StrictTimersStr =
            OSString::withCString(kStrictTimersValue);
      ...
      if ((wc->wc_flags & WORK_INTERVAL_WORKLOAD_ID_COMPLEXITY_ALLOWED) != 0) {
        ...
        if (WIComplexityAllowedStr == nullptr ||
            !flags->setObject(WIComplexityAllowedStr)) {
          ret = kIOReturnError;
          return true;
        }
      }
