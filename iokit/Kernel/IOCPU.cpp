...
void IOCPUInitialize(void) {
  ...
  gIOCPUStateNames[kIOCPUStateStopped] = OSString::withCStringNoCopy("Stopped");
  gIOCPUStateNames[kIOCPUStateRunning] = OSString::withCStringNoCopy("Running");
}
...
void PE_cpu_start_internal(cpu_id_t target, vm_offset_t start_paddr,
...
kern_return_t PE_cpu_perfmon_interrupt_install_handler(
...
void PE_cpu_perfmon_interrupt_enable(cpu_id_t target, boolean_t enable) {
  ...
  if (enable) {
    targetCPU->getProvider()->registerInterrupt(
        1, targetCPU, (IOInterruptAction)(void (*)(void))pmi_handler, NULL);
...
bool PE_cpu_power_check_kdp(int cpu_id) { return true; }
  ...
  for (cnt = 0; cnt < numCPUs; cnt++) {
    ...
    if (target->getCPUNumber() != (UInt32)master_cpu) {
      if (target->getCPUState() == kIOCPUStateRunning) {
        panic("Spurious wakeup of cpu %u",
              (unsigned int)(target->getCPUNumber()));
      }
...
static bool is_IOCPU_disabled(void) { return false; }
#else  /* !USE_APPLEARMSMP */
static bool is_IOCPU_disabled(void) { return true; }
#endif /* !USE_APPLEARMSMP */
...
bool IOCPU::start(IOService *provider) {
  ...
  if (gPEClockFrequencyInfo.bus_frequency_hz < 0x100000000ULL) {
    OSSharedPtr<OSData> busFrequency = OSData::withBytesNoCopy(
        (void *)&gPEClockFrequencyInfo.bus_clock_rate_hz, 4);
    provider->setProperty("bus-frequency", busFrequency.get());
  } else {
    OSSharedPtr<OSData> busFrequency = OSData::withBytesNoCopy(
        (void *)&gPEClockFrequencyInfo.bus_frequency_hz, 8);
    provider->setProperty("bus-frequency", busFrequency.get());
  }

  if (gPEClockFrequencyInfo.cpu_frequency_hz < 0x100000000ULL) {
    OSSharedPtr<OSData> cpuFrequency = OSData::withBytesNoCopy(
        (void *)&gPEClockFrequencyInfo.cpu_clock_rate_hz, 4);
    provider->setProperty("clock-frequency", cpuFrequency.get());
  } else {
    OSSharedPtr<OSData> cpuFrequency = OSData::withBytesNoCopy(
        (void *)&gPEClockFrequencyInfo.cpu_frequency_hz, 8);
    provider->setProperty("clock-frequency", cpuFrequency.get());
  }

  OSSharedPtr<OSData> timebaseFrequency = OSData::withBytesNoCopy(
      (void *)&gPEClockFrequencyInfo.timebase_frequency_hz, 4);
...
void IOCPU::detach(IOService *provider) {
  if (is_IOCPU_disabled()) {
    return;
  }
...
UInt32 IOCPU::getCPUNumber(void) { return _cpuNumber; }
...
UInt32 IOCPU::getCPUState(void) { return _cpuState; }
...
OSArray *IOCPU::getCPUGroup(void) { return _cpuGroup.get(); }

UInt32 IOCPU::getCPUGroupSize(void) { return _cpuGroup->getCount(); }

processor_t IOCPU::getMachProcessor(void) { return machProcessor; }
...
IOReturn IOCPUInterruptController::initCPUInterruptController(int sources,
  ...
  vectors = (IOInterruptVector *)zalloc_permanent(
      numSources * sizeof(IOInterruptVector), ZALIGN(IOInterruptVector));
...
IOReturn IOCPUInterruptController::registerInterrupt(IOService *nub, int source,
                                                     void *target,
