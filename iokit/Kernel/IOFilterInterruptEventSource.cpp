...
OSDefineMetaClassAndStructors(IOFilterInterruptEventSource,
 ...
 */
bool IOFilterInterruptEventSource::init(OSObject *inOwner, Action inAction,
...
IOFilterInterruptEventSource ::filterInterruptEventSource(OSObject *inOwner,
                                                          Action inAction,
                                                          Filter inFilterAction,
                                                          IOService *inProvider,
                                                          int inIntIndex) {
  OSSharedPtr<IOFilterInterruptEventSource> me =
      OSMakeShared<IOFilterInterruptEventSource>();
...
IOFilterInterruptEventSource ::filterInterruptEventSource(
    OSObject *inOwner, IOService *inProvider, int inIntIndex,
    ActionBlock inAction, FilterBlock inFilterAction) {
  OSSharedPtr<IOFilterInterruptEventSource> me =
      OSMakeShared<IOFilterInterruptEventSource>();
  ...
  if (me && !me->init(inOwner, (Action)NULL, (Filter)(void (*)(void))filter,
...
void IOFilterInterruptEventSource::normalInterruptOccurred(void * /*refcon*/,
                                                           IOService * /*prov*/,
                                                           int /*source*/) {
  bool filterRes;
  ...
  if (trace) {
    IOTimeStampStartConstant(
        IODBG_INTES(IOINTES_FILTER), VM_KERNEL_UNSLIDE(filterAction),
  ...
  if (IOInterruptEventSource::reserved->statistics) {
    ...
    if (IOInterruptEventSource::reserved->statistics->enablePrimaryTimestamp) {
      IOInterruptEventSource::reserved->statistics->primaryTimestamp =
          startTime;
    }
  }
  ...
  if (IOInterruptEventSource::reserved->statistics) {
    if (IA_GET_STATISTIC_ENABLED(kInterruptAccountingFirstLevelCountIndex)) {
      IA_ADD_VALUE(
          &IOInterruptEventSource::reserved->statistics
    ...
    if (IA_GET_STATISTIC_ENABLED(kInterruptAccountingFirstLevelTimeIndex)) {
      ...
      IA_ADD_VALUE(
          &IOInterruptEventSource::reserved->statistics
               ->interruptStatistics[kInterruptAccountingFirstLevelTimeIndex],
          endTime - startTime);
    }
  }
  ...
  if (trace) {
    IOTimeStampEndConstant(IODBG_INTES(IOINTES_FILTER),
                           VM_KERNEL_ADDRHIDE(filterAction),
                           VM_KERNEL_ADDRHIDE(owner), VM_KERNEL_ADDRHIDE(this),
                           VM_KERNEL_ADDRHIDE(workLoop));
  }
...
void IOFilterInterruptEventSource::disableInterruptOccurred(void * /*refcon*/,
                                                            IOService *prov,
                                                            int source) {
  bool filterRes;
  uint64_t startTime = 0;
  ...
  if (trace) {
    IOTimeStampEndConstant(IODBG_INTES(IOINTES_FILTER),
                           VM_KERNEL_UNSLIDE(filterAction),
                           VM_KERNEL_ADDRHIDE(owner), VM_KERNEL_ADDRHIDE(this),
                           VM_KERNEL_ADDRHIDE(workLoop));
  }
