...
IOMemoryDescriptor *IOGetBootKeyStoreData(void);
void IOSetKeyStoreData(IOMemoryDescriptor *data);
...
IOMemoryDescriptor *IOGetBootKeyStoreData(void) {
  ...
  DEBG("%s: data at address %u size %u\n", __func__, args->keyStoreDataStart,
  ...
  memoryDescriptor =
      IOMemoryDescriptor::withOptions(&ranges, 1, 0, NULL, options);
...
IOMemoryDescriptor *IOGetAPFSKeyStoreData() {
  ...
  // Looks like there was no in-memory key and it's the first call - try
  // boot_args
  boot_args *args = (boot_args *)PE_state.bootArgs;

  DEBG("%s: data at address %u size %u\n", __func__, args->apfsDataStart,
       args->apfsDataSize);
  ...
  const IOOptionBits options =
      kIODirectionInOut | kIOMemoryTypePhysical64 | kIOMemoryMapperNone;
...
IOMemoryDescriptor *IOGetARVRootHashData(void) {
  ...
  DEBG("%s: data at address %llu size %llu\n", __func__, args->arvRootHashStart,
       args->arvRootHashSize);
  if (args->arvRootHashStart == 0) {
    return NULL;
  }

  // We have the root hash in the boot_args, create IOMemoryDescriptor for the
  // blob
  IOAddressRange ranges;
...
IOMemoryDescriptor *IOGetBaseSystemARVRootHashData(void) {
  ...
  DEBG("%s: data at address %llu size %llu\n", __func__,
       args->bsARVRootHashStart, args->bsARVRootHashSize);
  if (args->bsARVRootHashStart == 0) {
    return NULL;
  }

  // We have the base system root hash in the boot_args, create
  // IOMemoryDescriptor for the blob
  IOAddressRange ranges;
...
IOMemoryDescriptor *IOGetARVManifestData(void) {
  ...
  DEBG("%s: data at address %llu size %llu\n", __func__, args->arvManifestStart,
       args->arvManifestSize);
  if (args->arvManifestStart == 0) {
    return NULL;
  }

  // We have the manifest in the boot_args, create IOMemoryDescriptor for the
  // blob
  IOAddressRange ranges;
...
IOMemoryDescriptor *IOGetBaseSystemARVManifestData(void) {
  ...
  DEBG("%s: data at address %llu size %llu\n", __func__,
       args->bsARVManifestStart, args->bsARVManifestSize);
