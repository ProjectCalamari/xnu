...
OSDefineMetaClassAndStructorsWithZone(IOGuardPageMemoryDescriptor,
                                      IOGeneralMemoryDescriptor,
                                      ZC_ZFREE_CLEARMEM);

OSSharedPtr<IOGuardPageMemoryDescriptor>
IOGuardPageMemoryDescriptor::withSize(vm_size_t size) {
  OSSharedPtr<IOGuardPageMemoryDescriptor> me =
      OSMakeShared<IOGuardPageMemoryDescriptor>();
...
bool IOGuardPageMemoryDescriptor::initWithSize(vm_size_t size) {
  mach_vm_address_t address;
  kern_return_t kr;
  IOOptionBits iomdOptions =
      kIOMemoryTypeVirtual64 | kIOMemoryAsReference | kIODirectionOutIn;
  ...
  if (!super::initWithOptions(_ranges.v64, 1, 0, kernel_task, iomdOptions,
                              NULL)) {
    return false;
  }
...
void IOGuardPageMemoryDescriptor::free() {
  if (_buffer) {
...
IOReturn IOGuardPageMemoryDescriptor::doMap(vm_map_t addressMap,
                                            IOVirtualAddress *atAddress,
                                            IOOptionBits options,
                                            IOByteCount sourceOffset,
                                            IOByteCount length) {
  IOReturn ret =
      super::doMap(addressMap, atAddress, options, sourceOffset, length);
  if (ret == kIOReturnSuccess) {
    ...
    kern_return_t kr =
        mach_vm_protect(map, address, length, true, VM_PROT_NONE);
    if (kr != KERN_SUCCESS) {
