...
IOHistogramReporter::with(IOService *reportingService,
                          ...
                          const char *channelName, IOReportUnit unit,
                          int nSegments, IOHistogramSegmentConfig *config) {
  OSSharedPtr<IOHistogramReporter> reporter =
      OSMakeShared<IOHistogramReporter>();
  ...
  if (reporter) {
    ...
    if (reporter->initWith(reportingService, categories, channelID,
                           tmpChannelName.get(), unit, nSegments, config)) {
      return reporter;
    }
  }
...
bool IOHistogramReporter::initWith(IOService *reportingService,
                                   IOReportCategories categories,
                                   uint64_t channelID,
                                   const OSSymbol *channelName,
                                   IOReportUnit unit, int nSegments,
  ...
  _histogramSegmentsConfig =
      (IOHistogramSegmentConfig *)IOMallocData(configSize);
  ...
  IORLOG("%s %d segments for a total dimension of %d elements", __func__,
         _nChannels, _nElements);
  ...
  for (cnt = 0; cnt < _segmentCount; cnt++) {
    ...
    for (cnt2 = 0;
         cnt2 < (int)_histogramSegmentsConfig[cnt].segment_bucket_count;
         cnt2++) {
      if (cnt3 >= _nElements) {
      ...
      if (previousBucketBound >= bucketBound) {
        IORLOG("Histogram ERROR: bucket bound does not increase linearly "
               "(segment %u / bucket # %u)",
               cnt, cnt2);
        result = false;
        goto finish;
...
void IOHistogramReporter::free(void) {
  if (_bucketBounds) {
...
OSSharedPtr<IOReportLegendEntry> IOHistogramReporter::handleCreateLegend(void) {
...
IOReturn IOHistogramReporter::overrideBucketValues(unsigned int index,
...
int IOHistogramReporter::tallyValue(int64_t value) {
  int result = -1;
  ...
  if (copyElementValues(element_index, (IOReportElementValues *)&hist_values) !=
      kIOReturnSuccess) {
    goto finish;
  }
...
/* static */ OSPtr<IOReportLegendEntry> IOHistogramReporter::createLegend(
    uint64_t channelID, const char *channelName, int segmentCount,
    IOHistogramSegmentConfig *config, IOReportCategories categories,
    IOReportUnit unit) {
  OSSharedPtr<IOReportLegendEntry> legendEntry;
  OSSharedPtr<OSData> tmpConfigData;
  OSDictionary *tmpDict; // no refcount
  ...
  legendEntry =
      IOReporter::legendWith(&channelID, &channelName, 1, channelType, unit);
  if (!legendEntry) {
    return nullptr;
  }

  PREFL_MEMOP_PANIC(segmentCount, IOHistogramSegmentConfig);
  tmpConfigData = OSData::withBytes(
      config, (unsigned)segmentCount * sizeof(IOHistogramSegmentConfig));
  if (!tmpConfigData) {
    return nullptr;
  }
