...
extern int __doprnt(const char *fmt, va_list argp, void (*putc)(int, void *),
                    void *arg, int radix, int is_log);
 ...
 * Binary compatibility is not guaranteed for kexts that reference these
 * symbols.
 */
...
KMEM_RANGE_REGISTER_STATIC(gIOKitPageableFixed, &gIOKitPageableFixedRange,
                           kIOPageableMapSize);
void IOLibInit(void) {
  ...
  gIOMallocTracking = IOTrackingQueueAlloc(kIOMallocTrackingName, 0, 0, 0,
                                           kIOTrackingQueueTypeAlloc, 37);
  gIOWireTracking =
      IOTrackingQueueAlloc(kIOWireTrackingName, 0, 0, page_size, 0, 0);

  size_t mapCaptureSize =
      (kIOTracking & gIOKitDebug) ? page_size : (1024 * 1024);
  gIOMapTracking = IOTrackingQueueAlloc(
      kIOMapTrackingName, 0, 0, mapCaptureSize,
      kIOTrackingQueueTypeDefaultOn | kIOTrackingQueueTypeMap |
          kIOTrackingQueueTypeUser,
      0);
#endif

  gIOKitPageableMap.map =
      kmem_suballoc(kernel_map, &gIOKitPageableFixedRange.min_address,
                    kIOPageableMapSize, VM_MAP_CREATE_PAGEABLE,
  ...
  gIOMallocContiguousEntriesLock =
      lck_mtx_alloc_init(IOLockGroup, LCK_ATTR_NULL);
...
IOThread IOCreateThread(IOThreadFunc fcn, void *arg) {
  ...
  result =
      kernel_thread_start((thread_continue_t)(void (*)(void))fcn, arg, &thread);
...
void IOExitThread(void) { (void)thread_terminate(current_thread()); }
...
#if IOTRACKING
#define sizeofIOLibMallocHeader                                                \
  (sizeof(IOLibMallocHeader) - (TRACK_ALLOC ? 0 : sizeof(IOTrackingAddress)))
#else
...
__typed_allocators_ignore_push // allocator implementation
    ...
    void *(IOMalloc_internal)(struct kalloc_heap * kheap, vm_size_t size,
...
void IOFree_internal(struct kalloc_heap *kheap, void *inAddress,
                     vm_size_t size) {
  void *address;
  ...
  if ((address = inAddress)) {
    ...
    if (TRACK_ALLOC) {
      IOLibMallocHeader *hdr;
      struct ptr_reference {
        void *ptr;
      };
      volatile struct ptr_reference ptr;

      // we're about to block in IOTrackingRemove(), make sure the original
      // pointer exists in memory or a register for leak scanning to find
      ptr.ptr = inAddress;
...
void *IOMalloc_external(vm_size_t size);
void *IOMalloc_external(vm_size_t size) {
...
void *IOMallocZero_external(vm_size_t size);
void *IOMallocZero_external(vm_size_t size) {
...
#if IOTRACKING
#define sizeofIOLibPageMallocHeader                                            \
  (sizeof(IOLibPageMallocHeader) -                                             \
   (TRACK_ALLOC ? 0 : sizeof(IOTrackingAddress)))
#else
...
IOMallocAlignedSetHdr(IOLibPageMallocHeader *hdr, mach_vm_size_t alignMask,
  ...
  offset = (mach_vm_offset_t)ptrauth_sign_unauthenticated(
      (void *)offset, ptrauth_key_process_independent_data,
      ptrauth_blend_discriminator(
          (void *)(alignedStart | alignMask),
...
__abortlike static void IOMallocAlignedHdrCorruptionPanic(
    mach_vm_offset_t offset, mach_vm_size_t alignMask,
    mach_vm_address_t alignedStart, vm_size_t size) {
  ...
  if (os_add3_overflow(address, sizeofIOLibPageMallocHeader, alignMask,
    ...
    panic("alignMask 0x%llx overflows recalAlignedStart %p for provided addr "
          "%p",
          alignMask, (void *)recalAlignedStart, (void *)alignedStart);
  }
  ...
  if (offset < sizeofIOLibPageMallocHeader) {
    panic("Offset %zd doesn't accomodate IOLibPageMallocHeader for aligned "
          "addr %p",
          (size_t)offset, (void *)alignedStart);
  }
...
IOMallocAlignedGetAddress(IOLibPageMallocHeader *hdr,
                          mach_vm_address_t alignedStart, vm_size_t *size) {
  mach_vm_address_t address = 0;
  mach_vm_address_t recalAlignedStart = 0;
...
#if __has_feature(ptrauth_calls)
  offset = (mach_vm_offset_t)ptrauth_auth_data(
      (void *)offset, ptrauth_key_process_independent_data,
      ptrauth_blend_discriminator(
          (void *)(alignedStart | alignMask),
          OS_PTRAUTH_DISCRIMINATOR("IOLibPageMallocHeader.allocationOffset")));
...
void *(IOMallocAligned_internal)(struct kalloc_heap * kheap, vm_size_t size,
  ...
  if (size > adjustedSize) {
    ...
    kr = kernel_memory_allocate(kernel_map, &address, size, alignMask,
                                kma_flags, IOMemoryTag(kernel_map));
    if (KERN_SUCCESS != kr) {
      address = 0;
    }
#if IOTRACKING
    ...
    if (adjustedSize >= page_size) {
      kr = kmem_alloc(kernel_map, &allocationAddress, adjustedSize, kma_flags,
                      IOMemoryTag(kernel_map));
      if (KERN_SUCCESS != kr) {
        allocationAddress = 0;
      }
    } else {
      allocationAddress = (vm_address_t)kheap_alloc(
          kheap, adjustedSize,
          Z_VM_TAG(Z_WAITOK | flags, VM_KERN_MEMORY_IOKIT));
    }
...
void IOFreeAligned_internal(kalloc_heap_t kheap, void *address,
                            vm_size_t size) {
  ...
  if (adjustedSize >= page_size) {
    ...
    allocationAddress = IOMallocAlignedGetAddress(
        hdr, (mach_vm_address_t)address, &adjustedSize);
...
void *IOMallocAligned_external(vm_size_t size, vm_size_t alignment);
void *IOMallocAligned_external(vm_size_t size, vm_size_t alignment) {
...
__typed_allocators_ignore_pop
    ...
    void
    IOKernelFreePhysical(kalloc_heap_t kheap, mach_vm_address_t address,
                         mach_vm_size_t size) {
  vm_address_t allocationAddress;
  vm_size_t adjustedSize;
  IOLibPageMallocHeader *hdr;
...
mach_vm_address_t IOKernelAllocateWithPhysicalRestrict(
    kalloc_heap_t kheap, mach_vm_size_t size, mach_vm_address_t maxPhys,
    mach_vm_size_t alignment, bool contiguous, bool noSoftLimit) {
  kern_return_t kr;
  ...
  if (os_mul_and_add_overflow(2, size, sizeofIOLibPageMallocHeader,
                              &adjustedSize)) {
    return 0;
  }

  contiguous =
      (contiguous && (adjustedSize > page_size)) || (alignment > page_size);
  ...
  if (contiguous || maxPhys) {
    ...
    if (contiguous || maxPhys) {
      kr = kmem_alloc_contig(kernel_map, &virt, size, alignMask,
                             (ppnum_t)atop(maxPhys), (ppnum_t)atop(alignMask),
                             options, IOMemoryTag(kernel_map));
    } else {
      kr = kernel_memory_allocate(kernel_map, &virt, size, alignMask, options,
                                  IOMemoryTag(kernel_map));
    }
    if (KERN_SUCCESS == kr) {
      address = virt;
    ...
    __typed_allocators_ignore_push // allocator implementation
        allocationAddress = (mach_vm_address_t)kheap_alloc(
            kheap, adjustedSize, Z_VM_TAG_BT(zflags, VM_KERN_MEMORY_IOKIT));
    __typed_allocators_ignore_pop
...
void *IOMallocContiguous(vm_size_t size, vm_size_t alignment,
  ...
  if (!physicalAddress) {
    address = IOKernelAllocateWithPhysicalRestrict(
        KHEAP_DEFAULT, size, 0 /*maxPhys*/, alignment, true,
        false /* noSoftLimit */);
  } else {
    do {
      ...
      _IOMallocContiguousEntry *entry = IOMallocType(_IOMallocContiguousEntry);
      if (!entry) {
        bmd->release();
        break;
      }
...
void IOFreeContiguous(void *_address, vm_size_t size) {
  ...
  lck_mtx_lock(gIOMallocContiguousEntriesLock);
  queue_iterate(&gIOMallocContiguousEntries, entry, _IOMallocContiguousEntry *,
                link) {
...
kern_return_t IOIteratePageableMaps(vm_size_t size,
                                    IOIteratePageableMapsCallback callback,
                                    void *ref) {
...
#if defined(__x86_64__)
static uintptr_t IOMallocOnePageablePage(kalloc_heap_t kheap __unused,
                                         iopa_t *a) {
...
static void *IOMallocPageableInternal(vm_size_t size, vm_size_t alignment,
                                      bool zeroed) {
  void *addr;
  ...
  if (size >= (page_size - 4 * gIOPageAllocChunkBytes) ||
    ...
    addr =
        ((void *)iopa_alloc(&gIOPageablePageAllocator, &IOMallocOnePageablePage,
                            KHEAP_DEFAULT, size, (uint32_t)alignment));
    if (addr && zeroed) {
      bzero(addr, size);
    }
  }
...
void IOFreePageable(void *address, vm_size_t size) {
  ...
  if (size < (page_size - 4 * gIOPageAllocChunkBytes)) {
    address =
        (void *)iopa_free(&gIOPageablePageAllocator, (uintptr_t)address, size);
    size = page_size;
  }
  if (address) {
...
void *IOMallocData_external(vm_size_t size) {
  return IOMalloc_internal(GET_KEXT_KHEAP_DATA(), size, Z_VM_TAG_BT_BIT);
}

void *IOMallocZeroData_external(vm_size_t size);
void *IOMallocZeroData_external(vm_size_t size) {
...
static uintptr_t iopa_allocinpage(iopa_page_t *pa, uint32_t count,
                                  uint64_t align) {
  uint32_t n, s;
...
uintptr_t iopa_alloc(iopa_t *a, iopa_proc_t alloc, kalloc_heap_t kheap,
                     vm_size_t bytes, vm_size_t balign) {
  static const uint64_t align_masks[] = {
      0xFFFFFFFFFFFFFFFF, 0xAAAAAAAAAAAAAAAA, 0x8888888888888888,
      0x8080808080808080, 0x8000800080008000, 0x8000000080000000,
      0x8000000000000000,
  };
  iopa_page_t *pa;
  uintptr_t addr = 0;
  ...
  count =
      (((uint32_t)bytes) + gIOPageAllocChunkBytes - 1) / gIOPageAllocChunkBytes;

  align_masks_idx =
      log2up((balign + gIOPageAllocChunkBytes - 1) / gIOPageAllocChunkBytes);
...
uintptr_t iopa_free(iopa_t *a, uintptr_t addr, vm_size_t bytes) {
  iopa_page_t *pa;
  uint32_t count;
  uintptr_t chunk;
...
IOReturn IOSetProcessorCacheMode(task_t task, IOVirtualAddress address,
  ...
  if ((address | length) & PAGE_MASK) {
    //	OSReportWithBacktrace("IOSetProcessorCacheMode(0x%x, 0x%x, 0x%x)
  ...
  while ((kIOReturnSuccess == ret) && (length > 0)) {
    ...
    if (pagenum) {
      ...
      ret = IOMapPages(get_task_map(task), address, ptoa_64(pagenum), page_size,
                       cacheMode);
    } else {
...
vm_offset_t OSKernelStackRemaining(void) { return ml_stack_remaining(); }
...
void IOSleepWithLeeway(unsigned intervalMilliseconds,
  ...
  delay_for_interval_with_leeway(intervalMilliseconds, leewayMilliseconds,
                                 kMillisecondScale);
}
...
__attribute__((noinline, not_tail_called)) void IOLog(const char *format, ...) {
...
__attribute__((noinline, not_tail_called)) void IOLogv(const char *format,
                                                       va_list ap) {
  void *caller = __builtin_return_address(0);
  _IOLogv(format, ap, caller);
}
...
#if !__LP64__
void IOPanic(const char *reason) { panic("%s", reason); }
#endif
...
IOReturn IOFindValueForName(const char *string,
                            const IONamedValue *regValueArray, int *value) {
  for (; regValueArray->name; regValueArray++) {
