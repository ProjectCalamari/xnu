...
typedef void (*IOPMActionPowerChangeStart)(
    void *target, IOService *service, IOPMActions *actions,
    const IOPMRequest *request, IOPMPowerStateIndex powerState,
    IOPMPowerChangeFlags *changeFlagsPtr);

typedef void (*IOPMActionPowerChangeDone)(void *target, IOService *service,
                                          IOPMActions *actions,
                                          const IOPMRequest *request,
                                          IOPMPowerStateIndex powerState,
                                          IOPMPowerChangeFlags changeFlags);

typedef void (*IOPMActionPowerChangeOverride)(
    void *target, IOService *service, IOPMActions *actions,
    const IOPMRequest *request, IOPMPowerStateIndex *powerStatePtr,
    IOPMPowerChangeFlags *changeFlagsPtr);

typedef void (*IOPMActionActivityTickle)(void *target, IOService *service,
                                         IOPMActions *actions);

typedef void (*IOPMActionUpdatePowerClient)(void *target, IOService *service,
                                            IOPMActions *actions,
...
// IOPMActions state
enum { kPMActionsStatePowerClamped = 0x0001 };
...
// Settle time after changing power state.
  ...
  // Used to uniquely identify power management notification to apps and
  // clients.
...
#define fPMDriverClass pwrMgt->PMDriverClass
  ...
  void pmTrace(uint32_t event, uint32_t eventFunc, uintptr_t param1,
...
#define StateOrder(state)                                                      \
  (((state) < fNumberOfPowerStates) ? pwrMgt->PowerStates[(state)].stateOrder  \
                                    : (state))
...
#define kIOPMDomainWillChange                                                  \
  0x0008                            // change started by PowerDomainWillChangeTo
...
#define kIOPMRootBroadcastFlags                                                \
  (kIOPMSynchronize | kIOPMRootChangeUp | kIOPMRootChangeDown)
...
typedef bool (*IOPMMessageFilter)(void *target, void *object, void *arg1,
                                  void *arg2, void *arg3);
...
enum { kIOPMDriverCallNoInactiveCheck = 1 };
...
public:
  ...
  inline bool isWorkBlocked(void) const { return fWorkWaitCount != 0; }

  inline bool isFreeBlocked(void) const { return fFreeWaitCount != 0; }

  inline IOPMRequest *getNextRequest(void) const { return fRequestNext; }
  ...
  inline uint32_t getType(void) const { return fRequestType; }

  inline uint32_t getTag(void) const { return fTag; }

  inline bool isReplyType(void) const {
    return fRequestType > kIOPMRequestTypeReplyStart;
  }

  inline IOService *getTarget(void) const { return fTarget; }

  inline bool isQuiesceBlocker(void) const { return fIsQuiesceBlocker; }
  ...
  inline void installCompletionAction(void *target, IOPMCompletionAction action,
                                      void *param) {
  ...
  inline void setTimestamp(uint64_t time) { fTimestamp = time; }

  inline uint64_t getTimestamp(void) const { return fTimestamp; }
...
public:
  static IOPMWorkQueue *create(IOService *inOwner, Action invoke,
                               Action retire);
