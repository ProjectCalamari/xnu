...
#define kIORegistryEntryAllowableSetPropertiesKey                              \
  "IORegistryEntryAllowableSetProperties"
// property name to single thread a user space call to
// IORegistryEntry::setProperties (OSBoolean)
#define kIORegistryEntryDefaultLockingSetPropertiesKey                         \
  "IORegistryEntryDefaultLockingSetProperties"
...
// Property specifying the entitlement to check against an IOUserClient's
// opening process kOSBooleanFalse - Allow access (no entitlements required)
// string - If the opening process has the named entitlement with value ==
// boolean true, allow access
...
// Allows the entitled process to open a user client connection to any dext that
// has specific entitlements Property is an array of strings containing
// entitlements, one of which needs to be present in the dext providing the user
// client being opened
#define kIODriverKitRequiredEntitlementsKey                                    \
  "com.apple.private.driverkit.driver-access"

// Specifies that this driver is used for internal tests. This opts the driver
// out of our policy to reboot the device if a driver crashes too often.
