...
template <typename T, typename TrappingPolicy> struct bounded_array_ref {
  ...
  // will trap.
  explicit constexpr bounded_array_ref(bounded_ptr<T, TrappingPolicy> data,
                                       size_t n)
    ...
    if (__improbable(n > UINT32_MAX)) {
      TrappingPolicy::trap("bounded_array_ref: Can't construct from a size "
  ...
  explicit constexpr bounded_array_ref(T *data, size_t n)
  ...
  explicit constexpr bounded_array_ref(T *first, T *last)
      : data_(first), size_(static_cast<uint32_t>(last - first)) {
    if (__improbable(first > last)) {
      TrappingPolicy::trap("bounded_array_ref: The [first, last) constructor "
  ...
  template <size_t N>
  constexpr bounded_array_ref(bounded_array<T, N, TrappingPolicy> &data)
      : data_(data.data()), size_(static_cast<uint32_t>(data.size())) {
  ...
  template <size_t N>
  constexpr bounded_array_ref(T (&array)[N])
      : data_(array), size_(static_cast<uint32_t>(N)) {
    if (__improbable(N > UINT32_MAX)) {
      TrappingPolicy::trap("bounded_array_ref: Can't construct from a size "
                           "greater than UINT32_MAX");
    }
  ...
  constexpr bounded_array_ref(bounded_array_ref const &) = default;
  constexpr bounded_array_ref(bounded_array_ref &&other) noexcept = default;
  ...
  // dereferenced.
  OS_ALWAYS_INLINE iterator begin() const noexcept {
  ...
  OS_ALWAYS_INLINE T &operator[](ptrdiff_t n) const { return begin()[n]; }
  ...
  bounded_array_ref<T, TrappingPolicy> slice(size_t n, size_t m) const {
    ...
    if (__improbable(os_add_overflow(n, m, &total))) {
      TrappingPolicy::trap("bounded_array_ref: n + m is larger than the size "
                           "of any bounded_array_ref");
    }
    if (__improbable(total > size())) {
      TrappingPolicy::trap("bounded_array_ref: invalid slice provided, the "
                           "indices are of bounds for the bounded_array_ref");
    }
...
bool operator==(bounded_array_ref<T, P> const &x, bar_detail::nullptr_t) {
...
template <typename T, typename P>
bool operator==(bar_detail::nullptr_t, bounded_array_ref<T, P> const &x) {
  return x == nullptr;
}
