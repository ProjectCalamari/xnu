...
template <typename T, typename U> struct copy_const<T const, U> {
  using type = U const;
};
...
template <typename T, typename U> struct copy_cv<T const, U> {
  using type = U const;
};
template <typename T, typename U> struct copy_cv<T volatile, U> {
  using type = U volatile;
};
template <typename T, typename U> struct copy_cv<T const volatile, U> {
...
template <typename T, typename U>
using WhenComparable = void_t<decltype(declval<T>() == declval<U>()),
                              decltype(declval<T>() != declval<U>())>;

template <typename T, typename U>
using WhenOrderable = void_t<decltype(declval<T>() < declval<U>()),
                             decltype(declval<T>() > declval<U>()),
                             decltype(declval<T>() >= declval<U>()),
                             decltype(declval<T>() <= declval<U>())>;
...
// This terminology is used below to describe the behavior of the
// `TrappingPolicy`.
//
...
// or exclusive? Instead, factory functions should be used to create
// `bounded_ptr`s.
//
...
public:
  ...
  OS_ALWAYS_INLINE constexpr bounded_ptr(detail::nullptr_t)
      : base_(nullptr), count_(0), offset_(0) {}

  OS_ALWAYS_INLINE constexpr explicit bounded_ptr() : bounded_ptr(nullptr) {}
  ...
  OS_ALWAYS_INLINE explicit bounded_ptr(T *pointer, T const *begin,
                                        T const *end) {
    ...
    if (__improbable(os_sub_overflow(reinterpret_cast<uintptr_t>(end),
                                     ...
                                     &count_))) {
      TrappingPolicy::trap(
          "The range of valid memory is too large to be represented "
  ...
  template <typename U, typename Policy,
            typename = detail::enable_if_t<detail::is_convertible_v<U *, T *>>>
  OS_ALWAYS_INLINE bounded_ptr(bounded_ptr<U, Policy> const &other)
      : base_(other.base_), count_(other.count_),
        offset_(static_cast<int32_t>(
            reinterpret_cast<CharType *>(static_cast<T *>(other.get_ptr_())) -
            other.base_)) {}
  ...
  template <typename U, typename Policy,
            typename = detail::enable_if_t<detail::is_convertible_v<U *, T *>>>
  OS_ALWAYS_INLINE bounded_ptr &operator=(bounded_ptr<U, Policy> const &other) {
    base_ = other.base_;
    count_ = other.count_;
    offset_ = static_cast<int32_t>(
        reinterpret_cast<CharType *>(static_cast<T *>(other.get_ptr_())) -
  ...
  OS_ALWAYS_INLINE bounded_ptr &operator=(detail::nullptr_t) {
    *this = bounded_ptr();
    return *this;
  }
  ...
  // diagnose.
  template <typename T_ = T> // delay instantiation to avoid forming invalid ref
                             // for bounded_ptr<void>
                             OS_ALWAYS_INLINE T_ &operator*() const {
    if (__improbable(!in_bounds_())) {
      TrappingPolicy::trap(
          "bounded_ptr<T>::operator*: Dereferencing this pointer "
          "would access memory outside of the bounds set originally");
    }
    return *get_ptr_();
  }

  OS_ALWAYS_INLINE T *operator->() const {
    if (__improbable(!in_bounds_())) {
      TrappingPolicy::trap(
          "bounded_ptr<T>::operator->: Accessing a member through this pointer "
          "would access memory outside of the bounds set originally");
    }
    return get_ptr_();
  }
  ...
  OS_ALWAYS_INLINE T *discard_bounds() const {
    if (__improbable(!in_bounds_())) {
      TrappingPolicy::trap(
  ...
  OS_ALWAYS_INLINE T *unsafe_discard_bounds() const { return get_ptr_(); }
  ...
  OS_ALWAYS_INLINE explicit operator bool() const {
  ...
  OS_ALWAYS_INLINE bounded_ptr &operator++() {
    *this += 1;
    return *this;
  }
  OS_ALWAYS_INLINE bounded_ptr operator++(int) {
  ...
  OS_ALWAYS_INLINE bounded_ptr &operator+=(ptrdiff_t n) {
    static_assert(!detail::is_void_v<T_>,
                  "Arithmetic on bounded_ptr<void> is not allowed.");
  ...
  OS_ALWAYS_INLINE bounded_ptr &operator-=(ptrdiff_t n) {
    ...
    if (__improbable(os_sub_overflow(offset_, bytes, &offset_))) {
      TrappingPolicy::trap(
          "bounded_ptr<T>::operator-=(n): Subtracting the specified number of "
          "bytes "
  ...
  friend OS_ALWAYS_INLINE bounded_ptr operator+(bounded_ptr p, ptrdiff_t n) {
    p += n;
    return p;
  }
  friend OS_ALWAYS_INLINE bounded_ptr operator+(ptrdiff_t n, bounded_ptr p) {
    p += n;
    return p;
  }
  friend OS_ALWAYS_INLINE bounded_ptr operator-(bounded_ptr p, ptrdiff_t n) {
  ...
  // operation.
  friend OS_ALWAYS_INLINE ptrdiff_t operator-(bounded_ptr const &a,
                                              bounded_ptr const &b) {
  ...
  friend OS_ALWAYS_INLINE ptrdiff_t operator-(T const *a,
                                              bounded_ptr const &b) {
    return a - b.get_ptr_();
  }

private:
  OS_ALWAYS_INLINE bool in_bounds_() const {
    static_assert(
        detail::sizeof_v<T> <= UINT32_MAX - INT32_MAX,
        "The type pointed-to by bounded_ptr is too large, which would defeat "
        "our optimization to check for inboundedness using arithmetic on "
        "unsigned");
    return offset_ >= 0 && static_cast<uint32_t>(offset_) +
                                   static_cast<uint32_t>(detail::sizeof_v<T>) <=
                               count_;
  }

  OS_ALWAYS_INLINE T *get_ptr_() const {
    // Compute `base_ + offset_`, catching overflows.
    uintptr_t ptr;
    if (__improbable(os_add_overflow(reinterpret_cast<uintptr_t>(base_),
                                     offset_, &ptr))) {
      TrappingPolicy::trap(
          "This bounded_ptr is pointing to memory outside of what can "
  ...
  friend bounded_ptr<T_, Policy>
  reinterpret_pointer_cast(bounded_ptr<U, Policy> const &) noexcept;
...
template <typename T, typename P1, typename U, typename P2,
          typename = detail::WhenComparable<T *, U *>>
OS_ALWAYS_INLINE bool operator==(bounded_ptr<T, P1> const &a,
                                 bounded_ptr<U, P2> const &b) {
...
template <typename T, typename P, typename U,
          typename = detail::WhenComparable<T *, U *>>
OS_ALWAYS_INLINE bool operator==(bounded_ptr<T, P> const &a, U *b) {
  return a.unsafe_discard_bounds() == b;
}

template <typename T, typename P, typename U,
          typename = detail::WhenComparable<T *, U *>>
OS_ALWAYS_INLINE bool operator==(U *a, bounded_ptr<T, P> const &b) {
...
OS_ALWAYS_INLINE bool operator==(detail::nullptr_t,
                                 bounded_ptr<T, Policy> const &p) {
...
template <typename T, typename Policy>
OS_ALWAYS_INLINE bool operator==(bounded_ptr<T, Policy> const &p,
                                 detail::nullptr_t) {
  return p.unsafe_discard_bounds() == nullptr;
}
...
template <typename T, typename U, typename P1, typename P2,
          typename = detail::WhenOrderable<T *, U *>>
OS_ALWAYS_INLINE bool operator<(bounded_ptr<T, P1> const &a,
                                bounded_ptr<U, P2> const &b) {
  return a.unsafe_discard_bounds() < b.unsafe_discard_bounds();
}
...
template <typename T, typename U, typename P,
          typename = detail::WhenOrderable<T *, U *>>
OS_ALWAYS_INLINE bool operator<(T *a, bounded_ptr<U, P> const &b) {
  return a < b.unsafe_discard_bounds();
}
...
template <typename T, typename U>
OS_ALWAYS_INLINE T *reinterpret_pointer_cast(U *p) noexcept {
