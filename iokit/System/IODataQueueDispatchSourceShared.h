...
bool IODataQueueDispatchSource::init() {
  if (!super::init()) {
    return false;
  }
...
kern_return_t IODataQueueDispatchSource::CheckForWork_Impl(const IORPC rpc,
                                                           bool synchronous) {
...
kern_return_t
IODataQueueDispatchSource::Create_Impl(uint64_t queueByteCount,
                                       IODispatchQueue *queue,
                                       IODataQueueDispatchSource **source) {
  ...
  bmd = IOBufferMemoryDescriptor::withOptions(
      kIODirectionOutIn | kIOMemoryKernelUserShared, queueByteCount, page_size);
  if (!bmd) {
    inst->release();
    return kIOReturnNoMemory;
  }
  ...
  inst->ivars->dataQueue =
      (typeof(inst->ivars->dataQueue))bmd->getBytesNoCopy();
...
kern_return_t
IODataQueueDispatchSource::CopyMemory_Impl(IOMemoryDescriptor **memory) {
  kern_return_t ret;
...
kern_return_t
IODataQueueDispatchSource::CopyDataAvailableHandler_Impl(OSAction **action) {
  kern_return_t ret;
  OSAction *result;
...
kern_return_t
IODataQueueDispatchSource::CopyDataServicedHandler_Impl(OSAction **action) {
  kern_return_t ret;
  OSAction *result;
...
kern_return_t
IODataQueueDispatchSource::SetDataAvailableHandler_Impl(OSAction *action) {
  IOReturn ret;
  OSAction *oldAction;

  oldAction = ivars->dataAvailableAction;
  if (oldAction &&
      OSCompareAndSwapPtr(oldAction, NULL, &ivars->dataAvailableAction)) {
    oldAction->release();
  }
  if (action) {
    action->retain();
...
kern_return_t
IODataQueueDispatchSource::SetDataServicedHandler_Impl(OSAction *action) {
  IOReturn ret;
  OSAction *oldAction;

  oldAction = ivars->dataServicedAction;
  if (oldAction &&
      OSCompareAndSwapPtr(oldAction, NULL, &ivars->dataServicedAction)) {
    oldAction->release();
  }
  if (action) {
    action->retain();
...
void IODataQueueDispatchSource::SendDataAvailable(void) {
  IOReturn ret;
...
void IODataQueueDispatchSource::SendDataServiced(void) {
  IOReturn ret;
...
kern_return_t IODataQueueDispatchSource::SetEnableWithCompletion_Impl(
...
void IODataQueueDispatchSource::free() {
...
kern_return_t
IODataQueueDispatchSource::Cancel_Impl(IODispatchSourceCancelHandler handler) {
#if !KERNEL
  if (handler) {
    handler();
  }
...
bool IODataQueueDispatchSource::IsDataAvailable(void) {
...
IODataQueueDispatchSource::Peek(IODataQueueClientDequeueEntryBlock callback) {
  ...
  headOffset =
      __c11_atomic_load((_Atomic uint32_t *)&dataQueue->head, __ATOMIC_RELAXED);
  tailOffset =
      __c11_atomic_load((_Atomic uint32_t *)&dataQueue->tail, __ATOMIC_ACQUIRE);
...
kern_return_t IODataQueueDispatchSource::Dequeue(
...
kern_return_t IODataQueueDispatchSource::DequeueWithCoalesce(
    bool *sendDataServiced, IODataQueueClientDequeueEntryBlock callback) {
  ...
  __c11_atomic_store((_Atomic uint32_t *)&dataQueue->head, newHeadOffset,
                     __ATOMIC_RELEASE);
...
kern_return_t IODataQueueDispatchSource::Enqueue(
    uint32_t callerDataSize, IODataQueueClientEnqueueEntryBlock callback) {
  kern_return_t ret;
...
kern_return_t IODataQueueDispatchSource::EnqueueWithCoalesce(
    uint32_t callerDataSize, bool *sendDataAvailable,
  ...
  tail =
      __c11_atomic_load((_Atomic uint32_t *)&dataQueue->tail, __ATOMIC_RELAXED);
  head =
      __c11_atomic_load((_Atomic uint32_t *)&dataQueue->head, __ATOMIC_ACQUIRE);
  ...
  if (tail >= head) {
    ...
    if ((entrySize <= (UINT32_MAX - tail)) &&
      ...
      if ((queueSize - tail) >= DATA_QUEUE_ENTRY_HEADER_SIZE) {
        ((IODataQueueEntry *)((uintptr_t)dataQueue->queue + tail))->size =
            dataSize;
      }
  ...
  if (retVal == kIOReturnSuccess) {
    ...
    __c11_atomic_store((_Atomic uint32_t *)&dataQueue->tail, newTail,
                       __ATOMIC_RELEASE);
    ...
    if (tail != head) {
      ...
      //
      __c11_atomic_thread_fence(__ATOMIC_SEQ_CST);
      head = __c11_atomic_load((_Atomic uint32_t *)&dataQueue->head,
                               __ATOMIC_RELAXED);
    }
...
kern_return_t IODataQueueDispatchSource::CanEnqueueData(uint32_t callerDataSize,
                                                        uint32_t dataCount) {
  IODataQueueMemory *dataQueue;
  uint32_t head;
  uint32_t tail;
...
size_t IODataQueueDispatchSource::GetDataQueueEntryHeaderSize() {
