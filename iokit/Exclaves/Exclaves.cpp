...
public:
  IOWorkLoop *workLoop;
  void closeGate() { workLoop->closeGate(); }
  void openGate() { workLoop->openGate(); }
};
...
bool IOService::exclaveStart(IOService *provider, IOExclaveProxyState **pRef) {
  ...
  do {
    ...
    if ((data = OSDynamicCast(OSData, prop))) {
      ...
      if (exclaves_driver_service_lookup(exclave_service, &mach_endpoint) !=
          KERN_SUCCESS) {
    ...
    if ((data = OSDynamicCast(OSData, prop))) {
      ...
      if (exclaves_driver_service_lookup(exclave_edk_service,
                                         &edk_mach_endpoint) != KERN_SUCCESS) {
    ...
    tb_endpoint = tb_endpoint_create_with_value(
        TB_TRANSPORT_TYPE_XNU, mach_endpoint, TB_ENDPOINT_OPTIONS_NONE);
    ...
    edk_tb_endpoint = tb_endpoint_create_with_value(
        TB_TRANSPORT_TYPE_XNU, edk_mach_endpoint, TB_ENDPOINT_OPTIONS_NONE);
    ...
    // Start() called after lookup table registration in case upcalls are made
    // during exclave start(). Use registry ID as exclave's upcall identifer
    tberr = ioservice_ioserviceprivate_startprivate(&edk_client, serviceID,
                                                    &result);
...
uint64_t IOService::exclaveEndpoint(IOExclaveProxyState *pRef) {
  return pRef->mach_endpoint;
}

bool IOExclaveProxy::start(IOService *provider) {
  bool ok;
...
bool IOExclaveInterruptUpcallHandler(uint64_t id,
                                     IOExclaveInterruptUpcallArgs *args) {
  assert(args);
  ...
  case kIOExclaveInterruptUpcallTypeRegister:
    // Register interrupt
    res = ref->service->exclaveRegisterInterrupt(
        ref, args->index, args->data.register_args.test_irq);
    break;
  ...
  case kIOExclaveInterruptUpcallTypeEnable:
    ...
    res = ref->service->exclaveEnableInterrupt(ref, args->index,
                                               args->data.enable_args.enable);
    break;
  default:
    res = false;
...
bool IOExclaveTimerUpcallHandler(uint64_t id, IOExclaveTimerUpcallArgs *args) {
  assert(args);
  IOService::IOExclaveProxyState *ref = getProxyStateFromRegistryID(id);
  if (!ref || !args) {
    return false;
  ...
  case kIOExclaveTimerUpcallTypeSetTimeout: {
    // Set timeout
    uint32_t options = args->data.set_timeout_args.clock_continuous
    ...
    res = ref->service->exclaveTimerSetTimeout(ref, timer_id, options, duration,
                                               0, kr);
    break;
  }
...
static void getExclaveInterruptKey(int index, char *key, size_t size) {
  snprintf(key, size, "%d", index);
}

static IOInterruptEventSource *
copyExclaveInterruptEventSource(IOService::IOExclaveProxyState *pRef,
                                int index) {
  OSObject *obj;
...
void IOExclaveTestSignalInterrupt(thread_call_param_t arg0,
                                  __unused thread_call_param_t arg1) {
  EXLOG("%s called\n", __func__);

  // Unpackage params
  struct IOExclaveTestSignalInterruptParam *params =
      (struct IOExclaveTestSignalInterruptParam *)arg0;
...
bool IOExclaveAsyncNotificationUpcallHandler(
  ...
  case AsyncNotificationUpcallTypeSignal:
    ret = ref->service->exclaveAsyncNotificationSignal(
              ref, args->notificationID) == kIOReturnSuccess;
    break;
  default:
    ret = false;
    break;
...
bool IOExclaveMapperOperationUpcallHandler(
    uint64_t id, IOExclaveMapperOperationUpcallArgs *args) {
  assert(args);
  ...
  IOMapper *mapper = IOMapper::copyMapperForDeviceWithIndex(
      provider, (unsigned int)(args->mapperIndex));
  if (!mapper) {
    goto finish;
  }
  ...
  switch (args->type) {
  case MapperActivate:
    res = kIOReturnSuccess == mapper->callPlatformFunction(
                                  gDARTMapperFunctionSetActive, false,
                                  (void *)(true), (void *)(false), NULL, NULL);
    break;
  case MapperDeactivate:
    res = kIOReturnSuccess == mapper->callPlatformFunction(
                                  gDARTMapperFunctionSetActive, false,
                                  (void *)(false), (void *)(false), NULL, NULL);
    break;
  default:
    break;
  }
  ...
  case kIOExclaveANEUpcallTypeWorkSubmit:
    if (ref->aneWorkSubmitUpcallHandler) {
      _result = (ref->aneWorkSubmitUpcallHandler)(
          args->work_args.arg0, args->work_args.arg1, args->work_args.arg2);
      ret = true;
    } else {
      printf("%s: no handler for upcall %d registered\n", __func__,
             (int)args->type);
...
IOReturn IOExclaveLPWUpcallHandler(struct IOExclaveLPWUpcallArgs *args) {
  ...
  case kIOExclaveLPWUpcallTypeCreateAssertion:
    return IOExclaveLPWCreateAssertion(&args->data.createassertion.id_out,
...
IOReturn IOExclaveLPWCreateAssertion(uint64_t *id_out, const char *desc) {
  ...
  assertionID = rootDomain->createPMAssertion(
      ...
      kIOPMDriverAssertionLevelOn, rootDomain, desc);
  if (assertionID == 0) {
    return kIOReturnInternalError;
  }
...
bool IOService::exclaveRegisterInterrupt(IOExclaveProxyState *pRef, int index,
  ...
  action = OSMemberFunctionCast(IOInterruptEventSource::Action, this,
                                &IOService::exclaveInterruptOccurred);
  ies = IOInterruptEventSource::interruptEventSource(
      this, action, noProvider ? nullptr : getProvider(), index);
  if (!ies) {
    return false;
  }
...
bool IOService::exclaveRemoveInterrupt(IOExclaveProxyState *pRef, int index) {
#if CONFIG_EXCLAVES
  IOInterruptEventSource *ies;
  IOWorkLoop *wl;
  char irqKey[5];
...
bool IOService::exclaveEnableInterrupt(IOExclaveProxyState *pRef, int index,
                                       bool enable) {
#if CONFIG_EXCLAVES
  IOInterruptEventSource *ies;
...
void IOService::exclaveInterruptOccurred(IOInterruptEventSource *eventSource,
                                         int count) {
#if CONFIG_EXCLAVES
  tb_error_t tberr;
  ...
  EXLOG("%s id 0x%llx (irq index %d)\n", __func__, getRegistryEntryID(),
        eventSource->getIntIndex());
  ...
  tberr = ioservice_ioserviceprivate_interruptoccurredprivate(
      &ref->edk_client, eventSource->getIntIndex(), count);
  assert(TB_ERROR_SUCCESS == tberr);
  if (TB_ERROR_SUCCESS != tberr) {
...
#if CONFIG_EXCLAVES
static void getExclaveTimerKey(uint32_t timer_id, char *key, size_t size) {
  snprintf(key, size, "%d", timer_id);
}

static IOTimerEventSource *
copyExclaveTimerEventSource(IOService::IOExclaveProxyState *pRef,
                            uint32_t timer_id) {
  OSObject *obj;
...
bool IOService::exclaveRegisterTimer(IOExclaveProxyState *pRef,
                                     uint32_t *timer_id) {
#if CONFIG_EXCLAVES
...
bool IOService::exclaveRemoveTimer(IOExclaveProxyState *pRef,
                                   uint32_t timer_id) {
#if CONFIG_EXCLAVES
  IOTimerEventSource *tes;
  IOWorkLoop *wl;
...
bool IOService::exclaveEnableTimer(IOExclaveProxyState *pRef, uint32_t timer_id,
                                   bool enable) {
#if CONFIG_EXCLAVES
  IOTimerEventSource *tes;
...
bool IOService::exclaveTimerSetTimeout(IOExclaveProxyState *pRef,
                                       uint32_t timer_id, uint32_t options,
...
bool IOService::exclaveTimerCancelTimeout(IOExclaveProxyState *pRef,
                                          uint32_t timer_id) {
#if CONFIG_EXCLAVES
  IOTimerEventSource *tes;
...
void IOService::exclaveTimerFired(IOTimerEventSource *eventSource) {
#if CONFIG_EXCLAVES
  tb_error_t tberr;
  IOService::IOExclaveProxyState *ref;
  ...
  EXLOG("%s id 0x%llx (timer_id %u)\n", __func__, getRegistryEntryID(),
        timer_id);
  tberr =
      ioservice_ioserviceprivate_timerfiredprivate(&ref->edk_client, timer_id);
  assert(TB_ERROR_SUCCESS == tberr);
  if (TB_ERROR_SUCCESS != tberr) {
    printf("%s ERROR: tightbeam call failed\n", __func__);
    return;
...
kern_return_t IOService::exclaveAsyncNotificationRegister(
    IOExclaveProxyState *pRef, IOInterruptEventSource *notification,
  ...
  if (pRef->exclaveAsyncNotificationEventSources) {
    *notificationID =
        (uint32_t)pRef->exclaveAsyncNotificationEventSources->getCount();
...
kern_return_t
IOService::exclaveAsyncNotificationSignal(IOExclaveProxyState *pRef,
                                          uint32_t notificationID) {
#if CONFIG_EXCLAVES
  kern_return_t ret;
  ...
  if (pRef->exclaveAsyncNotificationEventSources &&
      (event =
           OSDynamicCast(IOInterruptEventSource,
                         pRef->exclaveAsyncNotificationEventSources->getObject(
...
void exclaves_wait_for_cpu_init() {
  OSDictionary *match_dict =
      IOService::resourceMatching(gIOAllCPUInitializedKey);
...
exclaveRegisterANEUpcallHelper(IOService::IOExclaveProxyState *pRef,
                               IOExclaveANEUpcallType type, IOWorkLoop *wl,
                               void *block) {
  ...
  if (wl != NULL) {
    return wl->runActionBlock(^{
      ...
      case kIOExclaveANEUpcallTypeSetPowerState:
        ...
        pRef->aneSetPowerStateUpcallHandler =
            (ANEUpcallSetPowerStateHandler)_block;
        break;
      case kIOExclaveANEUpcallTypeWorkSubmit:
...
kern_return_t IOService::exclaveRegisterANEUpcallSetPowerState(
    IOExclaveProxyState *pRef, ANEUpcallSetPowerStateHandler handler) {
#if CONFIG_EXCLAVES
  return exclaveRegisterANEUpcallHelper(pRef,
                                        kIOExclaveANEUpcallTypeSetPowerState,
                                        getWorkLoop(), Block_copy(handler));
#else
...
kern_return_t
IOService::exclaveRegisterANEUpcallWorkSubmit(IOExclaveProxyState *pRef,
                                              ANEUpcallWorkHandler handler) {
#if CONFIG_EXCLAVES
  return exclaveRegisterANEUpcallHelper(pRef, kIOExclaveANEUpcallTypeWorkSubmit,
                                        getWorkLoop(), Block_copy(handler));
#else
#pragma unused(pRef, handler)
  return kIOReturnUnsupported;
...
kern_return_t
IOService::exclaveRegisterANEUpcallWorkBegin(IOExclaveProxyState *pRef,
                                             ANEUpcallWorkHandler handler) {
#if CONFIG_EXCLAVES
  return exclaveRegisterANEUpcallHelper(pRef, kIOExclaveANEUpcallTypeWorkBegin,
                                        getWorkLoop(), Block_copy(handler));
#else
#pragma unused(pRef, handler)
  return kIOReturnUnsupported;
...
kern_return_t
IOService::exclaveRegisterANEUpcallWorkEnd(IOExclaveProxyState *pRef,
                                           ANEUpcallWorkHandler handler) {
#if CONFIG_EXCLAVES
  return exclaveRegisterANEUpcallHelper(pRef, kIOExclaveANEUpcallTypeWorkEnd,
                                        getWorkLoop(), Block_copy(handler));
#else
#pragma unused(pRef, handler)
  return kIOReturnUnsupported;
