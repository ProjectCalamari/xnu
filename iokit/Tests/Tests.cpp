...
static void *TestZLib_alloc(void *__unused opaque, u_int items, u_int size) {
  kern_return_t kr;
  vm_offset_t result;

  kr = kmem_alloc(kernel_map, &result, round_page(items * size) + ptoa(2),
                  (kma_flags_t)(KMA_NOFAIL | KMA_KSTACK | KMA_KOBJECT |
...
static void TestZLib_free(void *__unused ref, void *ptr) {}

static int TestZLib(int newValue) {
  ...
  deflateInit2(&strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED, 15, 1, Z_FIXED);
...
static void TESAction(OSObject *owner, IOTimerEventSource *tes) {
  if (mach_absolute_time() < gIOWorkLoopTestDeadline) {
    tes->setTimeout(1, kMicrosecondScale);
  }
}

static int IOWorkLoopTest(int newValue) {
  IOReturn err;
  uint32_t idx;
  ...
  assert(wl);
  tes = IOTimerEventSource::timerEventSource(
      kIOTimerEventSourceOptionsPriorityWorkLoop, wl.get(), &TESAction);
  assert(tes);
  ...
  tes = IOTimerEventSource::timerEventSource(
      kIOTimerEventSourceOptionsDefault, wl.get(), ^(IOTimerEventSource *tes) {
  ...
  ies = IOInterruptEventSource::interruptEventSource(
      wl.get(), NULL, 0, ^void(IOInterruptEventSource *sender, int count) {
...
static int OSCollectionTest(int newValue) {
  ...
  array->iterateObjects(^bool(OSObject *obj) {
    kprintf("%d:%d ", index,
            (obj == kOSBooleanTrue)    ? 1
            : (obj == kOSBooleanFalse) ? 0
                                       : 2);
    index++;
    return false;
  });
  ...
  OSSharedPtr<OSSerializer> serializer =
      OSSerializer::withBlock(^bool(OSSerialize *s) {
        return gIOBSDUnitKey->serialize(s);
      });
  assert(serializer);
  IOService::getPlatform()->setProperty("OSSerializer_withBlock",
                                        serializer.get());
  ...
  OSSharedPtr<OSObject> o =
      OSUnserializeXML((const char *)s->text(), s->getLength());
  assert(dict->isEqualTo(o.get()));

  dict = OSDynamicPtrCast<OSDictionary>(o);
  __assert_only OSNumber *nnum =
      OSDynamicCast(OSNumber, dict->getObject("test"));
  assert(12345678 == (int)(10000 * nnum->doubleValue()));

  return 0;
}

static int OSAllocationTests(int) {
...
static int OSDataAllocationTests(int) {
  OSDataAllocation<int> ints(100, OSAllocateMemory);
  assert(ints);
...
static int OSBoundedArrayTests(int) {
...
static int OSBoundedArrayRefTests(int) {
...
unsigned int OSArraySubclass::iteratorSize() const {
  unsigned int result = 64;
  // Has to be larger than the OSArray iterator size to prevent out-of-bounds
  // writes
...
static int OSCollectionIteratorTests(int) {
  ...
  // Test iteration with a OSCollectionIterator subclass over a subclassed
  // OSArray, with a large iterator size.
...
static int OSBoundedPtrTests(int) {
...
static int IODataQueueDispatchSourceTests(int) {
  constexpr uint32_t NUM_ELEMENTS = 10;
  constexpr uint32_t ELEMENT_SIZE = 16;
  size_t entryHeaderSize;
  IODataQueueDispatchSource *dqds;
  kern_return_t kr;
  IODataQueueClientEnqueueEntryBlock enqueueBlock =
      ^(void *data, size_t dataSize) {
      };
  IODataQueueClientDequeueEntryBlock dequeueBlock =
  ...
  kr = IODataQueueDispatchSource::Create(
      NUM_ELEMENTS * (ELEMENT_SIZE + entryHeaderSize), NULL, &dqds);
  ...
  for (uint32_t i = 0; i < NUM_ELEMENTS * 10000; i++) {
    ...
    for (uint32_t j = 0; j <= NUM_ELEMENTS; j++) {
      ...
      if (j < NUM_ELEMENTS - enqueued ||
          (head <= tail && tail == enqueued && j == NUM_ELEMENTS - enqueued)) {
        if (kr != kIOReturnSuccess) {
          panic("i=%u j=%u enqueued=%u head=%u tail=%u kr=%d expected "
                "kIOReturnSuccess\n",
      ...
      if ((random() % 16) == 1) {
        ...
        if (kr == kIOReturnSuccess) {
          ...
          // if we are checking to see if we can enqueue one element and
          // CanEnqueueData says there's no space, ensure we can't Enqueue that
          // element
          kr = dqds->Enqueue(ELEMENT_SIZE, enqueueBlock);
          assert(kr == kIOReturnOverrun);
        }
...
OSDefineValueObjectForDependentType(char)
    OSDefineValueObjectForDependentType(TrivialAggregateTestType)
        ...
        static int OSValueObjectTests(int) {
  ...
  // test trivial aggregate type
  {
    ...
    if (test) {
      const auto *const bytes =
          reinterpret_cast<const uint8_t *>(test->getBytesNoCopy());
...
static int IOSharedDataQueue_44636964(__unused int newValue) {
  OSSharedPtr<IOSharedDataQueue> sd = IOSharedDataQueue::withCapacity(
      DATA_QUEUE_ENTRY_HEADER_SIZE + sizeof(UInt64));
...
static int IOServiceTest(int newValue) {
  ...
  assert(matching);
  found = nullptr;
  note = IOService::addMatchingNotification(
      gIOMatchedNotification, matching.get(), 0,
      ^bool(IOService *newService, IONotifier *notifier) {
        kprintf("found %s, %d\n", newService->getName(),
                newService->getRetainCount());
        found = newService;
        found->retain();
        return true;
      });
...
static void OSStaticPtrCastTests() {
  // const& overload
  {
  ...
  // && overload
  {
    ...
    __assert_only OSSharedPtr<OSCollection> collection =
        OSStaticPtrCast<OSCollection>(os::move(dict));
...
static void OSConstPtrCastTests() {
  // const& overload
  {
  ...
  {
    OSSharedPtr<OSDictionary const> const dict = OSMakeShared<OSDictionary>();
    OSSharedPtr<OSDictionary const> dict2 =
        OSConstPtrCast<OSDictionary const>(dict);
    assert(dict2 == dict);
  }
  {
  ...
  // && overload
  {
    ...
    __assert_only OSSharedPtr<OSDictionary> dict2 =
        OSConstPtrCast<OSDictionary>(os::move(dict));
    assert(dict == nullptr);
    assert(dict2 == oldDict);
  }
  {
    OSSharedPtr<OSDictionary const> dict = nullptr;
    OSSharedPtr<OSDictionary> dict2 =
        OSConstPtrCast<OSDictionary>(os::move(dict));
    assert(dict == nullptr);
    assert(dict2 == nullptr);
  }
...
static void OSDynamicPtrCastTests() {
  OSSharedPtr<OSDictionary> const dict = OSMakeShared<OSDictionary>();
  {
  ...
  {
    OSTaggedSharedPtr<OSCollection, OSCollection> taggedDict(dict.get(),
                                                             OSRetain);
    OSTaggedSharedPtr<OSCollection, OSCollection> collection =
        OSDynamicPtrCast<OSCollection>(taggedDict);
    assert(collection != nullptr);
  }
  {
    OSTaggedSharedPtr<OSCollection, OSCollection> taggedDict(dict.get(),
                                                             OSRetain);
    OSTaggedSharedPtr<OSArray, OSCollection> array =
        OSDynamicPtrCast<OSArray>(taggedDict);
    assert(array == nullptr);
    assert(dict != nullptr);
  }
  {
    ...
    OSSharedPtr<OSDictionary> newDict =
        OSDynamicPtrCast<OSDictionary>(os::move(collection));
    assert(collection == nullptr);
...
static bool IOUserNotificationMatchingHandler(void *target __unused,
                                              void *ref __unused,
                                              IOService *newService __unused,
                                              IONotifier *notifier __unused) {
  return true;
}

static void IOUserNotificationTestThread(void *arg,
                                         wait_result_t result __unused) {
  IOUserNotificationTestThreadArgs *threadArgs =
      (IOUserNotificationTestThreadArgs *)arg;
  ...
  for (size_t i = 0; i < threadArgs->iterations; i++) {
    if (i % 2 == 0) {
      IONotifier *notify = IOService::addMatchingNotification(
          gIOWillTerminateNotification, dict.get(),
...
static int IOUserNotificationTests(__unused int newValue) {
  constexpr size_t numThreads = 10;
  constexpr size_t numIterations = 500000;
  OSSharedPtr<IOTestUserNotification> userNotify =
      OSMakeShared<IOTestUserNotification>();
  ...
  for (i = 0; i < numThreads; i++) {
    kr = kernel_thread_start(&IOUserNotificationTestThread, (void *)&threadArgs,
                             &threads[i]);
...
static void IOServiceMatchingSharedPtrTests() {
...

  {
    ...
    OSSharedPtr<OSDictionary> result =
        IOService::serviceMatching("name", table);
    assert(result);
    assert(result->getRetainCount() == 1);

    table = result;
    assert(table->getRetainCount() == 2);
    OSSharedPtr<OSDictionary> result2 =
        IOService::serviceMatching("name", table);
    assert(result2);
...

  {
    OSSharedPtr<OSDictionary> table;
    OSSharedPtr<OSDictionary> result =
        IOService::serviceMatching(name.get(), table);
    assert(result);
    assert(result->getRetainCount() == 1);
...

  {
    OSSharedPtr<OSDictionary> table;
    OSSharedPtr<OSDictionary> result =
        IOService::nameMatching(name.get(), table);
    assert(result);
    assert(result->getRetainCount() == 1);
...

  {
    OSSharedPtr<OSDictionary> table;
    OSSharedPtr<OSDictionary> result =
        IOService::resourceMatching("name", table);
    assert(result);
    assert(result->getRetainCount() == 1);
...

  {
    OSSharedPtr<OSDictionary> table;
    OSSharedPtr<OSDictionary> result =
        IOService::resourceMatching(name.get(), table);
    assert(result);
    assert(result->getRetainCount() == 1);
...

  {
    OSSharedPtr<OSDictionary> table;
    OSSharedPtr<OSDictionary> result =
        IOService::propertyMatching(name.get(), value.get(), table);
    assert(result);
    assert(result->getRetainCount() == 1);
...

  {
    OSSharedPtr<OSDictionary> table;
    OSSharedPtr<OSDictionary> result =
        IOService::registryEntryIDMatching(12, table);
    assert(result);
    assert(result->getRetainCount() == 1);
...
static int OSSharedPtrTests(int) {
...
static int IOServiceStateNotificationTests(int) {
  ...
  void (^sendClam)(OSBoolean *state) = ^void(OSBoolean *state) {
    ...
    kr = IOService::getSystemStateNotificationService()
             ->StateNotificationItemSet(str.get(), value.get());
    assert(kIOReturnSuccess == kr);
  };
  ...
  kr = service->stateNotificationListenerAdd(
      array.get(), &listenerRef, ^kern_return_t() {
        IOLog("test handler\n");
        kern_return_t kr;
        OSDictionary *dict;
  ...
  OSSharedPtr<IOEventSource> es = IOServiceStateNotificationEventSource::
      serviceStateNotificationEventSource(service, array.get(), ^void() {
...
static int IOMallocPageableTests(int) {
  vm_size_t size = 0;
  ...
  for (size = 0; size <= 4 * page_size; size += 64) {
    for (alignment = 1; alignment <= page_size; alignment <<= 1) {
      ...
      if (((uintptr_t)addr & alignMask) != 0) {
        IOLog("IOMallocPageable(size=%u, alignment=%u) returned unaligned "
              "pointer %p\n",
      ...
      if (((uintptr_t)addr & alignMask) != 0) {
        IOLog("IOMallocPageableZero(size=%u, alignment=%u) returned unaligned "
              "pointer %p\n",
              (uint32_t)size, (uint32_t)alignment, addr);
        return kIOReturnError;
      }
      for (char *ptr = (char *)addr; ptr < (char *)addr + size; ptr++) {
        if (*ptr != 0) {
          IOLog("IOMallocPageableZero(size=%u, alignment=%u) -> %p, byte at %p "
                "is nonzero\n",
...
static int Test100367284(int) {
  OSSharedPtr<IOService> entry;
  OSSharedPtr<IOService> replace;
  OSSharedPtr<IOService> root;

  root =
      OSDynamicPtrCast<IOService>(IORegistryEntry::fromPath("/", gIODTPlane));
...
static void TestLockForArbitrationThread(void *arg,
                                         wait_result_t result __unused) {
  LockForArbitrationTestThreadArgs *threadArgs =
      (LockForArbitrationTestThreadArgs *)arg;
  bool ok;
...
static int TestLockForArbitration(int) {
  ...
  IOLockLock(&threadArgs.lock);
  kr = kernel_thread_start(&TestLockForArbitrationThread, (void *)&threadArgs,
                           &thread);
  assert(kr == KERN_SUCCESS);
...
static int sysctl_iokittest(__unused struct sysctl_oid *oidp,
...
SYSCTL_PROC(_kern, OID_AUTO, iokittest,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_KERN | CTLFLAG_LOCKED, NULL, 0,
            sysctl_iokittest, "I", "");
...
/*
 * A simple wrapper around an IOService. This terminates the IOService in
 * free().
 */
...
TestIOServiceHandle *TestIOServiceHandle::withService(IOService *service) {
...
bool TestIOServiceHandle::initWithService(IOService *service) {
  fService = service;
  fService->retain();
  return true;
}

void TestIOServiceHandle::free() {
  if (fService) {
    fService->terminate();
...
/*
 * Set up test IOServices. See the available services in
 * xnu/iokit/Tests/TestServices.
 *
 * xnu darwintests use this sysctl to make these test services available. A send
 * right is pushed to the task that called the sysctl, which when deallocated
 * removes the service. This ensures that the registry isn't polluted by
 * misbehaving tests.
 *
 * Since this sysctl allows callers to instantiate arbitrary classes based on
 * their class name, this can be a security concern. Tests that call this sysctl
 * need the kIOServiceTestServiceManagementEntitlementKey entitlement.
 */
static int sysctl_iokit_test_service_setup(struct sysctl_oid *oidp,
                                           __unused void *arg1,
                                           __unused int arg2,
                                           struct sysctl_req *req) {
  ...
  if (!IOTaskHasEntitlement(current_task(),
                            kIOServiceTestServiceManagementEntitlementKey)) {
    error = EPERM;
    goto finish;
  }
...
SYSCTL_PROC(_kern, OID_AUTO, iokit_test_service_setup,
            CTLTYPE_STRING | CTLFLAG_WR | CTLFLAG_KERN | CTLFLAG_LOCKED, NULL,
            0, sysctl_iokit_test_service_setup, "-", "");

#endif /* DEVELOPMENT || DEBUG */

static __unused void CastCompileTest(OSObject *obj) {
