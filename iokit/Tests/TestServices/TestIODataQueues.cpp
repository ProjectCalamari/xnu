...
bool TestIODataQueues::start(IOService *provider) {
  OSString *str = OSString::withCStringNoCopy("TestIODataQueuesUserClient");
  bool ok = IOService::start(provider);
  if (ok && str != NULL) {
    IOReturn ret;
    ret = IOCircularDataQueueCreateWithEntries(
        kIOCircularDataQueueCreateProducer, 128, 16, &fCDQueue);
...
IOReturn TestIODataQueuesUserClient::clientClose() {
  if (!isInactive()) {
    terminate();
  }
...
bool TestIODataQueuesUserClient::start(IOService *provider) {
...
IOReturn TestIODataQueuesUserClient::clientMemoryForType(
    UInt32 type, IOOptionBits *koptions, IOMemoryDescriptor **kmemory) {
  IOReturn ret = kIOReturnSuccess;

  *kmemory =
      IOCircularDataQueueCopyMemoryDescriptor(fTestIODataQueues->fCDQueue);
  assert(*kmemory);
...
IOReturn TestIODataQueuesUserClient::externalMethod(
