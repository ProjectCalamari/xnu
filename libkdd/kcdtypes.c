 ...
 * pointer to a malloc'ed buffer holding the type definition and each subtype
 * defintion for its fields. It may return NULL if no type with id == type_id is
 * found. Note: The caller is responsible to free() the memory when its no
 * longer used.
 *
 * @discussion
 * This function queries the known type definitions table. If found the
 * defintion data is returned else NULL is returned. It is advised to cache the
 * return value from this function since the data is always going to be the same
 * for same type_id. The definition setup requires memory on heap. The caller
 * should make sure to free() the data once its done with using it.
 *
 */
struct kcdata_type_definition *kcdata_get_typedescription(unsigned type_id,
                                                          uint8_t *buffer,
                                                          uint32_t buffer_size);

/* forward declarations for helper routines */
static uint32_t get_kctype_subtype_size(kctype_subtype_t type);
static void setup_subtype_description(kcdata_subtype_descriptor_t desc,
                                      kctype_subtype_t type, uint32_t offset,
                                      char *name);
static void setup_subtype_array_description(kcdata_subtype_descriptor_t desc,
                                            kctype_subtype_t type,
                                            uint32_t offset, uint32_t count,
                                            char *name);
static void setup_type_definition(struct kcdata_type_definition *d,
                                  uint32_t type, uint32_t num_elems,
                                  char *name);

struct kcdata_type_definition *
kcdata_get_typedescription(unsigned type_id, uint8_t *buffer,
                           uint32_t buffer_size) {
  unsigned int i = 0;
#define _STR_VALUE(x) #x
#define _SUBTYPE_TRUNC(t, s, f, name)                                          \
  do {                                                                         \
    setup_subtype_description(&subtypes[i++], (t), offsetof(s, f), name);      \
    _Static_assert(                                                            \
        sizeof(name) ==                                                        \
            sizeof((struct kcdata_subtype_descriptor *)0)->kcs_name,           \
        "\"" name "\" should fit exactly in kcs_name");                        \
  } while (0)
#define _SUBTYPE(t, s, f)                                                      \
  do {                                                                         \
    setup_subtype_description(&subtypes[i++], (t), offsetof(s, f),             \
                              _STR_VALUE(f));                                  \
    _Static_assert(                                                            \
        sizeof(_STR_VALUE(f)) <=                                               \
            sizeof((struct kcdata_subtype_descriptor *)0)->kcs_name,           \
        "\"" _STR_VALUE(f) "\" should fit in kcs_name");                       \
  } while (0)

#define _SUBTYPE_ARRAY(t, s, f, c)                                             \
  setup_subtype_array_description(&subtypes[i++], (t), offsetof(s, f), (c),    \
                                  _STR_VALUE(f))
#define _STRINGTYPE(f)                                                         \
  setup_subtype_array_description(&subtypes[i++], KC_ST_CHAR, 0, UINT16_MAX, f)

  if (buffer_size < sizeof(struct kcdata_type_definition) || buffer == NULL) {
    return NULL;
  }

  struct kcdata_type_definition *retval =
      (struct kcdata_type_definition *)&buffer[0];
  kcdata_subtype_descriptor_t subtypes =
      (kcdata_subtype_descriptor_t)&buffer[sizeof(
          struct kcdata_type_definition)];
  switch (type_id) {
  case KCDATA_TYPE_STRING_DESC: {
    i = 0;
    setup_subtype_array_description(&subtypes[i++], KC_ST_CHAR, 0,
                                    KCDATA_DESC_MAXLEN, "desc");
    setup_subtype_array_description(&subtypes[i++], KC_ST_CHAR,
                                    KCDATA_DESC_MAXLEN, UINT16_MAX, "data");
  ...
  case KCDATA_TYPE_UINT32_DESC: {
    ...
    setup_subtype_array_description(&subtypes[i++], KC_ST_CHAR, 0,
                                    KCDATA_DESC_MAXLEN, "desc");
    setup_subtype_description(&subtypes[i++], KC_ST_UINT32, KCDATA_DESC_MAXLEN,
                              "data");
  ...
  case KCDATA_TYPE_UINT64_DESC: {
    ...
    setup_subtype_array_description(&subtypes[i++], KC_ST_CHAR, 0,
                                    KCDATA_DESC_MAXLEN, "desc");
    setup_subtype_description(&subtypes[i++], KC_ST_UINT64, KCDATA_DESC_MAXLEN,
                              "data");
  ...
  case KCDATA_TYPE_INT32_DESC: {
    ...
    setup_subtype_array_description(&subtypes[i++], KC_ST_CHAR, 0,
                                    KCDATA_DESC_MAXLEN, "desc");
    setup_subtype_description(&subtypes[i++], KC_ST_INT32, KCDATA_DESC_MAXLEN,
                              "data");
  ...
  case KCDATA_TYPE_INT64_DESC: {
    ...
    setup_subtype_array_description(&subtypes[i++], KC_ST_CHAR, 0,
                                    KCDATA_DESC_MAXLEN, "desc");
    setup_subtype_description(&subtypes[i++], KC_ST_INT64, KCDATA_DESC_MAXLEN,
                              "data");
  ...
  case KCDATA_TYPE_TYPEDEFINTION: {
    i = 0;
    setup_subtype_description(
        &subtypes[i++], KC_ST_UINT32,
        offsetof(struct kcdata_type_definition, kct_type_identifier), "typeID");
    setup_subtype_description(
        &subtypes[i++], KC_ST_UINT32,
        offsetof(struct kcdata_type_definition, kct_num_elements),
        "numOfFields");
    setup_subtype_array_description(
        &subtypes[i++], KC_ST_CHAR,
        offsetof(struct kcdata_type_definition, kct_name), KCDATA_DESC_MAXLEN,
        "name");
    // Note "fields" is an array of run time defined length. So we populate
    // fields at parsing time.
  ...
  case KCDATA_TYPE_CONTAINER_BEGIN: {
    i = 0;
    setup_subtype_description(&subtypes[i++], KC_ST_UINT32, 0,
                              "kcContainerType");
  ...
  case STACKSHOT_KCTYPE_SHAREDCACHE_LOADINFO: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct dyld_shared_cache_loadinfo,
             sharedCacheSlidFirstMapping);
    _SUBTYPE(KC_ST_UINT32, struct dyld_shared_cache_loadinfo_v2, sharedCacheID);
    _SUBTYPE(KC_ST_UINT32, struct dyld_shared_cache_loadinfo_v2,
             sharedCacheFlags);
  ...
  case STACKSHOT_KCTYPE_SHAREDCACHE_INFO: {
    i = 0;
    _SUBTYPE(KC_ST_UINT64, struct dyld_shared_cache_loadinfo_v2,
             sharedCacheSlide);
    _SUBTYPE_ARRAY(KC_ST_UINT8, struct dyld_shared_cache_loadinfo_v2,
                   sharedCacheUUID, 16);
    _SUBTYPE_TRUNC(KC_ST_UINT64, struct dyld_shared_cache_loadinfo_v2,
                   sharedCacheUnreliableSlidBaseAddress,
                   "sharedCacheUnreliableSlidBaseAd");
    _SUBTYPE(KC_ST_UINT64, struct dyld_shared_cache_loadinfo_v2,
             sharedCacheSlidFirstMapping);
    _SUBTYPE(KC_ST_UINT32, struct dyld_shared_cache_loadinfo_v2, sharedCacheID);
    _SUBTYPE(KC_ST_UINT32, struct dyld_shared_cache_loadinfo_v2,
  ...
  case KCDATA_TYPE_MACH_ABSOLUTE_TIME:
    ...
    setup_subtype_description(&subtypes[0], KC_ST_UINT64, 0,
                              "mach_absolute_time");
    break;
  ...
  case KCDATA_TYPE_PROCNAME:
    i = 0;
    setup_subtype_array_description(&subtypes[i++], KC_ST_CHAR, 0, 64,
                                    "proc_name");
  ...
  case KCDATA_TYPE_LIBRARY_AOTINFO: {
    ...
    _SUBTYPE_ARRAY(KC_ST_UINT8, struct user64_dyld_aot_info, aotImageKey,
  ...
  case STACKSHOT_KCTYPE_IOSTATS: {
    ...
    _SUBTYPE_ARRAY(KC_ST_UINT64, struct io_stats_snapshot, ss_io_priority_count,
                   STACKSHOT_IO_NUM_PRIORITIES);
    _SUBTYPE_ARRAY(KC_ST_UINT64, struct io_stats_snapshot, ss_io_priority_size,
                   STACKSHOT_IO_NUM_PRIORITIES);
  ...
  case STACKSHOT_KCTYPE_GLOBAL_MEM_STATS: {
    ...
    _SUBTYPE(KC_ST_UINT8, struct mem_and_io_snapshot,
  ...
  case STACKSHOT_KCTYPE_TASK_SNAPSHOT: {
    ...
    _SUBTYPE_TRUNC(KC_ST_UINT64, struct task_snapshot_v3,
                   ts_user_time_in_terminated_threads,
                   "ts_user_time_in_terminated_thre");
    _SUBTYPE_TRUNC(KC_ST_UINT64, struct task_snapshot_v3,
                   ts_system_time_in_terminated_threads,
                   "ts_system_time_in_terminated_th");
  ...
  case STACKSHOT_KCTYPE_TRANSITIONING_TASK_SNAPSHOT: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct transitioning_task_snapshot,
             tts_transition_type);
    _SUBTYPE(KC_ST_INT32, struct transitioning_task_snapshot, tts_pid);
    _SUBTYPE_ARRAY(KC_ST_CHAR, struct transitioning_task_snapshot, tts_p_comm,
                   32);
  ...
  case STACKSHOT_KCTYPE_TASK_DELTA_SNAPSHOT: {
    ...
    _SUBTYPE_TRUNC(KC_ST_UINT64, struct task_delta_snapshot_v2,
                   tds_user_time_in_terminated_threads,
                   "tds_user_time_in_terminated_thr");
    _SUBTYPE_TRUNC(KC_ST_UINT64, struct task_delta_snapshot_v2,
                   tds_system_time_in_terminated_threads,
                   "tds_system_time_in_terminated_t");
    _SUBTYPE(KC_ST_UINT64, struct task_delta_snapshot_v2, tds_task_size);
    _SUBTYPE(KC_ST_UINT64, struct task_delta_snapshot_v2,
  ...
  case STACKSHOT_KCTYPE_THREAD_SNAPSHOT: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct thread_snapshot_v3,
  ...
  case STACKSHOT_KCTYPE_THREAD_DELTA_SNAPSHOT: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct thread_delta_snapshot_v2,
    ...
    _SUBTYPE(KC_ST_UINT64, struct thread_delta_snapshot_v3,
  ...
  case STACKSHOT_KCTYPE_KERN_STACKFRAME:
    ...
    setup_subtype_description(&subtypes[1], KC_ST_UINT32, sizeof(uint32_t),
                              "sp");
    break;
  ...
  case STACKSHOT_KCTYPE_KERN_STACKFRAME64:
    ...
    setup_subtype_description(&subtypes[1], KC_ST_UINT64, sizeof(uint64_t),
                              "sp");
    break;
  ...
  case STACKSHOT_KCTYPE_USER_ASYNC_START_INDEX:
    ...
    setup_subtype_description(&subtypes[0], KC_ST_UINT32, 0,
                              "user_async_start_index");
    break;
  ...
  case STACKSHOT_KCTYPE_NONRUNNABLE_TIDS:
    ...
    setup_subtype_description(&subtypes[0], KC_ST_INT64, 0,
                              "nonrunnable_threads");
    break;
  ...
  case STACKSHOT_KCTYPE_CODESIGNING_INFO:
    ...
    _SUBTYPE(KC_ST_UINT32, struct stackshot_task_codesigning_info,
             cs_trust_level);
    setup_type_definition(retval, type_id, i,
  ...
  case STACKSHOT_KCTYPE_EXCLAVE_IPCSTACKENTRY_INFO:
    ...
    _SUBTYPE(KC_ST_UINT64, struct exclave_ipcstackentry_info,
             eise_invocationid);
  ...
  case STACKSHOT_KCTYPE_EXCLAVE_TEXTLAYOUT_SEGMENTS:
    i = 0;
    _SUBTYPE_ARRAY(KC_ST_UINT8, struct exclave_textlayout_segment_v2,
                   layoutSegment_uuid, 16);
    _SUBTYPE(KC_ST_UINT64, struct exclave_textlayout_segment_v2,
             layoutSegment_loadAddress);
    _SUBTYPE(KC_ST_UINT64, struct exclave_textlayout_segment_v2,
             layoutSegment_rawLoadAddress);
  ...
  case STACKSHOT_KCTYPE_DELTA_SINCE_TIMESTAMP: {
    i = 0;
    setup_subtype_description(&subtypes[i++], KC_ST_UINT64, 0,
  ...
  case TASK_CRASHINFO_BSDINFOWITHUNIQID: {
    i = 0;
    _SUBTYPE_ARRAY(KC_ST_UINT8, struct crashinfo_proc_uniqidentifierinfo,
                   p_uuid, 16);
    _SUBTYPE(KC_ST_UINT64, struct crashinfo_proc_uniqidentifierinfo,
             p_uniqueid);
    _SUBTYPE(KC_ST_UINT64, struct crashinfo_proc_uniqidentifierinfo,
             p_puniqueid);
  ...
  case TASK_CRASHINFO_PPID: {
    ...
    /*      /\* */
    /*       * rusage is a complex structure and is only for legacy use for
     * crashed processes rusage info. */
    /*       * So we just consider it as opaque data. */
    /*       *\/ */
    /*      i = 0; */
    /*      setup_subtype_array_description(&subtypes[i++], KC_ST_UINT8, 0,
     * sizeof(struct rusage), "rusage"); */
  ...
  case TASK_CRASHINFO_RUSAGE_INFO: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct rusage_info_v3,
             ri_cpu_time_qos_user_initiated);
    _SUBTYPE_TRUNC(KC_ST_UINT64, struct rusage_info_v3,
                   ri_cpu_time_qos_user_interactive,
                   "ri_cpu_time_qos_user_interactiv");
  ...
  case STACKSHOT_KCTYPE_STACKSHOT_DURATION: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct stackshot_duration_v2,
  ...
  case STACKSHOT_KCTYPE_STACKSHOT_FAULT_STATS: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct stackshot_fault_stats,
  ...
  case STACKSHOT_KCTYPE_THREAD_GROUP_SNAPSHOT: {
    ...
    _SUBTYPE_ARRAY(KC_ST_CHAR, struct thread_group_snapshot_v3, tgs_name_cont,
                   16);
  ...
  case STACKSHOT_KCTYPE_JETSAM_COALITION_SNAPSHOT: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct jetsam_coalition_snapshot,
  ...
  case STACKSHOT_KCTYPE_INSTRS_CYCLES: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct instrs_cycles_snapshot_v2,
             ics_p_instructions);
  ...
  case STACKSHOT_KCTYPE_USER_STACKTOP: {
    ...
    _SUBTYPE_ARRAY(KC_ST_UINT8, struct stack_snapshot_stacktop, stack_contents,
                   8);
  ...
  case TASK_CRASHINFO_EXCEPTION_CODES: {
    ...
    for (i = 0; i < EXCEPTION_CODE_MAX; i++) {
      ...
      setup_subtype_description(&subtypes[i], KC_ST_UINT64,
                                i * (sizeof(uint64_t)), codenum);
    }
  ...
  case EXIT_REASON_CODESIGNING_INFO: {
    ...
    _SUBTYPE(KC_ST_UINT32, struct codesigning_exit_reason_info,
  ...
  case STACKSHOT_KCTYPE_THREAD_TURNSTILEINFO: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct stackshot_thread_turnstileinfo_v2,
             turnstile_context);
    _SUBTYPE(KC_ST_UINT8, struct stackshot_thread_turnstileinfo_v2,
             turnstile_priority);
    _SUBTYPE(KC_ST_UINT8, struct stackshot_thread_turnstileinfo_v2,
             number_of_hops);
    _SUBTYPE(KC_ST_UINT64, struct stackshot_thread_turnstileinfo_v2,
             turnstile_flags);
    _SUBTYPE(KC_ST_INT16, struct stackshot_thread_turnstileinfo_v2,
             portlabel_id);
  ...
  case STACKSHOT_KCTYPE_LATENCY_INFO: {
    i = 0;
    _SUBTYPE(KC_ST_UINT64, struct stackshot_latency_collection,
             latency_version);
    _SUBTYPE(KC_ST_UINT64, struct stackshot_latency_collection, setup_latency);
    _SUBTYPE(KC_ST_UINT64, struct stackshot_latency_collection,
             total_task_iteration_latency);
    _SUBTYPE_TRUNC(KC_ST_UINT64, struct stackshot_latency_collection,
                   total_terminated_task_iteration_latency,
                   "total_terminated_task_iteration");
  ...
  case STACKSHOT_KCTYPE_LATENCY_INFO_TASK: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct stackshot_latency_task,
  ...
  case STACKSHOT_KCTYPE_LATENCY_INFO_THREAD: {
    ...
    _SUBTYPE(KC_ST_UINT64, struct stackshot_latency_thread,
             cur_thsnap1_latency);
    _SUBTYPE(KC_ST_UINT64, struct stackshot_latency_thread,
  ...
  case TASK_CRASHINFO_KERNEL_TRIAGE_INFO_V1: {
    i = 0;
    _SUBTYPE_ARRAY(KC_ST_CHAR, struct kernel_triage_info_v1, triage_string1,
                   MAX_TRIAGE_STRING_LEN);
    _SUBTYPE_ARRAY(KC_ST_CHAR, struct kernel_triage_info_v1, triage_string2,
                   MAX_TRIAGE_STRING_LEN);
    _SUBTYPE_ARRAY(KC_ST_CHAR, struct kernel_triage_info_v1, triage_string3,
                   MAX_TRIAGE_STRING_LEN);
    _SUBTYPE_ARRAY(KC_ST_CHAR, struct kernel_triage_info_v1, triage_string4,
                   MAX_TRIAGE_STRING_LEN);
    _SUBTYPE_ARRAY(KC_ST_CHAR, struct kernel_triage_info_v1, triage_string5,
                   MAX_TRIAGE_STRING_LEN);
  ...
  assert(retval == NULL ||
         (buffer_size > sizeof(struct kcdata_type_definition) +
                            (retval->kct_num_elements *
...
static void setup_subtype_array_description(kcdata_subtype_descriptor_t desc,
  ...
  desc->kcs_elem_size =
      KCS_SUBTYPE_PACK_SIZE(count, get_kctype_subtype_size(type));
