...
#define _SLOT_DESC(_chrd, _idx) (SLOT_DESC_USD(&(_chrd)->chrd_slot_desc[_idx]))

#define _METADATA(_chrd, _ring, _midx)                                         \
  ((void *)((_chrd)->chrd_md_base_addr + ((_midx) * (_ring)->ring_md_size) +   \
            METADATA_PREAMBLE_SZ))

#define _SLOT_METADATA(_chrd, _ring, _idx)                                     \
  _METADATA(_chrd, _ring, _SLOT_DESC(_chrd, _idx)->sd_md_idx)

#define _SLOT_METADATA_IDX_VERIFY(_chrd, _md, _midx)                           \
  do {                                                                         \
    if (__improbable((_md) !=                                                  \
                     _METADATA((_chrd), (_chrd)->chrd_ring, (_midx))) &&       \
...
#define _SLOT_BFT_METADATA_IDX_VERIFY(_chrd, _md, _midx)                       \
  do {                                                                         \
    if (__improbable(                                                          \
            (mach_vm_address_t)(_md) !=                                        \
...
#define _SLOT_DESC_VERIFY(_chrd, _sdp)                                         \
  do {                                                                         \
...
#define _METADATA_VERIFY(_chrd, _md)                                           \
  do {                                                                         \
    if (__improbable(                                                          \
...
#define _PKT_BUFCNT_VERIFY(_chrd, _bcnt, _bmax)                                \
  do {                                                                         \
...
#define _SCHEMA_VER_VERIFY(_chd)                                               \
  do {                                                                         \
    ...
    if (CHD_SCHEMA(_chd)->csm_ver != CSM_CURRENT_VERSION) {                    \
      ...
      (void)_mach_snprintf(                                                    \
          ...
          CHD_SCHEMA(_chd)->csm_kern_name, CHD_SCHEMA(_chd)->csm_kern_uuid[0], \
...
#define _SLOT_ATTACH_METADATA(_usd, _md_idx)                                   \
  do {                                                                         \
    (_usd)->sd_md_idx = (_md_idx);                                             \
    (_usd)->sd_flags |= SD_IDX_VALID;                                          \
  } while (0)

#define _SLOT_DETACH_METADATA(_usd)                                            \
  do {                                                                         \
...
#define _CHANNEL_RING_DEF_BUF(_chrd, _ring, _idx)                              \
  ((_chrd)->chrd_def_buf_base_addr + ((_idx) * (_ring)->ring_def_buf_size))

#define _CHANNEL_RING_LARGE_BUF(_chrd, _ring, _idx)                            \
  ((_chrd)->chrd_large_buf_base_addr + ((_idx) * (_ring)->ring_large_buf_size))
...
#define _CHANNEL_IS_DEFUNCT(_chd) (!(CHD_SCHEMA(_chd)->csm_flags & CSM_ACTIVE))

#define _CH_PKT_GET_FIRST_BUFLET(_pkt, _bft, _chrd, _ring)                     \
  do {                                                                         \
    if (__probable((_pkt)->pkt_qum_buf.buf_idx != OBJ_IDX_NONE)) {             \
      ...
      (_bft) =                                                                 \
          _CHANNEL_RING_BFT(_chrd, _ring, (_pkt)->pkt_qum_buf.buf_nbft_idx);   \
    } else {                                                                   \
      (_bft) = NULL;                                                           \
    }                                                                          \
  } while (0)
 ...
 */
__attribute__((
    visibility("hidden"))) static uint64_t __os_ch_md_redzone_cookie = 0;

__attribute__((always_inline, visibility("hidden"))) static inline uint32_t
...
os_channel_init_ring(struct channel_ring_desc *chrd, struct channel *chd,
                     uint32_t ring_index) {
  ...
  *(uint32_t *)(uintptr_t)&chrd->chrd_max_bufs = CHD_PARAMS(chd)->nxp_max_frags;
}

__attribute__((always_inline,
               visibility("hidden"))) static inline mach_vm_address_t
_initialize_metadata_address(const channel_ring_t chrd,
  ...
  _Static_assert(sizeof(p->pkt_qum_buf.buf_addr) == sizeof(mach_vm_address_t),
                 "invalid buffer size");
  /*
  ...
  if (__improbable(p->pkt_qum_buf.buf_idx == OBJ_IDX_NONE)) {
    *__DECONST(mach_vm_address_t *, &p->pkt_qum_buf.buf_addr) = 0;
    *__DECONST(mach_vm_address_t *, &p->pkt_qum_buf.buf_nbft_addr) =
  ...
  for (i = 0; (i < bcnt) && (ubft != NULL); i++) {
    ...
    if (__probable(pbft->buf_idx != OBJ_IDX_NONE)) {
      *(mach_vm_address_t *)(uintptr_t)&(pbft->buf_addr) =
          _CHANNEL_RING_BUF(chrd, ring, pbft);
    } else {
      *(mach_vm_address_t *)(uintptr_t)&(pbft->buf_addr) = NULL;
    }
    if (pbft->buf_nbft_idx != OBJ_IDX_NONE) {
      ubft = _CHANNEL_RING_BFT(chrd, ring, pbft->buf_nbft_idx);
    } else {
      ubft = NULL;
    }
 ...
 * 'o' - valid
 */
__attribute__((always_inline, visibility("hidden"))) static inline int
...
channel_t os_channel_create_extended(const uuid_t uuid, const nexus_port_t port,
                                     const ring_dir_t dir, const ring_id_t ring,
                                     const channel_attr_t cha) {
  ...
  if ((init.ci_ch_mode & CHMODE_EVENT_RING) &&
      ((num_event_rings == 0) || !(ci->cinfo_ch_mode & CHMODE_EVENT_RING))) {
    err = errno = ENXIO;
    goto done;
  }
  ...
  *(nexus_meta_subtype_t *)&chd->chd_md_subtype =
  ...
  if (CHD_SCHEMA(chd)->csm_stats_ofs != 0) {
    *(void **)(uintptr_t)&chd->chd_nx_stats = _CHANNEL_OFFSET(
        void *, CHD_INFO(chd)->cinfo_mem_base, CHD_SCHEMA(chd)->csm_stats_ofs);
  }

  if (CHD_SCHEMA(chd)->csm_flowadv_ofs != 0) {
    *(struct __flowadv_entry **)(uintptr_t)&chd->chd_nx_flowadv =
        _CHANNEL_OFFSET(struct __flowadv_entry *, CHD_INFO(chd)->cinfo_mem_base,
  ...
  if (CHD_SCHEMA(chd)->csm_nexusadv_ofs != 0) {
    ...
    *(struct __kern_nexus_adv_metadata **)(uintptr_t)&chd->chd_nx_adv =
    ...
    if (adv_md->knam_version != NX_ADVISORY_MD_CURRENT_VERSION &&
      ...
      SK_ABORT_WITH_CAUSE("nexus advisory metadata version"
                          " mismatch",
                          NX_ADVISORY_MD_CURRENT_VERSION);
      /* NOTREACHED */
      __builtin_unreachable();
    }
    if (chd->chd_nx_adv->knam_type == NEXUS_ADVISORY_TYPE_NETIF) {
      ...
      if (netif_adv->nna_version != NX_NETIF_ADVISORY_CURRENT_VERSION &&
          !_CHANNEL_IS_DEFUNCT(chd)) {
    ...
    } else if (chd->chd_nx_adv->knam_type == NEXUS_ADVISORY_TYPE_FLOWSWITCH) {
      struct sk_nexusadv *fsw_adv;
      fsw_adv = CHD_NX_ADV_FSW(adv_md);
      if (fsw_adv->nxadv_ver != NX_FLOWSWITCH_ADVISORY_CURRENT_VERSION &&
          !_CHANNEL_IS_DEFUNCT(chd)) {
        SK_ABORT_WITH_CAUSE("nexus advisory "
  ...
  for (i = 0; i < num_tx_rings; i++) {
    ...
    os_channel_init_ring(&chd->chd_rings[ring_index], chd, ring_index);
  }
  ...
  if (init.ci_ch_mode & CHMODE_USER_PACKET_POOL) {
    ...
    *__DECONST(uint8_t *, &chd->chd_alloc_ring_idx) =
    ...
    if (num_allocator_rings > 2) {
      ...
      *__DECONST(uint8_t *, &chd->chd_buf_alloc_ring_idx) = CHD_RING_IDX_NONE;
      *__DECONST(uint8_t *, &chd->chd_buf_free_ring_idx) = CHD_RING_IDX_NONE;
      *__DECONST(uint8_t *, &chd->chd_free_ring_idx) =
          chd->chd_alloc_ring_idx + 1;
    }
    if (num_large_buf_alloc_rings > 0) {
      *__DECONST(uint8_t *, &chd->chd_large_buf_alloc_ring_idx) =
          num_tx_rings + num_rx_rings + num_allocator_rings + num_event_rings;
    } else {
      *__DECONST(uint8_t *, &chd->chd_large_buf_alloc_ring_idx) =
          CHD_RING_IDX_NONE;
    }
  } else {
    *__DECONST(uint8_t *, &chd->chd_alloc_ring_idx) = CHD_RING_IDX_NONE;
    *__DECONST(uint8_t *, &chd->chd_free_ring_idx) = CHD_RING_IDX_NONE;
    *__DECONST(uint8_t *, &chd->chd_buf_alloc_ring_idx) = CHD_RING_IDX_NONE;
    *__DECONST(uint8_t *, &chd->chd_buf_free_ring_idx) = CHD_RING_IDX_NONE;
    *__DECONST(uint8_t *, &chd->chd_large_buf_alloc_ring_idx) =
        CHD_RING_IDX_NONE;
  ...
  if (__os_ch_md_redzone_cookie == 0) {
    __os_ch_md_redzone_cookie = CHD_SCHEMA(chd)->csm_md_redzone_cookie;
  }
...
channel_t os_channel_create(const uuid_t uuid, const nexus_port_t port) {
  return os_channel_create_extended(uuid, port, CHANNEL_DIR_TX_RX,
                                    CHANNEL_RING_ID_ANY, NULL);
}

int os_channel_get_fd(const channel_t chd) { return chd->chd_fd; }

int os_channel_read_attr(const channel_t chd, channel_attr_t cha) {
  int err;

  if ((err = __channel_get_info(chd->chd_fd, CHD_INFO(chd), CHD_INFO_SIZE)) ==
      0) {
    os_channel_info2attr(chd, cha);
  }
...
int os_channel_write_attr(const channel_t chd, channel_attr_t cha) {
  int err = 0;

  if (CHD_INFO(chd)->cinfo_tx_lowat.cet_unit != cha->cha_tx_lowat.cet_unit ||
      CHD_INFO(chd)->cinfo_tx_lowat.cet_value != cha->cha_tx_lowat.cet_value) {
    if ((err = __channel_set_opt(chd->chd_fd, CHOPT_TX_LOWAT_THRESH,
                                 &cha->cha_tx_lowat,
                                 sizeof(cha->cha_tx_lowat))) != 0) {
      goto done;
    }
  ...
  if (CHD_INFO(chd)->cinfo_rx_lowat.cet_unit != cha->cha_rx_lowat.cet_unit ||
      CHD_INFO(chd)->cinfo_rx_lowat.cet_value != cha->cha_rx_lowat.cet_value) {
    if ((err = __channel_set_opt(chd->chd_fd, CHOPT_RX_LOWAT_THRESH,
                                 &cha->cha_rx_lowat,
                                 sizeof(cha->cha_rx_lowat))) != 0) {
      goto done;
    }
...
int os_channel_read_nexus_extension_info(const channel_t chd, nexus_type_t *nt,
                                         uint64_t *ext) {
...
int os_channel_sync(const channel_t chd, const sync_mode_t mode) {
  if (__improbable(mode != CHANNEL_SYNC_TX && mode != CHANNEL_SYNC_RX)) {
    return EINVAL;
  }

  return __channel_sync(
      chd->chd_fd, mode,
      (mode == CHANNEL_SYNC_TX)
          ? chd->chd_sync_flags
          : (chd->chd_sync_flags &
             ~(CHANNEL_SYNCF_ALLOC | CHANNEL_SYNCF_ALLOC_BUF)));
}

void os_channel_destroy(channel_t chd) {
  if (chd->chd_fd != -1) {
...
int os_channel_is_defunct(channel_t chd) { return _CHANNEL_IS_DEFUNCT(chd); }

__attribute__((always_inline, visibility("hidden"))) static inline ring_id_t
...
ring_id_t os_channel_ring_id(const channel_t chd, const ring_id_type_t type) {
  return _ring_id(CHD_INFO(chd), type);
}

channel_ring_t os_channel_tx_ring(const channel_t chd, const ring_id_t rid) {
...
channel_ring_t os_channel_rx_ring(const channel_t chd, const ring_id_t rid) {
  struct ch_info *ci = CHD_INFO(chd);
 ...
 */
int os_channel_pending(const channel_ring_t chrd) {
  struct __user_channel_ring *ring =
      __DECONST(struct __user_channel_ring *, chrd->chrd_ring);
  return ring->ring_head != ring->ring_khead;
}

uint64_t os_channel_ring_sync_time(const channel_ring_t chrd) {
  return chrd->chrd_ring->ring_sync_time;
}

uint64_t os_channel_ring_notify_time(const channel_ring_t chrd) {
  return chrd->chrd_ring->ring_notify_time;
}

uint32_t os_channel_available_slot_count(const channel_ring_t chrd) {
  const struct __user_channel_ring *ring = chrd->chrd_ring;
  uint32_t count;
  int n;
...
int os_channel_advance_slot(channel_ring_t chrd, const channel_slot_t slot) {
  struct __user_channel_ring *ring =
      __DECONST(struct __user_channel_ring *, chrd->chrd_ring);
  slot_idx_t idx;
  int err;
...
channel_slot_t os_channel_get_next_slot(const channel_ring_t chrd,
                                        const channel_slot_t slot0,
                                        slot_prop_t *prop) {
  const struct __user_channel_ring *ring = chrd->chrd_ring;
  ...
  if (__probable(slot != NULL)) {
    ...
    if (__probable(prop != NULL)) {
      ...
      prop->sp_len =
          (ring_kind == CR_KIND_TX) ? ring->ring_def_buf_size : q->qum_len;
    }
  }

  return __improbable(_CHANNEL_RING_IS_DEFUNCT(chrd)) ? NULL
                                                      : (channel_slot_t)slot;
}

void os_channel_set_slot_properties(const channel_ring_t chrd,
...
packet_t os_channel_slot_get_packet(const channel_ring_t chrd,
                                    const channel_slot_t slot) {
  const struct __user_channel_ring *ring = chrd->chrd_ring;
...
void *os_channel_get_stats_region(const channel_t chd,
  ...
  if (sp == NULL || CHD_SCHEMA(chd)->csm_stats_type != NEXUS_STATS_TYPE_FSW) {
    return NULL;
  }
...
void *os_channel_get_advisory_region(const channel_t chd) {
  struct __kern_nexus_adv_metadata *adv_md;
  /*
   ...
   */
  adv_md = CHD_NX_ADV_MD(chd);
  if (adv_md == NULL || adv_md->knam_type != NEXUS_ADVISORY_TYPE_FLOWSWITCH) {
    return NULL;
  }
...
_flowadv_id_equal(struct __flowadv_entry *fe, uuid_t id) {
  ...
  if (__probable(IS_P2ALIGNED(id, sizeof(uint64_t)))) {
    ...
    return fe->fae_id_64[0] == id_64[0] && fe->fae_id_64[1] == id_64[1];
  } else if (__probable(IS_P2ALIGNED(id, sizeof(uint32_t)))) {
    uint32_t *id_32 = (uint32_t *)(uintptr_t)id;
    return fe->fae_id_32[0] == id_32[0] && fe->fae_id_32[1] == id_32[1] &&
           fe->fae_id_32[2] == id_32[2] && fe->fae_id_32[3] == id_32[3];
  }

  return UUID_COMPARE(fe->fae_id, id);
}

int os_channel_flow_admissible(const channel_ring_t chrd, uuid_t flow_id,
  ...
  return __improbable((fe->fae_flags & FLOWADVF_SUSPENDED) != 0) ? ENOBUFS : 0;
}

int os_channel_flow_adv_get_ce_count(__unused const channel_ring_t chrd,
                                     __unused uuid_t flow_id,
                                     __unused const flowadv_idx_t flow_index,
                                     __unused uint32_t *ce_cnt,
                                     __unused uint32_t *pkt_cnt) {
  return 0;
}

int os_channel_flow_adv_get_feedback(const channel_ring_t chrd, uuid_t flow_id,
                                     const flowadv_idx_t flow_index,
...
channel_attr_t os_channel_attr_create(void) {
...
channel_attr_t os_channel_attr_clone(const channel_attr_t cha) {
  ...
  if (ncha != NULL && cha != NULL) {
    ...
    if (cha->cha_key != NULL && cha->cha_key_len != 0 &&
        os_channel_attr_set_key(ncha, cha->cha_key, cha->cha_key_len) != 0) {
...
int os_channel_attr_set(const channel_attr_t cha,
  ...
  case CHANNEL_ATTR_RX_LOWAT_UNIT:
    ...
    case CHANNEL_THRESHOLD_UNIT_SLOTS:
      if (type == CHANNEL_ATTR_TX_LOWAT_UNIT) {
        cha->cha_tx_lowat.cet_unit = (channel_threshold_unit_t)value;
      } else {
        cha->cha_rx_lowat.cet_unit = (channel_threshold_unit_t)value;
      }
      goto done;
    }
    err = EINVAL;
...
int os_channel_attr_set_key(const channel_attr_t cha, const void *key,
...
int os_channel_attr_get(const channel_attr_t cha,
...
int os_channel_attr_get_key(const channel_attr_t cha, void *key,
                            uint32_t *key_len) {
  int err = 0;
...
os_channel_info2attr(struct channel *chd, channel_attr_t cha) {
  ...
  cha->cha_user_packet_pool =
      !!(cinfo->cinfo_ch_mode & CHMODE_USER_PACKET_POOL);
  cha->cha_nexus_defunct_ok = !!(cinfo->cinfo_ch_mode & CHMODE_DEFUNCT_OK);
  ...
  cha->cha_enable_event_ring = (cinfo->cinfo_ch_mode & CHMODE_EVENT_RING) != 0;
  cha->cha_low_latency = (cinfo->cinfo_ch_mode & CHMODE_LOW_LATENCY) != 0;

  caps = CHD_PARAMS(chd)->nxp_capabilities;
  if (caps & NXPCAP_CHECKSUM_PARTIAL) {
    cha->cha_nexus_checksum_offload = CHANNEL_NEXUS_CHECKSUM_PARTIAL;
  } else {
...
void os_channel_attr_destroy(channel_attr_t cha) {
  if (cha->cha_key != NULL) {
    free(cha->cha_key);
    cha->cha_key = NULL;
  }
...
static int os_channel_packet_alloc_common(const channel_t chd, packet_t *ph,
                                          bool large) {
  struct __user_channel_ring *ring;
  ...
  chrd = &chd->chd_rings[large ? chd->chd_large_buf_alloc_ring_idx
                               : chd->chd_alloc_ring_idx];
  ...
  if (__improbable(idx == ring->ring_tail)) {
    return __improbable(_CHANNEL_IS_DEFUNCT(chd)) ? ENXIO : ENOMEM;
  }
...
int os_channel_packet_alloc(const channel_t chd, packet_t *ph) {
  return os_channel_packet_alloc_common(chd, ph, false);
}

int os_channel_large_packet_alloc(const channel_t chd, packet_t *ph) {
  return os_channel_packet_alloc_common(chd, ph, true);
}

int os_channel_packet_free(const channel_t chd, packet_t ph) {
  struct __user_channel_ring *ring;
  struct channel_ring_desc *chrd;
  slot_idx_t idx;
  obj_idx_t midx;
...
int os_channel_slot_attach_packet(const channel_ring_t chrd,
...
int os_channel_slot_detach_packet(const channel_ring_t chrd,
                                  const channel_slot_t slot, packet_t ph) {
  slot_idx_t idx;
  ...
  if (__improbable(!SD_VALID_METADATA(SLOT_DESC_USD(slot)))) {
    if (!_CHANNEL_RING_IS_DEFUNCT(chrd)) {
      SK_ABORT_WITH_CAUSE("Slot has no attached packet", slot);
      /* NOTREACHED */
      __builtin_unreachable();
    }
    goto done;
  ...
  if (__improbable(ph !=
                   SK_PTR_ENCODE(_SLOT_METADATA(chrd, chrd->chrd_ring, idx),
                                 chrd->chrd_md_type, chrd->chrd_md_subtype))) {
    if (!_CHANNEL_RING_IS_DEFUNCT(chrd)) {
...
__attribute__((visibility("hidden"))) static inline int
  ...
  while ((uint32_t)nbfts-- > curr_ws) {
    ...
    *(mach_vm_address_t *)(uintptr_t)&(ubft->buf_addr) =
    ...
    if (__improbable(ubft->buf_addr == 0)) {
      SK_ABORT_WITH_CAUSE("buflet with NULL buffer", ubft->buf_idx);
      /* NOTREACHED */
      __builtin_unreachable();
    }
    ...
    if (__improbable(nbft_idx != OBJ_IDX_NONE)) {
      if (_CHANNEL_IS_DEFUNCT(chd)) {
        ...
        SK_ABORT_WITH_CAUSE("buflet with invalid nidx", nbft_idx);
        /* NOTREACHED */
        __builtin_unreachable();
      }
    }
...
int os_channel_packet_pool_purge(const channel_t chd) {
  struct ch_info *ci = CHD_INFO(chd);
  int err;
  ...
  err = __channel_sync(
      chd->chd_fd, CHANNEL_SYNC_UPP,
      ((chd->chd_sync_flags & ~CHANNEL_SYNCF_FREE) | CHANNEL_SYNCF_PURGE));
  if (__improbable(err != 0)) {
    if (!_CHANNEL_IS_DEFUNCT(chd)) {
      SK_ABORT_WITH_CAUSE("packet pool purge sync failed", err);
      /* NOTREACHED */
      __builtin_unreachable();
    }
    return err;
  ...
  if (__improbable(err != 0)) {
    if (!_CHANNEL_IS_DEFUNCT(chd)) {
      SK_ABORT_WITH_CAUSE("packet pool free sync failed", err);
      /* NOTREACHED */
      __builtin_unreachable();
    }
    return err;
...
int os_channel_get_next_event_handle(const channel_t chd,
  ...
  if (__improbable((ehandle == NULL) || (etype == NULL) || (nevents == NULL))) {
    return EINVAL;
  }
  ...
  *ehandle = NULL;
  ...
  if (__improbable(idx == ring->ring_tail)) {
    return __improbable(_CHANNEL_IS_DEFUNCT(chd)) ? ENXIO : ENODATA;
  }
  _SLOT_DESC_VERIFY(chrd, _SLOT_DESC(chrd, idx));
  ...
  *ehandle = SK_PTR_ENCODE(qum, chrd->chrd_md_type, chrd->chrd_md_subtype);
...
int os_channel_event_free(const channel_t chd,
                          os_channel_event_handle_t ehandle) {
  return os_channel_packet_free(chd, (packet_t)ehandle);
}

int os_channel_get_interface_advisory(
    const channel_t chd, struct ifnet_interface_advisory *advisory) {
  ...
  if (intf_adv->cksum !=
      os_cpu_copy_in_cksum(&intf_adv->adv, advisory, sizeof(*advisory), 0)) {
    return _CHANNEL_IS_DEFUNCT(chd) ? ENXIO : EAGAIN;
  }
  return 0;
}

int os_channel_configure_interface_advisory(const channel_t chd,
                                            boolean_t enable) {
  uint32_t value = enable;

  return __channel_set_opt(chd->chd_fd, CHOPT_IF_ADV_CONF, &value,
                           sizeof(value));
}

int os_channel_buflet_alloc(const channel_t chd, buflet_t *bft) {
  struct __user_channel_ring *ring;
  struct channel_ring_desc *chrd;
  struct __user_buflet *ubft;
  obj_idx_t nbft_idx;
  ...
  *(mach_vm_address_t *)(uintptr_t)&(ubft->buf_addr) =
      _CHANNEL_RING_BUF(chrd, ring, ubft);
  if (__improbable(ubft->buf_addr == 0)) {
    SK_ABORT_WITH_CAUSE("buflet alloc with NULL buffer", ubft->buf_idx);
    /* NOTREACHED */
    __builtin_unreachable();
  }
  nbft_idx = ubft->buf_nbft_idx;
  if (__improbable(nbft_idx != OBJ_IDX_NONE)) {
    if (_CHANNEL_IS_DEFUNCT(chd)) {
      ...
      SK_ABORT_WITH_CAUSE("buflet alloc with invalid nidx", nbft_idx);
      /* NOTREACHED */
      __builtin_unreachable();
    }
  }
...
int os_channel_buflet_free(const channel_t chd, buflet_t ubft) {
  struct __user_channel_ring *ring;
  struct channel_ring_desc *chrd;
  slot_idx_t idx;
  obj_idx_t midx;
...
int os_channel_get_upp_buffer_stats(const channel_t chd, uint64_t *buffer_total,
