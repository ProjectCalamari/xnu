...
nexus_attr_t os_nexus_attr_create(void) {
...
nexus_attr_t os_nexus_attr_clone(const nexus_attr_t nxa) {
...
int os_nexus_attr_set(const nexus_attr_t nxa, const nexus_attr_type_t type,
                      const uint64_t value) {
  return __nexus_attr_set(nxa, type, value);
}

int os_nexus_attr_get(const nexus_attr_t nxa, const nexus_attr_type_t type,
                      uint64_t *value) {
  return __nexus_attr_get(nxa, type, value);
}

void os_nexus_attr_destroy(nexus_attr_t nxa) { free(nxa); }

nexus_controller_t os_nexus_controller_create(void) {
...
int os_nexus_controller_get_fd(const nexus_controller_t ncd) {
  return ncd->ncd_fd;
}

int os_nexus_controller_register_provider(const nexus_controller_t ncd,
                                          const nexus_name_t name,
                                          const nexus_type_t type,
                                          const nexus_attr_t nxa,
                                          uuid_t *prov_uuid) {
  struct nxprov_reg reg;
  int err;

  if ((err = __nexus_provider_reg_prepare(&reg, name, type, nxa)) == 0) {
    err = __nexus_register(ncd->ncd_fd, &reg, sizeof(reg), prov_uuid,
                           sizeof(uuid_t));
  }
  return err;
}

int os_nexus_controller_deregister_provider(const nexus_controller_t ncd,
                                            const uuid_t prov_uuid) {
  return __nexus_deregister(ncd->ncd_fd, prov_uuid, sizeof(uuid_t));
}

int os_nexus_controller_alloc_provider_instance(const nexus_controller_t ncd,
                                                const uuid_t prov_uuid,
                                                uuid_t *nx_uuid) {
  return __nexus_create(ncd->ncd_fd, prov_uuid, sizeof(uuid_t), nx_uuid,
                        sizeof(uuid_t));
}

int os_nexus_controller_free_provider_instance(const nexus_controller_t ncd,
                                               const uuid_t nx_uuid) {
  return __nexus_destroy(ncd->ncd_fd, nx_uuid, sizeof(uuid_t));
}

int os_nexus_controller_bind_provider_instance(
    const nexus_controller_t ncd, const uuid_t nx_uuid, const nexus_port_t port,
    const pid_t pid, const uuid_t exec_uuid, const void *key,
    const uint32_t key_len, uint32_t bind_flags) {
  struct nx_bind_req nbr;

  __nexus_bind_req_prepare(&nbr, nx_uuid, port, pid, exec_uuid, key, key_len,
                           bind_flags);

  return __nexus_set_opt(ncd->ncd_fd, NXOPT_NEXUS_BIND, &nbr, sizeof(nbr));
}

int os_nexus_controller_unbind_provider_instance(const nexus_controller_t ncd,
                                                 const uuid_t nx_uuid,
  ...
  return __nexus_set_opt(ncd->ncd_fd, NXOPT_NEXUS_UNBIND, &nbu, sizeof(nbu));
}

int os_nexus_controller_read_provider_attr(const nexus_controller_t ncd,
                                           const uuid_t prov_uuid,
                                           nexus_attr_t nxa) {
  ...
  ret = __nexus_get_opt(ncd->ncd_fd, NXOPT_NEXUS_PROV_ENTRY, &nre, &nre_len);
...
static int
add_traffic_rule_inet(const nexus_controller_t ncd, const char *ifname,
                      const struct ifnet_traffic_descriptor_inet *td,
                      const struct ifnet_traffic_rule_action_steer *ra,
                      const uint32_t flags, uuid_t *rule_uuid) {
...
static int
add_traffic_rule_eth(const nexus_controller_t ncd, const char *ifname,
...
int os_nexus_controller_add_traffic_rule(
    const nexus_controller_t ncd, const char *ifname,
  ...
  case IFNET_TRAFFIC_DESCRIPTOR_TYPE_INET: {
    if (td->itd_len != sizeof(struct ifnet_traffic_descriptor_inet)) {
      return EINVAL;
    }
    return add_traffic_rule_inet(
        ncd, ifname, (const struct ifnet_traffic_descriptor_inet *)td,
        (const struct ifnet_traffic_rule_action_steer *)ra, flags, rule_uuid);
  }
  case IFNET_TRAFFIC_DESCRIPTOR_TYPE_ETH: {
    if (td->itd_len != sizeof(struct ifnet_traffic_descriptor_eth)) {
      return EINVAL;
    }
    return add_traffic_rule_eth(
        ncd, ifname, (const struct ifnet_traffic_descriptor_eth *)td,
        (const struct ifnet_traffic_rule_action_steer *)ra, flags, rule_uuid);
  }
  default:
    return ENOTSUP;
...
int os_nexus_controller_remove_traffic_rule(const nexus_controller_t ncd,
...
static boolean_t rule_iterate(struct nxctl_traffic_rule_generic_iocinfo *ginfo,
...
static void inet_rule_iterate(void *buf, uint32_t count,
                              nexus_traffic_rule_iterator_t itr,
                              void *itr_arg) {
...
static void eth_rule_iterate(void *buf, uint32_t count,
...
struct traffic_rule_type {
  ...
  uint32_t tr_size;
  uint32_t tr_count;
  void (*tr_iterate)(void *, uint32_t, nexus_traffic_rule_iterator_t, void *);
};
#define NTRDEFAULTCOUNT 512
static struct traffic_rule_type traffic_rule_types[] = {
    {IFNET_TRAFFIC_DESCRIPTOR_TYPE_INET,
     sizeof(struct nxctl_traffic_rule_inet_iocinfo), NTRDEFAULTCOUNT,
     inet_rule_iterate},
    {IFNET_TRAFFIC_DESCRIPTOR_TYPE_ETH,
     sizeof(struct nxctl_traffic_rule_eth_iocinfo), NTRDEFAULTCOUNT,
     eth_rule_iterate},
};
#define NTRTYPES (sizeof(traffic_rule_types) / sizeof(struct traffic_rule_type))

int os_nexus_controller_iterate_traffic_rules(const nexus_controller_t ncd,
                                              nexus_traffic_rule_iterator_t itr,
                                              void *itr_arg) {
  ...
  for (i = 0; i < NTRTYPES; i++) {
    ...
    if (args.gtr_count > 0) {
      t->tr_iterate(args.gtr_buf, args.gtr_count, itr, itr_arg);
    }
    free(args.gtr_buf);
  }
  return 0;
...
void os_nexus_controller_destroy(nexus_controller_t ncd) {
  if (ncd->ncd_fd != -1) {
...
int __os_nexus_ifattach(const nexus_controller_t ncd, const uuid_t nx_uuid,
                        const char *ifname, const uuid_t netif_uuid,
  ...
  ret = __nexus_set_opt(ncd->ncd_fd, NXOPT_NEXUS_CONFIG, &ncr, sizeof(ncr));
...
int __os_nexus_ifdetach(const nexus_controller_t ncd, const uuid_t nx_uuid,
  ...
  return __nexus_set_opt(ncd->ncd_fd, NXOPT_NEXUS_CONFIG, &ncr, sizeof(ncr));
}

int __os_nexus_flow_add(const nexus_controller_t ncd, const uuid_t nx_uuid,
                        const struct nx_flow_req *nfr) {
  struct nx_cfg_req ncr;

  __nexus_config_req_prepare(&ncr, nx_uuid, NXCFG_CMD_FLOW_ADD, nfr,
                             sizeof(*nfr));

  return __nexus_set_opt(ncd->ncd_fd, NXOPT_NEXUS_CONFIG, &ncr, sizeof(ncr));
}

int __os_nexus_flow_del(const nexus_controller_t ncd, const uuid_t nx_uuid,
                        const struct nx_flow_req *nfr) {
  struct nx_cfg_req ncr;

  __nexus_config_req_prepare(&ncr, nx_uuid, NXCFG_CMD_FLOW_DEL, nfr,
                             sizeof(*nfr));

  return __nexus_set_opt(ncd->ncd_fd, NXOPT_NEXUS_CONFIG, &ncr, sizeof(ncr));
}

static int __os_nexus_config_flow(const uuid_t nx_uuid,
                                  struct nx_flow_req *nfr) {
  struct nx_cfg_req ncr;

  __nexus_config_req_prepare(&ncr, nx_uuid, NXCFG_CMD_FLOW_CONFIG, nfr,
                             sizeof(*nfr));
...
int os_nexus_flow_set_wake_from_sleep(const uuid_t nx_uuid,
...
int __os_nexus_get_llink_info(const nexus_controller_t ncd,
                              const uuid_t nx_uuid,
                              const struct nx_llink_info_req *nlir,
                              size_t len) {
  struct nx_cfg_req ncr;

  __nexus_config_req_prepare(&ncr, nx_uuid, NXCFG_CMD_GET_LLINK_INFO, nlir,
                             len);

  return __nexus_set_opt(ncd->ncd_fd, NXOPT_NEXUS_CONFIG, &ncr, sizeof(ncr));
}

int os_nexus_flow_set_connection_idle(const uuid_t nx_uuid,
                                      const uuid_t flow_uuid, bool enable) {
  struct nx_flow_req nfr = {0};
  memcpy(nfr.nfr_flow_uuid, flow_uuid, sizeof(uuid_t));
  nfr.nfr_flags =
      enable ? NXFLOWREQF_CONNECTION_IDLE : NXFLOWREQF_CONNECTION_REUSED;
