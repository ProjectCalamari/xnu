...
static const char *_simple_getenv(char *const *envp, const char *var) {
 ...
 */
static bool can_access(const char *path) {
  int saveerrno = errno;
 ...
 * changing errno.
 */
static bool evaluate_rules(const char *rules_file_path, const char *fname,
                           char **envs, size_t envs_capacity, char *env_storage,
                           size_t env_storage_capacity, cpu_type_t *type,
                           cpu_subtype_t *subtype, uint32_t *psa_options,
                           uint8_t *sec_flags) {
  int saveerrno = errno;
  int fd = -1;
  ...
  while (1) {
    ...
    size_t read_result =
        __read_nocancel(fd, read_buffer + bytes, sizeof(read_buffer) - bytes);
    ...
    while (bytes > 0) {
      ...
      *newline_pos = '\0';
      ...
      } else if (memcmp(line, "has_sec_transition:",
                        strlen("has_sec_transition:")) == 0) {
      ...
      } else if (memcmp(line, "sec_transition_inherit:",
                        strlen("sec_transition_inherit:")) == 0) {
...
_posix_spawn_with_filter(pid_t *pid, const char *fname, char *const *argp,
                         char *const *envp, struct _posix_spawn_args_desc *adp,
                         int *ret) {
  /*
...
#define MAX_ENV_STORAGE_SIZE 1024
  ...
  uint8_t sec_flags = 0;
  bool should_apply_rules =
      evaluate_rules(rules_file_path, fname, envs_to_add,
                     sizeof(envs_to_add) / sizeof(envs_to_add[0]), env_storage,
                     sizeof(env_storage), &cputype_binpref, &cpusubtype_binpref,
