...
int __commpage_gettimeofday_internal(struct timeval *tp, uint64_t *tbr_out) {
  ...
  COMM_PAGE_SLOT_TYPE(new_commpage_timeofday_data_t)
  ...
  do {
    ...
    /*
     * This barrier prevents the reordering of the second read of
     * gtod_TimeStamp_tick_p w.r.t the values read just after mach_absolute_time
     * is invoked.
     */
#if (__ARM_ARCH__ >= 7)
