 ...
 * check_init
 */
static fdOpenInfoRef check_init(const char *path, uint32_t flags) {
  fdOpenInfoRef info;
  int status;
 ...
 * check_free
 */
static void check_free(fdOpenInfoRef info) {
  if (info->pids != NULL) {
    free(info->pids);
  }
 ...
 *          1 if match
 */
static int check_file(fdOpenInfoRef info, struct vinfo_stat *sb) {
  if (sb->vst_dev == 0) {
    // if no info
    return 0;
  }
 ...
 *          1 if match
 */
static int check_process_vnodes(fdOpenInfoRef info, int pid) {
  int buf_used;
  int status;
 ...
 *          1 if match
 */
static int check_process_text(fdOpenInfoRef info, int pid) {
  int status;
  int buf_used;
  ...
  if (info->flags & PROC_LISTPIDSPATH_PATH_IS_VOLUME) {
    ...
    buf_used = proc_pidinfo(pid, PROC_PIDREGIONPATHINFO3,
                            info->match_stat.st_dev, &rwpi, sizeof(rwpi));
    if (buf_used <= 0) {
    ...
    while (1) { // for all memory regions
      ...
      buf_used =
          proc_pidinfo(pid, PROC_PIDREGIONPATHINFO2, a, &rwpi, sizeof(rwpi));
      if (buf_used <= 0) {
        if ((errno == ESRCH) || (errno == EINVAL)) {
          // if no more text information is available for this process.
          break;
 ...
 *          1 if match
 */
static int check_process_fds(fdOpenInfoRef info, int pid) {
  int buf_used;
  int i;
  int status;
  ...
  while (1) {
    ...
    buf_used =
        proc_pidinfo(pid, PROC_PIDLISTFDS, 0, info->fds, (int)info->fds_size);
    if (buf_used <= 0) {
      return -1;
    }
  ...
  for (i = 0; i < info->fds_count; i++) {
    ...
    case PROX_FDTYPE_VNODE: {
      ...
      buf_used = proc_pidfdinfo(pid, fdp->proc_fd, PROC_PIDFDVNODEINFO, &vi,
                                sizeof(vi));
      if (buf_used <= 0) {
        if (errno == ENOENT) {
          /*
 ...
 *          1 if match
 */
static int check_process_threads(fdOpenInfoRef info, int pid) {
  int buf_used;
  int status;
  ...
  if (tai.pbsd.pbi_flags & PROC_FLAG_THCWD) {
    ...
    while (1) {
      ...
      buf_used = proc_pidinfo(pid, PROC_PIDLISTTHREADS, 0, info->threads,
                              (int)info->thr_size);
      if (buf_used <= 0) {
        return -1;
      }
    ...
    for (i = 0; i < info->thr_count; i++) {
      ...
      buf_used =
          proc_pidinfo(pid, PROC_PIDTHREADPATHINFO, thr, &tpi, sizeof(tpi));
      if (buf_used <= 0) {
        if ((errno == ESRCH) || (errno == EINVAL)) {
 ...
 *          1 if match
 */
static int check_process_phase1(fdOpenInfoRef info, int pid) {
  int status;
 ...
 *          1 if match
 */
static int check_process_phase2(fdOpenInfoRef info, int pid) {
  int status;
