...
int __process_policy(int scope, int action, int policy, int policy_subtype,
...
int proc_listpids(uint32_t type, uint32_t typeinfo, void *buffer,
  ...
  if ((type >= PROC_ALL_PIDS) || (type <= PROC_PPID_ONLY)) {
    if ((retval = __proc_info(PROC_INFO_CALL_LISTPIDS, type, typeinfo,
                              (uint64_t)0, buffer, buffersize)) == -1) {
      return 0;
    }
  } else {
    errno = EINVAL;
...
int proc_pidinfo(int pid, int flavor, uint64_t arg, void *buffer,
  ...
  if ((retval = __proc_info(PROC_INFO_CALL_PIDINFO, pid, flavor, arg, buffer,
                            buffersize)) == -1) {
    return 0;
  }
...
int proc_pidoriginatorinfo(int flavor, void *buffer, int buffersize) {
  int retval;

  if ((retval = __proc_info(PROC_INFO_CALL_PIDORIGINATORINFO, getpid(), flavor,
...
int proc_listcoalitions(int flavor, int coaltype, void *buffer,
                        int buffersize) {
  int retval;

  if ((retval = __proc_info(PROC_INFO_CALL_LISTCOALITIONS, flavor, coaltype, 0,
                            buffer, buffersize)) == -1) {
    return 0;
  }
...
int proc_setthread_cpupercent(uint8_t percentage, uint32_t ms_refill) {
  ...
  /* Pack percentage and refill into a 32-bit number to match existing kernel
   * implementation */
  ...
  return proc_rlimit_control(-1, RLIMIT_THREAD_CPULIMITS,
                             (void *)(uintptr_t)arg);
}
...
int proc_pidfdinfo(int pid, int fd, int flavor, void *buffer, int buffersize) {
  ...
  if ((retval = __proc_info(PROC_INFO_CALL_PIDFDINFO, pid, flavor, (uint64_t)fd,
                            buffer, buffersize)) == -1) {
    return 0;
  }
...
int proc_pidfileportinfo(int pid, uint32_t fileport, int flavor, void *buffer,
  ...
  if ((retval = __proc_info(PROC_INFO_CALL_PIDFILEPORTINFO, pid, flavor,
                            (uint64_t)fileport, buffer, buffersize)) == -1) {
    return 0;
  }
  return retval;
}
...
int proc_piddynkqueueinfo(int pid, int flavor, kqueue_id_t kq_id, void *buffer,
  ...
  if ((ret = __proc_info(PROC_INFO_CALL_PIDDYNKQUEUEINFO, pid, flavor,
                         (uint64_t)kq_id, buffer, buffersize)) == -1) {
    return 0;
  }
...
int proc_udata_info(int pid, int flavor, void *buffer, int buffersize) {
  return __proc_info(PROC_INFO_CALL_UDATA_INFO, pid, flavor, 0, buffer,
                     buffersize);
}

/* only used by dyld which links with libsystem_kernel.a */
__private_extern__ int proc_set_dyld_all_image_info(void *buffer,
                                                    int buffersize) {
  return __proc_info(PROC_INFO_CALL_SET_DYLD_IMAGES, getpid(), 0, 0, buffer,
                     buffersize);
}
...
int proc_name(int pid, void *buffer, uint32_t buffersize) {
  ...
  retval = proc_pidinfo(pid, PROC_PIDTBSDINFO, (uint64_t)0, &pbsd,
...
int proc_regionfilename(int pid, uint64_t address, void *buffer,
  ...
  retval = proc_pidinfo(pid, PROC_PIDREGIONPATH, (uint64_t)address, &path,
...
int proc_kmsgbuf(void *buffer, uint32_t buffersize) {
  ...
  if ((retval = __proc_info(PROC_INFO_CALL_KERNMSGBUF, 0, 0, (uint64_t)0,
                            buffer, buffersize)) == -1) {
    return 0;
  }
  return retval;
...
int proc_pidpath(int pid, void *buffer, uint32_t buffersize) {
  ...
  retval = __proc_info(PROC_INFO_CALL_PIDINFO, pid, PROC_PIDPATHINFO,
...
int proc_pidpath_audittoken(audit_token_t *audittoken, void *buffer,
  ...
  retval = __proc_info_extended_id(
      PROC_INFO_CALL_PIDINFO, pid, PROC_PIDPATHINFO, PIF_COMPARE_IDVERSION,
      (uint64_t)idversion, (uint64_t)0, buffer, buffersize);
  if (retval != -1) {
    len = (int)strlen(buffer);
    return len;
  }
...
int proc_current_thread_schedinfo(void *buffer, size_t buffersize) {
  ...
  retval = __proc_info(PROC_INFO_CALL_PIDINFO, pid, PROC_PIDTHREADSCHEDINFO,
                       threadid, buffer, buffersize);
...
int proc_setpcontrol(const int control) {
  ...
  if ((retval = __proc_info(PROC_INFO_CALL_SETCONTROL, getpid(),
                            PROC_SELFSET_PCONTROL, (uint64_t)control, NULL,
                            0)) == -1) {
    return errno;
  }
...
__private_extern__ int proc_setthreadname(void *buffer, int buffersize) {
  ...
  retval =
      __proc_info(PROC_INFO_CALL_SETCONTROL, getpid(), PROC_SELFSET_THREADNAME,
                  (uint64_t)0, buffer, buffersize);
...
int proc_set_dirty(pid_t pid, bool dirty) {
  if (__proc_info(PROC_INFO_CALL_DIRTYCONTROL, pid, PROC_DIRTYCONTROL_SET,
                  dirty, NULL, 0) == -1) {
    return errno;
  }
...
int proc_get_dirty(pid_t pid, uint32_t *flags) {
  ...
  retval = __proc_info(PROC_INFO_CALL_DIRTYCONTROL, pid, PROC_DIRTYCONTROL_GET,
                       0, NULL, 0);
  if (retval == -1) {
    return errno;
  }
...
int proc_clear_dirty(pid_t pid, uint32_t flags) {
  if (__proc_info(PROC_INFO_CALL_DIRTYCONTROL, pid, PROC_DIRTYCONTROL_CLEAR,
                  flags, NULL, 0) == -1) {
    return errno;
  }
...
int proc_signal_delegate(audit_token_t instigator, audit_token_t target,
                         int sig) {
  struct proc_delegated_signal_info args = {
      .instigator = instigator,
      .target = target,
  };
  int retval = __proc_info(PROC_INFO_CALL_DELEGATE_SIGNAL, 0, sig, 0, &args,
                           sizeof(struct proc_delegated_signal_info));
  if (retval == -1) {
    return errno;
  }
...
int proc_terminate_delegate(audit_token_t instigator, audit_token_t target,
                            int *sig) {
  struct proc_delegated_signal_info args = {
      .instigator = instigator,
      .target = target,
  ...
  int retval = __proc_info(PROC_INFO_CALL_DELEGATE_TERMINATE, 0, 0, 0, &args,
                           sizeof(struct proc_delegated_signal_info));
  if (retval == -1) {
    return errno;
  }
...
int proc_signal_with_audittoken(audit_token_t *audittoken, int sig) {
  int retval = __proc_info(PROC_INFO_CALL_SIGNAL_AUDITTOKEN, 0, sig, 0,
                           audittoken, sizeof(audit_token_t));
  if (retval == -1) {
    return errno;
  }
...
int proc_terminate_with_audittoken(audit_token_t *audittoken, int *sig) {
  ...
  retval = __proc_info(PROC_INFO_CALL_TERMINATE_AUDITTOKEN, 0, 0, 0, audittoken,
 ...
 */
int proc_set_cpumon_params(pid_t pid, int percentage, int interval) {
  ...
  return __process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_SET,
                          PROC_POLICY_RESOURCE_USAGE, PROC_POLICY_RUSAGE_CPU,
                          (proc_policy_attribute_t *)&attr, pid, 0);
}

int proc_get_cpumon_params(pid_t pid, int *percentage, int *interval) {
  proc_policy_cpuusage_attr_t attr;
  int ret;

  ret = __process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_GET,
                         PROC_POLICY_RESOURCE_USAGE, PROC_POLICY_RUSAGE_CPU,
                         (proc_policy_attribute_t *)&attr, pid, 0);
...
int proc_resume_cpumon(pid_t pid) {
  return __process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_ENABLE,
                          PROC_POLICY_RESOURCE_USAGE, PROC_POLICY_RUSAGE_CPU,
                          NULL, pid, 0);
}
...
int proc_set_cpumon_params_fatal(pid_t pid, int percentage, int interval) {
  ...
  if ((ret = proc_rlimit_control(pid, RLIMIT_CPU_USAGE_MONITOR,
                                 (void *)(uintptr_t)CPUMON_MAKE_FATAL)) != 0) {
...
int proc_set_wakemon_params(pid_t pid, int rate_hz, int flags __unused) {
...
int proc_get_wakemon_params(pid_t pid, int *rate_hz, int *flags) {
  ...
  if ((error = proc_rlimit_control(pid, RLIMIT_WAKEUPS_MONITOR, &params)) !=
      0) {
    return error;
  }
...
int proc_list_uptrs(int pid, uint64_t *buf, uint32_t bufsz) {
  return __proc_info(PROC_INFO_CALL_PIDINFO, pid, PROC_PIDLISTUPTRS, 0, buf,
                     bufsz);
}
...
int proc_setcpu_percentage(pid_t pid, int action, int percentage) {
  ...
  attr.ppattr_cpu_percentage = percentage;
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY,
                       PROC_POLICY_RESOURCE_USAGE, PROC_POLICY_RUSAGE_CPU,
                       (proc_policy_attribute_t *)&attr, pid,
                       (uint64_t)0) != -1) {
    return 0;
  } else {
    return errno;
...
int proc_reset_footprint_interval(pid_t pid) {
  return proc_rlimit_control(pid, RLIMIT_FOOTPRINT_INTERVAL,
                             (void *)(uintptr_t)FOOTPRINT_INTERVAL_RESET);
}

int proc_clear_cpulimits(pid_t pid) {
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_RESTORE,
                       PROC_POLICY_RESOURCE_USAGE, PROC_POLICY_RUSAGE_CPU, NULL,
                       pid, (uint64_t)0) != -1) {
    return 0;
  } else {
    return errno;
...
int proc_setcpu_deadline(pid_t pid, int action, uint64_t deadline) {
  proc_policy_cpuusage_attr_t attr;
...
int proc_setcpu_percentage_withdeadline(pid_t pid, int action, int percentage,
                                        uint64_t deadline) {
  proc_policy_cpuusage_attr_t attr;
...
int proc_appstate(int pid, int *appstatep) {
  ...
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_GET,
                       PROC_POLICY_APP_LIFECYCLE, PROC_POLICY_APPLIFE_STATE,
                       (proc_policy_attribute_t *)&state, pid,
                       (uint64_t)0) != -1) {
    if (appstatep != NULL) {
      *appstatep = state;
    }
...
int proc_devstatusnotify(int devicestatus) {
  ...
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY,
                       PROC_POLICY_APP_LIFECYCLE, PROC_POLICY_APPLIFE_DEVSTATUS,
                       (proc_policy_attribute_t *)&state, getpid(),
                       (uint64_t)0) != -1) {
    return 0;
  } else {
    return errno;
...
int proc_pidbind(int pid, uint64_t threadid, int bind) {
  ...
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY,
                       PROC_POLICY_APP_LIFECYCLE, PROC_POLICY_APPLIFE_PIDBIND,
                       (proc_policy_attribute_t *)&state, passpid,
                       threadid) != -1) {
    return 0;
  } else {
    return errno;
...
int proc_can_use_foreground_hw(int pid, uint32_t *reason) {
  return __proc_info(PROC_INFO_CALL_CANUSEFGHW, pid, 0, 0, reason,
                     sizeof(*reason));
}
...
#if (TARGET_OS_IPHONE && !TARGET_OS_SIMULATOR)
  rval = __process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_ENABLE,
                          PROC_POLICY_APPTYPE, PROC_POLICY_IOS_DONATEIMP, NULL,
                          getpid(), (uint64_t)0);
#else  /* (TARGET_OS_IPHONE && !TARGET_OS_SIMULATOR) */
  rval = __process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_SET,
                          PROC_POLICY_BOOST, PROC_POLICY_IMP_DONATION, NULL,
                          getpid(), 0);
#endif /* (TARGET_OS_IPHONE && !TARGET_OS_SIMULATOR) */
 ...
 *
 * Returns EOVERFLOW if an attempt is made to take an extra assertion when not
 * boosted.
 *
...
int proc_importance_assertion_begin_with_msg(
    mach_msg_header_t *msg, __unused mach_msg_trailer_t *trailer,
...
#define LEGACYBOOSTED(m)                                                       \
  (((m)->msgh_bits & LEGACYBOOSTMASK) == MACH_MSGH_BITS_RAISEIMP)
  ...
  if (LEGACYBOOSTED(msg)) {
    ...
    rval = __process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_HOLD,
                            PROC_POLICY_BOOST, PROC_POLICY_IMP_IMPORTANT, NULL,
                            getpid(), 0);
    if (rval == 0) {
      *assertion_token = (uint64_t)&important_boost_assertion_token;
      return 0;
    } else if (errno == EOVERFLOW) {
      proc_importance_bad_assertion(
          "Attempted to take assertion while not boosted");
      return errno;
    } else {
      return errno;
    }
 ...
 */
int proc_importance_assertion_complete(uint64_t assertion_token) {
  int rval = 0;
  ...
  if (assertion_token == (uint64_t)&important_boost_assertion_token) {
    rval = __process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_DROP,
                            PROC_POLICY_BOOST, PROC_POLICY_IMP_IMPORTANT, NULL,
                            getpid(), 0);
    if (rval == 0) {
      return 0;
    ...
    proc_importance_bad_assertion(
        "Attempted to drop assertion with invalid token");
    return EIO;
  }
}
 ...
 */
int proc_denap_assertion_complete(uint64_t assertion_token) {
...
int proc_clear_vmpressure(pid_t pid) {
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_RESTORE,
                       ...
                       PROC_POLICY_RS_VIRTUALMEM, NULL, pid,
                       (uint64_t)0) != -1) {
    return 0;
  } else {
    return errno;
...
/* set the current process as one who can resume suspended processes due to low
 * virtual memory. Need to be root */
int proc_set_owner_vmpressure(void) {
  int retval;

  if ((retval = __proc_info(PROC_INFO_CALL_SETCONTROL, getpid(),
                            PROC_SELFSET_VMRSRCOWNER, (uint64_t)0, NULL, 0)) ==
      -1) {
    return errno;
  }
...
int proc_set_delayidlesleep(void) {
  ...
  if ((retval = __proc_info(PROC_INFO_CALL_SETCONTROL, getpid(),
                            PROC_SELFSET_DELAYIDLESLEEP, (uint64_t)1, NULL,
                            0)) == -1) {
    return errno;
  }
...
int proc_clear_delayidlesleep(void) {
  ...
  if ((retval = __proc_info(PROC_INFO_CALL_SETCONTROL, getpid(),
                            PROC_SELFSET_DELAYIDLESLEEP, (uint64_t)0, NULL,
                            0)) == -1) {
    return errno;
  }
...
/* disable the launch time backgroudn policy and restore the process to default
 * group */
int proc_disable_apptype(pid_t pid, int apptype) {
  ...
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_DISABLE,
                       PROC_POLICY_APPTYPE, apptype, NULL, pid,
                       (uint64_t)0) != -1) {
    return 0;
  } else {
    return errno;
...
int proc_enable_apptype(pid_t pid, int apptype) {
  ...
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_ENABLE,
                       PROC_POLICY_APPTYPE, apptype, NULL, pid,
                       (uint64_t)0) != -1) {
    return 0;
  } else {
...
int proc_suppress(__unused pid_t pid, __unused uint64_t *generation) {
  return 0;
}
...
int proc_set_no_smt(void) {
  if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY,
                       PROC_POLICY_NO_SMT, 0, NULL, getpid(),
                       (uint64_t)0) == -1) {
    return errno;
  }
  return 0;
...
int proc_setthread_no_smt(void) {
  extern uint64_t __thread_selfid(void);
  if (__process_policy(PROC_POLICY_SCOPE_THREAD, PROC_POLICY_ACTION_APPLY,
                       PROC_POLICY_NO_SMT, 0, NULL, 0,
                       __thread_selfid()) == -1) {
    return errno;
  }
  return 0;
...
int proc_set_csm(uint32_t flags) {
  ...
  if (flags & (PROC_CSM_TECS | PROC_CSM_ALL)) {
    if (__process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY,
                         PROC_POLICY_TECS, 0, NULL, getpid(),
                         (uint64_t)0) == -1) {
      return errno;
    }
  }
