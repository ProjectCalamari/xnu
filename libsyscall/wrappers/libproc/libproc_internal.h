...
int proc_setcpu_percentage(pid_t pid, int action, int percentage)
    __OSX_AVAILABLE_STARTING(__MAC_10_12_2, __IPHONE_5_0);
...
int proc_setthread_cpupercent(uint8_t percentage, uint32_t ms_refill)
    __OSX_AVAILABLE_STARTING(__MAC_10_10, __IPHONE_5_0);
...
/* set self as the one who is going to resume suspended processes due to low VM.
 * Need to be root */
...
#define PROC_POLICY_OSX_APPTYPE_DASHCLIENT                                     \
  2 /* rename to move away from widget */

/*
 * Resumes the backgrounded TAL or dashboard client. Only priv users can disable
 * TAL apps. Valid apptype are PROC_POLICY_OSX_APPTYPE_DASHCLIENT and
 * PROC_POLICY_OSX_APPTYPE_TAL. Returns 0 on success otherwise appropriate error
 * code.
 */
...
int proc_importance_assertion_begin_with_msg(mach_msg_header_t *msg,
    ...
    __OSX_AVAILABLE_BUT_DEPRECATED(__MAC_10_8, __MAC_10_10, __IPHONE_6_0,
                                   __IPHONE_8_0);
...
int proc_set_wakemon_params(pid_t pid, int rate_hz, int flags)
    __OSX_AVAILABLE_STARTING(__MAC_10_9, __IPHONE_7_0);
...
int proc_trace_log(pid_t pid, uint64_t uniqueid)
    __OSX_AVAILABLE_STARTING(__MAC_10_10, __IPHONE_8_0);
...
int proc_listcoalitions(int flavor, int coaltype, void *buffer, int buffersize)
    __OSX_AVAILABLE_STARTING(__MAC_10_11, __IPHONE_8_3);
