...
#if defined(__LP64__) &&                                                       \
    (defined(VARIANT_CANCELABLE) || defined(VARIANT_PRE1050))
...
#endif
int __pselect(int, fd_set *__restrict, fd_set *__restrict, fd_set *__restrict,
 ...
 * variants. The darwin extension variants (both cancelable & non-cancelable)
 * are mapped directly to the syscall stub.
 */
int select(int nfds, fd_set *__restrict readfds, fd_set *__restrict writefds,
   ...
   */
  if (intimeout && intimeout->tv_sec == 0 && intimeout->tv_usec > 0 &&
      intimeout->tv_usec < 10000) {
    tb.tv_sec = 0;
    tb.tv_usec = 10000;
    timeout = &tb;
  } else {
...
static int _pselect_emulated(int count, fd_set *__restrict rfds,
                             fd_set *__restrict wfds, fd_set *__restrict efds,
                             const struct timespec *__restrict timo,
...
int pselect(int nfds, fd_set *__restrict readfds, fd_set *__restrict writefds,
   ...
   * Legacy select behavior is minimum 10 msec when tv_usec is non-zero
   */
  if (intimeout && intimeout->tv_sec == 0 && intimeout->tv_nsec > 0 &&
      intimeout->tv_nsec < 10000000L) {
    tb.tv_sec = 0;
...
#else  /* !VARIANT_CANCELABLE && !VARIANT_PRE1050 */
  ret =
      __pselect_nocancel(nfds, readfds, writefds, exceptfds, timeout, sigmask);
#endif /* VARIANT_CANCELABLE || VARIANT_PRE1050 */

  if (ret == -1 && errno == ENOSYS) {
    ret =
        _pselect_emulated(nfds, readfds, writefds, exceptfds, timeout, sigmask);
  }
