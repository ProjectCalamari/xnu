...
static int __statfs_ext_default(const char *path, int fd, struct statfs *buf) {
  int ret = 0;
...
static int __statfs_ext_noblock(const char *path, int fd, struct statfs *buf) {
  int ret = 0;
  char *ptr;
  ...
  struct attrlist al = {
      ...
      .volattr = ATTR_VOL_INFO | ATTR_VOL_FSTYPE | ATTR_VOL_MOUNTPOINT |
                 ...
                 ATTR_VOL_MOUNTEXTFLAGS | ATTR_VOL_OWNER,
  };
  ...
  if (path) {
    ret = getattrlist(path, &al, attrbuf, sizeof(*attrbuf),
                      FSOPT_NOFOLLOW | FSOPT_RETURN_REALDEV);
  } else {
    ret =
        fgetattrlist(fd, &al, attrbuf, sizeof(*attrbuf), FSOPT_RETURN_REALDEV);
  }
  ...
  if (attrbuf->f_attrs.volattr & ATTR_VOL_FSTYPENAME) {
    ptr =
        (char *)&attrbuf->f_fstypename + attrbuf->f_fstypename.attr_dataoffset;
  ...
  if (attrbuf->f_attrs.volattr & ATTR_VOL_MOUNTEDDEVICE) {
    ptr = (char *)&attrbuf->f_mntfromname +
          attrbuf->f_mntfromname.attr_dataoffset;
...
static int __statfs_ext_impl(const char *path, int fd, struct statfs *buf,
                             int flags) {
  int ret = 0;
