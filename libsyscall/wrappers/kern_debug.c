...
int __debug_syscall_reject_config(uint64_t packed_selectors1,
                                  uint64_t packed_selectors2, uint64_t flags);
...
int debug_syscall_reject_config(const syscall_rejection_selector_t *selectors,
  ...
  _Static_assert(sizeof(syscall_rejection_selector_t) == 1,
                 "selector size is not 1 byte");
  ...
  if (len >
      (2 * 8 * sizeof(packed_selector_t)) / SYSCALL_REJECTION_SELECTOR_BITS) {
...
#define s_left_shift(x, n) ((n) < 0 ? ((x) >> -(n)) : ((x) << (n)))
  ...
  for (int i = 0; i < len; i++, shift += SYSCALL_REJECTION_SELECTOR_BITS) {
    ...
    if (shift < 8 * sizeof(packed_selector_t)) {
      packed_selectors[0] |=
          ((uint64_t)(selectors[i]) & SYSCALL_REJECTION_SELECTOR_MASK) << shift;
    }
    if (second_shift > -SYSCALL_REJECTION_SELECTOR_BITS) {
      packed_selectors[1] |= s_left_shift(
          (uint64_t)(selectors[i] & SYSCALL_REJECTION_SELECTOR_MASK),
          second_shift);
    }
  }

  int ret = __debug_syscall_reject_config(packed_selectors[0],
                                          packed_selectors[1], flags);
...
int debug_syscall_reject(const syscall_rejection_selector_t *selectors,
                         size_t len) {
  return debug_syscall_reject_config(selectors, len,
