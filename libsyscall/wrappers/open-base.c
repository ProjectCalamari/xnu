...
#else /* VARIANT_CANCELABLE */
int open$NOCANCEL(const char *path, int oflag, ...)
#endif
{
  int opened_fd = 0;
  mode_t mode = 0;
  ...
  /* check to see if the suffix of the path we opened matches one we are
   * shimming */
  if (!system_version_compat_check_path_suffix(path)) {
    return opened_fd;
  }

  /* at this point we call into the version compat open shim and return values
   * from there */
  return system_version_compat_open_shim(opened_fd, -1, path, oflag, mode,
                                         CLOSE_SYSCALL, OPEN_SYSCALL, NULL,
                                         FCNTL_SYSCALL);
#endif /* !SYSTEM_VERSION_COMPAT_ENABLED */
}
...
#else /* VARIANT_CANCELABLE */
int openat$NOCANCEL(int fd, const char *path, int oflag, ...)
#endif
{
  int opened_fd = 0;
  mode_t mode = 0;
  ...
  /* at this point we call into the version compat open shim and return values
   * from there */
  return system_version_compat_open_shim(opened_fd, fd, path, oflag, mode,
                                         CLOSE_SYSCALL, NULL, OPENAT_SYSCALL,
                                         FCNTL_SYSCALL);
#endif /* !SYSTEM_VERSION_COMPAT_ENABLED */
}
