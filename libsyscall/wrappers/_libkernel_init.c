...
extern int _system_version_compat_open_shim(
...
extern int (*system_version_compat_open_shim)(
    int opened_fd, int openat_fd, const char *orig_path, int oflag, mode_t mode,
    int (*close_syscall)(int), int (*open_syscall)(const char *, int, mode_t),
    int (*openat_syscall)(int, const char *, int, mode_t),
    int (*fcntl_syscall)(int, int, long));
...
int __sysctlbyname(const char *name, size_t namelen, void *oldp,
                   size_t *oldlenp, void *newp, size_t newlen);
#endif /* SYSTEM_VERSION_COMPAT_ENABLED */

#if POSIX_SPAWN_FILTERING_ENABLED
struct _posix_spawn_args_desc;
extern bool (*posix_spawn_with_filter)(pid_t *pid, const char *fname,
                                       char *const *argp, char *const *envp,
                                       struct _posix_spawn_args_desc *adp,
                                       int *ret);
extern bool _posix_spawn_with_filter(pid_t *pid, const char *fname,
                                     char *const *argp, char *const *envp,
                                     struct _posix_spawn_args_desc *adp,
                                     int *ret);
...
void *(*LIBKERNEL_FUNCTION_PTRAUTH(_dlsym))(void *, const char *)
    __attribute__((visibility("hidden")));

__attribute__((
    visibility("hidden"))) _libkernel_functions_t _libkernel_functions;

void __libkernel_init(_libkernel_functions_t fns,
  ...
  for (size_t i = 0; envp[i]; i++) {
    ...
    if (vm_kernel_page_shift != 12 &&
        strcmp(VM_KERNEL_PAGE_SHIFT_ENV, envp[i]) == 0) {
...
void __libkernel_init_late(_libkernel_late_init_config_t config) {
  if (config->version >= 1) {
    ...
    if (config->enable_system_version_compat) {
      ...
      system_version_compat_check_path_suffix =
       ...
       */
      int enable = 1;
      __sysctlbyname("kern.system_version_compat",
                     strlen("kern.system_version_compat"), NULL, NULL, &enable,
                     sizeof(enable));
#endif /* SYSTEM_VERSION_COMPAT_NEEDS_SYSCTL */
    }
#endif /* SYSTEM_VERSION_COMPAT_HAS_MODE_MACOSX */
#if SYSTEM_VERSION_COMPAT_HAS_MODE_IOS
    if (!config->enable_system_version_compat && (config->version >= 2) &&
        config->enable_ios_version_compat) {
       ...
       * don't need to inform the kernel that this app has the SystemVersion
       * shim enabled.
       */
    }
