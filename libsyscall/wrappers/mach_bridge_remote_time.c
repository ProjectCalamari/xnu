...
static uint64_t absolutetime_to_nanoseconds(uint64_t abs_time) {
  mach_timebase_info_data_t info;
  mach_timebase_info(&info);
  uint64_t time_in_ns =
      (uint64_t)(((double)info.numer / (double)info.denom) * abs_time);

  return time_in_ns;
}

uint64_t mach_bridge_remote_time(__unused uint64_t local_time) {
  uint64_t remote_time = 0;
  uint64_t local_time_ns = 0;
  uint64_t now = 0;
  struct bt_params params = {};

  COMM_PAGE_SLOT_TYPE(struct bt_params)
  commpage_bt_params_p = COMM_PAGE_SLOT(struct bt_params, REMOTETIME_PARAMS);
  ...
  do {
    ...
    params.rate = *rate_p;
    /*
     * This barrier prevents the second read of base_local_ts from being
     * reordered w.r.t the reads of other values in bt_params.
     */
    __asm__ volatile("dmb ishld" ::: "memory");
  } while (params.base_local_ts &&
           (params.base_local_ts != commpage_bt_params_p->base_local_ts));
