...
int __darwin_check_fd_set_overflow(int n, const void *fd_set,
  ...
  if (n >= __DARWIN_FD_SETSIZE) {
    if (pthread_current_stack_contains_np((const void *)fd_set,
                                          sizeof(struct fd_set))) {
      if (!unlimited_select) {
        os_fault_with_payload(OS_REASON_LIBSYSTEM,
                              OS_REASON_LIBSYSTEM_CODE_FAULT, &n, sizeof(n),
                              "FD_SET overflow", 0);
        return 0;
      } else {
        return 1;
...
int __terminate_with_payload(int pid, uint32_t reason_namespace,
...
static void abort_with_payload_wrapper_internal(
...
static void abort_with_payload_wrapper_internal(
  ...
  terminate_with_payload(getpid(), reason_namespace, reason_code, payload,
                         payload_size, reason_string,
...
void abort_with_reason(uint32_t reason_namespace, uint64_t reason_code,
                       const char *reason_string, uint64_t reason_flags) {
  abort_with_payload_wrapper_internal(reason_namespace, reason_code, 0, 0,
                                      reason_string, reason_flags);
}
...
void abort_with_payload(uint32_t reason_namespace, uint64_t reason_code,
                        void *payload, uint32_t payload_size,
                        const char *reason_string, uint64_t reason_flags) {
  abort_with_payload_wrapper_internal(reason_namespace, reason_code, payload,
                                      payload_size, reason_string,
                                      reason_flags);
}
