...
static int prepare_times_array_and_attrs(struct timespec times_in[2],
                                         ...
                                         size_t *times_out_size, int *flags) {
...
int futimens(int fd, const struct timespec _times_in[2]) {
  ...
  a.commonattr = prepare_times_array_and_attrs(times_in, times_out,
                                               &attrbuf_size, &flags_out);

  return fsetattrlist(fd, &a, &times_out, attrbuf_size, 0);
}

int utimensat(int fd, const char *path, const struct timespec _times_in[2],
              int flags) {
  struct timespec times_in[2];
  int flags_out = 0;
