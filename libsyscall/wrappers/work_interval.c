...
int work_interval_create(work_interval_t *interval_handle,
                         uint32_t create_flags) {
  int ret;
  ...
  ret = __work_interval_ctl(WORK_INTERVAL_OPERATION_CREATE2, 0, &create_params,
...
int work_interval_notify(work_interval_t interval_handle, uint64_t start,
                         ...
                         uint64_t next_start, uint32_t notify_flags) {
  int ret;
  uint64_t work_interval_id;
  struct work_interval_notification notification = {.start = start,
                                                    .finish = finish,
                                                    .deadline = deadline,
...
int work_interval_join_port(mach_port_t port) {
  ...
  return __work_interval_ctl(WORK_INTERVAL_OPERATION_JOIN, (uint64_t)port, NULL,
                             0);
}
...
int work_interval_copy_port(work_interval_t interval_handle,
                            mach_port_t *port) {
  if (port == NULL) {
    errno = EINVAL;
    return -1;
  ...
  kern_return_t kr =
      mach_port_mod_refs(mach_task_self(), wi_port, MACH_PORT_RIGHT_SEND, 1);
