...
exclaves_outbound_buffer_create(mach_port_t port, const char *buffer_name,
                                mach_vm_size_t size,
                                mach_port_t *out_outbound_buffer_port) {
...
kern_return_t exclaves_outbound_buffer_copyout(mach_port_t outbound_buffer_port,
  ...
  return EXCLAVES_CTL_TRAP(outbound_buffer_port, opf, (exclaves_id_t)offset1,
                           dst_buffer, size1, size2, offset2, 0);
}

kern_return_t
exclaves_inbound_buffer_create(mach_port_t port, const char *buffer_name,
                               mach_vm_size_t size,
                               mach_port_t *out_inbound_buffer_port) {
  const uint32_t opf = EXCLAVES_CTL_OP_AND_FLAGS(NAMED_BUFFER_CREATE, 0);
  return EXCLAVES_CTL_TRAP(port, opf, buffer_name,
...
kern_return_t exclaves_inbound_buffer_copyin(mach_port_t inbound_buffer_port,
  ...
  return EXCLAVES_CTL_TRAP(inbound_buffer_port, opf, (exclaves_id_t)offset1,
                           src_buffer, size1, size2, offset2, 0);
}

kern_return_t exclaves_named_buffer_create(mach_port_t port,
                                           exclaves_id_t buffer_id,
                                           mach_vm_size_t size,
                                           mach_port_t *out_named_buffer_port) {
  (void)port;
  (void)buffer_id;
  (void)size;
  (void)out_named_buffer_port;
  return KERN_NOT_SUPPORTED;
}
...
kern_return_t exclaves_lookup_service(mach_port_t port, const char *name,
  ...
  kr = EXCLAVES_CTL_TRAP(
      port, opf, 0, (mach_vm_address_t)&conclave_resource_user, size, 0, 0, 0);
...
kern_return_t exclaves_boot(mach_port_t port, exclaves_boot_stage_t stage) {
...
kern_return_t exclaves_audio_buffer_copyout(mach_port_t audio_buffer_port,
  ...
  return EXCLAVES_CTL_TRAP(audio_buffer_port, opf, (exclaves_id_t)offset1,
                           dst_buffer, size1, size2, offset2, 0);
}
...
kern_return_t exclaves_audio_buffer_copyout_with_status(
  ...
  const uint32_t opf = EXCLAVES_CTL_OP_AND_FLAGS(AUDIO_BUFFER_COPYOUT, 0);
  return EXCLAVES_CTL_TRAP(audio_buffer_port, opf, (exclaves_id_t)offset1,
                           dst_buffer, size1, size2, offset2, status);
}
...
kern_return_t exclaves_sensor_start(mach_port_t sensor_port, uint64_t flags,
  ...
  return EXCLAVES_CTL_TRAP(sensor_port, opf, flags, (uintptr_t)sensor_status, 0,
                           0, 0, 0);
}
...
kern_return_t exclaves_indicator_min_on_time(mach_port_t port, uint64_t flags,
  ...
  if ((camera_indicator == NULL) || (mic_indicator == NULL) ||
      (faceid_indicator == NULL)) {
    return KERN_INVALID_ARGUMENT;
  }

  struct exclaves_indicator_deadlines indicator = {.version = 1};

  const uint32_t opf = EXCLAVES_CTL_OP_AND_FLAGS(SENSOR_MIN_ON_TIME, 0);
  kern_return_t kr =
      EXCLAVES_CTL_TRAP(port, opf, flags, (mach_vm_address_t)&indicator,
                        sizeof(indicator), 0, 0, 0);
  if (kr == KERN_SUCCESS) {
...
kern_return_t exclaves_notification_create(__unused mach_port_t port,
                                           const char *name,
                                           uint64_t *notification_id) {
  const uint32_t opf =
      EXCLAVES_CTL_OP_AND_FLAGS(NOTIFICATION_RESOURCE_LOOKUP, 0);
  kern_return_t kr;
...
kern_return_t exclaves_aoe_setup(__unused mach_port_t port,
                                 uint8_t *num_message, uint8_t *num_worker) {
  /* Hack for now. */
...
kern_return_t exclaves_aoe_work_loop(__unused mach_port_t port) {
  const uint32_t opf = EXCLAVES_CTL_OP_AND_FLAGS(AOE_WORK_LOOP, 0);
  return EXCLAVES_CTL_TRAP(port, opf, 0, 0, 0, 0, 0, 0);
}

kern_return_t exclaves_aoe_message_loop(__unused mach_port_t port) {
