...
#if defined(__SYSCALL_32BIT_ARG_BYTES) &&                                      \
    ((__SYSCALL_32BIT_ARG_BYTES >= 4) && (__SYSCALL_32BIT_ARG_BYTES <= 20))
#define UNIX_SYSCALL_NONAME(name, nargs, cerror)                               \
  movl $(SYS_##name |                                                          \
         (__SYSCALL_32BIT_ARG_BYTES << I386_SYSCALL_ARG_BYTES_SHIFT)),         \
      % eax;                                                                   \
  UNIX_SYSCALL_SYSENTER;                                                       \
  jnb 2f;                                                                      \
...
#define SYSCALL_8(name)                                                        \
  SYSCALL_6to12(name, r4 - r6 COMMA r8, r4 - r6) /* 8th on stack */
#define SYSCALL_12(name)                                                       \
  SYSCALL_6to12(name, r4 - r6 COMMA r8, r4 - r6) /* 8th-12th on stack */
...
#define SYSCALL_NONAME_4(name, cerror)                                         \
  SYSCALL_NONAME_6to12(name, r4 - r5, r4 - r5, cerror)
#undef SYSCALL_NONAME_5
#define SYSCALL_NONAME_5(name, cerror)                                         \
  SYSCALL_NONAME_6to12(name, r4 - r5, r4 - r5, cerror)
#define SYSCALL_NONAME_6(name, cerror)                                         \
  SYSCALL_NONAME_6to12(name, r4 - r6 COMMA r8, r4 - r6 COMMA r8, cerror)
#define SYSCALL_NONAME_7(name, cerror)                                         \
  SYSCALL_NONAME_6to12(name, r4 - r6 COMMA r8, r4 - r6 COMMA r8, cerror)
#define SYSCALL_NONAME_8(name, cerror)                                         \
  SYSCALL_NONAME_6to12(name, r4 - r6 COMMA r8, r4 - r6 COMMA r8, cerror)
#define SYSCALL_NONAME_12(name, cerror)                                        \
  SYSCALL_NONAME_6to12(name, r4 - r6 COMMA r8, r4 - r6 COMMA r8, cerror)
...
#define SYSCALL_NONAME_6(name, cerror)                                         \
  SYSCALL_NONAME_6to12(name, r4 - r5, r4 - r5, cerror)
#define SYSCALL_NONAME_7(name, cerror)                                         \
  SYSCALL_NONAME_6to12(name, r4 - r6 COMMA r8, r4 - r6, cerror)
#define SYSCALL_NONAME_8(name, cerror)                                         \
  SYSCALL_NONAME_6to12(name, r4 - r6 COMMA r8, r4 - r6, cerror)
#define SYSCALL_NONAME_12(name, cerror)                                        \
  SYSCALL_NONAME_6to12(name, r4 - r6 COMMA r8, r4 - r6, cerror)
...
#define DO_SYSCALL(num, cerror)                                                \
  mov x16, #(num) % % svc #SWI_SYSCALL % % b.cc 2f % % ARM64_STACK_PROLOG % %  \
               PUSH_FRAME % % bl _##cerror % % POP_FRAME % %                   \
               ARM64_STACK_EPILOG % % 2:

#define MI_GET_ADDRESS(reg, var)                                               \
  adrp reg, var @page % % add reg, reg, var @pageoff % %

#define MI_CALL_EXTERNAL(sym) .globl sym % % bl sym

#define SYSCALL_NONAME(name, nargs, cerror)                                    \
  DO_SYSCALL(SYS_##name, cerror) % % 1:

#define MI_ENTRY_POINT(name) .text % %.align 2 % %.globl name % % name:

#define PSEUDO(pseudo, name, nargs, cerror)                                    \
  .text % %.align 2 % %.globl pseudo % %                                       \
      pseudo : % % SYSCALL_NONAME(name, nargs, cerror)

#define __SYSCALL(pseudo, name, nargs)                                         \
  PSEUDO(pseudo, name, nargs, cerror) % % ret
