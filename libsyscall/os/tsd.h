...
/*
 * The implementation details of this function are not ABI and are subject to
 * change, do not copy them in another project
 */
__attribute__((always_inline)) static __inline__ unsigned int
_os_cpu_number(void) {
#if defined(__arm__)
  uintptr_t p;
...
__attribute__((always_inline,
               const)) static __inline__ void **_os_tsd_get_base(void) {
#if defined(__arm__)
  uintptr_t tsd;
...
__attribute__((always_inline,
               const)) static __inline__ uintptr_t _os_ptr_munge_token(void) {
  return (uintptr_t)_os_tsd_get_direct(__TSD_PTR_MUNGE);
}

__attribute__((always_inline, const)) static __inline__ uintptr_t
...
#define _OS_TSD_OFFSET(_key) ((__POINTER_WIDTH__ / __CHAR_BIT__) * _key)

#if defined(__i386__) || defined(__x86_64__)

#define _OS_PTR_MUNGE(_reg) xor% gs : _OS_TSD_OFFSET(__TSD_PTR_MUNGE), _reg

#define _OS_PTR_UNMUNGE(_reg) _OS_PTR_MUNGE(_reg)
...
#define _OS_PTR_MUNGE_TOKEN(_reg, _token)                                      \
  mrs _reg, TPIDRRO_EL0 % % ldr _token,                                        \
      [ _reg, #_OS_TSD_OFFSET(__TSD_PTR_MUNGE) ]

#endif // defined(__arm64__)

#define _OS_PTR_MUNGE(_regdest, _regsrc, _token) eor _regdest, _regsrc, _token
