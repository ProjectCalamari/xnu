...
kern_return_t thread_policy(thread_act_t thr_act, policy_t policy,
...
kern_return_t thread_policy_set(thread_act_t thread,
  ...
  kr =
      _kernelrpc_thread_policy_set(thread, flavor, policy_info, policy_infoCnt);
  if (kr == KERN_SUCCESS) {
...
kern_return_t thread_set_policy(thread_act_t thr_act, processor_set_t pset,
  ...
  kr = _kernelrpc_thread_set_policy(thr_act, pset, policy, base, baseCnt, limit,
                                    limitCnt);
  if (kr == KERN_SUCCESS) {
    _pthread_clear_qos_tsd(thr_act);
  } else if (kr == KERN_POLICY_STATIC) {
