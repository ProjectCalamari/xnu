...
kern_return_t mach_port_names(ipc_space_t task, mach_port_name_array_t *names,
                              mach_msg_type_number_t *namesCnt,
                              mach_port_type_array_t *types,
                              mach_msg_type_number_t *typesCnt) {
  kern_return_t rv;

  rv = _kernelrpc_mach_port_names(task, names, namesCnt, types, typesCnt);

  return rv;
}

kern_return_t mach_port_type(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_rename(ipc_space_t task, mach_port_name_t old_name,
...
kern_return_t mach_port_allocate_name(ipc_space_t task, mach_port_right_t right,
...
kern_return_t mach_port_get_refs(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_mod_refs(ipc_space_t task, mach_port_name_t name,
                                 mach_port_right_t right,
...
kern_return_t mach_port_peek(ipc_space_t task, mach_port_name_t name,
  ...
  rv = _kernelrpc_mach_port_peek(task, name, trailer_type, seqnop, msg_sizep,
                                 msg_idp, trailer_infop, trailer_sizep);

  return rv;
}

kern_return_t mach_port_set_mscount(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_get_set_status(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_move_member(ipc_space_t task, mach_port_name_t member,
...
kern_return_t mach_port_request_notification(
  ...
  rv = _kernelrpc_mach_port_request_notification_trap(
      task, name, msgid, sync, notify, notifyPoly, previous);

  if (rv == MACH_SEND_INVALID_DEST) {
    rv = _kernelrpc_mach_port_request_notification(
        task, name, msgid, sync, notify, notifyPoly, previous);
  }
...
kern_return_t mach_port_insert_right(ipc_space_t task, mach_port_name_t name,
                                     mach_port_t poly,
  ...
  if (rv == MACH_SEND_INVALID_DEST) {
    rv = _kernelrpc_mach_port_insert_right(task, name, poly, polyPoly);
  }

  return rv;
}

kern_return_t mach_port_extract_right(ipc_space_t task, mach_port_name_t name,
  ...
  rv =
      _kernelrpc_mach_port_extract_right(task, name, msgt_name, poly, polyPoly);

  return rv;
}

kern_return_t mach_port_set_seqno(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_get_attributes(
    ipc_space_t task, mach_port_name_t name, mach_port_flavor_t flavor,
    mach_port_info_t port_info_out, mach_msg_type_number_t *port_info_outCnt) {
  kern_return_t rv;

  rv = _kernelrpc_mach_port_get_attributes_trap(
  ...
  if (rv == MACH_SEND_INVALID_DEST) {
    rv = _kernelrpc_mach_port_get_attributes(task, name, flavor, port_info_out,
                                             port_info_outCnt);
  }
...
kern_return_t mach_port_set_attributes(ipc_space_t task, mach_port_name_t name,
                                       mach_port_flavor_t flavor,
                                       mach_port_info_t port_info,
                                       mach_msg_type_number_t port_infoCnt) {
  kern_return_t rv;

  rv = _kernelrpc_mach_port_set_attributes(task, name, flavor, port_info,
                                           port_infoCnt);

  return rv;
}

kern_return_t mach_port_allocate_qos(ipc_space_t task, mach_port_right_t right,
                                     mach_port_qos_t *qos,
                                     mach_port_name_t *name) {
  kern_return_t rv;
...
kern_return_t mach_port_allocate_full(ipc_space_t task, mach_port_right_t right,
                                      mach_port_t proto, mach_port_qos_t *qos,
                                      mach_port_name_t *name) {
  kern_return_t rv;
...
kern_return_t mach_port_get_srights(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_space_info(ipc_space_t task,
...
kern_return_t
mach_port_space_basic_info(ipc_space_t task,
...
static inline mach_port_t _tsd_get_special_reply_port(void) {
  return (mach_port_t)(uintptr_t)_os_tsd_get_direct(__TSD_MACH_SPECIAL_REPLY);
}

static inline void _tsd_set_special_reply_port(mach_port_t port) {
...
void mig_dealloc_special_reply_port(mach_port_t migport) {
...
mach_sync_ipc_link_monitoring_start(mach_port_t *special_reply_port) {
  ...
  *special_reply_port = MACH_PORT_DEAD;
  ...
  kr = _kernelrpc_mach_port_special_reply_port_reset_link(mach_task_self(), srp,
                                                          &link_broken);
  if (kr != KERN_SUCCESS) {
...
kern_return_t mach_sync_ipc_link_monitoring_stop(mach_port_t srp,
                                                 boolean_t *in_effect) {
  kern_return_t kr;
...
kern_return_t mach_port_dnrequest_info(ipc_space_t task, mach_port_name_t name,
                                       unsigned *dnr_total,
                                       unsigned *dnr_used) {
  kern_return_t rv;

  rv = _kernelrpc_mach_port_dnrequest_info(task, name, dnr_total, dnr_used);

  return rv;
}

kern_return_t mach_port_kernel_object(ipc_space_t task, mach_port_name_t name,
                                      unsigned *object_type,
                                      unsigned *object_addr) {
  kern_return_t rv;
  mach_vm_address_t addr;

  rv = _kernelrpc_mach_port_kobject(task, name, object_type, &addr);
...
kern_return_t mach_port_insert_member(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_extract_member(ipc_space_t task, mach_port_name_t name,
                                       mach_port_name_t pset) {
  kern_return_t rv;
...
kern_return_t mach_port_get_context(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_set_context(ipc_space_t task, mach_port_name_t name,
                                    mach_port_context_t context) {
  kern_return_t rv;
...
kern_return_t mach_port_kobject(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_kobject_description(ipc_space_t task,
                                            mach_port_name_t name,
                                            natural_t *object_type,
                                            mach_vm_address_t *object_addr,
  ...
  rv = _kernelrpc_mach_port_kobject_description(task, name, object_type,
                                                object_addr, desc);

  return rv;
}

kern_return_t mach_port_construct(ipc_space_t task,
                                  mach_port_options_t *options,
                                  mach_port_context_t context,
                                  mach_port_name_t *name) {
  kern_return_t rv;

  rv = _kernelrpc_mach_port_construct_trap(task, options, (uint64_t)context,
                                           name);
...
kern_return_t mach_port_destruct(ipc_space_t task, mach_port_name_t name,
...
kern_return_t mach_port_guard(ipc_space_t task, mach_port_name_t name,
                              mach_port_context_t guard, boolean_t strict) {
  kern_return_t rv;
...
kern_return_t mach_port_unguard(ipc_space_t task, mach_port_name_t name,
                                mach_port_context_t guard) {
  kern_return_t rv;
...
extern kern_return_t _kernelrpc_mach_voucher_extract_attr_recipe(
    mach_port_name_t voucher, mach_voucher_attr_key_t key,
    mach_voucher_attr_raw_recipe_t recipe, mach_msg_type_number_t *recipe_size);

kern_return_t
mach_voucher_extract_attr_recipe(mach_port_name_t voucher,
  ...
  if (rv == MACH_SEND_INVALID_DEST) {
    rv = _kernelrpc_mach_voucher_extract_attr_recipe(voucher, key, recipe,
                                                     recipe_size);
  }
...
kern_return_t thread_destruct_special_reply_port(
...
kern_return_t mach_port_guard_with_flags(ipc_space_t task,
                                         mach_port_name_t name,
                                         mach_port_context_t guard,
                                         uint64_t flags) {
  kern_return_t rv;
  ...
  rv =
      _kernelrpc_mach_port_guard_with_flags(task, name, (uint64_t)guard, flags);

  return rv;
}

kern_return_t mach_port_swap_guard(ipc_space_t task, mach_port_name_t name,
                                   mach_port_context_t old_guard,
                                   mach_port_context_t new_guard) {
  kern_return_t rv;

  rv = _kernelrpc_mach_port_swap_guard(task, name, (uint64_t)old_guard,
                                       (uint64_t)new_guard);
...
kern_return_t mach_port_is_connection_for_service(
    ipc_space_t task, mach_port_name_t connection_port_name,
    mach_port_name_t service_port_name, uint64_t *filter_policy_id) {
  kern_return_t rv;

  rv = _kernelrpc_mach_port_is_connection_for_service(
      task, connection_port_name, service_port_name, filter_policy_id);

  return rv;
}

kern_return_t
mach_port_get_service_port_info(ipc_space_read_t task, mach_port_name_t name,
...
kern_return_t mach_port_assert_attributes(ipc_space_t task,
                                          mach_port_name_t name,
                                          mach_port_flavor_t flavor,
  ...
  rv =
      _kernelrpc_mach_port_assert_attributes(task, name, flavor, info, infoCnt);
