...
extern mach_msg_size_t voucher_mach_msg_fill_aux(mach_msg_aux_header_t *aux_hdr,
                                                 mach_msg_size_t sz);
...
mach_msg_return_t mach_msg2_internal(void *data, mach_msg_option64_t option64,
  ...
  if ((option64 & MACH64_RCV_INTERRUPT) == 0) {
    while (mr == MACH_RCV_INTERRUPTED) {
      mr = mach_msg2_trap(
          data, mach_msg_options_after_interruption(option64),
...
mach_msg_return_t mach_msg(mach_msg_header_t *msg, mach_msg_option_t option,
                           ...
                           mach_port_t notify) {
  return mach_msg_overwrite(msg, option, send_size, rcv_size, rcv_name, timeout,
                            notify, NULL, 0);
}
...
mach_msg_overwrite(mach_msg_header_t *msg, mach_msg_option_t option,
  ...
  /*
   * voucher_mach_msg_fill_aux_supported() is FALSE if libsyscall is linked
   * against an old libdispatch (e.g. old Simulator on new system), or if we are
   * building Libsyscall_static due to weak linking (e.g. dyld).
   *
   * Hoist this check to guard the malloc().
   *
   * See: _libc_weak_funcptr.c
   */
  if (voucher_mach_msg_fill_aux_supported() && (option64 & MACH64_RCV_MSG) &&
  ...
  if ((option64 & MACH64_SEND_MSG) &&
      ...
      ((aux_sz = voucher_mach_msg_fill_aux(
            aux, LIBSYSCALL_MSGV_AUX_MAX_SIZE)) != 0)) {
    option64 |= MACH64_MSG_VECTOR;
  }
  ...
  if (option64 & MACH64_MSG_VECTOR) {
    mr = mach_msg2(vecs, option64, *msg, 2, 2, rcv_name, timeout, priority);
  } else {
    mr = mach_msg2(msg, option64, *msg, send_size, rcv_limit, rcv_name, timeout,
                   priority);
  }
...
mach_msg_return_t mach_msg_send(mach_msg_header_t *msg) {
  return mach_msg(msg, MACH_SEND_MSG, msg->msgh_size, 0, MACH_PORT_NULL,
...
static void mach_msg_destroy_port(mach_port_t port, mach_msg_type_name_t type) {
  if (MACH_PORT_VALID(port)) {
    ...
    case MACH_MSG_TYPE_MOVE_RECEIVE:
      ...
      (void)mach_port_mod_refs(mach_task_self_, port, MACH_PORT_RIGHT_RECEIVE,
                               -1);
      break;

    case MACH_MSG_TYPE_MAKE_SEND:
      /* create a send right and then destroy it */
      (void)mach_port_insert_right(mach_task_self_, port, port,
    ...
    case MACH_MSG_TYPE_MAKE_SEND_ONCE:
      ...
      (void)mach_port_extract_right(mach_task_self_, port,
                                    MACH_MSG_TYPE_MAKE_SEND_ONCE, &port, &type);
      (void)mach_port_deallocate(mach_task_self_, port);
      break;
    }
  }
  ...
  if (mbits & MACH_MSGH_BITS_COMPLEX) {
    ...
    for (i = 0; i < count; i++) {
      ...
      case MACH_MSG_OOL_DESCRIPTOR: {
        ...
        if (dsc->deallocate) {
          mach_msg_destroy_memory((vm_offset_t)dsc->address, dsc->size);
        }
        daddr = (mach_msg_descriptor_t *)(dsc + 1);
        break;
      }
...
static kern_return_t mach_msg_server_mig_return_code(mig_reply_error_t *reply) {
  /*
...
static void mach_msg_server_consume_unsent_message(mach_msg_header_t *hdr) {
 ...
 *		calls will not be).
 */
mach_msg_return_t mach_msg_server_once(boolean_t (*demux)(mach_msg_header_t *,
                                                          mach_msg_header_t *),
  ...
  request_size =
      (options & MACH_RCV_LARGE) ? request_alloc : max_size + trailer_alloc;

  reply_alloc = (mach_msg_size_t)round_page(
      (options & MACH_SEND_TRAILER) ? (max_size + MAX_TRAILER_SIZE) : max_size);

  kr = vm_allocate(self, (vm_address_t *)&bufReply, reply_alloc,
  ...
  for (;;) {
    ...
    kr = vm_allocate(self, (vm_address_t *)&bufRequest, request_alloc,
                     VM_MAKE_TAG(VM_MEMORY_MACH_MSG) | TRUE);
    if (kr != KERN_SUCCESS) {
      vm_deallocate(self, (vm_address_t)bufReply, reply_alloc);
      return kr;
    }

    mr = mach_msg(&bufRequest->Head, MACH_RCV_MSG | MACH_RCV_VOUCHER | options,
                  0, request_size, rcv_name, MACH_MSG_TIMEOUT_NONE,
                  MACH_PORT_NULL);
    ...
    new_request_alloc =
        (mach_msg_size_t)round_page(bufRequest->Head.msgh_size + trailer_alloc);
    vm_deallocate(self, (vm_address_t)bufRequest, request_alloc);
  ...
  (void)vm_deallocate(self, (vm_address_t)bufRequest, request_alloc);
  (void)vm_deallocate(self, (vm_address_t)bufReply, reply_alloc);
  return mr;
}
 ...
 */
mach_msg_return_t
mach_msg_server(boolean_t (*demux)(mach_msg_header_t *, mach_msg_header_t *),
  ...
  request_size =
      (options & MACH_RCV_LARGE) ? new_request_alloc : max_size + trailer_alloc;
  ...
  for (;;) {
    ...
    while (mr == MACH_MSG_SUCCESS) {
                        ...
                        (MACH_MSGH_BITS_REMOTE(bufReply->Head.msgh_bits) ==
                         MACH_MSG_TYPE_MOVE_SEND_ONCE)
                            ? MACH_SEND_MSG | MACH_RCV_MSG | MACH_RCV_TIMEOUT |
                                  MACH_RCV_VOUCHER | options
                            : MACH_SEND_MSG | MACH_RCV_MSG | MACH_SEND_TIMEOUT |
                                  MACH_RCV_TIMEOUT | MACH_RCV_VOUCHER | options,
              ...
              (MACH_MSGH_BITS_REMOTE(bufReply->Head.msgh_bits) ==
               MACH_MSG_TYPE_MOVE_SEND_ONCE)
                  ...
                  : MACH_SEND_MSG | MACH_RCV_MSG | MACH_SEND_TIMEOUT |
                        MACH_RCV_TIMEOUT | MACH_RCV_VOUCHER | options,
              bufReply->Head.msgh_size, request_size, rcv_name,
              MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL, &bufRequest->Head, 0);
        }
      ...
      mr = mach_msg(&bufRequest->Head,
                    MACH_RCV_MSG | MACH_RCV_VOUCHER | options, 0, request_size,
                    rcv_name, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
    } /* while (mr == MACH_MSG_SUCCESS) */

    if ((mr == MACH_RCV_TOO_LARGE) && (options & MACH_RCV_LARGE)) {
      new_request_alloc = (mach_msg_size_t)round_page(
          bufRequest->Head.msgh_size + trailer_alloc);
 ...
 */
mach_msg_return_t mach_msg_server_importance(
