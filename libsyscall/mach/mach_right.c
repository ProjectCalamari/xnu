...
#pragma mark API
mach_right_recv_t mach_right_recv_construct(mach_right_flags_t flags,
                                            mach_right_send_t *_Nullable sr,
                                            uintptr_t ctx) {
...
void mach_right_recv_destruct(mach_right_recv_t r, mach_right_send_t *s,
                              uintptr_t ctx) {
  kern_return_t kr = KERN_FAILURE;
...
mach_right_send_t mach_right_send_create(mach_right_recv_t r) {
  kern_return_t kr = KERN_FAILURE;
...
mach_right_send_t mach_right_send_retain(mach_right_send_t s) {
  kern_return_t kr = KERN_FAILURE;
  mach_right_send_t rs = MACH_RIGHT_SEND_NULL;

  kr =
      mach_port_mod_refs(mach_task_self(), s.mrs_name, MACH_PORT_RIGHT_SEND, 1);
  switch (kr) {
  case 0:
    rs = s;
    break;
...
void mach_right_send_release(mach_right_send_t s) {
  ...
  kr = mach_port_mod_refs(mach_task_self(), s.mrs_name, MACH_PORT_RIGHT_SEND,
                          -1);
  switch (kr) {
  case 0:
    break;
...
mach_right_send_once_t mach_right_send_once_create(mach_right_recv_t r) {
  mach_msg_type_name_t right = 0;
  mach_port_t so = MACH_PORT_NULL;
  kern_return_t kr = mach_port_extract_right(
      mach_task_self(), r.mrr_name, MACH_MSG_TYPE_MAKE_SEND_ONCE, &so, &right);
