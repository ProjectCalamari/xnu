...
malloc_logger_t *__syscall_logger = NULL; // This may get set by Libc's malloc
                                          // stack logging initialization code.

kern_return_t mach_vm_allocate(mach_port_name_t target,
  ...
  if (__syscall_logger && rv == KERN_SUCCESS &&
      (userTagFlags != VM_MAKE_TAG(VM_MEMORY_STACK))) {
    __syscall_logger(stack_logging_type_vm_allocate | userTagFlags,
                     (uintptr_t)target, (uintptr_t)size, 0, (uintptr_t)*address,
                     0);
  }
...
kern_return_t mach_vm_deallocate(mach_port_name_t target,
                                 mach_vm_address_t address,
                                 mach_vm_size_t size) {
  kern_return_t rv;

  if (__syscall_logger) {
    __syscall_logger(stack_logging_type_vm_deallocate, (uintptr_t)target,
                     (uintptr_t)address, (uintptr_t)size, 0, 0);
  }
...
kern_return_t mach_vm_protect(mach_port_name_t task, mach_vm_address_t address,
  ...
  if (rv == MACH_SEND_INVALID_DEST) {
    rv = _kernelrpc_mach_vm_protect(task, address, size, set_maximum,
                                    new_protection);
  }
...
kern_return_t vm_allocate(mach_port_name_t task, vm_address_t *address,
...
kern_return_t vm_deallocate(mach_port_name_t task, vm_address_t address,
                            vm_size_t size) {
  kern_return_t rv;
...
kern_return_t vm_protect(mach_port_name_t task, vm_address_t address,
...
kern_return_t mach_vm_map(mach_port_name_t target, mach_vm_address_t *address,
  ...
  if (__syscall_logger && rv == KERN_SUCCESS &&
    ...
    __syscall_logger(eventTypeFlags | userTagFlags, (uintptr_t)target,
                     (uintptr_t)size, 0, (uintptr_t)*address, 0);
  }

  return rv;
}

kern_return_t mach_vm_remap(mach_port_name_t target, mach_vm_address_t *address,
                            mach_vm_size_t size, mach_vm_offset_t mask,
                            int flags, mach_port_name_t src_task,
  ...
  rv = _kernelrpc_mach_vm_remap(target, address, size, mask, flags, src_task,
                                src_address, copy, cur_protection,
...
kern_return_t
mach_vm_remap_new(mach_port_name_t target, mach_vm_address_t *address,
                  mach_vm_size_t size, mach_vm_offset_t mask, int flags,
                  mach_port_name_t src_task, mach_vm_address_t src_address,
...
kern_return_t mach_vm_read(mach_port_name_t target, mach_vm_address_t address,
  ...
  if (__syscall_logger && rv == KERN_SUCCESS) {
    ...
    // so pass mach_task_self() as the destination task receiving the
    // allocation.
    __syscall_logger(eventTypeFlags, (uintptr_t)mach_task_self(),
                     (uintptr_t)*dataCnt, 0, *data, 0);
  }

  return rv;
}

kern_return_t vm_map(mach_port_name_t target, vm_address_t *address,
                     vm_size_t size, vm_offset_t mask, int flags,
                     mem_entry_name_port_t object, vm_offset_t offset,
                     boolean_t copy, vm_prot_t cur_protection,
                     vm_prot_t max_protection, vm_inherit_t inheritance) {
  kern_return_t rv;

  rv = _kernelrpc_vm_map(target, address, size, mask, flags, object, offset,
...
kern_return_t vm_remap(mach_port_name_t target, vm_address_t *address,
                       vm_size_t size, vm_offset_t mask, int flags,
                       mach_port_name_t src_task, vm_address_t src_address,
                       boolean_t copy, vm_prot_t *cur_protection,
                       vm_prot_t *max_protection, vm_inherit_t inheritance) {
  kern_return_t rv;

  rv = _kernelrpc_vm_remap(target, address, size, mask, flags, src_task,
                           src_address, copy, cur_protection, max_protection,
                           inheritance);
...
kern_return_t vm_remap_new(mach_port_name_t target, vm_address_t *address,
                           vm_size_t size, vm_offset_t mask, int flags,
                           mach_port_name_t src_task, vm_address_t src_address,
                           boolean_t copy, vm_prot_t *cur_protection,
                           vm_prot_t *max_protection,
  ...
  /* {max,cur}_protection is inout */
  rv = _kernelrpc_vm_remap_new(target, address, size, mask, flags, src_task,
                               src_address, copy, cur_protection,
                               max_protection, inheritance);
...
kern_return_t vm_read(mach_port_name_t target, vm_address_t address,
...
kern_return_t mach_vm_purgable_control(mach_port_name_t target,
                                       mach_vm_offset_t address,
                                       vm_purgable_t control, int *state) {
  kern_return_t rv;

  rv =
      _kernelrpc_mach_vm_purgable_control_trap(target, address, control, state);
...
kern_return_t vm_purgable_control(mach_port_name_t task, vm_offset_t address,
                                  vm_purgable_t control, int *state) {
  return mach_vm_purgable_control(task, (mach_vm_offset_t)address, control,
                                  state);
}
...
kern_return_t mach_vm_update_pointers_with_remote_tags(
  ...
  return _kernelrpc_mach_vm_update_pointers_with_remote_tags(
      target, in_pointer_list, in_pointer_listCnt, out_pointer_list,
      out_pointer_listCnt);
}
