...
_Static_assert(VM_RECLAIM_MAX_CAPACITY <= UINT32_MAX,
               "Max capacity must fit in mach_vm_reclaim_count_t");
...
static uint64_t max_buffer_len_for_size(mach_vm_size_t size) {
  mach_vm_size_t entries_size =
      size - offsetof(struct mach_vm_reclaim_ring_s, entries);
  return entries_size / sizeof(struct mach_vm_reclaim_entry_s);
}

static mach_vm_reclaim_count_t round_buffer_len(mach_vm_reclaim_count_t count) {
...
mach_vm_reclaim_ring_allocate(mach_vm_reclaim_ring_t *ring_out,
  ...
  *ring_out = NULL;
  kr = mach_vm_deferred_reclamation_buffer_allocate(
      mach_task_self(), &vm_addr, &sampling_period_abs, initial_capacity,
      max_capacity);
  if (kr == ERR_SUCCESS) {
    mach_vm_reclaim_ring_t ring = (mach_vm_reclaim_ring_t)vm_addr;
...
mach_vm_reclaim_error_t
mach_vm_reclaim_ring_resize(mach_vm_reclaim_ring_t ring,
  ...
  err = mach_vm_deferred_reclamation_buffer_resize(mach_task_self(), capacity,
                                                   &bytes_reclaimed);
  if (err == ERR_SUCCESS) {
    ring->len = capacity;
  ...
  size_t reclaimable_bytes =
      os_atomic_sub(&ring->reclaimable_bytes, bytes_reclaimed, relaxed);
  os_atomic_min(&ring->reclaimable_bytes_min, reclaimable_bytes, relaxed);
  return err;
}

mach_vm_reclaim_count_t
mach_vm_reclaim_round_capacity(mach_vm_reclaim_count_t count) {
...
mach_vm_reclaim_error_t mach_vm_reclaim_try_enter(
    mach_vm_reclaim_ring_t ring, mach_vm_address_t region_start,
    mach_vm_size_t region_size, mach_vm_reclaim_action_t action,
    mach_vm_reclaim_id_t *id, bool *should_update_kernel_accounting) {
  ...
  *id = VM_RECLAIM_ID_NULL;
  ...
  if (requested_id == VM_RECLAIM_ID_NULL) {
    ...
    /*
     * idx must be >= head & the buffer is not full so it's not possible for the
     * kernel to be acting on the entry at (tail + 1) % size. Thus we don't need
     * to check the busy pointer here.
     */
    struct mach_vm_reclaim_entry_s entry =
        construct_entry(region_start, size32, action);
    entries[tail % buffer_len] = entry;
    os_atomic_thread_fence(seq_cst); // tail increment can not be seen before
                                     // the entry is cleared in the buffer
    ...
    *id = tail;
    ...
    _assert("mach_vm_reclaim_mark_free_with_id", requested_id < original_tail,
            original_tail);

    os_atomic_store_wide(&ring->tail, requested_id, relaxed);
    os_atomic_thread_fence(
        seq_cst); // Our write to tail must happen before our read of busy
  ...
  size_t reclaimable_bytes =
      os_atomic_add(&ring->reclaimable_bytes, region_size, relaxed);
  os_atomic_min(&ring->reclaimable_bytes_min, reclaimable_bytes, relaxed);
...
mach_vm_reclaim_error_t mach_vm_reclaim_try_cancel(
    mach_vm_reclaim_ring_t ring, mach_vm_reclaim_id_t id,
    mach_vm_address_t region_start, mach_vm_size_t region_size,
    mach_vm_reclaim_action_t behavior, mach_vm_reclaim_state_t *state,
  ...
  *should_update_kernel_accounting = false;
  ...
  _assert("mach_vm_reclaim_mark_used", entry->address == region_start,
          entry->address);
  _assert("mach_vm_reclaim_mark_used", entry->behavior == behavior,
          entry->behavior);
  ...
  size_t reclaimable_bytes =
      os_atomic_sub(&ring->reclaimable_bytes, region_size, relaxed);
  os_atomic_min(&ring->reclaimable_bytes_min, reclaimable_bytes, relaxed);
...
mach_vm_reclaim_error_t mach_vm_reclaim_query_state(
    mach_vm_reclaim_ring_t ring, mach_vm_reclaim_id_t id,
    mach_vm_reclaim_action_t action, mach_vm_reclaim_state_t *state) {
  if (ring == NULL) {
    return VM_RECLAIM_INVALID_RING;
  }
  if (id == VM_RECLAIM_ID_NULL) {
...
bool mach_vm_reclaim_is_reusable(mach_vm_reclaim_state_t state) {
  return state == VM_RECLAIM_FREED || state == VM_RECLAIM_UNRECLAIMED;
}

mach_vm_reclaim_error_t
mach_vm_reclaim_ring_capacity(mach_vm_reclaim_ring_t ring,
                              mach_vm_reclaim_count_t *capacity) {
  if (ring == NULL) {
    return VM_RECLAIM_INVALID_RING;
  }
...
mach_vm_reclaim_error_t
mach_vm_reclaim_ring_flush(mach_vm_reclaim_ring_t ring,
  ...
  err = mach_vm_deferred_reclamation_buffer_flush(
      mach_task_self(), num_entries_to_reclaim, &bytes_reclaimed);
  if (err == ERR_SUCCESS) {
    size_t reclaimable_bytes =
        os_atomic_sub(&ring->reclaimable_bytes, bytes_reclaimed, relaxed);
...
mach_vm_reclaim_ring_copy(task_read_t task, mach_vm_reclaim_ring_ref_t ref,
  ...
  kern_return_t kr = mach_vm_remap(mach_task_self(), &address, ref->size, 0,
                                   VM_FLAGS_ANYWHERE, task, ref->addr, TRUE,
...
mach_vm_reclaim_error_t
mach_vm_reclaim_copied_ring_free(mach_vm_reclaim_ring_copy_t *cring) {
  kern_return_t kr;
  mach_vm_reclaim_ring_t ring = (mach_vm_reclaim_ring_t)*cring;

  kr = mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)*cring,
                          ring->_unused);
...
mach_vm_reclaim_error_t
mach_vm_reclaim_copied_ring_query(mach_vm_reclaim_ring_copy_t *ring_copy,
