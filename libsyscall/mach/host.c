...
kern_return_t host_get_atm_diagnostic_flag(host_t host __unused,
                                           uint32_t *diagnostic_flag) {
  *diagnostic_flag = COMM_PAGE_READ(uint32_t, ATM_DIAGNOSTIC_CONFIG);
  return KERN_SUCCESS;
}

kern_return_t host_get_multiuser_config_flags(host_t host __unused,
...
kern_return_t host_check_multiuser_mode(host_t host __unused,
  ...
  *multiuser_mode =
      (multiuser_flags & kIsMultiUserDevice) == kIsMultiUserDevice;
  return KERN_SUCCESS;
#else
  (void)multiuser_mode;
  return KERN_NOT_SUPPORTED;
...
extern kern_return_t _kernelrpc_host_create_mach_voucher(
    mach_port_name_t host, mach_voucher_attr_raw_recipe_array_t recipes,
    mach_voucher_attr_recipe_size_t recipesCnt, mach_port_name_t *voucher);
...
kern_return_t host_create_mach_voucher(
  ...
  if (rv == MACH_SEND_INVALID_DEST) {
    rv =
        _kernelrpc_host_create_mach_voucher(host, recipes, recipesCnt, voucher);
  }
...
extern kern_return_t host_get_io_master(host_t host, io_main_t *io_main);

kern_return_t host_get_io_master(host_t host, io_main_t *io_main) {
