...
static bool cpu_needs_throttle_tunable(uint32_t midr_pnum) {
  switch (midr_pnum) {
...
void configure_late_apple_regs(bool cold_boot) {
  ...
  if (needs_rdar_145882231 && arm64_is_p_core()) {
    ...
    if (expect != hid16) {
      panic("HID16 not as expected, Got: 0x%llx, Expected: 0x%llx", hid16,
            expect);
    }
  }
#endif /* CHECK_RDAR_145882231 */
  ...
  if (tinfo->max_die_id > 0) {
    if (midr_pnum == MIDR_RHODES_DIE_AVALANCHE ||
        midr_pnum == MIDR_RHODES_DIE_BLIZZARD) {
...
static inline void vmapple_pac_get_default_keys() {
  ...
  asm volatile(
      ...
      "str	x3, %[el0_key]"
      "\n"
      : [b_key] "=m"(vmapple_default_rop_pid), [el0_key] "=m"(
                                                   vmapple_default_jop_pid)
      : [fn] "r"(fn)
      : "x0", "x1", "x2", "x3", "x4");
}
