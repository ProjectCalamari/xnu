...
MACHINE_TIMEOUT(lock_panic_timeout, "lock-panic",
                0xc00000 /* 12.5 m ticks ~= 524ms with 24MHz OSC */,
...
void atomic_exchange_abort(void) { os_atomic_clear_exclusive(); }
 ...
 */
MARK_AS_HIBERNATE_TEXT void inline arm_usimple_lock_init(
    simple_lock_t lck, __unused unsigned short initial_value) {
...
void lck_spin_lock(lck_spin_t *lock) { lck_ticket_lock(lock, LCK_GRP_NULL); }

void lck_spin_lock_nopreempt(lck_spin_t *lock) {
  lck_ticket_lock_nopreempt(lock, LCK_GRP_NULL);
}

int lck_spin_try_lock(lck_spin_t *lock) {
  return lck_ticket_lock_try(lock, LCK_GRP_NULL);
}

int lck_spin_try_lock_nopreempt(lck_spin_t *lock) {
  return lck_ticket_lock_try_nopreempt(lock, LCK_GRP_NULL);
}

void lck_spin_unlock(lck_spin_t *lock) { lck_ticket_unlock(lock); }
...
__abortlike static void __lck_spin_invalid_panic(lck_spin_t *lck) {
  ...
  panic("%s spinlock %p: <0x%016lx 0x%16lx>", how, lck, lck->lck_spin_data,
        lck->type);
}

static inline void lck_spin_verify(lck_spin_t *lck) {
...
void lck_spin_lock_nopreempt_grp(lck_spin_t *lock, lck_grp_t *grp) {
#pragma unused(grp)
  lck_spin_verify(lock);
...
int lck_spin_try_lock_grp(lck_spin_t *lock, lck_grp_t *grp) {
#pragma unused(grp)
  lck_spin_verify(lock);
...
int lck_spin_try_lock_nopreempt_grp(lck_spin_t *lock, lck_grp_t *grp) {
#pragma unused(grp)
  lck_spin_verify(lock);
...
void lck_spin_unlock_nopreempt(lck_spin_t *lock) {
  lck_spin_verify(lock);
 ...
 *	maintaining preemption state.
 */
void(usimple_unlock)(usimple_lock_t l) { simple_unlock((simple_lock_t)l); }
 ...
 */
unsigned int(usimple_lock_try)(usimple_lock_t l LCK_GRP_ARG(lck_grp_t *grp)) {
