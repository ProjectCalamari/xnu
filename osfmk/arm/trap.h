...
#define TRAP_NAMES                                                             \
  "reset", "undefined instruction", "software interrupt", "prefetch abort",    \
      "data abort", "irq interrupt", "fast interrupt", "perfmon"
...
#define ml_recoverable_trap(code) __asm__ volatile("brk #%0" : : "i"(code))
...
/*
 * Unfortunately brk instruction only takes constant, so we have to unroll all
 * the cases and let compiler do the real work.  ¯\_(ツ)_/¯
 *
...
__attribute__((cold, always_inline)) static inline void
