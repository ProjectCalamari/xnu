...
static void arm_slide_rebase_and_sign_image(void) {
  ...
  if (kernel_mach_header_is_in_fileset(k_mh)) {
    ...
    /* rebase and sign jops */
    static_kernelcache =
        &__thread_starts_sect_end[0] != &__thread_starts_sect_start[0];
    if (static_kernelcache) {
      rebase_threaded_starts(&__thread_starts_sect_start[0],
                             &__thread_starts_sect_end[0], (uintptr_t)k_mh,
                             (uintptr_t)k_mh - slide, slide);
    }
#if defined(HAS_APPLE_PAC)
...
/*
 *		Routine:		arm_init
 *		Function:		Runs on the boot CPU, once, on entry
 * from iBoot.
 */

__startup_func void arm_init(boot_args *args) {
  unsigned int maxmem;
  uint32_t memsize;
  uint64_t xmaxmem;
  thread_t thread;
  ...
  if (PE_parse_boot_argn("serial", &serialmode, sizeof(serialmode))) {
    ...
    if (on_demand && !(serialmode & SERIALMODE_INPUT)) {
      kprintf("WARNING: invalid serial boot-args : ON_DEMAND (0x%x) flag "
...
void arm_init_cpu(cpu_data_t *cpu_data_ptr, uint64_t __unused hib_header_phys) {
#if __ARM_PAN_AVAILABLE__
  __builtin_arm_wsr("pan", 1);
#endif
