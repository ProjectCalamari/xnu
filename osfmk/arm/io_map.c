...
__startup_func static void io_map_init(void) {
  vm_map_will_allocate_early_map(&io_submap);
  io_submap =
      kmem_suballoc(kernel_map, &io_range.min_address, IO_MAP_SIZE,
                    VM_MAP_CREATE_NEVER_FAULTS | VM_MAP_CREATE_DISABLE_HOLELIST,
                    VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,
                    KMS_PERMANENT | KMS_NOFAIL, VM_KERN_MEMORY_IOKIT)
          .kmr_submap;
}
 ...
 */
vm_offset_t io_map(vm_map_offset_t phys_addr, vm_size_t size,
  ...
  if (startup_phase < STARTUP_SUB_KMEM) {
    ...
    if (flags == VM_WIMG_WCOMB) {
      pmap_map_bd_with_options(start, phys_addr, phys_addr + alloc_size, prot,
                               PMAP_MAP_BD_WCOMB);
    } else {
    ...
    kmem_alloc(unmappable ? kernel_map : io_submap, &start, alloc_size,
               kmaflags, VM_KERN_MEMORY_IOKIT);
