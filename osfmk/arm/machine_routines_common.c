...
static void
sched_perfcontrol_oncore_default(perfcontrol_state_t new_thread_state __unused,
                                 going_on_core_t on __unused) {}

static void sched_perfcontrol_switch_default(
    perfcontrol_state_t old_thread_state __unused,
    perfcontrol_state_t new_thread_state __unused) {}

static void
sched_perfcontrol_offcore_default(perfcontrol_state_t old_thread_state __unused,
...
static void sched_perfcontrol_max_runnable_latency_default(
    perfcontrol_max_runnable_latency_t latencies __unused) {}

static void sched_perfcontrol_work_interval_notify_default(
    perfcontrol_state_t thread_state __unused,
    perfcontrol_work_interval_t work_interval __unused) {}

static void sched_perfcontrol_work_interval_ctl_default(
    perfcontrol_state_t thread_state __unused,
...
static void sched_perfcontrol_csw_default(
    __unused perfcontrol_event event, __unused uint32_t cpu_id,
    __unused uint64_t timestamp, __unused uint32_t flags,
    __unused struct perfcontrol_thread_data *offcore,
    __unused struct perfcontrol_thread_data *oncore,
    __unused struct perfcontrol_cpu_counters *cpu_counters,
    __unused uint64_t *timeout_ticks) {}

static void sched_perfcontrol_state_update_default(
    __unused perfcontrol_event event, __unused uint32_t cpu_id,
    __unused uint64_t timestamp, __unused uint32_t flags,
    __unused struct perfcontrol_thread_data *thr_data,
    __unused uint64_t *timeout_ticks) {}

static void sched_perfcontrol_thread_group_blocked_default(
    __unused thread_group_data_t blocked_tg,
    __unused thread_group_data_t blocking_tg, __unused uint32_t flags,
    __unused perfcontrol_state_t blocked_thr_state) {}

static void sched_perfcontrol_thread_group_unblocked_default(
    __unused thread_group_data_t unblocked_tg,
    __unused thread_group_data_t unblocking_tg, __unused uint32_t flags,
    __unused perfcontrol_state_t unblocked_thr_state) {}

static void sched_perfcontrol_running_timer_expire_default(
    __unused uint64_t now, __unused uint32_t flags, __unused uint32_t cpu_id,
    __unused uint64_t *timeout_ticks) {}

sched_perfcontrol_offcore_t sched_perfcontrol_offcore =
    sched_perfcontrol_offcore_default;
sched_perfcontrol_context_switch_t sched_perfcontrol_switch =
    sched_perfcontrol_switch_default;
sched_perfcontrol_oncore_t sched_perfcontrol_oncore =
    sched_perfcontrol_oncore_default;
sched_perfcontrol_thread_group_init_t sched_perfcontrol_thread_group_init =
    sched_perfcontrol_thread_group_default;
sched_perfcontrol_thread_group_deinit_t sched_perfcontrol_thread_group_deinit =
    sched_perfcontrol_thread_group_default;
sched_perfcontrol_thread_group_flags_update_t
    sched_perfcontrol_thread_group_flags_update =
        sched_perfcontrol_thread_group_default;
sched_perfcontrol_max_runnable_latency_t
    sched_perfcontrol_max_runnable_latency =
        sched_perfcontrol_max_runnable_latency_default;
sched_perfcontrol_work_interval_notify_t
    sched_perfcontrol_work_interval_notify =
        sched_perfcontrol_work_interval_notify_default;
sched_perfcontrol_work_interval_ctl_t sched_perfcontrol_work_interval_ctl =
    sched_perfcontrol_work_interval_ctl_default;
sched_perfcontrol_deadline_passed_t sched_perfcontrol_deadline_passed =
    sched_perfcontrol_deadline_passed_default;
sched_perfcontrol_csw_t sched_perfcontrol_csw = sched_perfcontrol_csw_default;
sched_perfcontrol_state_update_t sched_perfcontrol_state_update =
    sched_perfcontrol_state_update_default;
sched_perfcontrol_thread_group_blocked_t
    sched_perfcontrol_thread_group_blocked =
        sched_perfcontrol_thread_group_blocked_default;
sched_perfcontrol_thread_group_unblocked_t
    sched_perfcontrol_thread_group_unblocked =
        sched_perfcontrol_thread_group_unblocked_default;
sched_perfcontrol_running_timer_expire_t
    sched_perfcontrol_running_timer_expire =
...
void sched_perfcontrol_register_callbacks(
    sched_perfcontrol_callbacks_t callbacks, unsigned long size_of_state) {
  assert(callbacks == NULL ||
         callbacks->version >= SCHED_PERFCONTROL_CALLBACKS_VERSION_2);
  ...
  if (callbacks) {
    ...
    if (callbacks->version >= SCHED_PERFCONTROL_CALLBACKS_VERSION_6) {
      if (callbacks->thread_group_flags_update != NULL) {
        sched_perfcontrol_thread_group_flags_update =
            callbacks->thread_group_flags_update;
      } else {
        sched_perfcontrol_thread_group_flags_update =
            sched_perfcontrol_thread_group_default;
      }
    ...
    if (callbacks->version >= SCHED_PERFCONTROL_CALLBACKS_VERSION_8) {
      if (callbacks->thread_group_blocked != NULL) {
        sched_perfcontrol_thread_group_blocked =
            callbacks->thread_group_blocked;
      } else {
        sched_perfcontrol_thread_group_blocked =
            sched_perfcontrol_thread_group_blocked_default;
      }
      ...
      if (callbacks->thread_group_unblocked != NULL) {
        sched_perfcontrol_thread_group_unblocked =
            callbacks->thread_group_unblocked;
      } else {
        sched_perfcontrol_thread_group_unblocked =
            sched_perfcontrol_thread_group_unblocked_default;
      }
    ...
    if (callbacks->version >= SCHED_PERFCONTROL_CALLBACKS_VERSION_7) {
      if (callbacks->work_interval_ctl != NULL) {
        ...
        sched_perfcontrol_work_interval_ctl =
            sched_perfcontrol_work_interval_ctl_default;
      }
    }
    ...
    if (callbacks->version >= SCHED_PERFCONTROL_CALLBACKS_VERSION_4) {
      if (callbacks->deadline_passed != NULL) {
        ...
        sched_perfcontrol_deadline_passed =
            sched_perfcontrol_deadline_passed_default;
      }
    }
...
static void machine_switch_populate_perfcontrol_thread_data(
    struct perfcontrol_thread_data *data, thread_t thread,
...
static void machine_switch_populate_perfcontrol_cpu_counters(
...
static _Atomic uint64_t perfcontrol_callout_stats[PERFCONTROL_CALLOUT_MAX]
                                                 [PERFCONTROL_STAT_MAX];
static _Atomic uint64_t perfcontrol_callout_count[PERFCONTROL_CALLOUT_MAX];

#if CONFIG_CPU_COUNTERS
static inline bool perfcontrol_callout_counters_begin(uint64_t *counters) {
...
perfcontrol_callout_counters_end(uint64_t *start_counters,
  ...
  os_atomic_add(&perfcontrol_callout_stats[type][PERFCONTROL_STAT_CYCLES],
                end_counters[MT_CORE_CYCLES] - start_counters[MT_CORE_CYCLES],
                relaxed);
  os_atomic_add(&perfcontrol_callout_stats[type][PERFCONTROL_STAT_INSTRS],
                end_counters[MT_CORE_INSTRS] - start_counters[MT_CORE_INSTRS],
                relaxed);
...
/*
 * The Edge scheduler allows the performance controller to update properties
 * about the threads as part of the callouts. These properties typically include
 * shared cluster resource usage. This allows the scheduler to manage specific
 * threads within the workload more optimally.
 */
static void sched_perfcontrol_thread_flags_update(
    thread_t thread, struct perfcontrol_thread_data *thread_data,
    shared_rsrc_policy_agent_t agent) {
  kern_return_t kr = KERN_SUCCESS;
  if (thread_data->thread_flags_mask &
      PERFCTL_THREAD_FLAGS_MASK_CLUSTER_SHARED_RSRC_RR) {
    if (thread_data->thread_flags &
        PERFCTL_THREAD_FLAGS_MASK_CLUSTER_SHARED_RSRC_RR) {
      kr = thread_shared_rsrc_policy_set(thread, 0, CLUSTER_SHARED_RSRC_TYPE_RR,
                                         agent);
    } else {
  ...
  if (thread_data->thread_flags_mask &
      PERFCTL_THREAD_FLAGS_MASK_CLUSTER_SHARED_RSRC_NATIVE_FIRST) {
    if (thread_data->thread_flags &
        PERFCTL_THREAD_FLAGS_MASK_CLUSTER_SHARED_RSRC_NATIVE_FIRST) {
      kr = thread_shared_rsrc_policy_set(
          thread, 0, CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST, agent);
    } else {
      kr = thread_shared_rsrc_policy_clear(
          thread, CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST, agent);
    }
  }
  /*
   * The thread_shared_rsrc_policy_* routines only fail if the performance
   * controller is attempting to double set/clear a policy on the thread.
   */
...
void machine_switch_perfcontrol_context(perfcontrol_event event,
                                        uint64_t timestamp, uint32_t flags,
                                        uint64_t new_thread_same_pri_latency,
                                        thread_t old, thread_t new) {
  ...
  if (sched_perfcontrol_csw != sched_perfcontrol_csw_default) {
    ...
    machine_switch_populate_perfcontrol_thread_data(
...
#endif /* CONFIG_CPU_COUNTERS */
    sched_perfcontrol_csw(event, cpu_id, timestamp, flags, &offcore, &oncore,
                          &cpu_counters, &timeout_ticks);
#if CONFIG_CPU_COUNTERS
    if (ctrs_enabled) {
    ...
    recount_add_energy(old, get_threadtask(old), offcore.energy_estimate_nj);

#if CONFIG_SCHED_EDGE
    if (sched_perfcontrol_thread_shared_rsrc_flags_enabled) {
      sched_perfcontrol_thread_flags_update(
          old, &offcore, SHARED_RSRC_POLICY_AGENT_PERFCTL_CSW);
    }
    if (timeout_ticks != 0) {
...
void machine_switch_perfcontrol_state_update(perfcontrol_event event,
                                             uint64_t timestamp, uint32_t flags,
                                             thread_t thread) {

  if (sched_perfcontrol_state_update ==
      sched_perfcontrol_state_update_default) {
    return;
  }
  uint32_t cpu_id = (uint32_t)cpu_number();
  ...
  bool ctrs_enabled = perfcontrol_callout_counters_begin(counters);
#endif /* CONFIG_CPU_COUNTERS */
  sched_perfcontrol_state_update(event, cpu_id, timestamp, flags, &data,
                                 &timeout_ticks);
#if CONFIG_CPU_COUNTERS
  if (ctrs_enabled) {
...
#if CONFIG_SCHED_EDGE
  if (sched_perfcontrol_thread_shared_rsrc_flags_enabled &&
      (event == QUANTUM_EXPIRY)) {
    sched_perfcontrol_thread_flags_update(
        thread, &data, SHARED_RSRC_POLICY_AGENT_PERFCTL_QUANTUM);
  } else {
...
void machine_thread_going_on_core(thread_t new_thread, thread_urgency_t urgency,
  ...
  on_core.qos_class =
      (uint16_t)proc_get_effective_thread_policy(new_thread, TASK_POLICY_QOS);
...
void machine_thread_going_off_core(thread_t old_thread,
...
#if CONFIG_THREAD_GROUPS
void machine_thread_group_init(struct thread_group *tg) {
  if (sched_perfcontrol_thread_group_init ==
      sched_perfcontrol_thread_group_default) {
    return;
  }
...
void machine_thread_group_deinit(struct thread_group *tg) {
  if (sched_perfcontrol_thread_group_deinit ==
      sched_perfcontrol_thread_group_default) {
    return;
  }
  struct thread_group_data data;
...
void machine_thread_group_flags_update(struct thread_group *tg,
                                       uint32_t flags) {
  if (sched_perfcontrol_thread_group_flags_update ==
      sched_perfcontrol_thread_group_default) {
    return;
  }
  struct thread_group_data data;
...
void machine_thread_group_blocked(struct thread_group *blocked_tg,
                                  struct thread_group *blocking_tg,
                                  uint32_t flags, thread_t blocked_thread) {
  if (sched_perfcontrol_thread_group_blocked ==
      sched_perfcontrol_thread_group_blocked_default) {
    return;
  }
  ...
  if (blocking_tg == NULL) {
    ...
    blocking_data.thread_group_data =
        thread_group_get_machine_data(blocking_tg);
    blocking_data.thread_group_size = thread_group_machine_data_size();
    sched_perfcontrol_thread_group_blocked(&blocked_data, &blocking_data, flags,
                                           state);
  }
  KDBG(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_BLOCK) |
           DBG_FUNC_START,
...
void machine_thread_group_unblocked(struct thread_group *unblocked_tg,
                                    struct thread_group *unblocking_tg,
                                    uint32_t flags, thread_t unblocked_thread) {
  if (sched_perfcontrol_thread_group_unblocked ==
      sched_perfcontrol_thread_group_unblocked_default) {
    return;
  }
  ...
  unblocked_data.thread_group_data =
...
void machine_perfcontrol_running_timer_expire(uint64_t now, uint32_t flags,
                                              int cpu_id,
                                              uint64_t *timeout_ticks) {
  if (sched_perfcontrol_running_timer_expire !=
      sched_perfcontrol_running_timer_expire_default) {
    sched_perfcontrol_running_timer_expire(now, flags, cpu_id, timeout_ticks);
  }
}

void machine_max_runnable_latency(uint64_t bg_max_latency,
                                  uint64_t default_max_latency,
                                  uint64_t realtime_max_latency) {
  if (sched_perfcontrol_max_runnable_latency ==
      sched_perfcontrol_max_runnable_latency_default) {
    return;
  }
...
void machine_work_interval_notify(thread_t thread,
                                  struct kern_work_interval_args *kwi_args) {
  if (sched_perfcontrol_work_interval_notify ==
      sched_perfcontrol_work_interval_notify_default) {
    return;
  }
  ...
  struct perfcontrol_work_interval work_interval = {
      ...
      .qos_class =
          (uint16_t)proc_get_effective_thread_policy(thread, TASK_POLICY_QOS),
...
void machine_perfcontrol_deadline_passed(uint64_t deadline) {
  if (sched_perfcontrol_deadline_passed !=
      sched_perfcontrol_deadline_passed_default) {
...
void ml_spin_debug_reset(thread_t thread) {
  const timeout_flags_t flags =
      ML_TIMEOUT_TIMEBASE_FLAGS | ML_TIMEOUT_PMC_FLAGS;
 ...
 * false alarms
 */
void ml_spin_debug_clear_self(void) { ml_spin_debug_clear(current_thread()); }
...
void _ml_interrupt_masked_debug_start(uintptr_t handler_addr, int type) {
  ...
  thread->machine.int_handler_addr =
      (uintptr_t)VM_KERNEL_STRIP_UPTR(handler_addr);
...
static void
__ml_trigger_interrupts_disabled_handle(thread_t thread, uint64_t timeout,
                                        int_mask_hygiene_flags_t int_flags) {
#if __AMP__
  if (int_flags == INT_MASK_IS_STACKSHOT &&
      interrupt_masked_debug_mode == SCHED_HYGIENE_MODE_PANIC) {
    /*
     * If there are no recommended performance cores, we double the timeout to
     * compensate for the difference in time it takes Stackshot to run on
     * efficiency cores, and then recheck if we still exceeded the adjusted
     * timeout.
     */
    int cpu;
    int max_cpu;
  ...
  if (interrupt_masked_debug_mode == SCHED_HYGIENE_MODE_PANIC) {
    ...
    if (int_flags & INT_MASK_FROM_HANDLER) {
      snprintf(prefix_string, PREFIX_STRING_SIZE,
               "Processing of an interrupt (type = %u, handler address = %p, "
               "vector = %p) "
               "timed out:",
               thread->machine.int_type,
    ...
    kern_timeout_try_panic(KERN_TIMEOUT_INTERRUPT, thread->machine.int_type,
                           &thread->machine.int_timeout, prefix_string,
                           timeout);
    ...
    kern_timeout_cycles_instrs(&thread->machine.int_timeout, &cycles_elapsed,
                               &instrs_elapsed);
    ...
    if (int_flags != INT_MASK_BASE) {
      ...
      DTRACE_SCHED3(interrupt_handled_dbgid, uint64_t, time_elapsed, uint64_t,
                    cycles_elapsed, uint64_t, instrs_elapsed);
      KDBG(interrupt_handled_dbgid, time_elapsed, cycles_elapsed,
           instrs_elapsed);
    } else {
      static const uint32_t interrupt_masked_dbgid =
          MACHDBG_CODE(DBG_MACH_SCHED, MACH_INT_MASKED_EXPIRED);
      DTRACE_SCHED3(interrupt_masked_dbgid, uint64_t, time_elapsed, uint64_t,
                    cycles_elapsed, uint64_t, instrs_elapsed);
      KDBG(interrupt_masked_dbgid, time_elapsed, cycles_elapsed,
           instrs_elapsed);
    }
  }
}
...
static inline void
__ml_handle_interrupts_disabled_duration(thread_t thread, uint64_t timeout,
                                         int_mask_hygiene_flags_t int_flags) {
  const timeout_flags_t flags = ML_TIMEOUT_TIMEBASE_FLAGS;
  ...
  if (__improbable(interrupt_masked_debug_mode &&
    ...
    /*
     * Disable the actual panic for KASAN due to the overhead of KASAN itself,
     * leave the rest of the mechanism enabled so that KASAN can catch any bugs
  ...
  if (int_flags != INT_MASK_BASE) {
     ...
     * platforms.)
     */
    uint64_t const old_duration =
        os_atomic_load(&thread->machine.int_time_mt, relaxed);
    os_atomic_store(&thread->machine.int_time_mt, old_duration + duration,
                    relaxed);
  }
...
void ml_handle_interrupts_disabled_duration(thread_t thread) {
  __ml_handle_interrupts_disabled_duration(
      thread, os_atomic_load(&interrupt_masked_timeout, relaxed),
      INT_MASK_BASE);
}
...
void ml_handle_stackshot_interrupt_disabled_duration(thread_t thread) {
  ...
  uint64_t stackshot_timeout =
      os_atomic_load(&stackshot_interrupt_masked_timeout, relaxed);
  ...
  __ml_handle_interrupts_disabled_duration(thread, timeout,
                                           INT_MASK_IS_STACKSHOT);
}

void ml_handle_interrupt_handler_duration(thread_t thread) {
  __ml_handle_interrupts_disabled_duration(
      thread, os_atomic_load(&interrupt_masked_timeout, relaxed),
      INT_MASK_FROM_HANDLER);
}
...
void ml_irq_debug_end() { ml_interrupt_masked_debug_end(); }
...
static void ml_interrupt_masked_debug_timestamp(thread_t thread) {
  const timeout_flags_t flags =
      ML_TIMEOUT_TIMEBASE_FLAGS | ML_TIMEOUT_PMC_FLAGS;
...
__mockable
    boolean_t ml_set_interrupts_enabled_with_debug(boolean_t enable,
                                                   boolean_t __unused debug) {
  thread_t thread;
  uint64_t state;
  ...
  if (enable && (state & DAIF_STANDARD_DISABLE)) {
    ...
    assert(getCpuDatap()->cpu_int_state ==
           NULL); // Make sure we're not enabling interrupts from primary
                  // interrupt context
#if SCHED_HYGIENE_DEBUG
    if (__probable(debug && static_if(sched_debug_interrupt_disable))) {
      // Interrupts are currently masked, we will enable them (after finishing
      // this check)
      if (stackshot_active()) {
    ...
    __builtin_arm_wsr("DAIFClr", DAIFSC_STANDARD_DISABLE);
             ...
             ((state & DAIF_STANDARD_DISABLE) != DAIF_STANDARD_DISABLE)) {
...
uint64_t ml_get_abstime_offset(void) { return rtclock_base_abstime; }
...
void ml_set_reset_time(__unused uint64_t wake_time) {
#if HAS_CONTINUOUS_HWCLOCK
  wake_conttime = wake_time;
#endif
}

uint64_t ml_get_conttime_wake_time(void) {
#if HAS_CONTINUOUS_HWCLOCK
  /*
 ...
 */
bool ml_snoop_thread_is_on_core(thread_t thread) {
  ...
  for (cur_cpu_num = 0; cur_cpu_num <= max_cpu_id; cur_cpu_num++) {
    if (CpuDataEntries[cur_cpu_num].cpu_data_vaddr) {
      if (CpuDataEntries[cur_cpu_num].cpu_data_vaddr->cpu_active_thread ==
          thread) {
        return true;
      }
    }
...
void ml_set_max_cpus(unsigned int max_cpus __unused) {
...
unsigned int ml_wait_max_cpus(void) {
  ...
  while (max_cpus_initialized != MAX_CPUS_SET) {
    ...
    lck_mtx_sleep(&max_cpus_lock, LCK_SLEEP_DEFAULT, &max_cpus_initialized,
                  THREAD_UNINT);
  }
  lck_mtx_unlock(&max_cpus_lock);
 ...
 *	Function: Fill out the ml_cpu_info_t structure with parameters
 * associated with the boot cluster.
 */
void ml_cpu_get_info(ml_cpu_info_t *ml_cpu_info) {
  ml_cpu_get_info_type(ml_cpu_info,
                       ml_get_topology_info()->boot_cpu->cluster_type);
}

unsigned int ml_get_cpu_number_type(cluster_type_t cluster_type, bool logical,
                                    bool available) {
  /*
...
unsigned int ml_cpu_cache_sharing(unsigned int level,
                                  cluster_type_t cluster_type,
                                  bool include_all_cpu_types __unused) {
