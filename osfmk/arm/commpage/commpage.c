...
#define ARM_FEATURE_FLAG(x) extern int gARM_##x;
...
void commpage_populate(void) {
  ...
  vm_map_address_t kernel_data_addr, kernel_text_addr, kernel_ro_data_addr,
      user_text_addr;
  pmap_create_commpages(&kernel_data_addr, &kernel_text_addr,
                        &kernel_ro_data_addr, &user_text_addr);
  ...
  bcopy(_COMM_PAGE64_SIGNATURE_STRING,
        (void *)(_COMM_PAGE_SIGNATURE + _COMM_PAGE_RW_OFFSET),
        MIN(_COMM_PAGE_SIGNATURELEN, strlen(_COMM_PAGE64_SIGNATURE_STRING)));
#endif

  *((uint16_t *)(_COMM_PAGE_VERSION + _COMM_PAGE_RW_OFFSET)) =
  ...
  *((uint8_t *)(_COMM_PAGE_CPU_CLUSTERS + _COMM_PAGE_RW_OFFSET)) =
      (uint8_t)ml_get_cluster_count();
  *((uint8_t *)(_COMM_PAGE_PHYSICAL_CPUS + _COMM_PAGE_RW_OFFSET)) =
      (uint8_t)machine_info.physical_cpu_max;
  *((uint8_t *)(_COMM_PAGE_LOGICAL_CPUS + _COMM_PAGE_RW_OFFSET)) =
      (uint8_t)machine_info.logical_cpu_max;
  *((uint64_t *)(_COMM_PAGE_MEMORY_SIZE + _COMM_PAGE_RW_OFFSET)) =
      machine_info.max_mem;
  *((uint32_t *)(_COMM_PAGE_CPUFAMILY + _COMM_PAGE_RW_OFFSET)) =
      (uint32_t)cpufamily;
  *((uint32_t *)(_COMM_PAGE_DEV_FIRM_LEGACY + _COMM_PAGE_RW_OFFSET)) =
      (uint32_t)PE_i_can_has_debugger(NULL);
  *((uint32_t *)(_COMM_PAGE_DEV_FIRM + _COMM_PAGE_RO_OFFSET)) =
      (uint32_t)PE_i_can_has_debugger(NULL);
  *((uint8_t *)(_COMM_PAGE_USER_TIMEBASE + _COMM_PAGE_RW_OFFSET)) =
      user_timebase_type();

  // Populate logical CPU -> logical cluster table
  ml_map_cpus_to_clusters(
      (uint8_t *)(_COMM_PAGE_CPU_TO_CLUSTER + _COMM_PAGE_RW_OFFSET));

  *((uint8_t *)(_COMM_PAGE_CONT_HWCLOCK + _COMM_PAGE_RW_OFFSET)) =
      (uint8_t)user_cont_hwclock_allowed();
  *((uint8_t *)(_COMM_PAGE_KERNEL_PAGE_SHIFT_LEGACY + _COMM_PAGE_RW_OFFSET)) =
      (uint8_t)page_shift;
  *((uint8_t *)(_COMM_PAGE_KERNEL_PAGE_SHIFT + _COMM_PAGE_RO_OFFSET)) =
      (uint8_t)page_shift;

#if __arm64__
  *((uint8_t *)(_COMM_PAGE_USER_PAGE_SHIFT_32_LEGACY + _COMM_PAGE_RW_OFFSET)) =
      (uint8_t)page_shift_user32;
  *((uint8_t *)(_COMM_PAGE_USER_PAGE_SHIFT_32 + _COMM_PAGE_RO_OFFSET)) =
      (uint8_t)page_shift_user32;
  *((uint8_t *)(_COMM_PAGE_USER_PAGE_SHIFT_64_LEGACY + _COMM_PAGE_RW_OFFSET)) =
      (uint8_t)SIXTEENK_PAGE_SHIFT;
  *((uint8_t *)(_COMM_PAGE_USER_PAGE_SHIFT_64 + _COMM_PAGE_RO_OFFSET)) =
      (uint8_t)SIXTEENK_PAGE_SHIFT;
#endif /* __arm64__ */
  ...
  *((uint64_t *)(_COMM_PAGE_REMOTETIME_PARAMS + _COMM_PAGE_RW_OFFSET)) =
      BT_RESET_SENTINEL_TS;

#if CONFIG_QUIESCE_COUNTER
  cpu_quiescent_set_storage(
      (_Atomic uint64_t *)(_COMM_PAGE_CPU_QUIESCENT_COUNTER +
  ...
  *((uint64_t *)(_COMM_PAGE_ASB_TARGET_VALUE + _COMM_PAGE_RW_OFFSET)) =
      asb_value;
  ...
  asb_addr += user_min;
  *((uint64_t *)(_COMM_PAGE_ASB_TARGET_ADDRESS + _COMM_PAGE_RW_OFFSET)) =
      asb_addr;

  asb_kvalue = asb_rand_vals[randidx++ % nrandval];
  *((uint64_t *)(_COMM_PAGE_ASB_TARGET_KERN_VALUE + _COMM_PAGE_RW_OFFSET)) =
      asb_kvalue;
  ...
  *((uint64_t *)(_COMM_PAGE_ASB_TARGET_KERN_ADDRESS + _COMM_PAGE_RW_OFFSET)) =
      asb_kaddr;

#if __arm64__
  *((uint8_t *)(_COMM_PAGE_APT_MSG_POLICY + _COMM_PAGE_RW_OFFSET)) =
      apt_msg_policy();
#endif
 ...
 */
extern char commpage_text_start[] __SECTION_START_SYM(COMMPAGE_TEXT_SEGMENT,
...
extern char commpage_text_end[] __SECTION_END_SYM(COMMPAGE_TEXT_SEGMENT,
                                                  COMMPAGE_TEXT_SECTION);

/* This is defined in the commpage text section as a symbol at the start of the
...
void commpage_text_populate(void) {
  ...
  if (size_of_commpage_text == 0) {
    panic("ARM comm page text section %s,%s missing", COMMPAGE_TEXT_SEGMENT,
          COMMPAGE_TEXT_SECTION);
  }
  ...
  memcpy((void *)_COMM_PAGE64_TEXT_START_ADDRESS, (void *)commpage_text_start,
         size_of_commpage_text);

  // Make sure to populate the rest of it with brk 666 so that undefined code
  // doesn't get  run
  memset((char *)_COMM_PAGE64_TEXT_START_ADDRESS + size_of_commpage_text,
         BRK_666_OPCODE, PAGE_SIZE - size_of_commpage_text);
#endif
}
...
void commpage_set_timestamp(uint64_t tbr, uint64_t secs, uint64_t frac,
  ...
  commpage_timeofday_datap =
      (new_commpage_timeofday_data_t *)(_COMM_PAGE_NEWTIMEOFDAY_DATA +
                                        _COMM_PAGE_RW_OFFSET);
...
vm_address_t _get_commpage_priv_address(void) { return commpage_rw_addr; }

vm_address_t _get_commpage_ro_address(void) { return commpage_kernel_ro_addr; }

vm_address_t _get_commpage_text_priv_address(void) {
...
/**
 * Initializes all commpage entries and sysctls for EL0 visible features in
 * ID_AA64ISAR0_EL1
 */
static void commpage_init_arm_optional_features_isar0(uint64_t *commpage_bits) {
  uint64_t bits = 0;
 ...
 * ID_AA64ISAR1_EL1
 */
static void commpage_init_arm_optional_features_isar1(uint64_t *commpage_bits) {
  uint64_t bits = 0;
...
#ifdef HAS_SPECRES2
    if ((isar1 & ID_AA64ISAR1_EL1_SPECRES_MASK) >=
        ID_AA64ISAR1_EL1_SPECRES2_EN) {
 ...
 * ID_AA64ISAR2_EL1
 */
static void commpage_init_arm_optional_features_isar2(void) {
 ...
 * ID_AA64MMFR0_EL1
 */
static void commpage_init_arm_optional_features_mmfr0(uint64_t *commpage_bits) {
  uint64_t bits = 0;
 ...
 * ID_AA64MMFR2_EL1
 */
static void commpage_init_arm_optional_features_mmfr2(uint64_t *commpage_bits) {
  uint64_t bits = 0;
 ...
 * ID_AA64PFR0_EL1
 */
static void commpage_init_arm_optional_features_pfr0(uint64_t *commpage_bits) {
  uint64_t bits = 0;
  ...
  if ((pfr0 & ID_AA64PFR0_EL1_AdvSIMD_MASK) != ID_AA64PFR0_EL1_AdvSIMD_DIS) {
    gARM_AdvSIMD = 1;
    bits |= kHasAdvSIMD;
    if ((pfr0 & ID_AA64PFR0_EL1_AdvSIMD_MASK) >=
        ID_AA64PFR0_EL1_AdvSIMD_HPFPCVT) {
 ...
 * ID_AA64PFR1_EL1
 */
static void commpage_init_arm_optional_features_pfr1(uint64_t *commpage_bits) {
 ...
 * ID_AA64SMFR0_EL1
 */
__attribute__((target("sme"))) static void
...
static void commpage_init_arm_optional_features_mmfr1(uint64_t *commpage_bits) {
  uint64_t bits = 0;
 ...
 * a supported.
 */
static void commpage_init_arm_optional_features_fpcr(uint64_t *commpage_bits) {
...
/**
 * Initializes all commpage entries and sysctls for ARM64 optional features
 * accessible from EL0.
 */
static void commpage_init_arm_optional_features(uint64_t *commpage_bits) {
...
static void commpage_init_cpu_capabilities(void) {
  ...
  *((uint32_t *)(_COMM_PAGE_CPU_CAPABILITIES + _COMM_PAGE_RW_OFFSET)) =
...
void commpage_update_active_cpus(void) {
  ...
  *((uint8_t *)(_COMM_PAGE_ACTIVE_CPUS + _COMM_PAGE_RW_OFFSET)) =
      (uint8_t)processor_avail_count;
}

/*
 * Update the commpage bits for mach_absolute_time and mach_continuous_time (for
 * userspace)
 */
void commpage_update_timebase(void) {
  if (commPagePtr) {
    *((uint64_t *)(_COMM_PAGE_TIMEBASE_OFFSET + _COMM_PAGE_RW_OFFSET)) =
...
void commpage_update_kdebug_state(void) {
  if (commPagePtr) {
    ...
    *((volatile uint32_t *)(_COMM_PAGE_KDEBUG_ENABLE + _COMM_PAGE_RW_OFFSET)) =
        state;
  }
}
...
void commpage_update_atm_diagnostic_config(uint32_t diagnostic_config) {
  if (commPagePtr) {
    *((volatile uint32_t *)(_COMM_PAGE_ATM_DIAGNOSTIC_CONFIG +
                            _COMM_PAGE_RW_OFFSET)) = diagnostic_config;
  }
}
 ...
 */
void commpage_update_multiuser_config(uint32_t multiuser_config) {
  if (commPagePtr) {
    *((volatile uint32_t *)(_COMM_PAGE_MULTIUSER_CONFIG +
                            _COMM_PAGE_RW_OFFSET)) = multiuser_config;
  }
}
...
void commpage_update_mach_approximate_time(uint64_t abstime) {
  ...
  uint64_t *approx_time_base =
      (uint64_t *)(uintptr_t)(_COMM_PAGE_APPROX_TIME + _COMM_PAGE_RW_OFFSET);
  ...
  if (saved_data < abstime) {
     ...
     */
    __unused bool ret =
        os_atomic_cmpxchg(approx_time_base, saved_data, abstime, relaxed);
  }
...
void commpage_update_mach_continuous_time(uint64_t sleeptime) {
  ...
  uint64_t *cont_time_base =
      (uint64_t *)(uintptr_t)(_COMM_PAGE_CONT_TIMEBASE + _COMM_PAGE_RW_OFFSET);

  os_atomic_store_wide(cont_time_base, sleeptime, relaxed);
}

void commpage_update_mach_continuous_time_hw_offset(uint64_t offset) {
 ...
 */
void commpage_update_boottime(uint64_t value) {
  if (!commPagePtr) {
    return;
  }

  uint64_t *boottime_usec =
      (uint64_t *)(uintptr_t)(_COMM_PAGE_BOOTTIME_USEC + _COMM_PAGE_RW_OFFSET);
 ...
 */
void commpage_set_remotetime_params(double rate, uint64_t base_local_ts,
                                    uint64_t base_remote_ts) {
  if (commPagePtr) {
#ifdef __arm64__
    struct bt_params *paramsp =
        (struct bt_params *)(_COMM_PAGE_REMOTETIME_PARAMS +
                             _COMM_PAGE_RW_OFFSET);
    ...
    __builtin_arm_dmb(DMB_ISH);
    paramsp->base_local_ts =
        base_local_ts; // This will act as a generation count
#endif                 /* __arm64__ */
  }
}
...
#if CONFIG_DTRACE
  *((uint8_t *)(_COMM_PAGE_DTRACE_DOF_ENABLED + _COMM_PAGE_RW_OFFSET)) =
      (enabled ? 1 : 0);
#else
  (void)enabled;
#endif
 ...
 */
void commpage_update_apt_active(bool active) {
  uint8_t *slot =
      (uint8_t *)(void *)(_COMM_PAGE_APT_ACTIVE + _COMM_PAGE_RW_OFFSET);
 ...
 */
void commpage_set_erm_active(bool active) {
  if (startup_phase < STARTUP_SUB_LOCKDOWN) {
    uint8_t *slot =
        (uint8_t *)(void *)(_COMM_PAGE_SECURITY_RESEARCH_DEVICE_ERM_ACTIVE +
                            _COMM_PAGE_RW_OFFSET);
    os_atomic_store(slot, active ? 1 : 0, relaxed);
  }
    ...
    kprintf(
        "ERROR can't set ERM bit at startup_phase 0x%x. Action is ignored\n",
        startup_phase);
  }
#endif
}
