 ...
 *       attributes.
 */
SECURITY_READ_ONLY_LATE(volatile pp_attr_t *)
pp_attr_table = (volatile pp_attr_t *)NULL;
 ...
 */
static page_free_entry_t *pmap_page_reclaim_list MARK_AS_PMAP_DATA =
 ...
 */
static page_free_entry_t *pmap_ppl_free_page_list MARK_AS_PMAP_DATA =
    PAGE_FREE_ENTRY_NULL;
...
/* Pointer to the pmap's VM object that can't be modified after
 * machine_lockdown(). */
 ...
 */
static uint32_t pv_kern_low_water_mark MARK_AS_PMAP_DATA =
 ...
 */
uint32_t pv_alloc_initial_target MARK_AS_PMAP_DATA =
    PV_ALLOC_CHUNK_INITIAL * MAX_CPUS;
uint32_t pv_kern_alloc_initial_target MARK_AS_PMAP_DATA =
 ...
 */
static inline uint16_t pv_free_array_n_elems(void) {
 ...
 *
 * Some of the task ledger's entries use a smaller struct format.
 * TASK_LEDGER_NUM_SMALL_INDICES is used to determine how much memory we need
 * for those entries.
 *
...
#define PMAP_LEDGER_DATA_BYTES                                                 \
  (((sizeof(task_ledgers) / sizeof(int) - TASK_LEDGER_NUM_SMALL_INDICES) *     \
 ...
 */
SECURITY_READ_ONLY_LATE(pmap_io_filter_entry_t *)
io_filter_table = (pmap_io_filter_entry_t *)0;
...
struct pmap_cpu_data_array_entry
    pmap_cpu_data_array[MAX_CPUS] MARK_AS_PMAP_DATA;
...
void pmap_data_bootstrap(void) {
  ...
  ptd_per_page =
      (PAGE_SIZE - (l2_cline_bytes - 1)) / (sizeof(pt_desc_t) + ptd_info_size);
  ...
  while (try_next) {
    increment++;
    const unsigned pt_desc_total_size = PMAP_ALIGN(
        (ptd_per_page + increment) * sizeof(pt_desc_t), l2_cline_bytes);
    const unsigned ptd_info_total_size =
        (ptd_per_page + increment) * ptd_info_size;
  ...
  const uint32_t ptd_root_table_n_ptds =
      (ptd_per_page * ((num_leaf_page_tables * 2) / ptd_per_page)) +
      ptd_per_page;
  ...
  /* Scan the device tree and override heuristics in the PV entry management
   * code. */
  pmap_compute_pv_targets();

  /* Scan the device tree and figure out how many PPL-owned I/O regions there
   * are. */
  ...
  avail_start =
      PMAP_ALIGN(avail_start + pp_attr_table_size, __alignof(pmap_io_range_t));

  io_attr_table = (pmap_io_range_t *)phystokv(avail_start);

#if HAS_GUARDED_IO_FILTER
  /* Align avail_start to size of I/O filter entry. */
  avail_start = PMAP_ALIGN(avail_start + io_attr_table_size,
                           __alignof(pmap_io_filter_entry_t));
  ...
  avail_start =
      PMAP_ALIGN(avail_start + io_filter_table_size, __alignof(pv_entry_t *));
#else  /* !HAS_GUARDED_IO_FILTER */
  avail_start =
      PMAP_ALIGN(avail_start + io_attr_table_size, __alignof(pv_entry_t *));
#endif /* HAS_GUARDED_IO_FILTER */
  ...
  pmap_ptr_array_count =
      ((pmap_ptr_array_end - pmap_ptr_array_begin) / sizeof(*pmap_ptr_array));
  ...
  avail_start +=
      round_page(LEDGER_PTR_ARRAY_SIZE * sizeof(*pmap_ledger_ptr_array));
  const vm_map_address_t pmap_ledger_ptr_array_end = phystokv(avail_start);
  pmap_ledger_ptr_array_count =
      ((pmap_ledger_ptr_array_end - pmap_ledger_ptr_array_begin) /
       sizeof(*pmap_ledger_ptr_array));

  pmap_ledger_refcnt = (os_refcnt_t *)phystokv(avail_start);
  avail_start +=
      round_page(pmap_ledger_ptr_array_count * sizeof(*pmap_ledger_refcnt));
#endif /* XNU_MONITOR */
 ...
 */
MARK_AS_PMAP_TEXT static bool ppr_find_eligible_pt_page(pt_desc_t **ptdpp) {
  assert(ptdpp != NULL);
 ...
 */
MARK_AS_PMAP_TEXT static kern_return_t ppr_remove_pt_page(pt_desc_t *ptdp) {
  assert(ptdp != NULL);
  ...
  for (unsigned i = 0; i < (PAGE_SIZE / hw_page_size); i++) {
     ...
     */
    int pte_changed =
        pmap_remove_range_options(pmap, va, begin_pte, end_pte, &eva,
                                  &need_strong_sync, PMAP_OPTIONS_REMOVE);

    const vm_offset_t expected_va_end =
        va + (size_t)pt_attr_leaf_table_size(pt_attr);
    ...
    if (eva == expected_va_end) {
       ...
       * reuse.
       */
      pmap_tte_deallocate(pmap, va, expected_va_end, need_strong_sync, ttep,
                          pt_attr_twig_level(pt_attr));
      pmap_lock(
          pmap,
          PMAP_LOCK_EXCLUSIVE); /* pmap_tte_deallocate() dropped the lock */
    } else {
      /**
      ...
      if (pte_changed > 0) {
        pmap_get_pt_ops(pmap)->flush_tlb_region_async(
            va, (size_t)(eva - va), pmap, false, need_strong_sync);
 ...
 */
MARK_AS_PMAP_TEXT static pmap_paddr_t pmap_page_reclaim(void) {
 ...
 */
MARK_AS_PMAP_TEXT static void pmap_give_free_ppl_page(pmap_paddr_t pa) {
  if ((pa & PAGE_MASK) != 0) {
    panic("%s: Unaligned address passed in, pa=0x%llx", __func__, pa);
  }
 ...
 */
MARK_AS_PMAP_TEXT static pmap_paddr_t pmap_get_free_ppl_page(void) {
  pmap_paddr_t pa = 0;
...
pmap_mark_page_as_ppl_page_internal(pmap_paddr_t pa, bool initially_free) {
  ...
  do {
    ...
    if (__improbable(attr & PP_ATTR_NO_MONITOR)) {
      ...
      !OSCompareAndSwap16(attr, attr | PP_ATTR_MONITOR, &pp_attr_table[pai]));
 ...
 */
MARK_AS_PMAP_TEXT void pmap_mark_page_as_kernel_page(pmap_paddr_t pa) {
  const unsigned int pai = pa_index(pa);
  pvh_lock(pai);
 ...
 */
MARK_AS_PMAP_TEXT pmap_paddr_t pmap_release_ppl_pages_to_kernel_internal(void) {
  pmap_paddr_t pa = 0;
 ...
 */
void pmap_enqueue_pages(vm_page_t mem) {
  vm_page_t m_prev;
 ...
 *         mutex.
 */
MARK_AS_PMAP_TEXT kern_return_t pmap_pages_alloc_zeroed(pmap_paddr_t *pa,
                                                        unsigned size,
                                                        unsigned options) {
  assert(pa != NULL);
  ...
  if (size != PAGE_SIZE) {
    panic("%s: size != PAGE_SIZE, pa=%p, size=%u, options=%u", __func__, pa,
          size, options);
  }
 ...
 *         otherwise.
 */
pmap_paddr_t pmap_alloc_page_for_kern(unsigned int options) {
  pmap_paddr_t pa = 0;
  vm_page_t mem = VM_PAGE_NULL;
 ...
 */
void pmap_pages_free(pmap_paddr_t pa, __assert_only unsigned size) {
  /**
 ...
 *         if empty.
 */
MARK_AS_PMAP_TEXT static pv_entry_t *pv_free_array_get_batch(void) {
  ...
  if (pv_free_array_n_elems() > 0) {
    ...
    assert((free_list->count == PV_BATCH_SIZE) &&
           (free_list->list != PV_ENTRY_NULL));
 ...
 */
MARK_AS_PMAP_TEXT static void pv_free_list_alloc(pv_free_list_t *free_list,
                                                 pv_entry_t **pvepp) {
  assert(pvepp != NULL);
 ...
 */
MARK_AS_PMAP_TEXT static void pv_list_kern_alloc(pv_entry_t **pvepp) {
 ...
 *              allocation.
 */
MARK_AS_PMAP_TEXT static void pv_list_alloc(pv_entry_t **pvepp) {
  assert((pvepp != NULL) && (*pvepp == PV_ENTRY_NULL));
 ...
 * @param pv_cnt Number of elements in the list to be returned.
 */
MARK_AS_PMAP_TEXT void pv_list_free(pv_entry_t *pve_head, pv_entry_t *pve_tail,
                                    int pv_cnt) {
  assert((pve_head != PV_ENTRY_NULL) && (pve_tail != PV_ENTRY_NULL));
  ...
  const uint32_t available =
      PV_BATCH_SIZE - (pmap_cpu_data->pv_free.count % PV_BATCH_SIZE);
  ...
  if (__probable((pv_cnt <= available) &&
                 ((pmap_cpu_data->pv_free.count % PV_BATCH_SIZE != 0) ||
                  (pmap_cpu_data->pv_free.count == 0)))) {
  ...
  while (pv_cnt) {
    ...
    if (__improbable(pmap_cpu_data->pv_free.count == (PV_BATCH_SIZE + 1))) {
    ...
    } else if (__improbable(pmap_cpu_data->pv_free.count ==
                            (PV_BATCH_SIZE * 2) + 1)) {
      ...
      pmap_cpu_data->pv_free.list->pve_next =
          pmap_cpu_data->pv_free_spill_marker->pve_next;
 ...
 *         failure.
 */
MARK_AS_PMAP_TEXT static kern_return_t pve_feed_page(unsigned alloc_flags) {
 ...
 */
MARK_AS_PMAP_TEXT pv_alloc_return_t pv_alloc(pmap_t pmap, unsigned int pai,
  ...
  if ((pmap == NULL) || (kernel_pmap == pmap)) {
     ...
     * most likely an IOMMU allocation.  We therefore don't know what other
     * locks this path may hold or timing constraints it may have, so we should
     * avoid a potentially expensive call to pmap_page_reclaim() on this path.
     */
    if (pmap == NULL) {
   ...
   */
  alloc_flags |=
      (options & PMAP_OPTIONS_NOWAIT) ? PMAP_PAGES_ALLOCATE_NOWAIT : 0;
 ...
 */
MARK_AS_PMAP_TEXT void pv_free(pv_entry_t *pvep) {
 ...
 */
MARK_AS_PMAP_TEXT void pmap_compute_pv_targets(void) {
  DTEntry entry = NULL;
  ...
  if (kSuccess ==
      SecureDTGetProperty(entry, "pmap-pv-count", &prop, &prop_size)) {
  ...
  if (kSuccess ==
      SecureDTGetProperty(entry, "pmap-kern-pv-count", &prop, &prop_size)) {
  ...
  if (kSuccess ==
      SecureDTGetProperty(entry, "pmap-kern-pv-min", &prop, &prop_size)) {
 ...
 */
void mapping_adjust(void) { /* Not implemented for arm/arm64. */ }
 ...
 */
MARK_AS_PMAP_TEXT kern_return_t mapping_free_prime_internal(void) {
  kern_return_t kr = KERN_FAILURE;
   ...
   */
  while (
      ((pv_free_array_n_elems() * PV_BATCH_SIZE) < pv_alloc_initial_target) ||
...
pepv_convert_ptep_to_pvep(pmap_t pmap, unsigned int pai,
                          pmap_lock_mode_t lock_mode, unsigned int options) {
  pvh_assert_locked(pai);
 ...
 *         possible return values.
 */
MARK_AS_PMAP_TEXT pv_alloc_return_t pmap_enter_pv(pmap_t pmap, pt_entry_t *ptep,
                                                  int pai, unsigned int options,
                                                  pmap_lock_mode_t lock_mode,
  ...
  if (pvh_test_type(pvh, PVH_TYPE_NULL)) {
    ...
    if (pvh_test_type(pvh, PVH_TYPE_PTEP)) {
      ...
      if ((ret = pepv_convert_ptep_to_pvep(pmap, pai, lock_mode, options)) !=
          PV_ALLOC_SUCCESS) {
        return ret;
      }
      ...
      panic("%s: unexpected PV head %p, ptep=%p pmap=%p pvh=%p", __func__, *pvh,
            ptep, pmap, pvh);
    }
    ...
    if (pve_ptep_idx == -1) {
      ...
      pvh_set_flags(pvh, pvh_flags);
      pvep = PV_ENTRY_NULL;
      if ((ret = pv_alloc(pmap, pai, lock_mode, options, &pvep)) !=
          PV_ALLOC_SUCCESS) {
        return ret;
      }
 ...
 */
void pmap_remove_pv(pmap_t pmap, pt_entry_t *ptep, int pai,
                    bool flush_tlb_async __unused, bool *is_internal_p,
                    bool *is_altacct_p) {
  ASSERT_NOT_HIBERNATING();
  pvh_assert_locked(pai);
  ...
  if (pvh_test_type(pvh, PVH_TYPE_PTEP)) {
    if (__improbable((ptep != pvh_ptep(pvh)))) {
       ...
       * unmapping, weird.
       */
      panic("%s: ptep=%p does not match pvh=%p (%p), pai=0x%x", __func__, ptep,
            pvh, pvh_ptep(pvh), pai);
    }
    ...
    while ((pvep != PV_ENTRY_NULL) &&
           ((pve_pte_idx = pve_find_ptep_index(pvep, ptep)) == -1)) {
      ...
      do {
        if (pve_find_ptep_index(check_pvep, ptep) != -1) {
          panic_plain("%s: duplicate pve entry ptep=%p pmap=%p, pvh=%p, "
                      "pvep=%p, pai=0x%x",
                      __func__, ptep, pmap, pvh, pvep, pai);
        }
    ...
    if (pve_is_empty(pvep)) {
      ...
      /*
       * This PVE contains a single mapping. See if we can coalesce it with the
       * one at the top of the list.
       */
      pv_entry_t *head_pvep = pvh_pve_list(pvh);
      int head_pve_pte_empty_idx;
      if ((head_pve_pte_empty_idx =
               pve_find_ptep_index(head_pvep, PT_ENTRY_NULL)) != -1) {
        pve_set_ptep(head_pvep, head_pve_pte_empty_idx,
                     pve_get_ptep(pvep, other_pte_idx));
        ...
        /*
         * We could not coalesce it. Move it to the start of the list, so that
         * it can be coalesced against in the future.
         */
        *pvepp = pve_next(pvep);
        pve_add(pvh, pvep);
      }
      ...
      /*
       * This PVE contains a single mapping, and it's the last mapping for this
       * PAI. Collapse this list back into the head, turning it into a
       * PVH_TYPE_PTEP entry.
       */
      pve_remove(pvh, pvepp, pvep);
  ...
  if (__improbable((pvh_flags & PVH_FLAG_FLUSH_NEEDED) &&
                   pvh_test_type(pvh, PVH_TYPE_NULL))) {
 ...
 */
MARK_AS_PMAP_TEXT void ptd_bootstrap(pt_desc_t *ptdp, unsigned int num_pages) {
  assert(ptd_per_page > 0);
  assert((ptdp != NULL) && (((uintptr_t)ptdp & PAGE_MASK) == 0) &&
         (num_pages > 0));
 ...
 *         for new nodes).
 */
MARK_AS_PMAP_TEXT pt_desc_t *ptd_alloc_unlinked(void) {
  ...
  if (__improbable(ptd_free_count == 0)) {
    ...
    if (pmap_pages_alloc_zeroed(&pa, PAGE_SIZE, PMAP_PAGES_ALLOCATE_NOWAIT) !=
        KERN_SUCCESS) {
      return NULL;
    }
  ...
  if (__improbable((ptd_free_count == 0) ||
                   (ptd_free_list == PTD_ENTRY_NULL))) {
    panic_plain("%s: out of PTD entries and for some reason didn't "
                "allocate more %d %p",
                __func__, ptd_free_count, ptd_free_list);
  }
 ...
 */
MARK_AS_PMAP_TEXT pt_desc_t *ptd_alloc(pmap_t pmap) {
 ...
 */
MARK_AS_PMAP_TEXT void ptd_deallocate(pt_desc_t *ptdp) {
  pmap_t pmap = ptdp->pmap;
 ...
 */
MARK_AS_PMAP_TEXT void ptd_info_init(pt_desc_t *ptdp, pmap_t pmap,
                                     vm_map_address_t va, unsigned int level,
                                     pt_entry_t *ptep) {
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
   ...
   */
  ptdp->va[pt_index] =
      (vm_offset_t)va & ~pt_attr_ln_offmask(pt_attr, level - 1);
 ...
 *         will panic.
 */
MARK_AS_PMAP_TEXT uint64_t pmap_ledger_validate(const volatile void *ledger) {
  assert(ledger != NULL);
 ...
 */
MARK_AS_PMAP_TEXT void pmap_ledger_verify_size_internal(size_t size) {
 ...
 *         out of PPL pages.
 */
MARK_AS_PMAP_TEXT ledger_t pmap_ledger_alloc_internal(void) {
  /**
  ...
  if (pmap_ledger_free_list == NULL) {
    ...
    const vm_map_address_t vend =
        vstart + (ledgers_per_page * sizeof(pmap_ledger_t));
    assert(vend > vstart);
     ...
     */
    for (vm_map_address_t vaddr = vstart; vaddr < vend;
         vaddr += sizeof(pmap_ledger_t)) {
      ...
      if (index >= pmap_ledger_ptr_array_count) {
        panic("%s: pmap_ledger_ptr_array is full, index=%llu", __func__, index);
      }
     ...
     * in PPL-owned memory).
     */
    pa_set_range_xprr_perm(paddr, paddr + PAGE_SIZE, XPRR_PPL_RW_PERM,
                           XPRR_KERN_RW_PERM);
  }
 ...
 */
MARK_AS_PMAP_TEXT void pmap_ledger_free_internal(ledger_t ledger) {
  /**
 ...
 */
MARK_AS_PMAP_TEXT void pmap_ledger_retain(ledger_t ledger) {
  pmap_simple_lock(&pmap_ledger_lock);
  const uint64_t array_index = pmap_ledger_validate(ledger);
 ...
 */
MARK_AS_PMAP_TEXT void pmap_ledger_release(ledger_t ledger) {
  pmap_simple_lock(&pmap_ledger_lock);
  const uint64_t array_index = pmap_ledger_validate(ledger);
 ...
 */
void pmap_ledger_check_balance(pmap_t pmap) {
  ...
  ledger_check_new_balance(cur_thread, ledger,
 ...
 */
kern_return_t pmap_ledger_credit(pmap_t pmap, int entry,
 ...
 */
kern_return_t pmap_ledger_debit(pmap_t pmap, int entry,
                                ledger_amount_t amount) {
  assert(pmap != NULL);
 ...
 *         allocated object.
 */
MARK_AS_PMAP_TEXT kern_return_t pmap_alloc_pmap(pmap_t *pmap) {
  ...
  if (pmap_free_list == NULL) {
    ...
    const vm_map_address_t vend =
        vstart + (pmaps_per_page * sizeof(pmap_list_entry_t));
    assert(vend > vstart);
     ...
     */
    for (vm_map_address_t vaddr = vstart; vaddr < vend;
         vaddr += sizeof(pmap_list_entry_t)) {
      ...
      if (__improbable(index >= pmap_ptr_array_count)) {
        panic("%s: pmap array index %lu >= limit %lu; corruption?", __func__,
              index, pmap_ptr_array_count);
      }
 ...
 */
MARK_AS_PMAP_TEXT void pmap_free_pmap(pmap_t pmap) {
  /**
 ...
 */
static void validate_user_pmap(const volatile struct pmap *pmap,
                               const char *func) {
  /**
   ...
   */
  const unsigned long array_index =
      ((const volatile pmap_list_entry_t *)pmap)->array_index;
  if (__improbable(array_index >= pmap_ptr_array_count)) {
    panic("%s: pmap array index %lu >= limit %lu", func, array_index,
          pmap_ptr_array_count);
  }
   ...
   */
  if (__improbable(pmap_ptr_array[array_index] !=
                   (const volatile pmap_list_entry_t *)pmap)) {
    panic("%s: pmap %p does not match array element %p at index %lu", func,
          pmap, pmap_ptr_array[array_index], array_index);
  }
 ...
 *             in the case that the validation fails.
 */
void validate_pmap_internal(const volatile struct pmap *pmap,
                            const char *func) {
#if !XNU_MONITOR
 ...
 *             in the case that the validation fails.
 */
void validate_pmap_mutable_internal(const volatile struct pmap *pmap,
                                    const char *func) {
#if !XNU_MONITOR
#pragma unused(pmap, func)
  assert(pmap != NULL);
...
vm_size_t pmap_compute_io_rgns(void) {
  ...
  unsigned int prop_size = 0;
  if (kSuccess !=
      SecureDTGetProperty(entry, "pmap-io-ranges", &prop, &prop_size)) {
    return 0;
  }
  ...
  for (unsigned int i = 0; i < (prop_size / sizeof(*ranges)); ++i) {
    if (ranges[i].addr & PAGE_MASK) {
      panic("%s: %u addr 0x%llx is not page-aligned", __func__, i,
            ranges[i].addr);
    }

    if (ranges[i].len & PAGE_MASK) {
      panic("%s: %u length 0x%llx is not page-aligned", __func__, i,
            ranges[i].len);
    }

    uint64_t rgn_end = 0;
    if (os_add_overflow(ranges[i].addr, ranges[i].len, &rgn_end)) {
      panic("%s: %u addr 0x%llx length 0x%llx wraps around", __func__, i,
 ...
 *         > 0 for a > b
 */
static int cmp_io_rgns(const void *a, const void *b) {
 ...
 */
static pmap_io_range_t *pmap_exempt_io_range(pmap_io_range_t *io_range) {
#if DEBUG || DEVELOPMENT
 ...
 */
pmap_io_range_t *pmap_find_io_attr(pmap_paddr_t paddr) {
   ...
   * includes `paddr`.
   */
  const pmap_io_range_t wanted_range = {.addr = paddr & ~PAGE_MASK,
                                        .len = PAGE_SIZE};
 ...
 * how much memory is needed to store all of that data.
 *
 * @note See the definition of pmap_io_filter_entry_t for more information on
 * what a "pmap-io-filter" actually represents.
 *
...
vm_size_t pmap_compute_io_filters(void) {
  ...
  unsigned int prop_size = 0;
  if (kSuccess !=
      SecureDTGetProperty(entry, "pmap-io-filters", &prop, &prop_size)) {
    return 0;
  }
  ...
  for (unsigned int i = 0; i < (prop_size / sizeof(*entries)); ++i) {
    if (entries[i].offset + entries[i].length > ARM_PGMASK) {
      panic("%s: io filter entry %u offset 0x%hx length 0x%hx crosses page "
            "boundary",
 ...
 *         > 0 for a > b
 */
static int cmp_io_filter_entries_by_signature(const void *a, const void *b) {
 ...
 *         > 0 for a > b
 */
static int cmp_io_filter_entries_by_addr(const void *a, const void *b) {
  const pmap_io_filter_entry_t *entry_a = a;
  const pmap_io_filter_entry_t *entry_b = b;
 ...
 *         > 0 for a > b
 */
static int cmp_io_filter_entries(const void *a, const void *b) {
  const int cmp_signature_result = cmp_io_filter_entries_by_signature(a, b);
  return (cmp_signature_result != 0) ? cmp_signature_result
                                     : cmp_io_filter_entries_by_addr(a, b);
}
 ...
 *
 * @note This function assumes that the amount of memory required to store the
 *       entire pmap-io-filters device tree node has already been calculated
 * (via pmap_compute_io_filters()) and allocated in io_filter_table.
 *
 * @note This function will leave io_attr_table sorted by signature and addresss
 * to allow for performing a binary search when doing future lookups.
 */
void pmap_load_io_filters(void) {
  if (num_io_filter_entries == 0) {
    return;
  ...
  qsort(io_filter_table, num_io_filter_entries, sizeof(*entries),
        cmp_io_filter_entries);

  for (unsigned int i = 0; i < num_io_filter_entries - 1; i++) {
    if (io_filter_table[i].signature == io_filter_table[i + 1].signature) {
      if (io_filter_table[i].offset + io_filter_table[i].length >
          io_filter_table[i + 1].offset) {
        panic("%s: io filter entry %u and %u overlap.", __func__, i, i + 1);
      }
    }
  }
}
...
pmap_find_io_filter_entry(pmap_paddr_t paddr, uint64_t width,
                          const pmap_io_range_t **io_range_outp) {
  ...
  const pmap_io_filter_entry_t wanted_filter = {
      ...
      .length =
          (uint16_t)width // This downcast is safe because width is validated.
  };
  ...
  for (;;) {
    const unsigned int middle = (begin + end) / 2;
    const int cmp =
        cmp_io_filter_entries(&wanted_filter, &io_filter_table[middle]);
    ...
    if (cmp == 0) {
      ...
      if ((wanted_filter.offset >= entry_found.offset) &&
          ((wanted_filter.offset + wanted_filter.length) <=
           (entry_found.offset + entry_found.length))) {
        if (io_range) {
 ...
 *                   executing this code.
 */
MARK_AS_PMAP_TEXT void pmap_cpu_data_init_internal(unsigned int cpu_number) {
  pmap_cpu_data_t *pmap_cpu_data = pmap_get_cpu_data();
  ...
  if (pmap_cpu_data->cpu_number != PMAP_INVALID_CPU_NUM) {
    panic("%s: pmap_cpu_data->cpu_number=%u, cpu_number=%u", __func__,
...
void pmap_cpu_data_array_init(void) {
  ...
  assert(((uintptr_t)pmap_stacks_end - (uintptr_t)pmap_stacks_start) ==
  ...
  for (unsigned int cpu_num = 0; cpu_num < MAX_CPUS; cpu_num++) {
     ...
     */
    for (vm_offset_t cur_va = stack_va; cur_va < (stack_va + PPL_STACK_SIZE);
         cur_va += ARM_PGBYTES) {
      ...
      pt_entry_t template = pa_to_pte(avail_start) | ARM_PTE_AF |
                            ARM_PTE_SH(SH_OUTER_MEMORY) | ARM_PTE_TYPE_VALID |
    ...
    pmap_cpu_data_array[cpu_num].cpu_data.ppl_stack =
        (void *)(stack_va + PPL_STACK_SIZE);
  ...
  for (unsigned int cpu_num = 0; cpu_num < MAX_CPUS; cpu_num++) {
    ...
    while ((ppl_cpu_save_area_end - ppl_cpu_save_area_cur) <
           sizeof(arm_context_t)) {
      avail_start += PAGE_SIZE;
      ppl_cpu_save_area_end = avail_start;
    }

    pmap_cpu_data_array[cpu_num].cpu_data.save_area =
        (arm_context_t *)phystokv(ppl_cpu_save_area_cur);
    ppl_cpu_save_area_cur += sizeof(arm_context_t);
  }

#if HAS_GUARDED_IO_FILTER
  /**
   * Enough virtual address space to cover all I/O filter stacks for every CPU
   * should have already been allocated by arm_vm_init() before pmap_bootstrap()
   * is called.
   */
  assert((iofilter_stacks_start != NULL) && (iofilter_stacks_end != NULL));
  assert(((uintptr_t)iofilter_stacks_end - (uintptr_t)iofilter_stacks_start) ==
         IOFILTER_STACK_REGION_SIZE);

  /* Each I/O filter stack contains guard pages before and after. */
  vm_offset_t iofilter_stack_va =
      (vm_offset_t)iofilter_stacks_start + ARM_PGBYTES;

  /**
   * Globally save off the beginning of the I/O filter stacks physical space so
   * that we can update its physical aperture mappings later in the bootstrap
   * process.
   */
  ...
  for (unsigned int cpu_num = 0; cpu_num < MAX_CPUS; cpu_num++) {
     ...
     */
    for (vm_offset_t cur_va = iofilter_stack_va;
         cur_va < (iofilter_stack_va + IOFILTER_STACK_SIZE);
         cur_va += ARM_PGBYTES) {
     ...
     * be zero, then you can assume the field is already set to that.
     */
    pmap_cpu_data_array[cpu_num].cpu_data.iofilter_stack =
        (void *)(iofilter_stack_va + IOFILTER_STACK_SIZE);
  ...
  for (unsigned int cpu_num = 0; cpu_num < MAX_CPUS; cpu_num++) {
    pmap_cpu_data_array[cpu_num].cpu_data.scratch_page =
 ...
 */
pmap_cpu_data_t *pmap_get_cpu_data(void) {
 ...
 */
pmap_cpu_data_t *pmap_get_remote_cpu_data(unsigned int cpu) {
#if XNU_MONITOR
  assert(cpu < MAX_CPUS);
...
void pmap_mark_page_for_cache_flush(pmap_paddr_t pa) {
  if (!pa_valid(pa)) {
    return;
  }
  const unsigned int pai = pa_index(pa);
 ...
 */
unsigned int pmap_wcrt_on_non_dram_count_get() {
 ...
 */
void pmap_wcrt_on_non_dram_count_increment_atomic() {
