...
PMAP_SUPPORT_PROTOTYPES(void, pmap_mark_page_as_ppl_page,
                        (pmap_paddr_t pa, bool initially_free),
                        PMAP_MARK_PAGE_AS_PMAP_PAGE_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_cpu_data_init, (unsigned int cpu_number),
                        PMAP_CPU_DATA_INIT_INDEX);

PMAP_SUPPORT_PROTOTYPES(uint64_t, pmap_release_ppl_pages_to_kernel, (void),
                        PMAP_RELEASE_PAGES_TO_KERNEL_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_ledger_verify_size, (size_t),
                        PMAP_LEDGER_VERIFY_SIZE_INDEX);

PMAP_SUPPORT_PROTOTYPES(ledger_t, pmap_ledger_alloc, (void),
                        PMAP_LEDGER_ALLOC_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_ledger_free, (ledger_t),
 ...
 */
void pmap_mark_page_as_ppl_page(pmap_paddr_t pa) {
 ...
 */
uint64_t pmap_release_ppl_pages_to_kernel(void) {
  pmap_paddr_t pa = 0;
...
void mapping_free_prime(void) {
   ...
   */
  for (i = 0; i < pv_alloc_initial_target;
       i += (PAGE_SIZE / sizeof(pv_entry_t))) {
    pmap_alloc_page_for_ppl(0);
  }

  for (i = 0; i < pv_kern_alloc_initial_target;
       i += (PAGE_SIZE / sizeof(pv_entry_t))) {
    pmap_alloc_page_for_ppl(0);
  }
...
/**
 * See pmap_ledger_verify_size_internal()'s function header for more
 * information.
 */
#if !XNU_MONITOR
__attribute__((noreturn))
#endif /* !XNU_MONITOR */
void pmap_ledger_verify_size(size_t size) {
#if XNU_MONITOR
