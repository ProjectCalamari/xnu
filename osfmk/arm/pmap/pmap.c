...
#define PMAP_L1_MAX_ENTRY                                                      \
  (ARM_PTE_T1_REGION_MASK(TCR_EL1_BOOT) >> ARM_TT_L1_SHIFT)
...
/* Indicates if the PPL will enforce mapping policies; set by
 * -unsafe_kernel_text */
 ...
 * This variable is accessed from various assembly trampolines, so be sure to
 * change those if you change the size or layout of this variable.
 */
...
/**
 * This variable is set true during hibernation entry to protect pmap data
 * structures during image copying, and reset false on hibernation exit.
 */
...
/* A debug flag that controls SPECRES instructions behavior facilitating perf
 * evaluation. */
...
#define ARM_PTE_IS_COMPRESSED(x, p)                                            \
  ((((x) & 0x3) == 0) &&                   /* PTE is not valid... */           \
   ...
   ((!((x) & ~ARM_PTE_COMPRESSED_MASK)) || /* ...no other bits */              \
    (panic("compressed PTE %p 0x%llx has extra bits 0x%llx: corrupted?", (p),  \
           (x), (x) & ~ARM_PTE_COMPRESSED_MASK),                               \
     FALSE)))
...
static inline void pte_set_wired(pmap_t pmap, pt_entry_t *ptep,
                                 boolean_t wired) {
  if (wired) {
    *ptep |= ARM_PTE_WIRED;
  } else {
  ...
  /*
   * Do not track wired page count for kernel pagetable pages.  Kernel mappings
   * are not guaranteed to have PTDs in the first place, and kernel pagetable
  ...
  if (wired) {
    ...
    unsigned short prev_wired =
        os_atomic_sub_orig(ptd_wiredcnt_ptr, (unsigned short)1, relaxed);
...
static inline bool pte_is_xs(const pt_attr_t *pt_attr, pt_entry_t pte) {
...
#define PMAP_UPDATE_TLBS(pmap, s, e, strong, last_level_only)                  \
  {                                                                            \
    pmap_get_pt_ops(pmap)->flush_tlb_region_async(                             \
        s, (size_t)((e) - (s)), pmap, last_level_only, strong);                \
 ...
 * parameters.
 */
static void write_pte_strong(pt_entry_t *ptep, pt_entry_t pte) {
  write_pte_fast(ptep, pte);
  FLUSH_PTE_STRONG();
}
...
pmap_t current_pmap() {
  ...
  if ((current != kernel_pmap) &&
    ...
    panic_plain("%s: Current thread's pmap doesn't match up with TTBR0 "
                "%#llx %#llx",
                __func__, get_mmu_ttb(), current->ttep);
  }
#endif /* XNU_MONITOR */
...
static int pmap_remove_range(pmap_t, vm_map_address_t, pt_entry_t *,
                             pt_entry_t *);

static tt_entry_t *pmap_tt1_allocate(pmap_t, vm_size_t, unsigned int);

#define PMAP_TT_ALLOCATE_NOWAIT 0x1

static void pmap_tt1_deallocate(pmap_t, tt_entry_t *, vm_size_t, unsigned int);

#define PMAP_TT_DEALLOCATE_NOBLOCK 0x1

static kern_return_t pmap_tt_allocate(pmap_t, tt_entry_t **, unsigned int,
                                      unsigned int);
...
static bool pmap_update_cache_attributes_locked(ppnum_t, unsigned, bool);
...
kern_return_t
pmap_enter_options_addr(pmap_t pmap, vm_map_address_t v, pmap_paddr_t pa,
...
arm_force_fast_fault_with_flush_range(ppnum_t ppnum, vm_prot_t allow_mode,
                                      int options,
...
typedef struct {
  uint64_t page_index : 32, /* The page index to be operated on */
      ...
      tlb_flush_pass_needed
      : 1, /* Tracking whether the tlb flush pass is necessary */
      rt_cache_flush_pass_needed
      : 1, /* Tracking whether the cache flush pass is necessary */
      : 0;
...
PMAP_SUPPORT_PROTOTYPES(batch_set_cache_attr_state_t,
                        ...
                        (
                            ...
                            unsigned int page_cnt, unsigned int cacheattr),
                        PMAP_BATCH_SET_CACHE_ATTRIBUTES_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_change_wiring,
...
PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_set_shared_region,
                        (pmap_t grand, pmap_t subord, addr64_t vstart,
                         uint64_t size),
                        PMAP_SET_SHARED_REGION_INDEX);

PMAP_SUPPORT_PROTOTYPES(vm_map_offset_t, pmap_nest,
                        (pmap_t grand, pmap_t subord, addr64_t vstart,
                         uint64_t size, vm_map_offset_t vrestart,
                         kern_return_t *krp),
                        PMAP_NEST_INDEX);
...
PMAP_SUPPORT_PROTOTYPES(vm_map_offset_t, pmap_unnest_options,
                        (pmap_t grand, addr64_t vaddr, uint64_t size,
                         vm_map_offset_t vrestart, unsigned int option),
...
PMAP_SUPPORT_PROTOTYPES(void, pmap_switch, (pmap_t pmap), PMAP_SWITCH_INDEX);
...
typedef enum {
  /* Validates the inputs and computes the bounds of the pmaps. This state can
     also jump directly to DONE state in some cases. */
  PMAP_TRIM_STATE_START = 0,

  /* Trims the range from the start of the shared region to the "true" start of
     that of the grand pmap. */
  PMAP_TRIM_STATE_GRAND_BEFORE,

  /* Trims the range from the "true" end of the shared region to the end of that
     of the grand pmap. */
  PMAP_TRIM_STATE_GRAND_AFTER,

  /* Decreases the subord's "no-bound" reference by one. If that becomes zero,
     trims the subord. */
...
PMAP_SUPPORT_PROTOTYPES(pmap_trim_state_t, pmap_trim,
                        (pmap_t grand, pmap_t subord, addr64_t vstart,
                         uint64_t size, pmap_trim_state_t state),
                        PMAP_TRIM_INDEX);
...
PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_check_trust_cache_runtime_for_uuid,
                        (const uint8_t check_uuid[kUUIDSize]),
                        PMAP_CHECK_TRUST_CACHE_RUNTIME_FOR_UUID_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_load_trust_cache_with_type,
                        (TCType_t type, const vm_address_t pmap_img4_payload,
                         ...
                         const vm_size_t img4_aux_manifest_len),
                        PMAP_LOAD_TRUST_CACHE_WITH_TYPE_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_toggle_developer_mode, (bool state),
                        PMAP_TOGGLE_DEVELOPER_MODE_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_query_trust_cache,
                        (TCQueryType_t query_type,
                         const uint8_t cdhash[kTCEntryHashSize],
                         TrustCacheQueryToken_t *query_token),
                        PMAP_QUERY_TRUST_CACHE_INDEX);

PMAP_SUPPORT_PROTOTYPES(errno_t, pmap_image4_monitor_trap,
                        (image4_cs_trap_t selector, const void *input_data,
                         size_t input_size),
                        PMAP_IMAGE4_MONITOR_TRAP_INDEX);

#if PMAP_CS_INCLUDE_CODE_SIGNING

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_register_provisioning_profile,
                        (const vm_address_t payload_addr,
                         const vm_size_t payload_size),
                        PMAP_REGISTER_PROVISIONING_PROFILE_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_unregister_provisioning_profile,
                        (pmap_cs_profile_t * profile_obj),
                        PMAP_UNREGISTER_PROVISIONING_PROFILE_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_associate_provisioning_profile,
                        (pmap_cs_code_directory_t * cd_entry,
                         pmap_cs_profile_t *profile_obj),
                        PMAP_ASSOCIATE_PROVISIONING_PROFILE_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_disassociate_provisioning_profile,
                        (pmap_cs_code_directory_t * cd_entry),
                        PMAP_DISASSOCIATE_PROVISIONING_PROFILE_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_associate_kernel_entitlements,
                        (pmap_cs_code_directory_t * cd_entry,
                         const void *kernel_entitlements),
                        PMAP_ASSOCIATE_KERNEL_ENTITLEMENTS_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_resolve_kernel_entitlements,
                        (pmap_t pmap, const void **kernel_entitlements),
                        PMAP_RESOLVE_KERNEL_ENTITLEMENTS_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_accelerate_entitlements,
                        (pmap_cs_code_directory_t * cd_entry),
                        PMAP_ACCELERATE_ENTITLEMENTS_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_cs_allow_invalid, (pmap_t pmap),
                        PMAP_CS_ALLOW_INVALID_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_set_compilation_service_cdhash,
                        (const uint8_t cdhash[CS_CDHASH_LEN]),
                        PMAP_SET_COMPILATION_SERVICE_CDHASH_INDEX);

PMAP_SUPPORT_PROTOTYPES(bool, pmap_match_compilation_service_cdhash,
                        (const uint8_t cdhash[CS_CDHASH_LEN]),
                        PMAP_MATCH_COMPILATION_SERVICE_CDHASH_INDEX);

PMAP_SUPPORT_PROTOTYPES(
    void, pmap_set_local_signing_public_key,
    (const uint8_t public_key[PMAP_CS_LOCAL_SIGNING_KEY_SIZE]),
    PMAP_SET_LOCAL_SIGNING_PUBLIC_KEY_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_unrestrict_local_signing,
                        (const uint8_t cdhash[CS_CDHASH_LEN]),
                        PMAP_UNRESTRICT_LOCAL_SIGNING_INDEX);

#endif

PMAP_SUPPORT_PROTOTYPES(uint32_t, pmap_lookup_in_static_trust_cache,
                        (const uint8_t cdhash[CS_CDHASH_LEN]),
                        PMAP_LOOKUP_IN_STATIC_TRUST_CACHE_INDEX);

PMAP_SUPPORT_PROTOTYPES(bool, pmap_lookup_in_loaded_trust_caches,
                        (const uint8_t cdhash[CS_CDHASH_LEN]),
                        PMAP_LOOKUP_IN_LOADED_TRUST_CACHES_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_nop, (pmap_t pmap), PMAP_NOP_INDEX);
...
#if DEVELOPMENT || DEBUG
PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_test_text_corruption,
                        (pmap_paddr_t), PMAP_TEST_TEXT_CORRUPTION_INDEX);
#endif /* DEVELOPMENT || DEBUG */
...
long long alloc_tteroot_count __attribute__((aligned(8))) MARK_AS_PMAP_DATA =
    0LL;
long long alloc_ttepages_count __attribute__((aligned(8))) MARK_AS_PMAP_DATA =
    0LL;
long long alloc_ptepages_count __attribute__((aligned(8))) MARK_AS_PMAP_DATA =
    0LL;
...
const void *__ptrauth_ppl_handler const ppl_handler_table[PMAP_COUNT] = {
    ...
    [PMAP_RELEASE_PAGES_TO_KERNEL_INDEX] =
    ...
    [PMAP_SET_LOCAL_SIGNING_PUBLIC_KEY_INDEX] =
        pmap_set_local_signing_public_key_internal,
    [PMAP_UNRESTRICT_LOCAL_SIGNING_INDEX] =
 ...
 */
MARK_AS_PMAP_TEXT static void pmap_set_pte_xprr_perm(pt_entry_t *const ptep,
  ...
  if (__improbable((new_perm > XPRR_MAX_PERM) ||
                   (expected_perm > XPRR_MAX_PERM))) {
    panic_plain(
  ...
  if (__improbable(pte_to_xprr_perm(spte) != expected_perm)) {
    panic("%s: perm=%llu does not match expected_perm, spte=0x%llx, "
          ...
          __func__, pte_to_xprr_perm(spte), spte, ptep, new_perm,
          expected_perm);
  }
 ...
 * @param new_perm The new XPRR index to update the mapping with.
 */
MARK_AS_PMAP_TEXT void pmap_set_xprr_perm(unsigned int pai,
                                          unsigned int expected_perm,
                                          unsigned int new_perm) {
...
pmap_set_range_xprr_perm(vm_address_t start, vm_address_t end,
                         unsigned int expected_perm, unsigned int new_perm) {
  /**
  ...
  while (va < end) {
     ...
     */
    vm_address_t tte_va_end = (va + pt_attr_twig_size(native_pt_attr)) &
    ...
    if (ttep == NULL) {
      panic_plain("%s: physical aperture or static region tte is NULL, "
                  "start=%p, end=%p, new_perm=%u, expected_perm=%u",
                  __func__, (void *)start, (void *)end, new_perm,
                  expected_perm);
    }
    ...
    if (!tte_is_valid_table(tte)) {
      panic_plain("%s: tte=0x%llx is not a table type entry, "
                  "start=%p, end=%p, new_perm=%u, expected_perm=%u",
                  __func__, tte, (void *)start, (void *)end, new_perm,
                  expected_perm);
    }
    ...
    const uint64_t num_ptes =
        (tte_va_end - va) >> pt_attr_leaf_shift(native_pt_attr);
...
static inline void pmap_update_plru(uint16_t asid_index __unused) {
  ...
  if (__probable(pmap_asid_plru)) {
    ...
    if (__improbable(os_atomic_andnot(&asid_plru_bitmap[plru_index],
      ...
      asid_plru_bitmap[plru_index] =
          ((plru_index == (MAX_HW_ASIDS >> 6)) ? ~(1ULL << 63) : UINT64_MAX);
    }
  }
#endif /* !HAS_16BIT_ASID */
}
...
static bool alloc_asid(pmap_t pmap) {
  ...
  if (__improbable(hw_asid == MAX_HW_ASIDS)) {
    /* If we took a PLRU "miss" and ended up with a hardware ASID we can't
     * actually support, reassign to a reserved VASID. */
    assert(pmap->sw_asid < UINT8_MAX);
    pmap->sw_asid = UINT8_MAX;
    /* Allocate from the high end of the hardware ASID range to reduce the
     * likelihood of aliasing with vital system processes, which are likely to
     * have lower ASIDs. */
  ...
  hw_asid <<= 1; // We're really handing out 2 hardware ASIDs, one for EL0 and
                 // one for EL1 access
#endif
 ...
 */
static inline pt_entry_t pmap_construct_io_pte(pmap_paddr_t paddr,
                                               pt_entry_t tmplate) {
...
vm_map_address_t pmap_map_bd_with_options(vm_map_address_t virt,
  ...
  pt_entry_t tmplate = pa_to_pte(start) | ARM_PTE_TYPE_VALID | ARM_PTE_AF |
                       ARM_PTE_NX | ARM_PTE_PNX |
...
static uint32_t pmap_compute_max_asids(void) {
  DTEntry entry;
  ...
  if (kSuccess !=
      SecureDTGetProperty(entry, "pmap-max-asids", &prop, &prop_size)) {
...
#else
  /* Round up to the nearest 64 to make things a bit easier for the Pseudo-LRU
   * allocator. */
  max_asids = (max_asids + 63) & ~63UL;

  if (((max_asids + MAX_HW_ASIDS) / (MAX_HW_ASIDS + 1)) >
      MIN(MAX_HW_ASIDS, UINT8_MAX)) {
...
void pmap_bootstrap(vm_offset_t vstart) {
   ...
   * which the returned hardware ASID is MAX_HW_ASIDS, which we do in
   * alloc_asid() and free_asid(). */
  _Static_assert(sizeof(asid_plru_bitmap[0] == sizeof(uint64_t)),
                 "bitmap_t is not a 64-bit integer");
  _Static_assert(((MAX_HW_ASIDS + 1) % 64) == 0,
                 "MAX_HW_ASIDS + 1 is not divisible by 64");
  ...
  if ((specres_debug & SPECRES_DEBUG_FORCE_RCTX) &&
      (specres_debug & SPECRES_DEBUG_FORCE_NO_RCTX)) {
  ...
  /**
   * Bootstrap any necessary SART data structures and values needed from the
   * device tree.
   */
#if CONFIG_SART
  sart_bootstrap();
...
static inline void pa_set_range_monitor(pmap_paddr_t start_pa,
                                        pmap_paddr_t end_pa) {
  pmap_paddr_t cur_pa;
...
void pa_set_range_xprr_perm(pmap_paddr_t start_pa, pmap_paddr_t end_pa,
                            unsigned int expected_perm, unsigned int new_perm) {
...
static void pmap_lockdown_kc(void) {
  ...
  while (cur_pa < end_pa) {
    ...
    if (ptov_va != cur_va) {
      /*
       * If the physical address maps back to a virtual address that is
       * non-linear w.r.t. the kernelcache, that means it corresponds to memory
       * that will be reclaimed by the OS and should therefore not be locked
       * down.
       */
      cur_pa += range_size;
      cur_va += range_size;
  ...
  pmap_paddr_t exclude_pages[] = {kvtophys_nofail((vm_offset_t)&ctrr_ro_test),
                                  kvtophys_nofail((vm_offset_t)&ctrr_nx_test)};
  for (unsigned i = 0; i < (sizeof(exclude_pages) / sizeof(exclude_pages[0]));
       ++i) {
...
void pmap_static_allocations_done(void) {
  ...
  pa_set_range_xprr_perm(monitor_start_pa, monitor_end_pa, XPRR_KERN_RW_PERM,
                         XPRR_KERN_RO_PERM);
   ...
   * Here we only need to mark the underlying physical pages as PPL-owned to
   * ensure they can't be allocated for other uses.  We don't need a special
   * xPRR protection index, as there is no PPL_RO index, and these pages are
   * ultimately protected by KTRR/CTRR.  Furthermore, use of PPL_RW for these
   * pages would expose us to a functional issue on H11 devices where CTRR
   * shifts the APRR lookup table index to USER_XO before APRR is applied,
   * leading the hardware to believe we are dealing with an user XO page upon
   * performing a translation.
   */
  monitor_start_pa = kvtophys_nofail((vm_offset_t)&ropagetable_begin);
  monitor_end_pa = monitor_start_pa + ((vm_offset_t)&ropagetable_end -
                                       (vm_offset_t)&ropagetable_begin);
  ...
  pa_set_range_xprr_perm(monitor_start_pa, monitor_end_pa, XPRR_KERN_RX_PERM,
                         XPRR_PPL_RX_PERM);
  ...
  if (pmap_ppl_disable) {
    ...
    vm_offset_t monitor_end_va =
        monitor_start_va + (ppl_cpu_save_area_end - ppl_cpu_save_area_start);

    pmap_set_range_xprr_perm(monitor_start_va, monitor_end_va, XPRR_PPL_RW_PERM,
                             XPRR_KERN_RW_PERM);
  }
  ...
  if (segSizePPLDATACONST > 0) {
    ...
    pa_set_range_xprr_perm(monitor_start_pa, monitor_end_pa, XPRR_KERN_RO_PERM,
                           XPRR_KERN_RO_PERM);
  }

  /*
   * Mark the original physical aperture mapping for the PPL stack pages RO as
   * an additional security precaution.  The real RW mappings are at a different
   * location with guard pages.
   */
  pa_set_range_xprr_perm(pmap_stacks_start_pa, pmap_stacks_end_pa,
                         XPRR_PPL_RW_PERM, XPRR_KERN_RO_PERM);

  /* Prevent remapping of the kernelcache */
  pmap_lockdown_kc();
}

void pmap_lockdown_ppl(void) {
...
__mockable boolean_t pmap_virtual_region(unsigned int region_select,
 ...
 */
bool pmap_is_page_restricted(__unused ppnum_t pn) { return false; }
 ...
 * @param ppnum Physical page number to check there are no mappings to.
 */
void pmap_assert_free(ppnum_t ppnum) {
  const pmap_paddr_t pa = ptoa(ppnum);
...
inline void pmap_recycle_page(ppnum_t pn) {
  ...
  if (__improbable(!is_freed)) {
    ...
    pmap_assert_free(pn);
#endif /* MACH_ASSERT */
    panic("%s: page 0x%llx is referenced", __func__,
          (unsigned long long)ptoa(pn));
  }
}
...
MARK_AS_PMAP_TEXT pmap_t pmap_create_options_internal(ledger_t ledger,
...
#if XNU_MONITOR
  p->tte =
      pmap_tt1_allocate(p, pmap_root_alloc_size(p), PMAP_TT_ALLOCATE_NOWAIT);
#else
  ...
  /* Ensure prior updates to the new pmap are visible before the non-zero
   * ref_count is visible */
  ...
  /*
   * Certain ledger balances can be adjusted outside the PVH lock in
   * pmap_enter(), which can lead to a concurrent disconnect operation making
   * the balance transiently negative.  The ledger should still ultimately
   * balance out, which we still check upon pmap destruction.
   */
...
__mockable pmap_t pmap_create_options(ledger_t ledger, vm_map_size_t size,
                                      unsigned int flags) {
  pmap_t pmap;
  ...
  PMAP_TRACE(1, PMAP_CODE(PMAP__CREATE) | DBG_FUNC_END,
             VM_KERNEL_ADDRHIDE(pmap), PMAP_VASID(pmap), pmap->hw_asid);
  ...
  for (ttep = first_ttep; ttep <= last_ttep; ttep++) {
    ...
    if (tte_is_block(tte)) {
      panic("%s: found block mapping, ttep=%p, tte=%p, "
            ...
            __FUNCTION__, ttep, (void *)tte, pmap, first_ttep, level);
    }
    ...
    if (level < pt_attr_twig_level(pt_attr)) {
      ...
      pmap_deallocate_all_leaf_tts(
          pmap, (tt_entry_t *)phystokv((tte)&ARM_TTE_TABLE_MASK), level + 1);
    }
...
MARK_AS_PMAP_TEXT void pmap_destroy_internal(pmap_t pmap) {
  ...
  /*
   * Issue a store-load barrier to ensure the checks of nested_count and the
   * per-CPU pmaps below will not be speculated ahead of the decrement of
   * ref_count above. That ensures that if the pmap is currently in use
   * elsewhere, this path will either observe it in use and panic, or
   * PMAP_VALIDATE_MUTABLE will observe a ref_count of 0 and panic.
   */
  ...
  for (unsigned int i = 0; i <= max_cpu; ++i) {
    ...
    if (__improbable(os_atomic_load(&cpu_data->inflight_pmap, relaxed) ==
                     pmap)) {
      panic("pmap %p: attempting to destroy while in-flight on cpu %llu", pmap,
            (uint64_t)i);
    } else if (__improbable(os_atomic_load(&cpu_data->active_pmap, relaxed) ==
                            pmap)) {
      panic("pmap %p: attempting to destroy while active on cpu %llu", pmap,
            (uint64_t)i);
    }
  }
#endif
  ...
  if (__improbable(pmap->type == PMAP_TYPE_NESTED)) {
    pmap_get_pt_ops(pmap)->flush_tlb_region_async(
        pmap->nested_region_addr, pmap->nested_region_size, pmap, false, false);
    sync_tlb_flush();
  } else {
  ...
  if (pmap->nested_region_unnested_table_bitmap) {
    ...
    pmap_pages_free(
        kvtophys_nofail(
            (vm_offset_t)(pmap->nested_region_unnested_table_bitmap)),
        PAGE_SIZE);
#else
    kfree_data(pmap->nested_region_unnested_table_bitmap,
               pmap->nested_region_unnested_table_bitmap_size *
                   sizeof(unsigned int));
#endif
  }
...
__mockable void pmap_destroy(pmap_t pmap) {
...
static tt_entry_t *pmap_tt1_allocate(pmap_t pmap, vm_size_t size,
  ...
  ret = pmap_pages_alloc_zeroed(
      &pa, (unsigned)((size < PAGE_SIZE) ? PAGE_SIZE : size),
      ((option & PMAP_TT_ALLOCATE_NOWAIT) ? PMAP_PAGES_ALLOCATE_NOWAIT : 0));
...
static void pmap_tt1_deallocate(pmap_t pmap, tt_entry_t *tt, vm_size_t size,
  ...
  while (free_page_size_tt_count > FREE_PAGE_SIZE_TT_MAX) {
    ...
    OSAddAtomic(-(int32_t)(PAGE_SIZE / PMAP_ROOT_ALLOC_SIZE),
                (pmap == kernel_pmap ? &inuse_kernel_tteroot_count
                                     : &inuse_user_tteroot_count));
...
MARK_AS_PMAP_TEXT static kern_return_t pmap_tt_allocate(pmap_t pmap,
                                                        tt_entry_t **ttp,
                                                        unsigned int level,
                                                        unsigned int options) {
  pmap_paddr_t pa;
  *ttp = NULL;
  ...
  if (*ttp == NULL) {
     ...
     */
    while (pmap_pages_alloc_zeroed(&pa, PAGE_SIZE, alloc_flags) !=
           KERN_SUCCESS) {
    ...
    /* Clear all PVH flags when using a page for a PTD to avoid tripping
     * unexpected page flag usage checks. */
    ...
    if (PAGE_SIZE > pmap_page_size) {
      ...
      if (!pmap_lock_preempt(pmap, PMAP_LOCK_EXCLUSIVE)) {
        /* Deallocate all allocated resources so far. */
        pvh_update_head_unlocked(pai_to_pvh(pa_index(pa)), PV_ENTRY_NULL,
                                 PVH_TYPE_NULL);
      ...
      for (va_end = phystokv(pa) + PAGE_SIZE,
          va = phystokv(pa) + pmap_page_size;
           va < va_end; va = va + pmap_page_size) {
...
static void pmap_tt_deallocate(pmap_t pmap, tt_entry_t *ttp,
  ...
  if (pt_acc_cnt == 0) {
    ...
    while (pt_free_entry_cnt < max_pt_index && tt_free_list) {
      ...
      if ((((vm_offset_t)tt_free_list_next) - ((vm_offset_t)ttp & ~PAGE_MASK)) <
          PAGE_SIZE) {
        pt_free_entry_cnt++;
      }
 ...
 * for TLB maintenance.
 * @param va_end Non-inclusive end of the VA range mapped by the table being
 * removed, for TLB maintenance.
 * @param need_strong_sync Indicates whether strong DSB should be used to
 * synchronize TLB maintenance.
 ...
 */
static void pmap_tte_remove(pmap_t pmap, vm_offset_t va_start,
                            vm_offset_t va_end, bool need_strong_sync,
  ...
  *ttep = (tt_entry_t)0;
  FLUSH_PTE_STRONG();
  // If given a VA range, we're being asked to flush the TLB before the table in
  // ttep is freed.
  if (va_end > va_start) {
  ...
  if (remove_leaf_table) {
    ...
    do {
      ...
      for (int i = bitmap_first(&active_disconnects[0], max_cpu + 1); i >= 0;
    ...
    /* Ensure the refcount is observed after any observation of
     * inflight_disconnect */
    os_atomic_thread_fence(acquire);
    refcnt = os_atomic_load(
        &(ptep_get_info((pt_entry_t *)ttetokv(tte))->refcnt), relaxed);
  }
  ...
  if (__improbable(remove_leaf_table && refcnt != 0)) {
    ...
    for (unsigned int i = 0; i < (pmap_page_size / sizeof(*ptep));
         i++, ptep++) {
      /**
       * Note that, for older 4K devices that emulate 16K pages, we ignore the
       * compressed marker on PTEs that aren't at an index that's a multiple
       * of 4. That's because it's possible for the 4-tuple PTE clear operation
       * in pmap_remove() and the 4-tuple PTE 'compressed' marker write
       * operation in pmap_disconnect() to race each other in such a way that
       * the compressed marker may be left in the 2nd, 3rd, and/or 4th PTEs.
       * This should be harmless as only the 1st PTE is used for accounting
       * purposes, but we don't want it to trip our internal checks here.
       */
    ...
    /* We already know the leaf page-table has a non-zero refcnt, so panic. */
    {
      ...
      panic("%s: Found inconsistent state in soon to be deleted L%d table: %d "
            "valid, "
            "%d compressed, %d non-empty, refcnt=%d, L%d tte=%#llx, pmap=%p, "
            "bpte=%p",
            __func__, level + 1, valid, comp, non_empty, refcnt, level,
            (uint64_t)tte, pmap, bpte);
    }
  }
}
 ...
 *              deleted).
 */
void pmap_tte_deallocate(pmap_t pmap, vm_offset_t va_start, vm_offset_t va_end,
                         bool need_strong_sync, tt_entry_t *ttep,
                         unsigned int level) {
  tt_entry_t tte;
 ...
 */
MARK_AS_PMAP_TEXT static int pmap_remove_range(pmap_t pmap, vm_map_address_t va,
                                               pt_entry_t *bpte,
                                               pt_entry_t *epte) {
  bool need_strong_sync = false;
  int num_changed = pmap_remove_range_options(
      pmap, va, bpte, epte, NULL, &need_strong_sync, PMAP_OPTIONS_REMOVE);
  if (num_changed > 0) {
    PMAP_UPDATE_TLBS(
        pmap, va,
        va + (pt_attr_page_size(pmap_get_pt_attr(pmap)) * (epte - bpte)),
...
/*
 *	Update the access protection bits of the physical aperture mapping for a
 * page. This is useful, for example, in guranteeing that a verified executable
 * page has no writable mappings anywhere in the system, including the physical
 ...
 */
void pmap_set_ptov_ap(unsigned int pai __unused, unsigned int ap __unused,
...
pmap_remove_range_options(pmap_t pmap, vm_map_address_t va, pt_entry_t *bpte,
                          pt_entry_t *epte, vm_map_address_t *eva,
                          bool *need_strong_sync __unused, int options) {
  pt_entry_t *cpte;
  size_t npages = 0;
  ...
  if (__improbable((uintptr_t)epte > (((uintptr_t)bpte + pmap_page_size) &
                                      ~(pmap_page_size - 1)))) {
    panic("%s: PTE range [%p, %p) in pmap %p crosses page table boundary",
          __func__, bpte, epte, pmap);
  }
  ...
  if (__improbable((pmap == kernel_pmap) && (va >= physmap_base) &&
                   (va < physmap_end))) {
    panic(
        "%s: attempt to remove mappings from the physical aperture for va: %p",
        __func__, (const void *)va);
  }
...
#if XNU_MONITOR
  bool ro_va = false;
  if (__improbable((pmap == kernel_pmap) && (eva != NULL) &&
                   zone_spans_ro_va(va, *eva))) {
    ro_va = true;
  }
#endif
  for (cpte = bpte; cpte < epte; cpte += PAGE_RATIO, va += pmap_page_size) {
    pt_entry_t spte;
    boolean_t managed = FALSE;

    /*
     * Check for pending preemption on every iteration: the PV list may be
     * arbitrarily long, so we need to be as aggressive as possible in checking
     * for preemption when we can.
     */
    ...
    while (!managed) {
      if (pmap != kernel_pmap && (options & PMAP_OPTIONS_REMOVE) &&
      ...
      if (!pa_valid(pa)) {
        ...
        if (__improbable((cacheattr & PP_ATTR_MONITOR) &&
                         ...
                         !pmap_ppl_disable)) {
          panic("%s: attempt to remove mapping of writable PPL-protected I/O "
                "address 0x%llx",
      ...
      if (pai == pa_index(pa)) {
        managed = TRUE;
        break; // Leave pai locked as we will unlock it after we free the PV
               // entry
      }
      pvh_unlock(pai);
    }
    ...
    if (*cpte != ARM_PTE_EMPTY) {
      assertf(!ARM_PTE_IS_COMPRESSED(*cpte, cpte),
              "unexpected compressed pte %p (=0x%llx)", cpte, (uint64_t)*cpte);
      assertf(pte_is_valid(*cpte), "invalid pte %p (=0x%llx)", cpte,
              (uint64_t)*cpte);
#if MACH_ASSERT
      if (managed && (pmap != kernel_pmap) && (ptep_get_va(cpte) != va)) {
        panic("pmap_remove_range_options(): VA mismatch: cpte=%p ptd=%p "
              "pte=0x%llx va=0x%llx, cpte va=0x%llx",
    ...
    if ((spte != ARM_PTE_EMPTY) && (pmap != kernel_pmap)) {
      assertf(!ARM_PTE_IS_COMPRESSED(spte, cpte),
              "unexpected compressed pte %p (=0x%llx)", cpte, (uint64_t)spte);
      assertf(pte_is_valid(spte), "invalid pte %p (=0x%llx)", cpte,
              (uint64_t)spte);
      --refcnt;
    }
  ...
  if (pmap != kernel_pmap) {
    if ((refcnt != 0) &&
        (OSAddAtomic16(refcnt, (SInt16 *)&(ptep_get_info(bpte)->refcnt)) <=
         0)) {
      panic(
          "pmap_remove_range_options: over-release of ptdp %p for pte [%p, %p)",
          ptep_get_ptd(bpte), bpte, epte);
    }
...
MARK_AS_PMAP_TEXT vm_map_address_t pmap_remove_options_internal(
  ...
  if (__improbable((end < start) ||
                   ((start | end) & pt_attr_leaf_offmask(pt_attr)))) {
    panic("%s: pmap %p invalid address range %p, %p", __func__, pmap,
          (void *)start, (void *)end);
  }
  ...
  if (tte_is_valid_table(*tte_p)) {
    ...
    /*
     * This check is really intended to ensure that mappings in a nested pmap
     * can't be removed through a top-level user pmap, although it's also a
     * useful sanity check for other pmap types. Note that kernel page tables
     * may not have PTDs, so we can't use the check there.
     */
    if (__improbable((pmap->type != PMAP_TYPE_KERNEL) &&
                     (ptep_get_pmap(bpte) != pmap))) {
      panic("%s: attempt to remove mappings owned by pmap %p through pmap %p, "
            "starting at pte %p",
    ...
    if ((pmap->type == PMAP_TYPE_USER) && (ptep_get_info(pte_p)->refcnt == 0)) {
      pmap_tte_deallocate(pmap, start, eva, need_strong_sync, tte_p,
...
__mockable void pmap_remove_options(pmap_t pmap, vm_map_address_t start,
                                    vm_map_address_t end, int options) {
  vm_map_address_t va;
...
void pmap_set_pmap(pmap_t pmap, thread_t thread) {
...
static void pmap_flush_core_tlb_asid_async(pmap_t pmap) {
  flush_core_tlb_asid_async(((uint64_t)pmap->hw_asid) << TLBI_ASID_SHIFT);
}

#if HAS_SPECRES
static void pmap_flush_core_cfp_asid_async(pmap_t pmap) {
  const uint64_t rctx_operand =
      RCTX_EL(0ULL) | RCTX_ASID((uint64_t)pmap->hw_asid);
  asm volatile("cfp rctx, %0" : : "r"(rctx_operand));
}

#if REQUIRES_DVP_RCTX
static void pmap_flush_core_dvp_asid_async(pmap_t pmap) {
  const uint64_t rctx_operand =
      RCTX_EL(0ULL) | RCTX_ASID((uint64_t)pmap->hw_asid);
...
static inline bool pmap_user_ttb_is_clear(void) {
  return get_mmu_ttb() == (invalid_ttep & TTBR_BADDR_MASK);
}

MARK_AS_PMAP_TEXT void pmap_switch_internal(pmap_t pmap) {
  ...
  __unused const pt_attr_t *last_nested_pmap_attr =
      cpu_data_ptr->cpu_nested_pmap_attr;
  __unused vm_map_address_t last_nested_region_addr =
      cpu_data_ptr->cpu_nested_region_addr;
  __unused vm_map_offset_t last_nested_region_size =
      cpu_data_ptr->cpu_nested_region_size;
  bool do_shared_region_flush =
      ((pmap != kernel_pmap) && (last_nested_pmap != NULL) &&
       (pmap->nested_pmap != last_nested_pmap));
  ...
  if ((pmap_max_asids > MAX_HW_ASIDS) && (asid_index > 0)) {
    ...
    /* Paranoia. */
    assert(asid_index < (sizeof(cpu_data_ptr->cpu_sw_asids) /
                         sizeof(*cpu_data_ptr->cpu_sw_asids)));
...
#if __ARM_MIXED_PAGE_SIZE__
  /*
   * For mixed page size configurations, we need to flush the global commpage
   * mappings from the TLB when transitioning between address spaces with
   * different page sizes.  Otherwise it's possible for a TLB fill against the
   * incoming commpage to produce a TLB entry which which partially overlaps a
   * TLB entry from the outgoing commpage, leading to a TLB conflict abort or
  ...
  if (__improbable(break_before_make && !pmap_user_ttb_is_clear())) {
    PMAP_TRACE(1, PMAP_CODE(PMAP__CLEAR_USER_TTB), VM_KERNEL_ADDRHIDE(pmap),
               PMAP_VASID(pmap), pmap->hw_asid);
  ...
  /* If we're switching to a different nested pmap (i.e. shared region), we'll
   * need to flush the userspace mappings for that region.  Those mappings are
   * global and will not be protected by the ASID.  It should also be cheaper to
   * flush the entire local TLB rather than to do a broadcast MMU flush by VA
   * region. */
  if (__improbable(do_shared_region_flush)) {
    ...
    /* NOTE: here we flush the global TLB entries for the previous nested region
     * only. There may still be non-global entries that overlap with the
     * incoming pmap's nested region.  On Apple SoCs at least, this is
     * acceptable.  Those non-global entries must necessarily belong to a
     * different ASID than the incoming pmap, or they would be flushed in the
     * do_asid_flush case below.  This will prevent them from conflicting with
     * the incoming pmap's nested region.  However, the ARMv8 ARM is not crystal
     * clear on whether such a global/inactive-nonglobal overlap is acceptable,
     * so we may need to consider additional invalidation here in the future. */
    if ((npages_prev >= ARM64_TLB_RANGE_MIN_PAGES) &&
        (npages_prev <= ARM64_TLB_RANGE_MAX_PAGES)) {
      flush_core_tlb_allrange_async(generate_rtlbi_param(
          (ppnum_t)npages_prev, 0, last_nested_region_addr, page_shift_prev));
    } else {
      /*
       * Note that we also do a full flush if npages_prev is 1 (i.e. at or below
       * ARM64_RANGE_TLB_FLUSH_THRESHOLD), but a properly configured system will
       * never have a single-page shared region anyway, not least because
       * pmap_nest() requires L2 block alignment of the address and size.
       */
      do_asid_flush = false;
  ...
  if (__improbable(do_commpage_flush)) {
    ...
    const uint64_t rtlbi_param = generate_rtlbi_param(
        (ppnum_t)_COMM_PAGE64_NESTING_SIZE >> commpage_shift, 0,
        _COMM_PAGE64_NESTING_START, commpage_shift);
...
void pmap_switch(pmap_t pmap, thread_t thread __unused) {
...
MARK_AS_PMAP_TEXT static void pmap_page_protect_options_with_flush_range(
  ...
  if (remove) {
    ...
    /*
     * Ensure the store to inflight_disconnect will be observed before any of
     * the ensuing PTE/refcount stores in this function.  This flag is used to
     * avoid a race in which the VM may clear a pmap's mappings and destroy the
     * pmap on another CPU, in between this function's clearing a PTE and
     * dropping the corresponding pagetable refcount.  That can lead to a panic
     * if the destroying thread observes a non-zero refcount.  For this we need
     * a store- store barrier; a store-release operation would not be
     * sufficient.
     */
    os_atomic_thread_fence(release);
  }
  ...
  if (__improbable(remove && (pvh_flags & PVH_FLAG_LOCKDOWN_MASK))) {
    panic("%d is locked down (%#llx), cannot remove", pai,
          (uint64_t)pvh_get_flags(pv_h));
  }
  ...
  /*
   * issue_tlbi is used to indicate that this function will need to issue at
   * least one TLB invalidation during pass 2.  tlb_flush_needed only indicates
   * that PTE permissions have changed and that a TLB flush will be needed *at
   * some point*, so we'll need to call FLUSH_PTE_STRONG() to synchronize prior
   * PTE updates.  In the case of a flush_range operation, TLB invalidation may
   * be handled by the caller so it's possible for tlb_flush_needed to be true
   * while issue_tlbi is false.
   */
  ...
  while ((pve_p != PV_ENTRY_NULL) || (pte_p != PT_ENTRY_NULL)) {
...
#if XNU_MONITOR
      if (__improbable(pvh_flags & PVH_FLAG_LOCKDOWN_MASK)) {
        panic("pmap_page_protect: ppnum 0x%x locked down, cannot be owned by "
              "iommu %p, pve_p=%p",
              ppnum, ptep_get_iommu(pte_p), pve_p);
      }
#endif
      if (remove && (options & PMAP_OPTIONS_COMPRESSOR)) {
        panic("pmap_page_protect: attempt to compress ppnum 0x%x owned by "
              "iommu %p, pve_p=%p",
              ppnum, ptep_get_iommu(pte_p), pve_p);
      }
      goto protect_skip_pve_pass1;
        ...
        do {
          if (pve_find_ptep_index(check_pvep, pte_p) != -1) {
            panic_plain("%s: duplicate pve entry ptep=%p pmap=%p, pvh=%p, "
                        "pvep=%p, pai=0x%x",
                        __func__, pte_p, pmap, pv_h, pve_p, pai);
          }
...
#endif
      panic("pmap_page_protect: bad pve entry pte_p=%p pmap=%p prot=%d "
            "options=%u, pv_h=%p, pveh_p=%p, pve_p=%p, pte=0x%llx, va=0x%llx "
            "ppnum: 0x%x",
            pte_p, pmap, prot, options, pv_h, pveh_p, pve_p, (uint64_t)*pte_p,
            (uint64_t)va, ppnum);
    }
...
#if HAS_FEAT_XS
    /**
     * TODO: we don't currently allow XS MAIR types on managed memory (see
     * wimg_to_pte()), but if we change that we'll need to allow for "strong"
     * TLBIs and DSBs in this function.
     */
    ...
    if (remove) {
      if (__improbable(pmap->type == PMAP_TYPE_COMMPAGE)) {
        panic("%s: trying to remove mappings from a COMMPAGE pmap %p when "
              "unmapping ppnum %u.",
              __func__, pmap, ppnum);
      }
      ...
      assertf(atop(pte_to_pa(spte)) == ppnum,
              "unexpected value 0x%llx for pte %p mapping ppnum 0x%x",
      ...
      if (pmap != kernel_pmap) {
        if (ppattr_test_reusable(pai) && is_internal && !is_altacct) {
          pmap_ledger_debit(pmap, task_ledgers.reusable,
                            pt_attr_page_size(pt_attr) * PAGE_RATIO);
        } else if (!is_internal) {
          pmap_ledger_debit(pmap, task_ledgers.external,
        ...
        if (is_altacct) {
          ...
          if (options & PMAP_OPTIONS_COMPRESSOR) {
            ...
            /*
             * This internal page isn't going to the compressor, so adjust stats
             * to keep phys_footprint up to date.
             */
            pmap_ledger_debit(pmap, task_ledgers.phys_footprint,
                              pt_attr_page_size(pt_attr) * PAGE_RATIO);
          }
      ...
      if (spte != tmplate) {
        /*
         * Mark the PTE so that we'll know this mapping requires a TLB flush in
         * pass 2. This allows us to avoid unnecessary flushing e.g. for COW
         * aliases that didn't require permission updates.  We use the
         * ARM_PTE_WRITEABLE bit as that bit should always be cleared by this
         * function.
         */
        ...
        /*
         * We didn't change any of the relevant permission bits in the PTE, so
         * we don't need to flush the TLB, but we do want to clear the
         * "was_writeable" flag.  When revoking write access to a page, this
         * function should always at least clear that flag for all PTEs, as the
         * VM is effectively requesting that subsequent write accesses to these
         * mappings go through vm_fault().  We therefore don't want those
         * accesses to be handled through arm_fast_fault().
         */
  ...
  while ((pve_p != PV_ENTRY_NULL) || (pte_p != PT_ENTRY_NULL)) {
    ...
    if (remove) {
      if (!compress && (pmap != kernel_pmap)) {
        /*
         * We must wait to decrement the refcount until we're completely
         * finished using the PTE on this path.  Otherwise, if we happened to
         * drop the refcount to zero, a concurrent pmap_remove() call might
         * observe the zero refcount and free the pagetable out from under us.
         */
        if (OSAddAtomic16(-1, (SInt16 *)&(ptd_get_info(ptdp, pte_p)->refcnt)) <=
            0) {
          panic(
              "pmap_page_protect_options(): over-release of ptdp %p for pte %p",
    ...
    if (remove || !flush_range || (flush_range->ptfr_pmap != pmap) ||
        (va >= flush_range->ptfr_end) || (va < flush_range->ptfr_start)) {
      pmap_get_pt_ops(pmap)->flush_tlb_region_async(
          va, pt_attr_page_size(pmap_get_pt_attr(pmap)) * PAGE_RATIO, pmap,
          true, false);
    }
...
protect_finish:
  ...
  if (__improbable(pass1_updated != pass2_updated)) {
    panic(
        "%s: first pass (%u) and second pass (%u) disagree on updated mappings",
  ...
  /*
   * If we removed PV entries, ensure prior TLB flushes are complete before we
   * drop the PVH lock to allow the backing pages to be repurposed.  This is a
   * security precaution, aimed primarily at XNU_MONITOR configurations, to
   * reduce the likelihood of an attacker causing a page to be repurposed while
   * it is still live in the TLBs.
   */
...
MARK_AS_PMAP_TEXT void pmap_page_protect_options_internal(ppnum_t ppnum,
  ...
  if (arg != NULL) {
    /*
     * If the argument is non-NULL, the VM layer is conveying its intention that
     * the TLBs should ultimately be flushed.  The nature of ARM TLB maintenance
     * is such that we can flush the TLBs much more precisely if we do so inline
     * with the pagetable updates, and PPL security model requires that we not
     * exit the PPL without performing required TLB flushes anyway. In that
     * case, force the flush to take place.
     */
...
MARK_AS_PMAP_TEXT vm_map_address_t pmap_protect_options_internal(
  ...
  if ((tte_p != (tt_entry_t *)NULL) && tte_is_valid_table(*tte_p)) {
    ...
    for (pte_p = bpte_p; pte_p < epte_p;
         pte_p += PAGE_RATIO, va += pmap_page_size) {
      ++npages;
      if (__improbable(
              !(npages % PMAP_DEFAULT_PREEMPTION_CHECK_PAGE_INTERVAL) &&
      ...
      while (!managed) {
        ...
        if (pai == pa_index(pa)) {
          managed = TRUE;
          break; // Leave the PVH locked as we will unlock it after we free the
                 // PTE
        }
        pvh_unlock(pai);
      }
 ...
 */
static inline bool pmap_enter_pte(pmap_t pmap, pt_entry_t *pte_p,
                                  pt_entry_t *old_pte, pt_entry_t new_pte,
                                  vm_map_address_t v) {
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  ...
  __unreachable_ok_push if (TEST_PAGE_RATIO_4) {
     ...
     * We actually need to update 4 ptes here which can't easily be done
     * atomically. As a result we require the exclusive pmap lock.
     */
    pmap_assert_locked(pmap, PMAP_LOCK_EXCLUSIVE);
    *old_pte = *pte_p;
    if (*old_pte == new_pte) {
      ...
      success = true;
    } else if (pa_valid(pte_to_pa(new_pte)) &&
               pte_to_pa(*old_pte) != pte_to_pa(new_pte) &&
               pte_is_valid(*old_pte)) {
      /* pte has been modified by another thread and we hold the wrong PVH lock.
       * Retry. */
      success = false;
    } else {
  ...
  __unreachable_ok_pop
...
#endif /* HAS_FEAT_XS */
      PMAP_UPDATE_TLBS(pmap, v, v + (pt_attr_page_size(pt_attr) * PAGE_RATIO),
                       need_strong_sync, true);
    } else {
      FLUSH_PTE();
    ...
    changed_wiring =
        ARM_PTE_IS_COMPRESSED(*old_pte, pte_p)
    ...
    PMAP_TRACE(
        4 + pt_attr_leaf_level(pt_attr), PMAP_CODE(PMAP__TTE),
        VM_KERNEL_ADDRHIDE(pmap), VM_KERNEL_ADDRHIDE(v),
        VM_KERNEL_ADDRHIDE(v + (pt_attr_page_size(pt_attr) * PAGE_RATIO)),
        new_pte);
  }
  return success;
}
...
pmap_construct_pte(const pmap_t pmap, vm_map_address_t va, pmap_paddr_t pa,
                   vm_prot_t prot, vm_prot_t fault_type, boolean_t wired,
  ...
  *pp_attr_bits = 0;
  ...
  if (pmap == kernel_pmap) {
    ...
    if (pmap->type != PMAP_TYPE_NESTED) {
      ...
      unsigned int index = (unsigned int)((va - pmap->nested_region_addr) >>
                                          pt_attr_twig_shift(pt_attr));

      if ((pmap->nested_region_unnested_table_bitmap) &&
          testbit(UNNEST_BIT(index),
                  (int *)pmap->nested_region_unnested_table_bitmap)) {
        pte |= ARM_PTE_NG;
      }
    }
    if (prot & VM_PROT_WRITE) {
...
MARK_AS_PMAP_TEXT kern_return_t pmap_enter_options_internal(
  ...
  bool committed = false, drop_refcnt = false, had_valid_mapping = false,
       skip_footprint_debit = false;
  ...
  if (__improbable(v & pt_attr_leaf_offmask(pt_attr))) {
    panic("%s: pmap %p v 0x%llx not page-aligned", __func__, pmap,
          (unsigned long long)v);
  }

  if (__improbable((v < pmap->min) || (v >= pmap->max) ||
                   (v < pt_attr_pagezero_size(pt_attr)))) {
    panic("%s: attempt to map illegal VA 0x%llx in pmap %p", __func__,
          (unsigned long long)v, pmap);
  }

  /* Only check kernel_pmap here as CPUWINDOWS only exists in kernel address
   * space. */
  ...
  if (__improbable((pmap == kernel_pmap) &&
                   zone_spans_ro_va(v, v + pt_attr_page_size(pt_attr)))) {
    if (__improbable(prot != VM_PROT_READ)) {
      panic("%s: attempt to map RO zone VA 0x%llx with prot 0x%x", __func__,
            (unsigned long long)v, prot);
    }
    ro_va = true;
  }
  ...
  if ((prot & VM_PROT_EXECUTE) || TEST_PAGE_RATIO_4) {
    /*
     * We need to take the lock exclusive here because of SPLAY_FIND in
     * pmap_cs_enforce.
     *
     * See rdar://problem/59655632 for thoughts on synchronization and the splay
     * tree
     */
  ...
  while (!committed) {
    ...
    if (pmap != kernel_pmap) {
      ...
      /*
       * This check is really intended to ensure that mappings in a nested pmap
       * can't be inserted through a top-level user pmap, which would allow a
       * non-global mapping to be inserted into a shared region pmap and
       * leveraged into a TLB-based write gadget (rdar://91504354). It's also a
       * useful sanity check for other pmap types, but note that kernel page
       * tables may not have PTDs, so we can't use the check there.
       */
      if (__improbable(ptep_get_pmap(pte_p) != pmap)) {
        panic("%s: attempt to enter mapping at pte %p owned by pmap %p through "
              "pmap %p",
              __func__, pte_p, ptep_get_pmap(pte_p), pmap);
      }
      /*
       * Bump the wired count to keep the PTE page from being reclaimed.  We
       * need this because we may drop the PVH and pmap locks later in
       * pmap_enter() if we need to allocate or acquire the pmap lock exclusive.
       */
      if (!wiredcnt_updated) {
    ...
    if (had_valid_mapping && (pte_to_pa(spte) != pa)) {
       ...
       *
       * This requires that we take the pmap lock exclusive in order to call
       * pmap_remove_range.
       */
    ...
    if (options & PMAP_OPTIONS_MAP_TPRO) {
      if (__improbable(pmap == kernel_pmap)) {
        panic("%s: attempt to create kernel TPRO mapping, will produce kernel "
              "RX mapping instead.",
              __func__);
      }
      pte = pmap_construct_pte(pmap, v, pa, VM_PROT_RORW_TP, fault_type, wired,
                               pt_attr, &pp_attr_bits);
    } else {
      pte = pmap_construct_pte(pmap, v, pa, prot, fault_type, wired, pt_attr,
                               &pp_attr_bits);
    }
    ...
    if (pa_valid(pa)) {
       ...
       * PPL (which can't be preempted) or we've explicitly disabled
       * preemptions. Note that we can still be interrupted, but a primary
      ...
      const bool allocation_required =
          !pvh_test_type(pv_h, PVH_TYPE_NULL) &&
      ...
      if (__improbable(allocation_required && (local_pv_free->count < 2))) {
        ...
        while (new_allocated_pves < 2) {
          local_pv_free = &pmap_get_cpu_data()->pv_free;
          pv_status = pv_alloc(pmap, pai, lock_mode, options,
                               &new_pve_p[new_allocated_pves]);
      ...
      if (__improbable(ppattr_pa_test_monitor(pa))) {
        panic("%s: page belongs to PPL, "
              "pmap=%p, v=0x%llx, pa=%p, prot=0x%x, fault_type=0x%x, "
              "flags=0x%x, wired=%u, options=0x%x",
              __FUNCTION__, pmap, v, (void *)pa, prot, fault_type, flags, wired,
              options);
      }

      if (__improbable(pvh_get_flags(pai_to_pvh(pai)) &
                       PVH_FLAG_LOCKDOWN_MASK)) {
      ...
      if (!had_valid_mapping) {
        ...
        int pve_ptep_idx = 0;
        pv_status = pmap_enter_pv(pmap, pte_p, pai, options, lock_mode,
                                  &new_pve_p, &pve_ptep_idx);
      ...
      /**
       * PPL-protected MMIO mappings handed to IOMMUs must be PPL-writable and
       * cannot be removed, but in support of hibernation we allow temporary
       * read-only mappings of these pages to be created and later removed.  We
       * must therefore prevent an attacker from downgrading a a writable
       * mapping in order to allow it to be removed and remapped to something
       * else.
       */
      if (__improbable((wimg_bits & PP_ATTR_MONITOR) && pte_is_valid(spte) &&
                       (pte_to_xprr_perm(spte) != XPRR_KERN_RO_PERM) &&
                       (pte_to_xprr_perm(pte) == XPRR_KERN_RO_PERM))) {
        panic("%s: attempt to downgrade mapping of writable PPL-protected I/O "
              "address 0x%llx",
    ...
    if (committed) {
      if (ARM_PTE_IS_COMPRESSED(spte, pte_p)) {
        ...
        /* The old entry held a reference so drop the extra one that we took
         * above. */
        drop_refcnt = true;
      }
    }
  ...
  if (wiredcnt_updated &&
      (OSAddAtomic16(-1, (volatile int16_t *)wiredcnt) <= 0)) {
...
pmap_enter_options_addr(pmap_t pmap, vm_map_address_t v, pmap_paddr_t pa,
  ...
  do {
#if XNU_MONITOR
    kr = pmap_enter_options_ppl(pmap, v, pa, prot, fault_type, flags, wired,
                                options | PMAP_OPTIONS_NOWAIT);
#else
    kr = pmap_enter_options_internal(pmap, v, pa, prot, fault_type, flags,
                                     wired, options);
#endif

    if (kr == KERN_RESOURCE_SHORTAGE) {
#if XNU_MONITOR
      pmap_alloc_page_for_ppl(nowait_requested ? PMAP_PAGES_ALLOCATE_NOWAIT
                                               : 0);
#endif
      if (nowait_requested) {
        break;
 ...
 */
MARK_AS_PMAP_TEXT kern_return_t pmap_change_wiring_internal(pmap_t pmap,
                                                            vm_map_address_t v,
                                                            boolean_t wired) {
  pt_entry_t *pte_p;
  pmap_paddr_t pa;
  ...
  /*
   * Use volatile loads to prevent the compiler from collapsing references to
   * 'pa' back to loads of pte_p until we've grabbed the final PVH lock; PTE
   * contents may change during this time.
   */
  ...
  /* PTE checks must be performed after acquiring the PVH lock (if applicable
   * for the PA) */
  ...
  assertf(pte_is_valid(*pte_p), "invalid pte %p (=0x%llx)", pte_p,
          (uint64_t)*pte_p);
...
void pmap_change_wiring(pmap_t pmap, vm_map_address_t v, boolean_t wired) {
  /* This function is going to lock the pmap lock, so it'd better be
   * preemptible. */
  pmap_verify_preemptible();

  kern_return_t kr = KERN_FAILURE;
#if XNU_MONITOR
  /* Attempting to lock the pmap lock can abort when there is pending
   * preemption. Retry in such case. */
  do {
  ...
  if (kr != KERN_SUCCESS) {
    panic("%s: failed with return code %d; pmap: 0x%016llx, v: 0x%016llx, "
          "wired: %s",
...
MARK_AS_PMAP_TEXT void pmap_init_pte_page(pmap_t pmap, pt_entry_t *pte_p,
  ...
  // below barrier ensures previous updates to the page are visible to PTW
  // before it is linked to the PTE of previous level
...
MARK_AS_PMAP_TEXT static kern_return_t pmap_expand(pmap_t pmap,
  ...
  for (; ttlevel < level; ttlevel++) {
    ...
    if (pmap_ttne(pmap, ttlevel + 1, v) == PT_ENTRY_NULL) {
      pmap_unlock(pmap, PMAP_LOCK_SHARED);
      kern_return_t ret;
      while ((ret = pmap_tt_allocate(pmap, &tt_p, ttlevel + 1,
                                     ((options & PMAP_TT_ALLOCATE_NOWAIT)
        ...
        panic("%s: failed to allocate tt, "
              ...
              __FUNCTION__, pmap, (void *)v, options, level);
#else
        VM_PAGE_WAIT();
#endif
      }
...
void pmap_gc(void) {
   ...
   * active on a CPU or may contain wired mappings.  However, with the
   * relatively recent change to make pmap_page_reclaim() non-fatal in the event
   * that it doesn't find an eligible page, it may make sense to call that
   * function here.
   */
}
 ...
 * pushes any dirty data from the data cache for the same physical page
 */
void pmap_sync_page_data_phys(ppnum_t pp) { cache_sync_page(pp); }
...
phys_attribute_clear_with_flush_range(ppnum_t pn, unsigned int bits,
  ...
  if (__improbable(bits & PP_ATTR_PPL_OWNED_BITS)) {
    panic("%s: illegal request, "
          ...
          __FUNCTION__, pn, bits, options, arg, flush_range);
  }
#endif
  ...
  if (__improbable(
          (options & (PMAP_OPTIONS_FF_WIRED | PMAP_OPTIONS_FF_LOCKED)) != 0)) {
  ...
  if (options & PMAP_OPTIONS_CLEAR_WRITE) {
    ...
    pmap_page_protect_options_with_flush_range(
        pn, (VM_PROT_ALL & ~VM_PROT_WRITE), options, flush_range);
    /*
  ...
  if (arm_force_fast_fault_with_flush_range(pn, allow_mode, options,
                                            flush_range)) {
    ppattr_pa_clear_bits(pa, (pp_attr_t)bits);
  }
}
...
MARK_AS_PMAP_TEXT vm_map_address_t phys_attribute_clear_range_internal(
  ...
  if (flush_range.ptfr_flush_needed) {
    pmap_get_pt_ops(pmap)->flush_tlb_region_async(
        ...
        flush_range.ptfr_pmap, true, false);
    sync_tlb_flush();
  }
  return va;
}
...
#if XNU_MONITOR
boolean_t pmap_is_monitor(ppnum_t pn) {
...
pmap_switch_user_ttb(pmap_t pmap, pmap_cpu_data_t *cpu_data_ptr) {
  if (pmap != kernel_pmap) {
    ...
    if (nested_pmap != NULL) {
      ...
      /**
       * Obtain the full shared region bounds from the nested pmap.  If the
       * top-level pmap hasn't been fully nested yet, its bounds may not yet be
       * configured, or may be in the process of being configured on another
       * core.
       */
...
#if __ARM_MIXED_PAGE_SIZE__
    cpu_data_ptr->commpage_page_shift =
...
#if __ARM_MIXED_PAGE_SIZE__
  if ((pmap != kernel_pmap) &&
      (pmap_get_pt_attr(pmap)->pta_tcr_value != get_tcr())) {
    set_tcr(pmap_get_pt_attr(pmap)->pta_tcr_value);
  }
#endif /* __ARM_MIXED_PAGE_SIZE__ */

  if (pmap != kernel_pmap) {
    set_mmu_ttb((pmap->ttep & TTBR_BADDR_MASK) |
                (((uint64_t)pmap->hw_asid) << TTBR_ASID_SHIFT));
...
_Static_assert(ARM_PTE_COMPRESSED != ARM_PTE_WRITEABLE,
               "compressed bit aliases writeable");
_Static_assert(ARM_PTE_COMPRESSED != ARM_PTE_WIRED,
               "compressed bit aliases wired");
#endif
...
arm_force_fast_fault_with_flush_range(ppnum_t ppnum, vm_prot_t allow_mode,
  ...
  while ((pve_p != PV_ENTRY_NULL) || (pte_p != PT_ENTRY_NULL)) {
    ...
    bool wiredskip =
        pte_is_wired(*pte_p) && ((options & PMAP_OPTIONS_FF_WIRED) == 0);
    ...
    if (is_pte_xprr_protected(pmap, spte)) {
      if (pte_to_xprr_perm(spte) != pte_to_xprr_perm(tmplate)) {
        panic("%s: attempted to mutate an xPRR mapping pte_p=%p, pmap=%p, "
              "pv_h=%p, pve_p=%p, pte=0x%llx, tmplate=0x%llx, va=0x%llx, "
              "ppnum=0x%x, options=0x%x, allow_mode=0x%x",
              __FUNCTION__, pte_p, pmap, pv_h, pve_p, (unsigned long long)spte,
              (unsigned long long)tmplate, (unsigned long long)va, ppnum,
              options, allow_mode);
      }
    }
#endif /* MACH_ASSERT && XNU_MONITOR */
  ...
  while ((pve_p != PV_ENTRY_NULL) || (pte_p != PT_ENTRY_NULL)) {
    ...
    if (!(spte & ARM_PTE_FF_MARKER)) {
      ...
      /* No need to synchronize with the TLB flush; we're changing a SW-managed
       * bit */
      write_pte_fast(pte_p, spte);
      ++pass2_updated;
    }
...
MARK_AS_PMAP_TEXT boolean_t arm_force_fast_fault_internal(ppnum_t ppnum,
  ...
  return arm_force_fast_fault_with_flush_range(ppnum, allow_mode, options,
                                               NULL);
}
 ...
 *
 *	Function:
 *		Clear pending force fault for all mappings for this page based
 * on the observed fault type, update ref/modify bits.
 */
MARK_AS_PMAP_TEXT static boolean_t
arm_clear_fast_fault(ppnum_t ppnum, vm_prot_t fault_type, pt_entry_t *pte_p) {
...
arm_fast_fault_internal(pmap_t pmap, vm_map_address_t va, vm_prot_t fault_type,
  ...
  if (ptep != PT_ENTRY_NULL) {
    while (true) {
      ...
      if ((spte == ARM_PTE_EMPTY) || ARM_PTE_IS_COMPRESSED(spte, ptep)) {
        pmap_unlock(pmap, PMAP_LOCK_SHARED);
        return result;
      }
  ...
  if ((result != KERN_SUCCESS) &&
      ((ppattr_test_reffault(pai)) ||
       ((fault_type & VM_PROT_WRITE) && ppattr_test_modfault(pai)))) {
    /*
  ...
  if ((result == KERN_FAILURE) &&
      arm_clear_fast_fault((ppnum_t)atop(pa), fault_type, ptep)) {
    /*
...
void pmap_map_globals(void) {
  ...
  pte = pa_to_pte(ml_static_vtop((vm_offset_t)&lowGlo)) | AP_RONA | ARM_PTE_NX |
...
pmap_map_cpu_windows_copy_internal(ppnum_t pn, vm_prot_t prot,
  ...
  unsigned int cacheattr =
      (!pa_valid(ptoa(pn) & ARM_PTE_PAGE_MASK) ? pmap_cache_attributes(pn) : 0);
  need_strong_sync = ((cacheattr & PMAP_IO_RANGE_STRONG_SYNC) != 0);
#endif

#if XNU_MONITOR
#ifdef __ARM_COHERENT_IO__
  if (__improbable(pa_valid(ptoa(pn) & ARM_PTE_PAGE_MASK) &&
                   !pmap_ppl_disable)) {
    panic("%s: attempted to map a managed page, "
          "pn=%u, prot=0x%x, wimg_bits=0x%x",
          __FUNCTION__, pn, prot, wimg_bits);
  }
  if (__improbable((cacheattr & PP_ATTR_MONITOR) && (prot != VM_PROT_READ) &&
                   !pmap_ppl_disable)) {
    panic("%s: attempt to map PPL-protected I/O address 0x%llx as writable",
          __func__, (uint64_t)ptoa(pn));
  }
...
#error CPU copy windows are not properly supported with both the PPL and incoherent IO
  ...
  pte = pa_to_pte(ptoa(pn)) | ARM_PTE_TYPE_VALID | ARM_PTE_AF | ARM_PTE_NX |
        ARM_PTE_PNX;
#if __ARM_KERNEL_PROTECT__
  pte |= ARM_PTE_NG;
#endif /* __ARM_KERNEL_PROTECT__ */
  ...
  write_pte_fast(ptep, pte);
  /*
   * Invalidate tlb. Cover nested cpu_copywindow_vaddr usage with the
   * interrupted context in pmap_unmap_cpu_windows_copy() after clearing the pte
   * and before tlb invalidate.
   */
  FLUSH_PTE_STRONG();
  PMAP_UPDATE_TLBS(kernel_pmap, cpu_copywindow_vaddr,
                   cpu_copywindow_vaddr + PAGE_SIZE,
                   pmap_cpu_data->copywindow_strong_sync[i], true);
...
pmap_unmap_cpu_windows_copy_internal(unsigned int index) {
  ...
  PMAP_UPDATE_TLBS(kernel_pmap, cpu_copywindow_vaddr,
                   cpu_copywindow_vaddr + PAGE_SIZE,
                   pmap_cpu_data->copywindow_strong_sync[index], true);
}
...
MARK_AS_PMAP_TEXT void pmap_invoke_with_page(
    ppnum_t page_number, void *ctx,
 ...
 */
MARK_AS_PMAP_TEXT void pmap_hibernate_invoke(
    void *ctx, void (*callback)(void *ctx, uint64_t addr, uint64_t len)) {
 ...
 */
MARK_AS_PMAP_TEXT void pmap_set_ppl_hashed_flag(const pmap_paddr_t addr) {
 ...
 *
 * The HASHED flag needs to be cleared out between hibernation cycles because
 * the pv_head_table and pp_attr_table's might have been copied into the
 * hibernation image with the HASHED flag set on certain pages. It's important
 * to clear the HASHED flag to ensure that the enforcement of all PPL-owned
 * memory being hashed into the hibernation image can't be compromised across
 * hibernation cycles.
 */
MARK_AS_PMAP_TEXT void pmap_clear_ppl_hashed_flag_all(void) {
  ...
  for (int pai = 0; pai < last_index; ++pai) {
    ...
    /* Test for PPL-owned pages that have the HASHED flag set in its
     * pv_head_table entry. */
 ...
 * hibernation image.
 */
MARK_AS_PMAP_TEXT void pmap_check_ppl_hashed_flag_all(void) {
  const unsigned int last_index = pa_index(vm_last_phys);
  pv_entry_t **pv_h = NULL;
  ...
  for (int pai = 0; pai < last_index; ++pai) {
    ...
    if (!is_pmap_stack && (pp_attr_table[pai] & PP_ATTR_MONITOR) &&
        !(pvh_get_flags(pv_h) & PVH_FLAG_HASHED)) {
      panic("Found PPL-owned page that was not hashed into the hibernation "
            "image: pai %d",
            pai);
    }
  }
...
MARK_AS_PMAP_TEXT void pmap_set_nested_internal(pmap_t pmap) {
  validate_pmap_mutable(pmap);
  if (__improbable(!(os_atomic_cmpxchg(&pmap->type, PMAP_TYPE_USER,
                                       PMAP_TYPE_NESTED, seq_cst)))) {
  ...
  for (unsigned int i = 0; i <= max_cpu; ++i) {
    ...
    if (__improbable(os_atomic_load(&cpu_data->active_pmap, seq_cst) == pmap)) {
      panic("pmap %p: attempting to set nested while active on cpu %llu", pmap,
            (uint64_t)i);
    }
  }
#endif /* XNU_MONITOR */
  ...
  /**
   * Ensure that a (potentially concurrent) call to pmap_set_shared_region()
   * hasn't tried to give this pmap its own nested pmap.
   */
  if (__improbable(os_atomic_load(&pmap->nested_pmap, seq_cst) != NULL)) {
    panic("%s: attempt to nest pmap %p which already has a nested pmap",
          __func__, pmap);
  }
...
__mockable void pmap_set_nested(pmap_t pmap) {
#if XNU_MONITOR
...
MARK_AS_PMAP_TEXT static void pmap_trim_range(pmap_t pmap, addr64_t start,
  ...
  for (cur = adjusted_start; (cur < adjusted_end) && (cur >= adjusted_start);
    ...
    if ((tte_p != NULL) && tte_is_valid_table(*tte_p)) {
      ...
      if ((pmap->type == PMAP_TYPE_NESTED) &&
          (ptep_get_info(pte_p)->refcnt == 0)) {
        /* Deallocate for the nested map. */
        pmap_tte_deallocate(pmap, cur, cur + PAGE_SIZE, false, tte_p,
                            pt_attr_twig_level(pt_attr));
         ...
         */
        pmap_tte_remove(pmap, cur, cur + PAGE_SIZE, false, tte_p,
                        pt_attr_twig_level(pt_attr));
      } else {
  ...
  /* Remove empty L2 TTs. */
  adjusted_start = ((start + pt_attr_ln_offmask(pt_attr, PMAP_TT_L1_LEVEL)) &
  ...
  for (cur = adjusted_start; (cur < adjusted_end) && (cur >= adjusted_start);
    ...
    if (remove_tt1e) {
      pmap_tte_deallocate(pmap, cur, cur + PAGE_SIZE, false, tt1e_p,
                          PMAP_TT_L1_LEVEL);
    } else {
      pmap_unlock(pmap, PMAP_LOCK_EXCLUSIVE);
    }
 ...
 * the "grand", and trimming the "subord". Some of the steps can be skipped
...
pmap_trim_internal(pmap_t grand, pmap_t subord, addr64_t vstart, uint64_t size,
  ...
  if (__improbable((size != 0) && ((vstart < grand->nested_region_addr) ||
                                   (vend > (grand->nested_region_addr +
                                            grand->nested_region_size))))) {
  ...
  if (state == PMAP_TRIM_STATE_START) {
    ...
    /**
     * The "nested_no_bounds_ref_state" enum is set to
     * NESTED_NO_BOUNDS_REF_BEFORE_AND_AFTER by `pmap_nest()` if the subord is
     * nested into the grand when the bounds are not known yet. Therefore, if it
     * is NESTED_NO_BOUNDS_REF_NONE, either any nesting has not happened, or
     * trimming has been done, or nesting has been done with bounds known so the
     * "extra" region was not nested in the first place. Anyway, trimming is not
     * needed so we exit early with PMAP_TRIM_STATE_DONE.
     */
    if (grand->nested_no_bounds_ref_state == NESTED_NO_BOUNDS_REF_NONE) {
      assert(subord->nested_bounds_set);

      /* Nothing to do if the grand already has bounds set, otherwise inherit
       * from the subord. */
    ...
    /* If the subord doesn't have bounds set yet, compute them from vstart and a
     * non-zero size. */
    if ((!subord->nested_bounds_set) && size) {
      ...
      if (__improbable(os_add_overflow(subord->nested_region_true_end,
                                       adjust_offmask,
                                       &subord->nested_region_true_end))) {
    ...
    /* If the subord has bounds set now, let the grand inherit and continue to
     * trim. Otherwise, we are done. */
  ...
  if (state == PMAP_TRIM_STATE_GRAND_BEFORE) {
    ...
    if (__improbable(!os_atomic_cmpxchg(&grand->nested_no_bounds_ref_state,
                                        ...
                                        NESTED_NO_BOUNDS_REF_AFTER, release))) {
  ...
  if (state == PMAP_TRIM_STATE_GRAND_AFTER) {
    ...
    if (__improbable(!os_atomic_cmpxchg(
            &grand->nested_no_bounds_ref_state, NESTED_NO_BOUNDS_REF_AFTER,
            NESTED_NO_BOUNDS_REF_SUBORD, release))) {
      panic("%s: grand %p has unexpected no-bounds state %u", __func__, grand,
            (unsigned int)grand->nested_no_bounds_ref_state);
    }
  ...
  if (state == PMAP_TRIM_STATE_SUBORD) {
    /**
     * Since 'state' may be an attacker-controlled variable, we use
     * nested_no_bounds_ref_state to ensure that pmap_trim_subord (which may
     * free page tables from subord) can only be called once grand's nested
     * tables have been fully trimmed, and can only be called once for each
     * 'grand' pmap.  We use release ordering for the atomics above to ensure
     * that the state update is visible only once the preceding trim operation
     * is complete.  An attacker may be able to trigger multiple concurrent
     * trims on the same 'grand' region, but locking within pmap_trim_range()
     * should make that harmless (and all but one will ultimately panic due to a
     * failed atomic state CAS).  We use acquire ordering here to ensure that
     * modifications performed by pmap_trim_subord() can't be reordered ahead of
     * the state CAS.
     */
    if (__improbable(!os_atomic_cmpxchg(&grand->nested_no_bounds_ref_state,
                                        NESTED_NO_BOUNDS_REF_SUBORD,
                                        NESTED_NO_BOUNDS_REF_NONE, acquire))) {
      panic("%s: grand %p has unexpected no-bounds state %u", __func__, grand,
            (unsigned int)grand->nested_no_bounds_ref_state);
    }
...
MARK_AS_PMAP_TEXT static void pmap_trim_self(pmap_t pmap) {
  if ((pmap->nested_no_bounds_ref_state != NESTED_NO_BOUNDS_REF_NONE) &&
      pmap->nested_pmap) {
    ...
    vm_map_offset_t nested_region_true_start =
        pmap->nested_pmap->nested_region_true_start;
    vm_map_offset_t nested_region_true_end =
        pmap->nested_pmap->nested_region_true_end;
    ...
    if (nested_bounds_set) {
      ...
      pmap_trim_range(pmap, nested_region_true_end,
                      (pmap->nested_region_addr + pmap->nested_region_size));
    }
    /*
 ...
 */
MARK_AS_PMAP_TEXT static void pmap_trim_subord(pmap_t subord) {
...
#if HAS_APPLE_PAC
void *pmap_sign_user_ptr_internal(void *value, ptrauth_key key,
...
void *pmap_auth_user_ptr_internal(void *value, ptrauth_key key,
                                  uint64_t discriminator, uint64_t jop_key) {
  if ((key != ptrauth_key_asia) && (key != ptrauth_key_asda)) {
 ...
 */
MARK_AS_PMAP_TEXT kern_return_t pmap_set_shared_region_internal(pmap_t grand,
                                                                pmap_t subord,
                                                                addr64_t vstart,
                                                                uint64_t size) {
  addr64_t vend;
...
#if XNU_MONITOR
  /*
   * Ordering is important here.  validate_pmap() has already ensured subord is
   * a PPL-controlled pmap pointer, but it could have already been destroyed or
   * could be in the process of being destroyed.  If destruction is already
   * committed, then the check of ref_count below will cover us.  If destruction
   * is initiated during or after this call, then pmap_destroy() will catch the
   * non-zero nested_count.
   */
  ...
  if (os_atomic_load(&subord->nested_region_unnested_table_bitmap, acquire) ==
      NULL) {
    nested_region_unnested_table_bitmap_size =
        (size >> pt_attr_twig_shift(pt_attr)) / (sizeof(unsigned int) * NBBY) +
        1;

    /**
     * Each even-numbered entry in the unnested table bit stores the table's
     * unnesting state to be used by pmap_enter() in determining whether to set
     * the NG bit, while the subsequent odd-numbered entry stores the "unnest in
     * progress" indicator for the table, which is used by pmap_unnest() to
     * determine if an L3 table may not have been fully marked NG due to an
     * interrupted operation.
     */
    nested_region_unnested_table_bitmap_size <<= 1;

    if (__improbable((nested_region_unnested_table_bitmap_size > UINT_MAX))) {
      panic("%s: subord->nested_region_unnested_table_bitmap_size=%llu will "
            "truncate, "
            "grand=%p, subord=%p, vstart=0x%llx, size=%llx",
            __func__, nested_region_unnested_table_bitmap_size, grand, subord,
            vstart, size);
    }
    ...
    if (__improbable((nested_region_unnested_table_bitmap_size *
                      sizeof(unsigned int)) > PAGE_SIZE)) {
      panic("%s: nested_region_unnested_table_bitmap_size=%llu will not fit in "
            "a page, "
            "grand=%p, subord=%p, vstart=0x%llx, size=%llx",
            __FUNCTION__, nested_region_unnested_table_bitmap_size, grand,
            subord, vstart, size);
    }
    ...
    if (subord->nested_region_unnested_table_bitmap == NULL) {
      subord->nested_region_unnested_table_bitmap_size =
      ...
      /**
       * Use a store-release operation to ensure that the rest of the
       * subord->nested_region_* fields are initialized and visible before
       * setting the nested_region_unnested_table_bitmap field (which is used as
       * the flag to say that the rest are initialized).
       */
      os_atomic_store(&subord->nested_region_unnested_table_bitmap,
                      nested_region_unnested_table_bitmap, release);
      nested_region_unnested_table_bitmap = NULL;
    }
    pmap_unlock(subord, PMAP_LOCK_EXCLUSIVE);
  }

  if (__improbable(!os_atomic_cmpxchg(&grand->nested_pmap, PMAP_NULL, subord,
                                      seq_cst))) {
    panic("%s: attempt to nest pmap %p into pmap %p which already has a nested "
          "pmap %p",
...
#if XNU_MONITOR
    pmap_pages_free(
        kvtophys_nofail((vm_offset_t)nested_region_unnested_table_bitmap),
        PAGE_SIZE);
#else
...
__mockable void pmap_set_shared_region(pmap_t grand, pmap_t subord,
   ...
   * we have verified preemptibility. Therefore,
   * pmap_set_shared_region_internal() will wait for a page or a lock instead of
   * bailing out as in the PPL flavor.
   */
 ...
 * 1. Bookkeeping to ensure tracking structures for the nested region are set
 * up.
 * 2. Expansion of subord to ensure the required leaf-level page table pages for
 *    the mapping range are present in subord.
 * 3. Copying of twig-level TTEs from subord to grand, such that grand
 * ultimately contains pointers to subord's leaf-level pagetable pages for the
 * specified VA range.
 *
 ...
 * @param vrestart the twig-aligned starting address of the current call.  May
 * contain PMAP_NEST_GRAND in bit 0 to indicate the operation should skip to
 * step 3) above.
 * @param krp Should be initialized to KERN_SUCCESS by caller, will be set to
 *        KERN_RESOURCE_SHORTAGE on allocation failure.
 *
 * @return the virtual address at which to restart the operation, possibly
 * including PMAP_NEST_GRAND to indicate the phase at which to restart.  If
 *         (vstart + size) | PMAP_NEST_GRAND is returned, the operation
 * completed.
 */
MARK_AS_PMAP_TEXT vm_map_offset_t
pmap_nest_internal(pmap_t grand, pmap_t subord, addr64_t vstart, uint64_t size,
                   vm_map_offset_t vrestart, kern_return_t *krp) {
  kern_return_t kr = KERN_FAILURE;
  ...
  if (__improbable(((vrestart & ~PMAP_NEST_GRAND) > vend) ||
    ...
    panic("%s: vrestart 0x%llx is outside range [0x%llx, 0x%llx)", __func__,
          (unsigned long long)vrestart, (unsigned long long)vstart,
  ...
  if (__improbable(((size | vstart | (vrestart & ~PMAP_NEST_GRAND)) &
    ...
    panic(
        "pmap_nest() pmap %p unaligned nesting request 0x%llx, 0x%llx, 0x%llx",
  ...
  if (__improbable((subord->nested_region_addr + subord->nested_region_size) <
                   vend) ||
      (subord->nested_region_addr > vstart)) {
    panic("%s: attempt to nest [0x%llx, 0x%llx) in pmap %p outside nested pmap "
          "%p bounds [0x%llx, 0x%llx)\n",
          __func__, vstart, vend, grand, subord, subord->nested_region_addr,
          subord->nested_region_addr + subord->nested_region_size);
  }
  if (grand->nested_region_size == 0) {
    ...
    if (!subord->nested_bounds_set) {
       ...
       */
      if (__improbable(!os_atomic_cmpxchg(
              &grand->nested_no_bounds_ref_state, NESTED_NO_BOUNDS_REF_NONE,
              NESTED_NO_BOUNDS_REF_BEFORE_AND_AFTER, relaxed))) {
    ...
    /**
     * Ensure that we won't exceed the nested_region_unnested_table bitmap
     * bounds established in pmap_set_shared_region_internal().
     */
    if (__improbable((vstart < subord->nested_region_addr) ||
                     (vend > (subord->nested_region_addr +
                              subord->nested_region_size)))) {
      panic("%s: grand nested region (%p: [%p, %p)) will fall outside of "
            "subord nested region (%p: [%p, %p))",
            __func__, grand, (void *)vstart, (void *)vend, subord,
            (void *)subord->nested_region_addr,
    ...
    if (__improbable(grand->nested_region_addr > vstart)) {
      ...
      grand->nested_region_size =
          (mach_vm_offset_t)(vstart - grand->nested_region_addr + size);
    }
  }
  ...
  while (vaddr < true_end) {
    ...
    if (stte_p == PT_ENTRY_NULL || *stte_p == ARM_TTE_EMPTY) {
      pmap_unlock(subord, PMAP_LOCK_EXCLUSIVE);
      kr = pmap_expand(subord, vaddr, expand_options,
    ...
    ++ttecount;
    if (__improbable(
            !(ttecount % PMAP_DEFAULT_PREEMPTION_CHECK_PAGE_INTERVAL) &&
            pmap_pending_preemption())) {
      pmap_unlock(subord, PMAP_LOCK_EXCLUSIVE);
      kr = KERN_SUCCESS;
      goto done;
...
nest_grand:
  ...
  while (vaddr < true_end) {
    ...
    if (gtte_p == PT_ENTRY_NULL) {
      ...
      kr = pmap_expand(grand, vaddr, expand_options,
                       pt_attr_twig_level(pt_attr));
      if (!(grand_locked = pmap_lock_preempt(grand, PMAP_LOCK_EXCLUSIVE))) {
        if (kr == KERN_SUCCESS) {
          kr = KERN_ABORTED;
    ...
    if (__improbable(*gtte_p != ARM_TTE_EMPTY)) {
      panic("%s: attempting to overwrite non-empty TTE %p in pmap %p", __func__,
            gtte_p, grand);
    }
    /**
     ...
     * tables outside the trimmed range, as those tables may be immediately
     * freed which would lead to a dangling page table pointer in grand. Note
     * that pmap_trim() may concurrently update grand's bounds as we are making
     * these checks, but in that case pmap_trim_range() has not yet been called
     * on grand and will wait for us to drop grand's lock, so it should see any
     * TTEs we've nested here and clear them appropriately.
     */
    if (__probable((vaddr >= grand->nested_region_true_start) &&
                   (vaddr < grand->nested_region_true_end))) {
      stte_p = pmap_tte(subord, vaddr);
      if (__improbable(stte_p == PT_ENTRY_NULL)) {
        panic("%s: subord pmap %p not expanded at va 0x%llx", __func__, subord,
...
__mockable kern_return_t pmap_nest(pmap_t grand, pmap_t subord, addr64_t vstart,
                                   uint64_t size) {
  kern_return_t kr = KERN_SUCCESS;
  ...
  while (vaddr != (vend | PMAP_NEST_GRAND)) {
    ...
    if (kr == KERN_RESOURCE_SHORTAGE) {
      ...
      panic(
          "%s: failed to make forward progress from 0x%llx to 0x%llx at 0x%llx",
          __func__, (unsigned long long)vstart, (unsigned long long)vend,
          (unsigned long long)vaddr);
    }
    vlast = vaddr;
  }
 ...
 * @param vrestart the page-aligned starting address of the current call.  May
 * contain PMAP_NEST_GRAND in bit 0 to indicate the operation should skip to
 * step 2) above.
 ...
 * @return the virtual address at which to restart the operation, possibly
 * including PMAP_NEST_GRAND to indicate the phase at which to restart.  If
 *         (vaddr + size) | PMAP_NEST_GRAND is returned, the operation
 * completed.
 */
MARK_AS_PMAP_TEXT vm_map_offset_t
pmap_unnest_options_internal(pmap_t grand, addr64_t vaddr, uint64_t size,
                             vm_map_offset_t vrestart, unsigned int option) {
  ...
  if (__improbable(((vrestart & ~PMAP_NEST_GRAND) > vend) ||
    ...
    panic("%s: vrestart 0x%llx is outside range [0x%llx, 0x%llx)", __func__,
          (unsigned long long)vrestart, (unsigned long long)vaddr,
          (unsigned long long)vend);
  }
  ...
  if (((option & PMAP_UNNEST_CLEAN) == 0) && !(vrestart & PMAP_NEST_GRAND)) {
    ...
    for (current_index = start_index, addr = start; current_index < max_index;
      ...
      /*
       * If we've re-entered this function partway through unnesting a leaf
       * region, the 'unnest' bit will be set in the ASID bitmap, but we won't
       * have finished updating the run of PTEs and the adjacent "in-progress"
       * bit will be set.
       */
      if (!testbit(
              UNNEST_BIT(current_index),
              (int *)grand->nested_pmap->nested_region_unnested_table_bitmap) ||
          testbit(
              UNNEST_IN_PROGRESS_BIT(current_index),
              (int *)grand->nested_pmap->nested_region_unnested_table_bitmap)) {
        /*
         * Mark the 'twig' region as being unnested.  Every mapping entered
         * within the nested pmap in this region will now be marked non-global.
         * Do this before marking any of the PTEs within the region as
         * non-global to avoid the possibility of pmap_enter() subsequently
         * inserting a global mapping in the region, which could lead to a TLB
         * conflict if a non-global entry is later inserted for the same VA in a
         * pmap which has fully unnested this region.
         */
        setbit(UNNEST_BIT(current_index),
               (int *)grand->nested_pmap->nested_region_unnested_table_bitmap);
        setbit(UNNEST_IN_PROGRESS_BIT(current_index),
               (int *)grand->nested_pmap->nested_region_unnested_table_bitmap);
        for (cpte = bpte; (bpte != NULL) && (addr < vlim); cpte += PAGE_RATIO) {
          ...
          if (pte_is_valid(*cpte) && (!ARM_PTE_IS_COMPRESSED(*cpte, cpte))) {
            ...
            while (!managed) {
              ...
              if (pai == pa_index(pa)) {
                managed = TRUE;
                break; // Leave the PVH locked as we'll unlock it after we
                       // update the PTE
              }
              pvh_unlock(pai);
            }
          ...
          vrestart = addr;
          ++entry_count;
          if (__improbable(!(entry_count %
                             PMAP_DEFAULT_PREEMPTION_CHECK_PAGE_INTERVAL) &&
                           pmap_pending_preemption())) {
            goto unnest_subord_done;
          }
        }
        clrbit(UNNEST_IN_PROGRESS_BIT(current_index),
               (int *)grand->nested_pmap->nested_region_unnested_table_bitmap);
      }
      addr = vlim;
      vrestart = addr;
      ++entry_count;
      if (__improbable(
              !(entry_count % PMAP_DEFAULT_PREEMPTION_CHECK_PAGE_INTERVAL) &&
              pmap_pending_preemption())) {
        break;
      }
    }
  ...
  if (vrestart & PMAP_NEST_GRAND) {
    ...
    if (__improbable(addr & pt_attr_twig_offmask(pt_attr)) != 0x0ULL) {
      panic("%s: unaligned vrestart 0x%llx", __func__,
  ...
  while (addr < true_end) {
    ...
    /*
     * The nested pmap may have been trimmed before pmap_nest() completed for
     * grand, so it's possible that a region we're trying to unnest may not have
     * been nested in the first place.
     */
    if (tte_p != NULL) {
...
__mockable kern_return_t pmap_unnest_options(pmap_t grand, addr64_t vaddr,
                                             uint64_t size,
                                             unsigned int option) {
...
#else
    vrestart =
        pmap_unnest_options_internal(grand, vaddr, size, vrestart, option);
#endif
  }
...
#endif // __ARM_RANGE_TLBI__
static_assert(
    ARM64_FULL_TLB_FLUSH_THRESHOLD < (1ULL << (sizeof(ppnum_t) * 8)),
...
MARK_AS_PMAP_TEXT static void flush_mmu_tlb_full_asid_async(pmap_t pmap) {
...
void pmap_batch_set_cache_attributes(const unified_page_list_t *page_list,
  ...
  if (page_list->type != UNIFIED_PAGE_LIST_TYPE_UPL_ARRAY) {
    /**
     * For the UNIFIED_PAGE_LIST_TYPE_UPL_ARRAY case, we process the UPL in
     * batch below. In an ideal world we would just use these iterator functions
     * within pmap_batch_set_cache_attributes_internal() for all cases, but
     * since this is the PPL that means we'll need to take special care to
     * handle pending preemption and if necessary return the iterator position
     * out to this function and then re-enter
     * pmap_batch_set_cache_attributes_internal() at the same iterator position
     * in a secure manner.  Not impossible, but also not trivial, so unless
     * someone asks for this perf improvement on the PPL I'm going to take the
     * lazy approach here.
     */
...
#if XNU_MONITOR
    states = pmap_batch_set_cache_attributes_ppl(
        (volatile upl_page_info_t *)page_list->upl.upl_info, states,
        page_list->upl.upl_size, cacheattr);
#else  /* !XNU_MONITOR */
    states = pmap_batch_set_cache_attributes_internal(
        page_list->upl.upl_info, states, page_list->upl.upl_size, cacheattr);
#endif /* XNU_MONITOR */
  }
 ...
 *
 * In pass one, the pp_attr_table and the pte are updated for the pages in the
 * list. In pass two, TLB entries are flushed for each page in the list if
 * necessary. In pass three, caches are cleaned for each page in the list if
 * necessary.
 *
 * When running in PPL, this function may decide to return to the caller in
 * response to AST_URGENT.
 *
 * @param user_page_list List of pages to be updated.
 * @param states The state of the state machine. See definition of
 * batch_set_cache_attr_state_t.
...
pmap_batch_set_cache_attributes_internal(
    ...
    upl_page_info_array_t user_page_list,
#endif /* XNU_MONITOR */
    batch_set_cache_attr_state_t states, unsigned int page_cnt,
    unsigned int cacheattr) {
  ...
  if (!(page_index < page_cnt &&
    ...
    panic("%s: invalid input; page_index: %llu, page_cnt: %u, state: %llu",
          __func__, page_index, page_cnt, state);
  }

  if (state == PMAP_BATCH_SET_CACHE_ATTRIBUTES_UPDATE_PASS) {
    PMAP_TRACE(2, PMAP_CODE(PMAP__BATCH_UPDATE_CACHING), page_cnt, cacheattr,
               0xCECC0DE1, page_index);
    /* Update cache attributes of the pages until there's an urgent AST or it's
     * done. */
    while (page_index < page_cnt) {
      ...
      if (wimg_bits_new != wimg_bits_prev) {
        tlb_flush_pass_needed |=
            pmap_update_cache_attributes_locked(pn, cacheattr, false);
  ...
  if (state == PMAP_BATCH_SET_CACHE_ATTRIBUTES_CACHEFLUSH_PASS) {
    ...
    PMAP_TRACE(2, PMAP_CODE(PMAP__BATCH_UPDATE_CACHING), page_cnt, cacheattr,
               0xCECC0DE3, page_index);
#if !XNU_MONITOR
    /**
     * On non-PPL platforms, we disable preemption to ensure we are not
     * preempted in the state where DC by VA instructions remain enabled.
     */
    disable_preemption();
#endif /* !XNU_MONITOR */
...
MARK_AS_PMAP_TEXT static void
pmap_set_cache_attributes_priv(ppnum_t pn, unsigned int cacheattr,
  ...
  do {
    ...
    pp_attr_template = (pp_attr_current & ~PP_ATTR_WIMG_MASK) |
                       PP_ATTR_WIMG(cacheattr & (VM_WIMG_MASK));
 ...
 */
MARK_AS_PMAP_TEXT bool pmap_update_cache_attributes_locked(ppnum_t ppnum,
                                                           unsigned attributes,
                                                           bool perform_tlbi) {
  pmap_paddr_t phys = ptoa(ppnum);
  pv_entry_t *pve_p;
  ...
  PMAP_TRACE(2, PMAP_CODE(PMAP__UPDATE_CACHING) | DBG_FUNC_START, ppnum,
             attributes);
  ...
  PMAP_TRACE(2, PMAP_CODE(PMAP__UPDATE_CACHING) | DBG_FUNC_END, ppnum,
             attributes);
 ...
 */
static void pmap_set_commpage(pmap_t pmap) {
#if XNU_MONITOR
...
static void pmap_update_tt3e(pmap_t pmap, vm_address_t address,
                             tt_entry_t template) {
  tt_entry_t *ptep, pte;
...
void pmap_create_commpages(vm_map_address_t *kernel_data_addr,
  ...
  *user_text_addr = 0;
  ...
  kr =
      pmap_enter_addr(commpage_pmap_default, _COMM_PAGE64_BASE_ADDRESS, data_pa,
                      VM_PROT_READ, VM_PROT_NONE, VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  pmap_update_tt3e(commpage_pmap_default, _COMM_PAGE64_BASE_ADDRESS,
                   PMAP_COMM_PAGE_PTE_TEMPLATE);

  kr = pmap_enter_addr(commpage_pmap_default, _COMM_PAGE64_RO_ADDRESS,
                       ro_data_pa, VM_PROT_READ, VM_PROT_NONE,
                       VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  pmap_update_tt3e(commpage_pmap_default, _COMM_PAGE64_RO_ADDRESS,
                   PMAP_COMM_PAGE_PTE_TEMPLATE);
#if CONFIG_ARM_PFZ
  ...
  do {
    ...
    commpage_text_va =
        (_COMM_PAGE64_BASE_ADDRESS & ~pt_attr_leaf_index_mask(pt_attr));
    commpage_text_va |= (text_leaf_index << pt_attr_leaf_shift(pt_attr));
  } while (
      (commpage_text_va == _COMM_PAGE64_BASE_ADDRESS) ||
      (commpage_text_va == _COMM_PAGE64_RO_ADDRESS)); // Try again if we collide
                                                      // (should be unlikely)

  // Assert that this is empty
  __assert_only pt_entry_t *ptep =
      pmap_pte(commpage_pmap_default, commpage_text_va);
  assert(ptep != PT_ENTRY_NULL);
  assert(*ptep == ARM_TTE_EMPTY);

  // At this point, we've found the address we want to insert our comm page at
  kr = pmap_enter_addr(commpage_pmap_default, commpage_text_va, text_pa,
                       VM_PROT_READ | VM_PROT_EXECUTE, VM_PROT_NONE,
                       VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  // Mark it as global page R/X so that it doesn't get thrown out on tlb flush
  pmap_update_tt3e(commpage_pmap_default, commpage_text_va,
  ...
  *user_text_addr = commpage_text_va;
  ...
  kr =
      pmap_enter_addr(commpage_pmap_default, _COMM_PAGE32_BASE_ADDRESS, data_pa,
                      VM_PROT_READ, VM_PROT_NONE, VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  pmap_update_tt3e(commpage_pmap_default, _COMM_PAGE32_BASE_ADDRESS,
                   PMAP_COMM_PAGE_PTE_TEMPLATE);

  kr = pmap_enter_addr(commpage_pmap_default, _COMM_PAGE32_RO_ADDRESS,
                       ro_data_pa, VM_PROT_READ, VM_PROT_NONE,
                       VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  pmap_update_tt3e(commpage_pmap_default, _COMM_PAGE32_RO_ADDRESS,
                   PMAP_COMM_PAGE_PTE_TEMPLATE);
#if __ARM_MIXED_PAGE_SIZE__
  /**
   * To handle 4K tasks a new view/pmap of the shared page is needed. These are
   * a new set of page tables that point to the exact same 16K shared page as
  ...
  /* The user 64-bit mappings... */
  kr = pmap_enter_addr(commpage_pmap_4k, _COMM_PAGE64_BASE_ADDRESS, data_pa,
                       VM_PROT_READ, VM_PROT_NONE, VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  pmap_update_tt3e(commpage_pmap_4k, _COMM_PAGE64_BASE_ADDRESS,
                   PMAP_COMM_PAGE_PTE_TEMPLATE);

  kr = pmap_enter_addr(commpage_pmap_4k, _COMM_PAGE64_RO_ADDRESS, ro_data_pa,
                       VM_PROT_READ, VM_PROT_NONE, VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  pmap_update_tt3e(commpage_pmap_4k, _COMM_PAGE64_RO_ADDRESS,
                   PMAP_COMM_PAGE_PTE_TEMPLATE);

  /* ...and the user 32-bit mapping. */
  kr = pmap_enter_addr(commpage_pmap_4k, _COMM_PAGE32_BASE_ADDRESS, data_pa,
                       VM_PROT_READ, VM_PROT_NONE, VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  pmap_update_tt3e(commpage_pmap_4k, _COMM_PAGE32_BASE_ADDRESS,
                   PMAP_COMM_PAGE_PTE_TEMPLATE);

  kr = pmap_enter_addr(commpage_pmap_4k, _COMM_PAGE32_RO_ADDRESS, ro_data_pa,
                       VM_PROT_READ, VM_PROT_NONE, VM_WIMG_USE_DEFAULT, TRUE);
  assert(kr == KERN_SUCCESS);
  pmap_update_tt3e(commpage_pmap_4k, _COMM_PAGE32_RO_ADDRESS,
                   PMAP_COMM_PAGE_PTE_TEMPLATE);
#endif
...
#error                                                                         \
  ...
  if (pmap_page_size == 16384) {
    ...
    panic("No shared page pmap exists for the wanted page size: %llu",
          pmap_page_size);
  }
#endif /* __ARM_MIXED_PAGE_SIZE__ */
...
/* The following code assumes that commpage_pmap_default is a 16KB pmap. */
#error                                                                         \
    "pmap_unmap_commpage requires a 16KB default kernel page size when __ARM_MIXED_PAGE_SIZE__ is enabled"
#endif /* !__ARM_16K_PG__ */
...
#error We assume a single page.
  ...
  if (*ttep != ARM_TTE_EMPTY &&
      *ttep != *pmap_ttne(commpage_pmap, commpage_level, commpage_vaddr)) {
    panic("%s: Something other than commpage mapped in shared page slot?",
          __FUNCTION__);
  }
...
void pmap_insert_commpage(pmap_t pmap) {
  kern_return_t kr = KERN_FAILURE;
#if XNU_MONITOR
  do {
...
void pmap_pin_kernel_pages(vm_offset_t kva, size_t nbytes) {
  ...
  for (vm_offset_t ckva = trunc_page(kva); ckva < end;
       ckva = round_page(ckva + 1)) {
    pmap_paddr_t pa = kvtophys_nofail(ckva);
    unsigned int pai = pa_index(pa);
    pp_attr_t attr;
    if (__improbable(!pa_valid(pa))) {
      panic("%s(%s): attempt to pin mapping of non-managed page 0x%llx",
            __func__, (void *)kva, (uint64_t)pa);
    }
    pvh_lock(pai);
    if (__improbable(ckva == phystokv(pa))) {
      panic("%s(%p): attempt to pin static mapping for page 0x%llx", __func__,
            (void *)kva, (uint64_t)pa);
    }
    do {
      attr = pp_attr_table[pai] & ~PP_ATTR_NO_MONITOR;
      if (__improbable(attr & PP_ATTR_MONITOR)) {
        panic("%s(%p): physical page 0x%llx belongs to PPL", __func__,
              (void *)kva, (uint64_t)pa);
      }
    } while (!OSCompareAndSwap16(attr, attr | PP_ATTR_NO_MONITOR,
                                 &pp_attr_table[pai]));
    pvh_unlock(pai);
    if (__improbable(kvtophys_nofail(ckva) != pa)) {
      panic("%s(%p): VA no longer mapped to physical page 0x%llx", __func__,
            (void *)kva, (uint64_t)pa);
    }
  }
}
...
void pmap_unpin_kernel_pages(vm_offset_t kva, size_t nbytes) {
  ...
  for (vm_offset_t ckva = trunc_page(kva); ckva < end;
    ...
    if (!(pp_attr_table[pa_index(pa)] & PP_ATTR_NO_MONITOR)) {
      panic("%s(%p): physical page 0x%llx not pinned", __func__, (void *)kva,
            (uint64_t)pa);
    }
 ...
 * @param lockdown_flag Bit within PVH_FLAG_LOCKDOWN_MASK specifying the
 * lockdown reason
...
MARK_AS_PMAP_TEXT static void pmap_ppl_lockdown_page(vm_address_t kva,
                                                     uint64_t lockdown_flag,
                                                     bool ppl_writable) {
  pmap_ppl_lockdown_page_with_prot(kva, lockdown_flag, ppl_writable,
                                   VM_PROT_READ);
}
 ...
 * Effectively, it makes the physical page at kva immutable (see the
 * ppl_writable parameter for an exception to this).
 *
 * @param kva Valid address to any mapping of the physical page to lockdown.
 * @param lockdown_flag Bit within PVH_FLAG_LOCKDOWN_MASK specifying the
 * lockdown reason
 ...
 */
MARK_AS_PMAP_TEXT static void
pmap_ppl_lockdown_page_with_prot(vm_address_t kva, uint64_t lockdown_flag,
                                 bool ppl_writable, vm_prot_t prot) {
  ...
  if (__improbable(pvh_flags & (PVH_FLAG_LOCKDOWN_MASK | PVH_FLAG_EXEC))) {
    panic("%s: %#lx already locked down/executable (%#llx)", __func__, kva,
          (uint64_t)pvh_flags);
  }
  ...
  if (kvtophys_nofail(kva) != pa) {
    panic("%s: Physical address of mapping changed while setting LOCKDOWN "
          "flag %#lx %#llx",
          __func__, kva, (uint64_t)pa);
  }
}

/**
 * Helper for releasing a page from being locked down to the PPL, making it
 * writable to the kernel once again.
 *
 ...
 *
 * @param pai physical page index to release from lockdown.  PVH lock for this
 * page must be held.
 * @param lockdown_flag Bit within PVH_FLAG_LOCKDOWN_MASK specifying the
 * lockdown reason
 ...
 */
MARK_AS_PMAP_TEXT static void
pmap_ppl_unlockdown_page_locked(unsigned int pai, uint64_t lockdown_flag,
                                bool ppl_writable) {
  pvh_assert_locked(pai);
  pv_entry_t **pvh = pai_to_pvh(pai);
  const vm_offset_t pvh_flags = pvh_get_flags(pvh);
  ...
  if (__improbable(!(pvh_flags & lockdown_flag))) {
    panic("%s: unlockdown attempt on not locked down pai %d, type=0x%llx, PVH "
          "flags=0x%llx",
          __func__, pai, (unsigned long long)lockdown_flag,
          (unsigned long long)pvh_flags);
  }
 ...
 *                     deviation will result in a panic.
 */
MARK_AS_PMAP_TEXT static void pmap_ppl_unlockdown_page(vm_address_t kva,
                                                       uint64_t lockdown_flag,
                                                       bool ppl_writable) {
  const pmap_paddr_t pa = kvtophys_nofail(kva);
  const unsigned int pai = pa_index(pa);
...
void __unused pmap_pin_kernel_pages(vm_offset_t kva __unused,
                                    size_t nbytes __unused) {}

void __unused pmap_unpin_kernel_pages(vm_offset_t kva __unused,
                                      size_t nbytes __unused) {}

static void __unused pmap_ppl_lockdown_pages(vm_address_t kva __unused,
                                             vm_size_t size __unused,
                                             uint64_t lockdown_flag __unused,
                                             bool ppl_writable __unused) {}

static void __unused pmap_ppl_unlockdown_pages(vm_address_t kva __unused,
                                               vm_size_t size __unused,
                                               uint64_t lockdown_flag __unused,
                                               bool ppl_writable __unused) {}
...
MARK_AS_PMAP_TEXT static inline void __unused
...
MARK_AS_PMAP_TEXT static void pmap_ro_zone_lock_phy_page(const pmap_paddr_t pa,
                                                         vm_offset_t va,
                                                         vm_size_t size) {
  if (__improbable(trunc_page(va + size - 1) != trunc_page(va))) {
    panic("%s: va 0x%llx size 0x%llx crosses page boundary", __func__,
...
MARK_AS_PMAP_TEXT static void
pmap_ro_zone_unlock_phy_page(const pmap_paddr_t pa, vm_offset_t va __unused,
                             vm_size_t size __unused) {
  const unsigned int pai = pa_index(pa);
  pvh_unlock(pai);
}
...
MARK_AS_PMAP_TEXT mach_vm_size_t pmap_query_resident_internal(
  ...
  if (compressed_bytes_p) {
    pmap_pin_kernel_pages((vm_offset_t)compressed_bytes_p,
                          sizeof(*compressed_bytes_p));
    *compressed_bytes_p += compressed_bytes;
    pmap_unpin_kernel_pages((vm_offset_t)compressed_bytes_p,
                            sizeof(*compressed_bytes_p));
  }
...
void pmap_set_tpro(pmap_t pmap) {
#if XNU_MONITOR
...
void *pmap_claim_reserved_ppl_page(void) {
  // Unsupported
  return NULL;
}

void pmap_free_reserved_ppl_page(void __unused *kva) {
  // Unsupported
}
...
pmap_check_trust_cache_runtime_for_uuid_internal(
  ...
  TCReturn_t tc_ret = amfi->TrustCache.checkRuntimeForUUID(&ppl_trust_cache_rt,
                                                           check_uuid, NULL);
  ...
  if (tc_ret.error == kTCReturnSuccess) {
    ...
    ret = KERN_FAILURE;
    pmap_cs_log_error(
        "trust cache UUID check failed (TCReturn: 0x%02X | 0x%02X | %u)",
...
MARK_AS_PMAP_TEXT kern_return_t pmap_load_trust_cache_with_type_internal(
    TCType_t type, const vm_address_t pmap_img4_payload,
    const vm_size_t pmap_img4_payload_len, const vm_address_t img4_manifest,
    const vm_size_t img4_manifest_len, const vm_address_t img4_aux_manifest,
    const vm_size_t img4_aux_manifest_len) {
  kern_return_t ret = KERN_DENIED;
...
#if PMAP_CS_INCLUDE_CODE_SIGNING
  if (pmap_cs) {
    if ((type == kTCTypeStatic) || (type == kTCTypeEngineering) ||
        (type == kTCTypeLegacy)) {
    ...
    if (TCTypeConfig[type].entitlementValue != NULL) {
      const bool entitlement_satisfied = check_entitlement_pmap(
          NULL, "com.apple.private.pmap.load-trust-cache",
          TCTypeConfig[type].entitlementValue, false, true);
  ...
  if (ret != KERN_SUCCESS) {
    if (ret != KERN_RESOURCE_SHORTAGE) {
      pmap_cs_log_error(
          "unable to load trust cache (type: %u): unable to reserve page",
          type);
    }
    return ret;
  }
  ...
  /*
   * Lockdown the data passed in. The pmap image4 payload also contains the
   * trust cache data structure used by libTrustCache to manage the payload. We
   * need to be able to write to that data structure, so we keep the payload PPL
   * writable.
   */
  ...
  trust_cache =
      (TrustCache_t *)phystokv(kvtophys_nofail((vm_offset_t)trust_cache));

  /* Calculate the correct length of the img4 payload */
  if (os_sub_overflow(pmap_img4_payload_len, sizeof(pmap_img4_payload_t),
                      &img4_payload_len)) {
  ...
  TCReturn_t tc_ret = amfi->TrustCache.load(
      &ppl_trust_cache_rt, type, trust_cache,
  ...
  if (tc_ret.error == kTCReturnSuccess) {
    ...
    if (tc_ret.error == kTCReturnDuplicate) {
      ...
      pmap_cs_log_error(
          "unable to load trust cache (TCReturn: 0x%02X | 0x%02X | %u)",
          tc_ret.component, tc_ret.error, tc_ret.uniqueError);
...
kern_return_t pmap_load_trust_cache_with_type(
    TCType_t type, const vm_address_t pmap_img4_payload,
    const vm_size_t pmap_img4_payload_len, const vm_address_t img4_manifest,
    const vm_size_t img4_manifest_len, const vm_address_t img4_aux_manifest,
    const vm_size_t img4_aux_manifest_len) {
  ...
  ret = pmap_load_trust_cache_with_type_ppl(
      type, pmap_img4_payload, pmap_img4_payload_len, img4_manifest,
      img4_manifest_len, img4_aux_manifest, img4_aux_manifest_len);
...
MARK_AS_PMAP_TEXT kern_return_t pmap_query_trust_cache_safe(
    TCQueryType_t query_type, const uint8_t cdhash[kTCEntryHashSize],
    TrustCacheQueryToken_t *query_token) {
  kern_return_t ret = KERN_NOT_FOUND;

  /* Validate the query type preemptively */
  if (query_type >= kTCQueryTypeTotal) {
    pmap_cs_log_error("unable to query trust cache: invalid query type: %u",
                      query_type);
    return KERN_INVALID_ARGUMENT;
  }
  ...
  TCReturn_t tc_ret = amfi->TrustCache.query(&ppl_trust_cache_rt, query_type,
                                             cdhash, query_token);
  ...
  if (tc_ret.error == kTCReturnSuccess) {
    ...
    ret = KERN_FAILURE;
    pmap_cs_log_error(
        "trust cache query failed (TCReturn: 0x%02X | 0x%02X | %u)",
        tc_ret.component, tc_ret.error, tc_ret.uniqueError);
  }
...
MARK_AS_PMAP_TEXT kern_return_t pmap_query_trust_cache_internal(
    TCQueryType_t query_type, const uint8_t cdhash[kTCEntryHashSize],
    TrustCacheQueryToken_t *query_token) {
  kern_return_t ret = KERN_NOT_FOUND;
  ...
  if (query_token != NULL) {
    ...
    memcpy((void *)query_token, (void *)&query_token_safe,
           sizeof(*query_token));
...
kern_return_t pmap_query_trust_cache(TCQueryType_t query_type,
                                     const uint8_t cdhash[kTCEntryHashSize],
                                     TrustCacheQueryToken_t *query_token) {
  kern_return_t ret = KERN_NOT_FOUND;

  ret = pmap_query_trust_cache_ppl(query_type, cdhash, query_token);
...
MARK_AS_PMAP_TEXT void pmap_toggle_developer_mode_internal(bool state) {
#if PMAP_CS_INCLUDE_INTERNAL_CODE
  /*
   * On internal builds, we may call into the PPL twice in order to enable
   * developer mode during early boot and during data migration. The latter does
   * not happen for non-internal builds, and thus those only need to support a
   * single transition to enabling developer mode.
   */
  ...
  /*
   * We don't really care if the state is false -- in that case, the transition
   * can happen as many times as needed. However, we still need to increment
   * whenever we set the state as such. This is partly because we need to track
   * whether we have actually resolved the state or not, and also because we
   * expect developer mode to only be enabled during the first or second
   * (internal-only) call into this function.
   */
...
void pmap_toggle_developer_mode(bool state) {
...
MARK_AS_PMAP_TEXT static errno_t _pmap_image4_monitor_trap_set_release_type(
    const pmap_image4_dispatch_t *dispatch, const void *input_data) {
  /*
  ...
  return dispatch->handler(dispatch->selector, &input, sizeof(input), NULL,
                           NULL);
}

MARK_AS_PMAP_TEXT static errno_t
_pmap_image4_monitor_trap_nonce_set(const pmap_image4_dispatch_t *dispatch,
...
MARK_AS_PMAP_TEXT static errno_t
_pmap_image4_monitor_trap_nonce_roll(const pmap_image4_dispatch_t *dispatch,
                                     const void *input_data) {
...
MARK_AS_PMAP_TEXT static errno_t
_pmap_image4_monitor_trap_image_activate(const pmap_image4_dispatch_t *dispatch,
                                         const void *input_data) {
  /*
  ...
  pmap_cs_assert_addr(input.csmx_payload, round_page(input.csmx_payload_len),
                      false, false);

  /* Validate the manifest region */
  pmap_cs_assert_addr(input.csmx_manifest, round_page(input.csmx_manifest_len),
                      false, false);

  /* Lockdown the payload region */
  pmap_cs_lockdown_pages(input.csmx_payload, round_page(input.csmx_payload_len),
                         false);

  /* Lockdown the manifest region */
  pmap_cs_lockdown_pages(input.csmx_manifest,
                         round_page(input.csmx_manifest_len), false);

  /* Dispatch the handler */
  errno_t err =
      dispatch->handler(dispatch->selector, &input, sizeof(input), NULL, NULL);

  /*
   * Image activation always returns the manifest back to the kernel since it
   * isn't needed once the evaluation of the image has been completed. The
   * payload must remain owned by the monitor if the activation was successful.
   */
  if (err != 0) {
    /* Unlock the payload region */
    pmap_cs_unlockdown_pages(input.csmx_payload,
                             round_page(input.csmx_payload_len), false);
  }

  /* Unlock the manifest region */
  pmap_cs_unlockdown_pages(input.csmx_manifest,
                           round_page(input.csmx_manifest_len), false);

  return err;
}

MARK_AS_PMAP_TEXT static errno_t _pmap_image4_monitor_trap_passthrough(
    __unused const pmap_image4_dispatch_t *dispatch,
    __unused const void *input_data, __unused size_t input_size) {
#if DEVELOPMENT || DEBUG || KASAN
  return dispatch->handler(dispatch->selector, input_data, input_size, NULL,
                           NULL);
#else
  pmap_cs_log_error("%llu: image4 dispatch: pass-through not supported",
                    selector);
  return ENOSYS;
#endif
}

MARK_AS_PMAP_TEXT errno_t pmap_image4_monitor_trap_internal(
    image4_cs_trap_t selector, const void *input_data, size_t input_size) {
  kern_return_t ret = KERN_DENIED;
  errno_t err = EPERM;
  ...
  if (input_size != image4_cs_trap_vector_size(selector)) {
    pmap_cs_log_error("%llu: image4 dispatch: invalid input: %lu ", selector,
                      input_size);
    return EINVAL;
  }
  ...
  pmap_image4_dispatch_t dispatch = {.selector = selector, .handler = handler};
  ...
  default:
    err = _pmap_image4_monitor_trap_passthrough(&dispatch, input_data,
                                                input_size);
    break;
  }
...
errno_t pmap_image4_monitor_trap(image4_cs_trap_t selector,
...
static int pmap_cs_profiles_rbtree_compare(void *profile0, void *profile1) {
...
MARK_AS_PMAP_DATA static RB_HEAD(pmap_cs_profiles_rbtree,
                                 _pmap_cs_profile) pmap_cs_registered_profiles;

RB_PROTOTYPE(pmap_cs_profiles_rbtree, _pmap_cs_profile, link,
             pmap_cs_profiles_rbtree_compare);
RB_GENERATE(pmap_cs_profiles_rbtree, _pmap_cs_profile, link,
            pmap_cs_profiles_rbtree_compare);

/* Lock for the profile red-black tree */
MARK_AS_PMAP_DATA decl_lck_rw_data(, pmap_cs_profiles_rbtree_lock);

void pmap_initialize_provisioning_profiles(void) {
...
static bool pmap_is_testflight_profile(pmap_cs_profile_t *profile_obj) {
  ...
  memcpy(query[0].parameters.stringParameter.data, entitlement_name,
         entitlement_length);
  ...
  CEError_t ce_err = amfi->CoreEntitlements.ContextQuery(
      profile_obj->entitlements_ctx, query, 2);
...
static bool pmap_is_development_profile(pmap_cs_profile_t *profile_obj) {
  /* Check for UPP */
  const der_vm_context_t upp_ctx = amfi->CoreEntitlements.der_vm_execute(
      *profile_obj->profile_ctx, CESelectDictValue("ProvisionsAllDevices"));
...
pmap_initialize_profile_entitlements(pmap_cs_profile_t *profile_obj) {
  const der_vm_context_t entitlements_der_ctx =
      amfi->CoreEntitlements.der_vm_execute(*profile_obj->profile_ctx,
                                            CESelectDictValue("Entitlements"));

  if (amfi->CoreEntitlements.der_vm_context_is_valid(entitlements_der_ctx) ==
      false) {
    memset(&profile_obj->entitlements_ctx_storage, 0,
           sizeof(struct CEQueryContext));
  ...
  CEError_t ce_err = amfi->CoreEntitlements.Validate(
      pmap_cs_core_entitlements_runtime, &ce_result, der_start, der_end);
  if (ce_err != amfi->CoreEntitlements.kNoError) {
  ...
  ce_err = amfi->CoreEntitlements.AcquireUnmanagedContext(
      pmap_cs_core_entitlements_runtime, ce_result, &query_ctx);
  if (ce_err != amfi->CoreEntitlements.kNoError) {
...
kern_return_t
pmap_register_provisioning_profile_internal(const vm_address_t payload_addr,
                                            const vm_size_t payload_size) {
  kern_return_t ret = KERN_DENIED;
  ...
  if (ret != KERN_SUCCESS) {
    if (ret != KERN_RESOURCE_SHORTAGE) {
      pmap_cs_log_error(
          "unable to register profile: unable to reserve page: %d", ret);
    }
    return ret;
  }
  ...
  /*
   * Lockdown the data passed in. The pmap profile payload also contains the
   * profile data structure used by the PPL to manage the payload. We need to be
   * able to write to that data structure, so we keep the payload PPL writable.
   */
  ...
  if (os_sub_overflow(payload_size, sizeof(*profile_payload),
                      &max_profile_blob_size)) {
    ...
    panic("PMAP_CS: overflow on the profile_blob_size: %lu",
          profile_payload->profile_blob_size);
  }
  ...
  int ct_result = coretrust->CTEvaluateProvisioningProfile(
      ...
      allow_development_root_cert, &profile_content, &profile_content_length);

  /* Release the PPL page allocated for CoreCrypto */
  pmap_release_reserved_ppl_page();

  if (ct_result != 0) {
    panic("PMAP_CS: profile does not validate through CoreTrust: %d",
          ct_result);
  ...
  der_vm_context_t profile_ctx_storage =
      amfi->CoreEntitlements.der_vm_context_create(
          pmap_cs_core_entitlements_runtime, CCDER_CONSTRUCTED_SET, false,
          profile_content, profile_content + profile_content_length);
  if (amfi->CoreEntitlements.der_vm_context_is_valid(profile_ctx_storage) ==
      false) {
    panic(
        "PMAP_CS: unable to create a CoreEntitlements context for the profile");
  }

  /* Acquire a writable version of the profile data structure */
  profile_obj = &profile_payload->profile_obj_storage;
  profile_obj =
      (pmap_cs_profile_t *)phystokv(kvtophys_nofail((vm_offset_t)profile_obj));
  ...
  if ((ret != KERN_SUCCESS) && (ret != KERN_NOT_FOUND)) {
    panic("PMAP_CS: fatal error while setting up profile entitlements: %d",
          ret);
  }
  ...
  if (RB_INSERT(pmap_cs_profiles_rbtree, &pmap_cs_registered_profiles,
                profile_obj) != NULL) {
    panic("PMAP_CS: Anomaly, profile already exists in the tree: %p",
          profile_obj);
  }
...
pmap_register_provisioning_profile(const vm_address_t payload_addr,
  ...
  ret = pmap_register_provisioning_profile_ppl(payload_addr, payload_size);
...
kern_return_t
pmap_unregister_provisioning_profile_internal(pmap_cs_profile_t *profile_obj) {
  kern_return_t ret = KERN_DENIED;

  /* Lock the red-black tree exclusively */
  lck_rw_lock_exclusive(&pmap_cs_profiles_rbtree_lock);

  if (RB_FIND(pmap_cs_profiles_rbtree, &pmap_cs_registered_profiles,
              profile_obj) == NULL) {
    panic("PMAP_CS: unregistering an unknown profile: %p", profile_obj);
  }

  uint32_t reference_count =
      os_atomic_load(&profile_obj->reference_count, acquire);
...
kern_return_t
pmap_associate_provisioning_profile_internal(pmap_cs_code_directory_t *cd_entry,
                                             pmap_cs_profile_t *profile_obj) {
  kern_return_t ret = KERN_DENIED;
  ...
  if (cd_entry->trust != PMAP_CS_UNTRUSTED) {
    pmap_cs_log_error(
        "disallowing profile association with verified signature");
    goto exit;
  } else if (cd_entry->profile_obj != NULL) {
    pmap_cs_log_error(
        "disallowing multiple profile associations with signature");
    goto exit;
  }
  ...
  if (RB_FIND(pmap_cs_profiles_rbtree, &pmap_cs_registered_profiles,
    ...
    panic("PMAP_CS: attempted association with unverified profile: %p",
          profile_obj);
  }
  ...
  uint32_t reference_count =
      os_atomic_add(&profile_obj->reference_count, 1, relaxed);
  if (reference_count == 0) {
  ...
  pmap_cs_log_info("associated profile %p with signature %p", profile_obj,
                   cd_entry);
  ret = KERN_SUCCESS;
...
kern_return_t pmap_disassociate_provisioning_profile_internal(
    pmap_cs_code_directory_t *cd_entry) {
  pmap_cs_profile_t *profile_obj = NULL;
  kern_return_t ret = KERN_DENIED;
...
exit:
  ...
  if (ret == KERN_SUCCESS) {
    ...
    uint32_t reference_count =
        os_atomic_sub(&profile_obj->reference_count, 1, release);
    if (reference_count == UINT32_MAX) {
      panic("PMAP_CS: underflow on reference count for profile: %p",
            profile_obj);
    }
    pmap_cs_log_info("disassociated profile %p from signature %p", profile_obj,
                     cd_entry);
  }
  return ret;
}
...
kern_return_t
pmap_associate_kernel_entitlements_internal(pmap_cs_code_directory_t *cd_entry,
                                            const void *kernel_entitlements) {
  kern_return_t ret = KERN_DENIED;

  if (kernel_entitlements == NULL) {
    panic("PMAP_CS: attempted to associate NULL kernel entitlements: %p",
          cd_entry);
  }
...
kern_return_t
pmap_resolve_kernel_entitlements_internal(pmap_t pmap,
                                          const void **kernel_entitlements) {
  ...
  if (kernel_entitlements != NULL) {
    pmap_pin_kernel_pages((vm_offset_t)kernel_entitlements,
                          sizeof(*kernel_entitlements));
    *kernel_entitlements = entitlements;
    pmap_unpin_kernel_pages((vm_offset_t)kernel_entitlements,
                            sizeof(*kernel_entitlements));
  }
...
kern_return_t
pmap_accelerate_entitlements_internal(pmap_cs_code_directory_t *cd_entry) {
  ...
  /*
   * Only reconstituted code signatures can be accelerated. This is only a
   * policy decision we make since this allows us to re-use any unused space
   * within the locked down code signature region. There is also a decent bit of
   * validation within the reconstitution function to ensure blobs are ordered
   * and do not contain any padding around them which can cause issues here.
   *
  ...
  ce_err = CoreEntitlements->IndexSizeForContext(cd_entry->ce_ctx,
                                                 &acceleration_length);
  if (ce_err != CoreEntitlements->kNoError) {
    ...
    panic("PMAP_CS: unable to gauge index size for entitlements acceleration: "
          "%p | %s",
  ...
  required_length =
      acceleration_length + sizeof(pmap_cs_ce_acceleration_buffer_t);
  ...
  /*
   * First we'll check if the code signature has enough space within the locked
   * down region of memory to hold the buffer. If not, then we'll see if we can
   * bucket allocate the buffer, and if not, we'll just allocate an entire page
   * from the free list.
   *
   * When we're storing the buffer within the code signature, we also need to
   * make sure we account for alignment of the buffer.
   */
  const vm_address_t align_mask = sizeof(void *) - 1;
  size_t required_length_within_sig = required_length + align_mask;

  if ((cd_entry->superblob_size - signature_length) >=
      required_length_within_sig) {
    vm_address_t aligned_buf =
        (vm_address_t)cd_entry->superblob + signature_length;
    ...
    assert((aligned_buf & PAGE_MASK) ==
           ((vm_address_t)acceleration_buf & PAGE_MASK));

    acceleration_buf->allocated = false;
    pmap_cs_log_debug("[alloc] acceleration buffer thru signature: %p",
                      acceleration_buf);
  } else {
    if (required_length <= pmap_cs_blob_limit) {
      ...
      pmap_cs_log_debug("[alloc] acceleration buffer thru bucket: %p",
                        acceleration_buf);
    } else {
      pmap_paddr_t phys_addr = 0;
      ret = pmap_pages_alloc_zeroed(&phys_addr, PAGE_SIZE,
                                    PMAP_PAGES_ALLOCATE_NOWAIT);
      if (ret != KERN_SUCCESS) {
        goto out;
      }
      acceleration_buf = (void *)phystokv(phys_addr);
      pmap_cs_log_debug("[alloc] acceleration buffer thru page: %p",
                        acceleration_buf);
    }
  ...
  if (ce_err != CoreEntitlements->kNoError) {
    panic("PMAP_CS: unable to accelerate entitlements: %p | %s", cd_entry,
          CoreEntitlements->GetErrorString(ce_err));
  } else if (CoreEntitlements->ContextIsAccelerated(cd_entry->ce_ctx) != true) {
    panic("PMAP_CS: entitlements not marked as accelerated: %p", cd_entry);
  }

  /*
   * The global acceleration buffer lock is unlocked by the allocation function
   * itself (pmap_cs_alloc_index) so we don't need to unlock it here. Moreover,
   * we cannot add an assert that the lock is unlocked here since another thread
   * could have acquired it by now.
   */
  ret = KERN_SUCCESS;
...
kern_return_t pmap_accelerate_entitlements(pmap_cs_code_directory_t *cd_entry) {
  kern_return_t ret = KERN_DENIED;
...
MARK_AS_PMAP_TEXT bool pmap_lookup_in_loaded_trust_caches_internal(
    const uint8_t cdhash[CS_CDHASH_LEN]) {
  ...
  kr = pmap_query_trust_cache_safe(kTCQueryTypeLoadable, cdhash_safe, NULL);
#else
  kr = query_trust_cache(kTCQueryTypeLoadable, cdhash, NULL);
#endif
...
bool pmap_lookup_in_loaded_trust_caches(const uint8_t cdhash[CS_CDHASH_LEN]) {
#if XNU_MONITOR
...
MARK_AS_PMAP_TEXT uint32_t pmap_lookup_in_static_trust_cache_internal(
    const uint8_t cdhash[CS_CDHASH_LEN]) {
  ...
  kr = pmap_query_trust_cache_safe(kTCQueryTypeStatic, cdhash_safe,
                                   &query_token);
#else
...
MARK_AS_PMAP_TEXT void pmap_set_compilation_service_cdhash_internal(
    const uint8_t cdhash[CS_CDHASH_LEN]) {
  ...
  pmap_cs_log_info("Added Compilation Service CDHash through the PPL: 0x%02X "
                   "0x%02X 0x%02X 0x%02X",
                   cdhash[0], cdhash[1], cdhash[2], cdhash[4]);
}

MARK_AS_PMAP_TEXT bool pmap_match_compilation_service_cdhash_internal(
    const uint8_t cdhash[CS_CDHASH_LEN]) {
  bool match = false;
...
void pmap_set_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
#if XNU_MONITOR
...
bool pmap_match_compilation_service_cdhash(
    const uint8_t cdhash[CS_CDHASH_LEN]) {
#if XNU_MONITOR
...
MARK_AS_PMAP_DATA static uint8_t
    pmap_local_signing_public_key[PMAP_CS_LOCAL_SIGNING_KEY_SIZE] = {0};

MARK_AS_PMAP_TEXT void pmap_set_local_signing_public_key_internal(
    const uint8_t public_key[PMAP_CS_LOCAL_SIGNING_KEY_SIZE]) {
  bool key_set = false;

  /*
   * os_atomic_cmpxchg returns true in case the exchange was successful. For us,
   * a successful exchange means that the local signing public key has _not_
   * been set. In case the key has been set, we panic as we would never expect
   * the kernel to attempt to set the key more than once.
   */
  key_set = !os_atomic_cmpxchg(&pmap_local_signing_public_key_set, false, true,
                               relaxed);
  ...
  memcpy(pmap_local_signing_public_key, public_key,
         PMAP_CS_LOCAL_SIGNING_KEY_SIZE);
...
uint8_t *pmap_get_local_signing_public_key(void) {
...
/*
 * Locally signed applications need to be explicitly authorized by an entitled
 * application before we allow them to run.
 */
...
MARK_AS_PMAP_TEXT void
pmap_unrestrict_local_signing_internal(const uint8_t cdhash[CS_CDHASH_LEN]) {
  ...
  pmap_cs_log_debug(
      "unrestricted local signing for CDHash: 0x%02X%02X%02X%02X%02X...",
      cdhash[0], cdhash[1], cdhash[2], cdhash[3], cdhash[4]);
}

void pmap_unrestrict_local_signing(const uint8_t cdhash[CS_CDHASH_LEN]) {
#if XNU_MONITOR
...
#if PMAP_CS
MARK_AS_PMAP_TEXT static void pmap_restrict_local_signing(void) {
  pmap_simple_lock(&pmap_local_signing_cdhash_lock);
...
pmap_local_signing_restricted(const uint8_t cdhash[CS_CDHASH_LEN]) {
  pmap_simple_lock(&pmap_local_signing_cdhash_lock);
  int ret = memcmp(pmap_local_signing_cdhash, cdhash,
                   sizeof(pmap_local_signing_cdhash));
  pmap_simple_unlock(&pmap_local_signing_cdhash_lock);
...
MARK_AS_PMAP_TEXT void pmap_nop_internal(pmap_t pmap __unused) {
  validate_pmap_mutable(pmap);
}

void pmap_nop(pmap_t pmap) {
#if XNU_MONITOR
  pmap_nop_ppl(pmap);
#else
...
static int pmap_test_test_config(unsigned int flags) {
  ...
  pmap_enter_addr(pmap, va_base, pa, VM_PROT_EXECUTE, VM_PROT_EXECUTE, 0,
                  false);
 ...
 */
kern_return_t pmap_test_text_corruption(pmap_paddr_t pa) {
#if XNU_MONITOR
