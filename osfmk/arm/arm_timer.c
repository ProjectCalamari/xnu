 ...
 * over them
 */
void timer_intr(__unused int inuser, __unused uint64_t iaddr) {
  ...
  if ((cpu_data_ptr->idle_timer_deadline > 0) &&
      (cpu_data_ptr->idle_timer_deadline <= abstime)) {
...
boolean_t timer_resort_threshold(__unused uint64_t skew) { return FALSE; }
