...
typedef struct {
  uint32_t LineSize : 3, /* 2:0 - Number of words in cache line */
...
__private_extern__ void do_cpuid(void) {
...
arm_cpu_info_t *cpuid_info(void) { return &cpuid_cpu_info; }
...
int cpuid_get_cpusubfamily(void) {
...
void do_debugid(void) { machine_do_debugid(); }

arm_debug_info_t *arm_debug_info(void) { return machine_arm_debug_info(); }

void do_mvfpid(void) { return machine_do_mvfpid(); }

arm_mvfp_info_t *arm_mvfp_info(void) { return machine_arm_mvfp_info(); }

void do_cacheid(void) {
  ...
  cluster_type_t cluster_type =
      ml_get_topology_info()->cpus[ml_get_cpu_number_local()].cluster_type;
  uint8_t prev_cpuid_cache_info_bitmap = os_atomic_or_orig(
      &cpuid_cache_info_bitmap, (uint8_t)(1 << cluster_type), acq_rel);
  ...
  cpuid_cache_info_p->c_unified =
      (arm_cache_clidr_info.bits.Ctype1 == 0x4) ? 1 : 0;
  ...
  cpuid_cache_info_p->c_linesz =
      4 * (1 << (arm_cache_ccsidr_info.bits.LineSize + 2));
  cpuid_cache_info_p->c_assoc = (arm_cache_ccsidr_info.bits.Assoc + 1);

  /* I cache size */
  cpuid_cache_info_p->c_isize = (arm_cache_ccsidr_info.bits.NumSets + 1) *
                                cpuid_cache_info_p->c_linesz *
                                cpuid_cache_info_p->c_assoc;

  /* D cache size */
  cpuid_cache_info_p->c_dsize = (arm_cache_ccsidr_info.bits.NumSets + 1) *
                                cpuid_cache_info_p->c_linesz *
                                cpuid_cache_info_p->c_assoc;

  if ((arm_cache_clidr_info.bits.Ctype3 == 0x4) ||
      (arm_cache_clidr_info.bits.Ctype2 == 0x4) ||
      (arm_cache_clidr_info.bits.Ctype2 == 0x2)) {
    if (arm_cache_clidr_info.bits.Ctype3 == 0x4) {
    ...
    cpuid_cache_info_p->c_linesz =
        4 * (1 << (arm_cache_ccsidr_info.bits.LineSize + 2));
    cpuid_cache_info_p->c_assoc = (arm_cache_ccsidr_info.bits.Assoc + 1);
    cpuid_cache_info_p->c_l2size = (arm_cache_ccsidr_info.bits.NumSets + 1) *
                                   cpuid_cache_info_p->c_linesz *
                                   cpuid_cache_info_p->c_assoc;
     ...
     */
    vm_cache_geometry_colors = ((arm_cache_ccsidr_info.bits.NumSets + 1) *
                                cpuid_cache_info_p->c_linesz) /
                               PAGE_SIZE;
  ...
  if (cpuid_cache_info_p->c_unified == 0) {
    ...
    cpuid_cache_info_p->c_isize =
        (arm_cache_ccsidr_info.bits.NumSets + 1) * c_linesz * c_assoc;
  }
  ...
  kprintf(
      "%s() - %u bytes %s cache (I:%u D:%u (%s)), %u-way assoc, %u "
      "bytes/line\n",
      __FUNCTION__, cpuid_cache_info_p->c_dsize + cpuid_cache_info_p->c_isize,
      ((cpuid_cache_info_p->c_type == CACHE_WRITE_BACK)
           ? "WB"
           : (cpuid_cache_info_p->c_type == CACHE_WRITE_THROUGH ? "WT"
                                                                : "Unknown")),
      cpuid_cache_info_p->c_isize, cpuid_cache_info_p->c_dsize,
      (cpuid_cache_info_p->c_unified) ? "unified" : "separate",
      cpuid_cache_info_p->c_assoc, cpuid_cache_info_p->c_linesz);
}

cache_info_t *cache_info(void) { return cpuid_cache_info_boot_cpu; }

cache_info_t *cache_info_type(cluster_type_t cluster_type) {
