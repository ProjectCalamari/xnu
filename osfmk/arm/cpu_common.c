...
kern_return_t cpu_info(processor_flavor_t flavor, int slot_num,
 ...
 *	Function:
 */
void cpu_doshutdown(void (*doshutdown)(processor_t), processor_t processor) {
...
void cpu_idle_tickle(void) {
  ...
  if (cpu_data_ptr->idle_timer_notify != NULL) {
    cpu_data_ptr->idle_timer_notify(cpu_data_ptr->idle_timer_refcon,
                                    &new_idle_timeout_ticks);
    if (new_idle_timeout_ticks != 0x0ULL) {
      /* if a new idle timeout was requested set the new idle timer deadline */
      clock_absolutetime_interval_to_deadline(
          new_idle_timeout_ticks, &cpu_data_ptr->idle_timer_deadline);
    } else {
 ...
 *
 */
void cpu_set_perfcontrol_timer(uint64_t now, uint64_t timeout_ticks) {
  ...
  if (timeout_ticks == EndOfAllTime) {
    ...
    running_timer_enter(processor, RUNNING_TIMER_PERFCONTROL, NULL, deadline,
                        now);
  }
}

static void cpu_handle_xcall(cpu_data_t *cpu_data_ptr) {
  broadcastFunc xfunc;
  void *xparam;

  os_atomic_thread_fence(acquire);
  /* Come back around if cpu_signal_internal is running on another CPU and has
   * just added SIGPxcall to the pending mask, but hasn't yet assigned the call
   * params.*/
  if (cpu_data_ptr->cpu_xcall_p0 != NULL &&
      cpu_data_ptr->cpu_xcall_p1 != NULL) {
    xfunc = ptrauth_auth_function(cpu_data_ptr->cpu_xcall_p0,
                                  ptrauth_key_function_pointer, cpu_data_ptr);
  ...
  if (cpu_data_ptr->cpu_imm_xcall_p0 != NULL &&
      cpu_data_ptr->cpu_imm_xcall_p1 != NULL) {
    xfunc = ptrauth_auth_function(cpu_data_ptr->cpu_imm_xcall_p0,
                                  ptrauth_key_function_pointer, cpu_data_ptr);
    ml_interrupt_masked_debug_start(xfunc, DBG_INTR_TYPE_IPI);
...
static unsigned int cpu_broadcast_xcall_internal(unsigned int signal,
                                                 uint32_t *synch,
  ...
  *synch = max_cpu;
  ...
  for (cpu = 0; cpu < max_cpu; cpu++) {
    ...
    if ((target_cpu_datap == NULL) ||
        KERN_SUCCESS !=
            cpu_signal(target_cpu_datap, signal,
                       ptrauth_nop_cast(
                           void *,
                           ptrauth_auth_and_resign(
                               func, ptrauth_key_function_pointer,
                               ptrauth_type_discriminator(broadcastFunc),
                               ptrauth_key_function_pointer, target_cpu_datap)),
                       parm)) {
      failsig++;
    }
  }
  ...
  if (os_atomic_sub(synch, (!self_xcall) ? failsig + 1 : failsig, relaxed) ==
      0) {
...
unsigned int cpu_broadcast_xcall(uint32_t *synch, boolean_t self_xcall,
                                 broadcastFunc func, void *parm) {
...
static void cpu_broadcast_xcall_simple_cbk(void *parm) {
  struct cpu_broadcast_xcall_simple_data *data =
      (struct cpu_broadcast_xcall_simple_data *)parm;
...
static unsigned int cpu_xcall_simple(boolean_t self_xcall, broadcastFunc func,
                                     void *parm, bool immediate) {
  struct cpu_broadcast_xcall_simple_data data = {};

  data.func = func;
  data.parm = parm;

  return cpu_broadcast_xcall_internal(immediate ? SIGPxcallImm : SIGPxcall,
                                      &data.sync, self_xcall,
                                      cpu_broadcast_xcall_simple_cbk, &data);
}

unsigned int cpu_broadcast_xcall_simple(boolean_t self_xcall,
                                        broadcastFunc func, void *parm) {
  return cpu_xcall_simple(self_xcall, func, parm, false);
}

static kern_return_t cpu_xcall_internal(unsigned int signal, int cpu_number,
                                        broadcastFunc func, void *param) {
  cpu_data_t *target_cpu_datap;
  ...
  if (func == NULL || param == NULL) {
    ...
    panic("cpu_xcall_internal: cannot have null func/param: %p %p", func,
          param);
  }
  ...
  return cpu_signal(
      target_cpu_datap, signal,
      ptrauth_nop_cast(void *,
                       ptrauth_auth_and_resign(
                           func, ptrauth_key_function_pointer,
                           ptrauth_type_discriminator(broadcastFunc),
                           ptrauth_key_function_pointer, target_cpu_datap)),
...
kern_return_t cpu_xcall(int cpu_number, broadcastFunc func, void *param) {
  return cpu_xcall_internal(SIGPxcall, cpu_number, func, param);
}

kern_return_t cpu_immediate_xcall(int cpu_number, broadcastFunc func,
                                  void *param) {
  return cpu_xcall_internal(SIGPxcallImm, cpu_number, func, param);
}

static kern_return_t cpu_signal_internal(cpu_data_t *target_proc,
                                         cpu_signal_t signal, void *p0,
                                         void *p1, boolean_t defer) {
  cpu_signal_t current_signals;
  bool swap_success;
  boolean_t interruptible = ml_set_interrupts_enabled(FALSE);
  cpu_data_t *current_proc = getCpuDatap();

  /* We'll mandate that only IPIs meant to kick a core out of idle may ever be
   * deferred. */
  if (defer) {
  ...
  if ((signal == SIGPxcall) || (signal == SIGPxcallImm)) {
    ...
    do {
      ...
      swap_success = os_atomic_cmpxchg(&target_proc->cpu_signal,
                                       current_signals & (~signal),
                                       current_signals | signal, release);

      if (!swap_success && (signal == SIGPxcallImm) &&
          (target_proc->cpu_signal & SIGPxcallImm)) {
        ml_set_interrupts_enabled(interruptible);
        return KERN_ALREADY_WAITING;
      }

      /* Drain pending xcalls on this cpu; the CPU we're trying to xcall may in
       * turn be trying to xcall us.  Since we have interrupts disabled that can
       * deadlock, so break the deadlock by draining pending xcalls. */
      if (!swap_success && (current_proc->cpu_signal & signal)) {
        cpu_handle_xcall(current_proc);
      }
    } while (!swap_success &&
             ((current_mabs_time = mach_absolute_time()) < max_mabs_time));

    /*
     * If we time out while waiting for the target CPU to respond, it's possible
     * that no other CPU is available to handle the watchdog interrupt that
     * would eventually trigger a panic. To prevent this from happening, we just
     * panic here to flag this condition.
     */
    if (__improbable(current_mabs_time >= max_mabs_time)) {
      uint64_t end_time_ns, xcall_ack_timeout_ns;
      absolutetime_to_nanoseconds(current_mabs_time - start_mabs_time,
                                  &end_time_ns);
      absolutetime_to_nanoseconds(xcall_ack_timeout_abstime,
                                  &xcall_ack_timeout_ns);
      panic("CPU%u has failed to respond to cross-call after %llu nanoseconds "
            "(timeout = %llu ns)",
    ...
    do {
      ...
      swap_success =
          os_atomic_cmpxchg(&target_proc->cpu_signal, current_signals,
                            current_signals | signal, release);
    } while (!swap_success);
  }

  /*
   * DSB is needed here to ensure prior stores to the pending signal mask and
   * xcall params will be visible by the time the other cores are signaled.  The
   * IPI mechanism on any given platform will very likely use either an MSR or a
   * non-coherent store that would not be ordered by a simple DMB.
   */
  __builtin_arm_dsb(DSB_ISHST);

  if (!(target_proc->cpu_signal & SIGPdisabled)) {
    /* Make sure cpu_phys_id is actually initialized */
    assert3u(os_atomic_load(&target_proc->cpu_flags, relaxed) &
...
kern_return_t cpu_signal(cpu_data_t *target_proc, cpu_signal_t signal, void *p0,
                         void *p1) {
  return cpu_signal_internal(target_proc, signal, p0, p1, FALSE);
}

kern_return_t cpu_signal_deferred(cpu_data_t *target_proc,
                                  cpu_signal_t signal) {
  return cpu_signal_internal(target_proc, signal, NULL, NULL, TRUE);
}

void cpu_signal_cancel(cpu_data_t *target_proc, cpu_signal_t signal) {
  cpu_signal_t current_signals;
...
void cpu_signal_handler(void) { cpu_signal_handler_internal(FALSE); }

bool cpu_has_SIGPdebug_pending(void) {
  cpu_data_t *cpu_data_ptr = getCpuDatap();

  return cpu_data_ptr->cpu_signal & SIGPdebug;
}

void cpu_signal_handler_internal(boolean_t disable_signal) {
  cpu_data_t *cpu_data_ptr = getCpuDatap();
  ...
  if ((!(cpu_signal & SIGPdisabled)) && (disable_signal == TRUE)) {
    ...
    assert3u(os_atomic_load(&cpu_data_ptr->cpu_flags, relaxed) &
                 (InitState | StartedState),
             ==, InitState | StartedState);
    cpu_signal =
        os_atomic_andnot(&cpu_data_ptr->cpu_signal, SIGPdisabled, acq_rel);
  }
  ...
  while (cpu_signal & ~SIGPdisabled) {
    ...
    if (cpu_signal & SIGPTimerLocal) {
      ...
      ml_interrupt_masked_debug_start(timer_queue_expire_local,
                                      DBG_INTR_TYPE_IPI);
...
void cpu_exit_wait(int cpu_id) {
#if !APPLEVIRTUALPLATFORM /* AVP doesn't provide an equivalent                 \
                             poll-for-powerdown operation */
#if USE_APPLEARMSMP
  if (!ml_is_quiescing()) {
    ...
    volatile uint64_t *cpu_sts =
        (void *)(cpu->cpu_IMPL_regs + CPU_PIO_CPU_STS_OFFSET);

    // Poll the "CPU running state" field until it is 0 (off)
    // This loop typically finishes in about 600ns.  Sometimes it takes as long
    // as 10us. If it takes longer than 10s, assume something went horribly
    // wrong and panic.
  ...
  if (cpu_id != master_cpu ||
    ...
    // For S2R, ml_arm_sleep() will do some extra polling after setting
    ...
    while (!((*(volatile unsigned int *)&cpu_data_ptr->cpu_sleep_token) ==
             ARM_CPU_ON_SLEEP_PATH)) {
      if (mach_absolute_time() > start + interval) {
        panic("CPU %d failed to reach ARM_CPU_ON_SLEEP_PATH: %d", cpu_id,
              cpu_data_ptr->cpu_sleep_token);
      }
    }
  }
}
...
void cpu_machine_init(void) {
  ...
  bool started =
      os_atomic_or_orig(&cpu_data_ptr->cpu_flags, StartedState, relaxed) &
      StartedState;
...
processor_t current_processor(void) { return PERCPU_GET(processor); }
...
cpu_data_t *processor_to_cpu_datap(processor_t processor) {
...
__startup_func static void cpu_data_startup_init(void) {
...
cpu_data_t *cpu_data_alloc(boolean_t is_boot_cpu) {
  ...
  if (is_boot_cpu) {
    ...
    base = os_atomic_add_orig(&percpu_base_cur, percpu_section_size(), relaxed);
...
ast_t *ast_pending(void) { return &getCpuDatap()->cpu_pending_ast; }

cpu_type_t slot_type(int slot_num) { return cpu_datap(slot_num)->cpu_type; }
...
cpu_type_t cpu_type(void) { return getCpuDatap()->cpu_type; }

cpu_subtype_t cpu_subtype(void) { return getCpuDatap()->cpu_subtype; }

cpu_threadtype_t cpu_threadtype(void) { return getCpuDatap()->cpu_threadtype; }

int cpu_number(void) { return current_thread()->machine.cpu_number; }
...
uint64_t ml_get_wake_timebase(void) { return wake_abstime; }

/*
 * Called while running on a specific CPU to wait for it to handle the
 * self-IPI that clears SIGPdisabled.
 */
void ml_wait_for_cpu_signal_to_enable(void) {
  assert(ml_get_interrupts_enabled());
  cpu_data_t *cpu_data_ptr = getCpuDatap();

  hw_wait_while_equals32(__DEVOLATILE(uint32_t *, &cpu_data_ptr->cpu_signal),
                         SIGPdisabled);
}

void assert_ml_cpu_signal_is_enabled(bool enabled) {
  if (enabled) {
...
bool ml_cpu_can_exit(__unused int cpu_id) {
#if USE_APPLEARMSMP
  /*
...
void ml_cpu_begin_loop(void) { lck_rw_lock_shared(&cpu_state_lock); }

void ml_cpu_end_loop(void) { lck_rw_unlock_shared(&cpu_state_lock); }

void ml_cpu_power_enable(int cpu_id) { PE_cpu_power_enable(cpu_id); }

void ml_cpu_power_disable(int cpu_id) { PE_cpu_power_disable(cpu_id); }

#else /* USE_APPLEARMSMP */

void ml_cpu_begin_state_transition(__unused int cpu_id) {}

void ml_cpu_end_state_transition(__unused int cpu_id) {}
