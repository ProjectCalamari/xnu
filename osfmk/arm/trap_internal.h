...
#ifndef _ARM_TRAP_INTERNAL_H_
#define _ARM_TRAP_INTERNAL_H_
...
/*
 * Shifts, masks, and other values for load/store multiple decoding; largely
 * needed for supporting misaligned accesses.
 */
...
#define ARM_COND_UNCOND                                                        \
  0xF0000000 /* Instruction does not support condition codes */
...
#define arm_fault_type(op, spsr, vaddr)                                        \
  (((((op) & ARM_CDX_MASK) == ARM_STC) ||                                      \
    ...
    (((op) & ARM_BDX_MASK) == ARM_STM) || (((op) & ARM_SDX_MASK) == ARM_STR))  \
...
typedef kern_return_t (*perfCallback)(int trapno, struct arm_saved_state *ss,
                                      int, int);

#endif /* _ARM_TRAP_INTERNAL_H_ */
