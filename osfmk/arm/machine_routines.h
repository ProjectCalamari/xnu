...
static inline void ml_interrupt_masked_debug_start(void *handler_addr,
                                                   int type) {
#if SCHED_HYGIENE_DEBUG
...
static inline void ml_interrupt_masked_debug_end(void) {
#if SCHED_HYGIENE_DEBUG
  if (static_if(sched_debug_interrupt_disable)) {
...
typedef void (*cache_dispatch_t)(cpu_id_t cpu_id, unsigned int select,
...
/* Unused.  Left for historical reasons. */
typedef ex_cb_action_t (*ex_cb_t)(ex_cb_class_t cb_class, void *refcon,
 ...
 * @brief Maps per-CPU and per-cluster PIO registers found in EDT.  This needs
 * to be called after arm_vm_init() so it can't be part of
 ...
 */
kern_return_t
ml_processor_register(ml_processor_info_t *ml_processor_info,
...
__printflike(1, 0) void ml_panic_trap_to_debugger(
    const char *panic_format_str, va_list *panic_args, unsigned int reason,
...
kern_return_t ml_static_verify_page_protections(uint64_t base, uint64_t size,
                                                vm_prot_t prot);
...
uint128_t ml_phys_read_quad(vm_offset_t paddr);
uint128_t ml_phys_read_quad_64(addr64_t paddr);

/* Write physical address quad word */
void ml_phys_write_quad(vm_offset_t paddr, uint128_t data);
void ml_phys_write_quad_64(addr64_t paddr, uint128_t data);
#endif /* BUILD_QUAD_WORD_FUNCS */

void ml_static_mfree(vm_offset_t, vm_size_t);

kern_return_t ml_static_protect(vm_offset_t start, vm_size_t size,
                                vm_prot_t new_prot);
...
extern vm_offset_t io_map(vm_map_offset_t phys_addr, vm_size_t size,
                          unsigned int flags, vm_prot_t prot, bool unmappable);
...
void platform_syscall(struct arm_saved_state *);

void ml_set_decrementer(uint32_t dec_value);

boolean_t is_user_contex(void);
...
uintptr_t arm_user_protect_begin(thread_t thread);

void arm_user_protect_end(thread_t thread, uintptr_t up,
...
void ml_set_max_cpus(unsigned int max_cpus);

/* Return the maximum number of CPUs set by ml_set_max_cpus(), waiting if
 * necessary */
...
#ifdef XNU_KERNEL_PRIVATE
/* Return max offset */
vm_map_offset_t ml_get_max_offset(boolean_t is64, unsigned int option);
...
#define TCOAL_DEBUG(x, a, b, c, d, e)                                          \
  do {                                                                         \
  } while (0)
#endif /* XNU_KERNEL_PRIVATE */
...
unsigned long monitor_call(uintptr_t callnum, uintptr_t arg1, uintptr_t arg2,
                           uintptr_t arg3);
#endif /* MONITOR */
...
/* Please note that cpu_broadcast_xcall is not as simple is you would like it to
 * be. It will sometimes put the calling thread to sleep, and it is up to your
 * callback to wake it up as needed, where "as needed" is defined as "all other
 * CPUs have called the broadcast func". Look around the kernel for examples, or
 * instead use cpu_broadcast_xcall_simple() which does indeed act like you would
 * expect, given the prototype.
 */
...
__result_use_check kern_return_t cpu_immediate_xcall(int, broadcastFunc,
                                                     void *);
...
#ifdef __arm64__
typedef void (*cpu_qos_update_t)(int throughput_qos, uint64_t qos_param1,
                                 uint64_t qos_param2);
...
struct going_on_core {
  ...
  uint32_t is_32_bit : 1; /* uses 32-bit ISA/register state in userspace (which
                             may differ from address space size) */
  uint32_t is_kernel_thread : 1;
  uint64_t thread_group_id;
  void *thread_group_data;
  uint64_t scheduling_latency; /* absolute time between when thread was made
                                  runnable and this ctx switch */
  uint64_t start_time;
  uint64_t scheduling_latency_at_same_basepri;
  uint32_t energy_estimate_nj; /* return: In nanojoules */
  /* smaller of the time between last change to base priority and ctx switch and
   * scheduling_latency */
};
...
typedef struct perfcontrol_max_runnable_latency
...
typedef struct perfcontrol_work_interval_instance
...
__options_decl(
    perfcontrol_thread_flags_mask_t, uint64_t,
    {
...
struct perfcontrol_thread_data {
  ...
  void *perfctl_state;
  /* Bitmask to indicate which thread flags have been updated as part of the
   * callout */
 ...
 */
typedef void (*sched_perfcontrol_context_switch_t)(perfcontrol_state_t,
                                                   perfcontrol_state_t);
 ...
 */
typedef void (*sched_perfcontrol_offcore_t)(
    perfcontrol_state_t, going_off_core_t /* populated by callee */, boolean_t);
 ...
 * offcore callouts.
 */
typedef void (*sched_perfcontrol_oncore_t)(perfcontrol_state_t,
                                           going_on_core_t);
 ...
 */
typedef void (*sched_perfcontrol_max_runnable_latency_t)(
    perfcontrol_max_runnable_latency_t);
 ...
 */
typedef void (*sched_perfcontrol_work_interval_notify_t)(
    perfcontrol_state_t, perfcontrol_work_interval_t);

/*
 * Start, update and finish work interval instance with optional complexity
 * estimate.
 */
typedef void (*sched_perfcontrol_work_interval_ctl_t)(
    perfcontrol_state_t, perfcontrol_work_interval_instance_t);
...
typedef void (*sched_perfcontrol_thread_group_flags_update_t)(
    thread_group_data_t);

/*
 * Sometime after the timeout set by sched_perfcontrol_update_callback_deadline
 * has passed, this function will be called, passing the timeout deadline that
 * was previously armed as an argument.
 *
 * This is called inside context-switch/quantum-interrupt context and must
 * follow the safety rules for that context.
 */
...
typedef void (*sched_perfcontrol_csw_t)(
    perfcontrol_event event, uint32_t cpu_id, uint64_t timestamp,
    uint32_t flags, struct perfcontrol_thread_data *offcore,
    struct perfcontrol_thread_data *oncore,
...
typedef void (*sched_perfcontrol_state_update_t)(
    perfcontrol_event event, uint32_t cpu_id, uint64_t timestamp,
    uint32_t flags, struct perfcontrol_thread_data *thr_data,
 ...
 *
 * Parameters:
 * blocked_tg           - Thread group blocking on progress of another thread
 * group blocking_tg          - Thread group blocking progress of another thread
 * group flags                - Flags for other relevant information
 * blocked_thr_state    - Per-thread perfcontrol state for blocked thread
 */
typedef void (*sched_perfcontrol_thread_group_blocked_t)(
    thread_group_data_t blocked_tg, thread_group_data_t blocking_tg,
    uint32_t flags, perfcontrol_state_t blocked_thr_state);

/*
 * Thread Group Unblocking Callout
 *
 * Parameters:
 * unblocked_tg         - Thread group being unblocked from making forward
 * progress unblocking_tg        - Thread group unblocking progress of another
 * thread group flags                - Flags for other relevant information
...
typedef void (*sched_perfcontrol_thread_group_unblocked_t)(
    thread_group_data_t unblocked_tg, thread_group_data_t unblocking_tg,
    uint32_t flags, perfcontrol_state_t unblocked_thr_state);
...
/*
 * Callers should always use the CURRENT version so that the kernel can detect
 * both older and newer structure layouts. New callbacks should always be added
 * at the end of the structure, and xnu should expect existing source recompiled
 * against newer headers to pass NULL for unimplemented callbacks. Pass NULL as
 * the as the callbacks parameter to reset callbacks to their default in-kernel
 * values.
 */

#define SCHED_PERFCONTROL_CALLBACKS_VERSION_0 (0) /* up-to oncore */
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_1                                  \
  (1) /* up-to max_runnable_latency */
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_2                                  \
  (2) /* up-to work_interval_notify */
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_3                                  \
  (3) /* up-to thread_group_deinit */
...
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_6                                  \
  (6) /* up-to thread_group_flags_update */
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_7 (7) /* up-to work_interval_ctl   \
                                                   */
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_8                                  \
  (8) /* up-to thread_group_unblocked */
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_9                                  \
  (9) /* allows CLPC to specify resource contention flags */
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_10                                 \
  (10) /* allows CLPC to register a per core timer callback */
#define SCHED_PERFCONTROL_CALLBACKS_VERSION_CURRENT                            \
  SCHED_PERFCONTROL_CALLBACKS_VERSION_10
...
extern void
sched_perfcontrol_register_callbacks(sched_perfcontrol_callbacks_t callbacks,
                                     unsigned long size_of_state);
extern void
sched_perfcontrol_thread_group_recommend(void *data,
                                         cluster_type_t recommendation);
extern void sched_perfcontrol_inherit_recommendation_from_tg(
    perfcontrol_class_t perfctl_class, boolean_t inherit);
 ...
 *
 * The Edge scheduler expects thread group recommendations to be specific
 * clusters rather than just E/P. In order to allow more fine grained control,
 * CLPC can specify an override preferred cluster per QoS bucket. CLPC passes a
 * common preferred cluster `tg_preferred_cluster` and an array of size
 ...
 * of this TG as part of this preferred cluster change. It does that by
 ...
 *
 * For each QoS, the Edge scheduler uses edges between clusters to define the
 * likelihood of migrating threads of that QoS across the clusters. The edge
 * config between any two clusters defines the edge weight and whether migation
 * and steal operations are allowed across that edge. The getter and setter
 * allow CLPC to query and configure edge properties between various clusters on
 * the platform.
 *
...
extern void sched_perfcontrol_edge_matrix_by_qos_get(
    sched_clutch_edge *edge_matrix, bool *edge_requested, uint64_t flags,
    uint64_t num_psets, uint64_t num_classes);
extern void sched_perfcontrol_edge_matrix_by_qos_set(
    sched_clutch_edge *edge_matrix, bool *edge_changed, uint64_t flags,
    uint64_t num_psets, uint64_t num_classes);

/*
 * sched_perfcontrol_edge_matrix_get()/sched_perfcontrol_edge_matrix_set()
 *
 * Legacy interface for getting/setting the edge config properties, which
 * determine the edge weight and whether steal and migration are allowed between
 * any two clusters. Since the edge matrix has a per-QoS dimension,
 * sched_perfcontrol_edge_matrix_set() sets the configuration to be the same
 * across all QoSes. sched_perfcontrol_edge_matrix_get() reads the edge matrix
 * setting from the highest QoS (fixed priority).
 *
 * Superceded by
 * sched_perfcontrol_edge_matrix_by_qos_get()/sched_perfcontrol_edge_matrix_by_qos_set()
 */

extern void sched_perfcontrol_edge_matrix_get(sched_clutch_edge *edge_matrix,
                                              bool *edge_requested,
                                              uint64_t flags,
                                              uint64_t matrix_order);
extern void sched_perfcontrol_edge_matrix_set(sched_clutch_edge *edge_matrix,
                                              bool *edge_changed,
                                              uint64_t flags,
                                              uint64_t matrix_order);
 ...
 *
 * In order to drive intra-cluster core rotation CLPC supplies the edge
 * scheduler with a pair of per-cluster bitmasks. The preferred_bitmask is a
 * bitmask of CPU cores where if a bit is set, CLPC would prefer threads to be
 * scheduled on that core if it is idle. The migration_bitmask is a bitmask of
 * CPU cores where if a bit is set, CLPC would prefer threads no longer continue
 * running on that core if there is any other non-avoided idle core in the
 * cluster that is available.
 */

extern void
sched_perfcontrol_edge_cpu_rotation_bitmasks_set(uint32_t cluster_id,
                                                 uint64_t preferred_bitmask,
                                                 uint64_t migration_bitmask);
extern void
sched_perfcontrol_edge_cpu_rotation_bitmasks_get(uint32_t cluster_id,
                                                 uint64_t *preferred_bitmask,
                                                 uint64_t *migration_bitmask);

/*
 * Update the deadline after which sched_perfcontrol_deadline_passed will be
 * called. Returns TRUE if it successfully canceled a previously set callback,
 * and FALSE if it did not (i.e. one wasn't set, or callback already fired / is
 * in flight). The callback is automatically canceled when it fires, and does
 * not repeat unless rearmed.
 *
 * This 'timer' executes as the scheduler switches between threads, on a
 * non-idle core
 *
...
#define ptrauth_sign_volatile(__value, __suffix, __data)                       \
  ({                                                                           \
    void *__ret = __value;                                                     \
    asm volatile("pac" #__suffix "	%[value], %[data]"                          \
                 : [value] "+r"(__ret)                                         \
                 : [data] "r"(__data));                                        \
    __ret;                                                                     \
  })
...
#define _ml_auth_ptr_unchecked(_ptr, _suffix, _modifier)                       \
  ml_auth_ptr_unchecked_for_key(_ptr, _suffix, ptrauth_key_as##_suffix,        \
                                _modifier)
#else
#define _ml_auth_ptr_unchecked(_ptr, _suffix, _modifier)                       \
  asm volatile("aut" #_suffix " %[ptr], %[modifier]"                           \
               : [ptr] "+r"(_ptr)                                              \
               : [modifier] "r"(_modifier));
