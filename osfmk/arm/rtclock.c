...
static int deadline_to_decrementer(uint64_t deadline, uint64_t now);
...
uint64_t mach_absolute_time(void) {
  ...
  if (__improbable(timebase_validation)) {
    ...
    if (old_absolute_time > new_absolute_time) {
      ...
      panic("mach_absolute_time returning non-monotonically increasing value "
            "0x%llx (old value 0x%llx)",
            new_absolute_time, old_absolute_time);
    }

    if (old_absolute_time < new_absolute_time) {
      /* read again, to pretest the atomic max */
      uint64_t pretest_absolute_time =
          os_atomic_load(&s_last_absolute_time, relaxed);
      if (pretest_absolute_time < new_absolute_time) {
        uint64_t fresh_last_absolute_time = os_atomic_max_orig(
            &s_last_absolute_time, new_absolute_time, relaxed);

        if (fresh_last_absolute_time != pretest_absolute_time) {
          /*
           * Someone else published a newer time after we loaded
           * s_last_absolute_time. Enforce that our timebase is not behind this
           * new one. We can't compare it with our previous timebase read, as it
           * is too old.
           */
          ...
          if (fresh_last_absolute_time > newest_absolute_time) {
            timebase_validation = 0; // prevent nested panics
            panic("mach_absolute_time returning non-monotonically increasing "
                  "value 0x%llx (old values 0x%llx, 0x%llx, 0x%llx, 0x%llx)\n",
                  newest_absolute_time, fresh_last_absolute_time,
                  pretest_absolute_time, old_absolute_time, new_absolute_time);
          }
        }
      }
    }
    ...
    /* ARM 64: We need a dsb here to ensure that the load of
     * s_last_absolute_time completes before the timebase read. Were the load to
     * complete after the timebase read, there would be a window for another CPU
     * to update s_last_absolute_time and leave us in an inconsistent state.
    ...
    } while (attempts < MAX_TIMEBASE_TRIES &&
             !OSCompareAndSwap64(old_absolute_time, new_absolute_time,
                                 &s_last_absolute_time));

    if (attempts < MAX_TIMEBASE_TRIES &&
        old_absolute_time > new_absolute_time) {
      timebase_validation = 0; // we know it's bad, now prevent nested panics
      panic("mach_absolute_time returning non-monotonically increasing value "
            "0x%llx (old value 0x%llx\n)",
            new_absolute_time, old_absolute_time);
...
uint64_t mach_approximate_time(void) {
  ...
  /* Hardware supports a fast timestamp, so grab it without asserting
   * monotonicity */
  return ml_get_timebase();
#else
 ...
 */
void rtclock_intr(__unused unsigned int is_user_context) {
  uint64_t abstime;
  cpu_data_t *cdp;
  ...
  if (abstime >= cdp->rtcPop) {
    ...
    KDBG_RELEASE(DECR_TRAP_LATENCY | DBG_FUNC_NONE, -(abstime - cdp->rtcPop),
...
static int deadline_to_decrementer(uint64_t deadline, uint64_t now) {
  ...
  if (deadline <= now) {
    ...
    return (delt >= (DECREMENTER_MAX + 1))
               ? DECREMENTER_MAX
               : ((delt >= (DECREMENTER_MIN + 1)) ? (int)delt
                                                  : DECREMENTER_MIN);
  }
}
 ...
 */
void ClearIdlePop(boolean_t wfi) {
  cpu_data_t *cdp;
