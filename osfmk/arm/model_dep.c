...
/*
 * Make sure there's enough space to include the relevant bits in the format
 * required within the space allocated for the panic version string in the panic
 * header. The format required by OSAnalytics/DumpPanic is 'Product Version (OS
 * Version)'.
 */
...
/*Choosing the size for gTargetTypeBuffer as 16 and size for gModelTypeBuffer as
 * 32 since the target name and model name typically  doesn't exceed this size
 */
...
MACHINE_TIMEOUT_ALWAYS_ENABLED(debug_ack_timeout, "debug-ack", 240000,
...
_Atomic unsigned int mp_kdp_trap =
    0; /* CPUs signalled by the debug CPU will spin on this */
_Atomic unsigned int debug_cpus_spinning =
    0; /* Number of signalled CPUs still spinning on mp_kdp_trap (in
          DebuggerXCall). */
...
static void do_print_backtrace_internal(pmap_t pmap, vm_offset_t topfp,
...
static void panic_display_tpidrs(void) {
#if defined(__arm64__)
  paniclog_append_noflush(
...
static void panic_display_hung_cpus_help(void) {
#if defined(__arm64__)
  ...
  if (info) {
    ...
    for (i = 0; i < info->num_cpus; i++) {
      ...
      snprintf(cluster_name, sizeof(cluster_name), "%cACC%d", cluster_letter,
               cpu->cluster_id);
      ...
      if (cpu->cpu_UTTDBG_regs) {
        volatile uint64_t *pcsr =
            (volatile uint64_t *)(cpu->cpu_UTTDBG_regs + pcsr_offset);
        ...
        for (retry = 1024; retry && !pc; retry--) {
          // a 32-bit read is required to make a PC sample be produced, else
          // we'll only get a zero
          (void)*pcsrTrigger;
          pc = *pcsr;
        }
...
static void panic_display_pvhs_locked(void) {}

static void panic_display_pvh_to_lock(void) {}

static void panic_display_last_pc_lr(void) {
#if defined(__arm64__)
  ...
  for (int cpu = 0; cpu <= max_cpu; cpu++) {
    ...
    if (current_cpu_datap == getCpuDatap()) {
       ...
       */
      paniclog_append_noflush("CORE %u is the one that panicked. Check the "
    ...
    paniclog_append_noflush(
        ...
        current_cpu_datap->ipi_pc,
        (uint64_t)VM_KERNEL_STRIP_PTR(current_cpu_datap->ipi_lr),
...
#if CONFIG_EXCLAVES
static void panic_report_exclaves_stackshot(void) {
  ...
  } else if (exclaves_panic_ss_status ==
             EXCLAVES_PANIC_STACKSHOT_DECODE_FAILED) {
...
print_backtrace_internal(thread_t thread, bool filesetKC) {
  ...
  if (cur_fp < VM_MAX_KERNEL_ADDRESS) {
    paniclog_append_noflush(
        ...
        (addr64_t)cur_fp, thread_tid(thread));
#if __LP64__
    do_print_backtrace_internal(kernel_pmap, cur_fp, nohilite_thread_marker,
                                TRUE, filesetKC);
#else
    do_print_backtrace_internal(kernel_pmap, cur_fp, nohilite_thread_marker,
                                FALSE, filesetKC);
...
static void do_print_all_panic_info(const char *message, uint64_t panic_options,
  ...
  /* end_marker_bytes set to 200 for printing END marker + stackshot summary
   * info always */
    ...
    if (last_hwaccess_thread) {
      ...
      if (!last_hwaccess_size) {
        ...
        paniclog_append_noflush(
            ...
            last_hwaccess_size, last_hwaccess_paddr);
      }
    }
  ...
  paniclog_append_noflush("OS release type: %.256s\n",
                          ('\0' != osreleasetype[0]) ? osreleasetype
                                                     : "Not set yet");
...
#if DEVELOPMENT || DEBUG
  if (cs_debug_unsigned_exec_failures != 0 ||
  ...
  // Highlight threads that used high amounts of CPU in the panic log if
  // requested (historically requested for watchdog panics)
      ...
      if (thread->state & TH_RUN) {
        ...
        for (k = 0; k < TOP_RUNNABLE_LIMIT; k++) {
          if (top_runnable[k] == 0) {
            ...
            break;
          } else if (comparison_thread->sched_pri >
                     top_runnable[k]->sched_pri) {
            thread_t temp = top_runnable[k];
            top_runnable[k] = comparison_thread;
            comparison_thread = temp;
          } // if comparison thread has higher priority than previously saved
            // thread
    ...
    for (int i = 0; i < TOP_RUNNABLE_LIMIT; i++) {
      if (top_runnable[i] &&
        ...
        paniclog_append_noflush("%p %s %d %d\n", top_runnable[i], name,
                                top_runnable[i]->sched_pri,
                                top_runnable[i]->cpu_usage);
      }
    } // Loop through highest priority runnable threads
    paniclog_append_noflush("\n");
  }
  ...
  if (panic_get_thread_proc_task(cur_thread, &task, &proc)) {
    if (PANIC_VALIDATE_PTR(task->map) && PANIC_VALIDATE_PTR(task->map->pmap)) {
  ...
  /* set the os version data in the panic header in the format 'Product Version
   * (OS Version)' (only if they have been set) */
  if ((osversion[0] != '\0') && (osproductversion[0] != '\0')) {
    snprintf((char *)&panic_info->eph_os_version,
             sizeof(panic_info->eph_os_version), PANIC_HEADER_VERSION_FMT_STR,
             osproductversion, osversion);
  }
#if defined(XNU_TARGET_OS_BRIDGE)
  if ((macosversion[0] != '\0') && (macosproductversion[0] != '\0')) {
    snprintf((char *)&panic_info->eph_macos_version,
             sizeof(panic_info->eph_macos_version),
             PANIC_HEADER_VERSION_FMT_STR, macosproductversion, macosversion);
  }
#endif
  if (bootsessionuuid_string[0] != '\0') {
  ...
  if (panic_initiator != NULL) {
    bytes_remaining =
        debug_buf_size -
        (unsigned int)((uintptr_t)debug_buf_ptr - (uintptr_t)debug_buf_base);
    ...
    // Calculate the bytes to write, accounting for remaining buffer space, and
    // ensuring the lowest size we can have is 0
    panic_initiator_len = MAX(0, MIN(panic_initiator_len, bytes_remaining));
    panic_info->eph_panic_initiator_offset =
        (panic_initiator_len != 0)
            ? PE_get_offset_into_panic_region(debug_buf_ptr)
            : 0;
  ...
  if (debug_ack_timeout_count) {
    ...
    panic_info->eph_other_log_offset =
        PE_get_offset_into_panic_region(debug_buf_ptr);
    paniclog_append_noflush(
    ...
    /* Align the stackshot buffer to an 8-byte address (especially important for
     * armv7k devices) */
    ...
    if (err == KERN_SUCCESS) {
      ...
      kdp_snapshot_preflight(-1, stackshot_begin_loc,
                             bytes_remaining - end_marker_bytes,
      ...
      if (bytes_traced > 0 && !err) {
        ...
        panic_info->eph_stackshot_offset =
  ...
  panic_info->eph_ext_paniclog_offset =
      (ext_paniclog_bytes != 0)
          ? PE_get_offset_into_panic_region((debug_buf_base + debug_buf_size) -
                                            ext_paniclog_bytes)
          : 0;
 ...
 */
static void print_all_panic_info(const char *message, uint64_t panic_options,
                                 const char *panic_initiator) {
...
void panic_display_times() {
  ...
  if ((is_clock_configured) &&
      (lck_ticket_lock_try(&clock_lock, &clock_lock_grp))) {
    ...
    paniclog_append_noflush("  Sleep   : 0x%08x 0x%08x\n",
                            (unsigned int)gIOLastSleepTime.tv_sec,
                            (unsigned int)gIOLastSleepTime.tv_usec);
    paniclog_append_noflush("  Wake    : 0x%08x 0x%08x\n",
                            (unsigned int)gIOLastWakeTime.tv_sec,
                            (unsigned int)gIOLastWakeTime.tv_usec);
...
void SavePanicInfo(const char *message, __unused void *panic_data,
  ...
  /*
   * On newer targets, panic data is stored directly into the iBoot panic
   * region. If we re-enter SavePanicInfo (e.g. on a double panic) on such a
   * target, update the panic CRC so that iBoot can hopefully find *something*
   * useful in the panic region.
   */
  if (PanicInfoSaved && (debug_buf_base >= (char *)gPanicBase) &&
      (debug_buf_base < (char *)gPanicBase + gPanicSize)) {
    unsigned int pi_size = (unsigned int)(debug_buf_ptr - gPanicBase);
    PE_update_panic_crc((unsigned char *)gPanicBase, &pi_size);
    PE_sync_panic_buffers(); // extra precaution; panic path likely isn't
                             // reliable if we're here
  }
...
#if CONFIG_SPTM
/*
 * Patch thread state to appear as if a debugger stop IPI occurred, when a
 * thread is parked in SPTM panic loop. This allows stackshot to proceed as
 * usual.
 */
static void DebuggerPatchupThreadState(int cpu, xnu_saved_registers_t *regp) {
  ...
  kstackptr =
      (vm_offset_t)target_cpu_datap->cpu_active_thread->machine.kstackptr;
 ...
 */
kern_return_t DebuggerXCallEnter(boolean_t proceed_on_sync_failure,
                                 bool is_stackshot) {
   ...
   * synchronize with every CPU that we appeared to signal successfully
   * (cpu_signal is not synchronous).
   */
  ...
  if (!immediate_halt) {
    for (cpu = 0; cpu <= max_cpu; cpu++) {
      ...
      kern_return_t ret =
          cpu_signal(target_cpu_datap, SIGPdebug, (void *)NULL, NULL);
      if (ret == KERN_SUCCESS) {
        ...
        kprintf("%s: cpu_signal failed. cpu=%d ret=%d proceed=%d\n", __func__,
                cpu, ret, proceed_on_sync_failure);
      }
    }
    ...
    /*
     * Wait for DEBUG_ACK_TIMEOUT ns for a response from everyone we IPI'd.  If
     * we timeout, that is simply too bad; we don't have a true NMI, and one CPU
     * may be uninterruptibly spinning on someone else.  The best we can hope
     * for is that all other CPUs have either responded or are spinning in a
     * context that is debugger safe.
     */
    do {
      current_mabs_time = mach_absolute_time();
      sync_pending = os_atomic_load(&debugger_sync, acquire);
    } while ((sync_pending != 0) &&
             (max_mabs_time == 0 || current_mabs_time < max_mabs_time));
  }

  if (!immediate_halt && max_mabs_time > 0 &&
      current_mabs_time >= max_mabs_time) {
    /*
    ...
    for (cpu = 0; cpu <= max_cpu; cpu++) {
      ...
      if (proceed_on_sync_failure) {
        ...
        pmap_sptm_percpu_data_t *sptm_pcpu =
            PERCPU_GET_WITH_BASE(base, pmap_sptm_percpu);
        uint64_t sptm_cpuid = sptm_pcpu->sptm_cpu_id;

        if (sptm_get_cpu_state(sptm_cpuid, CPUSTATE_PANIC_SPIN,
                               &sptm_panic_loop) == SPTM_SUCCESS &&
            sptm_panic_loop) {
  ...
  if (immediate_halt ||
    ...
    for (cpu = 0; cpu <= max_cpu; cpu++) {
      ...
      dbgwrap_status_t halt_status = ml_dbgwrap_halt_cpu_with_state(
          cpu, NSEC_PER_SEC, &target_cpu_datap->halt_state);
      if ((halt_status < 0) || (halt_status == DBGWRAP_WARN_CPU_OFFLINE)) {
        paniclog_append_noflush(
            "Unable to obtain state for cpu %d with status %d: %s\n", cpu,
            halt_status, ml_dbgwrap_strerror(halt_status));
...
void DebuggerXCallReturn(void) {
   ...
   * Wait for DEBUG_ACK_TIMEOUT ns for a response and move on if we don't get
   * it.
   *
  ...
  if (current_mabs_time >= max_mabs_time &&
      os_atomic_load(&debug_ack_timeout, relaxed) != 0) {
    panic("Resuming from debugger synchronization failed: waited %llu "
          "nanoseconds\n",
          os_atomic_load(&debug_ack_timeout, relaxed));
  }
}
 ...
 * pending.
 */
void wait_while_mp_kdp_trap(bool check_SIGPdebug) {
  ...
  if (check_SIGPdebug && found_mp_kdp_trap) {
    kprintf("%s>found_mp_kdp_trap=true found_SIGPdebug=%s\n", __FUNCTION__,
            found_SIGPdebug ? "true" : "false");
  }
}

void DebuggerXCall(void *ctx) {
  ...
  /*
   * When running in serial mode, the core capturing the dump may hold
   * interrupts disabled for a time longer than the timeout. That path includes
   * logic to reset the timestamp so that we do not eventually trigger the
   * interrupt timeout assert().
   *
   * Here we check whether other cores have already gone over the timeout at
   * this point before spinning, so we at least cover the IPI reception path.
   * After spinning, however, we reset the timestamp so as to avoid hitting the
   * interrupt timeout assert().
   */
  ...
  /*
   * Before we decrement debugger sync, do stackshot preflight work (if
   * applicable). Namely, we want to signal that we're available to do stackshot
   * work, and we need to signal so before the stackshot-calling CPU starts
   * work.
   */
  ...
  if ((serialmode & SERIALMODE_OUTPUT) || trap_is_stackshot) {
    ml_interrupt_masked_debug_start(
        (void *)current_thread()->machine.int_handler_addr,
        current_thread()->machine.int_type);
  }
...
void DebuggerCall(unsigned int reason, void *ctx) {
#if !MACH_KDP
