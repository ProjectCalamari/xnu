 ...
 */
static_assert(sizeof(cpumap_t) * CHAR_BIT >= MAX_CPUS,
...
#define CPUWINDOWS_TOP                                                         \
  (CPUWINDOWS_BASE + (MAX_CPUS * CPUWINDOWS_MAX * ARM_PGBYTES))

#ifndef __BUILDING_XNU_LIBRARY__ /* in user-mode kernel addresses are low */
static_assert((CPUWINDOWS_BASE >= VM_MIN_KERNEL_ADDRESS) &&
                  ((CPUWINDOWS_TOP - 1) <= VM_MAX_KERNEL_ADDRESS),
...
typedef struct rtclock_timer {
  ...
  uint64_t deadline;
  uint32_t is_set : 1, has_expired : 1, : 0;
} rtclock_timer_t;
...
__options_closed_decl(cpu_flags_t, uint16_t,
                      {
                          SleepState = 0x0800,
                          /* For the boot processor, StartedState means
                             'interrupts initialized' - it is already running */
                          StartedState = 0x1000,
                          /* For the boot processor, InitState means 'cpu_data
                             fully initialized' - it is already running */
                          InitState = 0x2000,
                      });

__options_closed_decl(
    cpu_signal_t, unsigned int,
    {
        ...
        SIGPxcallImm =
            0x00000200U, /* Send a cross-call, fail if already pending */
        SIGPTimerLocal = 0x00000400U, /* Update the decrementer via
...
#define ARM_CPU_ON_SLEEP_PATH 0x50535553UL
  ...
  enum { CPU_NOT_HALTED = 0, CPU_HALTED, CPU_HALTED_WITH_STATE } halt_status;
