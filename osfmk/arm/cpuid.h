...
#define CPU_ARCH_ARMv5TEJ                                                      \
  0x6 /* ARMv5 + Thumb + Extensions(?) + //Jazelle(?) XXX */
...
#define CPU_ARCH_ARMv8E                                                        \
  0xd /* ARMv8.3a + Apple Private ISA Subtype for CPU_TYPE_ARM64 */

/* special code indicating we need to look somewhere else for the architecture
 * version */
...
typedef struct {
  uint32_t RB : 4, /* 3:0 - 32x64-bit media register bank supported: 0x2 */
...
typedef struct {
  uint32_t FZ : 4, /* 3:0 - Full denormal arithmetic supported for VFP: 0x1 */
      ...
      SP : 4, /* 19:16 - Single precision floating-point instructions supported
                 for NEON: 0x1 */
      HPFP
