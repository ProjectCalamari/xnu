...
__private_extern__ void qsort(void *a, size_t n, size_t es,
...
uint32_t kernel_post_tests_count =
    sizeof(kernel_post_tests) / sizeof(xnupost_test_data_t);
...
kern_return_t xnupost_parse_config() {
  ...
  if (PE_parse_boot_argn("kernPOST_config", &kernel_post_test_configs[0],
                         sizeof(kernel_post_test_configs)) == TRUE) {
...
kern_return_t xnupost_list_tests(xnupost_test_t test_list,
                                 uint32_t test_count) {
...
kern_return_t xnupost_run_tests(xnupost_test_t test_list, uint32_t test_count) {
  ...
  for (; i < test_count; i++) {
     ...
     */
    if ((testp->xt_config & XT_CONFIG_EXPECT_PANIC) &&
        !(kernel_post_args & POSTARGS_CONTROLLER_AVAILABLE)) {
      T_SKIP("Test expects panic but "
    ...
    if (T_STATE_UNRESOLVED == T_TESTRESULT) {
       ...
       * determine the test result based on the return value of the test
       * function.
       */
...
kern_return_t xnupost_register_panic_widget(xt_panic_widget_func funcp,
                                            const char *funcname, void *context,
                                            void **outval) {
  if (xt_panic_widgets.xtp_context_p != NULL ||
      xt_panic_widgets.xtp_func != NULL) {
...
void xnupost_reset_panic_widgets() {
...
kern_return_t xnupost_process_kdb_stop(const char *panic_s) {
  ...
  if (xt_panic_widgets.xtp_func) {
    T_LOG("%s: Calling out to widget: %s", __func__,
          xt_panic_widgets.xtp_func_name);
  ...
  case XT_RET_W_SUCCESS:
    T_EXPECT_EQ_INT(retval, XT_RET_W_SUCCESS,
                    "%s reported successful handling. Returning from kdb_stop.",
                    name);
...
xt_panic_return_t _xt_generic_assert_check(const char *s, void *str_to_match,
                                           void **outval) {
  xt_panic_return_t ret = XT_PANIC_UNRELATED;

  if (NULL != strnstr(__DECONST(char *, s), (char *)str_to_match, strlen(s))) {
    T_LOG("%s: kdb_stop string: '%s' MATCHED string: '%s'", __func__, s,
          (char *)str_to_match);
...
kern_return_t xnupost_reset_tests(xnupost_test_t test_list,
                                  uint32_t test_count) {
  uint32_t i = 0;
  xnupost_test_t testp;
  for (; i < test_count; i++) {
...
kern_return_t zalloc_test(void) {
  ...
  T_SETUPBEGIN;
  test_zone =
      zone_create("test_uint64_zone", sizeof(uint64_t), ZC_DESTRUCTIBLE);
  ...
  T_PERF("num_threads_at_ktest", threads_count, "count",
         "# of threads in system at zalloc_test");
 ...
 */
static int compare_numbers_ascending(const void *a, const void *b) {
 ...
 */
static int count_bits(uint64_t number) { return __builtin_popcountll(number); }

kern_return_t RandomULong_test() {
  /*
   ...
   *          6. No -1
   *
   * <rdar://problem/22526137> Add test to increase code coverage for
   * /dev/random
   */
...
#define CONF_WINDOW_TREND_LIMIT                                                \
  ((CONF_WINDOW_SIZE / 2) + (CONF_WINDOW_SIZE & 1)) >> 0
  ...
  T_PASS("Generated %d non-zero random numbers with atleast one bit reset.",
         CONF_ITERATIONS);
  ...
  for (i = 1; i < CONF_ITERATIONS; i++) {
    ...
    if (bit_entropy < CONF_MIN_ENTROPY) {
      T_EXPECT_GE_UINT(bit_entropy, CONF_MIN_ENTROPY,
                       "Number of differing bits in consecutive numbers does "
                       "not satisfy the min criteria.");
    }
  ...
  T_EXPECT_GE_UINT(mean_bit_entropy, CONF_MEAN_ENTROPY,
                   "Test criteria for mean number of differing bits.");
  T_PASS("Mean bit entropy criteria satisfied (Required %d, Actual: %d).",
         CONF_MEAN_ENTROPY, mean_bit_entropy);
  T_LOG("{PERFORMANCE} iterations: %d, min_bit_entropy: %d, mean_bit_entropy: "
        "%d, max_bit_entropy: %d",
        CONF_ITERATIONS, min_bit_entropy, mean_bit_entropy, max_bit_entropy);
  T_PERF("min_bit_entropy_" T_TOSTRING(CONF_ITERATIONS), min_bit_entropy,
         "bits", "minimum bit entropy in RNG. High is better");
  T_PERF("mean_bit_entropy_" T_TOSTRING(CONF_ITERATIONS), mean_bit_entropy,
         "bits", "mean bit entropy in RNG. High is better");
  T_PERF("max_bit_entropy_" T_TOSTRING(CONF_ITERATIONS), max_bit_entropy,
         "bits", "max bit entropy in RNG. High is better");

  /*
   * TEST 3: Incremental Pattern Search
   * Check that incremental/decremental pattern does not exist in the given
   * window
   */
  ...
  do {
    ...
    if (trend > CONF_WINDOW_TREND_LIMIT) {
      T_ASSERT_LE_INT(trend, CONF_WINDOW_TREND_LIMIT,
  ...
  T_PASS("Did not find increasing/decreasing trends in a window of %d numbers.",
         CONF_WINDOW_SIZE);
   ...
   */
  qsort(numbers, CONF_ITERATIONS, sizeof(numbers[0]),
        compare_numbers_ascending);
  for (i = 1; i < CONF_ITERATIONS; i++) {
    if (numbers[i] == numbers[i - 1]) {
      T_ASSERT_NE_ULLONG(numbers[i], numbers[i - 1],
                         "read_random generated duplicate values.");
    }
  }
...
kern_return_t kcdata_api_test(void) {
  ...
  /* test for NULL input */
  retval = kcdata_memory_static_init(NULL, (mach_vm_address_t)0,
                                     KCDATA_BUFFER_BEGIN_STACKSHOT, 100,
                                     KCFLAG_USE_MEMCOPY);
  T_ASSERT(retval == KERN_INVALID_ARGUMENT,
  ...
  retval = kcdata_memory_static_init(&test_kc_data, (mach_vm_address_t)&data,
                                     KCDATA_BUFFER_BEGIN_CRASHINFO,
                                     sizeof(data), KCFLAG_USE_MEMCOPY);
  T_ASSERT(retval == KERN_INSUFFICIENT_BUFFER_SIZE,
           "init with 30 bytes failed as expected with "
           "KERN_INSUFFICIENT_BUFFER_SIZE");

  /* test with COPYOUT for 0x0 address. Should return KERN_NO_ACCESS */
  retval = kcdata_memory_static_init(&test_kc_data, (mach_vm_address_t)0,
                                     KCDATA_BUFFER_BEGIN_CRASHINFO, PAGE_SIZE,
                                     KCFLAG_USE_COPYOUT);
  ...
  retval = kcdata_memory_static_init(&test_kc_data, (mach_vm_address_t)address,
                                     KCDATA_BUFFER_BEGIN_STACKSHOT, PAGE_SIZE,
                                     KCFLAG_USE_MEMCOPY);

  T_ASSERT(retval == KERN_SUCCESS, "successful kcdata_memory_static_init call");

  T_ASSERT(test_kc_data.kcd_length == PAGE_SIZE,
           "kcdata length is set correctly to PAGE_SIZE.");
  T_LOG("addr_begin 0x%llx and end 0x%llx and address 0x%llx",
        test_kc_data.kcd_addr_begin, test_kc_data.kcd_addr_end, address);
  T_ASSERT(test_kc_data.kcd_addr_begin == address,
  ...
  T_ASSERT(mem[0] == KCDATA_BUFFER_BEGIN_STACKSHOT,
           "buffer does contain KCDATA_BUFFER_BEGIN_STACKSHOT");
  T_ASSERT(mem[4] == KCDATA_TYPE_BUFFER_END,
  ...
  T_ASSERT(bytes_used == (2 * sizeof(struct kcdata_item)),
  ...
  retval = kcdata_get_memory_addr(NULL, KCDATA_TYPE_MACH_ABSOLUTE_TIME,
                                  sizeof(uint64_t), &user_addr);
  T_ASSERT(retval == KERN_INVALID_ARGUMENT,
           "kcdata_get_memory_addr with NULL struct -> KERN_INVALID_ARGUMENT");

  retval = kcdata_get_memory_addr(&test_kc_data, KCDATA_TYPE_MACH_ABSOLUTE_TIME,
                                  sizeof(uint64_t), NULL);
  T_ASSERT(
      retval == KERN_INVALID_ARGUMENT,
      "kcdata_get_memory_addr with NULL user_addr -> KERN_INVALID_ARGUMENT");

  /* successful case with size 0. Yes this is expected to succeed as just a item
   * type could be used as boolean */
  retval = kcdata_get_memory_addr(&test_kc_data, KCDATA_TYPE_USECS_SINCE_EPOCH,
                                  0, &user_addr);
  T_ASSERT(retval == KERN_SUCCESS,
           "Successfully got kcdata entry for 0 size data");
  T_ASSERT(user_addr == test_kc_data.kcd_addr_end,
  ...
  T_ASSERT(user_addr > test_kc_data.kcd_addr_begin,
  ...
  bytes_used = kcdata_memory_get_used_bytes(&test_kc_data);
  retval = kcdata_get_memory_addr(&test_kc_data, KCDATA_TYPE_MACH_ABSOLUTE_TIME,
                                  PAGE_SIZE * 4, &user_addr);
  T_ASSERT(
      retval == KERN_INSUFFICIENT_BUFFER_SIZE,
      "Allocating entry with size > buffer -> KERN_INSUFFICIENT_BUFFER_SIZE");
  T_ASSERT(user_addr == 0xdeadbeef,
           "user_addr remained unaffected with failed kcdata_get_memory_addr");
  T_ASSERT(bytes_used == kcdata_memory_get_used_bytes(&test_kc_data),
           "The data structure should be unaffected");

  /* verify convenience functions for uint32_with_description */
  retval = kcdata_add_uint32_with_description(&test_kc_data, 0xbdc0ffee,
                                              "This is bad coffee");
  T_ASSERT(retval == KERN_SUCCESS, "add uint32 with description succeeded.");

  retval = kcdata_add_uint64_with_description(&test_kc_data, 0xf001badc0ffee,
                                              "another 8 byte no.");
  ...
  retval = kcdata_get_memory_addr_for_array(&test_kc_data,
  ...
  T_ASSERT((kcdata_memory_get_used_bytes(&test_kc_data) - bytes_used) >=
               20 * sizeof(uint64_t),
           "memory allocation is in range");
  kcdata_iter_t iter = kcdata_iter(
      item_p,
      (unsigned long)(PAGE_SIZE - kcdata_memory_get_used_bytes(&test_kc_data)));
  T_ASSERT(kcdata_iter_array_elem_count(iter) == 20, "array count is 20");

  /* FIXME add tests here for ranges of sizes and counts */

  T_ASSERT(item_p->flags ==
               (((uint64_t)KCDATA_TYPE_MACH_ABSOLUTE_TIME << 32) | 20),
           "flags are set correctly");

  /* test adding of custom type */

  retval = kcdata_add_type_definition(
      &test_kc_data, 0x999, data, &test_disk_io_stats_def[0],
      sizeof(test_disk_io_stats_def) /
 ...
 *       T_EXPECT_NOTNULL(address, "kalloc of PAGE_SIZE data.");
 *
 *       retval = kcdata_memory_static_init(&test_kc_data2,
 * (mach_vm_address_t)address, KCDATA_BUFFER_BEGIN_STACKSHOT, PAGE_SIZE,
 *                                          KCFLAG_USE_MEMCOPY);
 *
 *       T_ASSERT(retval == KERN_SUCCESS, "successful kcdata_memory_static_init
 * call");
 *
 *       retval = T_REGISTER_ASSERT_CHECK("KCDATA_DESC_MAXLEN",
 * &assert_check_retval); T_ASSERT(retval == KERN_SUCCESS, "registered assert
 * widget");
 *
 *       // this will assert
 *       retval = kcdata_add_uint32_with_description(&test_kc_data2, 0xc0ffee,
 * "really long description string for kcdata"); T_ASSERT(retval ==
 * KERN_INVALID_ARGUMENT, "API param check returned KERN_INVALID_ARGUMENT
 * correctly"); T_ASSERT(assert_check_retval == (void *)XT_RET_W_SUCCESS,
 * "assertion handler verified that it was hit");
 *
 *       return KERN_SUCCESS;
 *  }
 */
...
static inline uintptr_t astris_vm_page_unpack_ptr(uintptr_t p) {
  if (!p) {
    return (uintptr_t)0;
  }

  return (p & lowGlo.lgPmapMemFromArrayMask)
             ? lowGlo.lgPmapMemStartAddr +
                   (p & ~(lowGlo.lgPmapMemFromArrayMask)) *
                       lowGlo.lgPmapMemPagesize
             : lowGlo.lgPmapMemPackedBaseAddr +
                   (p << lowGlo.lgPmapMemPackedShift);
}

// assume next pointer is the first element
#define astris_vm_page_queue_next(qc)                                          \
  (astris_vm_page_unpack_ptr(*((uint32_t *)(qc))))
...
#define astris_vm_page_queue_iterate(head, elt)                                \
  for ((elt) = (uintptr_t)astris_vm_page_queue_first((head));                  \
       !astris_vm_page_queue_end((head), (elt));                               \
       (elt) = (uintptr_t)astris_vm_page_queue_next(                           \
           ((elt) + (uintptr_t)lowGlo.lgPmapMemChainOffset)))

#define astris_ptoa(x) ((vm_address_t)(x) << lowGlo.lgPageShift)

static inline ppnum_t astris_vm_page_get_phys_page(uintptr_t m) {
  return (m >= lowGlo.lgPmapMemStartAddr && m < lowGlo.lgPmapMemEndAddr)
             ? (ppnum_t)((m - lowGlo.lgPmapMemStartAddr) /
                             lowGlo.lgPmapMemPagesize +
                         lowGlo.lgPmapMemFirstppnum)
...
kern_return_t pmap_coredump_test(void) {
  ...
  T_ASSERT_LE_ULONG(lowGlo.lgStaticAddr + lowGlo.lgStaticSize, first_avail,
                    NULL);
  ...
  T_ASSERT_EQ_ULONG(lowGlo.lgPmapMemQ,
                    ((typeof(lowGlo.lgPmapMemQ))&(pmap_object_store.memq)),
                    NULL);
  T_ASSERT_EQ_ULONG(lowGlo.lgPmapMemPageOffset,
                    offsetof(struct vm_page_with_ppnum, vmp_phys_page), NULL);
  T_ASSERT_EQ_ULONG(lowGlo.lgPmapMemChainOffset,
                    offsetof(struct vm_page, vmp_listq), NULL);
  T_ASSERT_EQ_ULONG(lowGlo.lgPmapMemPagesize, sizeof(struct vm_page), NULL);

#if defined(__arm64__)
  T_ASSERT_EQ_ULONG(lowGlo.lgPmapMemFromArrayMask, VM_PAGE_PACKED_FROM_ARRAY,
                    NULL);
  T_ASSERT_EQ_ULONG(lowGlo.lgPmapMemPackedShift, VM_PAGE_PACKED_PTR_SHIFT,
                    NULL);
  T_ASSERT_EQ_ULONG(lowGlo.lgPmapMemPackedBaseAddr, VM_PAGE_PACKED_PTR_BASE,
                    NULL);
#endif
...
static void wait_threads(int *var, int num) {
  if (var != NULL) {
...
static void wake_threads(int *var) {
  if (var) {
...
static void thread_lock_unlock_kernel_primitive(void *args,
  ...
  if (info->priority_to_check) {
    ...
    splx(s);
    T_ASSERT(pri == info->priority_to_check,
             "Priority thread: current sched %d sched wanted %d", pri,
             info->priority_to_check);
  }
...
kern_return_t ts_kernel_primitive_test(void) {
  ...
  result = kernel_thread_start_priority(
      (thread_continue_t)thread_lock_unlock_kernel_primitive, &targs[0], 80,
      &owner);
  ...
  result = kernel_thread_start_priority(
      (thread_continue_t)thread_lock_unlock_kernel_primitive, &targs[1], 85,
      &thread1);
  T_ASSERT(result == KERN_SUCCESS, "Starting thread1");

  result = kernel_thread_start_priority(
      (thread_continue_t)thread_lock_unlock_kernel_primitive, &targs[1], 90,
      &thread2);
...
static kern_return_t init_synch_test_common(struct synch_test_common *info,
                                            unsigned int nthreads) {
...
static void destroy_synch_test_common(struct synch_test_common *info) {
  kfree_type(thread_t, info->nthreads, info->threads);
}

static void start_threads(thread_continue_t func,
                          struct synch_test_common *info,
  ...
  for (i = 0; i < info->nthreads; i++) {
    result = kernel_thread_start_priority((thread_continue_t)func, info,
                                          priority, &thread);
    os_atomic_store(&info->threads[i], thread, release);
    T_ASSERT(result == KERN_SUCCESS, "Starting thread %d, priority %d, %p", i,
             priority, thread);
...
static unsigned int get_max_pri(struct synch_test_common *info) {
  return info->max_pri;
}

static void wait_all_thread(struct synch_test_common *info) {
  wait_threads(&info->test_done, info->nthreads);
}

static void notify_waiter(struct synch_test_common *info) {
  wake_threads(&info->test_done);
}

static void wait_for_waiters(struct synch_test_common *info) {
  uint i, j;
  thread_t thread;
...
static void exclude_current_waiter(struct synch_test_common *info) {
  uint i, j;
...
static void primitive_lock(struct info_sleep_inheritor_test *info) {
...
static void primitive_unlock(struct info_sleep_inheritor_test *info) {
  switch (info->prim_type) {
  case MTX_LOCK:
...
primitive_sleep_with_inheritor(struct info_sleep_inheritor_test *info) {
  ...
  switch (info->prim_type) {
  case MTX_LOCK:
    ret = lck_mtx_sleep_with_inheritor(
        &info->mtx_lock, LCK_SLEEP_DEFAULT, (event_t)&info->thread_inheritor,
        info->thread_inheritor, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
        TIMEOUT_WAIT_FOREVER);
    break;
  case RW_LOCK:
    ret = lck_rw_sleep_with_inheritor(
        &info->rw_lock, LCK_SLEEP_DEFAULT, (event_t)&info->thread_inheritor,
        info->thread_inheritor, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
        TIMEOUT_WAIT_FOREVER);
    break;
  default:
...
primitive_wakeup_one_with_inheritor(struct info_sleep_inheritor_test *info) {
  ...
  case MTX_LOCK:
  case RW_LOCK:
    wakeup_one_with_inheritor((event_t)&info->thread_inheritor, THREAD_AWAKENED,
                              LCK_WAKE_DEFAULT, &info->thread_inheritor);
    break;
  default:
    panic("invalid type %d", info->prim_type);
  }
...
primitive_wakeup_all_with_inheritor(struct info_sleep_inheritor_test *info) {
  ...
  case MTX_LOCK:
  case RW_LOCK:
    wakeup_all_with_inheritor((event_t)&info->thread_inheritor,
                              THREAD_AWAKENED);
    break;
  default:
    panic("invalid type %d", info->prim_type);
...
primitive_change_sleep_inheritor(struct info_sleep_inheritor_test *info) {
  ...
  case MTX_LOCK:
  case RW_LOCK:
    change_sleep_inheritor((event_t)&info->thread_inheritor,
                           info->thread_inheritor);
    break;
  default:
    panic("invalid type %d", info->prim_type);
  }
...
primitive_gate_wait(struct info_sleep_inheritor_test *info) {
  ...
  switch (info->prim_type) {
  case MTX_LOCK:
    ret = lck_mtx_gate_wait(&info->mtx_lock, gate, LCK_SLEEP_DEFAULT,
                            THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
                            TIMEOUT_WAIT_FOREVER);
    break;
  case RW_LOCK:
    ret = lck_rw_gate_wait(&info->rw_lock, gate, LCK_SLEEP_DEFAULT,
                           THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
                           TIMEOUT_WAIT_FOREVER);
    break;
  default:
...
static void primitive_gate_open(struct info_sleep_inheritor_test *info) {
  gate_t *gate = &info->gate;
  if (info->use_alloc_gate == true) {
    gate = info->alloc_gate;
  }
...
static void primitive_gate_close(struct info_sleep_inheritor_test *info) {
  gate_t *gate = &info->gate;
  if (info->use_alloc_gate == true) {
    gate = info->alloc_gate;
  }
...
static void primitive_gate_steal(struct info_sleep_inheritor_test *info) {
  gate_t *gate = &info->gate;
  if (info->use_alloc_gate == true) {
    gate = info->alloc_gate;
  }
...
static void primitive_gate_assert(struct info_sleep_inheritor_test *info,
                                  int type) {
  gate_t *gate = &info->gate;
  if (info->use_alloc_gate == true) {
    gate = info->alloc_gate;
...
static void primitive_gate_init(struct info_sleep_inheritor_test *info) {
  switch (info->prim_type) {
  case MTX_LOCK:
...
static void primitive_gate_destroy(struct info_sleep_inheritor_test *info) {
  switch (info->prim_type) {
  case MTX_LOCK:
...
static void primitive_gate_alloc(struct info_sleep_inheritor_test *info) {
  gate_t *gate;
  switch (info->prim_type) {
  case MTX_LOCK:
...
static void primitive_gate_free(struct info_sleep_inheritor_test *info) {
...
static void thread_inheritor_like_mutex(void *args, __unused wait_result_t wr) {
  wait_result_t wait;

  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  ...
  if (info->thread_inheritor == NULL) {
    ...
    T_ASSERT(wait == THREAD_AWAKENED || wait == THREAD_NOT_WAITING,
  ...
  T_ASSERT(info->thread_inheritor == current_thread(), "thread_inheritor is %p",
           info->thread_inheritor);
  ...
  if (info->thread_inheritor == NULL) {
    ...
    T_ASSERT(info->thread_inheritor != current_thread(),
             "thread_inheritor is %p", info->thread_inheritor);
...
static void thread_just_inheritor_do_work(void *args,
                                          __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  uint my_pri = current_thread()->sched_pri;
  ...
  if (info->thread_inheritor == NULL) {
    ...
    T_ASSERT((uint)current_thread()->sched_pri == max_pri,
             "sleep_inheritor inheritor priority current is %d, should be %d",
             current_thread()->sched_pri, max_pri);
...
static void thread_steal_work(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  uint my_pri = current_thread()->sched_pri;
  ...
  if (info->thread_inheritor == NULL) {
    ...
    if (info->steal_pri == 0) {
      ...
      T_ASSERT((uint)current_thread()->sched_pri == info->steal_pri,
               "sleep_inheritor inheritor priority current is %d, should be %d",
               current_thread()->sched_pri, info->steal_pri);
...
static void thread_no_inheritor_work(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  uint my_pri = current_thread()->sched_pri;
...
static void thread_mtx_work(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  uint my_pri = current_thread()->sched_pri;
  int i;
  ...
  for (i = 0; i < 10; i++) {
    ...
    case 1:
      lck_mtx_sleep_with_inheritor(
          &info->mtx_lock, LCK_SLEEP_UNLOCK, (event_t)&info->thread_inheritor,
          info->thread_inheritor, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
          TIMEOUT_WAIT_FOREVER);
      break;
    default:
  ...
  for (i = 0; i < 10; i++) {
    ...
    case 0:
      lck_mtx_sleep_with_inheritor(
          &info->mtx_lock, LCK_SLEEP_SPIN, (event_t)&info->thread_inheritor,
          info->thread_inheritor, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
          TIMEOUT_WAIT_FOREVER);
      lck_mtx_unlock(&info->mtx_lock);
      break;
    case 1:
      lck_mtx_sleep_with_inheritor(
          &info->mtx_lock, LCK_SLEEP_SPIN_ALWAYS,
          (event_t)&info->thread_inheritor, info->thread_inheritor,
...
static void thread_rw_work(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  uint my_pri = current_thread()->sched_pri;
  int i;
  ...
  try_again:
    ...
    if (info->thread_inheritor == NULL) {
      ...
      if (lck_rw_lock_shared_to_exclusive(&info->rw_lock)) {
        if (info->thread_inheritor == NULL) {
          ...
          max_pri = get_max_pri((struct synch_test_common *)info);
          T_ASSERT(
              (uint)current_thread()->sched_pri == max_pri,
              "sleep_inheritor inheritor priority current is %d, should be %d",
              current_thread()->sched_pri, max_pri);
    ...
    case 1:
      lck_rw_sleep_with_inheritor(
          &info->rw_lock, LCK_SLEEP_UNLOCK, (event_t)&info->thread_inheritor,
          info->thread_inheritor, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
          TIMEOUT_WAIT_FOREVER);
      break;
    case 2:
      lck_rw_sleep_with_inheritor(
          &info->rw_lock, LCK_SLEEP_SHARED, (event_t)&info->thread_inheritor,
          info->thread_inheritor, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
          TIMEOUT_WAIT_FOREVER);
    ...
    case 3:
      lck_rw_sleep_with_inheritor(
          &info->rw_lock, LCK_SLEEP_EXCLUSIVE, (event_t)&info->thread_inheritor,
          info->thread_inheritor, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
          TIMEOUT_WAIT_FOREVER);
      lck_rw_unlock(&info->rw_lock, LCK_RW_TYPE_EXCLUSIVE);
      break;
...
static struct obj_cached *alloc_init_cache_entry(void) {
  struct obj_cached *cache_entry =
      kalloc_type(struct obj_cached, 1, Z_WAITOK | Z_NOFAIL | Z_ZERO);
...
static void init_cache(struct info_sleep_inheritor_test *info) {
  struct obj_cached **obj_cache = kalloc_type(struct obj_cached *, CACHE_SIZE,
...
static void check_cache_empty(struct info_sleep_inheritor_test *info) {
  struct obj_cached **obj_cache = info->obj_cache;

  int i, ret;
  for (i = 0; i < CACHE_SIZE; i++) {
    if (obj_cache[i] != NULL) {
      T_ASSERT(obj_cache[i]->obj_state == OBJ_STATE_UNUSED,
               "checked OBJ_STATE_UNUSED");
...
static void free_cache(struct info_sleep_inheritor_test *info) {
  struct obj_cached **obj_cache = info->obj_cache;
...
static bool free_id_in_cache(int obj_id, struct info_sleep_inheritor_test *info,
                             __assert_only struct obj_cached *expected) {
  struct obj_cached **obj_cache = info->obj_cache;
  int i;
  for (i = 0; i < CACHE_SIZE; i++) {
    if (obj_cache[i] != NULL && obj_cache[i]->obj_id == obj_id) {
...
static int get_obj_cache(int obj_id, struct info_sleep_inheritor_test *info,
                         char **buff) {
...
try_again:
  ...
  if ((obj = find_id_in_cache(obj_id, info)) != NULL) {
    ...
    if (kern_apfs_reflock_try_get_ref(refcount, KERN_APFS_REFLOCK_IN_WILL_WAIT,
                                      &out_flags)) {
      /*
      ...
      case OBJ_STATE_PLACEHOLDER:
        panic("Thread %p observed OBJ_STATE_PLACEHOLDER %d for obj %d",
              current_thread(), obj->obj_state, obj_id);
      default:
        panic("Thread %p observed an unknown obj_state %d for obj %d",
              current_thread(), obj->obj_state, obj_id);
      }
    } else {
      /*
       ...
       */
      primitive_unlock(info);
      kern_apfs_reflock_wait_for_unlock(
          refcount, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
          TIMEOUT_WAIT_FOREVER);
      goto try_again;
    }
  } else {
    ...
    if ((obj = find_empty_spot_in_cache(info)) == NULL) {
       ...
       */
      refcount = obj->obj_refcount;
      if (kern_apfs_reflock_try_get_ref(
              refcount, KERN_APFS_REFLOCK_IN_WILL_WAIT, &out_flags)) {
        /*
...
init:
   ...
   */
  ret =
      kern_apfs_reflock_try_lock(refcount, KERN_APFS_REFLOCK_IN_DEFAULT, NULL);
  assert(ret == true);
...
static void put_obj_cache(int obj_id, struct info_sleep_inheritor_test *info,
                          bool free) {
  struct obj_cached *obj = NULL, *obj2 = NULL;
  bool ret;
  kern_apfs_reflock_out_flags_t out_flags;
   ...
   * to OBJ_STATE_REAL.
   */
  ret = kern_apfs_reflock_try_put_ref(
      refcount, KERN_APFS_REFLOCK_IN_LOCK_IF_LAST, &out_flags);
  assert(ret == true);
...
static void thread_use_cache(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  int my_obj;
  ...
  T_LOG("Thread %p started and it is going to use obj %d", current_thread(),
        my_obj);
  /*
  ...
  for (int i = 0; i < USE_CACHE_ROUNDS; i++) {
    ...
    T_ASSERT(memcmp(buff, my_string, my_string_size) == 0,
             "reflock: thread %p obj_id %d value in buff", current_thread(),
             my_obj);
    IOSleep(10);
    T_ASSERT(memcmp(buff, my_string, my_string_size) == 0,
             "reflock: thread %p obj_id %d value in buff", current_thread(),
...
static void thread_refcount_reflock(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  bool ret;
  kern_apfs_reflock_out_flags_t out_flags;
  ...
  for (int i = 0; i < REFCOUNT_REFLOCK_ROUNDS; i++) {
    ...
    if (ret == true) {
      ...
      if ((out_flags & KERN_APFS_REFLOCK_OUT_LOCKED) ==
          KERN_APFS_REFLOCK_OUT_LOCKED) {
      ...
      ret = kern_apfs_reflock_try_put_ref(
          &info->reflock, KERN_APFS_REFLOCK_IN_LOCK_IF_LAST, &out_flags);
      ...
      if ((in_flags & KERN_APFS_REFLOCK_IN_WILL_WAIT) ==
          KERN_APFS_REFLOCK_IN_WILL_WAIT) {
        kern_apfs_reflock_wait_for_unlock(
            &info->reflock, THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
            TIMEOUT_WAIT_FOREVER);
      }
    }
  }
...
static void thread_force_reflock(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  bool ret;
  kern_apfs_reflock_out_flags_t out_flags;
  ...
  if (os_atomic_inc_orig(&info->value, relaxed) == 0) {
    ...
    lock = true;
    ret = kern_apfs_reflock_try_lock(&info->reflock,
                                     KERN_APFS_REFLOCK_IN_ALLOW_FORCE, &count);
  ...
  if (lock) {
    ...
    for (int i = 0; i < REFCOUNT_REFLOCK_ROUNDS; i++) {
      ret = kern_apfs_reflock_try_get_ref(
          &info->reflock, KERN_APFS_REFLOCK_IN_FORCE, &out_flags);
...
static void thread_lock_reflock(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  bool ret;
  kern_apfs_reflock_out_flags_t out_flags;
  ...
  if (os_atomic_inc_orig(&info->value, relaxed) == 0) {
    ...
    lock = true;
    ret = kern_apfs_reflock_try_lock(&info->reflock,
                                     KERN_APFS_REFLOCK_IN_DEFAULT, &count);
    T_ASSERT(ret == true, "kern_apfs_reflock_try_lock success");
    T_ASSERT(count == 0, "refcount value");
    info->reflock_protected_status = 1;
  }
  ...
  if (lock) {
    ...
    for (int i = 0; i < REFCOUNT_REFLOCK_ROUNDS; i++) {
      ret = kern_apfs_reflock_try_get_ref(
          &info->reflock, KERN_APFS_REFLOCK_IN_DEFAULT, &out_flags);
      if (ret == true) {
...
static void test_cache_reflock(struct info_sleep_inheritor_test *info) {
  info->synch = 0;
  ...
  start_threads((thread_continue_t)thread_use_cache,
                (struct synch_test_common *)info, FALSE);
...
static void test_refcount_reflock(struct info_sleep_inheritor_test *info) {
  info->synch = 0;
  info->synch_value = info->head.nthreads;
  kern_apfs_reflock_init(&info->reflock);
  info->reflock_protected_status = 0;

  start_threads((thread_continue_t)thread_refcount_reflock,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
...
static void test_force_reflock(struct info_sleep_inheritor_test *info) {
  info->synch = 0;
  info->synch_value = info->head.nthreads;
  kern_apfs_reflock_init(&info->reflock);
  info->value = 0;

  start_threads((thread_continue_t)thread_force_reflock,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
...
static void test_lock_reflock(struct info_sleep_inheritor_test *info) {
  info->synch = 0;
  info->synch_value = info->head.nthreads;
  kern_apfs_reflock_init(&info->reflock);
  info->value = 0;

  start_threads((thread_continue_t)thread_lock_reflock,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
...
static void test_sleep_with_wake_all(struct info_sleep_inheritor_test *info,
                                     int prim_type) {
  ...
  start_threads((thread_continue_t)thread_just_inheritor_do_work,
                (struct synch_test_common *)info, TRUE);
  wait_all_thread((struct synch_test_common *)info);
}

static void test_sleep_with_wake_one(struct info_sleep_inheritor_test *info,
                                     int prim_type) {
  info->prim_type = prim_type;
  ...
  start_threads((thread_continue_t)thread_inheritor_like_mutex,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
...
static void test_change_sleep_inheritor(struct info_sleep_inheritor_test *info,
                                        int prim_type) {
  info->prim_type = prim_type;
  ...
  start_threads((thread_continue_t)thread_steal_work,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
}

static void test_no_inheritor(struct info_sleep_inheritor_test *info,
                              int prim_type) {
  info->prim_type = prim_type;
  info->synch = 0;
  info->synch_value = info->head.nthreads;
  ...
  start_threads((thread_continue_t)thread_no_inheritor_work,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
}

static void test_rw_lock(struct info_sleep_inheritor_test *info) {
  info->thread_inheritor = NULL;
  info->value = info->head.nthreads;
  info->synch = 0;
  info->synch_value = info->head.nthreads;

  start_threads((thread_continue_t)thread_rw_work,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
}

static void test_mtx_lock(struct info_sleep_inheritor_test *info) {
  info->thread_inheritor = NULL;
  info->value = info->head.nthreads;
  info->synch = 0;
  info->synch_value = info->head.nthreads;

  start_threads((thread_continue_t)thread_mtx_work,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
}
...
static void thread_gate_aggressive(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  uint my_pri = current_thread()->sched_pri;
  ...
  if (info->thread_inheritor == NULL) {
    ...
    if (info->steal_pri == 0) {
      ...
      wait_for_waiters((struct synch_test_common *)info);
      T_ASSERT((uint)current_thread()->sched_pri == info->steal_pri,
               "gate keeper priority current is %d, should be %d",
               current_thread()->sched_pri, info->steal_pri);
...
static void thread_gate_free(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  uint my_pri = current_thread()->sched_pri;
...
static void thread_gate_like_mutex(void *args, __unused wait_result_t wr) {
...
static void thread_just_one_do_work(void *args, __unused wait_result_t wr) {
  struct info_sleep_inheritor_test *info =
      (struct info_sleep_inheritor_test *)args;
  uint my_pri = current_thread()->sched_pri;
  uint max_pri;
...
check_again:
  if (info->work_to_do) {
    if (primitive_gate_try_close(info) == KERN_SUCCESS) {
      ...
      max_pri = get_max_pri((struct synch_test_common *)info);
      T_ASSERT((uint)current_thread()->sched_pri == max_pri,
               "gate owner priority current is %d, should be %d",
               current_thread()->sched_pri, max_pri);
      os_atomic_store(&info->synch, 0, relaxed);
...
static void test_gate_push(struct info_sleep_inheritor_test *info,
                           int prim_type) {
  info->prim_type = prim_type;
  ...
  start_threads((thread_continue_t)thread_just_one_do_work,
                (struct synch_test_common *)info, TRUE);
  wait_all_thread((struct synch_test_common *)info);
...
static void test_gate_handoff(struct info_sleep_inheritor_test *info,
                              int prim_type) {
  info->prim_type = prim_type;
  info->use_alloc_gate = false;
  ...
  start_threads((thread_continue_t)thread_gate_like_mutex,
                (struct synch_test_common *)info, false);
  wait_all_thread((struct synch_test_common *)info);
...
static void test_gate_steal(struct info_sleep_inheritor_test *info,
                            int prim_type) {
  info->prim_type = prim_type;
  info->use_alloc_gate = false;
  ...
  start_threads((thread_continue_t)thread_gate_aggressive,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
...
static void test_gate_alloc_free(struct info_sleep_inheritor_test *info,
                                 int prim_type) {
  (void)info;
  (void)prim_type;
  info->prim_type = prim_type;
  ...
  start_threads((thread_continue_t)thread_gate_free,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
...
kern_return_t ts_kernel_gate_test(void) {
  ...
  /*
   * Testing the steal
   * lck_mtx_gate_close, lck_mtx_gate_wait, lck_mtx_gate_steal,
   * lck_mtx_gate_handoff
   */
   ...
   * lck_mtx_gate_alloc_init, lck_mtx_gate_close, lck_mtx_gate_wait,
   * lck_mtx_gate_free
   */
...
static void thread_sleep_gate_chain_work(void *args,
                                         __unused wait_result_t wr) {
  ...
  while (os_atomic_load(&info->head.threads[info->head.nthreads - 1],
                        acquire) == NULL) {
    IOSleep(10);
  }
  ...
  if (self == os_atomic_load(&info->head.threads[0], acquire)) {
    ...
    T_ASSERT((uint)self->sched_pri == max_pri,
             "sleep_inheritor inheritor priority current is %d, should be %d",
             self->sched_pri, max_pri);
    ...
    if (i % 2 != 0) {
      lck_mtx_gate_wait(&info->mtx_lock, &info->gates[i - 1], LCK_SLEEP_UNLOCK,
                        THREAD_UNINT | THREAD_WAIT_NOREPORT_USER,
                        TIMEOUT_WAIT_FOREVER);
      T_ASSERT((uint)self->sched_pri == max_pri,
               "sleep_inheritor inheritor priority current is %d, should be %d",
      ...
      ret = wakeup_one_with_inheritor(wake_event, THREAD_AWAKENED,
                                      LCK_WAKE_DO_NOT_TRANSFER_PUSH, &woken_up);
      if (ret == KERN_SUCCESS) {
        T_ASSERT(i != (info->head.nthreads - 1), "thread id");
        T_ASSERT(woken_up == info->head.threads[i + 1],
      ...
      lck_mtx_sleep_with_inheritor(
          &info->mtx_lock, LCK_SLEEP_UNLOCK, wait_event, inheritor,
          THREAD_UNINT | THREAD_WAIT_NOREPORT_USER, TIMEOUT_WAIT_FOREVER);
      T_ASSERT((uint)self->sched_pri == max_pri,
               "sleep_inheritor inheritor priority current is %d, should be %d",
               self->sched_pri, max_pri);
...
static void thread_gate_chain_work(void *args, __unused wait_result_t wr) {
  struct turnstile_chain_test *info = (struct turnstile_chain_test *)args;
  thread_t self = current_thread();
  uint my_pri = self->sched_pri;
  uint max_pri;
...
static void thread_sleep_chain_work(void *args, __unused wait_result_t wr) {
  struct turnstile_chain_test *info = (struct turnstile_chain_test *)args;
  thread_t self = current_thread();
  uint my_pri = self->sched_pri;
  uint max_pri;
  ...
  if (self == os_atomic_load(&info->head.threads[0], acquire)) {
    ...
    ret = wakeup_one_with_inheritor((event_t)&info->head.threads[0],
                                    THREAD_AWAKENED,
                                    LCK_WAKE_DO_NOT_TRANSFER_PUSH, &woken_up);
    ...
    // i am still the inheritor, wake all to drop inheritership
    ret = wakeup_all_with_inheritor((event_t)&info->head.threads[0],
                                    LCK_WAKE_DEFAULT);
    T_ASSERT(ret == KERN_NOT_WAITING, "waiters on event");
  } else {
    wait_event = NULL;
...
static void test_sleep_chain(struct turnstile_chain_test *info) {
  info->synch = 0;
  info->synch_value = info->head.nthreads;

  start_threads((thread_continue_t)thread_sleep_chain_work,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
}

static void test_gate_chain(struct turnstile_chain_test *info) {
  info->synch = 0;
  info->synch2 = 0;
  info->synch_value = info->head.nthreads;

  start_threads((thread_continue_t)thread_gate_chain_work,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
}

static void test_sleep_gate_chain(struct turnstile_chain_test *info) {
  info->synch = 0;
  info->synch2 = 0;
  info->synch_value = info->head.nthreads;

  start_threads((thread_continue_t)thread_sleep_gate_chain_work,
                (struct synch_test_common *)info, FALSE);
  wait_all_thread((struct synch_test_common *)info);
}
...
kern_return_t kprintf_hhx_test(void) {
  printf("POST hhx test %hx%hx%hx%hx %hhx%hhx%hhx%hhx - %llx",
         (unsigned short)0xfeed, (unsigned short)0xface, (unsigned short)0xabad,
         (unsigned short)0xcafe, (unsigned char)'h', (unsigned char)'h',
         (unsigned char)'x', (unsigned char)'!', 0xfeedfaceULL);
...
__static_if_init_func static void
...
static void static_if_tests(void) {
  int n = 0;
...
/* these functions are used for testing the unittest mocking framework and
 * interposing */

__mockable size_t kernel_func1(__unused int a, __unused char b) { return 1000; }
__mockable size_t kernel_func2(__unused int a, __unused char b) { return 2000; }
__mockable size_t kernel_func3(__unused int a, __unused char b) { return 3000; }
__mockable size_t kernel_func4(__unused int a, __unused char b) { return 4000; }
__mockable size_t kernel_func5(__unused int a, __unused char b) { return 5000; }
int kernel_func6_was_called = 0;
__mockable void kernel_func6(__unused int a, __unused char b) {
  printf("in void func6");
  kernel_func6_was_called = a;
}
__mockable size_t kernel_func7(__unused int a, __unused char b) { return 7000; }
int kernel_func8_was_called = 0;
__mockable void kernel_func8(__unused int a, __unused char b) {
