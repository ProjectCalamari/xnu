...
uint64_t test_pmap_page_protect_overhead(unsigned int num_loops,
                                         unsigned int num_aliases);
#if CONFIG_SPTM
kern_return_t test_pmap_huge_pv_list(unsigned int num_loops,
 ...
 */
static pmap_t pmap_create_wrapper(unsigned int flags) {
 ...
 */
static vm_page_t pmap_test_alloc_vm_page(void) {
...
/**
 * Helper for freeing a VM page previously allocated by
 * pmap_test_alloc_vm_page().
 *
 * @param m The page to free.  This may be NULL, in which case this function
 * will do nothing.
 */
static void pmap_test_free_vm_page(vm_page_t m) {
  if (m != VM_PAGE_NULL) {
    vm_page_lock_queues();
    vm_page_free(m);
    vm_page_unlock_queues();
...
static void pmap_disconnect_thread(void *arg, wait_result_t __unused wres) {
...
kern_return_t test_pmap_enter_disconnect(unsigned int num_loops) {
  ...
  pmap_test_thread_args args = {
      .pmap = new_pmap, .stop = FALSE, .pn = phys_page};
  kern_return_t res = kernel_thread_start_priority(
      pmap_disconnect_thread, &args, thread_kern_get_pri(current_thread()),
      &disconnect_thread);
  if (res) {
    pmap_destroy(new_pmap);
...
static void pmap_remove_thread(void *arg, wait_result_t __unused wres) {
  pmap_test_thread_args *args = arg;
  do {
    __assert_only kern_return_t kr = pmap_enter_options(
        args->pmap, args->va, args->pn, VM_PROT_READ, VM_PROT_NONE,
        VM_WIMG_USE_DEFAULT, FALSE, PMAP_OPTIONS_INTERNAL, NULL,
...
kern_return_t test_pmap_compress_remove(unsigned int num_loops) {
  ...
  ppnum_t phys_page = VM_PAGE_GET_PHYS_PAGE(m);
  pmap_test_thread_args args = {
      .pmap = new_pmap, .stop = FALSE, .va = PMAP_TEST_VA, .pn = phys_page};
  kern_return_t res = kernel_thread_start_priority(
      pmap_remove_thread, &args, thread_kern_get_pri(current_thread()),
      &remove_thread);
  if (res) {
    pmap_destroy(new_pmap);
    pmap_test_free_vm_page(m);
...
kern_return_t test_pmap_exec_remove(unsigned int num_loops __unused) {
...
static const vm_map_address_t initial_unnest_size =
    nesting_size - final_unnest_size;
static const vm_map_address_t trimmed_start =
    nesting_start + ARM_16K_TT_L2_SIZE;
static const vm_map_address_t trimmed_size =
    nesting_size - (3 * ARM_16K_TT_L2_SIZE);

static void pmap_nest_thread(void *arg, wait_result_t __unused wres) {
  ...
  /**
   * Exercise nesting and unnesting while bound to the specified CPU (if
   * non-NULL). The unnesting size here should match the unnesting size used in
   * the first unnesting step of the main thread, in order to avoid concurrently
   * unnesting beyond that region and violating the checks against
   * over-unnesting performed in the main thread.
   */
  if (main_pmap != NULL) {
...
/**
 * Test that pmap_nest() and pmap_unnest() work correctly when executed
 * concurrently from multiple threads.  Spawn some worker threads at elevated
 * priority and bound to the same CPU in order to provoke preemption of the
 * nest/unnest operation.
 *
 * @param num_loops The number of nest/unnest loops to perform.  This should be
 * kept to a small number because each cycle is expensive and may consume a
 * global shared region ID.
 *
 * @return KERN_SUCCESS if all tests succeed, an appropriate error code
 * otherwise.
 */
kern_return_t test_pmap_nesting(unsigned int num_loops) {
  kern_return_t kr = KERN_SUCCESS;
  ...
  for (unsigned int i = 0; (i < num_loops) && (kr == KERN_SUCCESS); i++) {
    ...
    for (vm_map_address_t va = trimmed_start;
         va < (trimmed_start + trimmed_size); va += PAGE_SIZE) {
      uint8_t rand;
      ...
      kr = pmap_enter(nested_pmap, va, (rand_mod == 1) ? pp1 : pp2,
                      VM_PROT_READ, VM_PROT_NONE, VM_WIMG_USE_DEFAULT, FALSE,
                      PMAP_MAPPING_TYPE_INFER);
      assert(kr == KERN_SUCCESS);
    }
    ...
    /* Validate the initial nest operation produced global mappings within the
     * nested pmap. */
    for (vm_map_address_t va = nesting_start;
         va < (nesting_start + nesting_size); va += PAGE_SIZE) {
      pt_entry_t *nested_pte = pmap_pte(nested_pmap, va);
      pt_entry_t *main_pte = pmap_pte(main_pmap, va);
      if (nested_pte != main_pte) {
        panic("%s: nested_pte (%p) is not identical to main_pte (%p) for va "
              "0x%llx",
              __func__, nested_pte, main_pte, (unsigned long long)va);
      }
      if ((nested_pte != NULL) && (*nested_pte != ARM_PTE_EMPTY) &&
          (*nested_pte & ARM_PTE_NG)) {
        panic("%s: nested_pte (%p) is not global for va 0x%llx", __func__,
              nested_pte, (unsigned long long)va);
      }
    }
    ...
    /**
     * Validate that the trimmed-off regions at the beginning and end no longer
     * have L3 tables in the main or nested pmaps.
     */
    if (pmap_pte(main_pmap, nesting_start) != NULL) {
      panic("%s: L3 table still present in main pmap for trimmed VA 0x%llx",
            __func__, (unsigned long long)nesting_start);
    }
    if (pmap_pte(main_pmap, trimmed_start + trimmed_size) != NULL) {
      panic("%s: L3 table still present in main pmap for trimmed VA 0x%llx",
            __func__, (unsigned long long)(trimmed_start + trimmed_size));
    }
    if (pmap_pte(nested_pmap, nesting_start) != NULL) {
      panic("%s: L3 table still present in nested pmap for trimmed VA 0x%llx",
            __func__, (unsigned long long)nesting_start);
    }
    ...
    /* Now kick off various worker threads to concurrently nest, trim, and
     * unnest. */
...
#define TEST_NEST_THREADS MAX_CPUS - 1
    ...
    for (unsigned int j = 0;
         j < (sizeof(nest_threads) / sizeof(nest_threads[0])); j++) {
      args[j].pmap = nested_pmap;
      args[j].stop = FALSE;
      /**
       * Spawn the worker threads at various priorities at the high end of the
       * kernel range, and bind every other thread to the same CPU as this
       * thread to provoke preemption, while also allowing some threads to run
       * concurrently on other CPUs.
       */
      args[j].proc = ((j % 2) ? PROCESSOR_NULL : nest_proc);
      thread_krs[j] = kernel_thread_start_priority(pmap_nest_thread, &args[j],
                                                   MAXPRI_KERNEL - (j % 4),
                                                   &nest_threads[j]);
    ...
    /* Unnest the bulk of the nested region and validate that it produced the
     * expected PTE contents. */
    kr = pmap_unnest(main_pmap, nesting_start, initial_unnest_size);
    assert(kr == KERN_SUCCESS);

    /**
     * Explicitly install a new mapping in the nested pmap after unnesting; this
     * should be created non-global, which we'll verify below.
     */
    kr = pmap_enter(nested_pmap, trimmed_start, pp1, VM_PROT_READ, VM_PROT_NONE,
                    VM_WIMG_USE_DEFAULT, FALSE, PMAP_MAPPING_TYPE_INFER);
    assert(kr == KERN_SUCCESS);

    for (vm_map_address_t va = trimmed_start;
         va < (nesting_start + initial_unnest_size); va += PAGE_SIZE) {
      pt_entry_t *nested_pte = pmap_pte(nested_pmap, va);
      pt_entry_t *main_pte = pmap_pte(main_pmap, va);

      if (main_pte != NULL) {
        panic("%s: main_pte (%p) is not NULL for unnested VA 0x%llx", __func__,
              main_pte, (unsigned long long)va);
      }
      if ((nested_pte != NULL) && (*nested_pte != ARM_PTE_EMPTY) &&
          !(*nested_pte & ARM_PTE_NG)) {
    ...
    for (vm_map_address_t va = nesting_start + initial_unnest_size;
         va < (trimmed_start + trimmed_size); va += PAGE_SIZE) {
      pt_entry_t *nested_pte = pmap_pte(nested_pmap, va);
      pt_entry_t *main_pte = pmap_pte(main_pmap, va);
      if (nested_pte != main_pte) {
        panic("%s: nested_pte (%p) is not identical to main_pte (%p) for va "
              "0x%llx following adjacent unnest",
              __func__, nested_pte, main_pte, (unsigned long long)va);
      }
      if ((nested_pte != NULL) && (*nested_pte != ARM_PTE_EMPTY) &&
          (*nested_pte & ARM_PTE_NG)) {
        panic("%s: nested_pte (%p) is not global for va 0x%llx following "
              "adjacent unnest",
              __func__, nested_pte, (unsigned long long)va);
      }
    }
...
kern_return_t test_pmap_nesting(unsigned int num_loops __unused) {
  return KERN_NOT_SUPPORTED;
}

#endif /* defined(__arm64__) */

kern_return_t test_pmap_iommu_disconnect(void) { return KERN_SUCCESS; }

kern_return_t test_pmap_extended(void) { return KERN_SUCCESS; }

void test_pmap_call_overhead(unsigned int num_loops __unused) {
#if defined(__arm64__)
...
uint64_t test_pmap_page_protect_overhead(unsigned int num_loops __unused,
                                         unsigned int num_aliases __unused) {
  uint64_t duration = 0;
#if defined(__arm64__)
  pmap_t new_pmap = pmap_create_wrapper(0);
  vm_page_t m = pmap_test_alloc_vm_page();
  ...
  for (unsigned int loop = 0; loop < num_loops; ++loop) {
    ...
    pmap_remove(new_pmap, PMAP_TEST_VA,
                PMAP_TEST_VA + (num_aliases * PAGE_SIZE));
  }
...
/**
 * Worker thread that exercises pmap_remove() and pmap_enter() with a huge PV
 * list. This thread relies on the fact that PV lists are structured with newer
 * PTEs at the beginning of the list, so it maximizes PV list traversal time by
 * removing mappings sequentially starting with the beginning VA of the mapping
 * region (thus the oldest mapping), and then re-entering that removed mapping
 * at the beginning of the list.
 *
 * @param arg Thread argument parameter, actually of type
 * pmap_hugepv_test_thread_args*
 * @param wres Wait result, currently unused.
 */
static void hugepv_remove_enter_thread(void *arg, wait_result_t __unused wres) {
  ...
  do {
    ...
    kern_return_t kr = pmap_enter_options(
        args->args.pmap, va, args->args.pn, VM_PROT_READ | VM_PROT_WRITE,
        VM_PROT_NONE, VM_WIMG_USE_DEFAULT, FALSE, PMAP_OPTIONS_INTERNAL, NULL,
        PMAP_MAPPING_TYPE_INFER);
    assert(kr == KERN_SUCCESS);
  ...
  /* Ensure the update of nthreads is not speculated ahead of checking the stop
   * flag. */
 ...
 * @param wres Wait result, currently unused.
 */
static void hugepv_fast_fault_thread(void *arg, wait_result_t __unused wres) {
  pmap_hugepv_test_thread_args *args = arg;
  do {
    boolean_t success =
        arm_force_fast_fault(args->args.pn, VM_PROT_NONE, 0, NULL);
    assert(success);
    unsigned int rand;
    read_random(&rand, sizeof(rand));
    unsigned int mapping = rand % args->num_mappings;
    arm_fast_fault(args->args.pmap,
                   args->args.va + ((vm_offset_t)mapping << PAGE_SHIFT),
...
/**
 * Worker thread for updating cacheability of a physical page with a huge PV
 * list. This thread simply twiddles all mappings between write-combined and
 * normal (write-back) cacheability.
 *
 * @param arg Thread argument parameter, actually of type
 * pmap_hugepv_test_thread_args*
 * @param wres Wait result, currently unused.
 */
static void hugepv_cache_attr_thread(void *arg, wait_result_t __unused wres) {
  pmap_hugepv_test_thread_args *args = arg;
  do {
 ...
 */
static inline void huge_pv_start_panic_timer(thread_call_t panic_callout) {
  uint64_t deadline;
...
/**
 * Timer callout that executes in case the huge PV test incurs excessive (>=
 * 5min) runtime, which can happen due to unlucky scheduling of the main thread.
 * In this case we simply set the "stop" flag and expect the worker threads to
 * exit gracefully.
 *
 ...
 */
static void huge_pv_test_timeout(thread_call_param_t param0,
...
huge_pv_test_panic(__unused thread_call_param_t param0,
                   __unused thread_call_param_t param1) {
...
kern_return_t test_pmap_huge_pv_list(unsigned int num_loops,
  ...
  for (unsigned int mapping = 0; mapping < num_mappings; ++mapping) {
    kr = pmap_enter(new_pmap,
                    PMAP_TEST_VA + ((vm_offset_t)mapping << PAGE_SHIFT),
                    phys_page, VM_PROT_READ | VM_PROT_WRITE, VM_PROT_NONE,
                    VM_WIMG_USE_DEFAULT, FALSE, PMAP_MAPPING_TYPE_INFER);
    assert(kr == KERN_SUCCESS);
  }

  thread_call_t huge_pv_panic_call =
      thread_call_allocate(huge_pv_test_panic, NULL);

  pmap_hugepv_test_thread_args args = {.args = {.pmap = new_pmap,
                                                .stop = FALSE,
                                                .va = PMAP_TEST_VA,
                                                .pn = phys_page},
                                       .nthreads = 0,
                                       .num_mappings = num_mappings,
                                       .panic_callout = huge_pv_panic_call};

  thread_call_t huge_pv_timer_call =
      thread_call_allocate(huge_pv_test_timeout, &args);

  kr = kernel_thread_start_priority(hugepv_remove_enter_thread, &args,
                                    thread_kern_get_pri(current_thread()),
                                    &remove_enter_thread);
  if (kr != KERN_SUCCESS) {
    goto hugepv_cleanup;
  }
...
hugepv_cleanup:
  ...
  if (__improbable(args.args.stop)) {
    /**
     * If stop is already set, we hit the timeout, so we can't safely block
     * waiting for the workers to terminate as they may already be doing so.
     * Spin in a WFE loop instead.
     */
    ...
    /* Ensure prior stores to nthreads are visible before the update to
     * args.args.stop. */
  ...
  if (new_pmap != NULL) {
    pmap_remove(new_pmap, PMAP_TEST_VA,
                PMAP_TEST_VA + ((vm_offset_t)num_mappings << PAGE_SHIFT));
  }
...
kern_return_t test_pmap_reentrance(unsigned int num_loops __unused) {
  return KERN_NOT_SUPPORTED;
}
 ...
 */
static void surt_allocation_emulation_thread(void *arg,
  ...
  sptm_retype(surt_pa, XNU_DEFAULT, XNU_SUBPAGE_USER_ROOT_TABLES,
              retype_params);
 ...
 * @param wres Wait result - unused.
 */
static void surt_free_emulation_thread(void *arg, wait_result_t __unused wres) {
  surt_emulation_thread_data *thread_data = (surt_emulation_thread_data *)arg;
 ...
 */
kern_return_t test_surt(unsigned int num_surts) {
  surt_emulation_thread_data *thread_data_array =
      kalloc_type(surt_emulation_thread_data, num_surts, Z_WAITOK | Z_ZERO);
  ...
  for (unsigned int i = 0; i < num_surts; i++) {
    ...
    kernel_thread_start_priority(
        surt_allocation_emulation_thread, &thread_data_array[i],
        thread_kern_get_pri(current_thread()) - 1, &thread_array[i]);
  }
  ...
  for (unsigned int i = 0; i < num_surts; i++) {
    os_atomic_inc(&active_threads, relaxed);
    kernel_thread_start_priority(
        surt_free_emulation_thread, &thread_data_array[i],
        thread_kern_get_pri(current_thread()) - 1, &thread_array[i]);
  }
