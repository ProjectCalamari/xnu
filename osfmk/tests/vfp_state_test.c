...
const uint64_t vfp_state_test_regs[VFP_STATE_TEST_N_REGS] = {
    0x6a4cac4427ab5658, 0x51200e9ebbe0c9d1, 0xa94d20c2bbe367bc,
    0xfee45035460927db, 0x64f3f1f7e93d019f, 0x02a625f02b890a40,
...
static void vfp_state_test_thread_routine(void *args,
                                          __unused wait_result_t wr) {
  struct vfp_state_test_args *vfp_state_test_args =
      (struct vfp_state_test_args *)args;
  ...
  vfp_regs = kalloc_data(sizeof(vfp_state_test_regs), Z_WAITOK | Z_NOFAIL);

  vfp_regs_expected =
      kalloc_data(sizeof(vfp_state_test_regs), Z_WAITOK | Z_NOFAIL);
  ...
  asm volatile("ldr d8, [%0, #0] \t\n ldr d9, [%0, #8] \t\n\
               ...
               :
               : "r"(vfp_regs_expected)
               : "memory", "d8", "d9", "d10", "d11", "d12", "d13", "d14",
                 "d15");
 ...
 */
kern_return_t vfp_state_test(void) {
  ...
  for (int i = 0; i < VFP_STATE_TEST_N_THREADS; i++) {
    ...
    vfp_state_test_args[i].fp_control_mask =
    ...
    retval = kernel_thread_start(
        (thread_continue_t)vfp_state_test_thread_routine,
        (void *)&vfp_state_test_args[i], &vfp_state_thread[i]);
