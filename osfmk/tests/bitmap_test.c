...
void dump_bitmap_next(bitmap_t *map, uint nbits) {
...
void dump_bitmap_lsb(bitmap_t *map, uint nbits) {
  for (int i = bitmap_lsb_first(map, nbits); i >= 0;
       i = bitmap_lsb_next(map, nbits, i)) {
    printf(" %d", i);
  }
  printf("\n");
}
...
kern_return_t bitmap_post_test(void) {
  test_bits();
