...
static void test_once_callback(thread_call_param_t param0,
  ...
  kr = lck_mtx_sleep_deadline(&test_lock, LCK_SLEEP_DEFAULT, &wait_for_main,
...
static void test_once_thread_call(void) {
  lck_grp_init(&test_lock_grp, "test_thread_call", LCK_GRP_ATTR_NULL);
  lck_mtx_init(&test_lock, &test_lock_grp, LCK_ATTR_NULL);

  thread_call_t call;
  call = thread_call_allocate_with_options(
      &test_once_callback, (thread_call_param_t)TEST_ARG1,
  ...
  kr = lck_mtx_sleep_deadline(&test_lock, LCK_SLEEP_DEFAULT, &wait_for_callback,
                              THREAD_UNINT, deadline);
...
static void test_signal_callback(__unused thread_call_param_t param0,
...
static void test_signal_thread_call(void) {
  thread_call_t call;
  call = thread_call_allocate_with_options(
      &test_signal_callback, (thread_call_param_t)TEST_ARG1,
