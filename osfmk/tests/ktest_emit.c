...
void ktest_emit_testbegin(const char *test_name) {
  ...
  const char *fname =
      strnstr((char *)(uintptr_t)ktest_current_file, "xnu", 100);
  ...
  ret = snprintf(msg, size,
                 ...
                 "%d\t"        /* line */
                 "%s\n",       /* name */
                 ktest_current_time, ktest_test_index, fname,
                 ktest_current_line, test_name);
...
void ktest_emit_testskip(const char *skip_msg, va_list args) {
  ...
  ret = snprintf(msg, size,
                 ...
                 "%s\t"        /* file */
                 "%d\t",       /* line */
                 ktest_current_time, fname, ktest_current_line);
  BOUNDS_CHECK_AND_UPDATE(ret, size);
...
void ktest_emit_testend() {
  ...
  ret = snprintf(msg, size,
                 ...
                 "%s\n",       /* name */
                 ktest_current_time, ktest_test_index, fname,
                 ktest_current_line, ktest_test_name);
  BOUNDS_CHECK_AND_UPDATE(ret, size);
...
void ktest_emit_testcase(void) {
  ...
  ret = snprintf(
      ...
      "%s\t"        /* message */
      "%s",         /* current_expr */
      ktest_testcase_result_tokens[ktest_testcase_mode][ktest_testcase_result],
      ktest_current_time, ktest_expression_index, fname, ktest_current_line,
      ktest_current_msg, ktest_current_expr);
  BOUNDS_CHECK_AND_UPDATE(ret, size);

  for (int i = 0; ktest_current_var_names[i][0]; i++) {
    ret = snprintf(msg, size, "\t%s\t%s", ktest_current_var_names[i],
