...
typedef xt_panic_return_t (*xt_panic_widget_func)(const char *panicstr,
                                                  void *context, void **outval);
...
#define T_REGISTER_PANIC_WIDGET(func, ctx, outval)                             \
  xnupost_register_panic_widget((func), #func, (ctx), (outval))
#define T_REGISTER_ASSERT_CHECK(assert_str, retval)                            \
  T_REGISTER_PANIC_WIDGET(_xt_generic_assert_check,                            \
                          (void *)__DECONST(char *, assert_str), retval)
