 ...
 * They take all arguments needed to call the function and avoid calling
 * functions with PANICing values. test_* functions are used to call the call_
 * functions. They iterate through possibilities of interesting parameters and
 * provide those as arguments to the call_ functions.
 *
 * test_* functions are named in the following way:
 * Arguments under test are put at the end of the name. e.g. (test_mach_vm_prot)
 * tests a vm_prot_t test_mach_... functions test a function with the first
 * argument being a MAP_T. test_unix_... functions test a unix-y function. This
 * means it doesn't take a MAP_T. In kernel context, it means it operates on
 * current_map instead of an arbitrary vm_map_t test_..._with_allocated_...
 * means an allocation has already been created, and some parameters referring
 * to that allocation are passed in.
 *
 ...
 * and is just a very unlikely value for it. Tests should not rely on them to
 * compare against UNLIKELY_ values without explicit reason it cannot be
 * possible.
 *
 * INVALID_* means the value is 100% guaranteed to be invalid. They can be
 * relied on to be compared against.
 */
...
// This cannot be a purgable state (see vm_purgable.h) It's way above the last
// valid state
#define INVALID_PURGABLE_STATE 0xababab
static_assert(INVALID_PURGABLE_STATE > VM_PURGABLE_STATE_MAX,
              "This test requires a purgable state above the max");
...
static_assert(INVALID_INHERIT > VM_INHERIT_LAST_VALID,
              "This test requires an inheritance above the max");
...
// fixme for kext/kernel, verify that vm32 entrypoints are not used and not
// exported
#if KERNEL || __LP64__
...
#define ACCEPTABLE                                                             \
  -94 // trial should be considered successful no matter what the golden result
      // is
#define OUT_PARAM_BAD -93 // trial has incorrect setting of out parameter values

static inline bool is_fake_error(int err) {
  return err == BUSTED || err == IGNORED || err == ZEROSIZE || err == PANIC ||
         err == GUARD || err == OUT_PARAM_BAD;
}
...
#define KERN_TEST_FAILED                                                       \
  2 // failed without running any tests; error text in output buffer
...
static vm_parameter_validation_kern_thread_context_t *get_globals(void) {
  thread_test_context_t *ttc = thread_get_test_context();
  if (ttc == NULL ||
      ttc->ttc_identity != test_identity_vm_parameter_validation_kern) {
    panic("no thread context or wrong thread context in test "
          "vm_parameter_validation_kern");
  }

  return __container_of(ttc, vm_parameter_validation_kern_thread_context_t,
                        ttc);
}
...
#if !KERNEL
static inline uint64_t VM_MAP_PAGE_SIZE(MAP_T map __unused) {
  // fixme wrong for out-of-process maps
  // on platforms that support processes with two different page sizes
  return PAGE_SIZE;
}

static inline uint64_t VM_MAP_PAGE_MASK(MAP_T map __unused) {
  // fixme wrong for out-of-process maps
  // on platforms that support processes with two different page sizes
  return PAGE_MASK;
}
...
#define IMPL(T)                                                                \
  ...
  __attribute__((overloadable, used)) static inline T trunc_down_page(         \
      T addr, uint64_t pagesize) {                                             \
    return vm_sanitize_map_trunc_page_mask(addr, pagesize - 1);                \
  }                                                                            \
                                                                               \
  /* Truncate to the given map's page size. */                                 \
  __attribute__((overloadable, used)) static inline T trunc_down_map(          \
      MAP_T map, T addr) {                                                     \
    return vm_sanitize_map_trunc_page_mask(addr, VM_MAP_PAGE_MASK(map));       \
  }                                                                            \
                                                                               \
  __attribute__((overloadable,                                                 \
                 unavailable("use round_up_page instead"))) extern T           \
  round_up(T addr, uint64_t pagesize);                                         \
  __attribute__((overloadable,                                                 \
                 unavailable("use trunc_down_page instead"))) extern T         \
...
#define IMPL(T)                                                                \
  __attribute__((overloadable, used)) static bool                              \
...
static bool isRosetta() {
  ...
  if (sysctlbyname("sysctl.proc_translated", &out_value, &io_size, NULL, 0) ==
      0) {
...
// Needed to distinguish between rosetta kernel runs and generating trials names
// from kern golden files.
#if KERNEL
...
static inline addr_t adjust_page_size() {
  addr_t test_page_size = PAGE_SIZE;
#if !KERNEL && __x86_64__
  // Handle kernel page size variation while recreating trials names for golden
  // files in userspace.
...
// This prints either to stdout (userspace tests) or to a userspace buffer
// (kernel sysctl tests) Golden tests generation in userspace also writes to a
// buffer (GOLDEN_OUTPUT_BUF)
#if KERNEL
...
static inline void kfree_str(char *str) {
#if KERNEL
  kfree_data_addr(str);
#else
  free(str);
...
static inline int kasprintf(char **__restrict out_str,
...
/////////////////////////////////////////////////////
// Record trials and return values from tested functions (BSD int or Mach
// kern_return_t)

// Maintain list of known trials "smart" generator functions (trial formulae) as
// these are included in the golden result list (keeping the enum forces people
// to maintain the list up-to-date when adding new functions).
...
typedef enum { TRIALSFORMULA_ENUM(TRIALSFORMULA_ENUM_VARIANT) } trialsformula_t;

#define TRIALSARGUMENTS_NONE 0
#define TRIALSARGUMENTS_SIZE 2

// formula enum id to string
#define TRIALSFORMULA_ENUM_STRING(NAME)                                        \
  case NAME:                                                                   \
    return #NAME;
const char *trialsformula_name(trialsformula_t formula) {
  switch (formula) {
...
trialsformula_t trialsformula_from_string(const char *string) {
...
static __attribute__((overloadable)) results_t *alloc_results(
    const char *testname, char *testconfig, trialsformula_t trialsformula,
    uint64_t trialsargs[static TRIALSARGUMENTS_SIZE], unsigned capacity) {
  results_t *results;
#if KERNEL
...
static char *alloc_default_testconfig(void) {
  char *result;
  kasprintf(&result, "%s %s %s%s", OS_NAME, ARCH_NAME,
            kern_trialname_generation ? "kernel" : CALLER_NAME,
            !kern_trialname_generation && isRosetta() ? " rosetta" : "");
  return result;
}

static __attribute__((overloadable)) results_t *
alloc_results(const char *testname, trialsformula_t trialsformula,
              uint64_t *trialsargs, size_t trialsargs_count,
              unsigned capacity) {
  assert(trialsargs_count == TRIALSARGUMENTS_SIZE);
  return alloc_results(testname, alloc_default_testconfig(), trialsformula,
                       trialsargs, capacity);
}

static __attribute__((overloadable)) results_t *
alloc_results(const char *testname, trialsformula_t trialsformula,
              uint64_t trialsarg0, unsigned capacity) {
  uint64_t trialsargs[TRIALSARGUMENTS_SIZE] = {trialsarg0,
                                               TRIALSARGUMENTS_NONE};
  return alloc_results(testname, trialsformula, trialsargs,
                       TRIALSARGUMENTS_SIZE, capacity);
}

static __attribute__((overloadable)) results_t *
alloc_results(const char *testname, trialsformula_t trialsformula,
              unsigned capacity) {
  uint64_t trialsargs[TRIALSARGUMENTS_SIZE] = {TRIALSARGUMENTS_NONE,
                                               TRIALSARGUMENTS_NONE};
  return alloc_results(testname, trialsformula, trialsargs,
                       TRIALSARGUMENTS_SIZE, capacity);
}

static void __unused dealloc_results(results_t *results) {
...
append_result(results_t *results, int ret, const char *name) {
  ...
  assert(name_cpy);
  results->list[results->count++] = (result_t){.ret = ret, .name = name_cpy};
}
...
static results_t *__dump_results(results_t *results) {
...
static results_t *dump_golden_results(results_t *results) {
  trial_page_size = PAGE_SIZE;
  goldenprintf(TESTNAME_DELIMITER "%s\n", results->testname);
  goldenprintf(TRIALSFORMULA_DELIMITER "%s %s %llu,%llu,%llu\n",
               trialsformula_name(results->trialsformula),
               TRIALSARGUMENTS_DELIMITER, results->trialsargs[0],
               results->trialsargs[1], trial_page_size);
...
truncate_vm_map_addr_with_flags(MAP_T map, mach_vm_address_t addr, int flags) {
  ...
  if (flags & VM_FLAGS_RETURN_4K_DATA_ADDR) {
    // VM_FLAGS_RETURN_4K_DATA_ADDR means return a 4k aligned address rather
    // than the base of the page. Truncate to 4k.
    truncated_addr = trunc_down_page(addr, KB4);
  } else if (flags & VM_FLAGS_RETURN_DATA_ADDR) {
    // On VM_FLAGS_RETURN_DATA_ADDR, we expect to get back the unaligned
    // address. Don't truncate.
  } else {
...
get_expected_remap_misalignment(MAP_T map, mach_vm_address_t addr, int flags) {
  ...
  if (flags & VM_FLAGS_RETURN_4K_DATA_ADDR) {
    // VM_FLAGS_RETURN_4K_DATA_ADDR means return a 4k aligned address rather
    // than the base of the page. The misalignment is relative to the first 4k
    // page
    misalignment = addr - trunc_down_page(addr, KB4);
  } else if (flags & VM_FLAGS_RETURN_DATA_ADDR) {
    // On VM_FLAGS_RETURN_DATA_ADDR, we expect to get back the unaligned
    // address. The misalignment is therefore the low bits
...
static offset_list_t *allocate_offsets(unsigned capacity) {
  offset_list_t *offsets;
#if KERNEL
  offsets = kalloc_type(offset_list_t, absolute_or_relative_offset_t, capacity,
                        Z_WAITOK | Z_ZERO);
#else
  offsets = calloc(sizeof(offset_list_t) +
                       capacity * sizeof(absolute_or_relative_offset_t),
                   1);
#endif
  offsets->count = 0;
...
static void append_offset(offset_list_t *offsets, bool is_absolute,
                          addr_t offset) {
...
/* kernel globals are shared across processes, store cached offsets in
...
#else
#define ALLOC_TRIALS(NAME, new_capacity)                                       \
  (NAME##_trials_t *)calloc(                                                   \
      sizeof(NAME##_trials_t) + (new_capacity) * sizeof(NAME##_trial_t), 1)
#define FREE_TRIALS(NAME, trials) free(trials)
#endif

#define TRIALS_IMPL(NAME)                                                      \
  static NAME##_trials_t *__attribute__((used)) allocate_##NAME##_trials(      \
      unsigned capacity) {                                                     \
    ...
    return trials;                                                             \
                                                                               ...
                                                                               \
  static void __attribute__((overloadable, used)) free_trials(                 \
      NAME##_trials_t *trials) {                                               \
    FREE_TRIALS(NAME, trials);                                                 \
  }                                                                            \
                                                                               \
  static void __attribute__((overloadable, used)) append_trial(                \
      NAME##_trials_t *trials, NAME##_trial_t new_trial) {                     \
    assert(trials->count < trials->capacity);                                  \
    trials->list[trials->count++] = new_trial;                                 \
  }                                                                            \
                                                                               \
  static void __attribute__((overloadable, used)) append_trials(               \
      NAME##_trials_t *trials, NAME##_trial_t *new_trials,                     \
      unsigned new_count) {                                                    \
...
#define SMART_VM_INHERIT_TRIALS()                                              \
  ...
  append_trials(trials, vm_inherit_trials_values,                              \
                countof(vm_inherit_trials_values))
...
#define VM_INHERIT_TRIAL(new_value)                                            \
  (vm_inherit_trial_t) {                                                       \
    .value = (vm_inherit_t)(new_value), .name = "vm_inherit " #new_value       \
  }
...
static void cleanup_vm_inherit_trials(vm_inherit_trials_t **trials) {
  free_trials(*trials);
}
...
#define SMART_VM_BEHAVIOR_TRIALS()                                             \
  ...
  append_trials(trials, vm_behavior_trials_values,                             \
                countof(vm_behavior_trials_values))
...
#define VM_BEHAVIOR_TRIAL(new_value)                                           \
  (vm_behavior_trial_t) {                                                      \
    .value = (vm_behavior_t)(new_value), .name = "vm_behavior " #new_value     \
  }
...
static void cleanup_vm_behavior_trials(vm_behavior_trials_t **trials) {
  free_trials(*trials);
}
...
#define VM_SYNC_TRIAL(new_value)                                               \
  (vm_sync_trial_t) {                                                          \
    .value = (vm_sync_t)(new_value), .name = "vm_sync_t " #new_value           \
  }
...
static void cleanup_vm_sync_trials(vm_sync_trials_t **trials) {
  free_trials(*trials);
}
...
#define VM_MSYNC_TRIAL(new_value)                                              \
  (vm_msync_trial_t) {                                                         \
    .value = (int)(new_value), .name = "vm_msync_t " #new_value                \
  }
...
#define SMART_VM_ADVISE_TRIALS()                                               \
  ...
  append_trials(trials, vm_advise_trials_values,                               \
                countof(vm_advise_trials_values))
...
#define ADVISE_TRIAL(new_value)                                                \
  (vm_advise_trial_t) {                                                        \
    .value = (int)(new_value), .name = "advise " #new_value                    \
  }
...
#define SMART_VM_MACHINE_ATTRIBUTE_TRIALS()                                    \
  __attribute__((cleanup(cleanup_vm_machine_attribute_trials))) =              \
      allocate_vm_machine_attribute_trials(                                    \
          countof(vm_machine_attribute_trials_values));                        \
  append_trials(trials, vm_machine_attribute_trials_values,                    \
                countof(vm_machine_attribute_trials_values))
...
#define VM_MACHINE_ATTRIBUTE_TRIAL(new_value)                                  \
  (vm_machine_attribute_trial_t) {                                             \
    .value = (vm_machine_attribute_t)(new_value),                              \
    .name = "vm_machine_attribute_t " #new_value                               \
  }
...
#define VM_MAP_KERNEL_FLAGS_TRIAL(new_flags)                                   \
  (vm_map_kernel_flags_trial_t) {                                              \
    .flags = (int)(new_flags), .name = "vm_map_kernel_flags " #new_flags       \
  }

TRIALS_IMPL(vm_map_kernel_flags)

static vm_map_kernel_flags_trials_t *
generate_prefixed_vm_map_kernel_flags_trials(int prefix_flags,
...
#define APPEND(flag)                                                           \
  ({                                                                           \
    kasprintf(&str, "vm_map_kernel_flags %s%s%s", prefix_name,                 \
              prefix_flags == 0 ? "" : " | ", #flag);                          \
    append_trial(                                                              \
        trials, (vm_map_kernel_flags_trial_t){prefix_flags | (int)flag, str}); \
  })
...
static vm_map_kernel_flags_trials_t *generate_vm_map_kernel_flags_trials() {
  ...
  vm_map_kernel_flags_trials_t *anywhere =
      generate_prefixed_vm_map_kernel_flags_trials(VM_FLAGS_ANYWHERE,
                                                   "VM_FLAGS_ANYWHERE");
  vm_map_kernel_flags_trials_t *trials =
...
#define MMAP_FLAGS_TRIAL(new_flags)                                            \
  (mmap_flags_trial_t) {                                                       \
    .flags = (int)(new_flags), .name = "mmap flags " #new_flags                \
  }
...
static void cleanup_mmap_flags_trials(mmap_flags_trials_t **trials) {
  free_trials(*trials);
}
...
#define SMART_MMAP_FLAGS_TRIALS()                                              \
  ...
  append_trials(trials, mmap_flags_trials_values,                              \
                countof(mmap_flags_trials_values))
...
#define GENERIC_FLAG_TRIAL(new_flag)                                           \
  (generic_flag_trial_t) {                                                     \
    .flag = (int)(new_flag), .name = "generic flag " #new_flag                 \
  }

static generic_flag_trial_t generic_flag_trials_values[] = {
    GENERIC_FLAG_TRIAL(0),        GENERIC_FLAG_TRIAL(1),
    GENERIC_FLAG_TRIAL(2),        GENERIC_FLAG_TRIAL(3),
    GENERIC_FLAG_TRIAL(4),        GENERIC_FLAG_TRIAL(5),
    GENERIC_FLAG_TRIAL(6),        GENERIC_FLAG_TRIAL(7),
    GENERIC_FLAG_TRIAL(1u << 3),  GENERIC_FLAG_TRIAL(1u << 4),
    GENERIC_FLAG_TRIAL(1u << 5),  GENERIC_FLAG_TRIAL(1u << 6),
    GENERIC_FLAG_TRIAL(1u << 7),  GENERIC_FLAG_TRIAL(1u << 8),
    GENERIC_FLAG_TRIAL(1u << 9),  GENERIC_FLAG_TRIAL(1u << 10),
    GENERIC_FLAG_TRIAL(1u << 11), GENERIC_FLAG_TRIAL(1u << 12),
    GENERIC_FLAG_TRIAL(1u << 13), GENERIC_FLAG_TRIAL(1u << 14),
    GENERIC_FLAG_TRIAL(1u << 15), GENERIC_FLAG_TRIAL(1u << 16),
    GENERIC_FLAG_TRIAL(1u << 17), GENERIC_FLAG_TRIAL(1u << 18),
    GENERIC_FLAG_TRIAL(1u << 19), GENERIC_FLAG_TRIAL(1u << 20),
    GENERIC_FLAG_TRIAL(1u << 21), GENERIC_FLAG_TRIAL(1u << 22),
    GENERIC_FLAG_TRIAL(1u << 23), GENERIC_FLAG_TRIAL(1u << 24),
    GENERIC_FLAG_TRIAL(1u << 25), GENERIC_FLAG_TRIAL(1u << 26),
    GENERIC_FLAG_TRIAL(1u << 27), GENERIC_FLAG_TRIAL(1u << 28),
    GENERIC_FLAG_TRIAL(1u << 29), GENERIC_FLAG_TRIAL(1u << 30),
    GENERIC_FLAG_TRIAL(1u << 31),
};

TRIALS_IMPL(generic_flag)

static void cleanup_generic_flag_trials(generic_flag_trials_t **trials) {
  free_trials(*trials);
}
...
#define SMART_GENERIC_FLAG_TRIALS()                                            \
  ...
  append_trials(trials, generic_flag_trials_values,                            \
                countof(generic_flag_trials_values))
...
static void cleanup_vm_tag_trials(vm_tag_trials_t **trials) {
  free_trials(*trials);
}
...
#define VM_PROT_TRIAL(new_prot)                                                \
  (vm_prot_trial_t) {                                                          \
    .prot = (vm_prot_t)(new_prot), .name = "vm_prot " #new_prot                \
  }
...
static void cleanup_vm_prot_trials(vm_prot_trials_t **trials) {
  free_trials(*trials);
}
...
#define VM_PROT_PAIR_TRIAL(new_cur, new_max, new_name)                         \
  (vm_prot_pair_trial_t) {                                                     \
    .cur = (vm_prot_t)(new_cur), .max = (vm_prot_t)(new_max),                  \
    .name = new_name,                                                          \
  }

vm_prot_pair_trials_t *generate_vm_prot_pair_trials() {
...
static void cleanup_vm_prot_pair_trials(vm_prot_pair_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
    kfree_str((*trials)->list[i].name);
  }
  free_trials(*trials);
...
#define VM_PURGEABLE_TRIAL(new_value)                                          \
  (vm_purgeable_trial_t) {                                                     \
    .value = (vm_purgable_t)(new_value), .name = "vm_purgeable_t " #new_value  \
  }
...
#define VM_PURGEABLE_STATE_TRIAL(new_value)                                    \
  (vm_purgeable_state_trial_t) {                                               \
    .value = (int)(new_value), .name = "state " #new_value                     \
  }
...
#define VM_PURGEABLE_AND_STATE_TRIAL(new_control, new_state, new_name)         \
  (vm_purgeable_and_state_trial_t) {                                           \
    .control = (vm_purgable_t)(new_control), .state = (int)(new_state),        \
    .name = new_name,                                                          \
  }

vm_purgeable_and_state_trials_t *generate_vm_purgeable_t_and_state_trials() {
  ...
  vm_purgeable_and_state_trials_t *trials =
      allocate_vm_purgeable_and_state_trials(num_trials);
  for (size_t i = 0; i < purgeable_trial_count; i++) {
    for (size_t j = 0; j < state_trial_count; j++) {
      ...
      vm_purgeable_state_trial_t state_trial =
          vm_purgeable_state_trials_values[j];
      char *str;
      kasprintf(&str, "%s, %s", control_trial.name, state_trial.name);
      append_trial(trials, VM_PURGEABLE_AND_STATE_TRIAL(
                               control_trial.value, state_trial.value, str));
    }
  }
  return trials;
}
...
static void cleanup_vm_purgeable_t_and_state_trials(
    vm_purgeable_and_state_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
    kfree_str((*trials)->list[i].name);
  }
...
static void cleanup_ledger_tag_trials(ledger_tag_trials_t **trials) {
  free_trials(*trials);
}
...
#define SMART_LEDGER_TAG_TRIALS()                                              \
  ...
  append_trials(trials, ledger_tag_trials_values,                              \
                countof(ledger_tag_trials_values))
...
#define LEDGER_FLAG_TRIAL(new_flag)                                            \
  (ledger_flag_trial_t) {                                                      \
    .flag = (int)(new_flag), .name = "ledger flag " #new_flag                  \
  }
...
static void cleanup_ledger_flag_trials(ledger_flag_trials_t **trials) {
  free_trials(*trials);
}
...
#define SMART_LEDGER_FLAG_TRIALS()                                             \
  ...
  append_trials(trials, ledger_flag_trials_values,                             \
                countof(ledger_flag_trials_values))
...
#define ADDR_TRIAL(new_addr, new_absolute, new_name)                           \
  (addr_trial_t) {                                                             \
    .addr = (addr_t)(new_addr), .addr_is_absolute = new_absolute,              \
    .name = new_name                                                           \
  }

static addr_trial_t
    __attribute__((overloadable, used)) slide_trial(addr_trial_t trial,
...
static const offset_list_t *get_addr_trial_offsets(void) {
...
addr_trials_t *generate_addr_trials(addr_t base) {
  ...
  for (unsigned a = 0; a < ADDRS; a++) {
    ...
    char *str;
    kasprintf(&str, "addr: %s0x%llx", addr_is_absolute ? "" : "base+",
              addr_offset);
...
static void cleanup_addr_trials(addr_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
    kfree_str((*trials)->list[i].name);
  }
  free_trials(*trials);
...
#define SMART_ADDR_TRIALS(base)                                                \
  __attribute__((cleanup(cleanup_addr_trials))) = generate_addr_trials(base)
...
static const offset_list_t *get_size_trial_offsets(void) {
  addr_t test_page_size = adjust_page_size();
...
size_trials_t *generate_size_trials(void) {
...
static void cleanup_size_trials(size_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
    kfree_str((*trials)->list[i].name);
  }
  free_trials(*trials);
...
#define SMART_SIZE_TRIALS()                                                    \
  __attribute__((cleanup(cleanup_size_trials))) = generate_size_trials()
...
typedef struct {
  ...
  addr_t size;
  char *name;
  bool start_is_absolute; // start computation does not include any allocation's
                          // base address
...
#define START_SIZE_TRIAL(new_start, start_absolute, new_size, size_absolute,   \
                         new_name)                                             \
  (start_size_trial_t) {                                                       \
    .start = (addr_t)(new_start), .size = (addr_t)(new_size),                  \
    .name = new_name, .start_is_absolute = start_absolute,                     \
    .size_is_absolute = size_absolute                                          \
  }

static const offset_list_t *get_start_size_trial_start_offsets(void) {
  return get_addr_trial_offsets();
}

static const offset_list_t *get_start_size_trial_size_offsets(void) {
...
start_size_trials_t *generate_start_size_trials(addr_t base) {
  ...
  for (unsigned a = 0; a < ADDRS; a++) {
    for (unsigned s = 0; s < SIZES; s++) {
      ...
      append_trial(trials, START_SIZE_TRIAL(start, start_is_absolute, size,
                                            size_is_absolute, str));
    }
  }
  return trials;
}

static void cleanup_start_size_trials(start_size_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
    kfree_str((*trials)->list[i].name);
  }
  free_trials(*trials);
...
#define START_SIZE_OFFSET_OBJECT_TRIAL(new_start, new_size, new_offset,        \
                                       new_obj_size, new_start_is_absolute,    \
                                       new_size_is_absolute, new_name)         \
  (start_size_offset_object_trial_t) {                                         \
    .start = (mach_vm_address_t)(new_start),                                   \
    ...
    .size_is_absolute = (bool)(new_size_is_absolute), .name = new_name,        \
  }

bool obj_size_is_ok(mach_vm_size_t obj_size) {
  addr_t test_page_size = adjust_page_size();
...
static offset_list_t *get_ssoo_absolute_offsets() {
  addr_t test_page_size = adjust_page_size();
...
static offset_list_t *get_ssoo_absolute_and_relative_offsets() {
  addr_t test_page_size = adjust_page_size();
...
start_size_offset_object_trials_t *generate_start_size_offset_object_trials() {
  ...
  num_trials *=
      start_offsets->count * size_offsets->count * offset_values->count;

  start_size_offset_object_trials_t *trials =
      allocate_start_size_offset_object_trials(num_trials);
    ...
    for (size_t b = 0; b < size_offsets->count; b++) {
      for (size_t c = 0; c < offset_values->count; c++) {
        for (size_t d = 0; d < object_sizes->count; d++) {
          ...
          char *str;
          kasprintf(&str,
                    "start: %s0x%llx, size: %s0x%llx, offset: 0x%llx, "
                    "obj_size: 0x%llx",
                    start_is_absolute ? "" : "base+", start,
                    size_is_absolute ? "" : "-start+", size, offset, obj_size);
          append_trial(trials, START_SIZE_OFFSET_OBJECT_TRIAL(
                                   start, size, offset, obj_size,
                                   start_is_absolute, size_is_absolute, str));
        }
      }
    }
  }
...
static void cleanup_start_size_offset_object_trials(
    start_size_offset_object_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
    kfree_str((*trials)->list[i].name);
  }
...
#define START_SIZE_START_SIZE_TRIAL(                                           \
    new_start, new_size, new_second_start, new_second_size,                    \
    new_start_is_absolute, new_size_is_absolute, new_second_start_is_absolute, \
    new_second_size_is_absolute, new_name)                                     \
  (start_size_start_size_trial_t) {                                            \
    .start = (mach_vm_address_t)(new_start),                                   \
    .size = (mach_vm_size_t)(new_size),                                        \
...
slide_trial(start_size_start_size_trial_t trial, mach_vm_address_t slide,
            mach_vm_address_t second_slide) {
...
start_size_start_size_trials_t *generate_start_size_start_size_trials() {
  /*
   * Reuse the starts/sizes from start/size/offset/object
   */
  const offset_list_t *start_offsets = get_ssoo_absolute_and_relative_offsets();
  const offset_list_t *size_offsets = get_ssoo_absolute_and_relative_offsets();
  const offset_list_t *second_start_offsets =
      get_ssoo_absolute_and_relative_offsets();
  const offset_list_t *second_size_offsets =
      get_ssoo_absolute_and_relative_offsets();

  unsigned num_trials = start_offsets->count * size_offsets->count *
                        second_start_offsets->count *
                        second_start_offsets->count;

  start_size_start_size_trials_t *trials =
      allocate_start_size_start_size_trials(num_trials);
  for (size_t a = 0; a < start_offsets->count; a++) {
    for (size_t b = 0; b < size_offsets->count; b++) {
      for (size_t c = 0; c < second_start_offsets->count; c++) {
        for (size_t d = 0; d < second_size_offsets->count; d++) {
          ...
          bool size_is_absolute = size_offsets->list[b].is_absolute;
          bool second_start_is_absolute =
              second_start_offsets->list[c].is_absolute;
          bool second_size_is_absolute =
              second_size_offsets->list[d].is_absolute;
          mach_vm_address_t start = start_offsets->list[a].offset;
          mach_vm_size_t size = size_offsets->list[b].offset;
          ...
          char *str;
          kasprintf(&str,
                    "start: %s0x%llx, size: %s0x%llx, second_start: %s0x%llx, "
                    "second_size: %s0x%llx",
                    start_is_absolute ? "" : "base+", start,
                    size_is_absolute ? "" : "-start+", size,
                    second_start_is_absolute ? "" : "base+", second_start,
                    second_size_is_absolute ? "" : "-start+", second_size);
          append_trial(trials, START_SIZE_START_SIZE_TRIAL(
                                   start, size, second_start, second_size,
                                   start_is_absolute, size_is_absolute,
                                   second_start_is_absolute,
                                   second_size_is_absolute, str));
        }
      }
    }
  }
...
#define START_SIZE_OFFSET_TRIAL(new_start, new_size, new_offset,               \
                                new_start_is_absolute, new_size_is_absolute,   \
                                new_name)                                      \
  (start_size_offset_trial_t) {                                                \
    .start = (mach_vm_address_t)(new_start),                                   \
    .size = (mach_vm_size_t)(new_size),                                        \
...
start_size_offset_trials_t *generate_start_size_offset_trials() {
  const offset_list_t *start_offsets = get_ssoo_absolute_and_relative_offsets();
  const offset_list_t *offset_values = get_ssoo_absolute_offsets();
  const offset_list_t *size_offsets = get_ssoo_absolute_and_relative_offsets();
  ...
  unsigned num_trials =
      start_offsets->count * offset_values->count * size_offsets->count;
  start_size_offset_trials_t *trials =
      allocate_start_size_offset_trials(num_trials);
  for (size_t a = 0; a < start_offsets->count; a++) {
    for (size_t b = 0; b < offset_values->count; b++) {
      for (size_t c = 0; c < size_offsets->count; c++) {
        ...
        kasprintf(&str, "start: %s0x%llx, offset: 0x%llx, size: %s0x%llx",
                  start_is_absolute ? "" : "base+", start, offset,
                  size_is_absolute ? "" : "-start+", size);
        append_trial(trials, START_SIZE_OFFSET_TRIAL(start, size, offset,
                                                     start_is_absolute,
                                                     size_is_absolute, str));
      }
    }
...
typedef struct {
  ...
  addr_t size;
  char *name;
  bool src_is_absolute; // src computation does not include any allocation's
                        // base address
  bool dst_is_absolute; // dst computation does not include any allocation's
                        // base address
...
#define SRC_DST_SIZE_TRIAL(new_src, new_dst, new_size, new_name, src_absolute, \
                           dst_absolute, size_src_rel, size_dst_rel)           \
  (src_dst_size_trial_t) {                                                     \
    .src = (addr_t)(new_src), .dst = (addr_t)(new_dst),                        \
    .size = (addr_t)(new_size), .name = new_name,                              \
    .src_is_absolute = src_absolute, .dst_is_absolute = dst_absolute,          \
    ...
    for (size_t j = 0; j < dst_count; j++) {
      ...
      for (size_t k = 0; k < size_count; k++) {
        ...
        if (rebase_size_from_src) {
          ...
          kasprintf(&desc, "%s: %s%lli, %s: %s%lli, size: -%s%+lli", srcname,
                    rebase_src ? "base+" : "", (int64_t)src_offset, dstname,
                    rebase_dst ? "base+" : "", (int64_t)dst_offset, srcname,
                    (int64_t)size_offset);
        } else if (rebase_size_from_dst) {
          size = -dst_offset + size_offset;
          kasprintf(&desc, "%s: %s%lli, %s: %s%lli, size: -%s%+lli", srcname,
                    rebase_src ? "base+" : "", (int64_t)src_offset, dstname,
                    rebase_dst ? "base+" : "", (int64_t)dst_offset, dstname,
                    (int64_t)size_offset);
        } else {
          size = size_offset;
          kasprintf(&desc, "%s: %s%lli, %s: %s%lli, size: %lli", srcname,
                    rebase_src ? "base+" : "", (int64_t)src_offset, dstname,
                    rebase_dst ? "base+" : "", (int64_t)dst_offset,
                    (int64_t)size_offset);
        }
        assert(desc);
        append_trial(trials, SRC_DST_SIZE_TRIAL(src_offset, dst_offset, size,
                                                desc, !rebase_src, !rebase_dst,
                                                rebase_size_from_src,
                                                rebase_size_from_dst));
      }
    }
  }
  return trials;
...
static void cleanup_src_dst_size_trials(src_dst_size_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
    kfree_str((*trials)->list[i].name);
  }
  free_trials(*trials);
...
#define SHARED_FILE_NP_TRIAL(new_files_count, new_files, new_name)             \
  (shared_file_np_trial_t) {                                                   \
    .files_count = (uint32_t)(new_files_count),                                \
    .files = (struct shared_file_np *)(new_files),                             \
    .name = "files_count=" #new_files_count new_name                           \
  }

struct shared_file_np *alloc_shared_file_np(uint32_t files_count) {
  struct shared_file_np *files;
#if KERNEL
...
void free_shared_file_np(shared_file_np_trial_t *trial) {
#if KERNEL
...
shared_file_np_trials_t *get_shared_file_np_trials(uint64_t dyld_fd) {
  ...
  append_trial(trials, SHARED_FILE_NP_TRIAL(_SR_FILE_MAPPINGS_MAX_FILES - 1,
                                            NULL, " (NULL files)"));
  append_trial(trials, SHARED_FILE_NP_TRIAL(_SR_FILE_MAPPINGS_MAX_FILES, NULL,
                                            " (NULL files)"));
  append_trial(trials, SHARED_FILE_NP_TRIAL(_SR_FILE_MAPPINGS_MAX_FILES + 1,
                                            NULL, " (NULL files)"));
  ...
  append_trial(
      trials, SHARED_FILE_NP_TRIAL(_SR_FILE_MAPPINGS_MAX_FILES - 1, files, ""));
  files = alloc_shared_file_np(_SR_FILE_MAPPINGS_MAX_FILES);
  append_trial(trials,
               SHARED_FILE_NP_TRIAL(_SR_FILE_MAPPINGS_MAX_FILES, files, ""));
  files = alloc_shared_file_np(_SR_FILE_MAPPINGS_MAX_FILES + 1);
  append_trial(
      trials, SHARED_FILE_NP_TRIAL(_SR_FILE_MAPPINGS_MAX_FILES + 1, files, ""));
  files = alloc_shared_file_np(1);
  ...
  append_trial(trials,
               SHARED_FILE_NP_TRIAL(1, files, " non-zero shared_file_np"));
  ...
  append_trial(trials,
               SHARED_FILE_NP_TRIAL(2, files, " checks shared_file_np"));
  return trials;
}

static void cleanup_shared_file_np_trials(shared_file_np_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
...
#define SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(new_mappings_count, new_mappings,   \
                                           new_name)                           \
  (shared_file_mapping_slide_np_trial_t) {                                     \
...
alloc_shared_file_mapping_slide_np(uint32_t mappings_count) {
  struct shared_file_mapping_slide_np *mappings;
#if KERNEL
  mappings = kalloc_type(struct shared_file_mapping_slide_np, mappings_count,
                         Z_WAITOK | Z_ZERO);
#else
  mappings =
      calloc(mappings_count, sizeof(struct shared_file_mapping_slide_np));
#endif
  return mappings;
}

void free_shared_file_mapping_slide_np(
    shared_file_mapping_slide_np_trial_t *trial) {
#if KERNEL
  // some trials have files_count > 0 but null files.
  if (trial->mappings) {
    kfree_type(struct shared_file_mapping_slide_np, trial->mappings_count,
               trial->mappings);
  }
#else
  free(trial->mappings);
...
typedef enum {
  ...
  MP_OFFSET_SIZE,
  MP_PROTS
} mapping_slide_np_test_style_t;

static inline struct shared_file_mapping_slide_np *
alloc_and_fill_shared_file_mappings(uint32_t num_mappings,
                                    mapping_slide_np_test_style_t style) {
  assert(num_mappings > 0);
  struct shared_file_mapping_slide_np *mappings =
      alloc_shared_file_mapping_slide_np(num_mappings);

  // Checks happen in a for-loop so is desirable to differentiate the first
  // mapping.
  switch (style) {
  case MP_NORMAL:
...
get_shared_file_mapping_slide_np_trials(void) {
  ...
  shared_file_mapping_slide_np_trials_t *trials =
      allocate_shared_file_mapping_slide_np_trials(14);
  append_trial(trials,
               SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(0, NULL, " (NULL mappings)"));
  append_trial(trials,
               SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(1, NULL, " (NULL mappings)"));
  append_trial(trials, SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(SFM_MAX - 1, NULL,
                                                          " (NULL mappings)"));
  append_trial(trials, SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(SFM_MAX, NULL,
                                                          " (NULL mappings)"));
  append_trial(trials, SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(SFM_MAX + 1, NULL,
                                                          " (NULL mappings)"));
  mappings = alloc_and_fill_shared_file_mappings(1, MP_NORMAL);
  append_trial(trials,
               SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(1, mappings, " (normal)"));
  mappings = alloc_and_fill_shared_file_mappings(1, MP_ADDR_SIZE);
  append_trial(trials, SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(
                           1, mappings, " (sms_address+sms_size check)"));
  mappings = alloc_and_fill_shared_file_mappings(1, MP_OFFSET_SIZE);
  append_trial(trials, SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(
                           1, mappings, " (sms_file_offset+sms_size check)"));
  mappings = alloc_and_fill_shared_file_mappings(1, MP_PROTS);
  append_trial(trials, SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(
                           1, mappings, " (sms_init_prot check)"));
  mappings = alloc_and_fill_shared_file_mappings(SFM_MAX - 1, MP_NORMAL);
  append_trial(trials,
               SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(SFM_MAX - 1, mappings, ""));
  mappings = alloc_and_fill_shared_file_mappings(SFM_MAX, MP_NORMAL);
  append_trial(trials,
               SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(SFM_MAX, mappings, ""));
  mappings = alloc_and_fill_shared_file_mappings(SFM_MAX + 1, MP_NORMAL);
  append_trial(trials,
               SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(SFM_MAX + 1, mappings, ""));
  mappings =
      alloc_and_fill_shared_file_mappings(kNumSharedCacheMappings, MP_NORMAL);
  append_trial(trials, SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(
                           kNumSharedCacheMappings, mappings, ""));
  mappings = alloc_and_fill_shared_file_mappings(2 * kNumSharedCacheMappings,
                                                 MP_NORMAL);
  append_trial(trials, SHARED_FILE_MAPPING_SLIDE_NP_TRIAL(
                           2 * kNumSharedCacheMappings, mappings, ""));

  return trials;
}

static void cleanup_shared_file_mapping_slide_np_trials(
    shared_file_mapping_slide_np_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
...
#define SHARED_REGION_MAP_AND_SLIDE_2_TRIAL(                                   \
    new_files_count, new_files, new_mappings_count, new_mappings, new_name)    \
  (shared_region_map_and_slide_2_trial_t) {                                    \
    .files_count = (uint32_t)(new_files_count),                                \
    .files = (struct shared_file_np *)(new_files),                             \
    .mappings_count = (uint32_t)(new_mappings_count),                          \
...
generate_shared_region_map_and_slide_2_trials(uint64_t dyld_fd) {
  ...
  shared_file_mapping_slide_np_trials_t *shared_mappings =
      get_shared_file_mapping_slide_np_trials();
  unsigned num_trials = shared_files->count * shared_mappings->count;
  shared_region_map_and_slide_2_trials_t *trials =
      allocate_shared_region_map_and_slide_2_trials(num_trials);
  ...
  for (size_t i = 0; i < shared_files->count; i++) {
    for (size_t j = 0; j < shared_mappings->count; j++) {
      ...
      shared_file_mapping_slide_np_trial_t shared_mapping =
          shared_mappings->list[j];
      kasprintf(&buf, "%s, %s", shared_file.name, shared_mapping.name);
      append_trial(trials, SHARED_REGION_MAP_AND_SLIDE_2_TRIAL(
                               shared_file.files_count, shared_file.files,
                               shared_mapping.mappings_count,
                               shared_mapping.mappings, buf));
    }
  }
  return trials;
}
...
static void __attribute__((used)) cleanup_shared_region_map_and_slide_2_trials(
    shared_region_map_and_slide_2_trials_t **trials) {
  for (size_t i = 0; i < (*trials)->count; i++) {
    kfree_str((*trials)->list[i].name);
  }
  cleanup_shared_file_np_trials(&(*trials)->shared_files_trials);
  cleanup_shared_file_mapping_slide_np_trials(
      &(*trials)->shared_mappings_trials);
  free_trials(*trials);
}
#endif // !KERNEL
...
static inline bool is_fixed(int flags) {
  static_assert(VM_FLAGS_FIXED == 0,
                "this test requies VM_FLAGS_FIXED be zero");
  static_assert(VM_FLAGS_ANYWHERE != 0,
                "this test requires VM_FLAGS_ANYWHERE be nonzero");
  return !(flags & VM_FLAGS_ANYWHERE);
}

// Return true if flags has VM_FLAGS_FIXED and VM_FLAGS_OVERWRITE set.
static inline bool is_fixed_overwrite(int flags) {
  return is_fixed(flags) && (flags & VM_FLAGS_OVERWRITE);
}

// Return true if flags has VM_FLAGS_ANYWHERE and VM_FLAGS_RANDOM_ADDR set.
static inline bool is_random_anywhere(int flags) {
  static_assert(VM_FLAGS_ANYWHERE != 0,
                "this test requires VM_FLAGS_ANYWHERE be nonzero");
...
static void deallocate_if_not_fixed_overwrite(kern_return_t allocator_kr,
                                              MAP_T map,
                                              mach_vm_address_t start,
...
static inline bool dealloc_would_time_out(mach_vm_address_t addr __unused,
...
static inline vm_map_t create_map(mach_vm_address_t map_start,
                                  mach_vm_address_t map_end) {
  ledger_t ledger =
  ...
  vm_map_t map =
      vm_map_create_options(pmap, map_start, map_end, VM_MAP_CREATE_PAGEABLE);
  assert(map);

  /*
   * Normally, we would vm_map_setup a task's map, but since we're breaking the
   * assumed 1:1 correspondence between map and task here, we must manually set
   * up the map's back pointer, without repeating any one-time task setup (e.g.
   * registering reclaim buffers)
   */
...
static inline void cleanup_map(vm_map_t *map) {
  assert(*map);
...
#define SMART_RANGE_MAP                                                        \
  __attribute__((cleanup(cleanup_map))) =                                      \
      create_map(0, vm_compute_max_offset(true))
...
static kern_return_t allocate_after(MAP_T map, mach_vm_address_t *address,
                                    mach_vm_size_t size,
...
static inline mach_vm_address_t default_allocation_address_hint(void) {
  /*
...
#define SMART_ALLOCATE_PURGEABLE_VM(map, size, perm)                           \
  __attribute__((cleanup(cleanup_allocation))) =                               \
      create_allocation(map, size, 0, perm, false, VM_FLAGS_PURGABLE)

// allocate a VM region with size and permissions
// and deallocate it at end of scope
#define SMART_ALLOCATE_VM(map, size, perm)                                     \
  __attribute__((cleanup(cleanup_allocation))) =                               \
      create_allocation(map, size, 0, perm, false, 0)
...
#define SMART_TRY_ALLOCATE_VM(map, size, perm)                                 \
  __attribute__((cleanup(cleanup_allocation))) =                               \
      create_allocation(map, size, 0, perm, true, 0)
...
static allocation_t create_allocation_after(MAP_T new_map,
                                            mach_vm_address_t address_hint,
                                            mach_vm_address_t new_size,
                                            mach_vm_size_t align_mask,
                                            vm_prot_t perm, bool allow_failure,
                                            int additional_map_flags) {
  ...
  if (__builtin_add_overflow(result.size, result.guard_size * 4,
                             &allocated_size)) {
    if (allow_failure) {
      return (allocation_t){new_map, 0, 0, 0, 0, 0, 0, 0};
    } else {
  ...
  kern_return_t kr;
  kr = allocate_after(result.map, &allocated_base, allocated_size, align_mask,
                      additional_map_flags);
  ...
  assert(kr == 0);
  kr = mach_vm_protect(result.map, result.guard_prefix, result.guard_size, true,
                       VM_PROT_NONE);
  assert(kr == 0);
  kr = mach_vm_protect(result.map, result.guard_suffix, result.guard_size, true,
                       VM_PROT_NONE);
  assert(kr == 0);
  kr = mach_vm_deallocate(result.map, result.unallocated_prefix,
                          result.guard_size);
  assert(kr == 0);
  kr = mach_vm_deallocate(result.map, result.unallocated_suffix,
                          result.guard_size);
  assert(kr == 0);
...
static allocation_t create_allocation(MAP_T new_map, mach_vm_address_t new_size,
                                      mach_vm_size_t align_mask, vm_prot_t perm,
                                      bool allow_failure,
                                      int additional_map_flags) {
  mach_vm_address_t address_hint = default_allocation_address_hint();
  return create_allocation_after(new_map, address_hint, new_size, align_mask,
                                 perm, allow_failure, additional_map_flags);
}
...
static void set_already_deallocated(allocation_t *allocation) {
  allocation->addr = 0;
  allocation->size = 0;
}

static void cleanup_allocation(allocation_t *allocation) {
  // fixme verify allocations and unallocated spaces still exist where we expect
  if (allocation->size) {
    (void)mach_vm_deallocate(allocation->map, allocation->addr,
                             allocation->size);
  }
  if (allocation->guard_size) {
    (void)mach_vm_deallocate(allocation->map, allocation->guard_prefix,
                             allocation->guard_size);
    (void)mach_vm_deallocate(allocation->map, allocation->guard_suffix,
                             allocation->guard_size);
  }
}
...
#define SMART_UNALLOCATE_VM(map, size)                                         \
  __attribute__((cleanup(cleanup_unallocation))) =                             \
      create_unallocation(map, size)
...
#define SMART_UNALLOCATE_VM_AFTER(map, address_hint, size)                     \
  __attribute__((cleanup(cleanup_unallocation))) =                             \
      create_unallocation_after(map, address_hint, size, false)
...
#define SMART_TRY_UNALLOCATE_VM(map, size)                                     \
  __attribute__((cleanup(cleanup_unallocation))) =                             \
      create_unallocation(map, size, true)
...
create_unallocation_after(MAP_T new_map, mach_vm_address_t address_hint,
                          mach_vm_address_t new_size, bool allow_failure) {
  // allocations in address order:
  // 16K guard_prefix (allocated, prot none)
  ...
  mach_vm_address_t allocated_base = address_hint;
  mach_vm_size_t allocated_size = result.size;
  if (__builtin_add_overflow(result.size, result.guard_size * 2,
                             &allocated_size)) {
    if (allow_failure) {
      return (unallocation_t){new_map, 0, 0, 0, 0, 0};
    } else {
...
create_unallocation(MAP_T new_map, mach_vm_address_t new_size,
                    bool allow_failure) {
  mach_vm_address_t address_hint = default_allocation_address_hint();
  return create_unallocation_after(new_map, address_hint, new_size,
                                   allow_failure);
}
...
static void cleanup_unallocation(unallocation_t *unallocation) {
  // fixme verify allocations and unallocated spaces still exist where we expect
  if (unallocation->guard_size) {
    (void)mach_vm_deallocate(unallocation->map, unallocation->guard_prefix,
                             unallocation->guard_size);
    (void)mach_vm_deallocate(unallocation->map, unallocation->guard_suffix,
                             unallocation->guard_size);
  }
}
...
#define RECLAMATION_BUFFER_INIT_TRIAL(                                         \
    new_task, new_address, new_initial_capacity, new_max_capacity, new_name)   \
  (reclamation_buffer_init_trial_t) {                                          \
    .task = (task_t)(new_task), .address = (mach_vm_address_t)(new_address),   \
...
// mach_vm_remap_external/vm_remap_external/vm32_remap/mach_vm_remap_new_external
// infra mach_vm_remap/mach_vm_remap_new_kernel infra
...
typedef kern_return_t (*remap_fn_t)(
...
static kern_return_t help_call_remap_fn__src_size_etc(
    remap_fn_t fn, MAP_T map, int flags, bool copy, mach_vm_address_t src,
    mach_vm_size_t size, vm_prot_t cur, vm_prot_t max, vm_inherit_t inherit) {
  kern_return_t kr;
#if KERNEL
  ...
  if (is_fixed_overwrite(flags)) {
    ...
    kr =
        fn(map, &out_addr, size, 0, flags, map, src, copy, &cur, &max, inherit);
  } else {
...
static kern_return_t help_call_remap_fn__src_size(remap_fn_t fn, MAP_T map,
                                                  int unused_flags __unused,
                                                  bool copy,
                                                  mach_vm_address_t src,
                                                  mach_vm_size_t size) {
  assert(unused_flags == 0);
  return help_call_remap_fn__src_size_etc(fn, map, VM_FLAGS_ANYWHERE, copy, src,
                                          size, 0, 0, VM_INHERIT_NONE);
}

static kern_return_t help_call_remap_fn__dst_size(remap_fn_t fn, MAP_T map,
                                                  int flags, bool copy,
  ...
  vm_prot_t cur = 0;
  vm_prot_t max = 0;
  kern_return_t kr = fn(map, &out_addr, size, 0, flags, map, src.addr, copy,
                        &cur, &max, VM_INHERIT_NONE);
  deallocate_if_not_fixed_overwrite(kr, map, out_addr, size, flags);
  return kr;
}

static kern_return_t help_call_remap_fn__inherit(remap_fn_t fn, MAP_T map,
                                                 int flags, bool copy,
                                                 mach_vm_address_t src,
                                                 mach_vm_size_t size,
                                                 vm_inherit_t inherit) {
  return help_call_remap_fn__src_size_etc(fn, map, flags, copy, src, size, 0, 0,
                                          inherit);
}

static kern_return_t help_call_remap_fn__flags(remap_fn_t fn, MAP_T map,
                                               int unused_flags __unused,
                                               bool copy, mach_vm_address_t src,
                                               mach_vm_size_t size,
                                               int trial_flags) {
  assert(unused_flags == 0);
  return help_call_remap_fn__src_size_etc(fn, map, trial_flags, copy, src, size,
                                          0, 0, VM_INHERIT_NONE);
}

static kern_return_t
help_call_remap_fn__prot_pairs(remap_fn_t fn, MAP_T map, int flags, bool copy,
                               mach_vm_address_t src, mach_vm_size_t size,
                               vm_prot_t cur, vm_prot_t max) {
  return help_call_remap_fn__src_size_etc(fn, map, flags, copy, src, size, cur,
                                          max, VM_INHERIT_NONE);
}

static kern_return_t help_call_remap_fn__src_dst_size(remap_fn_t fn, MAP_T map,
                                                      int flags, bool copy,
                                                      mach_vm_address_t src,
                                                      mach_vm_size_t size,
                                                      mach_vm_address_t dst) {
  ...
  vm_prot_t cur = 0;
  vm_prot_t max = 0;
  kern_return_t kr = fn(map, &out_addr, size, 0, flags, map, src, copy, &cur,
                        &max, VM_INHERIT_NONE);
  deallocate_if_not_fixed_overwrite(kr, map, out_addr, size, flags);
  return kr;
}
...
#define DROP_TYPES(...)                                                        \
  GET_INSTANCE(_0 __VA_OPT__(, ) __VA_ARGS__, DROP_TYPES_8, DROP_TYPES_8,      \
               DROP_TYPES_6, DROP_TYPES_6, DROP_TYPES_4, DROP_TYPES_4,         \
               DROP_TYPES_2, DROP_TYPES_2, DROP_TYPES_0,                       \
               DROP_TYPES_0)(__VA_ARGS__)
...
#define DROP_COMMAS(...)                                                       \
  GET_INSTANCE(_0 __VA_OPT__(, ) __VA_ARGS__, DROP_COMMAS_8, DROP_COMMAS_8,    \
               DROP_COMMAS_6, DROP_COMMAS_6, DROP_COMMAS_4, DROP_COMMAS_4,     \
               DROP_COMMAS_2, DROP_COMMAS_2, DROP_COMMAS_0)(__VA_ARGS__)

// specialize helpers into implementations of call functions that are still
// agnostic to the remap function

#define IMPL_ONE_FROM_HELPER(type, variant, flags, copy, ...)                  \
  static kern_return_t call_remap_fn##__##variant##__##type(                   \
      remap_fn_t fn, MAP_T map, mach_vm_address_t src,                         \
      mach_vm_size_t size DROP_COMMAS(__VA_ARGS__)) {                          \
    return help_call_remap_fn__##type(fn, map, flags, copy, src,               \
                                      size DROP_TYPES(__VA_ARGS__));           \
  }

#define IMPL_FROM_HELPER(type, ...)                                            \
  IMPL_ONE_FROM_HELPER(type, fixed, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,       \
                       false, ##__VA_ARGS__)                                   \
  IMPL_ONE_FROM_HELPER(type, fixed_copy, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,  \
                       true, ##__VA_ARGS__)                                    \
...
// define call functions that are specific to the remap function, and rely on
// implementations above under the hood

#define IMPL_REMAP_FN_HELPER(remap_fn, instance, type, ...)                    \
  static kern_return_t call_##remap_fn##__##instance##__##type(                \
      MAP_T map DROP_COMMAS(__VA_ARGS__)) {                                    \
    return call_remap_fn__##instance##__##type(remap_fn,                       \
                                               map DROP_TYPES(__VA_ARGS__));   \
  }

#define IMPL_REMAP_FN_SRC_SIZE(remap_fn, instance)                             \
  IMPL_REMAP_FN_HELPER(remap_fn, instance, src_size, mach_vm_address_t, src,   \
                       mach_vm_size_t, size)
#define IMPL_REMAP_FN_DST_SIZE(remap_fn, instance)                             \
  IMPL_REMAP_FN_HELPER(remap_fn, instance, dst_size, mach_vm_address_t, src,   \
                       mach_vm_size_t, size)
#define IMPL_REMAP_FN_SRC_DST_SIZE(remap_fn, instance)                         \
  IMPL_REMAP_FN_HELPER(remap_fn, instance, src_dst_size, mach_vm_address_t,    \
                       src, mach_vm_size_t, size, mach_vm_address_t, dst)
#define IMPL_REMAP_FN_SRC_SIZE_INHERIT(remap_fn, instance)                     \
  IMPL_REMAP_FN_HELPER(remap_fn, instance, inherit, mach_vm_address_t, src,    \
                       mach_vm_size_t, size, vm_inherit_t, inherit)
#define IMPL_REMAP_FN_SRC_SIZE_FLAGS(remap_fn, instance)                       \
  IMPL_REMAP_FN_HELPER(remap_fn, instance, flags, mach_vm_address_t, src,      \
                       mach_vm_size_t, size, int, flags)
#define IMPL_REMAP_FN_PROT_PAIRS(remap_fn, instance)                           \
  IMPL_REMAP_FN_HELPER(remap_fn, instance, prot_pairs, mach_vm_address_t, src, \
                       mach_vm_size_t, size, vm_prot_t, cur, vm_prot_t, max)
...
static inline void
check_mach_vm_map_outparam_changes(kern_return_t *kr, mach_vm_address_t addr,
                                   mach_vm_address_t saved_addr, int flags,
                                   MAP_T map) {
  if (*kr == KERN_SUCCESS) {
    if (is_fixed(flags)) {
...
static inline void check_mach_vm_remap_outparam_changes(
    kern_return_t *kr, mach_vm_address_t addr, mach_vm_address_t saved_addr,
    int flags, vm_prot_t cur_prot, vm_prot_t saved_cur_prot, vm_prot_t max_prot,
    vm_prot_t saved_max_prot, MAP_T map, mach_vm_address_t src_addr) {
  if (*kr == KERN_SUCCESS) {
    if (is_fixed(flags)) {
      mach_vm_address_t expected_misalignment =
          get_expected_remap_misalignment(map, src_addr, flags);
...
static inline kern_return_t mach_vm_remap_wrapped_kern(
    vm_map_t target_task, mach_vm_address_t *target_address,
    mach_vm_size_t size, mach_vm_offset_t mask, int flags, vm_map_t src_task,
    mach_vm_address_t src_address, boolean_t copy, vm_prot_t *cur_protection,
    vm_prot_t *max_protection, vm_inherit_t inheritance) {
  ...
  kern_return_t kr = mach_vm_remap(target_task, target_address, size, mask,
                                   flags, src_task, src_address, copy,
                                   cur_protection, max_protection, inheritance);
  check_mach_vm_remap_outparam_changes(
      &kr, *target_address, saved_addr, flags, *cur_protection, saved_cur_prot,
      *max_protection, saved_max_prot, target_task, src_address);
  return kr;
}
IMPL(mach_vm_remap_wrapped_kern)

static inline kern_return_t mach_vm_remap_new_kernel_wrapped(
    vm_map_t target_task, mach_vm_address_t *target_address,
    mach_vm_size_t size, mach_vm_offset_t mask, int flags, vm_map_t src_task,
    mach_vm_address_t src_address, boolean_t copy, vm_prot_t *cur_protection,
    vm_prot_t *max_protection, vm_inherit_t inheritance) {
  ...
  vm_prot_t saved_cur_prot = *cur_protection;
  vm_prot_t saved_max_prot = *max_protection;
  kern_return_t kr = mach_vm_remap_new_kernel(
      target_task, target_address, size, mask,
      FLAGS_AND_TAG(flags, VM_KERN_MEMORY_OSFMK), src_task, src_address, copy,
      cur_protection, max_protection, inheritance);
  // remap_new sets VM_FLAGS_RETURN_DATA_ADDR
  check_mach_vm_remap_outparam_changes(
      &kr, *target_address, saved_addr, flags | VM_FLAGS_RETURN_DATA_ADDR,
      *cur_protection, saved_cur_prot, *max_protection, saved_max_prot,
      target_task, src_address);
  return kr;
}
...
mach_vm_remap_user(vm_map_t target_task, mach_vm_address_t *target_address,
...
static inline kern_return_t
mach_vm_remap_new_user(vm_map_t target_task, mach_vm_address_t *target_address,
                       mach_vm_size_t size, mach_vm_offset_t mask, int flags,
                       vm_map_t src_task, mach_vm_address_t src_address,
                       boolean_t copy, vm_prot_t *cur_protection,
                       vm_prot_t *max_protection, vm_inherit_t inheritance) {
  ...
  vm_prot_t saved_cur_prot = *cur_protection;
  vm_prot_t saved_max_prot = *max_protection;
  kern_return_t kr = mach_vm_remap_new(
      target_task, target_address, size, mask, flags, src_task, src_address,
...
static inline kern_return_t
vm_remap_retyped(vm_map_t target_task, mach_vm_address_t *target_address,
                 mach_vm_size_t size, mach_vm_offset_t mask, int flags,
                 vm_map_t src_task, mach_vm_address_t src_address,
                 boolean_t copy, vm_prot_t *cur_protection,
                 vm_prot_t *max_protection, vm_inherit_t inheritance) {
  ...
  vm_prot_t saved_max_prot = *max_protection;
  kern_return_t kr =
      vm_remap(target_task, &addr, (vm_size_t)size, (vm_address_t)mask, flags,
               src_task, (vm_address_t)src_address, copy, cur_protection,
               max_protection, inheritance);
  check_mach_vm_remap_outparam_changes(&kr, addr, (vm_address_t)*target_address,
                                       flags, *cur_protection, saved_cur_prot,
                                       *max_protection, saved_max_prot,
                                       target_task, src_address);
  *target_address = addr;
  return kr;
}
...
#define IMPL(NAME, T)                                                          \
  ...
  /* Run each trial with an allocated vm region and start/size parameters that \
   * reference it. */                                                          \
  typedef kern_return_t (*NAME##mach_with_start_size_fn)(MAP_T map, T start,   \
                                                         T size);              \
                                                                               \
  /* ...and the allocation has a specified minimum alignment */                \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_aligned_start_size(                         \
      NAME##mach_with_start_size_fn fn, T align_mask, const char *testname) {  \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_ALIGNED_VM(map, TEST_ALLOC_SIZE,          \
                                                align_mask, VM_PROT_DEFAULT);  \
    start_size_trials_t *trials SMART_START_SIZE_TRIALS(base.addr);            \
    results_t *results = alloc_results(testname, eSMART_START_SIZE_TRIALS,     \
                                       base.addr, trials->count);              \
                                                                               \
  ...
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_start_size(                                 \
      NAME##mach_with_start_size_fn fn, const char *testname) {                \
    return test_##NAME##mach_with_allocated_aligned_start_size(fn, 0,          \
                                                               testname);      \
  }                                                                            \
                                                                               \
  /* Test a Mach function. */                                                  \
  /* Run each trial with an allocated vm region and an addr parameter that     \
   * reference it. */                                                          \
  typedef kern_return_t (*NAME##mach_with_addr_fn)(MAP_T map, T addr);         \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_addr_of_size_n(                             \
      NAME##mach_with_addr_fn fn, size_t obj_size, const char *testname) {     \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    addr_trials_t *trials SMART_ADDR_TRIALS(base.addr);                        \
    /* Do all the addr trials and an additional trial such that obj_size +     \
     * addr == 0 */                                                            \
    uint64_t trial_args[TRIALSARGUMENTS_SIZE] = {base.addr, obj_size};         \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_ADDR_TRIALS, trial_args,                \
                      TRIALSARGUMENTS_SIZE, trials->count + 1);                \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  typedef kern_return_t (*NAME##mach_with_addr_fn)(MAP_T map, T addr);         \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_addr(NAME##mach_with_addr_fn fn,            \
                                        const char *testname) {                \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    addr_trials_t *trials SMART_ADDR_TRIALS(base.addr);                        \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_ADDR_TRIALS, base.addr, trials->count); \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
      T addr = (T)trials->list[i].addr;                                        \
      kern_return_t ret = fn(map, addr);                                       \
    ...
    return results;                                                            \
                                                                               ...
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_purgeable_addr(NAME##mach_with_addr_fn fn,  \
                                                  const char *testname) {      \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_PURGEABLE_VM(map, TEST_ALLOC_SIZE,        \
                                                  VM_PROT_DEFAULT);            \
    addr_trials_t *trials SMART_ADDR_TRIALS(base.addr);                        \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_ADDR_TRIALS, base.addr, trials->count); \
  ...
  typedef kern_return_t (*NAME##mach_with_size_fn)(MAP_T map, T size);         \
                                                                               \
  static results_t *__attribute__((used)) test_##NAME##mach_with_size(         \
      NAME##mach_with_size_fn fn, const char *testname) {                      \
    MAP_T map SMART_MAP;                                                       \
    size_trials_t *trials SMART_SIZE_TRIALS();                                 \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_SIZE_TRIALS, trials->count);            \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
      T size = (T)trials->list[i].size;                                        \
  ...
  /* Test a Mach function. */                                                  \
  /* Run each trial with a size parameter. */                                  \
  typedef kern_return_t (*NAME##mach_with_start_size_offset_object_fn)(        \
      MAP_T map, T addr, T size, T offset, T obj_size);                        \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_start_size_offset_object(                   \
      NAME##mach_with_start_size_offset_object_fn fn, const char *testname) {  \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    start_size_offset_object_trials_t *trials                                  \
    SMART_START_SIZE_OFFSET_OBJECT_TRIALS();                                   \
    results_t *results = alloc_results(                                        \
        testname, eSMART_START_SIZE_OFFSET_OBJECT_TRIALS, trials->count);      \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
      start_size_offset_object_trial_t trial =                                 \
          slide_trial(trials->list[i], base.addr);                             \
  ...
  /* Test a Mach function. */                                                  \
  /* Run each trial with a size parameter. */                                  \
  typedef kern_return_t (*NAME##mach_with_start_size_offset_fn)(               \
      MAP_T map, T addr, T size, T offset, T obj_size);                        \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_start_size_offset(                          \
      NAME##mach_with_start_size_offset_fn fn, const char *testname) {         \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    start_size_offset_trials_t *trials SMART_START_SIZE_OFFSET_TRIALS();       \
    results_t *results = alloc_results(                                        \
        testname, eSMART_START_SIZE_OFFSET_TRIALS, trials->count);             \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
      start_size_offset_trial_t trial =                                        \
          slide_trial(trials->list[i], base.addr);                             \
      T start = (T)trial.start;                                                \
      T size = (T)trial.size;                                                  \
      T offset = (T)trial.offset;                                              \
  ...
  typedef kern_return_t (*NAME##mach_with_allocated_mmap_flags_fn)(            \
      MAP_T map, T addr, T size, int flags);                                   \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_mmap_flags(                                 \
      NAME##mach_with_allocated_mmap_flags_fn fn, const char *testname) {      \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    mmap_flags_trials_t *trials SMART_MMAP_FLAGS_TRIALS();                     \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_MMAP_FLAGS_TRIALS, trials->count);      \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  typedef kern_return_t (*NAME##mach_with_allocated_generic_flag)(             \
      MAP_T map, T addr, T size, int flag);                                    \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_generic_flag(                               \
      NAME##mach_with_allocated_generic_flag fn, const char *testname) {       \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    generic_flag_trials_t *trials SMART_GENERIC_FLAG_TRIALS();                 \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_GENERIC_FLAG_TRIALS, trials->count);    \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  typedef kern_return_t (*NAME##mach_with_prot_fn)(MAP_T map, T size,          \
                                                   vm_prot_t prot);            \
                                                                               \
  static results_t *__attribute__((used)) test_##NAME##mach_vm_prot(           \
      NAME##mach_with_prot_fn fn, const char *testname) {                      \
    MAP_T map SMART_MAP;                                                       \
    vm_prot_trials_t *trials SMART_VM_PROT_TRIALS();                           \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_VM_PROT_TRIALS, trials->count);         \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  typedef kern_return_t (*NAME##mach_with_prot_pair_fn)(                       \
      MAP_T map, vm_prot_t cur, vm_prot_t max);                                \
                                                                               \
  static results_t *__attribute__((used)) test_##NAME##mach_vm_prot_pair(      \
      NAME##mach_with_prot_pair_fn fn, const char *testname) {                 \
    MAP_T map SMART_MAP;                                                       \
    vm_prot_pair_trials_t *trials SMART_VM_PROT_PAIR_TRIALS();                 \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_VM_PROT_PAIR_TRIALS, trials->count);    \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  /* Test a Mach function. */                                                  \
  /* Run each trial with a pair of vm_prot_t's. */                             \
  typedef kern_return_t (*NAME##mach_with_allocated_prot_pair_fn)(             \
      MAP_T map, T addr, T size, vm_prot_t cur, vm_prot_t max);                \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_vm_prot_pair(                               \
      NAME##mach_with_allocated_prot_pair_fn fn, const char *testname) {       \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    vm_prot_pair_trials_t *trials SMART_VM_PROT_PAIR_TRIALS();                 \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_VM_PROT_PAIR_TRIALS, trials->count);    \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
      kern_return_t ret = fn(map, (T)base.addr, (T)base.size,                  \
                             trials->list[i].cur, trials->list[i].max);        \
      append_result(results, ret, trials->list[i].name);                       \
    }                                                                          \
    return results;                                                            \
  }                                                                            \
  ...
  typedef kern_return_t (*NAME##mach_with_allocated_prot_fn)(                  \
      MAP_T map, T addr, T size, vm_prot_t prot);                              \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_vm_prot_t(                                  \
      NAME##mach_with_allocated_prot_fn fn, const char *testname) {            \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    vm_prot_trials_t *trials SMART_VM_PROT_TRIALS();                           \
  ...
  typedef kern_return_t (*NAME##mach_ledger_flag_fn)(MAP_T map,                \
                                                     int ledger_flag);         \
                                                                               \
  static results_t *__attribute__((used)) test_##NAME##mach_with_ledger_flag(  \
      NAME##mach_ledger_flag_fn fn, const char *testname) {                    \
    MAP_T map SMART_MAP;                                                       \
    ledger_flag_trials_t *trials SMART_LEDGER_FLAG_TRIALS();                   \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_LEDGER_FLAG_TRIALS, trials->count);     \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  typedef kern_return_t (*NAME##mach_ledger_tag_fn)(MAP_T map,                 \
                                                    int ledger_tag);           \
                                                                               \
  static results_t *__attribute__((used)) test_##NAME##mach_with_ledger_tag(   \
      NAME##mach_ledger_tag_fn fn, const char *testname) {                     \
    MAP_T map SMART_MAP;                                                       \
    ledger_tag_trials_t *trials SMART_LEDGER_TAG_TRIALS();                     \
    results_t *results =                                                       \
        alloc_results(testname, eSMART_LEDGER_TAG_TRIALS, trials->count);      \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  typedef kern_return_t (*NAME##mach_inherit_fn)(MAP_T map, T addr, T size,    \
                                                 vm_inherit_t inherit);        \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_vm_inherit_t(NAME##mach_inherit_fn fn,      \
                                                const char *testname) {        \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
  ...
  typedef kern_return_t (*NAME##with_start_end_fn)(MAP_T map, T addr, T end);  \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_start_end(NAME##with_start_end_fn fn,       \
                                             const char *testname) {           \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
  ...
  /* Test a Mach function. */                                                  \
  /* Run each trial with an allocated vm region and a vm_prot_t. */            \
  typedef kern_return_t (*NAME##with_tag_fn)(MAP_T map, T addr, T end,         \
                                             vm_tag_t tag);                    \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_tag(NAME##with_tag_fn fn,                   \
                                       const char *testname) {                 \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    ...
    results_t *results =                                                       \
        alloc_results(testname, eSMART_VM_TAG_TRIALS, trials->count);          \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
      kern_return_t ret = fn(map, (T)base.addr, (T)(base.addr + base.size),    \
                             trials->list[i].tag);                             \
      append_result(results, ret, trials->list[i].name);                       \
    }                                                                          \
    return results;                                                            \
  ...
  typedef kern_return_t (*NAME##mach_behavior_fn)(MAP_T map, T addr, T size,   \
                                                  vm_behavior_t behavior);     \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_aligned_vm_behavior_t(                      \
      NAME##mach_behavior_fn fn, mach_vm_size_t align_mask,                    \
      const char *testname) {                                                  \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_ALIGNED_VM(map, TEST_ALLOC_SIZE,          \
                                                align_mask, VM_PROT_DEFAULT);  \
    ...
    results_t *results =                                                       \
        alloc_results(testname, eSMART_VM_BEHAVIOR_TRIALS, trials->count);     \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
    ...
    return results;                                                            \
                                                                               ...
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_vm_behavior_t(NAME##mach_behavior_fn fn,    \
                                                 const char *testname) {       \
    return test_##NAME##mach_with_allocated_aligned_vm_behavior_t(fn, 0,       \
                                                                  testname);   \
  }                                                                            \
                                                                               \
  /* Test a Mach function. */                                                  \
  /* Run each trial with an allocated region and a vm_sync_t. */               \
  typedef kern_return_t (*NAME##mach_sync_fn)(MAP_T map, T addr, T size,       \
                                              vm_sync_t behavior);             \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_vm_sync_t(NAME##mach_sync_fn fn,            \
                                             const char *testname) {           \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    ...
    results_t *results =                                                       \
        alloc_results(testname, eSMART_VM_SYNC_TRIALS, trials->count);         \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  typedef kern_return_t (*NAME##mach_attribute_fn)(                            \
      MAP_T map, T addr, T size, vm_machine_attribute_t attr);                 \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_vm_machine_attribute_t(                     \
      NAME##mach_attribute_fn fn, const char *testname) {                      \
    MAP_T map SMART_MAP;                                                       \
    allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,                  \
                                        VM_PROT_DEFAULT);                      \
    vm_machine_attribute_trials_t *trials SMART_VM_MACHINE_ATTRIBUTE_TRIALS(); \
    results_t *results = alloc_results(                                        \
        testname, eSMART_VM_MACHINE_ATTRIBUTE_TRIALS, trials->count);          \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
  ...
  typedef kern_return_t (*NAME##mach_purgable_fn)(                             \
      MAP_T map, T addr, vm_purgable_t control, int state);                    \
                                                                               \
  static results_t *__attribute__((used))                                      \
  test_##NAME##mach_with_allocated_purgeable_and_state(                        \
      NAME##mach_purgable_fn fn, const char *testname) {                       \
    MAP_T map SMART_MAP;                                                       \
    vm_purgeable_and_state_trials_t *trials                                    \
    SMART_VM_PURGEABLE_AND_STATE_TRIALS();                                     \
    results_t *results = alloc_results(                                        \
        testname, eSMART_VM_PURGEABLE_AND_STATE_TRIALS, trials->count);        \
                                                                               \
    for (unsigned i = 0; i < trials->count; i++) {                             \
      allocation_t base SMART_ALLOCATE_PURGEABLE_VM(map, TEST_ALLOC_SIZE,      \
                                                    VM_PROT_DEFAULT);          \
...
/*
 * The vm_range_create tests assume we don't ever do range_creates that should
 * succeed that take more than 2 * PAGE_SIZE. This enforces that.
 */
void verify_largest_valid_trial_size_fits(start_size_start_size_trial_t trial) {
...
typedef kern_return_t(mach_with_start_size_start_size_fn)(
    MAP_T map, mach_vm_address_t addr, mach_vm_size_t size,
    mach_vm_address_t second_addr, mach_vm_size_t second_size);

static results_t *__attribute__((used))
test_mach_vm_range_create(mach_with_start_size_start_size_fn fn,
                          const char *testname) {
  start_size_start_size_trials_t *trials SMART_START_SIZE_START_SIZE_TRIALS();
  results_t *results = alloc_results(
      testname, eSMART_START_SIZE_START_SIZE_TRIALS, trials->count);

  for (unsigned i = 0; i < trials->count; i++) {
    /*
     * Allocate and configure a new map for every trial so that the map has no
     * user ranges.
     */
     ...
     * We also want at least 2 * PAGE_SIZE memory available after the start of
     * range2. We additionally start our first range 2 PAGE_SIZE away from the
     * start.
     */
    ...
    if (has_ranges && has_space_in_ranges) {
      ...
      start_size_start_size_trial_t trial =
          slide_trial(trials->list[i], addr_base, second_addr_base);
...
static results_t *test_mach_allocation_func_with_start_size(
    kern_return_t (*func)(MAP_T map, mach_vm_address_t *start,
                          mach_vm_size_t size),
    const char *testname) {
  MAP_T map SMART_MAP;
...
static results_t *test_mach_allocation_func_with_vm_map_kernel_flags_t(
    kern_return_t (*func)(MAP_T map, mach_vm_address_t *start,
                          mach_vm_size_t size, int flags),
    const char *testname) {
  MAP_T map SMART_MAP;
  ...
  results_t *results =
      alloc_results(testname, eSMART_VM_MAP_KERNEL_FLAGS_TRIALS, trials->count);

  for (unsigned i = 0; i < trials->count; i++) {
    allocation_t fixed_overwrite_dst SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,
                                                       VM_PROT_DEFAULT);
    ...
    deallocate_if_not_fixed_overwrite(ret, map, addr, TEST_ALLOC_SIZE,
                                      trial.flags);
    append_result(results, ret, trial.name);
  }
  return results;
...
static results_t *test_mach_with_allocated_vm_map_kernel_flags_t(
...
static results_t *test_unix_with_allocated_vm_prot_t(
    int (*func)(void *start, size_t size, int flags), const char *testname) {
  ...
  for (unsigned i = 0; i < trials->count; i++) {
    int ret =
        func((void *)base.addr, (size_t)base.size, (int)trials->list[i].prot);
    append_result(results, ret, trials->list[i].name);
  }
  return results;
}
...
static results_t *__unused test_unix_with_allocated_aligned_start_size(
    unix_with_start_size_fn fn, mach_vm_size_t align_mask,
    const char *testname) {
  MAP_T map CURRENT_MAP;
...
static results_t *__unused test_unix_with_allocated_start_size(
    unix_with_start_size_fn fn, const char *testname) {
  return test_unix_with_allocated_aligned_start_size(fn, 0, testname);
}

#if KERNEL
static results_t *__unused test_kext_unix_with_allocated_start_size(
    unix_with_start_size_fn fn, const char *testname) {
  MAP_T map CURRENT_MAP;
  allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  start_size_trials_t *trials SMART_START_SIZE_TRIALS(base.addr);
...
static results_t *__attribute__((used)) test_kext_tagged_with_allocated_addr(
    kern_return_t (*func)(MAP_T map, mach_vm_address_t addr),
    const char *testname) {
  MAP_T map CURRENT_MAP;
  allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  addr_trials_t *trials SMART_ADDR_TRIALS(base.addr);
...
extern kern_return_t short_circuit_deallocator(MAP_T map,
                                               start_size_trial_t trial);
...
static inline kern_return_t
short_circuit_deallocator(MAP_T map __unused,
                          start_size_trial_t trial __unused) {
  // Kernel tests run with an empty vm_map so we're free to deallocate whatever
  // we want.
  return 0;
}
...
// Similar to test_mach_with_allocated_addr_size, but mach_vm_deallocate is
// destructive so we can't test all values and we need to re-allocate the vm
// allocation each time.
static results_t *
test_deallocator(kern_return_t (*func)(MAP_T map, mach_vm_address_t start,
                                       mach_vm_size_t size),
                 const char *testname) {
  MAP_T map SMART_MAP;
...
static results_t *test_allocated_src_unallocated_dst_size(
    kern_return_t (*func)(MAP_T map, mach_vm_address_t src, mach_vm_size_t size,
                          mach_vm_address_t dst),
    const char *testname) {
  MAP_T map SMART_MAP;
  ...
  for (unsigned i = 0; i < trials->count; i++) {
    ...
    unallocation_t dst_base SMART_UNALLOCATE_VM_AFTER(map, src_base.addr,
                                                      TEST_ALLOC_SIZE);
...
static inline void check_mach_vm_allocate_outparam_changes(
    kern_return_t *kr, mach_vm_address_t addr, mach_vm_size_t size,
...
static kern_return_t call_mach_vm_behavior_set__start_size__default(
    MAP_T map, mach_vm_address_t start, mach_vm_size_t size) {
  kern_return_t kr =
      mach_vm_behavior_set(map, start, size, VM_BEHAVIOR_DEFAULT);
  return kr;
}

/*
 * VM_BEHAVIOR_CAN_REUSE is additionally tested as it uses slightly different
 * page rounding semantics
 */
static kern_return_t call_mach_vm_behavior_set__start_size__can_reuse(
    MAP_T map, mach_vm_address_t start, mach_vm_size_t size) {
  kern_return_t kr =
      mach_vm_behavior_set(map, start, size, VM_BEHAVIOR_CAN_REUSE);
  return kr;
}

static kern_return_t
call_mach_vm_behavior_set__vm_behavior(MAP_T map, mach_vm_address_t start,
                                       mach_vm_size_t size,
                                       vm_behavior_t behavior) {
...
static void check_mach_vm_purgable_control_outparam_changes(kern_return_t *kr,
                                                            int state,
                                                            int saved_state,
                                                            int control) {
  if (*kr == KERN_SUCCESS) {
...
static void check_mach_vm_region_outparam_changes(
    kern_return_t *kr, MAP_T map, void *info, void *saved_info,
    size_t info_size, mach_port_t object_name, mach_port_t saved_object_name,
    mach_vm_address_t addr, mach_vm_address_t saved_addr, mach_vm_size_t size,
  ...
  if (*kr == KERN_SUCCESS) {
    ...
    if (object_name != saved_object_name || addr != saved_addr ||
        size != saved_size || memcmp(info, saved_info, info_size) != 0) {
      *kr = OUT_PARAM_BAD;
    }
  }
}

static int call_mach_vm_region(MAP_T map, mach_vm_address_t addr) {
  ...
  kern_return_t kr =
      mach_vm_region(map, &addr_cpy, &size_out, VM_REGION_BASIC_INFO_64,
                     (vm_region_info_t)&info, &infoCnt, &object_name_out);
  check_mach_vm_region_outparam_changes(
      &kr, map, &info, &saved_info, sizeof(info), object_name_out, saved_name,
...
static int call_vm_region(MAP_T map, vm_address_t addr) {
  ...
  mach_msg_type_number_t infoCnt = VM_REGION_BASIC_INFO_COUNT_64;
  kern_return_t kr =
      vm_region(map, &addr_cpy, &size_out, VM_REGION_BASIC_INFO_64,
                (vm_region_info_t)&info, &infoCnt, &object_name_out);
  check_mach_vm_region_outparam_changes(
      &kr, map, &info, &saved_info, sizeof(info), object_name_out, saved_name,
      addr_cpy, addr, size_out, saved_size);
...
static void check_mach_vm_page_info_outparam_changes(
    kern_return_t *kr, vm_page_info_basic_data_t info,
    vm_page_info_basic_data_t saved_info, mach_msg_type_number_t count,
