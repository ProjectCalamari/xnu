...
void ktest_start(void) { ktest_emit_start(); }

void ktest_finish(void) { ktest_emit_finish(); }

void ktest_testbegin(const char *test_name) {
...
void ktest_testskip(const char *msg, ...) {
  va_list args;
...
void ktest_perf(const char *metric, const char *unit, double value,
                const char *desc) {
  ktest_current_time = mach_absolute_time();
  ktest_emit_perfdata(metric, unit, value, desc);
}

void ktest_testcase(int success) {
  ktest_current_time = mach_absolute_time();
  ...
  if (ktest_quiet == 0 || ktest_testcase_result == T_RESULT_FAIL ||
      ktest_testcase_result == T_RESULT_UXPASS) {
    ktest_emit_testcase();
  }
...
void ktest_update_test_result_state(void) {
  ktest_test_result =
      ktest_test_result_statetab[ktest_test_result][ktest_testcase_result]
                                [ktest_testcase_mode];
}

void ktest_assertion_check(void) {
  if (ktest_testcase_result == T_RESULT_FAIL ||
      ktest_testcase_result == T_RESULT_UXPASS) {
    ktest_testend();
    panic("XNUPOST: Assertion failed : %s : at %s:%d", ktest_test_name,
          ktest_current_file, ktest_current_line);
  }
}
