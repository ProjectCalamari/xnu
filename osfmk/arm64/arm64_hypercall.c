...
static inline void regs_to_uuid(const uint32_t (*const regs)[4],
...
static inline bool hvc_5(uint64_t *x0, uint64_t *x1, uint64_t *x2, uint64_t *x3,
                         uint64_t *x4) {
  asm volatile("mov   x0, %[i0]"
               "\n"
               "mov   x1, %[i1]"
               "\n"
               "mov   x2, %[i2]"
               ...
               "str   x4, %[o4]"
               "\n"
               : [o0] "=m"(*x0), [o1] "=m"(*x1), [o2] "=m"(*x2), [o3] "=m"(*x3),
                 [o4] "=m"(*x4)
               : [i0] "r"(*x0), [i1] "r"(*x1), [i2] "r"(*x2), [i3] "r"(*x3),
                 [i4] "r"(*x4)
               : "x0", "x1", "x2", "x3", "x4");
  return *(int64_t *)x0 >= 0;
}

static inline bool hvc_2(uint64_t *x0, uint64_t *x1) {
  uint64_t x = 0;
  return hvc_5(x0, x1, &x, &x, &x);
}

static inline bool hvc_1(uint64_t *x0) {
  uint64_t x = 0;
  return hvc_5(x0, &x, &x, &x, &x);
}

static inline bool hvc32_4(uint32_t *w0, uint32_t *w1, uint32_t *w2,
                           uint32_t *w3) {
  asm volatile("mov   w0, %w[i0]"
               "\n"
               "mov   w1, %w[i1]"
               "\n"
               "mov   w2, %w[i2]"
               ...
               "str   w3, %[o3]"
               "\n"
               : [o0] "=m"(*w0), [o1] "=m"(*w1), [o2] "=m"(*w2), [o3] "=m"(*w3)
               : [i0] "r"(*w0), [i1] "r"(*w1), [i2] "r"(*w2), [i3] "r"(*w3)
               : "w0", "w1", "w2", "w3");
  return *(int32_t *)w0 >= 0;
}

static inline bool hvc32_2(uint32_t *w0, uint32_t *w1) {
  uint32_t w = 0;
  return hvc32_4(w0, w1, &w, &w);
}

/* Unique identification */
static bool hvg_get_uid(uint32_t range, uuid_string_t uuid) {
  assert(range == HVC_FID_CPU || range == HVC_FID_OEM);

  uint32_t reg[4] = {[0] = HVC32_FI(range, HVC_FID_UID)};
...
static bool hvg_get_revision(uint32_t range, uint32_t *major, uint32_t *minor) {
  assert(range == HVC_FID_CPU || range == HVC_FID_OEM);
 ...
 */
static bool hvg_get_features(uint32_t range, uint32_t fid, int32_t *features) {
  assert(range == HVC_FID_CPU || range == HVC_FID_OEM);
...
bool hvg_is_hcall_available(hvg_hcall_code_t hcall) {
  int32_t features = 0;
  ...
  uint32_t fastcall = 0;
  if (hcall >= HVG_HCALL_COUNT || (fastcall = hcall_oem_table[hcall]) == 0) {
    return false;
  }
   ...
   * specified by Apple.
   */
  if (!hvg_get_uid(HVC_FID_OEM, uuids) || strcmp(uuids, VMAPPLE_HVC_UID) != 0) {
    return false;
  }
...
hvg_hcall_return_t hvg_hcall_get_bootsessionuuid(uuid_string_t uuid_str) {
  ...
  regs_to_uuid(&(uint32_t[4]){(uint32_t)reg[0], (uint32_t)reg[1],
...
hvg_hcall_return_t hvg_hcall_get_mabs_offset(uint64_t *mabs_offset) {
...
__attribute__((noinline)) void hvg_hc_kick_cpu(unsigned int cpu_id) {
...
__attribute__((noinline)) void hvg_hc_wait_for_kick(unsigned int ien) {
...
bool hvg_is_hcall_available(__unused hvg_hcall_code_t hcall) { return false; }

hvg_hcall_return_t hvg_hcall_get_mabs_offset(__attribute__((unused))
                                             uint64_t *mabs_offset) {
  return HVG_HCALL_UNSUPPORTED;
}
...
/* Unsupported. */
void hvg_hcall_set_coredump_data(void) {}
