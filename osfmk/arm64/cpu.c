...
#if APPLEVIRTUALPLATFORM
extern void
    __attribute__((noreturn)) arm64_prepare_for_sleep(boolean_t deep_sleep,
                                                      unsigned int cpu,
                                                      uint64_t entry_pa);
#else
extern void
...
TUNABLE(uint32_t, expecting_ipi_wfe_timeout_usec,
        "expecting_ipi_wfe_timeout_usec",
        DEFAULT_EXPECTING_IPI_WFE_TIMEOUT_USEC);
uint64_t expecting_ipi_wfe_timeout_mt =
    0x0ULL; /* initialized to a non-zero value in sched_init */
 ...
 */
static const uint8_t __attribute__((aligned(8))) suspend_signature[] = {
    'X', 'S', 'O', 'M', 'P', 'S', 'U', 'S'};
static const uint8_t __attribute__((aligned(8))) running_signature[] = {
    'X', 'S', 'O', 'M', 'N', 'N', 'U', 'R'};
#endif
...
void arm64_immediate_ipi_test_callback(void *parm) {
  volatile uint64_t *ipi_test_data = parm;
  cpu_data_t *cpu_data;
...
MACHINE_TIMEOUT(arm64_ipi_test_timeout, "arm64-ipi-test", 100,
                MACHINE_TIMEOUT_UNIT_MSEC, NULL);
...
void arm64_ipi_test() {
  ...
  for (unsigned int i = 0; i <= max_cpu_id; ++i) {
    ...
    *ipi_test_data = ~i;
    kern_return_t error = cpu_xcall((int)i, (void *)arm64_ipi_test_callback,
                                    (void *)(uintptr_t)ipi_test_data);
    if (error != KERN_SUCCESS) {
      panic("CPU %d was unable to IPI CPU %u: error %d", current_cpu_number, i,
            error);
    }

    while ((error = cpu_immediate_xcall(
                (int)i, (void *)arm64_immediate_ipi_test_callback,
                (void *)(uintptr_t)immediate_ipi_test_data)) ==
           KERN_ALREADY_WAITING) {
      now = mach_absolute_time();
      absolutetime_to_nanoseconds(now - then, &delta);
      if ((delta / NSEC_PER_MSEC) > timeout_ms) {
        panic("CPU %d was unable to immediate-IPI CPU %u within %lldms",
              current_cpu_number, i, timeout_ms);
      }
    }

    if (error != KERN_SUCCESS) {
      panic("CPU %d was unable to immediate-IPI CPU %u: error %d",
            current_cpu_number, i, error);
    }

    then = mach_absolute_time();

    while ((*ipi_test_data != i) ||
           (*immediate_ipi_test_data != (i + MAX_CPUS))) {
      now = mach_absolute_time();
      absolutetime_to_nanoseconds(now - then, &delta);
      if ((delta / NSEC_PER_MSEC) > timeout_ms) {
        panic("CPU %d tried to IPI CPU %d but didn't get correct responses "
              "within %lldms, responses: %llx, %llx",
              current_cpu_number, i, timeout_ms, *ipi_test_data,
...
static void configure_coresight_registers(cpu_data_t *cdp) {
  int i;

  assert(cdp);
  vm_offset_t coresight_regs =
      ml_get_topology_info()->cpus[cdp->cpu_number].coresight_regs;

  /*
   * ARMv8 coresight registers are optional. If the device tree did not
   * provide either cpu_regmap_paddr (from the legacy "reg-private" EDT
   * property) or coresight_regs (from the new "coresight-reg" property), assume
   * that coresight registers are not supported.
   */
  if (cdp->cpu_regmap_paddr || coresight_regs) {
    for (i = 0; i < CORESIGHT_REGIONS; ++i) {
      ...
      if (i != CORESIGHT_UTT) {
        *(volatile uint32_t *)(cdp->coresight_base[i] +
...
void cpu_sleep(void) {
...
#if CONFIG_SPTM
  cpu_data_ptr->cpu_reset_handler =
      (uintptr_t)VM_KERNEL_STRIP_PTR(arm_init_cpu);
#else
  ...
  if (ml_is_quiescing()) {
    ...
    CleanPoU_Dcache();
#if APPLEVIRTUALPLATFORM
    arm64_prepare_for_sleep(deep_sleep, cpu_data_ptr->cpu_number,
                            ml_vtophys(reset_vector_vaddr));
...
static bool cpu_proximate_timer(void) { return !SetIdlePop(); }
 ...
 *	Function:
 */
void __attribute__((noreturn)) cpu_idle(void) {
  cpu_data_t *cpu_data_ptr = getCpuDatap();
  ...
  if (wfe_timeout != 0) {
     ...
     */
    KDBG(CPUPM_IDLE_WFE | DBG_FUNC_START, ipending, cpu_data_ptr->wfe_count,
         wfe_timeout, !check_cluster_recommendation);
    ipending = wfe_to_deadline_or_interrupt(
        cid, wfe_deadline, cpu_data_ptr, false, check_cluster_recommendation);
    KDBG(CPUPM_IDLE_WFE | DBG_FUNC_END, ipending, cpu_data_ptr->wfe_count,
         wfe_deadline);
    if (ipending == true) {
  ...
  if (__improbable(cpu_proximate_timer())) {
    if (idle_proximate_timer_wfe == 1) {
       ...
       * timer FIQ arrives.
       */
      KDBG(CPUPM_IDLE_TIMER_WFE | DBG_FUNC_START, ipending,
           cpu_data_ptr->wfe_count, ~0ULL);
      ipending =
          wfe_to_deadline_or_interrupt(cid, ~0ULL, cpu_data_ptr, false, false);
      KDBG(CPUPM_IDLE_TIMER_WFE | DBG_FUNC_END, ipending,
           cpu_data_ptr->wfe_count, ~0ULL);
  ...
  if (wfi && (cpu_data_ptr->cpu_idle_notify != NULL)) {
    cpu_data_ptr->cpu_idle_notify(cpu_data_ptr->cpu_id, TRUE,
 ...
 *	Function:
 */
void cpu_idle_exit(boolean_t from_reset) {
  ...
  if (wfi && (cpu_data_ptr->cpu_idle_notify != NULL)) {
    cpu_data_ptr->cpu_idle_notify(cpu_data_ptr->cpu_id, FALSE,
                                  &new_idle_timeout_ticks);
  }
...
void cpu_stack_alloc(cpu_data_t *cpu_data_ptr) {
  vm_offset_t irq_stack = 0;
  vm_offset_t exc_stack = 0;

  kmem_alloc(kernel_map, &irq_stack, INTSTACK_SIZE + ptoa(2),
             KMA_NOFAIL | KMA_PERMANENT | KMA_ZERO | KMA_GUARD_FIRST |
                 KMA_GUARD_LAST | KMA_KSTACK | KMA_KOBJECT,
             VM_KERN_MEMORY_STACK);

  cpu_data_ptr->intstack_top = irq_stack + PAGE_SIZE + INTSTACK_SIZE;
  cpu_data_ptr->istackptr = (void *)cpu_data_ptr->intstack_top;

  kmem_alloc(kernel_map, &exc_stack, EXCEPSTACK_SIZE + ptoa(2),
             KMA_NOFAIL | KMA_PERMANENT | KMA_ZERO | KMA_GUARD_FIRST |
                 KMA_GUARD_LAST | KMA_KSTACK | KMA_KOBJECT,
             VM_KERN_MEMORY_STACK);
...
void cpu_data_init(cpu_data_t *cpu_data_ptr) {
  uint32_t i;
  ...
  bzero(&(pmap_cpu_data_ptr->cpu_sw_asids[0]),
        sizeof(pmap_cpu_data_ptr->cpu_sw_asids));
#endif
#endif /* !XNU_MONITOR */
...
void cpu_data_register(cpu_data_t *cpu_data_ptr) {
  int cpu = cpu_data_ptr->cpu_number;

#if KASAN
  for (int i = 0; i < CPUWINDOWS_MAX; i++) {
    kasan_notify_address_nopoison(pmap_cpu_windows_copy_addr(cpu, i),
                                  PAGE_SIZE);
  }
#endif
  ...
  CpuDataEntries[cpu].cpu_data_paddr =
      (void *)ml_vtophys((vm_offset_t)cpu_data_ptr);
}

#if defined(KERNEL_INTEGRITY_CTRR)
/* Hibernation needs to reset this state, so data and text are in the hib
 * segment; this allows them be accessed and executed early.
 */
LCK_GRP_DECLARE(ctrr_cpu_start_lock_grp, "ctrr_cpu_start_lock");
LCK_SPIN_DECLARE(ctrr_cpu_start_lck, &ctrr_cpu_start_lock_grp);
enum ctrr_cluster_states
    ctrr_cluster_locked[MAX_CPU_CLUSTERS] MARK_AS_HIBERNATE_DATA;

MARK_AS_HIBERNATE_TEXT
void init_ctrr_cluster_states(void) {
...
void cpu_start(int cpu) {
  ...
  if (cpu == cpu_number()) {
...
#if CONFIG_SPTM
    cpu_data_ptr->cpu_reset_handler =
        (vm_offset_t)VM_KERNEL_STRIP_PTR(arm_init_cpu);
#else
    ...
    first_thread->machine.pcpu_data_base_and_cpu_number =
        ml_make_pcpu_base_and_cpu_number((vm_address_t)cpu_data_ptr -
                                             __PERCPU_ADDR(cpu_data),
                                         cpu_data_ptr->cpu_number);

    configure_coresight_registers(cpu_data_ptr);

    flush_dcache((vm_offset_t)&CpuDataEntries[cpu], sizeof(cpu_data_entry_t),
                 FALSE);
    flush_dcache((vm_offset_t)cpu_data_ptr, sizeof(cpu_data_t), FALSE);
#if CONFIG_SPTM
    /**
     * On SPTM devices, CTRR is configured entirely by the SPTM. Due to this,
     * this logic is no longer required in XNU.
     */
#else
#if defined(KERNEL_INTEGRITY_CTRR)

    /* First CPU being started within a cluster goes ahead to lock CTRR for
     * cluster; other CPUs block until cluster is locked. */
    ...
    PE_cpu_start_internal(cpu_data_ptr->cpu_id, (vm_offset_t)NULL,
                          (vm_offset_t)NULL);
  }
}

void cpu_timebase_init(boolean_t from_boot) {
  cpu_data_t *cdp = getCpuDatap();
  ...
  if (!from_boot && (cdp == &BootCpuData) && ml_is_quiescing()) {
    ...
    if (PE_parse_boot_argn("timebase_offset", &timebase_offset,
                           sizeof(timebase_offset))) {
...
int cpu_cluster_id(void) { return getCpuDatap()->cpu_cluster_id; }

__attribute__((noreturn)) void ml_arm_sleep(void) {
  cpu_data_t *cpu_data_ptr = getCpuDatap();
  ...
  if (cpu_data_ptr == &BootCpuData) {
    ...
    if (sleepTokenBuffer != (vm_offset_t)NULL) {
      ...
      bcopy((const void *)suspend_signature, (void *)sleepTokenBuffer,
            sizeof(SleepToken));
    } else {
    ...
    if (cpu_data_ptr->coresight_base[CORESIGHT_ED]) {
      *(volatile uint32_t *)(cpu_data_ptr->coresight_base[CORESIGHT_ED] +
                             ARM_DEBUG_OFFSET_DBGLAR) = ARM_DBG_LOCK_ACCESS_KEY;
      *(volatile uint32_t *)(cpu_data_ptr->coresight_base[CORESIGHT_ED] +
                             ARM_DEBUG_OFFSET_DBGPRCR) = 0;
    }
    ...
    arm64_prepare_for_sleep(true, cpu_data_ptr->cpu_number,
                            ml_vtophys(reset_vector_vaddr));
#else  /* APPLEVIRTUALPLATFORM */
...
#if __ARM_GLOBAL_SLEEP_BIT__
    /*
     * With the exception of the CPU revisions listed above, our ARM64 CPUs have
     * a global register to manage entering deep sleep, as opposed to a per-CPU
     * register.  We cannot update this register until all CPUs are ready to
     * enter deep sleep, because if a CPU executes WFI outside of the deep sleep
     * context (by idling), it will hang (due to the side effects of enabling
     * deep sleep), which can hang the sleep process or cause memory corruption
     * on wake.
     *
...
void cpu_machine_idle_init(boolean_t from_boot) {
  ...
  if (from_boot) {
    ...
    case 2:
      ...
      nanoseconds_to_absolutetime(((wfi >> 16) & 0xFFFF) * NSEC_PER_MSEC,
                                  &wfi_delay);
      break;
#endif /* DEVELOPMENT || DEBUG */
    ...
    ResetHandlerData.cpu_data_entries =
        ml_static_vtop((vm_offset_t)CpuDataEntries);
#endif

#ifdef MONITOR
    monitor_call(MONITOR_SET_ENTRY,
                 (uintptr_t)ml_static_vtop((vm_offset_t)&LowResetVectorBase), 0,
                 0);
...
#error MONITOR undefined, WFI power gating may not operate correctly
    ...
    if (kSuccess == SecureDTLookupEntry(NULL, "/chosen", &entry)) {
      ...
      if (kSuccess == SecureDTGetProperty(entry,
...
#if CONFIG_SPTM
  cpu_data_ptr->cpu_reset_handler =
      (uintptr_t)VM_KERNEL_STRIP_PTR(arm_init_idle_cpu);
#else
...
void sleep_token_buffer_init(void) {
  ...
  if ((cpu_data_ptr == &BootCpuData) &&
      (sleepTokenBuffer == (vm_offset_t)NULL)) {
    /* Find the stpage node in the device tree */
    if (kSuccess != SecureDTLookupEntry(0, "stram", &entry)) {
      return;
    }

    if (kSuccess != SecureDTGetProperty(entry, "reg", (const void **)&prop,
                                        (unsigned int *)&size)) {
      return;
    }
    ...
    sleepTokenBuffer =
        ml_io_map(((vm_offset_t const *)prop)[0], ((vm_size_t const *)prop)[1]);
  }
}
#endif
