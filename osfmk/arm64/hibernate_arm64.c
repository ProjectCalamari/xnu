...
void pal_hib_teardown_pmap_structs(__unused addr64_t *unneeded_start,
                                   __unused addr64_t *unneeded_end) {}

void pal_hib_rebuild_pmap_structs(void) {}

static void set_dram_range(hibernate_bitmap_t *range, uint64_t start_addr,
                           uint64_t size) {
  ...
  assert(range->first_page ==
         first_page); // make sure the truncation wasn't lossy

  range->last_page = (uint32_t)last_page;
  assert(range->last_page ==
         last_page); // make sure the truncation wasn't lossy
}

// Comparison function used to sort the DRAM ranges list.
static int dram_range_compare(const void *a, const void *b) {
  return ((const hibernate_bitmap_t *)a)->first_page -
         ((const hibernate_bitmap_t *)b)->first_page;
}
...
hibernate_page_list_t *hibernate_page_list_allocate(boolean_t log) {
  ...
  // Sort the DRAM ranges based on the first page. Other parts of the
  // hibernation flow expect these ranges to be in order.
  qsort((void *)dram_ranges, num_banks, sizeof(dram_ranges[0]),
        dram_range_compare);
  ...
  for (bank = 0; bank < num_banks; bank++) {
    ...
    bitmap->bitmapwords =
        (bitmap->last_page + 1 - bitmap->first_page + 31) >> 5;
    if (log) {
...
void pal_hib_get_stack_pages(vm_offset_t *first_page, vm_offset_t *page_count) {
  ...
  *first_page =
      atop_64(SPTMArgs->hib_metadata->protected_metadata.hibtext_stack_top) - 1;
  *page_count = 1;
#else
...
void hibernate_page_list_setall_machine(hibernate_page_list_t *page_list,
  ...
  vm_offset_t pmap_stack_page_count =
      atop_64(pmap_stacks_end_pa - pmap_stacks_start_pa);
#endif /* XNU_MONITOR */
  ...
  if (!preflight) {
     ...
     */
    hibernate_set_page_state(page_list, page_list_wired, stack_first_page,
                             stack_page_count, kIOHibernatePageStateWiredSave);
    ...
    hibernate_set_page_state(page_list, page_list_wired,
                             atop_64(pmap_stacks_start_pa),
                             pmap_stack_page_count, kIOHibernatePageStateFree);
#endif /* XNU_MONITOR */
    ...
    bool (^exclude)(pmap_io_range_t const *) =
        ^bool(pmap_io_range_t const *range) {
          if (range->wimg & PMAP_IO_RANGE_PROHIBIT_HIB_WRITE) {
            ...
            hibernate_set_page_state(
                page_list, page_list_wired, range->addr >> PAGE_SHIFT,
                range->len >> PAGE_SHIFT, kIOHibernatePageStateWiredSave);
          }
          return true;
        };
...
void hibernate_page_list_set_volatile(hibernate_page_list_t *page_list,
  ...
  hibernate_set_page_state(page_list, page_list_wired, page, count,
  ...
  *pagesOut -= count;
  ...
  bool (^exclude)(pmap_io_range_t const *) =
      ^bool(pmap_io_range_t const *range) {
        if (range->wimg & PMAP_IO_RANGE_PROHIBIT_HIB_WRITE) {
          ...
          hibernate_set_page_state(
              page_list, page_list_wired, range->addr >> PAGE_SHIFT,
              range->len >> PAGE_SHIFT, kIOHibernatePageStateFree);
        }
        return true;
      };
  pmap_range_iterate(exclude);
  ...
  assertf(SPTMArgs->hib_metadata->iboot_loaded_ranges != NULL,
          "SPTM didn't setup iboot_loaded_ranges pointer in the hibernation "
          "metadata.");

  for (size_t i = 0; i < SPTMArgs->hib_metadata->num_iboot_loaded_ranges; ++i) {
    const hib_phys_range_t *range =
        &SPTMArgs->hib_metadata->iboot_loaded_ranges[i];
    hibernate_set_page_state(page_list, page_list_wired, range->first_page,
                             range->page_count, kIOHibernatePageStateFree);
...
// processor_doshutdown() calls hibernate_vm_lock() and hibernate_vm_unlock() on
// sleep with interrupts disabled. ml_hibernate_active_post() calls
// hibernate_vm_lock_end() on wake before interrupts are enabled. VM locks are
// safely single threaded between hibernate_vm_lock() and
