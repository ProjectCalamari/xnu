...
int main(int argc, char **argv) {
...
#if CONFIG_EXCLAVES
  DECLARE("TH_EXCLAVES_INTSTATE",
          offsetof(struct thread, th_exclaves_intstate));
  ...
  DECLARE("TH_KSTACKPTR_DIVERSIFIER",
          ptrauth_string_discriminator("machine_thread.kstackptr"));
#endif
...
#if CONFIG_XNUPOST
  DECLARE("TH_EXPECTED_FAULT_HANDLER",
          offsetof(struct thread, machine.expected_fault_handler));
#if __has_feature(ptrauth_calls)
  DECLARE(
      "TH_EXPECTED_FAULT_HANDLER_DIVERSIFIER",
      ptrauth_function_pointer_type_discriminator(expected_fault_handler_t));
#endif /* ptrauth_calls */
  DECLARE("TH_EXPECTED_FAULT_PC",
          offsetof(struct thread, machine.expected_fault_pc));
  DECLARE("TH_EXPECTED_FAULT_ADDR",
          offsetof(struct thread, machine.expected_fault_addr));
#endif /* CONFIG_XNUPOST */

  DECLARE("TH_ARM_MACHINE_FLAGS",
          offsetof(struct thread, machine.arm_machine_flags));

  /* These fields are being added on demand */
  DECLARE("TH_UPCB", offsetof(struct thread, machine.upcb));
#if __has_feature(ptrauth_calls)
  DECLARE("TH_UPCB_DIVERSIFIER",
          ptrauth_string_discriminator("machine_thread.upcb"));
#endif
#if HAS_ARM_FEAT_SME
  DECLARE("ACT_UMATRIX_HDR", offsetof(struct thread, machine.umatrix_hdr));
  DECLARE("ACT_UMATRIX_HDR_DIVERSIFIER",
          ptrauth_string_discriminator("machine_thread.umatrix_hdr"));
  ...
  DECLARE("SS64_KERNEL_PC_WAS_IN_USER",
          offsetof(arm_kernel_context_t, ss.pc_was_in_userspace));
...
#if __has_feature(ptrauth_calls)
  DECLARE("CPU_ISTACKPTR_DIVERSIFIER",
          ptrauth_string_discriminator("cpu_data.istackptr"));
#endif /* ptrauth_calls */
  DECLARE("CPU_INTSTACK_TOP", offsetof(cpu_data_t, intstack_top));
  DECLARE("CPU_EXCEPSTACKPTR", offsetof(cpu_data_t, excepstackptr));
#if __has_feature(ptrauth_calls)
  DECLARE("CPU_EXCEPSTACKPTR_DIVERSIFIER",
          ptrauth_string_discriminator("cpu_data.excepstackptr"));
#endif /* ptrauth_calls */
  ...
  DECLARE("CPU_DATA_ENTRIES",
          offsetof(struct reset_handler_data, cpu_data_entries));
...
#if XNU_MONITOR
  DECLARE("PMAP_CPU_DATA_INFLIGHT_PMAP",
          offsetof(struct pmap_cpu_data, inflight_pmap));
  DECLARE("PMAP_CPU_DATA_PPL_STATE", offsetof(struct pmap_cpu_data, ppl_state));
  DECLARE("PMAP_CPU_DATA_ARRAY_ENTRY_SIZE",
          sizeof(struct pmap_cpu_data_array_entry));
  DECLARE("PMAP_CPU_DATA_PPL_STACK", offsetof(struct pmap_cpu_data, ppl_stack));
  DECLARE("PMAP_CPU_DATA_KERN_SAVED_SP",
          offsetof(struct pmap_cpu_data, ppl_kern_saved_sp));
  DECLARE("PMAP_CPU_DATA_SAVE_AREA", offsetof(struct pmap_cpu_data, save_area));
#if HAS_GUARDED_IO_FILTER
  DECLARE("PMAP_CPU_DATA_IOFILTER_STACK",
          offsetof(struct pmap_cpu_data, iofilter_stack));
  DECLARE("PMAP_CPU_DATA_IOFILTER_SAVED_SP",
          offsetof(struct pmap_cpu_data, iofilter_saved_sp));
#endif
  ...
  DECLARE("THREAD_CONTINUE_T_DISC",
          __builtin_ptrauth_type_discriminator(thread_continue_t));
#else
...
#if HIBERNATION
  DECLARE("HIBHDR_STACKOFFSET",
          offsetof(IOHibernateImageHeader, restore1StackOffset));
  ...
  DECLARE("PANIC_LOCKDOWN_INITIATOR_STATE_INITIATOR_PC",
          offsetof(struct panic_lockdown_initiator_state, initiator_pc));
  DECLARE("PANIC_LOCKDOWN_INITIATOR_STATE_INITIATOR_SP",
          offsetof(struct panic_lockdown_initiator_state, initiator_sp));
  DECLARE("PANIC_LOCKDOWN_INITIATOR_STATE_INITIATOR_TPIDR",
          offsetof(struct panic_lockdown_initiator_state, initiator_tpidr));
  DECLARE("PANIC_LOCKDOWN_INITIATOR_STATE_INITIATOR_MPIDR",
          offsetof(struct panic_lockdown_initiator_state, initiator_mpidr));
  DECLARE("PANIC_LOCKDOWN_INITIATOR_STATE_TIMESTAMP",
          offsetof(struct panic_lockdown_initiator_state, timestamp));
  DECLARE("PANIC_LOCKDOWN_INITIATOR_STATE_ESR",
          offsetof(struct panic_lockdown_initiator_state, esr));
  DECLARE("PANIC_LOCKDOWN_INITIATOR_STATE_ELR",
          offsetof(struct panic_lockdown_initiator_state, elr));
  DECLARE("PANIC_LOCKDOWN_INITIATOR_STATE_FAR",
          offsetof(struct panic_lockdown_initiator_state, far));
