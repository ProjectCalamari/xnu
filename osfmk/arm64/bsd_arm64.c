...
static void arm_set_mach_syscall_ret(struct arm_saved_state *state,
                                     int retval) {
...
static kern_return_t arm_get_mach_syscall_args(struct arm_saved_state *state,
                                               struct mach_call_args *dest,
                                               const mach_trap_t *trapp) {
  uint32_t reg_count;

  if (is_saved_state32(state)) {
    /* The trap table entry defines the number of 32-bit words to be copied in
     * from userspace. */
     ...
     */
    if (reg_count <= 7) {
      bcopy((char *)saved_state32(state), (char *)dest,
            sizeof(uint32_t) * reg_count);
    } else if (reg_count <= 9) {
      bcopy((char *)saved_state32(state), (char *)dest, sizeof(uint32_t) * 7);
      bcopy((char *)&saved_state32(state)->r[8],
            ((char *)dest) + sizeof(uint32_t) * 7, reg_count - 7);
    } else {
...
#error U32 mach traps on ARM64 kernel requires munging
    ...
    bcopy((char *)saved_state64(state), (char *)dest,
          trapp->mach_trap_arg_count * sizeof(uint64_t));
  }
...
void mach_syscall(struct arm_saved_state *state) {
  ...
  DEBUG_KPRINT_SYSCALL_MACH("mach_syscall: code=%d(%s) (pid %d, tid %lld)\n",
  ...
  if (mach_call == (mach_call_t)kern_invalid) {
    DEBUG_KPRINT_SYSCALL_MACH("mach_syscall: kern_invalid 0x%x\n", call_number);
    goto bad;
  }
  ...
  if (argc) {
    retval =
        arm_get_mach_syscall_args(state, &args, &mach_trap_table[call_number]);
    if (retval != KERN_SUCCESS) {
      arm_set_mach_syscall_ret(state, retval);

      DEBUG_KPRINT_SYSCALL_MACH("mach_syscall: retval=0x%x\n", retval);
      return;
    }
  }
