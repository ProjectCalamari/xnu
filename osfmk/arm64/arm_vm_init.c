 ...
 */
static_assert(KERNEL_PMAP_HEAP_RANGE_START >
              ARM_KERNEL_PROTECT_EXCEPTION_START);

/* The exception vectors and the kernel cannot share root TTEs. */
static_assert((KERNEL_PMAP_HEAP_RANGE_START & ~ARM_TT_ROOT_OFFMASK) >
              ARM_KERNEL_PROTECT_EXCEPTION_START);
 ...
 */
SECURITY_READ_ONLY_LATE(uint8_t)
bootstrap_pagetables[BOOTSTRAP_TABLE_SIZE]
...
SECURITY_READ_ONLY_LATE(static vm_offset_t)
auxkc_mh, auxkc_base, auxkc_right_above;
...
/* "physical to kernel virtual" - given a physical address, return the
 * corresponding physical aperture address */
vm_map_address_t phystokv(pmap_paddr_t pa) {

  for (size_t i = 0; (i < PTOV_TABLE_SIZE) && (ptov_table[i].len != 0); i++) {
    if ((pa >= ptov_table[i].pa) &&
        (pa < (ptov_table[i].pa + ptov_table[i].len))) {
  ...
  if (__improbable((pa < gPhysBase) || ((pa - gPhysBase) >= real_phys_size))) {
    panic("%s: illegal PA: 0x%llx; phys base 0x%llx, size 0x%llx", __func__,
          (unsigned long long)pa, (unsigned long long)gPhysBase,
          (unsigned long long)real_phys_size);
  }
  return pa - gPhysBase + gVirtBase;
}

vm_map_address_t phystokv_range(pmap_paddr_t pa, vm_size_t *max_len) {
...
vm_offset_t ml_static_vtop(vm_offset_t va) {
  for (size_t i = 0; (i < PTOV_TABLE_SIZE) && (ptov_table[i].len != 0); i++) {
    if ((va >= ptov_table[i].va) &&
        (va < (ptov_table[i].va + ptov_table[i].len))) {
      return va - ptov_table[i].va + ptov_table[i].pa;
    }
  }
  if (__improbable((va < gVirtBase) ||
                   (((vm_address_t)(va)-gVirtBase) >= gPhysSize))) {
    panic("%s: illegal VA: %p; virt base 0x%llx, size 0x%llx", __func__,
          (void *)va, (unsigned long long)gVirtBase,
          (unsigned long long)gPhysSize);
  }
 ...
 * hint.
 */
static vm_offset_t round_up_pte_hint_address(vm_offset_t address) {
 ...
 *
 * for static mappings, we allocate from the region ropagetable_begin to
 * ro_pagetable_end-1, which is defined in the DATA_CONST segment and will be
 * protected RNX when vm_prot_finalize runs.
 *
...
void dump_kva_l2(vm_offset_t tt_base, tt_entry_t *tt, int indent,
                 uint64_t *rosz_out, uint64_t *rwsz_out);
...
void dump_kva_l2(vm_offset_t tt_base, tt_entry_t *tt, int indent,
  ...
  boolean_t tt_static =
      kvtophys((vm_offset_t)tt) >= robegin && kvtophys((vm_offset_t)tt) < roend;
  ...
  for (i = 0; i < TTE_PGENTRIES; i++) {
    ...
    if (tt_static) {
      ...
      cur_ro =
          (tte_type == ARM_TTE_TYPE_BLOCK) || (cur >= robegin && cur < roend);
    } else {
      cur_ro = 0;
    }

    if ((cur == 0 && prev != 0) ||
        (cur_ro != prev_ro && prev != 0)) { // falling edge
      ...
      printf("%*s0x%08x_%08x-0x%08x_%08x %s (%luMB)\n", indent * 4, "",
             (uint32_t)(start >> 32), (uint32_t)start, (uint32_t)(end >> 32),
             (uint32_t)end, prev_ro ? "Static " : "Dynamic", (sz >> 20));
    ...
    if ((prev == 0 && cur != 0) ||
        cur_ro != prev_ro) { // rising edge: set start
      start_entry = i;
    }
...
void dump_kva_space() {
  ...
  for (unsigned int i = 0; i < TTE_PGENTRIES; i++) {
    ...
    printf("0x%08x_%08x-0x%08x_%08x %s\n", (uint32_t)(start >> 32),
           (uint32_t)start, (uint32_t)(end >> 32), (uint32_t)end,
  ...
  printf("L2 Address space mapped: Static %lluMB Dynamic %lluMB Total %lluMB\n",
         tot_rosz >> 20, tot_rwsz >> 20, (tot_rosz >> 20) + (tot_rwsz >> 20));

  ro_ptpages =
      (int)((ropage_next - (vm_offset_t)&ropagetable_begin) >> ARM_PGSHIFT);
 ...
 */
static void arm_vm_map(tt_entry_t *root_ttp, vm_offset_t vaddr,
                       pt_entry_t pte) {
  ...
  if (l1_tte == ARM_TTE_EMPTY) {
    ...
    l1_tte |= ARM_TTE_VALID | ARM_TTE_TYPE_TABLE |
              ARM_TTE_TABLE_AP(ARM_TTE_TABLE_AP_USER_NA);
    *l1_ttep = l1_tte;
    ptpage = 0;
  }
 ...
 */
static void arm_vm_kernel_el0_map(vm_offset_t vaddr, pt_entry_t pte) {
 ...
 */
static void arm_vm_kernel_el1_map(vm_offset_t vaddr, pt_entry_t pte) {
 ...
 */
static pt_entry_t arm_vm_kernel_pte(vm_offset_t vaddr) {
 ...
 */
static void arm_vm_prepare_kernel_el0_mappings(bool alloc_only) {
  pt_entry_t pte = 0;
  vm_offset_t start = ((vm_offset_t)&ExceptionVectorsBase) & ~PAGE_MASK;
  vm_offset_t end =
      (((vm_offset_t)&ExceptionVectorsEnd) + PAGE_MASK) & ~PAGE_MASK;
  vm_offset_t cur = 0;
  vm_offset_t cur_fixed = 0;

  /* Expand for/map the exceptions vectors in the EL0 kernel mappings. */
  for (cur = start, cur_fixed = ARM_KERNEL_PROTECT_EXCEPTION_START; cur < end;
       cur += ARM_PGBYTES, cur_fixed += ARM_PGBYTES) {
    /*
 ...
 */
static void arm_vm_populate_kernel_el0_mappings(void) {
 ...
 * mappings.
 */
static void arm_vm_expand_kernel_el0_mappings(void) {
 ...
 * KTRR boundary.
 */
static void arm_replace_identity_map(void) {
  vm_offset_t addr;
  pmap_paddr_t paddr;
...
tt_entry_t *arm_kva_to_tte(vm_offset_t va) {
...
static inline pt_entry_t *arm_kva_to_pte(vm_offset_t va) {
...
/**
 * Updates a translation table entry (TTE) with the supplied value, unless doing
 * so might render the pagetable region read-only before subsequent updates have
 * finished.  In that case, the TTE value will be saved off for deferred
 * processing.
 *
 ...
 * @param ttebase L3-page- or L2-block-aligned base virtual address of the
 * pagetable region
 * @param granule mask indicating whether L2 block or L3 hint mappings are
 * allowed for this segment
 * @param deferred_ttep_pair 2-element array of addresses of deferred TTEs
 * @param deferred_tte_pair 2-element array containing TTE values for deferred
 * assignment to corresponding elements of deferred_ttep_pair
 */
static void update_or_defer_tte(tt_entry_t *ttep, tt_entry_t entry,
                                pmap_paddr_t pa, vm_map_address_t ttebase,
                                unsigned granule __unused,
                                tt_entry_t **deferred_ttep_pair,
                                tt_entry_t *deferred_tte_pair) {
  /*
   * If we're trying to assign an entry that maps the current TTE region
   * (identified by ttebase), and the pagetable is already live (indicated by
   * kva_active), defer assignment of the current entry and possibly the entry
   * after it until all other mappings in the segment have been updated.
   * Otherwise we may end up immediately marking the pagetable region read-only
   * leading to a fault later on a later assignment if we manage to outrun the
   * TLB.  This can happen on KTRR/CTRR-enabled devices when marking
   * segDATACONST read-only, as the pagetables that map that segment must come
   * from the segment itself.  We therefore store the initial recursive TTE in
   * deferred_ttep_pair[0] and its value in deferred_tte_pair[0].  We may also
   * defer assignment of the TTE following that recursive TTE and store its
   * value in deferred_tte_pair[1], because the TTE region following the current
   * one may also contain pagetables and we must avoid marking that region
   * read-only before updating those tables.
   *
   * We require that such recursive mappings must exist in regions that can be
   * mapped with L2 block entries if they are sufficiently large.  This is what
   * allows us to assume that no more than 2 deferred TTEs will be required,
   * because:
   * 	--If more than 2 adjacent L3 PTEs were required to map our pagetables,
   * that would mean we would have at least one full L3 pagetable page and would
   * instead use an L2 block.
   *	--If more than 2 adjacent L2 blocks were required to map our pagetables,
   * that would mean we would have at least one full L2-block-sized region of
   * TTEs and something is very wrong because no segment should be that large.
   */
  if ((deferred_ttep_pair != NULL) && (deferred_ttep_pair[0] != NULL) &&
      (ttep == (deferred_ttep_pair[0] + 1))) {
 ...
 * entry.  Then, it will iterate over the L3 table, updating protections as
 * necessary. This expects to be invoked on a L2 entry or sub L2 entry
 * granularity, so this should not be invoked from a context that does not do L2
 * iteration separately (basically, don't call this except from
 * arm_vm_page_granular_prot).
 *
...
static void arm_vm_page_granular_helper(vm_offset_t start, vm_offset_t _end,
                                        vm_offset_t va, pmap_paddr_t pa_offset,
                                        int pte_prot_APX, int pte_prot_XN,
                                        unsigned granule,
                                        __unused unsigned int guarded,
                                        tt_entry_t **deferred_ttep_pair,
                                        tt_entry_t *deferred_tte_pair) {
  if (va & ARM_TT_L2_OFFMASK) { /* ragged edge hanging over a ARM_TT_L2_SIZE
                                   boundary */
    tt_entry_t *tte2;
    tt_entry_t tmplate;
    pmap_paddr_t pa;
    ...
    for (i = 0; i <= (ARM_TT_L3_INDEX_MASK >> ARM_TT_L3_SHIFT); i++) {
      if ((start <= va) && (va < rounded_end)) {
        ptmp =
            pa | ARM_PTE_AF | ARM_PTE_SH(SH_OUTER_MEMORY) | ARM_PTE_TYPE_VALID;
         ...
         * applicable if the current address falls within a hint-sized range
         * that will be fully covered by this mapping request.
         */
        if ((va >= round_up_pte_hint_address(start)) &&
            (round_up_pte_hint_address(va + 1) <= _end) &&
            (granule & ARM64_GRANULE_ALLOW_HINT) && use_contiguous_hint) {
          assert((va & ((1 << ARM_PTE_HINT_ADDR_SHIFT) - 1)) ==
                 ((pa & ((1 << ARM_PTE_HINT_ADDR_SHIFT) - 1))));
          ptmp |= ARM_PTE_HINT;
          /* Do not attempt to reapply the hint bit to an already-active
           * mapping. This very likely means we're attempting to change
           * attributes on an already-active mapping, which violates the
           * requirement of the hint bit.*/
...
#endif /* BTI_ENFORCED */
        /*
         * Do not change the contiguous bit on an active mapping.  Even in a
         * single-threaded environment, it's possible for prefetch to produce a
         * TLB conflict by trying to pull in a hint-sized entry on top of one or
         * more existing page-sized entries.  It's also useful to make sure
         * we're not trying to unhint a sub-range of a larger hinted range,
         * which could produce a later TLB conflict.
         */
        assert(!kva_active || (ppte[i] == ARM_PTE_EMPTY) ||
               ((ppte[i] & ARM_PTE_HINT) == (ptmp & ARM_PTE_HINT)));

        update_or_defer_tte(&ppte[i], ptmp, pa, (vm_map_address_t)ppte, granule,
                            deferred_ttep_pair, deferred_tte_pair);
      }
...
/*
 * arm_vm_page_granular_prot updates protections by iterating over the L2
 * entries and changing them.  If a particular chunk necessitates L3 entries
 * (for reasons of alignment or length, or an explicit request that the entry be
 * fully expanded), we hand off to arm_vm_page_granular_helper to deal with the
 * L3 chunk of the logic.
 */
static void arm_vm_page_granular_prot(vm_offset_t start, unsigned long size,
  ...
  arm_vm_page_granular_helper(start, align_start, start, pa_offset,
                              pte_prot_APX, pte_prot_XN, granule, guarded,
                              deferred_ttep_pair, deferred_tte_pair);

  while ((_end - align_start) >= ARM_TT_L2_SIZE) {
    if (!(granule & ARM64_GRANULE_ALLOW_BLOCK)) {
      arm_vm_page_granular_helper(align_start, align_start + ARM_TT_L2_SIZE,
                                  align_start + 1, pa_offset, pte_prot_APX,
                                  pte_prot_XN, granule, guarded,
                                  deferred_ttep_pair, deferred_tte_pair);
    } else {
      ...
      if ((pa >= gPhysBase) && (pa < real_avail_end)) {
        ...
        update_or_defer_tte(tte2, tmplate, pa,
                            (vm_map_address_t)tte2 & ~ARM_TT_L2_OFFMASK,
  ...
  if (align_start < _end) {
    arm_vm_page_granular_helper(align_start, _end, _end, pa_offset,
                                pte_prot_APX, pte_prot_XN, granule, guarded,
                                deferred_ttep_pair, deferred_tte_pair);
  }
  ...
  if (deferred_ttep_pair[0] != NULL) {
    ...
    /*
     * Flush the TLB to catch bugs that might cause us to prematurely revoke
     * write access from the pagetable page. These bugs may otherwise be hidden
     * by TLB entries in most cases, resulting in very rare panics. Note that we
     * always flush the TLB at the end of arm_vm_prot_finalize().
     */
    flush_mmu_tlb();
#endif
    /*
     * The first TTE in the pair is a recursive mapping of the pagetable region,
     * so we must update it last to avoid potentially marking
     * deferred_pte_pair[1] read-only.
     */
...
static inline void arm_vm_page_granular_RNX(vm_offset_t start,
                                            unsigned long size,
                                            unsigned granule) {
  arm_vm_page_granular_prot(start, size, 0, 1, AP_RONA, 1, granule,
                            ARM64_PAGE_UNGUARDED);
}

static inline void arm_vm_page_granular_ROX(vm_offset_t start,
                                            unsigned long size,
                                            unsigned granule,
...
static inline void arm_vm_page_granular_RWNX(vm_offset_t start,
                                             unsigned long size,
                                             unsigned granule) {
...
static bool arm_vm_auxkc_init(void) {
  ...
  if (segLOWESTAuxKC != segLOWEST) {
    panic("segLOWESTAuxKC (%p) not equal to segLOWEST (%p). auxkc_mh: %p, "
          "auxkc_base: %p",
          (void *)segLOWESTAuxKC, (void *)segLOWEST, (void *)auxkc_mh,
          (void *)auxkc_base);
  }

  /*
   * The AuxKC LINKEDIT segment needs to be covered by the RO region but is
   * excluded from the RO address range returned by
   * kernel_collection_adjust_mh_addrs(). Ensure the highest non-LINKEDIT
   * address in the AuxKC is the current end of its RO region before extending
   * it.
   */
  ...
  /*
   * The AuxKC RO region must be right below the device tree/trustcache so that
   * it can be covered by CTRR, and the AuxKC RX region must be within the RO
   * region.
   */
  ...
  if (segHIGHESTRXAuxKC < segLOWEST) {
    arm_vm_page_granular_RNX(segHIGHESTRXAuxKC, segLOWEST - segHIGHESTRXAuxKC,
                             0);
  }
  if (segLOWESTRXAuxKC < segHIGHESTRXAuxKC) {
     ...
     * exploit in practice.
     */
    arm_vm_page_granular_ROX(
        segLOWESTRXAuxKC, segHIGHESTRXAuxKC - segLOWESTRXAuxKC, 0,
        ARM64_PAGE_UNGUARDED); // Refined in OSKext::readPrelinkedExtensions
  }
  if (segLOWESTROAuxKC < segLOWESTRXAuxKC) {
    arm_vm_page_granular_RNX(segLOWESTROAuxKC,
                             segLOWESTRXAuxKC - segLOWESTROAuxKC, 0);
  }
  if (segLOWESTAuxKC < segLOWESTROAuxKC) {
    arm_vm_page_granular_RWNX(segLOWESTAuxKC, segLOWESTROAuxKC - segLOWESTAuxKC,
                              0);
  }
...
void arm_vm_prot_init(__unused boot_args *args) {
  ...
  if (segLOWESTKC && segLOWESTKC < segLOWEST) {
    /*
     * kernel collections have segments below the kernel. In particular the
     * collection mach header is below PRELINK_TEXT and is not covered by any
     * other segments already tracked.
     */
    arm_vm_page_granular_RNX(segLOWESTKC, segLOWEST - segLOWESTKC,
  ...
  // Trust Caches portion of EXTRADATA
  {
    ...
    err = SecureDTGetProperty(memory_map, "TrustCache",
                              (void const **)&trustCacheRange,
                              &trustCacheRangeSize);
    if (err == kSuccess) {
      if (trustCacheRangeSize != sizeof(DTMemoryMapRange)) {
        panic(
            "Unexpected /chosen/memory-map/TrustCache property size %u != %zu",
            trustCacheRangeSize, sizeof(DTMemoryMapRange));
      }
      ...
      if (trustCacheRegion < segLOWEST) {
        ...
        panic("TrustCache region is in an unexpected place: %#lx > %#lx",
              trustCacheRegion, segLOWEST);
      }
#endif
  ...
  if (segSizeEXTRADATA != 0) {
    if (segEXTRADATA <= segLOWEST) {
      ...
      panic("EXTRADATA is in an unexpected place: %#lx > %#lx", segEXTRADATA,
            segLOWEST);
    }

    arm_vm_page_granular_RNX(segEXTRADATA, segSizeEXTRADATA,
                             ARM64_GRANULE_ALLOW_BLOCK |
                                 ARM64_GRANULE_ALLOW_HINT);
  }
  ...
  if (auxkc_base < segLOWEST) {
    ...
    panic("auxkc_base (%p) not below segLOWEST (%p)", (void *)auxkc_base,
          (void *)segLOWEST);
  }
...
noAuxKC:
  ...
  arm_vm_page_granular_RWNX(
      segPRELINKTEXTB, segSizePRELINKTEXT,
      ARM64_GRANULE_ALLOW_BLOCK); // Refined in OSKext::readPrelinkedExtensions

  /* Map coalesced kext DATA_CONST segment RWNX (could be empty) */
  arm_vm_page_granular_RWNX(
      segPLKDATACONSTB, segSizePLKDATACONST,
      ARM64_GRANULE_ALLOW_BLOCK); // Refined in OSKext::readPrelinkedExtensions

  /* Map coalesced kext TEXT_EXEC segment RX (could be empty) */
  arm_vm_page_granular_ROX(
      segPLKTEXTEXECB, segSizePLKTEXTEXEC,
      ARM64_GRANULE_ALLOW_BLOCK | ARM64_GRANULE_ALLOW_HINT,
      ARM64_PAGE_GUARDED); // Refined in OSKext::readPrelinkedExtensions

  /* if new segments not present, set space between PRELINK_TEXT and xnu TEXT to
   * RWNX otherwise we no longer expect any space between the coalesced kext
   * read only segments and xnu rosegments
   */
  if (!segSizePLKDATACONST && !segSizePLKTEXTEXEC) {
    if (segSizePRELINKTEXT) {
      arm_vm_page_granular_RWNX(
          segPRELINKTEXTB + segSizePRELINKTEXT,
          segTEXTB - (segPRELINKTEXTB + segSizePRELINKTEXT),
          ARM64_GRANULE_ALLOW_BLOCK | ARM64_GRANULE_ALLOW_HINT);
    }
  } else {
    /*
    ...
    if ((segPLKDATACONSTB + segSizePLKDATACONST) < segTEXTB) {
      arm_vm_page_granular_RWNX(
          segPLKDATACONSTB + segSizePLKDATACONST,
          segTEXTB - (segPLKDATACONSTB + segSizePLKDATACONST),
          ARM64_GRANULE_ALLOW_BLOCK | ARM64_GRANULE_ALLOW_HINT);
    }
  }

  /*
   * Protection on kernel text is loose here to allow shenanigans early on.
   * These protections are tightened in arm_vm_prot_finalize().  This is
   * necessary because we currently patch LowResetVectorBase in cpu.c.
   *
   * TEXT segment contains mach headers and other non-executable data. This will
   * become RONX later.
   */
  arm_vm_page_granular_RNX(segTEXTB, segSizeTEXT,
                           ARM64_GRANULE_ALLOW_BLOCK |
                               ARM64_GRANULE_ALLOW_HINT);
   ...
   */
  arm_vm_page_granular_RWNX(segDATACONSTB, segSizeDATACONST,
                            ARM64_GRANULE_ALLOW_BLOCK);

  arm_vm_page_granular_ROX(segTEXTEXECB, segSizeTEXTEXEC,
                           ARM64_GRANULE_ALLOW_BLOCK | ARM64_GRANULE_ALLOW_HINT,
                           ARM64_PAGE_GUARDED);

#if XNU_MONITOR
  arm_vm_page_granular_ROX(segPPLTEXTB, segSizePPLTEXT,
                           ARM64_GRANULE_ALLOW_BLOCK | ARM64_GRANULE_ALLOW_HINT,
                           ARM64_PAGE_UNGUARDED);
  arm_vm_page_granular_ROX(segPPLTRAMPB, segSizePPLTRAMP,
                           ARM64_GRANULE_ALLOW_BLOCK | ARM64_GRANULE_ALLOW_HINT,
                           ARM64_PAGE_UNGUARDED);
  arm_vm_page_granular_RNX(segPPLDATACONSTB, segSizePPLDATACONST,
                           ARM64_GRANULE_ALLOW_BLOCK |
                               ARM64_GRANULE_ALLOW_HINT);
#endif

  /* DATA segment will remain RWNX */
  arm_vm_page_granular_RWNX(segDATAB, segSizeDATA,
                            ARM64_GRANULE_ALLOW_BLOCK |
                                ARM64_GRANULE_ALLOW_HINT);
#if XNU_MONITOR
  arm_vm_page_granular_RWNX(segPPLDATAB, segSizePPLDATA,
                            ARM64_GRANULE_ALLOW_BLOCK |
                                ARM64_GRANULE_ALLOW_HINT);
#endif

  arm_vm_page_granular_RWNX(segHIBDATAB, segSizeHIBDATA,
                            ARM64_GRANULE_ALLOW_BLOCK |
                                ARM64_GRANULE_ALLOW_HINT);
  ...
  arm_vm_page_granular_RNX((vm_offset_t)&excepstack_high_guard, PAGE_MAX_SIZE,
                           0);
  ...
  arm_vm_page_granular_RWNX(segPLKLINKEDITB, segSizePLKLINKEDIT,
                            0); // Coalesced kext LINKEDIT segment
  arm_vm_page_granular_ROX(
      segLASTB, segSizeLAST, ARM64_GRANULE_ALLOW_BLOCK,
      ARM64_PAGE_GUARDED); // __LAST may be empty, but we cannot assume this
  if (segLASTDATACONSTB) {
    arm_vm_page_granular_RWNX(
        segLASTDATACONSTB, segSizeLASTDATACONST,
        ARM64_GRANULE_ALLOW_BLOCK); // __LASTDATA_CONST may be empty, but we
                                    // cannot assume this
  }
  arm_vm_page_granular_RWNX(
      segPRELINKDATAB, segSizePRELINKDATA,
      ARM64_GRANULE_ALLOW_BLOCK |
          ARM64_GRANULE_ALLOW_HINT); // Prelink __DATA for kexts (RW data)

  if (segSizePLKLLVMCOV > 0) {
    arm_vm_page_granular_RWNX(
        segPLKLLVMCOVB, segSizePLKLLVMCOV,
        ARM64_GRANULE_ALLOW_BLOCK |
            ARM64_GRANULE_ALLOW_HINT); // LLVM code coverage data
  }
  arm_vm_page_granular_RWNX(segPRELINKINFOB, segSizePRELINKINFO,
                            0); /* PreLinkInfoDictionary */
...
static int cmp_ptov_entries(const void *a, const void *b) {
...
#define ROUND_TWIG(addr)                                                       \
  (((addr) + ARM_TT_TWIG_OFFMASK) & ~(ARM_TT_TWIG_OFFMASK))

static void arm_vm_physmap_slide(ptov_table_entry *temp_ptov_table,
                                 vm_map_address_t orig_va, vm_size_t len,
                                 int pte_prot_APX, unsigned granule) {
  pmap_paddr_t pa_offset;

  if (__improbable(ptov_index >= PTOV_TABLE_SIZE)) {
    panic("%s: PTOV table limit exceeded; segment va = 0x%llx, size = 0x%llx",
          __func__, (unsigned long long)orig_va, (unsigned long long)len);
  }
  ...
  if (ptov_index == 0) {
    ...
    temp_ptov_table[ptov_index].va = temp_ptov_table[ptov_index - 1].va +
                                     temp_ptov_table[ptov_index - 1].len;
  }
  if (granule & ARM64_GRANULE_ALLOW_BLOCK) {
    vm_map_address_t orig_offset =
        temp_ptov_table[ptov_index].pa & ARM_TT_TWIG_OFFMASK;
    vm_map_address_t new_offset =
        temp_ptov_table[ptov_index].va & ARM_TT_TWIG_OFFMASK;
    if (new_offset < orig_offset) {
      temp_ptov_table[ptov_index].va += (orig_offset - new_offset);
    } else if (new_offset > orig_offset) {
      temp_ptov_table[ptov_index].va =
          ROUND_TWIG(temp_ptov_table[ptov_index].va) + orig_offset;
    }
  }
  ...
  arm_vm_page_granular_prot(temp_ptov_table[ptov_index].va,
                            temp_ptov_table[ptov_index].len, pa_offset, 1,
                            pte_prot_APX, 1, granule, ARM64_PAGE_UNGUARDED);
  ++ptov_index;
}
...
static void arm_vm_physmap_init(boot_args *args) {
  ptov_table_entry temp_ptov_table[PTOV_TABLE_SIZE];
  bzero(temp_ptov_table, sizeof(temp_ptov_table));

  // This is memory that will either be handed back to the VM layer via
  // ml_static_mfree(), or will be available for general-purpose use.   Physical
  // aperture mappings for this memory must be at page granularity, so that PPL
  // ownership or cache attribute changes can be reflected in the physical
  // aperture mappings.

  // Slid region between gPhysBase and beginning of protected text
  arm_vm_physmap_slide(temp_ptov_table, gVirtBase, segLOWEST - gVirtBase,
                       AP_RWNA, 0);
...
#endif
  arm_vm_physmap_slide(temp_ptov_table, segKLDDATAB, segSizeKLDDATA, AP_RONA,
                       0);

  // Early-boot data
  arm_vm_physmap_slide(temp_ptov_table, segBOOTDATAB, segSizeBOOTDATA, AP_RONA,
                       0);
  ...
  /* KASAN's dynamic denylist needs to query the LINKEDIT segment at runtime. As
   * such, the kext bootstrap code will not jettison LINKEDIT on kasan kernels,
   * so don't bother to relocate it. */
  keep_linkedit = TRUE;
  ...
  if (!keep_linkedit) {
    ...
    if (segSizePLKLINKEDIT) {
      ...
      arm_vm_physmap_slide(temp_ptov_table, segPLKLINKEDITB, segSizePLKLINKEDIT,
                           AP_RWNA, 0);
    }
  }

  // Prelinked kernel plists
  arm_vm_physmap_slide(temp_ptov_table, segPRELINKINFOB, segSizePRELINKINFO,
                       AP_RWNA, 0);

  // Device tree (if not locked down), ramdisk, boot args
  arm_vm_physmap_slide(
      temp_ptov_table, end_kern,
      (args->topOfKernelData - gPhysBase + gVirtBase) - end_kern, AP_RWNA, 0);
  if (!SecureDTIsLockedDown()) {
    PE_slide_devicetree(temp_ptov_table[ptov_index - 1].va - end_kern);
  }

  // Remainder of physical memory
  arm_vm_physmap_slide(temp_ptov_table,
                       (args->topOfKernelData - gPhysBase + gVirtBase),
                       real_avail_end - args->topOfKernelData, AP_RWNA, 0);

  assert((temp_ptov_table[ptov_index - 1].va +
          temp_ptov_table[ptov_index - 1].len) <= physmap_end);

  // Sort in descending order of segment length.  LUT traversal is linear, so
  // largest (most likely used) segments should be placed earliest in the table
  // to optimize lookup performance.
  qsort(temp_ptov_table, PTOV_TABLE_SIZE, sizeof(temp_ptov_table[0]),
        cmp_ptov_entries);
...
static void arm_vm_physmap_init(boot_args *args) {
  ...
  // Will be handed back to VM layer through ml_static_mfree() in
  ...
  arm_vm_page_granular_RWNX(
      end_kern, phystokv(args->topOfKernelData) - end_kern,
      ARM64_GRANULE_ALLOW_BLOCK |
          ARM64_GRANULE_ALLOW_HINT); /* Device Tree (if not locked down), RAM
                                        Disk (if present), bootArgs */

  arm_vm_physmap_slide(
      temp_ptov_table, (args->topOfKernelData - gPhysBase + gVirtBase),
      real_avail_end - args->topOfKernelData, AP_RWNA,
      ARM64_GRANULE_ALLOW_BLOCK | ARM64_GRANULE_ALLOW_HINT); // rest of physmem
...
void arm_vm_prot_finalize(boot_args *args __unused) {
  ...
  /*
   * if old style PRELINK segment exists, free memory before it, and after it
   * before XNU text otherwise we're dealing with a new style kernel cache, so
   * we should just free the memory before PRELINK_TEXT segment, since the rest
   * of the KEXT read only data segments should be immediately followed by XNU's
   * TEXT segment
   */
  ...
  if (!segSizePLKDATACONST && !segSizePLKTEXTEXEC && segSizePRELINKTEXT) {
    /* If new segments not present, PRELINK_TEXT is not dynamically sized, free
     * DRAM between it and xnu TEXT */
    ml_static_mfree(segPRELINKTEXTB + segSizePRELINKTEXT,
                    segTEXTB - (segPRELINKTEXTB + segSizePRELINKTEXT));
  }

  /* tighten permissions on kext read only data and code */
  arm_vm_page_granular_RNX(segPRELINKTEXTB, segSizePRELINKTEXT,
                           ARM64_GRANULE_ALLOW_BLOCK);
  arm_vm_page_granular_RNX(segPLKDATACONSTB, segSizePLKDATACONST,
                           ARM64_GRANULE_ALLOW_BLOCK);

  cpu_stack_alloc(&BootCpuData);
  arm64_replace_bootstack(&BootCpuData);
  ml_static_mfree(phystokv(segBOOTDATAB - gVirtBase + gPhysBase),
                  segSizeBOOTDATA);
  ...
  arm_vm_populate_kernel_el0_mappings();
    ...
    !defined(KERNEL_INTEGRITY_PV_CTRR)
  /* __KLD,__text is covered by the rorgn */
  for (vm_offset_t va = segKLDB; va < (segKLDB + segSizeKLD);
       va += ARM_PGBYTES) {
    pt_entry_t *pte = arm_kva_to_pte(va);
    *pte = ARM_PTE_EMPTY;
  }
#endif
  for (vm_offset_t va = segKLDDATAB; va < (segKLDDATAB + segSizeKLDDATA);
       va += ARM_PGBYTES) {
    pt_entry_t *pte = arm_kva_to_pte(va);
    *pte = ARM_PTE_EMPTY;
  }
  /* Clear the original stack mappings; these pages should be mapped through
   * ptov_table. */
  for (vm_offset_t va = segBOOTDATAB; va < (segBOOTDATAB + segSizeBOOTDATA);
       va += ARM_PGBYTES) {
    pt_entry_t *pte = arm_kva_to_pte(va);
    *pte = ARM_PTE_EMPTY;
  }
  /* Clear the original PRELINKINFO mapping. This segment should be jettisoned
   * during I/O Kit initialization before we reach this point. */
  for (vm_offset_t va = segPRELINKINFOB;
       va < (segPRELINKINFOB + segSizePRELINKINFO); va += ARM_PGBYTES) {
    pt_entry_t *pte = arm_kva_to_pte(va);
    *pte = ARM_PTE_EMPTY;
  }
  if (!keep_linkedit) {
    for (vm_offset_t va = segLINKB; va < (segLINKB + segSizeLINK);
         va += ARM_PGBYTES) {
      pt_entry_t *pte = arm_kva_to_pte(va);
      *pte = ARM_PTE_EMPTY;
    }
    if (segSizePLKLINKEDIT) {
      for (vm_offset_t va = segPLKLINKEDITB;
           va < (segPLKLINKEDITB + segSizePLKLINKEDIT); va += ARM_PGBYTES) {
        pt_entry_t *pte = arm_kva_to_pte(va);
        *pte = ARM_PTE_EMPTY;
      }
    }
  ...
  if (segLASTDATACONSTB) {
    arm_vm_page_granular_RNX(segLASTDATACONSTB, segSizeLASTDATACONST,
                             ARM64_GRANULE_ALLOW_BLOCK);
  }
  ...
  /*
   * Must wait until all other region permissions are set before locking down
   * DATA_CONST as the kernel static page tables live in DATA_CONST on KTRR
   * enabled systems and will become immutable.
   */
#endif

  arm_vm_page_granular_RNX(segDATACONSTB, segSizeDATACONST,
                           ARM64_GRANULE_ALLOW_BLOCK);
...
/*
 * Initialize and enter blank (invalid) page tables in a L1 translation table
 * for a given VA range.
 *
 * This is a helper function used to build up the initial page tables for the
 * kernel translation table. With KERNEL_INTEGRITY we keep at least the root
 * level of the kernel page table immutable, thus the need to preallocate before
 * machine_lockdown any L1 entries necessary during the entire kernel runtime.
 *
 * For a given VA range, if necessary, allocate new L2 translation tables and
 * install the table entries in the appropriate L1 table indexes. called before
 * the translation table is active
 *
 * parameters:
 *
 ...
 * end: end of VA range
 * static_map: whether to allocate the new translation table page from read only
 * memory table_attrs: attributes of new table entry in addition to VALID and
 * TYPE_TABLE attributes
 *
 */

static void init_ptpages(tt_entry_t *tt, vm_map_address_t start,
                         vm_map_address_t end, bool static_map,
  ...
  while (start < end) {
    if (*l1_tte == ARM_TTE_EMPTY) {
      ...
      *l1_tte = (kvtophys(ptpage_vaddr) & ARM_TTE_TABLE_MASK) |
                ARM_TTE_TYPE_TABLE | ARM_TTE_VALID | table_attrs;
...
void arm_vm_init(uint64_t memory_size, boot_args *args) {
  ...
  physmap_base =
      VM_MIN_KERNEL_ADDRESS - (physmap_l1_entries << ARM_TT_L1_SHIFT);
#else
  physmap_base = phystokv(args->topOfKernelData);
#endif

  // Slide the physical aperture to a random page-aligned location within the
  // slide range
  uint64_t physmap_slide =
      early_random() & ARM64_PHYSMAP_SLIDE_MASK & ~((uint64_t)PAGE_MASK);
...
#else
  static_memory_end =
      physmap_base + mem_size +
      (PTOV_TABLE_SIZE *
       ARM_TT_TWIG_SIZE); // worst possible case for block alignment
  physmap_end =
      physmap_base + real_phys_size + (PTOV_TABLE_SIZE * ARM_TT_TWIG_SIZE);
#endif // ARM_LARGE_MEMORY
#endif
  ...
  size_t l1_entries =
      1 + ((bytes_mapped + ARM_TT_L1_SIZE - 1) / ARM_TT_L1_SIZE);
   ...
   * after bootstrap complete, xnu can warm start with a single 16KB page
   * mapping to trampoline to KVA. this requires only 3 pages to stay resident.
   */
   ...
   */
  init_ptpages(cpu_tte, gVirtBase, dynamic_memory_begin, TRUE,
               ARM_TTE_TABLE_AP(ARM_TTE_TABLE_AP_USER_NA));
   ...
   */
  init_ptpages(cpu_tte, physmap_base, ROUND_L1(physmap_end), TRUE,
   ...
   */
  segPRELINKTEXTB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PRELINK_TEXT", &segSizePRELINKTEXT);
  segPLKDATACONSTB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PLK_DATA_CONST", &segSizePLKDATACONST);
  segPLKTEXTEXECB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PLK_TEXT_EXEC", &segSizePLKTEXTEXEC);
  segTEXTB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header, "__TEXT",
                                               &segSizeTEXT);
  segDATACONSTB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__DATA_CONST", &segSizeDATACONST);
  segTEXTEXECB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__TEXT_EXEC", &segSizeTEXTEXEC);
#if XNU_MONITOR
  segPPLTEXTB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header,
                                                  "__PPLTEXT", &segSizePPLTEXT);
  segPPLTRAMPB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PPLTRAMP", &segSizePPLTRAMP);
  segPPLDATACONSTB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PPLDATA_CONST", &segSizePPLDATACONST);
#endif
  segDATAB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header, "__DATA",
                                               &segSizeDATA);
#if XNU_MONITOR
  segPPLDATAB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header,
                                                  "__PPLDATA", &segSizePPLDATA);
#endif

  segBOOTDATAB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__BOOTDATA", &segSizeBOOTDATA);
  segLINKB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header,
                                               "__LINKEDIT", &segSizeLINK);
  segKLDB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header, "__KLD",
                                              &segSizeKLD);
  segKLDDATAB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header,
                                                  "__KLDDATA", &segSizeKLDDATA);
  segPRELINKDATAB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PRELINK_DATA", &segSizePRELINKDATA);
  segPRELINKINFOB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PRELINK_INFO", &segSizePRELINKINFO);
  segPLKLLVMCOVB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PLK_LLVM_COV", &segSizePLKLLVMCOV);
  segPLKLINKEDITB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__PLK_LINKEDIT", &segSizePLKLINKEDIT);
  segLASTB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header, "__LAST",
                                               &segSizeLAST);
  segLASTDATACONSTB = (vm_offset_t)getsegdatafromheader(
      &_mh_execute_header, "__LASTDATA_CONST", &segSizeLASTDATACONST);

  sectHIBTEXTB = (vm_offset_t)getsectdatafromheader(
      &_mh_execute_header, "__TEXT_EXEC", "__hib_text", &sectSizeHIBTEXT);
  sectHIBDATACONSTB =
      (vm_offset_t)getsectdatafromheader(&_mh_execute_header, "__DATA_CONST",
                                         "__hib_const", &sectSizeHIBDATACONST);
  segHIBDATAB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header,
  ...
  if (kernel_mach_header_is_in_fileset(&_mh_execute_header)) {
    ...
    // fileset has kext PLK_TEXT_EXEC under kernel collection TEXT_EXEC
    // following kernel's LAST
    segKCTEXTEXECB = (vm_offset_t)getsegdatafromheader(kc_mh, "__TEXT_EXEC",
                                                       &segSizeKCTEXTEXEC);
    assert(segPLKTEXTEXECB &&
           !segSizePLKTEXTEXEC); // kernel PLK_TEXT_EXEC must be empty
    ...
    // fileset has kext PLK_DATA_CONST under kernel collection DATA_CONST
    // following kernel's LASTDATA_CONST
    segKCDATACONSTB = (vm_offset_t)getsegdatafromheader(kc_mh, "__DATA_CONST",
                                                        &segSizeKCDATACONST);
    assert(segPLKDATACONSTB &&
           !segSizePLKDATACONST); // kernel PLK_DATA_CONST must be empty
    assert(segLASTDATACONSTB &&
           segSizeLASTDATACONST); // kernel LASTDATA_CONST must be non-empty
    assert(
        segKCDATACONSTB <=
        segLASTDATACONSTB); // KC DATA_CONST must contain kernel LASTDATA_CONST
    assert(segKCDATACONSTB + segSizeKCDATACONST >=
           segLASTDATACONSTB + segSizeLASTDATACONST);
    segPLKDATACONSTB = segLASTDATACONSTB + segSizeLASTDATACONST;
    segSizePLKDATACONST =
        segSizeKCDATACONST - (segPLKDATACONSTB - segKCDATACONSTB);

    // fileset has kext PRELINK_DATA under kernel collection DATA following
    // kernel's empty PRELINK_DATA
    segKCDATAB =
        (vm_offset_t)getsegdatafromheader(kc_mh, "__DATA", &segSizeKCDATA);
    assert(segPRELINKDATAB &&
           !segSizePRELINKDATA); // kernel PRELINK_DATA must be empty
    assert(segKCDATAB <=
           segPRELINKDATAB); // KC DATA must contain kernel PRELINK_DATA
    ...
    // fileset has consolidated PRELINK_TEXT, PRELINK_INFO and LINKEDIT at the
    // kernel collection level
    assert(segPRELINKTEXTB &&
           !segSizePRELINKTEXT); // kernel PRELINK_TEXT must be empty
    segPRELINKTEXTB = (vm_offset_t)getsegdatafromheader(kc_mh, "__PRELINK_TEXT",
                                                        &segSizePRELINKTEXT);
    assert(segPRELINKINFOB &&
           !segSizePRELINKINFO); // kernel PRELINK_INFO must be empty
    segPRELINKINFOB = (vm_offset_t)getsegdatafromheader(kc_mh, "__PRELINK_INFO",
                                                        &segSizePRELINKINFO);
    segLINKB =
        (vm_offset_t)getsegdatafromheader(kc_mh, "__LINKEDIT", &segSizeLINK);
  }

  (void)PE_parse_boot_argn("use_contiguous_hint", &use_contiguous_hint,
  ...
  end_kern = round_page(segHIGHESTKC
                            ? segHIGHESTKC
                            : getlastkerneladdr()); /* Force end to next page */
  ...
  if (!segSizePLKTEXTEXEC && !segSizePLKDATACONST) {
    ...
    vm_prelink_etext = segPRELINKTEXTB + segSizePRELINKTEXT +
                       segSizePLKDATACONST + segSizePLKTEXTEXEC;
  }
  ...
  cpu_l2_tte = ((tt_entry_t *)phystokv(((*cpu_l1_tte) & ARM_TTE_TABLE_MASK))) +
               ((va_l2 & ARM_TT_L2_INDEX_MASK) >> ARM_TT_L2_SHIFT);
  ptpage_vaddr = alloc_ptpage(TRUE);
  *cpu_l2_tte = (kvtophys(ptpage_vaddr) & ARM_TTE_TABLE_MASK) |
                ARM_TTE_TYPE_TABLE | ARM_TTE_VALID | ARM_TTE_TABLE_PXN |
                ARM_TTE_TABLE_XN;
  bzero((void *)ptpage_vaddr, ARM_PGBYTES);
...
#if defined(ARM_LARGE_MEMORY)
  /*
   * dynamic mapped memory outside the VM allocator VA range required to
   * bootstrap VM system don't expect to exceed 64GB, no sense mapping any more
   * space between here and the VM heap range
   */
  init_ptpages(
      cpu_tte, dynamic_memory_begin, ROUND_L1(dynamic_memory_begin), FALSE,
      ARM_DYNAMIC_TABLE_XN | ARM_TTE_TABLE_AP(ARM_TTE_TABLE_AP_USER_NA));
#else
  /*
   * TODO: do these pages really need to come from RO memory?
   * With legacy 3 level table systems we never mapped more than a single L1
   * entry so this may be dead code
   */
  init_ptpages(cpu_tte, dynamic_memory_begin, VM_MAX_KERNEL_ADDRESS, TRUE,
               ARM_DYNAMIC_TABLE_XN |
                   ARM_TTE_TABLE_AP(ARM_TTE_TABLE_AP_USER_NA));
#endif
  ...
  flush_mmu_tlb();
  kva_active = TRUE;
  // global table pointers may need to be different due to physical aperture
  // remapping
  ...
  vm_page_kernelcache_count = (unsigned int)(atop_64(
      arm_vm_kernelcache_phys_end - arm_vm_kernelcache_phys_start));
  ...
  if (kernel_mach_header_is_in_fileset(&_mh_execute_header)) {
    ...
    vm_kernel_etext =
        segTEXTB + segSizeTEXT + segSizeDATACONST + segSizeTEXTEXEC;
  ...
  // reserve a 32MB region without permission overrides to use later for a CTRR
  // unit test
  {
    ...
    cpu_l2_tte =
        ((tt_entry_t *)phystokv(((*cpu_l1_tte) & ARM_TTE_TABLE_MASK))) +
        ((ctrr_test_page & ARM_TT_L2_INDEX_MASK) >> ARM_TT_L2_SHIFT);
    assert((*cpu_l2_tte) == ARM_TTE_EMPTY);
    new_tte = (tt_entry_t *)alloc_ptpage(FALSE);
    bzero(new_tte, ARM_PGBYTES);
    *cpu_l2_tte = (kvtophys((vm_offset_t)new_tte) & ARM_TTE_TABLE_MASK) |
...
#if XNU_MONITOR
  for (vm_offset_t cur = (vm_offset_t)pmap_stacks_start;
       cur < (vm_offset_t)pmap_stacks_end; cur += ARM_PGBYTES) {
    arm_vm_map(cpu_tte, cur, ARM_PTE_EMPTY);
  }
#if HAS_GUARDED_IO_FILTER
  for (vm_offset_t cur = (vm_offset_t)iofilter_stacks_start;
       cur < (vm_offset_t)iofilter_stacks_end; cur += ARM_PGBYTES) {
    arm_vm_map(cpu_tte, cur, ARM_PTE_EMPTY);
  }
#endif
#endif
  ...
  while (va_l1 < va_l1_end) {
    ...
    while (va_l2 < va_l2_end) {
      ...
      *cpu_l2_tte = (pa_to_tte(ptp_phys)) | ARM_TTE_TYPE_TABLE | ARM_TTE_VALID |
                    ARM_DYNAMIC_TABLE_XN;
  ...
  init_ptpages(cpu_tte, VM_MIN_KERNEL_ADDRESS & ~ARM_TT_L1_OFFMASK,
               VM_MAX_KERNEL_ADDRESS, FALSE,
               ARM_DYNAMIC_TABLE_XN |
                   ARM_TTE_TABLE_AP(ARM_TTE_TABLE_AP_USER_NA));
   ...
   * Initialize l3 page table pages :
   *   cover this address range:
   *   ((VM_MAX_KERNEL_ADDRESS & CPUWINDOWS_BASE_MASK) - PE_EARLY_BOOT_VA) to
  ...
  patch_low_glo_static_region(args->topOfKernelData,
 ...
 */
bool kernel_text_contains(vm_offset_t addr) {
