...
static void lt_note_another_blocking_lock_holder() {
  hw_lock_lock(&lt_hw_lock, LCK_GRP_NULL);
  lt_num_holders++;
  lt_max_holders =
      (lt_max_holders < lt_num_holders) ? lt_num_holders : lt_max_holders;
  hw_lock_unlock(&lt_hw_lock);
}

static void lt_note_blocking_lock_release() {
  hw_lock_lock(&lt_hw_lock, LCK_GRP_NULL);
  lt_num_holders--;
  hw_lock_unlock(&lt_hw_lock);
}

static void lt_spin_a_little_bit() {
  uint32_t i;
...
static void lt_sleep_a_little_bit() { delay(100); }

static void lt_grab_mutex() {
  lck_mtx_lock(&lt_mtx);
...
static void lt_grab_mutex_with_try() {
...
static void lt_grab_rw_exclusive() {
...
static void lt_grab_rw_exclusive_with_try() {
...
static void lt_upgrade_downgrade_rw() {
...
static void lt_stress_ticket_lock() {
  ...
  while (lt_counter < lt_target_done_threads) {
    if (mach_absolute_time() > lt_setup_timeout) {
      kprintf(
          "%s>cpu %u noticed that we exceeded setup timeout of %d seconds "
          "during initial setup phase (only %u out of %u threads checked in)",
          __FUNCTION__, cpuid, LOCK_TEST_SETUP_TIMEOUT_SEC, lt_counter,
  ...
  while (lt_counter < 2 * lt_target_done_threads) {
    if (mach_absolute_time() > lt_setup_timeout) {
      kprintf(
          "%s>cpu %u noticed that we exceeded setup timeout of %d seconds "
          "during secondary setup phase (only %u out of %u threads checked in)",
          __FUNCTION__, cpuid, LOCK_TEST_SETUP_TIMEOUT_SEC,
          lt_counter - lt_target_done_threads, lt_target_done_threads);
      return;
    }
  }
...
static void lt_grab_hw_lock_with_try() {
...
static void lt_grab_hw_lock_with_to() {
...
static void lt_grab_spin_lock_with_try() {
...
static void lt_reset() {
  ...
  nanoseconds_to_absolutetime(LOCK_TEST_SETUP_TIMEOUT_SEC * NSEC_PER_SEC,
                              &lt_setup_timeout);
...
static void lt_trylock_hw_lock_with_to() {
  OSMemoryBarrier();
  while (!lt_thread_lock_grabbed) {
    lt_sleep_a_little_bit();
    OSMemoryBarrier();
  }
  lt_thread_lock_success =
      hw_lock_to(&lt_hw_lock, &hw_lock_test_give_up_policy, LCK_GRP_NULL);
  OSMemoryBarrier();
  mp_enable_preemption();
}

static void lt_trylock_spin_try_lock() {
  OSMemoryBarrier();
  while (!lt_thread_lock_grabbed) {
    lt_sleep_a_little_bit();
    OSMemoryBarrier();
...
static void lt_trylock_thread(void *arg, wait_result_t wres __unused) {
...
static void lt_start_trylock_thread(thread_continue_t func) {
  thread_t thread;
  kern_return_t kr;
...
static void lt_wait_for_lock_test_threads() {
  OSMemoryBarrier();
...
static kern_return_t lt_test_trylocks() {
  boolean_t success;
  ...
  if (real_ncpus == 1) {
    mp_enable_preemption(); /* if we re-enable preemption, the other thread can
                               timeout and exit */
  }
  OSIncrementAtomic((volatile SInt32 *)&lt_thread_lock_grabbed);
  lt_wait_for_lock_test_threads();
  T_ASSERT_NULL(lt_thread_lock_success,
  ...
  T_ASSERT_NOTNULL(success,
                   "spin trylock of previously unheld lock should succeed");
  success = lck_spin_try_lock(&lt_lck_spin_t);
  T_ASSERT_NULL(success, "spin trylock attempt of previously held lock (with "
                         "trylock) should fail");
  lck_spin_unlock(&lt_lck_spin_t);
...
static void lt_thread(void *arg, wait_result_t wres __unused) {
  void (*func)(void) = (void (*)(void))arg;
  uint32_t i;
...
static void lt_start_lock_thread(thread_continue_t func) {
  thread_t thread;
  kern_return_t kr;
...
#if __AMP__
static void lt_bound_thread(void *arg, wait_result_t wres __unused) {
  void (*func)(void) = (void (*)(void))arg;
...
static void lt_cluster_bound_thread(void *arg, char cluster_type) {
  void (*func)(void) = (void (*)(void))arg;
...
static void lt_e_thread(void *arg, wait_result_t wres __unused) {
  lt_cluster_bound_thread(arg, 'e');
}

static void lt_p_thread(void *arg, wait_result_t wres __unused) {
  lt_cluster_bound_thread(arg, 'p');
}

static void lt_start_lock_thread_with_bind(thread_continue_t bind_type,
...
static kern_return_t lt_test_locks() {
  ...
  lt_wait_for_lock_test_threads();
  T_EXPECT_EQ_UINT(lt_counter, LOCK_TEST_ITERATIONS * lt_target_done_threads,
                   NULL);
   ...
   *  lt_wait_for_lock_test_threads();
   *  T_EXPECT_EQ_UINT(lt_counter, LOCK_TEST_ITERATIONS *
   * lt_target_done_threads, NULL); T_EXPECT_EQ_UINT(lt_max_holders, 1, NULL);
   */
   ...
   *  lt_start_lock_thread(lt_grab_rw_shared_with_try);
   *  lt_wait_for_lock_test_threads();
   *  T_EXPECT_LE_UINT(lt_counter, LOCK_TEST_ITERATIONS *
   * lt_target_done_threads, NULL); T_EXPECT_LE_UINT(lt_max_holders, 3, NULL);
   */
   ...
   *  lt_wait_for_lock_test_threads();
   *  T_EXPECT_LE_UINT(lt_counter, LOCK_TEST_ITERATIONS *
   * lt_target_done_threads, NULL); T_EXPECT_LE_UINT(lt_max_holders, 2, NULL);
   */
  ...
  uint thread_count = 0;
  for (processor_t processor = processor_list; processor != NULL;
       processor = processor->processor_list) {
    lt_start_lock_thread_with_bind(lt_bound_thread, lt_stress_ticket_lock);
    thread_count++;
  }
  T_EXPECT_GE_UINT(thread_count, lt_target_done_threads,
                   "Spawned enough threads for valid test");
  lt_wait_for_lock_test_threads();
  ...
  for (processor_t processor = processor_list; processor != NULL;
       processor = processor->processor_list) {
    starvation =
        starvation || (lt_stress_local_counters[processor->cpu_id] < 10);
    total_local_count += lt_stress_local_counters[processor->cpu_id];
  }
  if (mach_absolute_time() > lt_setup_timeout) {
    T_FAIL("Stress test setup timed out after %d seconds",
           LOCK_TEST_SETUP_TIMEOUT_SEC);
    ...
    T_PASS(
        "Ticket locks stress test with lck_ticket_lock() (%u total acquires)",
        total_local_count);
  }

  /* AMP ticket locks stress test */
  T_LOG("Running AMP Ticket locks stress test bound to clusters with "
        "lck_ticket_lock()");
  lt_reset();
  lt_target_done_threads = real_ncpus;
  thread_count = 0;
...
#define MT_S_VAL                                                               \
  (0xfffffffffeedbeefULL) /* High bit is 1, so sign-extends as negative */
#define MT_L_VAL                                                               \
  (((uint64_t)MT_INITIAL_VALUE) |                                              \
   (((uint64_t)MT_INITIAL_VALUE) << 32)) /* Two back-to-back */
...
struct munger_test {
  ...
  uint64_t mt_expected[MT_MAX_ARGS];
    ...
    {MT_FUNC(munge_wwwww),
     5,
     5,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL}},
    {MT_FUNC(munge_wwwwww),
     6,
     6,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL}},
    {MT_FUNC(munge_wwwwwww),
     7,
     7,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL}},
    {MT_FUNC(munge_wwwwwwww),
     8,
     8,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL,
      MT_W_VAL}},
    ...
    {MT_FUNC(munge_wwlll),
     8,
     5,
     {MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wwlllll),
     12,
     7,
     {MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wwllllll),
     14,
     8,
     {MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL,
      MT_L_VAL}},
    {MT_FUNC(munge_wlw), 4, 3, {MT_W_VAL, MT_L_VAL, MT_W_VAL}},
    {MT_FUNC(munge_wlwwwll),
     10,
     7,
     {MT_W_VAL, MT_L_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wlwwwllw),
     11,
     8,
     {MT_W_VAL, MT_L_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL,
      MT_W_VAL}},
    {MT_FUNC(munge_wlwwlwlw),
     11,
     8,
     {MT_W_VAL, MT_L_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_W_VAL, MT_L_VAL,
      MT_W_VAL}},
    {MT_FUNC(munge_wll), 5, 3, {MT_W_VAL, MT_L_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wlll), 7, 4, {MT_W_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wllwwll),
     11,
     7,
     {MT_W_VAL, MT_L_VAL, MT_L_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wwwlw),
     6,
     5,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_W_VAL}},
    {MT_FUNC(munge_wwwlww),
     7,
     6,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_W_VAL, MT_W_VAL}},
    {MT_FUNC(munge_wwwlwww),
     8,
     7,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL}},
    {MT_FUNC(munge_wwwl), 5, 4, {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wwwwlw),
     7,
     6,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_W_VAL}},
    {MT_FUNC(munge_wwwwllww),
     10,
     8,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL, MT_W_VAL,
      MT_W_VAL}},
    {MT_FUNC(munge_wwwwl),
     6,
     5,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wwwwwl),
     7,
     6,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wwwwwlww),
     9,
     8,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_W_VAL,
      MT_W_VAL}},
    {MT_FUNC(munge_wwwwwllw),
     10,
     8,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL,
      MT_W_VAL}},
    {MT_FUNC(munge_wwwwwlll),
     11,
     8,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_L_VAL,
      MT_L_VAL}},
    {MT_FUNC(munge_wwwwwwl),
     8,
     7,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wwwwwwlw),
     9,
     8,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL,
      MT_W_VAL}},
    {MT_FUNC(munge_wwwwwwll),
     10,
     8,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL,
      MT_L_VAL}},
    ...
    {MT_FUNC(munge_wwwsw),
     5,
     5,
     {MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_S_VAL, MT_W_VAL}},
    {MT_FUNC(munge_llllll),
     12,
     6,
     {MT_L_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL, MT_L_VAL}},
    ...
    {MT_FUNC(munge_lwwwwwww),
     9,
     8,
     {MT_L_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL,
      MT_W_VAL}},
    {MT_FUNC(munge_wlwwwl),
     8,
     6,
     {MT_W_VAL, MT_L_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL}},
    {MT_FUNC(munge_wwlwwwl),
     9,
     7,
     {MT_W_VAL, MT_W_VAL, MT_L_VAL, MT_W_VAL, MT_W_VAL, MT_W_VAL, MT_L_VAL}}};

#define MT_TEST_COUNT (sizeof(munger_tests) / sizeof(struct munger_test))

static void mt_reset(uint32_t in_words, size_t total_size, uint32_t *data) {
  uint32_t i;
  ...
  for (i = 0; i < MT_TEST_COUNT; i++) {
    ...
    for (j = 0; j < test->mt_nout; j++) {
      if (data[j] != test->mt_expected[j]) {
        T_FAIL("Index %d: expected %llx, got %llx.", j, test->mt_expected[j],
               data[j]);
        pass = 0;
      }
    }
...
kern_return_t arm64_ropjop_test() {
  ...
  if (config_rop_enabled) {
    ...
    uint64_t kva_signed = (uint64_t)ptrauth_sign_unauthenticated(
        (void *)&config_rop_enabled, ptrauth_key_asib, 0);

    /* assert it was signed (changed) */
    T_EXPECT(kva_signed != (uint64_t)&config_rop_enabled, NULL);

    /* authenticate the newly signed KVA */
    uint64_t kva_authed = (uint64_t)ml_auth_ptr_unchecked((void *)kva_signed,
                                                          ptrauth_key_asib, 0);
    ...
    kva_authed = (uint64_t)ml_auth_ptr_unchecked((void *)kva_corrupted,
                                                 ptrauth_key_asib, 0);
...
static void arm64_pan_test_thread(void *arg, wait_result_t __unused wres) {
...
kern_return_t arm64_late_pan_test() {
  thread_t thread;
  kern_return_t kr;
...
// Disable KASAN checking for PAN tests as the fixed commpage address doesn't
// have a shadow mapping
...
arm64_pan_test_pan_enabled_fault_handler(arm_saved_state_t *state) {
  ...
  if ((class == ESR_EC_DABORT_EL1) && (fsc == FSC_PERMISSION_FAULT_L3) &&
    ...
    if (pan_exception_level == 1) {
      ...
      ml_expect_fault_end();
      __builtin_arm_wsr("pan", 1); // turn PAN back on after the nested
                                   // exception cleared it for this context
    }
...
arm64_pan_test_pan_disabled_fault_handler(arm_saved_state_t *state) {
  ...
  if ((class == ESR_EC_DABORT_EL1) && (fsc == FSC_PERMISSION_FAULT_L3) &&
    ...
    ml_expect_fault_begin(arm64_pan_test_pan_enabled_fault_handler,
                          pan_test_addr);
    pan_fault_value = *(volatile char *)pan_test_addr;
    ml_expect_fault_end();
    __builtin_arm_wsr("pan", 1); // turn PAN back on after the nested exception
                                 // cleared it for this context
...
NOKASAN kern_return_t arm64_pan_test() {
  bool values_match = false;
  vm_offset_t priv_addr = 0;

  T_LOG("Testing PAN.");

  T_ASSERT((__builtin_arm_rsr("SCTLR_EL1") & SCTLR_PAN_UNCHANGED) == 0,
           "SCTLR_EL1.SPAN must be cleared");
  ...
  *(volatile char *)priv_addr = 0xAB;
  ...
  ml_expect_fault_begin(arm64_pan_test_pan_enabled_fault_handler,
                        pan_test_addr);
  values_match =
      (*(volatile char *)pan_test_addr == *(volatile char *)priv_addr);
  ml_expect_fault_end();
...
kern_return_t arm64_lock_test() { return lt_test_locks(); }

kern_return_t arm64_munger_test() {
  mt_test_mungers();
  return 0;
}
...
kern_return_t ctrr_test(void) {
  ...
  if (!atomic_compare_exchange_strong_explicit(
          &ctrr_test_in_progress, &expected, true, memory_order_acq_rel,
...
static bool ctrr_test_ro_fault_handler(arm_saved_state_t *state) {
  bool retval = false;
  uint64_t esr = get_saved_state_esr(state);
  esr_exception_class_t class = ESR_EC(esr);
...
static bool ctrr_test_nx_fault_handler(arm_saved_state_t *state) {
  bool retval = false;
  uint64_t esr = get_saved_state_esr(state);
  esr_exception_class_t class = ESR_EC(esr);
  fault_status_t fsc = ISS_IA_FSC(ESR_ISS(esr));
...
NOKASAN kern_return_t ctrr_test_cpu(void) {
  ...
  if (/* DISABLES CODE */ (1)) {
    T_SKIP(
        "Skipping CTRR test because testing under SPTM is not supported yet");
    return KERN_SUCCESS;
  }
#endif
  ...
  // The CTRR read-only region is the physical address range [rorgn_begin,
  // rorgn_end]. rorgn_end will be one byte short of a page boundary.
  if (ctrr_enabled) {
    T_EXPECT(rorgn_begin != 0,
             "Expect rorgn_begin to be set when CTRR enabled");
    T_EXPECT_GE_ULONG(
        rorgn_end, rorgn_begin,
        "Expect rorgn_end to be >= rorgn_begin when CTRR enabled");

    pmap_paddr_t ro_test_pa = kvtophys_nofail(ro_test_va);
    pmap_paddr_t nx_test_pa = kvtophys_nofail(nx_test_va);

    T_EXPECT(rorgn_begin <= ro_test_pa && ro_test_pa <= rorgn_end,
             "Expect ro_test_pa to be inside the CTRR region");
    T_EXPECT((nx_test_pa < rorgn_begin) ^ (nx_test_pa > rorgn_end),
             "Expect nx_test_pa to be outside the CTRR region");
  } else {
    T_EXPECT_EQ_ULONG(rorgn_begin, 0,
                      "Expect rorgn_begin to be unset when CTRR disabled");
    T_EXPECT_EQ_ULONG(rorgn_end, 0,
                      "Expect rorgn_end to be unset when CTRR disabled");
    T_LOG("Skipping region check because CTRR is disabled");
  }

  if (ctrr_enabled) {
    T_LOG("Expect no faults when reading CTRR region to verify correct "
          "programming of CTRR limits");
    for (pmap_paddr_t page_pa = rorgn_begin; page_pa <= rorgn_end;
         page_pa += PAGE_SIZE) {
  ...
  T_EXPECT(ro_pn && nx_pn,
           "Expect ro page number and nx page number to be non zero");

  T_LOG("test virtual page: %p, ctrr_ro_test: %p, ctrr_nx_test: %p, ro_pn: %x, "
        "nx_pn: %x ",
  ...
  T_LOG(
      "Read only region test mapping virtual page %p to CTRR RO page number %d",
      ctrr_test_page, ro_pn);
  kr = pmap_enter(kernel_pmap, ctrr_test_page, ro_pn,
                  VM_PROT_READ | VM_PROT_WRITE, VM_PROT_NONE,
                  VM_WIMG_USE_DEFAULT, FALSE, PMAP_MAPPING_TYPE_INFER);
  ...
  prot = pmap_get_arm64_prot(kernel_pmap, ctrr_test_page);
  T_EXPECT(ARM_PTE_EXTRACT_AP(prot) == AP_RWNA && (prot & ARM_PTE_PNX),
           "Mapping is EL1 RWNX");

  ctrr_test_va = ctrr_test_page + (ro_test_va & PAGE_MASK);
  ctrr_ro_test_ptr = (void *)ctrr_test_va;

  T_LOG("Read only region test writing to %p to provoke data abort",
        ctrr_ro_test_ptr);
  ...
  *ctrr_ro_test_ptr = 1;
  ...
  // data abort handler will set ctrr_exception_esr when ctrr_test_va takes a
  // permission fault

  if (ctrr_enabled) {
    T_EXPECT(ESR_EC(ctrr_exception_esr) == ESR_EC_DABORT_EL1,
             "Data Abort from EL1 expected");
    T_EXPECT(ISS_DA_FSC(ESR_ISS(ctrr_exception_esr)) == FSC_PERMISSION_FAULT_L3,
  ...
  T_LOG("No execute test mapping virtual page %p to CTRR PXN page number %d",
        ctrr_test_page, nx_pn);
  ...
  prot = pmap_get_arm64_prot(kernel_pmap, ctrr_test_page);
  T_EXPECT(ARM_PTE_EXTRACT_AP(prot) == AP_RONA && (~prot & ARM_PTE_PNX),
           "Mapping is EL1 ROX (prot=0x%lx)", prot);

  ctrr_test_va = ctrr_test_page + (nx_test_va & PAGE_MASK);
#if __has_feature(ptrauth_calls)
  ctrr_nx_test_ptr = ptrauth_sign_unauthenticated(
      (void *)ctrr_test_va, ptrauth_key_function_pointer, 0);
#else
  ctrr_nx_test_ptr = (void *)ctrr_test_va;
#endif

  T_LOG("No execute test calling ctrr_nx_test_ptr(): %p to provoke instruction "
        "abort",
        ctrr_nx_test_ptr);
  ...
  if (ctrr_enabled) {
    ...
    // Without FEAT_XNX support on the host side, we cannot test kernel
    // execution outside CTXR regions.
#if !defined(KERNEL_INTEGRITY_PV_CTRR)
    // TODO: ensure execute permission fault at expected level
    T_EXPECT(ESR_EC(ctrr_exception_esr) == ESR_EC_IABORT_EL1,
 ...
 */
__unused static void assert_uniprocessor(void) {
  extern unsigned int real_ncpus;
  unsigned int ncpus = os_atomic_load(&real_ncpus, relaxed);
  T_QUIET;
  T_ASSERT_EQ_UINT(1, ncpus,
                   "arm64 kernel POST tests should run before any secondary "
                   "CPUs are brought up");
}
...
extern bool
arm64_panic_lockdown_test_sp1_invalid_stack_handler(arm_saved_state_t *);
extern panic_lockdown_helper_fcn_t
    arm64_panic_lockdown_test_sp1_exception_in_vector;
extern panic_lockdown_helper_fcn_t
    el1_sp1_synchronous_raise_exception_in_vector;
extern bool
arm64_panic_lockdown_test_sp1_exception_in_vector_handler(arm_saved_state_t *);

#if DEVELOPMENT || DEBUG
extern struct panic_lockdown_initiator_state
    debug_panic_lockdown_initiator_state;
...
static void panic_lockdown_expect_test(const char *treatment,
  ...
  ml_expect_fault_pc_begin(arm64_panic_lockdown_test_exception_handler,
                           fault_pc);
  ...
  if (expect_lockdown == xnu_post_panic_lockdown_did_fire &&
    ...
    T_FAIL("%s + %s FAIL (expected lockdown: %d, did lockdown: %d, caught "
           "exception: %d)\n",
           test->name, treatment, expect_lockdown,
           xnu_post_panic_lockdown_did_fire,
           arm64_panic_lockdown_caught_exception);
  }
  ...
  if (expect_lockdown) {
    T_EXPECT_NE_ULLONG(debug_panic_lockdown_initiator_state.initiator_pc, 0ULL,
                       "Initiator PC set");
  } else {
    T_EXPECT_EQ_ULLONG(debug_panic_lockdown_initiator_state.initiator_pc, 0ULL,
                       "Initiator PC not set");
  }
...
panic_lockdown_expect_fault_raw(const char *label,
  ...
  if (test_success && xnu_post_panic_lockdown_did_fire) {
    ...
    T_FAIL("%s FAIL (test returned: %d, did lockdown: %d)\n", label,
           test_success, xnu_post_panic_lockdown_did_fire);
  }
}
 ...
 */
static uint64_t panic_lockdown_pacia_get_invalid_ptr() {
  char *unsigned_ptr = (char *)0xFFFFFFFFAABBCC00;
  char *signed_ptr = NULL;
  do {
    unsigned_ptr += 4 /* avoid alignment exceptions */;
    signed_ptr =
        ptrauth_sign_unauthenticated(unsigned_ptr, ptrauth_key_asia, 0);
 ...
 * discriminator.
 */
static uint64_t panic_lockdown_pacda_get_invalid_ptr(void) {
  char *unsigned_ptr = (char *)0xFFFFFFFFAABBCC00;
  char *signed_ptr = NULL;
  do {
    unsigned_ptr += 8 /* avoid alignment exceptions */;
    signed_ptr =
        ptrauth_sign_unauthenticated(unsigned_ptr, ptrauth_key_asda, 0);
  } while ((uint64_t)unsigned_ptr == (uint64_t)signed_ptr);
...
kern_return_t arm64_panic_lockdown_test(void) {
  ...
  arm64_panic_lockdown_test_case_s tests[] = {
      ...
      {.name = "arm64_panic_lockdown_test_br_auth_fail",
       ...
       .arg = ia_invalid,
       .expected_ec =
           gARM_FEAT_FPACCOMBINE ? ESR_EC_PAC_FAIL : ESR_EC_IABORT_EL1,
       .expect_lockdown_exceptions_masked = true,
       .expect_lockdown_exceptions_unmasked = true,
       /*
       ...
       .override_expected_fault_pc = ia_invalid},
      {
          ...
          .expected_ec =
              gARM_FEAT_FPACCOMBINE ? ESR_EC_PAC_FAIL : ESR_EC_DABORT_EL1,
          .expect_lockdown_exceptions_masked = true,
          .expect_lockdown_exceptions_unmasked = true,
      },
      {
          ...
          .override_expected_fault_pc =
              (uint64_t)&arm64_panic_lockdown_test_copyio_fault_pc,
      },
#if __ARM_ARCH_8_6__
      {
  ...
  for (size_t i = 0; i < test_count; i++) {
    if (tests[i].precondition && !tests[i].precondition()) {
      T_LOG("%s skipped due to precondition check", tests[i].name);
      continue;
    }

    panic_lockdown_expect_test("Exceptions unmasked", &tests[i],
                               tests[i].expect_lockdown_exceptions_unmasked,
                               /* mask_interrupts */ false);

    panic_lockdown_expect_test("Exceptions masked", &tests[i],
                               tests[i].expect_lockdown_exceptions_masked,
                               /* mask_interrupts */ true);
  }

  panic_lockdown_expect_fault_raw(
 ...
 */
static void _cosprctx_exec(uint64_t raw) {
  asm volatile("ISB SY");
 ...
 */
static void _cfprctx_exec(uint64_t raw) {
  asm volatile("ISB SY");
 ...
 */
static void _cpprctx_exec(uint64_t raw) {
  asm volatile("ISB SY");
 ...
 */
static void _dvprctx_exec(uint64_t raw) {
  asm volatile("ISB SY");
...
static void _specres_do_test_std(void (*impl)(uint64_t raw)) {
  typedef struct {
    union {
      struct {
        uint64_t ASID : 16;
...
static void _rctx_do_test(void) {
...
kern_return_t specres_test(void) {
...
static bool arm64_bti_test_exception_handler(arm_saved_state_t *state) {
  uint64_t esr = get_saved_state_esr(state);
  esr_exception_class_t class = ESR_EC(esr);
...
static void
arm64_bti_test_func_with_shim(uint8_t expect_ok, const char *shim_str,
                              bti_shim_func_t *shim,
                              arm64_bti_test_func_case_s *test_case) {
  uint64_t result = -1;

  /* Capture BTI exceptions triggered by our target function */
  uintptr_t raw_func = (uintptr_t)ptrauth_strip((void *)test_case->func,
  ...
  bti_landing_pad_func_t *resigned = ptrauth_auth_and_resign(
      test_case->func, ptrauth_key_function_pointer,
 ...
 */
kern_return_t arm64_backtrace_test(void) {
  ml_expect_fault_pc_begin(arm64_backtrace_test_fault_handler,
                           (uintptr_t)&arm64_brk_lr_fault);
  arm64_brk_lr_gpr();
  ml_expect_fault_end();
