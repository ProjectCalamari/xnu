 ...
 * @discussion  ARM64-specific state preserved pre-hibernation and needed during
 * hibernation resume.
 *
 * @field       dockChannelRegPhysBase  Physical address of the dockchannel
 * registers
 * @field       dockChannelRegVirtBase  Virtual address of the dockchannel
 * registers
 * @field       dockChannelWstatMask    Mask to apply to dockchannel WSTAT
 * register to compute available FIFO entries
 ...
 * @field       hmacRegBase             Physical address of the hmac block
 * registers
 * @field       kernelSlide             Offset from physical address to virtual
 * address in the kernel map
 */
typedef struct {
 ...
 * @discussion  Returns the stack base address and number of pages to use during
 * hibernation resume.
 *
 * @param       first_page      Out parameter: the base address of the
 * hibernation resume stack
 * @param       page_count      Out parameter: the number of pages in the
 * hibernation stack
 */
 ...
 * @discussion  Platform-specific system setup before calling
 ...
 *
 * @field       ttbr0               Physical address of the first level
 * translation table (low mem)
 * @field       ttbr1               Physical address of the first level
 * translation table (high mem)
 * @field       memSlide            Offset from physical address to virtual
 * address during hibernation resume
 */
typedef struct {
  uint64_t ttbr0;
  uint64_t ttbr1;
 ...
 * @discussion  Platform-specific function for handling a trap during
 * hibernation resume.
 *
 * @param       context         The context captured during the trap
 * @param       trap_addr       The address of the low level trap handler that
 * was invoked
 */
void hibernate_trap(arm_context_t *context, uint64_t trap_addr)
