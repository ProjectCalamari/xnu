...
int io_filter_main(uint64_t addr, uint64_t value, uint64_t width,
                   unsigned int io_attr_index,
                   unsigned int io_filter_entry_index);

MARK_AS_PMAP_TEXT
static int io_filter_write(uint64_t addr, uint64_t value, uint64_t width,
                           unsigned int io_attr_index,
                           unsigned int io_filter_entry_index) {
  ...
  if (__improbable(io_attr_table == NULL ||
                   io_attr_index >= (uint64_t)num_io_rgns)) {
    return 0;
  }
  ...
  if (__improbable(io_filter_table == NULL ||
                   io_filter_entry_index >= (uint64_t)num_io_filter_entries)) {
    return 0;
  }

  const pmap_io_range_t *io_range = &io_attr_table[io_attr_index];
  const pmap_io_filter_entry_t *io_filter_entry =
      &io_filter_table[io_filter_entry_index];

  /* Fail if pa is not described by the io_attr_table entry. */
  if (__improbable(pa < io_range->addr ||
                   (pa + width) > (io_range->addr + io_range->len))) {
    return 0;
  }
  ...
  if (__improbable(pa_offset < io_filter_entry->offset ||
                   (pa_offset + pa_length) >
                       (io_filter_entry->offset + io_filter_entry->length))) {
    return 0;
  }
...
int io_filter_main(uint64_t addr, uint64_t value, uint64_t width,
  ...
  return io_filter_write(addr, value, width, io_attr_index,
                         io_filter_entry_index);
}
