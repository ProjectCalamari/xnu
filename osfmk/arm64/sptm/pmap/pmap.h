...
extern void *pmap_auth_user_ptr(void *value, ptrauth_key key, uint64_t data,
                                uint64_t jop_key);
extern bool pmap_batch_sign_user_ptr(void *location, void *value,
                                     ptrauth_key key, uint64_t discriminator,
                                     uint64_t jop_key);
#endif /* HAS_APPLE_PAC */
...
extern void
CleanPoC_DcacheRegion_Force_nopreempt_nohid_nobarrier(vm_offset_t va,
                                                      size_t length);
 ...
 */
#define ARM_KERNEL_PROTECT_EXCEPTION_START                                     \
  ((~((ARM_TT_ROOT_SIZE + ARM_TT_ROOT_INDEX_MASK) / 2ULL)) + 1ULL)
#endif /* __ARM_KERNEL_PROTECT__ */
