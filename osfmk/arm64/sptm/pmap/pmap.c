 ...
 * On the setup side, this structure is used to store parameters for batched
 * SPTM operations. These parameters may be large (upwards of 1K), and given
 * that SPTM calls are generally issued from preemption-disabled contexts
 * anyway, it's better to store them in per-CPU data rather than the local
 * stack. On the post-processing side, this structure exposes a pointer to the
 * SPTM's per-CPU array of 'prev_ptes', that is the prior value encountered in
 * each PTE at the time of the SPTM's atomic update of that PTE.
 */
...
static void flush_mmu_tlb_region_asid_async(vm_offset_t va, size_t length,
                                            pmap_t pmap, bool last_level_only);
...
const struct page_table_level_info pmap_table_level_info_4k_stage2[] = {
    [0] =
        {/* Unused */
         ...
         .type_block = ARM_TTE_TYPE_BLOCK},
    [1] =
        {/* Concatenated, so index mask is larger than normal */
 ...
 * Used by phys_attribute_clear_range to defer flushing pages in this range
 * until the end of the operation, and to accumulate batched operations for
 * submission to the SPTM as a performance optimization.
 */
typedef struct pmap_tlb_flush_range {
  ...
  /**
   * Address of current PTE position in ptfr_pmap's [ptfr_start, ptfr_end)
   * region. This is meant to be set up by the caller of
   * pmap_page_protect_options_with_flush_range() or
   * arm_force_fast_fault_with_flush_range(), and used by those functions to
   * determine when a given mapping can be added to the SPTM's per-CPU region
   * templates array vs. the more complex task of adding it to the disjoint ops
   * array.
   */
  pt_entry_t *current_ptep;

  /**
   * Starting VA for any not-yet-submitted per-CPU region templates.  This is
   * meant to be set up by the caller of
   * pmap_page_protect_options_with_flush_range() or
   * arm_force_fast_fault_with_flush_range() and used by
   * pmap_multipage_op_submit_region() when issuing the SPTM call to purge any
   * pending region ops.
   */
  ...
  /**
   * Indicates whether at least one region entry was added to the per-CPU region
   * ops array since the last time this field was checked.  Intended to be
   * cleared by the caller.
   */
  bool region_entry_added;

  /**
   * Marker for the current paddr "header" entry in the per-CPU SPTM disjoint
   * ops array. This field is intended to be modified only by
   * pmap_multipage_op_submit_disjoint() and pmap_multipage_op_add_page(), and
   * should be treated as opaque by callers of those functions.
   */
  ...
  /**
   * Number of entries in the per-CPU SPTM disjoint ops array, including paddr
   * headers, which have not yet been submitted to the SPTM.
   */
...
#define VREGION1_START                                                         \
  ((VM_MAX_KERNEL_ADDRESS & CPUWINDOWS_BASE_MASK) - VREGION1_HIGH_WINDOW)
...
const uint64_t arm64_root_pgtable_level =
    (3 - ((PGTABLE_ADDR_BITS - 1 - ARM_PGSHIFT) / (ARM_PGSHIFT - TTE_SHIFT)));

/* The number of entries in the root TT of a page table. */
const uint64_t arm64_root_pgtable_num_ttes =
    (2 << ((PGTABLE_ADDR_BITS - 1 - ARM_PGSHIFT) % (ARM_PGSHIFT - TTE_SHIFT)));
...
unsigned int inuse_user_ttepages_count MARK_AS_PMAP_DATA =
    0; /* non-root, non-leaf user pagetable pages, in units of PAGE_SIZE */
unsigned int inuse_user_ptepages_count MARK_AS_PMAP_DATA =
    0; /* leaf user pagetable pages, in units of PAGE_SIZE */
unsigned int inuse_user_tteroot_count MARK_AS_PMAP_DATA =
    0; /* root user pagetables, in units of PMAP_ROOT_ALLOC_SIZE */
unsigned int inuse_kernel_ttepages_count MARK_AS_PMAP_DATA =
    0; /* non-root, non-leaf kernel pagetable pages, in units of PAGE_SIZE */
unsigned int inuse_kernel_ptepages_count MARK_AS_PMAP_DATA =
    0; /* leaf kernel pagetable pages, in units of PAGE_SIZE */
unsigned int inuse_kernel_tteroot_count MARK_AS_PMAP_DATA =
    0; /* root kernel pagetables, in units of PMAP_ROOT_ALLOC_SIZE */
_Atomic unsigned int inuse_iommu_pages_count[SPTM_IOMMUS_N_IDS] = {
    0}; /* number of active pages for each IOMMU class */

SECURITY_READ_ONLY_LATE(tt_entry_t *) invalid_tte = 0;
SECURITY_READ_ONLY_LATE(pmap_paddr_t) invalid_ttep = 0;

SECURITY_READ_ONLY_LATE(tt_entry_t *)
cpu_tte = 0; /* set by arm_vm_init() - keep out of bss */
SECURITY_READ_ONLY_LATE(pmap_paddr_t)
cpu_ttep = 0; /* set by arm_vm_init() - phys tte addr */
...
SECURITY_READ_ONLY_LATE(boolean_t)
pmap_initialized = FALSE; /* Has pmap_init completed? */

SECURITY_READ_ONLY_LATE(vm_map_offset_t) arm_pmap_max_offset_default = 0x0;

/* end of shared region + 512MB for various purposes */
#define ARM64_MIN_MAX_ADDRESS                                                  \
  (SHARED_REGION_BASE_ARM64 + SHARED_REGION_SIZE_ARM64 + 0x20000000)
_Static_assert((ARM64_MIN_MAX_ADDRESS > SHARED_REGION_BASE_ARM64) &&
                   (ARM64_MIN_MAX_ADDRESS <= MACH_VM_MAX_ADDRESS),
...
_Static_assert((ARM64_MAX_OFFSET_DEVICE_LARGE > ARM64_MIN_MAX_ADDRESS) &&
                   (ARM64_MAX_OFFSET_DEVICE_LARGE <= MACH_VM_MAX_ADDRESS),
               "Large device address space size outside allowable range");
_Static_assert((ARM64_MAX_OFFSET_DEVICE_SMALL > ARM64_MIN_MAX_ADDRESS) &&
                   (ARM64_MAX_OFFSET_DEVICE_SMALL <= MACH_VM_MAX_ADDRESS),
               "Small device address space size outside allowable range");

#ifdef XNU_TARGET_OS_OSX
SECURITY_READ_ONLY_LATE(vm_map_offset_t)
arm64_pmap_max_offset_default = MACH_VM_MAX_ADDRESS;
#else
...
static uint64_t
    asid_plru_generation[BITMAP_LEN(MAX_HW_ASIDS)] MARK_AS_PMAP_DATA = {0};
...
/* SPTM TODO: Get rid of this once we export SPTM_PTE_IN_FLIGHT_MARKER from the
 * SPTM. */
#define SPTM_PTE_IN_FLIGHT_MARKER 0x80U
#endif /* SPTM_PTE_IN_FLIGHT_MARKER */

/**
 * Determine whether a PTE has been marked as compressed.  This function also
 * panics if the PTE contains bits that shouldn't be present in a compressed
 * PTE, which is most of them.
 *
 ...
 */
static inline bool pte_is_compressed(pt_entry_t pte, pt_entry_t *ptep) {
  const bool compressed = (!pte_is_valid(pte) && (pte & ARM_PTE_COMPRESSED));
  /**
   * Check for bits that shouldn't be present in a compressed PTE.  This is
   * everything except the compressed/compressed-alt bits, as well as the SPTM's
   * in-flight marker which may be set while the SPTM is in the process of
   * flushing the TLBs after marking a previously-valid PTE as compressed.
   */
  if (__improbable(compressed && (pte & ~(ARM_PTE_COMPRESSED_MASK |
                                          SPTM_PTE_IN_FLIGHT_MARKER)))) {
    panic("compressed PTE %p 0x%llx has extra bits 0x%llx: corrupted?", ptep,
          pte, pte & ~(ARM_PTE_COMPRESSED_MASK | SPTM_PTE_IN_FLIGHT_MARKER));
  }
  return compressed;
}

#define pte_is_wired(pte) (((pte) & ARM_PTE_WIRED_MASK) == ARM_PTE_WIRED)
 ...
 */
static inline void pte_update_wiredcnt(pmap_t pmap, pt_entry_t *pte_p,
                                       boolean_t wired) {
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  unsigned short *ptd_wiredcnt_ptr = &(ptep_get_info(pte_p)->wiredcnt);
  if (wired) {
    if (__improbable(os_atomic_inc_orig(ptd_wiredcnt_ptr, relaxed) ==
                     UINT16_MAX)) {
      panic("pmap %p (pte %p): wired count overflow", pmap, pte_p);
    }
    pmap_ledger_credit(pmap, task_ledgers.wired_mem,
...
/*
 * Synchronize updates to PTEs that were previously invalid or had the AF bit
 * cleared, therefore not requiring TLBI.  Use a store-load barrier to ensure
 * subsequent loads will observe the updated PTE.
 */
#define FLUSH_PTE() __builtin_arm_dmb(DMB_ISH);

/*
 * Synchronize updates to PTEs that were previously valid and thus may be cached
 * in TLBs.  DSB is required to ensure the PTE stores have completed prior to
 * the ensuing TLBI.  This should only require a store-store barrier, as
 * subsequent accesses in program order will not issue until the DSB completes.
 * Prior loads may be reordered after the barrier, but their behavior should not
 * be materially affected by the reordering.  For fault-driven PTE updates such
 * as COW, PTE contents should not matter for loads until the access is
 * re-driven well after the TLB update is synchronized.   For "involuntary" PTE
 * access restriction due to paging lifecycle, we should be in a position to
 * handle access faults.  For "voluntary" PTE access restriction due to
 * unmapping or protection, the decision to restrict access should have a data
 * dependency on prior loads in order to avoid a data race.
 */
#define FLUSH_PTE_STRONG() __builtin_arm_dsb(DSB_ISHST);
 ...
 *            write.
 */
static void write_pte_fast(pt_entry_t *ptep, pt_entry_t pte) {
  /**
   ...
   */
  __unreachable_ok_push if (TEST_PAGE_RATIO_4) {
    if (((uintptr_t)ptep) & 0x1f) {
      panic("%s: PTE write is unaligned, ptep=%p, pte=%p", __func__, ptep,
            (void *)pte);
    }
 ...
 * parameters.
 */
void write_pte(pt_entry_t *ptep, pt_entry_t pte) {
...
static kern_return_t pmap_expand(pmap_t, vm_map_address_t, unsigned int options,
                                 unsigned int level);
...
static kern_return_t pmap_tt_allocate(pmap_t, tt_entry_t **, pt_desc_t **,
...
static void pmap_unmap_commpage(pmap_t pmap);

static boolean_t pmap_is_64bit(pmap_t);
...
static boolean_t arm_clear_fast_fault(ppnum_t ppnum, vm_prot_t fault_type,
                                      uintptr_t pvh, pt_entry_t *pte_p,
                                      pp_attr_t attrs_to_clear);

static void pmap_tte_deallocate(pmap_t pmap, vm_offset_t va_start,
                                tt_entry_t *ttep, unsigned int level,
                                bool pmap_locked);
 ...
 */
kern_return_t pmap_enter(pmap_t pmap, vm_map_address_t v, ppnum_t pn,
                         vm_prot_t prot, vm_prot_t fault_type,
                         unsigned int flags, boolean_t wired,
                         pmap_mapping_type_t mapping_type);

static kern_return_t pmap_enter_addr(pmap_t pmap, vm_map_address_t v,
                                     pmap_paddr_t pa, vm_prot_t prot,
                                     vm_prot_t fault_type, unsigned int flags,
                                     boolean_t wired,
                                     pmap_mapping_type_t mapping_type);

kern_return_t pmap_enter_options_addr(pmap_t pmap, vm_map_address_t v,
                                      pmap_paddr_t pa, vm_prot_t prot,
                                      vm_prot_t fault_type, unsigned int flags,
...
PMAP_SUPPORT_PROTOTYPES(kern_return_t, arm_fast_fault,
                        (pmap_t pmap, vm_map_address_t va, vm_prot_t fault_type,
                         bool was_af_fault, bool from_user),
                        ARM_FAST_FAULT_INDEX);

PMAP_SUPPORT_PROTOTYPES(boolean_t, arm_force_fast_fault,
                        (ppnum_t ppnum, vm_prot_t allow_mode, int options),
                        ARM_FORCE_FAST_FAULT_INDEX);

MARK_AS_PMAP_TEXT static boolean_t arm_force_fast_fault_with_flush_range(
    ppnum_t ppnum, vm_prot_t allow_mode, int options, locked_pvh_t *locked_pvh,
    pp_attr_t bits_to_clear, pmap_tlb_flush_range_t *flush_range);

PMAP_SUPPORT_PROTOTYPES(void, pmap_batch_set_cache_attributes,
                        (const unified_page_list_t *page_list,
                         unsigned int cacheattr, bool update_attr_table),
                        PMAP_BATCH_SET_CACHE_ATTRIBUTES_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_change_wiring,
                        (pmap_t pmap, vm_map_address_t v, boolean_t wired),
                        PMAP_CHANGE_WIRING_INDEX);

PMAP_SUPPORT_PROTOTYPES(pmap_t, pmap_create_options,
                        (ledger_t ledger, vm_map_size_t size,
                         unsigned int flags, kern_return_t *kr),
                        PMAP_CREATE_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_destroy, (pmap_t pmap), PMAP_DESTROY_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_enter_options,
                        (pmap_t pmap, vm_map_address_t v, pmap_paddr_t pa,
                         vm_prot_t prot, vm_prot_t fault_type,
                         unsigned int flags, boolean_t wired,
...
PMAP_SUPPORT_PROTOTYPES(pmap_paddr_t, pmap_find_pa, (pmap_t pmap, addr64_t va),
                        PMAP_FIND_PA_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_insert_commpage, (pmap_t pmap),
                        PMAP_INSERT_COMMPAGE_INDEX);

PMAP_SUPPORT_PROTOTYPES(boolean_t, pmap_is_empty,
                        (pmap_t pmap, vm_map_offset_t va_start,
                         vm_map_offset_t va_end),
                        PMAP_IS_EMPTY_INDEX);

PMAP_SUPPORT_PROTOTYPES(unsigned int, pmap_map_cpu_windows_copy,
                        (ppnum_t pn, vm_prot_t prot, unsigned int wimg_bits),
                        PMAP_MAP_CPU_WINDOWS_COPY_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_ro_zone_memcpy,
                        (zone_id_t zid, vm_offset_t va, vm_offset_t offset,
                         const vm_offset_t new_data, vm_size_t new_data_size),
                        PMAP_RO_ZONE_MEMCPY_INDEX);

PMAP_SUPPORT_PROTOTYPES(uint64_t, pmap_ro_zone_atomic_op,
                        (zone_id_t zid, vm_offset_t va, vm_offset_t offset,
                         zro_atomic_op_t op, uint64_t value),
                        PMAP_RO_ZONE_ATOMIC_OP_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_ro_zone_bzero,
                        (zone_id_t zid, vm_offset_t va, vm_offset_t offset,
                         vm_size_t size),
                        PMAP_RO_ZONE_BZERO_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_nest,
                        (pmap_t grand, pmap_t subord, addr64_t vstart,
                         uint64_t size),
                        PMAP_NEST_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_page_protect_options,
                        (ppnum_t ppnum, vm_prot_t prot, unsigned int options,
                         void *arg),
                        PMAP_PAGE_PROTECT_OPTIONS_INDEX);

PMAP_SUPPORT_PROTOTYPES(vm_map_address_t, pmap_protect_options,
                        (pmap_t pmap, vm_map_address_t start,
                         vm_map_address_t end, vm_prot_t prot,
                         unsigned int options, void *args),
                        PMAP_PROTECT_OPTIONS_INDEX);

PMAP_SUPPORT_PROTOTYPES(kern_return_t, pmap_query_page_info,
                        (pmap_t pmap, vm_map_offset_t va, int *disp_p),
                        PMAP_QUERY_PAGE_INFO_INDEX);

PMAP_SUPPORT_PROTOTYPES(mach_vm_size_t, pmap_query_resident,
                        (pmap_t pmap, vm_map_address_t start,
                         vm_map_address_t end,
                         mach_vm_size_t *compressed_bytes_p),
                        PMAP_QUERY_RESIDENT_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_reference, (pmap_t pmap),
                        PMAP_REFERENCE_INDEX);

PMAP_SUPPORT_PROTOTYPES(vm_map_address_t, pmap_remove_options,
                        (pmap_t pmap, vm_map_address_t start,
                         vm_map_address_t end, int options),
                        PMAP_REMOVE_OPTIONS_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_set_cache_attributes,
                        (ppnum_t pn, unsigned int cacheattr,
                         bool update_attr_table),
                        PMAP_SET_CACHE_ATTRIBUTES_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_update_compressor_page,
                        (ppnum_t pn, unsigned int prev_cacheattr,
                         unsigned int new_cacheattr),
                        PMAP_UPDATE_COMPRESSOR_PAGE_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_set_nested, (pmap_t pmap),
                        PMAP_SET_NESTED_INDEX);

#if MACH_ASSERT
PMAP_SUPPORT_PROTOTYPES(void, pmap_set_process,
                        (pmap_t pmap, int pid, char *procname),
                        PMAP_SET_PROCESS_INDEX);
#endif

PMAP_SUPPORT_PROTOTYPES(void, pmap_unmap_cpu_windows_copy, (unsigned int index),
                        PMAP_UNMAP_CPU_WINDOWS_COPY_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_unnest_options,
                        (pmap_t grand, addr64_t vaddr, uint64_t size,
                         unsigned int option),
                        PMAP_UNNEST_OPTIONS_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, phys_attribute_set,
                        (ppnum_t pn, unsigned int bits),
                        PHYS_ATTRIBUTE_SET_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, phys_attribute_clear,
                        (ppnum_t pn, unsigned int bits, int options, void *arg),
                        PHYS_ATTRIBUTE_CLEAR_INDEX);

#if __ARM_RANGE_TLBI__
PMAP_SUPPORT_PROTOTYPES(vm_map_address_t, phys_attribute_clear_range,
                        (pmap_t pmap, vm_map_address_t start,
                         vm_map_address_t end, unsigned int bits,
                         unsigned int options),
                        PHYS_ATTRIBUTE_CLEAR_RANGE_INDEX);
#endif /* __ARM_RANGE_TLBI__ */

PMAP_SUPPORT_PROTOTYPES(void, pmap_switch, (pmap_t pmap, thread_t thread),
                        PMAP_SWITCH_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_clear_user_ttb, (void),
                        PMAP_CLEAR_USER_TTB_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_set_vm_map_cs_enforced,
                        (pmap_t pmap, bool new_value),
                        PMAP_SET_VM_MAP_CS_ENFORCED_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_set_tpro, (pmap_t pmap),
                        PMAP_SET_TPRO_INDEX);

PMAP_SUPPORT_PROTOTYPES(void, pmap_set_jit_entitled, (pmap_t pmap),
                        PMAP_SET_JIT_ENTITLED_INDEX);

#if __has_feature(ptrauth_calls) &&                                            \
    (defined(XNU_TARGET_OS_OSX) || (DEVELOPMENT || DEBUG))
PMAP_SUPPORT_PROTOTYPES(void, pmap_disable_user_jop, (pmap_t pmap),
                        PMAP_DISABLE_USER_JOP_INDEX);
#endif /* __has_feature(ptrauth_calls) && (defined(XNU_TARGET_OS_OSX) ||       \
          (DEVELOPMENT || DEBUG)) */

PMAP_SUPPORT_PROTOTYPES(void, pmap_trim,
                        (pmap_t grand, pmap_t subord, addr64_t vstart,
                         uint64_t size),
                        PMAP_TRIM_INDEX);

#if HAS_APPLE_PAC
PMAP_SUPPORT_PROTOTYPES(void *, pmap_sign_user_ptr,
                        (void *value, ptrauth_key key, uint64_t discriminator,
                         uint64_t jop_key),
                        PMAP_SIGN_USER_PTR);
PMAP_SUPPORT_PROTOTYPES(void *, pmap_auth_user_ptr,
                        (void *value, ptrauth_key key, uint64_t discriminator,
                         uint64_t jop_key),
                        PMAP_AUTH_USER_PTR);
#endif /* HAS_APPLE_PAC */

void pmap_footprint_suspend(vm_map_t map, boolean_t suspend);
PMAP_SUPPORT_PROTOTYPES(void, pmap_footprint_suspend,
                        (vm_map_t map, boolean_t suspend),
...
static inline void PMAP_ZINFO_PALLOC(pmap_t pmap, int bytes) {
  pmap_ledger_credit(pmap, task_ledgers.tkm_private, bytes);
}

static inline void PMAP_ZINFO_PFREE(pmap_t pmap, int bytes) {
  pmap_ledger_debit(pmap, task_ledgers.tkm_private, bytes);
}

void pmap_tt_ledger_credit(pmap_t pmap, vm_size_t size) {
  if (pmap != kernel_pmap) {
...
void pmap_tt_ledger_debit(pmap_t pmap, vm_size_t size) {
  if (pmap != kernel_pmap) {
...
static inline void pmap_update_plru(uint16_t asid_index __unused) {
#if !HAS_16BIT_ASID
  if (__probable(pmap_asid_plru)) {
    unsigned plru_index = asid_index >> 6;
    if (__improbable(os_atomic_andnot(&asid_plru_bitmap[plru_index],
                                      (1ULL << (asid_index & 63)),
                                      relaxed) == 0)) {
...
static bool alloc_asid(pmap_t pmap) {
  ...
  if (__probable(pmap_asid_plru)) {
    ...
    for (unsigned i = 1;
         i < (sizeof(asid_plru_generation) / sizeof(asid_plru_generation[0]));
         ++i) {
    ...
    for (; plru_index < BITMAP_LEN(pmap_max_asids);
         plru_index += (MAX_HW_ASIDS >> 6)) {
...
#else
  /**
   * For 16-bit ASID targets, we assume a 1:1 correspondence between ASIDs and
   * active tasks and therefore allocate directly from the ASID bitmap instead
   * of using the pLRU allocator. However, we first try to allocate starting
   * from the position of the most-recently allocated ASID.  This is done both
   * as an allocator performance optimization (as it avoids crowding the lower
   * bit positions and then re-checking those same lower positions every time we
   * allocate an ASID) as well as a security mitigation to increase the temporal
   * distance between ASID reuse.  This increases the difficulty of leveraging
   * ASID reuse to train branch predictor logic, without requiring prohibitively
   * expensive RCTX instructions.
   */
  vasid = bitmap_lsb_next(&asid_bitmap[0], pmap_max_asids, last_allocated_asid);
#endif /* !HAS_16BIT_ASID */
  if (__improbable(vasid < 0)) {
    // bitmap_first() returns highest-order bits first, but a 0-based scheme
    // works slightly better with the collision detection scheme used by
...
static void free_asid(pmap_t pmap) {
  ...
  if (pmap_asid_plru) {
    ...
    os_atomic_or(&asid_plru_bitmap[hw_asid >> 6], (1ULL << (hw_asid & 63)),
                 relaxed);
  }
#endif /* !HAS_16BIT_ASID */
  pmap_simple_lock(&asid_lock);
...
boolean_t pmap_valid_address(pmap_paddr_t addr) { return pa_valid(addr); }
 ...
 *      specified memory.
 */
vm_map_address_t pmap_map(vm_map_address_t virt, vm_offset_t start,
  ...
  ps = PAGE_SIZE;
  while (start < end) {
    kr = pmap_enter(kernel_pmap, virt, (ppnum_t)atop(start), prot, VM_PROT_NONE,
                    flags, FALSE, PMAP_MAPPING_TYPE_INFER);

    if (kr != KERN_SUCCESS) {
      panic("%s: failed pmap_enter, "
            "virt=%p, start_addr=%p, end_addr=%p, prot=%#x, flags=%#x",
            __FUNCTION__, (void *)virt, (void *)start, (void *)end, prot,
            flags);
    }
...
/**
 * Force the permission of a PTE to be kernel RO if a page has XNU_PROTECTED_IO
 * type.
 *
...
pmap_force_pte_kernel_ro_if_protected_io(pmap_paddr_t paddr,
                                         pt_entry_t tmplate) {
#if HAS_SPTM_SYSCTL
  ...
  if (frame_type == XNU_PROTECTED_IO) {
    ...
    case XPRR_KERN_RW_PERM:
      ...
      break;
    default:
      panic("%s: Unsupported xPRR perm %llu for pte 0x%llx", __func__,
            xprr_perm, (uint64_t)tmplate);
    }
  }
...
vm_map_address_t pmap_map_bd_with_options(vm_map_address_t virt,
                                          vm_offset_t start, vm_offset_t end,
                                          vm_prot_t prot, int32_t options) {
  pt_entry_t tmplate;
  ...
  for (vm_offset_t pa = start + PAGE_SIZE; pa < end; pa += PAGE_SIZE) {
    if (first_page_is_dram != is_dram_addr(pa)) {
      panic("%s: range crosses DRAM boundary. First inconsistent page 0x%lx %s "
            "DRAM",
  ...
  tmplate = ARM_PTE_AP((prot & VM_PROT_WRITE) ? AP_RWNA : AP_RONA) | mem_attr |
            ARM_PTE_TYPE_VALID | ARM_PTE_NX | ARM_PTE_PNX | ARM_PTE_AF;
  ...
  paddr = start;
  while (paddr < end) {
    __assert_only sptm_return_t ret =
        sptm_map_page(kernel_pmap->ttep, vaddr,
                      pmap_force_pte_kernel_ro_if_protected_io(paddr, tmplate) |
                          pa_to_pte(paddr));
 ...
 */
vm_map_address_t pmap_map_bd(vm_map_address_t virt, vm_offset_t start,
...
vm_map_address_t pmap_map_high_window_bd(vm_offset_t pa_start, vm_size_t len,
                                         vm_prot_t prot) {
  pt_entry_t *ptep, pte;
  ...
  if (len > (va_max - va_start)) {
    panic("%s: area too large, "
          ...
          __FUNCTION__, (void *)pa_start, (void *)len, prot);
  }
...
scan:
  ...
  for (va_end = va_start + PAGE_SIZE; va_end < va_start + len;
       va_end += PAGE_SIZE) {
  ...
  for (va = va_start; va < va_end; va += PAGE_SIZE, pa_start += PAGE_SIZE) {
    ...
    pte = pa_to_pte(pa_start) | ARM_PTE_TYPE_VALID | ARM_PTE_AF | ARM_PTE_NX |
          ARM_PTE_PNX | ARM_PTE_AP((prot & VM_PROT_WRITE) ? AP_RWNA : AP_RONA) |
...
uint64_t pmap_get_arm64_prot(pmap_t pmap, vm_offset_t addr) {
  tt_entry_t tte = 0;
  ...
  for (level = pt_attr->pta_root_level; level <= pt_attr->pta_max_level;
       level++) {
    ...
    if ((level == pt_attr->pta_max_level) || tte_is_block(tte)) {
      ...
      /* All of the table bits we care about are overrides, so just OR them
       * together. */
      aggregate_tte |= tte;
    }
  }
...
void pmap_bootstrap(vm_offset_t vstart) {
  vm_map_offset_t maxoffset;

  lck_grp_init(&pmap_lck_grp, "pmap", LCK_GRP_ATTR_NULL);

#if DEVELOPMENT || DEBUG
  if (PE_parse_boot_argn("pmap_trace", &pmap_trace_mask,
  ...
  const vm_size_t asid_table_size =
      sizeof(*asid_bitmap) * BITMAP_LEN(pmap_max_asids);
  ...
  memset((char *)phystokv(pmap_struct_start), 0,
         avail_start - pmap_struct_start);
...
#if !HAS_16BIT_ASID
  /**
   * Align the range of available hardware ASIDs to a multiple of 64 to enable
   * the masking used by the PLRU scheme.  This means we must handle the case in
   * which the returned hardware ASID is 0, which we do by clearing all vASIDs
   * that will alias the kernel ASID.
   */
  pmap_max_asids = pmap_max_asids & ~63ul;
  if (__improbable(pmap_max_asids == 0)) {
    panic("%s: insufficient number of ASIDs (%u) supplied by SPTM", __func__,
          (unsigned int)pmap_max_asids);
  }
  pmap_asid_plru = (pmap_max_asids > MAX_HW_ASIDS);
  PE_parse_boot_argn("pmap_asid_plru", &pmap_asid_plru, sizeof(pmap_asid_plru));
  _Static_assert(sizeof(asid_plru_bitmap[0] == sizeof(uint64_t)),
                 "bitmap_t is not a 64-bit integer");
  _Static_assert((MAX_HW_ASIDS % 64) == 0,
                 "MAX_HW_ASIDS is not divisible by 64");
  ...
  PE_parse_boot_argn("pmap_panic_dev_wimg_on_managed",
 ...
 * dedicated table hierarchy for the commpage.  We forcibly nest the translation
 * tables from this pmap into other pmaps.  The level we will nest at depends on
 * the MMU configuration (page size, TTBR range, etc). Typically, this is at L1
 * for 4K tasks and L2 for 16K tasks.
 *
 * @note that this is NOT "the nested pmap" (which is used to nest the shared
 * cache).
 *
 * @param rw_va Virtual address at which to insert a mapping to the kernel R/W
 * commpage
 * @param ro_va Virtual address at which to insert a mapping to the kernel R/O
 * commpage
 * @param rw_pa Physical address of kernel R/W commpage
 * @param ro_pa Physical address of kernel R/O commpage, may be 0 if not
 * supported in this configuration
 * @param rx_pa Physical address of user executable (and kernel R/O) commpage,
 * may be 0 if not supported in this configuration
 * @param pmap_create_flags Control flags for the temporary pmap created by this
 * function
 *
 * @return the physical address of the created commpage table, typed as
 *         XNU_PAGE_TABLE_COMMPAGE and containing all relevant commpage
 * mappings.
 */
static pmap_paddr_t
pmap_create_commpage_table(vm_map_address_t rw_va, vm_map_address_t ro_va,
                           pmap_paddr_t rw_pa, pmap_paddr_t ro_pa,
                           pmap_paddr_t rx_pa, unsigned int pmap_create_flags) {
  ...
  /*
   * We only use pmap_expand to expand the pmap up to the commpage nesting
   * level.  At that level and beyond, all the newly created tables will be
   * nested directly into the userspace region for each process, and as such
   * they must be of the dedicated SPTM commpage table type so that the SPTM can
   * enforce the commpage security model which forbids random replacement of
   * commpage mappings.
   */
  kern_return_t kr = pmap_expand(temp_commpage_pmap, rw_va, 0,
                                 pt_attr_commpage_level(pt_attr));
  assert(kr == KERN_SUCCESS);

  pmap_paddr_t commpage_table_pa = 0;
  for (unsigned int i = pt_attr_commpage_level(pt_attr);
       i < pt_attr_leaf_level(pt_attr); i++) {
    ...
    ptd_info_init(ptdp, temp_commpage_pmap, pt_attr_align_va(pt_attr, i, rw_va),
                  i + 1, NULL);
    ...
    const sptm_tte_t table_tte =
        (new_table & ARM_TTE_TABLE_MASK) | ARM_TTE_TYPE_TABLE | ARM_TTE_VALID;

    sptm_map_table(temp_commpage_pmap->ttep,
                   pt_attr_align_va(pt_attr, i, rw_va), (sptm_pt_level_t)i,
                   table_tte);

    ptd_info_finalize(ptdp);

    /* The PTD's assoicated pmap temp_commpage_pmap is to be destroyed, so set
     * it to NULL here. */
    ptdp->pmap = NULL;
  ...
  /*
   * Note the lack of ARM_PTE_NG here: commpage mappings are at fixed addresses
   * and frequently accessed, so we map them global to avoid unnecessary TLB
   * pressure.
   */
  static const sptm_pte_t commpage_pte_template =
      ARM_PTE_TYPE_VALID | ARM_PTE_ATTRINDX(CACHE_ATTRINDX_WRITEBACK) |
      ARM_PTE_SH(SH_INNER_MEMORY) | ARM_PTE_PNX | ARM_PTE_AP(AP_RORO) |
      ARM_PTE_AF;

  sptm_return_t sptm_ret =
      sptm_map_page(temp_commpage_pmap->ttep, rw_va,
                    commpage_pte_template | ARM_PTE_NX | pa_to_pte(rw_pa));
  assert(sptm_ret == SPTM_SUCCESS);

  if (ro_pa != 0) {
    assert((ro_va & ~pt_attr_twig_offmask(pt_attr)) ==
           (rw_va & ~pt_attr_twig_offmask(pt_attr)));
    sptm_ret =
        sptm_map_page(temp_commpage_pmap->ttep, ro_va,
                      commpage_pte_template | ARM_PTE_NX | pa_to_pte(ro_pa));
    assert(sptm_ret == SPTM_SUCCESS);
  }

  if (rx_pa != 0) {
    assert((commpage_text_user_va & ~pt_attr_twig_offmask(pt_attr)) ==
           (rw_va & ~pt_attr_twig_offmask(pt_attr)));
    assert((commpage_text_user_va != rw_va) &&
           (commpage_text_user_va != ro_va));
    sptm_ret = sptm_map_page(temp_commpage_pmap->ttep, commpage_text_user_va,
                             commpage_pte_template | pa_to_pte(rx_pa));
    assert(sptm_ret == SPTM_SUCCESS);
  }

  /* Unmap the commpage table here so that it won't be deallocated by
   * pmap_destroy(). */
  sptm_unmap_table(
      temp_commpage_pmap->ttep,
      pt_attr_align_va(pt_attr, pt_attr_commpage_level(pt_attr), rw_va),
...
/**
 * Helper for creating all commpage tables applicable to the current
 * configuration.
 *
 * @note This function is intended to be called during bootstrap.
 * @note This function assumes that pmap_create_commpages has already executed,
 * and therefore the commpage_*_pa variables have been assigned to their final
 * values.  commpage_data_pa is the kernel RW commpage and is assumed to be
 * present on all configurations, so it therefore must be non-zero at this
 * point.  The other variables are considered optional depending upon
 * configuration and may be zero.
 */
...
void pmap_prepare_commpages(void) {
  ...
  if (commpage_ro_data_pa != 0) {
    sptm_retype(commpage_ro_data_pa, XNU_DEFAULT, XNU_COMMPAGE_RO,
                retype_params);
  }
   ...
   *
   * We don't insert the text commpage into the 32 bit mapping because we don't
   * want 32-bit user processes to get this page mapped in, they should never
   * call into this page.
   */
  commpage_default_table = pmap_create_commpage_table(
      _COMM_PAGE64_BASE_ADDRESS, _COMM_PAGE64_RO_ADDRESS, commpage_data_pa,
      commpage_ro_data_pa, commpage_text_pa, PMAP_CREATE_64BIT);

  /*
   * SPTM TODO: Enable this, along with the appropriate 32-bit commpage address
   * checks and flushes in the SPTM, if we ever need to support arm64_32
   * processes in the SPTM.
   */
  commpage32_default_table = pmap_create_commpage_table(
      _COMM_PAGE32_BASE_ADDRESS, _COMM_PAGE32_RO_ADDRESS, commpage_data_pa,
      commpage_ro_data_pa, 0, 0);

#if __ARM_MIXED_PAGE_SIZE__
  commpage_4k_table = pmap_create_commpage_table(
      _COMM_PAGE64_BASE_ADDRESS, _COMM_PAGE64_RO_ADDRESS, commpage_data_pa,
      commpage_ro_data_pa, 0, PMAP_CREATE_64BIT | PMAP_CREATE_FORCE_4K_PAGES);

  /*
   * SPTM TODO: Enable this, along with the appropriate 32-bit commpage address
   * checks and flushes in the SPTM, if we ever need to support arm64_32
   * processes in the SPTM. commpage32_4k_table =
   * pmap_create_commpage_table(_COMM_PAGE32_BASE_ADDRESS,
   * _COMM_PAGE32_RO_ADDRESS, commpage_data_pa, commpage_ro_data_pa, 0,
...
boolean_t pmap_virtual_region(unsigned int region_select,
                              vm_map_offset_t *startp, vm_map_size_t *size) {
  boolean_t ret = FALSE;
#if defined(KERNEL_INTEGRITY_KTRR) || defined(KERNEL_INTEGRITY_CTRR) ||        \
    defined(KERNEL_INTEGRITY_PV_CTRR)
  if (region_select == 0) {
  ...
  if (region_select == 0) {
    /* Round to avoid overlapping with the V=P area; round to at least the L2
     * block size. */
    if (!TEST_PAGE_SIZE_4K) {
      *startp = gVirtBase & 0xFFFFFFFFFE000000;
      *size = ((virtual_space_start - (gVirtBase & 0xFFFFFFFFFE000000)) +
               ~0xFFFFFFFFFE000000) &
              0xFFFFFFFFFE000000;
    } else {
      *startp = gVirtBase & 0xFFFFFFFFFF800000;
      *size = ((virtual_space_start - (gVirtBase & 0xFFFFFFFFFF800000)) +
               ~0xFFFFFFFFFF800000) &
              0xFFFFFFFFFF800000;
    }
    ret = TRUE;
  ...
  /* We need to reserve a range that is at least the size of an L2 block mapping
   * for the low globals */
  if (!TEST_PAGE_SIZE_4K) {
  ...
  if (((gVirtBase & low_global_vr_mask) != LOW_GLOBAL_BASE_ADDRESS) &&
      (region_select == 2)) {
 ...
 */
bool pmap_is_bad_ram(__unused ppnum_t ppn) { return false; }
 ...
 */
static void initialize_ram_ranges(void) {
...
unsigned int pmap_free_pages_span(void) {
  if (need_ram_ranges_init) {
    initialize_ram_ranges();
  }
...
boolean_t pmap_next_page_hi(ppnum_t *pnum, __unused boolean_t might_free) {
  return pmap_next_page(pnum);
}

boolean_t pmap_next_page(ppnum_t *pnum) {
  if (need_ram_ranges_init) {
    initialize_ram_ranges();
  }
 ...
 */
bool pmap_is_page_restricted(ppnum_t pn) {
...
void pmap_init(void) {
   ...
   */
  pmap_zone = zone_create_ext("pmap", sizeof(struct pmap), ZC_ZFREE_CLEARMEM,
                              ZONE_ID_PMAP, NULL);
 ...
 *         kernel-managed.
 */
bool pmap_verify_free(ppnum_t ppnum) {
...
#if __ARM64_PMAP_SUBPAGE_L1__
static inline bool pmap_user_root_size_matches_subpage_l1(vm_size_t root_size) {
  return root_size == 8 * sizeof(tt_entry_t);
}
#endif /* __ARM64_PMAP_SUBPAGE_L1__ */

static vm_size_t pmap_root_alloc_size(pmap_t pmap) {
#pragma unused(pmap)
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  const unsigned int root_level = pt_attr_root_level(pt_attr);
  const uint64_t index = pt_attr_va_valid_mask(pt_attr);
  return ((index >> pt_attr_ln_shift(pt_attr, root_level)) + 1) *
         sizeof(tt_entry_t);
}
 ...
 */
MARK_AS_PMAP_TEXT pmap_t pmap_create_options_internal(ledger_t ledger,
                                                      vm_map_size_t size,
                                                      unsigned int flags,
                                                      kern_return_t *kr) {
  pmap_t p;
  ...
  if (__improbable(pmap_root_size > PAGE_SIZE)) {
    panic("%s: translation tables do not fit into a single physical page %u",
          __FUNCTION__, (unsigned)pmap_root_size);
  }
   ...
   */
  if (!(flags & PMAP_CREATE_NESTED) &&
      pmap_user_root_size_matches_subpage_l1(pmap_root_size)) {
  ...
  PMAP_TRACE(4, PMAP_CODE(PMAP__TTE), VM_KERNEL_ADDRHIDE(p),
             VM_KERNEL_ADDRHIDE(p->min), VM_KERNEL_ADDRHIDE(p->max), p->ttep);
  ...
  /**
   * The SPTM pmap's concurrency model can sometimes allow ledger balances to
   * transiently go negative.  Note that we still check overall ledger balance
   * on pmap destruction.
   */
  ...
  ledger_disable_panic_on_negative(p->ledger,
                                   task_ledgers.alternate_accounting);
  ledger_disable_panic_on_negative(
      p->ledger, task_ledgers.alternate_accounting_compressed);
...
pmap_t pmap_create_options(ledger_t ledger, vm_map_size_t size,
                           unsigned int flags) {
  pmap_t pmap;
  ...
  PMAP_TRACE(1, PMAP_CODE(PMAP__CREATE) | DBG_FUNC_END,
             VM_KERNEL_ADDRHIDE(pmap), PMAP_VASID(pmap), PMAP_HWASID(pmap));

  return pmap;
}

#if MACH_ASSERT
MARK_AS_PMAP_TEXT void pmap_set_process_internal(__unused pmap_t pmap,
                                                 __unused int pid,
...
#if MACH_ASSERT
void pmap_set_process(pmap_t pmap, int pid, char *procname) {
...
pmap_deallocate_all_leaf_tts(pmap_t pmap, tt_entry_t *first_ttep,
                             vm_map_address_t start_va, unsigned level) {
  ...
  for (ttep = first_ttep; ttep <= last_ttep;
       ttep += page_ratio, va += (size * page_ratio)) {
    ...
    for (unsigned i = 0; i < page_ratio; i++) {
      ...
      if (!(tte & ARM_TTE_VALID)) {
        panic("%s: found unexpectedly invalid tte, ttep=%p, tte=%p, "
              ...
              __FUNCTION__, ttep + i, (void *)tte, pmap, first_ttep, level);
      }
      ...
      if (level < pt_attr_twig_level(pt_attr)) {
        ...
        pmap_deallocate_all_leaf_tts(
            pmap, (tt_entry_t *)phystokv((tte)&ARM_TTE_TABLE_MASK),
            va + (size * i), level + 1);
      }
    }
 ...
 *	no valid mappings.
 */
MARK_AS_PMAP_TEXT void pmap_destroy_internal(pmap_t pmap) {
  if (pmap == PMAP_NULL) {
    return;
  }
   ...
   * ensure that we don't unmap and retype the page tables while those
   * operations are still finishing on other CPUs, leading to an SPTM violation.
   * In particular, the multipage batched cacheability/attribute update code may
   * issue SPTM calls without holding the relevant PVH or pmap locks, so we
   * can't guarantee those calls have actually completed despite observing
   * refcnt == 0.
   *
   ...
   *	pmap structure.
   */
  pmap_deallocate_all_leaf_tts(pmap, pmap->tte, pmap->min,
  ...
  if ((pmap->type == PMAP_TYPE_NESTED) &&
      (pmap->nested_region_unnested_table_bitmap != NULL)) {
    bitmap_free(
        pmap->nested_region_unnested_table_bitmap,
...
void pmap_destroy(pmap_t pmap) {
  PMAP_TRACE(1, PMAP_CODE(PMAP__DESTROY) | DBG_FUNC_START,
             VM_KERNEL_ADDRHIDE(pmap), PMAP_VASID(pmap), PMAP_HWASID(pmap));
 ...
 */
MARK_AS_PMAP_TEXT void pmap_reference_internal(pmap_t pmap) {
  if (pmap != PMAP_NULL) {
    validate_pmap_mutable(pmap);
...
void pmap_reference(pmap_t pmap) { pmap_reference_internal(pmap); }
...
static sptm_frame_type_t get_sptm_pt_type(pmap_t pmap) {
  ...
  if (is_stage2_pmap) {
    ...
    return pmap->type == PMAP_TYPE_NESTED ? XNU_PAGE_TABLE_SHARED
                                          : XNU_PAGE_TABLE;
  }
}

static tt_entry_t *pmap_tt1_allocate(pmap_t pmap, vm_size_t size,
                                     uint8_t sptm_root_flags) {
  pmap_paddr_t pa = 0;
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  const bool is_stage2_pmap = false;
  ...
  if (use_surt) {
    ...
    if (pa) {
      sptm_surt_alloc(surt_page_pa_from_surt_pa(pa),
                      surt_index_from_surt_pa(pa), pt_attr->geometry_id,
                      sptm_root_flags, pmap->asid);
...
#endif /* __ARM64_PMAP_SUBPAGE_L1__ */
  {
    ...
    sptm_retype(pa, XNU_DEFAULT,
                is_stage2_pmap ? XNU_STAGE2_ROOT_TABLE : XNU_USER_ROOT_TABLE,
                retype_params);
  }
...
ptt1a_done:
#endif /* __ARM64_PMAP_SUBPAGE_L1__ */
  /* Always report root allocations in units of PMAP_ROOT_ALLOC_SIZE, which can
   * be obtained by sysctl arm_pt_root_size. Depending on the device, this can
   * vary between 512b and 16K. */
  OSAddAtomic((uint32_t)(size / PMAP_ROOT_ALLOC_SIZE),
              (pmap == kernel_pmap ? &inuse_kernel_tteroot_count
...
static void pmap_tt1_deallocate(pmap_t pmap, tt_entry_t *tt, vm_size_t size) {
  ...
  if (use_surt) {
    ...
    sptm_surt_free(surt_page_pa_from_surt_pa(pa), surt_index_from_surt_pa(pa));
...
ptt1d_done:
#endif /* __ARM64_PMAP_SUBPAGE_L1__ */
  OSAddAtomic(-(int32_t)(size / PMAP_ROOT_ALLOC_SIZE),
              (pmap == kernel_pmap ? &inuse_kernel_tteroot_count
                                   : &inuse_user_tteroot_count));
...
pmap_tt_allocate(pmap_t pmap, tt_entry_t **ttp, pt_desc_t **ptdp_out,
                 unsigned int level, unsigned int options) {
  pmap_paddr_t pa;
  ...
  if (level < pt_attr_leaf_level(pmap_get_pt_attr(pmap))) {
    OSAddAtomic(1, (pmap == kernel_pmap ? &inuse_kernel_ttepages_count
                                        : &inuse_user_ttepages_count));
  } else {
    OSAddAtomic(1, (pmap == kernel_pmap ? &inuse_kernel_ptepages_count
  ...
  /**
   * SPTM TODO: To reduce the cost of draining and retyping, consider caching
   * freed page table pages in a small per-CPU bucket and reusing them in
   * preference to calling pmap_page_alloc() above.
   */
  pmap_epoch_drain();
...
static void pmap_tt_deallocate(pmap_t pmap, tt_entry_t *ttp,
                               unsigned int level) {
  pt_desc_t *ptdp;
  ...
  const uint16_t refcnt =
      sptm_get_page_table_refcnt(kvtophys_nofail((vm_offset_t)ttp));
...
/**
 * Check table refcounts after clearing a translation table entry pointing to
 * that table
 *
 ...
 */
static void pmap_tte_check_refcounts(pmap_t pmap, tt_entry_t tte,
                                     unsigned int level) {
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  ...
  /**
   * It's possible that a concurrent pmap_disconnect() operation may need to
   * reference a PTE on the pagetable page to be removed.  A full disconnect()
   * may have cleared one or more PTEs on this page but not yet dropped the
   * refcount, which would cause us to panic in this function on a non-zero
   * refcount.  Moreover, it's possible for a disconnect-to-compress operation
   * to set the compressed marker on a PTE, and for pmap_remove_range_options()
   * to concurrently observe that marker, clear it, and drop the pagetable
   * refcount accordingly, without taking any PVH locks that could synchronize
   * it against the disconnect operation.  If that removal caused the refcount
   * to reach zero, the pagetable page could be freed before the disconnect
   * operation is finished using the relevant pagetable descriptor.
   * Address these cases by draining the epochs to ensure other cores are no
   * longer consuming the page table we're preparing to delete.
   */
  ...
  if (__improbable(remove_leaf_table && refcnt != 0)) {
    ...
    unsigned short wiredcnt =
        ptep_get_info((pt_entry_t *)ttetokv(tte))->wiredcnt;
    ...
    if (__improbable(
            (non_empty != 0) ||
            (remove_leaf_table && ((refcnt != 0) || (wiredcnt != 0))))) {
#else  /* MACH_ASSERT */
    /* We already know the leaf page-table has a non-zero refcnt, so panic. */
    {
#endif /* MACH_ASSERT */
      panic("%s: Found inconsistent state in soon to be deleted L%d table: %d "
            "valid, "
            "%d compressed, %d non-empty, refcnt=%d, wiredcnt=%d, L%d "
            "tte=%#llx, pmap=%p, bpte=%p",
            __func__, level + 1, valid, comp, non_empty, refcnt, wiredcnt,
            level, (uint64_t)tte, pmap, bpte);
    }
  }
}
 ...
 *       of zero.
 *
 * @param pmap The user pmap containing the nested page table whose TTE is being
 * removed.
 * @param va_start Beginning of the VA range mapped by the table being removed,
 * for TLB maintenance.
 * @param ttep Pointer to the TTE that should be cleared out.
 */
static void pmap_tte_trim(pmap_t pmap, vm_offset_t va_start, tt_entry_t *ttep) {
  assert(ttep != NULL);
  ...
  if (__improbable(tte == ARM_TTE_EMPTY)) {
    panic("%s: L%d TTE is already empty. Potential double unmap or memory "
          "stomper? pmap=%p ttep=%p",
          __func__, pt_attr_twig_level(pt_attr), pmap, ttep);
  }

  const uint64_t page_ratio = PAGE_SIZE / pt_attr_page_size(pt_attr);
  sptm_unnest_region(pmap->ttep, pmap->nested_pmap->ttep, va_start,
                     (pt_attr_twig_size(pt_attr) * page_ratio) >>
                         pt_attr->pta_page_shift);
 ...
 *       expects pmap to be locked exclusive, and will return with pmap
 * unlocked.
 *
 * @param pmap The pmap containing the page table whose TTE is being removed.
 * @param va_start Beginning of the VA range mapped by the table being removed,
 ...
 * @param pmap_locked If true, the caller holds an exclusive pmap lock which
 * should be dropped after removing the table entry.
 */
static void pmap_tte_remove(pmap_t pmap, vm_offset_t va_start, tt_entry_t *ttep,
                            unsigned int level, bool pmap_locked) {
  assert(ttep != NULL);
  const tt_entry_t tte = *ttep;
  ...
  if (__improbable(tte == ARM_TTE_EMPTY)) {
    panic("%s: L%d TTE is already empty. Potential double unmap or memory "
          "stomper? pmap=%p ttep=%p",
          __func__, level, pmap, ttep);
  }

  sptm_unmap_table(pmap->ttep,
                   pt_attr_align_va(pmap_get_pt_attr(pmap), level, va_start),
...
static void pmap_tte_deallocate(pmap_t pmap, vm_offset_t va_start,
  ...
  if (tte_get_ptd(tte)->pmap != pmap) {
    panic("%s: Passed in pmap doesn't own the page table to be deleted ptd=%p "
          "ptd->pmap=%p pmap=%p",
 ...
 */
MARK_AS_PMAP_TEXT static void
pmap_remove_range(pmap_t pmap, vm_map_address_t va, vm_map_address_t end) {
  pmap_remove_range_options(pmap, va, end, PMAP_OPTIONS_REMOVE);
}

MARK_AS_PMAP_TEXT void pmap_remove_range_options(pmap_t pmap,
                                                 vm_map_address_t start,
                                                 vm_map_address_t end,
                                                 int options) {
  const unsigned int sptm_flags =
      ((options & PMAP_OPTIONS_REMOVE) ? SPTM_REMOVE_COMPRESSED : 0);
  ...
  /*
   * The pmap lock should be held here.  It will only be held shared in most if
   * not all cases.
   */
  ...
  while (va < end) {
    /**
     * We may need to sleep when taking the PVH lock below, and our
     * pmap_pv_remove() call below may also place the lock in sleep mode if
     * processing a large PV list. We therefore can't leave preemption disabled
     * across that code, which means we can't directly use the per-CPU prev_ptes
     * array in that code.  Since that code only cares about the physical
     * address stored in each prev_ptes entry, we'll use a local array to stash
     * off only the 4-byte physical address index in order to reduce stack
     * usage.
     */
    unsigned int pai_list[SPTM_MAPPING_LIMIT];
    _Static_assert(
        SPTM_MAPPING_LIMIT <= 64,
    ...
    /**
     * Disable preemption to ensure that we can safely access per-CPU mapping
     * data after issuing the SPTM call.
     */
    disable_preemption();
    /**
     * Enter the pmap epoch for the batched unmap operation.  This is necessary
     * because we cannot reasonably hold the PVH locks for all pages mapped by
     * the region during this call, so a concurrent pmap_page_protect()
     * operation against one of those pages may race this call.  That should be
     * perfectly fine as far as the PTE updates are concerned, but if
     * pmap_page_protect() then needs to retype the page, an SPTM violation may
     * result if it does not first drain our epoch.
     */
    pmap_epoch_enter();
    ...
    for (unsigned int i = 0; i < num_mappings; ++i, ++cpte) {
      ...
      pv_remove_return_t remove_status =
          pmap_remove_pv(pmap, cpte, &locked_pvh, &is_internal, &is_altacct);
      ...
      default:
        /*
         * PVE already removed; this can happen due to a concurrent
         * pmap_disconnect() executing before we grabbed the PVH lock.
         */
        break;
      }
  ...
  if (pmap != kernel_pmap) {
    if (num_unwired != 0) {
      ...
      if (__improbable(os_atomic_sub_orig(&ptd_info->wiredcnt, num_unwired,
                                          relaxed) < num_unwired)) {
        panic("%s: pmap %p VA [0x%llx, 0x%llx) (ptd info %p) wired count "
              "underflow",
              __func__, pmap, (unsigned long long)start,
              (unsigned long long)end, ptd_info);
      }
    }

    /* update ledgers */
    pmap_ledger_debit(pmap, task_ledgers.external,
                      (num_external)*pmap_page_size);
    pmap_ledger_debit(pmap, task_ledgers.reusable,
                      (num_reusable)*pmap_page_size);
    pmap_ledger_debit(pmap, task_ledgers.wired_mem,
                      (num_unwired)*pmap_page_size);
    pmap_ledger_debit(pmap, task_ledgers.internal,
                      (num_internal)*pmap_page_size);
    pmap_ledger_debit(pmap, task_ledgers.alternate_accounting,
                      (num_alt_internal)*pmap_page_size);
    pmap_ledger_debit(pmap, task_ledgers.alternate_accounting_compressed,
                      (num_alt_compressed)*pmap_page_size);
    pmap_ledger_debit(pmap, task_ledgers.internal_compressed,
                      (num_compressed)*pmap_page_size);
    /* make needed adjustments to phys_footprint */
    pmap_ledger_debit(pmap, task_ledgers.phys_footprint,
                      ((num_internal - num_alt_internal) +
                       (num_compressed - num_alt_compressed)) *
                          pmap_page_size);
  }
}
 ...
 */
void pmap_remove(pmap_t pmap, vm_map_address_t start, vm_map_address_t end) {
  pmap_remove_options(pmap, start, end, PMAP_OPTIONS_REMOVE);
}

MARK_AS_PMAP_TEXT vm_map_address_t pmap_remove_options_internal(
    pmap_t pmap, vm_map_address_t start, vm_map_address_t end, int options) {
  vm_map_address_t eva = end;
  tt_entry_t *tte_p;
  bool unlock = true;

  if (__improbable(end < start)) {
    panic("%s: invalid address range %p, %p", __func__, (void *)start,
          (void *)end);
  }
  if (__improbable(pmap->type == PMAP_TYPE_COMMPAGE)) {
    panic("%s: attempt to remove mappings from commpage pmap %p", __func__,
          pmap);
  }
  ...
  assertf(tte_is_table(*tte_p),
          "%s: invalid TTE %p (0x%llx) for pmap %p va 0x%llx", __func__, tte_p,
          (unsigned long long)*tte_p, pmap, (unsigned long long)start);
  ...
  if (__improbable(refcnt == 0)) {
    ...
    os_atomic_inc(&ptd_info->wiredcnt,
                  relaxed); // Prevent someone else from freeing the table if we
    ...
    if ((os_atomic_dec(&ptd_info->wiredcnt, relaxed) == 0) && (refcnt == 0)) {
      /**
       * Drain any concurrent retype-sensitive SPTM operations.  This is needed
       * to ensure that we don't unmap the page table and retype it while those
       * operations are still finishing on other CPUs, leading to an SPTM
       * violation.  In particular, the multipage batched cacheability/attribute
       * update code may issue SPTM calls without holding the relevant PVH or
       * pmap locks, so we can't guarantee those calls have actually completed
       * despite observing refcnt == 0.
       *
       * At this point, we CAN guarantee that:
       ...
       *    relevant PV list entries removed.  Subsequent calls not already in
       * the pmap epoch will no longer observe these mappings. 2) We now hold
       * the pmap lock exclusive, so there will be no further attempt to enter
       * mappings in this page table before it is unmapped.
       */
      pmap_epoch_prepare_drain();
      pmap_epoch_drain();
...
void pmap_remove_options(pmap_t pmap, vm_map_address_t start,
  ...
  if ((start | end) & pt_attr_leaf_offmask(pt_attr)) {
    panic("pmap_remove_options() pmap %p start 0x%llx end 0x%llx", pmap,
          (uint64_t)start, (uint64_t)end);
  }
  if ((end < start) || (start < pmap->min) || (end > pmap->max)) {
    panic("pmap_remove_options(): invalid address range, pmap=%p, "
 ...
 */
void pmap_remove_some_phys(__unused pmap_t map, __unused ppnum_t pn) {
 ...
 */
void pmap_switch_user(thread_t thread, vm_map_t new_map) {
  pmap_t new_pmap = new_map->pmap;

  thread->map = new_map;
  pmap_set_pmap(new_pmap, thread);
}
void pmap_set_pmap(pmap_t pmap, thread_t thread) { pmap_switch(pmap, thread); }

MARK_AS_PMAP_TEXT void pmap_switch_internal(pmap_t pmap, thread_t thread) {
  validate_pmap_mutable(pmap);
  __unused const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
...
void pmap_switch(pmap_t pmap, thread_t thread) {
  PMAP_TRACE(1, PMAP_CODE(PMAP__SWITCH) | DBG_FUNC_START,
             VM_KERNEL_ADDRHIDE(pmap), PMAP_VASID(pmap), PMAP_HWASID(pmap));
  pmap_switch_internal(pmap, thread);
  PMAP_TRACE(1, PMAP_CODE(PMAP__SWITCH) | DBG_FUNC_END);
}

void pmap_page_protect(ppnum_t ppnum, vm_prot_t prot) {
  pmap_page_protect_options(ppnum, prot, 0, NULL);
}

/**
 *  Helper function for performing per-mapping accounting following an SPTM
 * disjoint unmap request.
 *
 * @note [pmap] cannot be the kernel pmap. This is because we do not maintain a
 * ledger in the kernel pmap.
 *
 * @param pmap The pmap that contained the mapping
 * @param pai The physical page index mapped by the mapping
 * @param is_compressed Indicates whether the operation was an unmap-to-compress
 * vs. a full unmap
 * @param is_internal Indicates whether the mapping was for an internal (aka
 * anonymous) VM page
 * @param is_altacct Indicates whether the mapping was subject to alternate
 * accounting.
 */
static void pmap_disjoint_unmap_accounting(pmap_t pmap, unsigned int pai,
                                           bool is_compressed, bool is_internal,
                                           bool is_altacct) {
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  ...
  if (is_internal && !is_altacct && ppattr_test_reusable(pai)) {
    pmap_ledger_debit(pmap, task_ledgers.reusable,
                      pt_attr_page_size(pt_attr) * PAGE_RATIO);
  } else if (!is_internal) {
    pmap_ledger_debit(pmap, task_ledgers.external,
  ...
  if (is_altacct) {
    ...
    if (is_compressed) {
      pmap_ledger_credit(pmap, task_ledgers.internal_compressed,
                         pt_attr_page_size(pt_attr) * PAGE_RATIO);
      pmap_ledger_credit(pmap, task_ledgers.alternate_accounting_compressed,
                         pt_attr_page_size(pt_attr) * PAGE_RATIO);
    }
    ...
    if (is_compressed) {
      ...
      pmap_ledger_credit(pmap, task_ledgers.phys_footprint,
                         pt_attr_page_size(pt_attr) * PAGE_RATIO);
    }
  } else if (is_internal) {
    pmap_ledger_debit(pmap, task_ledgers.internal,
    ...
    if (is_compressed) {
      ...
      /*
       * This internal page isn't going to the compressor, so adjust stats to
       * keep phys_footprint up to date.
       */
      pmap_ledger_debit(pmap, task_ledgers.phys_footprint,
                        pt_attr_page_size(pt_attr) * PAGE_RATIO);
    }
...
/**
 * Helper function for issuing a disjoint unmap request to the SPTM and
 * performing related accounting.  This function uses the 'prev_ptes' list
 * generated by the sptm_unmap_disjoint() call to determine whether said call
 * altered the relevant PTEs in a manner that would require accounting updates.
 *
 * @param pa The physical address against which the disjoint unmap will be
 * issued.
 ...
 */
static void pmap_disjoint_unmap(pmap_paddr_t pa, unsigned int num_mappings) {
  const unsigned int pai = pa_index(pa);
  ...
  for (unsigned int cur_mapping = 0; cur_mapping < num_mappings;
       ++cur_mapping) {
    ...
    assertf(!pte_is_valid(prev_pte) ||
                ((pte_to_pa(prev_pte) & ~PAGE_MASK) == pa),
            "%s: prev_pte 0x%llx does not map pa 0x%llx", __func__,
            (unsigned long long)prev_pte, (unsigned long long)pa);

    const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
    pmap_ledger_debit(pmap, task_ledgers.phys_mem,
                      pt_attr_page_size(pt_attr) * PAGE_RATIO);

    if (pmap != kernel_pmap) {
      /*
       * If the prior PTE is invalid (which may happen due to a concurrent
       * remove operation), the compressed marker won't be written so we
       * shouldn't account the mapping as compressed.
       */
      const bool is_compressed =
          (pte_is_valid(prev_pte) &&
           ((sptm_pcpu->sptm_ops[cur_mapping].pte_template &
             ARM_PTE_COMPRESSED_MASK) != 0));
      const bool is_internal = (sptm_pcpu->sptm_acct_flags[cur_mapping] &
                                PMAP_SPTM_FLAG_INTERNAL) != 0;
      const bool is_altacct = (sptm_pcpu->sptm_acct_flags[cur_mapping] &
                               PMAP_SPTM_FLAG_ALTACCT) != 0;

      /*
       * The rule is that accounting related to PTE contents (wired, PTD
       * refcount) must be updated by whoever clears the PTE, while accounting
       * related to physical page attributes must be updated by whoever clears
       * the PVE.  We therefore always call pmap_disjoint_unmap_accounting()
       * here since we're removing the PVE, but only update wired/PTD accounting
       * if the prior PTE was valid.
       */
      pmap_disjoint_unmap_accounting(pmap, pai, is_compressed, is_internal,
                                     is_altacct);
      ...
      if (pte_is_wired(prev_pte)) {
        pmap_ledger_debit(pmap, task_ledgers.wired_mem,
                          pt_attr_page_size(pt_attr) * PAGE_RATIO);
        if (__improbable(os_atomic_dec_orig(
                             &sptm_pcpu->sptm_ptd_info[cur_mapping]->wiredcnt,
                             relaxed) == 0)) {
          panic("%s: over-unwire of ptdp %p, ptd info %p", __func__, ptdp,
                sptm_pcpu->sptm_ptd_info[cur_mapping]);
        }
      }
    }
  }
 ...
 * pmap_multipage_op_add_page(), are intended to allow callers to manage batched
 * SPTM operations that may span multiple physical pages.  They are intended to
 * operate in a way that allows callers such as
 * pmap_page_protect_options_with_flush_range() to insert mappings into the
 * per-CPU SPTM disjoint ops array in the same manner that they would for an
 * ordinary single-page operation. Functions such as
 * pmap_page_protect_options_with_flush_range() operate on a single physical
 * page but may be passed a non-NULL flush_range object to indicate that the
 * call is part of a larger batched operation which may span multiple physical
 * pages. In that scenario, these functions are intended to be used as follows:
 * 1) Call pmap_multipage_op_add_page() to insert a "header" for the page into
 * the per- CPU SPTM ops array.  Use the return value from this call as the
 * starting index at which to add ordinary mapping entries into the same array.
 * 2) Insert sptm_disjoint_op_t entries into the ops array in the normal manner
 * until the array is full, the SPTM options required for the upcoming sequence
 * of pages need to change, or the current mapping matches
 * flush_range->current_ptep. In the latter case,
 * pmap_insert_flush_range_template() may instead be used to insert the mapping
 * into the per-CPU SPTM region templates array.  See the documentation for
 * pmap_insert_flush_range_template() below. 3) If the array is full, call
 * pmap_multipage_op_submit_disjoint() and return to step 1). 4) If the SPTM
 * options need to change, call pmap_multipage_op_add_page() to insert a new
 * header with the updated options and, using the return value as the new
 *    insertion point for the ops array, resume step 2).
 * 5) Upon completion, if there are any pending not-yet-submitted mappings, do
 * not submit those mappings to the SPTM as would ordinarily be done for a
 * single-page call.  These trailing mappings will be submitted as part of the
 * next batch, or by the next-higher caller if the range operation is complete.
 *
 ...
 * flush_range->pending_disjoint_entries on every iteration, as long as it takes
 * care to do the following: 1) Initialize and update that insertion point as
 * described in steps 1) and 4) above. 2) Pass the updated insertion point as
 * the 'pending_disjoint_entries' parameter into the calls in steps 3) and 4)
 * above. 3) Update flush_range->pending_disjoint_entries with the
 * locally-maintained value along with step 5) above.
 */
 ...
 * @note [pending_disjoint_entries] must include *all* pending entries in the
 * SPTM ops array, including physical address "header" entries.
 * @note This function automatically updates the per_paddr_header.num_mappings
 * field for the most recent physical address header in the SPTM ops array to
 * its final value.
 *
 * @param pending_disjoint_entries The number of not-yet-submitted mappings
 * according to the caller. This value may be greater than
 * [flush_range]->pending_disjoint_entries if the caller has inserted mappings
 * into the ops array without updating [flush_range]->pending_disjoint_entries,
 * in which case this function will update
 * [flush_range]->pending_disjoint_entries with the caller's value.
 * @param flush_range The object tracking the current state of the multipage
 * disjoint operation.
 */
static inline void
pmap_multipage_op_submit_disjoint(unsigned int pending_disjoint_entries,
   ...
   * greater, we assume the caller has inserted locally-tracked mappings into
   * the array without directly updating flush_range->pending_disjoint_entries.
   * Otherwise, we assume the caller has no locally-tracked mappings and is
   * simply trying to purge any pending mappings from a prior call sequence.
   */
  ...
  if (flush_range->pending_disjoint_entries != 0) {
    ...
    flush_range->current_header->per_paddr_header.num_mappings =
        flush_range->pending_disjoint_entries -
        flush_range->current_header_first_mapping_index;
    const sptm_return_t sptm_return = sptm_update_disjoint_multipage(
        PERCPU_GET(pmap_sptm_percpu)->sptm_ops_pa,
        flush_range->pending_disjoint_entries);

    /**
     * We may be submitting the batch and exiting the epoch partway through
     * processing the PV list for a page.  That's fine, because in that case
     * we'll hold the PV lock for that page, which will prevent mappings of that
     * page from being disconnected and will prevent the completion of
     * pmap_remove() against any of those mappings, thus also guaranteeing the
     * relevant page table pages can't be freed.  The epoch still protects
     * mappings for any prior page in the batch, whose PV locks are no longer
     * held.
     */
    pmap_epoch_exit();
    enable_preemption();
...
/**
 * Insert a new physical address "header" entry into the per-CPU SPTM ops array
 * for a multi-page SPTM operation.  It is expected that the caller will
 * subsequently add mapping entries for this physical address into the array.
 *
 * @note This function will disable preemption upon creation of the first paddr
 * header (index 0 in the per-CPU SPTM ops array) and it is expected that
 *       pmap_multipage_op_submit() will subsequently be called on the same CPU.
 * @note Before inserting the new header, this function automatically updates
 * the per_paddr_header.num_mappings field for the previous physical address
 * header (if present) in the SPTM ops array to its final value.
 *
 * @param phys The physical address for which to insert a header entry.
 * @param inout_pending_disjoint_entries
 *              [input] The number of not-yet-submitted mappings according to
 * the caller. This value may be greater than
 * [flush_range]->pending_disjoint_entries if the caller has inserted mappings
 * into the ops array without updating [flush_range]->pending_disjoint_entries,
 * in which case this function will update
 * [flush_range]->pending_disjoint_entries with the caller's value. [output]
 * Returns the starting index at which the caller should insert mapping entries
 * into the per-CPU SPTM ops array.
 * @param sptm_update_options SPTM_UPDATE_* flags to pass to the SPTM call.
 *                            SPTM_UPDATE_SKIP_PAPT is automatically inserted by
 * this function.
 * @param flush_range The object tracking the current state of the multipage
 * operation.
 *
 * @return True if the region operation was submitted to the SPTM due to the ops
 * array already being full, false otherwise.  In the former case, the new
 * header will not be added to the array; the caller will need to re-invoke this
 * function after taking any necessary post-submission action (such as enabling
 * preemption).
 */
static inline bool pmap_multipage_op_add_page(
    pmap_paddr_t phys, unsigned int *inout_pending_disjoint_entries,
    uint32_t sptm_update_options, pmap_tlb_flush_range_t *flush_range) {
   ...
   * the array without directly updating flush_range->pending_disjoint_entries.
   * Otherwise, we assume the caller has no locally-tracked mappings and is
   * adding its paddr header for the first time.
   */
  if (pending_disjoint_entries > flush_range->pending_disjoint_entries) {
    flush_range->pending_disjoint_entries = pending_disjoint_entries;
  } else {
  ...
  if (pending_disjoint_entries == 0) {
     ...
     * and issue the SPTM call.  Since this operation may cover multiple
     * physical pages, we may construct the argument array and invoke the SPTM
     * without holding all relevant PVH locks or pmap locks.  We therefore need
     * to record that we are collecting and modifying mapping state so that e.g.
     * pmap_page_protect() does not attempt to retype the underlying pages and
     * pmap_remove() does not attempt to free the page tables used for these
     * mappings without first draining our epoch.
     */
    pmap_epoch_enter();
    ...
    flush_range->current_header->per_paddr_header.num_mappings =
        pending_disjoint_entries -
        flush_range->current_header_first_mapping_index;
  }
  sptm_disjoint_op_t *sptm_ops = PERCPU_GET(pmap_sptm_percpu)->sptm_ops;
  flush_range->current_header = (sptm_update_disjoint_multipage_op_t
                                     *)&sptm_ops[pending_disjoint_entries];
 ...
 * the VA region, which requires far fewer table walks than a disjoint
 * operation. Since the starting VA of the region, the owning pmap, and the
 * insertion point within the per-CPU region templates array are already known,
 * these functions don't require the special "header" entry or the complex array
 * position tracking of their disjoint equivalents above. Note that these
 * functions may be used together with the disjoint functions above; these
 * functions can be used for the "primary" mappings corresponding to the VA
 ...
 *       the preemption-disable count upon submitting to the SPTM.
 *
 * @param flush_range The object tracking the current state of the region
 * operation.
 */
static inline void
pmap_multipage_op_submit_region(pmap_tlb_flush_range_t *flush_range) {
  if (flush_range->pending_region_entries != 0) {
     ...
     * construct the disjoint ops array because those ops may point to some
     * arbitrary pmap and we need to ensure the relevant page tables and even
     * the pmap itself aren't concurrently reclaimed while our ops array points
     * to them. But for a region op like this, we know we already hold the
     * relevant pmap lock so none of the above can happen concurrently.  We
     * therefore only need to hold the epoch across the SPTM call itself to
     * prevent a concurrent unmap operation from attempting to retype the mapped
     * pages while our SPTM call has them in- flight.
     */
    if (flush_range->pending_disjoint_entries == 0) {
      pmap_epoch_enter();
    }
    const sptm_return_t sptm_return = sptm_update_region(
        flush_range->ptfr_pmap->ttep, flush_range->pending_region_start,
        flush_range->pending_region_entries,
        PERCPU_GET(pmap_sptm_percpu)->sptm_templates_pa,
        SPTM_UPDATE_PERMS_AND_WAS_WRITABLE | SPTM_UPDATE_AF |
            SPTM_UPDATE_DEFER_TLBI);
    if (flush_range->pending_disjoint_entries == 0) {
      pmap_epoch_exit();
    }
    enable_preemption();
    ...
    flush_range->pending_region_start +=
        (flush_range->pending_region_entries
 ...
 * This is meant to be used as a performance optimization for the case in which
 * a given mapping being processed by a function such as
 * pmap_page_protect_options_with_flush_range() happens to map the current
 * iteration position within [flush_range]'s VA region. In this case the mapping
 * can be inserted as a region-based template rather than a disjoint operation
 * as would be done in the general case.  The idea is that region-based SPTM
 * operations are significantly less expensive than disjoint operations, because
 * each region operation only requires a single page table walk at the beginning
 * vs. a table walk for each mapping in the disjoint case.  Since the majority
 * of mappings processed by a flush range operation belong to the main flush
 * range VA region (i.e. alias mappings outside the region are less common), the
 * performance improvement can be significant.
 *
 * @note This function will disable preemption upon inserting the first entry
 * into the per-CPU templates array, and will re-enable preemption upon
 * submitting the region operation to the SPTM.
 *
 * @param template The PTE template to insert into the per-CPU templates array.
 * @param flush_range The object tracking the current state of the region
 * operation.
 *
 * @return True if the region operation was submitted to the SPTM, false
 * otherwise.
 */
static inline bool
pmap_insert_flush_range_template(pt_entry_t template,
                                 pmap_tlb_flush_range_t *flush_range) {
  if (flush_range->pending_region_entries == 0) {
    disable_preemption();
  }
  flush_range->region_entry_added = true;
  PERCPU_GET(pmap_sptm_percpu)
      ->sptm_templates[flush_range->pending_region_entries++] = template;
...
/**
 * Wrapper function for submitting any pending operations, region-based or
 * disjoint, tracked by a flush range object.  This is meant to be used by the
 * top-level caller that iterates over the flush range's VA region and calls
 * functions such as pmap_page_protect_options_with_flush_range() or
 * arm_force_fast_fault_with_flush_range() to construct the relevant SPTM
 * operations arrays.
 *
 * @param flush_range The object tracking the current state of region and/or
 * disjoint operations.
 */
static inline void
...
/**
 * This is an internal-only flag that indicates the caller of
 * pmap_page_protect_options_with_flush_range() is removing/updating all
 * mappings in preparation for a retype operation.  In this case
 * pmap_page_protect_options() will assume (and assert) that the PVH lock for
 * the physical page is held by the calller, and will perform the necessary pmap
 * epoch drain and retype the page back to XNU_DEFAULT prior to returning.
 */
#define PMAP_OPTIONS_PPO_PENDING_RETYPE 0x80000000
_Static_assert(
    PMAP_OPTIONS_PPO_PENDING_RETYPE &PMAP_OPTIONS_RESERVED_MASK,
    "PMAP_OPTIONS_PPO_PENDING_RETYPE outside reserved encoding space");

/**
 * Lower the permission for all mappings to a given page. If VM_PROT_NONE is
 * specified, the mappings will be removed.
 *
 ...
 *                PMAP_OPTIONS_PPO_PENDING_RETYPE indicates the PVH lock for
 * ppnum is already locked and a pmap epoch drain shold be performed, along with
 *                retyping [ppnum] back to XNU_DEFAULT.
 *                PMAP_OPTIONS_COMPRESSOR indicates the function is called by
 * the VM compressor. PMAP_OPTIONS_RETYPE requests the [ppnum] be retyped back
 * to XNU_DEFAULT, along with an epoch drain; like
 * PMAP_OPTIONS_PPO_PENDING_RETYPE but without the PVH lock being held by the
 * caller.
 * @param locked_pvh If non-NULL, this indicates the PVH lock for [ppnum] is
 * already locked by the caller.  This is an input/output parameter which may be
 * updated to reflect a new PV head value to be passed to a later call to
 * pvh_unlock().
 * @param flush_range When present, this function will skip the TLB flush for
 * the mappings that are covered by the range, leaving that to be done later by
 * the caller.  It may also avoid submitting mapping updates directly to the
 * SPTM, instead accumulating them in a per-CPU array to be submitted later by
 * the caller.
 *
 * @note PMAP_OPTIONS_NOFLUSH and flush_range cannot both be specified.
 */
MARK_AS_PMAP_TEXT static void pmap_page_protect_options_with_flush_range(
    ppnum_t ppnum, vm_prot_t prot, unsigned int options,
    locked_pvh_t *locked_pvh, pmap_tlb_flush_range_t *flush_range) {
   ...
   *
   * PMAP_OPTIONS_NOFLUSH indicates there is no need of flushing the TLB in the
   * entire operation, and flush_range indicates the caller requests deferral of
   * the TLB flushing. Fundemantally, the two semantics conflict with each
   * other, so assert they are not both true.
   */
  ...
  case VM_PROT_READ | VM_PROT_EXECUTE:
    break;
  default:
    /* PPL security model requires that we flush TLBs before we exit if the page
     * may be recycled. */
  ...
  /**
   * We don't support cross-page batching (indicated by flush_range being
   * non-NULL) for removals, as removals must use the SPTM prev_ptes array for
   * accounting, which isn't supported for cross- page batches.
   */
  ...
  /*
   * PVH should be locked before accessing per-CPU data, as we're relying on the
   * lock to disable preemption.
   */
 ...
 * dereferencing __block variables through stack forwarding pointers) isn't
 * needed here, as we never need to use this code sequence as a closure.
 */
#define PPO_PERCPU_INIT()                                                      \
  do {                                                                         \
    disable_preemption();                                                      \
  ...
  if (pvh_test_type(local_locked_pvh.pvh, PVH_TYPE_PTEP)) {
    ...
    pveh_p = pve_p;
  } else if (__improbable(
                 !pvh_test_type(local_locked_pvh.pvh, PVH_TYPE_NULL))) {
    panic("%s: invalid PV head 0x%llx for PA 0x%llx", __func__,
          (uint64_t)local_locked_pvh.pvh, (uint64_t)phys);
  }
   ...
   *
   * 1. PMAP_OPTIONS_NOFLUSH is specified. In such case, SPTM doesn't need to
   * flush TLB and neither does pmap.
   * 2. PMAP_OPTIONS_NOFLUSH is not specified, but flush_range is, indicating
   * the caller intends to flush TLB itself (with range TLBI). In such case, we
   * check the flush_range limits and only issue the TLBI if a mapping is out of
   * the range.
   * 3. Neither PMAP_OPTIONS_NOFLUSH nor a valid flush_range pointer is
   * specified. In such case, we should just let SPTM handle TLBI flushing.
   */
  const bool defer_tlbi = (options & PMAP_OPTIONS_NOFLUSH) || flush_range;
  const uint32_t sptm_update_options =
      SPTM_UPDATE_PERMS_AND_WAS_WRITABLE |
      (defer_tlbi ? SPTM_UPDATE_DEFER_TLBI : 0);
  ...
  while ((pve_p != PV_ENTRY_NULL) || (pte_p != PT_ENTRY_NULL)) {
    if (__improbable(pvh_lock_sleep_mode_needed)) {
      ...
      /**
       * Undo the explicit preemption disable done in the last call to
       * PPO_PER_CPU_INIT(). If the PVH lock is placed in sleep mode, we can't
       * rely on it to disable preemption, so we need these explicit preemption
       * twiddles to ensure we don't get migrated off- core while processing
       * SPTM per-CPU data.  At the same time, we also want preemption to
       * briefly be re-enabled every SPTM_MAPPING_LIMIT mappings so that any
       * pending urgent ASTs can be handled.
       */
      enable_preemption();
    ...
    if (pvh_ptep_is_iommu(pte_p)) {
      ...
      if (__improbable(remove && (options & PMAP_OPTIONS_COMPRESSOR))) {
        ...
        panic("%s: attempt to compress ppnum 0x%x owned by iommu driver "
              "%u (token: %#x), pve_p=%p",
              __func__, ppnum, GET_IOMMU_ID(iommu), GET_IOMMU_TOKEN(iommu),
              pve_p);
      }
    ...
    if ((flush_range != NULL) && (pte_p == flush_range->current_ptep)) {
      /**
       * If the current mapping matches the flush range's current iteration
       * position, there's no need to do the work of getting the PTD.  We
       * already know the pmap, and the VA is implied by
       * flush_range->pending_region_start.
       */
    ...
    /**
     * If the PTD is NULL, we're adding the current mapping to the pending
     * region templates instead of the pending disjoint ops, so we don't need to
     * do flush range disjoint op management.
     */
    if ((flush_range != NULL) && (ptdp != NULL)) {
      /**
       * Insert a "header" entry for this physical page into the SPTM disjoint
       * ops array. We do this in three cases: 1) We're at the beginning of the
       * SPTM ops array (num_mappings == 0,
       * flush_range->pending_disjoint_entries == 0). 2) We may not be at the
       * beginning of the SPTM ops array, but we are about to add the first
       * operation for this physical page (num_mappings == 0,
       * flush_range->pending_disjoint_entries == ?). 3) We need to change the
       * options passed to the SPTM for a run of one or more mappings.
       * Specifically, if we encounter a run of mappings that reside outside the
       * VA region of our flush_range, or that belong to a pmap other than the
       * one targeted by our flush_range, we should ask the SPTM to flush the
       * TLB for us (i.e., clear SPTM_UPDATE_DEFER_TLBI), but only for those
       * specific mappings.
       */
      uint32_t per_mapping_sptm_update_options = sptm_update_options;
      if ((flush_range->ptfr_pmap != pmap) || (va >= flush_range->ptfr_end) ||
          (va < flush_range->ptfr_start)) {
        per_mapping_sptm_update_options &= ~SPTM_UPDATE_DEFER_TLBI;
      }
      if ((num_mappings == 0) ||
          (flush_range->current_header->per_paddr_header.options !=
           per_mapping_sptm_update_options)) {
        if (pmap_multipage_op_add_page(phys, &num_mappings,
                                       per_mapping_sptm_update_options,
                                       flush_range)) {
          /**
           * If we needed to submit the pending disjoint ops to make room for
           * the new page, flush any pending region ops to reenable preemption
           * and restart the loop with the lock in sleep mode.  This prevents
           * preemption from being held disabled for an arbitrary amount of time
           * in the pathological case in which we have both pending region ops
           * and an excessively long PV list that repeatedly requires new page
           * headers with SPTM_MAPPING_LIMIT - 1 entries already pending.
           */
          pmap_multipage_op_submit_region(flush_range);
    ...
    if (__improbable((pmap == NULL) || (pte_is_valid(spte) &&
                                        (atop(pte_to_pa(spte)) != ppnum)))) {
#if MACH_ASSERT
      if ((pmap != NULL) && (pve_p != PV_ENTRY_NULL) &&
          (kern_feature_override(KF_PMAPV_OVRD) == FALSE)) {
        /* Temporarily set PTEP to NULL so that the logic below doesn't pick it
         * up as duplicate. */
        ...
        do {
          if (pve_find_ptep_index(check_pvep, pte_p) != -1) {
            panic_plain("%s: duplicate pve entry ptep=%p pmap=%p, pvh=%p, "
                        "pvep=%p, pai=0x%x",
                        __func__, pte_p, pmap, (void *)local_locked_pvh.pvh,
                        pve_p, pai);
          }
...
#endif
      panic("%s: bad PVE pte_p=%p pmap=%p prot=%d options=%u, pvh=%p, "
            "pveh_p=%p, pve_p=%p, pte=0x%llx, va=0x%llx ppnum: 0x%x",
            __func__, pte_p, pmap, prot, options, (void *)local_locked_pvh.pvh,
            pveh_p, pve_p, (uint64_t)*pte_p, (uint64_t)va, ppnum);
    }
    ...
    if (remove) {
      ...
      if (pmap != kernel_pmap) {
        const bool is_internal =
            ppattr_pve_is_internal(pai, pve_p, pve_ptep_idx);
      ...
      /*
       * We must at least clear the 'was writeable' flag, as we're at least
       * revoking write access, meaning that the VM is effectively requesting
       * that subsequent write accesses to these mappings go through vm_fault()
       * instead of being handled by arm_fast_fault().
       */
  ...
  protect_skip_pve:
    if ((num_mappings + num_skipped_mappings) >= SPTM_MAPPING_LIMIT) {
      if (flush_range != NULL) {
        /* See comment above for why we submit both disjoint and region ops when
         * we hit the limit. */
        pmap_multipage_op_submit_disjoint(num_mappings, flush_range);
        pmap_multipage_op_submit_region(flush_range);
        ...
        if (remove) {
          ...
          sptm_update_disjoint(phys, sptm_pcpu->sptm_ops_pa, num_mappings,
                               sptm_update_options);
        }
      }
      pvh_lock_sleep_mode_needed = true;
  ...
  if (remove) {
    ...
    if (options & (PMAP_OPTIONS_PPO_PENDING_RETYPE | PMAP_OPTIONS_RETYPE)) {
       ...
       * can be directly retyped to one another without going through
       * XNU_DEFAULT.
       */
     ...
     * by passing PMAP_OPTIONS_PPO_PENDING_RETYPE, then we'll need to drain the
     * epochs regardless of current frame type to prepare for the caller's
     * retype.
     */
    const bool drain_needed =
        retype_needed || !!(options & PMAP_OPTIONS_PPO_PENDING_RETYPE);
    ...
    if (__improbable(retype_needed)) {
      const sptm_retype_params_t retype_params = {.raw =
...
MARK_AS_PMAP_TEXT void pmap_page_protect_options_internal(ppnum_t ppnum,
                                                          vm_prot_t prot,
                                                          unsigned int options,
                                                          void *arg) {
  if (arg != NULL) {
    /*
     * This is a legacy argument from pre-ARM era that the VM layer passes in to
     * hint that it will call pmap_flush() later to flush the TLB. On ARM
     * platforms, however, pmap_flush() is not implemented, as it's typically
     * more efficient to perform the TLB flushing inline with the page table
     * updates themselves. Therefore, if the argument is non-NULL, pmap will
     * take care of TLB flushing itself by clearing PMAP_OPTIONS_NOFLUSH.
     */
...
void pmap_page_protect_options(ppnum_t ppnum, vm_prot_t prot,
...
#if __has_feature(ptrauth_calls) &&                                            \
    (defined(XNU_TARGET_OS_OSX) || (DEVELOPMENT || DEBUG))
MARK_AS_PMAP_TEXT void pmap_disable_user_jop_internal(pmap_t pmap) {
  if (pmap == kernel_pmap) {
...
void pmap_disable_user_jop(pmap_t pmap) {
 ...
 */
bool pmap_has_prot_policy(__unused pmap_t pmap,
                          __unused bool translated_allow_execute,
 ...
 */
void pmap_protect(pmap_t pmap, vm_map_address_t b, vm_map_address_t e,
                  vm_prot_t prot) {
  pmap_protect_options(pmap, b, e, prot, 0, NULL);
}

static bool pmap_protect_strong_sync(unsigned int num_mappings __unused) {
  return false;
}

MARK_AS_PMAP_TEXT vm_map_address_t pmap_protect_options_internal(
    pmap_t pmap, vm_map_address_t start, vm_map_address_t end, vm_prot_t prot,
    unsigned int options, __unused void *args) {
  pt_entry_t *pte_p;
  bool set_NX = true;
  bool set_XO = false;
  ...
  if (__improbable((end < start) ||
                   (end > ((start + pt_attr_twig_size(pt_attr)) &
  ...
  if (__improbable(should_have_removed)) {
    panic("%s: should have been a remove operation, "
          ...
          __FUNCTION__, pmap, (void *)start, (void *)end, prot, options, args);
  }
  ...
  while (va < end) {
...
#if DEVELOPMENT || DEBUG
    /*
     * PMAP_OPTIONS_PROTECT_IMMEDIATE is an internal-only option that's intended
     * to provide a "backdoor" to allow normally write-protected compressor
     * pages to be be temporarily written without triggering expensive write
     * faults.
     */
    while (force_write) {
      ...
      __assert_only const sptm_return_t sptm_status =
          sptm_map_page(pmap->ttep, va, spte);

      /**
       * We don't expect the VM to be concurrently calling pmap_remove() against
       * these compressor mappings.  If it does for some reason, that could
       * cause the above call to return either SPTM_SUCCESS or
...
#endif
    {
      ...
      if (num_mappings == SPTM_MAPPING_LIMIT) {
        /**
         * Enter the pmap epoch for the batched update operation.  This is
         * necessary because we cannot reasonably hold the PVH locks for all
         * pages mapped by the region during this call, so a concurrent
         * pmap_page_protect() operation against one of those pages may race
         * this call.  That should be perfectly fine as far as the PTE updates
         * are concerned, but if pmap_page_protect() then needs to retype the
         * page, an SPTM violation may result if it does not first drain our
         * epoch.
         */
        pmap_epoch_enter();
        sptm_update_region(pmap->ttep, sptm_start_va, num_mappings,
                           sptm_pcpu->sptm_templates_pa,
                           SPTM_UPDATE_PERMS_AND_WAS_WRITABLE);
        pmap_epoch_exit();
        need_strong_sync =
            need_strong_sync || pmap_protect_strong_sync(num_mappings);

        /* Temporarily re-enable preemption to allow any urgent ASTs to be
         * processed. */
        enable_preemption();
        num_mappings = 0;
        sptm_start_va = va;
  ...
  /* This won't happen in the force_write case as we should never increment
   * num_mappings. */
  if (num_mappings != 0) {
    pmap_epoch_enter();
    sptm_update_region(pmap->ttep, sptm_start_va, num_mappings,
...
void pmap_protect_options(pmap_t pmap, vm_map_address_t b, vm_map_address_t e,
                          vm_prot_t prot, unsigned int options,
                          __unused void *args) {
  ...
  if ((b | e) & pt_attr_leaf_offmask(pt_attr)) {
    panic("pmap_protect_options() pmap %p start 0x%llx end 0x%llx", pmap,
 ...
 * @return KERN_SUCCESS.
 */
kern_return_t pmap_map_block(pmap_t pmap, addr64_t va, ppnum_t pa,
                             uint32_t size, vm_prot_t prot, int attr,
                             unsigned int flags) {
  return pmap_map_block_addr(pmap, va, ((pmap_paddr_t)pa) << PAGE_SHIFT, size,
                             prot, attr, flags);
}
 ...
 * @return KERN_SUCCESS.
 */
kern_return_t pmap_map_block_addr(pmap_t pmap, addr64_t va, pmap_paddr_t pa,
                                  uint32_t size, vm_prot_t prot, int attr,
                                  unsigned int flags) {
#if __ARM_MIXED_PAGE_SIZE__
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  ...
  for (ppnum_t page = 0; page < size; page++) {
    if (pmap_enter_addr(pmap, va, pa, prot, VM_PROT_NONE, attr, TRUE,
                        PMAP_MAPPING_TYPE_INFER) != KERN_SUCCESS) {
      panic("%s: failed pmap_enter_addr, "
            "pmap=%p, va=%#llx, pa=%llu, size=%u, prot=%#x, flags=%#x",
            __FUNCTION__, pmap, va, (uint64_t)pa, size, prot, flags);
    }
...
kern_return_t pmap_enter_addr(pmap_t pmap, vm_map_address_t v, pmap_paddr_t pa,
                              vm_prot_t prot, vm_prot_t fault_type,
                              unsigned int flags, boolean_t wired,
                              pmap_mapping_type_t mapping_type) {
  return pmap_enter_options_addr(pmap, v, pa, prot, fault_type, flags, wired, 0,
                                 NULL, mapping_type);
}
...
kern_return_t pmap_enter(pmap_t pmap, vm_map_address_t v, ppnum_t pn,
                         ...
                         unsigned int flags, boolean_t wired,
                         pmap_mapping_type_t mapping_type) {
  return pmap_enter_addr(pmap, v, ((pmap_paddr_t)pn) << PAGE_SHIFT, prot,
                         fault_type, flags, wired, mapping_type);
}

/**
 * Helper function for determining the frame type that will be required for a
 * physical page given a set of mapping constraints.
 *
 * @param pmap The address space in which the page will be mapped.
 * @param pte The fully-configured page table entry, including permissions and
 * output address, that will be used for the mapping.
 * @param vaddr The virtual address that will be mapped using [pte]
 * @param options Extra mapping options that would be passed to pmap_enter()
 * when performing the mapping
 * @param mapping_type The mapping type enum that would be passed to
 * pmap_enter() when performing the mapping
 * @param prev_frame_type Output param that will store the existing frame type
 * for the physical page mapped by [pte].  As an optimization, this will only be
 * queried if [*new_frame_type] is determined to be something other than
 * XNU_DEFAULT, otherwise it will be assumed to be XNU_DEFAULT
 * @param new_frame_type Output param that will store the new frame type that
 * will be required for the physical page mapped by [pte]
 */
static inline void pmap_frame_type_for_pte(pmap_t pmap __assert_only,
                                           pt_entry_t pte,
  ...
  *prev_frame_type = *new_frame_type = XNU_DEFAULT;
   ...
   * keep the existing logic of deriving the SPTM frame type from the XPRR
   * permissions.
   *
   * If the caller specified another mapping type, we simply follow that. This
   * refactor was needed for the XNU_KERNEL_RESTRICTED work, and it also allows
   * us to be more precise at what we want. It's better to let the caller
   * specify the mapping type rather than use the permissions for that.
   *
   * In the future, we should move entirely to use pmap_mapping_type_t; see
   * rdar://114886323.
   */
  if (mapping_type != PMAP_MAPPING_TYPE_INFER) {
    ...
    case PMAP_MAPPING_TYPE_ROZONE:
      assert(((pmap == kernel_pmap) &&
              zone_spans_ro_va(
                  vaddr, vaddr + pt_attr_page_size(pmap_get_pt_attr(pmap)))));
      *new_frame_type = (sptm_frame_type_t)mapping_type;
      break;
    ...
    /*
     * Always check for XPRR_USER_JIT_PERM before we check for anything else.
     * When using RWX permissions, the only allowed type is XNU_USER_JIT,
     * regardless of any other flags which the VM may have provided.
     *
     * TODO: Assert that the PMAP_OPTIONS_XNU_USER_DEBUG flag isn't set when
     * entering this case. We can't do this for now because this might trigger
     * on some macOS systems where applications use MAP_JIT with RW/RX
     * permissions, and then later switch to RWX (which will cause a switch to
     * XNU_USER_JIT from XNU_USER_DEBUG but the VM will still have
     * PMAP_OPTIONS_XNU_USER_DEBUG set). If the VM can catch this case, and
     * remove PMAP_OPTIONS_XNU_USER_DEBUG when an application switches to RWX,
     * then we can start asserting this requirement.
     */
    *new_frame_type = XNU_USER_JIT;
  } else if (__improbable(options & PMAP_OPTIONS_XNU_USER_DEBUG)) {
    /*
     * Both XNU_USER_DEBUG and XNU_USER_EXEC allow RX permissions. Given that,
     * we must test for PMAP_OPTIONS_XNU_USER_DEBUG before we test for
     * XNU_USER_EXEC since the XNU_USER_DEBUG type overlays the XNU_USER_EXEC
     * type.
     */
    ...
    *new_frame_type = XNU_USER_EXEC;
             ...
             (pte_was_writeable(pte) &&
              (pte_to_xprr_perm(pte) == XPRR_USER_RO_PERM))) {
    /**
     * Allow retyping from user executable types (except XNU_USER_DEBUG, which
     * already allows user RW mappings) back to XNU_DEFAULT if a writable
     * mapping is requested. Our retype logic will disconnect all existing
     * mappings, so future attempts to execute these pages will fault, retype
     * back to exec, and go back through any needed CS validation.  For all
     * other current frame types, just leave the previous and new frame types
     * unchanged; for most other types attempting to add a user RW mapping is a
     * bug and we should just let the SPTM throw a violation.
     */
 ...
 * @param pp_attr_bits Output parameter that will return the physical page
 * attributes to apply to pp_attr_table for the new mapping.
 *
...
pmap_construct_pte(const pmap_t pmap, pmap_paddr_t pa, vm_prot_t prot,
                   vm_prot_t fault_type, boolean_t wired,
...
/**
 * This function allows the VM to query whether a mapping operation will result
 * in a page being retyped, without actually performing the mapping operation.
 * It's useful for the VM to know this when performing up-front page validation
 * under the VM object lock.
 *
 ...
 * @param options The extra mapping options that would be passed to pmap_enter()
 * if the mapping operation were performed
 * @param mapping_type The mapping type enum that would be passed to
 * pmap_enter() if the mapping operation were performed
 *
 * @return True if the mapping operation would produce a retype of the page at
 * [pn], False otherwise
 */
bool pmap_will_retype(pmap_t pmap, vm_map_address_t vaddr, ppnum_t pn,
                      vm_prot_t prot, unsigned int options,
                      pmap_mapping_type_t mapping_type) {
  const pmap_paddr_t paddr = ptoa(pn);
  uint16_t pp_attr_bits;
  pt_entry_t pte =
      pmap_construct_pte(pmap, paddr, prot, prot, false, &pp_attr_bits);
  sptm_frame_type_t prev_frame_type, new_frame_type;
  pmap_frame_type_for_pte(pmap, pte, vaddr, options, mapping_type,
                          &prev_frame_type, &new_frame_type);
 ...
 *
 * @note performs no page table or accounting modifications, nor any lasting
 * SPTM page type modification, on failure.
 * @note expects to be called with preemption disabled to guarantee safe access
 * to SPTM per-CPU data.
 *
 * @param pmap The pmap representing the address space in which to store the new
 * PTE
 ...
 * @param locked_pvh Input/Output parameter pointing to a wrapped pv_head_table
 * entry returned by a previous call to pvh_lock().  *locked_pvh will be updated
 * if existing mappings need to be disconnected prior to retyping.
 * @param old_pte Returns the prior PTE contents, iff the PTE is successfully
 * updated
 * @param options bitmask of PMAP_OPTIONS_* flags passed to
 * pmap_enter_options().
 * @param mapping_type The type of the new mapping, this defines which SPTM
 * frame type to use.
 *
 * @return SPTM_SUCCESS iff able to successfully update *pte_p to new_pte via
 * sptm_map_page(), SPTM_MAP_VALID if an existing mapping was successfully
 * upgraded via sptm_map_page(), SPTM_MAP_FLUSH_PENDING if the TLB flush of a
 * previous mapping is still in-flight and the mapping operation should be
 * retried, or if the mapping operation should be retried because we had to
 * temporarily re-enable preemption which would invalidate caller-held per-CPU
 * data. Otherwise an appropriate SPTM or TXM error code; in these cases the
 * mapping should not be retried and the caller should return an error.
 */
static inline sptm_return_t
pmap_enter_pte(pmap_t pmap, pt_entry_t *pte_p, pt_entry_t new_pte,
               locked_pvh_t *locked_pvh, pt_entry_t *old_pte,
               vm_map_address_t v, unsigned int options,
               pmap_mapping_type_t mapping_type) {
  sptm_pte_t prev_pte;
  ...
  /* SPTM TODO: handle PAGE_RATIO_4 configurations if those devices remain
   * supported. */
  ...
  pmap_frame_type_for_pte(pmap, new_pte, v, options, mapping_type,
                          &prev_frame_type, &new_frame_type);

  if (__improbable(new_frame_type != prev_frame_type)) {
    /**
     * Remove all existing mappings prior to retyping, so that we can safely
     * retype without having to worry about a concurrent operation on one of
     * those mappings triggering an SPTM violation.  In particular,
     * pmap_remove() may clear a mapping to this page without holding its PVH
     * lock.  This approach works because we hold the PVH lock during this call,
     * and any attempt to enter a new mapping for the page will also need to
     * grab the PVH lock and call this function.
     */
    pmap_page_protect_options_with_flush_range((ppnum_t)atop(pa), VM_PROT_NONE,
                                               PMAP_OPTIONS_PPO_PENDING_RETYPE,
                                               locked_pvh, NULL);
    /**
     * In the unlikely event that pmap_page_protect_options_with_flush_range()
     * had to process an excessively long PV list, it will have enabled
     * preemption by placing the PVH lock in sleep mode.  In this case, we may
     * have been migrated to a different CPU, and caller assumptions about the
     * state of per-CPU data (such as per-CPU PVE availability) will no longer
     * hold true.  Ask the caller to retry by pretending we encountered a
     * pending flush.
     */
    ...
    sptm_retype_params_t retype_params = {.raw = SPTM_RETYPE_PARAMS_NULL};
    /* Reload the existing frame type, as pmap_page_protect_options() may have
     * changed it back to XNU_DEFAULT. */
  ...
  if (pmap->type == PMAP_TYPE_NESTED) {
    /**
     * Enter the epoch before we check the unnesting state of the leaf page
     * table, so that a concurrent pmap_unnest() operation can guarantee that we
     * either observe the unnested table state and install a non-global mapping,
     * or have finished installing a global mapping before it marks all existing
     * mappings as non-global.
     */
    pmap_epoch_enter();
    vm_map_offset_t nested_region_size =
        os_atomic_load(&pmap->nested_region_size, acquire);
    if (nested_region_size && (v >= pmap->nested_region_addr) &&
        (v < (pmap->nested_region_addr + nested_region_size))) {
      assert(pmap->nested_region_addr != 0);
      assert(pmap->nested_region_unnested_table_bitmap != NULL);
      unsigned int index =
          (unsigned int)((v - pmap->nested_region_addr) >>
                         pt_attr_twig_shift(pmap_get_pt_attr(pmap)));

      if ((bitmap_test(pmap->nested_region_unnested_table_bitmap,
                       UNNEST_IN_PROGRESS_BIT(index)))) {
        new_pte |= ARM_PTE_NG;
      }
    }
  }
  ...
  if (__improbable((sptm_status != SPTM_SUCCESS) &&
                   (sptm_status != SPTM_MAP_VALID))) {
    /*
     * We should always undo our previous retype, even if the SPTM returned
     * SPTM_MAP_FLUSH_PENDING as opposed to a TXM error.  In the case of
     * SPTM_MAP_FLUSH_PENDING, pmap_enter() will drop the PVH lock before
     * turning around to retry the mapping operation.  It may then be possible
     * for the mapping state of the page to change such that our next attempt to
     * map it will fail with a TXM error, so if we were to leave the new type in
     * place here we would then have lost our record of the previous type and
     * would effectively leave the page in an inconsistent state.
     */
    if (__improbable(new_frame_type != prev_frame_type)) {
      sptm_retype_params_t retype_params = {.raw = SPTM_RETYPE_PARAMS_NULL};
  ...
  *old_pte = prev_pte = PERCPU_GET(pmap_sptm_percpu)->sptm_prev_ptes[0];
  ...
  if (prev_pte != new_pte) {
    changed_wiring =
        pte_is_compressed(prev_pte, pte_p)
    ...
    PMAP_TRACE(
        4 + pt_attr_leaf_level(pmap_get_pt_attr(pmap)), PMAP_CODE(PMAP__TTE),
        VM_KERNEL_ADDRHIDE(pmap), VM_KERNEL_ADDRHIDE(v),
        VM_KERNEL_ADDRHIDE(
            v + (pt_attr_page_size(pmap_get_pt_attr(pmap)) * PAGE_RATIO)),
        new_pte);
  }
...
MARK_AS_PMAP_TEXT static pt_entry_t wimg_to_pte(unsigned int wimg,
                                                pmap_paddr_t pa) {
  pt_entry_t pte;
  ...
  case VM_WIMG_IO:
    ...
    if (is_dram_addr(pa)) {
      ...
      if (__improbable((io_rgn != NULL) &&
                       (io_rgn->wimg & PMAP_IO_RANGE_STRONG_SYNC))) {
...
MARK_AS_PMAP_TEXT kern_return_t pmap_enter_options_internal(
    pmap_t pmap, vm_map_address_t v, pmap_paddr_t pa, vm_prot_t prot,
    vm_prot_t fault_type, unsigned int flags, boolean_t wired,
    unsigned int options, pmap_mapping_type_t mapping_type) {
  ...
  /**
   * Prepare for the SPTM call early by prefetching the relavant FTEs. Cache
   * misses in SPTM accessing these turn out to contribute to a large portion of
   * delay on the critical path. Technically, sptm_prefetch_fte may not find an
   * FTE associated with pa and return LIBSPTM_FAILURE. However, we are okay
   * with that as it's only a best-effort performance optimization.
   */
  ...
  if ((v)&pt_attr_leaf_offmask(pt_attr)) {
    panic("pmap_enter_options() pmap %p v 0x%llx", pmap, (uint64_t)v);
  }

  if (__improbable((pmap == kernel_pmap) && (v >= CPUWINDOWS_BASE) &&
                   (v < CPUWINDOWS_TOP))) {
    panic("pmap_enter_options() kernel pmap %p v 0x%llx belongs to "
          "[CPUWINDOWS_BASE: 0x%llx, CPUWINDOWS_TOP: 0x%llx)",
          pmap, (uint64_t)v, (uint64_t)CPUWINDOWS_BASE,
          (uint64_t)CPUWINDOWS_TOP);
  }

  if ((pa)&pt_attr_leaf_offmask(pt_attr)) {
    panic("pmap_enter_options() pmap %p pa 0x%llx", pmap, (uint64_t)pa);
  }
...
#endif
      panic("pmap_enter_options(): attempt to add executable mapping to "
            "kernel_pmap");
  }
   ...
   * We need to be careful about modifying non-local data structures before
   * commiting the new pte since we may need to re-do the transaction.
   */
  ...
  if (pte_is_valid(prev_pte) && (pte_to_pa(prev_pte) != pa)) {
     ...
     * If that assumption is violated, sptm_map_page() will panic as the
     * architecture does not allow the output address of a mapping to be changed
     * without a break- before-make sequence.
     */
  ...
  while (!committed) {
     ...
     */
    pt_entry_t pte = pmap_construct_pte(
        pmap, pa, ((options & PMAP_OPTIONS_MAP_TPRO) ? VM_PROT_RORW_TP : prot),
        fault_type, wired, &pp_attr_bits);

    if (pa_valid(pa)) {
      unsigned int pai;
      boolean_t is_altacct = FALSE, is_internal = FALSE, is_reusable = FALSE,
                is_external = FALSE;
      ...
      const bool allocation_required =
          !pvh_test_type(locked_pvh.pvh, PVH_TYPE_NULL) &&
          !(pvh_test_type(locked_pvh.pvh, PVH_TYPE_PTEP) &&
            pvh_ptep(locked_pvh.pvh) == pte_p);
      ...
      if (__improbable(allocation_required && (local_pv_free->count < 2))) {
        ...
        while (new_allocated_pves < 2) {
          local_pv_free = &pmap_get_cpu_data()->pv_free;
          pv_status =
              pv_alloc(pmap, PMAP_LOCK_SHARED, options,
                       &new_pve_p[new_allocated_pves], &locked_pvh, wiredcnt);
          if (pv_status == PV_ALLOC_FAIL) {
             ...
             * On non-PPL devices, dropping the lock re-enables preemption so we
             * may be on a different CPU now.
             */
            local_pv_free = &pmap_get_cpu_data()->pv_free;
          } else {
      ...
      const sptm_return_t sptm_status = pmap_enter_pte(
          pmap, pte_p, pte, &locked_pvh, &spte, v, options, mapping_type);
      ...
      if (!had_valid_mapping) {
        ...
        int pve_ptep_idx = 0;
        pv_status = pmap_enter_pv(pmap, pte_p, options, PMAP_LOCK_SHARED,
                                  &locked_pvh, &new_pve_p, &pve_ptep_idx);
        /* We did all the allocations up top. So this shouldn't be able to fail.
         */
        if (pv_status != PV_ALLOC_SUCCESS) {
          panic(
              "%s: unexpected pmap_enter_pv ret code: %d. new_pve_p=%p pmap=%p",
        ...
        if (pmap != kernel_pmap) {
          ...
          if (ppattr_test_reusable(pai) && !is_altacct) {
            is_reusable = TRUE;
      ...
      if (!had_valid_mapping && (pmap != kernel_pmap)) {
        pmap_ledger_credit(pmap, task_ledgers.phys_mem,
                           pt_attr_page_size(pt_attr) * PAGE_RATIO);
        ...
        if (is_internal) {
           ...
           */
          pmap_ledger_credit(pmap, task_ledgers.internal,
                             pt_attr_page_size(pt_attr) * PAGE_RATIO);
          if (is_altacct) {
            /*
             ...
             */
            pmap_ledger_credit(pmap, task_ledgers.alternate_accounting,
                               pt_attr_page_size(pt_attr) * PAGE_RATIO);
          } else {
            if (pte_is_compressed(spte, pte_p) &&
                !(spte & ARM_PTE_COMPRESSED_ALT)) {
              /* Replacing a compressed page (with internal accounting). No
               * change to phys_footprint. */
        ...
        if (is_reusable) {
          pmap_ledger_credit(pmap, task_ledgers.reusable,
                             pt_attr_page_size(pt_attr) * PAGE_RATIO);
        } else if (is_external) {
          pmap_ledger_credit(pmap, task_ledgers.external,
                             pt_attr_page_size(pt_attr) * PAGE_RATIO);
        }
      }
    } else {
      ...
      wimg_bits = pmap_cache_attributes(pn);
      if ((flags & (VM_WIMG_MASK | VM_WIMG_USE_DEFAULT))) {
        wimg_bits = (wimg_bits & (~VM_WIMG_MASK)) |
                    (flags & (VM_WIMG_MASK | VM_WIMG_USE_DEFAULT));
      }

      pte |= pmap_get_pt_ops(pmap)->wimg_to_pte(wimg_bits, pa);

      /**
       * pmap_enter_pte() expects to be called with preemption disabled so it
       * can access the per-CPU prev_ptes array.
       */
      disable_preemption();
      const sptm_return_t sptm_status = pmap_enter_pte(
          pmap, pte_p, pte, NULL, &spte, v, options, mapping_type);
      enable_preemption();
      assert(committed == false);
      if ((sptm_status == SPTM_SUCCESS) || (sptm_status == SPTM_MAP_VALID)) {
        committed = true;
        ...
        panic("%s: Unexpected SPTM return code %u for non-managed PA 0x%llx",
              __func__, (unsigned int)sptm_status, (unsigned long long)pa);
      }
    }
    if (committed) {
      if (pte_is_compressed(spte, pte_p)) {
...
kern_return_t pmap_enter_options_addr(pmap_t pmap, vm_map_address_t v,
  ...
  kr = pmap_enter_options_internal(pmap, v, pa, prot, fault_type, flags, wired,
                                   options, mapping_type);
...
kern_return_t pmap_enter_options(pmap_t pmap, vm_map_address_t v, ppnum_t pn,
                                 vm_prot_t prot, vm_prot_t fault_type,
                                 unsigned int flags, boolean_t wired,
                                 unsigned int options, __unused void *arg,
                                 pmap_mapping_type_t mapping_type) {
  return pmap_enter_options_addr(pmap, v, ((pmap_paddr_t)pn) << PAGE_SHIFT,
                                 prot, fault_type, flags, wired, options, arg,
                                 mapping_type);
}
...
pmap_change_wiring_internal(pmap_t pmap, vm_map_address_t v, boolean_t wired) {
  ...
  if (pte_p == PT_ENTRY_NULL) {
    if (!wired) {
      /*
       * The PTE may have already been cleared by a disconnect/remove operation,
       * and the L3 table may have been freed by a remove operation.
       */
  ...
  pmap_epoch_enter();
  sptm_update_region(pmap->ttep, v, 1, sptm_pcpu->sptm_templates_pa,
...
void pmap_change_wiring(pmap_t pmap, vm_map_address_t v, boolean_t wired) {
  pmap_change_wiring_internal(pmap, v, wired);
}

MARK_AS_PMAP_TEXT pmap_paddr_t pmap_find_pa_internal(pmap_t pmap, addr64_t va) {
  pmap_paddr_t pa = 0;
...
pmap_paddr_t pmap_find_pa_nofault(pmap_t pmap, addr64_t va) {
  pmap_paddr_t pa = 0;

  if (pmap == kernel_pmap) {
    pa = mmu_kvtop(va);
  } else if ((current_thread()->map) &&
             (pmap == vm_map_pmap(current_thread()->map))) {
    /*
     * Note that this doesn't account for PAN: mmu_uvtop() may return a valid
     * translation even if PAN would prevent kernel access through the
     * translation. It's therefore assumed the UVA will be accessed in a
     * PAN-disabled context.
     */
    pa = mmu_uvtop(va);
  }
  return pa;
...
pmap_paddr_t pmap_find_pa(pmap_t pmap, addr64_t va) {
...
ppnum_t pmap_find_phys_nofault(pmap_t pmap, addr64_t va) {
  ppnum_t ppn;
 ...
 */
pmap_paddr_t kvtophys(vm_offset_t va) {
  sptm_paddr_t pa;
...
pmap_paddr_t pmap_vtophys(pmap_t pmap, addr64_t va) {
  ...
  for (cur_level = pt_attr_root_level(pt_attr);
       cur_level <= pt_attr_leaf_level(pt_attr); cur_level++) {
 ...
 */
MARK_AS_PMAP_TEXT void pmap_init_pte_page(pmap_t pmap, pt_entry_t *pte_p,
                                          vm_offset_t va, unsigned int ttlevel,
                                          boolean_t alloc_ptd) {
  pt_desc_t *ptdp = NULL;
  ...
  if (pvh_test_type(pvh, PVH_TYPE_NULL)) {
    if (alloc_ptd) {
      /*
       * This path should only be invoked from arm_vm_init.  If we are emulating
       * 16KB pages on 4KB hardware, we may already have allocated a page table
       * descriptor for a bootstrap request, so we check for an existing PTD
       * here.
       */
 ...
 *       deleted by another thread before control returns to the caller, iff
 * that table is an ordinary userspace table.  Callers that use this function to
 * allocate new user L3 tables are therefore expected to keep calling this
 * function until they observe a successful L3 PTE lookup with the pmap lock
 * held.  As long as it does not drop the pmap lock, the caller may then safely
 * use the looked-up L3 table.  See the use of this function in
 ...
 *                allocated.  The only valid option is PMAP_OPTIONS_NOWAIT,
 * which specifies that the allocation must not block.
 ...
 */
MARK_AS_PMAP_TEXT static kern_return_t pmap_expand(pmap_t pmap,
                                                   vm_map_address_t vaddr,
                                                   unsigned int options,
                                                   unsigned int level) {
  __unused const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  ...
  for (; ttlevel < level; ttlevel++) {
    /**
     * If the previous iteration didn't allocate a new table, obtain the table
     * from the previous TTE. Doing this step at the beginning of the loop
     * instead of the end (which would make it part of the prior iteration)
     * avoids the possibility of executing this step to extract an L3 table KVA
     * from an L2 TTE, which would be useless because there would be no next
     * iteration to make use of the table KVA.
     */
    if (table_ttep == NULL) {
    ...
    /**
     * We don't need to hold the pmap lock while walking the paging hierarchy.
     * Only L3 tables are allowed to be dynamically removed, and only for
     * regular user pmaps at that.  We may allocate a new L3 table below, but we
     * will only access L0-L2 tables, so there's no risk of a table being
     * deleted while we are using it for the next level(s) of lookup.
     */
    ...
    if (!tte_is_valid_table(old_tte)) {
      ...
      pt_desc_t *new_ptdp;
      while (pmap_tt_allocate(pmap, &new_ttep, &new_ptdp, ttlevel + 1,
                              options | PMAP_PAGE_NOZEROFILL) != KERN_SUCCESS) {
      ...
      /**
       * Grab the lower-level table's PVH lock to ensure we don't try to
       * concurrently map different tables at the same TTE.
       */
      locked_pvh_t locked_pvh = pvh_lock(pa_index(table_pa));
      old_tte = os_atomic_load(ttep, relaxed);
      if (!tte_is_valid_table(old_tte)) {
        /**
         * This call must be issued prior to sptm_map_table() so that the page
         * table's PTD info is valid by the time the new table becomes visible
         * in the paging hierarchy. sptm_map_table() is expected to issue a
         * barrier that effectively guarantees the PTD update will be visible to
         * concurrent observers as soon as the new table becomes visible in the
         * paging hierarchy.
         */
         ...
         * upgrade its SPTM type to XNU_PAGE_TABLE_ROZONE.  The SPTM's type
         * system requires the table to be transitioned through XNU_DEFAULT for
         * refcount enforcement, which is fine since this path is expected to
         * execute only once during boot.
         */
        ...
        new_tte =
            (pa & ARM_TTE_TABLE_MASK) | ARM_TTE_TYPE_TABLE | ARM_TTE_VALID;
        sptm_map_table(pmap->ttep, v, (sptm_pt_level_t)ttlevel, new_tte);
        PMAP_TRACE(
            4 + ttlevel, PMAP_CODE(PMAP__TTE), VM_KERNEL_ADDRHIDE(pmap),
            VM_KERNEL_ADDRHIDE(v & ~pt_attr_ln_offmask(pt_attr, ttlevel)),
            VM_KERNEL_ADDRHIDE((v & ~pt_attr_ln_offmask(pt_attr, ttlevel)) +
                               pt_attr_ln_size(pt_attr, ttlevel)),
            new_tte);

        /**
         * Now that we've fully mapped the table, do final initialization of PTD
         * state, which includes dropping the wired count to allow future
         * reclamation of the page table page.
         */
        ptd_info_finalize(new_ptdp);

        table_pa = pa;
        /**
         * If we need to set up multiple TTEs mapping different parts of the
         * same page (e.g. because we're carving multiple 4K page tables out of
         * a 16K native page, determine which of the grouped TTEs is the one
         * that we need to follow for the next level of the table walk.
         */
        table_ttep = new_ttep + ((((uintptr_t)ttep / sizeof(tt_entry_t)) &
                                  table_align_mask) *
...
void pmap_gc(void) {
  /*
   * TODO: as far as I can tell this has never been implemented to do anything
   * meaninful. We can't just destroy any old pmap on the chance that it may be
   * active on a CPU or may contain wired mappings.  However, it may make sense
   * to scan the pmap VM object here, and for each page consult the SPTM frame
   * table and if necessary the PTD in the PV head table.  If the frame table
   * indicates the page is a leaf page table page and the PTD indicates it has
   * no wired mappings, we can call pmap_remove() on the VA region mapped by the
   * page and therein return the page to the VM.
   */
}
...
kern_return_t
pmap_attribute_cache_sync(ppnum_t pp, vm_size_t size,
 ...
 */
void pmap_sync_page_data_phys(__unused ppnum_t pp) { return; }
 ...
 */
void pmap_sync_page_attributes_phys(ppnum_t pp) {
  flush_dcache((vm_offset_t)(pp << PAGE_SHIFT), PAGE_SIZE, TRUE);
}

#if CONFIG_COREDUMP
/* temporary workaround */
boolean_t coredumpok(vm_map_t map, mach_vm_offset_t va) {
  pt_entry_t *pte_p;
  pt_entry_t spte;
...
void fillPage(ppnum_t pn, unsigned int fill) {
  unsigned int *addr;
  int count;
...
void mapping_set_ref(ppnum_t pn) { pmap_set_reference(pn); }
 ...
 */
MARK_AS_PMAP_TEXT static void
phys_attribute_clear_with_flush_range(ppnum_t pn, unsigned int bits,
  ...
  if (options & PMAP_OPTIONS_CLEAR_WRITE) {
    ...
    pmap_page_protect_options_with_flush_range(
        pn, (VM_PROT_ALL & ~VM_PROT_WRITE), options, NULL, flush_range);
    /*
  ...
  arm_force_fast_fault_with_flush_range(pn, allow_mode, options, NULL,
                                        (pp_attr_t)bits, flush_range);
}

MARK_AS_PMAP_TEXT void phys_attribute_clear_internal(ppnum_t pn,
                                                     unsigned int bits,
                                                     int options, void *arg) {
...
phys_attribute_clear_twig_internal(pmap_t pmap, vm_map_address_t start,
                                   vm_map_address_t end, unsigned int bits,
                                   unsigned int options,
                                   pmap_tlb_flush_range_t *flush_range) {
  ...
  /**
   * It's possible that this portion of our VA region has never been paged in,
   * in which case there may not be a valid twig or leaf table here.
   */
  ...
  for (curr_pte_p = start_pte_p; curr_pte_p < end_pte_p;
       curr_pte_p++, va += pmap_page_size) {
    if (flush_range->pending_region_entries == 0) {
    ...
    if (pte_is_valid(spte) && pa_valid(pa)) {
      /* The PTE maps a managed page, so do the appropriate PV list-based
       * permission changes. */
      const ppnum_t pn = (ppnum_t)atop(pa);
      phys_attribute_clear_with_flush_range(pn, bits, options, NULL,
                                            flush_range);
      if (__probable(flush_range->region_entry_added)) {
        flush_range->region_entry_added = false;
      } else {
        /**
         * It's possible that some other thread removed the mapping between our
         * check of the PTE above and taking the PVH lock in the
         * phys_attribute_clear_with_flush_range() path.  In that case we have a
         * discontinuity in the region to update, so just submit any pending
         * region templates and start a new region op on the next iteration.
         */
        pmap_multipage_op_submit_region(flush_range);
      }
    } else if (__improbable(!pte_is_valid(spte))) {
      /**
       * We've found an invalid mapping, so we have a discontinuity in the the
       * region to update.  Handle this by submitting any pending region
       * templates and starting a new region on the next iteration.  In theory
       * we could instead handle this by installing a "safe" (AF bit cleared,
       * minimal permissions) PTE template; the SPTM would just ignore the
       * update on finding an invalid mapping in the PTE.  But we don't know
       * what a "safe" template will be in all cases: for example, JIT regions
       * require all mappings to either be invalid or to have full RWX
       * permissions.
       */
      pmap_multipage_op_submit_region(flush_range);
    } else if (pmap_insert_flush_range_template(spte, flush_range)) {
      /**
       * We've found a mapping to a non-managed page, so just insert the
       * existing PTE into the pending region ops since we don't manage
       * attributes for non-managed pages. If pmap_insert_flush_range_template()
       * returns true, indicating that it reached the mapping limit and
       * submitted the SPTM call, then we also submit any pending disjoint ops.
       * Having pending operations in either category will keep preemption
    ...
    /**
     * If the total number of pending + processed entries exceeds the mapping
     * threshold, we may need to submit all pending operations to avoid
     * excessive preemption latency. Otherwise, a small number of pending
     * disjoint or region ops can hold preemption disabled across an arbitrary
     * number of total processed entries. As an optimization, we may be able to
     * avoid submitting if no urgent AST is pending on the local CPU, but only
     * if we aren't currently in an epoch.  If we are in an epoch, failure to
     * submit in a timely manner can cause another CPU to wait too long for our
     * epoch to drain.
     */
    if (((flush_range->processed_entries +
          flush_range->pending_disjoint_entries +
  ...
  /* SPTM region ops can't span L3 table boundaries, so submit any pending
   * region templates now. */
  pmap_multipage_op_submit_region(flush_range);
  return end;
}

MARK_AS_PMAP_TEXT vm_map_address_t phys_attribute_clear_range_internal(
    pmap_t pmap, vm_map_address_t start, vm_map_address_t end,
    unsigned int bits, unsigned int options) {
  if (__improbable(end < start)) {
    panic("%s: invalid address range %p, %p", __func__, (void *)start,
          (void *)end);
  }
  ...
  vm_map_address_t va = start;
  pmap_tlb_flush_range_t flush_range = {.ptfr_pmap = pmap,
                                        .ptfr_start = start,
                                        .ptfr_end = end,
                                        .current_ptep = NULL,
  ...
  while (va < end) {
    ...
    curr_end =
        ((va + pt_attr_twig_size(pt_attr)) & ~pt_attr_twig_offmask(pt_attr));
    if (curr_end > end) {
      curr_end = end;
    }

    va = phys_attribute_clear_twig_internal(pmap, va, curr_end, bits, options,
                                            &flush_range);
  }
  pmap_multipage_op_submit(&flush_range);
  pmap_unlock(pmap, PMAP_LOCK_SHARED);
  assert((flush_range.pending_disjoint_entries == 0) &&
         (flush_range.pending_region_entries == 0));
  if (flush_range.ptfr_flush_needed) {
    pmap_get_pt_ops(pmap)->flush_tlb_region_async(
        flush_range.ptfr_start, flush_range.ptfr_end - flush_range.ptfr_start,
        flush_range.ptfr_pmap, true);
    sync_tlb_flush();
  }
  return va;
}

static void phys_attribute_clear_range(pmap_t pmap, vm_map_address_t start,
                                       vm_map_address_t end, unsigned int bits,
                                       unsigned int options) {
  /*
   * We allow single-page requests to execute non-preemptibly,
   ...
   */
  if ((end - start) >
      (pt_attr_page_size(pmap_get_pt_attr(pmap)) * PAGE_RATIO)) {
    pmap_verify_preemptible();
  }
...
static void phys_attribute_clear(ppnum_t pn, unsigned int bits, int options,
                                 void *arg) {
  /*
 ...
 *	modify bits.
 */
MARK_AS_PMAP_TEXT void phys_attribute_set_internal(ppnum_t pn,
                                                   unsigned int bits) {
  pmap_paddr_t pa = ptoa(pn);
  assert(pn != vm_page_fictitious_addr);
...
static void phys_attribute_set(ppnum_t pn, unsigned int bits) {
 ...
 */
static boolean_t phys_attribute_test(ppnum_t pn, unsigned int bits) {
  pmap_paddr_t pa = ptoa(pn);
  assert(pn != vm_page_fictitious_addr);
 ...
 */
void pmap_set_modify(ppnum_t pn) { phys_attribute_set(pn, PP_ATTR_MODIFIED); }
 ...
 */
boolean_t pmap_is_modified(ppnum_t pn) {
 ...
 *	by any physical maps.
 */
boolean_t pmap_is_referenced(ppnum_t pn) {
...
unsigned int pmap_get_refmod(ppnum_t pn) {
  return ((phys_attribute_test(pn, PP_ATTR_MODIFIED)) ? VM_MEM_MODIFIED : 0) |
         ((phys_attribute_test(pn, PP_ATTR_REFERENCED)) ? VM_MEM_REFERENCED
                                                        : 0);
}
 ...
 */
void pmap_clear_refmod_options(ppnum_t pn, unsigned int mask,
                               unsigned int options, void *arg) {
  unsigned int bits;
...
unsigned int pmap_disconnect_options(ppnum_t pn, unsigned int options,
                                     void *arg) {
 ...
 *
 *	Function:
 *		Disconnect all mappings for this page and return reference and
 * change status in generic format.
 *
 */
unsigned int pmap_disconnect(ppnum_t pn) {
  pmap_page_protect(pn, 0);   /* disconnect the page */
  return pmap_get_refmod(pn); /* return ref/chg status */
}

boolean_t pmap_has_managed_page(ppnum_t first, ppnum_t last) {
...
void pmap_lock_phys_page(ppnum_t pn) {
  unsigned int pai;
...
void pmap_unlock_phys_page(ppnum_t pn) {
  unsigned int pai;
  pmap_paddr_t phys = ptoa(pn);
...
MARK_AS_PMAP_TEXT void pmap_clear_user_ttb_internal(void) {
  set_mmu_ttb(invalid_ttep & TTBR_BADDR_MASK);
}

void pmap_clear_user_ttb(void) {
 ...
 *                   fault.
 *                   VM_PROT_READ will only cause the next write access to
 * fault. Other values are undefined.
 * @param options PMAP_OPTIONS_NOFLUSH indicates TLBI flush is not needed.
 ...
 * updated to reflect a new PV head value to be passed to a later call to
 * pvh_unlock().
 * @param bits_to_clear Mask of additional pp_attr_t bits to clear for the
 * physical page, iff this function completes successfully and returns TRUE.
 * This is typically some combination of the referenced, modified, and noencrypt
 * bits.
 * @param flush_range When present, this function will skip the TLB flush for
 * the mappings that are covered by the range, leaving that to be done later by
 * the caller.  It may also avoid submitting mapping updates directly to the
 ...
 *          EL0.  The kernel may assume that accesses to wired, kernel-owned
 * pages won't fault.
 */
MARK_AS_PMAP_TEXT static boolean_t arm_force_fast_fault_with_flush_range(
    ppnum_t ppnum, vm_prot_t allow_mode, int options, locked_pvh_t *locked_pvh,
...
#define FFF_PERCPU_INIT()                                                      \
  ...
  while ((pve_p != PV_ENTRY_NULL) || (pte_p != PT_ENTRY_NULL)) {
    ...
    if (__improbable(pvh_lock_sleep_mode_needed)) {
      ...
      /**
       * Undo the explicit preemption disable done in the last call to
       * FFF_PER_CPU_INIT(). If the PVH lock is placed in sleep mode, we can't
       * rely on it to disable preemption, so we need these explicit preemption
       * twiddles to ensure we don't get migrated off- core while processing
       * SPTM per-CPU data.  At the same time, we also want preemption to
       * briefly be re-enabled every SPTM_MAPPING_LIMIT mappings so that any
    ...
    bool skip_pte =
        pte_is_wired(spte) && ((options & PMAP_OPTIONS_FF_WIRED) == 0);
    ...
    if (is_altacct) {
       ...
       */
    } else if ((options & PMAP_OPTIONS_CLEAR_REUSABLE) && is_reusable &&
               is_internal && pmap != kernel_pmap) {
      /* one less "reusable" */
      pmap_ledger_debit(pmap, task_ledgers.reusable,
                        pt_attr_page_size(pt_attr) * PAGE_RATIO);
      /* one more "internal" */
  ...
  if (num_mappings != 0) {
    ...
    if (flush_range == NULL) {
      sptm_ret = sptm_update_disjoint(phys, sptm_pcpu->sptm_ops_pa,
                                      num_mappings, sptm_update_options);
    } else {
      /* Resync the pending mapping state in flush_range with our local state.
       */
      assert(num_mappings >= flush_range->pending_disjoint_entries);
  ...
  if (clear_write_fault && !ref_aliases_mod) {
    arm_clear_fast_fault(ppnum, VM_PROT_WRITE, local_locked_pvh.pvh,
                         PT_ENTRY_NULL, 0);
  }
...
MARK_AS_PMAP_TEXT boolean_t arm_force_fast_fault_internal(ppnum_t ppnum,
                                                          vm_prot_t allow_mode,
                                                          int options) {
  if (__improbable((options & (PMAP_OPTIONS_FF_LOCKED | PMAP_OPTIONS_FF_WIRED |
                               PMAP_OPTIONS_NOFLUSH)) != 0)) {
    panic("arm_force_fast_fault(0x%x, 0x%x, 0x%x): invalid options", ppnum,
          allow_mode, options);
  }
...
boolean_t arm_force_fast_fault(ppnum_t ppnum, vm_prot_t allow_mode, int options,
                               __unused void *arg) {
  pmap_paddr_t phys = ptoa(ppnum);
 ...
 * @param attrs_to_clear Mask of additional pp_attr_t bits to clear for the
 * physical page upon completion of this function.  This is typically some
 * combination of the REFFAULT and MODFAULT bits.
 *
 * @return TRUE if any PTEs were modified, FALSE otherwise.
 */
MARK_AS_PMAP_TEXT static boolean_t
arm_clear_fast_fault(ppnum_t ppnum, vm_prot_t fault_type, uintptr_t pvh,
                     pt_entry_t *pte_p, pp_attr_t attrs_to_clear) {
  const pmap_paddr_t pa = ptoa(ppnum);
  pv_entry_t *pve_p;
  boolean_t result;
  unsigned int num_mappings = 0, num_skipped_mappings = 0;
  ...
  if (pte_p == PT_ENTRY_NULL) {
    if (pvh_test_type(pvh, PVH_TYPE_PTEP)) {
      ...
      panic("%s: invalid PV head 0x%llx for PA 0x%llx", __func__, (uint64_t)pvh,
            (uint64_t)pa);
    }
  }
  ...
  cff_skip_pve:
    if ((num_mappings + num_skipped_mappings) == SPTM_MAPPING_LIMIT) {
      ...
      /*
       * We've reached the limit of mappings that can be processed in a single
       * arm_clear_fast_fault() call.  Bail out here to avoid excessive PVH lock
       * duration on the fault path.  If a fault is subsequently taken on a
       * mapping we haven't processed, arm_fast_fault() will call this function
       * with a non-NULL pte_p to perform a targeted fixup.
       */
      break;
    }
...
arm_fast_fault_internal(pmap_t pmap, vm_map_address_t va, vm_prot_t fault_type,
  ...
  if (__probable(preemption_enabled())) {
    ...
    /**
     * In certain cases, arm_fast_fault() may be invoked with preemption
     * disabled on the copyio path.  In theses cases the (in-kernel) caller
     * expects that any faults taken against the user address may not be handled
     * successfully (vm_fault() allows non-preemptible callers with the
     * possibility that the fault may not be successfully handled) and will
     * result in the copyio operation returning EFAULT.  It is then the caller's
     * responsibility to retry the copyio operation in a preemptible context.
     *
  ...
  if ((attrs & PP_ATTR_REFFAULT) ||
      ((fault_type & VM_PROT_WRITE) && (attrs & PP_ATTR_MODFAULT))) {
    /*
    ...
    if (arm_clear_fast_fault((ppnum_t)atop(pa), fault_type, locked_pvh.pvh,
                             PT_ENTRY_NULL, attrs_to_clear)) {
      /*
  ...
  /*
   * If the PTE already has sufficient permissions, we can report the fault as
   * handled. This may happen, for example, if multiple threads trigger roughly
   * simultaneous faults on mappings of the same page
   */
  ...
  if ((result == KERN_FAILURE) &&
      arm_clear_fast_fault((ppnum_t)atop(pa), fault_type, locked_pvh.pvh, ptep,
                           0)) {
    /*
     ...
     * Attempt a targeted fixup of the PTE that caused the fault to avoid
     * repeatedly taking a fault on the same mapping.
     */
    result = KERN_SUCCESS;
  }
...
kern_return_t arm_fast_fault(pmap_t pmap, vm_map_address_t va,
                             vm_prot_t fault_type, bool was_af_fault,
  ...
  result =
      arm_fast_fault_internal(pmap, va, fault_type, was_af_fault, from_user);
...
void pmap_copy_page(ppnum_t psrc, ppnum_t pdst, int options) {
  bcopy_phys_with_options((addr64_t)(ptoa(psrc)), (addr64_t)(ptoa(pdst)),
                          PAGE_SIZE, options);
}
 ...
 */
void pmap_copy_part_page(ppnum_t psrc, vm_offset_t src_offset, ppnum_t pdst,
                         vm_offset_t dst_offset, vm_size_t len) {
  bcopy_phys((addr64_t)(ptoa(psrc) + src_offset),
             (addr64_t)(ptoa(pdst) + dst_offset), len);
}
 ...
 */
void pmap_zero_page_with_options(ppnum_t pn, int options) {
  assert(pn != vm_page_fictitious_addr);
...
void pmap_map_globals(void) {
  pt_entry_t pte;

  pte = pa_to_pte(kvtophys_nofail((vm_offset_t)&lowGlo)) | AP_RONA |
        ARM_PTE_NX | ARM_PTE_PNX | ARM_PTE_AF | ARM_PTE_TYPE_VALID;
#if __ARM_KERNEL_PROTECT__
  pte |= ARM_PTE_NG;
#endif /* __ARM_KERNEL_PROTECT__ */
  pte |= ARM_PTE_ATTRINDX(CACHE_ATTRINDX_WRITEBACK);
...
vm_offset_t pmap_cpu_windows_copy_addr(int cpu_num, unsigned int index) {
  if (__improbable(index >= CPUWINDOWS_MAX)) {
    panic("%s: invalid index %u", __func__, index);
  }
  return (vm_offset_t)(CPUWINDOWS_BASE +
                       (PAGE_SIZE * ((CPUWINDOWS_MAX * cpu_num) + index)));
}

MARK_AS_PMAP_TEXT unsigned int
pmap_map_cpu_windows_copy_internal(ppnum_t pn, vm_prot_t prot,
                                   unsigned int wimg_bits) {
  ...
  for (cpu_window_index = 0; cpu_window_index < CPUWINDOWS_MAX;
       cpu_window_index++) {
    cpu_copywindow_vaddr =
        pmap_cpu_windows_copy_addr(cpu_num, cpu_window_index);
  ...
  const pmap_paddr_t paddr = ptoa(pn);
  pte = pa_to_pte(paddr) | ARM_PTE_TYPE_VALID | ARM_PTE_AF | ARM_PTE_NX |
        ARM_PTE_PNX;
#if __ARM_KERNEL_PROTECT__
  pte |= ARM_PTE_NG;
#endif /* __ARM_KERNEL_PROTECT__ */
  ...
  /*
   * It's expected to be safe for an interrupt handler to nest copy-window usage
   * with the active thread on a CPU, as long as a sufficient number of copy
   * windows are available.
   * --If the interrupt handler executes before the active thread creates the
   * per-CPU mapping, or after the active thread completely removes the mapping,
   * it may use the same mapping but will finish execution and tear down the
   * mapping without the thread needing to know.
   * --If the interrupt handler executes after the active thread creates the
   * per-CPU mapping, it will observe the valid mapping and use a different copy
   * window.
   * --If the interrupt handler executes after the active thread clears the PTE
   * in pmap_unmap_cpu_windows_copy() but before the active thread flushes the
   * TLB, the code for computing cpu_window_index above will observe the
   * PTE_INVALID_IN_FLIGHT token set by the SPTM, and will select a different
   * index.
   */
  const sptm_return_t sptm_status =
      sptm_map_page(kernel_pmap->ttep, cpu_copywindow_vaddr, pte);
  if (__improbable(sptm_status != SPTM_SUCCESS)) {
  ...
  /*
   * Clean up any pending strong TLB flush for the same window in a thread we
...
unsigned int pmap_map_cpu_windows_copy(ppnum_t pn, vm_prot_t prot,
                                       unsigned int wimg_bits) {
  return pmap_map_cpu_windows_copy_internal(pn, prot, wimg_bits);
}

MARK_AS_PMAP_TEXT void
pmap_unmap_cpu_windows_copy_internal(unsigned int index) {
  unsigned int cpu_num;
  vm_offset_t cpu_copywindow_vaddr = 0;
  pmap_cpu_data_t *pmap_cpu_data = pmap_get_cpu_data();
...
void pmap_unmap_cpu_windows_copy(unsigned int index) {
 ...
 */
MARK_AS_PMAP_TEXT void pmap_set_nested_internal(pmap_t pmap) {
  validate_pmap_mutable(pmap);
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  if (__improbable(pmap->type != PMAP_TYPE_USER)) {
    panic("%s: attempt to nest unsupported pmap %p of type 0x%hhx", __func__,
          pmap, pmap->type);
  }
  pmap->type = PMAP_TYPE_NESTED;
  sptm_retype_params_t retype_params = {.raw = SPTM_RETYPE_PARAMS_NULL};
  retype_params.attr_idx = (pt_attr_page_size(pt_attr) == 4096)
                               ? SPTM_PT_GEOMETRY_4K
                               : SPTM_PT_GEOMETRY_16K;
  pmap_txm_acquire_exclusive_lock(pmap);
  sptm_retype(pmap->ttep, XNU_USER_ROOT_TABLE, XNU_SHARED_ROOT_TABLE,
              retype_params);
  pmap_txm_release_exclusive_lock(pmap);
  pmap_get_pt_ops(pmap)->free_id(pmap);
}

void pmap_set_nested(pmap_t pmap) { pmap_set_nested_internal(pmap); }

bool pmap_is_nested(pmap_t pmap) { return pmap->type == PMAP_TYPE_NESTED; }
 ...
 */
MARK_AS_PMAP_TEXT static void pmap_trim_range(pmap_t pmap, addr64_t start,
                                              addr64_t end) {
  addr64_t cur;
  ...
  for (cur = adjusted_start; (cur < adjusted_end) && (cur >= adjusted_start);
       cur += (pt_attr_twig_size(pt_attr) * page_ratio)) {
    tte_p = pmap_tte(pmap, cur);

    if ((tte_p != NULL) && tte_is_valid_table(*tte_p)) {
      if ((pmap->type == PMAP_TYPE_NESTED) &&
          (sptm_get_page_table_refcnt(tte_to_pa(*tte_p)) == 0)) {
        ...
        panic("%s: Unsupported pmap type for nesting %p %d", __func__, pmap,
              pmap->type);
      }
    }
  }
 ...
 * a temporary [grand] pmap, and that it happens before [subord] is ever
 * actually nested in a real task pmap.  Unlike its PPL predecessor (which can't
 * trust its callers), the SPTM implementation therefore does not do any
 * refcounting to track top-level pmaps that may have nested tables outside the
 * trimmed range.
 */
MARK_AS_PMAP_TEXT void pmap_trim_internal(pmap_t grand, pmap_t subord,
                                          addr64_t vstart, uint64_t size) {
  addr64_t vend;
  addr64_t adjust_offmask;
  ...
  if (__improbable(
          (vstart < grand->nested_region_addr) ||
  ...
  os_atomic_store(&subord->nested_region_true_start, vstart & ~adjust_offmask,
                  relaxed);
  ...
  os_atomic_store(&subord->nested_region_true_end, true_end & ~adjust_offmask,
                  relaxed);

  os_atomic_store(&grand->nested_region_true_start,
                  subord->nested_region_true_start, relaxed);
  os_atomic_store(&grand->nested_region_true_end,
                  subord->nested_region_true_end, relaxed);
  /* Trim grand to only cover the given range. */
  pmap_trim_range(grand, grand->nested_region_addr,
                  grand->nested_region_true_start);
  pmap_trim_range(grand, grand->nested_region_true_end,
                  (grand->nested_region_addr + grand->nested_region_size));
  pmap_trim_range(subord, subord->nested_region_addr,
                  subord->nested_region_true_start);
  pmap_trim_range(subord, subord->nested_region_true_end,
                  subord->nested_region_addr + subord->nested_region_size);
}

void pmap_trim(pmap_t grand, pmap_t subord, addr64_t vstart, uint64_t size) {
  pmap_trim_internal(grand, subord, vstart, size);
}

#if HAS_APPLE_PAC

void *pmap_sign_user_ptr(void *value, ptrauth_key key, uint64_t discriminator,
                         uint64_t jop_key) {
  void *res = NULL;
...
static pmap_batch_sign_user_ptr_state_t
    PERCPU_DATA(percpu_pmap_batch_sign_user_ptr_state);
 ...
 */
bool pmap_batch_sign_user_ptr(void *location, void *value, ptrauth_key key,
                              uint64_t discriminator, uint64_t jop_key) {
  bool submitted_to_sptm = false;

  /* Disable preemption to access percpu data. */
  disable_preemption();

  pmap_batch_sign_user_ptr_state_t *state =
      PERCPU_GET(percpu_pmap_batch_sign_user_ptr_state);
  void **locations = state->locations;
  pmap_sptm_percpu_data_t *sptm_pcpu = PERCPU_GET(pmap_sptm_percpu);
  sptm_user_pointer_op_t *sptm_user_pointer_ops =
      (sptm_user_pointer_op_t *)sptm_pcpu->sptm_user_pointer_ops;
  uintptr_t *sptm_values = (uintptr_t *)sptm_pcpu->sptm_prev_ptes;

  if (state->index != 0) {
    /* Avoid leaking preemption counts by offsetting the disable at the
  ...
  if (state->index == SPTM_BATCHED_OPS_LIMIT || location == NULL) {
    if (__probable(state->index > 0)) {
      ...
      sptm_batch_sign_user_pointer(sptm_pcpu->sptm_user_pointer_ops_pa,
                                   state->index, state->jop_key);
...
void *pmap_auth_user_ptr(void *value, ptrauth_key key, uint64_t discriminator,
                         uint64_t jop_key) {
  void *res = NULL;
  const boolean_t current_intr_state = ml_set_interrupts_enabled(FALSE);
 ...
 */
void pmap_set_shared_region(pmap_t grand, pmap_t subord, addr64_t vstart,
                            uint64_t size) {
  addr64_t vend;

  PMAP_TRACE(2, PMAP_CODE(PMAP__SET_SHARED_REGION) | DBG_FUNC_START,
             VM_KERNEL_ADDRHIDE(grand), VM_KERNEL_ADDRHIDE(subord), vstart,
             size);

  if (__improbable(os_add_overflow(vstart, size, &vend))) {
    panic("%s: %p grand addr wraps around: 0x%llx + 0x%llx", __func__, grand,
          vstart, size);
  }
  ...
  if (__improbable(pmap_get_pt_attr(subord) != pt_attr)) {
    panic("%s: attempt to nest pmap %p into pmap %p with mismatched attributes",
          __func__, subord, grand);
  }

  if (__improbable(((size | vstart) & (pt_attr_leaf_table_offmask(pt_attr))) !=
                   0x0ULL)) {
    panic("%s: pmap %p unaligned nesting request 0x%llx, 0x%llx", __func__,
          grand, vstart, size);
  }

  if (__improbable(subord->type != PMAP_TYPE_NESTED)) {
    panic("%s: subordinate pmap %p is of non-nestable type 0x%hhx", __func__,
          subord, subord->type);
  }

  if (__improbable(grand->type != PMAP_TYPE_USER)) {
    panic("%s: grand pmap %p is of unsupported type 0x%hhx for nesting",
          __func__, grand, grand->type);
  }

  if (subord->nested_region_size == 0) {
    /**
     * Since subord->nested_region_size is 0, this is the first time subord is
     * being associated with a top-level pmap.  We therefore need to take a few
     * extra steps to ensure the shared region is properly configured.  This
     * initial setup step is expected to be issued by the VM layer against a
     * temporary grand pmap before any other pmap is allowed to associate with
     * subord, so synchronization is not needed here to prevent concurrent
     * initialization.
     */
    sptm_configure_shared_region(subord->ttep, vstart,
                                 size >> pt_attr->pta_page_shift);

    /**
     * Since this is the first time subord is being associated with a top-level
     * pmap, ensure its nested region is fully expanded to L3 so that all
     * relevant L3 tables can later be inserted into top-level pmaps via
     * pmap_nest().  Note that pmap_remove() will never dynamically free L3
     * tables from nested pmaps.  However, some of these tables may be freed by
     * a later call to pmap_trim().
     */
    ...
    const uint64_t nested_region_unnested_table_bits =
        (size >> (pt_attr_twig_shift(pt_attr) - 1));
    if (__improbable((nested_region_unnested_table_bits > UINT_MAX))) {
      panic("%s: bitmap allocation size %llu will truncate, "
            ...
            __func__, nested_region_unnested_table_bits, grand, subord, vstart,
            size);
    }

    subord->nested_region_unnested_table_bitmap =
        bitmap_alloc((uint)nested_region_unnested_table_bits);
  ...
  if (os_atomic_cmpxchg(&grand->nested_pmap, PMAP_NULL, subord, relaxed)) {
    ...
    panic("%s: pmap %p already has a nested pmap %p", __func__, grand,
          grand->nested_pmap);
  }
 ...
 * @param vstart twig-aligned virtual address for the beginning of the nesting
 * range
 ...
 */
MARK_AS_PMAP_TEXT kern_return_t pmap_nest_internal(pmap_t grand, pmap_t subord,
                                                   addr64_t vstart,
                                                   uint64_t size) {
  kern_return_t kr = KERN_SUCCESS;
  ...
  if (__improbable(subord != grand->nested_pmap)) {
    panic("%s: attempt to nest pmap %p into pmap %p which has a different "
          "nested pmap %p",
  ...
  /* Ensure grand is expanded to L2 so that sptm_nest_region() can copy L3
   * entries from subord. */
  ...
  while (vaddr < true_end) {
    /*
     * The SPTM requires the run of TTE updates to all reside within the same L2
     * page, so the region we supply to the SPTM can't span multiple L1 TTEs.
     */
    vm_map_offset_t vlim =
        ((vaddr + pt_attr_ln_size(pt_attr, PMAP_TT_L1_LEVEL)) &
         ~pt_attr_ln_offmask(pt_attr, PMAP_TT_L1_LEVEL));
    if (vlim > true_end) {
      vlim = true_end;
    }
    sptm_nest_region(grand->ttep, subord->ttep, vaddr,
                     (vlim - vaddr) >> pt_attr->pta_page_shift);
    vaddr = vlim;
  }
...
kern_return_t pmap_nest(pmap_t grand, pmap_t subord, addr64_t vstart,
                        uint64_t size) {
  kern_return_t kr = KERN_SUCCESS;
 ...
 * with the assumption that they will be demand-filled by subsequent access
 * faults.
 *
 * This function operates in 2 main phases:
 * 1. Iteration over the nested pmap's mappings for the specified range to mark
 *    them non-global.
 * 2. Calling the SPTM to clear the twig-level TTEs for the address range in
 * grand.
 *
 * @param grand pmap from which to unnest mappings
 * @param vaddr twig-aligned virtual address for the beginning of the nested
 * range
 * @param size twig-aligned size of the nested range
 * @param option Extra control flags; may contain PMAP_UNNEST_CLEAN to indicate
 * that grand is being torn down and step 1) above is not needed.
 */
MARK_AS_PMAP_TEXT void pmap_unnest_options_internal(pmap_t grand,
                                                    addr64_t vaddr,
                                                    uint64_t size,
                                                    unsigned int option) {
  ...
  if (__improbable(os_add_overflow(vaddr, size, &vend))) {
    panic("%s: %p vaddr wraps around: 0x%llx + 0x%llx", __func__, grand, vaddr,
          size);
  }
  ...
  if (__improbable(((size | vaddr) & pt_attr_twig_offmask(pt_attr)) !=
                   0x0ULL)) {
  ...
  if ((option & PMAP_UNNEST_CLEAN) == 0) {
    if ((vaddr < grand->nested_region_addr) ||
        (vend > (grand->nested_region_addr + grand->nested_region_size))) {
      panic("%s: %p: unnest request to not-fully-nested region [%p, %p)",
            __func__, grand, (void *)vaddr, (void *)vend);
    }
    ...
    start_index = (unsigned int)((start - grand->nested_region_addr) >>
                                 pt_attr_twig_shift(pt_attr));
    max_index = (unsigned int)((true_end - grand->nested_region_addr) >>
                               pt_attr_twig_shift(pt_attr));

    for (current_index = start_index, addr = start; current_index < max_index;
         current_index++) {
      vm_map_offset_t vlim =
          (addr + pt_attr_twig_size(pt_attr)) & ~pt_attr_twig_offmask(pt_attr);

      bool unnested = bitmap_test(subord->nested_region_unnested_table_bitmap,
                                  UNNEST_BIT(current_index));
      os_atomic_thread_fence(acquire);
      if (!unnested) {
        atomic_bitmap_set(
            (_Atomic bitmap_t *)subord->nested_region_unnested_table_bitmap,
            UNNEST_IN_PROGRESS_BIT(current_index), memory_order_relaxed);
        /*
         * Issue a store-load barrier to ensure the UNNEST_IN_PROGRESS bit is
         * visible to any pmap_enter() operation that enters the epoch after
         * this point.
         */
        ...
        pmap_sptm_percpu_data_t *sptm_pcpu = PERCPU_GET(pmap_sptm_percpu);
        /*
         * We've marked the 'twig' region as being unnested.  Every mapping
         * entered within the nested pmap in this region will now be marked
         * non-global.
         */
        while (addr < vlim) {
          ...
          if (num_mappings == SPTM_MAPPING_LIMIT) {
             ...
             * be concurrently trimmed, so re-check the bounds within the epoch
             * to avoid potentially issuing an SPTM operation against a deleted
             * leaf page table.  This assumes the following: 1) The pmap_trim()
             * code path always issues a barrier and an epoch drain in between
             * updating subord's true bounds and actually trimming subord,
             * effectively purging any operation here which may be using stale
             * bounds. 2) The true bounds, if set, will always be twig-aligned,
             * thus the region we operate on here can never span the starting or
             *    ending bounds.
             */
            if ((start >= subord->nested_region_true_start) &&
        ...
        /**
         * The SPTM does not allow region updates to span multiple leaf page
         * tables, so request any remaining updates up to vlim before moving to
         * the next page table page.
         */
        if (num_mappings != 0) {
          pmap_epoch_enter();
          if ((start >= subord->nested_region_true_start) &&
  ...
  while (addr < true_end) {
    vm_map_offset_t vlim =
        ((addr + pt_attr_ln_size(pt_attr, PMAP_TT_L1_LEVEL)) &
         ~pt_attr_ln_offmask(pt_attr, PMAP_TT_L1_LEVEL));
    if (vlim > true_end) {
      vlim = true_end;
    }
    sptm_unnest_region(grand->ttep, subord->ttep, addr,
                       (vlim - addr) >> pt_attr->pta_page_shift);
    addr = vlim;
  }
}

kern_return_t pmap_unnest_options(pmap_t grand, addr64_t vaddr, uint64_t size,
                                  unsigned int option) {
...
boolean_t pmap_adjust_unnest_parameters(__unused pmap_t p,
 ...
 */
kern_return_t pmap_fork_nest(pmap_t old_pmap, pmap_t new_pmap) {
  ...
  pmap_set_shared_region(new_pmap, old_pmap->nested_pmap,
 ...
 */
#if DEVELOPMENT || DEBUG
void pmap_disable_NX(pmap_t pmap) { pmap->nx_enabled = FALSE; }
#else
void pmap_disable_NX(__unused pmap_t pmap) {}
#endif
...
void flush_mmu_tlb_region(vm_offset_t va, unsigned length) {
...
unsigned int pmap_cache_attributes(ppnum_t pn) {
  ...
  if (!pa_valid(paddr)) {
    ...
    return (io_rgn == NULL || io_rgn->signature == 'SKIO') ? VM_WIMG_IO
                                                           : io_rgn->wimg;
  }
...
MARK_AS_PMAP_TEXT static void pmap_sync_wimg(ppnum_t pn,
  ...
  if ((wimg_bits_prev != wimg_bits_new) &&
      ((wimg_bits_prev == VM_WIMG_COPYBACK) ||
       ...
       ((wimg_bits_prev == VM_WIMG_WTHRU) &&
        ((wimg_bits_new != VM_WIMG_COPYBACK) ||
         (wimg_bits_new != VM_WIMG_INNERWBACK))))) {
...
pmap_update_compressor_page_internal(ppnum_t pn, unsigned int prev_cacheattr,
                                     unsigned int new_cacheattr) {
  ...
  pmap_sync_wimg(pn, prev_cacheattr & VM_WIMG_MASK,
                 new_cacheattr & VM_WIMG_MASK);
}

static inline bool cacheattr_supports_compressor(unsigned int cacheattr) {
  switch (cacheattr) {
...
void *pmap_map_compressor_page(ppnum_t pn) {
...
void pmap_unmap_compressor_page(ppnum_t pn __unused, void *kva __unused) {
  unsigned int cacheattr = pmap_cache_attributes(pn) & VM_WIMG_MASK;
  if (!cacheattr_supports_compressor(cacheattr)) {
 ...
 * issue barriers yet.
 *
 * @param paddr The physical address to be flushed from TLB. Must be a managed
 * address.
 */
static void pmap_flush_tlb_for_paddr_async(pmap_paddr_t paddr) {
  ...
  while ((pve_p != PV_ENTRY_NULL) || (pte_p != PT_ENTRY_NULL)) {
    ...
    pmap_get_pt_ops(pmap)->flush_tlb_region_async(
        va, pt_attr_page_size(pmap_get_pt_attr(pmap)) * PAGE_RATIO, pmap, true);
...
pmap_update_pp_attr_wimg_bits_locked(unsigned int pai, unsigned int cacheattr) {
  ...
  do {
    pp_attr_current = pp_attr_table[pai];
    pp_attr_template =
        (pp_attr_current & ~PP_ATTR_WIMG_MASK) | PP_ATTR_WIMG(cacheattr);

    /**
     * WIMG bits should only be updated under the PVH lock, but we should do
     * this in a CAS loop to avoid losing simultaneous updates to other bits
     * like refmod.
     */
  } while (!OSCompareAndSwap16(pp_attr_current, pp_attr_template,
                               &pp_attr_table[pai]));
}

/**
 * Structure for tracking where we are during the collection of mappings for
 * batch cache attribute updates.
 *
 * @note We need to track where in the per-cpu ops table we are filling the next
 * mappings into, because the collection routine can return with a not
 * completely filled ops table when it exhausts the PV list for a page. In such
 * case, the remaining slots in the ops table will be used for mappings of the
 * next page.
 *
 * @note We also need to record where we are in the PV list, because the
 * collection routine can also return when the ops table is filled but it's
 * still in the middle of the PV list. Those remaining items in the PV list need
 * to be handled by the next batch operation in a new ops table.
 */
typedef struct {
  ...
  /**
   * The last collected physical address from the previous full ops array (and
   * in turn, SPTM call). This is used to know whether the SPTM call for the
   * latest full ops table should skip updating the PAPT mapping (seeing as the
   * last call would have handled updating it).
   */
   ...
   *
   * When ptep is non-null, there's only one mapping to the page and the ptep is
   * the address of it.
   *
   * When pvep is non-null, there's more than one mapping and the mappings are
   * tracked by the PV list.
   *
   * When they are both null, it indicates we are collecting for a new page and
   * the collection function will initialize them to be one of the two states
   * above.
   *
 ...
 *         False otherwise.
 */
static inline bool pmap_is_sptm_update_cache_attr_ops_pending(
    pmap_sptm_update_cache_attr_ops_collect_state_t state) {
  return state.sptm_ops_index > 0;
}

/**
 * Struct for encoding the collection status into
 * pmap_sptm_update_cache_attr_ops_collect()'s return value indicating what kind
...
/**
 * Collects mappings of a physical page into an SPTM ops table for cache
 * attribute updates.
 *
 * @note This routine returns either when the ops table is full or the page
 * represented by pa has no more mapping to collect. The caller should call this
 * routine again with a fresh ops table, or a new page, or both, depending on
 * the return code.
 *
 * @note The PVH lock needs to be held for pa.
 *
 * @param state Tracks the state of PV list traversal and SPTM ops table
 * filling. It is used by this routine to save the progress of the collection.
 ...
 *
 * @return A pmap_sptm_update_cache_attr_ops_collect_return_t that encodes what
 * the caller should do before calling this routine again. See the inline
 * comments around pmap_sptm_update_cache_attr_ops_collect_return_t for details.
 */
static pmap_sptm_update_cache_attr_ops_collect_return_t
pmap_sptm_update_cache_attr_ops_collect(
    pmap_sptm_update_cache_attr_ops_collect_state_t *state,
    sptm_update_disjoint_multipage_op_t *sptm_ops, pmap_paddr_t pa,
    unsigned int attributes) {
  if (state == NULL || sptm_ops == NULL) {
    panic("%s: unexpected null arguments - state: %p, sptm_ops: %p", __func__,
          state, sptm_ops);
  }

  PMAP_TRACE(2, PMAP_CODE(PMAP__COLLECT_CACHE_OPS) | DBG_FUNC_START, pa,
             attributes, state->sptm_ops_index);
  ...
  /* If pvep and ptep are both null in the state, it's a new page. Initialize
   * the states. */
   ...
   *
   * 1) In the case of a fresh ops table, the first entry has to be a PAPT
   * header. 2) In the case of a fresh page, we need to insert a new PAPT header
   * to request SPTM to operate on a new page.
   *
  ...
  sptm_ops[papt_sptm_ops_index].per_paddr_header.options =
      SPTM_UPDATE_SH | SPTM_UPDATE_MAIR | SPTM_UPDATE_DEFER_TLBI;

  if ((papt_sptm_ops_index == 0) && (pa == last_table_last_papt_pa)) {
    /**
     * If the previous SPTM call was made with an ops table that already
     * included updating the PA of the page that this table starts with, then we
     * can assume that call already updated the PAPT and we can safely skip it
  ...
  /**
   * Main loop for collecting the mappings into the ops table. It terminates
   * either when the ops table is full or the PV list is exhausted.
   */
  while ((sptm_ops_index < SPTM_MAPPING_LIMIT) &&
         (pvep != PV_ENTRY_NULL || ptep != PT_ENTRY_NULL)) {
    /**
  ...
  const bool collection_done_for_page =
      (pvep == PV_ENTRY_NULL && ptep == PT_ENTRY_NULL);

  /**
   * The ops table is full, so the caller should now invoke the SPTM before
   * calling into this function again.
   */
  if (full_table) {
  ...
  PMAP_TRACE(2, PMAP_CODE(PMAP__COLLECT_CACHE_OPS) | DBG_FUNC_END, pa,
             attributes, sptm_ops_index);
 ...
 *         False otherwise.
 */
static bool pmap_is_cache_attribute_allowed(unsigned int attributes) {
 ...
 *
 * In pass one, the pp_attr_table and the pte are updated (by SPTM) for the
 * pages in the list. In pass two, TLB entries are flushed for each page in the
 * list if necessary. In pass three, caches are cleaned for each page in the
 * list if necessary.
 *
 * @param page_list List of pages to be updated.
 * @param cacheattr The new cache attributes.
 * @param update_attr_table Whether the pp_attr_table should be updated. This is
 * useful for compressor pages where it's desired to keep the old WIMG bits.
 */
void pmap_batch_set_cache_attributes_internal(
    const unified_page_list_t *page_list, unsigned int cacheattr,
    bool update_attr_table) {
  bool tlb_flush_pass_needed = false;
  bool rt_cache_flush_pass_needed = false;
  bool preemption_disabled = false;

  PMAP_TRACE(2, PMAP_CODE(PMAP__BATCH_UPDATE_CACHING), page_list, cacheattr,
             0xCECC0DE1);
  ...
  for (unified_page_list_iterator_init(page_list, &iter);
    ...
    if (pmap_is_sptm_update_cache_attr_ops_pending(state)) {
      ...
      if (__improbable(!pvh_try_lock_success(&locked_pvh))) {
        ...
        const sptm_return_t sptm_ret = sptm_update_disjoint_multipage(
            sptm_pcpu->sptm_ops_pa, state.sptm_ops_index);
        pmap_epoch_exit();
        enable_preemption();
    ...
    const pp_attr_t pp_attr_template =
        (pp_attr_current & ~PP_ATTR_WIMG_MASK) | PP_ATTR_WIMG(cacheattr);
    ...
    /**
     * When update_attr_table is false, we know that wimg_bits_prev read from
     * pp_attr_table is not to be trusted, and we should force update the cache
     * attribute.
     */
    ...
    if ((wimg_bits_new != wimg_bits_prev) || force_update) {
      if (!pmap_is_cache_attribute_allowed(cacheattr)) {
        panic("%s: trying to use unsupported VM_WIMG type for managed page, "
              "VM_WIMG=%x, pn=%#x",
      ...
      do {
        ...
        if (!preemption_disabled) {
           ...
           * Enter the pmap epoch while we gather the disjoint update arguments
           * and issue the SPTM call.  Since this operation may cover multiple
           * physical pages, we may construct the argument array and invoke the
           * SPTM without holding all relevant PVH locks, we need to record that
           * we are collecting and modifying mapping state so that e.g.
           * pmap_page_protect() does not attempt to retype the underlying pages
           * and pmap_remove() does not attempt to free the page tables used for
           * these mappings without first draining our epoch.
           */
          pmap_epoch_enter();
        ...
        /* The return value indicates if we should call into SPTM in this
         * iteration. */
        pmap_sptm_update_cache_attr_ops_collect_return_t retval =
            pmap_sptm_update_cache_attr_ops_collect(&state, sptm_ops, paddr,
                                                    cacheattr);
        ...
        if (call_sptm) {
          ...
          sptm_return_t sptm_ret = sptm_update_disjoint_multipage(
              sptm_pcpu->sptm_ops_pa, SPTM_MAPPING_LIMIT);
          /**
           * We may be submitting the batch and exiting the epoch partway
           * through processing the PV list for a page.  That's fine, because in
           * that case we'll hold the PV lock for that page, which will prevent
           * mappings of that page from being disconnected and will prevent the
           * completion of pmap_remove() against any of those mappings, thus
           * also guaranteeing the relevant page table pages can't be freed. The
           * epoch still protects mappings for any prior page in the batch,
           * whose PV locks are no longer held.
           */
          pmap_epoch_exit();
          /**
           * Balance out the explicit disable_preemption() made either at the
           * beginning of the function or on a prior iteration of the loop that
           * placed the PVH lock in sleep mode.  Note that enable_preemption()
           * decrements a per-thread counter, so if we still happen to hold the
           * PVH lock in spin mode preemption won't actually be re-enabled until
           * we switch the lock over to sleep mode on the next iteration.
           */
          enable_preemption();
          preemption_disabled = false;
          pvh_lock_sleep_mode_needed = true;
        ...
        /* We cannot be in a situation where we didn't call into SPTM while also
         * having not finished walking the pv list. */
        assert(call_sptm || mapping_collection_done);
      } while (!mapping_collection_done);

      /**
       * We could technically force the cache flush pass here when force_update
       * is true, but since the compressor mapping/unmapping path handles cache
       * flushing itself, it's fine leaving this as is.
       */
      if (wimg_bits_new == VM_WIMG_RT && wimg_bits_prev != VM_WIMG_RT) {
  ...
  if (tlb_flush_pass_needed) {
     ...
     * cache fills that defeat the uncacheable requirement of the RT memory
     * type. In those cases, strong DSB must instead be employed to drain the
     * prefetcher.
     */
  ...
  if (rt_cache_flush_pass_needed) {
    ...
    /**
     * On APPLEVIRTUALPLATFORM, HID register accesses cause a synchronous
     * exception and the host will handle cache maintenance for it. So we don't
     * need to worry about enabling the ops here for AVP.
     */
    enable_dc_mva_ops();
#endif /* defined(APPLE_ARM64_ARCH_FAMILY) && !APPLEVIRTUALPLATFORM */
    /**
     * DMB should be sufficient to ensure prior accesses to the memory in
     * question are correctly ordered relative to the upcoming cache maintenance
     * operations.
     */
    ...
    for (unified_page_list_iterator_init(page_list, &iter);
      ...
      CleanPoC_DcacheRegion_Force_nopreempt_nohid_nobarrier(phystokv(paddr),
                                                            PAGE_SIZE);
    ...
    /* Issue DSB to ensure cache maintenance is fully complete before subsequent
     * accesses. */
    __builtin_arm_dsb(DSB_SY);
#if defined(APPLE_ARM64_ARCH_FAMILY) && !APPLEVIRTUALPLATFORM
    disable_dc_mva_ops();
 ...
 * the ppl call on PPL-enabled platforms or the _internal helper on other
 * platforms.
 *
 * @param page_list List of pages to be updated.
 * @param cacheattr The new cache attribute.
 */
void pmap_batch_set_cache_attributes(const unified_page_list_t *page_list,
                                     unsigned int cacheattr) {
  PMAP_TRACE(2, PMAP_CODE(PMAP__BATCH_UPDATE_CACHING) | DBG_FUNC_START,
             page_list, cacheattr, 0xCECC0DE0);
  ...
  PMAP_TRACE(2, PMAP_CODE(PMAP__BATCH_UPDATE_CACHING) | DBG_FUNC_END, page_list,
             cacheattr, 0xCECC0DEF);
}

MARK_AS_PMAP_TEXT void
pmap_set_cache_attributes_internal(ppnum_t pn, unsigned int cacheattr,
                                   bool update_attr_table) {
  ...
  pmap_batch_set_cache_attributes_internal(&page_list, cacheattr,
                                           update_attr_table);
}
...
void pmap_create_commpages(vm_map_address_t *kernel_data_addr,
                           vm_map_address_t *kernel_text_addr,
                           vm_map_address_t *kernel_ro_data_addr,
  ...
  *user_text_addr = 0;
  ...
  do {
    ...
    /**
     * Generate a VA for the commpage text with the same root and twig index as
     * data comm page, but with new leaf index we've just generated.
     */
    commpage_text_user_va =
        (_COMM_PAGE64_BASE_ADDRESS & ~pt_attr_leaf_index_mask(native_pt_attr));
    commpage_text_user_va |=
        (text_leaf_index << pt_attr_leaf_shift(native_pt_attr));
  } while ((commpage_text_user_va == _COMM_PAGE64_BASE_ADDRESS) ||
           (commpage_text_user_va ==
            _COMM_PAGE64_RO_ADDRESS)); // Try again if we collide (should be
                                       // unlikely)
 ...
 */
static_assert((_COMM_PAGE32_BASE_ADDRESS & ~ARM_TT_L2_OFFMASK) >=
              VM_MAX_ADDRESS);
 ...
 */
static_assert((_COMM_PAGE64_BASE_ADDRESS & ~ARM_TT_L1_OFFMASK) >=
              MACH_VM_MAX_ADDRESS);
#else
#error Nested shared page mapping is unsupported on this config
#endif

MARK_AS_PMAP_TEXT kern_return_t pmap_insert_commpage_internal(pmap_t pmap) {
  kern_return_t kr = KERN_SUCCESS;
...
#error                                                                         \
    "pmap_insert_commpage_internal requires a 16KB default kernel page size when __ARM_MIXED_PAGE_SIZE__ is enabled"
#endif /* !__ARM_16K_PG__ */
  ...
  if (pmap_page_size == 4096) {
    if (pmap_is_64bit(pmap)) {
      ...
      panic(
          "32-bit 4k commpage not currently supported for SPTM configurations");
      // commpage_table = commpage32_4k_table;
    }
  } else if (pmap_page_size != 16384) {
    panic("No commpage table exists for the wanted page size: %llu",
          pmap_page_size);
  } else
#endif /* __ARM_MIXED_PAGE_SIZE__ */
  {
...
#error We assume a single page.
   ...
   */
  while ((ttep = pmap_ttne(pmap, commpage_level, commpage_vaddr)) ==
         TT_ENTRY_NULL) {
    pmap_unlock(pmap, PMAP_LOCK_SHARED);
  ...
  sptm_map_table(pmap->ttep,
                 pt_attr_align_va(pt_attr, commpage_level, commpage_vaddr),
                 (sptm_pt_level_t)commpage_level,
                 (commpage_table & ARM_TTE_TABLE_MASK) | ARM_TTE_TYPE_TABLE |
                     ARM_TTE_VALID);
...
static void pmap_unmap_commpage(pmap_t pmap) {
  ...
  if (pte_to_pa(*ptep) != commpage_pa) {
    panic("%s: non-commpage PA 0x%llx mapped at VA 0x%llx in pmap %p; expected "
          "0x%llx",
          __func__, (unsigned long long)pte_to_pa(*ptep),
          (unsigned long long)commpage_vaddr, pmap,
          (unsigned long long)commpage_pa);
  }
...
void pmap_insert_commpage(pmap_t pmap) { pmap_insert_commpage_internal(pmap); }

static boolean_t pmap_is_64bit(pmap_t pmap) { return pmap->is_64bit; }

bool pmap_is_exotic(pmap_t pmap __unused) { return false; }

/* ARMTODO -- an implementation that accounts for
 * holes in the physical map, if any.
 */
boolean_t pmap_valid_page(ppnum_t pn) { return pa_valid(ptoa(pn)); }

boolean_t pmap_bootloader_page(ppnum_t pn) {
  pmap_paddr_t paddr = ptoa(pn);
...
MARK_AS_PMAP_TEXT boolean_t pmap_is_empty_internal(pmap_t pmap,
  ...
  while (block_start < va_end) {
    ...
    block_end = (block_start + pt_attr_twig_size(pt_attr)) &
                ~pt_attr_twig_offmask(pt_attr);
    if (block_end > va_end) {
      block_end = va_end;
    }
...
boolean_t pmap_is_empty(pmap_t pmap, vm_map_offset_t va_start,
                        vm_map_offset_t va_end) {
  return pmap_is_empty_internal(pmap, va_start, va_end);
}

vm_map_offset_t pmap_max_offset(boolean_t is64, unsigned int option) {
  return (is64) ? pmap_max_64bit_offset(option) : pmap_max_32bit_offset(option);
}

vm_map_offset_t pmap_max_64bit_offset(__unused unsigned int option) {
  vm_map_offset_t max_offset_ret = 0;

  const vm_map_offset_t min_max_offset =
      ARM64_MIN_MAX_ADDRESS; // end of shared region + 512MB for various
                             // purposes
  if (option == ARM_PMAP_MAX_OFFSET_DEFAULT) {
    ...
    if (arm64_pmap_max_offset_default) {
      ...
      max_offset_ret = MACH_VM_JUMBO_ADDRESS; // Max offset is 64GB for pmaps
                                              // with special "jumbo" blessing
    }
...
vm_map_offset_t pmap_max_32bit_offset(unsigned int option) {
  vm_map_offset_t max_offset_ret = 0;
...
void pmap_flush_context_init(__unused pmap_flush_context *pfc) {}

void pmap_flush(__unused pmap_flush_context *cpus_to_flush) {
  /* not implemented yet */
  return;
}
...
MARK_AS_PMAP_TEXT static void
pmap_ro_zone_validate_element_dst(zone_id_t zid, vm_offset_t va,
                                  vm_offset_t offset, vm_size_t new_data_size) {
  ...
  if (__improbable(offset >= elem_size)) {
    panic("%s: Offset %lu too large for elem size %lu at addr %p", __func__,
          (uintptr_t)offset, (uintptr_t)elem_size, (void *)va);
  }
}
...
MARK_AS_PMAP_TEXT static void
pmap_ro_zone_validate_element(zone_id_t zid, vm_offset_t va, vm_offset_t offset,
                              const vm_offset_t new_data,
                              vm_size_t new_data_size) {
  vm_offset_t sum = 0;

  if (__improbable(os_add_overflow(new_data, new_data_size, &sum))) {
    panic("%s: Integer addition overflow %p + %lu = %lu", __func__,
          (void *)new_data, (uintptr_t)new_data_size, (uintptr_t)sum);
  }
...
/**
 * Function to configure RO zone access permissions for a forthcoming write
 * operation.
 */
static void pmap_ro_zone_prepare_write(void) {}

/**
 * Function to indicate that a preceding RO zone write operation is complete.
 */
static void pmap_ro_zone_complete_write(void) {}

/**
 * Function to align an address or size to the required RO zone mapping
 * alignment.
 *
 * For the SPTM the RO zone region must be aligned on a twig boundary so that at
 * least the last-level kernel pagetable can be of the appropriate SPTM RO zone
 * table type, which allows the SPTM to enforce RO zone mapping permission
 * restrictions.
 *
 ...
 */
vm_offset_t pmap_ro_zone_align(vm_offset_t value) {
...
extern void kauth_cred_copy(const uintptr_t kv, const uintptr_t new_data);
...
void pmap_ro_zone_memcpy(zone_id_t zid, vm_offset_t va, vm_offset_t offset,
                         const vm_offset_t new_data, vm_size_t new_data_size) {
  pmap_ro_zone_memcpy_internal(zid, va, offset, new_data, new_data_size);
}

MARK_AS_PMAP_TEXT void pmap_ro_zone_memcpy_internal(zone_id_t zid,
                                                    vm_offset_t va,
                                                    vm_offset_t offset,
                                                    const vm_offset_t new_data,
                                                    vm_size_t new_data_size) {
...
uint64_t pmap_ro_zone_atomic_op(zone_id_t zid, vm_offset_t va,
                                vm_offset_t offset, zro_atomic_op_t op,
                                uint64_t value) {
  return pmap_ro_zone_atomic_op_internal(zid, va, offset, op, value);
}

MARK_AS_PMAP_TEXT uint64_t pmap_ro_zone_atomic_op_internal(zone_id_t zid,
                                                           vm_offset_t va,
                                                           vm_offset_t offset,
                                                           zro_atomic_op_t op,
                                                           uint64_t value) {
...
void pmap_ro_zone_bzero(zone_id_t zid, vm_offset_t va, vm_offset_t offset,
                        vm_size_t size) {
  pmap_ro_zone_bzero_internal(zid, va, offset, size);
}

MARK_AS_PMAP_TEXT void pmap_ro_zone_bzero_internal(zone_id_t zid,
                                                   vm_offset_t va,
                                                   vm_offset_t offset,
                                                   vm_size_t size) {
  const pmap_paddr_t pa = kvtophys_nofail(va + offset);
...
MARK_AS_PMAP_TEXT mach_vm_size_t pmap_query_resident_internal(
    pmap_t pmap, vm_map_address_t start, vm_map_address_t end,
    mach_vm_size_t *compressed_bytes_p) {
  ...
  if (__improbable((start % pt_attr_page_size(pt_attr)) ||
    ...
    panic("%s: address range %p, %p not page-aligned to 0x%llx", __func__,
          (void *)start, (void *)end, pt_attr_page_size(pt_attr));
  }
...
mach_vm_size_t pmap_query_resident(pmap_t pmap, vm_map_address_t start,
                                   vm_map_address_t end,
                                   mach_vm_size_t *compressed_bytes_p) {
  ...
  while (va < end) {
    ...
    resident_bytes =
        pmap_query_resident_internal(pmap, va, l, compressed_bytes_p);
 ...
 */
uint64_t pmap_shared_region_size_min(__unused pmap_t pmap) {
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
...
MARK_AS_PMAP_TEXT void pmap_set_vm_map_cs_enforced_internal(pmap_t pmap,
                                                            bool new_value) {
  validate_pmap_mutable(pmap);
  pmap->pmap_vm_map_cs_enforced = new_value;
}

void pmap_set_vm_map_cs_enforced(pmap_t pmap, bool new_value) {
  pmap_set_vm_map_cs_enforced_internal(pmap, new_value);
}

extern int cs_process_enforcement_enable;
bool pmap_get_vm_map_cs_enforced(pmap_t pmap) {
...
MARK_AS_PMAP_TEXT void pmap_set_jit_entitled_internal(__unused pmap_t pmap) {}

void pmap_set_jit_entitled(pmap_t pmap) {
  pmap_set_jit_entitled_internal(pmap);
}

bool pmap_get_jit_entitled(__unused pmap_t pmap) { return false; }

MARK_AS_PMAP_TEXT void pmap_set_tpro_internal(__unused pmap_t pmap) { return; }

void pmap_set_tpro(pmap_t pmap) { pmap_set_tpro_internal(pmap); }

bool pmap_get_tpro(__unused pmap_t pmap) { return false; }

uint64_t pmap_query_page_info_retries MARK_AS_PMAP_DATA;

MARK_AS_PMAP_TEXT kern_return_t
pmap_query_page_info_internal(pmap_t pmap, vm_map_offset_t va, int *disp_p) {
  pmap_paddr_t pa;
  int disp;
  unsigned int pai;
  pt_entry_t *pte_p;
...
kern_return_t pmap_query_page_info(pmap_t pmap, vm_map_offset_t va,
                                   int *disp_p) {
  return pmap_query_page_info_internal(pmap, va, disp_p);
}

uint32_t pmap_user_va_bits(pmap_t pmap __unused) {
#if __ARM_MIXED_PAGE_SIZE__
...
uint32_t pmap_kernel_va_bits(void) { return 64 - T1SZ_BOOT; }

static vm_map_size_t pmap_user_va_size(pmap_t pmap) {
  return 1ULL << pmap_user_va_bits(pmap);
}

bool pmap_in_ppl(void) { return false; }

MARK_AS_PMAP_TEXT void pmap_footprint_suspend_internal(vm_map_t map,
                                                       boolean_t suspend) {
#if DEVELOPMENT || DEBUG
  if (suspend) {
...
void pmap_nop(pmap_t pmap) { validate_pmap_mutable(pmap); }

pmap_t pmap_txm_kernel_pmap(void) { return kernel_pmap; }

TXMAddressSpace_t *pmap_txm_addr_space(const pmap_t pmap) {
  if (pmap) {
...
void pmap_txm_set_addr_space(pmap_t pmap, TXMAddressSpace_t *txm_addr_space) {
  assert(pmap != NULL);

  if (pmap->txm_addr_space && txm_addr_space) {
    /* Attempted to overwrite the address space in the PMAP */
    panic("attempted ovewrite of TXM address space: %p | %p | %p", pmap,
...
void pmap_txm_set_trust_level(pmap_t pmap, CSTrust_t trust_level) {
  assert(pmap != NULL);
...
kern_return_t pmap_txm_get_trust_level_kdp(pmap_t pmap,
...
kern_return_t pmap_txm_get_jit_address_range_kdp(pmap_t pmap,
  ...
  if (ml_validate_nofault((vm_offset_t)txm_addr_space,
                          sizeof(*txm_addr_space)) == false) {
    return KERN_INVALID_ARGUMENT;
  }
  /**
   * It's a bit gross that we're dereferencing what is supposed to be an
   * abstract type. If we were running in the TXM, we would always perform
   * additional checks on txm_addr_space, but this isn't necessary here, since
   * we are running in the kernel and only using the results for diagnostic
   * purposes, rather than any policy enforcement.
   */
  if (txm_addr_space->jitRegion) {
    if (ml_validate_nofault((vm_offset_t)txm_addr_space->jitRegion,
                            sizeof(txm_addr_space->jitRegion)) == false) {
      return KERN_INVALID_ARGUMENT;
    }
...
static pmap_t _pmap_txm_resolve_pmap(pmap_t pmap) {
  if (pmap == NULL) {
    pmap = current_pmap();
    if (pmap == kernel_pmap) {
      return NULL;
...
void pmap_txm_acquire_shared_lock(pmap_t pmap) {
...
void pmap_txm_release_shared_lock(pmap_t pmap) {
  pmap = _pmap_txm_resolve_pmap(pmap);
  if (!pmap) {
    return;
  }
...
void pmap_txm_acquire_exclusive_lock(pmap_t pmap) {
  pmap = _pmap_txm_resolve_pmap(pmap);
  if (!pmap) {
    return;
  }
...
void pmap_txm_release_exclusive_lock(pmap_t pmap) {
  pmap = _pmap_txm_resolve_pmap(pmap);
  if (!pmap) {
    return;
  }
...
static void _pmap_txm_transfer_page(const pmap_paddr_t addr) {
  sptm_retype_params_t retype_params = {.raw = SPTM_RETYPE_PARAMS_NULL};
 ...
 */
static inline void _pmap_txm_retype_prepare(const pmap_paddr_t pa) {
 ...
 *             aligned.
 */
void pmap_txm_transfer_page(const vm_address_t addr) {
  assert((addr & PAGE_MASK) == 0);

  const pmap_paddr_t pa = kvtophys_nofail(addr);
  const unsigned int pai = pa_index(pa);

  /* Lock the PVH lock to prevent concurrent updates to the mappings during the
   * self retype below. */
  locked_pvh_t locked_pvh = pvh_lock(pai);
  ...
  /* Self retype to clear the SPTM internal flags tracking delayed TLBIs for
   * revoked writes. */
  _pmap_txm_retype_prepare(pa);

  pvh_unlock(&locked_pvh);

  /* XNU needs to hold an RO reference to the page despite the ownership being
   * transferred to TXM. */
  pmap_enter_addr(kernel_pmap, addr, pa, VM_PROT_READ, VM_PROT_NONE, 0, true,
...
vm_map_address_t pmap_txm_allocate_page(void) {
  ...
  vm_page_insert_wired(page, txm_vm_object, phys_addr - gPhysBase,
...
int pmap_cs_configuration(void) {
...
bool pmap_performs_stage2_translations(__unused pmap_t pmap) { return false; }

bool pmap_has_iofilter_protected_write(void) {
...
void pmap_iofilter_protected_write(__unused vm_address_t addr,
                                   __unused uint64_t value,
                                   __unused uint64_t width) {
  /**
   * Even though this is done from EL1/2 for an address potentially owned by
   * Guarded Mode, we should be fine as mmu_kvtop uses "at s1e1r" checking for
   * read access only.
   */
  const pmap_paddr_t pa = mmu_kvtop(addr);

  if (!pa) {
    panic("%s: addr 0x%016llx doesn't have a valid kernel mapping", __func__,
          (uint64_t)addr);
  }
  ...
  if (frame_type == XNU_PROTECTED_IO) {
    ...
    if (__improbable(is_hibernating)) {
      ...
      /**
       * If panic_source is invalid (NO_PANICKING_DOMAIN: sptm_panic_source()
       * failed or no panic occurred) OR if the panic_source is XNU_DOMAIN, then
       * use the hibernation-specific write.
       */
     ...
     * accessing the address from kernel mode. This allows addresses that are
     * not owned by SPTM to be accessed by this interface.
     */
    switch (width) {
    case 1:
...
pmap_iofilter_protected_write(__unused vm_address_t addr,
                              __unused uint64_t value,
                              __unused uint64_t width) {
...
void *__attribute__((noreturn)) pmap_claim_reserved_ppl_page(void) {
  panic("%s: function not supported in this environment", __FUNCTION__);
}

void __attribute__((noreturn)) pmap_free_reserved_ppl_page(void __unused *kva) {
  panic("%s: function not supported in this environment", __FUNCTION__);
}

bool pmap_lookup_in_loaded_trust_caches(
    __unused const uint8_t cdhash[CS_CDHASH_LEN]) {
  kern_return_t kr = query_trust_cache(kTCQueryTypeLoadable, cdhash, NULL);
...
uint32_t pmap_lookup_in_static_trust_cache(
    __unused const uint8_t cdhash[CS_CDHASH_LEN]) {
  ...
  kr = query_trust_cache(kTCQueryTypeStatic, cdhash, &query_token);
...
pmap_dump_page_tables_recurse(pmap_t pmap, const tt_entry_t *ttp,
                              unsigned int cur_level, unsigned int level_mask,
                              uint64_t start_va, void *buf_start, void *buf_end,
                              size_t *bytes_copied) {
  const pt_attr_t *const pt_attr = pmap_get_pt_attr(pmap);
  ...
  if (((vm_offset_t)buf_end - (vm_offset_t)bufp) <
      (tt_size + sizeof(struct page_table_dump_header))) {
    return KERN_INSUFFICIENT_BUFFER_SIZE;
  }

  if (level_mask & (1U << cur_level)) {
    struct page_table_dump_header *header =
        (struct page_table_dump_header *)bufp;
  ...
  for (const tt_entry_t *ttep = ttp; ttep < tt_end;
       ttep++, current_va += size) {
    tt_entry_t tte = *ttep;
    ...
    if ((tte & type_mask) == type_block) {
      ...
      if (cur_level >= pt_attr_leaf_level(pt_attr)) {
        panic("%s: corrupt entry %#llx at %p, "
              ...
              __FUNCTION__, tte, ttep, ttp, cur_level, bufp, buf_end);
      }

      const tt_entry_t *next_tt =
          (const tt_entry_t *)phystokv(tte & ARM_TTE_TABLE_MASK);

      kern_return_t recurse_result = pmap_dump_page_tables_recurse(
          pmap, next_tt, cur_level + 1, level_mask, current_va, buf_start,
          buf_end, bytes_copied);
...
kern_return_t pmap_dump_page_tables(pmap_t pmap, void *bufp, void *buf_end,
                                    unsigned int level_mask,
                                    size_t *bytes_copied) {
  if (not_in_kdp) {
    panic("pmap_dump_page_tables must only be called from kernel debugger "
          "context");
  }
  return pmap_dump_page_tables_recurse(
      pmap, pmap->tte, pt_attr_root_level(pmap_get_pt_attr(pmap)), level_mask,
      pmap->min, bufp, buf_end, bytes_copied);
}

#else  /* DEVELOPMENT || DEBUG */

kern_return_t pmap_dump_page_tables(pmap_t pmap __unused, void *bufp __unused,
                                    void *buf_end __unused,
                                    unsigned int level_mask __unused,
...
static bool pmap_test_fault_handler(arm_saved_state_t *state) {
  bool retval = false;
...
// Disable KASAN instrumentation, as the test pmap's TTBR0 space will not be in
// the shadow map
static NOKASAN bool pmap_test_access(pmap_t pmap, vm_map_address_t va,
                                     bool should_fault, bool is_write) {
...
static bool pmap_test_read(pmap_t pmap, vm_map_address_t va,
                           bool should_fault) {
  ...
  if (!retval) {
    T_FAIL("%s: %s, "
           ...
           __func__, should_fault ? "did not fault" : "faulted", pmap,
           (void *)va, (unsigned)should_fault);
  }

  return retval;
}

static bool pmap_test_write(pmap_t pmap, vm_map_address_t va,
                            bool should_fault) {
...
static bool pmap_test_check_refmod(pmap_paddr_t pa,
                                   unsigned int should_be_set) {
  unsigned int should_be_clear =
      (~should_be_set) & (VM_MEM_REFERENCED | VM_MEM_MODIFIED);
  unsigned int bits = pmap_get_refmod((ppnum_t)atop(pa));

  bool retval = (((bits & should_be_set) == should_be_set) &&
                 ((bits & should_be_clear) == 0));

  if (!retval) {
    T_FAIL("%s: bits=%u, "
           "pa=%p, should_be_set=%u",
           __func__, bits, (void *)pa, should_be_set);
  }

  return retval;
}

static __attribute__((noinline)) bool pmap_test_read_write(pmap_t pmap,
                                                           vm_map_address_t va,
                                                           bool allow_read,
                                                           bool allow_write) {
  bool retval = (pmap_test_read(pmap, va, !allow_read) |
                 pmap_test_write(pmap, va, !allow_write));
  return retval;
}

static int pmap_test_test_config(unsigned int flags) {
  ...
  if (pmap_page_size <= native_page_size) {
    ...
    panic("%s: page_ratio < 1, native_page_size=%lu, pmap_page_size=%lu"
          "flags=%u",
          __func__, native_page_size, pmap_page_size, flags);
  }
   ...
   */
  pmap_enter_addr(pmap, wired_va_base, wired_pa, VM_PROT_READ, VM_PROT_READ, 0,
                  true, PMAP_MAPPING_TYPE_INFER);
  ...
  for (map_count = 0; map_count < page_ratio; map_count++) {
    pmap_enter_addr(pmap, va_base + (pmap_page_size * map_count),
                    pa + (pmap_page_size * (map_count)), VM_PROT_READ,
                    VM_PROT_READ, 0, false, PMAP_MAPPING_TYPE_INFER);
  }
  ...
  for (map_count = 0; map_count < page_ratio; map_count++) {
    ...
    if (pte_to_pa(*ptep) != (pa + (pmap_page_size * map_count))) {
      T_FAIL("Unexpected pa=%p, expected %p, map_count=%u",
             (void *)pte_to_pa(*ptep),
             (void *)(pa + (pmap_page_size * map_count)), map_count);
    }

    if (ptep_get_va(ptep) != (va_base + (pmap_page_size * map_count))) {
      T_FAIL("Unexpected va=%p, expected %p, map_count=%u",
             (void *)ptep_get_va(ptep),
             (void *)(va_base + (pmap_page_size * map_count)), map_count);
    }
  }
  ...
  pmap_enter_addr(pmap, va_base, pa, VM_PROT_READ | VM_PROT_WRITE,
  ...
  for (map_count = 1; map_count < page_ratio; map_count++) {
    ...
    pmap_remove(pmap, va_base + (pmap_page_size * map_count),
                va_base + (pmap_page_size * map_count) + pmap_page_size);
  }
  ...
  pmap_clear_refmod(pn, VM_MEM_MODIFIED | VM_MEM_REFERENCED);
  pmap_enter_addr(pmap, va_base, pa, VM_PROT_READ | VM_PROT_WRITE, VM_PROT_NONE,
                  0, false, PMAP_MAPPING_TYPE_INFER);
  pmap_test_check_refmod(pa, VM_MEM_REFERENCED);

  T_LOG("ref/!mod: rw, read fault.  Expect ref/!mod");
  pmap_enter_addr(pmap, va_base, pa, VM_PROT_READ | VM_PROT_WRITE, VM_PROT_READ,
                  0, false, PMAP_MAPPING_TYPE_INFER);
  pmap_test_check_refmod(pa, VM_MEM_REFERENCED);
  ...
  pmap_enter_addr(pmap, rw_base, pa, VM_PROT_READ | VM_PROT_WRITE,
                  VM_PROT_READ | VM_PROT_WRITE, 0, false,
                  PMAP_MAPPING_TYPE_INFER);
  ...
  pmap_protect(pmap, ro_base, ro_base + pmap_page_size,
               VM_PROT_READ | VM_PROT_WRITE);
  pmap_protect(pmap, rw_base, rw_base + pmap_page_size,
               VM_PROT_READ | VM_PROT_WRITE);
  pmap_test_read_write(pmap, ro_base, true, false);
  pmap_test_read_write(pmap, rw_base, true, true);
...
#define PMAP_TEST_LARGE_MEMORY_VA 64 * (1ULL << 40) /* 64 TB */
  ...
  T_LOG("Create new wired mapping in the extended address space enabled by "
        "ARM_LARGE_MEMORY.");
  pmap_enter_addr(pmap, PMAP_TEST_LARGE_MEMORY_VA, wired_pa,
                  VM_PROT_READ | VM_PROT_WRITE, VM_PROT_READ | VM_PROT_WRITE, 0,
                  true, PMAP_MAPPING_TYPE_INFER);
  pmap_test_read_write(pmap, PMAP_TEST_LARGE_MEMORY_VA, true, true);
  pmap_remove(pmap, PMAP_TEST_LARGE_MEMORY_VA,
              PMAP_TEST_LARGE_MEMORY_VA + pmap_page_size);
#else  /* !defined(ARM_LARGE_MEMORY_KERNONLY) */
  /* Using kernel-only large memory. Make sure user pmap will fail. */
  T_LOG(
      "Expect wired mapping to fault in ARM_LARGE_MEMORY when using KERNONLY.");

  /* The mapping should be rejected, it's outside of T0SZ */
  const kern_return_t kr = pmap_enter_addr(
      pmap, PMAP_TEST_LARGE_MEMORY_VA, wired_pa, VM_PROT_READ | VM_PROT_WRITE,
      VM_PROT_READ | VM_PROT_WRITE, 0, true, PMAP_MAPPING_TYPE_INFER);
  T_QUIET;
  T_ASSERT_NE_INT(kr, KERN_SUCCESS, NULL);

  /* Addressing outside of T0SZ should result in a L0 xlate fault */
  const bool did_fault =
      pmap_test_read_write(pmap, PMAP_TEST_LARGE_MEMORY_VA, false, false);
  T_QUIET;
  T_ASSERT(did_fault, NULL);
 ...
 */
kern_return_t pmap_test_text_corruption(pmap_paddr_t pa __unused) {
  /*
