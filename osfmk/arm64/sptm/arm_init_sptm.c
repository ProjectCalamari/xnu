...
#if HAS_BP_RET
/* Enable both branch target retention (0x2) and branch direction retention
 * (0x1) across sleep */
uint32_t bp_ret = 3;
...
TUNABLE_DT_WRITEABLE(sched_hygiene_mode_t, interrupt_masked_debug_mode,
                     ...
                     "interrupt-masked-debug-mode", SCHED_HYGIENE_MODE_PANIC,
                     TUNABLE_DT_CHECK_CHOSEN);

MACHINE_TIMEOUT_DEV_WRITEABLE(interrupt_masked_timeout, "interrupt-masked",
                              DEFAULT_INTERRUPT_MASKED_TIMEOUT,
                              MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
#if __arm64__
#define SSHOT_INTERRUPT_MASKED_TIMEOUT 0xf9999 /* 64-bit: 42.599ms */
#endif
MACHINE_TIMEOUT_DEV_WRITEABLE(stackshot_interrupt_masked_timeout,
                              "sshot-interrupt-masked",
                              SSHOT_INTERRUPT_MASKED_TIMEOUT,
                              MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
...
extern uint32_t
    __thread_starts_sect_start[] __asm("section$start$__TEXT$__thread_starts");
extern uint32_t
    __thread_starts_sect_end[] __asm("section$end$__TEXT$__thread_starts");
...
extern vm_offset_t segLOWESTAuxKC, segHIGHESTAuxKC, segLOWESTROAuxKC,
    segHIGHESTROAuxKC;
extern vm_offset_t segLOWESTRXAuxKC, segHIGHESTRXAuxKC, segHIGHESTNLEAuxKC;

void arm_slide_rebase_and_sign_image(void);
MARK_AS_FIXUP_TEXT void arm_slide_rebase_and_sign_image(void) {
  ...
  kernel_collection_slide((struct mach_header_64 *)kc_mh,
  ...
  kernel_collection_adjust_mh_addrs(
      (struct mach_header_64 *)kc_mh, slide, false, (uintptr_t *)&segLOWESTKC,
      (uintptr_t *)&segHIGHESTKC, (uintptr_t *)&segLOWESTROKC,
      (uintptr_t *)&segHIGHESTROKC, NULL, NULL, NULL);
...
MARK_AS_FIXUP_TEXT void arm_static_if_init(boot_args *args) {
  static_if_init(args->CommandLine);
}

void arm_auxkc_init(void *mh, void *base) {
  /*
  ...
  /* rebase and sign jops */
  const void *collection_base_pointers[KCNumKinds];
  memcpy(collection_base_pointers, PE_get_kc_base_pointers(),
         sizeof(collection_base_pointers));
  kernel_collection_slide((struct mach_header_64 *)akc_mh,
                          collection_base_pointers);

  kernel_collection_adjust_mh_addrs(
      (struct mach_header_64 *)akc_mh, slide, false,
      (uintptr_t *)&segLOWESTAuxKC, (uintptr_t *)&segHIGHESTAuxKC,
      (uintptr_t *)&segLOWESTROAuxKC, (uintptr_t *)&segHIGHESTROAuxKC,
      (uintptr_t *)&segLOWESTRXAuxKC, (uintptr_t *)&segHIGHESTRXAuxKC,
 ...
 */
static_assert((((~ARM_KERNEL_PROTECT_EXCEPTION_START) + 1) * 2ULL) <=
              (ARM_TT_ROOT_SIZE + ARM_TT_ROOT_INDEX_MASK));
...
void kasan_bootstrap(boot_args *, vm_offset_t pgtable,
                     sptm_bootstrap_args_xnu_t *sptm_boot_args);
 ...
 */
void arm_init_kasan(boot_args *args,
                    sptm_bootstrap_args_xnu_t *sptm_boot_args) {
  ...
  kasan_bootstrap(args,
                  phystokv(sptm_boot_args->libsptm_state.root_table_paddr),
                  sptm_boot_args);
 ...
 */
void arm_init(boot_args *args, sptm_bootstrap_args_xnu_t *sptm_boot_args) {
  unsigned int maxmem;
  uint32_t memsize;
  uint64_t xmaxmem;
  thread_t thread;
  ...
  const sptm_vaddr_t handler_addr = (sptm_vaddr_t)ptrauth_strip(
      (void *)xnu_return_to_gl2, ptrauth_key_function_pointer);
  ...
  chosen = NULL; // Force a re-lookup later on since VM addresses are not final
                 // at this point
  dt_entry_size = 0;
...
#ifdef APPLETYPHOON
    if (PE_parse_boot_argn("cpus_defeatures", &cpus_defeatures,
  ...
  PE_parse_boot_argn("sptm_stability_hacks", &sptm_stability_hacks,
                     sizeof(sptm_stability_hacks));
  ...
  CpuDataEntries[master_cpu].cpu_data_paddr =
      (void *)((uintptr_t)(args->physBase) +
               ((uintptr_t)&BootCpuData - (uintptr_t)(args->virtBase)));
  ...
  {
    int wdt_boot_arg = 0;
    bool const wdt_disabled =
        (PE_parse_boot_argn("wdt", &wdt_boot_arg, sizeof(wdt_boot_arg)) &&
         (wdt_boot_arg == -1));
    ...
    if (wdt_disabled ||
        kern_feature_override(KF_PREEMPTION_DISABLED_DEBUG_OVRD)) {
  ...
  PE_parse_boot_argn("immediate_NMI", &force_immediate_debug_halt,
   ...
   */
  const uint64_t sptm_is_large_memory =
      SPTMArgs->feature_flags & SPTM_FEATURE_LARGE_MEMORY;
  const uint64_t sptm_is_large_memory_kernonly =
      SPTMArgs->feature_flags & SPTM_FEATURE_LARGE_MEMORY_KERNONLY;
#if ARM_LARGE_MEMORY
  const uint64_t xnu_is_large_memory = SPTM_FEATURE_LARGE_MEMORY;
#if ARM_LARGE_MEMORY_KERNONLY
  const uint64_t xnu_is_large_memory_kernonly =
      SPTM_FEATURE_LARGE_MEMORY_KERNONLY;
  ...
  if (sptm_is_large_memory != xnu_is_large_memory) {
    panic("Mismatch of ARM_LARGE_MEMORY in SPTM (%#llx)/XNU (%#llx)",
          sptm_is_large_memory, xnu_is_large_memory);
  }
  ...
  /*
   * gPhysBase/Size only represent kernel-managed memory. These globals
   * represent the actual DRAM base address and size as reported by iBoot
  ...
  if (SecureDTGetProperty(chosen, "dram-base", (void const **)&dram_base,
                          &dt_entry_size) != kSuccess) {
    panic("%s: Unable to find 'dram-base' entry in the 'chosen' DT node",
          __FUNCTION__);
  }

  if (SecureDTGetProperty(chosen, "dram-size", (void const **)&dram_size,
                          &dt_entry_size) != kSuccess) {
    panic("%s: Unable to find 'dram-size' entry in the 'chosen' DT node",
          __FUNCTION__);
  }
  ...
  if (PE_parse_boot_argn("serial", &serialmode, sizeof(serialmode))) {
    ...
    enable_dklog_serial_output =
        restore_boot || (serialmode & SERIALMODE_DKLOG) != 0;
    int force_sync = serialmode & SERIALMODE_SYNCDRAIN;
    if (force_sync || PE_parse_boot_argn("drain_uart_sync", &force_sync,
                                         sizeof(force_sync))) {
      if (force_sync) {
        serialmode |= SERIALMODE_SYNCDRAIN;
        kprintf("WARNING: Forcing uart driver to output synchronously."
  ...
  /* Start serial if requested and a serial device was enumerated in
   * serial_init(). */
  if ((serialmode & SERIALMODE_OUTPUT) && serial_exists) {
    serial_console_enabled = true;
    (void)switch_to_serial_console(); /* Switch into serial mode from video
                                         console */
  ...
  if (const_sptm_args.sptm_variant != SPTM_VARIANT_RELEASE) {
    panic("arm_init: Development SPTM / Release XNU is not a supported "
          "configuration.");
  }
#endif /* RELEASE */
  ...
  if (!cpu_config_correct) {
    panic("The cpumask=N boot arg cannot be used together with cpus=N, and the "
          "boot CPU must be enabled");
  }
...
void arm_init_cpu(cpu_data_t *cpu_data_ptr,
                  __unused uint64_t hibernation_args) {
#if HIBERNATION
  sptm_hibernation_args_xnu_t *hibargs =
      (sptm_hibernation_args_xnu_t *)hibernation_args;

  if ((hibargs != 0) && (hibargs->hib_header_phys != 0) &&
      (hibargs->handoff_page_count > 0)) {
    /*
     * We must copy the handoff region before anything else because the physical
     * pages holding the handoff region are not tracked by xnu as in-use.
     */
    HibernationCopyHandoffRegionFromPageArray(&hibargs->handoff_pages[0],
                                              hibargs->handoff_page_count);
  }
#endif /* HIBERNATION */
  ...
  if (hibargs != 0 && hibargs->hib_header_phys != 0) {
    ...
    __nosan_memcpy(gIOHibernateCurrentHeader,
                   (void *)phystokv(hibargs->hib_header_phys),
                   sizeof(IOHibernateImageHeader));
  }
  if ((cpu_data_ptr == &BootCpuData) &&
      (gIOHibernateState == kIOHibernateStateWakingFromHibernate) &&
      ml_is_quiescing()) {
    // the "normal" S2R code captures wake_abstime too early, so on a
    // hibernation resume we fix it up here
    extern uint64_t wake_abstime;
    wake_abstime = gIOHibernateCurrentHeader->lastHibAbsTime;

    // since the hw clock stops ticking across hibernation, we need to apply an
    // offset; iBoot computes this offset for us and passes it via the
    // hibernation header
    extern uint64_t hwclock_conttime_offset;
    hwclock_conttime_offset = gIOHibernateCurrentHeader->hwClockOffset;

    // during hibernation, we captured the idle thread's state from inside the
    // PPL context, so we have to fix up its preemption count
    unsigned int expected_preemption_count =
        (gEnforcePlatformActionSafety ? 2 : 1);
  ...
  if (cpu_data_ptr == &BootCpuData && ml_is_quiescing()) {
    ...
    exclaves_update_timebase(EXCLAVES_CLOCK_ABSOLUTE, rtclock_base_abstime);
#if HIBERNATION
 ...
 */
void __attribute__((noreturn)) arm_init_idle_cpu(cpu_data_t *cpu_data_ptr) {
#if __ARM_PAN_AVAILABLE__
  __builtin_arm_wsr("pan", 1);
#endif
  ...
  /*
   * Update the active debug object to reflect that debug registers have been
   * reset. This will force any thread with active debug state to resync the
   * debug registers if it returns to userspace on this CPU.
   */
...
/*
 * Looks up the set of properties that describe the physical load addresses and
 * sizes of the boot kernelcache's loaded segments in the device tree and
 * returns (1) the number of segments found in *arm_vm_kernelcache_numrangesp
 * and (2) their starting/ending addresses as an array of type arm_physrange_t
 * in *arm_vm_kernelcache_rangesp. The function returns the total number of
 * pages across all loaded boot kernelcache segments. If there is a problem
 * looking up the /chosen/memory-map node in the DT, all arguments are zeroed
 * and the function returns 0.
 */
static unsigned int arm_get_bootkc_ranges_from_DT(
    const arm_physrange_t **arm_vm_kernelcache_rangesp,
    int *arm_vm_kernelcache_numrangesp) {
  DTEntry memory_map;
  int err;
  DTMemoryMapRange const *range;
  unsigned int rangeSize;
#define NUM_BOOTKC_RANGES 5
  static arm_physrange_t bootkc_physranges[NUM_BOOTKC_RANGES] = {{
      0,
  }};
  ...
  if (bootkc_numranges == 0) {
    ...
    /* We're looking for 5 ranges: BootKC-ro, BootKC-rx, BootKC-bx, BootKC-rw,
     * and BootKC-le */
    ...
    for (int i = 0; i < NUM_BOOTKC_RANGES; i++) {
      err = SecureDTGetProperty(memory_map, BootKC_Properties[i],
                                (void const **)&range, &rangeSize);
      if (err == kSuccess && rangeSize == sizeof(DTMemoryMapRange)) {
        ...
        bootkc_numranges++;
        bootkc_total_pages += (unsigned int)atop_64(
            bootkc_physranges[i].end_phys - bootkc_physranges[i].start_phys);
      }
    }
  }
...
static void arm_vm_slide_region(vm_offset_t phys_start, size_t size) {
...
void arm_vm_prot_finalize(boot_args *args __unused) {
  ...
  /*
   * Replace the boot CPU's stacks with properly-guarded dynamically allocated
   * stacks. This must happen prior to sliding segBOOTDATAB, which will
   * effectively remove the existing boot stacks.
   */
 ...
 */
static void init_image_offsets(size_t debug_header_entry,
                               vm_image_offsets *offsets) {
  assert(offsets != NULL);
  assert((debug_header_entry == DEBUG_HEADER_ENTRY_SPTM) ||
         (debug_header_entry == DEBUG_HEADER_ENTRY_TXM));

  offsets->slid_base =
      (vm_offset_t)SPTMArgs->debug_header->image[debug_header_entry];
  kernel_mach_header_t *macho = (kernel_mach_header_t *)offsets->slid_base;
  offsets->unslid_base =
      (vm_offset_t)getsegbynamefromheader(macho, "__TEXT")->vmaddr;
...
void arm_vm_init(uint64_t memory_size_override, boot_args *args) {
  ...
  /* Make sure the system does not have more physical memory than what can be
   * mapped */
   ...
   * ml_static_mfree() uses the BootKC ranges from the DT to account for freed
   * kernelcache pages.
   */
  vm_page_kernelcache_count = arm_get_bootkc_ranges_from_DT(
      &arm_vm_kernelcache_ranges, &arm_vm_kernelcache_numranges);
   ...
   *
   * Note: This does not allocate L3 page tables, since page tables for all
   * static memory is allocated and inserted into the hierarchy by the SPTM
   * beforehand. Instead, this code simply walks the page tables to find those
   * pre-allocated tables and allocates PTD objects for them.
   */
