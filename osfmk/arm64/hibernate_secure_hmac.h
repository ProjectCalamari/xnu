...
void secure_hmac_hibernate_begin(secure_hmac_hib_state_t state,
...
int secure_hmac_update_and_compress_page(secure_hmac_hib_state_t state,
                                         ppnum_t page_number,
                                         const void **uncompressed,
                                         const void **encrypted,
                                         void *compressed);
void secure_hmac_final(secure_hmac_hib_state_t state, uint8_t *output,
                       size_t output_len);
...
void secure_hmac_finalize_image(const void *image_hash, size_t image_hash_len,
                                uint8_t *hmac, size_t hmac_len);
void secure_hmac_get_io_ranges(const hib_phys_range_t **io_ranges,
                               size_t *num_io_ranges);
#if CONFIG_SPTM
