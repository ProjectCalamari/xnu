 ...
 */
arm_sme_version_t arm_sme_version(void) { return sme_version; }
...
static arm_sme_version_t arm_sme_probe_version(void) {
...
void arm_sme_init(bool is_boot_cpu) {
  if (is_boot_cpu) {
 ...
 */
void arm_save_sme_za(arm_sme_context_t *sme_ss, uint16_t svl_b) {
 ...
 * @param svl_b SVL corresponding to sme_ss, in bytes
 */
void arm_load_sme_za(const arm_sme_context_t *sme_ss, uint16_t svl_b) {
 ...
 */
void arm_sme_trap_at_el0(bool trap_enabled) {
  uint64_t cpacr_el1 = __builtin_arm_rsr64("CPACR_EL1");
  unsigned int prev_mode = (unsigned int)(cpacr_el1 & CPACR_SMEN_MASK);
  unsigned int new_mode =
      trap_enabled ? CPACR_SMEN_EL0_TRAP : CPACR_SMEN_ENABLE;
 ...
 */
boolean_t arm_sme_is_active(void) {
  /* Kernel entry clobbers SVCR.SM, so check the saved state instead of live
   * register state */
 ...
 */
void arm_save_sme_zt0(arm_sme_context_t *sme_ss) {
  asm volatile("str zt0, [%[addr]]" : : [addr] "r"(sme_ss->zt0));
}
 ...
 */
void arm_load_sme_zt0(const arm_sme_context_t *sme_ss) {
  asm volatile("ldr	zt0, [%[addr]]" : : [addr] "r"(sme_ss->zt0));
}
 ...
 * @param svl_b SVL corresponding to sme_ss, in bytes
 */
void arm_save_sme_za_zt0(arm_sme_context_t *sme_ss, uint16_t svl_b) {
 ...
 * @param svl_b SVL corresponding to sme_ss, in bytes
 */
void arm_load_sme_za_zt0(const arm_sme_context_t *sme_ss, uint16_t svl_b) {
