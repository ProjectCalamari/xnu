...
static kern_return_t bcopy_phys_internal(addr64_t src, addr64_t dst,
                                         vm_size_t bytes, int flags) {
  ...
  while ((bytes > 0) && (res == KERN_SUCCESS)) {
    ...
    if (BCOPY_PHYS_DST_IS_PHYS(flags)) {
      // write preflighting needed for things like dtrace which may write static
      // read-only mappings
      use_copy_window_dst =
    ...
    if (use_copy_window_src) {
      src_index =
          pmap_map_cpu_windows_copy(pn_src, VM_PROT_READ, wimg_bits_src);
      tmp_src =
          (char *)(pmap_cpu_windows_copy_addr(cpu_num, src_index) + src_offset);
    ...
    if (use_copy_window_dst) {
      dst_index = pmap_map_cpu_windows_copy(
          pn_dst, VM_PROT_READ | VM_PROT_WRITE, wimg_bits_dst);
      tmp_dst =
...
void bcopy_phys(addr64_t src, addr64_t dst, vm_size_t bytes) {
  bcopy_phys_internal(src, dst, bytes, cppvPsrc | cppvPsnk);
}

void bcopy_phys_with_options(addr64_t src, addr64_t dst, vm_size_t bytes,
                             int options) {
...
static void bzero_phys_page(vm_offset_t buf) {
...
static void bzero_phys_internal(addr64_t src, vm_size_t bytes,
  ...
  while (bytes > 0) {
    ...
    if (use_copy_window) {
      mp_disable_preemption();
      cpu_num = cpu_number();
      index = pmap_map_cpu_windows_copy(pn, VM_PROT_READ | VM_PROT_WRITE,
                                        wimg_bits);
...
#endif
      /**
       * When we are zerofilling a normal page, there are a couple of
       * assumptions that can be made.
       *
       * 1. The destination to be zeroed is page-sized and page-aligned, making
       * the unconditional 4 stp instructions in bzero redundant.
       * 2. The dczva loop for zerofilling can be fully unrolled at compile-time
       * thanks to known size of the destination, reducing instruction fetch
       * overhead caused by the branch backward in a tight loop.
       */
      if (count == PAGE_SIZE) {
        /**
         * Thanks to how count is computed above, buf should always be page-size
         * aligned when count == PAGE_SIZE.
         */
...
void bzero_phys(addr64_t src, vm_size_t bytes) {
...
static inline uint128_t __read128(vm_address_t addr) {
  uint64_t hi, lo;

  asm volatile("ldp    %[lo], %[hi], [%[addr]]"
               "\n"
               : [lo] "=r"(lo), [hi] "=r"(hi)
               : [addr] "r"(addr)
               : "memory");
...
static uint128_t ml_phys_read_data(pmap_paddr_t paddr, int size) {
  vm_address_t addr;
  ...
  uint32_t report_phy_read_delay =
      os_atomic_load(&report_phy_read_delay_to, relaxed);
  uint32_t const trace_phy_read_delay =
  ...
  if (use_copy_window) {
    ...
    addr = pmap_cpu_windows_copy_addr(cpu_number(), index) |
  ...
  if (__improbable(use_timeout)) {
    ...
    iotrace(IOTRACE_PHYS_READ, 0, addr, size, result,
            kern_timeout_start_time(&timeout), duration);

    if (__improbable(duration > report_phy_read_delay)) {
      DTRACE_PHYSLAT4(physread, uint64_t, duration, uint64_t, addr, uint32_t,
    ...
    if (__improbable(duration > report_phy_read_delay)) {
      if (phy_read_panic && (machine_timeout_suspended() == FALSE)) {
        ...
        snprintf(str, sizeof(str),
                 "Read from physical addr 0x%llx (result: 0x%016llx%016llx) "
                 "timed out:",
    ...
    if (__improbable(trace_phy_read_delay > 0 &&
      ...
      KDBG(MACHDBG_CODE(DBG_MACH_IO, DBC_MACH_IO_PHYS_READ), duration,
           kern_timeout_start_time(&timeout), addr, result);
    }
...
uint128_t ml_phys_read_quad_64(addr64_t paddr64) {
...
#if BUILD_QUAD_WORD_FUNCS
static inline void __write128(vm_address_t addr, uint128_t data) {
  const uint64_t hi = (uint64_t)(data >> 64);
  const uint64_t lo = (uint64_t)(data);

  asm volatile("stp    %[lo], %[hi], [%[addr]]"
               "\n"
               : /**/
...
static void ml_phys_write_data(pmap_paddr_t paddr, uint128_t data, int size) {
  vm_address_t addr;
  ppnum_t pn = atop_kernel(paddr);
  ppnum_t pn_end = atop_kernel(paddr + size - 1);
  unsigned int index;
  ...
  uint32_t report_phy_write_delay =
      os_atomic_load(&report_phy_write_delay_to, relaxed);
  uint32_t const trace_phy_write_delay =
  ...
  if (use_copy_window) {
    ...
    unsigned int wimg_bits = pmap_cache_attributes(pn);
    index =
        pmap_map_cpu_windows_copy(pn, VM_PROT_READ | VM_PROT_WRITE, wimg_bits);
    addr = pmap_cpu_windows_copy_addr(cpu_number(), index) |
           ((uint32_t)paddr & PAGE_MASK);
  }
  ...
  if (__improbable(use_timeout)) {
    ...
    iotrace(IOTRACE_PHYS_WRITE, 0, paddr, size, data,
            kern_timeout_start_time(&timeout), duration);

    if (__improbable(duration > report_phy_write_delay)) {
      DTRACE_PHYSLAT4(physwrite, uint64_t, duration, uint64_t, paddr, uint32_t,
                      size, uint64_t, data);
    ...
    if (__improbable(trace_phy_write_delay > 0 &&
      ...
      KDBG(MACHDBG_CODE(DBG_MACH_IO, DBC_MACH_IO_PHYS_WRITE), duration,
           kern_timeout_start_time(&timeout), paddr, data);
    }
...
void ml_phys_write_quad_64(addr64_t paddr64, uint128_t data) {
...
int ffsll(unsigned long long mask) {
  if (mask == 0) {
    return 0;
  }
...
kern_return_t copypv(addr64_t source, addr64_t sink, unsigned int size,
                     int which) {
  if ((which & (cppvPsrc | cppvPsnk)) ==
...
boolean_t ml_probe_read(__unused vm_offset_t paddr,
                        __unused unsigned int *val) {
  panic("ml_probe_read() unimplemented");
  return 1;
}

boolean_t ml_probe_read_64(__unused addr64_t paddr,
                           __unused unsigned int *val) {
...
__dead2 void panic_unimplemented(void) { panic("Not yet implemented."); }
 ...
 * @param bytes Number of bytes to be accessed. This cannot cross page
 * boundaries.
 ...
 */
int apply_func_phys(addr64_t dst64, vm_size_t bytes,
                    int (*func)(void *buffer, vm_size_t bytes, void *arg),
                    void *arg) {
  /* The physical aperture is only guaranteed to work with kernel-managed
   * addresses. */
  if (!pmap_valid_address(dst64)) {
    panic("%s address error: passed in address (%#llx) not a kernel managed "
          "address",
          __FUNCTION__, dst64);
  }
  ...
  if (((((uint32_t)dst64 & (ARM_PGBYTES - 1)) + bytes) > ARM_PGBYTES)) {
    panic("%s alignment error: tried accessing addresses spanning more than "
          "one page %#llx %#lx",
