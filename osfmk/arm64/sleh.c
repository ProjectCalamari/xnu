...
#define TEST_CONTEXT32_SANITY(context)                                         \
  (context->ss.ash.flavor == ARM_SAVED_STATE32 &&                              \
   context->ss.ash.count == ARM_SAVED_STATE32_COUNT &&                         \
   context->ns.nsh.flavor == ARM_NEON_SAVED_STATE32 &&                         \
   context->ns.nsh.count == ARM_NEON_SAVED_STATE32_COUNT)

#define TEST_CONTEXT64_SANITY(context)                                         \
  (context->ss.ash.flavor == ARM_SAVED_STATE64 &&                              \
   context->ss.ash.count == ARM_SAVED_STATE64_COUNT &&                         \
   context->ns.nsh.flavor == ARM_NEON_SAVED_STATE64 &&                         \
   context->ns.nsh.count == ARM_NEON_SAVED_STATE64_COUNT)
...
#define COPYIN(src, dst, size)                                                 \
  (PSR64_IS_KERNEL(get_saved_state_cpsr(state))) ? copyin_kern(src, dst, size) \
                                                 : copyin(src, dst, size)
...
_Static_assert(ARM64_KDBG_CODE_GUEST <= KDBG_CODE_MAX,
               "arm64 KDBG trace codes out of range");
_Static_assert(ARM64_KDBG_CODE_GUEST <= UINT16_MAX,
               "arm64 KDBG trace codes out of range");

void panic_with_thread_kernel_state(const char *msg,
                                    arm_saved_state_t *ss) __abortlike;
...
void sleh_invalid_stack(arm_context_t *context, uint64_t esr,
                        vm_offset_t far) __dead2;

static void sleh_interrupt_handler_prologue(arm_saved_state_t *,
                                            unsigned int type);
...
static inline uint64_t fault_addr_bitmask(unsigned int bit_from,
                                          unsigned int bit_to);
#endif
static void handle_bti_fail(arm_saved_state_t *state, uint64_t esr);
extern kern_return_t arm_fast_fault(pmap_t, vm_map_address_t, vm_prot_t, bool,
                                    bool);
...
typedef void (*abort_handler_t)(arm_saved_state_t *, uint64_t, vm_offset_t,
                                fault_status_t, vm_prot_t,
                                expected_fault_handler_t);
static void handle_user_abort(arm_saved_state_t *, uint64_t, vm_offset_t,
                              fault_status_t, vm_prot_t,
                              expected_fault_handler_t);
static void handle_kernel_abort(arm_saved_state_t *, uint64_t, vm_offset_t,
                                fault_status_t, vm_prot_t,
                                expected_fault_handler_t);
...
static void handle_abort(arm_saved_state_t *, uint64_t, vm_offset_t,
                         abort_inspector_t, abort_handler_t,
                         expected_fault_handler_t);

static void handle_user_trapped_instruction32(arm_saved_state_t *,
                                              uint64_t esr) __dead2;
...
extern void unix_syscall(struct arm_saved_state *regs, thread_t thread_act,
                         struct proc *proc);

extern void mach_syscall(struct arm_saved_state *);

#if CONFIG_SPTM
bool sleh_panic_lockdown_should_initiate_el1_sp0_sync(uint64_t esr,
                                                      uint64_t elr,
                                                      uint64_t far,
                                                      uint64_t spsr);
#endif /* CONFIG_SPTM */
...
perfCallback tempDTraceTrapHook =
    NULL; /* Pointer to DTrace fbt trap hook routine */
#endif
...
__options_closed_decl(self_restrict_mode_t, unsigned int,
                      {
                          ...
                          /*
                           * Any of the other more specific modes, this should
                           * be active if any other mode is active.
                           */
...
static inline ptrdiff_t copyio_recovery_offset(uintptr_t addr) {
  return (ptrdiff_t)(addr - (uintptr_t)copyio_recover_table);
}

#if !HAS_APPLE_PAC
static inline uintptr_t copyio_recovery_addr(ptrdiff_t offset) {
...
static inline int is_parity_error(fault_status_t status) {
  switch (status) {
...
static inline int is_sync_external_abort(fault_status_t status) {
  switch (status) {
#if defined(ARM64_BOARD_CONFIG_T6020)
  /*
   * H14 Erratum (rdar://61553243): Despite having FEAT_RAS implemented,
...
static inline int is_table_walk_error(fault_status_t status) {
  switch (status) {
...
static inline int is_servicible_fault(fault_status_t status, uint64_t esr) {
#pragma unused(esr)
  return is_vm_fault(status);
}

__dead2 __unused static void
arm64_implementation_specific_error(arm_saved_state_t *state, uint64_t esr,
                                    vm_offset_t far) {
...
static void kernel_integrity_error_handler(uint64_t esr, vm_offset_t far) {
  ...
  if (ESR_WT_SERROR(esr)) {
    ...
    case WT_REASON_PT_VIOLATION:
      panic_plain("Kernel integrity, violation in mapping 0x%016lx.", far);
...
static void arm64_platform_error(arm_saved_state_t *state, uint64_t esr,
                                 vm_offset_t far,
                                 platform_error_source_t source) {
#if CONFIG_KERNEL_INTEGRITY
...
void panic_with_thread_kernel_state(const char *msg, arm_saved_state_t *ss) {
  ...
  if (twice > 0) {
    twice--;
    SOCD_TRACE_XNU(KERNEL_STATE_PANIC, SOCD_TRACE_MODE_STICKY_TRACEPOINT,
                   ADDR(state->pc), PACK_LSB(VALUE(state->lr), VALUE(ss_valid)),
  ...
  panic_plain(
      ...
      "\t  pc:  0x%016llx cpsr: 0x%08x         esr: 0x%016llx  far: "
      "0x%016llx\n",
      msg, state->pc, state->lr, ss, (ss_valid ? "" : " INVALID"), state->x[0],
      state->x[1], state->x[2], state->x[3], state->x[4], state->x[5],
      state->x[6], state->x[7], state->x[8], state->x[9], state->x[10],
      state->x[11], state->x[12], state->x[13], state->x[14], state->x[15],
      state->x[16], state->x[17], state->x[18], state->x[19], state->x[20],
      state->x[21], state->x[22], state->x[23], state->x[24], state->x[25],
      state->x[26], state->x[27], state->x[28], state->fp, state->lr, state->sp,
      state->pc, state->cpsr, state->esr, state->far);
}

void sleh_synchronous_sp1(arm_context_t *context, uint64_t esr,
  ...
  case ESR_EC_UNCATEGORIZED: {
    ...
    panic_with_thread_kernel_state(
        "Unexpected debugger trap while SP1 selected", state);
#endif /* (DEVELOPMENT || DEBUG) */
  }
  default:
    panic_with_thread_kernel_state(
        "Synchronous exception taken while SP1 selected", state);
  }
}

__attribute__((noreturn)) void thread_exception_return() {
  thread_t thread = current_thread();
  if (thread->machine.exception_trace_code != 0) {
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE,
        MACHDBG_CODE(DBG_MACH_EXCP_SYNC_ARM,
                     thread->machine.exception_trace_code) |
            DBG_FUNC_END,
        0, 0, 0, 0, 0);
 ...
 */
static inline int sleh_get_preemption_level(void) {
...
static inline bool is_platform_error(uint64_t esr) {
  esr_exception_class_t class = ESR_EC(esr);
...
void sleh_synchronous(arm_context_t *context, uint64_t esr, vm_offset_t far,
                      __unused bool did_initiate_panic_lockdown) {
  esr_exception_class_t class = ESR_EC(esr);
  arm_saved_state_t *state = &context->ss;
  thread_t thread = current_thread();
#if MACH_ASSERT
   ...
   * that this path will itself trigger a DTrace probe, which would lead to
   * infinite probe recursion.
   */
  if (__improbable((class == ESR_EC_UNCATEGORIZED) && tempDTraceTrapHook &&
                   (tempDTraceTrapHook(EXC_BAD_INSTRUCTION, state, 0, 0) ==
                    KERN_SUCCESS))) {
#if CONFIG_SPTM
  ...
  if (is_user) {
    ...
    case ESR_EC_IABORT_EL0:
      /* If this is a SEA, since we can't trust FnV, just clear FAR from the
       * save area. */
    ...
    thread->machine.exception_trace_code =
        (uint16_t)(ARM64_KDBG_CODE_USER | class);
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE,
        MACHDBG_CODE(DBG_MACH_EXCP_SYNC_ARM,
                     thread->machine.exception_trace_code) |
    ...
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE,
        MACHDBG_CODE(DBG_MACH_EXCP_SYNC_ARM, ARM64_KDBG_CODE_KERNEL | class) |
            DBG_FUNC_START,
        esr, VM_KERNEL_ADDRHIDE(far),
        VM_KERNEL_UNSLIDE(get_saved_state_pc(state)), 0, 0);
  }
...
#else
    matching_fault_pc = (saved_expected_fault_pc == get_saved_state_pc(state));
#endif /* ptrauth_call */
    if (saved_expected_fault_addr == far || matching_fault_pc) {
  ...
  case ESR_EC_DABORT_EL0:
    handle_abort(state, esr, far, inspect_data_abort, handle_user_abort,
  ...
  case ESR_EC_MSR_TRAP:
    ...
    break;
/**
 * Some APPLEVIRTUALPLATFORM targets do not specify armv8.6, but it's still
 * possible for them to be hosted by a host that implements ARM_FPAC. There's no
 * way for such a host to disable it or trap it without substantial performance
 * penalty. Therefore, the FPAC handler here needs to be built into the guest
 * kernels to prevent the exception to fall through.
 */
#if __has_feature(ptrauth_calls)
  ...
  case ESR_EC_IABORT_EL0:
    handle_abort(state, esr, far, inspect_instruction_abort, handle_user_abort,
                 expected_fault_handler);
    break;
  ...
  case ESR_EC_DABORT_EL1:
    handle_abort(state, esr, far, inspect_data_abort, handle_kernel_abort,
                 expected_fault_handler);
    break;
...
#if CONFIG_XNUPOST
    if (!is_user && (expected_fault_handler != NULL) &&
  ...
  case ESR_EC_SW_STEP_DEBUG_EL1:
    panic_with_thread_kernel_state(
        "Software Step Debug exception from kernel. Panic (by design)", state);
    __builtin_unreachable();
  ...
  case ESR_EC_WATCHPT_MATCH_EL1:
    /*
     * If we hit a watchpoint in kernel mode, probably in a copyin/copyout which
     * we don't want to abort.  Turn off watchpoints and keep going; we'll turn
     * them back on in return_from_exception..
     */
    if (FSC_DEBUG_FAULT == ISS_SSDE_FSC(esr)) {
      arm_debug_set(NULL);
  ...
  case ESR_EC_ILLEGAL_INSTR_SET:
    panic("Illegal instruction set exception. state=%p class=%u esr=%llu "
          "far=%p spsr=0x%x",
  ...
  if (preemption_level != sleh_get_preemption_level()) {
    panic("synchronous exception changed preemption level from %d to %d",
          preemption_level, sleh_get_preemption_level());
  }
#endif
}
 ...
 */
static void handle_uncategorized(arm_saved_state_t *state) {
...
#if __has_feature(ptrauth_calls)
static inline const char *ptrauth_key_to_string(ptrauth_key key) {
  switch (key) {
...
static const char *ptrauth_handle_brk_trap(void *tstate, uint16_t comment) {
...
static const char *xnu_soft_trap_handle_breakpoint(void *tstate,
                                                   uint16_t comment) {
#if CONFIG_UBSAN_MINIMAL
...
static const char *xnu_hard_trap_handle_breakpoint(void *tstate,
                                                   uint16_t comment) {
  kernel_panic_reason_t pr = PERCPU_GET(panic_reason);
  ...
  case XNU_HARD_TRAP_SAFE_UNLINK:
    snprintf(pr->buf, sizeof(pr->buf), "panic: corrupt list around element %p",
             (void *)state->x[8]);
    return pr->buf;
  ...
  case XNU_HARD_TRAP_ASSERT_FAILURE:
    ...
    panic_assert_format(pr->buf, sizeof(pr->buf),
                        (struct mach_assert_hdr *)state->x[8], state->x[16],
                        state->x[17]);
    return pr->buf;
...
KERNEL_BRK_DESCRIPTOR_DEFINE(libcxx_desc,
                             ...
                             .base = LIBCXX_TRAP_START, .max = LIBCXX_TRAP_END,
                             .options = BRK_TELEMETRY_OPTIONS_FATAL_DEFAULT,
                             .handle_breakpoint = NULL);

#if HAS_TELEMETRY_KERNEL_BRK
KERNEL_BRK_DESCRIPTOR_DEFINE(
    xnu_soft_traps_desc, .type = TRAP_TELEMETRY_TYPE_KERNEL_BRK_TELEMETRY,
    .base = XNU_SOFT_TRAP_START, .max = XNU_SOFT_TRAP_END,
...
KERNEL_BRK_DESCRIPTOR_DEFINE(
    xnu_hard_traps_desc, .type = TRAP_TELEMETRY_TYPE_KERNEL_BRK_XNU,
    .base = XNU_HARD_TRAP_START, .max = XNU_HARD_TRAP_END,
    .options = BRK_TELEMETRY_OPTIONS_FATAL_DEFAULT,
...
brk_out:
  if (msg == NULL) {
    ...
    if (comment == CLANG_ARM_TRAP_BOUND_CHK) {
      msg = tsnprintf(pr->buf, sizeof(pr->buf), "Bounds safety trap");
    } else {
 ...
 * When used, system policy may decide to kill the calling process without
 * giving them opportunity to catch the exception or continue execution from a
 * signal handler. This is used to enforce security boundaries: userspace code
 * may use this mechanism to reliably terminate when internal inconsistencies
 * are detected. Note that we don't invariably terminate without giving the
 * process a say: we might only enforce such a policy if a security feature is
 * enabled, for example.
 */
...
#endif /* __has_feature(ptrauth_calls) */
    /* Available for use by system libraries when detecting disallowed
       conditions */
    {
        /* Note this uses the same range as the kernel-specific XNU_HARD_TRAP
           range */
        .base = 0xB000,
        .max = 0xBFFF,
    }};
const int user_brk_descriptor_count =
    sizeof(user_brk_descriptors) / sizeof(user_brk_descriptors[0]);
...
find_user_brk_descriptor_by_comment(uint16_t comment) {
  for (int desc_idx = 0; desc_idx < user_brk_descriptor_count; desc_idx++) {
    const user_brk_label_range_descriptor_t *des =
        &user_brk_descriptors[desc_idx];
...
static void handle_user_breakpoint(arm_saved_state_t *state,
                                   uint64_t esr __unused) {
  ...
  if (ESR_EC(esr) == ESR_EC_BRK_AARCH64) {
    ...
    const struct user_brk_label_range_descriptor *descriptor =
        find_user_brk_descriptor_by_comment(brk_label);
    /*
    ...
    if (descriptor != NULL) {
       ...
       * Conceptually a PAC failure is absolutely 'maybe unrecoverable', but
       * it's not really worth excising the discrepency from the plumbing.
       */
...
static void handle_abort(arm_saved_state_t *state, uint64_t esr,
                         vm_offset_t fault_addr,
                         abort_inspector_t inspect_abort,
  ...
  handler(state, esr, fault_addr, fault_code, fault_type,
          expected_fault_handler);
}

static void inspect_instruction_abort(uint32_t iss, fault_status_t *fault_code,
                                      vm_prot_t *fault_type) {
...
static void inspect_data_abort(uint32_t iss, fault_status_t *fault_code,
                               vm_prot_t *fault_type) {
  ...
  *fault_code = ISS_DA_FSC(iss);
   ...
   * access to the mapping, but if a cache maintenance operation that only
   * requires read access generates a permission fault, then we will not be able
   * to handle the fault regardless of whether we treat it as a read or write
   * fault.
   */
  if ((iss & ISS_DA_WNR) &&
      (!(iss & ISS_DA_CM) || is_permission_fault(*fault_code))) {
...
static inline bool fault_addr_bit(vm_offset_t fault_addr, unsigned int bit) {
 ...
 */
static bool user_fault_matches_pac_error_code(vm_offset_t fault_addr,
                                              uint64_t pc, bool data_key) {
 ...
 * in RW mode, or JCTL_EL0 in pointer signing mode.  A fault in any of these
 * trusted code paths may indicate an attack on WebKit.  Rather than letting a
 * potentially-compromised process try to handle the exception, it will be
 * killed by the kernel and a crash report will be generated.
 */
...
static void handle_sp_align(arm_saved_state_t *ss) {
  ...
  exc = EXC_BAD_ACCESS;
#if __has_feature(ptrauth_calls)
  if (user_fault_matches_pac_error_code(get_saved_state_sp(ss),
                                        get_saved_state_pc(ss), true)) {
    exc |= EXC_PTRAUTH_BIT;
  }
#endif /* __has_feature(ptrauth_calls) */
...
static void handle_wf_trap(arm_saved_state_t *state) {
  exception_type_t exc;
  mach_exception_data_type_t codes[2];
  mach_msg_type_number_t numcodes = 2;
  uint32_t instr = 0;
...
static void handle_fp_trap(arm_saved_state_t *state, uint64_t esr) {
  ...
  if (PSR64_IS_KERNEL(get_saved_state_cpsr(state))) {
    panic_with_thread_kernel_state("Floating point exception from kernel",
                                   state);
  }
  ...
  if (!fp_exceptions_enabled) {
    ...
    panic("Unrecognized floating point exception, state=%p, esr=%#llx", state,
          esr);
  }
...
static int handle_alignment_fault_from_user(arm_saved_state_t *state,
                                            kern_return_t *vmfr) {
  int ret = -1;
...
static void handle_sw_step_debug(arm_saved_state_t *state) {
  thread_t thread = current_thread();
  exception_type_t exc;
  mach_exception_data_type_t codes[2];
  mach_msg_type_number_t numcodes = 2;
  ...
  if (!PSR64_IS_USER(get_saved_state_cpsr(state))) {
    panic_with_thread_kernel_state("SW_STEP_DEBUG exception from kernel.",
                                   state);
  }

  // Disable single step and unmask interrupts (in the saved state, anticipating
  // next exception return)
  if (thread->machine.DebugData != NULL) {
    thread->machine.DebugData->uds.ds64.mdscr_el1 &= ~0x1;
  } else {
    panic_with_thread_kernel_state(
        "SW_STEP_DEBUG exception thread DebugData is NULL.", state);
  }
...
#if MACH_ASSERT
TUNABLE_WRITEABLE(self_restrict_mode_t, panic_on_jit_guard,
                  "panic_on_jit_guard", SELF_RESTRICT_NONE);
#endif /* MACH_ASSERT */

static void handle_user_abort(arm_saved_state_t *state, uint64_t esr,
                              vm_offset_t fault_addr, fault_status_t fault_code,
                              vm_prot_t fault_type,
                              expected_fault_handler_t expected_fault_handler) {
  ...
  if (__improbable(!SPSR_INTERRUPTS_ENABLED(get_saved_state_cpsr(state)))) {
    panic_with_thread_kernel_state("User abort from non-interruptible context",
                                   state);
  }

  thread->iotier_override =
      THROTTLE_LEVEL_NONE; /* Reset IO tier override before handling abort from
                              userspace */
  ...
  if (is_vm_fault(fault_code)) {
    ...
    if (!is_translation_fault(fault_code)) {
      result = arm_fast_fault(map->pmap, vm_fault_addr, fault_type,
                              (fault_code == FSC_ACCESS_FLAG_FAULT_L3), TRUE);
    }
    if (result != KERN_SUCCESS) {

      {
        ...
        result = vm_fault(map, vm_fault_addr, fault_type,
                          /* change_wiring */ FALSE, VM_KERN_MEMORY_NONE,
                          THREAD_ABORTSAFE,
    ...
    /*
     * vm_fault() should never return KERN_FAILURE for page faults from user
     * space. If it does, we're leaking preemption disables somewhere in the
     * kernel.
     */
    ...
    } else if ((result == EFAULT) && (thread->machine.recover_esr)) {
      /*
  ...
  /*
   * If the code reaches here, it means we weren't able to resolve the fault and
   * we're going to be sending the task an exception. On systems which have the
   * code signing monitor enabled, an execute fault which cannot be handled must
   * result in sending a SIGKILL to the task.
   */
  ...
  if (user_fault_matches_pac_error_code(fault_addr, get_saved_state_pc(state),
                                        is_data_abort)) {
    exc |= EXC_PTRAUTH_BIT;
  }
#endif /* __has_feature(ptrauth_calls) */

  const self_restrict_mode_t self_restrict_mode =
      user_fault_in_self_restrict_mode(thread);
  if ((self_restrict_mode != SELF_RESTRICT_NONE) &&
      task_is_jit_exception_fatal(get_threadtask(thread))) {
    int flags = PX_KTRIAGE;
    exception_info_t info = {.os_reason = OS_REASON_SELF_RESTRICT,
                             .exception_type = exc,
                             .mx_code = codes[0],
                             .mx_subcode = codes[1]};
...
#if MACH_ASSERT
    /*
     * Case: panic_on_jit_guard=1. Catch an early process creation TPRO issue
     * causing rdar://129742083. Only panic during early process creation (1
     * thread, few syscalls issued) to avoid spurious panics.
     */
    const self_restrict_mode_t self_restrict_panic_mask =
        panic_on_jit_guard & self_restrict_mode;
    bool should_panic =
        ((self_restrict_panic_mask == SELF_RESTRICT_ANY) &&
         (current_task()->thread_count == 1) && (thread->syscalls_unix < 24));

    /*
     * Modes other than ANY will force panic, skipping checks that were done in
     * the ANY case, but allowing us to filter on a more specific scenario (e.g.
     * TPRO, JIT, etc).  This is meant to catch a TPRO issue causing
     * rdar://145703251. Restrict to KERN_PROTECTION_FAILURE only to avoid
     * failures from the more frequent case of KERN_INVALID_ADDRESS that aren't
     * of interest for that radar.
     */
    should_panic |= (codes[0] == KERN_PROTECTION_FAILURE) &&
                    ((self_restrict_panic_mask & ~SELF_RESTRICT_ANY) != 0);

    printf("\nGUARD_REASON_JIT exc %d codes=<0x%llx,0x%llx> syscalls %d task "
           "%p thread %p va 0x%lx code 0x%x type 0x%x esr 0x%llx\n",
           exc, codes[0], codes[1], thread->syscalls_unix, current_task(),
           thread, fault_addr, fault_code, fault_type, esr);
    if (should_panic) {
      panic("GUARD_REASON_JIT exc %d codes=<0x%llx,0x%llx> syscalls %d task %p "
            "thread %p va 0x%lx code 0x%x type 0x%x esr 0x%llx state %p j %d t "
            "%d s user 0x%llx (0x%llx) jb 0x%llx (0x%llx)",
            exc, codes[0], codes[1], thread->syscalls_unix, current_task(),
            thread, fault_addr, fault_code, fault_type, esr, state, 0, 0, 0ull,
            0ull, 0ull, 0ull);
    }
#endif /* MACH_ASSERT */
...
panic_on_invalid_recovery_handler(arm_saved_state_t *state,
                                  struct copyio_recovery_entry *recover) {
  panic("attempt to set invalid recovery handler %p on kernel saved-state %p",
        recover, state);
}
 ...
 */
static void handle_kernel_abort_recover_with_error_code(
    arm_saved_state_t *state, uint64_t esr, vm_offset_t fault_addr,
    thread_t thread, struct copyio_recovery_entry *_Nonnull recover,
    uint64_t x0) {
  thread->machine.recover_esr = esr;
  thread->machine.recover_far = fault_addr;
  saved_state64(state)->x[0] = x0;
#if defined(HAS_APPLE_PAC)
  MANIPULATE_SIGNED_THREAD_STATE(
      state,
      ...
      "add	x1, x1, x6					\n"
      "str	x1, [x0, %[SS64_PC]]				\n",
      [recover] "r"(recover), [SIZEOF_RECOVER] "r"((sizeof(*recover))),
      [CRE_RECOVERY] "i"(offsetof(struct copyio_recovery_entry, cre_recovery)));
#else
  ptrdiff_t recover_offset =
      (uintptr_t)recover - (uintptr_t)copyio_recover_table;
...
static inline void
handle_kernel_abort_recover(arm_saved_state_t *state, uint64_t esr,
                            vm_offset_t fault_addr, thread_t thread,
                            struct copyio_recovery_entry *_Nonnull recover) {
  handle_kernel_abort_recover_with_error_code(state, esr, fault_addr, thread,
                                              recover, EFAULT);
}

static void
handle_kernel_abort(arm_saved_state_t *state, uint64_t esr,
                    vm_offset_t fault_addr, fault_status_t fault_code,
                    vm_prot_t fault_type,
                    expected_fault_handler_t expected_fault_handler) {
  thread_t thread = current_thread();
  struct copyio_recovery_entry *recover =
      find_copyio_recovery_entry(get_saved_state_pc(state));
...
#if CONFIG_DTRACE
  if (is_vm_fault(fault_code) &&
      thread->t_dtrace_inprobe) { /* Executing under dtrace_probe? */
    if (dtrace_tally_fault(
            fault_addr)) { /* Should a fault under dtrace be ignored? */
      /*
       * Point to next instruction, or recovery handler if set.
       */
      if (recover) {
        handle_kernel_abort_recover(state, esr, VM_USER_STRIP_PTR(fault_addr),
                                    thread, recover);
      } else {
        add_saved_state_pc(state, 4);
      }
      ...
      panic_with_thread_kernel_state("Unexpected page fault under dtrace_probe",
                                     state);
    }
  }
#endif
  ...
  if (is_vm_fault(fault_code)) {
    ...
    if (VM_KERNEL_ADDRESS(fault_addr) || thread == THREAD_NULL ||
       ...
       * the kernel map.  If the fault was taken against a userspace VA,
       * indicating an unprotected access to user address space, vm_fault()
       * should fail and ultimately lead to a panic here.
       */
      map = kernel_map;
      ...
      /*
       * If kernel tagging is enabled, canonicalize the address here, so that we
       * have a chance to find it in the VM ranges. Do not mess with exec fault
       * cases.
       */
      if (!((fault_type)&VM_PROT_EXECUTE)) {
     ...
     */
    if (__improbable(fault_addr >= physmap_base) &&
        (fault_addr < physmap_end)) {
      panic_with_thread_kernel_state(
          "Unexpected fault in kernel physical aperture", state);
    }
    if (__improbable(fault_addr >= gVirtBase &&
                     fault_addr < static_memory_end)) {
    ...
    /* check to see if it is just a pmap ref/modify fault */
    if (!is_translation_fault(fault_code)) {
      result = arm_fast_fault(map->pmap, fault_addr, fault_type,
                              (fault_code == FSC_ACCESS_FLAG_FAULT_L3), FALSE);
      if (result == KERN_SUCCESS) {
        return;
      }
    }
    ...
    /**
     * vm_fault() can be called with preemption disabled (and indeed this is
     * expected for certain copyio() scenarios), but can't safely be called with
     * interrupts disabled once the system has gone multi-threaded.  Other than
     * some early-boot situations such as startup kext loading, kernel paging
     * operations should never be triggered by non-interruptible code in the
     * first place, so a fault from such a context will ultimately produce a
     * kernel data abort panic anyway.  In these cases, skip calling vm_fault()
     * to avoid masking the real kernel panic with a failed VM locking
     * assertion.
     */
    if (__probable(SPSR_INTERRUPTS_ENABLED(get_saved_state_cpsr(state)) ||
      ...
      if (result != KERN_PROTECTION_FAILURE) {
        // VM will query this property when deciding to throttle this fault, we
        // don't want to throttle kernel faults for copyio faults. The presence
        // of a recovery entry is used as a proxy for being in copyio code.
...
static void handle_svc(arm_saved_state_t *state) {
...
#define TRACE_SYSCALL 1
  ...
  thread->iotier_override =
      THROTTLE_LEVEL_NONE; /* Reset IO tier override before handling SVC from
                              userspace */
...
static void handle_mach_absolute_time_trap(arm_saved_state_t *state) {
  uint64_t now = mach_absolute_time();
  saved_state64(state)->x[0] = now;
}

static void handle_mach_continuous_time_trap(arm_saved_state_t *state) {
  uint64_t now = mach_continuous_time();
  saved_state64(state)->x[0] = now;
}

__attribute__((noreturn)) static void handle_msr_trap(arm_saved_state_t *state,
                                                      uint64_t esr) {
  exception_type_t exception = EXC_BAD_INSTRUCTION;
  mach_exception_data_type_t codes[2] = {EXC_ARM_UNDEFINED};
  mach_msg_type_number_t numcodes = 2;
...
#if __has_feature(ptrauth_calls)
static void stringify_gpr(unsigned int r, char reg[4]) {
  switch (r) {
  case 29:
    strncpy(reg, "fp", 4);
    return;
...
static void autxx_instruction_extract_reg(uint32_t instr, char reg[4]) {
  unsigned int rd = ARM64_INSTR_AUTxx_RD_GET(instr);
  stringify_gpr(rd, reg);
}

static const char *autix_system_instruction_extract_reg(uint32_t instr) {
...
static void bxrax_instruction_extract_reg(uint32_t instr, char reg[4]) {
...
#define MAX_PAC_MSG_FMT BXRAX_MSG_FMT
    ...
    char msg[strlen(MAX_PAC_MSG_FMT) - strlen("%s") + strlen("IA") -
             strlen("%s") + strlen("xzr") + 1];
...
__attribute__((noreturn)) static void handle_bti_fail(arm_saved_state_t *state,
                                                      uint64_t esr) {
...
static void handle_user_trapped_instruction32(arm_saved_state_t *state,
                                              uint64_t esr) {
  exception_type_t exception = EXC_BAD_INSTRUCTION;
  mach_exception_data_type_t codes[2] = {EXC_ARM_UNDEFINED};
  mach_msg_type_number_t numcodes = 2;
  ...
  if (is_saved_state64(state)) {
    panic("ESR (0x%llx) for instruction trapped from U32, but saved state is "
          "64-bit.",
          esr);
  }

  if (PSR64_IS_KERNEL(get_saved_state_cpsr(state))) {
    panic("ESR (0x%llx) for instruction trapped from U32, actually came from "
          "kernel?",
          esr);
  }
...
static void handle_simd_trap(arm_saved_state_t *state, uint64_t esr) {
  exception_type_t exception = EXC_BAD_INSTRUCTION;
  mach_exception_data_type_t codes[2] = {EXC_ARM_UNDEFINED};
  mach_msg_type_number_t numcodes = 2;
  uint32_t instr = 0;
  ...
  if (PSR64_IS_KERNEL(get_saved_state_cpsr(state))) {
    panic(
        "ESR (0x%llx) for SIMD trap from userland, actually came from kernel?",
        esr);
  }
...
void sleh_irq(arm_saved_state_t *state) {
  ...
  cdp->interrupt_handler(cdp->interrupt_target, cdp->interrupt_refCon,
                         cdp->interrupt_nub, cdp->interrupt_source);
#endif
...
#if MACH_ASSERT
  if (preemption_level != sleh_get_preemption_level()) {
    panic("irq handler %p changed preemption level from %d to %d",
          cdp->interrupt_handler, preemption_level,
          sleh_get_preemption_level());
  }
#endif
}

void sleh_fiq(arm_saved_state_t *state) {
...
#if MACH_ASSERT
  if (preemption_level != sleh_get_preemption_level()) {
    panic("fiq type %u changed preemption level from %d to %d", type,
          preemption_level, sleh_get_preemption_level());
  }
#endif
}
...
void sleh_serror(arm_context_t *context, uint64_t esr, vm_offset_t far) {
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCP_SERR_ARM, 0) | DBG_FUNC_START, esr,
...
#if MACH_ASSERT
  if (preemption_level != sleh_get_preemption_level()) {
    panic("serror changed preemption level from %d to %d", preemption_level,
          sleh_get_preemption_level());
  }
#endif
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCP_SERR_ARM, 0) | DBG_FUNC_END, esr,
               VM_KERNEL_ADDRHIDE(far));
}
...
__attribute__((noreturn)) void thread_syscall_return(kern_return_t error) {
  thread_t thread;
...
#if MACH_ASSERT
  kern_allocation_name_t prior __assert_only =
      thread_get_kernel_state(thread)->allocation_name;
  assertf(prior == NULL, "thread_set_allocation_name(\"%s\") not cleared",
          kern_allocation_get_name(prior));
#endif /* MACH_ASSERT */
...
void syscall_trace(struct arm_saved_state *regs __unused) {
  /* kprintf("syscall: %d\n", saved_state64(regs)->x[16]);  */
}

static void sleh_interrupt_handler_prologue(arm_saved_state_t *state,
                                            unsigned int type) {
  const bool is_user = PSR64_IS_USER(get_saved_state_cpsr(state));

  if (is_user == true) {
    /* Sanitize FAR (only if the interrupt occurred while the CPU was in
     * usermode) */
    saved_state64(state)->far = 0;
  }
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCP_INTR, 0) | DBG_FUNC_START, 0, pc,
               is_user, type);
}
...
void sleh_invalid_stack(arm_context_t *context, uint64_t esr __unused,
                        vm_offset_t far __unused) {
  thread_t thread = current_thread();
  ...
  if ((sp < kernel_stack_bottom) && (sp >= (kernel_stack_bottom - PAGE_SIZE))) {
    panic_with_thread_kernel_state(
        "Invalid kernel stack pointer (probable overflow).", &context->ss);
  }
...
static const char *handle_recoverable_kernel_trap(__unused void *tstate,
                                                  uint16_t comment) {
...
KERNEL_BRK_DESCRIPTOR_DEFINE(
    test_desc, .type = TRAP_TELEMETRY_TYPE_KERNEL_BRK_TEST,
    .base = TEST_RECOVERABLE_SOFT_TRAP, .max = TEST_RECOVERABLE_SOFT_TRAP,
    .options = BRK_TELEMETRY_OPTIONS_RECOVERABLE_DEFAULT(
        /* enable_telemetry */ false),
    .handle_breakpoint = handle_recoverable_kernel_trap);

static int recoverable_kernel_trap_test(__unused int64_t in, int64_t *out) {
...
bool sleh_panic_lockdown_should_initiate_el1_sp0_sync(uint64_t esr,
  ...
  case ESR_EC_DABORT_EL1: {
    const struct copyio_recovery_entry *cre = find_copyio_recovery_entry(elr);
    if (cre) {
