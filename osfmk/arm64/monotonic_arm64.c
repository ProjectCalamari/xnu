...
/*
 * PMC[0-1] are the 48/64-bit fixed counters -- S3_2_C15_C0_0 is cycles and
 * S3_2_C15_C1_0 is instructions (see arm64/monotonic.h).
 *
...
#endif
#define PMCR1_ALL_EN(CTR)                                                      \
  (PMCR1_EL0A32_EN(CTR) | PMCR1_EL0A64_EN(CTR) | S3_1_C15_C1_0A64_EN(CTR) |    \
   PMCR1_EL3A64_EN(CTR))

/* fixed counters always count in all modes */
#define PMCR1_INIT (PMCR1_ALL_EN(CYCLES) | PMCR1_ALL_EN(INSTRS))

static inline void core_init_execution_modes(void) {
  uint64_t pmcr1;
...
static int core_init(__unused mt_device_t dev) {
  /* the dev node interface to the core counters is still unsupported */
  return ENOTSUP;
}

struct mt_cpu *mt_cur_cpu(void) { return &getCpuDatap()->cpu_monotonic; }
...
void mt_core_set_snap(unsigned int ctr, uint64_t count) {
  switch (ctr) {
  case 0:
...
static void core_set_enabled(void) {
  uint32_t kpc_mask =
      kpc_get_running() & (KPC_CLASS_CONFIGURABLE_MASK | KPC_CLASS_POWER_MASK);
...
static void core_idle(__unused cpu_data_t *cpu) {
  assert(cpu != NULL);
...
static uint64_t _upmcr0_value(void) {
#if UPMU_9BIT_SELECTORS
 ...
 * of the LLCs.
 */
static unsigned int uncore_nmonitors(void) {
#if UNCORE_PER_CLUSTER
...
static unsigned int uncmon_get_curid(void) {
#if UNCORE_PER_CLUSTER
...
static int uncmon_lock(struct uncore_monitor *mon) {
  int intrs_en = ml_set_interrupts_enabled(FALSE);
  lck_spin_lock(&mon->um_lock);
  return intrs_en;
}

static void uncmon_unlock(struct uncore_monitor *mon, int intrs_en) {
  lck_spin_unlock(&mon->um_lock);
  (void)ml_set_interrupts_enabled(intrs_en);
}

static bool uncmon_is_remote(unsigned int monid) {
  if (monid >= MAX_NMONITORS) {
    panic("monotonic: %s: invalid monid %u (> %u)", __FUNCTION__, monid,
          MAX_NMONITORS);
  }
 ...
 */
static inline void uncmon_set_counting_locked_l(__unused unsigned int monid,
                                                uint64_t enctrmask) {
  /*
 ...
 * Turn counting on for counters set in the `enctrmask` and off, otherwise.
 */
static inline void uncmon_set_counting_locked_r(unsigned int monid,
                                                uint64_t enctrmask) {
...
static void _broadcast_block_trampoline(void *blk) {
...
static inline void uncmon_write_counter_locked_l(__unused unsigned int monid,
                                                 unsigned int ctr,
                                                 uint64_t count) {
...
uintptr_t upmc_offs[UNCORE_NCTRS] = {
    [0] = 0x4100,  [1] = 0x4248,  [2] = 0x4110,  [3] = 0x4250,
    [4] = 0x4120,  [5] = 0x4258,  [6] = 0x4130,  [7] = 0x4260,
    [8] = 0x4140,  [9] = 0x4268,  [10] = 0x4150, [11] = 0x4270,
    [12] = 0x4160, [13] = 0x4278, [14] = 0x4170, [15] = 0x4280,
};

static inline uint64_t uncmon_read_counter_locked_r(unsigned int mon_id,
                                                    unsigned int ctr) {
...
static inline void uncmon_write_counter_locked_r(unsigned int mon_id,
                                                 unsigned int ctr,
                                                 uint64_t count) {
  assert(count < UPMC_MAX);
  assert(ctr < UNCORE_NCTRS);
...
static inline void uncmon_update_locked(unsigned int monid, unsigned int ctr) {
  struct uncore_monitor *mon = &uncore_monitors[monid];
  if (!mon->um_sleeping) {
    uint64_t snap = 0;
#if UNCORE_PER_CLUSTER
...
#endif /* UNCORE_PER_CLUSTER */
            ,
            ctr, monid, mon->um_snaps[ctr], snap,
...
static inline void uncmon_program_events_locked_l(unsigned int monid) {
  /*
   ...
   */
  CTRL_REG_SET("S3_7_C15_C3_4", uncore_config.uc_cpu_masks[monid].uccm_regs[0]);
  CTRL_REG_SET("S3_7_C15_C4_4", uncore_config.uc_cpu_masks[monid].uccm_regs[1]);

#if UNCORE_NCTRS > 8
  CTRL_REG_SET("S3_7_C15_C8_5", uncore_config.uc_cpu_masks[monid].uccm_regs[2]);
  CTRL_REG_SET("S3_7_C15_C9_5", uncore_config.uc_cpu_masks[monid].uccm_regs[3]);
#endif /* UNCORE_NCTRS > 8 */
}

#if UNCORE_PER_CLUSTER

static inline void uncmon_program_events_locked_r(unsigned int monid) {
  const uintptr_t upmesr_offs[2] = {
      [0] = 0x41b0,
      [1] = 0x41b8,
  };
...
static void uncmon_clear_int_locked_l(__unused unsigned int monid) {
  __builtin_arm_wsr64("S3_7_C15_C6_4", 0);
}

#if UNCORE_PER_CLUSTER

static void uncmon_clear_int_locked_r(unsigned int monid) {
 ...
 */
static uint64_t uncmon_get_pmi_mask(unsigned int monid) {
...
static void uncmon_init_locked_l(unsigned int monid) {
  /*
  ...
  uncmon_set_counting_locked_l(monid, mt_uncore_enabled ? _upmcr0_value() : 0);
}
...
static void uncmon_init_locked_r(unsigned int monid) {
  const uintptr_t upmpcm_off = 0x1010;

  *(uint64_t *)(acc_impl[monid] + upmpcm_off) = uncmon_get_pmi_mask(monid);
  uncmon_set_counting_locked_r(monid, mt_uncore_enabled ? _upmcr0_value() : 0);
}
 ...
 */
static int uncore_init(__unused mt_device_t dev) {
#if HAS_UNCORE_CTRS
  assert(MT_NDEVS > 0);
...
static void uncmon_read_all_counters(unsigned int monid, uint64_t ctr_mask,
                                     uint64_t *counts) {
  struct uncore_monitor *mon = &uncore_monitors[monid];
 ...
 */
static int uncore_read(uint64_t ctr_mask, uint64_t *counts_out) {
  assert(ctr_mask != 0);
 ...
 * monitors.
 */
static int uncore_add(struct monotonic_config *config, uint32_t *ctr_out) {
...
#endif /* !UNCORE_PER_CLUSTER */
    uncore_config.uc_cpu_masks[i].uccm_masks[ctr] =
        (uint16_t)(cpu_mask >> shift);
  }
 ...
 * counting.
 */
static void uncore_reset(void) {
  mt_uncore_enabled = false;

  if (!mt_uncore_suspended_cpd) {
    /* If we haven't already suspended CPD, we need to do so now to ensure we
     * can issue remote reads to every cluster. */
    suspend_cluster_powerdown();
    mt_uncore_suspended_cpd = true;
  }
...
static void uncmon_set_enabled_l_locked(unsigned int monid, bool enable) {
  struct uncore_monitor *mon = &uncore_monitors[monid];
#pragma unused(mon)
  LCK_SPIN_ASSERT(&mon->um_lock, LCK_ASSERT_OWNED);
...
__unused static void uncmon_set_enabled_r_locked(unsigned int monid,
                                                 bool enable) {
  struct uncore_monitor *mon = &uncore_monitors[monid];
#pragma unused(mon)
  LCK_SPIN_ASSERT(&mon->um_lock, LCK_ASSERT_OWNED);
...
static void uncore_set_enabled(bool enable) {
...
static void uncore_fiq(uint64_t upmsr) {
  /*
   ...
   */
  uncmon_set_counting_locked_l(monid, uncore_active_ctrs & ~disable_ctr_mask);
...
static void uncore_restore(void) {
  if (!uncore_active_ctrs) {
    return;
  }
...
void mt_early_init(void) { topology_info = ml_get_topology_info(); }

void mt_cpu_idle(cpu_data_t *cpu) { core_idle(cpu); }
...
void mt_cpu_down(cpu_data_t *cpu) { mt_cpu_idle(cpu); }

void mt_cpu_up(cpu_data_t *cpu) { mt_cpu_run(cpu); }
...
uint64_t mt_count_pmis(void) {
  ...
  for (unsigned int i = 0; i < topology_info->num_cpus; i++) {
    cpu_data_t *cpu =
        (cpu_data_t *)CpuDataEntries[topology_info->cpus[i].cpu_id]
            .cpu_data_vaddr;
...
static void mt_cpu_pmi(cpu_data_t *cpu, uint64_t pmcr0) {
  assert(cpu != NULL);
  assert(ml_get_interrupts_enabled() == FALSE);
...
#pragma unused(pmcr0)
  ...
  for (unsigned int i = 0; i < MT_CORE_NFIXED; i++) {
    ...
    if (mt_microstackshots && mt_microstackshot_ctr == i) {
      ...
      KDBG_RELEASE(KDBG_EVENTID(DBG_MONOTONIC, DBG_MT_DEBUG, 2), i, count);
    }
  }
  ...
  if (pmsr_after_handling != 0) {
    ...
    panic("monotonic: PMI status not cleared on exit from handler, "
          ...
          "PMCR0 = 0x%llx, PMC%d = 0x%llx%s",
          pmsr, pmsr_after_handling, handled,
          __builtin_arm_rsr64("S3_1_C15_C0_0"), first_ctr_ovf, count, extra);
  }
#endif /* MACH_ASSERT */
...
#if CPMU_AIC_PMI
void mt_cpmu_aic_pmi(cpu_id_t source) {
...
static void mt_microstackshot_start_remote(__unused void *arg) {
...
static_assert((sizeof(mt_devices) / sizeof(mt_devices[0])) == MT_NDEVS,
