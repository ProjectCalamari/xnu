...
// as a workaround for <rdar://problem/70121432> References between different
// compile units in xnu shouldn't go through GOT all of the extern symbols that
// we refer to in this file have to be declared with hidden visibility
extern IOHibernateImageHeader *gIOHibernateCurrentHeader
    __attribute__((visibility("hidden")));
extern const uint32_t ccsha256_initial_state[8]
    __attribute__((visibility("hidden")));
extern void AccelerateCrypto_SHA256_compress(ccdigest_state_t state,
                                             size_t numBlocks, const void *data)
    __attribute__((visibility("hidden")));
extern void ccdigest_final_64be(const struct ccdigest_info *di, ccdigest_ctx_t,
...
uintptr_t hibernate_restore_phys_page(uint64_t src, uint64_t dst, uint32_t len,
                                      __unused uint32_t procFlags) {
...
void pal_hib_restore_pal_state(__unused uint32_t *arg) {}

void pal_hib_resume_init(pal_hib_ctx_t *ctx, hibernate_page_list_t *map,
                         uint32_t *nextFree) {
#pragma unused(ctx, map, nextFree)
}

void pal_hib_restored_page(pal_hib_ctx_t *ctx, pal_hib_restore_stage_t stage,
                           ppnum_t ppnum) {
#pragma unused(ctx, stage, ppnum)
}

void pal_hib_patchup(pal_hib_ctx_t *ctx) {
#pragma unused(ctx)
...
void pal_hib_decompress_page(void *src, void *dst, void *scratch,
                             unsigned int compressedSize) {
  const void *wkdmSrc;
  if (((uint64_t)src) & 63) {
    // the wkdm instruction requires that our source buffer be aligned, so copy
    // into an aligned buffer if necessary
...
static void hib_bzero(volatile void *s, size_t n) {
...
static uint64_t allocate_page(map_ctx *ctx) {
  // pages that were unnecessary for preservation when we entered hibernation
  // are marked as free in ctx->bitmap, so they are available for scratch usage
  // during resume; here, we "borrow" one of these free pages to use as part of
  // our temporary page tables
...
static void create_map_entries(map_ctx *ctx, uint64_t vaddr, uint64_t paddr,
                               uint64_t size, uint64_t map_flags) {
  // if we've set gHibTramp.memSlide, we should already be running with the MMU
  // on; in this case, we don't permit further modification to the page table
  ...
  while (size) {
    ...
    if ((level >= 2) && size >= level_size &&
      ...
      HIB_ASSERT((entry & (ARM_TTE_VALID | ARM_TTE_TYPE_MASK)) !=
                 (ARM_TTE_VALID |
                  ARM_TTE_TYPE_BLOCK)); // Breaking down blocks not implemented
      uint64_t sub_base = entry & ARM_TTE_TABLE_MASK;
      if (!sub_base) {
        sub_base = allocate_page(ctx);
        HIB_ASSERT((sub_base & PAGE_GRANULE_MASK) == 0);
        table_base[index] =
            sub_base | ARM_LPAE_NSTABLE | ARM_TTE_TYPE_TABLE | ARM_TTE_VALID;
      }
...
static void map_range_start_end(map_ctx *ctx, uint64_t start, uint64_t end,
                                uint64_t slide, uint64_t flags) {
  HIB_ASSERT(end >= start);
  create_map_entries(ctx, start + slide, start, end - start, flags);
}

#define MAP_FLAGS_COMMON                                                       \
  (ARM_PTE_AF | ARM_PTE_NS | ARM_TTE_VALID | ARM_PTE_SH(SH_OUTER_MEMORY) |     \
   ARM_PTE_ATTRINDX(CACHE_ATTRINDX_WRITEBACK))
#define MAP_DEVICE                                                             \
  (ARM_PTE_AF | ARM_TTE_VALID | ARM_PTE_PNX | ARM_PTE_NX |                     \
   ARM_PTE_SH(SH_NONE) | ARM_PTE_ATTRINDX(CACHE_ATTRINDX_DISABLE))
#define MAP_RO                                                                 \
  (MAP_FLAGS_COMMON | ARM_PTE_PNX | ARM_PTE_NX | ARM_PTE_AP(AP_RONA))
#define MAP_RW (MAP_FLAGS_COMMON | ARM_PTE_PNX | ARM_PTE_NX)
#define MAP_RX (MAP_FLAGS_COMMON | ARM_PTE_AP(AP_RONA))

static void map_register_page(map_ctx *ctx, vm_address_t regPage) {
...
static void
iterate_bitmaps(const map_ctx *ctx,
                bool (^callback)(const hibernate_bitmap_t *bank_bitmap)) {
  ...
  for (uint32_t bank = 0; bank < ctx->bitmap->bank_count; bank++) {
    ...
    bank_bitmap =
        (hibernate_bitmap_t *)&bank_bitmap->bitmap[bank_bitmap->bitmapwords];
  }
}

// during hibernation resume, we can't use the original kernel page table
// (because we don't know what it was), so we instead create a temporary page
// table to use during hibernation resume; since the original kernel page table
// was part of DRAM, it will be restored by the time we're done with hibernation
// resume, at which point we can jump through the reset vector to reload the
// original page table
void pal_hib_resume_tramp(uint32_t headerPpnum) {
  ...
  __block map_ctx ctx = {};
  uint64_t map_phys =
      header_phys + (offsetof(IOHibernateImageHeader, fileExtentMap) +
                     header->fileExtentMapSize +
                     ptoa_32(header->restore1PageCount) + header->previewSize);
  ...
  // During hibernation resume, we create temporary mappings that do not collide
  // with where any of the kernel mappings were originally. Technically,
  // non-collision isn't a requirement, but doing this means that if some code
  // accidentally jumps to a VA in the original kernel map, it won't be present
  // in our temporary map and we'll get an exception when jumping to an unmapped
  // address. The base address of our temporary mappings is adjusted by a random
  // amount as a "poor-man's ASLR". We don’t have a good source of random
  // numbers in this context, so we just use some of the bits from one of
  // imageHeaderHMMAC, which should be random enough.
  uint16_t rand = (uint16_t)(((header->imageHeaderHMAC[0]) << 8) |
                             header->imageHeaderHMAC[1]);
  uint64_t mem_slide = gHibernateGlobals.kernelSlide -
                       (phys_end - phys_start) * 4 - rand * 256 * PAGE_SIZE;
  ...
  for (size_t i = 0; i < NUM_HIBSEGINFO_SEGMENTS; i++) {
    ...
    if (size) {
      ...
      if (protection != VM_PROT_NONE) {
        ...
        // map the segments described in machine_header at their original
        // locations
        ...
        map_range_start_end(&ctx, seg_start, seg_end,
                            gHibernateGlobals.kernelSlide, map_flags);
  ...
  // map the rest of kernel memory (the pages that come before and after our
  // segments) as RW
...
#define dockchannel_uart_base gHibernateGlobals.dockChannelRegPhysBase
    vm_address_t dockChannelRegPhysBase = (vm_address_t)rDOCKCHANNELS_DEV_WSTAT(
