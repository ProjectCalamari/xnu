...
void arm_get_matrix_cpu_state(struct arm_matrix_cpu_state *cpu_state) {
#if HAS_ARM_FEAT_SME
 ...
 *
 */
void consider_machine_collect(void) { pmap_gc(); }
...
arm_sme_saved_state_t *machine_thread_get_sme_state(thread_t thread) {
...
static void
machine_save_sme_context(thread_t old, arm_sme_saved_state_t *old_sme_ss,
...
static void
machine_restore_sme_context(thread_t new,
                            const arm_sme_saved_state_t *new_sme_ss,
                            const struct arm_matrix_cpu_state *cpu_state) {
...
static void machine_switch_matrix_context(thread_t old, thread_t new) {
...
static inline void machine_thread_switch_cpu_data(thread_t old, thread_t new) {
  /*
 ...
 *
 * Helper function used by machine_switch_context and machine_stack_handoff to
 * switch the extended context and switch the pmap if necessary.
 *
 */

static inline void machine_switch_pmap_and_extended_context(thread_t old,
                                                            thread_t new) {
  pmap_t new_pmap;
  ...
  bool sec_override_changed = false;
  ...
  if (pmap_changed || sec_override_changed) {
     ...
     * it may be migrated to a different CPU between the completion of the
     * relevant maintenance instruction and the synchronizing DSB.   ARM
     * requires that the synchronizing DSB must be issued *on the PE that issued
     * the maintenance instruction* in order to guarantee completion of the
     * instruction and visibility of its effects. Issue DSB here to enforce that
     * guarantee.  We only do this for the case in which the pmap isn't
     * changing, as we expect pmap_switch() to issue DSB when it updates TTBR0.
     * Note also that cache maintenance may be performed in userspace, so we
     * cannot further limit this operation e.g. by setting a per-thread flag to
     * indicate a pending kernel TLB or cache maintenance instruction.
     */
    __builtin_arm_dsb(DSB_ISH);

    /*
     * An ISB is needed for similar userspace reasons to the DSB above. Unlike
     * the DSB case, the context synchronization needs to happen on the CPU the
     * 'old' thread will later be scheduled on. We can rely on the fact that
     * when 'old' is later scheduled, whatever thread it is replacing will go
     * through this function as 'old' and will issue this ISB on its behalf.
     */
...
#define machine_thread_create_kprintf(x...)                                    \
  ...
  if (task != kernel_task) {
    ...
    arm_context_t *contextData =
        zalloc_flags(user_ss_zone, Z_WAITOK | Z_NOFAIL);
  ...
  bzero(&thread->machine.perfctrl_state,
        sizeof(thread->machine.perfctrl_state));
 ...
 *
 */
kern_return_t machine_thread_process_signature(thread_t __unused thread,
  ...
  /**
   * Make sure the machine flags are observed before the thread becomes
   * available to run in user mode, especially in the posix_spawn() path.
   */
...
#if HAS_ARM_FEAT_SME
static arm_sme_saved_state_t *zalloc_sme_saved_state(void) {
  arm_sme_saved_state_t *sme_ss =
      zalloc_flags(sme_ss_zone, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
kern_return_t machine_thread_sme_state_alloc(thread_t thread) {
  assert(arm_sme_version());

  if (thread->machine.usme) {
    panic("thread %p already has SME saved state %p", thread,
          thread->machine.usme);
  }
...
void machine_thread_sme_state_free(thread_t thread) {
...
static void
machine_thread_sme_state_dup(const arm_sme_saved_state_t *src_sme_ss,
                             thread_t target) {
  arm_sme_saved_state_t *sme_ss = zalloc_sme_saved_state();
  ...
  if (sme_ss->svcr & SVCR_SM) {
    ...
    memcpy(arm_sme_z(context), const_arm_sme_z(src_context),
           arm_sme_z_size(svl_b));
    memcpy(arm_sme_p(context, svl_b), const_arm_sme_p(src_context, svl_b),
...
#if HAVE_MACHINE_THREAD_MATRIX_STATE
void machine_thread_matrix_state_dup(thread_t target) {
...
void machine_thread_init(void) {
  ...
  if (arm_sme_version()) {
    ...
    vm_size_t size =
        arm_sme_saved_state_count(sme_svl_b) * sizeof(unsigned int);
    sme_ss_zone =
        zone_create_ext("SME saved state", size, ZC_NONE, ZONE_ID_ANY, NULL);
  }
#endif
}
 ...
 *
 */
void machine_thread_template_init(thread_t __unused thr_template) {
...
#define machine_stack_attach_kprintf(x...)                                     \
...
#endif
  void *kstackptr =
      (void *)(stack + kernel_stack_size - sizeof(struct thread_kernel_state));
  ...
  uint64_t intr = ml_pac_safe_interrupts_disable();
  asm volatile("adrp	x17, _thread_continue@page"
               "\n"
...
#define SET_DBGWCRn(n, value, accum)                                           \
  __asm__ volatile("msr DBGWCR" #n "_EL1, %[val]\n"                            \
                   "orr %[result], %[result], %[val]\n"                        \
                   : [result] "+r"(accum)                                      \
                   : [val] "r"((value)))
...
void arm_debug_set32(arm_debug_state_t *debug_state) {
  ...
  if ((all_ctrls & (ARM_DBG_CR_MODE_CONTROL_PRIVILEGED |
                    ARM_DBG_CR_HIGHER_MODE_ENABLE)) != 0) {
...
boolean_t debug_legacy_state_is_valid(arm_legacy_debug_state_t *debug_state) {
  arm_debug_info_t *debug_info = arm_debug_info();
  uint32_t i;
...
boolean_t debug_state_is_valid32(arm_debug_state32_t *debug_state) {
  arm_debug_info_t *debug_info = arm_debug_info();
  uint32_t i;
  for (i = 0; i < debug_info->num_breakpoint_pairs; i++) {
    if (0 != debug_state->bcr[i] && VM_MAX_ADDRESS32 <= debug_state->bvr[i]) {
...
boolean_t debug_state_is_valid64(arm_debug_state64_t *debug_state) {
  arm_debug_info_t *debug_info = arm_debug_info();
  uint32_t i;
  for (i = 0; i < debug_info->num_breakpoint_pairs; i++) {
    if (0 != debug_state->bcr[i] &&
        MACH_VM_MAX_ADDRESS <= debug_state->bvr[i]) {
      return FALSE;
    }
  }

  for (i = 0; i < debug_info->num_watchpoint_pairs; i++) {
    if (0 != debug_state->wcr[i] &&
        MACH_VM_MAX_ADDRESS <= debug_state->wvr[i]) {
      return FALSE;
    }
  }
  return TRUE;
 ...
 */
void copy_legacy_debug_state(arm_legacy_debug_state_t *src,
                             arm_legacy_debug_state_t *target,
                             __unused boolean_t all) {
  bcopy(src, target, sizeof(arm_legacy_debug_state_t));
}

void copy_debug_state32(arm_debug_state32_t *src, arm_debug_state32_t *target,
                        __unused boolean_t all) {
  bcopy(src, target, sizeof(arm_debug_state32_t));
}

void copy_debug_state64(arm_debug_state64_t *src, arm_debug_state64_t *target,
                        __unused boolean_t all) {
...
void machine_tecs(__unused thread_t thr) {}

int machine_csv(__unused cpuvn_e cve) { return 0; }

void arm_context_switch_requires_sync() {
  current_cpu_datap()->sync_on_cswitch = 1;
}

void arm_context_switch_sync() {
...
#if __has_feature(ptrauth_calls)
boolean_t arm_user_jop_disabled(void) { return FALSE; }
