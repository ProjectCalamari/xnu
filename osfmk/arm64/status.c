 ...
 */
void saved_state_to_thread_state64(const arm_saved_state_t *saved_state,
 ...
 */
void thread_state64_to_saved_state(const arm_thread_state64_t *ts64,
  ...
  MANIPULATE_SIGNED_USER_THREAD_STATE(
      saved_state,
      ...
      "str	w2, [x0, %[SS64_CPSR]]"
      "\n",
      [cpsr] "r"(ts64->cpsr), [preserve_mask] "i"(CPSR_PRESERVE_MASK),
...
handle_get_arm32_thread_state(thread_state_t tstate,
  ...
  (void)saved_state_to_thread_state32(saved_state,
                                      (arm_thread_state32_t *)tstate);
...
handle_get_arm64_thread_state(thread_state_t tstate,
  ...
  (void)saved_state_to_thread_state64(saved_state,
...
handle_get_arm_thread_state(thread_state_t tstate,
  ...
  arm_unified_thread_state_t *unified_state =
      (arm_unified_thread_state_t *)tstate;
...
handle_set_arm32_thread_state(const thread_state_t tstate,
  ...
  (void)thread_state32_to_saved_state((const arm_thread_state32_t *)tstate,
                                      saved_state);
  return KERN_SUCCESS;
}
...
handle_set_arm64_thread_state(const thread_state_t tstate,
  ...
  (void)thread_state64_to_saved_state((const arm_thread_state64_t *)tstate,
                                      saved_state);
  return KERN_SUCCESS;
}
...
handle_set_arm_thread_state(const thread_state_t tstate,
  ...
  const arm_unified_thread_state_t *unified_state =
      (const arm_unified_thread_state_t *)tstate;
#if __arm64__
  if (is_thread_state64(unified_state)) {
    if (!is_saved_state64(saved_state)) {
      return KERN_INVALID_ARGUMENT;
    }
    (void)thread_state64_to_saved_state(const_thread_state64(unified_state),
                                        saved_state);
  } else
#endif
  {
    ...
    (void)thread_state32_to_saved_state(const_thread_state32(unified_state),
                                        saved_state);
  }
...
#if HAS_ARM_FEAT_SME
static kern_return_t handle_get_arm_sme_state(thread_state_t tstate,
                                              mach_msg_type_number_t *count,
                                              thread_t thread) {
  if (!arm_sme_version()) {
    return KERN_INVALID_ARGUMENT;
   ...
   *
   * We don't need to do this for threads active on other CPUs, since
   * thread_get_state() forcibly preempts them before calling
...
static kern_return_t handle_get_arm_sve_z_state(thread_state_t tstate,
                                                mach_msg_type_number_t *count,
                                                thread_t thread,
                                                size_t z_offset) {
  ...
  const uint8_t *saved_state_z =
      const_arm_sme_z(&saved_state->context) + z_offset * elem_size;
...
static kern_return_t handle_get_arm_sve_p_state(thread_state_t tstate,
                                                mach_msg_type_number_t *count,
                                                thread_t thread) {
  ...
  const uint8_t *saved_state_p =
      const_arm_sme_p(&saved_state->context, saved_state->svl_b);
...
static kern_return_t handle_get_arm_za_state(thread_state_t tstate,
                                             mach_msg_type_number_t *count,
                                             thread_t thread) {
  ...
  const uint8_t *saved_state_za =
      const_arm_sme_za(&saved_state->context, saved_state->svl_b);
...
static kern_return_t handle_set_arm_sme_state(const thread_state_t tstate,
                                              mach_msg_type_number_t count,
                                              thread_t thread) {
  if (!arm_sme_version()) {
    return KERN_INVALID_ARGUMENT;
...
static kern_return_t handle_set_arm_sve_z_state(const thread_state_t tstate,
                                                mach_msg_type_number_t count,
                                                thread_t thread,
                                                size_t z_offset) {
  if (count < ARM_SVE_Z_STATE_COUNT) {
  ...
  const size_t elem_size = saved_state->svl_b;
  uint8_t *saved_state_z =
      arm_sme_z(&saved_state->context) + z_offset * elem_size;
...
static kern_return_t handle_set_arm_sve_p_state(const thread_state_t tstate,
                                                mach_msg_type_number_t count,
                                                thread_t thread) {
  if (count < ARM_SVE_P_STATE_COUNT) {
    return KERN_INVALID_ARGUMENT;
...
static kern_return_t handle_set_arm_za_state(const thread_state_t tstate,
                                             mach_msg_type_number_t count,
                                             thread_t thread) {
  if (count < ARM_SME_ZA_STATE_COUNT) {
    return KERN_INVALID_ARGUMENT;
  ...
  uint8_t *saved_state_za =
      arm_sme_za(&saved_state->context, saved_state->svl_b);
  size_t za_size = saved_state->svl_b * saved_state->svl_b;
...
static kern_return_t handle_get_arm_sme2_state(thread_state_t tstate,
                                               mach_msg_type_number_t *count,
                                               thread_t thread) {
  ...
  memcpy(sme2_tstate->zt0, saved_state->context.zt0,
         sizeof(saved_state->context.zt0));

  *count = ARM_SME2_STATE_COUNT;
  return KERN_SUCCESS;
}

static kern_return_t handle_set_arm_sme2_state(const thread_state_t tstate,
                                               mach_msg_type_number_t count,
                                               thread_t thread) {
  if (arm_sme_version() < ARM_FEAT_SME2) {
    return KERN_INVALID_ARGUMENT;
  ...
  memcpy(saved_state->context.zt0, sme2_tstate->zt0,
         sizeof(saved_state->context.zt0));
  if (thread == current_thread()) {
...
static inline uint32_t thread_generate_sigreturn_token(void *ptr,
                                                       thread_t thread) {
  user64_addr_t token = (user64_addr_t)ptr;
  token ^= (user64_addr_t)thread_get_sigreturn_token(thread);
  token = (user64_addr_t)pmap_sign_user_ptr(
      (void *)token, ptrauth_key_process_independent_data,
      ptrauth_string_discriminator("nonce"), thread->machine.jop_pid);
  token >>= 32;
...
kern_return_t machine_thread_state_convert_to_user(
  ...
  if ((current_thread()->machine.arm_machine_flags &
       ARM_MACHINE_THREAD_DISABLE_USER_JOP) ||
      !thread_is_64bit_addr(current_thread()) ||
      (thread->machine.arm_machine_flags &
       ARM_MACHINE_THREAD_DISABLE_USER_JOP) ||
  ...
  ts64->flags = 0;
  if (ts64->lr) {
    // lr might contain an IB-signed return address (strip is a no-op on
    // unsigned addresses)
    uintptr_t stripped_lr =
        (uintptr_t)ptrauth_strip((void *)ts64->lr, ptrauth_key_return_address);
    ...
    // Note that an IB-signed return address that happens to have a 0 signature
    // value will round-trip correctly even if IA-signed again below (and
    // IA-authd later)
  }
  ...
  if (preserve_flags) {
    ...
    ts64->flags |=
        (old_flags & __DARWIN_ARM_THREAD_STATE64_USER_DIVERSIFIER_MASK);
    ...
    if (random_div || task_div) {
      /* Still use random div in case of task_div to avoid leaking the secret
       * key */
      do {
  ...
  if (ts64->pc) {
    ...
    if (!kernel_signed_pc && userland_diversifier != 0) {
      discriminator =
          ptrauth_blend_discriminator((void *)(long)userland_diversifier,
                                      ptrauth_string_discriminator("pc"));
    }

    ts64->pc = (uintptr_t)pmap_sign_user_ptr(
        (void *)ts64->pc, ptrauth_key_process_independent_code, discriminator,
  ...
  if (ts64->lr &&
    ...
    ts64->lr = (uintptr_t)pmap_sign_user_ptr(
        (void *)ts64->lr, ptrauth_key_process_independent_code, discriminator,
        thread->machine.jop_pid);
  }
  if (ts64->sp) {
    ts64->sp = (uintptr_t)pmap_sign_user_ptr(
        (void *)ts64->sp, ptrauth_key_process_independent_data,
        ptrauth_string_discriminator("sp"), thread->machine.jop_pid);
  }
  if (ts64->fp) {
    ts64->fp = (uintptr_t)pmap_sign_user_ptr(
        (void *)ts64->fp, ptrauth_key_process_independent_data,
        ptrauth_string_discriminator("fp"), thread->machine.jop_pid);
  }
  ...
  if (stash_sigreturn_token) {
    if (kernel_signed_pc) {
      uint32_t token =
          thread_generate_sigreturn_token((void *)ts64->pc, thread);
      __DARWIN_ARM_THREAD_STATE64_SET_SIGRETURN_TOKEN(
          ts64, token, __DARWIN_ARM_THREAD_STATE64_SIGRETURN_PC_MASK);
    }

    if (kernel_signed_lr) {
      uint32_t token =
          thread_generate_sigreturn_token((void *)ts64->lr, thread);
      __DARWIN_ARM_THREAD_STATE64_SET_SIGRETURN_TOKEN(
          ts64, token, __DARWIN_ARM_THREAD_STATE64_SIGRETURN_LR_MASK);
    }
  }
...
CA_EVENT(pac_thread_state_exception_event, CA_STATIC_STRING(CA_PROCNAME_LEN),
         proc_name);

static void
machine_thread_state_check_pac_state(arm_thread_state64_t *ts64,
...
CA_EVENT(pac_thread_state_sigreturn_event, CA_STATIC_STRING(CA_PROCNAME_LEN),
         proc_name);

static bool
machine_thread_state_check_sigreturn_token(arm_thread_state64_t *ts64,
                                           thread_t thread) {
  task_t task = current_task();
  void *proc = get_bsdtask_info(task);
  char *proc_name = (char *)"unknown";
  ...
  if (kernel_signed_pc) {
    ...
    if (!__DARWIN_ARM_THREAD_STATE64_CHECK_SIGRETURN_TOKEN(
            ts64, token, __DARWIN_ARM_THREAD_STATE64_SIGRETURN_PC_MASK)) {
      token_matched = false;
    }
  }
...
kern_return_t machine_thread_state_convert_from_user(
  ...
  case ARM_THREAD_STATE: {
    ...
    arm_unified_thread_state_t *old_unified_state =
        (arm_unified_thread_state_t *)old_tstate;
  ...
  if (ts64->flags & __DARWIN_ARM_THREAD_STATE64_FLAGS_IB_SIGNED_LR) {
    ...
    uintptr_t stripped_lr =
        (uintptr_t)ptrauth_strip((void *)ts64->lr, ptrauth_key_return_address);
    if (ts64->lr == stripped_lr) {
      // Don't allow unsigned pointer to be passed through as is. Ignore flag
      // and treat as IA-signed below (where auth failure may poison the value).
      ts64->flags &= ~__DARWIN_ARM_THREAD_STATE64_FLAGS_IB_SIGNED_LR;
    }
    // Note that an IB-signed return address that happens to have a 0 signature
    // value will also have been IA-signed (without this flag being set) and so
    // will IA-auth correctly below.
  }
  ...
  if ((tssf_flags & TSSF_CHECK_USER_FLAGS) &&
    ...
    if (old_ts64 && old_count == count) {
      ...
      if ((tssf_flags & TSSF_ALLOW_ONLY_USER_PTRS) && kernel_signed_pc) {
        ts64->pc = old_ts64->pc;
      }
  ...
  if (tssf_flags & TSSF_CHECK_SIGRETURN_TOKEN) {
    bool token_matched =
        machine_thread_state_check_sigreturn_token(ts64, thread);
  ...
  if (random_div && old_ts64 && old_count == count) {
    ...
    userland_diversifier =
        (void *)(long)(old_ts64->flags &
                       __DARWIN_ARM_THREAD_STATE64_USER_DIVERSIFIER_MASK);
  } else if (thread_div) {
    userland_diversifier =
        (void *)(long)(thread_get_sigreturn_diversifier(thread) &
                       __DARWIN_ARM_THREAD_STATE64_USER_DIVERSIFIER_MASK);
  } else if (task_div) {
    userland_diversifier =
        (void *)(long)((get_threadtask(thread)
                            ->hardened_exception_action.signed_pc_key) &
                       __DARWIN_ARM_THREAD_STATE64_USER_DIVERSIFIER_MASK);
  }
  ...
  if (ts64->pc) {
    ...
    if (!kernel_signed_pc && userland_diversifier != 0) {
      discriminator = ptrauth_blend_discriminator(
          userland_diversifier, ptrauth_string_discriminator("pc"));
    }
    ts64->pc = (uintptr_t)pmap_auth_user_ptr(
        (void *)ts64->pc, ptrauth_key_process_independent_code, discriminator,
        thread->machine.jop_pid);
  }
  if (ts64->lr &&
    ...
    if (!kernel_signed_lr && userland_diversifier != 0) {
      discriminator = ptrauth_blend_discriminator(
          userland_diversifier, ptrauth_string_discriminator("lr"));
    }
    ts64->lr = (uintptr_t)pmap_auth_user_ptr(
        (void *)ts64->lr, ptrauth_key_process_independent_code, discriminator,
        thread->machine.jop_pid);
  }
  if (ts64->sp) {
    ts64->sp = (uintptr_t)pmap_auth_user_ptr(
        (void *)ts64->sp, ptrauth_key_process_independent_data,
        ptrauth_string_discriminator("sp"), thread->machine.jop_pid);
  }
  if (ts64->fp) {
    ts64->fp = (uintptr_t)pmap_auth_user_ptr(
        (void *)ts64->fp, ptrauth_key_process_independent_data,
        ptrauth_string_discriminator("fp"), thread->machine.jop_pid);
  }
...
#if __has_feature(ptrauth_calls)
bool machine_thread_state_is_debug_flavor(int flavor) {
  if (flavor == ARM_DEBUG_STATE || flavor == ARM_DEBUG_STATE64 ||
...
machine_thread_siguctx_pointer_convert_to_user(thread_t thread,
  ...
  if ((current_thread()->machine.arm_machine_flags &
       ARM_MACHINE_THREAD_DISABLE_USER_JOP) ||
      !thread_is_64bit_addr(current_thread())) {
    assert((thread->machine.arm_machine_flags &
            ARM_MACHINE_THREAD_DISABLE_USER_JOP) ||
           !thread_is_64bit_addr(thread));
    return KERN_SUCCESS;
  }
  ...
  if (*uctxp) {
    *uctxp = (uintptr_t)pmap_sign_user_ptr(
        (void *)*uctxp, ptrauth_key_process_independent_data,
        ptrauth_string_discriminator("uctx"), thread->machine.jop_pid);
  }
...
kern_return_t machine_thread_get_state(thread_t thread, thread_flavor_t flavor,
  ...
  case THREAD_STATE_FLAVOR_LIST_NEW:
    ...
    tstate[0] = ARM_THREAD_STATE;
    tstate[1] = ARM_VFP_STATE;
    tstate[2] = thread_is_64bit_data(thread) ? ARM_EXCEPTION_STATE64
                                             : ARM_EXCEPTION_STATE;
    tstate[3] =
        thread_is_64bit_data(thread) ? ARM_DEBUG_STATE64 : ARM_DEBUG_STATE32;
    *count = 4;
    break;
  ...
  case ARM_THREAD_STATE: {
    kern_return_t rn =
        handle_get_arm_thread_state(tstate, count, thread->machine.upcb);
    if (rn) {
      return rn;
    }
    break;
  ...
  case ARM_THREAD_STATE32: {
    ...
    kern_return_t rn =
        handle_get_arm32_thread_state(tstate, count, thread->machine.upcb);
    if (rn) {
      return rn;
    }
    break;
  ...
  case ARM_THREAD_STATE64: {
    ...
    kern_return_t rn =
        handle_get_arm64_thread_state(tstate, count, thread->machine.upcb);
    if (rn) {
      return rn;
    }
  ...
  case ARM_NEON_STATE64: {
    ...
    /* Inside streaming SVE mode, reads from Q[i] access the lower 128 bits of
     * Z[i] */
 ...
 *
 */
kern_return_t machine_thread_get_kern_state(thread_t thread,
  ...
  switch (flavor) {
  case ARM_THREAD_STATE: {
    kern_return_t rn = handle_get_arm_thread_state(
        tstate, count, getCpuDatap()->cpu_int_state);
    if (rn) {
      return rn;
    }
    break;
  ...
  case ARM_THREAD_STATE32: {
    kern_return_t rn = handle_get_arm32_thread_state(
        tstate, count, getCpuDatap()->cpu_int_state);
    if (rn) {
      return rn;
    }
...
#if __arm64__
  case ARM_THREAD_STATE64: {
    kern_return_t rn = handle_get_arm64_thread_state(
        tstate, count, getCpuDatap()->cpu_int_state);
    if (rn) {
      return rn;
    }
...
kern_return_t machine_thread_set_state(thread_t thread, thread_flavor_t flavor,
  ...
  case ARM_DEBUG_STATE: {
    ...
    for (i = 0; i < 16; i++) {
      ...
      if (((state->bcr[i] & ARM_DBGBCR_TYPE_MASK) != ARM_DBGBCR_TYPE_IVA) ||
          ((state->bcr[i] & ARM_DBG_CR_LINKED_MASK) !=
           ARM_DBG_CR_LINKED_UNLINKED) ||
          ((state->wcr[i] & ARM_DBGBCR_TYPE_MASK) != ARM_DBGBCR_TYPE_IVA) ||
          ((state->wcr[i] & ARM_DBG_CR_LINKED_MASK) !=
           ARM_DBG_CR_LINKED_UNLINKED)) {
        return KERN_PROTECTION_FAILURE;
      }
      if ((((state->bcr[i] & ARM_DBG_CR_ENABLE_MASK) ==
            ARM_DBG_CR_ENABLE_ENABLE)) ||
          ((state->wcr[i] & ARM_DBG_CR_ENABLE_MASK) ==
           ARM_DBG_CR_ENABLE_ENABLE)) {
        enabled = TRUE;
      }
    }
    ...
    if (!enabled) {
      ...
      for (i = 0; i < 16; i++) {
        ...
        thread_state->bcr[i] =
            (state->bcr[i] &
             (ARM_DBG_CR_ADDRESS_MASK_MASK | ARM_DBGBCR_MATCH_MASK |
              ARM_DBG_CR_BYTE_ADDRESS_SELECT_MASK | ARM_DBG_CR_ENABLE_MASK)) |
            ARM_DBGBCR_TYPE_IVA | ARM_DBG_CR_LINKED_UNLINKED |
            ARM_DBG_CR_SECURITY_STATE_BOTH | ARM_DBG_CR_MODE_CONTROL_USER;
        thread_state->bvr[i] = state->bvr[i] & ARM_DBG_VR_ADDRESS_MASK;
        thread_state->wcr[i] =
            (state->wcr[i] &
             (ARM_DBG_CR_ADDRESS_MASK_MASK |
              ARM_DBGWCR_BYTE_ADDRESS_SELECT_MASK |
              ARM_DBGWCR_ACCESS_CONTROL_MASK | ARM_DBG_CR_ENABLE_MASK)) |
            ARM_DBG_CR_LINKED_UNLINKED | ARM_DBG_CR_SECURITY_STATE_BOTH |
            ARM_DBG_CR_MODE_CONTROL_USER;
        thread_state->wvr[i] = state->wvr[i] & ARM_DBG_VR_ADDRESS_MASK;
      }

      thread_state->mdscr_el1 =
          0ULL; // Legacy customers issuing ARM_DEBUG_STATE dont drive single
                // stepping.
    }
  ...
  case ARM_DEBUG_STATE32:
    /* ARM64_TODO  subtle bcr/wcr semantic differences e.g. wcr and
     * ARM_DBGBCR_TYPE_IVA */
    {
      arm_debug_state32_t *state;
      boolean_t enabled = FALSE;
  ...
  if (thread->machine.upcb != NULL) {
    ...
    asm volatile("mov	x0, %[iss]"
                 ...
                 : [iss] "r"(thread->machine.upcb),
                   [usr] "r"(thread->machine.upcb->ss_64.cpsr),
...
kern_return_t machine_thread_dup(thread_t self, thread_t target,
  ...
  if (!is_corpse && is_saved_state64(self_saved_state)) {
    check_and_sign_copied_user_thread_state(target_saved_state,
                                            self_saved_state);
  }
#endif /* defined(HAS_APPLE_PAC) */
 ...
 *
 */
struct arm_saved_state *get_user_regs(thread_t thread) {
 ...
 */
void set_user_neon_reg(thread_t thread, unsigned int regno, uint128_t val) {
#if HAS_ARM_FEAT_SME
  arm_sme_saved_state_t *sme_ss = machine_thread_get_sme_state(thread);
  if (sme_ss && (sme_ss->svcr & SVCR_SM)) {
 ...
 *
 */
struct arm_saved_state *find_user_regs(thread_t thread) {
  return thread->machine.upcb;
}
 ...
 *
 */
struct arm_saved_state *find_kern_regs(thread_t thread) {
  /*
   * This works only for an interrupted kernel thread
   */
  if (thread != current_thread() || getCpuDatap()->cpu_int_state == NULL) {
...
arm_debug_state32_t *find_debug_state32(thread_t thread) {
...
arm_debug_state64_t *find_debug_state64(thread_t thread) {
  if (thread && thread->machine.DebugData) {
 ...
 *           debug state fails.
 */
arm_debug_state64_t *find_or_allocate_debug_state64(thread_t thread) {
  arm_debug_state64_t *thread_state = find_debug_state64(thread);
  if (thread != NULL && thread_state == NULL) {
    thread->machine.DebugData = zalloc_flags(ads_zone, Z_WAITOK | Z_NOFAIL);
 ...
 *           debug state fails.
 */
arm_debug_state32_t *find_or_allocate_debug_state32(thread_t thread) {
 ...
 */
static inline void free_debug_state(thread_t thread) {
...
void *act_thread_csave(void) {
  ...
  kret = machine_thread_get_state(thread, ARM_THREAD_STATE,
                                  (thread_state_t)&ic->ss, &val);
  ...
  if (thread_is_64bit_data(thread)) {
    ...
    kret = machine_thread_get_state(thread, ARM_NEON_STATE64,
                                    (thread_state_t)&ic->ns, &val);
  } else {
    val = ARM_NEON_STATE_COUNT;
    kret = machine_thread_get_state(thread, ARM_NEON_STATE,
                                    (thread_state_t)&ic->ns, &val);
  }
  if (kret != KERN_SUCCESS) {
    kfree_type(struct arm_act_context, ic);
...
void act_thread_catt(void *ctx) {
  ...
  kret = machine_thread_set_state(thread, ARM_THREAD_STATE,
...
#if __ARM_VFP__
  if (thread_is_64bit_data(thread)) {
    kret = machine_thread_set_state(thread, ARM_NEON_STATE64,
                                    (thread_state_t)&ic->ns,
                                    ARM_NEON_STATE64_COUNT);
  } else {
    kret = machine_thread_set_state(
        thread, ARM_NEON_STATE, (thread_state_t)&ic->ns, ARM_NEON_STATE_COUNT);
  }
  if (kret != KERN_SUCCESS) {
    goto out;
  }
 ...
 *
 */
void act_thread_cfree(void *ctx) { kfree_type(struct arm_act_context, ctx); }
