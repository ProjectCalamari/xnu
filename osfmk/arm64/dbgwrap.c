...
boolean_t ml_dbgwrap_cpu_is_halted(int cpu_index) {
  ...
  return (*(volatile dbgwrap_reg_t *)(cdp->coresight_base[CORESIGHT_UTT] +
                                      DBGWRAP_REG_OFFSET) &
          DBGWRAP_DBGACK) != 0;
}

dbgwrap_status_t ml_dbgwrap_wait_cpu_halted(int cpu_index,
                                            uint64_t timeout_ns) {
  cpu_data_t *cdp = cpu_datap(cpu_index);
  if ((cdp == NULL) || (cdp->coresight_base[CORESIGHT_UTT] == 0)) {
  ...
  volatile dbgwrap_reg_t *dbgWrapReg =
      (volatile dbgwrap_reg_t *)(cdp->coresight_base[CORESIGHT_UTT] +
                                 DBGWRAP_REG_OFFSET);
...
dbgwrap_status_t ml_dbgwrap_halt_cpu(int cpu_index, uint64_t timeout_ns) {
  cpu_data_t *cdp = cpu_datap(cpu_index);
  if ((cdp == NULL) || (cdp->coresight_base[CORESIGHT_UTT] == 0) ||
      (cdp->coresight_base[CORESIGHT_ED] == 0)) {
    return DBGWRAP_ERR_UNSUPPORTED;
  }

  /* Only one cpu is allowed to initiate the halt sequence, to prevent cpus from
   * cross-halting each other.  The first cpu to request a halt may then halt
   * any and all other cpus besides itself. */
  int curcpu = cpu_number();
  if (cpu_index == curcpu) {
    return DBGWRAP_ERR_SELF_HALT;
  }

  if (!os_atomic_cmpxchg(&halt_from_cpu, (uint32_t)-1, (unsigned int)curcpu,
                         acq_rel) &&
      (halt_from_cpu != (uint32_t)curcpu)) {
    return DBGWRAP_ERR_INPROGRESS;
  }

  /* Accessing EDPRSR or DBGWRAP when cluster is powered off would result in LLC
   * bus error. Hence, check the cluster power status to make sure these PIO
  ...
  *((volatile uint32_t *)(cdp->coresight_base[CORESIGHT_ED] +
  ...
  /* A core that is not fully powered (e.g. idling in wfi) can still be halted;
   * the dbgwrap register and certain coresight registers such EDPRSR are in the
   * always-on domain and their values are retained over ACC power down. And the
   * OS lock defaults to being set but we clear it first thing when CPU is up,
   * so use that to detect the offline state of individual CPU. */
  ...
  /* Clear all other writable bits besides dbgHalt; none of the power-down or
   * reset bits must be set. */
...
static void ml_dbgwrap_stuff_instr(cpu_data_t *cdp, uint32_t instr,
                                   uint64_t timeout_ns,
                                   dbgwrap_status_t *status) {
  if (*status < 0) {
    return;
  }

  volatile uint32_t *editr =
      (volatile uint32_t *)(cdp->coresight_base[CORESIGHT_ED] +
                            EDITR_REG_OFFSET);
  volatile uint32_t *edscr =
      (volatile uint32_t *)(cdp->coresight_base[CORESIGHT_ED] +
                            EDSCR_REG_OFFSET);
  volatile uint32_t *edrcr =
      (volatile uint32_t *)(cdp->coresight_base[CORESIGHT_ED] +
                            EDRCR_REG_OFFSET);
...
static uint64_t ml_dbgwrap_read_dtr(cpu_data_t *cdp, uint64_t timeout_ns,
                                    dbgwrap_status_t *status) {
  if (*status < 0) {
    return 0;
  }
  ...
  /* Per armv8 debug spec, writes to DBGDTR_EL0 on target cpu will set
   * EDSCR.TXFull, with bits 63:32 available in EDDTRRX and bits 31:0 availabe
   * in EDDTRTX. */
  volatile uint32_t *edscr =
      (volatile uint32_t *)(cdp->coresight_base[CORESIGHT_ED] +
                            EDSCR_REG_OFFSET);
  ...
  uint32_t dtrrx = *((volatile uint32_t *)(cdp->coresight_base[CORESIGHT_ED] +
                                           EDDTRRX_REG_OFFSET));
  uint32_t dtrtx = *((volatile uint32_t *)(cdp->coresight_base[CORESIGHT_ED] +
                                           EDDTRTX_REG_OFFSET));

  return ((uint64_t)dtrrx << 32) | dtrtx;
}

dbgwrap_status_t ml_dbgwrap_halt_cpu_with_state(int cpu_index,
                                                uint64_t timeout_ns,
  ...
  instr =
      (0xD51U << 20) | (2 << 19) | (3 << 16) | (4 << 8) | 29; // msr DBGDTR0, fp
  ml_dbgwrap_stuff_instr(cdp, instr, timeout_ns, &status);
  state->fp = ml_dbgwrap_read_dtr(cdp, timeout_ns, &status);

  instr =
      (0xD51U << 20) | (2 << 19) | (3 << 16) | (4 << 8) | 30; // msr DBGDTR0, lr
  ml_dbgwrap_stuff_instr(cdp, instr, timeout_ns, &status);
  state->lr = ml_dbgwrap_read_dtr(cdp, timeout_ns, &status);

  /* Stack pointer (x31) can't be used as a register operand for msr; register
   * 31 is treated as xzr rather than sp when used as the transfer operand
   * there.  Instead, load sp into a GPR we've already saved off and then store
   * that register in the DTR.  I've chosen x18 as the temporary GPR since it's
   * reserved by the arm64 ABI and unused by xnu, so overwriting it poses the
   * least risk of causing trouble for external debuggers. */
  ...
  /* reading PC (e.g. through adr) is undefined in debug state.  Instead use
   * DLR_EL0, which contains PC at time of entry into debug state.*/

  instr = (0xD53U << 20) | (1 << 19) | (3 << 16) | (4 << 12) | (5 << 8) |
          (1 << 5) | 18; // mrs    x18, DLR_EL0
  ml_dbgwrap_stuff_instr(cdp, instr, timeout_ns, &status);
  instr = (0xD51U << 20) | (2 << 19) | (3 << 16) | (4 << 8) |
          18; // msr DBGDTR0, x18
  ml_dbgwrap_stuff_instr(cdp, instr, timeout_ns, &status);
