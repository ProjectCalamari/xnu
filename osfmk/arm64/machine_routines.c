...
MACHINE_TIMEOUT_DEV_WRITEABLE(LockTimeOut, "lock", 6e6 /* 0.25s */,
                              MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
machine_timeout_t LockTimeOutUsec; // computed in ml_init_lock_timeout

MACHINE_TIMEOUT_DEV_WRITEABLE(TLockTimeOut, "ticket-lock", 3e6 /* 0.125s */,
                              MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
...
SECURITY_READ_ONLY_LATE(static ml_topology_cluster_t)
topology_cluster_array[MAX_CPU_CLUSTERS];
SECURITY_READ_ONLY_LATE(static ml_topology_info_t)
topology_info = {
 ...
 * entries of an arbitrary data type.  This is intended for use by specialized
 * consumers that must quickly access per-CPU data using only the physical CPU
 * ID (MPIDR_EL1), as follows: hypothetical_array[cluster_offsets[AFF1] + AFF0]
 * Most consumers should instead use general-purpose facilities such as PERCPU
 * or ml_get_cpu_number().
 */
...
static const struct vm_reserved_region vm_reserved_regions[] = {
    {.vmrr_name = "GPU Carveout",
     ...
     .vmrr_size = (vm_map_size_t)(MACH_VM_MAX_GPU_CARVEOUT_ADDRESS -
...
#if defined(HAS_IPI)
static inline void ml_cpu_signal_type(unsigned int cpu_mpidr, uint32_t type) {
#if HAS_CLUSTER
...
#define IPI_RR_TARGET_CLUSTER_SHIFT 16
    uint64_t x = type |
                 (MPIDR_CLUSTER_ID(cpu_mpidr) << IPI_RR_TARGET_CLUSTER_SHIFT) |
...
__dead2
#endif
    void ml_cpu_signal(unsigned int cpu_mpidr __unused) {
#if defined(HAS_IPI)
...
__dead2
   ...
   *
   * IPICR is a global register but there are two copies in ACC: one at pBLK and
   * one at eBLK. IPICR write SPR token also traverses both pCPM and eCPM rings
   * and updates both copies.
   */
  uint64_t abstime;
...
__dead2
#endif
    void ml_cpu_signal_deferred(unsigned int cpu_mpidr __unused) {
#if defined(HAS_IPI)
...
__dead2
#endif
    void ml_cpu_signal_retract(unsigned int cpu_mpidr __unused) {
#if defined(HAS_IPI)
...
static bool wfe_process_recommendation(void) {
  bool ipending = false;
  if (__probable(idle_proximate_io_wfe_unmasked == 1)) {
    ...
    if (wfe_deadline != 0) {
      ...
      KDBG(CPUPM_IDLE_WFE | DBG_FUNC_START, ipending, wc, wfe_ttd,
           cdp->cpu_stat.irq_ex_cnt_wake);
#endif
      /* Issue WFE until the recommendation expires,
       * with IRQs unmasked.
       */
      ipending =
          wfe_to_deadline_or_interrupt(cid, wfe_deadline, cdp, true, true);
#if DEVELOPMENT || DEBUG
      KDBG(CPUPM_IDLE_WFE | DBG_FUNC_END, ipending, cdp->wfe_count - wc,
           wfe_deadline, cdp->cpu_stat.irq_ex_cnt_wake);
#endif
    }
  }
  return ipending;
...
void machine_idle(void) {
   ...
   * Idle_load_context()
   */
  assert((__builtin_arm_rsr("DAIF") & DAIF_STANDARD_DISABLE) ==
...
void OSSynchronizeIO(void) { __builtin_arm_dsb(DSB_SY); }
...
__mockable boolean_t ml_get_interrupts_enabled(void) {
  uint64_t value;
...
MARK_AS_FIXUP_TEXT uint32_t get_arm_cpu_version(void) {
  uint32_t value = machine_read_midr();

  /* Compose the register values into 8 bits; variant[7:4], revision[3:0]. */
  return ((value & MIDR_EL1_REV_MASK) >> MIDR_EL1_REV_SHIFT) |
         ((value & MIDR_EL1_VAR_MASK) >> (MIDR_EL1_VAR_SHIFT - 4));
}
 ...
 * USER_TIMEBASE_NOSPEC: EL0 has access to non speculative timebase reads
 * (CNTVCTSS_EL0)
 *
 */
...
void machine_startup(__unused boot_args *args) {
...
bool ml_is_secure_hib_supported(void) { return false; }
...
void machine_lockdown(void) {
   ...
   */
  disable_preemption();
  /* Signal the SPTM that XNU is ready for RO memory to actually become
   * read-only */
  sptm_lockdown_xnu();
...
char *machine_boot_info(__unused char *buf, __unused vm_size_t size) {
  return PE_boot_args();
}

void machine_cpu_reinit(__unused void *param) {
 ...
 *	Function:
 */
thread_t machine_processor_shutdown(__unused thread_t thread,
 ...
 *      Function:
 */
static void __startup_func ml_init_lock_timeout(void) {
  /*
  ...
  nanoseconds_to_absolutetime(maxwfeus * NSEC_PER_USEC,
 ...
 * initialized and all the processors have been started through
 * processor_boot().
 *
 * Required by the scheduler subsystem.
 */
void ml_cpu_init_completed(void) { sched_cpu_init_completed(); }
...
void ml_cpu_up(void) {
  ...
  assert(!bit_test(os_atomic_load(&ml_cpu_up_processors, relaxed),
                   cpu_data_ptr->cpu_number));

  atomic_bit_set(&ml_cpu_up_processors, cpu_data_ptr->cpu_number,
...
void ml_cpu_up_update_counts(int cpu_id) {
...
int ml_find_next_up_processor() {
  if (BootCpuData.cpu_running) {
    return BootCpuData.cpu_number;
  }

  int next_active_cpu =
      lsb_first(os_atomic_load(&ml_cpu_up_processors, relaxed));
...
void ml_cpu_down(void) {
  ...
  atomic_bit_clear(&ml_cpu_up_processors, cpu_data_ptr->cpu_number,
                   memory_order_release);

  if (cpu_data_ptr == &BootCpuData && ml_is_quiescing()) {
    /*
     * This is the boot CPU powering down for S2R, don't try to migrate its
     * timers, because there is nobody else active to migrate it to.
     */
    assert3u(os_atomic_load(&ml_cpu_up_processors, relaxed), ==, 0);
  } else if (cpu_data_ptr != &BootCpuData ||
             (support_bootcpu_shutdown && !ml_is_quiescing())) {
    int next_cpu = ml_find_next_up_processor();

    cpu_data_t *new_cpu_datap = cpu_datap(next_cpu);

    /*
     * Move all of this cpu's timers to another cpu that has not gone through
     * ml_cpu_down, and poke it in case there's a sooner deadline for it to
     * schedule.
     *
     * This depends on ml_cpu_down never running concurrently, which is
     * guaranteed by the processor_updown_lock.
     */
...
unsigned int ml_get_machine_mem(void) { return machine_info.memory_size; }
...
__attribute__((noreturn)) void halt_cpu(void) { halt_all_cpus(FALSE); }
...
void machine_signal_idle(processor_t processor) {
  cpu_signal(processor_to_cpu_datap(processor), SIGPnop, (void *)NULL,
             (void *)NULL);
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                            MACHDBG_CODE(DBG_MACH_SCHED, MACH_REMOTE_AST),
                            processor->cpu_id, 0 /* nop */, 0, 0, 0);
}

void machine_signal_idle_deferred(processor_t processor) {
  cpu_signal_deferred(processor_to_cpu_datap(processor), SIGPdeferred);
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, MACHDBG_CODE(DBG_MACH_SCHED, MACH_REMOTE_DEFERRED_AST),
      processor->cpu_id, 0 /* nop */, 0, 0, 0);
}

void machine_signal_idle_cancel(processor_t processor) {
  cpu_signal_cancel(processor_to_cpu_datap(processor), SIGPdeferred);
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, MACHDBG_CODE(DBG_MACH_SCHED, MACH_REMOTE_CANCEL_AST),
      processor->cpu_id, 0 /* nop */, 0, 0, 0);
}
 ...
 */
void ml_init_timebase(void *args, tbd_ops_t tbd_funcs, vm_offset_t int_address,
...
static uint64_t ml_readprop(const DTEntry entry, const char *propertyName,
...
static boolean_t ml_read_reg_range(const DTEntry entry,
                                   const char *propertyName, uint64_t *pa_ptr,
                                   uint64_t *len_ptr) {
  uint64_t const *prop;
  unsigned int propSize;

  if (SecureDTGetProperty(entry, propertyName, (void const **)&prop,
                          &propSize) != kSuccess) {
    return FALSE;
  }
...
static boolean_t ml_is_boot_cpu(const DTEntry entry) {
  void const *prop;
  unsigned int propSize;
...
static void ml_cluster_power_override(unsigned int *flag) {
...
static void ml_read_chip_revision(unsigned int *rev __unused) {
  ...
  if ((SecureDTFindEntry("name", "arm-io", &entryP) == kSuccess)) {
    *rev =
        (unsigned int)ml_readprop(entryP, "chip-revision", CPU_VERSION_UNKNOWN);
  } else {
...
void ml_parse_cpu_topology(void) {
  ...
  const boolean_t cpus_boot_arg_present =
      PE_parse_boot_argn("cpus", &cpu_boot_arg, sizeof(cpu_boot_arg));
  const boolean_t cpumask_boot_arg_present = PE_parse_boot_argn(
      "cpumask", &cpumask_boot_arg, sizeof(cpumask_boot_arg));

  // The cpus=N and cpumask=N boot args cannot be used simultaneously. Flag this
  // so that we trigger a panic later in the boot process, once serial is
  // enabled.
  ...
  while (kSuccess == SecureDTIterateEntries(&iter, &child)) {
    ...
    // If the number of CPUs is constrained by the cpus= boot-arg, and the boot
    // CPU hasn't been added to the topology struct yet, and we only have one
    // slot left, then skip every other non-boot CPU in order to leave room for
    // the boot CPU.
    //
    // e.g. if the boot-args say "cpus=3" and CPU4 is the boot CPU, then the
    // cpus[] array will list CPU0, CPU1, and CPU4.  CPU2-CPU3 and CPU5-CPUn
    // will be omitted.
    if (topology_info.num_cpus >= (cpu_boot_arg - 1) &&
        topology_info.boot_cpu == NULL && !is_boot_cpu) {
      continue;
    }
    ...
    ml_read_reg_range(child, "cpu-uttdbg-reg", &cpu->cpu_UTTDBG_pa,
                      &cpu->cpu_UTTDBG_len);
    ml_read_reg_range(child, "cpu-impl-reg", &cpu->cpu_IMPL_pa,
                      &cpu->cpu_IMPL_len);
    ml_read_reg_range(child, "coresight-reg", &cpu->coresight_pa,
                      &cpu->coresight_len);
     ...
     * on the value provided by EDT. Instead, use the MPIDR value to see if we
     * have seen this exact cluster before. If so, then reuse that cluster ID
     * for this CPU.
     */
#if HAS_CLUSTER
    ...
    cpu->cluster_id = ((cluster_phys_to_logical[phys_cluster_id] == -1)
                           ...
                           : cluster_phys_to_logical[phys_cluster_id]);
    ...
    if (cluster->num_cpus == 0) {
      ...
      topology_info.num_clusters++;
      topology_info.max_cluster_id =
          MAX(topology_info.max_cluster_id, cpu->cluster_id);
      ...
      // Since we don't have a per-cluster EDT node, this is repeated in each
      // CPU node. If we wind up with a bunch of these, we might want to create
      // separate per-cluster EDT nodes and have the CPU nodes reference them
      // through a phandle.
      ml_read_reg_range(child, "acc-impl-reg", &cluster->acc_IMPL_pa,
                        &cluster->acc_IMPL_len);
      ml_read_reg_range(child, "cpm-impl-reg", &cluster->cpm_IMPL_pa,
                        &cluster->cpm_IMPL_len);
    }
    ...
    cpu->die_cluster_id = (int)ml_readprop(child, "die-cluster-id",
                                           MPIDR_CLUSTER_ID(cpu->phys_id));
   ...
   * is linear across both clusters.   For the purpose of MPIDR_EL1-based
   * indexing, treat H10 and earlier devices as though they contain a single
   * cluster.
   */
  cluster_offsets[0] = 0;
#endif
  ...
  uint64_t tpidr_el0 =
      ((boot_cpu->cpu_id << MACHDEP_TPIDR_CPUNUM_SHIFT) &
       MACHDEP_TPIDR_CPUNUM_MASK) |
      ((boot_cpu->cluster_id << MACHDEP_TPIDR_CLUSTERID_SHIFT) &
       MACHDEP_TPIDR_CLUSTERID_MASK);
  assert(((tpidr_el0 & MACHDEP_TPIDR_CPUNUM_MASK) >>
          MACHDEP_TPIDR_CPUNUM_SHIFT) == boot_cpu->cpu_id);
  assert(((tpidr_el0 & MACHDEP_TPIDR_CLUSTERID_MASK) >>
          MACHDEP_TPIDR_CLUSTERID_SHIFT) == boot_cpu->cluster_id);
  __builtin_arm_wsr64("TPIDR_EL0", tpidr_el0);

  __builtin_arm_wsr64("TPIDRRO_EL0", 0);
}

const ml_topology_info_t *ml_get_topology_info(void) { return &topology_info; }
...
void ml_map_cpu_pio(void) {
  ...
  for (i = 0; i < topology_info.num_cpus; i++) {
    ...
    if (cpu->cpu_IMPL_pa) {
      cpu->cpu_IMPL_regs =
          (vm_offset_t)ml_io_map(cpu->cpu_IMPL_pa, cpu->cpu_IMPL_len);
      cpu->coresight_regs =
          (vm_offset_t)ml_io_map(cpu->coresight_pa, cpu->coresight_len);
    }
    if (cpu->cpu_UTTDBG_pa) {
      cpu->cpu_UTTDBG_regs =
          (vm_offset_t)ml_io_map(cpu->cpu_UTTDBG_pa, cpu->cpu_UTTDBG_len);
    }
  }
  ...
  for (i = 0; i < topology_info.num_clusters; i++) {
    ...
    if (cluster->acc_IMPL_pa) {
      cluster->acc_IMPL_regs =
          (vm_offset_t)ml_io_map(cluster->acc_IMPL_pa, cluster->acc_IMPL_len);
    }
    if (cluster->cpm_IMPL_pa) {
      cluster->cpm_IMPL_regs =
          (vm_offset_t)ml_io_map(cluster->cpm_IMPL_pa, cluster->cpm_IMPL_len);
    }
  }
}

__mockable unsigned int ml_get_cpu_count(void) {
  return topology_info.num_cpus;
}

unsigned int ml_get_cluster_count(void) { return topology_info.num_clusters; }

int ml_get_boot_cpu_number(void) { return topology_info.boot_cpu->cpu_id; }
...
int ml_get_cluster_number_local() {
  uint64_t mpidr_el1_value = 0;
  unsigned cluster_id;
...
int ml_get_max_cpu_number(void) { return topology_info.max_cpu_id; }

int ml_get_max_cluster_number(void) { return topology_info.max_cluster_id; }
...
static_assert(MAX_CPUS <= 256,
              "MAX_CPUS must fit in _COMM_PAGE_CPU_TO_CLUSTER; Increase table "
              "size if needed");
 ...
 */
unsigned int ml_get_max_die_id(void) { return topology_info.max_die_id; }
...
kern_return_t ml_lockdown_handler_register(lockdown_handler_t f, void *this) {
  if (!f) {
    return KERN_FAILURE;
  }
...
kern_return_t ml_mcache_flush_callback_register(mcache_flush_function func,
                                                void *service) {
...
kern_return_t ml_mem_fault_report_enable_register(void) { return KERN_SUCCESS; }

kern_return_t ml_amcc_error_inject_register(void);
kern_return_t ml_amcc_error_inject_register(void) { return KERN_SUCCESS; }

kern_return_t ml_dcs_error_inject_register(void);
kern_return_t ml_dcs_error_inject_register(void) { return KERN_SUCCESS; }
...
ml_processor_register(ml_processor_info_t *in_processor_info,
  ...
  if ((unsigned)OSIncrementAtomic((SInt32 *)&reg_cpu_count) >=
  ...
  if (!is_boot_cpu) {
    ...
    assert((this_cpu_datap->cpu_number & MACHDEP_TPIDR_CPUNUM_MASK) ==
           this_cpu_datap->cpu_number);
  }

  this_cpu_datap->cpu_idle_notify = in_processor_info->processor_idle;
  this_cpu_datap->cpu_cache_dispatch =
      (cache_dispatch_t)in_processor_info->platform_cache_dispatch;
  nanoseconds_to_absolutetime((uint64_t)in_processor_info->powergate_latency,
                              &this_cpu_datap->cpu_idle_latency);
  this_cpu_datap->cpu_reset_assist =
      kvtophys(in_processor_info->powergate_stub_addr);
   ...
   * by userspace.
   */
  this_cpu_datap->cpu_tpidr_el0 =
      ((this_cpu_datap->cpu_number << MACHDEP_TPIDR_CPUNUM_SHIFT) &
       MACHDEP_TPIDR_CPUNUM_MASK) |
      ((this_cpu_datap->cpu_cluster_id << MACHDEP_TPIDR_CLUSTERID_SHIFT) &
       MACHDEP_TPIDR_CLUSTERID_MASK);
  assert(((this_cpu_datap->cpu_tpidr_el0 & MACHDEP_TPIDR_CPUNUM_MASK) >>
          MACHDEP_TPIDR_CPUNUM_SHIFT) == this_cpu_datap->cpu_number);
  assert(((this_cpu_datap->cpu_tpidr_el0 & MACHDEP_TPIDR_CLUSTERID_MASK) >>
          MACHDEP_TPIDR_CLUSTERID_SHIFT) == this_cpu_datap->cpu_cluster_id);

#if HAS_CLUSTER
  this_cpu_datap->cluster_master =
      !OSTestAndSet(this_cpu_datap->cpu_cluster_id, &cluster_initialized);
#else  /* HAS_CLUSTER */
  ...
  kprintf("[%d]%s>pset_find(cluster_id=%d) returned pset %d\n",
          current_processor()->cpu_id, __FUNCTION__,
          in_processor_info->cluster_id, pset ? pset->pset_id : -1);
  if (pset == NULL) {
    pset = pset_create(this_cpu_datap->cpu_cluster_type,
                       this_cpu_datap->cpu_cluster_id,
                       this_cpu_datap->cpu_cluster_id);
    assert(pset != PROCESSOR_SET_NULL);
#if __AMP__
    kprintf("[%d]%s>pset_create(cluster_id=%d) returned pset %d\n",
            current_processor()->cpu_id, __FUNCTION__,
            this_cpu_datap->cpu_cluster_id, pset->pset_id);
#endif /* __AMP__ */
  }
  kprintf("[%d]%s>cpu_id %p cluster_id %d cpu_number %d is type %d\n",
          current_processor()->cpu_id, __FUNCTION__, in_processor_info->cpu_id,
          in_processor_info->cluster_id, this_cpu_datap->cpu_number,
          in_processor_info->cluster_type);
  lck_mtx_unlock(&pset_create_lock);

  processor_t processor =
      PERCPU_GET_RELATIVE(processor, cpu_data, this_cpu_datap);
  if (!is_boot_cpu) {
  ...
  *pmi_handler_out = NULL;
  ...
  this_cpu_datap->acc_reg_paddr =
      topology_info.clusters[cluster_id].acc_IMPL_pa;
  this_cpu_datap->cpm_reg_paddr =
      topology_info.clusters[cluster_id].cpm_IMPL_pa;
#endif
...
void ml_init_arm_debug_interface(void *in_cpu_datap, vm_offset_t virt_address) {
 ...
 *	Function:
 */
void init_ast_check(__unused processor_t processor) {}
...
void cause_ast_check(processor_t processor) {
  ...
  if (current_processor() != processor) {
    cpu_signal(processor_to_cpu_datap(processor), SIGPast, (void *)NULL,
               (void *)NULL);
    KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                              MACHDBG_CODE(DBG_MACH_SCHED, MACH_REMOTE_AST),
                              processor->cpu_id, 1 /* ast */, 0, 0, 0);
  }
}
 ...
 */
void ml_cause_interrupt(void) { return; /* BS_XXX */ }
...
vm_offset_t ml_io_map_with_prot(vm_offset_t phys_addr, vm_size_t size,
                                vm_prot_t prot) {
  return io_map(phys_addr, size, VM_WIMG_IO, prot, false);
}

vm_offset_t ml_io_map_unmappable(vm_offset_t phys_addr, vm_size_t size,
                                 unsigned int flags) {
  return io_map(phys_addr, size, flags, VM_PROT_DEFAULT, true);
}

vm_offset_t ml_io_map_wcomb(vm_offset_t phys_addr, vm_size_t size) {
  return io_map(phys_addr, size, VM_WIMG_WCOMB, VM_PROT_DEFAULT, false);
}

void ml_io_unmap(vm_offset_t addr, vm_size_t sz) {
...
vm_offset_t ml_static_ptovirt(vm_offset_t paddr) { return phystokv(paddr); }

vm_offset_t ml_static_slide(vm_offset_t vaddr) {
  vm_offset_t slid_vaddr = 0;

#if CONFIG_SPTM
  if ((vaddr >= vm_sptm_offsets.unslid_base) &&
      (vaddr < vm_sptm_offsets.unslid_top)) {
    slid_vaddr = vaddr + vm_sptm_offsets.slide;
  } else if ((vaddr >= vm_txm_offsets.unslid_base) &&
             (vaddr < vm_txm_offsets.unslid_top)) {
...
vm_offset_t ml_static_unslide(vm_offset_t vaddr) {
   ...
   * rest of the kernel.
   */
  if ((vaddr >= vm_sptm_offsets.slid_base) &&
      (vaddr < vm_sptm_offsets.slid_top)) {
    return vaddr - vm_sptm_offsets.slide;
  }

  if ((vaddr >= vm_txm_offsets.slid_base) &&
      (vaddr < vm_txm_offsets.slid_top)) {
...
kern_return_t ml_static_protect(vm_offset_t vaddr, /* kernel virtual address */
                                vm_size_t size, vm_prot_t new_prot __unused) {
#if CONFIG_SPTM
  /**
   * Retype any frames that may be passed to the VM to XNU_DEFAULT.
   */
  for (vm_offset_t sptm_vaddr_cur = vaddr;
       sptm_vaddr_cur < trunc_page_64(vaddr + size);
       sptm_vaddr_cur += PAGE_SIZE) {
  ...
  if (lockdown_done && (new_prot & VM_PROT_EXECUTE)) {
    panic("ml_static_protect(): attempt to inject executable mapping on %p",
          (void *)vaddr);
  }

  /* Set up the protection bits, and block bits so we can validate block
   * mappings. */
  ...
  for (vaddr_cur = vaddr; vaddr_cur < trunc_page_64(vaddr + size);
       vaddr_cur += PAGE_SIZE) {
    ...
    if (ppn != (vm_offset_t)NULL) {
      ...
      if (((*tte2) & ARM_TTE_TYPE_MASK) != ARM_TTE_TYPE_TABLE) {
        if ((((*tte2) & ARM_TTE_TYPE_MASK) == ARM_TTE_TYPE_BLOCK) &&
            ((*tte2 & (ARM_TTE_BLOCK_NXMASK | ARM_TTE_BLOCK_PNXMASK |
                       ARM_TTE_BLOCK_APMASK)) == arm_block_prot)) {
          /*
           * We can support ml_static_protect on a block mapping if the mapping
           * already has the desired protections.  We still want to run checks
           * on a per-page basis.
           */
          continue;
        }
      ...
      pte_p = (pt_entry_t *)&(
          (tt_entry_t *)(phystokv((*tte2) & ARM_TTE_TABLE_MASK)))[(
          ((vaddr_cur)&ARM_TT_L3_INDEX_MASK) >> ARM_TT_L3_SHIFT)];
      ptmp = *pte_p;

      if ((ptmp & ARM_PTE_HINT_MASK) &&
          ((ptmp & (ARM_PTE_APMASK | ARM_PTE_PNXMASK | ARM_PTE_NXMASK)) !=
           arm_prot)) {
        /*
         * The contiguous hint is similar to a block mapping for
         * ml_static_protect; if the existing protections do not match the
         * desired protections, then we will fail (as we cannot update this
         * mapping without updating other mappings as well).
         */
        result = KERN_FAILURE;
        break;
      }
      ...
      __unreachable_ok_push if (TEST_PAGE_RATIO_4) {
        {
          ...
          for (i = 0; i < 4; i++, ptep_iter++) {
            ...
            /* We only need to update the page tables if the protections do not
             * match. */
            if ((ptmp & (ARM_PTE_APMASK | ARM_PTE_PNXMASK | ARM_PTE_NXMASK)) !=
                arm_prot) {
              ptmp = (ptmp &
        ...
        if ((ptmp & (ARM_PTE_APMASK | ARM_PTE_PNXMASK | ARM_PTE_NXMASK)) !=
            arm_prot) {
          ptmp = (ptmp & ~(ARM_PTE_APMASK | ARM_PTE_PNXMASK | ARM_PTE_NXMASK)) |
                 arm_prot;
          *pte_p = ptmp;
        }
      }
...
#if defined(CONFIG_SPTM)
/*
 * Returns true if the given physical address is in one of the boot kernelcache
 * ranges.
 */
static bool ml_physaddr_in_bootkc_range(vm_offset_t physaddr) {
  for (int i = 0; i < arm_vm_kernelcache_numranges; i++) {
    if (physaddr >= arm_vm_kernelcache_ranges[i].start_phys &&
        physaddr < arm_vm_kernelcache_ranges[i].end_phys) {
      return true;
    }
  }
  return false;
 ...
 */
static vm_page_list_t ml_static_mfree_pre_slide_list;
 ...
 */
static bool ml_static_mfree_queue_up = true;
...
void ml_static_mfree(vm_offset_t vaddr, vm_size_t size) {
  ...
  for (vaddr_cur = vaddr; vaddr_cur < trunc_page_64(vaddr + size);
    ...
    if (ppn != (vm_offset_t)NULL) {
       ...
       */
      pmap_disconnect(ppn);
      if (ml_static_protect(vaddr_papt, PAGE_SIZE,
...
#else
      if (paddr_cur >= arm_vm_kernelcache_phys_start &&
          paddr_cur < arm_vm_kernelcache_phys_end)
#endif
      {
...
#if DEBUG
  kprintf("ml_static_mfree: Released 0x%x pages at VA %p, size:0x%llx, last "
          "ppn: 0x%x, +%d bad\n",
...
vm_offset_t ml_vtophys(vm_offset_t vaddr) { return kvtophys(vaddr); }
 ...
 *			in the kernel pmap.  If translations are present, they
 * are assumed to be wired; i.e. no attempt is made to guarantee that the
 * translation persist after the check. Returns: TRUE if the range is mapped and
 * will not cause a fault, FALSE otherwise.
 */
...
void active_rt_threads(__unused boolean_t active) {}

static void cpu_qos_cb_default(__unused int urgency,
                               __unused uint64_t qos_param1,
                               __unused uint64_t qos_param2) {
  return;
}
...
void thread_tell_urgency(thread_urgency_t urgency, uint64_t rt_period,
                         uint64_t rt_deadline, uint64_t sched_latency __unused,
                         __unused thread_t nthread) {
  SCHED_DEBUG_PLATFORM_KERNEL_DEBUG_CONSTANT(
      MACHDBG_CODE(DBG_MACH_SCHED, MACH_URGENCY) | DBG_FUNC_START, urgency,
      rt_period, rt_deadline, sched_latency, 0);

  cpu_qos_update((int)urgency, rt_period, rt_deadline);

  SCHED_DEBUG_PLATFORM_KERNEL_DEBUG_CONSTANT(
      MACHDBG_CODE(DBG_MACH_SCHED, MACH_URGENCY) | DBG_FUNC_END, urgency,
      rt_period, rt_deadline, 0, 0);
}

void machine_run_count(__unused uint32_t count) {}
...
vm_offset_t ml_stack_base(void) {
  ...
  if ((local < intstack_top_ptr) &&
      (local > intstack_top_ptr - INTSTACK_SIZE)) {
...
kcov_cpu_data_t *cpu_kcov_data(int cpuid) {
  return &cpu_datap(cpuid)->cpu_kcov_data;
}

#endif /* CONFIG_KCOV */

boolean_t machine_timeout_suspended(void) { return FALSE; }

kern_return_t ml_interrupt_prewarm(__unused uint64_t deadline) {
  return KERN_FAILURE;
}
 ...
 */
static inline uint64_t speculative_timebase(void) {
 ...
 * enforce ordering.
 */
static inline uint64_t nonspeculative_timebase(void) {
#if __ARM_ARCH_8_6__
...
uint64_t ml_get_hwclock_speculative() {
...
uint64_t ml_get_timebase() {
  ...
  // the retry is for the case where S2R catches us in the middle of this. see
  // rdar://77019633
  do {
 ...
 */
void ml_timebase_to_memory_fence(void) { __builtin_arm_isb(ISB_SY); }
...
uint64_t ml_get_timebase_entropy(void) { return ml_get_speculative_timebase(); }
...
__attribute__((noreturn)) void platform_syscall(arm_saved_state_t *state) {
  uint32_t code;
...
#define platform_syscall_kprintf(x...) /* kprintf("platform_syscall: " x) */
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_MACHDEP_EXCP_SC_ARM, code) | DBG_FUNC_START,
       get_saved_state_reg(state, 0), get_saved_state_reg(state, 1),
...
static void _enable_timebase_event_stream(uint32_t bit_index) {
  if (bit_index >= 64) {
 ...
 */
static void _enable_virtual_timer(void) {
  uint64_t cntvctl = CNTV_CTL_EL0_ENABLE; /* One wants to use 32 bits, but "mrs"
                                             prefers it this way */

  __builtin_arm_wsr64(KERNEL_CNTV_CTL_EL0, cntvctl);
  /* disable the physical timer as a precaution, as its registers reset to
...
void fiq_context_init(boolean_t enable_fiq __unused) {
  /* Interrupts still disabled. */
  assert(ml_get_interrupts_enabled() == FALSE);
  _enable_virtual_timer();
}

void wfe_timeout_init(void) { _enable_timebase_event_stream(arm64_eventi); }
...
boolean_t ml_timer_forced_evaluation(void) { return FALSE; }
...
uint64_t ml_gpu_stat(__unused thread_t t) { return 0; }

thread_t current_thread(void) { return current_thread_fast(); }
...
#endif
void ml_report_minor_badness(uint32_t __unused badness_id) {
#if DEVELOPMENT || DEBUG
 ...
 */
void *ml_poison_ptr(void *ptr, ptrauth_key key) {
  ...
  /* When PAC is enabled, only userspace data pointers use TBI, regardless of
   * boot parameters */
...
#ifdef CONFIG_XNUPOST
void ml_expect_fault_begin(expected_fault_handler_t expected_fault_handler,
...
void ml_expect_fault_pc_begin(expected_fault_handler_t expected_fault_handler,
                              uintptr_t expected_fault_pc) {
  thread_t thread = current_thread();
  thread->machine.expected_fault_handler = expected_fault_handler;
  thread->machine.expected_fault_addr = 0;
  uintptr_t raw_func = (uintptr_t)ptrauth_strip((void *)expected_fault_pc,
                                                ptrauth_key_function_pointer);
  thread->machine.expected_fault_pc = raw_func;
}

void ml_expect_fault_end(void) {
  thread_t thread = current_thread();
...
uint32_t
ml_update_cluster_wfe_recommendation(uint32_t wfe_cluster_id,
                                     uint64_t wfe_timeout_abstime_interval,
                                     __unused uint64_t wfe_hint_flags) {
  assert(wfe_cluster_id < MAX_CPU_CLUSTERS);
  assert(wfe_timeout_abstime_interval <= ml_wfe_hint_max_interval);
  os_atomic_store(&arm64_cluster_wfe_recs[wfe_cluster_id],
                  wfe_timeout_abstime_interval, relaxed);
  return 0; /* Success */
}
...
uint64_t ml_cluster_wfe_timeout(uint32_t wfe_cluster_id) {
   ...
   */
  uint64_t wfet =
      os_atomic_load(&arm64_cluster_wfe_recs[wfe_cluster_id], relaxed);
#if DEVELOPMENT || DEBUG
  if (wfe_rec_clamp) {
...
__pure2 bool ml_addr_in_non_xnu_stack(__unused uintptr_t addr) {
#if CONFIG_SPTM
  /**
  ...
  return false;
#elif XNU_MONITOR
  return (addr >= (uintptr_t)pmap_stacks_start) &&
         (addr < (uintptr_t)pmap_stacks_end);
#else
  return false;
 ...
 */
void ml_panic_on_invalid_old_cpsr(const arm_saved_state_t *ss) {
 ...
 */
void ml_panic_on_invalid_new_cpsr(const arm_saved_state_t *ss, uint32_t cpsr) {
 ...
 */
void ml_fp_save_area_prealloc(void) {}

void ml_task_post_signature_processing_hook(__unused task_t task) {
  /**
   * Have an acquire barrier here to make sure the machine flags read that is
   * going to happen below is not speculated before the task->t_returnwaitflags
   * earlier in task_wait_to_return().
   */
...
__mockable bool ml_unsafe_kernel_text(void) {
  assert(_unsafe_kernel_text_initialized);
  return _unsafe_kernel_text;
}

__startup_func static void ml_unsafe_kernel_text_init(void) {
  ...
  if (SecureDTLookupEntry(0, "/chosen", &entry) == kSuccess &&
      SecureDTGetProperty(entry, "kernel-ctrr-to-be-enabled", &value, &size) ==
          kSuccess &&
      size == sizeof(int)) {
