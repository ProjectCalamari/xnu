...
extern int _bcopyinstr(const user_addr_t src, char *dst, vm_size_t max,
                       vm_size_t *actual);
...
extern int _copyin_atomic32_wait_if_equals(const user_addr_t src,
                                           uint32_t value);
...
extern int copyoutstr_prevalidate(const void *kaddr, user_addr_t uaddr,
                                  size_t len);
...
typedef enum { USER_ACCESS_READ, USER_ACCESS_WRITE } user_access_direction_t;
...
static inline bool is_kernel_to_kernel_copy(pmap_t pmap) {
 ...
 */
static user_addr_t copy_ensure_address_space_spec(vm_map_t map,
                                                  const user_addr_t addr) {
...
static int copy_validate_user_addr(vm_map_t map, const user_addr_t user_addr,
                                   vm_size_t nbytes) {
  ...
  if (__improbable(
          canonicalized_user_addr < vm_map_min(map) ||
...
static void copy_validate_kernel_addr(uintptr_t kernel_addr, vm_size_t nbytes) {
  uintptr_t kernel_addr_last;

  if (__improbable(os_add_overflow(kernel_addr, nbytes, &kernel_addr_last))) {
    panic("%s(%p, %lu) - kaddr not in kernel", __func__, (void *)kernel_addr,
          nbytes);
  }
...
int copyin_kern(const user_addr_t user_addr, char *kernel_addr,
                vm_size_t nbytes) {
...
int copyout_kern(const char *kernel_addr, user_addr_t user_addr,
                 vm_size_t nbytes) {
...
int copyin_atomic32(const user_addr_t user_addr, uint32_t *kernel_addr) {
  ...
  user_addr_t guarded_user_addr =
      copy_ensure_address_space_spec(map, user_addr);
...
int copyin_atomic32_wait_if_equals(const user_addr_t user_addr,
  ...
  vm_map_t map = current_thread()->map;
  int result =
      copy_validate(map, user_addr, 0, 4,
...
int copyout_atomic64(uint64_t value, user_addr_t user_addr) {
  vm_map_t map = current_thread()->map;
  int result =
      copy_validate(map, user_addr, 0, 8,
                    COPYIO_OUT | COPYIO_ATOMIC | COPYIO_VALIDATE_USER_ONLY);
  if (__improbable(result)) {
    return result;
  }
...
int copyinstr(const user_addr_t user_addr, char *kernel_addr, vm_size_t nbytes,
  ...
  *lencopied = 0;
  ...
  result =
      copy_validate(map, user_addr, (uintptr_t)kernel_addr, nbytes, COPYIO_IN);
  if (__improbable(result)) {
    return result;
  }
...
int copyoutstr_prevalidate(const void *__unused kaddr,
                           user_addr_t __unused uaddr, size_t __unused len) {
  vm_map_t map = current_thread()->map;
