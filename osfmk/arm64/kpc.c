...
#define PMCR_PMC_SHIFT(PMC) (((PMC) <= 7) ? (PMC) : PMCR_PMC_8_9_SHIFT(PMC))
...
#define S3_1_C15_C1_0_A64_SHIFT(PMC)                                           \
  (S3_1_C15_C1_0_A64_OFFSET + PMCR_PMC_SHIFT(PMC))
...
#define S3_1_C15_C1_0_A64_ENABLE_MASK(PMC)                                     \
  (UINT64_C(1) << S3_1_C15_C1_0_A64_SHIFT(PMC))
...
#define PMCR1_EL_ALL_ENABLE_MASK(PMC)                                          \
  (PMCR1_EL0_A32_ENABLE_MASK(PMC) | PMCR1_EL0_A64_ENABLE_MASK(PMC) |           \
   S3_1_C15_C1_0_A64_ENABLE_MASK(PMC) | PMCR1_EL3_A64_ENABLE_MASK(PMC))
...
static boolean_t enable_counter(uint32_t counter) {
  uint64_t pmcr0 = 0;
  boolean_t counter_running, pmi_enabled, enabled;

  pmcr0 = SREG_READ("S3_1_C15_C0_0") |
...
static boolean_t disable_counter(uint32_t counter) {
  uint64_t pmcr0;
  boolean_t enabled;
 ...
 */
static void set_modes(uint32_t counter, kpc_config_t cfgword, bool secure) {
   ...
   */
  if (bits == 0 && cfgword != 0) {
    bits = allow_kernel ? PMCR1_EL_ALL_ENABLE_MASK(counter)
...
static uint64_t read_counter(uint32_t counter) {
  switch (counter) {
  // case 0: return SREG_READ("S3_2_C15_C0_0");
  // case 1: return SREG_READ("S3_2_C15_C1_0");
  case 2:
    return SREG_READ("S3_2_C15_C2_0");
  case 3:
    return SREG_READ("S3_2_C15_C3_0");
  case 4:
    return SREG_READ("S3_2_C15_C4_0");
  case 5:
    return SREG_READ("S3_2_C15_C5_0");
  case 6:
    return SREG_READ("S3_2_C15_C6_0");
  case 7:
    return SREG_READ("S3_2_C15_C7_0");
#if KPC_ARM64_CONFIGURABLE_COUNT > 6
  case 8:
    return SREG_READ("S3_2_C15_C9_0");
  case 9:
    return SREG_READ("S3_2_C15_C10_0");
...
static void write_counter(uint32_t counter, uint64_t value) {
  switch (counter) {
  case 2:
    SREG_WRITE("S3_2_C15_C2_0", value);
    break;
  case 3:
    SREG_WRITE("S3_2_C15_C3_0", value);
    break;
  case 4:
    SREG_WRITE("S3_2_C15_C4_0", value);
    break;
  case 5:
    SREG_WRITE("S3_2_C15_C5_0", value);
    break;
  case 6:
    SREG_WRITE("S3_2_C15_C6_0", value);
    break;
  case 7:
    SREG_WRITE("S3_2_C15_C7_0", value);
    break;
#if KPC_ARM64_CONFIGURABLE_COUNT > 6
  case 8:
    SREG_WRITE("S3_2_C15_C9_0", value);
    break;
  case 9:
    SREG_WRITE("S3_2_C15_C10_0", value);
    break;
#endif // KPC_ARM64_CONFIGURABLE_COUNT > 6
  default:
    break;
...
uint32_t kpc_rawpmu_config_count(void) { return RAWPMU_CONFIG_COUNT; }
...
static void restore_control_regs(uint32_t classes) {
  const uint64_t pmcr1_mask =
      kpc_allows_counting_system ? PMCR1_ALL_MASK : PMCR1_EL0_MASK;
...
static bool kpc_cpu_callback(void *__unused param, enum cpu_event event,
...
boolean_t kpc_is_running_configurable(uint64_t pmc_mask) {
  ...
  return ((kpc_running_classes & KPC_CLASS_CONFIGURABLE_MASK) ==
          KPC_CLASS_CONFIGURABLE_MASK) &&
         ((kpc_running_cfg_pmc_mask & pmc_mask) == pmc_mask);
}

uint32_t kpc_fixed_count(void) { return KPC_ARM64_FIXED_COUNT; }

uint32_t kpc_configurable_count(void) { return KPC_ARM64_CONFIGURABLE_COUNT; }

uint32_t kpc_fixed_config_count(void) { return 0; }
...
int kpc_get_fixed_config(kpc_config_t *configv __unused) { return 0; }

uint64_t kpc_fixed_max(void) { return (1ULL << KPC_ARM64_COUNTER_WIDTH) - 1; }

uint64_t kpc_configurable_max(void) {
  return (1ULL << KPC_ARM64_COUNTER_WIDTH) - 1;
}

static void set_running_configurable(uint64_t target_mask,
                                     uint64_t state_mask) {
...
static void kpc_set_running_xcall(void *vstate) {
...
static void kpc_get_curcpu_counters_xcall(void *args) {
  ...
  int r =
      kpc_get_curcpu_counters(handler->classes, NULL, &handler->buf[offset]);
...
int kpc_get_all_cpus_counters(uint32_t classes, int *curcpu, uint64_t *buf) {
  ...
  struct kpc_get_counters_remote hdl = {.classes = classes,
                                        .nb_counters = 0,
                                        .buf = buf,
                                        .buf_stride =
                                            kpc_get_counter_count(classes)};

  cpu_broadcast_xcall(&kpc_xread_sync, TRUE, kpc_get_curcpu_counters_xcall,
                      &hdl);
...
int kpc_get_configurable_counters(uint64_t *counterv, uint64_t pmc_mask) {
  ...
  for (uint32_t i = 0; i < cfg_count; ++i) {
    ...
    if (ctr & KPC_ARM64_COUNTER_OVF_MASK) {
      ...
      ctr = CONFIGURABLE_SHADOW(i) + (ctr - CONFIGURABLE_RELOAD(i));
    }
...
static void kpc_set_config_xcall(void *vmp_config) {
...
static uint64_t kpc_reload_counter(uint32_t ctr) {
...
static void kpc_set_reload_xcall(void *vmp_config) {
  struct kpc_config_remote *mp_config = vmp_config;
...
void kpc_pmi_handler(unsigned int ctr) {
  ...
  if (FIXED_ACTIONID(ctr)) {
    ...
    kpc_sample_kperf(FIXED_ACTIONID(ctr), ctr, config & 0xffff,
                     FIXED_SHADOW(ctr), pc, flags);
  }
}
...
int kpc_set_config_arch(struct kpc_config_remote *mp_config) {
  ...
  for (uint32_t i = 0; i < cfg_count; ++i) {
    ...
    kpc_state->pmesr[pmesr_idx] |=
        PMESR_EVT_ENCODE(config_value, counter, pmesr_off);
...
int kpc_set_sw_inc(uint32_t mask __unused) { return ENOTSUP; }

int kpc_get_pmu_version(void) { return KPC_PMU_ARM_APPLE; }

#else /* APPLE_ARM64_ARCH_FAMILY */

/* We don't currently support non-Apple arm64 PMU configurations like PMUv3 */

void kpc_pmi_handler(unsigned int ctr __unused) {}

void kpc_arch_init(void) { /* No-op */ }

uint32_t kpc_get_classes(void) { return 0; }

uint32_t kpc_fixed_count(void) { return 0; }

uint32_t kpc_configurable_count(void) { return 0; }

uint32_t kpc_fixed_config_count(void) { return 0; }

uint32_t kpc_configurable_config_count(uint64_t pmc_mask __unused) { return 0; }
...
int kpc_get_configurable_config(kpc_config_t *configv __unused,
                                uint64_t pmc_mask __unused) {
  return ENOTSUP;
}

int kpc_get_configurable_counters(uint64_t *counterv __unused,
                                  uint64_t pmc_mask __unused) {
  return ENOTSUP;
}

int kpc_get_fixed_counters(uint64_t *counterv __unused) { return 0; }

boolean_t kpc_is_running_fixed(void) { return FALSE; }

boolean_t kpc_is_running_configurable(uint64_t pmc_mask __unused) {
  return FALSE;
}

int kpc_set_running_arch(struct kpc_running_remote *mp_config __unused) {
  return ENOTSUP;
}

int kpc_set_period_arch(struct kpc_config_remote *mp_config __unused) {
  return ENOTSUP;
}

int kpc_set_config_arch(struct kpc_config_remote *mp_config __unused) {
  return ENOTSUP;
}
...
int kpc_get_all_cpus_counters(uint32_t classes __unused, int *curcpu __unused,
...
int kpc_get_pmu_version(void) { return KPC_PMU_ERROR; }

uint32_t kpc_rawpmu_config_count(void) { return 0; }

int kpc_get_rawpmu_config(__unused kpc_config_t *configv) { return 0; }
