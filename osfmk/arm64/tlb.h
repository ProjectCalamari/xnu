...
static inline void sync_tlb_flush(void) {
#if HAS_FEAT_XS
...
static inline void sync_tlb_flush_local(void) {
#if HAS_FEAT_XS
...
static inline void sync_tlb_flush_strong(void) {
  __builtin_arm_dsb(DSB_ISH);
  __builtin_arm_isb(ISB_SY);
}
...
static inline void arm64_sync_tlb(bool strong __unused) { sync_tlb_flush(); }

// flush_mmu_tlb: full TLB flush on all cores
static inline void flush_mmu_tlb_async(void) { asm volatile("tlbi vmalle1is"); }

static inline void flush_mmu_tlb(void) {
  flush_mmu_tlb_async();
#if HAS_FEAT_XS
  /* Full flush is always treated as "strong" when there is a HW-level
   * distinction. */
...
static inline void flush_core_tlb_async(void) {
#if HAS_FEAT_XS
...
static inline void flush_core_tlb(void) {
  flush_core_tlb_async();
  sync_tlb_flush_local();
}

// flush_mmu_tlb_allentries_async: flush entries that map VA range, all ASIDS,
// all cores start and end are in units of 4K pages.
static inline void flush_mmu_tlb_allentries_async(uint64_t start, uint64_t end,
...
static inline void flush_mmu_tlb_allentries(uint64_t start, uint64_t end,
                                            uint64_t pmap_page_size,
...
// flush_mmu_tlb_entries: flush TLB entries that map a VA range and ASID, all
// cores start and end must have the ASID in the high 16 bits, with the VA in
// units of 4K in the lowest bits Will also flush global entries that match the
// VA range
static inline void flush_mmu_tlb_entries_async(uint64_t start, uint64_t end,
                                               uint64_t pmap_page_size,
                                               bool last_level_only,
                                               bool strong __unused) {
#if __ARM_16K_PG__
...
static inline void flush_mmu_tlb_entries(uint64_t start, uint64_t end,
                                         uint64_t pmap_page_size,
                                         bool last_level_only, bool strong) {
...
static inline void flush_mmu_tlb_asid_async(uint64_t val,
                                            bool strong __unused) {
#if __ARM_KERNEL_PROTECT__
  /*
   * If we are flushing ASID 0, this is a kernel operation.  With this
...
static inline void flush_mmu_tlb_asid(uint64_t val, bool strong) {
...
// ASID must be in high 16 bits of argument
// Will not flush global entries
static inline void flush_core_tlb_asid_async(uint64_t val) {
#if __ARM_KERNEL_PROTECT__
  /*
   * If we are flushing ASID 0, this is a kernel operation.  With this
   * ASID scheme, this means we should flush all ASIDs.
...
static inline void flush_core_tlb_asid(uint64_t val) {
 ...
 */
static inline uint64_t generate_rtlbi_param(ppnum_t npages, uint32_t asid,
                                            vm_offset_t va,
                                            uint64_t pmap_page_shift) {
  assert(npages > 1);
  /**
   * Per the armv8.4 RTLBI extension spec, the range encoded in the rtlbi
   * register operand is defined by: BaseADDR <= VA <
   * BaseADDR+((NUM+1)*2^(5*SCALE+1) * Translation_Granule_Size)
   */
  unsigned order =
      (unsigned)(sizeof(npages) * 8) - (unsigned)__builtin_clz(npages - 1) - 1;
  unsigned scale = ((order ? order : 1) - 1) / 5;
  unsigned granule = 1 << ((5 * scale) + 1);
  unsigned num = (((npages + granule - 1) & ~(granule - 1)) / granule) - 1;
  return tlbi_asid(asid) | RTLBI_TG(pmap_page_shift) | rtlbi_scale(scale) |
         rtlbi_num(num) | rtlbi_addr(va, pmap_page_shift);
}

// flush_mmu_tlb_range: flush TLB entries that map a VA range using a single
// instruction The argument should be encoded according to
// generate_rtlbi_param(). Follows the same ASID matching behavior as
// flush_mmu_tlb_entries()
static inline void flush_mmu_tlb_range_async(uint64_t val, bool last_level_only,
                                             bool strong __unused) {
  if (last_level_only) {
#if HAS_FEAT_XS
    if (__probable(!strong)) {
...
static inline void flush_mmu_tlb_range(uint64_t val, bool last_level_only,
                                       bool strong) {
  flush_mmu_tlb_range_async(val, last_level_only, strong);
  arm64_sync_tlb(strong);
}

// flush_mmu_tlb_allrange: flush TLB entries that map a VA range using a single
// instruction The argument should be encoded according to
// generate_rtlbi_param(). Follows the same ASID matching behavior as
// flush_mmu_tlb_allentries()
static inline void flush_mmu_tlb_allrange_async(uint64_t val,
                                                bool last_level_only,
                                                bool strong __unused) {
  if (last_level_only) {
#if HAS_FEAT_XS
...
static inline void flush_mmu_tlb_allrange(uint64_t val, bool last_level_only,
                                          bool strong) {
  flush_mmu_tlb_allrange_async(val, last_level_only, strong);
  arm64_sync_tlb(strong);
}

// flush_core_tlb_allrange: flush TLB entries that map a VA range using a single
// instruction, local core only The argument should be encoded according to
// generate_rtlbi_param(). Follows the same ASID matching behavior as
// flush_mmu_tlb_allentries()
static inline void flush_core_tlb_allrange_async(uint64_t val) {
#if HAS_FEAT_XS
...
static inline void flush_core_tlb_allrange(uint64_t val) {
