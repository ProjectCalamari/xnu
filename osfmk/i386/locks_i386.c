...
#if !LCK_GRP_USE_ARG
#define usimple_lock_nopreempt(lck, grp) usimple_lock_nopreempt(lck)
#define usimple_lock_try_nopreempt(lck, grp) usimple_lock_try_nopreempt(lck)
#endif
 ...
 * atomic_exchange_abort() - cancel an exchange started with
 * atomic_exchange_begin()
 */
uint32_t atomic_exchange_begin32(uint32_t *target, uint32_t *previous,
...
boolean_t atomic_exchange_complete32(uint32_t *target, uint32_t previous,
                                     uint32_t newval, enum memory_order ord) {
  return __c11_atomic_compare_exchange_strong(
      (_Atomic uint32_t *)target, &previous, newval, ord, memory_order_relaxed);
}

void atomic_exchange_abort(void) {}

boolean_t atomic_test_and_set32(uint32_t *target, uint32_t test_mask,
                                uint32_t set_mask, enum memory_order ord,
                                boolean_t wait) {
  uint32_t value, prev;
...
static void lck_mtx_interlock_lock_clear_flags(lck_mtx_t *mutex,
                                               uint32_t and_flags,
                                               uint32_t *new_state);
static int lck_mtx_interlock_try_lock_set_flags(lck_mtx_t *mutex,
                                                uint32_t or_flags,
                                                uint32_t *new_state);
static boolean_t lck_mtx_lock_wait_interlock_to_clear(lck_mtx_t *lock,
                                                      uint32_t *new_state);
static boolean_t lck_mtx_try_lock_wait_interlock_to_clear(lck_mtx_t *lock,
                                                          uint32_t *new_state);
#endif /* LCK_MTX_USE_ARCH */
...
void lck_spin_lock(lck_spin_t *lck) { usimple_lock((usimple_lock_t)lck, NULL); }

void lck_spin_lock_nopreempt(lck_spin_t *lck) {
  usimple_lock_nopreempt((usimple_lock_t)lck, NULL);
}

void lck_spin_lock_nopreempt_grp(lck_spin_t *lck, lck_grp_t *grp) {
#pragma unused(grp)
 ...
 */
void lck_spin_unlock(lck_spin_t *lck) { usimple_unlock((usimple_lock_t)lck); }

void lck_spin_unlock_nopreempt(lck_spin_t *lck) {
  usimple_unlock_nopreempt((usimple_lock_t)lck);
}

boolean_t lck_spin_try_lock_grp(lck_spin_t *lck, lck_grp_t *grp) {
#pragma unused(grp)
...
boolean_t lck_spin_try_lock(lck_spin_t *lck) {
  boolean_t lrval =
      (boolean_t)usimple_lock_try((usimple_lock_t)lck, LCK_GRP_NULL);
#if DEVELOPMENT || DEBUG
  if (lrval) {
    pltrace(FALSE);
  }
...
int lck_spin_try_lock_nopreempt(lck_spin_t *lck) {
  boolean_t lrval =
      (boolean_t)usimple_lock_try_nopreempt((usimple_lock_t)lck, LCK_GRP_NULL);
#if DEVELOPMENT || DEBUG
  if (lrval) {
    pltrace(FALSE);
  }
...
int lck_spin_try_lock_nopreempt_grp(lck_spin_t *lck, lck_grp_t *grp) {
#pragma unused(grp)
  boolean_t lrval =
      (boolean_t)usimple_lock_try_nopreempt((usimple_lock_t)lck, grp);
#if DEVELOPMENT || DEBUG
  if (lrval) {
    pltrace(FALSE);
  }
 ...
 */
void lck_spin_assert(const lck_spin_t *lock, unsigned int type) {
 ...
 */
void usimple_lock_init(usimple_lock_t l, __unused unsigned short tag) {
  USLDBG(usld_lock_init(l, tag));
  hw_lock_init(&l->interlock);
}

static hw_spin_timeout_status_t
usimple_lock_acquire_timeout_panic(void *_lock, hw_spin_timeout_t to,
                                   hw_spin_state_t st) {
  ...
  panic("Spinlock[%p] " HW_SPIN_TIMEOUT_FMT " ;"
        ...
        "lock owner active on CPU %d, " HW_SPIN_TIMEOUT_DETAILS_FMT,
        l, HW_SPIN_TIMEOUT_ARG(to, st), lowner, current_thread(),
        lsti->owner_cpu, HW_SPIN_TIMEOUT_DETAILS_ARG(to, st));
}
 ...
 */
void(usimple_lock)(usimple_lock_t l LCK_GRP_ARG(lck_grp_t *grp)) {
  DECL_PC(pc);
 ...
 *	maintaining preemption state.
 */
static void usimple_lock_nopreempt(usimple_lock_t l, lck_grp_t *grp) {
  DECL_PC(pc);
 ...
 *	maintaining preemption state.
 */
void usimple_unlock_nopreempt(usimple_lock_t l) {
  DECL_PC(pc);
 ...
 */
unsigned int usimple_lock_try(usimple_lock_t l, lck_grp_t *grp) {
...
void usimple_lock_assert(usimple_lock_t l, unsigned int type) {
 ...
 *	doesn't it make sense to log misses?  XXX
 */
static unsigned int usimple_lock_try_nopreempt(usimple_lock_t l,
                                               lck_grp_t *grp) {
  unsigned int success;
  DECL_PC(pc);
 ...
 *
 */
unsigned int(usimple_lock_try_lock_mp_signal_safe_loop_deadline)(
    usimple_lock_t l, uint64_t deadline LCK_GRP_ARG(lck_grp_t *grp)) {
...
void(usimple_lock_try_lock_loop)(usimple_lock_t l LCK_GRP_ARG(lck_grp_t *grp)) {
...
unsigned int(usimple_lock_try_lock_mp_signal_safe_loop_duration)(
    usimple_lock_t l, uint64_t duration LCK_GRP_ARG(lck_grp_t *grp)) {
  uint64_t deadline;
  uint64_t base_at;
...
#define USLOCK_CHECKING(l) (uslock_check && ((l)->debug.state & USLOCK_CHECKED))

/*
 *	Initialize the debugging information contained
 *	in a usimple_lock.
 */
void usld_lock_init(usimple_lock_t l, __unused unsigned short tag) {
 ...
 */
int usld_lock_common_checks(usimple_lock_t l, char *caller) {
  if (l == USIMPLE_LOCK_NULL) {
  ...
  if (!(l->debug.state & USLOCK_INIT)) {
    panic("%s:  %p is not an initialized lock, 0x%x", caller, l,
          l->debug.state);
  }
 ...
 */
/* ARGSUSED */
void usld_lock_pre(usimple_lock_t l, pc_t pc) {
   ...
   *	current threads, therefore we can't tell if we are trying to retake a
   * lock we have or someone on the other processor has it.  Therefore we just
  ...
  if ((l->debug.state & USLOCK_TAKEN) && l->debug.lock_thread &&
    ...
    printf("%s:  lock %p already locked (at %p) by", caller, l,
           l->debug.lock_pc);
 ...
 */
void usld_lock_post(usimple_lock_t l, pc_t pc) {
  unsigned int mycpu;
  ...
  if (!((l->debug.state & ~USLOCK_TAKEN) == USLOCK_INITIALIZED)) {
    panic("%s:  lock %p became uninitialized", caller, l);
  }
  if ((l->debug.state & USLOCK_TAKEN)) {
    panic("%s:  lock 0x%p became TAKEN by someone else", caller, l);
  }
 ...
 */
void usld_unlock(usimple_lock_t l, pc_t pc) {
  unsigned int mycpu;
  ...
  if (!(l->debug.state & USLOCK_TAKEN)) {
    panic("%s:  lock 0x%p hasn't been taken", caller, l);
  }
  if (l->debug.lock_thread != (void *)current_thread()) {
    panic("%s:  unlocking lock 0x%p, owned by thread %p", caller, l,
          l->debug.lock_thread);
  }
  if (l->debug.lock_cpu != mycpu) {
    printf("%s:  unlocking lock 0x%p on cpu 0x%x", caller, l, mycpu);
 ...
 */
void usld_lock_try_pre(usimple_lock_t l, __unused pc_t pc) {
 ...
 *	to use cpu_number.
 */
void usld_lock_try_post(usimple_lock_t l, pc_t pc) {
  unsigned int mycpu;
  ...
  if (!((l->debug.state & ~USLOCK_TAKEN) == USLOCK_INITIALIZED)) {
    panic("%s:  lock 0x%p became uninitialized", caller, l);
  }
  if ((l->debug.state & USLOCK_TAKEN)) {
    panic("%s:  lock 0x%p became TAKEN by someone else", caller, l);
  }
 ...
 *
 * To avoid to inline these functions on the fast path, all functions directly
 * called by the fast paths have the __attribute__((noinline)) specified. Also
 * they are all implemented in such a way the fast path can tail call into them.
 * In this way the return address does not need to be pushed on the caller stack
 * and stack optimization can happen on the caller.
 *
 * Slow path code is structured in such a way there are no calls to functions
 * that will return on the context of the caller function, i.e. all functions
 * called are or tail call functions or inline functions. The number of
 * arguments of the tail call functions are less then six, so that they can be
 * passed over registers and do not need to be pushed on stack. This allows the
 * compiler to not create a stack frame for the functions.
 *
 * __improbable and __probable are used to compile the slow path code in such a
 * way the fast path case will be on a sequence of instructions with as less
 * jumps as possible, to make this case the most optimized even if falling
 ...
 *
 * lck_mtx_waiters: contains the count of threads currently in the mutex
 * waitqueue
 *
 ...
 * lock owner may not have been the last thread to have been woken up depending
 * on the luck of the draw.  Therefore a last-owner may still have the
 * promoted-on-wakeup flag set.
 *
 * TODO: Figure out an algorithm for stopping a lock holder which is already at
 * the right priority from dropping priority in the future without having to
 * take thread lock on acquire.
 */
...
#if DEVELOPMENT | DEBUG
__attribute__((noinline)) void lck_mtx_owner_check_panic(lck_mtx_t *lock) {
  thread_t owner = ctid_get_thread_unsafe(lock->lck_mtx_owner);
  panic("Mutex unlock attempted from non-owner thread. Owner=%p lock=%p", owner,
        lock);
}
#endif
 ...
 */
__attribute__((noinline)) void lck_mtx_unlock_slow(lck_mtx_t *lock) {
  thread_t thread;
  uint32_t state;
...
unlock:
  ...
  ordered_store_mtx_state_release(
      lock,
      state); /* since I own the interlock, I don't need an atomic update */
...
lck_mtx_unlock_wakeup_tail(lck_mtx_t *mutex, uint32_t state) {
  struct turnstile *ts;

  __kdebug_only uintptr_t trace_lck = unslide_for_kdebug(mutex);
  kern_return_t did_wake;

  KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_MTX_LCK_WAKEUP_CODE) |
                   DBG_FUNC_START,
...
lck_mtx_lock_acquire_inline(lck_mtx_t *mutex, struct turnstile *ts) {
  __kdebug_only uintptr_t trace_lck = unslide_for_kdebug(mutex);

  KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_MTX_LCK_ACQUIRE_CODE) |
                   DBG_FUNC_START,
               trace_lck, 0, mutex->lck_mtx_waiters, 0, 0);
...
void lck_mtx_lock_acquire_x86(lck_mtx_t *mutex) {
...
__attribute__((noinline)) static void
lck_mtx_lock_acquire_tail(lck_mtx_t *mutex, struct turnstile *ts) {
...
lck_mtx_try_lock_acquire_tail(lck_mtx_t *mutex) {
...
__attribute__((noinline)) static void
lck_mtx_convert_spin_acquire_tail(lck_mtx_t *mutex) {
  lck_mtx_lock_acquire_inline(mutex, NULL);
  lck_mtx_convert_spin_finish_inline(mutex, ordered_load_mtx_state(mutex));
}

static void lck_mtx_ilk_unlock(lck_mtx_t *mutex) {
  lck_mtx_ilk_unlock_inline(mutex, ordered_load_mtx_state(mutex));
}

static inline void
lck_mtx_interlock_lock_set_and_clear_flags(lck_mtx_t *mutex, uint32_t xor_flags,
                                           uint32_t and_flags,
                                           uint32_t *new_state) {
  uint32_t state, prev;
  state = *new_state;
...
static inline void lck_mtx_interlock_lock_clear_flags(lck_mtx_t *mutex,
                                                      uint32_t and_flags,
                                                      uint32_t *new_state) {
  return lck_mtx_interlock_lock_set_and_clear_flags(mutex, 0, and_flags,
                                                    new_state);
}

static inline void lck_mtx_interlock_lock(lck_mtx_t *mutex,
                                          uint32_t *new_state) {
  return lck_mtx_interlock_lock_set_and_clear_flags(mutex, 0, 0, new_state);
}

static inline int lck_mtx_interlock_try_lock_set_flags(lck_mtx_t *mutex,
                                                       uint32_t or_flags,
                                                       uint32_t *new_state) {
  uint32_t state, prev;
  state = *new_state;
...
__attribute__((noinline)) static void
lck_mtx_lock_contended(lck_mtx_t *lock, bool profile, boolean_t *first_miss) {
...
__attribute__((noinline)) __abortlike static void
lck_mtx_destroyed(lck_mtx_t *lock) {
  panic("trying to interlock destroyed mutex (%p)", lock);
}

__attribute__((noinline)) static boolean_t
lck_mtx_try_destroyed(lck_mtx_t *lock) {
  panic("trying to interlock destroyed mutex (%p)", lock);
  return FALSE;
}

__attribute__((always_inline)) static boolean_t
lck_mtx_lock_wait_interlock_to_clear(lck_mtx_t *lock, uint32_t *new_state) {
  uint32_t state;
...
__attribute__((always_inline)) static boolean_t
lck_mtx_try_lock_wait_interlock_to_clear(lck_mtx_t *lock, uint32_t *new_state) {
  uint32_t state;
 ...
 */
__attribute__((noinline)) void lck_mtx_lock_slow(lck_mtx_t *lock) {
  bool profile;
  uint32_t state;
  int first_miss = 0;
  ...
  while (__improbable(!lck_mtx_interlock_try_lock_set_flags(
...
__attribute__((noinline)) boolean_t lck_mtx_try_lock_slow(lck_mtx_t *lock) {
  bool profile;
  uint32_t state;
  int first_miss = 0;
  ...
  if (__improbable(!lck_mtx_interlock_try_lock_set_flags(
          lock, LCK_MTX_MLOCKED_MSK, &state))) {
    if (__improbable(profile)) {
      LCK_MTX_PROF_MISS(lock, lock->lck_mtx_grp, &first_miss);
    }
...
__attribute__((noinline)) void lck_mtx_lock_spin_slow(lck_mtx_t *lock) {
  bool profile;
  uint32_t state;
  int first_miss = 0;
  ...
  if (__probable(lck_mtx_interlock_try_lock_set_flags(lock, LCK_MTX_SPIN_MSK,
                                                      &state))) {
    goto past_spin;
  }
  ...
  while (__improbable(
      !lck_mtx_interlock_try_lock_set_flags(lock, LCK_MTX_SPIN_MSK, &state))) {
    if (!lck_mtx_lock_wait_interlock_to_clear(lock, &state)) {
      acquired = false;
      break;
    }
...
lck_mtx_try_lock_spin_slow(lck_mtx_t *lock) {
  bool profile;
  uint32_t state;
  int first_miss = 0;
  ...
  /* note - can't be DESTROYED or locked */
  if (__improbable(!lck_mtx_interlock_try_lock_set_flags(lock, LCK_MTX_SPIN_MSK,
                                                         &state))) {
    if (__improbable(profile)) {
      LCK_MTX_PROF_MISS(lock, lock->lck_mtx_grp, &first_miss);
    }
...
__attribute__((noinline)) void lck_mtx_convert_spin(lck_mtx_t *lock) {
  uint32_t state;

  state = ordered_load_mtx_state(lock);

  assertf(lock->lck_mtx_owner == current_thread()->ctid,
          "lock %p not owned by thread ctid %d (current owner %d)", lock,
          current_thread()->ctid, lock->lck_mtx_owner);
...
static inline boolean_t lck_mtx_lock_grab_mutex(lck_mtx_t *lock) {
  uint32_t state;

  state = ordered_load_mtx_state(lock);

  if (!lck_mtx_interlock_try_lock_set_flags(lock, LCK_MTX_MLOCKED_MSK,
                                            &state)) {
    return FALSE;
  }
...
__attribute__((noinline)) void lck_mtx_assert(lck_mtx_t *lock,
                                              unsigned int type) {
  thread_t thread;
  uint32_t state;
 ...
 * returns LCK_MTX_SPINWAIT_NO_SPIN if we didn't spin due to the holder not
 * running
 */
__attribute__((noinline)) lck_mtx_spinwait_ret_type_t
lck_mtx_lock_spinwait_x86(lck_mtx_t *mutex) {
  __kdebug_only uintptr_t trace_lck = unslide_for_kdebug(mutex);
  ...
  KERNEL_DEBUG(
      ...
      trace_lck,
      VM_KERNEL_UNSLIDE_OR_PERM(ctid_get_thread_unsafe(mutex->lck_mtx_owner)),
  ...
  do {
    ...
    if (owner) {
       ...
       * Check if the thread that is running on the other cpus matches the
       * owner.
       */
      if (owner) {
        ...
        do {
          if ((cpu_data_ptr[i] != NULL) &&
              (cpu_data_ptr[i]->cpu_active_thread == owner_thread)) {
            owner_on_core = true;
            break;
          }
          if (++i >= real_ncpus) {
    ...
    if (cur_time >= window_deadline) {
      ...
      if (adjust) {
        ...
        if (unfairness == 0) {
          /*
           * We observed the owner changing `total_hold_time_samples` times
           * which let us estimate the average hold time of this mutex for the
           * duration of the spin time. avg_hold_time = (cur_time - start_time)
           * / total_hold_time_samples;
           *
          ...
          sliding_deadline =
              start_time + (delta * (real_ncpus - 1)) / total_hold_time_samples;
        } else {
          /*
           * In this case at least one of the other cpus was able to get the
           * lock twice while I was spinning. We could spin longer but it won't
           * necessarily help if the system is unfair. Try to randomize the wait
           ...
           * distributed_increment = ((high_deadline - cur_time) / real_ncpus) *
           * bias
           */
 ...
 *
 * Priority:
 *      This avoids taking the thread lock if the owning thread is the same
 * priority. This optimizes the case of same-priority threads contending on a
 * lock. However, that allows the owning thread to drop in priority while
 * holding the lock, because there is no state that the priority change can
 * notice that says that the targeted thread holds a contended mutex.
 *
 *      One possible solution: priority changes could look for some atomic tag
 *      on the thread saying 'holding contended lock', and then set up a
 * promotion. Needs a story for dropping that promotion - the last contended
 * unlock has to notice that this has happened.
 */
__attribute__((noinline)) void lck_mtx_lock_wait_x86(lck_mtx_t *mutex,
                                                     struct turnstile **ts) {
  ...
  KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_MTX_LCK_WAIT_CODE) |
                   DBG_FUNC_START,
  ...
  /*
   * lck_mtx_lock_wait_x86 might be called on a loop. Call prepare just once and
   * reuse the same turnstile while looping, the matching turnstile compleate
   * will be called by lck_mtx_lock_contended when finally acquiring the lock.
   */
  if (*ts == NULL) {
    *ts = turnstile_prepare_hash((uintptr_t)mutex, TURNSTILE_KERNEL_MUTEX);
  }
  ...
  waitq_assert_wait64(&turnstile->ts_waitq, LCK_MTX_EVENT(mutex),
  ...
  KERNEL_DEBUG(
      MACHDBG_CODE(DBG_MACH_LOCKS, LCK_MTX_LCK_WAIT_CODE) | DBG_FUNC_END,
      trace_lck,
      VM_KERNEL_UNSLIDE_OR_PERM(ctid_get_thread_unsafe(mutex->lck_mtx_owner)),
      mutex->lck_mtx_waiters, 0, 0);
 ...
 *      Returns: TRUE if lock is acquired.
 */
boolean_t kdp_lck_mtx_lock_spin_is_acquired(lck_mtx_t *lck) {
  if (not_in_kdp) {
    panic("panic: kdp_lck_mtx_lock_spin_is_acquired called outside of kernel "
          "debugger");
  }
...
void kdp_lck_mtx_find_owner(__unused struct waitq *waitq, event64_t event,
...
#endif /* LCK_MTX_USE_ARCH */
#if CONFIG_PV_TICKET
__startup_func void lck_init_pv(void) {
  uint32_t pvtck = 1;
  PE_parse_boot_argn("pvticket", &pvtck, sizeof(pvtck));
  if (pvtck == 0) {
    return;
  }
  has_lock_pv = cpuid_vmm_present() && (cpuid_vmm_get_kvm_features() &
                                        CPUID_KVM_FEATURE_PV_UNHALT) != 0;
}
