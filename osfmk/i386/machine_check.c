...
typedef enum { CLEAR, DUMPING, DUMPED } mca_dump_state_t;
static volatile mca_dump_state_t mca_dump_state = CLEAR;

static void mca_get_availability(void) {
  ...
  if (family == 6 && ((model == CPUID_MODEL_HASWELL && stepping < 3) ||
...
boolean_t mca_is_cmci_present(void) {
  if (!mca_initialized) {
    mca_cpu_init();
  }
...
void mca_cpu_alloc(cpu_data_t *cdp) {
   ...
   */
  mca_state_size =
      sizeof(mca_state_t) + sizeof(mca_mci_bank_t) * mca_error_bank_count;
  cdp->cpu_mca_state =
      zalloc_permanent_tag(mca_state_size, ZALIGN_PTR, VM_KERN_MEMORY_CPU);
...
static void mca_save_state(mca_state_t *mca_state) {
  mca_mci_bank_t *bank;
  unsigned int i;
  ...
  mca_state->mca_mcg_ctl =
      mca_control_MSR_present ? rdmsr64(IA32_MCG_CTL) : 0ULL;
  ...
  for (i = 0; i < mca_error_bank_count; i++, bank++) {
    ...
    bank->mca_mci_misc =
        (bank->mca_mci_status.bits.miscv) ? rdmsr64(IA32_MCi_MISC(i)) : 0ULL;
    bank->mca_mci_addr =
        (bank->mca_mci_status.bits.addrv) ? rdmsr64(IA32_MCi_ADDR(i)) : 0ULL;
...
static void mca_dump_bank(mca_state_t *state, int i) {
  mca_mci_bank_t *bank;
  ...
  paniclog_append_noflush(" IA32_MC%d_STATUS(0x%x): 0x%016qx\n", i,
                          IA32_MCi_STATUS(i), status.u64);

  if (status.bits.addrv) {
    paniclog_append_noflush(" IA32_MC%d_ADDR(0x%x):   0x%016qx\n", i,
                            IA32_MCi_ADDR(i), bank->mca_mci_addr);
  }

  if (status.bits.miscv) {
    paniclog_append_noflush(" IA32_MC%d_MISC(0x%x):   0x%016qx\n", i,
                            IA32_MCi_MISC(i), bank->mca_mci_misc);
  }
}

static void mca_cpu_dump_error_banks(mca_state_t *state) {
  unsigned int i;
...
void mca_dump(void) {
  ...
  for (i = 0; i < real_ncpus; i++) {
    ...
    if (mcsp == NULL || mcsp->mca_is_saved == FALSE ||
        mcsp->mca_mcg_status.u64 == 0 || !mcsp->mca_is_valid) {
      continue;
    }
...
void mca_exception_panic(void) { lapic_trigger_MC(); }
#endif
