 ...
 */
static void *ALLOCPAGES(int npages) {
...
static void fillkpt(pt_entry_t *base, int prot, uintptr_t src, int index,
                    int count) {
  int i;
  for (i = 0; i < count; i++) {
...
static int physmap_init_L2(uint64_t *physStart, pt_entry_t **l2ptep) {
  unsigned i;
  pt_entry_t *physmapL2 = ALLOCPAGES(1);

  if (physmapL2 == NULL) {
    DBG("physmap_init_L2 page alloc failed when initting L2 for physAddr "
        "0x%llx.\n",
        *physStart);
    *l2ptep = NULL;
    return -1;
  ...
  for (i = 0; i < NPDPG; i++) {
    physmapL2[i] = *physStart | INTEL_PTE_PS | INTEL_PTE_VALID | INTEL_PTE_NX |
                   INTEL_PTE_WRITE;
...
static int physmap_init_L3(int startIndex, uint64_t highest_phys,
                           uint64_t *physStart, pt_entry_t **l3ptep) {
  unsigned i;
  int ret;
  pt_entry_t *l2pte;
  ...
  if (physmapL3 == NULL) {
    DBG("physmap_init_L3 page alloc failed when initting L3 for  physAddr "
        "0x%llx.\n",
        *physStart);
    *l3ptep = NULL;
    return -1;
  ...
  for (i = startIndex; i < NPDPTPG && *physStart < highest_phys; i++) {
    ...
    physmapL3[i] = ((uintptr_t)ID_MAP_VTOP(l2pte)) | INTEL_PTE_VALID |
                   INTEL_PTE_NX | INTEL_PTE_WRITE;
  }
...
static void physmap_init(uint8_t phys_random_L3, uint64_t *new_physmap_base,
  ...
  /*
   * Add 4GB to the loader-provided physical memory size to account for MMIO
   * space
   * XXX in a perfect world, we'd scan PCI buses and count the max memory
   * requested in BARs by
   * XXX all enumerated device, then add more for hot-pluggable devices.
   */
  highest_physaddr = kernelBootArgs->PhysicalMemorySize + 4 * GB;

  /*
   * Calculate the number of PML4 entries we'll need.  The total number of
   * entries is pdpte_count = (((highest_physaddr) >> PDPT_SHIFT) +
   * entropy_value +
  ...
  pdpte_count =
      (unsigned)(((highest_physaddr + PDPTMASK) >> PDPTSHIFT) + phys_random_L3);
  kernPhysPML4EntryCount =
      (pdpte_count + ((1U << (PML4SHIFT - PDPTSHIFT)) - 1)) >>
      (PML4SHIFT - PDPTSHIFT);
  ...
  if (kernPhysPML4EntryCount > KERNEL_PHYSMAP_PML4_COUNT_MAX) {
    ...
    kprintf("[pmap] Limiting physmap to %d PML4s (was %d)\n",
            KERNEL_PHYSMAP_PML4_COUNT_MAX, kernPhysPML4EntryCount);
  ...
  kernPhysPML4Index =
      KERNEL_KEXTS_INDEX -
      kernPhysPML4EntryCount; /* utb: KERNEL_PHYSMAP_PML4_INDEX */

  /*
   * XXX: Make sure that the addresses returned for physmapL3 and physmapL2 plus
   * their extents are in the system-available memory range
   */
  ...
  for (pml4_index = kernPhysPML4Index;
       pml4_index < (kernPhysPML4Index + kernPhysPML4EntryCount) &&
       physAddr < highest_physaddr;
       pml4_index++) {
    if (physmap_init_L3(L3_start_index, highest_physaddr, &physAddr, &l3pte) <
        0) {
    ...
    IdlePML4[pml4_index] = ((uintptr_t)ID_MAP_VTOP(l3pte)) | INTEL_PTE_VALID |
                           INTEL_PTE_NX | INTEL_PTE_WRITE;
  }
  ...
  *new_physmap_max = *new_physmap_base + physAddr;
  ...
  for (i = kernPhysPML4Index; i < (kernPhysPML4Index + kernPhysPML4EntryCount);
       i++) {
...
static void Idle_PTs_init(void) {
  uint64_t rand64;
  ...
  fillkpt(KPTphys, INTEL_PTE_WRITE, 0, 0,
          (int)(((uintptr_t)physfree) >> PAGE_SHIFT));

  /* IdlePTD */
  fillkpt(IdlePTD, INTEL_PTE_WRITE, (uintptr_t)ID_MAP_VTOP(KPTphys), 0, NKPT);

  // IdlePDPT entries
  fillkpt(IdlePDPT, INTEL_PTE_WRITE, (uintptr_t)ID_MAP_VTOP(IdlePTD), 0,
          NPGPTD);

  // IdlePML4 single entry for kernel space.
  fillkpt(IdlePML4 + KERNEL_PML4_INDEX, INTEL_PTE_WRITE,
          (uintptr_t)ID_MAP_VTOP(IdlePDPT), 0, 1);
 ...
 */
void Idle_PTs_release(vm_offset_t start, vm_offset_t end) {
  uint32_t i;
...
#define BOOT_TRAP_VECTOR(t)                                                    \
  [t] = {(uintptr_t)&vstart_trap_handler, KERNEL64_CS, 0,                      \
         ACC_P | ACC_PL_K | ACC_INTR_GATE, 0},
...
struct fake_descriptor64 master_boot_idt64[IDTSZ]
    ...
    __attribute__((aligned(PAGE_SIZE))) = {FOR_0_TO_31(BOOT_TRAP_VECTOR)};

static void vstart_idt_init(boolean_t master) {
  x86_64_desc_register_t vstart_idt = {sizeof(master_boot_idt64),
                                       master_boot_idt64};
...
kern_return_t i386_slide_individual_kext(kernel_mach_header_t *mh,
                                         uintptr_t slide) {
  int ret = kernel_collection_slide(
      mh, (const void **)(void *)collection_base_pointers);
  if (ret != 0) {
...
kern_return_t i386_slide_kext_collection_mh_addrs(kernel_mach_header_t *mh,
                                                  uintptr_t slide,
  ...
  kernel_collection_adjust_mh_addrs(mh, slide, adjust_mach_headers, NULL, NULL,
                                    NULL, NULL, NULL, NULL, NULL);

  return KERN_SUCCESS;
}

static void i386_slide_and_rebase_image(uintptr_t kstart_addr) {
   ...
   */
  kernel_collection_adjust_mh_addrs(kc_mh, slide, false, NULL, NULL, NULL, NULL,
                                    NULL, NULL, &kc_highest_nonlinkedit_vmaddr);
}
 ...
 */
__attribute__((noreturn)) void vstart(vm_offset_t boot_args_start) {
  ...
  if (is_boot_cpu) {
    ...
    physfree =
        (void *)(uintptr_t)((lphysfree + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1));
    ...
    DBG("bootargs: %p, &ksize: %p &kaddr: %p\n", kernelBootArgs,
        &kernelBootArgs->ksize, &kernelBootArgs->kaddr);
    ...
    if (kernelBootArgs->Version >= 2 && kernelBootArgs->Revision >= 1 &&
       ...
       * that correspond to the kexts present in the primary KC, into slid
       * addresses.
       */
      i386_slide_and_rebase_image(
          (uintptr_t)ml_static_ptovirt(kernelBootArgs->KC_hdrs_vaddr));
    }
...
void i386_init(void) {
  ...
  if (PE_parse_boot_argn("insn_capcnt", &insn_copyin_count,
   ...
   */
  PE_parse_boot_argn("himemory_mode", &vm_himemory_mode,
...
#if DEBUG
  nanoseconds_to_absolutetime(URGENCY_NOTIFICATION_ASSERT_NS,
...
static void __dead2 do_init_slave(boolean_t fast_restart) {
   ...
   * active thread (that is, that this CPU has already been started at some
   * point), otherwise the ktrace calls within the kptimer operations will try
   * to deref the current thread and will instead cause a system reset. If this
   * is the first time the CPU is being started, we don't need to call
   * kptimer_curcpu_up().
   */
 ...
 */
void i386_init_slave(void) { do_init_slave(FALSE); }
 ...
 * CPU again.
 */
void i386_init_slave_fast(void) { do_init_slave(TRUE); }
...
void doublemap_init(uint8_t randL3) {
  ...
  dblmapL3[randL3] = ((uintptr_t)ID_MAP_VTOP(&dblmapL2[0])) | INTEL_PTE_VALID |
                     INTEL_PTE_WRITE;
  ...
  kernel_section_t *thdescsect =
      getsectbynamefromheader(&_mh_execute_header, "__HIB", "__text");
  ...
  for (int i = 0; hdescc < hdesce; i++) {
    ...
    dblallocs++;
    dblmapL2[0].entries[i] = ((uintptr_t)ID_MAP_VTOP(&dblmapL1[0])) |
                             INTEL_PTE_VALID | INTEL_PTE_WRITE | INTEL_PTE_REF;
  ...
  IdlePML4[KERNEL_DBLMAP_PML4_INDEX] = ((uintptr_t)ID_MAP_VTOP(dblmapL3)) |
  ...
  idt64_hndl_table0[1] =
      DBLMAP(idt64_hndl_table0[1]); /* 64-bit exit trampoline */
  idt64_hndl_table0[3] =
      DBLMAP(idt64_hndl_table0[3]); /* 32-bit exit trampoline */
  ...
  DBG("double map idlepml4[%d]: 0x%llx\n", KERNEL_DBLMAP_PML4_INDEX,
      IdlePML4[KERNEL_DBLMAP_PML4_INDEX]);
...
vm_offset_t dyn_dblmap(vm_offset_t cva, vm_offset_t sz) {
  ...
  pmap_alias(ava, cva, cva + sz, VM_PROT_READ | VM_PROT_WRITE,
             PMAP_EXPAND_OPTIONS_ALIASMAP);
  dblmap_max += sz;
  return ava - cva;
}
/* Adjust offsets interior to the bootstrap interrupt descriptor table to
 * redirect control to the double-mapped interrupt vectors. The IDTR proper will
 * be programmed via cpu_desc_load()
 */
void idt64_remap(void) {
