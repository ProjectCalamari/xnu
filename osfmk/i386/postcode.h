...
#define POSTCODE_AX                                                            \
  ...
  jne 1b
#else
#define POSTCODE_AL outb % al, $(POSTPORT)
#define POSTCODE_AX outw % ax, $(POSTPORT)
...
#ifndef ASSEMBLER
inline static void _postcode_delay(uint32_t spincount) {
  asm volatile("1:			\n\t"
               "  rep; nop;		\n\t"
               "  decl %%eax;		\n\t"
               "  jne 1b"
...
inline static void _postcode(uint8_t xx) {
  asm volatile("outb %0, %1" : : "a"(xx), "N"(POSTPORT));
}
inline static void _postcode2(uint16_t xxxx) {
  asm volatile("outw %0, %1" : : "a"(xxxx), "N"(POSTPORT));
}
#if DEBUG_POSTCODE
inline static void postcode(uint8_t xx) {
  _postcode(xx);
#if POSTCODE_DELAY
  _postcode_delay(SPINCOUNT);
#endif
}
inline static void postcode2(uint8_t xxxx) {
  _postcode2(xxxx);
#if POSTCODE_DELAY
  _postcode_delay(SPINCOUNT);
#endif
