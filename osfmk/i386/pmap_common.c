...
pmap_pagetable_corruption_record_t
    pmap_pagetable_corruption_records[PMAP_PAGETABLE_CORRUPTION_MAX_LOG];
...
uint32_t pv_hashed_low_water_mark, pv_hashed_kern_low_water_mark,
    pv_hashed_alloc_chunk, pv_hashed_kern_alloc_chunk;
...
boolean_t pmap_has_managed_page(ppnum_t first, ppnum_t last) {
  ...
  for (result = FALSE, pn = first;
       !result && (pn <= last) && (pn <= last_managed_page); pn++) {
...
pmap_pagetable_corruption_msg_log(int (*log_func)(const char *fmt, ...)
                                      __printflike(1, 2)) {
  if (pmap_pagetable_corruption_incidents > 0) {
    int i, j,
        e = MIN(pmap_pagetable_corruption_incidents,
    ...
    for (i = 0; i < e; i++) {
      ...
      if (pmap_pagetable_corruption_records[i].adj_ptes_count > 0) {
        for (j = 0; j < pmap_pagetable_corruption_records[i].adj_ptes_count;
             j++) {
...
static inline void pmap_pagetable_corruption_log_setup(void) {
  if (pmap_pagetable_corruption_log_call == NULL) {
    nanotime_to_absolutetime(PMAP_PAGETABLE_CORRUPTION_INTERVAL, 0,
...
void mapping_free_prime(void) {
  ...
  pv_hashed_low_water_mark = MAX(PV_HASHED_LOW_WATER_MARK_DEFAULT,
                                 ((uint32_t)(sane_size >> 30)) * 2000);
  pv_hashed_low_water_mark = MIN(pv_hashed_low_water_mark, 16000);
  /* Alterable via sysctl */
  pv_hashed_kern_low_water_mark = MAX(PV_HASHED_KERN_LOW_WATER_MARK_DEFAULT,
                                      ((uint32_t)(sane_size >> 30)) * 1000);
...
void mapping_adjust(void) {
  ...
  mres =
      kernel_thread_start_priority((thread_continue_t)mapping_replenish, NULL,
                                   MAXPRI_KERNEL, &mapping_replenish_thread);
...
__attribute__((noreturn)) void mapping_replenish(void) {
  pv_hashed_entry_t pvh_e;
  pv_hashed_entry_t pvh_eh;
  pv_hashed_entry_t pvh_et;
  int pv_cnt;
...
void pmap_set_modify(ppnum_t pn) { phys_attribute_set(pn, PHYS_MODIFIED); }
 ...
 *
 */
unsigned int pmap_disconnect_options(ppnum_t pa, unsigned int options,
                                     void *arg) {
