 ...
 */
void print_one_backtrace(pmap_t pmap, vm_offset_t topfp, const char *cur_marker,
                         boolean_t is_64_bit) {
  unsigned int i = 0;
  addr64_t lr = 0;
  addr64_t fp = topfp;
  ...
  do {
    ...
    if (dump_kernel_stack &&
        ((fp < VM_MIN_KERNEL_ADDRESS) || (fp > VM_MAX_KERNEL_ADDRESS))) {
      break;
    }
    ...
    if (ppn != (ppnum_t)NULL) {
      if (is_64_bit) {
        lr = ml_phys_read_double_64(((((vm_offset_t)ppn) << PAGE_SHIFT)) |
                                    ((fp + FP_LR_OFFSET64) & PAGE_MASK));
      } else {
        lr = ml_phys_read_word(((((vm_offset_t)ppn) << PAGE_SHIFT)) |
                               ((fp + FP_LR_OFFSET) & PAGE_MASK));
      }
    } else {
      if (is_64_bit) {
        ...
        paniclog_append_noflush(
            "%s\t  Could not read LR from frame at 0x%08x\n", cur_marker,
            (uint32_t)(fp + FP_LR_OFFSET));
      }
      break;
    }
    ...
    if (ppn != (ppnum_t)NULL) {
      if (is_64_bit) {
        fp = ml_phys_read_double_64(((((vm_offset_t)ppn) << PAGE_SHIFT)) |
                                    (fp & PAGE_MASK));
      } else {
        fp = ml_phys_read_word(((((vm_offset_t)ppn) << PAGE_SHIFT)) |
                               (fp & PAGE_MASK));
      }
    } else {
      if (is_64_bit) {
        ...
        paniclog_append_noflush(
            "%s\t  Could not read FP from frame at 0x%08x\n", cur_marker,
            (uint32_t)fp);
      }
      break;
    }
...
void machine_startup(void) {
  ...
  if (!PE_parse_boot_argn("nvram_paniclog", &commit_paniclog_to_nvram,
...
static void efi_set_tables_64(EFI_SYSTEM_TABLE_64 *system_table) {
  ...
  do {
    ...
    runtime =
        (EFI_RUNTIME_SERVICES_64 *)(uintptr_t)system_table->RuntimeServices;
    ...
    for (i = 0; i < mcount;
      ...
      if (((mptr->Attribute & EFI_MEMORY_RUNTIME) == EFI_MEMORY_RUNTIME)) {
        ...
        DPRINTF(" Type: %x phys: %p EFIv: %p kv: %p size: %p\n", mptr->Type,
                ...
                (void *)(uintptr_t)mptr->VirtualStart, (void *)vm_addr,
                (void *)vm_size);
        pmap_map_bd(vm_addr, phys_addr, phys_addr + round_page(vm_size),
                    (mptr->Type == kEfiRuntimeServicesCode)
                        ? VM_PROT_READ | VM_PROT_EXECUTE
                        : VM_PROT_READ | VM_PROT_WRITE,
                    (mptr->Type == EfiMemoryMappedIO) ? VM_WIMG_IO
                                                      : VM_WIMG_USE_DEFAULT);
      }
    }

    if (args->Version != kBootArgsVersion2) {
      panic("Incompatible boot args version %d revision %d", args->Version,
            args->Revision);
    }

    DPRINTF("Boot args version %d revision %d mode %d\n", args->Version,
            args->Revision, args->efiMode);
    if (args->efiMode == kBootArgsEfiMode64) {
      efi_set_tables_64(
          (EFI_SYSTEM_TABLE_64 *)ml_static_ptovirt(args->efiSystemTable));
    } else {
...
boolean_t bootloader_valid_page(ppnum_t ppn) {
  boot_args *args = (boot_args *)PE_state.bootArgs;
...
void hibernate_newruntime_map(void *map, vm_size_t map_size,
    ...
    for (i = 0; i < mcount;
      ...
      if ((mptr->Attribute & EFI_MEMORY_RUNTIME) == EFI_MEMORY_RUNTIME) {
        ...
        kprintf("mapping[%u] %qx @ %lx, %llu\n", mptr->Type, phys_addr,
                (unsigned long)vm_addr, mptr->NumberOfPages);
      }
    }

    pmap_remove(kernel_pmap, i386_ptob(args->efiRuntimeServicesPageStart),
                i386_ptob(args->efiRuntimeServicesPageStart +
                          args->efiRuntimeServicesPageCount));
 ...
 * Halt a cpu.
 */
void halt_cpu(void) { halt_all_cpus(FALSE); }
 ...
 */
void RecordPanicStackshot() {
  int err = 0;
  size_t bytes_traced = 0, bytes_uncompressed = 0, bytes_used = 0,
         bytes_remaining = 0;
  ...
  if (begun_panic_stackshot) {
    if (panic_info->mph_other_log_offset == 0) {
      panic_info->mph_other_log_offset =
  ...
  /* The panic log length should have been set before we came to capture a
   * stackshot */
  if (panic_info->mph_panic_log_len == 0) {
    kdb_printf(
        "Found zero length panic log, skipping capturing panic stackshot\n");
    if (panic_info->mph_other_log_offset == 0) {
      panic_info->mph_other_log_offset =
          PE_get_offset_into_panic_region(debug_buf_ptr);
    }
  ...
  if (extended_debug_log_enabled) {
    ...
    bytes_remaining =
        debug_buf_size - (unsigned int)((uintptr_t)stackshot_begin_loc -
  ...
  err = kcdata_memory_static_init(
      &kc_panic_data, (mach_vm_address_t)stackshot_begin_loc,
      KCDATA_BUFFER_BEGIN_COMPRESSED, (unsigned int)bytes_remaining,
      KCFLAG_USE_MEMCOPY);
  if (err != KERN_SUCCESS) {
    panic_info->mph_panic_flags |=
    ...
    panic_info->mph_other_log_offset =
        PE_get_offset_into_panic_region(debug_buf_ptr);
    kdb_printf("Failed to initialize kcdata buffer for in-memory panic "
               "stackshot, skipping ...\n");
    return;
  }

  uint64_t stackshot_flags =
      (STACKSHOT_SAVE_KEXT_LOADINFO | STACKSHOT_SAVE_LOADINFO |
       STACKSHOT_KCDATA_FORMAT | STACKSHOT_ENABLE_BT_FAULTING |
       STACKSHOT_ENABLE_UUID_FAULTING | STACKSHOT_FROM_PANIC |
       STACKSHOT_DO_COMPRESS | STACKSHOT_NO_IO_STATS |
       STACKSHOT_THREAD_WAITINFO | STACKSHOT_DISABLE_LATENCY_INFO |
       STACKSHOT_GET_DQ);

  err = kcdata_init_compress(&kc_panic_data, KCDATA_BUFFER_BEGIN_STACKSHOT,
...
#if DEVELOPMENT
  /*
   * Include the shared cache layout in panic stackshots on DEVELOPMENT kernels
   * so that we can symbolicate panic stackshots from corefiles.
   */
  stackshot_flags |= STACKSHOT_COLLECT_SHAREDCACHE_LAYOUT;
#endif

  kdp_snapshot_preflight(-1, (void *)stackshot_begin_loc,
                         (uint32_t)bytes_remaining, stackshot_flags,
                         &kc_panic_data, 0, 0);
  ...
  if ((err != KERN_SUCCESS) && (bytes_used > 0)) {
    /*
     * We ran out of space while trying to capture a stackshot, try again
     * without user frames. It's not safe to log from here (in case we're
     * writing in the middle of the debug buffer on coprocessor systems) but
     * append a flag to the panic flags.
     */
    panic_info->mph_panic_flags |=
        MACOS_PANIC_HEADER_FLAG_STACKSHOT_KERNEL_ONLY;
    panic_stackshot_reset_state();

    /* Erase the stackshot data (this region is pre-populated with the NULL
     * character) */
    memset(stackshot_begin_loc, '\0', bytes_used);

    err = kcdata_memory_static_init(
        &kc_panic_data, (mach_vm_address_t)stackshot_begin_loc,
        KCDATA_BUFFER_BEGIN_STACKSHOT, (unsigned int)bytes_remaining,
        KCFLAG_USE_MEMCOPY);
    if (err != KERN_SUCCESS) {
      panic_info->mph_panic_flags |=
          MACOS_PANIC_HEADER_FLAG_STACKSHOT_FAILED_ERROR;
      panic_info->mph_other_log_offset =
          PE_get_offset_into_panic_region(debug_buf_ptr);
      kdb_printf("Failed to re-initialize kcdata buffer for kernel only "
                 "in-memory panic stackshot, skipping ...\n");
      return;
    }

    stackshot_flags = (STACKSHOT_SAVE_KEXT_LOADINFO | STACKSHOT_KCDATA_FORMAT |
                       STACKSHOT_FROM_PANIC | STACKSHOT_DISABLE_LATENCY_INFO |
                       STACKSHOT_NO_IO_STATS | STACKSHOT_THREAD_WAITINFO |
                       STACKSHOT_ACTIVE_KERNEL_THREADS_ONLY | STACKSHOT_GET_DQ);
#if DEVELOPMENT
    /*
     * Include the shared cache layout in panic stackshots on DEVELOPMENT
     * kernels so that we can symbolicate panic stackshots from corefiles.
     */
    stackshot_flags |= STACKSHOT_COLLECT_SHAREDCACHE_LAYOUT;
#endif
  ...
  if (err == KERN_SUCCESS) {
    ...
    /* On other systems this is not in the debug buffer itself, it's in a
     * separate buffer allocated at boot. */
    if (extended_debug_log_enabled) {
      panic_info->mph_stackshot_offset =
    ...
    if (!extended_debug_log_enabled &&
        (gIOPolledCoreFileMode == kIOPolledCoreFileModeStackshot)) {
    ...
    if (bytes_used > 0) {
      ...
      panic_info->mph_other_log_offset =
          PE_get_offset_into_panic_region(debug_buf_ptr);
      kdb_printf("\n** In Memory Panic Stackshot Incomplete ** Bytes Filled "
                 "%zu ** Err %d\n",
                 bytes_used, err);
    } else {
...
void SavePanicInfo(__unused const char *message, void *panic_data,
  ...
  if (panic_options & DEBUGGER_INTERNAL_OPTION_THREAD_BACKTRACE) {
    if (!mp_kdp_all_cpus_halted()) {
      ...
      debugger_msg = "Backtracing panicked thread because unable to access "
                     "specified thread\n";
    } else if (thread_to_trace->kernel_stack == 0) {
      debugger_msg = "Backtracing panicked thread because kernel_stack is NULL "
                     "for specified thread\n";
    } else if (kvtophys(STACK_IKS(thread_to_trace->kernel_stack) == 0ULL)) {
      debugger_msg = "Backtracing panicked thread because unable to access "
                     "kernel_stack for specified thread\n";
    } else {
      debugger_msg = "Backtracing specified thread\n";
      /* We construct a synthetic stack frame so we can include the current
       * instruction pointer */
      synthetic_stack_frame.prev =
          (cframe_t *)STACK_IKS(thread_to_trace->kernel_stack)->k_rbp;
      synthetic_stack_frame.caller =
          (uintptr_t)STACK_IKS(thread_to_trace->kernel_stack)->k_rip;
  ...
  if (panic_options & DEBUGGER_OPTION_INITPROC_PANIC) {
    ...
    panic_i386_backtrace(stackptr, ((panic_double_fault_cpu == cn) ? 80 : 48),
                         debugger_msg, FALSE, NULL);
  }
...
#if MACH_KDP
  /*
   * If this panic is due to a PTE corruption event, use the kdp cross-cpu
   * calling machinery to ask each CPU to dump their backtraces before
   * proceeding.  This mechanism was preferred to adding new synchronization
   * operations in NMIInterruptHandler and the normal panic flow; this mechanism
   * allows each CPU to add their backtraces after all other primary panic
   * output is complete while not adding additional complexity to the common
   * panic path.
   */
  if (NMI_panic_reason == PTE_CORRUPTION) {
    for (uint64_t cpu = 0; cpu < real_ncpus; cpu++) {
      ...
      (void)kdp_x86_xcpu_invoke(cpu, NMI_pte_corruption_callback, NULL, NULL,
                                NSEC_PER_SEC /* 1 second timeout */);
    }
  }
#else
#error NMI PTE Corruption panic flow requires KDP
  ...
  if (PE_get_offset_into_panic_region(debug_buf_ptr) <
      panic_info->mph_panic_log_offset) {
    kdb_printf(
        "Invalid panic log offset found (not properly initialized?): "
        "debug_buf_ptr : 0x%p, panic_info: 0x%p mph_panic_log_offset: 0x%x\n",
...
void paniclog_flush_internal(paniclog_flush_type_t variant) {
  ...
  /*
   * If we've detected that we're on a co-processor system, we flush the panic
   * log via the kPEPanicSync panic callbacks, otherwise we flush via nvram
   * (unless that has been disabled).
   */
  if (coprocessor_paniclog_flush) {
    ...
    if (extended_debug_log_enabled) {
      /*
       * debug_buf_size for the extended log does not include the length of the
       * header. There may be some extra data at the end of the 'basic' log that
       * wouldn't get flushed for the non-extended case (this is a concession we
       * make to not shrink the paniclog data for non-coprocessor systems that
       * only use the basic log).
       */
    ...
    /* Update the CRC */
    panic_info->mph_crc =
        crc32(0L, &panic_info->mph_version,
              (overall_buffer_size -
               offsetof(struct macos_panic_header, mph_version)));

    if (variant == kPaniclogFlushBase) {
      /* Flush the header and base panic log. */
      kprintf("Flushing base panic log\n");
      size_to_flush = ROUNDUP(
          (panic_info->mph_panic_log_offset + panic_info->mph_panic_log_len),
          PANIC_FLUSH_BOUNDARY);
      offset_to_flush = 0;
      PESavePanicInfoAction(panic_info, offset_to_flush, size_to_flush);
    } else if ((variant == kPaniclogFlushStackshot) ||
               (variant == kPaniclogFlushOtherLog)) {
      if (variant == kPaniclogFlushStackshot) {
        /*
         * We flush the stackshot before flushing the updated header because the
         * stackshot can take a while to flush. We want the paniclog header to
         * be as consistent as possible even if the stackshot isn't flushed
         * completely. Flush starting from the end of the panic log.
         */
        kprintf("Flushing panic log stackshot\n");
        offset_to_flush = ROUNDDOWN(
            (panic_info->mph_panic_log_offset + panic_info->mph_panic_log_len),
            PANIC_FLUSH_BOUNDARY);
        size_to_flush =
            ROUNDUP((panic_info->mph_stackshot_len +
                     (panic_info->mph_stackshot_offset - offset_to_flush)),
                    PANIC_FLUSH_BOUNDARY);
        PESavePanicInfoAction(panic_info, offset_to_flush, size_to_flush);
      }
      ...
      offset_to_flush = ROUNDDOWN(
          ...
          PANIC_FLUSH_BOUNDARY);
      size_to_flush =
          ROUNDUP((panic_info->mph_other_log_len +
                   (panic_info->mph_other_log_offset - offset_to_flush)),
                  PANIC_FLUSH_BOUNDARY);
      PESavePanicInfoAction(panic_info, offset_to_flush, size_to_flush);
      ...
      size_to_flush =
          ROUNDUP(panic_info->mph_panic_log_offset, PANIC_FLUSH_BOUNDARY);
      offset_to_flush = 0;
      PESavePanicInfoAction(panic_info, offset_to_flush, size_to_flush);
    }
     ...
     */
    bufpos =
        packA(debug_buf_base, (unsigned int)(debug_buf_ptr - debug_buf_base),
              debug_buf_size);
    /*
     ...
     * <rdar://problem/5915669> SnowLeopard10A67: AppleEFINVRAM should not
     * invoke any routines that use floating point (MMX in this case) when
     * saving panic logs to nvram/flash.
     */
    cr0 = get_cr0();
    clear_ts();
    ...
    pi_size =
        PESavePanicInfo((unsigned char *)debug_buf_base, (uint32_t)pi_size);
    set_cr0(cr0);
...
char *machine_boot_info(char *buf, __unused vm_size_t size) {
  *buf = '\0';
  return buf;
}
...
static int panic_print_macho_symbol_name(kernel_mach_header_t *mh,
  ...
  for (i = 0; i < mh->ncmds; i++) {
    if (cmd->cmd == LC_SEGMENT_KERNEL) {
      ...
      if (strncmp(SEG_TEXT, orig_sg->segname, sizeof(orig_sg->segname)) == 0) {
        ...
        orig_ts =
            orig_sg; /* pre-Lion i386 kexts have a single unnamed segment */
      }
  ...
  sym = (kernel_nlist_t *)(uintptr_t)(orig_le->vmaddr + orig_st->symoff -
                                      orig_le->fileoff);
  strings =
      (char *)(uintptr_t)(orig_le->vmaddr + orig_st->stroff - orig_le->fileoff);
  diff = search;
  ...
  if (bestsym != NULL) {
    if (diff != 0) {
      paniclog_append_noflush("%s : %s + 0x%lx", module_name, bestsym,
                              (unsigned long)diff);
    } else {
...
static void panic_display_hib_count(void) {
...
static void panic_display_times(void) {
  ...
  paniclog_append_noflush("  Sleep   : 0x%016llx 0x%016llx 0x%016llx\n",
                          gIOLastSleepAbsTime, gAcpiLastSleepTscBase,
                          gAcpiLastSleepNanoBase);
  paniclog_append_noflush("  Wake    : 0x%016llx 0x%016llx 0x%016llx\n",
                          gIOLastWakeAbsTime, gAcpiLastWakeTscBase,
...
static void panic_display_shutdown_status(void) {
  ...
  if (gIOPolledCoreFileMode == kIOPolledCoreFileModeNotInitialized) {
    paniclog_append_noflush("Panic diags file unavailable, panic occurred "
                            "prior to initialization\n");
  } else if (gIOPolledCoreFileMode != kIOPolledCoreFileModeDisabled) {
    /*
     * If we haven't marked the corefile as explicitly disabled, and we've made
     * it past initialization, then we know the current system was configured to
     * use disk based diagnostics at some point.
     */
    paniclog_append_noflush(
...
static void panic_display_system_configuration(boolean_t launchd_exit) {
  if (!launchd_exit) {
  ...
  for (i = 0; i < gLoadedKextSummaries->numSummaries; ++i) {
    ...
    if ((search >= summary->address) &&
      ...
      if (panic_print_macho_symbol_name(header, search, summary->name) == 0) {
        paniclog_append_noflush("%s + %llu", summary->name,
...
static void read_lbr_empty(void) {}

void (*read_lbr)(void) = read_lbr_empty;

static void capture_lbr_state(void) {
  ...
  if (lbr_stack_size > 0) {
    ...
    if (i386_filtered_lbr_state_to_mach_thread_state(thr_act, &thread_lbr_data,
                                                     false) == 0) {
...
static void copy_lbr_data_for_core(void) {
  unsigned int cpu;
  ...
  if (phys_carveout) {
    ...
    if (phys_carveout_size >= sizeof(last_branch_state_t) * max_ncpus) {
      for (cpu = 0; cpu < real_ncpus; cpu++) {
        void *buf =
            (void *)(phys_carveout + lbr_stack_size * sizeof(uint64_t) * cpu);
        memcpy(buf, lbrs[cpu].from, sizeof(uint64_t) * lbr_stack_size);
        memcpy((uint64_t *)buf + lbr_stack_size * sizeof(uint64_t),
               lbrs[cpu].to, sizeof(uint64_t) * lbr_stack_size);
      }
...
static void write_lbr_to_panic_log(void) {
  unsigned int cpu;
  int i;
...
void panic_print_symbol_name(vm_address_t search) {
  ...
  if (panic_print_macho_symbol_name(&_mh_execute_header, search,
                                    "mach_kernel") == 0) {
...
void panic_i386_backtrace(void *_frame, int nframes, const char *msg,
                          boolean_t regdump, x86_saved_state_t *regs) {
  ...
  if ((regdump == TRUE) && (regs != NULL)) {
    ...
    paniclog_append_noflush(
        ...
        ss64p->rax, ss64p->rbx, ss64p->rcx, ss64p->rdx, ss64p->isf.rsp,
        ss64p->rbp, ss64p->rsi, ss64p->rdi, ss64p->r8, ss64p->r9, ss64p->r10,
        ss64p->r11, ss64p->r12, ss64p->r13, ss64p->r14, ss64p->r15,
        ss64p->isf.rflags, ss64p->isf.rip, ss64p->isf.cs, ss64p->isf.ss);
    PC = ss64p->isf.rip;
  }

  // print current task info
  if (panic_get_thread_proc_task(cur_thread, &task, &proc)) {
    paniclog_append_noflush("Panicked task %p: %d threads: ", task,
                            task->thread_count);
    if (proc) {
  ...
  paniclog_append_noflush(
      "Backtrace (CPU %d), panicked thread: %p, "
#if PRINT_ARGS_FROM_STACK_FRAME
  ...
  for (frame_index = 0; frame_index < nframes; frame_index++) {
    ...
    if (!kvtophys(curframep) || !kvtophys(curframep + sizeof(cframe_t) - 1)) {
    ...
    if (kvtophys((vm_offset_t) & (frame->args[3]))) {
      paniclog_append_noflush("(0x%x 0x%x 0x%x 0x%x) ", frame->args[0],
                              frame->args[1], frame->args[2], frame->args[3]);
    }
#endif
...
static boolean_t debug_copyin(pmap_t p, uint64_t uaddr, void *dest,
                              size_t size) {
  size_t rem = size;
  char *kvaddr = dest;
...
void print_threads_registers(thread_t thread) {
  ...
  paniclog_append_noflush(
      ...
      "R12: 0x%016llx, R13: 0x%016llx, R14: 0x%016llx, R15: 0x%016llx\n"
      "RFL: 0x%016llx, RIP: 0x%016llx, CS:  0x%016llx, SS:  0x%016llx\n\n",
      savestate->ss_64.rax, savestate->ss_64.rbx, savestate->ss_64.rcx,
      savestate->ss_64.rdx, savestate->ss_64.isf.rsp, savestate->ss_64.rbp,
      savestate->ss_64.rsi, savestate->ss_64.rdi, savestate->ss_64.r8,
      savestate->ss_64.r9, savestate->ss_64.r10, savestate->ss_64.r11,
      savestate->ss_64.r12, savestate->ss_64.r13, savestate->ss_64.r14,
      savestate->ss_64.r15, savestate->ss_64.isf.rflags,
      savestate->ss_64.isf.rip, savestate->ss_64.isf.cs,
      savestate->ss_64.isf.ss);
}

void print_tasks_user_threads(task_t task) {
  ...
  for (j = 0, thread = (thread_t)queue_first(&task->threads);
       j < task->thread_count;
...
void print_curr_backtrace(void) { /*  Not implemented for i386 */ }

void print_thread_num_that_crashed(task_t task) {
...
void print_uuid_info(task_t task) {
  ...
  boolean_t have_pmap = have_map && (task->map->pmap != NULL) &&
                        (ml_validate_nofault((vm_offset_t)(task->map->pmap),
                                             sizeof(struct pmap)));
  ...
  if (have_pmap && task->active && task_pid > 0) {
    /* Read dyld_all_image_infos struct from task memory to get UUID array count
     * & location */
  ...
  if (task_pid > 0 && uuid_info_count > 0) {
    ...
    /* Copy in the UUID info array. It may be nonresident, in which case just
     * fix up nloadinfos to 0 */
    ...
    while (uuid_array_size) {
      ...
      if (have_pmap && !debug_copyin(task->map->pmap, uuid_info_addr,
                                     uuidbufptr, uuid_copy_size)) {
        paniclog_append_noflush(
            "Error!! Failed to copy UUID info for task %p pid %d\n", task,
            task_pid);
        uuid_image_count = 0;
        break;
      }
 ...
 */
static bool efi_compare_guids(EFI_GUID *guid1, EFI_GUID *guid2) {
 ...
 */
static uint64_t efi_get_cfgtbl_by_guid(EFI_GUID *guidp) {
  EFI_CONFIGURATION_TABLE_64 *cfg_table_entp, *cfgTable;
  boot_args *args = (boot_args *)PE_state.bootArgs;
  EFI_SYSTEM_TABLE_64 *estp;
  uint32_t i, hdr_cksum, cksum;

  estp = (EFI_SYSTEM_TABLE_64 *)efi_efiboot_virtual_to_physmap_virtual(
      args->efiSystemTable);
  ...
  /*
   * efiboot can (and will) change the address of ConfigurationTable (and each
   * table's VendorTable address) to a kernel-virtual address.  Reverse that to
   * get the physical address, which we then use to get a physmap-based virtual
   * address.
   */
  cfgTable =
      (EFI_CONFIGURATION_TABLE_64 *)efi_efiboot_virtual_to_physmap_virtual(
          estp->ConfigurationTable);
 ...
 */
uint64_t efi_get_rsdp_physaddr(void) {
  uint64_t rsdp_addr;
