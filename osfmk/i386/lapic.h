...
#define CPU_NUMBER(r) movl % gs : CPU_NUMBER_GS, r
...
#define LAPIC_MMIO(reg)                                                        \
  ((volatile uint32_t *)(LAPIC_MMIO_VBASE + LAPIC_MMIO_OFFSET(reg)))
...
#define LAPIC_PMC_SWI_VECTOR                                                   \
  (LAPIC_DEFAULT_INTERRUPT_BASE + LAPIC_PMC_SW_INTERRUPT)
...
#define LAPIC_VECTOR(src) (lapic_interrupt_base + LAPIC_##src##_INTERRUPT)
...
extern void lapic_config_timer(boolean_t interrupt, lapic_timer_mode_t mode,
                               lapic_timer_divide_t divisor);

extern void lapic_set_timer_fast(lapic_timer_count_t initial_count);

extern void lapic_set_timer(boolean_t interrupt, lapic_timer_mode_t mode,
                            lapic_timer_divide_t divisor,
                            lapic_timer_count_t initial_count);

extern void lapic_get_timer(lapic_timer_mode_t *mode,
                            lapic_timer_divide_t *divisor,
                            lapic_timer_count_t *initial_count,
...
static inline void lapic_timer_swi(void) {
  __asm__ __volatile__(
      "int %0" ::"i"(LAPIC_DEFAULT_INTERRUPT_BASE + LAPIC_TIMER_INTERRUPT)
      : "memory");
}
