 ...
 *
 *		1. Add a definition for it's address in
 * osfmk/i386/cpu_capabilities.h, being careful to reserve room for future
 * expansion.
 *
 *		2. Write one or more versions of the routine, each with it's own
 *		   commpage_descriptor.  The tricky part is getting the
 * "special", "musthave", and "canthave" fields right, so that exactly one
 *		   version of the routine is selected for every machine.
 *		   The source files should be in osfmk/i386/commpage/.
 *
 *		3. Add a ptr to your new commpage_descriptor(s) in the
 * "routines" array in osfmk/i386/commpage/commpage_asm.s.  There are two
...
static char
    *commPagePtr; // virtual addr in kernel map of commpage we are working on
static commpage_address_t
    commPageBaseOffset; // subtract from 32-bit runtime address to get offset in
                        // virtual commpage in kernel map
...
static void *commpage_allocate(
  ...
  kr = mach_vm_map_kernel(
      kernel_map, &kernel_addr, area_used, 0,
      VM_MAP_KERNEL_FLAGS_ANYWHERE(.vm_tag = VM_KERN_MEMORY_OSFMK), NULL, 0,
      FALSE, VM_PROT_ALL, VM_PROT_ALL, VM_INHERIT_NONE);
  if (kr != KERN_SUCCESS) {
    panic("cannot allocate commpage %d", kr);
  }

  if ((kr = vm_map_wire_kernel(kernel_map, kernel_addr, kernel_addr + area_used,
                               VM_PROT_DEFAULT, VM_KERN_MEMORY_OSFMK, FALSE))) {
    panic("cannot wire commpage: %d", kr);
  }

  /*
   * Now that the object is created and wired into the kernel map, mark it so
   * that no delay copy-on-write will ever be performed on it as a result of
   * mapping it into user-space. If such a delayed copy ever occurred, we could
   * remove the kernel's wired mapping - and that would be a real disaster.
   *
   * JMM - What we really need is a way to create it like this in the first
   * place.
   */
  if (!(kr = vm_map_lookup_entry(
                 kernel_map,
                 vm_map_trunc_page(kernel_addr, VM_MAP_PAGE_MASK(kernel_map)),
                 &entry) ||
             entry->is_sub_map)) {
  ...
  if ((kr =
           mach_make_memory_entry(kernel_map,  // target map
                                  &size,       // size
  ...
  kr = mach_vm_map_kernel(submap,    // target map (shared submap)
...
static void *commpage_addr_of(commpage_address_t addr_at_runtime) {
  return (void *)((uintptr_t)commPagePtr +
                  (addr_at_runtime - commPageBaseOffset));
}

/*
 * Calculate address of data within 32- and 64-bit commpages (not to be used
 * with commpage text).
 */
static void *commpage_specific_addr_of(char *commPageBase,
   ...
   */
  return (void *)((uintptr_t)commPageBase +
                  (addr_at_runtime - _COMM_PAGE32_BASE_ADDRESS));
}
...
#define setif(_bits, _bit, _condition)                                         \
  ...
  setif(bits, kHasAES, cpuid_features() & CPUID_FEATURE_AES);
  setif(bits, kHasF16C, cpuid_features() & CPUID_FEATURE_F16C);
  setif(bits, kHasRDRAND, cpuid_features() & CPUID_FEATURE_RDRAND);
  setif(bits, kHasFMA, cpuid_features() & CPUID_FEATURE_FMA);

  setif(bits, kHasBMI1, cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_BMI1);
  setif(bits, kHasBMI2, cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_BMI2);
  /* Do not advertise RTM and HLE if the TSX FORCE ABORT WA is required */
  if (cpuid_wa_required(CPU_INTEL_TSXFA) & CWA_OFF) {
    setif(bits, kHasRTM, cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_RTM);
    setif(bits, kHasHLE, cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_HLE);
  }
  setif(bits, kHasAVX2_0, cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX2);
  setif(bits, kHasRDSEED, cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_RDSEED);
  setif(bits, kHasADX, cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_ADX);
  ...
  if (ml_fpu_avx512_enabled()) {
    setif(bits, kHasAVX512F,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512F);
    setif(bits, kHasAVX512CD,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512CD);
    setif(bits, kHasAVX512DQ,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512DQ);
    setif(bits, kHasAVX512BW,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512BW);
    setif(bits, kHasAVX512VL,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512VL);
    setif(bits, kHasAVX512IFMA,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512IFMA);
    setif(bits, kHasAVX512VBMI,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512VBMI);
    setif(bits, kHasVAES, cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_VAES);
    setif(bits, kHasVPCLMULQDQ,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_VPCLMULQDQ);
    setif(bits, kHasAVX512VNNI,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512VNNI);
    setif(bits, kHasAVX512BITALG,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512BITALG);
    setif(bits, kHasAVX512VPOPCNTDQ,
          cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_AVX512VPCDQ);
  }
 ...
 */
static void commpage_mach_approximate_time_init(void) {
...
static void commpage_mach_continuous_time_init(void) {
  commpage_update_mach_continuous_time(0);
}

static void commpage_boottime_init(void) {
  clock_sec_t secs;
...
uint64_t _get_cpu_capabilities(void) { return _cpu_capabilities; }

/* Copy data into commpage. */

static void commpage_stuff(commpage_address_t address, const void *source,
                           int length) {
 ...
 */
static void commpage_update(commpage_address_t address, const void *source,
                            int length) {
...
void commpage_post_ucode_update(void) {
  commpage_init_cpu_capabilities();
  commpage_update(_COMM_PAGE_CPU_CAPABILITIES64, &_cpu_capabilities,
                  sizeof(_cpu_capabilities));
  commpage_update(_COMM_PAGE_CPU_CAPABILITIES, &_cpu_capabilities,
                  sizeof(uint32_t));
}

/* Copy a routine into comm page if it matches running machine.
 */
static void commpage_stuff_routine(commpage_descriptor *rd) {
...
static void commpage_populate_one(
    ...
    new_commpage_timeofday_data_t *
        *gtod_time_data,   // &gtod_time_data32 or &gtod_time_data64
  ...
  *gtod_time_data = commpage_addr_of(_COMM_PAGE_NEWTIMEOFDAY_DATA);
  ...
  commpage_stuff(_COMM_PAGE_SIGNATURE, signature,
                 (int)MIN(_COMM_PAGE_SIGNATURELEN, strlen(signature)));
  commpage_stuff(_COMM_PAGE_CPU_CAPABILITIES64, &_cpu_capabilities,
                 sizeof(_cpu_capabilities));
  ...
  if (next > _COMM_PAGE_END) {
    panic("commpage overflow: next = 0x%08x, commPagePtr = 0x%p", next,
          commPagePtr);
  }
}
...
void commpage_populate(void) {
  ...
  commpage_populate_one(commpage32_map, &commPagePtr32, _COMM_PAGE32_AREA_USED,
                        _COMM_PAGE32_BASE_ADDRESS, &time_data32,
                        &gtod_time_data32, _COMM_PAGE32_SIGNATURE_STRING,
                        VM_PROT_READ);
#ifndef __LP64__
  ...
  if (_cpu_capabilities & k64Bit) {
    commpage_populate_one(
        commpage64_map, &commPagePtr64, _COMM_PAGE64_AREA_USED,
        _COMM_PAGE32_START_ADDRESS, /* commpage address are relative to 32-bit
                                       commpage placement */
        &time_data64, &gtod_time_data64, _COMM_PAGE64_SIGNATURE_STRING,
        VM_PROT_READ);
#ifndef __LP64__
  ...
  uint64_t asb_rand_vals[] = {0x93e78adcded4d3d5, 0xd16c5b76ad99bccf,
                              0x67dfbbd12c4a594e, 0x7365636e6f6f544f,
                              0x239a974c9811e04b, 0xbf60e7fa45741446,
                              0x8acf5210b466b05,  0x67dfbbd12c4a594e};
  ...
  commpage_update(_COMM_PAGE_ASB_TARGET_KERN_VALUE, &asb_kvalue,
                  sizeof(asb_kvalue));
  ...
  commpage_update(_COMM_PAGE_ASB_TARGET_KERN_ADDRESS, &asb_kaddr,
                  sizeof(asb_kaddr));
...
void commpage_text_populate(void) {
  ...
  next = 0;
  commPagePtr = (char *)commpage_allocate(commpage_text32_map,
                                          (vm_size_t)_COMM_PAGE_TEXT_AREA_USED,
  ...
  if (_cpu_capabilities & k64Bit) {
    ...
    commPagePtr = (char *)commpage_allocate(
        commpage_text64_map, (vm_size_t)_COMM_PAGE_TEXT_AREA_USED,
        VM_PROT_READ | VM_PROT_EXECUTE);
  ...
  if (next > _COMM_PAGE_TEXT_END) {
    panic("commpage text overflow: next=0x%08x, commPagePtr=%p", next,
          commPagePtr);
  }
...
void commpage_set_nanotime(uint64_t tsc_base, uint64_t ns_base, uint32_t scale,
                           uint32_t shift) {
...
/* Update commpage gettimeofday() information.  As with nanotime(), we
 * interleave updates to the 32- and 64-bit commpage, in order to keep time more
...
/* Update _COMM_PAGE_MEMORY_PRESSURE.  Called periodically from vm's
 * compute_memory_pressure()  */

void commpage_set_memory_pressure(unsigned int pressure) {
  char *cp;
  uint32_t *ip;
 ...
 */
void commpage_update_dof(boolean_t enabled) {
#if CONFIG_DTRACE
  char *cp;
 ...
 */
void commpage_update_dyld_flags(uint64_t value) {
  char *cp;
...
void commpage_update_mach_approximate_time(uint64_t abstime) {
#ifdef CONFIG_MACH_APPROXIMATE_TIME
  uint64_t saved_data;
  char *cp;
  ...
  if (cp) {
    ...
    saved_data = atomic_load_explicit((_Atomic uint64_t *)(uintptr_t)cp,
...
void commpage_update_boottime(uint64_t boottime) {
  char *cp;
  cp = commPagePtr32;
  if (cp) {
...
uint32_t commpage_is_in_pfz32(uint32_t addr32) {
...
uint32_t commpage_is_in_pfz64(addr64_t addr64) {
