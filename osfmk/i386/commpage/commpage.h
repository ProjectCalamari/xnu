 ...
 */
#define COMMPAGE_JMP(target, from, start) jmp L##start - from + target
...
/* This is the data starting at _COMM_PAGE_TIME_DATA_START, ie for nanotime()
 * and gettimeofday() */
...
extern void commpage_set_timestamp(uint64_t abstime, uint64_t sec,
                                   uint64_t frac, uint64_t scale,
                                   uint64_t tick_per_sec);
#define commpage_disable_timestamp() commpage_set_timestamp(0, 0, 0, 0, 0);
extern void commpage_set_nanotime(uint64_t tsc_base, uint64_t ns_base,
