...
#define VERIFY_SAVEAREA_ALIGNED(p, a)                                          \
  assertf(!(((uintptr_t)(p)) & ((a) - 1)),                                     \
          "FP save area component @ 0x%lx not 8-byte aligned",                 \
          ((uintptr_t)(p)))

/* Forward */

extern void fpinit(void);
extern void fp_save(thread_t thr_act);
extern void fp_load(thread_t thr_act);
...
#define fninit() __asm__ volatile("fninit")
...
#define fnclex() __asm__ volatile("fnclex")

#define fnsave(state) __asm__ volatile("fnsave %0" : "=m"(*state))

#define frstor(state) __asm__ volatile("frstor %0" : : "m"(state))

#define fwait() __asm__("fwait");
...
ifps_zone[] = {[FP] = NULL, [AVX] = NULL, [AVX512] = NULL};
...
static uint32_t fp_state_size(xstate_t state) {
  switch (state) {
  case FP:
...
static struct fp_avx512 fp_avx512_get(struct x86_avx512_thread_state *iavx) {
  switch (fp_avx512_offset) {
  case AVX512_OFFSET:
...
static const char *const xstate_name[] = {
    [UNDEFINED] = "UNDEFINED", [FP] = "FP", [AVX] = "AVX", [AVX512] = "AVX512"};
...
static const uint32_t xstate_xmask[] = {
    [FP] = FP_XMASK, [AVX] = AVX_XMASK, [AVX512] = AVX512_XMASK};
...
static void DBG_AVX512_STATE(struct x86_avx512_thread_state *sp) {
...
#else
#define DBG(x...)
static void DBG_AVX512_STATE(__unused struct x86_avx512_thread_state *sp) {}
#endif /* DEBUG_AVX512 */

#if DEBUG
static inline unsigned short fnstsw(void) {
...
static void configure_mxcsr_capability_mask(x86_ext_thread_state_t *fps) {
  ...
  if (fpu_ZMM_capable) {
    xsave64((struct x86_fx_thread_state *)&default_avx512_state,
            xstate_xmask[AVX512]);
  }
  if (fpu_YMM_capable) {
    xsave64((struct x86_fx_thread_state *)&default_avx_state,
            xstate_xmask[AVX]);
  } else {
 ...
 * Don't load state.
 */
static void *fp_state_alloc(xstate_t xs) {
  assert(ifps_zone[xs] != NULL);
  return zalloc_flags(ifps_zone[xs], Z_WAITOK | Z_ZERO);
}

static inline void fp_state_free(void *ifps, xstate_t xs) {
  assert(ifps_zone[xs] != NULL);
  zfree(ifps_zone[xs], ifps);
}

void clear_fpu(void) { set_ts(); }

static boolean_t fpu_allzeroes(uint64_t *__attribute((aligned(8))) ptr,
                               uint32_t size) {
  VERIFY_SAVEAREA_ALIGNED(ptr, sizeof(uint64_t));
  assertf((size & (sizeof(uint64_t) - 1)) == 0,
          "FP save area component not a multiple of 8 bytes");
...
void fpu_module_init(void) {
  if (!IS_VALID_XSTATE(fpu_default)) {
    panic("fpu_module_init: invalid extended state %u", fpu_default);
  }
   ...
   */
  ifps_zone[fpu_default] =
      zone_create("x86 fpsave state", fp_state_size(fpu_default),
                  ZC_ALIGNMENT_REQUIRED | ZC_ZFREE_CLEARMEM);
  ...
  if (fpu_ZMM_capable) {
    ifps_zone[AVX512] =
        zone_create("x86 avx512 save state", fp_state_size(AVX512),
                    ZC_ALIGNMENT_REQUIRED | ZC_ZFREE_CLEARMEM);
  }
  ...
  PE_parse_boot_argn("fpsimd_fault_popc", &fpsimd_fault_popc,
 ...
 * Always save old thread`s FPU context but don't load new .. allow that to
 * fault-in. Switch to the new task's xstate.
 */

void fpu_switch_context(thread_t old, thread_t new) {
  ...
  if (ifps != 0 && (ifps->fp_valid == FALSE)) {
    ...
    boolean_t is64 =
        (thread_is_64bit_addr(old) && is_saved_state64(old->machine.iss));

    fpu_store_registers(ifps, is64);
    ifps->fp_valid = TRUE;

    if (fpu_ZMM_capable && (cdp->cpu_xstate == AVX512)) {
      xrstor64((struct x86_fx_thread_state *)&default_avx512_state,
               xstate_xmask[AVX512]);
    } else if (fpu_YMM_capable) {
      xrstor64((struct x86_fx_thread_state *)&default_avx_state,
               xstate_xmask[AVX]);
    } else {
  ...
  assertf(fpu_YMM_capable ? (xgetbv(XCR0) == xstate_xmask[cdp->cpu_xstate])
                          : TRUE,
          "XCR0 mismatch: 0x%llx 0x%x 0x%x", xgetbv(XCR0), cdp->cpu_xstate,
          xstate_xmask[cdp->cpu_xstate]);
  if (new_xstate != (xstate_t)cdp->cpu_xstate) {
    DBG("fpu_switch_context(%p,%p) new xstate: %s\n", old, new,
        xstate_name[new_xstate]);
 ...
 */
void fpu_free(thread_t thread, void *fps) {
 ...
 *
 * While translating between XNU FP state structures and the CPU-native XSAVE
 * area, if we detect state components that are all zeroes, we clear the
 * corresponding xstate_bv bit in the XSAVE area, because that allows the
 * corresponding state to be initialized to a "clean" state.  That's most
 * important when clearing the YMM bit, since an initialized "upper clean" state
 * results in a massive performance improvement due to elimination of false
 * dependencies between the XMMs and the upper bits of the YMMs.
 */
kern_return_t fpu_set_fxstate(thread_t thr_act, thread_state_t tstate,
                              thread_flavor_t f) {
  struct x86_fx_thread_state *ifps;
  ...
  if ((f == x86_AVX_STATE32 || f == x86_AVX_STATE64) && fpu_capability < AVX) {
    return KERN_FAILURE;
  }
  ...
  Retry:
...
#if DEBUG || DEVELOPMENT
    if ((fresh_state == FALSE) && (old_valid == FALSE) &&
    ...
    case FP:
      ifps->fp_save_layout =
          thread_is_64bit_addr(thr_act) ? FXSAVE64 : FXSAVE32;
      break;
    case AVX: {
      struct x86_avx_thread_state *iavx = (void *)ifps;
      x86_avx_state64_t *xs = (x86_avx_state64_t *)state;

      iavx->fp.fp_save_layout =
          thread_is_64bit_addr(thr_act) ? XSAVE64 : XSAVE32;
      ...
      /*
       * See the block comment at the top of the function for a description of
       * why we're clearing xstate_bv bits.
       */
      if (f == x86_AVX_STATE32) {
        __nochk_bcopy(&xs->fpu_ymmh0, iavx->x_YMM_Hi128,
                      8 * sizeof(_STRUCT_XMM_REG));
        if (fpu_allzeroes((uint64_t *)(void *)iavx->x_YMM_Hi128,
                          8 * sizeof(_STRUCT_XMM_REG)) == TRUE) {
        ...
        if (fpu_allzeroes((uint64_t *)(void *)iavx->x_YMM_Hi128,
                          16 * sizeof(_STRUCT_XMM_REG)) == TRUE) {
          iavx->_xh.xstate_bv &= ~XFEM_YMM;
        }
      } else {
    ...
    case AVX512: {
      ...
      case x86_AVX512_STATE32:
        __nochk_bcopy(&xs.s32->fpu_k0, p.x_Opmask,
                      8 * sizeof(_STRUCT_OPMASK_REG));
        __nochk_bcopy(&xs.s32->fpu_zmmh0, p.x_ZMM_Hi256,
                      8 * sizeof(_STRUCT_YMM_REG));

        if (fpu_allzeroes((uint64_t *)(void *)p.x_Opmask,
                          8 * sizeof(_STRUCT_OPMASK_REG)) == TRUE) {
          iavx->_xh.xstate_bv &= ~XFEM_OPMASK;
        }

        if (fpu_allzeroes((uint64_t *)(void *)p.x_ZMM_Hi256,
                          8 * sizeof(_STRUCT_YMM_REG)) == TRUE) {
          iavx->_xh.xstate_bv &= ~(XFEM_ZMM_HI256 | XFEM_HI16_ZMM);
        }
        __nochk_bcopy(&xs.s32->fpu_ymmh0, iavx->x_YMM_Hi128,
                      8 * sizeof(_STRUCT_XMM_REG));
        if (fpu_allzeroes((uint64_t *)(void *)iavx->x_YMM_Hi128,
                          8 * sizeof(_STRUCT_XMM_REG)) == TRUE) {
          iavx->_xh.xstate_bv &= ~XFEM_YMM;
      ...
      case x86_AVX512_STATE64:
        __nochk_bcopy(&xs.s64->fpu_k0, p.x_Opmask,
                      8 * sizeof(_STRUCT_OPMASK_REG));
        __nochk_bcopy(&xs.s64->fpu_zmm16, p.x_Hi16_ZMM,
                      16 * sizeof(_STRUCT_ZMM_REG));
        __nochk_bcopy(&xs.s64->fpu_zmmh0, p.x_ZMM_Hi256,
                      16 * sizeof(_STRUCT_YMM_REG));
        /*
         * Note that it is valid to have XFEM_ZMM_OPMASK set but XFEM_YMM
         * cleared.  In that case, the upper bits of the YMMs would be cleared
         * and would result in a clean-upper state, allowing SSE instruction to
        ...
        if (fpu_allzeroes((uint64_t *)(void *)p.x_Hi16_ZMM,
                          16 * sizeof(_STRUCT_ZMM_REG)) == TRUE &&
            fpu_allzeroes((uint64_t *)(void *)p.x_ZMM_Hi256,
                          16 * sizeof(_STRUCT_YMM_REG)) == TRUE) {
          iavx->_xh.xstate_bv &= ~(XFEM_ZMM_HI256 | XFEM_HI16_ZMM);
        }

        __nochk_bcopy(&xs.s64->fpu_ymmh0, iavx->x_YMM_Hi128,
                      16 * sizeof(_STRUCT_XMM_REG));
        if (fpu_allzeroes((uint64_t *)(void *)iavx->x_YMM_Hi128,
                          16 * sizeof(_STRUCT_XMM_REG)) == TRUE) {
          iavx->_xh.xstate_bv &= ~XFEM_YMM;
 ...
 */
kern_return_t fpu_get_fxstate(thread_t thr_act, thread_state_t tstate,
                              thread_flavor_t f) {
  struct x86_fx_thread_state *ifps;
  x86_float_state64_t *state;
  ...
  if (ifps->fp_valid) {
    ...
    case AVX: {
      ...
      x86_avx_state64_t *xs = (x86_avx_state64_t *)state;
      if (f == x86_AVX_STATE32) {
        __nochk_bcopy(iavx->x_YMM_Hi128, &xs->fpu_ymmh0,
                      8 * sizeof(_STRUCT_XMM_REG));
      } else if (f == x86_AVX_STATE64) {
        __nochk_bcopy(iavx->x_YMM_Hi128, &xs->fpu_ymmh0,
                      16 * sizeof(_STRUCT_XMM_REG));
    ...
    case AVX512: {
      ...
      switch (f) {
      case x86_AVX512_STATE32:
        __nochk_bcopy(p.x_Opmask, &xs.s32->fpu_k0,
                      8 * sizeof(_STRUCT_OPMASK_REG));
        __nochk_bcopy(p.x_ZMM_Hi256, &xs.s32->fpu_zmmh0,
                      8 * sizeof(_STRUCT_YMM_REG));
        __nochk_bcopy(iavx->x_YMM_Hi128, &xs.s32->fpu_ymmh0,
                      8 * sizeof(_STRUCT_XMM_REG));
        DBG_AVX512_STATE(iavx);
        break;
      case x86_AVX_STATE32:
        ...
        break;
      case x86_AVX512_STATE64:
        __nochk_bcopy(p.x_Opmask, &xs.s64->fpu_k0,
                      8 * sizeof(_STRUCT_OPMASK_REG));
        __nochk_bcopy(p.x_Hi16_ZMM, &xs.s64->fpu_zmm16,
                      16 * sizeof(_STRUCT_ZMM_REG));
        __nochk_bcopy(p.x_ZMM_Hi256, &xs.s64->fpu_zmmh0,
                      16 * sizeof(_STRUCT_YMM_REG));
        __nochk_bcopy(iavx->x_YMM_Hi128, &xs.s64->fpu_ymmh0,
                      16 * sizeof(_STRUCT_XMM_REG));
        DBG_AVX512_STATE(iavx);
        break;
      case x86_AVX_STATE64:
...
void fpu_dup_fxstate(thread_t parent, thread_t child) {
  ...
  if (ppcb->ifps != NULL) {
    ...
    if (ifps->fp_valid) {
      ...
      __nochk_bcopy((char *)(ppcb->ifps), (char *)(child->machine.ifps),
...
void fpexterrflt(void) {
   ...
   */
  fp_save(thr_act);
  /* Set TS to ensure we catch attempts to use the FPU before returning from
   * trap handling */
  set_ts();
...
void fp_load(thread_t thr_act) {
  ...
  if (ifps->fp_valid != FALSE && ifps->fp_valid != TRUE) {
    panic("fp_load() invalid fp_valid: %u, fp_save_layout: %u", ifps->fp_valid,
          ifps->fp_save_layout);
  }
#endif
 ...
 */
static void fpu_savearea_promote_avx512(thread_t thread) {
 ...
 */
boolean_t fpu_thread_promote_avx512(thread_t thread) {
...
void fp_setvalid(boolean_t value) {
  thread_t thr_act = current_thread();
...
boolean_t ml_fpu_avx_enabled(void) { return fpu_capability >= AVX; }

boolean_t ml_fpu_avx512_enabled(void) { return fpu_ZMM_capable; }

static xstate_t thread_xstate(thread_t thread) {
...
xstate_t current_xstate(void) { return thread_xstate(current_thread()); }

/*
 * Called when exec'ing between bitnesses.
 * If valid FPU state exists, adjust the layout.
 */
void fpu_switch_addrmode(thread_t thread, boolean_t is_64bit) {
...
bool thread_fpsimd_hash_enabled(void) {
  return fpsimd_fault_popc ? true : false;
}

uint32_t __attribute__((noinline)) thread_fpsimd_hash(thread_t ft) {
  uint32_t prv = 0;
  boolean_t istate = ml_set_interrupts_enabled(FALSE);
