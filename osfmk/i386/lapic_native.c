...
static void map_local_apic(void) {
  ...
  if (lapic_vbase == 0) {
     ...
     */
    kr = pmap_enter(pmap_kernel(), lapic_vbase, (ppnum_t)i386_btop(lapic_pbase),
                    VM_PROT_READ | VM_PROT_WRITE, VM_PROT_NONE, VM_WIMG_IO,
...
static void legacy_init(void) {
  ...
  if ((lo & MSR_IA32_APIC_BASE_EXTENDED) != 0) {
    ...
    wrmsr64(MSR_IA32_APIC_BASE,
            ((uint64_t)hi) << 32 | lo | MSR_IA32_APIC_BASE_ENABLE);
  }
  /*
...
static uint32_t legacy_read(lapic_register_t reg) { return *LAPIC_MMIO(reg); }

static void legacy_write(lapic_register_t reg, uint32_t value) {
  *LAPIC_MMIO(reg) = value;
}

static uint64_t legacy_read_icr(void) {
  return (((uint64_t)*LAPIC_MMIO(ICRD)) << 32) | ((uint64_t)*LAPIC_MMIO(ICR));
}

static void legacy_write_icr(uint32_t dst, uint32_t cmd) {
  *LAPIC_MMIO(ICRD) = dst << LAPIC_ICRD_DEST_SHIFT;
  *LAPIC_MMIO(ICR) = cmd;
}

static lapic_ops_table_t legacy_ops = {legacy_init, legacy_read, legacy_write,
...
static uint32_t x2apic_read(lapic_register_t reg) {
  uint32_t lo;
  uint32_t hi;
...
static void x2apic_write(lapic_register_t reg, uint32_t value) {
  if (LVT_TIMER == reg) {
...
static uint64_t x2apic_read_icr(void) { return rdmsr64(LAPIC_MSR(ICR)); }

static void x2apic_write_icr(uint32_t dst, uint32_t cmd) {
  wrmsr(LAPIC_MSR(ICR), cmd, dst);
}

static lapic_ops_table_t x2apic_ops = {x2apic_init, x2apic_read, x2apic_write,
                                       x2apic_read_icr, x2apic_write_icr};

/*
 * Used by APs to determine their APIC IDs; assumes master CPU has initialized
 * the local APIC interfaces.
 */
uint32_t lapic_safe_apicid(void) {
  uint32_t lo;
  uint32_t hi;
...
static void lapic_reinit(bool for_wake) {
  uint32_t lo;
  uint32_t hi;
...
void lapic_init(void) {
  ...
  kprintf("MSR_IA32_APIC_BASE 0x%llx %s %s mode %s\n", lapic_pbase,
          ...
          is_x2apic ? "extended" : "legacy", is_boot_processor ? "BSP" : "AP");
...
static int lapic_esr_read(void) {
  /* write-read register */
  LAPIC_WRITE(ERROR_STATUS, 0);
  return LAPIC_READ(ERROR_STATUS);
}

static void lapic_esr_clear(void) {
  LAPIC_WRITE(ERROR_STATUS, 0);
  LAPIC_WRITE(ERROR_STATUS, 0);
}

static const char *DM_str[8] = {
    "Fixed", "Lowest Priority", "Invalid", "Invalid",
    "NMI",   "Reset",           "Invalid", "ExtINT"};

static const char *TMR_str[] = {"OneShot", "Periodic", "TSC-Deadline",
                                "Illegal"};
...
#define BOOL(a) ((a) ? ' ' : '!')
#define VEC(lvt) LAPIC_READ(lvt) & LAPIC_LVT_VECTOR_MASK
#define DS(lvt)                                                                \
  (LAPIC_READ(lvt) & LAPIC_LVT_DS_PENDING) ? " SendPending" : "Idle"
#define DM(lvt)                                                                \
  DM_str[(LAPIC_READ(lvt) >> LAPIC_LVT_DM_SHIFT) & LAPIC_LVT_DM_MASK]
#define MASK(lvt) BOOL(LAPIC_READ(lvt) & LAPIC_LVT_MASKED)
#define TM(lvt) (LAPIC_READ(lvt) & LAPIC_LVT_TM_LEVEL) ? "Level" : "Edge"
#define IP(lvt) (LAPIC_READ(lvt) & LAPIC_LVT_IP_PLRITY_LOW) ? "Low " : "High"

  kprintf("LAPIC %d at %p version 0x%x\n", lapic_safe_apicid(),
          (void *)lapic_vbase, LAPIC_READ(VERSION) & LAPIC_VERSION_MASK);
  kprintf("Priorities: Task 0x%x  Arbitration 0x%x  Processor 0x%x\n",
          LAPIC_READ(TPR) & LAPIC_TPR_MASK, LAPIC_READ(APR) & LAPIC_APR_MASK,
  ...
  if (mca_is_cmci_present()) {
    kprintf("LVT_CMCI:    Vector 0x%02x [%s] %s %cmasked\n", VEC(LVT_CMCI),
            DM(LVT_CMCI), DS(LVT_CMCI), MASK(LVT_CMCI));
  }
#endif
  kprintf("LVT_TIMER:   Vector 0x%02x %s %cmasked %s\n", VEC(LVT_TIMER),
          DS(LVT_TIMER), MASK(LVT_TIMER),
  ...
  kprintf("LVT_PERFCNT: Vector 0x%02x [%s] %s %cmasked\n", VEC(LVT_PERFCNT),
          DM(LVT_PERFCNT), DS(LVT_PERFCNT), MASK(LVT_PERFCNT));
  kprintf("LVT_THERMAL: Vector 0x%02x [%s] %s %cmasked\n", VEC(LVT_THERMAL),
          DM(LVT_THERMAL), DS(LVT_THERMAL), MASK(LVT_THERMAL));
  kprintf("LVT_LINT0:   Vector 0x%02x [%s][%s][%s] %s %cmasked\n",
          VEC(LVT_LINT0), DM(LVT_LINT0), TM(LVT_LINT0), IP(LVT_LINT0),
          DS(LVT_LINT0), MASK(LVT_LINT0));
  kprintf("LVT_LINT1:   Vector 0x%02x [%s][%s][%s] %s %cmasked\n",
          VEC(LVT_LINT1), DM(LVT_LINT1), TM(LVT_LINT1), IP(LVT_LINT1),
          DS(LVT_LINT1), MASK(LVT_LINT1));
  kprintf("LVT_ERROR:   Vector 0x%02x %s %cmasked\n", VEC(LVT_ERROR),
          DS(LVT_ERROR), MASK(LVT_ERROR));
  kprintf("ESR: %08x \n", lapic_esr_read());
  kprintf("       ");
  for (i = 0xf; i >= 0; i--) {
...
void lapic_shutdown(bool for_sleep) {
  uint32_t lo;
  uint32_t hi;
  uint32_t value;
...
bool ml_cpu_can_exit(int cpu_id) { return cpu_id > lapic_max_interrupt_cpunum; }

void lapic_configure(bool for_wake) {
  int value;

  if (lapic_error_time_threshold == 0 && cpu_number() == 0) {
    nanoseconds_to_absolutetime(NSEC_PER_SEC >> 2, &lapic_error_time_threshold);
    if (!PE_parse_boot_argn("lapic_dont_panic", &lapic_dont_panic,
  ...
  if (cpu_number() == 0) {
    if (!PE_parse_boot_argn("intcpumax", &lapic_max_interrupt_cpunum,
                            sizeof(lapic_max_interrupt_cpunum))) {
      lapic_max_interrupt_cpunum =
          ((cpuid_features() & CPUID_FEATURE_HTT) ? 1 : 0);
    }
  }

  /*
   * Reinitialize the APIC (handles the case where we're configured to use the
   * X2APIC but firmware configured the Legacy APIC):
   */
...
void lapic_set_timer(boolean_t interrupt_unmasked, lapic_timer_mode_t mode,
...
void lapic_config_timer(boolean_t interrupt_unmasked, lapic_timer_mode_t mode,
                        lapic_timer_divide_t divisor) {
  uint32_t timer_vector;
  ...
  timer_vector = LAPIC_READ(LVT_TIMER);
  timer_vector &=
      ~(LAPIC_LVT_MASKED | LAPIC_LVT_PERIODIC | LAPIC_LVT_TSC_DEADLINE);
  timer_vector |= interrupt_unmasked ? 0 : LAPIC_LVT_MASKED;
  timer_vector |= (mode == periodic) ? LAPIC_LVT_PERIODIC : 0;
  LAPIC_WRITE(LVT_TIMER, timer_vector);
  LAPIC_WRITE(TIMER_DIVIDE_CONFIG, divisor);
 ...
 */
void lapic_config_tsc_deadline_timer(void) {
  uint32_t timer_vector;
...
void lapic_set_timer_fast(lapic_timer_count_t initial_count) {
  LAPIC_WRITE(LVT_TIMER, LAPIC_READ(LVT_TIMER) & ~LAPIC_LVT_MASKED);
  LAPIC_WRITE(TIMER_INITIAL_COUNT, initial_count);
}

void lapic_set_tsc_deadline_timer(uint64_t deadline) {
  /* Don't bother disarming: wrmsr64(MSR_IA32_TSC_DEADLINE, 0); */
  wrmsr64(MSR_IA32_TSC_DEADLINE, deadline);
}

uint64_t lapic_get_tsc_deadline_timer(void) {
  return rdmsr64(MSR_IA32_TSC_DEADLINE);
}

void lapic_get_timer(lapic_timer_mode_t *mode, lapic_timer_divide_t *divisor,
                     lapic_timer_count_t *initial_count,
                     lapic_timer_count_t *current_count) {
  mp_disable_preemption();
  if (mode) {
    *mode = (LAPIC_READ(LVT_TIMER) & LAPIC_LVT_PERIODIC) ? periodic : one_shot;
  }
  if (divisor) {
...
static inline void _lapic_end_of_interrupt(void) { LAPIC_WRITE(EOI, 0); }

void lapic_end_of_interrupt(void) { _lapic_end_of_interrupt(); }
...
void lapic_set_intr_func(int vector, i386_intr_func_t func) {
  ...
  case LAPIC_PM_INTERRUPT:
    ...
    break;
  default:
    panic("lapic_set_intr_func(%d,%p) invalid vector", vector, func);
  }
}
...
int lapic_interrupt(int interrupt_num, x86_saved_state_t *state) {
  int retval = 0;
  int esr = -1;
  ...
  case LAPIC_ERROR_INTERRUPT:
    ...
    if (cpu_number() == master_cpu) {
      ...
      printf(
          "Local APIC error on master CPU, ESR: %d, error count this run: %d\n",
...
void lapic_send_ipi(int cpu, int vector) {
  boolean_t state;
...
void lapic_send_nmi(int cpu) {
  if (!is_x2apic) {
    ...
    if (__improbable(LAPIC_READ_ICR() & LAPIC_ICR_DS_PENDING)) {
      /* Since it's not safe to invoke printf here, kprintf and counting is the
       * best we can do */
...
boolean_t lapic_is_interrupt_pending(void) {
  int i;
...
boolean_t lapic_is_interrupting(uint8_t vector) {
  int i;
  int bit;
  uint32_t irr;
  uint32_t isr;
...
void lapic_interrupt_counts(uint64_t intrs[256]) {
  int i;
  int j;
  int bit;
  uint32_t irr;
...
uint8_t lapic_get_cmci_vector(void) {
