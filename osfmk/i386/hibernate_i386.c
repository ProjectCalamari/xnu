 ...
 */
/*!
 * i386/x86_64-specific functions required to support hibernation entry, and
 * also to support hibernation exit after wired pages have already been
 * restored.
 */
  ...
  for (i = 0; i < mcount;
    ...
    case kEfiConventionalMemory:
      ...
      if (bank && (base == (1 + dram_ranges[bank - 1].last_page))) {
        ...
        bcopy(&dram_ranges[bank], &dram_ranges[bank + 1],
...
void hibernate_page_list_setall_machine(
    __unused hibernate_page_list_t *page_list,
    __unused hibernate_page_list_t *page_list_wired,
    __unused boolean_t preflight, __unused uint32_t *pagesOut) {}
...
void hibernate_page_list_set_volatile(hibernate_page_list_t *page_list,
  ...
  if (args->efiRuntimeServicesPageStart) {
    hibernate_set_page_state(
        page_list, page_list_wired, args->efiRuntimeServicesPageStart,
        args->efiRuntimeServicesPageCount, kIOHibernatePageStateFree);
...
// ACPI calls hibernate_vm_lock(), interrupt disable, hibernate_vm_unlock() on
// sleep, hibernate_vm_lock_end() and interrupt enable on wake. VM locks are
