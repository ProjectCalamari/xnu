...
char mp_slave_stack[PAGE_SIZE]
    __attribute__((aligned(PAGE_SIZE))); // Temp stack for slave init
...
static void cpu_warm_timer_call_func(timer_call_param_t p0,
...
void smp_init(void) {
...
#if DEBUG || DEVELOPMENT
  if (PE_parse_boot_argn("interrupt_watchdog", &mp_interrupt_watchdog_enabled,
  ...
  if (PE_parse_boot_argn("TSC_sync_margin", &TSC_sync_margin,
 ...
 */
static void mp_wait_for_cpu_up(int slot_num, unsigned int iters,
 ...
 */
kern_return_t intel_startCPU_fast(int slot_num) {
  kern_return_t rc;
  ...
  KERNEL_DEBUG_CONSTANT(TRACE_MP_CPU_FAST_START | DBG_FUNC_START, slot_num, 0,
                        0, 0, 0);
  ...
  KERNEL_DEBUG_CONSTANT(TRACE_MP_CPU_FAST_START | DBG_FUNC_END, slot_num,
                        cpu_datap(slot_num)->cpu_running, 0, 0, 0);
...
static void started_cpu(void) {
  /* Here on the started cpu with cpu_running set TRUE */

  if (TSC_sync_margin && start_info.target_cpu == cpu_number()) {
    /*
...
static void start_cpu(void *arg) {
  ...
  KERNEL_DEBUG_CONSTANT(TRACE_MP_CPU_START | DBG_FUNC_START, psip->target_cpu,
  ...
  KERNEL_DEBUG_CONSTANT(TRACE_MP_CPU_START | DBG_FUNC_END, psip->target_cpu,
                        cpu_datap(psip->target_cpu)->cpu_running, 0, 0, 0);

  if (TSC_sync_margin && cpu_datap(psip->target_cpu)->cpu_running) {
    /*
...
long NMI_pte_corruption_callback(__unused void *arg0, __unused void *arg1,
                                 uint16_t lcpu) {
  ...
  snprintf(&pstr[0], sizeof(pstr),
           ...
           lcpu, (unsigned long long)(uintptr_t)PTE_corrupted_ptr,
           *(uint64_t *)PTE_corrupted_ptr);
  panic_i386_backtrace(stackptr, 64, &pstr[0], TRUE,
                       current_cpu_datap()->cpu_int_state);
  return 0;
}

extern void kprintf_break_lock(void);
int NMIInterruptHandler(x86_saved_state_t *regs) {
  void *stackptr;
  char pstr[256];
  ...
  if (NMI_panic_reason == SPINLOCK_TIMEOUT) {
    ...
    snprintf(
        &pstr[0], sizeof(pstr),
        "Panic(CPU %d, time %llu): NMIPI for spinlock acquisition timeout, "
        "spinlock: %p, "
    ...
    snprintf(&pstr[0], sizeof(pstr),
             "Panic(CPU %d, time %llu): NMIPI for unresponsive processor: TLB "
             "flush timeout, TLB state:0x%x\n",
    ...
    snprintf(&pstr[0], sizeof(pstr),
             "Panic(CPU %d, time %llu): NMIPI for unresponsive processor: "
             "cross-call timeout\n",
             cpu_number(), now);
    panic_i386_backtrace(stackptr, 64, &pstr[0], TRUE, regs);
    ...
    snprintf(&pstr[0], sizeof(pstr),
             "Panic(CPU %d, time %llu): NMIPI for unresponsive processor: "
             "interrupt watchdog for vector 0x%x\n",
  ...
  } else if (!mp_kdp_trap && !mp_kdp_is_NMI && virtualized &&
             (debug_boot_arg & DB_NMI)) {
    /*
...
void cpu_interrupt(int cpu) {
  ...
  if (smp_initialized && pmCPUExitIdle(cpu_datap(cpu))) {
    i386_cpu_IPI(cpu);
    did_IPI = TRUE;
  }

  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_REMOTE_AST), cpu,
                        did_IPI, 0, 0, 0);
}
 ...
 */
void cpu_NMI_interrupt(int cpu) {
  if (smp_initialized) {
    i386_send_NMI(cpu);
  }
}
...
void PM_interrupt_register(void (*fn)(void)) { mp_PM_func = fn; }

void i386_signal_cpu(int cpu, mp_event_t event, mp_sync_t mode) {
  ...
  again:
    tsc_timeout =
        !machine_timeout_suspended() ? rdtsc64() + (1000 * 1000 * 1000) : ~0ULL;
 ...
 */
static boolean_t mp_spin_timeout(uint64_t tsc_start) {
  uint64_t tsc_timeout;
   ...
   */
  tsc_timeout =
      disable_serial_output ? LockTimeOutTSC << 2 : LockTimeOutTSC << 4;
  ...
  while (!simple_lock_try(lock, LCK_GRP_NULL)) {
    ...
    if (mp_spin_timeout(tsc_spin_start)) {
      ...
      panic("mp_safe_spin_lock() timed out, lock: %p, "
            ...
            lock, lowner, current_thread(), lsti->owner_cpu,
            mach_absolute_time());
    }
  }
...
static void mp_rendezvous_action(__unused void *null) {
  boolean_t intrs_enabled;
  ...
  while (mp_rv_entry < mp_rv_ncpus) {
    ...
    if (mp_spin_timeout(tsc_spin_start)) {
      panic("mp_rv_action() entry: %ld of %d responses, start: 0x%llx, cur: "
            "0x%llx",
            mp_rv_entry, mp_rv_ncpus, tsc_spin_start, rdtsc64());
    }
  }
  ...
  while (mp_rv_exit < mp_rv_ncpus) {
    ...
    if (mp_spin_timeout(tsc_spin_start)) {
      panic("mp_rv_action() exit: %ld of %d responses, start: 0x%llx, cur: "
            "0x%llx",
            mp_rv_exit, mp_rv_ncpus, tsc_spin_start, rdtsc64());
    }
  }
...
void mp_rendezvous(void (*setup_func)(void *), void (*action_func)(void *),
   ...
   */
  mp_rv_ncpus =
      mp_cpus_call(CPUMASK_OTHERS, NOSYNC, &mp_rendezvous_action, NULL) + 1;
  ...
  while (mp_rv_complete < mp_rv_ncpus) {
    if (mp_spin_timeout(tsc_spin_start)) {
      panic("mp_rendezvous() timeout: %ld of %d responses, start: 0x%llx, cur: "
            "0x%llx",
            mp_rv_complete, mp_rv_ncpus, tsc_spin_start, rdtsc64());
    }
  }
...
void mp_rendezvous_lock(void) { (void)mp_safe_spin_lock(&mp_rv_lock); }

void mp_rendezvous_unlock(void) { simple_unlock(&mp_rv_lock); }

void mp_rendezvous_break_lock(void) { simple_lock_init(&mp_rv_lock, 0); }

static void setup_disable_intrs(__unused void *param_not_used) {
...
static void teardown_restore_intrs(__unused void *param_not_used) {
...
void mp_rendezvous_no_intrs(void (*action_func)(void *), void *arg) {
  mp_rendezvous(setup_disable_intrs, action_func, teardown_restore_intrs, arg);
}
...
static inline boolean_t mp_call_head_lock(mp_call_queue_t *cqp) {
  boolean_t intrs_enabled;
 ...
 */
void NMIPI_panic(cpumask_t cpu_mask, NMI_reason_t why) {
  unsigned int cpu;
  cpumask_t cpu_bit;
  uint64_t deadline;
...
#if MACH_ASSERT
static inline boolean_t mp_call_head_is_locked(mp_call_queue_t *cqp) {
  return !ml_get_interrupts_enabled() && hw_lock_held((hw_lock_t)&cqp->lock);
}
#endif

static inline void mp_call_head_unlock(mp_call_queue_t *cqp,
                                       boolean_t intrs_enabled) {
  simple_unlock(&cqp->lock);
  ml_set_interrupts_enabled(intrs_enabled);
}

static inline mp_call_t *mp_call_alloc(void) {
...
static inline void mp_call_free(mp_call_t *callp) {
  boolean_t intrs_enabled;
  mp_call_queue_t *cqp = &mp_cpus_call_freelist;
...
static inline mp_call_t *mp_call_dequeue_locked(mp_call_queue_t *cqp) {
  mp_call_t *callp = NULL;
...
static inline void mp_call_enqueue_locked(mp_call_queue_t *cqp,
                                          mp_call_t *callp) {
 ...
 */
void mp_cpus_call_cpu_init(int cpu) {
  int i;
...
static void mp_cpus_call_action(void) {
  ...
  while ((callp = mp_call_dequeue_locked(cqp)) != NULL) {
    ...
    if (call.func != NULL) {
      mp_call_head_unlock(cqp, intrs_enabled);
      KERNEL_DEBUG_CONSTANT(TRACE_MP_CPUS_CALL_ACTION,
                            VM_KERNEL_UNSLIDE(call.func),
                            VM_KERNEL_UNSLIDE_OR_PERM(call.arg0),
                            VM_KERNEL_UNSLIDE_OR_PERM(call.arg1),
                            VM_KERNEL_ADDRPERM(call.maskp), 0);
 ...
 */
cpu_t mp_cpus_call(cpumask_t cpus, mp_sync_t mode, void (*action_func)(void *),
                   void *arg) {
  return mp_cpus_call1(cpus, mode, (void (*)(void *, void *))action_func, arg,
                       NULL, NULL);
}

#pragma clang diagnostic pop

static void mp_cpus_call_wait(boolean_t intrs_enabled, cpumask_t cpus_called,
  ...
  while (*cpus_responded != cpus_called) {
    ...
    if (mp_spin_timeout(tsc_spin_start)) {
      ...
      panic("mp_cpus_call_wait() timeout, cpus: 0x%llx", cpus_unresponsive);
    }
  }
}

cpu_t mp_cpus_call1(cpumask_t cpus, mp_sync_t mode,
                    void (*action_func)(void *, void *), void *arg0, void *arg1,
  ...
  KERNEL_DEBUG_CONSTANT(TRACE_MP_CPUS_CALL | DBG_FUNC_START, cpus, mode,
  ...
  for (cpu = 0; cpu < (cpu_t)real_ncpus; cpu++) {
    if (((cpu_to_cpumask(cpu) & cpus) == 0) || !cpu_is_running(cpu)) {
      continue;
    }
    tsc_spin_start = rdtsc64();
    if (cpu == (cpu_t)cpu_number()) {
      ...
      if (mode == SYNC && action_func != NULL) {
        KERNEL_DEBUG_CONSTANT(TRACE_MP_CPUS_CALL_LOCAL,
                              VM_KERNEL_UNSLIDE(action_func),
                              VM_KERNEL_UNSLIDE_OR_PERM(arg0),
                              VM_KERNEL_UNSLIDE_OR_PERM(arg1), 0, 0);
        action_func(arg0, arg1);
      }
    } else {
      /*
    ...
    queue_call:
      ...
      if (callp == NULL) {
        ...
        KERNEL_DEBUG_CONSTANT(TRACE_MP_CPUS_CALL_NOBUF, cpu, 0, 0, 0, 0);
        if (!intrs_inner) {
          /* Sniffing w/o locking */
          if (!queue_empty(&cqp->queue)) {
            mp_cpus_call_action();
  ...
  if (mode != SYNC && call_self) {
    KERNEL_DEBUG_CONSTANT(
        TRACE_MP_CPUS_CALL_LOCAL, VM_KERNEL_UNSLIDE(action_func),
...
out:
  ...
  KERNEL_DEBUG_CONSTANT(TRACE_MP_CPUS_CALL | DBG_FUNC_END, cpus_call_count,
                        cpus_called, 0, 0, 0);

  return (cpu_t)cpus_call_count;
}

static void mp_broadcast_action(__unused void *null) {
 ...
 */
void mp_broadcast(void (*action_func)(void *), void *arg) {
  if (!smp_initialized) {
    if (action_func != NULL) {
      action_func(arg);
    }
...
void mp_cpus_kick(cpumask_t cpus) {
  cpu_t cpu;
  boolean_t intrs_enabled = FALSE;
...
void i386_deactivate_cpu(void) {
  ...
  KERNEL_DEBUG_CONSTANT(TRACE_MP_CPU_DEACTIVATE | DBG_FUNC_START, 0, 0, 0, 0,
                        0);
   ...
   */
  timer_queue_shutdown(master_cpu, &cdp->rtclock_timer.queue,
                       &cpu_datap(master_cpu)->rtclock_timer.queue);

  mp_cpus_call(cpu_to_cpumask(master_cpu), NOSYNC, timer_queue_expire_local,
               NULL);
  ...
  KERNEL_DEBUG_CONSTANT(TRACE_MP_CPU_DEACTIVATE | DBG_FUNC_END, 0, 0, 0, 0, 0);
}
...
void mp_kdp_enter(boolean_t proceed_on_failure, bool is_stackshot) {
  unsigned int cpu;
  ...
  uint64_t start_time = cpu_datap(my_cpu)->debugger_entry_time =
      mach_absolute_time();
  int locked = 0;
  while (!locked || mp_kdp_trap) {
    ...
    if (proceed_on_failure) {
      if (mach_absolute_time() - start_time > 500000000ll) {
        paniclog_append_noflush("mp_kdp_enter() can't get x86_topo_lock! "
                                "Debugging anyway! #YOLO\n");
        break;
      }
  ...
  if (force_immediate_debugger_NMI == FALSE) {
    ...
    if (mp_kdp_ncpus != ncpus) {
      ...
      if (mp_kdp_ncpus != ncpus) {
        ...
        if (proceed_on_failure) {
          paniclog_append_noflush(
              "mp_kdp_enter() timed-out during %s wait after NMI;"
              "expected %u acks but received %lu after %u loops in %llu "
              "ticks\n",
              (locked ? "locked" : "unlocked"), ncpus, mp_kdp_ncpus,
              wait_cycles, LockTimeOutTSC);
        } else {
          panic(
              "mp_kdp_enter() timed-out during %s wait after NMI;"
  ...
  } else if (NMI_panic_reason !=
             PTE_CORRUPTION) { /* In the pte corruption case, the detecting CPU
                                  has already NMIed other CPUs */
    for (cpu = 0; cpu < real_ncpus; cpu++) {
      if (cpu == my_cpu || !cpu_is_running(cpu)) {
        continue;
      }
  ...
  DBG("mp_kdp_enter() %d processors done %s\n", (int)mp_kdp_ncpus,
      (mp_kdp_ncpus == ncpus) ? "OK" : "timed out");

  postcode(MP_KDP_ENTER);
}

boolean_t mp_kdp_all_cpus_halted() {
...
static void kdp_x86_xcpu_poll(void) {
  if ((uint16_t)cpu_number() == kdp_xcpu_call_func.cpu) {
    kdp_xcpu_call_func.ret = kdp_xcpu_call_func.func(
        kdp_xcpu_call_func.arg0, kdp_xcpu_call_func.arg1, cpu_number());
    kdp_xcpu_call_func.cpu = KDP_XCPU_NONE;
  }
}
...
static void mp_kdp_wait(boolean_t flush, boolean_t isNMI) {
  ...
  /* If this is a stackshot, setup the CPU state before signalling we've entered
   * the debugger. */
...
void mp_kdp_exit(void) {
  ...
  while (mp_kdp_ncpus > 0) {
    /*
     * a TLB shootdown request may be pending... this would result in the
     * requesting processor waiting in PMAP_UPDATE_TLBS() until this processor
     * deals with it. Process it, so it can now enter mp_kdp_wait()
     */
    handle_pending_TLB_flushes();
...
boolean_t mp_recent_debugger_activity(void) {
...
void cause_ast_check(processor_t processor) {
  ...
  if (cpu != cpu_number()) {
    ...
    KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_REMOTE_AST), cpu, 1,
                          0, 0, 0);
  }
}

void machine_cpu_reinit(void *param) {
  /*
...
#undef cpu_number
int cpu_number(void) { return get_cpu_number(); }

vm_offset_t current_percpu_base(void) { return get_current_percpu_base(); }

vm_offset_t other_percpu_base(int cpu) { return cpu_datap(cpu)->cpu_pcpu_base; }

static void cpu_prewarm_init() {
  int i;
...
static timer_call_t grab_warm_timer_call() {
  spl_t x;
...
static void _cpu_warm_setup(void *arg) {
  ...
  timer_call_enter(cwdp->cwd_call, cwdp->cwd_deadline,
...
#if DEBUG || DEVELOPMENT
void kernel_spin(uint64_t spin_ns) {
  boolean_t istate;
  uint64_t spin_abs;
  uint64_t deadline;
  cpu_data_t *cdp;
 ...
 */
void mp_interrupt_watchdog(void) {
  cpu_t cpu;
  boolean_t intrs_enabled = FALSE;
   ...
   */
  for (cpu = 0; cpu < (cpu_t)real_ncpus && !machine_timeout_suspended();
       cpu++) {
    if ((cpu == (cpu_t)cpu_number()) || (!cpu_is_running(cpu))) {
      continue;
    }
