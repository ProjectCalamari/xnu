 ...
 */
extern void __dead2 panic_idt64(x86_saved_state_t *rsp);
void panic_idt64(x86_saved_state_t *rsp) {
  kprint_state(saved_state64(rsp));
  panic("panic_idt64");
}
#endif

__dead2 static void panic_64(x86_saved_state_t *sp, __unused int pc,
                             __unused const char *msg, boolean_t do_mca_dump) {
  ...
  panic("%s at 0x%016llx, registers:\n"
        ...
        msg, regs->isf.rip, get_cr0(), get_cr2(), get_cr3_raw(), get_cr4(),
        regs->rax, regs->rbx, regs->rcx, regs->rdx, regs->isf.rsp, regs->rbp,
        regs->rsi, regs->rdi, regs->r8, regs->r9, regs->r10, regs->r11,
        regs->r12, regs->r13, regs->r14, regs->r15, regs->isf.rflags,
        regs->isf.rip, regs->isf.cs & 0xFFFF, regs->isf.ss & 0xFFFF,
        regs->isf.err, virtualized ? " VMM" : "");
}

void panic_double_fault64(x86_saved_state_t *sp) {
#if DEVELOPMENT || DEBUG
  uint64_t frameptr =
      is_saved_state64(sp) ? saved_state64(sp)->rbp : saved_state32(sp)->ebp;
  (void)traptrace_start(T_DOUBLE_FAULT, saved_state64(sp)->isf.rip,
                        mach_absolute_time(), frameptr);
#endif
  (void)OSCompareAndSwap((UInt32)-1, (UInt32)cpu_number(),
                         (volatile UInt32 *)&panic_double_fault_cpu);
  panic_64(sp, PANIC_DOUBLE_FAULT, "Double fault", FALSE);
}
void panic_machine_check64(x86_saved_state_t *sp) {
#if DEVELOPMENT || DEBUG
  uint64_t frameptr =
      is_saved_state64(sp) ? saved_state64(sp)->rbp : saved_state32(sp)->ebp;
  (void)traptrace_start(T_MACHINE_CHECK, saved_state64(sp)->isf.rip,
                        mach_absolute_time(), frameptr);
#endif
