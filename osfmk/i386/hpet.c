...
int hpet_register_callback(int (*hpet_reqst)(uint32_t apicid, void *arg,
                                             hpetRequest_t *hpet),
                           void *arg) {
  hpet_req = hpet_reqst;
  hpet_arg = arg;
 ...
 */
void hpet_get_info(hpetInfo_t *info) {
 ...
 */
void ml_hpet_cfg(uint32_t cpu, uint32_t hpetVect) {
  uint64_t *hpetVaddr;
  hpetTimer_t *hpet;
  x86_lcpu_t *lcpu;
  x86_core_t *core;
  ...
  hpetVaddr = (uint64_t *)(((uintptr_t)&(((hpetReg_t *)hpetArea)->TIM1_CONF)) +
                           (cpu << 5));
