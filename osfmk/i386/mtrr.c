...
/* 0 < bits <= 64 */
#define PHYS_BITS_TO_MASK(bits) ((((1ULL << (bits - 1)) - 1) << 1) | 1)
...
#define LEN_TO_MASK(len) (~((len) - 1) & IA32_MTRR_PHYSBASE_MASK)
 ...
 */
static void mtrr_set_fix_ranges(const struct mtrr_fix_range *range) {
  int i;
...
static boolean_t mtrr_check_fix_ranges(const struct mtrr_fix_range *range) {
  int i;
  ...
  match = range[0].types == rdmsr64(MSR_IA32_MTRR_FIX64K_00000) &&
          ...
          range[2].types == rdmsr64(MSR_IA32_MTRR_FIX16K_A0000);
  for (i = 0; match && i < 8; i++) {
    match = range[3 + i].types == rdmsr64(MSR_IA32_MTRR_FIX4K_C0000 + i);
  }

  return match;
}

static boolean_t mtrr_check_var_ranges(mtrr_var_range_t *range, int count) {
  int i;
  boolean_t match = TRUE;
...
#if MTRR_DEBUG
static void mtrr_msr_dump(void) {
  int i;
  ...
  for (i = 0; i < count; i++) {
    DBG(" %02x    0x%016llx  0x%016llx  0x%llx\n", i,
        rdmsr64(MSR_IA32_MTRR_PHYSBASE(i)), rdmsr64(MSR_IA32_MTRR_PHYSMASK(i)),
...
void mtrr_init(void) {
  ...
  if (mtrr_state.var_count) {
    mtrr_state.var_range = (mtrr_var_range_t *)zalloc_permanent_tag(
        sizeof(mtrr_var_range_t) * mtrr_state.var_count,
        ZALIGN(mtrr_var_range_t), VM_KERN_MEMORY_CPU);
  ...
  if (mtrr_state.var_count) {
    mtrr_get_var_ranges(mtrr_state.var_range, mtrr_state.var_count);
  }
 ...
 */
static void mtrr_update_action(void *cache_control_type) {
  uintptr_t cr0, cr4;
  uintptr_t tmp;
  ...
  if (CACHE_CONTROL_PAT == cache_control_type) {
     ...
     * The five high-order bits of each field are reserved, and must be set to
     * all 0s." So, we zero-out the high 5 bits of the PA6 entry here:
     */
    pat &= ~(0xFFULL << 48);
    pat |= (0x01ULL << 48);
    ...
    /* apply MTRR settings */
    if (mtrr_state.var_count) {
      mtrr_set_var_ranges(mtrr_state.var_range, mtrr_state.var_count);
    }
...
static void mtrr_update_setup(__unused void *param_not_used) {
  /* disable interrupts before the first barrier */
  current_cpu_datap()->cpu_iflag = ml_set_interrupts_enabled(FALSE);
  DBG("CPU%d: %s\n", get_cpu_number(), __FUNCTION__);
}

static void mtrr_update_teardown(__unused void *param_not_used) {
 ...
 */
kern_return_t mtrr_update_all_cpus(void) {
  if (mtrr_initialized == FALSE) {
    return KERN_NOT_SUPPORTED;
  }

  MTRR_LOCK();
  mp_rendezvous(mtrr_update_setup, mtrr_update_action, mtrr_update_teardown,
                NULL);
  MTRR_UNLOCK();
 ...
 */
kern_return_t mtrr_update_cpu(void) {
  boolean_t match = TRUE;
  ...
  if (match && mtrr_state.var_count) {
    match = mtrr_check_var_ranges(mtrr_state.var_range, mtrr_state.var_count);
  }
 ...
 */
kern_return_t mtrr_range_add(addr64_t address, uint64_t length, uint32_t type) {
  mtrr_var_range_t *vr;
  ...
  if ((type != MTRR_TYPE_UNCACHEABLE) && (type != MTRR_TYPE_WRITECOMBINE) &&
      (type != MTRR_TYPE_WRITETHROUGH) && (type != MTRR_TYPE_WRITEPROTECT) &&
   ...
   */
  if ((length < 0x1000) || (LSB(length) != length) ||
 ...
 */
kern_return_t mtrr_range_remove(addr64_t address, uint64_t length,
                                uint32_t type) {
  mtrr_var_range_t *vr;
  ...
  for (i = 0; i < mtrr_state.var_count; i++) {
    ...
    if (vr->refcnt && var_range_overlap(vr, address, length, type) > 0) {
...
static void var_range_encode(mtrr_var_range_t *range, addr64_t address,
  ...
  range->mask = LEN_TO_MASK(length) | (valid ? IA32_MTRR_PHYMASK_VALID : 0);
}
