...
typedef char
    check1_[sizeof(struct panic_hook) <= sizeof(panic_hook_t) ? 1 : -1];
...
void panic_check_hook(void) {
  ...
  queue_iterate(&panic_hooks, hook, struct panic_hook *, chain) {
    if (++count > 1024 || !kvtophys((vm_offset_t)hook) ||
...
void panic_dump_mem(const void *addr, int len) {
  ...
  for (; len > 0; addr = (const uint8_t *)addr + PAGE_SIZE, len -= PAGE_SIZE) {
    ...
    int n = WKdm_compress_new((const WK_word *)addr,
                              (WK_word *)(void *)panic_dump_buf, scratch, 4095);
...
boolean_t panic_phys_range_before(const void *addr, uint64_t *pphys,
  ...
  *pphys = kvtophys((vm_offset_t)addr);
  ...
  const EfiMemoryRange *r = PHYSMAP_PTOV((uintptr_t)args->MemoryMap),
                       *closest = NULL;
  ...
  for (uint32_t i = 0; i < count;
       ++i, r = (const EfiMemoryRange *)(const void *)((const uint8_t *)r +
                                                       size)) {
