...
uint64_t max_mem; /* Size of physical memory minus carveouts (bytes), adjusted
                     by maxmem */
 ...
 */
vm_offset_t segTEXTB;
unsigned long segSizeTEXT;
vm_offset_t segDATAB;
...
static void kprint_memmap(vm_offset_t maddr, unsigned int msize,
                          unsigned int mcount) {
  unsigned int i;
  unsigned int j;
  ...
  for (j = 0; j < pmap_memory_region_count; j++, p++) {
    kprintf(
        "pmap region %d type %d base 0x%llx alloc_up 0x%llx alloc_down 0x%llx"
        " alloc_frag_up 0x%llx alloc_frag_down 0x%llx top 0x%llx\n",
        j, p->type, (addr64_t)p->base << I386_PGSHIFT,
    ...
    for (i = 0; i < mcount;
      ...
      if (mptr->Type != kEfiLoaderCode && mptr->Type != kEfiLoaderData &&
        ...
        efi_end =
            efi_start + ((vm_offset_t)mptr->NumberOfPages << I386_PGSHIFT) - 1;
 ...
 */
void i386_vm_init(uint64_t maxmem, boolean_t IA32e, boot_args *args) {
  ...
  segHIBB = (vm_offset_t)getsegdatafromheader(&_mh_execute_header, "__HIB",
                                              &segSizeHIB);
  ...
  segCONST = getsegbynamefromheader(&_mh_execute_header, "__DATA_CONST");
  ...
  /* XXX: FIXME_IN_dyld: For new-style kernel caches, the ending address of
   * __DATA_CONST may not be page-aligned */
  ...
  vm_kernel_slid_top =
      (kc_format == KCFormatFileset) ? vm_slinkedit : vm_prelink_einfo;

  vm_page_kernelcache_count =
      (unsigned int)(atop_64(vm_kernel_top - vm_kernel_base));
...
#define ONEGIG 0x0000000040000000ULL
  ...
  for (i = 0; i < mcount;
    ...
    top = (ppnum_t)(((mptr->PhysicalStart) >> I386_PGSHIFT) +
                    mptr->NumberOfPages - 1);
    ...
    if ((mptr->Attribute & EFI_MEMORY_RUNTIME) == EFI_MEMORY_RUNTIME ||
       ...
       */
      if (top < I386_LOWMEM_RESERVED || !pal_is_usable_memory(base, top)) {
        prev_pmptr = 0;
        continue;
      }
      /*
      ...
      coalescing_permitted =
          (prev_pmptr && (pmptr->attribute == prev_pmptr->attribute) &&
           ((pmptr->attribute & EFI_MEMORY_KERN_RESERVED) == 0));
      /*
       * Consolidate contiguous memory regions, if possible
       */
      if (prev_pmptr && (pmptr->type == prev_pmptr->type) &&
          (coalescing_permitted) && (pmptr->base == pmptr->alloc_up) &&
  ...
  if (!PE_parse_boot_argn("max_valid_dma_addr", &maxdmaaddr,
                          sizeof(maxdmaaddr))) {
  ...
  if (avail_end >= max_valid_dma_address) {
    if (!PE_parse_boot_argn("maxloreserve", &maxloreserve,
                            sizeof(maxloreserve))) {
...
unsigned int pmap_free_pages(void) { return (unsigned int)avail_remaining; }
 ...
 * free list.
 */
boolean_t pmap_next_page_reserved(ppnum_t *pn) {
  uint32_t n;
...
#if DEBUG
      if (region->alloc_up > region->alloc_down) {
        kprintf("Exhausted reserved range index: %u, base: 0x%x end: 0x%x, "
                "type: 0x%x, attribute: 0x%llx\n",
                reserved_index, region->base, region->end, region->type,
                region->attribute);
      }
#endif
      return TRUE;
...
/*
 * Return the highest large page available. Fails once there are no more large
 * pages.
 */
...
boolean_t pmap_next_page_hi(ppnum_t *pn, boolean_t might_free) {
  pmap_memory_region_t *region;
  int n;
