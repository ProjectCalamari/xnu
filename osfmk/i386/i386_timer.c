 ...
 *
 * XXX a drawback of this implementation is that events serviced earlier must
 * not set deadlines that occur before the entire chain completes.
 *
 * XXX a better implementation would use a set of generic callouts and iterate
 * over them
 */
void timer_intr(int user_mode, uint64_t rip) {
  ...
  if (timer_processed) {
    ...
    if (abstime > rtclock_deadline) {
      TCOAL_DEBUG(0xEEEE0000, abstime, mytimer->queue.earliest_soft_deadline,
  ...
  if (__probable(timer_processed)) {
     ...
     */
    int64_t latency =
        min_deadline == EndOfAllTime ? 0 : (int64_t)(abstime - min_deadline);
    /*
...
void timer_resync_deadlines(void) {
  ...
  if (decr != 0 && deadline != pmdeadline) {
    ...
    KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE, DECR_SET_DEADLINE | DBG_FUNC_NONE,
                              decr, 2, deadline, queue_count, 0);
  }
  splx(s);
}

void timer_queue_expire_local(__unused void *arg) {
  rtclock_timer_t *mytimer;
  uint64_t abstime;
  cpu_data_t *pp;
...
void timer_queue_expire_rescan(__unused void *arg) {
  rtclock_timer_t *mytimer;
  uint64_t abstime;
  cpu_data_t *pp;
  ...
  mytimer->has_expired = TRUE;
  mytimer->deadline =
      timer_queue_expire_with_options(&mytimer->queue, abstime, TRUE);
  mytimer->has_expired = FALSE;
  mytimer->when_set = mach_absolute_time();
 ...
 */
uint32_t timer_queue_migrate_cpu(int target_cpu) {
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE, DECR_TIMER_MIGRATE | DBG_FUNC_START,
                            target_cpu, cdp->rtclock_timer.deadline,
                            (cdp->rtclock_timer.deadline >> 32), 0, 0);
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE, DECR_TIMER_MIGRATE | DBG_FUNC_END,
...
static timer_coalescing_priority_params_ns_t tcoal_prio_params_init = {
    ...
    .latency_qos_ns_max = {1 * NSEC_PER_MSEC, 5 * NSEC_PER_MSEC,
                           20 * NSEC_PER_MSEC, 75 * NSEC_PER_MSEC,
                           10000 * NSEC_PER_MSEC, 10000 * NSEC_PER_MSEC},
