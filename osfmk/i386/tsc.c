 ...
 */
static uint64_t EFI_get_frequency(const char *prop) {
...
void tsc_init(void) {
  ...
  case CPUFAMILY_INTEL_SKYLAKE: {
    ...
    kprintf(" ART: Frequency = %6d.%06dMHz, N/M = %lld/%llu\n",
            (uint32_t)(refFreq / Mega), (uint32_t)(refFreq % Mega), N, M);
  ...
  kprintf(" TSC: Frequency = %6d.%06dMHz, "
          ...
          (uint32_t)(tscFCvtn2t >> 32), (uint32_t)tscFCvtn2t, tscGranularity,
          N_by_2_bus_ratio ? " (N/2)" : "");
}

void tsc_get_info(tscInfo_t *info) {
...
void cpu_data_tsc_sync_deltas_string(char *buf, uint32_t buflen,
  ...
  for (uint32_t curcpu = start_cpu; curcpu <= end_cpu; curcpu++) {
    cnt = snprintf(buf + offset, buflen - offset, "0x%llx ",
                   cpu_datap(curcpu)->tsc_sync_delta);
