...
vm_offset_t acpi_install_wake_handler(void) {
#if CONFIG_SLEEP
...
static void acpi_hibernate(void *refcon) {
  uint32_t mode;
...
void acpi_sleep_kernel(acpi_sleep_callback func, void *refcon) {
#if HIBERNATION
  acpi_hibernate_callback_data_t data;
#endif
  ...
  for (cpu = 0; cpu < real_ncpus; cpu += 1) {
    ...
    if (rc != KERN_SUCCESS) {
      panic("Error %d trying to transition CPU %d to OFF", rc, cpu);
    }
  }
 ...
 */
void acpi_idle_kernel(acpi_sleep_callback func, void *refcon) {
  boolean_t istate = ml_get_interrupts_enabled();

  kprintf("acpi_idle_kernel, cpu=%d, interrupts %s\n", cpu_number(),
          istate ? "enabled" : "disabled");
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_DEEP_IDLE) |
                            DBG_FUNC_START,
...
void install_real_mode_bootstrap(void *prot_entry) {
  /*
   * Copy the boot entry code to the real-mode vector area
   * REAL_MODE_BOOTSTRAP_OFFSET. This is in page 1 which has been reserved for
   * this purpose by machine_startup() from the boot processor. The slave boot
   * code is responsible for switching to protected mode and then jumping to the
   * common startup, _start().
   */
   ...
   */
  ml_phys_write_word(PROT_MODE_START + REAL_MODE_BOOTSTRAP_OFFSET,
...
static uint8_t cksum8(uint8_t *ptr, uint32_t size) {
  uint8_t sum = 0;
  uint32_t i;
...
/*
 * Parameterized search for a specified table given an sdtp (either RSDT or
 * XSDT). Note that efiboot does not modify the addresses of tables in the RSDT
 * or XSDT TableOffsetEntry array, so we do not need to "convert" from efiboot
 * virtual to physical.
 */
#define SEARCH_FOR_ACPI_TABLE(sdtp, signature, entry_type)                     \
  {                                                                            \
    uint32_t i, pointer_count;                                                 \
                                                                               \
    /* Walk the list of tables in the *SDT, looking for the signature passed   \
     * in */                                                                   \
    pointer_count =                                                            \
        ((sdtp)->Length - sizeof(ACPI_TABLE_HEADER)) / sizeof(entry_type);     \
                                                                               \
    for (i = 0; i < pointer_count; i++) {                                      \
      ACPI_TABLE_HEADER *next_table = (ACPI_TABLE_HEADER *)PHYSMAP_PTOV(       \
...
static ACPI_TABLE_HEADER *acpi_find_table_via_xsdt(XSDT_DESCRIPTOR *xsdtp,
                                                   const char *signature) {
  SEARCH_FOR_ACPI_TABLE(xsdtp, signature, UINT64);
}

static ACPI_TABLE_HEADER *acpi_find_table_via_rsdt(RSDT_DESCRIPTOR *rsdtp,
                                                   const char *signature) {
 ...
 */
static ACPI_TABLE_HEADER *acpi_find_table(uintptr_t rsdp_physaddr,
                                          const char *signature) {
  ...
  if (__improbable(rsdp == NULL || (rsdtp == NULL && xsdtp == NULL))) {
     ...
     */
    if (__improbable(rsdp->Revision >= RSDP_VERSION_ACPI20 &&
                     rsdp->XsdtPhysicalAddress == 0ULL)) {
 ...
 */
uint32_t acpi_count_enabled_logical_processors(void) {
  ...
  if (__improbable(rsdp_physaddr == 0)) {
    DBG("acpi_count_enabled_logical_processors: Could not get RSDP physaddr "
        "from EFI.\n");
    return 0;
  }
  ...
  next_apic_entryp =
      (APIC_HEADER *)((uintptr_t)madtp + sizeof(MULTIPLE_APIC_TABLE));
  ...
  while ((void *)next_apic_entryp < end_ptr) {
    ...
    next_apic_entryp =
        (APIC_HEADER *)((uintptr_t)next_apic_entryp + next_apic_entryp->Length);
  }
