...
#define MSR_P5_CESR_MASK                                                       \
  (MSR_P5_CESR_PC | MSR_P5_CESR_CC | MSR_P5_CESR_ES) /* Mask Counter */
...
/* Legacy name for Hypervisor.  Remove once it has updated.
 * rdar://85833887&85613709 */
...
static inline void set_cr3_composed(uintptr_t base, uint16_t pcid,
                                    uint64_t preserve) {
...
static inline void clear_ts(void) { __asm__ volatile("clts"); }
...
static inline void clac(void) { __asm__ volatile("clac"); }

static inline void stac(void) { __asm__ volatile("stac"); }
...
#define rdtsc(lo, hi) __asm__ volatile("lfence; rdtsc" : "=a"(lo), "=d"(hi))

#define rdtsc_nofence(lo, hi) __asm__ volatile("rdtsc" : "=a"(lo), "=d"(hi))
...
struct x86_lbr_record {
  /*
   * Note that some CPUs convey extra info in the upper bits of the from/to
   * fields, whereas others convey that information in the LBR_INFO companion
   * MSRs. The proper info will be extracted based on the CPU family detected at
   * runtime when LBR thread state is requested.
   */
  uint64_t from_rip;
  uint64_t to_rip;
  uint64_t info;
...
static inline uint64_t rdtsc64_nofence(void) {
  uint64_t lo, hi;
  rdtsc_nofence(lo, hi);
  return ((hi) << 32) | (lo);
}
...
#define MSR_IA32_ARCH_CAPABILITIES_IFU_NO                                      \
  (1ULL << 6) /* This CPU is not susceptible to the instruction-fetch erratum  \
               */
#define MSR_IA32_ARCH_CAPABILITIES_TSX_CTRL                                    \
  (1ULL << 7) /* This CPU supports the TSX_CTRL MSR */
#define MSR_IA32_ARCH_CAPABILITIES_TAA_NO                                      \
  (1ULL << 8) /* This CPU is not susceptible to TAA */
...
#define LBR_SELECT_CPL_EQ_0                                                    \
  (1ULL) /* R/W When set, do not capture branches ending in ring 0 */
#define LBR_SELECT_CPL_NEQ_0                                                   \
  (1ULL << 1) /* R/W When set, do not capture branches ending in ring >0 */
#define LBR_SELECT_JCC                                                         \
  (1ULL << 2) /* R/W When set, do not capture conditional branches */
#define LBR_SELECT_NEAR_REL_CALL                                               \
  (1ULL << 3) /* R/W When set, do not capture near relative calls */
#define LBR_SELECT_NEAR_IND_CALL                                               \
  (1ULL << 4) /* R/W When set, do not capture near indirect calls */
#define LBR_SELECT_NEAR_RET                                                    \
  (1ULL << 5) /* R/W When set, do not capture near returns */
#define LBR_SELECT_NEAR_IND_JMP                                                \
  (1ULL << 6) /* R/W When set, do not capture near indirect jumps except near  \
                 indirect calls and near returns */
#define LBR_SELECT_NEAR_REL_JMP                                                \
  (1ULL << 7) /* R/W When set, do not capture near relative jumps except near  \
                 relative calls. */
#define LBR_SELECT_FAR_BRANCH                                                  \
  (1ULL << 8) /* R/W When set, do not capture far branches */
#define LBR_SELECT_HSW_EN_CALLSTACK1                                           \
  (1ULL << 9) /* Enable LBR stack to use LIFO filtering to capture Call stack  \
                 profile */
...
#define LBR_TYPE_MISPRED_FROMRIP(from_rip)                                     \
  (((from_rip) & 0xFFFFFFFFFFFFULL) |                                          \
   (((from_rip) & (1ULL << 47)) ? 0xFFFF000000000000ULL : 0))
#define LBR_TYPE_MISPRED_MISPREDICT(from_rip)                                  \
  (((from_rip) & MSR_IA32_LBRINFO_MISPREDICT) ? 1 : 0)

#define LBR_TYPE_TSXINFO_FROMRIP(from_rip) (LBR_TYPE_MISPRED_FROMRIP(from_rip))
#define LBR_TYPE_TSXINFO_MISPREDICT(from_rip)                                  \
  (((from_rip) & MSR_IA32_LBRINFO_MISPREDICT) ? 1 : 0)
#define LBR_TYPE_TSXINFO_TSX_ABORT(from_rip)                                   \
  (((from_rip) & MSR_IA32_LBRINFO_TSX_ABORT) ? 1 : 0)
#define LBR_TYPE_TSXINFO_IN_TSX(from_rip)                                      \
  (((from_rip) & MSR_IA32_LBRINFO_IN_TSX) ? 1 : 0)

#define LBR_TYPE_EIP_WITH_LBRINFO_MISPREDICT(lbrinfo)                          \
  LBR_TYPE_TSXINFO_MISPREDICT(lbrinfo)
#define LBR_TYPE_EIP_WITH_LBRINFO_TSX_ABORT(lbrinfo)                           \
  LBR_TYPE_TSXINFO_TSX_ABORT(lbrinfo)
#define LBR_TYPE_EIP_WITH_LBRINFO_IN_TSX(lbrinfo)                              \
  LBR_TYPE_TSXINFO_IN_TSX(lbrinfo)
