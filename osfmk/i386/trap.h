...
#define TRAP_NAMES                                                             \
  "divide error", "debug trap", "NMI", "breakpoint", "overflow",               \
      "bounds check", "invalid opcode", "no coprocessor", "double fault",      \
...
ml_trap(unsigned int code) {
  __asm__ volatile("ud1l %0(%%eax), %%eax"
                   :
                   : "p"((void *)((unsigned long long)code)));
}
