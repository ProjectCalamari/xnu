 ...
 */
static void panic_trap(x86_saved_state64_t *saved_state, uint16_t comment,
                       const char *trapreason, uint32_t pl,
                       kern_return_t fault_result) __dead2;
static void set_recovery_ip(x86_saved_state64_t *saved_state, vm_offset_t ip);
#if DEVELOPMENT || DEBUG
static __attribute__((noinline)) void
copy_instruction_stream(thread_t thread, uint64_t rip, int trap_code,
...
__attribute__((noreturn)) void thread_syscall_return(kern_return_t ret) {
  ...
  if (thread_is_64bit_addr(thr_act)) {
    ...
    if (kdebug_enable && is_mach) {
      ...
      KERNEL_DEBUG_CONSTANT_IST(
          KDEBUG_TRACE, MACHDBG_CODE(DBG_MACH_EXCP_SC, code) | DBG_FUNC_END,
          ret, 0, 0, 0, 0);
    }
    regs->rax = ret;
#if DEBUG
    if (is_mach) {
      DEBUG_KPRINT_SYSCALL_MACH("thread_syscall_return: 64-bit mach ret=%u\n",
                                ret);
    } else {
      DEBUG_KPRINT_SYSCALL_UNIX("thread_syscall_return: 64-bit unix ret=%u\n",
                                ret);
    }
#endif
  } else {
...
#if DEBUG
    if (is_mach) {
      DEBUG_KPRINT_SYSCALL_MACH("thread_syscall_return: 32-bit mach ret=%u\n",
                                ret);
    } else {
      DEBUG_KPRINT_SYSCALL_UNIX("thread_syscall_return: 32-bit unix ret=%u\n",
                                ret);
    }
#endif
  }
  ...
  kern_allocation_name_t prior __assert_only =
      thread_get_kernel_state(thr_act)->allocation_name;
...
void kprint_state(x86_saved_state64_t *saved_state) {
...
void interrupt_latency_tracker_setup(void) {
  ...
  if (PE_parse_boot_argn("interrupt_latency_cap_us", &ilat_cap_us,
                         sizeof(ilat_cap_us))) {
  ...
  PE_parse_boot_argn("-interrupt_latency_assert_enable", &ilat_assert,
                     sizeof(ilat_assert));
}
...
void interrupt_populate_latency_stats(char *buf, unsigned bufsize) {
  ...
  if (tcpu < real_ncpus) {
    snprintf(buf, bufsize, "0x%x 0x%x 0x%llx", tcpu,
             cpu_data_ptr[tcpu]->cpu_max_observed_int_latency_vector,
             cpu_data_ptr[tcpu]->cpu_max_observed_int_latency);
  }
}
 ...
 */
void interrupt(x86_saved_state_t *state) {
  uint64_t rip;
  uint64_t rsp;
  int interrupt_num;
...
#if DEVELOPMENT || DEBUG
  uint64_t frameptr =
      is_saved_state64(state) ? state64->rbp : saved_state32(state)->ebp;
  uint32_t traptrace_index =
      traptrace_start(interrupt_num, rip, mach_absolute_time(), frameptr);
#endif

  if (cpu_data_ptr[cnum]->lcpu.package->num_idle ==
      topoParms.nLThreadsPerPackage) {
    cpu_data_ptr[cnum]->cpu_hwIntpexits[interrupt_num]++;
  }

  if (interrupt_num ==
      (LAPIC_DEFAULT_INTERRUPT_BASE + LAPIC_INTERPROCESSOR_INTERRUPT)) {
    itype = DBG_INTR_TYPE_IPI;
  } else if (interrupt_num ==
             (LAPIC_DEFAULT_INTERRUPT_BASE + LAPIC_TIMER_INTERRUPT)) {
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, MACHDBG_CODE(DBG_MACH_EXCP_INTR, 0) | DBG_FUNC_START,
      interrupt_num, (user_mode ? rip : VM_KERNEL_UNSLIDE(rip)), user_mode,
      itype, 0);
  ...
  if (!handled) {
    if (interrupt_num ==
        (LAPIC_DEFAULT_INTERRUPT_BASE + LAPIC_CMCI_INTERRUPT)) {
      /*
  ...
  if (__improbable(get_preemption_level() != ipl)) {
    panic("Preemption level altered by interrupt vector 0x%x: initial 0x%x, "
          "final: 0x%x",
...
#define TCOAL_ILAT_THRESHOLD (30000ULL)
    ...
    if ((int_latency < TCOAL_ILAT_THRESHOLD) &&
      ...
      if ((ctime >= esdeadline) && (ctime < ehdeadline)) {
        ...
        TCOAL_DEBUG(0x88880000 | DBG_FUNC_START, ctime, esdeadline, ehdeadline,
                    interrupt_coalesced_timers, 0);
        rtclock_intr(state);
        TCOAL_DEBUG(0x88880000 | DBG_FUNC_END, ctime, esdeadline,
                    interrupt_coalesced_timers, 0, 0);
      } else {
        TCOAL_DEBUG(0x77770000, ctime,
                    cdp->rtclock_timer.queue.earliest_soft_deadline,
                    cdp->rtclock_timer.deadline, interrupt_coalesced_timers, 0);
      }
    }

    if (__improbable(ilat_assert && (int_latency > interrupt_latency_cap) &&
                     !machine_timeout_suspended())) {
      panic(
          "Interrupt vector 0x%x exceeded interrupt latency threshold, 0x%llx "
          "absolute time delta, prior signals: 0x%x, current signals: 0x%x",
          interrupt_num, int_latency, cdp->cpu_prior_signals, cdp->cpu_signals);
    }
  ...
  if (!user_mode) {
    uint64_t depth = cdp->cpu_kernel_stack +
                     ...
                     sizeof(struct i386_exception_link *) - rsp;
    if (__improbable(depth > kernel_stack_depth_max)) {
      kernel_stack_depth_max = (vm_offset_t)depth;
      KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_STACK_DEPTH),
...
static inline void reset_dr7(void) {
  long dr7 =
...
static const char *xnu_hard_trap_handle_breakpoint(void *tstate,
  ...
  case XNU_HARD_TRAP_ASSERT_FAILURE:
    ...
    panic_assert_format(pr->buf, sizeof(pr->buf),
                        (struct mach_assert_hdr *)state->rax, state->r10,
                        state->r11);
    return pr->buf;
...
static bool handle_kernel_breakpoint(x86_saved_state64_t *state,
                                     const char **reason,
  ...
  uint8_t inst_buf[8];
  uint32_t prefix16 =
      0x80B90F67; /* Encoding prefix for ud1 <16-bit code>(%eax), %eax */
  uint32_t prefix8 =
      0x40B90F67; /* Encoding prefix for ud1 <8-bit code>(%eax), %eax */
  bool found_prefix8 = false;

  vm_size_t sz =
      ml_nofault_copy(state->isf.rip, (vm_offset_t)inst_buf, sizeof(inst_buf));
...
static struct recovery const *find_recovery_entry(vm_offset_t kern_ip) {
  for (struct recovery const *rp = recover_table; rp < recover_table_end;
       rp++) {
...
void kernel_trap(x86_saved_state_t *state, uintptr_t *lo_spp) {
...
#if DEVELOPMENT || DEBUG
  uint32_t traptrace_index =
      traptrace_start(type, kern_ip, mach_absolute_time(), saved_state->rbp);
#endif
  ...
  if (__improbable(T_PREEMPT == type)) {
    ...
    KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                              (MACHDBG_CODE(DBG_MACH_EXCP_KTRAP_x86, type)) |
                                  DBG_FUNC_NONE,
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                            (MACHDBG_CODE(DBG_MACH_EXCP_KTRAP_x86, type)) |
                                DBG_FUNC_NONE,
                            (unsigned)(kd_vaddr >> 32), (unsigned)kd_vaddr,
                            is_user, VM_KERNEL_UNSLIDE(kern_ip), 0);
  ...
  if (T_PAGE_FAULT == type) {
    ...
    if (__probable((thread != THREAD_NULL) && (thread->map != kernel_map) &&
       ...
       */
      if (__improbable(code & T_PF_PROT && pmap_smap_enabled &&
       ...
       */
      if (no_shared_cr3 && (thread->machine.specFlags & CopyIOActive) &&
  ...
  case T_DEBUG:
    /*
     * Re-enable LBR tracing for core/panic files if necessary. i386_lbr_enable
     * confirms LBR should be re-enabled.
     */
    i386_lbr_enable();
  ...
  case T_PAGE_FAULT:
    ...
    if (thread != THREAD_NULL &&
      ...
      if (dtrace_tally_fault(
              vaddr)) { /* Should a fault under dtrace be ignored? */
        /*
     ...
     * kernel page fault panic anyway.  In these cases, skip calling vm_fault()
     * to avoid masking the real kernel panic with a failed VM locking
     * assertion.
     */
    if (__improbable(!(intr || startup_phase < STARTUP_SUB_EARLY_BOOT ||
    ...
    // don't want to throttle kernel faults for copyio faults. The presence of a
    // recovery entry is used as a proxy for being in copyio code.
    ...
    fault_result = result = vm_fault(
        map, vaddr, prot, FALSE, VM_KERN_MEMORY_NONE, THREAD_UNINT, NULL, 0);
...
static void panic_trap(x86_saved_state64_t *regs, uint16_t trapcomment,
                       const char *trapreason, uint32_t pl,
  ...
  if ((regs->isf.trapno == T_PAGE_FAULT) &&
      (regs->isf.err == (T_PF_PROT | T_PF_EXECUTE)) &&
      (regs->isf.rip == regs->cr2)) {
  ...
  } else if (pmap_smap_enabled && regs->isf.trapno == T_PAGE_FAULT &&
  ...
  if (trapreason == NULL) {
    ...
    if (trapcomment == 0) {
      snprintf(trapbuf, sizeof(trapbuf), "type = %d=%s, ", regs->isf.trapno,
               traptype);
    } else {
      snprintf(trapbuf, sizeof(trapbuf), "type = %d=%s #%#04hx, ",
...
#undef panic
  panic("%s at 0x%016llx, %sregisters:\n"
        ...
        "PL: %d, VF: %d\n",
        trapreason, regs->isf.rip, trapbuf, cr0, cr2, cr3, cr4, regs->rax,
        regs->rbx, regs->rcx, regs->rdx, regs->isf.rsp, regs->rbp, regs->rsi,
        regs->rdi, regs->r8, regs->r9, regs->r10, regs->r11, regs->r12,
        regs->r13, regs->r14, regs->r15, regs->isf.rflags, regs->isf.rip,
        regs->isf.cs & 0xFFFF, regs->isf.ss & 0xFFFF, regs->cr2, regs->isf.err,
        regs->isf.cpu, virtualized ? " VMM" : "",
        potential_kernel_NX_fault ? " Kernel NX fault" : "",
        potential_smep_fault ? " SMEP/User NX fault" : "",
        potential_smap_fault ? " SMAP fault" : "", pl, fault_result);
}
...
void user_trap(x86_saved_state_t *saved_state) {
...
#if DEVELOPMENT || DEBUG
    traptrace_index =
        traptrace_start(type, rip, mach_absolute_time(), regs->rbp);
#endif
  } else {
    x86_saved_state32_t *regs;
...
#if DEVELOPMENT || DEBUG
    traptrace_index =
        traptrace_start(type, rip, mach_absolute_time(), regs->ebp);
#endif
  }
   ...
   * before enabling interrupts (the assumption is that we have not otherwise
   * faulted or trapped since the original cache line stores).  If the saved
   * code is not valid, we'll catch it below when we process the copyin() for
   * unhandled faults.
   */
  if (thread->machine.insn_copy_optout == false &&
      (type == T_PAGE_FAULT || type == T_INVALID_OPCODE ||
       type == T_GENERAL_PROTECTION)) {
#define CACHELINE_SIZE 64
    THREAD_TO_PCB(thread)->insn_cacheline[CACHELINE_SIZE] =
        (uint8_t)(rip & (CACHELINE_SIZE - 1));
  ...
  if (type == T_DEBUG) {
    ...
    /* [Re]Enable LBRs *BEFORE* enabling interrupts to ensure we hit the right
     * CPU */
    i386_lbr_enable();
  }
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                            (MACHDBG_CODE(DBG_MACH_EXCP_UTRAP_x86, type)) |
                                DBG_FUNC_NONE,
  ...
  DEBUG_KPRINT_SYSCALL_MASK(
      1, "user_trap: type=0x%x(%s) err=0x%x cr2=%p rip=%p\n", type,
      trap_type[type], err, (void *)(long)vaddr, (void *)(long)rip);
...
#endif
#endif
    kret = vm_fault(thread->map, vaddr, prot, FALSE, VM_KERN_MEMORY_NONE,
...
#endif
      if (fsig != fsig2) {
        panic("FP/SIMD state hash mismatch across fault thread: %p 0x%x->0x%x",
              thread, fsig, fsig2);
      }
    } else {
#if DEBUG
  ...
  if (exc != 0) {
    ...
    if (last_branch_enabled_modes == LBR_ENABLED_USERMODE) {
      ...
      /*
       * This is a bit racy (it's possible for this thread to migrate to another
       * CPU, then migrate back, but that seems rather rare in practice), but
       * good enough to ensure the LBRs are saved before proceeding with
       * exception/signal dispatch.
       */
    ...
    /*
     * Do not try to copyin from the instruction stream if the page fault was
     * due to an access to rip and was unhandled. Do not deal with cases when
     * %cs != USER[64]_CS And of course there's no need to copy the instruction
     * stream if the boot-arg was set to 0.
     */
    if (thread->machine.insn_copy_optout == false && insn_copyin_count > 0 &&
        (cs == USER64_CS || cs == USER_CS) &&
        (type != T_PAGE_FAULT || vaddr != rip)) {
#if DEVELOPMENT || DEBUG
...
/*
 * Copyin up to x86_INSTRUCTION_STATE_MAX_INSN_BYTES bytes from the page that
 * includes `rip`, ensuring that we stay on the same page, clipping the start or
 * end, as needed. Add the clipped amount back at the start or end, depending on
 * where it fits. Consult the variable populated by the boot-arg `insn_capcnt'
 */
static __attribute__((noinline)) void
...
#pragma clang diagnostic pop
  ...
  if (start_addr < rip_page) {
    ...
    start_addr -=
        (end_addr -
         (rip_page + (~pagemask + 1))); /* Adjust start address backward */
    /* Adjust instruction offset due to start address change */
    insn_offset =
        (insn_copyin_count / 2) + (end_addr - (rip_page + (~pagemask + 1)));
    end_addr =
        rip_page +
        (~pagemask + 1); /* clip to the start of the next page (non-inclusive */
  } else {
    insn_offset = insn_copyin_count / 2;
  }

  disable_preemption(); /* Prevent copyin from faulting in the instruction
                           stream */
  if (
#if DEVELOPMENT || DEBUG
...
#endif
      ((end_addr > start_addr) &&
       (copyin_err = copyin(start_addr, stack_buffer, end_addr - start_addr)) ==
           0)) {
    enable_preemption();
...
#if DEVELOPMENT || DEBUG
      /* Now try to validate the cacheline we read at early-fault time matches
       * the code copied in. Before we do that, we have to make sure the buffer
       * contains a valid cacheline by looking for the 2 sentinel values written
       * in the event the cacheline could not be copied.
       */
#define CACHELINE_DATA_NOT_PRESENT 0xdeadc0debeefcafeULL
#define CACHELINE_MASK (CACHELINE_SIZE - 1)

      if (inspect_cacheline &&
          (*(uint64_t *)(uintptr_t)&pcb->insn_cacheline[0] !=
               CACHELINE_DATA_NOT_PRESENT &&
           *(uint64_t *)(uintptr_t)&pcb->insn_cacheline[8] !=
               CACHELINE_DATA_NOT_PRESENT)) {
        /*
        ...
        if (__improbable((rip & CACHELINE_MASK) > insn_offset)) {
          printf("thread %p code cacheline @ %p clipped wrt copied-in code "
                 "(offset %d)\n",
                 thread, (void *)(rip & ~CACHELINE_MASK),
                 (int)(rip & CACHELINE_MASK));
        } else if (bcmp(&pcb->insn_state
                             ->insn_bytes[insn_offset - (rip & CACHELINE_MASK)],
...
#error cacheline size mismatch
          ...
          printf("thread %p code cacheline @ %p mismatches with copied-in code "
                 "[trap 0x%x]\n",
                 thread, (void *)(rip & ~CACHELINE_MASK), trap_code);
          for (int i = 0; i < 8; i++) {
            printf("\t[%d] cl=0x%08llx vs. ci=0x%08llx\n", i,
                   *(uint64_t *)(uintptr_t)&pcb->insn_cacheline[i * 8],
                   *(uint64_t *)(uintptr_t)&pcb->insn_state
                        ->insn_bytes[(i * 8) + insn_offset -
                                     (rip & CACHELINE_MASK)]);
          }
        ...
        printf("thread %p could not capture code cacheline at fault IP %p "
               "[offset %d]\n",
               (void *)thread, (void *)rip,
               (int)(insn_offset - (rip & CACHELINE_MASK)));
        pcb->insn_state->out_of_synch = 0;
      }
#else
      pcb->insn_state->out_of_synch = 0;
      ...
      if (panic_on_trap_procname[0] != 0) {
        ...
        if (get_bsdtask_info(task) != NULL) {
          ...
          if (strcasecmp(panic_on_trap_procname, procnamebuf) == 0 &&
            ...
            panic("Panic requested on trap type 0x%x for process `%s'",
                  trap_code, panic_on_trap_procname);
            /*NORETURN*/
          }
        }
      }
 ...
 * emulator.
 */
void i386_exception(int exc, mach_exception_code_t code,
                    mach_exception_subcode_t subcode) {
  mach_exception_data_type_t codes[EXCEPTION_CODE_MAX];

  DEBUG_KPRINT_SYSCALL_MACH(
      "i386_exception: exc=%d code=0x%llx subcode=0x%llx\n", exc, code,
      subcode);
...
void sync_iss_to_iks(x86_saved_state_t *saved_state) {
 ...
 * or user space.
 */
void sync_iss_to_iks_unconditionally(__unused x86_saved_state_t *saved_state) {
...
void thread_exception_return(void) {
  ...
  if (thread_is_64bit_addr(thread) != task_has_64Bit_addr(task)) {
    panic("Task/thread bitness mismatch %p %p, task: %d, thread: %d", thread,
          task, thread_is_64bit_addr(thread), task_has_64Bit_addr(task));
  }

  if (thread_is_64bit_addr(thread)) {
    if ((gdt_desc_p(USER64_CS)->access & ACC_PL_U) == 0) {
      panic("64-GDT mismatch %p, descriptor: %p", thread,
