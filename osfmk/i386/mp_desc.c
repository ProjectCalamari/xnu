...
#define TRAP(n, name) [n] = {(uintptr_t)&name, KERNEL64_CS, 0, K_INTR_GATE, 0},

#define TRAP_ERR TRAP
#define TRAP_SPC TRAP

#define TRAP_IST1(n, name)                                                     \
  [n] = {(uintptr_t)&name, KERNEL64_CS, 1, K_INTR_GATE, 0},

#define TRAP_IST2(n, name)                                                     \
  [n] = {(uintptr_t)&name, KERNEL64_CS, 2, K_INTR_GATE, 0},

#define INTERRUPT(n)                                                           \
  [n] = {(uintptr_t)&_intr_##n, KERNEL64_CS, 0, K_INTR_GATE, 0},

#define USER_TRAP(n, name)                                                     \
  [n] = {(uintptr_t)&name, KERNEL64_CS, 0, U_INTR_GATE, 0},
 ...
 */
cpu_data_t cpshadows[MAX_CPUS] __attribute__((aligned(64)))
__attribute__((section("__HIB, __desc")));
...
cpu_desc_table64_t scdtables[MAX_CPUS] __attribute__((aligned(64)))
__attribute__((section("__HIB, __desc")));
cpu_fault_stack_t scfstks[MAX_CPUS] __attribute__((aligned(64)))
__attribute__((section("__HIB, __desc")));
 ...
 */
void fix_desc(void *d, int num_desc) {
...
void fix_desc64(void *descp, int count) {
...
void cpu_desc_init(cpu_data_t *cdp) {
  ...
  if (cdp == cpu_data_master) {
    ...
    *(struct fake_descriptor64 *)&master_gdt[sel_idx(KERNEL_TSS)] =
    ...
    master_ktss64.ist1 =
        (uintptr_t)low_eintstack - sizeof(x86_64_intr_stack_frame_t);
    ...
    bcopy((char *)&master_ktss64, (char *)&cdt->ktss,
 ...
 */
void cpu_syscall_init(cpu_data_t *cdp) {
#pragma unused(cdp)
  ...
  wrmsr64(MSR_IA32_STAR,
          (((uint64_t)USER_CS) << 48) | (((uint64_t)KERNEL64_CS) << 32));
  /*
...
__startup_func static void cpu_data_startup_init(void) {
  int flags = KMA_GUARD_FIRST | KMA_GUARD_LAST | KMA_PERMANENT | KMA_ZERO |
              KMA_KOBJECT | KMA_NOFAIL;
  ...
  kmem_alloc(kernel_map, &percpu_base.start, round_page(size) + ptoa(2), flags,
             VM_KERN_MEMORY_CPU);
...
cpu_data_t *cpu_data_alloc(boolean_t is_boot_cpu) {
  ...
  if (do_ldt_alloc) {
     ...
     * Allocate LDT
     */
    vm_offset_t ldtalloc = 0,
                ldtallocsz = round_page_64(
                    MAX_CPUS * sizeof(struct real_descriptor) * LDTSZ);

    kmem_alloc(kernel_map, (vm_offset_t *)&ldtalloc, ldtallocsz,
               KMA_NOFAIL | KMA_KOBJECT, VM_KERN_MEMORY_CPU);
  ...
  kprintf("cpu_data_alloc(%d) %p desc_table: %p "
          ...
          (long)(cdp->cpu_int_stack_top - INTSTACK_SIZE),
          (long)(cdp->cpu_int_stack_top));
...
boolean_t valid_user_code_selector(uint16_t selector) {
  sel_t sel = selector_to_sel(selector);
...
boolean_t valid_user_stack_selector(uint16_t selector) {
  sel_t sel = selector_to_sel(selector);
...
boolean_t valid_user_segment_selectors(uint16_t cs, uint16_t ss, uint16_t ds,
                                       uint16_t es, uint16_t fs, uint16_t gs) {
  return valid_user_code_selector(cs) && valid_user_stack_selector(ss) &&
         valid_user_data_selector(ds) && valid_user_data_selector(es) &&
         valid_user_data_selector(fs) && valid_user_data_selector(gs);
}
 ...
 */
void cpu_data_realloc(void) {
  vm_offset_t istk;
  cpu_data_t *cdp;
  boolean_t istate;

  kmem_alloc(kernel_map, &istk, INTSTACK_SIZE + ptoa(2),
             KMA_NOFAIL | KMA_PERMANENT | KMA_ZERO | KMA_GUARD_FIRST |
                 KMA_GUARD_LAST | KMA_KOBJECT,
             VM_KERN_MEMORY_CPU);
  ...
  master_ktss64.ist1 =
      DBLMAP((uintptr_t)cfstk - sizeof(x86_64_intr_stack_frame_t));
  wrmsr64(MSR_IA32_GS_BASE, (uintptr_t)cdp);
  wrmsr64(MSR_IA32_KERNEL_GS_BASE, (uintptr_t)cdp);
