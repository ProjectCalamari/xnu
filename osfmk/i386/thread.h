...
#define OnProc 0x1
#define CopyIOActive                                                           \
  0x2 /* Checked to ensure DTrace actions do not re-enter copyio(). */
...
#define MTHR_RSBST 2
  int insn_state_copyin_failure_errorcode; /* If insn_state is 0, this may hold
                                              the reason */
...
#define user_pc(Thr)                                                           \
  (is_saved_state32(USER_STATE(Thr)) ? USER_REGS32(Thr)->eip                   \
...
#define STACK_IKS(stack)                                                       \
  (&(((struct thread_kernel_state *)((stack) + kernel_stack_size)) - 1)        \
        ->machine)
 ...
 *
 */
static inline vm_offset_t current_kernel_stack_depth(void) {
  ...
  stack_depth = current_cpu_datap()->cpu_kernel_stack +
                sizeof(struct thread_kernel_state) - stack_ptr;

  if (stack_depth >= kernel_stack_size) {
    panic("kernel stack overflow; stack base: 0x%16lx, "
          "stack top: 0x%016lx, stack depth: 0x%016lx, "
          "depth limit: 0x%016lx",
          current_cpu_datap()->cpu_kernel_stack, stack_ptr, stack_depth,
          kernel_stack_size);
  }
