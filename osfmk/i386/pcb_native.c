...
static const struct cpu_lbrs
    nhm_cpu_lbrs = {16 /* LBR count */,
...
void i386_lbr_disable(void) {
  /* Enable LBRs */
  wrmsr64(MSR_IA32_DEBUGCTLMSR,
          rdmsr64(MSR_IA32_DEBUGCTLMSR) & ~DEBUGCTL_LBR_ENA);
}

/*
 * Disable ASAN for i386_lbr_enable and i386_lbr_init, otherwise we get a KASAN
 * panic because the shadow map is not been initialized when these functions are
 * called in early boot.
 */
void __attribute__((no_sanitize("address"))) i386_lbr_enable(void) {
  /* last_branch_kmode_only_enabled controls LBR data collection for core files
   * and paniclogs */
...
i386_lbr_init(i386_cpu_info_t *info_p, bool is_master) {
  ...
  if (last_branch_enabled_modes == LBR_ENABLED_ALLMODES) {
    panic(
        "Collecting LBR data from both user and kernel mode is not supported.");
  }
  ...
  if (is_master) {
    ...
    if (cpu_lbr_type < PERFCAP_LBR_TYPE_MISPRED ||
        cpu_lbr_type > PERFCAP_LBR_TYPE_EIP_WITH_LBRINFO) {
  ...
  /* Configure LBR_SELECT for CPL > 0 records only or CPL = 0 for use in panic
   * logs and core files */
  switch (last_branch_enabled_modes) {
  case LBR_ENABLED_USERMODE:
...
static uint64_t lbr_mode_based_filter(uint64_t record,
                                      __unused boolean_t from_userspace) {
...
#define LBR_SENTINEL_USER_MODE (0x757365726C616E64ULL /* "userland" */)
  ...
  case LBR_ENABLED_USERMODE:
    filtered_record =
        (record > VM_MAX_USER_PAGE_ADDRESS) ? LBR_SENTINEL_KERNEL_MODE : record;
    break;
  case LBR_ENABLED_KERNELMODE:
    /* For internal builds don't filter out userspace addresses from panic logs
     * and core files. */
#if DEBUG || DEVELOPMENT
    filtered_record = record;
#else
    /* If coming from user space use the correct filter in release builds
     * When LBRs are enabled for kernel mode and user space requests LBR data:
     * remove kernel addresses "
     * " and kernel mode requests LBR data: remove usermode addresses
     */
    if (from_userspace) {
      filtered_record = (record > VM_MAX_USER_PAGE_ADDRESS)
                            ? LBR_SENTINEL_KERNEL_MODE
                            : record;
    } else {
      filtered_record =
          (VM_KERNEL_ADDRESS(record)) ? record : LBR_SENTINEL_USER_MODE;
    }
#endif
    break;
  case LBR_ENABLED_ALLMODES:
...
static int i386_lbr_native_state_to_mach_thread_state(
    pcb_t pcb, last_branch_state_t *machlbrp, boolean_t from_userspace) {
  int last_entry;
  int i, j, lbr_tos;
  ...
  case PERFCAP_LBR_TYPE_MISPRED: /* NHM */
    ...
    for (j = 0, i = lbr_tos;;
         (i = (i == 0) ? (cpu_lbr_setp->lbr_count - 1) : (i - 1)), j++) {
      ...
      machlbrp->lbrs[j].from_ip =
          lbr_mode_based_filter(from_rip, from_userspace);
      machlbrp->lbrs[j].mispredict =
          LBR_TYPE_MISPRED_MISPREDICT(pcb->lbrs.lbrs[i].from_rip);
      machlbrp->lbrs[j].tsx_abort = machlbrp->lbrs[j].in_tsx =
          0; /* Not Supported */
      if (i == last_entry) {
        break;
      }
  ...
  case PERFCAP_LBR_TYPE_TSXINFO: /* HSW/BDW */
    ...
    for (j = 0, i = lbr_tos;;
      ...
      machlbrp->lbrs[j].from_ip =
          lbr_mode_based_filter(from_rip, from_userspace);
      machlbrp->lbrs[j].mispredict =
          LBR_TYPE_TSXINFO_MISPREDICT(pcb->lbrs.lbrs[i].from_rip);
      if (cpuid_tsx_supported) {
        machlbrp->lbrs[j].tsx_abort =
            LBR_TYPE_TSXINFO_TSX_ABORT(pcb->lbrs.lbrs[i].from_rip);
        machlbrp->lbrs[j].in_tsx =
            LBR_TYPE_TSXINFO_IN_TSX(pcb->lbrs.lbrs[i].from_rip);
      } else {
  ...
  case PERFCAP_LBR_TYPE_EIP_WITH_LBRINFO: /* SKL+ */
    ...
    for (j = 0, i = lbr_tos;;
      ...
      machlbrp->lbrs[j].to_ip = lbr_mode_based_filter(to_rip, from_userspace);
      machlbrp->lbrs[j].mispredict =
          LBR_TYPE_EIP_WITH_LBRINFO_MISPREDICT(pcb->lbrs.lbrs[i].info);
      machlbrp->lbrs[j].tsx_abort =
          LBR_TYPE_EIP_WITH_LBRINFO_TSX_ABORT(pcb->lbrs.lbrs[i].info);
      machlbrp->lbrs[j].in_tsx =
          LBR_TYPE_EIP_WITH_LBRINFO_IN_TSX(pcb->lbrs.lbrs[i].info);
      machlbrp->lbrs[j].cycle_count =
          LBR_TYPE_EIP_WITH_LBRINFO_CYC_COUNT(pcb->lbrs.lbrs[i].info);
      if (i == last_entry) {
        break;
      }
    }
  ...
  default:
#if DEBUG || DEVELOPMENT
    /* This should be impossible, based on the filtering we do in
     * i386_lbr_init() */
...
int i386_filtered_lbr_state_to_mach_thread_state(thread_t thr_act,
                                                 last_branch_state_t *machlbrp,
                                                 boolean_t from_userspace) {
  boolean_t istate;
  ...
  return i386_lbr_native_state_to_mach_thread_state(THREAD_TO_PCB(thr_act),
                                                    machlbrp, from_userspace);
}
...
void i386_lbr_synch(thread_t thr) {
  ...
  if (cpu_lbr_setp->msr_lbrs[0].msr_info != 0) {
    for (i = 0; i < cpu_lbr_setp->lbr_count; i++) {
      old_pcb->lbrs.lbrs[i].from_rip =
          rdmsr64(cpu_lbr_setp->msr_lbrs[i].msr_from);
...
static void i386_switch_lbrs(thread_t old, thread_t new) {
  pcb_t new_pcb;
  int i;
  ...
  if (restore_new) {
    ...
    if (cpu_lbr_setp->msr_lbrs[0].msr_info != 0) {
      ...
      if (lbr_need_tsx_workaround) {
        for (i = 0; i < cpu_lbr_setp->lbr_count; i++) {
          /*
           * If TSX has been disabled, the hardware expects those two bits to be
           * sign extensions of bit 47 (even though it didn't return them that
           * way via the rdmsr!)
           */
#define BIT_47 (1ULL << 47)
          wrmsr64(cpu_lbr_setp->msr_lbrs[i].msr_from,
                  new_pcb->lbrs.lbrs[i].from_rip |
                      ((new_pcb->lbrs.lbrs[i].from_rip & BIT_47)
                           ? 0x6000000000000000ULL
                           : 0));
          wrmsr64(cpu_lbr_setp->msr_lbrs[i].msr_to,
                  new_pcb->lbrs.lbrs[i].to_rip |
                      ((new_pcb->lbrs.lbrs[i].to_rip & BIT_47)
                           ? 0x6000000000000000ULL
                           : 0));
        }
      } else {
...
void act_machine_switch_pcb(thread_t old, thread_t new) {
  ...
  *current_sstk64() = pcb_stack_top;
  ...
  cdp->cd_estack = cpu_shadowp(cdp->cpu_number)->cd_estack =
      cdp->cpu_desc_index.cdi_sstku;
 ...
 */
void machine_thread_create(thread_t thread, task_t task,
...
void machine_thread_set_insn_copy_optout(thread_t thr) {
