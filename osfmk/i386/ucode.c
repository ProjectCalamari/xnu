...
static void update_microcode(void) {
...
/* Copy in an update */
static int copyin_update(uint64_t inaddr) {
  ...
  error = copyin((user_addr_t)inaddr, (void *)&update_header,
...
static void cpu_apply_microcode(void) {
  /* grab the lock */
...
static void cpu_update(__unused void *arg) {
  cpu_apply_microcode();
 ...
 * by sleeping.
 */
void ucode_update_wake_and_apply_cpu_was() {
  if (global_update) {
...
static void ucode_cpuid_set_info(void) {
  uint64_t saved_xcr0, dest_xcr0;
  int need_xcr0_restore = 0;
  boolean_t intrs_enabled = ml_set_interrupts_enabled(FALSE);

  /*
   * Before we cache the CPUID information, we must configure XCR0 with the
   * maximal set of features to ensure the save area returned in the xsave leaf
   * is correctly-sized.
   *
   * Since we are guaranteed that init_fpu() has already happened, we can use
   * state variables set there that were already predicated on the presence of
   * explicit boot-args enables/disables.
   */
 ...
 *
 */
int ucode_interface(uint64_t addr) {
  int error;
  char arg[16];
