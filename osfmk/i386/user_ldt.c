...
static int i386_set_ldt_impl(uint32_t *retval, uint64_t start_sel,
                             uint64_t descs, uint64_t num_sels);
static int i386_get_ldt_impl(uint32_t *retval, uint64_t start_sel,
                             uint64_t descs, uint64_t num_sels);
...
static int i386_set_ldt_impl(uint32_t *retval, uint64_t start_sel,
  ...
  unsigned int min_selector =
      LDTSZ_MIN; /* do not allow the system selectors to be changed */
  task_t task = current_task();
  unsigned int ldt_count;
  kern_return_t err;

  if (start_sel != LDT_AUTO_ALLOC && (start_sel != 0 || num_sels != 0) &&
  ...
  if (start_sel == 0 && num_sels == 0) {
    ...
    unsigned int end_sel = (unsigned int)begin_sel + (unsigned int)num_sels;
     ...
     */
    if (old_ldt) {
      bcopy(&old_ldt->ldt[0], &new_ldt->ldt[old_ldt->start - begin_sel],
      ...
      if (old_ldt->start + old_ldt->count < start_sel) {
        ...
        bzero(&new_ldt->ldt[num_sels],
              (old_ldt->start - (start_sel + num_sels)) *
                  sizeof(struct real_descriptor));
      }
    }
    ...
    if (descs != 0) {
      ...
      bzero(&new_ldt->ldt[start_sel - begin_sel],
            num_sels * sizeof(struct real_descriptor));
    }
    /*
     ...
     */
    for (i = 0,
        dp = (struct real_descriptor *)&new_ldt->ldt[start_sel - begin_sel];
         i < num_sels; i++, dp++) {
...
static int i386_get_ldt_impl(uint32_t *retval, uint64_t start_sel,
  ...
  if (start_sel < ldt_count) {
    ...
    err = copyout((char *)(current_ldt() + start_sel), descs,
                  copy_sels * sizeof(struct real_descriptor));
  }
...
void user_ldt_free(user_ldt_t user_ldt) {
  kfree_data(user_ldt, USER_LDT_SIZE(user_ldt->count));
}

user_ldt_t user_ldt_copy(user_ldt_t user_ldt) {
  if (user_ldt != NULL) {
...
void user_ldt_set_action(void *arg) {
 ...
 */
void user_ldt_set(thread_t thread) {
  ...
  if (user_ldt != 0) {
    ...
    gdt_desc_p(USER_LDT)->limit_low =
        (uint16_t)((sizeof(struct real_descriptor) *
                    (user_ldt->start + user_ldt->count)) -
                   1);
...
int i386_set_ldt(uint32_t *retval, uint32_t start_sel, uint32_t descs, /* out */
                 uint32_t num_sels) {
...
int i386_set_ldt64(uint32_t *retval, uint64_t start_sel,
                   uint64_t descs, /* out */
                   uint64_t num_sels) {
...
/* For 32-bit processes, called via machdep_syscall() */
int i386_get_ldt(uint32_t *retval, uint32_t start_sel, uint32_t descs, /* out */
                 uint32_t num_sels) {
...
/* For 64-bit processes, called via machdep_syscall64() */
int i386_get_ldt64(uint32_t *retval, uint64_t start_sel,
                   uint64_t descs, /* out */
                   uint64_t num_sels) {
