...
#define DIVISOR_GUARD(denom)                                                   \
  if ((denom) == 0) {                                                          \
    kprintf("%s: %d Zero divisor: " #denom, __FILE__, __LINE__);               \
  }
...
static boolean_t cpu_is_hyperthreaded(void) {
...
static x86_cpu_cache_t *x86_cache_alloc(void) {
...
static void x86_LLC_info(void) {
  ...
  topoParms.nCoresSharingLLC =
      nCPUsSharing / (cpuinfo->thread_count / cpuinfo->core_count);
...
static void initTopoParms(void) {
  i386_cpu_info_t *cpuinfo;
  ...
  topoParms.nPThreadsPerCore =
      cpuinfo->cpuid_logical_per_package / cpuinfo->cpuid_cores_per_package;
  ...
  topoParms.nPDiesPerPackage =
      cpuinfo->cpuid_cores_per_package /
      (topoParms.maxSharingLLC / topoParms.nPThreadsPerCore);
  ...
  topoParms.nLThreadsPerDie =
      topoParms.nLThreadsPerCore * topoParms.nLCoresPerDie;
  topoParms.nPThreadsPerDie =
      topoParms.nPThreadsPerCore * topoParms.nPCoresPerDie;

  /*
   * Compute the number of cores per package.
   */
  topoParms.nLCoresPerPackage =
      topoParms.nLCoresPerDie * topoParms.nLDiesPerPackage;
  topoParms.nPCoresPerPackage =
      topoParms.nPCoresPerDie * topoParms.nPDiesPerPackage;

  /*
   * Compute the number of threads per package.
   */
  topoParms.nLThreadsPerPackage =
      topoParms.nLThreadsPerCore * topoParms.nLCoresPerPackage;
  topoParms.nPThreadsPerPackage =
      topoParms.nPThreadsPerCore * topoParms.nPCoresPerPackage;
...
static void x86_cache_free(x86_cpu_cache_t *cache) {
  num_caches -= 1;
 ...
 */
static x86_cpu_cache_t *x86_cache_list(void) {
...
static x86_cpu_cache_t *x86_match_cache(x86_cpu_cache_t *list,
                                        x86_cpu_cache_t *matcher) {
...
static void x86_lcpu_init(int cpu) {
  cpu_data_t *cpup;
  x86_lcpu_t *lcpu;
  int i;
...
static x86_core_t *x86_core_alloc(int cpu) {
  x86_core_t *core;
  cpu_data_t *cpup;
  ...
  core->flags = X86CORE_FL_PRESENT | X86CORE_FL_READY | X86CORE_FL_HALTED |
                X86CORE_FL_IDLE;

  return core;
}

static void x86_core_free(x86_core_t *core) {
  mp_safe_spin_lock(&x86_topo_lock);
...
static x86_pkg_t *x86_package_find(int cpu) {
  x86_pkg_t *pkg;
  cpu_data_t *cpup;
  uint32_t pkg_num;
...
static x86_die_t *x86_die_find(int cpu) {
  x86_die_t *die;
  x86_pkg_t *pkg;
  cpu_data_t *cpup;
  uint32_t die_num;
...
static x86_core_t *x86_core_find(int cpu) {
  x86_core_t *core;
  x86_die_t *die;
  cpu_data_t *cpup;
  uint32_t core_num;
...
void x86_set_logical_topology(x86_lcpu_t *lcpu, int pnum, int lnum) {
...
static x86_die_t *x86_die_alloc(int cpu) {
  x86_die_t *die;
  cpu_data_t *cpup;
...
static void x86_die_free(x86_die_t *die) {
  mp_safe_spin_lock(&x86_topo_lock);
...
static x86_pkg_t *x86_package_alloc(int cpu) {
  x86_pkg_t *pkg;
  cpu_data_t *cpup;
...
static void x86_package_free(x86_pkg_t *pkg) {
  mp_safe_spin_lock(&x86_topo_lock);
  pkg->next = free_pkgs;
  free_pkgs = pkg;
...
static void x86_cache_add_lcpu(x86_cpu_cache_t *cache, x86_lcpu_t *lcpu) {
  x86_cpu_cache_t *cur_cache;
  int i;
...
static void x86_lcpu_add_caches(x86_lcpu_t *lcpu) {
...
static void x86_core_add_lcpu(x86_core_t *core, x86_lcpu_t *lcpu) {
  assert(core != NULL);
  assert(lcpu != NULL);
...
static void x86_die_add_lcpu(x86_die_t *die, x86_lcpu_t *lcpu) {
  assert(die != NULL);
  assert(lcpu != NULL);
...
static void x86_die_add_core(x86_die_t *die, x86_core_t *core) {
  assert(die != NULL);
  assert(core != NULL);
...
static void x86_package_add_lcpu(x86_pkg_t *pkg, x86_lcpu_t *lcpu) {
  assert(pkg != NULL);
  assert(lcpu != NULL);
...
static void x86_package_add_core(x86_pkg_t *pkg, x86_core_t *core) {
  assert(pkg != NULL);
  assert(core != NULL);
...
static void x86_package_add_die(x86_pkg_t *pkg, x86_die_t *die) {
  assert(pkg != NULL);
  assert(die != NULL);
 ...
 */
__attribute__((noreturn)) void cpu_thread_halt(void) {
  x86_core_t *core;
  cpu_data_t *cpup = current_cpu_datap();
 ...
 */
void x86_validate_topology(void) {
  x86_pkg_t *pkg;
  x86_die_t *die;
  x86_core_t *core;
  x86_lcpu_t *lcpu;
  ...
  while (pkg != NULL) {
    ...
    while (die != NULL) {
      if (die->package == NULL) {
        panic("Die(%d)->package is NULL", die->pdie_num);
      }
      if (die->package != pkg) {
        panic("Die %d points to package %d, should be %d", die->pdie_num,
              die->package->lpkg_num, pkg->lpkg_num);
      }
      ...
      while (core != NULL) {
        if (core->die == NULL) {
          panic("Core(%d)->die is NULL", core->pcore_num);
        }
        if (core->die != die) {
          panic("Core %d points to die %d, should be %d", core->pcore_num,
                core->die->pdie_num, die->pdie_num);
        }
        nCores += 1;
      ...
      while (lcpu != NULL) {
        if (lcpu->die == NULL) {
          panic("CPU(%d)->die is NULL", lcpu->cpu_num);
        }
        if (lcpu->die != die) {
          panic("CPU %d points to die %d, should be %d", lcpu->cpu_num,
                lcpu->die->pdie_num, die->pdie_num);
        }
        nCPUs += 1;
    ...
    while (core != NULL) {
      if (core->package == NULL) {
        panic("Core(%d)->package is NULL", core->pcore_num);
      }
      if (core->package != pkg) {
        panic("Core %d points to package %d, should be %d", core->pcore_num,
              core->package->lpkg_num, pkg->lpkg_num);
      }
      ...
      while (lcpu != NULL) {
        if (lcpu->core == NULL) {
          panic("CPU(%d)->core is NULL", lcpu->cpu_num);
        }
        if (lcpu->core != core) {
          panic("CPU %d points to core %d, should be %d", lcpu->cpu_num,
                lcpu->core->pcore_num, core->pcore_num);
        }
        TOPO_DBG("%d ", lcpu->cpu_num);
        nCPUs += 1;
    ...
    while (lcpu != NULL) {
      if (lcpu->package == NULL) {
        panic("CPU(%d)->package is NULL", lcpu->cpu_num);
      }
      if (lcpu->package != pkg) {
        panic("CPU %d points to package %d, should be %d", lcpu->cpu_num,
              lcpu->package->lpkg_num, pkg->lpkg_num);
      }
