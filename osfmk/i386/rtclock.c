 ...
 * used to maintain a monotonic clock, adjusted from an outside reference as
 * needed.
 *
 ...
 *
 * All of the routines which update the nanotime data are non-reentrant.  This
 * must be guaranteed by the caller.
 */
static inline void rtc_nanotime_set_commpage(pal_rtc_nanotime_t *rntp) {
  commpage_set_nanotime(rntp->tsc_base, rntp->ns_base, rntp->scale,
                        rntp->shift);
}
 ...
 */
static inline void _rtc_nanotime_init(pal_rtc_nanotime_t *rntp, uint64_t base) {
  uint64_t tsc = rdtsc64();

  _pal_rtc_nanotime_store(tsc, base, rntp->scale, rntp->shift, rntp);
}

void rtc_nanotime_init(uint64_t base) {
 ...
 */
void rtc_nanotime_init_commpage(void) {
  spl_t s = splclock();
 ...
 * context.
 */
static inline uint64_t rtc_nanotime_read(void) {
 ...
 */
void rtc_clock_napped(uint64_t base, uint64_t tsc_base) {
  ...
  tsc = rdtsc64();
  oldnsecs =
      rntp->ns_base + _rtc_tsc_to_nanoseconds(tsc - rntp->tsc_base, rntp);
 ...
 */
void rtc_clock_adjust(uint64_t tsc_base_delta) {
  pal_rtc_nanotime_t *rntp = &pal_rtc_nanotime_info;
 ...
 */
void rtc_sleep_wakeup(uint64_t base) {
...
void rtc_decrementer_configure(void) { rtc_timer->rtc_config(); }
/*
...
static void rtc_set_timescale(uint64_t cycles) {
  ...
  if (tsc_rebase_abs_time == 0) {
    tsc_rebase_abs_time =
        _rtc_tsc_to_nanoseconds(rdtsc64() - tsc_at_boot, rntp);
  }

  rtc_nanotime_init(0);
}

static uint64_t rtc_export_speed(uint64_t cyc_per_sec) {
  pal_rtc_nanotime_t *rntp = &pal_rtc_nanotime_info;
  uint64_t cycles;
 ...
 */
int rtclock_intr(x86_saved_state_t *tregs) {
  uint64_t rip;
...
uint64_t mach_absolute_time(void) { return rtc_nanotime_read(); }

uint64_t mach_approximate_time(void) { return rtc_nanotime_read(); }
