...
static inline uint16_t sel_to_selector(sel_t sel) {
  union {
    sel_t sel;
    uint16_t selector;
  } tconv;
...
static inline sel_t selector_to_sel(uint16_t selector) {
  union {
    uint16_t selector;
    sel_t sel;
  } tconv;
...
struct real_gate {
  uint32_t offset_low : 16, /* offset 0..15 */
      selector : 16, word_count : 8, access : 8,
      offset_high : 16; /* offset 16..31 */
};
struct real_gate64 {
  uint32_t offset_low16 : 16, /* offset 0..15 */
      selector16 : 16, IST : 3, zeroes5 : 5, access8 : 8,
