...
int diagCall64(x86_saved_state_t *state) {
  uint64_t curpos, i, j;
  ...
  case dgRuptStat:    /* Suck Interruption statistics */
    ...
    (void)copyout((char *)&real_ncpus, data,
                  sizeof(real_ncpus)); /* Copy out number of
                                        * processors */
    ...
    for (i = 0; i < real_ncpus; i++) {           /* Move 'em all out */
      (void)copyout((char *)&durNap, curpos, 8); /* Copy out the time
                                                  * since last clear */
      (void)copyout((char *)&cpu_data_ptr[i]->cpu_hwIntCnt, curpos + 8,
                    256 * sizeof(uint32_t));          /* Copy out interrupt
                                                       * data for this
                                                       * processor */
  ...
  case dgPowerStat: {
    ...
    rdmsr64_carefully(MSR_IA32_LLC_FLUSHED_RESIDENCY_TIMER,
                      &pkes.llc_flushed_cycles);

    pkes.ring_ratio_instantaneous = ~0ULL;
    rdmsr64_carefully(MSR_IA32_RING_PERF_STATUS,
                      &pkes.ring_ratio_instantaneous);
    ...
    rdmsr64_carefully(MSR_IA32_GT_PERF_LIMIT_REASONS,
                      &pkes.GT_frequency_clipping_cause);
    ...
    for (i = 0; i < real_ncpus; i++) {
      ...
      bcopy(&cpu_data_ptr[i]->cpu_rtimes[0], &cest.crtimes[0],
            sizeof(cest.crtimes));
      bcopy(&cpu_data_ptr[i]->cpu_itimes[0], &cest.citimes[0],
            sizeof(cest.citimes));
      ...
      cest.cpu_insns =
          cpu_data_ptr[i]->cpu_monotonic.mtc_counts[MT_CORE_INSTRS];
      cest.cpu_ucc = cpu_data_ptr[i]->cpu_monotonic.mtc_counts[MT_CORE_CYCLES];
      cest.cpu_urc =
          cpu_data_ptr[i]->cpu_monotonic.mtc_counts[MT_CORE_REFCYCLES];
