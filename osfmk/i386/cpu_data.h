...
#define HWINTCNT_SIZE 256
  ...
  /*
   * Note that the cacheline-copy mechanism uses the cpu_rtimes field in the
   * shadow CPU structures to temporarily stash the code cacheline that includes
   * the instruction pointer at the time of the fault (this field is otherwise
   * unused in the shadow CPU structures).
   */
 ...
 * the new pc.
 */
static inline thread_t get_active_thread_volatile(void) {
  return CPU_DATA()->cpu_active_thread;
}

static inline __attribute__((const)) thread_t get_active_thread(void) {
  return CPU_DATA()->cpu_active_thread;
}
...
static inline int get_cpu_number(void) { return CPU_DATA()->cpu_number; }
static inline vm_offset_t get_current_percpu_base(void) {
 ...
 * To enable, sysctl -w machdep.pltrace=1 on DEVELOPMENT or DEBUG kernels (DRK
 * '15) The bounds check currently doesn't account for non-default thread stack
 * sizes.
 */
#if DEVELOPMENT || DEBUG
static inline void rbtrace_bt(uint64_t *__counted_by(maxframes) rets,
                              int maxframes, cpu_data_t *cdata,
                              uint64_t frameptr, bool use_cursp) {
  ...
  if (cplthread) {
    ...
    if (__probable(kstackb && kstackt)) {
      ...
      for (rbbtf = btidx; rbbtf < maxframes; rbbtf++) {
         ...
         * that kstackb is non-zero from the containing if block.  The
         * os_add_overflow is necessary because it's not uncommon for backtraces
         * to terminate with bogus frame pointers.
         */
        if (((uint64_t)cfp < kstackb) ||
            os_add_overflow((uint64_t)cfp, sizeof(uint64_t), &cur_retp) ||
            cur_retp >= kstackt) {
          rets[rbbtf] = 0;
          continue;
        }
...
static inline uint32_t traptrace_start(int vecnum, uint64_t ipc, uint64_t sabs,
                                       uint64_t frameptr) {
  cpu_data_t *cdata;
  uint32_t nextidx;
  ...
  *nextidxp = (((nextidx + 1) >= (unsigned int)traptrace_entries_per_cpu)
                   ? 0
                   : (nextidx + 1));

  cur_traptrace_ring = __unsafe_forge_bidi_indexable(
      traptrace_entry_t *, *PERCPU_GET(traptrace_ring),
      sizeof(traptrace_entry_t) * traptrace_entries_per_cpu);
  ...
  rbtrace_bt(&cur_traptrace_ring[nextidx].backtrace[0], MAX_TRACE_BTFRAMES - 1,
             cdata, frameptr, false);
...
static inline void traptrace_end(uint32_t index, uint64_t eabs) {
  traptrace_entry_t *__unsafe_indexable ring;

  if (index != TRAPTRACE_INVALID_INDEX) {
    ring =
        *PERCPU_GET_WITH_BASE(other_percpu_base(index >> 16), traptrace_ring);
    index &= 0XFFFF;
...
__header_always_inline void pltrace(boolean_t plenable) {
#if DEVELOPMENT || DEBUG
...
static inline void disable_preemption_internal(void) {
...
static inline void enable_preemption_internal(void) {
  assert(get_preemption_level() > 0);
  pltrace(TRUE);
  os_compiler_barrier();
...
static inline void _enable_preemption_no_check(void) {
...
static inline void _mp_disable_preemption(void) {
  disable_preemption_internal();
}

static inline void mp_enable_preemption(void) { enable_preemption_internal(); }

static inline void _mp_enable_preemption(void) { enable_preemption_internal(); }

static inline void mp_enable_preemption_no_check(void) {
  enable_preemption_no_check();
}

static inline void _mp_enable_preemption_no_check(void) {
  enable_preemption_no_check();
}
...
static inline cpu_data_t *cpu_datap(int cpu) { return cpu_data_ptr[cpu]; }

static inline int cpu_is_running(int cpu) {
  return (cpu_datap(cpu) != NULL) && (cpu_datap(cpu)->cpu_running);
}

#ifdef MACH_KERNEL_PRIVATE
static inline cpu_data_t *cpu_shadowp(int cpu) {
