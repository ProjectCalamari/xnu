...
extern void i386_vm_init(uint64_t, boolean_t, struct boot_args *);
...
extern void fix_desc(void *desc, int num_desc);
extern void fix_desc64(void *desc, int num_desc);
extern void cnpollc(boolean_t on);
extern void form_pic_mask(void);
extern void intnull(int unit);
extern char *i386_boot_info(char *buf, vm_size_t buf_len);

extern void blkclr(const char *from, int nbytes);

extern void memset_word(int *dst, int pattern, int nwords);
...
extern int apply_func_phys(addr64_t src64, vm_size_t bytes,
                           int (*func)(void *buffer, vm_size_t bytes,
                                       void *arg),
                           void *arg);
...
extern void sysclk_gettime_interrupts_disabled(mach_timespec_t *cur_time);
...
__not_tail_called thread_t Shutdown_context(thread_t thread,
