...
#define pmap_assert(ex)                                                        \
  (pmap_asserts_enabled                                                        \
       ? ((ex) ? (void)0 : Assert(__FILE_NAME__, __LINE__, #ex))               \
       : (void)0)
  ...
  do {                                                                         \
    if (__improbable(pmap_asserts_enabled && !(ex))) {                         \
      if (pmap_asserts_traced) {                                               \
        KERNEL_DEBUG_CONSTANT(0xDEAD1000, __builtin_return_address(0),         \
                              __LINE__, 0, 0, 0);                              \
        kdebug_enable = 0;                                                     \
      } else {                                                                 \
        kprintf("Assertion %s failed (%s:%d, caller %p) " fmt, #ex,            \
                __FILE_NAME__, __LINE__, __builtin_return_address(0), ##args); \
        panic("Assertion %s failed (%s:%d, caller %p) " fmt, #ex,              \
              __FILE_NAME__, __LINE__, __builtin_return_address(0), ##args);   \
      }                                                                        \
    }                                                                          \
  } while (0)
...
#define KERNEL_KEXTS_INDEX                                                     \
  (KERNEL_PML4_INDEX - 1) /* 510: Home of KEXTs - the basement */
#define KERNEL_PHYSMAP_PML4_INDEX                                              \
  (kernPhysPML4Index) /* 50X: virtual to physical map */
#define KERNEL_PHYSMAP_PML4_COUNT (kernPhysPML4EntryCount)
#define KERNEL_PHYSMAP_PML4_COUNT_MAX                                          \
  (16 - 2) /* 1 for KERNEL, 1 for BASEMENT */
...
#define KERNEL_BASEMENT                                                        \
  (KERNEL_BASE - NBPML4) /* Basement uses one PML4 entry */

/*
 * Pte related macros
 */
#define KVADDR(pmi, pdpi, pdi, pti)                                            \
  ((vm_offset_t)((uint64_t)-1 << 47) | ((uint64_t)(pmi) << PML4SHIFT) |        \
   ((uint64_t)(pdpi) << PDPTSHIFT) | ((uint64_t)(pdi) << PDESHIFT) |           \
...
#define INTEL_PTE_COMPRESSED_ALT                                               \
  (1ULL << 61) /* compressed but with "alternate accounting" */
#define INTEL_PTE_COMPRESSED                                                   \
  (1ULL << 62) /* marker, for invalid PTE only -- ignored by hardware for both \
                  regular/EPT entries*/
...
#define INTEL_PTE_COMPRESSED_MASK                                              \
  (INTEL_PTE_COMPRESSED | INTEL_PTE_COMPRESSED_ALT | INTEL_PTE_SWLOCK)
#define PTE_IS_COMPRESSED(x, ptep, pmap, vaddr)                                \
  ((((x) & INTEL_PTE_VALID) == 0) &&         /* PTE is not valid... */         \
   ...
   ((!((x) & ~INTEL_PTE_COMPRESSED_MASK)) || /* ...no other bits */            \
    pmap_compressed_pte_corruption_repair((x), &(x), (ptep), (pmap),           \
                                          (vaddr))))
...
#define pte_kernel_rw(p)                                                       \
  ((pt_entry_t)(pa_to_pte(p) | INTEL_PTE_VALID | INTEL_PTE_RW))
#define pte_kernel_ro(p) ((pt_entry_t)(pa_to_pte(p) | INTEL_PTE_VALID))
#define pte_user_rw(p)                                                         \
  ((pt_entry_t)(pa_to_pte(p) | INTEL_PTE_VALID | INTEL_PTE_USER | INTEL_PTE_RW))
#define pte_user_ro(p)                                                         \
  ((pt_entry_t)(pa_to_pte(p) | INTEL_PTE_VALID | INTEL_PTE_USER))
...
#define INTEL_EPT_UEX                                                          \
  0x00000400ULL /* User-execute when MBE is enabled (ignored otherwise) */
...
static inline pt_entry_t pte_remove_ex(pt_entry_t pte, boolean_t is_ept) {
...
static inline pt_entry_t pte_set_ex(pt_entry_t pte, boolean_t is_ept) {
  if (__probable(!is_ept)) {
    return pte & (~INTEL_PTE_NX);
  }
...
static inline pt_entry_t pte_set_uex(pt_entry_t pte) {
  return pte | INTEL_EPT_UEX;
}

static inline pt_entry_t physmap_refmod_to_ept(pt_entry_t physmap_pte) {
...
static inline pt_entry_t ept_refmod_to_physmap(pt_entry_t ept_pte) {
...
#define PTE_VALID_MASK(is_ept)                                                 \
  ((is_ept)                                                                    \
       ? (INTEL_EPT_READ | INTEL_EPT_WRITE | INTEL_EPT_EX | INTEL_EPT_UEX)     \
       : INTEL_PTE_VALID)
...
#define PTE_IS_EXECUTABLE(is_ept, pte)                                         \
  ((is_ept) ? (((pte) & (INTEL_EPT_EX | INTEL_EPT_UEX)) != 0)                  \
            : (((pte) & INTEL_PTE_NX) == 0))
...
static inline void pmap_corrupted_pte_detected(pt_entry_t *ptep,
                                               uint64_t clear_bits,
                                               uint64_t set_bits) {
  if (__c11_atomic_compare_exchange_strong(
          (_Atomic(pt_entry_t *) *)&PTE_corrupted_ptr, &PTE_corrupted_ptr, ptep,
    ...
    if (clear_bits == 0 && set_bits == 0) {
      panic("PTE Corruption detected: ptep 0x%llx pte value 0x%llx",
            (unsigned long long)(uintptr_t)ptep, *(uint64_t *)ptep);
    } else {
      panic("PTE Corruption detected: ptep 0x%llx pte value 0x%llx clear "
            "0x%llx set 0x%llx",
            (unsigned long long)(uintptr_t)ptep, *(uint64_t *)ptep, clear_bits,
            set_bits);
    }
  }
}
 ...
 */
static inline void pmap_store_pte(boolean_t is_ept, pt_entry_t *entryp,
                                  pt_entry_t value) {
  /*
  ...
  *entryp = value;
  ...
  if (__improbable((is_ept == FALSE) && (value & PTE_COMPRESSED) &&
                   (value & INTEL_PTE_NX))) {
...
static inline uint64_t DBLMAP_CHECK(uintptr_t x) {
  ...
  if (__improbable((dbladdr >= dblmap_max) || (dbladdr < dblmap_base))) {
    panic("DBLMAP bounds exceeded, 0x%qx, 0x%qx 0x%qx, 0x%qx", (uint64_t)x,
          dbladdr, dblmap_base, dblmap_max);
  }
  return dbladdr;
}
...
static inline uint64_t LDTALIAS_CHECK(uintptr_t x) {
  ...
  if (__improbable((dbladdr >= dblmap_max) || (dbladdr < dblmap_base))) {
    panic("LDTALIAS: bounds exceeded, 0x%qx, 0x%qx 0x%qx, 0x%qx", (uint64_t)x,
          dbladdr, dblmap_base, dblmap_max);
  }
  return dbladdr;
}
...
#if MACH_KERNEL_PRIVATE
extern void pmap_tlbi_range(uint64_t startv, uint64_t endv, bool global,
                            uint16_t pcid);
...
struct pmap {
  ...
  pmap_paddr_t pm_cr3
      __attribute((aligned(64))); /* Kernel+user shared PML4 physical*/
...
static inline boolean_t is_ept_pmap(pmap_t p) {
...
static inline void set_dirbase(pmap_t tpmap, thread_t thread, int my_cpu) {
  int ccpu = my_cpu;
...
extern kern_return_t pmap_expand(pmap_t pmap, vm_map_offset_t addr,
                                 unsigned int options);
extern vm_offset_t pmap_map(vm_offset_t virt, vm_map_offset_t start,
                            vm_map_offset_t end, vm_prot_t prot,
                            unsigned int flags);

extern vm_offset_t pmap_map_bd(vm_offset_t virt, vm_map_offset_t start,
                               vm_map_offset_t end, vm_prot_t prot,
                               unsigned int flags);
extern void pmap_bootstrap(vm_offset_t load_start, boolean_t IA32e);
...
extern void pmap_mark_range(pmap_t npmap, uint64_t sv, uint64_t nxrosz,
                            boolean_t NX, boolean_t ro);
...
extern kern_return_t pmap_map_block_addr(pmap_t pmap, addr64_t va,
                                         pmap_paddr_t pa, uint32_t size,
                                         vm_prot_t prot, int attr,
                                         unsigned int flags);
extern kern_return_t pmap_map_block(pmap_t pmap, addr64_t va, ppnum_t pa,
...
extern void pmap_pagetable_corruption_msg_log(int (*)(const char *fmt, ...)
                                                  __printflike(1, 2));
...
static inline vm_offset_t pmap_ro_zone_align(vm_offset_t value) {
  return value;
}

extern void pmap_ro_zone_memcpy(zone_id_t zid, vm_offset_t va,
                                vm_offset_t offset, vm_offset_t new_data,
                                vm_size_t new_data_size);
extern uint64_t pmap_ro_zone_atomic_op(zone_id_t zid, vm_offset_t va,
                                       vm_offset_t offset, uint32_t op,
                                       uint64_t value);
extern void pmap_ro_zone_bzero(zone_id_t zid, vm_offset_t va,
...
#define PMAP_DEACTIVATE_MAP(map, thread, ccpu)                                 \
  pmap_assert2(                                                                \
      (pmap_pcid_ncpus ? (pcid_for_pmap_cpu_tuple(map->pmap, thread, ccpu) ==  \
                          (get_cr3_raw() & 0xFFF))                             \
                       : TRUE),                                                \
      "PCIDs: 0x%x, active PCID: 0x%x, CR3: 0x%lx, pmap_cr3: 0x%llx, "         \
      "kernel_cr3: 0x%llx, kernel pmap cr3: 0x%llx, CPU active PCID: 0x%x, "   \
      "CPU kernel PCID: 0x%x, specflags: 0x%x, pagezero: 0x%x",                \
      pmap_pcid_ncpus, pcid_for_pmap_cpu_tuple(map->pmap, thread, ccpu),       \
      get_cr3_raw(), map->pmap->pm_cr3, cpu_datap(ccpu)->cpu_kernel_cr3,       \
      kernel_pmap->pm_cr3, cpu_datap(ccpu)->cpu_active_pcid,                   \
      cpu_datap(ccpu)->cpu_kernel_pcid, thread->machine.specFlags,             \
      map->pmap->pagezero_accessible);
#else
 ...
 */
#define CPU_CR3_MARK_INACTIVE() current_cpu_datap()->cpu_active_cr3 |= 1

#define CPU_CR3_MARK_ACTIVE() current_cpu_datap()->cpu_active_cr3 &= ~1

#define CPU_CR3_IS_ACTIVE(cpu) ((cpu_datap(cpu)->cpu_active_cr3 & 1) == 0)

#define CPU_GET_ACTIVE_CR3(cpu) (cpu_datap(cpu)->cpu_active_cr3 & ~1)

#define CPU_GET_TASK_CR3(cpu) (cpu_datap(cpu)->cpu_task_cr3)
...
extern boolean_t pmap_is_empty(pmap_t pmap, vm_map_offset_t start,
                               vm_map_offset_t end);

#define MACHINE_BOOTSTRAPPTD 1 /* Static bootstrap page-tables */

kern_return_t pmap_permissions_verify(pmap_t, vm_map_t, vm_offset_t,
                                      vm_offset_t);
