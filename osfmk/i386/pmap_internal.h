...
static inline void PMAP_LOCK_EXCLUSIVE(pmap_t p) {
  mp_disable_preemption();
  lck_rw_lock_exclusive(&p->pmap_rwl);
}

static inline void PMAP_LOCK_SHARED(pmap_t p) {
  mp_disable_preemption();
  lck_rw_lock_shared(&p->pmap_rwl);
}

static inline void PMAP_LOCK_SHARED_TO_EXCLUSIVE(pmap_t p) {
  lck_rw_lock_shared_to_exclusive(&p->pmap_rwl);
}

static inline void PMAP_LOCK_EXCLUSIVE_TO_SHARED(pmap_t p) {
  lck_rw_lock_exclusive_to_shared(&p->pmap_rwl);
}

static inline void PMAP_UNLOCK_EXCLUSIVE(pmap_t p) {
  lck_rw_unlock_exclusive(&p->pmap_rwl);
  mp_enable_preemption();
}

static inline void PMAP_UNLOCK_SHARED(pmap_t p) {
...
kern_return_t pmap_expand_pml4(pmap_t map, vm_map_offset_t v,
                               unsigned int options);

kern_return_t pmap_expand_pdpt(pmap_t map, vm_map_offset_t v,
                               unsigned int options);

void phys_attribute_set(ppnum_t phys, int bits);

void pmap_set_reference(ppnum_t pn);

boolean_t phys_page_exists(ppnum_t pn);

void pmap_flush_tlbs(pmap_t, vm_map_offset_t, vm_map_offset_t, int,
                     pmap_flush_context *);

void pmap_update_cache_attributes_locked(ppnum_t, unsigned);

static inline void PMAP_UPDATE_TLBS(pmap_t fp, addr64_t s, addr64_t e) {
  pmap_flush_tlbs(fp, s, e, 0, NULL);
}

#define PMAP_DELAY_TLB_FLUSH 0x01

static inline void PMAP_UPDATE_TLBS_DELAYED(pmap_t fp, addr64_t s, addr64_t e,
 ...
 *
 *  Pve's establish physical to virtual mappings.  These are used for aliasing
 * of a physical page to (potentially many) virtual addresses within pmaps. In
 * the previous implementation the structure of the pv_entries (each 16 bytes in
 * size) was
 *
 ...
 *  creating new mappings.  Originally, we kept this pool around because the
 * code in pmap_enter() was unable to block if it needed an entry and none were
 *  available - we'd panic.  Some time ago I restructured the pmap_enter() code
 *  so that for user pmaps it can block while zalloc'ing a pv structure and
 * restart, removing a panic from the code (in the case of the kernel pmap we
 * cannot block and still panic, so, we keep a separate hot pool for use only on
 * kernel pmaps). The pool has not been removed since there is a large
 * performance gain keeping freed pv's around for reuse and not suffering the
 * overhead of zalloc for every new pv we need.
 *
 *  As pmap_enter() created new mappings it linked the new pve's for them off
 * the fixed pv array for that ppn (off the next pointer).  These pve's are
 * accessed for several operations, one of them being address space teardown. In
 * that case, we basically do this
 *
 ...
 *  entry could have thousands of entries.  We were continuously linearly
 * searching each of these lists as we stepped through the address space we were
 * tearing down.  Because of the locks we hold, likely taking a cache miss for
 * each node, and interrupt disabling for MP issues the system became completely
 ...
 *  basically the original structure accessed in an array by ppn, and a
 * ''hashed'' structure accessed on a hash list via a hash of [pmap, vaddr].
 * These have been designed with the two goals of minimizing wired memory and
 * making the lookup of a ppn faster.  Since a vast majority of pages in the
 * system are not aliased and hence represented by a single pv entry I've kept
 * the rooted entry size as small as possible because there is one of these
 * dedicated for every physical page of memory.  The hashed pve's are larger due
 * to the addition of the hash link and the ppn entry needed for matching while
 * running the hash list to find the entry we are looking for.  This way, only
 * systems that have lots of aliasing (like 2000+ httpd procs) will pay the
 * extra memory price. Both structures have the same first three fields allowing
 ...
 *  } *pv_hashed_entry_t;
 *
 *  The main flow difference is that the code is now aware of the rooted entry
 * and the hashed entries.  Code that runs the pv list still starts with the
 * rooted entry and then continues down the qlink onto the hashed entries.  Code
 * that is looking up a specific pv entry first checks the rooted entry and then
 * hashes and runs the hash list for the match. The hash list lengths are much
 * smaller than the original pv lists that contained all aliases for the
 * specific ppn.
 *
 */
...
static inline void PV_HASHED_FREE_LIST(pv_hashed_entry_t pvh_eh,
                                       pv_hashed_entry_t pvh_et, int pv_cnt) {
  simple_lock(&pv_hashed_free_list_lock, LCK_GRP_NULL);
...
static inline void PV_HASHED_KERN_FREE_LIST(pv_hashed_entry_t pvh_eh,
...
static inline void pmap_pv_throttle(__unused pmap_t p) {
  ...
  if (pv_hashed_kern_free_count < (pv_hashed_kern_low_water_mark / 2)) {
    ...
    assert_wait_timeout(&pmap_user_pv_throttle_event, THREAD_UNINT, 1,
                        1000 * NSEC_PER_USEC);
...
/*
 *	Index into pv_head table, its lock bits, and the modify/reference and
 * managed bits
 */
...
#define IS_INTERNAL_PAGE(x)                                                    \
  (IS_MANAGED_PAGE(x) &&                                                       \
   ((unsigned long long)pmap_phys_attributes[x] & PHYS_INTERNAL))
#define IS_REUSABLE_PAGE(x)                                                    \
  (IS_MANAGED_PAGE(x) &&                                                       \
   ((unsigned long long)pmap_phys_attributes[x] & PHYS_REUSABLE))
#define IS_ALTACCT_PAGE(x, pve)                                                \
  (IS_MANAGED_PAGE((x)) && (PVE_IS_ALTACCT_PAGE((pve))))
...
#define PHYS_NOENCRYPT                                                         \
  INTEL_PTE_USER /* no need to encrypt this page in the hibernation image */
...
static inline void pv_hash_add(pv_hashed_entry_t pvh_e,
...
/*
 * Routines to handle suppression of/recovery from some forms of pagetable
 * corruption incidents observed in the field. These can be either software
 * induced (wild stores to the mapwindows where applicable, use after free
 * errors (typically of pages addressed physically), mis-directed DMAs etc., or
 * due to DRAM/memory hierarchy/interconnect errors. Given the theoretical
 * rarity of these errors, the recording mechanism is deliberately not MP-safe.
 * The overarching goal is to still assert on potential software races, but
 * attempt recovery from incidents identifiable as occurring due to issues
 * beyond the control of the pmap module. The latter includes single-bit errors
 * and malformed pagetable entries. We currently limit ourselves to
 * recovery/suppression of one incident per PMAP_PAGETABLE_CORRUPTION_INTERVAL
 * seconds, and details of the incident are logged. Assertions are not
 * suppressed if kernel debugging is enabled. (DRK 09)
 */
...
typedef enum { ROOT_PRESENT = 0, ROOT_ABSENT = 1 } pmap_pv_assertion_t;
...
static inline pmap_pagetable_corruption_action_t pmap_pagetable_corruption_log(
    pmap_pv_assertion_t incident, pmap_pagetable_corruption_t suppress_reason,
    pmap_pagetable_corruption_action_t action, pmap_t pmap,
    vm_map_offset_t vaddr, pt_entry_t *ptep, ppnum_t ppn, pmap_t pvpmap,
    vm_map_offset_t pvva, int adj_pteps_cnt, uint64_t **adj_pteps) {
  uint32_t pmap_pagetable_corruption_log_index;
  uint64_t curtime = mach_absolute_time();

  if ((curtime - pmap_pagetable_corruption_last_abstime) <
  ...
  pmap_pagetable_corruption_log_index =
      pmap_pagetable_corruption_incidents++ % PMAP_PAGETABLE_CORRUPTION_MAX_LOG;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index]
      .incident = incident;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index]
      .reason = suppress_reason;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index]
      .action = action;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index].pmap =
      pmap;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index].vaddr =
      vaddr;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index].pte =
      *ptep;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index].ppn =
      ppn;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index]
      .pvpmap = pvpmap;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index].pvva =
      pvva;
  pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index]
      .abstime = curtime;
  if (adj_pteps_cnt > 0 && adj_pteps != NULL) {
    pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index]
        .adj_ptes_count = MIN(adj_pteps_cnt, PMPTCR_MAX_ADJ_PTES);
    for (int i = 0;
         i <
         pmap_pagetable_corruption_records[pmap_pagetable_corruption_log_index]
             .adj_ptes_count;
...
pmap_classify_pagetable_corruption(pmap_t pmap, vm_map_offset_t vaddr,
                                   ppnum_t *ppnp, pt_entry_t *ptep,
                                   pmap_pv_assertion_t incident) {
   ...
   */
  do {
    if ((popcnt1((uintptr_t)pv_e->pmap ^ (uintptr_t)pmap) &&
         PVE_VA(pv_e) == vaddr) ||
  ...
  } while (((pv_e = (pv_rooted_entry_t)queue_next(&pv_e->qlink))) &&
           (pv_e != pv_h));
   ...
   * The first check here isn't useful for EPT PTEs because INTEL_EPT_NCACHE ==
   * 0
   */
  if (!is_ept &&
      ((cpte & (INTEL_PTE_NCACHE | INTEL_PTE_WTHRU | INTEL_PTE_PAT)) ==
  ...
  } else if ((pmap != kernel_pmap) && (!is_ept) &&
             ((cpte & INTEL_PTE_USER) == 0)) {
    action = PMAP_ACTION_IGNORE;
    suppress_reason = PTE_SUPERVISOR;
  }
pmap_cpc_exit:
  PE_parse_boot_argn("-pmap_pagetable_corruption_deassert", &ppcd,
                     sizeof(ppcd));
  ...
  return pmap_pagetable_corruption_log(incident, suppress_reason, action, pmap,
                                       vaddr, &cpte, *ppnp, pvpmap, pvva, 0, 0);
}

static inline boolean_t
pmap_compressed_pte_corruption_repair(uint64_t pte, uint64_t *pte_addr,
                                      uint64_t *ptep, pmap_t pmap,
                                      vm_map_offset_t vaddr) {
  ...
  /*
   * Grab pointers to PTEs on either side of the PTE in question, unless we're
   * at the start of a PT (grab pointers to the next and next-next PTEs) or the
   * end of a PT (grab the previous 2 PTEs).
   */
  if (pteidx == 0) {
    adj_pteps[0] = ptep + 1;
    adj_pteps[1] = ptep + 2;
  ...
  /*
   * Since the compressed PTE no longer has a PTE associated, we cannot pass in
   * the pv data to pmap_pagetable_corruption_log, so instead supply adjacent
   * PTEs for logging.
   */
  if (pmap_pagetable_corruption_log(
          ROOT_ABSENT, (pte & INTEL_PTE_NX) ? PTE_NXBITFLIP : PTE_BITFLIP,
          action, pmap, vaddr, ptep, (ppnum_t)~0UL, 0, 0,
          sizeof(adj_pteps) / sizeof(adj_pteps[0]),
    ...
    return TRUE; /* Returning TRUE to indicate this is a now a valid compressed
                    PTE (we hope) */
  }

  panic("compressed PTE %p 0x%llx has extra bits 0x%llx: corrupted? Adjacent "
        "PTEs: 0x%llx@%p, 0x%llx@%p",
        pte_addr, pte, pte & ~INTEL_PTE_COMPRESSED_MASK, *adj_pteps[0],
        adj_pteps[0], *adj_pteps[1], adj_pteps[1]);
  /*NOTREACHED*/
}
...
pmap_pv_remove(pmap_t pmap, vm_map_offset_t vaddr, ppnum_t *ppnp,
               pt_entry_t *pte, boolean_t *was_altacct) {
...
pmap_pv_remove_retry:
  ...
  if (__improbable(pv_h->pmap == PMAP_NULL)) {
    pmap_pagetable_corruption_action_t pac =
        pmap_classify_pagetable_corruption(pmap, vaddr, ppnp, pte, ROOT_ABSENT);
    if (pac == PMAP_ACTION_IGNORE) {
      ...
      panic("Possible memory corruption: pmap_pv_remove(%p,0x%llx,0x%x, "
            "0x%llx, %p, %p): null pv_list, priors: %d",
            pmap, vaddr, ppn, *pte, ppnp, pte,
            pmap_pagetable_corruption_incidents);
    } else if (pac == PMAP_ACTION_RETRY_RELOCK) {
      LOCK_PVH(ppn_to_pai(*ppnp));
      pmap_phys_attributes[ppn_to_pai(*ppnp)] |=
          (PHYS_MODIFIED | PHYS_REFERENCED);
  ...
  if (PVE_VA(pv_h) == vaddr && pv_h->pmap == pmap) {
    *was_altacct = IS_ALTACCT_PAGE(ppn_to_pai(*ppnp), pv_h);
    ...
    while (PV_HASHED_ENTRY_NULL != pvh_e) {
      pv_cnt++;
      if (pvh_e->pmap == pmap && PVE_VA(pvh_e) == vaddr && pvh_e->ppn == ppn) {
        break;
      }
      pprevh = &pvh_e->nexth;
      pvh_e = pvh_e->nexth;
    ...
    if (PV_HASHED_ENTRY_NULL == pvh_e) {
      pmap_pagetable_corruption_action_t pac =
          pmap_classify_pagetable_corruption(pmap, vaddr, ppnp, pte,
                                             ROOT_PRESENT);

      if (pac == PMAP_ACTION_ASSERT) {
        panic("Possible memory corruption: pmap_pv_remove(%p, 0x%llx, 0x%x, "
              "0x%llx, %p, %p): pv not on hash, head: %p, 0x%llx, priors: %d",
              pmap, vaddr, ppn, *pte, ppnp, pte, pv_h->pmap, PVE_VA(pv_h),
              pmap_pagetable_corruption_incidents);
      } else {
        UNLOCK_PV_HASH(pvhash_idx);
...
pmap_pv_is_altacct(pmap_t pmap, vm_map_offset_t vaddr, ppnum_t ppn) {
  pv_hashed_entry_t pvh_e;
  pv_rooted_entry_t pv_h;
  uint32_t pvhash_idx;
...
int phys_attribute_test(ppnum_t phys, int bits);
void phys_attribute_clear(ppnum_t phys, int bits, unsigned int options,
                          void *arg);
...
static inline boolean_t pmap_cmpx_pte(pt_entry_t *entryp, pt_entry_t old,
                                      pt_entry_t new) {
  return __c11_atomic_compare_exchange_strong(
      (_Atomic pt_entry_t *)entryp, &old, new, memory_order_acq_rel_smp,
...
static inline void pmap_update_pte(boolean_t is_ept, pt_entry_t *mptep,
                                   uint64_t pclear_bits, uint64_t pset_bits,
                                   bool oldpte_invalid_ok) {
  ...
  do {
...
#endif
      return;
    } else if (__improbable(!oldpte_invalid_ok &&
                            (opte & PTE_VALID_MASK(is_ept)) == 0)) {
#if DEVELOPMENT || DEBUG
    ...
    npte |= pset_bits;
#if DEVELOPMENT || DEBUG
    if (__improbable(pmap_inject_pte_corruption != 0 && is_ept == FALSE &&
                     (npte & PTE_COMPRESSED))) {
  ...
  if (__improbable((is_ept == FALSE) && (npte & PTE_COMPRESSED) &&
                   (npte & INTEL_PTE_NX))) {
...
#if DEBUG
  return PHYSMAP_PTOV(
      &((pml4_entry_t *)pmap->pm_cr3)[(vaddr >> PML4SHIFT) & (NPML4PG - 1)]);
#else
  return &pmap->pm_pml4[(vaddr >> PML4SHIFT) & (NPML4PG - 1)];
#endif
}

static inline pml4_entry_t *pmap64_user_pml4(pmap_t pmap,
                                             vm_map_offset_t vaddr) {
  if (__improbable((vaddr > 0x00007FFFFFFFFFFFULL) &&
                   (vaddr < 0xFFFF800000000000ULL))) {
    return NULL;
...
#if DEBUG
  return PHYSMAP_PTOV(
      &((pml4_entry_t *)pmap->pm_ucr3)[(vaddr >> PML4SHIFT) & (NPML4PG - 1)]);
#else
...
static inline pdpt_entry_t *pmap64_pdpt(pmap_t pmap, vm_map_offset_t vaddr) {
  ...
  if (pml4 && (*pml4 & PTE_VALID_MASK(is_ept))) {
    newpf = *pml4 & PG_FRAME;
    return &((pdpt_entry_t *)PHYSMAP_PTOV(
        newpf))[(vaddr >> PDPTSHIFT) & (NPDPTPG - 1)];
  }
  return NULL;
}
/*
 ...
 */
static inline pd_entry_t *pmap_pde_internal1(vm_map_offset_t vaddr,
                                             boolean_t is_ept,
                                             pdpt_entry_t *pdpte) {
  if (*pdpte & PTE_VALID_MASK(is_ept)) {
    pdpt_entry_t newpf = *pdpte & PG_FRAME;
    return &(
        (pd_entry_t *)PHYSMAP_PTOV(newpf))[(vaddr >> PDSHIFT) & (NPDPG - 1)];
  } else {
    return NULL;
  }
}

static inline pd_entry_t *pmap_pde_internal0(pmap_t pmap, vm_map_offset_t vaddr,
                                             boolean_t is_ept) {
  pdpt_entry_t *pdpt;
...
static inline pd_entry_t *pmap_pde(pmap_t pmap, vm_map_offset_t vaddr) {
  pdpt_entry_t *pdpt;
  boolean_t is_ept;
...
static inline pt_entry_t *pmap_pte_internal(vm_map_offset_t vaddr,
                                            boolean_t is_ept, pd_entry_t *pde) {
  if (*pde & PTE_VALID_MASK(is_ept)) {
    ...
    return &((pt_entry_t *)PHYSMAP_PTOV(
        newpf))[i386_btop(vaddr) & (ppnum_t)(NPTEPG - 1)];
  } else {
    return NULL;
  }
}
...
extern void pmap_alias(vm_offset_t ava, vm_map_offset_t start,
