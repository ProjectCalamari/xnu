...
hvg_hcall_trigger_dump(hvg_hcall_vmcore_file_t *vmcore,
  ...
  assert3u(dump_option, ==,
           HVG_HCALL_DUMP_OPTION_REGULAR); /* Only known option for now. */

  hvg_hcall_output_regs_t output = {};
  const hvg_hcall_return_t ret =
      hvg_hypercall1(HVG_HCALL_TRIGGER_DUMP, dump_option, &output);
  ...
  if (vmcore) {
    ...
    static_assert(sizeof(vmcore->tag) > sizeof(output),
                  "not enough room for tag");
    static_assert(sizeof(vmcore->tag[0] * sizeof(uint64_t)) ==
                      sizeof(output.rax),
                  "mis-match of tag and output sizes");
...
void hvg_hcall_set_coredump_data(void) {
  ...
  const hvg_hcall_return_t ret = hvg_hypercall6(
...
hvg_hcall_return_t hvg_hcall_get_mabs_offset(uint64_t *mabs_offset) {
  ...
  const hvg_hcall_return_t ret =
      hvg_hypercall2(HVG_HCALL_GET_MABS_OFFSET, pal_rtc_nanotime_info.tsc_base,
                     pal_rtc_nanotime_info.ns_base, &output);
  if (ret != HVG_HCALL_SUCCESS) {
    return ret;
  }
...
hvg_hcall_return_t hvg_hcall_get_bootsessionuuid(uuid_string_t uuid) {
  assert(hvg_is_hcall_available(HVG_HCALL_GET_BOOTSESSIONUUID));

  hvg_hcall_output_regs_t output = {};

  const hvg_hcall_return_t ret =
      hvg_hypercall0(HVG_HCALL_GET_BOOTSESSIONUUID, &output);
  if (ret != HVG_HCALL_SUCCESS) {
    return ret;
  }

  static_assert(sizeof(uuid_string_t) == 37, "unexpected uuid string length");
