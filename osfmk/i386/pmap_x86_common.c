...
void pmap_remove_range(pmap_t pmap, vm_map_offset_t va, pt_entry_t *spte,
                       pt_entry_t *epte);

static void pmap_remove_range_options(pmap_t pmap, vm_map_offset_t va,
                                      pt_entry_t *spte, pt_entry_t *epte,
                                      int options);

void pmap_reusable_range(pmap_t pmap, vm_map_offset_t va, pt_entry_t *spte,
                         pt_entry_t *epte, boolean_t reusable);
...
uint64_t pmap_shared_region_size_min(__unused pmap_t pmap) { return NBPDE; }

uint64_t pmap_commpage_size_min(__unused pmap_t pmap) { return NBPDE; }

void pmap_set_shared_region(pmap_t grand __unused, pmap_t subord __unused,
                            addr64_t vstart __unused, uint64_t size __unused) {}

kern_return_t pmap_fork_nest(pmap_t old_pmap __unused,
 ...
 *
 *	Note that we depend upon higher level VM locks to insure that things
 * don't change while we are doing this.  For example, VM should not be doing
 * any pmap enters while it is nesting or do 2 nests at once.
 */
...
kern_return_t pmap_nest(pmap_t grand, pmap_t subord, addr64_t va_start,
  ...
  PMAP_TRACE(PMAP_CODE(PMAP__NEST) | DBG_FUNC_START, VM_KERNEL_ADDRHIDE(grand),
             VM_KERNEL_ADDRHIDE(subord), VM_KERNEL_ADDRHIDE(va_start));
...
kern_return_t pmap_unnest(pmap_t grand, addr64_t vaddr, uint64_t size) {
  ...
  if ((size & (pmap_shared_region_size_min(grand) - 1)) ||
    ...
    panic("pmap_unnest(%p,0x%llx,0x%llx): unaligned...", grand, vaddr, size);
  }
  ...
  if (os_add_overflow(vaddr, size + NBPDE - 1, &va_end)) {
    panic("pmap_unnest: Overflow when calculating range end: s=0x%llx "
          "sz=0x%llx\n",
          vaddr, size);
  }
...
kern_return_t pmap_unnest_options(pmap_t grand, addr64_t vaddr,
  ...
  if ((pdep != PD_ENTRY_NULL) && ((pde = *pdep) & PTE_VALID_MASK(is_ept))) {
    if (pde & PTE_PS) {
      ...
      if ((PT_ENTRY_NULL != ptp) &&
          (((pte = *ptp) & PTE_VALID_MASK(is_ept)) != 0)) {
...
ppnum_t pmap_find_phys_nofault(pmap_t pmap, addr64_t va) {
  if ((pmap == kernel_pmap) || ((current_thread()->map) &&
 ...
 */
kern_return_t pmap_get_prot(pmap_t pmap, addr64_t va, vm_prot_t *protp) {
  pt_entry_t *ptp;
  pd_entry_t *pdep;
  pd_entry_t pde;
  pt_entry_t pte;
 ...
 */
void pmap_update_cache_attributes_locked(ppnum_t pn, unsigned attributes) {
  ...
  if (pv_h->pmap != PMAP_NULL) {
    ...
    do {
      ...
      if (0 == ptep) {
        panic("pmap_update_cache_attributes_locked: Missing PTE, pmap: %p, pn: "
              "0x%x vaddr: 0x%llx kernel_pmap: %p",
              pmap, pn, vaddr, kernel_pmap);
      }
...
kern_return_t pmap_enter(pmap_t pmap, vm_map_offset_t vaddr, ppnum_t pn,
                         vm_prot_t prot, vm_prot_t fault_type,
                         unsigned int flags, boolean_t wired,
                         pmap_mapping_type_t mapping_type) {
  return pmap_enter_options(pmap, vaddr, pn, prot, fault_type, flags, wired,
                            PMAP_EXPAND_OPTIONS_NONE, NULL, mapping_type);
}
...
void PTE_LOCK_LOCK(pt_entry_t *lpte) {
  pt_entry_t pte;
plretry:
  while ((pte = __c11_atomic_load((_Atomic pt_entry_t *)lpte,
                                  memory_order_relaxed)) &
         PTE_LOCK(0)) {
    __builtin_ia32_pause();
  }
  if (__c11_atomic_compare_exchange_strong((_Atomic pt_entry_t *)lpte, &pte,
                                           pte | PTE_LOCK(0),
...
void PTE_LOCK_UNLOCK(pt_entry_t *lpte) {
  __c11_atomic_fetch_and((_Atomic pt_entry_t *)lpte, ~PTE_LOCK(0),
...
kern_return_t pmap_enter_options_addr(pmap_t pmap, vm_map_address_t v,
                                      ...
                                      __unused void *arg,
                                      pmap_mapping_type_t mapping_type) {
  return pmap_enter_options(pmap, v, intel_btop(pa), prot, fault_type, flags,
                            wired, options, arg, mapping_type);
}

kern_return_t pmap_enter_options(pmap_t pmap, vm_map_offset_t vaddr, ppnum_t pn,
                                 vm_prot_t prot, __unused vm_prot_t fault_type,
                                 unsigned int flags, boolean_t wired,
  ...
  PMAP_TRACE(PMAP_CODE(PMAP__ENTER) | DBG_FUNC_START, VM_KERNEL_ADDRHIDE(pmap),
             VM_KERNEL_ADDRHIDE(vaddr), pn, prot);
...
Retry:
  ...
  if (old_pa == 0 && PTE_IS_COMPRESSED(*pte, pte, pmap, vaddr)) {
    /*
  ...
  if (old_pa == pa) {
    ...
    need_tlbflush = ((old_attributes ^ template) != PTE_WIRED);
    ...
    /* For hardware that doesn't have EPT AD support, we always set REFMOD for
     * EPT PTEs */
  ...
  if (old_pa != (pmap_paddr_t)0) {
    ...
    if (IS_MANAGED_PAGE(pai)) {
       ...
       */
      pvh_e =
          pmap_pv_remove(pmap, vaddr, (ppnum_t *)&pai, &old_pte, &was_altacct);
    }
  ...
  if (IS_MANAGED_PAGE(pai)) {
    ...
    if (pv_h->pmap == PMAP_NULL) {
      ...
      if ((options & PMAP_OPTIONS_ALT_ACCT) && IS_INTERNAL_PAGE(pai)) {
      ...
      if (PV_HASHED_ENTRY_NULL == pvh_e) {
        panic("Mapping alias chain exhaustion, possibly induced by numerous "
              "kernel virtual double mappings");
      }
  ...
  /* For hardware that doesn't have EPT AD support, we always set REFMOD for EPT
   * PTEs */
  if (__improbable(is_ept && !pmap_ept_support_ad)) {
    template |= PTE_REF(is_ept);
    if (IS_MANAGED_PAGE(pai)) {
...
done2:
  if (need_tlbflush == TRUE) {
    if (options & PMAP_OPTIONS_NOFLUSH) {
      PMAP_UPDATE_TLBS_DELAYED(pmap, vaddr, vaddr + PAGE_SIZE,
...
void pmap_remove_range(pmap_t pmap, vm_map_offset_t start_vaddr,
                       pt_entry_t *spte, pt_entry_t *epte) {
  pmap_remove_range_options(pmap, start_vaddr, spte, epte, PMAP_OPTIONS_REMOVE);
}

static void pmap_remove_range_options(pmap_t pmap, vm_map_offset_t start_vaddr,
                                      pt_entry_t *spte, pt_entry_t *epte,
                                      int options) {
  pt_entry_t *cpte;
  ...
  int ledgers_external, ledgers_reusable, ledgers_internal,
  ...
  for (cpte = spte, vaddr = start_vaddr; cpte < epte;
    ...
    /* remember reference and change */
    if (!is_ept) {
      pmap_phys_attributes[pai] |= *cpte & (PHYS_MODIFIED | PHYS_REFERENCED);
    } else {
      pmap_phys_attributes[pai] |=
          ept_refmod_to_physmap((*cpte & (INTEL_EPT_REF | INTEL_EPT_MOD))) &
...
update_counts:
  ...
  if (pmap != kernel_pmap) {
    ...
    uint64_t net_debit = (ledgers_internal - ledgers_alt_internal) +
 ...
 */
void pmap_remove(pmap_t map, addr64_t s64, addr64_t e64) {
  pmap_remove_options(map, s64, e64, PMAP_OPTIONS_REMOVE);
}
#define PLCHECK_THRESHOLD (2)

void pmap_remove_options(pmap_t map, addr64_t s64, addr64_t e64, int options) {
  pt_entry_t *pde;
  ...
  PMAP_TRACE(PMAP_CODE(PMAP__REMOVE) | DBG_FUNC_START, VM_KERNEL_ADDRHIDE(map),
             VM_KERNEL_ADDRHIDE(s64), VM_KERNEL_ADDRHIDE(e64));
  ...
  while (s64 < e64) {
    ...
    if ((pml4e == NULL) || ((*pml4e & PTE_VALID_MASK(is_ept)) == 0)) {
    ...
    if ((pdpte == NULL) || ((*pdpte & PTE_VALID_MASK(is_ept)) == 0)) {
    ...
    if (pde && (*pde & PTE_VALID_MASK(is_ept))) {
      ...
      pmap_remove_range_options(map, s64, spte, epte, options);
    }
    s64 = l64;
 ...
 *		page.
 */
void pmap_page_protect_options(ppnum_t pn, vm_prot_t prot, unsigned int options,
                               void *arg) {
  pv_hashed_entry_t pvh_eh = PV_HASHED_ENTRY_NULL;
  pv_hashed_entry_t pvh_et = PV_HASHED_ENTRY_NULL;
  ...
  do {
    ...
    pmap_assert2((pa_index(pte_to_pa(*pte)) == pn),
                 "pmap_page_protect: PTE mismatch, pn: 0x%x, pmap: %p, vaddr: "
                 "0x%llx, pte: 0x%llx",
                 pn, pmap, vaddr, *pte);
    ...
    if (remove) {
      ...
      if (pmap != kernel_pmap && (options & PMAP_OPTIONS_COMPRESSOR) &&
      ...
      if (options & PMAP_OPTIONS_NOREFMOD) {
        ...
        if (!is_ept) {
          ...
          pmap_phys_attributes[pai] |=
              ept_refmod_to_physmap((*pte & (INTEL_EPT_REF | INTEL_EPT_MOD))) &
              (PHYS_MODIFIED | PHYS_REFERENCED);
        }
        if ((options & PMAP_OPTIONS_COMPRESSOR_IFF_MODIFIED) &&
            IS_INTERNAL_PAGE(pai) &&
        ...
        if (IS_ALTACCT_PAGE(pai, pv_e)) {
          ...
          if (options & PMAP_OPTIONS_COMPRESSOR) {
            pmap_ledger_credit(pmap, task_ledgers.internal_compressed,
                               PAGE_SIZE);
            pmap_ledger_credit(
                pmap, task_ledgers.alternate_accounting_compressed, PAGE_SIZE);
          }
        } else if (IS_REUSABLE_PAGE(pai)) {
 ...
 */
void phys_attribute_clear(ppnum_t pn, int bits, unsigned int options,
                          void *arg) {
  pv_rooted_entry_t pv_h;
  ...
  if ((bits & PHYS_MODIFIED) && (options & PMAP_OPTIONS_NOFLUSH) &&
      arg == NULL) {
  ...
  ept_bits_to_clear =
      (unsigned)physmap_refmod_to_ept(bits & (PHYS_MODIFIED | PHYS_REFERENCED));
    ...
    do {
      ...
      if (pte_bits) {
        ...
        if (!(options & PMAP_OPTIONS_NOFLUSH)) {
          ...
          PMAP_UPDATE_TLBS_DELAYED(pmap, va, va + PAGE_SIZE,
                                   (pmap_flush_context *)arg);
        } else {
...
int phys_attribute_test(ppnum_t pn, int bits) {
   ...
   */
  if (attributes != bits && pv_h->pmap != PMAP_NULL) {
    /*
     * There are some mappings.
     */
    pv_e = (pv_hashed_entry_t)pv_h;
    do {
      ...
      if (!is_ept) {
        ...
        attributes |= (int)(ept_refmod_to_physmap(
                                (*pte & (INTEL_EPT_REF | INTEL_EPT_MOD))) &
                            (PHYS_MODIFIED | PHYS_REFERENCED));
      }
 ...
 */
void pmap_change_wiring(pmap_t map, vm_map_offset_t vaddr, boolean_t wired) {
  pt_entry_t *pte;

  PMAP_LOCK_SHARED(map);

  if ((pte = pmap_pte(map, vaddr)) == PT_ENTRY_NULL) {
    panic("pmap_change_wiring(%p,0x%llx,%d): pte missing", map, vaddr, wired);
  }
...
vm_offset_t pmap_map_bd(vm_offset_t virt, vm_map_offset_t start_addr,
  ...
  template = pa_to_pte(start_addr) | INTEL_PTE_REF | INTEL_PTE_MOD |
             INTEL_PTE_WIRED | INTEL_PTE_VALID;
...
void pmap_alias(vm_offset_t ava, vm_map_offset_t start_addr,
                vm_map_offset_t end_addr, vm_prot_t prot,
  ...
  prot_template =
      INTEL_PTE_REF | INTEL_PTE_MOD | INTEL_PTE_WIRED | INTEL_PTE_VALID;
  if ((prot & VM_PROT_EXECUTE) == 0) {
...
mach_vm_size_t pmap_query_resident(pmap_t pmap, addr64_t s64, addr64_t e64,
  ...
  while (s64 < e64) {
    ...
    if (pde && (*pde & PTE_VALID_MASK(is_ept))) {
      if (*pde & PTE_PS) {
        ...
        spte = pmap_pte(pmap, (s64 & ~(PDE_MAPPED_SIZE - 1)));
        spte = &spte[ptenum(s64)];
        epte = &spte[intel_btop(l64 - s64)];
  ...
  PMAP_TRACE(PMAP_CODE(PMAP__QUERY_RESIDENT) | DBG_FUNC_END, resident_bytes);
...
kern_return_t pmap_query_page_info(pmap_t pmap, vm_map_offset_t va,
  ...
  if (!pde_p || !(*pde_p & PTE_VALID_MASK(is_ept)) || (*pde_p & PTE_PS)) {
    goto done;
  }
...
bool pmap_get_jit_entitled(__unused pmap_t pmap) {
  /* The x86 pmap layer does not care if a map is using JIT. */
  return false;
}

void pmap_set_tpro(__unused pmap_t pmap) {
  /* The x86 pmap layer does not care if a map is using TPRO */
  return;
}

bool pmap_get_tpro(__unused pmap_t pmap) {
  /* The x86 pmap layer does not care if a map is using TPRO */
  return false;
}
...
void pmap_trim(__unused pmap_t grand, __unused pmap_t subord,
               __unused addr64_t vstart, __unused uint64_t size) {
  return;
}

__dead2 void pmap_ledger_verify_size(size_t size) {
  panic("%s: unsupported, "
        "size=%lu",
        __func__, size);
}

__dead2 ledger_t pmap_ledger_alloc(void) { panic("%s: unsupported", __func__); }

__dead2 void pmap_ledger_free(ledger_t ledger) {
  panic("%s: unsupported, "
        "ledger=%p",
        __func__, ledger);
}
...
void *pmap_map_compressor_page(ppnum_t pn) {
  assertf(IS_MANAGED_PAGE(ppn_to_pai(pn)),
          "%s called on non-managed page 0x%08x", __func__, pn);
