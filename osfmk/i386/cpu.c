...
/*ARGSUSED*/
kern_return_t cpu_info_count(__unused processor_flavor_t flavor,
                             unsigned int *count) {
  *count = 0;
  return KERN_FAILURE;
}
...
void cpu_exit_wait(int cpu) {
  ...
  while ((cdp->lcpu.state != LCPU_HALT) && (cdp->lcpu.state != LCPU_OFF) &&
         !cdp->lcpu.stopped) {
...
ast_t *ast_pending(void) { return &current_cpu_datap()->cpu_pending_ast; }
...
cpu_type_t cpu_type(void) { return current_cpu_datap()->cpu_type; }

cpu_subtype_t cpu_subtype(void) { return current_cpu_datap()->cpu_subtype; }
...
const char *processor_to_datastring(const char *prefix,
  ...
  act = ml_validate_nofault((vm_offset_t)cpup->cpu_active_thread,
            ...
            ? cpup->cpu_active_thread
            : NULL;

  snprintf(
      printBuf, sizeof(printBuf),
      "%s: tCPU %u (%d) [tid=0x%llx(bp=%d sp=%d) s=0x%x ps=0x%x cpa=0x%x "
      "spa=0x%llx pl=%d il=%d r=%d]",
      prefix, cpu_num, target_processor->state, act ? act->thread_id : ~0ULL,
      act ? act->base_pri : -1, act ? act->sched_pri : -1, cpup->cpu_signals,
      cpup->cpu_prior_signals, cpup->cpu_pending_ast,
      target_processor->processor_set->pending_AST_URGENT_cpu_mask,
      cpup->cpu_preemption_level, cpup->cpu_interrupt_level, cpup->cpu_running);

  return (const char *)&printBuf[0];
}

void abandon_preemption_disable_measurement(void) { /* stub for libpthread */ }
