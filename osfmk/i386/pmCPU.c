...
uint64_t cpu_itime_bins[CPU_ITIME_BINS] = {
    16 * NSEC_PER_USEC,   32 * NSEC_PER_USEC,    64 * NSEC_PER_USEC,
    128 * NSEC_PER_USEC,  256 * NSEC_PER_USEC,   512 * NSEC_PER_USEC,
    1024 * NSEC_PER_USEC, 2048 * NSEC_PER_USEC,  4096 * NSEC_PER_USEC,
    8192 * NSEC_PER_USEC, 16384 * NSEC_PER_USEC, 32768 * NSEC_PER_USEC};
...
static inline void machine_classify_interval(uint64_t interval, uint64_t *bins,
                                             uint64_t *binvals,
                                             uint32_t nbins) {
  uint32_t i;
  for (i = 0; i < nbins; i++) {
...
void machine_idle(void) {
  ...
  if ((ctime >= esdeadline) && (ctime < ehdeadline) &&
    ...
    TCOAL_DEBUG(
        0xCCCC0000, ctime, my_cpu->rtclock_timer.queue.earliest_soft_deadline,
        my_cpu->rtclock_timer.deadline, idle_pending_timers_processed, 0);
  }
  ...
  machine_classify_interval(rtime, &my_cpu->cpu_rtimes[0], &cpu_rtime_bins[0],
                            CPU_RTIME_BINS);
#if CST_DEMOTION_DEBUG
  uint32_t cl = 0, ch = 0;
  ...
  if (pmInitDone && pmDispatch != NULL && pmDispatch->MachineIdle != NULL) {
  ...
  machine_classify_interval(itime, &my_cpu->cpu_itimes[0], &cpu_itime_bins[0],
                            CPU_ITIME_BINS);
#if CST_DEMOTION_DEBUG
  cl = ch = 0;
  rdmsr_carefully(MSR_IA32_CORE_C3_RESIDENCY, &cl, &ch);
...
machine_idle_exit:
  ...
  if (do_process_pending_timers) {
    TCOAL_DEBUG(0xBBBB0000 | DBG_FUNC_START, ctime, esdeadline, ehdeadline,
                idle_pending_timers_processed, 0);
    ...
    TCOAL_DEBUG(0xBBBB0000 | DBG_FUNC_END, ctime, esdeadline,
                idle_pending_timers_processed, 0, 0);
  }
#if CST_DEMOTION_DEBUG
  uint64_t nwakeups = my_cpu->cpu_wakeups_issued_total;

  if ((nwakeups == cwakeups) &&
      (topoParms.nLThreadsPerPackage == my_cpu->lcpu.package->num_idle)) {
...
void pmCPUHalt(uint32_t reason) {
  ...
  default:
    ...
    if (pmInitDone && pmDispatch != NULL && pmDispatch->pmCPUHalt != NULL) {
      /*
...
void pmMarkAllCPUsOff(void) {
  if (pmInitDone && pmDispatch != NULL && pmDispatch->markAllCPUsOff != NULL) {
    (*pmDispatch->markAllCPUsOff)();
  }
}

static void pmInitComplete(void) {
  if (earlyTopology && pmDispatch != NULL &&
...
x86_lcpu_t *pmGetLogicalCPU(int cpu) { return cpu_to_lcpu(cpu); }

x86_lcpu_t *pmGetMyLogicalCPU(void) {
  cpu_data_t *cpup = current_cpu_datap();
...
static x86_core_t *pmGetMyCore(void) {
  cpu_data_t *cpup = current_cpu_datap();

  return cpup->lcpu.core;
}

static x86_die_t *pmGetDie(int cpu) { return cpu_to_die(cpu); }

static x86_die_t *pmGetMyDie(void) {
  cpu_data_t *cpup = current_cpu_datap();

  return cpup->lcpu.die;
}

static x86_pkg_t *pmGetPackage(int cpu) { return cpu_to_package(cpu); }

static x86_pkg_t *pmGetMyPackage(void) {
  cpu_data_t *cpup = current_cpu_datap();

  return cpup->lcpu.package;
}

static void pmLockCPUTopology(int lock) {
  if (lock) {
 ...
 */
uint64_t pmCPUGetDeadline(cpu_data_t *cpu) {
  uint64_t deadline = 0;

  if (pmInitDone && pmDispatch != NULL && pmDispatch->GetDeadline != NULL) {
...
uint64_t pmCPUSetDeadline(cpu_data_t *cpu, uint64_t deadline) {
  if (pmInitDone && pmDispatch != NULL && pmDispatch->SetDeadline != NULL) {
 ...
 */
void pmCPUDeadline(cpu_data_t *cpu) {
  if (pmInitDone && pmDispatch != NULL && pmDispatch->Deadline != NULL) {
 ...
 */
boolean_t pmCPUExitIdle(cpu_data_t *cpu) {
  boolean_t do_ipi;

  if (pmInitDone && pmDispatch != NULL && pmDispatch->exitIdle != NULL) {
...
kern_return_t pmCPUExitHalt(int cpu) {
  ...
  if (pmInitDone && pmDispatch != NULL && pmDispatch->exitHalt != NULL) {
...
kern_return_t pmCPUExitHaltToOff(int cpu) {
  ...
  if (pmInitDone && pmDispatch != NULL && pmDispatch->exitHaltToOff != NULL) {
 ...
 */
void pmCPUMarkRunning(cpu_data_t *cpu) {
  cpu_data_t *cpup = current_cpu_datap();

  if (pmInitDone && pmDispatch != NULL && pmDispatch->markCPURunning != NULL) {
...
int pmCPUControl(uint32_t cmd, void *datap) {
  ...
  if (pmDispatch != NULL && pmDispatch->pmCPUControl != NULL) {
...
void pmTimerSave(void) {
  if (pmDispatch != NULL && pmDispatch->pmTimerStateSave != NULL) {
...
void pmTimerRestore(void) {
  if (pmDispatch != NULL && pmDispatch->pmTimerStateRestore != NULL) {
 ...
 */
unsigned ml_get_maxsnoop(void) {
  uint64_t max_snoop = 0;

  if (pmInitDone && pmDispatch != NULL && pmDispatch->getMaxSnoop != NULL) {
...
uint32_t ml_get_maxbusdelay(void) {
  uint64_t max_delay = 0;

  if (pmInitDone && pmDispatch != NULL && pmDispatch->getMaxBusDelay != NULL) {
 ...
 */
void ml_set_maxbusdelay(uint32_t mdelay) {
  uint64_t maxdelay = mdelay;

  if (pmDispatch != NULL && pmDispatch->setMaxBusDelay != NULL) {
...
uint64_t ml_get_maxintdelay(void) {
  uint64_t max_delay = 0;

  if (pmDispatch != NULL && pmDispatch->getMaxIntDelay != NULL) {
 ...
 */
void ml_set_maxintdelay(uint64_t mdelay) {
  if (pmDispatch != NULL && pmDispatch->setMaxIntDelay != NULL) {
...
boolean_t ml_get_interrupt_prewake_applicable() {
  boolean_t applicable = FALSE;

  if (pmInitDone && pmDispatch != NULL &&
...
void pmSafeMode(x86_lcpu_t *lcpu, uint32_t flags) {
  if (pmDispatch != NULL && pmDispatch->pmCPUSafeMode != NULL) {
...
void machine_run_count(uint32_t count) {
  if (pmDispatch != NULL && pmDispatch->pmSetRunCount != NULL) {
...
processor_t machine_choose_processor(processor_set_t pset,
  ...
  if (pmDispatch != NULL && pmDispatch->pmChooseCPU != NULL) {
...
static int pmThreadGetUrgency(uint64_t *rt_period, uint64_t *rt_deadline) {
...
uint64_t urgency_notification_assert_abstime_threshold,
...
void thread_tell_urgency(thread_urgency_t urgency, uint64_t rt_period,
  ...
  boolean_t urgency_assert =
      (urgency_notification_assert_abstime_threshold != 0);
  ...
  if (__improbable((urgency_assert == TRUE))) {
    ...
    if (__improbable(delta > urgency_notification_max_recorded)) {
      ...
      if (__improbable(
              (delta > urgency_notification_assert_abstime_threshold) &&
              !machine_timeout_suspended())) {
        panic("Urgency notification callout %p exceeded threshold, 0x%llx "
              "abstime units",
              pmDispatch->pmThreadTellUrgency, delta);
      }
    }
  }
...
void machine_thread_going_on_core(__unused thread_t new_thread,
...
void machine_max_runnable_latency(__unused uint64_t bg_max_latency,
...
void machine_switch_perfcontrol_context(
    __unused perfcontrol_event event, __unused uint64_t timestamp,
    __unused uint32_t flags, __unused uint64_t new_thread_same_pri_latency,
    __unused thread_t old, __unused thread_t new) {}
...
void machine_perfcontrol_running_timer_expire(
    __unused uint64_t now, __unused uint32_t flags, __unused int cpu_id,
...
static uint32_t pmGetSavedRunCount(void) { return saved_run_count; }

/*
 * Returns the root of the package tree.
 */
x86_pkg_t *pmGetPkgRoot(void) { return x86_pkgs; }

static boolean_t pmCPUGetHibernate(int cpu) {
  return cpu_datap(cpu)->cpu_hibernate;
}

processor_t pmLCPUtoProcessor(int lcpu) {
  return cpu_datap(lcpu)->cpu_processor;
}

static void pmReSyncDeadlines(int cpu) {
...
static void pmGetNanotimeInfo(pm_rtc_nanotime_t *rtc_nanotime) {
  /*
   * Make sure that nanotime didn't change while we were reading it.
   */
  do {
    rtc_nanotime->generation =
        pal_rtc_nanotime_info.generation; /* must be first */
...
uint32_t pmTimerQueueMigrate(int target_cpu) {
  /* Call the etimer code to do this. */
  return (target_cpu != cpu_number()) ? timer_queue_migrate_cpu(target_cpu) : 0;
}
...
void pmKextRegister(uint32_t version, pmDispatch_t *cpuFuncs,
  ...
  if (cpuFuncs != NULL) {
    if (pmDispatch) {
      panic("Attempt to re-register power management interface--AICPM present "
            "in xcpm mode? %p->%p",
            pmDispatch, cpuFuncs);
    }
    ...
    if (earlyTopology && pmDispatch->pmCPUStateInit != NULL) {
      (*pmDispatch->pmCPUStateInit)();
      earlyTopology = FALSE;
    }
