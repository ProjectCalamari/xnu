...
#define DECLAREULL(SYM, VAL)                                                   \
  __asm("DEFINITION__define__" SYM ":\t .ascii \"%0\""                         \
        :                                                                      \
...
#define ISF64_(x) offsetof(x86_64_intr_stack_frame_t, x)
  ...
  DECLARE("CPU_INTERRUPT_LEVEL", offsetof(cpu_data_t, cpu_interrupt_level));
  DECLARE("CPU_NEED_SEGCHK", offsetof(cpu_data_t, cpu_curthread_do_segchk));
  DECLARE("MTHR_SEGCHK", MTHR_SEGCHK);
  DECLARE("CPU_NESTED_ISTACK", offsetof(cpu_data_t, cpu_nested_istack));
  DECLARE("CPU_NUMBER_GS", offsetof(cpu_data_t, cpu_number));
  DECLARE("CPU_RUNNING", offsetof(cpu_data_t, cpu_running));
  ...
  DECLARE("CPU_INT_EVENT_TIME", offsetof(cpu_data_t, cpu_int_event_time));

  DECLARE("CPU_TASK_CR3", offsetof(cpu_data_t, cpu_task_cr3));
  DECLARE("CPU_SHADOWTASK_CR3", offsetof(cpu_data_t, cpu_shadowtask_cr3));
  DECLARE("CPU_ACTIVE_CR3", offsetof(cpu_data_t, cpu_active_cr3));
  DECLARE("CPU_KERNEL_CR3", offsetof(cpu_data_t, cpu_kernel_cr3));
  DECLARE("CPU_UCR3", offsetof(cpu_data_t, cpu_ucr3));
  DECLARE("CPU_IP_DESC", offsetof(cpu_data_t, cpu_ip_desc));
#if DEBUG
  DECLARE("CPU_ENTRY_CR3", offsetof(cpu_data_t, cpu_entry_cr3));
  DECLARE("CPU_EXIT_CR3", offsetof(cpu_data_t, cpu_exit_cr3));
#endif
  DECLARE("CPU_TLB_INVALID", offsetof(cpu_data_t, cpu_tlb_invalid));
  DECLARE("CPU_PAGEZERO_MAPPED", offsetof(cpu_data_t, cpu_pagezero_mapped));
  DECLARE("CPU_CURTASK_HAS_LDT", offsetof(cpu_data_t, cpu_curtask_has_ldt));

  DECLARE("CPU_TASK_MAP", offsetof(cpu_data_t, cpu_task_map));
  DECLARE("TASK_MAP_32BIT", TASK_MAP_32BIT);
  DECLARE("TASK_MAP_64BIT", TASK_MAP_64BIT);
  DECLARE("CPU_UBER_ISF", offsetof(cpu_data_t, cpu_uber.cu_isf));
  DECLARE("CPU_UBER_TMP", offsetof(cpu_data_t, cpu_uber.cu_tmp));

  DECLARE("CPU_NANOTIME", offsetof(cpu_data_t, cpu_nanotime));

  DECLARE("CPU_DR7", offsetof(cpu_data_t, cpu_dr7));

  DECLARE("hwIntCnt", offsetof(cpu_data_t, cpu_hwIntCnt));
  DECLARE("CPU_ACTIVE_PCID", offsetof(cpu_data_t, cpu_active_pcid));
  DECLARE("CPU_KERNEL_PCID", offsetof(cpu_data_t, cpu_kernel_pcid));

  DECLARE("CPU_PCID_COHERENTP", offsetof(cpu_data_t, cpu_pmap_pcid_coherentp));
  DECLARE("CPU_PCID_COHERENTP_KERNEL",
          offsetof(cpu_data_t, cpu_pmap_pcid_coherentp_kernel));
  DECLARE("CPU_PMAP_PCID_ENABLED", offsetof(cpu_data_t, cpu_pmap_pcid_enabled));

#ifdef PCID_STATS
  DECLARE("CPU_PMAP_USER_RETS", offsetof(cpu_data_t, cpu_pmap_user_rets));
  DECLARE("CPU_PMAP_PCID_PRESERVES",
          offsetof(cpu_data_t, cpu_pmap_pcid_preserves));
  DECLARE("CPU_PMAP_PCID_FLUSHES", offsetof(cpu_data_t, cpu_pmap_pcid_flushes));
#endif
  DECLARE("CPU_TLB_INVALID_LOCAL", offsetof(cpu_data_t, cpu_tlb_invalid_local));
  DECLARE("CPU_TLB_INVALID_GLOBAL",
          offsetof(cpu_data_t, cpu_tlb_invalid_global));
  DECLARE("CPU_ESTACK", offsetof(cpu_data_t, cd_estack));
  DECLARE("CPU_DSHADOW", offsetof(cpu_data_t, cd_shadow));
#if DEVELOPMENT || DEBUG
  DECLARE("CPU_RTIMES", offsetof(cpu_data_t, cpu_rtimes[0]));
#endif
  ...
  DECLARE("RNT_TSC_BASE", offsetof(pal_rtc_nanotime_t, tsc_base));
  DECLARE("RNT_NS_BASE", offsetof(pal_rtc_nanotime_t, ns_base));
  DECLARE("RNT_SCALE", offsetof(pal_rtc_nanotime_t, scale));
  DECLARE("RNT_SHIFT", offsetof(pal_rtc_nanotime_t, shift));
  DECLARE("RNT_GENERATION", offsetof(pal_rtc_nanotime_t, generation));
