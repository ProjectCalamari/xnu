...
__attribute__((noreturn)) void machdep_syscall(x86_saved_state_t *state) {
  ...
  DEBUG_KPRINT_SYSCALL_MDEP("machdep_syscall: trapno=%d\n", trapno);
  ...
  if (nargs != 0) {
    if (copyin((user_addr_t)regs->uesp + sizeof(int), (char *)args,
               (nargs * sizeof(int)))) {
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_MACHDEP_EXCP_SC_x86, trapno) | DBG_FUNC_END,
       regs->eax);
...
__attribute__((noreturn)) void machdep_syscall64(x86_saved_state_t *state) {
  int trapno;
  const machdep_call_t *entry;
  ...
  DEBUG_KPRINT_SYSCALL_MDEP("machdep_syscall64: trapno=%d\n", trapno);
  ...
  case 3:
    if (!entry->bsd_style) {
      ...
      error = (*entry->routine.args64_bsd_3)(&rval, regs->rdi, regs->rsi,
                                             regs->rdx);
      if (error) {
...
static kern_return_t mach_call_arg_munger32(uint32_t sp,
                                            struct mach_call_args *args,
                                            const mach_trap_t *trapp);

static kern_return_t mach_call_arg_munger32(uint32_t sp,
                                            struct mach_call_args *args,
                                            const mach_trap_t *trapp) {
  if (copyin((user_addr_t)(sp + sizeof(int)), (char *)args,
             trapp->mach_trap_u32_words * sizeof(int))) {
    return KERN_INVALID_ARGUMENT;
  }
...
__attribute__((noreturn)) void mach_call_munger(x86_saved_state_t *state) {
  int argc;
  int call_number;
  mach_call_t mach_call;
  kern_return_t retval;
  struct mach_call_args args = {.arg1 = 0,
                                .arg2 = 0,
                                .arg3 = 0,
                                .arg4 = 0,
                                .arg5 = 0,
  ...
  DEBUG_KPRINT_SYSCALL_MACH("mach_call_munger: code=%d(%s)\n", call_number,
                            mach_syscall_name_table[call_number]);
#if DEBUG_TRACE
  ...
  if (mach_call == (mach_call_t)kern_invalid) {
    DEBUG_KPRINT_SYSCALL_MACH("mach_call_munger: kern_invalid 0x%x\n",
                              regs->eax);
    i386_exception(EXC_SYSCALL, call_number, 1);
    /* NOTREACHED */
  }
  ...
  if (argc) {
    retval = mach_call_arg_munger32(regs->uesp, &args,
                                    &mach_trap_table[call_number]);
...
__attribute__((noreturn)) void mach_call_munger64(x86_saved_state_t *state) {
  int call_number;
  int argc;
  mach_call_t mach_call;
  kern_return_t retval;
  ...
  DEBUG_KPRINT_SYSCALL_MACH("mach_call_munger64: code=%d(%s)\n", call_number,
                            mach_syscall_name_table[call_number]);
  ...
  if (argc) {
    ...
    __nochk_memcpy(&args.arg1, &regs->rdi,
    ...
    if (argc > 6) {
      ...
      if (copyin((user_addr_t)(regs->isf.rsp + sizeof(user_addr_t)),
                 (char *)&args.arg7, copyin_count)) {
 ...
 * dependent thread state info.
 */
void thread_setentrypoint(thread_t thread, mach_vm_address_t entry) {
  pal_register_cache_state(thread, DIRTY);
  if (thread_is_64bit_addr(thread)) {
    x86_saved_state64_t *iss64;
...
void *get_user_regs(thread_t th) {
  pal_register_cache_state(th, DIRTY);
  return USER_STATE(th);
}

void *find_user_regs(thread_t thread) { return get_user_regs(thread); }
...
x86_saved_state_t *find_kern_regs(thread_t thread) {
