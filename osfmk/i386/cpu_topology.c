 ...
 */
void cpu_topology_sort(int ncpus) {
  int i;
  boolean_t istate;
   ...
   */
  if (ncpus > 1) {
    qsort((void *)&cpu_data_ptr[1], ncpus - 1, sizeof(cpu_data_t *),
          lapicid_cmp);
  }
  if (topo_dbg) {
  ...
  for (i = 0; i < ncpus; i++) {
    ...
    if (cpup->cpu_number != i) {
      kprintf("cpu_datap(%d):%p local apic id 0x%x "
              "remapped from %d\n",
              i, cpup, cpup->cpu_phys_number, cpup->cpu_number);
    }
    cpup->cpu_number = i;
 ...
 */
kern_return_t cpu_topology_start_cpu(int cpunum) {
...
static int lapicid_cmp(const void *x, const void *y) {
  ...
  TOPO_DBG("lapicid_cmp(%p,%p) (%d,%d)\n", x, y, cpu_x->cpu_phys_number,
           cpu_y->cpu_phys_number);
...
int ml_get_max_affinity_sets(void) { return x86_affinity_count; }
...
static void init_traptrace_bufs(int entries_per_cpu) {
...
static void gentrace_configure_from_bootargs(const char *ena_prop,
                                             int *ena_valp,
                                             const char *epc_prop, int *epcp,
                                             int max_epc, int def_epc,
                                             int override) {
...
void traptrace_init(void) {
  ...
  gentrace_configure_from_bootargs(
      "traptrace", &enable, "traptrace_epc", &entries_per_cpu,
      TRAPTRACE_MAX_ENTRIES_PER_CPU, DEFAULT_TRAPTRACE_ENTRIES_PER_CPU,
      KF_TRAPTRACE_OVRD);
