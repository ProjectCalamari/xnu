...
#define INTEL_LEAF2_DESC_NUM                                                   \
  (sizeof(intel_cpuid_leaf2_descriptor_table) /                                \
...
static void cpuid_vmm_detect_pv_interface(
    i386_vmm_info_t *info_p, const char *signature,
    bool (*)(i386_vmm_info_t *, const uint32_t, const uint32_t));
static bool cpuid_vmm_detect_applepv_features(i386_vmm_info_t *info_p,
                                              const uint32_t base,
                                              const uint32_t max_leaf);

static inline const cpuid_cache_descriptor_t *cpuid_leaf2_find(uint8_t value) {
  unsigned int i;
...
static const char *cache_type_str[LCACHE_MAX] = {"Lnone", "L1I", "L1D", "L2U",
                                                 "L3U"};
...
static void do_cwas(i386_cpu_info_t *cpuinfo, boolean_t on_slave) {
  ...
  /*
   * Workaround for reclaiming perf counter 3 due to TSX memory ordering
   * erratum. This workaround does not support being forcibly set (since an MSR
   * must be enumerated, lest we #GP when forced to access it.)
   *
   * Note that if disabling TSX is supported, disablement is prefered over
   * forcing TSX transactions to abort.
   */
  if (cpuid_wa_required(CPU_INTEL_TSXDA) == CWA_ON) {
    /* This must be executed on all logical processors */
    wrmsr64(MSR_IA32_TSX_CTRL,
            MSR_IA32_TSXCTRL_TSX_CPU_CLEAR | MSR_IA32_TSXCTRL_RTM_DISABLE);
  ...
  if (((wa_reqd = cpuid_wa_required(CPU_INTEL_SRBDS)) & CWA_ON) != 0 &&
      ((wa_reqd & CWA_FORCE_ON) == CWA_ON ||
       (cpuinfo->cpuid_leaf7_extfeatures & CPUID_LEAF7_EXTFEATURE_SRBDS_CTRL) !=
           0)) {
    /* This must be executed on all logical processors */
...
void cpuid_do_was(void) { do_cwas(cpuid_info(), TRUE); }

/* this function is Intel-specific */
static void cpuid_set_cache_info(i386_cpu_info_t *info_p) {
  ...
  for (index = 0; cpuid_deterministic_supported; index++) {
    ...
    info_p->cpuid_cores_per_package = bitfield32(reg[eax], 31, 26) + 1;
    ...
    if (type != Lnone) {
      cache_size =
          cache_linesize * cache_sets * cache_associativity * cache_partitions;
...
static void cpuid_set_generic_info(i386_cpu_info_t *info_p) {
  uint32_t reg[4];
  char str[128], *p;
  ...
  if (info_p->cpuid_max_ext >= 0x80000004) {
    ...
    strlcpy(info_p->cpuid_brand_string, p, sizeof(info_p->cpuid_brand_string));
  ...
  if (info_p->cpuid_max_ext >= 0x80000006) {
    ...
    info_p->cpuid_address_bits_physical = bitfield32(reg[eax], 7, 0);
    info_p->cpuid_address_bits_virtual = bitfield32(reg[eax], 15, 8);
  }
  ...
  if (info_p->cpuid_features & CPUID_FEATURE_HTT) {
    info_p->cpuid_logical_per_package = bitfield32(reg[ebx], 23, 16);
  } else {
  ...
  if (info_p->cpuid_max_ext >= 0x80000001) {
    ...
    info_p->cpuid_extfeatures = quad(reg[ecx], reg[edx]);
  }
  ...
  if (info_p->cpuid_max_ext >= 0x80000007) {
    ...
    info_p->cpuid_extfeatures |= reg[edx] & (uint32_t)CPUID_EXTFEATURE_TSCI;
    DBG(" extfeatures         : 0x%016llx\n", info_p->cpuid_extfeatures);
  }
  ...
  if (info_p->cpuid_max_basic >= 0xd) {
    ...
    const uint32_t valid = info_p->cpuid_xsave_leafp->extended_state[eax];
  ...
  if (info_p->cpuid_max_basic >= 7) {
    ...
    cpuid_tsx_supported =
        (reg[ebx] & (CPUID_LEAF7_FEATURE_HLE | CPUID_LEAF7_FEATURE_RTM)) != 0;
...
static uint32_t cpuid_set_cpufamily(i386_cpu_info_t *info_p) {
...
void cpuid_set_info(void) {
  ...
  /* Perform pre-cpuid workarounds (since their effects impact values returned
   * via cpuid) */
  ...
  if (!PE_parse_boot_argn("-enable_x86_64h", &enable_x86_64h,
                          sizeof(enable_x86_64h))) {
    boolean_t disable_x86_64h = FALSE;

    if (PE_parse_boot_argn("-disable_x86_64h", &disable_x86_64h,
  ...
  if (enable_x86_64h &&
      ((info_p->cpuid_features & CPUID_X86_64_H_FEATURE_SUBSET) ==
       CPUID_X86_64_H_FEATURE_SUBSET) &&
      ((info_p->cpuid_extfeatures & CPUID_X86_64_H_EXTFEATURE_SUBSET) ==
       CPUID_X86_64_H_EXTFEATURE_SUBSET) &&
      ((info_p->cpuid_leaf7_features & CPUID_X86_64_H_LEAF7_FEATURE_SUBSET) ==
...
static struct table {
  uint64_t mask;
  const char *name;
} feature_map[] = {{CPUID_FEATURE_FPU, "FPU"},
                   ...
                   {0, 0}},
  extfeature_map[] = {{CPUID_EXTFEATURE_SYSCALL, "SYSCALL"},
                      ...
                      {0, 0}},
  leaf7_feature_map[] = {{CPUID_LEAF7_FEATURE_RDWRFSGS, "RDWRFSGS"},
...
static char *cpuid_get_names(struct table *map, uint64_t bits, char *buf,
                             unsigned buf_len) {
  size_t len = 0;
  char *p = buf;
  int i;
...
char *cpuid_get_extfeature_names(uint64_t extfeatures, char *buf,
                                 unsigned buf_len) {
  return cpuid_get_names(extfeature_map, extfeatures, buf, buf_len);
}

char *cpuid_get_leaf7_feature_names(uint64_t features, char *buf,
                                    unsigned buf_len) {
  return cpuid_get_names(leaf7_feature_map, features, buf, buf_len);
}

char *cpuid_get_leaf7_extfeature_names(uint64_t features, char *buf,
                                       unsigned buf_len) {
  return cpuid_get_names(leaf7_extfeature_map, features, buf, buf_len);
}

void cpuid_feature_display(const char *header) {
  char buf[320];

  kprintf("%s: %s", header,
          cpuid_get_feature_names(cpuid_features(), buf, sizeof(buf)));
  if (cpuid_leaf7_features()) {
    kprintf(" %s", cpuid_get_leaf7_feature_names(cpuid_leaf7_features(), buf,
                                                 sizeof(buf)));
  }
  if (cpuid_leaf7_extfeatures()) {
    kprintf(" %s", cpuid_get_leaf7_extfeature_names(cpuid_leaf7_extfeatures(),
                                                    buf, sizeof(buf)));
  }
  kprintf("\n");
...
void cpuid_extfeature_display(const char *header) {
  char buf[256];
...
void cpuid_cpu_display(const char *header) {
  if (cpuid_cpu_infop->cpuid_brand_string[0] != '\0') {
    kprintf("%s: %s\n", header, cpuid_cpu_infop->cpuid_brand_string);
  }
}

unsigned int cpuid_family(void) { return cpuid_info()->cpuid_family; }

uint32_t cpuid_cpufamily(void) { return cpuid_info()->cpuid_cpufamily; }

cpu_type_t cpuid_cputype(void) { return cpuid_info()->cpuid_cpu_type; }

cpu_subtype_t cpuid_cpusubtype(void) { return cpuid_info()->cpuid_cpu_subtype; }
...
uint64_t cpuid_features(void) {
  ...
  if (!checked) {
    ...
    if (PE_parse_boot_argn("_fpu", &fpu_arg[0], sizeof(fpu_arg))) {
      ...
      if (!strncmp("387", fpu_arg, sizeof("387")) ||
          !strncmp("mmx", fpu_arg, sizeof("mmx"))) {
        printf("no sse or sse2\n");
        cpuid_cpu_infop->cpuid_features &=
            ~(CPUID_FEATURE_SSE | CPUID_FEATURE_SSE2 | CPUID_FEATURE_FXSR);
...
uint64_t cpuid_extfeatures(void) { return cpuid_info()->cpuid_extfeatures; }
...
const char *cpuid_vmm_family_string(void) {
...
static void cpuid_init_vmm_info(i386_vmm_info_t *info_p) {
  ...
  if (0 == strcmp(info_p->cpuid_vmm_vendor, CPUID_VMM_ID_VMWARE)) {
    ...
    if (max_vmm_leaf >= 0x40000001) {
      ...
      info_p->cpuid_vmm_kvm_features = quad(reg[edx], reg[eax]);
    }
  } else {
  ...
  cpuid_vmm_detect_pv_interface(info_p, APPLEPV_SIGNATURE,
                                &cpuid_vmm_detect_applepv_features);
...
boolean_t cpuid_vmm_present(void) {
  return (cpuid_features() & CPUID_FEATURE_VMM) ? TRUE : FALSE;
}

i386_vmm_info_t *cpuid_vmm_info(void) {
...
uint32_t cpuid_vmm_family(void) { return cpuid_vmm_info()->cpuid_vmm_family; }

uint64_t cpuid_vmm_get_kvm_features(void) {
  return cpuid_vmm_info()->cpuid_vmm_kvm_features;
}

uint64_t cpuid_vmm_get_applepv_features(void) {
  return cpuid_vmm_info()->cpuid_vmm_applepv_features;
}

cwa_classifier_e cpuid_wa_required(cpu_wa_e wa) {
  i386_cpu_info_t *info_p = &cpuid_cpu_info;
  ...
  if (!bootargs_overrides_processed) {
    if (!PE_parse_boot_argn("cwae", &bootarg_cpu_wa_enables,
                            sizeof(bootarg_cpu_wa_enables))) {
      bootarg_cpu_wa_enables = 0;
    }

    if (!PE_parse_boot_argn("cwad", &bootarg_cpu_wa_disables,
  ...
  case CPU_INTEL_SEGCHK:
    ...
    if ((info_p->cpuid_leaf7_extfeatures & CPUID_LEAF7_EXTFEATURE_ACAPMSR) !=
        0) {
      /* We have ARCHCAP, so check it for either RDCL_NO or MDS_NO */
      uint64_t archcap_msr = rdmsr64(MSR_IA32_ARCH_CAPABILITIES);
      if ((archcap_msr & (MSR_IA32_ARCH_CAPABILITIES_RDCL_NO |
                          MSR_IA32_ARCH_CAPABILITIES_MDS_NO)) != 0) {
        /* Workaround not needed */
        return CWA_OFF;
      }
    }

    if ((info_p->cpuid_leaf7_extfeatures & CPUID_LEAF7_EXTFEATURE_MDCLEAR) !=
        0) {
      return CWA_ON;
    }

    /*
     * If the CPU supports the ARCHCAP MSR and neither the RDCL_NO bit nor the
     * MDS_NO bit are set, OR the CPU does not support the ARCHCAP MSR and the
     * CPU does not enumerate the presence of the enhanced VERW instruction,
     * report that the workaround should not be enabled.
     */
    break;

  case CPU_INTEL_TSXFA:
    /*
     * Note that if TSX was disabled in cpuid_do_precpuid_was(), the cached
     * cpuid info will indicate that RTM is *not* supported and this workaround
     * will not be enabled.
     */
    /*
  ...
  case CPU_INTEL_TSXDA:
    /*
     * Since this workaround might be requested before cpuid_set_info() is
     * complete, we need to invoke cpuid directly when looking for the required
     * bits.
     */
    cpuid_fn(0x7, reg);
    if (reg[edx] & CPUID_LEAF7_EXTFEATURE_ACAPMSR) {
      uint64_t archcap_msr = rdmsr64(MSR_IA32_ARCH_CAPABILITIES);
      /*
       * If this CPU supports TSX (HLE being the proxy for TSX detection) AND it
       * does not include a hardware fix for TAA and it supports the TSX_CTRL
       * MSR, disable TSX entirely. (Note this can be overridden (above) if the
       * cwad boot-arg's value has bit 2 set.)
       */
      if ((reg[ebx] & CPUID_LEAF7_FEATURE_HLE) != 0 &&
          (archcap_msr & (MSR_IA32_ARCH_CAPABILITIES_TAA_NO |
  ...
  case CPU_INTEL_SRBDS:
     ...
     * at least for CPUs that advertise MDS_NO *and* (either TAA_NO is set OR
     * TSX has been disabled).
     */
    if ((info_p->cpuid_leaf7_extfeatures & CPUID_LEAF7_EXTFEATURE_SRBDS_CTRL) !=
        0) {
      if ((info_p->cpuid_leaf7_extfeatures & CPUID_LEAF7_EXTFEATURE_ACAPMSR) !=
          0) {
        uint64_t archcap_msr = rdmsr64(MSR_IA32_ARCH_CAPABILITIES);
...
static void cpuid_do_precpuid_was(void) {
  /*
   * Note that care must be taken not to use any data from the cached cpuid data
   * since it is likely uninitialized at this point.  That includes calling
   * functions that make use of that data as well.
   */

  /* Note the TSX disablement, we do not support force-on since it depends on
   * MSRs being present */
  if (cpuid_wa_required(CPU_INTEL_TSXDA) == CWA_ON) {
    /* This must be executed on all logical processors */
    wrmsr64(MSR_IA32_TSX_CTRL,
...
/*
 * Hunt for Apple Paravirtualization support in the hypervisor class leaves
 * [0x4000_0000-0x4001_0000]. Hypervisor interfaces are expected to be found at
 * 0x100 boundaries for compatibility.
 */
...
static void cpuid_vmm_detect_pv_interface(
  ...
  int hcalls;
  if (PE_parse_boot_argn("hcalls", &hcalls, sizeof(hcalls)) && hcalls == 0) {
    return;
  }
  ...
  for (uint32_t base = 0x40000100; base < 0x40010000; base += 0x100) {
    ...
    vendor[12] = '\0';
    if ((0 == strcmp(vendor, signature)) && (reg[eax] - base) < 0x100 &&
