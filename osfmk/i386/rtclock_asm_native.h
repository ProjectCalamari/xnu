 ...
 *
 *	ns = ((((rdtsc - rnt_tsc_base)<<rnt_shift)*rnt_tsc_scale) / 2**32) +
 * rnt_ns_base;
 *
 * rnt_shift, a constant computed during initialization, is the smallest value
 * for which:
 *
 *	(tscFreq << rnt_shift) > SLOW_TSC_THRESHOLD
 *
 * Where SLOW_TSC_THRESHOLD is about 10e9.  Since most processor's tscFreqs are
 * greater than 1GHz, rnt_shift is usually 0.  rnt_tsc_scale is also a 32-bit
 * constant:
 *
