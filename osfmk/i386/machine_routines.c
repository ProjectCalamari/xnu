...
kern_return_t ml_static_verify_page_protections(uint64_t base, uint64_t size,
  ...
  assert(kc_highest_nonlinkedit_vmaddr > 0 && base > vm_kernel_stext &&
         base < kc_highest_nonlinkedit_vmaddr);
#else /* On release kernels, assume this is a protection mismatch failure. */
  if (kc_highest_nonlinkedit_vmaddr == 0 || base < vm_kernel_stext ||
      base >= kc_highest_nonlinkedit_vmaddr) {
    return KERN_FAILURE;
  }
#endif
...
/*
 * Reclaim memory, by virtual address, that was used in early boot that is no
 * longer needed by the kernel.
 */
...
kern_return_t ml_static_protect(vm_offset_t vmaddr, vm_size_t size,
                                vm_prot_t prot) {
...
vm_size_t ml_nofault_copy(vm_offset_t virtsrc, vm_offset_t virtdst,
  ...
  while (size > 0) {
    ...
    if (!pmap_valid_page(i386_btop(cur_phys_dst)) ||
...
void ml_init_interrupt(void) { (void)ml_set_interrupts_enabled(TRUE); }
...
boolean_t ml_at_interrupt_context(void) { return get_interrupt_level() != 0; }
...
void ml_get_power_state(boolean_t *icp, boolean_t *pidlep) {
  *icp = (get_interrupt_level() != 0);
   ...
   */
  *pidlep = (current_cpu_datap()->lcpu.package->num_idle ==
             topoParms.nLThreadsPerPackage);
}

/* Generate a fake interrupt */
__dead2 void ml_cause_interrupt(void) {
 ...
 */
void ml_thread_policy(thread_t thread, __unused unsigned policy_id,
...
void ml_install_interrupt_handler(void *nub, int source, void *target,
  ...
  PE_install_interrupt_handler(nub, source, target, (IOInterruptHandler)handler,
                               refCon);
...
__dead2 void machine_signal_idle_deferred(__unused processor_t processor) {
  panic("Unimplemented");
}

__dead2 void machine_signal_idle_cancel(__unused processor_t processor) {
  panic("Unimplemented");
}

static void register_cpu(uint32_t lapic_id, processor_t *processor_out,
                         boolean_t boot_cpu) {
  int target_cpu;
  ...
  assert((boot_cpu && (target_cpu == 0)) || (!boot_cpu && (target_cpu != 0)));
 ...
 *
 * See https://support.apple.com/en-us/101870 for when EFI does this. (nvram
 * SMTDisable=%01) When this happens the processors show up in ACPI but they do
 * not get ml_processor_register'ed.
 */
kern_return_t ml_processor_register(cpu_id_t cpu_id, uint32_t lapic_id,
  ...
  *processor_out = this_cpu_datap->cpu_processor;
  ...
  if (cpunum == num_registered - 1) {
    ...
    success =
        os_atomic_cmpxchg(&done_registering_and_starting, false, true, relaxed);
    assert(success);
...
void ml_cpu_get_info_type(ml_cpu_info_t *cpu_infop,
                          cluster_type_t cluster_type __unused) {
...
void ml_get_cluster_type_name(cluster_type_t cluster_type __unused, char *name,
                              size_t name_size) {
  strlcpy(name, "Standard", name_size);
}

unsigned int ml_get_cluster_number_type(cluster_type_t cluster_type __unused) {
  /*
...
unsigned int ml_get_cpu_types(void) { return 1 << CLUSTER_TYPE_SMP; }
...
void ml_panic_trap_to_debugger(__unused const char *panic_format_str,
                               __unused va_list *panic_args,
                               __unused unsigned int reason, __unused void *ctx,
...
static uint64_t virtual_timeout_inflate64(unsigned int vti, uint64_t timeout,
...
static uint32_t virtual_timeout_inflate32(unsigned int vti, uint32_t timeout,
 ...
 */
static uint64_t virtual_timeout_inflate_abs(unsigned int vti,
                                            uint64_t timeout) {
  uint64_t max_timeout;
 ...
 */
static uint64_t virtual_timeout_inflate_tsc(unsigned int vti,
                                            uint64_t timeout) {
 ...
 */
static uint32_t virtual_timeout_inflate_us(unsigned int vti, uint64_t timeout) {
  const uint32_t max_timeout = ~0;
  return virtual_timeout_inflate32(vti, timeout, max_timeout);
}

uint64_t ml_get_timebase_entropy(void) { return __builtin_ia32_rdtsc(); }
...
void ml_init_delay_spin_threshold(int threshold_us) {
  nanoseconds_to_absolutetime(threshold_us * NSEC_PER_USEC,
 ...
 */
void ml_cpu_up(void) { return; }

void ml_cpu_up_update_counts(__unused int cpu_id) { return; }
...
void ml_cpu_down_update_counts(__unused int cpu_id) { return; }

thread_t current_thread(void) { return current_thread_fast(); }

boolean_t ml_is64bit(void) { return cpu_mode_is64bit(); }
...
void ml_cpu_set_ldt(int selector) {
   ...
   */
  if (selector == KERNEL_LDT && current_cpu_datap()->cpu_ldt == KERNEL_LDT) {
    return;
  }
...
void ml_fp_setvalid(boolean_t value) { fp_setvalid(value); }
...
void ml_timer_evaluate(void) {
  ...
  ml_timer_eager_evaluation_max =
      MAX(ml_timer_eager_evaluation_max, (te_end - te_start));
...
uint64_t ml_gpu_stat(thread_t t) { return t->machine.thread_gpu_ns; }

int plctrace_enabled = 0;

void _disable_preemption(void) { disable_preemption_internal(); }

void _enable_preemption(void) { enable_preemption_internal(); }

void plctrace_disable(void) { plctrace_enabled = 0; }

static boolean_t ml_quiescing;

void ml_set_is_quiescing(boolean_t quiescing) { ml_quiescing = quiescing; }

boolean_t ml_is_quiescing(void) { return ml_quiescing; }

uint64_t ml_get_booter_memory_size(void) { return 0; }

void machine_lockdown(void) { x86_64_protect_data_const(); }
...
bool ml_addr_in_non_xnu_stack(__unused uintptr_t addr) {
 ...
 */
void ml_fp_save_area_prealloc(void) { fpnoextflt(); }
