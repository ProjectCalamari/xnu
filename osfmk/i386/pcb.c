...
ZONE_DEFINE_TYPE(iss_zone, "x86_64 saved state", x86_saved_state_t, ZC_NONE);

ZONE_DEFINE_TYPE(ids_zone, "x86_64 debug state", x86_debug_state64_t, ZC_NONE);

/* Forward */

extern void Thread_continue(void);
extern void Load_context(thread_t thread) __attribute__((noreturn));

static void get_exception_state32(thread_t thread, x86_exception_state32_t *es);

static void get_exception_state64(thread_t thread, x86_exception_state64_t *es);

static void get_thread_state32(thread_t thread, x86_thread_state32_t *ts);

static void get_thread_state64(thread_t thread, void *ts, boolean_t full);

static int set_thread_state32(thread_t thread, x86_thread_state32_t *ts);

static int set_thread_state64(thread_t thread, void *ts, boolean_t full);
 ...
 * TRCTRAP.
 */
static boolean_t dr7d_is_valid(uint32_t *dr7d) {
  int i;
...
boolean_t debug_state_is_valid32(x86_debug_state32_t *ds) {
...
boolean_t debug_state_is_valid64(x86_debug_state64_t *ds) {
...
static kern_return_t set_debug_state32(thread_t thread,
...
static kern_return_t set_debug_state64(thread_t thread,
  ...
  if (pcb->ids == NULL) {
    ...
    if (thread->hv_thread_target) {
      hv_callbacks.volatile_state(thread->hv_thread_target, HV_DEBUG_STATE);
    }
#endif
...
static void get_debug_state32(thread_t thread, x86_debug_state32_t *ds) {
...
static void get_debug_state64(thread_t thread, x86_debug_state64_t *ds) {
 ...
 */
void machine_load_context(thread_t new) {
  new->machine.specFlags |= OnProc;
  act_machine_switch_pcb(NULL, new);
  Load_context(new);
}

static void machine_rsb_stuff(void) {
#define RSB_STUFF_SPACE_REQD                                                   \
  (256 + 16) /* 256 bytes plus a buffer of another 16 for misc. */

  asm volatile(".macro RSBST from=0, to=15\n"
               "       call    1f\n"
               "2:\n"
               "       pause\n"
               "       lfence\n"
...
static inline void pmap_switch_context(thread_t ot, thread_t nt, int cnum) {
  ...
  if ((omap != nmap) || (nmap->pmap->pagezero_accessible)) {
    ...
    if (__improbable((nt->machine.mthr_do_segchk & MTHR_RSBST) &&
                     (current_kernel_stack_depth() + RSB_STUFF_SPACE_REQD) <
                         kernel_stack_size)) {
      machine_rsb_stuff();
    }
  }
...
boolean_t machine_thread_on_core(thread_t thread) {
  return thread->machine.specFlags & OnProc;
}

boolean_t machine_thread_on_core_allow_invalid(thread_t thread) {
...
uint32_t get_eflags_exportmask(void) { return EFL_USER_SET; }

/*
 * x86_SAVED_STATE32	 - internal save/restore general register state on 32/64
 * bit processors for 32bit tasks only x86_SAVED_STATE64	 - internal
 * save/restore general register state on 64 bit processors for 64bit tasks only
 * x86_THREAD_STATE32	 - external set/get general register state on 32/64 bit
 * processors for 32bit tasks only x86_THREAD_STATE64	 - external set/get
 * general register state on 64 bit processors for 64bit tasks only
 * x86_SAVED_STATE	 - external set/get general register state on 32/64 bit
 * processors for either 32bit or 64bit tasks x86_FLOAT_STATE32	 -
 * internal/external save/restore float and xmm state on 32/64 bit processors
 *			   for 32bit tasks only
 * x86_FLOAT_STATE64	 - internal/external save/restore float and xmm state on
 * 64 bit processors for 64bit tasks only x86_FLOAT_STATE	 - external
 * save/restore float and xmm state on 32/64 bit processors for either 32bit or
 * 64bit tasks x86_EXCEPTION_STATE32 - external get exception state on 32/64 bit
 * processors for 32bit tasks only x86_EXCEPTION_STATE64 - external get
 * exception state on 64 bit processors for 64bit tasks only x86_EXCEPTION_STATE
 *   - external get exception state on 323/64 bit processors for either 32bit or
 * 64bit tasks
 */

static void get_exception_state64(thread_t thread,
...
static void get_exception_state32(thread_t thread,
...
static int set_thread_state32(thread_t thread, x86_thread_state32_t *ts) {
  ...
  if (!valid_user_segment_selectors(ts->cs, ts->ss, ts->ds, ts->es, ts->fs,
                                    ts->gs)) {
    return KERN_INVALID_ARGUMENT;
  }
...
static int set_thread_state64(thread_t thread, void *state, int full) {
  ...
  if (!IS_USERADDR64_CANONICAL(ts->rsp) || !IS_USERADDR64_CANONICAL(ts->rip)) {
    return KERN_INVALID_ARGUMENT;
  }
...
static void get_thread_state64(thread_t thread, void *state, boolean_t full) {
  x86_thread_state64_t *ts;
  x86_saved_state64_t *saved_state;
  ...
  if (full == TRUE) {
    ...
    ((x86_thread_full_state64_t *)state)->gsbase = thread->machine.cthread_self;
  }
...
kern_return_t machine_thread_state_convert_to_user(
    __unused thread_t thread, __unused thread_flavor_t flavor,
    __unused thread_state_t tstate, __unused mach_msg_type_number_t *count,
...
kern_return_t
machine_thread_siguctx_pointer_convert_to_user(__unused thread_t thread,
...
kern_return_t
machine_thread_function_pointers_convert_from_user(__unused thread_t thread,
...
kern_return_t machine_thread_set_state(thread_t thr_act, thread_flavor_t flavor,
  ...
  case x86_SAVED_STATE32: {
    ...
    /* Check segment selectors are safe */
    if (!valid_user_segment_selectors(state->cs, state->ss, state->ds,
  ...
  case x86_SAVED_STATE64: {
    ...
    saved_state->isf.rflags =
        (state->isf.rflags & ~EFL_USER_CLEAR) | EFL_USER_SET;
  ...
  case x86_FLOAT_STATE: {
    ...
    if (state->fsh.flavor == x86_FLOAT_STATE64 &&
    ...
    if (state->fsh.flavor == x86_FLOAT_STATE32 &&
  ...
  case x86_AVX512_STATE: {
    ...
    /* Flavors are defined to have sequential values: 32-bit, 64-bit,
     * non-specific */
    /* 64-bit flavor? */
    if (state->ash.flavor == (flavor - 1) &&
        state->ash.count == _MachineStateCount[flavor - 1] &&
        thread_is_64bit_addr(thr_act)) {
      return fpu_set_fxstate(thr_act, (thread_state_t)&state->ufs.as64,
                             flavor - 1);
    }
    /* 32-bit flavor? */
    if (state->ash.flavor == (flavor - 2) &&
        ...
        !thread_is_64bit_addr(thr_act)) {
      return fpu_set_fxstate(thr_act, (thread_state_t)&state->ufs.as32,
                             flavor - 2);
    }
    return KERN_INVALID_ARGUMENT;
  }
  ...
  case x86_THREAD_STATE: {
    ...
    if (state->tsh.flavor == x86_THREAD_STATE64 &&
      ...
      return set_thread_state64(thr_act, &state->uts.ts64, FALSE);
               ...
               thread_is_64bit_addr(thr_act) &&
               get_threadtask(thr_act)->i386_ldt != 0) {
...
mach_vm_address_t machine_thread_pc(thread_t thr_act) {
  if (thread_is_64bit_addr(thr_act)) {
...
void machine_thread_reset_pc(thread_t thr_act, mach_vm_address_t pc) {
  pal_register_cache_state(thr_act, DIRTY);
...
kern_return_t machine_thread_get_state(thread_t thr_act, thread_flavor_t flavor,
  ...
  case x86_INSTRUCTION_STATE: {
    ...
    if (src_state != 0 &&
        (src_state->insn_stream_valid_bytes > 0 || src_state->out_of_synch)) {
#if DEVELOPMENT || DEBUG
      extern int insnstream_force_cacheline_mismatch;
#endif
      size_t byte_count = (src_state->insn_stream_valid_bytes >
                           x86_INSTRUCTION_STATE_MAX_INSN_BYTES)
                              ? x86_INSTRUCTION_STATE_MAX_INSN_BYTES
                              : src_state->insn_stream_valid_bytes;
      if (byte_count > 0) {
...
#if DEVELOPMENT || DEBUG
      state->out_of_synch =
          src_state->out_of_synch || insnstream_force_cacheline_mismatch;
  ...
  case x86_LAST_BRANCH_STATE: {
    ...
    /* Callers to this function are assumed to be from user space and the LBR
     * values will be filtered accordingly */
    if (i386_filtered_lbr_state_to_mach_thread_state(
            thr_act, (last_branch_state_t *)tstate, true) < 0) {
      *count = 0;
      return KERN_INVALID_ARGUMENT;
    }
...
kern_return_t machine_thread_get_kern_state(thread_t thread,
  ...
  case x86_THREAD_STATE32: {
    ...
    if (!is_saved_state32(int_state) || *count < x86_THREAD_STATE32_COUNT) {
      return KERN_INVALID_ARGUMENT;
    }
  ...
  case x86_THREAD_STATE64: {
    ...
    if (!is_saved_state64(int_state) || *count < x86_THREAD_STATE64_COUNT) {
      return KERN_INVALID_ARGUMENT;
    }
 ...
 */
void machine_thread_init(void) { fpu_module_init(); }
...
void *act_thread_csave(void) {
  ...
  if (thread_is_64bit_addr(thr_act)) {
    ...
    kret = machine_thread_get_state(thr_act, x86_DEBUG_STATE64,
                                    (thread_state_t)&ic64->ds, &val);
    if (kret != KERN_SUCCESS) {
      kfree_data(ic64, sizeof(struct x86_act_context64));
      return (void *)0;
    }
    ...
    kret = machine_thread_get_state(thr_act, x86_DEBUG_STATE32,
                                    (thread_state_t)&ic32->ds, &val);
    if (kret != KERN_SUCCESS) {
      kfree_data(ic32, sizeof(struct x86_act_context32));
      return (void *)0;
    }
...
void act_thread_cfree(__unused void *ctx) { /* XXX - Unused */ }
 ...
 */
void copy_debug_state32(x86_debug_state32_t *src, x86_debug_state32_t *target,
                        boolean_t all) {
  if (all) {
    target->dr4 = src->dr4;
    target->dr5 = src->dr5;
 ...
 * machine_thread_set_state()).
 */
void copy_debug_state64(x86_debug_state64_t *src, x86_debug_state64_t *target,
                        boolean_t all) {
  if (all) {
    target->dr4 = src->dr4;
    target->dr5 = src->dr5;
