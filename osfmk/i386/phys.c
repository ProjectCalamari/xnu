...
void pmap_zero_page_with_options(ppnum_t pn, __unused int options) {
  pmap_zero_page(pn);
}
...
void pmap_copy_part_page(ppnum_t psrc, vm_offset_t src_offset, ppnum_t pdst,
  ...
  bcopy_phys((addr64_t)src + (src_offset & INTEL_OFFMASK),
             (addr64_t)dst + (dst_offset & INTEL_OFFMASK), len);
}
 ...
 */
void pmap_copy_part_lpage(__unused vm_offset_t src, __unused ppnum_t pdst,
 ...
 */
void pmap_copy_part_rpage(__unused ppnum_t psrc,
                          __unused vm_offset_t src_offset,
                          __unused vm_offset_t dst, __unused vm_size_t len) {
  assert(psrc != vm_page_fictitious_addr);
  assert(psrc != vm_page_guard_addr);
...
__private_extern__ int ml_copy_phys(addr64_t src64, addr64_t dst64,
                                    vm_size_t bytes) {
  void *src, *dst;
  int err = 0;
  ...
  if (debug_pa) {
     ...
     */
    pmap_store_pte(FALSE, debugger_ptep,
                   debug_pa | INTEL_PTE_NCACHE | INTEL_PTE_RW | INTEL_PTE_REF |
                       INTEL_PTE_MOD | INTEL_PTE_VALID);
  ...
  if (((((uint32_t)src64 & (I386_PGBYTES - 1)) + bytes) > I386_PGBYTES) ||
