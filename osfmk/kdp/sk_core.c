...
#define kDbgIdSecKernInfo                                                      \
  (DEBUG_RECORD_ID_LONG('S', 'E', 'C', 'K', 'E', 'R', 'N', 0))

static kern_return_t sc_init_cdbg(void) {
  ...
  if (!PE_consistent_debug_lookup_entry(kDbgIdSecKernInfo, &sk_debug_phys,
                                        &sk_debug_len)) {
...
} page_config[] = {[0] = {.size = ARM_16K_TT_L0_SIZE,
...
static inline uint64_t sc_va_size_bits(uint64_t tcr) {
  return 64 - ((tcr >> TCR_T1SZ_SHIFT) & TCR_TSZ_MASK);
}

/* Return page size in bits based on TCR. */
static inline uint64_t sc_va_page_size_bits(uint64_t tcr) {
...
static kern_return_t sc_boostrap_va(struct dbg_kernel_header *hdrp) {
  /* validate and set VA bit mask */
  const uint64_t va_size_bits = sc_va_size_bits(hdrp->tcr);
  if (va_size_bits == 0) {
    kern_coredump_log(NULL, "secure_core: Invalid VA bit size: 0x%llx",
                      hdrp->tcr);
    return KERN_FAILURE;
  }
  ...
  if (page_size_bits == 0) {
    kern_coredump_log(NULL, "secure_core: Invalid page size bits: 0x%llx",
                      hdrp->tcr);
    return KERN_FAILURE;
  }

  sc_page_levels =
      (int)(((va_size_bits - page_size_bits) + ((page_size_bits - 3) - 1)) /
            (page_size_bits - 3));
  return KERN_SUCCESS;
}
 ...
 * physical address. A physical address is then mapped back to XNU's PAPT
 * virtual address.
 *
 * Note: panics if SPTM has not unlocked SK_DOMAIN memory access.
 */
static vm_map_address_t sc_cL4_kvtov(vm_map_address_t vaddr) {
  tt_entry_t *ttp =
      (tt_entry_t *)phystokv(sk_dbg_header->ttbr1 & ARM_TTE_PA_MASK);
 ...
 */
static uuid_t *sc_find_uuid_cdbg(void) {
  uuid_t *uuid = NULL;
  ...
  case ASTRIS_UUID_MAGIC: {
    ...
    if (signpost->version != ASTRIS_UUID_VERSION) {
      kern_coredump_log(
          NULL, "secure_core: unsupported astris signpost version 0x%x\n",
          signpost->version);
      break;
    }
  ...
  case DEBUG_SIGNPOST_MAGIC: {
    ...
    if (signpost->version != DEBUG_SIGNPOST_VERSION) {
      kern_coredump_log(
          NULL, "secure_core: unsupported debug signpost version 0x%x\n",
          signpost->version);
      break;
    }
...
typedef kern_return_t (*papt_walk_callback)(pmap_paddr_t start,
                                            pmap_paddr_t end, void *context);
 ...
 */
static inline bool is_sk_type(int type) {
 ...
 */
static kern_return_t papt_walk(papt_walk_callback cb, void *context) {
  pmap_paddr_t seg_start = 0;
  pmap_paddr_t seg_end = 0;
  pmap_paddr_t last_paddr = 0;
  kern_return_t kr = KERN_SUCCESS;

  for (vm_map_address_t vaddr = physmap_base; vaddr < physmap_end;
       vaddr += PAGE_SIZE_64) {
    ...
    if (kr != KERN_SUCCESS) {
      kern_coredump_log(NULL,
                        "secure_core: PAPT walk callback failed with %d\n", kr);
      return kr;
    }
...
static kern_return_t secure_summary(pmap_paddr_t start, pmap_paddr_t end,
                                    void *refcon) {
...
static kern_return_t sk_dump_init(void *refcon, void *context) {
  ...
  if (sk_dbg_header->version != DBGREG_KERNEL_HEADER_VERSION) {
    kern_coredump_log(context,
                      "secure_core: Debug header version (%d) mismatch\n",
...
static kern_return_t sk_dump_get_summary(void *refcon,
...
static kern_return_t secure_seg_desc(pmap_paddr_t start, pmap_paddr_t end,
                                     void *context) {
...
sk_dump_save_seg_desc(void *refcon __unused,
                      core_save_segment_descriptions_cb callback,
                      void *context) {
  kern_return_t ret;
  secure_segment_desc_t ssd = {.ssd_callback = callback,
                               .ssd_context = context};

  ret = papt_walk(secure_seg_desc, &ssd);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(
        context, "secure_core: Unable to save segment description: %d\n", ret);
    return ret;
  }

  /* Save cL4's debug header as a special segment. */
  return callback(sk_dbg_header_seg_va,
                  sk_dbg_header_seg_va + sizeof(struct dbg_kernel_header),
                  context);
}
...
static kern_return_t secure_seg_data(pmap_paddr_t start, pmap_paddr_t end,
                                     void *context) {
  secure_segment_data_t *ssd = context;

  return ssd->ssd_callback((void *)phystokv(start), end - start,
                           ssd->ssd_context);
}

static kern_return_t sk_dump_save_seg_data(void *refcon __unused,
                                           core_save_segment_data_cb callback,
                                           void *context) {
  kern_return_t ret;
  secure_segment_data_t ssd = {.ssd_callback = callback,
                               .ssd_context = context};
...
sk_dump_save_thread_state(void *refcon __unused, void *buf __unused,
                          core_save_thread_state_cb callback __unused,
...
static kern_return_t
sk_dump_save_sw_vers_detail(void *refcon, core_save_sw_vers_detail_cb callback,
                            void *context) {
  struct secure_core_context *scc = refcon;
...
static kern_return_t
sk_dump_save_note_summary(void *refcon __unused,
                          core_save_note_summary_cb callback, void *context) {
  return callback(1, sizeof(seckern_note_t), context);
}

static kern_return_t
sk_dump_save_note_desc(void *refcon __unused,
                       core_save_note_descriptions_cb callback, void *context) {
  return callback(SECKERN_DATA_OWNER, sizeof(seckern_note_t), context);
}

static kern_return_t sk_dump_save_note_data(void *refcon __unused,
                                            core_save_note_data_cb callback,
                                            void *context) {
  seckern_note_t seckern_note = {.version = SECKERN_VER,
...
bool sk_core_enabled(void) { return SPTMArgs->sk_bootstrapped; }
...
bool sk_core_enabled(void) { return false; }

void sk_core_init(void) {}

uint64_t sk_core_size(void) { return 0; }
