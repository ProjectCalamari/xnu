...
void kern_collectth_state_size(uint64_t *tstate_count, uint64_t *ptstate_size) {
  unsigned int i;
  uint64_t tstate_size = 0;

  for (i = 0; i < sizeof(thread_flavor_array) / sizeof(thread_flavor_array[0]);
       i++) {
    tstate_size +=
        sizeof(x86_state_hdr_t) + (thread_flavor_array[i].count * sizeof(int));
  }
...
void kern_collect_userth_state_size(task_t task, uint64_t *tstate_count,
  ...
  size_t num_flavors =
      sizeof(thread_flavor_array) / sizeof(thread_flavor_array[0]);

  for (size_t i = 0; i < num_flavors; i++) {
    per_thread_size +=
        sizeof(x86_state_hdr_t) + flavors[i].count * sizeof(natural_t);
  }
...
void kern_collect_userth_state(task_t task __unused, thread_t thread,
  ...
  for (size_t i = 0; i < num_flavors; i++) {
    ...
    assert(hdr->count + sizeof(x86_state_hdr_t) +
    ...
    hdr = (x86_state_hdr_t *)((uintptr_t)(hdr + 1) +
...
void kern_collectth_state(thread_t thread, void *buffer, uint64_t size,
  ...
  *iter = NULL;
  ...
  for (i = 0; i < sizeof(thread_flavor_array) / sizeof(thread_flavor_array[0]);
    ...
    *(x86_state_hdr_t *)((uintptr_t)buffer + hoffset) = thread_flavor_array[i];
    ...
    if (thread_flavor_array[i].flavor == x86_THREAD_STATE64) {
      x86_thread_state64_t *tstate =
          (x86_thread_state64_t *)((uintptr_t)buffer + hoffset);
      vm_offset_t kstack;
 ...
 */
int kdp_dump_trap(int type, __unused x86_saved_state64_t *saved_state) {
  printf("An unexpected trap (type %d) occurred during the system dump, "
         "terminating.\n",
         type);
 ...
 *
 */
vm_map_offset_t kdp_core_start_addr() { return VM_MIN_KERNEL_AND_KEXT_ADDRESS; }
