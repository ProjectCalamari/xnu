...
mach_vm_size_t kdp_machine_vm_read(mach_vm_address_t src, caddr_t dst,
                                   mach_vm_size_t len) {
  ...
  /* If a different pmap has been specified with kdp_pmap, use it to translate
   * the source (cur_virt_src); otherwise, the source is translated using the
   * kernel_pmap.
   */
  if (kdp_pmap) {
    src_pmap = kdp_pmap;
  ...
  while (resid != 0) {
    if (!(cur_phys_src = kdp_vtophys(src_pmap, cur_virt_src))) {
      goto exit;
    }
    ...
    /* Do a physical copy */
    if (EFAULT == ml_copy_phys(cur_phys_src, cur_phys_dst, (vm_size_t)cnt)) {
      goto exit;
    }
    cur_virt_src += cnt;
    cur_virt_dst += cnt;
...
mach_vm_size_t kdp_machine_phys_read(kdp_readphysmem64_req_t *rq, caddr_t dst,
                                     uint16_t lcpu) {
  ...
  if ((lcpu != KDP_CURRENT_LCPU) && (lcpu != cpu_number())) {
    return (mach_vm_size_t)kdp_x86_xcpu_invoke(
        lcpu, (kdp_x86_xcpu_func_t)kdp_machine_phys_read, rq, dst, 0);
  }
 ...
 *
 */
mach_vm_size_t kdp_machine_vm_write(caddr_t src, mach_vm_address_t dst,
                                    mach_vm_size_t len) {
  ...
  printf("kdp_vm_write: src %p dst %llx len %llx - %08X %08X\n", (void *)src,
         dst, len, ((unsigned int *)src)[0], ((unsigned int *)src)[1]);
#endif
 ...
 *
 */
mach_vm_size_t kdp_machine_phys_write(kdp_writephysmem64_req_t *rq, caddr_t src,
                                      uint16_t lcpu) {
  ...
  if ((lcpu != KDP_CURRENT_LCPU) && (lcpu != cpu_number())) {
    return (mach_vm_size_t)kdp_x86_xcpu_invoke(
        lcpu, (kdp_x86_xcpu_func_t)kdp_machine_phys_write, rq, src, 0);
  }
...
int kdp_machine_ioport_read(kdp_readioport_req_t *rq, caddr_t data,
  ...
  if ((lcpu != KDP_CURRENT_LCPU) && (lcpu != cpu_number())) {
    return (int)kdp_x86_xcpu_invoke(
        lcpu, (kdp_x86_xcpu_func_t)kdp_machine_ioport_read, rq, data, 0);
  }
...
int kdp_machine_ioport_write(kdp_writeioport_req_t *rq, caddr_t data,
  ...
  if ((lcpu != KDP_CURRENT_LCPU) && (lcpu != cpu_number())) {
    return (int)kdp_x86_xcpu_invoke(
        lcpu, (kdp_x86_xcpu_func_t)kdp_machine_ioport_write, rq, data, 0);
  }
...
int kdp_machine_msr64_read(kdp_readmsr64_req_t *rq, caddr_t data,
  ...
  if ((lcpu != KDP_CURRENT_LCPU) && (lcpu != cpu_number())) {
    return (int)kdp_x86_xcpu_invoke(
        lcpu, (kdp_x86_xcpu_func_t)kdp_machine_msr64_read, rq, data, 0);
  }
...
int kdp_machine_msr64_write(kdp_writemsr64_req_t *rq, caddr_t data,
  ...
  if ((lcpu != KDP_CURRENT_LCPU) && (lcpu != cpu_number())) {
    return (int)kdp_x86_xcpu_invoke(
        lcpu, (kdp_x86_xcpu_func_t)kdp_machine_msr64_write, rq, data, 0);
  }
...
void kdp_jtag_coredump_init(void) {
  ...
  kdp_jtag_coredump.pmap_memory_region_count =
      (uint64_t)pmap_memory_region_count;
  kdp_jtag_coredump.pmap_memory_region_t_size =
...
void kdp_machine_init(void) {
  /*
   * If the kernel is running on top of a hypervisor that supports AH#1, it will
   * inform the hypervisor of its debugging info.
   */
