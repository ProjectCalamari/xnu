...
pmap_paddr_t kdp_vtophys(pmap_t pmap, vm_offset_t va) {
  ...
  if (!pmap ||
      ((pmap != kernel_pmap) && ((va < pmap->min) || (va >= pmap->max)))) {
#ifdef KDP_VTOPHYS_DEBUG
    printf("kdp_vtophys(%08x, %016lx) not in range %08x .. %08x\n",
           (unsigned int)pmap, (unsigned long)va,
...
mach_vm_size_t kdp_machine_vm_read(mach_vm_address_t src, caddr_t dst,
  ...
  kprintf("kdp_machine_vm_read1: src %x dst %x len %x - %08X %08X\n", src, dst,
          len, ((unsigned long *)src)[0], ((unsigned long *)src)[1]);
#endif
  ...
  if (kdp_trans_off) {
    ...
    while (resid != 0) {
      ...
      if (!kdp_read_io && ((!pmap_valid_address(cur_phys_dst)) ||
...
mach_vm_size_t kdp_machine_vm_write(caddr_t src, mach_vm_address_t dst,
  ...
  while (resid != 0) {
     ...
     */
    if ((!pmap_valid_address(cur_phys_dst)) ||
        (!pmap_valid_address(cur_phys_src))) {
      goto exit;
    }
...
mach_vm_size_t kdp_machine_phys_write(kdp_writephysmem64_req_t *rq __unused,
...
void kern_collectth_state(thread_t thread __unused, void *buffer, uint64_t size,
                          void **iter) {
  ...
  if (kpcb != NULL) {
    ...
    vm_offset_t kstackptr =
        (vm_offset_t)processor->active_thread->machine.kstackptr;
    arm_kernel_saved_state_t *saved_state =
        (arm_kernel_saved_state_t *)kstackptr;

    state->fp = saved_state->fp;
    state->lr = saved_state->lr;
    state->sp = saved_state->sp;
    state->pc = saved_state->pc_was_in_userspace
...
void kern_collect_userth_state_size(task_t task, uint64_t *tstate_count,
  ...
  if (is64bit) {
    ...
    num_flavors = sizeof(user64_thread_flavor_array) /
                  sizeof(user64_thread_flavor_array[0]);
  } else {
    flavors = user32_thread_flavor_array;
    num_flavors = sizeof(user32_thread_flavor_array) /
                  sizeof(user32_thread_flavor_array[0]);
  }
#else
  flavors = user32_thread_flavor_array;
  num_flavors = sizeof(user32_thread_flavor_array) /
  ...
  for (size_t i = 0; i < num_flavors; i++) {
    per_thread_size +=
        sizeof(arm_state_hdr_t) + (flavors[i].count * sizeof(natural_t));
  }
...
void kern_collect_userth_state(task_t task, thread_t thread, void *buffer,
  ...
  for (size_t i = 0; i < num_flavors; i++) {
    ...
    assert(hdr->count + sizeof(arm_state_hdr_t) +
               ((uintptr_t)hdr - (uintptr_t)buffer) <=
           size);
    ret = machine_thread_get_state(thread, hdr->flavor,
                                   (thread_state_t)(hdr + 1), &hdr->count);
    assert(ret == KERN_SUCCESS);

    hdr = (arm_state_hdr_t *)((uintptr_t)(hdr + 1) +
                              hdr->count * sizeof(natural_t));
  }
}
 ...
 *
 */
vm_map_offset_t kdp_core_start_addr() {
#if defined(__arm64__)
