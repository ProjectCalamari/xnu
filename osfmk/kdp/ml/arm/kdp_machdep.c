...
void kdp_call(void);
int kdp_getc(void);
int machine_trace_thread(thread_t thread, char *tracepos, char *tracebound,
...
void kdp_exception(unsigned char *pkt, int *len, unsigned short *remote_port,
                   unsigned int exception, unsigned int code,
...
boolean_t kdp_exception_ack(unsigned char *pkt, int len) {
...
static void kdp_getintegerstate(char *out_state) {
  ...
  bcopy((char *)&thread_state64, (char *)out_state,
...
kdp_error_t kdp_machine_read_regs(__unused unsigned int cpu,
                                  unsigned int flavor, char *data, int *size) {
  switch (flavor) {
#if defined(__arm64__)
...
static void kdp_setintegerstate(char *state_in) {
#if defined(__arm64__)
  struct arm_thread_state64 thread_state64;
  struct arm_saved_state *saved_state;

  bcopy((char *)state_in, (char *)&thread_state64,
        sizeof(struct arm_thread_state64));
  saved_state = kdp.saved_state;
  assert(is_saved_state64(saved_state));
...
kdp_error_t kdp_machine_write_regs(__unused unsigned int cpu,
                                   unsigned int flavor, char *data,
                                   __unused int *size) {
  switch (flavor) {
#if defined(__arm64__)
  case ARM_THREAD_STATE64:
...
void kdp_machine_hostinfo(kdp_hostinfo_t *hostinfo) {
  hostinfo->cpus_mask = 1;
  hostinfo->cpu_type = slot_type(0);
  hostinfo->cpu_subtype = slot_subtype(0);
}

__attribute__((noreturn)) void kdp_panic(const char *fmt, ...) {
...
void kdp_intr_enbl(int s) { splx(s); }

void kdp_us_spin(int usec) { delay(usec / 100); }

void kdp_call(void) { Debugger("inline call to debugger(machine_startup)"); }

int kdp_getc(void) { return console_try_read_char(); }

void kdp_machine_get_breakinsn(uint8_t *bytes, uint32_t *size) {
  *(uint32_t *)bytes = GDB_TRAP_INSTR1;
  *size = sizeof(uint32_t);
}

void kdp_sync_cache(void) {}

int kdp_machine_ioport_read(kdp_readioport_req_t *rq, caddr_t data,
                            uint16_t lcpu) {
#pragma unused(rq, data, lcpu)
  return 0;
}

int kdp_machine_ioport_write(kdp_writeioport_req_t *rq, caddr_t data,
                             uint16_t lcpu) {
#pragma unused(rq, data, lcpu)
  return 0;
}

int kdp_machine_msr64_read(kdp_readmsr64_req_t *rq, caddr_t data,
                           uint16_t lcpu) {
#pragma unused(rq, data, lcpu)
  return 0;
}

int kdp_machine_msr64_write(kdp_writemsr64_req_t *rq, caddr_t data,
                            uint16_t lcpu) {
#pragma unused(rq, data, lcpu)
  return 0;
}
...
void kdp_trap(unsigned int exception, struct arm_saved_state *saved_state) {
...
int machine_trace_thread64(thread_t thread, char *tracepos, char *tracebound,
  ...
  nframes = (tracebound > tracepos)
                ? MIN(nframes, (int)((tracebound - tracepos) / framesize))
                : 0;
  if (!nframes) {
    return 0;
  }
  ...
  if (state != NULL) {
    ...
    arm_kernel_saved_state_t *kstate =
        &thread_get_kernel_state(thread)->machine.ss;

    fp = kstate->fp;
    prevlr = kstate->lr;
    pc = kstate->pc_was_in_userspace
             ? (register_t)ptrauth_strip((void *)&_was_in_userspace,
    ...
    *tracebuf++ = prevlr;
    ...
    if (fp < prevfp) {
      ...
      for (cpu = 0; cpu <= max_cpu; cpu++) {
        ...
        if (prevfp >= (target_cpu_datap->intstack_top - INTSTACK_SIZE) &&
            prevfp < target_cpu_datap->intstack_top) {
          switched_stacks = true;
          break;
        }
#if defined(__arm64__)
        if (prevfp >= (target_cpu_datap->excepstack_top - EXCEPSTACK_SIZE) &&
            prevfp < target_cpu_datap->excepstack_top) {
          switched_stacks = true;
          break;
        }
#endif
    ...
    prevlr =
        (uint64_t)ptrauth_strip((void *)prevlr, ptrauth_key_return_address);
#endif
    prevlr = VM_KERNEL_UNSLIDE(prevlr);
...
void kdp_ml_enter_debugger(void) { __asm__ volatile(".long 0xe7ffdefe"); }
