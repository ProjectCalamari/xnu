...
void kdp_memcpy(void *dst, const void *src, size_t len) {
  ...
  if (((vm_offset_t)dst >= gPanicBase) &&
      ((vm_offset_t)dst < (gPanicBase + gPanicSize))) {
...
kern_return_t
kdp_traverse_mappings(task_t task, kdp_fault_flags_t fault_flags,
                      kdp_traverse_mappings_flags_t traverse_mappings_flags,
                      kdp_traverse_mappings_callback callback, void *context) {
  ...
  for (entry = vm_map_first_entry(map);
       ret == KERN_SUCCESS && entry != NULL && entry != vm_map_to_entry(map);
       entry = entry->vme_next) {
    // Found a region, iterate over pages in the region
    for (vcur = entry->vme_start; ret == KERN_SUCCESS && vcur < entry->vme_end;
         vcur += task_page_size) {
...
vm_offset_t kdp_find_phys(vm_map_t map, vm_offset_t target_addr,
                          kdp_fault_flags_t fault_flags,
                          struct kdp_fault_result *fault_results) {
  ...
  if (!pmap_valid_page((ppnum_t)atop(cur_phys_addr))) {
    ...
    cur_phys_addr =
        kdp_lightweight_fault(map, (target_addr & ~effective_page_mask),
    ...
    if (!pmap_valid_page((ppnum_t)atop(cur_phys_addr))) {
      if (fault_results) {
        fault_results->flags |=
            (KDP_FAULT_RESULT_TRIED_FAULT | KDP_FAULT_RESULT_PAGED_OUT);
      }
     ...
     */
    unsigned int cur_wimg_bits =
        pmap_cache_attributes((ppnum_t)atop(cur_phys_addr));
...
int kdp_generic_copyin(vm_map_t map, uint64_t uaddr, void *dest, size_t size,
                       kdp_fault_flags_t fault_flags,
                       find_phys_fn_t find_phys_fn, void *context) {
  size_t rem = size;
  char *kvaddr = dest;
  size_t effective_page_mask;
  size_t effective_page_size =
      kdp_vm_map_get_page_size(map, &effective_page_mask);
...
#if defined(__arm64__)
  /* Identify if destination buffer is in panic storage area */
  if (!not_in_kdp && ((vm_offset_t)dest >= gPanicBase) &&
      ((vm_offset_t)dest < (gPanicBase + gPanicSize))) {
  ...
  while (rem) {
    uint64_t phys_src =
        (*find_phys_fn)(map, (vm_offset_t)uaddr, fault_flags, context);
...
#if defined(__arm64__)
      /*
       * On arm devices the panic buffer is mapped as device memory and doesn't
       * allow unaligned accesses. To prevent these, we copy over bytes
       * individually here.
       */
      if (!not_in_kdp) {
        kdp_memcpy(kvaddr, (const void *)phystokv((pmap_paddr_t)phys_src),
                   cur_size);
      } else
...
int kdp_generic_copyin_word(task_t task, uint64_t addr, uint64_t *result,
                            kdp_fault_flags_t fault_flags,
                            find_phys_fn_t find_phys_fn, void *context) {
  /* must be called from debugger context */
  assert(!not_in_kdp);
  ...
  if (task_has_64Bit_addr(task)) {
    return kdp_generic_copyin(task->map, addr, result, sizeof(uint64_t),
                              fault_flags, find_phys_fn, context);
  } else {
    uint32_t buf;
    int r = kdp_generic_copyin(task->map, addr, &buf, sizeof(uint32_t),
                               fault_flags, find_phys_fn, context);
    if (r == KERN_SUCCESS) {
      *result = buf;
    }
...
static int kdp_generic_copyin_string_slowpath(task_t task, uint64_t addr,
                                              char *buf, int buf_sz,
                                              kdp_fault_flags_t fault_flags,
                                              find_phys_fn_t find_phys_fn,
                                              void *context) {
  int i;
  ...
  for (i = 0; i < buf_sz; i++) {
    if (validated == 0) {
      valid_from = i;
      phys_src =
          (*find_phys_fn)(map, (vm_offset_t)(addr + i), fault_flags, context);
...
int kdp_generic_copyin_string(task_t task, uint64_t addr, char *buf, int buf_sz,
                              kdp_fault_flags_t fault_flags,
                              find_phys_fn_t find_phys_fn, void *context) {
  ...
  res = kdp_generic_copyin(task->map, addr, optbuffer, sizeof(optbuffer),
                           fault_flags, find_phys_fn, context);
  if (res != KERN_SUCCESS ||
      strnlen(optbuffer, sizeof(optbuffer)) == sizeof(optbuffer)) {
    /* try the slowpath */
    return kdp_generic_copyin_string_slowpath(
        task, addr, buf, buf_sz, fault_flags, find_phys_fn, context);
  }
...
static int kdp_copyin(vm_map_t map, uint64_t uaddr, void *dest, size_t size,
                      kdp_fault_flags_t fault_flags) {
  return kdp_generic_copyin(map, uaddr, dest, size, fault_flags,
                            (find_phys_fn_t)kdp_find_phys, NULL);
}

kern_return_t kdp_task_dyld_info(task_t task, kdp_fault_flags_t fault_flags,
                                 uint64_t *dyld_load_address, uuid_t dyld_uuid,
  ...
  if (dyld_uuid == NULL || dyld_load_address == NULL ||
      task_page_size == NULL) {
  ...
  *task_page_size = kdp_vm_map_get_page_size(task->map, NULL);
  ...
  for (size_t i = 0; i < uuid_info_count; i++) {
    if (task_64bit_addr) {
      ...
      if (kdp_copyin(task->map,
                     uuid_info_addr +
                         (i * sizeof(struct user64_dyld_uuid_info)),
                     &uuid_info, sizeof(struct user64_dyld_uuid_info),
                     fault_flags) == KERN_SUCCESS) {
      ...
      if (kdp_copyin(task->map,
                     uuid_info_addr +
                         (i * sizeof(struct user32_dyld_uuid_info)),
                     &uuid_info, sizeof(struct user32_dyld_uuid_info),
                     fault_flags) == KERN_SUCCESS) {
        if (uuid_info.imageLoadAddress == dyld_load_addr) {
          uuid_copy(dyld_uuid, uuid_info.imageUUID);
          *dyld_load_address = dyld_load_addr;
