...
user_dump_map_traverse_preflight_callback(vm_offset_t start, vm_offset_t end,
                                          void *context) {
...
kern_return_t user_dump_save_summary(void *refcon,
                                     ...
                                     void *context) {
  struct kern_userspace_coredump_context *uccontext = refcon;
  struct user_dump_preflight_context udc_preflight = {
      .uccontext = uccontext, .region_count = 0, .dumpable_bytes = 0};
  uint64_t thread_state_size = 0, thread_count = 0;
  kern_return_t ret;

  ret = kdp_traverse_mappings(uccontext->task, KDP_FAULT_FLAGS_ENABLE_FAULTING,
  ...
  kern_collect_userth_state_size(uccontext->task, &thread_count,
                                 &thread_state_size);
...
static kern_return_t
user_dump_map_traverse_send_segdesc_callback(vm_offset_t start, vm_offset_t end,
                                             void *context) {
...
kern_return_t user_dump_save_seg_descriptions(
  ...
  kern_return_t ret = kdp_traverse_mappings(
      uccontext->task, KDP_FAULT_FLAGS_ENABLE_FAULTING,
      KDP_TRAVERSE_MAPPINGS_FLAGS_NONE,
      user_dump_map_traverse_send_segdesc_callback, &uds_context);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(context, "%s: vm map traversal failed: %d\n", __func__,
                      ret);
    return ret;
...
kern_return_t user_dump_save_sw_vers_detail(
  ...
  /*
   * For user coredumps we want the dyld UUID to be stored in the main bin spec
   * LC_NOTE
   */
  kern_return_t ret =
      kdp_task_dyld_info(uccontext->task, KDP_FAULT_FLAGS_ENABLE_FAULTING,
                         &dyld_load_address, dyld_uuid, &task_page_size);
  if (ret != KERN_SUCCESS) {
    return ret;
  }
  return callback(dyld_load_address, dyld_uuid, bit_log2(task_page_size),
                  context);
}
...
static kern_return_t user_dump_map_traverse_send_segdata_callback(
    vm_offset_t physstart, vm_offset_t physend, void *context) {
  struct user_dump_send_segdata_context *uds_context = context;

  assert(physstart < physend);

  void *vphysstart = (void *)phystokv(physstart);
  return uds_context->callback(
      vphysstart, (uint64_t)((uintptr_t)physend - (uintptr_t)physstart),
      uds_context->context);
}
...
kern_return_t user_dump_save_segment_data(void *refcon,
                                          ...
                                          void *context) {
  struct kern_userspace_coredump_context *uccontext = refcon;
  struct user_dump_send_segdata_context uds_context = {.callback = callback,
                                                       .context = context};
  kern_return_t ret = kdp_traverse_mappings(
      uccontext->task, KDP_FAULT_FLAGS_ENABLE_FAULTING,
      KDP_TRAVERSE_MAPPINGS_FLAGS_PHYSICAL,
      user_dump_map_traverse_send_segdata_callback, &uds_context);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(context, "%s: vm map traversal failed: %d\n", __func__,
                      ret);
    return ret;
...
/*
 * Add a LC_NOTE to the core to indicate that it was created during a kernel
 * panic.
 */

extern char kernel_uuid_string[];

kern_return_t user_dump_save_note_summary(void *refcon __unused,
                                          core_save_note_summary_cb callback,
                                          void *context) {
  return callback(2 /* two LC_NOTE */,
                  sizeof(panic_context_note_t) + sizeof(addrable_bits_note_t),
                  context);
}

kern_return_t
user_dump_save_note_descriptions(void *refcon __unused,
                                 core_save_note_descriptions_cb callback,
                                 void *context) {
  callback(PANIC_CONTEXT_DATA_OWNER, sizeof(panic_context_note_t), context);
  return callback(ADDRABLE_BITS_DATA_OWNER, sizeof(addrable_bits_note_t),
                  context);
}
...
kern_return_t user_dump_save_note_data(void *refcon,
  ...
  strlcpy(&note.kernel_uuid_string[0], kernel_uuid_string,
