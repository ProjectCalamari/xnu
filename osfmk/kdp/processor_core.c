...
/*
 * Format of the legacy bin spec (LC_IDENT-like) LC_NOTE payload as expected by
 * LLDB
 */
typedef struct {
...
typedef struct {
  struct kdp_core_out_vars
      *core_outvars; /* Output procedure info (see kdp_out_stage.h) */
  kern_coredump_callback_config
      *core_config; /* Information about core currently being dumped */
  void
      *core_refcon; /* Reference constant associated with the coredump helper */
  boolean_t core_should_be_skipped; /* Indicates whether this specific core
  ...
  uint64_t
      core_file_length; /* Overall corefile length including any zero padding */
  uint64_t core_file_length_compressed; /* File length after compression */
  uint64_t core_segment_count; /* Number of LC_SEGMENTs in the core currently
                                  being dumped */
  uint64_t core_segments_remaining; /* Number of LC_SEGMENTs that have not been
                                       added to the header */
  uint64_t core_segment_byte_total; /* Sum of all the data from the LC_SEGMENTS
                                       in the core */
  uint64_t
      core_segment_bytes_remaining; /* Quantity of data remaining from
                                       LC_SEGMENTs that have yet to be added */
  uint64_t core_thread_count;       /* Number of LC_THREADs to be included */
  uint64_t core_threads_remaining;  /* Number of LC_THREADs that have yet to be
                                       included */
  uint64_t core_thread_state_size;  /* Size of each LC_THREAD */
  uint64_t core_note_count;         /* Number of LC_NOTEs to be included */
  uint64_t core_notes_remaining;  /* Number of LC_NOTEs that have not been added
                                     to the header */
  uint64_t core_note_bytes_total; /* Sum of all data from the LC_NOTE segments
                                     in the core */
  uint64_t core_note_bytes_remaining; /* Quantity of data remaining from
                                         LC_NOTEs that have yet to be added */
  ...
  uint64_t core_total_bytes; /* Total amount of data to be included in this core
                                (excluding zero fill) */
...
struct kern_coredump_core {
  ...
  char kcc_corename[MACH_CORE_FILEHEADER_NAMELEN]; /* Description of this
                                                      processor */
  ...
  kern_coredump_callback_config
      kcc_cb; /* Registered processor callbacks for coredump */
};

struct kern_coredump_core *kern_coredump_core_list = NULL;
struct kern_coredump_core *kern_userspace_coredump_core_list = NULL;
LCK_GRP_DECLARE(kern_userspace_coredump_core_list_lock_grp,
                "userspace coredump list");
LCK_MTX_DECLARE(kern_userspace_coredump_core_list_lock,
                &kern_userspace_coredump_core_list_lock_grp);

typedef kern_return_t (*legacy_sw_vers_registered_cb)(
    void *refcon, core_save_sw_vers_cb callback, void *context);
...
static struct kern_coredump_core *kern_register_coredump_helper_internal(
    int kern_coredump_config_vers,
    const kern_coredump_callback_config *kc_callbacks, void *refcon,
    const char *core_description, kern_coredump_type_t type, boolean_t is64bit,
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  legacy_sw_vers_registered_cb legacy_vers_callback =
      kc_callbacks->kcc_coredump_save_sw_vers;
#pragma clang diagnostic pop
  ...
  if (kern_coredump_config_vers >= KERN_COREDUMP_MIN_CONFIG_NOTES) {
    ...
    if ((kc_callbacks->kcc_coredump_save_note_summary == NULL) !=
        (kc_callbacks->kcc_coredump_save_note_descriptions == NULL)) {
      return NULL;
    }
    if ((kc_callbacks->kcc_coredump_save_note_descriptions == NULL) !=
        (kc_callbacks->kcc_coredump_save_note_data == NULL)) {
      return NULL;
    }
  }
  ...
  if (type == XNU_COREDUMP || type == USERSPACE_COREDUMP ||
      type == SECURE_COREDUMP) {
    snprintf((char *)&core_helper->kcc_corename, MACH_CORE_FILEHEADER_NAMELEN,
             "%s", core_description);
  } else {
  ...
  core_callbacks->kcc_coredump_get_summary =
      kc_callbacks->kcc_coredump_get_summary;
  core_callbacks->kcc_coredump_save_segment_descriptions =
      kc_callbacks->kcc_coredump_save_segment_descriptions;
  core_callbacks->kcc_coredump_save_segment_data =
      kc_callbacks->kcc_coredump_save_segment_data;
  core_callbacks->kcc_coredump_save_thread_state =
      kc_callbacks->kcc_coredump_save_thread_state;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  core_callbacks->kcc_coredump_save_sw_vers =
      kc_callbacks->kcc_coredump_save_sw_vers;
#pragma clang diagnostic pop

  if (kern_coredump_config_vers >= KERN_COREDUMP_MIN_CONFIG_NOTES) {
    core_callbacks->kcc_coredump_save_note_summary =
        kc_callbacks->kcc_coredump_save_note_summary;
    core_callbacks->kcc_coredump_save_note_descriptions =
        kc_callbacks->kcc_coredump_save_note_descriptions;
    core_callbacks->kcc_coredump_save_note_data =
        kc_callbacks->kcc_coredump_save_note_data;
    core_callbacks->kcc_coredump_save_sw_vers_detail =
        kc_callbacks->kcc_coredump_save_sw_vers_detail;
  }
  ...
  if (type == XNU_COREDUMP) {
    ...
    } while (!OSCompareAndSwapPtr(kern_coredump_core_list, core_helper,
...
kern_register_coredump_helper(int kern_coredump_config_vers,
                              const kern_coredump_callback_config *kc_callbacks,
                              void *refcon, const char *core_description,
                              boolean_t is64bit, uint32_t mh_magic,
                              cpu_type_t cpu_type, cpu_subtype_t cpu_subtype) {
  if (coredump_registered_count >= KERN_COREDUMP_MAX_CORES) {
    return KERN_RESOURCE_SHORTAGE;
  }

  if (kern_register_coredump_helper_internal(
          kern_coredump_config_vers, kc_callbacks, refcon, core_description,
          COPROCESSOR_COREDUMP, is64bit, mh_magic, cpu_type,
          cpu_subtype) == NULL) {
...
kern_register_xnu_coredump_helper(kern_coredump_callback_config *kc_callbacks) {
  ...
  if (kern_register_coredump_helper_internal(
          KERN_COREDUMP_CONFIG_VERSION, kc_callbacks, NULL, "kernel",
          XNU_COREDUMP, is64bit, _mh_execute_header.magic,
...
kern_return_t
kern_register_sk_coredump_helper(kern_coredump_callback_config *sk_callbacks,
                                 void *refcon) {
  if (kern_register_coredump_helper_internal(
          KERN_COREDUMP_CONFIG_VERSION, sk_callbacks, refcon, "secure-kernel",
          SECURE_COREDUMP, TRUE, _mh_execute_header.magic,
          _mh_execute_header.cputype, _mh_execute_header.cpusubtype) == NULL) {
    return KERN_FAILURE;
  }
...
extern cpu_type_t process_cpu_type(void *bsd_info);

extern cpu_type_t process_cpu_subtype(void *bsd_info);
...
kern_return_t kern_register_userspace_coredump(task_t task, const char *name) {
  ...
  context = kalloc_type(struct kern_userspace_coredump_context,
  ...
  userkc_callbacks.kcc_coredump_save_segment_descriptions =
      user_dump_save_seg_descriptions;
  userkc_callbacks.kcc_coredump_save_thread_state = user_dump_save_thread_state;
  userkc_callbacks.kcc_coredump_save_sw_vers_detail =
      user_dump_save_sw_vers_detail;
  ...
  userkc_callbacks.kcc_coredump_save_note_descriptions =
      user_dump_save_note_descriptions;
  userkc_callbacks.kcc_coredump_save_note_data = user_dump_save_note_data;

  if (kern_register_coredump_helper_internal(
          KERN_COREDUMP_CONFIG_VERSION, &userkc_callbacks, context, name,
          USERSPACE_COREDUMP, is64bit, mh_magic, mh_cputype,
          mh_cpusubtype) == NULL) {
    result = KERN_FAILURE;
    goto finish;
  }
...
kern_return_t kern_unregister_userspace_coredump(task_t task) {
  ...
  while (current_core) {
    struct kern_userspace_coredump_context *context =
        (struct kern_userspace_coredump_context *)current_core->kcc_refcon;
    assert(context != NULL);
  ...
  if (current_core) {
    kfree_type(struct kern_userspace_coredump_context,
               current_core->kcc_refcon);
    OSAddAtomic(-1, &coredump_registered_count);
    return KERN_SUCCESS;
  }
...
/*
 * Save LC_NOTE metadata about the core we are going to write before we write
 * the mach header
 */
static int coredump_save_note_summary(uint64_t core_note_count,
                                      uint64_t core_note_byte_count,
                                      void *context) {
  ...
  core_context->core_note_count = core_context->core_notes_remaining =
      core_note_count;
  core_context->core_note_bytes_total =
      core_context->core_note_bytes_remaining = core_note_byte_count;
...
coredump_save_summary(uint64_t core_segment_count, uint64_t core_byte_count,
  ...
  /*
   * secure coredumps and coprocessor coredumps aren't required to contain any
   * thread state, because it's reconstructed during the lldb session
   */
  if (core_context->core_type != SECURE_COREDUMP &&
      core_context->core_type != COPROCESSOR_COREDUMP &&
      (!thread_count || !thread_state_size)) {
    return KERN_INVALID_ARGUMENT;
  }

  /* Initialize core_context */
  core_context->core_segments_remaining = core_context->core_segment_count =
      core_segment_count;
  core_context->core_segment_bytes_remaining =
      core_context->core_segment_byte_total = core_byte_count;
  ...
  core_context->core_note_count = core_context->core_notes_remaining =
      (core_context->core_note_count + 1);
  ...
  core_context->core_note_bytes_total =
      core_context->core_note_bytes_remaining =
          (core_context->core_note_bytes_total + vers_note_length);

#if defined(__LP64__)
  if (core_context->core_is64bit) {
    sizeofcmds = (uint32_t)(core_context->core_segment_count *
                                sizeof(struct segment_command_64) +
                            (core_context->core_threads_remaining *
                             core_context->core_thread_state_size) +
                            (core_context->core_note_count *
  ...
  core_context->core_total_bytes = core_context->core_header_size +
                                   core_context->core_segment_byte_total +
                                   core_context->core_note_bytes_total;
  core_context->core_file_length = round_page(core_context->core_header_size) +
                                   core_context->core_segment_byte_total +
                                   core_context->core_note_bytes_total;
  core_context->core_cur_foffset = round_page(core_context->core_header_size);

  numcmds = (uint32_t)(core_context->core_segment_count +
                       core_context->core_thread_count +
                       core_context->core_note_count);

  /*
   * Reset the zstream and other output context before writing any data out. We
   * do this here to update the total file length on the outvars before we start
   * writing out.
   */
  ret = kdp_reset_output_vars(
      core_context->core_outvars, core_context->core_file_length, true,
      &should_skip, core_context->core_name, core_context->core_type);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(
        context,
        "%s() : failed to reset the out vars : kdp_reset_output_vars(%p, %llu, "
        "true, %p) returned error 0x%x\n",
        __func__, core_context->core_outvars, core_context->core_file_length,
        &should_skip, ret);
    return ret;
  }
  ...
  if (core_context->core_is64bit) {
    ...
    ret = kdp_core_output(core_context->core_outvars, sizeof(core_header),
                          (caddr_t)&core_header);
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(context,
                        "%s() : failed to write mach header : "
                        "kdp_core_output(%p, %lu, %p) returned error 0x%x\n",
                        __func__, core_context->core_outvars,
                        sizeof(core_header), &core_header, ret);
      return ret;
    }
 ...
 */
static int coredump_save_segment_descriptions(uint64_t seg_start,
                                              uint64_t seg_end, void *context) {
  processor_core_context *core_context = (processor_core_context *)context;
  int ret;
  ...
  if (seg_end <= seg_start) {
    kern_coredump_log(context,
                      "%s(0x%llx, 0x%llx, %p) : called with invalid addresses "
                      ": start 0x%llx >= end 0x%llx\n",
                      __func__, seg_start, seg_end, context, seg_start,
                      seg_end);
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (core_context->core_segments_remaining == 0) {
    kern_coredump_log(
        context,
        "%s(0x%llx, 0x%llx, %p) : coredump_save_segment_descriptions() called "
        "too many times, %llu segment descriptions already recorded\n",
        __func__, seg_start, seg_end, context,
        core_context->core_segment_count);
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (core_context->core_is64bit) {
    ...
    if (core_context->core_cur_hoffset + sizeof(seg_command) >
        core_context->core_header_size) {
      kern_coredump_log(context,
                        "%s(0x%llx, 0x%llx, %p) : ran out of space to save "
                        "commands with %llu of %llu remaining\n",
                        __func__, seg_start, seg_end, context,
    ...
    ret = kdp_core_output(core_context->core_outvars, sizeof(seg_command),
                          (caddr_t)&seg_command);
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(
          context,
          "%s(0x%llx, 0x%llx, %p) : failed to write segment %llu of %llu. "
          "kdp_core_output(%p, %lu, %p) returned error %d\n",
          __func__, seg_start, seg_end, context,
          core_context->core_segment_count -
              core_context->core_segments_remaining,
          core_context->core_segment_count, core_context->core_outvars,
          sizeof(seg_command), &seg_command, ret);
      return ret;
    }
...
#endif /* defined(__LP64__) */
  {
    ...
    if (seg_start > UINT32_MAX || seg_end > UINT32_MAX) {
      kern_coredump_log(context,
                        "%s(0x%llx, 0x%llx, %p) : called with invalid "
                        "addresses for 32-bit : start 0x%llx, end 0x%llx\n",
                        __func__, seg_start, seg_end, context, seg_start,
                        seg_end);
      return KERN_INVALID_ARGUMENT;
    }
 ...
 */
static int coredump_save_note_description(const char *data_owner,
                                          uint64_t length, void *context) {
  processor_core_context *core_context = (processor_core_context *)context;
  int ret;
  ...
  /* Flush new command to output */
  ret =
      kdp_core_output(core_context->core_outvars, sizeof(note), (caddr_t)&note);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(context,
                      "%s() : failed to write note %llu of %llu : "
                      "kdp_core_output() returned  error 0x%x\n",
                      __func__,
                      core_context->core_note_count -
 ...
 */
static int coredump_save_thread_state(void *thread_state, void *context) {
  processor_core_context *core_context = (processor_core_context *)context;
  ...
  if (core_context->core_cur_hoffset + core_context->core_thread_state_size >
      core_context->core_header_size) {
    kern_coredump_log(
        context,
        "%s() : ran out of space to save threads with %llu of %llu remaining\n",
        __func__, core_context->core_threads_remaining,
        core_context->core_thread_count);
    return KERN_NO_SPACE;
  }

  ret = kdp_core_output(core_context->core_outvars,
                        core_context->core_thread_state_size,
                        (caddr_t)thread_state);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(context,
                      "%s() : failed to write thread data : kdp_core_output() "
                      "returned 0x%x\n",
                      __func__, ret);
    return ret;
  }
...
static int coredump_save_segment_data(void *seg_data, uint64_t length,
                                      void *context) {
  int ret;
  processor_core_context *core_context = (processor_core_context *)context;
  ...
  if (length > core_context->core_segment_bytes_remaining) {
    kern_coredump_log(context,
                      "%s(%p, %llu, %p) : called with too much data, %llu "
                      "written, %llu left\n",
                      __func__, seg_data, length, context,
                      core_context->core_segment_byte_total -
                          core_context->core_segment_bytes_remaining,
                      core_context->core_segment_bytes_remaining);
    return KERN_INVALID_ARGUMENT;
  ...
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(
        context, "%s() : failed to write data (%llu bytes remaining) :%d\n",
        __func__, core_context->core_segment_bytes_remaining, ret);
    return ret;
  }
...
static int coredump_save_note_data(void *note_data, uint64_t length,
                                   void *context) {
  int ret;
  processor_core_context *core_context = (processor_core_context *)context;
  ...
  if (length > core_context->core_note_bytes_remaining) {
    kern_coredump_log(context,
                      "%s(%p, %llu, %p) : called with too much data, %llu "
                      "written, %llu left\n",
                      __func__, note_data, length, context,
                      core_context->core_note_bytes_total -
                          core_context->core_note_bytes_remaining,
                      core_context->core_note_bytes_remaining);
    return KERN_INVALID_ARGUMENT;
  ...
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(
        context, "%s() : failed to write data (%llu bytes remaining) :%d\n",
        __func__, core_context->core_note_bytes_remaining, ret);
    return ret;
  }
...
static int coredump_save_sw_vers_legacy(void *sw_vers, uint64_t length,
                                        void *context) {
  processor_core_context *core_context = (processor_core_context *)context;
  int ret;
  ...
  if (length > KERN_COREDUMP_VERSIONSTRINGMAXSIZE || !length) {
    kern_coredump_log(context,
                      ...
                      __func__, sw_vers, length, context, length);
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(context,
                      "%s() : failed to write legacy bin spec version : "
                      "coredump_save_note_data() returned 0x%x\n",
                      __func__, ret);
    return ret;
  }
  ...
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(context,
                      "%s() : failed to write sw_vers string : "
                      "coredump_save_note_data() returned 0x%x\n",
                      __func__, ret);
    return ret;
  }
  ...
  if (length < KERN_COREDUMP_VERSIONSTRINGMAXSIZE) {
    ...
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(context,
                        "%s() : failed to write zero fill padding : "
                        "kdp_core_output(%p, %llu, NULL) returned 0x%x\n",
                        __func__, core_context->core_outvars, length_to_zero,
                        ret);
      return ret;
    }
...
static int coredump_save_sw_vers(uint64_t address, uuid_t uuid,
                                 uint32_t log2_pagesize, void *context) {
  processor_core_context *core_context = (processor_core_context *)context;
  int ret;
  ...
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(context,
                      "%s() : failed to write main bin spec structure : "
                      "coredump_save_note_data() returned 0x%x\n",
                      __func__, ret);
    return ret;
  }
...
kern_coredump_routine(void *core_outvars,
                      struct kern_coredump_core *current_core,
                      uint64_t core_begin_offset, uint64_t *core_file_length,
                      boolean_t *abort_on_failure, kern_coredump_type_t type,
  ...
  *core_file_length = 0;
  ...
  /* Retrieve information about LC_NOTE data we will write out as part of the
   * core before we populate the general header */
  if (current_core->kcc_cb.kcc_coredump_save_note_summary != NULL) {
    ret = current_core->kcc_cb.kcc_coredump_save_note_summary(
        context.core_refcon, coredump_save_note_summary, &context);
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(&context,
   ...
   */
  ret = current_core->kcc_cb.kcc_coredump_get_summary(
      context.core_refcon, coredump_save_summary, &context);
  ...
  ret = current_core->kcc_cb.kcc_coredump_save_segment_descriptions(
      context.core_refcon, coredump_save_segment_descriptions, &context);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(&context,
  ...
  if (context.core_segments_remaining != 0) {
    kern_coredump_log(&context,
                      "(%s) : save_segment_descriptions returned without all "
                      "segment descriptions written, %llu of %llu remaining\n",
                      __func__, context.core_segments_remaining,
                      context.core_segment_count);
    return KERN_FAILURE;
  }

  /* write out the LC_NOTE with the binary info */
  if (current_core->kcc_cb.kcc_coredump_save_sw_vers_detail != NULL) {
    ret = coredump_save_note_description(
        MAIN_BIN_SPEC_DATA_OWNER, sizeof(main_bin_spec_note_t), &context);
  } else {
    ret = coredump_save_note_description(DATA_OWNER_LEGACY_BIN_SPEC,
                                         sizeof(legacy_bin_spec), &context);
  }
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(&context,
                      "(%s) : coredump_save_note_description returned %d while "
                      "writing binary info LC_NOTE description",
                      __func__, ret);
    return ret;
  }
  ...
  if (current_core->kcc_cb.kcc_coredump_save_note_descriptions != NULL) {
    ret = current_core->kcc_cb.kcc_coredump_save_note_descriptions(
        context.core_refcon, coredump_save_note_description, &context);
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(
          &context,
  ...
  if (context.core_notes_remaining != 0) {
    kern_coredump_log(&context,
                      "(%s) : save_note_descriptions returned without all note "
                      "descriptions written, %llu of %llu remaining\n",
                      __func__, context.core_notes_remaining,
                      context.core_note_count);
    return KERN_FAILURE;
  }
  ...
  if (context.core_thread_state_size) {
    ...
    ret = current_core->kcc_cb.kcc_coredump_save_thread_state(
  ...
  if (context.core_threads_remaining != 0) {
    kern_coredump_log(&context,
                      "(%s) : save_thread_state returned without all thread "
                      "descriptions written, %llu of %llu remaining\n",
                      __func__, context.core_threads_remaining,
                      context.core_thread_count);
    return KERN_FAILURE;
  }
  assert(context.core_cur_hoffset == context.core_header_size);

  /* Zero fill between the end of the header and the beginning of the segment
   * data file offset */
  ret = kdp_core_output(
      context.core_outvars,
      (round_page(context.core_header_size) - context.core_header_size), NULL);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(
        &context,
        "(kern_coredump_routine) : failed to write zero fill padding (%llu "
        "bytes remaining) : kdp_core_output(%p, %llu, NULL) returned 0x%x\n",
        context.core_segment_bytes_remaining, context.core_outvars,
        (round_page(context.core_header_size) - context.core_header_size), ret);
    return ret;
  }

  /* Reset our local current file offset before we start writing out segment
   * data */
  context.core_cur_foffset = round_page(context.core_header_size);

  ret = current_core->kcc_cb.kcc_coredump_save_segment_data(
      context.core_refcon, coredump_save_segment_data, &context);
  if (ret != KERN_SUCCESS) {
  ...
  if (context.core_segment_bytes_remaining != 0) {
    kern_coredump_log(
        &context,
        "(kern_coredump_routine) : save_segment_data returned without all "
        "segment data written, %llu of %llu remaining\n",
        context.core_segment_bytes_remaining, context.core_segment_byte_total);
    return KERN_FAILURE;
  }

  /* Save out the LC_NOTE segment data, starting with the binary info / sw vers
   * one */
  if (current_core->kcc_cb.kcc_coredump_save_sw_vers_detail != NULL) {
    ret = current_core->kcc_cb.kcc_coredump_save_sw_vers_detail(
        context.core_refcon, coredump_save_sw_vers, &context);
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(
          &context,
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ret = current_core->kcc_cb.kcc_coredump_save_sw_vers(
        context.core_refcon, coredump_save_sw_vers_legacy, &context);
#pragma clang diagnostic pop
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(&context,
  ...
  if (current_core->kcc_cb.kcc_coredump_save_note_data != NULL) {
    ret = current_core->kcc_cb.kcc_coredump_save_note_data(
        context.core_refcon, coredump_save_note_data, &context);
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(&context,
  ...
  if (context.core_note_bytes_remaining != 0) {
    kern_coredump_log(&context,
                      "(%s) : kcc_coredump_save_note_data returned without all "
                      "note data written, %llu of %llu remaining\n",
                      __func__, context.core_note_bytes_remaining,
                      context.core_note_bytes_total);
    return KERN_FAILURE;
  }
  ...
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(&context,
                      "(kern_coredump_routine) : failed to flush final core "
                      "data : kdp_core_output(%p, 0, NULL) returned 0x%x\n",
  ...
  kern_coredump_log(
      &context,
      "Done\nCoredump complete of %s, dumped %llu segments (%llu bytes), %llu "
      "threads (%llu bytes) overall uncompressed file length %llu bytes.",
      current_core->kcc_corename, context.core_segment_count,
      context.core_segment_byte_total, context.core_thread_count,
      (context.core_thread_count * context.core_thread_state_size),
      context.core_file_length);
  ...
  if (core_begin_offset) {
    /* If we're writing to disk (we have a begin offset), we need to update the
     * header */
    ret = kern_dump_record_file(
        context.core_outvars, current_core->kcc_corename, core_begin_offset,
        &context.core_file_length_compressed, details_flags);
    if (ret != KERN_SUCCESS) {
 ...
 */
static kern_return_t kern_do_auxiliary_coredump(void *core_outvars,
                                                struct kern_coredump_core *list,
                                                uint64_t *last_file_offset,
                                                uint64_t details_flags,
                                                boolean_t *abort_on_failure) {
  struct kern_coredump_core *current_core = list;
  uint64_t prev_core_length = 0;
  kern_coredump_type_t type = current_core == kern_userspace_coredump_core_list
                                  ? USERSPACE_COREDUMP
  ...
  if (type == USERSPACE_COREDUMP &&
      kdp_lck_mtx_lock_spin_is_acquired(
          &kern_userspace_coredump_core_list_lock)) {
    // Userspace coredump list was being modified at the time of the panic. Skip
    // collecting userspace coredumps
  ...
  while (current_core) {
    ...
    cur_ret = kern_coredump_routine(core_outvars, current_core,
                                    *last_file_offset, &prev_core_length,
                                    abort_on_failure, type, details_flags);
    if (cur_ret != KERN_SUCCESS) {
      ...
      if (*abort_on_failure) {
        ...
        // Try to capture other corefiles even if one failed, update the overall
    ...
    *last_file_offset = roundup(((*last_file_offset) + prev_core_length),
...
kern_return_t kern_do_coredump(void *core_outvars, kern_coredump_flags_t flags,
  ...
  *last_file_offset = first_file_offset;
  cur_ret = kern_coredump_routine(core_outvars, kernel_helper,
                                  *last_file_offset, &prev_core_length,
                                  &abort_dump, XNU_COREDUMP, details_flags);

  if (cur_ret != KERN_SUCCESS) {
    // As long as we didn't fail while updating the header for the raw file, we
    // should be able to try to capture other corefiles.
    if (abort_dump) {
      // The header may be in an inconsistent state, so bail now
      return KERN_FAILURE;
    } else {
  ...
  *last_file_offset = roundup(((*last_file_offset) + prev_core_length),
  ...
  if (sk_helper) {
    ...
    if (cur_ret != KERN_SUCCESS) {
      kern_coredump_log(
          NULL, "secure_core: Unable to seek to the start of file: %d\n",
          cur_ret);
      return KERN_FAILURE;
    }

    /* Dump the secure core to disk. */
    cur_ret = kern_coredump_routine(core_outvars, sk_helper, *last_file_offset,
                                    &prev_core_length, &abort_dump,
                                    SECURE_COREDUMP, details_flags);
    if (cur_ret != KERN_SUCCESS) {
      if (abort_dump) {
        return KERN_FAILURE;
      } else {
  ...
  ret =
      kern_do_auxiliary_coredump(core_outvars, kern_coredump_core_list,
                                 last_file_offset, details_flags, &abort_dump);
  if (ret != KERN_SUCCESS) {
    kern_coredump_log(NULL, "Failed to dump coprocessor cores\n");
    return ret;
  }

  ret = kern_do_auxiliary_coredump(
      core_outvars, kern_userspace_coredump_core_list, last_file_offset,
      details_flags, &abort_dump);
  if (ret != KERN_SUCCESS) {
...
kern_register_coredump_helper(int kern_coredump_config_vers,
                              ...
                              boolean_t is64bit, uint32_t mh_magic,
                              cpu_type_t cpu_type, cpu_subtype_t cpu_subtype) {
#pragma unused(kern_coredump_config_vers, kc_callbacks, refcon,                \
               core_description, is64bit, mh_magic, cpu_type, cpu_subtype)
  return KERN_NOT_SUPPORTED;
}

kern_return_t kern_register_sk_coredump_helper(
    __unused kern_coredump_callback_config *sk_callbacks,
 ...
 */
void kern_coredump_log(void *context, const char *string, ...) {
