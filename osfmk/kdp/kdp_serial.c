...
static void kdp_serial_out(unsigned char byte, void (*outFunc)(char)) {
  ...
  if (byte == SKDP_START_CHAR || byte == SKDP_END_CHAR ||
      byte == SKDP_ESC_CHAR || byte == '\n') {
...
void kdp_serialize_packet(unsigned char *packet, unsigned int len,
                          void (*outFunc)(char)) {
  unsigned int index;
  unsigned char byte;
  uint32_t crc;

  // insert the CRC between back to back STARTs which is compatible with old
  // clients
...
unsigned char *kdp_unserialize_packet(unsigned char byte, unsigned int *len) {
  ...
  case DS_READING:
    ...
    if (byte == SKDP_START_CHAR) {
      if (dsPos >= 4) {
        dsHaveCRC = true;
        dsCRC = dsBuffer[0] | (dsBuffer[1] << 8) | (dsBuffer[2] << 16) |
                (dsBuffer[3] << 24);
      }
      // else				printf("unexpected start char,
      // resetting\n");
      dsPos = 0;
      *len = SERIALIZE_READING;
      return 0;
