...
/*	@(#)udp_usrreq.c	2.2 88/05/23 4.0NFSSRC SMI;	from UCB 7.1
 * 6/5/86	*/
...
static struct kdp_ether_addr kdp_current_mac_address = {
...
static struct kdp_ether_addr etherbroadcastaddr = {
    .ether_addr_octet = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff}};

static struct kdp_ether_addr router_mac = {
    .ether_addr_octet = {0, 0, 0, 0, 0, 0}};
static struct kdp_ether_addr destination_mac = {
    .ether_addr_octet = {0, 0, 0, 0, 0, 0}};
static struct kdp_ether_addr temp_mac = {
    .ether_addr_octet = {0, 0, 0, 0, 0, 0}};
static struct kdp_ether_addr current_resolved_MAC = {
    .ether_addr_octet = {0, 0, 0, 0, 0, 0}};
...
void kdp_set_ip_and_mac_addresses(struct kdp_in_addr *ipaddr,
...
static uint32_t kdp_crashdump_feature_mask =
    KDP_FEATURE_LARGE_CRASHDUMPS | KDP_FEATURE_LARGE_PKT_SIZE;
...
static thread_call_t kdp_timer_call;

static void kdp_ml_enter_debugger_wrapper(__unused void *param0,
                                          __unused void *param1) {
  kdp_ml_enter_debugger();
}

static void kdp_timer_callout_init(void) {
...
void kdp_register_send_receive(kdp_send_t send, kdp_receive_t receive) {
  ...
  if (PE_parse_boot_argn("_panicd_corename", &corename_str,
                         sizeof(corename_str))) {
...
void kdp_unregister_send_receive(__unused kdp_send_t send,
...
static void kdp_schedule_debugger_reentry(unsigned interval) {
  uint64_t deadline;
...
static unsigned short ip_sum(unsigned char *c, unsigned int hlen) {
...
static void kdp_reply(unsigned short reply_port, const boolean_t sideband) {
 ...
 */
void kdp_set_interface(void *ifp, const struct kdp_ether_addr *macaddr) {
  char kdpstr[80];
...
void *kdp_get_interface(void) { return kdp_current_ifp; }
...
void kdp_set_ip_and_mac_addresses(struct kdp_in_addr *ipaddr,
  ...
  if (!KDP_SERIAL_ENABLED() || (kdp_current_ip_address != KDP_SERIAL_IPADDR)) {
    kdp_current_mac_address = *macaddr;
  ...
  if (last_time == (uint64_t)-1 || ((cur_time - last_time) > throttle_val)) {
    PEWriteNVRAMProperty("_kdp_ipstr", addr, (const unsigned int)strlen(addr));
  }
  last_time = cur_time;
...
struct kdp_ether_addr kdp_get_mac_addr(void) { return kdp_current_mac_address; }

unsigned int kdp_get_ip_address(void) {
  return (unsigned int)kdp_current_ip_address;
}

void kdp_disable_arp(void) { kdp_flag &= ~(DB_ARP); }

static void kdp_arp_dispatch(void) {
...
static void kdp_process_arp_reply(struct kdp_ether_arp *ea) {
...
static void kdp_arp_reply(struct kdp_ether_arp *ea) {
  struct kdp_ether_header *eh;
...
static void kdp_poll(void) {
  ...
  if (pkt.len <
      (sizeof(struct kdp_ether_header) + sizeof(struct kdp_udpiphdr))) {
    return;
  }
  ...
  if (ntohs(ui->ui_dport) != KDP_REMOTE_PORT) {
    if (panicd_port == (ntohs(ui->ui_dport)) && flag_panic_dump_in_progress) {
   ...
   */
  pkt.len =
      ntohs((u_short)ui->ui_ulen) - (unsigned int)sizeof(struct kdp_udphdr);
  pkt.input = TRUE;
}
...
static void transmit_ARP_request(uint32_t ip_addr) {
  struct kdp_ether_header *eh = (struct kdp_ether_header *)&pkt.data[0];
  struct kdp_ether_arp *ea =
      (struct kdp_ether_arp *)&pkt.data[sizeof(struct kdp_ether_header)];
  ...
  memcpy(ea->arp_spa, (void *)&kdp_current_ip_address,
...
static boolean_t kdp_arp_resolve(uint32_t arp_target_ip,
                                 struct kdp_ether_addr *resolved_MAC) {
...
TRANSMIT_RETRY:
  ...
  while (!pkt.input && !flag_arp_resolved && flag_panic_dump_in_progress &&
         --poll_count) {
    kdp_poll();
  }
  ...
  if (!flag_panic_dump_in_progress ||
      pkt.input) { /* we received a debugging packet, bail*/
...
static void kdp_handler(void *saved_state) {
  ...
  do {
    ...
    if (hdr->seq == (exception_seq - 1)) {
      ...
      kdp_send_data(&saved_reply.data[saved_reply.off], saved_reply.len);
      goto again;
    } else if ((hdr->seq != exception_seq) && (hdr->request != KDP_CONNECT)) {
    ...
    if (manual_pkt.input) {
      ...
      if (!manual_hdr->is_reply) {
        ...
        kdp_packet((unsigned char *)&manual_pkt.data, &packet_length,
                   &manual_port_unused);
    ...
    if (kdp_packet((unsigned char *)&pkt.data[pkt.off], (int *)&pkt.len,
                   (unsigned short *)&reply_port)) {
      ...
      if (hdr->request == KDP_CONNECT) {
        kdp_connect_reply_t *rp = (kdp_connect_reply_t *)&pkt.data[pkt.off];
...
static void kdp_connection_wait(void) {
  ...
  if (KDP_SERIAL_ENABLED()) {
    ...
    printf("ip address: %u.%u.%u.%u\n", (ip_addr & 0xff000000) >> 24,
           (ip_addr & 0xff0000) >> 16, (ip_addr & 0xff00) >> 8,
           (ip_addr & 0xff));
...
static void kdp_send_exception(unsigned int exception, unsigned int code,
  ...
  do {
    ...
    kdp_exception((unsigned char *)&pkt.data[pkt.off], (int *)&pkt.len,
                  (unsigned short *)&remote_port, (unsigned int)exception,
...
static void kdp_debugger_loop(unsigned int exception, unsigned int code,
  ...
  if (exception != EXC_BREAKPOINT) {
    ...
    printf("%s exception (%x,%x,%x)\n", exception_message[index], exception,
           code, subcode);
  }
  ...
  if (((kdp_flag & KDP_PANIC_DUMP_ENABLED) || (kdp_flag & PANIC_LOG_DUMP)) &&
    ...
    if ((kdp_flag & PANIC_CORE_ON_NMI) && !panic_active() && !kdp.is_conn) {
...
struct corehdr *create_panic_header(unsigned int request, const char *corename,
  ...
  size_t fmask_size =
      sizeof(KDP_FEATURE_MASK_STRING) + sizeof(kdp_crashdump_feature_mask);

  pkt.off = sizeof(struct kdp_ether_header);
  pkt.len = (unsigned int)(length +
                           ((request == KDP_WRQ) ? modelen + fmask_size : 0) +
                           (corename ? (strlen(corename) + 1) : 0) +
  ...
  if (request == KDP_WRQ) {
    ...
    /* Calculate available string space (remaining space after accounting for
     * mandatory components). */
    size_t length_remaining =
        (sizeof(pkt.data) - pkt.off - offsetof(struct corehdr, th_u) -
     ...
     */
    int len = snprintf(cp, length_remaining, "%s%c%s%c%s%c", corename, '\0',
                       mode, '\0', KDP_FEATURE_MASK_STRING, '\0');
    if (len < 0) {
    ...
    /* Append feature flags. The value is already converted with htonl in
     * startup code. */
...
static int kdp_send_crashdump_seek(char *corename, uint64_t seek_off) {
  ...
  if (kdp_feature_large_crashdumps) {
    panic_error =
        kdp_send_crashdump_pkt(KDP_SEEK, corename, sizeof(seek_off), &seek_off);
  } else {
    uint32_t off = (uint32_t)seek_off;
    panic_error = kdp_send_crashdump_pkt(KDP_SEEK, corename, sizeof(off), &off);
  }
...
int kdp_send_crashdump_data(unsigned int request, char *corename,
  ...
  while ((length > 0) || !txstart) {
    ...
    panic_error = kdp_send_crashdump_pkt(request, corename, chunk, txstart);
    if (panic_error < 0) {
      printf("kdp_send_crashdump_pkt failed with error %d\n", panic_error);
      return panic_error;
    }
...
TRANSMIT_RETRY:
  ...
  if (tretries >= 15) {
    /* The crashdump server is unreachable for some reason. This could be a
     * network issue or, if we've been especially unfortunate, we've hit Radar
     * 2760413, which is a long standing problem with the IOKit polled mode
     * network driver shim which can prevent transmits/receives completely.
     */
  ...
  if (request == KDP_DATA) {
    ...
    if (!kdp_machine_vm_read((mach_vm_address_t)(uintptr_t)panic_data,
                             (caddr_t)th->th_data, length)) {
      ...
      if ((next_page - ((uintptr_t)panic_data)) < length) {
        ...
        if (!kdp_machine_vm_read((mach_vm_address_t)(uintptr_t)next_page,
                                 (caddr_t)th->th_data + (length - resid),
                                 resid)) {
...
RECEIVE_RETRY:
    ...
    if (request == KDP_WRQ) {
      ...
      if ((opcode64 & 0xFF) == KDP_ACK) {
        kdp_feature_large_crashdumps =
            features64 & KDP_FEATURE_LARGE_CRASHDUMPS;
  ...
  } else if (!flag_panic_dump_in_progress) { /* we received a debugging packet,
                                                bail*/
    printf("Received a debugger packet,transferring control to debugger\n");
  ...
  if (request == KDP_EOF) {
    ...
    printf("Avg. superblock transfer abstime 0x%llx\n",
           ((mach_absolute_time() - kdp_dump_start_time) / panic_block) *
               SBLOCKSZ);
...
static int isdigit(char c) { return (c > 47) && (c < 58); }
...
static int kdp_get_xnu_version(char *versionbuf) {
  ...
  if (kdp_machine_vm_read((mach_vm_address_t)(uintptr_t)version, versionbuf,
                          128)) {
    versionbuf[127] = '\0';
...
void kdp_set_dump_info(const uint32_t flags, const char *filename,
  ...
  flag_dont_abort_panic_dump = (flags & KDP_DUMPINFO_NOINTR) ? TRUE : FALSE;
...
void kdp_get_dump_info(kdp_dumpinfo_reply_t *rp) {
  if (panicd_specified) {
    strlcpy(rp->destip, panicd_ip_str, sizeof(rp->destip));
  } else {
    rp->destip[0] = '\0';
  }

  if (router_specified) {
    strlcpy(rp->routerip, router_ip_str, sizeof(rp->routerip));
  } else {
    rp->routerip[0] = '\0';
  }

  if (corename_specified) {
    strlcpy(rp->name, corename_str, sizeof(rp->name));
  } else {
    rp->name[0] = '\0';
  }
...
void kdp_panic_dump(void) {
  ...
  if (!panicd_specified) {
    kdb_printf("A dump server was not specified in the boot-args, terminating "
               "kernel core dump.\n");
    goto panic_dump_exit;
  }
  ...
  if (!corename_specified) {
    ...
    pkt.data[20] = '\0';
    snprintf(corename_str, sizeof(corename_str), "%s-%s-%u.%u.%u.%u-%x%s",
             coreprefix, &pkt.data[0], (current_ip & 0xff000000) >> 24,
             (current_ip & 0xff0000) >> 16, (current_ip & 0xff00) >> 8,
             (current_ip & 0xff), (unsigned int)(abstime & 0xffffffff),
             coresuffix);
  }
  ...
  if (kdp_arp_resolve(panic_server_ip, &temp_mac)) {
    ...
    if (router_specified) {
      if (0 ==
          inet_aton(router_ip_str, (struct kdp_in_addr *)&parsed_router_ip)) {
  ...
  kdb_printf("Transmitting packets to link level address: "
  ...
  if ((panic_error = kdp_send_crashdump_pkt(KDP_WRQ, corename_str, 0, NULL)) <
      0) {
  ...
  if ((debugger_panic_str != (char *)0) && (kdp_flag & PANIC_LOG_DUMP)) {
    ...
    kdp_send_crashdump_data(KDP_DATA, corename_str,
                            debug_buf_ptr - debug_buf_base, debug_buf_base);
...
void begin_panic_transfer(void) { flag_panic_dump_in_progress = TRUE; }
...
static void kdp_serial_receive(void *rpkt, unsigned int *rpkt_len,
                               unsigned int timeout) {
  int readkar;
  uint64_t now, deadline;

  clock_interval_to_deadline(timeout, 1000 * 1000 /* milliseconds */,
                             &deadline);
...
static boolean_t kdp_serial_setmode(boolean_t active) {
...
void kdp_init(void) {
  ...
  if (kernel_uuid_string[0] != '\0') {
    ...
    strlcat(kdp_kernelversion_string,
            "; UUID=", sizeof(kdp_kernelversion_string));
    strlcat(kdp_kernelversion_string, kernel_uuid_string,
  ...
  if (vm_kernel_slide) {
    ...
    strlcat(kdp_kernelversion_string, KASLR_stext,
            sizeof(kdp_kernelversion_string));
  }
#endif
  ...
  boolean_t kdp_match_name_found =
      PE_parse_boot_argn("kdp_match_name", kdpname, sizeof(kdpname));
  boolean_t kdp_not_serial = kdp_match_name_found
                                 ? (strncmp(kdpname, "serial", sizeof(kdpname)))
                                 : TRUE;
  ...
  if (kdp_not_serial && PE_consistent_debug_enabled() && debug_boot_arg) {
    ...
    printf("Serial requested, consistent debug disabled or debug boot arg not "
           "present, configuring debugging over serial\n");
  }
#endif /* defined(__arm64__) */
...
void *kdp_get_interface(void);
void kdp_set_ip_and_mac_addresses(struct kdp_in_addr *,
                                  struct kdp_ether_addr *);
...
void kdp_register_send_receive(__unused void *send, __unused void *receive) {}

void kdp_unregister_send_receive(__unused void *send, __unused void *receive) {}

void *kdp_get_interface(void) { return (void *)0; }

unsigned int kdp_get_ip_address(void) { return 0; }

struct kdp_ether_addr kdp_get_mac_addr(void) { return kdp_current_mac_address; }

void kdp_set_ip_and_mac_addresses(__unused struct kdp_in_addr *ipaddr,
                                  __unused struct kdp_ether_addr *macaddr) {}

void kdp_set_gateway_mac(__unused void *gatewaymac) {}

void kdp_set_interface(__unused void *ifp) {}
...
kdp_raise_exception(__unused unsigned int exception, __unused unsigned int code,
                    __unused unsigned int subcode, __unused void *saved_state)
#else
