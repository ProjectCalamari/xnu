 ...
 * kdp_memcpy() internally, which copies individual bytes if running in the
 * panic context.
 */
size_t kdp_strlcpy(char *dst, const char *src, size_t maxlen);

/*
 * Get the page size from the specified vm map. This correctly handles K16/U4
...
__options_closed_decl(
    kdp_fault_result_flags_t, uint32_t,
    {
        KDP_FAULT_RESULT_PAGED_OUT =
            0x1, /* some data was unable to be retrieved */
...
__options_closed_decl(
    kdp_fault_flags_t, uint32_t,
    {
        KDP_FAULT_FLAGS_NONE = 0x0,
        KDP_FAULT_FLAGS_ENABLE_FAULTING =
            0x1, /* try faulting if pages are not resident */
        KDP_FAULT_FLAGS_MULTICPU = 0x2, /* multicpu kdp context */
    });

__options_closed_decl(
    kdp_traverse_mappings_flags_t, uint32_t,
    {
        KDP_TRAVERSE_MAPPINGS_FLAGS_NONE = 0x0,
        KDP_TRAVERSE_MAPPINGS_FLAGS_PHYSICAL =
            0x1 /* Use physical addresses instead of virtual addresses */
    });

typedef int (*kdp_traverse_mappings_callback)(vm_offset_t start,
                                              vm_offset_t end, void *context);

/*
 * Traverse mappings in the specified task.
 *
 * - task                      The task
 * - fault_flags               Controls whether to fault in pages that are not
 * resident.
 * - traverse_mappings_flags   Controls whether the callback is called with
 * physical addresses
 ...
 */
typedef vm_offset_t (*find_phys_fn_t)(vm_map_t map, vm_offset_t target_addr,
                                      kdp_fault_flags_t fault_flags,
                                      void *context);
 ...
 * This is passed to `find_phys_fn`.
 * - find_phys_fn    The function to use to return a physical address given a
 * map and target address. If additional filtering/handling is not required, use
 * `(find_phys_fn_t)kdp_find_phys` for this parameter.
 * - context         Reference context passed to find_phys_fn
 *
 * Copies in `size` bytes from `map:uaddr` to `dest`, using the specified
 * function to find a physical address. Returns 0 if successful, an errno
 * otherwise.
 */
