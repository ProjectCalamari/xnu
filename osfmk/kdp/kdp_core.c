...
/*
 * The main orchestrator for kernel (and co-processor) coredumps. Here's a very
 * simplistic view of the flow:
 *
 * At kernel initialization time (kdp_core_init):
 * ----------------------------------------------
 *
 * - kdp_core_init() takes care of allocating all necessary data structures and
 * initializes the coredump output stages
 *
 * At coredump time (do_kern_dump):
 * --------------------------------
 *
 * - Depending on the coredump variant, we chain the necessary output stages
 * together in chain_output_stages()
 * - [Disk only] We initialize the corefile header
 * - [Disk only] We stream the stackshot out through the output stages and
 * update the corefile header
 * - We perform the kernel coredump, streaming it out through the output stages
 * - [Disk only] We update the corefile header
 * - [Disk only] We perform the co-processor coredumps (driven by
 * kern_do_coredump), streaming each out through the output stages and updating
 * the corefile header.
...
kern_return_t
kdp_core_polled_io_polled_file_available(IOCoreFileAccessCallback access_data,
                                         void *access_context,
...
typedef int (*pmap_traverse_callback)(vm_map_offset_t start,
                                      vm_map_offset_t end, void *context);

static kern_return_t kern_dump_init(void *refcon, void *context);
static int kern_dump_save_summary(void *refcon, core_save_summary_cb callback,
                                  void *context);
static int kern_dump_save_seg_descriptions(
    void *refcon, core_save_segment_descriptions_cb callback, void *context);
static int kern_dump_save_thread_state(void *refcon, void *buf,
                                       core_save_thread_state_cb callback,
                                       void *context);
static int kern_dump_save_sw_vers_detail(void *refcon,
                                         core_save_sw_vers_detail_cb callback,
                                         void *context);
static int kern_dump_save_segment_data(void *refcon,
                                       core_save_segment_data_cb callback,
                                       void *context);
static kern_return_t
kern_dump_save_note_summary(void *refcon, core_save_note_summary_cb callback,
                            void *context);
static kern_return_t kern_dump_save_note_descriptions(
    void *refcon, core_save_note_descriptions_cb callback, void *context);
static kern_return_t kern_dump_save_note_data(void *refcon,
                                              core_save_note_data_cb callback,
                                              void *context);

static int kern_dump_pmap_traverse_preflight_callback(vm_map_offset_t start,
                                                      vm_map_offset_t end,
                                                      void *context);
static int kern_dump_pmap_traverse_send_segdesc_callback(vm_map_offset_t start,
                                                         vm_map_offset_t end,
                                                         void *context);

static int kern_dump_pmap_traverse_send_segdata_callback(vm_map_offset_t start,
                                                         vm_map_offset_t end,
                                                         void *context);
...
static const size_t kdp_core_header_size =
    sizeof(struct mach_core_fileheader_v2) +
    (KERN_COREDUMP_MAX_CORES * sizeof(struct mach_core_details_v2));
...
#ifdef CONFIG_KDP_COREDUMP_ENCRYPTION
static const size_t PUBLIC_KEY_RESERVED_LENGTH =
    roundup(4096, KERN_COREDUMP_BEGIN_FILEBYTES_ALIGN);
...
void kdp_core_exclude_region(vm_offset_t addr, vm_size_t size) {
  ...
  if (addr >= addr + size) {
    panic("%s: cannot exclude region starting at %p with size %zu (zero or "
          "overflowing size)",
          __func__, (void *)addr, (size_t)size);
  }
  if (addr != round_page(addr) || size != round_page(size)) {
    panic("%s: cannot exclude region starting at %p with size %zu (not page "
          "aligned)",
          __func__, (void *)addr, (size_t)size);
  }
...
void kdp_core_unexclude_region(vm_offset_t addr, vm_size_t size) {
  ...
  if (!region) {
    panic("%s: cannot unexclude region starting at %p with size %zu (not "
          "currently excluded)",
          __func__, (void *)addr, (size_t)size);
  }
  lck_mtx_unlock(&excluded_regions_mtx);

  // We had exclusive access to the list when we removed the region, and it is
  // no longer reachable from the list, so it is safe to free.
  kfree_type(typeof(*region), region);
}

static bool kernel_vaddr_in_excluded_region(vm_offset_t addr, uint64_t *vincr) {
  struct kdp_core_excluded_region *region;

  // We check this earlier before attempting to dump the kernel, but verify
  // here.
...
kern_return_t kdp_core_output(void *kdp_core_out_state, uint64_t length,
                              void *data) {
  kern_return_t err = KERN_SUCCESS;
  uint64_t percent;
  struct kdp_core_out_state *vars =
      (struct kdp_core_out_state *)kdp_core_out_state;
  ...
  if (vars->kcos_error == KERN_SUCCESS) {
    ...
    if ((panic_test_case & PANIC_TEST_CASE_COREFILE_IO_ERR) &&
        (--panic_test_action_count == 0)) {
      panic_test_case &= ~PANIC_TEST_CASE_COREFILE_IO_ERR;
      length = -1;
    }
#endif

    if ((err = first_stage->kos_funcs.kosf_outproc(
             first_stage, KDP_DATA, NULL, length, data)) != KERN_SUCCESS) {
      kern_coredump_log(NULL,
                        "(kdp_core_output) outproc(KDP_DATA, NULL, 0x%llx, %p) "
                        "returned 0x%x\n",
                        length, data, err);
      vars->kcos_error = err;
    }
...
static bool kernel_vaddr_in_coredump_stage(const struct kdp_output_stage *stage,
                                           uint64_t vaddr, uint64_t *vincr) {
...
static bool kernel_vaddr_in_coredump_stages(uint64_t vaddr, uint64_t *vincr) {
  ...
  if (kernel_vaddr_in_coredump_stage(&progress_notify_output_stage, vaddr,
                                     vincr)) {
    return true;
  }
...
#if defined(__arm64__)
  if (kernel_vaddr_in_coredump_stage(&memory_backing_aware_buffer_output_stage,
                                     vaddr, vincr)) {
    return true;
  }
#endif /* defined(__arm64__) */
...
ppnum_t kernel_pmap_present_mapping(uint64_t vaddr, uint64_t *pvincr,
...
#endif /* defined(__arm64__) */
  {
    ppn = (pvphysaddr != NULL ? pmap_find_phys(kernel_pmap, vaddr)
...
static int pmap_traverse_present_mappings(pmap_t __unused pmap,
  ...
  vm_map_offset_t debug_end =
      round_page((vm_map_offset_t)(debug_buf_base + debug_buf_size));
#if defined(XNU_TARGET_OS_BRIDGE)
  vm_map_offset_t macos_panic_start =
      trunc_page((vm_map_offset_t)macos_panic_base);
  vm_map_offset_t macos_panic_end =
      round_page((vm_map_offset_t)(macos_panic_base + macos_panic_size));
#endif
  ...
  for (vcur = vcurstart = start; (ret == KERN_SUCCESS) && (vcur < end);) {
    ...
    if (m != VM_PAGE_NULL) {
      ...
      while (!vm_page_queue_end(&pmap_object_store.memq,
                                (vm_page_queue_entry_t)m)) {
        /* Ignore pages that come from the static region and have already been
         * dumped.*/
...
int kern_dump_pmap_traverse_preflight_callback(vm_map_offset_t start,
                                               vm_map_offset_t end,
                                               void *context) {
  struct kern_dump_preflight_context *kdc =
      (struct kern_dump_preflight_context *)context;
...
int kern_dump_pmap_traverse_send_segdesc_callback(vm_map_offset_t start,
                                                  vm_map_offset_t end,
                                                  void *context) {
  struct kern_dump_send_seg_desc_context *kds_context =
      (struct kern_dump_send_seg_desc_context *)context;
...
int kern_dump_pmap_traverse_send_segdata_callback(vm_map_offset_t start,
                                                  vm_map_offset_t end,
                                                  void *context) {
  struct kern_dump_send_segdata_context *kds_context =
      (struct kern_dump_send_segdata_context *)context;

  return kds_context->callback((void *)start, (uint64_t)(end - start),
                               kds_context->context);
}

static kern_return_t kern_dump_init(__unused void *refcon, void *context) {
...
static int kern_dump_save_summary(__unused void *refcon,
  ...
  ret = pmap_traverse_present_mappings(
      kernel_pmap, vstart, VM_MAX_KERNEL_ADDRESS,
      kern_dump_pmap_traverse_preflight_callback, &kdc_preflight);
  if (ret != KERN_SUCCESS) {
...
static int
kern_dump_save_seg_descriptions(__unused void *refcon,
                                core_save_segment_descriptions_cb callback,
                                void *context) {
  vm_map_offset_t vstart = kdp_core_start_addr();
  kern_return_t ret;
  ...
  ret = pmap_traverse_present_mappings(
      kernel_pmap, vstart, VM_MAX_KERNEL_ADDRESS,
      kern_dump_pmap_traverse_send_segdesc_callback, &kds_context);
  if (ret != KERN_SUCCESS) {
...
static int kern_dump_save_thread_state(__unused void *refcon, void *buf,
                                       core_save_thread_state_cb callback,
                                       void *context) {
  kern_return_t ret;
  uint64_t thread_state_size = 0, thread_count = 0;
...
static int kern_dump_save_sw_vers_detail(__unused void *refcon,
                                         core_save_sw_vers_detail_cb callback,
                                         void *context) {
  return callback(vm_kernel_stext, kernel_uuid, 0, context);
}

static int kern_dump_save_segment_data(__unused void *refcon,
                                       core_save_segment_data_cb callback,
                                       void *context) {
  vm_map_offset_t vstart = kdp_core_start_addr();
  kern_return_t ret;
  ...
  ret = pmap_traverse_present_mappings(
      kernel_pmap, vstart, VM_MAX_KERNEL_ADDRESS,
      kern_dump_pmap_traverse_send_segdata_callback, &kds_context);
  if (ret != KERN_SUCCESS) {
...
kern_return_t kdp_reset_output_vars(void *kdp_core_out_state,
                                    uint64_t totalbytes, bool encrypt_core,
                                    bool *out_should_skip_coredump,
                                    const char *corename,
  ...
  STAILQ_FOREACH(current_stage, &outstate->kcos_out_stage, kos_next) {
    kern_return_t res =
        current_stage->kos_funcs.kosf_reset(current_stage, corename, coretype);
...
#endif
      kern_coredump_log(
          NULL, "(kdp_reset_output_vars) Encryption requested, is unavailable, "
                "and enforcement is active. Skipping current core.\n");
    }
...
kern_dump_update_header(struct kdp_core_out_state *outstate) {
  struct kdp_output_stage *first_stage =
      STAILQ_FIRST(&outstate->kcos_out_stage);
  uint64_t foffset;
  kern_return_t ret;

  /* Write the file header -- first seek to the beginning of the file */
  foffset = 0;
  if ((ret = (first_stage->kos_funcs.kosf_outproc)(first_stage, KDP_SEEK, NULL,
                                                   sizeof(foffset),
                                                   &foffset)) != KERN_SUCCESS) {
    kern_coredump_log(NULL,
                      "(kern_dump_update_header) outproc(KDP_SEEK, NULL, %lu, "
                      "%p) foffset = 0x%llx returned 0x%x\n",
                      sizeof(foffset), &foffset, foffset, ret);
    return ret;
  }

  if ((ret = (first_stage->kos_funcs.kosf_outproc)(
           first_stage, KDP_DATA, NULL, kdp_core_header_size,
           kdp_core_header)) != KERN_SUCCESS) {
    kern_coredump_log(NULL,
                      "(kern_dump_update_header) outproc(KDP_DATA, NULL, %lu, "
                      "%p) returned 0x%x\n",
                      kdp_core_header_size, kdp_core_header, ret);
    return ret;
  }

  if ((ret = (first_stage->kos_funcs.kosf_outproc)(first_stage, KDP_DATA, NULL,
                                                   0, NULL)) != KERN_SUCCESS) {
    kern_coredump_log(
        NULL, "(kern_dump_update_header) outproc data flush returned 0x%x\n",
        ret);
    return ret;
  }

#if defined(__arm64__)
  if ((ret = (first_stage->kos_funcs.kosf_outproc)(first_stage, KDP_FLUSH, NULL,
                                                   0, NULL)) != KERN_SUCCESS) {
    kern_coredump_log(
        NULL,
...
kern_return_t kern_dump_record_file(void *kdp_core_out_state,
                                    const char *filename, uint64_t file_offset,
                                    uint64_t *out_file_length,
                                    uint64_t details_flags) {
  ...
  *out_file_length = 0;
  ...
  last_stage =
      STAILQ_LAST(&outstate->kcos_out_stage, kdp_output_stage, kos_next);
...
kern_return_t kern_dump_seek_to_next_file(void *kdp_core_out_state,
  ...
  if ((ret = (first_stage->kos_funcs.kosf_outproc)(
           first_stage, KDP_SEEK, NULL, sizeof(next_file_offset),
           &next_file_offset)) != KERN_SUCCESS) {
    kern_coredump_log(NULL,
                      "(kern_dump_seek_to_next_file) outproc(KDP_SEEK, NULL, "
                      "%lu, %p) foffset = 0x%llx returned 0x%x\n",
                      sizeof(next_file_offset), &next_file_offset,
                      next_file_offset, ret);
  }
...
kern_dump_write_public_key(struct kdp_core_out_state *outstate) {
  ...
  uint64_t foffset;
  uint64_t remainder =
      PUBLIC_KEY_RESERVED_LENGTH - kdp_core_header->pub_key_length;
  kern_return_t ret;

  if (kdp_core_header->pub_key_offset == 0 ||
  ...
  if ((ret = (first_stage->kos_funcs.kosf_outproc)(first_stage, KDP_SEEK, NULL,
                                                   ...
                                                   &foffset)) != KERN_SUCCESS) {
    kern_coredump_log(NULL,
                      "(kern_dump_write_public_key) outproc(KDP_SEEK, NULL, "
                      "%lu, %p) foffset = 0x%llx returned 0x%x\n",
                      sizeof(foffset), &foffset, foffset, ret);
    return ret;
  }
  ...
  if ((ret = (first_stage->kos_funcs.kosf_outproc)(
           first_stage, KDP_DATA, NULL, kdp_core_header->pub_key_length,
           kdp_core_public_key)) != KERN_SUCCESS) {
    kern_coredump_log(NULL,
                      "(kern_dump_write_public_key) outproc(KDP_DATA, NULL, "
                      "%u, %p) returned 0x%x\n",
                      kdp_core_header->pub_key_length, kdp_core_public_key,
                      ret);
    return ret;
  }
  ...
  if ((ret = (first_stage->kos_funcs.kosf_outproc)(
           first_stage, KDP_DATA, NULL, remainder, NULL)) != KERN_SUCCESS) {
    kern_coredump_log(NULL,
                      "(kern_dump_write_public_key) outproc(KDP_DATA, NULL, "
                      "%llu, NULL) returned 0x%x\n",
                      remainder, ret);
    return ret;
  }
  ...
  if ((ret = (first_stage->kos_funcs.kosf_outproc)(first_stage, KDP_DATA, NULL,
                                                   0, NULL)) != KERN_SUCCESS) {
    kern_coredump_log(
        NULL, "(kern_dump_write_public_key) outproc data flush returned 0x%x\n",
        ret);
    return ret;
  }
...
static kern_return_t chain_output_stages(enum kern_dump_type kd_variant,
                                         struct kdp_core_out_state *outstate,
                                         uint64_t *details_flags) {
  ...
  *details_flags = 0;
  ...
  case KERN_DUMP_DISK:
#if defined(__arm64__)
    STAILQ_INSERT_TAIL(&outstate->kcos_out_stage,
                       &memory_backing_aware_buffer_output_stage, kos_next);
#endif
    if (!kdp_corezip_disabled) {
      if (kdp_core_is_initializing_lz4_stage) {
        kern_coredump_log(NULL, "We were in the middle of initializing LZ4 "
                                "stage. Cannot write a coredump to disk\n");
        return KERN_FAILURE;
      } else if (!lz4_output_stage.kos_initialized) {
        kern_coredump_log(NULL, "LZ4 stage is not yet initialized. Cannot "
                                "write a coredump to disk\n");
        return KERN_FAILURE;
      }
      STAILQ_INSERT_TAIL(&outstate->kcos_out_stage, &lz4_output_stage,
                         kos_next);
      *details_flags |= MACH_CORE_DETAILS_V2_FLAG_COMPRESSED_LZ4;
    }
    STAILQ_INSERT_TAIL(&outstate->kcos_out_stage, &progress_notify_output_stage,
                       kos_next);
#ifdef CONFIG_KDP_COREDUMP_ENCRYPTION
    if (kdp_core_is_initializing_encryption_stage) {
      kern_coredump_log(NULL, "We were in the middle of initializing "
                              "encryption. Marking it as unavailable\n");
    } else if (aea_output_stage.kos_initialized) {
      STAILQ_INSERT_TAIL(&outstate->kcos_out_stage, &aea_output_stage,
                         kos_next);
    ...
    if (kdp_core_is_initializing_disk_stage) {
      kern_coredump_log(NULL, "We were in the middle of initializing the disk "
                              "stage. Cannot write a coredump to disk\n");
      return KERN_FAILURE;
  ...
  case KERN_DUMP_NET:
    if (!kdp_corezip_disabled) {
      if (!zlib_output_stage.kos_initialized) {
        kern_coredump_log(NULL, "Zlib stage is not initialized. Cannot write a "
                                "coredump to the network\n");
        return KERN_FAILURE;
      }
      STAILQ_INSERT_TAIL(&outstate->kcos_out_stage, &zlib_output_stage,
                         kos_next);
      *details_flags |= MACH_CORE_DETAILS_V2_FLAG_COMPRESSED_ZLIB;
    }
    STAILQ_INSERT_TAIL(&outstate->kcos_out_stage, &progress_notify_output_stage,
                       kos_next);
    STAILQ_INSERT_TAIL(&outstate->kcos_out_stage, &buffer_output_stage,
                       kos_next);
  ...
  case KERN_DUMP_HW_SHMEM_DBG:
    ...
    STAILQ_INSERT_TAIL(&outstate->kcos_out_stage, &shmem_output_stage,
                       kos_next);
    break;
#endif /* defined(__arm64__) */
  }
...
static kern_return_t dump_panic_buffer(struct kdp_core_out_state *outstate,
                                       char *panic_buf, size_t panic_len,
                                       uint64_t *foffset,
                                       uint64_t details_flags) {
  kern_return_t ret = KERN_SUCCESS;
  ...
  if (KERN_SUCCESS != ret) {
    kern_coredump_log(NULL,
                      "Failed to write panic region to file, "
                      "kdp_coreoutput(outstate, %zu, %p) returned 0x%x\n",
  ...
  if (KERN_SUCCESS != ret) {
    kern_coredump_log(NULL,
                      "Failed to flush panic region data : kdp_core_output(%p, "
                      "0, NULL) returned 0x%x\n",
                      outstate, ret);
    return ret;
  }

  ret = kern_dump_record_file(outstate, panic_buf_filename, *foffset,
                              &compressed_panic_region_len, details_flags);
  if (KERN_SUCCESS != ret) {
    kern_coredump_log(NULL,
                      "Failed to record panic region in corefile header, "
                      "kern_dump_record_file returned 0x%x\n",
                      ret);
    return ret;
  }

  kern_coredump_log(NULL,
                    "Recorded panic region in corefile at offset 0x%llx, "
                    "compressed to %llu bytes\n",
                    *foffset, compressed_panic_region_len);
  *foffset = roundup((*foffset + compressed_panic_region_len),
                     KERN_COREDUMP_BEGIN_FILEBYTES_ALIGN);

  ret = kern_dump_seek_to_next_file(outstate, *foffset);
  if (KERN_SUCCESS != ret) {
    kern_coredump_log(NULL,
                      "Failed to seek to panic region file offset 0x%llx, "
                      "kern_dump_seek_to_next_file returned 0x%x\n",
                      *foffset, ret);
    return ret;
  }
...
static int do_kern_dump(enum kern_dump_type kd_variant) {
  struct kdp_core_out_state outstate = {};
  ...
  prior_debug_logsize = (panic_info->eph_panic_log_offset -
                         sizeof(struct embedded_panic_header)) +
                        panic_info->eph_panic_log_len +
                        panic_info->eph_other_log_len;
#else  /* defined(__arm64__) */
  if (panic_info->mph_panic_log_offset != 0) {
    prior_debug_logsize =
        (panic_info->mph_panic_log_offset - sizeof(struct macos_panic_header)) +
  ...
  if ((kd_variant == KERN_DUMP_DISK) ||
      (kd_variant == KERN_DUMP_STACKSHOT_DISK)) {
    /* Open the file for output */
    if ((ret = first_stage->kos_funcs.kosf_outproc(first_stage, KDP_WRQ, NULL,
                                                   0, NULL)) != KERN_SUCCESS) {
  ...
  if ((kd_variant == KERN_DUMP_DISK) ||
      (kd_variant == KERN_DUMP_STACKSHOT_DISK)) {
    const size_t aligned_corefile_header_size =
        roundup(kdp_core_header_size, KERN_COREDUMP_BEGIN_FILEBYTES_ALIGN);
#ifdef CONFIG_KDP_COREDUMP_ENCRYPTION
    ...
    reserved_debug_logsize =
        prior_debug_logsize + KERN_COREDUMP_MAXDEBUGLOGSIZE;

    /* Space for file header, public key, panic log, core log */
    foffset = roundup(aligned_corefile_header_size + aligned_public_key_size +
                          reserved_debug_logsize,
                      KERN_COREDUMP_BEGIN_FILEBYTES_ALIGN);
    kdp_core_header->log_offset =
        aligned_corefile_header_size + aligned_public_key_size;
    ...
    /* Seek the calculated offset (we'll scrollback later to flush the logs and
     * header) */
    if ((ret = first_stage->kos_funcs.kosf_outproc(first_stage, KDP_SEEK, NULL,
                                                   sizeof(foffset),
                                                   &foffset)) != KERN_SUCCESS) {
      kern_coredump_log(NULL,
                        "(do_kern_dump seek begin) outproc(KDP_SEEK, NULL, "
                        "%lu, %p) foffset = 0x%llx returned 0x%x\n",
                        sizeof(foffset), &foffset, foffset, ret);
      dump_succeeded = FALSE;
      goto exit;
  ...
  kern_coredump_log(NULL, "%s",
                    (kd_variant == KERN_DUMP_DISK)
                        ? "Writing local cores...\n"
  ...
  if (kd_variant == KERN_DUMP_DISK && (panic_buf && panic_len)) {
    ret = dump_panic_buffer(&outstate, panic_buf, panic_len, &foffset,
                            details_flags);
    if (KERN_SUCCESS != ret) {
      dump_succeeded = FALSE;
    }
...
#if defined(__x86_64__)
  if (((kd_variant == KERN_DUMP_STACKSHOT_DISK) ||
       (kd_variant == KERN_DUMP_DISK)) &&
      ((panic_stackshot_buf != 0) && (panic_stackshot_len != 0))) {
    bool should_skip = false;
    static const char *stackshot_filename = "panic_stackshot.kcdata";

    kern_coredump_log(NULL, "\nBeginning dump of kernel stackshot\n");

    ret = kdp_reset_output_vars(&outstate, panic_stackshot_len, true,
                                &should_skip, stackshot_filename, RAW_COREDUMP);

    if (ret != KERN_SUCCESS) {
      kern_coredump_log(NULL,
                        "Failed to reset outstate for stackshot with len "
                        "0x%zx, returned 0x%x\n",
                        panic_stackshot_len, ret);
      dump_succeeded = FALSE;
    } else if (!should_skip) {
      uint64_t compressed_stackshot_len = 0;
      if ((ret = kdp_core_output(&outstate, panic_stackshot_len,
                                 (void *)panic_stackshot_buf)) !=
          KERN_SUCCESS) {
        kern_coredump_log(NULL,
                          "Failed to write panic stackshot to file, "
                          "kdp_coreoutput(outstate, %lu, %p) returned 0x%x\n",
                          panic_stackshot_len, (void *)panic_stackshot_buf,
                          ret);
        dump_succeeded = FALSE;
      } else if ((ret = kdp_core_output(&outstate, 0, NULL)) != KERN_SUCCESS) {
        kern_coredump_log(NULL,
                          "Failed to flush stackshot data : "
                          "kdp_core_output(%p, 0, NULL) returned 0x%x\n",
                          &outstate, ret);
        dump_succeeded = FALSE;
      } else if ((ret =
                      kern_dump_record_file(&outstate, stackshot_filename,
                                            foffset, &compressed_stackshot_len,
                                            details_flags)) != KERN_SUCCESS) {
        kern_coredump_log(NULL,
                          "Failed to record panic stackshot in corefile "
                          "header, kern_dump_record_file returned 0x%x\n",
                          ret);
        dump_succeeded = FALSE;
      } else {
        kern_coredump_log(NULL,
                          "Recorded panic stackshot in corefile at offset "
                          "0x%llx, compressed to %llu bytes\n",
                          foffset, compressed_stackshot_len);
        foffset = roundup((foffset + compressed_stackshot_len),
                          KERN_COREDUMP_BEGIN_FILEBYTES_ALIGN);
        if ((ret = kern_dump_seek_to_next_file(&outstate, foffset)) !=
            KERN_SUCCESS) {
          kern_coredump_log(NULL,
                            "Failed to seek to stackshot file offset 0x%llx, "
                            "kern_dump_seek_to_next_file returned 0x%x\n",
                            foffset, ret);
          dump_succeeded = FALSE;
        }
  ...
  if (kd_variant == KERN_DUMP_DISK) {
     ...
     */
    if (kern_do_coredump(&outstate, KCF_NONE, foffset, &foffset,
                         details_flags) != 0) {
      dump_succeeded = FALSE;
    }
#if defined(__arm64__)
    ...
    if (kern_do_coredump(&outstate, KCF_ABORT_ON_FAILURE, foffset, &foffset,
                         details_flags) != 0) {
      dump_succeeded = FALSE;
    }
#endif /* __arm64__ */
    ...
    /* Only the kernel */
    if (kern_do_coredump(&outstate, KCF_KERNEL_ONLY, foffset, &foffset,
                         details_flags) != 0) {
      dump_succeeded = FALSE;
    }
  }
  ...
  if (kd_variant == KERN_DUMP_DISK) {
    ...
    if ((ret = first_stage->kos_funcs.kosf_outproc(first_stage, KDP_SEEK, NULL,
                                                   ...
                                                   &foffset)) != KERN_SUCCESS) {
      kern_coredump_log(NULL,
                        "(do_kern_dump seek logfile) outproc(KDP_SEEK, NULL, "
                        "%lu, %p) foffset = 0x%llx returned 0x%x\n",
                        sizeof(foffset), &foffset, foffset, ret);
      dump_succeeded = FALSE;
      goto exit;
...
#if defined(__arm64__)
    initial_log_length = (panic_info->eph_panic_log_offset -
                          sizeof(struct embedded_panic_header)) +
                         panic_info->eph_panic_log_len;
#else
    if (panic_info->mph_panic_log_offset != 0) {
      initial_log_length = (panic_info->mph_panic_log_offset -
    ...
    buf = debug_buf_base;
    if ((ret = first_stage->kos_funcs.kosf_outproc(first_stage, KDP_DATA, NULL,
                                                   initial_log_length, buf)) !=
        KERN_SUCCESS) {
      kern_coredump_log(NULL,
                        "(do_kern_dump paniclog) outproc(KDP_DATA, NULL, %lu, "
                        "%p) returned 0x%x\n",
    ...
    /* Next include any log data from after the stackshot (the beginning of the
     * 'other' log). */
#if defined(__arm64__)
    buf = (char *)(((char *)panic_info) +
                   (uintptr_t)panic_info->eph_other_log_offset);
#else
    /*
     * There may be no paniclog if we're doing a coredump after a call to
     * Debugger() on x86 if debugger_is_panic was configured to FALSE based on
     * the boot-args. In that case just start from where the debug buffer was
     * when we began taking a coredump.
     */
    if (panic_info->mph_other_log_offset != 0) {
      buf = (char *)(((char *)panic_info) +
                     (uintptr_t)panic_info->mph_other_log_offset);
    } else {
    ...
    if ((ret = first_stage->kos_funcs.kosf_outproc(first_stage, KDP_DATA, NULL,
                                                   other_log_length, buf)) !=
        KERN_SUCCESS) {
      kern_coredump_log(NULL,
                        "(do_kern_dump coredump log) outproc(KDP_DATA, NULL, "
                        "%lu, %p) returned 0x%x\n",
...
exit:
  /* close / last packet */
  if (output_opened &&
      (ret = first_stage->kos_funcs.kosf_outproc(
           first_stage, KDP_EOF, NULL, 0, ((void *)0))) != KERN_SUCCESS) {
    kern_coredump_log(
        NULL,
...
#if defined(__arm64__)
  panic_info->eph_panic_flags |=
      (dump_succeeded ? EMBEDDED_PANIC_HEADER_FLAG_COREDUMP_COMPLETE
                      : EMBEDDED_PANIC_HEADER_FLAG_COREDUMP_FAILED);
  paniclog_flush();
#else
  if (panic_info->mph_panic_log_offset != 0) {
    panic_info->mph_panic_flags |=
        (dump_succeeded ? MACOS_PANIC_HEADER_FLAG_COREDUMP_COMPLETE
...
boolean_t dumped_kernel_core(void) { return kern_dump_successful; }
...
static kern_return_t kdp_core_init_output_stages(void) {
  kern_return_t ret = KERN_SUCCESS;
...
#if defined(__arm64__)
  bzero(&memory_backing_aware_buffer_output_stage,
        sizeof(memory_backing_aware_buffer_output_stage));
  ret = memory_backing_aware_buffer_stage_initialize(
      &memory_backing_aware_buffer_output_stage);
  if (KERN_SUCCESS != ret) {
    return ret;
  }
#endif /* defined(__arm64__) */
...
bool kern_dump_should_enforce_encryption(void) {
  ...
  if (enforce_encryption == -1) {
    ...
    if (!kernel_debugging_restricted() &&
        PE_parse_boot_argn("coredump_encryption", &coredump_encryption_flags,
                           sizeof(coredump_encryption_flags))) {
      enforce_encryption = (coredump_encryption_flags &
                            COREDUMP_ENCRYPTION_OVERRIDES_ENFORCEMENT) != 0
                               ? 1
                               : 0;
    } else {
...
static bool kern_dump_is_encryption_available(void) {
  // Default to feature enabled unless boot-arg says otherwise
  uint32_t coredump_encryption_flags =
      COREDUMP_ENCRYPTION_OVERRIDES_AVAILABILITY;
  ...
  if ((coredump_encryption_flags &
       COREDUMP_ENCRYPTION_OVERRIDES_AVAILABILITY) == 0) {
    return false;
  }
...
/*
 * Initialize (or de-initialize) the encryption stage. This is done in a way
 * such that if initializing the encryption stage with a new key fails, then the
 * existing encryption stage is left untouched. Once the new stage is
 * initialized, the old stage is uninitialized.
 *
 * This function is called whenever we have a new public key (whether from
 * someone calling our sysctl, or because we read it out of a corefile), or when
 * encryption becomes available.
 *
 * Parameters:
 *  - public_key:      The public key to use when initializing the encryption
 * stage. Can be NULL to indicate that the encryption stage should be
 * de-initialized.
 *  - public_key_size: The size of the given public key.
 */
static kern_return_t kdp_core_init_encryption_stage(void *public_key,
  ...
  if (public_key && kern_dump_is_encryption_available()) {
    ret = aea_stage_initialize(&new_encryption_stage, public_key,
                               public_key_size);
    if (KERN_SUCCESS != ret) {
      printf("(kdp_core_init_encryption_stage) Failed to initialize the "
             "encryption stage. Error 0x%x\n",
             ret);
      return ret;
    }
  }
  ...
  if (old_encryption_stage.kos_initialized &&
      old_encryption_stage.kos_funcs.kosf_free) {
...
kern_return_t
kdp_core_handle_new_encryption_key(IOCoreFileAccessCallback access_data,
                                   void *access_context,
                                   void *recipient_context) {
  kern_return_t ret = KERN_SUCCESS;
  struct kdp_core_encryption_key_descriptor *key_descriptor =
      (struct kdp_core_encryption_key_descriptor *)recipient_context;
  ...
  do {
    ...
    ret = kdp_core_init_encryption_stage(key_descriptor->kcekd_key,
                                         key_descriptor->kcekd_size);
    if (ret != KERN_SUCCESS) {
      printf("kdp_core_handle_new_encryption_key failed to re-initialize "
             "encryption stage. Error 0x%x\n",
             ret);
      break;
    }
    ...
    kdp_core_header->flags &=
        ~MACH_CORE_FILEHEADER_V2_FLAGS_NEXT_COREFILE_KEY_FORMAT_MASK;
    kdp_core_header->flags &=
        ~MACH_CORE_FILEHEADER_V2_FLAGS_EXISTING_COREFILE_KEY_FORMAT_MASK;
    if (key_descriptor->kcekd_key) {
      kdp_core_header->flags |=
          key_descriptor->kcekd_format &
          MACH_CORE_FILEHEADER_V2_FLAGS_NEXT_COREFILE_KEY_FORMAT_MASK;
      kdp_core_header->flags |=
          MACH_CORE_FILEHEADER_V2_FLAGS_NEXT_KEY_FORMAT_TO_KEY_FORMAT(
              key_descriptor->kcekd_format);
      kdp_core_header->pub_key_offset =
          roundup(kdp_core_header_size, KERN_COREDUMP_BEGIN_FILEBYTES_ALIGN);
    ...
    ret = access_data(access_context, FALSE, 0, sizeof(existing_header),
                      &existing_header);
    if (ret != KERN_SUCCESS) {
      printf("kdp_core_handle_new_encryption_key failed to read the existing "
             "corefile header. Error 0x%x\n",
             ret);
      break;
    }
    ...
    if (existing_header.signature == MACH_CORE_FILEHEADER_V2_SIGNATURE &&
      ...
      ret = access_data(access_context, TRUE, 0, sizeof(existing_header),
                        &existing_header);
      if (ret != KERN_SUCCESS) {
        printf("kdp_core_handle_new_encryption_key failed to update the "
               "existing corefile header. Error 0x%x\n",
               ret);
        break;
      }
    } else {
      ret =
          access_data(access_context, TRUE, 0,
                      sizeof(struct mach_core_fileheader_v2), kdp_core_header);
      if (ret != KERN_SUCCESS) {
        printf("kdp_core_handle_new_encryption_key failed to write the "
               "corefile header. Error 0x%x\n",
               ret);
        break;
      }
    ...
    if (kdp_core_header->pub_key_length) {
      uint64_t offset = used_existing_header ? existing_header.pub_key_offset
                                             : kdp_core_header->pub_key_offset;
      ret =
          access_data(access_context, TRUE, offset + PUBLIC_KEY_RESERVED_LENGTH,
                      kdp_core_header->pub_key_length, kdp_core_public_key);
      if (ret != KERN_SUCCESS) {
        printf("kdp_core_handle_new_encryption_key failed to write the next "
               "public key. Error 0x%x\n",
               ret);
        break;
      }

      if (!used_existing_header) {
        // Everything that happens here is optional. It's not the end of the
        // world if this stuff fails, so we don't return any errors Since we're
        // writing out a completely new header, we make sure to zero-out the
        // region that's reserved for the public key. This allows us consumers
        // of the corefile to know for sure that this corefile is not encrypted
        // (yet). Once we actually write out a corefile, we'll overwrite this
        // region with the key that we ended up using at the time. If we fail to
        // zero-out this region, consumers would read garbage data and properly
        // fail to interpret it as a public key, which is why it is OK for us to
        // fail here (it's hard to interpret garbage data as a valid key, and
        // even then, they wouldn't find a matching private key anyway)
        void *empty_key = NULL;
        ...
        temp_ret = access_data(access_context, TRUE, offset,
                               PUBLIC_KEY_RESERVED_LENGTH, empty_key);
        kfree_data(empty_key, PUBLIC_KEY_RESERVED_LENGTH);

        if (temp_ret != KERN_SUCCESS) {
          printf("kdp_core_handle_new_encryption_key failed to zero-out the "
                 "public key region. Error 0x%x\n",
                 temp_ret);
          break;
        }
      }
...
kern_return_t kdp_core_handle_encryption_available(void) {
  ...
  ret = kdp_core_init_encryption_stage(kdp_core_public_key,
                                       kdp_core_header->pub_key_length);
...
kdp_core_polled_io_polled_file_available(IOCoreFileAccessCallback access_data,
                                         void *access_context,
                                         __unused void *recipient_context) {
  kern_return_t ret = KERN_SUCCESS;
  ...
  // The kernel corefile is now available. Let's try to retrieve the public key
  // from its header (if available and supported).

  // First let's read the corefile header itself
  struct mach_core_fileheader_v2 temp_header = {};
  ret =
      access_data(access_context, FALSE, 0, sizeof(temp_header), &temp_header);
  if (KERN_SUCCESS != ret) {
    printf("kdp_core_polled_io_polled_file_available failed to read corefile "
           "header. Error 0x%x\n",
           ret);
    return ret;
  }

  // Check if the corefile header is initialized, and whether it's initialized
  // to values that we support (for backwards and forwards) compatibility, and
  // check whether the header indicates that the corefile has has a public key
  // stashed inside of it.
  if (temp_header.signature == MACH_CORE_FILEHEADER_V2_SIGNATURE &&
      temp_header.version == 2 && temp_header.pub_key_offset != 0 &&
      temp_header.pub_key_length != 0
      /* Future-proofing: make sure it's the key format that we support */
      && (temp_header.flags &
          MACH_CORE_FILEHEADER_V2_FLAGS_NEXT_COREFILE_KEY_FORMAT_MASK) ==
             MACH_CORE_FILEHEADER_V2_FLAG_NEXT_COREFILE_KEY_FORMAT_NIST_P256
      /* Add some extra sanity checks. These are not necessary */
      && temp_header.pub_key_length <= 4096 &&
      temp_header.pub_key_offset < 65535) {
    // The corefile header is properly initialized, is supported, and contains a
    // public key. Let's adopt that public key for our encryption needs
    void *public_key = NULL;

    public_key =
        kalloc_data(temp_header.pub_key_length, Z_ZERO | Z_WAITOK | Z_NOFAIL);

    // Read the public key from the corefile. Note that the key we're trying to
    // adopt is the "next" key, which is PUBLIC_KEY_RESERVED_LENGTH bytes after
    // the public key.
    ret = access_data(access_context, FALSE,
                      temp_header.pub_key_offset + PUBLIC_KEY_RESERVED_LENGTH,
                      temp_header.pub_key_length, public_key);
    if (KERN_SUCCESS != ret) {
      printf("kdp_core_polled_io_polled_file_available failed to read the "
             "public key. Error 0x%x\n",
             ret);
    ...
    ret =
        kdp_core_init_encryption_stage(public_key, temp_header.pub_key_length);
    if (KERN_SUCCESS == ret) {
      kdp_core_header->flags |=
          temp_header.flags &
          MACH_CORE_FILEHEADER_V2_FLAGS_NEXT_COREFILE_KEY_FORMAT_MASK;
...
kern_return_t kdp_core_polled_io_polled_file_unavailable(void) {
  ...
  if (disk_output_stage.kos_initialized &&
      disk_output_stage.kos_funcs.kosf_free) {
...
void kdp_core_init(void) {
  ...
  kmem_alloc(kernel_map, (vm_offset_t *)&kdp_core_header, kdp_core_header_size,
             KMA_NOFAIL | KMA_ZERO | KMA_PERMANENT | KMA_KOBJECT |
                 KMA_DATA_SHARED,
             VM_KERN_MEMORY_DIAG);
  ...
  kdp_core_initialization_lock_group =
      lck_grp_alloc_init("KDPCoreStageInit", LCK_GRP_ATTR_NULL);
  kdp_core_disk_stage_lock =
      lck_mtx_alloc_init(kdp_core_initialization_lock_group, LCK_ATTR_NULL);

#ifdef CONFIG_KDP_COREDUMP_ENCRYPTION
  kdp_core_encryption_stage_lock =
      lck_mtx_alloc_init(kdp_core_initialization_lock_group, LCK_ATTR_NULL);

  (void)kern_dump_should_enforce_encryption();
#endif // CONFIG_KDP_COREDUMP_ENCRYPTION

  kdp_core_lz4_stage_lock =
      lck_mtx_alloc_init(kdp_core_initialization_lock_group, LCK_ATTR_NULL);

  core_config.kcc_coredump_init = kern_dump_init;
  core_config.kcc_coredump_get_summary = kern_dump_save_summary;
  core_config.kcc_coredump_save_segment_descriptions =
  ...
  core_config.kcc_coredump_save_note_descriptions =
      kern_dump_save_note_descriptions;
...
static kern_return_t
kern_dump_save_note_summary(void *refcon __unused,
                            core_save_note_summary_cb callback, void *context) {
  int count = 1;
  ...
  struct debug_header const *debug_header =
      SPTMArgs != NULL ? SPTMArgs->debug_header : NULL;

  if (debug_header != NULL && debug_header->magic == DEBUG_HEADER_MAGIC_VAL &&
...
static kern_return_t
kern_dump_save_note_descriptions(void *refcon __unused,
                                 core_save_note_descriptions_cb callback,
                                 void *context) {
  int max_ret = KERN_SUCCESS;
  int ret;

  max_ret = ret =
      callback(ADDRABLE_BITS_DATA_OWNER, sizeof(addrable_bits_note_t), context);
  ...
  for (int i = 0; i < (debug_header != NULL ? debug_header->count : 0); i++) {
    ret = callback(LOAD_BINARY_SPEC_DATA_OWNER, sizeof(load_binary_spec_note_t),
                   context);
...
static kern_return_t kern_dump_save_note_data(void *refcon __unused,
                                              core_save_note_data_cb callback,
                                              void *context) {
  int max_ret = KERN_SUCCESS;
  int ret;

  addrable_bits_note_t note = {.version = ADDRABLE_BITS_VER,
  ...
  for (int i = 0; i < (debug_header != NULL ? debug_header->count : 0); i++) {
    ...
    default:
      ...
      kern_coredump_log(context,
                        "%s(): encountered unknown debug header entry %d, "
    ...
    if (ret != KERN_SUCCESS) {
      kern_coredump_log(context,
                        "%s(): failed to write load binary spec structure "
...
void kdp_core_exclude_region(__unused vm_offset_t addr,
                             __unused vm_size_t size) {}

void kdp_core_unexclude_region(__unused vm_offset_t addr,
                               __unused vm_size_t size) {}
