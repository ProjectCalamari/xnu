...
#define CORE_REMOTE_PORT                                                       \
  1069 /* hardwired, we can't really query the services file */
...
void kdp_set_dump_info(const uint32_t flags, const char *file,
                       const char *destip, const char *routerip,
                       const uint32_t port);
...
#if defined(__arm64__)
  KERN_DUMP_HW_SHMEM_DBG, /* coordinated hardware shared memory debugger core
                             dump */
#endif
...
struct corehdr *create_panic_header(unsigned int request, const char *corename,
                                    unsigned length, unsigned block);
...
void kern_collectth_state(thread_t thread, void *buffer, uint64_t size,
                          void **iter);
void kern_collect_userth_state_size(task_t task, uint64_t *tstate_count,
                                    uint64_t *tstate_size);
void kern_collect_userth_state(task_t task, thread_t thread, void *buffer,
                               uint64_t size);
...
#if PRIVATE
kern_return_t kdp_core_output(void *kdp_core_out_vars, uint64_t length,
                              void *data);

/*
 * Resets the coredump output vars such that they're ready to start writing out
 * coredump data. Note that the 'encrypt_core' parameter instructs the output
 * vars to encrypt the coredump data (if possible) The
 * 'out_should_skip_coredump' parameter will be set to true if the calling code
 * should skip this coredump (for reasons).
 */
kern_return_t kdp_reset_output_vars(void *kdp_core_out_vars,
                                    uint64_t totalbytes, bool encrypt_core,
                                    bool *out_should_skip_coredump,
                                    const char *corename,
                                    kern_coredump_type_t coretype);

kern_return_t kern_dump_record_file(void *kdp_core_out_vars,
                                    const char *filename, uint64_t file_offset,
                                    uint64_t *out_file_length,
                                    uint64_t details_flags);

kern_return_t kern_dump_seek_to_next_file(void *kdp_core_out_varss,
                                          uint64_t next_file_offset);

extern boolean_t bootloader_valid_page(ppnum_t ppn);

/*
 * Called whenever the encryption functionality becomes available (e.g. when an
 * encryption Kext is loaded and registers its interface with libkern). It is
 * expected that once encryption support is available, it will stay available
 * for the remainder of the kernel lifetime.
 */
kern_return_t kdp_core_handle_encryption_available(void);

/*
 * Called whenever the LZ4 functionality becomes available (e.g. when the
 * Compression kext is loaded and registers its interface with libkern). It is
 * expected that once LZ4 support is available, it will stay available for the
