...
kdp_error_t kdp_set_breakpoint_internal(mach_vm_address_t address);

kdp_error_t kdp_remove_breakpoint_internal(mach_vm_address_t address);

boolean_t kdp_packet(unsigned char *pkt, int *len, unsigned short *reply_port) {
  ...
  if (req >= KDP_INVALID_REQUEST) {
    printf("kdp_packet bad request %x len %d seq %x key %x\n", rd->hdr.request,
           rd->hdr.len, rd->hdr.seq, rd->hdr.key);
...
static boolean_t kdp_unknown(kdp_pkt_t *pkt, __unused int *len,
...
static boolean_t kdp_connect(kdp_pkt_t *pkt, int *len,
  ...
  if (kdp.is_conn) {
    if ((seq == kdp.conn_seq) && /* duplicate request */
        (rport == kdp.reply_port) && (eport == kdp.exception_port) &&
        (key == kdp.session_key)) {
...
static boolean_t kdp_disconnect(kdp_pkt_t *pkt, int *len,
                                unsigned short *reply_port) {
...
static boolean_t kdp_reattach(kdp_pkt_t *pkt, int *len,
                              unsigned short *reply_port) {
...
static boolean_t kdp_hostinfo(kdp_pkt_t *pkt, int *len,
                              unsigned short *reply_port) {
...
static boolean_t kdp_kernelversion(kdp_pkt_t *pkt, int *len,
                                   unsigned short *reply_port) {
...
static boolean_t kdp_suspend(kdp_pkt_t *pkt, int *len,
                             unsigned short *reply_port) {
...
static boolean_t kdp_resumecpus(kdp_pkt_t *pkt, int *len,
                                unsigned short *reply_port) {
...
static boolean_t kdp_writemem(kdp_pkt_t *pkt, int *len,
                              unsigned short *reply_port) {
  ...
  if (rq->nbytes > MAX_KDP_DATA_SIZE) {
    ...
    cnt = kdp_machine_vm_write((caddr_t)rq->data,
                               (mach_vm_address_t)rq->address, rq->nbytes);
...
static boolean_t kdp_writemem64(kdp_pkt_t *pkt, int *len,
                                unsigned short *reply_port) {
  ...
  if (rq->nbytes > MAX_KDP_DATA_SIZE) {
    ...
    cnt =
        kdp_machine_vm_write((caddr_t)rq->data, (mach_vm_address_t)rq->address,
                             (mach_vm_size_t)rq->nbytes);
    rp->error = KDPERR_ACCESS(rq->nbytes, cnt);
    dprintf(("  cnt %lld error %d\n", cnt, rp->error));
  }
...
static boolean_t kdp_writephysmem64(kdp_pkt_t *pkt, int *len,
                                    unsigned short *reply_port) {
...
static boolean_t kdp_readmem(kdp_pkt_t *pkt, int *len,
                             unsigned short *reply_port) {
  ...
  if (size > MAX_KDP_DATA_SIZE) {
    ...
    cnt = kdp_machine_vm_read((mach_vm_address_t)rq->address, (caddr_t)rp->data,
                              rq->nbytes);
    rp->error = KDPERR_ACCESS(size, cnt);
    dprintf(("  cnt %lld error %d\n", cnt, rp->error));
...
static boolean_t kdp_readmem64(kdp_pkt_t *pkt, int *len,
                               unsigned short *reply_port) {
...
static boolean_t kdp_readphysmem64(kdp_pkt_t *pkt, int *len,
                                   unsigned short *reply_port) {
...
static boolean_t kdp_maxbytes(kdp_pkt_t *pkt, int *len,
                              unsigned short *reply_port) {
...
static boolean_t kdp_version(kdp_pkt_t *pkt, int *len,
                             unsigned short *reply_port) {
...
static boolean_t kdp_regions(kdp_pkt_t *pkt, int *len,
                             unsigned short *reply_port) {
...
static boolean_t kdp_writeregs(kdp_pkt_t *pkt, int *len,
                               unsigned short *reply_port) {
  ...
  size = rq->hdr.len - (unsigned)sizeof(kdp_hdr_t) -
...
static boolean_t kdp_readregs(kdp_pkt_t *pkt, int *len,
                              unsigned short *reply_port) {
...
boolean_t kdp_breakpoint_set(kdp_pkt_t *pkt, int *len,
                             unsigned short *reply_port) {
...
boolean_t kdp_breakpoint64_set(kdp_pkt_t *pkt, int *len,
                               unsigned short *reply_port) {
...
boolean_t kdp_breakpoint_remove(kdp_pkt_t *pkt, int *len,
                                unsigned short *reply_port) {
  kdp_breakpoint_req_t *rq = &pkt->breakpoint_req;
  kdp_breakpoint_reply_t *rp = &pkt->breakpoint_reply;
  size_t plen = *len;
...
boolean_t kdp_breakpoint64_remove(kdp_pkt_t *pkt, int *len,
                                  unsigned short *reply_port) {
  kdp_breakpoint64_req_t *rq = &pkt->breakpoint64_req;
  kdp_breakpoint64_reply_t *rp = &pkt->breakpoint64_reply;
  size_t plen = *len;
...
kdp_error_t kdp_set_breakpoint_internal(mach_vm_address_t address) {
  ...
  cnt = kdp_machine_vm_read(address, (caddr_t)&oldinstr,
                            (mach_vm_size_t)breakinstrsize);
...
kdp_error_t kdp_remove_breakpoint_internal(mach_vm_address_t address) {
  ...
  for (i = 0; (i < MAX_BREAKPOINTS) && (breakpoint_list[i].address != address);
       i++) {
    ;
  }
  ...
  cnt = kdp_machine_vm_write((caddr_t)&breakpoint_list[i].oldbytes, address,
                             breakpoint_list[i].bytesused);

  return KDPERR_NO_ERROR;
}

boolean_t kdp_remove_all_breakpoints(void) {
  int i;
  ...
  if (breakpoints_initialized) {
    for (i = 0; i < MAX_BREAKPOINTS; i++) {
      if (breakpoint_list[i].address) {
        kdp_machine_vm_write((caddr_t) & (breakpoint_list[i].oldbytes),
                             (mach_vm_address_t)breakpoint_list[i].address,
                             (mach_vm_size_t)breakpoint_list[i].bytesused);
    ...
    if (breakpoint_found) {
      printf("kdp_remove_all_breakpoints: found extant breakpoints, removing "
             "them.\n");
    }
  }
...
boolean_t kdp_reboot(__unused kdp_pkt_t *pkt, __unused int *len,
                     __unused unsigned short *reply_port) {
  dprintf(("kdp_reboot\n"));
...
static boolean_t kdp_readioport(kdp_pkt_t *pkt, int *len,
                                unsigned short *reply_port) {
...
static boolean_t kdp_writeioport(kdp_pkt_t *pkt, int *len,
                                 unsigned short *reply_port) {
...
static boolean_t kdp_readmsr64(kdp_pkt_t *pkt, int *len,
                               unsigned short *reply_port) {
...
static boolean_t kdp_writemsr64(kdp_pkt_t *pkt, int *len,
                                unsigned short *reply_port) {
...
static boolean_t kdp_dumpinfo(kdp_pkt_t *pkt, int *len,
                              unsigned short *reply_port) {
  ...
  if ((rq->type & KDP_DUMPINFO_MASK) != KDP_DUMPINFO_GETINFO) {
    kdp_set_dump_info(rq->type, rq->name, rq->destip, rq->routerip, rq->port);
  }
