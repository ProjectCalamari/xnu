...
typedef struct main_bin_spec_note {
  ...
  uint32_t type;    // 0 == unspecified, 1 == kernel, 2 == user process, 3 ==
                    // standalone (ie FW)
  uint64_t address; // UINT64_MAX if address not specified
  uuid_t uuid;      // all zeros if uuid not specified
  uint32_t log2_pagesize; // process page size in log base 2, e.g. 4k pages
                          // are 12. 0 for unspecified
...
typedef struct load_binary_spec_note {
  ...
  uint64_t address; // virtual address where the macho is loaded, UINT64_MAX if
                    // unavail
