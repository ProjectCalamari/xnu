...
kern_return_t disk_stage_write(struct kdp_output_stage *stage, uint64_t offset,
  ...
  struct disk_stage_data *stage_data =
      (struct disk_stage_data *)stage->kos_data;
  bool already_seeked_this_chunk = false;

  if ((offset < stage_data->furthest_written_offset) ||
      (offset != stage_data->current_offset)) {
    // We need to seek to the proper offset and prefill the IOPolledInterface
    // internal buffers
    ...
    if (kIOReturnSuccess != err) {
      kern_coredump_log(
          NULL, "(disk_stage_write) IOPolledFileSeek(0x%llx) returned 0x%x\n",
          aligned_offset, err);
      return err;
    }
  ...
  while (KERN_SUCCESS == err && length != 0) {
    ...
    uint64_t chunk = gIOPolledCoreFileVars->bufferLimit -
                     gIOPolledCoreFileVars->bufferOffset;
    if (chunk > length) {
      chunk = length;

      // If we're about to write to a region that we've written to before,
      // we'll need to prefill the IOPolledInterface internal buffers with the
        ...
        if (!already_seeked_this_chunk) {
          ...
          if (kIOReturnSuccess != err) {
            kern_coredump_log(NULL,
                              "(disk_stage_write) disk_stage_read (during "
                              "final chunk seek) returned 0x%x\n",
                              err);
            break;
          }
    ...
    // Now write the chunk
    err = IOPolledFileWrite(gIOPolledCoreFileVars, data, (IOByteCount)chunk,
                            NULL);
    if (kIOReturnSuccess != err) {
      kern_coredump_log(
          NULL,
          "(disk_stage_write) IOPolledFileWrite(gIOPolledCoreFileVars, %p, "
          "0x%llx, NULL) returned 0x%x\n",
          data, chunk, err);
      break;
    }
    ...
    if (read_modify_write) {
      ...
      uint32_t remainder = gIOPolledCoreFileVars->bufferLimit -
                           gIOPolledCoreFileVars->bufferOffset;
      ...
      if (kIOReturnSuccess != err) {
        kern_coredump_log(NULL,
                          "(disk_stage_write) IOPolledFileWrite (during final "
                          "flush) returned 0x%x\n",
                          err);
        break;
      }
...
kern_return_t disk_stage_read(struct kdp_output_stage *stage, uint64_t offset,
  ...
  // First seek to the aligned position (this will update the position variable
  // and whatnot)
  err = IOPolledFileSeek(gIOPolledCoreFileVars, aligned_offset);
  if (kIOReturnSuccess != err) {
    kern_coredump_log(
        NULL, "(disk_stage_read) IOPolledFileSeek(0x%llx) returned 0x%x\n",
        aligned_offset, err);
    return err;
  }
  ...
  gIOPolledCoreFileVars->readEnd =
      roundup(gIOPolledCoreFileVars->fileSize, stage_data->alignment);
  ...
  // This read will (even if offset_misalignment is 0) wait for the previous
  // read to actually complete
  err = IOPolledFileRead(gIOPolledCoreFileVars, NULL,
                         (IOByteCount)offset_misalignment, NULL);
  if (kIOReturnSuccess != err) {
    kern_coredump_log(
        NULL, "(disk_stage_read) IOPolledFileRead(%llu) returned 0x%x\n",
  ...
  err = IOPolledFileRead(gIOPolledCoreFileVars, (uint8_t *)data,
                         (IOByteCount)length, NULL);
  if (kIOReturnSuccess != err) {
    kern_coredump_log(
        NULL, "(disk_stage_read) IOPolledFileRead(%llu) returned 0x%x\n",
        length, err);
...
static kern_return_t disk_stage_reset(struct kdp_output_stage *stage,
                                      __unused const char *corename,
                                      __unused kern_coredump_type_t coretype) {
...
static kern_return_t disk_stage_outproc(struct kdp_output_stage *stage,
  ...
  case KDP_WRQ:
    ...
    if (kIOReturnSuccess != err) {
      kern_coredump_log(
          NULL, "IOPolledFileSeek(gIOPolledCoreFileVars, 0) returned 0x%x\n",
          err);
      break;
    }
    err = IOPolledFilePollersOpen(gIOPolledCoreFileVars,
                                  kIOPolledBeforeSleepState, false);
    if (kIOReturnSuccess != err) {
  ...
  case KDP_DATA:
    err = IOPolledFileWrite(gIOPolledCoreFileVars, data, (IOByteCount)length,
                            NULL);
    if (kIOReturnSuccess != err) {
      kern_coredump_log(NULL,
                        "IOPolledFileWrite(gIOPolledCoreFileVars, %p, 0x%llx, "
                        "NULL) returned 0x%x\n",
                        data, length, err);
      break;
    }
  ...
  case KDP_EOF:
    ...
    err = IOPolledFilePollersClose(gIOPolledCoreFileVars,
...
static void disk_stage_free(struct kdp_output_stage *stage) {
