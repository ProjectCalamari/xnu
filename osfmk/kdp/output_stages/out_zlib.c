...
static void *zlib_alloc(void *opaque, u_int items, u_int size) {
  ...
  data->allocation_offset +=
      ROUND_32B(items * size); // 32b align for vector crc
  assert(data->allocation_offset <= stage->kos_data_size);

  return result;
}

static void zlib_free(void *__unused ref, void *__unused ptr) {}

/*
 * flushes any data to the output proc immediately
 */
static int zlib_zoutput(z_streamp strm, Bytef *buf, unsigned len) {
  ...
  if (stage->kos_outstate->kcos_error == kIOReturnSuccess) {
    ret = next_stage->kos_funcs.kosf_outproc(next_stage, KDP_DATA, NULL, len,
                                             buf);
...
static int zlib_zinput(z_streamp strm, Bytef *buf, unsigned size) {
  unsigned len;
...
static kern_return_t zlib_stream_output_chunk(struct kdp_output_stage *stage,
...
static kern_return_t zlib_stage_reset(struct kdp_output_stage *stage,
...
static kern_return_t zlib_stage_outproc(struct kdp_output_stage *stage,
  ...
  case KDP_DATA:
    if (!stage->kos_bypass) {
      do {
        ...
        if (KERN_SUCCESS != err) {
          kern_coredump_log(
              NULL, "(%s) zlib_stream_output_chunk failed with error 0x%x\n",
              __func__, err);
        }
...
static void zlib_stage_free(struct kdp_output_stage *stage) {
...
kern_return_t zlib_stage_initialize(struct kdp_output_stage *stage) {
  ...
  stage->kos_data_size = round_page(sizeof(struct zlib_stage_data) + NETBUF +
  ...
  data->allocation_offset =
      ROUND_32B(sizeof(struct zlib_stage_data)); // Start allocating from the
                                                 // end of the data struct
  data->zs.zalloc = zlib_alloc;
  data->zs.zfree = zlib_free;
  data->zs.opaque = (void *)stage;

  if (deflateInit2(&(data->zs), LEVEL, Z_DEFLATED, wbits + 16 /*gzip mode*/,
