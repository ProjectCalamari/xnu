...
static kern_return_t buffer_stage_flush(struct kdp_output_stage *stage) {
  kern_return_t err = KERN_SUCCESS;
  struct buffer_stage_data *data = (struct buffer_stage_data *)stage->kos_data;
  struct kdp_output_stage *next_stage = STAILQ_NEXT(stage, kos_next);

  err = next_stage->kos_funcs.kosf_outproc(next_stage, KDP_DATA, NULL,
                                           data->current_size, data->buffer);
...
static kern_return_t buffer_stage_outproc(struct kdp_output_stage *stage,
  ...
  if ((data->current_size &&
       (request == KDP_SEEK || request == KDP_FLUSH || request == KDP_EOF)) ||
  ...
  if (should_flush) {
    ...
    if (KERN_SUCCESS != err) {
      kern_coredump_log(
          NULL, "buffer_stage_outproc (during flush) returned 0x%x\n", err);
      return err;
    }
  }
  ...
  if (request == KDP_WRQ || request == KDP_SEEK || request == KDP_EOF) {
    ...
    if (KERN_SUCCESS != err) {
      kern_coredump_log(
          NULL, "buffer_stage_outproc (during forwarding) returned 0x%x\n",
          err);
      return err;
    }
    ...
    while (length != 0) {
      ...
      if (length < bytes_to_copy) {
        /* Safe to cast to size_t here since we just checked that 'length' is
         * less than a size_t value. */
        bytes_to_copy = (size_t)length;
      }

      bcopy(panic_data, (void *)((uintptr_t)data->buffer + data->current_size),
            bytes_to_copy);
...
static void buffer_stage_free(struct kdp_output_stage *stage) {
