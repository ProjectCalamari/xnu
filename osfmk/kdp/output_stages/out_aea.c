...
static ssize_t aea_write_callback(void *context, const void *buffer,
  ...
  uint64_t absolute_corefile_offset =
      stage_data->starting_corefile_offset + offset;

  err = disk_stage_write(next_stage, absolute_corefile_offset, length, buffer);

  if (KERN_SUCCESS != err) {
    kern_coredump_log(
        NULL, "(aea_write_callback) next stage outproc returned 0x%x\n", err);
    return -1;
  }
...
static ssize_t aea_read_callback(void *context, void *buffer, size_t length,
                                 off_t offset) {
  kern_return_t err = KERN_SUCCESS;
  struct kdp_output_stage *stage = (struct kdp_output_stage *)context;
  struct kdp_output_stage *next_stage = STAILQ_NEXT(stage, kos_next);
  ...
  if (KERN_SUCCESS != err) {
    kern_coredump_log(
        NULL, "(aea_read_callback) next stage read proc returned 0x%x\n", err);
    return -1;
  }
...
static kern_return_t aea_stage_reset(struct kdp_output_stage *stage,
  ...
  if (stage_data->encryption_open) {
    aea_ret = apple_encrypted_archive->aea_close(stage_data->state,
                                                 stage_data->state_size);
    if (aea_ret != 0) {
...
static kern_return_t aea_stage_outproc(struct kdp_output_stage *stage,
                                       unsigned int request,
                                       __unused char *corename, uint64_t length,
                                       void *panic_data) {
  kern_return_t err = KERN_SUCCESS;
  int aea_ret = 0;
  struct aea_stage_data *stage_data = (struct aea_stage_data *)stage->kos_data;
  ...
  case KDP_SEEK:
    ...
    if (KERN_SUCCESS == err) {
      err = next_stage->kos_funcs.kosf_outproc(next_stage, request, corename,
                                               length, panic_data);
    }
    if (KERN_SUCCESS == err) {
      stage_data->starting_corefile_offset = *((uint64_t *)panic_data);
      stage_data->current_corefile_offset =
          stage_data->starting_corefile_offset;
    }
    break;
  case KDP_DATA:
    if (!stage->kos_bypass) {
      if (!length && !panic_data) {
        ...
        if (stage_data->encryption_open) {
          ...
          if (aea_ret != 0) {
            kern_coredump_log(
                NULL, "(aea_stage_outproc) aea_close() returned %d\n", aea_ret);
            err = KERN_FAILURE;
          } else {
            stage_data->encryption_open = false;
          }
        ...
        if (stage_data->encryption_open == false) {
          aea_ret = apple_encrypted_archive->aea_open(
              stage_data->state, stage_data->state_size, (void *)stage,
              aea_write_callback, aea_read_callback);
          if (aea_ret != 0) {
            kern_coredump_log(
                NULL, "(aea_stage_outproc) aea_open() returned %d\n", aea_ret);
            err = KERN_FAILURE;
          } else {
        ...
        if (KERN_SUCCESS == err) {
          do {
            ...
            write_result = apple_encrypted_archive->aea_write(
                stage_data->state, stage_data->state_size, panic_data, chunk);
            if (write_result != chunk) {
              kern_coredump_log(
                  NULL, "(aea_stage_outproc) aea_write() returned %zd\n",
                  write_result);
              err = KERN_FAILURE;
            }
...
static void aea_stage_free(struct kdp_output_stage *stage) {
...
kern_return_t aea_stage_initialize(struct kdp_output_stage *stage,
  ...
  aea_ret = apple_encrypted_archive->aea_initialize_state(
      data->state, data->state_size, (const uint8_t *)recipient_public_key,
      recipient_public_key_size);
  if (aea_ret != 0) {
    printf("WARNING: Coredump encryption failed to initialize. "
           "aea_initialize_state() returned %d\n",
           aea_ret);
...
static void aea_availability_callback(void) {
  kern_return_t ret = kdp_core_handle_encryption_available();
  if (KERN_SUCCESS != ret) {
    printf("(aea_availability_callback) Failed to handle availability of "
           "encryption. Error 0x%x\n",
           ret);
  }
}

void aea_stage_monitor_availability(void) {
  apple_encrypted_archive_interface_set_registration_callback(
      aea_availability_callback);
}

bool aea_stage_is_available(void) { return apple_encrypted_archive != NULL; }
