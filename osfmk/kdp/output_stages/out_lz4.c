...
// One page seems to be a sweet-spot for both buffers; we tend to dump a page or
// less at a time so smaller sizes start incurring overhead for the loop, and
// larger sizes waste more memory without a noticable increase in throughput.
//
// Having a buffer full of zeroes wastes a bit of memory, but we need some way
// to pass zeroes to Compression when we are asked to pad the stream.
...
static kern_return_t lz4_stage_reset(struct kdp_output_stage *stage,
  ...
  status = compression_ki_ptr->compression_stream_reinit(
      &data->stream, COMPRESSION_STREAM_ENCODE, data->algorithm);
...
static kern_return_t lz4_stage_stream(struct lz4_stage_data *data,
                                      struct kdp_output_stage *next_stage,
                                      char *corename, const uint8_t *src_buf,
                                      size_t src_buf_size, uint64_t *written) {
  bool finalize;
  compression_status_t status;
  kern_return_t ret;
  size_t produced;
  ...
  // Every time we process more bytes, the stream source and destination
  // pointers and sizes are adjusted.
  do {
    status = compression_ki_ptr->compression_stream_process(
        &data->stream, finalize ? COMPRESSION_STREAM_FINALIZE : 0);
    ...
    // The difference between the original destination buffer size and the size
    // of the remaining space tells us how many bytes were produced.
    produced = DST_BUF_SIZE - data->stream.dst_size;
    // Pass along those bytes to the next stage so that we empty the destination
    // buffer.
    ...
    *written += produced;
    data->stream.dst_ptr = data->dst_buf;
    data->stream.dst_size = DST_BUF_SIZE;
    // Continue processing while the source buffer is non-empty, or we are
    // finalizing and there are still bytes being produced in the destination
    // buffer.
  } while (data->stream.src_size ||
           (finalize && COMPRESSION_STATUS_END != status));

  if (finalize) {
    ret = next_stage->kos_funcs.kosf_outproc(next_stage, KDP_DATA, corename, 0,
                                             NULL);
    if (KERN_SUCCESS != ret) {
      kern_coredump_log(NULL, "(%s) next stage output failed with error 0x%x\n",
                        __func__, ret);
...
static kern_return_t lz4_stage_outproc(struct kdp_output_stage *stage,
  ...
  if (stage->kos_bypass || KDP_DATA != request) {
    ret = next_stage->kos_funcs.kosf_outproc(next_stage, request, corename,
                                             length, panic_data);
    if (KERN_SUCCESS != ret) {
      kern_coredump_log(NULL, "(%s) next stage output failed with error 0x%x\n",
                        __func__, ret);
  ...
  if (panic_data) {
    ...
    ret = lz4_stage_stream(data, next_stage, corename, panic_data,
                           (size_t)length, &stage->kos_bytes_written);
    if (KERN_SUCCESS != ret) {
  ...
  // Finalize the stream.
  ret = lz4_stage_stream(data, next_stage, corename, NULL, 0,
...
static void lz4_stage_free(struct kdp_output_stage *stage) {
  struct lz4_stage_data *data;
...
kern_return_t lz4_stage_initialize(struct kdp_output_stage *stage) {
  ...
  status = compression_ki_ptr->compression_stream_init(
      &data->stream, COMPRESSION_STREAM_ENCODE, data->algorithm);
  if (COMPRESSION_STATUS_ERROR == status) {
    return KERN_FAILURE;
  }
...
static void lz4_stage_registration_callback(void) {
...
void lz4_stage_monitor_availability(void) {
  compression_interface_set_registration_callback(
