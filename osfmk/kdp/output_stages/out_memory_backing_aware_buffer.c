...
static bool is_normal_memory(uint64_t phys) {
  unsigned int attr = pmap_cache_attributes((ppnum_t)(phys >> PAGE_SHIFT));
  /*
   * Note that VM_WIMG_DEFAULT doesn't cover all normal memory, however it is a
   * good enough heuristic for our purposes. It also means we won't have to
   * update this check when new memory types are inevitably added.
   */
  return (attr & VM_WIMG_MASK) == VM_WIMG_DEFAULT;
}

static kern_return_t memory_backing_aware_buffer_stage_reset(
...
__static_testable kern_return_t memory_backing_aware_buffer_stage_outproc(
  ...
  while (length) {
    ...
    size_t bytes_in_page =
        MIN(length, PAGE_SIZE - ((vm_offset_t)panic_data % PAGE_SIZE));
    if (is_normal_memory(phys)) {
      err = next_stage->kos_funcs.kosf_outproc(next_stage, KDP_DATA, corename,
                                               bytes_in_page, panic_data);
      if (KERN_SUCCESS != err) {
       ...
       *
       * If the data pointer and length are 4-bytes-aligned, we can copy 4 bytes
       * at a time (which is the safest option for reading from SRAMs).
       * Otherwise, try copying byte-wise.
       */
      bool is_buffer_aligned =
          (((uintptr_t)panic_data & (sizeof(int32_t) - 1)) == 0);
