...
progress_notify_stage_reset(struct kdp_output_stage *stage,
  ...
  struct progress_notify_stage_data *data =
      (struct progress_notify_stage_data *)stage->kos_data;
...
static kern_return_t
progress_notify_stage_outproc(struct kdp_output_stage *stage,
...
static void progress_notify_stage_free(struct kdp_output_stage *stage) {
...
kern_return_t progress_notify_stage_initialize(struct kdp_output_stage *stage) {
  ...
  if (KERN_SUCCESS != ret) {
    printf("progress_notify_stage_initialize failed to allocate memory. Error "
           "0x%x\n",
           ret);
    return ret;
  }
  ...
  data->last_notify_timestamp = 0;
  nanoseconds_to_absolutetime(NOTIFY_INTERVAL_NSECS,
                              &data->notify_interval_matus);
