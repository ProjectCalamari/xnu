...
TUNABLE(uint64_t, shmem_timeout_sec, "shmem_timeout_sec",
...
__enum_closed_decl(
    xhsdci_status_t, uint32_t,
    {
        ...
        XHSDCI_COREDUMP_BEGIN = 3, /* indicates hardware debugger is ready to
                                      begin consuming coredump info */
        XHSDCI_COREDUMP_BUF_READY =
            4, /* indicates the kernel has populated the buffer */
        XHSDCI_COREDUMP_BUF_EMPTY = 5, /* indicates hardware debugger is done
                                          consuming the current data */
        XHSDCI_COREDUMP_STATUS_DONE =
            6, /* indicates last compressed data is in buffer */
        XHSDCI_COREDUMP_ERROR = 7, /* indicates an error was encountered */
        XHSDCI_COREDUMP_REMOTE_DONE =
            8, /* indicates that hardware debugger is done */
        XHSDCI_COREDUMP_INFO =
            9, /* anounces new file available for consumption */
...
static STAILQ_HEAD(, kdp_hw_shmem_dbg_buf_elm)
    free_hw_shmem_dbg_bufs = STAILQ_HEAD_INITIALIZER(free_hw_shmem_dbg_bufs);
 ...
 * and timeouts.
 */
static kern_return_t shmem_wait_for_state(struct shmem_stage_data *data,
                                          xhsdci_status_t status) {
  data->contact_deadline =
      mach_absolute_time() + data->contact_deadline_interval;
  ...
  while (hwsd_info->xhsdci_status != status) {
    ...
    if (hwsd_info->xhsdci_status == XHSDCI_COREDUMP_ERROR) {
      kern_coredump_log(
          NULL, "%s: Detected remote side error (state %d, waiting %d)\n",
    ...
    if (mach_absolute_time() > data->contact_deadline) {
      kern_coredump_log(
          NULL, "%s: Timed out waiting for the reply (state %d, waiting %d)\n",
          __func__, hwsd_info->xhsdci_status, status);
  ...
  if (hwsd_info->xhsdci_seq_no != (data->seq_no + 1)) {
    kern_coredump_log(NULL,
                      "%s: Detected stale/invalid seq num (state %d, waiting "
 ...
 */
static kern_return_t shmem_set_status(struct shmem_stage_data *data,
                                      xhsdci_status_t status) {
 ...
 *    KERN_*         - Error occured
 */
static kern_return_t shmem_stage_announce(struct kdp_output_stage *stage,
                                          const char *corename,
                                          uint8_t coretype) {
  struct shmem_stage_data *data = (struct shmem_stage_data *)stage->kos_data;
  kern_return_t ret = KERN_SUCCESS;

  /* Don't signal XHSDCI_COREDUMP_DONE unless remote side has seen
   * XHSDCI_COREDUMP_INFO. */
  ...
  strlcpy(hwsd_info->xhsdci_file_name, corename,
          sizeof(hwsd_info->xhsdci_file_name));
  ...
  /* Remote side has seen XHSDCI_COREDUMP_INFO so it will expect
   * XHSDCI_COREDUMP_DONE. */
 ...
 */
static kern_return_t shmem_stage_reset(struct kdp_output_stage *stage,
                                       const char *corename,
  ...
  if (data->currently_filling_buf != NULL) {
    ...
    STAILQ_INSERT_HEAD(&free_hw_shmem_dbg_bufs, data->currently_filling_buf,
                       khsd_elms);
  ...
  if (data->currently_flushing_buf != NULL) {
    ...
    STAILQ_INSERT_HEAD(&free_hw_shmem_dbg_bufs, data->currently_flushing_buf,
                       khsd_elms);
    data->currently_flushing_buf = NULL;
  }

  STAILQ_FOREACH_SAFE(cur_elm, &hw_shmem_dbg_bufs_to_flush, khsd_elms,
                      tmp_elm) {
    cur_elm->khsd_data_length = 0;

    STAILQ_REMOVE(&hw_shmem_dbg_bufs_to_flush, cur_elm,
                  kdp_hw_shmem_dbg_buf_elm, khsd_elms);
 ...
 */
static kern_return_t shmem_dbg_process_buffers(struct kdp_output_stage *stage) {
  struct shmem_stage_data *data = (struct shmem_stage_data *)stage->kos_data;
  ...
  if (hwsd_info->xhsdci_status == XHSDCI_COREDUMP_ERROR) {
    ...
    if (data->currently_flushing_buf != NULL) {
      data->currently_flushing_buf->khsd_data_length = 0;
      STAILQ_INSERT_TAIL(&free_hw_shmem_dbg_bufs, data->currently_flushing_buf,
                         khsd_elms);
    }
    ...
    if (data->currently_flushing_buf != NULL) {
      ...
      FlushPoC_DcacheRegion((vm_offset_t)hwsd_info, sizeof(*hwsd_info));
      hwsd_info->xhsdci_buf_phys_addr =
          kvtophys(data->currently_flushing_buf->khsd_buf);
      hwsd_info->xhsdci_buf_data_length =
          data->currently_flushing_buf->khsd_data_length;
      hwsd_info->xhsdci_coredump_total_size_uncomp =
          stage->kos_outstate->kcos_totalbytes;
      hwsd_info->xhsdci_coredump_total_size_sent_uncomp =
          stage->kos_outstate->kcos_bytes_written;
      FlushPoC_DcacheRegion((vm_offset_t)hwsd_info,
    ...
    kern_coredump_log(NULL, "Kernel timed out waiting for hardware debugger to "
                            "update handshake structure.");
 ...
 */
static kern_return_t shmem_dbg_get_buffer(struct kdp_output_stage *stage) {
  kern_return_t ret = KERN_SUCCESS;
  struct shmem_stage_data *data = (struct shmem_stage_data *)stage->kos_data;
 ...
 */
static kern_return_t shmem_stage_outproc(struct kdp_output_stage *stage,
  ...
  if (request == KDP_EOF || request == KDP_SEEK) {
    ...
    while (!STAILQ_EMPTY(&hw_shmem_dbg_bufs_to_flush) ||
      ...
      if (KERN_SUCCESS != ret) {
        kern_coredump_log(
            NULL, "(%s) shmem_dbg_process_buffers failed with error 0x%x\n",
            __func__, ret);
      }
      return ret;
    }
    ...
    if (hwsd_info->xhsdci_seq_no < data->seq_no) {
      kern_coredump_log(NULL,
                        "EOF Flush: Detected stale/invalid seq num. Expected: "
                        "%d, received %d\n",
                        data->seq_no, hwsd_info->xhsdci_seq_no);
      return -1;
    }
  ...
  if (length == 0) {
    ...
    if (data->currently_filling_buf != NULL) {
      STAILQ_INSERT_TAIL(&hw_shmem_dbg_bufs_to_flush,
                         data->currently_filling_buf, khsd_elms);
      data->currently_filling_buf = NULL;
    }
  ...
  while (bytes_remaining != 0) {
    ...
    assert(kdp_hw_shmem_dbg_bufsize >=
           data->currently_filling_buf->khsd_data_length);
    bytes_to_copy =
        MIN(bytes_remaining, kdp_hw_shmem_dbg_bufsize -
                                 data->currently_filling_buf->khsd_data_length);
    bcopy(panic_data,
          (void *)(data->currently_filling_buf->khsd_buf +
                   data->currently_filling_buf->khsd_data_length),
          bytes_to_copy);

    data->currently_filling_buf->khsd_data_length += bytes_to_copy;

    if (data->currently_filling_buf->khsd_data_length ==
...
static void shmem_stage_free(struct kdp_output_stage *stage) {
...
kern_return_t shmem_stage_initialize(struct kdp_output_stage *stage) {
  ...
  if (!hwsd_info) {
    ...
    /*
     * We need to allocate physically contiguous memory since astris isn't
     * capable of doing address translations while the CPUs are running.
     */
    kdp_hw_shmem_dbg_bufsize = KDP_CORE_HW_SHMEM_DBG_TOTAL_BUF_SIZE;
    kmem_alloc_contig(kernel_map, &kdp_core_hw_shmem_buf,
                      kdp_hw_shmem_dbg_bufsize, VM_MAP_PAGE_MASK(kernel_map), 0,
                      0, KMA_NOFAIL | KMA_KOBJECT | KMA_DATA | KMA_PERMANENT,
                      VM_KERN_MEMORY_DIAG);

    /*
     * Put the connection info structure at the beginning of this buffer and
    ...
    /* Leave the handshake structure on its own cache line so buffer writes
     * don't cause flushes of old handshake data */
    kdp_core_hw_shmem_buf =
        ROUNDUP(kdp_core_hw_shmem_buf, (vm_offset_t)cpuid_cache_info->c_linesz);
    kdp_hw_shmem_dbg_bufsize -=
        (uint32_t)(kdp_core_hw_shmem_buf - (vm_offset_t)hwsd_info);
    kdp_hw_shmem_dbg_bufsize /= KDP_CORE_HW_SHMEM_DBG_NUM_BUFFERS;
    /* The buffer size should be a cache-line length multiple */
    kdp_hw_shmem_dbg_bufsize -=
        (kdp_hw_shmem_dbg_bufsize %
         ROUNDDOWN(OPTIMAL_ASTRIS_READSIZE, cpuid_cache_info->c_linesz));
    ...
    PE_consistent_debug_register(kDbgIdAstrisConnection,
                                 kvtophys((vm_offset_t)hwsd_info),
                                 sizeof(pmap_paddr_t));
    PE_consistent_debug_register(kDbgIdAstrisConnectionVers,
                                 CUR_XNU_HWSDCI_STRUCT_VERS, sizeof(uint32_t));
  }

  stage->kos_data_size = sizeof(struct shmem_stage_data);

  ret = kmem_alloc(kernel_map, (vm_offset_t *)&stage->kos_data,
                   stage->kos_data_size, KMA_DATA_SHARED, VM_KERN_MEMORY_DIAG);
  if (KERN_SUCCESS != ret) {
    return ret;
  }
  ...
  nanoseconds_to_absolutetime(shmem_timeout_sec * NSEC_PER_SEC,
                              &(data->contact_deadline_interval));
  data->currently_filling_buf = NULL;
  data->currently_flushing_buf = NULL;
...
void panic_spin_shmcon(void) {
  ...
  kern_coredump_log(NULL,
                    "Waiting for hardware shared memory debugger, handshake "
                    "structure is at virt: %p, phys %p\n",
...
#ifdef __arm64__
    /* Avoid stalling in WFE on arm32, which may not have a maximum WFE timeout
     * like arm64. */
    __builtin_arm_wfe();
#endif
  }
