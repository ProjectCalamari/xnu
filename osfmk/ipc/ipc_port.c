...
KALLOC_ARRAY_TYPE_DEFINE(ipc_port_request_table, struct ipc_port_request,
                         KT_DEFAULT);
...
void __abortlike __ipc_port_inactive_panic(ipc_port_t port) {
  panic("Using inactive port %p", port);
}

static __abortlike void __ipc_port_translate_receive_panic(ipc_space_t space,
                                                           ipc_port_t port) {
  panic("found receive right in space %p for port %p owned by space %p", space,
        port, ip_get_receiver(port));
}

__abortlike void __ipc_right_delta_overflow_panic(ipc_port_t port,
                                                  natural_t *field, int delta) {
  const char *what;
...
static void ipc_port_send_turnstile_recompute_push_locked(ipc_port_t port);

static thread_t ipc_port_get_watchport_inheritor(ipc_port_t port);

static kern_return_t ipc_port_update_qos_n_iotier(ipc_port_t port, uint8_t qos,
                                                  uint8_t iotier);

void ipc_port_lock(ipc_port_t port) {
  ip_validate(port);
  waitq_lock(&port->ip_waitq);
}

ipc_object_label_t ipc_port_lock_label_get(ipc_port_t port) {
  ip_validate(port);
  waitq_lock(&port->ip_waitq);
  return ip_label_get(port);
}

ipc_object_label_t ipc_port_lock_check_aligned(ipc_port_t port) {
  zone_id_require_aligned(ZONE_ID_IPC_PORT, port);
  waitq_lock(&port->ip_waitq);
  return ip_label_get(port);
}

bool ipc_port_lock_try(ipc_port_t port) {
  ip_validate(port);
  return waitq_lock_try(&port->ip_waitq);
}

void ipc_port_release(ipc_port_t port) { ip_release(port); }
...
ipc_port_timestamp_t ipc_port_timestamp(void) {
 ...
 */
kern_return_t ipc_port_translate_send(ipc_space_t space, mach_port_name_t name,
                                      ipc_port_t *portp) {
 ...
 *		KERN_INVALID_RIGHT	Name doesn't denote the correct right
 */
kern_return_t ipc_port_translate_receive(ipc_space_t space,
...
kern_return_t ipc_port_request_alloc(ipc_port_t port, mach_port_name_t name,
                                     ipc_port_t soright,
...
kern_return_t ipc_port_request_hnotify_alloc(ipc_port_t port,
...
kern_return_t ipc_port_request_grow(ipc_port_t port) {
...
bool ipc_port_request_sparm(ipc_port_t port,
  ...
  if (index != IE_REQ_NONE) {
    ...
    if (IPR_SOR_SPREQ(ipr->ipr_soright)) {
      ...
      if (((option & MACH_SEND_NOIMPORTANCE) == 0) &&
          (port->ip_impdonation != 0) && (port->ip_spimportant == 0) &&
 ...
 */
mach_port_type_t ipc_port_request_type(ipc_port_t port,
                                       __assert_only mach_port_name_t name,
                                       ipc_port_request_index_t index) {
  ipc_port_request_table_t table;
  ipc_port_request_t ipr;
 ...
 * send-once right. Conditions: The port must be locked and active. The index
 * must not be IPR_REQ_NONE and must correspond with name.
 */

ipc_port_t ipc_port_request_cancel(ipc_port_t port,
                                   __assert_only mach_port_name_t name,
                                   ipc_port_request_index_t index) {
  ipc_port_request_table_t table;
...
ipc_port_prepare_move(ipc_port_t port, ipc_object_label_t *label,
...
__attribute__((always_inline)) ipc_port_t ipc_port_mark_in_space(
    ipc_port_t port, ipc_object_label_t *label, ipc_space_t space,
    mach_port_name_t name, ipc_object_state_t force_state) {
...
ipc_port_mark_in_limbo(ipc_port_t port, ipc_object_label_t *label,
                       waitq_link_list_t *free_l) {
...
__attribute__((always_inline)) static void
ipc_port_mark_in_limbo_pd(ipc_port_t port, ipc_object_label_t *label,
                          waitq_link_list_t *free_l) {
  ipc_release_assert(ipc_policy(label)->pol_movability !=
...
void ipc_port_mark_in_transit(ipc_port_t port, ipc_port_t dest) {
...
ipc_port_mark_inactive(ipc_port_t port, ipc_object_label_t *label,
                       waitq_link_list_t *free_l) {
 ...
 *
 *		The memory is expected to be zero initialized (allocated with
 * Z_ZERO).
 */
static void ipc_port_init(ipc_port_t port, ipc_space_t space,
                          ipc_object_label_t label, ipc_port_init_flags_t flags,
                          mach_port_name_t name) {
...
kern_return_t ipc_port_alloc(ipc_space_t space, ipc_object_label_t label,
                             ipc_port_init_flags_t flags,
                             mach_port_name_t *namep, ipc_port_t *portp) {
  mach_port_name_t name;
  kern_return_t kr;
...
kern_return_t ipc_port_alloc_name(ipc_space_t space, ipc_object_label_t label,
                                  ipc_port_init_flags_t flags,
                                  mach_port_name_t name, ipc_port_t *portp) {
  kern_return_t kr;
  ipc_entry_t entry;
  mach_port_type_t type = MACH_PORT_TYPE_RECEIVE;
  mach_port_urefs_t urefs = 0;
 ...
 */
void ipc_port_spnotify(ipc_port_t port) {
...
revalidate:
  if (ip_active(port)) {
    ...
    while (++index < size) {
      ipc_port_request_t ipr =
          ipc_port_request_table_get_nocheck(requests, index);
 ...
 */
void ipc_port_dnnotify(ipc_port_t port) {
...
void ipc_port_destroy(ipc_port_t port) {
  ...
  if (ipc_policy(label)->pol_kobject_permanent) {
    panic("trying to destroy a permanent port %p with kobject type: %d", port,
          ip_type(port));
  }
  ...
  if (release_imp_task != IIT_NULL) {
    ...
    if (top) {
      ...
      if (ipc_importance_task_is_any_receiver_type(release_imp_task)) {
        ipc_importance_task_drop_internal_assertion(release_imp_task,
                                                    assertcnt);
      }
    }
  }
 ...
 */
boolean_t ipc_port_destination_chain_lock(ipc_port_t port, ipc_port_t *base) {
  for (;;) {
    ip_mq_lock(port);
...
boolean_t ipc_port_check_circularity(ipc_port_t port, ipc_port_t dest) {
#if IMPORTANCE_INHERITANCE
 ...
 *		port locked
 */
static struct task_watchport_elem *ipc_port_watchport_elem(ipc_port_t port) {
...
ipc_port_update_watchport_elem(ipc_port_t port,
                               struct task_watchport_elem *we) {
 ...
 *	Purpose:
 *		Set the pid of process that copied out send once right to
 * special reply port.
 *
 *	Conditions:
 *		port locked
 */
static inline void ipc_special_reply_stash_pid_locked(ipc_port_t port,
                                                      int pid) {
  assert(ip_is_special_reply_port(port));
  port->ip_pid = pid;
}
 ...
 *	Purpose:
 *		Get the pid of process that copied out send once right to
 * special reply port.
 *
 *	Conditions:
 *		port locked
 */
int ipc_special_reply_get_pid_locked(ipc_port_t port) {
  assert(ip_is_special_reply_port(port));
  return port->ip_pid;
}
 ...
 */
void ipc_port_recv_update_inheritor(ipc_port_t port,
 ...
 *    to walk.
 */
void ipc_port_send_update_inheritor(ipc_port_t port,
 ...
 *		Nothing is locked.
 */
void ipc_port_send_turnstile_prepare(ipc_port_t port) {
 ...
 */
static struct turnstile *ipc_port_rcv_turnstile(ipc_port_t port) {
 ...
 *		Nothing is locked.
 */
void ipc_port_link_special_reply_port(ipc_port_t special_reply_port,
 ...
 *		Nothing is locked.
 */
void ipc_port_thread_group_blocked(ipc_port_t port __unused) {
#if CONFIG_THREAD_GROUPS
  bool port_tg_block_tracking = false;
  thread_t self = current_thread();

  if (self->thread_group == NULL || (self->options & TH_OPT_IPC_TG_BLOCKED)) {
    return;
  }
 ...
 *		Nothing is locked.
 */
void ipc_port_thread_group_unblocked(void) {
#if CONFIG_THREAD_GROUPS
  thread_t self = current_thread();
...
inline void ipc_special_reply_port_bits_reset(ipc_port_t special_reply_port) {
...
inline void ipc_special_reply_port_msg_sent(ipc_port_t special_reply_port) {
  if (ip_is_special_reply_port(special_reply_port)) {
 ...
 *		None.
 */
void ipc_port_adjust_special_reply_port_locked(ipc_port_t special_reply_port,
                                               struct knote *kn, uint8_t flags,
  ...
  not_special:
    if (get_turnstile) {
      turnstile_complete((uintptr_t)special_reply_port,
                         port_rcv_turnstile_address(special_reply_port), NULL,
                         TURNSTILE_SYNC_IPC);
    }
    ip_mq_unlock(special_reply_port);
    if (get_turnstile) {
  ...
  if (flags & IPC_PORT_ADJUST_SR_LINK_WORKLOOP) {
    if (ITH_KNOTE_VALID(kn, MACH_MSG_TYPE_PORT_SEND_ONCE)) {
      inheritor =
          filt_machport_stash_port(kn, special_reply_port, &sync_link_state);
    }
  ...
  if (special_reply_port->ip_sync_link_state == PORT_SYNC_LINK_PORT &&
    ...
    ipc_special_reply_stash_pid_locked(special_reply_port,
                                       pid_from_task(current_task()));
  } else if (special_reply_port->ip_sync_link_state ==
 ...
 *		None.
 */
void ipc_port_adjust_special_reply_port(ipc_port_t port, uint8_t flags) {
 ...
 *              None.
 */
void ipc_port_adjust_sync_link_state_locked(ipc_port_t port,
                                            int sync_link_state,
 ...
 *		None.
 */
void ipc_port_adjust_port_locked(ipc_port_t port, struct knote *kn,
                                 boolean_t sync_bootstrap_checkin) {
 ...
 *		None.
 */
void ipc_port_clear_sync_rcv_thread_boost_locked(ipc_port_t port) {
  ip_mq_lock_held(port); // ip_sync_link_state is touched
 ...
 *		Port is locked.
 */
bool ipc_port_has_prdrequest(ipc_port_t port) {
  if (ip_is_special_reply_port(port)) {
    return false;
  }
  if (port->ip_has_watchport) {
 ...
 */
kern_return_t
ipc_port_add_watchport_elem_locked(ipc_port_t port,
                                   struct task_watchport_elem *watchport_elem,
                                   struct task_watchport_elem **old_elem) {
  ip_mq_lock_held(port);

  /* Watchport boost only works for non-special active ports mapped in an ipc
   * space */
  if (!ip_active(port) || ip_is_special_reply_port(port) ||
      !ip_in_a_space(port)) {
    ip_mq_unlock(port);
...
/*
 *	Routine:
 *	ipc_port_clear_watchport_elem_internal_conditional_locked Purpose:
 ...
 *		KERN_FAILURE otherwise.
 */
kern_return_t ipc_port_clear_watchport_elem_internal_conditional_locked(
    ipc_port_t port, struct task_watchport_elem *watchport_elem) {
  ip_mq_lock_held(port);
...
kern_return_t ipc_port_replace_watchport_elem_conditional_locked(
    ipc_port_t port, struct task_watchport_elem *old_watchport_elem,
...
ipc_port_clear_watchport_elem_internal(ipc_port_t port) {
  ip_mq_lock_held(port);
 ...
 */
pid_t ipc_port_get_receiver_task_locked(ipc_port_t port, task_t *task) {
  ...
  if (ip_in_a_space(port) && !ip_in_space(port, ipc_space_kernel) &&
 ...
 */
pid_t ipc_port_get_receiver_task(ipc_port_t port, task_t *task) {
  pid_t pid = -1;
 ...
 */
mach_port_delta_t ipc_port_impcount_delta(ipc_port_t port,
  ...
  if (ip_in_a_space(port)) {
    ...
    printf("Over-release of importance assertions for port 0x%x receiver pid "
           "%d (%s), "
           "dropping %d assertion(s) but port only has %d remaining.\n",
           ip_get_receiver_name(port), target_pid, target_procname, absdelta,
           port->ip_impcount);
    ...
    printf("Over-release of importance assertions for port 0x%lx "
           ...
           ip_get_receiver_name(base), target_pid, target_procname, absdelta,
           port->ip_impcount);
  }
#endif
...
boolean_t ipc_port_importance_delta_internal(ipc_port_t port, natural_t options,
...
boolean_t ipc_port_importance_delta(ipc_port_t port, natural_t options,
                                    mach_port_delta_t delta) {
  ipc_importance_task_t imp_task = IIT_NULL;
  boolean_t dropped;

  dropped =
      ipc_port_importance_delta_internal(port, options, &delta, &imp_task);
...
ipc_port_t ipc_port_make_send_any_locked(ipc_port_t port) {
  require_ip_active(port);
  port->ip_mscount++;
  ip_srights_inc(port);
  ip_reference(port);
...
void ipc_port_copy_send_any_locked(ipc_port_t port) {
...
ipc_port_copyout_send_internal(ipc_port_t sright, ipc_space_t space,
  ...
  if (IP_VALID(sright)) {
    ...
    kr = ipc_object_copyout(space, sright, MACH_MSG_TYPE_PORT_SEND, flags, NULL,
                            &name);
    if (kr != KERN_SUCCESS) {
...
mach_port_name_t ipc_port_copyout_send(ipc_port_t sright, /* can be invalid */
                                       ipc_space_t space) {
...
mach_port_name_t
ipc_port_copyout_send_pinned(ipc_port_t sright, /* can be invalid */
                             ipc_space_t space) {
  return ipc_port_copyout_send_internal(sright, space,
 ...
 */
void ipc_port_release_send_and_unlock(ipc_port_t port) {
  ipc_notify_nsenders_t nsrequest = {};
...
__attribute__((flatten, noinline)) void ipc_port_release_send(ipc_port_t port) {
  if (IP_VALID(port)) {
    ip_mq_lock(port);
...
ipc_port_t ipc_port_make_sonce_locked(ipc_port_t port) {
  require_ip_active(port);
...
ipc_port_t ipc_port_make_sonce(ipc_port_t port) {
  if (!IP_VALID(port)) {
    return port;
  }
 ...
 */
void ipc_port_release_sonce_and_unlock(ipc_port_t port) {
  ip_mq_lock_held(port);
  ...
  if (ip_is_special_reply_port(port)) {
    ipc_port_adjust_special_reply_port_locked(
        port, NULL, IPC_PORT_ADJUST_RESET_BOOSTRAP_CHECKIN, FALSE);
  } else {
    ip_mq_unlock(port);
  }
 ...
 */
void ipc_port_release_sonce(ipc_port_t port) {
  if (IP_VALID(port)) {
    ip_mq_lock(port);
...
ipc_port_t ipc_port_alloc_special(ipc_space_t space, ipc_object_label_t label,
                                  ipc_port_init_flags_t flags) {
  ipc_port_t port;
 ...
 *		Nothing locked.
 */
void ipc_port_free(ipc_port_t port) {
  ipc_port_request_table_t requests = port->ip_requests;
 ...
 */
void kdp_mqueue_send_find_owner(struct waitq *waitq,
  ...
  ipc_port_t port =
      (ipc_port_t)turnstile->ts_proprietor; /* we are blocking on send */
 ...
 */
void kdp_mqueue_recv_find_owner(struct waitq *waitq,
                                __assert_only event64_t event,
                                thread_waitinfo_v2_t *waitinfo,
                                struct ipc_service_port_label **isplp) {
  ...
  if (waitq_type(waitq) == WQT_PORT_SET) {
    ...
    /* There is no specific owner "at the other end" of a port set, so leave
     * unset. */
...
kern_return_t ipc_port_reset_thread_attr(ipc_port_t port) {
...
kern_return_t
ipc_port_propagate_thread_attr(ipc_port_t port,
...
static kern_return_t ipc_port_update_qos_n_iotier(ipc_port_t port, uint8_t qos,
  ...
  if (ip_in_a_space(port) && is_active(ip_get_receiver(port)) &&
...
void db_port_stack_trace(ipc_port_t port);
void db_ref(int refs);
int db_port_walk(unsigned int verbose, unsigned int display,
                 unsigned int ref_search, unsigned int ref_target);
 ...
 */
void ipc_port_init_debug(ipc_port_t port, void *fp) {
