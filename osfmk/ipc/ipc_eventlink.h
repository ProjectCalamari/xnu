...
#define ipc_eventlink_active(eventlink)                                        \
  waitq_valid(&(eventlink)->el_base->elb_waitq)

#define eventlink_remote_side(eventlink)                                       \
  ((eventlink) == &((eventlink)->el_base->elb_eventlink[0])                    \
       ? &((eventlink)->el_base->elb_eventlink[1])                             \
       : &((eventlink)->el_base->elb_eventlink[0]))

#define ipc_eventlink_lock(eventlink)                                          \
  waitq_lock(&(eventlink)->el_base->elb_waitq)
#define ipc_eventlink_unlock(eventlink)                                        \
  waitq_unlock(&(eventlink)->el_base->elb_waitq)
