...
#define XPC_DOMAIN_PORT                                                        \
  7 /* This value should match what is in <xpc/launch_private.h> */

ZONE_DEFINE_TYPE(ipc_service_port_label_zone, "ipc_service_port_label",
                 struct ipc_service_port_label,
                 ZC_ZFREE_CLEARMEM | ZC_NOCACHING);
...
void kdp_ipc_splabel_size(size_t *ispl_size, size_t *maxnamelen) {
  *ispl_size = sizeof(struct ipc_service_port_label);
  *maxnamelen = MACH_SERVICE_PORT_INFO_STRING_NAME_MAX_BUF_LEN + 1;
}

void kdp_ipc_fill_splabel(struct ipc_service_port_label *ispl,
 ...
 *   send_side_filtering: indicates if the messages should be filtered during
 * mach_msg_send port_label_ptr: used to return the allocated service_port_label
 *
 * Returns:
 *   KERN_SUCCESS
 */
kern_return_t ipc_service_port_label_alloc(mach_service_port_info_t sp_info,
  ...
  if (mach_msg_filter_alloc_service_port_sblabel_callback) {
    ret =
        mach_msg_filter_alloc_service_port_sblabel_callback(sp_info, &sblabel);
    if (ret) {
  ...
  strlcpy(sp_label->ispl_service_name, sp_info->mspi_string_name,
          sp_string_name_len + 1);
...
void ipc_connection_port_label_dealloc(ipc_object_label_t label) {
 ...
 */
void ipc_service_port_label_dealloc(ipc_object_label_t label) {
  ipc_service_port_label_t sp_label = label.iol_service;
  void *sblabel = sp_label->ispl_sblabel;

#if CONFIG_SERVICE_PORT_INFO
  kfree_data(sp_label->ispl_service_name,
             strlen(sp_label->ispl_service_name) + 1);
#endif /* CONFIG_SERVICE_PORT_INFO */
  zfree(ipc_service_port_label_zone, sp_label);
 ...
 *
 * Description: Derive the port's sandbox label using info from the service
 * port's label
 *
 * Args:
 ...
 */
kern_return_t
ipc_service_port_derive_sblabel(mach_port_name_t service_port_name, bool force,
                                ipc_object_label_t *label) {
  ipc_port_t port;
  kern_return_t kr;
  ...
  if (mach_msg_filter_at_least(MACH_MSG_FILTER_CALLBACKS_VERSION_1)) {
    ...
    if (sblabel) {
      ...
      label->iol_connection =
          mach_msg_filter_derive_sblabel_from_service_port_callback(
              sblabel, &send_side_filtering);
...
#if CONFIG_SERVICE_PORT_INFO
void ipc_service_port_label_get_info(ipc_service_port_label_t port_splabel,
  ...
  strlcpy(info->mspi_string_name, port_splabel->ispl_service_name,
          sp_string_name_len + 1);
}
#endif /* CONFIG_SERVICE_PORT_INFO */
