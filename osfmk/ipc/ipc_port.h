...
struct ipc_port_request {
  union {
    ...
    struct host_notify_entry *
        XNU_PTRAUTH_SIGNED_PTR("ipc_port_request.ipr_hnotify") ipr_hnotify;
    struct ipc_port_request *
        XNU_PTRAUTH_SIGNED_PTR("ipc_port_request.ipr_hn_slot") ipr_hn_slot;
  };
...
struct ipc_port {
  ...
  union {
    ...
    WAITQ_FLAGS(ip_waitq,
                ip_fullwaiters
                : 1 /* Whether there are senders blocked on a full queue */
                ,
                ...
                ,
                ip_strict_guard
                : 1 /* Strict guarding; Prevents user manipulation of context
                       values directly */
                ,
                ip_sync_link_state
                ...
                : 1 /* port part of sync bootstrap checkin, push on thread doing
                       the checkin */
                ,
                ip_tg_block_tracking : 1 /* Track blocking relationship between
                                            thread groups during sync IPC */
                ,
  ...
  union {
    struct ipc_space *
        XNU_PTRAUTH_SIGNED_PTR("ipc_port.ip_receiver") ip_receiver;
    struct ipc_port *
        XNU_PTRAUTH_SIGNED_PTR("ipc_port.ip_destination") ip_destination;
  ...
  union {
    ...
    struct ipc_port *
        XNU_PTRAUTH_SIGNED_PTR("ipc_port.ip_nsrequest") ip_nsrequest;
  };
  ...
  union {
    int ip_pid;
    struct task_watchport_elem *
        XNU_PTRAUTH_SIGNED_PTR("ipc_port.ip_twe") ip_twe;
    struct ipc_port *
        XNU_PTRAUTH_SIGNED_PTR("ipc_port.ip_pdrequest") ip_pdrequest;
  };

  ipc_port_request_table_t
      XNU_PTRAUTH_SIGNED_PTR("ipc_port.ip_request") ip_requests;
...
static inline bool ip_in_pset(ipc_port_t port) {
...
#define port_send_turnstile(port) ((port)->ip_send_turnstile)

#define set_port_send_turnstile(port, value)                                   \
  MACRO_BEGIN(port)->ip_send_turnstile = (value);                              \
  MACRO_END

#define port_send_turnstile_address(port) (&((port)->ip_send_turnstile))

#define port_rcv_turnstile_address(port) (&(port)->ip_waitq.waitq_ts)

extern void __ipc_right_delta_overflow_panic(ipc_port_t port, natural_t *field,
...
#define ip_label_peek_kdp(port, ...)                                           \
  io_label_peek_kdp(ip_to_object(port), ##__VA_ARGS__)
...
#define ip_is_immovable_receive(port)                                          \
  (ip_to_object(port)->io_state == IO_STATE_IN_SPACE_IMMOVABLE)

#define ip_is_exception_port(port) (ip_type(port) == IOT_EXCEPTION_PORT)
#define ip_is_provisional_reply_port(port)                                     \
  (ip_type(port) == IOT_PROVISIONAL_REPLY_PORT)
#define ip_is_special_reply_port_type(type) ((type) == IOT_SPECIAL_REPLY_PORT)
#define ip_is_special_reply_port(port)                                         \
  (ip_is_special_reply_port_type(ip_type(port)))
#define ip_is_any_service_port(port) ip_is_any_service_port_type(ip_type(port))
#define ip_is_port_array_allowed(port)                                         \
  (ip_type(port) == IOT_CONNECTION_PORT_WITH_PORT_ARRAY)
#define ip_is_timer(port) (ip_type(port) == IOT_TIMER_PORT)
#define ip_is_bootstrap_port(port) ((port)->ip_bootstrap)

static inline bool ip_is_any_service_port_type(ipc_object_type_t type) {
  return type == IOT_SERVICE_PORT || type == IOT_WEAK_SERVICE_PORT;
}
static inline bool ip_is_reply_port_type(ipc_object_type_t type) {
  return type == IOT_REPLY_PORT || type == IOT_SPECIAL_REPLY_PORT;
}
static inline bool ip_is_reply_port(ipc_port_t port) {
...
static inline bool ip_is_tt_control_port_type(ipc_object_type_t type) {
...
#define ipc_port_multiple_unlock() lck_spin_unlock(&ipc_port_multiple_lock_data)

/*
 *	Search for the end of the chain (a port not in transit),
 *	acquiring locks along the way.
 */
extern boolean_t ipc_port_destination_chain_lock(ipc_port_t port,
                                                 ipc_port_t *base);
...
static inline void require_ip_active(ipc_port_t port) {
  if (!ip_active(port)) {
    __ipc_port_inactive_panic(port);
  }
}

static inline void ip_mq_unlock_label_put(ipc_port_t port,
                                          ipc_object_label_t *label) {
  ip_label_put(port, label);
  io_unlock_nocheck(ip_to_object(port));
}

static inline bool ip_in_space(ipc_port_t port, ipc_space_t space) {
...
static inline bool ip_in_space_noauth(ipc_port_t port, void *space) {
  void *__single raw_ptr = ptrauth_strip(*(void **)&port->ip_receiver,
                                         ptrauth_key_process_independent_data);
  return raw_ptr == space;
}

static inline ipc_space_t ip_get_receiver(ipc_port_t port) {
  ip_mq_lock_held(port); /* port must be locked, otherwise PAC could fail */
  return ip_in_a_space(port) ? port->ip_receiver : NULL;
}

static inline mach_port_name_t ip_get_receiver_name(ipc_port_t port) {
  return ip_in_a_space(port) ? port->ip_receiver_name : MACH_PORT_NULL;
}

static inline ipc_port_t ip_get_destination(ipc_port_t port) {
  ip_mq_lock_held(port); /* port must be locked, otherwise PAC could fail */
  return ip_is_moving(port) ? port->ip_destination : IP_NULL;
}

static inline ipc_port_timestamp_t ip_get_death_time(ipc_port_t port) {
  assert(!ip_active(port));
  return port->ip_timestamp;
}

static inline ipc_importance_task_t ip_get_imp_task(ipc_port_t port) {
  return (!ip_is_kobject(port) && !ip_is_special_reply_port(port) &&
          port->ip_tempowner)
             ? port->ip_imp_task
             : IIT_NULL;
}

extern kern_return_t ipc_port_translate_send(ipc_space_t space,
                                             mach_port_name_t name,
                                             ipc_port_t *portp);

extern kern_return_t ipc_port_translate_receive(ipc_space_t space,
                                                mach_port_name_t name,
                                                ipc_port_t *portp);

/* Allocate a notification request slot */
extern kern_return_t ipc_port_request_alloc(ipc_port_t port,
                                            mach_port_name_t name,
                                            ipc_port_t soright,
...
ipc_port_request_hnotify_alloc(ipc_port_t port,
                               struct host_notify_entry *hnotify,
                               ipc_port_request_index_t *indexp);

/* Grow one of a port's tables of notifcation requests */
extern kern_return_t ipc_port_request_grow(ipc_port_t port);

/* Return the type(s) of notification requests outstanding */
extern mach_port_type_t ipc_port_request_type(ipc_port_t port,
                                              mach_port_name_t name,
                                              ipc_port_request_index_t index);

/* Cancel a notification request and return the send-once right */
extern ipc_port_t ipc_port_request_cancel(ipc_port_t port,
                                          mach_port_name_t name,
                                          ipc_port_request_index_t index);

/* Arm any delayed send-possible notification */
extern bool ipc_port_request_sparm(ipc_port_t port, mach_port_name_t name,
                                   ipc_port_request_index_t index,
 ...
 */
extern ipc_port_t ipc_port_mark_in_space(ipc_port_t port,
                                         ipc_object_label_t *label,
                                         ipc_space_t space,
                                         mach_port_name_t name,
 ...
 *                      on it.
 */
extern void ipc_port_mark_in_limbo(ipc_port_t port, ipc_object_label_t *label,
 ...
 */
extern void ipc_port_mark_in_transit(ipc_port_t port, ipc_port_t dest);
...
extern void ipc_port_lock(ipc_port_t port);

extern ipc_object_label_t
ipc_port_lock_label_get(ipc_port_t port) __result_use_check;

extern ipc_object_label_t
ipc_port_lock_check_aligned(ipc_port_t port) __result_use_check;

extern bool ipc_port_lock_try(ipc_port_t port);

/* Allocate a port */
extern kern_return_t ipc_port_alloc(ipc_space_t space, ipc_object_label_t label,
                                    ipc_port_init_flags_t flags,
                                    mach_port_name_t *namep, ipc_port_t *portp);

/* Allocate a port, with a specific name */
extern kern_return_t ipc_port_alloc_name(ipc_space_t space,
                                         ipc_object_label_t label,
                                         ipc_port_init_flags_t flags,
                                         mach_port_name_t name,
                                         ipc_port_t *portp);
...
extern void ipc_port_dnnotify(ipc_port_t port);

/* Generate send-possible notifications */
extern void ipc_port_spnotify(ipc_port_t port);
...
extern boolean_t ipc_port_check_circularity(ipc_port_t port, ipc_port_t dest);
...
void ipc_special_reply_port_bits_reset(ipc_port_t special_reply_port);

void ipc_special_reply_port_msg_sent(ipc_port_t special_reply_port);
...
/* Adjust special reply port linkage */
void ipc_port_adjust_special_reply_port(ipc_port_t special_reply_port,
                                        uint8_t flags);
...
struct waitq *ipc_port_rcv_turnstile_waitq(struct waitq *waitq);

/* apply importance delta to port only */
extern mach_port_delta_t ipc_port_impcount_delta(ipc_port_t port,
                                                 mach_port_delta_t delta,
                                                 ipc_port_t base);

/* apply importance delta to port, and return task importance for update */
extern boolean_t
ipc_port_importance_delta_internal(ipc_port_t port, natural_t options,
                                   mach_port_delta_t *deltap,
                                   ipc_importance_task_t *imp_task);

/* Apply an importance delta to a port and reflect change in receiver task */
extern boolean_t ipc_port_importance_delta(ipc_port_t port, natural_t options,
                                           mach_port_delta_t delta);
 ...
 */
extern ipc_port_t ipc_port_make_send_any_locked(ipc_port_t port);
 ...
 */
extern ipc_port_t ipc_port_make_send_any(ipc_port_t port) __result_use_check;
 ...
 *                      a naked send right was made.
 */
extern ipc_port_t ipc_port_make_send_mqueue(ipc_port_t port) __result_use_check;
 ...
 * @param port          An active and locked port.
 */
extern void ipc_port_copy_send_any_locked(ipc_port_t port);
 ...
 *                      a naked send right was made.
 */
extern ipc_port_t ipc_port_copy_send_any(ipc_port_t port) __result_use_check;
 ...
 *                      a naked send right was made.
 */
extern ipc_port_t ipc_port_copy_send_mqueue(ipc_port_t port) __result_use_check;

/* Copyout a naked send right */
extern mach_port_name_t ipc_port_copyout_send(ipc_port_t sright,
                                              ipc_space_t space);

extern mach_port_name_t ipc_port_copyout_send_pinned(ipc_port_t sright,
                                                     ipc_space_t space);

extern void ipc_port_thread_group_blocked(ipc_port_t port);

extern void ipc_port_thread_group_unblocked(void);

extern void ipc_port_release_send_and_unlock(ipc_port_t port);

extern kern_return_t mach_port_deallocate_kernel(ipc_space_t space,
                                                 mach_port_name_t name,
                                                 ipc_object_type_t otype);

/* Make a naked send-once right from a locked and active receive right */
extern ipc_port_t ipc_port_make_sonce_locked(ipc_port_t port);

/* Make a naked send-once right from a receive right */
extern ipc_port_t ipc_port_make_sonce(ipc_port_t port);

/* Release a naked send-once right */
extern void ipc_port_release_sonce(ipc_port_t port);

/* Release a naked send-once right */
extern void ipc_port_release_sonce_and_unlock(ipc_port_t port);

/* Release a naked (in limbo or in transit) receive right */
extern void ipc_port_release_receive(ipc_port_t port);

/* Finalize the destruction of a port and free it */
extern void ipc_port_free(ipc_port_t port);

/* Get receiver task and its pid (if any) for port. Assumes port is locked. */
extern pid_t ipc_port_get_receiver_task_locked(ipc_port_t port, task_t *task);

/* Get receiver task and its pid (if any) for port. */
extern pid_t ipc_port_get_receiver_task(ipc_port_t port, task_t *task);

/* Allocate a port in a special space */
extern ipc_port_t ipc_port_alloc_special(ipc_space_t space,
                                         ipc_object_label_t label,
                                         ipc_port_init_flags_t flags);
...
extern int ipc_special_reply_get_pid_locked(ipc_port_t port);
...
struct thread_attr_for_ipc_propagation {
  union {
    struct {
      uint64_t tafip_iotier : 2, tafip_qos : 3;
    };
...
extern kern_return_t ipc_port_reset_thread_attr(ipc_port_t port);
