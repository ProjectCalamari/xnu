...
static void ipc_mqueue_select_on_thread_locked(ipc_mqueue_t mqueue,
                                               mach_msg_option64_t option64,
                                               thread_t thread);

/* Clear a message count reservation */
static void ipc_mqueue_release_msgcount(ipc_mqueue_t mqueue);

/* Deliver message to message queue or waiting receiver */
static void ipc_mqueue_post(ipc_mqueue_t mqueue, ipc_kmsg_t kmsg,
 ...
 */
void ipc_mqueue_init(ipc_mqueue_t mqueue) {
 ...
 */
static bool ipc_mqueue_msg_too_large(mach_msg_size_t msg_size,
 ...
 */
kern_return_t ipc_mqueue_add_locked(ipc_mqueue_t port_mqueue, ipc_pset_t pset,
                                    waitq_link_t *linkp) {
  ...
  while ((kmsg = ipc_kmsg_queue_first(kmsgq)) != IKM_NULL) {
    ...
    if (th->ith_state != MACH_RCV_IN_PROGRESS) {
      waitq_resume_identified_thread(wqset, th, THREAD_AWAKENED,
 ...
 *	Purpose:
 *		Returns whether the given port imq_klist field can be used as a
 * klist.
 */
bool ipc_port_has_klist(ipc_port_t port) {
  return !ip_is_special_reply_port(port) &&
         port->ip_sync_link_state == PORT_SYNC_LINK_ANY;
}

static inline struct klist *ipc_object_klist(ipc_object_t object) {
 ...
 */
void ipc_mqueue_changed(ipc_space_t space, struct waitq *waitq) {
   ...
   */
  waitq_wakeup64_all_locked(waitq, IPC_MQUEUE_RECEIVE, THREAD_RESTART,
                            WAITQ_KEEP_LOCKED);
}
 ...
 *	Purpose:
 *		Send a message to a message queue.  The message holds a
 * reference for the destination port for this message queue in the
 ...
 */
mach_msg_return_t ipc_mqueue_send_locked(ipc_mqueue_t mqueue, ipc_kmsg_t kmsg,
                                         mach_msg_option64_t option,
  ...
  if (!imq_full(mqueue) ||
    ...
    wresult = waitq_assert_wait64_leeway(
        &send_turnstile->ts_waitq, IPC_MQUEUE_FULL, THREAD_ABORTSAFE,
        TIMEOUT_URGENCY_USER_NORMAL, deadline, TIMEOUT_NO_LEEWAY);
 ...
 */
void ipc_mqueue_override_send_locked(ipc_mqueue_t mqueue,
 ...
 */
static void ipc_mqueue_release_msgcount(ipc_mqueue_t port_mq) {
  ipc_port_t port = ip_from_mq(port_mq);
  struct turnstile *send_turnstile = port_send_turnstile(port);

  ip_mq_lock_held(port);
  assert(port_mq->imq_msgcount > 1 ||
         ipc_kmsg_queue_empty(&port_mq->imq_messages));
  ...
  if (!imq_full(port_mq) && port->ip_fullwaiters &&
     ...
     */
    if (waitq_wakeup64_one(&send_turnstile->ts_waitq, IPC_MQUEUE_FULL,
                           THREAD_AWAKENED,
...
static void ipc_mqueue_post(ipc_mqueue_t mqueue, ipc_kmsg_t kmsg,
  ...
  for (;;) {
    ...
    receiver = waitq_wakeup64_identify_locked(waitq, IPC_MQUEUE_RECEIVE,
                                              WAITQ_KEEP_LOCKED);
    ...
    if (receiver == THREAD_NULL) {
      ...
      if (mqueue->imq_msgcount == 0) {
         ...
         */
      } else if (ip_in_a_space(port) && is_active(ip_get_receiver(port)) &&
                 ipc_port_has_klist(port)) {
        /*
    ...
    if (receiver->ith_state != MACH_RCV_IN_PROGRESS) {
      waitq_resume_identified_thread(waitq, receiver, THREAD_AWAKENED,
                                     WAITQ_WAKEUP_DEFAULT);
...
static void ipc_mqueue_receive_results(wait_result_t saved_wait_result) {
  ...
  case THREAD_AWAKENED:
    ...
    case MACH_MSG_SUCCESS:
      return;
    default:
      panic("ipc_mqueue_receive_results: strange ith_state %d",
            self->ith_state);
    }

  default:
    panic("ipc_mqueue_receive_results: strange wait_result %d",
          saved_wait_result);
  }
}

static void ipc_mqueue_receive_continue(__unused void *param,
...
void ipc_mqueue_receive(struct waitq *waitq, mach_msg_timeout_t rcv_timeout,
                        int interruptible, thread_t thread,
 ...
 */
wait_result_t
ipc_mqueue_receive_on_thread_and_unlock(struct waitq *waitq,
  ...
  if (port) {
    ipc_mqueue_select_on_thread_locked(&port->ip_messages, option64, thread);
    ip_mq_unlock(port);
  ...
  if (waitq_type(waitq) == WQT_PORT) {
    port = ip_from_waitq(waitq);
    rcv_turnstile =
        turnstile_prepare((uintptr_t)port, port_rcv_turnstile_address(port),
                          TURNSTILE_NULL, TURNSTILE_SYNC_IPC);
  ...
  wresult = waitq_assert_wait64_locked(waitq, IPC_MQUEUE_RECEIVE, interruptible,
  ...
  /* Check if its a port mqueue and if it needs to call
   * turnstile_update_inheritor_complete */
  if (rcv_turnstile != TURNSTILE_NULL) {
    turnstile_update_inheritor_complete(rcv_turnstile,
                                        TURNSTILE_INTERLOCK_NOT_HELD);
  }
  /* Its callers responsibility to call turnstile_complete to get the turnstile
   * back */
 ...
 *
 *	Returns:
 *		MACH_MSG_SUCCESS	Actually selected a message for
 * ourselves. MACH_RCV_TOO_LARGE  May or may not have pull it, but it is large
 */
static void ipc_mqueue_select_on_thread_locked(ipc_mqueue_t port_mq,
                                               mach_msg_option64_t options,
                                               thread_t thread) {
  mach_msg_size_t msize, tsize, asize;
  ipc_kmsg_t kmsg;
 ...
 *		Other locks may be held by callers, so this routine cannot
 * block. Caller holds reference on the message queue.
 */
unsigned ipc_mqueue_peek_locked(ipc_mqueue_t mq, mach_port_seqno_t *seqnop,
 ...
 */
boolean_t ipc_mqueue_destroy_locked(ipc_mqueue_t mqueue,
  ...
  if (send_turnstile != TURNSTILE_NULL) {
    waitq_wakeup64_all(&send_turnstile->ts_waitq, IPC_MQUEUE_FULL,
...
void ipc_mqueue_set_qlimit_locked(ipc_mqueue_t mqueue,
  ...
  if (qlimit > mqueue->imq_qlimit) {
    ...
    for (i = 0; i < wakeup; i++) {
       ...
       */
      if (send_turnstile == TURNSTILE_NULL ||
          waitq_wakeup64_one(&send_turnstile->ts_waitq, IPC_MQUEUE_FULL,
                             THREAD_AWAKENED,
 ...
 */
void ipc_mqueue_set_seqno_locked(ipc_mqueue_t mqueue, mach_port_seqno_t seqno) {
...
mach_msg_return_t ipc_mqueue_copyin(ipc_space_t space, mach_port_name_t name,
