...
mach_msg_rcv_link_special_reply_port(ipc_port_t special_reply_port,
                                     mach_port_name_t dest_name_port);

void mach_msg_receive_results_complete(ipc_object_t object);
 ...
 */
static mach_msg_return_t
mach_msg_receive_too_large(mach_msg_recv_bufs_t *recv_bufs,
                           mach_msg_recv_result_t *msgr,
                           mach_msg_option64_t options, mach_msg_return_t mr) {
  ...
  if ((options & MACH64_RCV_LARGE_IDENTITY) &&
     ...
     */
    if (mach_copyout_field(&msgr->msgr_recv_name, recv_bufs->recv_msg_addr,
  ...
  if (recv_bufs->recv_msg_size >=
     ...
     */
    if (mach_copyout_field(&msgr->msgr_msg_size, recv_bufs->recv_msg_addr,
  ...
  if (recv_bufs->recv_aux_addr) {
    ...
    if (mach_copyout_field(&msgr->msgr_aux_size, recv_bufs->recv_aux_addr,
 ...
 */
static mach_msg_return_t mach_msg_receive_error(ipc_kmsg_t kmsg,
 ...
 */
mach_msg_return_t mach_msg_receive_results(mach_msg_recv_result_t *msgr_out) {
   ...
   * get the thread's turnstile, if the thread donated it's turnstile to the
   * port
   */
  ...
  if (mr == MACH_RCV_TOO_LARGE) {
    ...
    if (options & MACH64_RCV_LARGE) {
      mr = mach_msg_receive_too_large(&recv_bufs, &msgr, options, mr);
    } else {
      /* discard importance in message */
      ipc_importance_clean(kmsg);
      mr = mach_msg_receive_error(kmsg, &recv_bufs, &msgr, options, space, map,
                                  mr);
      /* kmsg freed */
    }
  }
  ...
  if (mr != MACH_MSG_SUCCESS) {
    ...
    /* if we had a body error copyout what we have, otherwise a simple
     * header/trailer */
    if ((mr & ~MACH_MSG_MASK) == MACH_RCV_BODY_ERROR) {
      mr = ipc_kmsg_put_to_user(kmsg, &recv_bufs, &msgr, options, map, mr);
    } else {
      mr = mach_msg_receive_error(kmsg, &recv_bufs, &msgr, options, space, map,
                                  mr);
    }
...
void mach_msg_receive_continue(void) {
  mach_msg_return_t mr;
 ...
 *      we have copied in.
 */
static mach_msg_return_t mach_msg_validate_data_vectors(
    mach_msg_vector_t *msg_vec, mach_msg_vector_t *aux_vec,
    mach_msg_size_t vec_count, __unused mach_msg_option64_t option64,
    bool sending) {
 ...
 */
static mach_msg_return_t
mach_msg_copyin_data_vectors(mach_vm_address_t data_addr, /* user address */
                             ...
                             mach_msg_vector_t *aux_vecp) /* out */
{
  ...
  if (cpin_count > MACH_MSGV_MAX_COUNT) {
    return (option64 & MACH64_SEND_MSG) ? MACH_SEND_INVALID_DATA
                                        : MACH_RCV_INVALID_ARGUMENTS;
  }

  if (cpin_count == 0) {
    return (option64 & MACH64_SEND_MSG) ? MACH_SEND_MSG_TOO_SMALL
                                        : MACH_RCV_INVALID_ARGUMENTS;
  }
 ...
 *      large enough. Returns the header of the message and number of
 * descriptors. Used for mach_msg_overwrite_trap() only. Not available on
 * embedded. Returns: MACH_MSG_SUCCESS - Copyin succeeded, msg_addr and msg_size
 * are validated. MACH_SEND_MSG_TOO_SMALL MACH_SEND_TOO_LARGE
 *      MACH_SEND_INVALID_DATA
 */
static mach_msg_return_t
mach_msg_copyin_user_header(mach_msg_send_uctx_t *send_uctx,
                            mach_msg_option64_t options) {
  ...
  if (send_uctx->send_msg_size < sizeof(mach_msg_user_base_t)) {
    ...
    mr = copyinmsg(send_uctx->send_msg_addr, &send_uctx->send_header,
                   sizeof(mach_msg_user_header_t));
  } else {
    mr = copyinmsg(send_uctx->send_msg_addr, &send_uctx->send_header,
                   sizeof(mach_msg_user_base_t));
  }
  if (mr != KERN_SUCCESS) {
    return MACH_SEND_INVALID_DATA;
  }
...
__attribute__((noinline, cold)) static mach_msg_return_t
mach_msg_receive_pseudo(ipc_kmsg_t kmsg, mach_msg_send_uctx_t *send_uctx,
                        mach_msg_option64_t options, ipc_space_t space,
                        vm_map_t map) {
  mach_msg_recv_bufs_t recv_bufs = {
 ...
 */
static mach_msg_return_t mach_msg_trap_send(
    /* shared args between send and receive */
    mach_msg_send_uctx_t *send_uctx, mach_msg_option64_t options,
    mach_msg_timeout_t msg_timeout, mach_msg_priority_t priority) {
  ipc_space_t space = current_space();
  vm_map_t map = current_map();
  ipc_kmsg_t kmsg;
  mach_msg_return_t mr;
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_KMSG_LINK) |
                            DBG_FUNC_NONE,
                        (uintptr_t)send_uctx->send_msg_addr,
                        VM_KERNEL_ADDRPERM((uintptr_t)kmsg), 0, 0, 0);

  /*
   * holding kmsg ref
   * may add MACH64_SEND_ALWAYS to options
   */
  mr = ipc_kmsg_copyin_from_user(kmsg, send_uctx, space, map, priority,
                                 &options);
  if (mr != MACH_MSG_SUCCESS) {
    ipc_kmsg_free(kmsg);
    return mr;
 ...
 */
static mach_msg_return_t mach_msg_trap_receive(
    /* shared args between send and receive */
    mach_vm_address_t msg_addr,
    mach_vm_address_t aux_addr, /* 0 if not vector send/rcv */
    mach_msg_option64_t option64, mach_msg_timeout_t msg_timeout,
  ...
  ipc_mqueue_receive(io_waitq(object), msg_timeout, THREAD_ABORTSAFE, self,
                     /* continuation ? */ true);
...
mach_msg_overwrite_trap(struct mach_msg_overwrite_trap_args *args) {
  ...
  if (options & MACH_SEND_MSG) {
    ...
    mr = mach_msg_trap_send(&send_uctx, options, args->timeout, args->priority);
  }
  ...
  if (options & MACH_RCV_MSG) {
    ...
    mr = mach_msg_trap_receive(msg_addr, 0, options, args->timeout, sync_send,
                               args->rcv_size, 0, args->rcv_name);
  }
 ...
 *      All of mach_msg_send and mach_msg_receive error codes.
 */
mach_msg_return_t mach_msg_trap(struct mach_msg_overwrite_trap_args *args) {
 ...
 *      All of mach_msg_send and mach_msg_receive error codes.
 */
mach_msg_return_t mach_msg2_trap(struct mach_msg2_trap_args *args) {
  ...
  option64 =
      ipc_current_msg_options(current_task(), args->options) | MACH64_MACH_MSG2;
  ...
  if (option64 & MACH64_MSG_VECTOR) {
    ...
    mr = mach_msg_copyin_data_vectors(args->data,
                                      MAX(vec_snd_count, vec_rcv_count),
                                      option64, &msg_vec, &aux_vec);
    if (mr != MACH_MSG_SUCCESS) {
      goto send_fail;
    }
  }
  ...
  if (option64 & MACH64_SEND_MSG) {
    ...
    if (option64 & MACH64_MSG_VECTOR) {
       ...
       */
      mr = mach_msg_validate_data_vectors(&msg_vec, &aux_vec, vec_snd_count,
...
send_fail:
  ...
  if (option64 & MACH64_RCV_MSG) {
    ...
    if (option64 & MACH64_MSG_VECTOR) {
      ...
      mr = mach_msg_validate_data_vectors(&msg_vec, &aux_vec, vec_rcv_count,
    ...
    mr = mach_msg_trap_receive(msg_addr, aux_addr, option64, msg_timeout,
                               sync_send, msg_size, aux_size, rcv_name);
  }
...
mach_msg_rcv_link_special_reply_port(ipc_port_t special_reply_port,
  ...
  if (kr == KERN_SUCCESS) {
     ...
     */
    ipc_port_link_special_reply_port(special_reply_port, dest_port, FALSE);
...
void mach_msg_receive_results_complete(ipc_object_t object) {
   ...
   */
  if (!((self->ith_state == MACH_RCV_TOO_LARGE &&
         self->ith_option & MACH_RCV_LARGE) || // msg was too large and the next
                                               // receive will get it
  ...
  if (ip_is_special_reply_port(port) || get_turnstile) {
    ip_mq_lock(port);
    ipc_port_adjust_special_reply_port_locked(port, NULL, flags, get_turnstile);
    /* port unlocked */
  }
  assert(self->turnstile != TURNSTILE_NULL);
...
SECURITY_READ_ONLY_LATE(struct mach_msg_filter_callbacks)
mach_msg_filter_callbacks;

kern_return_t mach_msg_filter_register_callback(
  ...
  if (callbacks->version >= MACH_MSG_FILTER_CALLBACKS_VERSION_0) {
    ...
    size =
        offsetof(struct mach_msg_filter_callbacks, alloc_service_port_sblabel);
  }
...
/* This function should only be called if the task and port allow message
 * filtering */
boolean_t mach_msg_fetch_filter_policy(void *port_label, mach_msg_id_t msgh_id,
  ...
  ret = mach_msg_fetch_filter_policy_callback(current_task(), port_label,
                                              msgh_id, fid);
