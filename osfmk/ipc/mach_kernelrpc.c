...
int _kernelrpc_mach_vm_map_trap(struct _kernelrpc_mach_vm_map_trap_args *args) {
  task_t task = port_name_to_current_task_noref(args->target);
  mach_vm_offset_t addr;
  int rv = MACH_SEND_INVALID_DEST;
  ...
  rv = mach_vm_map_external(
      task->map, &addr, args->size, args->mask, args->flags, IPC_PORT_NULL, 0,
      FALSE, args->cur_protection, VM_PROT_ALL, VM_INHERIT_DEFAULT);
  if (rv == KERN_SUCCESS) {
    rv = mach_copyout(&addr, args->addr, sizeof(addr));
  }
...
int _kernelrpc_mach_vm_purgable_control_trap(
  ...
  rv =
      mach_vm_purgable_control(task->map, args->address, args->control, &state);
  if (rv == KERN_SUCCESS) {
...
int _kernelrpc_mach_port_mod_refs_trap(
  ...
  if (task) {
    rv = mach_port_mod_refs(task->itk_space, args->name, args->right,
                            args->delta);
  }
...
int _kernelrpc_mach_port_insert_right_trap(
  ...
  rv = ipc_object_copyin(task->itk_space, args->poly, args->polyPoly,
                         ...
                         IPC_COPYIN_KERNEL_DESTINATION, NULL, &port);
  if (rv != KERN_SUCCESS) {
    goto done;
  }
...
#define MACH_PORT_INFO_OUT                                                     \
  (((__Reply__mach_port_get_attributes_from_user_t *)NULL)->port_info_out)
#define MACH_PORT_INFO_STACK_LIMIT                                             \
  80 // current size is 68 == 17 * sizeof(integer_t)
  _Static_assert(
      sizeof(MACH_PORT_INFO_OUT) < MACH_PORT_INFO_STACK_LIMIT,
      "mach_port_info_t has grown significantly, reevaluate stack usage");
  const mach_msg_type_number_t max_count =
      (sizeof(MACH_PORT_INFO_OUT) / sizeof(MACH_PORT_INFO_OUT[0]));
  ...
  if ((rv =
           mach_copyin(CAST_USER_ADDR_T(args->count), &count, sizeof(count)))) {
    goto done;
  }
  if (count > max_count) {
    count = max_count;
  ...
  rv = mach_port_get_attributes(task->itk_space, args->name, args->flavor, info,
                                &count);
  if (rv == KERN_SUCCESS) {
    rv = mach_copyout(&count, CAST_USER_ADDR_T(args->count), sizeof(count));
  }
  if (rv == KERN_SUCCESS && count > 0) {
    rv = mach_copyout(info, CAST_USER_ADDR_T(args->info),
                      count * sizeof(info[0]));
  }
...
int _kernelrpc_mach_port_destruct_trap(
  ...
  if (task) {
    rv = mach_port_destruct(task->itk_space, args->name, args->srdelta,
                            args->guard);
  }
...
int _kernelrpc_mach_port_guard_trap(
  ...
  if (task) {
    rv =
        mach_port_guard(task->itk_space, args->name, args->guard, args->strict);
  }
...
int _kernelrpc_mach_port_request_notification_trap(
  ...
  if (MACH_PORT_VALID(args->notify)) {
    rv = ipc_object_copyin(task->itk_space, args->notify, args->notifyPoly,
                           ...
                           IPC_COPYIN_KERNEL_DESTINATION, NULL, &notify);
  } else {
  ...
  rv = mach_port_request_notification(task->itk_space, args->name, args->msgid,
                                      args->sync, notify, &previous);
  if (rv != KERN_SUCCESS) {
    if (IP_VALID(notify)) {
  ...
  if (IP_VALID(previous)) {
    ...
    rv = ipc_object_copyout(
        task->itk_space, previous, MACH_MSG_TYPE_PORT_SEND_ONCE,
        IPC_OBJECT_COPYOUT_FLAGS_NONE, NULL, &previous_name);
    if (rv != KERN_SUCCESS) {
      goto done;
    }
  }
...
host_create_mach_voucher_trap(struct host_create_mach_voucher_args *args) {
  ...
  if ((kr = mach_copyin(CAST_USER_ADDR_T(args->recipes), (void *)krecipes,
                        args->recipes_size))) {
    goto done;
  }

  kr = host_create_mach_voucher(host, krecipes, args->recipes_size,
                                &new_voucher);
  if (kr != KERN_SUCCESS) {
    goto done;
  }
...
kern_return_t mach_voucher_extract_attr_recipe_trap(
  ...
  if ((kr = mach_copyin(CAST_USER_ADDR_T(args->recipe), (void *)krecipe,
                        max_sz))) {
    goto done;
  }

  kr = mach_voucher_extract_attr_recipe(
      voucher, args->key, (mach_voucher_attr_raw_recipe_t)krecipe, &sz);
  assert(sz <= max_sz);
 ...
 *     KERN_NO_SPACE: User allocated memory for port names copyout is
 * insufficient.
 *
 *     Other error code see task_info().
 */
kern_return_t task_dyld_process_info_notify_get_trap(
  ...
  kr = task_info(current_task(), TASK_DYLD_INFO, (task_info_t)&dyld_info,
                 &info_count);
  if (kr) {
    return kr;
  }
...
copyout_failed:
  ...
  for (int i = 0; i < copyout_count; i++) {
    ...
    if (entry->ie_port == sright ||
      ...
      (void)ipc_right_dealloc(current_space(), copyout_names[i],
                              entry); /* unlocks space */
    } else {
...
#if __LP64__
mach_error_t mach_vm_reclaim_update_kernel_accounting_trap(
  ...
  err = vm_deferred_reclamation_update_accounting_internal(task,
                                                           &bytes_reclaimed);
  if (!err) {
    mach_copyout(&bytes_reclaimed, (user_addr_t)args->bytes_reclaimed_out,
                 sizeof(bytes_reclaimed));
  }
  return err;
