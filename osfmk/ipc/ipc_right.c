 ...
 */
kern_return_t ipc_right_lookup_read(ipc_space_t space, mach_port_name_t name,
...
kern_return_t ipc_right_lookup_write(ipc_space_t space, mach_port_name_t name,
                                     ipc_entry_t *entryp) {
  ipc_entry_t entry;
...
kern_return_t ipc_right_lookup_two_write(ipc_space_t space,
...
bool ipc_right_reverse(ipc_space_t space, ipc_port_t port,
...
static inline ipc_port_t ipc_right_request_cancel(ipc_port_t port,
                                                  mach_port_name_t name,
                                                  ipc_entry_t entry) {
...
kern_return_t ipc_right_request_alloc(ipc_space_t space, mach_port_name_t name,
                                      ipc_port_request_opts_t options,
                                      ipc_port_t notify, mach_msg_id_t id,
  ...
  for (;;) {
    ...
    if (entry->ie_bits & MACH_PORT_TYPE_PORT_RIGHTS) {
      ...
      if (!ipc_right_check(space, port, name, entry, IPC_COPYIN_REASON_NONE)) {
        ...
        if ((entry->ie_bits & MACH_PORT_TYPE_SEND_ONCE) &&
          ...
          is_write_unlock(space);
          *previousp = IP_NULL;
          mach_port_guard_exception(ip_type(port), id,
...
#endif /* IMPORTANCE_INHERITANCE */
        kr = ipc_port_request_alloc(port, name, notify, options, &new_request);
...
#if IMPORTANCE_INHERITANCE
        if (will_arm && port->ip_impdonation != 0 &&
            port->ip_spimportant == 0 &&
            task_is_importance_donor(current_task())) {
          if (ipc_port_importance_delta(port, IPID_OPTION_SENDPOSSIBLE, 1) ==
              FALSE) {
            ip_mq_unlock(port);
          }
        } else
    ...
    kr = (entry->ie_bits & MACH_PORT_TYPE_PORT_OR_DEAD) ? KERN_INVALID_ARGUMENT
                                                        : KERN_INVALID_RIGHT;
...
bool ipc_right_inuse(ipc_entry_t entry) {
...
bool ipc_right_check(ipc_space_t space, ipc_port_t port, mach_port_name_t name,
                     ipc_entry_t entry, ipc_copyin_op_t copyin_reason) {
  ...
  ip_mq_lock(port);
  if (ip_active(port) || ((copyin_reason == IPC_COPYIN_KMSG_DESTINATION) &&
                          entry->ie_request == IE_REQ_NONE &&
                          (entry->ie_bits & MACH_PORT_TYPE_SEND_ONCE))) {
    return false;
  }
...
void ipc_right_terminate(ipc_space_t space, mach_port_name_t name,
                         ipc_entry_t entry) {
...
kern_return_t ipc_right_destroy(ipc_space_t space, mach_port_name_t name,
                                ipc_entry_t entry) {
  ipc_entry_bits_t bits;
...
kern_return_t ipc_right_dealloc(ipc_space_t space, mach_port_name_t name,
                                ipc_entry_t entry) {
  ipc_port_t port = IP_NULL;
  ipc_entry_bits_t bits;
  mach_port_type_t type;
  ...
  default:
    ...
    mach_port_guard_exception(
        name, MPG_PAYLOAD(MPG_FLAGS_INVALID_RIGHT_DEALLOC, bits),
...
kern_return_t ipc_right_delta(ipc_space_t space, mach_port_name_t name,
                              ipc_entry_t entry, mach_port_right_t right,
  ...
  case MACH_PORT_RIGHT_RECEIVE: {
    ...
    if (bits & MACH_PORT_TYPE_SEND) {
      ...
      if (ipc_port_has_prdrequest(port)) {
         ...
         */
        bits &=
            ~(IE_BITS_TYPE_MASK | IE_BITS_PINNED_SEND | IE_BITS_IMMOVABLE_SEND);
  ...
  case MACH_PORT_RIGHT_SEND: {
    ...
    if ((bits & MACH_PORT_TYPE_SEND) == 0) {
      ...
      if ((bits & MACH_PORT_TYPE_DEAD_NAME) == 0
          ...
          /*
           * AE tries to add single send right without knowing if it already
           * owns one. But if it doesn't, it should own the receive right and
           * delta should be 1.
           */
  ...
  default:
    panic("ipc_right_delta: strange right %d for 0x%x (%p) in space:%p", right,
          name, (void *)entry, (void *)space);
  }
...
invalid_right:
  ...
  if (throw_exception) {
    mach_port_guard_exception(
        name, MPG_PAYLOAD(MPG_FLAGS_INVALID_RIGHT_DELTA, right, bits),
        kGUARD_EXC_INVALID_RIGHT);
  }
  return KERN_INVALID_RIGHT;
...
invalid_value:
  ...
  mach_port_guard_exception(name,
                            MPG_PAYLOAD(MPG_FLAGS_INVALID_VALUE_DELTA, right,
                                        (uint16_t)delta, IE_BITS_UREFS(bits)),
...
kern_return_t ipc_right_destruct(ipc_space_t space, mach_port_name_t name,
                                 ipc_entry_t entry, mach_port_delta_t srdelta,
                                 uint64_t guard) {
  ipc_port_t port = IP_NULL;
  ipc_entry_bits_t bits;
  ...
  if ((bits & MACH_PORT_TYPE_RECEIVE) == 0) {
    ...
    if ((bits & IE_BITS_EX_RECEIVE) == 0) {
      mach_port_guard_exception(
          name, MPG_PAYLOAD(MPG_FLAGS_INVALID_RIGHT_DESTRUCT, bits),
          kGUARD_EXC_INVALID_RIGHT);
    }
    return KERN_INVALID_RIGHT;
  }
...
invalid_value:
  is_write_unlock(space);
  mach_port_guard_exception(name,
                            MPG_PAYLOAD(MPG_FLAGS_INVALID_VALUE_DESTRUCT,
                                        srdelta, IE_BITS_UREFS(bits)),
                            kGUARD_EXC_INVALID_VALUE);
  return KERN_INVALID_VALUE;
}
...
kern_return_t ipc_right_info(ipc_space_t space, mach_port_name_t name,
                             ipc_entry_t entry, mach_port_type_t *typep,
 ...
 *		by ipc_kmsg_copyin_header to check if reply_port can be copied
 * in. If the reply port is an immovable send right, it errors out. Conditions:
 *		The space is locked (read or write) and active.
 */

bool ipc_right_copyin_check_reply(__assert_only ipc_space_t space,
  ...
  case MACH_MSG_TYPE_MOVE_SEND:
    if ((reply_bits & IE_BITS_PINNED_SEND) && ip_active(reply_port) &&
...
ipc_right_copyin_check_guard_locked(ipc_port_t port, mach_port_name_t name,
  ...
  if ((flags & MACH_MSG_GUARD_FLAGS_UNGUARDED_ON_SEND) && !port->ip_guarded &&
      !context) {
    return KERN_SUCCESS;
...
void ipc_right_copyin_rcleanup_init(ipc_copyin_rcleanup_t *icrc,
                                    mach_msg_guarded_port_descriptor_t *gdesc) {
...
void ipc_right_copyin_cleanup_destroy(ipc_copyin_cleanup_t *icc,
                                      mach_port_name_t name) {
...
void ipc_right_copyin_rcleanup_destroy(ipc_copyin_rcleanup_t *icrc) {
#if IMPORTANCE_INHERITANCE
...
kern_return_t ipc_right_copyin(ipc_space_t space, mach_port_name_t name,
                               ...
                               ipc_copyin_op_t copyin_reason, ipc_entry_t entry,
                               ipc_port_t *portp, ipc_copyin_cleanup_t *icc,
  ...
  bool allow_imm_send =
      !!(flags & IPC_OBJECT_COPYIN_FLAGS_ALLOW_IMMOVABLE_SEND);
  ...
  *portp = IP_NULL;
  ...
  if (IP_VALID(port) && ip_is_reply_port(port) &&
    ...
    mach_port_guard_exception(
        name, MPG_PAYLOAD(MPG_FLAGS_NONE, ip_type(port), msgt_name),
  ...
  case MACH_MSG_TYPE_MOVE_RECEIVE: {
    ...
    if (icrc->icrc_guarded_desc) {
      ...
      icrc->icrc_guarded_desc->flags |= MACH_MSG_GUARD_FLAGS_UNGUARDED_ON_SEND;
    }
  ...
  case MACH_MSG_TYPE_MOVE_SEND: {
    ...
    if ((bits & MACH_PORT_TYPE_SEND) == 0 || IE_BITS_UREFS(bits) < moves) {
      ip_mq_unlock(port);
      goto invalid_right;
    }
  ...
  case MACH_MSG_TYPE_MOVE_SEND_ONCE: {
    ...
    if (!allow_imm_send && (entry->ie_bits & IE_BITS_IMMOVABLE_SEND)) {
      ip_mq_unlock(port);
      mach_port_guard_exception_immovable(space, name, port,
                                          MACH_MSG_TYPE_MOVE_SEND_ONCE, entry);
      return KERN_INVALID_CAPABILITY;
    }
...
void ipc_right_copyout_any_send(ipc_space_t space, ipc_port_t port,
                                mach_msg_type_name_t msgt_name,
  ...
  case MACH_MSG_TYPE_PORT_SEND_ONCE:
    ...
    if (ip_is_special_reply_port(port)) {
      ipc_port_adjust_special_reply_port_locked(
          port, current_thread()->ith_knote, IPC_PORT_ADJUST_SR_LINK_WORKLOOP,
          FALSE);
...
void ipc_right_copyout_recv_and_unlock_space(
  ...
  if (ip_is_any_service_port_type(label->io_type) &&
    ...
    if (!task_is_in_privileged_coalition(current_task(),
  ...
  dest = ipc_port_mark_in_space(port, label, space, name,
                                (kn == ITH_KNOTE_PSEUDO) ? IO_STATE_IN_SPACE
                                                         : in_space);
