...
/* is_telemetry flags */
__options_decl(
    is_telemetry_t, uint8_t,
    {
        IS_HAS_BOOTSTRAP_PORT_TELEMETRY =
            0x01, /* space has emitted a bootstrap port telemetry */
        IS_HAS_CREATE_PRP_TELEMETRY =
            0x02, /* space has emitted a create provisional reply port telemetry
                   */
        IS_HAS_SERVICE_PORT_TELEMETRY =
            0x04, /* space has emitted a service port telemetry */
        IS_HAS_MOVE_PRP_TELEMETRY = 0x08, /* space has emitted a move
                                             provisional reply port telemetry */
    });
...
struct ipc_space {
  ...
  SMR_POINTER(ipc_entry_table_t XNU_PTRAUTH_SIGNED_PTR("ipc_space.is_table"))
  is_table; /* an array of entries */
  task_t
      XNU_PTRAUTH_SIGNED_PTR("ipc_space.is_task") is_task; /* associated task */
...
#if CONFIG_PROC_RESOURCE_LIMITS
  ipc_entry_num_t is_table_size_soft_limit; /* resource_notify is sent when the
                                               table size hits this limit */
  ipc_entry_num_t
      is_table_size_hard_limit; /* same as soft limit except the task is killed
                                   soon after data collection */
#endif                          /* CONFIG_PROC_RESOURCE_LIMITS */
  _Atomic is_telemetry_t
      is_telemetry; /* rate limit each type of telemetry to once per space */
};

#define IS_NULL ((ipc_space_t)0)
#define IS_INSPECT_NULL ((ipc_space_inspect_t)0)

static inline uintptr_t ipc_space_policy_discriminator(ipc_space_t is) {
  uint16_t base = ptrauth_string_discriminator("ipc_space.is_policy");

  return ptrauth_blend_discriminator(&is->is_policy, base);
}

static inline ipc_space_policy_t ipc_space_policy(ipc_space_t is) {
  ...
  return (ipc_space_policy_t)(unsigned long)ptrauth_auth_data(
      ...
      ptrauth_key_process_independent_data, ipc_space_policy_discriminator(is));
}

static inline void ipc_space_set_policy(ipc_space_t is,
                                        ipc_space_policy_t policy) {
  is->is_policy = (unsigned long)ptrauth_sign_unauthenticated(
      (void *)(unsigned long)policy, ptrauth_key_process_independent_data,
      ipc_space_policy_discriminator(is));
}

static inline bool is_bits_set(ipc_space_t is, uint32_t bit) {
  return (os_atomic_or_orig(&is->is_bits, bit, relaxed) & bit) == 0;
}

static inline bool is_bits_clear(ipc_space_t is, uint32_t bit) {
  return (os_atomic_andnot_orig(&is->is_bits, bit, relaxed) & bit) != 0;
}

static inline bool is_bits_test(ipc_space_t is, uint32_t bit) {
...
static inline ipc_entry_table_t is_active_table(ipc_space_t space) {
...
#if CONFIG_PROC_RESOURCE_LIMITS
#define is_above_soft_limit_notify(is)                                         \
  is_bits_test(is, IS_ABOVE_SOFT_LIMIT_NOTIFY)
#define is_above_hard_limit_notify(is)                                         \
  is_bits_test(is, IS_ABOVE_HARD_LIMIT_NOTIFY)

static inline void is_above_soft_limit_send_notification(ipc_space_t is) {
  is_bits_set(is, IS_ABOVE_SOFT_LIMIT_NOTIFY);
}

static inline void is_above_hard_limit_send_notification(ipc_space_t is) {
  is_bits_set(is, IS_ABOVE_HARD_LIMIT_NOTIFY);
}

#define is_soft_limit_already_notified(is)                                     \
  is_bits_test(is, IS_SOFT_LIMIT_NOTIFIED)
#define is_hard_limit_already_notified(is)                                     \
  is_bits_test(is, IS_HARD_LIMIT_NOTIFIED)

static inline void is_soft_limit_notified(ipc_space_t is) {
  is_bits_set(is, IS_SOFT_LIMIT_NOTIFIED);
}

static inline void is_hard_limit_notified(ipc_space_t is) {
...
#define is_at_max_limit_already_notified(is)                                   \
  is_bits_test(is, IS_AT_MAX_LIMIT_NOTIFIED)

static inline void is_at_max_limit_send_notification(ipc_space_t is) {
  is_bits_set(is, IS_AT_MAX_LIMIT_NOTIFY);
}

static inline void is_at_max_limit_notified(ipc_space_t is) {
...
extern void ipc_space_lock(ipc_space_t space);

extern void ipc_space_unlock(ipc_space_t space);

extern void ipc_space_lock_sleep(ipc_space_t space);

extern void ipc_space_retire_table(ipc_entry_table_t table);

/* Create a special IPC space */
extern ipc_space_t ipc_space_create_special(void);

/* Create a new IPC space */
extern kern_return_t ipc_space_create(ipc_label_t label, ipc_space_t *spacep);

/* Change the label on an existing space */
extern kern_return_t ipc_space_label(ipc_space_t space, ipc_label_t label);

/* Add a label to an existing space */
extern kern_return_t ipc_space_add_label(ipc_space_t space, ipc_label_t label);

/* Mark a space as dead and cleans up the entries*/
extern void ipc_space_terminate(ipc_space_t space);

/* Permute the order of a range within an IPC space */
extern void ipc_space_rand_freelist(ipc_space_t space, ipc_entry_t table,
...
ipc_space_set_table_size_limits(ipc_space_t space, ipc_entry_num_t soft_limit,
                                ipc_entry_num_t hard_limit);

extern void ipc_space_check_limit_exceeded(ipc_space_t space);
#endif /* CONFIG_PROC_RESOURCE_LIMITS */

extern bool ipc_space_check_table_size_limit(ipc_space_t space,
                                             ipc_entry_num_t *current_limit,
                                             ipc_entry_num_t *soft_limit,
                                             ipc_entry_num_t *hard_limit);

extern void ipc_space_set_at_max_limit(ipc_space_t space);
