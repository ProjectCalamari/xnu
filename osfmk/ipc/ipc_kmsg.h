...
__enum_decl(
    ...
    {/*
      ...
      * and message data, trailer, and aux are in buffer pointed to by
      * ikm_udata. mach_msg_header_t is immediately after the kmsg header.
      */
...
__options_closed_decl(
    ipc_kmsg_keep_alive_t, uint8_t,
    {
        IKM_KEEP_ALIVE_NONE = 0x0, /* keep-alive not used on this message     */
        IKM_KEEP_ALIVE_OWNED =
            0x1, /* keep-alive used, owned by the subsystem */
        IKM_KEEP_ALIVE_IN_USE = 0x2, /* keep-alive used, message is in flight */
    });

struct ipc_kmsg {
  queue_chain_t ikm_link;
  ipc_port_t XNU_PTRAUTH_SIGNED_PTR("kmsg.ikm_voucher_port")
      ikm_voucher_port;                       /* voucher port carried */
  ...
  mach_msg_type_name_t ikm_voucher_type
      : 6; /* disposition type the voucher came in with */
...
static_assert(offsetof(struct ipc_kmsg, ikm_big_data) + IKM_BIG_MSG_SIZE ==
              IKM_ALLOC_SIZE);
static_assert(offsetof(struct ipc_kmsg, ikm_small_data) + IKM_SMALL_MSG_SIZE +
                  2 * sizeof(void *) + 2 * sizeof(mach_msg_size_t) ==
              IKM_ALLOC_SIZE);
...
#define ipc_kmsg_rmqueue(queue, kmsg) circle_dequeue(queue, &(kmsg)->ikm_link)

extern bool ipc_kmsg_enqueue_qos(ipc_kmsg_queue_t queue, ipc_kmsg_t kmsg);

extern bool ipc_kmsg_override_qos(ipc_kmsg_queue_t queue, ipc_kmsg_t kmsg,
                                  mach_msg_qos_t qos_ovr);

/* Pull the (given) first kmsg out of a queue */
extern void ipc_kmsg_rmqueue_first(ipc_kmsg_queue_t queue, ipc_kmsg_t kmsg);
...
__options_decl(ipc_kmsg_alloc_flags_t, uint32_t,
               {
                   ...
                   IPC_KMSG_ALLOC_USE_KEEP_ALIVE =
                       0x0020, /* must call ipc_kmsg_keep_alive_abandon () */
               });

/* Allocate a kernel message */
extern ipc_kmsg_t ipc_kmsg_alloc(mach_msg_size_t msg_size,
...
extern void
ipc_kmsg_clean_descriptors(mach_msg_kdescriptor_t *kdesc __counted_by(number),
                           mach_msg_type_number_t number);

extern void ipc_kmsg_sign_descriptors(mach_msg_kdescriptor_t *kdesc,
...
extern void ipc_kmsg_destroy(ipc_kmsg_t kmsg, ipc_kmsg_destroy_flags_t flags);

/* Enqueue kernel message for deferred destruction */
extern bool ipc_kmsg_delayed_destroy(ipc_kmsg_t kmsg);

/* Enqueue queue of kernel messages for deferred destruction */
extern bool ipc_kmsg_delayed_destroy_queue(ipc_kmsg_queue_t queue);
...
extern bool ipc_kmsg_keep_alive_try_reusing(ipc_kmsg_t kmsg);

/* Abandons a message that was allocated with IPC_KMSG_ALLOC_USE_KEEP_ALIVE. */
extern void ipc_kmsg_keep_alive_abandon(ipc_kmsg_t kmsg);

/* get the unshifted message header of a kmsg */
extern mach_msg_header_t *ikm_header(ipc_kmsg_t kmsg);

/* get the start address of user data (after the last descriptor) for a kmsg */
extern void *ikm_udata(ipc_kmsg_t kmsg, mach_msg_size_t desc_count,
                       bool complex);
...
extern mach_msg_return_t ipc_kmsg_get_from_kernel(mach_msg_header_t *msg,
                                                  mach_msg_size_t size,
                                                  mach_msg_option64_t options,
                                                  ipc_kmsg_t *kmsgp);

/* Send a message to a port */
extern mach_msg_return_t ipc_kmsg_send(ipc_kmsg_t kmsg,
                                       mach_msg_option64_t options,
                                       mach_msg_timeout_t timeout_val);

/* Copy a kernel message buffer to a user message */
extern mach_msg_return_t
ipc_kmsg_put_to_user(ipc_kmsg_t kmsg, /* scalar or vector */
                     mach_msg_recv_bufs_t *recv_bufs,
                     mach_msg_recv_result_t *msgr, mach_msg_option64_t option,
                     vm_map_t map, mach_msg_return_t mr);

/* Copy a kernel message buffer to a kernel message */
extern void ipc_kmsg_put_to_kernel(mach_msg_header_t *msg,
                                   mach_msg_option64_t options, ipc_kmsg_t kmsg,
                                   mach_msg_size_t size);

/* Copyin data, port rights and out-of-line memory from a user message */
extern mach_msg_return_t ipc_kmsg_copyin_from_user(
    ipc_kmsg_t kmsg, mach_msg_send_uctx_t *send_uctx, ipc_space_t space,
    vm_map_t map, mach_msg_priority_t priority, mach_msg_option64_t *optionp);

/* Copyin port rights and out-of-line memory from a kernel message */
extern mach_msg_return_t ipc_kmsg_copyin_from_kernel(ipc_kmsg_t kmsg);

/* Copyout the header and body to a user message */
extern mach_msg_return_t ipc_kmsg_copyout(ipc_kmsg_t kmsg, ipc_space_t space,
                                          vm_map_t map,
...
extern mach_msg_size_t ipc_kmsg_copyout_size(ipc_kmsg_t kmsg, vm_map_t map);

/* Copyout the destination port in the message */
extern void ipc_kmsg_copyout_dest_to_user(ipc_kmsg_t kmsg, ipc_space_t space);

/* kernel's version of ipc_kmsg_copyout_dest_to_user */
extern void ipc_kmsg_copyout_dest_to_kernel(ipc_kmsg_t kmsg, ipc_space_t space);

/* Returns a pointer to a thread group in the kmsg if any. Caller has a
 * reference to the kmsg */
extern struct thread_group *ipc_kmsg_get_thread_group(ipc_kmsg_t kmsg);

extern mach_msg_trailer_size_t ipc_kmsg_trailer_size(mach_msg_option64_t option,
                                                     vm_map_t map);

extern mach_msg_max_trailer_t *ipc_kmsg_get_trailer(ipc_kmsg_t kmsg);

extern void ipc_kmsg_set_voucher_port(ipc_kmsg_t kmsg, ipc_port_t voucher,
                                      mach_msg_type_name_t type);

extern ipc_port_t ipc_kmsg_get_voucher_port(ipc_kmsg_t kmsg);

extern void ipc_kmsg_clear_voucher_port(ipc_kmsg_t kmsg);

/* checks signature and returns descriptor count */
extern mach_msg_size_t
ipc_kmsg_validate_signature(ipc_kmsg_t kmsg) __result_use_check;

#if (KDEBUG_LEVEL >= KDEBUG_LEVEL_STANDARD)
extern void ipc_kmsg_trace_send(ipc_kmsg_t kmsg, mach_msg_option64_t option);
#else
#define ipc_kmsg_trace_send(a, b)                                              \
  do {                                                                         \
  } while (0)
#endif
