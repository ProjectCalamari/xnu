...
/*!
 * @brief
 * Convert mach_msg policy options (originally derived from the current_task
 * space) back into the space namespace
 *
 * @param opts      the options to convert
 *
 * @return          the options for the space
 */
extern ipc_space_policy_t
 ...
 */
extern ipc_space_policy_t ipc_policy_for_task(task_t task);
 ...
 */
extern mach_msg_option64_t
ipc_current_msg_options(task_t task, mach_msg_option64_t user_flags);
 ...
 */
extern mach_msg_return_t ipc_preflight_msg_option64(mach_msg_option64_t opts);
 ...
 *
 * @param current_policy        the policy level for the task/space that we are
 * enforcing on
 * @param requested_level       the policy level that is required to be opted
 * into this enforcement
 *
 * @returns
 ...
 * - false      otherwise
 */
extern bool ipc_should_apply_policy(const ipc_space_policy_t current_policy,
...
ipc_policy_allow_legacy_send_trap(mach_msg_id_t msgid,
...
typedef const struct ipc_object_policy {
  ...
  /** whether the port requires incoming messages to use an IOT_REPLY_PORT
   * properly */
  ...
  /** required entitlement for platform restrictions binaries to create this
   * port */
  const char *pol_construct_entitlement;

  /** see iko_op_no_senders */
  void (*pol_kobject_no_senders)(ipc_port_t port, mach_port_mscount_t mscount);

  /** destroys the label for this port */
  void (*pol_label_free)(ipc_object_label_t label);
} *ipc_object_policy_t;
...
__attribute__((overloadable, always_inline,
               const)) static inline ipc_object_policy_t
 ...
 */
__enum_closed_decl(
    ipc_policy_violation_id_t, uint8_t,
    {
        ...
        IPCPV_REPLY_PORT_SEMANTICS, /* 1, normal reply port semantics violator
                                     */
        /* [2-5] were previously used; should be avoided to avoid telemetry
           confusion */
        ...
        /* services opted out of reply port semantics previously should have
           fixed their violations */
 ...
 */
extern void
ipc_stash_policy_violations_telemetry(ipc_policy_violation_id_t violation_id,
                                      ipc_port_t service_port, int aux_data);
 ...
 */
extern mach_msg_return_t
ipc_validate_kmsg_header_schema_from_user(mach_msg_user_header_t *hdr,
 ...
 * @param send_uctx     the IPC kmsg send context for the current send
 * operation.
 * @param opts          the mach_msg() options, after sanitization
 *                      via @c ipc_current_msg_options().
 *
 ...
 */
static inline void ipc_port_label_free(ipc_object_label_t label) {
 ...
 */
extern bool ipc_should_mark_immovable_send(task_t task, ipc_port_t port,
                                           ipc_object_label_t label);

/*!
 * @brief
 * Determine whether we need to protect this port from being stashed as a naked
 * send right in the kernel. We disallow this if the port is supposed to be
 * immovable send as this would allow userspace to bypass the immovable send
 * checks and move the send right to another process.
 *
 ...
 */
extern bool ipc_can_stash_naked_send(ipc_port_t port);
 ...
 * @param object        The *initialized* port/portset object that is getting a
 * new entry
 * @param type          The type of this entry (send, send-once, receive,
 * deadname, portset)
 ...
 */
extern void ipc_entry_init(ipc_space_t space, ipc_object_t object,
 ...
 */
extern bool ipc_move_receive_allowed(ipc_space_t space, ipc_port_t port,
 ...
 */
__cold extern void mach_port_guard_exception(uint32_t target, uint64_t payload,
                                             unsigned reason);
 ...
 */
__cold extern void
mach_port_guard_exception_immovable(ipc_space_t space, mach_port_name_t name,
                                    mach_port_t port, mach_msg_type_name_t disp,
                                    ipc_entry_t entry);
 ...
 * @param name          The name of the entry in @c space violating pinned
 * rules.
 * @param payload       A valid @c MPG_FLAGS_MOD_REFS_PINNED_* value.
 */
__cold extern void mach_port_guard_exception_pinned(ipc_space_t space,
                                                    mach_port_name_t name,
                                                    uint64_t payload);
 ...
 */
extern bool ipc_is_valid_exception_port(task_t task, ipc_port_t port);
 ...
 */
extern kern_return_t ipc_allow_register_pd_notification(ipc_port_t pd_port,
