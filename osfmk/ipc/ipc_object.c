...
__attribute__((noinline)) static void ipc_object_free(ipc_object_t object) {
...
ipc_object_free_safe(ipc_object_t object) {
  ...
  mpsc_daemon_enqueue(&ipc_object_deallocate_queue, &wq->waitq_defer,
                      MPSC_QUEUE_NONE);
}
...
void ipc_object_deallocate_register_queue(void) {
...
void ipc_object_reference(ipc_object_t io) {
...
void ipc_object_release(ipc_object_t io) {
#if DEBUG
...
void ipc_object_release_safe(ipc_object_t io) {
  if (os_ref_release_raw(&io->io_references, NULL) == 0) {
...
void ipc_object_release_live(ipc_object_t io) {
 ...
 */
kern_return_t ipc_object_translate(ipc_space_t space, mach_port_name_t name,
...
kern_return_t ipc_object_translate_port_pset(ipc_space_t space,
  ...
  kr = ipc_right_lookup_two_read(space, port_name, &port_entry, pset_name,
                                 &pset_entry);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
...
kern_return_t ipc_object_alloc_dead(ipc_space_t space,
...
kern_return_t ipc_object_alloc_entry(ipc_space_t space, ipc_object_t object,
                                     mach_port_name_t *namep,
                                     ipc_entry_t *entry) {
  kern_return_t kr;
 ...
 *	Purpose:
 *		Allocate an uninitalized entry for a new object, with a specific
 * name. That entry must later be initialized by `ipc_entry_init` after the
 * object has been initialized Conditions: Nothing locked. The space is write
...
kern_return_t ipc_object_alloc_entry_with_name(ipc_space_t space,
...
void ipc_object_validate(ipc_object_t object, ipc_object_type_t type) {
...
mach_msg_type_name_t ipc_object_copyin_type(mach_msg_type_name_t msgt_name) {
  switch (msgt_name) {
...
kern_return_t ipc_object_copyin(ipc_space_t space, mach_port_name_t name,
  ...
  ipc_object_copyin_flags_t copyin_mask =
  ...
  if (IP_VALID(*portp) && ip_type(*portp) == IOT_SERVICE_PORT &&
    ...
    ipc_stash_policy_violations_telemetry(IPCPV_MOVE_SERVICE_PORT, *portp,
                                          name);
  }
...
void ipc_object_copyin_from_kernel(ipc_port_t port,
                                   mach_msg_type_name_t msgt_name) {
...
void ipc_object_destroy(ipc_port_t port, mach_msg_type_name_t msgt_name) {
  assert(IP_VALID(port));
...
void ipc_object_destroy_dest(ipc_port_t port, mach_msg_type_name_t msgt_name) {
  assert(IP_VALID(port));
 ...
 */
kern_return_t ipc_object_insert_send_right(ipc_space_t space,
                                           mach_port_name_t name,
                                           mach_msg_type_name_t msgt_name) {
  ipc_entry_bits_t bits;
  ipc_object_t object;
...
kern_return_t ipc_object_copyout(ipc_space_t space, ipc_port_t port,
                                 mach_msg_type_name_t msgt_name,
  ...
  for (;;) {
    ...
    if (!io_state_active(label.io_state) ||
        !ip_label_check_or_substitute(space, port, &label, msgt_name,
                                      &port_subst)) {
      ip_mq_unlock_label_put(port, &label);
      is_write_unlock(space);
  ...
  if (msgt_name == MACH_MSG_TYPE_PORT_RECEIVE) {
    ipc_right_copyout_recv_and_unlock_space(space, port, &label, name, entry,
                                            gdesc);
  } else {
    ip_label_put(port, &label);
    ipc_right_copyout_any_send(space, port, msgt_name, flags, name, entry);
    is_write_unlock(space);
  }
...
kern_return_t ipc_object_copyout_name(ipc_space_t space, ipc_port_t port,
                                      mach_msg_type_name_t msgt_name,
                                      mach_port_name_t name) {
  ipc_object_label_t label;
...
void ipc_object_copyout_dest(ipc_space_t space, ipc_port_t port,
                             mach_msg_type_name_t msgt_name,
                             mach_port_name_t *namep) {
  mach_port_name_t name;
...
void ipc_object_unpin(ipc_space_t space, ipc_port_t port) {
  mach_port_name_t name;
  ipc_entry_t entry;
  ...
  if (IP_VALID(port)) {
    ...
    if (is_active(space) && ipc_right_reverse(space, port, &name, &entry) &&
...
__abortlike static void ipc_object_validate_preflight_panic(ipc_object_t io) {
 ...
 *		3. unmapped memory.
 *
 *		(2) is possible because the zone is made with ZC_ZFREE_CLEARMEM
 * which ensures freed elements are always zeroed.
 *
 ...
 */
bool ipc_object_lock_allow_invalid(ipc_object_t orig_io) {
...
__attribute__((always_inline)) void ipc_object_unlock(ipc_object_t object) {
