...
#define USER_HEADER_SIZE_DELTA                                                 \
  (sizeof(mach_msg_header_t) - sizeof(mach_msg_user_header_t))

#define mach_validate_desc_type(t, size) static_assert(sizeof(t) == (size))
...
KALLOC_TYPE_VAR_DEFINE(KT_IPC_KMSG_KDATA_OOL, mach_msg_base_t,
                       mach_msg_kdescriptor_t, KT_DEFAULT);

#pragma mark ipc_kmsg layout and accessors

/* Whether header, body, content and trailer occupy contiguous memory space */
static inline bool ikm_is_linear(ipc_kmsg_t kmsg) {
...
__attribute__((always_inline, overloadable)) static mach_msg_size_t
ikm_kdata_size(mach_msg_size_t dsc_count, bool complex) {
  if (complex) {
...
__attribute__((always_inline, overloadable)) static mach_msg_size_t
ikm_kdata_size(mach_msg_header_t *hdr) {
 ...
 */
void *ikm_udata(ipc_kmsg_t kmsg, mach_msg_size_t dsc_count, bool complex) {
 ...
 */
void *ikm_udata_from_header(ipc_kmsg_t kmsg) {
  if (ikm_is_linear(kmsg)) {
    mach_msg_header_t *hdr = ikm_header(kmsg);
 ...
 */
inline mach_msg_header_t *ikm_header(ipc_kmsg_t kmsg) {
  switch (kmsg->ikm_type) {
  case IKM_TYPE_ALL_INLINED:
...
static inline mach_msg_aux_header_t *ikm_aux_header(ipc_kmsg_t kmsg) {
...
__ipc_kmsg_descriptor_invalid_type_panic(const mach_msg_kdescriptor_t *kdesc) {
  panic("Invalid descriptor type (%p: %d)", kdesc,
        mach_msg_kdescriptor_type(kdesc));
}

mach_msg_trailer_size_t ipc_kmsg_trailer_size(mach_msg_option64_t option,
 ...
 */
mach_msg_max_trailer_t *ipc_kmsg_get_trailer(ipc_kmsg_t kmsg) {
  mach_msg_header_t *hdr = ikm_header(kmsg);
...
void ipc_kmsg_set_voucher_port(ipc_kmsg_t kmsg, ipc_port_t voucher_port,
...
ipc_port_t ipc_kmsg_get_voucher_port(ipc_kmsg_t kmsg) {
  return kmsg->ikm_voucher_port;
}

void ipc_kmsg_clear_voucher_port(ipc_kmsg_t kmsg) {
...
struct thread_group *ipc_kmsg_get_thread_group(ipc_kmsg_t kmsg) {
  struct thread_group *tg = NULL;
  kern_return_t __assert_only kr;

  ipc_voucher_t voucher =
      convert_port_to_voucher(ipc_kmsg_get_voucher_port(kmsg));
...
__abortlike static void __ikm_signature_check_panic(ipc_kmsg_t kmsg,
                                                    uint32_t sig) {
  mach_msg_header_t *hdr = ikm_header(kmsg);
...
static uint32_t __ipc_kmsg_sign(ipc_kmsg_t kmsg,
...
#if __has_feature(ptrauth_calls)
  {
    ...
    data = ptrauth_utils_sign_blob_generic(trailer, MAX_TRAILER_SIZE, data,
...
static void ipc_kmsg_sign(ipc_kmsg_t kmsg, mach_msg_max_trailer_t *trailer) {
 ...
 */
static void ipc_kmsg_init_trailer_and_sign(ipc_kmsg_t kmsg, task_t sender) {
 ...
 */
mach_msg_size_t ipc_kmsg_validate_signature(ipc_kmsg_t kmsg) {
  uint32_t sig;
  mach_msg_size_t dsc_count;
...
void ipc_kmsg_sign_descriptors(mach_msg_kdescriptor_t *kdesc,
                               mach_msg_size_t dsc_count) {
#if __has_feature(ptrauth_calls)
  for (mach_msg_size_t i = 0; i < dsc_count; i++, kdesc++) {
    switch (mach_msg_kdescriptor_type(kdesc)) {
    case MACH_MSG_PORT_DESCRIPTOR:
      kdesc->kdesc_port.name = kdesc->kdesc_port.kext_name;
      break;
    case MACH_MSG_OOL_VOLATILE_DESCRIPTOR:
    case MACH_MSG_OOL_DESCRIPTOR:
      kdesc->kdesc_memory.address = kdesc->kdesc_memory.kext_address;
      break;
    ...
    case MACH_MSG_GUARDED_PORT_DESCRIPTOR:
      kdesc->kdesc_guarded_port.name = kdesc->kdesc_guarded_port.kext_name;
      break;
    default:
...
static void ipc_kmsg_relocate_descriptors(mach_msg_kdescriptor_t *dst_dsc,
  ...
  for (mach_msg_size_t i = 0; i < dsc_count; i++, dst_dsc++, src_dsc++) {
    ...
    case MACH_MSG_PORT_DESCRIPTOR:
      dst_dsc->kdesc_port.name = src_dsc->kdesc_port.name;
      break;
    case MACH_MSG_OOL_VOLATILE_DESCRIPTOR:
    case MACH_MSG_OOL_DESCRIPTOR:
      dst_dsc->kdesc_memory.address = src_dsc->kdesc_memory.address;
      break;
    case MACH_MSG_OOL_PORTS_DESCRIPTOR:
      dst_dsc->kdesc_port_array.address = src_dsc->kdesc_port_array.address;
      break;
    case MACH_MSG_GUARDED_PORT_DESCRIPTOR:
      dst_dsc->kdesc_guarded_port.name = src_dsc->kdesc_guarded_port.name;
      break;
    default:
...
static void ipc_kmsg_strip_descriptors(mach_msg_kdescriptor_t *dst_dsc,
                                       const mach_msg_kdescriptor_t *src_dsc,
                                       mach_msg_size_t dsc_count) {
#if __has_feature(ptrauth_calls)
  for (mach_msg_size_t i = 0; i < dsc_count; i++, dst_dsc++, src_dsc++) {
    switch (mach_msg_kdescriptor_type(src_dsc)) {
    case MACH_MSG_PORT_DESCRIPTOR:
      dst_dsc->kdesc_port.kext_name = src_dsc->kdesc_port.name;
      break;
    case MACH_MSG_OOL_VOLATILE_DESCRIPTOR:
    case MACH_MSG_OOL_DESCRIPTOR:
      dst_dsc->kdesc_memory.kext_address = src_dsc->kdesc_memory.address;
      break;
    case MACH_MSG_OOL_PORTS_DESCRIPTOR: {
    ...
    case MACH_MSG_GUARDED_PORT_DESCRIPTOR:
      dst_dsc->kdesc_guarded_port.kext_name = src_dsc->kdesc_guarded_port.name;
      break;
    default:
      __ipc_kmsg_descriptor_invalid_type_panic(src_dsc);
    }
...
static inline void *ikm_alloc_kdata_ool(size_t size, zalloc_flags_t flags) {
  return kalloc_type_var_impl(KT_IPC_KMSG_KDATA_OOL, size, flags, NULL);
}

static inline void ikm_free_kdata_ool(void *ptr, size_t size) {
 ...
 */
ipc_kmsg_t ipc_kmsg_alloc(mach_msg_size_t kmsg_size, mach_msg_size_t aux_size,
                          mach_msg_size_t desc_count,
                          ipc_kmsg_alloc_flags_t flags) {
  mach_msg_size_t max_kmsg_size, max_delta, max_kdata_size, max_udata_size,
  ...
  if (os_add3_overflow(kmsg_size, MAX_TRAILER_SIZE, max_delta,
                       &max_kmsg_size)) {
    return IKM_NULL;
  }
  ...
  if (max_kmsg_and_aux_size <= IKM_BIG_MSG_SIZE) {
    ...
    /*
     * max_kdata_size: Maximum combined size of header plus (optional)
     * descriptors. This is _base_ size + descriptor count * kernel descriptor
     * size.
     */
    ...
    /*
     * min_kdata_size: Minimum combined size of header plus (optional)
     * descriptors. This is _header_ size + descriptor count * minimal
     * descriptor size.
     */
    mach_msg_size_t min_size =
        (flags & IPC_KMSG_ALLOC_KERNEL) ? KERNEL_DESC_SIZE : USER_DESC_SIZE_MIN;
    if (os_mul_and_add_overflow(desc_count, min_size, sizeof(mach_msg_header_t),
                                &min_kdata_size)) {
      return IKM_NULL;
    }

    /*
     * max_udata_size: Maximum combined size of message content, trailer and
     * aux. This is total kmsg and aux size (already accounts for max trailer
     * size) minus _minimum_ (header + descs) size.
     */
    if (os_sub_overflow(max_kmsg_and_aux_size, min_kdata_size,
                        &max_udata_size)) {
      return IKM_NULL;
    }
...
ipc_kmsg_t ipc_kmsg_alloc_uext_reply(mach_msg_size_t size) {
  return ipc_kmsg_alloc(size, 0, 0,
                        IPC_KMSG_ALLOC_KERNEL | IPC_KMSG_ALLOC_LINEAR |
 ...
 */
bool ipc_kmsg_keep_alive_try_reusing(ipc_kmsg_t kmsg) {
  uintptr_t v;

  v = os_atomic_or_orig(&kmsg->ikm_keep_alive, IKM_KEEP_ALIVE_IN_USE, relaxed);
 ...
 */
static bool ipc_kmsg_keep_alive_done_using(ipc_kmsg_t kmsg) {
  ...
  v = os_atomic_andnot_orig(&kmsg->ikm_keep_alive, IKM_KEEP_ALIVE_IN_USE,
                            release);
 ...
 */
void ipc_kmsg_keep_alive_abandon(ipc_kmsg_t kmsg) {
  uintptr_t v;

  v = os_atomic_andnot_orig(&kmsg->ikm_keep_alive, IKM_KEEP_ALIVE_OWNED,
                            release);
 ...
 *		Nothing locked.
 */
static void ipc_kmsg_free_allocations(ipc_kmsg_t kmsg) {
  ...
  case IKM_TYPE_ALL_OOL:
    dsc_count =
        (kmsg->ikm_kdata_size - sizeof(mach_msg_base_t)) / KERNEL_DESC_SIZE;
...
void ipc_kmsg_free(ipc_kmsg_t kmsg) {
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_KMSG_FREE) | DBG_FUNC_NONE,
       VM_KERNEL_ADDRPERM((uintptr_t)kmsg), 0, 0, 0, 0);
 ...
 *		Nothing locked.
 */
static void ipc_kmsg_clean_header(ipc_kmsg_t kmsg) {
  ipc_port_t port;
  mach_msg_header_t *hdr = ikm_header(kmsg);
 ...
 *		No locks held.
 */
void ipc_kmsg_clean_descriptors(mach_msg_kdescriptor_t *kdesc
                                    __counted_by(number),
...
static void ipc_kmsg_clean(ipc_kmsg_t kmsg, mach_msg_size_t dsc_count) {
...
bool ipc_kmsg_enqueue_qos(ipc_kmsg_queue_t queue, ipc_kmsg_t kmsg) {
...
bool ipc_kmsg_override_qos(ipc_kmsg_queue_t queue, ipc_kmsg_t kmsg,
...
void ipc_kmsg_destroy(ipc_kmsg_t kmsg, ipc_kmsg_destroy_flags_t flags) {
...
bool ipc_kmsg_delayed_destroy(ipc_kmsg_t kmsg) {
...
bool ipc_kmsg_delayed_destroy_queue(ipc_kmsg_queue_t queue) {
...
void ipc_kmsg_reap_delayed(void) {
...
static pthread_priority_compact_t ipc_get_current_thread_priority(void) {
...
static kern_return_t ipc_kmsg_set_qos(ipc_kmsg_t kmsg,
  ...
  if ((options & MACH_SEND_OVERRIDE) &&
    ...
    kmsg->ikm_ppriority =
        _pthread_priority_make_from_thread_qos(qos, relpri, 0);
    ...
    if (options & MACH_SEND_OVERRIDE) {
      ...
      kmsg->ikm_qos_override =
          _pthread_priority_thread_qos(kmsg->ikm_ppriority);
    }
  }
  ...
  if (IP_VALID(special_reply_port) &&
    ...
    boolean_t sync_bootstrap_checkin =
        !!(options & MACH_SEND_SYNC_BOOTSTRAP_CHECKIN);
    /*
...
static kern_return_t ipc_kmsg_set_qos_kernel(ipc_kmsg_t kmsg) {
  ipc_port_t dest_port = ikm_header(kmsg)->msgh_remote_port;
  kmsg->ikm_qos_override = dest_port->ip_kernel_qos_override;
  kmsg->ikm_ppriority =
      _pthread_priority_make_from_thread_qos(kmsg->ikm_qos_override, 0, 0);
  return KERN_SUCCESS;
}
 ...
 */
static void ipc_kmsg_link_reply_context_locked(ipc_port_t reply_port,
  ...
  kr = bank_get_bank_ledger_thread_group_and_persona(voucher, NULL, NULL,
                                                     &persona_id);
 ...
 */
static mach_msg_return_t ipc_kmsg_validate_reply_context_locked(
    mach_msg_option64_t option, ipc_port_t dest_port, ipc_voucher_t voucher,
  ...
  if (dest_ctx != persona_id) {
    if ((option & MACH_SEND_KERNEL) == 0) {
      mach_port_guard_exception(
          voucher_name,
          MPG_PAYLOAD(MPG_FLAGS_STRICT_REPLY_MISMATCHED_PERSONA, persona_id,
                      dest_ctx),
          kGUARD_EXC_STRICT_REPLY);
    }
    return MACH_SEND_INVALID_CONTEXT;
 ...
 */
#define ikm_udsc_type(addr)                                                    \
  __IGNORE_WCASTALIGN(((const mach_msg_type_descriptor_t *)(addr))->type)
#define ikm_udsc_get(dst, addr)                                                \
  __IGNORE_WCASTALIGN(*(dst) = *(const typeof(*(dst)) *)(addr))
...
ipc_kmsg_copyin_header_validate(ipc_kmsg_t kmsg,
 ...
 *             Nothing locked.
 */
static void ipc_kmsg_copyin_header_cleanup(ikm_copyinhdr_state_t *st) {
...
ipc_kmsg_copyin_dest_disposition(ikm_copyinhdr_state_t *st,
 ...
 */
static mach_msg_return_t
ipc_kmsg_copyin_header_rights(ipc_space_t space, ikm_copyinhdr_state_t *st) {
  ...
  if (MACH_PORT_VALID(st->reply_name)) {
    ...
    if (__improbable(reply_entry == IE_NULL ||
                     (reply_entry->ie_bits & MACH_PORT_TYPE_PORT_RIGHTS) ==
                         0)) {
      is_write_unlock(space);
      return MACH_SEND_INVALID_REPLY;
    }

    if (__improbable(!ipc_right_copyin_check_reply(
            space, st->reply_name, reply_entry, st->reply_type))) {
      is_write_unlock(space);
      return MACH_SEND_INVALID_REPLY;
    }
  }
  ...
  kr =
      ipc_right_copyin(space, st->dest_name, dest_type,
                       IPC_OBJECT_COPYIN_FLAGS_ALLOW_IMMOVABLE_SEND | dest_xtra,
                       IPC_COPYIN_KMSG_DESTINATION, dest_entry, &st->dest_port,
                       &st->dest_cleanup, NULL);
  if (kr == KERN_SUCCESS) {
  ...
  if (st->voucher_name == st->dest_name && dest_xtra) {
    ...
    kr = ipc_right_copyin(space, st->voucher_name, st->voucher_type,
                          IPC_OBJECT_COPYIN_FLAGS_NONE, IPC_COPYIN_KMSG_VOUCHER,
                          voucher_entry, &st->voucher_port,
                          &st->voucher_cleanup, NULL);
  ...
  if (st->reply_name == st->dest_name && dest_xtra) {
    ...
    kr = ipc_right_copyin(space, st->reply_name, st->reply_type,
                          IPC_OBJECT_COPYIN_FLAGS_DEADOK, IPC_COPYIN_KMSG_REPLY,
                          reply_entry, &st->reply_port, &st->reply_cleanup,
                          NULL);
...
send_telemetry:
  if (IP_VALID(st->dest_port) && ip_type(st->dest_port) == IOT_SERVICE_PORT &&
  ...
  if (violation_port && !task_is_initproc(space->is_task) &&
 ...
 */
static mach_msg_return_t
ipc_kmsg_copyin_header(ipc_kmsg_t kmsg, ipc_space_t space,
                       mach_msg_priority_t priority,
  ...
  if (((options & MACH64_SEND_NOTIFY) != 0) &&
      ...
      st.dest_request != IE_REQ_NONE && ip_active(st.dest_port) &&
      !ip_in_space(st.dest_port, ipc_space_kernel)) {
    /* st.dest_port could be in-transit, or in an ipc space */
    if (ip_full(st.dest_port)) {
      needboost = ipc_port_request_sparm(st.dest_port, st.dest_name,
                                         st.dest_request, options, priority);
    } else {
  ...
  } else if (!ipc_port_importance_delta(st.dest_port, IPID_OPTION_SENDPOSSIBLE,
                                        1)) {
    ip_mq_unlock(st.dest_port);
...
static mach_msg_return_t
ipc_kmsg_inflate_port_descriptor(char *kdesc_addr, const char *udesc_addr,
...
static mach_msg_return_t
ipc_kmsg_copyin_port_descriptor(mach_msg_port_descriptor_t *dsc,
                                ipc_space_t space, ipc_port_t dest_port,
                                ipc_kmsg_t kmsg) {
  ...
  if (MACH_PORT_VALID(name)) {
    ...
    if (kr != KERN_SUCCESS) {
      if (kr == KERN_INVALID_RIGHT) {
        mach_port_guard_exception(
            name, MPG_PAYLOAD(MPG_FLAGS_SEND_INVALID_RIGHT_PORT, user_disp),
...
static mach_msg_return_t
ipc_kmsg_inflate_ool_descriptor(char *kdesc_addr, const char *udesc_addr,
                                mach_msg_send_uctx_t *send_uctx, bool isU64) {
  ...
  if (udesc.size > msg_ool_size_small && udesc.copy == MACH_MSG_PHYSICAL_COPY &&
      !udesc.deallocate) {
    vm_size_t size;
...
static mach_msg_return_t
ipc_kmsg_copyin_ool_descriptor(mach_msg_ool_descriptor_t *dsc,
                               mach_vm_address_t *paddr,
                               vm_size_t *space_needed, vm_map_t map) {
  ...
  if (length == 0) {
    ...
    if (vm_map_copyin(ipc_kernel_copy_map, addr, length, true, &copy) !=
        KERN_SUCCESS) {
      return MACH_MSG_VM_KERNEL;
    }
    ...
    *space_needed -= length_aligned;
     ...
     */
    switch (
        vm_map_copyin(map, dsc->u_address, length, dsc->deallocate, &copy)) {
    case KERN_SUCCESS:
      break;
...
static mach_msg_return_t
ipc_kmsg_inflate_ool_ports_descriptor(char *kdesc_addr, const char *udesc_addr,
                                      mach_msg_send_uctx_t *send_uctx,
                                      bool isU64) {
...
static mach_msg_return_t ipc_kmsg_copyin_ool_ports_descriptor(
    mach_msg_ool_ports_descriptor_t *dsc, vm_map_t map, ipc_space_t space,
    ipc_port_t dest_port, ipc_kmsg_t kmsg, mach_msg_option64_t options) {
  mach_msg_type_name_t user_disp = dsc->disposition;
  ...
  for (mach_msg_size_t i = 0; i < count; i++) {
    ...
    kr = ipc_object_copyin(space, name, user_disp, kmsg->ikm_flags,
                           IPC_COPYIN_KMSG_OOL_PORT_ARRAY_DESCRIPTOR, NULL,
                           &port);
    ...
    if (kr != KERN_SUCCESS) {
      ...
      if (kr == KERN_INVALID_RIGHT) {
        mach_port_guard_exception(
            name, MPG_PAYLOAD(MPG_FLAGS_SEND_INVALID_RIGHT_OOL_PORT, user_disp),
            kGUARD_EXC_SEND_INVALID_RIGHT);
      }
      return MACH_SEND_INVALID_RIGHT;
    }
...
static mach_msg_return_t ipc_kmsg_inflate_guarded_port_descriptor(
    char *kdesc_addr, const char *udesc_addr, mach_msg_send_uctx_t *send_uctx,
    bool isU64) {
  ...
  if (!udesc.flags || ((udesc.flags & ~MACH_MSG_GUARD_FLAGS_MASK) != 0) ||
      ((udesc.flags & MACH_MSG_GUARD_FLAGS_UNGUARDED_ON_SEND) &&
       (udesc.context != 0))) {
    return MACH_SEND_INVALID_TYPE;
  }
...
static mach_msg_return_t
ipc_kmsg_copyin_guarded_port_descriptor(mach_msg_guarded_port_descriptor_t *dsc,
                                        ipc_space_t space, ipc_port_t dest_port,
                                        ipc_kmsg_t kmsg) {
  mach_msg_type_name_t user_disp = dsc->disposition;
  ...
  if (MACH_PORT_VALID(name)) {
    ...
    if (kr != KERN_SUCCESS) {
      if (kr == KERN_INVALID_RIGHT) {
        mach_port_guard_exception(
            name, MPG_PAYLOAD(MPG_FLAGS_SEND_INVALID_RIGHT_GUARDED, user_disp),
            kGUARD_EXC_SEND_INVALID_RIGHT);
      }
      return MACH_SEND_INVALID_RIGHT;
    }
...
static mach_msg_return_t
ipc_kmsg_inflate_descriptor(char *kdesc, const char *udesc,
                            mach_msg_send_uctx_t *send_uctx, bool isU64) {
  ...
  case MACH_MSG_OOL_PORTS_DESCRIPTOR:
    return ipc_kmsg_inflate_ool_ports_descriptor(kdesc, udesc, send_uctx,
                                                 isU64);
  case MACH_MSG_GUARDED_PORT_DESCRIPTOR:
    return ipc_kmsg_inflate_guarded_port_descriptor(kdesc, udesc, send_uctx,
                                                    isU64);
  default:
...
static mach_msg_return_t
ipc_kmsg_inflate_descriptors(char *const descs, mach_msg_send_uctx_t *send_uctx,
                             bool isU64) {
  ...
  if (__probable(desc_count <= 64)) {
    ...
    for (uint64_t bit = 1ull << (desc_count - 1); bit; bit >>= 1) {
      ...
      mr = ipc_kmsg_inflate_descriptor(kdesc, udesc, send_uctx, isU64);
...
static inline bool
ipc_kmsg_user_desc_type_is_valid(mach_msg_descriptor_type_t type,
                                 mach_msg_option64_t options) {
  switch (type) {
  case MACH_MSG_PORT_DESCRIPTOR:
  case MACH_MSG_OOL_DESCRIPTOR:
 ...
 */
__result_use_check static mach_msg_return_t
ipc_kmsg_measure_descriptors_from_user(vm_address_t addr, mach_msg_size_t size,
                                       mach_msg_send_uctx_t *send_uctx,
                                       mach_msg_option64_t options,
                                       bool isU64) {
...
static mach_msg_return_t ipc_kmsg_copyin_body(ipc_kmsg_t kmsg,
                                              mach_msg_send_uctx_t *send_uctx,
                                              ipc_space_t space, vm_map_t map,
                                              mach_msg_option64_t options) {
  ...
  if (psize) {
    ...
    kr = mach_vm_allocate_kernel(
        ipc_kernel_copy_map, &paddr, psize,
  ...
  for (mach_msg_size_t copied_in_dscs = 0; copied_in_dscs < dsc_count;
       copied_in_dscs++) {
    ...
    switch (mach_msg_kdescriptor_type(kdesc)) {
    case MACH_MSG_PORT_DESCRIPTOR:
      mr = ipc_kmsg_copyin_port_descriptor(&kdesc->kdesc_port, space, dest_port,
                                           kmsg);
      break;
    case MACH_MSG_OOL_VOLATILE_DESCRIPTOR:
    case MACH_MSG_OOL_DESCRIPTOR:
      mr = ipc_kmsg_copyin_ool_descriptor(&kdesc->kdesc_memory, &paddr, &psize,
                                          map);
      break;
    case MACH_MSG_OOL_PORTS_DESCRIPTOR:
      mr = ipc_kmsg_copyin_ool_ports_descriptor(
          &kdesc->kdesc_port_array, map, space, dest_port, kmsg, options);
      break;
    case MACH_MSG_GUARDED_PORT_DESCRIPTOR:
    ...
    if (MACH_MSG_SUCCESS != mr) {
      ...
      ipc_kmsg_clean_header(kmsg);
      ipc_kmsg_clean_descriptors(kbase->msgb_dsc_array, copied_in_dscs);
      if (psize) {
 ...
 */
static mach_msg_return_t ipc_kmsg_get_and_inflate_from_user(
    ipc_kmsg_t kmsg, mach_msg_send_uctx_t *send_uctx, mach_msg_header_t *khdr,
    vm_map_t map, mach_msg_option64_t options) {
  ...
  *khdr = (mach_msg_header_t){
  ...
  if (ikm_is_linear(kmsg)) {
    kbody = (char *)kdesc + send_uctx->send_dsc_count * KERNEL_DESC_SIZE;
  } else {
  ...
  if (send_uctx->send_dsc_count) {
     ...
     */
    mr = ipc_kmsg_measure_descriptors_from_user(
        (vm_address_t)kdesc, copyin_size, send_uctx, options, isU64);
    if (mr != MACH_MSG_SUCCESS) {
      return mr;
    }
  ...
  if (send_uctx->send_aux_size) {
    ...
    if (aux_size > sizeof(aux_hdr[0]) &&
        copyinmsg(send_uctx->send_aux_addr + sizeof(*aux_hdr), aux_hdr + 1,
                  aux_size - sizeof(*aux_hdr))) {
      return MACH_SEND_INVALID_DATA;
    }
  }
 ...
 *	Returns:
 *		MACH_MSG_SUCCESS	Successful copyin.
 *		MACH_SEND_INVALID_HEADER Illegal value in the message header
 * bits. MACH_SEND_INVALID_DEST	Can't copyin destination port.
 *		MACH_SEND_INVALID_REPLY	Can't copyin reply port.
 *		MACH_SEND_INVALID_MEMORY	Can't grab out-of-line memory.
 *		MACH_SEND_INVALID_RIGHT	Can't copyin port right in body.
 *		MACH_SEND_INVALID_TYPE	Bad type specification.
...
mach_msg_return_t ipc_kmsg_copyin_from_user(ipc_kmsg_t kmsg,
                                            mach_msg_send_uctx_t *send_uctx,
                                            ipc_space_t space, vm_map_t map,
                                            mach_msg_priority_t priority,
                                            mach_msg_option64_t *option64p) {
  ...
  mr = ipc_validate_kmsg_header_schema_from_user(
      &send_uctx->send_header, send_uctx->send_dsc_count, options);
  if (mr != MACH_MSG_SUCCESS) {
    return mr;
  }

  mr = ipc_kmsg_get_and_inflate_from_user(kmsg, send_uctx, hdr, map, options);
  if (mr != MACH_MSG_SUCCESS) {
    return mr;
  }
  ...
  KERNEL_DEBUG_CONSTANT(
      MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_MSG_SEND) | DBG_FUNC_NONE,
      VM_KERNEL_ADDRPERM((uintptr_t)kmsg), (uintptr_t)hdr->msgh_bits,
      (uintptr_t)hdr->msgh_id,
      VM_KERNEL_ADDRPERM((uintptr_t)unsafe_convert_port_to_voucher(
          ipc_kmsg_get_voucher_port(kmsg))),
      0);

  DEBUG_KPRINT_SYSCALL_IPC(
      "ipc_kmsg_copyin_from_user header:\n%.8x\n%.8x\n%p\n%p\n%p\n%.8x\n",
      hdr->msgh_size, hdr->msgh_bits, hdr->msgh_remote_port,
      hdr->msgh_local_port, ipc_kmsg_get_voucher_port(kmsg), hdr->msgh_id);
 ...
 */
static mach_msg_return_t ipc_kmsg_copyout_header(ipc_kmsg_t kmsg,
                                                 mach_msg_header_t *msg,
                                                 ipc_space_t space,
  ...
  handle_reply_again:
    ...
    if (IP_VALID(reply)) {
      ...
      if (!io_state_active(label.io_state) ||
          !ip_label_check_or_substitute(space, reply, &label, reply_type,
                                        &reply_subst)) {
      ...
      if ((reply_type != MACH_MSG_TYPE_PORT_SEND_ONCE) &&
        ...
        entries_held--;
        ipc_entry_claim(space, ip_to_object(reply), &reply_name, &entry);
      }
       ...
       */
      if (enforce_strict_reply && MACH_RCV_WITH_STRICT_REPLY(option) &&
          IP_VALID(voucher)) {
      ...
      ipc_right_copyout_any_send(space, reply, reply_type,
                                 IPC_OBJECT_COPYOUT_FLAGS_NONE, reply_name,
                                 entry);
      kr = KERN_SUCCESS;
  ...
  done_with_reply:
    ...
    if (voucher_type != MACH_MSGH_BITS_ZERO) {
      ...
      if ((option & MACH_RCV_VOUCHER) != 0) {
        ...
        if (ipc_right_reverse(space, voucher, &voucher_name, &entry)) {
        ...
        ipc_right_copyout_any_send(space, voucher, MACH_MSG_TYPE_MOVE_SEND,
                                   IPC_OBJECT_COPYOUT_FLAGS_NONE, voucher_name,
                                   entry);
  ...
  if (ip_active(dest)) {
    ...
    if (IP_VALID(reply)) {
      ip_mq_lock(reply);
      if (ip_active(reply) ||
          IP_TIMESTAMP_ORDER(timestamp, ip_get_death_time(reply))) {
  ...
  if ((option & MACH_RCV_VOUCHER) != 0) {
    KERNEL_DEBUG_CONSTANT(
        MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_MSG_RECV) | DBG_FUNC_NONE,
        VM_KERNEL_ADDRPERM((uintptr_t)kmsg), (uintptr_t)msg->msgh_bits,
        (uintptr_t)msg->msgh_id, VM_KERNEL_ADDRPERM(voucher_addr), 0);
  } else {
    KERNEL_DEBUG_CONSTANT(
        MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_MSG_RECV_VOUCHER_REFUSED) |
            DBG_FUNC_NONE,
        VM_KERNEL_ADDRPERM((uintptr_t)kmsg), (uintptr_t)msg->msgh_bits,
        (uintptr_t)msg->msgh_id, VM_KERNEL_ADDRPERM(voucher_addr), 0);
  }
  ...
  msg->msgh_bits =
      MACH_MSGH_BITS_SET(reply_type, dest_type, voucher_type, mbits);
 ...
 */
static mach_msg_return_t ipc_kmsg_copyout_port(
    ipc_space_t space, ipc_port_t port, mach_msg_type_name_t msgt_name,
    mach_msg_guarded_port_descriptor_t *gdesc, mach_port_name_t *namep) {
  kern_return_t kr;
  ...
  kr = ipc_object_copyout(space, port, msgt_name, IPC_OBJECT_COPYOUT_FLAGS_NONE,
                          gdesc, namep);
  if (kr != KERN_SUCCESS) {
 ...
 *              Name is valid if the space acquired the right and msgt_name
 * would be changed from MOVE_SO to MAKE_SO. Name is MACH_PORT_DEAD if the port
 * is dead. Name is MACH_PORT_NULL if its entry could not be found in task's ipc
 * space. MACH_MSG_IPC_SPACE The space is dead.  (Name is MACH_PORT_NULL.)
 *	Conditions:
 *      Nothing locked.
 */
static mach_msg_return_t
ipc_kmsg_copyout_reply_port(ipc_space_t space, ipc_port_t port,
...
static mach_msg_return_t
ipc_kmsg_copyout_port_descriptor(mach_msg_port_descriptor_t *dsc,
                                 ipc_space_t space) {
  mach_port_name_t name;
  mach_msg_return_t mr;

  /* Copyout port right carried in the message */
  mr = ipc_kmsg_copyout_port(space, dsc->name, dsc->disposition, NULL, &name);
  dsc->u_name = CAST_MACH_NAME_TO_PORT(name);
  return mr;
}

static char *
ipc_kmsg_deflate_port_descriptor(char *udesc_end,
...
static mach_msg_return_t
ipc_kmsg_copyout_ool_descriptor(mach_msg_ool_descriptor_t *dsc, vm_map_t map) {
  vm_map_copy_t copy = dsc->address;
  ...
  if (copy != VM_MAP_COPY_NULL) {
    ...
    if (misaligned) {
      ...
      rounded_size =
          vm_map_round_page(copy->offset + size, effective_page_mask) -
          vm_map_trunc_page(copy->offset, effective_page_mask);

      kr = mach_vm_allocate_kernel(
          map, &rounded_addr, rounded_size,
      ...
      if (kr == KERN_SUCCESS) {
         ...
         *
         * if (size >= VM_MAP_COPY_OVERWRITE_OPTIMIZATION_THRESHOLD_PAGES *
         * effective_page_size)
         *
        ...
        if ((rounded_addr & effective_page_mask) !=
          ...
          assert((rounded_addr & effective_page_mask) ==
                 (copy->offset & effective_page_mask));
        }
...
static char *ipc_kmsg_deflate_memory_descriptor(
    char *udesc_end, const mach_msg_ool_descriptor_t *kdesc, bool isU64) {
...
static mach_msg_return_t
ipc_kmsg_copyout_ool_ports_descriptor(mach_msg_kdescriptor_t *kdesc,
                                      vm_map_t map, ipc_space_t space) {
  mach_msg_ool_ports_descriptor_t *dsc = &kdesc->kdesc_port_array;
  ...
  if (count != 0 && array != NULL) {
    ...
    if (kr == MACH_MSG_SUCCESS) {
      for (mach_msg_size_t i = 0; i < count; i++) {
        mr |=
            ipc_kmsg_copyout_port(space, array[i].port, disp, NULL, &names[i]);
      }
...
static char *ipc_kmsg_deflate_port_array_descriptor(
    char *udesc_end, const mach_msg_ool_ports_descriptor_t *kdesc, bool isU64) {
  if (isU64) {
    mach_msg_ool_ports_descriptor64_t udesc = {
        .address = kdesc->u_address,
        .count = kdesc->count,
...
static mach_msg_return_t ipc_kmsg_copyout_guarded_port_descriptor(
    mach_msg_guarded_port_descriptor_t *dsc, ipc_space_t space,
    mach_msg_option64_t option) {
  ...
  if ((kn != ITH_KNOTE_PSEUDO) && ((option & MACH_RCV_GUARDED_DESC) == 0)) {
#if DEVELOPMENT || DEBUG
    /*
     * Simulated crash needed for debugging, notifies the receiver to opt into
     * receiving guarded descriptors.
     */
    mach_port_guard_exception(current_thread()->ith_receiver_name, 0,
                              kGUARD_EXC_RCV_GUARDED_DESC);
#endif
    KDBG(MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_DESTROY_GUARDED_DESC),
         current_thread()->ith_receiver_name, VM_KERNEL_ADDRPERM(port), disp,
         dsc->flags);
    ...
    mr = ipc_kmsg_copyout_port(space, port, disp, dsc, &dsc->u_name);
  }

  return mr;
}

static char *ipc_kmsg_deflate_guarded_port_descriptor(
    char *udesc_end, const mach_msg_guarded_port_descriptor_t *kdesc,
    bool isU64) {
  if (isU64) {
    mach_msg_guarded_port_descriptor64_t udesc = {
...
static mach_msg_return_t
ipc_kmsg_copyout_descriptors(mach_msg_kdescriptor_t *kdesc,
                             mach_msg_size_t dsc_count, ipc_space_t space,
                             vm_map_t map, mach_msg_option64_t option) {
  mach_msg_return_t mr = MACH_MSG_SUCCESS;
  ...
  for (mach_msg_size_t i = 0; i < dsc_count; i++, kdesc++) {
    switch (mach_msg_kdescriptor_type(kdesc)) {
    case MACH_MSG_PORT_DESCRIPTOR:
      mr |= ipc_kmsg_copyout_port_descriptor(&kdesc->kdesc_port, space);
      break;
    case MACH_MSG_OOL_VOLATILE_DESCRIPTOR:
    case MACH_MSG_OOL_DESCRIPTOR:
      mr |= ipc_kmsg_copyout_ool_descriptor(&kdesc->kdesc_memory, map);
      break;
    case MACH_MSG_OOL_PORTS_DESCRIPTOR:
      mr |= ipc_kmsg_copyout_ool_ports_descriptor(kdesc, map, space);
      break;
    case MACH_MSG_GUARDED_PORT_DESCRIPTOR:
...
static void ipc_kmsg_deflate_descriptors(ikm_deflate_context_t *dctx,
  ...
  for (mach_msg_size_t i = desc_count; i-- > 0;) {
    ...
    switch (mach_msg_kdescriptor_type(kdesc)) {
    case MACH_MSG_PORT_DESCRIPTOR:
      udesc = ipc_kmsg_deflate_port_descriptor(udesc, &kdesc->kdesc_port);
      break;
    case MACH_MSG_OOL_VOLATILE_DESCRIPTOR:
    case MACH_MSG_OOL_DESCRIPTOR:
      udesc = ipc_kmsg_deflate_memory_descriptor(udesc, &kdesc->kdesc_memory,
                                                 dctx->dctx_isU64);
      break;
    case MACH_MSG_OOL_PORTS_DESCRIPTOR:
      udesc = ipc_kmsg_deflate_port_array_descriptor(
          udesc, &kdesc->kdesc_port_array, dctx->dctx_isU64);
      break;
    case MACH_MSG_GUARDED_PORT_DESCRIPTOR:
      udesc = ipc_kmsg_deflate_guarded_port_descriptor(
          udesc, &kdesc->kdesc_guarded_port, dctx->dctx_isU64);
      break;
    default:
      __ipc_kmsg_descriptor_invalid_type_panic(kdesc);
    }
...
ipc_kmsg_descriptors_copyout_size(mach_msg_kdescriptor_t *kdesc,
                                  mach_msg_size_t count, vm_map_t map) {
  bool isU64 = (map->max_offset > VM_MAX_ADDRESS);
...
mach_msg_return_t ipc_kmsg_copyout(ipc_kmsg_t kmsg, ipc_space_t space,
                                   vm_map_t map, mach_msg_option64_t option) {
  mach_msg_header_t *hdr = ikm_header(kmsg);
  mach_msg_size_t dsc_count;
  mach_msg_return_t mr;
  ...
  if (dsc_count) {
    ...
    mr = ipc_kmsg_copyout_descriptors(kbase->msgb_dsc_array, dsc_count, space,
                                      map, option);
  }
...
mach_msg_return_t ipc_kmsg_copyout_pseudo(ipc_kmsg_t kmsg, ipc_space_t space,
                                          vm_map_t map) {
  mach_msg_header_t *hdr = ikm_header(kmsg);
  mach_msg_bits_t mbits = hdr->msgh_bits;
  ...
  if (!IP_VALID(reply)) {
    ...
    reply_mr =
        ipc_kmsg_copyout_reply_port(space, reply, &reply_type, &reply_name);
    mr = mr | reply_mr;
    if (reply_mr == MACH_MSG_SUCCESS) {
      mbits = MACH_MSGH_BITS_SET(dest_type, reply_type, voucher_type,
                                 MACH_MSGH_BITS_OTHER(mbits));
    }
  } else {
    mr =
        mr | ipc_kmsg_copyout_port(space, reply, reply_type, NULL, &reply_name);
  }
   ...
   *
   * If it was copied in via copy-send, the header still contains the old
   * voucher name. Restore the type and discard the copied-in/pre-processed
   * voucher.
   */
  if (IP_VALID(voucher)) {
    assert(voucher_type == MACH_MSG_TYPE_MOVE_SEND);
    if (kmsg->ikm_voucher_type == MACH_MSG_TYPE_MOVE_SEND) {
      mr |= ipc_kmsg_copyout_port(space, voucher, voucher_type, NULL,
                                  &voucher_name);
      hdr->msgh_voucher_port = voucher_name;
    } else {
      assert(kmsg->ikm_voucher_type == MACH_MSG_TYPE_COPY_SEND);
      hdr->msgh_bits =
          MACH_MSGH_BITS_SET(dest_type, reply_type, MACH_MSG_TYPE_COPY_SEND,
...
void ipc_kmsg_copyout_dest_to_user(ipc_kmsg_t kmsg, ipc_space_t space) {
...
void ipc_kmsg_copyout_dest_to_kernel(ipc_kmsg_t kmsg, ipc_space_t space) {
  ipc_port_t dest;
  mach_port_t reply;
  mach_msg_type_name_t dest_type;
  mach_msg_type_name_t reply_type;
  ...
  hdr->msgh_bits = (MACH_MSGH_BITS_OTHER(hdr->msgh_bits) |
                    MACH_MSGH_BITS(reply_type, dest_type));
  hdr->msgh_local_port = CAST_MACH_NAME_TO_PORT(dest_name);
  hdr->msgh_remote_port = reply;
}

static void ipc_kmsg_deflate_header(ikm_deflate_context_t *dctx,
                                    mach_msg_header_t *hdr) {
...
static void ipc_kmsg_deflate_trailer(ikm_deflate_context_t *dctx,
...
ipc_kmsg_deflate(ipc_kmsg_t kmsg, /* scalar or vector */
                 mach_msg_recv_result_t *msgr, mach_msg_option64_t options,
                 vm_map_t map) {
  mach_msg_header_t *hdr = ikm_header(kmsg);
  ...
  if (hdr->msgh_bits & MACH_MSGH_BITS_COMPLEX) {
    ...
    ipc_kmsg_deflate_descriptors(&dctx, kbase->msgb_dsc_array,
 ...
 */
mach_msg_return_t ipc_kmsg_put_to_user(ipc_kmsg_t kmsg, /* scalar or vector */
                                       mach_msg_recv_bufs_t *recv_bufs,
                                       mach_msg_recv_result_t *msgr,
                                       mach_msg_option64_t options,
                                       vm_map_t map, mach_msg_return_t mr) {
  ...
  if (options & MACH64_RCV_LINEAR_VECTOR) {
    ...
    if (options & MACH64_RCV_STACK) {
      msg_rcv_addr += recv_bufs->recv_msg_size - (usize + dctx.dctx_aux_size);
    }
...
out:
  ...
  KERNEL_DEBUG_CONSTANT(
      MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_KMSG_LINK) | DBG_FUNC_NONE,
...
mach_msg_return_t ipc_kmsg_get_from_kernel(mach_msg_header_t *msg,
  ...
  if (desc_count) {
    ...
    if (options & MACH64_POLICY_KERNEL_EXTENSION) {
      ipc_kmsg_sign_descriptors(dst_base->msgb_dsc_array, desc_count);
    } else {
...
static void ipc_kmsg_copyin_port_from_kernel(mach_msg_header_t *hdr,
                                             ipc_port_t port, ipc_port_t remote,
...
mach_msg_return_t ipc_kmsg_copyin_from_kernel(ipc_kmsg_t kmsg) {
  mach_msg_header_t *hdr = ikm_header(kmsg);
  ...
  if (bits & MACH_MSGH_BITS_COMPLEX) {
    ...
    for (mach_msg_size_t i = 0; i < count; i++) {
      ...
      case MACH_MSG_PORT_DESCRIPTOR: {
        ...
        if (IP_VALID(port)) {
          ipc_kmsg_copyin_port_from_kernel(hdr, port, remote, disp);
        }
        break;
      }
      case MACH_MSG_OOL_VOLATILE_DESCRIPTOR:
 ...
 */
mach_msg_return_t ipc_kmsg_send(ipc_kmsg_t kmsg, mach_msg_option64_t options,
  ...
  if (enforce_strict_reply && port->ip_reply_context != 0 &&
    ...
    error = ipc_kmsg_validate_reply_context_locked(
        options, port, th->ith_voucher, th->ith_voucher_name);
...
retry:
  ...
  if (error != MACH_MSG_SUCCESS) {
    ...
    error =
        ipc_mqueue_send_locked(&port->ip_messages, kmsg, options, send_timeout);
    /* port unlocked */
  }
  ...
  if (did_importance) {
    ...
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE, (IMPORTANCE_CODE(IMP_MSG, IMP_MSG_SEND)) | DBG_FUNC_END,
        task_pid(current_task()), sender_pid, imp_msgh_id, importance_cleared,
        0);
#endif /* IMPORTANCE_TRACE */
  }
#endif /* IMPORTANCE_INHERITANCE */
 ...
 *		Nothing locked.
 */
void ipc_kmsg_put_to_kernel(
    mach_msg_header_t *msg, mach_msg_option64_t options, ipc_kmsg_t kmsg,
    mach_msg_size_t rcv_size) /* includes trailer size */
{
  ...
  if (ikm_is_linear(kmsg)) {
    ...
    memcpy((char *)msg + kdata_sz, kmsg->ikm_udata, rcv_size - kdata_sz);
  }
...
void ipc_kmsg_trace_send(ipc_kmsg_t kmsg, mach_msg_option64_t option) {
  ...
  if (trailer->msgh_trailer_size <= sizeof(mach_msg_security_trailer_t)) {
     ...
     */
    if (memcmp(&strailer->msgh_sender, &KERNEL_SECURITY_TOKEN,
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_KMSG_INFO) | DBG_FUNC_END,
       (uintptr_t)send_pid, (uintptr_t)dst_pid,
       (uintptr_t)(((uint64_t)msg->msgh_id << KMSG_TRACE_ID_SHIFT) | msg_size),
       (uintptr_t)(((msg_flags & KMSG_TRACE_FLAGS_MASK)
                    << KMSG_TRACE_FLAGS_SHIFT) |
                   ((num_ports & KMSG_TRACE_PORTS_MASK)
