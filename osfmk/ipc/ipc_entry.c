 ...
 *		none
 */
unsigned int ipc_entry_table_count_max(void) {
 ...
 *		as appropriate.
 */
mach_port_name_t ipc_entry_name_mask(mach_port_name_t name) {
 ...
 * 1 1     16
 */
static ipc_entry_bits_t ipc_entry_make_gen(ipc_entry_bits_t ie_bits,
                                           ipc_space_t space) {
  ipc_entry_bits_t roll_bits;

  roll_bits = random_bool_gen_bits(&space->is_prng, space->is_entropy,
                                   IS_ENTROPY_CNT, IE_BITS_ROLL_BITS);

  roll_bits <<= __builtin_ctz(IE_BITS_ROLL_MASK);
  return (ie_bits & IE_BITS_GEN_MASK) | roll_bits;
}

static ipc_entry_bits_t ipc_entry_next_gen(ipc_entry_bits_t oldgen,
                                           ipc_space_t space) {
...
kern_return_t ipc_entries_hold(ipc_space_t space, uint32_t entries_needed) {
...
kern_return_t ipc_entry_claim(ipc_space_t space, ipc_object_t object,
  ...
  assert(entry && ipc_entry_table_contains(table, entry->ie_next) &&
...
kern_return_t ipc_entry_alloc(ipc_space_t space, ipc_object_t object,
                              mach_port_name_t *namep, ipc_entry_t *entryp) {
  kern_return_t kr;
...
kern_return_t ipc_entry_alloc_name(ipc_space_t space, mach_port_name_t name,
                                   ipc_entry_t *entryp) {
  ...
  for (;;) {
    ...
    if (!IE_BITS_TYPE(entry->ie_bits)) {
      ...
      if (prev_index > 0) {
         ...
         */
        ipc_entry_modified(
            space, MACH_PORT_MAKE(prev_index, IE_BITS_GEN(prev_entry->ie_bits)),
            prev_entry);
      }
...
void ipc_entry_dealloc(ipc_space_t space, ipc_object_t object,
...
void ipc_entry_modified(ipc_space_t space, mach_port_name_t name,
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_PORT_ENTRY_MODIFY) |
                            DBG_FUNC_NONE,
                        space->is_task ? task_pid(space->is_task) : 0, name,
                        entry->ie_bits, 0, 0);
}
...
static inline void ipc_space_start_growing(ipc_space_t is) {
  assert(!is_growing(is));
  is->is_grower = current_thread();
}

static void ipc_space_done_growing_and_unlock(ipc_space_t space) {
...
kern_return_t ipc_entry_grow_table(ipc_space_t space,
