...
static inline ipc_pset_t ips_from_waitq(waitq_t wq) {
...
extern void ipc_pset_lock(ipc_pset_t pset);

/* Allocate a port set */
extern kern_return_t ipc_pset_alloc(ipc_space_t space, mach_port_name_t *namep,
                                    ipc_pset_t *psetp);

/* Allocate a port set, with a specific name */
extern kern_return_t ipc_pset_alloc_name(ipc_space_t space,
                                         mach_port_name_t name,
                                         ipc_pset_t *psetp);

/* Allocate a port set in a special space */
extern ipc_pset_t ipc_pset_alloc_special(ipc_space_t space);

/* Destroy a port_set */
extern void ipc_pset_destroy(ipc_space_t space, ipc_pset_t pset);

/* Finalize the destruction of a pset and free it */
extern void ipc_pset_free(ipc_pset_t pset);

#if MACH_KERNEL_PRIVATE
extern struct turnstile *filt_ipc_kqueue_turnstile(struct knote *kn);

extern bool filt_machport_kqueue_has_turnstile(struct knote *kn);

extern void filt_machport_turnstile_prepare_lazily(
    struct knote *kn, mach_msg_type_name_t msgt_name, ipc_port_t port);

extern struct turnstile *filt_machport_stash_port(struct knote *kn,
                                                  ipc_port_t port, int *link);
#endif
