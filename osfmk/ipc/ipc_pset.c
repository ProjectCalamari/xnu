...
static void ipc_port_set_init(ipc_pset_t pset, mach_port_name_t name) {
...
void ipc_pset_lock(ipc_pset_t pset) {
  ips_validate(pset);
...
kern_return_t ipc_pset_alloc_name(ipc_space_t space, mach_port_name_t name,
                                  ipc_pset_t *psetp) {
  kern_return_t kr;
  ipc_entry_t entry;
  mach_port_type_t type = MACH_PORT_TYPE_PORT_SET;
 ...
 *		Nothing locked.
 */
ipc_pset_t ipc_pset_alloc_special(__assert_only ipc_space_t space) {
...
void ipc_pset_destroy(ipc_space_t space, ipc_pset_t pset) {
 ...
 *		Nothing locked.
 */
void ipc_pset_free(ipc_pset_t pset) {
...
static int filt_pset_filter_result(ipc_pset_t pset) {
...
static int filt_port_filter_result(struct knote *kn, ipc_port_t port) {
  ...
  if (tg) {
    ...
    kqueue_set_preadopted_thread_group(kq, tg, first->ikm_qos_override);
  }
#endif
...
struct turnstile *filt_ipc_kqueue_turnstile(struct knote *kn) {
  assert(kn->kn_filter == EVFILT_MACHPORT || kn->kn_filter == EVFILT_WORKLOOP);
  return kqueue_turnstile(knote_get_kq(kn));
}

bool filt_machport_kqueue_has_turnstile(struct knote *kn) {
  assert(kn->kn_filter == EVFILT_MACHPORT);
  return ((kn->kn_sfflags & MACH_RCV_MSG) ||
          (kn->kn_sfflags & MACH_RCV_SYNC_PEEK)) &&
 ...
 */
struct turnstile *filt_machport_stash_port(struct knote *kn, ipc_port_t port,
                                           int *link) {
 ...
 */
void filt_machport_turnstile_prepare_lazily(struct knote *kn,
  ...
  if ((msgt_name == MACH_MSG_TYPE_PORT_SEND_ONCE &&
    ...
    turnstile_update_inheritor(
        kn_ts, ts, TURNSTILE_IMMEDIATE_UPDATE | TURNSTILE_INHERITOR_TURNSTILE);
    turnstile_cleanup();
  }
}

static void filt_machport_turnstile_complete_port(struct knote *kn,
                                                  ipc_port_t port) {
  struct turnstile *ts = TURNSTILE_NULL;
...
void filt_wldetach_sync_ipc(struct knote *kn) {
 ...
 */
static void filt_machport_turnstile_complete(struct knote *kn) {
  if (kn->kn_ext[3]) {
  ...
  if (ts) {
    ...
    turnstile_complete((uintptr_t)kn, (struct turnstile **)&ts_store, &ts,
                       TURNSTILE_KNOTE);
    knote_kn_hook_set_raw(kn, ts_store);
...
static void filt_machport_link(struct klist *klist, struct knote *kn) {
...
static void filt_machport_unlink(struct klist *klist, struct knote *kn) {
  struct knote **knprev;
...
int filt_wlattach_sync_ipc(struct knote *kn) {
  ...
  if (ip_is_special_reply_port(port)) {
    ipc_port_adjust_special_reply_port_locked(
        port, kn, IPC_PORT_ADJUST_SR_LINK_WORKLOOP, FALSE);
  } else {
...
static int filt_psetattach(struct knote *kn, ipc_pset_t pset) {
  int result = 0;
...
static int filt_portattach(struct knote *kn, ipc_port_t port) {
...
static void filt_psetdetach(struct knote *kn) {
...
static void filt_portdetach(struct knote *kn) {
  ipc_port_t port = kn->kn_ipc_port;
  struct turnstile *send_turnstile = TURNSTILE_NULL;
 ...
 */
static int filt_psetevent(struct knote *kn __unused, long hint __assert_only) {
  /*
...
static int filt_portevent(struct knote *kn, long hint __assert_only) {
  assert(hint == 0);
  return filt_port_filter_result(kn, kn->kn_ipc_port);
}

void ipc_pset_prepost(struct waitq_set *wqs, struct waitq *waitq) {
  KNOTE(&ips_from_waitq(wqs)->ips_klist, (long)waitq);
}

static void filt_machporttouch(struct knote *kn, struct kevent_qos_s *kev) {
  /*
...
static int filt_psettouch(struct knote *kn, struct kevent_qos_s *kev) {
  ipc_pset_t pset = kn->kn_ipc_pset;
  int result = 0;
...
static int filt_porttouch(struct knote *kn, struct kevent_qos_s *kev) {
  ipc_port_t port = kn->kn_ipc_port;
  int result = 0;
...
static int filt_machportprocess(struct knote *kn, struct kevent_qos_s *kev,
                                ipc_object_t object, ipc_object_type_t otype) {
  ...
  wresult = ipc_mqueue_receive_on_thread_and_unlock(
      io_waitq(object), MACH_MSG_TIMEOUT_NONE, THREAD_INTERRUPTIBLE, self);
  /* port unlocked */
  ...
  if (kectx && kev->fflags != MACH_RCV_TOO_LARGE) {
    mach_vm_size_t size =
        msgr.msgr_msg_size + msgr.msgr_trailer_size + msgr.msgr_aux_size;
  ...
  if (kev->fflags == MACH_MSG_SUCCESS) {
    kev->ext[2] =
        ((uint64_t)msgr.msgr_priority << 32) |
...
static int filt_psetprocess(struct knote *kn, struct kevent_qos_s *kev) {
  ipc_object_t io = ips_to_object(kn->kn_ipc_pset);

  return filt_machportprocess(kn, kev, io, IOT_PORT_SET);
}

static int filt_portprocess(struct knote *kn, struct kevent_qos_s *kev) {
  ipc_object_t io = ip_to_object(kn->kn_ipc_port);

  return filt_machportprocess(kn, kev, io, IOT_PORT);
}

static void filt_machportsanitizedcopyout(struct knote *kn,
