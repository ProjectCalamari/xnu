...
struct ipc_importance_elem {
  ...
  mach_voucher_attr_value_reference_t
      iie_made;            /* references given to vouchers */
...
#define IIE_REF_DEBUG 0
  ...
  uint32_t iie_kmsg_refs_coalesced; /* kmsg refs coalesced into an existing
                                       inherit */
  uint32_t iie_kmsg_refs_dropped;   /* kmsg refs dropped by not accepting msg
                                       importance */
  uint32_t iie_task_refs_added;     /* refs added by a task reference call */
  uint32_t iie_task_refs_added_inherit_from; /* task references added by inherit
                                                from */
  uint32_t iie_task_refs_added_transition;   /* task references added by imp
                                                transition code */
  ...
  uint32_t iie_task_refs_coalesced; /* task refs coalesced into an existing
                                       inherit */
...
#define IIE_TYPE_INHERIT                                                       \
  0x00000001 /* Element inherits from a previous element */
...
struct ipc_importance_task {
  ...
  uint32_t iit_assertcnt; /* net number of boost assertions (internal, external
                             and legacy) */
  ...
  uint32_t iit_receiver : 1,      /* the task can receive importance boost */
      ...
      iit_donor
      : 1, /* the task always sends boosts regardless of boost status */
      iit_live_donor
      : 1, /* the task temporarily sends boosts regardless of boost status */
      iit_updatepolicy
      : 1, /* enqueue for policy update at the end of propagation */
...
#define IIT_LEGACY_EXTERN(t)                                                   \
  ((t)->iit_legacy_externcnt - (t)->iit_legacy_externdrop)
...
#define ipc_importance_task_release_internal(task_imp)                         \
  (assert(1 < IIT_REFS(task_imp)),                                             \
   ipc_importance_release_internal(&(task_imp)->iit_elem))
#endif
...
extern void ipc_importance_reset(ipc_importance_task_t task_imp,
                                 boolean_t donor);

extern ipc_importance_task_t ipc_importance_for_task(task_t task,
                                                     boolean_t made);
extern void ipc_importance_disconnect_task(task_t task);
extern ipc_importance_inherit_t
ipc_importance_exec_switch_task(task_t old_task, task_t new_task);
...
extern void ipc_importance_task_mark_donor(ipc_importance_task_t task_imp,
                                           boolean_t donating);
extern void ipc_importance_task_mark_live_donor(ipc_importance_task_t task_imp,
...
extern void ipc_importance_task_mark_receiver(ipc_importance_task_t task_imp,
                                              boolean_t receiving);
...
extern void
ipc_importance_task_mark_denap_receiver(ipc_importance_task_t task_imp,
                                        boolean_t receiving);
...
ipc_importance_task_hold_internal_assertion(ipc_importance_task_t task_imp,
                                            uint32_t count);
extern kern_return_t
...
extern kern_return_t ipc_importance_task_hold_legacy_external_assertion(
    ipc_importance_task_t task_imp, uint32_t count);
extern kern_return_t ipc_importance_task_drop_legacy_external_assertion(
    ipc_importance_task_t task_imp, uint32_t count);

extern boolean_t ipc_importance_check_circularity(ipc_port_t port,
                                                  ipc_port_t dest);

/* prepare importance attributes for sending */
extern boolean_t ipc_importance_send(ipc_kmsg_t kmsg,
                                     mach_msg_option64_t option);

/* receive importance attributes from message */
extern void ipc_importance_receive(ipc_kmsg_t kmsg, mach_msg_option64_t option);

/* undo receive of importance attributes from message */
extern void ipc_importance_unreceive(ipc_kmsg_t kmsg,
                                     mach_msg_option64_t option);
