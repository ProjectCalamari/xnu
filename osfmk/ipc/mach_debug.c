...
kern_return_t mach_port_get_srights(ipc_space_t space, mach_port_name_t name,
...
mach_port_space_info(ipc_space_t space, ipc_info_space_t *infop,
...
allocate_loop:
  for (;;) {
    ...
    table_size_needed = vm_map_round_page(tsize * sizeof(ipc_info_name_t),
                                          VM_MAP_PAGE_MASK(ipc_kernel_map));

    if ((table_size_needed <= table_size) && (pptrsize == tsize)) {
      break;
    }
    ...
    kr = kmem_alloc(ipc_kernel_map, &table_addr, table_size_needed, KMA_DATA,
                    VM_KERN_MEMORY_IPC);
    if (kr != KERN_SUCCESS) {
  ...
  for (mach_port_index_t index = 0; index < tsize; index++) {
    ...
    if ((bits & MACH_PORT_TYPE_PORT_RIGHTS) != MACH_PORT_TYPE_NONE &&
      ...
      ip_mq_lock(port);
      iin->iin_type |=
          ipc_port_request_type(port, iin->iin_name, entry->ie_request);
      ip_mq_unlock(port);
    }
  ...
  if (table_size > 0) {
    ...
    vm_map_size_t keep =
        vm_map_round_page(used, VM_MAP_PAGE_MASK(ipc_kernel_map));

    assert(pptrsize >= tsize);
    for (int index = 0; index < tsize; index++) {
      ipc_info_name_t *iin = &table_info[index];
      iin->iin_object =
          (natural_t)VM_KERNEL_ADDRHASH((uintptr_t)port_pointers[index]);
      port_pointers[index] = MACH_PORT_NULL;
    }
    kfree_type(ipc_object_t, pptrsize, port_pointers);

    if (keep < table_size) {
      kmem_free(ipc_kernel_map, table_addr + keep, table_size - keep);
      table_size = keep;
    }
    if (table_size > used) {
      bzero(&table_info[infop->iis_table_size], table_size - used);
    }

    kr = vm_map_unwire(ipc_kernel_map, table_addr, table_addr + table_size,
                       FALSE);
...
kern_return_t
mach_port_space_info_from_user(mach_port_t port, ipc_info_space_t *infop,
                               ipc_info_name_array_t *tablep,
                               mach_msg_type_number_t *tableCntp,
                               __unused ipc_info_tree_name_array_t *treep,
                               __unused mach_msg_type_number_t *treeCntp) {
...
kern_return_t mach_port_space_basic_info(ipc_space_t space,
...
kern_return_t mach_port_dnrequest_info(ipc_space_t space, mach_port_name_t name,
                                       unsigned int *totalp,
                                       unsigned int *usedp) {
  ...
  if (requests) {
    ...
    while ((ipr = ipc_port_request_table_next_elem(requests, ipr))) {
      if (ipr->ipr_soright != IP_NULL && ipr->ipr_name != IPR_HOST_NOTIFY) {
        used++;
      }
    }
...
static ipc_info_object_type_t mach_port_kobject_type(ipc_port_t port) {
#define MAKE_CASE(name)                                                        \
  case IKOT_##name:                                                            \
    return IPC_OTYPE_##name
...
static kern_return_t mach_port_kobject_description(
    ipc_space_t space, mach_port_name_t name, ipc_info_object_type_t *typep,
    mach_vm_address_t *addrp, kobject_description_t desc) {
  ipc_entry_bits_t bits;
  ...
  if (desc) {
    *desc = '\0';
    ...
    case IKOT_UEXT_OBJECT: {
      ...
      if (io_kobject) {
        ...
        io_object_t io_object =
            iokit_copy_object_for_consumed_kobject(io_kobject);
        io_kobject = NULL;
        if (io_object) {
    ...
    case IKOT_TASK_ID_TOKEN: {
      ...
      snprintf(desc, KOBJECT_DESCRIPTION_LENGTH, "%d,%llu,%d",
               token->ident.p_pid, token->ident.p_uniqueid,
               token->ident.p_idversion);
      break;
    }
    case IKOT_NAMED_ENTRY: {
      vm_named_entry_t named_entry =
          (vm_named_entry_t)ipc_kobject_get_stable(port, IKOT_NAMED_ENTRY);
...
kern_return_t mach_port_kobject_description_from_user(
    mach_port_t port, mach_port_name_t name, ipc_info_object_type_t *typep,
    mach_vm_address_t *addrp, kobject_description_t desc) {
  kern_return_t kr;
...
kern_return_t mach_port_kobject_from_user(mach_port_t port,
                                          mach_port_name_t name,
                                          ipc_info_object_type_t *typep,
                                          mach_vm_address_t *addrp) {
  return mach_port_kobject_description_from_user(port, name, typep, addrp,
                                                 NULL);
}

#if (DEVELOPMENT || DEBUG)
kern_return_t mach_port_special_reply_port_reset_link(
    ipc_space_t space, mach_port_name_t name, boolean_t *srp_lost_link) {
  ipc_port_t port;
  kern_return_t kr;
...
#else
kern_return_t
mach_port_special_reply_port_reset_link(__unused ipc_space_t space,
