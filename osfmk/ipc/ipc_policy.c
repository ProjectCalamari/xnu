...
extern int exit_with_guard_exception(struct proc *p,
...
TUNABLE(ipc_control_port_options_t, ipc_control_port_options,
        ...
        ICP_OPTIONS_IMMOVABLE_1P_HARD | ICP_OPTIONS_PINNED_1P_HARD |
#if !XNU_TARGET_OS_OSX
            ICP_OPTIONS_IMMOVABLE_3P_HARD |
#endif
            ICP_OPTIONS_PINNED_3P_SOFT);

TUNABLE(bool, service_port_defense_enabled, "-service_port_defense_enabled",
        true);
 ...
 */
TUNABLE(bool, bootstrap_port_telemetry_enabled,
        "-bootstrap_port_telemetry_enabled", true);
TUNABLE(bool, bootstrap_port_enforcement_enabled,
        "-bootstrap_port_enforcement_enabled", true);
...
ipc_space_policy_t ipc_policy_for_task(task_t task) {
#if XNU_TARGET_OS_OSX
...
mach_msg_option64_t ipc_current_msg_options(task_t task,
...
mach_msg_return_t ipc_preflight_msg_option64(mach_msg_option64_t opts) {
  bool success = true;
...
bool ipc_should_apply_policy(const ipc_space_policy_t current_policy,
                             const ipc_space_policy_t requested_level) {
  /* Do not apply security policies on these binaries to avoid bincompat
   * regression */
  ...
  if (requested_level & current_policy & IPC_SPACE_POLICY_ENHANCED) {
    ...
    const ipc_space_policy_t requested_version =
        requested_level & IPC_SPACE_POLICY_ENHANCED_VERSION_MASK;
    const ipc_space_policy_t current_es_version =
        current_policy & IPC_SPACE_POLICY_ENHANCED_VERSION_MASK;
...
CA_EVENT(mach_msg_trap_event, CA_INT, msgh_id, CA_INT, sw_platform, CA_INT, sdk,
...
static void mach_msg_legacy_send_analytics(mach_msg_id_t msgh_id,
...
static bool
ipc_policy_allow_legacy_mach_msg_trap_for_platform(mach_msg_id_t msgid) {
...
mach_msg_return_t ipc_policy_allow_legacy_send_trap(mach_msg_id_t msgid,
                                                    mach_msg_option64_t opts) {
 ...
 * we maintain a small buffer that gets flushed when full. This helps us avoid
 * taking spinlocks when working with CA.
 */
#define IPC_POLICY_VIOLATIONS_RB_SIZE 2

/*
 * Stripped down version of service port's string name. This is to avoid
 * overwhelming CA's dynamic memory allocation.
 */
...
struct ipc_policy_violations_rb_entry
    ipc_policy_violations_rb[IPC_POLICY_VIOLATIONS_RB_SIZE];
...
/*
 * Telemetry: report back the process name violating ipc policy. Note that this
 * event can be used to report any type of ipc violation through a
 * ipc_policy_violation_id_t. It is named reply_port_semantics_violations
 * because we are reusing an existing event.
 */
CA_EVENT(reply_port_semantics_violations, CA_STATIC_STRING(CA_PROCNAME_LEN),
         proc_name, CA_STATIC_STRING(CA_MACH_SERVICE_PORT_NAME_LEN),
         service_name, CA_STATIC_STRING(CA_TEAMID_MAX_LEN), team_id,
         CA_STATIC_STRING(CA_SIGNINGID_MAX_LEN), signing_id, CA_INT,
         reply_port_semantics_violation, CA_INT,
         msgh_id); /* for aux_data, keeping the legacy name msgh_id to avoid CA
                      shenanigan */

static void send_telemetry(const struct ipc_policy_violations_rb_entry *entry) {
  ca_event_t ca_event =
      CA_EVENT_ALLOCATE_FLAGS(reply_port_semantics_violations, Z_NOWAIT);
  if (ca_event) {
    CA_EVENT_TYPE(reply_port_semantics_violations) *event = ca_event->data;

    strlcpy(event->service_name, entry->service_name,
 ...
 */
static void flush_ipc_policy_violations_telemetry(void) {
  struct ipc_policy_violations_rb_entry local_rb[IPC_POLICY_VIOLATIONS_RB_SIZE];
  uint8_t local_rb_index = 0;

  if (__improbable(ipc_policy_violations_rb_index >
                   IPC_POLICY_VIOLATIONS_RB_SIZE)) {
    panic("Invalid ipc policy violation buffer index %d > %d",
          ipc_policy_violations_rb_index, IPC_POLICY_VIOLATIONS_RB_SIZE);
  }

  /*
   * We operate on local copy of telemetry buffer because CA framework relies on
   * successfully allocating zalloc memory. It can not do that if we are
   * accessing the shared buffer with spin locks held.
   */
  ...
  /*
   * Finally call out the buffer as empty. This is also a sort of rate limiting
   * mechanisms for the events. Events will get dropped until the buffer is not
   * fully flushed.
   */
...
void ipc_stash_policy_violations_telemetry(
    ipc_policy_violation_id_t violation_id, ipc_port_t service_port,
    int aux_data) {
  ...
  if (IP_VALID(service_port)) {
    ...
    if (io_state_active(label.io_state) &&
        ip_is_any_service_port_type(label.io_type)) {
...
#if DEBUG || DEVELOPMENT
void ipc_inc_telemetry_count(void) {
 ...
 * 2. You must use an IOT_REPLY_PORT (or weak variant) if the dest_port requires
 * it
 *
 ...
 * - false otherwise
 */
static mach_msg_return_t ipc_validate_local_port(mach_port_t reply_port,
                                                 mach_port_t dest_port,
                                                 mach_msg_option64_t opts) {
  assert(IP_VALID(dest_port));
  /* An empty reply port, or an inactive reply port / dest port violates nothing
   */
  if (!IP_VALID(reply_port) || !ip_active(reply_port) ||
      !ip_active(dest_port)) {
    return MACH_MSG_SUCCESS;
  }
  ...
  if (ip_is_kobject(dest_port) &&
    ...
    mach_port_guard_exception(ip_get_receiver_name(dest_port), 0,
                              kGUARD_EXC_KOBJECT_REPLY_PORT_SEMANTICS);
    return MACH_SEND_INVALID_REPLY;
  }

  if (!ipc_policy(dest_port)->pol_enforce_reply_semantics ||
  ...
  if (ip_is_bootstrap_port(dest_port) &&
    ...
    if (bootstrap_port_telemetry_enabled &&
        !ipc_space_has_telemetry_type(current_space(),
                                      IS_HAS_BOOTSTRAP_PORT_TELEMETRY)) {
      ipc_stash_policy_violations_telemetry(IPCPV_BOOTSTRAP_PORT, dest_port, 0);
    }
    if (bootstrap_port_enforcement_enabled) {
      mach_port_guard_exception(ip_get_receiver_name(dest_port), 1,
  ...
  if (!ip_is_bootstrap_port(dest_port)) {
    if (ip_type(dest_port) == IOT_SERVICE_PORT) {
      ipc_stash_policy_violations_telemetry(IPCPV_REPLY_PORT_SEMANTICS_OPTOUT,
                                            dest_port, 0);
    }
    mach_port_guard_exception(ip_get_receiver_name(dest_port), 0,
                              kGUARD_EXC_REQUIRE_REPLY_PORT_SEMANTICS);
...
mach_msg_return_t
ipc_validate_kmsg_header_schema_from_user(mach_msg_user_header_t *hdr __unused,
...
mach_msg_return_t
ipc_validate_kmsg_schema_from_user(mach_msg_header_t *kdata,
  ...
  if (os_add_overflow(send_uctx->send_dsc_vm_size,
                      send_uctx->send_dsc_port_count * sizeof(mach_port_t),
                      &vm_size)) {
    return MACH_SEND_TOO_LARGE;
  }
...
ipc_filter_kmsg_header_from_user(mach_msg_header_t *hdr, mach_port_t dport,
                                 mach_msg_option64_t opts) {
...
filtered_msg:
  if ((opts & MACH64_POLICY_FILTER_NON_FATAL) == 0) {
    ...
    mach_port_guard_exception(dest_name, hdr->msgh_id, kGUARD_EXC_MSG_FILTERED);
  }
  return MACH_SEND_MSG_FILTERED;
}

static bool ipc_policy_allow_send_only_kobject_calls(void) {
  struct proc_ro *pro = current_thread_ro()->tro_proc_ro;
  uint32_t sdk = pro->p_platform_data.p_sdk;
  uint32_t sdk_major = sdk >> 16;
...
static mach_msg_return_t
ipc_validate_kmsg_dest_from_user(mach_msg_header_t *hdr, ipc_port_t port,
                                 mach_msg_option64_t opts) {
  /*
   ...
   * since old simulators keep using mach_msg() in all cases
   * indiscriminatingly.)
   *
   * Since:
   ...
   *     (2) We checked in mach_msg2_trap() that _exactly_ one of the three bits
   * is set.
   *
  ...
  if (otype == IOT_TIMER_PORT) {
    ...
    if (otype == IKOT_UEXT_OBJECT) {
      ...
      if (__improbable((opts & MACH64_RCV_MSG) == 0)) {
        if ((opts & MACH64_POLICY_ENHANCED) || IP_VALID(hdr->msgh_local_port) ||
...
mach_msg_return_t
ipc_validate_kmsg_header_from_user(mach_msg_header_t *hdr,
                                   mach_msg_send_uctx_t *send_uctx,
                                   mach_msg_option64_t opts) {
  ...
  if (ool_port_array_enforced && send_uctx->send_dsc_port_arrays_count &&
...
bool ipc_move_receive_allowed(ipc_space_t space, ipc_port_t port,
   ...
   */
  if (service_port_defense_enabled && ip_type(port) == IOT_SERVICE_PORT &&
...
bool ipc_should_mark_immovable_send(task_t task, ipc_port_t port,
   ...
   */
  if ((!task_is_immovable(task) || !is_active(task->itk_space)) &&
...
static bool ip_is_currently_immovable_send(ipc_port_t port) {
...
bool ipc_can_stash_naked_send(ipc_port_t port) {
  return !IP_VALID(port) || !ip_is_currently_immovable_send(port);
}

#pragma mark entry init

void ipc_entry_init(ipc_space_t space, ipc_object_t object,
                    mach_port_type_t type, ipc_entry_t entry,
                    mach_port_urefs_t urefs, mach_port_name_t name) {
...
void mach_port_guard_exception(uint32_t target, uint64_t payload,
                               unsigned reason) {
...
void mach_port_guard_exception_immovable(ipc_space_t space,
                                         mach_port_name_t name,
                                         mach_port_t port,
  ...
  if (space == current_space()) {
    ...
    boolean_t hard = task_get_control_port_options(current_task()) &
...
void mach_port_guard_exception_pinned(ipc_space_t space, mach_port_name_t name,
                                      uint64_t payload) {
  ipc_space_policy_t policy = ipc_space_policy(space);
  int guard;
...
void mach_port_guard_ast(thread_t t, mach_exception_data_type_t code,
                         mach_exception_data_type_t subcode) {
  ...
  if (reason <= MAX_FATAL_kGUARD_EXC_CODE) {
    /*
     * Fatal Mach port guards - always delivered synchronously if dev mode is
     * on. Check if anyone has registered for Synchronous EXC_GUARD, if yes
     * then, deliver it synchronously and then kill the process, else kill the
     * process and deliver the exception via EXC_CORPSE_NOTIFY.
     */
    ...
    if (task_exception_notify(EXC_GUARD, code, subcode, fatal) ==
        KERN_SUCCESS) {
      flags |= PX_PSIGNAL;
    }
    ...
    while (behavior & TASK_EXC_GUARD_MP_ONCE) {
      ...
      if (os_atomic_cmpxchg(&task->task_exc_guard, behavior, new_behavior,
                            relaxed)) {
        break;
      }
...
static bool ipc_allow_service_port_register_pd(ipc_port_t service_port,
  ...
  if (ip_is_any_service_port(service_port)) {
    /* Only launchd should be able to register for port destroyed notification
     * on a service port. */
...
kern_return_t ipc_allow_register_pd_notification(ipc_port_t pd_port,
                                                 ipc_port_t notify_port) {
  uint64_t payload;
  ...
  if (ip_is_immovable_receive(pd_port) ||
    ...
    mach_port_guard_exception(ip_type(pd_port), MACH_NOTIFY_PORT_DESTROYED,
  ...
  if (ipc_port_has_prdrequest(pd_port)) {
    mach_port_guard_exception(0, MPG_FLAGS_KERN_FAILURE_MULTI_NOTI,
                              kGUARD_EXC_KERN_FAILURE);
    return KERN_FAILURE;
  }
...
__dead2 static void
no_kobject_no_senders(ipc_port_t port, mach_port_mscount_t mscount __unused) {
  panic("unexpected call to no_senders for object %p, type %d", port,
        ip_type(port));
}

__dead2 static void no_label_free(ipc_object_label_t label) {
...
__security_const_late struct ipc_object_policy ipc_policy_array[IOT_UNKNOWN] = {
    [IOT_PORT_SET] =
        {
            .pol_name = "port set",
    ...
    [IOT_SERVICE_PORT] =
        {
            ...
            .pol_enforce_reply_semantics =
                PENDING(true), /* pending on service port defense cleanup */
            .pol_notif_dead_name = true,
            .pol_notif_no_senders = true,
            .pol_notif_port_destroy = true,
        },
    ...
    [IOT_CONNECTION_PORT_WITH_PORT_ARRAY] =
        {
            ...
            .pol_movability = IPC_MOVE_POLICY_NEVER,
            .pol_movable_send = true,
            .pol_construct_entitlement =
                MACH_PORT_CONNECTION_PORT_WITH_PORT_ARRAY,
            .pol_notif_dead_name = true,
            .pol_notif_no_senders = true,
        },
    [IOT_EXCEPTION_PORT] =
    ...
    [IOT_PROVISIONAL_REPLY_PORT] =
        {
            ...
            .pol_movable_send = true,
            .pol_construct_entitlement =
                MACH_PORT_PROVISIONAL_REPLY_ENTITLEMENT,
            .pol_notif_dead_name = true,
            .pol_notif_no_senders = true,
            .pol_notif_port_destroy = true,
        },
...
__startup_func static void ipc_policy_update_from_tunables(void) {
  if (!service_port_defense_enabled) {
    ipc_policy_array[IOT_SERVICE_PORT].pol_movability = IPC_MOVE_POLICY_ALWAYS;
  }
}
 ...
 */
__startup_func static void ipc_policy_construct_entitlement_hardening(void) {
...
STARTUP(TUNABLES, STARTUP_RANK_LAST,
        ipc_policy_construct_entitlement_hardening);

__startup_func void ipc_kobject_register_startup(ipc_kobject_ops_t ops) {
...
__startup_func static void ipc_policy_set_defaults(void) {
  /*
...
bool ipc_is_valid_exception_port(task_t task, ipc_port_t port) {
  ...
  /*
   * rdar://77996387
   * Avoid exposing immovable ports send rights (kobjects) to
   * `get_exception_ports`, but exception ports to still be set.
   */
