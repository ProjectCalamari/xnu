...
IPC_KOBJECT_DEFINE(IKOT_VOUCHER, .iko_op_movable_send = true,
                   .iko_op_stable = true,
 ...
 */
static ipc_voucher_attr_manager_t
    ivam_global_table[MACH_VOUCHER_ATTR_KEY_NUM_WELL_KNOWN];
static struct ipc_voucher_attr_control
    ivac_global_table[MACH_VOUCHER_ATTR_KEY_NUM_WELL_KNOWN];
...
SMRH_TRAITS_DEFINE_MEM(voucher_traits, struct ipc_voucher, iv_hash_link,
                       .domain = &smr_ipc, .obj_hash = iv_obj_hash,
                       .obj_equ = iv_obj_equ, .obj_try_get = iv_obj_try_get);

os_refgrp_decl(static, iv_refgrp, "voucher", NULL);

static inline void iv_reference(ipc_voucher_t iv) {
  os_ref_retain_raw(&iv->iv_refs, &iv_refgrp);
}

static inline bool iv_try_reference(ipc_voucher_t iv) {
  return os_ref_retain_try_raw(&iv->iv_refs, &iv_refgrp);
}

static inline void iv_release(ipc_voucher_t iv) {
...
#define IV_HASH_VAL(sz, val) (((val) >> 3) % (sz))

static inline iv_index_t iv_hash_value(ipc_voucher_attr_control_t ivac,
 ...
 */
static inline iv_index_t iv_key_to_index(mach_voucher_attr_key_t key) {
  if (MACH_VOUCHER_ATTR_KEY_ALL == key || MACH_VOUCHER_ATTR_KEY_NUM < key) {
    return IV_UNUSED_KEYINDEX;
  }
  return (iv_index_t)key - 1;
}

static inline mach_voucher_attr_key_t iv_index_to_key(iv_index_t key_index) {
...
static void ivgt_lookup(iv_index_t, ipc_voucher_attr_manager_t *,
                        ipc_voucher_attr_control_t *);

static kern_return_t ipc_voucher_prepare_processing_recipe(
    ipc_voucher_t voucher, ipc_voucher_attr_raw_recipe_array_t recipes,
    ipc_voucher_attr_raw_recipe_array_size_t *in_out_size,
    mach_voucher_attr_recipe_command_t command,
    ipc_voucher_attr_manager_flags flags, int *need_processing);

__startup_func static void ipc_voucher_init(void) {
...
static ipc_voucher_t iv_alloc(void) {
  ipc_voucher_t iv;
 ...
 */
static void iv_set(ipc_voucher_t iv, iv_index_t key_index,
...
static smrh_key_t iv_key(ipc_voucher_t iv) {
  smrh_key_t key = {
...
static uint32_t iv_obj_hash(const struct smrq_slink *link, uint32_t seed) {
  ipc_voucher_t iv;
...
static bool iv_obj_equ(const struct smrq_slink *link, smrh_key_t key) {
  ipc_voucher_t iv;
...
static bool iv_obj_try_get(void *iv) { return iv_try_reference(iv); }
...
static void iv_dealloc(ipc_voucher_t iv, bool unhash) {
   ...
   */
  if (unhash) {
    smr_shash_remove(&voucher_table, &iv->iv_hash_link, &voucher_traits);
    KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_VOUCHER_DESTROY) |
                              DBG_FUNC_NONE,
                          VM_KERNEL_ADDRPERM((uintptr_t)iv), 0,
                          counter_load(&voucher_table.smrsh_count), 0, 0);
  }
  ...
  if (IP_VALID(port)) {
    ...
    ipc_kobject_dealloc_port(port, IPC_KOBJECT_NO_MSCOUNT, IKOT_VOUCHER);
 ...
 *		a lookup.
 */
static inline iv_index_t iv_lookup(ipc_voucher_t iv, iv_index_t key_index) {
  if (key_index < MACH_VOUCHER_ATTR_KEY_NUM) {
 ...
 */
uintptr_t unsafe_convert_port_to_voucher(ipc_port_t port) {
  if (IP_VALID(port)) {
...
static ipc_voucher_t ip_get_voucher(ipc_port_t port) {
 ...
 *		Port may or may not be locked.
 */
ipc_voucher_t convert_port_to_voucher(ipc_port_t port) {
...
ipc_voucher_t convert_port_name_to_voucher(mach_port_name_t voucher_name) {
  ipc_voucher_t iv;
  kern_return_t kr;
  ipc_port_t port;
...
void ipc_voucher_reference(ipc_voucher_t voucher) {
...
void ipc_voucher_release(ipc_voucher_t voucher) {
  if (IPC_VOUCHER_NULL != voucher) {
    iv_release(voucher);
  }
}
 ...
 *	on the voucher port.
 */
static void ipc_voucher_no_senders(ipc_port_t port,
 ...
 */
ipc_port_t convert_voucher_to_port(ipc_voucher_t voucher) {
  if (IV_NULL == voucher) {
    return IP_NULL;
  }
   ...
   */
  if (!ipc_kobject_make_send_lazy_alloc_port(&voucher->iv_port, voucher,
                                             IKOT_VOUCHER)) {
    ipc_voucher_release(voucher);
  }
 ...
 */
static void
ivace_lookup_values(ipc_voucher_attr_control_t ivac, iv_index_t value_index,
 ...
 */
static inline ivac_entry_t ivace_lookup(ipc_voucher_attr_control_t ivac,
                                        iv_index_t index) {
 ...
 */
static void ivac_grow_table(ipc_voucher_attr_control_t ivac) {
  iv_index_t i = 0;
  ...
  if (ivac->ivac_table_size >= IVAC_ENTRIES_MAX) {
    panic("Cannot grow ipc space beyond IVAC_ENTRIES_MAX. Some process is "
          "leaking vouchers");
    return;
  }
 ...
 */
static void ivace_reference_by_index(iv_index_t key_index,
 ...
 */
static iv_index_t
ivace_reference_by_value(ipc_voucher_attr_control_t ivac,
                         mach_voucher_attr_value_handle_t value,
 ...
 */
static void ivgt_lookup(iv_index_t key_index, ipc_voucher_attr_manager_t *ivamp,
...
ipc_replace_voucher_value(ipc_voucher_t voucher, mach_voucher_attr_key_t key,
                          mach_voucher_attr_recipe_command_t command,
                          ipc_voucher_t prev_voucher,
                          mach_voucher_attr_content_t content,
                          mach_voucher_attr_content_size_t content_size) {
  mach_voucher_attr_value_handle_t
      previous_vals[MACH_VOUCHER_ATTR_VALUE_MAX_NESTED];
  ...
  prev_val_index = (IV_NULL != prev_voucher)
                       ...
                       : save_val_index;
  ivace_lookup_values(ivac, prev_val_index, previous_vals,
                      &previous_vals_count);

  /* Call out to resource manager to get new value */
  new_value_voucher = IV_NULL;
  kr = (ivam->ivam_get_value)(ivam, key, command, previous_vals,
                              previous_vals_count, content, content_size,
 ...
 */
static kern_return_t
ipc_directly_replace_voucher_value(ipc_voucher_t voucher,
                                   mach_voucher_attr_key_t key,
                                   mach_voucher_attr_value_handle_t new_value) {
  ipc_voucher_attr_manager_t ivam;
  ipc_voucher_attr_control_t ivac;
...
static kern_return_t ipc_execute_voucher_recipe_command(
    ipc_voucher_t voucher, mach_voucher_attr_key_t key,
    mach_voucher_attr_recipe_command_t command, ipc_voucher_t prev_iv,
    mach_voucher_attr_content_t content,
    mach_voucher_attr_content_size_t content_size, boolean_t key_priv) {
  iv_index_t prev_val_index;
  iv_index_t val_index;
  kern_return_t kr;
  ...
  case MACH_VOUCHER_ATTR_SET_VALUE_HANDLE:
    if (key_priv) {
      ...
      kr = ipc_directly_replace_voucher_value(voucher, key, new_value);
      if (KERN_SUCCESS != kr) {
        return kr;
      }
    } else {
  ...
  case MACH_VOUCHER_ATTR_REDEEM:
    ...
    if (MACH_VOUCHER_ATTR_KEY_ALL == key) {
      ...
      for (iv_index_t j = 0; j < MACH_VOUCHER_ATTR_KEY_NUM; j++) {
        ...
        kr = ipc_replace_voucher_value(voucher, j_key, command, prev_iv,
   ...
   */
  default:
    kr = ipc_replace_voucher_value(voucher, key, command, prev_iv, content,
                                   content_size);
    if (KERN_SUCCESS != kr) {
      return kr;
    }
 ...
 */
static ipc_voucher_t iv_dedup(ipc_voucher_t new_iv) {
  ipc_voucher_t dupe_iv;

  dupe_iv = smr_shash_get_or_insert(&voucher_table, iv_key(new_iv),
                                    &new_iv->iv_hash_link, &voucher_traits);
  if (dupe_iv) {
...
#define PAYLOAD_SIZE 1024
      ...
      static_assert(PAYLOAD_SIZE % PAYLOAD_PER_TRACEPOINT == 0,
      ...
      kr = mach_voucher_extract_all_attr_recipes(
          new_iv, (mach_voucher_attr_raw_recipe_array_t)payload, &payload_size);
      if (KERN_SUCCESS == kr) {
        attr_tracepoints_needed = (payload_size + PAYLOAD_PER_TRACEPOINT - 1) /
      ...
      KDBG(MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_VOUCHER_CREATE), voucher_addr,
           ivht_count, payload_size);

      uintptr_t index = 0;
      while (attr_tracepoints_needed--) {
        KDBG(MACHDBG_CODE(DBG_MACH_IPC, MACH_IPC_VOUCHER_CREATE_ATTR_DATA),
 ...
 *		recipe port name that needs to be converted to a voucher.
 * Because we can't rely on the port namespace to hold a reference on each
 *		previous voucher port for the duration of processing that
 * command, we have to convert the name to a voucher reference and release it
 ...
 */
static kern_return_t ipc_create_mach_voucher_internal(
    ipc_voucher_attr_control_t control, uint8_t *recipes, size_t recipe_size,
    bool is_user_recipe, ipc_voucher_t *new_voucher) {
   ...
   */
  recipe_struct_size =
      (is_user_recipe) ? sizeof(*sub_recipe_user) : sizeof(*sub_recipe_kernel);
  ...
  while (0 < recipe_size - recipe_used) {
    ...
    if (is_user_recipe) {
       ...
       * reference
       */
      prev_iv = convert_port_name_to_voucher(sub_recipe_user->previous_voucher);
      ...
      kr = ipc_execute_voucher_recipe_command(
          voucher, sub_recipe_user->key, sub_recipe_user->command, prev_iv,
          sub_recipe_user->content, sub_recipe_user->content_size, key_priv);
      ...
      kr = ipc_execute_voucher_recipe_command(
          voucher, sub_recipe_kernel->key, sub_recipe_kernel->command,
 ...
 */
kern_return_t
ipc_create_mach_voucher(ipc_voucher_attr_raw_recipe_array_t recipes,
                        ipc_voucher_attr_raw_recipe_array_size_t recipe_size,
                        ipc_voucher_t *new_voucher) {
  return ipc_create_mach_voucher_internal(
      IPC_VOUCHER_ATTR_CONTROL_NULL, recipes, recipe_size, false, new_voucher);
}
 ...
 *		Previous vouchers are passed as voucher indexes.
 */
kern_return_t ipc_voucher_attr_control_create_mach_voucher(
  ...
  return ipc_create_mach_voucher_internal(control, recipes, recipe_size, false,
                                          new_voucher);
}
 ...
 */
void ipc_register_well_known_mach_voucher_attr_manager(
    ipc_voucher_attr_manager_t manager,
    mach_voucher_attr_value_handle_t default_value, mach_voucher_attr_key_t key,
    ipc_voucher_attr_control_t *control) {
  ipc_voucher_attr_control_t ivac;
  iv_index_t key_index;
  iv_index_t hash_index;
...
kern_return_t mach_voucher_extract_attr_content(
   ...
   */
  ivace_lookup_values(ivac, value_index, vals, &vals_count);
  assert(0 < vals_count);

  /* callout to manager */

  kr = (manager->ivam_extract_content)(manager, key, vals, vals_count, &command,
                                       content, in_out_size);
  return kr;
}
...
kern_return_t mach_voucher_extract_attr_recipe(
  ...
  /* callout to manager */
  kr = (manager->ivam_extract_content)(manager, key, vals, vals_count,
                                       &recipe->command, recipe->content,
                                       &recipe->content_size);
  if (KERN_SUCCESS == kr) {
...
kern_return_t mach_voucher_extract_all_attr_recipes(
    ipc_voucher_t voucher, mach_voucher_attr_raw_recipe_array_t recipes,
  ...
  for (iv_index_t key_index = 0; key_index < MACH_VOUCHER_ATTR_KEY_NUM;
       key_index++) {
    mach_voucher_attr_value_handle_t vals[MACH_VOUCHER_ATTR_VALUE_MAX_NESTED];
    mach_voucher_attr_value_handle_array_size_t vals_count;
    mach_voucher_attr_content_size_t content_size;
    ...
    if (os_sub3_overflow(recipe_size, recipe_used, sizeof(*recipe),
                         &content_size)) {
 ...
 *	Purpose:
 *		Extract all the (non-default) contents for a given mach port
 * name, building up a recipe that could be provided to a future voucher
 * creation call. Conditions: Nothing locked (may invoke user-space). Caller may
 * not hold a reference on the supplied voucher.
 */
#if !(DEVELOPMENT || DEBUG)
kern_return_t mach_voucher_debug_info(
    ipc_space_t __unused space, mach_port_name_t __unused voucher_name,
...
#else
kern_return_t mach_voucher_debug_info(
    ipc_space_t space, mach_port_name_t voucher_name,
 ...
 */
kern_return_t
mach_voucher_attr_command(ipc_voucher_t voucher, mach_voucher_attr_key_t key,
   ...
   */
  value_index = iv_lookup(voucher, key_index);
  ivace_lookup_values(control, value_index, vals, &vals_count);

  /* callout to manager */
  kr = (manager->ivam_command)(manager, key, vals, vals_count, command,
                               in_content, in_content_size, out_content,
                               out_content_size);
...
kern_return_t mach_voucher_attr_control_get_values(
    ipc_voucher_attr_control_t control, ipc_voucher_t voucher,
  ...
  ivace_lookup_values(control, value_index, out_values, in_out_size);
  return KERN_SUCCESS;
}
 ...
 */
kern_return_t
host_create_mach_voucher(host_t host,
                         mach_voucher_attr_raw_recipe_array_t recipes,
  ...
  return ipc_create_mach_voucher_internal(
      IPC_VOUCHER_ATTR_CONTROL_NULL, recipes, recipe_size, true, new_voucher);
}
 ...
 *	Purpose:
 *		Get the canonicalized pthread priority from the voucher attached
 * in the kmsg.
 */
kern_return_t ipc_get_pthpriority_from_kmsg_voucher(
    ipc_kmsg_t kmsg,
  ...
  memcpy(canonicalize_priority_value, cur_content->content,
         sizeof(ipc_pthread_priority_value_t));
 ...
 */
ipc_voucher_t ipc_voucher_get_default_voucher(void) {
  ...
  kr = ipc_create_mach_voucher(recipes, recipe_size, &recv_voucher);
 ...
 */
void ipc_voucher_send_preprocessing(ipc_kmsg_t kmsg) {
  uint8_t recipes[(MACH_VOUCHER_ATTR_KEY_NUM + 1) *
                  sizeof(ipc_voucher_attr_recipe_data_t)];
  ipc_voucher_attr_raw_recipe_array_size_t recipe_size =
      (MACH_VOUCHER_ATTR_KEY_NUM + 1) * sizeof(ipc_voucher_attr_recipe_data_t);
  ...
  kr = ipc_voucher_prepare_processing_recipe(
      pre_processed_voucher, (mach_voucher_attr_raw_recipe_array_t)recipes,
  ...
  if (need_preprocessing) {
    kr = ipc_create_mach_voucher(recipes, recipe_size, &voucher_to_send);
    assert(KERN_SUCCESS == kr);
 ...
 *		in order to maintain symmetry with
 * ipc_voucher_send_preprocessing.
 */
void ipc_voucher_receive_postprocessing(ipc_kmsg_t kmsg,
  ...
  kr = ipc_voucher_prepare_processing_recipe(
      sent_voucher, (mach_voucher_attr_raw_recipe_array_t)recipes, &recipe_size,
      MACH_VOUCHER_ATTR_AUTO_REDEEM, IVAM_FLAGS_SUPPORT_RECEIVE_POSTPROCESS,
      &need_postprocessing);
 ...
 */
void mach_init_activity_id(void) { voucher_activity_id = 1; }
 ...
 */
kern_return_t
mach_generate_activity_id(struct mach_generate_activity_id_args *args) {
  ...
  activity_id = os_atomic_add_orig(&voucher_activity_id, args->count, relaxed);
...
/* User data manager is removed on !macOS */
    ...
    defined(MACH_VOUCHER_ATTR_KEY_TEST)
...
#define user_data_lock() lck_spin_lock_grp(&user_data_lock_data, &ipc_lck_grp)
#define user_data_lock_try()                                                   \
  lck_spin_try_lock_grp(&user_data_lock_data, &ipc_lck_grp)
#define user_data_unlock() lck_spin_unlock(&user_data_lock_data)

static kern_return_t
user_data_release_value(ipc_voucher_attr_manager_t manager,
                        mach_voucher_attr_key_t key,
                        mach_voucher_attr_value_handle_t value,
                        mach_voucher_attr_value_reference_t sync);

static kern_return_t user_data_get_value(
...
static kern_return_t user_data_extract_content(
    ipc_voucher_attr_manager_t manager, mach_voucher_attr_key_t key,
    mach_voucher_attr_value_handle_array_t values,
...
static kern_return_t user_data_command(
    ipc_voucher_attr_manager_t manager, mach_voucher_attr_key_t key,
    mach_voucher_attr_value_handle_array_t values,
...
static void user_data_value_element_free(user_data_element_t elem) {
 ...
 */
static kern_return_t
user_data_release_value(ipc_voucher_attr_manager_t __assert_only manager,
  ...
  user_data_lock();
  if (sync == elem->e_made) {
    queue_remove(&user_data_bucket[hash], elem, user_data_element_t,
                 e_hash_link);
    user_data_unlock();
 ...
 */
static iv_index_t
user_data_checksum(mach_voucher_attr_content_t content,
                   mach_voucher_attr_content_size_t content_size) {
...
user_data_dedup(mach_voucher_attr_content_t content,
                mach_voucher_attr_content_size_t content_size) {
  iv_index_t sum;
  iv_index_t hash;
  user_data_element_t elem;
...
retry:
  user_data_lock();
  queue_iterate(&user_data_bucket[hash], elem, user_data_element_t,
                e_hash_link) {
  ...
  if (NULL == alloc) {
    ...
    alloc = kalloc_type(struct user_data_value_element, Z_WAITOK | Z_NOFAIL);
    alloc->e_made = 1;
...
static kern_return_t
user_data_command(ipc_voucher_attr_manager_t __assert_only manager,
                  mach_voucher_attr_key_t __assert_only key,
...
__startup_func static void user_data_attr_manager_init(void) {
#if defined(MACH_VOUCHER_ATTR_KEY_USER_DATA)
  ipc_register_well_known_mach_voucher_attr_manager(
      &user_data_manager, (mach_voucher_attr_value_handle_t)0,
      MACH_VOUCHER_ATTR_KEY_USER_DATA, &user_data_control);
#endif
#if defined(MACH_VOUCHER_ATTR_KEY_TEST)
  ipc_register_well_known_mach_voucher_attr_manager(
      &user_data_manager, (mach_voucher_attr_value_handle_t)0,
      MACH_VOUCHER_ATTR_KEY_TEST, &test_control);
#endif
#if defined(MACH_VOUCHER_ATTR_KEY_USER_DATA) ||                                \
    defined(MACH_VOUCHER_ATTR_KEY_TEST)
