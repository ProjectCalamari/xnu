...
#define COALITION_CREATE_FLAGS_GET_ROLE(flags)                                 \
  (((flags) & COALITION_CREATE_FLAGS_ROLE_MASK) >>                             \
   COALITION_CREATE_FLAGS_ROLE_SHIFT)
...
#define COALITION_TASKROLE_NONE                                                \
  (-1) /* task plays no role in the given coalition */
...
struct coalition_resource_usage {
  ...
  uint64_t
      gpu_energy_nj_billed_to_me; /* nanojoules that others did on my behalf */
  uint64_t gpu_energy_nj_billed_to_others; /* nanojoules that I did on others'
                                              behalf */
  uint64_t swapins;
};
...
#if COALITION_DEBUG
#define coal_dbg(fmt, ...) printf("%s: " fmt "\n", __func__, ##__VA_ARGS__)
#else
#define coal_dbg(fmt, ...)
#endif
...
extern bool coalition_add_to_gpu_energy(uint64_t coalition_id,
...
typedef struct coalition_pend_token {
  uint32_t cpt_update_timers : 1, cpt_update_j_coal_tasks : 1;
