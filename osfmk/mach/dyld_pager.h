...
struct mwl_region {
  ...
  mach_vm_address_t mwlr_address
      __kernel_data_semantics; /* start address of existing region */
...
struct mwl_info_hdr {
  ...
  uint32_t
      mwli_binds_offset; /* offset within this blob of bind pointers table */
  uint32_t mwli_binds_count;   /* number of pointers in bind pointers table (for
                                  range checks) */
  uint32_t mwli_chains_offset; /* offset within this blob of
                                  dyld_chained_starts_in_image */
  uint32_t mwli_chains_size;   /* size of dyld_chained_starts_in_image */
  uint64_t mwli_slide;         /* slide to add to rebased pointers */
  uint64_t
      mwli_image_address; /* add this to rebase offsets includes any slide */
  /* followed by the binds pointers and dyld_chained_starts_in_image */
};

#define MWL_MAX_REGION_COUNT                                                   \
  5 /* data, const, data auth, auth const, objc const */

#ifndef KERNEL_PRIVATE

extern int __map_with_linking_np(const struct mwl_region regions[],
...
/*
 * Special value for dyld to use with shared_region_check_np() to prevent
 * anymore use of map_with_linking_np() in a process
 */
