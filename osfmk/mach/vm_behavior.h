 ...
 *
 *  When making a new VM_BEHAVIOR_*, update tests
 * vm_parameter_validation_[user|kern] and their expected results; they
 * deliberately call VM functions with invalid behavior values and you may be
 * turning one of those invalid behaviors valid.
 */
...
#define VM_BEHAVIOR_FREE ((vm_behavior_t)6) /* free memory without write-back  \
                                             */
#define VM_BEHAVIOR_ZERO_WIRED_PAGES                                           \
  ((vm_behavior_t)7) /* zero out the wired pages of an entry if it is being    \
                        deleted without unwiring them first */
...
#define VM_BEHAVIOR_PAGEOUT                                                    \
  ((vm_behavior_t)11) /* force page-out of the pages in range (development     \
                         only) */
#define VM_BEHAVIOR_ZERO                                                       \
  ((vm_behavior_t)12) /* zero pages without faulting in additional pages */
