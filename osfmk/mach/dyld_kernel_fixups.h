...
sign_pointer(struct dyld_chained_ptr_64_kernel_cache_rebase pointer __unused,
             void *loc __unused, uint64_t target __unused) {
#if HAS_APPLE_PAC
  ...
  switch (pointer.key) {
  case 0: // IA
    return (uint64_t)__builtin_ptrauth_sign_unauthenticated((void *)target, 0,
                                                            discriminator);
  case 1: // IB
    return (uint64_t)__builtin_ptrauth_sign_unauthenticated((void *)target, 1,
                                                            discriminator);
  case 2: // DA
    return (uint64_t)__builtin_ptrauth_sign_unauthenticated((void *)target, 2,
                                                            discriminator);
  case 3: // DB
    return (uint64_t)__builtin_ptrauth_sign_unauthenticated((void *)target, 3,
                                                            discriminator);
  }
#endif
  return target;
...
fixup_value(union ChainedFixupPointerOnDisk *fixupLoc __unused,
            ...
            const void *basePointers[KCNumKinds] __unused, int *stop) {
  if (LogFixups) {
  ...
  case DYLD_CHAINED_PTR_X86_64_KERNEL_CACHE: {
    ...
    if (LogFixups) {
      dyldLogFunc("[LOG] kernel-fixups: slidValue %p (base=%p, target=%p)\n",
                  (void *)slidValue, (const void *)baseAddress,
                  (void *)(uintptr_t)fixupLoc->fixup64.target);
    }
#if HAS_APPLE_PAC
...
#endif // __LP64__
  default:
    dyldLogFunc("unsupported pointer chain format: 0x%04X",
...
walk_chain(const struct mach_header_64 *mh,
           const struct dyld_chained_starts_in_segment *segInfo,
           uint32_t pageIndex, uint16_t offsetInPage, uintptr_t slide __unused,
           const void *basePointers[KCNumKinds]) {
  if (LogFixups) {
    dyldLogFunc("[LOG] kernel-fixups: walk_chain page[%d]\n", pageIndex);
  }
  int stop = 0;
  uintptr_t pageContentStart = (uintptr_t)mh +
                               (uintptr_t)segInfo->segment_offset +
                               (pageIndex * segInfo->page_size);
  union ChainedFixupPointerOnDisk *chain =
      (union ChainedFixupPointerOnDisk *)(pageContentStart + offsetInPage);
  int chainEnd = 0;
  if (LogFixups) {
    ...
    if (!stop) {
      ...
      case DYLD_CHAINED_PTR_64_KERNEL_CACHE:
        if (chainContent.fixup64.next == 0) {
          ...
          chain =
              (union ChainedFixupPointerOnDisk *)((uintptr_t)chain +
                                                  chainContent.fixup64.next *
                                                      4);
        }
        break;
      case DYLD_CHAINED_PTR_X86_64_KERNEL_CACHE:
        if (chainContent.fixup64.next == 0) {
          ...
          if (LogFixups) {
            dyldLogFunc(
                "[LOG] kernel-fixups: chainContent fixup x86 64.next %d\n",
                chainContent.fixup64.next);
          }
          chain =
              (union ChainedFixupPointerOnDisk *)((uintptr_t)chain +
...
kernel_collection_slide(const struct mach_header_64 *mh,
  ...
  if (mh->magic == MH_MAGIC_64) {
    ...
    startCmds =
        (struct load_command *)((uintptr_t)mh + sizeof(struct mach_header));
  } else {
    // const uint32_t* h = (uint32_t*)mh;
    // diag.error("file does not start with MH_MAGIC[_64]: 0x%08X 0x%08X", h[0],
    // h [1]);
    return 1; // not a mach-o file
  }
  const struct load_command *const cmdsEnd =
      (struct load_command *)((uintptr_t)startCmds + mh->sizeofcmds);
  const struct load_command *cmd = startCmds;
  for (uint32_t i = 0; i < mh->ncmds; ++i) {
    if (LogFixups) {
      dyldLogFunc(
          "[LOG] kernel-fixups: parsing load command %d with cmd=0x%x\n", i,
          cmd->cmd);
    }
    const struct load_command *nextCmd =
        (struct load_command *)((uintptr_t)cmd + cmd->cmdsize);
    if (cmd->cmdsize < 8) {
      // diag.error("malformed load command #%d of %d at %p with mh=%p, size
      // (0x%X) too small", i, this->ncmds, cmd, this, cmd->cmdsize);
      return 1;
    }
    if ((nextCmd > cmdsEnd) || (nextCmd < startCmds)) {
      // diag.error("malformed load command #%d of %d at %p with mh=%p, size
      // (0x%X) is too large, load commands end at %p", i, this->ncmds, cmd,
      // this, cmd->cmdsize, cmdsEnd);
      return 1;
    }
    if (cmd->cmd == LC_DYLD_CHAINED_FIXUPS) {
      ...
      const struct segment_command_64 *seg =
          (const struct segment_command_64 *)(uintptr_t)cmd;

      if (LogFixups) {
        dyldLogFunc("[LOG] kernel-fixups: segment name vm start and size: %s "
                    "0x%llx 0x%llx\n",
  ...
  const struct dyld_chained_fixups_header *fixupsHeader =
      (const struct dyld_chained_fixups_header *)(linkeditStartAddr +
                                                  offsetInLinkedit);
  const struct dyld_chained_starts_in_image *fixupStarts =
      (const struct dyld_chained_starts_in_image
           *)((uintptr_t)fixupsHeader + fixupsHeader->starts_offset);
  if (LogFixups) {
  ...
  int stopped = 0;
  for (uint32_t segIndex = 0; segIndex < fixupStarts->seg_count && !stopped;
       ++segIndex) {
    if (LogFixups) {
    ...
    const struct dyld_chained_starts_in_segment *segInfo =
        (const struct dyld_chained_starts_in_segment
             *)((uintptr_t)fixupStarts +
                fixupStarts->seg_info_offset[segIndex]);
    for (uint32_t pageIndex = 0; pageIndex < segInfo->page_count && !stopped;
         ++pageIndex) {
...
kernel_collection_adjust_fileset_entry_addrs(struct mach_header_64 *mh,
                                             uintptr_t adj) {
  ...
  for (i = 0; i < mh->ncmds;
       i++, lc = (struct load_command *)((uintptr_t)lc + lc->cmdsize)) {
    if (lc->cmd == LC_SYMTAB) {
    ...
    if (strings_are_equal(((struct segment_command_64 *)(uintptr_t)lc)->segname,
                          SEG_LINKEDIT)) {
  ...
  if (symtab_cmd != NULL && linkedit_cmd != NULL) {
    ...
    sym = (struct nlist_64 *)(linkedit_cmd->vmaddr + symtab_cmd->symoff -
                              linkedit_cmd->fileoff);
...
MARK_AS_FIXUP_TEXT static void kernel_collection_adjust_mh_addrs(
    struct mach_header_64 *kc_mh, uintptr_t adj, bool pageable,
    uintptr_t *kc_lowest_vmaddr, uintptr_t *kc_highest_vmaddr,
  ...
  uintptr_t lowest_vmaddr = UINTPTR_MAX, highest_vmaddr = 0,
            highest_nle_vmaddr = 0;
  ...
  for (i = 0; i < kc_mh->ncmds;
    ...
    if (lc->cmd == LC_FILESET_ENTRY) {
      ...
      if (LogFixups) {
        dyldLogFunc(
            "[MH] sliding %s",
            (char *)((uintptr_t)fse + (uintptr_t)(fse->entry_id.offset)));
      }
       ...
       */
      if (seg->vmaddr < lowest_rx_vmaddr &&
          (lowest_rx_vmaddr <= seg->vmaddr + seg->vmsize ||
           lowest_rx_vmaddr == UINTPTR_MAX)) {
        lowest_rx_vmaddr = (uintptr_t)seg->vmaddr;
      }
      if (seg->vmaddr + seg->vmsize > highest_rx_vmaddr &&
          (highest_rx_vmaddr >= seg->vmaddr || highest_rx_vmaddr == 0)) {
...
static void rebase_chain(uintptr_t chainStartAddress, uint64_t stepMultiplier,
                         uintptr_t baseAddress __unused, uint64_t slide) {
  uint64_t delta = 0;
    ...
    if (isRebase) {
      if (isAuthenticated) {
        ...
        if (hasAddressDiversity) {
          // First calculate a new discriminator using the address of where we
          // are trying to store the value Only blend if we have a discriminator
          if (discriminator) {
            discriminator = __builtin_ptrauth_blend_discriminator(
                (void *)address, discriminator);
          } else {
        ...
        case ptrauth_key_asia:
          newValue = (uintptr_t)__builtin_ptrauth_sign_unauthenticated(
              (void *)newValue, ptrauth_key_asia, discriminator);
          break;
        case ptrauth_key_asib:
          newValue = (uintptr_t)__builtin_ptrauth_sign_unauthenticated(
              (void *)newValue, ptrauth_key_asib, discriminator);
          break;
        case ptrauth_key_asda:
          newValue = (uintptr_t)__builtin_ptrauth_sign_unauthenticated(
              (void *)newValue, ptrauth_key_asda, discriminator);
          break;
        case ptrauth_key_asdb:
          newValue = (uintptr_t)__builtin_ptrauth_sign_unauthenticated(
              (void *)newValue, ptrauth_key_asdb, discriminator);
          break;
        }
#endif
        *(uint64_t *)address = newValue;
        ...
        uint64_t targetValue =
            (top8Bits << 13) |
            (((intptr_t)(bottom43Bits << 21) >> 21) & 0x00FFFFFFFFFFFFFF);
...
static bool __unused rebase_threaded_starts(uint32_t *threadArrayStart,
                                            uint32_t *threadArrayEnd,
                                            uintptr_t macho_header_addr,
  ...
  for (uint32_t *threadOffset = threadArrayStart + 1;
       threadOffset != threadArrayEnd; ++threadOffset) {
    if (*threadOffset == 0xFFFFFFFF) {
      break;
    }
    rebase_chain(macho_header_addr + *threadOffset, stepMultiplier,
                 macho_header_vmaddr, slide);
  }
  return true;
}
