...
#define encode_eventlink_count_and_error(count, error)                         \
  (((count) & EVENTLINK_SIGNAL_COUNT_MASK) |                                   \
   ((((uint64_t)error) & EVENTLINK_SIGNAL_ERROR_MASK)                          \
    << EVENTLINK_SIGNAL_ERROR_SHIFT))
...
#define decode_eventlink_error_from_retval(retval)                             \
  ((kern_return_t)(((retval) >> EVENTLINK_SIGNAL_ERROR_SHIFT) &                \
                   EVENTLINK_SIGNAL_ERROR_MASK))
...
kern_return_t
mach_eventlink_wait_until(mach_port_t eventlink_port, uint64_t *count_ptr,
...
mach_eventlink_signal_wait_until(mach_port_t eventlink_port,
                                 uint64_t *count_ptr, uint64_t signal_count,
                                 mach_eventlink_signal_wait_option_t option,
                                 kern_clock_id_t clock_id, uint64_t deadline);
