...
struct vm_statistics64 {
  ...
  uint64_t
      swapins; /* # of pages swapped in via compressor segments (lifetime) */
  uint64_t
      swapouts; /* # of pages swapped out via compressor segments (lifetime) */
  natural_t compressor_page_count; /* # of pages used by the compressed pager to
                                      hold all the compressed data */
  natural_t throttled_count;       /* # of pages throttled */
  natural_t
      external_page_count; /* # of pages that are file-backed (non-swap) */
  natural_t internal_page_count; /* # of pages that are anonymous */
  uint64_t
      total_uncompressed_pages_in_compressor; /* # of pages (uncompressed) held
                                                 within the compressor. */
  /* added for rev2 */
  uint64_t
      swapped_count; /* # of compressor-stored pages currently stored in swap */
 ...
 *
 * This is used by host_statistics() to truncate and peg the 64-bit in-kernel
 * values from vm_statistics64 to the 32-bit values of the older structure above
 * (vm_statistics).
 */
...
struct vm_extmod_statistics {
  ...
  int64_t
      task_for_pid_caller_count; /* # of times this task called task_for_pid */
...
#define VM_FLAGS_SUPERPAGE_MASK 0x00070000 /* bits 0x10000, 0x20000, 0x40000   \
                                            */
#define VM_FLAGS_RETURN_DATA_ADDR                                              \
  0x00100000 /* Return address of target data, rather than base of page */
#define VM_FLAGS_RETURN_4K_DATA_ADDR                                           \
  0x00800000 /* Return 4K aligned address of target data */
 ...
 */
#define VM_FLAGS_ANY_MASK                                                      \
  (VM_FLAGS_FIXED | VM_FLAGS_ANYWHERE | VM_FLAGS_PURGABLE |                    \
   VM_FLAGS_4GB_CHUNK | VM_FLAGS_RANDOM_ADDR | VM_FLAGS_NO_CACHE |             \
   VM_FLAGS_RESILIENT_CODESIGN | VM_FLAGS_RESILIENT_MEDIA |                    \
   VM_FLAGS_PERMANENT | VM_FLAGS_TPRO | VM_FLAGS_OVERWRITE |                   \
   VM_FLAGS_SUPERPAGE_MASK | VM_FLAGS_RETURN_DATA_ADDR |                       \
   VM_FLAGS_RETURN_4K_DATA_ADDR | VM_FLAGS_ALIAS_MASK)
#endif /* XNU_KERNEL_PRIVATE */
...
#define VM_FLAGS_USER_ALLOCATE                                                 \
  (VM_FLAGS_FIXED | VM_FLAGS_ANYWHERE | VM_FLAGS_PURGABLE |                    \
   VM_FLAGS_4GB_CHUNK | VM_FLAGS_RANDOM_ADDR | VM_FLAGS_NO_CACHE |             \
   VM_FLAGS_PERMANENT | VM_FLAGS_OVERWRITE | VM_FLAGS_SUPERPAGE_MASK |         \
   VM_FLAGS_HW | VM_FLAGS_ALIAS_MASK)

#define VM_FLAGS_USER_MAP                                                      \
  (VM_FLAGS_USER_ALLOCATE | VM_FLAGS_RETURN_4K_DATA_ADDR |                     \
   VM_FLAGS_RETURN_DATA_ADDR)

#define VM_FLAGS_USER_REMAP                                                    \
...
__enum_decl(
    virtual_memory_guard_exception_code_t, uint32_t,
    {
        ...
        /*
         * rdar://151450801 (Remove spurious kGUARD_EXC_SEC_ACCESS_FAULT and
         * kGUARD_EXC_SEC_ASYNC_ACCESS_FAULT once CrashReporter is aligned)
         */
...
#define KMEM_RANGE_MAX                                                         \
  (UMEM_RANGE_ID_MAX < KMEM_RANGE_ID_MAX ? KMEM_RANGE_ID_MAX                   \
                                         : UMEM_RANGE_ID_MAX)
...
typedef union {
  struct {
    unsigned long long
         ...
         */
        vmf_fixed : 1,
        vmf_purgeable : 1, vmf_4gb_chunk : 1, vmf_random_addr : 1,
        vmf_no_cache : 1, vmf_resilient_codesign : 1, vmf_resilient_media : 1,
        vmf_permanent : 1,

        __unused_bit_8 : 1, __unused_bit_9 : 1, __unused_bit_10 : 1,
        __unused_bit_11 : 1, vmf_tpro : 1, __unused_bit_13 : 1,
        vmf_overwrite : 1, __unused_bit_15 : 1,

        vmf_superpage_size : 3, __unused_bit_19 : 1, vmf_return_data_addr : 1,
        __unused_bit_21 : 1, __unused_bit_22 : 1, vmf_return_4k_data_addr : 1,
        ...
        : 1, /* keep map locked when returning from vm_map_enter() */
        ...
        vmkf_submap_atomic
        : 1, /* keep entry atomic (no splitting/coalescing) */
    ...
    unsigned long long __vmkf_unused2 : 64;
  };
...
#define VM_MAP_KERNEL_FLAGS_DATA_SHARED_ANYWHERE(...)                          \
  VM_MAP_KERNEL_FLAGS_ANYWHERE(.vmkf_range_id =                                \
                                   kmem_needs_data_share_range()               \
                                       ? KMEM_RANGE_ID_DATA_SHARED             \
                                       : KMEM_RANGE_ID_DATA,                   \
                               __VA_ARGS__)

typedef struct {
  unsigned int vmnekf_ledger_tag : 3, vmnekf_ledger_no_footprint : 1,
      __vmnekf_unused : 28;
} vm_named_entry_kernel_flags_t;
#define VM_NAMED_ENTRY_KERNEL_FLAGS_NONE                                       \
  (vm_named_entry_kernel_flags_t){.vmnekf_ledger_tag = 0,                      \
...
/* for libchannel memory, mostly used on visionOS for communication with
 * realtime threads */
...
/* Differentiate memory needed by GPU drivers and frameworks from generic IOKit
 * allocations */
...
///
__SPI_AVAILABLE(macos(16.0), ios(19.0), watchos(12.0), tvos(19.0),
                visionos(3.0), bridgeos(10.0))
OS_EXPORT
 ...
 *     and their expected results; they deliberately call VM functions with
 * invalid kernel tag values and you may be turning one of those invalid tags
 * valid.
...
/* The size of the exclaves iboot carveout (exclaves memory not from XNU) in
 * bytes. */
#define VM_KERN_COUNT_EXCLAVES_CARVEOUT 14

/* The number of VM_KERN_COUNT_ stats. New VM_KERN_COUNT_ entries should be less
 * than this. */
