...
/*
 * mach_voucher_selector_t - A means of specifying which thread/task value to
 * extract - the current voucher set at this level, or a voucher representing
...
/*
 * mach_voucher_attr_key_t - The key used to identify a particular managed
 * resource or to select the specific resource manager’s data associated with a
 * given voucher.
 */
 ...
 * We define some system-wide commands here - related to creation, and transport
 * of vouchers and attributes.  Additional commands can be defined by, and
 * supported by, individual attribute resource managers.
 */
typedef uint32_t mach_voucher_attr_recipe_command_t;
typedef mach_voucher_attr_recipe_command_t
...
#define MACH_VOUCHER_ATTR_SET_VALUE_HANDLE                                     \
  ((mach_voucher_attr_recipe_command_t)3)
#define MACH_VOUCHER_ATTR_AUTO_REDEEM ((mach_voucher_attr_recipe_command_t)4)
#define MACH_VOUCHER_ATTR_SEND_PREPROCESS                                      \
  ((mach_voucher_attr_recipe_command_t)5)
...
#define MACH_VOUCHER_ATTR_IMPORTANCE_SELF                                      \
  ((mach_voucher_attr_recipe_command_t)200)

/* recipe command(s) for bit-store attribute manager */
#define MACH_VOUCHER_ATTR_USER_DATA_STORE                                      \
  ((mach_voucher_attr_recipe_command_t)211)
#define MACH_VOUCHER_ATTR_BITS_STORE                                           \
  MACH_VOUCHER_ATTR_USER_DATA_STORE /* deprecated */
 ...
 *
 * A handle through which the mach voucher mechanism communicates with the
 * voucher attribute manager for a given attribute key.
 */
 ...
 * through which it makes inquiries or control operations of the mach voucher
 * mechanism.
 */
...
/*
 * These types are different in-kernel vs user-space.  They are ports in
 * user-space, pointers to opaque structs in most of the kernel, and pointers to
 * known struct types in the Mach portion of the kernel.
 */
#if !defined(KERNEL)
...
typedef mach_voucher_attr_value_handle_t
    *mach_voucher_attr_value_handle_array_t;

typedef mach_msg_type_number_t mach_voucher_attr_value_handle_array_size_t;
#define MACH_VOUCHER_ATTR_VALUE_MAX_NESTED                                     \
  ((mach_voucher_attr_value_handle_array_size_t)4)
...
#define MACH_VOUCHER_ATTR_VALUE_FLAGS_PERSIST                                  \
  ((mach_voucher_attr_value_flags_t)1)

/* USE - TBD */
typedef uint32_t mach_voucher_attr_control_flags_t;
#define MACH_VOUCHER_ATTR_CONTROL_FLAGS_NONE                                   \
  ((mach_voucher_attr_control_flags_t)0)
 ...
 */
#define MACH_VOUCHER_IMPORTANCE_ATTR_ADD_EXTERNAL                              \
  1 /* Add some number of external refs (not supported) */
#define MACH_VOUCHER_IMPORTANCE_ATTR_DROP_EXTERNAL                             \
  2 /* Drop some number of external refs */
