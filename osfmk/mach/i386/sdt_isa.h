...
#define DTRACE_LABEL(p, n)                                                     \
  ...
  "1: .ascii \"" DTRACE_STRINGIFY(                                             \
      p##___) "\\0\"\n\t"                                                      \
              "2: .ascii \"" DTRACE_STRINGIFY(                                 \
                  n) "\\0\"\n\t"                                               \
                     ".popsection"                                             \
                     "\n\t"                                                    \
                     ".pushsection __DATA_CONST, __sdt, regular, "             \
                     "live_support\n\t"                                        \
                     ".p2align	3\n\t"                                          \
                     "l3_%=:\n\t"                                              \
                     ".quad 4f"                                                \
...
#define DTRACE_LABEL(p, n)                                                     \
  "__dtrace_probe$" DTRACE_TOSTRING(%= __LINE__)                               \
      DTRACE_STRINGIFY(_##p##___##n) ":"                                       \
                                     "\n\t"
...
#define DTRACE_PROBE(provider, name)                                           \
  do {                                                                         \
    asm volatile(DTRACE_CALL(provider, name) : : : "memory");                  \
  } while (0)

#define DTRACE_PROBE1(provider, name, arg0)                                    \
  do {                                                                         \
    register uintptr_t __dtrace_a0 asm("rdi") = (uintptr_t)arg0;               \
    asm volatile(DTRACE_CALL(provider, name) : : "r"(__dtrace_a0) : "memory"); \
  } while (0)
...
#define DTRACE_PROBE2(provider, name, arg0, arg1)                              \
  do {                                                                         \
    ...
    asm volatile(DTRACE_CALL(provider, name)                                   \
                 :                                                             \
...
#define DTRACE_PROBE7(provider, name, arg0, arg1, arg2, arg3, arg4, arg5,      \
  ...
  do {                                                                         \
    ...
    register uintptr_t __dtrace_a5 asm("r9") = (uintptr_t)arg5;                \
    uintptr_t tmp;                                                             \
    asm volatile(DTRACE_ALLOC_STACK(16) "movq\t%[a6],0x0(%%rsp)"               \
                                        "\n\t" DTRACE_CALL(provider, name)     \
                                            DTRACE_DEALLOC_STACK(16)           \
                 : "=r"(tmp)                                                   \
                 : "r"(__dtrace_a0), "r"(__dtrace_a1), "r"(__dtrace_a2),       \
                   "r"(__dtrace_a3), "r"(__dtrace_a4),                         \
                   "r"(__dtrace_a5), [a6] "r"((uintptr_t)arg6)                 \
                 : "memory");                                                  \
  } while (0)
