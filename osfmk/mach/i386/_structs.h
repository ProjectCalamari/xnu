...
_STRUCT_X86_INSTRUCTION_STATE {
  ...
  int __insn_offset;
  int __out_of_synch; /*
                       * non-zero when the cacheline that includes the
                       * insn_offset is replaced in the insn_bytes array due to
                       * a mismatch detected when comparing it with the same
                       * cacheline in memory
                       */
...
_STRUCT_LAST_BRANCH_RECORD {
  ...
  __uint64_t __to_ip;
  __uint32_t __mispredict : 1, __tsx_abort : 1, __in_tsx : 1,
      __cycle_count : 16, __reserved : 13;
};

#define _STRUCT_LAST_BRANCH_STATE struct __last_branch_state
_STRUCT_LAST_BRANCH_STATE {
  int __lbr_count;
  __uint32_t __lbr_supported_tsx : 1, __lbr_supported_cycle_count : 1,
      __reserved : 30;
...
_STRUCT_X86_INSTRUCTION_STATE {
  ...
  int out_of_synch; /*
                     ...
                     * detected when comparing it with the same cacheline in
                     * memory
                     */
...
_STRUCT_LAST_BRANCH_RECORD {
  __uint64_t from_ip;
  __uint64_t to_ip;
  __uint32_t mispredict : 1, tsx_abort : 1, in_tsx : 1, cycle_count : 16,
      reserved : 13;
};
...
_STRUCT_LAST_BRANCH_STATE {
  int lbr_count;
  __uint32_t lbr_supported_tsx : 1, lbr_supported_cycle_count : 1,
      reserved : 30;
#define __LASTBRANCH_MAX 32
