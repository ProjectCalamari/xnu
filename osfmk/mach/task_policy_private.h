...
#define trp_tal_enabled                                                        \
  trp_reserved /*  trp_tal_enabled is unused, reuse its slot to grow trp_role  \
                */

struct task_requested_policy {
  uint64_t trp_int_darwinbg : 1, /* marked as darwinbg via setpriority */
      trp_ext_darwinbg : 1, trp_int_iotier : 2, /* IO throttle tier */
      trp_ext_iotier : 2,
      trp_int_iopassive : 1, /* should IOs cause lower tiers to be throttled */
      trp_ext_iopassive : 1,
      trp_bg_iotier : 2,  /* what IO throttle tier should apply to me when I'm
                             darwinbg? (pushed to threads) */
      trp_terminated : 1, /* all throttles should be removed for quick exit or
                             SIGTERM handling */
      trp_base_latency_qos : 3, /* Timer latency QoS */
      trp_base_through_qos : 3, /* Computation throughput QoS */

      trp_apptype
      : 3, /* What apptype did launchd tell us this was (inherited) */
...
struct task_effective_policy {
  uint64_t tep_darwinbg
      ...
      tep_all_sockets_bg : 1, /* All existing sockets in process are marked as
                                 bg (thread: all created by thread) */
      tep_new_sockets_bg : 1, /* Newly created sockets should be marked as bg */
      tep_bg_iotier
      : 2, /* What throttle tier should I be in when darwinbg is set? */
      tep_terminated : 1, /* all throttles have been removed for quick exit or
                             SIGTERM handling */
      ...
      tep_suppressed_cpu
      : 1, /* cpu priority == MAXPRI_SUPPRESSED (trumped by lowpri_cpu) */
      tep_sfi_managed : 1, /* SFI Managed task */
      tep_live_donor : 1,  /* task is a live importance boost donor */
      tep_qos_clamp
      : 3, /* task qos clamp (applies to qos-disabled threads too) */
      tep_qos_ceiling
      : 3, /* task qos ceiling (applies to only qos-participating threads) */
      tep_promote_above_task
      : 1, /* task should allow turnstiles to boost above BG clamp */
...
#define TASK_POLICY_STATE_COUNT                                                \
  ((mach_msg_type_number_t)(sizeof(struct task_policy_state) /                 \
                            sizeof(integer_t)))
 ...
 * and the values exported through this API may change or change meaning at any
 * time
 *
 * Do not rely on these values, especially apptype, to decide behaviors at
 * runtime.
 *
...
#define POLICY_REQ_TH_QOS_OVER_MASK                                            \
  0x70000000 /* 3 bits (overlaps with TAL and SFI) */
...
extern kern_return_t qos_latency_policy_validate(task_latency_qos_t);

extern kern_return_t qos_throughput_policy_validate(task_throughput_qos_t);

extern uint32_t qos_extract(uint32_t);

extern uint32_t qos_latency_policy_package(uint32_t);
extern uint32_t qos_throughput_policy_package(uint32_t);
