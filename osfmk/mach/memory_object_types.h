...
typedef const struct memory_object_pager_ops {
  ...
  kern_return_t (*memory_object_init)(memory_object_t mem_obj,
  ...
  kern_return_t (*memory_object_data_unlock)(
      memory_object_t mem_obj, memory_object_offset_t offset,
      memory_object_size_t size, vm_prot_t desired_access); /* obsolete */
  kern_return_t (*memory_object_synchronize)(
      memory_object_t mem_obj, memory_object_offset_t offset,
      memory_object_size_t size, vm_sync_t sync_flags); /* obsolete */
#endif /* !XNU_KERNEL_PRIVATE */
  kern_return_t (*memory_object_map)(memory_object_t mem_obj, vm_prot_t prot);
  ...
  kern_return_t (*memory_object_data_reclaim)(
      memory_object_t mem_obj, boolean_t reclaim_backing_store); /* obsolete */
#endif /* !XNU_KERNEL_PRIVATE */
...
typedef struct old_memory_object_behave_info
    old_memory_object_behave_info_data_t;
...
#define OLD_MEMORY_OBJECT_BEHAVE_INFO_COUNT                                    \
  ((mach_msg_type_number_t)(sizeof(old_memory_object_behave_info_data_t) /     \
                            sizeof(int)))
#define OLD_MEMORY_OBJECT_ATTR_INFO_COUNT                                      \
  ((mach_msg_type_number_t)(sizeof(old_memory_object_attr_info_data_t) /       \
                            sizeof(int)))
...
extern int memory_object_control_uiomove(memory_object_control_t,
                                         memory_object_offset_t, void *, int,
                                         int, int, int);
__END_DECLS
...
#define MEMORY_OBJECT_BEHAVE_INFO_COUNT                                        \
  ((mach_msg_type_number_t)(sizeof(memory_object_behave_info_data_t) /         \
                            sizeof(int)))
#define MEMORY_OBJECT_PERF_INFO_COUNT                                          \
  ((mach_msg_type_number_t)(sizeof(memory_object_perf_info_data_t) /           \
                            sizeof(int)))
#define MEMORY_OBJECT_ATTR_INFO_COUNT                                          \
  ((mach_msg_type_number_t)(sizeof(memory_object_attr_info_data_t) /           \
                            sizeof(int)))

#define invalid_memory_object_flavor(f)                                        \
  (f != MEMORY_OBJECT_ATTRIBUTE_INFO && f != MEMORY_OBJECT_PERFORMANCE_INFO && \
   f != OLD_MEMORY_OBJECT_BEHAVIOR_INFO && f != MEMORY_OBJECT_BEHAVIOR_INFO && \
...
#define SET_MAP_MEM(caching, flags)                                            \
  ((flags) = ((((unsigned int)(caching)) << 24) & 0xFF000000) |                \
             ((flags) & 0xFFFFFF));

/* leave room for vm_prot bits (0xFF ?) */
#define MAP_MEM_PROT_MASK 0xFF
#define MAP_MEM_LEDGER_TAGGED                                                  \
  0x002000 /* object owned by a specific task and ledger */
#define MAP_MEM_PURGABLE_KERNEL_ONLY                                           \
  0x004000                             /* volatility controlled by kernel */
...
#define MAP_MEM_USE_DATA_ADDR                                                  \
  0x100000 /* preserve address of data, rather than base of page */
...
#define MAP_MEM_FLAGS_USER                                                     \
  (MAP_MEM_PURGABLE_KERNEL_ONLY | MAP_MEM_GRAB_SECLUDED | MAP_MEM_ONLY |       \
   MAP_MEM_NAMED_CREATE | MAP_MEM_PURGABLE | MAP_MEM_NAMED_REUSE |             \
   MAP_MEM_USE_DATA_ADDR | MAP_MEM_VM_COPY | MAP_MEM_VM_SHARE |                \
   MAP_MEM_LEDGER_TAGGED | MAP_MEM_4K_DATA_ADDR)
...
_Static_assert(sizeof(struct upl_page_info) == 8,
               "sizeof(struct upl_page_info) doesn't match expectation");
...
#define UPL_COMMIT_KERNEL_ONLY_FLAGS                                           \
  (UPL_COMMIT_CS_VALIDATED | UPL_COMMIT_FREE_ABSENT)
...
#define UPL_PHYS_PAGE(upl, index) ((upl)[(index)].phys_addr)
...
#define UPL_SET_PAGE_FREE_ON_COMMIT(upl, index)                                \
  (((upl)[(index)].phys_addr != 0) ? ((upl)[(index)].free_when_done = TRUE)    \
                                   : FALSE)
...
#define UPL_REPRIO_INFO_LEN(upl, index)                                        \
  ((((upl)->upl_reprio_info[(index)]) >> UPL_REPRIO_INFO_SHIFT) &              \
...
#define UPL_SET_CS_NX(upl, index, value) ((upl)[(index)].cs_nx = (value))

#define UPL_SET_REPRIO_INFO(upl, index, blkno, len)                            \
  ((upl)->upl_reprio_info[(index)]) =                                          \
      (((uint64_t)(blkno) & UPL_REPRIO_INFO_MASK) |                            \
...
extern void upl_set_blkno(upl_t upl, vm_offset_t upl_offset, int size,
                          int64_t blkno);
#endif
