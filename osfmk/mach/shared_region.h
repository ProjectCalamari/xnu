...
/*
 * This was used for the no longer present shared_region_map_and_slide_np()
 * interface. The struct got used by other external projects to represent shared
 * cache info, so it's left behind for now.
 */
...
#if KERNEL
VM_DEFINE_UNSAFE_TYPE(
    shared_file_mapping_slide_np_t, shared_file_mapping_slide_np_ut, struct {
...
#define VM_PROT_SFM_EXTENSIONS_MASK                                            \
  (VM_PROT_COW | VM_PROT_ZF | VM_PROT_SLIDE | VM_PROT_NOAUTH |                 \
