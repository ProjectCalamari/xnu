 ...
 * it also does double-duty by being set just while the exception is temporarily
 * shuffled around within xnu, and it can be cleared before we reached
 * exception_triage_thread().
 */
#define EXC_PTRAUTH_BIT                                                        \
  0x200 /* Set if the exception was caused by a ptrauth failure */
...
#define EXC_ARM_SME_DISALLOWED                                                 \
  2 /* Current thread state prohibits use of SME resources */
