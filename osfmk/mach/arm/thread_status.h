...
#define THREAD_STATE_FLAVORS                                                   \
  50 /* This must be updated to 1 more than the highest numerical state flavor \
      */
...
#define FLAVOR_MODIFIES_CORE_CPU_REGISTERS(x)                                  \
  ((x == ARM_THREAD_STATE) || (x == ARM_THREAD_STATE32) ||                     \
   (x == ARM_THREAD_STATE64))

#define VALID_THREAD_STATE_FLAVOR(x)                                           \
  ((x == ARM_THREAD_STATE) || (x == ARM_VFP_STATE) ||                          \
   (x == ARM_EXCEPTION_STATE) || (x == ARM_DEBUG_STATE) ||                     \
   (x == THREAD_STATE_NONE) || (x == ARM_THREAD_STATE32) ||                    \
   (x == ARM_THREAD_STATE64) || (x == ARM_EXCEPTION_STATE64) ||                \
   (x == ARM_EXCEPTION_STATE64_V2) || (x == ARM_NEON_STATE) ||                 \
   (x == ARM_NEON_STATE64) || (x == ARM_DEBUG_STATE32) ||                      \
   (x == ARM_DEBUG_STATE64) || (x == ARM_PAGEIN_STATE) ||                      \
...
#define ARM_THREAD_STATE32_COUNT                                               \
  ((mach_msg_type_number_t)(sizeof(arm_thread_state32_t) / sizeof(uint32_t)))
#define ARM_THREAD_STATE64_COUNT                                               \
  ((mach_msg_type_number_t)(sizeof(arm_thread_state64_t) / sizeof(uint32_t)))
#define ARM_UNIFIED_THREAD_STATE_COUNT                                         \
  ((mach_msg_type_number_t)(sizeof(arm_unified_thread_state_t) /               \
                            sizeof(uint32_t)))
...
#define ARM_EXCEPTION_STATE64_V2_COUNT                                         \
  ((mach_msg_type_number_t)(sizeof(arm_exception_state64_v2_t) /               \
                            sizeof(uint32_t)))
...
#define ARM_PAGEIN_STATE_COUNT                                                 \
  ((mach_msg_type_number_t)(sizeof(arm_pagein_state_t) / sizeof(uint32_t)))
...
extern void ml_panic_on_invalid_old_cpsr(const arm_saved_state_t *)
    __attribute__((noreturn));

extern void ml_panic_on_invalid_new_cpsr(const arm_saved_state_t *, uint32_t)
    __attribute__((noreturn));
 ...
 */
extern void ml_sign_thread_state(arm_saved_state_t *, uint64_t, uint32_t,
                                 uint64_t, uint64_t, uint64_t);

extern void ml_check_signed_state(const arm_saved_state_t *, uint64_t, uint32_t,
                                  uint64_t, uint64_t, uint64_t);

/* XXX: including stddef.f here breaks ctfmerge on some builds, so use
 * __builtin_offsetof() instead of offsetof() */
...
#define MANIPULATE_SIGNED_THREAD_STATE(_iss, _instr, ...)                      \
  do {                                                                         \
    ...
    asm volatile("mov	x9, lr"                                                  \
                 ...
                 "\n"                                                          \
                 :                                                             \
                 : [iss] "r"(_iss), [SS64_X16] "i"(ss64_offsetof(x[16])),      \
...
#define VERIFY_USER_THREAD_STATE_INPUTS                                        \
  [CPSR_EL_MASK] "i"(PSR64_MODE_EL_MASK), [CPSR_EL0] "i"(PSR64_MODE_EL0)

#define MANIPULATE_SIGNED_USER_THREAD_STATE(_iss, _instr, ...)                 \
  MANIPULATE_SIGNED_THREAD_STATE(_iss, VERIFY_USER_THREAD_STATE_INSTR _instr,  \
                                 VERIFY_USER_THREAD_STATE_INPUTS,              \
                                 ##__VA_ARGS__)

static inline void
check_and_sign_copied_user_thread_state(arm_saved_state_t *dst,
                                        const arm_saved_state_t *src) {
  MANIPULATE_SIGNED_USER_THREAD_STATE(src,
                                      "mov	x0, %[dst]", [dst] "r"(dst));
}
#endif /* HAS_APPLE_PAC */

static inline boolean_t is_saved_state32(const arm_saved_state_t *iss) {
  return iss->ash.flavor == ARM_SAVED_STATE32;
}

static inline boolean_t is_saved_state64(const arm_saved_state_t *iss) {
...
static inline register_t get_saved_state_pc(const arm_saved_state_t *iss) {
  return (register_t)(is_saved_state32(iss) ? const_saved_state32(iss)->pc
                                            : const_saved_state64(iss)->pc);
}

#if HAS_ADD_SAVED_STATE_PC
static inline void add_saved_state_pc(arm_saved_state_t *iss, int diff) {
...
static inline void add_user_saved_state_pc(arm_saved_state_t *iss, int diff) {
  if (is_saved_state32(iss)) {
    uint64_t pc = saved_state32(iss)->pc + (uint32_t)diff;
    saved_state32(iss)->pc = CAST_ASSERT_SAFE(uint32_t, pc);
  } else {
...
static inline void set_user_saved_state_pc(arm_saved_state_t *iss,
                                           register_t pc) {
  if (is_saved_state32(iss)) {
    saved_state32(iss)->pc = CAST_ASSERT_SAFE(uint32_t, pc);
  } else {
...
static inline register_t get_saved_state_sp(const arm_saved_state_t *iss) {
  return (register_t)(is_saved_state32(iss) ? const_saved_state32(iss)->sp
                                            : const_saved_state64(iss)->sp);
}
...
static inline register_t get_saved_state_lr(const arm_saved_state_t *iss) {
  return (register_t)(is_saved_state32(iss) ? const_saved_state32(iss)->lr
                                            : const_saved_state64(iss)->lr);
}
...
static inline void set_user_saved_state_lr(arm_saved_state_t *iss,
                                           register_t lr) {
  if (is_saved_state32(iss)) {
    saved_state32(iss)->lr = CAST_ASSERT_SAFE(uint32_t, lr);
  } else {
...
static inline register_t get_saved_state_fp(const arm_saved_state_t *iss) {
  return (register_t)(is_saved_state32(iss) ? const_saved_state32(iss)->r[7]
                                            : const_saved_state64(iss)->fp);
}
...
static inline int check_saved_state_reglimit(const arm_saved_state_t *iss,
                                             unsigned reg) {
  return is_saved_state32(iss) ? (reg < ARM_SAVED_STATE32_COUNT)
                               : (reg < ARM_SAVED_STATE64_COUNT);
}

static inline register_t get_saved_state_reg(const arm_saved_state_t *iss,
                                             unsigned reg) {
  if (!check_saved_state_reglimit(iss, reg)) {
    return 0;
  }

  return (register_t)(is_saved_state32(iss)
                          ? (const_saved_state32(iss)->r[reg])
                          : (const_saved_state64(iss)->x[reg]));
}
  ...
  if (is_saved_state32(iss)) {
    ...
    if (reg == 16) {
      ...
      MANIPULATE_SIGNED_THREAD_STATE(
          ...
          "mov	x5, %[value]		\n"
          "str	x5, [x0, %[SS64_X17]]	\n",
          [value] "r"(value), [SS64_X17] "i"(ss64_offsetof(x[17])));
      return;
    }
#endif
...
static inline void set_user_saved_state_reg(arm_saved_state_t *iss,
                                            unsigned reg, register_t value) {
  if (!check_saved_state_reglimit(iss, reg)) {
    return;
  }
...
static inline uint32_t get_saved_state_cpsr(const arm_saved_state_t *iss) {
  return is_saved_state32(iss) ? const_saved_state32(iss)->cpsr
                               : const_saved_state64(iss)->cpsr;
}

#if HAS_MASK_SAVED_STATE_CPSR
static inline void mask_saved_state_cpsr(arm_saved_state_t *iss,
                                         uint32_t set_bits,
                                         uint32_t clear_bits) {
  if (is_saved_state32(iss)) {
    ...
    MANIPULATE_SIGNED_THREAD_STATE(
        ...
        "bic	w2, w2, w6, lsl #0	\n"
        "str	w2, [x0, %[SS64_CPSR]]	\n",
        [set_bits] "r"(set_bits), [clear_bits] "r"(clear_bits));
#else
...
static inline void mask_user_saved_state_cpsr(arm_saved_state_t *iss,
                                              uint32_t set_bits,
                                              uint32_t clear_bits) {
  if (is_saved_state32(iss)) {
...
static inline void set_user_saved_state_cpsr(arm_saved_state_t *iss,
                                             uint32_t cpsr) {
  if (is_saved_state32(iss)) {
    if (!PSR_IS_USER(cpsr)) {
...
static inline register_t get_saved_state_far(const arm_saved_state_t *iss) {
  return (register_t)(is_saved_state32(iss) ? const_saved_state32(iss)->far
                                            : const_saved_state64(iss)->far);
}
...
static inline uint64_t get_saved_state_esr(const arm_saved_state_t *iss) {
  return is_saved_state32(iss) ? const_saved_state32(iss)->esr
                               : const_saved_state64(iss)->esr;
}

static inline void set_saved_state_esr(arm_saved_state_t *iss, uint64_t esr) {
  if (is_saved_state32(iss)) {
 ...
 */
static inline int get_saved_state_svc_number(const arm_saved_state_t *iss) {
  return is_saved_state32(iss)
             ? (int)const_saved_state32(iss)->r[12]
             : (int)const_saved_state64(iss)
...
#define ARM_NEON_SAVED_STATE32_COUNT                                           \
  ((mach_msg_type_number_t)(sizeof(arm_neon_saved_state32_t) /                 \
                            sizeof(unsigned int)))
...
#define ARM_NEON_SAVED_STATE64_COUNT                                           \
  ((mach_msg_type_number_t)(sizeof(arm_neon_saved_state64_t) /                 \
                            sizeof(unsigned int)))
...
static inline size_t arm_sme_z_size(uint16_t svl_b) { return 32 * svl_b; }

static inline size_t arm_sme_p_size(uint16_t svl_b) { return 2 * svl_b; }

static inline size_t arm_sme_za_size(uint16_t svl_b) { return svl_b * svl_b; }

static inline mach_msg_type_number_t arm_sme_saved_state_count(uint16_t svl_b) {
  assert(svl_b % 16 == 0);
  size_t size = sizeof(arm_sme_saved_state_t) + arm_sme_z_size(svl_b) +
                arm_sme_p_size(svl_b) + arm_sme_za_size(svl_b);
  return (mach_msg_type_number_t)(size / sizeof(unsigned int));
}

static inline uint8_t *arm_sme_z(arm_sme_context_t *ss) { return ss->__z_p_za; }

static inline const uint8_t *const_arm_sme_z(const arm_sme_context_t *ss) {
  return ss->__z_p_za;
}

static inline uint8_t *arm_sme_p(arm_sme_context_t *ss, uint16_t svl_b) {
  return ss->__z_p_za + arm_sme_z_size(svl_b);
}

static inline const uint8_t *const_arm_sme_p(const arm_sme_context_t *ss,
                                             uint16_t svl_b) {
  return ss->__z_p_za + arm_sme_z_size(svl_b);
}

static inline uint8_t *arm_sme_za(arm_sme_context_t *ss, uint16_t svl_b) {
  return ss->__z_p_za + arm_sme_z_size(svl_b) + arm_sme_p_size(svl_b);
}

static inline const uint8_t *const_arm_sme_za(const arm_sme_context_t *ss,
                                              uint16_t svl_b) {
  return ss->__z_p_za + arm_sme_z_size(svl_b) + arm_sme_p_size(svl_b);
}
...
extern void saved_state_to_thread_state64(const arm_saved_state_t *,
                                          arm_thread_state64_t *);
extern void thread_state64_to_saved_state(const arm_thread_state64_t *,
                                          arm_saved_state_t *);
...
extern void saved_state_to_thread_state32(const arm_saved_state_t *,
                                          arm_thread_state32_t *);
extern void thread_state32_to_saved_state(const arm_thread_state32_t *,
                                          arm_saved_state_t *);
