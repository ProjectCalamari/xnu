...
#define HOST_DEBUG_INFO_INTERNAL                                               \
  10 /* Used for kernel internal development tests only */
#define HOST_CAN_HAS_DEBUGGER 11
#define HOST_PREFERRED_USER_ARCH                                               \
  12 /* Get the preferred user-space architecture */
...
#define HOST_BASIC_INFO_OLD_COUNT                                              \
  ((mach_msg_type_number_t)(sizeof(host_basic_info_data_old_t) /               \
                            sizeof(integer_t)))
...
#define HOST_CAN_HAS_DEBUGGER_COUNT                                            \
  ((mach_msg_type_number_t)(sizeof(host_can_has_debugger_info_data_t) /        \
                            sizeof(integer_t)))
...
#define HOST_RESOURCE_SIZES_COUNT                                              \
  ((mach_msg_type_number_t)(sizeof(kernel_resource_sizes_data_t) /             \
                            sizeof(integer_t)))
...
#define HOST_PRIORITY_INFO_COUNT                                               \
  ((mach_msg_type_number_t)(sizeof(host_priority_info_data_t) /                \
                            sizeof(integer_t)))
...
struct vm_compressor_q_lens {
  ...
  uint32_t qcc_early_swappedin_count, qcc_regular_swappedin_count,
      qcc_late_swappedin_count;
  uint32_t qcc_early_swapout_count, qcc_regular_swapout_count,
      qcc_late_swapout_count;
...
#define VM_COMPRESSOR_Q_LENS_COUNT                                             \
  ((mach_msg_type_number_t)(sizeof(struct vm_compressor_q_lens) /              \
                            sizeof(integer_t)))
#endif
...
#define HOST_VM_PURGABLE_COUNT                                                 \
  ((mach_msg_type_number_t)(sizeof(host_purgable_info_data_t) /                \
                            sizeof(integer_t)))
...
#define HOST_VM_INFO64_REV1_COUNT                                              \
  ((mach_msg_type_number_t)(offsetof(vm_statistics64_data_t, swapped_count) /  \
                            sizeof(integer_t)))
/* previous versions: adjust the size according to what was added each time */
#define HOST_VM_INFO64_REV0_COUNT                                              \
  ((mach_msg_type_number_t)(offsetof(vm_statistics64_data_t, decompressions) / \
                            sizeof(integer_t)))

/* in <mach/vm_statistics.h> */
/* vm_extmod_statistics */
#define HOST_EXTMOD_INFO64_COUNT                                               \
  ((mach_msg_type_number_t)(sizeof(vm_extmod_statistics_data_t) /              \
                            sizeof(integer_t)))
...
#define HOST_VM_INFO_REV1_COUNT /* added "speculative_count" (1 int) */        \
  ((mach_msg_type_number_t)(HOST_VM_INFO_REV2_COUNT - 1))
#define HOST_VM_INFO_REV0_COUNT /* added "purgable" info (2 ints) */           \
  ((mach_msg_type_number_t)(HOST_VM_INFO_REV1_COUNT - 2))
...
#define HOST_CPU_LOAD_INFO_COUNT                                               \
  ((mach_msg_type_number_t)(sizeof(host_cpu_load_info_data_t) /                \
                            sizeof(integer_t)))
...
struct host_debug_info_internal {
  uint64_t config_bank : 1, /* built configurations */
      config_atm : 1, config_csr : 1, config_coalitions : 1, config_unused : 60;
};
...
#define HOST_DEBUG_INFO_INTERNAL_COUNT                                         \
  ((mach_msg_type_number_t)(sizeof(host_debug_info_internal_data_t) /          \
                            sizeof(integer_t)))
...
extern kern_return_t get_sched_statistics(struct _processor_statistics_np *out,
                                          uint32_t *count);
