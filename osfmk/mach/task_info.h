...
#elif defined(__arm__) && defined(__IPHONE_OS_VERSION_MIN_REQUIRED) &&         \
...
/* localized structure - cannot be safely passed between tasks of differing
 * sizes */
/* Don't use this, use MACH_TASK_BASIC_INFO instead */
...
#define TASK_EVENTS_INFO_COUNT                                                 \
  ((mach_msg_type_number_t)(sizeof(task_events_info_data_t) /                  \
                            sizeof(natural_t)))

#define TASK_THREAD_TIMES_INFO                                                 \
  3 /* total times for live threads -                                          \
...
#define TASK_THREAD_TIMES_INFO_COUNT                                           \
  ((mach_msg_type_number_t)(sizeof(task_thread_times_info_data_t) /            \
                            sizeof(natural_t)))
...
#define TASK_ABSOLUTETIME_INFO_COUNT                                           \
  ((mach_msg_type_number_t)(sizeof(task_absolutetime_info_data_t) /            \
                            sizeof(natural_t)))
...
#define TASK_KERNELMEMORY_INFO_COUNT                                           \
  ((mach_msg_type_number_t)(sizeof(task_kernelmemory_info_data_t) /            \
                            sizeof(natural_t)))

#define TASK_SECURITY_TOKEN 13
#define TASK_SECURITY_TOKEN_COUNT                                              \
  ((mach_msg_type_number_t)(sizeof(security_token_t) / sizeof(natural_t)))
...
#define TASK_POWER_INFO_COUNT                                                  \
  ((mach_msg_type_number_t)(sizeof(task_power_info_data_t) / sizeof(natural_t)))
...
#define TASK_TRACE_MEMORY_INFO_COUNT                                           \
  ((mach_msg_type_number_t)(sizeof(task_trace_memory_info_data_t) /            \
                            sizeof(natural_t)))

#define TASK_WAIT_STATE_INFO 25 /* deprecated. */
struct task_wait_state_info {
  uint64_t total_wait_state_time; /* Time that all threads past and present have
                                     been in a wait state */
  uint64_t total_wait_sfi_state_time; /* Time that threads have been in SFI wait
                                         (should be a subset of total wait state
                                         time */
...
#define TASK_WAIT_STATE_INFO_COUNT                                             \
  ((mach_msg_type_number_t)(sizeof(task_wait_state_info_data_t) /              \
                            sizeof(natural_t)))
...
#define TASK_VM_INFO_PURGEABLE_ACCOUNT                                         \
  27 /* Used for xnu purgeable vm unit tests */

#ifdef PRIVATE
struct pvm_account_info {
  uint64_t
      pvm_volatile_count; /* Number of volatile bytes associated with a task */
  uint64_t pvm_volatile_compressed_count; /* Number of volatile compressed bytes
                                             associated with a task */
  uint64_t pvm_nonvolatile_count; /* Number of nonvolatile bytes associated with
                                     a task */
  uint64_t pvm_nonvolatile_compressed_count; /* Number of nonvolatile compressed
                                                bytes associated with a task */
};

typedef struct pvm_account_info *pvm_account_info_t;
typedef struct pvm_account_info pvm_account_info_data_t;

#define PVM_ACCOUNT_INFO_COUNT                                                 \
  ((mach_msg_type_number_t)(sizeof(pvm_account_info_data_t) /                  \
                            sizeof(natural_t)))
#endif /* PRIVATE */
...
#define TASK_FLAGS_INFO_COUNT                                                  \
  ((mach_msg_type_number_t)(sizeof(task_flags_info_data_t) / sizeof(natural_t)))

#define TF_LP64 0x00000001     /* task has 64-bit addressing */
#define TF_64B_DATA 0x00000002 /* task has 64-bit data registers */

#define TASK_DEBUG_INFO_INTERNAL                                               \
  29 /* Used for kernel internal development tests. */
...
#define TASK_DEBUG_INFO_INTERNAL_COUNT                                         \
  ((mach_msg_type_number_t)(sizeof(task_debug_info_internal_data_t) /          \
                            sizeof(natural_t)))
...
#define TASK_SUSPEND_STATS_INFO_COUNT                                          \
  ((mach_msg_type_number_t)(sizeof(task_suspend_stats_data_t) /                \
                            sizeof(natural_t)))
...
typedef struct task_suspend_source_s
    task_suspend_source_array_t[TASK_SUSPEND_SOURCES_MAX];

typedef struct task_suspend_source_s task_suspend_source_data_t;
#define TASK_SUSPEND_SOURCES_INFO 31
#define TASK_SUSPEND_SOURCES_INFO_COUNT                                        \
  ((mach_msg_type_number_t)(sizeof(task_suspend_source_data_t) *               \
                            TASK_SUSPEND_SOURCES_MAX / sizeof(natural_t)))

#endif /* PRIVATE */

#define TASK_SECURITY_CONFIG_INFO                                              \
  32 /* Runtime security mitigations configuration for the task */
...
#define TASK_SECURITY_CONFIG_INFO_COUNT                                        \
  ((mach_msg_type_number_t)(sizeof(struct task_security_config_info) /         \
                            sizeof(natural_t)))

#define TASK_IPC_SPACE_POLICY_INFO                                             \
  33 /* Runtime security mitigations configuration for the task */
...
#define TASK_IPC_SPACE_POLICY_INFO_COUNT                                       \
  ((mach_msg_type_number_t)(sizeof(struct task_ipc_space_policy_info) /        \
                            sizeof(natural_t)))
...
#define TASK_EXC_GUARD_VM_DELIVER                                              \
  0x01 /* Deliver virtual memory EXC_GUARD exceptions */
...
#define TASK_EXC_GUARD_VM_FATAL                                                \
  0x08 /* Virtual Memory EXC_GUARD delivery is fatal */
#define TASK_EXC_GUARD_VM_ALL 0x0f

#define TASK_EXC_GUARD_MP_DELIVER                                              \
  0x10                              /* Deliver mach port EXC_GUARD exceptions */
 ...
 * The default for 3rd party guards is shifted up 8 bits - but otherwise the
 * same values as above.
 */
#define TASK_EXC_GUARD_THIRD_PARTY_DEFAULT_SHIFT                               \
  0x8 /* 3rd party default shifted up in boot-arg */

#define TASK_EXC_GUARD_HONOR_NAMED_DEFAULTS                                    \
  0x10000 /* Honor the by-process-name defaults */
...
#define TASK_CORPSE_FORKING_DISABLED_MEM_DIAG                                  \
  0x01 /* Disable corpse forking because the task is running under a           \
          diagnostic tool */

#ifdef XNU_KERNEL_PRIVATE

__options_decl(
    task_control_port_options_t, uint32_t,
    {
        ...
        TASK_CONTROL_PORT_IMMOVABLE_MASK = (TASK_CONTROL_PORT_IMMOVABLE_SOFT |
