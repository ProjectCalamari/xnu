...
#define VM_GENERATE_UNSAFE_WRAPPER(_safe_type, _unsafe_type)                   \
  typedef union {                                                              \
    _safe_type UNSAFE;                                                         \
  } _unsafe_type __attribute__((transparent_union));                           \
...
#define VM_DEFINE_UNSAFE_TYPE(_safe_type, _unsafe_type, _unsafe_contents)      \
  ...
  _Static_assert(                                                              \
      (sizeof(_unsafe_type) == sizeof(_safe_type)) &&                          \
          (_Alignof(_unsafe_type) == _Alignof(_safe_type)),                    \
...
VM_DEFINE_UNSAFE_TYPE(
    struct mach_vm_range, mach_vm_range_ut, struct {
...
VM_DEFINE_UNSAFE_TYPE(
    mach_vm_range_recipe_v1_t, mach_vm_range_recipe_v1_ut, struct {
