...
#define THREAD_BASIC_INFO_COUNT                                                \
  ((mach_msg_type_number_t)(sizeof(thread_basic_info_data_t) /                 \
                            sizeof(natural_t)))
...
#define THREAD_IDENTIFIER_INFO_COUNT                                           \
  ((mach_msg_type_number_t)(sizeof(thread_identifier_info_data_t) /            \
                            sizeof(natural_t)))
...
/*
 *  Thread extended info (returns same info as
 * proc_pidinfo(...,PROC_PIDTHREADINFO,...)
 */
#define THREAD_EXTENDED_INFO 5
#define MAXTHREADNAMESIZE 64
struct thread_extended_info { // same as proc_threadinfo (from proc_info.h) &
                              // proc_threadinfo_internal (from bsd_taskinfo.h)
...
#define THREAD_EXTENDED_INFO_COUNT                                             \
  ((mach_msg_type_number_t)(sizeof(thread_extended_info_data_t) /              \
                            sizeof(natural_t)))
...
#define THREAD_DEBUG_INFO_INTERNAL_COUNT                                       \
  ((mach_msg_type_number_t)(sizeof(thread_debug_info_internal_data_t) /        \
                            sizeof(natural_t)))
