...
#define CAST_MACH_NAME_TO_PORT(x)                                              \
  ((x) == MACH_PORT_DEAD ? (mach_port_t)IPC_PORT_DEAD                          \
                         : (mach_port_t)(uintptr_t)(x))
...
#define MACH_PORT_RIGHT_NUMBER                                                 \
  ((mach_port_right_t)6) /* right not implemented */
#endif
...
#define MACH_PORT_TYPE_LABELH                                                  \
  MACH_PORT_TYPE(MACH_PORT_RIGHT_LABELH) /* obsolete */
/* Dummy type bits that mach_port_type/mach_port_names can return. */
...
#define MACH_PORT_TEMPOWNER                                                    \
  4 /* indicates receive right will be reassigned to another task */
#define MACH_PORT_IMPORTANCE_RECEIVER                                          \
  5 /* indicates recieve right accepts priority donation */
#define MACH_PORT_DENAP_RECEIVER                                               \
  6 /* indicates receive right accepts de-nap donation */
#define MACH_PORT_INFO_EXT 7 /* uses mach_port_info_ext_t */
#define MACH_PORT_GUARD_INFO                                                   \
  8 /* asserts if the strict guard value is correct */
#define MACH_PORT_SERVICE_THROTTLED                                            \
  9 /* info is an integer that indicates if service port is throttled or not   \
     */
...
#define MACH_PORT_INFO_EXT_COUNT                                               \
  ((natural_t)(sizeof(mach_port_info_ext_t) / sizeof(natural_t)))
#define MACH_PORT_GUARD_INFO_COUNT                                             \
  ((natural_t)(sizeof(mach_port_guard_info_t) / sizeof(natural_t)))
 ...
 */
#define MACH_SERVICE_PORT_INFO_STRING_NAME_MAX_BUF_LEN                         \
  255 /* Maximum length of the port string name buffer */

typedef struct mach_service_port_info {
  char mspi_string_name
      [MACH_SERVICE_PORT_INFO_STRING_NAME_MAX_BUF_LEN]; /* Service port's string
                                                           name */
  uint8_t mspi_domain_type; /* Service port domain */
} mach_service_port_info_data_t;

#define MACH_SERVICE_PORT_INFO_COUNT                                           \
  ((char)(sizeof(mach_service_port_info_data_t) / sizeof(char)))
...
/* Allows 1p process to create provisional reply port (to be rename to weak
 * reply port) */
#define MACH_PORT_PROVISIONAL_REPLY_ENTITLEMENT                                \
...
#define MPO_IMMOVABLE_RECEIVE                                                  \
  0x80 /* Mark the port as immovable; protected by the guard context */
#define MPO_FILTER_MSG 0x100 /* Allow message filtering */
#define MPO_TG_BLOCK_TRACKING                                                  \
  0x200 /* Track blocking relationship for thread group during sync IPC */
#define MPO_ENFORCE_REPLY_PORT_SEMANTICS                                       \
  0x2000 /* When talking to this port, local port of mach msg needs to follow  \
            reply port semantics.*/
/* This service port has requested security hardening */
#define MPO_STRICT_SERVICE_PORT                                                \
  (MPO_SERVICE_PORT | MPO_ENFORCE_REPLY_PORT_SEMANTICS)

#define MPO_OPTIONS_MASK                                                       \
  (MPO_CONTEXT_AS_GUARD | MPO_QLIMIT | MPO_TEMPOWNER |                         \
   MPO_IMPORTANCE_RECEIVER | MPO_INSERT_SEND_RIGHT | MPO_STRICT |              \
   MPO_DENAP_RECEIVER | MPO_IMMOVABLE_RECEIVE | MPO_FILTER_MSG |               \
   MPO_TG_BLOCK_TRACKING | MPO_ENFORCE_REPLY_PORT_SEMANTICS)
...
__options_decl(
  ...
  MPO_PORT = MPO_MAKE_PORT_TYPE(0, 0), /* 0x0 */
      ...
      MPO_CONNECTION_PORT_WITH_PORT_ARRAY =
          MPO_MAKE_PORT_TYPE(4, 0), /* 0x10000 */
});
...
typedef struct mach_port_options {
  ...
  union {
    ...
    mach_service_port_info_t
        service_port_info; /* Service port (MPO_SERVICE_PORT) */
#endif
...
enum mach_port_guard_exception_codes {
  ...
  kGUARD_EXC_SERVICE_PORT_VIOLATION_FATAL =
      7, /* unused, for future sp defense enablement */
  ...
  kGUARD_EXC_INVALID_ARGUMENT = 1u
                                << 11, /* really kGUARD_EXC_ALREADY_GUARDED */
  ...
  kGUARD_EXC_SERVICE_PORT_VIOLATION_NON_FATAL =
      0x00100001, /* unused, for future sp defense enablement */
 ...
 * +-------------+----------------+----------------------+
 */
__header_always_inline __attribute__((overloadable)) uint64_t
...
MPG_PAYLOAD(uint8_t flag, uint32_t a, uint32_t b) {
  return ((uint64_t)flag << 56) | ((uint64_t)(a & MPG_24BIT_MASK) << 32) | b;
}
...
MPG_PAYLOAD(uint8_t flag, uint32_t a, uint16_t b, uint16_t c) {
  return ((uint64_t)flag << 56) | ((uint64_t)(a & MPG_24BIT_MASK) << 32) |
         ((uint64_t)b << 16) | c;
}
#endif /* XNU_KERNEL_PRIVATE */
...
/*
 * These flags are used as bits in the subcode of kGUARD_EXC_STRICT_REPLY
 * exceptions.
 */
...
/*
 * These flags are used as bits in the subcode of kGUARD_EXC_MOD_REFS
 * exceptions.
 */
...
/*
 * These flags are used as bits in the subcode of kGUARD_EXC_INVALID_RIGHT
 * exceptions.
 */
...
#define MPG_FLAGS_INVALID_RIGHT_DEALLOC_KERNEL                                 \
  0x06 /* mach_port_deallocate_kernel() */
#define MPG_FLAGS_INVALID_RIGHT_TRANSLATE_PORT                                 \
  0x07 /* port in ipc_object_translate_port_pset() */
#define MPG_FLAGS_INVALID_RIGHT_TRANSLATE_PSET                                 \
  0x08 /* pset in ipc_object_translate_port_pset() */

/*
 * These flags are used as bits in the subcode of kGUARD_EXC_INVALID_VALUE
 * exceptions.
 */
...
/*
 * These flags are used as bits in the subcode of kGUARD_EXC_KERN_FAILURE
 * exceptions.
 */
#define MPG_FLAGS_KERN_FAILURE_TASK                                            \
  0x01 /* task other than launchd arm pd on service ports */
#define MPG_FLAGS_KERN_FAILURE_NOTIFY_TYPE                                     \
  0x02 /* not using IOT_NOTIFICATION_PORT for pd notification */
#define MPG_FLAGS_KERN_FAILURE_NOTIFY_RECV                                     \
  0x03 /* notification port not owned by launchd */
#define MPG_FLAGS_KERN_FAILURE_MULTI_NOTI                                      \
  0x04 /* register multiple pd notification */

/*
 * These flags are used as bits in the subcode of kGUARD_EXC_SEND_INVALID_RIGHT
 * exceptions.
 */
#define MPG_FLAGS_SEND_INVALID_RIGHT_PORT                                      \
  0x01 /* ipc_kmsg_copyin_port_descriptor() */
#define MPG_FLAGS_SEND_INVALID_RIGHT_OOL_PORT                                  \
  0x02 /* ipc_kmsg_copyin_ool_ports_descriptor() */
#define MPG_FLAGS_SEND_INVALID_RIGHT_GUARDED                                   \
  0x03 /* ipc_kmsg_copyin_guarded_port_descriptor */

/*
 * These flags are used as bits in the subcode of kGUARD_EXC_INVALID_OPTIONS
 * exceptions.
 */
#define MPG_FLAGS_INVALID_OPTIONS_OOL_DISP                                     \
  0x01 /* ipc_kmsg_copyin_ool_ports_descriptor() */
#define MPG_FLAGS_INVALID_OPTIONS_OOL_ARRAYS                                   \
  0x02 /* ipc_validate_kmsg_header_from_user() */
#define MPG_FLAGS_INVALID_OPTIONS_OOL_RIGHT                                    \
  0x03 /* ipc_validate_kmsg_header_from_user() */
...
#define MPG_IMMOVABLE_RECEIVE                                                  \
  0x02 /* Receive right cannot be moved out of the space */
