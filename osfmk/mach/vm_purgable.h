 ...
 * Objects that will be needed in the future (forward cached objects) should be
 * queued LIFO. Objects that have been used and are cached for reuse (backward
 * cached) should be queued FIFO. Every user of purgeable memory is entitled to
 * using the highest volatile group (7). Only if a client wants some of its
 * objects to definitely be purged earlier, it can put those in another group.
 * This could be used to make all FIFO objects (in the lower group) go away
 * before any LIFO objects (in the higher group) go away. Objects that should
 * not get any chance to stay around can be marked as "obsolete". They will be
 * emptied before any other objects or pages are reclaimed. Obsolete objects are
 * not emptied in any particular order. 'purgeable' is recognized as the correct
 * spelling. For historical reasons, definitions in this file are spelled
 * 'purgable'.
 */
 ...
 *
 *  When making a new VM_PURGABLE_*, update tests
 * vm_parameter_validation_[user|kern] and their expected results; they
 * deliberately call VM functions with invalid values and you may be turning one
 * of those invalid bits valid.
 */
#define VM_PURGABLE_SET_STATE                                                  \
  ((vm_purgable_t)0) /* set state of purgeable object */
#define VM_PURGABLE_GET_STATE                                                  \
  ((vm_purgable_t)1) /* get state of purgeable object */
#define VM_PURGABLE_PURGE_ALL                                                  \
  ((vm_purgable_t)2) /* purge all volatile objects now */
#define VM_PURGABLE_SET_STATE_FROM_KERNEL                                      \
  ((vm_purgable_t)3) /* set state from kernel */
...
/*
 * Volatile memory ordering groups (group zero objects are purged before group
 * 1, etc... It is implementation dependent as to whether these groups are
 * global or per-address space. (for the moment, they are global).
 */
...
/*
 * Purgeable behavior
 * Within the same group, FIFO objects will be emptied before objects that are
 * added later. LIFO objects will be emptied after objects that are added later.
...
/*
 * Obsolete object.
 * Disregard volatile group, and put object into obsolete queue instead, so it
 * is the next object to be purged.
 * - Input only, not returned on state queries.
 */
...
#define VM_PURGABLE_ALL_MASKS                                                  \
  (VM_PURGABLE_STATE_MASK | VM_VOLATILE_ORDER_MASK |                           \
   VM_PURGABLE_ORDERING_MASK | VM_PURGABLE_BEHAVIOR_MASK |                     \
   VM_VOLATILE_GROUP_MASK | VM_PURGABLE_DEBUG_MASK |                           \
