...
struct bank_element {
  ...
  unsigned int be_made; /* Made refs for voucher, Actual ref is only taken for
                           voucher ref transition (0 to 1) */
#if DEVELOPMENT || DEBUG
  task_t
      be_task; /* Customer task, do not use it since ref is not taken on task */
#endif
};
...
struct bank_task {
  ...
  queue_head_t
      bt_accounts_to_pay; /* List of accounts worked for me and need to pay */
  queue_head_t bt_accounts_to_charge; /* List of accounts I did work and need to
                                         charge */
  decl_lck_mtx_data(
      , bt_acc_to_pay_lock); /* Lock to protect accounts to pay list */
  decl_lck_mtx_data(
      , bt_acc_to_charge_lock); /* Lock to protect accounts to charge list */
  uint32_t bt_persona_uid;      /* Persona UID of the process */
  uint32_t bt_hasentitlement
      : 1; /* If the secure persona entitlement is set on the task */
#if CONFIG_COALITIONS && CONFIG_TELEMETRY
  uint64_t bt_rsrc_coal_id; /* Task's resource coalition ID for
                               microstackshot/telemetry */
...
struct bank_account {
  ...
  bank_task_t
      ba_merchant; /* Task who worked for me, who will charge me on behalf of */
  bank_task_t ba_holder;           /* Credit Card task holder */
  bank_task_t ba_secureoriginator; /* Bank task of the secure originator */
  bank_task_t
      ba_proximateprocess; /* Process who propagated the voucher to us */
...
#endif
  struct bank_persona
      ba_so_persona; /* Persona of ba_secureoriginator,
...
extern void bank_billed_balance_safe(task_t task, uint64_t *cpu_time,
                                     uint64_t *energy);
extern void bank_billed_balance(bank_task_t bank_task, uint64_t *cpu_time,
                                uint64_t *energy);
extern void bank_serviced_balance_safe(task_t task, uint64_t *cpu_time,
                                       uint64_t *energy);
extern void bank_serviced_balance(bank_task_t bank_task, uint64_t *cpu_time,
                                  uint64_t *energy);
extern kern_return_t bank_get_bank_ledger_thread_group_and_persona(
    ...
    uint32_t *persona_id);
extern uint64_t
bank_get_bank_ledger_resource_coalition_id(ipc_voucher_t voucher);
