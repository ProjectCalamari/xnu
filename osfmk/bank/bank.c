...
static ZONE_DEFINE_TYPE(bank_task_zone, "bank_task", struct bank_task, ZC_NONE);
static ZONE_DEFINE_TYPE(bank_account_zone, "bank_account", struct bank_account,
                        ZC_NONE);
...
/* Need macro since bank_element_t is 4 byte aligned on release kernel and
 * direct type case gives compilation error */
...
ipc_voucher_attr_control_t
    bank_voucher_attr_control; /* communication channel from ATM to voucher
                                  system */
...
static bank_account_t bank_account_alloc_init(bank_task_t bank_holder,
...
static void bank_rollup_chit_to_tasks(ledger_t bill,
...
static uint64_t bank_get_bank_account_holder_resource_coalition_id(
    bank_account_t bank_account __unused);
static boolean_t bank_verify_persona_id(uint32_t persona_id,
                                        struct bank_persona *persona_out);
static boolean_t bank_task_can_adopt_persona(bank_task_t bank_merchant,
                                             struct bank_persona *persona);
static void bank_task_get_persona(bank_task_t bank_task,
                                  struct bank_persona *persona_out);
...
static LCK_SPIN_DECLARE_ATTR(g_bank_task_lock_data, &bank_lock_grp,
                             &bank_lock_attr);
...
#define global_bank_task_unlock() lck_spin_unlock(&g_bank_task_lock_data)
...
extern void proc_getexecutableuuid(void *p, unsigned char *uuidbuf,
                                   unsigned long size);
...
kern_return_t
bank_release_value(ipc_voucher_attr_manager_t __assert_only manager,
                   mach_voucher_attr_key_t __assert_only key,
                   mach_voucher_attr_value_handle_t value,
                   mach_voucher_attr_value_reference_t sync);

kern_return_t
bank_get_value(ipc_voucher_attr_manager_t __assert_only manager,
               mach_voucher_attr_key_t __assert_only key,
...
kern_return_t
bank_extract_content(ipc_voucher_attr_manager_t __assert_only manager,
                     mach_voucher_attr_key_t __assert_only key,
...
kern_return_t
bank_command(ipc_voucher_attr_manager_t __assert_only manager,
             mach_voucher_attr_key_t __assert_only key,
             mach_voucher_attr_value_handle_array_t values,
             mach_msg_type_number_t value_count,
...
const struct ipc_voucher_attr_manager bank_manager = {
    ...
    .ivam_flags = (IVAM_FLAGS_SUPPORT_SEND_PREPROCESS |
 ...
 * Returns: None.
 */
__startup_func static void bank_init(void) {
  init_bank_ledgers();

  /* Register the bank manager with the Vouchers sub system. */
  ipc_register_well_known_mach_voucher_attr_manager(
      &bank_manager, 0, MACH_VOUCHER_ATTR_KEY_BANK, &bank_voucher_attr_control);
...
bank_release_value(ipc_voucher_attr_manager_t __assert_only manager,
  ...
  assert(bank_element != BANK_DEFAULT_VALUE &&
         bank_element != BANK_DEFAULT_TASK_VALUE);
  ...
  if (bank_element->be_type == BANK_TASK) {
    ...
    /* Checking of the made ref with sync and clearing of voucher ref should be
     * done under a lock */
 ...
 */
kern_return_t bank_get_value(ipc_voucher_attr_manager_t __assert_only manager,
                             mach_voucher_attr_key_t __assert_only key,
                             mach_voucher_attr_recipe_command_t command,
                             mach_voucher_attr_value_handle_array_t prev_values,
  ...
  struct bank_persona persona = {.persona_id = PERSONA_ID_NONE,
    ...
    for (i = 0; i < prev_value_count; i++) {
      ...
      if (bank_holder->bt_persona_id == persona.persona_id) {
        ...
        if (bank_holder->bt_voucher_ref == 0) {
          /* Take a ref for voucher system, if voucher system does not have a
           * ref */
      ...
      bank_account = bank_account_alloc_init(
          bank_holder, bank_merchant, bank_secureoriginator,
          bank_proximateprocess, thread_group, &persona);
    ...
    for (i = 0; i < prev_value_count; i++) {
      ...
      if (bank_merchant->bt_hasentitlement == 0) {
        KERNEL_DEBUG_CONSTANT_IST(
            KDEBUG_TRACE,
            (BANK_CODE(BANK_ACCOUNT_INFO, (BANK_SECURE_ORIGINATOR_CHANGED))) |
                DBG_FUNC_NONE,
...
bank_extract_content(ipc_voucher_attr_manager_t __assert_only manager,
  ...
  for (i = 0; i < value_count && *in_out_recipe_size > 0; i++) {
    ...
    if (bank_element->be_type == BANK_TASK) {
      ...
      bank_account = CAST_TO_BANK_ACCOUNT(bank_element);
      snprintf(
          buf, MACH_VOUCHER_BANK_CONTENT_SIZE,
          " Bank Account linking holder pid %d with merchant pid %d, "
          "originator PID/persona: %d, %u and proximate PID/persona: %d, %u\n",
          bank_account->ba_holder->bt_pid, bank_account->ba_merchant->bt_pid,
...
bank_command(ipc_voucher_attr_manager_t __assert_only manager,
  ...
  case BANK_PERSONA_TOKEN:
    ...
    for (i = 0; i < value_count; i++) {
      ...
      memcpy(&token->originator, &bank_secureoriginator->bt_proc_persona,
             sizeof(struct proc_persona_info));
      memcpy(&token->proximate, &bank_proximateprocess->bt_proc_persona,
             sizeof(struct proc_persona_info));
  ...
  case BANK_PERSONA_ADOPT_ANY:
    ...
    adopt_any_persona = (bank_merchant->bt_flags &
                         PROC_PERSONA_INFO_FLAG_ADOPTION_ALLOWED) != 0;
    memcpy(out_content, &adopt_any_persona, sizeof(adopt_any_persona));
    *out_content_size =
        (mach_voucher_attr_content_size_t)sizeof(adopt_any_persona);
    return KERN_SUCCESS;
  ...
  case BANK_ORIGINATOR_PROXIMATE_PID:;
    ...
    for (i = 0; i < value_count; i++) {
      ...
      int32_t proximate_pid = bank_proximate_task != BANK_TASK_NULL
                                  ? bank_proximate_task->bt_pid
                                  : -1;
...
static boolean_t
bank_task_is_persona_adoption_allowed(task_t task __unused,
...
static bank_task_t bank_task_alloc_init(task_t task) {
  ...
  lck_mtx_init(&new_bank_task->bt_acc_to_pay_lock, &bank_lock_grp,
               &bank_lock_attr);
  lck_mtx_init(&new_bank_task->bt_acc_to_charge_lock, &bank_lock_grp,
               &bank_lock_attr);
  ...
  bzero(&new_bank_task->bt_proc_persona,
        sizeof(new_bank_task->bt_proc_persona));
  ...
  new_bank_task->bt_rsrc_coal_id =
      rsrc_coal != COALITION_NULL ? coalition_id(rsrc_coal) : 0;
#endif /* CONFIG_COALITIONS */
  proc_getexecutableuuid(bsd_info, new_bank_task->bt_macho_uuid,
                         sizeof(new_bank_task->bt_macho_uuid));
 ...
 */
static boolean_t bank_task_is_propagate_entitled(task_t t) {
  /* Check if it has an entitlement which disallows secure originator
   * propagation */
...
static bank_account_t bank_account_alloc_init(bank_task_t bank_holder,
  ...
  ledger_t new_ledger =
      ledger_instantiate(bank_ledger_template, LEDGER_CREATE_INACTIVE_ENTRIES);
  ...
  lck_mtx_lock(&bank_holder->bt_acc_to_pay_lock);
  queue_iterate(&bank_holder->bt_accounts_to_pay, bank_account, bank_account_t,
                ba_next_acc_to_pay) {
  ...
  if (!entry_found) {
    /*  Create a linkage between the holder and the merchant task, Grab both the
     * list locks before adding it to the list. */
    lck_mtx_lock(&bank_merchant->bt_acc_to_charge_lock);

    /* Add the account entry into Accounts need to pay account link list. */
    queue_enter(&bank_holder->bt_accounts_to_pay, new_bank_account,
                bank_account_t, ba_next_acc_to_pay);

    /* Add the account entry into Accounts need to charge account link list. */
    queue_enter(&bank_merchant->bt_accounts_to_charge, new_bank_account,
                bank_account_t, ba_next_acc_to_charge);
  ...
  queue_enter(&bank_accounts_list, new_bank_account, bank_account_t,
              ba_global_elt);
...
bank_account_dealloc_with_sync(bank_account_t bank_account,
  ...
  /* No need to take ledger reference for bank_holder ledger since
   * bt_acc_to_pay_lock is locked */
  bank_rollup_chit_to_tasks(bank_account->ba_bill, bank_holder->bt_ledger,
                            bank_merchant_ledger, bank_holder->bt_pid,
                            bank_merchant->bt_pid);

  /* Remove the account entry from Accounts need to pay account link list. */
  queue_remove(&bank_holder->bt_accounts_to_pay, bank_account, bank_account_t,
               ba_next_acc_to_pay);

  /* Remove the account entry from Accounts need to charge account link list. */
  queue_remove(&bank_merchant->bt_accounts_to_charge, bank_account,
               bank_account_t, ba_next_acc_to_charge);
...
#if DEVELOPMENT || DEBUG
  lck_mtx_lock(&bank_accounts_list_lock);
  queue_remove(&bank_accounts_list, bank_account, bank_account_t,
               ba_global_elt);
  lck_mtx_unlock(&bank_accounts_list_lock);
#endif
...
static void bank_rollup_chit_to_tasks(ledger_t bill,
  ...
  if (ret == KERN_SUCCESS) {
    ...
    if (bank_holder_ledger) {
      ledger_credit(bank_holder_ledger, task_ledgers.cpu_time_billed_to_me,
                    credit);
      ledger_debit(bank_holder_ledger, task_ledgers.cpu_time_billed_to_me,
                   debit);
    }

    if (bank_merchant_ledger) {
      ledger_credit(bank_merchant_ledger,
                    task_ledgers.cpu_time_billed_to_others, credit);
      ledger_debit(bank_merchant_ledger, task_ledgers.cpu_time_billed_to_others,
                   debit);
    }
  }
  ...
  if (ret == KERN_SUCCESS) {
    ...
    if (bank_holder_ledger) {
      ledger_credit(bank_holder_ledger, task_ledgers.energy_billed_to_me,
                    credit);
      ledger_debit(bank_holder_ledger, task_ledgers.energy_billed_to_me, debit);
    }

    if (bank_merchant_ledger) {
      ledger_credit(bank_merchant_ledger, task_ledgers.energy_billed_to_others,
                    credit);
      ledger_debit(bank_merchant_ledger, task_ledgers.energy_billed_to_others,
                   debit);
    }
  }
}
 ...
 * Returns: None.
 */
void bank_task_destroy(task_t task) {
  bank_task_t bank_task;
 ...
 * Returns: None.
 */
void bank_task_initialize(task_t task) { get_bank_task_context(task, TRUE); }
 ...
 * Purpose: Walk through all the bank accounts billed to me by other tasks and
 * get the current billing balance. Called from another task. It takes global
 * bank task lock to make sure the bank context is not deallocated while
 * accesing it. Returns: cpu balance and energy balance in out paremeters.
 */
void bank_billed_balance_safe(task_t task, uint64_t *cpu_time,
                              uint64_t *energy) {
  bank_task_t bank_task = BANK_TASK_NULL;
  ...
  /* Task might be in exec, grab the global bank task lock before accessing bank
   * context. */
  global_bank_task_lock();
 ...
 * Purpose: Walk through the Accounts need to pay account list and get the
 * current billing balance. Returns: cpu balance and energy balance in out
 * paremeters.
 */
void bank_billed_balance(bank_task_t bank_task, uint64_t *cpu_time,
                         uint64_t *energy) {
  ...
  if (bank_task->bt_ledger != LEDGER_NULL) {
    kr = ledger_get_balance(bank_task->bt_ledger,
                            task_ledgers.cpu_time_billed_to_me, &temp);
    if (kr == KERN_SUCCESS && temp >= 0) {
      cpu_balance += temp;
    }
#if DEVELOPMENT || DEBUG
    else {
      printf("bank_bill_time: ledger_get_balance failed or negative balance in "
             "ledger: %lld\n",
             temp);
    }
    ...
    kr = ledger_get_balance(bank_task->bt_ledger,
                            task_ledgers.energy_billed_to_me, &temp);
    if (kr == KERN_SUCCESS && temp >= 0) {
      energy_balance += temp;
    }
  }

  queue_iterate(&bank_task->bt_accounts_to_pay, bank_account, bank_account_t,
                ba_next_acc_to_pay) {
    temp = 0;
    kr =
 ...
 * Purpose: Walk through the bank accounts billed to other tasks by me and get
 * the current balance to be charged. Called from another task. It takes global
 * bank task lock to make sure the bank context is not deallocated while
 * accesing it. Returns: cpu balance and energy balance in out paremeters.
 */
void bank_serviced_balance_safe(task_t task, uint64_t *cpu_time,
                                uint64_t *energy) {
  bank_task_t bank_task = BANK_TASK_NULL;
  ledger_amount_t credit, debit;
  uint64_t cpu_balance = 0;
  ...
  if (bank_task) {
    ...
    kr = ledger_get_entries(
        task->ledger, task_ledgers.cpu_time_billed_to_others, &credit, &debit);
    if (kr == KERN_SUCCESS) {
      cpu_balance = credit - debit;
    }
 ...
 * Purpose: Walk through the Account need to charge account list and get the
 * current balance to be charged. Returns: cpu balance and energy balance in out
 * paremeters.
 */
void bank_serviced_balance(bank_task_t bank_task, uint64_t *cpu_time,
                           uint64_t *energy) {
  int64_t cpu_balance = 0;
  int64_t energy_balance = 0;
  bank_account_t bank_account;
  ...
  if (ledger) {
    kr = ledger_get_balance(ledger, task_ledgers.cpu_time_billed_to_others,
                            &temp);
    if (kr == KERN_SUCCESS && temp >= 0) {
      cpu_balance += temp;
    }
      ...
      printf("bank_serviced_time: ledger_get_balance failed or negative "
             "balance in ledger: %lld\n",
             temp);
    }
#endif /* DEVELOPMENT || DEBUG */

    kr =
        ledger_get_balance(ledger, task_ledgers.energy_billed_to_others, &temp);
    if (kr == KERN_SUCCESS && temp >= 0) {
      energy_balance += temp;
    }
  }

  queue_iterate(&bank_task->bt_accounts_to_charge, bank_account, bank_account_t,
 ...
 */
static bank_account_t bank_get_voucher_bank_account(ipc_voucher_t voucher) {
  bank_element_t bank_element = BANK_ELEMENT_NULL;
  bank_account_t bank_account = BANK_ACCOUNT_NULL;
  ...
  kr = mach_voucher_attr_control_get_values(bank_voucher_attr_control, voucher,
                                            vals, &val_count);
 ...
 * Purpose: Get the bankledger from the bank account if ba_merchant different
 * than ba_holder
 */
static ledger_t bank_get_bank_account_ledger(bank_account_t bank_account) {
 ...
 * Note: Make sure that the receiver is not an App before preadopting the
 * voucher.
 */
kern_return_t bank_get_preadopt_thread_group(ipc_voucher_t voucher,
                                             struct thread_group **banktg) {
  bank_account_t bank_account = BANK_ACCOUNT_NULL;
  bank_task_t bank_task = BANK_TASK_NULL;
  struct thread_group *thread_group = NULL;
  ...
  if (bank_element->be_type == BANK_TASK) {
    ...
    panic("Bogus bank type: %d passed in bank_get_preadopt_thread_group",
          bank_element->be_type);
  }
...
errorout:
  if (banktg != NULL) {
     ...
     * bank account. If the voucher has a bank task, this is the sender's bank
     * task, the receiver will convert the sender's bank task to bank account,
     * so give thread group from the bank task. */
 ...
 * Purpose: Get the bankledger (chit), thread group and persona id from the
 * voucher. Returns: bankledger, thread group if bank_account attribute present
 * in voucher and persona_id
 */
kern_return_t bank_get_bank_ledger_thread_group_and_persona(
    ipc_voucher_t voucher, ledger_t *bankledger, struct thread_group **banktg,
    uint32_t *persona_id) {
  bank_account_t bank_account;
  bank_task_t bank_task;
  struct thread_group *thread_group = NULL;
...
uint64_t bank_get_bank_ledger_resource_coalition_id(ipc_voucher_t voucher) {
 ...
 * Returns: None.
 * Note: Should be only called for current thread or thread which is not
 * started.
 */
void bank_swap_thread_bank_ledger(thread_t thread __unused,
   ...
   * Following is the timeline which shows all the variables used in the
   * calculation below.
   *
   *               thread ledger
   *                 cpu_time
  ...
  if (old_ledger != LEDGER_NULL) {
    ledger_credit(old_ledger, bank_ledgers.cpu_time,
                  effective_ledger_time_consumed);
    ledger_credit(old_ledger, bank_ledgers.energy, effective_energy_consumed);
  }
}
...
static boolean_t bank_task_can_adopt_persona(bank_task_t bank_merchant,
  ...
  if ((bank_merchant->bt_flags & PROC_PERSONA_INFO_FLAG_ADOPTION_ALLOWED) ==
      0) {
    return FALSE;
  }
