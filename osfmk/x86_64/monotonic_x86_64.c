...
struct mt_cpu *mt_cur_cpu(void) { return &current_cpu_datap()->cpu_monotonic; }
...
static inline void mt_fixed_counter_set_ctrl_mask(uint8_t ctrlbits) {
  uint64_t mask = 0;
...
static void enable_counters(void) {
  mt_fixed_counter_set_ctrl_mask(FIXED_CTR_CTRL_INIT_SINGLE |
                                 FIXED_CTR_CTRL_ENABLE_SINGLE);

  int fixed_count = kpc_fixed_count();
  uint64_t global_en = (fixed_count == 4) ? GLOBAL_CTRL_FIXED_EN_4FIXED
                                          : GLOBAL_CTRL_FIXED_EN_3FIXED;
...
static void disable_counters(void) { wrmsr64(GLOBAL_CTRL, 0); }

static void core_down(cpu_data_t *cpu) {
...
static void core_up(cpu_data_t *cpu) {
  struct mt_cpu *mtc;
...
void mt_cpu_down(cpu_data_t *cpu) { core_down(cpu); }
...
static int mt_pmi_x86_64(x86_saved_state_t *state) {
  assert(ml_get_interrupts_enabled() == FALSE);
  mt_check_for_pmi(mt_cur_cpu(), state);
  return 0;
}

void mt_ownership_change(bool __unused available) {}
...
int mt_microstackshot_start_arch(uint64_t period) {
  ...
  mp_cpus_call(CPUMASK_ALL, ASYNC, mt_microstackshot_start_remote, NULL);
  return 0;
}
...
static int core_init(__unused mt_device_t dev) { return ENOTSUP; }

#pragma mark common hooks

struct mt_device mt_devices[] = {
    [0] = {.mtd_name = "core", .mtd_init = core_init}};
