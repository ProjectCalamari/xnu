 ...
 */
char pmap_cpu_data_assert[(((offsetof(cpu_data_t, cpu_tlb_invalid) -
                             offsetof(cpu_data_t, cpu_active_cr3)) == 8) &&
                           (offsetof(cpu_data_t, cpu_active_cr3) % 64 == 0))
                              ? 1
                              : -1];
...
int allow_data_exec = VM_ABI_32; /* 32-bit apps may execute data by default,
                                    64-bit apps may not */
int allow_stack_exec = 0; /* No apps may execute from the stack by default */
#else                     /* DEBUG || DEVELOPMENT */
const int allow_data_exec = VM_ABI_32; /* 32-bit apps may execute data by
                                          default, 64-bit apps may not */
const int allow_stack_exec =
...
SECURITY_READ_ONLY_LATE(zone_t)
pv_hashed_list_zone; /* zone of pv_hashed_entry structures */
...
long long alloc_ptepages_count __attribute__((aligned(8))) =
    0; /* aligned for atomic access */
...
vm_offset_t pmap_map(vm_offset_t virt, vm_map_offset_t start_addr,
  ...
  while (start_addr < end_addr) {
    kr = pmap_enter(kernel_pmap, (vm_map_offset_t)virt,
                    (ppnum_t)i386_btop(start_addr), prot, VM_PROT_NONE, flags,
                    TRUE, PMAP_MAPPING_TYPE_INFER);

    if (kr != KERN_SUCCESS) {
      panic("%s: failed pmap_enter, "
            "virt=%p, start_addr=%p, end_addr=%p, prot=%#x, flags=%#x",
            __FUNCTION__, (void *)virt, (void *)start_addr, (void *)end_addr,
            prot, flags);
    }
...
static void pmap_ro_zone_validate_element_dst(zone_id_t zid, vm_offset_t va,
                                              vm_offset_t offset,
...
static void pmap_ro_zone_validate_element(zone_id_t zid, vm_offset_t va,
                                          vm_offset_t offset,
...
void pmap_bootstrap(__unused vm_offset_t load_start, __unused boolean_t IA32e) {
  assert(IA32e);
  ...
  current_cpu_datap()->cpu_kernel_cr3 =
      cpu_shadowp(cpu_number())->cpu_kernel_cr3 = (addr64_t)kernel_pmap->pm_cr3;
  ...
  virtual_avail =
      (vm_offset_t)(VM_MIN_KERNEL_ADDRESS) + (vm_offset_t)first_avail;
   ...
   */
  (void)PE_parse_boot_argn("-no_shared_cr3", &no_shared_cr3,
...
#if MACH_ASSERT
  PE_parse_boot_argn("pmap_asserts", &pmap_asserts_enabled,
...
int32_t pmap_unpack_index(pv_rooted_entry_t pv_h) {
  int32_t indx = 0;
...
void pmap_pack_index(uint32_t indx) {
...
void pal_hib_teardown_pmap_structs(addr64_t *unneeded_start,
                                   addr64_t *unneeded_end) {
  int32_t i;
  ...
  *unneeded_start =
      (addr64_t)&pv_head_table[pmap_teardown_last_valid_compact_indx + 1];
...
void pal_hib_rebuild_pmap_structs(void) {
  ...
  for (cindx = pmap_teardown_last_valid_compact_indx; cindx >= 0; cindx--) {
    ...
    if (rindx + 1 != eindx) {
       ...
       */
      bzero((char *)&pv_head_table[rindx + 1],
            (eindx - rindx - 1) * sizeof(struct pv_rooted_entry));
    }
    eindx = rindx;
  }
  if (rindx) {
 ...
 */
static void pmap_pv_fixup(vm_offset_t start_va, vm_offset_t end_va) {
  ppnum_t ppn;
  pv_rooted_entry_t pv_h;
  uint32_t pgsz;
...
#if DEVELOPMENT || DEBUG
      panic("pmap_pv_fixup: Unexpected address wrap (0x%lx after adding 0x%x)",
            start_va, pgsz);
#else
      start_va = end_va;
#endif
...
void pmap_init(void) {
  ...
  _vm_object_allocate((vm_object_size_t)NPML4PGS * PAGE_SIZE,
                      &kpml4obj_object_store, VM_MAP_SERIAL_SPECIAL);

  kernel_pmap->pm_obj_pdpt = &kpdptobj_object_store;
  _vm_object_allocate((vm_object_size_t)NPDPTPGS * PAGE_SIZE,
                      &kpdptobj_object_store, VM_MAP_SERIAL_SPECIAL);

  kernel_pmap->pm_obj = &kptobj_object_store;
  _vm_object_allocate((vm_object_size_t)NPDEPGS * PAGE_SIZE,
                      &kptobj_object_store, VM_MAP_SERIAL_SPECIAL);
  ...
  pmap_struct_map =
      kmem_suballoc(kernel_map, &pmap_struct_range.min_address,
                    pmap_struct_size, VM_MAP_CREATE_NEVER_FAULTS,
                    VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,
                    KMS_NOFAIL | KMS_PERMANENT, VM_KERN_MEMORY_PMAP)
          .kmr_submap;
  ...
  addr = (vm_offset_t)(pv_hash_lock_table +
                       pv_hash_lock_table_size((npvhashbuckets)));
  ...
  /* TODO: possible general optimisation...pre-allocate via zones commonly
   * created level3/2 pagetables
   */
  /* The anchor is required to be page aligned. Zone debugging adds
   * padding which may violate that requirement. Tell the zone
   * subsystem that alignment is required.
   ...
   */
  assert((uintptr_t)VM_MIN_KERNEL_ADDRESS + avail_start <=
         (uintptr_t)vm_page_get(0));
  pmap_pv_fixup((uintptr_t)VM_MIN_KERNEL_ADDRESS,
                (uintptr_t)VM_MIN_KERNEL_ADDRESS + avail_start);
  pmap_pv_fixup((uintptr_t)vm_page_get(0), (uintptr_t)vm_pages_end);

  pmap_initialized = TRUE;

  max_preemption_latency_tsc =
      tmrCvt((uint64_t)MAX_PREEMPTION_LATENCY_NS, tscFCvtn2t);
  ...
  pmap_ept_support_ad =
      vmx_hv_support() &&
      (VMX_CAP(MSR_IA32_VMX_EPT_VPID_CAP, MSR_IA32_VMX_EPT_VPID_CAP_AD_SHIFT, 1)
           ? TRUE
           : FALSE);
  pmap_eptp_flags = HV_VMX_EPTP_MEMORY_TYPE_WB | HV_VMX_EPTP_WALK_LENGTH(4) |
                    (pmap_ept_support_ad ? HV_VMX_EPTP_ENABLE_AD_FLAGS : 0);
#endif /* CONFIG_VMX */
}

void pmap_mark_range(pmap_t npmap, uint64_t sv, uint64_t nxrosz, boolean_t NX,
                     boolean_t ro) {
  uint64_t ev, cv = sv;
  pd_entry_t *pdep;
  ...
  if (os_add_overflow(sv, nxrosz, &ev)) {
    panic("pmap_mark_range: Unexpected address overflow: start=0x%llx "
          "size=0x%llx",
          sv, nxrosz);
  }

  /* XXX what if nxrosz is 0?  we end up marking the page whose address is
   * passed in via sv -- is that kosher? */
  ...
  for (pdep = pmap_pde(npmap, cv); pdep != NULL && (cv < ev);) {
...
#ifdef REMAP_DEBUG
      if ((NX ^ !!(*pdep & INTEL_PTE_NX)) ||
          (ro ^ !!!(*pdep & INTEL_PTE_WRITE))) {
        kprintf("WARNING: Remapping PDE for %p from %s%s%s to %s%s%s\n",
                (void *)cv, (*pdep & INTEL_PTE_VALID) ? "R" : "",
                (*pdep & INTEL_PTE_WRITE) ? "W" : "",
                (*pdep & INTEL_PTE_NX) ? "" : "X", "R", ro ? "" : "W",
                NX ? "" : "X");
      }
#endif
    ...
    for (ptep = pmap_pte(npmap, cv);
         ptep != NULL && (cv < (pdev + NBPD)) && (cv < ev);) {
#ifdef REMAP_DEBUG
      if ((NX ^ !!(*ptep & INTEL_PTE_NX)) ||
          (ro ^ !!!(*ptep & INTEL_PTE_WRITE))) {
        kprintf("WARNING: Remapping PTE for %p from %s%s%s to %s%s%s\n",
                (void *)cv, (*ptep & INTEL_PTE_VALID) ? "R" : "",
                (*ptep & INTEL_PTE_WRITE) ? "W" : "",
                (*ptep & INTEL_PTE_NX) ? "" : "X", "R", ro ? "" : "W",
                NX ? "" : "X");
      }
#endif
      if (NX) {
  ...
  DPRINTF("%s(0x%llx, 0x%llx, %u, %u): 0x%llx, 0x%llx\n", __FUNCTION__, sv,
          nxrosz, NX, ro, cv, ptep ? *ptep : 0);
}

/*
 * Reclaim memory for early boot 4K page tables that were converted to large
 * page mappings. We know this memory is part of the KPTphys[] array that was
 * allocated in Idle_PTs_init(), so we can free it using its address in that
 * array.
 */
static void pmap_free_early_PT(ppnum_t ppn, uint32_t cnt) {
  ppnum_t KPTphys_ppn;
  vm_offset_t offset;
...
void pmap_lowmem_finalize(void) {
   ...
   */
  for (i = 0; pmap_memory_regions[i].end < vm_kernel_base_page; i++) {
  ...
  if ((stext & I386_LPGMASK) == 0 && (sdata & I386_LPGMASK) == 0) {
    ...
    if (PE_parse_boot_argn("-kernel_text_ps_4K", &kernel_text_ps_4K,
  ...
  for (seg = firstseg(); seg != NULL;
       seg = nextsegfromheader(&_mh_execute_header, seg)) {
    if (!strcmp(seg->segname, "__TEXT") || !strcmp(seg->segname, "__DATA")) {
      continue;
    }
    ...
    if (!strcmp(seg->segname, "__HIB")) {
      for (sec = firstsect(seg); sec != NULL; sec = nextsect(seg, sec)) {
        ...
        if (!strcmp(sec->sectname, "__text")) {
          pmap_mark_range(kernel_pmap, sec->addr, round_page(sec->size), FALSE,
                          TRUE);
        } else {
          pmap_mark_range(kernel_pmap, sec->addr, round_page(sec->size), TRUE,
                          FALSE);
        }
      }
    } else {
      if (kc_format == KCFormatFileset) {
        ...
        /*
         * XXX: We are marking *every* segment with rwx permissions as a
         * workaround
        ...
        kprintf("Marking (%p, %p) as rwx\n",
                ...
                (void *)((seg->vmaddr & ~(uint64_t)PAGE_MASK) +
                         round_page_64(seg->vmsize)));
        pmap_mark_range(kernel_pmap, seg->vmaddr & ~(uint64_t)PAGE_MASK,
                        round_page_64(seg->vmsize), FALSE, FALSE);
      } else {
        pmap_mark_range(kernel_pmap, seg->vmaddr, round_page_64(seg->vmsize),
                        TRUE, FALSE);
      }
    }
  }
  ...
  if (debug_boot_arg) {
    ...
    pmap_store_pte(FALSE, pte,
                   kvtophys((vm_offset_t)&lowGlo) | INTEL_PTE_REF |
                       INTEL_PTE_MOD | INTEL_PTE_WIRED | INTEL_PTE_VALID |
    ...
    pmap_remove(kernel_pmap, LOWGLOBAL_ALIAS, LOWGLOBAL_ALIAS + PAGE_SIZE);
  }
 ...
 */
void x86_64_protect_data_const() {
  ...
  if (doconstro) {
    if (sconst & PAGE_MASK) {
      panic("CONST segment misaligned 0x%lx 0x%lx", sconst, econst);
    }
...
#if MACH_ASSERT
void pmap_assert_free(ppnum_t pn) {
  int pai;
...
boolean_t pmap_is_empty(pmap_t pmap, vm_map_offset_t va_start,
  ...
  for (offset = va_start; offset < va_end; offset += PAGE_SIZE_64) {
...
void hv_ept_pmap_create(void **ept_pmap, void **eptp) {
  ...
  p = pmap_create_options(get_task_ledger(current_task()), 0,
                          (PMAP_CREATE_64BIT | PMAP_CREATE_EPT));
  if (p == PMAP_NULL) {
    *ept_pmap = NULL;
    *eptp = NULL;
    return;
 ...
 */
extern pmap_t pmap_create(ledger_t ledger, vm_map_size_t sz,
                          boolean_t is_64bit);

__attribute__((used)) pmap_t pmap_create(ledger_t ledger, vm_map_size_t sz,
                                         boolean_t is_64bit) {
...
pmap_t pmap_create_options(ledger_t ledger, vm_map_size_t sz,
                           unsigned int flags) {
  pmap_t p;
  vm_size_t size;
  pml4_entry_t *pml4;
  ...
  p->pm_task_map =
      ((flags & PMAP_CREATE_64BIT) ? TASK_MAP_64BIT : TASK_MAP_32BIT);
  ...
  if (flags & PMAP_CREATE_EPT) {
    p->pm_eptp =
        (pmap_paddr_t)kvtophys((vm_offset_t)p->pm_pml4) | pmap_eptp_flags;
    p->pm_cr3 = 0;
  } else {
    p->pm_eptp = 0;
  ...
  if (!(flags & PMAP_CREATE_EPT)) {
    ...
    for (i = KERNEL_PHYSMAP_PML4_INDEX;
         i < (KERNEL_PHYSMAP_PML4_INDEX + KERNEL_PHYSMAP_PML4_COUNT); i++) {
      pml4[i] = kpml4[i];
    }
  ...
  PMAP_TRACE(PMAP_CODE(PMAP__CREATE) | DBG_FUNC_END, VM_KERNEL_ADDRHIDE(p));
...
void pmap_destroy(pmap_t p) {
  ...
  PMAP_TRACE(PMAP_CODE(PMAP__DESTROY) | DBG_FUNC_START, VM_KERNEL_ADDRHIDe(p));

  PMAP_LOCK_EXCLUSIVE(p);

  c = os_ref_release_locked(&p->ref_count);

  pmap_assert((current_thread() && (current_thread()->map))
                  ? (current_thread()->map->pmap != p)
                  : TRUE);
 ...
 */
void pmap_protect_options(pmap_t map, vm_map_offset_t sva, vm_map_offset_t eva,
                          vm_prot_t prot, unsigned int options, void *arg) {
  pt_entry_t *pde;
  ...
  PMAP_TRACE(PMAP_CODE(PMAP__PROTECT) | DBG_FUNC_START, VM_KERNEL_ADDRHIDE(map),
             VM_KERNEL_ADDRHIDE(sva), VM_KERNEL_ADDRHIDE(eva));

  is_ept = is_ept_pmap(map);

  if ((prot & VM_PROT_EXECUTE) ||
      __improbable(is_ept && (prot & VM_PROT_UEXEC))) {
    set_NX = FALSE;
  } else {
    set_NX = TRUE;
  }
    ...
    if (pde && (*pde & PTE_VALID_MASK(is_ept))) {
      ...
      for (; spte < epte; spte++) {
        ...
        if (set_NX) {
          ...
          /* This is the exception to the "Don't add permissions" statement,
           * above */
  ...
  for (page = 0; page < size; page += cur_page_size / PAGE_SIZE) {
    ...
    if (kr != KERN_SUCCESS) {
      ...
      panic("%s: failed pmap_enter, "
            ...
            __FUNCTION__, pmap, va, pa, size, prot, flags);
...
kern_return_t pmap_expand_pml4(pmap_t map, vm_map_offset_t vaddr,
...
#endif
  vm_page_insert_wired(m, map->pm_obj_pml4, (vm_object_offset_t)i * PAGE_SIZE,
                       VM_KERN_MEMORY_PTE);
  vm_object_unlock(map->pm_obj_pml4);
...
kern_return_t pmap_expand_pdpt(pmap_t map, vm_map_offset_t vaddr,
                               unsigned int options) {
  vm_page_t m;
  pmap_paddr_t pa;
  uint64_t i;
...
#endif
  vm_page_insert_wired(m, map->pm_obj_pdpt, (vm_object_offset_t)i * PAGE_SIZE,
                       VM_KERN_MEMORY_PTE);
  vm_object_unlock(map->pm_obj_pdpt);
 ...
 */
kern_return_t pmap_expand(pmap_t map, vm_map_offset_t vaddr,
                          unsigned int options) {
  pt_entry_t *pdp;
  vm_page_t m;
  pmap_paddr_t pa;
...
#endif
  vm_page_insert_wired(m, map->pm_obj, (vm_object_offset_t)i * PAGE_SIZE,
                       VM_KERN_MEMORY_PTE);
  vm_object_unlock(map->pm_obj);
 ...
 */
vm_size_t pmap_query_pagesize(pmap_t pmap, vm_map_offset_t vaddr) {
  pd_entry_t *pdep;
  vm_size_t size = 0;
...
uint32_t pmap_kernel_va_bits(void) {
  /* x86 has constant set of bits based on 4 level paging. */
  return 48;
}
 ...
 */
static kern_return_t pmap_pre_expand_large_internal(pmap_t pmap,
                                                    vm_map_offset_t vaddr) {
  ppnum_t pn;
  pt_entry_t *pte;
  ...
  if (pmap64_pdpt(pmap, vaddr) == PDPT_ENTRY_NULL) {
    ...
    pmap_store_pte(
        is_ept, pte,
        pa_to_pte(i386_ptob(pn)) | PTE_READ(is_ept) |
            (is_ept ? (INTEL_EPT_EX | INTEL_EPT_UEX) : INTEL_PTE_USER) |
            PTE_WRITE(is_ept));
 ...
 */
void pmap_sync_page_data_phys(__unused ppnum_t pa) { return; }
 ...
 */
void pmap_sync_page_attributes_phys(ppnum_t pa) { cache_flush_page_phys(pa); }

void pmap_copy_page(ppnum_t src, ppnum_t dst, int options __unused) {
  bcopy_phys((addr64_t)i386_ptob(src), (addr64_t)i386_ptob(dst), PAGE_SIZE);
}
 ...
 */
void pmap_pageable(__unused pmap_t pmap, __unused vm_map_offset_t start_addr,
...
void invalidate_icache(__unused vm_offset_t addr, __unused unsigned cnt,
                       __unused int phys) {
  return;
}

void flush_dcache(__unused vm_offset_t addr, __unused unsigned count,
                  __unused int phys) {
  return;
}
...
kern_return_t dtrace_copyio_preflight(__unused addr64_t va) {
  ...
  if (current_map() == kernel_map) {
    return KERN_FAILURE;
  } else if (((ccr3 = get_cr3_base()) != thread->map->pmap->pm_cr3) &&
             (no_shared_cr3 == FALSE)) {
    return KERN_FAILURE;
...
int pmap_list_resident_pages(__unused pmap_t pmap, __unused vm_offset_t *listp,
                             __unused int space) {
  return 0;
}
#endif /* MACH_VM_DEBUG */
...
boolean_t phys_page_exists(ppnum_t pn) {
  assert(pn != vm_page_fictitious_addr);
...
void pmap_switch(pmap_t tpmap, thread_t thread __unused) {
  PMAP_TRACE_CONSTANT(PMAP_CODE(PMAP__SWITCH) | DBG_FUNC_START,
...
void pmap_flush_context_init(pmap_flush_context *pfc) {
  pfc->pfc_cpus = 0;
  pfc->pfc_invalid_global = 0;
}

static bool pmap_tlbi_response(uint32_t lcpu, uint32_t rcpu, bool ngflush) {
  ...
  if (responded == false) {
    if ((cpu_datap(rcpu)->cpu_tlb_invalid == 0) || !CPU_CR3_IS_ACTIVE(rcpu) ||
        !cpu_is_running(rcpu)) {
      responded = true;
    }
  }
...
void pmap_flush(pmap_flush_context *pfc) {
  unsigned int my_cpu;
  unsigned int cpu;
  cpumask_t cpu_bit;
  ...
  for (cpu = 0, cpu_bit = 1; cpu < real_ncpus && cpus_to_signal;
       cpu++, cpu_bit <<= 1) {
    if (cpus_to_signal & cpu_bit) {
      ...
      cpu_datap(my_cpu)->cpu_tlb_gen_counts_global[cpu] =
          cpu_datap(cpu)->cpu_tlb_invalid_global_count;
      cpu_datap(my_cpu)->cpu_tlb_gen_counts_local[cpu] =
          cpu_datap(cpu)->cpu_tlb_invalid_local_count;
      mfence();
  ...
  if (cpus_to_respond) {
    deadline = mach_absolute_time() + (TLBTimeOut ? TLBTimeOut : LockTimeOut);
    ...
    while (cpus_to_respond != 0) {
      ...
      if (cpus_to_respond && (mach_absolute_time() > deadline)) {
        ...
        if (TLBTimeOut == 0) {
          ...
          PMAP_TRACE_CONSTANT(PMAP_CODE(PMAP__FLUSH_TLBS_TO), NULL,
                              cpus_to_signal, cpus_to_respond);
        ...
        panic("Uninterruptible processor(s): CPU bitmap: 0x%llx, NMIPI acks: "
              "0x%lx, now: 0x%lx, deadline: %llu",
              cpus_to_respond, orig_acks, NMIPI_acks, deadline);
      }
    }
  }

  PMAP_TRACE_CONSTANT(PMAP_CODE(PMAP__FLUSH_DELAYED_TLBS) | DBG_FUNC_END, NULL,
                      cpus_signaled, flush_self);

  mp_enable_preemption();
}

static void invept(void *eptp) {
  struct {
    uint64_t eptp;
    uint64_t reserved;
  }
...
void pmap_flush_tlbs(pmap_t pmap, vm_map_offset_t startv, vm_map_offset_t endv,
                     int options, pmap_flush_context *pfc) {
  unsigned int cpu;
  cpumask_t cpu_bit;
  cpumask_t cpus_to_signal = 0;
  ...
  PMAP_TRACE_CONSTANT(event_code | DBG_FUNC_START,
                      VM_KERNEL_UNSLIDE_OR_PERM(pmap), options, event_startv,
                      event_endv);
  ...
  for (cpu = 0, cpu_bit = 1; cpu < real_ncpus; cpu++, cpu_bit <<= 1) {
    ...
    if ((pmap_cr3 == cpu_task_cr3) || (pmap_cr3 == cpu_active_cr3) ||
        (pmap_is_shared)) {
       ...
       */
      if (CPU_CR3_IS_ACTIVE(cpu) &&
          (pmap_cr3 == CPU_GET_ACTIVE_CR3(cpu) || pmap->pm_shared ||
  ...
  if (cpus_to_signal) {
    ...
    while (cpus_to_respond != 0) {
      ...
      if (cpus_to_respond && (mach_absolute_time() > deadline)) {
        ...
        panic("IPI timeout, unresponsive CPU bitmap: 0x%llx, NMIPI acks: "
              "0x%lx, now: 0x%lx, deadline: %llu, pre-NMIPI time: 0x%llx, "
              "current: 0x%llx, global: %d",
              cpus_to_respond, orig_acks, NMIPI_acks, deadline, tstamp1,
              tstamp2, need_global_flush);
      }
    }
  }

  if (__improbable((pmap == kernel_pmap) && (flush_self != TRUE))) {
    panic("pmap_flush_tlbs: pmap == kernel_pmap && flush_self != TRUE; kernel "
          "CR3: 0x%llX, pmap_cr3: 0x%llx, CPU active CR3: 0x%llX, CPU Task "
          "Map: %d",
          kernel_pmap->pm_cr3, pmap_cr3, current_cpu_datap()->cpu_active_cr3,
...
static void process_pmap_updates(pmap_t p, bool pshared, addr64_t istart,
                                 addr64_t iend) {
  int ccpu = cpu_number();
  bool gtlbf = false;
  ...
  if (pmap_pcid_ncpus) {
    if (p) {
      ...
      pmap_tlbi_range(istart, iend, (pshared || gtlbf),
                      p->pmap_pcid_cpus[ccpu]);
    } else {
 ...
 */
kern_return_t pmap_permissions_verify(pmap_t ipmap, vm_map_t ivmmap,
                                      vm_offset_t sv, vm_offset_t ev) {
  vm_offset_t cv = sv;
    ...
    if (ptep && (*ptep & INTEL_PTE_VALID)) {
      if (*ptep & INTEL_PTE_WRITE) {
        if (!(*ptep & INTEL_PTE_NX)) {
          kprintf("W+X PTE at 0x%lx, P4: 0x%llx, P3: 0x%llx, P2: 0x%llx, PT: "
                  "0x%llx, VP: %u\n",
                  cv, *pmap64_pml4(ipmap, cv), *pmap64_pdpt(ipmap, cv),
                  *pmap_pde(ipmap, cv), *ptep,
                  pmap_valid_page((ppnum_t)(i386_btop(pte_to_pa(*ptep)))));
          rv = KERN_FAILURE;
        }
      }
    }
  ...
  while (cv < ev) {
    for (;;) {
      ...
      if ((kret = mach_vm_region_recurse(
               ivmmap, (mach_vm_address_t *)&cv, &vmsize, &nesting_depth,
    ...
    if (prot) {
      ...
      for (pcv = cv; pcv < cv + vmsize; pcv += PAGE_SIZE) {
        ...
        if (tprot != prot) {
          kprintf("PTE/map entry permissions mismatch at address 0x%lx, pte: "
                  "0x%llx, protection: 0x%x\n",
                  pcv, *ptep, prot);
          rv = KERN_FAILURE;
        }
      }
...
void pmap_set_process(pmap_t pmap, int pid, char *procname) {
  ...
  if (pmap_ledgers_panic_leeway) {
    ...
    ledger_disable_panic_on_negative(pmap->ledger, task_ledgers.phys_footprint);
    ledger_disable_panic_on_negative(pmap->ledger, task_ledgers.internal);
    ledger_disable_panic_on_negative(pmap->ledger,
                                     task_ledgers.internal_compressed);
    ledger_disable_panic_on_negative(pmap->ledger, task_ledgers.iokit_mapped);
    ledger_disable_panic_on_negative(pmap->ledger,
                                     task_ledgers.alternate_accounting);
    ledger_disable_panic_on_negative(
        pmap->ledger, task_ledgers.alternate_accounting_compressed);
  }
}
#endif /* MACH_ASSERT */
...
#endif
  lpmap->pagezero_accessible =
      ((pmap_smap_enabled == FALSE) && (low_bound < 0x1000));
...
uintptr_t pmap_verify_noncacheable(uintptr_t vaddr) {
  ...
  panic("pmap_verify_noncacheable: IO read from a cacheable address? address: "
        "0x%lx, PTE: %p, *PTE: 0x%llx",
        vaddr, ptep, *ptep);
  /*NOTREACHED*/
  return 0;
}

bool pmap_lookup_in_loaded_trust_caches(const uint8_t __unused cdhash[20]) {
  // Unsupported on this architecture.
  return false;
}

uint32_t pmap_lookup_in_static_trust_cache(const uint8_t __unused cdhash[20]) {
  // Unsupported on this architecture.
  return false;
}
...
bool pmap_has_iofilter_protected_write() {
