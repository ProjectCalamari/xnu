...
void pmap_pcid_configure(void) {
  ...
  if (PE_parse_boot_argn("-pmap_pcid_disable", &pmap_pcid_disabled,
...
pcid_t pmap_pcid_allocate_pcid(int ccpu) {
  ...
  pcid_ref_t *cpu_pcid_refcounts =
      &cpu_datap(ccpu)->cpu_pcid_data->cpu_pcid_refcounts[0];
...
void pmap_pcid_deallocate_pcid(int ccpu, pmap_t tpmap) {
  ...
  if (lp == tpmap) {
    (void)__sync_bool_compare_and_swap(
        &cpu_datap(ccpu)->cpu_pcid_data->cpu_pcid_last_pmap_dispatched[pcid],
        tpmap, PMAP_INVALID);
  }

  if ((prior_count = __sync_fetch_and_sub(
           &cpu_datap(ccpu)->cpu_pcid_data->cpu_pcid_refcounts[pcid], 1)) ==
      1) {
...
void pmap_pcid_activate(pmap_t tpmap, int ccpu, boolean_t nopagezero,
                        boolean_t copyio) {
...
#if PMAP_ASSERT
  pcid_record_array[ccpu % PCID_RECORD_SIZE] =
      tpmap->pm_cr3 | new_pcid |
      (((uint64_t)(!(pending_flush || pcid_conflict))) << 63);
  ...
  cpu_datap(ccpu)->cpu_pmap_pcid_coherentp =
      &(tpmap->pmap_pcid_coherency_vector[ccpu]);
#if DEBUG
  cpu_datap(ccpu)->cpu_pcid_last_cr3 = scr3;
  KERNEL_DEBUG_CONSTANT(0x9c1d0000, tpmap, new_pcid, pending_flush,
                        pcid_conflict, 0);
#endif
}
...
static inline void invpcid(invpcid_type_t itype, pcid_t ipcid, uint64_t iaddr) {
...
void pmap_tlbi_range(uint64_t startv, uint64_t endv, bool global,
                     uint16_t pcid) {
