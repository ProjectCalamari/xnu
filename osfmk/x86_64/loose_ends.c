...
extern void ovbcopy(const char *from, char *to, vm_size_t nbytes);
...
void ovbcopy(const char *from, char *to,
...
__private_extern__ uint64_t ml_phys_read_data(uint64_t paddr, int size) {
  uint64_t result = 0;
  unsigned char s1;
  unsigned short s2;
    ...
    if (__improbable((eabs - sabs) > report_phy_read_delay)) {
      if (phy_read_panic && (machine_timeout_suspended() == FALSE)) {
        ...
        panic("Read from physical addr 0x%llx took %llu ns, "
              ...
              paddr, (eabs - sabs), result, sabs, eabs, report_phy_read_delay);
      }

      DTRACE_PHYSLAT4(physread, uint64_t, (eabs - sabs), uint64_t, paddr,
                      uint32_t, size, uint64_t, result);
    }

    if (__improbable(trace_phy_read_delay > 0 &&
                     (eabs - sabs) > trace_phy_read_delay)) {
      KDBG(MACHDBG_CODE(DBG_MACH_IO, DBC_MACH_IO_PHYS_READ), (eabs - sabs),
           sabs, paddr, result);
    }
...
static unsigned long long ml_phys_read_long_long(uint64_t paddr) {
    ...
    if (__improbable((eabs - sabs) > report_phy_write_delay)) {
      if (phy_write_panic && (machine_timeout_suspended() == FALSE)) {
        ...
        panic("Write to physical addr 0x%llx took %llu ns, "
              ...
              paddr, (eabs - sabs), data, sabs, eabs, report_phy_write_delay);
      }

      DTRACE_PHYSLAT4(physwrite, uint64_t, (eabs - sabs), uint64_t, paddr,
                      uint32_t, size, uint64_t, data);
    }

    if (__improbable(trace_phy_write_delay > 0 &&
                     (eabs - sabs) > trace_phy_write_delay)) {
      KDBG(MACHDBG_CODE(DBG_MACH_IO, DBC_MACH_IO_PHYS_WRITE), (eabs - sabs),
           sabs, paddr, data);
    }
...
uint32_t ml_port_io_read(uint16_t ioport, int size) {
  uint32_t result = 0;
    ...
    if (__improbable((eabs - sabs) > report_phy_read_delay)) {
      if (phy_read_panic && (machine_timeout_suspended() == FALSE)) {
        ...
        panic("Read from IO port 0x%x took %llu ns, "
              ...
              ioport, (eabs - sabs), result, sabs, eabs, report_phy_read_delay);
      }

      DTRACE_PHYSLAT3(portioread, uint64_t, (eabs - sabs), uint16_t, ioport,
                      uint32_t, size);
    }

    if (__improbable(trace_phy_read_delay > 0 &&
                     (eabs - sabs) > trace_phy_read_delay)) {
      KDBG(MACHDBG_CODE(DBG_MACH_IO, DBC_MACH_IO_PORTIO_READ), (eabs - sabs),
           sabs, ioport, result);
    }
...
void ml_port_io_write(uint16_t ioport, uint32_t val, int size) {
  uint64_t sabs, eabs;
    ...
    if (__improbable((eabs - sabs) > report_phy_write_delay)) {
      if (phy_write_panic && (machine_timeout_suspended() == FALSE)) {
        ...
        panic("Write to IO port 0x%x took %llu ns, val: 0x%x"
              ...
              ioport, (eabs - sabs), val, sabs, eabs, report_phy_write_delay);
      }

      DTRACE_PHYSLAT4(portiowrite, uint64_t, (eabs - sabs), uint16_t, ioport,
                      uint32_t, size, uint64_t, val);
    }

    if (__improbable(trace_phy_write_delay > 0 &&
                     (eabs - sabs) > trace_phy_write_delay)) {
      KDBG(MACHDBG_CODE(DBG_MACH_IO, DBC_MACH_IO_PORTIO_WRITE), (eabs - sabs),
           sabs, ioport, val);
    }
...
uint8_t ml_port_io_read8(uint16_t ioport) { return ml_port_io_read(ioport, 1); }

uint16_t ml_port_io_read16(uint16_t ioport) {
  return ml_port_io_read(ioport, 2);
}

uint32_t ml_port_io_read32(uint16_t ioport) {
  return ml_port_io_read(ioport, 4);
}

void ml_port_io_write8(uint16_t ioport, uint8_t val) {
  ml_port_io_write(ioport, val, 1);
}

void ml_port_io_write16(uint16_t ioport, uint16_t val) {
  ml_port_io_write(ioport, val, 2);
}

void ml_port_io_write32(uint16_t ioport, uint32_t val) {
 ...
 */
boolean_t ml_probe_read_64(addr64_t paddr64, unsigned int *val) {
...
void invalidate_icache64(__unused addr64_t addr, __unused unsigned count,
                         __unused int phys) {}
...
void cache_flush_page_phys(ppnum_t pa) {
  boolean_t istate;
...
#if !CONFIG_VMX
int host_vmxon(boolean_t exclusive __unused) { return VMX_UNSUPPORTED; }

void host_vmxoff(void) { return; }
#endif
...
void xcpm_mbox_lock(void) { lck_spin_lock(&xcpm_lock); }

void xcpm_mbox_unlock(void) { lck_spin_unlock(&xcpm_lock); }
...
boolean_t xcpm_is_hwp_enabled(void) { return FALSE; }
