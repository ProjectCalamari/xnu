...
static void smaplog_add_entry(boolean_t enabling) {
  uint32_t index = 0;
  thread_t thread = current_thread();

  do {
    index = smaplog_head;
  } while (!OSCompareAndSwap(index, (index + 1) % SMAPLOG_BUFFER_SIZE,
                             &smaplog_head));
  ...
  smaplog_cbuf[index].copyio_active =
      (thread->machine.specFlags & CopyIOActive) ? 1 : 0;
}
...
#else
#define COPYIO_TRACE(x, a, b, c, d, e)                                         \
  do {                                                                         \
  } while (0)
#endif

static int copyio(int copy_type, user_addr_t user_addr, char *kernel_addr,
  ...
  COPYIO_TRACE(debug_type | DBG_FUNC_START, user_addr, kernel_addr, nbytes,
               use_kernel_map, 0);
  ...
  if ((copy_type != COPYINPHYS) && (copy_type != COPYOUTPHYS)) {
    if (__improbable((vm_offset_t)kernel_addr <
                     VM_MIN_KERNEL_AND_KEXT_ADDRESS)) {
      panic("Invalid copy parameter, copy type: %d, kernel address: %p",
            copy_type, kernel_addr);
    }
  ...
  if (__improbable(((pmap != kernel_pmap) && (use_kernel_map == 0)) &&
                   ((nbytes && (user_addr + nbytes <= user_addr)) ||
  ...
  if ((pmap != kernel_pmap) && !use_kernel_map &&
    ...
    panic("copyio(%d,%p,%p,%ld,%p,%d) cr3 is %p expects %p", copy_type,
          (void *)user_addr, kernel_addr, nbytes, lencopied, use_kernel_map,
  ...
  COPYIO_TRACE(0xeff70044 | DBG_FUNC_NONE, user_addr, kernel_addr, nbytes, 0,
               0);

  switch (copy_type) {
  case COPYIN:
    error = _bcopy((const void *)user_addr, kernel_addr, nbytes);
    break;

  case COPYOUT:
    error = _bcopy(kernel_addr, (void *)user_addr, nbytes);
    break;

  case COPYINPHYS:
    error = _bcopy((const void *)user_addr, PHYSMAP_PTOV(kernel_addr), nbytes);
    break;

  case COPYOUTPHYS:
    error = _bcopy((const void *)PHYSMAP_PTOV(kernel_addr), (void *)user_addr,
                   nbytes);
    break;

  case COPYINATOMIC32:
    error = _copyin_atomic32((const void *)user_addr, (void *)kernel_addr);
    break;

  case COPYINATOMIC64:
    error = _copyin_atomic64((const void *)user_addr, (void *)kernel_addr);
    break;

  case COPYOUTATOMIC32:
    error = _copyout_atomic32((const void *)kernel_addr, (void *)user_addr);
    break;

  case COPYOUTATOMIC64:
    error = _copyout_atomic64((const void *)kernel_addr, (void *)user_addr);
    break;

  case COPYINSTR:
    error = _bcopystr((const void *)user_addr, kernel_addr, (int)nbytes,
                      &bytes_copied);
...
out:
  COPYIO_TRACE(debug_type | DBG_FUNC_END, user_addr, kernel_addr, nbytes, error,
               0);

  return error;
}

static int copyio_phys(addr64_t source, addr64_t sink, vm_size_t csize,
                       int which) {
  char *paddr;
  user_addr_t vaddr;
  int ctype;
...
int copyinmsg(const user_addr_t user_addr, void *kernel_addr,
...
int copyin_atomic32(const user_addr_t user_addr, uint32_t *kernel_addr) {
  ...
  return copyio(COPYINATOMIC32, user_addr, (char *)(uintptr_t)kernel_addr, 4,
                NULL, 0);
}

int copyin_atomic32_wait_if_equals(const user_addr_t user_addr,
                                   uint32_t value) {
  uint32_t u32;
...
int copyin_atomic64(const user_addr_t user_addr, uint64_t *kernel_addr) {
  ...
  return copyio(COPYINATOMIC64, user_addr, (char *)(uintptr_t)kernel_addr, 8,
                NULL, 0);
}
...
int copyinstr(const user_addr_t user_addr, char *kernel_addr, vm_size_t nbytes,
              vm_size_t *lencopied) {
  *lencopied = 0;
...
int copyoutmsg(const void *kernel_addr, user_addr_t user_addr,
               mach_msg_size_t nbytes) {
  CALL_COPYOUT_SHIM_MSG(kernel_addr, user_addr, (vm_size_t)nbytes)
  return copyio(COPYOUT, user_addr, (char *)(uintptr_t)kernel_addr, nbytes,
                NULL, 0);
}
...
int verify_write(const void *source, void *dst, size_t size) {
  int rc;
  disable_preemption();
...
kern_return_t copypv(addr64_t src64, addr64_t snk64, unsigned int size,
  ...
  KERNEL_DEBUG(0xeff7004c | DBG_FUNC_START, (unsigned)src64, (unsigned)snk64,
               size, which, 0);
    ...
    if (bothphys) {
      lop =
          (unsigned int)(PAGE_SIZE -
                         (snk64 & (PAGE_SIZE - 1))); /* Assume sink smallest */

      if (lop > (unsigned int)(PAGE_SIZE - (src64 & (PAGE_SIZE - 1)))) {
        lop = (unsigned int)(PAGE_SIZE -
                             (src64 &
                              (PAGE_SIZE - 1))); /* No, source is smaller */
      }
    } else {
      /*
  ...
  KERNEL_DEBUG(0xeff7004c | DBG_FUNC_END, (unsigned)src64, (unsigned)snk64,
               size, which, 0);
