...
static uint64_t IA32_FIXED_CTR_CTRL(void) {
  return rdmsr64(MSR_IA32_PERF_FIXED_CTR_CTRL);
}

#ifdef FIXED_COUNTER_RELOAD
static void wrIA32_FIXED_CTRx(uint32_t ctr, uint64_t value) {
  return wrmsr64(MSR_IA32_PERF_FIXED_CTR0 + ctr, value);
}
#endif

static uint64_t IA32_PMCx(uint32_t ctr) {
#ifdef USE_RDPMC
  return rdpmc64(ctr);
#else  /* !USE_RDPMC */
...
static void wrIA32_PMCx(uint32_t ctr, uint64_t value) {
  return wrmsr64(MSR_IA32_PERFCTR0 + ctr, value);
}

static uint64_t IA32_PERFEVTSELx(uint32_t ctr) {
  return rdmsr64(MSR_IA32_EVNTSEL0 + ctr);
}

static void wrIA32_PERFEVTSELx(uint32_t ctr, uint64_t value) {
...
uint32_t kpc_fixed_config_count(void) { return KPC_X86_64_FIXED_CONFIGS; }
...
static uint8_t kpc_fixed_width(void) {
  i386_cpu_info_t *info = NULL;
...
static uint8_t kpc_configurable_width(void) {
  i386_cpu_info_t *info = NULL;
...
uint64_t kpc_fixed_max(void) { return (1ULL << kpc_fixed_width()) - 1; }
...
static uint64_t kpc_reload_fixed(int ctr) {
...
static uint64_t kpc_reload_configurable(int ctr) {
...
static void set_running_fixed(boolean_t on) {
  ...
  if (on) {
    ...
    for (i = 0; i < kpc_fixed_count(); i++) {
      fixed_ctrl |= ((uint64_t)(IA32_FIXED_CTR_ENABLE_ALL_RINGS |
                                IA32_FIXED_CTR_ENABLE_PMI)
                     << (4 * i));
    }
  } else {
...
static void set_running_configurable(uint64_t target_mask,
  ...
  global |=
      state_mask; /* update the targeted PMCs bits with their new states */
  wrmsr64(MSR_IA32_PERF_GLOBAL_CTRL, global);

  ml_set_interrupts_enabled(enabled);
}

static void kpc_set_running_mp_call(void *vstate) {
...
static int kpc_set_fixed_config(kpc_config_t *configv) {
  (void)configv;
...
static int kpc_set_configurable_config(kpc_config_t *configv,
                                       uint64_t pmc_mask) {
  uint32_t cfg_count = kpc_configurable_count();
  uint64_t save;
...
int kpc_get_configurable_counters(uint64_t *counterv, uint64_t pmc_mask) {
  ...
  for (uint32_t i = 0; i < cfg_count; ++i) {
    if ((1ULL << i) & pmc_mask) {
      *it_counterv++ =
          CONFIGURABLE_SHADOW(i) + (IA32_PMCx(i) - CONFIGURABLE_RELOAD(i));
    }
  }
  ...
  /*
   * If the overflow bit is set for a counter, our previous read may or may not
   * have been before the counter overflowed. Re-read any counter with it's
   * overflow bit set so we know for sure that it has overflowed. The reason
   * this matters is that the math is different for a counter that has
   * overflowed.
   */
  for (uint32_t i = 0; i < cfg_count; ++i) {
    if (((1ULL << i) & pmc_mask) && ((1ULL << i) & status)) {
...
static void kpc_get_curcpu_counters_mp_call(void *args) {
...
static void kpc_set_config_mp_call(void *vmp_config) {
...
static void kpc_set_reload_mp_call(void *vmp_config) {
  struct kpc_config_remote *mp_config = vmp_config;
...
static uintptr_t get_interrupted_pc(bool *kernel_out) {
...
static void kpc_sample_kperf_x86(uint32_t ctr, uint32_t actionid,
                                 uint64_t count, uint64_t config) {
  bool kernel = false;
  ...
  kpc_sample_kperf(actionid, ctr,
                   config & 0xffff /* just the number and umask */, count, pc,
                   flags);
}
...
void kpc_pmi_handler(void) {
  ...
  for (ctr = 0; ctr < kpc_configurable_count(); ctr++) {
    if ((1ULL << ctr) & status) {
      ...
      /* kperf can grab the PMCs when it samples so we need to make sure the
       * overflow bits are in the correct state before the call to kperf_sample
       */
      wrmsr64(MSR_IA32_PERF_GLOBAL_OVF_CTRL, 1ull << ctr);

      unsigned int actionid = CONFIGURABLE_ACTIONID(ctr);
      BUF_INFO(PERF_KPC_COUNTER, ctr, CONFIGURABLE_SHADOW(ctr), extra,
               actionid);
