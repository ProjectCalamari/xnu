...
/*
 * MAX_ZTRACE_DEPTH configures how deep of a stack trace is taken on each zalloc
 * in the zone of interest.  15 levels is usually enough to get past all the
 * layers of code in kalloc and IOKit and see who the actual caller is up above
 * these lower levels.
 *
 * This is used both for the zone leak detector and the zone corruption log.
 * Make sure this isn't greater than BTLOG_MAX_DEPTH defined in btlog.h. Also
 * make sure to update the definition of zone_btrecord_t in
