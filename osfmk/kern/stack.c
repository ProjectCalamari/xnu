...
static inline vm_offset_t roundup_pow2(vm_offset_t size) {
  if ((size & (size - 1)) == 0) {
...
static void stack_init(void) {
  ...
  if (PE_parse_boot_argn("kernel_stack_pages", &kernel_stack_pages,
...
static vm_offset_t stack_alloc_internal(void) {
  ...
  if (stack == 0) {
    ...
    kernel_memory_allocate(kernel_map, &stack, kernel_stack_size + ptoa(2),
                           stack_addr_mask, flags, VM_KERN_MEMORY_STACK);
...
void stack_handoff(thread_t from, thread_t to) {
...
void stack_free_reserved(thread_t thread) {
  if (thread->reserved_stack != thread->kernel_stack) {
    stack_free_stack(thread->reserved_stack);
  }
}

static void stack_free_stack(vm_offset_t stack) {
 ...
 *	Called at splsched.
 */
boolean_t stack_alloc_try(thread_t thread) {
  struct stack_cache *cache;
  vm_offset_t stack;
...
void stack_collect(void) {
  if (stack_collect_tick != last_stack_tick) {
    ...
    while (stack_free_count > target) {
      ...
      stack =
          (vm_offset_t)vm_map_trunc_page(stack, VM_MAP_PAGE_MASK(kernel_map));
      stack -= PAGE_SIZE;
 ...
 *	computation period.
 */
void compute_stack_target(__unused void *arg) {
  spl_t s;
  ...
  stack_free_target +=
      (stack_free_delta >= 0) ? stack_free_delta : -stack_free_delta;
...
void stack_privilege(__unused thread_t thread) { /* OBSOLETE */ }

/*
 * Return info on stack usage for threads in a specific processor set
 */
kern_return_t processor_set_stack_usage(processor_set_t pset,
                                        unsigned int *totalp, vm_size_t *spacep,
                                        vm_size_t *residentp,
                                        vm_size_t *maxusagep,
...
__mockable vm_offset_t min_valid_stack_address(void) {
  return (vm_offset_t)vm_map_min(kernel_map);
}

__mockable vm_offset_t max_valid_stack_address(void) {
