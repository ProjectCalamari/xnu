...
/*
 * All allocations of size less than KHEAP_MAX_SIZE are rounded to the next
 * nearest sized zone.  This allocator is built on top of the zone allocator.  A
 * zone is created for each potential size that we are willing to get in small
 * blocks.
 *
...
static const uint16_t kt_zone_cfg[] = {
    K_SIZE_CLASS(16),    K_SIZE_CLASS(32),    K_SIZE_CLASS(48),
    K_SIZE_CLASS(64),    K_SIZE_CLASS(80),    K_SIZE_CLASS(96),
    K_SIZE_CLASS(128),   K_SIZE_CLASS(160),   K_SIZE_CLASS(192),
    K_SIZE_CLASS(224),   K_SIZE_CLASS(256),   K_SIZE_CLASS(288),
    K_SIZE_CLASS(368),   K_SIZE_CLASS(400),   K_SIZE_CLASS(512),
    K_SIZE_CLASS(576),   K_SIZE_CLASS(768),   K_SIZE_CLASS(1024),
    K_SIZE_CLASS(1152),  K_SIZE_CLASS(1280),  K_SIZE_CLASS(1664),
    K_SIZE_CLASS(2048),  K_SIZE_CLASS(4096),  K_SIZE_CLASS(6144),
    K_SIZE_CLASS(8192),  K_SIZE_CLASS(12288), K_SIZE_CLASS(16384),
#if __arm64__
    K_SIZE_CLASS(24576), K_SIZE_CLASS(32768),
#endif /* __arm64__ */
};
...
SECURITY_READ_ONLY_LATE(struct kalloc_heap)
KHEAP_KT_VAR[1] = {{.kh_name = "kalloc.type.var",
...
static inline bool kheap_size_from_zone(void *addr, vm_size_t size,
                                        zalloc_flags_t flags) {
  vm_size_t max_size = KHEAP_MAX_SIZE;
  bool forcopyin = flags & Z_MAY_COPYINMAP;
...
kalloc_set_no_early_for_data(zone_kheap_id_t kheap_id, zone_stats_t zstats) {
...
#define kalloc_type_var(type, var)                                             \
  ((type) == KTV_FIXED ? (vm_offset_t)kalloc_type_##var##_fixed                \
                       : (vm_offset_t)kalloc_type_##var##_var)
#define kalloc_type_func(type, func, ...)                                      \
  ((type) == KTV_FIXED ? kalloc_type_##func##_fixed(__VA_ARGS__)               \
                       : kalloc_type_##func##_var(__VA_ARGS__))

TUNABLE(kalloc_type_options_t, kt_options, "kt", 0);
TUNABLE(uint16_t, kt_var_heaps, "kt_var_heaps", ZSECURITY_CONFIG_KT_VAR_BUDGET);
TUNABLE(uint16_t, kt_fixed_zones, "kt_fixed_zones", ZSECURITY_CONFIG_KT_BUDGET);
...
TUNABLE(restricted_data_mode_t, restricted_data_mode, "restricted_data_mode",
#if __x86_64__
...
inline bool kalloc_is_restricted_data_mode_telemetry(void) {
  return restricted_data_mode == RESTRICTED_DATA_MODE_TELEMETRY;
}

inline bool kalloc_is_restricted_data_mode_enforced(void) {
  return restricted_data_mode == RESTRICTED_DATA_MODE_ENFORCE;
}

inline bool kmem_needs_data_share_range(void) {
  /*
 ...
 */
extern struct kalloc_type_view
    kalloc_type_sec_start_fixed[] __SECTION_START_SYM(KALLOC_TYPE_SEGMENT,
                                                      "__kalloc_type");

extern struct kalloc_type_view
    kalloc_type_sec_end_fixed[] __SECTION_END_SYM(KALLOC_TYPE_SEGMENT,
                                                  "__kalloc_type");

/*
 * Section start/end for variable kalloc_type views
 */
extern struct kalloc_type_var_view
    kalloc_type_sec_start_var[] __SECTION_START_SYM(KALLOC_TYPE_SEGMENT,
                                                    "__kalloc_var");

extern struct kalloc_type_var_view
    kalloc_type_sec_end_var[] __SECTION_END_SYM(KALLOC_TYPE_SEGMENT,
                                                "__kalloc_var");

__startup_data static kalloc_type_views_t *kt_buffer = NULL;
__startup_data static uint64_t kt_count;
__startup_data uint32_t kalloc_type_hash_seed;

__startup_data static uint16_t kt_freq_list[MAX_K_ZONE(kt_zone_cfg)];
__startup_data static uint16_t kt_freq_list_total[MAX_K_ZONE(kt_zone_cfg)];
...
_Static_assert(__builtin_popcount(KT_SUMMARY_MASK_TYPE_BITS) ==
                   (KT_GRANULE_MAX + 1),
...
static_assert(
    KT_HASH >> 16 == (KMEM_RANGE_MASK | KMEM_HASH_SET | KMEM_DIRECTION_MASK),
...
static inline uint16_t kalloc_type_get_idx(uint32_t kt_size) {
  return (uint16_t)(kt_size >> KALLOC_TYPE_IDX_SHIFT);
}

static inline uint32_t kalloc_type_set_idx(uint32_t kt_size, uint16_t idx) {
  return kt_size | ((uint32_t)idx << KALLOC_TYPE_IDX_SHIFT);
}

static void kalloc_type_build_dlut(void) {
  vm_size_t size = 0;
...
static uint32_t kalloc_type_idx_for_size(uint32_t size) {
  assert(size <= KHEAP_MAX_SIZE);
  uint16_t idx = kalloc_type_dlut[INDEX_ZDLUT(size)];
  return kalloc_type_set_idx(size, idx);
}

static void kalloc_type_assign_zone_fixed(kalloc_type_view_t *cur,
                                          kalloc_type_view_t *end, zone_t z,
                                          zone_t sig_zone, zone_t early_zone) {
  /*
...
__startup_func static inline void kalloc_type_slide_fixed(vm_offset_t addr) {
...
__startup_func static inline void kalloc_type_slide_var(vm_offset_t addr) {
...
__startup_func static void
kalloc_type_validate_flags(kalloc_type_flags_t kt_flags, const char *kt_name,
...
static kalloc_type_flags_t kalloc_type_get_flags_var(vm_offset_t addr,
                                                     uuid_string_t kext_uuid) {
 ...
 */
static bool kalloc_type_is_data(kalloc_type_flags_t kt_flags) {
 ...
 */
static bool kalloc_type_is_ptr_array(kalloc_type_flags_t kt_flags) {
  assert(kt_flags & KT_CHANGED2);
  return kt_flags & KT_PTR_ARRAY;
}

static bool kalloc_type_from_vm(kalloc_type_flags_t kt_flags) {
  assert(kt_flags & KT_CHANGED);
  return kt_flags & KT_VM;
}

__startup_func static inline vm_size_t kalloc_type_view_sz_fixed(void) {
  return sizeof(struct kalloc_type_view);
}

__startup_func static inline vm_size_t kalloc_type_view_sz_var(void) {
  return sizeof(struct kalloc_type_var_view);
}

__startup_func static inline uint64_t
kalloc_type_view_count(kalloc_type_variant_t type, vm_offset_t start,
                       vm_offset_t end) {
  return (end - start) / kalloc_type_func(type, view_sz);
}

__startup_func static inline void
...
__startup_func static void kalloc_type_handle_data_view_var(vm_offset_t addr) {
  kalloc_type_var_view_t ktv = (kalloc_type_var_view_t)addr;
...
__startup_func static uint32_t kalloc_type_handle_parray_var(void) {
  uint32_t i = 0;
  ...
  while (kalloc_type_is_ptr_array(kt->kt_flags)) {
    ...
    p_name = c_name;
    kalloc_type_assign_zone_var(&kt_buffer[p_i].ktv_var, &kt_buffer[i].ktv_var,
                                heap_id);
  }
...
__startup_func static uint32_t kalloc_hash_adjust(uint32_t hash,
                                                  uint32_t shift) {
  /*
...
kalloc_type_set_type_hash(const char *sig_ty, const char *sig_hdr,
  ...
  assert(sig_ty != NULL);
  hash = os_hash_jenkins_update(sig_ty, strlen(sig_ty), kalloc_type_hash_seed);
  if (sig_hdr) {
...
__startup_func static void kalloc_type_set_type_hash_fixed(vm_offset_t addr) {
  /*
...
__startup_func static void kalloc_type_set_type_hash_var(vm_offset_t addr) {
  kalloc_type_var_view_t ktv = (kalloc_type_var_view_t)addr;
  kalloc_type_set_type_hash(ktv->kt_sig_type, ktv->kt_sig_hdr, &ktv->kt_flags);
}

__startup_func static void kalloc_type_mark_processed_fixed(vm_offset_t addr) {
  kalloc_type_view_t ktv = (kalloc_type_view_t)addr;
  ktv->kt_flags |= KT_PROCESSED;
}

__startup_func static void kalloc_type_mark_processed_var(vm_offset_t addr) {
  kalloc_type_var_view_t ktv = (kalloc_type_var_view_t)addr;
  ktv->kt_flags |= KT_PROCESSED;
}

__startup_func static void kalloc_type_update_view_fixed(vm_offset_t addr) {
  kalloc_type_view_t ktv = (kalloc_type_view_t)addr;
  ktv->kt_size = kalloc_type_idx_for_size(ktv->kt_size);
}

__startup_func static void kalloc_type_update_view_var(vm_offset_t addr) {
  (void)addr;
}
...
kalloc_type_view_copy(const kalloc_type_variant_t type, vm_offset_t start,
  ...
  while (cur < end) {
    ...
    kalloc_type_flags_t kt_flags =
        kalloc_type_func(type, get_flags, cur, kext_uuid);
     ...
     *
     * Note that KT_VAR_DATA_HEAP is fake "data" heap, variable kalloc_type
     * handles the actual redirection in the entry points
...
__startup_func static uint64_t
kalloc_type_view_parse(const kalloc_type_variant_t type) {
  ...
  if (kc_format == KCFormatStatic) {
     ...
     * xnu are coalesced.
     */
    kalloc_type_view_copy(type, kalloc_type_var(type, sec_start),
    ...
    while (cur < end) {
      ...
      if (kext_mh->magic == 0) {
        ...
        panic("kalloc_type_view_parse: couldn't find kext @ offset:%lx", cur);
      }
      ...
      for (uint32_t i = 0; i < kext_mh->ncmds && (vm_offset_t)lc < end;
        ...
        if (strcmp(seg_cmd->segname, KALLOC_TYPE_SEGMENT) == 0) {
          kernel_section_t *kt_sect = getsectbynamefromseg(
              seg_cmd, KALLOC_TYPE_SEGMENT, KALLOC_TYPE_SECTION(type));
          if (kt_sect) {
            kalloc_type_view_copy(type, kt_sect->addr + vm_kernel_slide,
                                  kt_sect->addr + kt_sect->size +
                                      vm_kernel_slide,
                                  &cur_count, true, NULL);
          }
        }
...
__startup_func static int kalloc_type_cmp_fixed(const void *a, const void *b) {
...
__startup_func static int kalloc_type_cmp_var(const void *a, const void *b) {
...
__startup_func static uint16_t *
...
__startup_func static uint32_t
...
__startup_func static uint16_t
kalloc_type_distribute_budget(uint16_t freq_list[MAX_K_ZONE(kt_zone_cfg)],
...
__startup_func static int kalloc_type_cmp_type_zones(const void *a,
                                                     const void *b) {
...
__startup_func static void kalloc_type_redistribute_budget(
  ...
  for (uint16_t i = 0; i < MAX_K_ZONE(kt_zone_cfg); i++) {
    if (kt_zones[i] == 0 && (freq_total_list[i] != 0) && (count > cur_count)) {
      kt_zones[i]++;
      count--;
    }
  }
...
static uint16_t
kalloc_type_apply_policy(uint16_t freq_list[MAX_K_ZONE(kt_zone_cfg)],
                         uint16_t freq_total_list[MAX_K_ZONE(kt_zone_cfg)],
  ...
  wasted_zones +=
      kalloc_type_distribute_budget(freq_list, kt_zones_sig, zbudget_sig, 2);
  wasted_zones += kalloc_type_distribute_budget(freq_total_list, kt_zones_type,
                                                zbudget_type, 0);
  ...
  if (kt_options & KT_OPTIONS_DEBUG) {
    ...
    for (uint16_t i = 0; i < MAX_K_ZONE(kt_zone_cfg); i++) {
      printf("%u\t%u\t%u\t%u\t%u\t%u\n", kt_zone_cfg[i],
             ...
             kt_zones_sig[i] + kt_zones_type[i], kt_zones_sig[i],
             kt_zones_type[i]);
    }
  }
...
__startup_func static void
kalloc_type_create_zone_for_size(zone_t *kt_zones_for_size, uint16_t kt_zones,
                                 vm_size_t z_size) {
  zone_t p_zone = NULL;
  char *z_name = NULL;
  ...
  for (uint16_t i = 0; i < kt_zones; i++) {
    ...
    snprintf(z_name, MAX_ZONE_NAME, "kalloc.type%u.%zu", i, (size_t)z_size);
  ...
  if (kt_shared_fixed) {
    ...
    if (zone_elem_inner_size(shared_z) != z_size) {
      z_name = zalloc_permanent(MAX_ZONE_NAME, ZALIGN_NONE);
      snprintf(z_name, MAX_ZONE_NAME, "kalloc.%zu", (size_t)z_size);
      shared_z =
          zone_create_ext(z_name, z_size, ZC_NONE, ZONE_ID_ANY, ^(zone_t zone) {
...
__startup_func static uint16_t
kalloc_type_zones_for_type(uint16_t zones_total_type, uint16_t unique_types,
                           uint16_t total_types, bool last_sig) {
  ...
  if (((unique_types > 3) && (zone_carry > 0)) ||
      (zone_carry >= (int)total_types) || (last_sig && (zone_carry > 0))) {
...
__startup_func static uint16_t
kalloc_type_build_skip_list(kalloc_type_view_t *start, kalloc_type_view_t *end,
...
__startup_func static void
kalloc_type_init_sig_eq(zone_t *zones, uint16_t n_zones, zone_t sig_zone) {
...
/* different init sequence in unit-test requires a bigger buffer in the kalloc
 * zones initialization */
#define KT_ZONES_FOR_SIZE_SIZE 35
#endif /* __BUILDING_XNU_LIB_UNITTEST__ */

__startup_func static uint16_t kalloc_type_distribute_zone_for_type(
    kalloc_type_view_t *start, kalloc_type_view_t *end, bool last_sig,
    uint16_t zones_total_type, uint16_t total_types, uint16_t *kt_skip_list,
    zone_t kt_zones_for_size[KT_ZONES_FOR_SIZE_SIZE], uint16_t type_zones_start,
    zone_t sig_zone, zone_t early_zone) {
  ...
  if (n_zones == 0) {
    kalloc_type_assign_zone_fixed(start, end, sig_zone, sig_zone, early_zone);
    return n_zones;
  }
...
kalloc_type_create_zones_fixed(uint16_t *kt_skip_list_start,
  ...
  uint16_t kt_zones_type[MAX_K_ZONE(kt_zone_cfg)] = {};
#if DEBUG || DEVELOPMENT
  __assert_only uint64_t kt_shuffle_count =
      ((vm_address_t)kt_shuffle_buf - (vm_address_t)kt_buffer) /
      sizeof(uint16_t);
#endif
  /*
   ...
   * class.
   */
  kalloc_type_apply_policy(kt_freq_list, kt_freq_list_total, kt_zones_sig,
                           kt_zones_type, kt_fixed_zones);
    ...
    *kt_shuffle_buf = 0;
    ...
    for (uint16_t j = 0; j < n_unique_sig; j++) {
      ...
      type_zones_used += kalloc_type_distribute_zone_for_type(
          &kt_buffer[cur].ktv_fixed, &kt_buffer[end].ktv_fixed, last_sig,
...
__startup_func static void kalloc_type_view_init_fixed(void) {
  ...
  uint16_t *kt_shuffle_buf =
      kalloc_type_create_iterators_fixed(kt_skip_list_start, kt_count);
...
__startup_func static void kalloc_type_heap_init(void) {
...
__startup_func static void kalloc_type_assign_heap(uint32_t start, uint32_t end,
                                                   uint32_t heap_id) {
...
__startup_func static void kalloc_type_split_heap(uint32_t start, uint32_t end,
                                                  uint32_t heap_id) {
...
__startup_func static void kalloc_type_view_init_var(void) {
  ...
  unique_sig = kalloc_type_create_iterators_var(kt_skip_list_start, buf_start);
  ...
  for (uint32_t i = 1; i <= unique_sig; i++) {
    uint32_t heap_id = shuffle_buf[i % (flex_heap_count + 1)] + fixed_heaps + 1;
...
__startup_func static void kalloc_init(void) {
  ...
  kmem_alloc(kernel_map, (vm_offset_t *)&kt_buffer, kt_scratch_size,
             KMA_NOFAIL | KMA_ZERO | KMA_KOBJECT | KMA_SPRAYQTN,
...
static void KALLOC_ZINFO_SALLOC(vm_size_t bytes) {
  thread_t thr = current_thread();
  ledger_debit_thread(thr, thr->t_ledger, task_ledgers.tkm_shared, bytes);
}

static void KALLOC_ZINFO_SFREE(vm_size_t bytes) {
  thread_t thr = current_thread();
  ledger_credit_thread(thr, thr->t_ledger, task_ledgers.tkm_shared, bytes);
}

static kmem_guard_t kalloc_guard(vm_tag_t tag, uint16_t type_hash,
                                 const void *owner) {
  kmem_guard_t guard = {
      .kmg_atomic = true,
      .kmg_tag = tag,
...
SECURITY_READ_ONLY_LATE(uint32_t)
kalloc_array_type_shift = KALLOC_ARRAY_TYPE_SHIFT;

__attribute__((always_inline)) struct kalloc_result
...
static inline void *__kalloc_array_encode_zone(zone_t z, void *ptr,
                                               vm_size_t size __unused) {
  return (void *)((vm_address_t)ptr | z->z_array_size_class);
}

static inline vm_address_t __kalloc_array_encode_vm(vm_address_t addr,
                                                    vm_size_t size) {
...
static inline void *__kalloc_array_encode_zone(zone_t z __unused, void *ptr,
                                               vm_size_t size) {
...
vm_size_t kalloc_next_good_size(vm_size_t size, uint32_t period) {
...
static inline kalloc_heap_t kalloc_type_get_heap(kalloc_type_flags_t kt_flags) {
  /*
...
kalloc_large(kalloc_heap_t kheap, vm_size_t req_size, zalloc_flags_t flags,
             uint16_t kt_hash, void *owner __unused) {
   ...
   */
  if ((flags & Z_NOWAIT) || (req_size >> VM_KERNEL_POINTER_SIGNIFICANT_BITS)) {
  ...
  addr = kmem_alloc_guard(kernel_map, req_size, 0, kma_flags,
                          kalloc_guard(tag, kt_hash, owner))
             .kmr_address;
...
static inline void kalloc_mark_unused_space(void *addr, vm_size_t size,
                                            vm_size_t used) {
#if KASAN_CLASSIC
  /*
...
static inline struct kalloc_result kalloc_zone(zone_t z, zone_stats_t zstats,
                                               zalloc_flags_t flags,
                                               vm_size_t req_size) {
  struct kalloc_result kr;
  vm_size_t esize;
...
static zone_id_t kalloc_use_early_heap(kalloc_heap_t kheap, zone_stats_t zstats,
                                       zone_id_t zstart,
                                       zalloc_flags_t *flags) {
...
__mockable struct kalloc_result kalloc_ext(void *kheap_or_kt_view,
...
__abortlike static void kalloc_data_require_panic(void *addr, vm_size_t size) {
  ...
  if (zid != ZONE_ID_INVALID) {
    ...
    if (!zone_is_data_kheap(zsflags.z_kheap_id)) {
      panic("kalloc_data_require failed: address %p in [%s%s]", addr,
            zone_heap_name(z), zone_name(z));
    }

    panic("kalloc_data_require failed: address %p in [%s%s], "
          "size too large %zd > %zd",
          addr, zone_heap_name(z), zone_name(z), (size_t)size,
          (size_t)zone_elem_inner_size(z));
  } else {
...
__abortlike static void kalloc_non_data_require_panic(void *addr,
                                                      vm_size_t size) {
  zone_id_t zid = zone_id_for_element(addr, size);
  ...
  if (zid != ZONE_ID_INVALID) {
    ...
    case KHEAP_ID_KT_VAR:
      panic("kalloc_non_data_require failed: address %p in [%s%s]", addr,
            zone_heap_name(z), zone_name(z));
    default:
      break;
    }
...
void kalloc_data_require(void *addr, vm_size_t size) {
  zone_id_t zid = zone_id_for_element(addr, size);
  ...
  } else if (kmem_range_id_contains(KMEM_RANGE_ID_DATA, (vm_address_t)addr,
                                    size)) {
    return;
...
void kalloc_non_data_require(void *addr, vm_size_t size) {
  zone_id_t zid = zone_id_for_element(addr, size);
...
bool kalloc_is_data_buffers(void *addr, vm_size_t size) {
  zone_id_t zid = zone_id_for_element(addr, size);
...
__mockable void *kalloc_type_impl_external(kalloc_type_view_t kt_view,
                                           zalloc_flags_t flags) {
  /*
...
void *kalloc_type_var_impl_external(kalloc_type_var_view_t kt_view,
                                    vm_size_t size, zalloc_flags_t flags,
                                    void *owner);
void *kalloc_type_var_impl_external(kalloc_type_var_view_t kt_view,
                                    vm_size_t size, zalloc_flags_t flags,
...
__abortlike static void kfree_heap_confusion_panic(kalloc_heap_t kheap,
  ...
  if (zsflags.z_kalloc_type) {
    ...
    panic("kfree: addr %p found in kalloc type zone '%s'"
          "but being freed to %s heap",
          data, z->z_name, kheap_name);
  }

  if (zsflags.z_kheap_id == KHEAP_ID_NONE) {
    panic("kfree: addr %p, size %zd found in regular zone '%s%s'", data, size,
          zone_heap_name(z), z->z_name);
  } else {
    panic("kfree: addr %p, size %zd found in heap %s* instead of %s*", data,
          size, zone_heap_name(z), kheap_name);
  }
}

__abortlike static void kfree_size_confusion_panic(zone_t z, void *data,
                                                   size_t oob_offs, size_t size,
                                                   size_t zsize) {
  if (z) {
    ...
    panic("kfree: addr %p, size %zd (offs:%zd) not found in any zone", data,
          size, oob_offs);
  }
}

__abortlike static void kfree_size_invalid_panic(void *data, size_t size) {
  panic("kfree: addr %p trying to free with nonsensical size %zd", data, size);
}

__abortlike static void kfree_size_require_panic(void *data, size_t size,
                                                 size_t min_size,
                                                 size_t max_size) {
...
static void kfree_size_require(kalloc_heap_t kheap, void *addr,
                               vm_size_t min_size, vm_size_t max_size) {
...
static void kfree_large(vm_offset_t addr, vm_size_t size, kmf_flags_t flags,
                        void *owner) {
  size =
      kmem_free_guard(kernel_map, addr, size, flags | KMF_TAG | KMF_KASAN_GUARD,
...
static void kfree_zone(void *kheap_or_kt_view __unsafe_indexable, void *data,
                       vm_size_t size, zone_t z, vm_size_t zsize) {
  zone_security_flags_t zsflags = zone_security_config(z);
  kalloc_type_var_view_t kt_view;
  kalloc_heap_t kheap;
  zone_stats_t zstats = NULL;
...
__mockable void kfree_ext(void *kheap_or_kt_view, void *data, vm_size_t size) {
  ...
  if (size <= KHEAP_MAX_SIZE) {
    ...
    if (size + oob_offs > bucket_size || bucket_size == 0) {
      kfree_size_confusion_panic(z, data, oob_offs, size, bucket_size);
    }
...
void kfree_addr_ext(kalloc_heap_t kheap, void *data) {
...
#if XNU_PLATFORM_MacOSX
void kfree_external(void *addr, vm_size_t size);
void kfree_external(void *addr, vm_size_t size) {
...
void(kheap_free_bounded)(kalloc_heap_t kheap, void *addr, vm_size_t min_sz,
                         vm_size_t max_sz) {
...
__mockable void *kalloc_type_impl_internal(kalloc_type_view_t kt_view,
                                           zalloc_flags_t flags) {
...
void kfree_type_impl_external(kalloc_type_view_t kt_view, void *ptr) {
  /*
...
void kfree_type_var_impl_external(kalloc_type_var_view_t kt_view, void *ptr,
                                  vm_size_t size);
void kfree_type_var_impl_external(kalloc_type_var_view_t kt_view, void *ptr,
                                  vm_size_t size) {
...
void kfree_data_external(void *ptr, vm_size_t size);
void kfree_data_external(void *ptr, vm_size_t size) {
  return kheap_free(GET_KEXT_KHEAP_DATA(), ptr, size);
}

void kfree_data_addr_external(void *ptr);
void kfree_data_addr_external(void *ptr) {
  return kheap_free_addr(GET_KEXT_KHEAP_DATA(), ptr);
}

void kfree_shared_data_external(void *ptr, vm_size_t size);
void kfree_shared_data_external(void *ptr, vm_size_t size) {
  return kheap_free(KHEAP_DATA_SHARED, ptr, size);
}

void kfree_shared_data_addr_external(void *ptr);
void kfree_shared_data_addr_external(void *ptr) {
...
__abortlike static void krealloc_size_invalid_panic(void *data, size_t size) {
  panic("krealloc: addr %p trying to free with nonsensical size %zd", data,
        size);
}

__attribute__((noinline)) static struct kalloc_result
krealloc_large(kalloc_heap_t kheap, vm_offset_t addr, vm_size_t old_size,
               vm_size_t new_size, zalloc_flags_t flags, uint16_t kt_hash,
               void *owner __unused) {
  ...
  if (addr != 0 || (flags & Z_REALLOCF)) {
    DTRACE_VM3(kfree, vm_size_t, old_size, vm_size_t, old_req_size, void *,
               addr);
  }
  ...
  DTRACE_VM3(kalloc, vm_size_t, new_size, vm_size_t, new_req_size, void *,
             kmr.kmr_address);

  if (flags & Z_KALLOC_ARRAY) {
    kmr.kmr_address = __kalloc_array_encode_vm(kmr.kmr_address, new_req_size);
  }
...
struct kalloc_result krealloc_ext(void *kheap_or_kt_view __unsafe_indexable,
                                  void *addr, vm_size_t old_size,
                                  vm_size_t new_size, zalloc_flags_t flags,
                                  void *owner) {
  ...
  if (new_size == 0) {
    ...
    new_bucket_size =
        new_z ? zone_elem_inner_size(new_z) : round_page(new_size);
  }
#if !KASAN_CLASSIC
  if (flags & Z_FULLSIZE) {
  ...
  if (addr == NULL) {
    ...
    if (old_size + oob_offs > old_bucket_size || old_bucket_size == 0) {
      kfree_size_confusion_panic(old_z, addr, oob_offs, old_size,
                                 old_bucket_size);
    }
  ...
  if (old_bucket_size == new_bucket_size && old_z) {
    ...
    kr.size = new_size;
#if ZSECURITY_CONFIG(PGZ_OOB_ADJUST)
    kr.addr = zone_element_pgz_oob_adjust(kr.addr, new_size, new_bucket_size);
    if (kr.addr != addr) {
      memmove(kr.addr, addr, min_size);
      bzero((char *)kr.addr + min_size, kr.size - min_size);
    }
#endif /* !ZSECURITY_CONFIG(PGZ_OOB_ADJUST) */
#if KASAN
    /*
    ...
    kasan_alloc((vm_offset_t)addr, new_bucket_size, kr.size, KASAN_GUARD_SIZE,
                false, __builtin_frame_address(0));
#endif /* KASAN_CLASSIC */
#if KASAN_TBI
    /*
  ...
  if (addr && !old_z && new_size && !new_z) {
    return krealloc_large(kheap, (vm_offset_t)addr, old_size, new_size, flags,
                          kt_hash, owner);
  }
  ...
  if (!new_size) {
    ...
    kr = kalloc_zone(new_z, zstats, flags & ~Z_KALLOC_ARRAY, new_size);
  } else if (old_z || addr == NULL) {
    kr = kalloc_large(kheap, new_size, flags & ~Z_KALLOC_ARRAY, kt_hash, owner);
  }
  ...
  if (addr && (kr.addr || (flags & Z_REALLOCF) || !new_size)) {
    if (old_z) {
      kfree_zone(kheap_or_kt_view, (char *)addr - oob_offs, old_size, old_z,
                 old_bucket_size);
    } else {
...
out_success:
  ...
  if (new_z) {
    kr.addr = __kalloc_array_encode_zone(new_z, kr.addr, kr.size);
  } else {
    kr.addr = (void *)__kalloc_array_encode_vm((vm_offset_t)kr.addr, kr.size);
  }
  return kr;
}

void *krealloc_data_external(void *ptr, vm_size_t old_size, vm_size_t new_size,
                             zalloc_flags_t flags);
void *krealloc_data_external(void *ptr, vm_size_t old_size, vm_size_t new_size,
                             zalloc_flags_t flags) {
  flags = Z_VM_TAG_BT(flags & Z_KPI_MASK, VM_KERN_MEMORY_KALLOC_DATA);
  return krealloc_ext(GET_KEXT_KHEAP_DATA(), ptr, old_size, new_size, flags,
                      NULL)
      .addr;
}

void *krealloc_shared_data_external(void *ptr, vm_size_t old_size,
                                    vm_size_t new_size, zalloc_flags_t flags);
void *krealloc_shared_data_external(void *ptr, vm_size_t old_size,
                                    vm_size_t new_size, zalloc_flags_t flags) {
  flags = Z_VM_TAG_BT(flags & Z_KPI_MASK, VM_KERN_MEMORY_KALLOC_SHARED);
...
__startup_func static void kheap_init(kalloc_heap_t parent_heap,
                                      kalloc_heap_t kheap) {
...
__startup_func static void kheap_init_var(kalloc_heap_t kheap) {
  ...
  kheap->kh_zstart = parent_heap->kh_zstart;
  kheap->kh_type_hash =
      (uint16_t)kalloc_hash_adjust((uint32_t)early_random(), 0);
  kheap->kh_views = parent_heap->kh_views;
  parent_heap->kh_views = kheap;
}

__startup_func void kheap_startup_init(kalloc_heap_t kheap) {
  ...
  case KHEAP_ID_KT_VAR:
    ...
    break;
  default:
    panic("kalloc_heap_startup_init: invalid KHEAP_ID: %d", kheap->kh_heap_id);
  }
}
...
void *kern_os_malloc_external(size_t size);
void *kern_os_malloc_external(size_t size) {
  if (size == 0) {
    return NULL;
  }
...
void kern_os_free_external(void *addr);
void kern_os_free_external(void *addr) {
  kheap_free_addr(KERN_OS_MALLOC, addr);
}

void *kern_os_realloc_external(void *addr, size_t nsize);
void *kern_os_realloc_external(void *addr, size_t nsize) {
  ...
  return __kheap_realloc(KERN_OS_MALLOC, addr, osize - oob_offs, nsize, flags,
                         NULL);
}

#endif /* XNU_PLATFORM_MacOSX */

void kern_os_zfree(zone_t zone, void *addr, vm_size_t size) {
...
bool IOMallocType_from_vm(kalloc_type_view_t ktv) {
  return kalloc_type_from_vm(ktv->kt_flags);
}

void kern_os_typed_free(kalloc_type_view_t ktv, void *addr, vm_size_t esize) {
#if ZSECURITY_CONFIG(STRICT_IOKIT_FREE)
#pragma unused(esize)
#else
  /*
 ...
 */
static int kalloc_type_feature_on(void) {
  ...
  for (uint16_t i = 0; i < MAX_K_ZONE(kt_zone_cfg); i++) {
    ...
    while (z != NULL) {
      zone_found = true;
      if (strncmp(z->z_name, kalloc_type_str, strlen(kalloc_type_str)) != 0) {
        return 0;
      }
      z = z->z_kt_next;
    }
 ...
 */
static int kalloc_type_test_policy(int64_t in) {
  ...
  wasted_zone_budget = kalloc_type_apply_policy(
      freq_list, freq_total_list, zones_per_sig, zones_per_type, zone_budget);
  ...
  for (uint16_t i = 0; i < MAX_K_ZONE(kt_zone_cfg); i++) {
    uint16_t n_zones_for_type = kalloc_type_zones_for_type(
        n_zones, freq_total_list[i], total_types,
 ...
 */
static int kalloc_type_check_size(zone_t z) {
...
static int kalloc_type_test_data_redirect(void) {
...
static int run_kalloc_type_test(int64_t in, int64_t *out) {
  *out = 0;
  for (uint16_t i = 0; i < MAX_K_ZONE(kt_zone_cfg); i++) {
    zone_t z = kalloc_type_zarray[i];
    while (z != NULL) {
...
static vm_size_t test_bucket_size(kalloc_heap_t kheap, vm_size_t size) {
  zone_t z = kalloc_zone_for_size(kheap->kh_zstart, size);

  return z ? zone_elem_inner_size(z) : round_page(size);
}

static int run_kalloc_test_kheap(kalloc_heap_t kheap) {
  uint64_t *data_ptr;
   ...
   */
  data_ptr = kalloc_ext(kheap, alloc_size, Z_WAITOK | Z_NOFAIL, NULL).addr;
  if (!data_ptr) {
  ...
  data_ptr = krealloc_ext(kheap, kr.addr, old_alloc_size, alloc_size,
  ...
  *data_ptr = 0;
  ...
  alloc_size = 3544;
  data_ptr =
      kalloc_ext(kheap, alloc_size, Z_WAITOK | Z_FULLSIZE, &data_ptr).addr;
  if (!data_ptr) {
  ...
  *data_ptr = 0;
  ...
  data_ptr = krealloc_ext(kheap, data_ptr, alloc_size, PAGE_SIZE * 2,
                          Z_REALLOCF | Z_WAITOK, &data_ptr)
                 .addr;
  if (!data_ptr) {
  ...
  *data_ptr = 0;
  ...
  data_ptr = krealloc_ext(kheap, data_ptr, PAGE_SIZE * 2, KHEAP_MAX_SIZE * 2,
                          Z_REALLOCF | Z_WAITOK, &data_ptr)
                 .addr;
  if (!data_ptr) {
  ...
  *data_ptr = 0;
  ...
  data_ptr = krealloc_ext(kheap, data_ptr, KHEAP_MAX_SIZE * 2,
                          KHEAP_MAX_SIZE * 4, Z_REALLOCF | Z_WAITOK, &data_ptr)
                 .addr;
  *data_ptr = 0;
  if (!data_ptr) {
  ...
  krealloc_ext(kheap, data_ptr, KHEAP_MAX_SIZE * 4, 0, Z_REALLOCF | Z_WAITOK,
               &data_ptr);
...
static int run_kalloc_test(int64_t in __unused, int64_t *out) {
  *out = 1;
