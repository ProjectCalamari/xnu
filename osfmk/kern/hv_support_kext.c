...
static hv_trap_table_t hv_trap_table[] = {
    [HV_TASK_TRAP] = {.traps = NULL, .trap_count = 0},
    [HV_THREAD_TRAP] = {.traps = NULL, .trap_count = 0}};
...
void *hv_get_task_target(void) { return current_task()->hv_task_target; }

/* get hv object associated with the current thread */
void *hv_get_thread_target(void) { return current_thread()->hv_thread_target; }
...
kern_return_t hv_set_traps(hv_trap_type_t trap_type, const hv_trap_t *traps,
                           unsigned trap_count) {
...
kern_return_t hv_task_trap(uint64_t index, uint64_t arg) {
  KDBG(MACHDBG_CODE(DBG_MACH_HV, HV_X86_TRAP_TASK) | DBG_FUNC_START, index,
       arg);
  DTRACE_HV2(task__trap__begin, uint64_t, index, uint64_t, arg);

  kern_return_t ret =
      HV_TRAP_DISPATCH(HV_TASK_TRAP, index, hv_get_task_target(), arg);
...
kern_return_t hv_thread_trap(uint64_t index, uint64_t arg) {
  KDBG(MACHDBG_CODE(DBG_MACH_HV, HV_X86_TRAP_THREAD) | DBG_FUNC_START, index,
       arg);
  DTRACE_HV2(thread__trap__begin, uint64_t, index, uint64_t, arg);

  kern_return_t ret =
      HV_TRAP_DISPATCH(HV_THREAD_TRAP, index, hv_get_thread_target(), arg);

  DTRACE_HV2(thread__trap__end, uint64_t, index, uint64_t, ret);
  KDBG(MACHDBG_CODE(DBG_MACH_HV, HV_X86_TRAP_THREAD) | DBG_FUNC_END, index,
       ret);
