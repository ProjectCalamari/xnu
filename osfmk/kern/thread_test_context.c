...
void thread_test_context_deinit(thread_test_context_t *ctx) {
  /*
...
static int thread_test_context_tests(int64_t in_value __unused,
                                     int64_t *out_value) {
  *out_value = 0;
  ...
  int data = 0;
  {
    ...
    thread_set_test_option(ttc_testing_ttc_int,
                           1 + thread_get_test_option(ttc_testing_ttc_int));
    if (thread_get_test_option(ttc_testing_ttc_int) != 0) {
      FAIL;
    }
    ...
    /* setting an option with no context has no effect */
    thread_set_test_option(ttc_testing_ttc_struct,
                           (struct mach_vm_range){55, 66});
    if (thread_get_test_option(ttc_testing_ttc_struct).min_address != 0) {
      FAIL;
    }
