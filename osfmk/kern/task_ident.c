...
/* Exported to kexts */
extern typeof(task_id_token_port_name_to_task)
...
static void tidt_reference(task_id_token_t token) {
...
static void tidt_release(task_id_token_t token) {
  ipc_port_t port;
  ...
  if (IP_VALID(port)) {
    ...
    /*
     * Ports of type IKOT_TASK_FATAL use task_ident objects to avoid holding a
     * task reference and are created to send resource limit notifications
     */
    ipc_kobject_type_t kotype = ip_type(port);
    release_assert(kotype == IKOT_TASK_ID_TOKEN || kotype == IKOT_TASK_FATAL);
    ipc_kobject_dealloc_port(port, IPC_KOBJECT_NO_MSCOUNT, kotype);
#else  /* CONFIG_PROC_RESOURCE_LIMITS */
    ipc_kobject_dealloc_port(port, IPC_KOBJECT_NO_MSCOUNT, IKOT_TASK_ID_TOKEN);
...
void task_id_token_release(task_id_token_t token) { tidt_release(token); }

static void task_id_token_no_senders(ipc_port_t port,
...
IPC_KOBJECT_DEFINE(IKOT_TASK_ID_TOKEN, .iko_op_movable_send = true,
                   .iko_op_stable = true,
...
kern_return_t task_create_identity_token(task_t task, task_id_token_t *tokenp) {
  ...
  if (task_is_a_corpse(task)) {
    ...
    token->ident =
        proc_ident_with_policy(bsd_info, IDENT_VALIDATION_PROC_EXACT);
  } else {
    task_unlock(task);
    zfree(task_id_token_zone, token);
    return KERN_INVALID_ARGUMENT;
...
kern_return_t task_identity_token_get_task_grp(task_id_token_t token,
  ...
  if (token->task_uniqueid) {
    kr = find_corpse_task_by_uniqueid_grp(token->task_uniqueid, &task,
                                          grp); /* produces ref */
    if (kr) {
...
kern_return_t task_identity_token_get_task_port(task_id_token_t token,
  ...
  if ((kr = task_identity_token_get_task_grp(token, &task, TASK_GRP_KERNEL)) !=
      KERN_SUCCESS) {
    return kr;
  }
...
static kern_return_t task_id_token_port_name_to_task_grp(mach_port_name_t name,
                                                         task_t *task,
                                                         task_grp_t grp) {
  kern_return_t kr;
  task_id_token_t token;
...
/* Used by kexts only */
kern_return_t task_id_token_port_name_to_task_external(mach_port_name_t name,
                                                       task_t *task) {
...
ipc_port_t convert_task_id_token_to_port(task_id_token_t token) {
   ...
   */
  if (!ipc_kobject_make_send_lazy_alloc_port(&token->port, token,
                                             IKOT_TASK_ID_TOKEN)) {
    tidt_release(token);
  }
