...
extern bool
processor_is_fast_track_candidate_for_realtime_thread(processor_set_t pset,
...
extern boolean_t thread_stop(thread_t thread, boolean_t until_not_runnable);

/* Release a previous stop request */
extern void thread_unstop(thread_t thread);

/* Wait for a thread to stop running */
extern void thread_wait(thread_t thread, boolean_t until_not_runnable);

/* Unblock thread on wake up */
extern boolean_t thread_unblock(thread_t thread, wait_result_t wresult);

/* Unblock and dispatch thread */
extern void thread_go(thread_t thread, wait_result_t wresult, bool try_handoff);

/* Check if direct handoff is allowed */
extern boolean_t thread_allowed_for_handoff(thread_t thread);

/* Handle threads at context switch */
extern void thread_dispatch(thread_t old_thread, thread_t new_thread);

/* Switch directly to a particular thread */
extern int thread_run(thread_t self, thread_continue_t continuation,
...
/* Invoke continuation */
extern __dead2 void call_continuation(thread_continue_t continuation,
                                      void *parameter, wait_result_t wresult,
 ...
 */
__options_decl(
    set_sched_pri_options_t, uint32_t,
    {
        SETPRI_DEFAULT = 0x0,
...
extern void set_sched_pri(thread_t thread, int16_t priority,
                          set_sched_pri_options_t options);

/* Set base priority of the specified thread */
extern void sched_set_thread_base_priority(thread_t thread, int priority);

/* Set absolute base priority of the specified thread */
extern void sched_set_kernel_thread_priority(thread_t thread, int priority);

/* Set the thread's true scheduling mode */
extern void sched_set_thread_mode(thread_t thread, sched_mode_t mode);

/*
 * Set the thread's scheduling mode taking into account that the thread may have
 * been demoted.
 * */
extern void sched_set_thread_mode_user(thread_t thread, sched_mode_t mode);
...
extern void sched_thread_mode_demote(thread_t thread, uint32_t reason);
/* Un-demote the true scheduler mode */
extern void sched_thread_mode_undemote(thread_t thread, uint32_t reason);
/* Check for a specific demotion */
extern bool sched_thread_mode_has_demotion(thread_t thread, uint32_t reason);

extern void sched_thread_promote_reason(thread_t thread, uint32_t reason,
                                        uintptr_t trace_obj);
extern void sched_thread_unpromote_reason(thread_t thread, uint32_t reason,
                                          uintptr_t trace_obj);
...
extern void thread_recompute_sched_pri(thread_t thread,
                                       set_sched_pri_options_t options);
...
extern boolean_t can_update_priority(thread_t thread);

extern void update_priority(thread_t thread);

extern void lightweight_update_priority(thread_t thread);

extern void sched_default_quantum_expire(thread_t thread);

/* Idle processor thread continuation */
extern void idle_thread(void *parameter, wait_result_t result);

extern void idle_thread_create(processor_t processor,
                               thread_continue_t continuation);

/* Continuation return from syscall */
extern void thread_syscall_return(kern_return_t ret);

/* Context switch */
extern wait_result_t thread_block_reason(thread_continue_t continuation,
...
extern void thread_setrun(thread_t thread, sched_options_t options);

extern processor_set_t task_choose_pset(task_t task);

/* Bind the current thread to a particular processor */
extern processor_t thread_bind(processor_t processor);

extern void thread_bind_during_wakeup(thread_t thread, processor_t processor);

extern void thread_unbind_after_queue_shutdown(thread_t thread,
                                               processor_t processor);

extern bool pset_has_stealable_threads(processor_set_t pset);

extern processor_set_t choose_starting_pset(pset_node_t node, thread_t thread,
                                            processor_t *processor_hint);

extern int pset_available_cpu_count(processor_set_t pset);

extern bool pset_is_recommended(processor_set_t pset);

extern bool pset_type_is_recommended(processor_set_t pset);

extern pset_node_t sched_choose_node(thread_t thread);

#if CONFIG_SCHED_SMT
extern processor_t choose_processor_smt(processor_set_t pset,
                                        processor_t processor, thread_t thread,
                                        sched_options_t *options);
#else  /* !CONFIG_SCHED_SMT */
/* Choose the best processor to run a thread */
extern processor_t choose_processor(processor_set_t pset, processor_t processor,
                                    thread_t thread, sched_options_t *options);
#endif /* !CONFIG_SCHED_SMT */

extern bool sched_SMT_balance(processor_t processor, processor_set_t pset);

extern void thread_quantum_init(thread_t thread, uint64_t now);

extern void run_queue_init(run_queue_t runq);

extern thread_t run_queue_dequeue(run_queue_t runq, sched_options_t options);

extern boolean_t run_queue_enqueue(run_queue_t runq, thread_t thread,
                                   sched_options_t options);

extern void run_queue_remove(run_queue_t runq, thread_t thread);

extern thread_t run_queue_peek(run_queue_t runq);
...
extern void sched_pset_made_schedulable(processor_t processor,
...
/* sched_ipi_deferred_policy() is the global default deferred IPI policy for all
 * schedulers */
...
extern boolean_t runq_scan(run_queue_t runq,
                           sched_update_scan_context_t scan_context);

#if CONFIG_SCHED_CLUTCH
extern boolean_t
sched_clutch_timeshare_scan(queue_t thread_queue, uint16_t count,
                            sched_update_scan_context_t scan_context);
...
extern void thread_timer_expire(void *thread, void *p1);
...
thread_interrupt_level(wait_interrupt_t interruptible);

__private_extern__ wait_result_t
thread_mark_wait_locked(thread_t thread, wait_interrupt_t interruptible);
...
extern void sched_stats_handle_csw(processor_t processor, int reasons,
                                   int selfpri, int otherpri);

extern void sched_stats_handle_runq_change(struct runq_stats *stats,
                                           int old_count);
...
#define SCHED_STATS_CSW(processor, reasons, selfpri, otherpri)                 \
  MACRO_BEGIN                                                                  \
  if (__improbable(sched_stats_active)) {                                      \
    sched_stats_handle_csw((processor), (reasons), (selfpri), (otherpri));     \
  }                                                                            \
  MACRO_END
...
extern void active_rt_threads(boolean_t active);

/* Returns the perfcontrol attribute for the thread */
extern perfcontrol_class_t thread_get_perfcontrol_class(thread_t thread);
...
__options_decl(
    thread_bind_option_t, uint64_t,
    {
        /* Unbind a previously cluster bound thread */
        THREAD_UNBIND = 0x1,
        /*
         * Bind thread to the cluster only if it is eligible to run on that
         * cluster. If the thread is not eligible to run on the cluster,
         * thread_soft_bind_cluster_id() returns KERN_INVALID_POLICY.
         */
...
__attribute__((nonnull(2))) extern void
thread_handoff_parameter(thread_t thread, thread_continue_t continuation,
                         void *parameter, thread_handoff_option_t) __dead2;
 ...
 * sched_cond_t:
 *
 * A atomic condition variable used to synchronize wake/block operations on
 * threads. Bits defined below are reserved for use by sched_prim. Remaining
 * bits may be used by caller for additional synchronization semantics.
 */
__options_decl(sched_cond_t, uint32_t,
               {
                   SCHED_COND_INIT = 0x0000,   /* initialize all bits to zero
                                                  (inactive and not awoken) */
                   SCHED_COND_ACTIVE = 0x0001, /* target thread is active */
                   SCHED_COND_WAKEUP =
                       0x0002 /* wakeup has been issued for target thread */
               });
 ...
 * sched_cond_init:
 *
 * Initialize an atomic condition variable. Note that this does not occur
 * atomically and should be performed during thread initialization, before the
 * condition is observable by other threads.
 */
extern void sched_cond_init(sched_cond_atomic_t *cond);

/*
 * sched_cond_signal:
 *
 * Wakeup the specified thread if it is waiting on this event and it has not
 * already been issued a wakeup.
 *
 * parameters:
 *      thread    thread to awaken
 *      cond      atomic condition variable
 */
extern kern_return_t sched_cond_signal(sched_cond_atomic_t *cond,
                                       thread_t thread);
 ...
 * If a wakeup has been issued on cond since the last `sched_cond_ack`,
 * clear_wait and return `THREAD_AWAKENED`.
 *
 ...
 *      parameter
 */
extern wait_result_t sched_cond_wait_parameter(sched_cond_atomic_t *cond,
 ...
 *      continuation     continuation if block succeeds
 */
extern wait_result_t sched_cond_wait(sched_cond_atomic_t *cond,
                                     wait_interrupt_t interruptible,
                                     thread_continue_t continuation);
 ...
 * It is the callers responsibility to ensure that the ACTIVE bit is always low
 * prior to calling (i.e. by calling `sched_cond_wait` prior to any rerun or
 * block). Synchronization schemes that allow for WAKEUP bit to be reset prior
 * to wakeup (e.g. a cancellation mechanism) should check that WAKEUP was indeed
 * cleared.
 *
 * e.g.
 * ```
 ...
 *      cond:    atomic condition variable
 */
extern sched_cond_t sched_cond_ack(sched_cond_atomic_t *cond);
 ...
 */
extern void thread_set_pending_block_hint(thread_t thread,
...
#if XNU_KERNEL_PRIVATE
extern void thread_yield_with_continuation(thread_continue_t continuation,
...
extern wait_result_t thread_block_parameter(thread_continue_t continuation,
                                            void *parameter);
...
extern wait_result_t assert_wait_timeout(event_t event,
                                         wait_interrupt_t interruptible,
                                         uint32_t interval,
...
assert_wait_timeout_with_leeway(event_t event, wait_interrupt_t interruptible,
                                wait_timeout_urgency_t urgency,
                                uint32_t interval, uint32_t leeway,
                                uint32_t scale_factor);

extern wait_result_t assert_wait_deadline(event_t event,
                                          wait_interrupt_t interruptible,
                                          uint64_t deadline);
 ...
 */
extern wait_result_t
assert_wait_deadline_with_leeway(event_t event, wait_interrupt_t interruptible,
                                 wait_timeout_urgency_t urgency,
                                 uint64_t deadline, uint64_t leeway);

/* Wake up thread (or threads) waiting on a particular event */
extern kern_return_t thread_wakeup_prim(event_t event, boolean_t one_thread,
                                        wait_result_t result);

/* Wake up up to given number of threads waiting on a particular event */
extern kern_return_t thread_wakeup_nthreads_prim(event_t event,
                                                 uint32_t nthreads,
                                                 wait_result_t result);
...
/*
 * The scheduling policy is fixed at compile-time, in order to save the
 * performance cost of function pointer indirection that we would otherwise pay
 * each time when making a policy-specific callout.
 */
#if __AMP__
...
struct sched_dispatch_table {
  ...
  void (*processor_init)(
      processor_t processor); /* Per-processor scheduler init */
  void (*pset_init)(
      processor_set_t pset); /* Per-processor set scheduler init */
   ...
   */
  thread_t (*choose_thread)(processor_t processor, int priority,
   ...
   */
  processor_t (*choose_processor)(processor_set_t pset, processor_t processor,
                                  thread_t thread, sched_options_t *options);
   ...
   * Used to send IPIs to a processor which would be preferred to be idle
   * instead. Returns true if the current processor should anticipate a quick
   * IPI reply back from another core. Called with pset lock held, returns with
   * pset lock unlocked.
   */
  bool (*processor_balance)(processor_t processor, processor_set_t pset);

  processor_t (*rt_choose_processor)(processor_set_t starting_pset,
                                     processor_t starting_processor,
                                     thread_t thread);
  ...
  /* Routine to inform the scheduler when CLPC changes a thread group
   * recommendation */
  void (*thread_group_recommendation_change)(struct thread_group *tg,
