 ...
 *  as well as importance inheritance, process suppression, task QoS, and
 * apptype. These properties have a suprising number of complex interactions, so
 * they are centralized here in one state machine to simplify the implementation
 * of those interactions.
 *
 *  Architecture:
 *  Threads and tasks have two policy fields: requested, effective.
 *  Requested represents the wishes of each interface that influences task
 * policy. Effective represents the distillation of that policy into a set of
 * behaviors.
 *
 *  Each thread making a modification in the policy system passes a 'pending'
 * struct, which tracks updates that will be applied after dropping the policy
 * engine lock.
 *
 *  Each interface that has an input into the task policy state machine controls
 * a field in requested. If the interface has a getter, it returns what is in
 * the field in requested, but that is not necessarily what is actually in
 * effect.
 *
 *  All kernel subsystems that behave differently based on task policy call into
 *  the proc_get_effective_(task|thread)_policy functions, which return the
 * decision of the task policy state machine for that subsystem by querying only
 * the 'effective' field.
 *
 ...
 *  3) Run a task policy update, which recalculates effective based on
 * requested, then takes a diff between the old and new versions of requested
 * and calls the relevant other subsystems to apply these changes, and updates
 * the pending field. 4) Unlock task 5) Run task policy update complete, which
 * looks at the pending field to update subsystems which cannot be touched while
 * holding the task lock.
 *
 *  To add a new requested policy, add the field in the requested struct, the
 * flavor in task.h, the setter and getter in proc_(set|get)_task_policy*, then
 * set up the effects of that behavior in task_policy_update*. If the policy
 * manifests itself as a distinct effective policy, add it to the effective
 * struct and add it to the proc_get_effective_task_policy accessor.
 *
 *  Most policies are set via proc_set_task_policy, but policies that don't fit
 * that interface roll their own lock/set/update/unlock/complete code inside
 * this file.
 *
 *
 *  Suppression policy
 *
 *  These are a set of behaviors that can be requested for a task.  They
 * currently have specific implied actions when they're enabled, but they may be
 * made customizable in the future.
 *
 *  When the affected task is boosted, we temporarily disable the suppression
 * behaviors so that the affected process has a chance to run so it can call the
 * API to permanently disable the suppression behaviors.
 *
 *  Locking
 *
 *  Changing task policy on a task takes the task lock.
 *  Changing task policy on a thread takes the thread mutex.
 *  Task policy changes that affect threads will take each thread's mutex to
 ...
 *
 *  This means that any notification of state change needs to be externally
 * synchronized. We do this by idempotent callouts after the state has changed
 * to ask other subsystems to update their view of the world.
 *
...
static void proc_set_task_policy_locked(task_t task, int category, int flavor,
                                        int value, int value2);

static void task_policy_update_locked(task_t task,
                                      task_pend_token_t pend_token);
static void task_policy_update_internal_locked(task_t task, bool in_create,
                                               task_pend_token_t pend_token);

/* For attributes that have two scalars as input/output */
static void proc_set_task_policy2(task_t task, int category, int flavor,
                                  int value1, int value2);
static void proc_get_task_policy2(task_t task, int category, int flavor,
                                  int *value1, int *value2);

static boolean_t task_policy_update_coalition_focal_tasks(
    task_t task, int prev_role, int next_role, task_pend_token_t pend_token);
...
static int task_set_cpuusage(task_t task, uint8_t percentage, uint64_t interval,
                             uint64_t deadline, int scope, int entitled);
static int task_get_cpuusage(task_t task, uint8_t *percentagep,
...
static void task_action_cpuusage(thread_call_param_t param0,
...
extern int proc_pidpathinfo_internal(proc_t p, uint64_t arg, char *buffer,
...
static int task_importance_hold_internal_assertion(task_t target_task,
                                                   uint32_t count);

static void task_add_importance_watchport(task_t task, mach_port_t port,
                                          int *boostp);
...
/* Latency/throughput QoS fields remain zeroed, i.e. TIER_UNSPECIFIED at
 * creation */
...
kern_return_t qos_latency_policy_validate(task_latency_qos_t ltier) {
...
kern_return_t qos_throughput_policy_validate(task_throughput_qos_t ttier) {
...
static kern_return_t task_qos_policy_validate(task_qos_policy_t qosinfo,
...
uint32_t qos_extract(uint32_t qv) { return qv & 0xFF; }

uint32_t qos_latency_policy_package(uint32_t qv) {
  return (qv == LATENCY_QOS_TIER_UNSPECIFIED) ? LATENCY_QOS_TIER_UNSPECIFIED
                                              : ((0xFF << 16) | qv);
}

uint32_t qos_throughput_policy_package(uint32_t qv) {
  return (qv == THROUGHPUT_QOS_TIER_UNSPECIFIED)
...
static boolean_t task_policy_suppression_flags =
    TASK_POLICY_SUPPRESSION_IOTIER2 | TASK_POLICY_SUPPRESSION_NONDONOR;

static void task_set_requested_apptype(task_t task, uint64_t apptype,
                                       __unused boolean_t update_tg_flag) {
...
kern_return_t task_policy_set(task_t task, task_policy_flavor_t flavor,
  ...
  case TASK_CATEGORY_POLICY: {
    ...
    case TASK_DEFAULT_APPLICATION:
      proc_set_task_policy(task, TASK_POLICY_ATTRIBUTE, TASK_POLICY_ROLE,
                           info->role);
      break;
  ...
  case TASK_BASE_LATENCY_QOS_POLICY: {
    ...
    proc_set_task_policy(task, TASK_POLICY_ATTRIBUTE,
                         TASK_BASE_LATENCY_QOS_POLICY, lqos);
  } break;
  ...
  case TASK_BASE_THROUGHPUT_QOS_POLICY: {
    ...
    proc_set_task_policy(task, TASK_POLICY_ATTRIBUTE,
                         TASK_BASE_THROUGHPUT_QOS_POLICY, tqos);
  } break;
  ...
  case TASK_SUPPRESSION_POLICY: {
    ...
    kern_return_t kr =
        task_qos_policy_validate(&qosinfo, TASK_QOS_POLICY_COUNT);
    ...
    KERNEL_DEBUG_CONSTANT_IST(
        ...
        proc_selfpid(), task_pid(task), trequested_0(task), trequested_1(task),
        0);
    ...
    task->requested_policy.trp_sup_throughput =
        qos_extract(info->throughput_qos);
    task->requested_policy.trp_sup_cpu = (info->suppressed_cpu) ? 1 : 0;
    task->requested_policy.trp_sup_bg_sockets =
        (info->background_sockets) ? 1 : 0;
...
kern_return_t task_importance(task_t task, integer_t importance) {
  ...
  if (proc_get_effective_task_policy(task, TASK_POLICY_ROLE) >=
...
kern_return_t task_policy_get(task_t task, task_policy_flavor_t flavor,
  ...
  case TASK_CATEGORY_POLICY: {
    ...
    if (*get_default) {
      ...
      info->role =
          proc_get_task_policy(task, TASK_POLICY_ATTRIBUTE, TASK_POLICY_ROLE);
    }
    break;
  }
  ...
  case TASK_OVERRIDE_QOS_POLICY: {
    ...
    if (*get_default) {
      ...
      proc_get_task_policy2(task, TASK_POLICY_ATTRIBUTE,
                            TASK_POLICY_BASE_LATENCY_AND_THROUGHPUT_QOS,
                            &value1, &value2);
  ...
  case TASK_POLICY_STATE: {
    ...
    if (*get_default) {
      ...
      if (task->task_imp_base != NULL) {
        ...
        info->flags |=
            (task_is_marked_importance_receiver(task) ? TASK_IMP_RECEIVER : 0);
        info->flags |= (task_is_marked_importance_denap_receiver(task)
                            ? TASK_DENAP_RECEIVER
                            : 0);
        info->flags |=
            (task_is_marked_importance_donor(task) ? TASK_IMP_DONOR : 0);
        info->flags |=
            (task_is_marked_live_importance_donor(task) ? TASK_IMP_LIVE_DONOR
                                                        : 0);
  ...
  case TASK_SUPPRESSION_POLICY: {
    ...
    if (*get_default) {
      ...
      info->timer_throttle =
          qos_latency_policy_package(task->requested_policy.trp_sup_timer);
      info->disk_throttle = task->requested_policy.trp_sup_disk;
      info->cpu_limit = 0;
      info->suspend = 0;
      info->throughput_qos = qos_throughput_policy_package(
          task->requested_policy.trp_sup_throughput);
 ...
 */
void task_policy_create(task_t task, task_t parent_task) {
  task_set_requested_apptype(task, parent_task->requested_policy.trp_apptype,
                             true);

  task->requested_policy.trp_int_darwinbg =
      parent_task->requested_policy.trp_int_darwinbg;
  task->requested_policy.trp_ext_darwinbg =
      parent_task->requested_policy.trp_ext_darwinbg;
  task->requested_policy.trp_int_iotier =
      parent_task->requested_policy.trp_int_iotier;
  task->requested_policy.trp_ext_iotier =
      parent_task->requested_policy.trp_ext_iotier;
  task->requested_policy.trp_int_iopassive =
      parent_task->requested_policy.trp_int_iopassive;
  task->requested_policy.trp_ext_iopassive =
      parent_task->requested_policy.trp_ext_iopassive;
  task->requested_policy.trp_bg_iotier =
      parent_task->requested_policy.trp_bg_iotier;
  task->requested_policy.trp_terminated =
      parent_task->requested_policy.trp_terminated;
  task->requested_policy.trp_qos_clamp =
      parent_task->requested_policy.trp_qos_clamp;
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE,
      (IMPORTANCE_CODE(IMP_UPDATE,
                       (IMP_UPDATE_TASK_CREATE | TASK_POLICY_TASK))) |
          DBG_FUNC_START,
      task_pid(task), teffective_0(task), teffective_1(task), task->priority,
      0);
...
static void task_policy_update_locked(task_t task,
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                            (IMPORTANCE_CODE(IMP_UPDATE, TASK_POLICY_TASK)) |
                                DBG_FUNC_END,
...
static void task_policy_update_internal_locked(task_t task, bool in_create,
  ...
  bool coalition_is_bg =
      task_get_effective_jetsam_coalition_policy(task, TASK_POLICY_DARWIN_BG);
  ...
  bool wants_all_sockets_bg =
      false;                     /* Do I want my existing sockets to be bg */
  bool wants_watchersbg = false; /* Do I want my pidbound threads to be bg */
  bool bg_clamps_turnstiles =
      false; /* This task does not want turnstile-boost-above-task */

  /*
   * Adaptive daemons are DARWIN_BG unless boosted, and don't get network
   * throttled. Their threads can be turnstile-boosted out of BG.
   */
  ...
  /* Background daemons are always DARWIN_BG, no exceptions, and don't get
   * network throttled. */
  ...
  if (wants_darwinbg) {
    next.tep_darwinbg = 1;
    /* darwinbg tasks always create bg sockets, but we don't always loop over
     * all sockets */
  ...
  if (next.tep_qos_clamp != THREAD_QOS_UNSPECIFIED) {
    latency_qos =
        MAX(latency_qos,
            (int)thread_qos_policy_params.qos_latency_qos[next.tep_qos_clamp]);
  }
  ...
  if (next.tep_qos_clamp != THREAD_QOS_UNSPECIFIED) {
    through_qos =
        MAX(through_qos,
            (int)thread_qos_policy_params.qos_through_qos[next.tep_qos_clamp]);
  }
  ...
  case TASK_APPTYPE_APP_DEFAULT:
    if (requested.trp_ext_darwinbg == 1 || next.tep_coalition_bg ||
        (next.tep_sup_active == 1 &&
  ...
  if (prev.tep_runaway_mitigation != next.tep_runaway_mitigation) {
    KDBG_RELEASE(
        IMPORTANCE_CODE(IMP_RUNAWAY_MITIGATION, 0) |
  ...
  /*
   * Check for the attributes that thread_policy_update_internal_locked()
   * consults, and trigger thread policy re-evaluation.
   */
  ...
  if (prev.tep_role != next.tep_role) {
    if (task_policy_update_coalition_focal_tasks(task, prev.tep_role,
                                                 next.tep_role, pend_token)) {
      update_sfi = true;
    }
  }
  ...
  if (next.tep_lowpri_cpu) {
    ...
    if (task->effective_policy.tep_qos_clamp != THREAD_QOS_UNSPECIFIED) {
      int16_t qos_clamp_priority =
          thread_qos_policy_params
              .qos_pri[task->effective_policy.tep_qos_clamp];
  ...
  if (priority != task->priority || max_priority != task->max_priority) {
  ...
  if (update_threads || update_priority || update_sfi) {
    ...
    queue_iterate(&task->threads, thread, thread_t, task_threads) {
      ...
      if (update_priority || update_threads) {
        ...
        thread_policy_update_tasklocked(thread, task->priority,
                                        task->max_priority, &thread_pend_token);
      }
...
/*
 * Yet another layering violation. We reach out and bang on the coalition
 * directly.
 */
static boolean_t task_policy_update_coalition_focal_tasks(
    task_t task, int prev_role, int next_role, task_pend_token_t pend_token) {
  ...
  if (prev_role != TASK_FOREGROUND_APPLICATION &&
      next_role == TASK_FOREGROUND_APPLICATION) {
    if (task_coalition_adjust_focal_count(task, 1, &new_count) &&
        (new_count == 1)) {
      sfi_transition = TRUE;
      pend_token->tpt_update_tg_ui_flag = TRUE;
    }
  } else if (prev_role == TASK_FOREGROUND_APPLICATION &&
             next_role != TASK_FOREGROUND_APPLICATION) {
    if (task_coalition_adjust_focal_count(task, -1, &new_count) &&
        (new_count == 0)) {
      sfi_transition = TRUE;
  ...
  if (prev_role != TASK_BACKGROUND_APPLICATION &&
      next_role == TASK_BACKGROUND_APPLICATION) {
    if (task_coalition_adjust_nonfocal_count(task, 1, &new_count) &&
        (new_count == 1)) {
      sfi_transition = TRUE;
    }
  } else if (prev_role == TASK_BACKGROUND_APPLICATION &&
             next_role != TASK_BACKGROUND_APPLICATION) {
    if (task_coalition_adjust_nonfocal_count(task, -1, &new_count) &&
        (new_count == 0)) {
      sfi_transition = TRUE;
 ...
 */
void coalition_policy_update_task(task_t task,
                                  coalition_pend_token_t coal_pend_token) {
  /*
...
/*
 * Called with task unlocked to do things that can't be done while holding the
 * task lock
 */
void task_policy_update_complete_unlocked(task_t task,
                                          task_pend_token_t pend_token) {
#ifdef MACH_BSD
  ...
  /* The timer throttle has been removed or reduced, we need to look for expired
   * timers and fire them */
  ...
  if (pend_token->tpt_update_coal_sfi) {
    coalition_for_each_task(
        task->coalition[COALITION_TYPE_RESOURCE], ^bool(task_t each_task) {
          thread_t thread;

          /* skip the task we're re-evaluating on behalf of: it's already
           * updated */
          if (each_task == task) {
            return false;
          }
 ...
 *
 * Everything that modifies requested except functions that need to hold the
 * task lock should use this function
 *
 * Argument validation should be performed before reaching this point.
 *
 * TODO: Do we need to check task->active?
 */
void proc_set_task_policy(task_t task, int category, int flavor, int value) {
  struct task_pend_token pend_token = {};
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      task_pid(task), trequested_0(task), trequested_1(task), value, 0);
...
/*
 * Variant of proc_set_task_policy() that sets two scalars in the requested
 * policy structure. Same locking rules apply.
 */
void proc_set_task_policy2(task_t task, int category, int flavor, int value,
                           int value2) {
  struct task_pend_token pend_token = {};
 ...
 */
int proc_get_task_policy(task_t task, int category, int flavor) {
  int value = 0;
 ...
 */
void proc_get_task_policy2(task_t task, __assert_only int category, int flavor,
                           int *value1, int *value2) {
  task_lock(task);
 ...
 * Gets what is actually in effect, for subsystems which pull policy instead of
 * receive updates.
 *
 ...
 * Notifications of state updates need to be externally synchronized with state
 * queries. This routine *MUST* remain interrupt safe, as it is potentially
 * invoked within the context of a timer interrupt.  It is also called in KDP
 * context for stackshot.
 */
int proc_get_effective_task_policy(task_t task, int flavor) {
  ...
  case TASK_POLICY_ALL_SOCKETS_BG:
    /*
     * do_background_socket() calls this to determine what it should do to the
     * proc's sockets Returns 1 for background mode, 0 for normal mode
     *
     * This consults both thread and task so un-DBGing a thread while the task
     * is BG doesn't get you out of the network throttle.
     */
  ...
  case TASK_POLICY_SFI_MANAGED:
    /*
     * This controls whether or not a process is targeted for specific control
     * by thermald.
     */
  ...
  case TASK_POLICY_TERMINATED:
    /*
     * This controls whether or not a process has its throttling properties shot
  ...
  case TASK_POLICY_RUNAWAY_MITIGATION:
    /*
     * This shows whether or not a process has been tagged for runaway
     * mitigation.
     */
...
void proc_iopol_to_tier(int iopolicy, int *tier, int *passive) {
  *passive = 0;
  *tier = 0;
  switch (iopolicy) {
...
int proc_tier_to_iopol(int tier, int passive) {
  if (passive == 1) {
    switch (tier) {
...
int proc_darwin_role_to_task_role(int darwin_role, task_role_t *task_role) {
...
int proc_task_role_to_darwin_role(task_role_t task_role) {
  switch (task_role) {
  case TASK_FOREGROUND_APPLICATION:
...
/*
 * Called at process exec to initialize the apptype, qos clamp, and qos seed of
 * a process
 *
 * TODO: Make this function more table-driven instead of ad-hoc
 */
void proc_set_task_spawnpolicy(task_t task, thread_t thread, int apptype,
                               int qos_clamp, task_role_t role,
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      task_pid(task), trequested_0(task), trequested_1(task), apptype, 0);
  ...
  case TASK_APPTYPE_APP_DEFAULT:
  case TASK_APPTYPE_APP_NONUI:
    /* Apps become donors via the 'live-donor' flag instead of the static donor
     * flag */
  ...
  case TASK_APPTYPE_DAEMON_INTERACTIVE:
     ...
     */
    task_importance_mark_receiver(
        task, /* FALSE */ ipc_importance_interactive_receiver);
    break;
  ...
  if (portwatch_count && portwatch_ports != NULL) {
    task_add_turnstile_watchports(task, thread, portwatch_ports,
                                  portwatch_count);
  }
  ...
  /* Ensure the donor bit is updated to be in sync with the new live donor
   * status */
  pend_token.tpt_update_live_donor = 1;
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, (IMPORTANCE_CODE(IMP_TASK_APPTYPE, apptype)) | DBG_FUNC_END,
      task_pid(task), trequested_0(task), trequested_1(task),
 ...
 */
void proc_inherit_task_role(task_t new_task, task_t old_task) {
  int role;

  /* inherit the role from old task to new task */
  role =
      proc_get_task_policy(old_task, TASK_POLICY_ATTRIBUTE, TASK_POLICY_ROLE);
...
boolean_t task_is_daemon(task_t task) {
  switch (task->requested_policy.trp_apptype) {
  case TASK_APPTYPE_DAEMON_INTERACTIVE:
  case TASK_APPTYPE_DAEMON_STANDARD:
  case TASK_APPTYPE_DAEMON_ADAPTIVE:
...
/* for telemetry */
integer_t task_grab_latency_qos(task_t task) {
  return qos_latency_policy_package(
      proc_get_effective_task_policy(task, TASK_POLICY_LATENCY_QOS));
}

/* update the darwin background action state in the flags field for libproc */
int proc_get_darwinbgstate(task_t task, uint32_t *flagsp) {
...
static uintptr_t trequested_0(task_t task) {
  static_assert(sizeof(struct task_requested_policy) == sizeof(uint64_t),
...
static uintptr_t teffective_0(task_t task) {
  static_assert(sizeof(struct task_effective_policy) == sizeof(uint64_t),
                "size invariant violated");
...
uint32_t tpending(task_pend_token_t pend_token) {
  return *(uint32_t *)(void *)(pend_token);
}

uint64_t task_requested_bitfield(task_t task) {
  uint64_t bits = 0;
  struct task_requested_policy requested = task->requested_policy;

  bits |= (requested.trp_int_darwinbg ? POLICY_REQ_INT_DARWIN_BG : 0);
  bits |= (requested.trp_ext_darwinbg ? POLICY_REQ_EXT_DARWIN_BG : 0);
  bits |= (requested.trp_int_iotier ? (((uint64_t)requested.trp_int_iotier)
                                       << POLICY_REQ_INT_IO_TIER_SHIFT)
                                    : 0);
  bits |= (requested.trp_ext_iotier ? (((uint64_t)requested.trp_ext_iotier)
  ...
  bits |= (requested.trp_bg_iotier ? (((uint64_t)requested.trp_bg_iotier)
  ...
  bits |= (requested.trp_apptype
               ? (((uint64_t)requested.trp_apptype) << POLICY_REQ_APPTYPE_SHIFT)
               : 0);
  bits |= (requested.trp_role
               ? (((uint64_t)requested.trp_role) << POLICY_REQ_ROLE_SHIFT)
               : 0);
  ...
  bits |= (requested.trp_sup_timer ? (((uint64_t)requested.trp_sup_timer)
                                      << POLICY_REQ_SUP_TIMER_THROTTLE_SHIFT)
                                   : 0);
  bits |=
      (requested.trp_sup_throughput ? (((uint64_t)requested.trp_sup_throughput)
  ...
  bits |= (requested.trp_base_latency_qos
               ? (((uint64_t)requested.trp_base_latency_qos)
                  << POLICY_REQ_BASE_LATENCY_QOS_SHIFT)
               : 0);
  bits |= (requested.trp_over_latency_qos
               ? (((uint64_t)requested.trp_over_latency_qos)
                  << POLICY_REQ_OVER_LATENCY_QOS_SHIFT)
               : 0);
  bits |= (requested.trp_base_through_qos
               ? (((uint64_t)requested.trp_base_through_qos)
                  << POLICY_REQ_BASE_THROUGH_QOS_SHIFT)
               : 0);
  bits |= (requested.trp_over_through_qos
               ? (((uint64_t)requested.trp_over_through_qos)
                  << POLICY_REQ_OVER_THROUGH_QOS_SHIFT)
               : 0);
  bits |= (requested.trp_sfi_managed ? POLICY_REQ_SFI_MANAGED : 0);
  bits |= (requested.trp_qos_clamp ? (((uint64_t)requested.trp_qos_clamp)
...
uint64_t task_effective_bitfield(task_t task) {
  uint64_t bits = 0;
  struct task_effective_policy effective = task->effective_policy;

  bits |= (effective.tep_io_tier
               ? (((uint64_t)effective.tep_io_tier) << POLICY_EFF_IO_TIER_SHIFT)
               : 0);
  ...
  bits |= (effective.tep_bg_iotier ? (((uint64_t)effective.tep_bg_iotier)
  ...
  bits |= (effective.tep_role
               ? (((uint64_t)effective.tep_role) << POLICY_EFF_ROLE_SHIFT)
               : 0);
  bits |= (effective.tep_latency_qos ? (((uint64_t)effective.tep_latency_qos)
                                        << POLICY_EFF_LATENCY_QOS_SHIFT)
                                     : 0);
  bits |= (effective.tep_through_qos ? (((uint64_t)effective.tep_through_qos)
                                        << POLICY_EFF_THROUGH_QOS_SHIFT)
                                     : 0);
  bits |= (effective.tep_sfi_managed ? POLICY_EFF_SFI_MANAGED : 0);
  bits |= (effective.tep_qos_ceiling ? (((uint64_t)effective.tep_qos_ceiling)
...
int proc_get_task_ruse_cpu(task_t task, uint32_t *policyp, uint8_t *percentagep,
                           uint64_t *intervalp, uint64_t *deadlinep) {
  int error = 0;
  int scope;
  ...
  /*
   * Reverse-map from CPU resource limit scopes back to policies (see comment
   * below).
   */
 ...
 * process consumes more CPU than this, an EXC_RESOURCE exception will be
 * generated.
 */
 ...
 *
 * Policy				| Deadline-based CPU limit |
 * Percentage-based CPU limit
 * -------------------------------------+--------------------------+------------------------------
 * PROC_POLICY_RSRCACT_THROTTLE		| ENOTSUP		   | Task-wide
 * scope only PROC_POLICY_RSRCACT_SUSPEND		| Task-wide scope only
 * | ENOTSUP PROC_POLICY_RSRCACT_TERMINATE	| Task-wide scope only	   |
 * ENOTSUP PROC_POLICY_RSRCACT_NOTIFY_KQ	| Task-wide scope only	   |
 * ENOTSUP PROC_POLICY_RSRCACT_NOTIFY_EXC	| ENOTSUP		   |
 * Per-thread scope only
 *
 * A deadline-based CPU limit is actually a simple wallclock timer - the
 * requested action is performed after the specified amount of wallclock time
 * has elapsed.
 *
 * A percentage-based CPU limit performs the requested action after the
 * specified amount of actual CPU time has been consumed -- regardless of how
 * much wallclock time has elapsed -- by either the task as an aggregate entity
 * (so-called "Task-wide" or "Proc-wide" scope, whereby the CPU time consumed by
 * all threads in the task are added together), or by any one thread in the task
 * (so-called "per-thread" scope).
 *
 * We support either deadline != 0 OR percentage != 0, but not both. The
 * original intention in having them share an API was to use actual CPU time as
 * the basis of the deadline-based limit (as in: perform an action after I have
 * used some amount of CPU time; this is different than the recurring
 * percentage/interval model) but the potential consumer of the API at the time
 * was insisting on wallclock time instead.
 *
 * Currently, requesting notification via an exception is the only way to get
 * per-thread scope for a CPU limit. All other types of notifications force
 * task-wide scope for the limit.
 */
int proc_set_task_ruse_cpu(task_t task, uint16_t policy, uint8_t percentage,
                           uint64_t interval, uint64_t deadline,
                           int cpumon_entitled) {
  int error = 0;
  int scope;

  /*
   * Enforce the matrix of supported configurations for policy, percentage, and
   * deadline.
   */
  switch (policy) {
...
int proc_clear_task_ruse_cpu(task_t task, int cpumon_entitled) {
  ...
  if (action != TASK_POLICY_RESOURCE_ATTRIBUTE_NONE) {
    ...
    task_unlock(task);
    proc_restore_resource_actions(bsdinfo, TASK_POLICY_CPU_RESOURCE_USAGE,
                                  action);
    goto out1;
  }
...
static int task_apply_resource_actions(task_t task, int type) {
  ...
  if (action != TASK_POLICY_RESOURCE_ATTRIBUTE_NONE) {
    bsdinfo = get_bsdtask_info(task);
    task_unlock(task);
    proc_apply_resource_actions(bsdinfo, TASK_POLICY_CPU_RESOURCE_USAGE,
                                action);
  } else {
    task_unlock(task);
  }
...
/*
 * XXX This API is somewhat broken; we support multiple simultaneous CPU limits,
 * but the get/set API only allows for one at a time. This means that if there
 * is a per-thread limit active, the other "scopes" will not be accessible via
 * this API. We could change it to pass in the scope of interest to the caller,
 * and prefer that, but there's no need for that at the moment.
 */
static int task_get_cpuusage(task_t task, uint8_t *percentagep,
                             uint64_t *intervalp, uint64_t *deadlinep,
                             int *scope) {
 ...
 */
int task_suspend_cpumon(task_t task) {
  thread_t thread;
   ...
   */
  task->rusage_cpu_flags &=
      ~(TASK_RUSECPU_FLAGS_PERTHR_LIMIT | TASK_RUSECPU_FLAGS_FATAL_CPUMON);
  queue_iterate(&task->threads, thread, thread_t, task_threads) {
...
int task_resume_cpumon(task_t task) {
  kern_return_t kret;
...
static int task_set_cpuusage(task_t task, uint8_t percentage, uint64_t interval,
  ...
  if (percentage != 0) {
    if (scope == TASK_RUSECPU_FLAGS_PERTHR_LIMIT) {
      ...
      ledger_set_action(task->ledger, task_ledgers.cpu_time,
                        LEDGER_ACTION_BLOCK);
    }
  }
  ...
  if (deadline != 0) {
    ...
    if (task->rusage_cpu_callt == NULL) {
      task->rusage_cpu_callt = thread_call_allocate_with_priority(
          task_action_cpuusage, (thread_call_param_t)task,
...
int task_clear_cpuusage(task_t task, int cpumon_entitled) {
  int retval = 0;
...
/* called by ledger unit to enforce action due to resource usage criteria being
 * met */
static void task_action_cpuusage(thread_call_param_t param0,
...
static void task_watch_lock(void) { lck_mtx_lock(&task_watch_mtx); }

static void task_watch_unlock(void) { lck_mtx_unlock(&task_watch_mtx); }
...
int proc_lf_pidbind(task_t curtask, uint64_t tid, task_t target_task,
                    int bind) {
  ...
  if (bind != 0) {
    ...
    setbg =
        proc_get_effective_task_policy(target_task, TASK_POLICY_WATCHERS_BG);
...
out:
  thread_deallocate(
      target_thread); /* drop thread ref acquired in this routine */
  return ret;
}

static void set_thread_appbg(thread_t thread, int setbg,
                             __unused int importance) {
  int enable = (setbg ? TASK_POLICY_ENABLE : TASK_POLICY_DISABLE);

  proc_set_thread_policy(thread, TASK_POLICY_ATTRIBUTE, TASK_POLICY_PIDBIND_BG,
                         enable);
}
...
retry:
  ...
  if (task->watchapplying != 0) {
    lck_mtx_sleep(&task_watch_mtx, LCK_SLEEP_DEFAULT, &task->watchapplying,
                  THREAD_UNINT);
    task_watch_unlock();
...
void thead_remove_taskwatch(thread_t thread) {
  task_watch_t *twp;
  int importance = 0;
...
void task_removewatchers(task_t task) {
  queue_head_t queue;
  task_watch_t *twp;
  ...
  queue_iterate(&queue, twp, task_watch_t *, tw_links) {
     ...
     */
    twp->tw_thread->taskwatch =
        NULL; /* removed linkage, clear thread holding ref */
  }
...
void task_importance_mark_live_donor(task_t task, boolean_t live_donating) {
#if IMPORTANCE_INHERITANCE
  ipc_importance_task_t task_imp;
...
void task_importance_mark_denap_receiver(task_t task, boolean_t denap) {
#if IMPORTANCE_INHERITANCE
  ipc_importance_task_t task_imp;
...
void task_importance_reset(__imp_only task_t task) {
#if IMPORTANCE_INHERITANCE
  ipc_importance_task_t task_imp;
...
void task_importance_init_from_parent(__imp_only task_t new_task,
                                      __imp_only task_t parent_task) {
#if IMPORTANCE_INHERITANCE
...
#if IMPORTANCE_INHERITANCE
/*
 * Sets the task boost bit to the provided value.  Does NOT run the update
 * function.
 *
...
static void task_set_boost_locked(task_t task, boolean_t boost_active) {
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      (IMPORTANCE_CODE(IMP_BOOST,
                       (boost_active ? IMP_BOOSTED : IMP_UNBOOSTED)) |
       DBG_FUNC_START),
      proc_selfpid(), task_pid(task), trequested_0(task), trequested_1(task),
      0);
#endif /* IMPORTANCE_TRACE */
 ...
 *
 * May be called without taking the task lock. In that case, donor status can
 * change so you must check only once for each donation event.
 */
boolean_t task_is_importance_donor(task_t task) {
 ...
 */
boolean_t task_is_marked_importance_donor(task_t task) {
  if (task->task_imp_base == IIT_NULL) {
    return FALSE;
  }
 ...
 */
boolean_t task_is_marked_live_importance_donor(task_t task) {
  if (task->task_imp_base == IIT_NULL) {
    return FALSE;
  }
 ...
 */
boolean_t task_is_importance_receiver(task_t task) {
  if (task->task_imp_base == IIT_NULL) {
    return FALSE;
  }
 ...
 */
boolean_t task_is_marked_importance_receiver(task_t task) {
  if (task->task_imp_base == IIT_NULL) {
    return FALSE;
  }
  return ipc_importance_task_is_marked_receiver(task->task_imp_base);
 ...
 */
boolean_t task_is_importance_denap_receiver(task_t task) {
  if (task->task_imp_base == IIT_NULL) {
    return FALSE;
  }
 ...
 */
boolean_t task_is_marked_importance_denap_receiver(task_t task) {
  if (task->task_imp_base == IIT_NULL) {
    return FALSE;
  }
 ...
 * since the value of imp_receiver can never be unset.
 */
boolean_t task_is_importance_receiver_type(task_t task) {
  if (task->task_imp_base == IIT_NULL) {
    return FALSE;
  }
 ...
 * Assertions are changed from internal to external via
 * task_importance_externalize_assertion
 */

int task_importance_hold_internal_assertion(task_t target_task,
                                            uint32_t count) {
  ipc_importance_task_t task_imp;
  kern_return_t ret;
...
int task_importance_hold_file_lock_assertion(task_t target_task,
                                             uint32_t count) {
  ipc_importance_task_t task_imp;
  kern_return_t ret;
...
int task_importance_hold_legacy_external_assertion(task_t target_task,
                                                   uint32_t count) {
  ipc_importance_task_t task_imp;
  kern_return_t ret;
...
int task_importance_drop_file_lock_assertion(task_t target_task,
                                             uint32_t count) {
  ipc_importance_task_t task_imp;
  kern_return_t ret;
  ...
  ret = ipc_importance_task_drop_file_lock_assertion(target_task->task_imp_base,
                                                     count);
  return (KERN_SUCCESS != ret) ? EOVERFLOW : 0;
}

int task_importance_drop_legacy_external_assertion(task_t target_task,
                                                   uint32_t count) {
  ipc_importance_task_t task_imp;
  kern_return_t ret;
...
static void task_add_importance_watchport(task_t task, mach_port_t port,
  ...
  if (IP_VALID(port) != 0) {
    ...
    if (port->ip_tempowner != 0) {
      ...
      /* mark the port is watching another task (reference held in
       * port->ip_imp_task) */
    ...
    if (IIT_NULL != release_imp_task) {
      ...
      // released_pid = task_pid(release_imp_task); /* TODO: Need ref-safe way
      // to get pid */
      ipc_importance_task_release(release_imp_task);
    }
#if IMPORTANCE_TRACE
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE, (IMPORTANCE_CODE(IMP_WATCHPORT, 0)) | DBG_FUNC_NONE,
 ...
 */
int task_low_mem_privileged_listener(task_t task, boolean_t new_value,
                                     boolean_t *old_value) {
  if (old_value != NULL) {
 ...
 */
boolean_t task_has_been_notified(task_t task, int pressurelevel) {
  if (task == NULL) {
    return FALSE;
  }
 ...
 * Condition: task lock should be held while calling this function.
 */
boolean_t task_used_for_purging(task_t task, int pressurelevel) {
  if (task == NULL) {
    return FALSE;
  }
 ...
 * Condition: task lock should be held while calling this function.
 */
void task_mark_used_for_purging(task_t task, int pressurelevel) {
  if (task == NULL) {
    return;
  }
 ...
 * Condition: task lock should be held while calling this function.
 */
void task_clear_has_been_notified(task_t task, int pressurelevel) {
  if (task == NULL) {
    return;
  }
 ...
 * Condition: task lock should be held while calling this function.
 */
void task_clear_used_for_purging(task_t task) {
  if (task == NULL) {
    return;
  }
 ...
 */
int task_importance_estimate(task_t task) {
...
boolean_t task_has_assertions(task_t task) {
  return task->task_imp_base->iit_assertcnt ? TRUE : FALSE;
}

kern_return_t send_resource_violation(typeof(send_cpu_usage_violation) sendfunc,
                                      task_t violator,
  ...
  if (flags & kRNFatalLimitFlag) {
    kr = proc_pidpathinfo_internal(proc, 0, proc_path, sizeof(proc_path), NULL);
  }
  task_unlock(violator);
  if (kr) {
    goto finish;
  ...
  kr = sendfunc(dstport, procname, pid, proc_path, timestamp,
                linfo->lei_balance, linfo->lei_last_refill, linfo->lei_limit,
                linfo->lei_refill_period, flags);
...
kern_return_t send_resource_violation_with_fatal_port(
    typeof(send_port_space_violation) sendfunc, task_t violator,
    int64_t current_size, int64_t limit, mach_port_t fatal_port,
    resource_notify_flags_t flags) {
#ifndef MACH_BSD
  ...
  /* send message */
  kr = task_get_special_port(current_task(), TASK_RESOURCE_NOTIFY_PORT,
                             &dstport);
  ...
  thread_set_honor_qlimit(curthread);
  kr = sendfunc(dstport, procname, pid, timestamp, current_size, limit,
                fatal_port, flags);
  thread_clear_honor_qlimit(curthread);
 ...
 */
#ifdef __LP64__
void trace_resource_violation(uint16_t code, struct ledger_entry_info *linfo) {
...
void trace_resource_violation(uint16_t code, struct ledger_entry_info *linfo) {
  ...
  KERNEL_DBG_IST_SANE(KDBG_CODE(DBG_MACH, DBG_MACH_RESOURCE, codeA),
                      balance_high, balance_low, last_refill_high,
                      last_refill_low);
  KERNEL_DBG_IST_SANE(KDBG_CODE(DBG_MACH, DBG_MACH_RESOURCE, codeB), limit_high,
                      limit_low, refill_period_high, refill_period_low);
}
#endif /* K64/K32 */
