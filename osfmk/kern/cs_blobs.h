...
#define CS_EXEC_SET_ENFORCEMENT                                                \
  0x00400000 /* set CS_ENFORCEMENT on any exec'ed process */
#define CS_EXEC_INHERIT_SIP                                                    \
  0x00800000 /* set CS_INSTALLER on any exec'ed process */

#define CS_KILLED 0x01000000 /* was killed by kernel for invalidity */
#define CS_NO_UNTRUSTED_HELPERS                                                \
  0x02000000 /* kernel did not load a non-platform-binary dyld or Rosetta      \
                runtime */
...
#define CS_DEBUGGED                                                            \
  0x10000000 /* process is currently or has previously been debugged and       \
                allowed to run with invalid pages */
#define CS_SIGNED                                                              \
...
#define CS_DATAVAULT_CONTROLLER                                                \
...
#define CS_EXECSEG_MAIN_BINARY 0x1 /* executable segment denotes main binary   \
                                    */
#define CS_EXECSEG_ALLOW_UNSIGNED                                              \
  0x10                           /* allow unsigned pages (for debugging) */
...
} CS_CodeDirectory __attribute__((aligned(1)));
...
} CS_BlobIndex __attribute__((aligned(1)));
...
typedef struct __SC_SuperBlob {
   ...
   */
} CS_SuperBlob __attribute__((aligned(1)));
...
} SC_Scatter __attribute__((aligned(1)));
...
/*
 * Ideally, this definition should live within sys/codesign.h, but adding it
 * there causes an issue with compiling certain Swift projects due to some
 * ambigious macro definition error on CD_CDHASH_LEN. Thus, we keep it here.
 *
