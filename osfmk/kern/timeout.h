...
extern void kern_timeout_try_panic(kern_timeout_type_t type, uint64_t payload,
                                   kern_timeout_t *to, const char *prefix,
                                   uint64_t threshold);

#if XNU_KERNEL_PRIVATE
extern void kern_timeout_cycles_instrs(kern_timeout_t *to, uint64_t *cycles,
                                       uint64_t *instrs);
extern void kern_timeout_cpi(kern_timeout_t *to, uint64_t *cpi_whole,
                             uint64_t *cpi_fractional);
#endif /* XNU_KERNEL_PRIVATE */

static inline void kern_timeout_stretch(kern_timeout_t *to, uint64_t mt_ticks) {
  to->start_mt -= mt_ticks;
}

static inline uint64_t kern_timeout_start_time(kern_timeout_t *to) {
  return to->start_mt;
}

/*
 * Return the mach time elapsed beteween calls to kern_timeout_start() and
 * kern_timeout_end().
 */
static inline uint64_t kern_timeout_gross_duration(kern_timeout_t *to) {
...
/*
 * Return the mach time elapsed beteween calls to kern_timeout_start() and
 * kern_timeout_end() subtracting the mach time that elapsed handling
 * interrupts.
 */
static inline uint64_t kern_timeout_net_duration(kern_timeout_t *to) {
...
static inline void kern_timeout_mach_times(kern_timeout_t *to,
                                           uint64_t *start_mt,
                                           uint64_t *end_mt) {
