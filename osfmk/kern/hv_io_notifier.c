 ...
 */
static hv_ion_entry_t *hv_io_notifier_grp_lookup(const hv_ion_grp_t *grp,
                                                 const hv_ion_entry_t *key) {
 ...
 */
kern_return_t hv_io_notifier_grp_add(hv_ion_grp_t *grp,
                                     const hv_ion_t *notifier) {
  hv_ion_entry_t *ion = NULL;
  ...
  kern_return_t ret = ipc_typed_port_copyin_send(
      current_task()->itk_space, ion->port_name, IOT_ANY, &ion->port);
 ...
 */
kern_return_t hv_io_notifier_grp_remove(hv_ion_grp_t *grp,
                                        const hv_ion_t *notifier) {
  hv_ion_entry_t ion = {};
 ...
 */
kern_return_t hv_io_notifier_grp_fire(hv_ion_grp_t *grp, uint64_t addr,
  ...
  LIST_FOREACH(ion, &grp->list, list) {
    ...
    kr = mach_msg_send_from_kernel_with_options(
        &msg.header, sizeof(msg), MACH_SEND_TIMEOUT, MACH_MSG_TIMEOUT_NONE);
...
kern_return_t hv_io_notifier_grp_alloc(hv_ion_grp_t **grp_p) {
...
void hv_io_notifier_grp_free(hv_ion_grp_t **grp_p) {
