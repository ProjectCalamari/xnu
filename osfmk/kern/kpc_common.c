...
#define COUNTERBUF_SIZE (machine_info.logical_cpu_max * COUNTERBUF_SIZE_PER_CPU)
...
static uint64_t *kpc_percpu_alloc(void) {
  return kalloc_data_tag(COUNTERBUF_SIZE_PER_CPU, Z_WAITOK | Z_ZERO,
                         VM_KERN_MEMORY_DIAG);
}

static void kpc_percpu_free(uint64_t *buf) {
...
static void kpc_task_set_forced_all_ctrs(task_t task, boolean_t state) {
  assert(task);
...
boolean_t kpc_controls_counter(uint32_t ctr) {
  ...
  if ((pmc_mask & kpc_pm_pmc_mask) && kpc_pm_has_custom_config &&
      !force_all_ctrs) {
    return FALSE;
  }
...
int kpc_get_cpu_counters(boolean_t all_cpus, uint32_t classes, int *curcpu,
                         uint64_t *buf) {
  assert(buf);
...
int kpc_get_shadow_counters(boolean_t all_cpus, uint32_t classes, int *curcpu,
                            uint64_t *buf) {
...
static int _kpc_set_config_internal(uint32_t classes, kpc_config_t *configv,
                                    bool secure) {
  int ret = 0;
...
int kpc_set_config_kernel(uint32_t classes, kpc_config_t *configv) {
...
uint32_t kpc_get_counterbuf_size(void) { return COUNTERBUF_SIZE; }
...
void kpc_counterbuf_free(uint64_t *buf) { kfree_data(buf, COUNTERBUF_SIZE); }
...
boolean_t kpc_register_pm_handler(kpc_pm_handler_t handler) {
  return kpc_reserve_pm_counters(0x38, handler, TRUE);
}

boolean_t kpc_reserve_pm_counters(uint64_t pmc_mask, kpc_pm_handler_t handler,
...
void kpc_release_pm_counters(void) {
  ...
  /* post-condition */
  assert(kpc_get_counter_count(KPC_CLASS_CONFIGURABLE_MASK) ==
...
uint64_t kpc_get_configurable_pmc_mask(uint32_t classes) {
  ...
  if ((force_all_ctrs == FALSE) && (kpc_pm_handler != NULL) &&
      (kpc_pm_has_custom_config == FALSE) && (classes & KPC_CLASS_POWER_MASK)) {
...
void kpc_pm_acknowledge(boolean_t __unused available_to_pm) {}

boolean_t kpc_register_pm_handler(kpc_pm_handler_t __unused handler) {
  return FALSE;
}

boolean_t kpc_reserve_pm_counters(uint64_t __unused pmc_mask,
...
int kpc_get_force_all_ctrs(void) { return 0; }

int kpc_get_cpu_counters(boolean_t __unused all_cpus, uint32_t __unused classes,
                         int *__unused curcpu, uint64_t *__unused buf) {
  return ENOTSUP;
}

int kpc_get_shadow_counters(boolean_t __unused all_cpus,
                            uint32_t __unused classes, int *__unused curcpu,
                            uint64_t *__unused buf) {
  return ENOTSUP;
}

uint32_t kpc_get_running(void) { return 0; }

int kpc_set_running(uint32_t __unused classes) { return ENOTSUP; }

int kpc_get_config(uint32_t __unused classes,
                   kpc_config_t *__unused current_config) {
  return ENOTSUP;
}

int kpc_set_config_external(uint32_t classes, kpc_config_t *configv);
int kpc_set_config_external(uint32_t __unused classes,
