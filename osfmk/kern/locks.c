...
#define ordered_load_hw(lock)                                                  \
  os_atomic_load(&(lock)->lock_data, compiler_acq_rel)
#define ordered_store_hw(lock, value)                                          \
  os_atomic_store(&(lock)->lock_data, (value), compiler_acq_rel)
...
MACHINE_TIMEOUT(dtrace_spin_threshold, "dtrace-spin-threshold",
                0xC /* 12 ticks == 500ns with 24MHz OSC */,
                MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
#endif
#endif

struct lck_mcs PERCPU_DATA(lck_mcs);

__kdebug_only uintptr_t unslide_for_kdebug(const void *object) {
...
static __abortlike void __lck_require_preemption_disabled_panic(void *lock) {
  panic("Attempt to take no-preempt lock %p in preemptible context", lock);
}

static inline void __lck_require_preemption_disabled(void *lock,
                                                     thread_t self __unused) {
...
/*
 * Input and output timeouts are expressed in absolute_time for arm and TSC for
 * Intel
 */
__attribute__((always_inline)) hw_spin_timeout_t
...
#if SCHED_HYGIENE_DEBUG
#ifndef KASAN
  if (ret.hwst_timeout > 0 && !ret.hwst_in_ppl && !ret.hwst_interruptible &&
...
__attribute__((always_inline)) bool hw_spin_in_ppl(hw_spin_timeout_t to) {
#if SCHED_HYGIENE_DEBUG
...
bool hw_spin_should_keep_spinning(void *lock, hw_spin_policy_t pol,
  ...
  rc = pol->hwsp_op_timeout((char *)lock - pol->hwsp_lock_offset, to, *state);
 ...
 */
MARK_AS_HIBERNATE_TEXT void hw_lock_init(hw_lock_t lock) {
  ordered_store_hw(lock, 0);
}

__result_use_check static inline bool
...
hw_spin_timeout_panic(void *_lock, hw_spin_timeout_t to, hw_spin_state_t st) {
  ...
  if (pmap_in_ppl()) {
    ...
    panic("Spinlock[%p] " HW_SPIN_TIMEOUT_FMT "; "
          ...
          lock, HW_SPIN_TIMEOUT_ARG(to, st), (void *)owner,
  ...
  panic("Spinlock[%p] " HW_SPIN_TIMEOUT_FMT "; "
        ...
        lock, HW_SPIN_TIMEOUT_ARG(to, st), (void *)lsti->owner_thread_cur,
        lsti->owner_cpu,
#if DEBUG || DEVELOPMENT
...
__startup_func static void hw_lock_test_to_init(void) {
  uint64_t timeout;
...
static hw_spin_timeout_status_t hw_lock_bit_timeout_panic(void *_lock,
                                                          hw_spin_timeout_t to,
                                                          hw_spin_state_t st) {
  ...
  panic("Spinlock[%p] " HW_SPIN_TIMEOUT_FMT "; "
        ...
        lock, HW_SPIN_TIMEOUT_ARG(to, st), *lock,
        HW_SPIN_TIMEOUT_DETAILS_ARG(to, st));
}
...
hw_lock_lock_contended(hw_lock_t lock, uintptr_t data,
                       hw_spin_policy_t pol LCK_GRP_ARG(lck_grp_t *grp)) {
...
static hw_spin_timeout_status_t
hw_wait_while_equals32_panic(void *_lock, hw_spin_timeout_t to,
                             hw_spin_state_t st) {
  ...
  panic("wait_while_equals32[%p] " HW_SPIN_TIMEOUT_FMT "; "
        "current value: 0x%08x, " HW_SPIN_TIMEOUT_DETAILS_FMT,
        address, HW_SPIN_TIMEOUT_ARG(to, st), *address,
        HW_SPIN_TIMEOUT_DETAILS_ARG(to, st));
}
...
static hw_spin_timeout_status_t
hw_wait_while_equals64_panic(void *_lock, hw_spin_timeout_t to,
                             hw_spin_state_t st) {
...
__result_use_check static inline hw_lock_status_t
hw_lock_to_internal(hw_lock_t lock, thread_t thread,
                    hw_spin_policy_t pol LCK_GRP_ARG(lck_grp_t *grp)) {
 ...
 */
void(hw_lock_lock)(hw_lock_t lock LCK_GRP_ARG(lck_grp_t *grp)) {
  thread_t thread = current_thread();
  lock_disable_preemption_for_thread(thread);
  (void)hw_lock_to_internal(lock, thread,
                            &hw_lock_spin_policy LCK_GRP_ARG(grp));
}
 ...
 *	Acquire lock, spinning until it becomes available.
 */
void(hw_lock_lock_nopreempt)(hw_lock_t lock LCK_GRP_ARG(lck_grp_t *grp)) {
  thread_t thread = current_thread();
 ...
 *	preemption disabled.
 */
unsigned int(hw_lock_to)(hw_lock_t lock,
                         hw_spin_policy_t pol LCK_GRP_ARG(lck_grp_t *grp)) {
  thread_t thread = current_thread();
  lock_disable_preemption_for_thread(thread);
 ...
 *	preemption disabled.
 */
unsigned int(hw_lock_to_nopreempt)(
    hw_lock_t lock, hw_spin_policy_t pol LCK_GRP_ARG(lck_grp_t *grp)) {
  thread_t thread = current_thread();
  __lck_require_preemption_disabled(lock, thread);
  return (unsigned)hw_lock_to_internal(lock, thread, pol LCK_GRP_ARG(grp));
}

__result_use_check static inline unsigned int
hw_lock_try_internal(hw_lock_t lock,
                     thread_t thread LCK_GRP_ARG(lck_grp_t *grp)) {
 ...
 */
unsigned int(hw_lock_try)(hw_lock_t lock LCK_GRP_ARG(lck_grp_t *grp)) {
  thread_t thread = current_thread();
  lock_disable_preemption_for_thread(thread);
...
unsigned int(hw_lock_try_nopreempt)(
    hw_lock_t lock LCK_GRP_ARG(lck_grp_t *grp)) {
  thread_t thread = current_thread();
  __lck_require_preemption_disabled(lock, thread);
...
#if DEBUG || DEVELOPMENT
__abortlike static void __hw_lock_unlock_unowned_panic(hw_lock_t lock) {
  panic("hwlock: thread %p is trying to lock %p recursively", current_thread(),
        lock);
}
#endif /* DEBUG || DEVELOPMENT */
 ...
 */
static inline void hw_lock_unlock_internal(hw_lock_t lock) {
#if DEBUG || DEVELOPMENT
...
void(hw_lock_unlock)(hw_lock_t lock) {
  hw_lock_unlock_internal(lock);
  lock_enable_preemption();
}

void(hw_lock_unlock_nopreempt)(hw_lock_t lock) {
  hw_lock_unlock_internal(lock);
}

void hw_lock_assert(__assert_only hw_lock_t lock,
                    __assert_only unsigned int type) {
#if MACH_ASSERT
 ...
 *	N.B.  Racy, of course.
 */
unsigned int hw_lock_held(hw_lock_t lock) { return ordered_load_hw(lock) != 0; }

static hw_lock_status_t NOINLINE hw_lock_bit_to_contended(
    hw_lock_bit_t *lock, uint32_t bit, hw_spin_policy_t pol,
    bool (^lock_pause)(void) LCK_GRP_ARG(lck_grp_t *grp)) {
  hw_spin_timeout_t to = hw_spin_compute_timeout(pol);
  hw_spin_state_t state = {};
  hw_lock_status_t rc = HW_LOCK_CONTENDED;
  ...
  do {
    for (int i = 0; i < LOCK_SNOOP_SPINS; i++) {
      rc = (hw_lock_trylock_bit(lock, bit, true) ? HW_LOCK_ACQUIRED
                                                 : HW_LOCK_CONTENDED);
...
__result_use_check static inline hw_lock_status_t
hw_lock_bit_to_internal(hw_lock_bit_t *lock, unsigned int bit,
 ...
 *	preemption disabled.
 */
unsigned int(hw_lock_bit_to)(hw_lock_bit_t *lock, uint32_t bit,
                             hw_spin_policy_t pol LCK_GRP_ARG(lck_grp_t *grp)) {
  _disable_preemption();
  return (unsigned int)hw_lock_bit_to_internal(lock, bit, pol,
                                               NULL LCK_GRP_ARG(grp));
}
 ...
 *	return with preemption disabled.
 */
void(hw_lock_bit)(hw_lock_bit_t *lock,
                  unsigned int bit LCK_GRP_ARG(lck_grp_t *grp)) {
  _disable_preemption();
  (void)hw_lock_bit_to_internal(lock, bit, &hw_lock_bit_policy,
                                NULL LCK_GRP_ARG(grp));
}
 ...
 *
 *	Acquire bit lock with preemption already disabled, spinning until it
 * becomes available.
 */
void(hw_lock_bit_nopreempt)(hw_lock_bit_t *lock,
                            unsigned int bit LCK_GRP_ARG(lck_grp_t *grp)) {
 ...
 */
hw_lock_status_t(hw_lock_bit_to_b)(hw_lock_bit_t *lock, uint32_t bit,
                                   hw_spin_policy_t pol,
                                   bool (^lock_pause)(void)
                                       LCK_GRP_ARG(lck_grp_t *grp)) {
  _disable_preemption();
  hw_lock_status_t ret =
      hw_lock_bit_to_internal(lock, bit, pol, lock_pause LCK_GRP_ARG(grp));
...
bool(hw_lock_bit_try)(hw_lock_bit_t *lock,
                      unsigned int bit LCK_GRP_ARG(lck_grp_t *grp)) {
  bool success = false;
...
static inline void hw_unlock_bit_internal(hw_lock_bit_t *lock,
                                          unsigned int bit) {
 ...
 *
 *		Release spin-lock. The second parameter is the bit number to
 * test and set. Decrement the preemption level.
 */
...
void hw_unlock_bit_nopreempt(hw_lock_bit_t *lock, unsigned int bit) {
  __lck_require_preemption_disabled(lock, current_thread());
  hw_unlock_bit_internal(lock, bit);
}
...
wait_result_t lck_mtx_sleep(lck_mtx_t *lck, lck_sleep_action_t lck_sleep_action,
  ...
  KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_MTX_SLEEP_CODE) |
                   DBG_FUNC_START,
 ...
 *
 * Functions that allow to sleep on an event and use turnstile to propagate the
 * priority of the sleeping threads to the latest thread specified as inheritor.
 *
 * The inheritor management is delegated to the caller, the caller needs to
 * store a thread identifier to provide to this functions to specified upon whom
 * direct the push. The inheritor cannot return to user space or exit while
 * holding a push from an event. Therefore is the caller responsibility to call
 * a wakeup_with_inheritor from inheritor before running in userspace or specify
 * another inheritor before letting the old inheritor run in userspace.
 *
 * sleep_with_inheritor requires to hold a locking primitive while invoked, but
 * wakeup_with_inheritor and change_sleep_inheritor don't require it.
 *
 * Turnstile requires a non blocking primitive as interlock to synchronize the
 * turnstile data structure manipulation, threfore sleep_with_inheritor,
 * change_sleep_inheritor and wakeup_with_inheritor will require the same
 * interlock to manipulate turnstiles. If sleep_with_inheritor is associated
 * with a locking primitive that can block (like lck_mtx_t or lck_rw_t), an
 * handoff to a non blocking primitive is required before invoking any turnstile
 * operation.
 *
 * All functions will save the turnstile associated with the event on the
 * turnstile kernel hash table and will use the the turnstile kernel hash table
 * bucket spinlock as the turnstile interlock. Because we do not want to hold
 * interrupt disabled while holding the bucket interlock a new turnstile kernel
 * hash table is instantiated for this KPI to manage the hash without interrupt
 * disabled. Also:
 * - all events on the system that hash on the same bucket will contend on the
 * same spinlock.
 * - every event will have a dedicated wait_queue.
 *
 * Different locking primitives can be associated with sleep_with_inheritor as
 * long as the primitive_lock() and primitive_unlock() functions are provided to
 * sleep_with_inheritor_turnstile to perform the handoff with the bucket
 * spinlock.
 */
...
static kern_return_t wakeup_with_inheritor_and_turnstile(
    event_t event, wait_result_t result, lck_wakeup_type_t wake_type,
  ...
  case LCK_WAKEUP_ONE: {
    ...
    if (thread_wokenup) {
      ...
      *thread_wokenup = wokeup;
      ...
      ret = waitq_wakeup64_one(&ts->ts_waitq, CAST_EVENT64_T(event), result,
                               flags);
    }
  ...
  /*
   * turnstile_update_inheritor_complete could be called while holding the
   * interlock. In this case the new inheritor or is null, or is a thread that
   * is just been woken up and have not blocked because it is racing with the
   * same interlock used here after the wait. So there is no chain to update for
   * the new inheritor.
   *
   * However unless the current thread is the old inheritor,
   * old inheritor can be blocked and requires a chain update.
   *
   * The chain should be short because kernel turnstiles cannot have user
   * turnstiles chained after them.
   *
 ...
 *
 * Description: Redirect the push of the waiting threads of event to the new
 * inheritor specified.
 *
 * Args:
 ...
 *
 * Conditions: In case of success, the new inheritor cannot return to user space
 * or exit until another inheritor is specified for the event or a wakeup for
 * the event is called. NOTE: this cannot be called from interrupt context.
 */
kern_return_t change_sleep_inheritor(event_t event, thread_t inheritor) {
  uint32_t index;
  struct turnstile *ts = NULL;
   ...
   */
  turnstile_update_inheritor(
      ts, inheritor, (TURNSTILE_IMMEDIATE_UPDATE | TURNSTILE_INHERITOR_THREAD));
...
wait_result_t hw_lck_ticket_sleep_with_inheritor(
    hw_lck_ticket_t *lock, lck_grp_t *grp __unused,
    lck_sleep_action_t lck_sleep_action, event_t event, thread_t inheritor,
...
wait_result_t lck_mtx_sleep_with_inheritor(lck_mtx_t *lock,
                                           lck_sleep_action_t lck_sleep_action,
                                           event_t event, thread_t inheritor,
                                           wait_interrupt_t interruptible,
                                           uint64_t deadline) {
...
wait_result_t lck_rw_sleep_with_inheritor(lck_rw_t *lock,
                                          lck_sleep_action_t lck_sleep_action,
                                          event_t event, thread_t inheritor,
                                          wait_interrupt_t interruptible,
                                          uint64_t deadline) {
...
kern_return_t wakeup_thread_with_inheritor(event_t event, wait_result_t result,
                                           lck_wake_action_t action,
                                           thread_t thread_towake) {
  return wakeup_with_inheritor_and_turnstile(event, result, LCK_WAKEUP_THREAD,
                                             action, &thread_towake);
}

kern_return_t wakeup_one_with_inheritor(event_t event, wait_result_t result,
                                        lck_wake_action_t action,
                                        thread_t *thread_wokenup) {
  return wakeup_with_inheritor_and_turnstile(event, result, LCK_WAKEUP_ONE,
                                             action, thread_wokenup);
}

kern_return_t wakeup_all_with_inheritor(event_t event, wait_result_t result) {
  return wakeup_with_inheritor_and_turnstile(event, result, LCK_WAKEUP_ALL, 0,
                                             NULL);
}

void kdp_sleep_with_inheritor_find_owner(struct waitq *waitq,
...
static wait_result_t cond_sleep_with_inheritor_and_turnstile_type(
    cond_swi_var_t cond, bool (^cond_sleep_check)(ctid_t *),
    wait_interrupt_t interruptible, uint64_t deadline, turnstile_type_t type) {
  wait_result_t ret;
  uint32_t index;
  struct turnstile *ts = NULL;
  ctid_t ctid = 0;
  ...
  ret = waitq_assert_wait64(&ts->ts_waitq, CAST_EVENT64_T(cond), interruptible,
                            deadline);
 ...
 *
 * Description: Conditionally sleeps with inheritor, with condition variable of
 * 32bits. Allows a thread to conditionally sleep while indicating which thread
 * should inherit the priority push associated with the condition. The condition
 * should be expressed through a cond_swi_var32_s pointer. The condition needs
 * to be populated by the caller with the ctid of the thread that should inherit
 * the push. The remaining bits of the condition can be used by the caller to
 * implement its own synchronization logic. A copy of the condition value
 * observed by the caller when it decided to call this function should be
 * provided to prevent races with matching wakeups. This function will
 * atomically check the value stored in the condition against the
 * expected/observed one provided only for the bits that are set in the mask. If
 * the check doesn't pass the thread will not sleep and the function will
 * return. The ctid provided in the condition will be used only after a
 * successful check.
 *
 * Args:
 ...
 *
 * Conditions: The inheritor specified cannot return to user space or exit until
 * another inheritor is specified for the cond or a wakeup for the cond is
 * called.
 *
 * Returns: result of the wait.
 */
static wait_result_t cond_sleep_with_inheritor32_mask(
    cond_swi_var_t cond, cond_swi_var32_s expected_cond, uint32_t check_mask,
    wait_interrupt_t interruptible, uint64_t deadline) {
  bool (^cond_sleep_check)(uint32_t *) = ^(ctid_t *ctid) {
    cond_swi_var32_s cond_val = {.cond32_data =
                                     os_atomic_load((uint32_t *)cond, relaxed)};
    bool ret;
    if ((cond_val.cond32_data & check_mask) ==
  ...
  return cond_sleep_with_inheritor_and_turnstile_type(
      cond, cond_sleep_check, interruptible, deadline,
 ...
 *
 * Description: Conditionally sleeps with inheritor, with condition variable of
 * 64bits. Allows a thread to conditionally sleep while indicating which thread
 * should inherit the priority push associated with the condition. The condition
 * should be expressed through a cond_swi_var64_s pointer. The condition needs
 * to be populated by the caller with the ctid of the thread that should inherit
 * the push. The remaining bits of the condition can be used by the caller to
 * implement its own synchronization logic. A copy of the condition value
 * observed by the caller when it decided to call this function should be
...
wait_result_t cond_sleep_with_inheritor64_mask(cond_swi_var_t cond,
                                               ...
                                               uint64_t deadline) {
  bool (^cond_sleep_check)(uint32_t *) = ^(ctid_t *ctid) {
    cond_swi_var64_s cond_val = {.cond64_data =
                                     os_atomic_load((uint64_t *)cond, relaxed)};
    bool ret;
 ...
 * provided to prevent races with matching wakeups. This function will
 * atomically check the value stored in the condition against the
 * expected/observed one provided. If the check doesn't pass the thread will not
 *              sleep and the function will return.
 *              The ctid provided in the condition will be used only after a
 * successful check.
 *
 * Args:
 *   Arg1: cond_swi_var32_s pointer that stores the condition to check.
 ...
 * Returns: result of the wait.
 */
wait_result_t cond_sleep_with_inheritor32(cond_swi_var_t cond,
                                          cond_swi_var32_s expected_cond,
                                          wait_interrupt_t interruptible,
                                          uint64_t deadline) {
  return cond_sleep_with_inheritor32_mask(cond, expected_cond, ~0u,
 ...
 * Returns: result of the wait.
 */
wait_result_t cond_sleep_with_inheritor64(cond_swi_var_t cond,
                                          cond_swi_var64_s expected_cond,
                                          wait_interrupt_t interruptible,
                                          uint64_t deadline) {
  return cond_sleep_with_inheritor64_mask(cond, expected_cond, ~0ull,
                                          interruptible, deadline);
}
 ...
 *              The thread woken up will be the one with the higher sched
 * priority waiting on the condition. The push for the condition will be
 * transferred from the last inheritor to the woken up thread.
 *
 * Args:
 ...
 * Returns: KERN_NOT_WAITING if no threads were waiting, KERN_SUCCESS otherwise.
 *
 * Conditions: The new inheritor wokenup cannot return to user space or exit
 * until another inheritor is specified for the condition or a wakeup for the
 * event is called. A reference for the wokenup thread is acquired. NOTE: this
 * cannot be called from interrupt context.
 */
kern_return_t cond_wakeup_one_with_inheritor(cond_swi_var_t cond,
                                             wait_result_t result,
                                             lck_wake_action_t action,
                                             thread_t *thread_wokenup) {
  return wakeup_with_inheritor_and_turnstile(
      (event_t)cond, result, LCK_WAKEUP_ONE, action, thread_wokenup);
}

/*
 * Name: cond_wakeup_all_with_inheritor
 *
 * Description: Wake up all waiters waiting on the same condition. The old
 * inheritor will lose the push.
 *
 * Args:
 *   Arg1: condition to wake from.
...
kern_return_t cond_wakeup_all_with_inheritor(cond_swi_var_t cond,
                                             wait_result_t result) {
  return wakeup_with_inheritor_and_turnstile((event_t)cond, result,
                                             LCK_WAKEUP_ALL, 0, NULL);
}
...
#define gate_ilock(gate)                                                       \
  hw_lock_bit((hw_lock_bit_t *)(&(gate)->gt_data), GATE_ILOCK_BIT, LCK_GRP_NULL)
#define gate_iunlock(gate)                                                     \
  hw_unlock_bit((hw_lock_bit_t *)(&(gate)->gt_data), GATE_ILOCK_BIT)
#define gate_has_waiter_bit(state) ((state & GATE_WAITERS) != 0)
#define ordered_load_gate(gate)                                                \
  os_atomic_load(&(gate)->gt_data, compiler_acq_rel)
#define ordered_store_gate(gate, value)                                        \
  os_atomic_store(&(gate)->gt_data, value, compiler_acq_rel)
...
__abortlike static void gate_verify_tag_panic(gate_t *gate) {
  panic("Gate used is invalid. gate %p data %lx turnstile %p refs %d flags %x ",
        gate, gate->gt_data, gate->gt_turnstile, gate->gt_refs, gate->gt_flags);
}

__abortlike static void gate_verify_destroy_panic(gate_t *gate) {
  panic(
      "Gate used was destroyed. gate %p data %lx turnstile %p refs %d flags %x",
      gate, gate->gt_data, gate->gt_turnstile, gate->gt_refs, gate->gt_flags);
}

static void gate_verify(gate_t *gate) {
...
__abortlike static void gate_already_owned_panic(gate_t *gate,
                                                 thread_t holder) {
  panic("Trying to close a gate already closed gate %p holder %p "
        "current_thread %p",
        gate, holder, current_thread());
}

static kern_return_t gate_try_close(gate_t *gate) {
  uintptr_t state;
  thread_t holder;
  kern_return_t ret;
  thread_t thread = current_thread();
  ...
  if (os_atomic_cmpxchg(&gate->gt_data, 0, GATE_THREAD_TO_STATE(thread),
                        acquire)) {
    return KERN_SUCCESS;
  }
...
static void gate_close(gate_t *gate) {
  uintptr_t state;
  thread_t holder;
  thread_t thread = current_thread();
...
static void gate_open_turnstile(gate_t *gate) {
  struct turnstile *ts = NULL;

  ts = turnstile_prepare((uintptr_t)gate, &gate->gt_turnstile, TURNSTILE_NULL,
  ...
  turnstile_complete((uintptr_t)gate, &gate->gt_turnstile, NULL,
                     TURNSTILE_KERNEL_MUTEX);
  /*
...
__abortlike static void gate_not_owned_panic(gate_t *gate, thread_t holder,
                                             bool open) {
  if (open) {
    panic("Trying to open a gate %p owned by %p from current_thread %p", gate,
          holder, current_thread());
  } else {
    panic("Trying to handoff a gate %p owned by %p from current_thread %p",
          gate, holder, current_thread());
  }
}
...
static void gate_open(gate_t *gate) {
  ...
  gate_verify(gate);
  if (os_atomic_cmpxchg(&gate->gt_data, GATE_THREAD_TO_STATE(thread), 0,
                        release)) {
    return;
  }
...
static kern_return_t gate_handoff_turnstile(gate_t *gate, int flags,
   ...
   */
  hp_thread =
      waitq_wakeup64_identify(&ts->ts_waitq, CAST_EVENT64_T(GATE_EVENT(gate)),
                              THREAD_AWAKENED, WAITQ_UPDATE_INHERITOR);

  if (hp_thread != NULL) {
    /*
     * In this case waitq_wakeup64_identify has called
     * turnstile_update_inheritor for us
     */
    turnstile_update_inheritor_complete(ts, TURNSTILE_INTERLOCK_HELD);
    ...
    *waiters = turnstile_has_waiters(ts);
     ...
     * and it shouldn't be it by the time it finishes the wait, so that its next
     * open or handoff can go through the fast path. We could set the inheritor
     * to NULL here, or the new holder itself can set it on its way back from
     * the sleep. In the latter case there are more chanses that new waiters
     * will come by, avoiding to do the opearation at all.
     */
    ret = KERN_SUCCESS;
  } else {
    /*
     ...
     * Update the inheritor to NULL here, so that the current thread can return
     * to userspace indipendently from when the interrupted waiters will finish
     * the wait.
     */
   ...
   * It is ok because:
   * 1. current_thread is the previous inheritor and it is running
   * 2. new inheritor is NULL or it is a just wokenup thread that will race
   * acquiring the lock of the gate before trying to sleep.
   * => No chain of turnstiles needs to be updated.
   */
  turnstile_cleanup();
...
static kern_return_t gate_handoff(gate_t *gate, int flags) {
  kern_return_t ret;
  ...
  assert(flags == GATE_HANDOFF_OPEN_IF_NO_WAITERS ||
         flags == GATE_HANDOFF_DEFAULT);
  gate_verify(gate);
...
static void_func_void gate_steal_turnstile(gate_t *gate,
...
__abortlike static void gate_not_closed_panic(gate_t *gate, bool wait) {
  if (wait) {
    panic("Trying to wait on a not closed gate %p from current_thread %p", gate,
          current_thread());
  } else {
    panic("Trying to steal a not closed gate %p from current_thread %p", gate,
          current_thread());
  }
}

static void gate_steal(gate_t *gate) {
  uintptr_t state;
  thread_t holder;
  thread_t thread = current_thread();
  bool waiters;
...
static void_func_void gate_wait_turnstile(gate_t *gate,
                                          wait_interrupt_t interruptible,
                                          uint64_t deadline, thread_t holder,
                                          wait_result_t *wait, bool *waiters) {
  struct turnstile *ts;
  uintptr_t state;
  ...
  waitq_assert_wait64(&ts->ts_waitq, CAST_EVENT64_T(GATE_EVENT(gate)),
                      interruptible, deadline);
  ...
  *waiters = turnstile_has_waiters(ts);
  ...
  if (!*waiters) {
    /*
     * We want to enable the fast path as soon as we see that there are no more
     * waiters. On the fast path the holder will not do any turnstile
     * operations. Set the inheritor as NULL here.
     *
     * NOTE: if it was an open operation that woke this thread up, the inheritor
    ...
    if (holder &&
        ((*wait != THREAD_AWAKENED) ||  // thread interrupted or timedout
         holder == current_thread())) { // thread was woken up and it is the new
                                        // holder
      turnstile_update_inheritor(ts, TURNSTILE_INHERITOR_NULL,
                                 TURNSTILE_IMMEDIATE_UPDATE);
      turnstile_update_inheritor_complete(ts, TURNSTILE_INTERLOCK_NOT_HELD);
  ...
  /*
   * turnstile_cleanup might need to update the chain of the old holder.
   * This operation should happen without the turnstile primitive interlock
   * held.
   */
  return turnstile_cleanup;
}

static void gate_free_internal(gate_t *gate) { zfree(KT_GATE, gate); }

__abortlike static void gate_too_many_refs_panic(gate_t *gate) {
  panic("Too many refs taken on gate. gate %p data %lx turnstile %p refs %d "
        "flags %x",
        gate, gate->gt_data, gate->gt_turnstile, gate->gt_refs, gate->gt_flags);
}

static gate_wait_result_t
gate_wait(gate_t *gate, wait_interrupt_t interruptible, uint64_t deadline,
          void (^primitive_unlock)(void), void (^primitive_lock)(void)) {
  ...
  func_after_interlock_unlock = gate_wait_turnstile(
      gate, interruptible, deadline, holder, &wait_result, &waiters);
...
static void gate_assert(gate_t *gate, int flags) {
  uintptr_t state;
  thread_t holder;
...
enum { GT_INIT_DEFAULT = 0, GT_INIT_ALLOC };

static void gate_init(gate_t *gate, uint type) {
...
static gate_t *gate_alloc_init(void) {
  gate_t *gate;
...
__abortlike static void gate_destroy_owned_panic(gate_t *gate,
                                                 thread_t holder) {
  panic("Trying to destroy a gate owned by %p. Gate %p", holder, gate);
}

__abortlike static void gate_destroy_waiter_panic(gate_t *gate) {
  panic("Trying to destroy a gate with waiters. Gate %p data %lx turnstile %p",
        gate, gate->gt_data, gate->gt_turnstile);
}

static uint16_t gate_destroy_internal(gate_t *gate) {
  uintptr_t state;
  thread_t holder;
  uint16_t ref;
...
__abortlike static void gate_destroy_panic(gate_t *gate) {
  panic("Trying to destroy a gate that was allocated by gate_alloc_init(). "
        "gate_free() should be used instead, gate %p thread %p",
        gate, current_thread());
}
...
__abortlike static void gate_free_panic(gate_t *gate) {
  panic("Trying to free a gate that was not allocated by gate_alloc_init(), "
        "gate %p thread %p",
        gate, current_thread());
}

static void gate_free(gate_t *gate) {
  uint16_t ref;
 ...
 */
void lck_rw_gate_init(lck_rw_t *lock, gate_t *gate) {
  (void)lock;
 ...
 *         gate_t allocated.
 */
gate_t *lck_rw_gate_alloc_init(lck_rw_t *lock) {
  (void)lock;
 ...
 *   Arg2: pointer to the gate data declared with decl_lck_rw_gate_data.
 */
void lck_rw_gate_destroy(lck_rw_t *lock, gate_t *gate) {
  (void)lock;
  gate_destroy(gate);
}
 ...
 */
void lck_rw_gate_free(lck_rw_t *lock, gate_t *gate) {
  (void)lock;
  gate_free(gate);
}
 ...
 *
 * Returns:
 *          KERN_SUCCESS in case the gate was successfully closed. The current
 * thread is the new holder of the gate. A matching lck_rw_gate_open() or
 * lck_rw_gate_handoff() needs to be called later on to wake up possible waiters
 * on the gate before returning to userspace. If the intent is to conditionally
 * probe the gate before waiting, the lock must not be dropped between the calls
 * to lck_rw_gate_try_close() and lck_rw_gate_wait().
 *
 *          KERN_FAILURE in case the gate was already closed. Will panic if the
 * current thread was already the holder of the gate. lck_rw_gate_wait() should
 * be called instead if the intent is to unconditionally wait on this gate. The
 * calls to lck_rw_gate_try_close() and lck_rw_gate_wait() should be done
 * without dropping the lock that is protecting the gate in between.
 */
int lck_rw_gate_try_close(__assert_only lck_rw_t *lock, gate_t *gate) {
  LCK_RW_ASSERT(lock, LCK_RW_ASSERT_HELD);
 ...
 *              the holder of the gate. Will panic if the gate is already
 * closed. A matching lck_rw_gate_open() or lck_rw_gate_handoff() needs to be
 * called later on to wake up possible waiters on the gate before returning to
 * userspace.
 *
 * Args:
 *   Arg1: lck_rw_t lock used to protect the gate.
 ...
 *
 */
void lck_rw_gate_close(__assert_only lck_rw_t *lock, gate_t *gate) {
  LCK_RW_ASSERT(lock, LCK_RW_ASSERT_HELD);
 ...
 *
 */
void lck_rw_gate_open(__assert_only lck_rw_t *lock, gate_t *gate) {
  LCK_RW_ASSERT(lock, LCK_RW_ASSERT_HELD);
 ...
 *
 * Description: Tries to transfer the ownership of the gate. The waiter with
 * highest sched priority will be selected as the new holder of the gate, and
 * woken up, with the gate remaining in the closed state throughout. If no
 * waiters are present, the gate will be kept closed and KERN_NOT_WAITING will
 * be returned. GATE_HANDOFF_OPEN_IF_NO_WAITERS flag can be used to specify if
 * the gate should be opened in case no waiters were found.
 *
 *
 * Args:
 *   Arg1: lck_rw_t lock used to protect the gate.
 ...
 *
 */
kern_return_t lck_rw_gate_handoff(__assert_only lck_rw_t *lock, gate_t *gate,
                                  gate_handoff_flags_t flags) {
  LCK_RW_ASSERT(lock, LCK_RW_ASSERT_HELD);
 ...
 *
 * Description: Set the current ownership of the gate. It sets the current
 * thread as the new holder of the gate. A matching lck_rw_gate_open() or
 * lck_rw_gate_handoff() needs to be called later on to wake up possible waiters
 * on the gate before returning to userspace. NOTE: the previous holder should
 * not call lck_rw_gate_open() or lck_rw_gate_handoff() anymore.
 *
 *
 * Args:
 *   Arg1: lck_rw_t lock used to protect the gate.
 ...
 *
 * Conditions: Lock must be held. Returns with the lock held.
 *             The gate must be closed and the current thread must not already
 * be the holder.
 *
 */
void lck_rw_gate_steal(__assert_only lck_rw_t *lock, gate_t *gate) {
  LCK_RW_ASSERT(lock, LCK_RW_ASSERT_HELD);
 ...
 *
 * Description: Waits for the current thread to become the holder of the gate or
 * for the gate to become open. An interruptible mode and deadline can be
 * specified to return earlier from the wait.
 *
 * Args:
 *   Arg1: lck_rw_t lock used to protect the gate.
 *   Arg2: pointer to the gate data declared with decl_lck_rw_gate_data.
 *   Arg3: sleep action. LCK_SLEEP_DEFAULT, LCK_SLEEP_SHARED,
 * LCK_SLEEP_EXCLUSIVE, LCK_SLEEP_UNLOCK. Arg3: interruptible flag for wait.
 *   Arg4: deadline
 *
 * Conditions: Lock must be held. Returns with the lock held according to the
 * sleep action specified. Lock will be dropped while waiting. The gate must be
 * closed.
 *
 * Returns: Reason why the thread was woken up.
 *          GATE_HANDOFF - the current thread was handed off the ownership of
 * the gate. A matching lck_rw_gate_open() or lck_rw_gate_handoff() needs to be
 * called later on. to wake up possible waiters on the gate before returning to
 * userspace. GATE_OPENED - the gate was opened by the holder. GATE_TIMED_OUT -
 * the thread was woken up by a timeout. GATE_INTERRUPTED - the thread was
 * interrupted while sleeping.
 */
gate_wait_result_t lck_rw_gate_wait(lck_rw_t *lock, gate_t *gate,
                                    lck_sleep_action_t lck_sleep_action,
                                    wait_interrupt_t interruptible,
                                    uint64_t deadline) {
  __block lck_rw_type_t lck_rw_type = LCK_RW_TYPE_EXCLUSIVE;
 ...
 *         GATE_ASSERT_HELD - the gate is currently closed and the current
 * thread is the holder
 */
void lck_rw_gate_assert(__assert_only lck_rw_t *lock, gate_t *gate,
                        gate_assert_flags_t flags) {
  LCK_RW_ASSERT(lock, LCK_RW_ASSERT_HELD);
 ...
 */
void lck_mtx_gate_init(lck_mtx_t *lock, gate_t *gate) {
  (void)lock;
  gate_init(gate, GT_INIT_DEFAULT);
}
 ...
 *         gate_t allocated.
 */
gate_t *lck_mtx_gate_alloc_init(lck_mtx_t *lock) {
  (void)lock;
  return gate_alloc_init();
}
 ...
 *   Arg2: pointer to the gate data declared with decl_lck_mtx_gate_data.
 */
void lck_mtx_gate_destroy(lck_mtx_t *lock, gate_t *gate) {
  (void)lock;
  gate_destroy(gate);
}
 ...
 *   Arg2: pointer to the gate obtained with lck_rw_gate_alloc_init().
 */
void lck_mtx_gate_free(lck_mtx_t *lock, gate_t *gate) {
  (void)lock;
  gate_free(gate);
}
 ...
 * Returns:
 *          KERN_SUCCESS in case the gate was successfully closed. The current
 * thread is the new holder of the gate. A matching lck_mtx_gate_open() or
 * lck_mtx_gate_handoff() needs to be called later on to wake up possible
 * waiters on the gate before returning to userspace. If the intent is to
 * conditionally probe the gate before waiting, the lock must not be dropped
 *          between the calls to lck_mtx_gate_try_close() and
 * lck_mtx_gate_wait().
 *
 *          KERN_FAILURE in case the gate was already closed. Will panic if the
 * current thread was already the holder of the gate. lck_mtx_gate_wait() should
 * be called instead if the intent is to unconditionally wait on this gate. The
 * calls to lck_mtx_gate_try_close() and lck_mtx_gate_wait() should be done
 * without dropping the lock that is protecting the gate in between.
 */
int lck_mtx_gate_try_close(__assert_only lck_mtx_t *lock, gate_t *gate) {
  LCK_MTX_ASSERT(lock, LCK_MTX_ASSERT_OWNED);
 ...
 * Description: Closes the gate. The current thread will be set as
 *              the holder of the gate. Will panic if the gate is already
 * closed. A matching lck_mtx_gate_open() or lck_mtx_gate_handoff() needs to be
 * called later on to wake up possible waiters on the gate before returning to
 * userspace.
 *
 * Args:
 ...
 *
 */
void lck_mtx_gate_close(__assert_only lck_mtx_t *lock, gate_t *gate) {
  LCK_MTX_ASSERT(lock, LCK_MTX_ASSERT_OWNED);
 ...
 *
 */
void lck_mtx_gate_open(__assert_only lck_mtx_t *lock, gate_t *gate) {
  LCK_MTX_ASSERT(lock, LCK_MTX_ASSERT_OWNED);
 ...
 *
 */
kern_return_t lck_mtx_gate_handoff(__assert_only lck_mtx_t *lock, gate_t *gate,
                                   gate_handoff_flags_t flags) {
  LCK_MTX_ASSERT(lock, LCK_MTX_ASSERT_OWNED);
 ...
 *
 * Description: Steals the ownership of the gate. It sets the current thread as
 * the new holder of the gate. A matching lck_mtx_gate_open() or
 * lck_mtx_gate_handoff() needs to be called later on to wake up possible
 * waiters on the gate before returning to userspace. NOTE: the previous holder
 * should not call lck_mtx_gate_open() or lck_mtx_gate_handoff() anymore.
 *
 *
 * Args:
 *   Arg1: lck_mtx_t lock used to protect the gate.
 ...
 *
 */
void lck_mtx_gate_steal(__assert_only lck_mtx_t *lock, gate_t *gate) {
  LCK_MTX_ASSERT(lock, LCK_MTX_ASSERT_OWNED);
 ...
 *   Arg1: lck_mtx_t lock used to protect the gate.
 *   Arg2: pointer to the gate data declared with decl_lck_mtx_gate_data.
 *   Arg3: sleep action. LCK_SLEEP_DEFAULT, LCK_SLEEP_UNLOCK, LCK_SLEEP_SPIN,
 * LCK_SLEEP_SPIN_ALWAYS. Arg3: interruptible flag for wait. Arg4: deadline
 *
 * Conditions: Lock must be held. Returns with the lock held according to the
 * sleep action specified. Lock will be dropped while waiting. The gate must be
 * closed.
 ...
 * Returns: Reason why the thread was woken up.
 *          GATE_HANDOFF - the current thread was handed off the ownership of
 * the gate. A matching lck_mtx_gate_open() or lck_mtx_gate_handoff() needs to
 * be called later on to wake up possible waiters on the gate before returning
 * to userspace. GATE_OPENED - the gate was opened by the holder. GATE_TIMED_OUT
 * - the thread was woken up by a timeout. GATE_INTERRUPTED - the thread was
 * interrupted while sleeping.
 */
gate_wait_result_t lck_mtx_gate_wait(lck_mtx_t *lock, gate_t *gate,
                                     lck_sleep_action_t lck_sleep_action,
                                     wait_interrupt_t interruptible,
                                     uint64_t deadline) {
  LCK_MTX_ASSERT(lock, LCK_MTX_ASSERT_OWNED);
 ...
 * thread is the holder
 */
void lck_mtx_gate_assert(__assert_only lck_mtx_t *lock, gate_t *gate,
                         gate_assert_flags_t flags) {
  LCK_MTX_ASSERT(lock, LCK_MTX_ASSERT_OWNED);
...
__startup_func void lck_spin_startup_init(struct lck_spin_startup_spec *sp) {
  lck_spin_init(sp->lck, sp->lck_grp, sp->lck_attr);
}

__startup_func void lck_mtx_startup_init(struct lck_mtx_startup_spec *sp) {
  lck_mtx_init(sp->lck, sp->lck_grp, sp->lck_attr);
}

__startup_func void lck_rw_startup_init(struct lck_rw_startup_spec *sp) {
