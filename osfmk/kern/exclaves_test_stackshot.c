...
static const char *testpoint_name(tp_id_t testpoint) {
  switch (testpoint) {
 ...
 *   stackshot thread (main test thread)
 ...
 |
 |
 |   create exclaves thread --------------------------------------> exclaves
 thread starts | | | | |   start stackshot call c-hello-world exclave service |
 | |                                                                  | |
 ...
 |                                                                  [UPCALL]
 |   unblock <----------------------------------------------------- release
 stackshot thread and wait in upcall | until it's flagged for AST check | | |
 stackshot collection                                           | | | | | |
 [STACKSHOT_DONE] --------------------------------------------> stop hanging in
 upcall and return | | | | |   compose kcdata done
 |
 |
 ...
 *   =========================
 *   Send one thread into upcall and start exclaves collection when the thread
 starts waiting on AST.
 *
 *   stackshot thread (main test thread)
 |
 |
 ...
 |   unblock <----------------------------------------------------- [UPCALL]
 |                                                                  release
 stackshot thread and wait in upcall | | |   collect xnu (without exclaves) mark
 exclave threads blocked    | | | | | |   [START_COLLECTION]
 ------------------------------------------> stop hanging in upcall and try to
 return |   block here, just before exclave threads collection             | | |
 |                                                                  |
 |   unblock <----------------------------------------------------- [TP_AST]
 |   collection pending                                             just before
 AST flag check | | | | | wait on AST check (assuming collection takes some
 time) |                                                                  | | |
...
void tp_call_stackshot_upcall(tp_id_t testpoint, tp_val_t __unused val) {
  ...
  case TP_UPCALL: // upcall thread, block itself and start stackshot
    tp_relay(testpoint, TP_WAIT_START_STACKSHOT);
    break;
  case TP_START_COLLECTION: // stackshot thread, just before exclaves collection
                            // start
