...
SCALABLE_COUNTER_DEFINE(
    vm_statistics_zero_fill_count); /* # of zero fill pages */
SCALABLE_COUNTER_DEFINE(
    vm_statistics_reactivations);                  /* # of pages reactivated */
...
SCALABLE_COUNTER_DEFINE(
    vm_statistics_decompressions); /* # of pages decompressed */
SCALABLE_COUNTER_DEFINE(vm_statistics_compressions); /* # of pages compressed */
SCALABLE_COUNTER_DEFINE(vm_statistics_swapins);  /* # of pages swapped in (via
                                                    compression segments) */
SCALABLE_COUNTER_DEFINE(vm_statistics_swapouts); /* # of pages swapped out (via
                                                    compression segments) */
SCALABLE_COUNTER_DEFINE(
    vm_statistics_total_uncompressed_pages_in_compressor); /* # of pages
                                                              (uncompressed)
                                                              held within the
                                                              compressor. */
...
static void get_host_vm_stats(vm_statistics64_t out) {
...
kern_return_t host_info(host_t host, host_flavor_t flavor, host_info_t info,
  ...
  case HOST_SCHED_INFO: {
    ...
    sched_info->min_timeout = sched_info->min_quantum =
        (uint32_t)(quantum_ns / 1000 / 1000);
  ...
  case HOST_PREFERRED_USER_ARCH: {
    ...
    if (!PE_get_default("kern.preferred_cpu_subtype", &preferred_cpu_subtype,
...
kern_return_t host_statistics(host_t host, host_flavor_t flavor,
  ...
  case HOST_LOAD_INFO: {
    ...
    bcopy((char *)mach_factor, (char *)load_info->mach_factor,
          sizeof mach_factor);
  ...
  case HOST_VM_INFO: {
    ...
    stat32->free_count = VM_STATISTICS_TRUNCATE_TO_32_BIT(vm_page_free_count +
                                                          speculative_count);
    stat32->active_count =
    ...
    stat32->zero_fill_count =
        VM_STATISTICS_TRUNCATE_TO_32_BIT(host_vm_stat.zero_fill_count);
    stat32->reactivations =
        VM_STATISTICS_TRUNCATE_TO_32_BIT(host_vm_stat.reactivations);
    ...
    stat32->cow_faults =
        VM_STATISTICS_TRUNCATE_TO_32_BIT(host_vm_stat.cow_faults);
    ...
    *count = HOST_VM_INFO_REV0_COUNT; /* rev0 already filled in */
    ...
    if (original_count >= HOST_VM_INFO_REV2_COUNT) {
      ...
      stat32->speculative_count =
          VM_STATISTICS_TRUNCATE_TO_32_BIT(speculative_count);
...
#define GET_TICKS_VALUE(state, ticks)                                          \
  MACRO_BEGIN cpu_load_info->cpu_ticks[(state)] +=                             \
...
#define GET_TICKS_VALUE_FROM_TIMER(processor, state, timer)                    \
  ...
  case HOST_EXPIRED_TASK_INFO: {
    ...
    tinfo1->task_interrupt_wakeups =
        dead_task_statistics.task_interrupt_wakeups;
    tinfo1->task_platform_idle_wakeups =
        dead_task_statistics.task_platform_idle_wakeups;

    tinfo1->task_timer_wakeups_bin_1 =
        dead_task_statistics.task_timer_wakeups_bin_1;

    tinfo1->task_timer_wakeups_bin_2 =
    ...
    if (*count < TASK_POWER_INFO_V2_COUNT) {
      *count = TASK_POWER_INFO_COUNT;
      ...
      tinfo2->gpu_energy.task_gpu_utilisation =
          dead_task_statistics.task_gpu_ns;
...
#define HOST_STATISTICS_MAX_REQUESTS                                           \
  10 /* maximum number of requests per window */
#define HOST_STATISTICS_MIN_REQUESTS                                           \
  2 /* minimum number of requests per window */
...
static struct host_stats_cache g_host_stats_cache[NUM_HOST_INFO_DATA_TYPES] = {
    [HOST_VM_INFO64_REV0] = {.last_access = 0,
                             .current_requests = 0,
                             .max_requests = 0,
                             .data = (uintptr_t)&host_vm_info64_rev0,
                             .count = HOST_VM_INFO64_REV0_COUNT},
    [HOST_VM_INFO64_REV1] = {.last_access = 0,
                             .current_requests = 0,
                             .max_requests = 0,
                             .data = (uintptr_t)&host_vm_info64_rev1,
                             .count = HOST_VM_INFO64_REV1_COUNT},
    [HOST_EXTMOD_INFO64_REV0] = {.last_access = 0,
                                 .current_requests = 0,
                                 .max_requests = 0,
                                 .data = (uintptr_t)&host_extmod_info64,
                                 .count = HOST_EXTMOD_INFO64_COUNT},
    [HOST_LOAD_INFO_REV0] = {.last_access = 0,
                             .current_requests = 0,
                             .max_requests = 0,
                             .data = (uintptr_t)&host_load_info,
                             .count = HOST_LOAD_INFO_COUNT},
    [HOST_VM_INFO_REV0] = {.last_access = 0,
                           .current_requests = 0,
                           .max_requests = 0,
                           .data = (uintptr_t)&host_vm_info_rev0,
                           .count = HOST_VM_INFO_REV0_COUNT},
    [HOST_VM_INFO_REV1] = {.last_access = 0,
                           .current_requests = 0,
                           .max_requests = 0,
                           .data = (uintptr_t)&host_vm_info_rev1,
                           .count = HOST_VM_INFO_REV1_COUNT},
    [HOST_VM_INFO_REV2] = {.last_access = 0,
                           .current_requests = 0,
                           .max_requests = 0,
                           .data = (uintptr_t)&host_vm_info_rev2,
                           .count = HOST_VM_INFO_REV2_COUNT},
    [HOST_CPU_LOAD_INFO_REV0] = {.last_access = 0,
                                 .current_requests = 0,
                                 .max_requests = 0,
                                 .data = (uintptr_t)&host_cpu_load_info,
                                 .count = HOST_CPU_LOAD_INFO_COUNT},
    [HOST_EXPIRED_TASK_INFO_REV0] = {.last_access = 0,
                                     .current_requests = 0,
                                     .max_requests = 0,
                                     .data = (uintptr_t)&host_expired_task_info,
                                     .count = TASK_POWER_INFO_COUNT},
    [HOST_EXPIRED_TASK_INFO_REV1] = {.last_access = 0,
                                     .current_requests = 0,
                                     .max_requests = 0,
                                     .data =
                                         (uintptr_t)&host_expired_task_info2,
                                     .count = TASK_POWER_INFO_V2_COUNT},
    [HOST_VM_COMPRESSOR_Q_LEN_REV0] =
        {.last_access = 0,
         .current_requests = 0,
         .max_requests = 0,
         .data = (uintptr_t)&host_vm_compressor_q_lens,
         .count = VM_COMPRESSOR_Q_LENS_COUNT},
    [HOST_VM_INFO64_REV2] = {.last_access = 0,
                             .current_requests = 0,
                             .max_requests = 0,
                             .data = (uintptr_t)&host_vm_info64_rev2,
                             .count = HOST_VM_INFO64_REV2_COUNT},
};

void host_statistics_init(void) {
  nanoseconds_to_absolutetime((HOST_STATISTICS_TIME_WINDOW * NSEC_PER_SEC),
                              &host_statistics_time_window);
}

static void cache_host_statistics(int index, host_info64_t info) {
  ...
  memcpy((void *)g_host_stats_cache[index].data, info,
         g_host_stats_cache[index].count * sizeof(integer_t));
  return;
}

static void get_cached_info(int index, host_info64_t info,
                            mach_msg_type_number_t *count) {
  if (index < 0 || index >= NUM_HOST_INFO_DATA_TYPES) {
    *count = 0;
    return;
  ...
  *count = g_host_stats_cache[index].count;
  memcpy(info, (void *)g_host_stats_cache[index].data,
         g_host_stats_cache[index].count * sizeof(integer_t));
}

static int get_host_info_data_index(bool is_stat64, host_flavor_t flavor,
                                    mach_msg_type_number_t *count,
                                    kern_return_t *ret) {
  switch (flavor) {
  case HOST_VM_INFO64:
...
static bool rate_limit_host_statistics(bool is_stat64, host_flavor_t flavor,
                                       host_info64_t info,
                                       mach_msg_type_number_t *count,
                                       kern_return_t *ret, int *pindex) {
  ...
  *pindex = -1;
  ...
  /* Rate limit to HOST_STATISTICS_MAX_REQUESTS queries for each
  ...
  *pindex = index;
  ...
  if (g_host_stats_cache[index].last_access >
      mach_continuous_time() - host_statistics_time_window) {
    set_last_access = FALSE;
    if (g_host_stats_cache[index].current_requests++ >=
        g_host_stats_cache[index].max_requests) {
      rate_limited = TRUE;
  ...
  if (set_last_access) {
    ...
    /*
     * select a random number of requests (included between
     * HOST_STATISTICS_MIN_REQUESTS and HOST_STATISTICS_MAX_REQUESTS) to let
     * query host_statistics. In this way it is not possible to infer looking at
     * when the a cached copy changes if host_statistics was called on the
     * provious window.
     */
    g_host_stats_cache[index].max_requests =
        (mach_absolute_time() %
         (HOST_STATISTICS_MAX_REQUESTS - HOST_STATISTICS_MIN_REQUESTS + 1)) +
...
#else  /* !XNU_TARGET_OS_OSX */
  stat->wire_count =
      vm_page_wire_count + throttled_count + vm_lopage_free_count;
#endif /* !XNU_TARGET_OS_OSX */
  ...
  *count = HOST_VM_INFO64_REV0_COUNT; /* rev0 already filled in */
  if (original_count >= HOST_VM_INFO64_REV1_COUNT) {
    ...
    stat->external_page_count =
        (vm_page_pageable_external_count + local_q_external_count);
    stat->internal_page_count =
        (vm_page_pageable_internal_count + local_q_internal_count);
...
extern uint32_t c_early_swappedin_count, c_regular_swappedin_count,
    c_late_swappedin_count;
extern uint32_t c_early_swapout_count, c_regular_swapout_count,
...
static kern_return_t vm_compressor_queue_lens(void *info, unsigned int *count) {
  if (*count < VM_COMPRESSOR_Q_LENS_COUNT) {
...
kern_return_t host_statistics64_from_user(host_t host, host_flavor_t flavor,
                                          host_info64_t info,
                                          mach_msg_type_number_t *count) {
...
kern_return_t host_statistics_from_user(host_t host, host_flavor_t flavor,
                                        host_info64_t info,
                                        mach_msg_type_number_t *count) {
  kern_return_t ret = KERN_SUCCESS;
  int index;
 ...
 */
kern_return_t host_priv_statistics(host_priv_t host_priv, host_flavor_t flavor,
                                   host_info_t info,
                                   mach_msg_type_number_t *count) {
  return host_statistics((host_t)host_priv, flavor, info, count);
}

kern_return_t set_sched_stats_active(boolean_t active) {
  sched_stats_active = active;
  return KERN_SUCCESS;
}

kern_return_t get_sched_statistics(struct _processor_statistics_np *out,
                                   uint32_t *count) {
  uint32_t pos = 0;
...
kern_return_t host_kernel_version(host_t host, kernel_version_t out_version) {
  if (host == HOST_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
 ...
 */
kern_return_t host_processor_sets(host_priv_t host_priv,
 ...
 */
kern_return_t host_processor_set_priv(host_priv_t host_priv,
...
kern_return_t host_processor_info(host_t host, processor_flavor_t flavor,
  ...
  result =
      kmem_alloc(ipc_kernel_map, &addr, size, KMA_DATA, VM_KERN_MEMORY_IPC);
  if (result != KERN_SUCCESS) {
    return KERN_RESOURCE_SHORTAGE;
  }
  ...
  result = vm_map_unwire(
      ipc_kernel_map, vm_map_trunc_page(addr, VM_MAP_PAGE_MASK(ipc_kernel_map)),
  ...
  result = vm_map_copyin(ipc_kernel_map, (vm_map_address_t)addr,
                         (vm_map_size_t)needed, TRUE, &copy);
  assert(result == KERN_SUCCESS);
...
static bool is_valid_host_special_port(int id) {
  return (id <= HOST_MAX_SPECIAL_PORT) && (id >= HOST_MIN_SPECIAL_PORT) &&
         ((id <= HOST_LAST_SPECIAL_KERNEL_PORT) ||
          (id > HOST_MAX_SPECIAL_KERNEL_PORT));
}

/*
 *      Kernel interface for setting a special port.
 */
kern_return_t kernel_set_special_port(host_priv_t host_priv, int id,
                                      ipc_port_t port) {
  ipc_port_t old_port;
 ...
 */
kern_return_t kernel_get_special_port(host_priv_t host_priv, int id,
                                      ipc_port_t *portp) {
  if (!is_valid_host_special_port(id)) {
 ...
 */
kern_return_t host_set_special_port_from_user(host_priv_t host_priv, int id,
                                              ipc_port_t port) {
  if (host_priv == HOST_PRIV_NULL || id <= HOST_MAX_SPECIAL_KERNEL_PORT ||
...
kern_return_t host_set_special_port(host_priv_t host_priv, int id,
                                    ipc_port_t port) {
  if (host_priv == HOST_PRIV_NULL || id <= HOST_MAX_SPECIAL_KERNEL_PORT ||
      id > HOST_MAX_SPECIAL_PORT) {
    return KERN_INVALID_ARGUMENT;
...
kern_return_t host_get_special_port_from_user(host_priv_t host_priv,
                                              __unused int node, int id,
                                              ipc_port_t *portp) {
  if (host_priv == HOST_PRIV_NULL || id == HOST_SECURITY_PORT ||
      id > HOST_MAX_SPECIAL_PORT || id < HOST_MIN_SPECIAL_PORT) {
    return KERN_INVALID_ARGUMENT;
  }
...
kern_return_t host_get_special_port(host_priv_t host_priv, __unused int node,
                                    int id, ipc_port_t *portp) {
  ipc_port_t port;
 ...
 */
kern_return_t host_get_io_main(host_t host, io_main_t *io_mainp) {
  if (host == HOST_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
...
host_t host_self(void) { return &realhost; }

host_priv_t host_priv_self(void) { return &realhost; }

kern_return_t host_set_atm_diagnostic_flag(host_t host,
...
kern_return_t host_set_multiuser_config_flags(host_priv_t host_priv,
