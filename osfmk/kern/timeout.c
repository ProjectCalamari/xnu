...
TUNABLE_DT_WRITEABLE(int, timeouts_are_fatal, "machine-timeouts",
                     "timeouts-are-fatal", "timeouts_are_fatal", 1,
                     TUNABLE_DT_CHECK_CHOSEN);
#endif

void kern_timeout_restart(kern_timeout_t *to, timeout_flags_t flags) {
  ...
  to->start_mt = (flags & TF_NONSPEC_TIMEBASE) ? ml_get_timebase()
                                               : ml_get_speculative_timebase();
}

void kern_timeout_start(kern_timeout_t *to, timeout_flags_t flags) {
...
void kern_timeout_end(kern_timeout_t *to, timeout_flags_t flags) {
  to->end_mt = (flags & TF_NONSPEC_TIMEBASE) ? ml_get_timebase()
                                             : ml_get_speculative_timebase();
  if (flags & TF_SAMPLE_INTERRUPT_TIME) {
    to->int_mt =
        recount_current_processor_interrupt_duration_mach() - to->int_mt;
  }
}

/*
 * Zero out the timeout state so that we won't have a timeout triggered later in
 * the processing of this timeout.
 */
void kern_timeout_override(kern_timeout_t *to) {
  to->start_mt = 0;
#if CONFIG_CPU_COUNTERS
  to->start_cycles = 0;
  to->start_instrs = 0;
...
#if CONFIG_CPU_COUNTERS
void kern_timeout_cycles_instrs(kern_timeout_t *to, uint64_t *cycles,
                                uint64_t *instrs) {
...
void kern_timeout_cpi(kern_timeout_t *to, uint64_t *cpi_whole,
...
void kern_timeout_cycles_instrs(kern_timeout_t __unused *to, uint64_t *cycles,
                                uint64_t *instrs) {
  *cycles = 0;
  *instrs = 0;
}

void kern_timeout_cpi(kern_timeout_t __unused *to, uint64_t *cpi_whole,
                      uint64_t *cpi_fractional) {
  *cpi_whole = 0;
  *cpi_fractional = 0;
}
...
__enum_closed_decl(timeout_mode_t, uint32_t,
                   {TIMEOUT_TELEMETRY, TIMEOUT_PANIC});
 ...
 */
void kern_timeout_try_panic(kern_timeout_type_t __unused type,
                            uint64_t __unused payload, kern_timeout_t *to,
  ...
  if (mode == TIMEOUT_TELEMETRY) {
    ...
    trap_telemetry_report_latency_violation(
        trap_type,
        (trap_telemetry_latency_s){.violation_cpi = cpi_whole,
  ...
  if (cycles > 0 && instrs > 0) {
    ...
    snprintf(cpi, sizeof(cpi),
             ", freq %llu MHz, type = %c, CPI = %llu.%llu [%llu, %llu]",
             average_freq, core_type, cpi_whole, cpi_fractional, cycles,
             instrs);
  }
#endif /* CONFIG_CPU_COUNTERS */
  ...
  if (gross_ns > net_ns) {
    if (threshold_ms > 0) {
      snprintf(duration, sizeof(duration),
               ...
               gross_us % 1000, net_ms, net_us % 1000, threshold_ms,
               threshold_us % 1000);
    } else {
      snprintf(duration, sizeof(duration),
               "gross %llu us, net %llu us >= %llu us", gross_us, net_us,
               threshold_us);
    }
  } else {
    if (threshold_ms > 0) {
      ...
      snprintf(duration, sizeof(duration), "%llu us >= %llu us", gross_us,
               threshold_us);
    }
  }

  panic_plain("%s %s (start: %llu, end: %llu)%s", prefix, duration, start_mt,
              end_mt, cpi);
}
