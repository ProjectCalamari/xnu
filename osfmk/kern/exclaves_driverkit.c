...
tb_error_t exclaves_driverkit_upcall_legacy_irq_register(
    const uint64_t id, const int32_t index,
  ...
  if (!IOExclaveInterruptUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_irq_register__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_irq_remove(
    const uint64_t id, const int32_t index,
  ...
  if (!IOExclaveInterruptUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_irq_remove__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_irq_enable(
    const uint64_t id, const int32_t index,
  ...
  if (!IOExclaveInterruptUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_irq_enable__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_irq_disable(
    const uint64_t id, const int32_t index,
  ...
  if (!IOExclaveInterruptUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_irq_disable__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_timer_register(
    const uint64_t id,
    tb_error_t (^completion)(xnuupcalls_xnuupcalls_timer_register__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls, "[iokit_upcalls] %s from id %llu\n",
                        __func__, id);
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_timer_register__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_timer_remove(
    const uint64_t id, const uint32_t timer_id,
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_timer_remove__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
extern tb_error_t exclaves_driverkit_upcall_legacy_timer_enable(
    const uint64_t id, const uint32_t timer_id,
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_timer_enable__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_timer_disable(
    const uint64_t id, const uint32_t timer_id,
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_timer_disable__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_timer_set_timeout(
    const uint64_t id, const uint32_t timer_id,
    const struct xnuupcalls_drivertimerspecification_s *duration,
    tb_error_t (^completion)(
  ...
  if (!duration) {
    ...
    xnuupcalls_xnuupcalls_timer_set_timeout__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
    return completion(result);
  }
  ...
  clock_interval_to_absolutetime_interval(duration->tv_nsec, kNanosecondScale,
                                          &nsecs);
  clock_interval_to_absolutetime_interval(duration->tv_sec, kSecondScale, &end);
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    ...
    xnuupcalls_xnuupcalls_timer_set_timeout__result_init_success(
...
tb_error_t exclaves_driverkit_upcall_legacy_timer_cancel_timeout(
    const uint64_t id, const uint32_t timer_id,
    tb_error_t (^completion)(
        xnuupcalls_xnuupcalls_timer_cancel_timeout__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls, "[iokit_upcalls] %s from id %llu\n",
                        __func__, id);
  struct IOExclaveTimerUpcallArgs args;
  args.timer_id = timer_id;
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_timer_cancel_timeout__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_lock_wl(
    const uint64_t id,
  ...
  if (!IOExclaveLockWorkloop(id, true)) {
    xnuupcalls_xnuupcalls_lock_wl__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_unlock_wl(
    const uint64_t id,
  ...
  if (!IOExclaveLockWorkloop(id, false)) {
    xnuupcalls_xnuupcalls_unlock_wl__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
extern tb_error_t exclaves_driverkit_upcall_legacy_async_notification_signal(
    const uint64_t id, const uint32_t notificationID,
    tb_error_t (^completion)(
        xnuupcalls_xnuupcalls_async_notification_signal__result_s)) {
  ...
  if (!IOExclaveAsyncNotificationUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_async_notification_signal__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
    xnuupcalls_xnuupcalls_async_notification_signal__result_init_success(
        &result);
  }
...
tb_error_t exclaves_driverkit_upcall_legacy_mapper_activate(
  ...
  if (!IOExclaveMapperOperationUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_mapper_activate__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
tb_error_t exclaves_driverkit_upcall_legacy_mapper_deactivate(
    const uint64_t id, const uint32_t mapperIndex,
    tb_error_t (^completion)(
        xnuupcalls_xnuupcalls_mapper_deactivate__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls,
  ...
  if (!IOExclaveMapperOperationUpcallHandler(id, &args)) {
    xnuupcalls_xnuupcalls_mapper_deactivate__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
...
extern tb_error_t exclaves_driverkit_upcall_legacy_ane_setpowerstate(
  ...
  if (!IOExclaveANEUpcallHandler(id, &args, &ret)) {
    xnuupcalls_xnuupcalls_ane_setpowerstate__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
    xnuupcalls_xnuupcalls_ane_setpowerstate__result_init_success(&result, ret);
  }
  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_legacy_ane_worksubmit(
    const uint64_t id, const uint64_t requestID,
  ...
  if (!IOExclaveANEUpcallHandler(id, &args, &ret)) {
    xnuupcalls_xnuupcalls_ane_worksubmit__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
    xnuupcalls_xnuupcalls_ane_worksubmit__result_init_success(&result, ret);
  }
  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_legacy_ane_workbegin(
    const uint64_t id, const uint64_t requestID, const uint64_t beginTimestamp,
  ...
  if (!IOExclaveANEUpcallHandler(id, &args, &ret)) {
    xnuupcalls_xnuupcalls_ane_workbegin__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
    xnuupcalls_xnuupcalls_ane_workbegin__result_init_success(&result, ret);
  }
  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_legacy_ane_workend(
    const uint64_t id, const uint64_t requestID,
  ...
  if (!IOExclaveANEUpcallHandler(id, &args, &ret)) {
    xnuupcalls_xnuupcalls_ane_workend__result_init_failure(
        &result, XNUUPCALLS_DRIVERUPCALLERROR_FAILURE);
  } else {
    xnuupcalls_xnuupcalls_ane_workend__result_init_success(&result, ret);
  }
  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_legacy_notification_signal(
    const uint64_t id, const uint32_t mask,
    tb_error_t (^completion)(
  ...
  if (notification_resource != NULL) {
    ...
    exclaves_debug_printf(show_notification_upcalls,
                          ...
                          id, mask);
    xnuupcalls_xnuupcalls_notification_signal__result_init_failure(
        &result, XNUUPCALLS_NOTIFICATIONERROR_NOTFOUND);
  }
...
tb_error_t exclaves_driverkit_upcall_irq_register(
    const uint64_t id, const int32_t index,
    tb_error_t (^completion)(
  ...
  if (!IOExclaveInterruptUpcallHandler(id, &args)) {
    ...
    xnuupcallsv2_driverupcallsprivate_irqregister__result_init_failure(&result,
                                                                       err);
  } else {
    xnuupcallsv2_driverupcallsprivate_irqregister__result_init_success(&result);
  }

  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_irq_remove(
    const uint64_t id, const int32_t index,
    tb_error_t (^completion)(
  ...
  if (!IOExclaveInterruptUpcallHandler(id, &args)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_driverupcallsprivate_irqremove__result_init_failure(&result,
                                                                     err);
  } else {
    xnuupcallsv2_driverupcallsprivate_irqremove__result_init_success(&result);
  }

  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_irq_enable(
    const uint64_t id, const int32_t index,
    tb_error_t (^completion)(
  ...
  if (!IOExclaveInterruptUpcallHandler(id, &args)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_driverupcallsprivate_irqenable__result_init_failure(&result,
                                                                     err);
  } else {
    xnuupcallsv2_driverupcallsprivate_irqenable__result_init_success(&result);
  }

  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_irq_disable(
    const uint64_t id, const int32_t index,
    tb_error_t (^completion)(
  ...
  if (!IOExclaveInterruptUpcallHandler(id, &args)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_driverupcallsprivate_irqdisable__result_init_failure(&result,
                                                                      err);
  } else {
    xnuupcallsv2_driverupcallsprivate_irqdisable__result_init_success(&result);
  }

  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_timer_register(
    const uint64_t id,
    tb_error_t (^completion)(
...
tb_error_t exclaves_driverkit_upcall_timer_remove(
    const uint64_t id, const uint32_t timer_id,
    tb_error_t (^completion)(
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_driverupcallsprivate_timerremove__result_init_failure(&result,
                                                                       err);
  } else {
    xnuupcallsv2_driverupcallsprivate_timerremove__result_init_success(&result);
  }

  return completion(result);
}

extern tb_error_t exclaves_driverkit_upcall_timer_enable(
    const uint64_t id, const uint32_t timer_id,
    tb_error_t (^completion)(
        xnuupcallsv2_driverupcallsprivate_timerenable__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls, "[iokit_upcalls] %s from id %llu\n",
                        __func__, id);
  struct IOExclaveTimerUpcallArgs args;
  args.timer_id = timer_id;
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_driverupcallsprivate_timerenable__result_init_failure(&result,
                                                                       err);
  } else {
    xnuupcallsv2_driverupcallsprivate_timerenable__result_init_success(&result);
  }

  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_timer_disable(
    const uint64_t id, const uint32_t timer_id,
    tb_error_t (^completion)(
        xnuupcallsv2_driverupcallsprivate_timerdisable__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls, "[iokit_upcalls] %s from id %llu\n",
                        __func__, id);
  struct IOExclaveTimerUpcallArgs args;
  args.timer_id = timer_id;
  ...
  if (!IOExclaveTimerUpcallHandler(id, &args)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_driverupcallsprivate_timerdisable__result_init_failure(&result,
                                                                        err);
  } else {
...
tb_error_t exclaves_driverkit_upcall_timer_set_timeout(
    const uint64_t id, const uint32_t timer_id,
  ...
  clock_interval_to_absolutetime_interval(duration->seconds, kSecondScale,
                                          &end);
  ADD_ABSOLUTETIME(&end, &nsecs);
  args.data.set_timeout_args.duration = end;
...
tb_error_t exclaves_driverkit_upcall_timer_cancel_timeout(
    const uint64_t id, const uint32_t timer_id,
    tb_error_t (^completion)(
...
tb_error_t exclaves_driverkit_upcall_lock_workloop(
    const uint64_t id,
    tb_error_t (^completion)(
  ...
  if (!IOExclaveLockWorkloop(id, true)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_driverupcallsprivate_lockworkloop__result_init_failure(&result,
                                                                        err);
  } else {
...
tb_error_t exclaves_driverkit_upcall_unlock_workloop(
    const uint64_t id,
    tb_error_t (^completion)(
        xnuupcallsv2_driverupcallsprivate_unlockworkloop__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls,
...
extern tb_error_t exclaves_driverkit_upcall_async_notification_signal(
    const uint64_t id, const uint32_t notificationID,
    tb_error_t (^completion)(
...
tb_error_t exclaves_driverkit_upcall_mapper_activate(
    const uint64_t id, const uint32_t mapperIndex,
    tb_error_t (^completion)(
...
tb_error_t exclaves_driverkit_upcall_mapper_deactivate(
    const uint64_t id, const uint32_t mapperIndex,
    tb_error_t (^completion)(
        xnuupcallsv2_driverupcallsprivate_mapperdeactivate__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls,
                        "[iokit_upcalls] mapper_deactivate from id %llu\n", id);
  struct IOExclaveMapperOperationUpcallArgs args;
  args.type = MapperDeactivate;
...
extern tb_error_t exclaves_driverkit_upcall_ane_setpowerstate(
    const uint64_t id, const uint32_t desiredState,
    tb_error_t (^completion)(
...
tb_error_t exclaves_driverkit_upcall_ane_worksubmit(
    const uint64_t id, const uint64_t requestID,
    const uint32_t taskDescriptorCount, const uint64_t submitTimestamp,
    tb_error_t (^completion)(
  ...
  if (!IOExclaveANEUpcallHandler(id, &args, &ret)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_aneupcallsprivate_aneworksubmit__result_init_failure(&result,
                                                                      err);
  } else {
    xnuupcallsv2_aneupcallsprivate_aneworksubmit__result_init_success(&result,
                                                                      ret);
  }
  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_ane_workbegin(
    const uint64_t id, const uint64_t requestID, const uint64_t beginTimestamp,
    tb_error_t (^completion)(
        xnuupcallsv2_aneupcallsprivate_aneworkbegin__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls,
                        "[iokit_upcalls] ane_workbegin from id %llu\n", id);
  struct IOExclaveANEUpcallArgs args;
  bool ret = false;
  ...
  if (!IOExclaveANEUpcallHandler(id, &args, &ret)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_aneupcallsprivate_aneworkbegin__result_init_failure(&result,
                                                                     err);
  } else {
    xnuupcallsv2_aneupcallsprivate_aneworkbegin__result_init_success(&result,
                                                                     ret);
  }
  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_ane_workend(
    const uint64_t id, const uint64_t requestID,
    tb_error_t (^completion)(
  ...
  if (!IOExclaveANEUpcallHandler(id, &args, &ret)) {
    xnuupcallsv2_driverupcallerror_s err = {
        .tag = XNUUPCALLSV2_DRIVERUPCALLERROR__FAILURE};
    xnuupcallsv2_aneupcallsprivate_aneworkend__result_init_failure(&result,
                                                                   err);
  } else {
    xnuupcallsv2_aneupcallsprivate_aneworkend__result_init_success(&result,
                                                                   ret);
  }
  return completion(result);
}

tb_error_t exclaves_driverkit_upcall_notification_signal(
    const uint64_t id, const uint32_t mask,
    tb_error_t (^completion)(
...
tb_error_t
exclaves_driverkit_upcall_lpw_createpowerassertion(tb_error_t (^completion)(
    xnuupcallsv2_lpwupcallsprivate_createpowerassertion__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls,
                        "[lpw_upcalls] createPowerAssertion\n");
...
tb_error_t exclaves_driverkit_upcall_lpw_releasepowerassertion(
    const uint64_t assertionID,
    tb_error_t (^completion)(
        xnuupcallsv2_lpwupcallsprivate_releasepowerassertion__result_s)) {
  exclaves_debug_printf(show_iokit_upcalls,
...
tb_error_t exclaves_driverkit_upcall_lpw_requestrunmode(
    const uint64_t runmode_mask,
    tb_error_t (^completion)(
  ...
  if (ret == kIOReturnSuccess) {
    xnuupcallsv2_lpwupcallsprivate_requestrunmode__result_init_success(&result);
  } else if (ret == kIOReturnBusy || ret == kIOReturnUnsupported) {
    xnuupcallsv2_lpwerror_s err;
    xnuupcallsv2_lpwerror_runmoderequestdenied__init(&err);
    xnuupcallsv2_lpwupcallsprivate_requestrunmode__result_init_failure(&result,
                                                                       err);
  } else {
    xnuupcallsv2_lpwerror_s err;
    xnuupcallsv2_lpwerror_internalerror__init(&err);
...
#define EXCLAVES_HELLO_DRIVER_INTERRUPTS_CHECK_RET(test)                       \
  if (test) {                                                                  \
    break;                                                                     \
  }
...
static const char *hello_driverkit_interrupts_test_string[] = {
    "IRQ_REGISTER",      "IRQ_REMOVE",         "IRQ_ENABLE",
    "IRQ_CHECK",         "IRQ_DISABLE",        "TIMER_REGISTER",
    "TIMER_REMOVE",      "TIMER_ENABLE",       "TIMER_DISABLE",
    "TIMER_SETTIMEOUT",  "TIMER_CHECK",        "TIMER_CANCELTIMEOUT",
    "MULTI_TIMER_SETUP", "MULTI_TIMER_CLEANUP"};
...
hello_driverkit_interrupts(hello_driverkit_interrupts_test_type_t test_type) {
  ...
  tb_endpoint_t ep = tb_endpoint_create_with_value(TB_TRANSPORT_TYPE_XNU, id,
  ...
  if (tb_err != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(
        show_test_output,
        "%s: Failed to initialize hellodriverinterrupts service\n", __func__);
    err = 2;
    goto out;
  }

  tb_err = hellodriverinterrupts_hellodriverinterrupts_hellointerrupt(
      &client, (uint8_t)test_type, ^(uint8_t result) {
        res = result;
      });
  if (tb_err != TB_ERROR_SUCCESS || res != 0) {
    exclaves_debug_printf(show_test_output,
                          "%s: Sending TB message hellointerrupt (type %d), "
...
static int exclaves_hello_driverkit_interrupts(uint64_t registryID,
                                               int64_t *out) {
  ...
  do {
    ...
    exclaves_debug_printf(show_test_output, "%s: TIMER TESTS\n", __func__);
...
static int exclaves_hello_driverkit_interrupts_test(int64_t in, int64_t *out) {
...
static int exclaves_hello_driverkit_multi_timers(int64_t *out) {
  if (exclaves_get_status() != EXCLAVES_STATUS_AVAILABLE) {
    exclaves_debug_printf(show_test_output,
                          "%s: SKIPPED: Exclaves not available\n", __func__);
    *out = -1;
...
static int exclaves_hello_driverkit_multi_timers_test(__unused int64_t in,
                                                      int64_t *out) {
...
kern_return_t exclaves_driver_service_lookup(const char *service_name,
                                             uint64_t *endpoint) {
  assert3p(endpoint, !=, NULL);
  static const char *domains[] = {EXCLAVES_DOMAIN_KERNEL,
                                  EXCLAVES_DOMAIN_DARWIN};

  for (size_t domainIndex = 0;
       domainIndex < sizeof(domains) / sizeof(domains[0]); domainIndex++) {
    uint64_t result =
        exclaves_service_lookup(domains[domainIndex], service_name);
