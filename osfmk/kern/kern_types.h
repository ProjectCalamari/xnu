 ...
 *    for the current process or thread (i.e. the loss of state would be only
 * visible to the current client).  Since the thread is exiting anyways, you're
 * willing to cut the operation short.  The system as a whole must be able to
 * cleanly deal with the interruption (i.e. remain in a consistent and
 * recoverable state).
 *
 * THREAD_ABORTSAFE:
 *    Wait will end if someone explicitly wakes up the thread, the wait timeout
 ...
 *    boundary.  Many syscalls will try to restart the operation they were
 * performing after the signal has been handled.
 *
 ...
 *    thread_interrupt_level(), will limit the conditions that will cause a
 * wake. This is useful for code that can't be interrupted to set before calling
 * code that doesn't know that.
 *
 * Thread termination vs safe abort:
 *
 *    Termination abort: thread_abort(), thread_terminate()
 *
 *    A termination abort is sticky.  Once a thread is marked for termination,
 * every THREAD_INTERRUPTIBLE wait will return immediately with
 * THREAD_INTERRUPTED until the thread successfully exits.
 *
 *    Safe abort: thread_abort_safely()
 *
 *    A safe abort is not sticky.  The current wait, (or the next wait if the
 * thread is not currently waiting) will be interrupted, but then the abort
 * condition is cleared. The next wait will sleep as normal. Safe aborts only
 * have a single effect.
 *
 *    The path back to the user/kernel boundary must not make any further
 * unbounded wait calls. The waiter should detect the THREAD_INTERRUPTED return
 * code from an ABORTSAFE wait and return an error code that causes its caller
 ...
 *    user/kernel boundary is reached.  For Mach, the error code is usually
 * KERN_ABORTED, for BSD it is EINTR.
 *
 *    Debuggers rely on the safe abort mechanism - a signaled thread must return
 * to the AST at the user/kernel boundary for the debugger to finish attaching.
 *
 *    No wait/block will ever disappear a thread out from under the waiter. The
 * block call will always either return or call the passed in continuation.
 */
...
#define THREAD_WAIT_NOREPORT                                                   \
  (THREAD_WAIT_NOREPORT_KERNEL | THREAD_WAIT_NOREPORT_USER)

typedef int wait_timeout_urgency_t;
#define TIMEOUT_URGENCY_SYS_NORMAL                                             \
  0x00 /* use default leeway thresholds for system */
#define TIMEOUT_URGENCY_SYS_CRITICAL                                           \
  0x01 /* use critical leeway thresholds for system */
#define TIMEOUT_URGENCY_SYS_BACKGROUND                                         \
  0x02 /* use background leeway thresholds for system */

#define TIMEOUT_URGENCY_USER_MASK                                              \
  0x10 /* mask to identify user timeout urgency classes */
#define TIMEOUT_URGENCY_USER_NORMAL                                            \
  0x10 /* use default leeway thresholds for user */
#define TIMEOUT_URGENCY_USER_CRITICAL                                          \
  0x11 /* use critical leeway thresholds for user */
#define TIMEOUT_URGENCY_USER_BACKGROUND                                        \
  0x12 /* use background leeway thresholds for user */
...
#define TIMEOUT_URGENCY_FIRST_AVAIL                                            \
  0x40 /* first available bit outside of urgency mask/leeway */
...
/*
 * n.b. this is defined in thread_call.h, but in the TIMEOUT_URGENCY flags
 * space: #define THREAD_CALL_CONTINUOUS	0x100
 */
...
typedef union sched_clutch_edge {
  struct {
    uint32_t
        ...
        /* boolean_t */ sce_steal_allowed : 1, _reserved : 30;
 ...
 */
__options_decl(
    cluster_shared_rsrc_type_t, uint32_t,
    {
