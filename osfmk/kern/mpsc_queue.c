...
__attribute__((noinline)) static mpsc_queue_chain_t
...
static void _mpsc_daemon_queue_init(mpsc_daemon_queue_t dq,
...
static void _mpsc_queue_thread_continue(void *param,
...
static void _mpsc_queue_thread_wakeup(mpsc_daemon_queue_t dq) {
  thread_wakeup_thread((event_t)dq, dq->mpd_thread);
}

static kern_return_t _mpsc_daemon_queue_init_with_thread(
    mpsc_daemon_queue_t dq, mpsc_daemon_invoke_fn_t invoke, int pri,
    const char *name, mpsc_daemon_queue_kind_t kind,
    mpsc_daemon_init_options_t flags) {
  kern_return_t kr;
  ...
  *dq = (struct mpsc_daemon_queue){
  ...
  if (kr == KERN_SUCCESS) {
    ...
    thread_start_in_assert_wait(dq->mpd_thread, assert_wait_queue(dq),
                                CAST_EVENT64_T(dq), THREAD_UNINT);
...
static void _mpsc_queue_thread_call_drain(thread_call_param_t arg0,
                                          thread_call_param_t arg1 __unused) {
  _mpsc_daemon_queue_drain((mpsc_daemon_queue_t)arg0, NULL);
}

static void _mpsc_queue_thread_call_wakeup(mpsc_daemon_queue_t dq) {
...
static void _mpsc_daemon_queue_nested_wakeup(mpsc_daemon_queue_t dq) {
...
static void _mpsc_daemon_queue_drain(mpsc_daemon_queue_t dq, thread_t self) {
...
again:
  ...
  st = os_atomic_xor(&dq->mpd_state,
                     MPSC_QUEUE_STATE_DRAINING | MPSC_QUEUE_STATE_WAKEUP,
                     dependency);
  ...
  if ((head = mpsc_queue_dequeue_batch(&dq->mpd_queue, &tail, dep))) {
    do {
      mpsc_queue_batch_foreach_safe(cur, head, tail) {
        os_atomic_store(&cur->mpqc_next, MPSC_QUEUE_NOTQUEUED_MARKER, relaxed);
        invoke(cur, dq);
      }
...
static void _mpsc_daemon_queue_wakeup(mpsc_daemon_queue_t dq) {
  switch (dq->mpd_kind) {
...
static void _mpsc_daemon_queue_enqueue(mpsc_daemon_queue_t dq,
...
void mpsc_daemon_queue_activate(mpsc_daemon_queue_t dq) {
  ...
  st =
      os_atomic_andnot_orig(&dq->mpd_state, MPSC_QUEUE_STATE_INACTIVE, relaxed);
...
void thread_deallocate_daemon_init(void) {
  ...
  kr = _mpsc_daemon_queue_init_with_thread(
      &thread_deferred_deallocation_queue, mpsc_daemon_queue_nested_invoke,
      MINPRI_KERNEL, "daemon.deferred-deallocation",
      MPSC_QUEUE_KIND_THREAD_CRITICAL, MPSC_DAEMON_INIT_NONE);
  if (kr != KERN_SUCCESS) {
...
void thread_deallocate_daemon_register_queue(mpsc_daemon_queue_t dq,
                                             mpsc_daemon_invoke_fn_t invoke) {
  mpsc_daemon_queue_init_with_target(
      dq, invoke, &thread_deferred_deallocation_queue, MPSC_DAEMON_INIT_NONE);
}
