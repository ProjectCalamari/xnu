...
__startup_func static void recount_startup(void) {
#if __AMP__
  ...
  for (unsigned int i = 0; i < cpu_count; i++) {
    ...
    uint8_t cluster_i =
        (type == CLUSTER_TYPE_P) ? RCT_CPU_PERFORMANCE : RCT_CPU_EFFICIENCY;
  ...
  for (unsigned int i = 0; i < RCT_TOPO_COUNT; i++) {
    if (_topo_allocates[i]) {
      ...
      _recount_usage_zones[i] = zone_create(
          usage_name, sizeof(struct recount_usage) * recount_topo_count(i), 0);

      const char *track_name = _track_zone_names[i];
      assert(track_name != NULL);
      _recount_track_zones[i] = zone_create(
          track_name, sizeof(struct recount_track) * recount_topo_count(i), 0);
    }
  }
...
OS_ALWAYS_INLINE
static inline uint64_t recount_timestamp_speculative(void) {
...
static struct recount_snap *recount_get_snap(processor_t processor) {
...
OS_ALWAYS_INLINE
static void _seqlock_shared_lock_slowpath(const uint32_t *lck, uint32_t gen) {
  disable_preemption();
  do {
...
OS_ALWAYS_INLINE
static uintptr_t _seqlock_shared_lock(const uint32_t *lck) {
...
OS_ALWAYS_INLINE
static bool _seqlock_shared_try_unlock(const uint32_t *lck,
                                       uintptr_t on_enter) {
  return os_atomic_load(lck, acquire) == on_enter;
}

OS_ALWAYS_INLINE
static void _seqlock_excl_lock_relaxed(uint32_t *lck) {
  __assert_only uintptr_t new = os_atomic_inc(lck, relaxed);
  assert3u((new & 1), ==, 1);
}

OS_ALWAYS_INLINE
static void _seqlock_excl_commit(void) { os_atomic_thread_fence(release); }

OS_ALWAYS_INLINE
static void _seqlock_excl_unlock_relaxed(uint32_t *lck) {
  __assert_only uint32_t new = os_atomic_inc(lck, relaxed);
  assert3u((new & 1), ==, 0);
}

OS_ALWAYS_INLINE
static struct recount_track *recount_update_start(struct recount_track *tracks,
                                                  recount_topo_t topo,
...
static void recount_update_commit(void) { _seqlock_excl_commit(); }

static void recount_update_end(struct recount_track *track) {
...
static bool recount_try_read_end(const struct recount_track *track,
                                 uintptr_t on_enter) {
  return _seqlock_shared_try_unlock(&track->rt_sync, on_enter);
}

static void recount_read_track(struct recount_usage *stats,
...
static void recount_metrics_add(struct recount_metrics *sum,
                                const struct recount_metrics *to_add) {
...
static void recount_usage_add(struct recount_usage *sum,
                              const struct recount_usage *to_add) {
...
OS_ALWAYS_INLINE
static inline void recount_usage_add_snap(struct recount_usage *usage,
                                          recount_level_t level,
...
static void recount_rollup(recount_plan_t plan,
                           const struct recount_track *tracks,
                           recount_topo_t to_topo,
...
// This function must be run when counters cannot increment for the track, like
// from the current thread.
static void recount_rollup_unsafe(recount_plan_t plan,
...
void recount_sum_and_isolate_cpu_kind(recount_plan_t plan,
                                      struct recount_track *tracks,
                                      recount_cpu_kind_t kind,
                                      struct recount_usage *sum,
                                      struct recount_usage *only_kind) {
...
static void recount_sum_usage(recount_plan_t plan,
                              const struct recount_usage *usages,
                              struct recount_usage *sum) {
...
void recount_sum_perf_levels(recount_plan_t plan, struct recount_track *tracks,
...
OS_ALWAYS_INLINE
static void recount_snap_diff(struct recount_snap *result,
                              const struct recount_snap *lhs,
                              const struct recount_snap *rhs) {
...
static void _fix_time_precision(struct recount_usage *usage) {
#if PRECISE_USER_KERNEL_TIME
#pragma unused(usage)
#else  // PRECISE_USER_KERNEL_TIME
...
void recount_current_thread_usage_perf_only(
    struct recount_usage *usage, struct recount_usage *usage_perf_only) {
...
void recount_current_thread_perf_level_usage(
    struct recount_usage *usage_levels) {
  assert(ml_get_interrupts_enabled() == FALSE);
...
static void _times_add_usage(struct recount_times_mach *times,
                             struct recount_usage *usage) {
...
static uint64_t _time_since_last_snapshot(void) {
  struct recount_snap *last = recount_get_snap(current_processor());
...
struct recount_times_mach recount_current_thread_times(void) {
  assert(ml_get_interrupts_enabled() == FALSE);
  struct recount_times_mach times = recount_thread_times(current_thread());
#if PRECISE_USER_KERNEL_TIME
...
void recount_thread_usage(thread_t thread, struct recount_usage *usage) {
...
void recount_work_interval_usage(struct work_interval *work_interval,
                                 struct recount_usage *usage) {
  recount_sum(&recount_work_interval_plan,
              work_interval_get_recount_tracks(work_interval), usage);
  _fix_time_precision(usage);
}
...
recount_work_interval_times(struct work_interval *work_interval) {
  ...
  for (size_t i = 0; i < topo_count; i++) {
    _times_add_usage(
        &times, &work_interval_get_recount_tracks(work_interval)[i].rt_usage);
  }
  return times;
}
...
uint64_t recount_work_interval_energy_nj(struct work_interval *work_interval) {
  ...
  uint64_t energy = 0;
  for (size_t i = 0; i < topo_count; i++) {
    energy += work_interval_get_recount_tracks(work_interval)[i]
                  .rt_usage.ru_energy_nj;
  }
  return energy;
#else // RECOUNT_ENERGY
...
void recount_current_task_usage_perf_only(
    struct recount_usage *usage, struct recount_usage *usage_perf_only) {
  task_t task = current_task();
  struct recount_track *tracks = task->tk_recount.rtk_lifetime;
  recount_sum_and_isolate_cpu_kind(&recount_task_plan, tracks,
                                   RCT_CPU_PERFORMANCE, usage, usage_perf_only);
  _fix_time_precision(usage);
  _fix_time_precision(usage_perf_only);
}

void recount_task_times_perf_only(struct task *task,
                                  struct recount_times_mach *sum,
                                  struct recount_times_mach *sum_perf_only) {
...
void recount_task_terminated_usage(task_t task, struct recount_usage *usage) {
...
void recount_task_terminated_usage_perf_only(task_t task,
                                             struct recount_usage *usage,
                                             struct recount_usage *perf_only) {
  recount_sum_usage_and_isolate_cpu_kind(&recount_task_terminated_plan,
                                         task->tk_recount.rtk_terminated,
                                         RCT_CPU_PERFORMANCE, usage, perf_only);
  _fix_time_precision(usage);
  _fix_time_precision(perf_only);
}

void recount_task_usage_perf_only(task_t task, struct recount_usage *sum,
                                  struct recount_usage *sum_perf_only) {
  recount_sum_and_isolate_cpu_kind(&recount_task_plan,
                                   task->tk_recount.rtk_lifetime,
                                   RCT_CPU_PERFORMANCE, sum, sum_perf_only);
  _fix_time_precision(sum);
  _fix_time_precision(sum_perf_only);
}

void recount_task_usage(task_t task, struct recount_usage *usage) {
...
void recount_coalition_usage_perf_only(struct recount_coalition *coal,
  ...
  recount_sum_usage_and_isolate_cpu_kind(&recount_coalition_plan,
                                         coal->rco_exited, RCT_CPU_PERFORMANCE,
                                         sum, sum_perf_only);
  _fix_time_precision(sum);
  _fix_time_precision(sum_perf_only);
}
...
OS_ALWAYS_INLINE
static void recount_absorb_snap(struct recount_snap *to_add, thread_t thread,
                                task_t task, processor_t processor,
                                recount_level_t level) {
  ...
  if (__probable(!was_idle)) {
    wi_tracks_array =
        work_interval_get_recount_tracks(thread->th_work_interval);
  }
  const bool absorb_work_interval = wi_tracks_array != NULL;

  struct recount_track *th_track = recount_update_start(
      thread->th_recount.rth_lifetime, recount_thread_plan.rpl_topo, processor);
  struct recount_track *wi_track = NULL;
  if (__improbable(absorb_work_interval)) {
    wi_track = recount_update_start(
        wi_tracks_array, recount_work_interval_plan.rpl_topo, processor);
  }
  struct recount_track *tk_track =
      was_idle ? NULL
               : recount_update_start(task->tk_recount.rtk_lifetime,
                                      recount_task_plan.rpl_topo, processor);
  struct recount_track *pr_track =
      was_idle
          ? NULL
          : recount_update_start(&processor->pr_recount.rpr_active,
...
void recount_switch_thread(struct recount_snap *cur, struct thread *off_thread,
...
void recount_add_energy(struct thread *off_thread, struct task *off_task,
  ...
  struct recount_track *wi_tracks_array =
      work_interval_get_recount_tracks(off_thread->th_work_interval);
  bool collect_work_interval_telemetry = wi_tracks_array != NULL;
  processor_t processor = current_processor();

  struct recount_track *th_track =
      recount_update_single_start(off_thread->th_recount.rth_lifetime,
                                  recount_thread_plan.rpl_topo, processor);
  struct recount_track *wi_track =
      (was_idle || !collect_work_interval_telemetry)
          ? NULL
  ...
  struct recount_track *pr_track =
      was_idle ? NULL
               : recount_update_single_start(&processor->pr_recount.rpr_active,
                                             recount_processor_plan.rpl_topo,
                                             processor);
...
static void recount_precise_transition_diff(struct recount_snap *diff,
                                            struct recount_snap *last,
                                            struct recount_snap *cur) {
#if PRECISE_USER_KERNEL_PMCS
...
PRECISE_TIME_ONLY_FUNC
static void recount_assert_level(thread_t thread, recount_level_t old) {
...
PRECISE_TIME_ONLY_FUNC
static void recount_assert_level(thread_t __unused thread,
                                 recount_level_t __unused old) {}

#endif // !(MACH_ASSERT && RECOUNT_THREAD_BASED_LEVEL)

/// Called when entering or exiting the kernel to maintain system vs. user
/// counts, extremely performance sensitive.
///
...
OS_ALWAYS_INLINE
static uint64_t recount_transition(recount_level_t from, recount_level_t to) {
#if PRECISE_USER_KERNEL_TIME
...
void recount_leave_interrupt(void) {
  ...
  uint64_t since =
      now - processor->pr_recount.rpr_last_interrupt_enter_time_mach;
...
void recount_enter_intel_interrupt(x86_saved_state_t *state) {
  ...
  bool from_user =
      (is_saved_state64(state) ? state->ss_64.isf.cs : state->ss_32.cs) & 0x03;
...
void recount_processor_idle(struct recount_processor *pr,
                            struct recount_snap *snap) {
  __assert_only uint64_t state_time =
      os_atomic_load_wide(&pr->rpr_state_last_abs_time, relaxed);
  ...
  os_atomic_store_wide(&pr->rpr_state_last_abs_time, new_state_stamp, relaxed);
}

OS_PURE OS_ALWAYS_INLINE static inline uint64_t
_state_time(uint64_t state_stamp) {
  return state_stamp & ~(RCT_PR_IDLING);
}

void recount_processor_init(processor_t processor) {
#if __AMP__
...
void recount_processor_online(processor_t processor, struct recount_snap *cur) {
...
struct recount_track *recount_tracks_create(recount_plan_t plan) {
  ...
  return zalloc_flags(
      _recount_track_zones[plan->rpl_topo],
      Z_VM_TAG(Z_WAITOK | Z_ZERO | Z_NOFAIL, VM_KERN_MEMORY_RECOUNT));
}

static void recount_tracks_copy(recount_plan_t plan, struct recount_track *dst,
...
void recount_task_copy(struct recount_task *dst,
  ...
  recount_tracks_copy(&recount_task_plan, dst->rtk_lifetime, src->rtk_lifetime);
}
...
void recount_task_init(struct recount_task *tk) {
  ...
  tk->rtk_terminated =
      recount_usage_alloc(recount_task_terminated_plan.rpl_topo);
}

void recount_task_deinit(struct recount_task *tk) {
  recount_tracks_destroy(&recount_task_plan, tk->rtk_lifetime);
  recount_usage_free(recount_task_terminated_plan.rpl_topo, tk->rtk_terminated);
}
