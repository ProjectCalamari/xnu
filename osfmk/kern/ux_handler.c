...
IPC_KOBJECT_DEFINE(IKOT_UX_HANDLER, .iko_op_movable_send = true,
...
void ux_handler_init(void) {
  ux_handler_port = ipc_kobject_alloc_port(&ux_handler_kobject, IKOT_UX_HANDLER,
 ...
 */
void ux_handler_setup(void) {
  ipc_port_t ux_handler_send_right;
  kern_return_t kr = KERN_SUCCESS;

  ux_handler_send_right = ipc_kobject_make_send(
      ux_handler_port, &ux_handler_kobject, IKOT_UX_HANDLER);
   ...
   */
  kr = host_set_exception_ports(
      host_priv_self(), EXC_MASK_ALL & ~(EXC_MASK_RPC_ALERT | EXC_MASK_GUARD),
      ux_handler_send_right, EXCEPTION_DEFAULT | MACH_EXCEPTION_CODES, 0);
 ...
 */
boolean_t is_ux_handler_port(mach_port_t port) {
...
catch_mach_exception_raise(mach_port_t exception_port,
  ...
  if ((target_thread = convert_port_to_thread_read(thread_port)) ==
      THREAD_NULL) {
...
kern_return_t catch_exception_raise(mach_port_t exception_port,
  ...
  return catch_mach_exception_raise(exception_port, thread, task, exception,
                                    big_code, codeCnt);
}

kern_return_t catch_exception_raise_state(
    __unused mach_port_t exception_port, __unused exception_type_t exception,
    __unused const exception_data_t code,
    __unused mach_msg_type_number_t codeCnt, __unused int *flavor,
...
kern_return_t catch_exception_raise_state_identity(
    ...
    __unused mach_port_t task, __unused exception_type_t exception,
    __unused exception_data_t code, __unused mach_msg_type_number_t codeCnt,
...
kern_return_t catch_mach_exception_raise_identity_protected(
    __unused mach_port_t exception_port, __unused uint64_t thread_id,
    __unused mach_port_t task_id_token, __unused exception_type_t exception,
    __unused mach_exception_data_t code,
    __unused mach_msg_type_number_t codeCnt) {
  return KERN_INVALID_ARGUMENT;
}
...
kern_return_t catch_mach_exception_raise_backtrace(
    __unused mach_port_t exception_port, __unused mach_port_t kcdata_object,
