...
static inline void iotrace(iotrace_type_e type, uint64_t vaddr, uint64_t paddr,
                           int size, uint64_t val, uint64_t sabs,
                           uint64_t duration) {
  uint32_t nextidx;
  ...
  *nextidxp = ((nextidx + 1) >= iotrace_entries_per_cpu) ? 0 : (nextidx + 1);
  ...
  (void)OSBacktrace(cur_iotrace_ring[nextidx].backtrace, MAX_IOTRACE_BTFRAMES);
}

static inline void iotrace_disable(void) { mmiotrace_enabled = 0; }
