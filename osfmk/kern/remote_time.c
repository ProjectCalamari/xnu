 ...
 */
void mach_bridge_add_timestamp(uint64_t remote_timestamp,
 ...
 * These variables should be exported by the platform specific time receiving
 * code.
 */
...
static inline struct bt_params *bt_params_find(uint64_t local_ts) {
  lck_spin_assert(&bt_ts_conversion_lock, LCK_ASSERT_OWNED);
...
static inline struct bt_params bt_params_get_latest_locked(void) {
  lck_spin_assert(&bt_ts_conversion_lock, LCK_ASSERT_OWNED);
...
void mach_bridge_add_timestamp(uint64_t remote_timestamp,
  ...
  /* sleep/wake might return the same mach_absolute_time as the previous
   * timestamp pair */
...
static double mach_bridge_compute_rate(uint64_t new_local_ts,
                                       uint64_t new_remote_ts,
                                       uint64_t old_local_ts,
...
static void bt_calibration_thread(void) {
  static uint64_t prev_local_ts = 0, prev_remote_ts = 0, curr_local_ts = 0,
                  curr_remote_ts = 0;
   ...
   * is predicted using the previous values of bt_params. After computing the
   * new bt_params.rate, bt_params.base_remote_time is set to this predicted
   * value and bt_params.base_local_time is set to the current local timestamp.
   */
recalculate:
  assertf(recalculate_count <= MAX_RECALCULATE_COUNT,
          "bt_caliberation_thread: recalculate \
					invocation exceeds MAX_RECALCULATE_COUNT");

  if ((received_remote_timestamp == BT_RESET_SENTINEL_TS) ||
      (received_remote_timestamp == BT_WAKE_SENTINEL_TS)) {
    KDBG(MACHDBG_CODE(DBG_MACH_CLOCK, MACH_BRIDGE_RESET_TS),
         received_local_timestamp, received_remote_timestamp, 1);
    reset = true;
    ...
    /* If the local timestamp isn't accurately captured, the received value will
     * be ignored */
    skip_rcv_ts = true;
    goto block;
  }
  ...
  if (!(reset || sleep) &&
      ((local_diff < MIN_LOCAL_TS_DISTANCE_NS) ||
       (!skip_rcv_ts && (local_diff > MAX_LOCAL_TS_DISTANCE_NS)))) {
    /* Skip the current timestamp */
    KDBG(MACHDBG_CODE(DBG_MACH_CLOCK, MACH_BRIDGE_SKIP_TS), curr_local_ts,
         curr_remote_ts, prev_received_local_ts);
    goto block;
  } else {
    skip_rcv_ts = false;
  ...
  if (reset) {
    if (skip_reset_count > 0) {
      KDBG(MACHDBG_CODE(DBG_MACH_CLOCK, MACH_BRIDGE_SKIP_TS), curr_local_ts,
           curr_remote_ts, prev_local_ts, skip_reset_count);
      skip_reset_count--;
      goto skip_reset;
    }
    ...
    reset = false;
    KDBG(MACHDBG_CODE(DBG_MACH_CLOCK, MACH_BRIDGE_RESET_TS), curr_local_ts,
         curr_remote_ts, 2);
  } else if (sleep) {
    ...
    /* Check if the predicted remote timestamp is within the expected current
     * remote timestamp range */
    uint64_t pred_remote_ts =
        mach_bridge_compute_timestamp(curr_local_ts, &bt_params_snapshot);
    uint64_t diff = 0;
    if (initial_sample_count >= max_initial_sample_count) {
      ...
      if (diff > TS_PAIR_MISMATCH_THRESHOLD_NS) {
        ts_pair_mismatch++;
        KDBG(MACHDBG_CODE(DBG_MACH_CLOCK, MACH_BRIDGE_TS_MISMATCH),
             curr_local_ts, curr_remote_ts, pred_remote_ts, ts_pair_mismatch);
      } else {
        ts_pair_mismatch = 0;
      }
    ...
    observed_rate = mach_bridge_compute_rate(curr_local_ts, curr_remote_ts,
                                             prev_local_ts, prev_remote_ts);
    /* Log bad observed rates and skip the timestamp pair */
    if ((observed_rate < MIN_OBSERVED_RATE) ||
        (observed_rate > MAX_OBSERVED_RATE)) {
      KDBG(MACHDBG_CODE(DBG_MACH_CLOCK, MACH_BRIDGE_OBSV_RATE),
    ...
    if (initial_sample_count <= MIN_INITIAL_SAMPLE_COUNT) {
      ...
      cumulative_rate = cumulative_rate + (observed_rate - cumulative_rate) /
                                              initial_sample_count;
    } else {
      if (initial_sample_count < max_initial_sample_count) {
        initial_sample_count++;
      }
      cumulative_rate = cumulative_rate + CUMULATIVE_RATE_DECAY_CONSTANT *
                                              (observed_rate - cumulative_rate);
    }
    rate_from_base = mach_bridge_compute_rate(
        curr_local_ts, curr_remote_ts, bt_params_snapshot.base_local_ts,
        bt_params_snapshot.base_remote_ts);
    new_rate = CUMULATIVE_RATE_WEIGHT * cumulative_rate +
               (1 - CUMULATIVE_RATE_WEIGHT) * rate_from_base;
    /*
    ...
    lck_spin_lock(&bt_ts_conversion_lock);
    absolutetime_to_nanoseconds(mach_absolute_time(), &bt_params.base_local_ts);
    bt_params.base_remote_ts = mach_bridge_compute_timestamp(
        bt_params.base_local_ts, &bt_params_snapshot);
    bt_params.rate = new_rate;
  }
  bt_params_add(&bt_params);
  commpage_set_remotetime_params(bt_params.rate, bt_params.base_local_ts,
                                 bt_params.base_remote_ts);
  KDBG(MACHDBG_CODE(DBG_MACH_CLOCK, MACH_BRIDGE_TS_PARAMS),
       bt_params.base_local_ts, bt_params.base_remote_ts,
       *(uint64_t *)((void *)&bt_params.rate));
...
void bt_calibration_thread_start(void) {
  ...
  kern_return_t result = kernel_thread_start_priority(
      (thread_continue_t)bt_calibration_thread, NULL, BASEPRI_KERNEL, &thread);
 ...
 */
uint64_t mach_bridge_remote_time(uint64_t local_timestamp) {
...
#error "You cannot define both sides of the bridge!"
  ...
  if (local_timestamp < now) {
    ...
    remote_timestamp =
        mach_bridge_compute_timestamp(local_timestamp_ns, params);
  }
#else
...
#endif /* defined(XNU_TARGET_OS_BRIDGE) */
  lck_spin_unlock(&bt_ts_conversion_lock);
  KDBG(MACHDBG_CODE(DBG_MACH_CLOCK, MACH_BRIDGE_REMOTE_TIME), local_timestamp,
