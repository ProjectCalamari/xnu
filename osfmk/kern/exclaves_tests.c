...
static int exclaves_hello_exclave_test(__unused int64_t in, int64_t *out) {
  ...
  tb_result =
      exclaveschelloserver_tests_default_hello(&client, ^(uint64_t result) {
        assert3u(tb_result, ==, TB_ERROR_SUCCESS);
...
static int exclaves_panic_exclave_test(__unused int64_t in, int64_t *out) {
  tb_error_t tb_result;
  exclaveschelloserver_tests_s client;
...
static int exclaves_sensor_kpi_test(int64_t in, int64_t *out) {
#pragma unused(in)
#define SENSOR_TEST(x)                                                         \
  if (!(x)) {                                                                  \
    exclaves_debug_printf(show_errors, "%s: FAILURE -- %s:%d\n", __func__,     \
                          __FILE__, __LINE__);                                 \
    success = false;                                                           \
    goto out;                                                                  \
  }
  ...
  for (unsigned i = 0; i < num_sensors; i++) {
    ...
    SENSOR_TEST(sensor_status ==
                EXCLAVES_SENSOR_STATUS_ALLOWED); /* Not enforced */
  }
...
SYSCTL_TEST_REGISTER(exclaves_sensor_kpi_test, exclaves_sensor_kpi_test);

static int exclaves_check_mem_usage_test(__unused int64_t in, int64_t *out) {
  if (exclaves_get_status() != EXCLAVES_STATUS_AVAILABLE) {
    exclaves_debug_printf(show_test_output,
                          "%s: SKIPPED: Exclaves not available\n", __func__);
    *out = -1;
  ...
  if (r == KERN_FAILURE) {
    exclaves_debug_printf(
        show_errors, "Exclave Check Memory Usage failed: Kernel Failure\n");
    return 0;
  }
...
SYSCTL_TEST_REGISTER(exclaves_check_mem_usage_test,
                     exclaves_check_mem_usage_test);
