...
static TUNABLE(int, turnstile_max_hop, "turnstile_max_hop",
...
static const turnstile_promote_policy_t
    turnstile_promote_policy[TURNSTILE_TOTAL_TYPES] = {
...
static const turnstile_hash_lock_policy_t
    turnstile_hash_lock_policy[TURNSTILE_TOTAL_TYPES] = {
        ...
        [TURNSTILE_SLEEP_INHERITOR] =
            (TURNSTILE_IRQ_UNSAFE_HASH | TURNSTILE_LOCKED_HASH),
...
static struct turnstile_stats turnstile_boost_stats[TURNSTILE_MAX_HOP_DEFAULT] =
    {};
static struct turnstile_stats
    turnstile_unboost_stats[TURNSTILE_MAX_HOP_DEFAULT] = {};
...
static turnstile_type_t turnstile_get_type(struct turnstile *turnstile);
static bool turnstile_is_send_turnstile(struct turnstile *turnstile);
static uint32_t turnstile_get_gencount(struct turnstile *turnstile);
static void
turnstile_set_type_and_increment_gencount(struct turnstile *turnstile,
                                          turnstile_type_t type);
static void turnstile_destroy(struct turnstile *turnstile);
static void
turnstile_update_inheritor_workq_priority_chain(struct turnstile *in_turnstile,
                                                spl_t s);
static void turnstile_update_inheritor_thread_priority_chain(
    struct turnstile **in_turnstile, thread_t *out_thread, int total_hop,
    turnstile_stats_update_flags_t tsu_flags);
static void turnstile_update_inheritor_turnstile_priority_chain(
    struct turnstile **in_out_turnstile, int total_hop,
    turnstile_stats_update_flags_t tsu_flags);
static void thread_update_waiting_turnstile_priority_chain(
    thread_t *in_thread, struct turnstile **out_turnstile, int thread_hop,
...
turnstile_need_thread_promotion_update(struct turnstile *dst_turnstile,
                                       thread_t thread);
static boolean_t thread_add_turnstile_promotion(thread_t thread,
                                                struct turnstile *turnstile);
static boolean_t thread_remove_turnstile_promotion(thread_t thread,
                                                   struct turnstile *turnstile);
static boolean_t
thread_needs_turnstile_promotion_update(thread_t thread,
                                        struct turnstile *turnstile);
static boolean_t thread_update_turnstile_promotion(thread_t thread,
                                                   struct turnstile *turnstile);
static boolean_t
thread_update_turnstile_promotion_locked(thread_t thread,
                                         struct turnstile *turnstile);
static boolean_t workq_add_turnstile_promotion(struct workqueue *wq_inheritor,
                                               struct turnstile *turnstile);
...
static int turnstile_compute_thread_push(struct turnstile *turnstile,
                                         thread_t thread);
...
static void tstile_test_prim_init(struct tstile_test_prim *test_prim_ptr);
#endif

union turnstile_type_gencount {
  uint32_t value;
  struct {
    uint32_t ts_type : (8 * sizeof(turnstile_type_t)), ts_gencount
...
static turnstile_type_t turnstile_get_type(struct turnstile *turnstile) {
  ...
  type_and_gencount.value =
      atomic_load_explicit(&turnstile->ts_type_gencount, memory_order_relaxed);
...
static bool turnstile_is_send_turnstile(struct turnstile *turnstile) {
  if (not_in_kdp) {
    panic("turnstile_is_send_turnstile() called outside of kernel debugger "
          "context");
  }
...
/* Only safe to be called from stackshot context */
static bool turnstile_is_receive_turnstile(struct turnstile *turnstile) {
  if (not_in_kdp) {
    panic("turnstile_is_receive_turnstile() called outside of kernel debugger "
          "context");
  }
...
turnstile_set_type_and_increment_gencount(struct turnstile *turnstile,
  ...
  /* No need to compare exchange since the store happens under interlock of the
   * primitive */
  type_and_gencount.value =
      atomic_load_explicit(&turnstile->ts_type_gencount, memory_order_relaxed);
  type_and_gencount.ts_type = type;
  type_and_gencount.ts_gencount++;
  atomic_store_explicit(&turnstile->ts_type_gencount, type_and_gencount.value,
...
SECURITY_READ_ONLY_LATE(static struct turnstile_htable_bucket *)
...
#define turnstile_bucket_lock_init(bucket)                                     \
  lck_spin_init(&bucket->ts_ht_bucket_lock, &turnstiles_htable_lock_grp,       \
                LCK_ATTR_NULL)
 ...
 *   None
 */
static void turnstiles_hashtable_init(void) {
  /* Initialize number of buckets in the hashtable */
  if (PE_parse_boot_argn("ts_htable_buckets", &ts_htable_buckets,
                         sizeof(ts_htable_buckets)) != TRUE) {
    ts_htable_buckets = TURNSTILE_HTABLE_BUCKETS_DEFAULT;
  }

  assert(ts_htable_buckets <= TURNSTILE_HTABLE_BUCKETS_MAX);
  uint32_t ts_htable_size =
      ts_htable_buckets * sizeof(struct turnstile_htable_bucket);
 ...
 */
static inline boolean_t turnstile_freelist_empty(struct turnstile *ts) {
 ...
 *   None
 */
static void turnstile_freelist_insert(struct turnstile *dst_ts,
 ...
 */
static struct turnstile *turnstile_freelist_remove(struct turnstile *ts) {
 ...
 */
static inline uint32_t turnstile_hash(uintptr_t proprietor) {
 ...
 *              If an entry is present, add the new turnstile to the entry's
 * freelist. Otherwise add the passed in turnstile for that proprietor. The
 * routine assumes that the turnstile->proprietor does not change while the
 * turnstile is in the global hash table.
 *
 * Args:
 *   Arg1: proprietor
...
turnstile_htable_lookup_add(uintptr_t proprietor,
  ...
  SLIST_FOREACH(ts, &ts_bucket->ts_ht_bucket_list, ts_htable_link) {
    if (ts->ts_proprietor == proprietor) {
      /*
       * Found an entry in the hashtable for this proprietor; add thread
       * turnstile to freelist and return this turnstile
       */
      if (needs_lock) {
        turnstile_bucket_unlock(ts_bucket);
        if (irq_safe) {
  ...
  SLIST_INSERT_HEAD(&ts_bucket->ts_ht_bucket_list, new_turnstile,
                    ts_htable_link);
  ...
  /* Since there was no previous entry for this proprietor, return
   * TURNSTILE_NULL */
 ...
 *
 * Description: Lookup the proprietor in the global turnstile hash table.
 *              For the turnstile in the hash table, if the freelist has
 * turnstiles on it return one of them from the freelist. Otherwise remove the
 * turnstile from the hashtable and return that. The routine assumes that the
 * turnstile->proprietor does not change while the turnstile is in the global
 * hash table.
 *
 * Args:
 *   Arg1: proprietor
 ...
 */
static struct turnstile *
turnstable_htable_lookup_remove(uintptr_t proprietor,
  ...
  SLIST_FOREACH_PREVPTR(ts, prev_tslink, &ts_bucket->ts_ht_bucket_list,
                        ts_htable_link) {
    if (ts->ts_proprietor == proprietor) {
      ret_turnstile = ts;
      break;
  ...
  if (turnstile_freelist_empty(ret_turnstile)) {
    /* No turnstiles on the freelist; remove the turnstile from the hashtable
     * and mark it freed */
 ...
 *
 * Description: Lookup the proprietor in the global turnstile hash table.
 *              The routine assumes that the turnstile->proprietor does not
 * change while the turnstile is in the global hash table.
 *
 * Args:
 *   Arg1: proprietor
 ...
 */
static struct turnstile *turnstile_htable_lookup(uintptr_t proprietor,
                                                 turnstile_type_t type) {
  uint32_t index = turnstile_hash(proprietor);
  assert(index < ts_htable_buckets);
...
static uint32_t ctsid_mangle(compact_id_t cid) {
  return (cid == ctsid_nonce ? CTSID_MASK : cid) ^ ctsid_nonce;
}

static compact_id_t ctsid_unmangle(uint32_t ctsid) {
  ctsid ^= ctsid_nonce;
...
void turnstile_compact_id_put(uint32_t ctsid) {
  assert3u(ctsid, !=, 0);
 ...
 * primitive.
 */
static struct turnstile *turnstile_prepare_common(uintptr_t proprietor,
                                                  thread_t thread,
...
struct turnstile *turnstile_prepare(uintptr_t proprietor,
  ...
  thread_turnstile =
      turnstile_prepare_common(proprietor, thread, turnstile, type);

  /*
   * If the primitive stores the turnstile,
   * If there is already a turnstile, put the thread_turnstile if the primitive
   * currently does not have a turnstile. Else, add the thread turnstile to
   * freelist of the primitive turnstile.
   */
  ...
  if (*tstore == TURNSTILE_NULL) {
    ...
    *tstore = thread_turnstile;
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE,
        (TURNSTILE_CODE(TURNSTILE_FREELIST_OPERATIONS, (TURNSTILE_PREPARE))) |
            DBG_FUNC_NONE,
...
struct turnstile *turnstile_prepare_compact_id(uintptr_t proprietor,
  ...
  thread_turnstile =
      turnstile_prepare_common(proprietor, thread, TURNSTILE_NULL, type);
...
struct turnstile *turnstile_prepare_hash(uintptr_t proprietor,
  ...
  /*
   * Lookup the primitive in the turnstile hash table and see if it already has
   * an entry.
   */
  ret_turnstile =
      turnstile_htable_lookup_add(proprietor, thread_turnstile, type);
 ...
 * current thread. Current thread will have a turnstile attached to it after
 * this call.
 */
static void turnstile_complete_common(uintptr_t proprietor __kdebug_only,
                                      thread_t thread,
                                      struct turnstile *thread_turnstile,
  ...
  if (primitive_turnstile == NULL) {
    ...
    if (thread_turnstile->ts_inheritor != TURNSTILE_INHERITOR_NULL) {
      turnstile_update_inheritor(
          thread_turnstile, TURNSTILE_INHERITOR_NULL,
    ...
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE,
        (TURNSTILE_CODE(TURNSTILE_FREELIST_OPERATIONS, (TURNSTILE_COMPLETE))) |
            DBG_FUNC_NONE,
        VM_KERNEL_UNSLIDE_OR_PERM(thread_turnstile),
        VM_KERNEL_UNSLIDE_OR_PERM(proprietor),
        turnstile_get_type(thread_turnstile), 0, 0);
  } else {
    /* If primitive's turnstile needs priority update, set it up for turnstile
     * cleanup */
...
void turnstile_complete_hash(uintptr_t proprietor, turnstile_type_t type) {
  ...
  primitive_turnstile =
      turnstable_htable_lookup_remove(proprietor, &thread_turnstile, type);
...
void turnstile_kernel_update_inheritor_on_wake_locked(
  ...
  assert(policy == TURNSTILE_KERNEL_PROMOTE ||
         policy == TURNSTILE_USER_PROMOTE);

  turnstile_stash_inheritor((thread_t)new_inheritor,
...
void turnstile_update_inheritor_locked(struct turnstile *turnstile) {
  ...
  case TURNSTILE_KERNEL_PROMOTE:
    ...
    break;
  default:
    panic("turnstile promotion for type %d not yet implemented",
  ...
  if (old_inheritor == new_inheritor) {
    if (new_inheritor_flags & TURNSTILE_INHERITOR_THREAD) {
      ...
      new_inheritor_needs_update =
          thread_update_turnstile_promotion(thread_inheritor, turnstile);
      ...
      new_inheritor_needs_update =
          turnstile_update_turnstile_promotion(inheritor_turnstile, turnstile);
      ...
      turnstile_stats_update(
          1, TSU_NO_PRI_CHANGE_NEEDED | TSU_TURNSTILE_ARG | TSU_BOOST_ARG,
          turnstile);
    } else {
    ...
    if (!new_inheritor_needs_update) {
      turnstile_stats_update(1,
                             TSU_PRI_PROPAGATION | TSU_TURNSTILE_ARG |
                                 TSU_BOOST_ARG | tsu_flags,
                             turnstile);
    }
    goto done;
  }
  ...
  if (old_inheritor != NULL) {
    if (old_inheritor_flags & TURNSTILE_INHERITOR_THREAD) {
      ...
      old_inheritor_needs_update =
          thread_remove_turnstile_promotion(thread_inheritor, turnstile);
    } else if (old_inheritor_flags & TURNSTILE_INHERITOR_TURNSTILE) {
      struct turnstile *old_turnstile = old_inheritor;

      old_inheritor_needs_update =
          turnstile_remove_turnstile_promotion(old_turnstile, turnstile);
       ...
       */
      turnstile_stats_update(1, TSU_NO_PRI_CHANGE_NEEDED | TSU_TURNSTILE_ARG,
                             turnstile);
    } else {
      panic("Inheritor flags lost along the way");
    }
  ...
  if (new_inheritor != NULL) {
    if (new_inheritor_flags & TURNSTILE_INHERITOR_THREAD) {
      ...
      new_inheritor_needs_update =
          thread_add_turnstile_promotion(thread_inheritor, turnstile);
    } else if (new_inheritor_flags & TURNSTILE_INHERITOR_TURNSTILE) {
      struct turnstile *new_turnstile = new_inheritor;

      new_inheritor_needs_update =
          turnstile_add_turnstile_promotion(new_turnstile, turnstile);
    } else if (new_inheritor_flags & TURNSTILE_INHERITOR_WORKQ) {
      struct workqueue *wq_inheritor = new_inheritor;

      new_inheritor_needs_update =
          workq_add_turnstile_promotion(wq_inheritor, turnstile);
      if (!new_inheritor_needs_update) {
        turnstile_stats_update(
            1, TSU_NO_PRI_CHANGE_NEEDED | TSU_TURNSTILE_ARG | TSU_BOOST_ARG,
            turnstile);
  ...
  /*
   * If new inheritor needs priority updated, then set
   * TURNSTILE_NEEDS_PRI_UPDATE on the old_inheritor_flags which will be copied
   * to the thread.
   */
  if (new_inheritor_needs_update) {
 ...
 *              current thread. It will take a thread reference on the
 * inheritor. Called with the interlock of the primitive held.
 *
 * Args:
 *   Arg1: inheritor
 *   Arg2: flags
 ...
 *
 * Description: Check if thread's place in the turnstile waitq needs to be
 * updated.
 *
 * Arg1: dst turnstile
 * Arg2: thread
 *
 * Returns: TRUE: if turnstile_update_thread_promotion_locked needs to be
...
turnstile_need_thread_promotion_update(struct turnstile *dst_turnstile,
  ...
  thread_link_priority = priority_queue_entry_sched_pri(
      &dst_turnstile->ts_waitq.waitq_prio_queue, &thread->wait_prioq_links);
...
turnstile_priority_queue_update_entry_key(struct priority_queue_sched_max *q,
                                          priority_queue_entry_sched_t elt,
 ...
 * Arg2: thread
 *
 * Returns: TRUE: if the dst turnstile priority has changed and needs
 * propagation. FALSE: if the dst turnstile priority did not change or it does
 * not need propagation.
 *
  ...
  if (priority != thread_link_priority) {
    KERNEL_DEBUG_CONSTANT_IST(
        ...
        (TURNSTILE_CODE(TURNSTILE_HEAP_OPERATIONS,
            ...
            DBG_FUNC_NONE,
        VM_KERNEL_UNSLIDE_OR_PERM(dst_turnstile), thread_tid(thread), priority,
        thread_link_priority, 0);
  }

  if (!turnstile_priority_queue_update_entry_key(
          &dst_turnstile->ts_waitq.waitq_prio_queue, &thread->wait_prioq_links,
          (priority_queue_key_t)priority)) {
    return FALSE;
  }
 ...
 *
 * Description: Add a turnstile to thread's inheritor list and update thread's
 * priority.
 *
 * Arg1: thread
 * Arg2: turnstile
 ...
 *          FALSE: if the thread's priority did not change or it does not need
 * propagation.
 *
 ...
 *
 * Description: Remove turnstile from thread's inheritor list and update
 * thread's priority.
 *
 * Arg1: thread
 * Arg2: turnstile
 ...
 */
static boolean_t
thread_remove_turnstile_promotion(thread_t thread,
                                  struct turnstile *turnstile) {
  boolean_t needs_update = FALSE;
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      (TURNSTILE_CODE(TURNSTILE_HEAP_OPERATIONS,
          ...
          DBG_FUNC_NONE,
      thread_tid(thread), VM_KERNEL_UNSLIDE_OR_PERM(turnstile), 0, 0, 0);
 ...
 *
 * Description: Check if turnstile position in thread's inheritor list needs to
 * be updated.
 *
 * Arg1: thread
 * Arg2: turnstile
 ...
 */
static boolean_t
thread_needs_turnstile_promotion_update(thread_t thread __assert_only,
                                        struct turnstile *turnstile) {
  boolean_t needs_update = FALSE;
  ...
  case TURNSTILE_USER_IPC_PROMOTE:
    turnstile_link_priority = priority_queue_entry_sched_pri(
        &thread->base_inheritor_queue, &turnstile->ts_inheritor_links);
    break;
  case TURNSTILE_KERNEL_PROMOTE:
    turnstile_link_priority = priority_queue_entry_sched_pri(
        &thread->sched_inheritor_queue, &turnstile->ts_inheritor_links);
    break;
  default:
    panic("turnstile promotion for type %d not yet implemented",
          turnstile_get_type(turnstile));
  ...
  needs_update =
      (turnstile_link_priority == turnstile->ts_priority) ? FALSE : TRUE;
  return needs_update;
}
 ...
 *
 * Description: Update turnstile position in thread's inheritor list and update
 * thread's priority.
 *
 * Arg1: thread
 * Arg2: turnstile
  ...
  if (turnstile->ts_priority != turnstile_link_priority) {
    KERNEL_DEBUG_CONSTANT_IST(
        ...
        (TURNSTILE_CODE(TURNSTILE_HEAP_OPERATIONS,
            ...
            DBG_FUNC_NONE,
        thread_tid(thread), VM_KERNEL_UNSLIDE_OR_PERM(turnstile),
        turnstile->ts_priority, turnstile_link_priority, 0);
  }
 ...
 */
static boolean_t
thread_update_turnstile_promotion(thread_t thread,
                                  struct turnstile *turnstile) {
  /* Before grabbing the thread lock, check if update is needed */
  boolean_t needs_update =
      thread_needs_turnstile_promotion_update(thread, turnstile);
...
int thread_get_inheritor_turnstile_sched_priority(thread_t thread) {
  ...
  if (max_turnstile) {
    return priority_queue_entry_sched_pri(&thread->sched_inheritor_queue,
...
int thread_get_inheritor_turnstile_base_priority(thread_t thread) {
  ...
  if (max_turnstile) {
    return priority_queue_entry_sched_pri(&thread->base_inheritor_queue,
                                          &max_turnstile->ts_inheritor_links);
  }
 ...
 * Arg1: turnstile
 *
 * Returns: TSU_ABOVE_UI_PRI_CHANGE: if turnstile priority is above 47 and it is
 * not an ulock. TSU_FLAGS_NONE: otherwise.
 *
 * Condition: turnstile locked.
 */
static turnstile_stats_update_flags_t
turnstile_get_update_flags_for_above_UI_pri_change(
    struct turnstile *turnstile) {
 ...
 */
static boolean_t
workq_add_turnstile_promotion(struct workqueue *wq_inheritor __unused,
                              struct turnstile *turnstile) {
  /*
 ...
 *
 * Description: Check if turnstile position in turnstile's inheritor list needs
 * to be updated.
 *
 * Arg1: dst turnstile
 * Arg2: src turnstile
...
static boolean_t turnstile_need_turnstile_promotion_update(
  ...
  needs_update = (src_turnstile_link_priority == src_turnstile->ts_priority)
                     ? FALSE
                     : TRUE;
  return needs_update;
}
...
turnstile_update_turnstile_promotion_locked(struct turnstile *dst_turnstile,
  ...
  if (src_turnstile->ts_priority != src_turnstile_link_priority) {
    KERNEL_DEBUG_CONSTANT_IST(
        ...
        VM_KERNEL_UNSLIDE_OR_PERM(src_turnstile), src_turnstile->ts_priority,
        src_turnstile_link_priority, 0);
  }
...
turnstile_update_turnstile_promotion(struct turnstile *dst_turnstile,
  ...
  boolean_t needs_update =
      turnstile_need_turnstile_promotion_update(dst_turnstile, src_turnstile);
  if (!needs_update) {
    turnstile_stats_update(
        1, TSU_NO_PRI_CHANGE_NEEDED | TSU_TURNSTILE_ARG | TSU_BOOST_ARG,
        src_turnstile);
  ...
  needs_update =
      turnstile_update_turnstile_promotion_locked(dst_turnstile, src_turnstile);

  /* Update turnstile stats */
  if (!needs_update) {
    turnstile_stats_update(1,
                           (dst_turnstile->ts_inheritor
...
turnstile_add_turnstile_promotion(struct turnstile *dst_turnstile,
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      (TURNSTILE_CODE(TURNSTILE_HEAP_OPERATIONS,
          ...
          DBG_FUNC_NONE,
      VM_KERNEL_UNSLIDE_OR_PERM(dst_turnstile),
      VM_KERNEL_UNSLIDE_OR_PERM(src_turnstile), src_turnstile->ts_priority, 0,
      0);
  ...
  priority_queue_entry_set_sched_pri(&dst_turnstile->ts_inheritor_queue,
                                     ...
                                     src_turnstile->ts_priority, false);
...
turnstile_remove_turnstile_promotion(struct turnstile *dst_turnstile,
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      (TURNSTILE_CODE(TURNSTILE_HEAP_OPERATIONS,
          ...
          DBG_FUNC_NONE,
      VM_KERNEL_UNSLIDE_OR_PERM(dst_turnstile),
      VM_KERNEL_UNSLIDE_OR_PERM(src_turnstile), 0, 0, 0);
...
void turnstile_waitq_add_thread_priority_queue(struct waitq *wq,
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      (TURNSTILE_CODE(TURNSTILE_HEAP_OPERATIONS,
          ...
          DBG_FUNC_NONE,
      VM_KERNEL_UNSLIDE_OR_PERM(turnstile), thread_tid(thread), priority, 0, 0);
  /*
  ...
  priority_queue_entry_set_sched_pri(
      &wq->waitq_prio_queue, &thread->wait_prioq_links, priority, false);
  priority_queue_insert(&wq->waitq_prio_queue, &thread->wait_prioq_links);
}
...
boolean_t turnstile_recompute_priority_locked(struct turnstile *turnstile) {
  ...
  case TURNSTILE_KERNEL_PROMOTE:
    ...
    if (max_turnstile) {
      assert(turnstile_promote_policy[turnstile_get_type(turnstile)] !=
             TURNSTILE_KERNEL_PROMOTE);
      turnstile_max_pri = priority_queue_entry_sched_pri(
          &turnstile->ts_inheritor_queue, &max_turnstile->ts_inheritor_links);
    }
    ...
    if (old_priority != new_priority) {
      KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                                (TURNSTILE_CODE(TURNSTILE_PRIORITY_OPERATIONS,
                                                (TURNSTILE_PRIORITY_CHANGE))) |
                                    DBG_FUNC_NONE,
                                VM_KERNEL_UNSLIDE_OR_PERM(turnstile),
                                new_priority, old_priority, 0, 0);
    }
    needs_priority_update =
        (!(old_priority == new_priority)) && (turnstile->ts_inheritor != NULL);
    break;
...
int turnstile_workloop_pusher_info(struct turnstile *turnstile,
                                   thread_t *thread_out, ipc_port_t *port_out,
  ...
  max_ts = priority_queue_max(&turnstile->ts_inheritor_queue, struct turnstile,
                              ts_inheritor_links);
  if (max_ts) {
    max_ts_pri = priority_queue_entry_sched_pri(&turnstile->ts_inheritor_queue,
 ...
 * Returns: None.
 */
static void turnstile_update_inheritor_priority_chain(
    turnstile_inheritor_t inheritor, turnstile_update_flags_t turnstile_flags) {
  struct turnstile *turnstile = TURNSTILE_NULL;
  thread_t thread = THREAD_NULL;
  int total_hop = 0, thread_hop = 0;
  spl_t s;
  turnstile_stats_update_flags_t tsu_flags =
      ((turnstile_flags & TURNSTILE_UPDATE_BOOST) ? TSU_BOOST_ARG
                                                  : TSU_FLAGS_NONE) |
      TSU_PRI_PROPAGATION;
    ...
    if (turnstile != TURNSTILE_NULL) {
      if (turnstile->ts_inheritor == NULL) {
        turnstile_stats_update(total_hop + 1,
                               TSU_NO_INHERITOR | TSU_TURNSTILE_ARG | tsu_flags,
                               turnstile);
        waitq_unlock(&turnstile->ts_waitq);
      ...
      if (turnstile->ts_inheritor_flags & TURNSTILE_INHERITOR_THREAD) {
        ...
        turnstile_stats_update(total_hop + 1,
                               TSU_NO_PRI_CHANGE_NEEDED | tsu_flags, NULL);
        return;
      } else {
      ...
      thread_update_waiting_turnstile_priority_chain(
          &thread, &turnstile, thread_hop, total_hop, tsu_flags);
      thread_hop++;
    }
    total_hop++;
  }
 ...
 * Returns: None.
 */
void turnstile_update_inheritor_complete(struct turnstile *turnstile,
  ...
  if (inheritor_flags & TURNSTILE_NEEDS_PRI_UPDATE) {
    turnstile_update_inheritor_priority_chain(
        turnstile, TURNSTILE_INHERITOR_TURNSTILE | TURNSTILE_UPDATE_BOOST);
  }
}
...
void turnstile_cleanup(void) {
  ...
  if (inheritor_flags & TURNSTILE_INHERITOR_NEEDS_PRI_UPDATE) {
    turnstile_update_inheritor_priority_chain(old_inheritor, inheritor_flags);
  }
...
void turnstile_update_thread_priority_chain(thread_t thread) {
  turnstile_update_inheritor_priority_chain(thread, TURNSTILE_INHERITOR_THREAD |
 ...
 */
static void
turnstile_update_inheritor_workq_priority_chain(struct turnstile *turnstile,
                                                spl_t s) {
...
static void turnstile_update_inheritor_thread_priority_chain(
  ...
  *in_turnstile = TURNSTILE_NULL;
  ...
  needs_update =
      thread_needs_turnstile_promotion_update(thread_inheritor, turnstile);
  if (!needs_update && !first_update) {
    turnstile_stats_update(
        total_hop + 1, TSU_NO_PRI_CHANGE_NEEDED | TSU_TURNSTILE_ARG | tsu_flags,
        turnstile);
    waitq_unlock(&turnstile->ts_waitq);
    return;
  }
  ...
  /* adjust turnstile position in the thread's inheritor list */
  needs_update =
      thread_update_turnstile_promotion_locked(thread_inheritor, turnstile);
  ...
  if (!needs_update && !first_update) {
    ...
    turnstile_stats_update(
        total_hop + 1,
        (thread_get_update_flags_for_turnstile_propagation_stoppage(
            thread_inheritor)) |
 ...
 *            will be dropped for old value and will be acquired for the new
 * value.
 */
static void turnstile_update_inheritor_turnstile_priority_chain(
    struct turnstile **in_out_turnstile, int total_hop,
  ...
  *in_out_turnstile = TURNSTILE_NULL;
  ...
  /* Check if the inheritor turnstile needs to be updated before grabbing the
   * lock */
  needs_update =
      turnstile_need_turnstile_promotion_update(inheritor_turnstile, turnstile);
  if (!needs_update && !first_update) {
    turnstile_stats_update(
        total_hop + 1, TSU_NO_PRI_CHANGE_NEEDED | TSU_TURNSTILE_ARG | tsu_flags,
        turnstile);
  ...
  if (!needs_update && !first_update) {
    /* Update turnstile stats before returning */
    turnstile_stats_update(total_hop + 1,
                           (inheritor_turnstile->ts_inheritor
                                ? TSU_NO_PRI_CHANGE_NEEDED
                                : TSU_NO_INHERITOR) |
                               TSU_TURNSTILE_ARG | tsu_flags,
                           turnstile);
 ...
 *
 * Returns: *out_turnstile returns the inheritor if it needs further
 * propagation.
 *
...
static void thread_update_waiting_turnstile_priority_chain(
  ...
  *in_thread = THREAD_NULL;
  ...
  if (waiting_turnstile == TURNSTILE_NULL || thread_hop > turnstile_max_hop) {
    KERNEL_DEBUG_CONSTANT_IST(
        ...
        (TURNSTILE_CODE(TURNSTILE_HEAP_OPERATIONS,
            ...
            DBG_FUNC_NONE,
        thread_tid(thread), turnstile_max_hop, thread_hop,
        VM_KERNEL_UNSLIDE_OR_PERM(waiting_turnstile), 0);
    turnstile_stats_update(
        total_hop + 1, TSU_NO_TURNSTILE | TSU_THREAD_ARG | tsu_flags, thread);
    thread_unlock(thread);
    return;
  }
  ...
  needs_update =
      turnstile_need_thread_promotion_update(waiting_turnstile, thread);
  if (!needs_update && !first_update) {
    turnstile_stats_update(
        total_hop + 1, TSU_NO_PRI_CHANGE_NEEDED | TSU_THREAD_ARG | tsu_flags,
        thread);
    thread_unlock(thread);
    return;
  }
   ...
   * 1. The ticket lock guarantees that waiting_turnstile will not be reused by
   * other threads
   * 2. We are holding other turnstile lock, and doing turnstile free when we
   * dereference it can cause deadlock
  ...
  if (!waitq_lock_reserve(&waiting_turnstile->ts_waitq, &ticket)) {
    ...
    /* Check if the gencount matches and thread is still waiting on same
     * turnstile */
  ...
  needs_update =
      turnstile_update_thread_promotion_locked(waiting_turnstile, thread);
  ...
  if (!needs_update && !first_update) {
    turnstile_stats_update(total_hop + 1,
                           (waiting_turnstile->ts_inheritor
                                ? TSU_NO_PRI_CHANGE_NEEDED
                                : TSU_NO_INHERITOR) |
                               TSU_THREAD_ARG | tsu_flags,
                           thread);
    thread_unlock(thread);
    waitq_unlock(&waiting_turnstile->ts_waitq);
    return;
...
void turnstile_stats_update(int hop __assert_only,
  ...
  if (flags & TSU_NO_PRI_CHANGE_NEEDED) {
    os_atomic_inc(&turnstile_stats[hop - 1].ts_no_priority_change_required,
                  relaxed);
  }
...
#define STACKSHOT_TURNSTILE_FLAGS_MODE(flags)                                  \
  ((flags) & ~STACKSHOT_TURNSTILE_STATUS_PORTFLAGS)
#define STACKSHOT_TURNSTILE_FLAGS_PORT(flags)                                  \
  ((flags) & STACKSHOT_TURNSTILE_STATUS_PORTFLAGS)
...
static uint64_t
kdp_turnstile_traverse_inheritor_chain(struct turnstile *ts, uint64_t *flags,
                                       uint8_t *hops,
  ...
  *hops = *hops + 1;
  ...
  if (turnstile_is_send_turnstile(ts)) {
    ...
    if (port && ip_active(port) && ip_is_any_service_port(port)) {
      *flags = STACKSHOT_TURNSTILE_FLAGS_WITHPORT(
          *flags, STACKSHOT_TURNSTILE_STATUS_SENDPORT);
  ...
  if (turnstile_is_receive_turnstile(ts)) {
    ...
    if (port && ip_active(port) && ip_is_any_service_port(port)) {
      *flags = STACKSHOT_TURNSTILE_FLAGS_WITHPORT(
          *flags, STACKSHOT_TURNSTILE_STATUS_RECEIVEPORT);
      *isplp = ptrauth_strip(port->ip_object.iol_service,
                             ptrauth_key_process_independent_data);
    }
  }
  /*
   * If a turnstile is inheriting our priority, recurse.  If we get back
   * *exactly* UNKNOWN, continue on, since we may be able to give a more
   * specific answer.  To give an accurate hops count, we reset *hops, saving
   * the recursive value in unknown_hops to use if we can't give a better
   * answer.
   */
  if (ts->ts_inheritor_flags & TURNSTILE_INHERITOR_TURNSTILE) {
    uint8_t pre_hops = *hops;
    uint64_t ret = kdp_turnstile_traverse_inheritor_chain(ts->ts_inheritor,
                                                          flags, hops, isplp);
    /*
     ...
     */
    if (ret != 0 || STACKSHOT_TURNSTILE_FLAGS_MODE(*flags) !=
                        STACKSHOT_TURNSTILE_STATUS_UNKNOWN) {
      return ret;
    }
    /* restore original hops value, saving the new one if we fall through to
     * unknown */
    unknown_hops = *hops;
    *hops = pre_hops;
...
void kdp_turnstile_fill_tsinfo(struct turnstile *ts,
  ...
  final_inheritor =
      kdp_turnstile_traverse_inheritor_chain(ts, &flags, &hops, isplp);
...
int turnstile_get_boost_stats_sysctl(void *req) {
  return sysctl_io_opaque(
      req, turnstile_boost_stats,
      sizeof(struct turnstile_stats) * TURNSTILE_MAX_HOP_DEFAULT, NULL);
}
...
static void tstile_test_prim_init(struct tstile_test_prim *test_prim) {
  test_prim->ttprim_turnstile = TURNSTILE_NULL;
  test_prim->ttprim_owner = NULL;
  lck_spin_init(&test_prim->ttprim_interlock, &turnstiles_dev_lock_grp,
                LCK_ATTR_NULL);
...
lock_start:
  ...
  if (use_hashtable) {
    ...
    prim_turnstile =
        turnstile_prepare((uintptr_t)test_prim, &test_prim->ttprim_turnstile,
                          TURNSTILE_NULL, type);
  }
  ...
  if (test_prim->ttprim_owner == NULL) {
    ...
    if (use_hashtable) {
      ...
      turnstile_complete((uintptr_t)test_prim, &test_prim->ttprim_turnstile,
                         NULL, type);
    }
  ...
  turnstile_update_inheritor(
      prim_turnstile, test_prim->ttprim_owner,
      (TURNSTILE_DELAYED_UPDATE | TURNSTILE_INHERITOR_THREAD));

  waitq_assert_wait64(&prim_turnstile->ts_waitq, CAST_EVENT64_T(test_prim),
                      wait_type, TIMEOUT_WAIT_FOREVER);
  ...
  if (use_hashtable) {
    ...
    turnstile_complete((uintptr_t)test_prim, &test_prim->ttprim_turnstile, NULL,
                       type);
  }
...
int tstile_test_prim_unlock(int val) {
  ...
  waitq_wakeup64_one(&prim_turnstile->ts_waitq, CAST_EVENT64_T(test_prim),
  ...
  if (old_owner) {
    /* Changing this to thread_deallocate_safe to exercise
     * thread_deallocate_safe path */
