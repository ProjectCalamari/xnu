...
static void mpsc_test_pingpong_invoke(mpsc_queue_chain_t elm,
...
int mpsc_test_pingpong(uint64_t count, uint64_t *out) {
  ...
  kr = mpsc_daemon_queue_init_with_thread(
      &ping.queue, mpsc_test_pingpong_invoke, MINPRI_KERNEL, "ping",
  ...
  kr = mpsc_daemon_queue_init_with_thread(
      &pong.queue, mpsc_test_pingpong_invoke, MINPRI_KERNEL, "pong",
      MPSC_DAEMON_INIT_NONE);
  ...
  assert_wait_timeout(&mpsc_test_pingpong_invoke, THREAD_UNINT, timeout,
                      NSEC_PER_SEC);
  ...
  printf(
      "mpsc_test_pingpong: %lld ping-pongs in %lld ns (%lld.%03lld us/async)\n",
