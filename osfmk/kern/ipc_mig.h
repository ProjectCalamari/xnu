...
/*
 * Define the trace points for MIG-generated calls.  One traces the input
 * parameters to MIG called things, another traces the outputs, and one traces
 * bad message IDs.
 */
...
#define __BeforeRcvCallTrace(msgid, arg1, arg2, arg3, arg4)                    \
  KERNEL_DEBUG_CONSTANT_IST(                                                   \
      KDEBUG_TRACE, KDBG_MIGCODE(msgid) | DBG_FUNC_START,                      \
      (unsigned int)(arg1), (unsigned int)(arg2), (unsigned int)(arg3),        \
      (unsigned int)(arg4), (unsigned int)(0));

#define __AfterRcvCallTrace(msgid, arg1, arg2, arg3, arg4)                     \
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE, KDBG_MIGCODE(msgid) | DBG_FUNC_END,  \
...
#define __BeforeKobjectServerTrace(msgid)                                      \
  KERNEL_DEBUG_CONSTANT_IST(                                                   \
      KDEBUG_TRACE, KDBG_MIGCODE(msgid) | DBG_FUNC_START, 0u, 0u, 0u, 0u, 0u)
...
#define _MIG_MSGID_INVALID(msgid)                                              \
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,                                      \
                            MACHDBG_CODE(DBG_MACH_MSGID_INVALID, (msgid)), 0u, \
                            0u, 0u, 0u, 0u)
...
extern mach_msg_return_t mach_msg_send_from_kernel(mach_msg_header_t *msg,
                                                   mach_msg_size_t send_size);
#else
extern mach_msg_return_t
mach_msg_send_from_kernel_proper(mach_msg_header_t *msg,
                                 mach_msg_size_t send_size);
 ...
 *
 *     - descriptor_count: Descriptor count in the outgoing message. If
 * non-zero, kernel expects a complex message, and vice-versa.
 *
 ...
 *
 * void (^builder)(mach_msg_header_t *header, mach_msg_descriptor_t *descs, void
 * *payload))
 *
 *            header* ───────► ┌─────────────────────────┐
 *                             │                         │
 ...
 *
 *     descs: Pointers to the start of kernel descriptors region, if caller
 * requested a non-zero descriptor_count. Or NULL if descriptor_count is 0
 * (meaning the messsage is not complex). Caller should populate this with
 * descriptors.
 *
 *     payload: Points to the start of data not processed by kernel, which is
 * after the last descriptor, if caller requested a non-zero payload_size. Or
 *          NULL if payload_size is 0.
 *
 *     Mach message body (descriptor count) will be set after the builder
 * accordingly.
 */
...
extern mach_msg_return_t kernel_mach_msg_send_with_builder(
    ...
    void (^builder)(mach_msg_header_t *header,
                    mach_msg_descriptor_t *__counted_by(descriptor_count)
                        descs,                                /* Nullable */
...
extern mach_msg_return_t mach_msg_rpc_from_kernel(mach_msg_header_t *msg,
                                                  mach_msg_size_t send_size,
                                                  mach_msg_size_t rcv_size);

extern void mach_msg_destroy_from_kernel(mach_msg_header_t *msg);

#else

extern mach_msg_return_t
mach_msg_rpc_from_kernel_proper(mach_msg_header_t *msg,
                                mach_msg_size_t send_size,
                                mach_msg_size_t rcv_size);
...
extern mach_msg_return_t kernel_mach_msg_send(mach_msg_header_t *msg,
                                              mach_msg_size_t send_size,
                                              mach_msg_option64_t option,
                                              mach_msg_timeout_t timeout_val,
                                              boolean_t *message_moved);

extern mach_msg_return_t kernel_mach_msg_send_kmsg(struct ipc_kmsg *kmsg);
