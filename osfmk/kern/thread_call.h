 ...
 *  @discussion Thread call priorities should not be assumed to have any
 * specific numerical value; they should be interpreted as importances or roles
 * for work items, priorities for which will be reasonably managed by the
 * subsystem.
 *  @constant THREAD_CALL_PRIORITY_HIGH Importance above everything but
 * realtime. Thread calls allocated with this priority execute at extremely high
 * priority, above everything but realtime threads.  They are generally executed
 * in serial. Though they may execute concurrently under some circumstances, no
 * fan-out is implied. These work items should do very small amounts of work or
 * risk disrupting system responsiveness.
 *  @constant THREAD_CALL_PRIORITY_KERNEL Importance similar to that of normal
 * kernel threads.
 *  @constant THREAD_CALL_PRIORITY_USER Importance similar to that of normal
 * user threads.
 *  @constant THREAD_CALL_PRIORITY_LOW Very low importance.
 *  @constant THREAD_CALL_PRIORITY_KERNEL_HIGH Importance higher than most
 * kernel threads.
 */
typedef enum {
...
enum {
  /* if call is re-submitted while the call is executing on a call thread, then
     delay the re-enqueue until it returns */
  THREAD_CALL_OPTIONS_ONCE = 0x00000001,
#ifdef XNU_KERNEL_PRIVATE
  /* execute call from the timer interrupt instead of from the thread call
     thread, private interface for IOTES workloop signaling */
 ...
 *  @discussion If the work item is already scheduled for delayed execution, and
 * it has not yet begun to run, that delayed invocation will be cancelled.  Note
 * that if a thread call is rescheduled from its own callback, then multiple
 * invocations of the callback may be in flight at the same time.
 *  @result TRUE if the call was already pending for either delayed or immediate
 *  execution, FALSE otherwise.
 *  @param call The thread call to execute.
 */
extern boolean_t thread_call_enter(thread_call_t call);
/*!
 *  @function thread_call_enter1
 *  @abstract Submit a thread call work item for immediate execution, with an
 * extra parameter.
 ...
 */
extern boolean_t thread_call_enter1(thread_call_t call,
                                    thread_call_param_t param1);

/*!
 *  @function thread_call_enter_delayed
 *  @abstract Submit a thread call to be executed at some point in the future.
 *  @discussion If the work item is already scheduled for delayed or immediate
 * execution, and it has not yet begun to run, that invocation will be cancelled
 * in favor of execution at the newly specified time.  Note that if a thread
 * call is rescheduled from its own callback, then multiple invocations of the
 ...
 */
extern boolean_t thread_call_enter_delayed(thread_call_t call,
                                           uint64_t deadline);
/*!
 ...
 *  @param deadline Time, in absolute time units, at which to execute callback.
 */
extern boolean_t thread_call_enter1_delayed(thread_call_t call,
                                            thread_call_param_t param1,
                                            uint64_t deadline);
#ifdef XNU_KERNEL_PRIVATE
 ...
 *  @param param1 Second parameter to callback.
 *  @param deadline Time, in absolute time units, at which to execute callback.
 *  @param leeway Time delta, in absolute time units, which sets range of time
 * allowing kernel to decide appropriate time to run.
 *  @param flags configuration for timers in kernel.
 */
extern boolean_t thread_call_enter_delayed_with_leeway(
    thread_call_t call, thread_call_param_t param1, uint64_t deadline,
    uint64_t leeway, uint32_t flags);
 ...
 */
extern boolean_t thread_call_cancel(thread_call_t call);
/*!
 ...
 *  called on a work item which has already finished, it will return FALSE
 * immediately. Note that this routine can only be used on thread calls set up
 * with either thread_call_allocate or thread_call_allocate_with_priority, and
 * that invocations of the thread call <i>after</i> the current invocation may
 * be in flight when thread_call_cancel_wait returns.
 *  @result TRUE if the call was successfully cancelled, FALSE otherwise.
 */
extern boolean_t thread_call_cancel_wait(thread_call_t call);
...
thread_call_allocate_with_priority(thread_call_func_t func,
                                   thread_call_param_t param0,
...
extern thread_call_t thread_call_allocate_with_options(
    thread_call_func_t func, thread_call_param_t param0,
    thread_call_priority_t pri, thread_call_options_t options);

#ifdef KERNEL_PRIVATE
/*!
 *  @function thread_call_allocate_with_qos
 *  @abstract Allocate a thread call to execute with a specified QoS.
 *  @discussion Identical to thread_call_allocate_with_options, except it uses
 * the QoS namespace. Private interface for pthread kext.
 *  @param func Callback to invoke when thread call is scheduled.
 *  @param param0 First argument to pass to callback.
 *  @param qos_tier QoS tier to execute callback at (as in THREAD_QOS_POLICY)
 *  @param options flags from thread_call_options_t to influence the thread call
 * behavior
 *  @result Thread call which can be passed to thread_call_enter variants.
 */
extern thread_call_t
thread_call_allocate_with_qos(thread_call_func_t func,
                              thread_call_param_t param0, int qos_tier,
                              thread_call_options_t options);

/*!
 *  @function thread_call_wait_once
 *  @abstract Wait for a THREAD_CALL_OPTIONS_ONCE call to finish executing if it
 * is executing
 ...
 */
extern boolean_t thread_call_wait_once(thread_call_t call);
#endif /* KERNEL_PRIVATE */

/*!
 *  @function thread_call_free
 *  @abstract Release a thread call.
 *  @discussion Should only be used on thread calls allocated with
 * thread_call_allocate or thread_call_allocate_with_priority.  Once
 * thread_call_free has been called, no other operations may be performed on a
 * thread call.  If the thread call is currently pending, thread_call_free will
 * return FALSE and will have no effect. Calling thread_call_free from a thread
 * call's own callback is safe; the work item is not considering "pending" at
 * that point.
 *  @result TRUE if the thread call has been successfully released, else FALSE.
 *  @param call The thread call to release.
 */
extern boolean_t thread_call_free(thread_call_t call);
 ...
 *  @result TRUE if the thread call is either scheduled for execution
 * (immediately or at some point in the future) or is currently executing.
 */
...
__enum_closed_decl(
    thread_call_index_t, uint16_t,
    {
        THREAD_CALL_INDEX_INVALID =
            0, /* make sure zero tc_index is detected as invalid */
...
extern void thread_call_setup(thread_call_t call, thread_call_func_t func,
                              thread_call_param_t param0);

extern void thread_call_setup_with_options(thread_call_t call,
...
extern void thread_call_func_delayed(thread_call_func_t func,
                                     thread_call_param_t param,
                                     uint64_t deadline);

extern void thread_call_func_delayed_with_leeway(thread_call_func_t func,
                                                 thread_call_param_t param,
                                                 uint64_t deadline,
                                                 uint64_t leeway,
                                                 uint32_t flags);
 ...
 */
extern boolean_t thread_call_func_cancel(thread_call_func_t func,
                                         thread_call_param_t param,
                                         boolean_t cancel_all);

/*
 * Called on the wake path to adjust the thread callouts running in
