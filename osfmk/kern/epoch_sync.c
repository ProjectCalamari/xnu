...
static ht_t *space_to_ht(esync_space_t space) {
  switch (space) {
...
static __startup_func void ht_startup_init(ht_t *ht) {
...
static inline ht_bucket_t *ht_get_bucket(ht_t *ht, const uint64_t key) {
...
static esync_t *ht_put(ht_t *ht, const uint64_t key, esync_t *new_value) {
...
static void ht_remove(ht_t *ht, const uint64_t key, esync_t *value) {
  ht_bucket_t *bucket = ht_get_bucket(ht, key);
...
static esync_t *ht_get(ht_t *ht, const uint64_t key) {
  ht_bucket_t *bucket = ht_get_bucket(ht, key);
 ...
 */
static esync_t *esync_alloc(const uint64_t id, const esync_policy_t policy) {
 ...
 */
static void esync_free(esync_t *sync) {
 ...
 */
static void esync_put(ht_t *ht, esync_t *sync, esync_t *to_be_freed) {
 ...
 */
static esync_t *esync_get(ht_t *ht, const uint64_t id,
                          const esync_policy_t policy,
 ...
 */
static bool esync_update_epoch(const uint64_t epoch,
                               os_atomic(uint64_t) * counter) {
  uint64_t old, new;
...
static void esync_wait_set_block_hint(const esync_space_t space,
                                      const esync_policy_t policy) {
...
void kdp_esync_find_owner(struct waitq *waitq, event64_t event,
 ...
 */
wait_result_t esync_wait(esync_space_t space, const uint64_t id,
                         const uint64_t epoch, os_atomic(uint64_t) * counter,
                         const ctid_t owner_ctid, const esync_policy_t policy,
                         const wait_interrupt_t interruptible) {
  assert3u(id, !=, ES_INVALID_ID);
  ...
  if (!fresh_epoch) {
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EPOCH_SYNC, MACH_EPOCH_SYNC_WAIT_STALE),
                 unique_id, epoch, NULL);
  ...
  struct turnstile *ts = turnstile_prepare((uintptr_t)sync, &sync->es_turnstile,
                                           TURNSTILE_NULL, tt);
  ...
  wait_result_t wr = waitq_assert_wait64(&ts->ts_waitq, CAST_EVENT64_T(sync),
 ...
 */
kern_return_t esync_wake(esync_space_t space, const uint64_t id,
                         const uint64_t epoch, os_atomic(uint64_t) * counter,
                         const esync_wake_mode_t mode, const ctid_t ctid) {
  assert3u(id, !=, ES_INVALID_ID);
  assert(mode == ESYNC_WAKE_ONE || mode == ESYNC_WAKE_ALL ||
         mode == ESYNC_WAKE_ONE_WITH_OWNER || mode == ESYNC_WAKE_THREAD);
  ...
  if (sync == NULL) {
    ...
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_EPOCH_SYNC, MACH_EPOCH_SYNC_WAKE_NO_WAITERS),
        unique_id, epoch, mode);
  ...
  case ESYNC_WAKE_ONE:
    ...
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EPOCH_SYNC, MACH_EPOCH_SYNC_WAKE_ONE),
                 unique_id, epoch);
  ...
  case ESYNC_WAKE_ALL:
    ...
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EPOCH_SYNC, MACH_EPOCH_SYNC_WAKE_ALL),
                 unique_id, epoch);
  ...
  case ESYNC_WAKE_ONE_WITH_OWNER:
    ...
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_EPOCH_SYNC, MACH_EPOCH_SYNC_WAKE_ONE_WITH_OWNER),
        unique_id, epoch, tid);
    kr = waitq_wakeup64_one(&ts->ts_waitq, CAST_EVENT64_T(sync),
                            THREAD_AWAKENED, WAITQ_WAKEUP_DEFAULT);
    turnstile_update_inheritor(
        ts, thread, TURNSTILE_IMMEDIATE_UPDATE | TURNSTILE_INHERITOR_THREAD);
    break;

  case ESYNC_WAKE_THREAD:
    /* No new inheritor. Wake the specified thread (if waiting). */
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EPOCH_SYNC, MACH_EPOCH_SYNC_WAKE_THREAD),
                 unique_id, epoch, tid);
    kr = waitq_wakeup64_thread(&ts->ts_waitq, CAST_EVENT64_T(sync), thread,
                               THREAD_AWAKENED);
  }
 ...
 * the lock.
 */
static void test_lock(uint64_t *lock) {
  ...
  while (true) {
    ...
    do {
      ...
      if (os_atomic_cmpxchgv(lock, old, old | WAITER_BIT, &old, relaxed) == 1) {
        ...
        random_delay();
        const wait_result_t wr = esync_wait(
            ESYNC_SPACE_TEST, (uint32_t)(uintptr_t)lock, epoch, &server_counter,
            OWNER(old), ESYNC_POLICY_KERNEL, THREAD_UNINT);
 ...
 * Drop the lock.
 */
static void test_unlock(uint64_t *lock) {
  const ctid_t ctid = thread_get_ctid(current_thread());
...
static void test_lock_unlock(__unused void *arg, __unused int a) {
  for (int c = 0; c < 10; c++) {
...
/* Wait then wake. */
static int esync_test(int64_t count, int64_t *out) {
  kern_return_t ret;
  thread_t *thread = kalloc_type(thread_t, count, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  for (int64_t i = 0; i < count; i++) {
    ret = kernel_thread_start_priority(test_lock_unlock, NULL, BASEPRI_DEFAULT,
                                       &thread[i]);
 ...
 */
static int esync_test_wait(__unused int64_t in, __unused int64_t *out) {
