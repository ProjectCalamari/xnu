...
/* Forwards */
static void processor_offline(void *parameter, __unused wait_result_t result);

static void processor_offline_intstack(processor_t processor) __dead2;
 ...
 *	for scheduling.
 */
void processor_up(processor_t processor) {
  spl_t s = splsched();
  ...
  is_first_online_processor =
      sched_mark_processor_online(processor, processor->last_startup_reason);
...
kern_return_t processor_assign(__unused processor_t processor,
                               __unused processor_set_t new_pset,
                               __unused boolean_t wait) {
  return KERN_FAILURE;
}

void processor_doshutdown(processor_t processor, bool is_final_system_sleep) {
 ...
 * in this routine, but it's actually been powered off and has no hardware
 * state.
 */
static void processor_offline(void *parameter, __unused wait_result_t result) {
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PROCESSOR_SHUTDOWN) | DBG_FUNC_START,
       processor->cpu_id);
   ...
   */
  old_thread =
      machine_processor_shutdown(self, processor_offline_intstack, processor);
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PROCESSOR_SHUTDOWN) | DBG_FUNC_END,
       processor->cpu_id);

  /*
   * Now that the processor is back, invoke the idle thread to find out what to
   * do next. idle_thread will enable interrupts.
   */
 ...
 */
static void processor_offline_intstack(processor_t processor) {
  assert(processor == current_processor());
 ...
 */
void processor_cpu_reinit(void *machine_param,
   ...
   *
   * SIGPdisabled is cleared (to enable cpu_signal() to succeed with this
   * processor) the first time we take an IPI.  This is triggered by
   * machine_cpu_reinit(), above, which calls
   * cpu_machine_init()->PE_cpu_machine_init()->PE_cpu_signal() which sends a
   * self-IPI to ensure that happens when we enable interrupts.  So enable
   * interrupts here so that cpu_signal() can succeed before we spin on
   * mp_kdp_trap.
   */
...
kern_return_t host_get_boot_info(host_priv_t host_priv,
...
MACHINE_TIMEOUT_DEV_WRITEABLE(report_phy_read_delay_to, "report-phy-read-delay",
                              0, MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
MACHINE_TIMEOUT_DEV_WRITEABLE(report_phy_write_delay_to,
                              "report-phy-write-delay", 0,
                              MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
MACHINE_TIMEOUT_DEV_WRITEABLE(trace_phy_read_delay_to, "trace-phy-read-delay",
                              DEFAULT_TRACE_PHY_TIMEOUT,
                              MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
MACHINE_TIMEOUT_DEV_WRITEABLE(trace_phy_write_delay_to, "trace-phy-write-delay",
                              DEFAULT_TRACE_PHY_TIMEOUT,
                              MACHINE_TIMEOUT_UNIT_TIMEBASE, NULL);
 ...
 */
static void ml_io_init_timeouts(void) {
  /*
   ...
   */
  const uint64_t interrupt_masked_to =
      os_atomic_load(&interrupt_masked_timeout, relaxed);
  ...
  if (!PE_parse_boot_argn("ml-timeout-report-phy-read-delay", &arg,
                          sizeof(arg))) {
    uint64_t report_phy_read_delay =
        os_atomic_load(&report_phy_read_delay_to, relaxed);
    report_phy_read_delay =
        report_phy_read_delay == 0
            ? interrupt_masked_to
            : MIN(report_phy_read_delay, interrupt_masked_to);
    os_atomic_store(&report_phy_read_delay_to, report_phy_read_delay, relaxed);
  }

  if (!PE_parse_boot_argn("ml-timeout-report-phy-write-delay", &arg,
                          sizeof(arg))) {
    uint64_t report_phy_write_delay =
        os_atomic_load(&report_phy_write_delay_to, relaxed);
    report_phy_write_delay =
        report_phy_write_delay == 0
            ? interrupt_masked_to
            : MIN(report_phy_write_delay, interrupt_masked_to);
    os_atomic_store(&report_phy_write_delay_to, report_phy_write_delay,
                    relaxed);
  }
}
...
static LCK_SPIN_DECLARE(io_timeout_override_lock,
                        &io_timeout_override_lock_grp);
...
static inline int
io_timeout_override_cmp(const struct io_timeout_override_entry *a,
                        const struct io_timeout_override_entry *b) {
...
static RB_HEAD(io_timeout_override,
               io_timeout_override_entry) io_timeout_override_root_pa,
    io_timeout_override_root_va;

RB_PROTOTYPE_PREV(io_timeout_override, io_timeout_override_entry, tree,
                  io_timeout_override_cmp);
RB_GENERATE_PREV(io_timeout_override, io_timeout_override_entry, tree,
                 io_timeout_override_cmp);

static int io_increase_timeouts(struct io_timeout_override *root,
                                uintptr_t ioaddr_base, unsigned int size,
  ...
  nanoseconds_to_absolutetime(NSEC_PER_USEC * read_timeout_us,
                              &read_timeout_abs);
  nanoseconds_to_absolutetime(NSEC_PER_USEC * write_timeout_us,
                              &write_timeout_abs);
  if (read_timeout_abs > MAX_TIMEOUT_ABS ||
      write_timeout_abs > MAX_TIMEOUT_ABS) {
    return KERN_INVALID_ARGUMENT;
  }

  struct io_timeout_override_entry *node = kalloc_type(
      struct io_timeout_override_entry, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  struct io_timeout_override_entry *prev =
      RB_PREV(io_timeout_override, root, node);
  if (prev && (prev->ioaddr_base + prev->size) > node->ioaddr_base) {
    RB_REMOVE(io_timeout_override, root, node);
    ret = KERN_INVALID_ARGUMENT;
    goto out;
  }
  struct io_timeout_override_entry *next =
      RB_NEXT(io_timeout_override, root, node);
...
static int io_reset_timeouts(struct io_timeout_override *root,
                             uintptr_t ioaddr_base, unsigned int size) {
  assert(preemption_enabled());
  ...
  boolean_t istate = ml_set_interrupts_enabled(FALSE);
  lck_spin_lock(&io_timeout_override_lock);
  struct io_timeout_override_entry *node =
      RB_FIND(io_timeout_override, root, &key);
  if (node) {
    if (node->size == size) {
      RB_REMOVE(io_timeout_override, root, node);
    } else {
...
static bool io_override_timeout(struct io_timeout_override *root,
                                uintptr_t addr, uint64_t *read_timeout,
  ...
  lck_spin_lock(&io_timeout_override_lock);
  /* RB_FIND() doesn't support custom cmp functions, so we have to open-code our
   * own */
  while (node) {
...
static bool io_override_timeout_ss(uint64_t paddr, uint64_t *read_timeout,
                                   uint64_t *write_timeout) {
#if defined(__arm64__)
 ...
 */
void override_io_timeouts(uintptr_t vaddr, uint64_t paddr,
...
int ml_io_increase_timeouts(uintptr_t ioaddr_base, unsigned int size,
                            uint32_t read_timeout_us,
                            uint32_t write_timeout_us) {
#if ML_IO_TIMEOUTS_ENABLED
  ...
  return io_increase_timeouts(&io_timeout_override_root_va, ioaddr_base, size,
                              read_timeout_us, write_timeout_us);
#else
...
int ml_io_increase_timeouts_phys(vm_offset_t ioaddr_base, unsigned int size,
                                 uint32_t read_timeout_us,
                                 uint32_t write_timeout_us) {
#if ML_IO_TIMEOUTS_ENABLED
  return io_increase_timeouts(&io_timeout_override_root_pa, ioaddr_base, size,
                              read_timeout_us, write_timeout_us);
#else
#pragma unused(ioaddr_base, size, read_timeout_us, write_timeout_us)
  return KERN_SUCCESS;
...
int ml_io_reset_timeouts(uintptr_t ioaddr_base, unsigned int size) {
#if ML_IO_TIMEOUTS_ENABLED
...
int ml_io_reset_timeouts_phys(vm_offset_t ioaddr_base, unsigned int size) {
#if ML_IO_TIMEOUTS_ENABLED
...
#if ML_IO_TIMEOUTS_ENABLED
boolean_t ml_io_check_for_mmio_overrides(__unused uint64_t mt) {
#if __arm64__
  ...
  percpu_foreach(mmiot, mmio_tracker) {
    ...
    override_io_timeouts(mmiot->mmio_vaddr, mmiot->mmio_paddr, &read_timeout,
                         &write_timeout);
...
unsigned long long ml_io_read(uintptr_t vaddr, int size) {
  ...
  uint64_t const trace_phy_read_delay =
      os_atomic_load(&trace_phy_read_delay_to, relaxed);
#endif /* __x86_64__ */
  ...
  if (__improbable(use_timeout == TRUE)) {
    ...
    if (current_processor()->state == PROCESSOR_RUNNING) {
      iotrace(IOTRACE_IO_READ, vaddr, paddr, size, result,
              kern_timeout_start_time(&timeout), duration);
    }

    if (__improbable(duration > report_read_delay)) {
      DTRACE_PHYSLAT5(physioread, uint64_t, duration, uint64_t, vaddr, uint32_t,
                      size, uint64_t, paddr, uint64_t, result);
    ...
    if (__improbable(trace_phy_read_delay > 0 &&
                     duration > trace_phy_read_delay)) {
      KDBG(MACHDBG_CODE(DBG_MACH_IO, DBC_MACH_IO_MMIO_READ), duration,
           VM_KERNEL_UNSLIDE_OR_PERM(vaddr), paddr, result);
    }
...
uint64_t ml_io_read_cpu_reg(uintptr_t vaddr, int sz, __unused int logical_cpu) {
  uint64_t val;
...
void ml_io_write(uintptr_t vaddr, uint64_t val, int size) {
  ...
  uint64_t trace_phy_write_delay =
      os_atomic_load(&trace_phy_write_delay_to, relaxed);
#endif /* !defined(__x86_64__) */
  ...
  case 8:
    *(volatile uint64_t *)vaddr = (uint64_t)val;
    break;
  default:
    panic("Invalid size %d for ml_io_write(%p, 0x%llx)", size, (void *)vaddr,
          val);
    break;
  }
  ...
  if (__improbable(use_timeout == TRUE)) {
     ...
     * initialization procedure. */
    if (current_processor()->state == PROCESSOR_RUNNING) {
      iotrace(IOTRACE_IO_WRITE, vaddr, paddr, size, val,
              kern_timeout_start_time(&timeout), duration);
    }

    if (__improbable(duration > report_write_delay)) {
      DTRACE_PHYSLAT5(physiowrite, uint64_t, duration, uint64_t, vaddr,
                      uint32_t, size, uint64_t, paddr, uint64_t, val);
    ...
    if (__improbable(trace_phy_write_delay > 0 &&
                     duration > trace_phy_write_delay)) {
      KDBG(MACHDBG_CODE(DBG_MACH_IO, DBC_MACH_IO_MMIO_WRITE), duration,
           VM_KERNEL_UNSLIDE_OR_PERM(vaddr), paddr, val);
    }
...
void ml_io_write8(uintptr_t vaddr, uint8_t val) { ml_io_write(vaddr, val, 1); }
...
static const char *cpu_event_log_string(enum cpu_event e) {
...
void cpu_event_debug_log(__unused enum cpu_event event,
                         __unused unsigned int cpu_or_cluster) {
  /* no logging on production builds */
}

void dump_cpu_event_log(__unused int (*printf_func)(const char *fmt, ...)) {}
...
void machine_timeout_init_with_suffix(const struct machine_timeout_spec *spec,
                                      char const *suffix, bool always_enabled) {
  if (!always_enabled && (wdt == -1 || (spec->skip_predicate != NULL &&
                                        spec->skip_predicate(spec)))) {
  ...
  size_t const scale_name_size =
      MACHINE_TIMEOUT_MAX_NAME_LEN + suffix_len + strlen("-scale") + 1;
  ...
  size_t const boot_arg_name_size =
      MACHINE_TIMEOUT_MAX_NAME_LEN + strlen("ml-timeout-") + suffix_len + 1;
  ...
  if (SecureDTLookupEntry(NULL, "/chosen/machine-timeouts", &chosen) !=
      kSuccess) {
    chosen = NULL;
  }
  ...
  if (base != NULL && SecureDTGetProperty(base, dt_name, (const void **)&data,
                                          &data_size) == kSuccess) {
    if (data_size != sizeof(*data)) {
      panic("%s: unexpected machine timeout data_size %u for "
            "/machine-timeouts/%s",
            __func__, data_size, dt_name);
    }
  ...
  if (chosen != NULL &&
      SecureDTGetProperty(chosen, dt_name, (const void **)&data, &data_size) ==
          kSuccess) {
    if (data_size != sizeof(*data)) {
      panic("%s: unexpected machine timeout data_size %u for "
            "/chosen/machine-timeouts/%s",
            __func__, data_size, dt_name);
    }
  ...
  if (base != NULL &&
      SecureDTGetProperty(base, scale_name, (const void **)&scale_data,
                          &scale_size) == kSuccess) {
    if (scale_size != sizeof(*scale_data)) {
      panic("%s: unexpected machine timeout data_size %u for "
            "/machine-timeouts/%s-scale",
            __func__, scale_size, dt_name);
    }
   ...
   */
  if (chosen != NULL &&
      SecureDTGetProperty(chosen, scale_name, (const void **)&scale_data,
                          &scale_size) == kSuccess) {
    if (scale_size != sizeof(*scale_data)) {
      panic("%s: unexpected machine timeout data_size %u for "
            "/chosen/machine-timeouts/%s-scale",
            __func__, scale_size, dt_name);
    }
  ...
  if (!global_scale_set) {
    ...
    if (SecureDTGetProperty(base, "global-scale", (const void **)&scale_data,
                            &scale_size) == kSuccess) {
      if (scale_size != sizeof(*scale_data)) {
        panic("%s: unexpected machine timeout data_size %u for "
              "/machine-timeouts/global-scale",
              __func__, scale_size);
      }
    ...
    if (SecureDTGetProperty(chosen, "global-scale", (const void **)&scale_data,
                            &scale_size) == kSuccess) {
      if (scale_size != sizeof(*scale_data)) {
        panic("%s: unexpected machine timeout data_size %u for "
              "/chosen/machine-timeouts/global-scale",
              __func__, scale_size);
      }
    ...
    if (PE_parse_boot_argn("ml-timeout-global-scale", &boot_arg,
                           sizeof(boot_arg))) {
...
void machine_timeout_init(const struct machine_timeout_spec *spec) {
  machine_timeout_init_with_suffix(spec, "", false);
}

void machine_timeout_init_always_enabled(
    const struct machine_timeout_spec *spec) {
 ...
 */
void machine_timeout_bsd_init(void) {
  char const *const __unused mt_suffix = "-b";
#if SCHED_HYGIENE_DEBUG
  machine_timeout_init_with_suffix(
      MACHINE_TIMEOUT_SPEC_REF(interrupt_masked_timeout), mt_suffix, false);
  machine_timeout_init_with_suffix(
      MACHINE_TIMEOUT_SPEC_REF(sched_preemption_disable_threshold_mt),
      mt_suffix, false);
...
static inline void ml_io_timeout_test_get_timeouts(uintptr_t vaddr,
                                                   uint64_t *read_timeout,
                                                   uint64_t *write_timeout) {
  *read_timeout = 0;
  *write_timeout = 0;
...
static inline void
ml_io_timeout_test_get_timeouts_phys(vm_offset_t paddr, uint64_t *read_timeout,
                                     uint64_t *write_timeout) {
  *read_timeout = 0;
  *write_timeout = 0;
...
kern_return_t ml_io_timeout_test(void) {
  ...
  nanoseconds_to_absolutetime(NSEC_PER_USEC * READ_TIMEOUT1_US,
                              &read_timeout1_abs);
  nanoseconds_to_absolutetime(NSEC_PER_USEC * WRITE_TIMEOUT1_US,
                              &write_timeout1_abs);
  nanoseconds_to_absolutetime(NSEC_PER_USEC * READ_TIMEOUT2_US,
                              &read_timeout2_abs);
  nanoseconds_to_absolutetime(NSEC_PER_USEC * WRITE_TIMEOUT2_US,
                              &write_timeout2_abs);

  int err = ml_io_increase_timeouts(iovaddr_base1, 0, READ_TIMEOUT1_US,
                                    WRITE_TIMEOUT1_US);
  T_EXPECT_EQ_INT(err, KERN_INVALID_ARGUMENT,
                  "Can't set timeout for empty region");

  err = ml_io_increase_timeouts(iovaddr_base1, 4097, READ_TIMEOUT1_US,
                                WRITE_TIMEOUT1_US);
  T_EXPECT_EQ_INT(err, KERN_INVALID_ARGUMENT,
                  "Can't set timeout for region > 4096 bytes");

  err = ml_io_increase_timeouts(UINTPTR_MAX, SIZE, READ_TIMEOUT1_US,
                                WRITE_TIMEOUT1_US);
  T_EXPECT_EQ_INT(err, KERN_INVALID_ARGUMENT,
                  "Can't set timeout for overflowed region");

  err = ml_io_increase_timeouts(iovaddr_base1, SIZE, READ_TIMEOUT1_US,
                                WRITE_TIMEOUT1_US);
  T_EXPECT_EQ_INT(err, KERN_SUCCESS,
                  "Setting timeout for first VA region should succeed");

  err = ml_io_increase_timeouts(iovaddr_base2, SIZE, READ_TIMEOUT2_US,
                                WRITE_TIMEOUT2_US);
  T_EXPECT_EQ_INT(err, KERN_SUCCESS,
                  "Setting timeout for second VA region should succeed");

  err = ml_io_increase_timeouts(iovaddr_base1, SIZE, READ_TIMEOUT1_US,
                                WRITE_TIMEOUT1_US);
  T_EXPECT_EQ_INT(err, KERN_INVALID_ARGUMENT,
                  "Can't set timeout for same region twice");

  err = ml_io_increase_timeouts(vaddr1, (uint32_t)(vaddr2 - vaddr1),
                                READ_TIMEOUT1_US, WRITE_TIMEOUT1_US);
  T_EXPECT_EQ_INT(err, KERN_INVALID_ARGUMENT,
                  "Can't set timeout for overlapping regions");

  uint64_t read_timeout, write_timeout;
  ml_io_timeout_test_get_timeouts(vaddr1, &read_timeout, &write_timeout);
  T_EXPECT_EQ_ULLONG(read_timeout, read_timeout1_abs,
                     "Read timeout for first region");
  T_EXPECT_EQ_ULLONG(write_timeout, write_timeout1_abs,
                     "Write timeout for first region");

  ml_io_timeout_test_get_timeouts(vaddr2, &read_timeout, &write_timeout);
  T_EXPECT_EQ_ULLONG(read_timeout, read_timeout2_abs,
                     "Read timeout for first region");
  T_EXPECT_EQ_ULLONG(write_timeout, write_timeout2_abs,
                     "Write timeout for first region");

  ml_io_timeout_test_get_timeouts(iovaddr_base2 + SIZE, &read_timeout,
                                  &write_timeout);
  ...
  T_EXPECT_EQ_INT(err, KERN_NOT_FOUND,
                  "Can't reset timeout for non-existent region");

  err = ml_io_reset_timeouts(iovaddr_base1, SIZE);
  T_EXPECT_EQ_INT(err, KERN_SUCCESS,
                  "Resetting timeout for first VA region should succeed");
  ...
  err = ml_io_reset_timeouts(iovaddr_base1, SIZE);
  T_EXPECT_EQ_INT(err, KERN_NOT_FOUND,
                  "Can't reset timeout for same region twice");

  err = ml_io_reset_timeouts(iovaddr_base2, SIZE);
  T_EXPECT_EQ_INT(err, KERN_SUCCESS,
                  "Resetting timeout for second VA region should succeed");

  err = ml_io_increase_timeouts_phys(iopaddr_base1, SIZE, READ_TIMEOUT1_US,
                                     WRITE_TIMEOUT1_US);
  T_EXPECT_EQ_INT(err, KERN_SUCCESS,
                  "Setting timeout for first PA region should succeed");

  err = ml_io_increase_timeouts_phys(iopaddr_base2, SIZE, READ_TIMEOUT2_US,
                                     WRITE_TIMEOUT2_US);
  T_EXPECT_EQ_INT(err, KERN_SUCCESS,
                  "Setting timeout for second PA region should succeed");
  ...
  T_EXPECT_EQ_ULLONG(read_timeout, read_timeout2_abs,
                     "Read timeout for first physical region");
  T_EXPECT_EQ_ULLONG(write_timeout, write_timeout2_abs,
                     "Write timeout for first physical region");

  err = ml_io_reset_timeouts_phys(iopaddr_base1, SIZE);
  T_EXPECT_EQ_INT(err, KERN_SUCCESS,
                  "Resetting timeout for first PA region should succeed");

  err = ml_io_reset_timeouts_phys(iopaddr_base2, SIZE);
  T_EXPECT_EQ_INT(err, KERN_SUCCESS,
                  "Resetting timeout for second PA region should succeed");
...
#if DEVELOPMENT || DEBUG
static int ml_io_read_cpu_reg_test(__unused int64_t in, int64_t *out) {
