...
struct thread_group {
  ...
  uint8_t tg_machine_data[]
      __attribute__((aligned(TG_MACHINE_DATA_ALIGN_SIZE)));
...
static void
thread_group_deallocate_queue_invoke(mpsc_queue_chain_t e,
...
void thread_group_init(void) {
  // Get thread group structure extension from EDT or boot-args (which can
  // override EDT)
  if (!PE_parse_boot_argn("kern.thread_group_extra_bytes",
                          &tg_machine_data_size,
                          sizeof(tg_machine_data_size))) {
    if (!PE_get_default("kern.thread_group_extra_bytes", &tg_machine_data_size,
                        sizeof(tg_machine_data_size))) {
      tg_machine_data_size = 8;
    }
  }

  if (!PE_parse_boot_argn("kern.perf_tg_no_dipi",
                          &perf_controller_thread_group_immediate_ipi,
                          sizeof(perf_controller_thread_group_immediate_ipi))) {
    if (!PE_get_default("kern.perf_tg_no_dipi",
                        &perf_controller_thread_group_immediate_ipi,
                        sizeof(perf_controller_thread_group_immediate_ipi))) {
  ...
  // Check if thread group can be set by voucher adoption from EDT or boot-args
  // (which can override EDT)
  if (!PE_parse_boot_argn("kern.thread_group_set_by_bankvoucher",
                          &tg_set_by_bankvoucher,
                          sizeof(tg_set_by_bankvoucher))) {
    if (!PE_get_default("kern.thread_group_set_by_bankvoucher",
                        &tg_set_by_bankvoucher,
                        sizeof(tg_set_by_bankvoucher))) {
  ...
  if (tg_size % TG_MACHINE_DATA_ALIGN_SIZE) {
    tg_size +=
        TG_MACHINE_DATA_ALIGN_SIZE - (tg_size % TG_MACHINE_DATA_ALIGN_SIZE);
  }
  tg_machine_data_size = tg_size - sizeof(struct thread_group);
  // printf("tg_size=%d(%lu+%d)\n", tg_size, sizeof(struct thread_group),
  // tg_machine_data_size);
  assert(offsetof(struct thread_group, tg_machine_data) %
             TG_MACHINE_DATA_ALIGN_SIZE ==
         0);
  ...
  tg_next_id++; // Skip ID 2, which used to be the "adaptive" group. (It was
                // never used.)
  ...
  tg_perf_controller =
      thread_group_create_and_retain(THREAD_GROUP_FLAGS_DEFAULT);
  ...
  mpsc_daemon_queue_init_with_thread_call(
      &thread_group_deallocate_queue, thread_group_deallocate_queue_invoke,
 ...
 */
sched_clutch_t sched_clutch_for_thread(thread_t thread) {
...
static inline bool thread_get_reevaluate_tg_hierarchy_locked(thread_t t) {
  return t->sched_flags & TH_SFLAG_REEVALUTE_TG_HIERARCHY_LATER;
}

static inline void thread_set_reevaluate_tg_hierarchy_locked(thread_t t) {
  t->sched_flags |= TH_SFLAG_REEVALUTE_TG_HIERARCHY_LATER;
}

static inline void thread_clear_reevaluate_tg_hierarchy_locked(thread_t t) {
  t->sched_flags &= ~TH_SFLAG_REEVALUTE_TG_HIERARCHY_LATER;
}
#endif
...
struct thread_group *thread_group_create_and_retain(uint32_t flags) {
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_NEW),
               thread_group_id(tg), thread_group_get_flags(tg));
  if (flags) {
    KDBG(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_FLAGS),
         thread_group_id(tg), thread_group_get_flags(tg), 0);
  }
...
void thread_group_init_thread(thread_t t, task_t task) {
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_SET),
               THREAD_GROUP_INVALID, thread_group_id(tg),
 ...
 */
void thread_group_set_name(__unused struct thread_group *tg,
...
#if defined(__LP64__)
    KDBG(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_NAME), tg->tg_id,
...
void thread_group_set_flags_locked(struct thread_group *tg, uint32_t flags) {
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_FLAGS), tg->tg_id,
       tg->tg_flags, old_flags);
}
...
static void thread_group_deallocate_complete(struct thread_group *tg) {
  lck_mtx_lock(&tg_lock);
  tg_count--;
  remqueue(&tg->tg_queue_chain);
  lck_mtx_unlock(&tg_lock);
  static_assert(THREAD_GROUP_MAXNAME >= (sizeof(uint64_t) * 3),
                "thread group name is too short");
  static_assert(__alignof(struct thread_group) >= __alignof(uint64_t),
                "thread group name is not 8 bytes aligned");
#if defined(__LP64__)
  KDBG(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_NAME_FREE),
       tg->tg_id, *(uint64_t *)(void *)&tg->tg_name[0],
       *(uint64_t *)(void *)&tg->tg_name[sizeof(uint64_t)],
       *(uint64_t *)(void *)&tg->tg_name[sizeof(uint64_t) * 2]);
#else  /* defined(__LP64__) */
  KDBG(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_NAME_FREE),
       tg->tg_id, *(uint32_t *)(void *)&tg->tg_name[0],
       *(uint32_t *)(void *)&tg->tg_name[sizeof(uint32_t)],
       *(uint32_t *)(void *)&tg->tg_name[sizeof(uint32_t) * 2]);
#endif /* defined(__LP64__) */
  machine_thread_group_deinit(tg);
...
#endif /* CONFIG_SCHED_CLUTCH */
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_FREE),
               tg->tg_id);
  zfree(tg_zone, tg);
}
...
thread_group_deallocate_queue_invoke(mpsc_queue_chain_t e,
  ...
  struct thread_group *tg =
      mpsc_queue_element(e, struct thread_group, tg_destroy_link);
 ...
 */
inline struct thread_group *thread_group_get(thread_t t) {
 ...
 */
static bool
thread_compute_resolved_thread_group(thread_t t,
                                     struct thread_group **resolved_tg) {
 ...
 */
static inline void thread_assert_has_valid_thread_group(thread_t t) {
 ...
 */
static void thread_mark_thread_group_hierarchy_resolved(thread_t __unused t) {
#if CONFIG_PREADOPT_TG
  /*
 ...
 */
static void
thread_notify_thread_group_change_self(thread_t t,
                                       struct thread_group *__unused old_tg,
  ...
  uint64_t arg1, arg2;
  machine_thread_going_on_core(t, thread_get_urgency(t, &arg1, &arg2), 0, 0,
                               ctime);
 ...
 */
static void thread_set_resolved_thread_group(thread_t t,
                                             struct thread_group *old_tg,
                                             struct thread_group *resolved_tg,
                                             bool on_self) {
  ...
  if (resolved_tg == t->preadopt_thread_group) {
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_PREADOPT),
        thread_group_id(old_tg), thread_group_id(resolved_tg), thread_tid(t),
        thread_group_id(home_tg));
  }
#endif
  ...
  sched_clutch_t new_clutch =
      (resolved_tg) ? &(resolved_tg->tg_sched_clutch) : NULL;
...
static void thread_resolve_thread_group_hierarchy_self_locked(
    thread_t t, __unused bool clear_preadopt) {
  assert(current_thread() == t);
  ...
  if (clear_preadopt) {
    if (t->preadopt_thread_group) {
      KDBG_RELEASE(
          MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_PREADOPT_CLEAR),
          (uintptr_t)thread_tid(t), thread_group_id(t->preadopt_thread_group),
          0, 0);
...
void thread_set_preadopt_thread_group(thread_t t, struct thread_group *tg) {
  ...
  if (t != current_thread()) {
    ...
    if (thread_run_queue_remove(t)) {
      ...
      KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_THREAD_GROUP,
                                MACH_THREAD_GROUP_PREADOPT_NEXTTIME),
                   thread_group_id(old_tg), thread_group_id(tg),
                   (uintptr_t)thread_tid(t), thread_group_id(home_tg));
    }
  } else {
    ...
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_THREAD_GROUP, MACH_THREAD_GROUP_PREADOPT),
        thread_group_id(old_tg), thread_group_id(tg), thread_tid(t),
        thread_group_id(home_tg));
  }
...
out:
  if (thread_get_reevaluate_tg_hierarchy_locked(t)) {
     ...
     */
    assert(t->thread_group == home_tg || t->thread_group == old_preadopt_tg ||
 ...
 * groups only.
 */
static void thread_set_thread_group(thread_t t, struct thread_group *tg) {
  struct thread_group *home_tg = thread_group_get_home_group(t);
 ...
 *
 * Preconditions:
 * - Thread must not be part of a runq (freshly made runnable threads or
 * terminating only)
 * - Thread must be locked by the caller already
 */
void thread_set_autojoin_thread_group_locked(thread_t t,
                                             struct thread_group *tg) {
  thread_assert_runq_null(t);
  ...
  if (needs_change) {
    ...
    if (t == current_thread()) {
      ...
      machine_thread_going_on_core(t, thread_get_urgency(t, &arg1, &arg2), 0, 0,
                                   ctime);
      machine_switch_perfcontrol_state_update(
          THREAD_GROUP_UPDATE, ctime, PERFCONTROL_CALLOUT_WAKE_UNSAFE, t);
    }
  }
...
void thread_set_work_interval_thread_group(thread_t t,
                                           struct thread_group *tg) {
  assert(current_thread() == t);
...
inline cluster_type_t thread_group_recommendation(struct thread_group *tg) {
  if (tg == NULL) {
...
inline uint64_t thread_group_get_id(struct thread_group *tg) {
  return tg->tg_id;
}

uint32_t thread_group_count(void) { return tg_count; }

/*
 * Can only be called while tg cannot be destroyed
 */
inline const char *thread_group_get_name(struct thread_group *tg) {
  return tg->tg_name;
}

inline void *thread_group_get_machine_data(struct thread_group *tg) {
  return &tg->tg_machine_data;
}

inline uint32_t thread_group_machine_data_size(void) {
  return tg_machine_data_size;
}

inline boolean_t thread_group_uses_immediate_ipi(struct thread_group *tg) {
  return thread_group_get_id(tg) == THREAD_GROUP_PERF_CONTROLLER &&
         perf_controller_thread_group_immediate_ipi != 0;
}

kern_return_t thread_group_iterate_stackshot(thread_group_iterate_fn_t callout,
                                             void *arg) {
  struct thread_group *tg;
  int i = 0;
  qe_foreach_element(tg, &tg_queue, tg_queue_chain) {
    if (tg == NULL ||
        !ml_validate_nofault((vm_offset_t)tg, sizeof(struct thread_group))) {
      return KERN_FAILURE;
    }
    callout(arg, i, tg);
    i++;
...
void thread_group_join_cellular(void) {
  struct thread_group *tg =
      thread_group_find_by_id_and_retain(THREAD_GROUP_CELLULAR);
  assert(tg != NULL);
...
void thread_group_vm_add(void) {
  assert(tg_vm != NULL);
  thread_set_thread_group(current_thread(),
                          thread_group_find_by_id_and_retain(THREAD_GROUP_VM));
}

uint32_t thread_group_get_flags(struct thread_group *tg) {
  return tg->tg_flags;
}

void thread_group_update_recommendation(struct thread_group *tg,
...
OS_NORETURN
void sched_perfcontrol_thread_group_recommend(
    __unused void *machine_data, __unused cluster_type_t new_recommendation) {
  panic("sched_perfcontrol_thread_group_recommend() not supported on the Edge "
        "scheduler");
...
/*
 * Iterate through indices of the edge matrix (dimension: num_psets X num_psets
 * X TH_BUCKET_SCHED_MAX), and along the way, compute the corresponding index in
 * CLPC's version of the matrix, which has dimension: num_psets X num_psets X
 * PERFCONTROL_CLASS_MAX
 */
#define sched_perfcontrol_sched_edge_matrix_iterate(num_psets, edge_ind,       \
                                                    sched_ind, ...)            \
  for (int src = 0; src < num_psets; src++) {                                  \
    for (int dst = 0; dst < num_psets; dst++) {                                \
      for (sched_bucket_t bucket = 0; bucket < TH_BUCKET_SCHED_MAX;            \
           bucket++) {                                                         \
        perfcontrol_class_t pc = sched_bucket_to_perfcontrol_class(bucket);    \
        int edge_ind = (src * (int)num_psets * PERFCONTROL_CLASS_MAX) +        \
                       (dst * PERFCONTROL_CLASS_MAX) + pc;                     \
        int sched_ind = (src * (int)num_psets * TH_BUCKET_SCHED_MAX) +         \
                        (dst * TH_BUCKET_SCHED_MAX) + bucket;                  \
        __VA_ARGS__;                                                           \
      }                                                                        \
    }                                                                          \
  }
...
static uint64_t rt_config_edge_index(uint64_t src_pset_id, uint64_t dst_pset_id,
                                     uint64_t num_psets) {
  return (src_pset_id * num_psets * PERFCONTROL_CLASS_MAX) +
         (dst_pset_id * PERFCONTROL_CLASS_MAX) + PERFCONTROL_CLASS_REALTIME;
}

void sched_perfcontrol_edge_matrix_by_qos_get(
  ...
  sched_perfcontrol_sched_edge_matrix_iterate(
      num_psets, edge_matrix_ind, sched_matrix_ind, {
  ...
  for (uint src = 0; src < num_psets; src++) {
    for (uint dst = 0; dst < num_psets; dst++) {
      const uint64_t edge_matrix_index =
          rt_config_edge_index(src, dst, num_psets);
...
void sched_perfcontrol_edge_matrix_by_qos_set(
  ...
  for (uint src = 0; src < num_psets; src++) {
    for (uint dst = 0; dst < num_psets; dst++) {
      const uint64_t edge_matrix_ind =
          rt_config_edge_index(src, dst, num_psets);
...
void sched_perfcontrol_edge_matrix_get(sched_clutch_edge *edge_matrix,
                                       bool *edge_requested, uint64_t flags,
  ...
  sched_edge_matrix_get(expanded_matrix, edge_requested_per_qos, flags,
                        matrix_order);
...
void sched_perfcontrol_edge_matrix_set(sched_clutch_edge *edge_matrix,
                                       bool *edge_changed, uint64_t flags,
                                       uint64_t matrix_order) {
  assert3u(matrix_order, <=, MAX_PSETS);
  ...
  sched_edge_matrix_set(expanded_matrix, edge_changed_per_qos, flags,
                        matrix_order);
}

void sched_perfcontrol_thread_group_preferred_clusters_set(
  ...
  struct thread_group *tg =
      (struct thread_group *)((uintptr_t)machine_data -
                              offsetof(struct thread_group, tg_machine_data));
  uint32_t tg_bucket_preferred_cluster[TH_BUCKET_SCHED_MAX];
  for (sched_bucket_t bucket = 0; bucket < TH_BUCKET_SCHED_MAX; bucket++) {
    perfcontrol_class_t pc = sched_bucket_to_perfcontrol_class(bucket);
    tg_bucket_preferred_cluster[bucket] =
        (overrides[pc] != SCHED_PERFCONTROL_PREFERRED_CLUSTER_OVERRIDE_NONE)
            ? overrides[pc]
            : tg_preferred_cluster;
  }
  sched_edge_tg_preferred_cluster_change(tg, tg_bucket_preferred_cluster,
                                         options);
}

void sched_perfcontrol_edge_cpu_rotation_bitmasks_set(
...
void sched_perfcontrol_edge_matrix_by_qos_get(
    __unused sched_clutch_edge *edge_matrix, __unused bool *edge_requested,
    __unused uint64_t flags, __unused uint64_t num_psets,
    __unused uint64_t num_classes) {}

void sched_perfcontrol_edge_matrix_by_qos_set(
    __unused sched_clutch_edge *edge_matrix, __unused bool *edge_changed,
    __unused uint64_t flags, __unused uint64_t num_psets,
    __unused uint64_t num_classes) {}

void sched_perfcontrol_edge_matrix_get(__unused sched_clutch_edge *edge_matrix,
                                       __unused bool *edge_request_bitmap,
                                       __unused uint64_t flags,
                                       __unused uint64_t matrix_order) {}

void sched_perfcontrol_edge_matrix_set(__unused sched_clutch_edge *edge_matrix,
                                       __unused bool *edge_changes_bitmap,
                                       __unused uint64_t flags,
                                       __unused uint64_t matrix_order) {}

void sched_perfcontrol_thread_group_preferred_clusters_set(
    __unused void *machine_data, __unused uint32_t tg_preferred_cluster,
    __unused uint32_t overrides[PERFCONTROL_CLASS_MAX],
...
/*
 * Can only be called while tg cannot be destroyed.
 * Names can be up to THREAD_GROUP_MAXNAME long and are not necessarily
 * null-terminated.
 */
const char *sched_perfcontrol_thread_group_get_name(void *machine_data) {
  struct thread_group *tg =
      __container_of(machine_data, struct thread_group, tg_machine_data);
