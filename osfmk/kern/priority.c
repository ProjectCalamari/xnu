...
void thread_quantum_expire(timer_call_param_t p0, timer_call_param_t p1) {
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_QUANTUM_EXPIRED) |
               DBG_FUNC_START);
   ...
   */
  ledger_credit(thread->t_ledger, task_ledgers.cpu_time,
                thread->quantum_remaining);
  ledger_credit(thread->t_threadledger, thread_ledgers.cpu_time,
                thread->quantum_remaining);
  if (thread->t_bankledger) {
    ledger_credit(
        thread->t_bankledger, bank_ledgers.cpu_time,
  ...
  sched_update_pset_avg_execution_time(processor->processor_set,
                                       thread->quantum_remaining, ctime,
  ...
  if ((thread->sched_mode == TH_MODE_REALTIME ||
     ...
     * control.
     */
    new_computation -= recount_current_thread_interrupt_time_mach() -
                       thread->computation_interrupt_epoch;
    ...
    if (demote) {
      KERNEL_DEBUG_CONSTANT(
          MACHDBG_CODE(DBG_MACH_SCHED, MACH_FAILSAFE) | DBG_FUNC_NONE,
  ...
  machine_switch_perfcontrol_state_update(QUANTUM_EXPIRY, ctime, 0, thread);
  ...
  KERNEL_DEBUG_CONSTANT(
      MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_QUANTUM_EXPIRED) | DBG_FUNC_END,
      preempt, 0, 0, 0, 0);
}
 ...
 */
void sched_set_thread_base_priority(thread_t thread, int priority) {
  ...
  if (thread == current_thread() && old_base_pri != priority) {
    ...
    machine_switch_perfcontrol_state_update(PERFCONTROL_ATTR_UPDATE, ctime,
 ...
 */
void sched_set_kernel_thread_priority(thread_t thread, int new_priority) {
  spl_t s = splsched();
 ...
 *
 *	TODO: Add an 'update urgency' flag to avoid urgency callouts on every
 * rwlock operation
 */
void thread_recompute_sched_pri(thread_t thread,
...
void sched_default_quantum_expire(thread_t thread __unused) {
  /*
...
const struct shift_data sched_decay_shifts[SCHED_DECAY_TICKS] = {
    {.shift1 = 1, .shift2 = 1},    {.shift1 = 1, .shift2 = 3},
    {.shift1 = 1, .shift2 = -3},   {.shift1 = 2, .shift2 = -7},
    {.shift1 = 3, .shift2 = 5},    {.shift1 = 3, .shift2 = -5},
    {.shift1 = 4, .shift2 = -8},   {.shift1 = 5, .shift2 = 7},
    {.shift1 = 5, .shift2 = -7},   {.shift1 = 6, .shift2 = -10},
    {.shift1 = 7, .shift2 = 10},   {.shift1 = 7, .shift2 = -9},
    {.shift1 = 8, .shift2 = -11},  {.shift1 = 9, .shift2 = 12},
    {.shift1 = 9, .shift2 = -11},  {.shift1 = 10, .shift2 = -13},
    {.shift1 = 11, .shift2 = 14},  {.shift1 = 11, .shift2 = -13},
    {.shift1 = 12, .shift2 = -15}, {.shift1 = 13, .shift2 = 17},
    {.shift1 = 13, .shift2 = -15}, {.shift1 = 14, .shift2 = -17},
    {.shift1 = 15, .shift2 = 19},  {.shift1 = 16, .shift2 = 18},
    {.shift1 = 16, .shift2 = -19}, {.shift1 = 17, .shift2 = 22},
    {.shift1 = 18, .shift2 = 20},  {.shift1 = 18, .shift2 = -20},
    {.shift1 = 19, .shift2 = 26},  {.shift1 = 20, .shift2 = 22},
    {.shift1 = 20, .shift2 = -22}, {.shift1 = 21, .shift2 = -27}};
...
int sched_compute_timeshare_priority(thread_t thread) {
  int decay_amount;
 ...
 *	can_update_priority
 *
 *	Make sure we don't do re-dispatches more frequently than a scheduler
 * tick.
 *
 *	Called with the thread locked.
 */
...
#if CONFIG_SCHED_CLUTCH
  thread->pri_shift =
      sched_clutch_thread_pri_shift(thread, thread->th_sched_bucket);
...
static void sched_incr_bucket(sched_bucket_t bucket) {
  assert(bucket >= TH_BUCKET_FIXPRI && bucket <= TH_BUCKET_SHARE_BG);

  os_atomic_inc(&sched_run_buckets[bucket], relaxed);
}

static void sched_decr_bucket(sched_bucket_t bucket) {
  assert(bucket >= TH_BUCKET_FIXPRI && bucket <= TH_BUCKET_SHARE_BG);
...
static void sched_add_bucket(sched_bucket_t bucket, uint8_t run_weight) {
  assert(bucket >= TH_BUCKET_FIXPRI && bucket <= TH_BUCKET_SHARE_BG);
...
uint32_t sched_run_incr(thread_t thread) {
...
uint32_t sched_run_decr(thread_t thread) {
  assert((thread->state & (TH_RUN | TH_IDLE)) != TH_RUN);
...
uint32_t sched_smt_run_incr(thread_t thread) {
  assert((thread->state & (TH_RUN | TH_IDLE)) == TH_RUN);
  ...
  uint32_t new_count =
      os_atomic_add(&sched_run_buckets[TH_BUCKET_RUN], run_weight, relaxed);
...
uint32_t sched_smt_run_decr(thread_t thread) {
  assert((thread->state & (TH_RUN | TH_IDLE)) != TH_RUN);
  ...
  uint32_t new_count =
      os_atomic_sub(&sched_run_buckets[TH_BUCKET_RUN], run_weight, relaxed);

  return new_count;
}

void sched_update_thread_bucket(thread_t thread) {
...
void sched_smt_update_thread_bucket(thread_t thread) {
  sched_bucket_t old_bucket = thread->th_sched_bucket;
  sched_bucket_t new_bucket = TH_BUCKET_RUN;
...
static inline void sched_validate_mode(sched_mode_t mode) {
  switch (mode) {
  case TH_MODE_FIXED:
  case TH_MODE_REALTIME:
  case TH_MODE_TIMESHARE:
 ...
 */
void sched_set_thread_mode(thread_t thread, sched_mode_t new_mode) {
  thread_assert_runq_null(thread);
   ...
   */
  if ((thread->sched_flags & TH_SFLAG_THREAD_GROUP_AUTO_JOIN) &&
      (new_mode != TH_MODE_REALTIME)) {
    assert((thread->sched_mode == TH_MODE_REALTIME) ||
           (thread->th_work_interval_flags &
            TH_WORK_INTERVAL_FLAGS_AUTO_JOIN_LEAK));
 ...
 */
void sched_set_thread_mode_user(thread_t thread, sched_mode_t new_mode) {
  thread_assert_runq_null(thread);
...
sched_mode_t sched_get_thread_mode_user(thread_t thread) {
  if (thread->sched_flags & TH_SFLAG_DEMOTED_MASK) {
 ...
 */
void sched_thread_mode_demote(thread_t thread, uint32_t reason) {
 ...
 */
bool sched_thread_mode_has_demotion(thread_t thread, uint32_t reason) {
  assert(reason & TH_SFLAG_DEMOTED_MASK);
  return (thread->sched_flags & reason) != 0;
}

/*
 * Un-demote the true scheduler mode back to the saved mode (called with the
 * thread locked)
 */
void sched_thread_mode_undemote(thread_t thread, uint32_t reason) {
  assert(reason & TH_SFLAG_DEMOTED_MASK);
  assert((thread->sched_flags & reason) == reason);
  assert(thread->saved_mode != TH_MODE_NONE);
 ...
 */
void sched_thread_promote_reason(thread_t thread, uint32_t reason,
                                 __kdebug_only uintptr_t
                                     trace_obj /* already unslid */) {
  assert(reason & TH_SFLAG_PROMOTE_REASON_MASK);
  assert((thread->sched_flags & reason) != reason);

  switch (reason) {
  case TH_SFLAG_RW_PROMOTED:
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_RW_PROMOTE), thread_tid(thread),
         thread->sched_pri, thread->base_pri, trace_obj);
    break;
  case TH_SFLAG_WAITQ_PROMOTED:
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_WAITQ_PROMOTE), thread_tid(thread),
         thread->sched_pri, thread->base_pri, trace_obj);
    break;
  case TH_SFLAG_EXEC_PROMOTED:
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_EXEC_PROMOTE), thread_tid(thread),
         thread->sched_pri, thread->base_pri, trace_obj);
    break;
  case TH_SFLAG_FLOOR_PROMOTED:
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_FLOOR_PROMOTE), thread_tid(thread),
         thread->sched_pri, thread->base_pri, trace_obj);
    break;
  }
 ...
 * Called at splsched with thread locked
 */
void sched_thread_unpromote_reason(thread_t thread, uint32_t reason,
                                   __kdebug_only uintptr_t
                                       trace_obj /* already unslid */) {
  assert(reason & TH_SFLAG_PROMOTE_REASON_MASK);
  assert((thread->sched_flags & reason) == reason);
  ...
  switch (reason) {
  case TH_SFLAG_RW_PROMOTED:
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_RW_DEMOTE), thread_tid(thread),
         thread->sched_pri, thread->base_pri, trace_obj);
    break;
  case TH_SFLAG_WAITQ_PROMOTED:
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_WAITQ_DEMOTE), thread_tid(thread),
         thread->sched_pri, thread->base_pri, trace_obj);
    break;
  case TH_SFLAG_EXEC_PROMOTED:
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_EXEC_DEMOTE), thread_tid(thread),
         thread->sched_pri, thread->base_pri, trace_obj);
    break;
  case TH_SFLAG_FLOOR_PROMOTED:
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_FLOOR_DEMOTE), thread_tid(thread),
         thread->sched_pri, thread->base_pri, trace_obj);
    break;
  }
