...
typedef void (*pmsSetFunc_t)(
    uint32_t, uint32_t,
    uint32_t); /* Function used to set hardware power state */
typedef uint32_t (*pmsQueryFunc_t)(
...
#define pmsSync                                                                \
  0x00400000 /* Make changes synchronously, i.e., spin until delay finished */
...
#define pmsDelay                                                               \
  0xFFFFFFFD /* Delayed step, no processor or platform changes.  Timer         \
                expiration causes transition to pmsTDelay */
#define pmsParkIt                                                              \
  0xFFFFFFFF /* Enters the parked state.  No processor or platform changes.    \
                Timers cancelled */
#define pmsCInit                                                               \
  ((pmsXUnk << 24) | (pmsCPUUnk << 16) |                                       \
   (pmsVoltUnk << 8)) /* Initial current set command value */
  /*	Note:  pmsSetFuncInd is an index into a table of function pointers and
   * pmsSetFunc is the address of a function.  Initially, when you create a step
   * table, this field is set as an index into a table of function addresses
   * that gets passed as a parameter to pmsBuild.  When pmsBuild internalizes
   * the step and function tables, it converts the index to the function
   * address.
   */
  union sf {
...
typedef struct pmsCtl {
  pmsStat (*pmsStats)[pmsMaxStates]; /* Pointer to statistics information, 0 if
                                        not enabled */
  pmsDef *pmsDefs[pmsMaxStates];     /* Indexed pointers to steps */
} pmsCtl;

/*
 *	Note that this block is in the middle of the per_proc and the size (32
 * bytes) can't be changed without moving it.
 */
...
extern kern_return_t pmsControl(uint32_t request, user_addr_t reqaddr,
                                uint32_t reqsize);
...
extern kern_return_t pmsBuild(pmsDef *pd, uint32_t pdsize,
                              pmsSetFunc_t *functab, uint32_t platformData,
                              pmsQueryFunc_t queryFunc);
extern void pmsRun(uint32_t nstep);
extern void pmsPark(void);
extern void pmsStart(void);
extern kern_return_t pmsCPULoadVIDTable(uint16_t *tablep,
                                        int nstates); /* i386 only */
