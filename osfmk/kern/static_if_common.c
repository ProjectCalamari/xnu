...
extern struct static_if_entry
    __static_if_entries[] __SECTION_START_SYM(STATIC_IF_SEGMENT,
                                              STATIC_IF_SECTION);

extern struct static_if_entry
    __static_if_entries_end[] __SECTION_END_SYM(STATIC_IF_SEGMENT,
                                                STATIC_IF_SECTION);

extern static_if_initializer
    __static_if_initializer_entries[] __SECTION_START_SYM(
        STATIC_IF_SEGMENT, STATIC_IFINIT_SECTION);

extern static_if_initializer
    __static_if_initializer_entries_end[] __SECTION_END_SYM(
        STATIC_IF_SEGMENT, STATIC_IFINIT_SECTION);
...
__attribute__((always_inline)) static inline bool isargsep(char c) {
  return c == ' ' || c == '\0' || c == '\t';
}

__attribute__((always_inline)) static const char *skip_seps(const char *s) {
  while (*s && isargsep(*s)) {
    s++;
  }
  return s;
...
__attribute__((always_inline)) static const char *skip_to_sep(const char *s) {
  while (!isargsep(*s)) {
    s++;
  }
  return s;
...
MARK_AS_FIXUP_TEXT uint64_t static_if_boot_arg_uint64(const char *args,
                                                      const char *key,
                                                      uint64_t defval) {
  uint64_t ret = defval;
 ...
 * (b) an asm goto() statement might jump to a label or fall through depending
 * on the state of the key (implemented with STATIC_IF_{NOP,JUMP}), (c) in the
 * fall through code, the variable is set to 1 (resp 0).
 *
...
MARK_AS_FIXUP_TEXT void static_if_init(const char *args) {
...
MARK_AS_FIXUP_TEXT void __static_if_key_delta(static_if_key_t key, int delta) {
  /*
  ...
  if (was_enabled != (key->sik_enable_count >= 0)) {
    ...
    while (sie) {
      ml_static_if_entry_patch(sie, (was_enabled == init_enabled) ^
