...
void sched_pset_search_order_compute(
  ...
  os_atomic_store_wide(&search_order_out->spso_packed, search_order.spso_packed,
                       relaxed);
}
...
void sched_pset_search_order_init(processor_set_t src_pset,
  ...
  os_atomic_store_wide(&search_order_out->spso_packed, spill_order.spso_packed,
                       relaxed);
}
  ...
  while (istate->spis_search_index < num_psets - 1) {
    ...
    if (istate->spis_search_index == -1) {
      ...
      istate->spis_cached_search_order =
          (sched_pset_search_order_t)os_atomic_load_wide(
              &search_order->spso_packed, relaxed);
    } else {
      pset_id = istate->spis_cached_search_order
                    .spso_search_order[istate->spis_search_index];
