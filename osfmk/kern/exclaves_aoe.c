...
static kern_return_t exclaves_aoe_boot(void) {
  exclaves_id_t aoeproxy_id =
      exclaves_service_lookup(EXCLAVES_DOMAIN_KERNEL, EXCLAVES_AOE_PROXY);
  ...
  tb_error_t ret = exclavesmessagequeueproxy_exclavesmessagequeueproxy__init(
      &aoeproxy_client, ep);
...
kern_return_t exclaves_aoe_setup(uint8_t *num_message, uint8_t *num_worker) {
    ...
    ret = exclavesmessagequeueproxy_exclavesmessagequeueproxy_setup(
        ...
        ^(exclavesmessagequeueproxy_exclavesmessagequeueproxy_setup__result_s
          ...
          exclavesmessagequeuetypes_workercount_s *wc =
              exclavesmessagequeueproxy_exclavesmessagequeueproxy_setup__result_get_success(
                  &result);
          if (wc != NULL) {
          ...
          exclavesmessagequeueproxy_proxyerror_s *error =
              exclavesmessagequeueproxy_exclavesmessagequeueproxy_setup__result_get_failure(
                  &result);
          assert3p(error, !=, NULL);

          exclaves_debug_printf(
              show_errors, "AOE setup failed for service: %llu (error: %llu)\n",
...
static bool exclaves_aoe_service_is_idle(const aoe_item_t *const item) {
  return item->aoei_message_count == 0 && item->aoei_work_count == 0 &&
         item->aoei_worker_count == 0;
}

static void
exclaves_aoe_service_try_take_assertion(exclaves_resource_t *const conclave,
                                        aoe_item_t *const item) {
  assert3p(conclave, !=, NULL);
  ...
  if (item->aoei_assertion_id == 0 && exclaves_aoe_service_is_idle(item)) {
    ...
    __assert_only IOReturn ret =
        IOExclaveLPWCreateAssertion(&item->aoei_assertion_id, desc);
    assert3u(ret, ==, kIOReturnSuccess);
  }
}

static void exclaves_aoe_service_drop_assertion(
    exclaves_resource_t *const __assert_only conclave, aoe_item_t *const item) {
  assert3p(conclave, !=, NULL);
  LCK_MTX_ASSERT(&conclave->r_mutex, LCK_MTX_ASSERT_OWNED);

  __assert_only IOReturn ret =
      IOExclaveLPWReleaseAssertion(item->aoei_assertion_id);
  assert3u(ret, ==, kIOReturnSuccess);
  item->aoei_assertion_id = 0;
}

static void exclaves_aoe_service_try_drop_assertion(
    exclaves_resource_t *const __assert_only conclave, aoe_item_t *const item) {
  assert3p(conclave, !=, NULL);
  LCK_MTX_ASSERT(&conclave->r_mutex, LCK_MTX_ASSERT_OWNED);
...
void exclaves_aoe_teardown(void) {
  exclaves_resource_t *conclave = task_get_conclave(current_task());
  assert3p(conclave, !=, NULL);
  ...
  while (!queue_empty(&conclave->r_conclave.c_aoe_q)) {
    queue_remove_first(&conclave->r_conclave.c_aoe_q, aitem, aoe_item_t *,
                       aoei_chain);
...
exclaves_aoe_claim_work(exclaves_resource_t *conclave,
  ...
  while (true) {
    ...
    assert_wait(&conclave->r_conclave.c_aoe_q, THREAD_INTERRUPTIBLE);
...
exclaves_aoe_post_work(exclaves_resource_t *conclave,
                       exclavesmessagequeuetypes_serviceidentifier_s service_id,
  ...
  queue_iterate(&conclave->r_conclave.c_aoe_q, aitem, aoe_item_t *,
    ...
    if (aitem->aoei_serviceid == service_id) {
      if (worker_count != 0) {
        ...
        // If there are no workers, check if the active assertion can be
        // dropped.
        exclaves_aoe_service_try_drop_assertion(conclave, aitem);
      }
      break;
 ...
 */
kern_return_t exclaves_aoe_work_loop(void) {
  uint64_t id = EXCLAVESMESSAGEQUEUETYPES_SERVICEIDENTIFIER_INVALID;
  exclaves_resource_t *conclave = task_get_conclave(current_task());
  assert3p(conclave, !=, NULL);
...
static wait_result_t exclaves_aoe_claim_message(exclaves_resource_t *conclave,
                                                aoe_item_t *item) {
  while (true) {
    lck_mtx_lock(&conclave->r_mutex);
    ...
    assert_wait(&item->aoei_message_count, THREAD_INTERRUPTIBLE);
...
static void exclaves_aoe_post_message(
...
static aoe_item_t *exclaves_aoe_associate_serviceid(void) {
  exclaves_resource_t *conclave = task_get_conclave(current_task());
  assert3p(conclave, !=, NULL);
...
kern_return_t exclaves_aoe_message_loop(void) {
  exclaves_resource_t *conclave = task_get_conclave(current_task());
  assert3p(conclave, !=, NULL);
  ...
  // Wait to be interrupted or aborted..
  while (exclaves_aoe_claim_message(conclave, item) != THREAD_INTERRUPTED) {
    // Call into AOE proxy to handle message.

    /* BEGIN IGNORE CODESTYLE */
    __assert_only tb_error_t ret =
        exclavesmessagequeueproxy_exclavesmessagequeueproxy_messagedeliver(
            ...
            ^(workercount__opt_s wc_opt) {
              ...
              exclaves_aoe_post_work(conclave, item->aoei_serviceid,
                                     wc ? *wc : 0);
            });
    /* END IGNORE CODESTYLE */
...
exclaves_aoe_upcall_work_available(const xnuupcallsv2_aoeworkinfo_s *work_info,
  ...
  exclaves_resource_t *conclave = exclaves_conclave_lookup_by_aoeserviceid(id);
  if (conclave == NULL || queue_empty(&conclave->r_conclave.c_aoe_q)) {
