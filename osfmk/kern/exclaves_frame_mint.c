 ...
 */
static kern_return_t exclaves_frame_mint_init(void) {
  exclaves_id_t id =
      exclaves_service_lookup(EXCLAVES_DOMAIN_KERNEL, EXCLAVES_FRAME_MINT);
  ...
  if (tb_result != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(show_errors, "frame mint init: failure %u\n",
                          tb_result);
    return KERN_FAILURE;
  }
...
kern_return_t exclaves_frame_mint_populate(void) {
  ...
  tb_result = framemint_framemint_populate(
      &frame_mint_client, ^(framemint_framemint_populate__result_s result) {
        ...
        assert3p(error, !=, NULL);
        exclaves_debug_printf(show_errors, "frame mint failure: failure %u\n",
                              *error);
      });
