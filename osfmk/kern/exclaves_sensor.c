...
static inline exclaves_sensor_status_t eic_sensorstatus_to_sensor_status(
    exclaveindicatorcontroller_sensorstatusresponse_s status) {
  assert3u(status, >, 0);
  assert3u(status, <=,
           EXCLAVEINDICATORCONTROLLER_SENSORSTATUSRESPONSE_SENSOR_PENDING);
...
static kern_return_t exclaves_eic_init(void) {
  exclaves_id_t eic_id =
      exclaves_service_lookup(EXCLAVES_DOMAIN_KERNEL, EXCLAVES_EIC);
...
static kern_return_t exclaves_eic_tick_rate(uint64_t rate_hz) {
  ...
  tb_error_t ret =
      exclaveindicatorcontroller_sensorrequest_setindicatorrefreshrate(
          &eic_client, &rate,
...
exclaves_eic_sensor_copy(uint32_t buffer, uint64_t size1, uint64_t offset1,
                         uint64_t size2, uint64_t offset2,
                         exclaves_sensor_status_t *status) {
  assert3u(size1, >, 0);
  assert3p(status, !=, NULL);
...
static bool exclaves_sensor_tick(void) {
...
static inline bool valid_sensor(exclaves_sensor_type_t sensor_type) {
  switch (sensor_type) {
  case EXCLAVES_SENSOR_CAM:
  case EXCLAVES_SENSOR_MIC:
  case EXCLAVES_SENSOR_CAM_ALT_FACEID:
...
static void healthcheck_deadline(uint64_t *deadline, uint64_t *leeway) {
  const uint32_t interval = NSEC_PER_SEC / exclaves_display_healthcheck_rate_hz;
 ...
 */
static void exclaves_sensor_healthcheck(__unused void *param0,
  ...
  if (exclaves_sensor_tick()) {
    thread_call_enter_delayed_with_leeway(sensor_healthcheck_tcall, NULL,
                                          hc_deadline, hc_leeway,
                                          THREAD_CALL_DELAY_LEEWAY);
  }
}

static kern_return_t exclaves_sensor_init(void) {
  ...
  sensor_healthcheck_tcall = thread_call_allocate_with_priority(
      exclaves_sensor_healthcheck, NULL, THREAD_CALL_PRIORITY_KERNEL);
...
kern_return_t exclaves_display_healthcheck_rate(uint64_t __unused ns) {
...
kern_return_t exclaves_indicator_min_on_time_deadlines(
  ...
  // For now, only one version is supported. Return an error if libsyscall sends
  // us any other versions
...
kern_return_t exclaves_display_healthcheck_rate(__unused uint64_t ns) {
  return KERN_NOT_SUPPORTED;
}

kern_return_t exclaves_sensor_tick_rate(uint64_t __unused rate_hz) {
  return KERN_NOT_SUPPORTED;
}
