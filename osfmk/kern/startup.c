...
static void load_context(thread_t thread);
...
extern struct startup_entry
    startup_entries[] __SECTION_START_SYM(STARTUP_HOOK_SEGMENT,
                                          STARTUP_HOOK_SECTION);

extern struct startup_entry
    startup_entries_end[] __SECTION_END_SYM(STARTUP_HOOK_SEGMENT,
                                            STARTUP_HOOK_SECTION);

static struct startup_entry *__startup_data startup_entry_cur = startup_entries;

SECURITY_READ_ONLY_LATE(startup_subsystem_id_t)
startup_phase = STARTUP_SUB_NONE;
...
static inline void kernel_bootstrap_log(const char *message) {
...
static inline void kernel_bootstrap_thread_log(const char *message) {
  if ((startup_debug & STARTUP_DEBUG_VERBOSE) &&
      startup_phase >= STARTUP_SUB_KPRINTF) {
...
__startup_func static int startup_entry_cmp(const void *e1, const void *e2) {
...
__startup_func void kernel_startup_bootstrap(void) {
  /*
  ...
  if (n == 0) {
    panic("Section %s,%s missing", STARTUP_HOOK_SEGMENT, STARTUP_HOOK_SECTION);
  }
  if (((uintptr_t)startup_entries_end - (uintptr_t)startup_entries) %
      sizeof(struct startup_entry)) {
    panic("Section %s,%s has invalid size", STARTUP_HOOK_SEGMENT,
          STARTUP_HOOK_SECTION);
  }
...
__startup_func void kernel_startup_tunable_dt_source_init(
    const struct startup_tunable_dt_source_spec *spec) {
  DTEntry base;
  ...
  *spec->source_addr = STARTUP_SOURCE_DEFAULT;
  ...
  if (spec->dt_chosen_override) {
    ...
    if (chosen != NULL &&
        SecureDTLookupEntry(chosen, spec->dt_base, &chosen_base) == kSuccess) {
      base = chosen_base;
  ...
  if (base != NULL &&
      SecureDTGetProperty(base, spec->dt_name, (const void **)&data,
                          &data_size) == kSuccess) {
    if (data_size != spec->var_len) {
      panic("unexpected tunable size %u in DT entry %s/%s/%s", data_size,
            found_in_chosen ? "/chosen" : "", spec->dt_base, spec->dt_name);
    }
  ...
  if (spec->boot_arg_name != NULL) {
    if (PE_parse_boot_argn(spec->boot_arg_name, spec->var_addr,
                           spec->var_len)) {
      if (spec->var_is_bool) {
        *(bool *)spec->var_addr = *(uint8_t *)spec->var_addr;
      }
...
static void kernel_startup_log(startup_subsystem_id_t subsystem) {
...
__startup_func void event_register_handler(struct event_hdr *hdr) {
...
kernel_startup_initialize_upto(startup_subsystem_id_t upto) {
  ...
  while (cur < startup_entries_end && cur->subsystem <= upto) {
    if ((startup_debug & STARTUP_DEBUG_VERBOSE) &&
        startup_phase >= STARTUP_SUB_KPRINTF) {
      kprintf("%s[%d, rank %d]: %p(%p)\n", __func__, cur->subsystem, cur->rank,
              cur->func, cur->arg);
    }
  ...
  if ((startup_debug & STARTUP_DEBUG_VERBOSE) && upto >= STARTUP_SUB_KPRINTF) {
...
void kernel_bootstrap(void) {
  ...
  result = kernel_thread_create((thread_continue_t)kernel_bootstrap_thread,
 ...
 */
void secondary_cpu_main(void *machine_param) {
  processor_t processor = current_processor();
 ...
 *	Called at splsched.
 */
void processor_start_thread(void *machine_param,
 ...
 */
static void __attribute__((noreturn)) load_context(thread_t thread) {
  processor_t processor = current_processor();
...
void scale_setup(void) {
  ...
  pe_serverperfmode = PE_get_default("kern.serverperfmode", &pe_serverperfmode,
                                     sizeof(pe_serverperfmode));
  ...
  if (serverperfmode &&
      ((uint64_t)max_mem_actual >= (uint64_t)(16 * 1024 * 1024 * 1024ULL))) {
