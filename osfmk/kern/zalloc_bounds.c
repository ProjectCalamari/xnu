...
void kalloc_type_bound_checks(void) {
  ...
  ts_a = krealloc_type(struct turnstile, 10, 20, ts_a, Z_WAITOK | Z_REALLOCF);

  kfree_type(struct turnstile, 20, ts_a);

  ts_a = kalloc_type(struct turnstile, struct turnstile, 10, Z_WAITOK);

  ts_a = krealloc_type(struct turnstile, struct turnstile, 10, 20, ts_a,
                       Z_WAITOK | Z_REALLOCF);
