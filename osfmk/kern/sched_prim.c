...
TUNABLE(uint32_t, nonurgent_preemption_timer_us, "nonurgent_preemption_timer",
        50); /* microseconds */
...
TUNABLE(int, default_bg_preemption_rate, "bg_preempt",
...
TUNABLE_DEV_WRITEABLE(int, max_unsafe_rt_quanta, "max_unsafe_rt_quanta",
                      MAX_UNSAFE_RT_QUANTA);
TUNABLE_DEV_WRITEABLE(int, max_unsafe_fixed_quanta, "max_unsafe_fixed_quanta",
                      MAX_UNSAFE_FIXED_QUANTA);

TUNABLE_DEV_WRITEABLE(int, safe_rt_multiplier, "safe_rt_multiplier",
                      SAFE_RT_MULTIPLIER);
TUNABLE_DEV_WRITEABLE(int, safe_fixed_multiplier, "safe_fixed_multiplier",
                      SAFE_FIXED_MULTIPLIER);
...
#define TIMER_DEADLINE_TRACKING_BIN_1_DEFAULT                                  \
  2000000 /* Timers with deadlines <=                                          \
           * 2ms */
#define TIMER_DEADLINE_TRACKING_BIN_2_DEFAULT                                  \
  5000000 /* Timers with deadlines                                             \
           *   <= 5ms */
...
static struct global_powered_cores_state {
   ...
   */
  cpumap_t pcs_requested_online_user; /* updated by processor_start/exit from
                                         userspace */
  cpumap_t pcs_requested_online_clpc_user;
  cpumap_t pcs_requested_online_clpc_system;
  cpumap_t pcs_required_online_pmgr;   /* e.g. ANE needs these powered for their
                                          rail to be happy */
  cpumap_t pcs_required_online_system; /* e.g. smt1 for interrupts, boot
                                          processor unless boot arg is set,
                                          makes them disable instead of sleep */
  ...
  /*
   * Disable automatic cluster powerdown in favor of explicit user core online
   * control
   */
...
static void sched_update_recommended_cores_locked(processor_reason_t reason,
                                                  cpumap_t core_going_offline);
static void
sched_update_powered_cores_drops_lock(processor_reason_t requested_reason,
                                      spl_t s);
...
thread_t processor_idle(thread_t thread, processor_t processor);

static ast_t csw_check_locked(thread_t thread, processor_t processor,
                              processor_set_t pset, ast_t check_reason);

static void processor_setrun(processor_t processor, thread_t thread,
                             sched_options_t options);

static void sched_timer_deadline_tracking_init(void);

#if DEBUG
extern int debug_task;
#define TLOG(a, fmt, args...)                                                  \
  if (debug_task & a)                                                          \
  kprintf(fmt, ##args)
#else
#define TLOG(a, fmt, args...)                                                  \
...
static processor_t thread_bind_internal(thread_t thread, processor_t processor);

static void sched_vm_group_maintenance(void);
...
void sched_init(void) {
  ...
  if (!PE_parse_boot_argn("sched_pri_decay_limit", &sched_pri_decay_band_limit,
  ...
  if (PE_parse_boot_argn("sched_debug", &sched_debug_flags,
...
#if __arm64__
  clock_interval_to_absolutetime_interval(expecting_ipi_wfe_timeout_usec,
                                          NSEC_PER_USEC,
                                          &expecting_ipi_wfe_timeout_mt);
#endif /* __arm64__ */
  ...
  if (PE_parse_boot_argn("direct_handoff", &direct_handoff,
                         sizeof(direct_handoff))) {
    allow_direct_handoff = direct_handoff;
  }

#if DEVELOPMENT || DEBUG
  if (PE_parse_boot_argn("enable_skstsct", &enable_task_set_cluster_type,
                         sizeof(enable_task_set_cluster_type))) {
...
void sched_timeshare_init(void) {
  /*
...
void sched_set_max_unsafe_rt_quanta(int max) {
...
void sched_set_max_unsafe_fixed_quanta(int max) {
  const uint32_t quantum_size = SCHED(initial_quantum_size)(THREAD_NULL);
...
void sched_timeshare_timebase_init(void) {
  ...
  clock_interval_to_absolutetime_interval(std_quantum_us, NSEC_PER_USEC,
                                          &abstime);
  ...
  clock_interval_to_absolutetime_interval(bg_quantum_us, NSEC_PER_USEC,
                                          &abstime);
  assert((abstime >> 32) == 0 && (uint32_t)abstime != 0);
  ...
  clock_interval_to_absolutetime_interval(sched_load_compute_interval_us,
                                          NSEC_PER_USEC,
  ...
  if (nonurgent_preemption_timer_us) {
    clock_interval_to_absolutetime_interval(nonurgent_preemption_timer_us,
                                            NSEC_PER_USEC, &abstime);
...
void sched_check_spill(processor_set_t pset, thread_t thread) {
  (void)pset;
  (void)thread;
...
bool sched_thread_should_yield(processor_t processor, thread_t thread) {
  (void)thread;

  return !SCHED(processor_queue_empty)(processor) ||
         rt_runq_count(processor->processor_set) > 0;
}

/* Default implementations of .steal_thread_enabled */
bool sched_steal_thread_DISABLED(processor_set_t pset) {
  (void)pset;
  return false;
}

bool sched_steal_thread_enabled(processor_set_t pset) {
...
static void load_shift_init(void) {
  ...
  if (PE_parse_boot_argn("sched_decay_penalty", &sched_decay_penalty,
                         sizeof(sched_decay_penalty))) {
    kprintf("Overriding scheduler decay penalty %u\n", sched_decay_penalty);
  }

  if (PE_parse_boot_argn("sched_decay_usage_age_factor",
...
void check_monotonic_time(uint64_t ctime) {
 ...
 */
void thread_timer_expire(void *p0, __unused void *p1) {
...
boolean_t thread_unblock(thread_t thread, wait_result_t wresult) {
  ...
  if ((old_thread_state & TH_RUN) == 0) {
    ...
    if (aticontext == FALSE &&
        work_interval_should_propagate(cthread, thread)) {
  ...
  if (__improbable(aticontext &&
                   !(thread_get_tag_internal(thread) & THREAD_TAG_CALLOUT))) {
    DTRACE_SCHED2(iwakeup, struct thread *, thread, struct proc *,
                  current_proc());
 ...
 *	recommendations.
 */
boolean_t thread_allowed_for_handoff(thread_t thread) {
  thread_t self = current_thread();

  if (allow_direct_handoff && thread->sched_mode == TH_MODE_REALTIME &&
 ...
 */
void thread_go(thread_t thread, wait_result_t wresult, bool try_handoff) {
  thread_t self = current_thread();
...
thread_mark_wait_locked(thread_t thread, wait_interrupt_t interruptible_orig) {
  ...
  assert(!(thread->state & (TH_WAIT | TH_WAKING | TH_IDLE | TH_UNINT |
                            TH_TERMINATE2 | TH_WAIT_REPORT)));
  ...
  if (interruptible == THREAD_UNINT ||
      ...
      (!at_safe_point && (thread->sched_flags & TH_SFLAG_ABORTSAFELY))) {
 ...
 */
__private_extern__ wait_interrupt_t
thread_interrupt_level(wait_interrupt_t new_level) {
  thread_t thread = current_thread();
  wait_interrupt_t result = thread->options & TH_OPT_INTMASK;

  thread->options =
      (thread->options & ~TH_OPT_INTMASK) | (new_level & TH_OPT_INTMASK);
...
wait_result_t assert_wait(event_t event, wait_interrupt_t interruptible) {
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, MACHDBG_CODE(DBG_MACH_SCHED, MACH_WAIT) | DBG_FUNC_NONE,
      VM_KERNEL_UNSLIDE_OR_PERM(event), 0, 0, 0, 0);

  struct waitq *waitq;
  waitq = global_eventq(event);
  return waitq_assert_wait64(waitq, CAST_EVENT64_T(event), interruptible,
 ...
 */
struct waitq *assert_wait_queue(event_t event) { return global_eventq(event); }

wait_result_t assert_wait_timeout(event_t event, wait_interrupt_t interruptible,
                                  uint32_t interval, uint32_t scale_factor) {
  thread_t thread = current_thread();
  ...
  wresult = waitq_assert_wait64_locked(
      waitq, CAST_EVENT64_T(event), interruptible, TIMEOUT_URGENCY_SYS_NORMAL,
...
wait_result_t assert_wait_timeout_with_leeway(event_t event,
  ...
  wresult =
      waitq_assert_wait64_locked(waitq, CAST_EVENT64_T(event), interruptible,
                                 urgency, deadline, slop, thread);
...
wait_result_t assert_wait_deadline(event_t event,
                                   wait_interrupt_t interruptible,
                                   uint64_t deadline) {
  thread_t thread = current_thread();
  wait_result_t wresult;
...
wait_result_t assert_wait_deadline_with_leeway(event_t event,
                                               wait_interrupt_t interruptible,
                                               wait_timeout_urgency_t urgency,
                                               uint64_t deadline,
                                               uint64_t leeway) {
...
void sched_cond_init(sched_cond_atomic_t *cond) {
  os_atomic_init(cond, SCHED_COND_INIT);
}

wait_result_t sched_cond_wait_parameter(sched_cond_atomic_t *cond,
                                        wait_interrupt_t interruptible,
                                        thread_continue_t continuation,
                                        void *parameter) {
  assert_wait((event_t)cond, interruptible);
  /* clear active bit to indicate future wakeups will have to unblock this
   * thread */
  sched_cond_t new_state =
      (sched_cond_t)os_atomic_andnot(cond, SCHED_COND_ACTIVE, relaxed);
...
wait_result_t sched_cond_wait(sched_cond_atomic_t *cond,
                              wait_interrupt_t interruptible,
                              thread_continue_t continuation) {
  return sched_cond_wait_parameter(cond, interruptible, continuation, NULL);
}

sched_cond_t sched_cond_ack(sched_cond_atomic_t *cond) {
  sched_cond_t new_cond = (sched_cond_t)os_atomic_xor(
      cond, SCHED_COND_ACTIVE | SCHED_COND_WAKEUP, acquire);
  assert(new_cond & SCHED_COND_ACTIVE);
  return new_cond;
}

kern_return_t sched_cond_signal(sched_cond_atomic_t *cond, thread_t thread) {
  disable_preemption();
  sched_cond_t old_cond =
      (sched_cond_t)os_atomic_or_orig(cond, SCHED_COND_WAKEUP, release);
 ...
 */
static inline boolean_t thread_isoncpu(thread_t thread) {
  ...
  /* TODO: This is invalid - it can get dequeued without thread lock, but not
   * context switched. */
 ...
 */
boolean_t thread_stop(thread_t thread, boolean_t until_not_runnable) {
  wait_result_t wresult;
  spl_t s = splsched();
  boolean_t oncpu;
 ...
 */
void thread_unstop(thread_t thread) {
  spl_t s = splsched();
 ...
 *
 */
void thread_wait(thread_t thread, boolean_t until_not_runnable) {
  wait_result_t wresult;
  boolean_t oncpu;
 ...
 *		KERN_FAILURE		thread was waiting but could not be
 * rousted KERN_NOT_WAITING	thread was not waiting
 */
__private_extern__ kern_return_t clear_wait_internal(thread_t thread,
                                                     wait_result_t wresult) {
 ...
 *	  result		Wakeup result the thread should see
 */
__mockable kern_return_t clear_wait(thread_t thread, wait_result_t result) {
  kern_return_t ret;
  spl_t s;
 ...
 *
 */
kern_return_t thread_wakeup_nthreads_prim(event_t event, uint32_t nthreads,
 ...
 *
 */
__mockable kern_return_t thread_wakeup_prim(event_t event, boolean_t one_thread,
                                            wait_result_t result) {
  if (one_thread) {
...
kern_return_t thread_wakeup_thread(event_t event, thread_t thread) {
  ...
  return waitq_wakeup64_thread(wq, CAST_EVENT64_T(event), thread,
                               THREAD_AWAKENED);
}
 ...
 */
processor_t thread_bind(processor_t processor) {
  thread_t self = current_thread();
  processor_t prev;
  spl_t s;
...
void thread_bind_during_wakeup(thread_t thread, processor_t processor) {
...
void thread_unbind_after_queue_shutdown(thread_t thread,
...
static processor_t thread_bind_internal(thread_t thread,
                                        processor_t processor) {
  processor_t prev;

  /* <rdar://problem/15102234> */
  assert(thread->sched_pri < BASEPRI_RTQUEUES);
  /* A thread can't be bound if it's sitting on a (potentially incorrect)
   * runqueue */
  thread_assert_runq_null(thread);

  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_THREAD_BIND),
                        thread_tid(thread),
                        processor ? processor->cpu_id : ~0ul, 0, 0, 0);
 ...
 * 2 x vm_pageout_iothread_internal (92) when
 * vm_restricted_to_single_processor==TRUE vm_pageout_continue (92)
...
static void sched_vm_group_maintenance(void) {
  ...
  for (i = 0; i < sched_vm_group_thread_count; i++) {
    ...
    if ((thread->state & (TH_RUN | TH_WAIT)) == TH_RUN) {
      if (thread_get_runq(thread) != PROCESSOR_NULL &&
          thread->last_made_runnable_time < longtime) {
        high_latency_observed = TRUE;
      } else if (thread_get_runq(thread) == PROCESSOR_NULL) {
        /* There are some cases where a thread be transitiong that also fall
         * into this case */
  ...
  if (sched_vm_group_temporarily_unbound) {
    ...
    if (!high_latency_observed) {
      ...
      sched_vm_group_temporarily_unbound =
          FALSE; /* might be reset to TRUE if change cannot be completed */
    }
  } else {
    /*
...
int sched_backup_cpu_timeout_count =
    5; /* The maximum number of 10us delays to wait before using a backup cpu */
int sched_rt_n_backup_processors = SCHED_DEFAULT_BACKUP_PROCESSORS;

int sched_get_rt_n_backup_processors(void) {
  return sched_rt_n_backup_processors;
}

void sched_set_rt_n_backup_processors(int n) {
  if (n < 0) {
    n = 0;
...
/*
 * Invoked prior to idle entry to determine if, on SMT capable processors, an
 * SMT rebalancing opportunity exists when a core is (instantaneously) idle, but
 * other SMT-capable cores may be over-committed. TODO: some possible negatives:
 * IPI thrash if this core does not remain idle following the load balancing
 * ASTs Idle "thrash", when IPI issue is followed by idle entry/core power down
...
bool sched_SMT_balance(processor_t cprocessor, processor_set_t cpset) {
  ...
  processor_t sib_processor = cprocessor->processor_secondary
                                  ? cprocessor->processor_secondary
                                  : cprocessor->processor_primary;
  ...
  uint64_t running_secondary_map =
      (cpset->cpu_state_map[PROCESSOR_RUNNING] & ~cpset->primary_map);
  for (int cpuid = lsb_first(running_secondary_map); cpuid >= 0;
       cpuid = lsb_next(running_secondary_map, cpuid)) {
...
smt_balance_exit:
  ...
  if (ast_processor) {
    KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_SMT_BALANCE),
                          ast_processor->cpu_id, ast_processor->state,
                          ast_processor->processor_primary->state, 0, 0);
...
/* Invoked with pset locked, returns with pset unlocked */
bool sched_SMT_balance(__unused processor_t cprocessor,
                       __unused processor_set_t cpset) {
  pset_unlock(cpset);
  return false;
}
#endif /* CONFIG_SCHED_SMT */

int pset_available_cpu_count(processor_set_t pset) {
  return bit_count(pset_available_cpumap(pset));
}

bool pset_is_recommended(processor_set_t pset) {
  if (!pset) {
    return false;
  }
...
bool pset_type_is_recommended(processor_set_t pset) {
  if (!pset) {
    return false;
  }
  pset_map_t recommended_psets =
      os_atomic_load(&pset->node->pset_recommended_map, relaxed);
  return bit_count(recommended_psets) > 0;
}

static cpumap_t pset_available_but_not_running_cpumap(processor_set_t pset) {
  return (pset->cpu_state_map[PROCESSOR_IDLE] |
          pset->cpu_state_map[PROCESSOR_DISPATCHING]) &
         pset->recommended_bitmask;
}

bool pset_has_stealable_threads(processor_set_t pset) {
  pset_assert_locked(pset);

  cpumap_t avail_map = pset_available_but_not_running_cpumap(pset);
#if CONFIG_SCHED_SMT
  /*
   * Secondary CPUs never steal, so allow stealing of threads if there are more
   * threads than available primary CPUs
   */
  avail_map &= pset->primary_map;
#endif /* CONFIG_SCHED_SMT */

  return (pset->pset_runq.count > 0) &&
         ((pset->pset_runq.count + rt_runq_count(pset)) > bit_count(avail_map));
}

static void clear_pending_AST_bits(processor_set_t pset, processor_t processor,
                                   __kdebug_only const int trace_point_number) {
  ...
  if (bit_clear_if_set(pset->pending_AST_URGENT_cpu_mask, processor->cpu_id)) {
    KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PENDING_AST_URGENT) | DBG_FUNC_END,
         processor->cpu_id, pset->pending_AST_URGENT_cpu_mask, 0,
         trace_point_number);
  }
...
/*
 * Called with pset locked, on a processor that is committing to run a new
 * thread Will transition an idle or dispatching processor to running as it
 * picks up the first new thread from the idle thread.
 */
static void pset_commit_processor_to_new_thread(processor_set_t pset,
                                                processor_t processor,
                                                thread_t new_thread) {
  pset_assert_locked(pset);
  ...
  if (new_thread->sched_pri >= BASEPRI_RTQUEUES) {
    if ((avail_map & pset->realtime_map) == avail_map) {
      ...
      atomic_bit_clear(&node->pset_non_rt_map, pset->pset_id,
    ...
    if ((avail_map & pset->realtime_map) == avail_map) {
      ...
      atomic_bit_clear(&node->pset_non_rt_primary_map, pset->pset_id,
                       memory_order_relaxed);
    }
#endif /* CONFIG_SCHED_SMT */
  } else {
    if ((avail_map & pset->realtime_map) != avail_map) {
      if (!bit_test(atomic_load(&node->pset_non_rt_map), pset->pset_id)) {
        atomic_bit_set(&node->pset_non_rt_map, pset->pset_id,
                       memory_order_relaxed);
      }
    }
#if CONFIG_SCHED_SMT
    avail_map &= pset->primary_map;
    if ((avail_map & pset->realtime_map) != avail_map) {
      if (!bit_test(atomic_load(&node->pset_non_rt_primary_map),
                    pset->pset_id)) {
        atomic_bit_set(&node->pset_non_rt_primary_map, pset->pset_id,
                       memory_order_relaxed);
      }
    }
#endif /* CONFIG_SCHED_SMT */
...
all_available_primaries_are_running_realtime_threads(processor_set_t pset,
                                                     bool include_backups);
static bool these_processors_are_running_realtime_threads(processor_set_t pset,
                                                          uint64_t these_map,
                                                          bool include_backups);
#else  /* !CONFIG_SCHED_SMT */
...
#endif /* !CONFIG_SCHED_SMT */
static bool sched_ok_to_run_realtime_thread(processor_set_t pset,
                                            processor_t processor,
                                            bool as_backup);

static bool
other_psets_have_earlier_rt_threads_pending(processor_set_t stealing_pset,
                                            uint64_t earliest_deadline) {
  pset_map_t pset_map = stealing_pset->node->pset_map;

  bit_clear(pset_map, stealing_pset->pset_id);

  for (int pset_id = lsb_first(pset_map); pset_id >= 0;
       pset_id = lsb_next(pset_map, pset_id)) {
    processor_set_t nset = pset_array[pset_id];

    if (rt_deadline_add(
            os_atomic_load(&nset->stealable_rt_threads_earliest_deadline,
                           relaxed),
            rt_deadline_epsilon) < earliest_deadline) {
      return true;
    }
  }
...
/*
 * backup processor - used by choose_processor to send a backup IPI to in case
 * the preferred processor can't immediately respond followup processor - used
 * in thread_select when there are still threads on the run queue and available
 * processors spill processor - a processor in a different processor set that is
 * signalled to steal a thread from this run queue
 */
typedef enum { none, backup, followup, spill } next_processor_type_t;
 ...
 */
static thread_t thread_select(thread_t thread, processor_t processor,
                              ast_t *reason) {
  ...
  restart:
    ...
    if (loop_count > max_loop_count[processor->cpu_id]) {
      ...
      if (bit_count(loop_count) == 1) {
        kprintf("[%d]%s>max_loop_count = %d\n", processor->cpu_id, __FUNCTION__,
                loop_count);
      }
    }
#endif
    pending_AST_URGENT =
        bit_test(pset->pending_AST_URGENT_cpu_mask, processor->cpu_id);
    pending_AST_PREEMPT =
        bit_test(pset->pending_AST_PREEMPT_cpu_mask, processor->cpu_id);
     ...
     * choose_processor(), so in those cases we must continue trying to dequeue
     * work as we are the only option.
     */
    if (!SCHED(processor_bound_count)(processor)) {
      if (!processor->is_recommended) {
        /*
         * The performance controller has provided a hint to not dispatch more
         * threads,
         */
        idle_reason = 1;
        goto send_followup_ipi_before_idle;
      } else if (rt_runq_count(pset)) {
        bool ok_to_run_realtime_thread =
            sched_ok_to_run_realtime_thread(pset, processor, false);
        /* Give the current RT thread a chance to complete */
        ok_to_run_realtime_thread |= (thread->sched_pri >= BASEPRI_RTQUEUES &&
                                      processor->first_timeslice);
#if CONFIG_SCHED_SMT
        /*
         * On Intel we want to avoid SMT secondary processors and processor 0
         * but allow them to be used as backup processors in case the preferred
         * chosen processor is delayed by interrupts or processor stalls.  So if
         * it is not ok_to_run_realtime_thread as preferred
         * (sched_ok_to_run_realtime_thread(pset, processor, as_backup=false))
         * but ok_to_run_realtime_thread as backup
         * (sched_ok_to_run_realtime_thread(pset, processor, as_backup=true)) we
         * delay up to (timeout_count * 10us) to give the preferred processor
         * chance to grab the thread before the (current) backup processor does.
         *
         * timeout_count defaults to 5 but can be tuned using sysctl
         * kern.sched_backup_cpu_timeout_count on DEVELOPMENT || DEBUG kernels.
         * It is also adjusted (see above) depending on whether we want to use
        ...
        /*
         * Should this secondary SMT processor attempt to find work? For pset
         * runqueue systems, we should look for work only under the same
         * conditions that choose_processor() would have assigned work, which is
         * when all primary processors have been assigned work.
         */
        if ((pset->recommended_bitmask & pset->primary_map &
             pset->cpu_state_map[PROCESSOR_IDLE]) != 0) {
    ...
    bool still_running = ((thread->state & (TH_TERMINATE | TH_IDLE | TH_WAIT |
                                            TH_RUN | TH_SUSP)) == TH_RUN);

    /*
     * Threads running on SMT processors are forced to context switch. Don't
     * rebalance realtime threads.
     * TODO: This should check if it's worth it to rebalance, i.e. 'are there
     * any idle primary processors' <rdar://problem/47907700>
     *
...
#if CONFIG_SCHED_SMT
    bool needs_smt_rebalance = !is_yielding &&
                               thread->sched_pri < BASEPRI_RTQUEUES &&
                               processor->processor_primary != processor;
#endif /* CONFIG_SCHED_SMT */

    bool affinity_mismatch = thread->affinity_set != AFFINITY_SET_NULL &&
                             thread->affinity_set->aset_pset != pset;

    bool bound_elsewhere = thread->bound_processor != PROCESSOR_NULL &&
                           thread->bound_processor != processor;

    bool avoid_processor =
        !is_yielding && SCHED(avoid_processor_enabled) &&
        SCHED(thread_avoid_processor)(processor, thread, *reason);

    bool ok_to_run_realtime_thread =
        sched_ok_to_run_realtime_thread(pset, processor, true);
    ...
    bool current_thread_can_keep_running =
        (still_running
         ...
         && !needs_smt_rebalance
#endif /* CONFIG_SCHED_SMT */
         && !affinity_mismatch && !bound_elsewhere && !avoid_processor);
    if (current_thread_can_keep_running) {
      ...
      if (thread->sched_pri >= BASEPRI_RTQUEUES && processor->first_timeslice) {
        ...
        if (ok_to_run_realtime_thread) {
          ...
          if (rt_runq_priority(pset) > thread->sched_pri) {
            ...
            /*
             * See if the current lower priority thread can continue to run
             * without causing the higher priority thread on the runq queue to
             * miss its deadline.
             */
            thread_t hi_thread = rt_runq_first(&pset->rt_runq);
            if (thread->realtime.computation + hi_thread->realtime.computation +
                    rt_deadline_epsilon >=
                hi_thread->realtime.constraint) {
              /* The next RT thread is better, so pick it off the runqueue. */
              goto pick_new_rt_thread;
            }
          } else if ((rt_runq_count(pset) > 0) &&
                     (rt_deadline_add(rt_runq_earliest_deadline(pset),
                                      rt_deadline_epsilon) <
                      thread->realtime.deadline)) {
            /* The next RT thread is better, so pick it off the runqueue. */
            goto pick_new_rt_thread;
          }
          if (other_psets_have_earlier_rt_threads_pending(
        ...
        if (rt_pset_has_stealable_threads(pset)) {
          nptype = spill;
          pset_unlocked = rt_choose_next_processor_for_spill_IPI(
              pset, processor, &next_rt_processor, &next_rt_ipi_type);
        }
        ...
        if (next_rt_processor) {
          KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_NEXT_PROCESSOR) |
                   DBG_FUNC_NONE,
               next_rt_processor->cpu_id, next_rt_processor->state, nptype, 2);
          sched_ipi_perform(next_rt_processor, next_rt_ipi_type);
        }

        KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_THREAD_SELECT) |
                 DBG_FUNC_END,
             (uintptr_t)thread_tid(thread), pset->pending_AST_URGENT_cpu_mask,
             delay_count, 1);
        return thread;
      }

      if ((rt_runq_count(pset) == 0) &&
          SCHED(processor_queue_has_priority)(processor, thread->sched_pri,
                                              TRUE) == FALSE) {
       ...
       * If it's due to a rebalance, we should aggressively find this thread a
       * new home.
       */
      bool ast_rebalance =
          affinity_mismatch || bound_elsewhere || avoid_processor;
...
#if CONFIG_SCHED_SMT
    bool secondary_forced_idle =
        ((processor->processor_secondary != PROCESSOR_NULL) &&
      ...
      if (new_thread != THREAD_NULL) {
        ...
        if (processor->processor_secondary != NULL) {
          ...
          if ((sprocessor->state == PROCESSOR_RUNNING) ||
              (sprocessor->state == PROCESSOR_DISPATCHING)) {
            ipi_type =
                sched_ipi_action(sprocessor, NULL, SCHED_IPI_EVENT_SMT_REBAL);
            ast_processor = sprocessor;
          }
        }
#endif /* CONFIG_SCHED_SMT */
  ...
  send_followup_ipi_before_idle:
    /* This might not have been cleared if we didn't call
    ...
    if (new_thread || next_rt_processor) {
      ...
      if (new_thread) {
        KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_THREAD_SELECT) |
             ...
             pset->pending_AST_URGENT_cpu_mask, delay_count, 3);
        return new_thread;
      }
    }
    ...
    if (processor->is_recommended) {
      bool spill_pending =
          bit_test(pset->rt_pending_spill_cpu_mask, processor->cpu_id);
      if (sched_ok_to_run_realtime_thread(pset, processor, true) &&
          (spill_pending || rt_runq_count(pset))) {
        /* Things changed while we dropped the lock */
        goto restart;
      }
      ...
      if ((processor->processor_primary != processor) &&
          (processor->processor_primary->current_pri >= BASEPRI_RTQUEUES)) {
    ...
    if ((new_thread = SCHED(choose_thread)(
             processor, MINPRI,
             current_thread_can_keep_running ? thread : THREAD_NULL,
             *reason)) != THREAD_NULL) {
      if (new_thread != thread) {
        ...
        if (sprocessor != NULL) {
          ...
          } else if (secondary_forced_idle && !thread_no_smt(new_thread) &&
                     pset_has_stealable_threads(pset)) {
            ipi_type =
                sched_ipi_action(sprocessor, NULL, SCHED_IPI_EVENT_PREEMPT);
            ast_processor = sprocessor;
          }
        }
#endif /* CONFIG_SCHED_SMT */
      ...
      KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_THREAD_SELECT) |
               DBG_FUNC_END,
           (uintptr_t)thread_tid(new_thread), pset->pending_AST_URGENT_cpu_mask,
           delay_count, 4);
      return new_thread;
    }
    ...
    if (SCHED(steal_thread_enabled)(pset)
      ...
      if ((new_thread = SCHED(steal_thread)(pset)) != THREAD_NULL) {
        ...
        KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_THREAD_SELECT) |
                 DBG_FUNC_END,
             (uintptr_t)thread_tid(new_thread),
             pset->pending_AST_URGENT_cpu_mask, delay_count, 5);
        return new_thread;
      }
 ...
 */
static boolean_t thread_invoke(thread_t self, thread_t thread, ast_t reason) {
  if (__improbable(get_preemption_level() != 0)) {
    int pl = get_preemption_level();
    panic("thread_invoke: preemption_level %d, possible cause: %s", pl,
          (pl < 0 ? "unlocking an unlocked mutex or spinlock"
                  : "blocking while holding a spinlock, or within interrupt "
                    "context"));
  }
  ...
  if (ctime < thread->last_made_runnable_time) {
    panic("Non-monotonic time: invoke at 0x%llx, runnable at 0x%llx", ctime,
  ...
  assert_thread_magic(thread);
  assert((thread->state & (TH_RUN | TH_WAIT | TH_UNINT | TH_TERMINATE |
                           TH_TERMINATE2)) == TH_RUN);
  ...
  if (continuation != NULL) {
    if (!thread->kernel_stack) {
       ...
       */
      if (self->kernel_stack == self->reserved_stack &&
          !thread->reserved_stack) {
        goto need_stack;
      }
      ...
      KERNEL_DEBUG_CONSTANT_IST(
          ...
          self->reason, (uintptr_t)thread_tid(thread), self->sched_pri,
          thread->sched_pri, 0);

      if ((thread->chosen_processor != processor) &&
          (thread->chosen_processor != PROCESSOR_NULL)) {
        SCHED_DEBUG_CHOOSE_PROCESSOR_KERNEL_DEBUG_CONSTANT_IST(
            MACHDBG_CODE(DBG_MACH_SCHED, MACH_MOVED) | DBG_FUNC_NONE,
            (uintptr_t)thread_tid(thread),
            (uintptr_t)thread->chosen_processor->cpu_id, 0, 0, 0);
      }

      DTRACE_SCHED2(off__cpu, struct thread *, thread, struct proc *,
                    current_proc());

      SCHED_STATS_CSW(processor, self->reason, self->sched_pri,
                      thread->sched_pri);
      ...
      KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                                MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED) |
                                    DBG_FUNC_NONE,
                                self->reason, (uintptr_t)thread_tid(thread),
                                self->sched_pri, thread->sched_pri, 0);
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED) | DBG_FUNC_NONE,
      self->reason, (uintptr_t)thread_tid(thread), self->sched_pri,
      thread->sched_pri, 0);

  if ((thread->chosen_processor != processor) &&
      (thread->chosen_processor != NULL)) {
    SCHED_DEBUG_CHOOSE_PROCESSOR_KERNEL_DEBUG_CONSTANT_IST(
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_MOVED) | DBG_FUNC_NONE,
        (uintptr_t)thread_tid(thread),
        (uintptr_t)thread->chosen_processor->cpu_id, 0, 0, 0);
  ...
  TLOG(1,
       "thread_invoke: returning machine_switch_context: self %p continuation "
       "%p thread %p\n",
 ...
 */
static void pset_cancel_deferred_dispatch(processor_set_t pset,
                                          processor_t processor) {
  processor_t active_processor = NULL;
  uint32_t sampled_sched_run_count;

  pset_lock(pset);
  sampled_sched_run_count =
      os_atomic_load(&sched_run_buckets[TH_BUCKET_RUN], relaxed);
  ...
  if ((sampled_sched_run_count == 1) && (pset->pending_deferred_AST_cpu_mask)) {
    ...
    for (int cpuid = lsb_first(dispatching_map); cpuid >= 0;
         cpuid = lsb_next(dispatching_map, cpuid)) {
      ...
      if (active_processor != processor) {
        ...
        bit_clear(pset->pending_deferred_AST_cpu_mask,
                  active_processor->cpu_id);
...
static void thread_csw_callout(thread_t old, thread_t new, uint64_t timestamp) {
  perfcontrol_event event = (new->state & TH_IDLE) ? IDLE : CONTEXT_SWITCH;
  uint64_t same_pri_latency =
      (new->state & TH_IDLE) ? 0 : new->same_pri_latency;
  machine_switch_perfcontrol_context(event, timestamp, 0, same_pri_latency, old,
                                     new);
}
 ...
 *
 */
void thread_dispatch(thread_t thread, thread_t self) {
  ...
  if (thread != THREAD_NULL) {
    ...
    if (thread->state & TH_IDLE) {
      was_idle = true;
      KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                                MACHDBG_CODE(DBG_MACH_SCHED, MACH_DISPATCH) |
                                    DBG_FUNC_NONE,
      ...
      if (processor->quantum_end > processor->last_dispatch) {
        remainder = processor->quantum_end - processor->last_dispatch;
      }
      ...
      if ((thread->reason & AST_LEDGER) == 0) {
         ...
         */
        ledger_credit_thread(thread, thread->t_ledger, task_ledgers.cpu_time,
                             consumed);
        ...
        if (consumed > 0) {
          /*
           * This should never be negative, but in traces we are seeing some
           * instances of consumed being negative. <rdar://problem/57782596>
           * thread_dispatch() thread CPU consumed calculation sometimes results
           * in negative value
           */
          sched_update_pset_avg_execution_time(
              current_processor()->processor_set, consumed,
              processor->last_dispatch, thread->th_sched_bucket);
        }
      }
      ...
      if (keep_quantum && processor->quantum_end > processor->last_dispatch) {
      ...
      if (thread->sched_mode == TH_MODE_REALTIME) {
        ...
        if (thread->quantum_remaining == 0) {
          KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_CANCEL_RT_DEADLINE) |
                   DBG_FUNC_NONE,
               (uintptr_t)thread_tid(thread), thread->realtime.deadline,
               thread->realtime.computation, 0);
      ...
      thread->computation_metered +=
          (processor->last_dispatch - thread->computation_epoch);

      if (!(thread->state & TH_WAIT)) {
        /*
         *	Still runnable.
         */
        thread->last_made_runnable_time = thread->last_basepri_change_time =
        ...
        if (reason & AST_REBALANCE) {
          ...
          if (reason & AST_QUANTUM) {
            /*
             * Having gone to the trouble of forcing this thread off a less
             * preferred core, we should force the preferable core to reschedule
             * immediately to give this thread a chance to run instead of just
             * sitting on the run queue where it may just be stolen back by the
             * idle core we just forced it off. But only do this at the end of a
             * quantum to prevent cascading effects.
             */
        ...
        machine_thread_going_off_core(thread, should_terminate,
                                      processor->last_dispatch, FALSE);

        KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                                  MACHDBG_CODE(DBG_MACH_SCHED, MACH_DISPATCH) |
                                      DBG_FUNC_NONE,
                                  (uintptr_t)thread_tid(thread), thread->reason,
                                  thread_state, new_run_count, 0);
   ...
   * context switch, here).
   */
  self->computation_interrupt_epoch =
      recount_current_thread_interrupt_time_mach();
   ...
   * uninteresting?
   */
  if ((os_atomic_load(&sched_run_buckets[TH_BUCKET_RUN], relaxed) == 1) &&
      !(self->state & TH_IDLE)) {
 ...
 */
__mockable wait_result_t thread_block_reason(thread_continue_t continuation,
                                             void *parameter, ast_t reason) {
  thread_t self = current_thread();
  processor_t processor;
  thread_t new_thread;
  spl_t s;
  ...
  if (self->state & ~(TH_RUN | TH_IDLE)) {
    KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                              MACHDBG_CODE(DBG_MACH_SCHED, MACH_BLOCK), reason,
                              VM_KERNEL_UNSLIDE(continuation), 0, 0, 0);
  }
...
wait_result_t thread_block_parameter(thread_continue_t continuation,
                                     void *parameter) {
 ...
 *	Called at splsched.
 */
int thread_run(thread_t self, thread_continue_t continuation, void *parameter,
               thread_t new_thread) {
 ...
 */
void thread_continue(thread_t thread) {
  thread_t self = current_thread();
  thread_continue_t continuation;
  void *parameter;
...
void thread_quantum_init(thread_t thread, uint64_t now) {
...
uint32_t sched_timeshare_initial_quantum_size(thread_t thread) {
  if ((thread != THREAD_NULL) &&
      thread->th_sched_bucket == TH_BUCKET_SHARE_BG) {
    return bg_quantum;
  } else {
    return std_quantum;
  }
 ...
 */
void run_queue_init(run_queue_t rq) {
  rq->highq = NOPRI;
 ...
 */
thread_t run_queue_dequeue(run_queue_t rq, sched_options_t options) {
  thread_t thread;
 ...
 *	for more info).
 */
boolean_t run_queue_enqueue(run_queue_t rq, thread_t thread,
                            sched_options_t options) {
 ...
 */
void run_queue_remove(run_queue_t rq, thread_t thread) {
  circle_queue_t queue = &rq->queues[thread->sched_pri];
 ...
 *	The run queue must be locked.
 */
thread_t run_queue_peek(run_queue_t rq) {
  if (rq->count > 0) {
    circle_queue_t queue = &rq->queues[rq->highq];
 ...
 */
static void realtime_setrun(processor_t chosen_processor, thread_t thread) {
  ...
  int existing_backups =
      bit_count(pset->pending_AST_URGENT_cpu_mask) - rt_runq_count(pset);
  ...
  for (int i = 0; i <= n_backup; i++) {
    if (i == 0) {
      ...
      if (thread->sched_pri > processor->current_pri) {
        ...
        if (rt_deadline_add(thread->realtime.deadline, rt_deadline_epsilon) <
            processor->deadline) {
          preempt = (AST_PREEMPT | AST_URGENT);
        }
      }
      ...
      if (preempt != AST_NONE) {
        if (processor->state == PROCESSOR_IDLE) {
          if (processor == current_processor()) {
            ...
            if ((preempt & AST_URGENT) == AST_URGENT) {
              if (bit_set_if_clear(pset->pending_AST_URGENT_cpu_mask,
                                   processor->cpu_id)) {
                KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PENDING_AST_URGENT) |
                         DBG_FUNC_START,
                     processor->cpu_id, pset->pending_AST_URGENT_cpu_mask,
                     (uintptr_t)thread_tid(thread), 1);
              }
            }
            ...
            ipi_type[i] =
                sched_ipi_action(processor, thread, SCHED_IPI_EVENT_RT_PREEMPT);
          }
        ...
        /* Selected processor was too busy, just keep thread enqueued and let
         * other processors drain it naturally. */
      }
    } else {
      if (!pset_is_locked) {
        pset_lock(pset);
      ...
      rt_choose_next_processor_for_followup_IPI(
          pset, chosen_processor, &ipi_processor[i], &ipi_type[i]);
...
#if CONFIG_SCHED_SMT
#define p_is_good(p)                                                           \
  (((p)->processor_primary == (p)) &&                                          \
   ((sched_avoid_cpu0 != 1) || ((p)->cpu_id != 0)))
      if (n_backup == SCHED_DEFAULT_BACKUP_PROCESSORS_SMT) {
        ...
        if (p_is_good(p0) && p_is_good(p1)) {
          /*
           * Both the chosen processor and the first backup are non-cpu0
           * primaries, so there is no need for a 2nd backup processor.
           */
          break;
        }
      }
...
sched_ipi_type_t sched_ipi_deferred_policy(processor_set_t pset,
                                           processor_t dst, thread_t thread,
  ...
  (void)thread;
  panic("Request for deferred IPI on an unsupported platform; pset: %p CPU: %d",
        pset, dst->cpu_id);
...
sched_ipi_type_t sched_ipi_action(processor_t dst, thread_t thread,
  ...
  default:
    if (bit_set_if_clear(pset->pending_AST_URGENT_cpu_mask, dst->cpu_id)) {
      KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PENDING_AST_URGENT) |
               DBG_FUNC_START,
           dst->cpu_id, pset->pending_AST_URGENT_cpu_mask,
           (uintptr_t)thread_tid(thread), 4);
    }
...
sched_ipi_type_t sched_ipi_policy(processor_t dst, thread_t thread,
  ...
  case SCHED_IPI_EVENT_PREEMPT:
     ...
     * If the core is idle, use deferred IPIs if supported; otherwise immediate
     * IPI.
     */
...
static void processor_setrun(processor_t processor, thread_t thread,
  ...
  if (SCHED(priority_is_urgent)(thread->sched_pri) &&
    ...
    preempt = (AST_PREEMPT | AST_URGENT);
  } else if ((thread->sched_mode == TH_MODE_TIMESHARE) &&
             (thread->sched_pri < thread->base_pri)) {
    if (SCHED(priority_is_urgent)(thread->base_pri) &&
        thread->sched_pri > processor->current_pri) {
  ...
  if ((options & SCHED_STIR_POT) ||
      ((options & (SCHED_PREEMPT | SCHED_REBALANCE)) ==
       (SCHED_PREEMPT | SCHED_REBALANCE))) {
    /*
     * Having gone to the trouble of forcing this thread off a less preferred
     * core, we should force the preferable core to reschedule immediately to
     * give this thread a chance to run instead of just sitting on the run queue
     * where it may just be stolen back by the idle core we just forced it off.
     */
    preempt |= AST_PREEMPT;
  }
  ...
  if (ipi_action != eDoNothing) {
    if (processor == current_processor()) {
      ...
      if ((preempt = csw_check_locked(processor->active_thread, processor, pset,
                                      AST_NONE)) != AST_NONE) {
        ast_on(preempt);
      }
      ...
      if ((preempt & AST_URGENT) == AST_URGENT) {
        if (bit_set_if_clear(pset->pending_AST_URGENT_cpu_mask,
          ...
          KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PENDING_AST_URGENT) |
                   DBG_FUNC_START,
               processor->cpu_id, pset->pending_AST_URGENT_cpu_mask,
               (uintptr_t)thread_tid(thread), 7);
        }
      } else {
        if (bit_clear_if_set(pset->pending_AST_URGENT_cpu_mask,
                             processor->cpu_id)) {
          KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PENDING_AST_URGENT) |
                   DBG_FUNC_END,
               processor->cpu_id, pset->pending_AST_URGENT_cpu_mask, 0, 7);
        }
      }
      ...
      sched_ipi_event_t event = (options & SCHED_REBALANCE)
 ...
 *	suitable.
 */
static processor_set_t choose_next_pset(processor_set_t pset) {
 ...
 */
processor_t choose_processor_smt(processor_set_t starting_pset,
      ...
      case PROCESSOR_IDLE:
        ...
        if ((thread->sched_pri < BASEPRI_RTQUEUES) ||
            processor_is_fast_track_candidate_for_realtime_thread(pset,
                                                                  processor)) {
          uint64_t idle_primary_map =
              (pset->cpu_state_map[PROCESSOR_IDLE] & pset->primary_map &
               pset->recommended_bitmask);
          uint64_t non_avoided_idle_primary_map =
              idle_primary_map & ~pset->perfcontrol_cpu_migration_bitmask;
          /*
           * If the rotation bitmask to force a migration is set for this core
           * and there's an idle core that that needn't be avoided, don't
           * continue running on the same core.
           */
          if (!(bit_test(
                    processor->processor_set->perfcontrol_cpu_migration_bitmask,
                    processor->cpu_id) &&
                non_avoided_idle_primary_map != 0)) {
            return processor;
          }
        }
        processor = PROCESSOR_NULL;
  ...
  do {
    ...
    uint64_t idle_primary_map = (pset->cpu_state_map[PROCESSOR_IDLE] &
                                 pset->primary_map & pset->recommended_bitmask);
    uint64_t preferred_idle_primary_map =
        idle_primary_map & pset->perfcontrol_cpu_preferred_bitmask;
     ...
     */
    int cpuid =
        lsb_next(preferred_idle_primary_map, pset->cpu_preferred_last_chosen);
    if (cpuid < 0) {
    ...
    if (pset->perfcontrol_cpu_migration_bitmask != 0) {
      uint64_t non_avoided_idle_primary_map =
          idle_primary_map & ~pset->perfcontrol_cpu_migration_bitmask;
      cpuid = lsb_next(non_avoided_idle_primary_map,
                       pset->cpu_preferred_last_chosen);
      if (cpuid < 0) {
    ...
    /*
     * Fall back to any remaining idle cores if none of the preferred ones and
     * non-avoided ones are available.
     */
    ...
    /*
     * Otherwise, enumerate active and idle processors to find primary
     * candidates with lower priority/etc.
     */

    uint64_t active_map =
        ((pset->cpu_state_map[PROCESSOR_RUNNING] |
          pset->cpu_state_map[PROCESSOR_DISPATCHING]) &
         pset->recommended_bitmask & ~pset->pending_AST_URGENT_cpu_mask);
    ...
    for (int rotid = lsb_first(active_map); rotid >= 0;
         rotid = lsb_next(active_map, rotid)) {
    ...
    /*
     * For SMT configs, these idle secondary processors must have active
     * primary. Otherwise the idle primary would have short-circuited the loop
     * above
     */
    uint64_t idle_secondary_map =
        (pset->cpu_state_map[PROCESSOR_IDLE] & ~pset->primary_map &
         pset->recommended_bitmask);
    ...
    for (cpuid = lsb_first(idle_secondary_map); cpuid >= 0;
         cpuid = lsb_next(idle_secondary_map, cpuid)) {
      processor = processor_array[cpuid];
      ...
      /* If the primary is running a no-smt thread, then don't choose its
       * secondary */
      ...
      if (primary_pri < lowest_unpaired_primary_priority) {
        /* If the primary processor is offline or starting up, it's not a
         * candidate for this path */
        if (cprimary->state != PROCESSOR_RUNNING &&
...
#if CONFIG_SCHED_EDGE
    /*
     * The edge scheduler expects a CPU to be selected from the pset it passed
     * in as the starting pset for non-RT workloads. The edge migration
     * algorithm should already have considered idle CPUs and loads to decide
     * the starting_pset; which means that this loop can be short-circuted.
     */
    nset = starting_pset;
 ...
 */
processor_t choose_processor(processor_set_t starting_pset,
                             processor_t processor, thread_t thread,
                             __unused sched_options_t *options) {
  processor_set_t pset = starting_pset;
  processor_set_t nset;
      ...
      case PROCESSOR_IDLE:
        ...
        if ((thread->sched_pri < BASEPRI_RTQUEUES) ||
            processor_is_fast_track_candidate_for_realtime_thread(pset,
                                                                  processor)) {
          uint64_t idle_map =
              (pset->cpu_state_map[PROCESSOR_IDLE] & pset->recommended_bitmask);
          uint64_t non_avoided_idle_map =
              idle_map & ~pset->perfcontrol_cpu_migration_bitmask;
          /*
           * If the rotation bitmask to force a migration is set for this core
           * and there's an idle core that that needn't be avoided, don't
           * continue running on the same core.
  ...
  do {
    ...
    uint64_t idle_map =
        (pset->cpu_state_map[PROCESSOR_IDLE] & pset->recommended_bitmask);
    uint64_t preferred_idle_map =
        idle_map & pset->perfcontrol_cpu_preferred_bitmask;
 ...
 */
pset_node_t sched_choose_node(__unused thread_t thread) { return &pset_node0; }
 ...
 */
processor_set_t choose_starting_pset(pset_node_t node, thread_t thread,
  ...
  if (thread->sched_pri >= BASEPRI_RTQUEUES) {
    ...
    if ((avoid_cpu0 && pset->pset_id == 0) ||
        !bit_test(rt_target_map, pset->pset_id)) {
      if (avoid_cpu0) {
    ...
    if (!pset->is_SMT || !sched_allow_rt_smt) {
      /* All psets are full of RT threads - fall back to choose processor to
       * find the furthest deadline RT thread */
      goto out;
    }
#endif /* CONFIG_SCHED_SMT*/
    ...
    /* All psets are full of RT threads - fall back to choose processor to find
     * the furthest deadline RT thread */
  } else {
 ...
 *	Thread must be locked.
 */
void thread_setrun(thread_t thread, sched_options_t options) {
  processor_t processor = PROCESSOR_NULL;
  processor_set_t pset;
...
#define CHOOSE_PROCESSOR_MAX_RETRIES 3
    for (int retry = 0; retry <= CHOOSE_PROCESSOR_MAX_RETRIES; retry++) {
      ...
      processor_set_t starting_pset =
          choose_starting_pset(node, thread, &processor_hint);
    ...
    if (processor == PROCESSOR_NULL) {
      ...
      pset = processor->processor_set;
      pset_lock(pset);
      assert((pset_available_cpu_count(pset) > 0) ||
    ...
    if (!(task->t_flags & TF_USE_PSET_HINT_CLUSTER_TYPE)) {
      task->pset_hint =
          pset; /* NRG this is done without holding the task lock */
    }
    SCHED_DEBUG_CHOOSE_PROCESSOR_KERNEL_DEBUG_CONSTANT_IST(
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_CHOOSE_PROCESSOR) |
            DBG_FUNC_NONE,
        (uintptr_t)thread_tid(thread), (uintptr_t)-1, processor->cpu_id,
        processor->state, 0);
    assert(
        (pset_available_cpu_count(pset) > 0) ||
        (processor->state != PROCESSOR_OFF_LINE && processor->is_recommended));
  } else {
    /*
     *	Bound case:
     *
    ...
    SCHED_DEBUG_CHOOSE_PROCESSOR_KERNEL_DEBUG_CONSTANT_IST(
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_CHOOSE_PROCESSOR) |
            DBG_FUNC_NONE,
        (uintptr_t)thread_tid(thread), (uintptr_t)-2, processor->cpu_id,
        processor->state, 0);
  }
...
processor_set_t task_choose_pset(task_t task) {
 ...
 */
ast_t csw_check(thread_t thread, processor_t processor, ast_t check_reason) {
  processor_set_t pset = processor->processor_set;
  ...
  if ((preempt & AST_URGENT) == 0) {
    if (bit_clear_if_set(pset->pending_AST_URGENT_cpu_mask,
                         processor->cpu_id)) {
      KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PENDING_AST_URGENT) | DBG_FUNC_END,
           processor->cpu_id, pset->pending_AST_URGENT_cpu_mask, 0, 8);
    }
  }
...
void clear_pending_nonurgent_preemption(processor_t processor) {
  if (!processor->pending_nonurgent_preemption) {
    return;
  }

  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_PREEMPT_TIMER_ACTIVE) |
               DBG_FUNC_END);
...
ast_t update_pending_nonurgent_preemption(processor_t processor, ast_t reason) {
  ...
  running_timer_enter(processor, RUNNING_TIMER_PREEMPT, NULL, deadline, now);
 ...
 * processor.
 */
ast_t csw_check_locked(thread_t thread, processor_t processor,
                       processor_set_t pset, ast_t check_reason) {
  assert(processor == current_processor());
  /*
   * If the current thread is running on a processor that is no longer
   * recommended, urgently preempt it, at which point thread_select() should try
   * to idle the processor and re-dispatch the thread to a recommended
   * processor.
   */
  if (!processor->is_recommended) {
  ...
  if (rt_runq_count(pset) > 0) {
    if ((rt_runq_priority(pset) > processor->current_pri) ||
        !processor->first_timeslice) {
      return check_reason | AST_PREEMPT | AST_URGENT;
    } else if (rt_deadline_add(rt_runq_earliest_deadline(pset),
                               rt_deadline_epsilon) < processor->deadline) {
      return check_reason | AST_PREEMPT | AST_URGENT;
    } else {
  ...
  if (result != AST_NONE) {
    return check_reason | result |
           (thread_is_eager_preempt(thread) ? AST_URGENT : AST_NONE);
  }
   ...
   */
  if (SCHED(avoid_processor_enabled) &&
      SCHED(thread_avoid_processor)(processor, thread, check_reason)) {
    return check_reason | AST_PREEMPT;
  }
   ...
   * if no bound threads target this processor, and idle primaries exist,
   * preempt The case of RT threads existing is already taken care of above
   */
 ...
 * Called at splsched
 */
void ast_check(processor_t processor) {
  smr_ack_ipi();
  ...
  SCHED_DEBUG_AST_CHECK_KDBG_RELEASE(
      MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_AST_CHECK) | DBG_FUNC_START);
  ...
  if (old_urgency != processor->current_urgency) {
    ...
    thread_urgency_t urgency =
        thread_get_urgency(thread, &urgency_param1, &urgency_param2);
  ...
  SCHED_DEBUG_AST_CHECK_KDBG_RELEASE(
      MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_AST_CHECK) | DBG_FUNC_END,
      preempt);
}

void thread_preempt_expire(timer_call_param_t p0,
 ...
 *	Thread must be locked.
 */
void set_sched_pri(thread_t thread, int16_t new_priority,
  ...
  KERNEL_DEBUG_CONSTANT(
      MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_CHANGE_PRIORITY),
      (uintptr_t)thread_tid(thread), thread->base_pri, thread->sched_pri,
  ...
  if (removed_from_runq) {
     ...
     * those are lazily handled. QoS classes have distinct priority bands, and
     * QoS inheritance is expected to involve priority changes.
     */
    if (processor->current_urgency != old_urgency) {
      uint64_t urgency_param1, urgency_param2;

      thread_urgency_t new_urgency =
          thread_get_urgency(thread, &urgency_param1, &urgency_param2);

      thread_tell_urgency(new_urgency, urgency_param1, urgency_param2, 0,
                          thread);
    }

    /* TODO: only call this if current_perfctl_class changed */
    uint64_t ctime = mach_approximate_time();
    machine_thread_going_on_core(thread, processor->current_urgency, 0, 0,
                                 ctime);
  } else if (thread->state & TH_RUN) {
    processor_t processor = thread->last_processor;

    if (!lazy_update && processor != PROCESSOR_NULL &&
        processor != current_processor() &&
        processor->active_thread == thread) {
      cause_ast_check(processor);
    }
 ...
 * Called at splsched
 *
 * Returns the thread that was pulled or THREAD_NULL if no thread could be
 * pulled. This may be different than the thread that was passed in.
 */
 ...
 *
 * If the thread was woken up from wait for handoff, make sure it is not bound
 ...
 * pulled. This may be different than the thread that was passed in.
 */
thread_t thread_prepare_for_handoff(thread_t thread,
 ...
 */
boolean_t thread_run_queue_remove(thread_t thread) {
 ...
 */
void thread_run_queue_reinsert(thread_t thread, sched_options_t options) {
  thread_assert_runq_null(thread);
  assert(thread->state & (TH_RUN));
...
void sys_override_cpu_throttle(boolean_t enable_override) {
  if (enable_override) {
...
thread_urgency_t thread_get_urgency(thread_t thread, uint64_t *arg1,
                                    uint64_t *arg2) {
  ...
  if (thread == NULL || task == TASK_NULL || (thread->state & TH_IDLE)) {
  ...
  } else if (cpu_throttle_enabled && (thread->sched_pri <= MAXPRI_THROTTLE) &&
     ...
     */
    boolean_t thread_lacks_qos =
        (proc_get_effective_thread_policy(thread, TASK_POLICY_QOS) ==
         THREAD_QOS_UNSPECIFIED); // thread_has_qos_policy(thread);
    boolean_t task_is_suppressed =
        (proc_get_effective_task_policy(task, TASK_POLICY_SUP_ACTIVE) == 0x1);
    ...
    urgency_param1 =
        proc_get_effective_thread_policy(thread, TASK_POLICY_THROUGH_QOS);
    urgency_param2 =
        proc_get_effective_task_policy(task, TASK_POLICY_THROUGH_QOS);
...
perfcontrol_class_t thread_get_perfcontrol_class(thread_t thread) {
...
#else
#define IDLE_KERNEL_DEBUG_CONSTANT(...)                                        \
  do {                                                                         \
  } while (0)
#endif
...
thread_t processor_idle(thread_t thread, processor_t processor) {
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, MACHDBG_CODE(DBG_MACH_SCHED, MACH_IDLE) | DBG_FUNC_START,
  ...
  while (1) {
    ...
    IDLE_KERNEL_DEBUG_CONSTANT(
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_IDLE) | DBG_FUNC_NONE,
        (uintptr_t)thread_tid(thread), rt_runq_count(pset),
        SCHED(processor_runq_count)(processor), -1, 0);
    ...
    IDLE_KERNEL_DEBUG_CONSTANT(
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_IDLE) | DBG_FUNC_NONE,
        (uintptr_t)thread_tid(thread), rt_runq_count(pset),
        SCHED(processor_runq_count)(processor), -2, 0);
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, MACHDBG_CODE(DBG_MACH_SCHED, MACH_IDLE) | DBG_FUNC_END,
      (uintptr_t)thread_tid(thread), processor->state,
      (uintptr_t)thread_tid(new_thread), reason, 0);
 ...
 */
void idle_thread(__assert_only void *parameter, __unused wait_result_t result) {
  assert(ml_get_interrupts_enabled() == FALSE);
  ...
  if (new_thread != THREAD_NULL) {
    thread_run(processor->idle_thread, idle_thread, NULL, new_thread);
    /*NOTREACHED*/
  }
...
void idle_thread_create(processor_t processor, thread_continue_t continuation) {
...
void sched_timeshare_maintenance_continue(void) {
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_MAINTENANCE) |
                            DBG_FUNC_START,
   ...
   *  may need to be updated, and find the earliest runnable thread on the
   * runqueue to report its latency.
   */
  ...
  uint64_t bg_max_latency =
      (ctime > scan_context.earliest_bg_make_runnable_time)
          ? ctime - scan_context.earliest_bg_make_runnable_time
          : 0;

  uint64_t default_max_latency =
      (ctime > scan_context.earliest_normal_make_runnable_time)
          ? ctime - scan_context.earliest_normal_make_runnable_time
          : 0;

  uint64_t realtime_max_latency =
      (ctime > scan_context.earliest_rt_make_runnable_time)
          ? ctime - scan_context.earliest_rt_make_runnable_time
          : 0;

  machine_max_runnable_latency(bg_max_latency, default_max_latency,
...
#include <i386/misc_protos.h>
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_MAINTENANCE) |
                            DBG_FUNC_END,
                        sched_pri_shifts[TH_BUCKET_SHARE_FG],
                        sched_pri_shifts[TH_BUCKET_SHARE_BG],
                        sched_pri_shifts[TH_BUCKET_SHARE_UT],
                        sched_pri_shifts[TH_BUCKET_SHARE_DF], 0);
...
void sched_timeshare_consider_maintenance(uint64_t ctime, bool safe_point) {
  ...
  if (__improbable(ctime >= deadline)) {
    ...
    if (__probable(os_atomic_cmpxchg(&sched_maintenance_deadline, deadline,
                                     ndeadline, seq_cst))) {
...
#if !CONFIG_SCHED_CLUTCH
  /*
   * Only non-clutch schedulers use the global load calculation EWMA algorithm.
   * For clutch scheduler, the load is maintained at the thread group and bucket
   * level.
   */
  uint64_t load_compute_deadline =
      os_atomic_load_wide(&sched_load_compute_deadline, relaxed);

  if (__improbable(load_compute_deadline && ctime >= load_compute_deadline)) {
    uint64_t new_deadline = 0;
    if (os_atomic_cmpxchg(&sched_load_compute_deadline, load_compute_deadline,
                          new_deadline, relaxed)) {
      compute_sched_load();
...
#if __arm64__
  uint64_t perf_deadline =
      os_atomic_load(&sched_perfcontrol_callback_deadline, relaxed);

  if (__improbable(perf_deadline && ctime >= perf_deadline)) {
    /* CAS in 0, if success, make callback. Otherwise let the next context
     * switch check again. */
    if (os_atomic_cmpxchg(&sched_perfcontrol_callback_deadline, perf_deadline,
                          0, relaxed)) {
...
boolean_t thread_update_add_thread(thread_t thread) {
...
/* Returns whether the kernel should report that a thread triggered the
 * fail-safe. */
static bool thread_should_report_failsafe(thread_t thread) {
...
void thread_update_process_threads(void) {
  ...
  for (uint32_t i = 0; i < thread_update_count; i++) {
    ...
    if (should_report_failsafe) {
      ...
      const int quanta = (saved_mode & TH_MODE_REALTIME)
                             ? max_unsafe_rt_quanta
                             : max_unsafe_fixed_quanta;
      const char *mode = (saved_mode & TH_MODE_REALTIME) ? "realtime" : "fixed";
      os_log_error(
          OS_LOG_DEFAULT,
          "scheduler: thread %s [%llx] in "
          "process %s [%d] triggered fail-safe by spinning for at least %d"
          "us at %s priority\n",
          th_name, th_id, t_name, t_pid, quanta * (int)sched_get_quantum_us(),
          mode);
    }
...
static boolean_t runq_scan_thread(thread_t thread,
  ...
  if (cpu_throttle_enabled && ((thread->sched_pri <= MAXPRI_THROTTLE) &&
                               (thread->base_pri <= MAXPRI_THROTTLE))) {
    if (thread->last_made_runnable_time <
        scan_context->earliest_bg_make_runnable_time) {
      scan_context->earliest_bg_make_runnable_time =
          thread->last_made_runnable_time;
    }
  } else {
    if (thread->last_made_runnable_time <
        scan_context->earliest_normal_make_runnable_time) {
      scan_context->earliest_normal_make_runnable_time =
          thread->last_made_runnable_time;
    }
  }
 ...
 */
boolean_t runq_scan(run_queue_t runq,
                    sched_update_scan_context_t scan_context) {
  ...
  for (queue_index = bitmap_first(runq->bitmap, NRQS); queue_index >= 0;
...
boolean_t
sched_clutch_timeshare_scan(queue_t thread_queue, uint16_t thread_count,
                            sched_update_scan_context_t scan_context) {
  if (thread_count == 0) {
    return FALSE;
  }
 ...
 */
void sched_stats_handle_csw(processor_t processor, int reasons, int selfpri,
                            int otherpri) {
...
void sched_stats_handle_runq_change(struct runq_stats *stats, int old_count) {
 ...
 */
#undef thread_wakeup
void thread_wakeup(event_t x);

void thread_wakeup(event_t x) { thread_wakeup_with_result(x, THREAD_AWAKENED); }

boolean_t preemption_enabled(void) {
...
/*
 * Check that all CPUs are successfully powered up in places where that's
 * expected.
 */
static void
check_all_cpus_are_done_starting(processor_start_kind_t start_kind) {
  /*
   * `processor_count` may include registered CPUs above cpus= or cpumask=
   * limit. Use machine_info.logical_cpu_max for the CPU IDs that matter.
   */
...
static void assert_no_processors_in_transition_locked() {
...
static struct powered_cores_state sched_compute_requested_powered_cores() {
  simple_lock_assert(&sched_available_cores_lock, LCK_ASSERT_OWNED);
  ...
  /*
   * if we unify this with derecommendation, note that only sleep should stop
   * derecommendation, not dtrace et al
   */
  if (pcs.pcs_powerdown_suspend_count) {
     ...
     */
    cpumap_t system_required_powered_cores =
        pcs.pcs_required_online_pmgr | pcs.pcs_required_online_system;
    ...
    if (pcs.pcs_user_online_core_control) {
      ...
      online_cores_goal =
          pcs.pcs_requested_online_user | system_required_powered_cores;
    } else {
      ...
      online_cores_goal =
          clpc_wanted_powered_cores | system_required_powered_cores;

      /* Any cores in managed cores that are not in wanted powered become
       * temporary */
      output.pcs_tempdown_cores =
          (pcs.pcs_managed_cores & ~clpc_wanted_powered_cores);
...
static bool sched_needs_update_requested_powered_cores() {
  if (!pcs.pcs_init_completed) {
    return false;
  }

  struct powered_cores_state requested =
      sched_compute_requested_powered_cores();

  struct powered_cores_state effective = pcs.pcs_effective;

  if (requested.pcs_powerdown_recommended_cores !=
          effective.pcs_powerdown_recommended_cores ||
...
kern_return_t sched_processor_exit_user(processor_t processor) {
  assert(processor);
  ...
  if ((pcs.pcs_recommended_cores & pcs.pcs_effective.pcs_online_cores) ==
      BIT(processor->cpu_id)) {
...
kern_return_t sched_processor_start_user(processor_t processor) {
  assert(processor);
...
static void OS_NORETURN sched_update_powered_cores_continue(
    void *param __unused, wait_result_t wr __unused);

/*
 * After all processors have been ml_processor_register'ed and processor_boot'ed
 * the scheduler can finalize its datastructures and allow CPU power state
 * changes.
 *
 * Enforce that this only happens *once*. More than once is definitely not OK.
 * rdar://121270513
 */
void sched_cpu_init_completed(void) {
  static bool sched_cpu_init_completed_called = false;

  if (!os_atomic_cmpxchg(&sched_cpu_init_completed_called, false, true,
                         relaxed)) {
    panic("sched_cpu_init_completed called twice! %d",
          sched_cpu_init_completed_called);
  }
  ...
  /* Wait for any cpu that is still starting, and enforce that they eventually
   * complete. */
  ...
  kern_return_t result = kernel_thread_start_priority(
      sched_update_powered_cores_continue, NULL, MAXPRI_KERNEL,
  ...
  assert(__builtin_popcountll(pcs.pcs_managed_cores) ==
  ...
  assert(pcs.pcs_requested.pcs_powerdown_recommended_cores ==
         pcs.pcs_managed_cores);
  assert(pcs.pcs_requested.pcs_online_cores == pcs.pcs_managed_cores);
  assert(pcs.pcs_requested.pcs_tempdown_cores == 0);

  assert(pcs.pcs_effective.pcs_powerdown_recommended_cores ==
         pcs.pcs_managed_cores);
...
bool sched_is_in_sleep(void) {
  return pcs.pcs_in_kernel_sleep || pcs.pcs_wants_kernel_sleep;
}

bool sched_is_cpu_init_completed(void) { return pcs.pcs_init_completed; }
  ...
  while (true) {
    ...
    if (needs_update) {
      ...
      processor_reason_t reason =
          last_sched_update_powered_cores_continue_reason;
...
__options_decl(sched_powered_cores_flags_t, uint32_t,
               {
                   ...
                   POWERED_CORES_OPTIONS_MASK = ASSERT_IN_SLEEP |
                                                ASSERT_POWERDOWN_SUSPENDED,
               });

/*
 * This is KPI with CLPC.
 */
void sched_perfcontrol_update_powered_cores(uint64_t requested_powered_cores,
                                            processor_reason_t reason,
 ...
 */
bool sched_perfcontrol_check_oncore_thread_preemption(
    __unused uint64_t flags, int cpu_id __assert_only) {
  bool ret = false;
  assert(ml_get_interrupts_enabled() == false);
  ...
  if (preempt != AST_NONE) {
    /*
     * TODO: Returning true here is best effort and isn't guaranteed to preempt
     * the thread since thread_select can choose to leave the thread on the same
     * processor. Consider using the flags passed in here to callback into CLPC
     * before the next scheduling decision point (or sampler tick) if this
     * decision needs to be reevaluated or to otherwise adjust this behavior.
     */
    ret = true;
    ast_on(preempt);
    KERNEL_DEBUG_CONSTANT(
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_ONCORE_PREEMPT),
        thread_tid(thread), processor->cpu_id, 0, 0, 0);
  }
  thread_unlock(thread);
 ...
 */
static void suspend_cluster_powerdown_locked(bool for_sleep) {
  lck_mtx_assert(&cluster_powerdown_lock, LCK_MTX_ASSERT_OWNED);
  ...
  if (pcs.pcs_init_completed) {
    ...
    check_all_cpus_are_done_starting(for_sleep
                                         ? PROCESSOR_BEFORE_ENTERING_SLEEP
                                         : PROCESSOR_CLUSTER_POWERDOWN_SUSPEND);
  }
}

static void resume_cluster_powerdown_locked(bool for_sleep) {
  lck_mtx_assert(&cluster_powerdown_lock, LCK_MTX_ASSERT_OWNED);

  if (pcs.pcs_init_completed) {
    /* At this point, no cpu should be still starting. Let's enforce that. */
    check_all_cpus_are_done_starting(for_sleep
                                         ? PROCESSOR_WAKE_FROM_SLEEP
                                         : PROCESSOR_CLUSTER_POWERDOWN_RESUME);
  }
  ...
  if (pcs.pcs_powerdown_suspend_count <= 0) {
    panic("resume_cluster_powerdown() called with "
          "pcs.pcs_powerdown_suspend_count=%d\n",
          pcs.pcs_powerdown_suspend_count);
  }
...
static uint64_t
die_and_cluster_to_cpu_mask(__unused unsigned int die_id,
 ...
 */
void sched_enable_acc_rail(unsigned int die_id, unsigned int die_cluster_id) {
 ...
 */
void sched_disable_acc_rail(unsigned int die_id, unsigned int die_cluster_id) {
  uint64_t core_mask = die_and_cluster_to_cpu_mask(die_id, die_cluster_id);
...
void suspend_cluster_powerdown(void) {
  lck_mtx_lock(&cluster_powerdown_lock);
  suspend_cluster_powerdown_locked(false);
  lck_mtx_unlock(&cluster_powerdown_lock);
}

void resume_cluster_powerdown(void) {
  lck_mtx_lock(&cluster_powerdown_lock);
...
kern_return_t suspend_cluster_powerdown_from_user(void) {
...
kern_return_t resume_cluster_powerdown_from_user(void) {
  kern_return_t ret = KERN_FAILURE;
...
void sched_set_powered_cores(int requested_powered_cores) {
  processor_reason_t reason = bit_test(requested_powered_cores, 31)
                                  ? REASON_CLPC_USER
                                  : REASON_CLPC_SYSTEM;
  sched_powered_cores_flags_t flags =
      requested_powered_cores & POWERED_CORES_OPTIONS_MASK;

  saved_requested_powered_cores = requested_powered_cores;

  requested_powered_cores = bits(requested_powered_cores, 28, 0);

  sched_perfcontrol_update_powered_cores(requested_powered_cores, reason,
                                         flags);
}
int sched_get_powered_cores(void) { return (int)saved_requested_powered_cores; }
 ...
 */
void sched_override_available_cores_for_sleep(void) {
  if (!pcs.pcs_init_completed) {
 ...
 */
void sched_restore_available_cores_after_sleep(void) {
  lck_mtx_assert(&cluster_powerdown_lock, LCK_MTX_ASSERT_OWNED);
 ...
 *
 * No scheduler locks are held, no other locks are held that scheduler might
 * depend on, interrupts are enabled
 *
 * currently prototype is in osfmk/arm/machine_routines.h
 */
void sched_perfcontrol_update_recommended_cores_reason(
    uint64_t recommended_cores, processor_reason_t reason,
    __unused uint32_t flags) {
  assert(preemption_enabled());
  ...
  pcs.pcs_requested_recommended_clpc =
      pcs.pcs_requested_recommended_clpc_system &
...
void sched_perfcontrol_update_recommended_cores(uint32_t recommended_cores) {
  sched_perfcontrol_update_recommended_cores_reason(recommended_cores,
                                                    REASON_CLPC_USER, 0);
}
 ...
 */
void sched_consider_recommended_cores(uint64_t ctime, thread_t cur_thread) {
  /*
  ...
  if (__probable(bit_count(pcs.pcs_requested_recommended_clpc) >=
                 processor_count)) {
    return;
  }
  ...
  /* Capture some data about who screwed up (assuming that the thread on core is
   * at fault) */
  ...
  uint64_t recent_computation =
      (ctime - cur_thread->computation_epoch) + cur_thread->computation_metered;

  uint64_t last_seen = recount_current_thread_time_mach();

  /* Compute the start time of the bad behavior in terms of the thread's on core
   * time */
...
/*
 * Now that our bacon has been saved by the failsafe, consider whether to turn
 * it off
 *
 ...
 */
bool sched_all_cpus_offline(void) {
  return pcs.pcs_effective.pcs_online_cores == 0;
}

void sched_assert_not_last_online_cpu(__assert_only int cpu_id) {
  assertf(pcs.pcs_effective.pcs_online_cores != BIT(cpu_id),
          "attempting to shut down the last online CPU!");
}

/*
 * This is the unified single function to change published active core counts
 * based on processor mode. Each type of flag affects the other in terms of how
 * the counts change.
 *
 * Future: Add support for not decrementing counts in 'temporary derecommended
 * online' mode Future: Shutdown for system sleep should be 'temporary'
 * according to the user counts so that no client sees a transiently low number
 * of CPUs.
 */
void sched_processor_change_mode_locked(processor_t processor,
                                        processor_mode_t pcm_mode, bool set) {
  simple_lock_assert(&sched_available_cores_lock, LCK_ASSERT_OWNED);
  ...
  case PCM_TEMPORARY:
    if (set) {
      ...
      assert(
          !bit_test(pcs.pcs_effective.pcs_tempdown_cores, processor->cpu_id));
...
bool sched_mark_processor_online(processor_t processor,
                                 __assert_only processor_reason_t reason) {
  assert(processor == current_processor());
  ...
  assert(processor->processor_offline_state == PROCESSOR_OFFLINE_STARTING ||
         processor->processor_offline_state ==
             PROCESSOR_OFFLINE_STARTED_NOT_RUNNING ||
         processor->processor_offline_state ==
             PROCESSOR_OFFLINE_FINAL_SYSTEM_SLEEP);

  processor_update_offline_state_locked(processor,
                                        PROCESSOR_OFFLINE_STARTED_NOT_WAITED);

  ++pset->online_processor_count;
  pset_update_processor_state(pset, processor, PROCESSOR_RUNNING);

  if (processor->is_recommended) {
    SCHED(pset_made_schedulable)(processor, pset,
                                 false); /* May relock the pset lock */
  }
  pset_unlock(pset);
...
void sched_mark_processor_offline(processor_t processor,
  ...
  assert(bit_test(pcs.pcs_effective.pcs_online_cores, processor->cpu_id));
  assert(processor->processor_offline_state ==
         PROCESSOR_OFFLINE_BEGIN_SHUTDOWN);
  ...
  if (!is_final_system_sleep) {
    ...
    if ((pcs.pcs_effective.pcs_online_cores & pcs.pcs_recommended_cores) ==
        BIT(processor->cpu_id)) {
      sched_update_recommended_cores_locked(REASON_SYSTEM,
                                            BIT(processor->cpu_id));
    }
    ...
    if ((pcs.pcs_effective.pcs_online_cores & pcs.pcs_recommended_cores) ==
        BIT(processor->cpu_id)) {
      panic(
          "shutting down the last available core! online: 0x%llx rec: 0x%llxx",
          pcs.pcs_effective.pcs_online_cores, pcs.pcs_recommended_cores);
    }
  }
  ...
  if (is_final_system_sleep) {
    ...
    processor_update_offline_state_locked(processor,
                                          PROCESSOR_OFFLINE_PENDING_OFFLINE);
  }
...
static void sched_update_recommended_cores_locked(processor_reason_t reason,
  ...
  if (pcs.pcs_sleep_override_recommended ||
      pcs.pcs_recommended_clpc_failsafe_active) {
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE,
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_REC_CORES_FAILSAFE) | DBG_FUNC_NONE,
        recommended_cores, sched_maintenance_thread->last_made_runnable_time, 0,
        0, 0);

    recommended_cores = pcs.pcs_managed_cores;
  }

  if (bit_count(recommended_cores & pcs.pcs_effective.pcs_online_cores &
                ~core_going_offline) == 0) {
    /*
    ...
    if (!support_bootcpu_shutdown) {
      ...
      last_resort =
          lsb_first(pcs.pcs_effective.pcs_online_cores & ~core_going_offline);

      if (last_resort == -1) {
        panic("%s> no last resort cpu found: 0x%llx 0x%llx", __func__,
              pcs.pcs_effective.pcs_online_cores, core_going_offline);
      }
    }
  ...
  KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_UPDATE_REC_CORES) |
           DBG_FUNC_START,
       recommended_cores, pcs.pcs_recommended_clpc_failsafe_active,
       pcs.pcs_sleep_override_recommended, 0);
  ...
  foreach_node(node) {
    foreach_pset_id(pset_id, node) {
      ...
      cpumap_t changed_recommendations =
          (recommended_cores & pset->cpu_bitmask) ^ pset->recommended_bitmask;
      ...
      cpumap_foreach(cpu_id, newly_recommended) {
        ...
        if (pset->recommended_bitmask == 0) {
          ...
          atomic_bit_set(&pset->node->pset_recommended_map, pset->pset_id,
                         memory_order_relaxed);
        }
  ...
  foreach_node(node) {
    foreach_pset_id(pset_id, node) {
      ...
      cpumap_t changed_recommendations =
          (recommended_cores & pset->cpu_bitmask) ^ pset->recommended_bitmask;
      cpumap_t newly_unrecommended =
          changed_recommendations & ~recommended_cores;
      ...
      cpumap_foreach(cpu_id, newly_unrecommended) {
        ...
        if (pset->recommended_bitmask == 0) {
          ...
          atomic_bit_clear(&pset->node->pset_recommended_map, pset->pset_id,
                           memory_order_relaxed);
        }
...
/*
 * Enters with the available cores lock held, returns with it held, but will
 * drop it in the meantime. Enters with the cluster_powerdown_lock held, returns
 * with it held, keeps it held.
 */
static void
sched_update_powered_cores_drops_lock(processor_reason_t requested_reason,
                                      spl_t caller_s) {
  ...
  assert(ml_get_interrupts_enabled() == false);
  assert(caller_s == true); /* Caller must have had interrupts enabled when they
                               took the lock */
  ...
  KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_UPDATE_POWERED_CORES) | DBG_FUNC_START,
       requested.pcs_online_cores, requested_reason, 0,
       effective.pcs_online_cores);

  /* The bits that are different and in the new value */
  cpumap_t newly_online_cores =
      (requested.pcs_online_cores ^ effective.pcs_online_cores) &
      requested.pcs_online_cores;

  /* The bits that are different and are not in the new value */
  cpumap_t newly_offline_cores =
      (requested.pcs_online_cores ^ effective.pcs_online_cores) &
      ~requested.pcs_online_cores;

  cpumap_t newly_recommended_cores =
      (requested.pcs_powerdown_recommended_cores ^
       effective.pcs_powerdown_recommended_cores) &
      requested.pcs_powerdown_recommended_cores;

  cpumap_t newly_derecommended_cores =
      (requested.pcs_powerdown_recommended_cores ^
       effective.pcs_powerdown_recommended_cores) &
      ~requested.pcs_powerdown_recommended_cores;

  cpumap_t newly_temporary_cores =
      (requested.pcs_tempdown_cores ^ effective.pcs_tempdown_cores) &
      requested.pcs_tempdown_cores;

  cpumap_t newly_nontemporary_cores =
      (requested.pcs_tempdown_cores ^ effective.pcs_tempdown_cores) &
      ~requested.pcs_tempdown_cores;
  ...
  cpumap_t newly_online_and_derecommended =
      newly_online_cores & newly_derecommended_cores;
  ...
  pcs.pcs_effective.pcs_powerdown_recommended_cores &=
      ~newly_online_and_derecommended;
  ...
  cpumap_foreach(cpu_id, started_cores) {
    processor_wait_for_start(processor_array[cpu_id],
                             PROCESSOR_POWERED_CORES_CHANGE);
  }
  ...
  foreach_node(node) {
    foreach_pset_id(pset_id, node) {
      ...
      cpumap_t pset_newly_nontemporary =
          newly_nontemporary_cores & pset->cpu_bitmask;
  ...
  assert3u(pcs.pcs_requested.pcs_online_cores, ==,
           pcs.pcs_effective.pcs_online_cores);
  assert3u(pcs.pcs_requested.pcs_tempdown_cores, ==,
           pcs.pcs_effective.pcs_tempdown_cores);
  assert3u(pcs.pcs_requested.pcs_powerdown_recommended_cores, ==,
           pcs.pcs_effective.pcs_powerdown_recommended_cores);
...
uint32_t sched_qos_max_parallelism(__unused int qos, uint64_t options) {
  ...
  /*
   * The QOS_PARALLELISM_CLUSTER_SHARED_RESOURCE should be used on AMP platforms
   * only which implement their own qos_max_parallelism() interfaces.
   */
  ...
  __assert_only kern_return_t kret =
      host_info(host_self(), HOST_BASIC_INFO, (host_info_t)&hinfo, &count);
...
#if CONFIG_SCHED_SMT
bool thread_no_smt(thread_t thread) {
...
bool processor_active_thread_no_smt(processor_t processor) {
  return sched_allow_NO_SMT_threads && !processor->current_is_bound &&
 ...
 */
boolean_t sched_perfcontrol_update_callback_deadline(uint64_t new_deadline) {
  /*
 ...
 */
kern_return_t sched_perfcontrol_sfi_set_window(uint64_t window_usecs) {
 ...
 */
kern_return_t sched_perfcontrol_sfi_set_bg_offtime(uint64_t offtime_usecs) {
  kern_return_t ret = KERN_NOT_SUPPORTED;
#if CONFIG_THREAD_GROUPS
 ...
 * migrated etc.
 */
static void sched_edge_pset_running_higher_bucket(processor_set_t pset,
  ...
  for (int cpu = bitmap_first(active_map, MAX_CPUS); cpu >= 0;
       cpu = bitmap_next(active_map, cpu)) {
    sched_bucket_t cpu_bucket =
        os_atomic_load(&pset->cpu_running_buckets[cpu], relaxed);
  ...
  for (sched_bucket_t bucket = TH_BUCKET_FIXPRI;
       bucket < TH_BUCKET_SCHED_MAX - 1; bucket++) {
 ...
 */
void sched_update_pset_load_average(processor_set_t pset, uint64_t curtime) {
  ...
  for (cluster_shared_rsrc_type_t shared_rsrc_type =
    ...
    uint64_t shared_rsrc_runnable_load = sched_edge_shared_rsrc_runnable_load(
        &pset->pset_clutch_root, shared_rsrc_type);
    uint64_t shared_rsrc_running_load = bit_count(
        pset->cpu_running_cluster_shared_rsrc_thread[shared_rsrc_type]);
    uint64_t new_shared_load =
        shared_rsrc_runnable_load + shared_rsrc_running_load;
    uint64_t old_shared_load =
        os_atomic_xchg(&pset->pset_cluster_shared_rsrc_load[shared_rsrc_type],
                       new_shared_load, relaxed);
    if (old_shared_load != new_shared_load) {
      KTRC(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH,
                        MACH_SCHED_EDGE_CLUSTER_SHARED_LOAD) |
               DBG_FUNC_NONE,
           pset->pset_cluster_id, shared_rsrc_type, new_shared_load,
           shared_rsrc_running_load);
    }
  }
  ...
  for (sched_bucket_t sched_bucket = TH_BUCKET_FIXPRI;
       sched_bucket < TH_BUCKET_SCHED_MAX; sched_bucket++) {
    uint64_t old_load_average =
        os_atomic_load(&pset->pset_load_average[sched_bucket], relaxed);
    uint64_t old_load_average_factor =
        old_load_average * SCHED_PSET_LOAD_EWMA_TC_NSECS;
    uint32_t current_runq_depth =
        sched_edge_cluster_cumulative_count(&pset->pset_clutch_root,
                                            sched_bucket) +
        rt_runq_count(pset) + running_higher[sched_bucket];
    os_atomic_store(&pset->pset_runnable_depth[sched_bucket],
                    current_runq_depth, relaxed);

    uint32_t current_load = current_runq_depth / avail_cpu_count;
    /*
     * For the new load average multiply current_load by delta_nsecs (which
     * results in a 32.0 value). Since we want to maintain the load average as
     * a 24.8 fixed arithmetic value for precision, the new load average needs
     * to be shifted before it can be added to the old load average.
     */
    uint64_t new_load_average_factor = (current_load * delta_nsecs)
                                       << SCHED_PSET_LOAD_EWMA_FRACTION_BITS;

    /*
     * For extremely parallel workloads, it is important that the load average
     * on a cluster moves zero to non-zero instantly to allow threads to be
     * migrated to other (potentially idle) clusters quickly. Hence use the EWMA
     * when the system is already loaded; otherwise for an idle system use the
     * latest load average immediately.
     */
    int old_load_shifted =
        (int)((old_load_average + SCHED_PSET_LOAD_EWMA_ROUND_BIT) >>
              SCHED_PSET_LOAD_EWMA_FRACTION_BITS);
    ...
    if (load_uptick || load_downtick) {
      ...
      load_average = (old_load_average_factor + new_load_average_factor) /
                     (delta_nsecs + SCHED_PSET_LOAD_EWMA_TC_NSECS);
    }
    os_atomic_store(&pset->pset_load_average[sched_bucket], load_average,
                    relaxed);
    if (load_average != old_load_average) {
      KTRC(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_LOAD_AVG) |
               DBG_FUNC_NONE,
           pset->pset_cluster_id,
           (load_average >> SCHED_PSET_LOAD_EWMA_FRACTION_BITS),
           load_average & SCHED_PSET_LOAD_EWMA_FRACTION_MASK, sched_bucket);
    }
  }
  os_atomic_store(&pset->pset_load_last_update, curtime, relaxed);
}

void sched_update_pset_avg_execution_time(processor_set_t pset,
  ...
  os_atomic_rmw_loop(
      &pset->pset_execution_time[sched_bucket].pset_execution_time_packed,
      old_execution_time_packed.pset_execution_time_packed,
      new_execution_time_packed.pset_execution_time_packed, relaxed, {
        uint64_t last_update =
            old_execution_time_packed.pset_execution_time_last_update;
        int64_t delta_ticks = curtime - last_update;
        if (delta_ticks <= 0) {
          /*
           * Its possible that another CPU came in and updated the
           * pset_execution_time before this CPU could do it. Since the average
           * execution time is meant to be an approximate measure per cluster,
           * ignore the older update.
           */
        ...
        /*
         * Since the average execution time is stored in microseconds, avoid
         * rounding errors in the EWMA calculation by only using a non-zero
         * previous value.
         */
        uint64_t old_avg_thread_execution_time =
            MAX(old_execution_time_packed.pset_avg_thread_execution_time, 1ULL);

        uint64_t old_execution_time =
            (old_avg_thread_execution_time * SCHED_PSET_LOAD_EWMA_TC_NSECS);
        uint64_t new_execution_time = (execution_time_us * delta_nsecs);

        avg_thread_execution_time =
            (old_execution_time + new_execution_time) /
            (delta_nsecs + SCHED_PSET_LOAD_EWMA_TC_NSECS);
        new_execution_time_packed.pset_avg_thread_execution_time =
            avg_thread_execution_time;
        new_execution_time_packed.pset_execution_time_last_update = curtime;
      });
  if (new_execution_time_packed.pset_avg_thread_execution_time !=
      old_execution_time_packed.pset_execution_time_packed) {
    KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PSET_AVG_EXEC_TIME) | DBG_FUNC_NONE,
         pset->pset_cluster_id, avg_thread_execution_time, sched_bucket);
  }
}

uint64_t sched_pset_cluster_shared_rsrc_load(
    processor_set_t pset, cluster_shared_rsrc_type_t shared_rsrc_type) {
  /* Prevent migrations to derecommended clusters */
  if (!pset_is_recommended(pset)) {
    return UINT64_MAX;
  }
  return os_atomic_load(&pset->pset_cluster_shared_rsrc_load[shared_rsrc_type],
                        relaxed);
}

#else /* CONFIG_SCHED_EDGE */

void sched_update_pset_load_average(processor_set_t pset,
                                    __unused uint64_t curtime) {
  int non_rt_load = pset->pset_runq.count;
  int load = ((bit_count(pset->cpu_state_map[PROCESSOR_RUNNING]) + non_rt_load +
               rt_runq_count(pset))
  ...
  if (pset->pset_cluster_type == PSET_AMP_P) {
    KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PSET_LOAD_AVERAGE) | DBG_FUNC_NONE,
         sched_get_pset_load_average(pset, 0),
         (bit_count(pset->cpu_state_map[PROCESSOR_RUNNING]) +
          pset->pset_runq.count + rt_runq_count(pset)));
  }
#endif
#endif
}
...
void sched_update_pset_avg_execution_time(
    __unused processor_set_t pset, __unused uint64_t execution_time,
    __unused uint64_t curtime, __unused sched_bucket_t sched_bucket) {}

#endif /* CONFIG_SCHED_EDGE */

/* pset is locked */
bool processor_is_fast_track_candidate_for_realtime_thread(
    processor_set_t pset, processor_t processor) {
  ...
  cpumap_t fasttrack_map = pset_available_cpumap(pset) &
                           ~pset->pending_AST_URGENT_cpu_mask &
                           ~pset->realtime_map;
...
all_available_primaries_are_running_realtime_threads(processor_set_t pset,
  ...
  cpumap_t cpu_map =
      pset_available_cpumap(pset) & pset->primary_map & ~pset->realtime_map;
...
/* pset is locked */
static bool these_processors_are_running_realtime_threads(
    processor_set_t pset, uint64_t these_map, bool include_backups) {
  int nbackup_cpus = 0;
  ...
  cpumap_t cpu_map =
      pset_available_cpumap(pset) & these_map & ~pset->realtime_map;
  return (rt_runq_count(pset) + nbackup_cpus) > bit_count(cpu_map);
}
#endif /* CONFIG_SCHED_SMT */
...
static bool sched_ok_to_run_realtime_thread(processor_set_t pset,
  ...
  if (processor->cpu_id == 0) {
    if (sched_avoid_cpu0 == 1) {
      ok_to_run_realtime_thread = these_processors_are_running_realtime_threads(
          pset, pset->primary_map & ~0x1, as_backup);
    } else if (sched_avoid_cpu0 == 2) {
      ok_to_run_realtime_thread =
          these_processors_are_running_realtime_threads(pset, ~0x3, as_backup);
    }
  } else if (sched_avoid_cpu0 && (processor->cpu_id == 1) &&
             processor->is_SMT) {
    ok_to_run_realtime_thread =
        sched_allow_rt_smt &&
        these_processors_are_running_realtime_threads(pset, ~0x2, as_backup);
  } else if (processor->processor_primary != processor) {
    ok_to_run_realtime_thread =
        (sched_allow_rt_smt &&
         all_available_primaries_are_running_realtime_threads(pset, as_backup));
  }
#else  /* CONFIG_SCHED_SMT */
  (void)pset;
  (void)processor;
...
void sched_pset_made_schedulable(__unused processor_t processor,
                                 processor_set_t pset, boolean_t drop_lock) {
  if (drop_lock) {
    pset_unlock(pset);
  }
}
...
#if defined(__x86_64__)
void thread_set_no_smt(bool set) {
  (void)set;
#if CONFIG_SCHED_SMT
  if (!system_is_SMT) {
...
void sysctl_task_set_no_smt(char no_smt) {
  if (!system_is_SMT) {
    /* Not a machine that supports SMT */
    return;
  }
...
char sysctl_task_get_no_smt(void) {
  task_t task = current_task();
...
extern void task_set_no_smt(task_t);
void task_set_no_smt(__unused task_t task) { return; }

#if DEBUG || DEVELOPMENT
extern void sysctl_task_set_no_smt(char no_smt);
void sysctl_task_set_no_smt(__unused char no_smt) { return; }

extern char sysctl_task_get_no_smt(void);
char sysctl_task_get_no_smt(void) { return '1'; }
#endif /* DEBUG || DEVELOPMENT */
#endif /* CONFIG_SCHED_SMT */

#if __AMP__
static kern_return_t
pset_cluster_type_from_name_char(char cluster_type_name,
...
uint32_t thread_bound_cluster_id(thread_t thread) {
  return thread->th_bound_cluster_id;
}

__private_extern__ kern_return_t
thread_soft_bind_cluster_id(thread_t thread, uint32_t cluster_id,
                            thread_bind_option_t options) {
#if __AMP__
...
static char pset_cluster_type_to_name_char(pset_cluster_type_t pset_type) {
  switch (pset_type) {
  case PSET_AMP_E:
    return 'E';
  case PSET_AMP_P:
...
char sysctl_get_task_cluster_type(void) {
#if __AMP__
  task_t task = current_task();
...
char sysctl_get_bound_cluster_type(void) {
  thread_t self = current_thread();
  ...
  pset_cluster_type_t pset_type =
      pset_array[self->th_bound_cluster_id]->pset_cluster_type;
  return pset_cluster_type_to_name_char(pset_type);
}

static processor_set_t find_pset_of_type(pset_cluster_type_t t) {
...
kern_return_t sysctl_task_set_cluster_type(char cluster_type) {
#if __AMP__
  kern_return_t kr;
  task_t task = current_task();
  pset_cluster_type_t pset_cluster_type;
...
kern_return_t sysctl_clutch_thread_group_cpu_time_for_thread(
    thread_t thread, int sched_bucket, uint64_t *cpu_stats) {
  return sched_clutch_thread_group_cpu_time_for_thread(thread, sched_bucket,
                                                       cpu_stats);
}
...
sysctl_clutch_thread_group_cpu_time_for_thread(__unused thread_t thread,
                                               __unused int sched_bucket,
                                               __unused uint64_t *cpu_stats) {
