...
/* socd_client.h: machine-independent API for interfacing with soc diagnostics
 * data pipeline. NOTE: this file is included by socd parser and should not
 * declare any symbols nor include kernel specific headers. Use
 * socd_client_kern.h for kernel specifics.
 */
...
/* "stickiness" is an attribute in the debugid that means "dont overwrite this
 * entry" */
...
#define SOCD_TRACE_EXTRACT_MODE(debugid)                                       \
  ((SOCD_TRACE_EXTRACT_EVENTID(debugid) & SOCD_TRACE_MODE_MASK) >>             \
   SOCD_TRACE_MODE_OFFSET)
#define SOCD_TRACE_EXTRACT_CLASS(debugid)                                      \
  ((SOCD_TRACE_EXTRACT_EVENTID(debugid) & SOCD_TRACE_CLASS_MASK) >>            \
   SOCD_TRACE_CLASS_OFFSET)
#define SOCD_TRACE_EXTRACT_CODE(debugid)                                       \
  ((SOCD_TRACE_EXTRACT_EVENTID(debugid) & SOCD_TRACE_CODE_MASK) >>             \
   SOCD_TRACE_CODE_OFFSET)
...
/* SOCD_TRACE_GEN_STR is used by socd parser to symbolicate trace classes &
 * codes */
 ...
 * SOC: Tracepoints relating to SoC-wide events or errors (e.g. an SoC thermal
 * sensor reading)
 */
#define SOCD_TRACE_FOR_EACH_CLASS(iter) iter(XNU) iter(WDT) iter(SOC)

/* List of xnu trace codes */
#define SOCD_TRACE_FOR_EACH_XNU_CODE(iter)                                     \
  iter(XNU_PANIC) iter(XNU_START_IOKIT) iter(XNU_PLATFORM_ACTION)              \
      iter(XNU_PM_SET_POWER_STATE) iter(XNU_PM_INFORM_POWER_CHANGE)            \
          iter(XNU_STACKSHOT) iter(XNU_PM_SET_POWER_STATE_ACK)                 \
              iter(XNU_PM_INFORM_POWER_CHANGE_ACK)                             \
                  iter(XNU_KERNEL_STATE_PANIC)

typedef enum {
  SOCD_TRACE_FOR_EACH_CLASS(SOCD_TRACE_GEN_CLASS_ENUM) SOCD_TRACE_CLASS_MAX
} socd_client_trace_class_t;

typedef enum {
  SOCD_TRACE_FOR_EACH_XNU_CODE(SOCD_TRACE_GEN_CODE_ENUM) SOCD_TRACE_CODE_XNU_MAX
 ...
 *     (4 bytes) socd_desc_t (size of overall region, number of records that are
 * supported === 1) socd_push_record_t: (1 byte) agent ID (1 byte) version (2
 * bytes) offset into buff for start of record in 32-bit words (2 bytes) size
 * (same accounting) (2 bytes) --padding for alignement-- socd_client_hdr_t:
 * <--- header reports offset 0x14 here (4 bytes) version (8 bytes) boot time
