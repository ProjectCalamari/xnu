...
mach_kobj_count; /* count of total number of kobjects */

ZONE_DEFINE_TYPE(ipc_kobject_label_zone, "ipc kobject labels",
                 struct ipc_kobject_label, ZC_ZFREE_CLEARMEM);

__startup_const static struct mig_kern_subsystem *mig_e[] = {
...
__startup_func static void mig_init(void) {
    ...
    for (j = 0; j < range; j++) {
      if (mig_e[i]->kroutine[j].kstub_routine) {
        ...
        if (mig_e[i]->kroutine[j].max_reply_msg) {
          ...
          mig_buckets[pos].kreply_desc_cnt =
              mig_e[i]->kroutine[j].reply_descr_count;
          assert3u(mig_e[i]->kroutine[j].descr_count, <=, IPC_KOBJECT_DESC_MAX);
 ...
 * if not found.
 */
static mig_hash_t *find_mig_hash_entry(int msgh_id) {
 ...
 *
 *       If complex bit is set, we can deduce the call is successful since the
 * bit is set at the very end. If complex bit is not set, we must have returned
 * from MIG_RETURN_ERROR. MIG writes RetCode to immediately after the header,
 * and we know this is safe to do for all kmsg layouts. (See discussion in
 * ipc_kmsg_server_internal()).
 *
 *  (B) Simple Reply with Out Params
 *
 *      E.g.: thread_get_states()
 *
 *        If the call failed, we return from MIG_RETURN_ERROR, which writes
 * RetCode to immediately after the header. If the call succeeded, MIG writes
 * RetCode as KERN_SUCCESS to USER DATA buffer. *BUT* since the region after
 * header is always initialized with KERN_SUCCESS, reading from there gives us
 * the same result. We rely on this behavior to not make a special case.
 *
 ...
 */
static kern_return_t ipc_kobject_reply_status(ipc_kmsg_t reply) {
...
static void ipc_kobject_set_reply_error_status(ipc_kmsg_t reply,
                                               kern_return_t kr) {
 ...
 */
kern_return_t ipc_kobject_set_kobjidx(int msgh_id, int index) {
...
static void ipc_kobject_init_reply(ipc_kmsg_t reply, const ipc_kmsg_t request,
                                   kern_return_t kr) {
...
static void ipc_kobject_init_new_reply(ipc_kmsg_t new_reply,
...
static ipc_kmsg_t ipc_kobject_alloc_mig_error(void) {
  ipc_kmsg_alloc_flags_t flags = IPC_KMSG_ALLOC_KERNEL | IPC_KMSG_ALLOC_ZERO |
 ...
 *		Nothing locked.
 */
static kern_return_t ipc_kobject_server_internal(__unused ipc_port_t port,
                                                 ipc_kmsg_t request,
                                                 ipc_kmsg_t *replyp) {
  int request_msgh_id;
   ...
   * whole reply buffer.
   */
  reply = ipc_kmsg_alloc(reply_size, 0, reply_desc_cnt,
  ...
  assert((vm_offset_t)reply_hdr + sizeof(mig_reply_error_t) <=
         ikm_kdata_end(reply));
  ...
  static_assert(sizeof(mig_reply_error_t) <
                sizeof(mach_msg_base_t) + 1 * sizeof(mach_msg_kdescriptor_t));
  ...
  if (filter_mask != NULL &&
     ...
     */
    kern_return_t kr =
        mac_task_kobj_msg_evaluate(curproc, request_msgh_id, idx);
    if (kr != KERN_SUCCESS) {
...
skip_kobjcall:
  ...
  if (reply_status == MIG_NO_REPLY) {
    ...
    ipc_kmsg_free(reply);
    reply = IKM_NULL;
  } else if (reply_status != KERN_SUCCESS &&
             reply_size > sizeof(mig_reply_error_t)) {
 ...
 *		Nothing locked.
 */
ipc_kmsg_t ipc_kobject_server(ipc_port_t port, ipc_kmsg_t request,
  ...
  if (kr != KERN_SUCCESS) {
    assert(kr != MACH_SEND_TIMED_OUT && kr != MACH_SEND_INTERRUPTED &&
  ...
  if (reply == IKM_NULL || ipc_kobject_reply_status(reply) == KERN_SUCCESS) {
    /*
     ...
     */
    ipc_kmsg_destroy(request, IPC_KMSG_DESTROY_SKIP_LOCAL |
                                  IPC_KMSG_DESTROY_SKIP_REMOTE);
  }
...
static inline void ipc_kobject_set_raw(ipc_port_t port, ipc_kobject_type_t type,
                                       ipc_kobject_t kobject) {
  uintptr_t *store = &port->ip_kobject;

#if __has_feature(ptrauth_calls)
  type ^= OS_PTRAUTH_DISCRIMINATOR("ipc_port.ip_kobject");
  kobject = ptrauth_sign_unauthenticated(
      kobject, ptrauth_key_process_independent_data,
...
ipc_kobject_get_raw(ipc_port_t port, ipc_kobject_type_t type) {
  uintptr_t *store = &port->ip_kobject;
  ipc_kobject_t kobject = (ipc_kobject_t)*store;

#if __has_feature(ptrauth_calls)
  type ^= OS_PTRAUTH_DISCRIMINATOR("ipc_port.ip_kobject");
  kobject = ptrauth_auth_data(kobject, ptrauth_key_process_independent_data,
                              ptrauth_blend_discriminator(store, type));
#else
  (void)type;
#endif // __has_feature(ptrauth_calls)
...
__abortlike static void ipc_kobject_require_panic(ipc_port_t port,
                                                  ipc_kobject_t kobject,
                                                  ipc_kobject_type_t kotype) {
  if (ip_type(port) != kotype) {
    panic("port %p: invalid kobject type, got %d wanted %d", port,
          ip_type(port), kotype);
  }
  panic("port %p: invalid kobject, got %p wanted %p", port,
        ipc_kobject_get_raw(port, kotype), kobject);
}

__header_always_inline void ipc_kobject_require(ipc_port_t port,
                                                ipc_kobject_t kobject,
                                                ipc_kobject_type_t kotype) {
  ipc_kobject_t cur;
 ...
 *		The port is a kobject of the proper type.
 */
ipc_kobject_t ipc_kobject_get_locked(ipc_port_t port, ipc_kobject_type_t type) {
 ...
 *		The port is a kobject of the proper type.
 */
ipc_kobject_t ipc_kobject_get_stable(ipc_port_t port, ipc_kobject_type_t type) {
  assert(ipc_policy(type)->pol_kobject_stable);
  return ipc_kobject_get_locked(port, type);
}

ipc_object_label_t ipc_kobject_label_alloc(ipc_object_type_t otype,
...
void ipc_kobject_label_free(ipc_object_label_t label) {
 ...
 *	Purpose:
 *		Allocate a kobject port in the kernel space of the specified
 * type.
 *
 ...
 */
ipc_port_t ipc_kobject_alloc_port(ipc_kobject_t kobject,
...
bool ipc_kobject_make_send_lazy_alloc_port(ipc_port_t *port_store,
                                           ipc_kobject_t kobject,
  ...
  if (!IP_VALID(port)) {
    port = ipc_kobject_alloc_port(kobject, type, IPC_KOBJECT_ALLOC_MAKE_SEND);
...
bool ipc_kobject_is_mscount_current_locked(ipc_port_t port,
                                           mach_port_mscount_t mscount) {
  return ip_active(port) && port->ip_srights == 0 &&
         port->ip_mscount == mscount;
}

bool ipc_kobject_is_mscount_current(ipc_port_t port,
                                    mach_port_mscount_t mscount) {
  bool is_last;
...
kern_return_t ipc_typed_port_copyin_send(ipc_space_t space,
  ...
  kr = ipc_object_copyin(space, name, MACH_MSG_TYPE_COPY_SEND,
                         ...
                         IPC_COPYIN_KERNEL_DESTINATION, NULL, portp);
  if (kr != KERN_SUCCESS) {
    *portp = IP_NULL;
    return kr;
  }

  if (kotype != IOT_ANY && IP_VALID(*portp) && ip_type(*portp) != kotype) {
...
ipc_port_t ipc_kobject_copy_send(ipc_port_t port, ipc_kobject_t kobject,
                                 ipc_kobject_type_t kotype) {
...
ipc_port_t ipc_kobject_make_send(ipc_port_t port, ipc_kobject_t kobject,
                                 ipc_kobject_type_t kotype) {
  ipc_port_t sright = port;
...
void ipc_typed_port_release_send(ipc_port_t port, ipc_kobject_type_t kotype) {
...
ipc_kobject_disable_internal(ipc_port_t port, ipc_kobject_label_t kolabel,
                             ipc_kobject_type_t type) {
 ...
 */
__abortlike static void __ipc_kobject_bad_type_panic(ipc_port_t port,
                                                     ipc_kobject_type_t type) {
  panic("port %p of type %d, expecting %d", port, ip_type(port), type);
}

__abortlike static void
__ipc_kobject_dealloc_bad_mscount_panic(ipc_port_t port, uint64_t mscount,
                                        ipc_kobject_type_t type) {
  panic("unexpected make-send count: %p[%d], %d, %lld", port, type,
        port->ip_mscount, mscount);
}

__abortlike static void
__ipc_kobject_dealloc_bad_srights_panic(ipc_port_t port,
                                        ipc_kobject_type_t type) {
  panic("unexpected send right count: %p[%d], %d", port, type,
        port->ip_srights);
}

ipc_kobject_t ipc_kobject_dealloc_port_and_unlock(ipc_port_t port,
                                                  uint64_t mscount,
                                                  ipc_kobject_type_t type) {
  ipc_kobject_t kobject = IKO_NULL;
 ...
 */
ipc_kobject_t ipc_kobject_dealloc_port(ipc_port_t port, uint64_t mscount,
                                       ipc_kobject_type_t type) {
  ip_mq_lock(port);
 ...
 */
void ipc_kobject_enable(ipc_port_t port, ipc_kobject_t kobject,
                        ipc_kobject_type_t type) {
  assert(!ipc_policy(type)->pol_kobject_stable);
 ...
 */
ipc_kobject_t ipc_kobject_disable_locked(ipc_port_t port,
                                         ipc_kobject_type_t type) {
  ipc_object_label_t label;
  ipc_kobject_t kobject;
 ...
 *		Returns the current kobject pointer.
 */
ipc_kobject_t ipc_kobject_disable(ipc_port_t port, ipc_kobject_type_t type) {
  ipc_kobject_t kobject;
 ...
 *		Port is locked.
 */
void ipc_kobject_notify_send_once_and_unlock(ipc_port_t port) {
  /*
 ...
 */
static ipc_port_t ipc_kobject_label_substitute_task_read(
    ipc_space_t space, ipc_kobject_label_t kolabel, ipc_port_t port) {
 ...
 *		- a valid port if a substitution needs to happen
 */
static ipc_port_t ipc_kobject_label_substitute_thread_read(
    ipc_space_t space, ipc_kobject_label_t kolabel, ipc_port_t port) {
  ipc_port_t subst = IP_NULL;
 ...
 */
bool ipc_kobject_label_check_or_substitute(ipc_space_t space, ipc_port_t port,
                                           ipc_object_label_t *label,
  ...
  *subst_portp = IP_NULL;
  ...
  if ((label_tag & IPC_LABEL_SUBST_MASK)) {
    ...
    case IPC_LABEL_SUBST_TASK_READ:
      subst = ipc_kobject_label_substitute_task_read(space, kolabel, port);
      break;
    case IPC_LABEL_SUBST_THREAD_READ:
      subst = ipc_kobject_label_substitute_thread_read(space, kolabel, port);
      break;
    default:
