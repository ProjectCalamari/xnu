...
#define EXCLAVES_PANIC_STRING_SIZE                                             \
  (sizeof(EXCLAVE_PANIC_MESSAGE_MARKER) + XNUPROXY_PANIC_MESSAGE_LEN +         \
   XNUPROXY_PANIC_NAME_BYTES + 64)

#define P2ROUNDUP(x, align) (-(-(x) & -(align)))
#define PANIC_BUFFER_PAGE_COUNT                                                \
  (P2ROUNDUP(sizeof(xnuproxy_panic_buffer_t), PAGE_SIZE) / PAGE_SIZE)
...
static void copy_panic_buffer_pages(pmap_paddr_t addr) {
...
static void exclaves_xnuproxy_panic_thread(void *arg __unused,
  ...
  if (endpoint == EXCLAVES_INVALID_ID) {
    exclaves_debug_printf(show_errors, "exclaves: panic thread init: "
...
static bool exclaves_panic_buffer_sync(void) {
...
static void exclaves_append_panic_backtrace(void) {
  ...
  for (size_t i = 0; i < exclaves_panic_buffer.panicked_thread.backtrace.frames;
       i++) {
    uuid_unparse_upper((const unsigned char *)exclaves_panic_buffer.backtrace
                           .images[words[i].image]
                           .uuid,
                       uuid_string);
...
exclaves_append_panic_addl_info(xnuproxy_panicked_thread_t *ex_thread) {
  ...
  paniclog_append_noflush(
      ...
      "." EXCLAVES_PANIC_FOUR_CC_FORMAT "." EXCLAVES_PANIC_FOUR_CC_FORMAT
...
kern_return_t exclaves_panic_get_string(char **string) {
  uint32_t status = 0;
  char component_name[XNUPROXY_PANIC_NAME_BYTES] = {0};
...
void exclaves_panic_append_info(void) {
  uint32_t status = 0;
  kern_return_t kr = 0;
  char *status_str;
...
__attribute__((noinline, noreturn)) void exclaves_panic_thread_wait(void) {
...
void handle_response_panic_buffer_address(pmap_paddr_t addr) {
  return copy_panic_buffer_pages(addr);
}

kern_return_t exclaves_panic_thread_setup(void) {
