...
typedef struct {
  union {
    ...
    struct {
      _Atomic uint64_t sent_offset, latest_offset;
    } a_u64;
    ...
    struct {
      uint64_t sent_offset, latest_offset;
    } u64;
...
static kern_return_t exclaves_endpoint_call_internal(ipc_port_t port,
                                                     exclaves_id_t endpoint_id);

static kern_return_t exclaves_enter(void);
static kern_return_t exclaves_bootinfo(uint64_t *out_boot_info,
                                       bool *early_enter);

static kern_return_t exclaves_scheduler_init(uint64_t boot_info,
                                             uint64_t *xnuproxy_boot_info);
OS_NORETURN OS_NOINLINE static void exclaves_wait_for_panic(void);

static inline bool exclaves_clocks_need_update(void);

static kern_return_t exclaves_scheduler_boot(void);

static kern_return_t exclaves_hosted_error(bool success,
                                           XrtHosted_Error_t *error);

static kern_return_t
exclaves_scheduler_request_update_timer(XrtHosted_Timer_t timer,
                                        uint64_t offset);

static kern_return_t exclaves_scheduler_request_boot(void);
...
TUNABLE_DT_WRITEABLE(exclaves_requirement_t, exclaves_relaxed_requirements,
                     "/defaults", "kern.exclaves_relaxed_reqs",
                     "exclaves_relaxed_requirements", 0, TUNABLE_DT_NONE);
#else
...
#if CONFIG_EXCLAVES
static kern_return_t operation_boot(mach_port_name_t name,
                                    exclaves_boot_stage_t stage) {
...
kern_return_t _exclaves_ctl_trap(struct exclaves_ctl_trap_args *uap) {
#if CONFIG_EXCLAVES
  ...
  case EXCLAVES_CTL_OP_ENDPOINT_CALL: {
     ...
     * domain.
     */
    if (!exclaves_conclave_has_service(task_get_conclave(task), identifier)) {
      return KERN_INVALID_ARGUMENT;
    }
  ...
  case EXCLAVES_CTL_OP_NAMED_BUFFER_CREATE: {
    ...
    if (copyinstr(identifier, id_name, EXCLAVES_RESOURCE_NAME_MAX, &len) != 0 ||
        id_name[0] == '\0') {
      return KERN_INVALID_ARGUMENT;
    }
    ...
    kr = exclaves_resource_shared_memory_map(domain, id_name, usize, perm,
                                             &resource);
    if (kr != KERN_SUCCESS) {
      return kr;
    }

    kr = exclaves_resource_create_port_name(resource, current_space(), &name);
    if (kr != KERN_SUCCESS) {
      return kr;
    }
  ...
  case EXCLAVES_CTL_OP_NAMED_BUFFER_COPYIN: {
    ...
    kr = exclaves_resource_shared_memory_copyin(resource, ubuffer, usize,
                                                uoffset, usize2, uoffset2);
  ...
  case EXCLAVES_CTL_OP_NAMED_BUFFER_COPYOUT: {
    ...
    kr = exclaves_resource_shared_memory_copyout(resource, ubuffer, usize,
                                                 uoffset, usize2, uoffset2);
  ...
  case EXCLAVES_CTL_OP_LOOKUP_SERVICES: {
    ...
    if (usize > (MAX_CONCLAVE_RESOURCE_NUM *
                 sizeof(struct exclaves_resource_user)) ||
    ...
    if (exclaves_requirement_is_relaxed(EXCLAVES_R_CONCLAVE_RESOURCES) ||
      ...
      if (id == EXCLAVES_INVALID_ID) {
        id = exclaves_service_lookup(EXCLAVES_DOMAIN_DARWIN, uresource.r_name);
      }
      if (id == EXCLAVES_INVALID_ID) {
        id = exclaves_service_lookup(EXCLAVES_DOMAIN_KERNEL, uresource.r_name);
      }
    }
  ...
  case EXCLAVES_CTL_OP_AUDIO_BUFFER_CREATE: {
    ...
    char id_name[EXCLAVES_RESOURCE_NAME_MAX] = "";
    size_t done = 0;
    if (copyinstr(identifier, id_name, EXCLAVES_RESOURCE_NAME_MAX, &done) !=
        0) {
      return KERN_INVALID_ARGUMENT;
    }
    ...
    const char *domain = exclaves_conclave_get_domain(task_get_conclave(task));
    exclaves_resource_t *resource = NULL;
    kr = exclaves_resource_audio_memory_map(domain, id_name, usize, &resource);
    if (kr != KERN_SUCCESS) {
      return kr;
    }
  ...
  case EXCLAVES_CTL_OP_AUDIO_BUFFER_COPYOUT: {
    ...
    if (resource->r_type != XNUPROXY_RESOURCETYPE_ARBITRATEDAUDIOMEMORY) {
      exclaves_resource_release(resource);
      return KERN_INVALID_CAPABILITY;
    }

    kr = exclaves_resource_audio_memory_copyout(
        resource, ubuffer, usize, uoffset, usize2, uoffset2, ustatus);
  ...
  case EXCLAVES_CTL_OP_NOTIFICATION_RESOURCE_LOOKUP: {
    ...
    notification_resource_user = (struct exclaves_resource_user *)kalloc_data(
        usize, Z_WAITOK | Z_ZERO | Z_NOFAIL);
    ...
    const char *domain = exclaves_conclave_get_domain(task_get_conclave(task));
    kr = exclaves_notification_create(
        domain, notification_resource_user->r_name, &notification_resource);
    if (kr != KERN_SUCCESS) {
      goto notification_resource_lookup_out;
    }
...
kern_return_t exclaves_endpoint_call(ipc_port_t port, exclaves_id_t endpoint_id,
  ...
  exclaves_debug_printf(
      show_progress, "exclaves: endpoint call:\tendpoint id %lld tag 0x%llx\n",
      endpoint_id, *tag);
...
kern_return_t exclaves_allocate_ipc_buffer(void **out_ipc_buffer) {
#if CONFIG_EXCLAVES
  kern_return_t kr = KERN_SUCCESS;
...
kern_return_t exclaves_thread_terminate(__unused thread_t thread) {
  kern_return_t kr = KERN_SUCCESS;
  ...
  if (thread->th_exclaves_ipc_ctx.ipcb != NULL) {
    exclaves_debug_printf(
        show_progress, "exclaves: thread_terminate freeing abandoned exclaves "
                       "ipc buffer\n");
...
static void bind_to_boot_core(void) {
  /*
...
static void unbind_from_boot_core(void) {
...
kern_return_t exclaves_boot_early(void) {
  ...
  if (kr != KERN_SUCCESS) {
    exclaves_debug_printf(show_errors, "exclaves: Get bootinfo failed\n");
    return kr;
  }
  ...
  if (kr != KERN_SUCCESS) {
    exclaves_debug_printf(show_errors, "exclaves: Init scheduler failed\n");
    return kr;
  }

  kr = exclaves_xnuproxy_init(xnuproxy_boot_info);
  if (kr != KERN_SUCCESS) {
    exclaves_debug_printf(show_errors, "XNU proxy setup failed\n");
    return KERN_FAILURE;
  }
  ...
  if (kr != KERN_SUCCESS) {
    exclaves_debug_printf(show_errors, "XNU proxy panic thread setup failed\n");
    return KERN_FAILURE;
  }
...
void exclaves_update_timebase(exclaves_clock_type_t type, uint64_t offset) {
  assert(type == EXCLAVES_CLOCK_CONTINUOUS || type == EXCLAVES_CLOCK_ABSOLUTE);
#if CONFIG_EXCLAVES
  ...
  while (latest_offset != offset) {
    ...
    if (os_atomic_cmpxchgv(&clock->a_u64.latest_offset, latest_offset, offset,
                           &latest_offset, relaxed)) {
      break;
    }
  }
#else
...
static bool exclaves_save_matrix_state(void) {
  bool saved = false;
#if HAS_ARM_FEAT_SME
  /* Save only the ZA/ZT0 state. SPTM will save/restore TPIDR2. */
  if (arm_sme_version() > 0 && !!(__builtin_arm_rsr64("SVCR") & SVCR_ZA)) {
    arm_sme_saved_state_t *sme_state =
        machine_thread_get_sme_state(current_thread());
...
static void exclaves_restore_matrix_state(bool did_save_sme __unused) {
#if HAS_ARM_FEAT_SME
  if (did_save_sme) {
    arm_sme_saved_state_t *sme_state =
        machine_thread_get_sme_state(current_thread());
...
enum { RINGGATE_EP_ENTER, RINGGATE_EP_INFO };
...
static kern_return_t exclaves_enter(void) {
  ...
  __assert_only const thread_exclaves_state_flags_t mask =
      (TH_EXCLAVES_RPC | TH_EXCLAVES_XNUPROXY | TH_EXCLAVES_SCHEDULER_CALL |
   ...
   * entered from xnu.
   */
  __asm__ volatile("EXCLAVES_ENTRY_START:\n\t");
  result = sk_enter(endpoint, &regs);
  __asm__ volatile("EXCLAVES_ENTRY_END:\n\t");
...
#if CONFIG_SPTM
  /**
   * SPTM will return here with debug exceptions disabled (MDSCR_{KDE,MDE} ==
   * {0,0}) but SK might have clobbered individual breakpoints, etc. Invalidate
   * the current CPU debug state forcing a reload on the next return to user
   * mode.
   */
...
static void exclaves_init_multicore(void) {
...
static kern_return_t exclaves_scheduler_init(uint64_t boot_info,
  ...
  if (!pmap_valid_address(boot_info)) {
    exclaves_debug_printf(show_errors, "exclaves: %s: 0x%012llx\n",
                          "Invalid root physical address", boot_info);
    return KERN_FAILURE;
  }

  if (exclaves_callbacks == NULL) {
    exclaves_debug_printf(show_errors, "exclaves: Callbacks not registered\n");
    return KERN_FAILURE;
  }

  /* Initialise XrtHostedXnu kext */
  kr = exclaves_hosted_error(
      exclaves_callbacks->v1.init(XrtHosted_Version_current,
                                  phystokv(boot_info), &hosted_error),
      &hosted_error);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
  ...
  XrtHosted_Mapped_t **pages = zalloc_permanent(
      frames * sizeof(XrtHosted_Mapped_t *), ZALIGN(XrtHosted_Mapped_t *));
  size_t index = 0;
  uint64_t phys = boot_info;
  while (index < frames) {
    if (!pmap_valid_address(phys)) {
      exclaves_debug_printf(show_errors, "exclaves: %s: 0x%012llx\n",
                            "Invalid shared physical address", phys);
      return KERN_FAILURE;
    }
    pages[index] = (XrtHosted_Mapped_t *)phystokv(phys);
    kr = exclaves_hosted_error(exclaves_callbacks->v1.nextPhys(
                                   pages[index], &index, &phys, &hosted_error),
                               &hosted_error);
    if (kr != KERN_SUCCESS) {
      return kr;
    }
  ...
  exclaves_callbacks->v1.setMapping(XrtHosted_Region_scattered(frames, pages));
  ...
  if (global->v2.smpStatus != XrtHosted_SmpStatus_MulticoreMpidr) {
    exclaves_debug_printf(
        show_errors, "exclaves: exclaves scheduler doesn't support multicore");
    return KERN_FAILURE;
  }
  ...
  if (!pmap_valid_address(global->v1.proxyInit)) {
    exclaves_debug_printf(show_errors, "exclaves: %s: 0x%012llx\n",
                          "Invalid xnu prpoxy physical address", phys);
    return KERN_FAILURE;
  }
    ...
    if (exclaves_debug_enabled(show_scheduler_request_response)) {             \
      ...
      for (size_t arg = 0; arg < XrtHosted_Buffer_args; arg += 1) {            \
        printf("exclaves: Scheduler request.arguments[%02zu] = "               \
               "0x%04llx\n",                                                   \
               arg, _request_buf##_copy.arguments[arg]);                       \
      }                                                                        \
      ...
      for (size_t arg = 0; arg < XrtHosted_Buffer_args; arg += 1) {            \
        printf("exclaves: Scheduler response.arguments[%02zu] = "              \
               "0x%04llx\n",                                                   \
               arg, _response_buf##_copy.arguments[arg]);                      \
      }                                                                        \
    }                                                                          \
  })
...
static void request_trace_start(const XrtHosted_Request_t *request) {
  switch (request->tag) {
  case XrtHosted_Request_ResumeWithHostId:
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                              MACH_EXCLAVES_SCHEDULER_REQ_RESUME_WITH_HOSTID) |
                     DBG_FUNC_START,
                 request->ResumeWithHostId.hostId,
                 request->ResumeWithHostId.thread);
    break;

  case XrtHosted_Request_InterruptWithHostId:
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                     MACH_EXCLAVES_SCHEDULER_REQ_INTERRUPT_WITH_HOSTID) |
            DBG_FUNC_START,
        request->InterruptWithHostId.hostId,
        request->InterruptWithHostId.thread);
    break;

  case XrtHosted_Request_UpdateTimerOffset:
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                              MACH_EXCLAVES_SCHEDULER_REQ_UPDATE_TIMER_OFFSET) |
                     DBG_FUNC_START,
                 request->UpdateTimerOffset.timer,
                 request->UpdateTimerOffset.offset);
    break;

  case XrtHosted_Request_BootExclaves:
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                              MACH_EXCLAVES_SCHEDULER_REQ_BOOT_EXCLAVES) |
                 DBG_FUNC_START);
    break;
  ...
  case XrtHosted_Request_PmmEarlyAllocResponse:
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                     MACH_EXCLAVES_SCHEDULER_REQ_PMM_EARLY_ALLOC_RESPONSE) |
            DBG_FUNC_START,
  ...
  case XrtHosted_Request_WatchdogPanic:
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                              MACH_EXCLAVES_SCHEDULER_REQ_WATCHDOG_PANIC) |
                 DBG_FUNC_START);
    break;
...
static void request_trace_end(const XrtHosted_Request_t *request) {
  switch (request->tag) {
  case XrtHosted_Request_ResumeWithHostId:
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                              MACH_EXCLAVES_SCHEDULER_REQ_RESUME_WITH_HOSTID) |
...
__attribute__((always_inline)) static kern_return_t
...
handle_response_yield(bool early, __assert_only Exclaves_L4_Word_t scid,
  ...
  KDBG_RELEASE(
      MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER, MACH_EXCLAVES_SCHEDULER_YIELD),
      yielded_to_scid, early);
...
handle_response_spawned(__assert_only Exclaves_L4_Word_t scid,
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                            MACH_EXCLAVES_SCHEDULER_SPAWNED),
               spawned->spawned);
...
handle_response_terminated(const XrtHosted_Terminated_t *terminated) {
  ...
  exclaves_debug_printf(show_errors,
                        ...
                        responding_scid, terminated->terminated);
...
static kern_return_t handle_response_wait(const XrtHosted_Wait_t *wait) {
  ...
  exclaves_debug_printf(
      ...
      "epoch 0x%llx\n",
      responding_scid, wait->owner, wait->queueId, wait->epoch);
  ...
  default:
    panic("Unknown exclaves interruptibility: %llu", wait->interruptible);
  }

  KDBG_RELEASE(
      MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER, MACH_EXCLAVES_SCHEDULER_WAIT) |
          DBG_FUNC_START,
      id, epoch, owner, wait->interruptible);
  const wait_result_t wr =
      esync_wait(ESYNC_SPACE_EXCLAVES_Q, id, epoch,
...
static kern_return_t handle_response_wake(const XrtHosted_Wake_t *wake) {
  ...
  exclaves_debug_printf(show_progress,
                        ...
                        responding_scid, wake->queueId, wake->epoch, wake->all);
  assert3u(wake->wakerHostId, ==, ctid);

  const XrtHosted_Word_t id = wake->queueId;
  const uint64_t epoch = wake->epoch;
  const esync_wake_mode_t mode =
      wake->all != 0 ? ESYNC_WAKE_ALL : ESYNC_WAKE_ONE;

  KDBG_RELEASE(
      MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER, MACH_EXCLAVES_SCHEDULER_WAKE) |
          DBG_FUNC_START,
      id, epoch, 0, mode);
...
handle_response_wake_with_owner(const XrtHosted_WakeWithOwner_t *wake) {
  ...
  exclaves_debug_printf(show_progress,
                        ...
                        responding_scid, wake->queueId, wake->epoch,
                        wake->owner);
  ...
  KDBG_RELEASE(
      MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER, MACH_EXCLAVES_SCHEDULER_WAKE) |
          DBG_FUNC_START,
      id, epoch, owner, ESYNC_WAKE_ONE_WITH_OWNER);

  kern_return_t kr = esync_wake(ESYNC_SPACE_EXCLAVES_Q, id, epoch,
                                exclaves_get_queue_counter(id),
                                ESYNC_WAKE_ONE_WITH_OWNER, owner);
...
handle_response_suspended(const XrtHosted_Suspended_t *suspended) {
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                            MACH_EXCLAVES_SCHEDULER_SUSPENDED) |
                   DBG_FUNC_START,
               id, epoch);

  const wait_result_t wr = esync_wait(ESYNC_SPACE_EXCLAVES_T, id, epoch,
                                      exclaves_get_thread_counter(id), 0,
                                      ESYNC_POLICY_KERNEL, THREAD_UNINT);
...
handle_response_resumed(const XrtHosted_Resumed_t *resumed) {
  ...
  exclaves_debug_printf(
      ...
      responding_scid, resumed->resumed, resumed->resumedHostId,
      resumed->epoch);
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                            MACH_EXCLAVES_SCHEDULER_RESUMED) |
                   DBG_FUNC_START,
               id, epoch, target);

  kern_return_t kr =
      esync_wake(ESYNC_SPACE_EXCLAVES_T, id, epoch,
...
handle_response_interrupted(const XrtHosted_Interrupted_t *interrupted) {
  ...
  exclaves_debug_printf(show_progress,
                        ...
                        responding_scid, interrupted->queueId,
                        interrupted->epoch, interrupted->interruptedHostId);
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                            MACH_EXCLAVES_SCHEDULER_INTERRUPTED) |
                   DBG_FUNC_START,
               id, epoch, target);

  kern_return_t kr =
      esync_wake(ESYNC_SPACE_EXCLAVES_Q, id, epoch,
...
static kern_return_t handle_response_nothing_scheduled(
...
static kern_return_t handle_response_all_exclaves_booted(
 ...
 */
static kern_return_t handle_response_pmm_early_alloc(
    const XrtHosted_PmmEarlyAlloc_t *pmm_early_alloc, uint64_t *pagelist_pa) {
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
                            MACH_EXCLAVES_SCHEDULER_EARLY_ALLOC),
               npages, flags);
...
static void handle_response_watchdog_panic_complete(
...
OS_NORETURN
static void
handle_response_panicking(__unused const XrtHosted_Panicking_t *panicking) {
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES_SCHEDULER,
...
static inline bool exclaves_clocks_need_update(void) {
  const exclaves_clock_type_t clocks[] = {EXCLAVES_CLOCK_ABSOLUTE,
                                          EXCLAVES_CLOCK_CONTINUOUS};
...
OS_NOINLINE
static kern_return_t exclaves_clocks_update(void) {
  const exclaves_clock_type_t clocks[] = {EXCLAVES_CLOCK_ABSOLUTE,
                                          EXCLAVES_CLOCK_CONTINUOUS};
  ...
  for (int i = 0; i < ARRAY_COUNT(clocks); i++) {
    ...
    while (local.u64.sent_offset != local.u64.latest_offset) {
      ...
      kern_return_t kr = exclaves_scheduler_request_update_timer(
          timer, local.u64.latest_offset);
      if (kr != KERN_SUCCESS) {
        return kr;
      }
      ...
      os_atomic_cmpxchgv(&clock->a_u64.sent_offset, local.u64.sent_offset,
                         local.u64.latest_offset, &local.u64.sent_offset,
                         relaxed);
      ...
      local.u64.latest_offset =
          os_atomic_load(&clock->a_u64.latest_offset, relaxed);
    }
  }
...
exclaves_scheduler_request_update_timer(XrtHosted_Timer_t timer,
  ...
  const XrtHosted_Request_t request =
      XrtHosted_Request_UpdateTimerOffsetMsg(.timer = timer,
                                             .offset = offset, );
  ...
  default:
    exclaves_debug_printf(show_errors, "exclaves: scheduler request failed\n");
    return kr;
  }
...
static kern_return_t exclaves_scheduler_request_boot(void) {
  ...
  while (response.tag != XrtHosted_Response_AllExclavesBooted) {
    ...
    case XrtHosted_Response_PmmEarlyAlloc:
      kr = handle_response_pmm_early_alloc(&response.PmmEarlyAlloc,
                                           &pagelist_pa);
      break;

    case XrtHosted_Response_PanicBufferAddress:
      handle_response_panic_buffer_address(
          response.PanicBufferAddress.physical);
      break;
    ...
    default:
      exclaves_debug_printf(
          show_errors, "exclaves: Scheduler: Unexpected response: tag 0x%x\n",
          response.tag);
      kr = KERN_FAILURE;
      break;
    }
...
OS_INLINE
kern_return_t exclaves_scheduler_request_resume(const exclaves_ctx_t *ctx,
                                                bool interrupted) {
  thread_t thread = current_thread();
  const ctid_t ctid = thread_get_ctid(thread);

  assert3u(thread->th_exclaves_state &
               (TH_EXCLAVES_RESUME_PANIC_THREAD | TH_EXCLAVES_RPC),
           !=, 0);
  ...
  const XrtHosted_Request_t request =
      interrupted
          ? XrtHosted_Request_InterruptWithHostIdMsg(.thread = ctx->scid,
                                                     .hostId = ctid, )
          : XrtHosted_Request_ResumeWithHostIdMsg(.thread = ctx->scid,
                                                  .hostId = ctid, );
...
OS_NOINLINE
kern_return_t exclaves_run(thread_t thread, bool interrupted) {
 ...
 */
kern_return_t exclaves_scheduler_request_watchdog_panic(void) {
  thread_t thread = current_thread();
  ...
  default:
    panic("exclaves: unexpected scheduler response "
          "when sending watchdog panic request: %d",
          response.tag);
  }
...
static kern_return_t exclaves_hosted_error(bool success,
  ...
  if (success) {
    ...
    exclaves_debug_printf(show_errors, "exclaves: XrtHosted: %s[%d] (%s): %s\n",
 ...
 */
TUNABLE(unsigned int, exclaves_entitlement_flags, "exclaves_entitlement_flags",
        DEFAULT_ENTITLEMENT_FLAGS);

static bool has_entitlement(task_t task, const exclaves_priv_t priv,
...
static bool has_entitlement_vnode(void *vnode, const int64_t off,
                                  const exclaves_priv_t priv,
                                  const char *entitlement) {
  /* Skip the entitlement if not enabled. */
  if ((exclaves_entitlement_flags & priv) == 0) {
    return true;
...
bool exclaves_has_priv(task_t task, exclaves_priv_t priv) {
  ...
  case EXCLAVES_PRIV_BOOT:
    ...
    return has_entitlement(task, priv, "com.apple.private.exclaves.boot");
...
bool exclaves_has_priv_vnode(void *vnode, int64_t off, exclaves_priv_t priv) {
  switch (priv) {
  case EXCLAVES_PRIV_CONCLAVE_HOST: {
    const bool has_conclave_host = has_entitlement_vnode(
        vnode, off, priv, "com.apple.private.exclaves.conclave-host");
    ...
    const bool has_domain_kernel =
        has_entitlement_vnode(vnode, off, EXCLAVES_PRIV_KERNEL_DOMAIN,
                              "com.apple.private.exclaves.kernel-domain");
...
/* Unslid pointers defining the range of code which handles exclaves scheduler
 * request */
uintptr_t exclaves_scheduler_request_range_start;
uintptr_t exclaves_scheduler_request_range_end;

__startup_func static void initialize_exclaves_ranges(void) {
  ...
  exclaves_scheduler_request_range_start =
      VM_KERNEL_UNSLIDE(&exclaves_scheduler_request_start_label);
  assert3u(exclaves_scheduler_request_range_start, !=, 0);
  exclaves_scheduler_request_range_end =
      VM_KERNEL_UNSLIDE(&exclaves_scheduler_request_end_label);
 ...
 */
static bool exclaves_enter_in_range(uintptr_t addr, bool slid) {
  return slid ? exclaves_in_range(addr, (uintptr_t)&exclaves_enter_start_label,
                                  (uintptr_t)&exclaves_enter_end_label)
              : exclaves_in_range(addr, exclaves_enter_range_start,
                                  exclaves_enter_range_end);
}

/*
 * Return true if the specified address is in scheduler request handlers.
 */
static bool exclaves_scheduler_request_in_range(uintptr_t addr, bool slid) {
  return slid ? exclaves_in_range(
                    addr, (uintptr_t)&exclaves_scheduler_request_start_label,
                    (uintptr_t)&exclaves_scheduler_request_end_label)
              : exclaves_in_range(addr, exclaves_scheduler_request_range_start,
                                  exclaves_scheduler_request_range_end);
}

uint32_t exclaves_stack_offset(const uintptr_t *addr, size_t nframes,
                               bool slid) {
  size_t i = 0;
...
__startup_func static void exclaves_requirement_startup(void) {
  /*
