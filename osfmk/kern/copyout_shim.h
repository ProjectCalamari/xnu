...
typedef void (*copyout_shim_fn_t)(const void *, user_addr_t, vm_size_t,
                                  unsigned co_src);
...
// void call_copyout_shim(const void *kernel_addr,user_addr_t
// user_addr,vm_size_t nbytes,int copy_type,int copyout_flavors);

#define CALL_COPYOUT_SHIM_NRML(ka, ua, nb)                                     \
  if (copyout_shim_fn && (co_src_flags & CO_SRC_NORMAL)) {                     \
    copyout_shim_fn(ka, ua, nb, CO_SRC_NORMAL);                                \
  }

#define CALL_COPYOUT_SHIM_MSG(ka, ua, nb)                                      \
  if (copyout_shim_fn && (co_src_flags & CO_SRC_MSG)) {                        \
    copyout_shim_fn(ka, ua, nb, CO_SRC_MSG);                                   \
  }

#define CALL_COPYOUT_SHIM_PHYS(ka, ua, nb)                                     \
  if (copyout_shim_fn && (co_src_flags & CO_SRC_PHYS)) {                       \
    copyout_shim_fn(ka, ua, nb, CO_SRC_PHYS);                                  \
  }

#else
// Make these calls disappear if we're RELEASE or if COPYOUT_SHIM didn't get
// built
#define CALL_COPYOUT_SHIM_NRML(ka, ua, nb)
#define CALL_COPYOUT_SHIM_MSG(ka, ua, nb)
#define CALL_COPYOUT_SHIM_PHYS(ka, ua, nb)
...
kern_return_t register_copyout_shim(copyout_shim_fn_t copyout_shim_fn,
                                    unsigned co_src_flags);

#define unregister_copyout_shim() register_copyout_shim(NULL, 0)

void *cos_kernel_unslide(const void *);

void *cos_kernel_reslide(const void *);
