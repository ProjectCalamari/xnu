 ...
 *               uint32_t baz;
 *               uint32_t quux;
 *               ///////// This is where the original structure's layout ended!
 * sizeof(struct foobar) was 8 //////// uint32_t frozzle; } __attribute__
 * ((packed));
 *
 *   If you are parsing kcdata formats, you MUST
 *
 *     * Check the length field of each struct, including array elements.   If
 * the struct is longer than you expect, you must ignore the extra data.
 *
 ...
 *
 *
 * The type field describes what kind of data is passed. For example type =
 * TASK_CRASHINFO_UUID means the following data is a uuid. These types need to
 * be defined in task_corpse.h for easy consumption by userspace inspection
 * tools.
 *
 * Some range of types is reserved for special types like ints, longs etc. A
 * cool new functionality made possible with this extensible data format is that
 * kernel can decide to put more information as required without requiring user
 * space tools to re-compile to be compatible. The case of rusage struct
 * versions could be introduced without breaking existing tools.
 *
 * Feature description: Generic data with description
 * -------------------
 * Further more generic data with description is very much possible now. For
 * example
 *
 *   - kcdata_add_uint64_with_description(cdatainfo, 0x700, "NUM MACH PORTS");
 *   - and more functions that allow adding description.
 * The userspace tools can then look at the description and print the data even
 * if they are not compiled with knowledge of the field apriori.
 *
 *  Example data:
 ...
 * ------------------
 * If a given kernel data type is complex and requires adding multiple optional
 * fields inside a container object for a consumer to understand arbitrary data,
 * we package it using container markers.
 *
 * For example, the stackshot code gathers information and describes the state
 * of a given task with respect to many subsystems. It includes data such as io
 * stats, vm counters, process names/flags and syscall counts.
 *
 * kcdata_add_container_marker(kcdata_p, KCDATA_TYPE_CONTAINER_BEGIN,
 * STACKSHOT_KCCONTAINER_TASK, task_uniqueid);
 * // add multiple data, or add_<type>_with_description()s here
 *
 * kcdata_add_container_marker(kcdata_p, KCDATA_TYPE_CONTAINER_END,
 * STACKSHOT_KCCONTAINER_TASK, task_uniqueid);
 *
 * Feature description: Custom Data formats on demand
 * --------------------
 * With the self describing nature of format, the kernel provider can describe a
 * data type (uniquely identified by a number) and use it in the buffer for
 * sending data. The consumer can parse the type information and have knowledge
 * of describing incoming data. Following is an example of how we can describe a
 * kernel specific struct sample_disk_io_stats in buffer.
 *
 ...
 *     {KCS_SUBTYPE_FLAGS_NONE, KC_ST_UINT64, 0 * sizeof(uint64_t),
 * sizeof(uint64_t), "disk_reads_count"}, {KCS_SUBTYPE_FLAGS_NONE, KC_ST_UINT64,
 * 1 * sizeof(uint64_t), sizeof(uint64_t), "disk_reads_size"},
 *     {KCS_SUBTYPE_FLAGS_ARRAY, KC_ST_UINT64, 2 * sizeof(uint64_t),
 * KCS_SUBTYPE_PACK_SIZE(4, sizeof(uint64_t)), "io_priority_count"},
 *     {KCS_SUBTYPE_FLAGS_ARRAY, KC_ST_UINT64, (2 + 4) * sizeof(uint64_t),
 * sizeof(uint64_t), "io_priority_size"},
 * };
 *
 * Now you can add this custom type definition into the buffer as
 * kcdata_add_type_definition(kcdata_p, KCTYPE_SAMPLE_DISK_IO_STATS,
 * "sample_disk_io_stats", &disk_io_stats_def[0],
 * sizeof(disk_io_stats_def)/sizeof(struct kcdata_subtype_descriptor));
 *
 * Feature description: Compression
 * --------------------
 * In order to avoid keeping large amounts of memory reserved for a panic
 * stackshot, kcdata has support for compressing the buffer in a streaming
 * fashion. New data pushed to the kcdata buffer will be automatically
 * compressed using an algorithm selected by the API user (currently, we only
 * support pass-through and zlib, in the future we plan to add WKDM support,
 * see: 57913859).
 *
 ...
 *   `hdr_tag` is the usual header tag denoting what type of kcdata buffer this
 * will be `memcpy_f` a memcpy(3) function to use to copy into the buffer,
 * optional. `compy_type` is the compression type, see KCDCT_ZLIB for an
 * example.
 *
 ...
 *    (None of the following will compress unless kcdata_init_compress() has
 * been called)
 *
 * - kcdata_push_data(kcdata_descriptor_t data, uint32_t type, uint32_t size,
 * const void *input_data) Pushes the buffer of kctype @type at[@input_data,
 * @input_data + @size] into the kcdata buffer @data, compressing if needed.
 *
 ...
 *   Pushes the array found at @input_data, with element type @type_of_element,
 * where each element is of size @size_of_element and there are @count elements
 * into the kcdata buffer at @data.
 *
 * - kcdata_compression_window_open/close(kcdata_descriptor_t data)
 *   In case the data you are trying to push to the kcdata buffer @data is
 * difficult to predict, you can open a "compression window". Between an open
 * and a close, no compression will be done. Once you close the window, the
 * underlying compression algorithm will compress the data into the buffer and
 * automatically rewind the current end marker of the kcdata buffer. There is an
 * ASCII art in kern_cdata.c to aid the reader in understanding this.
 *
 * - kcdata_finish_compression(kcdata_descriptor_t data)
 *   Must be called at the end to flush any underlying buffers used by the
 * compression algorithms. This function will also add some statistics about the
 * compression to the buffer which helps with decompressing later.
 *
 */
 ...
 */
#define KCS_SUBTYPE_FLAGS_STRUCT                                               \
  0x2 /* force struct type even if only one element */
#define KCS_SUBTYPE_FLAGS_MERGE                                                \
  0x4 /* treat as multiple elements of parents instead of struct */
 ...
 */
#define KCS_SUBTYPE_PACK_SIZE(e_count, e_size)                                 \
  (((e_count) & 0xffffu) << 16 | ((e_size) & 0xffffu))
...
static inline uint32_t kcs_get_elem_size(kcdata_subtype_descriptor_t d) {
  if (d->kcs_flags & KCS_SUBTYPE_FLAGS_ARRAY) {
    ...
    return (uint32_t)((d->kcs_elem_size & 0xffff) *
                      ((d->kcs_elem_size & 0xffff0000) >> 16));
  }
...
static inline int kcs_set_elem_size(kcdata_subtype_descriptor_t d,
...
#define KCDATA_TYPE_TYPEDEFINTION                                              \
  0x12u /* Meta type that describes a type on the fly. */
...
#define KCDATA_BUFFER_BEGIN_CRASHINFO                                          \
  0xDEADF157u /* owner: corpses/task_corpse.h */
...
#define KCDATA_BUFFER_BEGIN_COMPRESSED 0x434f4d50u /* owner: sys/stackshot.h   \
                                                    */
/* type-range: 0x900 - 0x93f */
#define KCDATA_BUFFER_BEGIN_DELTA_STACKSHOT                                    \
...
#define KCDATA_BUFFER_BEGIN_XNUPOST_CONFIG                                     \
  0x1e21c09fu /* owner: osfmk/tests/kernel_tests.c */
...
#define STACKSHOT_KCTYPE_GLOBAL_MEM_STATS                                      \
  0x902u /* struct mem_and_io_snapshot */
#define STACKSHOT_KCCONTAINER_TASK 0x903u
#define STACKSHOT_KCCONTAINER_THREAD 0x904u
#define STACKSHOT_KCTYPE_TASK_SNAPSHOT                                         \
  0x905u /* task_snapshot_v2, task_snapshot_v3 */
#define STACKSHOT_KCTYPE_THREAD_SNAPSHOT                                       \
  0x906u /* thread_snapshot_v2, thread_snapshot_v3 */
#define STACKSHOT_KCTYPE_DONATING_PIDS 0x907u /* int[] */
#define STACKSHOT_KCTYPE_SHAREDCACHE_LOADINFO                                  \
  0x908u                                    /* dyld_shared_cache_loadinfo */
#define STACKSHOT_KCTYPE_THREAD_NAME 0x909u /* char[] */
#define STACKSHOT_KCTYPE_KERN_STACKFRAME                                       \
  0x90Au /* struct stack_snapshot_frame32 */
#define STACKSHOT_KCTYPE_KERN_STACKFRAME64                                     \
  0x90Bu /* struct stack_snapshot_frame64 */
#define STACKSHOT_KCTYPE_USER_STACKFRAME                                       \
  0x90Cu /* struct stack_snapshot_frame32 */
#define STACKSHOT_KCTYPE_USER_STACKFRAME64                                     \
  0x90Du                                 /* struct stack_snapshot_frame64 */
#define STACKSHOT_KCTYPE_BOOTARGS 0x90Eu /* boot args string */
#define STACKSHOT_KCTYPE_OSVERSION                                             \
  0x90Fu /* os version string, same as running uname -a */
#define STACKSHOT_KCTYPE_KERN_PAGE_SIZE                                        \
  0x910u                                     /* kernel page size in uint32_t */
#define STACKSHOT_KCTYPE_JETSAM_LEVEL 0x911u /* jetsam level in uint32_t */
#define STACKSHOT_KCTYPE_DELTA_SINCE_TIMESTAMP                                 \
  0x912u /* timestamp used for the delta stackshot */
...
#define STACKSHOT_KCTYPE_CPU_TIMES                                             \
  0x919u /* struct stackshot_cpu_times or stackshot_cpu_times_v2 */
#define STACKSHOT_KCTYPE_STACKSHOT_DURATION                                    \
  0x91au /* struct stackshot_duration */
#define STACKSHOT_KCTYPE_STACKSHOT_FAULT_STATS                                 \
  0x91bu /* struct stackshot_fault_stats */
#define STACKSHOT_KCTYPE_KERNELCACHE_LOADINFO                                  \
  0x91cu /* kernelcache UUID -- same as KCDATA_TYPE_LIBRARY_LOADINFO64 */
#define STACKSHOT_KCTYPE_THREAD_WAITINFO                                       \
  0x91du /* struct stackshot_thread_waitinfo */
#define STACKSHOT_KCTYPE_THREAD_GROUP_SNAPSHOT                                 \
  0x91eu /* struct thread_group_snapshot{,_v2,_v3} */
#define STACKSHOT_KCTYPE_THREAD_GROUP 0x91fu /* uint64_t */
#define STACKSHOT_KCTYPE_JETSAM_COALITION_SNAPSHOT                             \
  0x920u /* struct jetsam_coalition_snapshot */
#define STACKSHOT_KCTYPE_JETSAM_COALITION 0x921u /* uint64_t */
#define STACKSHOT_KCTYPE_THREAD_POLICY_VERSION                                 \
  0x922u /* THREAD_POLICY_INTERNAL_STRUCT_VERSION in uint32 */
#define STACKSHOT_KCTYPE_INSTRS_CYCLES                                         \
  0x923u /* struct instrs_cycles_snapshot_v2 */
#define STACKSHOT_KCTYPE_USER_STACKTOP                                         \
  0x924u                                    /* struct stack_snapshot_stacktop */
#define STACKSHOT_KCTYPE_ASID 0x925u        /* uint32_t */
#define STACKSHOT_KCTYPE_PAGE_TABLES 0x926u /* uint64_t */
#define STACKSHOT_KCTYPE_SYS_SHAREDCACHE_LAYOUT                                \
  0x927u /* same as KCDATA_TYPE_LIBRARY_LOADINFO64 */
#define STACKSHOT_KCTYPE_THREAD_DISPATCH_QUEUE_LABEL                           \
  0x928u /* dispatch queue label */
#define STACKSHOT_KCTYPE_THREAD_TURNSTILEINFO                                  \
  0x929u /* struct stackshot_thread_turnstileinfo */
#define STACKSHOT_KCTYPE_TASK_CPU_ARCHITECTURE                                 \
  0x92au /* struct stackshot_cpu_architecture */
#define STACKSHOT_KCTYPE_LATENCY_INFO                                          \
  0x92bu /* struct stackshot_latency_collection_v2 */
#define STACKSHOT_KCTYPE_LATENCY_INFO_TASK                                     \
  0x92cu /* struct stackshot_latency_task */
#define STACKSHOT_KCTYPE_LATENCY_INFO_THREAD                                   \
  0x92du /* struct stackshot_latency_thread */
#define STACKSHOT_KCTYPE_LOADINFO64_TEXT_EXEC                                  \
  0x92eu /* TEXT_EXEC load info -- same as KCDATA_TYPE_LIBRARY_LOADINFO64 */
#define STACKSHOT_KCTYPE_AOTCACHE_LOADINFO                                     \
  0x92fu /* struct dyld_aot_cache_uuid_info */
#define STACKSHOT_KCTYPE_TRANSITIONING_TASK_SNAPSHOT                           \
  0x930u /* transitioning_task_snapshot */
#define STACKSHOT_KCCONTAINER_TRANSITIONING_TASK 0x931u
#define STACKSHOT_KCTYPE_USER_ASYNC_START_INDEX                                \
  0x932u /* uint32_t index in user_stack of beginning of async stack */
#define STACKSHOT_KCTYPE_USER_ASYNC_STACKLR64                                  \
  0x933u /* uint64_t async stack pointers */
#define STACKSHOT_KCCONTAINER_PORTLABEL                                        \
  0x934u                                  /* container for port label info */
...
#define STACKSHOT_KCTYPE_DYLD_COMPACTINFO                                      \
  0x937u /* binary blob of dyld info (variable size) */
#define STACKSHOT_KCTYPE_SUSPENSION_INFO                                       \
  0x938u /* struct stackshot_suspension_info */
#define STACKSHOT_KCTYPE_SUSPENSION_SOURCE                                     \
  0x939u /* struct stackshot_suspension_source */

#define STACKSHOT_KCTYPE_TASK_DELTA_SNAPSHOT 0x940u /* task_delta_snapshot_v2  \
                                                     */
#define STACKSHOT_KCTYPE_THREAD_DELTA_SNAPSHOT                                 \
  0x941u /* thread_delta_snapshot_v* */
#define STACKSHOT_KCCONTAINER_SHAREDCACHE                                      \
  0x942u /* container for shared cache info */
#define STACKSHOT_KCTYPE_SHAREDCACHE_INFO                                      \
  0x943u /* dyld_shared_cache_loadinfo_v2 */
#define STACKSHOT_KCTYPE_SHAREDCACHE_AOTINFO                                   \
  0x944u /* struct dyld_aot_cache_uuid_info */
#define STACKSHOT_KCTYPE_SHAREDCACHE_ID                                        \
  0x945u /* uint32_t in task: if we aren't attached to Primary, which one */
#define STACKSHOT_KCTYPE_CODESIGNING_INFO                                      \
  0x946u /* struct stackshot_task_codesigning_info */
#define STACKSHOT_KCTYPE_OS_BUILD_VERSION                                      \
  0x947u /* os build version string (ex: 20A123) */
#define STACKSHOT_KCTYPE_KERN_EXCLAVES_THREADINFO                              \
  0x948u                                      /* struct thread_exclaves_info */
#define STACKSHOT_KCCONTAINER_EXCLAVES 0x949u /* exclave threads info */
#define STACKSHOT_KCCONTAINER_EXCLAVE_SCRESULT                                 \
  0x94au /* exclave thread container for one scid */
#define STACKSHOT_KCTYPE_EXCLAVE_SCRESULT_INFO                                 \
  0x94bu /* struct exclave_scresult_info */
#define STACKSHOT_KCCONTAINER_EXCLAVE_IPCSTACKENTRY                            \
  0x94cu /* container for one chunk of exclave IPC chain */
#define STACKSHOT_KCTYPE_EXCLAVE_IPCSTACKENTRY_INFO                            \
  0x94du /* struct exclave_ipcstackentry_info */
#define STACKSHOT_KCTYPE_EXCLAVE_IPCSTACKENTRY_ECSTACK                         \
  0x94eu /* exclave_ecstackentry_addr_t */
#define STACKSHOT_KCCONTAINER_EXCLAVE_ADDRESSSPACE                             \
  0x94fu /* exclave address space container */
#define STACKSHOT_KCTYPE_EXCLAVE_ADDRESSSPACE_INFO                             \
  0x950u /* struct exclave_addressspace_info */
#define STACKSHOT_KCTYPE_EXCLAVE_ADDRESSSPACE_NAME                             \
  0x951u /* exclave component name */
#define STACKSHOT_KCCONTAINER_EXCLAVE_TEXTLAYOUT                               \
  0x952u /* exclave text layout container */
#define STACKSHOT_KCTYPE_EXCLAVE_TEXTLAYOUT_INFO                               \
  0x953u /* struct exclave_textlayout_info */
#define STACKSHOT_KCTYPE_EXCLAVE_TEXTLAYOUT_SEGMENTS                           \
  0x954u /* struct exclave_textlayout_segment_v2 */
#define STACKSHOT_KCTYPE_KERN_EXCLAVES_CRASH_THREADINFO                        \
  0x955u /* struct thread_crash_exclaves_info */
#define STACKSHOT_KCTYPE_LATENCY_INFO_CPU                                      \
  0x956u /* struct stackshot_latency_cpu */
#define STACKSHOT_KCTYPE_TASK_EXEC_META 0x957u /* struct task_exec_meta */
#define STACKSHOT_KCTYPE_TASK_MEMORYSTATUS                                     \
  0x958u /* struct task_memorystatus_snapshot */
#define STACKSHOT_KCTYPE_LATENCY_INFO_BUFFER                                   \
  0x95au /* struct stackshot_latency_buffer */
...
struct dyld_uuid_info_64_v2 {
  ...
  uint64_t imageSlidBaseAddress; /* slid base address or slid first mapping of
                                    image */
};

enum dyld_shared_cache_flags {
  kSharedCacheSystemPrimary =
      0x1, /* primary shared cache on the system; attached tasks will have
              kTaskSharedRegionSystem set */
...
struct dyld_shared_cache_loadinfo_v2 {
  ...
  /* end of version 1 of dyld_uuid_info_64. sizeof v1 was 24 */
  uint64_t
      sharedCacheUnreliableSlidBaseAddress; /* for backwards-compatibility; use
                                               sharedCacheSlidFirstMapping if
                                               available */
...
struct dyld_aot_cache_uuid_info {
  uint64_t
      x86SlidBaseAddress; /* slid first mapping address of x86 shared cache */
...
enum task_snapshot_flags {
  /* k{User,Kernel}64_p (values 0x1 and 0x2) are defined in
     generic_snapshot_flags */
  ...
  kTaskSharedRegionSystem = 0x40000000, /* task attached to region with
                                           kSharedCacheSystemPrimary set */
...
enum task_exec_flags : uint64_t {
  kTaskExecTranslated =
      0x01, /* Task is running under translation (eg, Rosetta) */
  kTaskExecHardenedHeap =
      0x02, /* Task has the hardened heap security feature */
...
enum thread_snapshot_flags {
  ...
  kThreadTruncUserAsyncBT =
      0x10000, /* Unmapped pages caused truncated user async BT */
...
struct stackshot_fault_stats {
  uint32_t sfs_pages_faulted_in; /* number of pages faulted in using KDP fault
                                    path */
  ...
  uint8_t sfs_stopped_faulting; /* we stopped decompressing because we hit the
                                   limit */
} __attribute__((packed));
...
typedef struct stackshot_thread_waitinfo_v2 {
  ...
  uint8_t wait_type; /* The type of object that the thread is waiting on */
  int16_t
      portlabel_id; /* matches to a stackshot_portlabel, or NONE or MISSING */
  uint32_t wait_flags; /* info about the wait */
#define STACKSHOT_WAITINFO_FLAGS_SPECIALREPLY                                  \
  0x1 /* We're waiting on a special reply port */
} __attribute__((packed)) thread_waitinfo_v2_t;

typedef struct stackshot_thread_turnstileinfo {
  uint64_t waiter; /* The thread that's waiting on the object */
  uint64_t
      turnstile_context; /* Associated data (either thread id, or workq addr) */
...
typedef struct stackshot_thread_turnstileinfo_v2 {
  ...
  uint8_t turnstile_priority;
  uint8_t number_of_hops;
#define STACKSHOT_TURNSTILE_STATUS_UNKNOWN                                     \
  0x01 /* The final inheritor is unknown (bug?) */
#define STACKSHOT_TURNSTILE_STATUS_LOCKED_WAITQ                                \
  0x02 /* A waitq was found to be locked */
#define STACKSHOT_TURNSTILE_STATUS_WORKQUEUE                                   \
  0x04 /* The final inheritor is a workqueue */
#define STACKSHOT_TURNSTILE_STATUS_THREAD                                      \
  0x08 /* The final inheritor is a thread */
#define STACKSHOT_TURNSTILE_STATUS_BLOCKED_ON_TASK                             \
  0x10 /* blocked on task, dind't find thread */
#define STACKSHOT_TURNSTILE_STATUS_HELD_IPLOCK 0x20 /* the ip_lock was held */
#define STACKSHOT_TURNSTILE_STATUS_SENDPORT                                    \
  0x40 /* port_labelid was from a send port */
#define STACKSHOT_TURNSTILE_STATUS_RECEIVEPORT                                 \
  0x80 /* port_labelid was from a receive port */
  uint64_t
      turnstile_flags; // Note: Add any new flags to kcdata.py (turnstile_flags)
  int16_t
      portlabel_id; /* matches to a stackshot_portlabel, or NONE or MISSING */
} __attribute__((packed)) thread_turnstileinfo_v2_t;

#define STACKSHOT_TURNSTILE_STATUS_PORTFLAGS                                   \
  (STACKSHOT_TURNSTILE_STATUS_SENDPORT | STACKSHOT_TURNSTILE_STATUS_RECEIVEPORT)

#define STACKSHOT_PORTLABELID_NONE (0) /* No port label found */
#define STACKSHOT_PORTLABELID_MISSING                                          \
  (-1) /* portlabel found, but stackshot ran out of space to track it */
...
#define STACKSHOT_WAITOWNER_THREQUESTED                                        \
  (UINT64_MAX - 6) /* workloop waiting for a new worker thread */
#define STACKSHOT_WAITOWNER_SUSPENDED                                          \
  (UINT64_MAX - 7) /* workloop is suspended */

#define STACKSHOT_PORTLABEL_READFAILED 0x1 /* could not read port information  \
                                            */
#define STACKSHOT_PORTLABEL_THROTTLED                                          \
  0x2 /* service port is marked as throttled */
...
struct stackshot_suspension_info {
  uint64_t
      tss_last_start;    /* mach_absolute_time of beginning of last suspension*/
...
enum thread_exclaves_flags : uint32_t {
  ...
  kExclaveUpcallActive =
      0x2, /* Thread has upcalled back into xnu while handling RPC */
...
struct thread_exclaves_info {
  ...
  uint32_t tei_thread_offset; /* # frames from top of stack exclave frames
                                 should be inserted */
...
struct exclave_ipcstackentry_info {
  ...
  uint64_t
      eise_flags; /* A combination of enum exclave_ipcstackentry_flags values */
} __attribute__((packed));
...
struct exclave_addressspace_info {
  ...
  uint64_t
      eas_flags; /* A combination of enum exclave_addressspace_flags values */
  uint64_t eas_layoutid; /* textLayout for this address space */
  uint64_t
      eas_slide; /* slide to apply to textlayout, or UINT64_MAX if omitted */
  uint64_t eas_asroot; /* ASRoot/TTBR0 value used as an identifier for the
                          address space by cL4 */
} __attribute__((packed));

enum exclave_textlayout_flags : uint64_t {
  kExclaveTextLayoutLoadAddressesSynthetic =
      0x1, /* Load Addresses are synthetic */
  kExclaveTextLayoutLoadAddressesUnslid =
      0x2, /* Load Addresses are accurate and unslid */
  kExclaveTextLayoutHasSharedCache =
      0x4, /* sharedcache_index is valid, refers to entry # in
              STACKSHOT_KCTYPE_EXCLAVE_TEXTLAYOUT_SEGMENTS array */
};

struct exclave_textlayout_info_v1 {
  uint64_t layout_id;
  uint64_t
      etl_flags; /* A combination of enum exclave_textlayout_flags values */
} __attribute__((packed));
...
/* FIXME some of these types aren't clean (fixed width,  packed, and defined
 * *here*) */
...
#define TASK_CRASHINFO_BSDINFOWITHUNIQID                                       \
...
#define TASK_CRASHINFO_RUSAGE                                                  \
  0x807 /* struct rusage DEPRECATED do not use.                                \
         *                                                      This struct    \
         * has longs in it */
#define TASK_CRASHINFO_RUSAGE_INFO                                             \
  0x808 /* struct rusage_info_v3 from resource.h */
...
#define TASK_CRASHINFO_LEDGER_ALTERNATE_ACCOUNTING_COMPRESSED                  \
  0x822                                                   /* uint64_t */
#define TASK_CRASHINFO_LEDGER_PURGEABLE_NONVOLATILE 0x823 /* uint64_t */
#define TASK_CRASHINFO_LEDGER_PURGEABLE_NONVOLATILE_COMPRESSED                 \
  0x824                                                         /* uint64_t */
...
#define TASK_CRASHINFO_LEDGER_NETWORK_NONVOLATILE_COMPRESSED                   \
  0x829                                                           /* uint64_t */
...
#define TASK_CRASHINFO_KERNEL_TRIAGE_INFO_V1                                   \
  0x836 /* struct kernel_triage_info_v1 */
...
#define TASK_CRASHINFO_CS_SIGNING_ID                                           \
  0x83B /* string of len MAX_CRASHINFO_SIGNING_ID_LEN */
#define TASK_CRASHINFO_CS_TEAM_ID                                              \
  0x83C /* string of len MAX_CRASHINFO_TEAM_ID_LEN */
...
#define TASK_CRASHINFO_JIT_ADDRESS_RANGE                                       \
  0x840                         /* struct crashinfo_jit_address_range */
...
#define TASK_CRASHINFO_TASK_SECURITY_CONFIG                                    \
  0x845 /* struct task_security_config */
...
#define TASK_BTINFO_THREAD_EXCEPTION_STATE                                     \
  0xA23                               /* struct btinfo_thread_state_data_t */
...
#define TASK_BTINFO_FLAG_KCDATA_INCOMPLETE                                     \
  0x8 /* lw corpse collection is incomplete */
...
static inline int kcdata_iter_valid(kcdata_iter_t iter) {
  return ((uintptr_t)iter.item + sizeof(struct kcdata_item) <=
          (uintptr_t)iter.end) &&
         ((uintptr_t)iter.item + sizeof(struct kcdata_item) + iter.item->size <=
          (uintptr_t)iter.end);
}

static inline kcdata_iter_t kcdata_iter_next(kcdata_iter_t iter) {
  iter.item = (kcdata_item_t)(((uintptr_t)iter.item) +
                              sizeof(struct kcdata_item) + (iter.item->size));
  return iter;
}
...
static inline int kcdata_iter_is_legacy_item(kcdata_iter_t iter,
  ...
  return iter.item->size == legacy_size_padded &&
         (iter.item->flags & (KCDATA_FLAGS_STRUCT_PADDING_MASK |
                              KCDATA_FLAGS_STRUCT_HAS_PADDING)) == 0;
}
...
static inline int kcdata_iter_array_valid(kcdata_iter_t iter) {
  ...
  if (iter.item->type == KCDATA_TYPE_ARRAY) {
    ...
    return kcdata_iter_array_elem_count(iter) <= iter.item->size / elem_size &&
           iter.item->size % kcdata_iter_array_elem_count(iter) < 16;
  } else {
    return (iter.item->type & 0xf) <= iter.item->size &&
           kcdata_iter_array_elem_count(iter) <=
               iter.item->size - (iter.item->type & 0xf) &&
           (iter.item->size - (iter.item->type & 0xf)) %
                   kcdata_iter_array_elem_count(iter) ==
               0;
  }
...
static inline uint32_t kcdata_iter_array_elem_size(kcdata_iter_t iter) {
  ...
  return (iter.item->size - (iter.item->type & 0xf)) /
         kcdata_iter_array_elem_count(iter);
}

static inline int kcdata_iter_container_valid(kcdata_iter_t iter) {
  return kcdata_iter_valid(iter) &&
...
#define KCDATA_ITER_FOREACH(iter)                                              \
  for (; kcdata_iter_valid(iter) && iter.item->type != KCDATA_TYPE_BUFFER_END; \
       iter = kcdata_iter_next(iter))
#define KCDATA_ITER_FOREACH_FAILED(iter)                                       \
  (!kcdata_iter_valid(iter) || (iter).item->type != KCDATA_TYPE_BUFFER_END)

static inline kcdata_iter_t kcdata_iter_find_type(kcdata_iter_t iter,
                                                  uint32_t type) {
...
static inline int kcdata_iter_data_with_desc_valid(kcdata_iter_t iter,
                                                   uint32_t minsize) {
  return kcdata_iter_valid(iter) &&
...
static inline void kcdata_iter_get_data_with_desc(kcdata_iter_t iter,
                                                  char **desc_ptr,
                                                  void **data_ptr,
                                                  uint32_t *size_ptr) {
  if (desc_ptr) {
  ...
  if (data_ptr) {
    *data_ptr =
        (void *)((uintptr_t)kcdata_iter_payload(iter) + KCDATA_DESC_MAXLEN);
  }
  if (size_ptr) {
