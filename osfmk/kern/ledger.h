...
#define LEDGER_DIAG_MEM_THRESHOLD_INFINITY ((int16_t)((1ULL << 16) - 1))
  ...
  int16_t le_diag_threshold_scaled; /* Diag mem threshold for this entry,
                                       handled in Mbytes */
  ...
  union {
    ...
    struct {
      ...
      ledger_amount_t
          le_interval_max; /* Interval peak XXX better name needed */
#endif
    } _le_max;
  } _le;
...
struct ledger_entry_info_v2 {
  ...
  uint64_t lei_last_refill;   /* Time since last refill */
  int64_t
      lei_lifetime_max; /* for phys_footprint/neural_nofootprint_lifetime_max */
...
typedef void (*ledger_callback_t)(int warning, const void *param0,
                                  const void *param1);

extern ledger_template_t ledger_template_create(const char *name);
extern ledger_template_t ledger_template_copy(ledger_template_t template,
                                              const char *name);
 ...
 */
extern int ledger_entry_add_with_flags(ledger_template_t template,
                                       const char *key, const char *group,
                                       const char *units,
                                       ledger_entry_flags flags);
...
extern kern_return_t ledger_get_actions(ledger_t ledger, int entry,
                                        int *actions);
...
extern void ledger_check_new_balance(thread_t thread, ledger_t ledger,
                                     int entry);
...
extern kern_return_t ledger_disable_panic_on_negative(ledger_t ledger,
                                                      int entry);
extern kern_return_t ledger_get_panic_on_negative(ledger_t ledger, int entry,
                                                  int *panic_on_negative);

extern kern_return_t ledger_rollup(ledger_t to_ledger, ledger_t from_ledger);
extern kern_return_t ledger_rollup_entry(ledger_t to_ledger,
                                         ledger_t from_ledger, int entry);
...
extern uint64_t ledger_get_interval_remaining(ledger_t ledger, int entry,
                                              uint64_t now);
...
extern int ledger_get_task_entry_info_multiple(task_t task, void **buf,
                                               int *len, bool v2);

extern void ledger_get_entry_info(ledger_t ledger, int entry,
...
extern kern_return_t ledger_set_diag_mem_threshold(ledger_t ledger, int entry,
                                                   ledger_amount_t limit);
extern kern_return_t ledger_set_diag_mem_threshold_disabled(ledger_t ledger,
                                                            int entry);
extern kern_return_t ledger_set_diag_mem_threshold_enabled(ledger_t ledger,
                                                           int entry);
extern kern_return_t ledger_is_diag_threshold_enabled(ledger_t ledger,
                                                      int entry, bool *status);
