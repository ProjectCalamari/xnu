...
__options_decl(thread_work_interval_options_t, uint32_t,
               {
                   ...
                   /* Change the work interval from the context switch path
                      (thread may not be running or on a runq) */
                   THREAD_WI_THREAD_CTX_SWITCH = 0x10,
               });

static kern_return_t thread_set_work_interval(thread_t, struct work_interval *,
                                              thread_work_interval_options_t);
static void work_interval_port_no_senders(ipc_port_t, mach_port_mscount_t);

IPC_KOBJECT_DEFINE(IKOT_WORK_INTERVAL, .iko_op_movable_send = true,
                   .iko_op_stable = true,
 ...
 *
 * work_interval_auto_join_status_t represents the state of auto-join for a
 * given work interval. It packs the following information:
...
#define WORK_INTERVAL_STATUS_AUTO_JOIN_COUNT_MASK                              \
  ((uint32_t)(WORK_INTERVAL_STATUS_DEFERRED_FINISH_MASK - 1))
#define WORK_INTERVAL_STATUS_AUTO_JOIN_COUNT_MAX                               \
  WORK_INTERVAL_STATUS_AUTO_JOIN_COUNT_MASK
 ...
 *
 * Helper routine to check if work interval has the collection of telemetry data
 * enabled.
 */
static inline bool
work_interval_telemetry_data_enabled(struct work_interval *work_interval) {
  return (work_interval->wi_create_flags &
          WORK_INTERVAL_FLAG_ENABLE_TELEMETRY_DATA) != 0;
}
...
work_interval_get_recount_tracks(struct work_interval *work_interval) {
  if (work_interval != NULL &&
      work_interval_telemetry_data_enabled(work_interval)) {
 ...
 *
 * Perform a deferred finish for a work interval. The routine accepts the
 * deferred_finish_state as an argument rather than looking at the work_interval
 * since the deferred finish can race with another start-finish cycle. To
 * address that, the caller ensures that it gets a consistent snapshot of the
 * deferred state before calling this routine. This allows the racing
 * start-finish cycle to overwrite the deferred state without issues.
 */
static inline void work_interval_perform_deferred_finish(
    __unused struct work_interval_deferred_finish_state *deferred_finish_state,
...
work_interval_auto_join_increment(struct work_interval *work_interval) {
  struct work_interval_auto_join_info *join_info =
      &work_interval->wi_auto_join_info;
  __assert_only work_interval_auto_join_status_t old_status =
      os_atomic_add_orig(&join_info->status, 1, relaxed);
  assert(work_interval_status_auto_join_count(old_status) <
         WORK_INTERVAL_STATUS_AUTO_JOIN_COUNT_MAX);
}

/*
 * work_interval_auto_join_decrement()
 *
 * Routine to decrement the auto-join counter when a thread unjoins the work
 * interval (due to blocking or termination). If this was the last auto-joined
 * thread in the work interval and there was a deferred finish, performs the
 * finish operation for the work interval.
 */
static void
work_interval_auto_join_decrement(struct work_interval *work_interval,
                                  thread_t thread) {
  struct work_interval_auto_join_info *join_info =
      &work_interval->wi_auto_join_info;
  ...
  if (perform_finish == true) {
    ...
    work_interval_perform_deferred_finish(&deferred_finish_state, work_interval,
                                          thread);
  }
}
...
work_interval_auto_join_enabled(struct work_interval *work_interval) {
  return (work_interval->wi_create_flags &
          WORK_INTERVAL_FLAG_ENABLE_AUTO_JOIN) != 0;
}
...
work_interval_deferred_finish_enabled(struct work_interval *work_interval) {
  return (work_interval->wi_create_flags &
          WORK_INTERVAL_FLAG_ENABLE_DEFERRED_FINISH) != 0;
}

#endif /* CONFIG_SCHED_AUTO_JOIN */

static inline void work_interval_retain(struct work_interval *work_interval) {
  /*
 ...
 *
 * Routine to release a ref count on the work interval. If the refcount goes
 * down to zero, the work interval needs to be de-allocated.
 *
 * For non auto-join work intervals, they are de-allocated in this context.
 *
 * For auto-join work intervals, the de-allocation cannot be done from this
 * context since that might need the kernel memory allocator lock. In that case,
 * the deallocation is done via a thread-call based mpsc queue.
 */
static void
work_interval_release(struct work_interval *work_interval,
                      __unused thread_work_interval_options_t options) {
...
void kern_work_interval_release(struct work_interval *work_interval) {
 ...
 */
inline bool work_interval_should_propagate(thread_t cthread, thread_t thread) {
  /* Only allow propagation if the current thread has a work interval and the
   * woken up thread does not */
  if ((cthread->th_work_interval == NULL) ||
  ...
  if ((cthread->sched_mode != TH_MODE_REALTIME) ||
      (thread->sched_mode != TH_MODE_REALTIME)) {
    return false;
  }

  /* Work interval propagation only works for threads with the same home thread
   * group */
  ...
  /* If woken up thread has adopted vouchers and other thread groups, it does
   * not get propagation */
  if (thread->thread_group != thread_home_tg) {
    return false;
  }

  /* If either thread is inactive (in the termination path), do not propagate
   * auto-join */
...
void work_interval_auto_join_propagate(thread_t from, thread_t to) {
  ...
  __assert_only kern_return_t kr = thread_set_work_interval(
      to, from->th_work_interval,
      THREAD_WI_AUTO_JOIN_POLICY | THREAD_WI_THREAD_LOCK_HELD |
          THREAD_WI_THREAD_CTX_SWITCH);
 ...
 *
 * Routine to un-join an auto-joined work interval for a thread that is
 * blocking.
 *
 ...
 *
 * Routine to un-join an auto-joined work interval when a thread is changing
 * from realtime to non-realtime scheduling mode. This could happen due to
 * multiple reasons such as RT failsafe, thread backgrounding or thread
 * termination. Also, the thread being demoted may not be the current thread.
 *
 * Expects thread to be locked.
 */
void work_interval_auto_join_demote(thread_t thread) {
  __assert_only kern_return_t kr = thread_set_work_interval(
      thread, NULL, THREAD_WI_AUTO_JOIN_POLICY | THREAD_WI_THREAD_LOCK_HELD);
  assert(kr == KERN_SUCCESS);
}
...
work_interval_deallocate_queue_invoke(mpsc_queue_chain_t e,
  ...
  work_interval =
      mpsc_queue_element(e, struct work_interval, wi_deallocate_link);
...
#if CONFIG_SCHED_AUTO_JOIN
__startup_func static void work_interval_subsystem_init(void) {
  /*
  ...
  mpsc_daemon_queue_init_with_thread_call(
      &work_interval_deallocate_queue, work_interval_deallocate_queue_invoke,
 ...
 *
 * Description: Obtain a reference to the work_interval associated with a given
 * port.
 *
 ...
 *              Destroys the port and releases its reference on the work
 * interval.
 *
 * Parameters:  msg     A Mach no-senders notification message.
 *
 * Note: This assumes that there is only one create-right-from-work-interval
 * point, if the ability to extract another send right after creation is added,
 *       this will have to change to handle make-send counts correctly.
 */
static void work_interval_port_no_senders(ipc_port_t port,
                                          mach_port_mscount_t mscount) {
  struct work_interval *work_interval = NULL;

  work_interval = ipc_kobject_dealloc_port(port, mscount, IKOT_WORK_INTERVAL);
 ...
 */
static uint32_t work_interval_port_type(mach_port_name_t port_name) {
  struct work_interval *work_interval = NULL;
  kern_return_t kr;
 ...
 * interval.
 */
int work_interval_get_priority(thread_t thread) {
...
kern_return_t kern_work_interval_get_policy(struct work_interval *work_interval,
                                            integer_t *policy,
                                            integer_t *priority) {
  ...
  const sched_mode_t mode =
      work_interval_class_data[work_interval->wi_class].sched_mode;
 ...
 */
static void work_interval_set_policy(thread_t thread) {
 ...
 */
static void work_interval_clear_policy(thread_t thread) {
  assert3p(thread, ==, current_thread());
  ...
  proc_set_thread_policy_ext(
      thread, TASK_POLICY_ATTRIBUTE, TASK_POLICY_WI_DRIVEN, false,
 ...
 *
 * For non auto-join work intervals, deallocate any old work interval on the
 * thread For auto-join work intervals, the routine may wakeup the work interval
 * deferred deallocation queue since thread locks might be currently held.
 */
static kern_return_t
thread_set_work_interval(thread_t thread, struct work_interval *work_interval,
                         thread_work_interval_options_t options) {
  /* All explicit work interval operations should always be from the current
   * thread */
  ...
  if (work_interval) {
    ...
    if (options & THREAD_WI_EXPLICIT_JOIN_POLICY) {
      /* Ensure no kern_work_interval_set_workload_id can happen after this
       * point */
      uint32_t wlid_flags;
      (void)os_atomic_cmpxchgv(&work_interval->wi_wlid_flags, 0,
                               WORK_INTERVAL_WORKLOAD_ID_ALREADY_JOINED,
                               &wlid_flags, relaxed);
   ...
   *
   * If thread might be running on a remote core and it's not in the context
   * switch path (where thread is neither running, blocked or in the runq), its
   * not possible to update the work interval & thread group remotely since its
   * not possible to update CLPC for a remote core. This situation might happen
   * when a thread is transitioning from realtime to non-realtime due to
   * backgrounding etc., which would mean that non-RT threads would now be part
   * of the work interval.
   *
   * Since there is no immediate mitigation to this issue, the policy is to set
   * a new flag on the thread which indicates that such a "leak" has happened.
   * This flag will be cleared when the remote thread eventually blocks and
   * unjoins from the work interval.
   */
  bool thread_on_remote_core =
      ((thread != current_thread()) && (thread->state & TH_RUN) &&
       (thread_get_runq(thread) == PROCESSOR_NULL));

  if (thread_on_remote_core && ((options & THREAD_WI_THREAD_CTX_SWITCH) == 0)) {
    assert((options & THREAD_WI_THREAD_LOCK_NEEDED) == 0);
    os_atomic_or(&thread->th_work_interval_flags,
                 TH_WORK_INTERVAL_FLAGS_AUTO_JOIN_LEAK, relaxed);
    return KERN_SUCCESS;
  }

  const bool old_wi_auto_joined =
      ((thread->sched_flags & TH_SFLAG_THREAD_GROUP_AUTO_JOIN) != 0);

  if ((options & THREAD_WI_AUTO_JOIN_POLICY) || old_wi_auto_joined) {
    __kdebug_only uint64_t old_tg_id =
        (old_th_wi && old_th_wi->wi_group)
            ? thread_group_get_id(old_th_wi->wi_group)
            : ~0;
    __kdebug_only uint64_t new_tg_id =
        (work_interval && work_interval->wi_group)
            ? thread_group_get_id(work_interval->wi_group)
            : ~0;
  ...
  if (old_wi_auto_joined) {
    /*
     * If thread was auto-joined to a work interval and is not realtime, make
     * sure it happened due to the "leak" described above.
     */
    if (thread->sched_mode != TH_MODE_REALTIME) {
      assert((thread->th_work_interval_flags &
              TH_WORK_INTERVAL_FLAGS_AUTO_JOIN_LEAK) != 0);
    }

    os_atomic_andnot(&thread->th_work_interval_flags,
                     TH_WORK_INTERVAL_FLAGS_AUTO_JOIN_LEAK, relaxed);
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_WORKGROUP, WORKGROUP_INTERVAL_CHANGE),
               thread_tid(thread), (old_th_wi ? old_th_wi->wi_id : 0),
               (work_interval ? work_interval->wi_id : 0),
               !!(options & THREAD_WI_AUTO_JOIN_POLICY));
  ...
  struct thread_group *new_tg =
      (work_interval) ? (work_interval->wi_group) : NULL;
  ...
  if (options & THREAD_WI_EXPLICIT_JOIN_POLICY) {
    ...
    thread_work_interval_flags_t th_wi_xor_mask =
...
static kern_return_t
thread_set_work_interval_explicit_join(thread_t thread,
                                       struct work_interval *work_interval) {
  assert(thread == current_thread());
  return thread_set_work_interval(thread, work_interval,
                                  THREAD_WI_EXPLICIT_JOIN_POLICY |
                                      THREAD_WI_THREAD_LOCK_NEEDED);
}

kern_return_t work_interval_thread_terminate(thread_t thread) {
  assert(thread == current_thread());
  if (thread->th_work_interval != NULL) {
    return thread_set_work_interval(thread, NULL,
                                    THREAD_WI_EXPLICIT_JOIN_POLICY |
                                        THREAD_WI_THREAD_LOCK_NEEDED);
  }
  return KERN_SUCCESS;
...
kern_work_interval_notify(thread_t thread,
  ...
  if (work_interval->wi_creator_uniqueid != get_task_uniqueid(notifying_task) ||
      work_interval->wi_creator_pidversion !=
  ...
  kwi_args->urgency =
      (uint16_t)thread_get_urgency(thread, &urgency_param1, &urgency_param2);
...
kern_return_t kern_work_interval_create(
    thread_t thread, struct kern_work_interval_create_args *create_params) {
  assert(thread == current_thread());
  ...
  if ((create_flags & WORK_INTERVAL_TYPE_MASK) ==
      WORK_INTERVAL_TYPE_CA_CLIENT) {
    /*
     * CA_CLIENT work intervals do not create new thread groups.
     * There can only be one CA_CLIENT work interval (created by UIKit or
     * AppKit) per each application task
     */
  ...
  struct work_interval *work_interval =
      kalloc_type(struct work_interval, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  *work_interval = (struct work_interval){
  ...
  tg_id = work_interval->wi_group ? thread_group_get_id(work_interval->wi_group)
                                  : ~0;
  ...
  if (create_flags & WORK_INTERVAL_FLAG_JOINABLE) {
    ...
    work_interval->wi_port = ipc_kobject_alloc_port(
        work_interval, IKOT_WORK_INTERVAL, IPC_KOBJECT_ALLOC_MAKE_SEND);
    ...
    kern_return_t kr =
        thread_set_work_interval_explicit_join(thread, work_interval);
    if (kr != KERN_SUCCESS) {
  ...
  if (tg_id != ~0) {
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_WORKGROUP, WORKGROUP_INTERVAL_CREATE),
                 work_interval_id, create_flags, pid_from_task(creating_task),
                 tg_id);
  }
  return KERN_SUCCESS;
}

kern_return_t kern_work_interval_get_flags_from_port(mach_port_name_t port_name,
                                                     uint32_t *flags) {
  assert(flags != NULL);
...
kern_return_t kern_work_interval_set_name(mach_port_name_t port_name,
                                          __unused char *name, size_t len) {
  kern_return_t kr;
  struct work_interval *work_interval;
...
#if CONFIG_THREAD_GROUPS
  uint32_t wi_group_flags =
      os_atomic_load(&work_interval->wi_group_flags, relaxed);
  ...
  if (name[0] && (work_interval->wi_create_flags & WORK_INTERVAL_FLAG_GROUP)) {
    ...
    snprintf(tgname, sizeof(tgname), "WI%lld %s", work_interval->wi_id, name);
...
kern_return_t kern_work_interval_set_workload_id(
  ...
  if (kr == KERN_SUCCESS) {
    if ((wl_config.wc_create_flags & WORK_INTERVAL_TYPE_MASK) !=
        (work_interval->wi_create_flags & WORK_INTERVAL_TYPE_MASK)) {
      if ((wl_config.wc_create_flags & WORK_INTERVAL_TYPE_MASK) ==
              WORK_INTERVAL_TYPE_CA_RENDER_SERVER &&
          (work_interval->wi_create_flags & WORK_INTERVAL_TYPE_MASK) ==
              WORK_INTERVAL_TYPE_FRAME_COMPOSITOR) {
        /* WORK_INTERVAL_TYPE_FRAME_COMPOSITOR is a valid related type of
         * WORK_INTERVAL_TYPE_CA_RENDER_SERVER */
      } else {
        kr = KERN_INVALID_ARGUMENT;
        goto out;
...
kern_return_t kern_work_interval_destroy(thread_t thread,
...
kern_return_t kern_work_interval_join(thread_t thread,
                                      mach_port_name_t port_name) {
  struct work_interval *work_interval = NULL;
  kern_return_t kr;
...
bool work_interval_port_type_render_server(mach_port_name_t port_name) {
  return work_interval_port_type(port_name) ==
         WORK_INTERVAL_TYPE_CA_RENDER_SERVER;
}
