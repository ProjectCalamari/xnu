...
static size_t __btstack_len(bt_stack_t bts) {
  return bts->bts_ref_len & BTS_FRAMES_LEN_MASK;
}

static size_t __btstack_size(bt_stack_t bts) {
  return sizeof(uint32_t) * __btstack_len(bts);
}

static bool __btstack_same(bt_stack_t a, bt_stack_t b) {
  return a->bts_hash == b->bts_hash && __btstack_len(a) == __btstack_len(b) &&
         memcmp(a->bts_frames, b->bts_frames, __btstack_size(a)) == 0;
}

static uint32_t __btstack_capture(bt_stack_t bts, void *fp, bool permanent) {
...
static btref_t __btstack_try_retain(btref_t btref, bt_stack_t bts,
  ...
  } while (!os_atomic_cmpxchgv(&bts->bts_ref_len, oref, nref, &oref, relaxed));
...
__abortlike static void __btstack_resurrect_panic(bt_stack_t bts) {
  panic("trying to resurrect bt stack %p", bts);
}

static btref_t __btstack_retain(btref_t btref, bt_stack_t bts,
                                btref_get_flags_t flags) {
  uint32_t oref, nref;
...
__abortlike static void __btstack_over_release_panic(bt_stack_t bts) {
  panic("trying to over-release bt stack %p", bts);
}

static bool __btstack_release(bt_stack_t bts) {
  uint32_t oref, nref;
...
static bt_stack_t __btlib_deref(bt_library_t btl, btref_t ref) {
  uint32_t slab = 0;
...
static void __btlib_lock(bt_library_t btl) {
  lck_ticket_lock(&btl->btl_lock, &bt_library_lck_grp);
}

static void __btlib_unlock(bt_library_t btl) {
  lck_ticket_unlock(&btl->btl_lock);
}

static inline btref_smr_t *__btlib_head(bt_library_t btl, uint32_t param,
                                        uint32_t hash) {
...
static bool __btlib_growth_needed(bt_library_t btl) {
...
static bool __btlib_rehash_needed(bt_library_t btl) {
...
static void __btlib_callout_wakeup(bt_library_t btl) {
  if (startup_phase >= STARTUP_SUB_THREAD_CALL && !btl->btl_in_callout) {
    thread_call_enter(btl->btl_call);
  }
}

__attribute__((noinline)) static void __btlib_grow(bt_library_t btl) {
  ...
  while (btl->btl_grower) {
    ...
    lck_ticket_sleep_with_inheritor(
        &btl->btl_lock, &bt_library_lck_grp, LCK_SLEEP_DEFAULT,
        &btl->btl_grower, btl->btl_grower, THREAD_UNINT, TIMEOUT_WAIT_FOREVER);
  ...
  if (btl->btl_faulted_pos < btl->btl_alloc_pos + PAGE_SIZE / 2) {
    ...
    kr = kernel_memory_populate(addr, PAGE_SIZE,
                                KMA_KOBJECT | KMA_ZERO | KMA_SPRAYQTN,
                                VM_KERN_MEMORY_DIAG);
  }
...
static void __btlib_split_step(bt_library_t btl, bt_hash_t *bthp, uint32_t idx,
                               uint32_t mask) {
...
#if BTLIB_VALIDATE
static void __btlib_validate(bt_library_t btl, bt_hash_t *bthp, uint32_t size,
...
__btlib_rehash_and_lock(bt_library_t btl) {
  ...
  for (uint32_t i = 0; i < size_old; i++) {
    bthp[2 * i] = bthp_old[i];
    bthp[2 * i + 1] = kalloc_type(struct bt_hash, Z_WAITOK_ZERO_NOFAIL);
  }
...
static void __btlib_callout(thread_call_param_t arg0,
                            thread_call_param_t __unused arg1) {
...
static void __btlib_init(bt_library_t btl) {
  kern_return_t kr;
  vm_address_t addr;
  bt_hash_t *bthp;
...
__attribute__((noinline)) static btref_t __btlib_insert(bt_library_t btl,
                                                        bt_stack_t needle,
                                                        btref_get_flags_t flags,
                                                        uint32_t hash) {
  bt_stack_t bts;
...
#if BTLIB_VALIDATE
    assert3u(bts->bts_hash & BTL_HASH_MASK, ==, hash & BTL_HASH_MASK);
#endif /* BTLIB_VALIDATE */
...
__abortlike static void __btlib_remove_notfound_panic(bt_library_t btl,
                                                      bt_stack_t bts) {
  panic("couldn't find stack %p in library %p", bts, btl);
}

static void __btlib_remove_locked(bt_library_t btl, btref_t ref,
                                  bt_stack_t bts) {
...
static btref_t __btlib_get(bt_library_t btl, void *fp,
                           btref_get_flags_t flags) {
...
btref_t btref_get(void *fp, btref_get_flags_t flags) {
  return __btlib_get(&bt_library, fp, flags);
}

__abortlike static void __btref_invalid(btref_t btref) {
  panic("trying to manipulate invalid backtrace ref: 0x%08x", btref);
}

static inline bool __btref_isvalid(btref_t btref) {
  return ((btref & BTREF_VALID_MASK) & ~BTREF_GET_PERMANENT) == 0;
}

btref_t btref_retain(btref_t btref) {
...
uint32_t btref_decode_unslide(btref_t btref, mach_vm_address_t bt_out[]) {
  ...
  if (__btref_isvalid(btref)) {
    ...
    backtrace_unpack(BTP_KERN_OFFSET_32, (uintptr_t *)bt_out, BTLOG_MAX_DEPTH,
...
static void __btlog_lock(btlogu_t btlu) {
  lck_ticket_lock(&btlu.btl->btl_lock, &btlog_lck_grp);
}

static void __btlog_unlock(btlogu_t btlu) {
  lck_ticket_unlock(&btlu.btl->btl_lock);
}

static void *__btlog_elem_normalize(void *addr) {
  addr = (void *)vm_memtag_canonicalize_kernel((vm_offset_t)addr);
  return addr;
}

static long __btlog_elem_encode(void *addr) {
  return ~(long)__btlog_elem_normalize(addr);
}

static void *__btlog_elem_decode(long addr) { return (void *)~addr; }

static struct bt_hash_head *__btlog_hash_hash(struct btlog_hash *btlh) {
  return (struct bt_hash_head *)(btlh->btlh_entries + btlh->btlh_count);
}

static uint32_t __btlog_hash_count(struct btlog_hash *btlh) {
  return btlh->btlh_count >> 2;
}

static struct bt_hash_head *__btlog_hash_head(struct btlog_hash *btlh,
                                              void *addr) {
...
__attribute__((overloadable)) static struct btlog_size_pair {
  vm_size_t btsp_size;
  uint32_t btsp_count;
  ...
  case BTLOG_HASH:
    ...
    pair.btsp_size =
        round_page(sizeof(struct btlog_hash) +
...
static inline btref_t __bt_ref(uint32_t stack_and_op) {
  return stack_and_op & ~BTREF_OP_MASK;
}

static inline btref_t __bt_op(uint32_t stack_and_op) {
  return stack_and_op & BTREF_OP_MASK;
}

#pragma mark btlog_log

static void __btlog_log_destroy(struct btlog_log *btll) {
  for (uint32_t i = 0; i < btll->btll_count; i++) {
    btref_put(__bt_ref(btll->btll_entries[i].btle_where));
  }
}

static void __btlog_log_record(struct btlog_log *btll, void *addr, uint8_t op,
                               btref_t btref) {
...
static void __btlog_hash_init(struct btlog_hash *btlh) {
...
static void __btlog_hash_destroy(struct btlog_hash *btlh) {
  for (uint32_t i = 0; i < btlh->btlh_count; i++) {
    btref_put(__bt_ref(btlh->btlh_entries[i].bthe_where));
  }
}

static uint32_t __btlog_hash_stailq_pop_first(struct btlog_hash *btlh,
...
static void __btlog_hash_stailq_remove(struct bt_hash_head *head,
                                       struct bt_hash_entry *bthe,
                                       uint32_t *prev, uint32_t ppos) {
  *prev = bthe->bthe_next;
  if (bthe->bthe_next == BT_HASH_END_MARKER) {
    head->bthh_last = ppos;
  } else {
...
static void __btlog_hash_stailq_append(struct btlog_hash *btlh,
                                       struct bt_hash_head *head,
                                       uint32_t pos) {
...
static void __btlog_hash_remove(struct btlog_hash *btlh,
                                struct bt_hash_entry *bthe) {
  struct bt_hash_head *head;
  uint32_t *prev;
  uint32_t ppos;
...
static void __btlog_hash_record(struct btlog_hash *btlh, void *addr, uint8_t op,
                                btref_t btref) {
  struct bt_hash_head *head;
  struct bt_hash_entry *bthe;
  btref_t old = BTREF_NULL;
  ...
  *bthe = (struct bt_hash_entry){
  ...
  if (btref & BTREF_VALID_MASK) {
    assert(__btlib_deref(&bt_library, btref & BTREF_VALID_MASK)->bts_ref_len >=
           BTS_FRAMES_REF_INC);
  }
...
static void __btlog_hash_erase(struct btlog_hash *btlh, void *addr) {
  struct bt_hash_head *head;
  struct bt_hash_entry *bthe;
  uint32_t *prev;
  uint32_t pos, ppos;
...
static void __btlog_init(btlogu_t btlu) {
...
btlog_t btlog_create(btlog_type_t type, uint32_t count, uint32_t sample) {
  ...
  if (sample > 1) {
    btlu.btl->btl_sample =
        zalloc_percpu(percpu_u64_zone, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static void __btlog_destroy(btlogu_t btlu) {
  switch (btlu.btl->btl_type) {
  case BTLOG_LOG:
...
void btlog_destroy(btlogu_t btlu) {
...
kern_return_t btlog_enable(btlogu_t btlu) {
  vm_size_t size;
  kern_return_t kr = KERN_SUCCESS;

  size = __btlog_size(btlu).btsp_size;
  if (size > PAGE_SIZE) {
    kr = kernel_memory_populate(btlu.bta + PAGE_SIZE, size - PAGE_SIZE,
                                KMA_KOBJECT | KMA_ZERO | KMA_SPRAYQTN,
                                VM_KERN_MEMORY_DIAG);
  }
...
void btlog_disable(btlogu_t btlu) {
  vm_size_t size;
  ...
  size = __btlog_size(btlu).btsp_size;
  bzero((char *)btlu.bta + sizeof(*btlu.btl), PAGE_SIZE - sizeof(*btlu.btl));
  if (size > PAGE_SIZE) {
    kernel_memory_depopulate(btlu.bta + PAGE_SIZE, size - PAGE_SIZE,
                             KMA_KOBJECT, VM_KERN_MEMORY_DIAG);
  }
}

btlog_type_t btlog_get_type(btlog_t btlog) { return btlog->btl_type; }

uint32_t btlog_get_count(btlog_t btlog) { return btlog->btl_count; }

bool btlog_sample(btlog_t btlog) {
  uint32_t *counter;
...
void btlog_record(btlogu_t btlu, void *addr, uint8_t op, btref_t btref) {
  if (btlu.btl->btl_disabled) {
    return;
  }
  switch (btlu.btl->btl_type) {
...
void btlog_erase(btlogu_t btlu, void *addr) {
  if (btlu.btl->btl_disabled) {
    return;
  }
  switch (btlu.btl->btl_type) {
...
static int btlog_record_cmp_where(const void *e1, const void *e2) {
...
static bool btlog_records_pack(struct btlog_record *array, uint32_t *countp) {
...
static int btlog_record_cmp_rev_count(const void *e1, const void *e2) {
  const struct btlog_record *a = e1;
  const struct btlog_record *b = e2;
...
kern_return_t btlog_get_records(btlogu_t btl, zone_btrecord_t **records,
  ...
  ipc_map_size =
      (vm_offset_t)(vm_map_max(ipc_kernel_map) - vm_map_min(ipc_kernel_map));
...
uint32_t btlog_guess_top(btlogu_t btlu, vm_address_t bt[], uint32_t *len) {
...
void btlog_copy_backtraces_for_elements(btlogu_t btlu, vm_address_t *instances,
                                        uint32_t *countp, uint32_t elem_size,
                                        leak_site_proc proc) {
  struct btlog_hash *btlh = btlu.btlh;
  struct bt_hash_head *head;
