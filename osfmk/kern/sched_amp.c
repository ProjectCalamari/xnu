...
static thread_t sched_amp_steal_thread(processor_set_t pset);

static void
sched_amp_thread_update_scan(sched_update_scan_context_t scan_context);

static boolean_t sched_amp_processor_enqueue(processor_t processor,
                                             thread_t thread,
                                             sched_options_t options);

static boolean_t sched_amp_processor_queue_remove(processor_t processor,
                                                  thread_t thread);

static ast_t sched_amp_processor_csw_check(processor_t processor);

static boolean_t sched_amp_processor_queue_has_priority(processor_t processor,
                                                        int priority,
                                                        boolean_t gte);

static int sched_amp_runq_count(processor_t processor);

static boolean_t sched_amp_processor_queue_empty(processor_t processor);

static uint64_t sched_amp_runq_stats_count_sum(processor_t processor);

static int sched_amp_processor_bound_count(processor_t processor);

static void sched_amp_pset_init(processor_set_t pset);

static void sched_amp_processor_init(processor_t processor);

static thread_t sched_amp_choose_thread(processor_t processor, int priority,
                                        __unused thread_t prev, ast_t reason);

static void sched_amp_processor_queue_shutdown(processor_t processor);

static sched_mode_t sched_amp_initial_thread_sched_mode(task_t parent_task);

static processor_t sched_amp_choose_processor(processor_set_t pset,
                                              processor_t processor,
                                              thread_t thread,
                                              sched_options_t *options);

static bool sched_amp_thread_avoid_processor(processor_t processor,
                                             thread_t thread,
                                             __unused ast_t reason);

static bool sched_amp_thread_should_yield(processor_t processor,
                                          thread_t thread);

static void
sched_amp_thread_group_recommendation_change(struct thread_group *tg,
                                             cluster_type_t new_recommendation);

static bool sched_amp_thread_eligible_for_pset(thread_t thread,
                                               processor_set_t pset);

static void sched_amp_cpu_init_completed(void);
...
static thread_t sched_amp_choose_thread(processor_t processor, int priority,
  ...
  if (pset == ecore_set &&
      bit_test(pset->pending_spill_cpu_mask, processor->cpu_id)) {
    spill_pending = true;
  ...
  if ((bound_runq->highq < priority) && (main_runq->highq < priority) &&
      (spill_pri < priority)) {
    return THREAD_NULL;
  }

  if ((spill_pri > bound_runq->highq) && (spill_pri > main_runq->highq)) {
    /*
...
static boolean_t sched_amp_processor_queue_empty(processor_t processor) {
  ...
  return (amp_main_runq(processor)->count == 0) &&
         (amp_bound_runq(processor)->count == 0) && !spill_pending;
}

static bool sched_amp_thread_should_yield(processor_t processor,
                                          thread_t thread) {
  if (!sched_amp_processor_queue_empty(processor) ||
      (rt_runq_count(processor->processor_set) > 0)) {
    return true;
  }

  if ((processor->processor_set->pset_cluster_type == PSET_AMP_E) &&
      (recommended_pset_type(thread) == PSET_AMP_P)) {
 ...
 *
 */
thread_t sched_amp_steal_thread(processor_set_t pset) {
  ...
  if (sched_get_pset_load_average(nset, 0) >=
      sched_amp_steal_threshold(nset, spill_pending)) {
    pset_unlock(pset);
    ...
    /* Allow steal if load average still OK, no idle cores, and more threads on
     * runq than active cores DISPATCHING */
    if ((sched_get_pset_load_average(pset, 0) >=
         sched_amp_steal_threshold(pset, spill_pending)) &&
        (pset->pset_runq.count >
         bit_count(pset->cpu_state_map[PROCESSOR_DISPATCHING])) &&
        (bit_count(pset->recommended_bitmask &
                   pset->cpu_state_map[PROCESSOR_IDLE]) == 0)) {
      thread = run_queue_dequeue(&pset->pset_runq, SCHED_HEADQ);
      KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_AMP_STEAL) | DBG_FUNC_NONE,
           spill_pending, 0, 0, 0);
...
static bool pcores_recommended(thread_t thread) {
  if (!pcore_set) {
    return false;
  }
...
static processor_t
sched_amp_choose_processor(processor_set_t pset, processor_t processor,
                           thread_t thread, __unused sched_options_t *options) {
  ...
  /* Now that the chosen pset is definitely locked, make sure nothing important
   * has changed */
...
void sched_amp_thread_group_recommendation_change(
...
static char *pct_name[] = {"PSET_SMP", "PSET_AMP_E", "PSET_AMP_P"};

static void sched_amp_cpu_init_completed(void) {
  if (PE_parse_boot_argn("cpus", NULL, 0) ||
      PE_parse_boot_argn("cpumask", NULL, 0)) {
  ...
  for (processor_t p = processor_list; p != NULL; p = p->processor_list) {
    ...
    kprintf("%s>cpu_id %02d in pset_id %02d type %s\n", __FUNCTION__, p->cpu_id,
            pset->pset_id, pct_name[pset->pset_cluster_type]);
