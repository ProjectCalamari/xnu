 ...
 * in the depot or shrink it based on the @c zc_grow_level and @c
 * zc_shrink_level thresholds.
 *
...
__startup_data static struct zone_page_metadata
    zone_early_meta_array_startup[ZONE_EARLY_META_INLINE_COUNT];

__startup_data __attribute__((aligned(PAGE_MAX_SIZE))) static uint8_t
    zone_early_pages_to_cram[PAGE_MAX_SIZE * 16];
...
SECURITY_READ_ONLY_LATE(zone_security_flags_t)
zone_security_array[MAX_ZONES] = {
    [0 ... MAX_ZONES - 1] =
        {
...
SECURITY_READ_ONLY_LATE(struct zone_size_params)
zone_ro_size_params[ZONE_ID__LAST_RO + 1];
SECURITY_READ_ONLY_LATE(zone_cache_ops_t) zcache_ops[ZONE_ID__FIRST_DYNAMIC];

#if DEBUG || DEVELOPMENT
unsigned int zone_max_zones(void) { return MAX_ZONES; }
#endif
...
static TUNABLE_WRITEABLE(uint32_t, zone_early_thres_mul, "zone_early_thres_mul",
                         5);
...
static char const *const zone_submaps_names[Z_SUBMAP_IDX_COUNT] = {
    [Z_SUBMAP_IDX_VM] = "VM",          [Z_SUBMAP_IDX_READ_ONLY] = "RO",
#if ZSECURITY_CONFIG(SAD_FENG_SHUI)
    [Z_SUBMAP_IDX_GENERAL_0] = "GEN0", [Z_SUBMAP_IDX_GENERAL_1] = "GEN1",
    [Z_SUBMAP_IDX_GENERAL_2] = "GEN2", [Z_SUBMAP_IDX_GENERAL_3] = "GEN3",
#else
...
zone_metadata_corruption(zone_t zone, struct zone_page_metadata *meta,
                         const char *kind) {
  panic("zone metadata corruption: %s (meta %p, zone %s%s)", kind, meta,
        zone_heap_name(zone), zone->z_name);
}

__abortlike static void zone_invalid_element_addr_panic(zone_t zone,
                                                        vm_offset_t addr) {
...
__abortlike static void
zone_page_metadata_list_corruption(zone_t zone,
                                   struct zone_page_metadata *meta) {
...
zone_meta_double_free_panic(zone_t zone, vm_offset_t addr, const char *caller) {
  panic("%s: double free of %p to zone %s%s", caller, (void *)addr,
        zone_heap_name(zone), zone->z_name);
}

__abortlike static void zone_accounting_panic(zone_t zone, const char *kind) {
  panic("accounting mismatch (%s) for zone %s%s", kind, zone_heap_name(zone),
        zone->z_name);
}
...
static inline uint16_t zone_meta_alloc_size_add(zone_t z,
...
static inline uint16_t zone_meta_alloc_size_sub(zone_t z,
                                                struct zone_page_metadata *m,
                                                vm_offset_t esize) {
...
__abortlike static void zone_nofail_panic(zone_t zone) {
  panic("zalloc(Z_NOFAIL) can't be satisfied for zone %s%s (potential leak)",
        zone_heap_name(zone), zone->z_name);
}

__header_always_inline bool zone_spans_ro_va(vm_offset_t addr_start,
...
#define from_range(r, addr, size)                                              \
  __builtin_choose_expr(                                                       \
      __builtin_constant_p(size) ? (size) == 1 : 0,                            \
      mach_vm_range_contains(                                                  \
          r, vm_memtag_canonicalize_kernel((mach_vm_offset_t)(addr))),         \
      mach_vm_range_contains(                                                  \
          r, vm_memtag_canonicalize_kernel((mach_vm_offset_t)(addr)), size))

#define from_ro_map(addr, size) from_range(&zone_info.zi_ro_range, addr, size)

#define from_zone_map(addr, size)                                              \
  from_range(&zone_info.zi_map_range, addr, size)

__header_always_inline bool zone_pva_is_null(zone_pva_t page) {
  return page.packed_address == 0;
}

__header_always_inline bool zone_pva_is_queue(zone_pva_t page) {
  // actual kernel pages have the top bit set
  return (int32_t)page.packed_address > 0;
}

__header_always_inline bool zone_pva_is_equal(zone_pva_t pva1,
                                              zone_pva_t pva2) {
  return pva1.packed_address == pva2.packed_address;
}

__header_always_inline zone_pva_t *zone_pageq_base(void) {
...
__header_always_inline zone_pva_t zone_queue_encode(zone_pva_t *headp) {
  return (zone_pva_t){(uint32_t)(headp - zone_pageq_base())};
}

__header_always_inline zone_pva_t zone_pva_from_addr(vm_address_t addr) {
  // cannot use atop() because we want to maintain the sign bit
  return (zone_pva_t){(uint32_t)((intptr_t)addr >> PAGE_SHIFT)};
}

__header_always_inline vm_address_t zone_pva_to_addr(zone_pva_t page) {
...
zone_pva_to_meta(zone_pva_t page) {
  return VM_FAR_ADD_PTR_UNBOUNDED(zone_info.zi_meta_base, page.packed_address);
}
...
__header_always_inline zone_id_t zone_index_from_ptr(const void *ptr) {
...
__attribute__((overloadable)) __header_always_inline void
...
__header_always_inline void zone_meta_requeue(zone_t z, zone_pva_t *headp,
                                              struct zone_page_metadata *meta) {
...
static inline void zone_meta_lock_in_partial(zone_t z,
                                             struct zone_page_metadata *m,
                                             uint32_t len) {
...
static inline void zone_meta_unlock_from_partial(zone_t z,
                                                 struct zone_page_metadata *m,
                                                 uint32_t len) {
 ...
 */
static void zone_meta_populate(vm_offset_t base, vm_size_t size) {
  ...
  for (; page_addr < (vm_offset_t)to; page_addr += PAGE_SIZE) {
    ...
    for (;;) {
      ...
      if (0 == pmap_find_phys(kernel_pmap, page_addr)) {
        ret = kernel_memory_populate(page_addr, PAGE_SIZE,
...
__abortlike static void zone_invalid_element_panic(zone_t zone,
                                                   vm_offset_t addr) {
  struct zone_page_metadata *meta;
  ...
  if (meta->zm_chunk_len == ZM_SECONDARY_PCPU_PAGE) {
    panic("metadata %p corresponding to addr %p being freed to "
          ...
          meta, (void *)addr, zone_heap_name(zone), zone->z_name, from_cache);
  }
...
__attribute__((always_inline)) static struct zone_page_metadata *
zone_element_resolve(zone_t zone, vm_offset_t addr, vm_offset_t *idx) {
  struct zone_page_metadata *meta;
...
void *zone_element_pgz_oob_adjust(void *ptr, vm_size_t req_size,
                                  vm_size_t elem_size) {
   ...
   */
  if (req_size == elem_size || (end & PAGE_MASK) ||
...
__abortlike static void zone_element_bounds_check_panic(vm_address_t addr,
                                                        vm_size_t len) {
  struct zone_page_metadata *meta;
  ...
  panic("zone bound checks: buffer %p of length %zd overflows "
        ...
        zone_elem_inner_size(zone), zone, zone_heap_name(zone),
        zone_name(zone));
}

void zone_element_bounds_check(vm_address_t addr, vm_size_t len) {
  struct zone_page_metadata *meta;
...
__abortlike static void zone_require_panic(zone_t zone, void *addr) {
  uint32_t zindex;
  zone_t other;
  ...
  if (zindex >= os_atomic_load(&num_zones, relaxed) || !other->z_self) {
    panic("zone_require failed: invalid zone index %d "
          "(addr: %p, expected: %s%s)",
          zindex, addr, zone_heap_name(zone), zone->z_name);
  } else {
    panic("zone_require failed: address in unexpected zone id %d (%s%s) "
          "(addr: %p, expected: %s%s)",
          zindex, zone_heap_name(other), other->z_name, addr,
          zone_heap_name(zone), zone->z_name);
  }
}

__abortlike static void zone_id_require_panic(zone_id_t zid, void *addr) {
 ...
 */
__mockable void zone_require(zone_t zone, void *addr) {
...
__mockable void zone_id_require(zone_id_t zid, vm_size_t esize, void *addr) {
...
void zone_id_require_aligned(zone_id_t zid, void *addr) {
  ...
  if (from_zone_map(addr, 1)) {
    ...
    if (zid == meta->zm_index && Z_FAST_ALIGNED(offs, zone->z_align_magic)) {
      return;
    }
  }
...
bool zone_owns(zone_t zone, void *addr) {
  vm_size_t esize = zone_elem_inner_size(zone);
...
static inline struct mach_vm_range zone_kmem_suballoc(mach_vm_offset_t addr,
  ...
  *new_map =
      kmem_suballoc(kernel_map, &addr, size,
                    VM_MAP_CREATE_NEVER_FAULTS | VM_MAP_CREATE_DISABLE_HOLELIST,
                    flags, KMS_PERMANENT | KMS_NOFAIL | KMS_NOSOFTLIMIT, tag)
          .kmr_submap;
#else
...
static_assert(2ul * ZBA_GRANULE << ZBA_MAX_ORDER == ZBA_CHUNK_SIZE,
              "chunk sizes");
...
static struct zone_bits_allocator_header *zba_base_header(void) {
  return (struct zone_bits_allocator_header *)zba_test_info.zbats_base;
}

static kern_return_t zba_populate(uint32_t n, bool with_extra __unused) {
...
#else
__startup_data __attribute__((
    aligned(ZBA_CHUNK_SIZE))) static uint8_t zba_chunk_startup[ZBA_CHUNK_SIZE];

static SECURITY_READ_ONLY_LATE(uint8_t) zba_xtra_shift;
static LCK_MTX_DECLARE(zba_mtx, &zone_locks_grp);

static struct zone_bits_allocator_header *zba_base_header(void) {
  return (struct zone_bits_allocator_header *)
      zone_info.zi_bits_range.min_address;
}

static void zba_lock(void) { lck_mtx_lock(&zba_mtx); }

static void zba_unlock(void) { lck_mtx_unlock(&zba_mtx); }

__abortlike static void zba_memory_exhausted(void) {
  uint64_t zsize = 0;
  ...
  panic("zba_populate: out of bitmap space, "
        ...
        zone_heap_name(z), zone_name(z), mach_vm_size_pretty(zsize),
        mach_vm_size_unit(zsize), zone_count_allocated(z));
}

static kern_return_t zba_populate(uint32_t n, bool with_extra) {
...
__pure2 static struct zone_bits_allocator_meta *zba_meta(void) {
  return (struct zone_bits_allocator_meta *)&zba_base_header()[1];
}

__pure2 static uint64_t *zba_slot_base(void) {
  return (uint64_t *)zba_base_header();
}

__pure2 static struct zone_bits_head *zba_head(uint32_t order,
                                               bool with_extra) {
  if (with_extra) {
...
__pure2 static uint32_t zba_head_index(struct zone_bits_head *hd) {
  return (uint32_t)((uint64_t *)hd - zba_slot_base());
}

__pure2 static struct zone_bits_chain *zba_chain_for_index(uint32_t index) {
  return (struct zone_bits_chain *)(zba_slot_base() + index);
}

__pure2 static uint32_t zba_chain_to_index(const struct zone_bits_chain *zbc) {
  return (uint32_t)((const uint64_t *)zbc - zba_slot_base());
}

__abortlike static void zba_head_corruption_panic(uint32_t order,
                                                  bool with_extra) {
  panic("zone bits allocator head[%d:%d:%p] is corrupt", order, with_extra,
        zba_head(order, with_extra));
}

__abortlike static void zba_chain_corruption_panic(struct zone_bits_chain *a,
                                                   struct zone_bits_chain *b) {
  panic("zone bits allocator freelist is corrupt (%p <-> %p)", a, b);
}

static void zba_push_block(struct zone_bits_chain *zbc, uint32_t order,
                           bool with_extra) {
...
static vm_address_t zba_try_pop_block(uint32_t order, bool with_extra) {
  struct zone_bits_head *hd = zba_head(order, with_extra);
...
static struct zone_bits_allocator_header *zba_header(vm_offset_t addr) {
...
static size_t zba_node(vm_offset_t addr, uint32_t order) {
  vm_offset_t offs = (addr % ZBA_CHUNK_SIZE) / ZBA_GRANULE;
  return (offs >> order) + (1 << (ZBA_MAX_ORDER - order + 1)) - 1;
}

static struct zone_bits_chain *
zba_chain_for_node(struct zone_bits_allocator_header *zbah, size_t node,
                   uint32_t order) {
  vm_offset_t offs = (node - (1 << (ZBA_MAX_ORDER - order + 1)) + 1) << order;
  return (struct zone_bits_chain *)((vm_offset_t)zbah + offs * ZBA_GRANULE);
}

static void zba_node_flip_split(struct zone_bits_allocator_header *zbah,
                                size_t node) {
  zbah->zbah_bits[node / 64] ^= 1ull << (node % 64);
}

static bool zba_node_is_split(struct zone_bits_allocator_header *zbah,
                              size_t node) {
  return zbah->zbah_bits[node / 64] & (1ull << (node % 64));
}

static void zba_free(vm_offset_t addr, uint32_t order, bool with_extra) {
...
static vm_size_t zba_chunk_header_size(uint32_t n) {
...
__attribute__((noinline)) static void zba_grow(bool with_extra) {
...
static vm_offset_t zba_alloc(uint32_t order, bool with_extra) {
...
static void *zba_extra_ref_ptr(uint32_t bref, vm_offset_t idx) {
...
static uint32_t zba_bits_ref_order(uint32_t bref) {
  return bref >> ZBA_ORDER_SHIFT;
}

static bitmap_t *zba_bits_ref_ptr(uint32_t bref) {
  return zba_slot_base() + (bref & ZBA_PTR_MASK);
}

static vm_offset_t zba_scan_bitmap_inline(zone_t zone,
                                          struct zone_page_metadata *meta,
...
static vm_offset_t zba_scan_bitmap_ref(zone_t zone,
                                       struct zone_page_metadata *meta,
                                       vm_offset_t eidx) {
 ...
 */
static vm_offset_t zone_meta_find_and_clear_bit(zone_t zone, zone_stats_t zs,
                                                struct zone_page_metadata *meta,
                                                zalloc_flags_t flags) {
 ...
 */
static void zone_meta_bits_init_inline(struct zone_page_metadata *meta,
                                       uint32_t count) {
  /*
 ...
 */
static uint32_t zone_meta_bits_alloc_init(uint32_t count, uint32_t nbits,
                                          bool with_extra) {
  static_assert(ZONE_MAX_ALLOC_SIZE / ZONE_MIN_ELEM_SIZE <=
                    ZBA_GRANULE_BITS << ZBA_MAX_ORDER,
  ...
  return (uint32_t)(bits - zba_slot_base()) + (order << ZBA_ORDER_SHIFT) +
 ...
 */
static void zone_meta_bits_merge(struct zone_page_metadata *meta,
  ...
  if (meta->zm_inline_bitmap) {
    while (start < end) {
      ...
      if (s_i == s_e) {
        meta[s_i].zm_bitmap |=
            zba_map_mask_lt(uint32_t, end) & zba_map_mask_ge(uint32_t, start);
        break;
      }
    ...
    while (start < end) {
      ...
      if (s_i == s_e) {
        bits[s_i] |=
            zba_map_mask_lt(uint64_t, end) & zba_map_mask_ge(uint64_t, start);
        break;
      }
 ...
 */
static bool zone_meta_is_free(struct zone_page_metadata *meta,
                              vm_offset_t eidx) {
  if (meta->zm_inline_bitmap) {
 ...
 */
static bool zone_meta_mark_free(struct zone_page_metadata *meta,
                                vm_offset_t eidx) {
  if (meta->zm_inline_bitmap) {
    uint32_t bit = zba_map_bit(uint32_t, eidx);
...
__startup_func void __zone_site_register(vm_allocation_site_t *site) {
  if (zone_tagging_on) {
    vm_tag_alloc(site);
  }
}

uint16_t zone_index_from_tag_index(uint32_t sizeclass_idx) {
...
static inline void *zstack_tbi_fix(vm_offset_t elem) {
  elem = vm_memtag_load_tag(elem);
  return (void *)elem;
}

static inline vm_offset_t zstack_tbi_fill(void *addr) {
...
__attribute__((always_inline)) void zstack_push(zstack_t *stack, void *addr) {
...
__attribute__((always_inline)) void *zstack_pop(zstack_t *stack) {
  stack->z_count--;
  return zstack_pop_no_delta(stack);
}

static inline void zone_recirc_lock_nopreempt_check_contention(zone_t zone) {
  uint32_t ticket;

  if (__probable(hw_lck_ticket_reserve_nopreempt(&zone->z_recirc_lock, &ticket,
                                                 &zone_locks_grp))) {
    return;
  }
...
static inline void zone_recirc_lock_nopreempt(zone_t zone) {
  hw_lck_ticket_lock_nopreempt(&zone->z_recirc_lock, &zone_locks_grp);
}

static inline void zone_recirc_unlock_nopreempt(zone_t zone) {
  hw_lck_ticket_unlock_nopreempt(&zone->z_recirc_lock);
}

static inline void zone_lock_nopreempt_check_contention(zone_t zone) {
  uint32_t ticket;
#if KASAN_FAKESTACK
  spl_t s = 0;
...
static inline void zone_lock_nopreempt(zone_t zone) {
#if KASAN_FAKESTACK
  spl_t s = 0;
  if (zone->z_kasan_fakestacks) {
    s = splsched();
...
static inline void zone_unlock_nopreempt(zone_t zone) {
#if KASAN_FAKESTACK
...
static inline void zone_depot_lock_nopreempt(zone_cache_t zc) {
  hw_lck_ticket_lock_nopreempt(&zc->zc_depot_lock, &zone_locks_grp);
}

static inline void zone_depot_unlock_nopreempt(zone_cache_t zc) {
  hw_lck_ticket_unlock_nopreempt(&zc->zc_depot_lock);
}

static inline void zone_depot_lock(zone_cache_t zc) {
  hw_lck_ticket_lock(&zc->zc_depot_lock, &zone_locks_grp);
}

static inline void zone_depot_unlock(zone_cache_t zc) {
...
static inline bool zone_supports_vm(zone_t z) {
  /*
...
const char *zone_name(zone_t z) { return z->z_name; }

const char *zone_heap_name(zone_t z) {
...
static uint32_t zone_alloc_pages_for_nelems(zone_t z, vm_size_t max_elems) {
  vm_size_t elem_count, chunks;

  elem_count =
      ptoa(z->z_percpu ? 1 : z->z_chunk_pages) / zone_elem_outer_size(z);
  chunks = (max_elems + elem_count - 1) / elem_count;

  return (uint32_t)MIN(UINT32_MAX, chunks * z->z_chunk_pages);
}

static inline vm_size_t zone_submaps_approx_size(void) {
  vm_size_t size = 0;
...
static inline void zone_depot_init(struct zone_depot *zd) {
  *zd = (struct zone_depot){
      .zd_tail = &zd->zd_head,
  };
}

static inline void zone_depot_insert_head_full(struct zone_depot *zd,
                                               zone_magazine_t mag) {
  if (zd->zd_full++ == 0) {
...
static inline void zone_depot_insert_tail_full(struct zone_depot *zd,
                                               zone_magazine_t mag) {
  zd->zd_full++;
...
static inline void zone_depot_insert_head_empty(struct zone_depot *zd,
                                                zone_magazine_t mag) {
  zd->zd_empty++;
  mag->zm_next = *zd->zd_tail;
  *zd->zd_tail = mag;
...
static inline zone_magazine_t zone_depot_pop_head_full(struct zone_depot *zd,
                                                       zone_t z) {
...
static inline zone_magazine_t zone_depot_pop_head_empty(struct zone_depot *zd,
                                                        zone_t z) {
...
static inline smr_seq_t zone_depot_move_full(struct zone_depot *dst,
                                             struct zone_depot *src, uint32_t n,
                                             zone_t z) {
...
static inline void zone_depot_move_empty(struct zone_depot *dst,
                                         struct zone_depot *src, uint32_t n,
                                         zone_t z) {
  zone_magazine_t head, last;
...
static inline bool zone_depot_poll(struct zone_depot *depot, smr_t smr) {
  if (depot->zd_full == 0) {
    return false;
  }
...
static void zone_cache_swap_magazines(zone_cache_t cache) {
...
__pure2 static smr_t zone_cache_smr(zone_cache_t cache) {
  return cache->zc_smr;
}
 ...
 */
static zone_magazine_t zone_magazine_replace(zone_cache_t zc,
                                             zone_magazine_t mag, bool empty) {
  zone_magazine_t old;
  vm_offset_t **elems;
...
static zone_magazine_t zone_magazine_alloc(zalloc_flags_t flags) {
  return zalloc_flags(zc_magazine_zone, flags | Z_ZERO);
}

static void zone_magazine_free(zone_magazine_t mag) {
  (zfree)(zc_magazine_zone, mag);
}

static void zone_magazine_free_list(struct zone_depot *zd) {
...
__mockable void zone_enable_caching(zone_t zone) {
...
#if KASAN_LIGHT
bool kasan_zone_maps_owned(vm_address_t addr, vm_size_t size) {
  return from_zone_map(addr, size) ||
...
__attribute__((always_inline)) vm_map_t
zone_submap(zone_security_flags_t zsflags) {
  return zone_submaps[zsflags.z_submap_idx];
}

unsigned zpercpu_count(void) { return zpercpu_early_count; }
 ...
 */
static uint64_t zalloc_random_mask64(uint32_t bits) {
  ...
  if (__probable(cpuid_features() & CPUID_FEATURE_RDRAND)) {
    ...
    v = random_bool_gen_bits(&zone_bool_gen[cpu].zbg_bg,
                             zone_bool_gen[cpu].zbg_entropy, ZONE_ENTROPY_CNT,
                             bits);
    enable_preemption();
  }
#else
 ...
 */
static uint32_t zalloc_random_uniform32(uint32_t bound_min,
                                        uint32_t bound_max) {
 ...
 */
static bool track_kalloc_zones(zone_t z, const char *logname) {
  const char *prefix;
  size_t len;
  zone_security_flags_t zsflags = zone_security_config(z);
...
vm_size_t zone_element_info(void *addr, vm_tag_t *ptag) {
  ...
  if (from_zone_map(addr, sizeof(void *))) {
    ...
    if (__improbable(src_zone->z_uses_tags)) {
      ...
      meta = zone_element_resolve(src_zone, (vm_offset_t)addr, &eidx);
...
vm_size_t kasan_quarantine_resolve(vm_address_t addr, zone_t *zonep) {
 ...
 */
static bool zalloc_skip_zero_check(void) {
#if defined(__x86_64__)
...
__abortlike static void zalloc_uaf_panic(zone_t z, uintptr_t elem,
                                         size_t size) {
  ...
  (panic)("[%s%s]: element modified after free "
          ...
          zone_heap_name(z), zone_name(z), first_offs, first_bits, esize,
          (void *)elem, buf);
}

static void zalloc_validate_element(zone_t zone, vm_offset_t elem,
                                    vm_size_t size, zalloc_flags_t flags) {
  if (flags & Z_NOZZC) {
    return;
  }
...
#pragma unused(zone, cpu, zs)
  ...
  bits =
      random_bool_gen_bits(&zone_bool_gen[cpu].zbg_bg,
...
__startup_func static void zone_leaks_init_zrecs(void) {
  /*
   ...
   */
  if (zrecs == 0) {
    zrecs = ZRECORDS_DEFAULT * (uint32_t)((max_mem + (1ul << 30)) >> 30);
  }
...
static uint32_t zone_leaks_record_count(zone_t z) {
  uint32_t recs = (zlsize << 10) / zone_elem_inner_size(z);

  return MIN(MAX(recs, ZRECORDS_DEFAULT), ZRECORDS_MAX);
}

static uint32_t zone_leaks_sample_rate(zone_t z) {
 ...
 */
static void zone_setup_logging(zone_t z) {
  char zone_name[MAX_ZONE_NAME]; /* Temp. buffer for the zone name */
  char zlog_name[MAX_ZONE_NAME]; /* Temp. buffer to create the strings zlog1,
                                    zlog2 etc... */
   ...
   * boot-args.
   */
  if (!logging_on && PE_parse_boot_argn("zlog", zlog_val, sizeof(zlog_val))) {
    if (track_this_zone(zone_name, zlog_val) ||
        track_kalloc_zones(z, zlog_val)) {
      logging_on = true;
    }
  ...
  if (logging_on) {
    if (corruption_debug_flag) {
      ...
      z->z_btlog = btlog_create(BTLOG_HASH, zone_leaks_record_count(z), 0);
    }
    if (z->z_btlog) {
      z->z_log_on = true;
...
__startup_func static void kasan_tbi_init_zrecs(void) {
  /*
   * Don't allow more than ZRECORDS_MAX records,
   * even if the user asked for more.
   *
   ...
   */
  if (kasan_zrecs == 0) {
    kasan_zrecs = ZRECORDS_DEFAULT * (uint32_t)((max_mem + (1ul << 30)) >> 30);
  }
...
static void zone_setup_kasan_logging(zone_t z) {
  if (!z->z_tbi_tag) {
...
static inline bool zleak_should_enable_for_zone(zone_t z) {
  if (z->z_log_on) {
    return false;
  }
  if (z->z_btlog) {
...
static inline bool zleak_should_disable_for_zone(zone_t z) {
  if (z->z_log_on) {
    return false;
  }
  if (!z->z_btlog) {
...
static void zleaks_enable_async(__unused thread_call_param_t p0,
  ...
  zone_foreach(z) {
    ...
    if (zleak_should_enable_for_zone(z)) {
      ...
      if (log == NULL) {
        log = btlog_create(BTLOG_HASH, zone_leaks_record_count(z),
...
__startup_func static void zleak_init(void) {
  ...
  thread_call_setup_with_options(&zone_leaks_callout, zleaks_enable_async, NULL,
                                 THREAD_CALL_PRIORITY_USER,
                                 THREAD_CALL_OPTIONS_ONCE);
}
STARTUP(ZALLOC, STARTUP_RANK_SECOND, zleak_init);

kern_return_t zleak_update_threshold(vm_size_t *arg, uint64_t value) {
...
static void panic_display_zleaks(bool has_syms) {
...
__cold
#endif
    static void zalloc_log(zone_t zone, vm_offset_t addr, uint32_t count,
                           void *fp) {
...
__cold
#endif
    static void zfree_log(zone_t zone, vm_offset_t addr, uint32_t count,
                          void *fp) {
  btlog_t log = zone->z_btlog;
  btref_get_flags_t flags = 0;
  btref_t ref;
...
zone_kma_flags(zone_t z, zone_security_flags_t zsflags, zalloc_flags_t flags) {
  ...
  if (z->z_permanent ||
      (!z->z_destructible && zone_submap_is_sequestered(zsflags))) {
...
static inline void zone_add_wired_pages(zone_t z, uint32_t pages) {
...
static inline void zone_remove_wired_pages(zone_t z, uint32_t pages) {
...
static inline void zone_tag_element(zone_t zone, caddr_t addr,
                                    vm_size_t elem_size) {
  if (zone->z_percpu) {
...
static inline caddr_t zone_tag_free_element(zone_t zone, caddr_t addr,
                                            vm_size_t elem_size) {
...
static inline void zcram_memtag_init(zone_t zone, vm_offset_t base,
                                     uint32_t start, uint32_t end) {
...
#define zcram_memtag_init(z, b, s, e)                                          \
  do {                                                                         \
  } while (0)
#endif /* ZSECURITY_CONFIG(ZONE_TAGGING) */
 ...
 */
static void zcram_and_lock(zone_t zone, vm_offset_t addr, uint32_t pg_va_new,
  ...
  if (pg_start == 0) {
    ...
    if (!zone->z_permanent && !inline_bitmap) {
      meta[0].zm_bitmap = zone_meta_bits_alloc_init(
          free_end, zone->z_chunk_elems, zone->z_uses_tags);
    }
  ...
  if (zone->z_permanent) {
    ...
    for (uint32_t i = 0; i < pg_end; i++) {
      kasan_zmem_add(addr + ptoa(i), PAGE_SIZE, zone_elem_outer_size(zone),
                     zone_elem_outer_offs(zone), zone_elem_redzone(zone));
    }
  } else {
    kasan_zmem_add(addr, ptoa(pg_end), zone_elem_outer_size(zone),
                   zone_elem_outer_offs(zone), zone_elem_redzone(zone));
  }
#endif /* KASAN_CLASSIC */
...
static void zcram(zone_t zone, vm_offset_t addr, uint32_t pages,
                  uint16_t lock) {
  uint32_t chunk_pages = zone->z_chunk_pages;
...
void zone_fill_initially(zone_t zone, vm_size_t nelems) {
  ...
  if (zone_submap_is_sequestered(zsflags)) {
    ...
    kernel_memory_populate(addr, ptoa(pages), kmaflags, VM_KERN_MEMORY_ZONE);
  } else {
    assert(zsflags.z_submap_idx != Z_SUBMAP_IDX_READ_ONLY);
    kmem_alloc(zone_submap(zsflags), &addr, ptoa(pages), kmaflags,
               VM_KERN_MEMORY_ZONE);
  }
...
#if ZSECURITY_CONFIG(SAD_FENG_SHUI)
__attribute__((noinline)) static void
zone_scramble_va_and_unlock(zone_t z, struct zone_page_metadata *meta,
                            uint32_t runs, uint32_t pages, uint32_t chunk_pages,
                            uint64_t guard_mask) {
...
static inline uint32_t dist_u32(uint32_t a, uint32_t b) {
  return a < b ? b - a : a - b;
}

static uint64_t zalloc_random_clear_n_bits(uint64_t mask, uint32_t pop,
                                           uint32_t n) {
  for (; n-- > 0; pop--) {
 ...
 */
static uint64_t zalloc_random_bits(uint32_t pop, uint32_t width) {
...
static void zone_allocate_va_locked(zone_t z, zalloc_flags_t flags) {
  zone_security_flags_t zsflags = zone_security_config(z);
  struct zone_page_metadata *meta;
  ...
  if (zone_submap_is_sequestered(zsflags)) {
    kr = zone_submap_alloc_sequestered_va(zsflags, pages + guard_pages, &addr);
  } else {
    assert(zsflags.z_submap_idx != Z_SUBMAP_IDX_READ_ONLY);
    kr = kmem_alloc(zone_submap(zsflags), &addr, ptoa(pages + guard_pages),
  ...
  if (kr != KERN_SUCCESS) {
    ...
    panic("zalloc[%d]: zone map exhausted while allocating from zone [%s%s], "
          "likely due to memory leak in zone [%s%s] "
          "(%u%c, %d elements allocated)",
          kr, zone_heap_name(z), zone_name(z), zone_heap_name(zone_largest),
          zone_name(zone_largest), mach_vm_size_pretty(zone_size),
          mach_vm_size_unit(zone_size), zone_count_allocated(zone_largest));
  }
...
static inline void ZONE_TRACE_VM_KERN_REQUEST_START(vm_size_t size) {
#if DEBUG || DEVELOPMENT
...
static inline void ZONE_TRACE_VM_KERN_REQUEST_END(uint32_t pages) {
...
__ZONE_MAP_EXHAUSTED_AND_WAITING_FOR_GC__(zone_t z, uint32_t pgs) {
  ...
  for (uint32_t n = 1; mapped >= zone_pages_wired_max - pgs; n++) {
    ...
    if (zone_exhausted_timeout > 0 && wait_ms > zone_exhausted_timeout) {
      panic("zone map exhaustion: waited for %dms "
            "(pages: %ld, max: %ld, wanted: %d)",
            wait_ms, mapped, zone_pages_wired_max, pgs);
    }

    clock_interval_to_absolutetime_interval(wait_ms, NSEC_PER_MSEC, &interval);

    lck_spin_lock(&zone_exhausted_lock);
    lck_spin_sleep_with_inheritor(&zone_exhausted_lock, LCK_SLEEP_UNLOCK,
                                  &zone_pages_wired, vm_pageout_gc_thread,
                                  THREAD_UNINT, wait_start + interval);

    mapped = os_atomic_load(&zone_pages_wired, relaxed);
  }
}

static bool zone_expand_wait_for_pages(bool waited) {
  if (waited) {
    return false;
  }
#if DEBUG || DEVELOPMENT
...
static inline void zone_expand_async_schedule_if_allowed(zone_t zone) {
...
zalloc_expand_drain_exhausted_caches_locked(zone_t z) {
  ...
  if (z->z_depot_size) {
    ...
    if (zd.zd_full) {
      zone_depot_move_full(&z->z_recirc, &zd, zd.zd_full, NULL);
    }
    if (zd.zd_empty) {
      zone_depot_move_empty(&z->z_recirc, &zd, zd.zd_empty, NULL);
    }
...
static bool zalloc_needs_refill(zone_t zone, zalloc_flags_t flags) {
...
static void zone_wakeup_exhausted_waiters(zone_t z) {
...
static pmap_mapping_type_t zone_mapping_type(zone_t z) {
  zone_security_flags_t zsflags = zone_security_config(z);
...
static vm_prot_t zone_page_prot(zone_security_flags_t zsflags) {
  switch (zsflags.z_submap_idx) {
  case Z_SUBMAP_IDX_READ_ONLY:
    return VM_PROT_READ;
  default:
...
static void zone_expand_locked(zone_t z, zalloc_flags_t flags) {
  zone_security_flags_t zsflags = zone_security_config(z);
  ...
  for (;;) {
    ...
    hw_lck_ticket_sleep_with_inheritor(
        &z->z_lock, &zone_locks_grp, LCK_SLEEP_DEFAULT, &z->z_expander,
        z->z_expander->ze_thread, TH_UNINT, TIMEOUT_WAIT_FOREVER);
  }
  ...
  do {
...
#if !KASAN_CLASSIC
    if (!z->z_percpu) {
      min_pages = (uint32_t)atop(
          round_page(zone_elem_outer_offs(z) + zone_elem_outer_size(z)));
    }
#endif /* !KASAN_CLASSIC */
    ...
    while (pages < z->z_chunk_pages - cur_pages) {
      ...
      if (pages >= min_pages && !zone_expand_wait_for_pages(waited)) {
        break;
      }
      ...
      if (meta) {
        zone_meta_queue_push(z, &z->z_pageq_va, meta + cur_pages);
      }
      goto page_shortage;
    }
    vm_object_t object;
    ...
    kernel_memory_populate_object_and_unlock(
        object, addr + ptoa(cur_pages), addr + ptoa(cur_pages), ptoa(pages),
        page_list, zone_kma_flags(z, zsflags, flags), VM_KERN_MEMORY_ZONE,
...
static void zone_expand_async(__unused thread_call_param_t p0,
                              __unused thread_call_param_t p1) {
  zone_foreach(z) {
    if (z->no_callout) {
...
/*
 * We're being very conservative here and picking a value of 95%. We might need
 * to lower this if we find that we're not catching the problem and are still
 * hitting zone map exhaustion panics.
 */
...
kern_return_t zone_map_jetsam_set_limit(uint32_t value) {
...
void get_largest_zone_info(char *zone_name, size_t zone_name_len,
                           uint64_t *zone_size) {
   ...
   * Append kalloc heap name to zone name (if zone is used by kalloc)
   */
  snprintf(zone_name, zone_name_len, "%s%s", zone_heap_name(largest_zone),
           largest_zone->z_name);
}

static bool zone_map_nearing_threshold(unsigned int threshold) {
...
/*
 * Tries to kill a single process if it can attribute one to the largest zone.
 * If not, wakes up the memorystatus thread to walk through the jetsam priority
 * bands and kill processes.
 */
static zone_t kill_process_in_largest_zone(void) {
  pid_t pid = -1;
  uint64_t zone_size = 0;
  zone_t largest_zone = zone_find_largest(&zone_size);

  printf("zone_map_exhaustion: Zone mapped %lld of %lld, used %lld, capacity "
         "%lld [jetsam limit %d%%]\n",
         ptoa_64(os_atomic_load(&zone_pages_wired, relaxed)),
         ptoa_64(zone_pages_wired_max), (uint64_t)zone_submaps_approx_size(),
  ...
  if (largest_zone == vm_object_zone) {
    ...
    unsigned int vm_map_entry_zone_count =
        zone_count_allocated(vm_map_entry_zone);
    /* Is the VM map entries zone count >= 98% of the VM objects zone count? */
    if (vm_map_entry_zone_count >=
        ((vm_object_zone_count * VMENTRY_TO_VMOBJECT_COMPARISON_RATIO) / 100)) {
      largest_zone = vm_map_entry_zone;
      printf("zone_map_exhaustion: Picking VM map entries as the zone to "
             "target, size %lu\n",
...
__header_always_inline void zfree_drop(zone_t zone, vm_offset_t addr) {
  ...
  if (new_size == 0) {
    /* whether the page was on the intermediate or all_used, queue, move it to
     * free */
...
__attribute__((noinline)) static void zfree_item(zone_t zone,
                                                 vm_offset_t addr) {
...
static void zfree_cached_depot_recirculate(zone_t zone, uint32_t depot_max,
                                           zone_cache_t cache) {
  ...
  if (n >= depot_max) {
    ...
    seq = zone_depot_move_full(&zone->z_recirc, &cache->zc_depot,
                               smr ? n : n - depot_max / 2, NULL);

    if (smr) {
      smr_deferred_advance_commit(smr, seq);
      if (depot_max > 1 && zone_depot_poll(&zone->z_recirc, smr)) {
        zone_depot_move_full(&cache->zc_depot, &zone->z_recirc, depot_max / 2,
                             NULL);
      }
    }
  }
  ...
  if (n) {
    zone_depot_move_empty(&cache->zc_depot, &zone->z_recirc, n, zone);
  }

  zone_recirc_unlock_nopreempt(zone);
}

static zone_cache_t zfree_cached_recirculate(zone_t zone, zone_cache_t cache) {
...
__attribute__((noinline)) static zone_cache_t
...
__attribute__((always_inline)) static inline zone_cache_t
...
__attribute__((always_inline)) static inline vm_offset_t
...
#pragma unused(combined_size)
  ...
  if (__improbable(zone->z_uses_tags)) {
    ...
    slot =
        zba_extra_ref_ptr(meta->zm_bitmap, Z_FAST_QUO(offs, zone->z_quo_magic));
  ...
  elem = (vm_offset_t)zone_tag_free_element(zone, (caddr_t)elem,
                                            ZFREE_ELEM_SIZE(combined_size));
  return elem;
}

__attribute__((always_inline)) void *zcache_mark_invalid(zone_t zone,
                                                         void *elem) {
  vm_size_t esize = zone_elem_inner_size(zone);

  ZFREE_LOG(zone, (vm_offset_t)elem, 1);
  return (void *)__zcache_mark_invalid(zone, (vm_offset_t)elem,
                                       ZFREE_PACK_SIZE(esize, esize));
}

/*
 *     The function is noinline when zlog can be used so that the backtracing
 * can reliably skip the zfree_ext() and zfree_log() boring frames.
 */
#if ZALLOC_ENABLE_LOGGING
__attribute__((noinline))
#endif /* ZALLOC_ENABLE_LOGGING */
__mockable void zfree_ext(zone_t zone, zone_stats_t zstats, void *addr,
...
__attribute__((always_inline)) static inline zstack_t
zcache_free_stack_to_cpu(zone_id_t zid, zone_cache_t cache, zstack_t stack,
                         vm_size_t esize, zone_cache_ops_t ops, bool zero) {
  ...
  do {
    ...
    if (ops) {
      ...
      o = (void *)__zcache_mark_invalid(zone_by_id(zid), (vm_offset_t)o,
                                        ZFREE_PACK_SIZE(esize, esize));
    }
    *--p = (vm_offset_t)o;
  } while (--n > 0);
...
__attribute__((always_inline)) static inline void
zcache_free_n_ext(zone_id_t zid, zstack_t stack, zone_cache_ops_t ops,
                  bool zero) {
  zone_t zone = zone_by_id(zid);
  zone_cache_t cache;
  vm_size_t esize;
  ...
  cpu = cpu_number();
  esize = zone_elem_inner_size(zone);
  zpercpu_get_cpu(zone->z_stats, cpu)->zs_mem_freed += stack.z_count * esize;

  for (;;) {
    cache = zfree_cached_get_pcpu_cache(zone, cpu);
    if (__probable(cache)) {
      stack = zcache_free_stack_to_cpu(zid, cache, stack, esize, ops, zero);
      enable_preemption();
    } else if (ops) {
      enable_preemption();
...
void(zcache_free)(zone_id_t zid, void *addr, zone_cache_ops_t ops) {
  __builtin_assume(ops != NULL);
  zcache_free_1_ext(zid, addr, ops);
}

void(zcache_free_n)(zone_id_t zid, zstack_t stack, zone_cache_ops_t ops) {
  __builtin_assume(ops != NULL);
  zcache_free_n_ext(zid, stack, ops, false);
}

void(zfree_n)(zone_id_t zid, zstack_t stack) {
  zcache_free_n_ext(zid, stack, NULL, true);
}

void(zfree_nozero)(zone_id_t zid, void *addr) {
  zcache_free_1_ext(zid, addr, NULL);
}

void(zfree_nozero_n)(zone_id_t zid, zstack_t stack) {
  zcache_free_n_ext(zid, stack, NULL, false);
}

void(zfree)(zone_t zov, void *addr) {
...
__attribute__((noinline)) void zfree_percpu(union zone_or_view zov,
                                            void *addr) {
...
void(zfree_id)(zone_id_t zid, void *addr) { (zfree)(&zone_array[zid], addr); }

void(zfree_ro)(zone_id_t zid, void *addr) {
...
__attribute__((noinline)) static void zfree_item_smr(zone_t zone,
                                                     vm_offset_t addr) {
...
void(zfree_smr)(zone_t zone, void *addr) {
  vm_offset_t elem = (vm_offset_t)addr;
  vm_offset_t esize;
  zone_cache_t cache;
  int cpu;
...
void(zfree_id_smr)(zone_id_t zid, void *addr) {
 ...
 */
__header_always_inline vm_size_t zalloc_import(zone_t zone, vm_offset_t *elems,
                                               zalloc_flags_t flags,
                                               uint32_t n) {
  vm_offset_t esize = zone_elem_outer_size(zone);
...
#pragma unused(zone, flags)
  ...
  if (__improbable(zone->z_uses_tags)) {
    ...
    *slot = tag;
    ...
    vm_tag_update_zone_size(tag, zone->z_tags_sizeclass, (long)esize);
  }
#endif /* VM_TAG_SIZECLASSES */
  ...
  if ((flags & Z_SKIP_KASAN) == 0) {
    kasan_alloc(addr, esize, esize, zone_elem_redzone(zone), (flags & Z_PCPU),
...
__attribute__((always_inline)) void *zcache_mark_valid(zone_t zone,
                                                       void *addr) {
 ...
 */
__attribute__((always_inline)) static struct kalloc_result
zalloc_return(zone_t zone, vm_offset_t addr, zalloc_flags_t flags,
...
static vm_size_t zalloc_get_shared_threshold(zone_t zone, vm_size_t esize) {
  if (esize <= 512) {
...
__attribute__((noinline)) static struct kalloc_result
...
static void zalloc_cached_import(zone_t zone, zalloc_flags_t flags,
                                 zone_cache_t cache) {
...
static void zalloc_cached_depot_recirculate(zone_t zone, uint32_t depot_max,
                                            zone_cache_t cache, smr_t smr) {
  smr_seq_t seq;
  uint32_t n;
  ...
  if (n >= depot_max) {
    zone_depot_move_empty(&zone->z_recirc, &cache->zc_depot, n - depot_max / 2,
                          NULL);
  }
  ...
  if (n && zone_depot_poll(&zone->z_recirc, smr)) {
    zone_depot_move_full(&cache->zc_depot, &zone->z_recirc, n, zone);
  }

  zone_recirc_unlock_nopreempt(zone);
}

static void zalloc_cached_reuse_smr(zone_t z, zone_cache_t cache,
                                    zone_magazine_t mag) {
  ...
  for (uint16_t i = 0; i < zc_mag_size(); i++) {
    ...
    elem = __zcache_mark_invalid(z, elem, ZFREE_PACK_SIZE(esize, esize));
    mag->zm_elems[i] = elem;
  }
}

static void zalloc_cached_recirculate(zone_t zone, zone_cache_t cache) {
  zone_magazine_t mag = NULL;
...
__attribute__((noinline)) static zone_cache_t
zalloc_cached_prime(zone_t zone, zone_cache_ops_t ops, zalloc_flags_t flags,
                    zone_cache_t cache) {
  zone_magazine_t mag = NULL;
  uint32_t depot_max;
  smr_t smr;
  ...
  if (depot_max) {
    ...
    if (!zone_depot_poll(&cache->zc_depot, smr)) {
      zalloc_cached_depot_recirculate(zone, depot_max, cache, smr);
    }
...
__attribute__((always_inline)) static inline zone_cache_t
zalloc_cached_get_pcpu_cache(zone_t zone, zone_cache_ops_t ops, int cpu,
                             zalloc_flags_t flags) {
  zone_cache_t cache = zpercpu_get_cpu(zone->z_pcpu_cache, cpu);
 ...
 *
 * @brief
 * The core implementation of @c zalloc(), @c zalloc_flags(), @c
 * zalloc_percpu().
 */
__mockable struct kalloc_result zalloc_ext(zone_t zone, zone_stats_t zstats,
                                           zalloc_flags_t flags) {
  /*
...
#endif /* KASAN_FAKESTACK */
              ml_get_interrupts_enabled() || ml_is_quiescing() ||
              debug_mode_active(),
  ...
  if (__improbable(zone->z_uses_tags)) {
    ...
    if (tag != VM_KERN_MEMORY_NONE) {
      tag = vm_tag_will_update_zone(
          tag, flags & (Z_WAITOK | Z_NOWAIT | Z_NOPAGEWAIT));
    }
    if (tag == VM_KERN_MEMORY_NONE) {
      zone_security_flags_t zsflags = zone_security_config(zone);
  ...
  __attribute__((musttail)) return zalloc_item(zone, zstats, flags);
}

__attribute__((always_inline)) static inline zstack_t
zcache_alloc_stack_from_cpu(zone_id_t zid, zone_cache_t cache, zstack_t stack,
                            uint32_t n, zone_cache_ops_t ops) {
  vm_offset_t *p;
...
__attribute__((noinline)) static zstack_t
...
__attribute__((noinline)) static void *
zcache_alloc_one(zone_id_t zid, zalloc_flags_t flags, zone_cache_ops_t ops) {
  zone_t zone = zone_by_id(zid);
  void *o;
...
__attribute__((always_inline)) static zstack_t
zcache_alloc_n_ext(zone_id_t zid, uint32_t count, zalloc_flags_t flags,
...
zstack_t zalloc_n(zone_id_t zid, uint32_t count, zalloc_flags_t flags) {
  return zcache_alloc_n_ext(zid, count, flags, NULL);
}

zstack_t(zcache_alloc_n)(zone_id_t zid, uint32_t count, zalloc_flags_t flags,
                         zone_cache_ops_t ops) {
  __builtin_assume(ops != NULL);
  return zcache_alloc_n_ext(zid, count, flags, ops);
}

__attribute__((always_inline)) void *zalloc(zone_t zov) {
  return zalloc_flags(zov, Z_WAITOK);
}

__attribute__((always_inline)) void *zalloc_noblock(zone_t zov) {
  return zalloc_flags(zov, Z_NOWAIT);
}

void *(zalloc_flags)(zone_t zov, zalloc_flags_t flags) {
  zone_t zone = zov->z_self;
  zone_stats_t zstats = zov->z_stats;
...
void *(zalloc_ro)(zone_id_t zid, zalloc_flags_t flags) {
  assert(zid >= ZONE_ID__FIRST_RO && zid <= ZONE_ID__LAST_RO);
  zone_t zone = zone_by_id(zid);
  zone_stats_t zstats = zone->z_stats;
...
#if ZSECURITY_CONFIG(READ_ONLY) &&                                             \
    !__BUILDING_XNU_LIBRARY__ /* zalloc mocks don't create ro memory */
  assert(zone_security_array[zid].z_submap_idx == Z_SUBMAP_IDX_READ_ONLY);
  if (kr.addr) {
...
__attribute__((always_inline)) static bool from_current_stack(vm_offset_t addr,
                                                              vm_size_t size) {
...
__abortlike static void zalloc_ro_mut_validation_panic(zone_id_t zid,
                                                       void *elem,
                                                       const vm_offset_t src,
                                                       vm_size_t src_size) {
  vm_offset_t stack_start = (vm_offset_t)__builtin_frame_address(0);
  vm_offset_t stack_end =
      (stack_start + kernel_stack_size - 1) & -kernel_stack_size;
#if defined(KERNEL_INTEGRITY_KTRR) || defined(KERNEL_INTEGRITY_CTRR) ||        \
    defined(KERNEL_INTEGRITY_PV_CTRR)
  ...
  if (from_ro_map(src, src_size)) {
    ...
    panic("zalloc_ro_mut failed: source (%p) not from same zone as dst (%p)"
          " (expected: %s, actual: %s",
          (void *)src, elem, src_zone->z_name, dst_zone->z_name);
  }

  panic("zalloc_ro_mut failed: source (%p, phys %p) not from RO zone map (%p - "
        "%p), "
        "current stack (%p - %p) or const memory (phys %p - %p)",
        (void *)src, (void *)kvtophys(src),
        (void *)zone_info.zi_ro_range.min_address,
        (void *)zone_info.zi_ro_range.max_address, (void *)stack_start,
        (void *)stack_end, (void *)rorgn_begin, (void *)rorgn_end);
}

__attribute__((always_inline)) static void
zalloc_ro_mut_validate_src(zone_id_t zid, void *elem, const vm_offset_t src,
                           vm_size_t src_size) {
  if (from_current_stack(src, src_size) ||
      (from_ro_map(src, src_size) && zid == zone_index_from_ptr((void *)src)) ||
...
__mockable __attribute__((noinline)) void
zalloc_ro_mut(zone_id_t zid, void *elem, vm_offset_t offset,
  ...
  if (skip_src_check == false) {
    zalloc_ro_mut_validate_src(zid, elem, (vm_offset_t)new_data, new_data_size);
  }
  pmap_ro_zone_memcpy(zid, (vm_offset_t)elem, offset, (vm_offset_t)new_data,
                      new_data_size);
#else
  (void)zid;
...
__abortlike static void zone_id_require_ro_panic(zone_id_t zid, void *addr) {
  ...
  if (!Z_FAST_ALIGNED(PAGE_SIZE - (elem & PAGE_MASK), p.z_align_magic)) {
    panic("zone_require_ro failed: element improperly aligned (addr: %p)",
          addr);
  }
...
void *(zalloc_percpu)(union zone_or_view zov, zalloc_flags_t flags) {
  zone_t zone = zov.zov_view->zv_zone;
  zone_stats_t zstats = zov.zov_view->zv_stats;
...
static void *_zalloc_permanent(zone_t zone, vm_size_t size, vm_offset_t mask) {
  struct zone_page_metadata *page_meta;
  vm_offset_t offs, addr;
  zone_pva_t pva;

  assert(ml_get_interrupts_enabled() || ml_is_quiescing() ||
         debug_mode_active() || startup_phase < STARTUP_SUB_EARLY_BOOT);
...
static void *_zalloc_permanent_large(size_t size, vm_offset_t mask,
                                     vm_tag_t tag) {
  vm_offset_t addr;
...
__mockable void *zalloc_permanent_tag(vm_size_t size, vm_offset_t mask,
                                      vm_tag_t tag) {
  if (size <= PAGE_SIZE) {
...
__mockable void *zalloc_percpu_permanent(vm_size_t size, vm_offset_t mask) {
...
static void zone_reclaim_chunk(zone_t z, struct zone_page_metadata *meta,
                               uint32_t free_count) {
  ...
  if (z->z_percpu) {
    for (uint32_t i = 0; i < z->z_chunk_pages; i++) {
      kasan_zmem_remove(page_addr + ptoa(i), PAGE_SIZE, zone_elem_outer_size(z),
                        zone_elem_outer_offs(z), zone_elem_redzone(z));
    }
  } else {
    kasan_zmem_remove(page_addr, size_to_free, zone_elem_outer_size(z),
                      zone_elem_outer_offs(z), zone_elem_redzone(z));
  }
#endif /* KASAN_CLASSIC */
  ...
  if (sequester) {
    ...
    kernel_memory_depopulate(page_addr, size_to_free, flags,
                             VM_KERN_MEMORY_ZONE);
  } else {
...
static void zcache_reclaim_elements(zone_id_t zid, uint16_t n,
                                    vm_offset_t *elems) {
  z_debug_assert(n <= zc_mag_size());
...
static void zone_reclaim_pcpu(zone_t z, zone_reclaim_mode_t mode,
                              struct zone_depot *zd) {
  ...
  if (mode == ZONE_RECLAIM_DESTROY) {
    zpercpu_foreach(zc, z->z_pcpu_cache) {
      zone_reclaim_elements(z, zc->zc_alloc_cur, zc->zc_alloc_elems);
      zone_reclaim_elements(z, zc->zc_free_cur, zc->zc_free_elems);
...
static void zone_reclaim_recirc_drain(zone_t z, struct zone_depot *zd) {
...
static void zone_reclaim_recirc_trim(zone_t z, struct zone_depot *zd) {
  for (;;) {
    uint64_t maxtime =
        mach_continuous_speculative_time() + zc_free_batch_timeout();
    uint32_t budget = zc_free_batch_size();
    uint32_t count;
    bool done = true;

    zone_recirc_lock_nopreempt(z);
    count = MIN(z->z_recirc_empty_wma / Z_WMA_UNIT, z->z_recirc_empty_min);
    ...
    zone_unlock(z);
    zone_lock(z);
    maxtime = mach_continuous_speculative_time() + zc_free_batch_timeout();
  }
}
 ...
 */
static void zone_reclaim(zone_t z, zone_reclaim_mode_t mode) {
  struct zone_depot zd;
  ...
  if (mode == ZONE_RECLAIM_DESTROY) {
    if (!z->z_destructible || z->z_elems_rsv) {
      panic("zdestroy: Zone %s%s isn't destructible", zone_heap_name(z),
            z->z_name);
    }

    if (!z->z_self || z->z_expander || z->z_async_refilling ||
        z->z_expanding_wait) {
  ...
  if (z->z_pcpu_cache) {
    ...
    if (z->z_chunk_elems) {
      ...
      while (zd.zd_full) {
        mag = zone_depot_pop_head_full(&zd, NULL);
        zcache_reclaim_elements(zid, zc_mag_size(), mag->zm_elems);
        zone_magazine_free(mag);
      }
  ...
  while (!zone_pva_is_null(z->z_pageq_empty)) {
    ...
    if (mode == ZONE_RECLAIM_TRIM && z->z_pcpu_cache == NULL) {
      limit =
          MAX(limit, z->z_elems_free - MIN(z->z_elems_free_min,
                                           z->z_elems_free_wma / Z_WMA_UNIT));
    }
...
void zcache_drain(zone_id_t zid) { zone_drain(zone_by_id(zid)); }

static void zone_reclaim_all(zone_reclaim_mode_t mode) {
  /*
   ...
   */
  for (zone_id_t zid = ZONE_ID__LAST_RO + 1; zid < ZONE_ID__FIRST_DYNAMIC;
       zid++) {
...
void zone_gc_trim(void) { zone_gc(ZONE_GC_TRIM); }

void zone_gc_drain(void) { zone_gc(ZONE_GC_DRAIN); }

static bool zone_trim_needed(zone_t z) {
  if (z->z_depot_cleanup) {
    return true;
  }
...
static void zone_trim_async(__unused thread_call_param_t p0,
                            __unused thread_call_param_t p1) {
  current_thread()->options |= TH_OPT_ZONE_PRIV;
...
void compute_zone_working_set_size(__unused void *param) {
...
static const char *panic_print_get_typename(kalloc_type_views_t cur,
                                            kalloc_type_views_t *next,
                                            bool is_kt_var) {
  if (is_kt_var) {
...
static void panic_print_types_in_zone(zone_t z, const char *debug_str) {
  ...
  if (zsflags.z_kheap_id == KHEAP_ID_KT_VAR) {
    uint32_t heap_id =
        KT_VAR_PTR_HEAP0 +
        ((zone_index(z) - kalloc_type_heap_array[KT_VAR_PTR_HEAP0].kh_zstart) /
         KHEAP_NUM_ZONES);
  ...
  paniclog_append_noflush("kalloc %s in zone, %s (%s):\n",
                          is_kt_var ? "type arrays" : "types", debug_str,
                          z->z_name);

  while (kt_cur.ktv_fixed) {
    kalloc_type_views_t kt_next = {};
    const char *typename =
        panic_print_get_typename(kt_cur, &kt_next, is_kt_var) + skip_over_site;
...
static void panic_display_kalloc_types(void) {
  if (kalloc_type_src_zone) {
    panic_print_types_in_zone(kalloc_type_src_zone, "addr belongs to");
  }
  if (kalloc_type_dst_zone) {
    panic_print_types_in_zone(kalloc_type_dst_zone, "addr is being freed to");
  }
}

static void zone_find_n_largest(const uint32_t n, zone_t *largest_zones,
                                uint64_t *zone_size) {
  zone_index_foreach(zid) {
...
static void panic_display_largest_zones(void) {
  ...
  for (uint32_t i = 0; i < NUM_LARGEST_ZONES; i++) {
    ...
    paniclog_append_noflush("%-8s%-20s %9u%c %9u%c\n", zone_heap_name(z),
                            z->z_name, mach_vm_size_pretty(largest_size[i]),
...
static void panic_display_zprint(void) {
  ...
  if (panic_kext_memory_info) {
    ...
    paniclog_append_noflush("\n%-5s %10s\n", "Kmod", "Size");
    for (uint32_t i = 0; i < panic_kext_memory_size / sizeof(mem_info[0]);
         i++) {
...
static void panic_display_zone_info(void) {
...
static void panic_display_zone_fault(vm_offset_t addr) {
  struct zone_page_metadata meta = {};
  ...
  if (ml_nofault_copy((vm_offset_t)zone_meta_from_addr(addr),
    ...
    if (meta.zm_chunk_len == ZM_PGZ_GUARD) {
      ...
      oob = true;
      size = zone_element_size((void *)addr, &z, false, &oob_offs);
    } else {
  ...
  if (oob) {
    paniclog_append_noflush("  Element : [%p, %p) of size %d\n",
                            ...
                            (void *)trunc_page(addr),
                            (uint32_t)(size - oob_offs));
  }
  paniclog_append_noflush("  Submap  : %s [%p; %p)\n",
                          zone_submaps_names[map_idx], (void *)map->min_offset,
                          (void *)map->max_offset);
  ...
  paniclog_append_noflush("  Metadata: zid:%d inl:%d cl:0x%x "
                          ...
                          meta.zm_index, meta.zm_inline_bitmap,
                          meta.zm_chunk_len, meta.zm_alloc_size, meta.zm_bitmap,
 ...
 */
static vm_map_copy_t create_vm_map_copy(vm_offset_t start_addr,
  ...
  if (used_size != total_size) {
    ...
    if (free_size >= PAGE_SIZE) {
      kmem_free(ipc_kernel_map, round_page(end_addr), free_size);
    }
...
static boolean_t get_zone_info(zone_t z, mach_zone_name_t *zn,
                               mach_zone_info_t *zi) {
  struct zone zcopy;
  vm_size_t cached = 0;
  ...
  if (zn != NULL) {
    ...
    snprintf(temp_zone_name, MAX_ZONE_NAME, "%s%s", zone_heap_name(z),
             z->z_name);
  ...
  if (zi != NULL) {
    *zi = (mach_zone_info_t){
        ...
        .mzi_cur_size =
            ptoa_64(zone_scale_for_percpu(&zcopy, zcopy.z_wired_cur)),
        // max_size for zprint is now high-watermark of pages used
        .mzi_max_size =
            ptoa_64(zone_scale_for_percpu(&zcopy, zcopy.z_wired_hwm)),
...
static kern_return_t mach_memory_info_internal(
    host_t host, mach_zone_name_array_t *namesp,
    mach_msg_type_number_t *namesCntp, mach_zone_info_array_t *infop,
    mach_msg_type_number_t *infoCntp, mach_memory_info_array_t *memoryInfop,
    mach_msg_type_number_t *memoryInfoCntp, bool redact_info);

static kern_return_t mach_memory_info_security_check(bool redact_info) {
  ...
  if (task != kernel_task &&
      !IOTaskHasEntitlement(task, MEMORYINFO_ENTITLEMENT)) {
    return KERN_DENIED;
  }
...
kern_return_t zone_reset_peak(const char *zonename) {
  ...
  for (unsigned int i = 0; i < max_zones; i++) {
    ...
    if (zone_name(z) && track_this_zone(zone_name(z), zonename)) {
      /* Found the matching zone */
      os_log_info(OS_LOG_DEFAULT, "zalloc: resetting peak size for zone %s\n",
                  zone_name(z));
      zone_lock(z);
...
kern_return_t mach_zone_info(mach_port_t host_port,
                             mach_zone_name_array_t *namesp,
                             mach_msg_type_number_t *namesCntp,
                             mach_zone_info_array_t *infop,
                             mach_msg_type_number_t *infoCntp) {
  return mach_memory_info(host_port, namesp, namesCntp, infop, infoCntp, NULL,
                          NULL);
}

kern_return_t mach_memory_info(mach_port_t host_port,
                               mach_zone_name_array_t *namesp,
                               mach_msg_type_number_t *namesCntp,
                               mach_zone_info_array_t *infop,
                               mach_msg_type_number_t *infoCntp,
  ...
  return mach_memory_info_internal(host, namesp, namesCntp, infop, infoCntp,
                                   memoryInfop, memoryInfoCntp, redact_info);
}

static void zone_info_redact(mach_zone_info_t *zi) {
  zi->mzi_cur_size = 0;
  zi->mzi_max_size = 0;
  zi->mzi_alloc_size = 0;
  zi->mzi_sum_size = 0;
...
static bool zone_info_needs_to_be_coalesced(int zone_index) {
...
static bool zone_info_find_coalesce_zone(mach_zone_info_t *zi,
                                         mach_zone_info_t *info, int *coalesce,
                                         int coalesce_count,
                                         int *coalesce_index) {
...
static void zone_info_coalesce(mach_zone_info_t *info, int coalesce_index,
                               mach_zone_info_t *zi) {
  info[coalesce_index].mzi_count += zi->mzi_count;
}

kern_return_t mach_memory_info_sample(mach_zone_name_t *names,
                                      mach_zone_info_t *info, int *coalesce,
  ...
  zone_index_foreach(i) {
    ...
    int coalesce_index;
    bool found_coalesce_zone = zone_info_find_coalesce_zone(
        zi, info, coalesce, coalesce_count, &coalesce_index);
  ...
  *zonesCnt = used_zones;
  ...
  if (memoryInfo) {
    ...
    kr = vm_page_diagnose(memoryInfo, memoryInfoCnt, zones_collectable_bytes,
                          redact_info);
    if (kr != KERN_SUCCESS) {
      return kr;
    }
...
static kern_return_t mach_memory_info_internal(
  ...
  kr = kmem_alloc(ipc_kernel_map, &info_addr, info_size,
                  KMA_PAGEABLE | KMA_DATA_SHARED, VM_KERN_MEMORY_IPC);
  if (kr != KERN_SUCCESS) {
    kmem_free(ipc_kernel_map, names_addr, names_size);
    return kr;
  }
  ...
  if (redact_info) {
    ...
    if (kr != KERN_SUCCESS) {
      kmem_free(ipc_kernel_map, names_addr, names_size);
      kmem_free(ipc_kernel_map, info_addr, info_size);
      return kr;
    }
  ...
  if (memoryInfop && memoryInfoCntp) {
    ...
    kr = vm_map_wire_kernel(
        ipc_kernel_map, memory_info_addr, memory_info_addr + memory_info_vmsize,
  ...
  mach_memory_info_sample(names, info, coalesce, &used_zones, memory_info,
                          num_info, redact_info);

  if (redact_info) {
    kmem_free(ipc_kernel_map, coalesce_addr, coalesce_size);
  }

  *namesp = (mach_zone_name_t *)create_vm_map_copy(names_addr, names_size,
                                                   used_zones * sizeof *names);
  *namesCntp = used_zones;

  *infop = (mach_zone_info_t *)create_vm_map_copy(info_addr, info_size,
                                                  used_zones * sizeof *info);
  *infoCntp = used_zones;

  if (memoryInfop && memoryInfoCntp) {
    kr = vm_map_unwire(ipc_kernel_map, memory_info_addr,
                       memory_info_addr + memory_info_vmsize, FALSE);
    assert(kr == KERN_SUCCESS);

    kr =
        vm_map_copyin(ipc_kernel_map, (vm_map_address_t)memory_info_addr,
...
kern_return_t mach_zone_info_for_zone(host_priv_t host, mach_zone_name_t name,
...
kern_return_t mach_zone_info_for_largest_zone(host_priv_t host,
...
uint64_t get_zones_collectable_bytes(void) {
  uint64_t zones_collectable_bytes = 0;
  mach_zone_info_t zi;
...
kern_return_t mach_zone_get_zlog_zones(host_priv_t host,
                                       mach_zone_name_array_t *namesp,
                                       mach_msg_type_number_t *namesCntp) {
#if ZALLOC_ENABLE_LOGGING
  ...
  *namesp = (mach_zone_name_t *)create_vm_map_copy(
      names_addr, names_size, logged_zones * sizeof *names);
...
kern_return_t mach_zone_get_btlog_records(host_priv_t host,
...
zone_t zone_find_largest(uint64_t *zone_size) {
...
void zone_get_stats(zone_t zone, struct zone_basic_stats *stats) {
  ...
  if (zone->z_pcpu_cache) {
    zpercpu_foreach(zc, zone->z_pcpu_cache) {
      stats->zbs_cached += zc->zc_alloc_cur + zc->zc_free_cur +
   ...
   */
  if (os_sub_overflow(stats->zbs_avail, stats->zbs_free, &stats->zbs_alloc)) {
...
void zone_set_exhaustible(zone_t zone, vm_size_t nelems,
...
static zone_t zone_create_find(const char *name, vm_size_t size,
  ...
  if (zid != ZONE_ID_ANY) {
    ...
    if (flags & ZC_DESTRUCTIBLE) {
       ...
       */
      for (int i = bitmap_first(zone_destroyed_bitmap, MAX_ZONES); i >= 0;
           i = bitmap_next(zone_destroyed_bitmap, i)) {
        z = &zone_array[i];
         ...
         */
        if (strcmp(z->z_name, name) || zone_elem_outer_size(z) != size) {
          continue;
        }
...
__abortlike static void zone_create_panic(const char *name, const char *f1,
                                          const char *f2) {
  panic("zone_create: creating zone %s: flag %s and %s are incompatible", name,
        f1, f2);
}
...
__mockable void zone_enable_smr(zone_t zone, struct smr *smr,
...
vm_size_t zone_get_elem_size(zone_t zone) { return zone->z_elem_size; }

static_assert(ZONE_ID__LAST_RO_EXT - ZONE_ID__FIRST_RO_EXT == ZC_RO_ID__LAST);

zone_id_t zone_create_ro(const char *name, vm_size_t size,
                         zone_create_flags_t flags,
 ...
 *	before zone_init.
 */
__startup_func void zone_bootstrap(void) {
#if DEBUG || DEVELOPMENT
#if __x86_64__
   ...
   */
  zpercpu_foreach_cpu(cpu) { random_bool_init(&zone_bool_gen[cpu].zbg_bg); }
  ...
  for (zone_id_t i = 1; i < MAX_ZONES; i++) {
    uint32_t r =
        zalloc_random_uniform32(0, ZSECURITY_CONFIG_GENERAL_SUBMAPS * 2);
  ...
  thread_call_setup_with_options(&zone_expand_callout, zone_expand_async, NULL,
                                 THREAD_CALL_PRIORITY_HIGH,
                                 THREAD_CALL_OPTIONS_ONCE);

  thread_call_setup_with_options(&zone_trim_callout, zone_trim_async, NULL,
                                 THREAD_CALL_PRIORITY_USER,
                                 THREAD_CALL_OPTIONS_ONCE);
}
...
__startup_func static void zone_tunables_fixup(void) {
  int wdt = 0;
...
__pure2 __startup_func static vm_map_size_t
...
__startup_func static void zone_submap_init(mach_vm_offset_t *submap_min,
  ...
  submap_usable_size = zone_sub_map_numer * *remaining_size / *remaining_denom;
  ...
  submap_actual_size = submap_usable_size + left_guard_size + right_guard_size;
  ...
  if (idx == Z_SUBMAP_IDX_VM) {
    vm_packing_verify_range("vm_compressor", submap_start, submap_end,
                            VM_PACKING_PARAMS(C_SLOT_PACKED_PTR));
    vm_packing_verify_range("vm_page", submap_start, submap_end,
  ...
  vm_map_kernel_flags_t vmk_flags =
      VM_MAP_KERNEL_FLAGS_FIXED_PERMANENT(.vmkf_no_soft_limit = true,
                                          .vm_tag = VM_KERN_MEMORY_ZONE);
  vm_object_t kobject = kernel_object_default;

  kr = vm_map_enter(submap, &addr, left_guard_size, 0, vmk_flags, kobject, addr,
                    FALSE, prot, prot_max, VM_INHERIT_NONE);
  if (kr != KERN_SUCCESS) {
  ...
  kr = vm_map_enter(submap, &addr, right_guard_size, 0, vmk_flags, kobject,
                    addr, FALSE, prot, prot_max, VM_INHERIT_NONE);
  if (kr != KERN_SUCCESS) {
...
static inline void zone_pva_relocate(zone_pva_t *pva, uint32_t delta) {
 ...
 */
__startup_func static void zone_metadata_init(void) {
  ...
  zone_info.zi_meta_base = VM_FAR_ADD_PTR_UNBOUNDED(
      ...
      -(ptrdiff_t)zone_pva_from_addr(zone_map_range.min_address)
           .packed_address);
  ...
  kernel_memory_populate(reloc_base, early_sz, flags, VM_KERN_MEMORY_OSFMK);
...
#if KASAN_CLASSIC
      kasan_alloc(addr, zone_elem_inner_size(z), zone_elem_inner_size(z),
                  zone_elem_redzone(z), false, __builtin_frame_address(0));
#endif
...
__startup_data static uint16_t submap_ratios[Z_SUBMAP_IDX_COUNT] = {
#if ZSECURITY_CONFIG(READ_ONLY)
    [Z_SUBMAP_IDX_VM] = 15,        [Z_SUBMAP_IDX_READ_ONLY] = 5,
#else
    [Z_SUBMAP_IDX_VM] = 20,
#endif /* !ZSECURITY_CONFIG(READ_ONLY) */
#if ZSECURITY_CONFIG(SAD_FENG_SHUI)
    [Z_SUBMAP_IDX_GENERAL_0] = 15, [Z_SUBMAP_IDX_GENERAL_1] = 15,
    [Z_SUBMAP_IDX_GENERAL_2] = 15, [Z_SUBMAP_IDX_GENERAL_3] = 15,
...
__startup_func static inline uint16_t zone_submap_ratios_denom(void) {
  uint16_t denom = 0;
...
__startup_func static inline vm_offset_t zone_restricted_va_max(void) {
...
__startup_func static void zone_set_map_sizes(void) {
  vm_size_t zsize;
  vm_size_t zsizearg;
  ...
  if (zsizearg == 0 && zsize > ZONE_MAP_MAX) {
    /* if zsize boot-arg not present and zsize exceeds platform maximum, clip
     * zsize */
   ...
   */
  vm_offset_t vm_submap_size =
      round_page((submap_ratios[Z_SUBMAP_IDX_VM] * ZONE_MAP_VA_SIZE) /
  ...
  zone_meta_size =
      round_page(atop(ZONE_MAP_VA_SIZE) * sizeof(struct zone_page_metadata)) +
      ZONE_GUARD_SIZE * 2;

  static_assert(ZONE_MAP_MAX / (CHAR_BIT * KALLOC_MINSIZE) <= ZBA_PTR_MASK + 1);
  zone_bits_size =
      round_page(ptoa(zone_pages_wired_max) / (CHAR_BIT * KALLOC_MINSIZE));
 ...
 */
__startup_func static void zone_init(void) {
...
#ifndef __BUILDING_XNU_LIB_UNITTEST__ /* zone submap is not maintained in      \
                                         unit-test */
  /*
  ...
  for (zone_submap_idx_t idx = 0; idx < Z_SUBMAP_IDX_COUNT; idx++) {
    if (submap_ratios[idx] == 0) {
      ...
      zone_submap_init(&submap_min, idx, submap_ratios[idx], &denom,
                       &remaining_size);
    }
  }
...
#pragma unused(denom, remaining_size)
  ...
  (void)zone_create_ext("vm.permanent.percpu", 1,
                        kma_flags | ZC_PERCPU | ZC_NO_TBI_TAG,
                        ZONE_ID_PERCPU_PERMANENT, ^(zone_t z) {
                          z->z_permanent = true;
                          z->z_elem_size = 1;
                        });

  zc_magazine_zone = zone_create("zcc_magazine_zone",
                                 sizeof(struct zone_magazine) +
...
void zalloc_iokit_lockdown(void) { zone_share_always = false; }

void zalloc_first_proc_made(void) {
...
__startup_func vm_offset_t zone_early_mem_init(vm_size_t size) {
  ...
  zone_info.zi_bits_range = (struct mach_vm_range){
      ...
      .max_address =
          (mach_vm_offset_t)zba_chunk_startup + sizeof(zba_chunk_startup),
  };
...
static uintptr_t *zone_copy_allocations(zone_t z, uintptr_t *elems,
  ...
  while (!zone_pva_is_null(page_index)) {
    ...
    if (meta->zm_inline_bitmap) {
      for (size_t i = 0; i < meta->zm_chunk_len; i++) {
        ...
        for (; map; map &= map - 1) {
          *elems++ = INSTANCE_PUT(base + elem_size * __builtin_clz(map));
        }
        base += elem_size * 32;
      }
    } else {
      ...
      for (size_t i = 0; i < (1u << order); i++) {
        ...
        for (; map; map &= map - 1) {
          *elems++ = INSTANCE_PUT(base + elem_size * __builtin_clzll(map));
        }
        base += elem_size * 64;
      }
    }
...
kern_return_t zone_leaks(const char *zoneName, uint32_t nameLen,
                         leak_site_proc proc) {
  zone_t zone = NULL;
  uintptr_t *array;
  uintptr_t *next;
  ...
  if (zone->z_btlog && !corruption_debug_flag) {
    // btlog_copy_backtraces_for_elements will set kInstanceFlagReferenced on
    // elements it found
    static_assert(sizeof(vm_address_t) == sizeof(uintptr_t));
    btlog_copy_backtraces_for_elements(zone->z_btlog, (vm_address_t *)array,
                                       &count, elemSize, proc);
  }
#endif /* ZALLOC_ENABLE_LOGGING */
...
static int zone_ro_basic_test_run(__unused int64_t in, int64_t *out) {
  zone_security_flags_t zsflags;
  uint32_t x = 4;
  uint32_t *test_ptr;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    printf("zone_ro_basic_test: verify we can't write to it after assigning "
           "value\n");
    assert(verify_write(&x, test_ptr, sizeof(x)) == EFAULT);
    ...
    assert(!test_ptr);
#else
    printf("zone_ro_basic_test: Read-only allocator n/a on 32bit platforms, "
...
static int zone_basic_test_run(__unused int64_t in, int64_t *out) {
  ...
  printf("zone_basic_test: Testing zinit(), zalloc(), zfree() and zdestroy() "
         "on zone \"test_zone_sysctl\"\n");

  /* zinit() and zdestroy() a zone with the same name a bunch of times, verify
   * that we get back the same zone each time */
  do {
    test_zone = zinit(sizeof(uint64_t), 100 * sizeof(uint64_t),
                      sizeof(uint64_t), "test_zone_sysctl");
    assert(test_zone);
    ...
    if (test_zone_ptr == NULL) {
      ...
      printf("zone_basic_test: old zone pointer and new zone pointer don't "
             "match\n");
      rc = EIO;
      goto out;
    }
  ...
  /* test Z_VA_SEQUESTER */
  {
    ...
    test_zone = zone_create("test_zone_sysctl", elem_size, ZC_DESTRUCTIBLE);
    assert(test_zone);
    ...
    for (idx = 0; idx < PAGE_SIZE / sizeof(uint64_t); idx++) {
      allocs_pcpu[idx] = zalloc_percpu(test_pcpu_zone, Z_WAITOK | Z_ZERO);
    ...
    zone_foreach(z) {
      ...
      printf("%s%s: seq %d, res %d, %qd %%\n", zone_heap_name(z), z->z_name,
             z->z_va_cur - z->z_wired_cur, z->z_wired_cur,
             zone_size_allocated(z) * 100ULL / size);
      zone_unlock(z);
    }
    ...
    for (idx = 0; idx < num_allocs; idx++) {
      assert(0 ==
             pmap_find_phys(kernel_pmap, (addr64_t)(uintptr_t)allocs[idx]));
    }
...
static int run_kalloc_guard_insertion_test(int64_t in __unused, int64_t *out) {
  ...
  *out = 0;
  ...
  for (uint i = 0; i < N_ALLOCATIONS; ++i) {
    uint64_t *data_ptr =
        kalloc_ext(KHEAP_DATA_BUFFERS, alloc_size, flags, &data_ptr).addr;
    if (!data_ptr) {
    ...
    if (meta->zm_guarded) {
      ...
      if (meta->zm_chunk_len == chunk_pages) {
        ...
        printf("%s: kalloc_ext gave us address 0x%lx for a guard region.\n",
               __func__, addr);
        goto cleanup;
      } else if ((meta->zm_chunk_len == ZM_SECONDARY_PCPU_PAGE) &&
                 !z->z_percpu) {
        printf("%s: zone [%s%s] is not per-CPU.\n", __func__, zone_heap_name(z),
               zone_name(z));
        goto cleanup;
      } else {
...
static void zone_stress_worker(void *arg, wait_result_t __unused wr) {
  struct zone_stress_ctx *ctx = arg;
  bool leader = ctx->zsc_leader == current_thread();
  TAILQ_HEAD(zone_stress_head, zone_stress_obj)
  ...
  do {
    for (int i = 0; i < 2000; i++) {
      uint32_t what =
          random_bool_gen_bits(&bg.zbg_bg, bg.zbg_entropy, ZONE_ENTROPY_CNT, 1);
      switch (what) {
      case 0:
      case 1:
        if (allocs < 10000) {
  ...
  if (--ctx->zsc_workers == 0) {
    ...
    while (ctx->zsc_workers) {
      lck_mtx_sleep(&ctx->zsc_lock, LCK_SLEEP_DEFAULT, ctx, THREAD_UNINT);
    }
  }
...
static int zone_stress_test_run(__unused int64_t in, int64_t *out) {
  ...
  ctx.zsc_zone =
      zone_create("test_zone_344", 344, ZC_DESTRUCTIBLE | ZC_NOCACHING);
  ...
  for (uint32_t i = 1; i < ctx.zsc_workers; i++) {
    kr = kernel_thread_start_priority(zone_stress_worker, &ctx, BASEPRI_DEFAULT,
                                      &th);
    if (kr == KERN_SUCCESS) {
...
static void zone_gc_stress_test_alloc_n(struct zone_gc_stress_head *head,
                                        size_t n) {
...
static void zone_gc_stress_test_free_n(struct zone_gc_stress_head *head) {
  struct zone_gc_stress_obj *obj;
...
__dead2 static void zone_gc_stress_worker(void *arg,
...
static int zone_gc_stress_test_run(__unused int64_t in, int64_t *out) {
  struct zone_gc_stress_head head = STAILQ_HEAD_INITIALIZER(head);
  ...
  for (uint64_t i = 0; i < in; i++) {
    ...
    lck_mtx_lock(&zone_gc_lock);
    zone_reclaim(zone_gc_stress_zone->kt_zv.zv_zone, ZONE_RECLAIM_TRIM);
    lck_mtx_unlock(&zone_gc_lock);
  ...
  lck_mtx_lock(&zone_gc_lock);
  zone_reclaim(zone_gc_stress_zone->kt_zv.zv_zone, ZONE_RECLAIM_DRAIN);
  lck_mtx_unlock(&zone_gc_lock);
...
static int zone_gc_replenish_test(__unused int64_t in, int64_t *out) {
  zone_gc(ZONE_GC_DRAIN);
  *out = 1;
  return 0;
}
...
static int zone_alloc_replenish_test(__unused int64_t in, int64_t *out) {
