...
void *get_bsdtask_info(task_t t) {
  ...
  proc_require(proc_from_task,
               PROC_REQUIRE_ALLOW_NULL | PROC_REQUIRE_ALLOW_ALL);
...
__abortlike static void __thread_ro_circularity_panic(thread_t th,
                                                      thread_ro_t tro) {
  panic("tro %p points back to %p instead of %p", tro, tro->tro_owner, th);
}

__attribute__((always_inline)) thread_ro_t
get_thread_ro_unchecked(thread_t th) {
  return th->t_tro;
}

thread_ro_t get_thread_ro(thread_t th) {
...
__attribute__((always_inline)) thread_ro_t current_thread_ro_unchecked(void) {
  return get_thread_ro_unchecked(current_thread());
}

thread_ro_t current_thread_ro(void) { return get_thread_ro(current_thread()); }

void clear_thread_ro_proc(thread_t th) {
...
thread_t get_machthread(struct uthread *uth) {
 ...
 */
void set_thread_pagein_error(thread_t th, int error) {
 ...
 */
int thread_task_has_ldt(thread_t th) {
...
kern_return_t get_signalact(task_t task, thread_t *result_out, int setast) {
...
kern_return_t check_actforsig(task_t task, thread_t thread, int setast) {
...
ledger_t get_task_ledger(task_t t) { return t->ledger; }
 ...
 */
vm_map_t get_task_map(task_t t) { return t->map; }
...
int get_task_numacts(task_t t) { return t->thread_count; }
 ...
 */
vm_map_t swap_task_map(task_t task, thread_t thread, vm_map_t map) {
  vm_map_t old_map;
  ...
  /* verify that the map has been activated if the task is enabled for IPC
   * access and is not a corpse */
 ...
 * we go to return it here.
 */
pmap_t get_task_pmap(task_t t) { return t->map->pmap; }
 ...
 */
static uint64_t get_task_ledger_balance(task_t task, int entry) {
...
uint64_t get_task_purgeable_size(task_t task) {
  ...
  ret = ledger_get_balance(
      task->ledger, task_ledgers.purgeable_volatile_compressed, &balance);
  if (ret != KERN_SUCCESS) {
    return 0;
  }
...
uint64_t get_task_phys_footprint_interval_max(task_t task, int reset) {
  ...
  ret = ledger_get_interval_max(task->ledger, task_ledgers.phys_footprint, &max,
                                reset);
...
uint64_t get_task_phys_footprint_lifetime_max(task_t task) {
  ...
  ret =
      ledger_get_lifetime_max(task->ledger, task_ledgers.phys_footprint, &max);
...
uint64_t get_task_purgeable_nonvolatile_compressed(task_t task) {
  return get_task_ledger_balance(task,
...
#if CONFIG_FREEZE
uint64_t get_task_frozen_to_swap(task_t task) {
...
uint64_t get_task_tagged_footprint(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(task->ledger, task_ledgers.tagged_footprint, &credit,
                           &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
uint64_t get_task_tagged_footprint_compressed(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(
      task->ledger, task_ledgers.tagged_footprint_compressed, &credit, &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
uint64_t get_task_media_footprint(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(task->ledger, task_ledgers.media_footprint, &credit,
                           &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
uint64_t get_task_media_footprint_compressed(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(
      task->ledger, task_ledgers.media_footprint_compressed, &credit, &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
uint64_t get_task_graphics_footprint(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(task->ledger, task_ledgers.graphics_footprint,
                           &credit, &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
uint64_t get_task_graphics_footprint_compressed(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(task->ledger,
                           task_ledgers.graphics_footprint_compressed, &credit,
                           &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
uint64_t get_task_neural_footprint(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(task->ledger, task_ledgers.neural_footprint, &credit,
                           &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
uint64_t get_task_neural_footprint_compressed(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(
      task->ledger, task_ledgers.neural_footprint_compressed, &credit, &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
uint64_t get_task_neural_nofootprint_total(task_t task) {
  kern_return_t ret;
  ledger_amount_t credit, debit;

  ret = ledger_get_entries(task->ledger, task_ledgers.neural_nofootprint_total,
                           &credit, &debit);
  if (KERN_SUCCESS == ret) {
    return credit - debit;
  }
...
#if CONFIG_LEDGER_INTERVAL_MAX
uint64_t get_task_neural_nofootprint_total_interval_max(task_t task,
                                                        int reset) {
  kern_return_t ret;
  ledger_amount_t max;

  ret = ledger_get_interval_max(
      task->ledger, task_ledgers.neural_nofootprint_total, &max, reset);
...
uint64_t get_task_neural_nofootprint_total_lifetime_max(task_t task) {
  kern_return_t ret;
  ledger_amount_t max;

  ret = ledger_get_lifetime_max(task->ledger,
                                task_ledgers.neural_nofootprint_total, &max);
...
uint32_t get_task_loadTag(task_t task) {
  return os_atomic_load(&task->loadTag, relaxed);
}

uint32_t set_task_loadTag(task_t task, uint32_t loadTag) {
  return os_atomic_xchg(&task->loadTag, loadTag, relaxed);
}

task_t get_threadtask(thread_t th) { return get_thread_ro(th)->tro_task; }

task_t get_threadtask_early(thread_t th) {
 ...
 *
 */
vm_map_offset_t get_map_min(vm_map_t map) { return vm_map_min(map); }

/*
 *
 */
vm_map_offset_t get_map_max(vm_map_t map) { return vm_map_max(map); }
vm_map_size_t get_vmmap_size(vm_map_t map) { return vm_map_adjusted_size(map); }
int get_task_page_size(task_t task) { return vm_map_page_size(task->map); }

#if CONFIG_COREDUMP

static int get_vmsubmap_entries(vm_map_t map, vm_object_offset_t start,
  ...
  while ((entry != vm_map_to_entry(map)) && (entry->vme_start < end)) {
    if (entry->is_sub_map) {
      total_entries += get_vmsubmap_entries(
          VME_SUBMAP(entry), VME_OFFSET(entry),
          (VME_OFFSET(entry) + entry->vme_end - entry->vme_start));
    } else {
      total_entries += 1;
    }
    entry = entry->vme_next;
...
int get_vmmap_entries(vm_map_t map) {
  int total_entries = 0;
  vm_map_entry_t entry;
...
int get_task_userstop(task_t task) { return task->user_stop_count; }

int get_thread_userstop(thread_t th) { return th->user_stop_count; }

boolean_t get_task_pidsuspended(task_t task) { return task->pidsuspended; }

boolean_t get_task_frozen(task_t task) { return task->frozen; }

boolean_t thread_should_abort(thread_t th) {
...
void task_act_iterate_wth_args(task_t task,
...
void fill_taskprocinfo(task_t task, struct proc_taskinfo_internal *ptinfo) {
  ...
  ptinfo->pti_messages_sent =
      (int32_t)MIN(counter_load(&task->messages_sent), INT32_MAX);
  ptinfo->pti_messages_received =
      (int32_t)MIN(counter_load(&task->messages_received), INT32_MAX);
  ptinfo->pti_syscalls_mach =
      (int32_t)MIN(task->syscalls_mach + syscalls_mach, INT32_MAX);
  ptinfo->pti_syscalls_unix =
      (int32_t)MIN(task->syscalls_unix + syscalls_unix, INT32_MAX);
...
int fill_taskthreadinfo(task_t task, uint64_t thaddr, bool thuniqueid,
                        struct proc_threadinfo_internal *ptinfo, void *vpp,
                        int *vidp) {
  thread_t thact;
  int err = 0;
...
int fill_taskthreadschedinfo(
    task_t task, uint64_t thread_id,
  ...
  absolutetime_to_nanoseconds(thread->machine.int_time_mt,
                              &thread_sched_info->int_time_ns);
#else
  (void)thread;
...
int get_numthreads(task_t task) { return task->thread_count; }
...
void fill_task_billed_usage(task_t task __unused, rusage_info_current *ri) {
  bank_billed_balance_safe(task, &ri->ri_billed_system_time,
                           &ri->ri_billed_energy);
  bank_serviced_balance_safe(task, &ri->ri_serviced_system_time,
                             &ri->ri_serviced_energy);
}
...
int fill_task_qos_rusage(task_t task, rusage_info_current *ri) {
  ...
  ri->ri_cpu_time_qos_maintenance =
      task->cpu_time_eqos_stats.cpu_time_qos_maintenance;
  ri->ri_cpu_time_qos_background =
  ...
  ri->ri_cpu_time_qos_user_initiated =
...
uint64_t get_task_dispatchqueue_serialno_offset(task_t task) {
...
uint64_t get_task_dispatchqueue_label_offset(task_t task) {
...
uint64_t get_task_uniqueid(task_t task) {
  void *bsd_info = get_bsdtask_info(task);
...
int get_task_version(task_t task) {
  void *bsd_info = get_bsdtask_info(task);
...
#if CONFIG_MACF
struct label *get_task_crash_label(task_t task) { return task->crash_label; }

void set_task_crash_label(task_t task, struct label *label) {
  task->crash_label = label;
}
#endif

int fill_taskipctableinfo(task_t task, uint32_t *table_size,
                          uint32_t *table_free) {
...
int get_task_cdhash(task_t task, char cdhash[static CS_CDHASH_LEN]) {
  int result = 0;
  void *bsd_info = NULL;
...
bool current_thread_in_kernel_fault(void) {
