 ...
 * 2) Add a #define for it to mach/sfi_class.h. It need not be inserted in order
 * of restrictiveness. 3) Add a call to SFI_CLASS_REGISTER below 4) Augment
 * sfi_thread_classify to categorize threads as early as possible for as
 * restrictive as possible. 5) Modify thermald to use the SFI class
 */
...
#define SFI_CLASS_REGISTER(clsid, ledger_name)                                 \
  ...
  SFI_##clsid##_THREAD_IS_WAITING(void *arg __unused,                          \
                                  wait_result_t wret __unused) {               \
    _sfi_wait_cleanup();                                                       \
  ...
  STARTUP_ARG(TUNABLES, STARTUP_RANK_MIDDLE, sfi_class_register,               \
              &SFI_##clsid##_registration)
...
static void sfi_timer_global_off(timer_call_param_t param0,
                                 timer_call_param_t param1);

static void sfi_timer_per_class_on(timer_call_param_t param0,
                                   timer_call_param_t param1);

/* Called early in boot, when kernel is single-threaded */
__startup_func static void sfi_class_register(sfi_class_registration_t *reg) {
  ...
  for (i = 0; i < MAX_SFI_CLASS_ID; i++) {
    ...
    if (sfi_classes[i].continuation) {
      sfi_classes[i].on_timer = thread_call_allocate_with_options(
          sfi_timer_per_class_on, (void *)(uintptr_t)i,
          THREAD_CALL_PRIORITY_HIGH, THREAD_CALL_OPTIONS_ONCE);
...
/* Can be called before sfi_init() by task initialization, but after
 * sfi_early_init() */
sfi_class_id_t sfi_get_ledger_alias_for_class(sfi_class_id_t class_id) {
  sfi_class_id_t i;
  ...
  /* We are permissive on SFI class lookup failures. In sfi_init(), we assert
   * more */
  return SFI_CLASS_UNSPECIFIED;
}

int sfi_ledger_entry_add(ledger_template_t template, sfi_class_id_t class_id) {
  const char *ledger_name = NULL;
...
static void sfi_timer_global_off(timer_call_param_t param0 __unused,
  ...
  if (!sfi_is_enabled) {
    ...
    KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_OFF_TIMER) |
                              DBG_FUNC_NONE,
                          1, 0, 0, 0, 0);
  ...
  /* First set all configured classes into the off state, and program their "on"
   * timer */
  for (i = 0; i < MAX_SFI_CLASS_ID; i++) {
    if (sfi_classes[i].class_sfi_is_enabled) {
      ...
      thread_call_enter_delayed_with_leeway(sfi_classes[i].on_timer, NULL,
                                            on_timer_deadline, 0,
                                            THREAD_CALL_DELAY_SYS_CRITICAL);
    } else {
  ...
  /* Iterate over processors, call cause_ast_check() on ones running a thread
   * that should be in an off phase */
  ...
  for (int cpuid = lsb_first(needs_cause_ast_mask); cpuid >= 0;
       cpuid = lsb_next(needs_cause_ast_mask, cpuid)) {
  ...
  simple_lock(&sfi_lock, LCK_GRP_NULL);
  if (sfi_is_enabled) {
    clock_deadline_for_periodic_event(sfi_window_interval, now,
                                      &sfi_next_off_deadline);
    timer_call_enter1(&sfi_timer_call_entry, NULL, sfi_next_off_deadline,
                      TIMER_CALL_SYS_CRITICAL);
  }

  KERNEL_DEBUG_CONSTANT(
      MACHDBG_CODE(DBG_MACH_SFI, SFI_OFF_TIMER) | DBG_FUNC_END, 0, 0, 0, 0, 0);
...
static void sfi_timer_per_class_on(timer_call_param_t param0,
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_ON_TIMER) |
                            DBG_FUNC_START,
                        sfi_class_id, 0, 0, 0, 0);

  /*
   * Any threads that may have accumulated in the ready queue for this class
   * should get re-enqueued. Since we have the sfi_lock held and have changed
   * "class_in_on_phase", we expect no new threads to be put on this wait queue
   * until the global "off timer" has fired.
   */
  ...
  kret = waitq_wakeup64_all(&sfi_class->waitq, CAST_EVENT64_T(sfi_class_id),
                            THREAD_AWAKENED, waitq_flags_splx(s));
  assert(kret == KERN_SUCCESS || kret == KERN_NOT_WAITING);

  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_ON_TIMER) | DBG_FUNC_END,
                        0, 0, 0, 0, 0);
}
...
kern_return_t sfi_set_window(uint64_t window_usecs) {
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_SET_WINDOW),
                        window_usecs, 0, 0, 0, 0);

  clock_interval_to_absolutetime_interval((uint32_t)window_usecs, NSEC_PER_USEC,
                                          &interval);
  ...
  for (i = 0; i < MAX_SFI_CLASS_ID; i++) {
    if (sfi_classes[i].class_sfi_is_enabled) {
      largest_class_off_interval =
          MAX(largest_class_off_interval, sfi_classes[i].off_time_interval);
    }
  }
   ...
   * just let the current one expire (and the new cadence will be established
   * thereafter). If the new "off" deadline is nearer than the current one,
   * bring it in, so we can start the new behavior sooner. Note that this may
   * cause the "off" timer to fire before some of the class "on" timers have
   * fired.
   */
...
kern_return_t sfi_window_cancel(void) {
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_CANCEL_WINDOW), 0, 0, 0,
                        0, 0);
 ...
 * an effective SFI duty cycle that is scaled by the GFI duty cycle. Initial
 * phase alignment and congruency of the SFI/GFI periods can distort this to
 * some extent.
 */
...
kern_return_t sfi_defer(uint64_t sfi_defer_matus) {
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_GLOBAL_DEFER),
                        sfi_defer_matus, 0, 0, 0, 0);
  ...
  for (int i = 0; i < MAX_SFI_CLASS_ID; i++) {
    if (sfi_classes[i].class_sfi_is_enabled) {
      if (sfi_classes[i].on_timer_programmed) {
        uint64_t new_on_deadline =
            sfi_classes[i].on_timer_deadline + sfi_defer_matus;
...
kern_return_t sfi_set_class_offtime(sfi_class_id_t class_id,
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_SET_CLASS_OFFTIME),
                        offtime_usecs, class_id, 0, 0, 0);

  clock_interval_to_absolutetime_interval((uint32_t)offtime_usecs,
                                          NSEC_PER_USEC, &interval);
  ...
  /* We never re-program the per-class on-timer, but rather just let it expire
   * naturally */
...
kern_return_t sfi_class_offtime_cancel(sfi_class_id_t class_id) {
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_CANCEL_CLASS_OFFTIME),
                        class_id, 0, 0, 0, 0);
...
kern_return_t sfi_get_class_offtime(sfi_class_id_t class_id,
                                    uint64_t *offtime_usecs) {
...
/*
 * sfi_thread_classify and sfi_processor_active_thread_classify perform the
 * critical role of quickly categorizing a thread into its SFI class so that an
 * AST_SFI can be set. As the thread is unwinding to userspace, sfi_ast()
 * performs full locking and determines whether the thread should enter an SFI
 * wait state. Because of the inherent races between the time the AST is set and
 * when it is evaluated, thread classification can be inaccurate (but should
 * always be safe). This is especially the case for
 * sfi_processor_active_thread_classify, which must classify the active thread
 * on a remote processor without taking the thread lock. When in doubt,
 * classification should err on the side of *not* classifying a thread at all,
 * and wait for the thread itself to either hit a quantum expiration or block
 * inside the kernel.
 */
...
sfi_class_id_t sfi_thread_classify(thread_t thread) {
  ...
  /* no need to re-classify threads unless there is at least one enabled SFI
   * class */
  if (os_atomic_load(&sfi_enabled_class_count, relaxed) == 0) {
  ...
  int managed_task =
      proc_get_effective_task_policy(task, TASK_POLICY_SFI_MANAGED);
  int runaway_bg =
      proc_get_effective_task_policy(task, TASK_POLICY_RUNAWAY_MITIGATION);

  int thread_qos = proc_get_effective_thread_policy(thread, TASK_POLICY_QOS);
  int thread_bg =
      proc_get_effective_thread_policy(thread, TASK_POLICY_DARWIN_BG);
   ...
   */
  if (thmode == TH_MODE_REALTIME || thmode == TH_MODE_FIXED ||
      task_role == TASK_GRAPHICS_SERVER) {
    return SFI_CLASS_OPTED_OUT;
  }

  /*
   * Threads with unspecified, legacy, or user-initiated QOS class can be
...
static inline void _sfi_wait_cleanup(void) {
  ...
  if (self->wait_sfi_begin_time != 0) {
    uint64_t made_runnable =
        os_atomic_load(&self->last_made_runnable_time, relaxed);
...
void sfi_ast(thread_t thread) {
  ...
  /*
   * Once the sfi_lock is taken and the thread's ->sfi_class field is updated,
   * we are committed to transitioning to whatever state is indicated by
   * "->class_in_on_phase". If another thread tries to call sfi_reevaluate()
   * after this point, it will take the sfi_lock and see the thread in this wait
   * state. If another thread calls sfi_reevaluate() before this point, it would
   * see a runnable thread and at most attempt to send an AST to this processor,
   * but we would have the most accurate classification.
   */
  ...
  if (!sfi_class->class_in_on_phase) {
    ...
    waitret =
        waitq_assert_wait64(&sfi_class->waitq, CAST_EVENT64_T(class_id),
...
void sfi_reevaluate(thread_t thread) {
  ...
  if ((current_class_id = thread->sfi_wait_class) != SFI_CLASS_UNSPECIFIED) {
    ...
    if ((sfi_ast == AST_NONE) || (class_id != current_class_id)) {
      ...
      KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_WAIT_CANCELED),
                            thread_tid(thread), current_class_id, class_id, 0,
                            0);

      kret = waitq_wakeup64_thread(&sfi_class->waitq,
                                   CAST_EVENT64_T(current_class_id), thread,
                                   THREAD_AWAKENED);
      assert(kret == KERN_SUCCESS || kret == KERN_NOT_WAITING);
    }
  } else {
...
kern_return_t sfi_window_cancel(void) { return KERN_NOT_SUPPORTED; }

kern_return_t sfi_get_window(uint64_t *window_usecs __unused) {
  return KERN_NOT_SUPPORTED;
}

kern_return_t sfi_set_class_offtime(sfi_class_id_t class_id __unused,
...
kern_return_t sfi_get_class_offtime(sfi_class_id_t class_id __unused,
                                    uint64_t *offtime_usecs __unused) {
  return KERN_NOT_SUPPORTED;
}

void sfi_reevaluate(thread_t thread __unused) { return; }
