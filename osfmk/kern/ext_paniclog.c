...
LIST_HEAD(_ext_paniclog_handle_list, ext_paniclog_handle)
ext_paniclog_handle_list;
...
void ext_paniclog_init(void) {
  ...
  uuid_t uuid;
  panic_with_data_handle = ext_paniclog_handle_alloc_with_uuid(
      uuid, PANIC_WITH_DATA_DATA_ID, PANIC_WITH_DATA_MAX_LEN,
...
ext_paniclog_handle_alloc_with_uuid(uuid_t uuid, const char *data_id,
  ...
  if (max_len >
      (MAX_EXT_PANICLOG_SIZE - data_id_size - sizeof(ext_paniclog_header_t))) {
    os_log_error(OS_LOG_DEFAULT,
                 "EXT_PANICLOG_: Input len %d greater than max allowed %d\n",
                 max_len, MAX_EXT_PANICLOG_SIZE);
    return NULL;
  }

  ext_paniclog_handle_t *handle =
      kalloc_type(ext_paniclog_handle_t, Z_WAITOK | Z_ZERO);
  if (!handle) {
    return NULL;
  }
  ...
  handle->flags = (options & EXT_PANICLOG_OPTIONS_ADD_SEPARATE_KEY)
...
static int ext_paniclog_insert_data_internal(ext_paniclog_handle_t *handle,
...
int ext_paniclog_append_data(ext_paniclog_handle_t *handle, void *addr,
  ...
  ret = ext_paniclog_insert_data_internal(handle, addr, len, handle->used_len);
  if (ret == 0) {
    handle->used_len += len;
  }
...
int ext_paniclog_set_used_len(ext_paniclog_handle_t *handle,
                              uint32_t used_len) {
  if (handle == NULL) {
    return -1;
  }
...
int ext_paniclog_yield_buffer(ext_paniclog_handle_t *handle,
                              uint32_t used_len) {
...
static uint32_t ext_paniclog_get_size_required(void) {
  uint32_t size_req = 0;
...
bool is_debug_ptr_in_ext_paniclog(void) {
  bool ext_paniclog_exceeds = ((panic_info->eph_ext_paniclog_offset != 0)
                                   ? ((uint32_t)(debug_buf_ptr - gPanicBase) >=
 ...
 * +---------+------------+---------+---------+------------+------------+---------+---------+---------+-----------+------------+----------+
 * |         |            |         |         |            |            | | | |
 * |            |          | |Version  | No of logs | UUID 1  | Flags 1 | Data
 * ID 1  | Data len 1 | Data 1  | UUID 2  | Flags 2 | Data ID 2 | Data len 2 |
 * Data 2   | |         |            |         |         |            | | | | |
 * |            |          |
 * +---------+------------+---------+---------+------------+------------+---------+---------+---------+-----------+------------+----------+
 *
...
uint32_t ext_paniclog_write_panicdata(void) {
  ...
  if (!OSCompareAndSwap8(0, 1, &ext_paniclog_panic_write_done) ||
      (ext_paniclog_handle_count == 0) ||
  ...
  LIST_FOREACH(tmp, &ext_paniclog_handle_list, handles) {
    ...
    if ((dst + tmp->used_len + data_id_size + sizeof(ext_paniclog_header_t)) >
        end) {
      break;
    }
...
static int
ext_paniclog_create_multiple_handles(ext_paniclog_handle_t *handles[],
  ...
  for (int i = 0; i < 2; i++) {
    ...
    os_log(OS_LOG_DEFAULT,
           "EXT_PANICLOG_TEST: Creating handle %d with UUID: %s\n", (i + 1),
           uuid_string);

    handle = ext_paniclog_handle_alloc_with_uuid(uuid, data_id[i], 1024,
                                                 EXT_PANICLOG_OPTIONS_NONE);
    if (handle == NULL) {
 ...
 */
static int ext_paniclog_multiple_handles_panic_test(void) {
  ...
  uuid_generate(uuid);
  uuid_unparse(uuid, uuid_string);
  os_log(OS_LOG_DEFAULT, "EXT_PANICLOG_TEST: panic with data UUID: %s",
         uuid_string);
 ...
 */
static int ext_paniclog_multiple_handles_test(void) {
  char data[17] = "abcdefghabcdefgh";
 ...
 */
static int ext_paniclog_write_panicdata_test(void) {
  uuid_t uuid;
  uuid_generate(uuid);
  uuid_string_t uuid_string;
  uuid_unparse(uuid, uuid_string);
  ...
  // Including ADD_SEPARATE_KEY option allows us to test if the option --> flag
  // translation works well
  ext_paniclog_handle_t *handle = ext_paniclog_handle_alloc_with_uuid(
      uuid, data_id, 1024, EXT_PANICLOG_OPTIONS_ADD_SEPARATE_KEY);
  if (handle == NULL) {
 ...
 */
static int ext_paniclog_handle_create_test(void) {
  ...
  ext_paniclog_handle_t *handle = ext_paniclog_handle_alloc_with_uuid(
      uuid, "Test Handle", max_len, EXT_PANICLOG_OPTIONS_NONE);
  if (handle == NULL) {
    os_log(OS_LOG_DEFAULT,
           "EXT_PANICLOG_TEST: Handle create failed. Returned NULL\n");
    return ECANCELED;
  }
...
static int ext_paniclog_get_list_count(void) {
  int cnt = 0;
  ext_paniclog_handle_t *tmp;
 ...
 */
static int ext_paniclog_set_active_inactive_test(void) {
  uuid_t uuid;
  uuid_generate(uuid);
  uuid_string_t uuid_string;
  uuid_unparse(uuid, uuid_string);
  ...
  ext_paniclog_handle_t *handle = ext_paniclog_handle_alloc_with_uuid(
      uuid, "Test handle", 1024, EXT_PANICLOG_OPTIONS_NONE);
  ...
  if (cnt != (initial_cnt + 1)) {
    os_log(OS_LOG_DEFAULT,
           "EXT_PANICLOG_TEST: List count error after active\n");
    ext_paniclog_handle_free(handle);
    return ECANCELED;
  }
  ...
  if (cnt != initial_cnt) {
    os_log(OS_LOG_DEFAULT,
           "EXT_PANICLOG_TEST: List count error after inactive\n");
    ext_paniclog_handle_free(handle);
    return ECANCELED;
  }
 ...
 */
static int ext_paniclog_insert_data_test(void) {
  uuid_t uuid;
  uuid_generate(uuid);
  uuid_string_t uuid_string;
  uuid_unparse(uuid, uuid_string);
...
/* Test 7 */
static int ext_paniclog_insert_dummy_handles_test(void) {
  ...
  handle_1 = ext_paniclog_handle_alloc_with_uuid(uuid_1, "Dummy handle 1", 1024,
                                                 EXT_PANICLOG_OPTIONS_NONE);
  if (handle_1 == NULL) {
  ...
  handle_2 = ext_paniclog_handle_alloc_with_uuid(uuid_1, "Dummy handle 2", 1024,
                                                 EXT_PANICLOG_OPTIONS_NONE);
  if (handle_2 == NULL) {
  ...
  handle_3 = ext_paniclog_handle_alloc_with_uuid(uuid_3, "Dummy handle 3", 1024,
                                                 EXT_PANICLOG_OPTIONS_NONE);
  if (handle_3 == NULL) {
...
/* Test 8 */
static int ext_paniclog_insert_struct_handles_test(void) {
  ...
  handle_2 = ext_paniclog_handle_alloc_with_uuid(uuid_2, "Dummy handle 2", 1024,
                                                 EXT_PANICLOG_OPTIONS_NONE);
  if (handle_2 == NULL) {
    os_log(OS_LOG_DEFAULT, "EXT_PANICLOG_TEST: Failed to create handle 3\n");
    return ECANCELED;
 ...
 */
static int ext_paniclog_insert_dummy_handles_separate_fields_test(void) {
  ext_paniclog_handle_t *handle_1, *handle_2;
  char data1[18] = "Data for handle 1";
  ...
  uuid_t uuid_1;
  uuid_parse(uuid_string_1, uuid_1);
  handle_1 = ext_paniclog_handle_alloc_with_uuid(
      uuid_1, "Dummy handle 1", 1024, EXT_PANICLOG_OPTIONS_ADD_SEPARATE_KEY);
  if (handle_1 == NULL) {
    os_log(OS_LOG_DEFAULT, "EXT_PANICLOG_TEST: Failed to create handle 1\n");
    return ECANCELED;
  }
  ...
  handle_2 = ext_paniclog_handle_alloc_with_uuid(
      uuid_2, "Dummy handle 2", 1024, EXT_PANICLOG_OPTIONS_ADD_SEPARATE_KEY);
  if (handle_2 == NULL) {
    os_log(OS_LOG_DEFAULT, "EXT_PANICLOG_TEST: Failed to create handle 2\n");
    return ECANCELED;
  }
...
int ext_paniclog_test_hook(uint32_t option) {
  ...
  case EXT_PANICLOG_TEST_SET_ACTIVE_INACTIVE:
    os_log(OS_LOG_DEFAULT,
           "EXT_PANICLOG_TEST: Testing set active and inactive\n");
  ...
  case EXT_PANICLOG_TEST_MULTIPLE_HANDLES_PANIC:
    os_log(OS_LOG_DEFAULT,
           "EXT_PANICLOG_TEST: Testing multiple handles with panic\n");
  ...
  case EXT_PANICLOG_TEST_INSERT_DUMMY_HANDLES_AS_SEPARATE_FIELDS:
    os_log(OS_LOG_DEFAULT,
           "EXT_PANICLOG_TEST: Insert dummy handles as separate fields\n");
...
ext_paniclog_handle_t *ext_paniclog_handle_alloc_with_uuid(
    uuid_t uuid __unused, const char *data_id __unused,
    uint32_t max_len __unused, ext_paniclog_create_options_t options __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return NULL;
}

ext_paniclog_handle_t *ext_paniclog_handle_alloc_with_buffer(
    uuid_t uuid __unused, const char *data_id __unused,
    uint32_t max_len __unused, void *buff __unused,
    ext_paniclog_create_options_t options __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return NULL;
}

void ext_paniclog_handle_free(ext_paniclog_handle_t *handle __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return;
}

int ext_paniclog_handle_set_active(ext_paniclog_handle_t *handle __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return -1;
}

int ext_paniclog_handle_set_inactive(ext_paniclog_handle_t *handle __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return -1;
}

int ext_paniclog_insert_data(ext_paniclog_handle_t *handle __unused,
                             void *addr __unused, uint32_t len __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return -1;
}

int ext_paniclog_append_data(ext_paniclog_handle_t *handle __unused,
                             void *addr __unused, uint32_t len __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return -1;
}

void *ext_paniclog_claim_buffer(ext_paniclog_handle_t *handle __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return NULL;
}

void *ext_paniclog_get_buffer(ext_paniclog_handle_t *handle __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return NULL;
}

int ext_paniclog_set_used_len(ext_paniclog_handle_t *handle __unused,
                              uint32_t used_len __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return -1;
}

int ext_paniclog_yield_buffer(ext_paniclog_handle_t *handle __unused,
                              uint32_t used_len __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return -1;
}

void ext_paniclog_panic_with_data(uuid_t uuid __unused, void *addr __unused,
                                  uint32_t len __unused) {
  os_log_error(OS_LOG_DEFAULT, "Extensible paniclog not supported");
  return;
}

bool is_debug_ptr_in_ext_paniclog(void) { return false; }
