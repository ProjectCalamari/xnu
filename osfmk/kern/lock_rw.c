...
#define lck_rw_ilk_lock(lock)                                                  \
  hw_lock_bit((hw_lock_bit_t *)(&(lock)->lck_rw_tag), LCK_RW_INTERLOCK_BIT,    \
              LCK_GRP_NULL)
#define lck_rw_ilk_unlock(lock)                                                \
  hw_unlock_bit((hw_lock_bit_t *)(&(lock)->lck_rw_tag), LCK_RW_INTERLOCK_BIT)

#define ordered_load_rw(lock)                                                  \
  os_atomic_load(&(lock)->lck_rw_data, compiler_acq_rel)
#define ordered_store_rw(lock, value)                                          \
  os_atomic_store(&(lock)->lck_rw_data, (value), compiler_acq_rel)
#define ordered_store_rw_owner(lock, value)                                    \
  os_atomic_store(&(lock)->lck_rw_owner, (value), compiler_acq_rel)
...
rwlde_caller_packing_params = VM_PACKING_PARAMS(LCK_RW_CALLER_PACKED);

#define set_rwlde_caller_packed(entry, caller)                                 \
  ((entry)->rwlde_caller_packed =                                              \
       VM_PACK_POINTER((vm_offset_t)caller, LCK_RW_CALLER_PACKED))
#define get_rwlde_caller(entry)                                                \
  ((void *)VM_UNPACK_POINTER(entry->rwlde_caller_packed, LCK_RW_CALLER_PACKED))
 ...
 *
 * @discussion
 * Usage statistics for the lock are going to be added to the lock group
 * provided.
 *
 * The lock attribute can be used to specify the lock contention behaviour.
 * RW_WRITER_PRIORITY is the default behaviour (LCK_ATTR_NULL defaults to
 * RW_WRITER_PRIORITY) and lck_attr_rw_shared_priority() can be used to set the
 * behaviour to RW_SHARED_PRIORITY.
 *
 * RW_WRITER_PRIORITY gives priority to the writers upon contention with the
 * readers; if the lock is held and a writer starts waiting for the lock,
 * readers will not be able to acquire the lock until all writers stop
 * contending. Readers could potentially starve. RW_SHARED_PRIORITY gives
 * priority to the readers upon contention with the writers: unleass the lock is
 * held in exclusive mode, readers will always be able to acquire the lock.
 * Readers can lock a shared lock even if there are writers waiting. Writers
 ...
 * Just up to LCK_RW_EXPECTED_MAX_NUMBER rw lock debug information can be
 * stored.
 *
 ...
 *
 * If we reached the overflow threshold we might stop asserting because we
 * cannot be sure anymore if the lock was acquired or not.
 *
 * Even if we reached the overflow threshold, we try to store the debug
 * information for the new locks acquired. This can be useful in core dumps to
 * debug possible return to userspace without unlocking and to find possible
 * readers holding the lock.
 */
#if DEBUG_RW

__static_if_init_func void lck_rw_assert_init(const char *args,
                                              uint64_t kf_ovrd) {
...
__abortlike static void rwlock_slot_panic(rw_lock_debug_t *rw_locks_held) {
  panic("No empty slot found in %p slot_used %d", rw_locks_held,
...
canlock_rwlock_panic(lck_rw_t *lock, thread_t thread,
                     struct rw_lock_debug_entry *entry) {
  panic("RW lock %p already held by %p caller %p mode_count %d state 0x%x "
        "owner 0x%p ",
...
assert_canlock_rwlock_slow(lck_rw_t *lock, thread_t thread,
                           lck_rw_type_t type) {
  ...
  struct rw_lock_debug_entry *entry =
      find_lock_in_savedlocks(lock, rw_locks_held);
  if (__improbable(entry != NULL)) {
    ...
    if ((type == LCK_RW_TYPE_SHARED) && can_be_shared_recursive &&
...
static inline void assert_canlock_rwlock(lck_rw_t *lock, thread_t thread,
                                         lck_rw_type_t type) {
...
__abortlike static void held_rwlock_notheld_panic(lck_rw_t *lock,
                                                  thread_t thread) {
  panic("RW lock %p not held by %p", lock, thread);
}

__abortlike static void
held_rwlock_notheld_with_info_panic(lck_rw_t *lock, thread_t thread,
                                    lck_rw_type_t type,
                                    struct rw_lock_debug_entry *entry) {
  if (type == LCK_RW_TYPE_EXCLUSIVE) {
    panic("RW lock %p not held in exclusive by %p caller %p read %d state 0x%x "
          "owner 0x%p ",
          lock, thread, get_rwlde_caller(entry), entry->rwlde_mode_count,
          ordered_load_rw(lock), ctid_get_thread_unsafe(lock->lck_rw_owner));
  } else {
    panic("RW lock %p not held in shared by %p caller %p read %d state 0x%x "
          "owner 0x%p ",
          lock, thread, get_rwlde_caller(entry), entry->rwlde_mode_count,
          ordered_load_rw(lock), ctid_get_thread_unsafe(lock->lck_rw_owner));
  }
...
assert_held_rwlock_slow(lck_rw_t *lock, thread_t thread, lck_rw_type_t type) {
  ...
  if (__improbable(rw_locks_held->rwld_locks_acquired == 0 ||
                   rw_locks_held->rwld_locks_saved == 0)) {
...
static inline void assert_held_rwlock(lck_rw_t *lock, thread_t thread,
                                      lck_rw_type_t type) {
  if (lck_rw_assert_enabled()) {
...
__attribute__((noinline)) static void
change_held_rwlock_slow(lck_rw_t *lock, thread_t thread, lck_rw_type_t typeFrom,
                        void *caller) {
  rw_lock_debug_t *rw_locks_held = &thread->rw_lock_held;
...
static inline void change_held_rwlock(lck_rw_t *lock, thread_t thread,
                                      lck_rw_type_t typeFrom, void *caller) {
  if (lck_rw_assert_enabled()) {
    change_held_rwlock_slow(lock, thread, typeFrom, caller);
  }
}

__abortlike static void add_held_rwlock_too_many_panic(thread_t thread) {
  panic("RW lock too many rw locks held, rwld_locks_acquired maxed out for "
        "thread %p",
        thread);
}

static __attribute__((noinline)) void add_held_rwlock_slow(lck_rw_t *lock,
                                                           thread_t thread,
                                                           lck_rw_type_t type,
                                                           void *caller) {
  rw_lock_debug_t *rw_locks_held = &thread->rw_lock_held;
  ...
  if (type == LCK_RW_TYPE_EXCLUSIVE) {
    if (__improbable(rw_locks_held->rwld_locks_saved ==
                     LCK_RW_EXPECTED_MAX_NUMBER)) {
      // array is full
...
static inline void add_held_rwlock(lck_rw_t *lock, thread_t thread,
                                   lck_rw_type_t type, void *caller) {
  if (lck_rw_assert_enabled()) {
    add_held_rwlock_slow(lock, thread, type, caller);
  }
}

static void remove_held_rwlock_slow(lck_rw_t *lock, thread_t thread,
                                    lck_rw_type_t type) {
  rw_lock_debug_t *rw_locks_held = &thread->rw_lock_held;
  if (__improbable(rw_locks_held->rwld_locks_acquired == 0)) {
    return;
...
static inline void remove_held_rwlock(lck_rw_t *lock, thread_t thread,
                                      lck_rw_type_t type) {
  if (lck_rw_assert_enabled()) {
 ...
 */
static inline boolean_t lck_interlock_lock(lck_rw_t *lck) {
  boolean_t istate;
...
static inline void lck_interlock_unlock(lck_rw_t *lck, boolean_t istate) {
 ...
 */
static inline uint64_t lck_rw_deadline_for_spin(lck_rw_t *lck) {
  lck_rw_word_t word;

  word.data = ordered_load_rw(lck);
  if (word.can_sleep) {
    if (word.r_waiting || word.w_waiting ||
        (word.shared_count > machine_info.max_cpus)) {
      /*
       * there are already threads waiting on this lock... this
       * implies that they have spun beyond their deadlines waiting for
       * the desired state to show up so we will not bother spinning at this
       * time... or the current number of threads sharing this lock exceeds our
       * capacity to run them concurrently and since all states we're going to
       * spin for require the rw_shared_count to be at 0, we'll not bother
       * spinning since the latency for this to happen is unpredictable...
       */
 ...
 */
static inline void lck_rw_lock_pause(boolean_t interrupts_enabled) {
#if X86_64
...
static lck_rw_drain_state_t lck_rw_drain_status(lck_rw_t *lock,
 ...
 */
static inline void lck_rw_interlock_spin(lck_rw_t *lock) {
  uint32_t data, prev;

  for (;;) {
    data = atomic_exchange_begin32(&lock->lck_rw_data, &prev,
...
typedef enum __enum_closed __enum_options {
  LCK_RW_GRAB_F_SHARED =
      0x0, // Not really a flag obviously but makes call sites more readable.
...
static lck_rw_grab_state_t lck_rw_grab(lck_rw_t *lock,
  ...
  for (;;) {
    ...
    if (do_exch) {
      if (atomic_exchange_complete32(&lock->lck_rw_data, prev, data,
                                     memory_order_acquire_smp)) {
 ...
 */
static void lck_rw_drop(lck_rw_t *lock, lck_rw_grab_flags_t flags) {
  uint32_t data, prev;
...
static boolean_t lck_rw_lock_exclusive_gen(lck_rw_t *lock,
                                           bool (^lock_pause)(void)) {
  ...
  boolean_t dtrace_rwl_excl_spin, dtrace_rwl_excl_block,
      dtrace_ls_enabled = FALSE;
  uint64_t wait_interval = 0;
  int readers_at_sleep = 0;
#endif

  assertf(lock->lck_rw_owner != self->ctid,
          "Lock already held state=0x%x, owner=%p", ordered_load_rw(lock),
          self);
   ...
   */
  while (lck_rw_grab(lock, LCK_RW_GRAB_F_WANT_EXCL, NULL) !=
         LCK_RW_GRAB_S_LOCKED) {
#if CONFIG_DTRACE
    if (dtrace_ls_initialized == FALSE) {
      ...
      dtrace_rwl_excl_block =
          (lockstat_probemap[LS_LCK_RW_LOCK_EXCL_BLOCK] != 0);
    ...
    KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_RW_LCK_EX_WRITER_SPIN_CODE) |
                     DBG_FUNC_START,
                 trace_lck, 0, 0, 0, 0);

    grab_state = lck_rw_grab(lock, LCK_RW_GRAB_F_WANT_EXCL | LCK_RW_GRAB_F_WAIT,
                             lock_pause);
   ...
   */
  while (lck_rw_drain_status(lock, LCK_RW_SHARED_MASK | LCK_RW_WANT_UPGRADE,
                             FALSE, NULL) != LCK_RW_DRAIN_S_DRAINED) {
#if CONFIG_DTRACE
    /*
    ...
    drain_state = lck_rw_drain_status(
        lock, LCK_RW_SHARED_MASK | LCK_RW_WANT_UPGRADE, TRUE, lock_pause);

    KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_RW_LCK_EX_READER_SPIN_CODE) |
                     DBG_FUNC_END,
                 trace_lck, 0, 0, drain_state, 0);
...
static inline void lck_rw_lock_check_preemption(lck_rw_t *lock __unused) {
  assertf(
      (get_preemption_level() == 0 && ml_get_interrupts_enabled()) ||
          startup_phase < STARTUP_SUB_EARLY_BOOT ||
          current_cpu_datap()->cpu_hibernate || ml_is_quiescing() ||
          !not_in_kdp,
      "%s: attempt to take rwlock %p in non-preemptible or interrupt context: "
      "preemption level = %d, interruptible = %d",
      __func__, lock, get_preemption_level(), (int)ml_get_interrupts_enabled());
}

#define LCK_RW_LOCK_EXCLUSIVE_TAS(lck)                                         \
  (atomic_test_and_set32(&(lck)->lck_rw_data,                                  \
                         (LCK_RW_SHARED_MASK | LCK_RW_WANT_EXCL |              \
                          LCK_RW_WANT_UPGRADE | LCK_RW_INTERLOCK),             \
 ...
 * It's only used for the vm object lock, and use for other subsystems is
 * UNSUPPORTED. Note that the return value is ONLY A HEURISTIC w.r.t. the lock's
 * contention.
 *
 * @param lock           rw_lock to lock.
 *
 * @returns Returns TRUE if the thread spun or blocked while attempting to
 * acquire the lock, FALSE otherwise.
 */
bool lck_rw_lock_exclusive_check_contended(lck_rw_t *lock) {
...
#ifdef DEBUG_RW
  add_held_rwlock(lock, thread, LCK_RW_TYPE_EXCLUSIVE,
                  __builtin_return_address(0));
#endif /* DEBUG_RW */
  return contended;
}

__attribute__((always_inline)) static boolean_t
lck_rw_lock_exclusive_internal_inline(lck_rw_t *lock, void *caller,
                                      bool (^lock_pause)(void)) {
...
__attribute__((noinline)) static void
lck_rw_lock_exclusive_internal(lck_rw_t *lock, void *caller) {
 ...
 * Multiple threads can acquire the lock in shared mode at the same time, but
 * only one thread at a time can acquire it in exclusive mode. NOTE: the thread
 * cannot return to userspace while the lock is held. Recursive locking is not
 * supported.
 *
 * @param lock           rw_lock to lock.
 */
__mockable void lck_rw_lock_exclusive(lck_rw_t *lock) {
  (void)lck_rw_lock_exclusive_internal_inline(lock, __builtin_return_address(0),
                                              NULL);
}
 ...
 *
 * @abstract
 * Locks a rw_lock in exclusive mode. Returns early if the lock can't be
 * acquired and the specified block returns true.
 *
 * @discussion
 * Identical to lck_rw_lock_exclusive() but can return early if the lock can't
 * be acquired and the specified block returns true. The block is called
 ...
 *                       not been acquired.
 */
boolean_t lck_rw_lock_exclusive_b(lck_rw_t *lock, bool (^lock_pause)(void)) {
  assert(!lock->lck_rw_can_sleep);

  return lck_rw_lock_exclusive_internal_inline(
      lock, __builtin_return_address(0), lock_pause);
}
 ...
 */
static boolean_t lck_rw_lock_shared_gen(lck_rw_t *lck,
                                        bool (^lock_pause)(void)) {
  __assert_only thread_t self = current_thread();
  ...
  int readers_at_sleep = 0;
  boolean_t dtrace_ls_initialized = FALSE;
  boolean_t dtrace_rwl_shared_spin, dtrace_rwl_shared_block,
      dtrace_ls_enabled = FALSE;
#endif /* CONFIG_DTRACE */

  assertf(lck->lck_rw_owner != self->ctid,
          "Lock already held state=0x%x, owner=%p", ordered_load_rw(lck), self);
  ...
  while (lck_rw_grab(lck, LCK_RW_GRAB_F_SHARED, NULL) != LCK_RW_GRAB_S_LOCKED) {
    ...
    if (dtrace_ls_initialized == FALSE) {
      ...
      dtrace_rwl_shared_spin =
          (lockstat_probemap[LS_LCK_RW_LOCK_SHARED_SPIN] != 0);
      dtrace_rwl_shared_block =
          (lockstat_probemap[LS_LCK_RW_LOCK_SHARED_BLOCK] != 0);
    ...
    KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_RW_LCK_SHARED_SPIN_CODE) |
                     DBG_FUNC_START,
                 trace_lck, lck->lck_rw_want_excl, lck->lck_rw_want_upgrade, 0,
                 0);

    grab_state =
        lck_rw_grab(lck, LCK_RW_GRAB_F_SHARED | LCK_RW_GRAB_F_WAIT, lock_pause);

    KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_RW_LCK_SHARED_SPIN_CODE) |
                     DBG_FUNC_END,
                 trace_lck, lck->lck_rw_want_excl, lck->lck_rw_want_upgrade,
                 grab_state, 0);
    ...
    if (lck->lck_rw_can_sleep) {
      ...
      if ((word.want_excl || word.want_upgrade) &&
        ...
        KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_RW_LCK_SHARED_WAIT_CODE) |
                         DBG_FUNC_START,
  ...
  if (dtrace_ls_enabled == TRUE) {
    if (slept == 0) {
      LOCKSTAT_RECORD(LS_LCK_RW_LOCK_SHARED_SPIN, lck,
...
__attribute__((always_inline)) static boolean_t
lck_rw_lock_shared_internal_inline(lck_rw_t *lock, void *caller,
                                   bool (^lock_pause)(void)) {
#pragma unused(caller)
...
__attribute__((noinline)) static void
lck_rw_lock_shared_internal(lck_rw_t *lock, void *caller) {
 ...
 * This function can block.
 * Multiple threads can acquire the lock in shared mode at the same time, but
 * only one thread at a time can acquire it in exclusive mode. If the lock is
 * held in shared mode and there are no writers waiting, a reader will be able
 * to acquire the lock without waiting. If the lock is held in shared mode and
 * there is at least a writer waiting, a reader will wait for all the writers to
 * make progress if the lock was initialized with the default settings. Instead
 * if RW_SHARED_PRIORITY was selected at initialization time, a reader will
 * never wait if the lock is held in shared mode. NOTE: the thread cannot return
 * to userspace while the lock is held. Recursive locking is not supported.
 *
 * @param lock           rw_lock to lock.
 */
__mockable void lck_rw_lock_shared(lck_rw_t *lock) {
  (void)lck_rw_lock_shared_internal_inline(lock, __builtin_return_address(0),
                                           NULL);
}
 ...
 *                       not been acquired.
 */
boolean_t lck_rw_lock_shared_b(lck_rw_t *lock, bool (^lock_pause)(void)) {
  assert(!lock->lck_rw_can_sleep);

  return lck_rw_lock_shared_internal_inline(lock, __builtin_return_address(0),
                                            lock_pause);
}
 ...
 *		count and determined that someone else owns
 * 'lck_rw_want_upgrade' if 'lck_rw_shared_count' == 0, its also already dropped
 * 'lck_w_waiting' all we need to do here is determine if a wakeup is needed
 */
static boolean_t
lck_rw_lock_shared_to_exclusive_failure(lck_rw_t *lck,
  ...
  KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_RW_LCK_SH_TO_EX_CODE) |
                   DBG_FUNC_NONE,
               VM_KERNEL_UNSLIDE_OR_PERM(lck), lck->lck_rw_shared_count,
 ...
 */
static void lck_rw_lock_shared_to_exclusive_success(lck_rw_t *lock) {
  __kdebug_only uintptr_t trace_lck = VM_KERNEL_UNSLIDE_OR_PERM(lock);
  int slept = 0;
  lck_rw_word_t word;
  wait_result_t res;
  ...
  int readers_at_sleep = 0;
  boolean_t dtrace_ls_initialized = FALSE;
  boolean_t dtrace_rwl_shared_to_excl_spin, dtrace_rwl_shared_to_excl_block,
      dtrace_ls_enabled = FALSE;
#endif

  while (lck_rw_drain_status(lock, LCK_RW_SHARED_MASK, FALSE, NULL) !=
         LCK_RW_DRAIN_S_DRAINED) {
    word.data = ordered_load_rw(lock);
#if CONFIG_DTRACE
    if (dtrace_ls_initialized == FALSE) {
      dtrace_ls_initialized = TRUE;
      dtrace_rwl_shared_to_excl_spin =
          (lockstat_probemap[LS_LCK_RW_LOCK_SHARED_TO_EXCL_SPIN] != 0);
      dtrace_rwl_shared_to_excl_block =
          (lockstat_probemap[LS_LCK_RW_LOCK_SHARED_TO_EXCL_BLOCK] != 0);
      dtrace_ls_enabled =
          dtrace_rwl_shared_to_excl_spin || dtrace_rwl_shared_to_excl_block;
      if (dtrace_ls_enabled) {
        /*
         * Either sleeping or spinning is happening,
         *  start a timing of our delay interval now.
    ...
    KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_RW_LCK_SH_TO_EX_SPIN_CODE) |
                     DBG_FUNC_START,
    ...
    if (word.can_sleep) {
      ...
      if (word.shared_count != 0) {
        KERNEL_DEBUG(
            MACHDBG_CODE(DBG_MACH_LOCKS, LCK_RW_LCK_SH_TO_EX_WAIT_CODE) |
                DBG_FUNC_START,
            trace_lck, word.shared_count, 0, 0, 0);
...
#if CONFIG_DTRACE
  /*
   * We infer whether we took the sleep/spin path above by checking
   * readers_at_sleep.
   */
  if (dtrace_ls_enabled == TRUE) {
    if (slept == 0) {
      LOCKSTAT_RECORD(LS_LCK_RW_LOCK_SHARED_TO_EXCL_SPIN, lock,
                      mach_absolute_time() - wait_interval, 0);
    } else {
 ...
 * @discussion
 * This function can block.
 * Only one reader at a time can upgrade to exclusive mode. If the upgrades
 * fails the function will return with the lock not held. The caller needs to
 * hold the lock in shared mode to upgrade it.
 *
 ...
 *          If the function was not able to upgrade the lock, the lock will be
 * dropped by the function.
 */
__mockable boolean_t lck_rw_lock_shared_to_exclusive(lck_rw_t *lock) {
  thread_t thread = current_thread();
  uint32_t data, prev;

  assertf(lock->lck_rw_priv_excl != 0, "lock %p thread %p", lock,
          current_thread());
  ...
  if (data &
      LCK_RW_SHARED_MASK) { /* check to see if all of the readers are drained */
    lck_rw_lock_shared_to_exclusive_success(
        lock); /* if not, we need to go wait */
  }
...
#if DEBUG_RW
  change_held_rwlock(lock, thread, LCK_RW_TYPE_SHARED,
                     __builtin_return_address(0));
#endif /* DEBUG_RW */
  return TRUE;
}
 ...
 */
static void lck_rw_lock_exclusive_to_shared_gen(lck_rw_t *lck,
                                                uint32_t prior_lock_state,
                                                void *caller) {
#pragma unused(caller)
  __kdebug_only uintptr_t trace_lck = VM_KERNEL_UNSLIDE_OR_PERM(lck);
 ...
 *
 * @discussion
 * The caller needs to hold the lock in exclusive mode to be able to downgrade
 * it.
 *
 * @param lock           rw_lock already held in exclusive mode to downgrade.
 */
__mockable void lck_rw_lock_exclusive_to_shared(lck_rw_t *lock) {
  uint32_t data, prev;

  assertf(lock->lck_rw_owner == current_thread()->ctid, "state=0x%x, owner=%p",
          lock->lck_rw_data, ctid_get_thread_unsafe(lock->lck_rw_owner));
 ...
 */
void _lck_rw_lock_type_panic(lck_rw_t *lck, lck_rw_type_t lck_rw_type);
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wmissing-noreturn"
__attribute__((noinline, weak)) void
_lck_rw_lock_type_panic(lck_rw_t *lck, lck_rw_type_t lck_rw_type) {
  panic("lck_rw_lock(): Invalid RW lock type: %x for lock %p", lck_rw_type,
        lck);
}
 ...
 */
__mockable void lck_rw_lock(lck_rw_t *lck, lck_rw_type_t lck_rw_type) {
...
__attribute__((always_inline)) static boolean_t
lck_rw_try_lock_shared_internal_inline(lck_rw_t *lock, void *caller) {
#pragma unused(caller)
...
__attribute__((noinline)) static boolean_t
lck_rw_try_lock_shared_internal(lck_rw_t *lock, void *caller) {
 ...
 * @param lock           rw_lock to lock.
 *
 * @returns TRUE if the lock is successfully acquired, FALSE in case it was
 * already held.
 */
__mockable boolean_t lck_rw_try_lock_shared(lck_rw_t *lock) {
  return lck_rw_try_lock_shared_internal_inline(lock,
                                                __builtin_return_address(0));
}

__attribute__((always_inline)) static boolean_t
lck_rw_try_lock_exclusive_internal_inline(lck_rw_t *lock, void *caller) {
#pragma unused(caller)
  uint32_t data, prev;
...
__attribute__((noinline)) static boolean_t
lck_rw_try_lock_exclusive_internal(lck_rw_t *lock, void *caller) {
 ...
 * already held.
 */
__mockable boolean_t lck_rw_try_lock_exclusive(lck_rw_t *lock) {
  return lck_rw_try_lock_exclusive_internal_inline(lock,
                                                   __builtin_return_address(0));
}
 ...
 * than it should. rdar://76570684
 */
boolean_t _lck_rw_try_lock_type_panic(lck_rw_t *lck, lck_rw_type_t lck_rw_type);
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wmissing-noreturn"
__attribute__((noinline, weak)) boolean_t
 ...
 *
 * @discussion
 * This function will return and not wait/block in case the lock is already
 * held. See lck_rw_try_lock_shared() or lck_rw_try_lock_exclusive() for more
 * details.
 *
 * @param lck           rw_lock to lock.
 * @param lck_rw_type   LCK_RW_TYPE_SHARED or LCK_RW_TYPE_EXCLUSIVE
 ...
 * already held.
 */
__mockable boolean_t lck_rw_try_lock(lck_rw_t *lck, lck_rw_type_t lck_rw_type) {
  if (lck_rw_type == LCK_RW_TYPE_SHARED) {
...
#if CONFIG_DTRACE
  LOCKSTAT_RECORD(LS_LCK_RW_DONE_RELEASE, lck,
                  lock_type == LCK_RW_TYPE_SHARED ? 0 : 1);
#endif
 ...
 */
__mockable lck_rw_type_t lck_rw_done(lck_rw_t *lock) {
  uint32_t data, prev;
 ...
 */
__mockable void lck_rw_unlock_shared(lck_rw_t *lck) {
  lck_rw_type_t ret;

  assertf(lck->lck_rw_owner == 0, "state=0x%x, owner=%p", lck->lck_rw_data,
          ctid_get_thread_unsafe(lck->lck_rw_owner));
  assertf(lck->lck_rw_shared_count > 0, "shared_count=0x%x",
 ...
 */
__mockable void lck_rw_unlock_exclusive(lck_rw_t *lck) {
  lck_rw_type_t ret;

  assertf(lck->lck_rw_owner == current_thread()->ctid, "state=0x%x, owner=%p",
          lck->lck_rw_data, ctid_get_thread_unsafe(lck->lck_rw_owner));
  ret = lck_rw_done(lck);
 ...
 * The type of the lock/unlock have to match, unless an upgrade/downgrade was
 * performed while holding the lock.
 *
 * @param lck           rw_lock to unlock.
 * @param lck_rw_type   LCK_RW_TYPE_SHARED or LCK_RW_TYPE_EXCLUSIVE
 */
__mockable void lck_rw_unlock(lck_rw_t *lck, lck_rw_type_t lck_rw_type) {
  if (lck_rw_type == LCK_RW_TYPE_SHARED) {
 ...
 * so this function merely asserts that someone is holding the lock, not
 * necessarily the caller. However if rw_lock_debug is on, a best effort
 * mechanism to track the owners is in place, and this function can be more
 * accurate. Type can be LCK_RW_ASSERT_SHARED, LCK_RW_ASSERT_EXCLUSIVE,
 * LCK_RW_ASSERT_HELD LCK_RW_ASSERT_NOTHELD.
 *
 * @param lck   rw_lock to check.
 * @param type  assert type
 */
__mockable void lck_rw_assert(lck_rw_t *lck, unsigned int type) {
  thread_t thread = current_thread();

  switch (type) {
  case LCK_RW_ASSERT_SHARED:
    if ((lck->lck_rw_shared_count != 0) && (lck->lck_rw_owner == 0)) {
#if DEBUG_RW
  ...
  panic("rw lock (%p)%s held (mode=%u)", lck,
        (type == LCK_RW_ASSERT_NOTHELD ? "" : " not"), type);
}
 ...
 */
boolean_t kdp_lck_rw_lock_is_acquired_exclusive(lck_rw_t *lck) {
  if (not_in_kdp) {
    panic("panic: rw lock exclusive check done outside of kernel debugger");
  }
  return ((lck->lck_rw_want_upgrade || lck->lck_rw_want_excl) &&
          (lck->lck_rw_shared_count == 0))
             ? TRUE
             : FALSE;
}

void kdp_rwlck_find_owner(__unused struct waitq *waitq, event64_t event,
 ...
 */
__mockable bool lck_rw_lock_would_yield_shared(lck_rw_t *lck) {
  lck_rw_word_t word;
 ...
 *
 * @param lck           rw_lock already held in shared mode to yield.
 * @param force_yield   if set to true it will always yield irrespective of the
 * lock status
 *
 * @returns TRUE if the lock was yield, FALSE otherwise
 */
bool lck_rw_lock_yield_shared(lck_rw_t *lck, boolean_t force_yield) {
 ...
 * @returns TRUE if the lock would yield, FALSE otherwise
 */
__mockable bool lck_rw_lock_would_yield_exclusive(lck_rw_t *lck,
                                                  lck_rw_yield_t mode) {
  lck_rw_word_t word;
  bool yield = false;
 ...
 * @returns TRUE if the lock was yield, FALSE otherwise
 */
bool lck_rw_lock_yield_exclusive(lck_rw_t *lck, lck_rw_yield_t mode) {
 ...
 */
__attribute__((noinline)) static void lck_rw_clear_promotion(thread_t thread,
                                                             const void *lock) {
  /* Cancel any promotions if the thread had actually blocked while holding a RW
   * lock */
  spl_t s = splsched();
  thread_lock(thread);
...
__abortlike static void __lck_rw_lock_count_dec_panic(thread_t thread) {
  panic("rw lock count underflow for thread %p", thread);
}

__attribute__((always_inline)) void lck_rw_lock_count_dec(thread_t thread,
                                                          const void *lock) {
