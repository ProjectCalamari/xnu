...
int sched_get_rt_deadline_epsilon(void) { return rt_deadline_epsilon_us; }

void sched_set_rt_deadline_epsilon(int new_epsilon_us) {
  rt_deadline_epsilon_us = new_epsilon_us;

  uint64_t abstime;
  clock_interval_to_absolutetime_interval(rt_deadline_epsilon_us, NSEC_PER_USEC,
                                          &abstime);
  assert((abstime >> 32) == 0 &&
         ((rt_deadline_epsilon_us == 0) || (uint32_t)abstime != 0));
...
static void sched_rt_config_pset_push(processor_set_t pset);
#endif /* CONFIG_SCHED_EDGE */

static void rt_init_completed(void) {
  /* This should be unified with sched_edge_max_clusters and moved to a common
   * location. <rdar://145162647> */
  sched_rt_max_clusters = ml_get_cluster_count();

  /* Realtime spill/steal are only supported on platforms with the edge
   * scheduler. */
#if CONFIG_SCHED_EDGE
  /* Hold sched_available_cores_lock to prevent multiple concurrent matrix
   * updates. */
  spl_t s = splsched();
  simple_lock(&sched_available_cores_lock, LCK_GRP_NULL);
  for (int src_cluster_id = 0; src_cluster_id < sched_rt_max_clusters;
       src_cluster_id++) {
    processor_set_t src_pset = pset_array[src_cluster_id];
    assert3p(src_pset, !=,
             PROCESSOR_SET_NULL); /* all psets should be initialized */

    /* For each cluster, set all its outgoing edge parameters */
    for (int dst_cluster_id = 0; dst_cluster_id < sched_rt_max_clusters;
         dst_cluster_id++) {
      ...
      if (clusters_homogenous) {
        ...
        sched_rt_config_set((pset_id_t)src_cluster_id,
                            (pset_id_t)dst_cluster_id,
                            (sched_clutch_edge){
...
static void pset_rt_init(processor_set_t pset) {
  ...
  os_atomic_init(&pset->stealable_rt_threads_earliest_deadline,
                 RT_DEADLINE_NONE);
...
#if CONFIG_SCHED_EDGE
void sched_rt_config_set(uint8_t src_pset, uint8_t dst_pset,
                         sched_clutch_edge edge_config) {
  assert(src_pset != dst_pset ||
         !edge_config.sce_migration_allowed); /* No self-edges. */
  os_atomic_store(&pset_array[src_pset]->sched_rt_edges[dst_pset], edge_config,
                  relaxed);
}

sched_clutch_edge sched_rt_config_get(uint8_t src_pset, uint8_t dst_pset) {
  return os_atomic_load(&pset_array[src_pset]->sched_rt_edges[dst_pset],
                        relaxed);
}

void sched_rt_matrix_get(sched_clutch_edge *edge_matrix, bool *edge_requests,
                         uint64_t num_psets) {
 ...
 *
 * After using sched_rt_config_set() to update edge tunables outgoing from a
...
static void sched_rt_config_pset_push(processor_set_t pset) {
  ...
  sched_pset_search_order_sort_data_t spill_datas[MAX_PSETS - 1],
      steal_datas[MAX_PSETS - 1];
  uint num_spill_datas = 0, num_steal_datas = 0;
  for (pset_id_t other_pset_id = 0; other_pset_id < sched_rt_max_clusters;
       other_pset_id++) {
    if (pset->pset_id == other_pset_id) {
      continue; /* No self-edges. */
    }
    /* Spill */
    sched_clutch_edge out_edge =
        sched_rt_config_get((pset_id_t)pset->pset_cluster_id, other_pset_id);
    ...
    /* Steal */
    sched_clutch_edge in_edge =
        sched_rt_config_get(other_pset_id, (pset_id_t)pset->pset_cluster_id);
  ...
  sched_pset_search_order_compute(
      &pset->sched_rt_spill_search_order, spill_datas, num_spill_datas,
      sched_edge_search_order_weight_then_locality_cmp);
  sched_pset_search_order_compute(
      &pset->sched_rt_steal_search_order, steal_datas, num_steal_datas,
      sched_edge_search_order_weight_then_locality_cmp);
}
  ...
  for (uint8_t src_pset_id = 0; src_pset_id < num_psets; src_pset_id++) {
    for (uint8_t dst_pset_id = 0; dst_pset_id < num_psets; dst_pset_id++) {
      ...
      if (edge_changes[rt_matrix_index]) {
        sched_rt_config_set(src_pset_id, dst_pset_id,
                            rt_matrix[rt_matrix_index]);
      }
    }
  }
...
try_again:
  ...
  int consider_secondaries =
      (!pset->is_SMT) || (bit_count(node->pset_map) == 1) ||
      (node->pset_non_rt_primary_map == 0) || include_ast_urgent_pending_cpus;
  for (; consider_secondaries < 2; consider_secondaries++) {
    ...
    do {
      ...
      processor = pset_choose_processor_for_realtime_thread_smt(
          pset, PROCESSOR_NULL, consider_secondaries, false);
      if (processor) {
        return processor;
      }

      if (consider_secondaries) {
        processor = pset_choose_furthest_deadline_processor_for_realtime_thread(
            pset, thread->sched_pri, thread->realtime.deadline, PROCESSOR_NULL,
            false, include_ast_urgent_pending_cpus);
        if (processor) {
          /*
           * Instead of looping through all the psets to find the global
           * furthest deadline processor, preempt the first candidate found.
           * The preempted thread will then find any other available far
           * deadline processors to preempt.
           */
          return processor;
        }
 ...
 * locked on return.
 */
processor_t sched_rt_choose_processor(const processor_set_t starting_pset,
   ...
   * pset_choose_furthest_deadline_processor_for_realtime_thread().
   */
  bool found_ast_urgent_pending =
      false; /* Tracks whether any (eligible) processors have pending urgent
                ASTs. */
  for (int include_ast_urgent_pending_cpus = 0;
       include_ast_urgent_pending_cpus < 2; include_ast_urgent_pending_cpus++) {
    ...
    while (sched_iterate_psets_ordered(
        starting_pset, &starting_pset->sched_rt_spill_search_order, ~0,
        &istate)) {
      ...
      processor = pset_choose_processor_for_realtime_thread(
          pset, PROCESSOR_NULL, false);
      ...
      /* TODO <rdar://140219824>: Policy question of EDF vs targeting idle cores
       * on another pset. */
      processor = pset_choose_furthest_deadline_processor_for_realtime_thread(
          pset, thread->sched_pri, thread->realtime.deadline, PROCESSOR_NULL,
          false, include_ast_urgent_pending_cpus);
      ...
      found_ast_urgent_pending =
          found_ast_urgent_pending || (pset->pending_AST_URGENT_cpu_mask != 0);
    }
  }
  ...
  /* Check (again, for AMP systems) that there is no lower-priority or idle
   * processor. */
  processor =
      pset_choose_processor_for_realtime_thread(pset, PROCESSOR_NULL, false);
  if (processor != PROCESSOR_NULL) {
    /* We found a candidate processor on this pset to wake or preempt. */
    pset_assert_locked(processor->processor_set);
    return processor;
  ...
  processor = pset_choose_furthest_deadline_processor_for_realtime_thread(
      pset, thread->sched_pri, thread->realtime.deadline, PROCESSOR_NULL, false,
      true);
  if (processor == PROCESSOR_NULL) {
...
thread_t sched_rt_steal_thread(processor_set_t stealing_pset) {
  ...
  /* Continue searching until there are no steal candidates found in a single
   * iteration. */
  while (true) {
    processor_set_t target_pset = NULL;
    uint64_t target_deadline;
    if (__improbable(os_sub_overflow(earliest_deadline, rt_deadline_epsilon,
                                     &target_deadline))) {
      target_deadline = 0;
    }
    ...
    sched_pset_iterate_state_t istate = SCHED_PSET_ITERATE_STATE_INIT;
    while (sched_iterate_psets_ordered(
        stealing_pset, &stealing_pset->sched_rt_steal_search_order,
        ~BIT(stealing_pset->pset_id), &istate)) {
      assert3s(istate.spis_pset_id, !=,
               stealing_pset->pset_id); /* stealing_pset's runqueue is drained
                                           by sched_rt_choose_processor */
      const processor_set_t nset = pset_array[istate.spis_pset_id];
      /* Check for null because we do not use acquire/release semantics for
       * steal order. */
      if (__improbable(nset == PROCESSOR_SET_NULL)) {
        continue;
      }
      uint64_t nset_deadline = os_atomic_load(
          &nset->stealable_rt_threads_earliest_deadline, relaxed);
    ...
    if (target_pset != PROCESSOR_SET_NULL) {
      ...
      if (os_atomic_load(&pset->stealable_rt_threads_earliest_deadline,
                         relaxed) <= target_deadline) {
        ...
        KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_RT_STEAL) | DBG_FUNC_NONE,
             (uintptr_t)thread_tid(new_thread), pset->pset_id,
             pset->cpu_set_low, 0);
        ...
        KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_RT_STEAL) | DBG_FUNC_NONE,
             (uintptr_t)thread_tid(THREAD_NULL), pset->pset_id,
             pset->cpu_set_low, 1);
        pset = change_locked_pset(pset, stealing_pset);
        /* Update earliest_deadline in case it changed while the stealing_pset
         * lock was not held. */
...
void sched_rt_init_pset(processor_set_t pset) { pset_rt_init(pset); }

void sched_rt_init_completed(void) { rt_init_completed(); }
  ...
  do {
    while (pset != NULL) {
      ...
      for (int i = bitmap_first(map, NRTQS); i >= 0; i = bitmap_next(map, i)) {
        ...
        qe_foreach_element_safe(thread, &rt_runq->pri_queue, runq_links) {
          if (thread->last_made_runnable_time <
              scan_context->earliest_rt_make_runnable_time) {
            scan_context->earliest_rt_make_runnable_time =
      ...
      KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_PSET_BITMASKS),
           pset->pset_id, pset->recommended_bitmask,
...
/* pset is locked */
static processor_t pset_choose_next_processor_for_realtime_thread(
    processor_set_t pset, int max_pri, uint64_t minimum_deadline,
    processor_t skip_processor, bool consider_secondaries) {
  (void)consider_secondaries;
  bool skip_spills = true;
  bool include_ast_urgent_pending_cpus = false;

#if CONFIG_SCHED_SMT
  processor_t next_processor = pset_choose_processor_for_realtime_thread_smt(
      pset, skip_processor, consider_secondaries, skip_spills);
#else  /* CONFIG_SCHED_SMT */
  processor_t next_processor = pset_choose_processor_for_realtime_thread(
      pset, skip_processor, skip_spills);
#endif /* CONFIG_SCHED_SMT */
  if (next_processor != PROCESSOR_NULL) {
    return next_processor;
  }

  next_processor = pset_choose_furthest_deadline_processor_for_realtime_thread(
      pset, max_pri, minimum_deadline, skip_processor, skip_spills,
      include_ast_urgent_pending_cpus);
  return next_processor;
}
...
void pset_update_rt_stealable_state(processor_set_t pset) {
  ...
  if (rt_pset_has_stealable_threads(pset)) {
    os_atomic_store(&pset->stealable_rt_threads_earliest_deadline,
                    rt_runq_earliest_deadline(pset), relaxed);
  } else {
    os_atomic_store(&pset->stealable_rt_threads_earliest_deadline,
                    RT_DEADLINE_NONE, relaxed);
  }
}
...
bool rt_choose_next_processor_for_spill_IPI(processor_set_t starting_pset,
  ...
  __kdebug_only uint64_t spill_tid =
      thread_tid(rt_runq_first(&starting_pset->rt_runq));
  processor_set_t pset = starting_pset; /* lock is held on this pset */
  processor_t next_rt_processor = PROCESSOR_NULL;
  /* Optimization so caller can avoid unnecessary lock-takes if there are no
   * psets eligible for spill: */
  bool starting_pset_was_unlocked = false;

  cpumap_t candidate_map =
      ~BIT(starting_pset->pset_id); /* exclude stealing_pset */
  sched_pset_iterate_state_t istate = SCHED_PSET_ITERATE_STATE_INIT;
  while (sched_iterate_psets_ordered(
      starting_pset, &starting_pset->sched_rt_spill_search_order, candidate_map,
      &istate)) {
    assert3u(starting_pset->pset_id, !=, istate.spis_pset_id);
    /* Check for null pset because we do not use acquire/release semantics for
     * spill order. */
    processor_set_t nset = pset_array[istate.spis_pset_id];
    if (__improbable(nset == PROCESSOR_SET_NULL)) {
      continue;
    ...
    /* Make sure the pset is allowed to steal threads from stealing_pset's
     * runqueue. */
    sched_clutch_edge edge = sched_rt_config_get(
        (pset_id_t)starting_pset->pset_id, (pset_id_t)istate.spis_pset_id);
    if (istate.spis_pset_id != starting_pset->pset_id &&
    ...
    next_rt_processor = pset_choose_next_processor_for_realtime_thread(
        pset, max_pri, earliest_deadline, chosen_processor, true);
  ...
  if (next_rt_processor != PROCESSOR_NULL) {
    if (pset != starting_pset) {
      if (bit_set_if_clear(pset->rt_pending_spill_cpu_mask,
                           next_rt_processor->cpu_id)) {
        KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_RT_SIGNAL_SPILL) |
                 DBG_FUNC_START,
             next_rt_processor->cpu_id, pset->rt_pending_spill_cpu_mask,
             starting_pset->cpu_set_low, spill_tid);
      }
    }
    *result_ipi_type =
        sched_ipi_action(next_rt_processor, NULL, SCHED_IPI_EVENT_RT_PREEMPT);
...
bool rt_pset_needs_a_followup_IPI(processor_set_t pset) {
  ...
  return (rt_rq_count > 0) &&
         ((rt_rq_count + nbackup_cpus -
           bit_count(pset->pending_AST_URGENT_cpu_mask)) > 0);
}
...
try_again:
  ...
  /* Consider secondary processors whose primary is actually running a realtime
   * thread */
  cpumap_t secondary_map =
      cpu_map & ~pset->primary_map & (pset->realtime_map << 1);
  if (avoid_cpu0) {
    /* Also avoid cpu1 */
...
out:
  ...
  /*
   * If we didn't find an obvious processor to choose, but there are still more
   * CPUs not already running realtime threads than realtime threads in the
   * realtime run queue, this thread belongs in this pset, so choose some other
   * processor in this pset to ensure the thread is enqueued here.
   */
  cpumap_t non_realtime_map =
...
processor_t pset_choose_processor_for_realtime_thread(
  ...
  cpumap_t cpu_map = pset_available_cpumap(pset) &
                     ~pset->pending_AST_URGENT_cpu_mask & ~pset->realtime_map;
  if (skip_processor) {
    bit_clear(cpu_map, skip_processor->cpu_id);
  }
  if (skip_spills) {
...
/*
 * Choose the processor with (1) the lowest priority less than max_pri and (2)
 * the furthest deadline for that priority. If all available processors are at
 * max_pri, choose the furthest deadline that is greater than minimum_deadline.
 *
 * pset is locked.
 */
processor_t pset_choose_furthest_deadline_processor_for_realtime_thread(
  ...
  uint64_t furthest_deadline =
      rt_deadline_add(minimum_deadline, rt_deadline_epsilon);
  processor_t fd_processor = PROCESSOR_NULL;
  int lowest_priority = max_pri;

  cpumap_t cpu_map =
      pset_available_cpumap(pset) & ~pset->pending_AST_URGENT_cpu_mask;
  if (skip_processor) {
    bit_clear(cpu_map, skip_processor->cpu_id);
  }
  if (skip_spills) {
  ...
  for (int cpuid = bit_first(cpu_map); cpuid >= 0;
       cpuid = bit_next(cpu_map, cpuid)) {
    processor_t processor = processor_array[cpuid];
   ...
   * choose_processor() takes pset lock A, sees the pending_AST_URGENT_cpu_mask
   * set for a processor in that set and finds no suitable candiate CPU, so it
   * drops pset lock A and tries to take pset lock B.  Meanwhile the
   * pending_AST_URGENT_cpu_mask CPU is looking for a thread to run and holds
   * pset lock B. It doesn't find any threads (because the candidate thread
   * isn't yet on any run queue), so drops lock B, takes lock A again to clear
   * the pending_AST_URGENT_cpu_mask bit, and keeps running the current (far
   * deadline) thread. choose_processor() now has lock B and can only find the
   * lowest count processor in set B so enqueues it on set B's run queue but
   * doesn't IPI anyone. (The lowest count includes all threads, near and far
   * deadlines, so will prefer a low count of earlier deadlines to a high count
   * of far deadlines, which is suboptimal for EDF scheduling. To make a better
   * choice we would need to know how many threads with earlier deadlines than
   * the candidate thread exist on each pset's run queue. But even if we chose
   * the better run queue, we still wouldn't send an IPI in this case.)
   *
   * The migitation is to also look for suitable CPUs that have their
   * pending_AST_URGENT_cpu_mask bit set where there are no earlier deadline
   * threads on the run queue of that pset.
   */
  if (include_ast_urgent_pending_cpus &&
      (rt_runq_earliest_deadline(pset) > furthest_deadline)) {
    cpu_map = pset_available_cpumap(pset) & pset->pending_AST_URGENT_cpu_mask;
...
bool rt_clear_pending_spill(processor_t processor, int reason) {
  ...
  if (bit_clear_if_set(pset->rt_pending_spill_cpu_mask, processor->cpu_id)) {
    KTRC(MACHDBG_CODE(DBG_MACH_SCHED, MACH_RT_SIGNAL_SPILL) | DBG_FUNC_END,
         processor->cpu_id, pset->rt_pending_spill_cpu_mask, 0, reason);
    return true;
  } else {
    return false;
  }
...
void check_rt_runq_consistency(rt_queue_t rt_run_queue, thread_t thread) {
  ...
  for (int pri = BASEPRI_RTQUEUES; pri <= MAXPRI; pri++) {
    ...
    qe_foreach(iter, queue) {
      ...
      if (iter == queue_first(queue)) {
        assert(rt_runq->pri_earliest_deadline ==
  ...
  assert(os_atomic_load_wide(&rt_run_queue->earliest_deadline, relaxed) ==
         earliest_deadline);
...
static bool rt_runq_enqueue(rt_queue_t rt_run_queue, thread_t thread,
                            processor_t processor) {
  ...
  if (earliest && (deadline < os_atomic_load_wide(
                                  &rt_run_queue->earliest_deadline, relaxed))) {
    os_atomic_store_wide(&rt_run_queue->earliest_deadline, deadline, relaxed);
    os_atomic_store(&rt_run_queue->constraint, thread->realtime.constraint,
                    relaxed);
    os_atomic_store(&rt_run_queue->ed_index, pri - BASEPRI_RTQUEUES, relaxed);
  }

  SCHED_STATS_RUNQ_CHANGE(&rt_run_queue->runq_stats,
                          os_atomic_load(&rt_run_queue->count, relaxed));
  rt_runq->pri_count++;
...
uint64_t rt_runq_earliest_deadline(processor_set_t pset) {
...
bool rt_runq_is_low_latency(processor_set_t pset) {
  return os_atomic_load(&pset->rt_runq.constraint, relaxed) <=
...
thread_t rt_runq_dequeue(rt_queue_t rt_run_queue) {
  ...
  if (!sched_rt_runq_strict_priority) {
    ...
    if (ed_index != i) {
      ...
      thread_t ed_thread =
          qe_queue_first(&ed_runq->pri_queue, struct thread, runq_links);
      thread_t hi_thread =
          qe_queue_first(&rt_runq->pri_queue, struct thread, runq_links);

      if (ed_thread->realtime.computation + hi_thread->realtime.computation +
              rt_deadline_epsilon <
  ...
  int ed_index = NOPRI;
  thread_t new_thread =
      qe_dequeue_head(&rt_runq->pri_queue, struct thread, runq_links);
  SCHED_STATS_RUNQ_CHANGE(&rt_run_queue->runq_stats,
                          os_atomic_load(&rt_run_queue->count, relaxed));
  ...
  os_atomic_store_wide(&rt_run_queue->earliest_deadline, earliest_deadline,
                       relaxed);
