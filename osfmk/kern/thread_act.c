...
#define task_has_tss_entitlement(task)                                         \
  IOTaskHasEntitlement((task), "com.apple.private.thread-set-state")

static inline bool thread_set_state_allowed(thread_t thread, int flavor,
  ...
  if ((thread->options & TH_IN_MACH_EXCEPTION) && target_task != curr_task &&
  ...
  if ((flags & TSSF_CHECK_ENTITLEMENT) &&
      !(thread->options &
        TH_IN_MACH_EXCEPTION) && /* Allowed for now - rdar://103085786 */
      ipc_should_apply_policy(target_pol, IPC_POLICY_ENHANCED_V1) &&
      FLAVOR_MODIFIES_CORE_CPU_REGISTERS(
          flavor) && /* only care about locking down PC/LR */
      !task_has_tss_entitlement(curr_task)) {
    mach_port_guard_exception(flavor, 0, kGUARD_EXC_THREAD_SET_STATE);
    return false;
  }
 ...
 */
void thread_start_in_assert_wait(thread_t thread, struct waitq *waitq,
                                 event64_t event,
  ...
  wait_result = waitq_assert_wait64_locked(
      waitq, event, interruptible, TIMEOUT_URGENCY_SYS_NORMAL,
      TIMEOUT_WAIT_FOREVER, TIMEOUT_NO_LEEWAY, thread);
 ...
 * until it hits AST.
 */
kern_return_t thread_terminate_internal(thread_t thread) {
  ...
  ipc_thread_port_unpin(
      get_thread_ro(thread)->tro_ports[THREAD_FLAVOR_CONTROL]);
 ...
 */
kern_return_t thread_terminate_from_user(thread_t thread) {
  if (thread == THREAD_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
 ...
 */
kern_return_t thread_terminate_immovable(thread_t thread) {
 ...
 */
void thread_hold(thread_t thread) {
  if (thread->suspend_count++ == 0) {
    ...
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_SUSPENSION, MACH_THREAD_SUSPEND) | DBG_FUNC_NONE,
...
void thread_release(thread_t thread) {
  ...
  if (--thread->suspend_count == 0) {
    ...
    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_SUSPENSION, MACH_THREAD_RESUME) |
                     DBG_FUNC_NONE,
                 thread->thread_id);
  }
}
...
kern_return_t thread_info(thread_t thread, thread_flavor_t flavor,
  ...
  if (thread->active || thread->inspection) {
    result = thread_info_internal(thread, flavor, thread_info_out,
                                  thread_info_count);
  } else {
    result = KERN_TERMINATED;
  }
...
thread_get_state_internal(thread_t thread, int flavor,
...
/* No prototype, since thread_act_server.h has the _to_user version if
 * KERNEL_SERVER */

kern_return_t thread_get_state(thread_t thread, int flavor,
...
kern_return_t thread_get_state(thread_t thread, int flavor,
                               ...
                               mach_msg_type_number_t *state_count) /*IN/OUT*/
{
  return thread_get_state_internal(thread, flavor, state, state_count,
                                   TSSF_FLAGS_NONE);
}

kern_return_t
thread_get_state_to_user(thread_t thread, int flavor,
                         thread_state_t state, /* pointer to OUT array */
                         mach_msg_type_number_t *state_count) /*IN/OUT*/
{
  return thread_get_state_internal(thread, flavor, state, state_count,
                                   TSSF_TRANSLATE_TO_USER);
}
 ...
 */
static inline kern_return_t thread_set_state_internal(
    thread_t thread, int flavor, thread_state_t state,
    mach_msg_type_number_t state_count, thread_state_t old_state,
    mach_msg_type_number_t old_state_count, thread_set_status_flags_t flags) {
  kern_return_t result = KERN_SUCCESS;
  ...
  if (thread->active) {
    if (from_user) {
      result = machine_thread_state_convert_from_user(thread, flavor, state,
                                                      state_count, old_state,
                                                      old_state_count, flags);
    ...
    if (thread != current_thread()) {
      ...
      if (thread_stop(thread, TRUE)) {
        thread_mtx_lock(thread);
        result = machine_thread_set_state(thread, flavor, state, state_count);
        thread_unstop(thread);
      } else {
        thread_mtx_lock(thread);
        result = KERN_ABORTED;
...
/* No prototype, since thread_act_server.h has the _from_user version if
 * KERNEL_SERVER */
kern_return_t thread_set_state(thread_t thread, int flavor,
                               thread_state_t state,
                               mach_msg_type_number_t state_count);
...
kern_return_t thread_set_state(thread_t thread, int flavor,
                               thread_state_t state,
                               mach_msg_type_number_t state_count) {
  return thread_set_state_internal(thread, flavor, state, state_count, NULL, 0,
                                   TSSF_FLAGS_NONE);
}

kern_return_t thread_set_state_from_user(thread_t thread, int flavor,
                                         thread_state_t state,
                                         mach_msg_type_number_t state_count) {
  return thread_set_state_internal(thread, flavor, state, state_count, NULL, 0,
                                   TSSF_TRANSLATE_TO_USER |
                                       TSSF_CHECK_ENTITLEMENT);
}

kern_return_t
thread_convert_thread_state(thread_t thread, int direction,
                            ...
                            mach_msg_type_number_t *out_state_count) /*IN/OUT*/
{
  ...
  kr = machine_thread_state_convert_from_user(
      from_thread, flavor, in_state, state_count, NULL, 0, TSSF_FLAGS_NONE);
  ...
  kr = machine_thread_state_convert_to_user(to_thread, flavor, in_state,
                                            &state_count, TSSF_PRESERVE_FLAGS);
 ...
 */
kern_return_t thread_state_initialize(thread_t thread) {
  kern_return_t result = KERN_SUCCESS;
...
kern_return_t thread_dup(thread_t target) {
...
kern_return_t thread_dup2(thread_t source, thread_t target) {
  kern_return_t result = KERN_SUCCESS;
  uint32_t active = 0;
...
kern_return_t thread_setstatus_from_user(thread_t thread, int flavor,
                                         thread_state_t tstate,
                                         mach_msg_type_number_t count,
...
kern_return_t thread_getstatus_to_user(thread_t thread, int flavor,
                                       thread_state_t tstate,
                                       mach_msg_type_number_t *count,
                                       thread_set_status_flags_t flags) {
  return thread_get_state_internal(thread, flavor, tstate, count,
                                   flags | TSSF_TRANSLATE_TO_USER);
}
 ...
 */
__attribute__((noreturn)) static void thread_suspended(__unused void *parameter,
...
/*
 * thread_apc_ast - handles AST_APC and drives thread suspension and
 * termination. Called with nothing locked.  Returns (if it returns) the same
 * way.
 */
void thread_apc_ast(thread_t thread) {
  thread_mtx_lock(thread);
...
exception_deliver(thread_t thread, exception_type_t exception,
                  mach_exception_data_t code, mach_msg_type_number_t codeCnt,
                  struct exception_action *excp, lck_mtx_t *mutex);

kern_return_t thread_raise_exception(thread_t thread,
                                     exception_type_t exception,
                                     natural_t code_count, int64_t code,
                                     int64_t sub_code) {
  task_t task;
  ...
  kern_return_t kr =
      exception_deliver(thread, exception, codes, code_count,
                        host_priv->exc_actions, &host_priv->lock);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
...
void thread_debug_return_to_user_ast(thread_t thread) {
#pragma unused(thread)
#if MACH_ASSERT
  if ((thread->sched_flags & TH_SFLAG_RW_PROMOTED) ||
      thread->rwlock_count > 0) {
    panic("Returning to userspace with rw lock held, thread %p sched_flag %u "
          "rwlock_count %d",
          thread, thread->sched_flags, thread->rwlock_count);
  }

  if ((thread->sched_flags & TH_SFLAG_FLOOR_PROMOTED) ||
      thread->priority_floor_count > 0) {
    panic("Returning to userspace with floor boost set, thread %p sched_flag "
          "%u priority_floor_count %d",
          thread, thread->sched_flags, thread->priority_floor_count);
  }
...
kern_return_t act_set_state(thread_t thread, int flavor, thread_state_t state,
                            mach_msg_type_number_t count);
...
kern_return_t act_set_state_from_user(thread_t thread, int flavor,
                                      thread_state_t state,
                                      mach_msg_type_number_t count) {
  if (thread == current_thread()) {
    return KERN_INVALID_ARGUMENT;
...
/* Prototype, see justification above */
kern_return_t act_get_state(thread_t thread, int flavor, thread_state_t state,
                            mach_msg_type_number_t *count);
...
kern_return_t act_get_state_to_user(thread_t thread, int flavor,
                                    thread_state_t state,
                                    mach_msg_type_number_t *count) {
  if (thread == current_thread()) {
    return KERN_INVALID_ARGUMENT;
 ...
 */
static void act_set_ast_async(thread_t thread, ast_t ast) {
  thread_ast_set(thread, ast);
...
void act_set_debug_assert(void) {
  thread_t thread = current_thread();
...
void act_set_astkevent(thread_t thread, uint16_t bits) {
...
uint16_t act_clear_astkevent(thread_t thread, uint16_t bits) {
  /*
...
bool act_set_ast_reset_pcs(task_t task, thread_t thread) {
  processor_t processor;
  ...
  if (thread == current_thread()) {
    ...
    if (!thread->active) {
      ...
      if (thread->t_rr_state.trr_fault_state) {
        thread->t_rr_state.trr_fault_state = TRR_FAULT_OBSERVED;
        needs_wait = true;
      }
      thread_ast_set(thread, AST_RESET_PCS);
    }
...
void act_set_kperf(thread_t thread) {
  /* safety check */
  if (thread != current_thread()) {
...
void act_set_astledger(thread_t thread) { act_set_ast(thread, AST_LEDGER); }
 ...
 */
void act_set_astledger_async(thread_t thread) {
  act_set_ast_async(thread, AST_LEDGER);
}

void act_set_io_telemetry_ast(thread_t thread) {
  act_set_ast(thread, AST_TELEMETRY_IO);
}

void act_set_macf_telemetry_ast(thread_t thread) {
  act_set_ast(thread, AST_TELEMETRY_MACF);
}

void act_set_astproc_resource(thread_t thread) {
