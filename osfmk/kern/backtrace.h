...
__options_decl(
    backtrace_info_t, uint32_t,
    {
        BTI_NONE = 0x0,
        // BTI_64_BIT is set when the backtrace is made up of 64-bit addresses.
        BTI_64_BIT = 0x1,
        // BTI_TRUNCATED is set when the backtrace has been truncated, either
        // due to an error copying data, an invalid frame pointer, or running
        // out of buffer space.
        BTI_TRUNCATED = 0x2,
    });
...
unsigned int backtrace(uintptr_t *bt, unsigned int btlen,
                       ...
                       backtrace_info_t *info_out) __attribute__((noinline));

// backtrace_pack_t changes the packing scheme for backtraces.
__enum_decl(
    backtrace_pack_t, uint32_t,
    {
