 ...
 */
#define KCDATA_ITEM_HEADER_SIZE                                                \
...
#define KCDATA_ITEM_ARRAY_GET_EL_TYPE(item)                                    \
  kcdata_iter_array_elem_type(KCDATA_ITEM_ITER(item))
#define KCDATA_ITEM_ARRAY_GET_EL_COUNT(item)                                   \
  kcdata_iter_array_elem_count(KCDATA_ITEM_ITER(item))
#define KCDATA_ITEM_ARRAY_GET_EL_SIZE(item)                                    \
  kcdata_iter_array_elem_size(KCDATA_ITEM_ITER(item))
#define KCDATA_CONTAINER_ID(item)                                              \
  kcdata_iter_container_id(KCDATA_ITEM_ITER(item))
#define KCDATA_ITEM_NEXT_HEADER(itemx)                                         \
  (kcdata_iter_next(KCDATA_ITEM_ITER(itemx)).item)
#define KCDATA_ITEM_FOREACH(head)                                              \
  for (; KCDATA_ITEM_TYPE(head) != KCDATA_TYPE_BUFFER_END;                     \
       (head) = KCDATA_ITEM_NEXT_HEADER(head))
#define KCDATA_ITEM_DATA_PTR(item) kcdata_iter_payload(KCDATA_ITEM_ITER(item))
#define KCDATA_ITEM_FIND_TYPE(itemx, type)                                     \
  (kcdata_iter_find_type(KCDATA_ITEM_ITER(itemx), type).item)
#define kcdata_get_container_type(buffer)                                      \
  kcdata_iter_container_type(KCDATA_ITEM_ITER(buffer))
#define kcdata_get_data_with_desc(buf, desc, data)                             \
  kcdata_iter_get_data_with_desc(KCDATA_ITEM_ITER(buf), desc, data, NULL)
/* Do not use these macros! */
...
#define KCFLAG_ALLOC_CALLBACK 0x8
  ...
  struct kcdata_descriptor *(*kcd_alloc_callback)(struct kcdata_descriptor *,
                                                  size_t);
};
...
kcdata_descriptor_t kcdata_memory_alloc_init(mach_vm_address_t crash_data_p,
...
kern_return_t kcdata_add_uint64_with_description(kcdata_descriptor_t crashinfo,
                                                 uint64_t data,
                                                 const char *description);
kern_return_t kcdata_add_uint32_with_description(kcdata_descriptor_t crashinfo,
                                                 uint32_t data,
                                                 const char *description);
...
kern_return_t kcdata_init_compress(kcdata_descriptor_t, int hdr_tag,
