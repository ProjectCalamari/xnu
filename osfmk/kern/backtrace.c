...
#if __x86_64__
static void _backtrace_packed_out_of_reach(void) {
...
static size_t _backtrace_pack_addr(backtrace_pack_t packing, uint8_t *dst,
                                   size_t dst_size, uintptr_t addr) {
  switch (packing) {
  case BTP_NONE:
...
backtrace_internal(backtrace_pack_t packing, uint8_t *bt, size_t btsize,
                   void *start_frame, int64_t addr_offset,
...
#define IN_STK_BOUNDS(__addr)                                                  \
  ...
  while (fp != NULL && size_used < btsize) {
    ...
    in_valid_stack =
        IN_STK_BOUNDS(next_fp) || ml_addr_in_non_xnu_stack((uintptr_t)next_fp);
    ...
    uintptr_t pc =
        (uintptr_t)ptrauth_strip((void *)ret_addr, ptrauth_key_return_address);
    ...
    pc += addr_offset;
    size_used +=
        _backtrace_pack_addr(packing, bt + size_used, btsize - size_used, pc);
...
static kern_return_t interrupted_kernel_pc_fp(uintptr_t *pc, uintptr_t *fp) {
...
unsigned int __attribute__((noinline)) backtrace(uintptr_t *bt,
  ...
  size_t size = backtrace_internal(
      BTP_NONE, (uint8_t *)bt, max_frames * sizeof(uintptr_t),
...
size_t backtrace_packed(backtrace_pack_t packing, uint8_t *bt, size_t btsize,
  ...
  size_t written_size =
      backtrace_internal(packing, (uint8_t *)bt, btsize, (void *)start_frame,
                         ctl ? ctl->btc_addr_offset : 0, info_out);
  return written_size + size_adj;
}

// Convert an array of addresses to a packed representation.
size_t backtrace_pack(backtrace_pack_t packing, uint8_t *dst, size_t dst_size,
...
unsigned int backtrace_unpack(backtrace_pack_t packing, uintptr_t *dst,
                              unsigned int dst_len, const uint8_t *src,
                              size_t src_size) {
  switch (packing) {
  case BTP_NONE:;
...
static errno_t _backtrace_copyin(void *__unused ctx, void *dst, user_addr_t src,
                                 size_t size) {
...
unsigned int backtrace_user(uintptr_t *bt, unsigned int max_frames,
...
#error "unsupported architecture"
  ...
  while (fp != 0 && frame_index < max_frames) {
    ...
    if (user_64 && async_frame) {
      ...
      error = copy(ctx, (char *)&async_ctx, async_ctx_ptr, sizeof(async_ctx));
      if (error) {
        goto out;
      }
#if defined(HAS_APPLE_PAC)
      async_frame_addr = (uintptr_t)ptrauth_strip(
          (void *)async_ctx, ptrauth_key_process_dependent_data);
#else  // defined(HAS_APPLE_PAC)
...
#if defined(HAS_APPLE_PAC)
    next_fp =
        (uintptr_t)ptrauth_strip((void *)next_fp, ptrauth_key_frame_pointer);
#endif // defined(HAS_APPLE_PAC)
    if (INVALID_USER_FP(next_fp)) {
      break;
    }

    // User space stacks generally grow down, but in some cases can jump to a
    // different stack. Skip the check that the frame pointer moves downward
    // here.
