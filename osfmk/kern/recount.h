...
// Recount maintains counters for resources used by software, like CPU time and
// cycles. These counters are tracked at different levels of granularity
// depending on what execution bucket they're tracked in. For instance, while
// threads only differentiate on the broad CPU kinds due to memory constraints,
// the fewer number of tasks are free to use more memory and accumulate counters
// per-CPU.
//
// At context-switch, the scheduler calls `recount_switch_thread` to update the
// counters. The difference between the current counter values and per-CPU
// snapshots are added to each thread. On modern systems with fast timebase
// reads, the counters are also updated on entering and exiting the kernel.

#pragma mark - config

// A domain of the system's CPU topology, used as granularity when tracking
// counter values.
__enum_decl(
    recount_topo_t, unsigned int,
    {
        // Attribute counts to the entire system, i.e. only a single counter.
        // Note that mutual exclusion must be provided to update this kind of
        // counter.
        RCT_TOPO_SYSTEM,
        // Attribute counts to the CPU they accumulated on.
        // Mutual exclusion is not required to update this counter, but
...
#define RECOUNT_PLAN_DECLARE(_name) extern const struct recount_plan _name;
...
struct recount_track {
  // Used to synchronize updates so multiple values appear to be updated
  // atomically.
  uint32_t rt_pad;
  uint32_t rt_sync;
  ...
  struct recount_usage {
    ...
    // This is not a "metric" because it is sampled out-of-band by ApplePMGR
    // through CLPC.
...
// Attribute tracks to usage structures, to read their values for typical
...
void recount_sum_usage_and_isolate_cpu_kind(recount_plan_t plan,
                                            struct recount_usage *usage_list,
                                            recount_cpu_kind_t kind,
                                            struct recount_usage *sum,
                                            struct recount_usage *only_kind);
...
// Get the sum of all terminated threads in the task (not including active
// threads).
...
struct recount_work_interval {
  ...
  // threads participating in the work interval, according to
...
// Get the sum of all currently-exited tasks in the coalition, and a separate
// P-only structure.
