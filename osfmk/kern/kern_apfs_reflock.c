...
void kern_apfs_reflock_destroy(kern_apfs_reflock_t reflock) {
  ...
  if (reflock->kern_apfs_rl_allocated == 1) {
    panic("kern_apfs_reflock_t %p was allocated. kern_apfs_reflock_free should "
          "be called instead of kern_apfs_reflock_destroy",
          reflock);
  }
  if (reflock->kern_apfs_rl_owner != 0) {
    panic("kern_apfs_reflock_t %p: destroying a reflock currently locked by "
          "ctid %d",
          reflock, reflock->kern_apfs_rl_owner);
  }
  if (reflock->kern_apfs_rl_wake != 0) {
    panic("kern_apfs_reflock_t %p: destroying a reflock with threads currently "
          "waiting or in the process of waiting",
          reflock);
  }
...
kern_apfs_reflock_t kern_apfs_reflock_alloc_init(void) {
  kern_apfs_reflock_t reflock =
      zalloc_flags(KT_KERN_APFSREFLOCK, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  reflock->kern_apfs_rl_allocated = 1;
  return reflock;
}

static void kern_apfs_reflock_free_internal(kern_apfs_reflock_t reflock) {
  assert(reflock->kern_apfs_rl_waiters == 0);
...
static void inline kern_apfs_reflock_check_valid(kern_apfs_reflock_t reflock) {
  if (reflock->kern_apfs_rl_data.cond64_data == KERN_APFS_REFLOCK_DESTROYED) {
    panic("reflock %p was destoryed", reflock);
  }
  if (reflock->kern_apfs_rl_allocated == 1 &&
      reflock->kern_apfs_rl_delayed_free == 1) {
...
void kern_apfs_reflock_free(kern_apfs_reflock_t reflock) {
  ...
  if (reflock->kern_apfs_rl_allocated == 0) {
    panic("kern_apfs_reflock_t %p was not allocated. kern_apfs_reflock_destroy "
          "should be called instead of kern_apfs_reflock_free",
          reflock);
  }
   ...
   */
  os_atomic_rmw_loop(
      &reflock->kern_apfs_rl_data.cond64_data,
      old_reflock.kern_apfs_rl_data.cond64_data,
      new_reflock.kern_apfs_rl_data.cond64_data, release, {
        new_reflock = old_reflock;

        if (reflock->kern_apfs_rl_delayed_free == 1) {
          panic(
              "kern_apfs_reflock_t %p is already in the process of being freed",
              reflock);
        }
        if (reflock->kern_apfs_rl_owner != 0) {
          panic("kern_apfs_reflock_t %p: freeing a reflock currently locked by "
                "ctid %d",
                reflock, reflock->kern_apfs_rl_owner);
        }
        assert(reflock->kern_apfs_rl_wake == 0);
...
bool kern_apfs_reflock_try_get_ref(struct kern_apfs_reflock *reflock,
  ...
  if (force && try_lock) {
    panic("Cannot use KERN_APFS_REFLOCK_IN_FORCE and "
          "KERN_APFS_REFLOCK_IN_LOCK_IF_FIRST together");
  }
  ...
  *out_flags = KERN_APFS_REFLOCK_OUT_DEFAULT;
  ...
  os_atomic_rmw_loop(
      &reflock->kern_apfs_rl_data.cond64_data,
      old_reflock.kern_apfs_rl_data.cond64_data,
      new_reflock.kern_apfs_rl_data.cond64_data, acquire, {
        new_reflock = old_reflock;
        locked = false;
        /*
        ...
        if (old_reflock.kern_apfs_rl_owner != 0 &&
          ...
          if (will_wait && reflock->kern_apfs_rl_allocated == 1) {
             ...
             */
            if (old_reflock.kern_apfs_rl_waiters ==
                KERN_APFS_REFLOCK_MAXWAITERS) {
              panic("kern_apfs_reflock: too many waiters for %p thread %p",
                    reflock, current_thread());
            }
            new_reflock.kern_apfs_rl_waiters =
                old_reflock.kern_apfs_rl_waiters + 1;
          } else {
            /*
             * Caller does not want to wait or we do not need to remember how
          ...
          if (old_reflock.kern_apfs_rl_count == KERN_APFS_REFLOCK_MAXREFCOUNT) {
            panic("kern_apfs_reflock: too many refs for %p thread %p", reflock,
                  current_thread());
          }
...
bool kern_apfs_reflock_try_put_ref(kern_apfs_reflock_t reflock,
  ...
  if (force && try_lock) {
    panic("Cannot use KERN_APFS_REFLOCK_IN_FORCE and "
          "KERN_APFS_REFLOCK_IN_LOCK_IF_LAST together");
  }
  ...
  os_atomic_rmw_loop(
      ...
      new_reflock.kern_apfs_rl_data.cond64_data, release, {
        if (old_reflock.kern_apfs_rl_count == 0) {
          panic("kern_apfs_reflock: over releasing reflock %p thread %p",
                reflock, current_thread());
        }
      ...
      new_reflock.kern_apfs_rl_data.cond64_data, acquire, {
        ...
        if (old_reflock.kern_apfs_rl_owner != 0) {
          if (old_reflock.kern_apfs_rl_owner == my_ctid) {
            panic("Trying to lock a reflock owned by the same thread %p, "
                  "reflock %p",
                  current_thread(), reflock);
          }
          acquired = false;
          if (will_wait && reflock->kern_apfs_rl_allocated == 1) {
            /*
...
wait_result_t kern_apfs_reflock_wait_for_unlock(kern_apfs_reflock_t reflock,
  ...
  os_atomic_rmw_loop(
      &reflock->kern_apfs_rl_data.cond64_data,
      old_reflock.kern_apfs_rl_data.cond64_data,
      new_reflock.kern_apfs_rl_data.cond64_data, relaxed, {
        new_reflock = old_reflock;
        free = false;
        ...
        if (old_reflock.kern_apfs_rl_allocated == 1 &&
            old_reflock.kern_apfs_rl_waiters == 0) {
          panic("kern_apfs_reflock: kern_apfs_rl_waiters are 0 when trying to "
                "wait reflock %p thread %p. Probably a try* function with a "
                "positive will_wait wasn't called before waiting.",
                reflock, current_thread());
        }
        ...
        if (old_reflock.kern_apfs_rl_owner != 0) {
          if (old_reflock.kern_apfs_rl_owner == my_ctid) {
            panic("Trying to wait on a reflock owned by the same thread %p, "
                  "reflock %p",
                  current_thread(), reflock);
          }
          /*
          ...
          if (old_reflock.kern_apfs_rl_allocated == 1) {
            new_reflock.kern_apfs_rl_waiters =
                old_reflock.kern_apfs_rl_waiters - 1;
            if (old_reflock.kern_apfs_rl_delayed_free == 1 &&
                new_reflock.kern_apfs_rl_waiters == 0) {
              free = true;
            }
          } else {
            os_atomic_rmw_loop_give_up(break);
  ...
  /*
   * We want to sleep only if we see an owner still set and if the wakeup flag
   * is set. If the owner observed is different from the one saved we want to
   * not sleep.
   */
  ret = cond_sleep_with_inheritor64_mask(
      (cond_swi_var_t)reflock, new_reflock.kern_apfs_rl_data,
      KERN_APFS_SLEEP_DEBOUNCE_MASK, interruptible, deadline);
  ...
  if (new_reflock.kern_apfs_rl_allocated == 1) {
    os_atomic_rmw_loop(&reflock->kern_apfs_rl_data.cond64_data,
                       old_reflock.kern_apfs_rl_data.cond64_data,
                       new_reflock.kern_apfs_rl_data.cond64_data, acquire, {
                         new_reflock = old_reflock;
  ...
  if (new_reflock.kern_apfs_rl_delayed_free == 1 &&
      new_reflock.kern_apfs_rl_waiters == 0) {
    kern_apfs_reflock_free_internal(reflock);
  }
...
void kern_apfs_reflock_unlock(kern_apfs_reflock_t reflock) {
  ...
  os_atomic_rmw_loop(
      ...
      new_reflock.kern_apfs_rl_data.cond64_data, release, {
        if (old_reflock.kern_apfs_rl_owner != my_ctid) {
          panic("Unlocking swiref_t %p from thread ctid %u owned by ctid %u",
                reflock, my_ctid, old_reflock.kern_apfs_rl_owner);
        }
...
uint64_t kern_apfs_reflock_read_ref(kern_apfs_reflock_t reflock) {
  ...
  reflock_value.kern_apfs_rl_data.cond64_data =
      os_atomic_load(&reflock->kern_apfs_rl_data.cond64_data, relaxed);
