...
static_assert(TH_BUCKET_SCHED_MAX == 6,
              "Ensure layout of sched_clutch_dbg_thread_select_packed can fit "
              "root bucket bitmasks");
static_assert(sizeof(sched_clutch_dbg_thread_select_packed_t) <=
                  sizeof(uint64_t),
              "Ensure sched_clutch_dbg_thread_select_packed_t can fit in one "
              "tracepoint argument");
...
static void sched_clutch_root_bucket_init(sched_clutch_root_bucket_t,
                                          sched_bucket_t, bool);
...
static_assert(SCHED_CLUTCH_TRAVERSE_CHECK_PREEMPT < (1 << 3),
              "Ensure traverse mode can be encoded within 3 bits of "
              "sched_clutch_dbg_thread_select_packed_t");
static sched_clutch_root_bucket_t sched_clutch_root_highest_root_bucket(
    sched_clutch_root_t, uint64_t, sched_clutch_highest_root_bucket_type_t,
    sched_clutch_root_bucket_t, thread_t, bool *, sched_clutch_traverse_mode_t,
    sched_clutch_dbg_thread_select_packed_t *);
...
static uint64_t
sched_clutch_root_bucket_deadline_calculate(sched_clutch_root_bucket_t,
                                            uint64_t);
static void sched_clutch_root_bucket_deadline_update(sched_clutch_root_bucket_t,
                                                     sched_clutch_root_t,
                                                     uint64_t, bool);
static int sched_clutch_root_highest_runnable_qos(
    sched_clutch_root_t, sched_clutch_highest_root_bucket_type_t);
...
static void sched_clutch_bucket_hierarchy_insert(sched_clutch_root_t,
                                                 sched_clutch_bucket_t,
                                                 sched_bucket_t, uint64_t,
                                                 sched_clutch_bucket_options_t);
static void sched_clutch_bucket_hierarchy_remove(sched_clutch_root_t,
                                                 sched_clutch_bucket_t,
                                                 sched_bucket_t, uint64_t,
                                                 sched_clutch_bucket_options_t);
static boolean_t sched_clutch_bucket_runnable(sched_clutch_bucket_t,
                                              sched_clutch_root_t, uint64_t,
                                              sched_clutch_bucket_options_t);
static boolean_t sched_clutch_bucket_update(sched_clutch_bucket_t,
                                            sched_clutch_root_t, uint64_t,
                                            sched_clutch_bucket_options_t);
static void sched_clutch_bucket_empty(sched_clutch_bucket_t,
                                      sched_clutch_root_t, uint64_t,
                                      sched_clutch_bucket_options_t);
static uint8_t sched_clutch_bucket_pri_calculate(sched_clutch_bucket_t,
                                                 uint64_t);

/* Clutch bucket group level properties management */
static void
sched_clutch_bucket_group_cpu_usage_update(sched_clutch_bucket_group_t,
                                           uint64_t);
static void sched_clutch_bucket_group_cpu_adjust(sched_clutch_bucket_group_t,
                                                 uint8_t);
static void
    sched_clutch_bucket_group_pri_shift_update(sched_clutch_bucket_group_t);
static uint8_t
sched_clutch_bucket_group_pending_ageout(sched_clutch_bucket_group_t, uint64_t);
static uint32_t
    sched_clutch_bucket_group_run_count_inc(sched_clutch_bucket_group_t);
static uint32_t
    sched_clutch_bucket_group_run_count_dec(sched_clutch_bucket_group_t);
static uint8_t sched_clutch_bucket_group_interactivity_score_calculate(
    sched_clutch_bucket_group_t, uint64_t);
...
static boolean_t sched_clutch_thread_insert(sched_clutch_root_t, thread_t,
                                            integer_t);
static void sched_clutch_thread_remove(sched_clutch_root_t, thread_t, uint64_t,
                                       sched_clutch_bucket_options_t);
static thread_t sched_clutch_hierarchy_thread_highest(
    sched_clutch_root_t, processor_t, thread_t, sched_clutch_traverse_mode_t);
...
static sched_clutch_bucket_t sched_clutch_root_bucket_highest_clutch_bucket(
    sched_clutch_root_t, sched_clutch_root_bucket_t,
    processor_t _Nullable processor, thread_t _Nullable prev_thread,
...
static inline sched_clutch_bucket_t
    sched_clutch_bucket_for_thread(sched_clutch_root_t, thread_t);
static inline sched_clutch_bucket_group_t
    sched_clutch_bucket_group_for_thread(thread_t);
...
/* Special warp deadline value to indicate that the bucket has not used any warp
 * yet */
 ...
 */
static void sched_clutch_us_to_abstime(uint32_t *us_vals,
  ...
  for (int i = 0; i < TH_BUCKET_SCHED_MAX; i++) {
    if (us_vals[i] == SCHED_CLUTCH_INVALID_TIME_32) {
      ...
      clock_interval_to_absolutetime_interval(us_vals[i], NSEC_PER_USEC,
                                              &abstime_vals[i]);
    }
  }
}
...
#define SCHED_CLUTCH_DBG_THR_COUNT_PACK(a, b, c)                               \
  ((uint64_t)c | ((uint64_t)b << 16) | ((uint64_t)a << 32))

#if DEVELOPMENT || DEBUG

kern_return_t
sched_clutch_thread_group_cpu_time_for_thread(thread_t thread, int sched_bucket,
                                              uint64_t *cpu_stats) {
  if (sched_bucket < 0 || sched_bucket >= TH_BUCKET_MAX) {
    return KERN_INVALID_ARGUMENT;
  }
  sched_clutch_bucket_group_t clutch_bucket_group =
      &sched_clutch_for_thread(thread)->sc_clutch_groups[sched_bucket];
  sched_clutch_bucket_cpu_data_t scb_cpu_data;
  scb_cpu_data.scbcd_cpu_data_packed = os_atomic_load_wide(
      &clutch_bucket_group->scbg_cpu_data.scbcd_cpu_data_packed, relaxed);
 ...
 */
static inline void
sched_clutch_hierarchy_locked_assert(sched_clutch_root_t root_clutch) {
...
static inline void
sched_clutch_hierarchy_locked_assert(__unused sched_clutch_root_t root_clutch) {
}
 ...
 */
static void sched_clutch_thr_count_inc(uint16_t *thr_count) {
 ...
 */
static void sched_clutch_thr_count_dec(uint16_t *thr_count) {
  if (__improbable(os_dec_overflow(thr_count))) {
    panic("sched_clutch thread count underflowed!");
  }
}

static sched_bucket_t sched_convert_pri_to_bucket(uint8_t priority) {
 ...
 */
static sched_bucket_t sched_clutch_thread_bucket_map(thread_t thread, int pri) {
 ...
 */
static void sched_clutch_root_init(sched_clutch_root_t root_clutch,
  ...
  for (cluster_shared_rsrc_type_t shared_rsrc_type =
           CLUSTER_SHARED_RSRC_TYPE_MIN;
       shared_rsrc_type < CLUSTER_SHARED_RSRC_TYPE_COUNT; shared_rsrc_type++) {
  ...
  /* Initialize the queue which maintains all runnable clutch_buckets for
   * timesharing purposes */
  queue_init(&root_clutch->scr_clutch_buckets);

  /* Initialize the priority queue which maintains all runnable foreign clutch
   * buckets */
  priority_queue_init(&root_clutch->scr_foreign_buckets);
  bzero(&root_clutch->scr_cumulative_run_count,
        sizeof(root_clutch->scr_cumulative_run_count));
  ...
  for (uint32_t i = 0; i < TH_BUCKET_SCHED_MAX; i++) {
    sched_clutch_root_bucket_init(&root_clutch->scr_unbound_buckets[i], i,
                                  false);
 ...
 */
static void
sched_clutch_bucket_runq_init(sched_clutch_bucket_runq_t clutch_buckets_rq) {
 ...
 */
static boolean_t
sched_clutch_bucket_runq_empty(sched_clutch_bucket_runq_t clutch_buckets_rq) {
...
sched_clutch_bucket_runq_peek(sched_clutch_bucket_runq_t clutch_buckets_rq) {
  if (clutch_buckets_rq->scbrq_count > 0) {
    circle_queue_t queue =
        &clutch_buckets_rq->scbrq_queues[clutch_buckets_rq->scbrq_highq];
 ...
 */
static void
sched_clutch_bucket_runq_enqueue(sched_clutch_bucket_runq_t clutch_buckets_rq,
                                 sched_clutch_bucket_t clutch_bucket,
                                 sched_clutch_bucket_options_t options) {
  circle_queue_t queue =
      &clutch_buckets_rq->scbrq_queues[clutch_bucket->scb_priority];
 ...
 */
static void
sched_clutch_bucket_runq_remove(sched_clutch_bucket_runq_t clutch_buckets_rq,
                                sched_clutch_bucket_t clutch_bucket) {
  circle_queue_t queue =
      &clutch_buckets_rq->scbrq_queues[clutch_bucket->scb_priority];
  ...
  if (circle_queue_empty(queue)) {
    ...
    clutch_buckets_rq->scbrq_highq =
        bitmap_first(clutch_buckets_rq->scbrq_bitmap, NRQS);
  }
}

static void
sched_clutch_bucket_runq_rotate(sched_clutch_bucket_runq_t clutch_buckets_rq,
                                sched_clutch_bucket_t clutch_bucket) {
  circle_queue_t queue =
      &clutch_buckets_rq->scbrq_queues[clutch_bucket->scb_priority];
  assert(clutch_bucket ==
         cqe_queue_first(queue, struct sched_clutch_bucket, scb_runqlink));
 ...
 */
static void
sched_clutch_root_bucket_init(sched_clutch_root_bucket_t root_bucket,
                              sched_bucket_t bucket, bool bound_root_bucket) {
  ...
  root_bucket->scrb_warp_remaining =
      sched_clutch_root_bucket_warp[root_bucket->scrb_bucket];
 ...
 * important to maintain some native priority order between those buckets. For
 * unbounded root buckets, the policy is to compare the highest clutch buckets
 * of both buckets; if the Above UI bucket is higher, schedule it immediately.
 * Otherwise fall through to the deadline based scheduling which should pickup
 * the timeshare buckets. For the bound case, the policy simply compares the
 * priority of the highest runnable threads in the above UI and timeshare
 * buckets.
 *
 * The implementation allows extremely low latency CPU access for Above UI
 * threads while supporting the use case of high priority timeshare threads
 * contending with lower priority fixed priority threads.
 */

/*
 * sched_clutch_root_unbound_select_aboveui()
 *
 * Routine to determine if the above UI unbounded bucket should be selected for
 * execution.
 *
 * Writes the highest unbound (timeshare FG vs. above UI) bucket, its priority,
 * and whether it is an above UI bucket into the pointer parameters.
 */
static void sched_clutch_root_unbound_select_aboveui(
    sched_clutch_root_t root_clutch, sched_clutch_root_bucket_t *highest_bucket,
    int *highest_pri, bool *highest_is_aboveui,
  ...
  if (bitmap_test(root_clutch->scr_unbound_runnable_bitmap, TH_BUCKET_FIXPRI)) {
    ...
    higher_clutch_bucket = sched_clutch_root_bucket_highest_clutch_bucket(
        root_clutch, higher_root_bucket, NULL, NULL, NULL);
    higher_bucket_sched_pri = priority_queue_max_sched_pri(
        &higher_clutch_bucket->scb_clutchpri_prioq);
    higher_is_aboveui = true;
  }
  /* Consider unbound Timeshare FG */
  if (bitmap_test(root_clutch->scr_unbound_runnable_bitmap,
                  TH_BUCKET_SHARE_FG)) {
    sched_clutch_root_bucket_t root_bucket_sharefg =
        &root_clutch->scr_unbound_buckets[TH_BUCKET_SHARE_FG];
    sched_clutch_bucket_t clutch_bucket_sharefg =
        sched_clutch_root_bucket_highest_clutch_bucket(
            root_clutch, root_bucket_sharefg, NULL, NULL, NULL);
    /* Strict greater-than because unbound timeshare FG root bucket loses all
     * priority ties at this level */
    if (higher_root_bucket == NULL || clutch_bucket_sharefg->scb_priority >
  ...
  if (prev_thread != NULL) {
    ...
    sched_clutch_bucket_group_t prev_clutch_bucket_group =
        sched_clutch_bucket_group_for_thread(prev_thread);
    int prev_clutch_bucket_pri =
        prev_thread->sched_pri +
        (int)(os_atomic_load(
            &prev_clutch_bucket_group->scbg_interactivity_data.scct_count,
            relaxed));
    sched_clutch_bucket_t prev_clutch_bucket =
        sched_clutch_bucket_for_thread(root_clutch, prev_thread);
    bool prev_bucket_should_win_ties =
        prev_bucket->scrb_bucket == TH_BUCKET_FIXPRI &&
        higher_is_aboveui == false;
    if (higher_clutch_bucket == NULL ||
        sched_clutch_pri_greater_than_tiebreak(
            prev_clutch_bucket_pri, higher_clutch_bucket->scb_priority,
            prev_bucket_should_win_ties)) {
  ...
  /* Compare highest priority in the highest unbound Clutch bucket to highest
   * priority seen from the bound buckets */
  if (higher_root_bucket != NULL) {
    bool unbound_should_win_ties =
        higher_is_aboveui == true && *highest_is_aboveui == false;
    if (sched_clutch_pri_greater_than_tiebreak(
            higher_bucket_sched_pri, *highest_pri, unbound_should_win_ties)) {
 ...
 *
 * Routine to determine if the above UI bounded bucket should be selected for
 * execution.
 *
 * Writes the highest bound (timeshare FG vs. above UI) bucket, its priority,
 * and whether it is an above UI bucket into the pointer parameters.
 */
static void sched_clutch_root_bound_select_aboveui(
    sched_clutch_root_t root_clutch, sched_clutch_root_bucket_t *highest_bucket,
    int *highest_pri, bool *highest_is_aboveui,
    sched_clutch_root_bucket_t _Nullable prev_bucket,
    thread_t _Nullable prev_thread) {
  /* Consider bound Above UI */
  sched_clutch_root_bucket_t root_bucket_aboveui =
      &root_clutch->scr_bound_buckets[TH_BUCKET_FIXPRI];
  if (bitmap_test(root_clutch->scr_bound_runnable_bitmap, TH_BUCKET_FIXPRI) &&
      sched_clutch_pri_greater_than_tiebreak(
          root_bucket_aboveui->scrb_bound_thread_runq.highq, *highest_pri,
  ...
  sched_clutch_root_bucket_t root_bucket_sharefg =
      &root_clutch->scr_bound_buckets[TH_BUCKET_SHARE_FG];
  if (bitmap_test(root_clutch->scr_bound_runnable_bitmap, TH_BUCKET_SHARE_FG) &&
      sched_clutch_pri_greater_than_tiebreak(
          root_bucket_sharefg->scrb_bound_thread_runq.highq, *highest_pri,
          false)) {
  ...
  if (prev_thread != NULL) {
    ...
    bool prev_bucket_should_win_ties =
        prev_bucket->scrb_bucket == TH_BUCKET_FIXPRI &&
        *highest_is_aboveui == false;
    if (sched_clutch_pri_greater_than_tiebreak(prev_thread->sched_pri,
                                               *highest_pri,
                                               prev_bucket_should_win_ties)) {
...
static int sched_clutch_root_highest_runnable_qos(
  ...
  int highest_unbound_bucket = bitmap_lsb_first(
      root_clutch->scr_unbound_runnable_bitmap, TH_BUCKET_SCHED_MAX);
  ...
  int highest_bound_bucket = bitmap_lsb_first(
      root_clutch->scr_bound_runnable_bitmap, TH_BUCKET_SCHED_MAX);
 ...
 *
 * Routine to determine if an above UI root bucket should be selected for
 * execution.
 *
 * Returns the root bucket if we should run an above UI bucket or NULL
 * otherwise.
 */
static sched_clutch_root_bucket_t sched_clutch_root_highest_aboveui_root_bucket(
    sched_clutch_root_t root_clutch,
    sched_clutch_highest_root_bucket_type_t type,
    sched_clutch_root_bucket_t _Nullable prev_bucket,
    thread_t _Nullable prev_thread, bool *chose_prev_thread) {
  assert((prev_thread == NULL && prev_bucket == NULL) ||
         (prev_thread != NULL && prev_bucket != NULL));
  assert((type == SCHED_CLUTCH_HIGHEST_ROOT_BUCKET_ALL) ||
         (prev_bucket == NULL));
   ...
   */
  sched_clutch_root_bucket_t bound_prev_bucket = NULL,
                             unbound_prev_bucket = NULL;
  ...
  /* Consider bound Above UI vs. Timeshare FG first, so those buckets will win
   * ties against the corresponding unbound buckets */
  if (type == SCHED_CLUTCH_HIGHEST_ROOT_BUCKET_ALL) {
    sched_clutch_root_bound_select_aboveui(
        root_clutch, &highest_bucket, &highest_pri, &highest_is_aboveui,
        bound_prev_bucket, bound_prev_thread);
  }

  /* Consider unbound Above UI vs. Timeshare FG */
  sched_clutch_root_unbound_select_aboveui(
      root_clutch, &highest_bucket, &highest_pri, &highest_is_aboveui,
      unbound_prev_bucket, unbound_prev_thread);
  if (type == SCHED_CLUTCH_HIGHEST_ROOT_BUCKET_UNBOUND_ONLY) {
   ...
   */
  if (prev_thread != NULL &&
      bitmap_test(highest_bucket->scrb_bound
                      ? root_clutch->scr_bound_runnable_bitmap
                      : root_clutch->scr_unbound_runnable_bitmap,
                  highest_bucket->scrb_bucket) == false) {
    /* Highest bucket we saw is empty, so the previous thread must have been the
     * highest */
 ...
 */
static sched_clutch_root_bucket_t sched_clutch_root_highest_root_bucket(
    sched_clutch_root_t root_clutch, uint64_t timestamp,
    sched_clutch_highest_root_bucket_type_t type,
    sched_clutch_root_bucket_t _Nullable prev_bucket,
    thread_t _Nullable prev_thread, bool *chose_prev_thread,
  ...
  assert(prev_thread == NULL ||
         (mode != SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY));
  sched_clutch_hierarchy_locked_assert(root_clutch);

  int highest_runnable_bucket =
      sched_clutch_root_highest_runnable_qos(root_clutch, type);
  if (highest_runnable_bucket == -1) {
    ...
    assert(sched_clutch_root_count(root_clutch) == 0 ||
           type == SCHED_CLUTCH_HIGHEST_ROOT_BUCKET_UNBOUND_ONLY);
    *chose_prev_thread = true;
    if (prev_thread != NULL) {
  ...
  sched_clutch_root_bucket_t highest_aboveui_bucket =
      sched_clutch_root_highest_aboveui_root_bucket(
          root_clutch, type, prev_bucket, prev_thread, chose_prev_thread);
  ...
  /*
   * Above UI bucket is not runnable or has a low priority runnable thread; use
   * the earliest deadline model to schedule threads. The idea is that as the
   * timeshare buckets use CPU, they will drop their interactivity score/sched
   * priority and allow the low priority AboveUI buckets to be scheduled.
   */
...
evaluate_root_buckets:
  ...
  if (type == SCHED_CLUTCH_HIGHEST_ROOT_BUCKET_UNBOUND_ONLY) {
    edf_bucket =
        priority_queue_min(&root_clutch->scr_unbound_root_buckets,
                           struct sched_clutch_root_bucket, scrb_pqlink);
  } else {
    assert(type == SCHED_CLUTCH_HIGHEST_ROOT_BUCKET_ALL);
    sched_clutch_root_bucket_t unbound_bucket =
        priority_queue_min(&root_clutch->scr_unbound_root_buckets,
                           struct sched_clutch_root_bucket, scrb_pqlink);
    sched_clutch_root_bucket_t bound_bucket =
        priority_queue_min(&root_clutch->scr_bound_root_buckets,
                           struct sched_clutch_root_bucket, scrb_pqlink);
    if (bound_bucket && unbound_bucket) {
      /* If bound and unbound root buckets are runnable, select the one with the
       * earlier deadline */
      edf_bucket = (bound_bucket->scrb_pqlink.deadline <=
                    unbound_bucket->scrb_pqlink.deadline)
                       ? bound_bucket
                       : unbound_bucket;
    } else {
  ...
  if (prev_bucket != NULL &&
      prev_bucket->scrb_pqlink.deadline < edf_bucket->scrb_pqlink.deadline) {
    /* The previous thread's root bucket has the earliest deadline and is not
     * currently enqueued */
  ...
  if (edf_bucket->scrb_starvation_avoidance) {
    ...
    uint64_t starvation_window =
        sched_clutch_thread_quantum[edf_bucket->scrb_bucket];
    if (timestamp >= (edf_bucket->scrb_starvation_ts + starvation_window)) {
      /* Starvation avoidance window is over; update deadline and re-evaluate
       * EDF */
      edf_bucket->scrb_starvation_avoidance = false;
      edf_bucket->scrb_starvation_ts = 0;
      sched_clutch_root_bucket_deadline_update(
          edf_bucket, root_clutch, timestamp, edf_bucket_enqueued_normally);
      bit_set(debug_info->trace_data.starvation_avoidance_window_close,
              edf_bucket->scrb_bound * TH_BUCKET_SCHED_MAX +
  ...
  /*
   * Check if any of the buckets have warp available. The implementation only
   * allows root buckets to warp ahead of buckets of the same type (i.e.
   * bound/unbound). The reason for doing that is because warping is a concept
   * that makes sense between root buckets of the same type since its
   * effectively a scheduling advantage over a lower QoS root bucket.
   */
  bitmap_t *warp_available_bitmap =
      (edf_bucket->scrb_bound) ? (root_clutch->scr_bound_warp_available)
                               : (root_clutch->scr_unbound_warp_available);
  int warp_bucket_index =
      bitmap_lsb_first(warp_available_bitmap, TH_BUCKET_SCHED_MAX);

  /* Allow the prev_bucket to use its warp as well */
  bool prev_bucket_warping =
      (prev_bucket != NULL) &&
      (prev_bucket->scrb_bound == edf_bucket->scrb_bound) &&
      prev_bucket->scrb_bucket < edf_bucket->scrb_bucket &&
      (prev_bucket->scrb_warp_remaining > 0) &&
      (warp_bucket_index == -1 || prev_bucket->scrb_bucket < warp_bucket_index);

  bool non_edf_bucket_can_warp =
      (warp_bucket_index != -1 &&
       warp_bucket_index < edf_bucket->scrb_bucket) ||
      prev_bucket_warping;
  ...
  if (non_edf_bucket_can_warp == false) {
    ...
    bool should_update_edf_starvation_state =
        edf_bucket == prev_bucket ||
        mode == SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY ||
        mode == SCHED_CLUTCH_TRAVERSE_REMOVE_CONSIDER_CURRENT;
    if (edf_bucket->scrb_starvation_avoidance == false &&
        should_update_edf_starvation_state) {
      /* Looks like the EDF bucket is not in starvation avoidance mode; check if
       * it should be */
      if (highest_runnable_bucket < edf_bucket->scrb_bucket ||
          (prev_bucket != NULL &&
           prev_bucket->scrb_bucket < edf_bucket->scrb_bucket)) {
        /*
         * Since a higher bucket is runnable, it indicates that the EDF bucket
         * should be in starvation avoidance.
         *
         * The starvation avoidance window is allocated as a single quantum for
         * the starved bucket, enforced simultaneously across all CPUs in the
         * cluster. The idea is to grant the starved bucket roughly one quantum
         * per core, each time the bucket reaches the earliest deadline
         * position. Note that this cadence is driven by the difference between
         * the starved bucket's and highest-runnable bucket's WCELs.
         */
        edf_bucket->scrb_starvation_avoidance = true;
        edf_bucket->scrb_starvation_ts = timestamp;
        debug_info->trace_data.selection_opened_starvation_avoidance_window =
            true;
      } else {
        /* EDF bucket is being selected in the natural order; update deadline
         * and reset warp */
        sched_clutch_root_bucket_deadline_update(
            edf_bucket, root_clutch, timestamp, edf_bucket_enqueued_normally);
        edf_bucket->scrb_warp_remaining =
            sched_clutch_root_bucket_warp[edf_bucket->scrb_bucket];
        ...
        if (edf_bucket_enqueued_normally) {
          if (edf_bucket->scrb_bound) {
            bitmap_set(root_clutch->scr_bound_warp_available,
                       edf_bucket->scrb_bucket);
          } else {
            bitmap_set(root_clutch->scr_unbound_warp_available,
                       edf_bucket->scrb_bucket);
          }
        }
      }
  ...
  if (prev_bucket_warping) {
    assert(warp_bucket_index == -1 ||
           prev_bucket->scrb_bucket < warp_bucket_index);
    warp_bucket = prev_bucket;
  } else {
    warp_bucket = (edf_bucket->scrb_bound)
                      ? &root_clutch->scr_bound_buckets[warp_bucket_index]
                      : &root_clutch->scr_unbound_buckets[warp_bucket_index];
  }

  bool warp_is_being_utilized =
      warp_bucket == prev_bucket ||
      mode == SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY ||
      mode == SCHED_CLUTCH_TRAVERSE_REMOVE_CONSIDER_CURRENT;

  if (warp_bucket->scrb_warped_deadline ==
      SCHED_CLUTCH_ROOT_BUCKET_WARP_UNUSED) {
    if (warp_is_being_utilized) {
      /* Root bucket has not used any of its warp; set a deadline to expire its
       * warp and return it */
      warp_bucket->scrb_warped_deadline =
          timestamp + warp_bucket->scrb_warp_remaining;
      sched_clutch_root_bucket_deadline_update(warp_bucket, root_clutch,
                                               timestamp, !prev_bucket_warping);
  ...
  if (!prev_bucket_warping) {
    if (warp_bucket->scrb_bound) {
      bitmap_clear(root_clutch->scr_bound_warp_available,
                   warp_bucket->scrb_bucket);
    } else {
      bitmap_clear(root_clutch->scr_unbound_warp_available,
                   warp_bucket->scrb_bucket);
    }
  }
  bit_set(debug_info->trace_data.warp_window_close,
          warp_bucket->scrb_bound * TH_BUCKET_SCHED_MAX +
              warp_bucket->scrb_bucket);
  goto evaluate_root_buckets;
}
 ...
 */
static uint64_t sched_clutch_root_bucket_deadline_calculate(
    sched_clutch_root_bucket_t root_bucket, uint64_t timestamp) {
  ...
  /* For all timeshare buckets set the deadline as current time +
   * worst-case-execution-latency */
 ...
 * queue.
 */
static void sched_clutch_root_bucket_deadline_update(
    sched_clutch_root_bucket_t root_bucket, sched_clutch_root_t root_clutch,
    uint64_t timestamp, bool bucket_is_enqueued) {
  if (sched_clutch_bucket_is_above_timeshare(root_bucket->scrb_bucket)) {
    /* The algorithm never uses the deadlines for scheduling TH_BUCKET_FIXPRI
     * bucket */
    return;
  }
  ...
  uint64_t new_deadline =
      sched_clutch_root_bucket_deadline_calculate(root_bucket, timestamp);
  if (__improbable(old_deadline > new_deadline)) {
    panic("old_deadline (%llu) > new_deadline (%llu); root_bucket (%d); "
          "timestamp (%llu)",
          old_deadline, new_deadline, root_bucket->scrb_bucket, timestamp);
  }
  if (old_deadline != new_deadline) {
    root_bucket->scrb_pqlink.deadline = new_deadline;
    if (bucket_is_enqueued) {
      struct priority_queue_deadline_min *prioq =
          (root_bucket->scrb_bound) ? &root_clutch->scr_bound_root_buckets
                                    : &root_clutch->scr_unbound_root_buckets;
 ...
 */
static void
sched_clutch_root_bucket_runnable(sched_clutch_root_bucket_t root_bucket,
                                  sched_clutch_root_t root_clutch,
                                  uint64_t timestamp) {
  /* Mark the root bucket as runnable */
  bitmap_t *runnable_bitmap = (root_bucket->scrb_bound)
                                  ? root_clutch->scr_bound_runnable_bitmap
                                  : root_clutch->scr_unbound_runnable_bitmap;
  bitmap_set(runnable_bitmap, root_bucket->scrb_bucket);

  if (sched_clutch_bucket_is_above_timeshare(root_bucket->scrb_bucket)) {
    /* Since the TH_BUCKET_FIXPRI bucket is not scheduled based on deadline,
  ...
  if (root_bucket->scrb_starvation_avoidance == false) {
    /*
     * Only update the deadline if the bucket was not in starvation avoidance
     * mode. If the bucket was in starvation avoidance and its window has
     * expired, the highest root bucket selection logic will notice that and fix
     * it up.
     */
    root_bucket->scrb_pqlink.deadline =
        sched_clutch_root_bucket_deadline_calculate(root_bucket, timestamp);
  }
  struct priority_queue_deadline_min *prioq =
      (root_bucket->scrb_bound) ? &root_clutch->scr_bound_root_buckets
  ...
  if (root_bucket->scrb_warp_remaining) {
    /* Since the bucket has some warp remaining and its now runnable, mark it as
     * available for warp */
    bitmap_t *warp_bitmap = (root_bucket->scrb_bound)
                                ? root_clutch->scr_bound_warp_available
                                : root_clutch->scr_unbound_warp_available;
 ...
 */
static void
sched_clutch_root_bucket_empty(sched_clutch_root_bucket_t root_bucket,
                               sched_clutch_root_t root_clutch,
                               uint64_t timestamp) {
  bitmap_t *runnable_bitmap = (root_bucket->scrb_bound)
                                  ? root_clutch->scr_bound_runnable_bitmap
  ...
  if (root_bucket->scrb_warped_deadline !=
      SCHED_CLUTCH_ROOT_BUCKET_WARP_UNUSED) {
    if (root_bucket->scrb_warped_deadline > timestamp) {
       ...
       */
      root_bucket->scrb_warp_remaining =
          root_bucket->scrb_warped_deadline - timestamp;
    } else {
      /*
...
static int sched_clutch_global_bucket_load_get(sched_bucket_t bucket) {
 ...
 */
static void sched_clutch_root_pri_update(sched_clutch_root_t root_clutch) {
  sched_clutch_hierarchy_locked_assert(root_clutch);
  int16_t root_bound_pri = NOPRI;
  int16_t root_unbound_pri = NOPRI;

  /* Consider bound root buckets */
  if (bitmap_lsb_first(root_clutch->scr_bound_runnable_bitmap,
                       TH_BUCKET_SCHED_MAX) == -1) {
  ...
  sched_clutch_root_bound_select_aboveui(
      root_clutch, &highest_bound_root_bucket, &highest_bound_root_bucket_pri,
      &highest_bound_root_bucket_is_fixpri, NULL, NULL);
  if (highest_bound_root_bucket_is_fixpri == false) {
    int root_bucket_index =
        bitmap_lsb_next(root_clutch->scr_bound_runnable_bitmap,
                        TH_BUCKET_SCHED_MAX, TH_BUCKET_FIXPRI);
    assert(root_bucket_index != -1);
    highest_bound_root_bucket =
        &root_clutch->scr_bound_buckets[root_bucket_index];
  }
...
root_pri_update_unbound:
  ...
  if (bitmap_lsb_first(root_clutch->scr_unbound_runnable_bitmap,
                       TH_BUCKET_SCHED_MAX) == -1) {
  ...
  sched_clutch_root_unbound_select_aboveui(
      root_clutch, &highest_unbound_root_bucket,
      &highest_unbound_root_bucket_pri, &highest_unbound_root_bucket_is_fixpri,
      NULL, NULL);
  if (highest_unbound_root_bucket_is_fixpri == false) {
    int root_bucket_index =
        bitmap_lsb_next(root_clutch->scr_unbound_runnable_bitmap,
                        TH_BUCKET_SCHED_MAX, TH_BUCKET_FIXPRI);
    assert(root_bucket_index != -1);
    highest_unbound_root_bucket =
        &root_clutch->scr_unbound_buckets[root_bucket_index];
  }

  /* For the selected root bucket, find the highest priority clutch bucket */
  sched_clutch_bucket_t clutch_bucket =
      sched_clutch_root_bucket_highest_clutch_bucket(
          root_clutch, highest_unbound_root_bucket, NULL, NULL, NULL);
  root_unbound_pri =
      priority_queue_max_sched_pri(&clutch_bucket->scb_clutchpri_prioq);
 ...
 */
static void sched_clutch_root_urgency_inc(sched_clutch_root_t root_clutch,
                                          thread_t thread) {
 ...
 * Always called with the pset lock held.
 */
static void sched_clutch_root_urgency_dec(sched_clutch_root_t root_clutch,
                                          thread_t thread) {
  if (SCHED(priority_is_urgent)(thread->sched_pri)) {
    root_clutch->scr_urgency--;
  }
...
static uint8_t sched_clutch_bucket_group_interactive_pri =
    SCHED_CLUTCH_BUCKET_GROUP_INTERACTIVE_PRI_DEFAULT;

/* window to scale the cpu usage and blocked values (currently 500ms). Its the
 * threshold of used+blocked */
 ...
 *
 * The values chosen here are very close to the WCEL values for each sched
 * bucket. Theses values are added into the pending interval used to determine
 * how frequently we will ageout the CPU usage, ensuring a reasonable limit on
 * the frequency.
 */
static uint32_t
    sched_clutch_bucket_group_pending_delta_us[TH_BUCKET_SCHED_MAX] = {
        SCHED_CLUTCH_INVALID_TIME_32, /* FIXPRI */
        10000,                        /* FG */
        37500,                        /* IN */
        75000,                        /* DF */
...
static uint64_t sched_clutch_bucket_group_pending_delta[TH_BUCKET_SCHED_MAX] = {
    0};
 ...
 */
static void
sched_clutch_bucket_init(sched_clutch_bucket_t clutch_bucket,
                         sched_clutch_bucket_group_t clutch_bucket_group,
                         sched_bucket_t bucket) {
  clutch_bucket->scb_bucket = bucket;
  /* scb_priority will be recalculated when a thread is inserted in the clutch
   * bucket */
 ...
 */
static void
sched_clutch_bucket_group_init(sched_clutch_bucket_group_t clutch_bucket_group,
                               sched_clutch_t clutch, sched_bucket_t bucket) {
  ...
  clutch_bucket_group->scbg_clutch_buckets =
      kalloc_type(struct sched_clutch_bucket, max_clusters, Z_WAITOK | Z_ZERO);
  for (int i = 0; i < max_clusters; i++) {
    sched_clutch_bucket_init(&clutch_bucket_group->scbg_clutch_buckets[i],
                             clutch_bucket_group, bucket);
  }
  ...
  os_atomic_store(&clutch_bucket_group->scbg_preferred_cluster,
                  pset0.pset_cluster_id, relaxed);
  /*
   * All thread groups should be initialized to be interactive; this allows the
   * newly launched thread groups to fairly compete with already running thread
   * groups.
   */
  clutch_bucket_group->scbg_interactivity_data.scct_count =
      (sched_clutch_bucket_group_interactive_pri * 2);
  clutch_bucket_group->scbg_interactivity_data.scct_timestamp = 0;
  os_atomic_store(
      &clutch_bucket_group->scbg_cpu_data.cpu_data.scbcd_cpu_blocked,
      (clutch_cpu_data_t)sched_clutch_bucket_group_adjust_threshold, relaxed);
  clutch_bucket_group->scbg_blocked_data.scct_timestamp =
      SCHED_CLUTCH_BUCKET_GROUP_BLOCKED_TS_INVALID;
  clutch_bucket_group->scbg_pending_data.scct_timestamp =
      SCHED_CLUTCH_BUCKET_GROUP_PENDING_INVALID;
}

static void sched_clutch_bucket_group_destroy(
    sched_clutch_bucket_group_t clutch_bucket_group) {
 ...
 */
void sched_clutch_init_with_thread_group(sched_clutch_t clutch,
 ...
 */
void sched_clutch_destroy(sched_clutch_t clutch) {
 ...
 * QoS level in a TG. These preferences are stored at the
 * sched_clutch_bucket_group level since that represents all threads at a
 * particular QoS level within a sched_clutch. For any lookup of preferred
 * cluster, the logic always goes back to the preference stored at the
 * clutch_bucket_group.
 */

static uint32_t sched_edge_clutch_bucket_group_preferred_cluster(
    sched_clutch_bucket_group_t clutch_bucket_group) {
...
sched_clutch_bucket_preferred_cluster(sched_clutch_bucket_t clutch_bucket) {
  return sched_edge_clutch_bucket_group_preferred_cluster(
      clutch_bucket->scb_group);
}

uint32_t sched_edge_thread_preferred_cluster(thread_t thread) {
  ...
  sched_clutch_bucket_group_t clutch_bucket_group =
      &clutch->sc_clutch_groups[sched_bucket];
 ...
 *
 * In the Edge Scheduler, each cluster maintains a priority queue of clutch
 * buckets containing threads that are not native to the cluster. A clutch
 * bucket is considered native if its preferred cluster has the same type as the
 * cluster its enqueued in. The foreign clutch bucket priority queue is used for
 * rebalance operations to get threads back to their native cluster quickly.
 *
 * It is possible to make this policy even more aggressive by considering all
 * clusters that are not the preferred cluster as the foreign cluster, but that
 * would mean a lot of thread migrations which might have performance
 * implications.
 */

static void sched_clutch_bucket_mark_native(sched_clutch_bucket_t clutch_bucket,
                                            sched_clutch_root_t root_clutch) {
  if (clutch_bucket->scb_foreign) {
    clutch_bucket->scb_foreign = false;
    priority_queue_remove(&root_clutch->scr_foreign_buckets,
                          &clutch_bucket->scb_foreignlink);
  }
}

static void
sched_clutch_bucket_mark_foreign(sched_clutch_bucket_t clutch_bucket,
                                 sched_clutch_root_t root_clutch) {
  if (!clutch_bucket->scb_foreign) {
    clutch_bucket->scb_foreign = true;
    priority_queue_entry_set_sched_pri(&root_clutch->scr_foreign_buckets,
                                       &clutch_bucket->scb_foreignlink,
                                       clutch_bucket->scb_priority, 0);
    priority_queue_insert(&root_clutch->scr_foreign_buckets,
                          &clutch_bucket->scb_foreignlink);
  }
}
...
static void
sched_edge_cluster_cumulative_count_incr(sched_clutch_root_t root_clutch,
                                         sched_bucket_t bucket) {
  switch (bucket) {
  case TH_BUCKET_FIXPRI:
    os_atomic_inc(&root_clutch->scr_cumulative_run_count[TH_BUCKET_FIXPRI],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_FG:
    os_atomic_inc(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_FG],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_IN:
    os_atomic_inc(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_IN],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_DF:
    os_atomic_inc(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_DF],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_UT:
    os_atomic_inc(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_UT],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_BG:
    os_atomic_inc(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_BG],
                  relaxed);
    break;
  default:
    panic("Unexpected sched_bucket passed to "
          "sched_edge_cluster_cumulative_count_incr()");
  }
}

static void
sched_edge_cluster_cumulative_count_decr(sched_clutch_root_t root_clutch,
                                         sched_bucket_t bucket) {
  switch (bucket) {
  case TH_BUCKET_FIXPRI:
    os_atomic_dec(&root_clutch->scr_cumulative_run_count[TH_BUCKET_FIXPRI],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_FG:
    os_atomic_dec(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_FG],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_IN:
    os_atomic_dec(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_IN],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_DF:
    os_atomic_dec(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_DF],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_UT:
    os_atomic_dec(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_UT],
                  relaxed);
    OS_FALLTHROUGH;
  case TH_BUCKET_SHARE_BG:
    os_atomic_dec(&root_clutch->scr_cumulative_run_count[TH_BUCKET_SHARE_BG],
                  relaxed);
    break;
  default:
    panic("Unexpected sched_bucket passed to "
          "sched_edge_cluster_cumulative_count_decr()");
  }
}

uint16_t sched_edge_cluster_cumulative_count(sched_clutch_root_t root_clutch,
                                             sched_bucket_t bucket) {
  return os_atomic_load(&root_clutch->scr_cumulative_run_count[bucket],
                        relaxed);
}
 ...
 */
static void
sched_clutch_bucket_hierarchy_insert(sched_clutch_root_t root_clutch,
                                     sched_clutch_bucket_t clutch_bucket,
                                     sched_bucket_t bucket, uint64_t timestamp,
                                     sched_clutch_bucket_options_t options) {
  sched_clutch_hierarchy_locked_assert(root_clutch);
  if (sched_clutch_bucket_is_above_timeshare(bucket) == false) {
    /* Enqueue the timeshare clutch buckets into the global runnable
     * clutch_bucket list; used for sched tick operations */
    enqueue_tail(&root_clutch->scr_clutch_buckets,
                 &clutch_bucket->scb_listlink);
  }
#if CONFIG_SCHED_EDGE
  /* Check if the bucket is a foreign clutch bucket and add it to the foreign
   * buckets list */
  uint32_t preferred_cluster =
      sched_clutch_bucket_preferred_cluster(clutch_bucket);
  if (pset_type_for_id(preferred_cluster) !=
      pset_type_for_id(root_clutch->scr_cluster_id)) {
    sched_clutch_bucket_mark_foreign(clutch_bucket, root_clutch);
  }
#endif /* CONFIG_SCHED_EDGE */
  sched_clutch_root_bucket_t root_bucket =
      &root_clutch->scr_unbound_buckets[bucket];

  /* If this is the first clutch bucket in the root bucket, insert the root
   * bucket into the root priority queue */
  ...
  /* Insert the clutch bucket into the root bucket run queue with order based on
   * options */
  sched_clutch_bucket_runq_enqueue(&root_bucket->scrb_clutch_buckets,
                                   clutch_bucket, options);
 ...
 */
static void sched_clutch_bucket_hierarchy_remove(
    sched_clutch_root_t root_clutch, sched_clutch_bucket_t clutch_bucket,
    sched_bucket_t bucket, uint64_t timestamp,
    __unused sched_clutch_bucket_options_t options) {
  sched_clutch_hierarchy_locked_assert(root_clutch);
  if (sched_clutch_bucket_is_above_timeshare(bucket) == false) {
    /* Remove the timeshare clutch bucket from the globally runnable
     * clutch_bucket list */
  ...
  sched_clutch_bucket_runq_remove(&root_bucket->scrb_clutch_buckets,
                                  clutch_bucket);
  os_atomic_store(&clutch_bucket->scb_root, NULL, relaxed);

  /* If the root bucket priority queue is now empty, remove it from the root
   * priority queue */
  if (sched_clutch_bucket_runq_empty(&root_bucket->scrb_clutch_buckets)) {
 ...
 *
 * Calculates the "base" priority of the clutch bucket, which is equal to the
 * max of the highest base_pri and the highest sched_pri in the clutch bucket.
 */
static uint8_t
sched_clutch_bucket_base_pri(sched_clutch_bucket_t clutch_bucket) {
  assert(priority_queue_empty(&clutch_bucket->scb_thread_runq) == false);
  /*
   * Since the clutch bucket can contain threads that are members of the group
   * due to the sched_pri being promoted or due to their base pri, the base
   * priority of the entire clutch bucket should be based on the highest thread
   * (promoted or base) in the clutch bucket.
   */
  uint8_t max_pri = 0;
 ...
 *
 * Routine to calculate the interactivity score of a clutch bucket group from
 * its CPU usage
 */
static uint8_t sched_clutch_interactivity_from_cpu_data(
    sched_clutch_bucket_group_t clutch_bucket_group) {
  sched_clutch_bucket_cpu_data_t scb_cpu_data;
  scb_cpu_data.scbcd_cpu_data_packed = os_atomic_load_wide(
      &clutch_bucket_group->scbg_cpu_data.scbcd_cpu_data_packed, relaxed);
  ...
  if (cpu_blocked > cpu_used) {
    ...
    interactive_score = sched_clutch_bucket_group_interactive_pri +
                        ((sched_clutch_bucket_group_interactive_pri *
                          (cpu_blocked - cpu_used)) /
                         cpu_blocked);
  } else {
    /* Non-interactive clutch_bucket case */
    interactive_score =
        ((sched_clutch_bucket_group_interactive_pri * cpu_blocked) / cpu_used);
  }
 ...
 */
static uint8_t
sched_clutch_bucket_pri_calculate(sched_clutch_bucket_t clutch_bucket,
                                  uint64_t timestamp) {
  ...
  uint8_t interactive_score =
      sched_clutch_bucket_group_interactivity_score_calculate(
          clutch_bucket->scb_group, timestamp);
  ...
  if (pri != clutch_bucket->scb_priority) {
    KERNEL_DEBUG_CONSTANT_IST(
        KDEBUG_TRACE,
        MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_CLUTCH_TG_BUCKET_PRI) |
            DBG_FUNC_NONE,
        thread_group_get_id(clutch_bucket->scb_group->scbg_clutch->sc_tg),
        clutch_bucket->scb_bucket, pri, interactive_score, 0);
  }
  return pri;
}
 ...
 */
static sched_clutch_bucket_t sched_clutch_root_bucket_highest_clutch_bucket(
    sched_clutch_root_t root_clutch, sched_clutch_root_bucket_t root_bucket,
    processor_t _Nullable processor, thread_t _Nullable prev_thread,
    bool *_Nullable chose_prev_thread) {
  if (sched_clutch_bucket_runq_empty(&root_bucket->scrb_clutch_buckets)) {
    if (prev_thread != NULL) {
  ...
  sched_clutch_bucket_t clutch_bucket =
      sched_clutch_bucket_runq_peek(&root_bucket->scrb_clutch_buckets);
  ...
  if (prev_thread != NULL) {
    ...
    if (prev_clutch_bucket != clutch_bucket &&
        sched_clutch_pri_greater_than_tiebreak(prev_clutch_bucket_pri,
 ...
 */
static boolean_t sched_clutch_bucket_runnable(
    sched_clutch_bucket_t clutch_bucket, sched_clutch_root_t root_clutch,
    uint64_t timestamp, sched_clutch_bucket_options_t options) {
  sched_clutch_hierarchy_locked_assert(root_clutch);
  /* Since the clutch bucket became newly runnable, update its pending timestamp
   */
  clutch_bucket->scb_priority =
      sched_clutch_bucket_pri_calculate(clutch_bucket, timestamp);
  sched_clutch_bucket_hierarchy_insert(root_clutch, clutch_bucket,
                                       clutch_bucket->scb_bucket, timestamp,
                                       options);

  /* Update the timesharing properties of this clutch_bucket_group; also done
   * every sched_tick */
 ...
 */
static boolean_t
sched_clutch_bucket_update(sched_clutch_bucket_t clutch_bucket,
                           sched_clutch_root_t root_clutch, uint64_t timestamp,
                           sched_clutch_bucket_options_t options) {
  sched_clutch_hierarchy_locked_assert(root_clutch);
  uint64_t new_pri =
      sched_clutch_bucket_pri_calculate(clutch_bucket, timestamp);
  sched_clutch_bucket_runq_t bucket_runq =
      &root_clutch->scr_unbound_buckets[clutch_bucket->scb_bucket]
           .scrb_clutch_buckets;
  if (new_pri == clutch_bucket->scb_priority) {
    /*
     * If SCHED_CLUTCH_BUCKET_OPTIONS_SAMEPRI_RR is specified, move the clutch
     * bucket to the end of the runq. Typically used when a thread is selected
     * for execution from a clutch bucket.
     */
 ...
 * It involves removing the clutch bucket from the hierarchy and updaing the
 * root priority appropriately.
 */
static void sched_clutch_bucket_empty(sched_clutch_bucket_t clutch_bucket,
                                      sched_clutch_root_t root_clutch,
                                      uint64_t timestamp,
                                      sched_clutch_bucket_options_t options) {
  sched_clutch_hierarchy_locked_assert(root_clutch);
  assert3u(clutch_bucket->scb_thr_count, ==, 0);
  sched_clutch_bucket_hierarchy_remove(root_clutch, clutch_bucket,
                                       clutch_bucket->scb_bucket, timestamp,
                                       options);
 ...
 */
void sched_clutch_cpu_usage_update(thread_t thread, uint64_t delta) {
  ...
  sched_clutch_t clutch = sched_clutch_for_thread(thread);
  sched_clutch_bucket_group_t clutch_bucket_group =
      &(clutch->sc_clutch_groups[thread->th_sched_bucket]);
 ...
 */
static void sched_clutch_bucket_group_cpu_usage_update(
    sched_clutch_bucket_group_t clutch_bucket_group, uint64_t delta) {
  if (sched_clutch_bucket_is_above_timeshare(
          clutch_bucket_group->scbg_bucket)) {
    /* Since Above UI bucket has maximum interactivity score always, nothing to
     * do here */
    return;
  }
  delta = MIN(delta, sched_clutch_bucket_group_adjust_threshold);
  os_atomic_add(&(clutch_bucket_group->scbg_cpu_data.cpu_data.scbcd_cpu_used),
                (clutch_cpu_data_t)delta, relaxed);
}

/*
 * sched_clutch_bucket_group_cpu_pending_adjust()
 *
 * Routine to calculate the adjusted CPU usage value based on the pending
 * intervals. The calculation is done such that one "pending interval" provides
 * one point improvement in interactivity score.
 */
static inline uint64_t sched_clutch_bucket_group_cpu_pending_adjust(
    uint64_t cpu_used, uint64_t cpu_blocked, uint8_t pending_intervals) {
  ...
  if (cpu_blocked < cpu_used) {
    ...
    cpu_used_adjusted =
        cpu_used_adjusted /
        ((sched_clutch_bucket_group_interactive_pri * cpu_blocked) +
         (cpu_used * pending_intervals));
  } else {
    uint64_t adjust_factor = (cpu_blocked * pending_intervals) /
                             sched_clutch_bucket_group_interactive_pri;
    cpu_used_adjusted =
        (adjust_factor > cpu_used) ? 0 : (cpu_used - adjust_factor);
  }
 ...
 */
static void sched_clutch_bucket_group_cpu_adjust(
    sched_clutch_bucket_group_t clutch_bucket_group,
    uint8_t pending_intervals) {
  sched_clutch_bucket_cpu_data_t old_cpu_data = {};
  sched_clutch_bucket_cpu_data_t new_cpu_data = {};
  os_atomic_rmw_loop(
      &clutch_bucket_group->scbg_cpu_data.scbcd_cpu_data_packed,
      old_cpu_data.scbcd_cpu_data_packed, new_cpu_data.scbcd_cpu_data_packed,
      relaxed, {
        ...
        if ((cpu_used + cpu_blocked) >=
            sched_clutch_bucket_group_adjust_threshold) {
          /* Only keep the recent CPU history to better indicate how this TG has
           * been behaving */
        ...
        cpu_used =
            (clutch_cpu_data_t)sched_clutch_bucket_group_cpu_pending_adjust(
                cpu_used, cpu_blocked, pending_intervals);
 ...
 * thread.
 */
uint32_t sched_clutch_thread_run_bucket_incr(thread_t thread,
                                             sched_bucket_t bucket) {
  if (!SCHED_CLUTCH_THREAD_ELIGIBLE(thread)) {
    return 0;
  }
...
static uint32_t sched_clutch_run_bucket_incr(sched_clutch_t clutch,
                                             sched_bucket_t bucket) {
  assert(bucket != TH_BUCKET_RUN);
  sched_clutch_bucket_group_t clutch_bucket_group =
      &(clutch->sc_clutch_groups[bucket]);
 ...
 * thread.
 */
uint32_t sched_clutch_thread_run_bucket_decr(thread_t thread,
                                             sched_bucket_t bucket) {
  if (!SCHED_CLUTCH_THREAD_ELIGIBLE(thread)) {
    return 0;
  }
...
static uint32_t sched_clutch_run_bucket_decr(sched_clutch_t clutch,
                                             sched_bucket_t bucket) {
  assert(bucket != TH_BUCKET_RUN);
  sched_clutch_bucket_group_t clutch_bucket_group =
      &(clutch->sc_clutch_groups[bucket]);
 ...
 * thread group + QoS.
 */
static void sched_clutch_bucket_group_pri_shift_update(
    sched_clutch_bucket_group_t clutch_bucket_group) {
  if (sched_clutch_bucket_is_above_timeshare(
          clutch_bucket_group->scbg_bucket)) {
   ...
   */
  uint32_t sched_ts =
      os_atomic_load(&clutch_bucket_group->scbg_timeshare_tick, relaxed);
  uint32_t current_sched_ts = os_atomic_load(&sched_tick, relaxed);
  if (sched_ts < current_sched_ts) {
    os_atomic_store(&clutch_bucket_group->scbg_timeshare_tick, current_sched_ts,
                    relaxed);
    /* NCPU wide workloads should not experience decay */
    uint64_t bucket_group_run_count =
        os_atomic_load_wide(&clutch_bucket_group->scbg_blocked_data.scct_count,
                            relaxed) -
        1;
    uint32_t bucket_group_load =
        (uint32_t)(bucket_group_run_count / processor_avail_count);
    bucket_group_load = MIN(bucket_group_load, NRQS - 1);
    uint32_t pri_shift =
        sched_fixed_shift - sched_load_shifts[bucket_group_load];
 ...
 * every sched_tick. For multi-cluster platforms, each QoS level will have
 * multiple clutch buckets with runnable threads in them. So it is important to
 * maintain the timesharing information at the clutch_bucket_group level instead
 * of individual clutch buckets (because the algorithm is trying to timeshare
 * all threads at the same QoS irrespective of which hierarchy they are enqueued
 * in).
 *
 * The routine is called from the sched tick handling code to make sure this
 * value is updated at least once every sched tick. For clutch bucket groups
 * which have not been runnable for very long, the clutch_bucket_group maintains
 * a "last updated schedtick" parameter. As threads become runnable in the
 * clutch bucket group, if this value is outdated, we update the priority shift.
 *
 * Possible optimization:
 * - The current algorithm samples the load at most once every sched tick
 * (125ms). This is prone to spikes in runnable counts; if that turns out to be
 ...
 *   shift calculation.
 */
static void sched_clutch_bucket_group_timeshare_update(
    sched_clutch_bucket_group_t clutch_bucket_group,
    sched_clutch_bucket_t clutch_bucket, uint64_t ctime) {
  if (sched_clutch_bucket_is_above_timeshare(
          clutch_bucket_group->scbg_bucket)) {
    /* No timesharing needed for fixed priority Above UI threads */
    return;
  ...
  /*
   * Update the clutch bucket priority; this allows clutch buckets that have
   * been pending for a long time to get an updated interactivity score.
   */
 ...
 */
static inline void sched_clutch_thread_tick_delta(
    thread_t thread,
    sched_clutch_bucket_group_t _Nullable clutch_bucket_group) {
  uint32_t cpu_delta;
 ...
 */
void sched_clutch_thread_clutch_update(thread_t thread,
  ...
  if (old_clutch) {
    ...
    if (!SCHED_CLUTCH_THREAD_CLUSTER_BOUND(thread)) {
      old_clutch_bucket_group =
          &(old_clutch->sc_clutch_groups[thread->th_sched_bucket]);
    }
 ...
 *
 * The edge scheduler allows threads to be bound to specific clusters. The
 ...
 * that are enqueued in the separate runq have the th_bound_cluster_enqueued
 * flag set to allow easy removal.
 *
...
static boolean_t sched_edge_bound_thread_insert(sched_clutch_root_t root_clutch,
                                                thread_t thread,
                                                integer_t options) {
  /* Update the clutch runnable count and priority */
  sched_clutch_thr_count_inc(&root_clutch->scr_thr_count);
  sched_clutch_root_bucket_t root_bucket =
      &root_clutch->scr_bound_buckets[thread->th_sched_bucket];
  if (root_bucket->scrb_bound_thread_runq.count == 0) {
    sched_clutch_root_bucket_runnable(root_bucket, root_clutch,
                                      mach_absolute_time());
  }
  ...
  /*
   * Trigger an update to the thread's clutch bucket group's priority shift
   * parameters, needed for global timeshare within a clutch bucket group.
   */
  sched_clutch_bucket_group_pri_shift_update(
      sched_clutch_bucket_group_for_thread(thread));
...
static void sched_edge_bound_thread_remove(sched_clutch_root_t root_clutch,
                                           thread_t thread) {
  sched_clutch_root_bucket_t root_bucket =
      &root_clutch->scr_bound_buckets[thread->th_sched_bucket];
  ...
  if (root_bucket->scrb_bound_thread_runq.count == 0) {
    sched_clutch_root_bucket_empty(root_bucket, root_clutch,
                                   mach_absolute_time());
  }
  sched_clutch_root_pri_update(root_clutch);
 ...
 *
 * The Edge scheduler attempts to load balance cluster shared resource intensive
 * threads across clusters in order to reduce contention on the shared
 * resources. It achieves that by maintaining the runnable and running shared
 * resource load on each cluster and balancing the load across multiple
 * clusters.
 *
 * The current implementation for cluster shared resource load balancing looks
 * at the per-cluster load at thread runnable time to enqueue the thread in the
 * appropriate cluster. The thread is enqueued in the cluster bound runqueue to
 * ensure idle CPUs do not steal/rebalance shared resource threads. Some more
 * details for the implementation:
 *
 * - When threads are tagged as shared resource, they go through the cluster
 * selection logic which looks at cluster shared resource loads and picks a
 * cluster accordingly. The thread is enqueued in the cluster bound runqueue.
 *
 * - When the threads start running and call avoid_processor, the load balancing
 * logic will be invoked and cause the thread to be sent to a more preferred
 * cluster if one exists and has no shared resource load.
 *
 * - If a CPU in a preferred cluster is going idle and that cluster has no more
 * shared load, it will look at running shared resource threads on foreign
 * clusters and actively rebalance them.
 *
 * - Runnable shared resource threads are not stolen by the preferred cluster
 * CPUs as they go idle intentionally.
 *
 * - One caveat of this design is that if a preferred CPU has already run and
 * finished its shared resource thread execution, it will not go out and steal
 * the runnable thread in the non-preferred cluster. The rebalancing will happen
 * when the thread actually runs on a non-preferred cluster and one of the
 *   events listed above happen.
 *
 * - Also it currently does not consider other properties such as thread
 * priorities and qos level thread load in the thread placement decision.
 *
 * Edge Scheduler cluster shared resource thread scheduling policy
 *
 * The threads for shared resources can be scheduled using one of the two
 * policies:
 *
 * EDGE_SHARED_RSRC_SCHED_POLICY_RR
 * This policy distributes the threads so that they spread across all available
 * clusters irrespective of type. The idea is that this scheduling policy will
 * put a shared resource thread on each cluster on the platform before it starts
 * doubling up on clusters.
 *
 * EDGE_SHARED_RSRC_SCHED_POLICY_NATIVE_FIRST
 * This policy distributes threads so that the threads first fill up all the
 * capacity on the preferred cluster and its homogeneous peers before spilling
 * to different core type. The current implementation defines capacity based on
 * the number of CPUs in the cluster; so a cluster's shared resource is
 * considered full if there are "n" runnable + running shared resource threads
 * on the cluster with n cpus. This policy is different from the default
 * scheduling policy of the edge scheduler since this always tries to fill up
 * the native clusters to capacity even when non-native clusters might be idle.
 */
...
static const edge_shared_rsrc_sched_policy_t
    edge_shared_rsrc_policy[CLUSTER_SHARED_RSRC_TYPE_COUNT] = {
        [CLUSTER_SHARED_RSRC_TYPE_RR] = EDGE_SHARED_RSRC_SCHED_POLICY_RR,
        [CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST] =
            EDGE_SHARED_RSRC_SCHED_POLICY_NATIVE_FIRST,
};

static void
sched_edge_shared_rsrc_runnable_load_incr(sched_clutch_root_t root_clutch,
                                          thread_t thread) {
  ...
  if (thread_shared_rsrc_policy_get(thread,
                                    CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST)) {
    root_clutch->scr_shared_rsrc_load_runnable
        [CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST]++;
    thread->th_shared_rsrc_enqueued[CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST] =
        true;
  }
}

static void
sched_edge_shared_rsrc_runnable_load_decr(sched_clutch_root_t root_clutch,
                                          thread_t thread) {
  for (cluster_shared_rsrc_type_t shared_rsrc_type =
           CLUSTER_SHARED_RSRC_TYPE_MIN;
       shared_rsrc_type < CLUSTER_SHARED_RSRC_TYPE_COUNT; shared_rsrc_type++) {
...
uint16_t sched_edge_shared_rsrc_runnable_load(
    sched_clutch_root_t root_clutch,
    cluster_shared_rsrc_type_t shared_rsrc_type) {
 ...
 *
 * Routine used to determine if the constrained resource for the pset is idle.
 * This is used by a CPU going idle to decide if it should rebalance a running
 * shared resource thread from a non-preferred cluster.
 */
static boolean_t
sched_edge_shared_rsrc_idle(processor_set_t pset,
                            cluster_shared_rsrc_type_t shared_rsrc_type) {
 ...
 *
 * Routine to lookup the highest priority runnable thread in a bounded root
 * bucket.
 */
static thread_t
sched_clutch_thread_bound_lookup(__unused sched_clutch_root_t root_clutch,
                                 sched_clutch_root_bucket_t root_bucket,
  ...
  if ((prev_thread != NULL) &&
      (bound_thread == NULL ||
       sched_clutch_pri_greater_than_tiebreak(prev_thread->sched_pri,
                                              bound_thread->sched_pri,
                                              processor->first_timeslice))) {
    return prev_thread;
  }
 ...
 *
 * The pending time on the clutch_bucket_group allows the scheduler to track if
 * it needs to ageout the CPU usage because the clutch_bucket_group has been
 * pending for a very long time. The pending time is set to the timestamp as
 * soon as a thread becomes runnable. When a thread is picked up for execution
 * from this clutch_bucket_group, the pending time is advanced to the time of
 * thread selection.
 *
 * Since threads for a clutch bucket group can be added or removed from multiple
 * CPUs simulataneously, it is important that the updates to thread counts and
 * pending timestamps happen atomically. The implementation relies on the
 * following aspects to make that work as expected:
 * - The clutch scheduler would be deployed on single cluster platforms where
 * the pset lock is held when threads are added/removed and pending timestamps
 * are updated
 * - The thread count and pending timestamp can be updated atomically using
 * double wide 128 bit atomics
 *
 * Clutch bucket group interactivity timestamp and score updates also rely on
 * the properties above to atomically update the interactivity score for a
 * clutch bucket group.
 */
...
static void sched_clutch_bucket_group_thr_count_inc(
    sched_clutch_bucket_group_t clutch_bucket_group, uint64_t timestamp) {
  sched_clutch_counter_time_t old_pending_data;
  sched_clutch_counter_time_t new_pending_data;
  os_atomic_rmw_loop(
      &clutch_bucket_group->scbg_pending_data.scct_packed,
      old_pending_data.scct_packed, new_pending_data.scct_packed, relaxed, {
...
static void sched_clutch_bucket_group_thr_count_dec(
    sched_clutch_bucket_group_t clutch_bucket_group, uint64_t timestamp) {
  sched_clutch_counter_time_t old_pending_data;
  sched_clutch_counter_time_t new_pending_data;
  os_atomic_rmw_loop(
      ...
      old_pending_data.scct_packed, new_pending_data.scct_packed, relaxed, {
        ...
        if (new_pending_data.scct_count == 0) {
          new_pending_data.scct_timestamp =
              SCHED_CLUTCH_BUCKET_GROUP_PENDING_INVALID;
        } else {
          new_pending_data.scct_timestamp = timestamp;
        }
...
static uint8_t sched_clutch_bucket_group_pending_ageout(
    sched_clutch_bucket_group_t clutch_bucket_group, uint64_t timestamp) {
  int bucket_load =
      sched_clutch_global_bucket_load_get(clutch_bucket_group->scbg_bucket);
  sched_clutch_counter_time_t old_pending_data;
  sched_clutch_counter_time_t new_pending_data;
  ...
  os_atomic_rmw_loop(
      ...
      old_pending_data.scct_packed, new_pending_data.scct_packed, relaxed, {
        ...
        bool no_pending_time =
            (old_pending_ts == SCHED_CLUTCH_BUCKET_GROUP_PENDING_INVALID);
        ...
        /*
         * Other buckets should get a chance to run first before artificially
         * boosting this clutch bucket group's interactivity score, at least
         * when the entire root bucket is getting a large enough share of CPU.
         */
        uint64_t interactivity_delta =
            sched_clutch_bucket_group_pending_delta[clutch_bucket_group
                                                        ->scbg_bucket] +
            (bucket_load *
             sched_clutch_thread_quantum[clutch_bucket_group->scbg_bucket]);
        if (pending_delta < interactivity_delta) {
          os_atomic_rmw_loop_give_up();
        }
        cpu_usage_shift = (pending_delta / interactivity_delta);
        new_pending_data.scct_timestamp =
            old_pending_ts + (cpu_usage_shift * interactivity_delta);
...
static boolean_t
sched_edge_thread_should_be_inserted_as_bound(sched_clutch_root_t root_clutch,
                                              thread_t thread) {
  /*
   * Check if the thread is bound and is being enqueued in its desired bound
   * cluster. If the thread is cluster-bound but to a different cluster, we
   * should enqueue as unbound.
   */
  if (SCHED_CLUTCH_THREAD_CLUSTER_BOUND(thread) &&
      (sched_edge_thread_bound_cluster_id(thread) ==
       root_clutch->scr_cluster_id)) {
    return TRUE;
  }
  /*
   * Use bound runqueue for shared resource threads. See "cluster shared
   * resource threads load balancing" section for details.
   */
  if (sched_edge_thread_shared_rsrc_type(thread) !=
      CLUSTER_SHARED_RSRC_TYPE_NONE) {
    return TRUE;
  }
  return FALSE;
}
 ...
 * are happening under the pset lock of the only cluster present on the
 * platform.
 */
static void sched_clutch_bucket_group_thr_count_inc(
    sched_clutch_bucket_group_t clutch_bucket_group, uint64_t timestamp) {
...
static uint8_t sched_clutch_bucket_group_pending_ageout(
   ...
   * this clutch bucket group's interactivity score, at least when the entire
   * root bucket is getting a large enough share of CPU.
   */
  uint64_t interactivity_delta =
      sched_clutch_bucket_group_pending_delta[clutch_bucket_group
                                                  ->scbg_bucket] +
...
static uint8_t sched_clutch_bucket_group_interactivity_score_calculate(
  ...
  if (sched_clutch_bucket_is_above_timeshare(
          clutch_bucket_group->scbg_bucket)) {
    /*
     * Since the root bucket selection algorithm for Above UI looks at clutch
     * bucket priorities, make sure all AboveUI buckets are marked interactive.
     */
    assert(clutch_bucket_group->scbg_interactivity_data.scct_count ==
           (2 * sched_clutch_bucket_group_interactive_pri));
    return (uint8_t)clutch_bucket_group->scbg_interactivity_data.scct_count;
  }
  /* Check if the clutch bucket group CPU usage needs to be aged out due to
   * pending time */
  uint8_t pending_intervals =
      sched_clutch_bucket_group_pending_ageout(clutch_bucket_group, timestamp);
  /* Adjust CPU stats based on the calculated shift and to make sure only recent
   * behavior is used */
  sched_clutch_bucket_group_cpu_adjust(clutch_bucket_group, pending_intervals);
  uint8_t interactivity_score =
  ...
  os_atomic_rmw_loop(&clutch_bucket_group->scbg_interactivity_data.scct_packed,
                     old_interactivity_data.scct_packed,
                     new_interactivity_data.scct_packed, relaxed, {
                       new_interactivity_data.scct_count =
                           old_interactivity_data.scct_count;
 ...
 *
 * The clutch bucket group maintains the number of runnable/running threads in
 * the group. Since the blocked time of the clutch bucket group is based on this
 * count, it is important to make sure the blocking timestamp and the run count
 * are updated atomically.
 *
 * Since the run count increments happen without any pset locks held, the
 * scheduler updates the count & timestamp using double wide 128 bit atomics.
 */

static uint32_t sched_clutch_bucket_group_run_count_inc(
    sched_clutch_bucket_group_t clutch_bucket_group) {
  ...
  os_atomic_rmw_loop(
      &clutch_bucket_group->scbg_blocked_data.scct_packed,
      old_blocked_data.scct_packed, new_blocked_data.scct_packed, relaxed, {
        ...
        if (old_blocked_data.scct_count == 0) {
          new_blocked_data.scct_timestamp =
              SCHED_CLUTCH_BUCKET_GROUP_BLOCKED_TS_INVALID;
          update_blocked_time = true;
        }
      });
  if (update_blocked_time && (old_blocked_data.scct_timestamp !=
                              SCHED_CLUTCH_BUCKET_GROUP_BLOCKED_TS_INVALID)) {
    uint64_t ctime = mach_absolute_time();
    if (ctime > old_blocked_data.scct_timestamp) {
      uint64_t blocked_time = ctime - old_blocked_data.scct_timestamp;
      blocked_time =
          MIN(blocked_time, sched_clutch_bucket_group_adjust_threshold);
      os_atomic_add(
          &(clutch_bucket_group->scbg_cpu_data.cpu_data.scbcd_cpu_blocked),
          (clutch_cpu_data_t)blocked_time, relaxed);
    }
  }
...
static inline sched_clutch_bucket_t
sched_clutch_bucket_for_thread(sched_clutch_root_t root_clutch,
                               thread_t thread) {
  sched_clutch_t clutch = sched_clutch_for_thread(thread);
  ...
  sched_clutch_bucket_group_t clutch_bucket_group =
      &(clutch->sc_clutch_groups[thread->th_sched_bucket]);
  sched_clutch_bucket_t clutch_bucket =
      &(clutch_bucket_group->scbg_clutch_buckets[root_clutch->scr_cluster_id]);
...
sched_clutch_bucket_group_for_thread(thread_t prev_thread) {
  sched_clutch_t clutch =
      sched_clutch_for_thread_group(prev_thread->thread_group);
 ...
 */
static boolean_t sched_clutch_thread_insert(sched_clutch_root_t root_clutch,
                                            thread_t thread,
                                            integer_t options) {
  ...
  sched_clutch_hierarchy_locked_assert(root_clutch);
#if CONFIG_SCHED_EDGE
  sched_edge_cluster_cumulative_count_incr(root_clutch,
  ...
  assert(thread->thread_group == clutch->sc_tg);
  sched_clutch_bucket_t clutch_bucket =
      sched_clutch_bucket_for_thread(root_clutch, thread);
  assert((clutch_bucket->scb_root == NULL) ||
         (clutch_bucket->scb_root == root_clutch));
   ...
   * - A stable priority queue linkage which is the main runqueue (based on
   * sched_pri) for the clutch bucket
   * - A regular priority queue linkage which is based on thread's base/promoted
   * pri (used for clutch bucket priority calculation)
   * - A queue linkage used for timesharing operations of threads at the
   * scheduler tick
   */

  /* Insert thread into the clutch_bucket stable priority runqueue using
   * sched_pri */
  thread->th_clutch_runq_link.stamp = current_timestamp;
  priority_queue_entry_set_sched_pri(
      &clutch_bucket->scb_thread_runq, &thread->th_clutch_runq_link,
      thread->sched_pri,
      (options & SCHED_TAILQ) ? PRIORITY_QUEUE_ENTRY_NONE
                              : PRIORITY_QUEUE_ENTRY_PREEMPTED);
  priority_queue_insert(&clutch_bucket->scb_thread_runq,
                        &thread->th_clutch_runq_link);

  /* Insert thread into clutch_bucket priority queue based on the promoted or
   * base priority */
  priority_queue_entry_set_sched_pri(
      &clutch_bucket->scb_clutchpri_prioq, &thread->th_clutch_pri_link,
      sched_thread_sched_pri_promoted(thread) ? thread->sched_pri
                                              : thread->base_pri,
      false);
  priority_queue_insert(&clutch_bucket->scb_clutchpri_prioq,
                        &thread->th_clutch_pri_link);

  /* Insert thread into timesharing queue of the clutch bucket */
  enqueue_tail(&clutch_bucket->scb_thread_timeshare_queue,
               &thread->th_clutch_timeshare_link);

  /* Increment the urgency counter for the root if necessary */
  sched_clutch_root_urgency_inc(root_clutch, thread);

  os_atomic_inc(&clutch->sc_thr_count, relaxed);
  sched_clutch_bucket_group_thr_count_inc(clutch_bucket->scb_group,
                                          current_timestamp);

  /* Enqueue the clutch into the hierarchy (if needed) and update properties;
   * pick the insertion order based on thread options */
  sched_clutch_bucket_options_t scb_options =
      (options & SCHED_HEADQ) ? SCHED_CLUTCH_BUCKET_OPTIONS_HEADQ
                              : SCHED_CLUTCH_BUCKET_OPTIONS_TAILQ;
  if (clutch_bucket->scb_thr_count == 0) {
    sched_clutch_thr_count_inc(&clutch_bucket->scb_thr_count);
    sched_clutch_thr_count_inc(&root_clutch->scr_thr_count);
    result = sched_clutch_bucket_runnable(clutch_bucket, root_clutch,
                                          current_timestamp, scb_options);
  } else {
    sched_clutch_thr_count_inc(&clutch_bucket->scb_thr_count);
    sched_clutch_thr_count_inc(&root_clutch->scr_thr_count);
    result = sched_clutch_bucket_update(clutch_bucket, root_clutch,
                                        current_timestamp, scb_options);
  }

  KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_CLUTCH_THR_COUNT) |
           DBG_FUNC_NONE,
       root_clutch->scr_cluster_id,
       thread_group_get_id(clutch_bucket->scb_group->scbg_clutch->sc_tg),
       clutch_bucket->scb_bucket,
       SCHED_CLUTCH_DBG_THR_COUNT_PACK(
           root_clutch->scr_thr_count,
           os_atomic_load(&clutch->sc_thr_count, relaxed),
 ...
 */
static void sched_clutch_thread_remove(sched_clutch_root_t root_clutch,
                                       thread_t thread,
                                       uint64_t current_timestamp,
                                       sched_clutch_bucket_options_t options) {
  sched_clutch_hierarchy_locked_assert(root_clutch);
#if CONFIG_SCHED_EDGE
  sched_edge_cluster_cumulative_count_decr(root_clutch,
                                           thread->th_sched_bucket);
  ...
  priority_queue_remove(&clutch_bucket->scb_thread_runq,
                        &thread->th_clutch_runq_link);
  remqueue(&thread->th_clutch_timeshare_link);

  priority_queue_remove(&clutch_bucket->scb_clutchpri_prioq,
                        &thread->th_clutch_pri_link);
  ...
  sched_clutch_bucket_group_thr_count_dec(clutch_bucket->scb_group,
                                          current_timestamp);
  sched_clutch_thr_count_dec(&root_clutch->scr_thr_count);
 ...
 * Edge scheduler.
 */
static thread_t sched_clutch_thread_unbound_lookup(
    sched_clutch_root_t root_clutch, sched_clutch_root_bucket_t root_bucket,
    processor_t _Nullable processor, thread_t _Nullable prev_thread) {
  ...
  sched_clutch_bucket_t clutch_bucket =
      sched_clutch_root_bucket_highest_clutch_bucket(
          root_clutch, root_bucket, processor, prev_thread, &chose_prev_thread);
  assert(clutch_bucket != NULL);

  if (chose_prev_thread) {
    /* We have determined that prev_thread is the highest thread, based on the
     * Clutch bucket level policy */
    assert(processor != NULL && prev_thread != NULL);
    return prev_thread;
  }

  /* Find the highest priority runnable thread in this clutch bucket */
  thread_t thread = priority_queue_max(&clutch_bucket->scb_thread_runq,
...
sched_clutch_root_bucket_for_thread(sched_clutch_root_t root_clutch,
 ...
 * depending on the traverse mode and whether it is the previously running
 * thread. Always called with the pset lock held.
 */
static thread_t sched_clutch_hierarchy_thread_highest(
    sched_clutch_root_t root_clutch, processor_t processor,
    thread_t _Nullable prev_thread, sched_clutch_traverse_mode_t mode) {
  assert(mode != SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY ||
         prev_thread == NULL);
  sched_clutch_hierarchy_locked_assert(root_clutch);
  ...
  sched_clutch_root_bucket_t prev_root_bucket =
      prev_thread != NULL
          ? sched_clutch_root_bucket_for_thread(root_clutch, prev_thread)
          : NULL;
  sched_clutch_root_bucket_t root_bucket =
      sched_clutch_root_highest_root_bucket(
          root_clutch, current_timestamp, SCHED_CLUTCH_HIGHEST_ROOT_BUCKET_ALL,
          prev_root_bucket, prev_thread, &chose_prev_thread, mode, &debug_info);
  if (chose_prev_thread) {
  ...
  if (root_bucket == NULL) {
    /* The Clutch hierarchy has no runnable threads, including the previous
     * thread */
    assert(sched_clutch_root_count(root_clutch) == 0);
    assert(prev_thread == NULL);
    return NULL;
  ...
  if (root_bucket != prev_root_bucket) {
    /* We have ruled out continuing to run the previous thread, based on the
     * root bucket level policy */
    prev_thread = NULL;
    assert((mode == SCHED_CLUTCH_TRAVERSE_CHECK_PREEMPT) ||
           (prev_root_bucket == NULL) ||
           (prev_root_bucket->scrb_bucket >= root_bucket->scrb_bucket) ||
           (root_bucket->scrb_starvation_avoidance) ||
           (prev_root_bucket->scrb_bound != root_bucket->scrb_bound) ||
           (root_bucket->scrb_warp_remaining > 0 &&
            root_bucket->scrb_warped_deadline > current_timestamp &&
            prev_root_bucket->scrb_warp_remaining == 0));
  }

  if (root_bucket->scrb_bound) {
    highest_thread = sched_clutch_thread_bound_lookup(root_clutch, root_bucket,
                                                      processor, prev_thread);
  } else {
    highest_thread = sched_clutch_thread_unbound_lookup(
        root_clutch, root_bucket, processor, prev_thread);
  }
  ...
  if (mode == SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY ||
    ...
    assert(mode != SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY ||
           highest_thread != processor->active_thread);
    sched_clutch_thread_remove(root_clutch, highest_thread, current_timestamp,
...
done_selecting_thread:
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE,
      MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_CLUTCH_THREAD_SELECT) |
          DBG_FUNC_NONE,
      thread_tid(highest_thread),
      thread_group_get_id(highest_thread->thread_group),
      root_bucket->scrb_bucket, debug_info.scdts_trace_data_packed, 0);
 ...
 */
static uint32_t sched_clutch_root_urgency(sched_clutch_root_t root_clutch) {
 ...
 */
static uint32_t
sched_clutch_root_count_sum(__unused sched_clutch_root_t root_clutch) {
  return 0;
}
 ...
 * thread in the hierarchy.
 */
static int sched_clutch_root_priority(sched_clutch_root_t root_clutch) {
 ...
 */
uint32_t sched_clutch_root_count(sched_clutch_root_t root_clutch) {
 ...
 *
 * Routine to check if the foreign clutch bucket priority list is empty for a
 * cluster.
 */
static boolean_t
sched_clutch_root_foreign_empty(sched_clutch_root_t root_clutch) {
 ...
 *
 * Routine to return the thread in the highest priority clutch bucket in a
 * cluster. Must be called with the pset for the cluster locked.
 */
static thread_t sched_clutch_root_highest_foreign_thread_remove(
    sched_clutch_root_t root_clutch) {
  ...
  sched_clutch_bucket_t clutch_bucket =
      priority_queue_max(&root_clutch->scr_foreign_buckets,
                         struct sched_clutch_bucket, scb_foreignlink);
  thread = priority_queue_max(&clutch_bucket->scb_thread_runq, struct thread,
                              th_clutch_runq_link);
 ...
 * values from there.
 */
uint32_t sched_clutch_thread_pri_shift(thread_t thread, sched_bucket_t bucket) {
  if (!SCHED_CLUTCH_THREAD_ELIGIBLE(thread)) {
    return INT8_MAX;
  }
  assert(bucket != TH_BUCKET_RUN);
...
static void sched_clutch_init(void);

static thread_t sched_clutch_steal_thread(processor_set_t pset);
...
static boolean_t sched_clutch_processor_enqueue(processor_t processor,
                                                thread_t thread,
                                                sched_options_t options);

static boolean_t sched_clutch_processor_queue_remove(processor_t processor,
                                                     thread_t thread);

static ast_t sched_clutch_processor_csw_check(processor_t processor);

static boolean_t
sched_clutch_processor_queue_has_priority(processor_t processor, int priority,
                                          boolean_t gte);

static int sched_clutch_runq_count(processor_t processor);

static boolean_t sched_clutch_processor_queue_empty(processor_t processor);

#if !SCHED_TEST_HARNESS

static uint64_t sched_clutch_runq_stats_count_sum(processor_t processor);

#endif /* !SCHED_TEST_HARNESS */

static int sched_clutch_processor_bound_count(processor_t processor);

static void sched_clutch_pset_init(processor_set_t pset);

static void sched_clutch_processor_init(processor_t processor);

static thread_t
sched_clutch_processor_highest_thread(processor_t processor,
                                      sched_clutch_traverse_mode_t mode);

static thread_t sched_clutch_choose_thread(processor_t processor, int priority,
                                           thread_t prev_thread, ast_t reason);

#if !SCHED_TEST_HARNESS

static void sched_clutch_processor_queue_shutdown(processor_t processor);

#endif /* !SCHED_TEST_HARNESS */

static sched_mode_t sched_clutch_initial_thread_sched_mode(task_t parent_task);

static uint32_t sched_clutch_initial_quantum_size(thread_t thread);

static uint32_t sched_clutch_run_incr(thread_t thread);

static uint32_t sched_clutch_run_decr(thread_t thread);

static void sched_clutch_update_thread_bucket(thread_t thread);

#if !SCHED_TEST_HARNESS

static void sched_clutch_thread_group_recommendation_change(
    struct thread_group *tg, cluster_type_t new_recommendation);
...
const struct sched_dispatch_table sched_clutch_dispatch = {
    ...
    .thread_group_recommendation_change =
        sched_clutch_thread_group_recommendation_change,
...
static void sched_clutch_tunables_init(void) {
  sched_clutch_us_to_abstime(sched_clutch_root_bucket_wcel_us,
                             sched_clutch_root_bucket_wcel);
  sched_clutch_us_to_abstime(sched_clutch_root_bucket_warp_us,
                             sched_clutch_root_bucket_warp);
  sched_clutch_us_to_abstime(sched_clutch_thread_quantum_us,
                             sched_clutch_thread_quantum);
  clock_interval_to_absolutetime_interval(
      SCHED_CLUTCH_BUCKET_GROUP_ADJUST_THRESHOLD_USECS, NSEC_PER_USEC,
      &sched_clutch_bucket_group_adjust_threshold);
  assert(sched_clutch_bucket_group_adjust_threshold <= CLUTCH_CPU_DATA_MAX);
  sched_clutch_us_to_abstime(sched_clutch_bucket_group_pending_delta_us,
...
static inline bool sched_clutch_pri_greater_than_tiebreak(int pri_one,
                                                          int pri_two,
                                                          bool one_wins_ties) {
  if (one_wins_ties) {
 ...
 * should be selected to run next, optionally comparing against the previously
 * running thread. Removes the highest thread from the runqueue, depending on
 * the traverse mode and whether the highest thread is the previously running
 * thread.
 *
...
sched_clutch_processor_highest_thread(processor_t processor,
  ...
  bool has_prev_thread = mode == SCHED_CLUTCH_TRAVERSE_CHECK_PREEMPT ||
                         mode == SCHED_CLUTCH_TRAVERSE_REMOVE_CONSIDER_CURRENT;
  ...
  if (has_prev_thread) {
    if (prev_thread->sched_pri >= BASEPRI_RTQUEUES) {
      /* The previous thread is real-time and thus guaranteed higher than the
       * non-RT runqueue */
      return prev_thread;
    }
    /* Allow the previous thread to influence the priority comparison of Clutch
     * hierarchy vs. processor-bound runqueue */
  ...
  bool prev_thread_is_not_processor_bound =
      has_prev_thread && (prev_thread->bound_processor == NULL);
  bool prev_thread_is_processor_bound =
      has_prev_thread && (prev_thread->bound_processor != NULL);
  ...
  if (clutch_pri > bound_pri) {
    ...
    next_thread = sched_clutch_hierarchy_thread_highest(
        root_clutch, processor,
        prev_thread_is_not_processor_bound ? prev_thread : NULL, mode);
  } else {
    if (bound_runq->count == 0 ||
        (prev_thread_is_processor_bound &&
         sched_clutch_pri_greater_than_tiebreak(prev_thread->sched_pri,
                                                bound_runq->highq,
                                                processor->first_timeslice))) {
      goto found_thread;
    }
    next_thread = (mode == SCHED_CLUTCH_TRAVERSE_REMOVE_CONSIDER_CURRENT ||
                   mode == SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY)
                      ? run_queue_dequeue(bound_runq, SCHED_HEADQ)
                      : run_queue_peek(bound_runq);
    assert(mode == SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY ||
...
static thread_t sched_clutch_choose_thread(processor_t processor,
                                           __unused int priority,
                                           thread_t _Nullable prev_thread,
                                           __unused ast_t reason) {
  assert(prev_thread == NULL || prev_thread == processor->active_thread);
  return sched_clutch_processor_highest_thread(
      processor, prev_thread != NULL
                     ? SCHED_CLUTCH_TRAVERSE_REMOVE_CONSIDER_CURRENT
                     : SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY);
}
...
static boolean_t sched_clutch_processor_enqueue(processor_t processor,
  ...
  if (SCHED_CLUTCH_THREAD_ELIGIBLE(thread)) {
    sched_clutch_root_t pset_clutch_root =
        sched_clutch_processor_root_clutch(processor);
...
static boolean_t sched_clutch_processor_queue_empty(processor_t processor) {
  return sched_clutch_root_count(
             sched_clutch_processor_root_clutch(processor)) == 0 &&
         sched_clutch_bound_runq(processor)->count == 0;
}

static ast_t sched_clutch_processor_csw_check(processor_t processor) {
  assert(processor->active_thread != NULL);
  thread_t runqueue_thread = sched_clutch_processor_highest_thread(
      processor, SCHED_CLUTCH_TRAVERSE_CHECK_PREEMPT);
  if (runqueue_thread != processor->active_thread) {
    /* Found a better thread to run */
    if (sched_clutch_root_urgency(
            sched_clutch_processor_root_clutch(processor)) > 0 ||
...
static boolean_t
sched_clutch_processor_queue_has_priority(__unused processor_t processor,
                                          __unused int priority,
...
static int sched_clutch_runq_count(processor_t processor) {
  return (int)sched_clutch_root_count(
             sched_clutch_processor_root_clutch(processor)) +
         sched_clutch_bound_runq(processor)->count;
}
...
static uint64_t sched_clutch_runq_stats_count_sum(processor_t processor) {
  ...
  if (processor->cpu_id == processor->processor_set->cpu_set_low) {
    return bound_sum + sched_clutch_root_count_sum(
                           sched_clutch_processor_root_clutch(processor));
  } else {
    return bound_sum;
  }
...
static void sched_clutch_processor_queue_shutdown(processor_t processor) {
  ...
  /* We only need to migrate threads if this is the last active processor in the
   * pset */
  ...
  while (sched_clutch_root_count(pset_clutch_root) > 0) {
    thread = sched_clutch_hierarchy_thread_highest(
        pset_clutch_root, processor, NULL,
        SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY);
...
static thread_t sched_clutch_steal_thread(__unused processor_set_t pset) {
  /* Thread stealing is not enabled for single cluster clutch scheduler
   * platforms */
  return THREAD_NULL;
}
...
sched_clutch_thread_update_scan(sched_update_scan_context_t scan_context) {
  ...
  /*
   *  We update the threads associated with each processor (bound and idle
   * threads) and then update the threads in each pset runqueue.
   */
  ...
  do {
    do {
      ...
      restart_needed =
          runq_scan(sched_clutch_bound_runq(processor), scan_context);
    ...
    do {
      ...
      while (pset != NULL) {
        ...
        if (sched_clutch_root_count(&pset->pset_clutch_root) > 0) {
          for (sched_bucket_t bucket = TH_BUCKET_SHARE_FG;
               bucket < TH_BUCKET_SCHED_MAX; bucket++) {
            restart_needed =
                runq_scan(&pset->pset_clutch_root.scr_bound_buckets[bucket]
          ...
          queue_t clutch_bucket_list =
              &pset->pset_clutch_root.scr_clutch_buckets;
          sched_clutch_bucket_t clutch_bucket;
          qe_foreach_element(clutch_bucket, clutch_bucket_list, scb_listlink) {
            sched_clutch_bucket_group_timeshare_update(
                clutch_bucket->scb_group, clutch_bucket,
                scan_context->sched_tick_last_abstime);
            restart_needed = sched_clutch_timeshare_scan(
                &clutch_bucket->scb_thread_timeshare_queue,
                clutch_bucket->scb_thr_count, scan_context);
            if (restart_needed) {
              break;
            }
          }
 ...
 * or demotions.
 */
static boolean_t sched_thread_sched_pri_promoted(thread_t thread) {
...
static uint32_t sched_clutch_run_incr(thread_t thread) {
  assert((thread->state & (TH_RUN | TH_IDLE)) == TH_RUN);
  uint32_t new_count =
      os_atomic_inc(&sched_run_buckets[TH_BUCKET_RUN], relaxed);
...
static uint32_t sched_clutch_run_decr(thread_t thread) {
  assert((thread->state & (TH_RUN | TH_IDLE)) != TH_RUN);
  uint32_t new_count =
      os_atomic_dec(&sched_run_buckets[TH_BUCKET_RUN], relaxed);
...
void sched_clutch_update_thread_bucket(thread_t thread) {
  sched_bucket_t old_bucket = thread->th_sched_bucket;
  thread_assert_runq_null(thread);
  int pri = (sched_thread_sched_pri_promoted(thread)) ? thread->sched_pri
                                                      : thread->base_pri;
...
static void sched_clutch_thread_group_recommendation_change(
    __unused struct thread_group *tg,
...
static void sched_edge_init(void);

static void sched_edge_pset_init(processor_set_t pset);

static thread_t sched_edge_processor_idle(processor_set_t pset);

static boolean_t sched_edge_processor_queue_empty(processor_t processor);

static void sched_edge_processor_queue_shutdown(processor_t processor);

static processor_t sched_edge_choose_processor(processor_set_t pset,
                                               processor_t processor,
                                               thread_t thread,
                                               sched_options_t *options_inout);

static void sched_edge_quantum_expire(thread_t thread);

static bool sched_edge_thread_avoid_processor(processor_t processor,
                                              thread_t thread, ast_t reason);

static bool sched_edge_balance(processor_t cprocessor, processor_set_t cpset);

static void sched_edge_check_spill(processor_set_t pset, thread_t thread);

static bool sched_edge_thread_should_yield(processor_t processor,
                                           thread_t thread);

static void sched_edge_pset_made_schedulable(processor_t processor,
                                             processor_set_t dst_pset,
                                             boolean_t drop_lock);

static void sched_edge_cpu_init_completed(void);

static bool sched_edge_thread_eligible_for_pset(thread_t thread,
                                                processor_set_t pset);

static bool sched_edge_steal_thread_enabled(processor_set_t pset);

static sched_ipi_type_t sched_edge_ipi_policy(processor_t dst, thread_t thread,
                                              boolean_t dst_idle,
                                              sched_ipi_event_t event);

static uint32_t sched_edge_qos_max_parallelism(int qos, uint64_t options);

static uint32_t sched_edge_cluster_load_metric(processor_set_t pset,
                                               sched_bucket_t sched_bucket);

static uint32_t sched_edge_run_count_incr(thread_t thread);

static bool sched_edge_stir_the_pot_core_type_is_desired(processor_set_t pset);
 ...
 */
static uint32_t sched_edge_thread_bound_cluster_id(thread_t thread) {
...
static processor_set_t sched_edge_migrate_candidate(
    processor_set_t preferred_pset, thread_t thread,
    processor_set_t locked_pset, bool switch_pset_locks,
    processor_t *processor_hint_out, sched_options_t *options_inout);

static_assert(sizeof(sched_clutch_edge) == sizeof(uint64_t),
              "sched_clutch_edge fits in 64 bits");

/*
 * sched_edge_config_set()
 *
 * Support to update an edge configuration. Typically used by CLPC to affect
 * thread migration policies in the scheduler.
 */
static void sched_edge_config_set(uint32_t src_cluster, uint32_t dst_cluster,
                                  sched_bucket_t bucket,
                                  sched_clutch_edge edge_config) {
  os_atomic_store(&pset_array[src_cluster]->sched_edges[dst_cluster][bucket],
                  edge_config, relaxed);
}
 ...
 *
 * Support to get an edge configuration. Typically used by CLPC to query edge
 * configs to decide if it needs to update edges.
 */
static sched_clutch_edge sched_edge_config_get(uint32_t src_cluster,
                                               uint32_t dst_cluster,
                                               sched_bucket_t bucket) {
  return os_atomic_load(
      &pset_array[src_cluster]->sched_edges[dst_cluster][bucket], relaxed);
}

/*
 * sched_edge_config_pset_push()
 *
 * After using sched_edge_config_set() to update edge tunables outgoing from a
 * particular source pset, this function should be called in order to propagate
 * the updates to derived metadata for the pset, such as search orders for
 * outgoing spill and steal.
 */
static void sched_edge_config_pset_push(uint32_t src_pset_id) {
  ...
  for (sched_bucket_t bucket = 0; bucket < TH_BUCKET_SCHED_MAX; bucket++) {
    ...
    for (int i = 0; i < search_order_len; i++, dst_pset_id++) {
      ...
      sched_clutch_edge edge =
          sched_edge_config_get(src_pset->pset_id, dst_pset_id, bucket);
      search_order_datas[i].spsosd_migration_weight =
          edge.sce_migration_allowed ? edge.sce_migration_weight : UINT32_MAX;
    }
    sched_pset_search_order_compute(
        &src_pset->spill_search_order[bucket], search_order_datas,
        search_order_len, sched_edge_search_order_weight_then_locality_cmp);
  }
}

static int sched_edge_search_order_weight_then_locality(const void *a,
                                                        const void *b) {
  const sched_pset_search_order_sort_data_t *data_a =
      (const sched_pset_search_order_sort_data_t *)a;
  const sched_pset_search_order_sort_data_t *data_b =
      (const sched_pset_search_order_sort_data_t *)b;
  ...
  if (data_a->spsosd_migration_weight != data_b->spsosd_migration_weight) {
    return (data_a->spsosd_migration_weight < data_b->spsosd_migration_weight)
               ? -1
               : 1;
  }

  bool is_local_a = bitmap_test(data_a->spsosd_src_pset->local_psets,
                                data_a->spsosd_dst_pset_id);
  bool is_local_b = bitmap_test(data_b->spsosd_src_pset->local_psets,
                                data_b->spsosd_dst_pset_id);
...
cmpfunc_t sched_edge_search_order_weight_then_locality_cmp =
    &sched_edge_search_order_weight_then_locality;

/*
 * sched_edge_matrix_set()
 *
 * Routine to update various edges in the edge migration graph. The edge_changed
 * array indicates which edges need to be updated. Both the edge_matrix and
 * edge_changed arrays are matrices with dimension num_psets * num_psets *
 * TH_BUCKET_SCHED_MAX, flattened into a single-dimensional array.
 */
void sched_edge_matrix_set(sched_clutch_edge *edge_matrix, bool *edge_changed,
                           __unused uint64_t flags,
                           __assert_only uint64_t num_psets) {
  assert3u(num_psets, ==, sched_edge_max_clusters);
  uint32_t edge_index = 0;
  for (uint32_t src_cluster = 0; src_cluster < sched_edge_max_clusters;
       src_cluster++) {
    for (uint32_t dst_cluster = 0; dst_cluster < sched_edge_max_clusters;
         dst_cluster++) {
      for (sched_bucket_t bucket = 0; bucket < TH_BUCKET_SCHED_MAX; bucket++) {
        if (edge_changed[edge_index]) {
          sched_edge_config_set(src_cluster, dst_cluster, bucket,
                                edge_matrix[edge_index]);
        }
        edge_index++;
      }
    }
 ...
 *
 * Routine to retrieve various edges in the edge migration graph. The
 * edge_requested array indicates which edges need to be retrieved. Both the
 * edge_matrix and edge_requested arrays are matrices with dimension num_psets *
 * num_psets * TH_BUCKET_SCHED_MAX, flattened into a single-dimensional array.
 */
void sched_edge_matrix_get(sched_clutch_edge *edge_matrix, bool *edge_requested,
                           __unused uint64_t flags,
                           __assert_only uint64_t num_psets) {
  assert3u(num_psets, ==, sched_edge_max_clusters);
  uint32_t edge_index = 0;
  for (uint32_t src_pset = 0; src_pset < sched_edge_max_clusters; src_pset++) {
    for (uint32_t dst_pset = 0; dst_pset < sched_edge_max_clusters;
         dst_pset++) {
      for (sched_bucket_t bucket = 0; bucket < TH_BUCKET_SCHED_MAX; bucket++) {
        if (edge_requested[edge_index]) {
          edge_matrix[edge_index] =
              sched_edge_config_get(src_pset, dst_pset, bucket);
        }
        edge_index++;
      }
    }
...
static void sched_edge_check_spill(__unused processor_set_t pset,
...
static bool sched_edge_thread_should_yield(processor_t processor,
                                           __unused thread_t thread) {
  if (!sched_edge_processor_queue_empty(processor) ||
      (rt_runq_count(processor->processor_set) > 0)) {
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_SHOULD_YIELD) |
             DBG_FUNC_NONE,
         thread_tid(thread), processor->processor_set->pset_cluster_id, 0,
         SCHED_EDGE_YIELD_RUNQ_NONEMPTY);
    return true;
  }
  ...
  if (sched_edge_foreign_runnable_thread_available(processor->processor_set)) {
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_SHOULD_YIELD) |
             DBG_FUNC_NONE,
         thread_tid(thread), processor->processor_set->pset_cluster_id, 0,
         SCHED_EDGE_YIELD_FOREIGN_RUNNABLE);
    return true;
  }
  if (sched_edge_foreign_running_thread_available(processor->processor_set)) {
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_SHOULD_YIELD) |
             DBG_FUNC_NONE,
         thread_tid(thread), processor->processor_set->pset_cluster_id, 0,
         SCHED_EDGE_YIELD_FOREIGN_RUNNING);
    return true;
  }

  processor_set_t steal_candidate =
      sched_edge_steal_candidate(processor->processor_set);
  if (steal_candidate != NULL) {
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_SHOULD_YIELD) |
             DBG_FUNC_NONE,
         thread_tid(thread), processor->processor_set->pset_cluster_id, 0,
         SCHED_EDGE_YIELD_STEAL_POSSIBLE);
    return true;
  }
...
static void sched_edge_processor_queue_shutdown(processor_t processor) {
  ...
  /* We only need to migrate threads if this is the last active or last
   * recommended processor in the pset */
 ...
 * of threads in the runqueue and the average execution time for threads. The
 * metric has special values in the following cases:
 * - UINT32_MAX: If the cluster is not available for scheduling, its load is set
 * to the maximum value to disallow any threads to migrate to this cluster.
 * - 0: If there are idle CPUs in the cluster or an empty runqueue; this allows
 * threads to be spread across the platform quickly for ncpu wide workloads.
 */
static uint32_t sched_edge_cluster_load_metric(processor_set_t pset,
                                               sched_bucket_t sched_bucket) {
 ...
 *
 * The SCHED(steal_thread) scheduler callout is invoked when the processor does
 * not find any thread for execution in its runqueue. The aim of the steal
 * operation is to find other threads running/runnable in other clusters which
 * should be executed here.
 *
 * If the steal callout does not return a thread, the thread_select() logic
 * calls SCHED(processor_balance) callout which is supposed to IPI other CPUs to
 * rebalance threads and idle out the current CPU.
 *
 * = SCHED(steal_thread) for Edge Scheduler =
 *
 * The edge scheduler hooks into sched_edge_processor_idle() for steal_thread.
 * This routine tries to do the following operations in order: (1) Find foreign
 * runnnable threads in non-native cluster runqueues
 * (sched_edge_foreign_runnable_thread_remove()) (2) Check if foreign threads
 * are running on the non-native clusters
 * (sched_edge_foreign_running_thread_available())
 *         - If yes, return THREAD_NULL for the steal callout and
 *         perform rebalancing as part of SCHED(processor_balance) i.e.
 * sched_edge_balance() (3) Steal a thread from another cluster based on edge
 ...
 * sched_edge_balance() to rebalance foreign running threads and idle out this
 * CPU.
 *
 ...
 * The thread_should_yield() logic needs to have the same logic as
 * sched_edge_processor_idle() since that is expecting the same answer as if
 * thread_select() was called on a core with an empty runqueue.
 */

static bool sched_edge_steal_thread_enabled(__unused processor_set_t pset) {
  /*
   * For edge scheduler, the gating for steal is being done by
   * sched_edge_steal_candidate()
   */
  return true;
}
...
static processor_set_t sched_edge_steal_candidate(processor_set_t pset) {
  ...
  for (int cluster_id = 0; cluster_id < sched_edge_max_clusters; cluster_id++) {
    ...
    int highest_bucket = bitmap_lsb_first(
        candidate_pset->pset_clutch_root.scr_unbound_runnable_bitmap,
        TH_BUCKET_SCHED_MAX);
    if (highest_bucket != -1) {
      /* Assumes that higher root buckets have the less restrictive
       * sce_steal_allowed edges */
      sched_clutch_edge edge =
          sched_edge_config_get(cluster_id, dst_cluster_id, highest_bucket);
...
sched_edge_foreign_runnable_thread_available(processor_set_t pset) {
  ...
  for (int cluster = bitmap_first(foreign_pset_bitmap, sched_edge_max_clusters);
       cluster >= 0; cluster = bitmap_next(foreign_pset_bitmap, cluster)) {
    /*
     * For each cluster, see if there are any runnable foreign threads.
     * This check is currently being done without the pset lock to make it cheap
     * for the common case.
     */
...
static thread_t sched_edge_foreign_runnable_thread_remove(processor_set_t pset,
                                                          uint64_t ctime) {
  thread_t thread = THREAD_NULL;
  ...
  for (int cluster = bitmap_first(foreign_pset_bitmap, sched_edge_max_clusters);
    ...
    /*
     * Looks like there are runnable foreign threads in the hierarchy; lock the
     * pset and get the highest priority thread.
     */
    pset_lock(target_pset);
    if (pset_is_recommended(target_pset)) {
      thread = sched_clutch_root_highest_foreign_thread_remove(
          &target_pset->pset_clutch_root);
     ...
     *
     * The current implementation immediately returns as soon as it finds a
     * foreign runnable thread. This could be enhanced to look at highest
     * priority threads from all foreign clusters and pick the highest amongst
     * them. That would need some form of global state across psets to make that
     * kind of a check cheap.
     */
    if (thread != THREAD_NULL) {
      KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_REBAL_RUNNABLE) |
               DBG_FUNC_NONE,
           thread_tid(thread), pset->pset_cluster_id,
           target_pset->pset_cluster_id, 0);
      break;
    }
    /* Looks like the thread escaped after the check but before the pset lock
     * was taken; continue the search */
  }
 ...
 *
 * Routine to determine if the thread running on a CPU is a shared resource
 * thread and can be rebalanced to the cluster with an idle CPU. It is used to
 * determine if a CPU going idle on a pset should rebalance a running shared
 * resource heavy thread from another non-ideal cluster based on the former's
 * shared resource load.
 */
static boolean_t sched_edge_cpu_running_foreign_shared_rsrc_available(
    processor_set_t target_pset, int foreign_cpu, processor_set_t idle_pset) {
  boolean_t idle_pset_shared_rsrc_rr_idle =
      sched_edge_shared_rsrc_idle(idle_pset, CLUSTER_SHARED_RSRC_TYPE_RR);
  if (bit_test(target_pset->cpu_running_cluster_shared_rsrc_thread
                   [CLUSTER_SHARED_RSRC_TYPE_RR],
               foreign_cpu) &&
      !idle_pset_shared_rsrc_rr_idle) {
    return false;
  }

  boolean_t idle_pset_shared_rsrc_biu_idle = sched_edge_shared_rsrc_idle(
      idle_pset, CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST);
  if (bit_test(target_pset->cpu_running_cluster_shared_rsrc_thread
                   [CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST],
               foreign_cpu) &&
      !idle_pset_shared_rsrc_biu_idle) {
    return false;
  }
  return true;
}
...
sched_edge_foreign_running_thread_available(processor_set_t pset) {
  ...
  while (sched_iterate_psets_ordered(pset, &pset->spill_search_order[0],
                                     foreign_pset_bitmap[0], &istate)) {
    ...
    uint64_t running_foreign_bitmap =
        target_pset->cpu_state_map[PROCESSOR_RUNNING] &
        target_pset->cpu_running_foreign;
    for (int cpu_foreign = bit_first(running_foreign_bitmap); cpu_foreign >= 0;
         cpu_foreign = bit_next(running_foreign_bitmap, cpu_foreign)) {
      if (!sched_edge_cpu_running_foreign_shared_rsrc_available(
              target_pset, cpu_foreign, pset)) {
        continue;
      }
      return true;
    }
 ...
 */
static bool sched_edge_steal_possible(processor_set_t idle_pset,
                                      processor_set_t candidate_pset,
                                      sched_bucket_t *bucket_for_steal) {
  sched_clutch_root_t candidate_clutch_root = &candidate_pset->pset_clutch_root;

  int highest_runnable_bucket = sched_clutch_root_highest_runnable_qos(
      candidate_clutch_root, SCHED_CLUTCH_HIGHEST_ROOT_BUCKET_UNBOUND_ONLY);
  if (highest_runnable_bucket == -1) {
    /* Candidate cluster runq is empty of unbound threads */
    return false;
  }

  for (int unbound_qos = highest_runnable_bucket; unbound_qos >= 0;
       unbound_qos =
           bitmap_lsb_next(candidate_clutch_root->scr_unbound_runnable_bitmap,
                           TH_BUCKET_SCHED_MAX, unbound_qos)) {
    /* Confirm we are allowed to steal across the edge at this QoS */
    sched_clutch_edge edge =
        sched_edge_config_get(candidate_pset->pset_cluster_id,
                              idle_pset->pset_cluster_id, unbound_qos);
    if (edge.sce_steal_allowed == false) {
      continue;
    }
    if (edge.sce_migration_weight == 0) {
      /* Allow free stealing across a zero edge weight, even with idle cores in
       * the candidate pset */
      *bucket_for_steal = (sched_bucket_t)unbound_qos;
      return true;
    }
    uint32_t candidate_runq_depth = os_atomic_load(
        &candidate_pset->pset_runnable_depth[unbound_qos], relaxed);
    if (candidate_runq_depth > pset_available_cpu_count(candidate_pset)) {
      /* Candidate cluster has excess load at this QoS (and at least one unbound
       * thread we can steal!) */
      *bucket_for_steal = (sched_bucket_t)unbound_qos;
      return true;
    }
...
static thread_t sched_edge_steal_thread(processor_set_t pset,
                                        uint64_t candidate_pset_bitmap) {
  thread_t stolen_thread = THREAD_NULL;

  /*
   * Edge Scheduler Optimization
   *
   * The logic today bails as soon as it finds a cluster where the cluster load
   * is greater than the edge weight. Maybe it should have a more advanced
   * version which looks for the maximum delta etc.
   */
  sched_pset_iterate_state_t istate = SCHED_PSET_ITERATE_STATE_INIT;
  while (sched_iterate_psets_ordered(pset, &pset->spill_search_order[0],
                                     candidate_pset_bitmap, &istate)) {
    processor_set_t steal_from_pset = pset_array[istate.spis_pset_id];
    if (steal_from_pset == NULL) {
      continue;
    }
    bool steal_allowed = false;
    for (sched_bucket_t bucket = TH_BUCKET_FIXPRI; bucket < TH_BUCKET_SCHED_MAX;
         bucket++) {
      sched_clutch_edge edge = sched_edge_config_get(
          istate.spis_pset_id, pset->pset_cluster_id, bucket);
      if (edge.sce_steal_allowed) {
        steal_allowed = true;
        break;
      }
    ...
    if (sched_edge_steal_possible(pset, steal_from_pset, &bucket_for_steal)) {
      uint64_t current_timestamp = mach_absolute_time();
      sched_clutch_root_t clutch_root_for_steal =
          &steal_from_pset->pset_clutch_root;
      stolen_thread = sched_clutch_thread_unbound_lookup(
          clutch_root_for_steal,
          &clutch_root_for_steal->scr_unbound_buckets[bucket_for_steal], NULL,
          NULL);
      sched_clutch_thread_remove(clutch_root_for_steal, stolen_thread,
                                 current_timestamp,
                                 SCHED_CLUTCH_BUCKET_OPTIONS_SAMEPRI_RR);

      sched_clutch_dbg_thread_select_packed_t debug_info = {0};
      debug_info.trace_data.version =
          SCHED_CLUTCH_DBG_THREAD_SELECT_PACKED_VERSION;
      debug_info.trace_data.traverse_mode =
          SCHED_CLUTCH_TRAVERSE_REMOVE_HIERARCHY_ONLY;
      ...
      KERNEL_DEBUG_CONSTANT_IST(
          ...
          MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_CLUTCH_THREAD_SELECT) |
              DBG_FUNC_NONE,
          thread_tid(stolen_thread),
          thread_group_get_id(stolen_thread->thread_group), bucket_for_steal,
          debug_info.scdts_trace_data_packed, 0);
      KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_STEAL) |
               DBG_FUNC_NONE,
           thread_tid(stolen_thread), pset->pset_cluster_id,
           steal_from_pset->pset_cluster_id, 0);
...
static thread_t sched_edge_processor_idle(processor_set_t pset) {
  ...
  /* Find foreign running threads to rebalance; the actual rebalance is done in
   * sched_edge_balance() */
  boolean_t rebalance_needed =
      sched_edge_foreign_running_thread_available(pset);
  if (rebalance_needed) {
    return THREAD_NULL;
  }

  /* No foreign threads found; find a thread to steal from all clusters based on
   * weights/loads etc. */
  thread = sched_edge_steal_thread(pset, pset->native_psets[0] |
                                             pset->foreign_psets[0]);
  return thread;
}

/* Return true if this shared resource thread has a better cluster to run on */
static bool
sched_edge_shared_rsrc_migrate_possible(thread_t thread,
                                        processor_set_t preferred_pset,
                                        processor_set_t current_pset) {
  cluster_shared_rsrc_type_t shared_rsrc_type =
      sched_edge_thread_shared_rsrc_type(thread);
  uint64_t current_pset_load =
      sched_pset_cluster_shared_rsrc_load(current_pset, shared_rsrc_type);
  /*
   * Adjust the current pset load to discount the current thread only if the
   * current pset is a preferred pset type. This allows the scheduler to
   * rebalance threads from non-preferred cluster to an idle cluster of the
   * preferred type.
   *
   * Edge Scheduler Optimization
   * For multi-cluster machines, it might be useful to enhance this mechanism to
   * migrate between clusters of the preferred type.
   */
  uint64_t current_pset_adjusted_load =
      (current_pset->pset_type != preferred_pset->pset_type)
          ? current_pset_load
          : (current_pset_load - 1);

  uint64_t eligible_pset_bitmask = 0;
  if (edge_shared_rsrc_policy[shared_rsrc_type] ==
      EDGE_SHARED_RSRC_SCHED_POLICY_NATIVE_FIRST) {
    /*
     * For the EDGE_SHARED_RSRC_SCHED_POLICY_NATIVE_FIRST policy, the load
     * balancing occurs only among clusters native with the preferred cluster.
     */
    eligible_pset_bitmask = preferred_pset->native_psets[0];
    bit_set(eligible_pset_bitmask, preferred_pset->pset_cluster_id);
  } else {
    /* For EDGE_SHARED_RSRC_SCHED_POLICY_RR, the load balancing happens among
     * all clusters */
    eligible_pset_bitmask = sched_edge_available_pset_bitmask[0];
  }

  /* For each eligible cluster check if there is an under-utilized cluster;
   * return true if there is */
  for (int cluster_id = bit_first(eligible_pset_bitmask); cluster_id >= 0;
       cluster_id = bit_next(eligible_pset_bitmask, cluster_id)) {
    if (cluster_id == current_pset->pset_cluster_id) {
      continue;
    }
    uint64_t cluster_load = sched_pset_cluster_shared_rsrc_load(
        pset_array[cluster_id], shared_rsrc_type);
    if (current_pset_adjusted_load > cluster_load) {
      KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH,
                        MACH_SCHED_EDGE_SHARED_RSRC_MIGRATE) |
               DBG_FUNC_NONE,
           current_pset_load, current_pset->pset_cluster_id, cluster_load,
           cluster_id);
      return true;
    }
  }
...
#define SESTP_BIT_POS(cpu_id)                                                  \
  ((sched_edge_stp_registry_t)(cpu_id * SESTP_BITS_PER_CORE))
#define SESTP_MASK(cpu_id)                                                     \
  ((sched_edge_stp_registry_t)mask(SESTP_BITS_PER_CORE)                        \
   << SESTP_BIT_POS(cpu_id))
static_assert((SESTP_BITS_PER_CORE * MAX_CPUS) <=
                  (sizeof(sched_edge_stp_registry_t) * 8),
              "Global registry must fit per-core bits for each core");

#define SESTP_EXTRACT_STATE(registry, cpu_id)                                  \
  ((registry >> SESTP_BIT_POS(cpu_id)) & mask(SESTP_BITS_PER_CORE))
#define SESTP_SET_STATE(registry, cpu_id, state)                               \
  ((registry & ~SESTP_MASK(cpu_id)) |                                          \
   ((sched_edge_stp_registry_t)state << SESTP_BIT_POS(cpu_id)))
__enum_decl(sched_edge_stp_state_t, uint8_t,
            {SCHED_EDGE_STP_NOT_WANT = 0, SCHED_EDGE_STP_REQUESTED = 1,
 ...
 */
static inline int sched_edge_stir_the_pot_try_trigger_swap(thread_t thread) {
  ...
  if (sched_edge_stir_the_pot_core_type_is_desired(
    ...
    offset = os_atomic_inc_orig(&sched_edge_stp_selection_non_p_core_offset,
                                relaxed);
  } else {
    swap_candidates_map = sched_edge_p_core_map;
    offset =
        os_atomic_inc_orig(&sched_edge_stp_selection_p_core_offset, relaxed);
  }
  ...
  os_atomic_rmw_loop(
      &sched_edge_stir_the_pot_global_registry, old_registry, new_registry,
      relaxed, {
        swap_cpu = -1;
        ...
        /* Scan candidates */
        for (int rotid = lsb_first(swap_candidates_map); rotid != -1;
             rotid = lsb_next(swap_candidates_map, rotid)) {
          int candidate_cpu =
              (rotid + search_start_ind) % 64; // un-rotate the bit
          sched_edge_stp_state_t candidate_state =
              SESTP_EXTRACT_STATE(old_registry, candidate_cpu);
          if (candidate_state == SCHED_EDGE_STP_REQUESTED) {
            sched_bucket_t candidate_qos = os_atomic_load(
                &processor_array[candidate_cpu]
                     ->processor_set->cpu_running_buckets[candidate_cpu],
                relaxed);
        ...
        if (swap_cpu == -1) {
          ...
          intermediate_registry =
              SESTP_SET_STATE(old_registry, self_cpu, SCHED_EDGE_STP_REQUESTED);
        } else {
          /*
           ...
           */
          intermediate_registry =
              SESTP_SET_STATE(old_registry, self_cpu, SCHED_EDGE_STP_PENDING);
          intermediate_registry = SESTP_SET_STATE(
              intermediate_registry, swap_cpu, SCHED_EDGE_STP_PENDING);
        }
    ...
    if (swap_cpu == -1) {
      if (self_state != SCHED_EDGE_STP_REQUESTED) {
        ...
        KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_STIR_THE_POT) |
                 DBG_FUNC_START,
             0, self_cpu, cpu_of_type_offset_ind, 0);
      }
    } else {
      if (self_state == SCHED_EDGE_STP_REQUESTED) {
        /* Now pending */
        KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_STIR_THE_POT) |
                 DBG_FUNC_END,
             1, self_cpu, cpu_of_type_offset_ind, 0);
      }
      ...
      if (swap_state == SCHED_EDGE_STP_REQUESTED) {
        ...
        KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_STIR_THE_POT) |
                 DBG_FUNC_END,
             1, swap_cpu, cpu_of_type_offset_ind, 0);
      }
    }
  }
  if (swap_cpu != -1) {
    ...
    assert3u(sched_edge_stir_the_pot_core_type_is_desired(
                 swap_processor->processor_set),
             !=,
             sched_edge_stir_the_pot_core_type_is_desired(
                 self_processor->processor_set));
    if (sched_edge_stir_the_pot_core_type_is_desired(
            self_processor->processor_set)) {
      /*
       ...
       * thread back here.
       */
      os_atomic_store(&swap_processor->stir_the_pot_inbox_cpu, self_cpu,
                      relaxed);
      processor_set_t swap_pset = swap_processor->processor_set;
      pset_lock(swap_pset);
      sched_ipi_type_t ipi_type =
          sched_ipi_action(swap_processor, NULL, SCHED_IPI_EVENT_REBALANCE);
       ...
       */
      os_atomic_store(&self_processor->stir_the_pot_inbox_cpu, swap_cpu,
                      relaxed);
    }
  }
  KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_STIR_THE_POT) |
           DBG_FUNC_NONE,
       (swap_cpu != -1) ? 1 : 0, swap_cpu, old_registry,
       cpu_of_type_offset_ind);
  return swap_cpu;
}
 ...
 */
void sched_edge_stir_the_pot_clear_registry_entry(void) {
  int self_cpu = current_processor()->cpu_id;
  sched_edge_stp_state_t self_state;
  sched_edge_stp_registry_t old_registry, new_registry;
  os_atomic_rmw_loop(&sched_edge_stir_the_pot_global_registry, old_registry,
                     new_registry, relaxed, {
                       self_state = SESTP_EXTRACT_STATE(old_registry, self_cpu);
                       if (self_state == SCHED_EDGE_STP_NOT_WANT) {
                         /* State already cleared, nothing to be done */
                         os_atomic_rmw_loop_give_up(break);
                       }
                       new_registry = SESTP_SET_STATE(old_registry, self_cpu,
                                                      SCHED_EDGE_STP_NOT_WANT);
                     });
  if (self_state == SCHED_EDGE_STP_REQUESTED) {
    /* Request was cleared */
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_STIR_THE_POT) |
 ...
 * Preemption must be disabled.
 */
static inline void sched_edge_stir_the_pot_set_registry_entry(void) {
  int self_cpu = current_processor()->cpu_id;
  sched_edge_stp_state_t self_state;
  sched_edge_stp_registry_t old_registry, new_registry;
  bool newly_requested = os_atomic_rmw_loop(
      &sched_edge_stir_the_pot_global_registry, old_registry, new_registry,
      relaxed, {
        self_state = SESTP_EXTRACT_STATE(old_registry, self_cpu);
        if (self_state == SCHED_EDGE_STP_REQUESTED) {
 ...
 */
static inline bool sched_edge_stir_the_pot_thread_eligible(thread_t thread) {
  processor_set_t preferred_pset;
  if ((thread == THREAD_NULL) ||
      ((preferred_pset =
            pset_array[sched_edge_thread_preferred_cluster(thread)]) ==
       PROCESSOR_SET_NULL)) {
 ...
 */
static inline int
sched_edge_stir_the_pot_check_inbox_for_thread(thread_t thread,
  ...
  if (sched_edge_stir_the_pot_thread_eligible(thread)) {
    ...
    /* Ensure registry state is cleared for ineligible thread, if it hasn't been
     * already */
    sched_edge_stir_the_pot_clear_registry_entry();
    /*
  ...
  if (consume_message) {
    ...
    /*
     * We may have delayed requesting stir-the-pot swap for the the current
     * thread due to a pending inbox message for the previous thread. Now that
     * that such a message has been received, finishing updating the registry
     * state.
     */
    if (sched_edge_stir_the_pot_thread_eligible(
            self_processor->active_thread)) {
      sched_edge_stir_the_pot_set_registry_entry();
    }
  }
  return dst_cpu;
 ...
 * Preemption must be disabled.
 */
void sched_edge_stir_the_pot_update_registry_state(thread_t thread) {
  processor_t self_processor = current_processor();
  /*
  ...
  if (sched_edge_stir_the_pot_thread_eligible(thread)) {
    int inbox_message =
        os_atomic_load(&self_processor->stir_the_pot_inbox_cpu, relaxed);
    if (inbox_message == -1) {
      /* Set the registry bit */
      sched_edge_stir_the_pot_set_registry_entry();
    } else {
      assert(sched_edge_stir_the_pot_core_type_is_desired(
                 self_processor->processor_set) == false);
      /*
...
static bool sched_edge_thread_avoid_processor(processor_t processor,
  ...
  /*
   * On quantum expiry, check the migration bitmask if this thread should be
   * migrated off this core. A migration is only recommended if there's also an
   * idle core available that needn't be avoided.
   */
  if (reason & AST_QUANTUM) {
    if (bit_test(processor->processor_set->perfcontrol_cpu_migration_bitmask,
                 processor->cpu_id)) {
      uint64_t non_avoided_idle_primary_map =
          processor->processor_set->cpu_state_map[PROCESSOR_IDLE] &
          processor->processor_set->recommended_bitmask &
          ~processor->processor_set->perfcontrol_cpu_migration_bitmask;
  ...
  sched_clutch_edge edge =
      (thread->sched_pri >= BASEPRI_RTQUEUES)
          ? sched_rt_config_get(preferred_pset->pset_cluster_id,
                                processor->processor_set->pset_cluster_id)
          : sched_edge_config_get(preferred_pset->pset_cluster_id,
                                  processor->processor_set->pset_cluster_id,
                                  thread->th_sched_bucket);
  if (SCHED_CLUTCH_THREAD_CLUSTER_BOUND(thread) == false &&
      preferred_pset->pset_id != processor->processor_set->pset_id &&
      edge.sce_migration_allowed == false && edge.sce_steal_allowed == false) {
    /*
     * Thread isn't allowed to be here, according to the edge migration graph.
     * Perhaps the thread's priority or boundness or its thread group's
     * preferred pset or the edge migration graph changed.
     *
    ...
    for (uint32_t pset_id = 0; pset_id < sched_edge_max_clusters; pset_id++) {
      ...
      edge = (thread->sched_pri >= BASEPRI_RTQUEUES)
                 ...
                 : sched_edge_config_get(preferred_pset->pset_id, pset_id,
                                         thread->th_sched_bucket);
      if (pset_is_recommended(pset_array[pset_id]) &&
          ((pset_id == preferred_pset->pset_id) ||
  ...
  if (thread_shared_rsrc_policy_get(thread, CLUSTER_SHARED_RSRC_TYPE_RR)) {
    return sched_edge_shared_rsrc_migrate_possible(thread, preferred_pset,
                                                   processor->processor_set);
  }
  if (thread_shared_rsrc_policy_get(thread,
                                    CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST)) {
   ...
   * quantum-expired thread to for stir-the-pot. This is in order for threads in
   * a multi-threaded workload to share time on the P-cores so they make roughly
  ...
  /*
   * Compaction:
   * If the preferred pset for the thread is now idle, try and migrate the
   * thread to that cluster.
   */
  if ((processor->processor_set != preferred_pset) &&
      (sched_edge_cluster_load_metric(preferred_pset,
                                      thread->th_sched_bucket) == 0)) {
    return true;
  }

  /*
   * Running Rebalance:
   * We are willing to preempt the thread in order to migrate it onto an idle
   * core of the preferred type.
   */
  if ((processor->processor_set->pset_type != preferred_pset->pset_type) &&
      pset_type_is_recommended(preferred_pset)) {
    /* Scan for idle pset */
    for (uint32_t pset_id = 0; pset_id < sched_edge_max_clusters; pset_id++) {
      ...
      edge = sched_edge_config_get(preferred_pset->pset_id, pset_id,
                                   thread->th_sched_bucket);
...
static bool sched_edge_balance(__unused processor_t cprocessor,
  ...
  for (int cluster = bitmap_first(foreign_pset_bitmap, sched_edge_max_clusters);
    ...
    pset_lock(target_pset);
    uint64_t cpu_running_foreign_map =
        (target_pset->cpu_running_foreign &
         target_pset->cpu_state_map[PROCESSOR_RUNNING]);
    for (int cpuid = lsb_first(cpu_running_foreign_map); cpuid >= 0;
         cpuid = lsb_next(cpu_running_foreign_map, cpuid)) {
      if (!sched_edge_cpu_running_foreign_shared_rsrc_available(target_pset,
                                                                cpuid, cpset)) {
        continue;
      }
      processor_t target_cpu = processor_array[cpuid];
      ipi_type[target_cpu->cpu_id] =
          sched_ipi_action(target_cpu, NULL, SCHED_IPI_EVENT_REBALANCE);
  ...
  for (int cpuid = lsb_first(ast_processor_map); cpuid >= 0;
       cpuid = lsb_next(ast_processor_map, cpuid)) {
    processor_t ast_processor = processor_array[cpuid];
    sched_ipi_perform(ast_processor, ipi_type[cpuid]);
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_REBAL_RUNNING) |
             DBG_FUNC_NONE,
         0, cprocessor->cpu_id, cpuid, 0);
  }
 ...
 *
 * Routine to evaluate an edge between two clusters to decide if migration is
 ...
 * found. The routine looks at the regular edges and cluster loads or the shared
 * resource loads based on the type of thread.
 */
static bool sched_edge_migration_check(uint32_t cluster_id,
                                       processor_set_t preferred_pset,
                                       uint32_t preferred_cluster_load,
                                       thread_t thread,
                                       processor_set_t *selected_pset,
                                       uint32_t *max_edge_delta) {
  uint32_t preferred_cluster_id = preferred_pset->pset_cluster_id;
  cluster_type_t preferred_cluster_type =
  ...
  sched_clutch_edge edge = sched_edge_config_get(
      preferred_cluster_id, cluster_id, thread->th_sched_bucket);
  if (edge.sce_migration_allowed == false) {
    return false;
  }
  uint32_t dst_load =
      shared_rsrc_thread
          ? (uint32_t)sched_pset_cluster_shared_rsrc_load(dst_pset,
                                                          shared_rsrc_type)
          : sched_edge_cluster_load_metric(dst_pset, thread->th_sched_bucket);
  if (dst_load == 0) {
  ...
  if (!shared_rsrc_thread && (edge_delta < edge.sce_migration_weight)) {
    /*
     * For non shared resource threads, use the edge migration weight to decide
     * if this cluster is over-committed at the QoS level of this thread.
     */
    return false;
  }
  ...
  if (edge_delta == *max_edge_delta) {
    /* If the edge delta is the same as the max delta, make sure a homogeneous
     * cluster is picked */
    boolean_t selected_homogeneous =
        ((*selected_pset)->pset_type == preferred_cluster_type);
    boolean_t candidate_homogeneous =
        (dst_pset->pset_type == preferred_cluster_type);
    if (selected_homogeneous || !candidate_homogeneous) {
      return false;
    }
  }
  /* dst_pset seems to be the best candidate for migration; however other
   * candidates should still be evaluated */
 ...
 *
 * Returns the most ideal cluster for execution of this thread based on outgoing
 * edges of the preferred pset. Can return preferred_pset if its the most ideal
 * destination for this thread.
 */
static processor_set_t
sched_edge_migrate_edges_evaluate(processor_set_t preferred_pset,
                                  uint32_t preferred_cluster_load,
                                  thread_t thread) {
  ...
  sched_pset_iterate_state_t istate = SCHED_PSET_ITERATE_STATE_INIT;
  while (sched_iterate_psets_ordered(
      preferred_pset,
      &preferred_pset->spill_search_order[thread->th_sched_bucket],
      native_pset_bitmap[0], &istate)) {
    search_complete = sched_edge_migration_check(
        istate.spis_pset_id, preferred_pset, preferred_cluster_load, thread,
        &selected_pset, &max_edge_delta);
    if (search_complete) {
      break;
    }
  }
  ...
  if (shared_rsrc_thread && (edge_shared_rsrc_policy[shared_rsrc_type] ==
                             EDGE_SHARED_RSRC_SCHED_POLICY_NATIVE_FIRST)) {
    /*
     * If the shared resource scheduling policy is
     * EDGE_SHARED_RSRC_SCHED_POLICY_NATIVE_FIRST, the scheduler tries to fill
     * up the preferred cluster and its homogeneous peers first.
     */

    if (max_edge_delta > 0) {
      /*
       * This represents that there is a peer cluster of the same type as the
       * preferred cluster (since the code above only looks at the native_psets)
       * which has lesser threads as compared to the preferred cluster of the
       * shared resource type. This indicates that there is capacity on a native
       * cluster where this thread should be placed.
       */
      return selected_pset;
    }
    /*
     * Indicates that all peer native clusters are at the same shared resource
     * usage; check if the preferred cluster has any more capacity left.
     */
    if (sched_pset_cluster_shared_rsrc_load(preferred_pset, shared_rsrc_type) <
        pset_available_cpu_count(preferred_pset)) {
      return preferred_pset;
    }
    /*
     * Looks like the preferred cluster and all its native peers are full with
     * shared resource threads; need to start looking at non-native clusters for
     * capacity.
     */
  }
 ...
 *
 * Routine to find an alternative cluster from candidate_cluster_bitmap since
 * the selected_pset is not available for execution. The logic tries to prefer
 * homogeneous clusters over heterogeneous clusters since this is typically used
 * in thread placement decisions.
 */
_Static_assert(MAX_PSETS <= 64,
               "Unable to fit maximum number of psets in uint64_t bitmask");
static processor_set_t
sched_edge_candidate_alternative(processor_set_t selected_pset,
                                 uint64_t candidate_cluster_bitmap) {
  /*
   * It looks like the most ideal pset is not available for scheduling
   * currently. Try to find a homogeneous cluster that is still available.
   */
  uint64_t available_native_clusters =
      selected_pset->native_psets[0] & candidate_cluster_bitmap;
  int available_cluster_id = lsb_first(available_native_clusters);
  if (available_cluster_id == -1) {
    /* Looks like none of the homogeneous clusters are available; pick the first
     * available cluster */
 ...
 *
 * Helper routine for sched_edge_migrate_candidate() which switches pset locks
 * (if needed) based on switch_pset_locks. Returns the newly locked pset after
 * the switch.
 */
static processor_set_t
sched_edge_switch_pset_lock(processor_set_t selected_pset,
 ...
 *
 * Routine to find an appropriate cluster for scheduling a thread. The routine
 * looks at the properties of the thread and the preferred cluster to determine
 * the best available pset for scheduling.
 *
 * The switch_pset_locks parameter defines whether the routine should switch
 * pset locks to provide an accurate scheduling decision. This mode is typically
 * used when choosing a pset for scheduling a thread since the decision has to
 * be synchronized with another CPU changing the recommendation of clusters
 * available on the system. If this parameter is set to false, this routine
 * returns the best effort indication of the cluster the thread should be
 * scheduled on. It is typically used in fast path contexts (such as
 * SCHED(thread_avoid_processor) to determine if there is a possibility of
 * scheduling this thread on a more appropriate cluster.
 *
 * Routine returns the most ideal cluster for scheduling. If switch_pset_locks
 * is set, it ensures that the resultant pset lock is held.
 */
static processor_set_t sched_edge_migrate_candidate(
    processor_set_t _Nullable preferred_pset, thread_t thread,
    processor_set_t locked_pset, bool switch_pset_locks,
    processor_t *processor_hint_out, sched_options_t *options_inout) {
  processor_set_t selected_pset = preferred_pset;
  cluster_shared_rsrc_type_t shared_rsrc_type =
  ...
  if (SCHED_CLUTCH_THREAD_CLUSTER_BOUND(thread)) {
    /*
     * For cluster-bound threads, choose the cluster to which the thread is
     * bound, unless that cluster is unavailable. If it's not available, fall
     * through to the regular cluster selection logic which handles
     * derecommended clusters appropriately.
     */
    selected_pset = pset_array[sched_edge_thread_bound_cluster_id(thread)];
    if (selected_pset != NULL) {
      locked_pset = sched_edge_switch_pset_lock(selected_pset, locked_pset,
                                                switch_pset_locks);
  ...
  if (enable_task_set_cluster_type &&
      (task->t_flags & TF_USE_PSET_HINT_CLUSTER_TYPE)) {
    processor_set_t pset_hint = task->pset_hint;
    if (pset_hint &&
        (selected_pset == NULL ||
         selected_pset->pset_cluster_type != pset_hint->pset_cluster_type)) {
  ...
  uint32_t preferred_cluster_load =
      shared_rsrc_thread
          ? (uint32_t)sched_pset_cluster_shared_rsrc_load(preferred_pset,
                                                          shared_rsrc_type)
          : sched_edge_cluster_load_metric(preferred_pset,
                                           thread->th_sched_bucket);
  ...
  /*
   * If this thread has expired quantum on a non-preferred core and is waiting
   * on "stir-the-pot" to get a turn running on a P-core, check our processor
   * inbox for stir-the-pot to see if an eligible P-core has already been found
   * for swap. If so, try to migrate to the corresponding pset and also carry
   * over the processor hint to preempt that specific P-core.
   *
   * The AMP rebalancing mechanism is available for regular threads or shared
   * resource threads with the EDGE_SHARED_RSRC_SCHED_POLICY_NATIVE_FIRST
   * policy.
   */
  int stir_the_pot_swap_cpu =
      sched_edge_stir_the_pot_check_inbox_for_thread(thread, true);
  ...
  /* Look at edge weights to decide the most ideal migration candidate for this
   * thread */
  selected_pset = sched_edge_migrate_edges_evaluate(
      preferred_pset, preferred_cluster_load, thread);
...
migrate_candidate_available_check:
  ...
  if (pset_is_recommended(selected_pset) == true) {
    ...
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_CLUSTER_OVERLOAD) |
             DBG_FUNC_NONE,
         thread_tid(thread), preferred_pset->pset_cluster_id,
         selected_pset->pset_cluster_id, preferred_cluster_load);
    return selected_pset;
  }
  stirring_the_pot = false;
  /* Looks like selected_pset is not available for scheduling; remove it from
   * candidate_cluster_bitmap */
  bitmap_clear(&candidate_cluster_bitmap, selected_pset->pset_cluster_id);
  if (__improbable(bitmap_first(&candidate_cluster_bitmap,
                                sched_edge_max_clusters) == -1)) {
    pset_unlock(locked_pset);
    return NULL;
  }
  /* Try and find an alternative for the selected pset */
  selected_pset =
      sched_edge_candidate_alternative(selected_pset, candidate_cluster_bitmap);
  goto migrate_candidate_available_check;
}
...
static processor_t sched_edge_choose_processor(processor_set_t pset,
   ...
   * It should take the passed in "pset" as a hint which represents the recency
   * metric for pset selection logic.
   */
  processor_set_t preferred_pset =
      pset_array[sched_edge_thread_preferred_cluster(thread)];
  processor_set_t chosen_pset = preferred_pset;
  /*
   * If the preferred pset is overloaded, find a pset which is the best
   * candidate to migrate threads to. sched_edge_migrate_candidate() returns the
   * preferred pset if it has capacity; otherwise finds the best candidate pset
   * to migrate this thread to.
   *
   * Edge Scheduler Optimization
   * It might be useful to build a recency metric for the thread for multiple
   * clusters and factor that into the migration decisions.
   */
  chosen_pset = sched_edge_migrate_candidate(preferred_pset, thread, pset, true,
                                             &processor, options_inout);
  if (chosen_pset) {
    chosen_processor =
        choose_processor(chosen_pset, processor, thread, options_inout);
  }
 ...
 *
 * Drains all the runnable threads which are not restricted to the root_clutch
 * (due to clutch bucket overrides etc.) into a local thread queue.
 */
static void
sched_edge_clutch_bucket_threads_drain(sched_clutch_bucket_t clutch_bucket,
                                       sched_clutch_root_t root_clutch,
                                       queue_t clutch_threads) {
  thread_t thread = THREAD_NULL;
  uint64_t current_timestamp = mach_approximate_time();
  qe_foreach_element_safe(thread, &clutch_bucket->scb_thread_timeshare_queue,
 ...
 */
static void sched_edge_run_drained_threads(queue_t clutch_threads) {
  thread_t thread;
 ...
 *
 * Routine to update the preferred cluster for QoS buckets within a thread
 * group. The buckets to be updated are specifed as a bitmap
 * (clutch_bucket_modify_bitmap).
 */
static void
sched_edge_update_preferred_cluster(sched_clutch_t sched_clutch,
                                    bitmap_t *clutch_bucket_modify_bitmap,
                                    uint32_t *tg_bucket_preferred_cluster) {
  for (int bucket =
           bitmap_first(clutch_bucket_modify_bitmap, TH_BUCKET_SCHED_MAX);
       bucket >= 0; bucket = bitmap_next(clutch_bucket_modify_bitmap, bucket)) {
    os_atomic_store(
        &sched_clutch->sc_clutch_groups[bucket].scbg_preferred_cluster,
        tg_bucket_preferred_cluster[bucket], relaxed);
  }
}
 ...
 *
 * Routine to implement the migration of threads on a cluster when the thread
 * group recommendation is updated. The migration works using a 2-phase
 * algorithm.
 *
 * Phase 1: With the pset lock held, check the recommendation of the clutch
 * buckets. For each clutch bucket, if it needs to be migrated immediately,
 * drain the threads into a local thread queue. Otherwise mark the clutch bucket
 * as native/foreign as appropriate.
 *
 * Phase 2: After unlocking the pset, drain all the threads from the local
 * thread queue and mark them runnable which should land them in the right
 * hierarchy.
 *
 ...
 */
static void sched_edge_migrate_thread_group_runnable_threads(
    sched_clutch_t sched_clutch, sched_clutch_root_t root_clutch,
    bitmap_t *clutch_bucket_modify_bitmap,
    __unused uint32_t *tg_bucket_preferred_cluster, bool migrate_immediately) {
  /* Queue to hold threads that have been drained from clutch buckets to be
   * migrated */
  ...
  for (int bucket =
    ...
    sched_clutch_bucket_group_t clutch_bucket_group =
        &(sched_clutch->sc_clutch_groups[bucket]);
    sched_clutch_bucket_t clutch_bucket = &(
        clutch_bucket_group->scbg_clutch_buckets[root_clutch->scr_cluster_id]);
    sched_clutch_root_t scb_root =
        os_atomic_load(&clutch_bucket->scb_root, relaxed);
    if (scb_root == NULL) {
      /* Clutch bucket not runnable or already in the right hierarchy; nothing
       * to do here */
      assert(clutch_bucket->scb_thr_count == 0);
      continue;
    }
    assert(scb_root == root_clutch);
    uint32_t clutch_bucket_preferred_cluster =
        sched_clutch_bucket_preferred_cluster(clutch_bucket);

    if (migrate_immediately) {
      /*
       * For transitions where threads need to be migrated immediately, drain
       * the threads into a local queue unless we are looking at the clutch
       * buckets for the newly recommended cluster.
       */
      if (root_clutch->scr_cluster_id != clutch_bucket_preferred_cluster) {
        sched_edge_clutch_bucket_threads_drain(clutch_bucket, scb_root,
                                               &clutch_threads);
      } else {
        sched_clutch_bucket_mark_native(clutch_bucket, root_clutch);
      }
       ...
       */
      boolean_t homogeneous_cluster =
          (pset_type_for_id(root_clutch->scr_cluster_id) ==
           pset_type_for_id(clutch_bucket_preferred_cluster));
      /*
       * If threads do not have to be migrated immediately, just change the
       * native/foreign flag on the clutch bucket.
       */
 ...
 */
static void sched_edge_migrate_thread_group_running_threads(
    sched_clutch_t sched_clutch, sched_clutch_root_t root_clutch,
    __unused bitmap_t *clutch_bucket_modify_bitmap,
    uint32_t *tg_bucket_preferred_cluster, bool migrate_immediately) {
  if (migrate_immediately == false) {
    /* If CLPC has recommended not to move threads immediately, nothing to do
     * here */
    return;
  }
   ...
   * Edge Scheduler Optimization
   *
   * When the system has a large number of clusters and cores, it might be
   * useful to narrow down the iteration by using a thread running bitmap per
   * clutch.
   */
  uint64_t ast_processor_map = 0;
  sched_ipi_type_t ipi_type[MAX_CPUS] = {SCHED_IPI_NONE};

  uint64_t running_map =
      root_clutch->scr_pset->cpu_state_map[PROCESSOR_RUNNING];
  /*
   * Iterate all CPUs and look for the ones running threads from this thread
   * group and are not restricted to the specific cluster (due to overrides
   * etc.)
   */
  for (int cpuid = lsb_first(running_map); cpuid >= 0;
       cpuid = lsb_next(running_map, cpuid)) {
    processor_t src_processor = processor_array[cpuid];
    boolean_t expected_tg =
        (src_processor->current_thread_group == sched_clutch->sc_tg);
    sched_bucket_t processor_sched_bucket =
        src_processor->processor_set->cpu_running_buckets[cpuid];
    if (processor_sched_bucket == TH_BUCKET_SCHED_MAX) {
      continue;
    }
    boolean_t non_preferred_cluster =
        tg_bucket_preferred_cluster[processor_sched_bucket] !=
        root_clutch->scr_cluster_id;

    if (expected_tg && non_preferred_cluster) {
      ipi_type[cpuid] =
          sched_ipi_action(src_processor, NULL, SCHED_IPI_EVENT_REBALANCE);
      if (ipi_type[cpuid] != SCHED_IPI_NONE) {
        bit_set(ast_processor_map, cpuid);
      } else if (src_processor == current_processor()) {
        bit_set(root_clutch->scr_pset->pending_AST_PREEMPT_cpu_mask, cpuid);
        ast_t new_preempt =
            update_pending_nonurgent_preemption(src_processor, AST_PREEMPT);
        ast_on(new_preempt);
      }
    }
  }
  ...
  if (bit_first(ast_processor_map) != -1) {
    ...
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_AMP_RECOMMENDATION_CHANGE) |
             DBG_FUNC_NONE,
         thread_group_get_id(sched_clutch->sc_tg), ast_processor_map, 0, 0);
  }
}
 ...
 * Scheduler, the preferred cluster is specified on a per-QoS basis within a
 * thread group. The routine updates the preferences and performs thread
 * migrations based on the policy specified by CLPC. tg_bucket_preferred_cluster
 * is an array of size TH_BUCKET_SCHED_MAX which specifies the new preferred
 * cluster for each QoS within the thread group.
 */
void sched_edge_tg_preferred_cluster_change(
  ...
  /*
   * In order to optimize the processing, create a bitmap which represents all
   * QoS buckets for which the preferred cluster has changed.
   */
  bitmap_t clutch_bucket_modify_bitmap[BITMAP_LEN(TH_BUCKET_SCHED_MAX)] = {0};
  for (sched_bucket_t bucket = TH_BUCKET_FIXPRI; bucket < TH_BUCKET_SCHED_MAX;
       bucket++) {
    uint32_t old_preferred_cluster =
        sched_edge_clutch_bucket_group_preferred_cluster(
            &clutch->sc_clutch_groups[bucket]);
    ...
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_PREFERRED_PSET) | DBG_FUNC_NONE,
        thread_group_get_id(tg), bucket, new_preferred_cluster, options);
  }
  if (bitmap_lsb_first(clutch_bucket_modify_bitmap, TH_BUCKET_SCHED_MAX) ==
      -1) {
  ...
  /*
   * The first operation is to update the preferred cluster for all QoS buckets
   * within the thread group so that any future threads becoming runnable would
   * see the new preferred cluster value.
   */
  sched_edge_update_preferred_cluster(clutch, clutch_bucket_modify_bitmap,
                                      tg_bucket_preferred_cluster);

  for (uint32_t cluster_id = 0; cluster_id < sched_edge_max_clusters;
       cluster_id++) {
    processor_set_t pset = pset_array[cluster_id];
    spl_t s = splsched();
    pset_lock(pset);
    /*
     * Currently iterates all clusters looking for running threads for a TG to
     * be migrated. Can be optimized by keeping a per-clutch bitmap of clusters
     * running threads for a particular TG.
     *
     * Edge Scheduler Optimization
     */
    /* Migrate all running threads of the TG on this cluster based on options
     * specified by CLPC */
    sched_edge_migrate_thread_group_running_threads(
        clutch, &pset->pset_clutch_root, clutch_bucket_modify_bitmap,
        tg_bucket_preferred_cluster,
        (options & SCHED_PERFCONTROL_PREFERRED_CLUSTER_MIGRATE_RUNNING));
    /* Migrate all runnable threads of the TG in this cluster's hierarchy based
     * on options specified by CLPC */
    sched_edge_migrate_thread_group_runnable_threads(
        clutch, &pset->pset_clutch_root, clutch_bucket_modify_bitmap,
        tg_bucket_preferred_cluster,
        (options & SCHED_PERFCONTROL_PREFERRED_CLUSTER_MIGRATE_RUNNABLE));
    /* sched_edge_migrate_thread_group_runnable_threads() returns with pset
     * unlocked */
    splx(s);
  }
}
 ...
 * pset hierarchy now that a new pset has become runnable. Its possible that
 * this routine is called when the pset is already marked schedulable.
 *
 * Invoked with the pset lock held and interrupts disabled.
 */
static void sched_edge_pset_made_schedulable(__unused processor_t processor,
                                             processor_set_t dst_pset,
                                             boolean_t drop_lock) {
  if (bitmap_test(sched_edge_available_pset_bitmask,
                  dst_pset->pset_cluster_id)) {
 ...
 *
 * Callback routine from the platform layer once all CPUs/clusters have been
 * initialized. This provides an opportunity for the edge scheduler to
 * initialize all the edge parameters.
 */
static void sched_edge_cpu_init_completed(void) {
  /* Now that all cores have registered, compute bitmaps for different core
   * types */
  ...
  sched_clutch_edge free_spill = (sched_clutch_edge){.sce_migration_weight = 0,
                                                     .sce_migration_allowed = 1,
                                                     .sce_steal_allowed = 1};
  sched_clutch_edge no_spill = (sched_clutch_edge){.sce_migration_weight = 0,
                                                   .sce_migration_allowed = 0,
                                                   .sce_steal_allowed = 0};
  sched_clutch_edge weighted_spill =
      (sched_clutch_edge){.sce_migration_weight = 64,
                          .sce_migration_allowed = 1,
                          .sce_steal_allowed = 1};
  /* P -> P */
  ...
  spl_t s = splsched();
  for (int src_cluster_id = 0; src_cluster_id < sched_edge_max_clusters;
       src_cluster_id++) {
    ...
    for (int dst_cluster_id = 0; dst_cluster_id < sched_edge_max_clusters;
         dst_cluster_id++) {
      ...
      if (clusters_homogenous) {
        ...
        sched_rt_config_set(src_cluster_id, dst_cluster_id,
                            (sched_clutch_edge){
      ...
      bool clusters_local =
          (ml_get_die_id(src_cluster_id) == ml_get_die_id(dst_cluster_id));
      if (clusters_local) {
      ...
      for (sched_bucket_t bucket = 0; bucket < TH_BUCKET_SCHED_MAX; bucket++) {
        /* Set tunables for an edge based on the cluster types at either ends of
         * it */
        sched_clutch_edge edge_config =
            edge_config_defaults[src_pset->pset_type][dst_pset->pset_type];
        sched_edge_config_set(src_cluster_id, dst_cluster_id, bucket,
                              edge_config);
...
static bool sched_edge_thread_eligible_for_pset(thread_t thread,
  ...
  if (preferred_cluster_id == pset->pset_cluster_id) {
    ...
    if (thread->sched_pri >= BASEPRI_RTQUEUES) {
      ...
      edge = sched_edge_config_get(preferred_cluster_id, pset->pset_cluster_id,
                                   thread->th_sched_bucket);
    }
...
sched_ipi_type_t sched_edge_ipi_policy(processor_t dst, thread_t thread,
                                       boolean_t dst_idle,
                                       sched_ipi_event_t event) {
  ...
  case SCHED_IPI_EVENT_PREEMPT:
    ...
    if (thread && thread->sched_pri < BASEPRI_RTQUEUES) {
      if (sched_amp_pcores_preempt_immediate_ipi &&
          (pset_type_for_id(pset->pset_cluster_id) == CLUSTER_TYPE_P)) {
      ...
      if (sched_edge_migrate_ipi_immediate) {
        processor_set_t preferred_pset =
            pset_array[sched_edge_thread_preferred_cluster(thread)];
        /*
         * For IPI'ing CPUs that are homogeneous with the preferred cluster, use
         * immediate IPIs
         */
        ...
        /*
         * For workloads that are going wide, it might be useful to use
         * Immediate IPI to wakeup the idle CPU if the scheduler estimates that
         * the preferred pset will be busy for the deferred IPI timeout. The
         * Edge Scheduler uses the avg execution latency on the preferred pset
         * as an estimate of busyness.
         */
        if ((preferred_pset->pset_execution_time[thread->th_sched_bucket]
                 .pset_avg_thread_execution_time *
             NSEC_PER_USEC) >= ml_cpu_signal_deferred_get_timer()) {
          return dst_idle ? SCHED_IPI_IDLE : SCHED_IPI_IMMEDIATE;
        }
      }
    }
 ...
 */
uint32_t sched_edge_qos_max_parallelism(int qos, uint64_t options) {
  ...
  if (options & QOS_PARALLELISM_REALTIME) {
     ...
     */
    uint32_t high_cpu_count =
        ml_get_cpu_number_type(high_core_type, false, false);
    uint32_t high_cluster_count = ml_get_cluster_number_type(high_core_type);
    return (options & QOS_PARALLELISM_CLUSTER_SHARED_RESOURCE)
               ? high_cluster_count
               : high_cpu_count;
  }
  ...
  case THREAD_QOS_MAINTENANCE:;
    uint32_t low_cpu_count =
        ml_get_cpu_number_type(low_core_type, false, false);
  ...
  default:;
    ...
    return (options & QOS_PARALLELISM_CLUSTER_SHARED_RESOURCE) ? total_clusters
                                                               : total_cpus;
  }
}
