...
kern_return_t exception_deliver(thread_t thread, exception_type_t exception,
...
#ifdef MACH_BSD
kern_return_t bsd_exception(exception_type_t exception,
                            mach_exception_data_t code,
                            mach_msg_type_number_t codeCnt);
#endif /* MACH_BSD */
...
TUNABLE_WRITEABLE(unsigned int, exception_log_max_pid, "exception_log_max_pid",
                  0);
...
ipc_port_t exception_port_copy_send(ipc_port_t port) {
  if (IP_VALID(port)) {
...
kern_return_t exception_deliver(thread_t thread, exception_type_t exception,
  ...
  /* The global exception-to-signal translation port is safe to be an exception
   * handler. */
  ...
  case EXCEPTION_STATE: {
    ...
    old_state_cnt =
        (flavor < THREAD_STATE_FLAVORS) ? _MachineStateCount[flavor] : 0;
    kr = thread_getstatus_to_user(thread, flavor, (thread_state_t)old_state,
                                  &old_state_cnt, get_flags);
    new_state_cnt = old_state_cnt;
    if (kr == KERN_SUCCESS) {
      new_state = (thread_state_t)kalloc_data(sizeof(thread_state_data_t),
                                              Z_WAITOK | Z_ZERO);
      if (new_state == NULL) {
      ...
      if (code64) {
        kr = mach_exception_raise_state(exc_port, exception, code, codeCnt,
                                        &flavor, old_state, old_state_cnt,
                                        new_state, &new_state_cnt);
      } else {
        kr = exception_raise_state(exc_port, exception, small_code, codeCnt,
                                   &flavor, old_state, old_state_cnt, new_state,
                                   &new_state_cnt);
      }
      if (kr == KERN_SUCCESS) {
        if (exception != EXC_CORPSE_NOTIFY) {
          kr = thread_setstatus_from_user(
              thread, flavor, (thread_state_t)new_state, new_state_cnt,
  ...
  case EXCEPTION_DEFAULT: {
    ...
    if (code64) {
      kr = mach_exception_raise(exc_port, thread_port, task_port, exception,
                                code, codeCnt);
    } else {
      kr = exception_raise(exc_port, thread_port, task_port, exception,
                           small_code, codeCnt);
    }
  ...
  case EXCEPTION_IDENTITY_PROTECTED: {
    ...
    if (code64) {
      kr = mach_exception_raise_identity_protected(exc_port, thread->thread_id,
                                                   task_token_port, exception,
                                                   code, codeCnt);
    } else {
  ...
  case EXCEPTION_STATE_IDENTITY_PROTECTED: {
    ...
    if (kr == KERN_SUCCESS) {
      ...
      if (code64) {
        kr = mach_exception_raise_state_identity_protected(
            exc_port, thread->thread_id, task_token_port, exception, code,
            codeCnt, &flavor, old_state, old_state_cnt, new_state,
            &new_state_cnt);
      } else {
    ...
    if (kr == KERN_SUCCESS) {
      ...
      if (code64) {
        kr = mach_exception_raise_state_identity(
            exc_port, thread_port, task_port, exception, code, codeCnt, &flavor,
            old_state, old_state_cnt, new_state, &new_state_cnt);
      } else {
        kr = exception_raise_state_identity(
            exc_port, thread_port, task_port, exception, small_code, codeCnt,
            &flavor, old_state, old_state_cnt, new_state, &new_state_cnt);
      }
 ...
 *      in exc_ports in order.
 *	Conditions:
 *		Caller has a reference on bt_object, and send rights on
 * exc_ports. Does not consume any passed references or rights
 */
void exception_deliver_backtrace(
    kcdata_object_t bt_object, ipc_port_t exc_ports[static BT_EXC_PORTS_COUNT],
  ...
  for (unsigned int i = 0; i < BT_EXC_PORTS_COUNT; i++) {
    ...
    kr = mach_exception_raise_backtrace(
        target_port, bt_obj_port, EXC_CORPSE_NOTIFY, code, EXCEPTION_CODE_MAX);
...
check_exc_receiver_dependency(exception_type_t exception,
 ...
 */
kern_return_t exception_triage_thread(exception_type_t exception,
                                      mach_exception_data_t code,
                                      mach_msg_type_number_t codeCnt,
                                      thread_t thread) {
  task_t task;
  ...
  if (panic_on_exception_triage) {
    panic("called exception_triage when it was forbidden by the boot "
          "environment");
  }
  ...
  if (KERN_SUCCESS ==
      check_exc_receiver_dependency(exception, actions, mutex)) {
...
static TUNABLE(bool, pac_exception_telemetry, "-pac_exception_telemetry",
               false);

CA_EVENT(pac_exception_event, CA_INT, exception, CA_INT, exception_code_0,
         CA_INT, exception_code_1, CA_STATIC_STRING(CA_PROCNAME_LEN),
         proc_name);

static void pac_exception_triage(exception_type_t exception,
                                 mach_exception_data_t code) {
  ...
  if (proc) {
     ...
     * the task has the TFRO_PAC_EXC_FATAL flag set. If both conditions are
     * true, terminate the task via exit_with_reason
     */
    if (!traced_flag) {
      ...
      if (task_is_pac_exception_fatal(task)) {
        ...
        exception_info_t info = {.os_reason = OS_REASON_PAC_EXCEPTION,
...
static void maybe_unrecoverable_exception_triage(exception_type_t exception,
                                                 mach_exception_data_t code) {
  task_t task = current_task();
  void *proc = get_bsdtask_info(task);
  ...
  /*
   * Note that the below policy to decide whether this should be unrecoverable
   * is likely conceptually specific to the particular exception. If you find
   * yourself adding another user_brk_..._descriptor and want to customize the
   * policy for whether it should be unrecoverable, consider attaching each
   * policy to the corresponding descriptor and somehow carrying it through to
   * here.
   */
  /* These exceptions are deliverable (and potentially recoverable) if the
  ...
  exception_info_t info = {
      /*
       * For now, hard-code this to OS_REASON_FOUNDATION as that's the path we
       * expect to be on today. In the future this should probably be carried by
       * the user_brk_..._descriptor and piped through.
       */
...
kern_return_t exception_triage(exception_type_t exception,
                               mach_exception_data_t code,
                               mach_msg_type_number_t codeCnt) {
  ...
  if (VM_MAP_PAGE_SIZE(task->map) < PAGE_SIZE) {
    DEBUG4K_EXC("thread %p task %p map %p exception %d codes 0x%llx 0x%llx\n",
                thread, task, task->map, exception, code[0],
                codeCnt > 1 ? code[1] : 0);
  ...
  if (proc_pid(get_bsdtask_info(task)) <= exception_log_max_pid) {
    record_system_event(SYSTEM_EVENT_TYPE_INFO, SYSTEM_EVENT_SUBSYSTEM_PROCESS,
                        "process exit",
                        "exception_log_max_pid: pid %d (%s): sending exception "
                        "%d (0x%llx 0x%llx)",
                        proc_pid(get_bsdtask_info(task)),
                        proc_name_address(get_bsdtask_info(task)), exception,
                        code[0], codeCnt > 1 ? code[1] : 0);
  }
#endif /* MACH_BSD */
#endif /* DEVELOPMENT || DEBUG */
  ...
  if (exception & EXC_PTRAUTH_BIT) {
    ...
    assert(codeCnt == 2);
    /* Note this may consume control flow if it decides the exception is
     * unrecoverable. */
...
kern_return_t bsd_exception(exception_type_t exception,
  ...
  kr = exception_deliver(self, exception, code, codeCnt, task->exc_actions,
                         mutex);
 ...
 * If the exception is fatal, we should be careful about sending a synchronous
 * exception
 */
kern_return_t task_exception_notify(exception_type_t exception,
                                    mach_exception_data_type_t exccode,
...
kern_return_t sys_perf_notify(thread_t thread, int pid) {
  ...
  mac_exc_associate_action_label(
      &saved_exc_actions[EXC_RPC_ALERT],
  ...
  if (!IP_VALID(xport) || !ip_active(xport) ||
  ...
  /* Save hostp->exc_actions and hold a sright to xport so it can't be dropped
   * after unlock */
...
#if CONFIG_MACF
  mac_exc_inherit_action_label(&hostp->exc_actions[EXC_RPC_ALERT],
                               &saved_exc_actions[EXC_RPC_ALERT]);
#endif /* CONFIG_MACF */
  ...
  wsave = thread_interrupt_level(THREAD_UNINT);
  ret = exception_deliver(thread, EXC_RPC_ALERT, code, 2, saved_exc_actions,
                          &hostp->lock);
  (void)thread_interrupt_level(wsave);
