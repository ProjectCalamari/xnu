...
typedef void (*sched_avg_comp_t)(void *param);
...
static_assert((SCHED_LOAD_EWMA_ALPHA_OLD + SCHED_LOAD_EWMA_ALPHA_NEW) ==
              (1ul << SCHED_LOAD_EWMA_ALPHA_SHIFT));

/* For fixpoint EWMA, roundup the load to make it converge */
#define SCHED_LOAD_EWMA_ROUNDUP(load)                                          \
  (((load) & (1ul << (SCHED_LOAD_EWMA_ALPHA_SHIFT - 1))) != 0)

/* Macro to convert scaled sched load to a real load value */
#define SCHED_LOAD_EWMA_UNSCALE(load)                                          \
  (((load) >> SCHED_LOAD_EWMA_ALPHA_SHIFT) + SCHED_LOAD_EWMA_ROUNDUP(load))

/*
 * Routine to capture the latest runnable counts and update sched_load (only
 * used for non-clutch schedulers)
 */
void compute_sched_load(void) {
  /*
   * Retrieve a snapshot of the current run counts.
   *
   * Why not a bcopy()? Because we need atomic word-sized reads of
   * sched_run_buckets, not byte-by-byte copy.
   */
  uint32_t ncpus = processor_avail_count;
  uint32_t load_now[TH_BUCKET_MAX];

  load_now[TH_BUCKET_RUN] =
      os_atomic_load(&sched_run_buckets[TH_BUCKET_RUN], relaxed);
  load_now[TH_BUCKET_FIXPRI] =
      os_atomic_load(&sched_run_buckets[TH_BUCKET_FIXPRI], relaxed);
  load_now[TH_BUCKET_SHARE_FG] =
      os_atomic_load(&sched_run_buckets[TH_BUCKET_SHARE_FG], relaxed);
  load_now[TH_BUCKET_SHARE_DF] =
      os_atomic_load(&sched_run_buckets[TH_BUCKET_SHARE_DF], relaxed);
  load_now[TH_BUCKET_SHARE_UT] =
      os_atomic_load(&sched_run_buckets[TH_BUCKET_SHARE_UT], relaxed);
  load_now[TH_BUCKET_SHARE_BG] =
      os_atomic_load(&sched_run_buckets[TH_BUCKET_SHARE_BG], relaxed);
  ...
  for (uint32_t i = TH_BUCKET_SHARE_FG; i <= TH_BUCKET_SHARE_BG; i++) {
    ...
    /* Plug the load values into the EWMA algorithm to calculate (scaled for
     * fixpoint) sched_load */
    sched_load[i] = (sched_load[i] * SCHED_LOAD_EWMA_ALPHA_OLD) +
                    ((bucket_load << SCHED_LOAD_EWMA_ALPHA_SHIFT) *
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      SCHED_LOAD_EWMA_UNSCALE(sched_load[TH_BUCKET_SHARE_FG]),
      SCHED_LOAD_EWMA_UNSCALE(sched_load[TH_BUCKET_SHARE_DF]),
      SCHED_LOAD_EWMA_UNSCALE(sched_load[TH_BUCKET_SHARE_UT]),
      SCHED_LOAD_EWMA_UNSCALE(sched_load[TH_BUCKET_SHARE_BG]), 0);
}

void compute_averages(uint64_t stdelta) {
  uint32_t nthreads =
      os_atomic_load(&sched_run_buckets[TH_BUCKET_RUN], relaxed) - 1;
  uint32_t ncpus = processor_avail_count;
  ...
  for (uint32_t index = 0; index < stdelta; index++) {
    for (uint32_t i = 0; i < 3; i++) {
      ...
      avenrun[i] =
          ((avenrun[i] * fract[i]) + (average_now * (LOAD_SCALE - fract[i]))) /
          LOAD_SCALE;
    }
  }
