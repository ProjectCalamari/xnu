...
/*
 *	To add a new entry:
 *		Add an "MACH_TRAP(routine, arg_count, num_32_bit_words,
 * munge_routine)" to the table below. where,
 *		- routine:		The trap handling routine in the kernel
 *		- arg_count:		The number of arguments for the mach
 * trap (independant of arch/arg size). This value also defines the number of
 * 64-bit words copied in for a U64 process.
 *		- num_32_bit_words:	The number of 32-bit words to be copied
 * in for a U32 process.
 *		- munge_routine:	The argument munging routine to align
 * input args correctly.
 *
...
const mach_trap_t mach_trap_table[MACH_TRAP_TABLE_COUNT] = {
    ...
    /* 26 */
    MACH_TRAP(mach_reply_port, 0, 0, NULL, .mach_trap_returns_port = 1),
    /* 27 */
    MACH_TRAP(thread_self_trap, 0, 0, NULL, .mach_trap_returns_port = 1),
    ...
    /* 63 */
    MACH_TRAP(mach_vm_reclaim_update_kernel_accounting_trap, 2, 2, munge_wl),
    ...
    /* 77 */
    MACH_TRAP(_kernelrpc_mach_port_request_notification_trap, 7, 7,
              munge_wwwwwww),
    ...
    /* 91 */
    MACH_TRAP(mk_timer_create_trap, 0, 0, NULL, .mach_trap_returns_port = 1),
...
const int mach_trap_count =
    (sizeof(mach_trap_table) / sizeof(mach_trap_table[0]));
