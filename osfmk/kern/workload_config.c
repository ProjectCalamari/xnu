...
__startup_func static void workload_config_setup(void) {
...
kern_return_t workload_config_init(workload_config_ctx_t *ctx) {
  ...
  ctx->wlcc_hashtbl =
      hashinit(workload_config_hash_size, 0, &ctx->wlcc_hash_mask);
  if (ctx->wlcc_hashtbl == NULL) {
    lck_mtx_unlock(&ctx->wlcc_mtx);
    return KERN_FAILURE;
  }
...
bool workload_config_initialized(const workload_config_ctx_t *ctx) {
  return ctx->wlcc_hashtbl != NULL;
}

void workload_config_free(workload_config_ctx_t *ctx) {
  ...
  for (int i = 0; i < workload_config_hash_size; i++) {
    ...
    LIST_FOREACH_SAFE(entry, head, wce_link, tmp) {
      ...
      LIST_FOREACH_SAFE(phase_entry, &entry->wce_phases, wpe_link, phase_tmp) {
 ...
 */
static workload_config_entry_t *lookup_entry(workload_config_ctx_t *ctx,
                                             const char *id) {
  assert(id != NULL);
 ...
 */
static kern_return_t insert_config(workload_config_ctx_t *ctx,
                                   workload_config_entry_t *entry,
                                   const char *phase,
                                   const workload_config_t *new_config) {
  assert(entry != NULL);
  assert(phase != NULL);
 ...
 */
static kern_return_t insert_entry(workload_config_ctx_t *ctx, const char *id,
                                  const char *phase,
 ...
 */
kern_return_t workload_config_insert(workload_config_ctx_t *ctx, const char *id,
                                     const char *phase,
                                     const workload_config_t *config) {
  assert(id != NULL);
  assert(phase != NULL);
  ...
  ret = (entry == NULL) ? insert_entry(ctx, id, phase, config)
 ...
 * development.
 */
static workload_config_ctx_t *get_ctx_locked(void) {
#if DEVELOPMENT || DEBUG
  /*
 ...
 */
kern_return_t workload_config_lookup(const char *id, const char *phase,
 ...
 */
kern_return_t workload_config_lookup_default(const char *id,
                                             workload_config_t *config) {
  assert(id != NULL);
  assert(config != NULL);
...
kern_return_t workload_config_set_default(workload_config_ctx_t *ctx,
...
void workload_config_iterate(bool (^cb)(const char *, const void *)) {
  workload_config_ctx_t *ctx = get_ctx_locked();
...
/* Iterate over phases. */
void workload_config_phases_iterate(const void *config,
                                    bool (^cb)(const char *phase,
  ...
  LIST_FOREACH(phase_entry, &entry->wce_phases, wpe_link) {
    ...
    if (cb(phase_entry->wpe_phase, is_default, &phase_entry->wpe_config)) {
      return;
    }
  }
}

kern_return_t workload_config_get_flags(workload_config_flags_t *flags) {
  assert(flags != NULL);
...
kern_return_t workload_config_clear_flag(workload_config_ctx_t *ctx,
...
bool workload_config_available(void) {
  workload_config_ctx_t *ctx = get_ctx_locked();
