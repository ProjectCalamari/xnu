...
void tp_block(tp_id_t testpoint) { tp_blocked_info[testpoint] = 1; }
...
void tp_wait(tp_id_t testpoint) {
  ...
  while ((tp_blocked_info[testpoint]) && wr <= 0) {
    wr = lck_mtx_sleep(&tp_mtx, LCK_SLEEP_DEFAULT, (event_t)&tp_blocked_info,
...
static int testpoint_handler(int64_t testpoint, int64_t *out) {
...
static int tp_scenario_handler(int64_t scenario, int64_t *out) {
...
static int tp_pid_handler(int64_t pid, int64_t *out) {
