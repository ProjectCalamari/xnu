 ...
 * P/E processor sets.
 */
void sched_amp_init(void) { sched_timeshare_init(); }

/* Spill threshold load average is ncpus in pset + (sched_amp_spill_count/(1 <<
...
_Atomic sched_perfctl_class_policy_t sched_perfctl_policy_util =
    SCHED_PERFCTL_POLICY_DEFAULT;
_Atomic sched_perfctl_class_policy_t sched_perfctl_policy_bg =
    SCHED_PERFCTL_POLICY_DEFAULT;
...
int sched_amp_spill_threshold(processor_set_t pset) {
  int recommended_processor_count =
      bit_count(pset->recommended_bitmask & pset->cpu_bitmask);

  return (recommended_processor_count << PSET_LOAD_FRACTIONAL_SHIFT) +
...
void pset_signal_spill(processor_set_t pset, int spilled_thread_priority) {
  ...
  uint64_t idle_map =
      pset->recommended_bitmask & pset->cpu_state_map[PROCESSOR_IDLE];
  for (int cpuid = lsb_first(idle_map); cpuid >= 0;
       cpuid = lsb_next(idle_map, cpuid)) {
    processor = processor_array[cpuid];
    if (bit_set_if_clear(pset->pending_spill_cpu_mask, processor->cpu_id)) {
      KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_AMP_SIGNAL_SPILL) | DBG_FUNC_NONE,
           processor->cpu_id, 0, 0, 0);
      ...
      if (processor == current_processor()) {
        ...
        if (bit_set_if_clear(pset->pending_AST_URGENT_cpu_mask,
                             processor->cpu_id)) {
          KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_PENDING_AST_URGENT) |
                   DBG_FUNC_START,
  ...
  uint64_t running_map =
      pset->recommended_bitmask & pset->cpu_state_map[PROCESSOR_RUNNING];
 ...
 * This function must be safe to call (to use as a hint) without holding the
 * pset lock.
 */
bool pset_should_accept_spilled_thread(processor_set_t pset,
  ...
  for (int cpuid = lsb_first(cpu_map); cpuid >= 0;
       cpuid = lsb_next(cpu_map, cpuid)) {
...
bool should_spill_to_ecores(processor_set_t nset, thread_t thread) {
  ...
  if ((sched_get_pset_load_average(nset, 0) >=
       sched_amp_spill_threshold(
           nset)) && /* There is already a load on P cores */
      pset_should_accept_spilled_thread(
          ecore_set,
          thread->sched_pri)) { /* There are lower priority E cores */
    return true;
  }
 ...
 *
 * Routine to check if the thread should be spilled and signal the pset if
 * needed.
 */
...
int sched_amp_steal_threshold(processor_set_t pset, bool spill_pending) {
  ...
  return (recommended_processor_count << PSET_LOAD_FRACTIONAL_SHIFT) +
         (spill_pending ? sched_amp_spill_steal : sched_amp_idle_steal);
}
...
bool sched_amp_steal_thread_enabled(processor_set_t pset) {
  return (pset->pset_cluster_type == PSET_AMP_E) && (pcore_set != NULL) &&
         (pcore_set->online_processor_count > 0);
}
...
bool sched_amp_balance(processor_t cprocessor, processor_set_t cpset) {
  ...
  if (!ecore_set || cpset->pset_cluster_type == PSET_AMP_E ||
      !cprocessor->is_recommended) {
    return false;
  }
  ...
  for (int cpuid = lsb_first(running_map); cpuid >= 0;
    ...
    if ((eprocessor->current_pri < BASEPRI_RTQUEUES) &&
      ...
      ipi_type[eprocessor->cpu_id] =
          sched_ipi_action(eprocessor, NULL, SCHED_IPI_EVENT_REBALANCE);
...
uint32_t sched_amp_qos_max_parallelism(int qos, uint64_t options) {
  ...
  if (options & QOS_PARALLELISM_REALTIME) {
     ...
     */
    return (options & QOS_PARALLELISM_CLUSTER_SHARED_RESOURCE) ? pcluster_count
                                                               : pcount;
  }
...
pset_node_t sched_amp_choose_node(thread_t thread) {
  pset_cluster_type_t pset_cluster_type =
      (recommended_pset_type(thread) == PSET_AMP_P) ? PSET_AMP_P : PSET_AMP_E;
