...
static inline bool semaphore_active(semaphore_t semaphore) {
  return semaphore->owner != TASK_NULL;
}

static __inline__ uint64_t semaphore_deadline(unsigned int sec,
                                              clock_res_t nsec) {
  uint64_t abstime;
 ...
 */
kern_return_t semaphore_create(task_t task, semaphore_t *new_semaphore,
 ...
 */
static void semaphore_destroy_internal(task_t task, semaphore_t semaphore,
  ...
  if (old_count < 0) {
    waitq_wakeup64_all_locked(&semaphore->waitq, SEMAPHORE_EVENT,
                              THREAD_RESTART,
 ...
 *			Nothing is locked.
 */
__attribute__((noinline)) static void semaphore_free(semaphore_t semaphore) {
  ipc_port_t port;
  task_t task;
  ...
  if (IP_VALID(port)) {
    ...
    ipc_kobject_dealloc_port(port, IPC_KOBJECT_NO_MSCOUNT, IKOT_SEMAPHORE);
  }
...
void semaphore_destroy_all(task_t task) {
 ...
 *		Semaphore is locked.
 */
static kern_return_t semaphore_signal_internal(semaphore_t semaphore,
                                               thread_t thread, int options) {
  kern_return_t kr;
  ...
  if (thread != THREAD_NULL) {
    if (semaphore->count < 0) {
      kr = waitq_wakeup64_thread_and_unlock(&semaphore->waitq, SEMAPHORE_EVENT,
  ...
  if (options & SEMAPHORE_SIGNAL_ALL) {
    ...
    if (old_count < 0) {
      ...
      kr = waitq_wakeup64_all_locked(
          &semaphore->waitq, SEMAPHORE_EVENT, THREAD_AWAKENED,
  ...
  if (semaphore->count < 0) {
    ...
    kr = waitq_wakeup64_one_locked(&semaphore->waitq, SEMAPHORE_EVENT,
...
kern_return_t semaphore_signal_thread(semaphore_t semaphore, thread_t thread) {
  ...
  return semaphore_signal_internal(semaphore, thread, SEMAPHORE_OPTION_NONE);
}
...
semaphore_signal_thread_trap(struct semaphore_signal_thread_trap_args *args) {
  ...
  if (kr == KERN_SUCCESS) {
    kr = semaphore_signal_internal(semaphore, thread, SEMAPHORE_OPTION_NONE);
    semaphore_dereference(semaphore);
  }
  if (thread != THREAD_NULL) {
...
kern_return_t semaphore_signal(semaphore_t semaphore) {
  ...
  kr = semaphore_signal_internal(semaphore, THREAD_NULL,
 ...
 */
kern_return_t semaphore_signal_trap(struct semaphore_signal_trap_args *args) {
  mach_port_name_t sema_name = args->signal_name;
...
kern_return_t semaphore_signal_all(semaphore_t semaphore) {
  ...
  kr = semaphore_signal_internal(semaphore, THREAD_NULL, SEMAPHORE_SIGNAL_ALL);
  if (kr == KERN_NOT_WAITING) {
    return KERN_SUCCESS;
  }
  return kr;
...
semaphore_signal_all_trap(struct semaphore_signal_all_trap_args *args) {
  ...
  if (kr == KERN_SUCCESS) {
    kr =
        semaphore_signal_internal(semaphore, THREAD_NULL, SEMAPHORE_SIGNAL_ALL);
    semaphore_dereference(semaphore);
    if (kr == KERN_NOT_WAITING) {
      kr = KERN_SUCCESS;
    }
 ...
 *	appropriate result.
 */
static kern_return_t semaphore_convert_wait_result(int wait_result) {
  switch (wait_result) {
 ...
 */
static void semaphore_wait_continue(void *arg __unused, wait_result_t wr) {
 ...
 */
static kern_return_t semaphore_wait_internal(semaphore_t wait_semaphore,
                                             semaphore_t signal_semaphore,
                                             uint64_t deadline, int option,
  ...
  if (!semaphore_active(wait_semaphore)) {
    ...
    (void)waitq_assert_wait64_locked(
        &wait_semaphore->waitq, SEMAPHORE_EVENT, THREAD_ABORTSAFE,
        TIMEOUT_URGENCY_USER_NORMAL, deadline, TIMEOUT_NO_LEEWAY, self);
  ...
  if (signal_semaphore != SEMAPHORE_NULL) {
     ...
     */
    signal_kr = semaphore_signal_internal(signal_semaphore, THREAD_NULL,
  ...
  if (caller_cont) {
    ...
    thread_handoff_parameter(handoff_thread, semaphore_wait_continue, NULL,
                             handoff_option);
  } else {
...
kern_return_t semaphore_wait(semaphore_t semaphore) {
  ...
  return semaphore_wait_internal(semaphore, SEMAPHORE_NULL, 0ULL,
                                 SEMAPHORE_OPTION_NONE, SEMAPHORE_CONT_NULL);
}
...
kern_return_t semaphore_wait_deadline(semaphore_t semaphore,
                                      uint64_t deadline) {
  if (semaphore == SEMAPHORE_NULL) {
    return KERN_INVALID_ARGUMENT;
  }

  return semaphore_wait_internal(semaphore, SEMAPHORE_NULL, deadline,
                                 SEMAPHORE_OPTION_NONE, SEMAPHORE_CONT_NULL);
}
...
kern_return_t semaphore_wait_trap(struct semaphore_wait_trap_args *args) {
  return semaphore_wait_trap_internal(args->wait_name, thread_syscall_return);
}

kern_return_t semaphore_wait_trap_internal(mach_port_name_t name,
                                           semaphore_cont_t caller_cont) {
  semaphore_t semaphore;
  kern_return_t kr;
  ...
  if (kr == KERN_SUCCESS) {
    kr = semaphore_wait_internal(semaphore, SEMAPHORE_NULL, 0ULL,
                                 SEMAPHORE_OPTION_NONE, caller_cont);
    semaphore_dereference(semaphore);
  }
  return kr;
}
...
kern_return_t semaphore_timedwait(semaphore_t semaphore,
  ...
  return semaphore_wait_internal(semaphore, SEMAPHORE_NULL, deadline, option,
                                 SEMAPHORE_CONT_NULL);
}
...
semaphore_timedwait_trap(struct semaphore_timedwait_trap_args *args) {
  return semaphore_timedwait_trap_internal(args->wait_name, args->sec,
                                           args->nsec, thread_syscall_return);
}

kern_return_t semaphore_timedwait_trap_internal(mach_port_name_t name,
                                                unsigned int sec,
                                                clock_res_t nsec,
                                                semaphore_cont_t caller_cont) {
  semaphore_t semaphore;
  ...
  if (kr == KERN_SUCCESS) {
    ...
    kr = semaphore_wait_internal(semaphore, SEMAPHORE_NULL, deadline, option,
                                 caller_cont);
    semaphore_dereference(semaphore);
  }
  return kr;
...
kern_return_t semaphore_wait_signal(semaphore_t wait_semaphore,
  ...
  return semaphore_wait_internal(wait_semaphore, signal_semaphore, 0ULL,
                                 SEMAPHORE_OPTION_NONE, SEMAPHORE_CONT_NULL);
}
...
semaphore_wait_signal_trap(struct semaphore_wait_signal_trap_args *args) {
  return semaphore_wait_signal_trap_internal(args->wait_name, args->signal_name,
                                             thread_syscall_return);
}

kern_return_t
semaphore_wait_signal_trap_internal(mach_port_name_t wait_name,
  ...
  if (kr == KERN_SUCCESS) {
    ...
    if (kr == KERN_SUCCESS) {
      kr = semaphore_wait_internal(wait_semaphore, signal_semaphore, 0ULL,
                                   SEMAPHORE_OPTION_NONE, caller_cont);
...
kern_return_t semaphore_timedwait_signal(semaphore_t wait_semaphore,
  ...
  return semaphore_wait_internal(wait_semaphore, signal_semaphore, deadline,
                                 option, SEMAPHORE_CONT_NULL);
}
...
kern_return_t semaphore_timedwait_signal_trap(
  ...
  return semaphore_timedwait_signal_trap_internal(
      args->wait_name, args->signal_name, args->sec, args->nsec,
      thread_syscall_return);
}

kern_return_t semaphore_timedwait_signal_trap_internal(
    mach_port_name_t wait_name, mach_port_name_t signal_name, unsigned int sec,
    clock_res_t nsec, semaphore_cont_t caller_cont) {
  semaphore_t wait_semaphore;
  semaphore_t signal_semaphore;
  mach_timespec_t wait_time;
  kern_return_t kr;
  ...
  if (kr == KERN_SUCCESS) {
    ...
    if (kr == KERN_SUCCESS) {
      ...
      kr = semaphore_wait_internal(wait_semaphore, signal_semaphore, deadline,
                                   option, caller_cont);
      semaphore_dereference(wait_semaphore);
    }
    semaphore_dereference(signal_semaphore);
 ...
 */
void semaphore_reference(semaphore_t semaphore) {
 ...
 */
void semaphore_dereference(semaphore_t semaphore) {
  if (semaphore == NULL) {
    return;
  }
...
void kdp_sema_find_owner(struct waitq *waitq, __assert_only event64_t event,
 ...
 *		Nothing locked.
 */
kern_return_t port_name_to_semaphore(mach_port_name_t name,
...
ipc_port_t convert_semaphore_to_port(semaphore_t semaphore) {
   ...
   */
  if (!ipc_kobject_make_send_lazy_alloc_port(&semaphore->port, semaphore,
                                             IKOT_SEMAPHORE)) {
    semaphore_dereference(semaphore);
  }
 ...
 *	send-right (reference) to the semaphore's port. This notification
 * function will consume the semaphore reference donated to the extant
 * collection of send-rights.
 */
static void semaphore_no_senders(ipc_port_t port,
...
IPC_KOBJECT_DEFINE(IKOT_SEMAPHORE, .iko_op_movable_send = true,
                   .iko_op_stable = true,
