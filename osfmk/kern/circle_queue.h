...
static inline queue_entry_t circle_queue_next(circle_queue_t cq,
                                              queue_entry_t elt) {
...
static inline bool circle_queue_concat_tail(circle_queue_t dq,
                                            circle_queue_t sq) {
 ...
 *	Macro:		cqe_element
 *	Function:
 *		Convert a cirle_queue_entry_t pointer to a queue element
 * pointer. Get a pointer to the user-defined element containing a given
 * cirle_queue_entry_t Header: <type> * cqe_element(cirle_queue_entry_t qe,
 * <type>, field) qe      - queue entry to convert <type>  - what's in the queue
 * (e.g., struct some_data) <field> - is the chain field in <type> Note: Do not
 * use pointer types for <type>
 */
...
#define cqe_foreach_safe(qe, head)                                             \
  for (queue_entry_t _ne, _qe = circle_queue_first(head);                      \
       (qe = _qe) && (_ne = circle_queue_next(head, _qe), 1); _qe = _ne)
