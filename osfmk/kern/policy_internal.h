...
#define TASK_POLICY_BASE_LATENCY_AND_THROUGHPUT_QOS                            \
  IMP_TASK_POLICY_BASE_LATENCY_AND_THROUGHPUT_QOS /* latency as value1,        \
                                                     throughput as value2 */
#define TASK_POLICY_OVERRIDE_LATENCY_AND_THROUGHPUT_QOS                        \
  IMP_TASK_POLICY_OVERRIDE_LATENCY_AND_THROUGHPUT_QOS /* latency as value1,    \
                                                         throughput as value2  \
                                                       */
...
#define TASK_POLICY_QOS_AND_RELPRIO                                            \
  IMP_TASK_POLICY_QOS_AND_RELPRIO /* QoS as value1, relative priority as       \
                                     value2 */
...
#define TASK_POLICY_IOTIER_KEVENT_OVERRIDE                                     \
  IMP_TASK_POLICY_IOTIER_KEVENT_OVERRIDE
...
extern void proc_set_task_policy(task_t task, int category, int flavor,
                                 int value);
extern int proc_get_task_policy(task_t task, int category, int flavor);

extern void proc_set_thread_policy(thread_t thread, int category, int flavor,
                                   int value);
extern void proc_set_thread_policy_ext(thread_t thread, int category,
...
extern void proc_set_thread_policy_with_tid(task_t task, uint64_t tid,
                                            int category, int flavor,
                                            int value);
...
extern int proc_darwin_role_to_task_role(int darwin_role,
...
extern void proc_set_task_spawnpolicy(task_t task, thread_t thread, int apptype,
...
extern int proc_thread_qos_add_override(task_t task, thread_t thread,
                                        uint64_t tid, int override_qos,
                                        boolean_t first_override_for_resource,
                                        user_addr_t resource,
                                        int resource_type);
extern int proc_thread_qos_remove_override(task_t task, thread_t thread,
                                           uint64_t tid, user_addr_t resource,
                                           int resource_type);
...
extern void thread_set_workq_pri(thread_t thread, thread_qos_t qos,
                                 integer_t priority, integer_t policy);
...
extern thread_qos_t task_get_default_manager_qos(task_t task);
...
extern int proc_lf_pidbind(task_t curtask, uint64_t tid, task_t target_task,
                           int bind);
...
extern void task_importance_init_from_parent(task_t new_task,
                                             task_t parent_task);
...
extern int task_importance_hold_file_lock_assertion(task_t target_task,
                                                    uint32_t count);
extern int task_importance_drop_file_lock_assertion(task_t target_task,
                                                    uint32_t count);

extern int task_importance_hold_legacy_external_assertion(task_t target_task,
                                                          uint32_t count);
extern int task_importance_drop_legacy_external_assertion(task_t target_task,
                                                          uint32_t count);
...
#define TASK_POLICY_RESOURCE_ATTRIBUTE_DEFAULT                                 \
  TASK_POLICY_RESOURCE_ATTRIBUTE_NONE

extern int proc_get_task_ruse_cpu(task_t task, uint32_t *policyp,
                                  uint8_t *percentagep, uint64_t *intervalp,
                                  uint64_t *deadlinep);
extern int proc_set_task_ruse_cpu(task_t task, uint16_t policy,
                                  uint8_t percentage, uint64_t interval,
                                  uint64_t deadline, int cpumon_entitled);
...
extern int task_low_mem_privileged_listener(task_t task, boolean_t new_value,
                                            boolean_t *old_value);
...
extern kern_return_t thread_policy_set_internal(thread_t thread,
...
extern void thread_add_servicer_override(thread_t thread,
                                         uint32_t qos_override);
extern void thread_update_servicer_override(thread_t thread,
                                            uint32_t qos_override);
extern void thread_drop_servicer_override(thread_t thread);
extern void thread_update_servicer_iotier_override(thread_t thread,
...
extern void thread_update_kevent_override(thread_t thread,
                                          uint32_t qos_override);
...
extern void
coalition_policy_update_task(task_t task,
                             coalition_pend_token_t coal_pend_token);

extern bool task_get_effective_jetsam_coalition_policy(task_t task, int flavor);

extern void task_policy_update_complete_unlocked(task_t task,
                                                 task_pend_token_t pend_token);
extern void task_update_boost_locked(task_t task, boolean_t boost_active,
                                     task_pend_token_t pend_token);

extern void thread_policy_update_locked(thread_t thread,
                                        task_pend_token_t pend_token);
extern void
thread_policy_update_complete_unlocked(thread_t task,
                                       task_pend_token_t pend_token);
...
#define TASK_RUSECPU_FLAGS_FATAL_CPUMON                                        \
  0x08 /* CPU usage monitor violations are fatal */
#define TASK_RUSECPU_FLAGS_FATAL_WAKEUPSMON                                    \
  0x10 /* wakeups monitor violations are fatal */
...
extern kern_return_t thread_set_mode_and_absolute_pri(thread_t thread,
                                                      integer_t policy,
                                                      integer_t priority);

extern void thread_policy_update_tasklocked(thread_t thread, integer_t priority,
 ...
 *   @param violator the task (process) violating its limits (should be the
 * current task)
 ...
 *
 *   @discussion
 *       send_resource_violation_with_fatal_port() calls the corresponding MIG
 * routine over the host special RESOURCE_NOTIFY port. If fatal_port is set,
 * then deallocating that port right, will kill the process.
 *
 */
