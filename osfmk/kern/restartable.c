 ...
 */
static int _ranges_cmp(const void *_r1, const void *_r2) {
 ...
 */
static kern_return_t _ranges_validate(task_t task,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (os_add_overflow(ranges[i].location, ranges[i].recovery_offs,
                        &recovery)) {
      return KERN_INVALID_ARGUMENT;
    }
 ...
 */
__attribute__((always_inline)) static mach_vm_address_t
 ...
 */
static bool _restartable_ranges_equals(const struct restartable_ranges *rr1,
                                       const struct restartable_ranges *rr2) {
  size_t rr1_size = rr1->rr_count * sizeof(task_restartable_range_t);
  return rr1->rr_hash == rr2->rr_hash && rr1->rr_count == rr2->rr_count &&
...
_restartable_ranges_create(task_t task, task_restartable_range_t *ranges,
                           uint32_t count,
                           struct restartable_ranges **rr_storage) {
  ...
  rr->rr_hash = os_hash_jenkins(
      rr->rr_ranges, rr->rr_count * sizeof(task_restartable_range_t));
...
void thread_reset_pcs_ack_IPI(struct thread *thread) {
  ...
  if (__improbable(trrs.trr_ipi_ack_pending)) {
    ...
    os_atomic_store(&thread->t_rr_state.trr_value, trrs.trr_value, relaxed);
  }
}

static bool thread_rr_wait_if_needed(task_t task, thread_t thread) {
  thread_rr_state_t state;
  ...
  while (state.trr_ipi_ack_pending) {
    ...
    state.trr_value =
        hw_wait_while_equals32(&thread->t_rr_state.trr_value, state.trr_value);
  ...
  if (state.trr_fault_state == TRR_FAULT_OBSERVED) {
    ...
    if (os_atomic_cmpxchg(&thread->t_rr_state, state, nstate, relaxed)) {
      lck_mtx_sleep_with_inheritor(&task->lock, LCK_SLEEP_DEFAULT,
                                   &thread->t_rr_state, thread, THREAD_UNINT,
...
bool thread_reset_pcs_in_range(task_t task, thread_t thread) {
  return _ranges_lookup(task->t_rr_ranges, machine_thread_pc(thread)) != 0;
}

void thread_reset_pcs_ast(task_t task, thread_t thread) {
...
kern_return_t task_restartable_ranges_register(task_t task,
...
#if CONFIG_ROSETTA
  // <rdar://problem/48527888> Obj-C adoption of
  // task_restartable_ranges_register breaks Cambria
  ...
  if (kr == KERN_SUCCESS) {
    ...
    if (task->thread_count > 1) {
      ...
      kr = _restartable_ranges_create(task, ranges, count, &task->t_rr_ranges);
    }
