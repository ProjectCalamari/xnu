...
static uint64_t sane_us2abs(uint64_t us) {
  uint64_t t;
...
#if !KASAN
static void hw_lck_ticket_test_wait_for_delta(hw_lck_ticket_t *lck,
                                              uint8_t delta, int msec) {
  hw_lck_ticket_t tmp;
...
#if CONFIG_PV_TICKET
    const uint8_t cticket = tmp.cticket & ~HW_LCK_TICKET_LOCK_PVWAITFLAG;
#else
...
hw_lck_ticket_allow_invalid_worker(void *arg, wait_result_t __unused wr) {
  ...
  rc =
      hw_lck_ticket_lock_allow_invalid(lck, &hw_lock_test_give_up_policy, NULL);
...
static int hw_lck_ticket_allow_invalid_test(__unused int64_t in, int64_t *out) {
  vm_offset_t addr = 0;
  hw_lck_ticket_t *lck;
  kern_return_t kr;
  hw_lock_status_t rc;
  ...
  kr = kmem_alloc(kernel_map, &addr, PAGE_SIZE, KMA_ZERO | KMA_KOBJECT,
                  VM_KERN_MEMORY_DIAG);
  if (kr != KERN_SUCCESS) {
...
#if SCHED_HYGIENE_DEBUG
  if (os_atomic_load(&sched_preemption_disable_threshold_mt, relaxed) <
      sane_us2abs(20 * 1000)) {
    /*
  ...
  kernel_memory_depopulate(addr, PAGE_SIZE, KMA_KOBJECT, VM_KERN_MEMORY_DIAG);
...
SYSCTL_TEST_REGISTER(hw_lck_ticket_allow_invalid,
                     hw_lck_ticket_allow_invalid_test);
...
static bool smrh_elem_try_get(void *arg __unused) { return true; }
...
static int smr_hash_basic_test(__unused int64_t in, int64_t *out) {
...
static int smr_shash_basic_test(__unused int64_t in, int64_t *out) {
  __auto_type T = &smrh_test_traits;
...
static void smr_sleepable_stress_cb(smr_node_t node) {
...
static void smr_sleepable_stress_make_call(struct smr_ctx *ctx) {
  struct smr_call_ctx *cctx;
...
static void smr_sleepable_stress_log(struct smr_ctx *ctx, uint64_t n) {
  printf("%s[%lld]: "
         "%d/%d calls, %d syncs, %d barriers, "
         "rd-seq: %ld, wr-seq: %ld\n",
         __func__, n, ctx->calls_done, ctx->calls_sent, ctx->syncs_done,
         ctx->barriers_done, ctx->smr->smr_clock.s_rd_seq / SMR_SEQ_INC,
         ctx->smr->smr_clock.s_wr_seq / SMR_SEQ_INC);
}

static uint32_t smr_sleepable_stress_idx(struct smr_ctx *ctx, thread_t self) {
  if (ctx->driver == self) {
    return 0;
  }
...
static void smr_sleepable_stress_worker(void *arg, wait_result_t wr __unused) {
...
static int smr_sleepable_stress_test(int64_t seconds, int64_t *out) {
  ...
  for (uint32_t i = 1; i < ctx.active; i++) {
    kernel_thread_start_priority(smr_sleepable_stress_worker, &ctx,
                                 BASEPRI_DEFAULT, &th);
    thread_deallocate(th);
  }
