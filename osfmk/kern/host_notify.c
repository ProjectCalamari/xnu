...
static KALLOC_TYPE_DEFINE(host_notify_zone, struct host_notify_entry,
                          KT_DEFAULT);
...
kern_return_t host_request_notification(host_t host, host_flavor_t notify_type,
                                        ipc_port_t port) {
  host_notify_t entry;
  kern_return_t kr;
...
static void host_notify_all(host_flavor_t notify_type, mach_msg_header_t *msg,
  ...
  qe_foreach_safe(e, &host_notify_queue[notify_type]) {
    ...
    ip_mq_lock(port);
    if (ip_active(port)) {
      ipc_port_request_cancel(port, IPR_HOST_NOTIFY, entry->index);
      remqueue(e);
  ...
  msg->msgh_bits = MACH_MSGH_BITS_SET(MACH_MSG_TYPE_MOVE_SEND_ONCE, 0, 0, 0);
