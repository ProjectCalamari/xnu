...
#define EXCLAVES_ID_TIGHTBEAM_UPCALL ((exclaves_id_t)XNUPROXY_UPCALL_TIGHTBEAM)
...
static tb_error_t exclaves_helloupcall(const uint64_t arg,
                                       tb_error_t (^completion)(uint64_t));
...
,
    ...
    .alloc =
        ^(const uint32_t npages, xnuupcalls_pagekind_s kind,
          tb_error_t (^completion)(xnuupcalls_pagelist_s)) {
          return exclaves_memory_upcall_legacy_alloc(npages, kind, completion);
        },

    .alloc_ext =
        ^(const uint32_t npages, xnuupcalls_pageallocflags_s flags,
          tb_error_t (^completion)(xnuupcalls_pagelist_s)) {
          return exclaves_memory_upcall_legacy_alloc_ext(npages, flags,
                                                         completion);
        },

    .free =
        ^(const uint32_t pages[_Nonnull EXCLAVES_MEMORY_MAX_REQUEST],
          const uint32_t npages, const xnuupcalls_pagekind_s kind,
          tb_error_t (^completion)(void)) {
          return exclaves_memory_upcall_legacy_free(pages, npages, kind,
                                                    completion);
        },

    .free_ext =
        ^(const uint32_t pages[_Nonnull EXCLAVES_MEMORY_MAX_REQUEST],
          const uint32_t npages, xnuupcalls_pagefreeflags_s flags,
          tb_error_t (^completion)(void)) {
          return exclaves_memory_upcall_legacy_free_ext(pages, npages, flags,
                                                        completion);
        },
    ...
    .open =
        ^(const enum xnuupcalls_fstag_s fstag, const uint64_t rootid,
          const uint8_t name[_Nonnull 256],
          tb_error_t (^completion)(xnuupcalls_xnuupcalls_open__result_s)) {
          return exclaves_storage_upcall_legacy_open(fstag, rootid, name,
                                                     completion);
        },

    .close =
        ^(const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
          tb_error_t (^completion)(xnuupcalls_xnuupcalls_close__result_s)) {
          return exclaves_storage_upcall_legacy_close(fstag, fileid,
                                                      completion);
        },
    ...
    .create =
        ^(const enum xnuupcalls_fstag_s fstag, const uint64_t rootid,
          ...
          return exclaves_storage_upcall_legacy_create(fstag, rootid, name,
                                                       completion);
        },
    ...
    .read =
        ^(const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
          ...
          return exclaves_storage_upcall_legacy_read(fstag, fileid, descriptor,
                                                     completion);
        },
    ...
    .write =
        ^(const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
          ...
          return exclaves_storage_upcall_legacy_write(fstag, fileid, descriptor,
                                                      completion);
        },
    ...
    .remove =
        ^(const enum xnuupcalls_fstag_s fstag, const uint64_t rootid,
          ...
          return exclaves_storage_upcall_legacy_remove(fstag, rootid, name,
                                                       completion);
        },

    .sync =
        ^(const enum xnuupcalls_fstag_s fstag,
          const enum xnuupcalls_syncop_s op, const uint64_t fileid,
          tb_error_t (^completion)(xnuupcalls_xnuupcalls_sync__result_s)) {
          return exclaves_storage_upcall_legacy_sync(fstag, op, fileid,
                                                     completion);
        },

    .readdir =
        ^(const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
          const uint64_t buf, const uint32_t length,
          tb_error_t (^completion)(xnuupcalls_xnuupcalls_readdir__result_s)) {
          return exclaves_storage_upcall_legacy_readdir(fstag, fileid, buf,
                                                        length, completion);
        },
    ...
    .getsize =
        ^(const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
          ...
          return exclaves_storage_upcall_legacy_getsize(fstag, fileid,
                                                        completion);
        },
    ...
    .irq_register =
        ^(const uint64_t id, const int32_t index,
          ...
          return exclaves_driverkit_upcall_legacy_irq_register(id, index,
                                                               completion);
        },
    ...
    .irq_remove =
        ^(const uint64_t id, const int32_t index,
          tb_error_t (^completion)(
              xnuupcalls_xnuupcalls_irq_remove__result_s)) {
          return exclaves_driverkit_upcall_legacy_irq_remove(id, index,
                                                             completion);
        },
    ...
    .irq_enable =
        ^(const uint64_t id, const int32_t index,
          tb_error_t (^completion)(
              xnuupcalls_xnuupcalls_irq_enable__result_s)) {
          return exclaves_driverkit_upcall_legacy_irq_enable(id, index,
                                                             completion);
        },
    ...
    .irq_disable =
        ^(const uint64_t id, const int32_t index,
          ...
          return exclaves_driverkit_upcall_legacy_irq_disable(id, index,
                                                              completion);
        },
    ...
    .timer_register =
        ^(const uint64_t id,
          ...
          return exclaves_driverkit_upcall_legacy_timer_register(id,
                                                                 completion);
        },
    ...
    .timer_remove =
        ^(const uint64_t id, const uint32_t timer_id,
          tb_error_t (^completion)(
              xnuupcalls_xnuupcalls_timer_remove__result_s)) {
          return exclaves_driverkit_upcall_legacy_timer_remove(id, timer_id,
                                                               completion);
        },
    ...
    .timer_enable =
        ^(const uint64_t id, const uint32_t timer_id,
          ...
          return exclaves_driverkit_upcall_legacy_timer_enable(id, timer_id,
                                                               completion);
        },
    ...
    .timer_disable =
        ^(const uint64_t id, const uint32_t timer_id,
          ...
          return exclaves_driverkit_upcall_legacy_timer_disable(id, timer_id,
                                                                completion);
        },
    ...
    .timer_set_timeout =
        ^(const uint64_t id, const uint32_t timer_id,
          ...
          return exclaves_driverkit_upcall_legacy_timer_set_timeout(
              id, timer_id, duration, completion);
        },
    ...
    .timer_cancel_timeout =
        ^(const uint64_t id, const uint32_t timer_id,
          ...
          return exclaves_driverkit_upcall_legacy_timer_cancel_timeout(
              id, timer_id, completion);
        },
    ...
    .async_notification_signal =
        ^(const uint64_t id, const uint32_t notificationID,
          ...
          return exclaves_driverkit_upcall_legacy_async_notification_signal(
    ...
    .mapper_activate =
        ^(const uint64_t id, const uint32_t mapperIndex,
          ...
          return exclaves_driverkit_upcall_legacy_mapper_activate(
              id, mapperIndex, completion);
        },
    ...
    .mapper_deactivate =
        ^(const uint64_t id, const uint32_t mapperIndex,
          ...
          return exclaves_driverkit_upcall_legacy_mapper_deactivate(
              id, mapperIndex, completion);
        },
    ...
    .notification_signal =
        ^(const uint64_t id, const uint32_t mask,
          ...
          return exclaves_driverkit_upcall_legacy_notification_signal(
              id, mask, completion);
        },
    ...
    .ane_setpowerstate =
        ^(const uint64_t id, const uint32_t desiredState,
          ...
          return exclaves_driverkit_upcall_legacy_ane_setpowerstate(
              id, desiredState, completion);
        },
    ...
    .ane_worksubmit =
        ^(const uint64_t id, const uint64_t requestID,
          ...
          return exclaves_driverkit_upcall_legacy_ane_worksubmit(
              id, requestID, taskDescriptorCount, submitTimestamp, completion);
        },
    ...
    .ane_workbegin =
        ^(const uint64_t id, const uint64_t requestID,
          ...
          return exclaves_driverkit_upcall_legacy_ane_workbegin(
              id, requestID, beginTimestamp, completion);
        },

    .ane_workend =
        ^(const uint64_t id, const uint64_t requestID,
          tb_error_t (^completion)(
              xnuupcalls_xnuupcalls_ane_workend__result_s)) {
          return exclaves_driverkit_upcall_legacy_ane_workend(id, requestID,
                                                              completion);
        },
    ...
    .conclave_stop =
        ^(const uint32_t flags,
          tb_error_t (^completion)(
              xnuupcalls_xnuupcalls_conclave_stop__result_s)) {
          return exclaves_conclave_upcall_legacy_stop(flags, completion);
        },
    .conclave_crash_info =
        ^(const xnuupcalls_conclavesharedbuffer_s *shared_buf,
          const uint32_t length,
          tb_error_t (^completion)(
              xnuupcalls_xnuupcalls_conclave_crash_info__result_s)) {
          return exclaves_conclave_upcall_legacy_crash_info(shared_buf, length,
                                                            completion);
        },
...
static const xnuupcallsv2_memoryupcallsprivate__server_s
    exclaves_tightbeam_memory_upcalls_v2 = {
        /* BEGIN IGNORE CODESTYLE */
        /* Uncrustify doesn't deal well with Blocks. */
...
,
    ...
    .rootex =
        ^(const uint32_t fstag, const uint8_t exclaveid[_Nonnull 32],
          tb_error_t (^completion)(
    ...
    .open =
        ^(const uint32_t fstag, const uint64_t rootid,
          const uint8_t name[_Nonnull 256],
          tb_error_t (^completion)(
    ...
    .close =
        ^(const uint32_t fstag, const uint64_t fileid,
          tb_error_t (^completion)(
              xnuupcallsv2_storageupcallsprivate_close__result_s)) {
    ...
    .create =
        ^(const uint32_t fstag, const uint64_t rootid,
          const uint8_t name[_Nonnull 256],
          tb_error_t (^completion)(
              xnuupcallsv2_storageupcallsprivate_create__result_s)) {
          return exclaves_storage_upcall_create(fstag, rootid, name,
                                                completion);
        },
    ...
    .read =
        ^(const uint32_t fstag, const uint64_t fileid,
          ...
          tb_error_t (^completion)(
              xnuupcallsv2_storageupcallsprivate_read__result_s)) {
          return exclaves_storage_upcall_read(fstag, fileid, descriptor,
                                              completion);
        },
    ...
    .write =
        ^(const uint32_t fstag, const uint64_t fileid,
          ...
          return exclaves_storage_upcall_write(fstag, fileid, descriptor,
                                               completion);
        },
    ...
    .remove =
        ^(const uint32_t fstag, const uint64_t rootid,
          const uint8_t name[_Nonnull 256],
          tb_error_t (^completion)(
              xnuupcallsv2_storageupcallsprivate_remove__result_s)) {
          return exclaves_storage_upcall_remove(fstag, rootid, name,
                                                completion);
        },

    .sync =
        ^(const uint32_t fstag, const xnuupcallsv2_syncop_s *_Nonnull op,
          const uint64_t fileid,
          tb_error_t (^completion)(
              xnuupcallsv2_storageupcallsprivate_sync__result_s)) {
          return exclaves_storage_upcall_sync(fstag, op, fileid, completion);
        },

    .readdir =
        ^(const uint32_t fstag, const uint64_t fileid, const uint64_t buf,
          const uint32_t length,
          tb_error_t (^completion)(
              xnuupcallsv2_storageupcallsprivate_readdir__result_s)) {
          return exclaves_storage_upcall_readdir(fstag, fileid, buf, length,
                                                 completion);
        },
    ...
    .queryvolumegroup =
        ^(const uint8_t vguuid[_Nonnull 37],
          tb_error_t (^completion)(
    ...
    .timerremove =
        ^(const uint64_t id, const uint32_t timerid,
          tb_error_t (^completion)(
              xnuupcallsv2_driverupcallsprivate_timerremove__result_s)) {
          return exclaves_driverkit_upcall_timer_remove(id, timerid,
                                                        completion);
        },
    ...
    .timerenable =
        ^(const uint64_t id, const uint32_t timerid,
          ...
          return exclaves_driverkit_upcall_timer_enable(id, timerid,
                                                        completion);
        },
    ...
    .timerdisable =
        ^(const uint64_t id, const uint32_t timerid,
          ...
          return exclaves_driverkit_upcall_timer_disable(id, timerid,
                                                         completion);
        },
    ...
    .timersettimeout =
        ^(const uint64_t id, const uint32_t timerid,
          ...
          return exclaves_driverkit_upcall_timer_set_timeout(
              id, timerid, duration, completion);
        },
    ...
    .timercanceltimeout =
        ^(const uint64_t id, const uint32_t timerid,
          ...
          return exclaves_driverkit_upcall_timer_cancel_timeout(id, timerid,
                                                                completion);
        },
    ...
    .asyncnotificationsignal =
        ^(const uint64_t id, const uint32_t notificationID,
          ...
          return exclaves_driverkit_upcall_async_notification_signal(
              id, notificationID, completion);
        },
    ...
    .mapperactivate =
        ^(const uint64_t id, const uint32_t mapperIndex,
          ...
          return exclaves_driverkit_upcall_mapper_activate(id, mapperIndex,
                                                           completion);
        },
    ...
    .mapperdeactivate =
        ^(const uint64_t id, const uint32_t mapperIndex,
          ...
          return exclaves_driverkit_upcall_mapper_deactivate(id, mapperIndex,
                                                             completion);
        },
    ...
    .aneworksubmit =
        ^(const uint64_t id, const uint64_t requestID,
          ...
          return exclaves_driverkit_upcall_ane_worksubmit(
              id, requestID, taskDescriptorCount, submitTimestamp, completion);
        },
    ...
    .aneworkbegin =
        ^(const uint64_t id, const uint64_t requestID,
          ...
          return exclaves_driverkit_upcall_ane_workbegin(
              id, requestID, beginTimestamp, completion);
        },
    ...
    .aneworkend =
        ^(const uint64_t id, const uint64_t requestID,
          ...
          return exclaves_driverkit_upcall_ane_workend(id, requestID,
                                                       completion);
        },
    ...
    .suspend =
        ^(const uint32_t flags,
          tb_error_t (^completion)(
              xnuupcallsv2_conclaveupcallsprivate_suspend__result_s)) {
    ...
    .stop =
        ^(const uint32_t flags,
          tb_error_t (^completion)(
              xnuupcallsv2_conclaveupcallsprivate_stop__result_s)) {
          return exclaves_conclave_upcall_stop(flags, completion);
        },
    .crashinfo =
        ^(const xnuupcallsv2_conclavesharedbuffer_s *shared_buf,
          const uint32_t length,
          tb_error_t (^completion)(
              xnuupcallsv2_conclaveupcallsprivate_crashinfo__result_s)) {
          return exclaves_conclave_upcall_crash_info(shared_buf, length,
                                                     completion);
        },
    ...
    .releasepowerassertion =
        ^(const xnuupcallsv2_assertionid_s id,
          tb_error_t (^completion)(
              xnuupcallsv2_lpwupcallsprivate_releasepowerassertion__result_s)) {
          return exclaves_driverkit_upcall_lpw_releasepowerassertion(
              id, completion);
        },
    .requestrunmode =
        ^(const xnuupcallsv2_lpwrunmode_s mode,
          tb_error_t (^completion)(
...
kern_return_t exclaves_upcall_init(void) {
  lck_mtx_assert(&exclaves_boot_lock, LCK_MTX_ASSERT_OWNED);

#ifdef EXCLAVES_OBSOLETE_UPCALL_TESTING
#if DEVELOPMENT || DEBUG
  kern_return_t kr;
  kr = exclaves_register_upcall_handler(XNUPROXY_UPCALL_HELLOUPCALL, NULL,
...
#pragma clang diagnostic ignored "-Wcast-qual" /* FIXME: rdar://103647654 */
  tb_error_t error = xnuupcalls_xnuupcalls__server_start(
      tb_upcall_ep,
  ...
  tb_error_t error2 = xnuupcallsv2_memoryupcallsprivate__server_start(
      tb_pmm_upcall_v2_ep, (xnuupcallsv2_memoryupcallsprivate__server_s
                                *)&exclaves_tightbeam_memory_upcalls_v2);
  tb_error_t error3 = xnuupcallsv2_xnuupcalls__server_start(
      tb_upcall_v2_ep,
      (xnuupcallsv2_xnuupcalls__server_s *)&exclaves_tightbeam_upcalls_v2);
#pragma clang diagnostic pop

  return (error == TB_ERROR_SUCCESS && error2 == TB_ERROR_SUCCESS &&
...
__startup_func static void initialize_exclaves_upcall_range(void) {
...
bool exclaves_upcall_in_range(uintptr_t addr, bool slid) {
  return slid ? exclaves_in_range(addr, (uintptr_t)&exclaves_upcall_start_label,
                                  (uintptr_t)&exclaves_upcall_end_label)
              : exclaves_in_range(addr, exclaves_upcall_range_start,
                                  exclaves_upcall_range_end);
}

OS_NOINLINE
kern_return_t exclaves_call_upcall_handler(exclaves_id_t upcall_id) {
...
static tb_error_t exclaves_helloupcall(const uint64_t arg,
                                       tb_error_t (^completion)(uint64_t)) {
#if DEVELOPMENT || DEBUG
  exclaves_debug_printf(show_test_output, "%s: Hello Tightbeam Upcall!\n",
                        __func__);
...
#ifdef EXCLAVES_OBSOLETE_UPCALL_TESTING
static kern_return_t exclaves_test_upcall_handler(void *context,
                                                  exclaves_tag_t *tag,
...
static int exclaves_hello_upcall_test(__unused int64_t in, int64_t *out) {
  ...
  tb_endpoint_t ep = tb_endpoint_create_with_value(TB_TRANSPORT_TYPE_XNU,
                                                   EXCLAVES_ID_HELLO_EXCLAVE_EP,
                                                   TB_ENDPOINT_OPTIONS_NONE);

  tb_result = exclaveschelloserver_tests__init(&client, ep);
  assert3u(tb_result, ==, TB_ERROR_SUCCESS);

  tb_result = exclaveschelloserver_tests_default_upcall(
      &client, request, ^(exclaveschelloserver_result_s result) {
        assert3u(tb_result, ==, TB_ERROR_SUCCESS);
