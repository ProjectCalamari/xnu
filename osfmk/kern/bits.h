...
#define bit_assign(x, b, e)                                                    \
  ((x) = (((x) & ~BIT(b))) | ((((uint64_t)(!!(e)))) << (b)))
#define bit_set(x, b) ((x) |= BIT(b))
#define bit_clear(x, b) ((x) &= ~BIT(b))
#define bit_test(x, b) ((bool)((x) & BIT(b)))

inline static uint64_t bit_ror64(uint64_t bitmap, uint n) {
  return __builtin_rotateright64(bitmap, n);
}

inline static uint64_t bit_rol64(uint64_t bitmap, uint n) {
...
inline static int bit_first(uint64_t bitmap) {
  return 63 - __builtin_clzg(bitmap, 64);
}

inline static int __bit_next(uint64_t bitmap, int previous_bit) {
  uint64_t mask = previous_bit ? mask(previous_bit) : ~0ULL;

  return bit_first(bitmap & mask);
}

/* Returns the most significant '1' bit that is less significant than
 * previous_bit, or -1 if no such bit exists.
 */
inline static int bit_next(uint64_t bitmap, int previous_bit) {
  if (previous_bit == 0) {
    return -1;
  } else {
...
inline static int lsb_first(uint64_t bitmap) {
 ...
 */
inline static int lsb_next(uint64_t bitmap, int previous_bit) {
  uint64_t mask = mask(previous_bit + 1);

  return lsb_first(bitmap & ~mask);
}

inline static int bit_count(uint64_t x) { return __builtin_popcountll(x); }

/* Return the highest power of 2 that is <= n, or -1 if n == 0 */
inline static int bit_floor(uint64_t n) { return bit_first(n); }

/* Return the lowest power of 2 that is >= n, or -1 if n == 0 */
inline static int bit_ceiling(uint64_t n) {
  if (n == 0) {
    return -1;
  }
...
inline static bool atomic_bit_set(_Atomic bitmap_t *__single map, int n,
                                  int mem_order) {
  bitmap_t prev;
...
inline static bool atomic_bit_clear(_Atomic bitmap_t *__single map, int n,
                                    int mem_order) {
  bitmap_t prev;
...
#define BITMAP_SIZE(n)                                                         \
  (                                                                            \
      size_t)(BITMAP_LEN(n)                                                    \
              << 3) /* Round to 64bit bitmap_t, then convert to bytes */
...
inline static bool bitmap_is_empty(bitmap_t *__header_indexable map,
                                   uint nbits) {
...
inline static bool bitmap_is_full(bitmap_t *__header_indexable map,
                                  uint nbits) {
  uint i;
...
inline static bitmap_t *__header_indexable bitmap_alloc(uint nbits) {
  assert(nbits > 0);
  return (bitmap_t *)kalloc_data(BITMAP_SIZE(nbits), Z_WAITOK_ZERO);
}

inline static void bitmap_free(bitmap_t *map, uint nbits) {
  assert(nbits > 0);
  kfree_data(map, BITMAP_SIZE(nbits));
}

inline static void bitmap_set(bitmap_t *__header_indexable map, uint n) {
  bit_set(map[bitmap_index(n)], bitmap_bit(n));
}

inline static void bitmap_clear(bitmap_t *__header_indexable map, uint n) {
  bit_clear(map[bitmap_index(n)], bitmap_bit(n));
}

inline static bool atomic_bitmap_set(_Atomic bitmap_t *__header_indexable map,
                                     uint n, int mem_order) {
  return atomic_bit_set(&map[bitmap_index(n)], bitmap_bit(n), mem_order);
}

inline static bool atomic_bitmap_clear(_Atomic bitmap_t *__header_indexable map,
                                       uint n, int mem_order) {
  return atomic_bit_clear(&map[bitmap_index(n)], bitmap_bit(n), mem_order);
}

inline static bool bitmap_test(const bitmap_t *__header_indexable map, uint n) {
  return bit_test(map[bitmap_index(n)], bitmap_bit(n));
}

inline static int bitmap_first(bitmap_t *__header_indexable map, uint nbits) {
...
inline static void bitmap_not(bitmap_t *__header_indexable out,
...
inline static void bitmap_and(bitmap_t *__header_indexable out,
...
inline static void bitmap_and_not(bitmap_t *__header_indexable out,
                                  const bitmap_t *__header_indexable in1,
                                  const bitmap_t *__header_indexable in2,
                                  uint nbits) {
  uint i;
...
inline static void bitmap_or(bitmap_t *__header_indexable out,
                             const bitmap_t *__header_indexable in1,
                             const bitmap_t *__header_indexable in2,
                             uint nbits) {
  for (uint i = 0; i <= bitmap_index(nbits - 1); i++) {
...
inline static bool bitmap_equal(const bitmap_t *__header_indexable in1,
                                const bitmap_t *__header_indexable in2,
                                uint nbits) {
  for (uint i = 0; i <= bitmap_index(nbits - 1); i++) {
    if (in1[i] != in2[i]) {
...
inline static int
bitmap_and_not_mask_first(bitmap_t *__header_indexable map,
                          const bitmap_t *__header_indexable mask, uint nbits) {
  for (int i = (int)bitmap_index(nbits - 1); i >= 0; i--) {
    if ((map[i] & ~mask[i]) == 0) {
      continue;
    }
...
inline static int bitmap_lsb_first(const bitmap_t *__header_indexable map,
                                   uint nbits) {
  for (uint i = 0; i <= bitmap_index(nbits - 1); i++) {
    if (map[i] == 0) {
      continue;
...
inline static int bitmap_next(const bitmap_t *__header_indexable map,
                              uint prev) {
  if (prev == 0) {
    return -1;
  }
...
inline static int bitmap_lsb_next(const bitmap_t *__header_indexable map,
                                  uint nbits, uint prev) {
  if ((prev + 1) >= nbits) {
    return -1;
  }
