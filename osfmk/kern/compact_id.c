...
#define compact_id_table_sleep(table)                                          \
  lck_mtx_sleep_with_inheritor(&(table)->cidt_lock, LCK_SLEEP_DEFAULT,         \
                               (event_t)(table), (table)->cidt_allocator,      \
                               THREAD_UNINT, TIMEOUT_WAIT_FOREVER)
#define compact_id_table_wake(table)                                           \
  wakeup_all_with_inheritor((event_t)(table), THREAD_AWAKENED)

static inline uint32_t compact_id_prev_max(uint32_t idx) {
  /*
...
static uint32_t compact_id_slab_size(uint32_t table_index) {
  if (table_index == 0) {
...
static uint32_t compact_id_slab_index(compact_id_t cid) {
  /*
...
void compact_id_table_init(compact_id_table_t table) {
  /* lck_group_init knows about what this function does */
  lck_mtx_init(&table->cidt_lock, &compact_id_lck_grp, LCK_ATTR_NULL);
}

void **compact_id_resolve(compact_id_table_t table, compact_id_t cid) {
  return table->cidt_array[compact_id_slab_index(cid)] + cid;
}

bool compact_id_slab_valid(compact_id_table_t table, compact_id_t cid) {
...
compact_id_table_grow(compact_id_table_t table, uint32_t idx) {
  ...
  table->cidt_bitmap[idx] =
      zalloc_permanent(BITMAP_SIZE(size), ZALIGN(bitmap_t));
  table->cidt_array[idx] =
      zalloc_permanent(size * sizeof(thread_t), ZALIGN_PTR);
  /*
...
compact_id_t compact_id_get_locked(compact_id_table_t table, compact_id_t limit,
                                   void *value) {
  compact_id_t cid;
  uint32_t slab_size;
  uint32_t idx = 0;
...
compact_id_t compact_id_get(compact_id_table_t table, compact_id_t limit,
                            void *value) {
  compact_id_t cid;
...
void *compact_id_put(compact_id_table_t table, compact_id_t cid) {
...
void compact_id_for_each(compact_id_table_t table, uint32_t stride,
                         bool (^cb)(void *v)) {
...
void compact_id_table_lock(compact_id_table_t table) {
  lck_mtx_lock(&table->cidt_lock);
}

void compact_id_table_unlock(compact_id_table_t table) {
