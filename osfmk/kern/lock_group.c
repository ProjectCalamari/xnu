...
void lck_grp_attr_setstat(lck_grp_attr_t *attr __unused) {
  attr->grp_attr_val |= LCK_GRP_ATTR_STAT;
}

void lck_grp_attr_free(lck_grp_attr_t *attr) { zfree(KT_LCK_GRP_ATTR, attr); }

#pragma mark lock groups

__startup_func static void lck_group_init(void) {
...
__startup_func void lck_grp_startup_init(struct lck_grp_spec *sp) {
  lck_grp_init_flags(sp->grp, sp->grp_name,
                     sp->grp_flags | lck_grp_attr_default.grp_attr_val);
}

bool lck_grp_has_stats(lck_grp_t *grp) {
...
void lck_grp_init(lck_grp_t *grp, const char *grp_name, lck_grp_attr_t *attr) {
  if (attr == LCK_GRP_ATTR_NULL) {
    attr = &lck_grp_attr_default;
  }
...
lck_grp_t *lck_grp_init_flags(lck_grp_t *grp, const char *grp_name,
                              lck_grp_options_t flags) {
...
lck_grp_t *lck_grp_resolve(uint32_t grp_attr_id) {
  grp_attr_id &= LCK_GRP_ATTR_ID_MASK;
  return *compact_id_resolve(&lck_grp_table, grp_attr_id);
}

__abortlike static void __lck_grp_assert_id_panic(lck_grp_t *grp,
...
__attribute__((always_inline)) void lck_grp_assert_id(lck_grp_t *grp,
                                                      uint32_t grp_attr_id) {
...
static void lck_grp_destroy(lck_grp_t *grp) {
  compact_id_put(&lck_grp_table, grp->lck_grp_attr_id & LCK_GRP_ATTR_ID_MASK);
  zfree(KT_LCK_GRP, grp);
}

void lck_grp_free(lck_grp_t *grp) { lck_grp_deallocate(grp, NULL); }

void lck_grp_reference(lck_grp_t *grp, uint32_t *cnt) {
  if (cnt) {
...
void lck_grp_deallocate(lck_grp_t *grp, uint32_t *cnt) {
  if (cnt) {
...
void lck_grp_foreach(bool (^block)(lck_grp_t *)) {
  compact_id_for_each(&lck_grp_table, 64, (bool (^)(void *))block);
}

void lck_grp_enable_feature(lck_debug_feature_t feat) {
...
void lck_grp_disable_feature(lck_debug_feature_t feat) {
  uint32_t bit = 1u << feat;
  long v;
...
kern_return_t host_lockgroup_info(host_t host,
  ...
  kr = kmem_alloc(ipc_kernel_map, &addr, vmsize, KMA_DATA | KMA_ZERO,
                  VM_KERN_MEMORY_IPC);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
  ...
  lck_grp_foreach(^bool(lck_grp_t *grp) {
...
#if CONFIG_DTRACE
    info[count].lock_spin_held_cnt =
        grp->lck_grp_stats.lgss_spin_held.lgs_count;
    info[count].lock_spin_miss_cnt =
        grp->lck_grp_stats.lgss_spin_miss.lgs_count;
    ...
    info[count].lock_mtx_held_cnt =
        grp->lck_grp_stats.lgss_mtx_direct_wait.lgs_count;
...
__startup_func void lck_attr_startup_init(struct lck_attr_startup_spec *sp) {
...
void lck_attr_rw_shared_priority(lck_attr_t *attr) {
  os_atomic_or(&attr->lck_attr_val, LCK_ATTR_RW_SHARED_PRIORITY, relaxed);
}

void lck_attr_free(lck_attr_t *attr) { zfree(KT_LCK_ATTR, attr); }

#pragma mark lock stat
#if CONFIG_DTRACE

void lck_grp_stat_enable(lck_grp_stat_t *stat) {
  /* callers ensure this is properly synchronized */
  stat->lgs_enablings++;
}

void lck_grp_stat_disable(lck_grp_stat_t *stat) { stat->lgs_enablings--; }

bool lck_grp_stat_enabled(lck_grp_stat_t *stat) {
...
static inline void lck_grp_inc_time_stats(lck_grp_t *grp, lck_grp_stat_t *stat,
                                          uint64_t time) {
...
void __lck_grp_spin_update_held(lck_grp_t *grp) {
  if (grp) {
    lck_grp_stat_inc(grp, &grp->lck_grp_stats.lgss_spin_held, false);
  }
}

void __lck_grp_spin_update_miss(lck_grp_t *grp) {
  if (grp) {
    lck_grp_stat_inc(grp, &grp->lck_grp_stats.lgss_spin_miss, false);
  }
}

void __lck_grp_spin_update_spin(lck_grp_t *grp, uint64_t time) {
  if (grp) {
...
void __lck_grp_ticket_update_held(lck_grp_t *grp) {
  if (grp) {
    lck_grp_stat_inc(grp, &grp->lck_grp_stats.lgss_ticket_held, false);
  }
}

void __lck_grp_ticket_update_miss(lck_grp_t *grp) {
  if (grp) {
    lck_grp_stat_inc(grp, &grp->lck_grp_stats.lgss_ticket_miss, false);
  }
}

void __lck_grp_ticket_update_spin(lck_grp_t *grp, uint64_t time) {
  if (grp) {
...
void lck_mtx_time_stat_record(enum lockstat_probe_id pid, lck_mtx_t *mtx,
                              uint32_t grp_attr_id, uint64_t start) {
