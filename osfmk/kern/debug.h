...
/*
 * mirrors the dyld_cache_header struct defined in dyld_cache_format.h from dyld
 * source code
 */
struct _dyld_cache_header {
  ...
  uint64_t codeSignatureSize;  // size of code signature blob (zero means to end
                               // of file)
  ...
  uint64_t dylibsImageGroupAddr; // (unslid) address of ImageGroup for dylibs in
                                 // this cache
  uint64_t dylibsImageGroupSize; // size of ImageGroup for dylibs in this cache
  uint64_t
      otherImageGroupAddr; // (unslid) address of ImageGroup for other OS dylibs
  uint64_t otherImageGroupSize; // size of oImageGroup for other OS dylibs
  uint64_t
      progClosuresAddr; // (unslid) address of list of program launch closures
  uint64_t progClosuresSize;     // size of list of program launch closures
  uint64_t progClosuresTrieAddr; // (unslid) address of trie of indexes into
                                 // program launch closures
  uint64_t progClosuresTrieSize; // size of trie of indexes into program launch
                                 // closures
  uint32_t platform;             // platform number (macOS=1, etc)
  uint32_t formatVersion : 8,    // dyld3::closure::kFormatVersion
      dylibsExpectedOnDisk
      : 1, // dyld should expect the dylib exists on disk and to compare
...
/*
 * mirrors the dyld_cache_image_text_info struct defined in dyld_cache_format.h
 * from dyld source code
 */
...
__options_decl(stackshot_flags_t, uint64_t,
               {
                    ...
                    *
                    * See microstackshot_flags_t whose members used to be part
                    * of this declaration.
                    */
                   ...
                   /*
                    * Kernel consumers of stackshot (via
                    * stack_snapshot_from_kernel) can ask that we try to take
                    * the stackshot lock, and fail if we don't get it.
                    */
                   ...
                   /* Report owners of and pointers to kernel objects that
                   ...
                   /* Include all Exclaves address space layouts, instead of
                      just on-core ones (default) */
...
__options_decl(
    microstackshot_flags_t, uint32_t,
    {
        /*
...
__options_closed_decl(kf_override_flag_t, uint32_t,
                      {
                          ...
                          /*
                           * Disable panics (with retaining backtraces) on
                           * leaked proc refs across syscall boundary.
                           */
                          KF_DISABLE_PROCREF_TRACKING_OVRD = 0x20000,
                      });

#define KF_SERVER_PERF_MODE_OVRD                                               \
  (KF_SERIAL_OVRD | KF_PMAPV_OVRD | KF_MATV_OVRD | KF_COMPRSV_OVRD |           \
   KF_INTERRUPT_MASKED_DEBUG_OVRD | KF_TRAPTRACE_OVRD | KF_IOTRACE_OVRD |      \
   KF_SCHED_HYGIENE_DEBUG_PMC_OVRD | KF_MACH_ASSERT_OVRD |                     \
   KF_MADVISE_FREE_DEBUG_OVRD | KF_DISABLE_PROD_TRC_VALIDATION | 0)

boolean_t kern_feature_override(kf_override_flag_t fmask);

__options_decl(
    eph_panic_flags_t, uint64_t,
    {
        EMBEDDED_PANIC_HEADER_FLAG_COREDUMP_COMPLETE =
            0x01, /* INFO: coredump completed */
        EMBEDDED_PANIC_HEADER_FLAG_STACKSHOT_SUCCEEDED =
            0x02, /* INFO: stackshot completed */
        EMBEDDED_PANIC_HEADER_FLAG_STACKSHOT_FAILED_DEBUGGERSYNC =
            0x04, /* ERROR: stackshot failed to sync with external debugger */
        EMBEDDED_PANIC_HEADER_FLAG_STACKSHOT_FAILED_ERROR =
            0x08, /* ERROR: stackshot failed */
        EMBEDDED_PANIC_HEADER_FLAG_STACKSHOT_FAILED_INCOMPLETE =
            0x10, /* ERROR: stackshot is partially complete */
        EMBEDDED_PANIC_HEADER_FLAG_STACKSHOT_FAILED_NESTED =
            0x20, /* ERROR: stackshot caused a nested panic */
        EMBEDDED_PANIC_HEADER_FLAG_NESTED_PANIC =
            0x40, /* ERROR: panic handler encountered a panic */
        EMBEDDED_PANIC_HEADER_FLAG_BUTTON_RESET_PANIC =
            0x80, /* INFO: force-reset panic: user held power button to force
                     shutdown */
        EMBEDDED_PANIC_HEADER_FLAG_COMPANION_PROC_INITIATED_PANIC =
            0x100, /* INFO: panic was triggered by a companion processor
                      (external to the SOC) */
        EMBEDDED_PANIC_HEADER_FLAG_COREDUMP_FAILED =
            0x200, /* ERROR: coredump failed to complete */
        ...
        EMBEDDED_PANIC_HEADER_FLAG_STACKSHOT_DATA_COMPRESSED =
            0x800, /* INFO: stackshot data is compressed */
        EMBEDDED_PANIC_HEADER_FLAG_ENCRYPTED_COREDUMP_SKIPPED =
            0x1000, /* ERROR: coredump policy requires encryption, but
                       encryptions is not initialized or available */
        EMBEDDED_PANIC_HEADER_FLAG_KERNEL_COREDUMP_SKIPPED_EXCLUDE_REGIONS_UNAVAILABLE =
            0x2000, /* ERROR: coredump region exclusion list is not available */
        EMBEDDED_PANIC_HEADER_FLAG_COREFILE_UNLINKED =
            0x4000, /* ERROR: coredump output file is not linked */
        EMBEDDED_PANIC_HEADER_FLAG_INCOHERENT_PANICLOG =
            0x8000, /* ERROR: paniclog integrity check failed (a warning to
                       consumer code i.e. DumpPanic) */
        EMBEDDED_PANIC_HEADER_FLAG_EXCLAVE_PANIC =
            0x10000, /* INFO: panic originated from exclaves */
        EMBEDDED_PANIC_HEADER_FLAG_USERSPACE_INITIATED_PANIC =
            0x20000, /* INFO: panic was initiated by userspace */
        EMBEDDED_PANIC_HEADER_FLAG_INTEGRATED_COPROC_INITIATED_PANIC =
            0x40000, /* INFO: panic was initiated by an SOC-integrated
                        coprocessor */
    });
 ...
 *       Any modifications should add new fields at the end, bump the version
 * number and be done alongside astris and DumpPanic changes.
 */
struct embedded_panic_header {
  uint32_t eph_magic; /* EMBEDDED_PANIC_MAGIC if valid */
  uint32_t eph_crc; /* CRC of everything following the ph_crc in the header and
                       the contents */
  uint32_t eph_version; /* embedded_panic_header version */
  eph_panic_flags_t
      eph_panic_flags; /* Flags indicating any state or relevant details */
  uint32_t eph_panic_log_offset; /* Offset of the beginning of the panic log
                                    from the beginning of the header */
  uint32_t eph_panic_log_len;    /* length of the panic log */
  uint32_t
      eph_stackshot_offset; /* Offset of the beginning of the panic stackshot
                               from the beginning of the header */
  uint32_t
      eph_stackshot_len; /* length of the panic stackshot (0 if not valid ) */
  uint32_t eph_other_log_offset; /* Offset of the other log (any logging
                                    subsequent to the stackshot) from the
                                    beginning of the header */
  uint32_t eph_other_log_len;    /* length of the other log */
  union {
    struct {
      uint64_t eph_x86_power_state : 8, eph_x86_efi_boot_state : 8,
          eph_x86_system_state : 8, eph_x86_unused_bits : 40;
    }; // anonymous struct to group the bitfields together.
    uint64_t
        eph_x86_do_not_use; /* Used for offsetof/sizeof when parsing header */
  } __attribute__((packed));
  ...
  uint64_t eph_roots_installed; /* bitmap indicating which roots are installed
                                   on this system */
...
__options_decl(
    mph_panic_flags_t, uint64_t,
    {
        ...
        MACOS_PANIC_HEADER_FLAG_USERSPACE_INITIATED_PANIC =
            0x10000, /* INFO: panic was initiated by userspace */
        MACOS_PANIC_HEADER_FLAG_INTEGRATED_COPROC_INITIATED_PANIC =
            0x20000, /* INFO: panic was initiated by an SOC-integrated
                        coprocessor */
    });

struct macos_panic_header {
  uint32_t mph_magic; /* MACOS_PANIC_MAGIC if valid */
  uint32_t mph_crc; /* CRC of everything following mph_crc in the header and the
                       contents */
  uint32_t mph_version; /* macos_panic_header version */
  uint32_t mph_padding; /* unused */
  mph_panic_flags_t
      mph_panic_flags; /* Flags indicating any state or relevant details */
  uint32_t mph_panic_log_offset; /* Offset of the panic log from the beginning
                                    of the header */
  uint32_t mph_panic_log_len;    /* length of the panic log */
  uint32_t mph_stackshot_offset; /* Offset of the panic stackshot from the
                                    beginning of the header */
  uint32_t mph_stackshot_len;    /* length of the panic stackshot */
  uint32_t mph_other_log_offset; /* Offset of the other log (any logging
                                    subsequent to the stackshot) from the
                                    beginning of the header */
  uint32_t mph_other_log_len;    /* length of the other log */
  uint64_t mph_roots_installed;  /* bitmap indicating which roots are installed
                                    on this system */
  char mph_data[]; /* panic data -- DO NOT ACCESS THIS FIELD DIRECTLY. Use the
                      offsets above relative to the beginning of the header */
} __attribute__((packed));

/*
 * Any change to the below structure should mirror the structure defined in
 * MacEFIFirmware (and vice versa)
 */
...
/*
 * EXTENDED_/DEBUG_BUF_SIZE can't grow without updates to SMC and iBoot to store
 * larger panic logs on co-processor systems
 */
#define EXTENDED_DEBUG_BUF_SIZE 0x0013ff80

#define EFI_AURR_PANIC_STRING_MAX_LEN 112
#define EFI_AURR_EXTENDED_LOG_SIZE                                             \
  (EXTENDED_DEBUG_BUF_SIZE - sizeof(struct efi_aurr_panic_header) -            \
   EFI_AURR_PANIC_STRING_MAX_LEN)
...
#define DEBUG_KPRINT_SYSCALL_MASK(mask, fmt, args...)                          \
  do {                                                                         \
    const char *dks_name = NULL;                                               \
    if (DEBUG_KPRINT_SYSCALL_PREDICATE_INTERNAL(mask, &dks_name)) {            \
      kprintf("[%s%s%p]" fmt, dks_name ? dks_name : "", dks_name ? "@" : "",   \
              current_thread(), args);                                         \
    }                                                                          \
  } while (0)
#else /* !DEBUG */
...
#define DB_REBOOT_POST_CORE                                                    \
  0x4000                      /* Attempt to reboot after                       \
...
#define DB_DISABLE_LOCAL_CORE                                                  \
  0x20000                            /* ignore local kernel core dump support */
#define DB_DISABLE_GZIP_CORE 0x40000 /* don't gzip kernel core dumps */
#define DB_DISABLE_CROSS_PANIC                                                 \
  0x80000 /* x86 only - don't trigger cross panics. Only                       \
...
#define DB_DISABLE_STACKSHOT_TO_DISK                                           \
  0x200000 /* Disable writing stackshot to local disk */
#define DB_DEBUG_IP_INIT                                                       \
  0x400000 /* iBoot specific: Allow globally enabling debug IPs during init */
#define DB_SOC_HALT_ENABLE                                                     \
  0x800000 /* iBoot specific: Enable SoC Halt during init */

/*
 * Values for a 64-bit mask that's passed to the debugger.
 */
#define DEBUGGER_OPTION_NONE 0x0ULL
#define DEBUGGER_OPTION_PANICLOGANDREBOOT                                      \
  0x1ULL /* capture a panic log and then reboot immediately */
#define DEBUGGER_OPTION_INITPROC_PANIC 0x20ULL
#define DEBUGGER_OPTION_COMPANION_PROC_INITIATED_PANIC                         \
  0x40ULL /* panic triggered by a companion processor (external to the SOC) */
#define DEBUGGER_OPTION_SKIP_LOCAL_COREDUMP                                    \
  0x80ULL /* don't try to save local coredumps for this panic */
#define DEBUGGER_OPTION_ATTEMPTCOREDUMPANDREBOOT                               \
  0x100ULL /* attempt to save coredump. always reboot */
#define DEBUGGER_INTERNAL_OPTION_THREAD_BACKTRACE                              \
  0x200ULL /* backtrace the specified thread in the paniclog (x86 only) */
#define DEBUGGER_OPTION_PRINT_CPU_USAGE_PANICLOG                               \
  0x400ULL /* print extra CPU usage data in the panic log */
#define DEBUGGER_OPTION_SKIP_PANICEND_CALLOUTS                                 \
  0x800ULL /* (bridgeOS) skip the kPEPanicEnd callouts -- don't wait for x86   \
              to finish sending panic data */
#define DEBUGGER_OPTION_SYNC_ON_PANIC_UNSAFE                                   \
  0x1000ULL /* sync() early in Panic - Can add unbounded delay, may be unsafe  \
               for some panic scenarios. Intended for userspace, watchdogs and \
               RTBuddy panics */
#define DEBUGGER_OPTION_USERSPACE_INITIATED_PANIC                              \
  0x2000ULL /* panic initiated by userspace */
#define DEBUGGER_OPTION_INTEGRATED_COPROC_INITIATED_PANIC                      \
  0x4000ULL /* panic initiated by an SOC-integrated coprocessor */
#define DEBUGGER_OPTION_USER_WATCHDOG                                          \
  0x8000ULL /* A watchdog panic caused by an unresponsive user daemon */

#define DEBUGGER_INTERNAL_OPTIONS_MASK                                         \
 ...
 * @param pagetable_mask if pagetable dumping is set in flags, the mask of page
 * table levels to dump
...
void SavePanicInfo(const char *message, void *panic_data,
                   uint64_t panic_options, const char *panic_initiator);
...
__printflike(3, 0) kern_return_t
    DebuggerTrapWithState(debugger_op db_op, const char *db_message,
                          const char *db_panic_str, va_list *db_panic_args,
                          uint64_t db_panic_options, void *db_panic_data_ptr,
...
typedef void (^leak_site_proc)(uint32_t siteCount, uint32_t elem_size,
                               uint32_t btref);

extern kern_return_t zone_leaks(const char *zoneName, uint32_t nameLen,
                                leak_site_proc proc);

extern void zone_leaks_scan(uintptr_t *instances, uint32_t count,
                            uint32_t zoneSize, uint32_t *found);

/* panic testing hooks */

#define PANIC_TEST_CASE_DISABLED 0
#define PANIC_TEST_CASE_RECURPANIC_ENTRY                                       \
  0x2 // recursive panic at panic entrypoint, before panic data structures are
      // initialized
#define PANIC_TEST_CASE_RECURPANIC_PRELOG                                      \
  0x4 // recursive panic prior to paniclog being written
#define PANIC_TEST_CASE_RECURPANIC_POSTLOG                                     \
  0x8 // recursive panic after paniclog has been written
#define PANIC_TEST_CASE_RECURPANIC_POSTCORE                                    \
  0x10 // recursive panic after corefile has been written
#define PANIC_TEST_CASE_COREFILE_IO_ERR                                        \
  0x20 // single IO error in the corefile write path
...
extern unsigned int
    panic_test_action_count; // test parameter, depends on test case

#endif /* DEBUG || DEVELOPMENT */

/*
 * A callback that reads or writes data from a given offset into the corefile.
 * It is understood that this callback should only be used from within the
 * context where it is given. It should never be stored and reused later on.
 */
typedef kern_return_t (*IOCoreFileAccessCallback)(void *context,
                                                  boolean_t write,
 ...
 *
 * Parameters:
 *  - access:            A function to call for reading/writing the kernel
 * corefile.
 *  - access_context:    The context that should be passed to the 'access'
 * function.
 *  - recipient_context: The recipient-specific context. Can be anything.
 */
typedef kern_return_t (*IOCoreFileAccessRecipient)(
    IOCoreFileAccessCallback access, void *access_context,
    void *recipient_context);

/*
 * Provides safe and temporary file-system access to the kernel corefile to the
 * given recipient callback. It does so by opening the kernel corefile, then
 * calling the 'recipient' callback, passing it an IOCoreFileAccessCallback
 * function that it can use to read/write data, then closing the kernel corefile
 * as soon as the recipient returns.
 *
 * Parameters:
 *  - recipient:         A function to call, providing it access to the kernel
 * corefile.
 *  - recipient_context: Recipient-specific context. Can be anything.
 */
extern kern_return_t
IOProvideCoreFileAccess(IOCoreFileAccessRecipient recipient,
                        void *recipient_context);
...
/*
 * Registers a new kernel (and co-processor) coredump encryption key. The key
 * format should be one of the supported "next" key formats in
 * mach_debug_types.h. The recipient context pointer should point to a
 * kdp_core_encryption_key_descriptor structure.
 *
 * Note that the given key pointer should be allocated using
 * `kmem_alloc(kernel_map, <pointer>, <size>, VM_KERN_MEMORY_DIAG)`
 *
 * Note that upon successful completion, this function will adopt the given
 * public key pointer and the caller should NOT release it.
 */
kern_return_t
