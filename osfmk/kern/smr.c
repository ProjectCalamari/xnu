...
__attribute__((always_inline, overloadable)) static inline smr_pcpu_t
...
static inline bool __smr_pcpu_queued(smr_pcpu_t pcpu) {
  return pcpu->drain_next != SMR_PCPU_NOT_QUEUED;
}

static inline void __smr_pcpu_set_not_queued(smr_pcpu_t pcpu) {
  pcpu->drain_next = SMR_PCPU_NOT_QUEUED;
}

static inline void __smr_pcpu_associate(smr_t smr, smr_pcpu_t pcpu) {
...
static inline event64_t __smrw_oncore_event(struct smr_worker *smrw) {
  return CAST_EVENT64_T(&smrw->sect_queue);
}

static inline event64_t __smrw_drain_event(struct smr_worker *smrw) {
  return CAST_EVENT64_T(&smrw->whead);
}

static inline processor_t __smrw_drain_bind_target(struct smr_worker *smrw) {
  return smrw->detach_reason ? PROCESSOR_NULL : smrw->processor;
}

static inline void __smrw_lock(struct smr_worker *smrw) {
  waitq_lock(&smrw->waitq);
}

static inline void __smrw_unlock(struct smr_worker *smrw) {
 ...
 */
static bool __smrw_wakeup_and_unlock(struct smr_worker *smrw) {
  thread_t thread;

  assert(!ml_get_interrupts_enabled());

  thread = waitq_wakeup64_identify_locked(
      &smrw->waitq, __smrw_drain_event(smrw), WAITQ_UNLOCK);

  if (thread != THREAD_NULL) {
    assert(thread == smrw->thread);

    waitq_resume_and_bind_identified_thread(
        &smrw->waitq, thread, __smrw_drain_bind_target(smrw), THREAD_AWAKENED,
...
static void __smr_call_drain(smr_node_t head) {
  smr_node_t node;
...
__startup_func void __smr_domain_init(smr_t smr) {
  smr_pcpu_t pcpu;
  vm_size_t size;
...
smr_t smr_domain_create(smr_flags_t flags, const char *name) {
  smr_pcpu_t pcpu;
  smr_t smr;
...
void smr_domain_free(smr_t smr) {
  smr_barrier(smr);
...
__attribute__((always_inline)) bool smr_entered_cpu_noblock(smr_t smr,
                                                            int cpu) {
  assert((smr->smr_flags & SMR_SLEEPABLE) == 0);
  return __smr_pcpu(smr, cpu)->c_rd_seq != SMR_SEQ_INVALID;
}

__attribute__((always_inline)) static smr_seq_t
...
__attribute__((always_inline)) static void __smr_leave(smr_pcpu_t pcpu) {
  assert(!ml_at_interrupt_context());
  /* [R2] */
  os_atomic_store(&pcpu->c_rd_seq, SMR_SEQ_INVALID, release);
}

__attribute__((always_inline)) void smr_enter(smr_t smr) {
  disable_preemption();
  __smr_enter(smr, __smr_pcpu(smr), 0);
}

__attribute__((always_inline)) void smr_leave(smr_t smr) {
  __smr_leave(__smr_pcpu(smr));
  enable_preemption();
}

void smr_enter_sleepable(smr_t smr, smr_tracker_t tracker) {
  thread_t self = current_thread();
...
__smr_wake_oncore_sleepers(struct smr_worker *smrw) {
  ...
  if (__improbable(os_atomic_load(&smrw->sect_waiter, compiler_acq_rel))) {
    if (smrq_empty(&smrw->sect_queue)) {
      ...
      waitq_wakeup64_all(&smrw->waitq, __smrw_oncore_event(smrw),
                         THREAD_AWAKENED, WAITQ_WAKEUP_DEFAULT);
    }
  }
}

void smr_ack_ipi(void) {
  /*
...
void smr_mark_active_trackers_stalled(thread_t self) {
...
__smr_leave_stalled(smr_t smr, smr_tracker_t tracker, thread_t self) {
  ...
  progress = smrq_serialized_first(&pcpu->stall_queue, struct smr_tracker,
                                   smrt_link) == tracker;
  ...
  if (progress) {
    if (!smrq_empty(&pcpu->stall_queue)) {
      first = smrq_serialized_first(&pcpu->stall_queue, struct smr_tracker,
                                    smrt_link);
  ...
  if (progress) {
    ...
    ts = turnstile_prepare((uintptr_t)pcpu, &pcpu->stall_ts, TURNSTILE_NULL,
    ...
    if (new_stall_seq == SMR_SEQ_INVALID ||
      ...
      waitq_wakeup64_all(&ts->ts_waitq, CAST_EVENT64_T(pcpu), THREAD_AWAKENED,
    ...
    turnstile_complete((uintptr_t)pcpu, &pcpu->stall_ts, NULL,
                       TURNSTILE_KERNEL_MUTEX);
  }
...
void smr_leave_sleepable(smr_t smr, smr_tracker_t tracker) {
  struct smr_worker *smrw;
  thread_t self = current_thread();
...
static inline smr_seq_t __smr_wr_advance(smr_t smr) {
  /* [W] */
  return os_atomic_add(&smr->smr_clock.s_wr_seq, SMR_SEQ_INC, release);
}

static inline bool __smr_rd_advance(smr_t smr, smr_seq_t goal,
                                    smr_seq_t rd_seq) {
  smr_seq_t o_seq;
...
__attribute__((noinline)) static smr_seq_t
__smr_wait_for_stalled(smr_pcpu_t pcpu, smr_seq_t goal) {
  ...
  if (stall_rd_seq == SMR_SEQ_INVALID || SMR_SEQ_CMP(goal, <=, stall_rd_seq)) {
    hw_lck_ticket_unlock(&pcpu->stall_lock);
    return stall_rd_seq;
  }
  ...
  inheritor = ctid_get_thread(
      smrq_serialized_first(&pcpu->stall_queue, struct smr_tracker, smrt_link)
          ->smrt_ctid);
  ...
  wr = waitq_assert_wait64(&ts->ts_waitq, CAST_EVENT64_T(pcpu), THREAD_UNINT,
...
__smr_wait_for_oncore(smr_pcpu_t pcpu, smr_seq_t goal, uint32_t cpu) {
  ...
  for (;;) {
    if (hw_spin_wait_until(&pcpu->c_rd_seq, rd_seq,
                           rd_seq == SMR_SEQ_INVALID ||
                               SMR_SEQ_CMP(goal, <=, rd_seq))) {
      return rd_seq;
    }

    if (deadline == 0) {
      clock_interval_to_deadline(smr_wait_spin_us, NSEC_PER_USEC, &deadline);
  ...
  waitq_assert_wait64(&smrw->waitq, __smrw_oncore_event(smrw), THREAD_UNINT,
                      TIMEOUT_WAIT_FOREVER);
...
__attribute__((noinline)) static bool __smr_scan(smr_t smr, smr_seq_t goal,
                                                 smr_clock_t clk, bool wait) {
  smr_delta_t delta;
  smr_seq_t rd_seq;
  ...
  if (delta == SMR_SEQ_INC) {
    ...
    if (lock_cmpxchgv(&smr->smr_clock.s_wr_seq, clk.s_wr_seq, goal,
                      &clk.s_wr_seq, relaxed)) {
      clk.s_wr_seq = goal;
    }
  } else if (delta > 0) {
    /*
...
static inline bool __smr_poll(smr_t smr, smr_seq_t goal, bool wait) {
  smr_clock_t clk;
...
smr_seq_t smr_deferred_advance(smr_t smr) {
  os_atomic_thread_fence(seq_cst);
  return SMR_SEQ_INC + os_atomic_load(&smr->smr_clock.s_wr_seq, relaxed);
}

void smr_deferred_advance_commit(smr_t smr, smr_seq_t seq) {
  /*
...
bool smr_poll(smr_t smr, smr_seq_t goal) {
...
static void __smr_worker_check_invariants(struct smr_worker *smrw) {
#if MACH_ASSERT
  ...
  for (; pcpu != *smrw->wold_tail; pcpu = pcpu->drain_next) {
    assertf(pcpu->qold_seq != SMR_SEQ_INVALID && __smr_pcpu_queued(pcpu),
  ...
  for (; pcpu != *smrw->wcur_tail; pcpu = pcpu->drain_next) {
    assertf(pcpu->qold_seq == SMR_SEQ_INVALID &&
                pcpu->qage_seq != SMR_SEQ_INVALID && __smr_pcpu_queued(pcpu),
...
static void __smr_cpu_lazy_up_if_needed(struct smr_worker *smrw) {
  if (__improbable(smrw->detach_reason != SMR_CPU_REASON_NONE)) {
    __smr_cpu_lazy_up(smrw);
  }
}

static bool __smr_call_should_advance(smr_pcpu_t pcpu) {
...
static void __smr_call_advance_qcur(smr_t smr, smr_pcpu_t pcpu,
                                    bool needs_commit) {
  smr_seq_t new_seq;
...
static void __smr_call_push(smr_pcpu_t pcpu, smr_node_t node, smr_cb_t cb) {
...
static void __smr_call_dispatch(struct smr_worker *smrw, smr_pcpu_t pcpu) {
...
void smr_call(smr_t smr, smr_node_t node, vm_size_t size, smr_cb_t cb) {
  struct smr_worker *smrw;
  smr_pcpu_t pcpu;
...
static inline event_t __smrb_event(struct smr_barrier_ctx *ctx) { return ctx; }

static void __smr_barrier_cb(struct smr_node *node) {
...
static bool __smr_barrier_drain(struct smr_worker *smrw, bool needs_commit) {
...
static void __smr_worker_drain_lock(struct smr_worker *smrw) {
  for (;;) {
...
static void __smr_worker_drain_unlock(struct smr_worker *smrw) {
  __smrw_unlock(smrw);
  ml_set_interrupts_enabled(true);
}
 ...
 */
static void __smr_worker_tick(struct smr_worker *smrw, uint64_t ctime,
                              bool wakeup) {
...
static void __smr_worker_update_wold_tail(struct smr_worker *smrw,
                                          smr_pcpu_t *new_tail) {
...
static void __smr_worker_drain_one(struct smr_worker *smrw, smr_pcpu_t pcpu) {
  ...
  *pcpu->qold_tail = NULL;
  ...
  if (pcpu->qold_seq != SMR_SEQ_INVALID) {
    ...
    *smrw->wold_tail = pcpu;
    __smr_worker_update_wold_tail(smrw, &pcpu->drain_next);
...
static void __smr_worker_continue(void *arg, wait_result_t wr __unused) {
...
#if CONFIG_QUIESCE_COUNTER
__startup_data static uint64_t _Atomic quiesce_gen_startup;
static uint64_t _Atomic *quiesce_genp = &quiesce_gen_startup;
static uint64_t _Atomic quiesce_ctime;

void cpu_quiescent_set_storage(uint64_t _Atomic *ptr) {
  /*
...
static smr_seq_t cpu_quiescent_gen_to_seq(uint64_t gen) {
  return gen * SMR_SEQ_INC + SMR_SEQ_INIT;
}

static void cpu_quiescent_advance(uint64_t gen, uint64_t ctime __kdebug_only) {
  ...
  if (lock_cmpxchg(quiesce_genp, gen, gen + 1, relaxed)) {
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_QUIESCENT_COUNTER), gen, 0, ctime,
         0);
  }
}

static void cpu_quiescent_join(struct smr_worker *smrw) {
  uint64_t gen = os_atomic_load(quiesce_genp, relaxed);

  assert(smrw->rd_quiesce_seq == SMR_SEQ_INVALID);
  os_atomic_store(&smrw->rd_quiesce_seq, cpu_quiescent_gen_to_seq(gen),
                  relaxed);
  os_atomic_thread_fence(seq_cst);
}

static void cpu_quiescent_tick(struct smr_worker *smrw, uint64_t ctime,
                               uint64_t interval) {
  uint64_t gen = os_atomic_load(quiesce_genp, relaxed);
  smr_seq_t seq = cpu_quiescent_gen_to_seq(gen);
...
static void cpu_quiescent_leave(struct smr_worker *smrw) {
  assert(smrw->rd_quiesce_seq != SMR_SEQ_INVALID);
  os_atomic_store(&smrw->rd_quiesce_seq, SMR_SEQ_INVALID, release);
}
#endif /* CONFIG_QUIESCE_COUNTER */

uint32_t smr_cpu_checkin_get_min_interval_us(void) {
  return cpu_checkin_min_interval_us;
}

void smr_cpu_checkin_set_min_interval_us(uint32_t new_value_us) {
...
__startup_func static void smr_cpu_checkin_init_min_interval_us(void) {
...
static void __smr_cpu_init_thread(struct smr_worker *smrw) {
  ...
  thread_start_in_assert_wait(th, &smrw->waitq, __smrw_drain_event(smrw),
                              THREAD_UNINT);
}

void smr_cpu_init(struct processor *processor) {
  struct smr_worker *smrw;
...
STARTUP_ARG(THREAD_CALL, STARTUP_RANK_LAST, __smr_cpu_init_thread,
            PERCPU_GET_MASTER(smr_worker));
 ...
 */
void smr_cpu_up(struct processor *processor, smr_cpu_reason_t reason) {
  struct smr_worker *smrw;
...
static void __smr_cpu_down_and_unlock(struct processor *processor,
                                      struct smr_worker *smrw,
 ...
 */
void smr_cpu_down(struct processor *processor, smr_cpu_reason_t reason) {
  struct smr_worker *smrw;
 ...
 * Called at splsched().
 */
void smr_cpu_join(struct processor *processor, uint64_t ctime __unused) {
#if CONFIG_QUIESCE_COUNTER
  struct smr_worker *smrw;
 ...
 * Called at splsched().
 */
void smr_cpu_tick(uint64_t ctime, bool safe_point) {
  struct smr_worker *smrw = PERCPU_GET(smr_worker);
 ...
 * Called at splsched().
 */
void smr_cpu_leave(struct processor *processor, uint64_t ctime) {
  struct smr_worker *smrw;
 ...
 * Called at splsched().
 */
void smr_maintenance(uint64_t ctime) {
#if CONFIG_QUIESCE_COUNTER
...
static struct smrq_slist_head *smr_hash_alloc_array(size_t size) {
  return kalloc_type(struct smrq_slist_head, size,
                     Z_WAITOK | Z_ZERO | Z_SPRAYQTN);
}

static void smr_hash_free_array(struct smrq_slist_head *array, size_t size) {
  kfree_type(struct smrq_slist_head, size, array);
}

static inline uintptr_t smr_hash_array_encode(struct smrq_slist_head *array,
                                              uint16_t order) {
  uintptr_t ptr;
...
__security_const_late static struct smrq_slist_head __smrh_empty[2] = {
    SMRQ_SLIST_INITIALIZER(__smrh_empty[0]),
    SMRQ_SLIST_INITIALIZER(__smrh_empty[1]),
};

void smr_hash_init_empty(struct smr_hash *smrh) {
  *smrh = (struct smr_hash){
      .smrh_array = smr_hash_array_encode(__smrh_empty, 63),
  };
}

bool smr_hash_is_empty_initialized(struct smr_hash *smrh) {
  return smrh->smrh_array == smr_hash_array_encode(__smrh_empty, 63);
}

void smr_hash_init(struct smr_hash *smrh, size_t size) {
  uintptr_t array;
  uint16_t shift;
  ...
  if (startup_phase >= STARTUP_SUB_LOCKDOWN) {
    assert(size * sizeof(struct smrq_slist_head) <= KALLOC_SAFE_ALLOC_SIZE);
  }
...
void smr_hash_destroy(struct smr_hash *smrh) {
  if (!smr_hash_is_empty_initialized(smrh)) {
...
void __smr_hash_serialized_clear(struct smr_hash *smrh, smrh_traits_t smrht,
                                 void (^free)(void *obj)) {
  struct smr_hash_array array = smr_hash_array_decode(smrh);
...
kern_return_t __smr_hash_shrink_and_unlock(struct smr_hash *smrh,
                                           lck_mtx_t *lock,
                                           smrh_traits_t smrht) {
   ...
   */
  os_atomic_store(&smrh->smrh_array, smr_hash_array_encode(newarray, neworder),
                  release);
...
kern_return_t __smr_hash_grow_and_unlock(struct smr_hash *smrh, lck_mtx_t *lock,
                                         smrh_traits_t smrht) {
  struct smr_hash_array decptr = smr_hash_array_decode(smrh);
  struct smrq_slist_head *newarray, *oldarray;
  ...
  prevarray =
      kalloc_type(__smrq_slink_t *, newsize, Z_WAITOK | Z_ZERO | Z_SPRAYQTN);
  if (prevarray == NULL) {
  ...
  for (size_t i = 0; i < oldsize; i++) {
    ...
    smr_serialized_store_relaxed(&oldarray[i].first, next ? link : NULL);
...
static inline size_t __smr_shash_min_size(struct smr_shash *smrh) {
  return 1ul << smrh->smrsh_min_shift;
}

static inline size_t __smr_shash_size_for_shift(uint8_t shift) {
  return (~0u >> shift) + 1;
}

static inline size_t __smr_shash_cursize(smrsh_state_t state) {
  return __smr_shash_size_for_shift(state.curshift);
}

static void __smr_shash_bucket_init(hw_lck_ptr_t *head) {
  hw_lck_ptr_init(head, __smr_shash_bucket_stop(head), &smr_shash_grp);
}

static void __smr_shash_bucket_destroy(hw_lck_ptr_t *head) {
  hw_lck_ptr_destroy(head, &smr_shash_grp);
}

__attribute__((noinline)) void *
__smr_shash_entered_find_slow(const struct smr_shash *smrh, smrh_key_t key,
                              hw_lck_ptr_t *head, smrh_traits_t traits) {
  struct smrq_slink *link;
  smrsh_state_t state;
  uint32_t hash;
...
static inline uint64_t __smr_shash_count(struct smr_shash *smrh) {
...
static inline bool __smr_shash_should_grow(struct smr_shash *smrh,
                                           smrsh_state_t state,
                                           uint64_t count) {
...
static inline bool __smr_shash_should_reseed(struct smr_shash *smrh,
                                             size_t observed_depth) {
  return observed_depth > 10 * __smr_shash_grow_ratio[smrh->smrsh_policy];
}

static inline bool __smr_shash_should_shrink(struct smr_shash *smrh,
                                             smrsh_state_t state,
                                             uint64_t count) {
  size_t size = __smr_shash_cursize(state);
...
static inline void __smr_shash_schedule_rehash(struct smr_shash *smrh,
  ...
  if (!rehash) {
    thread_call_enter1(smrh->smrsh_callout, __DECONST(void *, traits));
  }
}

void *__smr_shash_entered_get_or_insert(struct smr_shash *smrh, smrh_key_t key,
                                        struct smrq_slink *link,
                                        smrh_traits_t traits) {
  ...
  while (!__smr_shash_is_stop(other)) {
    ...
    if (traits->obj_equ(other, key)) {
      ...
      if (traits->obj_try_get(obj)) {
        hw_lck_ptr_unlock(head, first, &smr_shash_grp);
        return obj;
      }
      break;
    }
...
__abortlike static void __smr_shash_missing_elt_panic(struct smr_shash *smrh,
                                                      struct smrq_slink *link,
                                                      smrh_traits_t traits) {
...
smr_shash_mut_cursor_t __smr_shash_entered_mut_begin(struct smr_shash *smrh,
                                                     struct smrq_slink *link,
                                                     smrh_traits_t traits) {
...
void __smr_shash_entered_mut_erase(struct smr_shash *smrh,
...
void __smr_shash_entered_mut_replace(smr_shash_mut_cursor_t cursor,
...
void __smr_shash_entered_mut_abort(smr_shash_mut_cursor_t cursor) {
  hw_lck_ptr_unlock(cursor.head, hw_lck_ptr_value(cursor.head), &smr_shash_grp);
}

static kern_return_t __smr_shash_rehash_with_target(struct smr_shash *smrh,
                                                    smrsh_state_t state,
                                                    uint8_t newshift,
                                                    smrh_traits_t traits) {
  ...
  for (size_t i = 0; i < oldsize; i++) {
    ...
    while (n-- > 0) {
      for (size_t j = (n % FLAT_SIZE) + 1; j-- > 0;) {
        ...
        next = hw_lck_ptr_lock_nopreempt(head, &smr_shash_grp);
...
static void __smr_shash_rehash(thread_call_param_t arg0,
  ...
  do {
    ...
    if (kr == KERN_RESOURCE_SHORTAGE) {
      ...
      thread_call_enter1_delayed(smrh->smrsh_callout, arg1, deadline);
      break;
    }
  } while (!os_atomic_cmpxchg(&smrh->smrsh_rehashing, SMRSH_REHASH_RUNNING,
                              SMRSH_REHASH_NONE, relaxed));
}

void smr_shash_init(struct smr_shash *smrh, smrsh_policy_t policy,
                    size_t min_size) {
  smrsh_state_t state;
  hw_lck_ptr_t *array;
  uint8_t shift;
  ...
  *smrh = (struct smr_shash){
  ...
  smrh->smrsh_callout = thread_call_allocate_with_options(
      __smr_shash_rehash, smrh, THREAD_CALL_PRIORITY_KERNEL,
      THREAD_CALL_OPTIONS_ONCE);
}

void __smr_shash_destroy(struct smr_shash *smrh, smrh_traits_t traits,
                         void (^free)(void *)) {
  smrsh_state_t state;
  hw_lck_ptr_t *array;
  size_t size;
