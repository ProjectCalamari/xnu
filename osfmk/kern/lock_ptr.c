...
static inline void __hw_lck_ptr_encode(hw_lck_ptr_t *lck, const void *ptr) {
...
__abortlike static void __hw_lck_ptr_invalid_panic(hw_lck_ptr_t *lck) {
  hw_lck_ptr_t tmp = os_atomic_load(lck, relaxed);

  panic("Invalid/destroyed ptr spinlock %p: <%p %d 0x%04x>", lck,
        __hw_lck_ptr_value(tmp), tmp.lck_ptr_locked, tmp.lck_ptr_mcs_tail);
}
...
hw_lck_ptr_timeout_panic(void *_lock, hw_spin_timeout_t to,
                         hw_spin_state_t st) {
  ...
  panic("Ptr spinlock[%p] " HW_SPIN_TIMEOUT_FMT "; "
        "ptr_value: %p, mcs_tail: 0x%04x, " HW_SPIN_TIMEOUT_DETAILS_FMT,
        lck, HW_SPIN_TIMEOUT_ARG(to, st), __hw_lck_ptr_value(tmp),
        tmp.lck_ptr_mcs_tail, HW_SPIN_TIMEOUT_DETAILS_ARG(to, st));
}
...
hw_lck_ptr_contended(hw_lck_ptr_t *lck LCK_GRP_ARG(lck_grp_t *grp)) {
   ...
   */
  do {
    while (!hw_spin_wait_until(&lck->lck_ptr_value, value.lck_ptr_value,
...
#endif /* !CONFIG_DTRACE */
static void *
hw_lck_ptr_lock_slow(hw_lck_ptr_t *lck,
                     hw_lck_ptr_t tmp LCK_GRP_ARG(lck_grp_t *grp)) {
...
hw_lck_ptr_lock_fastpath(hw_lck_ptr_t *lck LCK_GRP_ARG(lck_grp_t *grp)) {
  ...
  if (__probable(tmp.lck_ptr_locked == 0 && tmp.lck_ptr_mcs_tail == 0)) {
    tmp.lck_ptr_value =
        os_atomic_or_orig(&lck->lck_ptr_value, lock_bit.lck_ptr_value, acquire);
...
hw_lck_ptr_unlock_slow(hw_lck_ptr_t *lck,
                       bool do_preempt LCK_GRP_ARG(lck_grp_t *grp)) {
  if (do_preempt) {
...
static inline void
hw_lck_ptr_unlock_fastpath(hw_lck_ptr_t *lck, void *val,
                           bool do_preempt LCK_GRP_ARG(lck_grp_t *grp)) {
  hw_lck_ptr_t curv;
  hw_lck_ptr_t xorv = {};
   ...
   */
  curv =
      atomic_load_explicit((hw_lck_ptr_t _Atomic *)lck, memory_order_relaxed);
...
static void __attribute__((noinline))
hw_lck_ptr_wait_for_value_contended(hw_lck_ptr_t *lck,
                                    void *val LCK_GRP_ARG(lck_grp_t *grp)) {
  hw_spin_policy_t pol = &hw_lck_ptr_spin_policy;
  hw_spin_timeout_t to = hw_spin_compute_timeout(pol);
  hw_spin_state_t ss = {};
  hw_lck_ptr_t tmp;
  ...
  while (
      __improbable(!hw_spin_wait_until(&lck->lck_ptr_value, tmp.lck_ptr_value,
                                       __hw_lck_ptr_value(tmp) == val))) {
    hw_spin_should_keep_spinning(lck, pol, to, &ss);
  }
