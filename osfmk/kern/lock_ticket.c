...
static_assert(HW_LCK_TICKET_LOCK_INC_WORD ==
              (HW_LCK_TICKET_LOCK_INCREMENT
               << (8 * offsetof(hw_lck_ticket_t, nticket))));
...
__header_always_inline int equal_tickets(uint8_t t0, uint8_t t1) {
  return !((t0 ^ t1) & ~HW_LCK_TICKET_LOCK_PVWAITFLAG);
}

__header_always_inline uint8_t ticket_count(uint8_t t) {
  return t & ~HW_LCK_TICKET_LOCK_PVWAITFLAG;
}

__abortlike static void __hw_lck_ticket_invalid_panic(hw_lck_ticket_t *lck) {
  panic("Invalid HW ticket lock %p <0x%08x>", lck, lck->lck_value);
}

__abortlike static void __lck_ticket_invalid_panic(lck_ticket_t *lck) {
  panic("Invalid ticket lock %p <0x%08x 0x%08x 0x%08x 0x%08x>", lck,
        *(uint32_t *)lck, lck->lck_ticket_owner, lck->tu.lck_value,
        lck->lck_ticket_padding);
}

__abortlike static void __lck_ticket_owned_panic(const lck_ticket_t *lck) {
  thread_t self = current_thread();

  panic("Ticket lock %p is unexpectedly owned by thread %p", lck, self);
}

__abortlike static void __lck_ticket_not_owned_panic(const lck_ticket_t *lck) {
  thread_t self = current_thread();

  panic("Ticket lock %p is unexpectedly not owned by thread %p", lck, self);
}

static inline void hw_lck_ticket_verify(hw_lck_ticket_t *lck) {
  if (lck->lck_type != LCK_TYPE_TICKET) {
    __hw_lck_ticket_invalid_panic(lck);
  }
}

static inline void lck_ticket_verify(lck_ticket_t *tlock) {
...
MARK_AS_HIBERNATE_TEXT void hw_lck_ticket_init(hw_lck_ticket_t *lck,
                                               lck_grp_t *grp) {
...
void hw_lck_ticket_init_locked(hw_lck_ticket_t *lck, lck_grp_t *grp) {
  assert(((uintptr_t)lck & 3) == 0);
...
MARK_AS_HIBERNATE_TEXT void lck_ticket_init(lck_ticket_t *tlock,
                                            __unused lck_grp_t *grp) {
  *tlock = (lck_ticket_t){
...
static inline void
hw_lck_ticket_destroy_internal(hw_lck_ticket_t *lck,
                               bool sync LCK_GRP_ARG(lck_grp_t *grp)) {
  __assert_only hw_lck_ticket_t tmp;

  tmp.lck_value = os_atomic_load(&lck->lck_value, relaxed);

  if (__improbable(sync && !tmp.lck_valid &&
                   !equal_tickets(tmp.nticket, tmp.cticket))) {
    /*
...
void hw_lck_ticket_destroy(hw_lck_ticket_t *lck, lck_grp_t *grp) {
  hw_lck_ticket_verify(lck);
  hw_lck_ticket_destroy_internal(lck, true LCK_GRP_ARG(grp));
}

void lck_ticket_destroy(lck_ticket_t *tlock, __unused lck_grp_t *grp) {
...
bool hw_lck_ticket_held(hw_lck_ticket_t *lck) {
  hw_lck_ticket_t tmp;
...
static inline void tlock_mark_owned(lck_ticket_t *tlock, thread_t cthread) {
  /*
   ...
   * properties of an unconditional fetch-add, and a 16 byte alignment
   * requirement.
   */
...
hw_lck_ticket_timeout_panic(void *_lock, hw_spin_timeout_t to,
                            hw_spin_state_t st) {
  ...
  if (pmap_in_ppl()) {
    panic(
        "Ticket spinlock[%p] " HW_SPIN_TIMEOUT_FMT "; "
        "cticket: 0x%x, nticket: 0x%x, valid: %d, " HW_SPIN_TIMEOUT_DETAILS_FMT,
        lck, HW_SPIN_TIMEOUT_ARG(to, st), tmp.cticket, tmp.nticket,
        tmp.lck_valid, HW_SPIN_TIMEOUT_DETAILS_ARG(to, st));
  }

  lsti = lck_spinlock_timeout_hit(lck, 0);
  panic("Ticket spinlock[%p] " HW_SPIN_TIMEOUT_FMT "; "
        "cticket: 0x%x, nticket: 0x%x, waiting for 0x%x, valid: "
        "%d, " HW_SPIN_TIMEOUT_DETAILS_FMT,
        lck, HW_SPIN_TIMEOUT_ARG(to, st), tmp.cticket, tmp.nticket, lsti->extra,
        tmp.lck_valid, HW_SPIN_TIMEOUT_DETAILS_ARG(to, st));
}

__abortlike static hw_spin_timeout_status_t
lck_ticket_timeout_panic(void *_lock, hw_spin_timeout_t to,
                         hw_spin_state_t st) {
  lck_spinlock_to_info_t lsti;
...
static inline void hw_lck_ticket_unlock_inner_pv(hw_lck_ticket_t *lck) {
  const uint8_t cticket = (uint8_t)os_atomic_add(
      &lck->cticket, HW_LCK_TICKET_LOCK_INCREMENT, acq_rel);
...
__header_always_inline void hw_lck_ticket_unlock_inner(hw_lck_ticket_t *lck) {
  ...
  atomic_store_explicit(ctp, cticket + HW_LCK_TICKET_LOCK_INCREMENT,
 ...
 */
__result_use_check static hw_lock_status_t __attribute__((noinline))
hw_lck_ticket_contended(hw_lck_ticket_t *lck,
  ...
  while (__improbable(!hw_spin_wait_until_n(
      TICKET_LOCK_SNOOP_LOOPS, &lck->lck_value, value.lck_value,
  ...
  if (__improbable(value.cticket & HW_LCK_TICKET_LOCK_PVWAITFLAG)) {
    ...
    const hw_lck_ticket_t olck = {
        ...
        .nticket = ticket_count(value.cticket) + HW_LCK_TICKET_LOCK_INCREMENT,
    };
    const hw_lck_ticket_t nlck = {
        .cticket = ticket_count(value.cticket),
        .nticket = olck.nticket,
    };
    if (os_atomic_cmpxchg(&lck->tcurnext, olck.tcurnext, nlck.tcurnext,
                          acq_rel)) {
...
lck_ticket_contended(lck_ticket_t *tlock, uint8_t mt,
                     thread_t cthread LCK_GRP_ARG(lck_grp_t *grp)) {
  ...
  (void)hw_lck_ticket_contended(&tlock->tu, arg,
                                &lck_ticket_spin_policy LCK_GRP_ARG(grp));
  lck_spinlock_timeout_set_orig_ctid(0);
  tlock_mark_owned(tlock, cthread);
}

static inline uint32_t hw_lck_ticket_reserve_orig(hw_lck_ticket_t *lck) {
  /*
   ...
   */
  return os_atomic_add_orig(&lck->lck_value, HW_LCK_TICKET_LOCK_INC_WORD,
                            acquire);
}

__header_always_inline void
hw_lck_ticket_lock_internal(hw_lck_ticket_t *lck LCK_GRP_ARG(lck_grp_t *grp)) {
  hw_lck_ticket_t tmp;
  ...
  (void)hw_lck_ticket_contended(lck, arg,
                                &hw_lck_ticket_spin_policy LCK_GRP_ARG(grp));
}

void hw_lck_ticket_lock_nopreempt(hw_lck_ticket_t *lck, lck_grp_t *grp) {
  hw_lck_ticket_lock_internal(lck LCK_GRP_ARG(grp));
}

void hw_lck_ticket_lock(hw_lck_ticket_t *lck, lck_grp_t *grp) {
  lock_disable_preemption_for_thread(current_thread());
  hw_lck_ticket_lock_internal(lck LCK_GRP_ARG(grp));
}

__header_always_inline hw_lock_status_t hw_lck_ticket_lock_to_internal(
    hw_lck_ticket_t *lck, hw_spin_policy_t pol LCK_GRP_ARG(lck_grp_t *grp)) {
  hw_lck_ticket_t tmp;
...
hw_lock_status_t hw_lck_ticket_lock_nopreempt_to(hw_lck_ticket_t *lck,
                                                 hw_spin_policy_t pol,
                                                 lck_grp_t *grp) {
  return hw_lck_ticket_lock_to_internal(lck, pol LCK_GRP_ARG(grp));
}

hw_lock_status_t hw_lck_ticket_lock_to(hw_lck_ticket_t *lck,
                                       hw_spin_policy_t pol, lck_grp_t *grp) {
  lock_disable_preemption_for_thread(current_thread());
  return hw_lck_ticket_lock_to_internal(lck, pol LCK_GRP_ARG(grp));
}

__header_always_inline void lck_ticket_lock_internal(lck_ticket_t *tlock,
                                                     thread_t cthread,
                                                     __unused lck_grp_t *grp) {
  hw_lck_ticket_t tmp;
...
void lck_ticket_lock(lck_ticket_t *tlock, __unused lck_grp_t *grp) {
...
void lck_ticket_lock_nopreempt(lck_ticket_t *tlock, __unused lck_grp_t *grp) {
  thread_t cthread = current_thread();
...
hw_lck_ticket_lock_try_internal(hw_lck_ticket_t *lck,
                                bool nopreempt LCK_GRP_ARG(lck_grp_t *grp)) {
...
bool hw_lck_ticket_lock_try(hw_lck_ticket_t *lck, lck_grp_t *grp) {
  return hw_lck_ticket_lock_try_internal(lck, false LCK_GRP_ARG(grp));
}

bool hw_lck_ticket_lock_try_nopreempt(hw_lck_ticket_t *lck, lck_grp_t *grp) {
  return hw_lck_ticket_lock_try_internal(lck, true LCK_GRP_ARG(grp));
}

__header_always_inline bool
lck_ticket_lock_try_internal(lck_ticket_t *tlock, __unused lck_grp_t *grp,
                             bool nopreempt) {
  thread_t cthread = current_thread();
  hw_lck_ticket_t olck, nlck;
  ...
  os_atomic_rmw_loop(
      &tlock->tu.tcurnext, olck.tcurnext, nlck.tcurnext, acquire, {
        if (__improbable(!equal_tickets(olck.cticket, olck.nticket))) {
          os_atomic_rmw_loop_give_up({
            if (!nopreempt) {
              lock_enable_preemption();
...
bool lck_ticket_lock_try(lck_ticket_t *tlock, __unused lck_grp_t *grp) {
  return lck_ticket_lock_try_internal(tlock, grp, false);
}

bool lck_ticket_lock_try_nopreempt(lck_ticket_t *tlock,
                                   __unused lck_grp_t *grp) {
...
bool hw_lck_ticket_reserve_nopreempt(hw_lck_ticket_t *lck, uint32_t *ticket,
                                     lck_grp_t *grp) {
  hw_lck_ticket_t tmp;
...
bool hw_lck_ticket_reserve(hw_lck_ticket_t *lck, uint32_t *ticket,
                           lck_grp_t *grp) {
  lock_disable_preemption_for_thread(current_thread());
...
hw_lock_status_t hw_lck_ticket_reserve_allow_invalid(hw_lck_ticket_t *lck,
                                                     uint32_t *ticket,
                                                     lck_grp_t *grp) {
  hw_lck_ticket_t tmp;
...
#if KASAN_TBI
  /* Expand the check to also include the tag value. See machine_routines_asm.s
   * for the details */
  tmp = hw_lck_ticket_reserve_orig_allow_invalid(
      lck, kasan_tbi_get_tag_address((vm_offset_t)lck));
#else  /* KASAN_TBI */
...
hw_lock_status_t hw_lck_ticket_wait(hw_lck_ticket_t *lck, uint32_t ticket,
                                    hw_spin_policy_t pol, lck_grp_t *grp) {
...
hw_lock_status_t hw_lck_ticket_lock_allow_invalid(hw_lck_ticket_t *lck,
                                                  hw_spin_policy_t pol,
                                                  lck_grp_t *grp) {
  hw_lock_status_t st;
  hw_lck_ticket_t tmp;
...
void hw_lck_ticket_invalidate(hw_lck_ticket_t *lck) {
  hw_lck_ticket_t tmp = {.lck_valid = 1};

  os_atomic_andnot(&lck->lck_value, tmp.lck_value, relaxed);
}

void hw_lck_ticket_unlock_nopreempt(hw_lck_ticket_t *lck) {
  HW_LCK_TICKET_VERIFY(lck);
  hw_lck_ticket_unlock_internal_nopreempt(lck);
}

void hw_lck_ticket_unlock(hw_lck_ticket_t *lck) {
  HW_LCK_TICKET_VERIFY(lck);
  hw_lck_ticket_unlock_internal(lck);
}
...
void lck_ticket_assert_owned(const lck_ticket_t *tlock) {
  if (current_thread()->ctid != tlock->lck_ticket_owner) {
    __lck_ticket_not_owned_panic(tlock);
  }
}

void lck_ticket_assert_not_owned(const lck_ticket_t *tlock) {
  if (current_thread()->ctid == tlock->lck_ticket_owner) {
    __lck_ticket_owned_panic(tlock);
  }
}
