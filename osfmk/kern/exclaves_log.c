...
static size_t oslogdarwin_logdata_data(const oslogdarwin_logdata_s *ld,
                                       uint8_t *ld_data, size_t ld_data_size) {
...
static void os_log_replay_log(const oslogdarwin_logdata_s *ld, uint8_t *ld_data,
                              size_t ld_data_size) {
...
#if DEVELOPMENT || DEBUG
    panic("ld_size:%lu was >: %lu or <=: %hu", ld_size, ld_data_size,
          ld->pubsize);
#else
  ...
  firehose_tracepoint_id_u ftid = {.ftid_value = ld->ftid};
  ...
  case firehose_tracepoint_namespace_metadata:
    ...
    if (stream != firehose_stream_metadata ||
        !os_log_encoded_metadata(ftid, ld->stamp, ld_data, ld_size)) {
      counter_inc(&oslog_e_metadata_dropped_count);
    }
    break;
  case firehose_tracepoint_namespace_log:
    counter_inc(&oslog_e_log_count);
    if (!os_log_encoded_log(stream, ftid, ld->stamp, ld_data, ld_size,
                            ld->pubsize)) {
  ...
  case firehose_tracepoint_namespace_signpost:
    ...
    if (!os_log_encoded_signpost(stream, ftid, ld->stamp, ld_data, ld_size,
                                 ld->pubsize)) {
...
static void os_log_replay_logs(const oslogdarwin_logdata_v_s *logs,
                               uint8_t *log_buffer, size_t log_buffer_size) {
  oslogdarwin_logdata__v_visit(
      logs, ^(size_t __unused i, const oslogdarwin_logdata_s *_Nonnull log) {
 ...
 */
static void log_server_retrieve_logs(__unused void *arg,
  ...
  uint8_t *log_buffer =
      kalloc_data_tag(OS_LOG_MAX_SIZE, Z_WAITOK_ZERO, VM_KERN_MEMORY_LOG);
  if (!log_buffer) {
  ...
  do {
    err = oslogdarwin_consumer_getlogs(
        &consumer, ^(oslogdarwin_logdata_v_s logs) {
...
static void replay_redacted_log(const oslogdarwin_redactedlogdata_log_s *log) {
  uuid_string_t uuidstr;
  uuid_unparse(log->uuid, uuidstr);

  os_log_at_time(OS_LOG_DEFAULT, (os_log_type_t)log->type, log->stamp,
                 "log,%s,%0x", uuidstr, log->offset);
}

static void replay_redacted_signpost(
    const oslogdarwin_redactedlogdata_signpost_s *signpost) {
  uuid_string_t uuidstr;
  uuid_unparse(signpost->uuid, uuidstr);

  os_log_at_time(OS_LOG_DEFAULT, OS_LOG_TYPE_DEBUG, signpost->stamp,
                 "signpost,%s,%0x,%0x,%u,%u", uuidstr, signpost->fmtOffset,
                 signpost->nameOffset, signpost->type, signpost->scope);
}
...
os_log_replay_redacted_logs(const oslogdarwin_redactedlogdata_v_s *logs) {
  oslogdarwin_redactedlogdata__v_visit(
      logs,
      ^(size_t __unused i, const oslogdarwin_redactedlogdata_s *_Nonnull log) {
 ...
 * and runs only when there are new logs in Exclaves to pick up and to replay.
 */
static void redacted_log_server_retrieve_logs(__unused void *arg,
                                              __unused wait_result_t w) {
  os_log_t log = os_log_create(OS_LOG_SUBSYSTEM, EXCLAVES_LOGS_CATEGORY);
  ...
  do {
    err = oslogdarwin_redactedconsumer_getlogs(
        &consumer, ^(oslogdarwin_redactedlogdata_v_s logs) {
...
kern_return_t exclaves_oslog_set_trace_mode(uint32_t mode) {
...
static bool exclaves_oslog_init_config_admin(oslogdarwin_configadmin_s *admin) {
  os_log_t log = os_log_create(OS_LOG_SUBSYSTEM, EXCLAVES_CONFIG_CATEGORY);
...
static kern_return_t exclaves_oslog_init(void) {
  if (os_log_disabled()) {
  ...
  if (EXCLAVES_ID_LOGSERVER_EP == EXCLAVES_INVALID_ID) {
    exclaves_requirement_assert(EXCLAVES_R_LOG_SERVER, "log server not found");
    return KERN_SUCCESS;
  }
  ...
  kern_return_t err =
      kernel_thread_start(log_handler, NULL, &oslog_exclaves_thread);
  if (err != KERN_SUCCESS) {
    os_log_error(log,
                 "Exclaves logging: Disabled. Failed to start retrieval thread "
                 "(error: %d)\n",
                 err);
    return KERN_FAILURE;
  }
