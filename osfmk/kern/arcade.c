...
extern int proc_pidpathinfo_internal(struct proc *p, uint64_t arg, char *buffer,
...
arcade_register_t convert_port_to_arcade_register(ipc_port_t port) {
...
ipc_port_t convert_arcade_register_to_port(arcade_register_t arcade_reg) {
...
kern_return_t arcade_register_new_upcall(arcade_register_t arcade_reg,
                                         mach_port_t port) {
...
static kern_return_t arcade_upcall_refresh(uint64_t deadline) {
  ...
  if (arcade_upcall_refresh_in_progress) {
    ...
    wr =
        lck_mtx_sleep(&arcade_upcall_mutex, LCK_SLEEP_DEFAULT,
  ...
  if (host_get_fairplayd_port(host_priv_self(), &fairplayd_port) !=
      KERN_SUCCESS) {
  ...
  wr = lck_mtx_sleep_deadline(&arcade_upcall_mutex, LCK_SLEEP_DEFAULT,
                              &arcade_upcall_port, THREAD_INTERRUPTIBLE,
                              deadline);
...
static kern_return_t __MAKING_UPCALL_TO_ARCADE_VALIDATION_SERVICE__(
    mach_port_t port, vm_map_copy_t path, vm_size_t pathlen, off_t offset,
...
void arcade_ast(__unused thread_t thread) {
  ipc_port_t port;
  uint64_t deadline;
  kern_return_t kr;
  int retval;
...
restart:
  ...
  assert(!retval);
  kr = vm_map_copyin(kernel_map, (vm_map_address_t)path, MAXPATHLEN, FALSE,
                     &copy);
  ...
  kr = __MAKING_UPCALL_TO_ARCADE_VALIDATION_SERVICE__(port, copy, MAXPATHLEN,
                                                      offset, &should_kill);
  os_log(OS_LOG_DEFAULT, "arcade: subscription validation upcall returned %#x",
         kr);
  ...
  fail:
    ...
    os_log(OS_LOG_DEFAULT,
           "arcade: unable to make subscription upcall, error %#x", kr);
    task_terminate_internal(current_task());
    break;
  }
}
