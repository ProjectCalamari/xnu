...
static thread_call_t _internal_call_allocate(thread_call_func_t func,
                                             thread_call_param_t param0);
static bool _is_internal_call(thread_call_t call);
static void _internal_call_release(thread_call_t call);
static bool _pending_call_enqueue(thread_call_t call, thread_call_group_t group,
                                  uint64_t now);
...
static void thread_call_dealloc_timer(timer_call_param_t p0,
...
static boolean_t thread_call_enter_delayed_internal(
    thread_call_t call, thread_call_func_t alt_func,
    thread_call_param_t alt_param0, thread_call_param_t param1,
    uint64_t deadline, uint64_t leeway, unsigned int flags);

/* non-static so dtrace can find it rdar://problem/31156135&31379348 */
extern void thread_call_delayed_timer(timer_call_param_t p0,
                                      timer_call_param_t p1);

LCK_GRP_DECLARE(thread_call_lck_grp, "thread_call");

static void thread_call_lock_spin(thread_call_group_t group) {
  lck_ticket_lock(&group->tcg_lock, &thread_call_lck_grp);
}

static void thread_call_unlock(thread_call_group_t group) {
  lck_ticket_unlock(&group->tcg_lock);
}

static void __assert_only thread_call_assert_locked(thread_call_group_t group) {
  lck_ticket_assert_owned(&group->tcg_lock);
}

static spl_t disable_ints_and_lock(thread_call_group_t group) {
  spl_t s = splsched();
...
static void enable_ints_and_unlock(thread_call_group_t group, spl_t s) {
  thread_call_unlock(group);
  splx(s);
}

static thread_call_group_t thread_call_get_group(thread_call_t call) {
...
/* Lock held */
static thread_call_flavor_t thread_call_get_flavor(thread_call_t call) {
  return (call->tc_flags & THREAD_CALL_FLAG_CONTINUOUS) ? TCF_CONTINUOUS
                                                        : TCF_ABSOLUTE;
}
...
static bool thread_call_enqueue_tail(thread_call_t call, queue_t new_queue) {
  ...
  if (old_queue != NULL && old_queue != &group->delayed_queues[flavor]) {
    panic("thread call (%p %p) on bad queue (old_queue: %p)", call,
          call->tc_func, old_queue);
  }
...
static queue_head_t *thread_call_dequeue(thread_call_t call) {
  queue_t old_queue = call->tc_queue;

  thread_call_group_t group = thread_call_get_group(call);
  thread_call_flavor_t flavor = thread_call_get_flavor(call);

  if (old_queue != NULL && old_queue != &group->pending_queue &&
...
static queue_head_t *thread_call_enqueue_deadline(thread_call_t call,
...
uint64_t thread_call_get_armed_deadline(thread_call_t call) {
  return call->tc_pqlink.deadline;
}

static bool group_isparallel(thread_call_group_t group) {
  return (group->tcg_flags & TCG_PARALLEL) != 0;
}

static bool thread_call_group_should_add_thread(thread_call_group_t group) {
  if ((group->active_count + group->blocked_count + group->idle_count) >=
      THREAD_CALL_GROUP_MAX_THREADS) {
    panic("thread_call group '%s' reached max thread cap (%d): active: %d, "
          "blocked: %d, idle: %d",
          group->tcg_name, THREAD_CALL_GROUP_MAX_THREADS, group->active_count,
          group->blocked_count, group->idle_count);
  }
  ...
  if (group->pending_count > 0) {
     ...
     * it's high.
     */
    if ((thread_count == 0) || (thread_count < group->target_thread_count) ||
...
static void thread_call_group_setup(thread_call_group_t group) {
  ...
  for (thread_call_flavor_t flavor = 0; flavor < TCF_COUNT; flavor++) {
    ...
    timer_call_setup(&group->delayed_timers[flavor], thread_call_delayed_timer,
                     group);
  }
  ...
  /* Reverse the wait order so we re-use the most recently parked thread from
   * the pool */
 ...
 * thread call groups.
 */
static void thread_call_thread_create(thread_call_group_t group) {
  thread_t thread;
  ...
  int group_thread_count =
      group->idle_count + group->active_count + group->blocked_count;

  snprintf(name, sizeof(name), "thread call %s #%d", group->tcg_name,
           group_thread_count);
 ...
 */
__startup_func static void thread_call_initialize(void) {
  nanotime_to_absolutetime(0, THREAD_CALL_DEALLOC_INTERVAL_NS,
  ...
  result =
      kernel_thread_start_priority((thread_continue_t)thread_call_daemon, NULL,
                                   BASEPRI_PREEMPT_HIGH + 1, &thread);
  if (result != KERN_SUCCESS) {
...
void thread_call_setup_with_options(thread_call_t call, thread_call_func_t func,
                                    thread_call_param_t param0,
...
void thread_call_setup(thread_call_t call, thread_call_func_t func,
                       thread_call_param_t param0) {
  thread_call_setup_with_options(call, func, param0, THREAD_CALL_PRIORITY_HIGH,
                                 0);
}
...
static void _internal_call_init(void) {
  ...
  for (unsigned i = 0; i < INTERNAL_CALL_COUNT; i++) {
    enqueue_tail(&thread_call_internal_queue,
                 &internal_call_storage[i].tc_qlink);
...
static bool _delayed_call_enqueue(thread_call_t call, thread_call_group_t group,
  ...
  queue_head_t *old_queue =
      thread_call_enqueue_deadline(call, group, flavor, deadline);
 ...
 * Called with thread_call_lock held.
 */
static bool _arm_delayed_call_timer(thread_call_t new_call,
                                    thread_call_group_t group,
                                    thread_call_flavor_t flavor) {
  ...
  thread_call_t call = priority_queue_min(&group->delayed_pqueues[flavor],
                                          struct thread_call, tc_pqlink);

  /* We only need to change the hard timer if this new call is the first in the
   * list */
  if (new_call != NULL && new_call != call) {
    return false;
  }

  assert((call->tc_soft_deadline != 0) &&
  ...
  /*
   * Note: This picks the soonest-deadline call's leeway as the hard timer's
   * leeway, which does not take into account later-deadline timers with a
   * larger leeway. This is a valid coalescing behavior, but masks a possible
   * window to fire a timer instead of going idle.
   */
  ...
  timer_call_enter_with_leeway(
      ...
      leeway, TIMER_CALL_SYS_CRITICAL | TIMER_CALL_LEEWAY,
 ...
 *	Called with thread_call_lock held.
 */
static boolean_t _cancel_func_from_queue(thread_call_func_t func,
                                         thread_call_param_t param0,
                                         thread_call_group_t group,
  ...
  qe_foreach_element_safe(call, queue, tc_qlink) {
    if (call->tc_func != func || call->tc_param0 != param0) {
      continue;
    }
 ...
 */
void thread_call_func_delayed(thread_call_func_t func,
                              thread_call_param_t param, uint64_t deadline) {
  (void)thread_call_enter_delayed_internal(NULL, func, param, 0, deadline, 0,
                                           0);
}
...
void thread_call_func_delayed_with_leeway(thread_call_func_t func,
                                          thread_call_param_t param,
                                          uint64_t deadline, uint64_t leeway,
                                          uint32_t flags) {
  (void)thread_call_enter_delayed_internal(NULL, func, param, 0, deadline,
                                           leeway, flags);
}
 ...
 *	which is really inefficient.  Switch to an allocated thread call
 * instead.
 *
 *	TODO: Give 'func' thread calls their own group, so this silliness
 * doesn't affect the main 'high' group.
 */
boolean_t thread_call_func_cancel(thread_call_func_t func,
                                  thread_call_param_t param,
  ...
  if (cancel_all) {
    /* exhaustively search every queue, and return true if any search found
     * something */
    result = _cancel_func_from_queue(func, param, group, cancel_all,
                                     &group->pending_queue) |
             _cancel_func_from_queue(func, param, group, cancel_all,
                                     &group->delayed_queues[TCF_ABSOLUTE]) |
             _cancel_func_from_queue(func, param, group, cancel_all,
 ...
 * THREAD_CALL_PRIORITY_HIGH or THREAD_CALL_PRIORITY_KERNEL_HIGH will be run in
 * threads with eager preemption enabled (i.e. may be aggressively preempted by
 * higher-priority threads which are not in the normal "urgent" bands).
 */
thread_call_t thread_call_allocate_with_priority(thread_call_func_t func,
                                                 thread_call_param_t param0,
                                                 thread_call_priority_t pri) {
  return thread_call_allocate_with_options(func, param0, pri, 0);
}

thread_call_t thread_call_allocate_with_options(thread_call_func_t func,
                                                thread_call_param_t param0,
                                                thread_call_priority_t pri,
                                                thread_call_options_t options) {
...
thread_call_t thread_call_allocate_with_qos(thread_call_func_t func,
                                            thread_call_param_t param0,
                                            int qos_tier,
                                            thread_call_options_t options) {
...
boolean_t thread_call_free(thread_call_t call) {
  ...
  if (refs == 0) {
    ...
    if ((call->tc_flags & THREAD_CALL_WAIT) != 0) {
      panic(
          "(%p %p) Someone waiting on a thread call that is scheduled for free",
          call, call->tc_func);
    }
    ...
    if (call->tc_finish_count != call->tc_submit_count) {
      panic("(%p %p) thread call submit/finish imbalance: %lld %lld", call,
            call->tc_func, call->tc_submit_count, call->tc_finish_count);
    }
...
boolean_t thread_call_enter1_delayed(thread_call_t call,
  ...
  return thread_call_enter_delayed_internal(call, NULL, 0, param1, deadline, 0,
                                            0);
}

boolean_t thread_call_enter_delayed_with_leeway(thread_call_t call,
                                                thread_call_param_t param1,
                                                uint64_t deadline,
                                                uint64_t leeway,
                                                unsigned int flags) {
  ...
  return thread_call_enter_delayed_internal(call, NULL, 0, param1, deadline,
                                            leeway, flags);
}
 ...
 * alt_func/alt_param0 - if call is NULL, allocate temporary storage using these
 * parameters deadline - time deadline in nanoseconds leeway   - timer slack
 * represented as delta of deadline. flags    - THREAD_CALL_DELAY_XXX :
 * classification of caller's desires wrt timer coalescing.
 *            THREAD_CALL_DELAY_LEEWAY : value in leeway is used for timer
 * coalescing. THREAD_CALL_CONTINUOUS: thread call will be called according to
 * mach_continuous_time rather than mach_absolute_time
 */
boolean_t thread_call_enter_delayed_internal(thread_call_t call,
  ...
  thread_call_flavor_t flavor =
      (flags & THREAD_CALL_CONTINUOUS) ? TCF_CONTINUOUS : TCF_ABSOLUTE;
  ...
  if (call == NULL) {
    /* allocate a structure out of internal storage, as a convenience for BSD
     * callers */
  ...
  uint64_t slop =
      timer_call_slop(deadline, now, urgency, current_thread(), &ratelimited);
 ...
 * Called with thread_call_lock held
 */
static bool thread_call_cancel_locked(thread_call_t call) {
  bool canceled;
  ...
  if (call->tc_flags & THREAD_CALL_RESCHEDULE) {
    ...
    if (call->tc_pqlink.deadline != 0 &&
        call == priority_queue_min(&group->delayed_pqueues[flavor],
                                   struct thread_call, tc_pqlink)) {
...
#if CONFIG_DTRACE
  DTRACE_TMR4(thread_callout__cancel, thread_call_func_t, call->tc_func, 0,
...
boolean_t thread_call_cancel_wait(thread_call_t call) {
  ...
  if ((call->tc_flags & THREAD_CALL_ALLOC) == 0) {
    panic("(%p %p) thread_call_cancel_wait: can't wait on thread call whose "
          "storage I don't own",
          call, call->tc_func);
  }

  if (!ml_get_interrupts_enabled()) {
    panic("(%p %p) unsafe thread_call_cancel_wait", call, call->tc_func);
  }

  thread_t self = current_thread();

  if ((thread_get_tag_internal(self) & THREAD_TAG_CALLOUT) && self->thc_state &&
      self->thc_state->thc_call == call) {
    panic("thread_call_cancel_wait: deadlock waiting on self from inside call: "
          "%p to function %p",
          call, call->tc_func);
  }
 ...
 *	Called with thread_call_lock held.
 *
 *	For high-priority group, only does wakeup/creation if there are no
 * threads running.
 */
static void thread_call_wake(thread_call_group_t group) {
  /*
  ...
  if (group_isparallel(group) || group->active_count == 0) {
    if (group->idle_count) {
      ...
      if (group->idle_count == 0 &&
          (group->tcg_flags & TCG_DEALLOC_ACTIVE) == TCG_DEALLOC_ACTIVE) {
      ...
      if (thread_call_group_should_add_thread(group) &&
          os_atomic_cmpxchg(&thread_call_daemon_awake, false, true, relaxed)) {
 ...
 */
static bool thread_call_finish(thread_call_t call, thread_call_group_t group,
                               spl_t *s) {
  thread_call_group_t call_group = thread_call_get_group(call);
  if (group != call_group) {
    panic("(%p %p) call finishing from wrong group: %p", call, call->tc_func,
          call_group);
  }
  ...
  call->tc_flags &=
      ~(THREAD_CALL_RESCHEDULE | THREAD_CALL_RUNNING | THREAD_CALL_WAIT);
 ...
 *
 * Note that the thread call object can be deallocated by the function if we do
 * not control its storage.
 */
static void __attribute__((noinline))
thread_call_invoke(thread_call_func_t func, thread_call_param_t param0,
                   thread_call_param_t param1, __unused thread_call_t call) {
#if DEVELOPMENT || DEBUG
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_CALLOUT) |
                            DBG_FUNC_START,
                        VM_KERNEL_UNSLIDE(func), VM_KERNEL_ADDRHIDE(param0),
                        VM_KERNEL_ADDRHIDE(param1), 0, 0);
#endif /* DEVELOPMENT || DEBUG */

#if CONFIG_DTRACE
  uint64_t tc_ttd = call->tc_ttd;
  boolean_t is_delayed = call->tc_flags & THREAD_CALL_DELAYED;
  DTRACE_TMR6(thread_callout__start, thread_call_func_t, func, int, 0, int,
              (tc_ttd >> 32), (unsigned)(tc_ttd & 0xFFFFFFFF), is_delayed,
              call);
#endif
...
#if CONFIG_DTRACE
  DTRACE_TMR6(thread_callout__end, thread_call_func_t, func, int, 0, int,
              (tc_ttd >> 32), (unsigned)(tc_ttd & 0xFFFFFFFF), is_delayed,
              call);
#endif
...
static void thread_call_thread(thread_call_group_t group, wait_result_t wres) {
  ...
  while (group->pending_count > 0) {
    thread_call_t call =
        qe_dequeue_head(&group->pending_queue, struct thread_call, tc_qlink);
    assert(call != NULL);
    ...
    thread_call_group_t call_group = thread_call_get_group(call);
    if (group != call_group) {
      panic("(%p %p) call on pending_queue from wrong group %p", call,
            call->tc_func, call_group);
    }
  ...
  if (group_isparallel(group)) {
    ...
    if (((group->tcg_flags & TCG_DEALLOC_ACTIVE) == 0) &&
        ((group->active_count + group->idle_count) >
         group->target_thread_count)) {
      thread_call_start_deallocate_timer(group);
    }

    /* Wait for more work (or termination) */
    wres = waitq_assert_wait64(&group->idle_waitq, CAST_EVENT64_T(group),
    ...
    if (group->idle_count < group->target_thread_count) {
      ...
      waitq_assert_wait64(&group->idle_waitq, CAST_EVENT64_T(group),
                          THREAD_UNINT, 0); /* Interrupted means to exit */
...
void thread_call_start_iotes_invocation(__assert_only thread_call_t call) {
  thread_t self = current_thread();
 ...
 */
static void thread_call_daemon_continue(__unused void *arg,
  ...
  waitq_assert_wait64(&daemon_waitq, CAST_EVENT64_T(&thread_call_daemon_awake),
                      THREAD_UNINT, 0);
...
static void thread_call_daemon(__unused void *arg, __unused wait_result_t w) {
  thread_t self = current_thread();
...
static inline uint64_t thread_call_get_time(thread_call_flavor_t flavor) {
  if (flavor == TCF_CONTINUOUS) {
...
/* non-static so dtrace can find it rdar://problem/31156135&31379348 */
void thread_call_delayed_timer(timer_call_param_t p0, timer_call_param_t p1) {
...
static void thread_call_delayed_timer_rescan(thread_call_group_t group,
                                             thread_call_flavor_t flavor) {
  thread_call_t call;
  uint64_t now;
  ...
  qe_foreach_element_safe(call, &group->delayed_queues[flavor], tc_qlink) {
    if (call->tc_soft_deadline <= now) {
       ...
       * timer in the queue.
       *
       * TODO: keep a hint on the timer to tell whether its inputs changed, so
       * we only have to crush coalescing for timers that need it.
       *
...
void thread_call_delayed_timer_rescan_all(void) {
  for (int i = THREAD_CALL_INDEX_HIGH; i < THREAD_CALL_INDEX_MAX; i++) {
    for (thread_call_flavor_t flavor = 0; flavor < TCF_COUNT; flavor++) {
...
static void thread_call_dealloc_timer(timer_call_param_t p0,
   ...
   */
  if (group->idle_count > 0 &&
      (group->idle_count + group->active_count > group->target_thread_count)) {
    /*
 ...
 *      This lets us avoid a spurious take/drop after waking up from
 * thread_block
 *
 * This thread could be a thread call thread itself, blocking and therefore
 * making a sched_call upcall into the thread call subsystem, needing the group
 * lock. However, we're saved from deadlock because the 'block' upcall is made
 * in thread_block, not in assert_wait.
 */
static bool thread_call_wait_once_locked(thread_call_t call, spl_t s) {
  ...
  wait_result_t res = waitq_assert_wait64(
      &group->waiters_waitq, CAST_EVENT64_T(call), THREAD_UNINT, 0);
 ...
 */
boolean_t thread_call_wait_once(thread_call_t call) {
  if ((call->tc_flags & THREAD_CALL_ALLOC) == 0) {
    panic("(%p %p) thread_call_wait_once: can't wait on thread call whose "
          "storage I don't own",
          call, call->tc_func);
  }
  ...
  if (!ml_get_interrupts_enabled()) {
    panic("(%p %p) unsafe thread_call_wait_once", call, call->tc_func);
  }
  ...
  if ((thread_get_tag_internal(self) & THREAD_TAG_CALLOUT) && self->thc_state &&
      self->thc_state->thc_call == call) {
    panic("thread_call_wait_once: deadlock waiting on self from inside call: "
          "%p to function %p",
          call, call->tc_func);
  }
