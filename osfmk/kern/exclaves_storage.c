...
static int verify_string_length(const char *str, size_t size) {
  return (strnlen(str, size) < size) ? 0 : ERANGE;
}

static int verify_storage_buf_offset(uint64_t buf, uint64_t length) {
  uint64_t off;
...
static int consolidate_storage_error(int error) {
  switch (error) {
...
tb_error_t exclaves_storage_upcall_legacy_root(
  ...
  if ((error = verify_string_length((const char *)&exclaveid[0], 32))) {
    xnuupcalls_xnuupcalls_root__result_init_failure(
        &result, consolidate_storage_error(error));
...
tb_error_t exclaves_storage_upcall_legacy_open(
  ...
  if ((error = verify_string_length((const char *)&name[0], 256))) {
    xnuupcalls_xnuupcalls_open__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  error = vfs_exclave_fs_open((uint32_t)fstag, rootid, (const char *)&name[0],
                              &fileid);
  if (error) {
    exclaves_debug_printf(
        show_errors,
...
tb_error_t exclaves_storage_upcall_legacy_close(
  ...
  if (error) {
    exclaves_debug_printf(
        ...
        "%d\n",
        error);
    xnuupcalls_xnuupcalls_close__result_init_failure(
        &result, consolidate_storage_error(error));
  } else {
    exclaves_debug_printf(
...
tb_error_t exclaves_storage_upcall_legacy_create(
    const enum xnuupcalls_fstag_s fstag, const uint64_t rootid,
    const uint8_t name[_Nonnull 256],
  ...
  if ((error = verify_string_length((const char *)&name[0], 256))) {
    xnuupcalls_xnuupcalls_create__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  error = vfs_exclave_fs_create((uint32_t)fstag, rootid, (const char *)&name[0],
                                &fileid);
  if (error) {
    exclaves_debug_printf(
        show_errors,
...
static bool is_restore(void) {
  bool is_restore = false;
  (void)PE_parse_boot_argn("-restore", &is_restore, sizeof(is_restore));
  return is_restore;
}

static bool dt_string_is_equal(DTEntry *entry, const char *name,
                               const char *str) {
  const void *value;
  unsigned size;
  size_t str_size;
  ...
  return entry != NULL &&
         ...
         value != NULL && size == str_size &&
         strncmp(str, value, str_size) == 0;
}

static bool is_recovery_environment(void) {
  DTEntry chosen;
...
static kern_return_t exclaves_storage_init(void) {
  ...
  kern_return_t kr = exclaves_resource_shared_memory_map(
      EXCLAVES_DOMAIN_KERNEL, v2_seg_name, STORAGE_EXCLAVE_BUF_SIZE,
      EXCLAVES_BUFFER_PERM_WRITE, &storage_resource);
  ...
  if (kr == KERN_SUCCESS) {
    ...
    storage_buffer =
        exclaves_resource_shared_memory_get_buffer(storage_resource, NULL);
    return kr;
  }

  if (kr != KERN_NOT_FOUND) {
    exclaves_debug_printf(show_errors,
                          "[storage_upcalls] Cannot map shared memory segment "
                          "'%s': failed with %d\n",
                          v2_seg_name, kr);
  }
...
tb_error_t exclaves_storage_upcall_legacy_read(
    const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
  ...
  if (error != 0) {
    xnuupcalls_xnuupcalls_read__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }

  error =
      vfs_exclave_fs_read((uint32_t)fstag, fileid, descriptor->fileoffset,
                          descriptor->length, storage_buffer + descriptor->buf);
  if (error) {
    exclaves_debug_printf(show_errors,
                          "[storage_upcalls_server] "
...
tb_error_t exclaves_storage_upcall_legacy_write(
    const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
    const struct xnuupcalls_iodesc_s *descriptor,
  ...
  error = vfs_exclave_fs_write((uint32_t)fstag, fileid, descriptor->fileoffset,
                               descriptor->length,
...
tb_error_t exclaves_storage_upcall_legacy_remove(
    const enum xnuupcalls_fstag_s fstag, const uint64_t rootid,
    const uint8_t name[_Nonnull 256],
  ...
  if ((error = verify_string_length((const char *)&name[0], 256))) {
    xnuupcalls_xnuupcalls_remove__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  error =
      vfs_exclave_fs_remove((uint32_t)fstag, rootid, (const char *)&name[0]);
  if (error) {
    exclaves_debug_printf(
        show_errors,
...
tb_error_t exclaves_storage_upcall_legacy_sync(
    const enum xnuupcalls_fstag_s fstag, const enum xnuupcalls_syncop_s op,
  ...
  if (error) {
    ...
    xnuupcalls_xnuupcalls_sync__result_init_failure(
        &result, consolidate_storage_error(error));
  } else {
    exclaves_debug_printf(
        show_storage_upcalls,
...
tb_error_t exclaves_storage_upcall_legacy_readdir(
    const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
  ...
  if ((error = verify_storage_buf_offset(buf, length))) {
    xnuupcalls_xnuupcalls_readdir__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
...
tb_error_t exclaves_storage_upcall_legacy_getsize(
    const enum xnuupcalls_fstag_s fstag, const uint64_t fileid,
  ...
  if (error) {
    exclaves_debug_printf(
        ...
        "failed with %d\n",
        fstag, fileid, error);
    xnuupcalls_xnuupcalls_getsize__result_init_failure(
        &result, consolidate_storage_error(error));
  } else {
    exclaves_debug_printf(
        show_storage_upcalls,
...
tb_error_t exclaves_storage_upcall_legacy_sealstate(
  ...
  if (error) {
    exclaves_debug_printf(
        ...
        "failed with %d\n",
        fstag, error);
    xnuupcalls_xnuupcalls_sealstate__result_init_failure(
        &result, consolidate_storage_error(error));
  } else {
    exclaves_debug_printf(
        show_storage_upcalls,
...
tb_error_t exclaves_storage_upcall_root(
    const uint8_t exclaveid[_Nonnull 32],
    tb_error_t (^completion)(
  ...
  if ((error = verify_string_length((const char *)&exclaveid[0], 32))) {
    xnuupcallsv2_storageupcallsprivate_root__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  error = vfs_exclave_fs_root((const char *)&exclaveid[0], &rootid);
  if (error) {
    ...
    exclaves_debug_printf(show_storage_upcalls,
                          ...
                          "rootId %lld\n",
                          rootid);
    xnuupcallsv2_storageupcallsprivate_root__result_init_success(&result,
                                                                 rootid);
  }
...
tb_error_t exclaves_storage_upcall_rootex(
  ...
  if ((error = verify_string_length((const char *)&exclaveid[0], 32))) {
    xnuupcallsv2_storageupcallsprivate_rootex__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  ...
  if (error) {
    ...
    exclaves_debug_printf(
        ...
        "rootId %lld\n",
        rootid);
    xnuupcallsv2_storageupcallsprivate_rootex__result_init_success(&result,
                                                                   rootid);
  }
...
tb_error_t exclaves_storage_upcall_open(
  ...
  if ((error = verify_string_length((const char *)&name[0], 256))) {
    xnuupcallsv2_storageupcallsprivate_open__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  error = vfs_exclave_fs_open(fstag, rootid, (const char *)&name[0], &fileid);
  if (error) {
    exclaves_debug_printf(
        show_errors,
        "[storage_upcalls_server] vfs_exclave_fs_open failed with %d\n", error);
    ...
    exclaves_debug_printf(show_storage_upcalls,
                          ...
                          "fileId %lld\n",
                          fileid);
    xnuupcallsv2_storageupcallsprivate_open__result_init_success(&result,
                                                                 fileid);
  }
...
tb_error_t exclaves_storage_upcall_close(
  ...
  if (error) {
    exclaves_debug_printf(
        ...
        "%d\n",
        error);
    xnuupcallsv2_storageupcallsprivate_close__result_init_failure(
        &result, consolidate_storage_error(error));
  } else {
    exclaves_debug_printf(
        show_storage_upcalls,
...
tb_error_t exclaves_storage_upcall_create(
    const uint32_t fstag, const uint64_t rootid,
    const uint8_t name[_Nonnull 256],
    tb_error_t (^completion)(
  ...
  if ((error = verify_string_length((const char *)&name[0], 256))) {
    xnuupcallsv2_storageupcallsprivate_create__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  error = vfs_exclave_fs_create(fstag, rootid, (const char *)&name[0], &fileid);
  if (error) {
    exclaves_debug_printf(
        show_errors,
        "[storage_upcalls_server] vfs_exclave_fs_create failed with"
    ...
    exclaves_debug_printf(
        ...
        "fileId %lld\n",
        fileid);
    xnuupcallsv2_storageupcallsprivate_create__result_init_success(&result,
                                                                   fileid);
  }
...
tb_error_t exclaves_storage_upcall_read(
    const uint32_t fstag, const uint64_t fileid,
  ...
  if (!storage_buffer) {
    exclaves_debug_printf(
        show_errors,
        "[storage_upcalls] shared memory buffer not initialized\n");
    xnuupcallsv2_storageupcallsprivate_read__result_init_failure(&result,
                                                                 ENOMEM);
    return completion(result);
  }

  error =
      verify_storage_buf_offset(descriptor->bufferoffset, descriptor->length);
  if (error != 0) {
    xnuupcallsv2_storageupcallsprivate_read__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }

  error = vfs_exclave_fs_read((uint32_t)fstag, fileid, descriptor->fileoffset,
                              descriptor->length,
...
tb_error_t exclaves_storage_upcall_write(
    const uint32_t fstag, const uint64_t fileid,
    const struct xnuupcallsv2_iodesc_s *descriptor,
    tb_error_t (^completion)(
  ...
  if (!storage_buffer) {
    exclaves_debug_printf(
        show_errors,
        "[storage_upcalls] shared memory buffer not initialized\n");
    xnuupcallsv2_storageupcallsprivate_write__result_init_failure(&result,
                                                                  ENOMEM);
    return completion(result);
  }
  ...
  error =
      verify_storage_buf_offset(descriptor->bufferoffset, descriptor->length);
  if (error != 0) {
    xnuupcallsv2_storageupcallsprivate_write__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
...
tb_error_t exclaves_storage_upcall_remove(
    const uint32_t fstag, const uint64_t rootid,
    const uint8_t name[_Nonnull 256],
    tb_error_t (^completion)(
  ...
  if ((error = verify_string_length((const char *)&name[0], 256))) {
    xnuupcallsv2_storageupcallsprivate_remove__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  error = vfs_exclave_fs_remove(fstag, rootid, (const char *)&name[0]);
  if (error) {
    exclaves_debug_printf(
        show_errors,
        "[storage_upcalls_server] vfs_exclave_fs_remove failed with "
...
tb_error_t exclaves_storage_upcall_sync(
    const uint32_t fstag, const xnuupcallsv2_syncop_s *op,
    const uint64_t fileid,
    tb_error_t (^completion)(
  ...
  if (error) {
    exclaves_debug_printf(show_errors,
                          "[storage_upcalls_server] vfs_exclave_fs_sync (op: "
                          "%llu) failed with %d\n",
                          _op, error);
    xnuupcallsv2_storageupcallsprivate_sync__result_init_failure(
        &result, consolidate_storage_error(error));
  } else {
    exclaves_debug_printf(
        show_storage_upcalls,
...
tb_error_t exclaves_storage_upcall_readdir(
  ...
  if (!storage_buffer) {
    exclaves_debug_printf(
        show_errors,
        "[storage_upcalls] shared memory buffer not initialized\n");
    xnuupcallsv2_storageupcallsprivate_readdir__result_init_failure(&result,
                                                                    ENOMEM);
    return completion(result);
  }

  if ((error = verify_storage_buf_offset(buf, length))) {
    xnuupcallsv2_storageupcallsprivate_readdir__result_init_failure(
        &result, consolidate_storage_error(error));
    return completion(result);
  }
  ...
  exclaves_debug_printf(show_storage_upcalls,
                        "[storage_upcalls_server] readdir succeeded\n");
  xnuupcallsv2_storageupcallsprivate_readdir__result_init_success(&result,
                                                                  count);

  return completion(result);
}

tb_error_t exclaves_storage_upcall_getsize(
    const uint32_t fstag, const uint64_t fileid,
    tb_error_t (^completion)(
  ...
  if (error) {
    exclaves_debug_printf(
        ...
        "failed with %d\n",
        fstag, fileid, error);
    xnuupcallsv2_storageupcallsprivate_getsize__result_init_failure(
        &result, consolidate_storage_error(error));
  } else {
    exclaves_debug_printf(
        show_storage_upcalls,
        "[storage_upcalls_server] vfs_exclave_fs_getsize succeeded\n");
    xnuupcallsv2_storageupcallsprivate_getsize__result_init_success(&result,
                                                                    size);
  }
...
tb_error_t exclaves_storage_upcall_sealstate(
  ...
  if (error) {
    exclaves_debug_printf(
        ...
        "failed with %d\n",
        fstag, error);
    xnuupcallsv2_storageupcallsprivate_sealstate__result_init_failure(
        &result, consolidate_storage_error(error));
  } else {
    exclaves_debug_printf(
        show_storage_upcalls,
        "[storage_upcalls_server] vfs_exclave_fs_sealstate succeeded\n");
    xnuupcallsv2_storageupcallsprivate_sealstate__result_init_success(&result,
                                                                      sealed);
  }
...
tb_error_t exclaves_storage_upcall_queryvolumegroup(
  ...
  if ((error = verify_string_length((const char *)vguuid, 37))) {
    xnuupcallsv2_storageupcallsprivate_queryvolumegroup__result_init_failure(
        &result, error);
    return completion(result);
  }
  ...
  if (error) {
    exclaves_debug_printf(show_errors,
                          "[storage_upcalls_server] "
                          "vfs_exclave_fs_queryvolumegroup failed with %d\n",
                          error);
    xnuupcallsv2_storageupcallsprivate_queryvolumegroup__result_init_failure(
        &result, error);
  } else {
    exclaves_debug_printf(
        ...
        "exists %d\n",
        exists);
    xnuupcallsv2_storageupcallsprivate_queryvolumegroup__result_init_success(
        &result, exists);
  }
