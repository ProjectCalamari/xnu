...
extern void dtrace_probe(uint32_t, uint64_t, uint64_t, uint64_t, uint64_t,
                         uint64_t);

static inline void lockprof_probe(lck_grp_t *grp, lck_grp_stat_t *stat,
                                  uint64_t val) {
  dtrace_probe(stat->lgs_probeid, (uintptr_t)grp, val, 0, 0, 0);
}

__attribute__((always_inline)) static inline void
lockstat_probe(enum lockstat_probe_id pid, const void *lock, uint64_t arg0,
...
__pure2 static inline uint32_t lockstat_enabled(void) {
...
__attribute__((always_inline, overloadable)) static inline bool
...
static inline enum lockstat_probe_id lck_mtx_acquire_probe(bool spin,
                                                           bool try_lock) {
  if (spin) {
    if (try_lock) {
...
__attribute__((cold)) __header_always_inline void
lck_mtx_prof_probe(enum lockstat_probe_id id, lck_mtx_t *mtx,
                   uint32_t grp_attr_id, bool profile) {
#pragma unused(mtx)
  if (profile) {
...
extern void lck_mtx_time_stat_record(enum lockstat_probe_id id, lck_mtx_t *mtx,
...
static inline void LCK_MTX_PROF_MISS(lck_mtx_t *mtx, uint32_t grp_attr_id,
                                     int *first_miss) {
...
static void inline LCK_MTX_PROF_WAIT(lck_mtx_t *mtx, uint32_t grp_attr_id,
                                     bool direct_wait, int *first_miss) {
  lck_grp_t *grp = lck_grp_resolve(grp_attr_id);
...
#pragma unused(lock)
#if CONFIG_DTRACE
  LOCKSTAT_RECORD(LS_LCK_SPIN_LOCK_ACQUIRE, lock,
...
static inline void
lck_grp_spin_update_spin(void *lock LCK_GRP_ARG(lck_grp_t *grp),
                         uint64_t time) {
#pragma unused(lock, time)
#if CONFIG_DTRACE
  if (time > os_atomic_load(&dtrace_spin_threshold, relaxed)) {
    LOCKSTAT_RECORD(LS_LCK_SPIN_LOCK_SPIN, lock,
                    time LCK_GRP_ARG((uintptr_t)grp));
  }
...
#pragma unused(lock)
  bool enabled =
      __lck_time_stat_enabled(LS_LCK_SPIN_LOCK_SPIN, LCK_GRP_PROBEARG(grp));
...
#pragma unused(lock)
#if CONFIG_DTRACE
  LOCKSTAT_RECORD(LS_LCK_TICKET_LOCK_ACQUIRE, lock,
                  (uintptr_t)LCK_GRP_PROBEARG(grp));
...
#pragma unused(lock)
  bool enabled =
      __lck_time_stat_enabled(LS_LCK_TICKET_LOCK_SPIN, LCK_GRP_PROBEARG(grp));
#if CONFIG_DTRACE && LOCK_STATS
  enabled |=
      (grp && lck_grp_stat_enabled(&grp->lck_grp_stats.lgss_ticket_spin));
#endif /* CONFIG_DTRACE && LOCK_STATS */
  return enabled;
}

static inline void
lck_grp_ticket_update_spin(void *lock LCK_GRP_ARG(lck_grp_t *grp),
                           uint64_t time) {
#pragma unused(lock, time)
#if CONFIG_DTRACE
  if (time > os_atomic_load(&dtrace_spin_threshold, relaxed)) {
    LOCKSTAT_RECORD(LS_LCK_TICKET_LOCK_SPIN, lock,
                    time LCK_GRP_ARG((uintptr_t)grp));
  }
...
#define LCK_MTX_BLOCK_BEGIN() lck_mtx_time_stat_begin(LS_LCK_MTX_LOCK_BLOCK)
