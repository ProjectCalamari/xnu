...
static KALLOC_TYPE_DEFINE(thread_qos_override_zone, struct thread_qos_override,
                          KT_DEFAULT);
...
static void proc_thread_qos_remove_override_internal(thread_t thread,
...
/*
 * THREAD_QOS_UNSPECIFIED is assigned the highest tier available, so it does not
 * provide a limit to threads that don't have a QoS class set.
 */
const qos_policy_params_t thread_qos_policy_params = {
    ...
    .qos_iotier[THREAD_QOS_BACKGROUND] =
        THROTTLE_LEVEL_TIER2, /* possibly overridden by bg_iotier */
    ...
    .qos_through_qos[THREAD_QOS_UNSPECIFIED] =
        QOS_EXTRACT(THROUGHPUT_QOS_TIER_UNSPECIFIED),
    .qos_through_qos[THREAD_QOS_USER_INTERACTIVE] =
        QOS_EXTRACT(THROUGHPUT_QOS_TIER_0),
    .qos_through_qos[THREAD_QOS_USER_INITIATED] =
    ...
    .qos_through_qos[THREAD_QOS_BACKGROUND] =
        QOS_EXTRACT(THROUGHPUT_QOS_TIER_5),
    .qos_through_qos[THREAD_QOS_MAINTENANCE] =
        QOS_EXTRACT(THROUGHPUT_QOS_TIER_5),

    .qos_latency_qos[THREAD_QOS_UNSPECIFIED] =
        QOS_EXTRACT(LATENCY_QOS_TIER_UNSPECIFIED),
    .qos_latency_qos[THREAD_QOS_USER_INTERACTIVE] =
        QOS_EXTRACT(LATENCY_QOS_TIER_0),
    .qos_latency_qos[THREAD_QOS_USER_INITIATED] =
...
static void thread_set_user_sched_mode_and_recompute_pri(thread_t thread,
                                                         sched_mode_t mode);

static int thread_qos_scaled_relative_priority(int qos, int qos_relprio);

static void proc_get_thread_policy_bitfield(thread_t thread,
                                            thread_policy_state_t info);

static void proc_set_thread_policy_locked(thread_t thread, int category,
                                          int flavor, int value, int value2,
                                          task_pend_token_t pend_token);

static void proc_set_thread_policy_spinlocked(thread_t thread, int category,
                                              int flavor, int value, int value2,
                                              task_pend_token_t pend_token);

static void
thread_set_requested_policy_spinlocked(thread_t thread, int category,
                                       int flavor, int value, int value2,
                                       task_pend_token_t pend_token);

static int thread_get_requested_policy_spinlocked(thread_t thread, int category,
                                                  int flavor, int *value2);

static int proc_get_thread_policy_locked(thread_t thread, int category,
                                         int flavor, int *value2);

static void thread_policy_update_spinlocked(thread_t thread,
                                            bool recompute_priority,
                                            task_pend_token_t pend_token);

static void thread_policy_update_internal_spinlocked(
    thread_t thread, bool recompute_priority, task_pend_token_t pend_token);

boolean_t thread_has_qos_policy(thread_t thread) {
  return (proc_get_thread_policy(thread, TASK_POLICY_ATTRIBUTE,
                                 TASK_POLICY_QOS) != THREAD_QOS_UNSPECIFIED)
             ? TRUE
             : FALSE;
}

static void thread_remove_qos_policy_locked(thread_t thread,
                                            task_pend_token_t pend_token) {
...
kern_return_t thread_remove_qos_policy(thread_t thread) {
...
boolean_t thread_is_static_param(thread_t thread) {
...
static int thread_qos_scaled_relative_priority(int qos, int qos_relprio) {
  ...
  int prio_range_min =
      next_lower_qos ? thread_qos_policy_params.qos_pri[next_lower_qos] : 0;

  /*
   * We now have the valid range that the scaled relative priority can map to.
   * Note that the lower bound is exclusive, but the upper bound is inclusive.
   * If the range is (21,31], 0REL should map to 31 and -15REL should map to 22.
   * We use the fact that the max relative priority is -15 and use ">>4" to
   * divide by 16 and discard remainder.
   */
  int scaled_relprio =
      -(((prio_range_max - prio_range_min) * (-qos_relprio)) >> 4);
...
kern_return_t thread_policy_set(thread_t thread, thread_policy_flavor_t flavor,
  ...
  if (!allow_qos_policy_set) {
    ...
    if (flavor == THREAD_TIME_CONSTRAINT_WITH_PRIORITY_POLICY) {
      ...
      thread_time_constraint_with_priority_policy_t info =
          (thread_time_constraint_with_priority_policy_t)policy_info;
  ...
  if (flavor == THREAD_TIME_CONSTRAINT_POLICY ||
      flavor == THREAD_TIME_CONSTRAINT_WITH_PRIORITY_POLICY) {
    thread_work_interval_flags_t th_wi_flags =
    ...
    if ((th_wi_flags & TH_WORK_INTERVAL_FLAGS_HAS_WORKLOAD_ID) &&
      ...
      /* Fail requests to become realtime for threads having joined
       * workintervals with workload ID that don't have the rt-allowed flag. */
      return KERN_INVALID_POLICY;
    }
  }

  /* Threads without static_param set reset their QoS when other policies are
   * applied. */
  ...
  if (req_qos.qos_tier != THREAD_QOS_UNSPECIFIED) {
    if (kr != KERN_SUCCESS) {
      ...
      (void)thread_policy_set_internal(thread, THREAD_QOS_POLICY,
                                       (thread_policy_t)&req_qos,
...
static_assert(offsetof(thread_time_constraint_with_priority_policy_data_t,
                       period) ==
              offsetof(thread_time_constraint_policy_data_t, period));
static_assert(offsetof(thread_time_constraint_with_priority_policy_data_t,
                       computation) ==
              offsetof(thread_time_constraint_policy_data_t, computation));
static_assert(offsetof(thread_time_constraint_with_priority_policy_data_t,
                       constraint) ==
              offsetof(thread_time_constraint_policy_data_t, constraint));
static_assert(offsetof(thread_time_constraint_with_priority_policy_data_t,
                       preemptible) ==
              offsetof(thread_time_constraint_policy_data_t, preemptible));

kern_return_t thread_policy_set_internal(thread_t thread,
  ...
  case THREAD_TIME_CONSTRAINT_WITH_PRIORITY_POLICY: {
    ...
    mach_msg_type_number_t min_count =
    ...
    if (!thread->requested_policy.thrp_wi_driven) {
      if (flavor == THREAD_TIME_CONSTRAINT_WITH_PRIORITY_POLICY) {
        thread->realtime.priority_offset =
            (uint8_t)(info->priority - BASEPRI_RTQUEUES);
      } else {
  ...
  case THREAD_BACKGROUND_POLICY: {
    ...
    int category = (current_thread() == thread) ? TASK_POLICY_INTERNAL
                                                : TASK_POLICY_EXTERNAL;

    proc_set_thread_policy_locked(thread, category, TASK_POLICY_DARWIN_BG,
                                  enable, 0, &pend_token);
  ...
  case THREAD_THROUGHPUT_QOS_POLICY: {
    thread_throughput_qos_policy_t info =
        (thread_throughput_qos_policy_t)policy_info;
    ...
    if ((result = qos_throughput_policy_validate(
             info->thread_throughput_qos_tier)) != KERN_SUCCESS) {
      break;
    }
    ...
    proc_set_thread_policy_locked(thread, TASK_POLICY_ATTRIBUTE,
                                  TASK_POLICY_THROUGH_QOS, tqos, 0,
                                  &pend_token);
  ...
  case THREAD_LATENCY_QOS_POLICY: {
    ...
    proc_set_thread_policy_locked(thread, TASK_POLICY_ATTRIBUTE,
                                  TASK_POLICY_LATENCY_QOS, lqos, 0,
                                  &pend_token);
  ...
  case THREAD_QOS_POLICY: {
    ...
    if (info->tier_importance > 0 ||
        info->tier_importance < THREAD_QOS_MIN_TIER_IMPORTANCE) {
      result = KERN_INVALID_ARGUMENT;
      break;
    }
    ...
    proc_set_thread_policy_locked(thread, TASK_POLICY_ATTRIBUTE,
                                  TASK_POLICY_QOS_AND_RELPRIO, info->qos_tier,
                                  -info->tier_importance, &pend_token);
...
/*
 * Note that there is no implemented difference between POLICY_RR and
 * POLICY_FIFO. Both result in FIXED mode scheduling.
 */
static sched_mode_t convert_policy_to_sched_mode(integer_t policy) {
  switch (policy) {
...
thread_set_mode_and_absolute_pri_internal(thread_t thread, sched_mode_t mode,
                                          integer_t priority,
                                          task_pend_token_t pend_token) {
  ...
  if (thread->base_pri != old_base_pri || thread->sched_pri != old_sched_pri) {
...
void thread_freeze_base_pri(thread_t thread) {
...
bool thread_unfreeze_base_pri(thread_t thread) {
  assert(thread == current_thread());
  integer_t base_pri;
  ast_t ast = 0;
...
uint8_t thread_workq_pri_for_qos(thread_qos_t qos) {
  assert(qos < THREAD_QOS_LAST);
  return (uint8_t)thread_qos_policy_params.qos_pri[qos];
}

thread_qos_t thread_workq_qos_for_pri(int priority) {
  thread_qos_t qos;
  if (priority >
      thread_qos_policy_params.qos_pri[THREAD_QOS_USER_INTERACTIVE]) {
 ...
 */
void thread_reset_workq_qos(thread_t thread, uint32_t qos) {
  struct task_pend_token pend_token = {};
  ...
  proc_set_thread_policy_spinlocked(thread, TASK_POLICY_ATTRIBUTE,
                                    TASK_POLICY_QOS_AND_RELPRIO, qos, 0,
                                    &pend_token);
  proc_set_thread_policy_spinlocked(thread, TASK_POLICY_ATTRIBUTE,
                                    TASK_POLICY_QOS_WORKQ_OVERRIDE,
                                    THREAD_QOS_UNSPECIFIED, 0, &pend_token);
 ...
 */
void thread_set_workq_override(thread_t thread, uint32_t qos) {
  struct task_pend_token pend_token = {};
  ...
  proc_set_thread_policy_spinlocked(thread, TASK_POLICY_ATTRIBUTE,
                                    TASK_POLICY_QOS_WORKQ_OVERRIDE, qos, 0,
                                    &pend_token);
 ...
 * Thread mutex lock is not held
 */
void thread_set_workq_pri(thread_t thread, thread_qos_t qos, integer_t priority,
                          integer_t policy) {
  struct task_pend_token pend_token = {};
 ...
 */
kern_return_t thread_set_mode_and_absolute_pri(thread_t thread,
                                               integer_t policy,
                                               integer_t priority) {
  kern_return_t kr = KERN_SUCCESS;
  struct task_pend_token pend_token = {};
 ...
 *      Only remove when it actually changes? Attempt to re-insert at
...
static void thread_update_qos_cpu_time_locked(thread_t thread) {
  ...
  /* Update the task-level effective and requested qos stats atomically, because
   * we don't have the task lock. */
  switch (thread->effective_policy.thep_qos) {
  case THREAD_QOS_UNSPECIFIED:
    task_counter = &task->cpu_time_eqos_stats.cpu_time_qos_default;
    break;
  case THREAD_QOS_MAINTENANCE:
    task_counter = &task->cpu_time_eqos_stats.cpu_time_qos_maintenance;
    break;
  case THREAD_QOS_BACKGROUND:
    task_counter = &task->cpu_time_eqos_stats.cpu_time_qos_background;
    break;
  case THREAD_QOS_UTILITY:
    task_counter = &task->cpu_time_eqos_stats.cpu_time_qos_utility;
    break;
  case THREAD_QOS_LEGACY:
    task_counter = &task->cpu_time_eqos_stats.cpu_time_qos_legacy;
    break;
  case THREAD_QOS_USER_INITIATED:
    task_counter = &task->cpu_time_eqos_stats.cpu_time_qos_user_initiated;
    break;
  case THREAD_QOS_USER_INTERACTIVE:
    task_counter = &task->cpu_time_eqos_stats.cpu_time_qos_user_interactive;
    break;
  default:
  ...
  /* Update the task-level qos stats atomically, because we don't have the task
   * lock. */
  switch (thread->requested_policy.thrp_qos) {
  case THREAD_QOS_UNSPECIFIED:
    task_counter = &task->cpu_time_rqos_stats.cpu_time_qos_default;
    break;
  case THREAD_QOS_MAINTENANCE:
    task_counter = &task->cpu_time_rqos_stats.cpu_time_qos_maintenance;
    break;
  case THREAD_QOS_BACKGROUND:
    task_counter = &task->cpu_time_rqos_stats.cpu_time_qos_background;
    break;
  case THREAD_QOS_UTILITY:
    task_counter = &task->cpu_time_rqos_stats.cpu_time_qos_utility;
    break;
  case THREAD_QOS_LEGACY:
    task_counter = &task->cpu_time_rqos_stats.cpu_time_qos_legacy;
    break;
  case THREAD_QOS_USER_INITIATED:
    task_counter = &task->cpu_time_rqos_stats.cpu_time_qos_user_initiated;
    break;
  case THREAD_QOS_USER_INTERACTIVE:
    task_counter = &task->cpu_time_rqos_stats.cpu_time_qos_user_interactive;
    break;
  default:
 ...
 * may hold task lock
 */
void thread_update_qos_cpu_time(thread_t thread) {
  thread_mtx_lock(thread);
  ...
  if (thread->sched_mode == TH_MODE_REALTIME) {
    ...
    int qos_relprio =
        -(thread->effective_policy
              .thep_qos_relprio); /* stored in task policy inverted */
...
void thread_policy_update_tasklocked(thread_t thread, integer_t priority,
  ...
  __unused integer_t old_max_priority = thread->max_priority;
   ...
   *
   * TODO: Do this inside the thread policy update routine in order to avoid
   * double remove/reinsert for a runnable thread
   */
  if ((max_priority <= MAXPRI_THROTTLE) &&
      (old_max_priority > MAXPRI_THROTTLE)) {
    sched_thread_mode_demote(thread, TH_SFLAG_THROTTLED);
  } else if ((max_priority > MAXPRI_THROTTLE) &&
             (old_max_priority <= MAXPRI_THROTTLE)) {
 ...
 */
void thread_policy_reset(thread_t thread) {
  spl_t s;
  ...
  sched_mode_t newmode =
      SCHED(initial_thread_sched_mode)(get_threadtask(thread));
...
kern_return_t thread_policy_get(thread_t thread, thread_policy_flavor_t flavor,
                                thread_policy_t policy_info,
                                mach_msg_type_number_t *count,
  ...
  case THREAD_POLICY_STATE: {
    ...
    if (!(*get_default)) {
      ...
      info->flags |=
          (thread->static_param ? THREAD_POLICY_STATE_FLAG_STATIC_PARAM : 0);

      info->thps_requested_policy =
          *(uint64_t *)(void *)(&thread->requested_policy);
      info->thps_effective_policy =
  ...
  case THREAD_REQUESTED_STATE_POLICY: {
    ...
    thread_requested_qos_policy_t info =
        (thread_requested_qos_policy_t)policy_info;
  ...
  case THREAD_LATENCY_QOS_POLICY: {
    ...
    if (*get_default) {
      ...
      plqos = proc_get_thread_policy_locked(thread, TASK_POLICY_ATTRIBUTE,
                                            TASK_POLICY_LATENCY_QOS, NULL);
    }
  ...
  case THREAD_THROUGHPUT_QOS_POLICY: {
    ...
    if (*get_default) {
      ...
      ptqos = proc_get_thread_policy_locked(thread, TASK_POLICY_ATTRIBUTE,
                                            TASK_POLICY_THROUGH_QOS, NULL);
    }
  ...
  case THREAD_QOS_POLICY: {
    ...
    if (!(*get_default)) {
      int relprio_value = 0;
      info->qos_tier = proc_get_thread_policy_locked(
          thread, TASK_POLICY_ATTRIBUTE, TASK_POLICY_QOS_AND_RELPRIO,
          &relprio_value);
...
void thread_policy_create(thread_t thread) {
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE,
      (IMPORTANCE_CODE(IMP_UPDATE,
                       (IMP_UPDATE_TASK_CREATE | TASK_POLICY_THREAD))) |
          DBG_FUNC_START,
      thread_tid(thread), theffective_0(thread), theffective_1(thread),
      thread->base_pri, 0);
...
static void thread_policy_update_spinlocked(thread_t thread,
  ...
  thread_policy_update_internal_spinlocked(thread, recompute_priority,
                                           pend_token);

  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                            (IMPORTANCE_CODE(IMP_UPDATE, TASK_POLICY_THREAD)) |
                                DBG_FUNC_END,
 ...
 *
 * Called with thread spinlock locked, task may be locked, thread mutex may be
 * locked
 */
static void thread_policy_update_internal_spinlocked(
    thread_t thread, bool recompute_priority, task_pend_token_t pend_token) {
  ...
  const struct task_effective_policy task_effective =
      get_threadtask(thread)->effective_policy;

  /*
   * Step 2:
   *  Calculate new effective policies from requested policy, task and thread
   * state Rules: Don't change requested, it won't take effect
   */
  ...
  /* A task clamp will result in an effective QoS even when requested is
   * UNSPECIFIED */
   ...
   * This allows QoS promotions to work properly even after the process is
   * unclamped.
   */
   ...
   * USER_INITIATED-13REL clamped to UTILITY), the relative priority is not
   * honored, since otherwise it would be lower than unclamped threads.
   * Similarly, in the presence of boosting, the programmer doesn't know what
   * other actors are boosting the thread.
   */
  if ((requested.thrp_qos != THREAD_QOS_UNSPECIFIED) &&
  ...
  if (next.thep_darwinbg && (next.thep_qos > THREAD_QOS_BACKGROUND ||
                             next.thep_qos == THREAD_QOS_UNSPECIFIED)) {
  ...
  if (requested.thrp_ext_iopassive || requested.thrp_int_iopassive ||
      qos_io_override_active || task_effective.tep_io_passive) {
  ...
  latency_qos =
      MAX(latency_qos, thread_qos_policy_params.qos_latency_qos[next.thep_qos]);
  ...
  through_qos =
      MAX(through_qos, thread_qos_policy_params.qos_through_qos[next.thep_qos]);
  ...
  if (prev.thep_promote_above_task != next.thep_promote_above_task) {
    KDBG_RELEASE(
        IMPORTANCE_CODE(IMP_THREAD_PROMOTE_ABOVE_TASK, 0) |
  ...
  if (prev.thep_qos != next.thep_qos ||
      ...
      pend_token->tpt_force_recompute_pri == 1 || recompute_priority) {
    thread_recompute_priority(thread);
  }

  /*
   * Check if the thread is waiting on a turnstile and needs priority
   * propagation.
   */
 ...
 */
void proc_set_thread_policy_with_tid(task_t task, uint64_t tid, int category,
                                     int flavor, int value) {
 ...
 * Precondition: No locks are held
 */
void proc_set_thread_policy(thread_t thread, int category, int flavor,
                            int value) {
  proc_set_thread_policy_ext(thread, category, flavor, value, 0);
}

void proc_set_thread_policy_ext(thread_t thread, int category, int flavor,
                                int value, int value2) {
  struct task_pend_token pend_token = {};

  thread_mtx_lock(thread);

  proc_set_thread_policy_locked(thread, category, flavor, value, value2,
                                &pend_token);
 ...
 * The only required semantic is 'call this sometime after updating effective
 * policy'
 *
 ...
 */
void thread_policy_update_complete_unlocked(thread_t thread,
                                            task_pend_token_t pend_token) {
#ifdef MACH_BSD
...
static void proc_set_thread_policy_locked(thread_t thread, int category,
  ...
  proc_set_thread_policy_spinlocked(thread, category, flavor, value, value2,
                                    pend_token);
...
static void proc_set_thread_policy_spinlocked(thread_t thread, int category,
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE,
      (IMPORTANCE_CODE(flavor, (category | TASK_POLICY_THREAD))) |
          DBG_FUNC_START,
      thread_tid(thread), threquested_0(thread), threquested_1(thread), value,
      0);

  thread_set_requested_policy_spinlocked(thread, category, flavor, value,
                                         value2, pend_token);
...
thread_set_requested_policy_spinlocked(thread_t thread, int category,
  ...
  case TASK_POLICY_QOS_AND_RELPRIO:
    ...
    pend_token->tpt_update_turnstile = 1;
    DTRACE_BOOST3(qos_set, uint64_t, thread->thread_id, int, requested.thrp_qos,
                  int, requested.thrp_qos_relprio);
    break;
  ...
  case TASK_POLICY_WI_DRIVEN:
    ...
    if (mode == TH_MODE_REALTIME) {
      ...
      thread->realtime.priority_offset =
          set_policy ? (uint8_t)(pri - BASEPRI_RTQUEUES) : 0;
    } else {
      sched_set_thread_mode_user(thread, mode);
      if (set_policy) {
 ...
 * Precondition: No locks are held
 */
int proc_get_thread_policy(thread_t thread, int category, int flavor) {
  int value = 0;
  thread_mtx_lock(thread);
...
static int proc_get_thread_policy_locked(thread_t thread, int category,
  ...
  value =
      thread_get_requested_policy_spinlocked(thread, category, flavor, value2);
 ...
 * invoked within the context of a timer interrupt.
 *
 * TODO: I think we can get away with architecting this such that we don't need
 * to look at the task ever. Is that a good idea? Maybe it's best to avoid
 * evaluate-all-the-threads updates. I don't think that cost is worth not having
 * the right answer.
 */
int proc_get_effective_thread_policy(thread_t thread, int flavor) {
  ...
  case TASK_POLICY_IO:
    /*
     * The I/O system calls here to find out what throttling tier to apply to an
     * operation. Returns THROTTLE_LEVEL_* values
     */
  ...
  case TASK_POLICY_PASSIVE_IO:
    /*
     * The I/O system calls here to find out whether an operation should be
     * passive. (i.e. not cause operations with lower throttle tiers to be
     * throttled) Returns 1 for passive mode, 0 for normal mode
     *
  ...
  case TASK_POLICY_ALL_SOCKETS_BG:
    ...
    value = (thread->effective_policy.thep_all_sockets_bg ||
             get_threadtask(thread)->effective_policy.tep_all_sockets_bg)
                ? 1
                : 0;
    break;
  case TASK_POLICY_NEW_SOCKETS_BG:
    /*
     * socreate() calls this to determine if it should mark a new socket as
     * background Returns 1 for background mode, 0 for normal mode
     */
...
static void proc_get_thread_policy_bitfield(thread_t thread,
  ...
  bits |= (requested.thrp_int_iotier ? (((uint64_t)requested.thrp_int_iotier)
                                        << POLICY_REQ_INT_IO_TIER_SHIFT)
                                     : 0);
  bits |= (requested.thrp_ext_iotier ? (((uint64_t)requested.thrp_ext_iotier)
  ...
  bits |= (requested.thrp_qos
               ? (((uint64_t)requested.thrp_qos) << POLICY_REQ_TH_QOS_SHIFT)
               : 0);
  bits |=
      (requested.thrp_qos_override ? (((uint64_t)requested.thrp_qos_override)
                                      << POLICY_REQ_TH_QOS_OVER_SHIFT)
                                   : 0);

  bits |= (requested.thrp_pidbind_bg ? POLICY_REQ_PIDBIND_BG : 0);

  bits |= (requested.thrp_latency_qos ? (((uint64_t)requested.thrp_latency_qos)
                                         << POLICY_REQ_BASE_LATENCY_QOS_SHIFT)
                                      : 0);
  bits |= (requested.thrp_through_qos ? (((uint64_t)requested.thrp_through_qos)
  ...
  bits |= (effective.thep_io_tier ? (((uint64_t)effective.thep_io_tier)
  ...
  bits |= (effective.thep_qos
               ? (((uint64_t)effective.thep_qos) << POLICY_EFF_TH_QOS_SHIFT)
               : 0);

  bits |= (effective.thep_latency_qos ? (((uint64_t)effective.thep_latency_qos)
                                         << POLICY_EFF_LATENCY_QOS_SHIFT)
                                      : 0);
  bits |= (effective.thep_through_qos ? (((uint64_t)effective.thep_through_qos)
 ...
 * We do have room on LP64. On LP32, we have to split it between two
 * uintptr_t's.
 *
 *                                LP32            LP64
...
uintptr_t threquested_0(thread_t thread) {
  static_assert(sizeof(struct thread_requested_policy) == sizeof(uint64_t),
...
uintptr_t theffective_0(thread_t thread) {
  static_assert(sizeof(struct thread_effective_policy) == sizeof(uint64_t),
                "size invariant violated");
...
void set_thread_iotier_override(thread_t thread, int policy) {
  ...
  do {
    ...
    if (current_override == policy) {
      ...
      !OSCompareAndSwap(current_override, policy, &thread->iotier_override));
...
/*
 * Userspace synchronization routines (like pthread mutexes, pthread
 * reader-writer locks, semaphores, dispatch_sync) may result in priority
 * inversions where a higher priority (i.e. scheduler priority, I/O tier, QoS
 * tier) is waiting on a resource owned by a lower priority thread. In these
 * cases, we attempt to propagate the priority token, as long as the subsystem
 * informs us of the relationships between the threads. The userspace
 * synchronization subsystem should maintain the information of owner->resource
 * and resource->waiters itself.
 */

/*
 * This helper canonicalizes the resource/resource_type given the current
 * qos_override_mode in effect. Note that wildcards
 * (THREAD_QOS_OVERRIDE_RESOURCE_WILDCARD) may need to be handled specially in
 * the future, but for now it's fine to slam *resource to USER_ADDR_NULL even if
 * it was previously a wildcard.
 */
static void canonicalize_resource_and_type(user_addr_t *resource,
                                           int *resource_type) {
  if (qos_override_mode == QOS_OVERRIDE_MODE_OVERHANG_PEAK ||
      qos_override_mode == QOS_OVERRIDE_MODE_IGNORE_OVERRIDE) {
...
/* This helper routine finds an existing override if known. Locking should be
 * done by caller */
static struct thread_qos_override *
find_qos_override(thread_t thread, user_addr_t resource, int resource_type) {
...
static void find_and_decrement_qos_override(
    thread_t thread, user_addr_t resource, int resource_type, boolean_t reset,
  ...
  while (override) {
    ...
    if ((THREAD_QOS_OVERRIDE_RESOURCE_WILDCARD == resource ||
        ...
        (THREAD_QOS_OVERRIDE_TYPE_WILDCARD == resource_type ||
         override->override_resource_type == resource_type)) {
      if (reset) {
...
/* This helper recalculates the current requested override using the policy
 * selected at boot */
static int calculate_requested_qos_override(thread_t thread) {
 ...
 */
static int
proc_thread_qos_add_override_internal(thread_t thread, int override_qos,
  ...
  KERNEL_DEBUG_CONSTANT(
      (IMPORTANCE_CODE(IMP_USYNCH_QOS_OVERRIDE, IMP_USYNCH_ADD_OVERRIDE)) |
          DBG_FUNC_START,
      thread_tid(thread), override_qos, first_override_for_resource ? 1 : 0, 0,
      0);

  DTRACE_BOOST5(qos_add_override_pre, uint64_t, thread_tid(thread), uint64_t,
                thread->requested_policy.thrp_qos, uint64_t,
                thread->effective_policy.thep_qos, int, override_qos, boolean_t,
  ...
  if (override) {
    if (override->override_qos == THREAD_QOS_UNSPECIFIED) {
      ...
      override->override_qos =
          MAX(override->override_qos, (int16_t)override_qos);
    }
  }
  ...
  prev_qos_override = proc_get_thread_policy_locked(
      thread, TASK_POLICY_ATTRIBUTE, TASK_POLICY_QOS_OVERRIDE, NULL);

  if (new_qos_override != prev_qos_override) {
    proc_set_thread_policy_locked(thread, TASK_POLICY_ATTRIBUTE,
                                  TASK_POLICY_QOS_OVERRIDE, new_qos_override, 0,
                                  &pend_token);
  }
  ...
  DTRACE_BOOST4(qos_add_override_post, int, prev_qos_override, int,
                new_qos_override, int, new_effective_qos, int, rc);
...
int proc_thread_qos_add_override(task_t task, thread_t thread, uint64_t tid,
                                 int override_qos,
                                 boolean_t first_override_for_resource,
                                 user_addr_t resource, int resource_type) {
...
static void proc_thread_qos_remove_override_internal(thread_t thread,
  ...
  find_and_decrement_qos_override(thread, resource, resource_type, reset,
                                  &deferred_free_override_list);

  KERNEL_DEBUG_CONSTANT(
      (IMPORTANCE_CODE(IMP_USYNCH_QOS_OVERRIDE, IMP_USYNCH_REMOVE_OVERRIDE)) |
          DBG_FUNC_START,
      thread_tid(thread), resource, reset, 0, 0);

  DTRACE_BOOST3(qos_remove_override_pre, uint64_t, thread_tid(thread), uint64_t,
                thread->requested_policy.thrp_qos, uint64_t,
                thread->effective_policy.thep_qos);
  ...
  /*
   * The override chain and therefore the value of the current override is
   * locked with thread mutex, so we can do a get/set without races.  However,
   * the rest of thread policy is locked under the spinlock. This means you
   * can't change the current override from a spinlock-only setter.
   */
  prev_qos_override = thread_get_requested_policy_spinlocked(
      thread, TASK_POLICY_ATTRIBUTE, TASK_POLICY_QOS_OVERRIDE, NULL);
  ...
  DTRACE_BOOST3(qos_remove_override_post, int, prev_qos_override, int,
                new_qos_override, int, new_effective_qos);
...
int proc_thread_qos_remove_override(task_t task, thread_t thread, uint64_t tid,
                                    user_addr_t resource, int resource_type) {
  boolean_t has_thread_reference = FALSE;
  ...
  if (thread == THREAD_NULL) {
    ...
    if (thread == THREAD_NULL) {
      KERNEL_DEBUG_CONSTANT((IMPORTANCE_CODE(IMP_USYNCH_QOS_OVERRIDE,
  ...
  proc_thread_qos_remove_override_internal(thread, resource, resource_type,
                                           FALSE);
...
void proc_thread_qos_deallocate(thread_t thread) {
  ...
  assert(thread->requested_policy.thrp_qos_kevent_override ==
  ...
  /* We don't need to re-evaluate thread policy here because the thread has
   * already exited */
  thread_mtx_unlock(thread);
 ...
 */
void task_set_main_thread_qos(task_t task, thread_t thread) {
  struct task_pend_token pend_token = {};
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, (IMPORTANCE_CODE(IMP_MAIN_THREAD_QOS, 0)) | DBG_FUNC_START,
      thread_tid(thread), threquested_0(thread), threquested_1(thread),
  ...
  proc_set_thread_policy_locked(thread, TASK_POLICY_ATTRIBUTE,
                                TASK_POLICY_QOS_AND_RELPRIO, primordial_qos, 0,
                                &pend_token);
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, (IMPORTANCE_CODE(IMP_MAIN_THREAD_QOS, 0)) | DBG_FUNC_END,
      thread_tid(thread), threquested_0(thread), threquested_1(thread),
      primordial_qos, 0);
}
 ...
 */
thread_qos_t task_get_default_manager_qos(task_t task) {
  thread_qos_t primordial_qos = task_compute_main_thread_qos(task);
...
boolean_t thread_recompute_kernel_promotion_locked(thread_t thread) {
   ...
   * TURNSTILE_KERNEL_PROMOTE adds threads on the waitq already capped to
   * MAXPRI_PROMOTE and propagates the priority through the chain with the same
   * cap, because as of now it does not differenciate on the kernel primitive.
   *
   * If this assumption will change with the adoption of a kernel primitive that
   * does not cap the when adding/propagating, then here is the place to put the
   * generic cap for all kernel primitives (converts the assert to
   * kern_promotion_schedpri = MIN(priority, MAXPRI_PROMOTE))
   */
  assert(kern_promotion_schedpri <= MAXPRI_PROMOTE);

  if (kern_promotion_schedpri != thread->kern_promotion_schedpri) {
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_TURNSTILE_KERNEL_CHANGE) |
             DBG_FUNC_NONE,
         thread_tid(thread), kern_promotion_schedpri,
...
boolean_t thread_recompute_user_promotion_locked(thread_t thread) {
  ...
  uint8_t user_promotion_basepri =
      MIN((uint8_t)thread_get_inheritor_turnstile_base_priority(thread),
          MAXPRI_USER);
  ...
  if (thread->user_promotion_basepri == user_promotion_basepri) {
    ...
    KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                              (TURNSTILE_CODE(TURNSTILE_PRIORITY_OPERATIONS,
                                  ...
                                  DBG_FUNC_NONE,
                              thread_tid(thread), user_promotion_basepri,
                              thread->user_promotion_basepri, 0, 0);
    KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_TURNSTILE_USER_CHANGE) |
             DBG_FUNC_NONE,
         thread_tid(thread), user_promotion_basepri,
  ...
  proc_set_thread_policy_spinlocked(thread, TASK_POLICY_ATTRIBUTE,
                                    TASK_POLICY_QOS_PROMOTE, qos_promotion, 0,
                                    &pend_token);
 ...
 * that may deadlock against the thread lock, the throttle lock, or the SFI
 * lock.
 *
 * One 'add' must be balanced by one 'drop'.
 * Between 'add' and 'drop', the overide QoS value may be updated with an
 * 'update'. Before the thread is deallocated, there must be 0 remaining
 * overrides.
 */
static void thread_kevent_override(thread_t thread, uint32_t qos_override,
...
void thread_add_kevent_override(thread_t thread, uint32_t qos_override) {
  thread_kevent_override(thread, qos_override, TRUE);
}

void thread_update_kevent_override(thread_t thread, uint32_t qos_override) {
  thread_kevent_override(thread, qos_override, FALSE);
}

void thread_drop_kevent_override(thread_t thread) {
  struct task_pend_token pend_token = {};
 ...
 * overrides.
 */
static void thread_servicer_override(thread_t thread, uint32_t qos_override,
                                     boolean_t is_new_override) {
  struct task_pend_token pend_token = {};
...
void thread_add_servicer_override(thread_t thread, uint32_t qos_override) {
  assert(qos_override > THREAD_QOS_UNSPECIFIED);
  assert(qos_override < THREAD_QOS_LAST);
...
void thread_update_servicer_override(thread_t thread, uint32_t qos_override) {
  assert(qos_override > THREAD_QOS_UNSPECIFIED);
  assert(qos_override < THREAD_QOS_LAST);
...
void thread_drop_servicer_override(thread_t thread) {
  thread_servicer_override(thread, THREAD_QOS_UNSPECIFIED, FALSE);
}

void thread_update_servicer_iotier_override(thread_t thread,
                                            uint8_t iotier_override) {
  struct task_pend_token pend_token = {};
  uint8_t current_iotier;

  /* Check if the update is needed */
  current_iotier = (uint8_t)thread_get_requested_policy_spinlocked(
      thread, TASK_POLICY_ATTRIBUTE, TASK_POLICY_IOTIER_KEVENT_OVERRIDE, NULL);
...
thread_qos_t thread_get_requested_qos(thread_t thread, int *relpri) {
  ...
  qos = (thread_qos_t)proc_get_thread_policy_locked(
      thread, TASK_POLICY_ATTRIBUTE, TASK_POLICY_QOS_AND_RELPRIO,
      &relprio_value);
  if (relpri) {
 ...
 *
 * This should be replaced with a generic 'priority inheriting gate' mechanism
 * (24194397)
 */
void thread_set_exec_promotion(thread_t thread) {
  spl_t s = splsched();
  thread_lock(thread);
 ...
 */
void thread_clear_exec_promotion(thread_t thread) {
  spl_t s = splsched();
  thread_lock(thread);
 ...
 * to threads that have joined a workinterval with
 * WORK_INTERVAL_WORKLOAD_ID_RT_ALLOWED.
 */
static TUNABLE(bool, rt_allow_policy_enabled, "-rt-allow_policy-enable",
#if XNU_TARGET_OS_XR
               true
#else
               false
...
TUNABLE_DEV_WRITEABLE(uint8_t, rt_allow_limit_percent, "rt_allow_limit_percent",
                      70);
...
static bool thread_has_rt(thread_t thread) {
  return thread->sched_mode == TH_MODE_REALTIME ||
         thread->saved_mode == TH_MODE_REALTIME;
}
 ...
 */
static void thread_rt_set_cpulimit(thread_t thread) {
...
static void thread_rt_clear_cpulimit(thread_t thread) {
  thread->sched_flags &= ~TH_SFLAG_RT_CPULIMIT;
 ...
 */
void thread_rt_evaluate(thread_t thread) {
  task_t task = get_threadtask(thread);
   ...
   */
  bool set_ast = false;
  if (!platform_binary && thread_has_rt(thread) &&
...
void thread_rt_evaluate(__unused thread_t thread) {}
