 ...
 */
static void exclaves_bootstrap_context_acquire(exclaves_ctx_t *save_ctx) {
...
size_t exclaves_ipc_buffer_count(void) {
  return os_atomic_load(&exclaves_ipcb_cnt, relaxed);
}

static void exclaves_ipc_buffer_count_inc(void) {
  os_atomic_inc(&exclaves_ipcb_cnt, relaxed);
}

static void exclaves_ipc_buffer_count_dec(void) {
...
static bool exclaves_ipc_buffer_cache_alloc(exclaves_ctx_t *ctx) {
...
static bool exclaves_ipc_buffer_cache_free(exclaves_ctx_t *ctx) {
...
static kern_return_t exclaves_ipc_buffer_cache_init(void) {
  ...
  exclaves_ctx_t *ctx =
      kalloc_type(exclaves_ctx_t, exclaves_ipc_buffer_cache_max,
...
kern_return_t exclaves_xnuproxy_ctx_alloc(exclaves_ctx_t *ctx) {
  assert3p(ctx, !=, NULL);
  ...
  tb_error_t ret = xnuproxy_cmd_ipccontextallocate(
      &xnuproxy_cmd_client, ^(xnuproxy_ipccontext_s c) {
  ...
  if (ret != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(show_errors, "allocate context: failure %u\n", ret);
    return KERN_FAILURE;
  }
...
kern_return_t exclaves_xnuproxy_ctx_free(exclaves_ctx_t *ctx) {
  assert3p(ctx, !=, NULL);
  ...
  if (ret != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(show_errors, "free context: failure %u\n", ret);
    return KERN_FAILURE;
  }
...
static size_t countof_char_v(const char_v_s *cv) {
  assert3p(cv, !=, NULL);

  __block size_t count = 0;

  char__v_visit(cv, ^(__unused size_t i, __unused const xnuproxy_char_s item) {
    count++;
  });
...
static void copy_char_v(const char_v_s *src, char *dst) {
  assert3p(src, !=, NULL);
  assert3p(dst, !=, NULL);

  char__v_visit(src, ^(size_t i, const xnuproxy_char_s item) {
    dst[i] = item;
  });
}
 ...
 */
kern_return_t exclaves_xnuproxy_resource_info(
    void (^cb)(const char *name, const char *domain, xnuproxy_resourcetype_s,
               uint64_t id, bool)) {
  /* BEGIN IGNORE CODESTYLE */
  tb_error_t ret = xnuproxy_cmd_resourceinfo(
      &xnuproxy_cmd_client, ^(xnuproxy_resourceinfo_v_s ri) {
        xnuproxy_resourceinfo__v_visit(
            &ri, ^(__unused size_t i, const xnuproxy_resourceinfo_s *item) {
              ...
              cb(name_copy, domain_copy, (xnuproxy_resourcetype_s)item->type,
                 item->id, item->connected);
            });
      });
  /* END IGNORE CODESTYLE */

  if (ret != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(show_errors, "resource info: failure %u\n", ret);
    return KERN_FAILURE;
  }
...
kern_return_t exclaves_xnuproxy_pmm_usage(void) {
  tb_error_t ret = xnuproxy_cmd_pmmmemusage(&xnuproxy_cmd_client);
  if (ret != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(show_errors, "pmm usage: failure %u\n", ret);
    return KERN_FAILURE;
  }
...
#define exclaves_xnuproxy_endpoint_call_show_progress(operation, step, eid,    \
                                                      scid, status)            \
  exclaves_debug_printf(show_progress,                                         \
                        ...
                        (eid), (scid), (status))
OS_NOINLINE
kern_return_t exclaves_xnuproxy_endpoint_call(Exclaves_L4_Word_t endpoint_id) {
  ...
  xnuproxy_msg_status_t status = XNUPROXY_MSG_STATUS_PROCESSING;

  XNUPROXY_CR_ENDPOINT_ID(ipcb) = endpoint_id;
  XNUPROXY_CR_STATUS(ipcb) = status;

  exclaves_xnuproxy_endpoint_call_show_progress(call, entry, endpoint_id, scid,
                                                status);
  ...
  while (1) {
    ...
    status = (xnuproxy_msg_status_t)XNUPROXY_CR_STATUS(ipcb);

    switch (status) {
    case XNUPROXY_MSG_STATUS_PROCESSING:
      exclaves_xnuproxy_endpoint_call_show_progress(call, yielded, endpoint_id,
                                                    scid, status);
      continue;

    case XNUPROXY_MSG_STATUS_REPLY:
      exclaves_xnuproxy_endpoint_call_show_progress(call, returned, endpoint_id,
                                                    scid, status);
      kr = KERN_SUCCESS;
      break;
    ...
    case XNUPROXY_MSG_STATUS_UPCALL:
      ...
      KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES, MACH_EXCLAVES_UPCALL) |
                       DBG_FUNC_START,
                   scid, endpoint_id);
      exclaves_xnuproxy_endpoint_call_show_progress(upcall, entry, endpoint_id,
                                                    scid, status);
      kr = exclaves_call_upcall_handler(endpoint_id);
      XNUPROXY_CR_STATUS(ipcb) = XNUPROXY_MSG_STATUS_PROCESSING;
      /* TODO: More state returned than Success or OperationInvalid? */
      XNUPROXY_CR_RETVAL(ipcb) = (kr == KERN_SUCCESS)
                                     ? Exclaves_L4_Success
                                     : Exclaves_L4_ErrorOperationInvalid;
      KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES, MACH_EXCLAVES_UPCALL) |
                   DBG_FUNC_END);
      thread->th_exclaves_state &= ~TH_EXCLAVES_UPCALL;
      exclaves_xnuproxy_endpoint_call_show_progress(
          upcall, returned, endpoint_id, scid,
          (unsigned int)XNUPROXY_CR_RETVAL(ipcb));
      continue;

    default:
      // Should we have an assert(valid return) here?
      exclaves_xnuproxy_endpoint_call_show_progress(call, failed, endpoint_id,
                                                    scid, status);
      kr = KERN_FAILURE;
      break;
    }
   ...
   * With opposite order, Stackshot could put the thread with RPC flag on
   * collection list but the thread would be free to continue and to release its
   * SCID.
   */
  os_compiler_barrier();
...
kern_return_t exclaves_xnuproxy_init(uint64_t bootinfo_pa) {
  ...
  /* BEGIN IGNORE CODESTYLE */
  tb_error_t ret = xnuproxy_bootinfo__unmarshal(
      bootinfo_va, bootinfo_size, ^(xnuproxy_bootinfo_s bootinfo) {
        ...
        if (bootinfo.version != XNUPROXY_VERSION_CURRENT) {
          exclaves_debug_printf(
              ...
              "xnuproxy: %u, xnu: %u\n",
              bootinfo.version, XNUPROXY_VERSION_CURRENT);
          return;
        }
  ...
  /* END IGNORE CODESTYLE */
  if (ret != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(show_errors, "failed to unmarshal bootinfo\n");
    return KERN_FAILURE;
  }
  ...
  if (ret != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(show_errors, "failed to create xnuproxy endpoint\n");
    return KERN_FAILURE;
  }
