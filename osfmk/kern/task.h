...
struct task_pend_token {
  union {
    struct {
      uint32_t tpt_update_sockets : 1, tpt_update_timers : 1,
          tpt_update_watchers : 1, tpt_update_live_donor : 1,
          tpt_update_coal_sfi : 1, tpt_update_throttle : 1,
          tpt_update_thread_sfi : 1, tpt_force_recompute_pri : 1,
          tpt_update_tg_ui_flag : 1, tpt_update_turnstile : 1,
          tpt_update_tg_app_flag : 1, tpt_update_game_mode : 1,
          tpt_update_carplay_mode : 1, tpt_update_appnap : 1;
    };
    uint32_t tpt_value;
  };
};
...
struct task_security_config {
  union {
    struct {
      uint8_t hardened_heap : 1, tpro : 1, reserved : 1,
...
__options_closed_decl(
    task_memlimit_flags_t, uint32_t,
    {/* if set, use active attributes, otherwise use inactive attributes */
     ...
     /* if set, suppress exc_resource exception when task exceeds active memory
        limit */
     TASK_MEMLIMIT_ACTIVE_EXC_RESOURCE = 0x04,
     /* if set, suppress exc_resource exception when task exceeds inactive
        memory limit */
...
struct task {
  ...
  /* Miscellaneous */
  vm_map_t
      XNU_PTRAUTH_SIGNED_PTR("task.map") map; /* Address space description */
  ...
  struct task_suspend_stats_s
      t_suspend_stats; /* suspension statistics for this task */
  task_suspend_source_array_t
      t_suspend_sources; /* array of suspender debug info for this task */
...
#define task_is_immovable(task)                                                \
   ...
   */
  struct ipc_port *XNU_PTRAUTH_SIGNED_PTR("task.itk_task_ports")
      itk_task_ports[TASK_SELF_PORT_COUNT];
#if CONFIG_CSR
  struct ipc_port *XNU_PTRAUTH_SIGNED_PTR("task.itk_settable_self")
      itk_settable_self; /* a send right */
#endif                   /* CONFIG_CSR */
  ...
  struct ipc_port *
      XNU_PTRAUTH_SIGNED_PTR("task.itk_host") itk_host; /* a send right */
  struct ipc_port *XNU_PTRAUTH_SIGNED_PTR("task.itk_bootstrap")
      itk_bootstrap; /* a send right */
  struct ipc_port *XNU_PTRAUTH_SIGNED_PTR("task.itk_debug_control")
      itk_debug_control; /* send right for debugmode communications */
  struct ipc_port *XNU_PTRAUTH_SIGNED_PTR("task.itk_task_access")
      itk_task_access; /* and another send right */
  struct ipc_port *XNU_PTRAUTH_SIGNED_PTR("task.itk_resume")
      itk_resume; /* a receive right to resume this task */
  struct ipc_port *XNU_PTRAUTH_SIGNED_PTR("task.itk_registered")
      itk_registered[TASK_PORT_REGISTER_MAX];
  /* all send rights */
  ipc_port_t *XNU_PTRAUTH_SIGNED_PTR("task.itk_dyld_notify")
      itk_dyld_notify; /* lazy send rights array of size
                          DYLD_MAX_PROCESS_INFO_NOTIFY_COUNT */
#if CONFIG_PROC_RESOURCE_LIMITS
  struct ipc_port *XNU_PTRAUTH_SIGNED_PTR("task.itk_resource_notify")
      itk_resource_notify; /* a send right to the resource notify port */
...
#define VM_BACKING_STORE_PRIV 0x1
  ...
  uint32_t decompressions; /* decompression counter (from threads that already
                              terminated) */
...
#endif
  volatile uint32_t
      t_flags; /* general-purpose task flags protected by task_lock (TL) */
#define TF_NONE 0
#define TF_64B_ADDR 0x00000001 /* task has 64-bit addressing */
#define TF_64B_DATA 0x00000002 /* task has 64-bit data registers */
#define TF_CPUMON_WARNING                                                      \
  0x00000004 /* task has at least one thread in CPU usage warning zone */
#define TF_WAKEMON_WARNING                                                     \
  0x00000008 /* task is in wakeups monitor warning zone */
#define TF_TELEMETRY                                                           \
  (TF_CPUMON_WARNING | TF_WAKEMON_WARNING) /* task is a telemetry participant  \
                                            */
...
#define TF_NO_SMT                                                              \
  0x00002000 /* task threads must not be paired with SMT threads */
#define TF_SYS_VERSION_COMPAT                                                  \
  0x00008000 /* shim task accesses to OS version data (macOS - app             \
                compatibility) */
#define TF_TECS 0x00020000 /* task threads must enable CPU security */
#if defined(__x86_64__)
#define TF_INSN_COPY_OPTOUT                                                    \
  0x00040000 /* task threads opt out of unhandled-fault instruction stream     \
                collection */
#endif
...
#define TF_NO_CORPSE_FORKING 0x00100000 /* do not fork a corpse for this task  \
                                         */
#define TF_USE_PSET_HINT_CLUSTER_TYPE                                          \
  0x00200000 /* bind task to task->pset_hint->pset_cluster_type */
#define TF_DYLD_ALL_IMAGE_FINAL                                                \
  0x00400000                  /* all_image_info_addr can no longer be changed */
...
/*
 * WARNING: These TF_ and TFRO_ flags are NOT automatically inherited by a child
 * of fork If you believe something should be inherited, you must manually
 * inherit the flags in `task_create_internal`
 */
...
#define TFRO_MACH_HARDENING_OPT_OUT                                            \
  0x00000040 /* task might load third party plugins on macOS and should be     \
                opted out of mach hardening */
#endif       /* XNU_TARGET_OS_OSX */
#define TFRO_PLATFORM 0x00000080 /* task is a platform binary */

#define TFRO_FILTER_MSG                                                        \
  0x00004000 /* task calls into message filter callback before sending a       \
                message */
#define TFRO_PAC_EXC_FATAL                                                     \
  0x00010000 /* task is marked a corpse if a PAC exception occurs */
#define TFRO_JIT_EXC_FATAL                                                     \
  0x00020000 /* kill the task on access violations from privileged JIT code */
#define TFRO_PAC_ENFORCE_USER_STATE                                            \
  0x01000000 /* Enforce user and kernel signed thread state */
#if CONFIG_EXCLAVES
#define TFRO_HAS_KD_ACCESS                                                     \
  0x02000000 /* Access to the kernel exclave resource domain  */
#endif       /* CONFIG_EXCLAVES */
#define TFRO_FREEZE_EXCEPTION_PORTS                                            \
  0x04000000 /* Setting new exception ports on the task/thread is disallowed   \
              */
#if CONFIG_EXCLAVES
#define TFRO_HAS_SENSOR_MIN_ON_TIME_ACCESS                                     \
  0x08000000 /* Access to sensor minimum on time call  */
#endif       /* CONFIG_EXCLAVES */

/*
 * Task is running within a 64-bit address space.
 */
#define task_has_64Bit_addr(task) (((task)->t_flags & TF_64B_ADDR) != 0)
#define task_set_64Bit_addr(task) ((task)->t_flags |= TF_64B_ADDR)
#define task_clear_64Bit_addr(task) ((task)->t_flags &= ~TF_64B_ADDR)

/*
 * Task is using 64-bit machine state.
 */
#define task_has_64Bit_data(task) (((task)->t_flags & TF_64B_DATA) != 0)
#define task_set_64Bit_data(task) ((task)->t_flags |= TF_64B_DATA)
#define task_clear_64Bit_data(task) ((task)->t_flags &= ~TF_64B_DATA)
...
#define task_is_a_corpse_fork(task) (((task)->t_flags & TF_CORPSE_FORK) != 0)

#define task_set_coalition_member(task) ((task)->t_flags |= TF_COALITION_MEMBER)
...
#define task_has_proc(task) (((task)->t_flags & TF_HASPROC) != 0)

#define task_set_has_proc(task) ((task)->t_flags |= TF_HASPROC)

#define task_clear_has_proc(task) ((task)->t_flags &= ~TF_HASPROC)

  uint32_t
      t_procflags; /* general-purpose task flags protected by proc_lock (PL) */
#define TPF_NONE 0
...
#define task_did_exec_internal(task) (((task)->t_procflags & TPF_DID_EXEC) != 0)
...
#define TASK_KPC_FORCED_ALL_CTRS                                               \
  0x2             /* Bit in "t_kpc" signifying this task forced all counters */
  ...
  uint16_t policy_ru_cpu : 4, policy_ru_cpu_ext : 4, applied_ru_cpu : 4,
...
#if MACH_ASSERT
  int8_t suspends_outstanding; /* suspends this task performed in excess of
                                  resumes */
#endif
  uint8_t t_returnwaitflags;
#define TWF_NONE 0
#define TRW_LRETURNWAIT                                                        \
  0x01 /* task is waiting for fork/posix_spawn/exec to complete */
#define TRW_LRETURNWAITER                                                      \
  0x02 /* task is waiting for TRW_LRETURNWAIT to get cleared */
#define TRW_LEXEC_COMPLETE 0x04 /* thread should call exec complete */

#if CONFIG_EXCLAVES
  uint8_t t_exclave_state;
#define TES_NONE 0
#define TES_CONCLAVE_TAINTED                                                   \
  0x01 /* Task has talked to conclave, xnu has tainted the process */
#define TES_CONCLAVE_UNTAINTABLE                                               \
  0x02 /* Task can not be tainted by xnu when it talks to conclave */
#endif /* CONFIG_EXCLAVES */
  ...
  /*
   * Can be merged with imp_donor bits, once the IMPORTANCE_INHERITANCE macro
   * goes away.
   */
  uint32_t low_mem_notified_warn
      ...
      purged_memory_warn : 1,     /* purgeable memory of the task is purged for
                                     warning level pressure */
      purged_memory_critical : 1, /* purgeable memory of the task is purged for
                                     critical level pressure */
      low_mem_privileged_listener
      : 1, /* if set, task would like to know about pressure changes before
...
#if HYPERVISOR
  void *XNU_PTRAUTH_SIGNED_PTR("task.hv_task_target")
      hv_task_target; /* hypervisor virtual machine object associated with this
                         task */
#endif                /* HYPERVISOR */
...
#endif                             /* CONFIG_FREEZE */
  boolean_t donates_own_pages; /* pages land on the special Q (only swappable
                                  pages on iPadOS, early swap on macOS) */
  uint32_t task_shared_region_slide; /* cached here to avoid locking during
                                        telemetry */
  ...
  uint64_t memstat_dirty_start; /* last abstime transition into the dirty band
                                   or last call to task_ledger_settle_dirty_time
                                   while dirty */
...
#if CONFIG_DEFERRED_RECLAIM
  vm_deferred_reclamation_metadata_t
      deferred_reclamation_metadata; /* Protected by the task lock */
...
static inline void task_require(struct task *task) {
  zone_id_require(ZONE_ID_PROC_TASK, proc_and_task_size,
                  task_get_proc_raw(task));
}

#define task_lock(task) lck_mtx_lock(&(task)->lock)
#define task_lock_assert_owned(task)                                           \
...
#define task_objq_lock_init(task)                                              \
  lck_mtx_init(&(task)->task_objq_lock, &vm_object_lck_grp, &vm_object_lck_attr)
#define task_objq_lock_destroy(task)                                           \
  lck_mtx_destroy(&(task)->task_objq_lock, &vm_object_lck_grp)
#define task_objq_lock(task) lck_mtx_lock(&(task)->task_objq_lock)
#define task_objq_lock_assert_owned(task)                                      \
  LCK_MTX_ASSERT(&(task)->task_objq_lock, LCK_MTX_ASSERT_OWNED)
...
#define itk_lock_init(task)                                                    \
  lck_mtx_init(&(task)->itk_lock_data, &ipc_lck_grp, &ipc_lck_attr)
#define itk_lock_destroy(task)                                                 \
  lck_mtx_destroy(&(task)->itk_lock_data, &ipc_lck_grp)
...
__pure2 static inline ipc_space_t current_space(void) {
...
struct task_watchports {
  ...
  struct task_watchport_elem
      tw_elem[]; /* (Space lock) & (Portlock) & (mq lock) */
};
...
#define task_watchport_elem_clear(elem)                                        \
  task_watchport_elem_init((elem), NULL, NULL)

extern void task_add_turnstile_watchports(task_t task, thread_t thread,
                                          ipc_port_t *portwatch_ports,
                                          uint32_t portwatch_count);

extern void
task_watchport_elem_deallocate(struct task_watchport_elem *watchport_elem);

extern boolean_t task_has_watchports(task_t task);

void task_dyld_process_info_update_helper(task_t task, size_t active_count,
                                          vm_map_address_t magic_addr,
                                          ipc_port_t *release_ports,
                                          size_t release_count);

extern kern_return_t task_suspend2_mig(task_t task,
                                       task_suspension_token_t *suspend_token);

extern kern_return_t
task_suspend2_external(task_t task, task_suspension_token_t *suspend_token);

extern kern_return_t task_resume2_mig(task_suspension_token_t suspend_token);

extern kern_return_t
task_resume2_external(task_suspension_token_t suspend_token);

extern void
task_suspension_token_deallocate_grp(task_suspension_token_t suspend_token,
                                     task_grp_t grp);

extern ipc_port_t convert_task_to_port_with_flavor(task_t task,
...
/* Hold all threads in a task, Wait for task to stop running, just to get off
 * CPU */
extern kern_return_t task_hold_and_wait(task_t task, bool suspend_conclave);
...
extern kern_return_t task_pidsuspend(task_t task);

/* Resumes a previously paused task */
extern kern_return_t task_pidresume(task_t task);

extern kern_return_t task_send_trace_memory(task_t task, uint32_t pid,
                                            uint64_t uniqueid);

extern void task_remove_turnstile_watchports(task_t task);

extern void task_transfer_turnstile_watchports(task_t old_task, task_t new_task,
                                               thread_t new_thread);

extern kern_return_t task_violated_guard(mach_exception_code_t,
                                         mach_exception_subcode_t, void *,
                                         bool);

#if DEVELOPMENT || DEBUG

extern kern_return_t task_disconnect_page_mappings(task_t task);
#endif /* DEVELOPMENT || DEBUG */
...
extern kern_return_t task_freeze(task_t task, uint32_t *purgeable_count,
                                 uint32_t *wired_count, uint32_t *clean_count,
                                 uint32_t *dirty_count, uint32_t dirty_budget,
                                 uint32_t *shared_count,
                                 int *freezer_error_code, boolean_t eval_only);

/* Thaw a currently frozen task */
extern kern_return_t task_thaw(task_t task);

typedef enum { CREDIT_TO_SWAP = 1, DEBIT_FROM_SWAP = 2 } freezer_acct_op_t;

extern void task_update_frozen_to_swap_acct(task_t task, int64_t amount,
                                            freezer_acct_op_t op);

#endif /* CONFIG_FREEZE */

/* Halt all other threads in the current task */
extern kern_return_t task_start_halt(task_t task);

/* Wait for other threads to halt and free halting task resources */
extern void task_complete_halt(task_t task);

extern kern_return_t task_terminate_internal(task_t task);

struct proc_ro;
typedef struct proc_ro *proc_ro_t;

extern kern_return_t
task_create_internal(task_t parent_task, proc_ro_t proc_ro,
                     coalition_t *parent_coalitions, boolean_t inherit_memory,
                     boolean_t is_64bit, boolean_t is_64bit_data,
                     uint32_t t_flags, uint32_t t_flags_ro, uint32_t procflags,
                     uint8_t t_returnwaitflags, task_t child_task);

extern kern_return_t task_set_special_port_internal(task_t task, int which,
                                                    ipc_port_t port);

extern kern_return_t task_set_security_tokens(task_t task,
                                              security_token_t sec_token,
                                              audit_token_t audit_token,
                                              host_priv_t host_priv);

extern kern_return_t task_info(task_t task, task_flavor_t flavor,
...
extern uint64_t task_gpu_utilisation(task_t task);

extern void task_update_cpu_time_qos_stats(task_t task, uint64_t *eqos_stats,
                                           uint64_t *rqos_stats);

extern void task_vtimer_set(task_t task, integer_t which);

extern void task_vtimer_clear(task_t task, integer_t which);

extern void task_vtimer_update(task_t task, integer_t which,
                               uint32_t *microsecs);
...
extern void task_set_64bit(task_t task, boolean_t is_64bit,
                           boolean_t is_64bit_data);

extern bool task_get_64bit_addr(task_t task);

extern bool task_get_64bit_data(task_t task);

extern void task_set_platform_binary(task_t task, boolean_t is_platform);
...
#endif /* DEVELOPMENT || DEBUG */
extern void task_disable_mach_hardening(task_t task);

extern bool task_opted_out_mach_hardening(task_t task);
#endif /* XNU_TARGET_OS_OSX */

extern boolean_t task_is_a_corpse(task_t task);

extern boolean_t task_is_ipc_active(task_t task);

extern void task_set_corpse(task_t task);

extern void task_set_exc_guard_default(task_t task, const char *name,
                                       unsigned long namelen,
                                       boolean_t is_simulated,
                                       uint32_t platform, uint32_t sdk);

extern void task_copyout_control_port(task_t task);

extern bool task_set_ca_client_wi(task_t task, boolean_t ca_client_wi);

extern kern_return_t task_set_dyld_info(task_t task, mach_vm_address_t addr,
                                        mach_vm_size_t size,
                                        bool finalize_value);

extern void task_set_mach_header_address(task_t task, mach_vm_address_t addr);

extern void task_set_uniqueid(task_t task);

/* Get number of activations in a task */
extern int get_task_numacts(task_t task);

extern bool task_donates_own_pages(task_t task);
...
#if CONFIG_LEDGER_INTERVAL_MAX
extern uint64_t get_task_neural_nofootprint_total_interval_max(task_t,
                                                               int reset);
...
extern kern_return_t task_set_phys_footprint_limit_internal(task_t, int, int *,
                                                            boolean_t,
                                                            boolean_t);
extern kern_return_t task_get_phys_footprint_limit(task_t task, int *limit_mb);
#if DEBUG || DEVELOPMENT
#if CONFIG_MEMORYSTATUS
extern kern_return_t task_set_diag_footprint_limit_internal(task_t, uint64_t,
                                                            uint64_t *);
extern kern_return_t task_get_diag_footprint_limit_internal(task_t, uint64_t *,
                                                            bool *);
extern kern_return_t task_set_diag_footprint_limit(task_t task,
                                                   uint64_t new_limit_mb,
                                                   uint64_t *old_limit_mb);
#endif /* CONFIG_MEMORYSTATUS */
#endif /* DEBUG || DEVELOPMENT */
extern kern_return_t task_get_conclave_mem_limit(task_t,
                                                 uint64_t *conclave_limit);
extern kern_return_t task_set_conclave_mem_limit(task_t,
                                                 uint64_t conclave_limit);
...
extern void task_set_tokens(task_t task, security_token_t *sec_token,
                            audit_token_t *audit_token);
...
extern void task_reset_triggered_exc_resource(task_t task,
...
#if CONFIG_PROC_RESOURCE_LIMITS
extern kern_return_t task_set_port_space_limits(task_t task,
                                                uint32_t soft_limit,
                                                uint32_t hard_limit);
#endif /* CONFIG_PROC_RESOURCE_LIMITS */
...
extern void
task_set_system_version_compat_enabled(task_t task,
...
extern kern_return_t
machine_task_get_state(task_t task, int flavor, thread_state_t state,
                       mach_msg_type_number_t *state_count);

extern kern_return_t machine_task_set_state(task_t task, int flavor,
                                            thread_state_t state,
                                            mach_msg_type_number_t state_count);

extern void machine_task_terminate(task_t task);

extern kern_return_t machine_task_process_signature(task_t task,
                                                    uint32_t platform,
                                                    uint32_t sdk,
...
extern kern_return_t task_wakeups_monitor_ctl(task_t task, uint32_t *rate_hz,
                                              int32_t *flags);
...
/* returns true if update must be pushed to coalition (Automatically handled by
 * task_set_game_mode) */
...
extern void task_copy_vmobjects(task_t task, vm_object_query_t query,
                                size_t len, size_t *num);
extern void task_get_owned_vmobjects(task_t task, size_t buffer_size,
...
extern void task_get_corpse_vmobject_list(task_t task,
...
extern kern_return_t _kernelrpc_mach_ports_register3(task_t task,
                                                     mach_port_t port1,
                                                     mach_port_t port2,
                                                     mach_port_t port3);
...
#if CONFIG_EXCLAVES
int task_add_conclave(task_t task, void *, int64_t,
...
extern void convert_task_array_to_ports(task_array_t, size_t,
                                        mach_task_flavor_t);
...
/* Convert from a port (in this case, an SO right to a task's resume port) to a
 * task. */
...
extern void task_update_physical_writes(task_t task,
                                        task_physical_write_flavor_t flavor,
                                        uint64_t io_size,
                                        task_balance_flags_t flags);

#if CONFIG_SECLUDED_MEMORY
extern void task_set_can_use_secluded_mem(task_t task,
                                          boolean_t can_use_secluded_mem);
extern void task_set_could_use_secluded_mem(task_t task,
                                            boolean_t could_use_secluded_mem);
extern void
task_set_could_also_use_secluded_mem(task_t task,
                                     boolean_t could_also_use_secluded_mem);
extern boolean_t task_can_use_secluded_mem(task_t task, boolean_t is_allocate);
...
extern int task_find_region_details(
    task_t task, vm_map_offset_t offset, find_region_details_options_t options,
    uintptr_t *vp_p, /* caller must call vnode_put(vp) when done */
    uint32_t *vid_p, bool *is_mapped_shared_p, uint64_t *start_p,
    uint64_t *len_p);
...
#if DEVELOPMENT || DEBUG
extern void task_set_no_footprint_for_debug(task_t task, boolean_t value);
extern int task_get_no_footprint_for_debug(task_t task);
#endif /* DEVELOPMENT || DEBUG */

#ifdef KERNEL_PRIVATE
extern kern_return_t task_get_suspend_stats(task_t task,
                                            task_suspend_stats_t stats);
extern kern_return_t task_get_suspend_stats_kdp(task_t task,
                                                task_suspend_stats_t stats);
#endif /* KERNEL_PRIVATE*/
