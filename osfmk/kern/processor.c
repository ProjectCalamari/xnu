...
static void
pset_node_set_for_pset_cluster_type(pset_node_t node,
                                    pset_cluster_type_t pset_cluster_type) {
  assert3p(os_atomic_load(&pset_nodes_by_cluster_type[pset_cluster_type - 1],
                          relaxed),
           ==, PSET_NODE_NULL);
  os_atomic_store(&pset_nodes_by_cluster_type[pset_cluster_type - 1], node,
                  release);
}

pset_node_t
pset_node_for_pset_cluster_type(pset_cluster_type_t pset_cluster_type) {
  assert3u(pset_cluster_type, !=, PSET_SMP);
  return os_atomic_load(&pset_nodes_by_cluster_type[pset_cluster_type - 1],
                        acquire);
}
...
void processor_update_offline_state_locked(
    processor_t processor, processor_offline_state_t new_state) {
...
void processor_update_offline_state(processor_t processor,
...
#if __AMP__
  /*
   * Since this is an AMP system, fill up cluster type and ID information; this
   * should do the same kind of initialization done via ml_processor_register()
   */
  const ml_topology_info_t *topology_info = ml_get_topology_info();
  ml_topology_cluster_t *boot_cluster = topology_info->boot_cluster;
  pset_cluster_type_t boot_cluster_type =
      cluster_type_to_pset_cluster_type(boot_cluster->cluster_type);
 ...
 */
void processor_init(processor_t processor, int cpu_id, processor_set_t pset) {
  spl_t s;
  ...
  processor->active_thread = processor->startup_thread =
  ...
  processor->processor_primary =
      processor; /* no SMT relationship known at this point */
  ...
  for (int i = 0; i < RUNNING_TIMER_MAX; i++) {
    timer_call_setup(&processor->running_timers[i], running_timer_funcs[i],
                     processor);
  ...
  if (pset->cpu_set_count++ == 0) {
    ...
    pset->cpu_set_low =
        (cpu_id < pset->cpu_set_low) ? cpu_id : pset->cpu_set_low;
  ...
  bit_set(processor_offline_state_map[processor->processor_offline_state],
          cpu_id);

  if (processor == master_processor) {
    processor_update_offline_state_locked(processor,
                                          PROCESSOR_OFFLINE_STARTING);
  }
...
void processor_set_primary(processor_t processor, processor_t primary) {
  assert(processor->processor_primary == primary ||
  ...
  assert(primary->processor_secondary == NULL ||
...
processor_set_t processor_pset(processor_t processor) {
...
cluster_type_t pset_type_for_id(uint32_t cluster_id) {
 ...
 * threads which are foreign to the pset/cluster. A thread is defined as foreign
 * for a cluster if its of a different type than its preferred cluster type
 * (E/P). The bitmap should be updated every time a new thread is assigned to
 * run on a processor. Cluster shared resource intensive threads are also not
 * counted as foreign threads since these threads should not be rebalanced when
 * running on non-preferred clusters.
 *
 * This bitmap allows the Edge scheduler to quickly find CPUs running foreign
 * threads for rebalancing.
 */
static void processor_state_update_running_foreign(processor_t processor,
                                                   thread_t thread) {
  cluster_type_t current_processor_type =
      pset_type_for_id(processor->processor_set->pset_cluster_id);
  cluster_type_t thread_type =
 ...
 */
static void
processor_state_update_running_cluster_shared_rsrc(processor_t processor,
                                                   thread_t thread) {
  if (thread_shared_rsrc_policy_get(thread, CLUSTER_SHARED_RSRC_TYPE_RR)) {
    bit_set(processor->processor_set->cpu_running_cluster_shared_rsrc_thread
                [CLUSTER_SHARED_RSRC_TYPE_RR],
            processor->cpu_id);
  } else {
    bit_clear(processor->processor_set->cpu_running_cluster_shared_rsrc_thread
                  [CLUSTER_SHARED_RSRC_TYPE_RR],
              processor->cpu_id);
  }
...
void processor_state_update_idle(processor_t processor) {
...
#if CONFIG_SCHED_EDGE
  os_atomic_store(
      &processor->processor_set->cpu_running_buckets[processor->cpu_id],
      TH_BUCKET_SCHED_MAX, relaxed);
  bit_clear(
      processor->processor_set
          ->cpu_running_cluster_shared_rsrc_thread[CLUSTER_SHARED_RSRC_TYPE_RR],
      processor->cpu_id);
  bit_clear(processor->processor_set->cpu_running_cluster_shared_rsrc_thread
                [CLUSTER_SHARED_RSRC_TYPE_NATIVE_FIRST],
            processor->cpu_id);
...
void processor_state_update_from_thread(processor_t processor, thread_t thread,
  ...
  /* Since idle and bound threads are not tracked by the edge scheduler, ignore
   * when those threads go on-core */
  sched_bucket_t bucket =
      ((thread->state & TH_IDLE) || (thread->bound_processor != PROCESSOR_NULL))
          ? TH_BUCKET_SCHED_MAX
          : thread->th_sched_bucket;
  os_atomic_store(
      &processor->processor_set->cpu_running_buckets[processor->cpu_id], bucket,
      relaxed);
  sched_edge_stir_the_pot_update_registry_state(thread);
#endif /* CONFIG_SCHED_EDGE */
...
pset_node_t pset_node_root(void) { return &pset_node0; }

#if __AMP__

/*
 * Only need to dynamically initialize pset nodes when
 * there are multiple cluster types.
 */
static pset_node_t pset_node_create(cluster_type_t cluster_type) {
  lck_spin_lock(&pset_node_lock);
  assert3u(cluster_type, !=, CLUSTER_TYPE_SMP);

  pset_node_t node;
  pset_cluster_type_t pset_cluster_type =
      cluster_type_to_pset_cluster_type(cluster_type);
  /*
   * Check if we raced with another booting pset of the same type,
   * and this node has already been created.
   */
  if ((node = pset_node_for_pset_cluster_type(pset_cluster_type)) !=
      PSET_NODE_NULL) {
...
processor_set_t pset_create(cluster_type_t cluster_type,
                            uint32_t pset_cluster_id, int pset_id) {
  ...
  pset_cluster_type_t pset_cluster_type;
#if __AMP__
  pset_cluster_type = cluster_type_to_pset_cluster_type(cluster_type);
  node = pset_node_for_pset_cluster_type(pset_cluster_type);
  if (node == PSET_NODE_NULL) {
    /* First pset of this cluster type */
    node = pset_node_create(cluster_type);
  }
#else  /* !__AMP__ */
  pset_cluster_type = PSET_SMP;
  node = &pset_node0;
  (void)cluster_type;
#endif /* !__AMP__ */
 ...
 */
processor_set_t pset_find(uint32_t cluster_id, processor_set_t default_pset) {
  lck_spin_lock(&pset_node_lock);
 ...
 */
void pset_init(processor_set_t pset, pset_node_t node) {
  ...
  processor_set_t *prev = &node->psets;
...
kern_return_t processor_info_count(processor_flavor_t flavor,
...
void processor_cpu_load_info(processor_t processor,
  ...
  ticks[CPU_STATE_USER] +=
      (uint32_t)(usage.ru_metrics[RCT_LVL_USER].rm_time_mach /
                 hz_tick_interval);
  ticks[CPU_STATE_SYSTEM] +=
      (uint32_t)(recount_usage_system_time_mach(&usage) / hz_tick_interval);
  ticks[CPU_STATE_IDLE] += (uint32_t)(idle_time / hz_tick_interval);
}

kern_return_t processor_info(processor_t processor, processor_flavor_t flavor,
                             host_t *host, processor_info_t info,
                             mach_msg_type_number_t *count) {
  int cpu_id, state;
 ...
 * relax the timeout.
 */
TUNABLE(uint32_t, cpu_boot_timeout_secs, "cpu_boot_timeout_secs",
        1); /* seconds, default to 1 second */

static const char *processor_start_panic_strings[] = {
    [PROCESSOR_FIRST_BOOT] = "boot for the first time",
    [PROCESSOR_BEFORE_ENTERING_SLEEP] =
        "come online while entering system sleep",
    [PROCESSOR_WAKE_FROM_SLEEP] =
        "come online after returning from system sleep",
    [PROCESSOR_CLUSTER_POWERDOWN_SUSPEND] =
        "come online while disabling cluster powerdown",
    [PROCESSOR_CLUSTER_POWERDOWN_RESUME] =
        "come online before enabling cluster powerdown",
    [PROCESSOR_POWERED_CORES_CHANGE] =
        "come online during dynamic cluster power state change",
};

void processor_wait_for_start(processor_t processor,
                              processor_start_kind_t start_kind) {
  if (!processor->processor_booted) {
    panic("processor_boot() missing for cpu %d", processor->cpu_id);
  }

  uint32_t boot_timeout_extended =
      cpu_boot_timeout_secs * debug_cpu_performance_degradation_factor;

  spl_t s = splsched();
  simple_lock(&processor_start_state_lock, LCK_GRP_NULL);
  while (processor->processor_instartup) {
    assert_wait_timeout((event_t)&processor->processor_instartup, THREAD_UNINT,
                        boot_timeout_extended, NSEC_PER_SEC);
    ...
    if (wait_result == THREAD_TIMED_OUT) {
      panic("cpu %d failed to %s, waited %d seconds\n", processor->cpu_id,
            processor_start_panic_strings[start_kind], boot_timeout_extended);
    }
  ...
  if (processor->processor_inshutdown) {
    panic("%s>cpu %d still in shutdown", __func__, processor->cpu_id);
  }
  ...
  if (!processor->processor_online) {
    panic("%s>cpu %d not online", __func__, processor->cpu_id);
  }

  if (processor->processor_offline_state ==
...
static void processor_dostartup(processor_t processor, bool first_boot) {
  ...
  if (first_boot) {
    ...
    assert(processor->processor_offline_state ==
           PROCESSOR_OFFLINE_FULLY_OFFLINE);
  }
  ...
  if (processor->processor_offline_state == PROCESSOR_OFFLINE_STARTING) {
    processor_update_offline_state_locked(
        processor, PROCESSOR_OFFLINE_STARTED_NOT_RUNNING);
  } else {
    assert(processor->processor_offline_state ==
           PROCESSOR_OFFLINE_STARTED_NOT_WAITED);
  }
...
void processor_exit_reason(processor_t processor, processor_reason_t reason,
  ...
  bool is_final_system_sleep =
      is_system_sleep && (processor == master_processor);
 ...
 */
kern_return_t processor_exit_from_kext(__unused processor_t processor) {
  /* This is a no-op now. */
  return KERN_FAILURE;
}

void processor_sleep(processor_t processor) {
  lck_mtx_assert(&cluster_powerdown_lock, LCK_MTX_ASSERT_OWNED);
...
void processor_start_reason(processor_t processor, processor_reason_t reason) {
  lck_mtx_lock(&processor_updown_lock);
 ...
 */
kern_return_t processor_start_from_kext(__unused processor_t processor) {
  /* This is a no-op now. */
  return KERN_FAILURE;
}
 ...
 */
void processor_boot(processor_t processor) {
  lck_mtx_lock(&cluster_powerdown_lock);
  lck_mtx_lock(&processor_updown_lock);
 ...
 */
void processor_wake(processor_t processor) {
  lck_mtx_assert(&cluster_powerdown_lock, LCK_MTX_ASSERT_OWNED);
...
#if CONFIG_SCHED_SMT
kern_return_t enable_smt_processors(bool enable) {
...
bool processor_should_kprintf(processor_t processor, bool starting) {
  processor_reason_t reason = starting ? processor->last_startup_reason
                                       : processor->last_shutdown_reason;

  return reason != REASON_CLPC_SYSTEM;
}

kern_return_t processor_control(processor_t processor, processor_info_t info,
                                mach_msg_type_number_t count) {
  if (processor == PROCESSOR_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
...
kern_return_t processor_set_info(processor_set_t pset, int flavor, host_t *host,
 ...
 */
kern_return_t processor_set_statistics(processor_set_t pset, int flavor,
                                       processor_set_info_t info,
                                       mach_msg_type_number_t *count) {
 ...
 */
static kern_return_t processor_set_things(processor_set_t pset,
                                          mach_port_array_t *thing_list,
                                          mach_msg_type_number_t *countp,
                                          int type, mach_task_flavor_t flavor) {
  unsigned int i;
  task_t task;
  thread_t thread;
  ...
  for (;;) {
    ...
    if (task_count_needed > task_count_cur) {
      ...
      task_addr = mach_port_array_alloc(task_count_cur, Z_WAITOK | Z_ZERO);
      if (task_addr == NULL) {
    ...
    if (thread_count_needed > thread_count_cur) {
      ...
      thread_addr = mach_port_array_alloc(thread_count_cur, Z_WAITOK | Z_ZERO);
      if (thread_addr == NULL) {
        mach_port_array_free(task_addr, task_count_cur);
        return KERN_RESOURCE_SHORTAGE;
      }
 ...
 */
static kern_return_t processor_set_tasks_internal(processor_set_t pset,
...
kern_return_t processor_set_tasks(processor_set_t pset, task_array_t *task_list,
                                  mach_msg_type_number_t *count) {
  return processor_set_tasks_internal(pset, task_list, count,
                                      TASK_FLAVOR_CONTROL);
}
 ...
 *
 *	Based on flavor, return task/inspect/read port to all tasks in the
 * processor set.
 */
kern_return_t processor_set_tasks_with_flavor(processor_set_t pset,
                                              mach_task_flavor_t flavor,
                                              task_array_t *task_list,
                                              mach_msg_type_number_t *count) {
  switch (flavor) {
 ...
 */
#if defined(SECURE_KERNEL)
kern_return_t processor_set_threads(__unused processor_set_t pset,
...
#else
kern_return_t processor_set_threads(processor_set_t pset,
                                    thread_act_array_t *thread_list,
                                    mach_msg_type_number_t *count) {
  kern_return_t ret;

  ret = processor_set_things(pset, thread_list, count, PSET_THING_THREAD,
                             TASK_FLAVOR_CONTROL);
  if (ret != KERN_SUCCESS) {
    return ret;
  }
...
pset_cluster_type_t recommended_pset_type(thread_t thread) {
  ...
  if (thread->base_pri <= MAXPRI_THROTTLE) {
    if (os_atomic_load(&sched_perfctl_policy_bg, relaxed) !=
        SCHED_PERFCTL_POLICY_FOLLOW_GROUP) {
      return PSET_AMP_E;
    }
  } else if (thread->base_pri <= BASEPRI_UTILITY) {
    if (os_atomic_load(&sched_perfctl_policy_util, relaxed) !=
        SCHED_PERFCTL_POLICY_FOLLOW_GROUP) {
      return PSET_AMP_E;
    }
  }
...
void sched_perfcontrol_inherit_recommendation_from_tg(
    perfcontrol_class_t perfctl_class, boolean_t inherit) {
  sched_perfctl_class_policy_t sched_policy =
      inherit ? SCHED_PERFCTL_POLICY_FOLLOW_GROUP
              : SCHED_PERFCTL_POLICY_RESTRICT_E;

  KDBG(MACHDBG_CODE(DBG_MACH_SCHED, MACH_AMP_PERFCTL_POLICY_CHANGE) |
           DBG_FUNC_NONE,
       perfctl_class, sched_policy, 0, 0);
...
void sched_perfcontrol_inherit_recommendation_from_tg(
    __unused perfcontrol_class_t perfctl_class, __unused boolean_t inherit) {}
