...
#define timer_queue_lock_spin(queue)                                           \
  lck_ticket_lock(&(queue)->lock_data, &timer_queue_lck_grp)
...
TUNABLE_WRITEABLE(uint64_t, timer_scan_interval_us, "timer_scan_interval_us",
                  40);
...
static void timer_longterm_callout(timer_call_param_t p0,
                                   timer_call_param_t p1);
extern void timer_longterm_scan(timer_longterm_t *tlp, uint64_t now);
static void timer_longterm_update(timer_longterm_t *tlp);
static void timer_longterm_update_locked(timer_longterm_t *tlp);
static mpqueue_head_t *timer_longterm_enqueue_unlocked(
    timer_call_t call, uint64_t now, uint64_t deadline,
    mpqueue_head_t **old_queue, uint64_t soft_deadline, uint64_t ttd,
    timer_call_param_t param1, uint32_t callout_flags);
static void timer_longterm_dequeued_locked(timer_call_t call);
...
static boolean_t timer_call_enter_internal(timer_call_t call,
...
mpqueue_head_t *
timer_call_enqueue_deadline_unlocked(timer_call_t call, mpqueue_head_t *queue,
                                     uint64_t deadline, uint64_t soft_deadline,
                                     uint64_t ttd, timer_call_param_t param1,
                                     uint32_t flags);

mpqueue_head_t *timer_call_dequeue_unlocked(timer_call_t call);
...
static void timer_call_init_abstime(void) {
  int i;
  uint64_t result;
  timer_coalescing_priority_params_ns_t *tcoal_prio_params_init =
      timer_call_get_priority_params();
  nanoseconds_to_absolutetime(PAST_DEADLINE_TIMER_ADJUSTMENT_NS,
                              &past_deadline_timer_adjustment);
  nanoseconds_to_absolutetime(
      tcoal_prio_params_init
          ->idle_entry_timer_processing_hdeadline_threshold_ns,
      &result);
  tcoal_prio_params.idle_entry_timer_processing_hdeadline_threshold_abstime =
      (uint32_t)result;
  nanoseconds_to_absolutetime(
      tcoal_prio_params_init->interrupt_timer_coalescing_ilat_threshold_ns,
      &result);
  tcoal_prio_params.interrupt_timer_coalescing_ilat_threshold_abstime =
      (uint32_t)result;
  nanoseconds_to_absolutetime(tcoal_prio_params_init->timer_resort_threshold_ns,
                              &result);
  tcoal_prio_params.timer_resort_threshold_abstime = (uint32_t)result;
  tcoal_prio_params.timer_coalesce_rt_shift =
      tcoal_prio_params_init->timer_coalesce_rt_shift;
  tcoal_prio_params.timer_coalesce_bg_shift =
      tcoal_prio_params_init->timer_coalesce_bg_shift;
  tcoal_prio_params.timer_coalesce_kt_shift =
      tcoal_prio_params_init->timer_coalesce_kt_shift;
  tcoal_prio_params.timer_coalesce_fp_shift =
      tcoal_prio_params_init->timer_coalesce_fp_shift;
  tcoal_prio_params.timer_coalesce_ts_shift =
      tcoal_prio_params_init->timer_coalesce_ts_shift;
  ...
  for (i = 0; i < NUM_LATENCY_QOS_TIERS; i++) {
    tcoal_prio_params.latency_qos_scale[i] =
        tcoal_prio_params_init->latency_qos_scale[i];
    nanoseconds_to_absolutetime(tcoal_prio_params_init->latency_qos_ns_max[i],
                                &tcoal_prio_params.latency_qos_abstime_max[i]);
    tcoal_prio_params.latency_tier_rate_limited[i] =
        tcoal_prio_params_init->latency_tier_rate_limited[i];
  }
...
void timer_call_init(void) {
  timer_longterm_init();
  timer_call_init_abstime();
}

void timer_call_queue_init(mpqueue_head_t *queue) {
  DBG("timer_call_queue_init(%p)\n", queue);
  mpqueue_init(queue, &timer_call_lck_grp, LCK_ATTR_NULL);
}

void timer_call_setup(timer_call_t call, timer_call_func_t func,
...
timer_call_t timer_call_alloc(timer_call_func_t func,
                              timer_call_param_t param0) {
  timer_call_t call;
...
void timer_call_free(timer_call_t call) { kfree_type(struct timer_call, call); }

static mpqueue_head_t *mpqueue_for_timer_call(timer_call_t entry) {
...
static __inline__ mpqueue_head_t *timer_call_entry_dequeue(timer_call_t entry) {
  ...
  if (old_mpqueue != timer_longterm_queue) {
    priority_queue_remove(&old_mpqueue->mpq_pqhead, &entry->tc_pqlink);
  }
...
static __inline__ mpqueue_head_t *timer_call_entry_enqueue_deadline(
    timer_call_t entry, mpqueue_head_t *new_mpqueue, uint64_t deadline) {
  mpqueue_head_t *old_mpqueue = mpqueue_for_timer_call(entry);
  ...
  timer_call_t thead = priority_queue_min(&new_mpqueue->mpq_pqhead,
                                          struct timer_call, tc_pqlink);

  new_mpqueue->earliest_soft_deadline = thead->tc_flags & TIMER_CALL_RATELIMITED
                                            ? thead->tc_pqlink.deadline
                                            : thead->tc_soft_deadline;
...
static __inline__ void timer_call_entry_enqueue_tail(timer_call_t entry,
 ...
 */
static __inline__ void timer_call_entry_dequeue_async(timer_call_t entry) {
  mpqueue_head_t *old_mpqueue = mpqueue_for_timer_call(entry);
  if (old_mpqueue) {
    old_mpqueue->count--;
...
#if TIMER_ASSERT
      TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_ASYNC_DEQ | DBG_FUNC_NONE,
                         VM_KERNEL_UNSLIDE_OR_PERM(call),
                         call->tc_async_dequeue,
                         VM_KERNEL_UNSLIDE_OR_PERM(call->tc_queue), 0x1c, 0);
...
#if TIMER_ASSERT
  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_ASYNC_DEQ | DBG_FUNC_NONE,
                     VM_KERNEL_UNSLIDE_OR_PERM(call), call->tc_async_dequeue,
                     VM_KERNEL_UNSLIDE_OR_PERM(call->tc_queue), 0, 0);
#endif
  if (old_queue != NULL) {
    timer_queue_lock_spin(old_queue);
    if (call->tc_async_dequeue) {
...
static boolean_t timer_call_enter_internal(timer_call_t call,
  ...
  slop = timer_call_slop(deadline, ctime, urgency, current_thread(),
                         &slop_ratelimited);
  ...
  DTRACE_TMR7(callout__create, timer_call_func_t, call->tc_func,
              ...
              (deadline - sdeadline), (ttd >> 32), (unsigned)(ttd & 0xFFFFFFFF),
              call);
#endif
  ...
  if (queue == NULL) {
    ...
    old_queue = timer_call_enqueue_deadline_unlocked(
        call, queue, deadline, sdeadline, ttd, param1, flags);
  }
  ...
  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_ENTER | DBG_FUNC_END,
                     VM_KERNEL_UNSLIDE_OR_PERM(call), (old_queue != NULL),
                     deadline, queue->count, 0);
 ...
 */
boolean_t timer_call_enter(timer_call_t call, uint64_t deadline,
                           uint32_t flags) {
  return timer_call_enter_internal(call, NULL, deadline, 0, flags, FALSE);
}

boolean_t timer_call_enter1(timer_call_t call, timer_call_param_t param1,
                            uint64_t deadline, uint32_t flags) {
  return timer_call_enter_internal(call, param1, deadline, 0, flags, FALSE);
}

boolean_t timer_call_enter_with_leeway(timer_call_t call,
                                       timer_call_param_t param1,
                                       uint64_t deadline, uint64_t leeway,
                                       uint32_t flags, boolean_t ratelimited) {
  return timer_call_enter_internal(call, param1, deadline, leeway, flags,
                                   ratelimited);
}
...
boolean_t timer_call_cancel(timer_call_t call) {
  ...
  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_CANCEL | DBG_FUNC_START,
                     VM_KERNEL_UNSLIDE_OR_PERM(call), call->tc_pqlink.deadline,
                     call->tc_soft_deadline, call->tc_flags, 0);
  ...
  if (old_queue != NULL) {
    ...
    timer_call_t new_head = priority_queue_min(&old_queue->mpq_pqhead,
                                               struct timer_call, tc_pqlink);

    if (new_head) {
      timer_queue_cancel(old_queue, call->tc_pqlink.deadline,
                         new_head->tc_pqlink.deadline);
      old_queue->earliest_soft_deadline =
          new_head->tc_flags & TIMER_CALL_RATELIMITED
              ? new_head->tc_pqlink.deadline
              : new_head->tc_soft_deadline;
    } else {
...
void timer_queue_shutdown(__kdebug_only int target_cpu, mpqueue_head_t *queue,
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE, DECR_TIMER_SHUTDOWN | DBG_FUNC_START,
                            target_cpu, queue->earliest_soft_deadline, 0, 0, 0);

  while (TRUE) {
    timer_queue_lock_spin(queue);

    timer_call_t call =
        qe_queue_first(&queue->head, struct timer_call, tc_qlink);
    ...
    if (!simple_lock_try(&call->tc_lock, LCK_GRP_NULL)) {
      ...
      TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_ASYNC_DEQ | DBG_FUNC_NONE,
                         VM_KERNEL_UNSLIDE_OR_PERM(call),
                         call->tc_async_dequeue,
                         VM_KERNEL_UNSLIDE_OR_PERM(call->tc_queue), 0x2b, 0);
#endif
      timer_queue_unlock(queue);
      continue;
    }
    ...
    if (call_local == FALSE) {
      ...
      timer_call_entry_enqueue_deadline(call, new_queue,
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE, DECR_TIMER_SHUTDOWN | DBG_FUNC_END,
                            target_cpu, ntimers_moved, lock_skips,
...
uint64_t timer_queue_expire_with_options(mpqueue_head_t *queue,
                                         uint64_t deadline, boolean_t rescan) {
  ...
  while (!queue_empty(&queue->head)) {
    if (++tc_iterations > 1) {
      ...
      if (rescan == FALSE && now > time_limit) {
        TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_PAUSE | DBG_FUNC_NONE,
                           queue->count, tc_iterations - 1, 0, 0, 0);
    ...
    if (call == NULL) {
      if (rescan == FALSE) {
        call = priority_queue_min(&queue->mpq_pqhead, struct timer_call,
                                  tc_pqlink);
      } else {
    ...
    if (call->tc_soft_deadline <= cur_deadline) {
      ...
      TCOAL_DEBUG(0xDDDD0000, queue->earliest_soft_deadline,
                  call->tc_soft_deadline, 0, 0, 0);
      TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_EXPIRE | DBG_FUNC_NONE,
                         VM_KERNEL_UNSLIDE_OR_PERM(call),
                         call->tc_soft_deadline, call->tc_pqlink.deadline,
      ...
      TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_CALLOUT | DBG_FUNC_START,
                         VM_KERNEL_UNSLIDE_OR_PERM(call),
                         VM_KERNEL_UNSLIDE(func), VM_KERNEL_ADDRHIDE(param0),
                         VM_KERNEL_ADDRHIDE(param1), 0);

#if CONFIG_DTRACE
      DTRACE_TMR7(callout__start, timer_call_func_t, func, timer_call_param_t,
                  param0, unsigned, call->tc_flags, 0, (call->tc_ttd >> 32),
      ...
      *ttdp = 0;
#if CONFIG_DTRACE
      DTRACE_TMR4(callout__end, timer_call_func_t, func, param0, param1, call);
#endif
      ...
      if (__probable(rescan == FALSE)) {
        ...
        if (timer_resort_threshold(skew)) {
          if (__probable(simple_lock_try(&call->tc_lock, LCK_GRP_NULL))) {
            ...
            timer_call_entry_dequeue(call);
            timer_call_entry_enqueue_deadline(call, queue,
  ...
  if (call) {
    ...
    if (time_limit_deadline > call->tc_pqlink.deadline) {
      TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_POSTPONE | DBG_FUNC_NONE,
                         VM_KERNEL_UNSLIDE_OR_PERM(call),
                         call->tc_pqlink.deadline, time_limit_deadline, 0, 0);
      counter_inc(&timer_scan_postpones_cnt);
      cur_deadline = time_limit_deadline;
    } else {
      cur_deadline = call->tc_pqlink.deadline;
    }
    queue->earliest_soft_deadline = (call->tc_flags & TIMER_CALL_RATELIMITED)
                                        ? call->tc_pqlink.deadline
                                        : call->tc_soft_deadline;
  } else {
...
uint64_t timer_queue_expire(mpqueue_head_t *queue, uint64_t deadline) {
 ...
 */
int timer_queue_migrate(mpqueue_head_t *queue_from, mpqueue_head_t *queue_to) {
  timer_call_t call;
  ...
  head_to =
      priority_queue_min(&queue_to->mpq_pqhead, struct timer_call, tc_pqlink);
  ...
  call =
      priority_queue_min(&queue_from->mpq_pqhead, struct timer_call, tc_pqlink);
  ...
  qe_foreach_element_safe(call, &queue_from->head, tc_qlink) {
    if (!simple_lock_try(&call->tc_lock, LCK_GRP_NULL)) {
      ...
      TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_ASYNC_DEQ | DBG_FUNC_NONE,
                         VM_KERNEL_UNSLIDE_OR_PERM(call),
                         VM_KERNEL_UNSLIDE_OR_PERM(call->tc_queue), 0, 0x2b, 0);
#endif
    ...
    timer_call_entry_dequeue(call);
    timer_call_entry_enqueue_deadline(call, queue_to, call->tc_pqlink.deadline);
    timers_migrated++;
    simple_unlock(&call->tc_lock);
  }
...
void timer_queue_trace_cpu(int ncpu) {
  timer_call_nosync_cpu(ncpu, (void (*)(void *))timer_queue_trace,
                        (void *)timer_queue_cpu(ncpu));
}

void timer_queue_trace(mpqueue_head_t *queue) {
  timer_call_t call;
  spl_t s;
  ...
  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_QUEUE | DBG_FUNC_START,
                     queue->count, mach_absolute_time(), 0, 0, 0);

  qe_foreach_element(call, &queue->head, tc_qlink) {
    TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_QUEUE | DBG_FUNC_NONE,
                       call->tc_soft_deadline, call->tc_pqlink.deadline,
                       call->tc_entry_time, VM_KERNEL_UNSLIDE(call->tc_func),
                       0);
  }

  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_QUEUE | DBG_FUNC_END,
                     queue->count, mach_absolute_time(), 0, 0, 0);
...
void timer_longterm_dequeued_locked(timer_call_t call) {
 ...
 */
mpqueue_head_t *timer_longterm_enqueue_unlocked(
    timer_call_t call, uint64_t now, uint64_t deadline,
    mpqueue_head_t **old_queue, uint64_t soft_deadline, uint64_t ttd,
    timer_call_param_t param1, uint32_t callout_flags) {
  timer_longterm_t *tlp = &timer_longterm;
  ...
  *old_queue = timer_call_dequeue_unlocked(call);
  ...
  if (update_required) {
     ...
     */
    timer_call_nosync_cpu(master_cpu, (void (*)(void *))timer_longterm_update,
                          (void *)tlp);
  }
 ...
 */
void timer_longterm_scan(timer_longterm_t *tlp, uint64_t time_start) {
  timer_call_t call;
  ...
  qe_foreach_element_safe(call, &timer_longterm_queue->head, tc_qlink) {
    ...
    if (!simple_lock_try(&call->tc_lock, LCK_GRP_NULL)) {
      ...
      TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_ASYNC_DEQ | DBG_FUNC_NONE,
                         VM_KERNEL_UNSLIDE_OR_PERM(call),
                         VM_KERNEL_UNSLIDE_OR_PERM(call->tc_queue), 0, 0x2c, 0);
#endif
      timer_call_entry_dequeue_async(call);
      continue;
    }
    if (deadline < threshold) {
      ...
      if (deadline < time_start) {
        TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_OVERDUE | DBG_FUNC_NONE,
                           VM_KERNEL_UNSLIDE_OR_PERM(call), deadline,
                           time_start, threshold, 0);
      }
#endif
      TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_ESCALATE | DBG_FUNC_NONE,
                         VM_KERNEL_UNSLIDE_OR_PERM(call),
                         call->tc_pqlink.deadline, call->tc_entry_time,
                         VM_KERNEL_UNSLIDE(call->tc_func), 0);
      tlp->escalates++;
      timer_call_entry_dequeue(call);
      timer_call_entry_enqueue_deadline(call, timer_master_queue,
                                        call->tc_pqlink.deadline);
      /*
...
void timer_longterm_callout(timer_call_param_t p0,
...
void timer_longterm_update_locked(timer_longterm_t *tlp) {
  uint64_t latency;

  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_UPDATE | DBG_FUNC_START,
  ...
  if (tlp->threshold.preempted != TIMER_LONGTERM_NONE) {
    ...
    if (latency < tlp->threshold.interval) {
      tlp->threshold.latency_min = MIN(tlp->threshold.latency_min, latency);
      tlp->threshold.latency_max = MAX(tlp->threshold.latency_max, latency);
      tlp->threshold.latency = (tlp->threshold.latency * 99 + latency) / 100;
    }
  ...
  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_UPDATE | DBG_FUNC_END,
                     VM_KERNEL_UNSLIDE_OR_PERM(&tlp->queue),
                     tlp->threshold.deadline, tlp->threshold.scans,
                     tlp->queue.count, 0);
}

void timer_longterm_update(timer_longterm_t *tlp) {
  spl_t s = splclock();
  ...
  if (tlp->threshold.deadline != TIMER_LONGTERM_NONE) {
    timer_call_enter(&tlp->threshold.timer, tlp->threshold.deadline_set,
...
void timer_longterm_init(void) {
  ...
  tlp->threshold.interval =
      serverperfmode ? TIMER_LONGTERM_NONE : TIMER_LONGTERM_THRESHOLD;
  if (PE_parse_boot_argn("longterm", &longterm, sizeof(longterm))) {
    tlp->threshold.interval =
        (longterm == 0) ? TIMER_LONGTERM_NONE : longterm * NSEC_PER_MSEC;
  }
  if (tlp->threshold.interval != TIMER_LONGTERM_NONE) {
  ...
  timer_call_setup(&tlp->threshold.timer, timer_longterm_callout,
                   (timer_call_param_t)tlp);
...
uint64_t timer_sysctl_get(int oid) {
  timer_longterm_t *tlp = &timer_longterm;
 ...
 */
static void timer_master_scan(timer_longterm_t *tlp, uint64_t now) {
  timer_call_t call;
  uint64_t threshold;
  uint64_t deadline;
  mpqueue_head_t *timer_master_queue;
...
static void timer_sysctl_set_threshold(void *valp) {
...
int timer_sysctl_set(__unused int oid, __unused uint64_t value) {
  ...
  switch (oid) {
  case THRESHOLD:
    timer_call_cpu(master_cpu, timer_sysctl_set_threshold, (void *)value);
    return KERN_SUCCESS;
  case LONG_TERM_SCAN_LIMIT:
  ...
  case LONG_TERM_SCAN_INTERVAL:
    ...
    return KERN_SUCCESS;
  case SCAN_LIMIT:
    if (value > MAX_TIMER_SCAN_LIMIT || value < MIN_TIMER_SCAN_LIMIT) {
    ...
    return KERN_SUCCESS;
  case SCAN_INTERVAL:
    if (value > MAX_TIMER_SCAN_INTERVAL || value < MIN_TIMER_SCAN_INTERVAL) {
      return KERN_INVALID_ARGUMENT;
    }
...
static boolean_t tcoal_qos_adjust(thread_t t, int32_t *tshift,
                                  uint64_t *tmax_abstime,
                                  boolean_t *pratelimited) {
  ...
  if (ctask) {
    ...
    if (latency_qos) {
      ...
      *tmax_abstime =
          tcoal_prio_params.latency_qos_abstime_max[latency_qos - 1];
      *pratelimited =
          tcoal_prio_params.latency_tier_rate_limited[latency_qos - 1];
      adjusted = TRUE;
    }
  }
  return adjusted;
 ...
 */
static void timer_compute_leeway(thread_t cthread, int32_t urgency,
                                 int32_t *tshift, uint64_t *tmax_abstime,
                                 boolean_t *pratelimited) {
  ...
  if ((urgency & TIMER_CALL_USER_MASK) != 0) {
    ...
    tg_critical = tg_flags & (THREAD_GROUP_FLAGS_CRITICAL |
                              THREAD_GROUP_FLAGS_STRICT_TIMERS);
#endif /* CONFIG_THREAD_GROUPS */
    bool timer_critical = (tpri >= BASEPRI_RTQUEUES) ||
                          (urgency == TIMER_CALL_USER_CRITICAL) || tg_critical;
    if (timer_critical) {
    ...
    } else if (proc_get_effective_thread_policy(cthread,
...
uint64_t timer_call_slop(uint64_t deadline, uint64_t now, uint32_t flags,
                         thread_t cthread, boolean_t *pratelimited) {
  int32_t tcs_shift = 0;
  uint64_t tcs_max_abstime = 0;
  uint64_t adjval;
  uint32_t urgency = (flags & TIMER_CALL_URGENCY_MASK);

  if (mach_timer_coalescing_enabled && (deadline > now) &&
      (urgency != TIMER_CALL_SYS_CRITICAL)) {
    timer_compute_leeway(cthread, urgency, &tcs_shift, &tcs_max_abstime,
                         pratelimited);
...
#define RUNNING_TIMER_FAKE_FLAGS (TIMER_CALL_SYS_CRITICAL | TIMER_CALL_LOCAL)
...
static void timer_call_trace_enter_before(struct timer_call *call,
                                          uint64_t deadline, uint32_t flags,
                                          uint64_t now) {
#pragma unused(call, deadline, flags, now)
  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_ENTER | DBG_FUNC_START,
                     VM_KERNEL_UNSLIDE_OR_PERM(call),
                     VM_KERNEL_ADDRHIDE(call->tc_param1), deadline, flags, 0);
#if CONFIG_DTRACE
...
static void timer_call_trace_enter_after(struct timer_call *call,
                                         uint64_t deadline) {
...
static void timer_call_trace_cancel(struct timer_call *call) {
#pragma unused(call)
...
static void timer_call_trace_expire_entry(struct timer_call *call) {
#pragma unused(call)
  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_CALLOUT | DBG_FUNC_START,
                     VM_KERNEL_UNSLIDE_OR_PERM(call),
                     VM_KERNEL_UNSLIDE(call->tc_func),
                     VM_KERNEL_ADDRHIDE(call->tc_param0),
                     VM_KERNEL_ADDRHIDE(call->tc_param1), 0);
#if CONFIG_DTRACE
#if TIMER_TRACE
  ...
  DTRACE_TMR7(callout__start, timer_call_func_t, call->tc_func,
              timer_call_param_t, call->tc_param0, unsigned, call->tc_flags, 0,
              (ttd >> 32), (unsigned int)(ttd & 0xFFFFFFFF), NULL);
#endif /* CONFIG_DTRACE */
}

static void timer_call_trace_expire_return(struct timer_call *call) {
#pragma unused(call)
#if CONFIG_DTRACE
  DTRACE_TMR4(callout__end, timer_call_func_t, call->tc_func, call->tc_param0,
              call->tc_param1, NULL);
#endif /* CONFIG_DTRACE */
  TIMER_KDEBUG_TRACE(KDEBUG_TRACE, DECR_TIMER_CALLOUT | DBG_FUNC_END,
                     VM_KERNEL_UNSLIDE_OR_PERM(call),
...
void running_timer_setup(processor_t processor, enum running_timer timer,
  ...
  timer_call_trace_enter_before(call, deadline, RUNNING_TIMER_FAKE_FLAGS, now);
...
void running_timers_sync(void) { timer_resync_deadlines(); }

void running_timer_enter(processor_t processor, unsigned int timer, void *param,
...
void running_timer_cancel(processor_t processor, unsigned int timer) {
...
uint64_t running_timers_deadline(processor_t processor) {
  ...
  for (int i = 0; i < RUNNING_TIMER_MAX; i++) {
    uint64_t candidate = processor->running_timers[i].tc_pqlink.deadline;
