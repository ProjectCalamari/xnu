...
static inline __pure2 struct debugger_state *current_debugger_state(void) {
...
/*
 *  Usage:
 *  panic_test_action_count is in the context of other flags, e.g. for IO errors
 * it is "succeed this many times then fail" and for nesting it is "panic this
 * many times then succeed" panic_test_failure_mode is a bit map of things to do
 *  panic_test_case is what sort of test we are injecting
 *
 *  For more details see definitions in debugger.h
 *
 *  Note that not all combinations are sensible, but some actions can be
 * combined, e.g.
 *  - BADPTR+SPIN with action count = 3 will cause panic->panic->spin
 *  - BADPTR with action count = 2 will cause 2 nested panics (in addition to
 * the initial panic)
 *  - IO_ERR with action 15 will cause 14 successful IOs, then fail on the next
 * one
 */
#if DEVELOPMENT || DEBUG
#define INJECT_NESTED_PANIC_IF_REQUESTED(requested)                            \
  ...
  if ((panic_test_case & requested) && panic_test_action_count) {              \
    ...
    if ((panic_test_failure_mode & PANIC_TEST_FAILURE_MODE_SPIN) &&            \
        (!panic_test_action_count)) {                                          \
      printf("inject spin...\n");                                              \
      while (panic_test_badpointer)                                            \
        ;                                                                      \
    }                                                                          \
    if ((panic_test_failure_mode & PANIC_TEST_FAILURE_MODE_BADPTR) &&          \
        (panic_test_action_count + 1)) {                                       \
      printf("inject badptr...\n");                                            \
      *panic_test_badpointer = 0;                                              \
    }                                                                          \
    if ((panic_test_failure_mode & PANIC_TEST_FAILURE_MODE_PANIC) &&           \
        (panic_test_action_count + 1)) {                                       \
      printf("inject panic...\n");                                             \
...
void panic_trap_to_debugger(const char *panic_format_str, va_list *panic_args,
                            unsigned int reason, void *ctx,
                            uint64_t panic_options_mask, void *panic_data,
                            unsigned long panic_caller,
                            const char *panic_initiator) __dead2
    __printflike(1, 0);
...
static bool awl_pm_state_change_cbk(void *param, enum cpu_event event,
...
#define DEBUG_BUF_SIZE                                                         \
  ((3 * PAGE_SIZE) + offsetof(struct macos_panic_header, mph_data))
/* EXTENDED_DEBUG_BUF_SIZE definition is now in debug.h */
static_assert(((EXTENDED_DEBUG_BUF_SIZE % PANIC_FLUSH_BOUNDARY) == 0),
              "Extended debug buf size must match SMC alignment requirements");

char debug_buf[DEBUG_BUF_SIZE];
struct macos_panic_header *panic_info = (struct macos_panic_header *)debug_buf;
char *debug_buf_base =
    (debug_buf + offsetof(struct macos_panic_header, mph_data));
char *debug_buf_ptr =
    (debug_buf + offsetof(struct macos_panic_header, mph_data));

/*
 * We don't include the size of the panic header in the length of the data we
 * actually write. On co-processor platforms, we lose sizeof(struct
 * macos_panic_header) bytes from the end of the end of the log because we only
 * support writing (3*PAGESIZE) bytes.
 */
unsigned int debug_buf_size =
    (DEBUG_BUF_SIZE - offsetof(struct macos_panic_header, mph_data));
...
TUNABLE_DEV_WRITEABLE(unsigned int, verbose_panic_flow_logging,
                      "verbose_panic_flow_logging", 0);
...
__startup_func static void panic_init(void) {
  ...
  kdp_explicitly_requested =
      PE_parse_boot_argn("kdp_match_name", kdpname, sizeof(kdpname));
#endif /* defined(__arm64__) */
...
#if defined(__x86_64__)
void extended_debug_log_init(void) {
   ...
   *
   * iBoot pre-initializes the panic region with the NULL character. We set this
   * here so we can accurately calculate the CRC for the region without needing
   * to flush the full region over SMC.
   */
  ...
  debug_buf_ptr = debug_buf_base =
      (new_debug_buf + offsetof(struct macos_panic_header, mph_data));
  debug_buf_size =
      (EXTENDED_DEBUG_BUF_SIZE - offsetof(struct macos_panic_header, mph_data));
...
void debug_log_init(void) {
  ...
  /*
   * We allocate a buffer to store a panic time stackshot. If we later discover
   * that this is a system that supports flushing a stackshot via an extended
   * debug log (see above), we'll free this memory as it's not necessary on this
   * platform. This information won't be available until the IOPlatform has come
   * up.
   */
...
void phys_carveout_init(void) {
  ...
  for (int i = 0; i < (sizeof(carveouts) / sizeof(struct carveout)); i++) {
    if (carveouts[i].present) {
      ...
      if (os_mul_overflow(carveouts[i].requested_size, 1024 * 1024,
                          &temp_carveout_size)) {
        panic("%s_mb size overflowed (%uMB)", carveouts[i].name,
              carveouts[i].requested_size);
        return;
      }

      kmem_alloc_contig(kernel_map, carveouts[i].va, temp_carveout_size,
                        PAGE_MASK, 0, 0,
                        KMA_NOFAIL | KMA_PERMANENT | KMA_NOPAGEWAIT |
                            KMA_DATA_SHARED | KMA_NOSOFTLIMIT,
                        VM_KERN_MEMORY_DIAG);
...
boolean_t debug_is_in_phys_carveout(vm_map_offset_t va) {
  return phys_carveout_size && va >= phys_carveout &&
         va < (phys_carveout + phys_carveout_size);
}

boolean_t debug_can_coredump_phys_carveout(void) { return phys_carveout_core; }

static boolean_t DebuggerLock(void) {
  ...
  if (!atomic_compare_exchange_strong(&debugger_cpu, &debugger_exp_cpu,
                                      my_cpu)) {
    return false;
  }
...
static void DebuggerUnlock(void) {
  assert(atomic_load_explicit(&debugger_cpu, memory_order_relaxed) ==
         cpu_number());
...
static kern_return_t DebuggerHaltOtherCores(boolean_t proceed_on_failure,
                                            bool is_stackshot) {
#if defined(__arm64__)
...
static void DebuggerResumeOtherCores(void) {
#if defined(__arm64__)
  DebuggerXCallReturn();
#else /* defined(__arm64__) */
  mp_kdp_exit();
...
__printflike(3, 0) static void DebuggerSaveState(
  ...
  /*
   * Note:
   * if CPUDEBUGGERCOUNT == 1 then we are in the normal case - record the panic
   * data if CPUDEBUGGERCOUNT > 1 and CPUPANICSTR == NULL then we are in a
   * nested panic that happened before DebuggerSaveState was called, so store
   * the nested panic data if CPUDEBUGGERCOUNT > 1 and CPUPANICSTR != NULL then
   * we are in a nested panic that happened after DebuggerSaveState was called,
   * so leave the original panic data
   *
...
void __attribute__((noinline)) Assert(const char *file, int line,
                                      const char *expression) {
  panic_plain("%s:%d Assertion failed: %s", file, line, expression);
}

void panic_assert_format(char *buf, size_t len, struct mach_assert_hdr *hdr,
                         long a, long b) {
  ...
  case MACH_ASSERT_DEFAULT:
    ...
    snprintf(buf, len, "%s:%d Assertion failed: %s", hdr->filename, hdr->lineno,
             adef->expr);
    break;
  ...
  case MACH_ASSERT_3P:
    ...
    snprintf(buf, len,
             ...
             "%s %s %s (%p %s %p)",
             hdr->filename, hdr->lineno, a3x->a, a3x->op, a3x->b, (void *)a,
             a3x->op, (void *)b);
    break;
  ...
  case MACH_ASSERT_3S:
    ...
    snprintf(buf, len,
             ...
             hdr->filename, hdr->lineno, a3x->a, a3x->op, a3x->b, a, a3x->op, b,
             a, a3x->op, b);
    break;
...
boolean_t debug_is_current_cpu_in_panic_state(void) {
  return current_debugger_state()->db_entry_count > 0;
}

/*
 * check if we are in a nested panic, report findings, take evasive action where
 * necessary
 *
 * see also PE_update_panicheader_nestedpanic
 */
static void check_and_handle_nested_panic(uint64_t panic_options_mask,
                                          unsigned long panic_caller,
                                          const char *db_panic_str,
                                          va_list *db_panic_args) {
  if ((CPUDEBUGGERCOUNT > 1) && (CPUDEBUGGERCOUNT < max_debugger_entry_count)) {
    // Note: this is the first indication in the panic log or serial that we are
    // off the rails...
    //
    // if we panic *before* the paniclog is finalized then this will end up in
    // the ips report with a panic_caller addr that gives us a clue if we panic
    // *after* the log is finalized then we will only see it in the serial log
    //
    ...
    // print the *new* panic string to the console, we might not get it by other
    // means...
    // TODO: I tried to write this stuff to the paniclog, but the serial output
    // gets corrupted and the panicstring in the ips file is <mysterious>
...
#pragma clang diagnostic pop
  ...
  // Stage 1 bailout
  //
  // Try to complete the normal panic flow, i.e. try to make sure the callouts
  // happen and we flush the paniclog.  If this fails with another nested panic
  // then we will land in Stage 2 below...
  //
  if (CPUDEBUGGERCOUNT == max_debugger_entry_count) {
    ...
    // normally the kPEPanicBegin is sent from debugger_collect_diagnostics(),
    // but we might nested-panic before we get there.  To be safe send another
    // notification, the function called below will only send kPEPanicBegin if
    ...
    // prints to console
    paniclog_append_noflush(
        "\nNested panic stall. Stage 1 bailout. Please go to "
        "https://panic.apple.com to report this panic\n");
    panic_spin_forever();
  }

  // Stage 2 bailout
  //
  // Things are severely hosed, we have nested to the point of bailout and then
  // nested again during the bailout path.  Try to issue a chipreset as quickly
  // as possible, hopefully something in the panic log is salvageable, since we
  // flushed it during Stage 1.
  //
  if (CPUDEBUGGERCOUNT == max_debugger_entry_count + 1) {
    if (!panicDebugging) {
      // note that:
      // - this code path should be audited for prints, as that is a common
      // cause of nested panics
      // - this code path should take the fastest route to the actual reset, and
      // not call any un-necessary code
      kdp_machine_reboot_type(kPEPanicRestartCPU,
                              panic_options_mask &
                                  DEBUGGER_OPTION_SKIP_PANICEND_CALLOUTS);
    }

    // prints to console, but another nested panic will land in Stage 3 where we
    // simply spin, so that is sort of ok...
    paniclog_append_noflush(
        "\nIn Nested panic stall. Stage 2 bailout. Please go to "
        "https://panic.apple.com to report this panic\n");
    panic_spin_forever();
  }
  ...
  // Stage 3 bailout
  //
  // We are done here, we were unable to reset the platform without another
  // nested panic.  Spin until the watchdog kicks in.
  //
  if (CPUDEBUGGERCOUNT > max_debugger_entry_count + 1) {
 ...
 *  - the system is resumable on x86 (in theory, however it is not clear if this
 * is tested)
 *  - rdar://57738811 (xnu: support resume from debugger via KDP on arm devices)
 *
 */
void DebuggerWithContext(unsigned int reason, void *ctx, const char *message,
                         uint64_t debugger_options_mask,
                         unsigned long debugger_caller) {
  ...
  SOCD_TRACE_XNU(PANIC,
                 ((CPUDEBUGGERCOUNT <= 2) ? SOCD_TRACE_MODE_STICKY_TRACEPOINT
                                          : SOCD_TRACE_MODE_NONE),
                 PACK_2X32(VALUE(cpu_number()), VALUE(CPUDEBUGGERCOUNT)),
                 VALUE(debugger_options_mask), ADDR(message),
                 ADDR(debugger_caller));

  /* do max nested panic/debugger check, this will report nesting to the console
   * and spin forever if we exceed a limit */
  check_and_handle_nested_panic(debugger_options_mask, debugger_caller, message,
                                NULL);
  ...
  if (ctx != NULL) {
    DebuggerSaveState(DBOP_DEBUGGER, message, NULL, NULL, debugger_options_mask,
                      NULL, TRUE, 0, "");
    handle_debugger_trap(reason, 0, 0, ctx);
    DebuggerSaveState(DBOP_NONE, NULL, NULL, NULL, 0, NULL, FALSE, 0, "");
  } else {
    DebuggerTrapWithState(DBOP_DEBUGGER, message, NULL, NULL,
                          debugger_options_mask, NULL, TRUE, 0, NULL);
  }
 ...
 *
 * Note that producer_name and buf should never be de-allocated as we reference
 * these during panic.
 */
void register_additional_panic_data_buffer(const char *producer_name, void *buf,
                                           int len) {
  if (panic_data_buffers != NULL) {
    panic("register_additional_panic_data_buffer called with buffer already "
          "registered");
  }

  if (producer_name == NULL || (strlen(producer_name) == 0)) {
    panic("register_additional_panic_data_buffer called with invalid "
          "producer_name");
  }

  if (buf == NULL) {
    panic("register_additional_panic_data_buffer called with invalid buffer "
          "pointer");
  }
  ...
  struct additional_panic_data_buffer *new_panic_data_buffer =
      zalloc_permanent_type(struct additional_panic_data_buffer);
 ...
 *
 * Several panic wrappers (panic(), panic_with_options(), etc.) all funnel into
 * panic_trap_to_debugger(). panic_trap_to_debugger() sets the panic state in
 * the current processor's debugger_state prior to trapping into the debugger.
 * Once we trap to the debugger, we end up in handle_debugger_trap() which tries
 * to acquire the panic lock by atomically swapping the current CPU number into
 * debugger_cpu. debugger_cpu acts as a synchronization point, from which the
 * winning CPU can halt the other cores and continue to
 * debugger_collect_diagnostics() where we write the paniclog, corefile (if
 * appropriate) and proceed according to the device's boot-args.
 */
#undef panic
void panic(const char *str, ...) {
  ...
  panic_trap_to_debugger(str, &panic_str_args, 0, NULL, 0, NULL,
                         (unsigned long)(char *)__builtin_return_address(0),
                         NULL);
  va_end(panic_str_args);
}

void panic_with_data(uuid_t uuid, void *addr, uint32_t len,
  ...
  panic_trap_to_debugger(
      str, &panic_str_args, 0, NULL,
      (debugger_options_mask & ~DEBUGGER_INTERNAL_OPTIONS_MASK), NULL,
      (unsigned long)(char *)__builtin_return_address(0), NULL);
  va_end(panic_str_args);
}
...
void panic_with_options(unsigned int reason, void *ctx,
  ...
  va_start(panic_str_args, str);
  panic_trap_to_debugger(
      str, &panic_str_args, reason, ctx,
      (debugger_options_mask & ~DEBUGGER_INTERNAL_OPTIONS_MASK), NULL,
      (unsigned long)(char *)__builtin_return_address(0), NULL);
  va_end(panic_str_args);
}

void panic_with_options_and_initiator(const char *initiator,
                                      unsigned int reason, void *ctx,
                                      uint64_t debugger_options_mask,
                                      const char *str, ...) {
  va_list panic_str_args;
  ...
  panic_trap_to_debugger(
      str, &panic_str_args, reason, ctx,
      (debugger_options_mask & ~DEBUGGER_INTERNAL_OPTIONS_MASK), NULL,
      (unsigned long)(char *)__builtin_return_address(0), initiator);
  va_end(panic_str_args);
}
...
boolean_t panic_get_thread_proc_task(struct thread *thread, struct task **task,
                                     struct proc **proc) {
  ...
  if (!panic_validate_ptr(thread->t_tro->tro_proc, sizeof(struct proc *),
                          "bsd_info")) {
    *proc = NULL;
  } else {
...
#if defined(__x86_64__)
/*
 * panic_with_thread_context() is used on x86 platforms to specify a different
 * thread that should be backtraced in the paniclog. We don't generally need
 * this functionality on embedded platforms because embedded platforms include a
 * panic time stackshot from customer devices. We plumb the thread pointer via
 * the debugger trap mechanism and backtrace the kernel stack from the thread
 * when writing the panic log.
 *
 * NOTE: panic_with_thread_context() should be called with an explicit thread
 * reference held on the passed thread.
 */
void panic_with_thread_context(unsigned int reason, void *ctx,
                               uint64_t debugger_options_mask, thread_t thread,
                               const char *str, ...) {
  va_list panic_str_args;
  ...
  assertf(th_ref_count > 0,
          "panic_with_thread_context called with invalid thread %p with "
          "refcount %u",
          thread, th_ref_count);

  /* Take a reference on the thread so it doesn't disappear by the time we try
   * to backtrace it */
  ...
  panic_trap_to_debugger(
      str, &panic_str_args, reason, ctx,
      ((debugger_options_mask & ~DEBUGGER_INTERNAL_OPTIONS_MASK) |
...
__mockable void panic_trap_to_debugger(const char *panic_format_str,
...
#pragma clang diagnostic pop
  ...
  /* optionally call sync, to reduce lost logs on restart, avoid on recursive
   * panic. Unsafe due to unbounded sync() duration */
  if ((panic_options_mask & DEBUGGER_OPTION_SYNC_ON_PANIC_UNSAFE) &&
      (CPUDEBUGGERCOUNT == 0)) {
    sync_internal();
  }
  ...
  ml_panic_trap_to_debugger(panic_format_str, panic_args, reason, ctx,
                            panic_options_mask, panic_caller, panic_initiator);

  /* track depth of debugger/panic entry */
  CPUDEBUGGERCOUNT++;

  __unused uint32_t panic_initiator_crc =
      panic_initiator
          ? crc32(0, panic_initiator,
                  strnlen(panic_initiator, MAX_PANIC_INITIATOR_SIZE))
          : 0;
  ...
  SOCD_TRACE_XNU(PANIC,
                 ((CPUDEBUGGERCOUNT <= 2) ? SOCD_TRACE_MODE_STICKY_TRACEPOINT
                                          : SOCD_TRACE_MODE_NONE),
                 PACK_2X32(VALUE(cpu_number()), VALUE(CPUDEBUGGERCOUNT)),
                 PACK_2X32(VALUE(panic_initiator_crc),
                           VALUE(panic_options_mask & 0xFFFFFFFF)),
                 ADDR(panic_format_str), ADDR(panic_caller));

  /* do max nested panic/debugger check, this will report nesting to the console
   * and spin forever if we exceed a limit */
  check_and_handle_nested_panic(panic_options_mask, panic_caller,
                                panic_format_str, panic_args);

  /* If we're in a stackshot, signal that we've started panicking and wait for
   * other CPUs to coalesce and spin before proceeding */
  ...
  if (ctx != NULL) {
     ...
     */
    DebuggerSaveState(DBOP_PANIC, "panic", panic_format_str, panic_args,
  ...
  DebuggerTrapWithState(DBOP_PANIC, "panic", panic_format_str, panic_args,
                        panic_options_mask, panic_data_ptr, TRUE, panic_caller,
                        panic_initiator);
...
void __attribute__((noinline, optnone)) panic_spin_forever(void) {
  for (;;) {
#if defined(__arm__) || defined(__arm64__)
    /* On arm32, which doesn't have a WFE timeout, this may not return.  But
     * that should be OK on this path. */
    __builtin_arm_wfe();
#else
    cpu_pause();
#endif
...
static void kdp_machine_reboot_type(unsigned int type,
                                    uint64_t debugger_flags) {
  if ((type == kPEPanicRestartCPU) &&
      (debugger_flags & DEBUGGER_OPTION_SKIP_PANICEND_CALLOUTS)) {
...
void kdp_machine_reboot(void) {
  kdp_machine_reboot_type(kPEPanicRestartCPU, 0);
}

static __attribute__((unused)) void panic_debugger_log(const char *string,
                                                       ...) {
 ...
 *
 * On embedded, Debugger and Panic are treated very similarly -- WDT uses
 * Debugger so we can theoretically return from it. On desktop, Debugger is
 * treated as a conventional debugger -- i.e no paniclog is written and no core
 * is written unless we request a core on NMI.
 *
 * This routine handles kicking off local coredumps, paniclogs, calling into the
 * Debugger/KDP (if it's configured), and calling out to any other functions we
 * have for collecting diagnostic info.
 */
static void debugger_collect_diagnostics(unsigned int exception,
                                         unsigned int code,
                                         unsigned int subcode, void *state) {
#if DEVELOPMENT || DEBUG
...
#endif
  /*
   * DB_HALT (halt_in_debugger) can be requested on startup, we shouldn't
   * generate a coredump/paniclog for this type of debugger entry. If KDP isn't
   * configured, we'll just spin in kdp_raise_exception.
   */
  ...
  if ((debugger_current_op == DBOP_PANIC) ||
     ...
     * static variable rather than contents from the panic header itself in case
     * someone has stomped over the panic_info structure. Also initializes the
     * header magic.
     */
    ...
    if (CPUDEBUGGERCOUNT > 1) {
      /*
       * we are in a nested panic.  Record the nested bit in panic flags and do
       * some housekeeping
       */
      PE_update_panicheader_nestedpanic();
      paniclog_flush();
   ...
   */
  if (debugger_current_op == DBOP_PANIC) {
    paniclog_append_noflush("panic(cpu %u caller 0x%lx): ",
...
#pragma clang diagnostic pop
  ...
  /*
   * Debugger() is treated like panic() on embedded -- for example we use it for
   * WDT panics (so we need to write a paniclog). On desktop Debugger() is used
...
#endif /* __x86_64__ */
  {
    ...
    SavePanicInfo(debugger_message, debugger_panic_data, debugger_panic_options,
                  debugger_panic_initiator);

#if DEVELOPMENT || DEBUG
    INJECT_NESTED_PANIC_IF_REQUESTED(PANIC_TEST_CASE_RECURPANIC_POSTLOG);
#endif

    /* DEBUGGER_OPTION_PANICLOGANDREBOOT is used for two finger resets on
     * embedded so we get a paniclog */
    if (debugger_panic_options & DEBUGGER_OPTION_PANICLOGANDREBOOT) {
  ...
  /*
   * If reboot on panic is enabled and the caller of panic indicated that we
   * should skip local coredumps, don't try to write these and instead go
   * straight to reboot. This allows us to persist any data that's stored in the
   * panic log.
   */
  ...
  if (on_device_corefile_enabled()) {
#if CONFIG_SPTM
    /* We want to skip taking a local core dump if this is a panic from
     * SPTM/TXM/cL4. */
    ...
    (void)sptm_get_cpu_state(sptm_pcpu->sptm_cpu_id, CPUSTATE_SPTM_INTERRUPTED,
                             &sptm_interrupted);
#endif
    if (!kdp_has_polled_corefile()) {
      if (debug_boot_arg & (DB_KERN_DUMP_ON_PANIC | DB_KERN_DUMP_ON_NMI)) {
        paniclog_append_noflush(
            "skipping local kernel core because core file could not be opened "
            "prior to panic (mode : 0x%x, error : 0x%x)\n",
      ...
      paniclog_append_noflush("skipping local kernel core because the PPL is "
                              "not in KERNEL state\n");
      ...
      paniclog_append_noflush(
          "skipping local kernel core because the SPTM is in PANIC state and "
          "can't support core dump generation\n");
      panic_info->eph_panic_flags |= EMBEDDED_PANIC_HEADER_FLAG_COREDUMP_FAILED;
      paniclog_flush();
    } else if (sptm_interrupted) {
      paniclog_append_noflush(
          "skipping local kernel core because the SPTM is in INTERRUPTED state "
          "and can't support core dump generation\n");
      panic_info->eph_panic_flags |= EMBEDDED_PANIC_HEADER_FLAG_COREDUMP_FAILED;
      paniclog_flush();
    }
#endif /* XNU_MONITOR */
...
#if defined(__x86_64__)
      /* On x86 we don't do a coredump on Debugger unless the
       * DB_KERN_DUMP_ON_NMI boot-arg is specified. */
      if (debugger_current_op != DBOP_DEBUGGER ||
          (debug_boot_arg & DB_KERN_DUMP_ON_NMI))
#endif
      {
        /*
      ...
      /*
       * If DB_REBOOT_POST_CORE is set, then reboot if coredump is sucessfully
       * saved or if option to ignore failures is set.
       */
      if ((debug_boot_arg & DB_REBOOT_POST_CORE) &&
          ((ret == 0) || (debugger_panic_options &
  ...
  if (current_debugger != NO_CUR_DB) {
     ...
     */
    if (debugger_current_op == DBOP_DEBUGGER && debugger_safe_to_return &&
        kernel_debugger_entry_count == 1 && !debugger_is_panic) {
      return;
    }
  }
  ...
  if (PE_i_can_has_debugger(NULL) && panicDebugging) {
    ...
    if (debugger_panic_str) {
      ...
      panic_debugger_log("panic(cpu %u caller 0x%lx): ", (unsigned)cpu_number(),
                         debugger_panic_caller);
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wformat-nonliteral"
#pragma clang diagnostic ignored "-Wformat"
      _doprnt(debugger_panic_str, debugger_panic_args, consdebug_putc, 0);
#pragma clang diagnostic pop
    ...
    /* If panic debugging is configured and we're on a dev fused device, spin
     * for astris to connect */
    panic_spin_shmcon();
  }
#endif /* defined(__arm64__) */
...
#define DEBUGGER_TRAP_TIMESTAMP(i)                                             \
  debugger_trap_timestamps[i] = mach_absolute_time();
#else
#define DEBUGGER_TRAP_TIMESTAMP(i)
#endif /* SCHED_HYGIENE_DEBUG */

void handle_debugger_trap(unsigned int exception, unsigned int code,
                          unsigned int subcode, void *state) {
   ...
   * trapped more than twice should we call into the debugger if it's configured
   * and then reboot if the panic log has been written?
   */

  if (CPUDEBUGGEROP == DBOP_NONE) {
    /* If there was no debugger context setup, we trapped due to a software
     * breakpoint */
  ...
  if (debugger_current_op == DBOP_BREAKPOINT) {
    ...
    if (!PE_arm_debug_and_trace_initialized()) {
      paniclog_append_noflush(
          "kernel panicked before debug and trace infrastructure initialized!\n"
          "spinning forever...\n");
      panic_spin_forever();
    }
#endif
...
__attribute__((noinline, not_tail_called)) void log(__unused int level,
                                                    char *fmt, ...) {
...
boolean_t oslog_is_safe(void) {
  return kernel_debugger_entry_count == 0 && not_in_kdp == 1 &&
         get_preemption_level() == 0 && ml_get_interrupts_enabled() == TRUE;
}
...
__private_extern__ void panic_display_process_name(void) {
...
void panic_display_model_name(void) {
  ...
  if (ml_nofault_copy((vm_offset_t)&model_name, (vm_offset_t)&tmp_model_name,
                      sizeof(model_name)) != sizeof(model_name)) {
    return;
  }
...
void panic_display_kernel_uuid(void) {
  ...
  if (ml_nofault_copy(
          (vm_offset_t)&kernel_uuid_string, (vm_offset_t)&tmp_kernel_uuid,
          sizeof(kernel_uuid_string)) != sizeof(kernel_uuid_string)) {
    return;
  }
...
#if CONFIG_SPTM
static void panic_display_component_uuid(char const *component_name,
                                         void *component_address) {
  uuid_t *component_uuid;
  unsigned long component_uuid_len = 0;
  uuid_string_t component_uuid_string;

  component_uuid = getuuidfromheader((kernel_mach_header_t *)component_address,
                                     &component_uuid_len);
...
void panic_display_kernel_aslr(void) {
#if CONFIG_SPTM
  {
    ...
    if (dh != NULL) {
      ...
      case 2:
        ...
        panic_display_component_uuid("Debug Header kernelcache",
                                     component_address);
        OS_FALLTHROUGH;
      case 1:
  ...
  if (kc_format == KCFormatFileset) {
    ...
    paniclog_append_noflush("Kernel slide:      0x%016lx\n",
                            vm_kernel_stext - (unsigned long)kch +
                                vm_kernel_slide);
    ...
    paniclog_append_noflush("Kernel text exec slide: 0x%016lx\n",
                            (unsigned long)segTEXTEXECB - (unsigned long)kch +
                                vm_kernel_slide);
...
__private_extern__ void panic_display_ecc_errors(void) {
...
void panic_display_compressor_stats(void) {
  ...
  paniclog_append_noflush(
      "Compressor Info: %u%% of compressed pages limit (%s) and %u%% of "
      "segments limit (%s) with %d swapfiles and %s swap space\n",
      (incore_compressed_pages * 100) / c_segment_pages_compressed_limit,
      (incore_compressed_pages > c_segment_pages_compressed_nearing_limit)
          ? "BAD"
          : "OK",
      (incore_seg_count * 100) / c_segments_limit,
      (incore_seg_count > c_segments_nearing_limit) ? "BAD" : "OK",
      vm_num_swap_files, isswaplow ? "LOW" : "OK");
#else  /* CONFIG_FREEZE */
  paniclog_append_noflush(
      "Compressor Info: %u%% of compressed pages limit (%s) and %u%% of "
      "segments limit (%s) with %d swapfiles and %s swap space\n",
      (c_segment_pages_compressed * 100) / c_segment_pages_compressed_limit,
      (c_segment_pages_compressed > c_segment_pages_compressed_nearing_limit)
          ? "BAD"
          : "OK",
...
#if !CONFIG_TELEMETRY
int telemetry_gather(user_addr_t buffer __unused, uint32_t *length __unused,
                     bool mark __unused) {
...
__startup_func static void kern_feature_override_init(void) {
  /*
...
static boolean_t device_corefile_valid_on_ephemeral(void) {
  ...
  if (kSuccess != SecureDTGetProperty(node, "ephemeral-data-mode",
...
boolean_t panic_stackshot_to_disk_enabled(void) {
  ...
  if (PEGetCoprocessorVersion() < kCoprocessorVersion2) {
    /* Only enabled on pre-Gibraltar machines where it hasn't been disabled
     * explicitly */
    if ((debug_boot_arg != 0) &&
        (debug_boot_arg & DB_DISABLE_STACKSHOT_TO_DISK)) {
      return FALSE;
    }
...
const char *sysctl_debug_get_preoslog(size_t *size) {
  ...
  if (result || preoslog_pa == NULL || preoslog_size == 0) {
    kprintf("Couldn't obtain preoslog region: result = %d, preoslog_pa = %p, "
            "preoslog_size = %d\n",
  ...
  /*
   *  Beware:
   *  On release builds, we would need to call IODTFreeLoaderInfo("preoslog",
   * preoslog_pa, preoslog_size) to free the preoslog buffer. On Development &
   * Debug builds, we retain the buffer so it can be extracted from coredumps.
   */
  *size = preoslog_size;
...
#else
  /*  On Development & Debug builds, we retain the buffer so it can be extracted
   * from coredumps. */
#endif // RELEASE
}
...
__attribute__((optnone)) static void SPINNING_FOREVER(void) {
...
void check_for_failure_injection(failure_injection_stage_t current_stage) {
  // Can't call this function with the default initialization for
  // xnu_upsi_injection_stage
...
static inline void awl_set_scratch_reg_hv_bit(void) {
#if defined(__arm64__)
...
void awl_mark_hv_entry(void) {
  if (__probable(*PERCPU_GET(hv_entry_detected) ||
                 !awl_scratch_reg_supported)) {
    return;
  }
 ...
 */
static bool awl_pm_state_change_cbk(void *param __unused, enum cpu_event event,
                                    unsigned int cpu_or_cluster __unused) {
  if (event == CPU_BOOTED) {
...
set_awl_scratch_exists_flag_and_subscribe_for_pm(void) {
  ...
  if (base != NULL &&
      SecureDTGetProperty(base, "awl-scratch-supported", (const void **)&data,
                          &data_size) == kSuccess) {
...
STARTUP(EARLY_BOOT, STARTUP_RANK_MIDDLE,
        set_awl_scratch_exists_flag_and_subscribe_for_pm);

/**
 * Signal that the system is going down for a panic. Returns true if it is safe
 * to proceed with the panic flow, false if we should re-enable interrupts and
 * spin to allow another CPU to proceed with its panic flow.
 *
