...
kcdata_get_memory_addr_with_flavor(kcdata_descriptor_t data, uint32_t type,
                                   uint32_t size, uint64_t flags,
                                   mach_vm_address_t *user_addr);
static size_t kcdata_get_memory_size_for_data(uint32_t size);
static kern_return_t kcdata_compress_chunk_with_flags(kcdata_descriptor_t data,
                                                      uint32_t type,
                                                      const void *input_data,
                                                      uint32_t input_size,
                                                      uint64_t flags);
static kern_return_t kcdata_compress_chunk(kcdata_descriptor_t data,
                                           uint32_t type,
                                           const void *input_data,
                                           uint32_t input_size);
static kern_return_t kcdata_write_compression_stats(kcdata_descriptor_t data);
static kern_return_t kcdata_get_compression_stats(kcdata_descriptor_t data,
                                                  uint64_t *totalout,
                                                  uint64_t *totalin);
static void kcdata_object_no_senders(ipc_port_t port,
...
IPC_KOBJECT_DEFINE(IKOT_KCDATA, .iko_op_movable_send = true,
                   .iko_op_stable = true,
    ...
    os_atomic_rmw_loop(&lw_corpse_obj_cnt, oval, nval, relaxed, {
      ...
      if (__improbable(nval < 0)) {
        os_atomic_rmw_loop_give_up(
            panic("Lightweight corpse kcdata object over-released"));
      }
    });
  }
}
 ...
 */
kern_return_t kcdata_create_object(kcdata_descriptor_t data,
                                   kcdata_obj_flags_t flags, uint32_t size,
...
static void kcdata_object_destroy(kcdata_object_t obj) {
  void *begin_addr;
  ipc_port_t port;
  ...
  /* Release the port */
  if (IP_VALID(port)) {
    ipc_kobject_dealloc_port(port, IPC_KOBJECT_NO_MSCOUNT, IKOT_KCDATA);
  }
...
ipc_port_t convert_kcdata_object_to_port(kcdata_object_t obj) {
  ...
  if (!ipc_kobject_make_send_lazy_alloc_port(&obj->ko_port, obj, IKOT_KCDATA)) {
...
/*
 * Estimates how large of a buffer that should be allocated for a buffer that
 * will contain num_items items of known types with overall length payload_size.
 *
 ...
 */
uint32_t kcdata_estimate_required_buffer_size(uint32_t num_items,
  ...
  if (os_mul_overflow(num_items, KCDATA_ALIGNMENT_SIZE - 1,
                      &max_padding_bytes)) {
    panic("%s: Overflow in required buffer size estimate", __func__);
  }

  if (os_mul_and_add_overflow(num_items, sizeof(struct kcdata_item),
                              max_padding_bytes,
                              &max_padding_with_item_description_bytes)) {
    panic("%s: Overflow in required buffer size estimate", __func__);
  }

  if (os_add3_overflow(max_padding_with_item_description_bytes,
                       begin_and_end_marker_bytes, payload_size,
...
kcdata_descriptor_t kcdata_memory_alloc_init(mach_vm_address_t buffer_addr_p,
                                             unsigned data_type, unsigned size,
                                             unsigned flags) {
  ...
  data->kcd_flags = (clamped_flags & KCFLAG_USE_COPYOUT)
                        ? clamped_flags
                        : clamped_flags | KCFLAG_USE_MEMCOPY;
...
kern_return_t kcdata_memory_static_init(kcdata_descriptor_t data,
...
void *kcdata_endalloc(kcdata_descriptor_t data, size_t length) {
  ...
  size_t toalloc =
      ROUNDUP(length, sizeof(uint64_t)) + (curend % sizeof(uint64_t));
  /* an arbitrary limit: make sure we don't allocate more then 1/4th of the
   * remaining buffer. */
...
static void kcdata_release_endallocs(kcdata_descriptor_t data) {
  mach_vm_address_t curend = data->kcd_addr_begin + data->kcd_length;
...
uint64_t kcdata_memory_get_used_bytes(kcdata_descriptor_t kcd) {
  assert(kcd != NULL);
  return ((uint64_t)kcd->kcd_addr_end - (uint64_t)kcd->kcd_addr_begin) +
...
static void *kcdata_compress_zalloc(void *opaque, u_int items, u_int size) {
  void *result;
  ...
  if ((uintptr_t)result + alloc_size >
      (uintptr_t)cd->kcd_cd_base + cd->kcd_cd_maxoffset) {
    result = Z_NULL;
  } else {
...
static void kcdata_compress_zfree(void *opaque, void *ptr) {
  (void)opaque;
  (void)ptr;
...
/* Used to initialize the selected compression algorithm's internal state (if
 * any) */
static kern_return_t
kcdata_init_compress_state(kcdata_descriptor_t data,
                           void (*memcpy_f)(void *, const void *, size_t),
                           uint64_t type, mach_vm_address_t totalout_addr,
  ...
  case KCDCT_ZLIB:
    ...
    size = round_page(ZLIB_METADATA_SIZE +
                      zlib_deflate_memory_size(wbits, memlevel));
    kcdata_debug_printf("%s: size = %zu kcd_length: %d\n", __func__, size,
                        data->kcd_length);
    kcdata_debug_printf("%s: kcd buffer [%p - %p]\n", __func__,
                        (void *)data->kcd_addr_begin,
                        (void *)data->kcd_addr_begin + data->kcd_length);
    ...
    kcdata_debug_printf("%s: buffer [%p - %p]\n", __func__, cd->kcd_cd_base,
                        cd->kcd_cd_base + size);

    if (deflateInit2(&cd->kcd_cd_zs, Z_BEST_SPEED, Z_DEFLATED, wbits, memlevel,
    ...
    break;
  default:
    panic("kcdata_init_compress_state: invalid compression type: %d",
          (int)type);
  }
 ...
 */
kern_return_t kcdata_init_compress(kcdata_descriptor_t data, int hdr_tag,
  ...
  kr = kcdata_get_memory_addr(data, KCDATA_TYPE_UINT64_DESC, size_req,
                              &totalout_addr);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
  ...
  kr = kcdata_init_compress_state(
      data, memcpy_f, type,
      totalout_addr + offsetof(struct _uint64_with_description_data, data),
      totalin_addr + offsetof(struct _uint64_with_description_data, data));
  if (kr != KERN_SUCCESS) {
...
kcdata_zlib_translate_kcd_cf_flag(enum kcdata_compression_flush flush) {
  ...
  case KCDCF_NO_FLUSH:
    return Z_NO_FLUSH;
  case KCDCF_SYNC_FLUSH:
    return Z_SYNC_FLUSH;
  case KCDCF_FINISH:
    return Z_FINISH;
  default:
    panic("invalid kcdata_zlib_translate_kcd_cf_flag flag");
  }
}
...
kcdata_zlib_translate_kcd_cf_expected_ret(enum kcdata_compression_flush flush) {
  ...
  case KCDCF_FINISH:
    return Z_STREAM_END;
  default:
    panic("invalid kcdata_zlib_translate_kcd_cf_expected_ret flag");
  }
}

/* Called by kcdata_do_compress() when the configured compression algorithm is
 * zlib */
static kern_return_t
kcdata_do_compress_zlib(kcdata_descriptor_t data, void *inbuffer, size_t insize,
                        void *outbuffer, size_t outsize, size_t *wrote,
 ...
 * @outbuffer to @wrote.
 */
static kern_return_t kcdata_do_compress(kcdata_descriptor_t data,
  ...
  kcdata_debug_printf("%s: %p (%zu) <- %p (%zu); flush: %d\n", __func__,
                      outbuffer, outsize, inbuffer, insize, flush);

  /* don't compress if we are in a window */
  if (cd->kcd_cd_flags & KCD_CD_FLAG_IN_MARK ||
      data->kcd_comp_d.kcd_cd_compression_type == KCDCT_NONE) {
  ...
  case KCDCT_ZLIB:
    return kcdata_do_compress_zlib(data, inbuffer, insize, outbuffer, outsize,
                                   wrote, flush);
  default:
    panic("invalid compression type 0x%llx in kcdata_do_compress",
          data->kcd_comp_d.kcd_cd_compression_type);
  }
}

static size_t kcdata_compression_bound_zlib(kcdata_descriptor_t data,
                                            size_t size) {
  struct kcdata_compress_descriptor *cd = &data->kcd_comp_d;
  z_stream *zs = &cd->kcd_cd_zs;
 ...
 */
static size_t kcdata_compression_bound(kcdata_descriptor_t data, size_t size) {
  switch (data->kcd_comp_d.kcd_cd_compression_type) {
  case KCDCT_ZLIB:
 ...
 *		Compress buffer found at @input_data (length @input_size) to the
 * kcdata buffer described by @data. This method will construct the
 * kcdata_item_t required by parsers using the type information @type and flags
 * @flags.
 *
 *	Returns KERN_SUCCESS when successful. Currently, asserts on failure.
 */
kern_return_t kcdata_compress_chunk_with_flags(kcdata_descriptor_t data,
                                               uint32_t type,
                                               const void *input_data,
                                               uint32_t input_size,
   ...
   */
  size_t total_uncompressed_size = kcdata_compression_bound(
      data, (size_t)kcdata_get_memory_size_for_data(input_size));
  if (total_uncompressed_size > data->kcd_length ||
      data->kcd_length - total_uncompressed_size <
          data->kcd_addr_end - data->kcd_addr_begin) {
    kcdata_debug_printf("%s: insufficient buffer size: kcd_length => %d e-b=> "
                        "%lld our size: %zu\n",
                        __func__, data->kcd_length,
                        data->kcd_addr_end - data->kcd_addr_begin,
  ...
  enum kcdata_compression_flush flush =
      (padding || input_size)                   ? KCDCF_NO_FLUSH
      : cd->kcd_cd_flags & KCD_CD_FLAG_FINALIZE ? KCDCF_FINISH
                                                : KCDCF_SYNC_FLUSH;
  kr = kcdata_do_compress(data, &info, sizeof(info), space_ptr,
                          total_uncompressed_space_remaining, &wrote, flush);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
  ...
  if (input_size) {
    flush = padding                                   ? KCDCF_NO_FLUSH
            : cd->kcd_cd_flags & KCD_CD_FLAG_FINALIZE ? KCDCF_FINISH
                                                      : KCDCF_SYNC_FLUSH;
    kr = kcdata_do_compress(data, (void *)(uintptr_t)input_data, input_size,
                            space_ptr, total_uncompressed_space_remaining,
                            &wrote, flush);
    if (kr != KERN_SUCCESS) {
      return kr;
    }
  ...
  if (padding) {
    /* write the padding */
    kr = kcdata_do_compress(data, padding_data, padding, space_ptr,
                            total_uncompressed_space_remaining, &wrote,
                            cd->kcd_cd_flags & KCD_CD_FLAG_FINALIZE
                                ? KCDCF_FINISH
                                : KCDCF_SYNC_FLUSH);
    if (kr != KERN_SUCCESS) {
      return kr;
  ...
  assert((size_t)((uintptr_t)space_ptr - (uintptr_t)space_start) <=
         total_uncompressed_size);

  /* move the end marker forward */
  data->kcd_addr_end =
      (mach_vm_address_t)space_start +
      (total_uncompressed_size - total_uncompressed_space_remaining);
 ...
 *		Like kcdata_compress_chunk_with_flags(), but uses the default
 * set of kcdata flags, i.e. padding and also saves the amount of padding bytes.
 *
 * Returns are the same as in kcdata_compress_chunk_with_flags()
 */
kern_return_t kcdata_compress_chunk(kcdata_descriptor_t data, uint32_t type,
                                    const void *input_data,
                                    uint32_t input_size) {
  /* these flags are for kcdata - store that the struct is padded and store the
   * amount of padding bytes */
  uint64_t flags =
      (KCDATA_FLAGS_STRUCT_PADDING_MASK & kcdata_calc_padding(input_size)) |
      KCDATA_FLAGS_STRUCT_HAS_PADDING;
  return kcdata_compress_chunk_with_flags(data, type, input_data, input_size,
                                          flags);
}

kern_return_t kcdata_push_data(kcdata_descriptor_t data, uint32_t type,
                               uint32_t size, const void *input_data) {
...
kern_return_t kcdata_push_array(kcdata_descriptor_t data,
                                uint32_t type_of_element,
                                uint32_t size_of_element, uint32_t count,
                                const void *input_data) {
  ...
  if (data->kcd_flags & KCFLAG_USE_COMPRESSION) {
    return kcdata_compress_chunk_with_flags(data, KCDATA_TYPE_ARRAY_PAD0 | pad,
                                            input_data, total_size, flags);
  } else {
    kern_return_t ret;
    mach_vm_address_t uaddr = 0;
    ret = kcdata_get_memory_addr_with_flavor(data, KCDATA_TYPE_ARRAY_PAD0 | pad,
                                             total_size, flags, &uaddr);
    if (ret != KERN_SUCCESS) {
      return ret;
    }
 ...
 *      compressed one.
 *  (3) kcd_addr_end is then rewound to kcd_cd_mark_begin +
 * sizeof_compressed_data
 */
...
kern_return_t kcdata_compression_window_close(kcdata_descriptor_t data) {
  ...
  if (max_size > data->kcd_length ||
    ...
    kcdata_debug_printf("%s: insufficient buffer size: kcd_length => %d e-b=> "
                        "%lld our size: %lld\n",
                        __func__, data->kcd_length,
                        data->kcd_addr_end - data->kcd_addr_begin, max_size);
    return KERN_INSUFFICIENT_BUFFER_SIZE;
  }
  ...
  kr = kcdata_do_compress(data, (void *)cd->kcd_cd_mark_begin, total_size,
                          space_ptr, total_uncompressed_space_remaining, &wrote,
                          KCDCF_SYNC_FLUSH);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
  ...
  kcdata_memcpy(data, cd->kcd_cd_mark_begin, space_start,
                (uint32_t)(max_size - total_uncompressed_space_remaining));

  /* rewind the end marker */
  data->kcd_addr_end =
      cd->kcd_cd_mark_begin + (max_size - total_uncompressed_space_remaining);

  return KERN_SUCCESS;
}

static kern_return_t kcdata_get_compression_stats_zlib(kcdata_descriptor_t data,
                                                       uint64_t *totalout,
                                                       uint64_t *totalin) {
  struct kcdata_compress_descriptor *cd = &data->kcd_comp_d;
  z_stream *zs = &cd->kcd_cd_zs;
...
static kern_return_t kcdata_get_compression_stats(kcdata_descriptor_t data,
  ...
  case KCDCT_NONE:
    *totalout = *totalin = kcdata_memory_get_used_bytes(data);
    ...
    break;
  default:
    panic("invalid compression flag 0x%llx in kcdata_write_compression_stats",
          (data->kcd_comp_d.kcd_cd_compression_type));
  }
...
kern_return_t kcdata_write_compression_stats(kcdata_descriptor_t data) {
  kern_return_t kr;
  uint64_t totalout, totalin;
...
static kern_return_t kcdata_finish_compression_zlib(kcdata_descriptor_t data) {
  struct kcdata_compress_descriptor *cd = &data->kcd_comp_d;
  z_stream *zs = &cd->kcd_cd_zs;

  /*
   * macOS on x86 w/ coprocessor ver. 2 and later context: Stackshot compression
   * leaves artifacts in the panic buffer which interferes with CRC checks. The
   * CRC is calculated here over the full buffer but only the portion with valid
   * panic data is sent to iBoot via the SMC. When iBoot calculates the CRC to
   * compare with the value in the header it uses a zero-filled buffer. The
   * stackshot compression leaves non-zero bytes behind so those must be cleared
   * prior to the CRC calculation. This doesn't get the compression metadata;
   * that's zeroed by kcdata_release_endallocs().
   *
   * All other contexts: The stackshot compression artifacts are present in its
   * panic buffer but the CRC check is done on the same buffer for the before
   * and after calculation so there's nothing functionally broken. The same
   * buffer cleanup is done here for completeness' sake. From
   * rdar://problem/64381661
   */

  void *stackshot_end =
      (char *)data->kcd_addr_begin + kcdata_memory_get_used_bytes(data);
  uint32_t zero_fill_size =
      data->kcd_length - kcdata_memory_get_used_bytes(data);
...
static kern_return_t kcdata_finish_compression(kcdata_descriptor_t data) {
  ...
  case KCDCT_NONE:
    return KERN_SUCCESS;
  default:
    panic("invalid compression type 0x%llxin kcdata_finish_compression",
          data->kcd_comp_d.kcd_cd_compression_type);
  }
}
 ...
 */
kern_return_t kcdata_get_memory_addr(kcdata_descriptor_t data, uint32_t type,
                                     uint32_t size,
                                     mach_vm_address_t *user_addr) {
  /* record number of padding bytes as lower 4 bits of flags */
  uint64_t flags =
      (KCDATA_FLAGS_STRUCT_PADDING_MASK & kcdata_calc_padding(size)) |
      KCDATA_FLAGS_STRUCT_HAS_PADDING;
 ...
 * Desc: internal function with flags field. See documentation for
 * kcdata_get_memory_addr for details
 */
...
kcdata_get_memory_addr_with_flavor(kcdata_descriptor_t data, uint32_t type,
  ...
  if (user_addr == NULL || data == NULL ||
      total_size + sizeof(info) < orig_size) {
    return KERN_INVALID_ARGUMENT;
  }

  assert(((data->kcd_flags & KCFLAG_USE_COMPRESSION) &&
          (data->kcd_comp_d.kcd_cd_flags & KCD_CD_FLAG_IN_MARK)) ||
  ...
  if (total_size + sizeof(info) > data->kcd_length ||
      data->kcd_length - (total_size + sizeof(info)) <
          data->kcd_addr_end - data->kcd_addr_begin) {
    if (data->kcd_alloc_callback) {
      size_t const hdr_ftr_sz = 2 * sizeof(info);
      kcdata_descriptor_t new_data =
          data->kcd_alloc_callback(data, total_size + hdr_ftr_sz);
      if (new_data != NULL) {
        *data = *new_data;
        return kcdata_get_memory_addr_with_flavor(data, type, size, flags,
                                                  user_addr);
      }
    }
    return KERN_INSUFFICIENT_BUFFER_SIZE;
 ...
 *             data in middle of operation.
 * params:  data - pointer describing the crash info allocation
 *          type_of_element - type of data to be put. See kern_cdata.h for
 * defined types size_of_element - size of element. The header describes this
 * size count - num of elements in array. returns: mach_vm_address_t address in
 * user memory for copyout().
 */

kern_return_t kcdata_get_memory_addr_for_array(kcdata_descriptor_t data,
                                               uint32_t type_of_element,
  ...
  return kcdata_get_memory_addr_with_flavor(data, KCDATA_TYPE_ARRAY_PAD0 | pad,
                                            total_size, flags, user_addr);
}
 ...
 * params:  data - pointer describing the crash info allocation
 *          header_type - one of (KCDATA_TYPE_CONTAINER_BEGIN
 * ,KCDATA_TYPE_CONTAINER_END) container_type - type of data to be put. See
 * kern_cdata.h for defined types identifier - unique identifier. This is
 * required to match nested containers. returns: return value of
 * kcdata_get_memory_addr()
 */

kern_return_t kcdata_add_container_marker(kcdata_descriptor_t data,
  ...
  assert(header_type == KCDATA_TYPE_CONTAINER_END ||
         header_type == KCDATA_TYPE_CONTAINER_BEGIN);

  data_size =
      (header_type == KCDATA_TYPE_CONTAINER_BEGIN) ? sizeof(uint32_t) : 0;

  if (!(data->kcd_flags & KCFLAG_USE_COMPRESSION)) {
    kr = kcdata_get_memory_addr_with_flavor(data, header_type, data_size,
                                            identifier, &user_addr);
    if (kr != KERN_SUCCESS) {
      return kr;
    }
    ...
    kr = kcdata_compress_chunk_with_flags(data, header_type, &container_type,
 ...
 * Desc: call this after adding a container begin but before adding anything
 * else to revert.
 */
 ...
 * Desc: a common function to copy data out based on either copyout or memcopy
 * flags params:  data - pointer describing the kcdata buffer dst_addr -
 * destination address src_addr - source address size - size in bytes to copy.
 * returns: KERN_NO_ACCESS if copyout fails.
 */

kern_return_t kcdata_memcpy(kcdata_descriptor_t data,
                            mach_vm_address_t dst_addr, const void *src_addr,
                            uint32_t size) {
 ...
 */
kern_return_t kcdata_bzero(kcdata_descriptor_t data, mach_vm_address_t dst_addr,
                           uint32_t size) {
...
kern_return_t kcdata_add_type_definition(
    kcdata_descriptor_t data, uint32_t type_id, char *type_name,
  ...
  /* record number of padding bytes as lower 4 bits of flags */
  if (KERN_SUCCESS != (kr = kcdata_get_memory_addr_with_flavor(
                           data, KCDATA_TYPE_TYPEDEFINTION, total_size,
                           kcdata_calc_padding(total_size), &user_addr))) {
    return kr;
  }
  if (KERN_SUCCESS !=
      (kr = kcdata_memcpy(data, user_addr, (void *)&kc_type_definition,
                          sizeof(struct kcdata_type_definition)))) {
    return kr;
  }
  user_addr += sizeof(struct kcdata_type_definition);
  if (KERN_SUCCESS !=
      (kr = kcdata_memcpy(data, user_addr, (void *)elements_array_addr,
                          elements_count *
...
kern_return_t kcdata_add_uint64_with_description(kcdata_descriptor_t data_desc,
                                                 uint64_t data,
  ...
  if (data_desc->kcd_flags & KCFLAG_USE_COMPRESSION) {
    ...
    return kcdata_compress_chunk(data_desc, KCDATA_TYPE_UINT64_DESC, &save_data,
                                 size_req);
  }

  kr = kcdata_get_memory_addr(data_desc, KCDATA_TYPE_UINT64_DESC, size_req,
                              &user_addr);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
...
kern_return_t kcdata_add_uint32_with_description(kcdata_descriptor_t data_desc,
                                                 uint32_t data,
                                                 const char *description) {
  ...
  if (data_desc->kcd_flags & KCFLAG_USE_COMPRESSION) {
    /* allocate space for the output */
    return kcdata_compress_chunk(data_desc, KCDATA_TYPE_UINT32_DESC, &save_data,
                                 size_req);
  }

  kr = kcdata_get_memory_addr(data_desc, KCDATA_TYPE_UINT32_DESC, size_req,
                              &user_addr);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
