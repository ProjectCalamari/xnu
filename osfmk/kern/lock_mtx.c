...
#define LCK_EVENT_TO_MUTEX(e)                                                  \
  __container_of((uint32_t *)(e), lck_mtx_t, lck_mtx.data)
...
static inline void lck_mtx_mcs_clear(lck_mtx_mcs_t mcs) {
  *mcs = (struct lck_mtx_mcs){};
}

static inline lck_mcs_id_t lck_mtx_get_mcs_id(void) {
  return lck_mcs_id_current(LCK_MCS_SLOT_0);
}

__pure2 static inline lck_mtx_mcs_t lck_mtx_get_mcs(lck_mcs_id_t idx) {
  return &lck_mcs_get_other(idx)->mcs_mtx;
}

#pragma mark lck_mtx_t: validation

__abortlike static void __lck_mtx_invalid_panic(lck_mtx_t *lck) {
...
__abortlike static void __lck_mtx_not_owned_panic(lck_mtx_t *lock,
                                                  thread_t thread) {
  panic("Mutex %p is unexpectedly not owned by thread %p", lock, thread);
}

__abortlike static void __lck_mtx_owned_panic(lck_mtx_t *lock,
                                              thread_t thread) {
  panic("Mutex %p is unexpectedly owned by thread %p", lock, thread);
}

__abortlike static void __lck_mtx_lock_is_sleepable_panic(lck_mtx_t *lck) {
  // "Always" variants can never block. If the lock is held as a normal mutex
  // then someone is mixing always and non-always calls on the same lock, which
  // is forbidden.
  panic("Mutex %p is held as a full-mutex (spin-always lock attempted)", lck);
}

#if DEVELOPMENT || DEBUG
__abortlike static void __lck_mtx_preemption_disabled_panic(lck_mtx_t *lck,
                                                            int expected) {
  panic("Attempt to take mutex %p with preemption disabled (%d)", lck,
        get_preemption_level() - expected);
}

__abortlike static void __lck_mtx_at_irq_panic(lck_mtx_t *lck) {
 ...
 */
static inline void lck_mtx_check_preemption(lck_mtx_t *lock, thread_t thread,
                                            int expected) {
...
#if CONFIG_SPTM
  /*
   * If a panic has been initiated on SPTM devices, preemption was disabled by
   * sleh, but platform callbacks could be acquiring mutexes
   */
...
static inline void lck_mtx_check_irq(lck_mtx_t *lock) {
  if (ml_at_interrupt_context()) {
    __lck_mtx_at_irq_panic(lock);
  }
}

#define LCK_MTX_SNIFF_PREEMPTION(thread)                                       \
  lock_preemption_level_for_thread(thread)
...
__mockable void lck_mtx_init(lck_mtx_t *lck, lck_grp_t *grp, lck_attr_t *attr) {
...
__mockable void lck_mtx_destroy(lck_mtx_t *lck, lck_grp_t *grp) {
  ...
  if (lck->lck_mtx_type != LCK_TYPE_MUTEX ||
      (lck->lck_mtx.data & ~LCK_MTX_PROFILE) || lck->lck_mtx.as_tail ||
      lck->lck_mtx.ilk_tail) {
...
static hw_spin_timeout_status_t lck_mtx_ilk_timeout_panic(void *_lock,
  ...
  panic("Mutex interlock[%p] " HW_SPIN_TIMEOUT_FMT "; "
        "current owner: %p, "
        "<0x%06x 0x%02x 0x%08x 0x%08x 0x%04x "
        "0x%04x>, " HW_SPIN_TIMEOUT_DETAILS_FMT,
        lck, HW_SPIN_TIMEOUT_ARG(to, st),
        ctid_get_thread_unsafe(lck->lck_mtx.owner), lck->lck_mtx_tsid,
        lck->lck_mtx_type, lck->lck_mtx_grp, lck->lck_mtx.data,
        lck->lck_mtx.as_tail, lck->lck_mtx.ilk_tail,
...
static void lck_mtx_ilk_lock_cleanup_as_mcs(lck_mtx_t *lock, lck_mcs_id_t idx,
                                            lck_mtx_mcs_t mcs,
                                            hw_spin_timeout_t to,
                                            hw_spin_state_t *ss) {
  ...
  if (was_last) {
     ...
     */
    if (pidx) {
      os_atomic_cmpxchg(&lck_mtx_get_mcs(pidx)->lmm_as_next, mcs, nnode,
                        relaxed);
    }
  } else {
    /*
    ...
    if (pidx) {
      os_atomic_store(&lck_mtx_get_mcs(pidx)->lmm_as_next, nnode, relaxed);
    }
  }
...
static NOINLINE void lck_mtx_ilk_lock_contended(lck_mtx_t *lock,
   ...
   */
  do {
    while (!hw_spin_wait_until(&lock->lck_mtx.val, state.val,
                               state.ilocked == 0)) {
      hw_spin_should_keep_spinning(lock, pol, to, &ss);
    }
...
static void lck_mtx_ilk_lock_nopreempt(lck_mtx_t *lock, lck_ilk_mode_t mode) {
...
static void lck_mtx_ilk_unlock_v(lck_mtx_t *lock, uint32_t data) {
  os_atomic_store(&lock->lck_mtx.data, data, release);
  lock_enable_preemption();
}

static void lck_mtx_ilk_unlock(lck_mtx_t *lock) {
 ...
 *      <rdar://problem/30737670> ARM mutex contention logic could avoid taking
 * the thread lock
 */
static struct turnstile *lck_mtx_lock_wait(lck_mtx_t *lck, thread_t self,
                                           thread_t holder,
                                           struct turnstile *ts) {
  ...
  if (ts == TURNSTILE_NULL) {
    ts = turnstile_prepare_compact_id((uintptr_t)lck, lck->lck_mtx_tsid,
...
static void lck_mtx_lock_wait_done(lck_mtx_t *lck, struct turnstile *ts) {
 ...
 */
static void lck_mtx_unlock_wakeup(lck_mtx_t *lck,
                                  __kdebug_only thread_t thread) {
  struct turnstile *ts;
  kern_return_t did_wake;

  KERNEL_DEBUG(MACHDBG_CODE(DBG_MACH_LOCKS, LCK_MTX_UNLCK_WAKEUP_CODE) |
                   DBG_FUNC_START,
...
static inline bool lck_mtx_ctid_on_core(uint32_t ctid) {
...
static void lck_mtx_lock_adaptive_spin(lck_mtx_t *lock, lck_mtx_state_t state) {
  ...
  deadline = ml_get_timebase() +
             os_atomic_load(&MutexSpin, relaxed) * processor_avail_count;
  ...
  if (pidx) {
    ...
    while (!hw_spin_wait_until(
        &mcs->lmm_as_prev, prev,
        prev == 0 || (os_atomic_load(astp, relaxed) & AST_URGENT) ||
            (ml_get_timebase() > deadline))) {
      hw_spin_should_keep_spinning(lock, pol, to, &ss);
    }
  ...
  for (;;) {
    ...
    if (__probable(!state.ilocked && !state.ilk_tail && !state.owner)) {
      ...
      if (__probable(lock_store_exclusive(&lock->lck_mtx.val, state.val,
                                          nstate.val, acquire))) {
        break;
      }
    } else {
  ...
  KERNEL_DEBUG(
      ...
      trace_lck, VM_KERNEL_UNSLIDE_OR_PERM(thread), lock->lck_mtx_tsid, 0, 0);
  return;
...
static NOINLINE void lck_mtx_lock_contended(lck_mtx_t *lock, thread_t thread,
                                            lck_mtx_mode_t mode) {
  ...
  for (;;) {
    ...
    if ((state.val & ~(uint64_t)LCK_MTX_PROFILE) == 0) {
      if (!os_atomic_cmpxchgv(&lock->lck_mtx.val, state.val,
                              state.val | LCK_MTX_ILOCK, &state.val, acquire)) {
        continue;
      }
      break;
    }
...
#if CONFIG_DTRACE
    if (profile) {
      LCK_MTX_PROF_WAIT(lock, lock->lck_mtx_grp, direct_wait, &first_miss);
    }
#endif /* CONFIG_DTRACE */
    os_atomic_store(&lock->lck_mtx.data,
                    state.data | LCK_MTX_ILOCK | LCK_MTX_NEEDS_WAKEUP,
                    compiler_acq_rel);
    ts = lck_mtx_lock_wait(lock, thread, ctid_get_thread(state.owner), ts);
  ...
  LCK_MTX_ACQUIRED(lock, lock->lck_mtx_grp, mode != LCK_MTX_MODE_SLEEPABLE,
                   profile);
}
...
#endif
static void lck_mtx_lock_slow(lck_mtx_t *lock, thread_t thread,
                              lck_mtx_state_t state, lck_mtx_mode_t mode) {
#pragma unused(state)
#if CONFIG_DTRACE
  ...
  if (mode != LCK_MTX_MODE_SPIN_ALWAYS) {
    lck_mtx_check_preemption(lock, thread, (mode == LCK_MTX_MODE_SPIN));
  }
  ...
  if (state.val == ostate.val) {
    ...
    os_atomic_cmpxchgv(&lock->lck_mtx.val, ostate.val, state.val, &state.val,
                       acquire);
  }
...
lck_mtx_lock_fastpath(lck_mtx_t *lock, lck_mtx_mode_t mode) {
   ...
   */
  lock_cmpxchgv(&lock->lck_mtx.val, 0, state.val, &state.val, acquire);
...
__mockable void lck_mtx_lock(lck_mtx_t *lock) {
...
lck_mtx_try_lock_slow_inline(lck_mtx_t *lock, thread_t thread, uint32_t odata,
                             uint32_t ndata, bool spin) {
#pragma unused(lock, thread, odata, ndata)
#if CONFIG_DTRACE
  if (odata == LCK_MTX_PROFILE) {
    os_atomic_cmpxchgv(&lock->lck_mtx.data, odata, ndata | LCK_MTX_PROFILE,
                       &odata, acquire);
  }
  if ((odata & ~LCK_MTX_PROFILE) == 0) {
    LCK_MTX_TRY_ACQUIRED(lock, lock->lck_mtx_grp, spin,
                         odata & LCK_MTX_PROFILE);
    return true;
  }
  if (odata & LCK_MTX_PROFILE) {
...
__attribute__((always_inline))
#endif
static bool lck_mtx_try_lock_slow(lck_mtx_t *lock, thread_t thread,
                                  uint32_t odata, uint32_t ndata) {
...
__attribute__((always_inline))
#endif
static bool lck_mtx_try_lock_slow_spin(lck_mtx_t *lock, thread_t thread,
                                       uint32_t odata, uint32_t ndata) {
...
lck_mtx_try_lock_fastpath(lck_mtx_t *lock, lck_mtx_mode_t mode) {
  ...
  if (mode == LCK_MTX_MODE_SPIN_ALWAYS && (odata & LCK_MTX_CTID_MASK) &&
...
boolean_t lck_mtx_try_lock_spin_always(lck_mtx_t *lock) {
  return lck_mtx_try_lock_fastpath(lock, LCK_MTX_MODE_SPIN_ALWAYS);
}

#pragma mark lck_mtx_t: lck_mtx_unlock

static NOINLINE void lck_mtx_unlock_contended(lck_mtx_t *lock, thread_t thread,
                                              uint32_t data) {
  bool cleanup = false;
...
__attribute__((always_inline))
#endif
static void lck_mtx_unlock_slow(lck_mtx_t *lock, thread_t thread,
                                uint32_t data) {
#if CONFIG_DTRACE
  /*
  ...
  if ((data & LCK_MTX_BITS_MASK) == LCK_MTX_PROFILE) {
    os_atomic_cmpxchgv(&lock->lck_mtx.data, data, LCK_MTX_PROFILE, &data,
                       release);
  }
  if (thread->ctid != (data & LCK_MTX_CTID_MASK)) {
    __lck_mtx_not_owned_panic(lock, thread);
...
__mockable void lck_mtx_unlock(lck_mtx_t *lock) {
  thread_t thread = current_thread();
  uint32_t take_slowpath = 0;
  uint32_t data;
   ...
   */
  if (__probable(lock_cmpxchgv(&lock->lck_mtx.data, thread->ctid, 0, &data,
                               release))) {
    if (__probable(!take_slowpath)) {
      return;
    }
 ...
 */
void lck_mtx_convert_spin(lck_mtx_t *lock) {
  lck_mtx_state_t state = os_atomic_load(&lock->lck_mtx, relaxed);
  thread_t thread = current_thread();
...
void kdp_lck_mtx_find_owner(struct waitq *waitq __unused, event64_t event,
...
uint32_t collision_backoffs[MAX_COLLISION] = {10,  50,  100, 200,
                                              400, 600, 800, 1000};
...
void mutex_pause(uint32_t collisions) {
  ...
  wait_result = assert_wait_timeout((event_t)mutex_pause, THREAD_UNINT,
                                    back_off, NSEC_PER_USEC);
...
boolean_t lck_mtx_yield(lck_mtx_t *lck) {
