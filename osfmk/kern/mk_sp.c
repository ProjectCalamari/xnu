 ...
 */
kern_return_t thread_set_policy(thread_t thread, processor_set_t pset,
                                policy_t policy, policy_base_t base,
  ...
  if (thread == THREAD_NULL || pset == PROCESSOR_SET_NULL || pset != &pset0) {
  ...
  case POLICY_TIMESHARE: {
    ...
    policy_timeshare_limit_t ts_limit = (policy_timeshare_limit_t)limit;
 ...
 */
kern_return_t thread_policy(thread_t thread, policy_t policy,
                            policy_base_t base, mach_msg_type_number_t count,
                            boolean_t set_limit) {
  kern_return_t result = KERN_SUCCESS;
  ...
  if (result == KERN_SUCCESS) {
    result =
        thread_set_policy(thread, pset, policy, base, count, limit, limcount);
  }
