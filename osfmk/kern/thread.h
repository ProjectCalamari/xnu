...
__options_decl(thread_work_interval_flags_t, uint32_t,
               {
                   ...
                   /* Flags to indicate status about work interval thread is
                      currently part of */
...
__options_decl(thread_exclaves_intstate_flags_t, uint32_t,
               {
                   /* Thread is currently executing in secure kernel or exclaves
                    * userspace or was interrupted/preempted while doing so. */
                   TH_EXCLAVES_EXECUTION = 0x1,
               });

__options_decl(
    thread_exclaves_state_flags_t, uint16_t,
    {
        /* Thread exclaves state bits (ORd) */
        /* Thread is handling RPC from a client in xnu or Darwin userspace (but
         * may have returned to xnu due to an exclaves scheduler request or
         * having upcalled). Must not re-enter exclaves via RPC or return to
         * Darwin userspace. */
        TH_EXCLAVES_RPC = 0x1,
        /* Thread has made an upcall RPC request back into xnu while handling
         * RPC into exclaves from a client in xnu or Darwin userspace. Must not
         * re-enter exclaves via RPC or return to Darwin userspace. */
        TH_EXCLAVES_UPCALL = 0x2,
        /* Thread has made an exclaves scheduler request (such as a wait or
         * wake) from the xnu scheduler while handling RPC into exclaves from a
         * client in xnu or Darwin userspace. Must not re-enter exclaves via RPC
...
#define TH_EXCLAVES_STATE_ANY                                                  \
  (TH_EXCLAVES_RPC | TH_EXCLAVES_UPCALL | TH_EXCLAVES_SCHEDULER_REQUEST |      \
   TH_EXCLAVES_XNUPROXY | TH_EXCLAVES_SCHEDULER_CALL |                         \
...
__options_decl(thread_exclaves_inspection_flags_t, uint16_t,
               {
                   ...
                   /* Thread must not be inspected (may deadlock, etc.) - set by
                      collector thread*/
...
#define THREAD_MAGIC 0x1234ABCDDCBA4321ULL
  ...
  union {
    ...
    struct priority_queue_entry_sched
        wait_prioq_links; /* priority ordered waitq links */
  };

  event64_t wait_event; /* wait queue event */
  struct {
    processor_t runq;
  } __runq; /* internally managed run queue assignment, see above comment */
  waitq_t waitq; /* wait queue this thread is enqueued on */
  struct turnstile
      *turnstile;  /* thread's turnstile, protected by primitives interlock */
  void *inheritor; /* inheritor of the primitive the thread will block on */
  struct priority_queue_sched_max
      sched_inheritor_queue; /* Inheritor queue for kernel promotion */
  struct priority_queue_sched_max
      base_inheritor_queue; /* Inheritor queue for user promotion */
  ...
  uint8_t th_expired_quantum_on_lower_core : 1,
      th_expired_quantum_on_higher_core : 1, : 0;
#endif /* CONFIG_SCHED_EDGE */

#if CONFIG_SCHED_CLUTCH
  /*
   * In the clutch scheduler, the threads are maintained in runqs at the
   * clutch_bucket level (clutch_bucket defines a unique thread group and
   * scheduling bucket pair). The thread is linked via a couple of linkages in
   * the clutch bucket:
   *
...
#define TH_OPT_SYSTEM_CRITICAL                                                 \
  0x0010 /* Thread must always be allowed to run - even under heavy load */
#define TH_OPT_PROC_CPULIMIT                                                   \
  0x0020 /* Thread has a task-wide CPU limit applied to it */
#define TH_OPT_PRVT_CPULIMIT                                                   \
  0x0040 /* Thread has a thread-private CPU limit applied to it */
#define TH_OPT_IDLE_THREAD 0x0080 /* Thread is a per-processor idle thread */
#define TH_OPT_GLOBAL_FORCED_IDLE                                              \
  0x0100 /* Thread performs forced idle for thermal control */
#define TH_OPT_SCHED_VM_GROUP                                                  \
  0x0200 /* Thread belongs to special scheduler VM group */
#define TH_OPT_HONOR_QLIMIT                                                    \
  0x0400 /* Thread will honor qlimit while sending mach_msg, regardless of     \
            MACH_SEND_ALWAYS */
#define TH_OPT_SEND_IMPORTANCE                                                 \
  0x0800 /* Thread will allow importance donation from kernel rpc */
#define TH_OPT_ZONE_PRIV 0x1000 /* Thread may use the zone replenish reserve   \
                                 */
#define TH_OPT_IPC_TG_BLOCKED                                                  \
  0x2000 /* Thread blocked in sync IPC and has made the thread group blocked   \
            callout */
#define TH_OPT_FORCED_LEDGER 0x4000 /* Thread has a forced CPU limit  */
#define TH_IN_MACH_EXCEPTION                                                   \
  0x8000 /* Thread is currently handling a mach exception */
#if CONFIG_EXCLAVES
...
#define TH_WAIT_REPORT                                                         \
  0x40                  /* the wait is using the sched_call,                   \
...
#define TH_WAKING 0x100 /* between waitq remove and thread_go */
  ...
  sfi_class_id_t sfi_wait_class; /* Currently in SFI wait for this class,
                                    protected by sfi_lock */

  uint32_t sched_flags; /* current flag bits */
#if CONFIG_SCHED_SMT
#define TH_SFLAG_NO_SMT                                                        \
  0x0001 /* On an SMT CPU, this thread must be scheduled alone */
#endif   /* CONFIG_SCHED_SMT */

#define TH_SFLAG_FAILSAFE 0x0002 /* fail-safe has tripped */
#define TH_SFLAG_THROTTLED                                                     \
  0x0004 /* throttled thread forced to timeshare mode (may be applied in       \
            addition to failsafe) */
...
#define TH_SFLAG_EAGERPREEMPT                                                  \
  0x0200 /* Any preemption of this thread should be treated as if AST_URGENT   \
            applied */
#define TH_SFLAG_RW_PROMOTED                                                   \
  0x0400 /* promote reason: blocking with RW lock held */
#define TH_SFLAG_BASE_PRI_FROZEN 0x0800 /* (effective) base_pri is frozen */
#define TH_SFLAG_WAITQ_PROMOTED                                                \
  0x1000 /* promote reason: waitq wakeup (generally for IPC receive) */
...
#define TH_SFLAG_THREAD_GROUP_AUTO_JOIN                                        \
  0x10000 /* thread has been auto-joined to thread group */
#if __AMP__
/* unused TH_SFLAG_BOUND_SOFT           0x20000 */
#endif /* __AMP__ */

#if CONFIG_PREADOPT_TG
#define TH_SFLAG_REEVALUTE_TG_HIERARCHY_LATER                                  \
  0x40000 /* thread needs to reevaluate its TG hierarchy */
#endif
 ...
 */
#define TH_SFLAG_PROMOTE_REASON_MASK                                           \
  (TH_SFLAG_RW_PROMOTED | TH_SFLAG_WAITQ_PROMOTED | TH_SFLAG_EXEC_PROMOTED |   \
   TH_SFLAG_FLOOR_PROMOTED)

#define TH_SFLAG_RT_DISALLOWED                                                 \
  0x100000 /* thread wants RT but may not have joined a work interval that     \
              allows it */
#define TH_SFLAG_DEMOTED_MASK                                                  \
  (TH_SFLAG_THROTTLED | TH_SFLAG_FAILSAFE |                                    \
   TH_SFLAG_RT_DISALLOWED) /* saved_mode contains previous sched_mode */
#define TH_SFLAG_RT_CPULIMIT                                                   \
  0x200000 /* thread should have a CPU limit applied. */

#define TH_SFLAG_FAILSAFE_REPORTED                                             \
  0x400000 /* whether the kernel has already logged that thread triggered the  \
              failsafe (to prevent log spam) */

  int16_t sched_pri;    /* scheduled (current) priority */
  int16_t base_pri;     /* effective base priority (equal to req_base_pri unless
  ...
  uint16_t
      priority_floor_count; /* number of push to boost the floor priority */
  ...
  uint64_t
      last_made_runnable_time; /* time when thread was unblocked or preempted */
  uint64_t last_basepri_change_time; /* time when thread was last changed in
                                        basepri while runnable */
...
#define WORKQ_QUANTUM_HISTORY_WRITE_ENTRY(thread, ...)                         \
  ({                                                                           \
    ...
    uint64_t __index =                                                         \
        os_atomic_inc_orig(&thread->workq_quantum_history_index, relaxed);     \
    struct workq_quantum_history _wq_quantum_history = {                       \
        mach_approximate_time(), __VA_ARGS__};                                 \
...
#define THREAD_NOT_RUNNABLE (~0ULL)
  ...
  timer_data_t
      runnable_timer; /* time the thread is runnable (including running) */
  ...
  /* Various bits of state to stash across a continuation, exclusive to the
   * current thread block point */
  union {
    struct {
      ...
      struct ipc_kmsg *
          XNU_PTRAUTH_SIGNED_PTR("thread.ith_kmsg") kmsg; /* received message */
    } receive;
    struct {
...
#define THREAD_SAVE_IOKIT_TLS_COUNT 8
  ...
  /* Only user threads can cause Mach exceptions, only kernel threads can be
   * thread call threads */
  union {
  ...
  struct thread_test_context
      *th_test_ctx; /* thread-specific data for kernel tests */
#endif
...
#define T_KPERF_SET_CALLSTACK_DEPTH(DEPTH)                                     \
  (((uint32_t)(DEPTH)) << T_KPERF_CALLSTACK_DEPTH_OFFSET)
#define T_KPERF_GET_CALLSTACK_DEPTH(FLAGS)                                     \
  ((FLAGS) >> T_KPERF_CALLSTACK_DEPTH_OFFSET)
...
#define T_KPERF_AST_ALL                                                        \
  ...
  uint64_t
      t_deduct_bank_ledger_time; /* cpu time to be deducted from bank ledger */
  uint64_t
      t_deduct_bank_ledger_energy; /* energy to be deducted from bank ledger */
  ...
  /* usynch override is protected by the task lock, eventually will be thread
   * mutex */
  ...
  /*
   * callout_* fields are only set for thread call threads whereas mach_exc_*
   * are set by user threads on themselves while taking a Mach exception. So
   * it's okay for them to share this bitfield.
   */
  uint16_t callout_woken_from_icontext : 1,
      callout_woken_from_platform_idle : 1, callout_woke_thread : 1,
      mach_exc_fatal : 1, mach_exc_ktriage : 1, thread_bitfield_unused : 11;

#define THREAD_BOUND_CLUSTER_NONE (UINT32_MAX)
  /*
   * Cluster to which the thread is soft-bound for scheduling. The thread will
   * always run on its bound cluster unless that cluster has been derecommended
   * for scheduling.
   */
   ...
   *	hierarchy resolution is done, we know that t->thread_group will not
   * point to this field anymore so we can clear it.
   *
  ...
  turnstile_update_flags_t
      inheritor_flags; /* inheritor flags for inheritor field */
  block_hint_t pending_block_hint;
  block_hint_t block_hint; /* What type of primitive last caused us to block. */
  uint32_t decompressions; /* Per-thread decompressions counter to be added to
                              per-task decompressions counter */
  int thread_region_page_shift; /* Page shift that this thread would like to use
                                   when */
  ...
  uint32_t t_ledger_req_action : 2, t_ledger_req_percentage : 7,
      t_ledger_req_interval_ms : 16, : 0;
...
#if MACH_ASSERT
#define assert_thread_magic(thread)                                            \
  assertf((thread)->thread_magic == THREAD_MAGIC,                              \
...
extern void thread_inspect_deallocate(thread_inspect_t thread);

extern void thread_read_deallocate(thread_read_t thread);
...
extern void thread_start_in_assert_wait(thread_t thread, struct waitq *waitq,
                                        event64_t event,
                                        wait_interrupt_t interruptible)
    __attribute__((noinline));

extern void thread_terminate_enqueue(thread_t thread);

extern void thread_exception_enqueue(task_t task, thread_t thread,
                                     exception_type_t etype);

extern void
thread_backtrace_enqueue(kcdata_object_t obj,
...
extern void thread_stack_enqueue(thread_t thread);
...
/* Locking for scheduler state, always acquired with interrupts disabled
 * (splsched()) */
...
extern void stack_handoff(thread_t from, thread_t to);

extern void stack_free(thread_t thread);

extern void stack_free_reserved(thread_t thread);

extern boolean_t stack_alloc_try(thread_t thread);
...
extern kern_return_t kernel_thread_create(thread_continue_t continuation,
...
extern void machine_stack_attach(thread_t thread, vm_offset_t stack);

extern vm_offset_t machine_stack_detach(thread_t thread);

extern void machine_stack_handoff(thread_t old, thread_t new);

extern thread_t machine_switch_context(thread_t old_thread,
...
extern kern_return_t machine_thread_set_state(thread_t thread,
                                              thread_flavor_t flavor,
                                              thread_state_t state,
                                              mach_msg_type_number_t count);

extern mach_vm_address_t machine_thread_pc(thread_t thread);
...
extern kern_return_t machine_thread_get_state(thread_t thread,
                                              thread_flavor_t flavor,
                                              thread_state_t state,
                                              mach_msg_type_number_t *count);
...
extern kern_return_t machine_thread_dup(thread_t self, thread_t target,
                                        boolean_t is_corpse);
...
extern void machine_thread_create(thread_t thread, task_t task,
                                  bool first_thread);

extern kern_return_t machine_thread_process_signature(thread_t thread,
                                                      task_t task);

extern void machine_thread_switch_addrmode(thread_t thread);

extern void machine_thread_destroy(thread_t thread);

extern void machine_set_current_thread(thread_t thread);

extern kern_return_t
machine_thread_get_kern_state(thread_t thread, thread_flavor_t flavor,
                              thread_state_t tstate,
                              mach_msg_type_number_t *count);

extern kern_return_t machine_thread_inherit_taskwide(thread_t thread,
                                                     task_t parent_task);

extern kern_return_t machine_thread_set_tsd_base(thread_t thread,
                                                 mach_vm_offset_t tsd_base);

#define thread_mtx_try(thread) lck_mtx_try_lock(&(thread)->mutex)
#define thread_mtx_held(thread)                                                \
  lck_mtx_assert(&(thread)->mutex, LCK_MTX_ASSERT_OWNED)
...
extern uint64_t thread_rettokern_addr(thread_t thread);

extern uint64_t thread_wqquantum_addr(thread_t thread);
...
kern_return_t thread_shared_rsrc_policy_set(thread_t thread, uint32_t index,
                                            cluster_shared_rsrc_type_t type,
                                            shared_rsrc_policy_agent_t agent);
kern_return_t thread_shared_rsrc_policy_clear(thread_t thread,
                                              cluster_shared_rsrc_type_t type,
                                              shared_rsrc_policy_agent_t agent);

#ifdef MACH_KERNEL_PRIVATE
static inline thread_tag_t thread_set_tag_internal(thread_t thread,
                                                   thread_tag_t tag) {
  return os_atomic_or_orig(&thread->thread_tag, tag, relaxed);
}

static inline thread_tag_t thread_get_tag_internal(thread_t thread) {
...
extern kern_return_t thread_setstatus_from_user(
    thread_t thread, int flavor, thread_state_t tstate,
    mach_msg_type_number_t count, thread_state_t old_tstate,
    mach_msg_type_number_t old_count, thread_set_status_flags_t flags);

extern kern_return_t thread_getstatus(thread_t thread, int flavor,
                                      thread_state_t tstate,
                                      mach_msg_type_number_t *count);

extern kern_return_t thread_getstatus_to_user(thread_t thread, int flavor,
                                              thread_state_t tstate,
                                              mach_msg_type_number_t *count,
                                              thread_set_status_flags_t flags);

extern kern_return_t
thread_create_with_continuation(task_t task, thread_t *new_thread,
                                thread_continue_t continuation);
...
extern kern_return_t
thread_create_workq_waiting(task_t task, thread_continue_t thread_return,
                            thread_t *new_thread, bool is_permanently_bound);

extern kern_return_t
thread_create_aio_workq_waiting(task_t task, thread_continue_t thread_return,
                                thread_t *new_thread);

extern void thread_yield_internal(mach_msg_timeout_t interval);
...
/*
 * Thread-private CPU limits: apply a private CPU limit to this thread only.
 * Available actions are:
 *
 * 1) Block. Prevent CPU consumption of the thread from exceeding the limit.
 * 2) Exception. Generate a resource consumption exception when the limit is
 * exceeded. 3) Disable. Remove any existing CPU limit.
 */
...
extern void thread_read_times(thread_t thread, time_value_t *user_time,
                              time_value_t *system_time,
                              time_value_t *runnable_time);

extern void thread_read_times_unsafe(thread_t thread, time_value_t *user_time,
                                     time_value_t *system_time,
                                     time_value_t *runnable_time);
...
extern user_addr_t thread_adjuserstack(thread_t thread, int adjust);

extern void thread_setentrypoint(thread_t thread, mach_vm_offset_t entry);

extern kern_return_t thread_set_tsd_base(thread_t thread,
                                         mach_vm_offset_t tsd_base);

extern kern_return_t thread_setsinglestep(thread_t thread, int on);
...
extern kern_return_t thread_wire_internal(host_priv_t host_priv,
...
extern void thread_sched_call(thread_t thread, sched_call_t call);

extern boolean_t thread_is_static_param(thread_t thread);
 ...
 */
#define thread_is_64bit_addr(thd) task_has_64Bit_addr(get_threadtask(thd))

/*
 * Thread is using 64-bit machine state.
 */
#define thread_is_64bit_data(thd) task_has_64Bit_data(get_threadtask(thd))
...
extern void uthread_set_exec_data(struct uthread *uth,
...
extern boolean_t thread_should_halt(thread_t thread);
...
extern kern_return_t thread_set_wq_state32(thread_t thread,
                                           thread_state_t tstate);

extern kern_return_t thread_set_wq_state64(thread_t thread,
                                           thread_state_t tstate);
...
extern void fd_guard_ast(thread_t, mach_exception_code_t,
                         mach_exception_subcode_t);
extern void vn_guard_ast(thread_t, mach_exception_code_t,
                         mach_exception_subcode_t);
extern void mach_port_guard_ast(thread_t, mach_exception_code_t,
                                mach_exception_subcode_t);
extern void virt_memory_guard_ast(thread_t, mach_exception_code_t,
                                  mach_exception_subcode_t);
extern void thread_ast_mach_exception(thread_t, int, exception_type_t,
                                      mach_exception_code_t,
                                      mach_exception_subcode_t, bool, bool);
extern void thread_guard_violation(thread_t, mach_exception_code_t,
...
extern kern_return_t thread_get_voucher_origin_pid(thread_t thread,
                                                   int32_t *pid);
extern kern_return_t
thread_get_voucher_origin_proximate_pid(thread_t thread, int32_t *origin_pid,
...
extern kern_return_t
machine_thread_siguctx_pointer_convert_to_user(thread_t thread,
                                               user_addr_t *uctxp);
...
/*
 * Translate array of function pointer syscall arguments from userspace
 * representation
 */
...
extern uint64_t thread_dispatchqaddr(thread_t thread);
...
extern void convert_thread_array_to_ports(thread_act_array_t, size_t,
...
extern bool thread_has_expired_workqueue_quantum(thread_t thread,
                                                 bool should_trace);
...
extern uintptr_t thread_get_txm_thread_stack(void);
...
extern kern_return_t thread_get_special_port(thread_inspect_t thread, int which,
                                             ipc_port_t *portp);
 ...
 *   @param continuation A C-function pointer where the thread will begin
 * execution.
 *   @param parameter Caller specified data to be passed to the new thread.
 *   @param new_thread Reference to the new thread is returned in this
 * parameter.
