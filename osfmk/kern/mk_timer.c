...
static ZONE_DEFINE_TYPE(mk_timer_zone, "mk_timer", struct mk_timer,
                        ZC_ZFREE_CLEARMEM);

static void mk_timer_expire(void *p0, void *p1);

mach_port_name_t
mk_timer_create_trap(__unused struct mk_timer_create_trap_args *args) {
...
static void mk_timer_unlock_and_destroy(struct mk_timer *timer,
                                        ipc_port_t port) {
...
void mk_timer_port_label_dealloc(ipc_object_label_t label) {
...
static void mk_timer_expire(void *p0, __unused void *p1) {
  ...
  while (timer->is_armed && timer->active == 1) {
    ...
    if (ipc_kmsg_keep_alive_try_reusing(kmsg)) {
      ...
      msg = __container_of(ikm_header(kmsg), mk_timer_expire_msg_t, header);
 ...
 *
 */
kern_return_t mk_timer_destroy_trap(struct mk_timer_destroy_trap_args *args) {
...
static kern_return_t mk_timer_arm_trap_internal(mach_port_name_t name,
  ...
  if (timer) {
    ...
    if (!timer->is_dead) {
      ...
      if (expire_time > mach_absolute_time()) {
        ...
        if (!thread_call_enter_delayed_with_leeway(&timer->mkt_thread_call,
                                                   NULL, expire_time, mk_leeway,
                                                   tcflags)) {
          timer->active++;
        }
      } else {
...
kern_return_t mk_timer_arm_trap(struct mk_timer_arm_trap_args *args) {
  return mk_timer_arm_trap_internal(args->name, args->expire_time, 0,
                                    MK_TIMER_NORMAL);
}

kern_return_t
mk_timer_arm_leeway_trap(struct mk_timer_arm_leeway_trap_args *args) {
  return mk_timer_arm_trap_internal(args->name, args->expire_time,
                                    args->mk_leeway, args->mk_timer_flags);
}
 ...
 *
 * Indirect:     args->name               Mach port name
 *               args->result_time        The armed time of the cancelled timer
 * (return value)
 *
 *
 * Returns:        0                      Success
 ...
 *
 */
kern_return_t mk_timer_cancel_trap(struct mk_timer_cancel_trap_args *args) {
  mach_port_name_t name = args->name;
  ...
  if (result == KERN_SUCCESS && result_time_addr != 0) {
    if (copyout((void *)&armed_time, result_time_addr, sizeof(armed_time)) !=
        0) {
      result = KERN_FAILURE;
    }
  }
