 ...
 */
size_t coalitions_get_list(int type, struct procinfo_coalinfo *coal_list,
                           size_t list_sz);
...
TUNABLE_WRITEABLE(int, unrestrict_coalition_syscalls,
...
static kern_return_t i_coal_resource_init(coalition_t coal,
...
static kern_return_t i_coal_resource_set_taskrole(coalition_t coal, task_t task,
                                                  int role);
static int i_coal_resource_get_taskrole(coalition_t coal, task_t task);
static task_t
i_coal_resource_iterate_tasks(coalition_t coal,
...
struct i_resource_coalition {
  ...
  uint64_t
      cpu_time_eqos[COALITION_NUM_THREAD_QOS_TYPES]; /* cpu time per effective
                                                        QoS class */
  uint64_t
      cpu_time_rqos[COALITION_NUM_THREAD_QOS_TYPES]; /* cpu time per requested
                                                        QoS class */
  ...
  /*
   * This ledger is used for triggering resource exception. For the tracked
   * resources, this is updated when the member tasks' resource usage changes.
   */
...
static kern_return_t i_coal_jetsam_set_taskrole(coalition_t coal, task_t task,
                                                int role);
int i_coal_jetsam_get_taskrole(coalition_t coal, task_t task);
static task_t
i_coal_jetsam_iterate_tasks(coalition_t coal,
                            coalition_for_each_task_block_t block);
...
struct coalition {
  ...
  uint32_t type;
  uint32_t
      role; /* default task role (background, adaptive, interactive, etc) */
  os_ref_atomic_t
      ref_count; /* Number of references to the memory containing this struct */
  uint32_t active_count;     /* Number of members of (tasks in) the
                              *  coalition, plus vouchers referring
                              *  to the coalition */
  uint32_t focal_task_count; /* Number of TASK_FOREGROUND_APPLICATION tasks in
                                the coalition */
  uint32_t nonfocal_task_count; /* Number of TASK_BACKGROUND_APPLICATION tasks
                                   in the coalition */
  uint32_t game_task_count;     /* Number of GAME_MODE tasks in the coalition */
  uint32_t
      carplay_task_count; /* Number of CARPLAY_MODE tasks in the coalition */
  ...
  uint32_t reaped : 1; /* reaped, invisible to userspace, but waiting for
                          ref_count to go to zero */
  uint32_t notified
...
#define coal_ref_init(coal, c)                                                 \
  os_ref_init_count_raw(&(coal)->ref_count, &coal_ref_grp, c)
#define coal_ref_count(coal) os_ref_get_count_raw(&(coal)->ref_count)
#define coal_ref_try_retain(coal)                                              \
  os_ref_retain_try_raw(&(coal)->ref_count, &coal_ref_grp)
#define coal_ref_retain(coal)                                                  \
  os_ref_retain_raw(&(coal)->ref_count, &coal_ref_grp)
#define coal_ref_release(coal)                                                 \
  os_ref_release_raw(&(coal)->ref_count, &coal_ref_grp)
#define coal_ref_release_live(coal)                                            \
  os_ref_release_live_raw(&(coal)->ref_count, &coal_ref_grp)

#define COALITION_HASH_SIZE_MIN 16

static bool coal_hash_obj_try_get(void *coal) {
 ...
 * On embedded, track them using jetsam coalitions since they have associated
 * thread groups which reflect this property as a flag (and pass it down to
 * CLPC). On non-embedded platforms, since not all coalitions have jetsam
 * coalitions track focal counts on the resource coalition.
 */
...
ledger_t coalition_ledger_get_from_task(task_t task) {
  ...
  if (coal != NULL &&
      (!queue_empty(&task->task_coalition[COALITION_TYPE_RESOURCE]))) {
...
enum { COALITION_IO_LEDGER_ENABLE, COALITION_IO_LEDGER_DISABLE };

void coalition_io_monitor_ctl(struct coalition *coalition, uint32_t flags,
                              int64_t limit) {
  ledger_t ledger = coalition->r.resource_monitor_ledger;

  if (flags == COALITION_IO_LEDGER_ENABLE) {
    /* Configure the logical I/O ledger */
    ledger_set_limit(ledger, coalition_ledgers.logical_writes,
                     (limit * 1024 * 1024), 0);
    ledger_set_period(ledger, coalition_ledgers.logical_writes,
                      (COALITION_LEDGER_MONITOR_INTERVAL_SECS * NSEC_PER_SEC));
...
int coalition_ledger_set_logical_writes_limit(struct coalition *coalition,
                                              int64_t limit) {
  int error = 0;
...
SENDING_NOTIFICATION__THIS_COALITION_IS_CAUSING_TOO_MUCH_IO(int flavor) {
  ...
  os_log(
      OS_LOG_DEFAULT,
      "Coalition [%lld] caught causing excessive I/O (flavor: %d). Task I/O: "
      "%lld MB. [Limit : %lld MB per %lld secs]. Triggered by process [%d]\n",
      coalition->id, flavor, (lei.lei_balance / (1024 * 1024)),
      (lei.lei_limit / (1024 * 1024)), (lei.lei_refill_period / NSEC_PER_SEC),
      pid);
  ...
  /*
   * Continue to monitor the coalition after it hits the initital limit, but
   * increase the limit exponentially so that we don't spam the listener.
   */
...
void coalition_io_rate_exceeded(int warning, const void *param0,
...
void init_coalition_ledgers(void) {
  ...
  coalition_ledgers.logical_writes =
  ...
  ledger_set_callback(t, coalition_ledgers.logical_writes,
                      coalition_io_rate_exceeded,
                      (void *)FLAVOR_IO_LOGICAL_WRITES, NULL);
  ledger_template_complete(t);

  coalition_task_ledger_template =
      ledger_template_copy(task_ledger_template, "Coalition task ledgers");
...
void coalition_io_ledger_update(task_t task, int32_t flavor,
                                boolean_t is_credit, uint32_t io_size) {
  ledger_t ledger;
  coalition_t coal = task->coalition[COALITION_TYPE_RESOURCE];
...
static void coalition_notify_user(uint64_t id, uint32_t flags) {
...
#pragma unused(privileged, efficient)
  ...
  coal->r.resource_monitor_ledger = ledger_instantiate(
      coalition_ledger_template, LEDGER_CREATE_ACTIVE_ENTRIES);
...
static kern_return_t i_coal_resource_adopt_task(coalition_t coal, task_t task) {
  ...
  if (cr->task_count < cr->dead_task_count) {
    panic("%s: coalition %p id:%llu type:%s task_count(%llu) < "
          "dead_task_count(%llu)",
          __func__, coal, coal->id, coal_type_str(coal->type), cr->task_count,
          cr->dead_task_count);
  }
  ...
  coal_dbg("Added PID:%d to id:%llu, task_count:%llu, dead_count:%llu, "
           "nonempty_time:%llu",
...
static kern_return_t i_coal_resource_remove_task(coalition_t coal,
                                                 task_t task) {
  struct i_resource_coalition *cr;
  ...
  if (cr->task_count - cr->dead_task_count == 0) {
    uint64_t last_time_nonempty =
        mach_absolute_time() - cr->last_became_nonempty_time;
  ...
  if (!task_is_exec_copy(task) && !task_did_exec(task)) {
    ...
    cr->byteswritten += task->task_io_stats->total_io.size -
    ...
    cr->logical_immediate_writes +=
        task->task_writes_counters_internal.task_immediate_writes;
    cr->logical_deferred_writes +=
        task->task_writes_counters_internal.task_deferred_writes;
    cr->logical_invalidated_writes +=
        task->task_writes_counters_internal.task_invalidated_writes;
    cr->logical_metadata_writes +=
        task->task_writes_counters_internal.task_metadata_writes;
    cr->logical_immediate_writes_to_external +=
        task->task_writes_counters_external.task_immediate_writes;
    cr->logical_deferred_writes_to_external +=
        task->task_writes_counters_external.task_deferred_writes;
    cr->logical_invalidated_writes_to_external +=
        task->task_writes_counters_external.task_invalidated_writes;
    cr->logical_metadata_writes_to_external +=
        task->task_writes_counters_external.task_metadata_writes;
#if CONFIG_PHYS_WRITE_ACCT
...
static kern_return_t i_coal_resource_set_taskrole(__unused coalition_t coal,
                                                  __unused task_t task,
                                                  __unused int role) {
  return KERN_SUCCESS;
}

static int i_coal_resource_get_taskrole(__unused coalition_t coal,
                                        __unused task_t task) {
  task_t t;

  assert(coal && coal->type == COALITION_TYPE_RESOURCE);

  qe_foreach_element(t, &coal->r.tasks,
                     task_coalition[COALITION_TYPE_RESOURCE]) {
    if (t == task) {
...
coalition_resource_usage_internal(coalition_t coal,
                                  struct coalition_resource_usage *cru_out) {
  kern_return_t kr;
  ...
  uint64_t logical_immediate_writes_to_external =
      coal->r.logical_immediate_writes_to_external;
  uint64_t logical_deferred_writes_to_external =
      coal->r.logical_deferred_writes_to_external;
  uint64_t logical_invalidated_writes_to_external =
      coal->r.logical_invalidated_writes_to_external;
  uint64_t logical_metadata_writes_to_external =
      coal->r.logical_metadata_writes_to_external;
   ...
   */
  task_t task;
  qe_foreach_element(task, &coal->r.tasks,
                     task_coalition[COALITION_TYPE_RESOURCE]) {
    int64_t task_phys_footprint = 0;

    /*
     * Rolling up stats for exec copy task or exec'd task will lead to double
     * accounting. Cannot take task lock after taking coaliton lock
     */
    if (task_is_exec_copy(task) || task_did_exec(task)) {
      continue;
    }
    ...
    byteswritten += task->task_io_stats->total_io.size -
                    task->task_io_stats->disk_reads.size;
#if defined(__x86_64__)
    ...
    /* The exited process ledger can have a phys_footprint balance, which should
     * be ignored. */
    kr = ledger_get_balance(task->ledger, task_ledgers.phys_footprint,
                            (int64_t *)&task_phys_footprint);
  ...
  kr =
      ledger_get_balance(sum_ledger, task_ledgers.swapins, (int64_t *)&swapins);
  ...
  cru_out->gpu_energy_nj_billed_to_me =
      os_atomic_load(&coal->r.gpu_energy_nj_billed_to_me, relaxed);
  cru_out->gpu_energy_nj_billed_to_others =
      os_atomic_load(&coal->r.gpu_energy_nj_billed_to_others, relaxed);

  coalition_unlock(coal);

  /* Copy the totals out of sum_ledger */
  kr = ledger_get_entries(sum_ledger, task_ledgers.cpu_time, &credit, &debit);
  if (kr != KERN_SUCCESS) {
    credit = 0;
  }
  ...
  kr = ledger_get_entries(sum_ledger, task_ledgers.interrupt_wakeups, &credit,
                          &debit);
  if (kr != KERN_SUCCESS) {
    credit = 0;
  }
  ...
  cru_out->logical_immediate_writes_to_external =
      logical_immediate_writes_to_external;
  cru_out->logical_deferred_writes_to_external =
      logical_deferred_writes_to_external;
  cru_out->logical_invalidated_writes_to_external =
      logical_invalidated_writes_to_external;
  cru_out->logical_metadata_writes_to_external =
      logical_metadata_writes_to_external;
#if CONFIG_PHYS_WRITE_ACCT
...
bool coalition_add_to_gpu_energy(uint64_t coalition_id,
                                 coalition_gpu_energy_t which,
                                 uint64_t energy) {
  ...
  coalition_t coal =
      smr_hash_entered_find(&coalition_hash, key, &coal_hash_traits);
  /* Preemption is disabled, and until we leave, coalition_free won't be called.
   */

  if (coal != COALITION_NULL && coal->type == COALITION_TYPE_RESOURCE &&
    ...
    if (which & CGE_BILLED) {
      os_atomic_add(&coal->r.gpu_energy_nj_billed_to_me, energy, relaxed);
    }

    if (which & CGE_OTHERS) {
      os_atomic_add(&coal->r.gpu_energy_nj_billed_to_others, energy, relaxed);
    }
  }
...
coalition_debug_info_internal(coalition_t coal,
  ...
  if (group != NULL) {
    ...
    c_debuginfo->thread_group_recommendation =
        thread_group_recommendation(group);
  }
...
static kern_return_t i_coal_jetsam_init(coalition_t coal, boolean_t privileged,
  ...
  default:
    coal->j.thread_group = thread_group_create_and_retain(
        efficient ? THREAD_GROUP_FLAGS_EFFICIENT : THREAD_GROUP_FLAGS_DEFAULT);
  }
...
static void i_coal_jetsam_dealloc(__unused coalition_t coal) {
  assert(coal && coal->type == COALITION_TYPE_JETSAM);
...
static kern_return_t i_coal_jetsam_adopt_task(coalition_t coal, task_t task) {
  ...
  coal_dbg("coalition %lld adopted PID:%d as UNDEF", coal->id, task_pid(task));
...
static kern_return_t i_coal_jetsam_remove_task(coalition_t coal, task_t task) {
  ...
  coal_dbg("removing PID:%d from coalition id:%lld", task_pid(task), coal->id);
...
static kern_return_t i_coal_jetsam_set_taskrole(coalition_t coal, task_t task,
  ...
  case COALITION_TASKROLE_LEADER:
    coal_dbg("setting PID:%d as LEADER of %lld", task_pid(task), coal->id);
    if (cj->leader != TASK_NULL) {
      /* re-queue the exiting leader onto the "other" list */
      coal_dbg("    re-queue existing leader (%d) as OTHER",
               task_pid(cj->leader));
      re_queue_tail(&cj->other,
                    &cj->leader->task_coalition[COALITION_TYPE_JETSAM]);
    }
    /*
  ...
  case COALITION_TASKROLE_XPC:
    coal_dbg("setting PID:%d as XPC in %lld", task_pid(task), coal->id);
    q = (queue_t)&cj->services;
    break;
  case COALITION_TASKROLE_EXT:
    coal_dbg("setting PID:%d as EXT in %lld", task_pid(task), coal->id);
  ...
  case COALITION_TASKROLE_NONE:
    ...
    if (task->coalition[COALITION_TYPE_JETSAM] != coal) {
      panic("%s: task %p attempting to set role %d "
            "in coalition %p to which it does not belong!",
            __func__, task, role, coal);
    }
    OS_FALLTHROUGH;
  case COALITION_TASKROLE_UNDEF:
    coal_dbg("setting PID:%d as UNDEF in %lld", task_pid(task), coal->id);
    q = (queue_t)&cj->other;
    break;
  default:
...
int i_coal_jetsam_get_taskrole(coalition_t coal, task_t task) {
  ...
  qe_foreach_element(t, &cj->extensions,
                     task_coalition[COALITION_TYPE_JETSAM]) {
    if (t == task) {
 ...
 */
kern_return_t coalition_create_internal(int type, int role,
                                        boolean_t privileged,
                                        boolean_t efficient, coalition_t *out,
...
#if CONFIG_THREAD_GROUPS
  KDBG_RELEASE(
      MACHDBG_CODE(DBG_MACH_COALITION, MACH_COALITION_NEW), new_coal->id,
      new_coal->type,
      (new_coal->type == COALITION_TYPE_JETSAM && new_coal->j.thread_group)
          ? thread_group_get_id(new_coal->j.thread_group)
          : 0);
  ...
  if (smr_hash_serialized_should_grow(&coalition_hash, 1, 4)) {
    ...
    smr_hash_grow_and_unlock(&coalition_hash, &coalitions_list_lock,
                             &coal_hash_traits);
  } else {
...
static void coalition_free(struct smr_node *node) {
...
static __attribute__((noinline)) void coalition_retire(coalition_t coal) {
  coalition_lock(coal);

  coal_dbg("id:%llu type:%s active_count:%u%s", coal->id,
           coal_type_str(coal->type), coal->active_count,
...
#if CONFIG_THREAD_GROUPS
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_COALITION, MACH_COALITION_FREE), coal->id,
               coal->type,
               coal->type == COALITION_TYPE_JETSAM ? coal->j.thread_group : 0);
#else
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_COALITION, MACH_COALITION_FREE), coal->id,
               coal->type);
#endif
 ...
 * coalition_retain
 * */
void coalition_retain(coalition_t coal) { coal_ref_retain(coal); }
 ...
 */
coalition_t coalition_find_by_id(uint64_t cid) {
 ...
 */
coalition_t coalition_find_and_activate_by_id(uint64_t cid) {
  ...
  coal_dbg("id:%llu type:%s ref_count:%u, active_count:%u", coal->id,
           coal_type_str(coal->type), rc, ac);
...
void task_coalition_roles(task_t task, int roles[COALITION_NUM_TYPES]) {
  ...
  for (i = 0; i < COALITION_NUM_TYPES; i++) {
    if (task->coalition[i]) {
      ...
      roles[i] = coal_call(task->coalition[i], get_taskrole, task);
...
int task_coalition_role_for_type(task_t task, int coalition_type) {
  ...
  if (coalition_type >= COALITION_NUM_TYPES) {
    panic("Attempt to call task_coalition_role_for_type with invalid "
          "coalition_type: %d\n",
          coalition_type);
  }
...
int coalition_type(coalition_t coal) { return coal->type; }

boolean_t coalition_term_requested(coalition_t coal) {
  return coal->termrequested;
}

boolean_t coalition_is_terminated(coalition_t coal) { return coal->terminated; }

boolean_t coalition_is_reaped(coalition_t coal) { return coal->reaped; }
...
void coalition_update_ane_stats(coalition_t coalition, uint64_t ane_mach_time,
                                uint64_t ane_energy_nj) {
...
boolean_t task_coalition_adjust_focal_count(task_t task, int count,
                                            uint32_t *new_count) {
...
boolean_t task_coalition_adjust_nonfocal_count(task_t task, int count,
                                               uint32_t *new_count) {
  coalition_t coal = task->coalition[COALITION_FOCAL_TASKS_ACCOUNTING];
  if (coal == COALITION_NULL) {
    return FALSE;
...
bool task_coalition_adjust_game_mode_count(task_t task, int count,
                                           uint32_t *new_count) {
  coalition_t coal = task->coalition[COALITION_FOCAL_TASKS_ACCOUNTING];
  if (coal == COALITION_NULL) {
    return false;
...
bool task_coalition_adjust_carplay_mode_count(task_t task, int count,
                                              uint32_t *new_count) {
  coalition_t coal = task->coalition[COALITION_FOCAL_TASKS_ACCOUNTING];
  if (coal == COALITION_NULL) {
    return false;
...
void task_coalition_thread_group_focal_update(task_t task) {
  ...
  if (focal_count) {
    thread_group_set_flags_locked(task_coalition_get_thread_group(task),
                                  THREAD_GROUP_FLAGS_UI_APP);
  } else {
    thread_group_clear_flags_locked(task_coalition_get_thread_group(task),
                                    THREAD_GROUP_FLAGS_UI_APP);
  }
...
static void coalition_for_each_task_locked(
    coalition_t coal, OS_NOESCAPE coalition_for_each_task_block_t block) {
  assert(coal != COALITION_NULL);
...
static task_t
coalition_select_task(coalition_t coal,
                      OS_NOESCAPE coalition_for_each_task_block_t block) {
  assert(coal != COALITION_NULL);
...
void coalition_remove_active(coalition_t coal) {
  ...
  coal_dbg("id:%llu type:%s ref_count:%u, active_count:%u,%s", cid,
           coal_type_str(ct), rc, ac, do_notify ? " NOTIFY" : " ");
 ...
 */
static kern_return_t coalition_adopt_task_internal(coalition_t coal,
                                                   task_t task) {
  kern_return_t kr;
...
out_unlock:
  ...
  coal_dbg("task:%d, id:%llu type:%s ref_count:%u, kr=%d", task_pid(task), cid,
           coal_type_str(ct), rc, kr);
  return kr;
}

static kern_return_t coalition_remove_task_internal(task_t task, int type) {
  kern_return_t kr;
  ...
  coal_dbg("id:%llu type:%s ref_count:%u, active_count:%u, kr=%d", cid,
           coal_type_str(ct), rc, ac, kr);
 ...
 */
kern_return_t coalitions_adopt_task(coalition_t *coals, task_t task) {
  int i;
  kern_return_t kr;
 ...
 * Condition: task must be referenced and UNLOCKED; all task's coalitions must
 * be UNLOCKED
 */
...
void task_release_coalitions(task_t task) {
  ...
  for (i = 0; i < COALITION_NUM_TYPES; i++) {
    if (task->coalition[i]) {
      ...
      panic("deallocating task %p was not a member of a resource coalition",
            task);
    }
  }
}
...
kern_return_t coalition_reap_internal(coalition_t coal) {
  ...
  lck_mtx_lock(&coalitions_list_lock);
  smr_hash_serialized_remove(&coalition_hash, &coal->link, &coal_hash_traits);
  if (smr_hash_serialized_should_shrink(&coalition_hash,
    ...
    smr_hash_shrink_and_unlock(&coalition_hash, &coalitions_list_lock,
                               &coal_hash_traits);
  } else {
    lck_mtx_unlock(&coalitions_list_lock);
  }
...
void coalitions_init(void) {
  ...
  for (i = 0, ctype = &s_coalition_types[0]; i < COALITION_NUM_TYPES;
       ctype++, i++) {
    /* verify the entry in the global coalition types array */
    if (ctype->type != i || !ctype->init || !ctype->dealloc ||
        !ctype->adopt_task || !ctype->remove_task) {
      panic("%s: Malformed coalition type %s(%d) in slot for type:%s(%d)",
            __func__, coal_type_str(ctype->type), ctype->type, coal_type_str(i),
            i);
    }
    if (!ctype->has_default) {
      continue;
    }
    kr = coalition_create_internal(ctype->type, COALITION_ROLE_SYSTEM, TRUE,
                                   FALSE, &init_coalition[ctype->type], NULL);
    if (kr != KERN_SUCCESS) {
      panic("%s: could not create init %s coalition: kr:%d", __func__,
            coal_type_str(i), kr);
    }
    if (i == COALITION_TYPE_RESOURCE) {
      assert(COALITION_ID_KERNEL == init_coalition[ctype->type]->id);
    }
    kr = coalition_create_internal(ctype->type, COALITION_ROLE_SYSTEM, FALSE,
                                   FALSE, &corpse_coalition[ctype->type], NULL);
    if (kr != KERN_SUCCESS) {
      panic("%s: could not create corpse %s coalition: kr:%d", __func__,
            coal_type_str(i), kr);
    }
  }
 ...
 */
coalition_t task_get_coalition(task_t task, int coal_type) {
  coalition_t c;
...
kern_return_t coalition_iterate_stackshot(coalition_iterate_fn_t callout,
                                          void *arg, uint32_t coalition_type) {
  coalition_t coal;
  int i = 0;

  smr_hash_foreach(coal, &coalition_hash, &coal_hash_traits) {
    if (coal == NULL ||
        !ml_validate_nofault((vm_offset_t)coal, sizeof(struct coalition))) {
      return KERN_FAILURE;
    }
...
int coalition_get_task_count(coalition_t coal) {
  ...
  switch (coal->type) {
  case COALITION_TYPE_RESOURCE:
    qe_foreach(qe, &coal->r.tasks) ntasks++;
    break;
  case COALITION_TYPE_JETSAM:
    if (coal->j.leader) {
      ntasks++;
    }
    qe_foreach(qe, &coal->j.other) ntasks++;
    qe_foreach(qe, &coal->j.extensions) ntasks++;
    qe_foreach(qe, &coal->j.services) ntasks++;
    break;
  default:
    break;
  }
...
static uint64_t i_get_list_footprint(queue_t list, int type, int *ntasks) {
  ...
  qe_foreach_element(task, list, task_coalition[type]) {
    ...
    coal_dbg("    [%d] task_pid:%d, type:%d, footprint:%lld", *ntasks,
             task_pid(task), type, bytes);
    *ntasks += 1;
  }
...
uint64_t coalition_get_page_count(coalition_t coal, int *ntasks) {
  ...
  switch (coal->type) {
  case COALITION_TYPE_RESOURCE:
    bytes += i_get_list_footprint(&coal->r.tasks, COALITION_TYPE_RESOURCE,
                                  &num_tasks);
    break;
  case COALITION_TYPE_JETSAM:
    if (coal->j.leader) {
    ...
    bytes += i_get_list_footprint(&coal->j.extensions, COALITION_TYPE_JETSAM,
                                  &num_tasks);
    bytes += i_get_list_footprint(&coal->j.services, COALITION_TYPE_JETSAM,
                                  &num_tasks);
    bytes +=
        i_get_list_footprint(&coal->j.other, COALITION_TYPE_JETSAM, &num_tasks);
    break;
  default:
    break;
  }
...
static int coalition_get_sort_list(coalition_t coal, int sort_order,
                                   queue_t list, struct coal_sort_s *sort_array,
                                   int array_sz) {
  int ntasks = 0;
  task_t task;
  ...
  if (!list) {
     ...
     */
    if (coal->type != COALITION_TYPE_JETSAM || coal->j.leader == TASK_NULL) {
      return 0;
    }
...
int coalition_get_pid_list(coalition_t coal, uint32_t rolemask, int sort_order,
  ...
  struct coal_sort_s sort_array[MAX_SORTED_PIDS] = {
      {0, 0, 0}}; /* keep to < 2k */

  if (!coal || !(rolemask & COALITION_ROLEMASK_ALLROLES) || !pid_list ||
      list_sz < 1) {
    coal_dbg("Invalid parameters: coal:%p, type:%d, rolemask:0x%x, "
             "pid_list:%p, list_sz:%d",
             coal, coal ? coal->type : -1, rolemask, pid_list, list_sz);
    return -EINVAL;
  }
  ...
  if (rolemask & COALITION_ROLEMASK_UNDEF) {
    ntasks +=
        coalition_get_sort_list(coal, sort_order, &cj->other,
                                sort_array + ntasks, MAX_SORTED_PIDS - ntasks);
  }

  if (rolemask & COALITION_ROLEMASK_XPC) {
    ntasks +=
        coalition_get_sort_list(coal, sort_order, &cj->services,
                                sort_array + ntasks, MAX_SORTED_PIDS - ntasks);
  }

  if (rolemask & COALITION_ROLEMASK_EXT) {
    ntasks +=
        coalition_get_sort_list(coal, sort_order, &cj->extensions,
                                sort_array + ntasks, MAX_SORTED_PIDS - ntasks);
  }

  if (rolemask & COALITION_ROLEMASK_LEADER) {
    ntasks += coalition_get_sort_list(
        coal, sort_order, NULL, sort_array + ntasks, MAX_SORTED_PIDS - ntasks);
  }
...
unlock_coal:
  ...
  for (int i = 0; i < ntasks; i++) {
    ...
    coal_dbg(" [%d] PID:%d, footprint:%lld, usr_order:%d", i, sort_array[i].pid,
             sort_array[i].bytes, sort_array[i].usr_order);
...
static uintptr_t crequested(coalition_t coal) {
  static_assert(sizeof(struct coalition_requested_policy) == sizeof(uint64_t),
...
static uintptr_t ceffective(coalition_t coal) {
  static_assert(sizeof(struct coalition_requested_policy) == sizeof(uint64_t),
                "size invariant violated");
...
static uint32_t cpending(coalition_pend_token_t pend_token) {
  static_assert(sizeof(struct coalition_pend_token) == sizeof(uint32_t),
                "size invariant violated");
...
static void
jetsam_coalition_policy_update_locked(coalition_t coal,
                                      coalition_pend_token_t pend_token) {
 ...
 */
static task_t coalition_find_next_task_with_pending_work(coalition_t coal) {
 ...
 * Called with global coalition_policy_lock held.
 */
static void coalition_policy_update_complete_unlocked(
  ...
  if (coalition_pend_token->cpt_update_j_coal_tasks) {
    ...
    while ((task = coalition_find_next_task_with_pending_work(coal)) !=
           TASK_NULL) {
      task_policy_update_complete_unlocked(task,
                                           &task->pended_coalition_changes);
...
kern_return_t jetsam_coalition_set_policy(coalition_t coal, int flavor,
                                          int value) {
  ...
  KERNEL_DEBUG_CONSTANT_IST(KDEBUG_TRACE,
                            (IMPORTANCE_CODE(flavor, TASK_POLICY_COALITION)) |
                                DBG_FUNC_START,
...
kern_return_t jetsam_coalition_get_policy(coalition_t coal, int flavor,
                                          int *value) {
  lck_mtx_lock(&coalition_policy_lock);
...
bool task_get_effective_jetsam_coalition_policy(task_t task, int flavor) {
  coalition_t coal = task->coalition[COALITION_TYPE_JETSAM];
