...
static void post_alarm(alarm_t alarm);

static void set_alarm(mach_timespec_t *alarm_time);

static int check_time(alarm_type_t alarm_type, mach_timespec_t *alarm_time,
...
static kern_return_t clock_sleep_internal(clock_t clock,
                                          sleep_type_t sleep_type,
                                          mach_timespec_t *sleep_time);

int rtclock_init(void);

kern_return_t rtclock_gettime(mach_timespec_t *cur_time);

kern_return_t rtclock_getattr(clock_flavor_t flavor, clock_attr_t attr,
                              mach_msg_type_number_t *count);

SECURITY_READ_ONLY_EARLY(struct clock_ops)
sysclk_ops = {
    .c_config = NULL,
    .c_init = rtclock_init,
...
kern_return_t calend_gettime(mach_timespec_t *cur_time);

kern_return_t calend_getattr(clock_flavor_t flavor, clock_attr_t attr,
                             mach_msg_type_number_t *count);
 ...
 */
SECURITY_READ_ONLY_LATE(struct clock)
clock_list[] = {
    [SYSTEM_CLOCK] =
        {
            .cl_ops = &sysclk_ops,
 ...
 */
void clock_service_create(void) {
  /*
 ...
 */
kern_return_t clock_get_time(clock_t clock, mach_timespec_t *cur_time) /* OUT */
{
  if (clock == CLOCK_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
...
kern_return_t rtclock_gettime(mach_timespec_t *time) /* OUT */
{
  clock_sec_t secs;
  clock_nsec_t nsecs;
...
kern_return_t calend_gettime(mach_timespec_t *time) /* OUT */
{
  clock_sec_t secs;
  clock_nsec_t nsecs;
 ...
 */
kern_return_t clock_get_attributes(clock_t clock, clock_flavor_t flavor,
...
kern_return_t rtclock_getattr(clock_flavor_t flavor,
                              clock_attr_t attr,             /* OUT */
                              mach_msg_type_number_t *count) /* IN/OUT */
{
  if (*count != 1) {
...
kern_return_t calend_getattr(clock_flavor_t flavor, clock_attr_t attr, /* OUT */
                             mach_msg_type_number_t *count) /* IN/OUT */
{
  if (*count != 1) {
    return KERN_FAILURE;
 ...
 * Setup a clock alarm.
 */
kern_return_t clock_alarm(clock_t clock, alarm_type_t alarm_type,
                          mach_timespec_t alarm_time, ipc_port_t alarm_port,
  ...
  if (chkstat <= 0) {
    ...
    clock_alarm_reply(alarm_port, alarm_port_type, reply_code, alarm_type,
                      clock_time);
    return KERN_SUCCESS;
  }
 ...
 */
kern_return_t clock_sleep_trap(struct clock_sleep_trap_args *args) {
...
static void set_alarm(mach_timespec_t *alarm_time) {
  ...
  timer_call_enter_with_leeway(&alarm_expire_timer, NULL, abstime, 0,
                               TIMER_CALL_USER_NORMAL, FALSE);
}
