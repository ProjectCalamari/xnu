 ...
 * system during boot. This mechanism sets up a periodic timer and on every
 * timer fire, captures a full backtrace into the boot profiling buffer. This
 * buffer can be pulled out and analyzed from user-space. It is turned on using
 * the following boot-args: "bootprofile_buffer_size" specifies the size of the
 * boot profile buffer "bootprofile_interval_ms" specifies the interval for the
 * profiling timer
 *
 ...
 * Trigger Events
 *
 * The boot-arg "bootprofile_type=boot" starts the timer during early boot.
 * Using "wake" starts the timer at AP wake from suspend-to-RAM.
 */
...
#define BOOTPROFILE_MAX_BUFFER_SIZE                                            \
  (64 * 1024 * 1024) /* see also COPYSIZELIMIT_PANIC */
...
#define BOOTPROFILE_LOCK()                                                     \
  do {                                                                         \
    lck_mtx_lock(&bootprofile_mtx);                                            \
  } while (0)
#define BOOTPROFILE_TRY_SPIN_LOCK() lck_mtx_try_lock_spin(&bootprofile_mtx)
#define BOOTPROFILE_UNLOCK()                                                   \
  do {                                                                         \
    lck_mtx_unlock(&bootprofile_mtx);                                          \
  } while (0)

static void bootprofile_timer_call(timer_call_param_t param0,
...
static void _bootprofile_init(void) {
  ...
  clock_interval_to_absolutetime_interval(
      bootprofile_interval_ms, NSEC_PER_MSEC, &bootprofile_interval_abs);

  /* Both boot args must be set to enable */
  if ((bootprofile_type == kBootProfileDisabled) ||
      (bootprofile_buffer_size == 0) || (bootprofile_interval_abs == 0)) {
    return;
  }

  kern_return_t ret =
      kmem_alloc(kernel_map, &bootprofile_buffer, bootprofile_buffer_size,
  ...
  kprintf("Boot profile: Sampling %s once per %u ms at %s\n",
          bootprofile_all_procs ? "all procs" : bootprofile_proc_name,
          bootprofile_interval_ms,
          bootprofile_type == kBootProfileStartTimerAtBoot
              ? "boot"
              : (bootprofile_type == kBootProfileStartTimerAtWake ? "wake"
                                                                  : "unknown"));

  timer_call_setup(&bootprofile_timer_call_entry, bootprofile_timer_call, NULL);

  if (bootprofile_type == kBootProfileStartTimerAtBoot) {
    bootprofile_next_deadline = mach_absolute_time() + bootprofile_interval_abs;
    timer_call_enter_with_leeway(&bootprofile_timer_call_entry, NULL,
                                 bootprofile_next_deadline, 0,
                                 TIMER_CALL_SYS_NORMAL, false);
  }
}

STARTUP(SYSCTL, STARTUP_RANK_FIRST, _bootprofile_init);

void bootprofile_wake_from_sleep(void) {
...
static void bootprofile_timer_call(timer_call_param_t param0 __unused,
  ...
  if (!bootprofile_all_procs) {
    ...
    if ((current_task() != NULL) &&
        (get_bsdtask_info(current_task()) != NULL) &&
        (0 == strncmp(bootprofile_proc_name,
  ...
  if (bootprofile_buffer_current_position < bootprofile_buffer_size) {
    uint64_t flags = STACKSHOT_KCDATA_FORMAT | STACKSHOT_TRYLOCK |
    ...
    if ((flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT) &&
        (bootprofile_delta_since_timestamp == 0)) {
    ...
    kern_return_t r = stack_snapshot_from_kernel(
        ...
        bootprofile_buffer_size - bootprofile_buffer_current_position, flags,
        bootprofile_delta_since_timestamp, 0, &retbytes);
   ...
   */
  if ((retbytes == 0) ||
      (bootprofile_buffer_current_position == bootprofile_buffer_size)) {
    return;
  }
