...
boolean_t thread_affinity_is_supported(void) {
 ...
 */
uint32_t thread_affinity_get(thread_t thread) {
  uint32_t tag;
 ...
 */
void task_affinity_create(task_t parent_task, task_t child_task) {
 ...
 */
void task_affinity_deallocate(task_t task) {
 ...
 */
kern_return_t task_affinity_info(task_t task, task_info_t task_info_out,
  ...
  *task_info_count = TASK_AFFINITY_TAG_INFO_COUNT;
  ...
  if (aspc) {
    lck_mtx_lock(&aspc->aspc_lock);
    queue_iterate(&aspc->aspc_affinities, aset, affinity_set_t,
                  aset_affinities) {
      info->set_count++;
 ...
 */
void thread_affinity_dup(thread_t parent, thread_t child) {
  affinity_set_t aset;
  affinity_space_t aspc;
 ...
 */
void thread_affinity_terminate(thread_t thread) {
 ...
 */
void thread_affinity_exec(thread_t thread) {
...
static void affinity_set_add(affinity_set_t aset, thread_t thread) {
  ...
  queue_enter(&aset->aset_threads, thread, thread_t, affinity_threads);
 ...
 * The caller must have the thread mutex and space locked.
 */
static affinity_set_t affinity_set_remove(affinity_set_t aset,
                                          thread_t thread) {
  spl_t s;
  ...
  aset->aset_thread_count--;
  queue_remove(&aset->aset_threads, thread, thread_t, affinity_threads);
  if (queue_empty(&aset->aset_threads)) {
    queue_remove(&aset->aset_space->aspc_affinities, aset, affinity_set_t,
                 aset_affinities);
 ...
 */
static affinity_set_t affinity_set_find(affinity_space_t space, uint32_t tag) {
  affinity_set_t aset;

  queue_iterate(&space->aspc_affinities, aset, affinity_set_t,
                aset_affinities) {
 ...
 * The caller must have the space locked.
 */
static void affinity_set_place(affinity_space_t aspc, affinity_set_t new_aset) {
  ...
  if (__improbable(num_cpu_asets > MAX_CPUS)) {
    ...
    panic("num_cpu_asets = %d > %d too big in %s", num_cpu_asets, MAX_CPUS,
          __FUNCTION__);
  }
   ...
   */
  queue_iterate(&aspc->aspc_affinities, aset, affinity_set_t, aset_affinities) {
  ...
  queue_enter(&aspc->aspc_affinities, new_aset, affinity_set_t,
              aset_affinities);

  DBG("affinity_set_place(%p,%p) selected affinity %u pset %p\n", aspc,
      new_aset, new_aset->aset_num, new_aset->aset_pset);
}
