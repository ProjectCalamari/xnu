 ...
 */
kern_return_t pfz_exit(__unused struct pfz_exit_args *args) {
...
boolean_t swtch(__unused struct swtch_args *args) {
  processor_t myprocessor;
...
boolean_t swtch_pri(__unused struct swtch_pri_args *args) {
  processor_t myprocessor;
...
static void thread_switch_continue(void *parameter, __unused int ret) {
 ...
 */
kern_return_t thread_switch(struct thread_switch_args *args) {
  ...
  if (option == SWITCH_OPTION_OSLOCK_DEPRESS ||
  ...
  if (thread != THREAD_NULL) {
    ...
    KERNEL_DEBUG_CONSTANT(
        MACHDBG_CODE(DBG_MACH_SCHED, MACH_SCHED_THREAD_SWITCH) | DBG_FUNC_NONE,
        thread_tid(thread), thread->state, pulled_thread ? TRUE : FALSE, 0, 0);
    ...
    if (pulled_thread != THREAD_NULL) {
      ...
      thread_run(self, thread_switch_continue, (void *)(intptr_t)option,
                 pulled_thread);
  ...
  if (wait_option) {
    ...
    disable_preemption();
    bool should_yield =
        SCHED(thread_should_yield)(current_processor(), current_thread());
    enable_preemption();
  ...
  thread_yield_with_continuation(thread_switch_continue,
                                 (void *)(intptr_t)option);
  __builtin_unreachable();
}

void thread_yield_with_continuation(thread_continue_t continuation,
                                    void *parameter) {
  assert(continuation);
 ...
 */
static wait_result_t thread_handoff_internal(thread_t thread,
...
void thread_handoff_parameter(thread_t thread, thread_continue_t continuation,
...
wait_result_t thread_handoff_deallocate(thread_t thread,
                                        thread_handoff_option_t option) {
...
void thread_depress_timer_setup(thread_t self) {
  self->depress_timer =
      kalloc_type(struct timer_call, Z_ZERO | Z_WAITOK | Z_NOFAIL);
 ...
 */
void thread_depress_abstime(uint64_t interval) {
  thread_t self = current_thread();
...
void thread_depress_ms(mach_msg_timeout_t interval) {
  uint64_t abstime;
 ...
 */
void thread_depress_expire(void *p0, __unused void *p1) {
 ...
 */
kern_return_t thread_depress_abort_locked(thread_t thread) {
 ...
 *
 * Forces a fixpri thread to yield if it is detected polling without blocking
 * for too long.
 */
void thread_poll_yield(thread_t self) {
  ...
  uint64_t total_computation =
      abstime - self->computation_epoch + self->computation_metered;
 ...
 *
 * WARNING: Will still yield to priority 0 even if the thread is holding a
 * contended lock!
 */
void thread_yield_internal(mach_msg_timeout_t ms) {
  thread_t self = current_thread();

  assert((self->sched_flags & TH_SFLAG_DEPRESSED_MASK) !=
         TH_SFLAG_DEPRESSED_MASK);
...
/*
 * This yields to a possible non-urgent preemption pending on the current
 * processor.
 *
 * This is useful when doing a long computation in the kernel without returning
 * to userspace.
 *
 * As opposed to other yielding mechanisms, this does not drop the priority of
 * the current thread.
 */
void thread_yield_to_preemption() {
  /*
