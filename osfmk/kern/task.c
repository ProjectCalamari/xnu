...
static void task_port_with_flavor_no_senders(ipc_port_t,
...
static void task_set_control_port_options(task_t task,
                                          task_control_port_options_t opts);
...
kern_return_t task_crash_info_conclave_upcall(
    task_t task, const struct conclave_sharedbuffer_t *shared_buf,
    uint32_t length);
#endif /* CONFIG_EXCLAVES */

IPC_KOBJECT_DEFINE(IKOT_TASK_NAME, .iko_op_movable_send = true);
IPC_KOBJECT_DEFINE(
    IKOT_TASK_CONTROL, .iko_op_no_senders = task_port_no_senders,
...
IPC_KOBJECT_DEFINE(IKOT_TASK_INSPECT,
                   .iko_op_no_senders = task_port_with_flavor_no_senders);
IPC_KOBJECT_DEFINE(IKOT_TASK_RESUME, .iko_op_movable_send = true,
...
IPC_KOBJECT_DEFINE(IKOT_TASK_FATAL, .iko_op_movable_send = true,
                   .iko_op_stable = true,
...
/* Flag set by core audio when audio is playing. Used to stifle EXC_RESOURCE
 * generation when active. */
int audio_active = 0;
...
/* global lock for task_dyld_process_info_notify_{register, deregister,
 * get_trap} */
LCK_GRP_DECLARE(g_dyldinfo_mtx_grp, "g_dyldinfo");
LCK_MTX_DECLARE(g_dyldinfo_mtx, &g_dyldinfo_mtx_grp);

SECURITY_READ_ONLY_LATE(struct _task_ledger_indices)
task_ledgers __attribute__((used)) = {
    .cpu_time = -1,
    .tkm_private = -1,
    .tkm_shared = -1,
    .phys_mem = -1,
...
__options_decl(send_exec_resource_is_fatal, bool,
               {IS_NOT_FATAL = false, IS_FATAL = true});

__options_decl(send_exec_resource_is_diagnostics, bool,
               {IS_NOT_DIAGNOSTICS = false, IS_DIAGNOSTICS = true});

__options_decl(send_exec_resource_is_warning, bool,
               {IS_NOT_WARNING = false, IS_WARNING = true});

__options_decl(
    send_exec_resource_options_t, uint8_t,
    {
        EXEC_RESOURCE_FATAL = 0x01, EXEC_RESOURCE_DIAGNOSTIC = 0x02,
        EXEC_RESOURCE_WARNING = 0x04,
        EXEC_RESOURCE_CONCLAVE =
            0x08 // A side memory limit independent of the main footprint.
    });

/**
 * Actions to take when a process has reached the memory limit or the
 * diagnostics threshold limits
 */
static inline void task_process_crossed_limit_no_diag(
    task_t task, ledger_amount_t ledger_limit_size, bool memlimit_is_fatal,
    bool memlimit_is_active, send_exec_resource_is_warning is_warning);
#if DEBUG || DEVELOPMENT
static inline void
task_process_crossed_limit_diag(ledger_amount_t ledger_limit_size);
#endif
void init_task_ledgers(void);
void task_footprint_exceeded(int warning, __unused const void *param0,
                             __unused const void *param1);
void task_wakeups_rate_exceeded(int warning, __unused const void *param0,
                                __unused const void *param1);
void task_io_rate_exceeded(int warning, const void *param0,
                           __unused const void *param1);
void task_conclave_mem_limit_exceeded(int warning, __unused const void *param0,
                                      __unused const void *param1);
...
PROC_CROSSED_HIGH_WATERMARK__SEND_EXC_RESOURCE_AND_SUSPEND(
    int max_footprint_mb, send_exec_resource_options_t exception_options);
void __attribute__((noinline))
SENDING_NOTIFICATION__THIS_PROCESS_IS_CAUSING_TOO_MUCH_IO(int flavor);
#if CONFIG_PROC_RESOURCE_LIMITS
void __attribute__((noinline))
SENDING_NOTIFICATION__THIS_PROCESS_HAS_TOO_MANY_FILE_DESCRIPTORS(
    task_t task, int current_size, int soft_limit, int hard_limit);
mach_port_name_t current_task_get_fatal_port_name(void);
void __attribute__((noinline))
SENDING_NOTIFICATION__THIS_PROCESS_HAS_TOO_MANY_KQWORKLOOPS(task_t task,
                                                            int current_size,
                                                            int soft_limit,
                                                            int hard_limit);
#endif /* CONFIG_PROC_RESOURCE_LIMITS */
...
static kern_return_t task_start_halt_locked(task_t task,
...
extern kern_return_t exception_deliver(thread_t, exception_type_t,
                                       mach_exception_data_t,
                                       mach_msg_type_number_t,
                                       struct exception_action *, lck_mtx_t *);
...
#define RESETTABLE_DIAG_FOOTPRINT_LIMITS                                       \
  ((DEBUG || DEVELOPMENT) && CONFIG_MEMORYSTATUS)
...
/*
 * Level (in terms of percentage of the limit) at which the wakeups monitor
 * triggers telemetry.
 *
...
int task_wakeups_monitor_interval; /* In seconds. Time period over which wakeups
                                      rate is observed */
int task_wakeups_monitor_rate;     /* In hz. Maximum allowable wakeups per task
                                      before EXC_RESOURCE is sent */

unsigned int
    task_wakeups_monitor_ustackshots_trigger_pct; /* Percentage. Level at which
                                                     we start gathering
                                                     telemetry. */

TUNABLE(bool, disable_exc_resource, "disable_exc_resource",
        false); /* Global override to suppress EXC_RESOURCE for resource monitor
                   violations. */
TUNABLE(
    bool, disable_exc_resource_during_audio,
    "disable_exc_resource_during_audio",
    true); /* Global override to suppress EXC_RESOURCE while audio is active */

ledger_amount_t max_task_footprint =
    0; /* Per-task limit on physical memory consumption in bytes     */
unsigned int max_task_footprint_warning_level =
 ...
 */
TUNABLE_DT_WRITEABLE(int, max_task_footprint_mb, "/defaults",
                     "kern.max_task_pmem", "max_task_pmem", 0, TUNABLE_DT_NONE);
...
int64_t io_telemetry_limit; /* Threshold to take a microstackshot (0 indicated
                               I/O telemetry is turned off) */
int64_t global_logical_writes_count = 0; /* Global count for logical writes */
int64_t global_logical_writes_to_external_count =
...
task_check_memorythreshold_is_valid(task_t task, uint64_t new_limit,
                                    bool is_diagnostics_value);
#endif
...
extern void memorystatus_on_ledger_footprint_exceeded(int warning,
                                                      bool memlimit_is_active,
                                                      bool memlimit_is_fatal);
extern void memorystatus_log_exception(const int max_footprint_mb,
                                       bool memlimit_is_active,
                                       bool memlimit_is_fatal);
extern void
...
#define _TASK_EXC_GUARD_MP_CORPSE                                              \
  (TASK_EXC_GUARD_MP_DELIVER | TASK_EXC_GUARD_MP_CORPSE)
#define _TASK_EXC_GUARD_MP_ONCE                                                \
  (_TASK_EXC_GUARD_MP_CORPSE | TASK_EXC_GUARD_MP_ONCE)
#define _TASK_EXC_GUARD_MP_FATAL                                               \
  (TASK_EXC_GUARD_MP_DELIVER | TASK_EXC_GUARD_MP_FATAL)

#define _TASK_EXC_GUARD_VM_CORPSE                                              \
  (TASK_EXC_GUARD_VM_DELIVER | TASK_EXC_GUARD_VM_ONCE)
#define _TASK_EXC_GUARD_VM_ONCE                                                \
  (_TASK_EXC_GUARD_VM_CORPSE | TASK_EXC_GUARD_VM_ONCE)
#define _TASK_EXC_GUARD_VM_FATAL                                               \
  (TASK_EXC_GUARD_VM_DELIVER | TASK_EXC_GUARD_VM_FATAL)

#define _TASK_EXC_GUARD_ALL_CORPSE                                             \
  (_TASK_EXC_GUARD_MP_CORPSE | _TASK_EXC_GUARD_VM_CORPSE)
#define _TASK_EXC_GUARD_ALL_ONCE                                               \
  (_TASK_EXC_GUARD_MP_ONCE | _TASK_EXC_GUARD_VM_ONCE)
#define _TASK_EXC_GUARD_ALL_FATAL                                              \
  (_TASK_EXC_GUARD_MP_FATAL | _TASK_EXC_GUARD_VM_FATAL)

/* cannot turn off FATAL and DELIVER bit if set */
uint32_t task_exc_guard_no_unset_mask =
    TASK_EXC_GUARD_MP_FATAL | TASK_EXC_GUARD_VM_FATAL |
    TASK_EXC_GUARD_MP_DELIVER | TASK_EXC_GUARD_VM_DELIVER;
/* cannot turn on ONCE bit if unset */
uint32_t task_exc_guard_no_set_mask =
    TASK_EXC_GUARD_MP_ONCE | TASK_EXC_GUARD_VM_ONCE;

#if !defined(XNU_TARGET_OS_BRIDGE)

uint32_t task_exc_guard_default = _TASK_EXC_GUARD_ALL_FATAL;
uint32_t task_exc_guard_config_mask =
    TASK_EXC_GUARD_MP_ALL | TASK_EXC_GUARD_VM_ALL;
/*
 * These "by-process-name" default overrides are intended to be a short-term fix
 * to quickly get over races between changes introducing new EXC_GUARD raising
 * behaviors in some process and a change in default behavior for same. We
 * should ship with these lists empty (by fixing the bugs, or explicitly
 * changing the task's EXC_GUARD exception behavior via
...
static os_ref_count_t task_add_turnstile_watchports_locked(
    task_t task, struct task_watchports *watchports,
    struct task_watchport_elem **previous_elem_array,
    ipc_port_t *portwatch_ports, uint32_t portwatch_count);

static os_ref_count_t task_remove_turnstile_watchports_locked(
    task_t task, struct task_watchports *watchports, ipc_port_t *port_freelist);

static struct task_watchports *
task_watchports_alloc_init(task_t task, thread_t thread, uint32_t count);

static void task_watchports_deallocate(struct task_watchports *watchports);

void task_set_64bit(task_t task, boolean_t is_64bit, boolean_t is_64bit_data) {
...
bool task_get_64bit_addr(task_t task) { return task_has_64Bit_addr(task); }

bool task_get_64bit_data(task_t task) { return task_has_64Bit_data(task); }

void task_set_platform_binary(task_t task, boolean_t is_platform) {
  if (is_platform) {
...
void task_disable_mach_hardening(task_t task) {
  task_ro_flags_set(task, TFRO_MACH_HARDENING_OPT_OUT);
}

bool task_opted_out_mach_hardening(task_t task) {
  return task_ro_flags_get(task) & TFRO_MACH_HARDENING_OPT_OUT;
}
#endif /* XNU_TARGET_OS_OSX */

boolean_t task_get_platform_binary(task_t task) {
  return (task_ro_flags_get(task) & TFRO_PLATFORM) != 0;
}

boolean_t task_is_a_corpse(task_t task) {
  return (task_ro_flags_get(task) & TFRO_CORPSE) != 0;
}

boolean_t task_is_ipc_active(task_t task) { return task->ipc_active; }

void task_set_corpse(task_t task) {
  return task_ro_flags_set(task, TFRO_CORPSE);
}

void task_copyout_control_port(task_t task) {
 ...
 */
bool task_set_ca_client_wi(task_t task, boolean_t set_or_clear) {
  bool ret = true;
  task_lock(task);
  if (set_or_clear) {
 ...
 * 2) in mach_loader.c to set location of __all_image_info section in loaded
 * dyld 3) is from dyld itself to update location of all_image_info For security
 * any calls after that are ignored.  The TF_DYLD_ALL_IMAGE_SET bit is used to
 * determine state.
 */
kern_return_t task_set_dyld_info(task_t task, mach_vm_address_t addr,
                                 mach_vm_size_t size, bool finalize_value) {
  ...
  if ((task->t_flags & TF_DYLD_ALL_IMAGE_FINAL) == 0) {
    ...
    bool currentNonZero =
        ((task->all_image_info_addr != 0) || (task->all_image_info_size != 0));
...
bool task_donates_own_pages(task_t task) { return task->donates_own_pages; }

void task_set_mach_header_address(task_t task, mach_vm_address_t addr) {
  task_lock(task);
  task->mach_header_vm_address = addr;
  task_unlock(task);
}

void task_bank_reset(__unused task_t task) {
 ...
 */
void task_bank_init(__unused task_t task) {
  if (task->bank_context != NULL) {
    panic("Task bank init called with non null bank context for task: %p and "
          "bank_context: %p",
          task, task->bank_context);
  }
  bank_task_initialize(task);
}

void task_set_did_exec_flag(task_t task) { task->t_procflags |= TPF_DID_EXEC; }

void task_clear_exec_copy_flag(task_t task) {
  task->t_procflags &= ~TPF_EXEC_COPY;
}

event_t task_get_return_wait_event(task_t task) {
  return (event_t)&task->returnwait_inheritor;
}

void task_clear_return_wait(task_t task, uint32_t flags) {
  ...
  if (flags & TCRW_CLEAR_FINAL_WAIT) {
    ...
    if (task->t_returnwaitflags & TRW_LRETURNWAITER) {
      struct turnstile *turnstile = turnstile_prepare_hash(
          (uintptr_t)task_get_return_wait_event(task), TURNSTILE_ULOCK);
      ...
      turnstile_complete_hash((uintptr_t)task_get_return_wait_event(task),
                              TURNSTILE_ULOCK);
      turnstile_cleanup();
 ...
 */
static void task_set_ctrl_port_default(task_t task, thread_t thread) {
  ...
  if (movable_allowed || is_simulated || is_translated) {
    ...
    /* set control port options for 1p code, inherited from parent task by
     * default */
    ...
    /* set control port options for 3p code, inherited from parent task by
     * default */
  ...
  /*
   * now that we have marked the task as immovable, copyout the task/thread
   * ports again so that they get marked as immovable on copyout
   */
  ipc_task_copyout_control_port(task);
...
void __attribute__((noreturn)) task_wait_to_return(void) {
  ...
  if (task->t_returnwaitflags & TRW_LRETURNWAIT) {
    ...
    do {
      ...
      turnstile_update_inheritor(
          turnstile, task->returnwait_inheritor,
      ...
      turnstile_update_inheritor_complete(turnstile,
 ...
 */
void task_post_signature_processing_hook(task_t task) {
  ml_task_post_signature_processing_hook(task);
}

bool task_is_initproc(task_t task) {
...
boolean_t task_did_exec(task_t task) { return task_did_exec_internal(task); }

boolean_t task_is_active(task_t task) { return task->active; }

boolean_t task_is_halting(task_t task) { return task->halting; }
...
void task_init(void) {
  if (max_task_footprint_mb != 0) {
    ...
    max_task_footprint = (ledger_amount_t)max_task_footprint_mb * 1024 *
    ...
    if (max_mem < 0x40000000) {
      ...
      if (max_task_footprint_mb > 50) {
        max_task_footprint_warning_level =
            ((max_task_footprint_mb - 50) * 100) / max_task_footprint_mb;
      }
    } else {
      /*
       * On devices with >= 1GB of memory:
...
#else
    printf("Warning: max_task_pmem specified, but jetsam not configured; "
           "ignoring.\n");
#endif /* CONFIG_MEMORYSTATUS */
  }

#if DEVELOPMENT || DEBUG
  PE_parse_boot_argn("task_exc_guard_default", &task_exc_guard_default,
  ...
  if (!PE_parse_boot_argn("task_iomon_limit_mb", &task_iomon_limit_mb,
                          sizeof(task_iomon_limit_mb))) {
    task_iomon_limit_mb = IOMON_DEFAULT_LIMIT;
  }

  if (!PE_parse_boot_argn("task_iomon_interval_secs", &task_iomon_interval_secs,
                          sizeof(task_iomon_interval_secs))) {
    task_iomon_interval_secs = IOMON_DEFAULT_INTERVAL;
  }

  if (!PE_parse_boot_argn("io_telemetry_limit", &io_telemetry_limit,
 ...
 */
   ...
   */
  if (task_create_internal(TASK_NULL, NULL, NULL, FALSE, is_64bit, is_64bit,
                           TF_NONE, TF_NONE, TPF_NONE, TWF_NONE,
                           kernel_task) != KERN_SUCCESS) {
    panic("task_init");
  }
  ...
  // kernel_task never runs at EL0, but
  // machine_thread_state_convert_from/to_user() relies on disable_user_jop to
  // be false for kernel threads (e.g. in exception delivery on
...
static inline void task_zone_init(void) {
  ...
  proc_task_zone = zone_create_ext(
      "proc_task", proc_and_task_size, ZC_ZFREE_CLEARMEM | ZC_SEQUESTER,
      ZONE_ID_PROC_TASK, NULL); /* sequester is needed for proc_rele() */
}
 ...
 *   Such memory is no longer actually resident for the task [i.e., resident in
 * its pmap], and could be either decompressed back into memory, or paged out to
 * storage, depending on our implementation.
 *
 * iokit_mapped
 *   IOKit mappings: The total size of all IOKit mappings in this task,
 * regardless of clean/dirty or internal/external state].
 *
 * alternate_accounting
 *   The number of internal dirty pages which are part of IOKit mappings. By
 * definition, these pages are counted in both internal *and* iokit_mapped, so
 * we must subtract them from the total to avoid double counting.
 *
 * pages_grabbed
 *   pages_grabbed counts all page grabs in a task.  It is also broken out into
 * three subtypes which track UPL, IOPL and Kernel page grabs.
 */
void init_task_ledgers(void) {
  ledger_template_t t;
  ...
  PE_parse_boot_argn("pmap_ledgers_panic_leeway", &pmap_ledgers_panic_leeway,
  ...
  task_ledgers.tkm_private =
      ledger_entry_add(t, "tkm_private", "physmem", "bytes");
  task_ledgers.tkm_shared =
      ledger_entry_add(t, "tkm_shared", "physmem", "bytes");
  task_ledgers.phys_mem = ledger_entry_add(t, "phys_mem", "physmem", "bytes");
  task_ledgers.wired_mem = ledger_entry_add(t, "wired_mem", "physmem", "bytes");
  task_ledgers.conclave_mem = ledger_entry_add_with_flags(
      t, "conclave_mem", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE | LEDGER_ENTRY_ALLOW_DEBIT);
  task_ledgers.internal = ledger_entry_add(t, "internal", "physmem", "bytes");
  task_ledgers.iokit_mapped =
      ledger_entry_add_with_flags(t, "iokit_mapped", "mappings", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.alternate_accounting =
      ledger_entry_add_with_flags(t, "alternate_accounting", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.alternate_accounting_compressed = ledger_entry_add_with_flags(
      t, "alternate_accounting_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.page_table =
      ledger_entry_add_with_flags(t, "page_table", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.phys_footprint =
      ledger_entry_add(t, "phys_footprint", "physmem", "bytes");
  task_ledgers.internal_compressed =
      ledger_entry_add(t, "internal_compressed", "physmem", "bytes");
  ...
  task_ledgers.purgeable_volatile =
      ledger_entry_add_with_flags(t, "purgeable_volatile", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.purgeable_nonvolatile = ledger_entry_add_with_flags(
      t, "purgeable_nonvolatile", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.purgeable_volatile_compressed = ledger_entry_add_with_flags(
      t, "purgeable_volatile_compress", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.purgeable_nonvolatile_compressed = ledger_entry_add_with_flags(
      t, "purgeable_nonvolatile_compress", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.pages_grabbed = ledger_entry_add_with_flags(
      t, "pages_grabbed", "physmem", "count", LEDGER_ENTRY_USE_COUNTER);
  task_ledgers.pages_grabbed_kern = ledger_entry_add_with_flags(
      t, "pages_grabbed_kern", "physmem", "count", LEDGER_ENTRY_USE_COUNTER);
  task_ledgers.pages_grabbed_iopl = ledger_entry_add_with_flags(
      t, "pages_grabbed_iopl", "physmem", "count", LEDGER_ENTRY_USE_COUNTER);
  task_ledgers.pages_grabbed_upl = ledger_entry_add_with_flags(
      t, "pages_grabbed_upl", "physmem", "count", LEDGER_ENTRY_USE_COUNTER);
  task_ledgers.tagged_nofootprint =
      ledger_entry_add_with_flags(t, "tagged_nofootprint", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.tagged_footprint =
      ledger_entry_add_with_flags(t, "tagged_footprint", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.tagged_nofootprint_compressed = ledger_entry_add_with_flags(
      t, "tagged_nofootprint_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.tagged_footprint_compressed = ledger_entry_add_with_flags(
      t, "tagged_footprint_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.network_volatile =
      ledger_entry_add_with_flags(t, "network_volatile", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.network_nonvolatile =
      ledger_entry_add_with_flags(t, "network_nonvolatile", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.network_volatile_compressed = ledger_entry_add_with_flags(
      t, "network_volatile_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.network_nonvolatile_compressed = ledger_entry_add_with_flags(
      t, "network_nonvolatile_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.media_nofootprint =
      ledger_entry_add_with_flags(t, "media_nofootprint", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.media_footprint =
      ledger_entry_add_with_flags(t, "media_footprint", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.media_nofootprint_compressed = ledger_entry_add_with_flags(
      t, "media_nofootprint_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.media_footprint_compressed = ledger_entry_add_with_flags(
      t, "media_footprint_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.graphics_nofootprint =
      ledger_entry_add_with_flags(t, "graphics_nofootprint", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.graphics_footprint =
      ledger_entry_add_with_flags(t, "graphics_footprint", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.graphics_nofootprint_compressed = ledger_entry_add_with_flags(
      t, "graphics_nofootprint_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.graphics_footprint_compressed = ledger_entry_add_with_flags(
      t, "graphics_footprint_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.neural_nofootprint =
      ledger_entry_add_with_flags(t, "neural_nofootprint", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.neural_footprint =
      ledger_entry_add_with_flags(t, "neural_footprint", "physmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.neural_nofootprint_compressed = ledger_entry_add_with_flags(
      t, "neural_nofootprint_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.neural_footprint_compressed = ledger_entry_add_with_flags(
      t, "neural_footprint_compressed", "physmem", "bytes",
      LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
  task_ledgers.neural_nofootprint_total =
      ledger_entry_add(t, "neural_nofootprint_total", "physmem", "bytes");

#if CONFIG_DEFERRED_RECLAIM
  task_ledgers.est_reclaimable =
      ledger_entry_add_with_flags(t, "est_reclaimable", "virtmem", "bytes",
                                  LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);
#endif /* CONFIG_DEFERRED_RECLAIM */

#if CONFIG_FREEZE
  task_ledgers.frozen_to_swap =
      ledger_entry_add(t, "frozen_to_swap", "physmem", "bytes");
#endif /* CONFIG_FREEZE */

  task_ledgers.platform_idle_wakeups =
      ledger_entry_add(t, "platform_idle_wakeups", "power", "count");
  task_ledgers.interrupt_wakeups =
      ledger_entry_add(t, "interrupt_wakeups", "power", "count");

#if CONFIG_SCHED_SFI
  sfi_class_id_t class_id, ledger_alias;
  for (class_id = SFI_CLASS_UNSPECIFIED; class_id < MAX_SFI_CLASS_ID;
       class_id++) {
    task_ledgers.sfi_wait_times[class_id] = -1;
  }

  /* don't account for UNSPECIFIED */
  for (class_id = SFI_CLASS_UNSPECIFIED + 1; class_id < MAX_SFI_CLASS_ID;
       class_id++) {
    ...
    if (ledger_alias != SFI_CLASS_UNSPECIFIED) {
      ...
      if (task_ledgers.sfi_wait_times[ledger_alias] != -1) {
        task_ledgers.sfi_wait_times[class_id] =
            task_ledgers.sfi_wait_times[ledger_alias];
      } else {
        /* Otherwise, initialize it first */
        task_ledgers.sfi_wait_times[class_id] =
            task_ledgers.sfi_wait_times[ledger_alias] =
                sfi_ledger_entry_add(t, ledger_alias);
      }
    } else {
    ...
    if (task_ledgers.sfi_wait_times[class_id] < 0) {
      panic(
          "couldn't create entries for task ledger template for SFI class 0x%x",
          class_id);
    }
  }
  ...
  task_ledgers.cpu_time_billed_to_me =
      ledger_entry_add(t, "cpu_time_billed_to_me", "sched", "ns");
  task_ledgers.cpu_time_billed_to_others =
      ledger_entry_add(t, "cpu_time_billed_to_others", "sched", "ns");
  task_ledgers.physical_writes =
      ledger_entry_add(t, "physical_writes", "res", "bytes");
  task_ledgers.logical_writes =
      ledger_entry_add(t, "logical_writes", "res", "bytes");
  task_ledgers.logical_writes_to_external =
      ledger_entry_add(t, "logical_writes_to_external", "res", "bytes");
#if CONFIG_PHYS_WRITE_ACCT
  task_ledgers.fs_metadata_writes =
      ledger_entry_add(t, "fs_metadata_writes", "res", "bytes");
#endif /* CONFIG_PHYS_WRITE_ACCT */
  task_ledgers.energy_billed_to_me =
      ledger_entry_add(t, "energy_billed_to_me", "power", "nj");
  task_ledgers.energy_billed_to_others =
      ledger_entry_add(t, "energy_billed_to_others", "power", "nj");

#if CONFIG_MEMORYSTATUS
  task_ledgers.memorystatus_dirty_time =
      ledger_entry_add(t, "memorystatus_dirty_time", "physmem", "ns");
#endif /* CONFIG_MEMORYSTATUS */

  task_ledgers.swapins = ledger_entry_add_with_flags(
      t, "swapins", "physmem", "bytes", LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE);

  if ((task_ledgers.cpu_time < 0) || (task_ledgers.tkm_private < 0) ||
      (task_ledgers.tkm_shared < 0) || (task_ledgers.phys_mem < 0) ||
      (task_ledgers.wired_mem < 0) || (task_ledgers.conclave_mem < 0) ||
      (task_ledgers.internal < 0) || (task_ledgers.external < 0) ||
      (task_ledgers.reusable < 0) || (task_ledgers.iokit_mapped < 0) ||
      (task_ledgers.alternate_accounting < 0) ||
      (task_ledgers.alternate_accounting_compressed < 0) ||
      (task_ledgers.page_table < 0) || (task_ledgers.phys_footprint < 0) ||
      ...
      (task_ledgers.cpu_time_billed_to_me < 0) ||
      (task_ledgers.cpu_time_billed_to_others < 0) ||
      (task_ledgers.physical_writes < 0) || (task_ledgers.logical_writes < 0) ||
...
#if CONFIG_MEMORYSTATUS
  ledger_set_callback(t, task_ledgers.phys_footprint, task_footprint_exceeded,
                      NULL, NULL);
  ledger_set_callback(t, task_ledgers.conclave_mem,
                      task_conclave_mem_limit_exceeded, NULL, NULL);
#endif /* CONFIG_MEMORYSTATUS */

  ledger_set_callback(t, task_ledgers.interrupt_wakeups,
                      task_wakeups_rate_exceeded, NULL, NULL);
  ledger_set_callback(t, task_ledgers.physical_writes, task_io_rate_exceeded,
                      (void *)FLAVOR_IO_PHYSICAL_WRITES, NULL);
...
kern_return_t task_create_internal(task_t parent_task, /* Null-able */
                                   proc_ro_t proc_ro,
                                   coalition_t *parent_coalitions __unused,
                                   boolean_t inherit_memory, boolean_t is_64bit,
                                   boolean_t is_64bit_data, uint32_t t_flags,
                                   uint32_t t_flags_ro, uint32_t t_procflags,
  ...
  assert(task_ledger_template != NULL);
  ledger =
      ledger_instantiate(task_ledger_template, LEDGER_CREATE_ACTIVE_ENTRIES);
  if (ledger == NULL) {
...
#if defined(HAS_APPLE_PAC)
  const uint8_t disable_user_jop =
      inherit_memory ? parent_task->disable_user_jop : FALSE;
  ...
  if (!(t_flags & TF_CORPSE_FORK) && inherit_memory) {
    ...
    if (new_task->map != NULL && parent_task->deferred_reclamation_metadata) {
      new_task->deferred_reclamation_metadata =
          vm_deferred_reclamation_task_fork(
              new_task, parent_task->deferred_reclamation_metadata);
    }
    if (parent_task->deferred_reclamation_metadata) {
    ...
    new_map = vm_map_create_options(pmap, (vm_map_offset_t)(VM_MIN_ADDRESS),
  ...
  if (parent_task && inherit_memory) {
    ...
    task_ro_data.t_flags_ro |=
        (parent_t_flags_ro &
         (TFRO_PAC_ENFORCE_USER_STATE | TFRO_PAC_EXC_FATAL));
#endif /* __has_feature(ptrauth_calls) */
    /* Inherit the platform restrictions flags from parent if in fork */
    task_ro_data.t_flags_ro |=
        parent_t_flags_ro & (TFRO_PLATFORM | TFRO_JIT_EXC_FATAL);
#if XNU_TARGET_OS_OSX
  ...
  if (max_task_footprint != 0) {
    ledger_set_limit(ledger, task_ledgers.phys_footprint, max_task_footprint,
  ...
  if (parent_task != NULL) {
    ...
    if (!(t_flags & TF_CORPSE_FORK)) {
      task_ro_data.task_filters.mach_trap_filter_mask =
          task_get_mach_trap_filter_mask(parent_task);
      task_ro_data.task_filters.mach_kobj_filter_mask =
          task_get_mach_kobj_filter_mask(parent_task);
    }
#endif
  } else {
  ...
  if (parent_task != TASK_NULL) {
    ...
    char *shared_region_id =
        task_get_vm_shared_region_id_and_jop_pid(parent_task, NULL);
    ...
    new_task->pset_hint = parent_task->pset_hint =
  ...
  new_task->task_io_stats =
      kalloc_data(sizeof(struct io_stat_info), Z_WAITOK | Z_ZERO | Z_NOFAIL);

  bzero(&(new_task->cpu_time_eqos_stats),
        sizeof(new_task->cpu_time_eqos_stats));
  bzero(&(new_task->cpu_time_rqos_stats),
        sizeof(new_task->cpu_time_rqos_stats));
  ...
  if (!(t_flags & TF_CORPSE_FORK)) {
    ...
    if (parent_coalitions && parent_coalitions[COALITION_TYPE_RESOURCE]) {
      ...
      if (parent_coalitions[COALITION_TYPE_JETSAM]) {
        new_task->donates_own_pages =
            coalition_is_swappable(parent_coalitions[COALITION_TYPE_JETSAM]);
      }
      ...
      if (parent_task->coalition[COALITION_TYPE_JETSAM]) {
        new_task->donates_own_pages = coalition_is_swappable(
            parent_task->coalition[COALITION_TYPE_JETSAM]);
      }
    } else {
 ...
 */
void task_rollup_accounting_info(task_t to_task, task_t from_task) {
  ...
  counter_add(&to_task->messages_received,
              counter_load(&from_task->messages_received));
  ...
  *to_task->task_io_stats = *from_task->task_io_stats;
  ...
  to_task->task_writes_counters_internal.task_immediate_writes =
      from_task->task_writes_counters_internal.task_immediate_writes;
  to_task->task_writes_counters_internal.task_deferred_writes =
      from_task->task_writes_counters_internal.task_deferred_writes;
  to_task->task_writes_counters_internal.task_invalidated_writes =
  ...
  to_task->task_writes_counters_external.task_deferred_writes =
      from_task->task_writes_counters_external.task_deferred_writes;
  to_task->task_writes_counters_external.task_invalidated_writes =
      from_task->task_writes_counters_external.task_invalidated_writes;
  to_task->task_writes_counters_external.task_metadata_writes =
...
#if CONFIG_MEMORYSTATUS
  ledger_rollup_entry(to_task->ledger, from_task->ledger,
                      task_ledgers.memorystatus_dirty_time);
#endif /* CONFIG_MEMORYSTATUS */

  /* Skip ledger roll up for memory accounting entries */
  ledger_rollup_entry(to_task->ledger, from_task->ledger,
                      task_ledgers.cpu_time);
  ledger_rollup_entry(to_task->ledger, from_task->ledger,
                      task_ledgers.platform_idle_wakeups);
  ledger_rollup_entry(to_task->ledger, from_task->ledger,
                      task_ledgers.interrupt_wakeups);
#if CONFIG_SCHED_SFI
  for (sfi_class_id_t class_id = SFI_CLASS_UNSPECIFIED;
       class_id < MAX_SFI_CLASS_ID; class_id++) {
    ledger_rollup_entry(to_task->ledger, from_task->ledger,
                        task_ledgers.sfi_wait_times[class_id]);
  }
#endif
...
void task_deallocate_internal(task_t task, os_ref_count_t refs) {
...
#if MACH_ASSERT
  if (task->ledger != NULL && task->map != NULL && task->map->pmap != NULL &&
  ...
  if (task->task_owned_objects != 0) {
    panic("task_deallocate(%p): "
          ...
          task, task->task_volatile_objects, task->task_nonvolatile_objects,
  ...
  dead_task_statistics.task_timer_wakeups_bin_1 +=
      task->task_timer_wakeups_bin_1;
  dead_task_statistics.task_timer_wakeups_bin_2 +=
 ...
 */
void task_name_deallocate_mig(task_name_t task_name) {
 ...
 */
void task_policy_set_deallocate_mig(task_policy_set_t task_policy_set) {
 ...
 *	Drop a reference on a task type.
 */
void task_policy_get_deallocate_mig(task_policy_get_t task_policy_get) {
 ...
 */
void task_inspect_deallocate_mig(task_inspect_t task_inspect) {
...
void task_suspension_token_deallocate_grp(task_suspension_token_t token,
                                          task_grp_t grp) {
 ...
 */
kern_return_t task_collect_crash_info(task_t task,
#ifdef CONFIG_MACF
  ...
  if (task->corpse_info == NULL &&
      (is_corpse_fork || get_bsdtask_info(task) != NULL)) {
#if CONFIG_MACF
    ...
    crash_data_kernel =
        kalloc_data(CORPSEINFO_ALLOCATION_SIZE, Z_WAITOK | Z_ZERO);
    ...
    crash_data = task_crashinfo_alloc_init(
        (mach_vm_address_t)crash_data_ptr, size,
        is_corpse_fork ? 0 : CORPSE_CRASHINFO_HAS_REF, KCFLAG_USE_MEMCOPY);
    if (crash_data) {
      task_lock(task);
      ...
      kfree_data(crash_data_kernel, CORPSEINFO_ALLOCATION_SIZE);
      kr = KERN_FAILURE;
    }
 ...
 */
kern_return_t
task_deliver_crash_notification(task_t corpse, /* corpse or corpse fork */
  ...
  /*
   * Always populate code[0] as the effective exception type for
   * EXC_CORPSE_NOTIFY. Crash reporters should derive whether it's fatal from
   * corpse blob.
   */
  code[0] = etype;
  code[1] = subcode;
  ...
  kr = exception_triage_thread(EXC_CORPSE_NOTIFY, code, EXCEPTION_CODE_MAX,
                               thread);
  if (kr != KERN_SUCCESS) {
    printf("Failed to send exception EXC_CORPSE_NOTIFY. error code: %d for pid "
           "%d\n",
           kr, task_pid(corpse));
  }
...
static void __unused task_partial_reap(task_t task, __unused int pid) {
  ...
  KDBG(VMDBG_CODE(DBG_VM_MAP_PARTIAL_REAP) | DBG_FUNC_START, pid,
       task_page_count);

  vm_map_partial_reap(task->map, &reclaimed_resident, &reclaimed_compressed);

  KDBG(VMDBG_CODE(DBG_VM_MAP_PARTIAL_REAP) | DBG_FUNC_END, pid,
       reclaimed_resident, reclaimed_compressed);
}
 ...
 */
kern_return_t task_mark_corpse(task_t task) {
  kern_return_t kr = KERN_SUCCESS;
...
#if CONFIG_MACF
  crash_label =
      mac_exc_create_label_for_proc((struct proc *)get_bsdtask_info(task));
#endif
  ...
  if (!task->active) {
    ...
    ;
    void *crash_data_kernel_release =
        kcdata_memory_get_begin_addr(crash_data_release);
    ;
 ...
 */
void task_set_uniqueid(task_t task) {
 ...
 */
void task_clear_corpse(task_t task) {
  thread_t th_iter = NULL;
 ...
 *	the task (corpse).
 */
static void task_port_no_senders(ipc_port_t port,
  ...
  if (task) {
    ...
    ip_mq_lock(port);
    assert(task_is_a_corpse(task));
    is_corpse = (ipc_kobject_get_locked(port, IKOT_TASK_CONTROL) != TASK_NULL);
    ip_mq_unlock(port);
    itk_unlock(task);
 ...
 */
static void task_port_with_flavor_no_senders(ipc_port_t port,
...
task_duplicate_map_and_threads(task_t task, void *p, task_t new_task,
                               thread_t *thread_ret, uint64_t **udata_buffer,
  ...
  if (memorystatus_allowed_vm_map_fork(task, &is_large)) {
    ...
    oldmap = new_task->map;
    new_task->map = vm_map_fork(
        new_task->ledger, task->map,
  ...
  /* Iterate all the threads and drop the task lock before calling
  ...
  for (i = 0; i < array_count; i++) {
    kr = thread_create_with_continuation(
    ...
    new_thread->thread_tag =
        thread_array[i]->thread_tag & ~THREAD_TAG_USER_JOIN;
...
#if CONFIG_SECLUDED_MEMORY
extern void
task_set_can_use_secluded_mem_locked(task_t task,
...
#endif /* MACH_ASSERT */
kern_return_t task_terminate_internal(task_t task) {
  ...
  proc_set_task_policy(task, TASK_POLICY_ATTRIBUTE, TASK_POLICY_TERMINATED,
                       TASK_POLICY_ENABLE);
  ...
  ledger_disable_panic_on_negative(
      task->map->pmap->ledger, task_ledgers.alternate_accounting_compressed);
#endif
  ...
  if (vm_map_page_shift(task->map) < (int)PAGE_SHIFT) {
    ...
    if (debug4k_panic_on_terminate) {
      panic("DEBUG4K: %s:%d %d[%s] map %p", __FUNCTION__, __LINE__, pid,
            procname, task->map);
    }
  }
#endif /* MACH_ASSERT */
...
void tasks_system_suspend(boolean_t suspend) {
  task_t task;
 ...
 *	termination.
 */
kern_return_t task_start_halt(task_t task) {
  kern_return_t kr = KERN_SUCCESS;
  task_lock(task);
...
static kern_return_t task_start_halt_locked(task_t task,
  ...
  queue_iterate(&task->threads, thread, thread_t, task_threads) {
     ...
     * mutex lock.
     *
     * We need task_is_a_corpse() check so that we don't accidently update
     * policy for tasks that are doing posix_spawn().
     *
 ...
 */
void task_complete_halt(task_t task) {
  task_lock(task);
  assert(task->halting);
  assert(task == current_task());
...
static void _task_mark_suspend_source(task_t task) {
  int idx;
...
static inline void _task_mark_suspend_start(task_t task) {
  task_lock_assert_owned(task);
  task->t_suspend_stats.tss_last_start = mach_absolute_time();
}

static inline void _task_mark_suspend_end(task_t task) {
  task_lock_assert_owned(task);
...
kern_return_t task_get_suspend_stats(task_t task, task_suspend_stats_t stats) {
#ifdef CONFIG_TASK_SUSPEND_STATS
  kern_return_t kr;
  if (task == TASK_NULL || stats == NULL) {
    return KERN_INVALID_ARGUMENT;
...
kern_return_t task_get_suspend_stats_kdp(task_t task,
                                         task_suspend_stats_t stats) {
#ifdef CONFIG_TASK_SUSPEND_STATS
  if (task == TASK_NULL || stats == NULL) {
    return KERN_INVALID_ARGUMENT;
...
kern_return_t task_get_suspend_sources(task_t task,
 ...
 */
bool task_hold_locked(task_t task) {
  thread_t thread;
  void *bsd_info = get_bsdtask_info(task);
 ...
 */
kern_return_t task_hold_and_wait(task_t task, bool suspend_conclave __unused) {
  if (task == TASK_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
...
boolean_t task_is_app_suspended(task_t task) { return task->pidsuspended; }
 ...
 *      CONDITIONS: the task is locked and active
 */
void task_release_locked(task_t task) {
  thread_t thread;
  void *bsd_info = get_bsdtask_info(task);
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_SUSPENSION, MACH_TASK_RESUME) |
                   DBG_FUNC_NONE,
               task_pid(task));
 ...
 */
kern_return_t task_release(task_t task) {
  if (task == TASK_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
...
static kern_return_t task_threads_internal(task_t task,
...
kern_return_t task_threads_from_user(mach_port_t port,
                                     thread_act_array_t *threads_out,
...
static kern_return_t place_task_hold(task_t task, int mode) {
...
static kern_return_t release_task_hold(task_t task, int mode) {
...
#if MACH_ASSERT
    /*
     * This is obviously not robust; if we suspend one task and then resume a
     * different one, we'll fly under the radar. This is only meant to catch the
     * common case of a crashed or buggy suspender.
     */
    current_task()->suspends_outstanding--;
#endif
...
boolean_t get_task_suspended(task_t task) { return 0 != task->user_stop_count; }
...
kern_return_t task_suspend(task_t task) {
  ...
  /*
   * Copyout the send right into the calling task's IPC space.  It won't know it
   * is there, but we'll look it up when calling a traditional resume.  Any IPC
   * operations that deallocate the send right will auto-release the suspension.
   */
  if (IP_VALID(port)) {
    kr = ipc_object_copyout(current_space(), port, MACH_MSG_TYPE_MOVE_SEND,
                            IPC_OBJECT_COPYOUT_FLAGS_NONE, NULL, &name);
  } else {
    kr = KERN_SUCCESS;
  }
  if (kr != KERN_SUCCESS) {
    printf("warning: %s(%d) failed to copyout suspension "
           ...
           proc_pid(get_bsdtask_info(current_task())), task_pid(task), kr);
  }
...
kern_return_t task_resume(task_t task) {
  ...
  if (is_active(space) && IP_VALID(task->itk_resume) &&
      ipc_hash_lookup(space, ip_to_object(task->itk_resume), &resume_port_name,
                      &resume_port_entry) == TRUE) {
    /*
     * We found a suspension token in the caller's IPC space. Release a send
     * right to indicate that we are holding one less legacy hold on the task
     * from this caller.  If the release failed, go ahead and drop all the
     * rights, as someone either already released our holds or the task is gone.
     */
    itk_unlock(task);
    if (kr == KERN_SUCCESS) {
    ...
    if (kr == KERN_SUCCESS) {
      printf("warning: %s(%d) performed out-of-band resume on pid %d\n",
             proc_name_address(get_bsdtask_info(current_task())),
             proc_pid(get_bsdtask_info(current_task())), task_pid(task));
    }
  }
 ...
 */
kern_return_t task_suspend_internal(task_t task) {
  kern_return_t kr;
 ...
 */
static kern_return_t task_suspend2_grp(task_t task,
...
kern_return_t task_suspend2_mig(task_t task,
                                task_suspension_token_t *suspend_token) {
  return task_suspend2_grp(task, suspend_token, TASK_GRP_MIG);
}

kern_return_t task_suspend2_external(task_t task,
                                     task_suspension_token_t *suspend_token) {
 ...
 */
kern_return_t task_resume_internal(task_suspension_token_t task) {
  kern_return_t kr;
 ...
 */
static kern_return_t task_resume2_grp(task_suspension_token_t task,
                                      task_grp_t grp) {
  kern_return_t kr;
...
kern_return_t task_resume2_mig(task_suspension_token_t task) {
  return task_resume2_grp(task, TASK_GRP_MIG);
}

kern_return_t task_resume2_external(task_suspension_token_t task) {
  return task_resume2_grp(task, TASK_GRP_EXTERNAL);
}

static void task_suspension_no_senders(ipc_port_t port,
                                       mach_port_mscount_t mscount) {
 ...
 */
void task_suspension_send_once(ipc_port_t port) {
  task_t task = convert_port_to_task_suspension_token(port);
...
static kern_return_t task_pidsuspend_locked(task_t task) {
  kern_return_t kr;
 ...
 *		Nothing locked.
 */
void task_add_turnstile_watchports(task_t task, thread_t thread,
  ...
  refs = task_add_turnstile_watchports_locked(
      task, watchports, previous_elem_array, portwatch_ports, portwatch_count);
  ...
  for (uint32_t i = 0; i < portwatch_count && previous_elem_array[i] != NULL;
       i++) {
 ...
 *		Nothing locked.
 */
void task_remove_turnstile_watchports(task_t task) {
  ...
  refs =
      task_remove_turnstile_watchports_locked(task, watchports, port_freelist);
 ...
 *		Transfer all watchport turnstile boost from old task to new
 * task.
 *
 *	Arguments:
 ...
 *		Nothing locked.
 */
void task_transfer_turnstile_watchports(task_t old_task, task_t new_task,
                                        thread_t new_thread) {
  ...
  new_watchports =
      task_watchports_alloc_init(new_task, new_thread, portwatch_count);
  ...
  for (uint32_t i = 0; i < portwatch_count; i++) {
    ...
    if (ipc_port_replace_watchport_elem_conditional_locked(
            port, &old_watchports->tw_elem[i], &new_watchports->tw_elem[i]) ==
        KERN_SUCCESS) {
  ...
  /* Drop the reference on new task_watchports struct returned by
 ...
 *		previous_elem_array: an array of old watchport_elem to be
 * returned to caller portwatch_ports: array of watchports portwatch_count:
 * number of watchports
 *
 *	Conditions:
...
task_remove_turnstile_watchports_locked(task_t task,
  ...
  for (uint32_t i = 0, j = 0; i < watchports->tw_elem_array_count; i++) {
    ...
    /* Lock the port and check if it has the entry */
    ip_mq_lock(port);
    if (ipc_port_clear_watchport_elem_internal_conditional_locked(
            port, &watchports->tw_elem[i]) == KERN_SUCCESS) {
      task_watchport_elem_clear(&watchports->tw_elem[i]);
...
task_watchports_alloc_init(task_t task, thread_t thread, uint32_t count) {
  struct task_watchports *watchports =
      kalloc_type(struct task_watchports, struct task_watchport_elem, count,
 ...
 *		Deallocate task watchport element and release its ref on
 * task_watchport.
 *
 *	Conditions:
 *		Nothing locked.
 */
void task_watchport_elem_deallocate(
 ...
 *		Nothing locked.
 */
boolean_t task_has_watchports(task_t task) { return task->watchports != NULL; }

#if DEVELOPMENT || DEBUG

extern void IOSleep(int);

kern_return_t task_disconnect_page_mappings(task_t task) {
  int n;
  ...
  for (n = 0; n < 100; n++) {
    ...
    KDBG_RELEASE(
        (MACHDBG_CODE(DBG_MACH_WORKINGSET, VM_DISCONNECT_TASK_PAGE_MAPPINGS)) |
            DBG_FUNC_START,
...
kern_return_t task_freeze(task_t task, uint32_t *purgeable_count,
                          uint32_t *wired_count, uint32_t *clean_count,
                          uint32_t *dirty_count, uint32_t dirty_budget,
                          uint32_t *shared_count, int *freezer_error_code,
                          boolean_t eval_only) {
  kern_return_t kr = KERN_SUCCESS;
  ...
  if (vm_deferred_reclamation_task_has_ring(task)) {
    ...
    if (kr != KERN_SUCCESS) {
      os_log_error(OS_LOG_DEFAULT,
                   "Failed to drain reclamation ring prior to freezing (%d)\n",
                   kr);
    }
  }
#endif /* CONFIG_DEFERRED_RECLAIM */

  kr = vm_map_freeze(task, purgeable_count, wired_count, clean_count,
                     dirty_count, dirty_budget, shared_count,
                     freezer_error_code, eval_only);
  ...
  if ((kr == KERN_SUCCESS) && (eval_only == FALSE)) {
    ...
    if (VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
      ...
      *dirty_count =
          (uint32_t)(freezer_context_global.freezer_ctx_swapped_bytes /
                     PAGE_SIZE_64); /*used to track pageouts*/
    }
  ...
  if (VM_CONFIG_COMPRESSOR_IS_PRESENT && (kr == KERN_SUCCESS) &&
      (eval_only == FALSE)) {
 ...
 *      The caller holds a reference to the task
 */
kern_return_t task_thaw(task_t task) {
  if (task == TASK_NULL || task == kernel_task) {
    return KERN_INVALID_ARGUMENT;
  }
...
void task_update_frozen_to_swap_acct(task_t task, int64_t amount,
...
kern_return_t task_set_security_tokens(task_t task, security_token_t sec_token,
...
kern_return_t task_send_trace_memory(__unused task_t target_task,
 ...
 */
kern_return_t
task_set_info(task_t task, task_flavor_t flavor,
...
static void _task_fill_times(task_t task, time_value_t *user_time,
                             time_value_t *sys_time) {
  clock_sec_t sec;
  clock_usec_t usec;
...
kern_return_t task_info(task_t task, task_flavor_t flavor,
...
#endif
  {
    ...
    basic_info->virtual_size =
        (typeof(basic_info->virtual_size))vm_map_adjusted_size(
            is_kernel_task ? kernel_map : task->map);
    ...
    basic_info->resident_size =
        (natural_t)MIN((ledger_amount_t)UINT32_MAX, tmp);

    _task_fill_times(task, &basic_info->user_time, &basic_info->system_time);
  ...
  case MACH_TASK_BASIC_INFO: {
    ...
    ledger_get_lifetime_max(task->ledger, task_ledgers.phys_mem,
                            (ledger_amount_t *)&basic_info->resident_size_max);
    basic_info->policy = is_kernel_task ? POLICY_RR : POLICY_TIMESHARE;
    basic_info->suspend_count = task->user_stop_count;
    _task_fill_times(task, &basic_info->user_time, &basic_info->system_time);
...
#define TASK_LEGACY_DYLD_INFO_COUNT                                            \
  ...
  case TASK_KERNELMEMORY_INFO: {
    ...
    if (task == kernel_task) {
      ...
      /* XXX this really shouldn't be using the function parameter 'task' as a
       * local var! */
      queue_iterate(&tasks, task, task_t, tasks) {
        if (task == kernel_task) {
  ...
  case TASK_EVENTS_INFO: {
    ...
    events_info->pageins =
        (int32_t)MIN(counter_load(&task->pageins), INT32_MAX);
    events_info->cow_faults =
        (int32_t)MIN(counter_load(&task->cow_faults), INT32_MAX);
    events_info->messages_sent =
        (int32_t)MIN(counter_load(&task->messages_sent), INT32_MAX);
    events_info->messages_received =
        (int32_t)MIN(counter_load(&task->messages_received), INT32_MAX);
  ...
  case TASK_VM_INFO_PURGEABLE: {
    ...
    vmlp_api_start(
        TASK_INFO); /* this is the only case that is relevant to the lock */
    ...
    if (original_task_info_count > TASK_VM_INFO_COUNT) {
      ...
      if (!proc_is_simulated(p)) {
        os_log(OS_LOG_DEFAULT,
               "%s[%d] task_info: possibly invalid "
               "task_info_count %d > TASK_VM_INFO_COUNT=%d on platform %d sdk "
               "%d.%d.%d - please use TASK_VM_INFO_COUNT",
               proc_name_address(p), proc_pid(p), original_task_info_count,
               TASK_VM_INFO_COUNT, platform, (sdk >> 16), ((sdk >> 8) & 0xff),
               (sdk & 0xff));
      }
      DTRACE_VM4(suspicious_task_vm_info_count, mach_msg_type_number_t,
                 original_task_info_count, mach_msg_type_number_t,
                 TASK_VM_INFO_COUNT, uint32_t, platform, uint32_t, sdk);
    }
#if __arm64__
    if (original_task_info_count > TASK_VM_INFO_REV2_COUNT &&
        platform == PLATFORM_IOS && sdk != 0 && (sdk >> 16) <= 12) {
      /*
       * Some iOS apps pass an incorrect value for
       * task_info_count, expressed in number of bytes
      ...
      printf("%s:%d %d[%s] rdar://49484582 task_info_count %d -> %d "
             ...
             original_task_info_count, TASK_VM_INFO_REV2_COUNT, platform,
             (sdk >> 16), ((sdk >> 8) & 0xff), (sdk & 0xff));
#endif /* DEVELOPMENT || DEBUG */
      DTRACE_VM4(workaround_task_vm_info_count, mach_msg_type_number_t,
                 original_task_info_count, mach_msg_type_number_t,
                 TASK_VM_INFO_REV2_COUNT, uint32_t, platform, uint32_t, sdk);
    ...
    if (original_task_info_count > TASK_VM_INFO_REV5_COUNT &&
        platform == PLATFORM_IOS && sdk != 0 && (sdk >> 16) <= 15) {
      /*
       * Some iOS apps pass an incorrect value for
       * task_info_count, expressed in number of bytes
       * instead of number of "natural_t" elements.
      ...
      printf("%s:%d %d[%s] task_info_count=%d > TASK_VM_INFO_COUNT=%d "
             "platform %d sdk %d.%d.%d\n",
             __FUNCTION__, __LINE__, proc_pid(p), proc_name_address(p),
             original_task_info_count, TASK_VM_INFO_REV5_COUNT, platform,
             (sdk >> 16), ((sdk >> 8) & 0xff), (sdk & 0xff));
      DTRACE_VM4(workaround_task_vm_info_count, mach_msg_type_number_t,
                 original_task_info_count, mach_msg_type_number_t,
                 TASK_VM_INFO_REV5_COUNT, uint32_t, platform, uint32_t, sdk);
#if DEVELOPMENT || DEBUG
      /*
    ...
    if (original_task_info_count > TASK_VM_INFO_REV7_COUNT &&
        platform == PLATFORM_IOS && sdk != 0 && (sdk >> 16) == 17) {
      /*
      ...
      printf("%s:%d %d[%s] task_info_count=%d > TASK_VM_INFO_COUNT=%d "
             "platform %d sdk %d.%d.%d\n",
             __FUNCTION__, __LINE__, proc_pid(p), proc_name_address(p),
             original_task_info_count, TASK_VM_INFO_REV7_COUNT, platform,
             (sdk >> 16), ((sdk >> 8) & 0xff), (sdk & 0xff));
      DTRACE_VM4(workaround_task_vm_info_count, mach_msg_type_number_t,
                 original_task_info_count, mach_msg_type_number_t,
                 TASK_VM_INFO_REV6_COUNT, uint32_t, platform, uint32_t, sdk);
#if DEVELOPMENT || DEBUG
      /*
       * For the sake of internal builds livability,
       * work around this user-space bug by capping the
    ...
    if (is_kernel_task) {
       ...
       */
      vm_info->internal -= (VM_PAGE_COMPRESSOR_COUNT * PAGE_SIZE);
    } else {
      ...
      if (flavor == TASK_VM_INFO_PURGEABLE) {
        kr = vm_map_query_volatile(
            map, &volatile_virtual_size, &volatile_resident_size,
            &volatile_compressed_size, &volatile_pmap_size,
            &volatile_compressed_pmap_size);
        if (kr == KERN_SUCCESS) {
          vm_info->purgeable_volatile_pmap = volatile_pmap_size;
          if (radar_20146450) {
            vm_info->compressed -= volatile_compressed_pmap_size;
          }
          vm_info->purgeable_volatile_resident = volatile_resident_size;
          vm_info->purgeable_volatile_virtual = volatile_virtual_size;
        }
      }
    }
    *task_info_count = TASK_VM_INFO_REV0_COUNT;
    ...
    if (original_task_info_count >= TASK_VM_INFO_REV1_COUNT) {
      vm_info->phys_footprint = (mach_vm_size_t)get_task_phys_footprint(task);
    ...
    if (original_task_info_count >= TASK_VM_INFO_REV3_COUNT) {
      ledger_get_lifetime_max(task->ledger, task_ledgers.phys_footprint,
                              &vm_info->ledger_phys_footprint_peak);
      ledger_get_balance(task->ledger, task_ledgers.purgeable_nonvolatile,
      ...
      ledger_get_balance(task->ledger, task_ledgers.purgeable_volatile,
                         &vm_info->ledger_purgeable_volatile);
      ledger_get_balance(task->ledger,
                         task_ledgers.purgeable_volatile_compressed,
                         &vm_info->ledger_purgeable_volatile_compressed);
      ledger_get_balance(task->ledger, task_ledgers.network_nonvolatile,
      ...
      ledger_get_balance(task->ledger, task_ledgers.network_volatile,
                         &vm_info->ledger_tag_network_volatile);
      ledger_get_balance(task->ledger, task_ledgers.network_volatile_compressed,
                         &vm_info->ledger_tag_network_volatile_compressed);
      ledger_get_balance(task->ledger, task_ledgers.media_footprint,
                         &vm_info->ledger_tag_media_footprint);
      ledger_get_balance(task->ledger, task_ledgers.media_footprint_compressed,
                         &vm_info->ledger_tag_media_footprint_compressed);
      ledger_get_balance(task->ledger, task_ledgers.media_nofootprint,
      ...
      ledger_get_balance(task->ledger, task_ledgers.graphics_footprint,
      ...
      ledger_get_balance(task->ledger, task_ledgers.graphics_nofootprint,
      ...
      ledger_get_balance(task->ledger, task_ledgers.neural_footprint,
                         &vm_info->ledger_tag_neural_footprint);
      ledger_get_balance(task->ledger, task_ledgers.neural_footprint_compressed,
                         &vm_info->ledger_tag_neural_footprint_compressed);
      ledger_get_balance(task->ledger, task_ledgers.neural_nofootprint,
  ...
  case TASK_DEBUG_INFO_INTERNAL: {
    ...
    if (space) {
      ...
      if (table) {
        dbg_info->ipc_space_size = ipc_entry_table_count(table);
      }
      smr_ipc_leave();
    }
  ...
  case TASK_SUSPEND_STATS_INFO: {
    ...
    if (*task_info_count < TASK_SUSPEND_STATS_INFO_COUNT ||
        task_info_out == NULL) {
      error = KERN_INVALID_ARGUMENT;
      break;
    }
    error = _task_get_suspend_stats_locked(task,
                                           (task_suspend_stats_t)task_info_out);
...
#if CONFIG_TASK_SUSPEND_STATS && (DEVELOPMENT || DEBUG)
    if (*task_info_count < TASK_SUSPEND_SOURCES_INFO_COUNT ||
        task_info_out == NULL) {
      error = KERN_INVALID_ARGUMENT;
      break;
    }
    error = _task_get_suspend_sources_locked(
        task, (task_suspend_source_t)task_info_out);
 ...
 *
 */
kern_return_t task_info_from_user(mach_port_t task_port, task_flavor_t flavor,
                                  task_info_t task_info_out,
                                  mach_msg_type_number_t *task_info_count) {
  task_t task;
  kern_return_t ret;
 ...
 */
void task_dyld_process_info_update_helper(
    task_t task, size_t active_count,
  ...
  if (active_count == 0) {
    ...
    (void)copyoutmap_atomic32(task->map, MACH_PORT_NULL,
                              magic_addr); /* unset magic */
  } else {
    itk_unlock(task);
    (void)copyoutmap_atomic32(task->map,
                              (mach_port_name_t)DYLD_PROCESS_INFO_NOTIFY_MAGIC,
 ...
 */
kern_return_t task_dyld_process_info_notify_register(task_t task,
                                                     ipc_port_t sright) {
  ...
  if (dyld_info.all_image_info_format == TASK_DYLD_ALL_IMAGE_INFO_32) {
    ports_addr =
        (mach_vm_address_t)(dyld_info.all_image_info_addr +
                            offsetof(struct user32_dyld_all_image_infos,
                                     notifyMachPorts));
  } else {
    ports_addr =
        (mach_vm_address_t)(dyld_info.all_image_info_addr +
                            offsetof(struct user64_dyld_all_image_infos,
                                     notifyMachPorts));
  }

retry:
  if (task->itk_dyld_notify == NULL) {
    notifiers_ptr = kalloc_type(ipc_port_t, DYLD_MAX_PROCESS_INFO_NOTIFY_COUNT,
                                Z_WAITOK | Z_ZERO | Z_NOFAIL);
  }
...
out:
  ...
  task_dyld_process_info_update_helper(task, active_count,
                                       (vm_map_address_t)ports_addr,
                                       release_ports, release_count);
 ...
 *
 * Remove a send right in target task's itk_dyld_notify array matching the
 * receive right name passed in. Deallocate kernel memory for the array if it's
 * the last port to be deregistered, or all ports have died. Also cleanup any
 * dead rights found in the array.
 *
 ...
 *     KERN_INVALID_RIGHT: The name passed in does not represent a valid rcv
 * right.
 *
 *     Other error code see task_info().
 *
 ...
 */
kern_return_t
task_dyld_process_info_notify_deregister(task_t task,
  ...
  kr = ipc_port_translate_receive(current_space(), rcv_name,
                                  &sright); /* does not produce port ref */
  if (kr) {
    return KERN_INVALID_RIGHT;
  }
 ...
 */
void task_power_info_locked(task_t task, task_power_info_t info,
  ...
  ledger_get_entries(task->ledger, task_ledgers.platform_idle_wakeups,
                     (ledger_amount_t *)&info->task_platform_idle_wakeups,
                     &tmp);
  ...
  if (extra_info) {
    ...
    if (PE_i_can_has_debugger(NULL)) {
      ...
      extra_info->secure_ptime =
          usage_perf.ru_metrics[RCT_LVL_SECURE].rm_time_mach;
    }
 ...
 *  (both dead and alive)
 */
uint64_t task_gpu_utilisation(task_t task) {
  uint64_t gpu_time = 0;
#if defined(__x86_64__)
  thread_t thread;
 ...
 */
void task_update_cpu_time_qos_stats(task_t task, uint64_t *eqos_stats,
                                    uint64_t *rqos_stats) {
  ...
  if (eqos_stats) {
    eqos_stats[THREAD_QOS_DEFAULT] +=
        task->cpu_time_eqos_stats.cpu_time_qos_default;
    eqos_stats[THREAD_QOS_MAINTENANCE] +=
        task->cpu_time_eqos_stats.cpu_time_qos_maintenance;
    eqos_stats[THREAD_QOS_BACKGROUND] +=
        task->cpu_time_eqos_stats.cpu_time_qos_background;
    eqos_stats[THREAD_QOS_UTILITY] +=
        task->cpu_time_eqos_stats.cpu_time_qos_utility;
    eqos_stats[THREAD_QOS_LEGACY] +=
        task->cpu_time_eqos_stats.cpu_time_qos_legacy;
    eqos_stats[THREAD_QOS_USER_INITIATED] +=
        task->cpu_time_eqos_stats.cpu_time_qos_user_initiated;
    eqos_stats[THREAD_QOS_USER_INTERACTIVE] +=
        task->cpu_time_eqos_stats.cpu_time_qos_user_interactive;
  }

  if (rqos_stats) {
    rqos_stats[THREAD_QOS_DEFAULT] +=
        task->cpu_time_rqos_stats.cpu_time_qos_default;
    rqos_stats[THREAD_QOS_MAINTENANCE] +=
        task->cpu_time_rqos_stats.cpu_time_qos_maintenance;
    rqos_stats[THREAD_QOS_BACKGROUND] +=
        task->cpu_time_rqos_stats.cpu_time_qos_background;
    rqos_stats[THREAD_QOS_UTILITY] +=
        task->cpu_time_rqos_stats.cpu_time_qos_utility;
    rqos_stats[THREAD_QOS_LEGACY] +=
        task->cpu_time_rqos_stats.cpu_time_qos_legacy;
    rqos_stats[THREAD_QOS_USER_INITIATED] +=
        task->cpu_time_rqos_stats.cpu_time_qos_user_initiated;
    rqos_stats[THREAD_QOS_USER_INTERACTIVE] +=
        task->cpu_time_rqos_stats.cpu_time_qos_user_interactive;
  }
...
void task_vtimer_set(task_t task, integer_t which) {
  thread_t thread;
  spl_t x;
...
void task_vtimer_clear(task_t task, integer_t which) {
  task_lock(task);
...
void task_vtimer_update(__unused task_t task, integer_t which,
                        uint32_t *microsecs) {
  thread_t thread = current_thread();
  uint32_t tdelt = 0;
  clock_sec_t secs = 0;
...
uint64_t get_task_dispatchqueue_offset(task_t task) {
  return task->dispatchqueue_offset;
}

void task_synchronizer_destroy_all(task_t task) {
  /*
 ...
 */
kern_return_t task_set_state(task_t task, int flavor, thread_state_t state,
 ...
 */
kern_return_t task_get_state(task_t task, int flavor, thread_state_t state,
                             mach_msg_type_number_t *state_count) {
  kern_return_t ret;
...
PROC_VIOLATED_GUARD__SEND_EXC_GUARD(mach_exception_code_t code,
                                    mach_exception_subcode_t subcode,
                                    void *reason, boolean_t backtrace_only) {
#ifdef MACH_BSD
  if (1 == proc_selfpid()) {
...
kern_return_t task_violated_guard(mach_exception_code_t code,
                                  mach_exception_subcode_t subcode,
                                  void *reason, bool backtrace_only) {
  return PROC_VIOLATED_GUARD__SEND_EXC_GUARD(code, subcode, reason,
                                             backtrace_only);
}

#if CONFIG_MEMORYSTATUS

bool task_get_memlimit_is_active(task_t task) {
  assert(task != NULL);

  return os_atomic_load(&task->memlimit_flags, relaxed) &
         TASK_MEMLIMIT_IS_ACTIVE;
}

void task_set_memlimit_is_active(task_t task, bool memlimit_is_active) {
  assert(task != NULL);
...
bool task_get_memlimit_is_fatal(task_t task) {
  assert(task != NULL);

  return os_atomic_load(&task->memlimit_flags, relaxed) &
         TASK_MEMLIMIT_IS_FATAL;
}

void task_set_memlimit_is_fatal(task_t task, bool memlimit_is_fatal) {
  assert(task != NULL);
...
uint64_t task_get_dirty_start(task_t task) { return task->memstat_dirty_start; }

void task_set_dirty_start(task_t task, uint64_t start) {
  task_lock(task);
  task->memstat_dirty_start = start;
  task_unlock(task);
}

bool task_set_exc_resource_bit(task_t task, bool memlimit_is_active) {
  /*
  ...
  task_memlimit_flags_t bit = memlimit_is_active
...
void task_reset_triggered_exc_resource(task_t task, bool memlimit_is_active) {
  task_memlimit_flags_t bit = memlimit_is_active
                                  ? TASK_MEMLIMIT_ACTIVE_EXC_RESOURCE
                                  : TASK_MEMLIMIT_INACTIVE_EXC_RESOURCE;
...
bool task_get_jetsam_realtime_audio(task_t task) {
  return task->task_jetsam_realtime_audio;
}

void task_set_jetsam_realtime_audio(task_t task, bool realtime_audio) {
  task_lock(task);
  task->task_jetsam_realtime_audio = realtime_audio;
  task_unlock(task);
}

#define HWM_USERCORE_MINSPACE                                                  \
  250 // free space (in MB) required *after* core file creation
...
PROC_CROSSED_HIGH_WATERMARK__SEND_EXC_RESOURCE_AND_SUSPEND(
  ...
  if (hwm_user_cores) {
     ...
     * core file.
     */
    if ((error = coredump(cur_bsd_info, HWM_USERCORE_MINSPACE,
                          COREDUMP_IGNORE_ULIMIT)) != 0) {
   ...
   */
  if ((exception_options & EXEC_RESOURCE_FATAL) || send_sync_exc_resource ||
    ...
    if (disable_exc_resource_during_audio && audio_active &&
        task->task_jetsam_realtime_audio) {
      printf(
          "process %s[%d] crossed memory high watermark (%d MB); EXC_RESOURCE "
          "suppressed due to audio playback.\n",
          procname, pid, max_footprint_mb);
    } else {
      task_enqueue_exception_with_corpse(task, EXC_RESOURCE, code,
                                         EXCEPTION_CODE_MAX, NULL, FALSE);
    }
  }
...
void task_footprint_exceeded(int warning, __unused const void *param0,
  ...
  if (is_diag_mem_threshold == IS_DIAGNOSTICS) {
    ledger_get_diag_mem_threshold(task->ledger, task_ledgers.phys_footprint,
                                  &enforced_limit);
  } else {
    ledger_get_limit(task->ledger, task_ledgers.phys_footprint,
                     &enforced_limit);
  }
  ...
  memlimit_is_fatal =
      task_get_memlimit_is_fatal(task) == FALSE ? IS_NOT_FATAL : IS_FATAL;
#if DEBUG || DEVELOPMENT
  if (is_diag_mem_threshold == IS_NOT_DIAGNOSTICS) {
    task_process_crossed_limit_no_diag(task, enforced_limit_mb,
                                       memlimit_is_fatal, memlimit_is_active,
                                       is_warning);
  } else {
    task_process_crossed_limit_diag(enforced_limit_mb);
  }
#else  /* DEBUG || DEVELOPMENT */
  task_process_crossed_limit_no_diag(task, enforced_limit_mb, memlimit_is_fatal,
                                     memlimit_is_active, is_warning);
#endif /* !(DEBUG || DEVELOPMENT) */
}

/*
 * Actions to perfrom when a process has crossed watermark or is a fatal
 * consumption */
static inline void task_process_crossed_limit_no_diag(
    task_t task, ledger_amount_t ledger_limit_size, bool memlimit_is_fatal,
    bool memlimit_is_active, send_exec_resource_is_warning is_warning) {
  ...
  /*
   * If this is an actual violation (not a warning), then generate EXC_RESOURCE
   * exception. We only generate the exception once per process per memlimit
   * (active/inactive limit). To enforce this, we monitor state based on the
   * memlimit's active/inactive attribute and we disable it by marking that
   * memlimit as exception triggered.
   */
  if (is_warning == IS_NOT_WARNING &&
      task_set_exc_resource_bit(task, memlimit_is_active)) {
    PROC_CROSSED_HIGH_WATERMARK__SEND_EXC_RESOURCE_AND_SUSPEND(
        (int)ledger_limit_size, exception_options);
    // If it was not a diag threshold (if was a memory limit), then we do not
    // want more signalling, however, if was a diag limit, the user may reload a
    // different limit and signal again the violation
    memorystatus_log_exception((int)ledger_limit_size, memlimit_is_active,
                               memlimit_is_fatal);
  }
  memorystatus_on_ledger_footprint_exceeded(
      is_warning == IS_NOT_WARNING ? FALSE : TRUE, memlimit_is_active,
      memlimit_is_fatal);
}
 ...
 */
void task_conclave_mem_limit_exceeded(__unused int warning,
                                      __unused const void *param0,
                                      __unused const void *param1) {
   ...
   */
  PROC_CROSSED_HIGH_WATERMARK__SEND_EXC_RESOURCE_AND_SUSPEND(
      (int)max_footprint_mb, EXEC_RESOURCE_FATAL | EXEC_RESOURCE_CONCLAVE);
...
task_process_crossed_limit_diag(ledger_amount_t ledger_limit_size) {
  /*
   * If this is an actual violation (not a warning), then generate EXC_RESOURCE
   * exception. In the case of the diagnostics thresholds, the exception will be
   * signaled only once, but the inhibit / rearm mechanism if performed at
   * ledger level.
   */
...
kern_return_t task_set_phys_footprint_limit(task_t task, int new_limit_mb,
                                            int *old_limit_mb) {
  kern_return_t error;
  ...
  return task_set_phys_footprint_limit_internal(
      task, new_limit_mb, old_limit_mb, memlimit_is_active, memlimit_is_fatal);
}
...
#if CONFIG_MEMORYSTATUS
#if DEVELOPMENT || DEBUG
kern_return_t task_set_diag_footprint_limit(task_t task, uint64_t new_limit_mb,
  ...
  return task_set_diag_footprint_limit_internal(task, new_limit_mb,
                                                old_limit_mb);
}
...
kern_return_t task_convert_phys_footprint_limit(int limit_mb,
  ...
  if (limit_mb == -1) {
    ...
    if (max_task_footprint != 0) {
      *converted_limit_mb =
          (int)(max_task_footprint / 1024 / 1024); /* bytes to MB */
    } else {
...
kern_return_t task_set_phys_footprint_limit_internal(
    task_t task, int new_limit_mb, int *old_limit_mb,
    boolean_t memlimit_is_active, boolean_t memlimit_is_fatal) {
  ledger_amount_t old;
  kern_return_t ret;
#if DEVELOPMENT || DEBUG
   ...
   */
#if DEVELOPMENT || DEBUG
  diag_threshold_validity =
      task_check_memorythreshold_is_valid(task, new_limit_mb, false);
  /**
   * If the footprint and diagnostics threshold are going to be same, lets
   * disable the threshold
   */
  if (diag_threshold_validity == THRESHOLD_IS_SAME_AS_LIMIT_FLAG_ENABLED) {
    ledger_set_diag_mem_threshold_disabled(task->ledger,
                                           task_ledgers.phys_footprint);
  } else if (diag_threshold_validity ==
             THRESHOLD_IS_NOT_SAME_AS_LIMIT_FLAG_DISABLED) {
    ledger_set_diag_mem_threshold_enabled(task->ledger,
                                          task_ledgers.phys_footprint);
  }
#endif
  ...
  if (new_limit_mb == -1) {
     ...
     */
    ledger_set_limit(
        task->ledger, task_ledgers.phys_footprint,
    ...
    task_unlock(task);
    /**
     * If the diagnostics were disabled, and now we have a new limit, we have to
     * re-enable it.
     */
#if DEVELOPMENT || DEBUG
    if (diag_threshold_validity == THRESHOLD_IS_SAME_AS_LIMIT_FLAG_ENABLED) {
  ...
  ledger_set_limit(task->ledger, task_ledgers.phys_footprint,
                   (ledger_amount_t)new_limit_mb << 20,
                   PHYS_FOOTPRINT_WARNING_LEVEL);
...
kern_return_t
task_set_diag_footprint_limit_internal(task_t task, uint64_t new_limit_bytes,
                                       uint64_t *old_limit_bytes) {
  ledger_amount_t old = 0;
  kern_return_t ret = KERN_SUCCESS;
  diagthreshold_check_return diag_threshold_validity;
  ret = ledger_get_diag_mem_threshold(task->ledger, task_ledgers.phys_footprint,
                                      &old);
   ...
   */
  diag_threshold_validity =
      task_check_memorythreshold_is_valid(task, new_limit_bytes >> 20, true);
  /**
   * If the footprint and diagnostics threshold are going to be same, lets
   * disable the threshold
   */
...
kern_return_t task_get_diag_footprint_limit_internal(task_t task,
  ...
  if (ret == KERN_SUCCESS) {
    *new_limit_bytes = ledger_limit;
    ret = ledger_is_diag_threshold_enabled(
        task->ledger, task_ledgers.phys_footprint, threshold_disabled);
  }
  return ret;
}
#endif /* RESETTABLE_DIAG_FOOTPRINT_LIMITS */

kern_return_t task_get_phys_footprint_limit(task_t task, int *limit_mb) {
   ...
   * equates to LEDGER_LIMIT_INFINITY.
   */
  assert(((limit & 0xFFF0000000000000LL) == 0) ||
...
kern_return_t task_set_phys_footprint_limit(__unused task_t task,
                                            __unused int new_limit_mb,
                                            __unused int *old_limit_mb) {
  return KERN_FAILURE;
}

kern_return_t task_get_phys_footprint_limit(__unused task_t task,
                                            __unused int *limit_mb) {
  return KERN_FAILURE;
}
#endif /* CONFIG_MEMORYSTATUS */

security_token_t *task_get_sec_token(task_t task) {
  return &task_get_ro(task)->task_tokens.sec_token;
}

void task_set_sec_token(task_t task, security_token_t *token) {
  zalloc_ro_update_field(ZONE_ID_PROC_RO, task_get_ro(task),
                         task_tokens.sec_token, token);
}

audit_token_t *task_get_audit_token(task_t task) {
  return &task_get_ro(task)->task_tokens.audit_token;
}

void task_set_audit_token(task_t task, audit_token_t *token) {
  zalloc_ro_update_field(ZONE_ID_PROC_RO, task_get_ro(task),
                         task_tokens.audit_token, token);
}

void task_set_tokens(task_t task, security_token_t *sec_token,
                     audit_token_t *audit_token) {
...
boolean_t task_is_privileged(task_t task) {
  return task_get_sec_token(task)->val[0] == 0;
}

#ifdef CONFIG_MACF
uint8_t *task_get_mach_trap_filter_mask(task_t task) {
  return task_get_ro(task)->task_filters.mach_trap_filter_mask;
}

void task_set_mach_trap_filter_mask(task_t task, uint8_t *mask) {
  zalloc_ro_update_field(ZONE_ID_PROC_RO, task_get_ro(task),
                         task_filters.mach_trap_filter_mask, &mask);
}

uint8_t *task_get_mach_kobj_filter_mask(task_t task) {
  return task_get_ro(task)->task_filters.mach_kobj_filter_mask;
}

mach_vm_address_t task_get_all_image_info_addr(task_t task) {
  return task->all_image_info_addr;
}

void task_set_mach_kobj_filter_mask(task_t task, uint8_t *mask) {
  zalloc_ro_update_field(ZONE_ID_PROC_RO, task_get_ro(task),
...
void task_set_thread_limit(task_t task, uint16_t thread_limit) {
  assert(task != kernel_task);
...
kern_return_t task_get_conclave_mem_limit(task_t task,
...
kern_return_t task_set_conclave_mem_limit(task_t task,
                                          uint64_t conclave_limit) {
  kern_return_t error;
...
#if CONFIG_PROC_RESOURCE_LIMITS
kern_return_t task_set_port_space_limits(task_t task, uint32_t soft_limit,
                                         uint32_t hard_limit) {
  return ipc_space_set_table_size_limits(task->itk_space, soft_limit,
                                         hard_limit);
}
#endif /* CONFIG_PROC_RESOURCE_LIMITS */

#if XNU_TARGET_OS_OSX
boolean_t task_has_system_version_compat_enabled(task_t task) {
...
void task_set_system_version_compat_enabled(
    task_t task, boolean_t enable_system_version_compat) {
  assert(task == current_task());
  assert(task != kernel_task);
...
boolean_t is_corpsefork(task_t t) { return task_is_a_corpse_fork(t); }

task_t current_task_early(void) {
...
task_t current_task(void) { return get_threadtask(current_thread()); }
 ...
 *       We should make a tid hash, or transition all tid clients to thread
 * ports
 *
 ...
 */
kern_return_t task_cpu_usage_monitor_ctl(task_t task, uint32_t *flags) {
 ...
 */
kern_return_t task_wakeups_monitor_ctl(task_t task, uint32_t *flags,
                                       int32_t *rate_hz) {
...
#ifndef CONFIG_NOMONITORS
    ledger_set_limit(ledger, task_ledgers.interrupt_wakeups,
                     *rate_hz * task_wakeups_monitor_interval,
                     (uint8_t)task_wakeups_monitor_ustackshots_trigger_pct);
    ledger_set_period(ledger, task_ledgers.interrupt_wakeups,
                      task_wakeups_monitor_interval * NSEC_PER_SEC);
...
TUNABLE(bool, enable_wakeup_reports, "enable_wakeup_reports",
        false); /* Enable wakeup reports. */
...
SENDING_NOTIFICATION__THIS_PROCESS_IS_CAUSING_TOO_MANY_WAKEUPS(void) {
  ...
  os_log(OS_LOG_DEFAULT,
         ...
         procname, pid, lei.lei_balance, lei.lei_last_refill / NSEC_PER_SEC,
         lei.lei_last_refill == 0
             ? 0
             : (NSEC_PER_SEC * lei.lei_balance / lei.lei_last_refill),
         fatal ? "FATAL " : "", lei.lei_limit,
  ...
  EXC_RESOURCE_CPUMONITOR_ENCODE_WAKEUPS_PERMITTED(
      code[0], NSEC_PER_SEC * lei.lei_limit / lei.lei_refill_period);
  EXC_RESOURCE_CPUMONITOR_ENCODE_OBSERVATION_INTERVAL(code[0],
                                                      lei.lei_last_refill);
  EXC_RESOURCE_CPUMONITOR_ENCODE_WAKEUPS_OBSERVED(
      code[1], NSEC_PER_SEC * lei.lei_balance / lei.lei_last_refill);
  exception_triage(EXC_RESOURCE, code, EXCEPTION_CODE_MAX);
#endif /* EXC_RESOURCE_MONITORS */
...
static boolean_t global_update_logical_writes(int64_t io_delta,
...
void task_update_physical_writes(__unused task_t task,
                                 __unused task_physical_write_flavor_t flavor,
                                 __unused uint64_t io_size,
  ...
  KDBG((VMDBG_CODE(DBG_VM_PHYS_WRITE_ACCT)) | DBG_FUNC_NONE, task_pid(task),
       flavor, io_size, flags);
  DTRACE_IO4(physical_writes, struct task *, task, task_physical_write_flavor_t,
             flavor, uint64_t, io_size, task_balance_flags_t, flags);
  ...
  if (flags & TASK_BALANCE_CREDIT) {
    if (flavor == TASK_PHYSICAL_WRITE_METADATA) {
      ...
      ledger_credit_nocheck(task->ledger, task_ledgers.fs_metadata_writes,
                            io_size);
    }
  } else if (flags & TASK_BALANCE_DEBIT) {
    if (flavor == TASK_PHYSICAL_WRITE_METADATA) {
      OSAddAtomic64(-1 * io_size, (SInt64 *)&(task->task_fs_metadata_writes));
      ledger_debit_nocheck(task->ledger, task_ledgers.fs_metadata_writes,
                           io_size);
    }
  }
#endif /* CONFIG_PHYS_WRITE_ACCT */
...
void task_update_logical_writes(task_t task, uint32_t io_size, int flags,
  ...
  KDBG((VMDBG_CODE(DBG_VM_DATA_WRITE)) | DBG_FUNC_NONE, task_pid(task), io_size,
       flags, (uintptr_t)VM_KERNEL_ADDRPERM(vp));
  DTRACE_IO4(logical_writes, struct task *, task, uint32_t, io_size, int, flags,
             vnode *, vp);
  ...
  case TASK_WRITE_IMMEDIATE:
    OSAddAtomic64(
        io_size, (SInt64 *)&(writes_counters_to_update->task_immediate_writes));
  ...
  case TASK_WRITE_DEFERRED:
    OSAddAtomic64(io_size,
                  (SInt64 *)&(writes_counters_to_update->task_deferred_writes));
    ledger_credit(task->ledger, ledger_to_update, io_size);
    if (!is_external_device) {
      coalition_io_ledger_update(task, FLAVOR_IO_LOGICAL_WRITES, TRUE, io_size);
    }
  ...
  case TASK_WRITE_INVALIDATED:
    OSAddAtomic64(
        io_size,
        (SInt64 *)&(writes_counters_to_update->task_invalidated_writes));
    ledger_debit(task->ledger, ledger_to_update, io_size);
    if (!is_external_device) {
      coalition_io_ledger_update(task, FLAVOR_IO_LOGICAL_WRITES, FALSE,
                                 io_size);
    }
    break;
  case TASK_WRITE_METADATA:
    OSAddAtomic64(io_size,
                  (SInt64 *)&(writes_counters_to_update->task_metadata_writes));
    ledger_credit(task->ledger, ledger_to_update, io_size);
    if (!is_external_device) {
      coalition_io_ledger_update(task, FLAVOR_IO_LOGICAL_WRITES, TRUE, io_size);
    }
  ...
  io_delta = (flags == TASK_WRITE_INVALIDATED) ? ((int64_t)io_size * -1ll)
                                               : ((int64_t)io_size);
  if (io_telemetry_limit != 0) {
    /* If io_telemetry_limit is 0, disable global updates and I/O telemetry */
    needs_telemetry =
        global_update_logical_writes(io_delta, global_counter_to_update);
 ...
 */
kern_return_t task_io_monitor_ctl(task_t task, uint32_t *flags) {
  ledger_t ledger = task->ledger;

  task_lock(task);
  if (*flags & IOMON_ENABLE) {
    /* Configure the physical I/O ledger */
    ledger_set_limit(ledger, task_ledgers.physical_writes,
                     (task_iomon_limit_mb * 1024 * 1024), 0);
    ledger_set_period(ledger, task_ledgers.physical_writes,
                      (task_iomon_interval_secs * NSEC_PER_SEC));
...
SENDING_NOTIFICATION__THIS_PROCESS_IS_CAUSING_TOO_MUCH_IO(int flavor) {
...
#endif
  /*
   * Get the ledger entry info. We need to do this before disabling the
   * exception to get correct values for all fields.
   */
  switch (flavor) {
  ...
  os_log(OS_LOG_DEFAULT,
         "process [%d] caught causing excessive I/O (flavor: %d). Task I/O: "
         "%lld MB. [Limit : %lld MB per %lld secs]\n",
         pid, flavor, (lei.lei_balance / (1024 * 1024)),
         (lei.lei_limit / (1024 * 1024)),
         (lei.lei_refill_period / NSEC_PER_SEC));

  kr = send_resource_violation(send_disk_writes_violation, task, &lei,
                               kRNFlagsNone);
  if (kr) {
  ...
  EXC_RESOURCE_ENCODE_FLAVOR(code[0], flavor);
  EXC_RESOURCE_IO_ENCODE_INTERVAL(code[0],
                                  (lei.lei_refill_period / NSEC_PER_SEC));
...
void task_port_space_ast(__unused task_t task) {
  uint32_t current_size, soft_limit, hard_limit;
  assert(task == current_task());
  bool should_notify = ipc_space_check_table_size_limit(
      task->itk_space, &current_size, &soft_limit, &hard_limit);
  if (should_notify) {
    SENDING_NOTIFICATION__THIS_PROCESS_HAS_TOO_MANY_MACH_PORTS(
        task, current_size, soft_limit, hard_limit);
  }
}
...
static mach_port_t task_allocate_fatal_port(void) {
  ...
  kern_return_t kr = task_create_identity_token(
      current_task(), &token); /* Takes a reference on the token */
  if (kr) {
...
static void task_fatal_port_no_senders(ipc_port_t port,
                                       __unused mach_port_mscount_t mscount) {
  ...
  assert(token != NULL);
  if (token) {
    kr = task_identity_token_get_task_grp(
        token, &task, TASK_GRP_KERNEL); /* takes a reference on task */
    if (task) {
...
void __attribute__((noinline))
SENDING_NOTIFICATION__THIS_PROCESS_HAS_TOO_MANY_MACH_PORTS(
    task_t task, uint32_t current_size, uint32_t soft_limit,
    uint32_t hard_limit) {
  int pid = 0;
  ...
  os_log(OS_LOG_DEFAULT,
         "process %s[%d] caught allocating too many mach ports. \
	    Num of ports allocated %u; \n",
         procname, pid, current_size);

  /* Abort the process if it has hit the system-wide limit for ipc port table
   * size */
  if (!hard_limit && !soft_limit) {
    ...
    exception_info_t info = {.os_reason = OS_REASON_PORT_SPACE,
  ...
  if (hard_limit > 0) {
    ...
    if (!task_fatal_port) {
      os_log(OS_LOG_DEFAULT,
             "process %s[%d] Unable to create task token ident object",
             procname, pid);
      task_bsdtask_kill(task);
    }
  } else {
  ...
  kr = send_resource_violation_with_fatal_port(
      send_port_space_violation, task, (int64_t)current_size, (int64_t)limit,
      task_fatal_port, flags);
  if (kr) {
...
#if CONFIG_PROC_RESOURCE_LIMITS
void task_kqworkloop_ast(task_t task, int current_size, int soft_limit,
                         int hard_limit) {
  assert(task == current_task());
  return SENDING_NOTIFICATION__THIS_PROCESS_HAS_TOO_MANY_KQWORKLOOPS(
      task, current_size, soft_limit, hard_limit);
}
...
SENDING_NOTIFICATION__THIS_PROCESS_HAS_TOO_MANY_KQWORKLOOPS(task_t task,
  ...
  os_log(OS_LOG_DEFAULT,
         "process %s[%d] caught allocating too many kqworkloops. \
  ...
  kern_return_t kr;
  kr = send_resource_violation_with_fatal_port(
      send_kqworkloops_violation, task, (int64_t)current_size, (int64_t)limit,
      task_fatal_port, flags);
  if (kr) {
    os_log(OS_LOG_DEFAULT,
           "send_resource_violation_with_fatal_port(kqworkloops, ...): error "
           "%#x\n",
           kr);
  }
  if (task_fatal_port) {
...
void task_filedesc_ast(__unused task_t task, __unused int current_size,
                       __unused int soft_limit, __unused int hard_limit) {
  assert(task == current_task());
  SENDING_NOTIFICATION__THIS_PROCESS_HAS_TOO_MANY_FILE_DESCRIPTORS(
      task, current_size, soft_limit, hard_limit);
}
...
SENDING_NOTIFICATION__THIS_PROCESS_HAS_TOO_MANY_FILE_DESCRIPTORS(
  ...
  os_log(OS_LOG_DEFAULT,
         "process %s[%d] caught allocating too many file descriptors. \
  ...
  kr = send_resource_violation_with_fatal_port(
      send_file_descriptors_violation, task, (int64_t)current_size,
      (int64_t)limit, task_fatal_port, flags);
  if (kr) {
    os_log(
        OS_LOG_DEFAULT,
        "send_resource_violation_with_fatal_port(filedesc, ...): error %#x\n",
        kr);
  }
  if (task_fatal_port) {
    ipc_port_release_send(task_fatal_port);
...
kern_return_t task_get_mach_voucher(task_t task,
...
kern_return_t task_set_mach_voucher(task_t task,
...
kern_return_t task_swap_mach_voucher(__unused task_t task,
...
void task_set_gpu_role(task_t task, darwin_gpu_role_t gpu_role) {
  task_lock(task);
...
darwin_gpu_role_t task_get_gpu_role(task_t task) {
  return os_atomic_load(&task->t_gpu_role, relaxed);
}

boolean_t task_is_gpu_denied(task_t task) {
  return (os_atomic_load(&task->t_gpu_role, relaxed) == PRIO_DARWIN_GPU_DENY)
             ? TRUE
             : FALSE;
}
 ...
 */
bool task_set_game_mode_locked(task_t task, bool enabled) {
  task_lock_assert_owned(task);
  ...
  if (enabled && !previously_enabled) {
    if (task_coalition_adjust_game_mode_count(task, 1, &new_count) &&
        (new_count == 1)) {
      needs_update = true;
    }
...
void task_set_game_mode(task_t task, bool enabled) {
  bool needs_update = false;
...
bool task_get_game_mode(task_t task) {
  /* We don't need the lock to read this flag */
  return task->t_flags & TF_GAME_MODE;
}

bool task_set_carplay_mode_locked(task_t task, bool enabled) {
  task_lock_assert_owned(task);
  ...
  if (enabled && !previously_enabled) {
    if (task_coalition_adjust_carplay_mode_count(task, 1, &new_count) &&
        (new_count == 1)) {
      needs_update = true;
    }
  } else if (!enabled && previously_enabled) {
...
void task_set_carplay_mode(task_t task, bool enabled) {
  bool needs_update = false;
...
bool task_get_carplay_mode(task_t task) {
  /* We don't need the lock to read this flag */
...
static void kdebug_trace_dyld_internal(uint32_t base_code,
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, KDBG_EVENTID(DBG_DYLD, DBG_DYLD_UUID, base_code), uuid[0],
      uuid[1], info->load_addr,
      (uint64_t)info->fsid.val[0] | ((uint64_t)info->fsid.val[1] << 32), 0);
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, KDBG_EVENTID(DBG_DYLD, DBG_DYLD_UUID, base_code + 1),
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, KDBG_EVENTID(DBG_DYLD, DBG_DYLD_UUID, base_code + 2),
      uuid[0], uuid[1], uuid[2], uuid[3], 0);
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, KDBG_EVENTID(DBG_DYLD, DBG_DYLD_UUID, base_code + 3),
      (uint32_t)info->load_addr, info->fsid.val[0], info->fsid.val[1],
      info->fsobjid.fid_objno, 0);
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE, KDBG_EVENTID(DBG_DYLD, DBG_DYLD_UUID, base_code + 4),
      info->fsobjid.fid_generation, 0, 0, 0, 0);
#endif /* !defined(__LP64__) */
}

static kern_return_t kdebug_trace_dyld(task_t task, uint32_t base_code,
                                       vm_map_copy_t infos_copy,
...
task_register_dyld_image_infos(task_t task,
                               ...
                               mach_msg_type_number_t infos_len) {
  return kdebug_trace_dyld(task, DBG_DYLD_UUID_MAP_A, (vm_map_copy_t)infos_copy,
                           infos_len);
}
...
kern_return_t task_register_dyld_shared_cache_image_info(
    task_t task, dyld_kernel_image_info_t cache_img,
    __unused boolean_t no_cache, __unused boolean_t private_cache) {
  if (task == NULL || task != current_task()) {
    return KERN_INVALID_TASK;
  }
...
kern_return_t task_register_dyld_set_dyld_state(__unused task_t task,
...
kern_return_t task_inspect(task_inspect_t task_insp,
                           task_inspect_flavor_t flavor,
                           task_inspect_info_t info_out,
                           mach_msg_type_number_t *size_in_out) {
#if CONFIG_PERVASIVE_CPI
...
void task_set_can_use_secluded_mem(task_t task,
                                   boolean_t can_use_secluded_mem) {
...
void task_set_can_use_secluded_mem_locked(task_t task,
                                          boolean_t can_use_secluded_mem) {
  assert(task->task_could_use_secluded_mem);
  if (can_use_secluded_mem && secluded_for_apps && /* global boot-arg */
      !task->task_can_use_secluded_mem) {
    assert(num_tasks_can_use_secluded_mem >= 0);
    OSAddAtomic(+1, (volatile SInt32 *)&num_tasks_can_use_secluded_mem);
    task->task_can_use_secluded_mem = TRUE;
  } else if (!can_use_secluded_mem && task->task_can_use_secluded_mem) {
    assert(num_tasks_can_use_secluded_mem > 0);
    OSAddAtomic(-1, (volatile SInt32 *)&num_tasks_can_use_secluded_mem);
...
void task_set_could_use_secluded_mem(task_t task,
                                     boolean_t could_use_secluded_mem) {
  task->task_could_use_secluded_mem = !!could_use_secluded_mem;
}

void task_set_could_also_use_secluded_mem(
    task_t task, boolean_t could_also_use_secluded_mem) {
  task->task_could_also_use_secluded_mem = !!could_also_use_secluded_mem;
}

boolean_t task_can_use_secluded_mem(task_t task, boolean_t is_alloc) {
  if (task->task_can_use_secluded_mem) {
    assert(task->task_could_use_secluded_mem);
    assert(num_tasks_can_use_secluded_mem > 0);
    return TRUE;
...
boolean_t task_could_use_secluded_mem(task_t task) {
  return task->task_could_use_secluded_mem;
}

boolean_t task_could_also_use_secluded_mem(task_t task) {
  return task->task_could_also_use_secluded_mem;
}
#endif /* CONFIG_SECLUDED_MEMORY */

queue_head_t *task_io_user_clients(task_t task) {
  return &task->io_user_clients;
}

void task_set_message_app_suspended(task_t task, boolean_t enable) {
  task->message_app_suspended = enable;
}

void task_copy_fields_for_exec(task_t dst_task, task_t src_task) {
...
kern_return_t task_self_region_info_flags_set(int newval) {
  task_t curtask;
...
void task_set_darkwake_mode(task_t task, boolean_t set_mode) {
  assert(task);
...
boolean_t task_get_darkwake_mode(task_t task) {
  assert(task);
 ...
 */
void task_set_exc_guard_default(task_t task, const char *name,
                                unsigned long namelen, boolean_t is_simulated,
                                uint32_t platform, uint32_t sdk) {
  if (task_get_platform_restrictions_version(task) >= 1) {
    ...
    if (1 == task_pid(task)) {
      ...
      int count = sizeof(task_exc_guard_named_defaults) /
                  sizeof(struct task_exc_guard_named_default);
    ...
    task->task_exc_guard =
        ((task_exc_guard_default >> TASK_EXC_GUARD_THIRD_PARTY_DEFAULT_SHIFT) &
         TASK_EXC_GUARD_ALL);
  }
...
kern_return_t
task_get_exc_guard_behavior(task_t task, task_exc_guard_behavior_t *behaviorp) {
  if (task == TASK_NULL) {
    return KERN_INVALID_TASK;
  }
...
kern_return_t
task_set_exc_guard_behavior(task_t task,
  ...
  os_atomic_rmw_loop(&task->task_exc_guard, cur_behavior, new_behavior, relaxed,
                     {
                       if ((cur_behavior & task_exc_guard_no_unset_mask) &
                           ~(new_behavior & task_exc_guard_no_unset_mask)) {
                         os_atomic_rmw_loop_give_up(return KERN_DENIED);
                       }

                       if ((new_behavior & task_exc_guard_no_set_mask) &
                           ~(cur_behavior & task_exc_guard_no_set_mask)) {
                         os_atomic_rmw_loop_give_up(return KERN_DENIED);
                       }
...
kern_return_t
task_set_corpse_forking_behavior(task_t task,
...
boolean_t task_corpse_forking_disabled(task_t task) {
...
extern void memorystatus_act_on_legacy_footprint_entitlement(struct proc *,
                                                             boolean_t);
extern void
memorystatus_act_on_ios13extended_footprint_entitlement(struct proc *);

void task_set_legacy_footprint(task_t task) {
  task_lock(task);
  task->task_legacy_footprint = TRUE;
  task_unlock(task);
}

void task_set_extra_footprint_limit(task_t task) {
  ...
  task_unlock(task);
  memorystatus_act_on_legacy_footprint_entitlement(get_bsdtask_info(task),
                                                   TRUE);
}

void task_set_ios13extended_footprint_limit(task_t task) {
  ...
  task_unlock(task);
  memorystatus_act_on_ios13extended_footprint_entitlement(
      get_bsdtask_info(task));
}
#endif /* __arm64__ */

static inline ledger_amount_t task_ledger_get_balance(ledger_t ledger,
                                                      int ledger_idx) {
 ...
 */
void task_ledgers_footprint(ledger_t ledger, ledger_amount_t *ledger_resident,
  ...
  *ledger_resident +=
      task_ledger_get_balance(ledger, task_ledgers.purgeable_nonvolatile);
  *ledger_compressed += task_ledger_get_balance(
      ledger, task_ledgers.purgeable_nonvolatile_compressed);

  /* "default" tagged memory */
  *ledger_resident +=
      task_ledger_get_balance(ledger, task_ledgers.tagged_footprint);
  *ledger_compressed +=
  ...
  /* "media" tagged memory */
  *ledger_resident +=
      task_ledger_get_balance(ledger, task_ledgers.media_footprint);
  *ledger_compressed +=
      task_ledger_get_balance(ledger, task_ledgers.media_footprint_compressed);

  /* "graphics" tagged memory */
  *ledger_resident +=
      task_ledger_get_balance(ledger, task_ledgers.graphics_footprint);
  *ledger_compressed += task_ledger_get_balance(
      ledger, task_ledgers.graphics_footprint_compressed);

  /* "neural" tagged memory */
  *ledger_resident +=
      task_ledger_get_balance(ledger, task_ledgers.neural_footprint);
  *ledger_compressed +=
      task_ledger_get_balance(ledger, task_ledgers.neural_footprint_compressed);
}

#if CONFIG_MEMORYSTATUS
void task_ledger_settle_dirty_time(task_t t) {
  task_lock(t);
 ...
 */
void task_ledger_settle_dirty_time_locked(task_t t) {
...
void task_ledger_settle(task_t t) {
#if CONFIG_MEMORYSTATUS
  task_lock(t);
...
void task_set_no_footprint_for_debug(task_t task, boolean_t value) {
  task_lock(task);
  task->task_no_footprint_for_debug = !!value;
  task_unlock(task);
}

int task_get_no_footprint_for_debug(task_t task) {
  return task->task_no_footprint_for_debug;
}

#endif /* DEVELOPMENT || DEBUG */

void task_copy_vmobjects(task_t task, vm_object_query_t query, size_t len,
                         size_t *num) {
  ...
  if (query != NULL) {
    queue_iterate(&task->task_objq, find_vmo, vm_object_t, task_objq) {
      ...
      if (find_vmo->internal && find_vmo->pager_created &&
          find_vmo->pager != NULL) {
        p->compressed_size =
            vm_compressor_pager_get_count(find_vmo->pager) * PAGE_SIZE;
      } else {
        p->compressed_size = 0;
      }
    }
...
void task_get_owned_vmobjects(task_t task, size_t buffer_size,
                              vmobject_list_output_t buffer,
                              size_t *output_size, size_t *entries) {
  assert(output_size);
  assert(entries);
  ...
  if (buffer_size == 0) {
    ...
    *output_size = (*entries > 0) ? *entries * sizeof(vm_object_query_data_t) +
                                        sizeof(*buffer)
                                  : 0;
  } else {
    assert(buffer);
    task_copy_vmobjects(task, &buffer->data[0], buffer_size - sizeof(*buffer),
                        entries);
...
static void task_store_owned_vmobject_info(task_t to_task, task_t from_task) {
  ...
  if (output_size) {
    ...
    if (buffer) {
      ...
      task_get_owned_vmobjects(from_task, buffer_size, buffer, &output_size,
                               &entries);
...
void task_set_filter_msg_flag(task_t task, boolean_t flag) {
...
boolean_t task_get_filter_msg_flag(task_t task) {
  if (!task) {
    return false;
  }
...
bool task_is_exotic(task_t task) {
  if (task == TASK_NULL) {
    return false;
  }
...
bool task_is_alien(task_t task) {
  if (task == TASK_NULL) {
    return false;
  }
...
#if CONFIG_MACF
uint8_t *mac_task_get_mach_filter_mask(task_t task) {
  assert(task);
  return task_get_mach_trap_filter_mask(task);
}

uint8_t *mac_task_get_kobj_filter_mask(task_t task) {
  assert(task);
  return task_get_mach_kobj_filter_mask(task);
}

/* Set the filter mask for Mach traps. */
void mac_task_set_mach_filter_mask(task_t task, uint8_t *maskptr) {
  assert(task);
...
void mac_task_set_kobj_filter_mask(task_t task, uint8_t *maskptr) {
  assert(task);
...
SECURITY_READ_ONLY_LATE(mac_task_mach_filter_cbfunc_t)
mac_task_mach_trap_evaluate = NULL;

/* Hook for kobj message filter evaluation policy. */
SECURITY_READ_ONLY_LATE(mac_task_kobj_filter_cbfunc_t)
...
#if CONFIG_ROSETTA
bool task_is_translated(task_t task) {
...
uint32_t task_get_security_config(task_t task) {
  assert(task);
...
#if __has_feature(ptrauth_calls)
/* On FPAC, we want to deliver all PAC violations as fatal exceptions,
 * regardless of the enable_pac_exception boot-arg value or any other
 * entitlements. The only case where we allow non-fatal PAC exceptions on FPAC
 * is for debugging, which requires Developer Mode enabled.
 *
...
void task_set_pac_exception_fatal_flag(task_t task) {
  assert(task != TASK_NULL);
  bool pac_hardened_task = false;
  uint32_t set_flags = 0;

  /*
   * We must not apply this security policy on tasks which have opted out of
   * mach hardening to avoid regressions in third party plugins and third party
   * apps when using AMFI boot-args
   */
  ipc_space_policy_t pol = ipc_policy_for_task(task);
  ...
  /* On non-FPAC hardware, gate the fatal property behind entitlements and
   * boot-arg. */
...
bool task_is_pac_exception_fatal(task_t task) {
  assert(task != TASK_NULL);
...
void task_set_jit_flags(task_t task) {
  assert(task != TASK_NULL);
  if (IOTaskHasStringEntitlement(task, FATAL_EXCEPTION_ENTITLEMENT,
                                 FATAL_EXCEPTION_ENTITLEMENT_JIT)) {
    task_ro_flags_set(task, TFRO_JIT_EXC_FATAL);
  }
}

bool task_is_jit_exception_fatal(__unused task_t task) {
...
bool task_needs_user_signed_thread_state(task_t task) {
  assert(task != TASK_NULL);
  return !!(task_ro_flags_get(task) & TFRO_PAC_ENFORCE_USER_STATE);
}

void task_set_tecs(task_t task) {
  if (task == TASK_NULL) {
    task = current_task();
  }
...
kern_return_t task_test_sync_upcall(task_t task, ipc_port_t send_port) {
#if DEVELOPMENT || DEBUG
...
kern_return_t task_test_async_upcall_propagation(task_t task,
                                                 ipc_port_t send_port, int qos,
                                                 int iotier) {
#if DEVELOPMENT || DEBUG
  kern_return_t kr;
  ...
  struct thread_attr_for_ipc_propagation attr = {.tafip_iotier = iotier,
                                                 .tafip_qos = qos};
...
#if defined(__x86_64__)
bool curtask_get_insn_copy_optout(void) {
  bool optout;
...
void curtask_set_insn_copy_optout(void) {
  task_t cur_task = current_task();
...
void task_get_corpse_vmobject_list(task_t task, vmobject_list_output_t *list,
                                   size_t *list_size) {
  assert(task);
  assert(list_size);
...
__abortlike static void panic_proc_ro_task_backref_mismatch(task_t t,
                                                            proc_ro_t ro) {
  panic("proc_ro->task backref mismatch: t=%p, ro=%p, "
        "proc_ro_task(ro)=%p",
        t, ro, proc_ro_task(ro));
}

proc_ro_t task_get_ro(task_t t) {
...
uint32_t task_ro_flags_get(task_t task) {
  return task_get_ro(task)->t_flags_ro;
}

void task_ro_flags_set(task_t task, uint32_t flags) {
  zalloc_ro_update_field_atomic(ZONE_ID_PROC_RO, task_get_ro(task), t_flags_ro,
                                ZRO_ATOMIC_OR_32, flags);
}

void task_ro_flags_clear(task_t task, uint32_t flags) {
  zalloc_ro_update_field_atomic(ZONE_ID_PROC_RO, task_get_ro(task), t_flags_ro,
                                ZRO_ATOMIC_AND_32, ~flags);
}

task_control_port_options_t task_get_control_port_options(task_t task) {
 ...
 */
boolean_t kdp_task_is_locked(task_t task) {
...
task_check_memorythreshold_is_valid(task_t task, uint64_t new_limit,
  ...
  bool dummy;
  ret_value = ledger_is_diag_threshold_enabled(
      task->ledger, task_ledgers.phys_footprint, &threshold_enabled);
  ...
  if (is_diagnostics_value == true) {
    ...
    uint64_t diag_limit;
    ret_value =
        task_get_diag_footprint_limit_internal(task, &diag_limit, &dummy);
...
#if CONFIG_EXCLAVES
kern_return_t task_add_conclave(task_t task, void *vnode, int64_t off,
                                const char *task_conclave_id) {
  /*
...
kern_return_t task_launch_conclave(mach_port_name_t port __unused) {
...
void task_clear_conclave(task_t task) {
  if (task->exclave_crash_info) {
    kfree_data(task->exclave_crash_info,
               CONCLAVE_CRASH_BUFFER_PAGECOUNT * PAGE_SIZE);
...
void task_stop_conclave(task_t task, bool gather_crash_bt) {
  ...
  if (task->conclave == NULL ||
      exclaves_is_forwarding_resource(task->conclave)) {
    return;
  }
...
void task_suspend_conclave(task_t task) {
  ...
  __assert_only kern_return_t ret = exclaves_conclave_suspend(task->conclave);
...
void task_resume_conclave(task_t task) {
  ...
  __assert_only kern_return_t ret = exclaves_conclave_resume(task->conclave);
...
kern_return_t task_suspend_conclave_upcall(uint64_t *scid_list,
                                           size_t scid_list_count) {
  task_t task = current_task();
  thread_t thread;
  int scid_count = 0;
  kern_return_t kr;
...
kern_return_t task_crash_info_conclave_upcall(
  ...
  if (!length) {
    printf("Conclave upcall: task_crash_info_conclave_upcall did not return "
           "any page addresses\n");
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (!task_crash_info_buffer) {
    panic("task_crash_info_conclave_upcall: cannot allocate buffer for "
          "task_info shared memory");
    return KERN_INVALID_ARGUMENT;
  }
  ...
  for (size_t i = 0; i < CONCLAVE_CRASH_BUFFER_PAGECOUNT; i++) {
    if (remaining) {
      memcpy(dst, (uint8_t *)phystokv((pmap_paddr_t)shared_buf->physaddr[i]),
             PAGE_SIZE);
...
exclaves_resource_t *task_get_conclave(task_t task) { return task->conclave; }

extern boolean_t IOPMRootDomainGetWillShutdown(void);

TUNABLE(bool, disable_conclave_taint, "disable_conclave_taint",
        true); /* Do not taint processes when they talk to conclave, so system
                  does not panic when exit. */
...
void task_add_conclave_crash_info(task_t task, void *crash_info_ptr) {
  ...
  error =
      kcdata_add_container_marker(crash_info_ptr, KCDATA_TYPE_CONTAINER_BEGIN,
  ...
  tberr = stackshot_stackshotresult__unmarshal(
      crash_info, (uint64_t)crash_info_length,
      ^(stackshot_stackshotresult_s result) {
        error = stackshot_exclaves_process_stackshot(&result, crash_info_ptr,
                                                     false);
        if (error != KERN_SUCCESS) {
          printf("task_add_conclave_crash_info: error processing stackshot "
                 "result %d\n",
                 error);
        }
      });
  if (tberr != TB_ERROR_SUCCESS) {
    printf("task_conclave_crash: task_add_conclave_crash_info could not "
           "unmarshal stackshot data 0x%x\n",
           tberr);
    error = KERN_FAILURE;
    goto error_exit;
  }
