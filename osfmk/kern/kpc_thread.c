...
static void kpc_update_thread_counters(thread_t thread) {
  uint32_t i;
  uint64_t *tmp = NULL;
  cpu_data_t *cpu = NULL;
  ...
  kpc_get_cpu_counters(FALSE, kpc_thread_classes, NULL, cpu->cpu_kpc_buf[1]);
...
int kpc_get_curthread_counters(uint32_t *inoutcount, uint64_t *buf) {
  ...
  /* copy out */
  memcpy(buf, thread->kpc_buf, kpc_thread_classes_count * sizeof(*buf));
...
void kpc_off_cpu_update(void) { kpc_off_cpu_active = kpc_threads_counting; }

void kpc_off_cpu_internal(thread_t thread) {
