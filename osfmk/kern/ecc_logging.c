...
kern_return_t ecc_log_memory_error(__unused pmap_paddr_t physical_address,
                                   __unused uint32_t ecc_flags) {
  return KERN_NOT_SUPPORTED;
}

kern_return_t
ecc_log_memory_error_internal(__unused pmap_paddr_t physical_address,
                              __unused uint32_t ecc_flags) {
  return KERN_NOT_SUPPORTED;
}

kern_return_t ecc_log_memory_error_ce(__unused pmap_paddr_t physical_address,
                                      __unused uint32_t ecc_flags,
...
kern_return_t
kern_ecc_poll_register(__unused platform_error_handler_ecc_poll_t poll_func,
 ...
 */
kern_return_t
ecc_log_memory_error_delayed(__unused pmap_paddr_t physical_address,
                             __unused uint32_t ecc_flags) {
  return KERN_FAILURE;
}
...
static inline int mcc_events_next(int idx) {
  assert(idx < MCC_ECC_NUM_ERRORS);
  return (idx + 1) % MCC_ECC_NUM_ERRORS;
}

/* MCC ECC CoreAnalytics Error Logging */
static void mcc_error_notify_user(mcc_ecc_event_t event) {
...
static void
mcc_memory_error_event_queue_invoke(mpsc_queue_chain_t e,
  ...
  int mcc_events_consumer_next_idx =
      mcc_events_next(mcc_events_consumer_curr_idx);
...
mcc_memory_error_create_event(mcc_ecc_event_t mcc_event) {
  ...
  /**
   * @note We are unable to dynamically allocate events, because this function
   * can be called from the primary interrupt context.  Instead, we allocate
   * from a statically sized ring buffer.
   */
  ...
  int mcc_events_producer_next_idx =
      mcc_events_next(mcc_events_producer_curr_idx);
  if (mcc_events_producer_next_idx == atomic_load(&mcc_events_consumer_idx)) {
    /**
     * The consumer is running behind the producer, and we're in the primary
     * interrupt context. Drop this event and return NULL to the caller.
     */
...
__startup_func static void mcc_logging_init(void) {
  mpsc_daemon_queue_init_with_thread(
      &mcc_memory_error_event_queue, mcc_memory_error_event_queue_invoke,
      MCC_ERROR_EVENT_QUEUE_PRIORITY, "daemon.mcc_error-events",
      MPSC_DAEMON_INIT_INACTIVE);
...
kern_return_t mcc_log_memory_error(mcc_ecc_event_t mcc_event __unused) {
#if XNU_HANDLE_MCC
  ...
  mpsc_daemon_enqueue(&mcc_memory_error_event_queue, &event->link,
...
static int mcc_memory_error_notify_test_run(int64_t in, int64_t *out) {
  ...
  for (uint64_t i = 0; i < in; i++) {
    ...
    /**
     * To accurately test mcc_log_memory_error, we must disable preemption,
     * because it is called from the primary interrupt context.
     */
    disable_preemption();
...
static boolean_t ecc_data_empty =
    TRUE; // next read == next write : empty or full?
...
kern_return_t ecc_log_record_event(const struct ecc_event *ev) {
  spl_t x;
...
kern_return_t ecc_log_get_next_event(struct ecc_event *ev) {
  spl_t x;
