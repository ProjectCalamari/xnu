...
static os_atomic(exclaves_boot_status_t)
    exclaves_boot_status = EXCLAVES_BS_NOT_DEFINED;

static thread_t exclaves_boot_thread = THREAD_NULL;

extern exclaves_boot_task_entry_t
    exclaves_boot_task_entries[] __SECTION_START_SYM(
        EXCLAVES_BOOT_TASK_SEGMENT, EXCLAVES_BOOT_TASK_SECTION);

extern exclaves_boot_task_entry_t
    exclaves_boot_task_entries_end[] __SECTION_END_SYM(
        EXCLAVES_BOOT_TASK_SEGMENT, EXCLAVES_BOOT_TASK_SECTION);

static int ebt_cmp(const void *e1, const void *e2) {
...
static void exclaves_boot_tasks(void) {
  const size_t count =
      exclaves_boot_task_entries_end - exclaves_boot_task_entries;
  assert3u(count, >, 0);

  __assert_only const size_t size = ((uintptr_t)exclaves_boot_task_entries_end -
  ...
  for (size_t i = 0; i < count; i++) {
    exclaves_debug_printf(show_progress, "exclaves: boot task started, %s\n",
                          boot_tasks[i].ebt_name);

    KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_EXCLAVES, MACH_EXCLAVES_BOOT_TASK) |
                     DBG_FUNC_START,
                 i);
    ...
    exclaves_debug_printf(show_progress, "exclaves: boot task done, %s\n",
                          boot_tasks[i].ebt_name);

    if (ret != KERN_SUCCESS) {
      panic("exclaves: boot task failed: %s (%p)", boot_tasks[i].ebt_name,
            &boot_tasks[i]);
    }
  }
}
 ...
 * boot status if not.
 */
__startup_func static void exclaves_check_sk(void) {
  const bool sk_bootstrapped = SPTMArgs->sk_bootstrapped;

  if (sk_bootstrapped) {
    os_atomic_store(&exclaves_boot_status, EXCLAVES_BS_NOT_STARTED, relaxed);
  } else {
    os_atomic_store(&exclaves_boot_status, EXCLAVES_BS_NOT_SUPPORTED, relaxed);
  }
}
STARTUP(EXCLAVES, STARTUP_RANK_FIRST, exclaves_check_sk);

static void exclaves_boot_status_wait(const exclaves_boot_status_t status) {
...
static void exclaves_boot_status_wake(void) {
  lck_mtx_assert(&exclaves_boot_lock, LCK_MTX_ASSERT_OWNED);
  thread_wakeup((event_t)(uintptr_t)&exclaves_boot_status);
}

static void exclaves_boot_status_set(const exclaves_boot_status_t status) {
...
static bool exclaves_boot_status_is_supported(exclaves_boot_status_t status) {
...
static kern_return_t exclaves_boot_exclavecore(void) {
  lck_mtx_assert(&exclaves_boot_lock, LCK_MTX_ASSERT_OWNED);
...
static kern_return_t exclaves_boot_exclavekit(void) {
  lck_mtx_assert(&exclaves_boot_lock, LCK_MTX_ASSERT_OWNED);
...
kern_return_t exclaves_boot(exclaves_boot_stage_t boot_stage) {
...
exclaves_boot_wait(const exclaves_boot_stage_t desired_boot_stage) {
  ...
  const exclaves_boot_status_t desired_boot_status =
      desired_boot_stage == EXCLAVES_BOOT_STAGE_EXCLAVECORE
          ? EXCLAVES_BS_BOOTED_EXCLAVECORE
          : EXCLAVES_BS_BOOTED_EXCLAVEKIT;
...
/*
 * This returns AVAILABLE once EXCLAVES_BOOT_STAGE_EXCLAVECORE has completed.
 * This is to maintain backwards compatibility with existing code.
 */
...
const char *exclaves_get_boot_status_string(void) {
  exclaves_boot_status_t boot_status =
      os_atomic_load(&exclaves_boot_status, relaxed);
...
bool exclaves_boot_supported(void) { return false; }
