...
CA_EVENT(set_exception, CA_STATIC_STRING(CA_PROCNAME_LEN), current_proc,
         CA_STATIC_STRING(CA_PROCNAME_LEN), thread_proc, CA_INT, mask,
...
special_port_allowed_with_task_flavor(int which, mach_task_flavor_t flavor);
static kern_return_t
special_port_allowed_with_thread_flavor(int which, mach_thread_flavor_t flavor);
static void
ipc_port_bind_special_reply_port_locked(ipc_port_t port,
...
static ipc_port_t
convert_thread_to_port_with_flavor(thread_t, thread_ro_t,
                                   mach_thread_flavor_t flavor);
ipc_port_t convert_task_to_port_with_flavor(task_t task,
...
void ipc_task_init(task_t task, task_t parent) {
  ipc_space_t space;
  ipc_port_t kport;
  ipc_port_t nport;
  kern_return_t kr;
  ...
  nport =
      ipc_kobject_alloc_port(IKO_NULL, IKOT_TASK_NAME, IPC_KOBJECT_ALLOC_NONE);
  ...
  bzero(&task->hardened_exception_action,
        sizeof(task->hardened_exception_action));
  ...
  if (parent == TASK_NULL) {
    ...
    task->itk_bootstrap = ipc_port_copy_send_mqueue(parent->itk_bootstrap);

    task->itk_task_access = ipc_port_copy_send_mqueue(parent->itk_task_access);
 ...
 */
void ipc_task_copyout_control_port(task_t task) {
 ...
 */
void ipc_thread_set_immovable_pinned(thread_t thread) {
...
void ipc_task_enable(task_t task) {
  ...
  assert(task_is_a_corpse(task) ||
         task->map->owning_task == task); /* verify vm_map_setup called */
...
void ipc_task_disable(task_t task) {
  ipc_port_t kport;
  ipc_port_t nport;
  ipc_port_t iport;
  ipc_port_t rdport;
...
void ipc_task_terminate(task_t task) {
  ipc_port_t kport;
  ipc_port_t nport;
  ipc_port_t iport;
  ipc_port_t rdport;
  ...
  if (rdport != IP_NULL) {
    ipc_kobject_dealloc_port(rdport, IPC_KOBJECT_NO_MSCOUNT, IKOT_TASK_READ);
  }
  ipc_kobject_dealloc_port(kport, IPC_KOBJECT_NO_MSCOUNT, IKOT_TASK_CONTROL);

  /* destroy other kernel ports */
  ipc_kobject_dealloc_port(nport, IPC_KOBJECT_NO_MSCOUNT, IKOT_TASK_NAME);
  if (iport != IP_NULL) {
    ipc_kobject_dealloc_port(iport, IPC_KOBJECT_NO_MSCOUNT, IKOT_TASK_INSPECT);
  }
  if (rport != IP_NULL) {
    ipc_kobject_dealloc_port(rport, IPC_KOBJECT_NO_MSCOUNT, IKOT_TASK_RESUME);
  }
...
void ipc_task_reset(task_t task) {
...
void ipc_thread_init(__unused task_t task, thread_t thread, thread_ro_t tro) {
  ipc_port_t kport;
...
static void ipc_thread_init_exc_actions(thread_ro_t tro) {
  struct exception_action *actions;

  actions =
      kalloc_type(struct thread_init_exc_actions, Z_WAITOK | Z_ZERO | Z_NOFAIL)
          ->array;
...
static void ipc_thread_destroy_exc_actions(thread_ro_t tro) {
...
static void ipc_thread_ro_update_ports(thread_ro_t tro,
  ...
  zalloc_ro_mut(ZONE_ID_THREAD_RO, tro, offs, &tro_tpl->tro_ports, size);
}
 ...
 *		Thread locked.
 */
void ipc_thread_disable(thread_t thread) {
...
void ipc_thread_terminate(thread_t thread) {
  thread_ro_t tro = get_thread_ro(thread);
  ...
  /* clears read port ikol_alt_port, must be done first */
  if (rdport != IP_NULL) {
    ipc_kobject_dealloc_port(rdport, IPC_KOBJECT_NO_MSCOUNT, IKOT_THREAD_READ);
  }
...
void ipc_thread_reset(thread_t thread) {
  thread_ro_t tro = get_thread_ro(thread);
  ipc_port_t old_kport, new_kport;
#if CONFIG_CSR
  ipc_port_t old_sself;
...
static ipc_port_t retrieve_task_self_fast(task_t task) {
  ipc_port_t port = IP_NULL;
  ipc_port_t kport = task->itk_task_ports[TASK_FLAVOR_CONTROL];
 ...
 */
kern_return_t mach_task_is_self(task_t task, boolean_t *is_self) {
  if (task == TASK_NULL) {
...
ipc_port_t retrieve_thread_self_fast(thread_t thread) {
  thread_ro_t tro = get_thread_ro(thread);
  ipc_port_t port = IP_NULL;
...
#endif
  {
    port = ipc_kobject_make_send(tro->tro_ports[THREAD_FLAVOR_CONTROL], thread,
                                 IKOT_THREAD_CONTROL);
  }
...
mach_port_name_t task_self_trap(__unused struct task_self_trap_args *args) {
...
mach_port_name_t thread_self_trap(__unused struct thread_self_trap_args *args) {
...
mach_port_name_t mach_reply_port(__unused struct mach_reply_port_args *args) {
  ipc_port_t port;
  mach_port_name_t name;
  kern_return_t kr;

  kr = ipc_port_alloc(current_space(), IPC_OBJECT_LABEL(IOT_PORT), IP_INIT_NONE,
                      &name, &port);
  if (kr == KERN_SUCCESS) {
    ip_mq_unlock(port);
  } else {
 ...
 *		Nothing locked.
 *	Returns:
 *		mach_port_name_t: send right & receive right for special reply
 * port. MACH_PORT_NULL if there are any resource failures or other errors.
 */

mach_port_name_t thread_get_special_reply_port(
...
kern_return_t thread_get_kernel_special_reply_port(void) {
  ipc_port_t port = IP_NULL;
  thread_t thread = current_thread();
  ...
  port = ipc_port_alloc_special(
      ipc_space_reply, IPC_OBJECT_LABEL(IOT_SPECIAL_REPLY_PORT), IP_INIT_NONE);
...
ipc_port_unbind_special_reply_port(thread_t thread,
  ...
  *reply_portp = NULL;
  ipc_port_adjust_special_reply_port_locked(
      special_reply_port, NULL, IPC_PORT_ADJUST_UNLINK_THREAD, FALSE);
  /* port unlocked */
...
void thread_dealloc_kernel_special_reply_port(thread_t thread) {
  ipc_port_unbind_special_reply_port(thread, IRPT_KERNEL);
}
...
kern_return_t thread_get_special_port(thread_inspect_t thread, int which,
                                      ipc_port_t *portp);

static kern_return_t
thread_get_special_port_internal(thread_inspect_t thread, thread_ro_t tro,
                                 int which, ipc_port_t *portp,
                                 mach_thread_flavor_t flavor) {
  kern_return_t kr;
  ipc_port_t port;

  if ((kr = special_port_allowed_with_thread_flavor(which, flavor)) !=
      KERN_SUCCESS) {
    return kr;
  }
...
kern_return_t thread_get_special_port(thread_inspect_t thread, int which,
  ...
  return thread_get_special_port_internal(thread, get_thread_ro(thread), which,
                                          portp, THREAD_FLAVOR_CONTROL);
}

kern_return_t thread_get_special_port_from_user(mach_port_t port, int which,
                                                ipc_port_t *portp) {
  thread_ro_t tro;
...
static kern_return_t
special_port_allowed_with_thread_flavor(int which,
                                        mach_thread_flavor_t flavor) {
  switch (flavor) {
...
kern_return_t thread_set_special_port(thread_t thread, int which,
                                      ipc_port_t port) {
  ...
  /*
   * rdar://70585367
   * disallow immovable send so other process can't retrieve it through
  ...
  case THREAD_KERNEL_PORT:
    ...
    if (csr_check(CSR_ALLOW_KERNEL_DEBUGGER) == 0) {
      ...
      if (thread->active) {
        ...
        zalloc_ro_update_field(ZONE_ID_THREAD_RO, tro, tro_settable_self_port,
                               &port);
      } else {
        result = KERN_FAILURE;
      }
...
static kern_return_t task_get_special_port_internal(task_t task, int which,
                                                    ipc_port_t *portp,
                                                    mach_task_flavor_t flavor) {
  kern_return_t kr;
  ipc_port_t port;
  ...
  if ((kr = special_port_allowed_with_task_flavor(which, flavor)) !=
      KERN_SUCCESS) {
    return kr;
  }
  ...
  case TASK_INSPECT_PORT:
    ...
    task_reference(task);
    port =
        convert_task_to_port_with_flavor(task, current_flavor, TASK_GRP_KERNEL);
    break;

  case TASK_NAME_PORT:
    port = ipc_kobject_make_send(task->itk_task_ports[TASK_FLAVOR_NAME], task,
                                 IKOT_TASK_NAME);
    itk_unlock(task);
    break;
...
/* Kernel/Kext call only and skips MACF checks. MIG uses
 * task_get_special_port_from_user(). */
kern_return_t task_get_special_port(task_t task, int which, ipc_port_t *portp) {
  return task_get_special_port_internal(task, which, portp,
                                        TASK_FLAVOR_CONTROL);
}

/* MIG call only. Kernel/Kext uses task_get_special_port() */
kern_return_t task_get_special_port_from_user(mach_port_t port, int which,
                                              ipc_port_t *portp) {
  ipc_kobject_type_t kotype;
  mach_task_flavor_t flavor;
  kern_return_t kr = KERN_SUCCESS;
...
kern_return_t task_set_special_port_from_user(task_t task, int which,
                                              ipc_port_t port) {
  if (task == TASK_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
...
kern_return_t task_set_special_port_internal(task_t task, int which,
                                             ipc_port_t port) {
  ipc_port_t old = IP_NULL;
...
kern_return_t _kernelrpc_mach_ports_register3(task_t task, mach_port_t port1,
                                              mach_port_t port2,
                                              mach_port_t port3) {
...
kern_return_t _kernelrpc_mach_ports_lookup3(task_t task, ipc_port_t *port1,
                                            ipc_port_t *port2,
                                            ipc_port_t *port3) {
  if (task == TASK_NULL) {
    return KERN_INVALID_ARGUMENT;
                              ...
                              int flavor) /* control or read */
{
  ...
  if (!developer_mode_state()) {
    if ((caller != victim) && (flavor == TASK_FLAVOR_CONTROL) &&
        !task_is_a_corpse(victim)) {
      ...
      if (out_trans) {
        ...
        panic("Just like pineapple on pizza, this task/thread port doesn't "
              "belong here.");
      }
  ...
  /*
   * On platforms other than macOS, only a platform binary can resolve the task
   * port of another platform binary.
   */
...
kern_return_t task_conversion_eval(task_t caller, task_t victim, int flavor) {
...
static kern_return_t task_conversion_eval_out_trans(task_t caller,
                                                    task_t victim, int flavor) {
 ...
 */
static boolean_t task_port_kotype_valid_for_flavor(natural_t kotype,
                                                   mach_task_flavor_t flavor) {
  switch (flavor) {
 ...
 */
static task_t
convert_port_to_task_with_flavor_locked_noref(ipc_port_t port,
                                              mach_task_flavor_t flavor,
 ...
 */
static task_t convert_port_to_task_with_flavor_locked(
    ipc_port_t port, mach_task_flavor_t flavor, port_intrans_options_t options,
    task_grp_t grp) {
  task_t task;

  task = convert_port_to_task_with_flavor_locked_noref(port, flavor, options);
 ...
 *		Nothing locked.
 */
static task_t convert_port_to_task_with_flavor(ipc_port_t port,
                                               mach_task_flavor_t flavor,
                                               port_intrans_options_t options,
                                               task_grp_t grp) {
  ...
  if (IP_VALID(port)) {
    ...
    if (ip_active(port)) {
      task =
          convert_port_to_task_with_flavor_locked(port, flavor, options, grp);
    }
    ip_mq_unlock(port);
  }
...
task_t convert_port_to_task(ipc_port_t port) {
  return convert_port_to_task_with_flavor(
      port, TASK_FLAVOR_CONTROL, PORT_INTRANS_OPTIONS_NONE, TASK_GRP_KERNEL);
}

task_t convert_port_to_task_mig(ipc_port_t port) {
  return convert_port_to_task_with_flavor(
      port, TASK_FLAVOR_CONTROL, PORT_INTRANS_OPTIONS_NONE, TASK_GRP_MIG);
}

task_read_t convert_port_to_task_read(ipc_port_t port) {
  return convert_port_to_task_with_flavor(
      port, TASK_FLAVOR_READ, PORT_INTRANS_ALLOW_CORPSE_TASK, TASK_GRP_KERNEL);
}

task_read_t convert_port_to_task_read_no_eval(ipc_port_t port) {
  return convert_port_to_task_with_flavor(port, TASK_FLAVOR_READ,
                                          PORT_INTRANS_SKIP_TASK_EVAL |
                                              PORT_INTRANS_ALLOW_CORPSE_TASK,
                                          TASK_GRP_KERNEL);
}

task_read_t convert_port_to_task_read_mig(ipc_port_t port) {
  return convert_port_to_task_with_flavor(
      port, TASK_FLAVOR_READ, PORT_INTRANS_ALLOW_CORPSE_TASK, TASK_GRP_MIG);
}

task_inspect_t convert_port_to_task_inspect(ipc_port_t port) {
...
task_inspect_t convert_port_to_task_inspect_no_eval(ipc_port_t port) {
  return convert_port_to_task_with_flavor(port, TASK_FLAVOR_INSPECT,
                                          PORT_INTRANS_SKIP_TASK_EVAL |
                                              PORT_INTRANS_ALLOW_CORPSE_TASK,
                                          TASK_GRP_KERNEL);
...
task_inspect_t convert_port_to_task_inspect_mig(ipc_port_t port) {
  return convert_port_to_task_with_flavor(port, TASK_FLAVOR_INSPECT,
                                          PORT_INTRANS_SKIP_TASK_EVAL |
                                              PORT_INTRANS_ALLOW_CORPSE_TASK,
                                          TASK_GRP_MIG);
...
task_name_t convert_port_to_task_name(ipc_port_t port) {
...
task_name_t convert_port_to_task_name_mig(ipc_port_t port) {
  return convert_port_to_task_with_flavor(port, TASK_FLAVOR_NAME,
                                          PORT_INTRANS_SKIP_TASK_EVAL |
                                              PORT_INTRANS_ALLOW_CORPSE_TASK,
                                          TASK_GRP_MIG);
 ...
 *		Nothing locked.
 */
static task_t convert_port_to_task_policy_mig(ipc_port_t port, boolean_t set) {
  task_t task = TASK_NULL;
  ...
  task = set ? convert_port_to_task_mig(port)
...
task_policy_set_t convert_port_to_task_policy_set_mig(ipc_port_t port) {
  return convert_port_to_task_policy_mig(port, true);
}

task_policy_get_t convert_port_to_task_policy_get_mig(ipc_port_t port) {
...
convert_port_to_task_suspension_token_grp(ipc_port_t port, task_grp_t grp) {
...
convert_port_to_task_suspension_token_external(ipc_port_t port) {
  return convert_port_to_task_suspension_token_grp(port, TASK_GRP_EXTERNAL);
}

task_suspension_token_t
convert_port_to_task_suspension_token_mig(ipc_port_t port) {
  return convert_port_to_task_suspension_token_grp(port, TASK_GRP_MIG);
}

task_suspension_token_t
convert_port_to_task_suspension_token_kernel(ipc_port_t port) {
 ...
 */
static ipc_space_t
convert_port_to_space_with_flavor(ipc_port_t port, mach_task_flavor_t flavor,
                                  port_intrans_options_t options) {
  ...
  if (IP_VALID(port)) {
    ...
    if (ip_active(port)) {
      task =
          convert_port_to_task_with_flavor_locked_noref(port, flavor, options);
    }
...
ipc_space_t convert_port_to_space(ipc_port_t port) {
  return convert_port_to_space_with_flavor(port, TASK_FLAVOR_CONTROL,
                                           PORT_INTRANS_OPTIONS_NONE);
}

ipc_space_read_t convert_port_to_space_read(ipc_port_t port) {
  return convert_port_to_space_with_flavor(port, TASK_FLAVOR_READ,
                                           PORT_INTRANS_ALLOW_CORPSE_TASK);
}

ipc_space_read_t convert_port_to_space_read_no_eval(ipc_port_t port) {
  return convert_port_to_space_with_flavor(port, TASK_FLAVOR_READ,
                                           PORT_INTRANS_SKIP_TASK_EVAL |
                                               PORT_INTRANS_ALLOW_CORPSE_TASK);
}

ipc_space_inspect_t convert_port_to_space_inspect(ipc_port_t port) {
 ...
 */
static vm_map_t
convert_port_to_map_with_flavor(ipc_port_t port, mach_task_flavor_t flavor,
                                port_intrans_options_t options) {
  task_t task = TASK_NULL;
...
vm_map_t convert_port_to_map(ipc_port_t port) {
  return convert_port_to_map_with_flavor(port, TASK_FLAVOR_CONTROL,
                                         PORT_INTRANS_OPTIONS_NONE);
}

vm_map_read_t convert_port_to_map_read(ipc_port_t port) {
  return convert_port_to_map_with_flavor(port, TASK_FLAVOR_READ,
                                         PORT_INTRANS_ALLOW_CORPSE_TASK);
}

vm_map_inspect_t convert_port_to_map_inspect(__unused ipc_port_t port) {
  /* there is no vm_map_inspect_t routines at the moment. */
 ...
 */
static boolean_t
thread_port_kotype_valid_for_flavor(natural_t kotype,
                                    mach_thread_flavor_t flavor) {
  switch (flavor) {
  /* Ascending capability */
  case THREAD_FLAVOR_INSPECT:
 ...
 *	Purpose:
 *		Internal helper routine to convert from a locked port to a
 * thread. Args: port   - target port flavor - requested thread port flavor
 *		options - port translation options
 *	Conditions:
 *		Port is locked and active.
 ...
 */
static thread_t
convert_port_to_thread_with_flavor_locked(ipc_port_t port,
                                          mach_thread_flavor_t flavor,
                                          port_intrans_options_t options) {
 ...
 */
static thread_t
convert_port_to_thread_with_flavor(ipc_port_t port, mach_thread_flavor_t flavor,
                                   port_intrans_options_t options) {
  thread_t thread = THREAD_NULL;
...
thread_read_t convert_port_to_thread_read(ipc_port_t port) {
...
thread_inspect_t convert_port_to_thread_inspect(ipc_port_t port) {
...
convert_port_to_thread_inspect_no_eval(ipc_port_t port) {
  return convert_port_to_thread_with_flavor(port, THREAD_FLAVOR_INSPECT,
                                            PORT_INTRANS_SKIP_TASK_EVAL |
                                                PORT_INTRANS_ALLOW_CORPSE_TASK);
}
...
ipc_port_t convert_thread_to_port_immovable(thread_t thread) {
  thread_ro_t tro = get_thread_ro(thread);
  ipc_port_t port = IP_NULL;
 ...
 */
static ipc_port_t
convert_thread_to_port_with_flavor(thread_t thread, thread_ro_t tro,
                                   mach_thread_flavor_t flavor) {
  ...
  if (flavor == THREAD_FLAVOR_CONTROL) {
    ...
    if (!developer_mode_state() && flavor == THREAD_FLAVOR_READ) {
      label = ipc_kobject_label_alloc(kotype, IPC_LABEL_SUBST_THREAD_READ,
                                      tro->tro_ports[THREAD_FLAVOR_CONTROL]);
    }

    /*
     * Claim a send right on the thread read/inspect port, and request a
     * no-senders notification on that port (if none outstanding). A thread
     * reference is not donated here even though the ports are created lazily
     * because it doesn't own the kobject that it points to. Threads manage
     * their lifetime explicitly and have to synchronize with each other,
     * between the task/thread terminating and the send-once notification
     * firing, and this is done under the thread mutex rather than with atomics.
     */
    port = ipc_kobject_alloc_port(thread, label, IPC_KOBJECT_ALLOC_MAKE_SEND);

    zalloc_ro_update_field(ZONE_ID_THREAD_RO, tro, tro_ports[flavor], &port);
  }
...
void convert_thread_array_to_ports(thread_act_array_t array, size_t count,
                                   mach_thread_flavor_t flavor) {
 ...
 *		Nothing locked.
 */
thread_t port_name_to_thread(mach_port_name_t name,
                             port_intrans_options_t options) {
  thread_t thread = THREAD_NULL;
  ipc_port_t kport;
  kern_return_t kr;
  ...
  if (MACH_PORT_VALID(name)) {
    ...
    if (kr == KERN_SUCCESS) {
      ...
      thread = convert_port_to_thread_with_flavor_locked(
          kport, THREAD_FLAVOR_CONTROL, options);
      ip_mq_unlock(kport);
    }
  }
 ...
 *		Nothing locked.
 */
static bool port_name_is_pinned_self(task_t self, mach_port_name_t name) {
 ...
 */
static task_t
port_name_to_current_task_internal_noref(mach_port_name_t name,
                                         mach_task_flavor_t flavor) {
  ipc_port_t kport;
  kern_return_t kr;
  task_t task = TASK_NULL;
...
task_t port_name_to_current_task_noref(mach_port_name_t name) {
  return port_name_to_current_task_internal_noref(name, TASK_FLAVOR_CONTROL);
}

task_read_t port_name_to_current_task_read_noref(mach_port_name_t name) {
 ...
 *		Nothing locked.
 */
static task_t port_name_to_task_grp(mach_port_name_t name,
                                    mach_task_flavor_t flavor,
                                    port_intrans_options_t options,
                                    task_grp_t grp, ipc_port_t *kportp) {
  ipc_port_t kport;
  kern_return_t kr;
  task_t task = TASK_NULL;
  task_t self = current_task();
  ...
  if (MACH_PORT_VALID(name)) {
    ...
    if (kr == KERN_SUCCESS) {
      /* port is locked and active */
      task =
          convert_port_to_task_with_flavor_locked(kport, flavor, options, grp);
      if (kportp) {
...
task_t port_name_to_task_external(mach_port_name_t name) {
  return port_name_to_task_grp(name, TASK_FLAVOR_CONTROL,
                               PORT_INTRANS_OPTIONS_NONE, TASK_GRP_EXTERNAL,
                               NULL);
}

task_t port_name_to_task_kernel(mach_port_name_t name) {
  return port_name_to_task_grp(name, TASK_FLAVOR_CONTROL,
 ...
 *		Nothing locked.
 */
task_read_t port_name_to_task_read(mach_port_name_t name) {
  return port_name_to_task_grp(name, TASK_FLAVOR_READ,
 ...
 */
task_read_t port_name_to_task_read_and_send_right(mach_port_name_t name,
                                                  ipc_port_t *kportp) {
  return port_name_to_task_grp(name, TASK_FLAVOR_READ,
                               PORT_INTRANS_ALLOW_CORPSE_TASK, TASK_GRP_KERNEL,
                               kportp);
 ...
 *		Nothing locked.
 */
task_read_t port_name_to_task_read_no_eval(mach_port_name_t name) {
  port_intrans_options_t options =
      PORT_INTRANS_SKIP_TASK_EVAL | PORT_INTRANS_ALLOW_CORPSE_TASK;
  return port_name_to_task_grp(name, TASK_FLAVOR_READ, options, TASK_GRP_KERNEL,
                               NULL);
}
 ...
 *		Nothing locked.
 */
task_name_t port_name_to_task_name(mach_port_name_t name) {
  port_intrans_options_t options =
      PORT_INTRANS_SKIP_TASK_EVAL | PORT_INTRANS_ALLOW_CORPSE_TASK;
  return port_name_to_task_grp(name, TASK_FLAVOR_NAME, options, TASK_GRP_KERNEL,
                               NULL);
}
 ...
 *		Nothing locked.
 */
task_id_token_t port_name_to_task_id_token(mach_port_name_t name) {
  ipc_port_t port;
  kern_return_t kr;
 ...
 *		Nothing locked.
 */
host_t port_name_to_host(mach_port_name_t name) {
...
ipc_port_t convert_task_to_port_with_flavor(task_t task,
  ...
  case TASK_FLAVOR_CONTROL:
  case TASK_FLAVOR_NAME:
    port = ipc_kobject_make_send(task->itk_task_ports[flavor], task, kotype);
    break;
  /*
  ...
  case TASK_FLAVOR_INSPECT:
    ...
    if (IP_VALID(port)) {
      ...
      if (!developer_mode_state() && flavor == TASK_FLAVOR_READ) {
        label =
            ipc_kobject_label_alloc(kotype, IPC_LABEL_SUBST_TASK_READ,
                                    task->itk_task_ports[TASK_FLAVOR_CONTROL]);
      }

      port = ipc_kobject_alloc_port(task, label, IPC_KOBJECT_ALLOC_MAKE_SEND);
...
ipc_port_t convert_corpse_to_port_and_nsrequest(task_t corpse) {
  ipc_port_t port = IP_NULL;
...
ipc_port_t convert_task_read_to_port(task_read_t task) {
  return convert_task_to_port_with_flavor(task, TASK_FLAVOR_READ,
                                          TASK_GRP_KERNEL);
}

ipc_port_t convert_task_inspect_to_port(task_inspect_t task) {
  return convert_task_to_port_with_flavor(task, TASK_FLAVOR_INSPECT,
                                          TASK_GRP_KERNEL);
}

ipc_port_t convert_task_name_to_port(task_name_t task) {
  return convert_task_to_port_with_flavor(task, TASK_FLAVOR_NAME,
                                          TASK_GRP_KERNEL);
}

ipc_port_t convert_task_to_port_external(task_t task) {
  return convert_task_to_port_with_flavor(task, TASK_FLAVOR_CONTROL,
                                          TASK_GRP_EXTERNAL);
}

ipc_port_t convert_task_read_to_port_external(task_t task) {
  return convert_task_to_port_with_flavor(task, TASK_FLAVOR_READ,
                                          TASK_GRP_EXTERNAL);
}

void convert_task_array_to_ports(task_array_t array, size_t count,
                                 mach_task_flavor_t flavor) {
 ...
 *		Consumes a task suspension token ref; produces a naked send-once
 * right which may be invalid. Conditions: Nothing locked.
 */
static ipc_port_t
convert_task_suspension_token_to_port_grp(task_suspension_token_t task,
                                          task_grp_t grp) {
  ipc_port_t port;
  ...
  if (task->active) {
    ...
    if (task->itk_resume == IP_NULL) {
      task->itk_resume = ipc_kobject_alloc_port(
          (ipc_kobject_t)task, IKOT_TASK_RESUME, IPC_KOBJECT_ALLOC_NONE);
    }
...
ipc_port_t
convert_task_suspension_token_to_port_external(task_suspension_token_t task) {
  return convert_task_suspension_token_to_port_grp(task, TASK_GRP_EXTERNAL);
}

ipc_port_t
convert_task_suspension_token_to_port_mig(task_suspension_token_t task) {
...
void space_deallocate(ipc_space_t space) {
  if (space != IS_NULL) {
    is_release(space);
  }
}
 ...
 *	Purpose:
 *		Deallocate a space read ref produced by
 * convert_port_to_space_read. Conditions: Nothing locked.
 */

void space_read_deallocate(ipc_space_read_t space) {
 ...
 *	Purpose:
 *		Deallocate a space inspect ref produced by
 * convert_port_to_space_inspect. Conditions: Nothing locked.
 */

void space_inspect_deallocate(ipc_space_inspect_t space) {
  if (space != IS_INSPECT_NULL) {
    is_release((ipc_space_t)space);
  }
}

static boolean_t behavior_is_identity_protected(int new_behavior) {
  return ((new_behavior & ~MACH_EXCEPTION_MASK) ==
          EXCEPTION_IDENTITY_PROTECTED) ||
         ((new_behavior & ~MACH_EXCEPTION_MASK) == EXCEPTION_STATE) ||
         ((new_behavior & ~MACH_EXCEPTION_MASK) ==
          EXCEPTION_STATE_IDENTITY_PROTECTED);
}

static void send_set_exception_telemetry(const task_t excepting_task,
  ...
  task_procname(current_task(), (char *)&event->current_proc,
                sizeof(event->current_proc));
  task_procname(excepting_task, (char *)&event->thread_proc,
                sizeof(event->thread_proc));
  event->mask = mask;
...
static boolean_t set_exception_behavior_violation(const task_t excepting_task,
                                                  const exception_mask_t mask,
                                                  int new_behavior) {
  if (thid_should_crash) {
    /* create lightweight corpse */
    mach_port_guard_exception(new_behavior, mask,
...
static bool exception_exposes_protected_ports(const ipc_port_t new_port) {
  /*
   * sending exceptions to invalid port does not pose risk
   * ux_handler port is an immovable, read-only kobject port; doesn't need
   * protection.
   */
  return IP_VALID(new_port) && !is_ux_handler_port(new_port);
}

static bool exception_ports_frozen(task_t excepting_task) {
  return excepting_task &&
         (task_ro_flags_get(excepting_task) & TFRO_FREEZE_EXCEPTION_PORTS);
}

#if XNU_TARGET_OS_OSX && CONFIG_CSR
static bool SIP_is_enabled() {
  return csr_check(CSR_ALLOW_UNRESTRICTED_FS) != 0;
}
#endif /* XNU_TARGET_OS_OSX && CONFIG_CSR*/

static bool exception_is_identity_protected(const ipc_port_t new_port,
                                            int new_behavior,
                                            const task_t excepting_task,
                                            const exception_mask_t mask) {
...
static boolean_t set_exception_behavior_allowed(const ipc_port_t new_port,
                                                int new_behavior,
                                                const task_t excepting_task,
                                                const exception_mask_t mask,
                                                const bool hardened_exception) {
  ...
  /* Allow debuggers, tests, and tooling to set exception ports however they
   * wish */
  ...
  /* Everyone else follows the standard policy and must use identity protected
   * exceptions */
  return exception_is_identity_protected(new_port, new_behavior, excepting_task,
                                         mask);
}
 ...
 */
kern_return_t set_exception_ports_validation(task_t task,
  ...
  if (!set_exception_behavior_allowed(new_port, new_behavior, task,
                                      exception_mask, hardened_exception)) {
    return KERN_DENIED;
  }
 ...
 *	Conditions:
 *		Arguments have been validated via
 * `set_exception_ports_validation` Nothing locked. Returns: KERN_SUCCESS
 *	Setting the exception port is allowed with these arguments KERN_FAILURE
 *	Thread is inactive
 */
kern_return_t thread_set_exception_ports_internal(
    thread_t thread, exception_mask_t exception_mask, ipc_port_t new_port,
    exception_behavior_t new_behavior, thread_state_flavor_t new_flavor,
    boolean_t hardened) {
...
#if CONFIG_MACF
  if (mac_task_check_set_thread_exception_ports(
          current_task(), get_threadtask(thread), exception_mask,
          new_behavior) != 0) {
    return KERN_NO_ACCESS;
  }
...
kern_return_t thread_set_exception_ports(thread_t thread,
                                         exception_mask_t exception_mask,
                                         ipc_port_t new_port,
                                         exception_behavior_t new_behavior,
                                         thread_state_flavor_t new_flavor) {
  ...
  kern_return_t kr = set_exception_ports_validation(
      get_threadtask(thread), exception_mask, new_port, new_behavior,
      new_flavor, hardened_exception_flow);
  if (kr != KERN_SUCCESS) {
    return kr;
  }

  return thread_set_exception_ports_internal(thread, exception_mask, new_port,
                                             new_behavior, new_flavor, false);
}
...
kern_return_t task_set_exception_ports(task_t task,
  ...
  bool hardened_exception_flow = false;
  kern_return_t kr = set_exception_ports_validation(
      task, exception_mask, new_port, new_behavior, new_flavor,
      hardened_exception_flow);
  if (kr != KERN_SUCCESS) {
    return kr;
  }

#if CONFIG_MACF
  if (mac_task_check_set_task_exception_ports(
          current_task(), task, exception_mask, new_behavior) != 0) {
    return KERN_NO_ACCESS;
  }
...
kern_return_t thread_swap_exception_ports(
    thread_t thread, exception_mask_t exception_mask, ipc_port_t new_port,
    exception_behavior_t new_behavior, thread_state_flavor_t new_flavor,
    exception_mask_array_t masks, mach_msg_type_number_t *CountCnt,
    exception_port_array_t ports, exception_behavior_array_t behaviors,
  ...
  for (count = 0, i = FIRST_EXCEPTION; i < EXC_TYPES_COUNT && count < *CountCnt;
       ++i) {
    struct exception_action *action = &tro->tro_exc_actions[i];
    ...
    if ((exception_mask & (1 << i))
      ...
      for (j = 0; j < count; ++j) {
         ...
         */
        if (action->port == ports[j] && action->behavior == behaviors[j] &&
...
kern_return_t task_swap_exception_ports(
    task_t task, exception_mask_t exception_mask, ipc_port_t new_port,
    exception_behavior_t new_behavior, thread_state_flavor_t new_flavor,
    exception_mask_array_t masks, mach_msg_type_number_t *CountCnt,
    exception_port_array_t ports, exception_behavior_array_t behaviors,
    thread_state_flavor_array_t flavors) {
 ...
 *		KERN_FAILURE		The thread is dead.
 */
static kern_return_t thread_get_exception_ports_internal(
    thread_t thread, exception_mask_t exception_mask,
    exception_mask_array_t masks, mach_msg_type_number_t *CountCnt,
    exception_port_info_array_t ports_info, exception_port_array_t ports,
    exception_behavior_array_t behaviors, thread_state_flavor_array_t flavors) {
  unsigned int count;
  boolean_t info_only = (ports_info != NULL);
  thread_ro_t tro;
  ipc_port_t
      port_ptrs[EXC_TYPES_COUNT]; /* pointers only, does not hold right */
  ...
  for (int i = FIRST_EXCEPTION, j = 0; i < EXC_TYPES_COUNT; ++i) {
    if (exception_mask & (1 << i)) {
      ...
      for (j = 0; j < count; ++j) {
         ...
         * set corresponding mask for this exception.
         */
        if (exc_port == port_ptrs[j] && exc_behavior == behaviors[j] &&
      ...
      if (j == count && count < *CountCnt) {
        ...
        if (info_only) {
          if (!IP_VALID(exc_port)) {
            ports_info[j] = (ipc_info_port_t){.iip_port_object = 0,
                                              .iip_receiver_object = 0};
          } else {
            task_t receiver = TASK_NULL;
            (void)ipc_port_get_receiver_task(exc_port, &receiver);
            ports_info[j].iip_port_object =
                (natural_t)VM_KERNEL_ADDRHASH(exc_port);
            ports_info[j].iip_receiver_object =
                receiver ? (natural_t)VM_KERNEL_ADDRHASH(receiver) : 0;
          }
        } else {
...
kern_return_t thread_get_exception_ports(thread_t thread,
                                         exception_mask_t exception_mask,
                                         ...
                                         thread_state_flavor_array_t flavors) {
  return thread_get_exception_ports_internal(
      thread, exception_mask, masks, CountCnt, NULL, ports, behaviors, flavors);
}

kern_return_t thread_get_exception_ports_info(
    mach_port_t port, exception_mask_t exception_mask,
    exception_mask_array_t masks, mach_msg_type_number_t *CountCnt,
  ...
  kr = thread_get_exception_ports_internal(thread, exception_mask, masks,
                                           CountCnt, ports_info, NULL,
                                           behaviors, flavors);
...
kern_return_t thread_get_exception_ports_from_user(
  ...
  kr = thread_get_exception_ports(thread, exception_mask, masks, CountCnt,
...
static kern_return_t task_get_exception_ports_internal(
    task_t task, exception_mask_t exception_mask, exception_mask_array_t masks,
    mach_msg_type_number_t *CountCnt, exception_port_info_array_t ports_info,
    exception_port_array_t ports, exception_behavior_array_t behaviors,
    thread_state_flavor_array_t flavors) {
  unsigned int count;
  boolean_t info_only = (ports_info != NULL);
...
kern_return_t task_get_exception_ports(
    task_t task, exception_mask_t exception_mask, exception_mask_array_t masks,
    mach_msg_type_number_t *CountCnt, exception_port_array_t ports,
    exception_behavior_array_t behaviors, thread_state_flavor_array_t flavors) {
  return task_get_exception_ports_internal(
      task, exception_mask, masks, CountCnt, NULL, ports, behaviors, flavors);
}
...
kern_return_t task_get_exception_ports_from_user(
  ...
  kr = task_get_exception_ports(task, exception_mask, masks, CountCnt, ports,
                                behaviors, flavors);
 ...
 */
void ipc_thread_port_unpin(ipc_port_t port) {
...
kern_return_t task_register_hardened_exception_handler(
    task_t task, uint32_t signed_pc_key, exception_mask_t exceptions_allowed,
    exception_behavior_t behaviors_allowed,
    thread_state_flavor_t flavors_allowed, mach_port_t new_port) {
  ipc_port_t old_port;
  ...
  kern_return_t kr = set_exception_ports_validation(
      task, exceptions_allowed, new_port, behaviors_allowed, flavors_allowed,
      hardened_exception_flow);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
...
kern_return_t thread_adopt_exception_handler(
    thread_t thread, mach_port_t exc_port, exception_mask_t exc_mask,
    exception_behavior_t behavior_mask, thread_state_flavor_t flavor_mask) {
  if (thread == THREAD_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
   ...
   */
  if (exc_mask & ~hea.exception || behavior_mask & ~hea.ea.behavior ||
