...
kern_return_t
exclaves_conclave_launcher_init(uint64_t id,
  ...
  tb_endpoint_t conclave_control_endpoint = tb_endpoint_create_with_value(
      TB_TRANSPORT_TYPE_XNU, id, TB_ENDPOINT_OPTIONS_NONE);

  tb_result = conclave_launcher_conclavecontrol__init(
      &control, conclave_control_endpoint);
  if (tb_result != TB_ERROR_SUCCESS) {
    exclaves_debug_printf(show_errors, "conclave init failure: %llu\n", id);
    return KERN_FAILURE;
  }
...
exclaves_conclave_launcher_launch(const tb_client_connection_t connection) {
  ...
  tb_result = conclave_launcher_conclavecontrol_launch(
      &control, ^(conclave_launcher_conclavecontrol_launch__result_s result) {
        conclave_launcher_conclavestatus_s *status = NULL;
        status = conclave_launcher_conclavecontrol_launch__result_get_success(
            &result);
        if (status != NULL) {
          success = true;
          return;
        ...
        failure = conclave_launcher_conclavecontrol_launch__result_get_failure(
            &result);
        assert3p(failure, !=, NULL);
        exclaves_debug_printf(
            show_errors, "conclave launch failure: failure %u\n", *failure);
      });
...
exclaves_conclave_launcher_stop(const tb_client_connection_t connection,
  ...
  tb_result = conclave_launcher_conclavecontrol_stop(
      ...
      ^(conclave_launcher_conclavecontrol_stop__result_s result) {
        conclave_launcher_conclavestatus_s *status = NULL;
        status =
            conclave_launcher_conclavecontrol_stop__result_get_success(&result);
        if (status != NULL) {
          success = true;
          return;
        }
        ...
        conclave_launcher_conclavelauncherfailure_s *failure = NULL;
        failure =
            conclave_launcher_conclavecontrol_stop__result_get_failure(&result);
        assert3p(failure, !=, NULL);
...
exclaves_conclave_launcher_suspend(const tb_client_connection_t connection,
  ...
  tb_result = conclave_launcher_conclavecontrol_suspend(
      ...
      ^(conclave_launcher_conclavecontrol_suspend__result_s result) {
        conclave_launcher_conclavestatus_s *status = NULL;
        status = conclave_launcher_conclavecontrol_suspend__result_get_success(
            &result);
        if (status != NULL) {
          success = true;
          return;
        ...
        conclave_launcher_conclavelauncherfailure_s *failure = NULL;
        failure = conclave_launcher_conclavecontrol_suspend__result_get_failure(
            &result);
        assert3p(failure, !=, NULL);

        exclaves_debug_printf(
            show_errors, "conclave suspend failure: failure %u\n", *failure);
      });
  /* END IGNORE CODESTYLE */
...
tb_error_t exclaves_conclave_upcall_legacy_suspend(
  ...
  kern_return_t kret =
      task_suspend_conclave_upcall(scid_list.scid, ARRAY_COUNT(scid_list.scid));

  exclaves_debug_printf(
      show_lifecycle_upcalls,
  ...
  switch (kret) {
  case KERN_SUCCESS:
    xnuupcalls_xnuupcalls_conclave_suspend__result_init_success(&result,
                                                                scid_list);
    break;

  case KERN_INVALID_TASK:
  case KERN_INVALID_ARGUMENT:
    xnuupcalls_xnuupcalls_conclave_suspend__result_init_failure(
        &result, XNUUPCALLS_LIFECYCLEERROR_INVALIDTASK);
    break;

  default:
    xnuupcalls_xnuupcalls_conclave_suspend__result_init_failure(
        &result, XNUUPCALLS_LIFECYCLEERROR_FAILURE);
    break;
  }
...
tb_error_t exclaves_conclave_upcall_legacy_stop(
    const uint32_t flags,
  ...
  case KERN_INVALID_TASK:
  case KERN_INVALID_ARGUMENT:
    xnuupcalls_xnuupcalls_conclave_stop__result_init_failure(
        &result, XNUUPCALLS_LIFECYCLEERROR_INVALIDTASK);
    break;
...
tb_error_t exclaves_conclave_upcall_legacy_crash_info(
    const xnuupcalls_conclavesharedbuffer_s *shared_buf, const uint32_t length,
    tb_error_t (^completion)(
  ...
  const conclave_sharedbuffer_t *buf =
      (const conclave_sharedbuffer_t *)shared_buf;
  ...
  case KERN_INVALID_TASK:
  case KERN_INVALID_ARGUMENT:
    xnuupcalls_xnuupcalls_conclave_crash_info__result_init_failure(
        &result, XNUUPCALLS_LIFECYCLEERROR_INVALIDTASK);
    break;
...
tb_error_t exclaves_conclave_upcall_suspend(
    const uint32_t flags,
    tb_error_t (^completion)(
  ...
  switch (kret) {
  case KERN_SUCCESS:
    xnuupcallsv2_conclaveupcallsprivate_suspend__result_init_success(&result,
                                                                     scid_list);
    break;
...
tb_error_t exclaves_conclave_upcall_stop(
    const uint32_t flags,
    tb_error_t (^completion)(
  ...
  switch (kret) {
  case KERN_SUCCESS:
    xnuupcallsv2_conclaveupcallsprivate_stop__result_init_success(&result);
    break;
...
tb_error_t exclaves_conclave_upcall_crash_info(
  ...
  switch (kret) {
  case KERN_SUCCESS:
    xnuupcallsv2_conclaveupcallsprivate_crashinfo__result_init_success(&result);
    break;
