...
void ast_taken_user(void) {
  ...
  /*
   * AST_KEVENT does not send an IPI when setting the ast for a thread running
   * in parallel on a different processor. Only the ast bit on the thread will
   * be set.
   *
  ...
  if (preemption_reasons & AST_PREEMPT) {
    /* Conditions may have changed from when the AST_PREEMPT was originally set,
     * so re-check. */

    thread_lock(thread);
    preemption_reasons = csw_check(thread, current_processor(),
                                   (preemption_reasons & AST_QUANTUM));
 ...
 * Called at splsched
 */
ast_t ast_consume(ast_t reasons) {
  ast_t *pending_ast = ast_pending();
 ...
 * Called at splsched
 */
void ast_context(thread_t thread) {
  ast_t *pending_ast = ast_pending();
 ...
 * Called at splsched
 */
void ast_propagate(thread_t thread) { ast_on(thread_ast_get(thread)); }

void ast_dtrace_on(void) { ast_on(AST_DTRACE); }
