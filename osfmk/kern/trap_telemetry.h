...
typedef struct {
  uint8_t
       ...
       *
       * This should be set for traps which may fire very often in order to
       * avoid overwhelming Core Analytics.
       *
...
__enum_decl(
    trap_telemetry_type_t, uint32_t,
    {
        /* These show up in telemetry, do not renumber */
        TRAP_TELEMETRY_TYPE_KERNEL_BRK_KASAN =
            0, /* <unrecoverable> KASan violation traps */
        TRAP_TELEMETRY_TYPE_KERNEL_BRK_PTRAUTH =
            1, /* <unrecoverable> Pointer Auth failure traps */
        TRAP_TELEMETRY_TYPE_KERNEL_BRK_CLANG =
            2, /* <unrecoverable> Clang sanitizer traps */
        TRAP_TELEMETRY_TYPE_KERNEL_BRK_LIBCXX =
            3, /* <unrecoverable> Libc++ abort trap*/
        TRAP_TELEMETRY_TYPE_KERNEL_BRK_TELEMETRY =
            4, /* <  recoverable> Soft telemetry collection traps */
        TRAP_TELEMETRY_TYPE_KERNEL_BRK_XNU =
            5, /* <??recoverable> XNU defined traps */
 ...
 */
extern bool trap_telemetry_report_exception(trap_telemetry_type_t trap_type,
                                            uint64_t trap_code,
 ...
 * Returns true if the event was submitted (or duped) and false on error.
 */
extern bool trap_telemetry_report_simulated_trap_with_backtrace(
...
extern void trap_telemetry_init(void);
...
#define BRK_TELEMETRY_OPTIONS_FATAL_DEFAULT                                    \
  (brk_telemetry_options_s) {                                                  \
    .recoverable = false,                                                      \
    .enable_trap_telemetry = false, /* ignored when trap telemetry disabled,   \
                                       but zero init anyways */                \
        .telemetry_options = {0},                                              \
  }
...
#define BRK_TELEMETRY_OPTIONS_RECOVERABLE_DEFAULT(enable_telemetry)            \
  (brk_telemetry_options_s) {                                                  \
    .recoverable = true, .enable_trap_telemetry = (enable_telemetry),          \
    .telemetry_options = {                                                     \
        .telemetry_ca_event =                                                  \
            TRAP_TELEMETRY_CA_EVENT_KERNEL_BRK, /* Different backtraces may be \
                                                   useful, so report all */    \
...
extern struct kernel_brk_descriptor
    kernel_brk_descriptors[] __SECTION_START_SYM("__DATA_CONST",
                                                 "__kern_brk_desc");

extern struct kernel_brk_descriptor
    kernel_brk_descriptors_end[] __SECTION_END_SYM("__DATA_CONST",
                                                   "__kern_brk_desc");
...
find_kernel_brk_descriptor_by_comment(uint16_t comment) {
  for (kernel_brk_descriptor_t des = kernel_brk_descriptors;
       des < kernel_brk_descriptors_end; des++) {
 ...
 */
__unused __attribute__((always_inline)) static void
trap_telemetry_report_kernel_soft_error(
    trap_telemetry_kernel_soft_error_code_t code, bool report_once_per_site) {
  trap_telemetry_report_simulated_trap(
      TRAP_TELEMETRY_TYPE_KERNEL_SOFT_ERROR, (uint64_t)code,
      (trap_telemetry_options_s){.telemetry_ca_event =
                                     TRAP_TELEMETRY_CA_EVENT_INTERNAL,
