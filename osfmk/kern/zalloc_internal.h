 ...
 */
static inline uint32_t Z_FAST_QUO(uint64_t offs, uint64_t magic) {
 ...
 * and magic = Z_MAGIC_QUO(size)
 */
static inline uint32_t Z_FAST_MOD(uint64_t offs, uint64_t magic,
                                  uint64_t size) {
 ...
 */
static inline bool Z_FAST_ALIGNED(uint64_t offs, uint32_t magic) {
...
struct zone {
  ...
  uint32_t                   /* 32 bits */
      ...
      no_callout : 1, z_destructible : 1, /* zone can be zdestroy()ed  */
      ...
      z_uses_tags : 1, z_log_on : 1, /* zone logging was enabled by boot-arg */
   ...
   */
  struct zone_cache *
      __zpercpu OS_PTRAUTH_SIGNED_PTR("zone.z_pcpu_cache") z_pcpu_cache;
...
__options_decl(
    kalloc_type_options_t, uint64_t,
    {
        /*
...
__enum_decl(kt_var_heap_id_t, uint32_t,
            {
                /*
                 * Fake "data" heap used to link views of data-only allocation
                 * that have been redirected to KHEAP_DATA_BUFFERS
                 */
                KT_VAR_DATA_HEAP,
                /*
                 * Fake "data" heap used to link views of data-only allocation
                 * that have been redirected to KHEAP_DATA_SHARED
                 */
...
__abortlike extern void zone_invalid_panic(zone_t zone);

__pure2 static inline zone_id_t zone_index(zone_t z) {
  unsigned long delta;
  uint64_t quo;
...
__pure2 static inline bool zone_is_ro(zone_t zone) {
  return zone >= &zone_array[ZONE_ID__FIRST_RO] &&
         zone <= &zone_array[ZONE_ID__LAST_RO];
}

static inline bool zone_addr_size_crosses_page(mach_vm_address_t addr,
                                               mach_vm_size_t size) {
  return atop(addr ^ (addr + size - 1)) != 0;
}

__pure2 static inline uint16_t zone_elem_redzone(zone_t zone) {
#if KASAN_CLASSIC
...
__pure2 static inline uint16_t zone_elem_inner_offs(zone_t zone) {
  return zone->z_elem_offs;
}

__pure2 static inline uint16_t zone_elem_outer_offs(zone_t zone) {
  return zone_elem_inner_offs(zone) - zone_elem_redzone(zone);
}

__pure2 static inline vm_offset_t zone_elem_inner_size(zone_t zone) {
...
__pure2 static inline zone_security_flags_t zone_security_config(zone_t z) {
  zone_id_t zid = zone_index(z);
  return zone_security_array[zid];
}

static inline uint32_t zone_count_free(zone_t zone) {
...
static inline vm_size_t zone_scale_for_percpu(zone_t zone, vm_size_t size) {
  if (zone->z_percpu) {
    size *= zpercpu_count();
  }
  return size;
...
static inline vm_size_t zone_size_wired(zone_t zone) {
  /*
...
static inline vm_size_t zone_size_free(zone_t zone) {
  return zone_scale_for_percpu(zone, zone_elem_inner_size(zone) *
                                         zone_count_free(zone));
}

/* Under KASAN builds, this also accounts for quarantined elements. */
static inline vm_size_t zone_size_allocated(zone_t zone) {
  return zone_scale_for_percpu(zone, zone_elem_inner_size(zone) *
                                         zone_count_allocated(zone));
}

static inline vm_size_t zone_size_wasted(zone_t zone) {
  return zone_size_wired(zone) -
         zone_scale_for_percpu(zone, zone_elem_outer_size(zone) *
                                         zone->z_elems_avail);
}

__pure2 static inline bool zone_exhaustible(zone_t zone) {
  return zone->z_wired_max != ~0u;
}

__pure2 static inline bool zone_exhausted(zone_t zone) {
 ...
 */
static inline vm_size_t zone_stats_get_mem_allocated(zone_stats_t stats) {
  return stats->zs_mem_allocated;
}

/*
 * For sysctl kern.zones_collectable_bytes used by memory_maintenance to check
 * if a userspace reboot is needed. The only other way to query for this
 * information is via mach_memory_info() which is unavailable on release
 * kernels.
 */
 ...
 */
__startup_func extern vm_offset_t zone_early_mem_init(vm_size_t size);
 ...
 * element size.
 */
__startup_func extern vm_size_t
zone_get_early_alloc_size(const char *name __unused, vm_size_t elem_size,
                          zone_create_flags_t flags, vm_size_t min_elems);
 ...
 */
__startup_func extern void zone_cram_early(zone_t zone, vm_offset_t newmem,
                                           vm_size_t size);

extern bool zone_maps_owned(vm_address_t addr, vm_size_t size);

#if KASAN_LIGHT
extern bool kasan_zone_maps_owned(vm_address_t addr, vm_size_t size);
#endif /* KASAN_LIGHT */

extern void zone_map_sizes(vm_map_size_t *psize, vm_map_size_t *pfree,
                           vm_map_size_t *plargest_free);

extern bool zone_map_nearing_exhaustion(void);

static inline vm_tag_t zalloc_flags_get_tag(zalloc_flags_t flags) {
  return (vm_tag_t)((flags & Z_VM_TAG_MASK) >> Z_VM_TAG_SHIFT);
}

extern struct kalloc_result zalloc_ext(zone_t zone, zone_stats_t zstats,
...
extern void zfree_ext(zone_t zone, zone_stats_t zstats, void *addr,
                      uint64_t combined_size);

extern zone_id_t zone_id_for_element(void *addr, vm_size_t esize);

#if ZSECURITY_CONFIG(PGZ_OOB_ADJUST)
extern void *zone_element_pgz_oob_adjust(void *addr, vm_size_t req_size,
                                         vm_size_t elem_size);
#endif /* !ZSECURITY_CONFIG(PGZ_OOB_ADJUST) */

extern void zone_element_bounds_check(vm_address_t addr, vm_size_t len);

extern vm_size_t zone_element_size(void *addr, zone_t *z, bool clear_oob,
 ...
 */
extern bool zone_spans_ro_va(vm_offset_t addr_start, vm_offset_t addr_end);
 ...
 */
extern bool zone_owns(zone_t zone, void *addr);
 ...
 */
__pure2 extern vm_map_t zone_submap(zone_security_flags_t zsflags);
...
extern uint16_t zone_index_from_tag_index(uint32_t tag_zone_index);

#endif /* VM_TAG_SIZECLASSES */

extern lck_grp_t zone_locks_grp;

static inline void zone_lock(zone_t zone) {
#if KASAN_FAKESTACK
  spl_t s = 0;
...
static inline void zone_unlock(zone_t zone) {
#if KASAN_FAKESTACK
