 ...
 */
__startup_func void kext_alloc_init(void) {
#if CONFIG_KEXT_BASEMENT
  ...
  text_start = vm_map_trunc_page(text->vmaddr, VM_MAP_PAGE_MASK(kernel_map));
  ...
  g_kext_map =
      kmem_suballoc(kernel_map, &kext_alloc_base, kext_alloc_size,
                    VM_MAP_CREATE_PAGEABLE, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,
                    KMS_PERMANENT | KMS_NOFAIL, VM_KERN_MEMORY_KEXT)
          .kmr_submap;
  ...
  printf("kext submap [0x%lx - 0x%lx], kernel text [0x%lx - 0x%lx]\n",
         VM_KERNEL_UNSLIDE(kext_alloc_base), VM_KERNEL_UNSLIDE(kext_alloc_max),
...
vm_offset_t get_address_from_kext_map(vm_size_t fsize) {
  ...
  addr = vm_map_trunc_page(addr, VM_MAP_PAGE_MASK(g_kext_map));
  return addr;
}

kern_return_t kext_alloc(vm_offset_t *_addr, vm_size_t size, boolean_t fixed) {
   ...
   */
  rval = mach_vm_map_kernel(g_kext_map, &addr, size, 0, vmk_flags,
                            MACH_PORT_NULL, 0, TRUE, VM_PROT_DEFAULT,
...
kern_return_t kext_receipt(void **addrp, size_t *sizep) {
  ...
  size_t size;
  void *addr = getsectdatafromheader(kc, kReceiptInfoSegment,
                                     kAuxKCReceiptSection, &size);
  if (addr == NULL) {
    ret = KERN_INVALID_KC;
    goto finish;
  }
 ...
 */
kern_return_t kext_receipt_set_queried() { return OSKextSetReceiptQueried(); }
