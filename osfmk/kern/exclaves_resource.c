 ...
 */
static bitmap_t kernel_service_bitmap[BITMAP_LEN(CONCLAVE_SERVICE_MAX)] = {0};
 ...
 */
static queue_head_t *get_bucket(table_t *table, const void *key,
                                size_t key_len) {
  const uint32_t idx =
      os_hash_jenkins(key, key_len) & (table->t_buckets_count - 1);
 ...
 */
static void table_put(table_t *table, const void *key, size_t key_len,
                      table_item_t *item) {
 ...
 */
static void table_get(table_t *table, const void *key, size_t key_len,
                      bool (^cb)(void *)) {
 ...
 */
static table_t *table_alloc(size_t nbuckets) {
  assert3u(nbuckets, >, 0);
  ...
  table->t_buckets =
      kalloc_type(queue_head_t, nbuckets, Z_WAITOK | Z_ZERO | Z_NOFAIL);

  return table;
}

static void table_iterate(table_t *table,
                          bool (^cb)(const void *key, size_t key_len,
                                     void *value)) {
  for (size_t i = 0; i < table->t_buckets_count; i++) {
...
static exclaves_resource_domain_t *lookup_domain(const char *domain_name) {
...
static void iterate_domains(bool (^cb)(exclaves_resource_domain_t *)) {
  table_iterate(&root_table, ^(__unused const void *key,
                               __unused size_t key_len, void *value) {
    exclaves_resource_domain_t *domain = value;
    return cb(domain);
  });
}

static void iterate_resources(exclaves_resource_domain_t *domain,
                              bool (^cb)(exclaves_resource_t *)) {
  table_iterate(domain->d_table_name, ^(__unused const void *key,
                                        __unused size_t key_len, void *value) {
...
exclaves_resource_domain_alloc(const char *scope) {
  ...
  exclaves_resource_domain_t *domain =
      kalloc_type(exclaves_resource_domain_t, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  (void)strlcpy(domain->d_name, scope, sizeof(domain->d_name));
  ...
  table_item_t *item = kalloc_type(table_item_t, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static void exclaves_resource_insert_name_table(
    xnuproxy_resourcetype_s type __unused, const char *name,
    exclaves_resource_domain_t *domain, exclaves_resource_t *resource) {
  table_item_t *name_item =
      kalloc_type(table_item_t, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static void exclaves_resource_insert_id_table(xnuproxy_resourcetype_s type,
  ...
  case XNUPROXY_RESOURCETYPE_NOTIFICATION: {
     ...
     */
    exclaves_resource_domain_t *kernel_domain =
        lookup_domain(EXCLAVES_DOMAIN_KERNEL);
...
static exclaves_resource_t *
exclaves_resource_alloc(xnuproxy_resourcetype_s type, const char *name,
                        uint64_t id, exclaves_resource_domain_t *domain,
                        bool connected) {
  ...
  exclaves_resource_t *resource =
      kalloc_type(exclaves_resource_t, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
IPC_KOBJECT_DEFINE(IKOT_EXCLAVES_RESOURCE, .iko_op_movable_send = true,
                   .iko_op_stable = true,
...
static void populate_conclave_services(void) {
  ...
  iterate_domains(^(exclaves_resource_domain_t *domain) {
    ...
    exclaves_resource_t *cm =
        exclaves_resource_lookup_by_name(EXCLAVES_DOMAIN_KERNEL, domain->d_name,
    ...
    iterate_resources(domain, ^(exclaves_resource_t *resource) {
      ...
      if (is_kernel_domain) {
        bitmap_set(kernel_service_bitmap, (uint32_t)resource->r_id);
        return (bool)false;
      }
...
static void populate_aoeservice_to_conclave(void) {
  ...
  iterate_domains(^(exclaves_resource_domain_t *domain) {
    ...
    iterate_resources(domain, ^(exclaves_resource_t *resource) {
      ...
      table_put(&aoe_service_table, &resource->r_id, sizeof(resource->r_id),
                item);
 ...
 * we go.
 */
kern_return_t exclaves_resource_init(void) {
  /* Initialize the root table. */
  table_init(&root_table);

  /* BEGIN IGNORE CODESTYLE */
  kern_return_t kr = exclaves_xnuproxy_resource_info(
      ^(const char *name, const char *scope, xnuproxy_resourcetype_s type,
        uint64_t id, bool connected) {
        /*
        ...
        exclaves_resource_t *resource =
            exclaves_resource_alloc(type, name, id, domain, connected);
...
const char *exclaves_resource_name(const exclaves_resource_t *resource) {
 ...
 */
uint32_t exclaves_resource_retain(exclaves_resource_t *resource) {
  uint32_t orig = os_atomic_inc_orig(&resource->r_usecnt, relaxed);
  assert3u(orig, <, UINT32_MAX);

  return orig;
}

void exclaves_resource_release(exclaves_resource_t *resource) {
  /*
...
kern_return_t exclaves_resource_from_port_name(ipc_space_t space,
  ...
  exclaves_resource_t *resource =
      ipc_kobject_get_stable(port, IKOT_EXCLAVES_RESOURCE);
 ...
 */
kern_return_t exclaves_resource_create_port_name(exclaves_resource_t *resource,
                                                 ipc_space_t space,
                                                 mach_port_name_t *name) {
   ...
   */
  if (!ipc_kobject_make_send_lazy_alloc_port(&resource->r_port, resource,
                                             IKOT_EXCLAVES_RESOURCE)) {
...
static void exclaves_conclave_init(exclaves_resource_t *resource) {
  ...
  __assert_only kern_return_t kr =
      exclaves_conclave_launcher_init(resource->r_id, &connection);
...
kern_return_t exclaves_conclave_attach(const char *name, task_t task) {
...
kern_return_t exclaves_conclave_detach(exclaves_resource_t *resource,
                                       task_t task) {
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_CONCLAVEMANAGER);
  ...
  while (conclave->c_active_downcall) {
    ...
    lck_mtx_sleep_with_inheritor(&resource->r_mutex, LCK_SLEEP_DEFAULT,
                                 (event_t)&conclave->c_active_downcall,
                                 conclave->c_downcall_thread, THREAD_UNINT,
...
kern_return_t exclaves_conclave_inherit(exclaves_resource_t *resource,
                                        task_t old_task, task_t new_task) {
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_CONCLAVEMANAGER);
  ...
  if (conclave->c_state != CONCLAVE_S_ATTACHED || conclave->c_active_downcall ||
...
bool exclaves_conclave_is_attached(const exclaves_resource_t *resource) {
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_CONCLAVEMANAGER);
...
kern_return_t exclaves_conclave_launch(exclaves_resource_t *resource) {
  kern_return_t kr;
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_CONCLAVEMANAGER);
  ...
  if (exclaves_boot_wait(EXCLAVES_BOOT_STAGE_EXCLAVEKIT) != KERN_SUCCESS) {
    ...
    if (exclaves_requirement_is_relaxed(EXCLAVES_R_EXCLAVEKIT) ||
      ...
      exclaves_debug_printf(show_errors,
                            "exclaves: requirement was relaxed, ignoring "
                            "error: failed to boot to exclavekit\n");
    } else {
...
bool exclaves_is_forwarding_resource(exclaves_resource_t *resource) {
...
void exclaves_conclave_prepare_teardown(task_t task __unused) {
...
exclaves_update_state_machine_locked(exclaves_resource_t *resource) {
  ...
  while (1) {
    ...
    if (conclave->c_request & CONCLAVE_R_LAUNCH_REQUESTED) {
      ...
      ret = exclaves_conclave_launcher_suspend(conclave->c_control, suspend);
      ...
      ret = exclaves_conclave_launcher_stop(
          conclave->c_control, CONCLAVE_LAUNCHER_CONCLAVESTOPREASON_EXIT);
      assert3u(ret, ==, KERN_SUCCESS);
    } else {
      lck_mtx_unlock(&resource->r_mutex);
      break;
    ...
    if (conclave->c_active_detach) {
      wakeup_all_with_inheritor((event_t)&conclave->c_active_downcall,
                                THREAD_AWAKENED);
    }
 ...
 */
const char *exclaves_conclave_get_domain(exclaves_resource_t *resource) {
  if (resource != NULL) {
    assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_CONCLAVEMANAGER);
    return resource->r_name;
  }
...
kern_return_t exclaves_conclave_stop(exclaves_resource_t *resource,
                                     bool gather_crash_bt __unused) {
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_CONCLAVEMANAGER);
...
kern_return_t exclaves_conclave_suspend(exclaves_resource_t *resource) {
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_CONCLAVEMANAGER);
...
kern_return_t exclaves_conclave_resume(exclaves_resource_t *resource) {
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_CONCLAVEMANAGER);
...
kern_return_t exclaves_conclave_stop_upcall(exclaves_resource_t *resource) {
...
bool exclaves_conclave_has_service(exclaves_resource_t *resource, uint64_t id) {
...
kern_return_t exclaves_resource_sensor_open(const char *domain,
                                            const char *id_name,
                                            exclaves_resource_t **out) {
  assert3p(out, !=, NULL);

  exclaves_resource_t *sensor = exclaves_resource_lookup_by_name(
      domain, id_name, XNUPROXY_RESOURCETYPE_SENSOR);
...
static int
filt_exclaves_notification_attach(struct knote *kn,
                                  __unused struct kevent_qos_s *kev) {
  int error = 0;
  exclaves_resource_t *exclaves_resource = NULL;
  kern_return_t kr = exclaves_resource_from_port_name(
...
static void filt_exclaves_notification_detach(struct knote *kn) {
...
static int filt_exclaves_notification_event(struct knote *kn, long hint) {
  /* ALWAYS CALLED WITH exclaves_resource mutex held */
  exclaves_resource_t *exclaves_resource __assert_only =
      kn->kn_exclaves_resource;
...
static int filt_exclaves_notification_touch(struct knote *kn,
...
static int filt_exclaves_notification_process(struct knote *kn,
                                              struct kevent_qos_s *kev) {
  int result = 0;
  exclaves_resource_t *exclaves_resource = kn->kn_exclaves_resource;
  assert3p(exclaves_resource, !=, NULL);
...
kern_return_t exclaves_notification_create(const char *domain, const char *name,
                                           exclaves_resource_t **out) {
  assert3p(out, !=, NULL);

  exclaves_resource_t *resource = exclaves_resource_lookup_by_name(
      domain, name, XNUPROXY_RESOURCETYPE_NOTIFICATION);
...
kern_return_t
exclaves_notification_signal(exclaves_resource_t *exclaves_resource,
                             long event_mask) {
  assert3p(exclaves_resource, !=, NULL);
  assert3u(exclaves_resource->r_type, ==, XNUPROXY_RESOURCETYPE_NOTIFICATION);
...
uint64_t exclaves_service_lookup(const char *domain, const char *name) {
  assert3p(domain, !=, NULL);
  assert3p(name, !=, NULL);

  exclaves_resource_t *resource = exclaves_resource_lookup_by_name(
      domain, name, XNUPROXY_RESOURCETYPE_SERVICE);
  if (resource == NULL) {
...
kern_return_t exclaves_resource_shared_memory_copyin(
    exclaves_resource_t *resource, user_addr_t buffer, mach_vm_size_t size1,
    mach_vm_size_t offset1, mach_vm_size_t size2, mach_vm_size_t offset2) {
  assert3u(os_atomic_load(&resource->r_usecnt, relaxed), >, 0);
...
kern_return_t exclaves_resource_shared_memory_copyout(
    exclaves_resource_t *resource, user_addr_t buffer, mach_vm_size_t size1,
    mach_vm_size_t offset1, mach_vm_size_t size2, mach_vm_size_t offset2) {
  assert3u(os_atomic_load(&resource->r_usecnt, relaxed), >, 0);
...
static uint32_t audio_memory_get_endpoint(exclaves_resource_t *resource) {
...
static uint32_t audio_memory_get_buffer_id(exclaves_resource_t *resource) {
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_ARBITRATEDAUDIOMEMORY);
  return resource->r_id >> 32;
}

static kern_return_t shared_memory_map(exclaves_resource_t *resource,
                                       size_t size,
  ...
  const sharedmemorybase_perms_s sm_perm =
      perm == EXCLAVES_BUFFER_PERM_WRITE ? SHAREDMEMORYBASE_PERMS_READWRITE
                                         : SHAREDMEMORYBASE_PERMS_READONLY;
  sharedmemorybase_mapping_s mapping = 0;
  kr = exclaves_shared_memory_setup(&sm->sm_client, sm_perm, 0, page_count,
                                    &mapping);
  if (kr != KERN_SUCCESS) {
    lck_mtx_unlock(&resource->r_mutex);
    return kr;
   ...
   */
  uint32_t *pages =
      kalloc_type(uint32_t, page_count, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  __block uint32_t idx = 0;
  /* BEGIN IGNORE CODESTYLE */
  kr = exclaves_shared_memory_iterate(&sm->sm_client, &mapping, 0, page_count,
                                      ^(uint64_t pa) {
...
kern_return_t exclaves_resource_shared_memory_map(const char *domain,
  ...
  exclaves_resource_t *resource = exclaves_resource_lookup_by_name(
      domain, name, XNUPROXY_RESOURCETYPE_SHAREDMEMORY);
  if (resource == NULL) {
    return KERN_NOT_FOUND;
  }
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_SHAREDMEMORY);
...
exclaves_resource_shared_memory_unmap(exclaves_resource_t *resource) {
  ...
  kern_return_t kr =
      exclaves_shared_memory_teardown(&sm->sm_client, &sm->sm_mapping);
  if (kr != KERN_SUCCESS) {
    exclaves_debug_printf(show_errors,
...
char *exclaves_resource_shared_memory_get_buffer(exclaves_resource_t *resource,
                                                 size_t *buffer_len) {
  assert3u(os_atomic_load(&resource->r_usecnt, relaxed), >, 0);
  assert(resource->r_type == XNUPROXY_RESOURCETYPE_SHAREDMEMORY ||
         resource->r_type == XNUPROXY_RESOURCETYPE_ARBITRATEDAUDIOMEMORY);
...
kern_return_t exclaves_resource_audio_memory_map(const char *domain,
                                                 const char *name, size_t size,
                                                 exclaves_resource_t **out) {
  assert3p(out, !=, NULL);

  exclaves_resource_t *resource = exclaves_resource_lookup_by_name(
      domain, name, XNUPROXY_RESOURCETYPE_ARBITRATEDAUDIOMEMORY);
  if (resource == NULL) {
    return KERN_NOT_FOUND;
  }
  assert3u(resource->r_type, ==, XNUPROXY_RESOURCETYPE_ARBITRATEDAUDIOMEMORY);
  ...
  kern_return_t kr =
      shared_memory_map(resource, size, EXCLAVES_BUFFER_PERM_READ);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
...
kern_return_t exclaves_resource_audio_memory_copyout(
    exclaves_resource_t *resource, user_addr_t buffer, mach_vm_size_t size1,
    mach_vm_size_t offset1, mach_vm_size_t size2, mach_vm_size_t offset2,
    user_addr_t ustatus) {
  assert3u(os_atomic_load(&resource->r_usecnt, relaxed), >, 0);
  ...
  kr = exclaves_resource_shared_memory_copyout(resource, buffer, size1, offset1,
                                               size2, offset2);
  if (kr != KERN_SUCCESS) {
    return kr;
  }

  if (ustatus != 0 && copyout(&status, ustatus, sizeof(status)) != 0) {
    return KERN_FAILURE;
  }
...
void exclaves_resource_aoeservice_iterate(const char *domain_name,
                                          bool (^cb)(exclaves_resource_t *)) {
  assert3u(strlen(domain_name), >, 0);
