...
static const size_t _telemetry_sample_size_static =
    sizeof(struct micro_snapshot) + sizeof(struct task_snapshot) +
    sizeof(struct thread_snapshot);

static void
telemetry_instrumentation_begin(struct micro_snapshot_buffer *buffer,
                                enum micro_snapshot_flags flags);

static void telemetry_instrumentation_end(struct micro_snapshot_buffer *buffer);

static void telemetry_take_sample(thread_t thread,
                                  enum micro_snapshot_flags flags);

#if HAS_PMI_MICROSTACKSHOTS
static void _telemetry_take_sample_kernel(thread_t thread,
                                          enum micro_snapshot_flags flags);
static void _telemetry_mark_curthread(bool interrupted_userspace);
#endif /* HAS_PMI_MICROSTACKSHOTS */

#if CONFIG_MACF
static void telemetry_macf_take_sample(thread_t thread,
                                       enum micro_snapshot_flags flags);
#endif
...
static int telemetry_process_sample(const struct telemetry_target *target,
                                    bool release_buffer_lock,
                                    uint32_t *out_current_record_start);

static int
telemetry_buffer_gather(user_addr_t buffer, uint32_t *length, bool mark,
...
#define TELEMETRY_DEFAULT_NOTIFY_LEEWAY                                        \
  (4 * 1024) // Userland gets 4k of leeway to collect data after notification
#define TELEMETRY_MAX_UUID_COUNT                                               \
  (128) // Max of 128 non-shared-cache UUIDs to log for symbolication
...
struct micro_snapshot_buffer telemetry_buffer = {
    .buffer = 0, .size = 0, .current_position = 0, .end_point = 0};
...
int telemetry_bytes_since_last_mark =
...
#define TELEMETRY_LOCK()                                                       \
  do {                                                                         \
    lck_mtx_lock(&telemetry_mtx);                                              \
  } while (0)
#define TELEMETRY_TRY_SPIN_LOCK() lck_mtx_try_lock_spin(&telemetry_mtx)
#define TELEMETRY_UNLOCK()                                                     \
  do {                                                                         \
    lck_mtx_unlock(&telemetry_mtx);                                            \
  } while (0)

#define TELEMETRY_PMI_LOCK()                                                   \
  do {                                                                         \
    lck_mtx_lock(&telemetry_pmi_mtx);                                          \
  } while (0)
#define TELEMETRY_PMI_UNLOCK()                                                 \
  do {                                                                         \
    lck_mtx_unlock(&telemetry_pmi_mtx);                                        \
  } while (0)

#define TELEMETRY_MACF_LOCK()                                                  \
  do {                                                                         \
    lck_mtx_lock(&telemetry_macf_mtx);                                         \
  } while (0)
#define TELEMETRY_MACF_UNLOCK()                                                \
  do {                                                                         \
    lck_mtx_unlock(&telemetry_macf_mtx);                                       \
  } while (0)
...
TUNABLE(uint32_t, telemetry_buffer_size, "telemetry_buffer_size",
        TELEMETRY_DEFAULT_BUFFER_SIZE);
TUNABLE(uint8_t, telemetry_kernel_buffer_size_pow_2,
        "telemetry_kernel_buffer_size_pow_2", 16);
TUNABLE(uint32_t, telemetry_notification_leeway,
        "telemetry_notification_leeway", TELEMETRY_DEFAULT_NOTIFY_LEEWAY);

__startup_func static void _telemetry_init(void) {
  telemetry_buffer.size = MIN(telemetry_buffer_size, TELEMETRY_MAX_BUFFER_SIZE);

  kern_return_t ret =
      kmem_alloc(kernel_map, &telemetry_buffer.buffer, telemetry_buffer.size,
  ...
  if (telemetry_notification_leeway >= telemetry_buffer.size) {
    printf("telemetry: nonsensical telemetry_notification_leeway boot-arg %d "
           "changed to %d\n",
           telemetry_notification_leeway, TELEMETRY_DEFAULT_NOTIFY_LEEWAY);
    telemetry_notification_leeway = TELEMETRY_DEFAULT_NOTIFY_LEEWAY;
  }
  telemetry_buffer_notify_at =
      telemetry_buffer.size - telemetry_notification_leeway;
  ...
  mpsc_ring_init(&_telemetry_kernel_ring, telemetry_kernel_buffer_size_pow_2,
                 (uint8_t)cpu_count);
 ...
 */
static void _telemetry_notify_user(telemetry_notice_t flags) {
...
static void telemetry_pmi_handler(bool user_mode, __unused void *ctx) {
...
static void _telemetry_mark_curthread(bool interrupted_userspace) {
  ...
  ast_bits |=
      (interrupted_userspace ? AST_TELEMETRY_USER : AST_TELEMETRY_KERNEL);
  thread_ast_set(thread, ast_bits);
  ast_propagate(thread);
}

static void _telemetry_kernel_notify(void *__unused p1, void *__unused p2) {
...
void telemetry_ast(thread_t thread, ast_t reasons) {
  ...
  if (reasons & (AST_TELEMETRY_USER | AST_TELEMETRY_KERNEL)) {
    record_type |=
        (reasons & AST_TELEMETRY_PMI) ? kPMIRecord : kInterruptRecord;
  }
  ...
  enum micro_snapshot_flags user_telemetry =
      (reasons & AST_TELEMETRY_USER) ? kUserMode : 0;
...
static bool _telemetry_task_can_sample(task_t task) {
  return (task != TASK_NULL) && !task_did_exec(task) &&
 ...
 */
static void
_telemetry_take_sample_kernel(thread_t thread,
                              enum micro_snapshot_flags __unused flags) {
  ...
  unsigned int call_stack_count = backtrace(
      scratch->ts_call_stack, TELEMETRY_KERNEL_FRAMES_MAX, &ctl, &info);
  unsigned int call_stack_size =
      call_stack_count * sizeof(scratch->ts_call_stack[0]);
  ...
  uint64_t serial_number =
      os_atomic_inc(&telemetry_metadata.tm_samples_recorded, relaxed);

  struct recount_times_mach term_times =
      recount_task_terminated_times(kernel_task);
  ...
  scratch->ts_sample = (struct _telemetry_kernel_sample){
      ...
      .tks_telemetry_skipped =
          os_atomic_load(&telemetry_metadata.tm_samples_skipped, relaxed),
  ...
  uint32_t size_needed =
      sizeof(struct _telemetry_kernel_sample) + call_stack_size;
  uint32_t available =
 ...
 */
static void
_telemetry_kernel_snapshot(struct _telemetry_kernel_snapshots *snaps,
  ...
  snaps->tkse_task_snap = (struct task_snapshot){
      ...
      .did_throttle = 0,
      .p_start_sec =
          coalition_id(kernel_task->coalition[COALITION_TYPE_RESOURCE]),
      /* Set the on-behalf-of pids to -1. */
      .p_start_usec = UINT64_MAX,
      .latency_qos = LATENCY_QOS_TIER_UNSPECIFIED,
      .io_priority_size =
          {
              [0] = ((uint64_t)sample->tks_telemetry_source << 32) |
                    sample->tks_telemetry_generation,
  ...
  memset(snaps->tkse_thread_snap.pth_name, 0,
         sizeof(snaps->tkse_thread_snap.pth_name));
  strlcpy(snaps->tkse_thread_snap.pth_name, sample->tks_thread_name,
          sizeof(snaps->tkse_thread_snap.pth_name));
}
...
int telemetry_kernel_gather(user_addr_t user_buffer, uint32_t *user_length) {
  ...
  *user_length = 0;
  ...
  while (copied < copy_length) {
    ...
    bool advanced = mpsc_ring_cursor_advance(&_telemetry_kernel_ring, &cursor,
     ...
     */
    assert3u(sample.tks_call_stack_size, <,
             sizeof(uintptr_t) * TELEMETRY_KERNEL_FRAMES_MAX);
    ...
    advanced =
        mpsc_ring_cursor_advance(&_telemetry_kernel_ring, &cursor, &call_stack,
     ...
     */
    result =
        copyout(&call_stack, user_buffer + copied, sample.tks_call_stack_size);
    if (result != 0) {
      break;
    }
...
void telemetry_instrumentation_begin(
...
void telemetry_instrumentation_end(
    __unused struct micro_snapshot_buffer *buffer) {
  /* telemetry_XXX accessed outside of lock for instrumentation only */
...
static void _telemetry_take_sample_user(thread_t thread,
                                        enum micro_snapshot_flags flags) {
  ...
  if (btinfo.btui_async_frame_addr != 0 && btinfo.btui_async_start_index != 0) {
    /*
    ...
    struct backtrace_control ctl = {
        .btc_frame_addr = frame_addr,
    };
    btinfo = BTUINFO_INIT;
    unsigned int async_filled =
        backtrace_user(frames + async_start_index, frames_left, &ctl, &btinfo);
    if (btinfo.btui_error == 0) {
...
void telemetry_take_sample(thread_t thread, enum micro_snapshot_flags flags) {
...
#if CONFIG_MACF
void telemetry_macf_take_sample(thread_t thread,
                                enum micro_snapshot_flags flags) {
  task_t task;
  ...
  while (retry_count < max_retries) {
    btcount +=
        backtrace_user(frames + btcount, btcapacity - btcount, &btctl, &btinfo);

    if ((btinfo.btui_info & BTI_TRUNCATED) != 0 &&
        btinfo.btui_next_frame_addr != 0) {
      /*
      ...
      if (frames == frames_stack) {
        ...
        frames = krealloc_data(frames, btcapacity * sizeof(*frames),
                               (btcapacity + 128) * sizeof(*frames), Z_WAITOK);
  ...
  struct telemetry_target target = {.thread = thread,
                                    .frames = frames,
                                    .frames_count = btcount,
                                    .user64_regs =
                                        (btinfo.btui_info & BTI_64_BIT) != 0,
                                    .microsnapshot_flags = flags,
...
out:
  ...
  if (rv == 0) {
    ...
    mac_thread_telemetry(thread, 0, (void *)(telbuf->buffer + record_start),
...
static void _write_task_snapshot(struct task_snapshot *tsnap,
  ...
  tsnap->task_size =
      (typeof(tsnap->task_size))(get_task_phys_footprint(task) / PAGE_SIZE);
   ...
   * struct to save space and since we do not expect them to overflow 32-bits.
   * If we find these values overflowing in the future, the fix would be to
   * simply upgrade these counters to 64-bit in the task_snapshot struct
   */
  ...
  (void)thread_get_voucher_origin_proximate_pid(target->thread, &origin_pid,
                                                &proximate_pid);
  ...
  if (longname[0] != '\0') {
     ...
     */
    strlcpy((char *)tsnap->io_priority_count, &longname[16],
            sizeof(tsnap->io_priority_count));
  }
  if (target->include_metadata) {
    tsnap->io_priority_size[0] =
        ((uint64_t)telemetry_metadata.tm_source << 32) |
        telemetry_metadata.tm_generation;
    tsnap->io_priority_size[1] = telemetry_metadata.tm_period;
    tsnap->io_priority_size[2] =
        os_atomic_inc(&telemetry_metadata.tm_samples_recorded, relaxed);
...
static void _write_thread_snapshot(struct thread_snapshot *thsnap,
                                   const struct telemetry_target *target) {
 ...
 */
static struct _telemetry_uuids _telemetry_sample_uuids(task_t task) {
  ...
  /*
   * If dyld is updating the data structure (indicated by a NULL uuidArray
   * field), do not provide any UUIDs with the sample.
   */
  ...
  /*
   * The main binary and interesting non-shared-cache libraries should be in the
   * first few images.
   */
...
static bool _telemetry_sample_dispatch_serialno(task_t task, thread_t thread,
                                                uint64_t *serialno_out) {
  uint64_t const dqkeyaddr = thread_dispatchqaddr(thread);
  if (dqkeyaddr != 0) {
    uint64_t dqaddr = 0;
    size_t const user_ptr_size = task_has_64Bit_addr(task) ? 8 : 4;

    uint64_t const dq_serialno_offset =
...
static void *_telemetry_buffer_alloc(struct micro_snapshot_buffer *buf,
                                     size_t size) {
...
int telemetry_process_sample(const struct telemetry_target *target,
                             bool release_buffer_lock,
                             uint32_t *out_current_record_start) {
  ...
  /*
   * Look for a dispatch queue serial number, and copy it in from userland if
   * present.
   */
  uint64_t dqserial = 0;
  bool dqserial_valid =
      _telemetry_sample_dispatch_serialno(task, thread, &dqserial);

  size_t const frames_size = btcount * (user64_regs ? 8 : 4);
  size_t const sample_size =
      _telemetry_sample_size_static + uuids.uuid_info_size +
      (dqserial_valid ? sizeof(dqserial) : 0) + frames_size;

  clock_get_calendar_microtime(&secs, &usecs);

  /*
   * We do the bulk of the operation under the telemetry lock, on assumption
   * that any page faults during execution will not cause another
   * AST_TELEMETRY_ALL to deadlock; they will just block until we finish. This
   * makes it easier to copy into the buffer directly. As soon as we unlock,
   * userspace can copy out of our buffer.
   */
  ...
  if (rv == 0) {
    if ((current_buffer->size - current_buffer->current_position) <
        sample_size) {
      /*
       * We can't fit a record in the space available, so wrap around to the
       * beginning. Save the current position as the known end point of valid
       * data.
       */
    ...
    struct micro_snapshot *msnap =
        _telemetry_buffer_alloc(current_buffer, sizeof(*msnap));
    *msnap = (struct micro_snapshot){
    ...
    struct task_snapshot *tsnap =
        _telemetry_buffer_alloc(current_buffer, sizeof(*tsnap));
    _write_task_snapshot(tsnap, target);

    if (uuids.uuid_info_size > 0) {
      void *uuid_info_buf =
          _telemetry_buffer_alloc(current_buffer, uuids.uuid_info_size);
    ...
    struct thread_snapshot *thsnap =
        _telemetry_buffer_alloc(current_buffer, sizeof(*thsnap));
    _write_thread_snapshot(thsnap, target);

    if (dqserial_valid) {
      thsnap->ss_flags |= kHasDispatchSerial;
      uint64_t *dqserial_buf =
          _telemetry_buffer_alloc(current_buffer, sizeof(*dqserial_buf));
...
int telemetry_buffer_gather(user_addr_t buffer, uint32_t *length, bool mark,
                            struct micro_snapshot_buffer *current_buffer) {
  int result = 0;
  ...
  /*
   * Copy the ring buffer out to userland in order sorted by time: least recent
   * to most recent. First, we need to search forward from the cursor to find
   * the oldest record in our buffer.
   */
  oldest_record_offset = current_buffer->current_position;
  do {
    if (((oldest_record_offset + sizeof(uint32_t)) > current_buffer->size) ||
        ((oldest_record_offset + sizeof(uint32_t)) >
         current_buffer->end_point)) {
      ...
      /*
       * We've looked through the end of the active buffer without finding a
       * valid record; that means all valid records are in a single chunk,
       * beginning at the very start of the buffer.
       */

      oldest_record_offset = 0;
      assert(*(uint32_t *)(uintptr_t)(current_buffer->buffer) ==
             STACKSHOT_MICRO_SNAPSHOT_MAGIC);
      break;
    }

    if (*(uint32_t *)(uintptr_t)(current_buffer->buffer +
    ...
    /*
     * There are no alignment guarantees for micro-stackshot records, so we must
     * search at each byte offset.
     */
    oldest_record_offset++;
  } while (oldest_record_offset != current_buffer->current_position);

  /*
   * If needed, copyout in two chunks: from the oldest record to the end of the
   * buffer, and then from the beginning of the buffer up to the current
   * position.
   */
  if (oldest_record_offset != 0) {
    if ((result = copyout(
             (void *)(current_buffer->buffer + oldest_record_offset), buffer,
...
out:
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_STACKSHOT, MICROSTACKSHOT_GATHER) | DBG_FUNC_END,
       current_buffer->current_position, *length, current_buffer->end_point,
       (&telemetry_buffer != current_buffer));
...
#if CONFIG_MACF
static int telemetry_macf_init_locked(size_t buffer_size) {
  kern_return_t kr;
  ...
  kr = kmem_alloc(kernel_map, &telemetry_macf_buffer.buffer,
                  ...
                  KMA_DATA | KMA_ZERO | KMA_PERMANENT, VM_KERN_MEMORY_SECURITY);
...
static int telemetry_backtrace_add_kernel(char *buf, size_t buflen) {
  int rc = 0;
  ...
  rc += scnprintf(buf, buflen, "%s@%lx:%lx\n", kernel_uuid_string, unslid,
                  unslid + segSizeTEXTEXEC - 1);
 ...
 */
void telemetry_backtrace_to_string(char *buf, size_t buflen, uint32_t tot,
                                   uintptr_t *frames) {
  size_t l = 0;
