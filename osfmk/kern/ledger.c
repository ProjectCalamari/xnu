...
#define LF_DIAG_DISABLED                                                       \
  0x40000 /* diagnostics threshold are disabled at the moment */
 ...
 * Size is stored in the upper 16 bits, and offset is stored in the lower 16
 * bits.
 *
...
#define ENTRY_ID_SIZE(x)                                                       \
  ((((uint32_t)(x)) & (ENTRY_ID_SIZE_MASK)) >> ENTRY_ID_SIZE_SHIFT)
_Static_assert(((sizeof(struct ledger_entry_small) << ENTRY_ID_SIZE_SHIFT) |
                (UINT16_MAX / sizeof(struct ledger_entry_small))) > 0,
               "Valid ledger index < 0");
_Static_assert(((sizeof(struct ledger_entry) << ENTRY_ID_SIZE_SHIFT) |
                (UINT16_MAX / sizeof(struct ledger_entry_small))) > 0,
               "Valid ledger index < 0");
_Static_assert(sizeof(int) * 8 >= ENTRY_ID_SIZE_SHIFT * 2,
               "Ledger indices don't fit in an int.");
#define MAX_LEDGER_ENTRIES (UINT16_MAX / sizeof(struct ledger_entry_small))

#define LEDGER_DIAG_MEM_THRESHOLD_SHIFT 20
#define LEDGER_DIAG_MEM_AMOUNT_TO_THRESHOLD(X)                                 \
  ((X) >> (LEDGER_DIAG_MEM_THRESHOLD_SHIFT))
#define LEDGER_DIAG_MEM_AMOUNT_FROM_THRESHOLD(X)                               \
  (((ledger_amount_t)(X)) << (LEDGER_DIAG_MEM_THRESHOLD_SHIFT))

/* These features can fit in a small ledger entry. All others require a full
 * size ledger entry */
#define LEDGER_ENTRY_SMALL_FLAGS                                               \
  (LEDGER_ENTRY_ALLOW_PANIC_ON_NEGATIVE | LEDGER_ENTRY_ALLOW_INACTIVE)

/*
 * struct ledger_entry_info is available to user space and used in ledger()
 * syscall. Changing its size would cause memory corruption. See
 * rdar://132747700
 */
 ...
 */
static_assert(sizeof(struct ledger_entry_small) ==
              sizeof(struct ledger_entry_counter));
...
static inline volatile uint32_t *get_entry_flags(ledger_t l, int index) {
  assert(l != NULL);
...
#define INVALID_LEDGER_ACCESS(l, e)                                            \
  if ((e) != -1)                                                               \
    panic("Invalid ledger access: ledger=%p, entry=0x%x, entry_size=0x%x, "    \
          "entry_offset=0x%x\n",                                               \
...
static inline bool is_entry_valid(ledger_t l, int entry) {
  ...
  if (os_mul_overflow(offset, sizeof(struct ledger_entry_small), &offset) ||
      offset >= l->l_size) {
...
static inline bool is_entry_active(ledger_t l, int entry) {
...
static inline bool is_entry_valid_and_active(ledger_t l, int entry) {
...
/*
 * Modifying the reference count, table size, table contents, lt_next_offset, or
 * lt_entries_lut, requires holding the lt_lock.  Modfying the table address
 * requires both lt_lock and setting the inuse bit.  This means that the
 * lt_entries field can be safely dereferenced if you hold either the lock or
 * the inuse bit.  The inuse bit exists solely to allow us to swap in a new,
 * larger entries table without requiring a full lock to be acquired on each
 * lookup. Accordingly, the inuse bit should never be held for longer than it
 * takes to extract a value from the table - i.e., 2 or 3 memory references.
 */
...
ledger_template_entries_lut_size(uint16_t lt_table_size) {
  /*
   * The lookup table needs to be big enough to store lt_table_size entries of
   * the largest entry size (struct ledger_entry) given a stride of the smallest
   * entry size (struct ledger_entry_small)
   */
  if (os_mul_overflow(
          lt_table_size,
          (sizeof(struct ledger_entry) / sizeof(struct ledger_entry_small)),
          &lt_table_size)) {
    /*
     * This means MAX_LEDGER_ENTRIES is misconfigured or
     * someone has accidently passed in an lt_table_size that is >
     * MAX_LEDGER_ENTRIES
     */
    panic("Attempt to create a lookup table for a ledger template with too "
          "many entries. lt_table_size=%u, MAX_LEDGER_ENTRIES=%lu\n",
...
static uint64_t abstime_to_nsecs(uint64_t abstime) {
  uint64_t nsecs;
...
static uint64_t nsecs_to_abstime(uint64_t nsecs) {
  uint64_t abstime;
...
static const uint16_t *ledger_entry_to_template_idx(ledger_template_t template,
                                                    int index) {
 ...
 * ledger entry.
 */
static struct ledger_entry *ledger_entry_identifier_to_entry(ledger_t ledger,
                                                             int id) {
...
ledger_template_t ledger_template_create(const char *name) {
  ...
  template->lt_entries = kalloc_type(
      struct entry_template, template->lt_table_size, Z_WAITOK | Z_ZERO);
  if (template->lt_entries == NULL) {
    kfree_type(struct ledger_template, template);
    return NULL;
  }
  template->lt_entries_lut = kalloc_type(
      uint16_t, ledger_template_entries_lut_size(template->lt_table_size),
      Z_WAITOK | Z_ZERO);
...
static void ledger_template_create_counter_zone(ledger_template_t template) {
#if ATOMIC_COUNTER_USE_PERCPU
  if (template->lt_counters) {
    snprintf(template->lt_counter_zone_name,
             sizeof(template->lt_counter_zone_name), "%s.c", template->lt_name);

    template->lt_counter_zone =
        zone_create(template->lt_counter_zone_name,
...
ledger_template_t ledger_template_copy(ledger_template_t template,
                                       const char *name) {
  ...
  new_entries_lut_size =
      ledger_template_entries_lut_size(template->lt_table_size);

  new_entries_lut =
      kalloc_type(uint16_t, new_entries_lut_size, Z_WAITOK | Z_ZERO);
  ...
  bcopy(template->lt_entries, new_entries,
        sizeof(struct entry_template) * template->lt_table_size);
  kfree_type(struct entry_template, new_template->lt_table_size,
             new_template->lt_entries);
  /* Copy the look up table. */
  bcopy(template->lt_entries_lut, new_entries_lut,
        sizeof(uint16_t) * new_entries_lut_size);
  kfree_type(uint16_t,
             ledger_template_entries_lut_size(new_template->lt_table_size),
             new_template->lt_entries_lut);
...
void ledger_template_dereference(ledger_template_t template) {
  template_lock(template);
  template->lt_refs--;
  template_unlock(template);

  if (template->lt_refs == 0) {
    kfree_type(struct entry_template, template->lt_table_size,
               template->lt_entries);
    kfree_type(uint16_t,
               ledger_template_entries_lut_size(template->lt_table_size),
               template->lt_entries_lut);
...
static inline int ledger_entry_id(uint16_t size, uint16_t offset) {
  int idx = offset;
...
int ledger_entry_add_with_flags(ledger_template_t template, const char *key,
  ...
  if (template->lt_cnt == template->lt_table_size) {
    ...
    if (new_cnt > MAX_LEDGER_ENTRIES) {
      template_unlock(template);
      panic("Attempt to create a ledger template with more than "
            "MAX_LEDGER_ENTRIES. MAX_LEDGER_ENTRIES=%lu, old_cnt=%u, "
            "new_cnt=%u\n",
            MAX_LEDGER_ENTRIES, old_cnt, new_cnt);
    }

    new_entries =
        kalloc_type(struct entry_template, new_cnt, Z_WAITOK | Z_ZERO);
    if (new_entries == NULL) {
      template_unlock(template);
      return -1;
    }
    new_entries_lut = kalloc_type(
        uint16_t, ledger_template_entries_lut_size(new_cnt), Z_WAITOK | Z_ZERO);
    if (new_entries_lut == NULL) {
      template_unlock(template);
    ...
    kfree_type(uint16_t, ledger_template_entries_lut_size(old_cnt),
               old_entries_lut);
  }
  ...
  if ((flags & LEDGER_ENTRY_USE_COUNTER) != 0) {
    ...
    if (template->lt_counters == 0) {
      template->lt_counter_offset =
          (template->lt_next_offset / sizeof(struct ledger_entry_small));
    }
 ...
 */
int ledger_entry_add(ledger_template_t template, const char *key,
                     const char *group, const char *units) {
  /*
   * When using the legacy interface we have to be pessimistic
   * and allocate memory for all of the features.
   */
  return ledger_entry_add_with_flags(
      template, key, group, units,
...
kern_return_t ledger_entry_setactive(ledger_t ledger, int entry) {
...
int ledger_key_lookup(ledger_template_t template, const char *key) {
  int id = -1;
 ...
 * template will fail.
 */
void ledger_template_complete(ledger_template_t template) {
  size_t ledger_size;
  ledger_size = sizeof(struct ledger) + template->lt_next_offset;
  assert(ledger_size > sizeof(struct ledger));
  template->lt_zone =
      zone_create(template->lt_name, ledger_size, ZC_DESTRUCTIBLE);
 ...
 * pmap layer.
 */
void ledger_template_complete_secure_alloc(ledger_template_t template) {
  size_t ledger_size;
  ledger_size = sizeof(struct ledger) + template->lt_next_offset;
 ...
 */
ledger_t ledger_instantiate(ledger_template_t template, int entry_type) {
  ledger_t ledger;
  ...
  if (template->lt_counter_zone) {
    counters =
        zalloc_percpu(template->lt_counter_zone, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  }
#endif
 ...
 */
void ledger_reference(ledger_t ledger) {
...
#if ATOMIC_COUNTER_USE_PERCPU
static void ledger_free_counters(ledger_t ledger) {
  ...
  /* We hold the index of the first counter entry which has the pointer to the
   * allocation */
  lec = (struct ledger_entry_counter *)&ledger
            ->l_entries[template->lt_counter_offset];
 ...
 */
void ledger_dereference(ledger_t ledger) {
  if (!LEDGER_VALID(ledger)) {
    return;
  }
 ...
 */
static inline bool warn_level_exceeded(struct ledger_entry *le) {
 ...
 */
static inline bool diag_mem_threshold_exceeded(struct ledger_entry *le) {
  ledger_amount_t balance;
  ledger_amount_t diag_mem_threshold;

  if ((le->le_diag_threshold_scaled != LEDGER_DIAG_MEM_THRESHOLD_INFINITY) &&
      (ledger_is_diag_threshold_enabled_internal(le) == true)) {
    if (le->le_flags & LF_TRACK_CREDIT_ONLY) {
      assert(le->le_debit == 0);
    } else {
      assert((le->le_credit >= 0) && (le->le_debit >= 0));
    ...
    diag_mem_threshold =
        LEDGER_DIAG_MEM_AMOUNT_FROM_THRESHOLD(le->le_diag_threshold_scaled);
    balance = le->le_credit - le->le_debit;
 ...
 */
static inline bool limit_exceeded(struct ledger_entry *le) {
  ledger_amount_t balance;
...
static inline struct ledger_callback *entry_get_callback(ledger_t ledger,
                                                         int entry) {
  ...
  ledger_template_idx_p =
      ledger_entry_to_template_idx(ledger->l_template, entry);
  if (ledger_template_idx_p != NULL) {
    callback =
        ledger->l_template->lt_entries[*ledger_template_idx_p].et_callback;
  }
 ...
 */
static inline void ledger_limit_entry_wakeup(struct ledger_entry *le) {
 ...
 * Refill the coffers.
 */
static void ledger_refill(uint64_t now, ledger_t ledger, int entry) {
  ...
  if (due < 0 && (le->le_flags & LF_PANIC_ON_NEGATIVE)) {
    assertf(due >= 0, "now=%llu, ledger=%p, entry=%d, balance=%lld, due=%lld",
            now, ledger, entry, balance, due);
  } else {
  ...
  if (balance == due) {
    ...
    le->_le.le_refill.le_last_refill +=
        (le->_le.le_refill.le_refill_period * periods);
  }
...
void ledger_entry_check_new_balance(thread_t thread, ledger_t ledger,
                                    int entry) {
  uint16_t size, offset;
  ...
  if (size == sizeof(struct ledger_entry_small)) {
    ...
    if ((les->les_flags & LF_PANIC_ON_NEGATIVE) && les->les_credit < 0) {
      panic("ledger_entry_check_new_balance(%p,%d): negative ledger %p "
            "credit:%lld debit:0 balance:%lld",
            ledger, entry, les, les->les_credit, les->les_credit);
    }
    ...
    if ((le->le_flags & LF_PANIC_ON_NEGATIVE) &&
      ...
      panic("ledger_entry_check_new_balance(%p,%d): negative ledger %p "
            "credit:%lld debit:%lld balance:%lld",
...
void ledger_check_new_balance(thread_t thread, ledger_t ledger, int entry) {
 ...
 */
kern_return_t ledger_credit_thread(thread_t thread, ledger_t ledger, int entry,
 ...
 */
kern_return_t ledger_credit(ledger_t ledger, int entry,
                            ledger_amount_t amount) {
 ...
 */
kern_return_t ledger_credit_nocheck(ledger_t ledger, int entry,
                                    ledger_amount_t amount) {
 ...
 */
kern_return_t ledger_rollup(ledger_t to_ledger, ledger_t from_ledger) {
  int id;
 ...
 */
kern_return_t ledger_rollup_entry(ledger_t to_ledger, ledger_t from_ledger,
                                  int entry) {
  ...
  if (is_entry_valid(from_ledger, entry) && is_entry_valid(to_ledger, entry)) {
    ...
    if (entry_size == sizeof(struct ledger_entry)) {
      ...
      if (from_les->les_flags & LF_IS_COUNTER) {
        struct ledger_entry_counter *from_lec =
            (struct ledger_entry_counter *)from_les;
        struct ledger_entry_counter *to_lec =
            (struct ledger_entry_counter *)to_les;
...
/*
 * Zero the balance of a ledger by adding to its credit or debit, whichever is
 * smaller. Note that some clients of ledgers (notably, task wakeup statistics)
 * require that le_credit only ever increase as a function of ledger_credit().
 */
kern_return_t ledger_zero_balance(ledger_t ledger, int entry) {
  struct ledger_entry *le;
  ...
  top:
    ...
    if (le->le_flags & LF_TRACK_CREDIT_ONLY) {
      ...
      lprintf(("%p zeroed %lld->%lld\n", current_thread(), le->le_debit,
               le->le_credit));
    } else if (credit < debit) {
      if (!OSCompareAndSwap64(credit, debit, &le->le_credit)) {
        goto top;
      }
      lprintf(("%p zeroed %lld->%lld\n", current_thread(), le->le_credit,
               le->le_debit));
    }
  } else {
    panic("Unknown ledger entry size! ledger=%p, entry=0x%x, entry_size=%d\n",
...
kern_return_t ledger_get_limit(ledger_t ledger, int entry,
 ...
 */
kern_return_t ledger_set_limit(ledger_t ledger, int entry,
                               ledger_amount_t limit,
  ...
  if (warn_level_percentage != 0) {
    ...
    assert(limit !=
           LEDGER_LIMIT_INFINITY); /* warn % without limit makes no sense */
...
#if CONFIG_LEDGER_INTERVAL_MAX
kern_return_t ledger_get_interval_max(ledger_t ledger, int entry,
...
kern_return_t ledger_get_lifetime_max(ledger_t ledger, int entry,
 ...
 */
kern_return_t ledger_track_maximum(ledger_template_t template, int entry,
...
kern_return_t ledger_panic_on_negative(ledger_template_t template, int entry) {
  const uint16_t *idx_p;
  uint16_t idx;
  template_lock(template);
...
kern_return_t ledger_track_credit_only(ledger_template_t template, int entry) {
  const uint16_t *idx_p;
  uint16_t idx;
  struct entry_template *et = NULL;
  kern_return_t kr = KERN_INVALID_VALUE;
 ...
 */
kern_return_t ledger_set_callback(ledger_template_t template, int entry,
                                  ledger_callback_t func, const void *param0,
                                  const void *param1) {
  struct entry_template *et;
 ...
 */
kern_return_t ledger_disable_callback(ledger_t ledger, int entry) {
  struct ledger_entry *le = NULL;
  ...
  /*
   * le_warn_percent is used to indicate *if* this ledger has a warning
   * configured, in addition to what that warning level is set to. This means a
   * side-effect of ledger_disable_callback() is that the warning level is
   * forgotten.
   */
  le->le_warn_percent = LEDGER_PERCENT_NONE;
 ...
 * configured.
 */
kern_return_t ledger_enable_callback(ledger_t ledger, int entry) {
  struct ledger_entry *le = NULL;
 ...
 */
kern_return_t ledger_get_period(ledger_t ledger, int entry, uint64_t *period) {
  struct ledger_entry *le;
 ...
 */
kern_return_t ledger_set_period(ledger_t ledger, int entry, uint64_t period) {
  struct ledger_entry *le = NULL;
 ...
 */
kern_return_t ledger_disable_refill(ledger_t ledger, int entry) {
  struct ledger_entry *le = NULL;
...
kern_return_t ledger_get_actions(ledger_t ledger, int entry, int *actions) {
  struct ledger_entry *le = NULL;
  *actions = 0;
...
kern_return_t ledger_set_action(ledger_t ledger, int entry, int action) {
...
kern_return_t ledger_debit_thread(thread_t thread, ledger_t ledger, int entry,
                                  ledger_amount_t amount) {
  struct ledger_entry *le;
  ledger_amount_t old, new;
  uint16_t entry_size = ENTRY_ID_SIZE(entry);
...
kern_return_t ledger_debit(ledger_t ledger, int entry, ledger_amount_t amount) {
  return ledger_debit_thread(current_thread(), ledger, entry, amount);
}

kern_return_t ledger_debit_nocheck(ledger_t ledger, int entry,
                                   ledger_amount_t amount) {
  return ledger_debit_thread(NULL, ledger, entry, amount);
}

void ledger_ast(thread_t thread) {
  ...
  if (req_action != 0) {
    ...
    if (req_action == THREAD_CPULIMIT_BLOCK) {
      thread->options &= ~TH_OPT_FORCED_LEDGER;
      ret = thread_set_cpulimit(THREAD_CPULIMIT_BLOCK, req_percentage,
                                req_interval_ns);
      assert3u(ret, ==, KERN_SUCCESS);
      thread->options |= TH_OPT_FORCED_LEDGER;
    }
...
top:
   ...
   * task can change them at any point (with the task locked).
   */
  task_lock(task);
  ...
  /*
   * Make sure this thread is up to date with regards to any task-wide
   * per-thread CPU limit, but only if it doesn't have a thread-private blocking
   * CPU limit.
   */
  if (((task_flags & TASK_RUSECPU_FLAGS_PERTHR_LIMIT) != 0) &&
    ...
    if (((thread->options & TH_OPT_PROC_CPULIMIT) == 0) ||
      ...
      thread_set_cpulimit(THREAD_CPULIMIT_EXCEPTION, task_percentage,
                          task_interval);
...
static uint32_t ledger_check_needblock(ledger_t l, uint64_t now) {
  ...
  for (i = 0; i < template->lt_cnt; i++) {
      ...
      if (le->le_flags & LEDGER_ACTION_CALLBACK) {
        ...
        if (warn_level_exceeded(le)) {
          ...
          if ((flags & LF_WARNED) == 0) {
            lc->lc_func(LEDGER_WARNING_ROSE_ABOVE, lc->lc_param0,
                        lc->lc_param1);
          }
        } else {
          flags = flag_clear(&le->le_flags, LF_WARNED);
          if (flags & LF_WARNED) {
            lc->lc_func(LEDGER_WARNING_DIPPED_BELOW, lc->lc_param0,
                        lc->lc_param1);
          }
        }
      }
      ...
      if (diag_mem_threshold_exceeded(le)) {
        if (le->le_flags & LEDGER_ACTION_CALLBACK) {
          ...
          if ((flags & LF_DIAG_WARNED) == 0) {
            lc->lc_func(LEDGER_WARNING_DIAG_MEM_THRESHOLD, lc->lc_param0,
                        lc->lc_param1);
          }
        }
      }
    ...
    if (le->le_flags & LF_REFILL_SCHEDULED) {
      ...
      if ((le->_le.le_refill.le_last_refill +
           le->_le.le_refill.le_refill_period) <= now) {
...
static kern_return_t ledger_perform_blocking(ledger_t l) {
  ...
  for (i = 0; i < template->lt_cnt; i++) {
    ...
    le = (struct ledger_entry *)&l->l_entries[et->et_offset];
    if ((!limit_exceeded(le)) || ((le->le_flags & LEDGER_ACTION_BLOCK) == 0)) {
      continue;
    }
    ...
    ret = thread_block_reason(THREAD_CONTINUE_NULL, NULL, AST_LEDGER);
...
kern_return_t ledger_get_entries(ledger_t ledger, int entry,
...
kern_return_t ledger_reset_callback_state(ledger_t ledger, int entry) {
  struct ledger_entry *le = NULL;
...
kern_return_t ledger_disable_panic_on_negative(ledger_t ledger, int entry) {
...
kern_return_t ledger_get_panic_on_negative(ledger_t ledger, int entry,
...
kern_return_t ledger_get_balance(ledger_t ledger, int entry,
                                 ledger_amount_t *balance) {
  kern_return_t kr;
...
int ledger_template_info(void **buf, int *len) {
  ...
  lti = kalloc_data((*len) * sizeof(struct ledger_template_info), Z_WAITOK);
  if (lti == NULL) {
    return ENOMEM;
  }
  *buf = lti;
...
static kern_return_t _ledger_fill_entry_info(ledger_t ledger, int entry,
  ...
  if (entry_size == sizeof(struct ledger_entry_small)) {
    ...
    lei->lei_refill_period =
        (le->le_flags & LF_REFILL_SCHEDULED)
            ? abstime_to_nsecs(le->_le.le_refill.le_refill_period)
            : 0;
    lei->lei_last_refill =
        abstime_to_nsecs(now - le->_le.le_refill.le_last_refill);
  } else {
    panic("Unknown ledger entry size! ledger=%p, entry=0x%x, entry_size=%d\n",
          ledger, entry, entry_size);
  }
...
static kern_return_t ledger_fill_entry_info(ledger_t ledger, int entry,
                                            void *lei_generic, uint64_t now,
                                            bool v2) {
  ledger_amount_t max;
  kern_return_t kr;
  struct ledger_entry_info *lei = (struct ledger_entry_info *)lei_generic;
  struct ledger_entry_info_v2 *lei_v2 =
      (struct ledger_entry_info_v2 *)lei_generic;
...
int ledger_get_task_entry_info_multiple(task_t task, void **buf, int *len,
                                        bool v2) {
  ...
  entry_size = (v2) ? sizeof(struct ledger_entry_info_v2)
...
void ledger_get_entry_info(ledger_t ledger, int entry,
                           struct ledger_entry_info *lei) {
  uint64_t now = mach_absolute_time();
...
int ledger_info(task_t task, struct ledger_info *info) {
  ledger_t l;
 ...
 */
ledger_amount_t ledger_get_remaining(ledger_t ledger, int entry) {
 ...
 * Must be a valid, active, full-sized ledger.
 */
void ledger_restart(ledger_t ledger, int entry, uint64_t now) {
 ...
 * Must be a valid, active, full-sized ledger.
 */
uint64_t ledger_get_interval_remaining(ledger_t ledger, int entry,
                                       uint64_t now) {
  const struct ledger_entry *le =
      ledger_entry_identifier_to_entry(ledger, entry);
...
#ifdef LEDGER_DEBUG
int ledger_limit(task_t task, struct ledger_limit_args *args) {
  ledger_t l;
  int64_t limit;
  int idx;
  ...
  if (idx == task_ledgers.cpu_time) {
    ...
    if (args->lla_refill_period) {
      ...
      nsecs = args->lla_limit * (args->lla_refill_period / 100);
  ...
  flag_set(ledger_entry_identifier_to_entry(l, idx)->le_flags,
           LEDGER_ACTION_BLOCK);
  return 0;
}
#endif
...
/*
 * Adjust the diag mem threshold limit of a resource. The diag mem threshold
 * limit only works prescaled by 20 bits (mb)
 */
#if DEBUG || DEVELOPMENT
kern_return_t ledger_set_diag_mem_threshold(ledger_t ledger, int entry,
                                            ledger_amount_t limit) {
  struct ledger_entry *le;
  ...
  le = ledger_entry_identifier_to_entry(ledger, entry);
  le->le_diag_threshold_scaled =
      (int16_t)LEDGER_DIAG_MEM_AMOUNT_TO_THRESHOLD(limit);
...
kern_return_t ledger_get_diag_mem_threshold(ledger_t ledger, int entry,
                                            ledger_amount_t *limit) {
  struct ledger_entry *le;
...
static inline void
ledger_set_diag_mem_threshold_flag_disabled_internal(struct ledger_entry *le,
                                                     bool value) {
  if (value == true) {
 ...
 */
kern_return_t ledger_set_diag_mem_threshold_disabled(ledger_t ledger,
                                                     int entry) {
  struct ledger_entry *le;
  ...
  le = ledger_entry_identifier_to_entry(ledger, entry);
  if (le->le_diag_threshold_scaled == LEDGER_DIAG_MEM_THRESHOLD_INFINITY) {
    lprintf(("ledger_set_diag_mem_threshold_disabled, cannot disable a ledger "
             "entry that have no value, returning error"));
    return KERN_INVALID_ARGUMENT;
  }
 ...
 */
kern_return_t ledger_set_diag_mem_threshold_enabled(ledger_t ledger,
                                                    int entry) {
  struct ledger_entry *le;
  ...
  /*
   *  if (le->le_diag_threshold_scaled == LEDGER_DIAG_MEM_THRESHOLD_INFINITY) {
   *       lprintf(("ledger_set_diag_mem_threshold_enabled, cannot disable a
   * ledger entry that have no value, returning error")); return
...
/**
 * Obtain the diagnostics threshold enabled flag. If the diagnostics threshold
 * is enabled, returns true else returns false.
 */
kern_return_t ledger_is_diag_threshold_enabled(ledger_t ledger, int entry,
                                               bool *status) {
  struct ledger_entry *le;
  ...
  /*
   *  if (le->le_diag_threshold_scaled == LEDGER_DIAG_MEM_THRESHOLD_INFINITY) {
   *       lprintf(("ledger_is_diag_threshold_enabled, get enabled flag for a
   * ledger entry that have no value, returning error")); return
   * KERN_INVALID_ARGUMENT;
   *  }
   */
