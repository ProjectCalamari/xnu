...
__startup_data static struct thread init_thread;
static SECURITY_READ_ONLY_LATE(uint32_t) ctid_nonce;
COMPACT_ID_TABLE_DEFINE(__static_testable, ctid_table);

__startup_func static void thread_zone_startup(void) {
  size_t size = sizeof(struct thread);

#ifdef MACH_BSD
  size += roundup(uthread_size, _Alignof(struct thread));
#endif
  thread_zone = zone_create_ext(
      "threads", size, ZC_SEQUESTER | ZC_ZFREE_CLEARMEM, ZONE_ID_THREAD, NULL);
}
...
/*
 * Level (in terms of percentage of the limit) at which the CPU usage monitor
 * triggers telemetry.
 *
 * (ie when any thread's CPU consumption exceeds 70% of the limit, start taking
 * user stacktraces, aka micro-stackshots)
 */
...
TUNABLE_WRITEABLE(int, exc_resource_threads_enabled,
                  "exc_resource_threads_enabled", 1);

void __attribute__((noinline))
SENDING_NOTIFICATION__TASK_HAS_TOO_MANY_THREADS(task_t, int);
...
static void thread_ro_create(task_t parent_task, thread_t th,
                             thread_ro_t tro_tpl) {
#if __x86_64__
...
static void thread_ro_destroy(thread_t th) {
...
__startup_func thread_t thread_bootstrap(void) {
  /*
...
void thread_machine_init_template(void) {
...
boolean_t thread_is_active(thread_t thread) { return thread->active; }

void thread_corpse_continue(void) {
...
__dead2 static void thread_terminate_continue(void) {
...
void thread_terminate_self(void) {
  ...
  if (kdebug_enable && bsd_info && !task_is_exec_copy(task)) {
    ...
    if (kdebug_debugid_enabled(DBG_MT_INSTRS_CYCLES_THR_EXIT)) {
      ...
      KDBG_RELEASE(
          DBG_MT_INSTRS_CYCLES_THR_EXIT, recount_usage_instructions(&usage),
          recount_usage_cycles(&usage), recount_usage_system_time_mach(&usage),
  ...
  if (threadcnt == 0 && bsd_info != NULL) {
    ...
    if (kdebug_enable) {
      /* since we're the last thread in this process, trace out the command name
       * too */
      long args[4] = {0};
    ...
    if (task->corpse_info) {
      ...
      exception_type_t etype =
          get_exception_from_corpse_crashinfo(task->corpse_info);
  ...
  while (thread->wait_timer_active > 0) {
    ...
    if (delay_us > USEC_PER_SEC) {
      panic("wait timer failed to inactivate!"
            ...
            "wait_timer_armed: %d",
            thread, thread->wait_timer_active, thread->wait_timer_armed);
    }
  ...
  if (thread->rwlock_count > 0) {
    panic("rwlock_count is %d for thread %p, possibly it still holds a rwlock",
          thread->rwlock_count, thread);
  }
  ...
  assert3u(0, ==,
           thread->sched_flags &
               (TH_SFLAG_WAITQ_PROMOTED | TH_SFLAG_RW_PROMOTED |
                TH_SFLAG_EXEC_PROMOTED | TH_SFLAG_FLOOR_PROMOTED |
                TH_SFLAG_DEPRESS));
...
void thread_deallocate_safe(thread_t thread) {
  if (__improbable(thread_ref_release(thread))) {
    /* enqueue the thread for thread deallocate deamon to call
...
void thread_deallocate_complete(thread_t thread) {
  task_t task;
 ...
 */
void thread_exception_enqueue(task_t task, thread_t thread,
                              exception_type_t etype) {
...
void thread_backtrace_enqueue(kcdata_object_t obj,
                              exception_port_t ports[static BT_EXC_PORTS_COUNT],
                              exception_type_t etype) {
  struct thread_backtrace_elt *elt =
      kalloc_type(struct thread_backtrace_elt, Z_WAITOK | Z_NOFAIL);
  elt->obj = obj;
  elt->exception_type = etype;
...
thread_terminate_queue_invoke(mpsc_queue_chain_t e,
  ...
  if (thread_get_tag(thread) & THREAD_TAG_USER_JOIN) {
    ...
    (void)copyoutmap_atomic32(task->map, kport, uthread_joiner_address(uth));
 ...
 *	Called at splsched.
 */
void thread_terminate_enqueue(thread_t thread) {
 ...
 */
void thread_terminate_crashed_threads(void) {
  thread_t th_remove;
 ...
 *	invoke failures.
 */
static void thread_stack_queue_invoke(mpsc_queue_chain_t elm,
                                      __assert_only mpsc_daemon_queue_t dq) {
  ...
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_STACK_WAIT) |
                            DBG_FUNC_END,
 ...
 *	Called at splsched.
 */
void thread_stack_enqueue(thread_t thread) {
  KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SCHED, MACH_STACK_WAIT) |
                            DBG_FUNC_START,
                        thread_tid(thread), 0, 0, 0, 0);
  assert_thread_magic(thread);
...
void thread_daemon_init(void) {
  ...
  result = mpsc_daemon_queue_init_with_thread(
      &thread_stack_queue, thread_stack_queue_invoke, BASEPRI_PREEMPT_HIGH,
  ...
  result = mpsc_daemon_queue_init_with_thread(
      &thread_exception_queue, thread_exception_queue_invoke, MINPRI_KERNEL,
  ...
  result = mpsc_daemon_queue_init_with_thread(
      &thread_backtrace_queue, thread_backtrace_queue_invoke, MINPRI_KERNEL,
...
thread_create_internal(task_t parent_task, integer_t priority,
                       thread_continue_t continuation, void *parameter,
...
#pragma clang diagnostic pop
...
#ifdef MACH_BSD
  uthread_init(parent_task, get_bsdthread_info(new_thread), &tro_tpl,
               (options & (TH_OPTION_WORKQ | TH_OPTION_AIO_WORKQ)) != 0);
  ...
  if (exc_resource_threads_enabled && thread_limit > 0 &&
  ...
  /*
   * Fail thread creation if parent task is being torn down or has too many
   * threads If the caller asked for TH_OPTION_NOSUSP, also fail if the parent
   * task is suspended
   */
  if (parent_task->active == 0 || parent_task->halting ||
    ...
    kfree_data(new_thread->thread_io_stats, sizeof(struct io_stat_info));
  ...
  /* Instantiate a thread ledger. Do not fail thread creation if ledger creation
   * fails. */
  if ((new_thread->t_threadledger =
  ...
  new_thread->depress_timer =
      timer_call_alloc(thread_depress_expire, new_thread);
...
#if CONFIG_SCHED_CLUTCH
  new_thread->pri_shift =
      sched_clutch_thread_pri_shift(new_thread, new_thread->th_sched_bucket);
  ...
  *out_thread = new_thread;
  ...
  if (kdebug_enable) {
    ...
    KDBG_RELEASE(TRACE_STRING_NEWTHREAD, args[0], args[1], args[2], args[3]);
  }
...
static kern_return_t thread_create_with_options_internal(
    task_t task, thread_t *new_thread, boolean_t from_user,
    thread_create_internal_options_t options, thread_continue_t continuation) {
  kern_return_t result;
  thread_t thread;
  ...
  result =
      thread_create_internal(task, -1, continuation, NULL, options, &thread);
  if (result != KERN_SUCCESS) {
    return result;
  }
...
kern_return_t thread_create_immovable(task_t task, thread_t *new_thread) {
  return thread_create_with_options_internal(
      task, new_thread, FALSE, TH_OPTION_NONE,
...
kern_return_t thread_create_from_user(task_t task, thread_t *new_thread) {
  ...
  return thread_create_with_options_internal(
      task, new_thread, TRUE, TH_OPTION_NONE,
      (thread_continue_t)thread_bootstrap_return);
}

kern_return_t thread_create_with_continuation(task_t task, thread_t *new_thread,
                                              thread_continue_t continuation) {
  return thread_create_with_options_internal(task, new_thread, FALSE,
                                             TH_OPTION_NONE, continuation);
}
 ...
 */
static kern_return_t
thread_create_waiting_internal(task_t task, thread_continue_t continuation,
                               event_t event, block_hint_t block_hint,
                               thread_create_internal_options_t options,
                               thread_t *new_thread) {
  kern_return_t result;
  thread_t thread;
  ...
  switch (options &
          (TH_OPTION_WORKQ | TH_OPTION_AIO_WORKQ | TH_OPTION_MAINTHREAD)) {
  ...
  thread_start_in_assert_wait(thread, assert_wait_queue(event),
                              CAST_EVENT64_T(event), wait_interrupt);
  thread_mtx_unlock(thread);
...
kern_return_t main_thread_create_waiting(task_t task,
                                         thread_continue_t continuation,
                                         event_t event, thread_t *new_thread) {
  return thread_create_waiting_internal(task, continuation, event,
                                        kThreadWaitNone, TH_OPTION_MAINTHREAD,
                                        new_thread);
}

static kern_return_t
thread_create_running_internal2(task_t task, int flavor,
                                thread_state_t new_state,
                                mach_msg_type_number_t new_state_count,
                                thread_t *new_thread, boolean_t from_user) {
  kern_return_t result;
  thread_t thread;
...
#if CONFIG_MACF
  if (from_user && current_task() != task &&
      mac_proc_check_remote_thread_create(task, flavor, new_state,
                                          new_state_count) != 0) {
    return KERN_DENIED;
  }
#endif
  ...
  if (from_user) {
    result = machine_thread_state_convert_from_user(
        thread, flavor, new_state, new_state_count, NULL, 0, TSSF_FLAGS_NONE);
  }
  if (result == KERN_SUCCESS) {
    result =
        machine_thread_set_state(thread, flavor, new_state, new_state_count);
  }
  if (result != KERN_SUCCESS) {
    task_unlock(task);
    lck_mtx_unlock(&tasks_threads_lock);
...
kern_return_t thread_create_running(task_t task, int flavor,
                                    ...
                                    mach_msg_type_number_t new_state_count,
                                    thread_t *new_thread) {
  return thread_create_running_internal2(task, flavor, new_state,
                                         new_state_count, new_thread, FALSE);
}

kern_return_t thread_create_running_from_user(
    task_t task, int flavor, thread_state_t new_state,
    mach_msg_type_number_t new_state_count, thread_t *new_thread) {
  return thread_create_running_internal2(task, flavor, new_state,
                                         new_state_count, new_thread, TRUE);
}

kern_return_t thread_create_workq_waiting(task_t task,
                                          thread_continue_t continuation,
                                          thread_t *new_thread,
...
kern_return_t thread_create_aio_workq_waiting(task_t task,
                                              thread_continue_t continuation,
                                              thread_t *new_thread) {
  /*
   * Create thread, but don't pin control port just yet, in case someone calls
  ...
  return thread_create_waiting_internal(task, continuation, NULL,
                                        kThreadWaitParkedWorkQueue, options,
                                        new_thread);
}
 ...
 */
kern_return_t kernel_thread_create(thread_continue_t continuation,
                                   void *parameter, integer_t priority,
                                   thread_t *new_thread) {
  kern_return_t result;
  thread_t thread;
...
kern_return_t kernel_thread_start_priority(thread_continue_t continuation,
                                           void *parameter, integer_t priority,
                                           thread_t *new_thread) {
  kern_return_t result;
  thread_t thread;
...
kern_return_t kernel_thread_start(thread_continue_t continuation,
                                  void *parameter, thread_t *new_thread) {
  return kernel_thread_start_priority(continuation, parameter, -1, new_thread);
}

/* Separated into helper function so it can be used by THREAD_BASIC_INFO and
 * THREAD_EXTENDED_INFO */
/* it is assumed that the thread is locked by the caller */
static void retrieve_thread_basic_info(thread_t thread,
                                       thread_basic_info_t basic_info) {
  int state, flags;

  /* fill in info */

  thread_read_times(thread, &basic_info->user_time, &basic_info->system_time,
                    NULL);
  ...
  if (sched_tick_interval) {
    basic_info->cpu_usage =
        (integer_t)(((uint64_t)thread->cpu_usage * TH_USAGE_SCALE) /
                    sched_tick_interval);
  ...
  basic_info->policy =
      ((thread->sched_mode == TH_MODE_TIMESHARE) ? POLICY_TIMESHARE
                                                 : POLICY_RR);
...
kern_return_t
thread_info_internal(thread_t thread, thread_flavor_t flavor,
                     ...
                     mach_msg_type_number_t *thread_info_count) /*IN/OUT*/
{
  ...
  if (flavor == THREAD_BASIC_INFO) {
    ...
    *thread_info_count = THREAD_BASIC_INFO_COUNT;
    ...
    identifier_info =
        __IGNORE_WCASTALIGN((thread_identifier_info_t)thread_info_out);
    ...
    *thread_info_count = POLICY_RR_INFO_COUNT;
    ...
    thread_extended_info_t extended_info =
        __IGNORE_WCASTALIGN((thread_extended_info_t)thread_info_out);
    ...
    /* NOTE: This mimics fill_taskthreadinfo(), which is the function used by
     * proc_pidinfo() for the PROC_PIDTHREADINFO flavor (which can't be used on
     * corpses)
     */
    retrieve_thread_basic_info(thread, &basic_info);
    extended_info->pth_user_time =
        (((uint64_t)basic_info.user_time.seconds * NSEC_PER_SEC) +
         ((uint64_t)basic_info.user_time.microseconds * NSEC_PER_USEC));
    extended_info->pth_system_time =
        (((uint64_t)basic_info.system_time.seconds * NSEC_PER_SEC) +
    ...
    *thread_info_count = THREAD_EXTENDED_INFO_COUNT;
    ...
    dbg_info =
        __IGNORE_WCASTALIGN((thread_debug_info_internal_t)thread_info_out);
...
static void _convert_mach_to_time_value(uint64_t time_mach,
                                        time_value_t *time) {
  clock_sec_t secs;
  clock_usec_t usecs;
...
void thread_read_times(thread_t thread, time_value_t *user_time,
                       time_value_t *system_time, time_value_t *runnable_time) {
 ...
 */
kern_return_t thread_wire_internal(host_priv_t host_priv, thread_t thread,
                                   boolean_t wired, boolean_t *prev_state) {
 ...
 */
kern_return_t thread_wire(host_priv_t host_priv __unused,
                          thread_t thread __unused, boolean_t wired __unused) {
  return KERN_NOT_SUPPORTED;
}

boolean_t is_external_pageout_thread(void) {
  return current_thread() == pgo_iothread_external_state.pgo_iothread;
}

boolean_t is_vm_privileged(void) {
  return current_thread()->options & TH_OPT_VMPRIV ? TRUE : FALSE;
}

boolean_t set_vm_privilege(boolean_t privileged) {
...
void thread_floor_boost_set_promotion_locked(thread_t thread) {
  assert(thread->priority_floor_count > 0);
 ...
 *   @discussion Increase the priority of the current thread to at least
 * MINPRI_FLOOR. The boost will be mantained until a corresponding
 * thread_priority_floor_end() is called. Every call of
 * thread_priority_floor_start() needs to have a corresponding call to
 * thread_priority_floor_end() from the same thread. No thread can return to
 * userspace before calling thread_priority_floor_end().
 *
 *       NOTE: avoid to use this function. Try to use gate_t or
 * sleep_with_inheritor() instead.
 *   @result a token to be given to the corresponding
 * thread_priority_floor_end()
 */
thread_pri_floor_t thread_priority_floor_start(void) {
 ...
 *   @discussion ends the priority floor boost started with
 * thread_priority_floor_start()
 */
void thread_priority_floor_end(thread_pri_floor_t *token) {
  thread_t thread = current_thread();

  assert(thread->priority_floor_count > 0);
  assertf(token->thread == thread,
          "thread_priority_floor_end called from a different thread from "
          "thread_priority_floor_start %p %p",
          thread, token->thread);

  if ((thread->priority_floor_count-- == 1) &&
      (thread->sched_flags & TH_SFLAG_FLOOR_PROMOTED)) {
    spl_t s = splsched();
    thread_lock(thread);
 ...
 */
void thread_ast_mach_exception(thread_t thread, int os_reason,
                               exception_type_t exception_type,
                               mach_exception_data_type_t code,
                               mach_exception_data_type_t subcode, bool fatal,
                               bool ktriage) {
...
void thread_guard_violation(thread_t thread, mach_exception_data_type_t code,
                            mach_exception_data_type_t subcode, bool fatal) {
  assert(EXC_GUARD_DECODE_GUARD_TYPE(code));
  thread_ast_mach_exception(thread, OS_REASON_GUARD, EXC_GUARD, code, subcode,
                            fatal, false);
}

#if CONFIG_DEBUG_SYSCALL_REJECTION
extern void rejected_syscall_guard_ast(thread_t __unused t,
                                       mach_exception_data_type_t code,
 ...
 */
static void guard_ast(thread_t t, mach_exception_data_type_t code,
                      mach_exception_data_type_t subcode) {
...
static void thread_cputime_callback(int warning, __unused const void *arg0,
                                    __unused const void *arg1) {
  if (warning == 0) {
...
SENDING_NOTIFICATION__THIS_THREAD_IS_CONSUMING_TOO_MUCH_CPU(void) {
  ...
  printf("process %s[%d] thread %llu caught burning CPU! It used more than "
         "%d%% CPU over %u seconds\n",
...
SENDING_NOTIFICATION__TASK_HAS_TOO_MANY_THREADS(task_t task, int thread_count) {
  ...
  if (disable_exc_resource) {
    printf(
        "process %s[%d] crossed thread count high watermark (%d), EXC_RESOURCE "
...
void thread_update_io_stats(thread_t thread, int size, int io_flags) {
  ...
  if (!(io_flags & DKIO_READ)) {
    DTRACE_IO3(physical_writes, struct task *, task, uint32_t, size, int,
               io_flags);
 ...
 */
uint64_t thread_cpulimit_remaining(uint64_t now) {
  thread_t thread = current_thread();

  if ((thread->options & (TH_OPT_PROC_CPULIMIT | TH_OPT_PRVT_CPULIMIT)) == 0) {
    return UINT64_MAX;
  }

  /* Amount of time left in the current interval. */
  const uint64_t interval_remaining = ledger_get_interval_remaining(
      thread->t_threadledger, thread_ledgers.cpu_time, now);
 ...
 */
bool thread_cpulimit_interval_has_expired(uint64_t now) {
  thread_t thread = current_thread();
 ...
 */
void thread_cpulimit_restart(uint64_t now) {
  thread_t thread = current_thread();

  assert3u(thread->options & (TH_OPT_PROC_CPULIMIT | TH_OPT_PRVT_CPULIMIT), !=,
           0);
 ...
 */
int thread_get_cpulimit(int *action, uint8_t *percentage,
                        uint64_t *interval_ns) {
  int64_t abstime = 0;
  ...
  *action = 0;
  ...
  ledger_get_period(thread->t_threadledger, thread_ledgers.cpu_time,
                    interval_ns);
 ...
 */
int thread_set_cpulimit(int action, uint8_t percentage, uint64_t interval_ns) {
  thread_t thread = current_thread();
  ledger_t l;
  uint64_t limittime = 0;
  uint64_t abstime = 0;
  ...
  if (l == LEDGER_NULL) {
    /*
     * This thread doesn't yet have a per-thread ledger; so create one with the
     * CPU time entry active.
     */
    if ((l = ledger_instantiate(thread_ledger_template,
    ...
    /*
     * We are the first to create this thread's ledger, so only activate our
     * entry.
     */
  ...
  /*
   * The limit is specified as a percentage of CPU over an interval in
   * nanoseconds. Calculate the amount of CPU time that the thread needs to
   * consume in order to hit the limit.
   */
  ...
  if (action == THREAD_CPULIMIT_EXCEPTION) {
    /*
     * We don't support programming the CPU usage monitor on a task if any of
     * its threads have a per-thread blocking CPU limit configured.
     */
    ...
    /*
     * Make a note that this thread's CPU limit is being used for the task-wide
     * CPU usage monitor. We don't have to arm the callback which will trigger
     * the exception, because that was done for us in ledger_instantiate
     * (because the ledger template used has a default callback).
     */
...
uint64_t uthread_tid(struct uthread *uth) {
  if (uth) {
...
uint64_t thread_c_switch(thread_t thread) {
...
uint16_t thread_get_tag(thread_t th) { return thread_get_tag_internal(th); }

uint64_t thread_last_run_time(thread_t th) { return th->last_run_time; }
...
boolean_t thread_shared_rsrc_policy_get(thread_t thread,
                                        cluster_shared_rsrc_type_t type) {
  return thread->th_shared_rsrc_heavy_user[type] ||
         thread->th_shared_rsrc_heavy_perf_control[type];
}
...
kern_return_t thread_shared_rsrc_policy_set(thread_t thread,
  ...
  bool user = (agent == SHARED_RSRC_POLICY_AGENT_DISPATCH) ||
              (agent == SHARED_RSRC_POLICY_AGENT_SYSCTL);
  bool *thread_flags = (user) ? thread->th_shared_rsrc_heavy_user
                              : thread->th_shared_rsrc_heavy_perf_control;
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_RSRC_HEAVY_THREAD) |
           DBG_FUNC_NONE,
       SCHED_EDGE_RSRC_HEAVY_THREAD_SET, thread_tid(thread), type, agent);
...
kern_return_t
thread_shared_rsrc_policy_clear(thread_t thread,
                                cluster_shared_rsrc_type_t type,
                                shared_rsrc_policy_agent_t agent) {
  spl_t s = splsched();
  thread_lock(thread);
  ...
  KDBG(MACHDBG_CODE(DBG_MACH_SCHED_CLUTCH, MACH_SCHED_EDGE_RSRC_HEAVY_THREAD) |
           DBG_FUNC_NONE,
       SCHED_EDGE_RSRC_HEAVY_THREAD_CLR, thread_tid(thread), type, agent);
  if (thread == current_thread()) {
    if (agent == SHARED_RSRC_POLICY_AGENT_PERFCTL_QUANTUM) {
      ast_on(AST_PREEMPT);
    } else {
...
boolean_t
thread_shared_rsrc_policy_get(__unused thread_t thread,
                              __unused cluster_shared_rsrc_type_t type) {
  return false;
}

kern_return_t
thread_shared_rsrc_policy_set(thread_t thread, uint32_t index,
                              __unused cluster_shared_rsrc_type_t type,
                              __unused shared_rsrc_policy_agent_t agent) {
...
uint64_t thread_dispatchqaddr(thread_t thread) {
  ...
  if (thread->inspection == TRUE) {
    ...
    dispatchqueue_addr =
        thread_handle + get_dispatchqueue_offset_from_proc(bsd_info);
  } else {
...
uint64_t thread_wqquantum_addr(thread_t thread) {
  uint64_t thread_handle;
  task_t task;
  ...
  uint64_t wq_quantum_expiry_offset =
      get_wq_quantum_offset_from_proc(get_bsdtask_info(task));
...
uint64_t thread_rettokern_addr(thread_t thread) {
...
void thread_mtx_lock(thread_t thread) { lck_mtx_lock(&thread->mutex); }

void thread_mtx_unlock(thread_t thread) { lck_mtx_unlock(&thread->mutex); }
...
void thread_require(thread_t thread) {
  zone_id_require(ZONE_ID_THREAD, sizeof(struct thread), thread);
}

#undef thread_should_halt

boolean_t thread_should_halt(thread_t th) {
...
kern_return_t thread_set_voucher_name(mach_port_name_t voucher_name) {
  ...
  bank_get_bank_ledger_thread_group_and_persona(new_voucher, &bankledger,
                                                &banktg, &persona_id);
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      ...
      (uintptr_t)thread_tid(thread), (uintptr_t)voucher_name,
      VM_KERNEL_ADDRPERM((uintptr_t)new_voucher), persona_id, 0);
...
/*
 *  thread_get_mach_voucher - return a voucher reference for the specified
 * thread voucher
 *
 *  Conditions:  nothing locked
 *
 *  NOTE:       At the moment, there is no distinction between the current and
 * effective vouchers because we only set them at the thread level currently.
 */
kern_return_t thread_get_mach_voucher(thread_act_t thread,
...
/*
 *  thread_set_mach_voucher - set a voucher reference for the specified thread
 * voucher
 *
...
kern_return_t thread_set_mach_voucher(thread_t thread, ipc_voucher_t voucher) {
  ...
  bank_get_bank_ledger_thread_group_and_persona(voucher, &bankledger, &banktg,
                                                &persona_id);
  ...
  KERNEL_DEBUG_CONSTANT_IST(
      KDEBUG_TRACE,
      MACHDBG_CODE(DBG_MACH_IPC, MACH_THREAD_SET_VOUCHER) | DBG_FUNC_NONE,
      (uintptr_t)thread_tid(thread), (uintptr_t)MACH_PORT_NULL,
      VM_KERNEL_ADDRPERM((uintptr_t)voucher), persona_id, 0);
...
/*
 *  thread_swap_mach_voucher - swap a voucher reference for the specified thread
 * voucher
 *
 *  Conditions: callers holds a reference on the new and presumed old
 * voucher(s). nothing locked.
 *
 *  This function is no longer supported.
 */
kern_return_t thread_swap_mach_voucher(__unused thread_t thread,
...
/*
 *  thread_get_current_voucher_origin_pid - get the pid of the originator of the
 * current voucher.
 */
kern_return_t thread_get_current_voucher_origin_pid(int32_t *pid) {
 ...
 * current voucher.
 */
kern_return_t thread_get_voucher_origin_pid(thread_t thread, int32_t *pid) {
  uint32_t buf_size = sizeof(*pid);
  return mach_voucher_attr_command(
      thread->ith_voucher, MACH_VOUCHER_ATTR_KEY_BANK, BANK_ORIGINATOR_PID,
      NULL, 0, (mach_voucher_attr_content_t)pid, &buf_size);
}

/*
 *  thread_get_current_voucher_proximate_pid - get the pid of the proximate
 * process of the current voucher.
 */
kern_return_t thread_get_voucher_origin_proximate_pid(thread_t thread,
                                                      int32_t *origin_pid,
                                                      int32_t *proximate_pid) {
  int32_t origin_proximate_pids[2] = {};
  uint32_t buf_size = sizeof(origin_proximate_pids);
  kern_return_t kr = mach_voucher_attr_command(
      thread->ith_voucher, MACH_VOUCHER_ATTR_KEY_BANK,
      BANK_ORIGINATOR_PROXIMATE_PID, NULL, 0,
      (mach_voucher_attr_content_t)origin_proximate_pids, &buf_size);
  if (kr == KERN_SUCCESS) {
...
struct thread_group *thread_get_current_voucher_thread_group(thread_t thread) {
  ...
  bank_get_bank_ledger_thread_group_and_persona(thread->ith_voucher,
                                                &bankledger, &banktg, NULL);
...
extern struct workqueue *proc_get_wqptr(void *proc);

static bool task_supports_cooperative_workqueue(task_t task) {
  void *bsd_info = get_bsdtask_info(task);
...
bool thread_supports_cooperative_workqueue(thread_t thread) {
  struct uthread *uth = get_bsdthread_info(thread);
  task_t task = get_threadtask(thread);
...
static inline bool thread_has_armed_workqueue_quantum(thread_t thread) {
 ...
 */
static inline uint64_t thread_workq_quantum_size(thread_t thread) {
 ...
 */
void thread_arm_workqueue_quantum(thread_t thread) {
  /*
  ...
  WORKQ_QUANTUM_HISTORY_WRITE_ENTRY(thread, thread->workq_quantum_deadline,
                                    true);
}

/* Called by a thread on itself when it is about to park */
void thread_disarm_workqueue_quantum(thread_t thread) {
  /* The update of a workqueue quantum should always be followed by the update
   * of the AST - see explanation in kern/thread.h for synchronization of this
   * field */
 ...
 * and at AST time
 */
bool thread_has_expired_workqueue_quantum(thread_t thread, bool should_trace) {
  ...
  if (expired && should_trace) {
    WQ_TRACE(TRACE_wq_quantum_expired, runtime, thread->workq_quantum_deadline,
             0, 0);
  }
 ...
 */
void thread_evaluate_workqueue_quantum_expiry(thread_t thread) {
...
void thread_get_thread_name(thread_t th, char *name) {
  if (!name) {
    return;
  }
  if (th) {
...
processor_t thread_get_runq(thread_t thread) {
...
processor_t thread_get_runq_locked(thread_t thread) {
  thread_lock_assert(thread, LCK_ASSERT_OWNED);
  processor_t runq = thread->__runq.runq;
...
void thread_set_runq_locked(thread_t thread, processor_t new_runq) {
  thread_lock_assert(thread, LCK_ASSERT_OWNED);
...
void thread_clear_runq(thread_t thread) {
  thread_assert_runq_nonnull(thread);
  os_atomic_thread_fence(release);
  thread->__runq.runq = PROCESSOR_NULL;
}

void thread_clear_runq_locked(thread_t thread) {
  thread_lock_assert(thread, LCK_ASSERT_OWNED);
  thread_assert_runq_nonnull(thread);
  thread->__runq.runq = PROCESSOR_NULL;
}

void thread_assert_runq_null(__assert_only thread_t thread) {
  assert(thread->__runq.runq == PROCESSOR_NULL);
}

void thread_assert_runq_nonnull(thread_t thread) {
...
/*
 * thread_enable_send_importance - set/clear the SEND_IMPORTANCE thread option
 * bit.
 */
...
kern_return_t
thread_get_ipc_propagate_attr(thread_t thread,
...
void *thread_iokit_tls_get(uint32_t index) {
  assert(index < THREAD_SAVE_IOKIT_TLS_COUNT);
  return current_thread()->saved.iokit.tls[index];
}

void thread_iokit_tls_set(uint32_t index, void *data) {
  assert(index < THREAD_SAVE_IOKIT_TLS_COUNT);
...
integer_t thread_kern_get_pri(thread_t thr) { return thr->base_pri; }

void thread_kern_set_pri(thread_t thr, integer_t pri) {
  sched_set_kernel_thread_priority(thr, pri);
}

integer_t thread_kern_get_kernel_maxpri(void) { return MAXPRI_KERNEL; }
/*
 ...
 */
static void thread_port_with_flavor_no_senders(ipc_port_t port,
...
void thread_self_region_page_shift_set(int pgshift) {
  /*
...
__startup_func __static_testable void ctid_table_init(void) {
  /*
 ...
 */
static ctid_t ctid_mangle(compact_id_t cid) {
  return (cid == ctid_nonce ? CTID_MASK : cid) ^ ctid_nonce;
}

static compact_id_t ctid_unmangle(ctid_t ctid) {
  ctid ^= ctid_nonce;
  return ctid == CTID_MASK ? ctid_nonce : ctid;
}

void ctid_table_add(thread_t thread) {
  compact_id_t cid;
...
void ctid_table_remove(thread_t thread) {
...
thread_t ctid_get_thread_unsafe(ctid_t ctid) {
  if (ctid && ctid <= CTID_MAX_THREAD_NUMBER &&
      compact_id_slab_valid(&ctid_table, ctid_unmangle(ctid))) {
    return *compact_id_resolve(&ctid_table, ctid_unmangle(ctid));
  }
  return THREAD_NULL;
}

thread_t ctid_get_thread(ctid_t ctid) {
...
ctid_t thread_get_ctid(thread_t thread) { return thread->ctid; }
 ...
 *
 */
kern_return_t thread_process_signature(thread_t thread, task_t task) {
  return machine_thread_process_signature(thread, task);
}

#if CONFIG_SPTM

void thread_associate_txm_thread_stack(uintptr_t thread_stack) {
...
void thread_disassociate_txm_thread_stack(uintptr_t thread_stack) {
  thread_t self = current_thread();
...
uintptr_t thread_get_txm_thread_stack(void) {
...
uint16_t dtrace_get_thread_inprobe(thread_t thread) {
  if (thread != THREAD_NULL) {
...
vm_offset_t thread_get_kernel_stack(thread_t thread) {
  if (thread != THREAD_NULL) {
...
#if KASAN
struct kasan_thread_data *kasan_get_thread_data(thread_t thread) {
  return &thread->kasan_data;
}
#endif

#if CONFIG_KCOV
kcov_thread_data_t *kcov_get_thread_data(thread_t thread) {
 ...
 *
 * Coverage sanitizer instruments every function including those that
 * participates in stack handoff between threads. There is a window in which CPU
 * still holds old values but stack has been handed over to anoher thread
 * already. In this window kernel_stack is 0 but CPU still uses the original
 * stack (until contex switch occurs). The original kernel_stack value is
 * preserved in ksancov_stack during this window.
 */
vm_offset_t kcov_stksz_get_thread_stkbase(thread_t thread) {
  if (thread != THREAD_NULL) {
...
vm_offset_t kcov_stksz_get_thread_stksize(thread_t thread) {
  if (thread != THREAD_NULL) {
...
void kcov_stksz_set_thread_stack(thread_t thread, vm_offset_t stack) {
  kcov_thread_data_t *data = kcov_get_thread_data(thread);
...
void dtrace_set_thread_inprobe(thread_t thread, uint16_t inprobe) {
  if (thread != THREAD_NULL) {
...
void dtrace_thread_bootstrap(void) {
  ...
  if (task->thread_count == 1) {
    ...
    if (thread->t_dtrace_flags & TH_DTRACE_EXECSUCCESS) {
      ...
      KDBG(BSDDBG_CODE(DBG_BSD_PROC, BSD_PROC_EXEC), task_pid(task),
           kdp_task_exec_meta_flags(task));
    }
    DTRACE_PROC(start);
  }
...
void dtrace_thread_didexec(thread_t thread) {
  thread->t_dtrace_flags |= TH_DTRACE_EXECSUCCESS;
}
#endif /* CONFIG_DTRACE */
