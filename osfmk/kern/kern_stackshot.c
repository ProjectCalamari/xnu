...
TUNABLE_DEV_WRITEABLE(unsigned int, stackshot_single_thread,
                      "stackshot_single_thread", 0);
...
struct stackshot_workitem {
  ...
  int sswi_idx; /* The index of this job, used for ordering kcdata across
                   multiple queues. */
};
...
struct freelist_entry {
  ...
  size_t fl_size;                 /* Size of the entry (must be >= sizeof(struct
                                     freelist_entry)) */
};
...
struct stackshot_buffer {
  ...
  size_t _Atomic ssb_overhead; /* Total amount ever freed (even if re-allocated
                                  from freelist) */
};
 ...
 * indexes into the parallel arrays, with -1 ending the chain.  Array indices
 * are allocated using a bump allocator.
 *
...
struct port_label_hash {
  ...
  int16_t *plh_hash; /* (1 << STACKSHOT_PLH_SHIFT) entry hash table: hash(ptr)
                        -> array index */
...
#define plh_lock(plh)                                                          \
  while (!os_atomic_cmpxchg(&(plh)->plh_lock, 0, 1, acquire)) {                \
    loop_wait();                                                               \
  }
...
struct stackshot_cpu_context {
  ...
  linked_kcdata_descriptor_t
      scc_kcdata_head; /* See `linked_kcdata_alloc_callback */
  linked_kcdata_descriptor_t
      scc_kcdata_tail;         /* See `linked_kcdata_alloc_callback */
...
struct stackshot_context {
  ...
  /* State & Errors */
  stackshot_state_t _Atomic sc_state; /* Only modified by calling CPU, main CPU,
                                         or panicking CPU. See comment above
  ...
  /* Buffers & Queues */
  struct stackshot_buffer
      __counted_by(num_buffers) sc_buffers[STACKSHOT_NUM_BUFFERS];
  size_t sc_num_buffers;
  struct stackshot_workqueue __counted_by(STACKSHOT_NUM_WORKQUEUES)
      sc_workqueues[STACKSHOT_NUM_WORKQUEUES];
...
struct stackshot_trace_buffer {
  ...
  size_t sstb_size;
  struct stackshot_trace_entry __counted_by(STACKSHOT_DEBUG_TRACEBUF_SIZE)
      sstb_entries[STACKSHOT_DEBUG_TRACEBUF_SIZE];
};
...
static struct {
  ...
  uint64_t stackshots_duration; /* total abs time spent in stackshot_trap()
                                   since boot */
} stackshot_stats = {0};
...
uint32_t stackshot_estimate_adj =
    25; /* experiment factor: 0-100, adjust our estimate up by this amount */
...
void *kernel_stackshot_buf = NULL; /* Pointer to buffer for stackshots triggered
                                      from the kernel and retrieved later */
...
static vm_offset_t stackshot_find_phys(vm_map_t map, vm_offset_t target_addr,
                                       kdp_fault_flags_t fault_flags,
                                       uint32_t *kdp_fault_result_flags);
static boolean_t stackshot_copyin(vm_map_t map, uint64_t uaddr, void *dest,
                                  size_t size, boolean_t try_fault,
                                  uint32_t *kdp_fault_result);
static int stackshot_copyin_string(task_t task, uint64_t addr, char *buf,
                                   int buf_sz, boolean_t try_fault,
                                   uint32_t *kdp_fault_results);
static boolean_t stackshot_copyin_word(task_t task, uint64_t addr,
                                       uint64_t *result, boolean_t try_fault,
                                       uint32_t *kdp_fault_results);
static uint64_t proc_was_throttled_from_task(task_t task);
static void stackshot_thread_wait_owner_info(thread_t thread,
                                             thread_waitinfo_v2_t *waitinfo);
static int stackshot_thread_has_valid_waitinfo(thread_t thread);
static void stackshot_thread_turnstileinfo(thread_t thread,
                                           thread_turnstileinfo_v2_t *tsinfo);
static int stackshot_thread_has_valid_turnstileinfo(thread_t thread);
static uint32_t get_stackshot_estsize(uint32_t prev_size_hint, uint32_t adj,
                                      uint64_t trace_flags, pid_t target_pid);
static kern_return_t
kdp_snapshot_preflight_internal(struct kdp_snapshot_args args);

#if CONFIG_COALITIONS
static void stackshot_coalition_jetsam_count(void *arg, int i,
                                             coalition_t coal);
static void stackshot_coalition_jetsam_snapshot(void *arg, int i,
                                                coalition_t coal);
#endif /* CONFIG_COALITIONS */

#if CONFIG_THREAD_GROUPS
static void stackshot_thread_group_count(void *arg, int i,
                                         struct thread_group *tg);
static void stackshot_thread_group_snapshot(void *arg, int i,
                                            struct thread_group *tg);
...
extern void proc_starttime_kdp(void *p, uint64_t *tv_sec, uint64_t *tv_usec,
                               uint64_t *abstime);
extern void proc_archinfo_kdp(void *p, cpu_type_t *cputype,
...
extern void proc_memstat_data_kdp(void *p, int32_t *current_memlimit,
                                  int32_t *prio_effective,
                                  int32_t *prio_requested,
                                  int32_t *prio_assertion);
...
extern int count_busy_buffers(
    void); /* must track with declaration in bsd/sys/buf_internal.h */

#if CONFIG_TELEMETRY
extern kern_return_t stack_microstackshot(user_addr_t tracebuf,
                                          uint32_t tracebuf_size,
                                          uint32_t flags, int32_t *retval);
...
#define MAX_DYLD_COMPACTINFO                                                   \
  (20 * 1024) // max bytes of compactinfo to include per proc/shared region
...
#define STACKSHOT_SUBSYS_ASSERT_LOCKED()                                       \
...
#define kcd_end_address(kcd)                                                   \
  ((void *)((uint64_t)((kcd)->kcd_addr_begin) +                                \
            kcdata_memory_get_used_bytes((kcd))))
#define kcd_max_address(kcd)                                                   \
  ((void *)((kcd)->kcd_addr_begin + (kcd)->kcd_length))
/*
...
static __attribute__((noreturn, noinline)) void stackshot_panic_spin(void) {
 ...
 */
static inline void stackshot_panic_guard(void) {
  if (__improbable(os_atomic_load(&stackshot_ctx.sc_state, relaxed) ==
                   SS_PANICKED)) {
 ...
 */
void stackshot_cpu_signal_panic(void) {
 ...
 */
static inline void stackshot_set_error(kern_return_t error) {
  stackshot_state_t cur_state;
  stackshot_state_t err_state = SS_MKERR(error);
  if (__improbable(!os_atomic_cmpxchgv(&stackshot_ctx.sc_state, SS_RUNNING,
                                       err_state, &cur_state, seq_cst))) {
    if (cur_state == SS_SETUP) {
      os_atomic_cmpxchg(&stackshot_ctx.sc_state, SS_SETUP, err_state, seq_cst);
    } else {
      /* Our state is something other than SS_RUNNING or SS_SETUP... Check for
       * panic. */
      stackshot_panic_guard();
    }
  }
 ...
 */
__result_use_check static inline kern_return_t stackshot_status_check(void) {
  ...
  if (__improbable(SS_STATE(state) == SS_ERRORED)) {
    ...
    assert(err != KERN_SUCCESS); /* SS_ERRORED should always store an associated
                                    error code. */
    return err;
  }
...
#if STACKSHOT_COLLECTS_DIAGNOSTICS
static void stackshot_trace(int line_no, mach_vm_address_t data) {
  struct stackshot_trace_buffer *buffer = PERCPU_GET(stackshot_trace_buffer);
  buffer->sstb_entries[buffer->sstb_tail_idx] =
      (struct stackshot_trace_entry){.sste_line_no = line_no,
                                     .sste_timestamp = mach_continuous_time(),
                                     .sste_data = data};
  buffer->sstb_tail_idx =
      (buffer->sstb_tail_idx + 1) % STACKSHOT_DEBUG_TRACEBUF_SIZE;
  buffer->sstb_size = MIN(buffer->sstb_size + 1, STACKSHOT_DEBUG_TRACEBUF_SIZE);
}
#define STACKSHOT_TRACE(data)                                                  \
  stackshot_trace(__LINE__, (mach_vm_address_t)(data))
...
#define freelist_lock(buffer)                                                  \
  while (!os_atomic_cmpxchg(&buffer->ssb_freelist_lock, 0, 1, acquire)) {      \
    loop_wait();                                                               \
  }
#define freelist_unlock(buffer)                                                \
  os_atomic_store(&buffer->ssb_freelist_lock, 0, release);
 ...
 */
static void *stackshot_freelist_alloc(size_t size,
  ...
  while (*cur_freelist != NULL) {
    if (((*cur_freelist)->fl_size >= size) &&
        ((best_freelist == NULL) ||
         ((*best_freelist)->fl_size > (*cur_freelist)->fl_size))) {
  ...
  if (best_freelist != NULL) {
    ...
    /* If there's enough unused space at the end of this entry, we should make a
     * new one */
    if (((*best_freelist)->fl_size - size) > sizeof(struct freelist_entry)) {
      struct freelist_entry *new_freelist =
          (struct freelist_entry *)((mach_vm_address_t)*best_freelist + size);
      *new_freelist =
          (struct freelist_entry){.fl_next = (*best_freelist)->fl_next,
 ...
 */
static void *stackshot_buffer_alloc(size_t size,
                                    struct stackshot_buffer *buffer,
                                    kern_return_t *error) {
  ...
  os_atomic_rmw_loop(&buffer->ssb_used, o_used, new_used, relaxed, {
    ...
    if (new_used > buffer->ssb_size) {
      os_atomic_rmw_loop_give_up(
          return stackshot_freelist_alloc(size, buffer, error));
    }
  });
 ...
 * `stackshot_alloc_arr` macros.
 */
__result_use_check static void *
stackshot_best_buffer_alloc(size_t size, kern_return_t *error) {
  ...
  ret =
      stackshot_buffer_alloc(size, &stackshot_ctx.sc_buffers[my_cluster], &err);

  /* Try other buffers now. */
  if (err != KERN_SUCCESS) {
    for (size_t buf_idx = 0; buf_idx < stackshot_ctx.sc_num_buffers;
         buf_idx++) {
      if ((buf_idx == my_cluster) ||
          (stackshot_ctx.sc_buffers[buf_idx].ssb_ptr == NULL)) {
        continue;
      }

      ret = stackshot_buffer_alloc(size, &stackshot_ctx.sc_buffers[buf_idx],
                                   &err);
      if (err == KERN_SUCCESS) {
#if STACKSHOT_COLLECTS_LATENCY_INFO
 ...
 */
static void stackshot_buffer_free(void *ptr, struct stackshot_buffer *buffer,
                                  size_t size) {
  stackshot_panic_guard();
  ...
  *((struct freelist_entry *)ptr) =
      (struct freelist_entry){.fl_size = size, .fl_next = buffer->ssb_freelist};
 ...
 */
void *stackshot_alloc_with_size(size_t size, kern_return_t *err) {
  void *ptr;
  assert(err != NULL);
 ...
 */
__result_use_check static kern_return_t
linked_kcdata_init(linked_kcdata_descriptor_t descriptor, size_t min_size,
                   unsigned int data_type, unsigned int flags) {
  void *buf_ptr;
  kern_return_t error;
  ...
  error =
      kcdata_memory_static_init(&descriptor->kcdata, (mach_vm_address_t)buf_ptr,
                                data_type, buf_size, flags);
  if (error != KERN_SUCCESS) {
    return error;
  }
...
static void stackshot_kcdata_free_unused(kcdata_descriptor_t descriptor) {
  /*
  ...
  if (free_size > 0) {
    ...
    buffer = 0;
#endif /* !__arm64__ */
    stackshot_buffer_free((void *)descriptor->kcd_addr_end,
                          &stackshot_ctx.sc_buffers[buffer], free_size);
...
linked_kcdata_alloc_callback(kcdata_descriptor_t descriptor, size_t min_size) {
  ...
  /* This callback should ALWAYS be coming from our per-cpu kcdata. If not,
   * something has gone horribly wrong.*/
  stackshot_panic_guard();
  assert(descriptor == stackshot_kcdata_p);

  /* Free the unused space in the buffer and copy it to the tail of the linked
   * kcdata list. */
  ...
  /* It doesn't matter what we mark the data type as - we're throwing it away
   * when weave the data together anyway. */
  error =
      linked_kcdata_init(new_kcdata, min_size, KCDATA_BUFFER_BEGIN_STACKSHOT,
                         descriptor->kcd_flags);
  if (error != KERN_SUCCESS) {
    return NULL;
  }
 ...
 */
__result_use_check static kern_return_t stackshot_new_linked_kcdata(void) {
  kern_return_t error;

  stackshot_panic_guard();
  assert(!stackshot_ctx.sc_panic_stackshot);

  stackshot_cpu_ctx.scc_kcdata_head =
      stackshot_alloc(struct linked_kcdata_descriptor, &error);
  if (error != KERN_SUCCESS) {
    return error;
  }

  kcd_exit_on_error(linked_kcdata_init(
      stackshot_cpu_ctx.scc_kcdata_head, 0, KCDATA_BUFFER_BEGIN_STACKSHOT,
 ...
 */
static void stackshot_finalize_linked_kcdata(void) {
  stackshot_panic_guard();
  assert(!stackshot_ctx.sc_panic_stackshot);
...
__private_extern__ void stackshot_init(void) {
  ...
  stackshot_max_fault_time =
      ((KDP_FAULT_PATH_MAX_TIME_PER_STACKSHOT_NSECS * timebase.denom) /
       timebase.numer);

  max_tracebuf_size =
      MAX(max_tracebuf_size,
          ((ROUNDUP(max_mem, GIGABYTES) / GIGABYTES) * TRACEBUF_SIZE_PER_GB));
 ...
 * initialized.
 */
static kern_return_t stackshot_trap(void) {
  kern_return_t rv;

#if defined(__x86_64__)
  /*
   * Since mp_rendezvous and stackshot both attempt to capture cpus then perform
   * an operation, it's essential to apply mutual exclusion to the other when
   * one mechanism is in operation, lest there be a deadlock as the mechanisms
   * race to capture CPUs.
   *
   * Further, we assert that invoking stackshot from mp_rendezvous*() is not
   * allowed, so we check to ensure there there is no rendezvous in progress
   * before trying to grab the lock (if there is, a deadlock will occur when we
   * try to grab the lock).  This is accomplished by setting
   * cpu_rendezvous_in_progress to TRUE in the mp rendezvous action function. If
   * stackshot_trap() is called by a subordinate of the call chain within the mp
   * rendezvous action, this flag will be set and can be used to detect the
   * inevitable deadlock that would occur if this thread tried to grab the
   * rendezvous lock.
   */
  ...
  rv = DebuggerTrapWithState(DBOP_STACKSHOT, NULL, NULL, NULL, 0, NULL, FALSE,
                             0, NULL);

  stackshot_stats.last_abs_end = mach_absolute_time();
  stackshot_stats.stackshots_taken++;
  stackshot_stats.stackshots_duration +=
      (stackshot_stats.last_abs_end - stackshot_stats.last_abs_start);
...
extern void stackshot_get_timing(uint64_t *last_abs_start,
                                 uint64_t *last_abs_end, uint64_t *count,
                                 uint64_t *total_duration);
void stackshot_get_timing(uint64_t *last_abs_start, uint64_t *last_abs_end,
                          uint64_t *count, uint64_t *total_duration) {
...
kern_return_t stack_snapshot_from_kernel(int pid, void *buf, uint32_t size,
                                         uint64_t flags,
  ...
  args = (struct kdp_snapshot_args){.pid = pid,
                                    .buffer = buf,
                                    .buffer_size = size,
                                    .flags = flags,
  ...
  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_STACKSHOT, STACKSHOT_KERN_RECORD) |
                   DBG_FUNC_START,
  ...
  ca_event_t new_event =
      CA_EVENT_ALLOCATE_FLAGS(bad_stackshot_upper16, Z_NOWAIT);
  if (new_event) {
    if (os_atomic_cmpxchg(&rdar_126582377_event, NULL, new_event, relaxed) ==
        0) {
...
#if CONFIG_EXCLAVES
  /* stackshot trap should only finish successfully or with no pending Exclave
   * threads */
   ...
   * (We have to do this here for the special interrupt disable timeout case to
   * work)
   */
...
#if CONFIG_TELEMETRY
kern_return_t stack_microstackshot(user_addr_t tracebuf, uint32_t tracebuf_size,
                                   uint32_t flags, int32_t *retval) {
...
stackshot_get_workitem(struct stackshot_workqueue *queue) {
  ...
  /* note: this relies on give_up not performing the write, just bailing out
   * immediately */
 ...
 */
static kern_return_t stackshot_put_workitem(struct stackshot_workitem item) {
  struct stackshot_workqueue *queue;

  /* Put in higher queue if task has more threads, with highest queue having >=
   * STACKSHOT_HARDEST_THREADCOUNT threads */
  size_t queue_idx =
      ((item.sswi_task->thread_count * (STACKSHOT_NUM_WORKQUEUES - 1)) /
       STACKSHOT_HARDEST_THREADCOUNT);
...
#define calc_num_linked_kcdata_frames(size, kcdata_size)                       \
  (1 + ((size) - 1) / (kcdata_size))
#define calc_linked_kcdata_size(size, kcdata_size)                             \
  (calc_num_linked_kcdata_frames((size), (kcdata_size)) *                      \
   ((kcdata_size) + sizeof(struct linked_kcdata_descriptor)))

#define TASK_UUID_AVG_SIZE                                                     \
  (16 * sizeof(uuid_t)) /* Average space consumed by UUIDs/task */
#define TASK_SHARED_CACHE_AVG_SIZE                                             \
  (128) /* Average space consumed by task shared cache info */
#define sizeof_if_traceflag(a, flag)                                           \
  (((trace_flags & (flag)) != 0) ? sizeof(a) : 0)
 ...
 */
static uint32_t get_stackshot_est_tasksize(uint64_t trace_flags) {
  size_t total_size;
  size_t threads_per_task = (((threads_count + terminated_threads_count) - 1) /
                             (tasks_count + terminated_tasks_count)) +
                            1;
  size_t est_thread_size =
      sizeof(struct thread_snapshot_v4) + 42 * sizeof(uintptr_t);
  size_t est_task_size =
      sizeof(struct task_snapshot_v3) + TASK_UUID_AVG_SIZE +
      ...
      sizeof_if_traceflag(sizeof(uintptr_t) * STACKSHOT_PAGETABLE_BUFSZ,
                          STACKSHOT_PAGE_TABLES) +
      sizeof_if_traceflag(struct instrs_cycles_snapshot_v2,
...
static uint32_t get_stackshot_estsize(uint32_t prev_size_hint, uint32_t adj,
  ...
  bool process_scoped =
      ((target_pid != -1) &&
       ((trace_flags & STACKSHOT_INCLUDE_DRIVER_THREADS_IN_KERNEL) == 0));
  ...
  uint64_t est_preamble_size =
      calc_linked_kcdata_size(8192 * 4, est_kcdata_size);
  uint64_t est_postamble_size =
      calc_linked_kcdata_size(8192 * 2, est_kcdata_size);
  ...
  est_extra_size +=
      real_ncpus * MAX_FRAMES * sizeof(uintptr_t); /* Stacktrace buffers */
  est_extra_size += FUDGED_SIZE(tasks_count, 10) * sizeof(uintptr_t) *
                    STACKSHOT_NUM_WORKQUEUES; /* Work queues */
  est_extra_size += sizeof_if_traceflag(
      sizeof(uintptr_t) * STACKSHOT_PAGETABLE_BUFSZ * real_ncpus,
      STACKSHOT_PAGE_TABLES);

  thread_and_task_total =
      calc_linked_kcdata_size(est_task_size, est_kcdata_size);
  if (!process_scoped) {
    thread_and_task_total *= tasks_count;
  }
  size = thread_and_task_total + est_preamble_size + est_postamble_size +
         est_extra_size;            /* estimate */
  ...
  size =
      MIN(size, VM_MAP_TRUNC_PAGE(UINT32_MAX, PAGE_MASK)); /* avoid overflow */
  estimated_size =
      (uint32_t)VM_MAP_ROUND_PAGE(size, PAGE_MASK); /* round to pagesize */
 ...
 */
__result_use_check static kern_return_t
stackshot_copy_linked_kcdata(kcdata_descriptor_t final_kcdata,
                             linked_kcdata_descriptor_t linked_kcdata) {
  kern_return_t error = KERN_SUCCESS;
  ...
  while (linked_kcdata) {
    ...
    /* Every item in the linked kcdata should have a header tag of type
     * KCDATA_BUFFER_BEGIN_STACKSHOT. */
    assert(((struct kcdata_item *)cur_kcdata->kcd_addr_begin)->type ==
           KCDATA_BUFFER_BEGIN_STACKSHOT);
    assert((final_kcdata->kcd_addr_begin + final_kcdata->kcd_length) >
           final_kcdata->kcd_addr_end);
    size_t header_size =
        sizeof(kcdata_item_t) + kcdata_calc_padding(sizeof(kcdata_item_t));
    size_t size =
        cur_kcdata->kcd_addr_end - cur_kcdata->kcd_addr_begin - header_size;
    size_t free = (final_kcdata->kcd_length + final_kcdata->kcd_addr_begin) -
                  final_kcdata->kcd_addr_end;
    if (free < size) {
      error = KERN_INSUFFICIENT_BUFFER_SIZE;
      goto error_exit;
    ...
    error =
        kcdata_memcpy(final_kcdata, final_kcdata->kcd_addr_end,
                      (void *)(cur_kcdata->kcd_addr_begin + header_size), size);
    if (error != KERN_SUCCESS) {
      goto error_exit;
    }
 ...
 * Only used by stackshot_finalize_kcdata and
 * stackshot_finalize_singlethreaded_kcdata.
 */
__result_use_check static kern_return_t
stackshot_push_duration_and_latency(kcdata_descriptor_t kcdata) {
  ...
  bool use_fault_path =
      ((stackshot_flags &
        (STACKSHOT_ENABLE_UUID_FAULTING | STACKSHOT_ENABLE_BT_FAULTING)) != 0);
#if STACKSHOT_COLLECTS_LATENCY_INFO
  ...
  if (use_fault_path) {
    ...
    percpu_foreach_base(base) {
      struct stackshot_cpu_context *cpu_ctx =
          PERCPU_GET_WITH_BASE(base, stackshot_cpu_ctx_percpu);
      if (!cpu_ctx->scc_did_work) {
        continue;
      }
      stats.sfs_pages_faulted_in +=
          cpu_ctx->scc_fault_stats.sfs_pages_faulted_in;
      stats.sfs_time_spent_faulting +=
          cpu_ctx->scc_fault_stats.sfs_time_spent_faulting;
      stats.sfs_stopped_faulting =
          stats.sfs_stopped_faulting ||
          cpu_ctx->scc_fault_stats.sfs_stopped_faulting;
    }
  ...
  if (collect_latency_info) {
    ...
    kcd_exit_on_error(kcdata_get_memory_addr_for_array(
        kcdata, STACKSHOT_KCTYPE_LATENCY_INFO_CPU,
        sizeof(struct stackshot_latency_cpu), num_working_cpus, &out_addr));
    percpu_foreach_base(base) {
      if (PERCPU_GET_WITH_BASE(base, stackshot_cpu_ctx_percpu)->scc_did_work) {
        kcdata_memcpy(kcdata, out_addr,
                      PERCPU_GET_WITH_BASE(base, stackshot_cpu_latency_percpu),
    ...
    kcdata_compression_window_open(kcdata);
    kcd_exit_on_error(kcdata_get_memory_addr_for_array(
        kcdata, STACKSHOT_KCTYPE_LATENCY_INFO_BUFFER,
        sizeof(struct stackshot_latency_buffer), stackshot_ctx.sc_num_buffers,
        &out_addr));

    /* Add up buffer info */
    for (size_t buf_idx = 0; buf_idx < stackshot_ctx.sc_num_buffers;
         buf_idx++, out_addr += sizeof(buffer_latency)) {
...
#if defined(__arm64__)
      ml_topology_cluster_t *cluster =
          &ml_get_topology_info()->clusters[buf_idx];
      ...
      kcd_exit_on_error(kcdata_memcpy(kcdata, out_addr, &buffer_latency,
                                      sizeof(buffer_latency)));
    ...
    kcd_exit_on_error(kcdata_push_data(kcdata, STACKSHOT_KCTYPE_LATENCY_INFO,
                                       sizeof(stackshot_ctx.sc_latency),
                                       &stackshot_ctx.sc_latency));
  }
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */

  if ((stackshot_flags & STACKSHOT_DO_COMPRESS) == 0) {
    assert(!stackshot_ctx.sc_panic_stackshot);
    kcd_exit_on_error(kcdata_get_memory_addr(
        kcdata, STACKSHOT_KCTYPE_STACKSHOT_DURATION,
    ...
    stackshot_duration_outer =
        (unaligned_u64 *)&duration_p->stackshot_duration_outer;
    kcd_exit_on_error(kcdata_add_uint64_with_description(
        kcdata, stackshot_tries, "stackshot_tries"));
  } else {
    kcd_exit_on_error(kcdata_push_data(
        kcdata, STACKSHOT_KCTYPE_STACKSHOT_DURATION,
        sizeof(stackshot_ctx.sc_duration), &stackshot_ctx.sc_duration));
    stackshot_duration_outer = NULL;
  }
 ...
 */
__result_use_check static kern_return_t stackshot_alloc_final_kcdata(void) {
  vm_offset_t final_kcdata_buffer = 0;
  kern_return_t error = KERN_SUCCESS;
  uint32_t hdr_tag = (stackshot_flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT)
  ...
  if ((error = kmem_alloc(
           kernel_map, &final_kcdata_buffer, stackshot_args.buffer_size,
           KMA_ZERO | KMA_DATA_SHARED, VM_KERN_MEMORY_DIAG)) != KERN_SUCCESS) {
    os_log_error(OS_LOG_DEFAULT,
                 "stackshot: final allocation failed: %d, allocating %u bytes "
                 "of %u max, try %llu\n",
                 (int)error, stackshot_args.buffer_size, max_tracebuf_size,
                 stackshot_tries);
    return KERN_RESOURCE_SHORTAGE;
  }

  stackshot_ctx.sc_finalized_kcdata = kcdata_memory_alloc_init(
      final_kcdata_buffer, hdr_tag, stackshot_args.buffer_size,
 ...
 */
static void stackshot_free_final_kcdata(void) {
  if (stackshot_ctx.sc_is_singlethreaded ||
      (stackshot_ctx.sc_finalized_kcdata == NULL)) {
    return;
  }

  kmem_free(kernel_map, stackshot_ctx.sc_finalized_kcdata->kcd_addr_begin,
            stackshot_args.buffer_size);
 ...
 */
__result_use_check static kern_return_t stackshot_collect_kcdata(void) {
  kern_return_t error = 0;
  uint32_t hdr_tag;

  assert(!stackshot_ctx.sc_panic_stackshot &&
         !stackshot_ctx.sc_is_singlethreaded);
  LCK_MTX_ASSERT(&stackshot_subsys_mutex, LCK_MTX_ASSERT_OWNED);
  ...
  if (stackshot_flags & STACKSHOT_DO_COMPRESS) {
    hdr_tag = (stackshot_flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT)
                  ? KCDATA_BUFFER_BEGIN_DELTA_STACKSHOT
                  : KCDATA_BUFFER_BEGIN_STACKSHOT;
    kcd_exit_on_error(kcdata_init_compress(stackshot_ctx.sc_finalized_kcdata,
                                           hdr_tag, kdp_memcpy, KCDCT_ZLIB));
  }

  /* Copy over all of the pre task-iteration kcdata (to preserve order as if it
   * were single-threaded) */
  kcd_exit_on_error(stackshot_copy_linked_kcdata(
      stackshot_ctx.sc_finalized_kcdata, stackshot_ctx.sc_pretask_kcdata));
  ...
  while (true) {
    ...
    kcd_exit_on_error(stackshot_copy_linked_kcdata(
        stackshot_ctx.sc_finalized_kcdata, next_item->sswi_data));
  }

  /* Write post-task kcdata */
  kcd_exit_on_error(stackshot_copy_linked_kcdata(
      stackshot_ctx.sc_finalized_kcdata, stackshot_ctx.sc_posttask_kcdata));
error_exit:
  if (error != KERN_SUCCESS) {
 ...
 * Called with interrupts enabled, stackshot subsys lock held.
 */
__result_use_check static kern_return_t stackshot_finalize_kcdata(void) {
  kern_return_t error = 0;
  ...
  kcd_exit_on_error(
      stackshot_push_duration_and_latency(stackshot_ctx.sc_finalized_kcdata));

  /* Note: exactly 0 or 1 call to something pushing more data can be called
   * after kcd_finalize_compression */
  kcd_finalize_compression(stackshot_ctx.sc_finalized_kcdata);
  kcd_exit_on_error(kcdata_add_uint64_with_description(
      stackshot_ctx.sc_finalized_kcdata, stackshot_flags,
      "stackshot_out_flags"));
  kcd_exit_on_error(kcdata_write_buffer_end(stackshot_ctx.sc_finalized_kcdata));

  stackshot_ctx.sc_bytes_traced =
      (uint32_t)kcdata_memory_get_used_bytes(stackshot_ctx.sc_finalized_kcdata);
  stackshot_ctx.sc_bytes_uncompressed =
      (uint32_t)kcdata_memory_get_uncompressed_bytes(
          stackshot_ctx.sc_finalized_kcdata);
  ...
  memcpy(stackshot_args.buffer,
         (void *)stackshot_ctx.sc_finalized_kcdata->kcd_addr_begin,
         stackshot_args.buffer_size);

  /* Fix duration_outer offset */
  if (stackshot_duration_outer != NULL) {
    stackshot_duration_outer =
        (unaligned_u64 *)((mach_vm_address_t)stackshot_args.buffer +
                          ((mach_vm_address_t)stackshot_duration_outer -
                           stackshot_ctx.sc_finalized_kcdata->kcd_addr_begin));
  }
...
/*
 * stackshot_remap_buffer:	Utility function to remap bytes_traced bytes
 * starting at stackshotbuf into the current task's user space and subsequently
 * copy out the address at which the buffer has been mapped in user space to
 * out_buffer_addr.
 *
 * Inputs:			stackshotbuf - pointer to the original buffer in
 * the kernel's address space bytes_traced - length of the buffer to remap
 * starting from stackshotbuf out_buffer_addr - pointer to placeholder where
 * newly mapped buffer will be mapped. out_size_addr - pointer to be filled in
 * with the size of the buffer
 *
 * Outputs:			ENOSPC if there is not enough free space in the
 * task's address space to remap the buffer EINVAL for all other errors returned
 * by task_remap_buffer/mach_vm_remap an error from copyout
 */
static kern_return_t stackshot_remap_buffer(void *stackshotbuf,
  ...
  error = mach_vm_remap(current_map(), &stackshotbuf_user_addr, bytes_traced, 0,
                        VM_FLAGS_ANYWHERE, kernel_map,
                        (mach_vm_offset_t)stackshotbuf, FALSE, &cur_prot,
                        &max_prot, VM_INHERIT_DEFAULT);
  /*
   * If the call to mach_vm_remap fails, we return the appropriate converted
   * error
   */
  if (error == KERN_SUCCESS) {
    /* If the user addr somehow didn't get set, we should make sure that we
     * fail, and (eventually) panic on development kernels to find out why
     */
    if (stackshotbuf_user_addr == (mach_vm_offset_t)NULL) {
#if DEVELOPMENT || DEBUG
      os_log_error(OS_LOG_DEFAULT,
                   "stackshot: mach_vm_remap succeeded with NULL\n");
#endif // DEVELOPMENT || DEBUG
      return KERN_FAILURE;
    }

    /*
     * If we fail to copy out the address or size of the new buffer, we remove
     * the buffer mapping that we just made in the task's user space.
     */
    error =
        copyout(CAST_DOWN(void *, &stackshotbuf_user_addr),
                (user_addr_t)out_buffer_addr, sizeof(stackshotbuf_user_addr));
    if (error != KERN_SUCCESS) {
      mach_vm_deallocate(get_task_map(current_task()), stackshotbuf_user_addr,
                         (mach_vm_size_t)bytes_traced);
      return error;
    }
    error = copyout(&bytes_traced, (user_addr_t)out_size_addr,
                    sizeof(bytes_traced));
    if (error != KERN_SUCCESS) {
      mach_vm_deallocate(get_task_map(current_task()), stackshotbuf_user_addr,
                         (mach_vm_size_t)bytes_traced);
...
static kern_return_t stackshot_setup_exclave_waitlist(void) {
  ...
  if (exclaves_inspection_is_initialized() && exclave_threads_max) {
    ...
    stackshot_exclave_inspect_ctids =
        stackshot_alloc_with_size(waitlist_size, &error);
    if (!stackshot_exclave_inspect_ctids) {
      goto error;
    }
...
static kern_return_t collect_exclave_threads(uint64_t ss_flags) {
  size_t i;
  ctid_t ctid;
  thread_t thread;
  ...
  // When asking for ASIDs, make sure we get all exclaves asids and mappings as
  // well
  exclaves_stackshot_raw_addresses = (ss_flags & STACKSHOT_ASID);
  exclaves_stackshot_all_address_spaces =
      (ss_flags & (STACKSHOT_ASID | STACKSHOT_EXCLAVES));
   ...
   */
  kcdata_add_container_marker(stackshot_ctx.sc_finalized_kcdata,
  ...
  for (i = 0; i < stackshot_exclave_inspect_ctid_count; ++i) {
    ...
    assert(thread);
    exclaves_inspection_queue_add(
        &exclaves_inspection_queue_stackshot,
        &thread->th_exclaves_inspection_queue_stackshot);
  }
  exclaves_inspection_begin_collecting();
  exclaves_inspection_wait_complete(&exclaves_inspection_queue_stackshot);
  kr = stackshot_exclave_kr; /* Read the result of work done on our behalf, by
                                collection thread */
  if (kr != KERN_SUCCESS) {
    goto out;
  }

  kr = kcdata_add_container_marker(stackshot_ctx.sc_finalized_kcdata,
...
static kern_return_t
stackshot_exclaves_process_stacktrace(const address_v__opt_s *_Nonnull st,
                                      void *kcdata_ptr) {
  kern_return_t error = KERN_SUCCESS;
  ...
  address__v_visit(&st->value, ^(size_t __unused i,
                                 const stackshottypes_address_s __unused item) {
    count++;
  });

  kcdata_compression_window_open(kcdata_ptr);
  kcd_exit_on_error(kcdata_get_memory_addr_for_array(
      kcdata_ptr, STACKSHOT_KCTYPE_EXCLAVE_IPCSTACKENTRY_ECSTACK,
      sizeof(exclave_ecstackentry_addr_t), count, (mach_vm_address_t *)&addr));

  address__v_visit(&st->value,
                   ^(size_t i, const stackshottypes_address_s item) {
...
static kern_return_t stackshot_exclaves_process_ipcstackentry(
    uint64_t index, const stackshottypes_ipcstackentry_s *_Nonnull ise,
    void *kcdata_ptr) {
  kern_return_t error = KERN_SUCCESS;

  kcd_exit_on_error(kcdata_add_container_marker(
      kcdata_ptr, KCDATA_TYPE_CONTAINER_BEGIN,
  ...
  info.eise_flags |=
      (ise->stacktrace.has_value ? kExclaveIpcStackEntryHaveStack : 0);

  kcd_exit_on_error(
      kcdata_push_data(kcdata_ptr, STACKSHOT_KCTYPE_EXCLAVE_IPCSTACKENTRY_INFO,
                       sizeof(struct exclave_ipcstackentry_info), &info));

  if (ise->stacktrace.has_value) {
    kcd_exit_on_error(
        stackshot_exclaves_process_stacktrace(&ise->stacktrace, kcdata_ptr));
  }

  kcd_exit_on_error(kcdata_add_container_marker(
      kcdata_ptr, KCDATA_TYPE_CONTAINER_END,
      STACKSHOT_KCCONTAINER_EXCLAVE_IPCSTACKENTRY, index));
...
static kern_return_t stackshot_exclaves_process_ipcstack(
    const stackshottypes_ipcstackentry_v__opt_s *_Nonnull ipcstack,
    void *kcdata_ptr) {
  ...
  stackshottypes_ipcstackentry__v_visit(
      &ipcstack->value,
      ^(size_t i, const stackshottypes_ipcstackentry_s *_Nonnull item) {
        if (kr == KERN_SUCCESS) {
...
static kern_return_t stackshot_exclaves_process_stackshotentry(
    const stackshot_stackshotentry_s *_Nonnull se, void *kcdata_ptr) {
  kern_return_t error = KERN_SUCCESS;
  ...
  kcd_exit_on_error(
      kcdata_push_data(kcdata_ptr, STACKSHOT_KCTYPE_EXCLAVE_SCRESULT_INFO,
                       sizeof(struct exclave_scresult_info), &info));

  if (se->ipcstack.has_value) {
    kcd_exit_on_error(
        stackshot_exclaves_process_ipcstack(&se->ipcstack, kcdata_ptr));
  }
...
static kern_return_t stackshot_exclaves_process_textlayout_segments(
    const stackshottypes_textlayout_s *_Nonnull tl, void *kcdata_ptr,
    bool want_raw_addresses) {
  kern_return_t error = KERN_SUCCESS;
  __block struct exclave_textlayout_segment_v2 *info = NULL;

  __block size_t count = 0;
  stackshottypes_textsegment__v_visit(
      &tl->textsegments,
      ^(size_t __unused i,
        const stackshottypes_textsegment_s __unused *_Nonnull item) {
        count++;
      });
  ...
  kcdata_compression_window_open(kcdata_ptr);
  kcd_exit_on_error(kcdata_get_memory_addr_for_array(
      kcdata_ptr, STACKSHOT_KCTYPE_EXCLAVE_TEXTLAYOUT_SEGMENTS,
      sizeof(struct exclave_textlayout_segment_v2), count,
      (mach_vm_address_t *)&info));

  stackshottypes_textsegment__v_visit(
      &tl->textsegments,
      ^(size_t __unused i, const stackshottypes_textsegment_s *_Nonnull item) {
        ...
        if (want_raw_addresses) {
          info->layoutSegment_rawLoadAddress =
              item->rawloadaddress.has_value ? item->rawloadaddress.value : 0;
        } else {
...
static kern_return_t stackshot_exclaves_process_textlayout(
    const stackshottypes_textlayout_s *_Nonnull tl, void *kcdata_ptr,
    bool want_raw_addresses) {
  kern_return_t error = KERN_SUCCESS;
  ...
  info.sharedcache_index =
      (sharedcache_index == NULL) ? UINT32_MAX : *sharedcache_index;

  kcd_exit_on_error(
      kcdata_push_data(kcdata_ptr, STACKSHOT_KCTYPE_EXCLAVE_TEXTLAYOUT_INFO,
                       sizeof(struct exclave_textlayout_info), &info));
  kcd_exit_on_error(stackshot_exclaves_process_textlayout_segments(
      tl, kcdata_ptr, want_raw_addresses));
  kcd_exit_on_error(kcdata_add_container_marker(
      kcdata_ptr, KCDATA_TYPE_CONTAINER_END,
      STACKSHOT_KCCONTAINER_EXCLAVE_TEXTLAYOUT, tl->textlayoutid));
error_exit:
...
static kern_return_t stackshot_exclaves_process_addressspace(
    const stackshottypes_addressspace_s *_Nonnull as, void *kcdata_ptr,
    bool want_raw_addresses) {
  kern_return_t error = KERN_SUCCESS;
  ...
  kcd_exit_on_error(
      kcdata_push_data(kcdata_ptr, STACKSHOT_KCTYPE_EXCLAVE_ADDRESSSPACE_INFO,
                       sizeof(struct exclave_addressspace_info), &info));

  if (name_len > 0) {
    kcdata_compression_window_open(kcdata_ptr);
    kcd_exit_on_error(kcdata_get_memory_addr(
        kcdata_ptr, STACKSHOT_KCTYPE_EXCLAVE_ADDRESSSPACE_NAME, name_len + 1,
...
kern_return_t
stackshot_exclaves_process_stackshot(const stackshot_stackshotresult_s *result,
                                     void *kcdata_ptr, bool want_raw_addresses);
...
stackshot_exclaves_process_stackshot(const stackshot_stackshotresult_s *result,
  ...
  stackshot_stackshotentry__v_visit(
      &result->stackshotentries,
      ^(size_t __unused i, const stackshot_stackshotentry_s *_Nonnull item) {
        if (kr == KERN_SUCCESS) {
          kr = stackshot_exclaves_process_stackshotentry(item, kcdata_ptr);
        }
      });

  stackshottypes_addressspace__v_visit(
      &result->addressspaces,
      ^(size_t __unused i, const stackshottypes_addressspace_s *_Nonnull item) {
        if (kr == KERN_SUCCESS) {
  ...
  stackshottypes_textlayout__v_visit(
      &result->textlayouts,
      ^(size_t __unused i, const stackshottypes_textlayout_s *_Nonnull item) {
        if (kr == KERN_SUCCESS) {
          kr = stackshot_exclaves_process_textlayout(item, kcdata_ptr,
                                                     want_raw_addresses);
        }
      });
...
kern_return_t
stackshot_exclaves_process_result(kern_return_t collect_kr,
                                  const stackshot_stackshotresult_s *result,
                                  bool want_raw_addresses);
...
stackshot_exclaves_process_result(kern_return_t collect_kr,
  ...
  kr = stackshot_exclaves_process_stackshot(
      result, stackshot_ctx.sc_finalized_kcdata, want_raw_addresses);
...
static void commit_exclaves_ast(void) {
  size_t i = 0;
  ...
  if (stackshot_exclave_inspect_ctids) {
    for (i = 0; i < count; ++i) {
      ...
      os_atomic_or(&thread->th_exclaves_inspection_state,
                   TH_EXCLAVES_INSPECTION_STACKSHOT, relaxed);
    }
  }
}
...
kern_return_t kern_stack_snapshot_internal(int stackshot_config_version,
  ...
  case STACKSHOT_CONFIG_TYPE:
     ...
     */
    if (snapshot_args.flags & STACKSHOT_PAGE_TABLES &&
        config->sc_pagetable_mask) {
  ...
  if (stackshot_from_user) {
    if (snapshot_args.flags &
        (STACKSHOT_TRYLOCK | STACKSHOT_SAVE_IN_KERNEL_BUFFER |
  ...
  if (!((snapshot_args.flags & STACKSHOT_KCDATA_FORMAT) ||
        (snapshot_args.flags & STACKSHOT_RETRIEVE_EXISTING_BUFFER))) {
    return KERN_NOT_SUPPORTED;
  }

  /* Compresssed delta stackshots or page dumps are not yet supported */
  if (((snapshot_args.flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT) ||
       (snapshot_args.flags & STACKSHOT_PAGE_TABLES)) &&
      (snapshot_args.flags & STACKSHOT_DO_COMPRESS)) {
    return KERN_NOT_SUPPORTED;
  }

  /*
   * If we're not saving the buffer in the kernel pointer, we need a place to
   * copy into.
   */
  if ((!out_buffer_addr || !out_size_addr) &&
      !(snapshot_args.flags & STACKSHOT_SAVE_IN_KERNEL_BUFFER)) {
    return KERN_INVALID_ARGUMENT;
  }

  if (snapshot_args.since_timestamp != 0 &&
      ((snapshot_args.flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT) == 0)) {
    return KERN_INVALID_ARGUMENT;
  }

  /* EXCLAVES and SKIP_EXCLAVES conflict */
  if ((snapshot_args.flags & (STACKSHOT_EXCLAVES | STACKSHOT_SKIP_EXCLAVES)) ==
      (STACKSHOT_EXCLAVES | STACKSHOT_SKIP_EXCLAVES)) {
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (snapshot_args.flags & STACKSHOT_SAVE_IN_KERNEL_BUFFER) {
    ...
    error =
        stackshot_remap_buffer(kernel_stackshot_buf, kernel_stackshot_buf_size,
                               out_buffer_addr, out_size_addr);
    /*
     * If we successfully remapped the buffer into the user's address space, we
     * set buf_to_free and size_to_free so the prior kernel mapping will be
     * removed and then clear the kernel stackshot pointer and associated size.
     */
    if (error == KERN_SUCCESS) {
      did_copyout = true;
      buf_to_free = kernel_stackshot_buf;
      size_to_free =
          (int)VM_MAP_ROUND_PAGE(kernel_stackshot_buf_size, PAGE_MASK);
  ...
  if (snapshot_args.flags & STACKSHOT_GET_BOOT_PROFILE) {
    ...
    error = stackshot_remap_buffer(bootprofile, len, out_buffer_addr,
                                   out_size_addr);
    if (error == KERN_SUCCESS) {
      did_copyout = true;
    }
  ...
  stackshot_duration_prior_abs = 0;
  stackshot_initial_estimate_adj =
      os_atomic_load(&stackshot_estimate_adj, relaxed);
  snapshot_args.buffer_size = stackshot_estimate =
      get_stackshot_estsize(size_hint, stackshot_initial_estimate_adj,
                            snapshot_args.flags, snapshot_args.pid);
  stackshot_initial_estimate = stackshot_estimate;

  // ensure at least one attempt, even if the initial size from estimate was too
  // big
  snapshot_args.buffer_size = MIN(snapshot_args.buffer_size, max_tracebuf_size);

  KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_STACKSHOT, STACKSHOT_RECORD) |
                   DBG_FUNC_START,
               snapshot_args.flags, snapshot_args.buffer_size,
               snapshot_args.pid, snapshot_args.since_timestamp);
  is_traced = true;
  ...
  for (; snapshot_args.buffer_size <= max_tracebuf_size;
       snapshot_args.buffer_size =
           MIN(snapshot_args.buffer_size << 1, max_tracebuf_size)) {
    stackshot_tries++;
    if ((error = kmem_alloc(kernel_map, (vm_offset_t *)&snapshot_args.buffer,
                            snapshot_args.buffer_size,
                            KMA_ZERO | KMA_DATA_SHARED, VM_KERN_MEMORY_DIAG)) !=
        KERN_SUCCESS) {
      os_log_error(OS_LOG_DEFAULT,
                   "stackshot: initial allocation failed: %d, allocating %u "
                   "bytes of %u max, try %llu\n",
                   (int)error, snapshot_args.buffer_size, max_tracebuf_size,
                   stackshot_tries);
      error = KERN_RESOURCE_SHORTAGE;
      goto error_exit;
    }

    uint32_t hdr_tag = (snapshot_args.flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT)
                           ? KCDATA_BUFFER_BEGIN_DELTA_STACKSHOT
                       : (snapshot_args.flags & STACKSHOT_DO_COMPRESS)
                           ? KCDATA_BUFFER_BEGIN_COMPRESSED
                           : KCDATA_BUFFER_BEGIN_STACKSHOT;
#pragma unused(hdr_tag)
    ...
    if (snapshot_args.flags & STACKSHOT_DO_COMPRESS) {
      hdr_tag = (snapshot_args.flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT)
                    ? KCDATA_BUFFER_BEGIN_DELTA_STACKSHOT
                    : KCDATA_BUFFER_BEGIN_STACKSHOT;
      if (error != KERN_SUCCESS) {
    ...
    /*
     * Stackshot is no longer active.
     * (We have to do this here for the special interrupt disable timeout case
     * to work)
     */
    os_atomic_store(&stackshot_ctx.sc_state, SS_INACTIVE, release);
    ...
    if (error == KERN_SUCCESS) {
      ...
      if (error == KERN_INSUFFICIENT_BUFFER_SIZE &&
          snapshot_args.buffer_size == max_tracebuf_size) {
        os_log_error(
            OS_LOG_DEFAULT,
            "stackshot: final buffer size was insufficient at maximum size: "
            "try %llu, estimate %u, flags %llu, pid %d, "
            "tasks: %d, terminated_tasks %d, threads: %d, terminated_threads: "
            "%d\n",
            stackshot_tries, snapshot_args.buffer_size, snapshot_args.flags,
            snapshot_args.pid, tasks_count, terminated_tasks_count,
    ...
    /* record the duration that interupts were disabled + kcdata was being
     * finalized */
    if (stackshot_duration_outer) {
    ...
    if (error != KERN_SUCCESS) {
      os_log_error(OS_LOG_DEFAULT,
                   "stackshot: debugger call failed: %d, try %llu, buffer %u "
                   "estimate %u\n",
                   (int)error, stackshot_tries, snapshot_args.buffer_size,
                   stackshot_estimate);
      kmem_free(kernel_map, (vm_offset_t)snapshot_args.buffer,
                snapshot_args.buffer_size);
      ...
      if (error == KERN_INSUFFICIENT_BUFFER_SIZE) {
         ...
         */
        KDBG_RELEASE(MACHDBG_CODE(DBG_MACH_STACKSHOT, STACKSHOT_RECORD_SHORT) |
                         DBG_FUNC_NONE,
                     time_end - time_start, stackshot_estimate,
                     snapshot_args.buffer_size);
        stackshot_duration_prior_abs += (time_end - time_start);
        if (snapshot_args.buffer_size == max_tracebuf_size) {
          os_log_error(OS_LOG_DEFAULT,
                       "stackshot: initial buffer size was insufficient at "
                       "maximum size: "
                       "try %llu, estimate %u, flags %llu, pid %d, "
                       "tasks: %d, terminated_tasks %d, threads: %d, "
                       "terminated_threads: %d\n",
                       stackshot_tries, snapshot_args.buffer_size,
                       snapshot_args.flags, snapshot_args.pid, tasks_count,
                       terminated_tasks_count, threads_count,
    ...
    if (!(snapshot_args.flags & STACKSHOT_SAVE_IN_KERNEL_BUFFER)) {
      error = stackshot_remap_buffer(snapshot_args.buffer, bytes_traced,
                                     out_buffer_addr, out_size_addr);
      if (error == KERN_SUCCESS) {
        did_copyout = true;
      }
    ...
    if (!(snapshot_args.flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT)) {
      os_log_info(OS_LOG_DEFAULT,
                  "stackshot: succeeded, traced %u bytes to %u buffer "
                  "(estimate %u) try %llu\n",
                  bytes_traced, snapshot_args.buffer_size, stackshot_estimate,
                  stackshot_tries);
    }
    ...
    /*
     * Figure out if we didn't use all the pages in the buffer. If so, we set
     * buf_to_free to the beginning of the next page after the end of the
     * stackshot in the buffer so that the kmem_free clips the buffer and update
     * size_to_free for kmem_free accordingly.
     */
    size_to_free = snapshot_args.buffer_size -
                   (int)VM_MAP_ROUND_PAGE(bytes_traced, PAGE_MASK);

    assert(size_to_free >= 0);

    if (size_to_free != 0) {
      buf_to_free = (void *)((uint64_t)snapshot_args.buffer +
                             snapshot_args.buffer_size - size_to_free);
    }
...
error_exit:
  if (is_traced) {
    KDBG_RELEASE(
        MACHDBG_CODE(DBG_MACH_STACKSHOT, STACKSHOT_RECORD) | DBG_FUNC_END,
...
#if DEVELOPMENT || DEBUG
    os_log_error(OS_LOG_DEFAULT,
                 "stackshot: reached end without doing copyout\n");
#endif // DEVELOPMENT || DEBUG
    error = KERN_FAILURE;
  }
...
kdp_snapshot_preflight_internal(struct kdp_snapshot_args args) {
  ...
  bool is_singlethreaded =
      stackshot_single_thread || (process_scoped || is_panic ||
                                  ((args.flags & STACKSHOT_PAGE_TABLES) != 0));
  clock_get_calendar_microtime((clock_sec_t *)&secs,
                               (clock_usec_t *)&microsecs);

  cur_stackshot_ctx_idx =
      (is_panic ? STACKSHOT_CTX_IDX_PANIC : STACKSHOT_CTX_IDX_NORMAL);
  ...
  if (!stackshot_ctx.sc_panic_stackshot) {
#if defined(__AMP__)
    /* On AMP systems, we want to split the buffers up by cluster to avoid cache
     * line effects. */
    stackshot_ctx.sc_num_buffers =
        is_singlethreaded ? 1 : ml_get_cluster_count();
#else  /* __AMP__ */
    stackshot_ctx.sc_num_buffers = 1;
#endif /* !__AMP__ */

    /*
     * Set all buffer sizes to zero. We'll use ssb_size to track how many CPUs
     * in that cluster are participating in the stackshot.
     */
    ...
    percpu_foreach_base(base) {
      *PERCPU_GET_WITH_BASE(base, stackshot_cpu_ctx_percpu) =
          (struct stackshot_cpu_context){0};
    }

    if (is_singlethreaded) {
      /* If the stackshot is singlethreaded, set up the kcdata - we don't bother
       * with linked-list kcdata in singlethreaded mode. */
      uint32_t hdr_tag = (stackshot_flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT)
                             ? KCDATA_BUFFER_BEGIN_DELTA_STACKSHOT
                         : (stackshot_flags & STACKSHOT_DO_COMPRESS)
                             ? KCDATA_BUFFER_BEGIN_COMPRESSED
                             : KCDATA_BUFFER_BEGIN_STACKSHOT;
      kcdata_memory_static_init(stackshot_kcdata_p,
                                (mach_vm_address_t)stackshot_args.buffer,
                                hdr_tag, stackshot_args.buffer_size,
                                KCFLAG_USE_MEMCOPY | KCFLAG_NO_AUTO_ENDBUFFER);
      if (stackshot_flags & STACKSHOT_DO_COMPRESS) {
        hdr_tag = (stackshot_flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT)
                      ? KCDATA_BUFFER_BEGIN_DELTA_STACKSHOT
                      : KCDATA_BUFFER_BEGIN_STACKSHOT;
        kcd_exit_on_error(kcdata_init_compress(stackshot_kcdata_p, hdr_tag,
                                               kdp_memcpy, KCDCT_ZLIB));
      }
      stackshot_cpu_ctx.scc_stack_buffer =
          kcdata_endalloc(stackshot_kcdata_p, sizeof(uintptr_t) * MAX_FRAMES);
    }
  } else {
    /*
    ...
    *stackshot_kcdata_p = *stackshot_args.descriptor;
    stackshot_cpu_ctx = (struct stackshot_cpu_context){
        .scc_can_work = true,
        .scc_stack_buffer = kcdata_endalloc(stackshot_kcdata_p,
                                            sizeof(uintptr_t) * MAX_FRAMES)};
#if STACKSHOT_COLLECTS_LATENCY_INFO
...
/*
 * The old function signature for kdp_snapshot_preflight, used in the panic
 * path. Called when interrupts are disabled, but we're not in the debugger trap
 * yet.
 */
void kdp_snapshot_preflight(int pid, void *tracebuf, uint32_t tracebuf_size,
                            uint64_t flags, kcdata_descriptor_t data_p,
                            uint64_t since_timestamp, uint32_t pagetable_mask) {
  __assert_only kern_return_t err;
  err = kdp_snapshot_preflight_internal(
      (struct kdp_snapshot_args){.pid = pid,
                                 .buffer = tracebuf,
...
static void stackshot_reset_state(void) {
  stackshot_ctx = (struct stackshot_context){0};
}

void panic_stackshot_reset_state(void) { stackshot_reset_state(); }

boolean_t stackshot_active(void) {
  return os_atomic_load(&stackshot_ctx.sc_state, relaxed) != SS_INACTIVE;
}

boolean_t panic_stackshot_active(void) {
  return os_atomic_load(&stackshot_contexts[STACKSHOT_CTX_IDX_PANIC].sc_state,
                        relaxed) != SS_INACTIVE;
}
...
static bool _stackshot_validate_kva(vm_offset_t addr, size_t size) {
...
#define SIZE_EST(x) ROUNDUP((x), sizeof(uintptr_t))
  return SIZE_EST(size * sizeof(*plh->plh_array)) +
         ...
         SIZE_EST(size * sizeof(*stackshot_cpu_ctx.scc_plh_gen.pgs_gen) *
                  real_ncpus) +
         SIZE_EST((1ul << STACKSHOT_PLH_SHIFT) * sizeof(*plh->plh_hash));
#undef SIZE_EST
}

static void stackshot_plh_reset(void) {
  stackshot_ctx.sc_plh =
      (struct port_label_hash){.plh_size = 0}; /* structure assignment */
}

static kern_return_t stackshot_plh_setup(void) {
  kern_return_t error;
  size_t size;
  ...
  plh.plh_array =
      stackshot_alloc_with_size(size * sizeof(*plh.plh_array), &error);
  plh.plh_chains =
      stackshot_alloc_with_size(size * sizeof(*plh.plh_chains), &error);
  percpu_foreach_base(base) {
    struct stackshot_cpu_context *cpu_ctx =
        PERCPU_GET_WITH_BASE(base, stackshot_cpu_ctx_percpu);
    cpu_ctx->scc_plh_gen.pgs_gen = stackshot_alloc_with_size(
        size * sizeof(*cpu_ctx->scc_plh_gen.pgs_gen), &error);
  ...
  plh.plh_hash = stackshot_alloc_with_size(
      (1ul << STACKSHOT_PLH_SHIFT) * sizeof(*plh.plh_hash), &error);
  if (error != KERN_SUCCESS) {
    return error;
  }
  if (plh.plh_array == NULL || plh.plh_chains == NULL || percpu_alloc_failed ||
      plh.plh_hash == NULL) {
...
static int16_t stackshot_plh_hash(struct ipc_service_port_label *ispl) {
...
#define PLH_HASH_STEP(ptr, x)                                                  \
  ((((x) * STACKSHOT_PLH_SHIFT) < (sizeof(ispl) * CHAR_BIT))                   \
       ? ((ptr) >> ((x) * STACKSHOT_PLH_SHIFT))                                \
       : 0)
...
static void stackshot_plh_resetgen(void) {
  ...
  pgs->pgs_curgen_max = 0;
  if (pgs->pgs_curgen ==
      0) { // wrapped, zero the array and increment the generation
...
static int16_t
stackshot_plh_lookup_locked(struct ipc_service_port_label *ispl,
                            enum stackshot_plh_lookup_type type) {
  struct port_label_hash *plh = &stackshot_ctx.sc_plh;
  int depth;
  int16_t cur;
  if (ispl == NULL) {
  ...
  for (cur = plh->plh_hash[hash]; cur >= 0; cur = plh->plh_chains[cur]) {
     ...
     */
    if (cur >= plh->plh_count || depth > plh->plh_count ||
        depth > plh->plh_size) {
...
static kern_return_t kdp_stackshot_plh_record_locked(void) {
  kern_return_t error = KERN_SUCCESS;
  struct port_label_hash *plh = &stackshot_ctx.sc_plh;
  struct _stackshot_plh_gen_state *pgs = &stackshot_cpu_ctx.scc_plh_gen;
    ...
    for (int idx = curgen_min; idx <= curgen_max; idx++) {
      ...
      if (_stackshot_validate_kva((vm_offset_t)ispl, ispl_size)) {
        ...
        if (ispl != NULL && (vm_offset_t)ispl >> 48 == 0x0000) {
          ca_event_t event_to_send =
              os_atomic_xchg(&rdar_126582377_event, NULL, relaxed);
          if (event_to_send) {
      ...
      kcd_exit_on_error(kcdata_add_container_marker(
          stackshot_kcdata_p, KCDATA_TYPE_CONTAINER_BEGIN,
          STACKSHOT_KCCONTAINER_PORTLABEL, idx + 1));
      if (name != NULL && (name_sz = _stackshot_strlen(name, max_namelen)) >
                              0) { /* validates the kva */
        kcd_exit_on_error(kcdata_push_data(stackshot_kcdata_p,
      ...
      kcd_exit_on_error(kcdata_push_data(
          stackshot_kcdata_p, STACKSHOT_KCTYPE_PORTLABEL, sizeof(spl), &spl));
      kcd_exit_on_error(kcdata_add_container_marker(
          stackshot_kcdata_p, KCDATA_TYPE_CONTAINER_END,
          STACKSHOT_KCCONTAINER_PORTLABEL, idx + 1));
    }
  }
...
static kern_return_t kdp_stackshot_plh_record(void) {
  kern_return_t error;
...
static int16_t stackshot_plh_lookup(struct ipc_service_port_label *ispl,
                                    enum stackshot_plh_lookup_type type) {
  int16_t result;
  plh_lock(&stackshot_ctx.sc_plh);
...
#if DEVELOPMENT || DEBUG
static kern_return_t kdp_stackshot_plh_stats(void) {
  kern_return_t error = KERN_SUCCESS;
  struct port_label_hash *plh = &stackshot_ctx.sc_plh;

#define PLH_STAT(x)                                                            \
  do {                                                                         \
    if (os_atomic_load(&plh->x, relaxed) != 0) {                               \
      kcd_exit_on_error(kcdata_add_uint32_with_description(                    \
          stackshot_kcdata_p, os_atomic_load(&plh->x, relaxed),                \
          "stackshot_" #x));                                                   \
    }                                                                          \
  } while (0)
  PLH_STAT(plh_size);
 ...
 */
uint64_t kcdata_get_task_ss_flags(task_t task, bool from_stackshot) {
  uint64_t ss_flags = 0;
  ...
  memorystatus_proc_flags_unsafe(bsd_info, &dirty, &dirty_tracked,
...
static kern_return_t
kcdata_record_shared_cache_info(kcdata_descriptor_t kcd, task_t task,
  ...
  if (task->shared_region &&
      _stackshot_validate_kva((vm_offset_t)task->shared_region,
  ...
  /*
   * No refcounting here, but we are in debugger context, so that should be
   * safe.
   */
   ...
   */
  kcd_exit_on_error(kcdata_push_data(kcd, STACKSHOT_KCTYPE_SHAREDCACHE_ID,
                                     sizeof(shared_cache_id),
                                     &shared_cache_id));
  ...
  kdp_memcpy(&shared_cache_data.sharedCacheUUID, task->shared_region->sr_uuid,
             sizeof(task->shared_region->sr_uuid));
  shared_cache_data.sharedCacheUnreliableSlidBaseAddress =
      shared_cache_first_mapping;
  shared_cache_data.sharedCacheSlidFirstMapping = shared_cache_first_mapping;
  kcd_exit_on_error(kcdata_push_data(kcd, STACKSHOT_KCTYPE_SHAREDCACHE_LOADINFO,
...
static kern_return_t
kcdata_record_uuid_info(kcdata_descriptor_t kcd, task_t task,
                        uint64_t trace_flags, boolean_t have_pmap,
                        unaligned_u64 *task_snap_ss_flags) {
  bool save_loadinfo_p = ((trace_flags & STACKSHOT_SAVE_LOADINFO) != 0);
  bool save_kextloadinfo_p =
      ((trace_flags & STACKSHOT_SAVE_KEXT_LOADINFO) != 0);
  bool save_compactinfo_p =
      ((trace_flags & STACKSHOT_SAVE_DYLD_COMPACTINFO) != 0);
...
#pragma unused(uuid_info_timestamp)
  ...
  if ((save_loadinfo_p || save_compactinfo_p) && have_pmap && task->active &&
      task_pid > 0) {
    /* Read the dyld_all_image_infos struct from the task memory to get UUID
     * array count and location */
    if (task_64bit_addr) {
      struct user64_dyld_all_image_infos task_image_infos;
      if (stackshot_copyin(task->map, task->all_image_info_addr,
                           &task_image_infos,
                           sizeof(struct user64_dyld_all_image_infos),
                           should_fault, &kdp_fault_results)) {
        uuid_info_count = (uint32_t)task_image_infos.uuidArrayCount;
        uuid_info_addr = task_image_infos.uuidArray;
        if (task_image_infos.version >=
            DYLD_ALL_IMAGE_INFOS_TIMESTAMP_MINIMUM_VERSION) {
          uuid_info_timestamp = task_image_infos.timestamp;
        }
        if (task_image_infos.version >=
            DYLD_ALL_IMAGE_INFOS_COMPACTINFO_MINIMUM_VERSION) {
    ...
    /*
     * If we get a NULL uuid_info_addr (which can happen when we catch dyld in
     * the middle of updating this data structure), we zero the uuid_info_count
     * so that we won't even try to save load info for this task.
     */
    if (!uuid_info_addr) {
      uuid_info_count = 0;
    }
  ...
  if (have_pmap && task_pid == 0) {
    if (save_kextloadinfo_p &&
        _stackshot_validate_kva((vm_offset_t)(gLoadedKextSummaries),
                                sizeof(OSKextLoadedKextSummaryHeader))) {
      uuid_info_count =
          gLoadedKextSummaries->numSummaries + 1; /* include main kernel UUID */
    } else {
      uuid_info_count =
  ...
  if (save_compactinfo_p && task_pid > 0) {
    if (dyld_compactinfo_size == 0) {
      ...
      *task_snap_ss_flags |= kTaskDyldCompactInfoTooBig;
      ...
      kcd_exit_on_error(
          kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_DYLD_COMPACTINFO,
                                 dyld_compactinfo_size, &out_addr));

      if (!stackshot_copyin(task->map, dyld_compactinfo_addr, (void *)out_addr,
                            dyld_compactinfo_size, should_fault,
                            &ci_kdp_fault_results)) {
  ...
  if (save_loadinfo_p && task_pid > 0 && (uuid_info_count < MAX_LOADINFOS)) {
    ...
    uint32_t uuid_info_size =
        (uint32_t)(task_64bit_addr ? sizeof(struct user64_dyld_uuid_info)
    ...
    /* If we found some UUID information, first try to copy it in -- this will
     * only be non-zero if we had a pmap above */
    if (uuid_info_count > 0) {
      uuid_info_array_size = uuid_info_count * uuid_info_size;

      kcd_exit_on_error(kcdata_get_memory_addr_for_array(
          kcd,
          (task_64bit_addr ? KCDATA_TYPE_LIBRARY_LOADINFO64
                           : KCDATA_TYPE_LIBRARY_LOADINFO),
          uuid_info_size, uuid_info_count, &out_addr));

      if (!stackshot_copyin(task->map, uuid_info_addr, (void *)out_addr,
                            uuid_info_array_size, should_fault,
                            &kdp_fault_results)) {
    ...
    if (!copied_uuid_count && proc_binary_uuid_kdp(task, binary_uuid)) {
      /* We failed to copyin the UUID information, try to store the UUID of the
       * main binary we have in the proc */
    ...
    do {
      ...
      if (kernelcache_uuid_valid && !save_kextloadinfo_p) {
        ...
        kcd_exit_on_error(
            kcdata_push_data(kcd, STACKSHOT_KCTYPE_KERNELCACHE_LOADINFO,
                             sizeof(struct dyld_uuid_info_64), &kc_uuid));
        break;
      }
#endif /* defined(__arm64__) */

      if (!kernel_uuid ||
          !_stackshot_validate_kva((vm_offset_t)kernel_uuid, sizeof(uuid_t))) {
      ...
      if ((sizeof(kernel_uuid_info) == sizeof(struct user64_dyld_uuid_info))) {
        ...
        if (PE_get_primary_kc_format(&primary_kc_type) &&
            (primary_kc_type == KCFormatFileset)) {
          /* return TEXT_EXEC based load information on arm devices running with
      ...
      kcdata_compression_window_open(kcd);
      kcd_exit_on_error(kcdata_get_memory_addr_for_array(
          kcd, uuid_type, sizeof(kernel_uuid_info), uuid_info_count,
          &out_addr));
      ...
      if (uuid_type == STACKSHOT_KCTYPE_LOADINFO64_TEXT_EXEC) {
        /* If we're reporting TEXT_EXEC load info, populate the TEXT_EXEC base
         * instead */
      ...
      if (save_kextloadinfo_p &&
          _stackshot_validate_kva((vm_offset_t)(gLoadedKextSummaries),
                                  sizeof(OSKextLoadedKextSummaryHeader)) &&
          _stackshot_validate_kva(
              (vm_offset_t)(&gLoadedKextSummaries->summaries[0]),
              gLoadedKextSummaries->entry_size *
                  gLoadedKextSummaries->numSummaries)) {
        uint32_t kexti;
        for (kexti = 0; kexti < gLoadedKextSummaries->numSummaries; kexti++) {
          image_load_address = (uintptr_t)VM_KERNEL_UNSLIDE(
              gLoadedKextSummaries->summaries[kexti].address);
#if defined(__arm64__)
          if (uuid_type == STACKSHOT_KCTYPE_LOADINFO64_TEXT_EXEC) {
            /* If we're reporting TEXT_EXEC load info, populate the TEXT_EXEC
             * base instead */
            image_load_address = (uintptr_t)VM_KERNEL_UNSLIDE(
                gLoadedKextSummaries->summaries[kexti].text_exec_address);
          }
#endif
          uuid_info_array[kexti + 1].imageLoadAddress = image_load_address;
          kdp_memcpy(&uuid_info_array[kexti + 1].imageUUID,
                     &gLoadedKextSummaries->summaries[kexti].uuid,
                     sizeof(uuid_t));
        }
      }
      kcd_exit_on_error(kcdata_compression_window_close(kcd));
...
/* Compute the set of flags that kdp_task_exec_meta_flags can return based on
 * the kernel config */
static uint64_t stackshot_available_task_exec_flags(void) {
...
static kern_return_t kcdata_record_task_exec_meta(kcdata_descriptor_t kcd,
                                                  task_t task) {
  ...
  if (tem.tem_flags != 0) {
    kcd_exit_on_error(kcdata_push_data(kcd, STACKSHOT_KCTYPE_TASK_EXEC_META,
                                       sizeof(struct task_exec_meta), &tem));
  }
...
static kern_return_t kcdata_record_task_iostats(kcdata_descriptor_t kcd,
                                                task_t task) {
  kern_return_t error = KERN_SUCCESS;
  mach_vm_address_t out_addr = 0;
  ...
  if (task->task_io_stats &&
      !memory_iszero(task->task_io_stats, sizeof(struct io_stat_info))) {
    /* struct io_stats_snapshot is quite large - avoid overflowing the stack. */
    kcdata_compression_window_open(kcd);
    kcd_exit_on_error(kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_IOSTATS,
    ...
    _iostat->ss_disk_writes_count = (task->task_io_stats->total_io.count -
                                     task->task_io_stats->disk_reads.count);
    _iostat->ss_disk_writes_size = (task->task_io_stats->total_io.size -
    ...
    _iostat->ss_non_paging_count = (task->task_io_stats->total_io.count -
                                    task->task_io_stats->paging.count);
    _iostat->ss_non_paging_size =
    ...
    _iostat->ss_data_count = (task->task_io_stats->total_io.count -
                              task->task_io_stats->metadata.count);
    _iostat->ss_data_size = (task->task_io_stats->total_io.size -
                             task->task_io_stats->metadata.size);
    for (int i = 0; i < IO_NUM_PRIORITIES; i++) {
      _iostat->ss_io_priority_count[i] =
          task->task_io_stats->io_priority[i].count;
      _iostat->ss_io_priority_size[i] =
          task->task_io_stats->io_priority[i].size;
    }
    kcd_exit_on_error(kcdata_compression_window_close(kcd));
  }
...
#if CONFIG_PERVASIVE_CPI
static kern_return_t kcdata_record_task_instrs_cycles(kcdata_descriptor_t kcd,
                                                      task_t task) {
  ...
  return kcdata_push_data(kcd, STACKSHOT_KCTYPE_INSTRS_CYCLES,
                          sizeof(instrs_cycles), &instrs_cycles);
}
#endif /* CONFIG_PERVASIVE_CPI */
...
kcdata_record_task_codesigning_info(kcdata_descriptor_t kcd, task_t task) {
  ...
  ret = kcdata_push_data(kcd, STACKSHOT_KCTYPE_CODESIGNING_INFO,
                         sizeof(struct stackshot_task_codesigning_info),
                         &codesigning_info);
  if (ret != KERN_SUCCESS) {
    return ret;
  }
  return kcdata_push_data(kcd, TASK_CRASHINFO_CS_AUXILIARY_INFO,
                          sizeof(cs_auxiliary_info), &cs_auxiliary_info);
}
...
kcdata_record_task_jit_address_range(kcdata_descriptor_t kcd, task_t task) {
  ...
  ret = get_jit_address_range_kdp(pmap, (uintptr_t *)&jit_start_addr,
                                  (uintptr_t *)&jit_end_addr);
  if (KERN_SUCCESS == ret) {
    range.start_address = jit_start_addr;
    range.end_address = jit_end_addr;
    return kcdata_push_data(kcd, TASK_CRASHINFO_JIT_ADDRESS_RANGE,
                            sizeof(struct crashinfo_jit_address_range), &range);
  } else {
    return KERN_SUCCESS;
  }
}
...
static kern_return_t kcdata_record_task_suspension_info(kcdata_descriptor_t kcd,
                                                        task_t task) {
  ...
  struct stackshot_suspension_source
      suspension_sources[TASK_SUSPEND_SOURCES_MAX];
  int i;
  ...
  ret = kcdata_push_data(kcd, STACKSHOT_KCTYPE_SUSPENSION_INFO,
                         sizeof(suspension_info), &suspension_info);
  if (ret != KERN_SUCCESS) {
    return ret;
  }
  ...
  for (i = 0; i < TASK_SUSPEND_SOURCES_MAX; ++i) {
    ...
    strlcpy(suspension_sources[i].tss_procname, suspend_sources[i].tss_procname,
            sizeof(suspend_sources[i].tss_procname));
    suspension_sources[i].tss_tid = suspend_sources[i].tss_tid;
    suspension_sources[i].tss_time = suspend_sources[i].tss_time;
  }
  return kcdata_push_array(kcd, STACKSHOT_KCTYPE_SUSPENSION_SOURCE,
                           sizeof(suspension_sources[0]),
                           TASK_SUSPEND_SOURCES_MAX, &suspension_sources);
}
#endif /* CONFIG_TASK_SUSPEND_STATS */

static kern_return_t
kcdata_record_transitioning_task_snapshot(kcdata_descriptor_t kcd, task_t task,
                                          unaligned_u64 task_snap_ss_flags,
                                          uint64_t transition_type) {
  kern_return_t error = KERN_SUCCESS;
  mach_vm_address_t out_addr = 0;
  ...
  if (task_pid &&
      (task_did_exec_internal(task) || task_is_exec_copy_internal(task))) {
    /*
  ...
  /* the transitioning_task_snapshot struct is large - avoid overflowing the
   * stack */
  kcdata_compression_window_open(kcd);
  kcd_exit_on_error(kcdata_get_memory_addr(
      kcd, STACKSHOT_KCTYPE_TRANSITIONING_TASK_SNAPSHOT,
      sizeof(struct transitioning_task_snapshot), &out_addr));
  ...
  if (task_pid != -1 && get_bsdtask_info(task) != NULL) {
    proc_name_kdp(get_bsdtask_info(task), cur_tsnap->tts_p_comm,
                  sizeof(cur_tsnap->tts_p_comm));
  } else {
...
static kern_return_t
#if STACKSHOT_COLLECTS_LATENCY_INFO
kcdata_record_task_snapshot(kcdata_descriptor_t kcd, task_t task,
                            uint64_t trace_flags, boolean_t have_pmap,
                            unaligned_u64 task_snap_ss_flags,
...
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */
{
  bool collect_delta_stackshot =
      ((trace_flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT) != 0);
  bool collect_iostats =
      !collect_delta_stackshot && !(trace_flags & STACKSHOT_NO_IO_STATS);
#if CONFIG_PERVASIVE_CPI
  ...
  kcdata_compression_window_open(kcd);
  kcd_exit_on_error(kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_TASK_SNAPSHOT,
                                           sizeof(struct task_snapshot_v3),
                                           &out_addr));
  ...
  cur_tsnap->ts_latency_qos =
      (task->effective_policy.tep_latency_qos == LATENCY_QOS_TIER_UNSPECIFIED)
  ...
  if (task_pid != -1 && bsd_info != NULL) {
    ...
    if (task->task_imp_base != NULL) {
      kdp_strlcpy(cur_tsnap->ts_p_comm, &task->task_imp_base->iit_procname[0],
                  MIN((int)sizeof(task->task_imp_base->iit_procname),
                      (int)sizeof(cur_tsnap->ts_p_comm)));
    }
...
#if CONFIG_MEMORYSTATUS
  kcd_exit_on_error(kcdata_get_memory_addr(
      kcd, STACKSHOT_KCTYPE_TASK_MEMORYSTATUS,
      sizeof(struct task_memorystatus_snapshot), &memorystatus_addr));
  memorystatus_snapshot =
      (struct task_memorystatus_snapshot *)memorystatus_addr;
  bzero(memorystatus_snapshot, sizeof(*memorystatus_snapshot));

  int32_t current_memlimit = 0, effectiveprio = 0, requestedprio = 0,
          assertionprio = 0;
  proc_memstat_data_kdp(bsd_info, &current_memlimit, &effectiveprio,
                        &requestedprio, &assertionprio);
  ...
  if (task_pid != -1 && bsd_info != NULL &&
     ...
     */
    uint64_t jetsam_coal_id =
        coalition_id(task->coalition[COALITION_TYPE_JETSAM]);
    kcd_exit_on_error(kcdata_push_data(kcd, STACKSHOT_KCTYPE_JETSAM_COALITION,
  ...
  if (collect_asid && have_pmap) {
    ...
    kcd_exit_on_error(
        kcdata_push_data(kcd, STACKSHOT_KCTYPE_ASID, sizeof(asid), &asid));
  }
#endif

#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info->cur_tsnap_latency =
      mach_absolute_time() - latency_info->cur_tsnap_latency;
  latency_info->pmap_latency = mach_absolute_time();
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */

  if (collect_pagetables && have_pmap) {
#if SCHED_HYGIENE_DEBUG
    // pagetable dumps can be large; reset the interrupt timeout to avoid a
    // panic
    ml_spin_debug_clear_self();
#endif
    assert(stackshot_ctx.sc_is_singlethreaded);
    size_t bytes_dumped = 0;
    error = pmap_dump_page_tables(task->map->pmap, kcd_end_address(kcd),
                                  kcd_max_address(kcd),
                                  stackshot_args.pagetable_mask, &bytes_dumped);
    if (error != KERN_SUCCESS) {
      goto error_exit;
    } else {
      ...
      kcdata_compression_window_open(kcd);
      kcd_exit_on_error(kcdata_get_memory_addr_for_array(
          kcd, STACKSHOT_KCTYPE_PAGE_TABLES, sizeof(uint64_t),
          (uint32_t)(bytes_dumped / sizeof(uint64_t)), &out_addr));
      kcd_exit_on_error(kcdata_compression_window_close(kcd));
    }
  }

#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info->pmap_latency =
      mach_absolute_time() - latency_info->pmap_latency;
  latency_info->bsd_proc_ids_latency = mach_absolute_time();
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */

#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info->bsd_proc_ids_latency =
      mach_absolute_time() - latency_info->bsd_proc_ids_latency;
...
static kern_return_t
kcdata_record_task_delta_snapshot(kcdata_descriptor_t kcd, task_t task,
                                  uint64_t trace_flags, boolean_t have_pmap,
                                  unaligned_u64 task_snap_ss_flags) {
#if !CONFIG_PERVASIVE_CPI
#pragma unused(trace_flags)
...
#endif
#if CONFIG_PERVASIVE_CPI
  boolean_t collect_instrs_cycles =
      ((trace_flags & STACKSHOT_INSTRS_CYCLES) != 0);
#endif /* CONFIG_PERVASIVE_CPI */

  uint64_t task_uniqueid = get_task_uniqueid(task);

  kcd_exit_on_error(
      kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_TASK_DELTA_SNAPSHOT,
                             sizeof(struct task_delta_snapshot_v2), &out_addr));
  ...
  if (collect_asid && have_pmap) {
    uint32_t asid = PMAP_VASID(task->map->pmap);
    kcd_exit_on_error(kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_ASID,
...
static kern_return_t kcdata_record_thread_iostats(kcdata_descriptor_t kcd,
                                                  thread_t thread) {
  kern_return_t error = KERN_SUCCESS;
  mach_vm_address_t out_addr = 0;
  ...
  if (thread->thread_io_stats &&
      !memory_iszero(thread->thread_io_stats, sizeof(struct io_stat_info))) {
    kcd_exit_on_error(kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_IOSTATS,
                                             sizeof(struct io_stats_snapshot),
                                             &out_addr));
    struct io_stats_snapshot *_iostat = (struct io_stats_snapshot *)out_addr;
    ...
    _iostat->ss_disk_writes_count = (thread->thread_io_stats->total_io.count -
                                     thread->thread_io_stats->disk_reads.count);
    _iostat->ss_disk_writes_size = (thread->thread_io_stats->total_io.size -
                                    thread->thread_io_stats->disk_reads.size);
    _iostat->ss_paging_count = thread->thread_io_stats->paging.count;
    _iostat->ss_paging_size = thread->thread_io_stats->paging.size;
    _iostat->ss_non_paging_count = (thread->thread_io_stats->total_io.count -
                                    thread->thread_io_stats->paging.count);
    _iostat->ss_non_paging_size = (thread->thread_io_stats->total_io.size -
                                   thread->thread_io_stats->paging.size);
    ...
    _iostat->ss_data_count = (thread->thread_io_stats->total_io.count -
                              thread->thread_io_stats->metadata.count);
    _iostat->ss_data_size = (thread->thread_io_stats->total_io.size -
                             thread->thread_io_stats->metadata.size);
    for (int i = 0; i < IO_NUM_PRIORITIES; i++) {
      _iostat->ss_io_priority_count[i] =
          thread->thread_io_stats->io_priority[i].count;
      _iostat->ss_io_priority_size[i] =
          thread->thread_io_stats->io_priority[i].size;
    }
  }
...
bool machine_trace_thread_validate_kva(vm_offset_t addr) {
...
static errno_t _stackshot_backtrace_copy(void *vctx, void *dst, user_addr_t src,
                                         size_t size) {
  struct _stackshot_backtrace_context *ctx = vctx;
  size_t map_page_mask = 0;
  size_t __assert_only map_page_size =
      kdp_vm_map_get_page_size(ctx->sbc_map, &map_page_mask);
  ...
  if (src_page != ctx->sbc_prev_page) {
    ...
    uint32_t flags = 0;
    vm_offset_t src_pa =
        stackshot_find_phys(ctx->sbc_map, src, ctx->sbc_allow_faulting, &res);
...
static kern_return_t kcdata_record_thread_snapshot(kcdata_descriptor_t kcd,
                                                   thread_t thread, task_t task,
  ...
  boolean_t active_kthreads_only_p =
      ((trace_flags & STACKSHOT_ACTIVE_KERNEL_THREADS_ONLY) != 0);
  boolean_t collect_delta_stackshot =
      ((trace_flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT) != 0);
  boolean_t collect_iostats =
      !collect_delta_stackshot && !(trace_flags & STACKSHOT_NO_IO_STATS);
#if CONFIG_PERVASIVE_CPI
  boolean_t collect_instrs_cycles =
      ((trace_flags & STACKSHOT_INSTRS_CYCLES) != 0);
  ...
  kcd_exit_on_error(
      kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_THREAD_SNAPSHOT,
                             sizeof(struct thread_snapshot_v4), &out_addr));
  ...
  cur_thread_snap->ths_wait_event =
      VM_KERNEL_UNSLIDE_OR_PERM(thread->wait_event);
  cur_thread_snap->ths_continuation = VM_KERNEL_UNSLIDE(thread->continuation);
  cur_thread_snap->ths_total_syscalls =
      thread->syscalls_mach + thread->syscalls_unix;

  if (IPC_VOUCHER_NULL != thread->ith_voucher) {
    cur_thread_snap->ths_voucher_identifier =
        VM_KERNEL_ADDRPERM(thread->ith_voucher);
  } else {
  ...
  latency_info.cur_thsnap1_latency =
      mach_absolute_time() - latency_info.cur_thsnap1_latency;
  ...
  if (dispatch_p && (task != kernel_task) && (task->active) && have_pmap) {
    ...
    if (dqkeyaddr != 0) {
      uint64_t dqaddr = 0;
      boolean_t copyin_ok =
          stackshot_copyin_word(task, dqkeyaddr, &dqaddr, FALSE, NULL);
      if (copyin_ok && dqaddr != 0) {
        uint64_t dqserialnumaddr =
            dqaddr + get_task_dispatchqueue_serialno_offset(task);
        uint64_t dqserialnum = 0;
        copyin_ok = stackshot_copyin_word(task, dqserialnumaddr, &dqserialnum,
                                          FALSE, NULL);
        if (copyin_ok) {
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
        latency_info.dispatch_serial_latency =
            mach_absolute_time() - latency_info.dispatch_serial_latency;
        ...
        if (label_offs) {
          ...
          copyin_ok = stackshot_copyin_word(task, dqlabeladdr,
                                            &actual_dqlabeladdr, FALSE, NULL);
          if (copyin_ok && actual_dqlabeladdr != 0) {
            ...
            len = stackshot_copyin_string(task, actual_dqlabeladdr, label_buf,
                                          STACKSHOT_QUEUE_LABEL_MAXSIZE, FALSE,
                                          NULL);
            if (len > 0) {
              mach_vm_address_t label_addr = 0;
              kcd_exit_on_error(kcdata_get_memory_addr(
                  kcd, STACKSHOT_KCTYPE_THREAD_DISPATCH_QUEUE_LABEL, len,
                  &label_addr));
        ...
        latency_info.dispatch_label_latency =
            mach_absolute_time() - latency_info.dispatch_label_latency;
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */
      }
    }
  }
  ...
  if ((thread->th_exclaves_state & TH_EXCLAVES_RPC) &&
      stackshot_exclave_inspect_ctids && !stackshot_ctx.sc_panic_stackshot) {
    /* certain threads, like the collector, must never be inspected */
    if ((os_atomic_load(&thread->th_exclaves_inspection_state, relaxed) &
         TH_EXCLAVES_INSPECTION_NOINSPECT) == 0) {
      uint32_t ctid_index =
          os_atomic_inc_orig(&stackshot_exclave_inspect_ctid_count, acq_rel);
      if (ctid_index < stackshot_exclave_inspect_ctid_capacity) {
        stackshot_exclave_inspect_ctids[ctid_index] = thread_get_ctid(thread);
      } else {
        os_atomic_store(&stackshot_exclave_inspect_ctid_count,
                        stackshot_exclave_inspect_ctid_capacity, release);
      }
      if ((os_atomic_load(&thread->th_exclaves_inspection_state, relaxed) &
           TH_EXCLAVES_INSPECTION_STACKSHOT) != 0) {
  ...
  cur_thread_snap->ths_last_made_runnable_time =
      thread->last_made_runnable_time;
  ...
  cur_thread_snap->ths_rqos_override =
      MAX(thread->requested_policy.thrp_qos_override,
          thread->requested_policy.thrp_qos_workq_override);
  cur_thread_snap->ths_io_tier =
      (uint8_t)proc_get_effective_thread_policy(thread, TASK_POLICY_IO);
  ...
  cur_thread_snap->ths_requested_policy =
      *(unaligned_u64 *)&thread->requested_policy;
  cur_thread_snap->ths_effective_policy =
      *(unaligned_u64 *)&thread->effective_policy;

#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info.cur_thsnap2_latency =
      mach_absolute_time() - latency_info.cur_thsnap2_latency;
  latency_info.thread_name_latency = mach_absolute_time();
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */

  /* if there is thread name then add to buffer */
  cur_thread_name[0] = '\0';
  proc_threadname_kdp(get_bsdthread_info(thread), cur_thread_name,
                      STACKSHOT_MAX_THREAD_NAME_SIZE);
  if (strnlen(cur_thread_name, STACKSHOT_MAX_THREAD_NAME_SIZE) > 0) {
    kcd_exit_on_error(kcdata_get_memory_addr(
        kcd, STACKSHOT_KCTYPE_THREAD_NAME, sizeof(cur_thread_name), &out_addr));
    kdp_memcpy((void *)out_addr, (void *)cur_thread_name,
               sizeof(cur_thread_name));
  }

#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info.thread_name_latency =
      mach_absolute_time() - latency_info.thread_name_latency;
  ...
  kcd_exit_on_error(
      kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_CPU_TIMES,
                             sizeof(struct stackshot_cpu_times_v2), &out_addr));
  struct stackshot_cpu_times_v2 *stackshot_cpu_times =
      (struct stackshot_cpu_times_v2 *)out_addr;
  *stackshot_cpu_times = (struct stackshot_cpu_times_v2){
      ...
      .runnable_usec = (uint64_t)runnable_time.seconds * USEC_PER_SEC +
                       runnable_time.microseconds,
  };

#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info.sur_times_latency =
      mach_absolute_time() - latency_info.sur_times_latency;
  ...
  if (!active_kthreads_only_p && task->active && task->map != kernel_map) {
    ...
    saved_count = backtrace_user(stackshot_cpu_ctx.scc_stack_buffer, MAX_FRAMES,
                                 &ctl, &info);
    if (saved_count > 0) {
#if __LP64__
...
#define STACKLR_WORDS STACKSHOT_KCTYPE_USER_STACKLR
      ...
      if (info.btui_async_frame_addr != 0) {
        ...
        kcd_exit_on_error(
            kcdata_push_data(kcd, STACKSHOT_KCTYPE_USER_ASYNC_START_INDEX,
        ...
        info = BTUINFO_INIT;
        unsigned int async_count = backtrace_user(
            stackshot_cpu_ctx.scc_stack_buffer, MAX_FRAMES, &ctl, &info);
        if (async_count > 0) {
          kcd_exit_on_error(kcdata_push_array(
              kcd, STACKSHOT_KCTYPE_USER_ASYNC_STACKLR64, sizeof(uintptr_t),
              async_count, stackshot_cpu_ctx.scc_stack_buffer));
          if ((info.btui_info & BTI_TRUNCATED) ||
              (ctx.sbc_flags & kThreadTruncatedBT)) {
            user_ths_ss_flags |= kThreadTruncatedBT;
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info.user_stack_latency =
      mach_absolute_time() - latency_info.user_stack_latency;
  ...
  if (thread->kernel_stack != 0) {
    ...
    if (saved_count > 0) {
      ...
      if (thread->th_exclaves_state & TH_EXCLAVES_RPC) {
        ...
        info.tei_thread_offset =
            exclaves_stack_offset(stackshot_cpu_ctx.scc_stack_buffer,
                                  saved_count / frame_size, false);

        kcd_exit_on_error(
            kcdata_push_data(kcd, STACKSHOT_KCTYPE_KERN_EXCLAVES_THREADINFO,
                             sizeof(struct thread_exclaves_info), &info));
      }
#endif /* CONFIG_EXCLAVES */
      kcd_exit_on_error(kcdata_push_array(kcd, stack_kcdata_type, frame_size,
                                          saved_count / frame_size,
                                          stackshot_cpu_ctx.scc_stack_buffer));
    }
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info.kernel_stack_latency =
      mach_absolute_time() - latency_info.kernel_stack_latency;
  ...
  if (trace_flags & STACKSHOT_THREAD_GROUP) {
    uint64_t thread_group_id =
        thread->thread_group ? thread_group_get_id(thread->thread_group) : 0;
    kcd_exit_on_error(kcdata_get_memory_addr(kcd, STACKSHOT_KCTYPE_THREAD_GROUP,
  ...
  if (collect_instrs_cycles) {
    ...
    kcd_exit_on_error(kcdata_get_memory_addr(
        kcd, STACKSHOT_KCTYPE_INSTRS_CYCLES,
        sizeof(struct instrs_cycles_snapshot), &out_addr));
    struct instrs_cycles_snapshot *instrs_cycles =
        (struct instrs_cycles_snapshot *)out_addr;
    instrs_cycles->ics_instructions = recount_usage_instructions(&usage);
    instrs_cycles->ics_cycles = recount_usage_cycles(&usage);
  }
#endif /* CONFIG_PERVASIVE_CPI */
  ...
  if (collect_latency_info) {
    kcd_exit_on_error(kcdata_push_data(kcd,
                                       STACKSHOT_KCTYPE_LATENCY_INFO_THREAD,
                                       sizeof(latency_info), &latency_info));
  }
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */
...
static int kcdata_record_thread_delta_snapshot(
    struct thread_delta_snapshot_v3 *cur_thread_snap, thread_t thread,
    boolean_t thread_on_core) {
  cur_thread_snap->tds_thread_id = thread_tid(thread);
  if (IPC_VOUCHER_NULL != thread->ith_voucher) {
    cur_thread_snap->tds_voucher_identifier =
        VM_KERNEL_ADDRPERM(thread->ith_voucher);
  } else {
  ...
  cur_thread_snap->tds_last_made_runnable_time =
      thread->last_made_runnable_time;
  ...
  cur_thread_snap->tds_rqos_override =
      MAX(thread->requested_policy.thrp_qos_override,
          thread->requested_policy.thrp_qos_workq_override);
  cur_thread_snap->tds_io_tier =
      (uint8_t)proc_get_effective_thread_policy(thread, TASK_POLICY_IO);

  static_assert(sizeof(thread->effective_policy) == sizeof(uint64_t));
  static_assert(sizeof(thread->requested_policy) == sizeof(uint64_t));
  cur_thread_snap->tds_requested_policy =
      *(unaligned_u64 *)&thread->requested_policy;
  cur_thread_snap->tds_effective_policy =
      *(unaligned_u64 *)&thread->effective_policy;
...
classify_thread(thread_t thread, boolean_t *thread_on_core_p,
                boolean_t collect_delta_stackshot) {
  processor_t last_processor = thread->last_processor;

  boolean_t thread_on_core = FALSE;
  if (last_processor != PROCESSOR_NULL) {
    /* Idle threads are always treated as on-core, since the processor state can
  ...
  *thread_on_core_p = thread_on_core;
  ...
  /* Capture the full thread snapshot if this is not a delta stackshot or if the
   * thread has run subsequent to the previous full stackshot */
  if (!collect_delta_stackshot || thread_on_core ||
      (thread->last_run_time > stackshot_args.since_timestamp)) {
...
static kern_return_t kdp_stackshot_record_task(task_t task) {
  boolean_t active_kthreads_only_p =
      ((stackshot_flags & STACKSHOT_ACTIVE_KERNEL_THREADS_ONLY) != 0);
  boolean_t collect_delta_stackshot =
      ((stackshot_flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT) != 0);
  boolean_t save_owner_info =
      ((stackshot_flags & STACKSHOT_THREAD_WAITINFO) != 0);
  boolean_t include_drivers =
  ...
  if ((task == NULL) ||
      !_stackshot_validate_kva((vm_offset_t)task, sizeof(struct task))) {
    error = KERN_FAILURE;
    goto error_exit;
  }

  void *bsd_info = get_bsdtask_info(task);
  boolean_t task_in_teardown =
      (bsd_info == NULL) ||
      proc_in_teardown(bsd_info); // has P_LPEXIT set during proc_exit()
  boolean_t task_in_transition =
      task_in_teardown; // here we can add other types of transition.
  uint32_t container_type = (task_in_transition)
  ...
  have_map =
      (task->map != NULL) && (_stackshot_validate_kva((vm_offset_t)(task->map),
                                                      sizeof(struct _vm_map)));
  have_pmap = have_map && (task->map->pmap != NULL) &&
              (_stackshot_validate_kva((vm_offset_t)(task->map->pmap),
                                       sizeof(struct pmap)));
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
  latency_info.setup_latency =
      mach_absolute_time() - latency_info.setup_latency;
  latency_info.task_uniqueid = task_uniqueid;
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */

  /* Trace everything, unless a process was specified. Add in driver tasks if
   * requested. */
  if ((stackshot_args.pid == -1) ||
    ...
    kcd_exit_on_error(kcdata_add_container_marker(
        stackshot_kcdata_p, KCDATA_TYPE_CONTAINER_BEGIN, container_type,
        task_uniqueid));

    if (collect_delta_stackshot) {
      /*
       * For delta stackshots we need to know if a thread from this task has run
       * since the previous timestamp to decide whether we're going to record a
       * full snapshot and UUID info.
       */
      thread_t thread = THREAD_NULL;
      queue_iterate(&task->threads, thread, thread_t, task_threads) {
        if ((thread == NULL) ||
            !_stackshot_validate_kva((vm_offset_t)thread,
        ...
        boolean_t thread_on_core;
        enum thread_classification thread_classification =
            classify_thread(thread, &thread_on_core, collect_delta_stackshot);
    ...
    if (collect_delta_stackshot) {
      proc_starttime_kdp(get_bsdtask_info(task), NULL, NULL,
                         &task_start_abstime);
    }

    /* Next record any relevant UUID info and store the task snapshot */
    if (task_in_transition || !collect_delta_stackshot ||
       ...
       * 2) a delta stackshot where the task started after the previous full
       * stackshot 3) a delta stackshot where any thread from the task has run
       * since the previous full stackshot
       *
       * because the task may have exec'ed, changing its name, architecture,
       * load info, etc
       */

      kcd_exit_on_error(kcdata_record_shared_cache_info(
          stackshot_kcdata_p, task, &task_snap_ss_flags));
      kcd_exit_on_error(kcdata_record_uuid_info(stackshot_kcdata_p, task,
                                                stackshot_flags, have_pmap,
                                                &task_snap_ss_flags));
      kcd_exit_on_error(kcdata_record_task_exec_meta(stackshot_kcdata_p, task));
#if STACKSHOT_COLLECTS_LATENCY_INFO
      if (!task_in_transition) {
        kcd_exit_on_error(kcdata_record_task_snapshot(
            stackshot_kcdata_p, task, stackshot_flags, have_pmap,
            task_snap_ss_flags, &latency_info));
      } else {
        kcd_exit_on_error(kcdata_record_transitioning_task_snapshot(
            stackshot_kcdata_p, task, task_snap_ss_flags, transition_type));
      }
#else
      if (!task_in_transition) {
        kcd_exit_on_error(kcdata_record_task_snapshot(
      ...
      kcd_exit_on_error(kcdata_record_task_delta_snapshot(
          stackshot_kcdata_p, task, stackshot_flags, have_pmap,
          task_snap_ss_flags));
    }
    ...
    if (num_delta_thread_snapshots > 0) {
      kcd_exit_on_error(kcdata_get_memory_addr_for_array(
          stackshot_kcdata_p, STACKSHOT_KCTYPE_THREAD_DELTA_SNAPSHOT,
          sizeof(struct thread_delta_snapshot_v3), num_delta_thread_snapshots,
          &out_addr));
    ...
    thread_t thread = THREAD_NULL;
    queue_iterate(&task->threads, thread, thread_t, task_threads) {
      if ((thread == NULL) || !_stackshot_validate_kva((vm_offset_t)thread,
                                                       sizeof(struct thread))) {
        error = KERN_FAILURE;
        goto error_exit;
      }
      ...
      case tc_full_snapshot:
        ...
        /* thread snapshot can be large, including strings, avoid overflowing
         * the stack. */
        kcdata_compression_window_open(stackshot_kcdata_p);

        kcd_exit_on_error(kcdata_record_thread_snapshot(
            stackshot_kcdata_p, thread, task, stackshot_flags, have_pmap,
            thread_on_core));
        ...
        break;
      case tc_delta_snapshot:
        kcd_exit_on_error(kcdata_record_thread_delta_snapshot(
            &delta_snapshots[current_delta_snapshot_index++], thread,
            thread_on_core));
        break;
      }
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
    latency_info.task_thread_count_loop_latency =
        mach_absolute_time() - latency_info.task_thread_count_loop_latency;
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */
    ...
    if (num_waitinfo_threads > 0 || num_turnstileinfo_threads > 0) {
      /* thread waitinfo and turnstileinfo can be quite large, avoid overflowing
       * the stack */
      kcdata_compression_window_open(stackshot_kcdata_p);

      if (num_waitinfo_threads > 0) {
        kcd_exit_on_error(kcdata_get_memory_addr_for_array(
            stackshot_kcdata_p, STACKSHOT_KCTYPE_THREAD_WAITINFO,
      ...
      if (num_turnstileinfo_threads > 0) {
        ...
        kcd_exit_on_error(kcdata_get_memory_addr_for_array(
            stackshot_kcdata_p, STACKSHOT_KCTYPE_THREAD_TURNSTILEINFO,
            sizeof(thread_turnstileinfo_v2_t), num_turnstileinfo_threads,
            &out_addr));
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
    latency_info.misc_latency =
        mach_absolute_time() - latency_info.misc_latency;
...
#pragma unused(thread_uniqueid)
      ...
      /* If we want owner info, we should capture it regardless of its
       * classification */
      if (save_owner_info) {
        if (stackshot_thread_has_valid_waitinfo(thread)) {
          stackshot_thread_wait_owner_info(
              thread, &thread_waitinfo[current_waitinfo_index++]);
        }

        if (stackshot_thread_has_valid_turnstileinfo(thread)) {
          stackshot_thread_turnstileinfo(
              thread, &thread_turnstileinfo[current_turnstileinfo_index++]);
        }
      }
    }

#if STACKSHOT_COLLECTS_LATENCY_INFO
    latency_info.task_thread_data_loop_latency =
        mach_absolute_time() - latency_info.task_thread_data_loop_latency;
    latency_info.misc2_latency = mach_absolute_time();
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */

#if DEBUG || DEVELOPMENT
    if (current_delta_snapshot_index != num_delta_thread_snapshots) {
      panic("delta thread snapshot count mismatch while capturing snapshots "
            "for task %p. expected %d, found %d",
            task, num_delta_thread_snapshots, current_delta_snapshot_index);
    }
    if (current_waitinfo_index != num_waitinfo_threads) {
      panic("thread wait info count mismatch while capturing snapshots for "
            "task %p. expected %d, found %d",
            task, num_waitinfo_threads, current_waitinfo_index);
    }
#endif
...
#if IMPORTANCE_INHERITANCE
    /* Ensure the buffer is big enough, since we're using the stack buffer for
     * this. */
    static_assert(TASK_IMP_WALK_LIMIT * sizeof(int32_t) <=
                  MAX_FRAMES * sizeof(uintptr_t));
    saved_count = task_importance_list_pids(
        task, TASK_IMP_LIST_DONATING_PIDS,
        (char *)stackshot_cpu_ctx.scc_stack_buffer, TASK_IMP_WALK_LIMIT);
    if (saved_count > 0) {
      /* Variable size array - better not have it on the stack. */
      kcdata_compression_window_open(stackshot_kcdata_p);
      kcd_exit_on_error(kcdata_push_array(
          stackshot_kcdata_p, STACKSHOT_KCTYPE_DONATING_PIDS, sizeof(int32_t),
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
    latency_info.misc2_latency =
        mach_absolute_time() - latency_info.misc2_latency;
    if (collect_latency_info) {
      kcd_exit_on_error(kcdata_push_data(stackshot_kcdata_p,
    ...
    kcd_exit_on_error(kcdata_add_container_marker(
        stackshot_kcdata_p, KCDATA_TYPE_CONTAINER_END, container_type,
        task_uniqueid));
  }
...
static kern_return_t kdp_stackshot_shared_regions(uint64_t trace_flags) {
  kern_return_t error = KERN_SUCCESS;
  ...
  queue_iterate(&vm_shared_region_queue, sr, vm_shared_region_t, sr_q) {
    ...
    scinfo.sharedCacheUnreliableSlidBaseAddress =
        sr->sr_base_address + sr->sr_first_mapping;
    scinfo.sharedCacheSlidFirstMapping =
        sr->sr_base_address + sr->sr_first_mapping;
  ...
  if (!collect_delta_stackshot && sr &&
      _stackshot_validate_kva((vm_offset_t)sr,
                              sizeof(struct vm_shared_region))) {
    ...
    scinfo.sharedCacheSlide = sr->sr_slide;
    scinfo.sharedCacheUnreliableSlidBaseAddress =
        sr->sr_slide + sr->sr_base_address;
    scinfo.sharedCacheSlidFirstMapping =
        sr->sr_base_address + sr->sr_first_mapping;
    ...
    if (trace_flags & STACKSHOT_COLLECT_SHAREDCACHE_LAYOUT) {
      /*
       * Include a map of the system shared cache layout if it has been
       * populated (which is only when the system is using a custom shared
       * cache).
       */
      if (sr->sr_images &&
          _stackshot_validate_kva(
              (vm_offset_t)sr->sr_images,
...
static kern_return_t kdp_stackshot_kcdata_format(void) {
  kern_return_t error = KERN_SUCCESS;
  mach_vm_address_t out_addr = 0;
  uint64_t abs_time = 0;
  ...
  collect_latency_info =
      stackshot_flags & STACKSHOT_DISABLE_LATENCY_INFO ? false : true;
#endif
  /* process the flags */
  bool collect_delta_stackshot =
      ((stackshot_flags & STACKSHOT_COLLECT_DELTA_SNAPSHOT) != 0);
  bool collect_exclaves = !disable_exclave_stackshot &&
                          ((stackshot_flags & STACKSHOT_SKIP_EXCLAVES) == 0);
  stackshot_ctx.sc_enable_faulting =
      (stackshot_flags & (STACKSHOT_ENABLE_BT_FAULTING));

  /* Currently we only support returning explicit KEXT load info on fileset
   * kernels */
  kc_format_t primary_kc_type = KCFormatUnknown;
  if (PE_get_primary_kc_format(&primary_kc_type) &&
      (primary_kc_type != KCFormatFileset)) {
  ...
  if (!stackshot_ctx.sc_panic_stackshot && collect_exclaves) {
    kcd_exit_on_error(stackshot_setup_exclave_waitlist()); /* Allocate list of
                                                              exclave threads */
  }
#else
#pragma unused(collect_exclaves)
  ...
  /* setup mach_absolute_time and timebase info -- copy out in some cases and
   * needed to convert since_timestamp to seconds for proc start time */
  clock_timebase_info(&timebase);
   ...
   * The idea here is that we want the kcdata to be in (roughly) the same order
   * as it was before we made this multithreaded, so we have separate buffers
   * for pre and post task-iteration, since that's the parallelized part.
   */
  if (!stackshot_ctx.sc_is_singlethreaded) {
  ...
  kcd_exit_on_error(kcdata_add_uint64_with_description(
      stackshot_kcdata_p, stackshot_flags, "stackshot_in_flags"));
  kcd_exit_on_error(kcdata_add_uint32_with_description(
      stackshot_kcdata_p, (uint32_t)stackshot_flags, "stackshot_in_pid"));
  kcd_exit_on_error(kcdata_add_uint64_with_description(
      stackshot_kcdata_p, system_state_flags, "system_state_flags"));
  if (stackshot_flags & STACKSHOT_PAGE_TABLES) {
    kcd_exit_on_error(kcdata_add_uint32_with_description(
        stackshot_kcdata_p, stackshot_args.pagetable_mask,
        "stackshot_pagetable_mask"));
  }
  if (stackshot_initial_estimate != 0) {
    kcd_exit_on_error(kcdata_add_uint32_with_description(
        stackshot_kcdata_p, stackshot_initial_estimate,
        "stackshot_size_estimate"));
    kcd_exit_on_error(kcdata_add_uint32_with_description(
        stackshot_kcdata_p, stackshot_initial_estimate_adj,
        "stackshot_size_estimate_adj"));
  }
  kcd_exit_on_error(kcdata_add_uint64_with_description(
      stackshot_kcdata_p, stackshot_available_task_exec_flags(),
  ...
  if (!collect_delta_stackshot) {
    ...
    kcd_exit_on_error(kcdata_push_data(stackshot_kcdata_p,
                                       ...
                                       length_to_copy, (const void *)version));
    length_to_copy = MIN((uint32_t)(strlen(osversion) + 1), OSVERSIZE);
    kcd_exit_on_error(kcdata_push_data(stackshot_kcdata_p,
                                       STACKSHOT_KCTYPE_OS_BUILD_VERSION,
                                       length_to_copy, (void *)osversion));

    length_to_copy =
        MIN((uint32_t)(strlen(PE_boot_args()) + 1), BOOT_LINE_LENGTH);
    kcd_exit_on_error(kcdata_push_data(stackshot_kcdata_p,
                                       STACKSHOT_KCTYPE_BOOTARGS,
                                       length_to_copy, PE_boot_args()));

    kcd_exit_on_error(kcdata_push_data(stackshot_kcdata_p, KCDATA_TYPE_TIMEBASE,
                                       sizeof(timebase), &timebase));
  } else {
    kcd_exit_on_error(kcdata_push_data(
        stackshot_kcdata_p, STACKSHOT_KCTYPE_DELTA_SINCE_TIMESTAMP,
        sizeof(uint64_t), &stackshot_args.since_timestamp));
  }
  ...
  kcd_exit_on_error(
      kcdata_push_data(stackshot_kcdata_p, KCDATA_TYPE_USECS_SINCE_EPOCH,
                       sizeof(uint64_t), &stackshot_ctx.sc_microsecs));
  ...
  if (stackshot_flags & STACKSHOT_THREAD_GROUP) {
    ...
    if (thread_group_iterate_stackshot(stackshot_thread_group_count,
                                       &num_thread_groups) != KERN_SUCCESS) {
      stackshot_flags &= ~(STACKSHOT_THREAD_GROUP);
    }

    if (num_thread_groups > 0) {
      kcd_exit_on_error(kcdata_get_memory_addr_for_array(
          stackshot_kcdata_p, STACKSHOT_KCTYPE_THREAD_GROUP_SNAPSHOT,
          sizeof(struct thread_group_snapshot_v3), num_thread_groups,
          &out_addr));
      thread_groups = (struct thread_group_snapshot_v3 *)out_addr;
    }

    if (thread_group_iterate_stackshot(stackshot_thread_group_snapshot,
                                       thread_groups) != KERN_SUCCESS) {
      error = KERN_FAILURE;
      goto error_exit;
    }
...
#if SCHED_HYGIENE_DEBUG && CONFIG_PERVASIVE_CPI
  if (!stackshot_ctx.sc_panic_stackshot &&
      (thread_group_begin_cpu_cycle_count != 0)) {
    kcd_exit_on_error(kcdata_add_uint64_with_description(
        stackshot_kcdata_p,
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
  stackshot_ctx.sc_latency.setup_latency_mt =
      mach_absolute_time() - stackshot_ctx.sc_latency.setup_latency_mt;
  if (stackshot_ctx.sc_is_singlethreaded) {
    stackshot_ctx.sc_latency.total_task_iteration_latency_mt =
        mach_absolute_time();
  } else {
    stackshot_ctx.sc_latency.task_queue_building_latency_mt =
        mach_absolute_time();
  }
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */

  bool const process_scoped =
      (stackshot_args.pid != -1) &&
      ((stackshot_flags & STACKSHOT_INCLUDE_DRIVER_THREADS_IN_KERNEL) == 0);
  ...
  queue_iterate(&tasks, task, task_t, tasks) {
    ...
    if (stackshot_ctx.sc_is_singlethreaded) {
      ...
      kcd_exit_on_error(stackshot_put_workitem(
          (struct stackshot_workitem){.sswi_task = task,
                                      .sswi_data = NULL,
...
#if CONFIG_COALITIONS
  /* Don't collect jetsam coalition snapshots in delta stackshots - these don't
   * change */
  if (!collect_delta_stackshot ||
      (last_task_start_time > stackshot_args.since_timestamp)) {
    ...
    if (stackshot_flags & STACKSHOT_SAVE_JETSAM_COALITIONS) {
      if (coalition_iterate_stackshot(stackshot_coalition_jetsam_count,
                                      &num_coalitions,
                                      COALITION_TYPE_JETSAM) != KERN_SUCCESS) {
        stackshot_flags &= ~(STACKSHOT_SAVE_JETSAM_COALITIONS);
      }
    }
    if (stackshot_flags & STACKSHOT_SAVE_JETSAM_COALITIONS) {
      if (num_coalitions > 0) {
        ...
        kcdata_compression_window_open(stackshot_kcdata_p);
        kcd_exit_on_error(kcdata_get_memory_addr_for_array(
            stackshot_kcdata_p, STACKSHOT_KCTYPE_JETSAM_COALITION_SNAPSHOT,
            sizeof(struct jetsam_coalition_snapshot), num_coalitions,
            &out_addr));
        coalitions = (struct jetsam_coalition_snapshot *)out_addr;

        if (coalition_iterate_stackshot(stackshot_coalition_jetsam_snapshot,
                                        coalitions, COALITION_TYPE_JETSAM) !=
            KERN_SUCCESS) {
          error = KERN_FAILURE;
          goto error_exit;
        }
...
#if SCHED_HYGIENE_DEBUG && CONFIG_PERVASIVE_CPI
    if (!stackshot_ctx.sc_panic_stackshot &&
        (coalition_begin_cpu_cycle_count != 0)) {
      kcd_exit_on_error(kcdata_add_uint64_with_description(
          stackshot_kcdata_p,
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
  if (stackshot_ctx.sc_is_singlethreaded) {
    stackshot_ctx.sc_latency.total_terminated_task_iteration_latency_mt =
        mach_absolute_time();
  } else {
    stackshot_ctx.sc_latency.terminated_task_queue_building_latency_mt =
        mach_absolute_time();
  }
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */
  ...
  for (size_t i = 0; i < STACKSHOT_NUM_WORKQUEUES; i++) {
    os_atomic_store(&stackshot_ctx.sc_workqueues[i].sswq_populated, true,
                    release);
  }
  ...
  if (collect_latency_info) {
    ...
    stackshot_ctx.sc_latency.calling_cpu_number =
        stackshot_ctx.sc_calling_cpuid;
  }
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */
  ...
  kcdata_add_uint64_with_description(stackshot_kcdata_p, tasks_in_stackshot,
                                     "stackshot_tasks_count");
  kcdata_add_uint64_with_description(stackshot_kcdata_p, threads_in_stackshot,
...
#if CONFIG_EXCLAVES
  /* If this is the panic stackshot, check if Exclaves panic left its stackshot
   * in the shared region */
  if (stackshot_ctx.sc_panic_stackshot) {
    struct exclaves_panic_stackshot excl_ss;
    kdp_read_panic_exclaves_stackshot(&excl_ss);

    if (excl_ss.stackshot_buffer != NULL &&
        excl_ss.stackshot_buffer_size != 0) {
      ...
      kcdata_add_container_marker(stackshot_kcdata_p,
                                  KCDATA_TYPE_CONTAINER_BEGIN,
                                  STACKSHOT_KCCONTAINER_EXCLAVES, 0);
      tberr = stackshot_stackshotresult__unmarshal(
          excl_ss.stackshot_buffer, excl_ss.stackshot_buffer_size,
          ^(stackshot_stackshotresult_s result) {
            *error_in_block = stackshot_exclaves_process_stackshot(
                &result, stackshot_kcdata_p, false);
          });
...
static void kdp_mem_and_io_snapshot(struct mem_and_io_snapshot *memio_snap) {
  ...
  kErr = mach_vm_pressure_monitor(FALSE, VM_PRESSURE_TIME_WINDOW,
...
static vm_offset_t stackshot_find_phys(vm_map_t map, vm_offset_t target_addr,
  ...
  if ((fault_results.flags & KDP_FAULT_RESULT_TRIED_FAULT) ||
      (fault_results.flags & KDP_FAULT_RESULT_FAULTED_IN)) {
    stackshot_cpu_ctx.scc_fault_stats.sfs_time_spent_faulting +=
        fault_results.time_spent_faulting;

#if STACKSHOT_COLLECTS_LATENCY_INFO
    stackshot_cpu_latency.faulting_time_mt += fault_results.time_spent_faulting;
#endif

    if ((stackshot_cpu_ctx.scc_fault_stats.sfs_time_spent_faulting >=
         stackshot_max_fault_time) &&
        !stackshot_ctx.sc_panic_stackshot) {
...
/*
 * Wrappers around kdp_generic_copyin, kdp_generic_copyin_word,
 * kdp_generic_copyin_string that use stackshot_find_phys in order to:
 *   1. collect statistics on the number of pages faulted in
 *   2. stop faulting if the time spent faulting has exceeded the limit.
 */
static boolean_t
stackshot_copyin(vm_map_t map, uint64_t uaddr, void *dest, size_t size,
                 boolean_t try_fault,
                 kdp_fault_result_flags_t *kdp_fault_result_flags) {
  kdp_fault_flags_t fault_flags = KDP_FAULT_FLAGS_NONE;
  if (try_fault) {
    fault_flags |= KDP_FAULT_FLAGS_ENABLE_FAULTING;
  }
  return kdp_generic_copyin(map, uaddr, dest, size, fault_flags,
                            (find_phys_fn_t)stackshot_find_phys,
                            kdp_fault_result_flags) == KERN_SUCCESS;
}
static boolean_t
stackshot_copyin_word(task_t task, uint64_t addr, uint64_t *result,
                      boolean_t try_fault,
                      kdp_fault_result_flags_t *kdp_fault_result_flags) {
  kdp_fault_flags_t fault_flags = KDP_FAULT_FLAGS_NONE;
  if (try_fault) {
  ...
  return kdp_generic_copyin_word(task, addr, result, fault_flags,
                                 (find_phys_fn_t)stackshot_find_phys,
                                 kdp_fault_result_flags) == KERN_SUCCESS;
}
static int
stackshot_copyin_string(task_t task, uint64_t addr, char *buf, int buf_sz,
                        boolean_t try_fault,
                        kdp_fault_result_flags_t *kdp_fault_result_flags) {
  kdp_fault_flags_t fault_flags = KDP_FAULT_FLAGS_NONE;
  if (try_fault) {
  ...
  return kdp_generic_copyin_string(task, addr, buf, buf_sz, fault_flags,
                                   (find_phys_fn_t)stackshot_find_phys,
                                   kdp_fault_result_flags);
}

kern_return_t do_stackshot(void *context) {
#pragma unused(context)
  ...
  if (!stackshot_ctx.sc_is_singlethreaded) {
    ...
    size_t buf_per_cpu =
        stackshot_args.buffer_size /
        os_atomic_load(&stackshot_ctx.sc_cpus_working, relaxed);
    ...
    for (int buf_idx = 0; buf_idx < stackshot_ctx.sc_num_buffers; buf_idx++) {
      ...
      stackshot_ctx.sc_buffers[buf_idx] =
          (struct stackshot_buffer){.ssb_ptr = (void *)cur_addr,
                                    .ssb_size = bufsz,
                                    .ssb_used = 0,
                                    .ssb_freelist = NULL,
                                    .ssb_freelist_lock = 0,
    ...
    assert(cur_addr <= ((mach_vm_address_t)stackshot_args.buffer +
                        stackshot_args.buffer_size));
#else  /* __arm64__ */
    /*
     * On Intel, we always just have one buffer
     */
    stackshot_ctx.sc_buffers[0] =
        (struct stackshot_buffer){.ssb_ptr = stackshot_args.buffer,
    ...
    /* Set up queues. These numbers shouldn't change, but slightly fudge queue
     * size just in case. */
    queue_size = FUDGED_SIZE(tasks_count + terminated_tasks_count, 10);
    for (size_t i = 0; i < STACKSHOT_NUM_WORKQUEUES; i++) {
      stackshot_ctx.sc_workqueues[i] = (struct stackshot_workqueue){
          .sswq_items = stackshot_alloc_arr(struct stackshot_workitem,
                                            queue_size, &error),
  ...
  if (stackshot_ctx.sc_panic_stackshot ||
      (stackshot_ctx.sc_main_cpuid == cpu_number())) {
  ...
  stackshot_panic_guard();
#if STACKSHOT_COLLECTS_LATENCY_INFO
  stackshot_ctx.sc_latency.cpu_wait_latency_mt =
      mach_absolute_time() - stackshot_ctx.sc_latency.cpu_wait_latency_mt;
#endif
...
#if SCHED_HYGIENE_DEBUG
  bool disable_interrupts_masked_check =
      kern_feature_override(KF_INTERRUPT_MASKED_DEBUG_STACKSHOT_OVRD) ||
  ...
  if (!stackshot_ctx.sc_panic_stackshot && interrupt_masked_debug_mode) {
    /*
     * Try to catch instances where stackshot takes too long BEFORE returning
     * from the debugger
     */
  ...
  if (!stackshot_ctx.sc_panic_stackshot &&
      stackshot_ctx.sc_retval == KERN_SUCCESS) {
    commit_exclaves_ast();
  }
  if (stackshot_ctx.sc_retval != KERN_SUCCESS &&
      stackshot_exclave_inspect_ctids) {
  ...
  /* If this is a singlethreaded stackshot, the "final" kcdata buffer is just
   * our CPU's kcdata buffer */
  if (stackshot_ctx.sc_is_singlethreaded) {
...
kern_return_t do_panic_stackshot(void *context) {
 ...
 */
void stackshot_cpu_preflight(void) {
...
#if CONFIG_PERVASIVE_CPI
  mt_cur_cpu_cycles_instrs_speculative(&stackshot_cpu_latency.total_cycles,
                                       &stackshot_cpu_latency.total_instrs);
#endif /* CONFIG_PERVASIVE_CPI */
  stackshot_cpu_latency.init_latency_mt =
      stackshot_cpu_latency.total_latency_mt = mach_absolute_time();
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */
  ...
  stackshot_cpu_ctx.scc_can_work =
      is_calling_cpu || (is_recommended && !stackshot_ctx.sc_is_singlethreaded);
  ...
  if (stackshot_cpu_ctx.scc_can_work) {
     ...
     */
#if defined(__arm64__)
    os_atomic_inc(&stackshot_ctx.sc_buffers[cpu_cluster_id()].ssb_size,
                  relaxed);
#endif /* __arm64__ */
...
stackshot_cpu_work_on_queue(struct stackshot_workqueue *queue) {
  ...
  while (((cur_workitemp = stackshot_get_workitem(queue)) != NULL ||
          !os_atomic_load(&queue->sswq_populated, acquire))) {
    ...
    if (cur_workitemp) {
      ...
      loop_wait();
#if STACKSHOT_COLLECTS_LATENCY_INFO
      stackshot_cpu_latency.workqueue_latency_mt +=
          mach_absolute_time() - time_begin;
#endif
    }
  }
...
static void stackshot_cpu_do_work(void) {
  ...
  stackshot_cpu_ctx.scc_stack_buffer =
      stackshot_alloc_arr(uintptr_t, MAX_FRAMES, &error);
  if (error != KERN_SUCCESS) {
    goto error_exit;
  }
...
#if STACKSHOT_COLLECTS_LATENCY_INFO
  stackshot_cpu_latency.init_latency_mt =
      mach_absolute_time() - stackshot_cpu_latency.init_latency_mt;
#endif
  ...
  if (high_perf) {
    ...
    for (size_t i = STACKSHOT_NUM_WORKQUEUES; i > 0; i--) {
      kcd_exit_on_error(
          stackshot_cpu_work_on_queue(&stackshot_ctx.sc_workqueues[i - 1]));
    }
  } else {
    /* Low Perf: Work from least difficult to most difficult */
    for (size_t i = 0; i < STACKSHOT_NUM_WORKQUEUES; i++) {
      kcd_exit_on_error(
          stackshot_cpu_work_on_queue(&stackshot_ctx.sc_workqueues[i]));
    }
  }
#if STACKSHOT_COLLECTS_LATENCY_INFO
  stackshot_cpu_latency.total_latency_mt =
      mach_absolute_time() - stackshot_cpu_latency.total_latency_mt;
#if CONFIG_PERVASIVE_CPI
  uint64_t cycles, instrs;
  mt_cur_cpu_cycles_instrs_speculative(&cycles, &instrs);
  stackshot_cpu_latency.total_cycles =
      cycles - stackshot_cpu_latency.total_cycles;
  stackshot_cpu_latency.total_instrs =
      instrs - stackshot_cpu_latency.total_instrs;
#endif /* CONFIG_PERVASIVE_CPI */
#endif /* STACKSHOT_COLLECTS_LATENCY_INFO */
 ...
 */
void stackshot_aux_cpu_entry(void) {
  /*
   * This is where the other CPUs will end up when we take a stackshot.
...
static void stackshot_coalition_jetsam_snapshot(void *arg, int i,
  ...
  struct jetsam_coalition_snapshot *coalitions =
      (struct jetsam_coalition_snapshot *)arg;
...
static void stackshot_thread_group_snapshot(void *arg, int i,
  ...
  tgs->tgs_flags =
      ...
      ((flags & THREAD_GROUP_FLAGS_STRICT_TIMERS) ? kThreadGroupStrictTimers
                                                  : 0) |
      0;
}
#endif /* CONFIG_THREAD_GROUPS */
...
static int stackshot_thread_has_valid_turnstileinfo(thread_t thread) {
  ...
  return stackshot_thread_has_valid_waitinfo(thread) && ts != TURNSTILE_NULL;
}
...
static void stackshot_thread_turnstileinfo(thread_t thread,
  ...
  tsinfo->portlabel_id = stackshot_plh_lookup(
      ispl, (tsinfo->turnstile_flags & STACKSHOT_TURNSTILE_STATUS_SENDPORT)
                ? STACKSHOT_PLH_LOOKUP_SEND
            : (tsinfo->turnstile_flags & STACKSHOT_TURNSTILE_STATUS_RECEIVEPORT)
                ? STACKSHOT_PLH_LOOKUP_RECEIVE
                : STACKSHOT_PLH_LOOKUP_UNKNOWN);
}
...
static void stackshot_thread_wait_owner_info(thread_t thread,
  ...
  case kThreadWaitPortReceive:
    kdp_mqueue_recv_find_owner(thread->waitq.wq_q, thread->wait_event, waitinfo,
                               &ispl);
    waitinfo->portlabel_id =
        stackshot_plh_lookup(ispl, STACKSHOT_PLH_LOOKUP_RECEIVE);
    break;
  case kThreadWaitPortSend:
    kdp_mqueue_send_find_owner(thread->waitq.wq_q, thread->wait_event, waitinfo,
                               &ispl);
    waitinfo->portlabel_id =
        stackshot_plh_lookup(ispl, STACKSHOT_PLH_LOOKUP_SEND);
    break;
  ...
  case kThreadWaitSleepWithInheritor:
    kdp_sleep_with_inheritor_find_owner(thread->waitq.wq_q, thread->wait_event,
                                        waitinfo_v1);
    break;
  case kThreadWaitEventlink:
    kdp_eventlink_find_owner(thread->waitq.wq_q, thread->wait_event,
                             waitinfo_v1);
    break;
  ...
  case kThreadWaitPageInThrottle:
    kdp_vm_object_sleep_find_owner(thread->wait_event, waitinfo->wait_type,
                                   waitinfo_v1);
    break;
  default:
    waitinfo->owner = 0;
