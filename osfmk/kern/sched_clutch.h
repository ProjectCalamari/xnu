 ...
 */
#define SCHED_CLUTCH_THREAD_ELIGIBLE(thread)                                   \
  ((thread->bound_processor) == PROCESSOR_NULL)

#if CONFIG_SCHED_EDGE
#define SCHED_CLUTCH_THREAD_CLUSTER_BOUND(thread)                              \
 ...
 * and the worst-case-execution-latency values specied in
 ...
 */
struct sched_clutch_root {
  /* (P) root level priority; represents the highest runnable thread in the
   * hierarchy */
  int16_t scr_priority;
  /* (P) total number of runnable threads in the hierarchy */
  uint16_t scr_thr_count;
  /* (P) root level urgency; represents the urgency of the whole hierarchy for
  ...
  /* (P) bitmap of all runnable unbounded root buckets which have warps
   * remaining */
...
struct sched_clutch_bucket {
  ...
  /* (P) linkage for all clutch_buckets in a root bucket; used for tick
   * operations */
...
void sched_clutch_thread_clutch_update(thread_t, sched_clutch_t,
                                       sched_clutch_t);
...
/*
 * Getter and Setter for Edge configuration. Used by CLPC to affect thread
 * migration behavior.
 */
void sched_edge_matrix_get(sched_clutch_edge *edge_matrix,
                           bool *edge_request_bitmap, uint64_t flags,
                           uint64_t num_psets);
void sched_edge_matrix_set(sched_clutch_edge *edge_matrix,
                           bool *edge_changes_bitmap, uint64_t flags,
                           uint64_t num_psets);
...
uint16_t
sched_edge_shared_rsrc_runnable_load(sched_clutch_root_t root_clutch,
                                     cluster_shared_rsrc_type_t load_type);
 ...
 * by least pset id.
 */
extern int (*sched_edge_search_order_weight_then_locality_cmp)(const void *a,
                                                               const void *b);
