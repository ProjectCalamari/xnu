...
int ccgcm_inc_iv(CC_UNUSED const struct ccmode_gcm *mode, ccgcm_ctx *ctx,
                 void *iv) {
  CC_ENSURE_DIT_ENABLED
...
int ccgcm_one_shot(const struct ccmode_gcm *mode, size_t key_nbytes,
                   const void *key, size_t iv_nbytes, const void *iv,
                   size_t adata_nbytes, const void *adata, size_t nbytes,
                   const void *in, void *out, size_t tag_nbytes, void *tag) {
  CC_ENSURE_DIT_ENABLED
  ...
  rc = ccgcm_set_iv(mode, ctx, iv_nbytes, iv);
  cc_require(rc == 0, errOut);
  rc = ccgcm_aad(mode, ctx, adata_nbytes, adata);
  cc_require(rc == 0, errOut);
  rc = ccgcm_update(mode, ctx, nbytes, in, out);
  cc_require(rc == 0, errOut);
  rc = ccgcm_finalize(mode, ctx, tag_nbytes, tag);
  cc_require(rc == 0, errOut);
...
// ccgcm_one_shot_legacy() is created because in the previous implementation of
// aes-gcm set_iv() could be skipped. In the new version of aes-gcm set_iv()
// cannot be skipped and IV length cannot be zero, as specified in FIPS. do not
// call ccgcm_one_shot_legacy() in any new application
int ccgcm_set_iv_legacy(const struct ccmode_gcm *mode, ccgcm_ctx *key,
  ...
  if (iv_nbytes == 0 || iv == NULL) {
    /* must be in IV state */
    cc_require(_CCMODE_GCM_KEY(key)->state == CCMODE_GCM_STATE_IV,
               errOut); /* CRYPT_INVALID_ARG */
...
int ccgcm_one_shot_legacy(const struct ccmode_gcm *mode, size_t key_nbytes,
                          const void *key, size_t iv_nbytes, const void *iv,
                          size_t adata_nbytes, const void *adata, size_t nbytes,
                          const void *in, void *out, size_t tag_nbytes,
                          void *tag) {
  CC_ENSURE_DIT_ENABLED
  ...
  rc = ccgcm_init(mode, ctx, key_nbytes, key);
  cc_require(rc == 0, errOut);
  rc = ccgcm_set_iv_legacy(mode, ctx, iv_nbytes, iv);
  cc_require(rc == 0, errOut);
  rc = ccgcm_aad(mode, ctx, adata_nbytes, adata);
  cc_require(rc == 0, errOut);
  rc = ccgcm_update(mode, ctx, nbytes, in, out);
...
void ccmode_gcm_update_pad(ccgcm_ctx *key) {
  ...
  CCMODE_GCM_KEY_ECB(key)->ecb(CCMODE_GCM_KEY_ECB_KEY(key), 1,
                               CCMODE_GCM_KEY_Y(key), CCMODE_GCM_KEY_PAD(key));
}
