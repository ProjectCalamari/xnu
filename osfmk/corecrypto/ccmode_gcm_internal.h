 ...
 *  @discussion For decryption, the tag parameter must be the expected-tag. A
 * secure compare is performed between the provided expected-tag and the
 * computed-tag. If they are the same, 0 is returned. Otherwise, non zero is
 * returned. For encryption, tag is output and provides the authentication tag.
 *
 */
...
#define CCMODE_GCM_KEY_ECB_KEY(K)                                              \
  ((ccecb_ctx *)_CCMODE_GCM_ECB_MODE(K)->ecb_key) // set in init function
