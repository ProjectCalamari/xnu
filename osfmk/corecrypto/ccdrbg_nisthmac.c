...
static void dump_state(const char *label,
                       struct ccdrbg_nisthmac_state *drbg_ctx) {
...
static bool entropy_isvalid(size_t entropy_nbytes,
                            const struct ccdigest_info *info) {
  return (entropy_nbytes <= CCDRBG_MAX_ENTROPY_SIZE) &&
         (entropy_nbytes >= MIN_REQ_ENTROPY(info));
}

// See NIST SP 800-90A, Rev. 1, 9.1 and 10.1.2.3
static int init(const struct ccdrbg_info *info, struct ccdrbg_state *ctx,
                size_t entropy_nbytes, const void *entropy, size_t nonce_nbytes,
                const void *nonce, size_t ps_nbytes, const void *ps) {
  struct ccdrbg_nisthmac_state *drbg_ctx = (struct ccdrbg_nisthmac_state *)ctx;
...
static int reseed(struct ccdrbg_state *ctx, size_t entropy_nbytes,
                  const void *entropy, size_t add_nbytes, const void *add) {
  struct ccdrbg_nisthmac_state *drbg_ctx = (struct ccdrbg_nisthmac_state *)ctx;
  const struct ccdigest_info *digest_info = drbg_ctx->custom->di;
...
static bool must_reseed(const struct ccdrbg_state *ctx) {
  const struct ccdrbg_nisthmac_state *drbg_ctx =
      (const struct ccdrbg_nisthmac_state *)ctx;
...
static int generate(struct ccdrbg_state *ctx, size_t out_nbytes, void *out,
                    size_t add_nbytes, const void *add) {
  struct ccdrbg_nisthmac_state *drbg_ctx = (struct ccdrbg_nisthmac_state *)ctx;
  const struct ccdigest_info *info = drbg_ctx->custom->di;
  size_t outlen = info->output_size;
...
void ccdrbg_factory_nisthmac(struct ccdrbg_info *info,
  ...
  info->size = sizeof(struct ccdrbg_nisthmac_state) +
