...
#define CC_ENSURE_DIT_ENABLED                                                  \
  volatile bool _cc_dit_auto_disable __attribute__((cleanup(cc_disable_dit)))  \
...
#define CC_MULTI_IF_AND(condition) ((condition) && (condition) && (condition))
...
CC_INLINE uint32_t CC_ROL(uint32_t word, int i) {
  __asm__("roll %%cl,%0" : "=r"(word) : "0"(word), "c"(i));
  return word;
}

CC_INLINE uint32_t CC_ROR(uint32_t word, int i) {
  __asm__("rorl %%cl,%0" : "=r"(word) : "0"(word), "c"(i));
  return word;
}
...
#define CC_ROLc(word, i)                                                       \
  ({                                                                           \
    uint32_t _word = (word);                                                   \
    __asm__ __volatile__("roll %2,%0" : "=r"(_word) : "0"(_word), "I"(i));     \
    _word;                                                                     \
  })
...
#define CC_RORc(word, i)                                                       \
  ({                                                                           \
    uint32_t _word = (word);                                                   \
    __asm__ __volatile__("rorl %2,%0" : "=r"(_word) : "0"(_word), "I"(i));     \
    _word;                                                                     \
  })
...
// MARK: - 64 bits rotates
    ...
    !defined(                                                                  \
        _MSC_VER) // clang _MSVC doesn't support GNU-style inline assembly
// MARK: -- intel 64 asm version

CC_INLINE uint64_t CC_ROL64(uint64_t word, int i) {
  __asm__("rolq %%cl,%0" : "=r"(word) : "0"(word), "c"(i));
  return word;
}

CC_INLINE uint64_t CC_ROR64(uint64_t word, int i) {
  __asm__("rorq %%cl,%0" : "=r"(word) : "0"(word), "c"(i));
  return word;
}
...
#define CC_ROL64c(word, i)                                                     \
  ({                                                                           \
    uint64_t _word = (word);                                                   \
    __asm__("rolq %2,%0" : "=r"(_word) : "0"(_word), "J"(i));                  \
    _word;                                                                     \
  })
...
#define CC_ROR64c(word, i)                                                     \
  ({                                                                           \
    uint64_t _word = (word);                                                   \
    __asm__("rorq %2,%0" : "=r"(_word) : "0"(_word), "J"(i));                  \
    _word;                                                                     \
  })
...
#elif defined(__x86_64__) || defined(__i386__) || defined(__arm64__) ||        \
    defined(__arm__)
...
CC_INLINE CC_CONST unsigned cc_clz32(uint32_t data) {
  ...
  cc_static_assert(sizeof(unsigned) == 4,
                   "clz relies on an unsigned int being 4 bytes");
...
CC_INLINE CC_CONST unsigned cc_ctz32(uint32_t data) {
  ...
  cc_static_assert(sizeof(unsigned) == 4,
                   "ctz relies on an unsigned int being 4 bytes");
 ...
 *
 *  @result One plus the index of the least-significant bit set in @p data or,
 * if @p data is zero, zero
 */
...
/*!
 *  @brief     CC_MUXU(r, s, a, b) is equivalent to r = s ? a : b, but executes
 * in constant time
 *  @param a   Input a
 *  @param b   Input b
