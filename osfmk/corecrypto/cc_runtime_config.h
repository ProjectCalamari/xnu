...
#define CC_HAS_SupplementalSSE3()                                              \
  ((cpuid_features() & CPUID_FEATURE_SSSE3) != 0)
#define CC_HAS_AVX1() ((cpuid_features() & CPUID_FEATURE_AVX1_0) != 0)
#define CC_HAS_AVX2()                                                          \
  ((cpuid_info()->cpuid_leaf7_features & CPUID_LEAF7_FEATURE_AVX2) != 0)
#define CC_HAS_AVX512_AND_IN_KERNEL()                                          \
  ((cpuid_info()->cpuid_leaf7_features & CPUID_LEAF7_FEATURE_AVX512F) != 0)
#define CC_HAS_BMI2()                                                          \
  ((cpuid_info()->cpuid_leaf7_features & CPUID_LEAF7_FEATURE_BMI2) != 0)
#define CC_HAS_ADX()                                                           \
  ((cpuid_info()->cpuid_leaf7_features & CPUID_LEAF7_FEATURE_ADX) != 0)
...
CC_INLINE bool _cpu_supports(uint64_t leaf, uint64_t subleaf,
                             uint8_t cpuid_register, uint8_t bit) {
  uint64_t registers[4] = {0};
  registers[CPUID_REG_RAX] = leaf;
  registers[CPUID_REG_RCX] = subleaf;
  if (oe_emulate_cpuid(&registers[CPUID_REG_RAX], &registers[CPUID_REG_RBX],
                       &registers[CPUID_REG_RCX], &registers[CPUID_REG_RDX])) {
    return false;
  }
...
#define CC_HAS_SupplementalSSE3()                                              \
  _cpu_supports(1, 0, CPUID_REG_RCX, CPUID_FEATURE_SSE3)
#define CC_HAS_AVX1() _cpu_supports(1, 0, CPUID_REG_RCX, CPUID_FEATURE_AVX1)
#define CC_HAS_AVX2()                                                          \
  _cpu_supports(7, 0, CPUID_REG_RBX, CPUID_FEATURE_LEAF7_AVX2)
#define CC_HAS_AVX512_AND_IN_KERNEL() 0
#define CC_HAS_BMI2()                                                          \
  _cpu_supports(7, 0, CPUID_REG_RBX, CPUID_FEATURE_LEAF7_BMI2)
...
#define CC_COMMPAGE_CPU_CAPABILITIES                                           \
  (*((volatile __attribute__((address_space(1)))                               \
      uint64_t *)_COMM_PAGE_CPU_CAPABILITIES64))
#else
#define CC_COMMPAGE_CPU_CAPABILITIES                                           \
