...
#define CCMODE_CFB_FACTORY(_cipher_, _mode_, _dir_)                            \
  static CC_READ_ONLY_LATE(struct ccmode_##_mode_)                             \
      _mode_##_##_cipher_##_##_dir_;                                           \
                                                                               \
  const struct ccmode_##_mode_ *cc##_cipher_##_##_mode_##_##_dir_##_mode(      \
      void) {                                                                  \
...
#define CCMODE_FACTORY_CBC_DECRYPT(ECB)                                        \
  {.size = ccn_sizeof_size(sizeof(struct _ccmode_cbc_key)) +                   \
           ccn_sizeof_size((ECB)->block_size) + ccn_sizeof_size((ECB)->size),  \
...
int ccmode_cfb_decrypt(cccfb_ctx *ctx, size_t nbytes, const void *in,
                       void *out);
int ccmode_cfb_encrypt(cccfb_ctx *ctx, size_t nbytes, const void *in,
                       void *out);

/* Use this to statically initialize a ccmode_cfb object for decryption. */
#define CCMODE_FACTORY_CFB_DECRYPT(ECB)                                        \
  {.size = ccn_sizeof_size(sizeof(struct _ccmode_cfb_key)) +                   \
           2 * ccn_sizeof_size((ECB)->block_size) +                            \
...
#define CCMODE_CFB_KEY_IV(K)                                                   \
  (&_CCMODE_CFB_KEY(K)->u[ccn_nof_size(CCMODE_CFB_KEY_ECB(K)->block_size)])
#define CCMODE_CFB_KEY_ECB_KEY(K)                                              \
  ((ccecb_ctx *)&_CCMODE_CFB_KEY(K)                                            \
       ->u[2 * ccn_nof_size(CCMODE_CFB_KEY_ECB(K)->block_size)])

int ccmode_cfb8_init(const struct ccmode_cfb8 *cfb8, cccfb8_ctx *ctx,
                     size_t rawkey_len, const void *rawkey, const void *iv);
int ccmode_cfb8_decrypt(cccfb8_ctx *ctx, size_t nbytes, const void *in,
                        void *out);
int ccmode_cfb8_encrypt(cccfb8_ctx *ctx, size_t nbytes, const void *in,
                        void *out);

/* Use this to statically initialize a ccmode_cfb8 object for decryption. */
#define CCMODE_FACTORY_CFB8_DECRYPT(ECB)                                       \
  {.size = ccn_sizeof_size(sizeof(struct _ccmode_cfb8_key)) +                  \
           2 * ccn_sizeof_size((ECB)->block_size) +                            \
           ccn_sizeof_size((ECB)->size),                                       \
   .block_size = 1,                                                            \
...
#define CCMODE_CFB8_KEY_IV(K)                                                  \
  (&_CCMODE_CFB8_KEY(K)->u[ccn_nof_size(CCMODE_CFB8_KEY_ECB(K)->block_size)])
#define CCMODE_CFB8_KEY_ECB_KEY(K)                                             \
  ((ccecb_ctx *)&_CCMODE_CFB8_KEY(K)                                           \
       ->u[2 * ccn_nof_size(CCMODE_CFB8_KEY_ECB(K)->block_size)])
...
#define CCMODE_FACTORY_CTR_CRYPT(ECB_ENCRYPT)                                  \
  {.size = ccn_sizeof_size(sizeof(struct _ccmode_ctr_key)) +                   \
           2 * ccn_sizeof_size((ECB_ENCRYPT)->block_size) +                    \
...
#define CCMODE_CTR_KEY_CTR(K)                                                  \
  (&_CCMODE_CTR_KEY(K)->u[ccn_nof_size(CCMODE_CTR_KEY_ECB(K)->block_size)])
#define CCMODE_CTR_KEY_ECB_KEY(K)                                              \
  ((ccecb_ctx *)&_CCMODE_CTR_KEY(K)                                            \
       ->u[2 * ccn_nof_size(CCMODE_CTR_KEY_ECB(K)->block_size)])

CC_INLINE int ccctr_setctr(const struct ccmode_ctr *mode, ccctr_ctx *ctx,
                           const void *ctr) {
...
#define CCMODE_FACTORY_OFB_CRYPT(ECB)                                          \
  {.size = ccn_sizeof_size(sizeof(struct _ccmode_ofb_key)) +                   \
           ccn_sizeof_size((ECB)->block_size) + ccn_sizeof_size((ECB)->size),  \
   .block_size = 1,                                                            \
...
#define CCMODE_OFB_KEY_ECB_KEY(K)                                              \
  ((ccecb_ctx *)&_CCMODE_OFB_KEY(K)                                            \
       ->u[ccn_nof_size(CCMODE_OFB_KEY_ECB(K)->block_size)])
...
void ccmode_xts_key_sched(const struct ccmode_xts *xts, ccxts_ctx *ctx,
                          size_t key_nbytes, const void *data_key,
                          const void *tweak_key);
void *ccmode_xts_crypt(const ccxts_ctx *ctx, ccxts_tweak *tweak, size_t nblocks,
                       const void *in, void *out);
...
#define CCMODE_FACTORY_XTS_DECRYPT(ECB, ECB_ENCRYPT)                           \
  {                                                                            \
      .size = ccn_sizeof_size(sizeof(struct _ccmode_xts_key)) +                \
              2 * ccn_sizeof_size((ECB)->size),                                \
      .tweak_size = ccn_sizeof_size(sizeof(struct _ccmode_xts_tweak)) +        \
                    ccn_sizeof_size(ecb->block_size),                          \
      .block_size = ecb->block_size,                                           \
...
struct _ccmode_xts_tweak {
  // FIPS requires that for XTS that no more that 2^20 AES blocks may be
  // processed for any given Key, Tweak Key, and tweak combination the
  // bytes_processed field in the context will accumuate the number of blocks
  // processed and will fail the encrypt/decrypt if the size is violated.  This
  // counter will be reset to 0 when set_tweak is called.
...
#define CCMODE_XTS_KEY_TWEAK_KEY(K)                                            \
  ((ccecb_ctx *)&_CCMODE_XTS_KEY(K)                                            \
       ->u[ccn_nof_size(CCMODE_XTS_KEY_ECB(K)->size)])
...
const ccecb_ctx *ccmode_xts_key_tweak_key(const ccxts_ctx *K) {
  return (const ccecb_ctx *)&((const struct _ccmode_xts_key *)K)
      ->u[ccn_nof_size(ccmode_xts_key_ecb(K)->size)];
}
...
#define CCMODE_XTS_TWEAK_BLOCK_PROCESSED(T)                                    \
  (_CCMODE_XTS_TWEAK(T)->blocks_processed)
...
int ccmode_ccm_init(const struct ccmode_ccm *ccm, ccccm_ctx *ctx,
                    size_t rawkey_len, const void *rawkey);
int ccmode_ccm_set_iv(ccccm_ctx *ctx, ccccm_nonce *nonce_ctx, size_t nonce_len,
                      const void *nonce, size_t mac_size, size_t auth_len,
                      size_t data_len);
/* internal function */
void ccmode_ccm_macdata(ccccm_ctx *key, ccccm_nonce *nonce_ctx,
                        unsigned new_block, size_t nbytes, const void *in);
/* api function - disallows only mac'd data after data to encrypt was sent */
int ccmode_ccm_cbcmac(ccccm_ctx *ctx, ccccm_nonce *nonce_ctx, size_t nbytes,
                      const void *in);
/* internal function */
void ccmode_ccm_crypt(ccccm_ctx *key, ccccm_nonce *nonce_ctx, size_t nbytes,
                      const void *in, void *out);
int ccmode_ccm_decrypt(ccccm_ctx *ctx, ccccm_nonce *nonce_ctx, size_t nbytes,
                       const void *in, void *out);
int ccmode_ccm_encrypt(ccccm_ctx *ctx, ccccm_nonce *nonce_ctx, size_t nbytes,
                       const void *in, void *out);
int ccmode_ccm_finalize(ccccm_ctx *key, ccccm_nonce *nonce_ctx, void *mac);
int ccmode_ccm_finalize_and_verify(ccccm_ctx *ctx, ccccm_nonce *nonce_ctx,
                                   void *mac);
...
#define CCMODE_FACTORY_CCM_DECRYPT(ECB_ENCRYPT)                                \
  {                                                                            \
      .size = ccn_sizeof_size(sizeof(struct _ccmode_ccm_key)) +                \
...
int ccmode_omac_decrypt(ccomac_ctx *ctx, size_t nblocks, const void *tweak,
                        const void *in, void *out);
int ccmode_omac_encrypt(ccomac_ctx *ctx, size_t nblocks, const void *tweak,
                        const void *in, void *out);
