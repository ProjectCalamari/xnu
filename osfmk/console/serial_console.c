...
#define KERN_CONSOLE_RING_SIZE                                                 \
  vm_map_round_page(CPU_CONS_BUF_SIZE *(MAX_CPUS + 1), PAGE_SIZE - 1)
...
TUNABLE(bool, allow_printf_from_interrupts_disabled_context, "nointr_consio",
        false);
#else
#define allow_printf_from_interrupts_disabled_context false
#endif

SECURITY_READ_ONLY_EARLY(struct console_ops)
cons_ops[] = {
    {
        .putc = serial_putc_options,
        .getc = _serial_getc,
    },
    {
        .putc = vcputc_options,
...
nconsops = (sizeof cons_ops / sizeof cons_ops[0]);
...
/* If the NMI string is entered into the console, the system will enter the
 * debugger. */
 ...
 */
static inline bool console_io_allowed(void) {
  if (!allow_printf_from_interrupts_disabled_context && !console_suspended &&
      startup_phase >= STARTUP_SUB_EARLY_BOOT && !ml_get_interrupts_enabled()) {
...
void console_init(void) {
  ...
  kmem_alloc(kernel_map, (vm_offset_t *)&console_ring.buffer,
             KERN_CONSOLE_RING_SIZE + ptoa(2),
             KMA_NOFAIL | KMA_PERMANENT | KMA_KOBJECT | KMA_PERMANENT |
                 KMA_GUARD_FIRST | KMA_GUARD_LAST | KMA_ZERO | KMA_DATA,
 ...
 */
static inline bool is_console_initialized(void) {
 ...
 */
static inline uint32_t get_cons_ops_index(void) {
 ...
 */
static inline void _cnputc(char c, bool poll) {
 ...
 *
 * @note disableConsoleOutput is to be used only by core console and init
 * accessors such as this function. Returns early if the serial output is
 * disabled.
 *
 ...
 */
static inline void _cnputs(char *c, int size, bool poll) {
 ...
 *         space.
 */
static inline char *console_ring_reserve_space(int nchars) {
  char *write_ptr = NULL;
  ...
  if ((console_ring.len - console_ring.used) >= nchars) {
    ...
    /* Move the console ring's write pointer to the beginning of the next free
     * space. */
    const ptrdiff_t write_index = console_ring.write_ptr - console_ring.buffer;
    console_ring.write_ptr =
        console_ring.buffer + ((write_index + nchars) % console_ring.len);
  }
...
/**
 * Decrement the number of reserved spaces in the console ring (now that the
 * data has been written) and re-enable preemption.
 *
 * Every call to this function should be paired with a corresponding call to
 * console_ring_reserve_space().
 */
static inline void console_ring_unreserve_space(void) {
 ...
 */
static inline void console_ring_put(char **write_ptr, char ch) {
  assert(console_ring.nreserved > 0);
  **write_ptr = ch;
  ++(*write_ptr);
 ...
 *
 * @note To prevent one thread from being the drain thread for too long
 * (presumably that thread has other things it wants to do besides draining
 * serial), the total number of characters a single call to this function can
 * drain is restricted to MAX_TOTAL_FLUSH_SIZE.
 */
static void console_ring_try_empty(bool fail_fast) {
  ...
  if (__improbable(!console_io_allowed()) ||
      (!in_debugger && get_preemption_level() != 0)) {
    return;
  }

  do {
    if (__probable(!in_debugger) && fail_fast &&
        !lck_mtx_try_lock(&console_ring.flush_lock)) {
      return;
    ...
    /* Try small chunk at a time, so we allow writes from other cpus into the
     * buffer. */
    nchars_out = MIN(console_ring.used, (int)sizeof(flush_buf));

    /* Account for data to be read before wrap around. */
    size_before_wrap =
        (int)((console_ring.buffer + console_ring.len) - console_ring.read_ptr);
    ...
    if (nchars_out > 0) {
      ...
      console_ring.read_ptr =
          console_ring.buffer + ((read_index + nchars_out) % console_ring.len);
 ...
 */
void console_resume() { console_suspended = false; }
...
void console_write(char *str, int size) {
  ...
  if (__improbable(console_suspended || !is_console_initialized() ||
                   pmap_in_ppl())) {
    /*
...
/**
 * Output a character directly to the underlying console (either video or
 * serial). This directly bypasses the console serial buffer (as provided by
 * console_write()) and all of the synchronization that provides.
 *
 ...
 * @param c The character to print.
 */
void console_write_unbuffered(char c) {
  if (disableConsoleOutput) {
    return;
  }
 ...
 * @param c The character to print.
 */
void console_write_char(char c) { console_write(&c, 1); }
 ...
 */
int _vcgetc(__unused bool wait) {
  char c;
...
/*
 * Log to console by multiple methods - printf, unbuffered write,
 * console_write()
 */
static void log_to_console_func(void *arg __unused,
...
/* Test that outputting to the console can occur on multiple threads at the same
 * time. */
kern_return_t console_serial_parallel_log_tests(void) {
  thread_t thread;
  kern_return_t kr;
  ...
  T_LOG("parallel_logging tests is now complete. From this point forward we "
        "expect full lines\n");
  return KERN_SUCCESS;
}

/* Basic serial test that prints serial output through various methods
 * (printf/T_LOG). */
kern_return_t console_serial_test(void) {
  unsigned long i;
  ...
  T_ASSERT_EQ_INT(console_ring.len, KERN_CONSOLE_RING_SIZE,
  ...
  T_LOG("Printing %d char string to serial one char at a time.",
        CPU_CONS_BUF_SIZE);
  for (i = 0; i < CPU_CONS_BUF_SIZE; i++) {
    printf("%c", buffer[i]);
  }
