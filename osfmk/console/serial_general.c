...
serialmode; /* Serial mode keyboard and console control */
...
void serial_keyboard_init(void) {
  ...
  if (!(serialmode &
        SERIALMODE_INPUT)) { /* Leave if we do not want a serial console */
    return;
  }

  kprintf("Serial keyboard started\n");
  result = kernel_thread_start_priority(
      (thread_continue_t)serial_keyboard_start, NULL, MAXPRI_KERNEL, &thread);
...
__dead2 void serial_keyboard_poll(void) {
  int chr;
  uint64_t next;
  ...
  assert_wait_deadline((event_t)serial_keyboard_poll, THREAD_UNINT,
                       next); /* Show we are "waiting" */
  thread_block((thread_continue_t)serial_keyboard_poll); /* Wait for it */
  panic("serial_keyboard_poll: Shouldn't never ever get here...");
}

boolean_t console_is_serial(void) { return cons_ops_index == SERIAL_CONS_OPS; }
...
void console_printbuf_state_init(struct console_printbuf_state *data,
                                 int write_on_newline, int can_block) {
  if (data == NULL) {
    return;
  }
