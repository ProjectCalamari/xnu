...
ipc_voucher_attr_control_t
    ipc_pthread_priority_voucher_attr_control; /* communication channel from
                                                  PTHPRIORITY to voucher system
                                                */

#define PTHPRIORITY_ATTR_DEFAULT_VALUE (0)

#define IPC_PTHREAD_PRIORITY_VALUE_TO_HANDLE(x)                                \
  ((mach_voucher_attr_value_handle_t)(x))
#define HANDLE_TO_IPC_PTHREAD_PRIORITY_VALUE(x)                                \
  ((ipc_pthread_priority_value_t)(x))

kern_return_t ipc_pthread_priority_release_value(
    ipc_voucher_attr_manager_t __assert_only manager,
    mach_voucher_attr_key_t __assert_only key,
    mach_voucher_attr_value_handle_t value,
    mach_voucher_attr_value_reference_t sync);

kern_return_t ipc_pthread_priority_get_value(
    ipc_voucher_attr_manager_t __assert_only manager,
    mach_voucher_attr_key_t __assert_only key,
...
kern_return_t ipc_pthread_priority_extract_content(
    ipc_voucher_attr_manager_t __assert_only manager,
    mach_voucher_attr_key_t __assert_only key,
...
kern_return_t ipc_pthread_priority_command(
    ipc_voucher_attr_manager_t __assert_only manager,
    mach_voucher_attr_key_t __assert_only key,
    mach_voucher_attr_value_handle_array_t values,
 ...
 * Returns: None.
 */
__startup_func static void ipc_pthread_priority_init(void) {
  /* Register the ipc_pthread_priority manager with the Vouchers sub system. */
  ipc_register_well_known_mach_voucher_attr_manager(
      &ipc_pthread_priority_manager, 0, MACH_VOUCHER_ATTR_KEY_PTHPRIORITY,
...
kern_return_t ipc_pthread_priority_release_value(
  ...
  panic("ipc_pthread_priority_release_value called for a persistent "
        "PTHPRIORITY value %x with sync value %d",
  ...
  *out_flags = MACH_VOUCHER_ATTR_VALUE_FLAGS_NONE;
  ...
  case MACH_VOUCHER_ATTR_PTHPRIORITY_CREATE:
    ...
    canonicalize_priority_value =
        (ipc_pthread_priority_value_t)_pthread_priority_normalize_for_ipc(
            (unsigned long)ipc_pthread_priority_value);
 ...
 * Purpose: Extract a set of pthread_priority value from an array of voucher
 * values. Returns: KERN_SUCCESS: on Success. KERN_NO_SPACE: insufficeint buffer
 * provided to fill an array of pthread_priority values.
 */
kern_return_t ipc_pthread_priority_extract_content(
    ipc_voucher_attr_manager_t __assert_only manager,
    mach_voucher_attr_key_t __assert_only key,
  ...
  for (i = 0; i < value_count && *in_out_recipe_size > 0; i++) {
    ipc_pthread_priority_value =
        HANDLE_TO_IPC_PTHREAD_PRIORITY_VALUE(values[i]);
    ...
    memcpy(&out_recipe[0], &ipc_pthread_priority_value,
           sizeof(ipc_pthread_priority_value));
    *out_command = MACH_VOUCHER_ATTR_PTHPRIORITY_NULL;
    *in_out_recipe_size =
        (mach_voucher_attr_content_size_t)sizeof(ipc_pthread_priority_value);
    return kr;
  }
