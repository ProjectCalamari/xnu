 ...
 * ================
 *
 * A corpse is a state of process that is past the point of its death. This
 * means that process has completed all its termination operations like
 * releasing file descriptors, mach ports, sockets and other constructs used to
 * identify a process. For all the processes this mimics the behavior as if the
 * process has died and no longer available by any means.
 *
 * Why do we need Corpses?
 * -----------------------
 * For crash inspection we need to inspect the state and data that is associated
 * with process so that crash reporting infrastructure can build backtraces,
 * find leaks etc.
 *
 * Corpses functionality in kernel
 * ===============================
 * The corpse functionality is an extension of existing exception reporting
 * mechanisms we have. The exception_triage calls will try to deliver the first
 * round of exceptions allowing task/debugger/ReportCrash/launchd level
 * exception handlers to respond to exception. If even after notification the
 * exception is not handled, then the process begins the death operations and
 * during proc_prepareexit, we decide to create a corpse for inspection.
 * Following is a sample run through of events and data shuffling that happens
 * when corpses is enabled.
 *
 *   * a process causes an exception during normal execution of threads.
 *   * The exception generated by either mach(e.g GUARDED_MARCHPORT) or bsd(eg
 * SIGABORT, GUARDED_FD etc) side is passed through the exception_triage()
 * function to follow the thread -> task -> host level exception handling
 * system. This set of steps are same as before and allow for existing crash
 * reporting systems (both internal and 3rd party) to catch and create reports
 * as required.
 *   * If above exception handling returns failed (when nobody handles the
 * notification), then the proc_prepareexit path has logic to decide to create
 * corpse.
 *   * The task_mark_corpse function allocates userspace vm memory and attaches
 * the information kcdata_descriptor_t to task->corpse_info field of task.
 *     - All the task's threads are marked with the "inspection" flag which
 * signals the termination daemon to not reap them but hold until they are being
 * inspected.
 *     - task flags t_flags reflect the corpse bit and also a PENDING_CORPSE
 * bit. PENDING_CORPSE prevents task_terminate from stripping important data
 * from task.
 *     - It marks all the threads to terminate and return to AST for
 * termination.
 *     - The allocation logic takes into account the rate limiting policy of
 * allowing only `total_corpses_allowed` in flight.
 *   * The proc exit threads continues and collects required information in the
 * allocated vm region. Once complete it marks itself for termination.
 *   * In the thread_terminate_self(), the last thread to enter will do a call
 * to proc_exit(). Following this is a check to see if task is marked for corpse
 * notification and will invoke the the task_deliver_crash_notification().
 *   * Once EXC_CORPSE_NOTIFY is delivered, it removes the PENDING_CORPSE flag
 * from task (and inspection flag from all its threads) and allows
 * task_terminate to go ahead and continue the mach task termination process.
 *   * ASIDE: The rest of the threads that are reaching the
 * thread_terminate_daemon() with the inspection flag set are just bounced to
 * another holding queue (crashed_threads_queue). Only after the corpse
 * notification these are pulled out from holding queue and enqueued back to
 * termination queue
 *
 *
 * Corpse info format
 * ==================
 * The kernel (task_mark_corpse()) makes a vm allocation in the dead task's vm
 * space (with tag VM_MEMORY_CORPSEINFO (80)). Within this memory all corpse
 * information is saved by various subsystems like
 *   * bsd proc exit path may write down pid, parent pid, number of file
 * descriptors etc
 *   * mach side may append data regarding ledger usage, memory stats etc
 * See detailed info about the memory structure and format in kern_cdata.h
 * documentation.
 *
 * Configuring Corpses functionality
 * =================================
 *   boot-arg: -no_corpses disables the corpse generation. This can be
 * added/removed without affecting any other subsystem.
 *   DEFAULT_TOTAL_CORPSES_ALLOWED: Controls the number of corpse instances to
 * be held for inspection before allowing memory to be reclaimed by the system.
 *     On a live system, the maximum corpse count can be reconfigured via the
 * `kern.total_corpses_allowed` sysctl. CORPSEINFO_ALLOCATION_SIZE: is the
 * default size of vm allocation. If in future there is much more data to be put
 * in, then please re-tune this parameter.
 *
 * Debugging/Visibility
 * ====================
 *   * lldbmacros for thread and task summary are updated to show "C" flag for
 * corpse task/threads.
 *   * there are macros to see list of threads in termination queue
 * (dumpthread_terminate_queue) and holding queue (dumpcrashed_thread_queue).
 *   * In case of corpse creation is disabled of ignored then the system log is
 * updated with printf data with reason.
 *
 * Limitations of Corpses
 * ======================
 *   With holding off memory for inspection, it creates vm pressure which might
 * not be desirable on low memory devices. There are limits to max corpses being
 * inspected at a time which is marked by `total_corpses_allowed`.
 *
 */
...
TUNABLE(bool, corpse_threshold_system_limit, "corpse_threshold_system_limit",
        false);
 ...
 */
boolean_t corpses_enabled(void) { return !corpses_disabled; }

unsigned long total_corpses_count(void) {
 ...
 */
static kern_return_t task_crashinfo_get_ref(corpse_flags_t kcd_u_flags) {
  ...
  for (;;) {
    ...
    if (atomic_compare_exchange_strong_explicit(
            &inflight_corpses, &oldgate.value, newgate.value,
 ...
 */
static kern_return_t task_crashinfo_release_ref(corpse_flags_t kcd_u_flags) {
  union corpse_creation_gate oldgate, newgate;
...
kcdata_descriptor_t task_crashinfo_alloc_init(mach_vm_address_t crash_data_p,
                                              unsigned size,
                                              corpse_flags_t kc_u_flags,
                                              unsigned kc_flags) {
...
kcdata_descriptor_t task_btinfo_alloc_init(mach_vm_address_t addr,
                                           unsigned size) {
  kcdata_descriptor_t kcdata;

  kcdata = kcdata_memory_alloc_init(addr, TASK_BTINFO_BEGIN, size,
                                    KCFLAG_USE_MEMCOPY);
 ...
 * returns: None.
 */
void task_add_to_corpse_task_list(task_t corpse_task) {
 ...
 */
kern_return_t find_corpse_task_by_uniqueid_grp(uint64_t task_uniqueid,
                                               task_t *target, task_grp_t grp) {
  task_t task;
 ...
 *          KERN_RESOURCE_SHORTAGE on memory alloc failure or reaching max
 * corpse.
 */
...
/*
 * Only generate lightweight corpse if any of thread, task, or host level
 * registers EXC_CORPSE_NOTIFY with behavior EXCEPTION_BACKTRACE.
 *
 * Save a send right and behavior of those ports on out param EXC_PORTS.
 */
static boolean_t task_should_generate_lightweight_corpse(
    task_t task, ipc_port_t exc_ports[static BT_EXC_PORTS_COUNT]) {
  kern_return_t kr;
 ...
 * corpse.
 */
kern_return_t task_enqueue_exception_with_corpse(
  ...
  kern_return_t kr;
  ipc_port_t exc_ports[BT_EXC_PORTS_COUNT]; /* send rights in thread, task, host
                                               order */
  ...
  if (lightweight && task_should_generate_lightweight_corpse(task, exc_ports)) {
    ...
    kr = current_thread_collect_backtrace_info(&desc, etype, code, codeCnt,
                                               reason);
    if (kr != KERN_SUCCESS) {
    ...
    kr = kcdata_create_object(desc, KCDATA_OBJECT_TYPE_LW_CORPSE,
                              BTINFO_ALLOCATION_SIZE, &obj);
  ...
  out:
    ...
    thread_set_exec_promotion(current_thread());
    /* Generate a corpse for the given task, will return with a ref on corpse
     * task */
    kr = task_generate_corpse_internal(task, &corpse, &thread, etype, code[0],
                                       code[1], reason);
    thread_clear_exec_promotion(current_thread());
    if (kr == KERN_SUCCESS) {
      if (thread == THREAD_NULL) {
 ...
 * corpse.
 */
kern_return_t task_generate_corpse_internal(task_t task, task_t *corpse_task,
                                            thread_t *exc_thread,
                                            exception_type_t etype,
                                            mach_exception_data_type_t code,
                                            mach_exception_data_type_t subcode,
  ...
  if (etype == EXC_GUARD &&
      EXC_GUARD_DECODE_GUARD_TYPE(code) == GUARD_TYPE_USER) {
  ...
  is_64bit_data =
      (task == TASK_NULL) ? is_64bit_addr : task_get_64bit_data(task);
  t_flags = TF_CORPSE_FORK | TF_PENDING_CORPSE |
  ...
  kr =
      task_create_internal(task, NULL, NULL, TRUE, is_64bit_addr, is_64bit_data,
                           t_flags, t_flags_ro, TPF_NONE, TWF_NONE, new_task);
  if (kr != KERN_SUCCESS) {
    new_task = TASK_NULL;
    goto error_task_generate_corpse;
  }
  ...
  kr = task_duplicate_map_and_threads(task, p, new_task, &thread, &udata_buffer,
                                      &size, &num_udata, (etype != 0));
  if (kr != KERN_SUCCESS) {
    goto error_task_generate_corpse;
  }
  ...
  /* Populate the corpse blob, use the proc struct of task instead of corpse
   * task */
  gather_populate_corpse_crashinfo(p, new_task, code, subcode, udata_buffer,
...
static kern_return_t task_map_kcdata_64(task_t task, void *kcdata_addr,
                                        mach_vm_address_t *uaddr,
                                        mach_vm_size_t kcd_size, vm_tag_t tag) {
  kern_return_t kr;
 ...
 */
kern_return_t task_map_corpse_info(task_t task, task_t corpse_task,
 ...
 * params: task - Map the corpse info in task's address space
 *         corpse_task - task port of the corpse
 *         kcd_addr_begin - address of the mapped corpse info (takes
 * mach_vm_addess_t *) kcd_size - size of the mapped corpse info (takes
 * mach_vm_size_t *) returns: KERN_SUCCESS on Success. KERN_FAILURE on Failure.
 *          KERN_INVALID_ARGUMENT on invalid arguments.
 */
kern_return_t task_map_corpse_info_64(task_t task, task_t corpse_task,
  ...
  if (task == TASK_NULL || task_is_a_corpse(task) || corpse_task == TASK_NULL ||
 ...
 *          KERN_INVALID_ARGUMENT on invalid arguments.
 */
kern_return_t task_map_kcdata_object_64(task_t task, kcdata_object_t kcdata_obj,
                                        mach_vm_address_t *kcd_addr_begin,
                                        mach_vm_size_t *kcd_size) {
  kern_return_t kr;
...
uint64_t task_corpse_get_crashed_thread_id(task_t corpse_task) {
