...
bool kperf_action_has_non_system(unsigned int actionid) {
  if (actionid > actionc) {
    return false;
  }
...
static void kperf_system_memory_log(void) {
  ...
  BUF_DATA(PERF_MI_SYS_DATA_3,
           ...
           (uintptr_t)vm_page_purgeable_count, memorystatus_level);
}

static void kperf_sample_user_internal(struct kperf_usample *sbuf,
...
static unsigned int kperf_prepare_sample_what(unsigned int sample_what,
...
void kperf_sample_user(struct kperf_usample *sbuf,
  ...
  unsigned int sample_what =
      kperf_prepare_sample_what(actionv[actionid - 1].sample, sample_flags);
  if (sample_what == 0) {
    return;
  }
...
kperf_sample_internal(struct kperf_sample *sbuf, struct kperf_context *context,
  ...
  if (actionid && actionid <= actionc) {
    sbuf->kcallstack.kpkc_nframes = actionv[actionid - 1].kcallstack_depth;
  } else {
  ...
  if (!is_kernel) {
    ...
    if (sample_flags & SAMPLE_FLAG_PEND_USER) {
      if (sample_what & SAMPLER_USTACK) {
        pended_ucallstack =
            kperf_ucallstack_pend(context, ucallstack_depth, actionid);
      }

      if (sample_what & SAMPLER_TH_DISPATCH) {
        pended_th_dispatch = kperf_thread_dispatch_pend(context, actionid);
      }
    }
  }
...
log_sample:
  ...
  BUF_DATA(PERF_GEN_EVENT | DBG_FUNC_START, sample_what, actionid, userdata,
           sample_flags);
...
kern_return_t kperf_sample(struct kperf_sample *sbuf,
                           struct kperf_context *context, unsigned actionid,
  ...
  return kperf_sample_internal(sbuf, context, sample_what, sample_flags,
                               actionid, ucallstack_depth);
}
 ...
 */
static void kperf_ast_sample_min_stack_phase(struct kperf_usample_min *sbuf_min,
                                             struct kperf_context *context,
                                             unsigned int sample_what) {
  if (sample_what & SAMPLER_TH_DISPATCH) {
 ...
 */
__attribute__((noinline)) static void kperf_ast_sample_max_stack_phase(
    struct kperf_usample_min *sbuf_min, struct kperf_context *context,
    uint32_t actionid, unsigned int sample_what, unsigned int nframes) {
 ...
 */
__attribute__((noinline)) void kperf_thread_ast_handler(thread_t thread) {
...
__attribute__((noinline)) void kperf_thread_exclaves_ast_handler(
...
__attribute__((noinline)) void kperf_thread_exclaves_ast_handler(
  ...
  if (entry->ipcstack.has_value) {
    stackshottypes_ipcstackentry__v_visit(
        &entry->ipcstack.value,
        ^(size_t __unused i,
          const stackshottypes_ipcstackentry_s *_Nonnull __unused ipcstack) {
          ipcstack_count += 1;
        });

    BUF_DATA(PERF_CS_EXSTACKHDR, ipcstack_count, thread->thread_id,
             entry->scid);

    stackshottypes_ipcstackentry__v_visit(
        &entry->ipcstack.value,
        ^(size_t __unused j,
          const stackshottypes_ipcstackentry_s *_Nonnull ipcstack) {
...
int kperf_ast_pend(thread_t thread, uint32_t set_flags,
...
int kperf_kdbg_cswitch_get(void) { return kperf_kdebug_cswitch; }
 ...
 */
unsigned int kperf_action_get_count(void) { return actionc; }

int kperf_action_set_samplers(unsigned actionid, uint32_t samplers) {
  if ((actionid > actionc) || (actionid == 0)) {
    return EINVAL;
  }
...
int kperf_action_get_samplers(unsigned actionid, uint32_t *samplers_out) {
  if ((actionid > actionc)) {
    return EINVAL;
  }
...
int kperf_action_set_userdata(unsigned actionid, uint32_t userdata) {
  if ((actionid > actionc) || (actionid == 0)) {
    return EINVAL;
  }
...
int kperf_action_get_userdata(unsigned actionid, uint32_t *userdata_out) {
  if ((actionid > actionc)) {
    return EINVAL;
  }
...
int kperf_action_set_filter(unsigned actionid, int pid) {
  if ((actionid > actionc) || (actionid == 0)) {
    return EINVAL;
  }
...
int kperf_action_get_filter(unsigned actionid, int *pid_out) {
  if ((actionid > actionc)) {
    return EINVAL;
  }
...
int kperf_action_set_count(unsigned count) {
  ...
  /* create a new array */
  new_actionv = kalloc_data_tag(count * sizeof(*new_actionv), Z_WAITOK,
                                VM_KERN_MEMORY_DIAG);
  if (new_actionv == NULL) {
    return ENOMEM;
  }
...
int kperf_action_set_ucallstack_depth(unsigned action_id, uint32_t depth) {
...
int kperf_action_set_kcallstack_depth(unsigned action_id, uint32_t depth) {
  if ((action_id > actionc) || (action_id == 0)) {
    return EINVAL;
  }
...
int kperf_action_get_ucallstack_depth(unsigned action_id, uint32_t *depth_out) {
...
int kperf_action_get_kcallstack_depth(unsigned action_id, uint32_t *depth_out) {
  if ((action_id > actionc)) {
    return EINVAL;
  }
