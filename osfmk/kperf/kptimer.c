...
#if defined(__x86_64__)
    [KTPL_FG] =
        20 * NSEC_PER_USEC, /* The minimum timer period in xnu, period. */
...
static void kptimer_set_cpu_deadline(int cpuid, int timerid,
                                     uint64_t deadline) {
  kptimer.g_cpu_deadlines[(cpuid * KPTIMER_MAX) + timerid] = deadline;
}

static void kptimer_setup(void) {
  if (kptimer.g_setup) {
    return;
  }
  ...
  kptimer.g_timers = zalloc_permanent_tag(timers_size, ZALIGN(struct kptimer),
                                          VM_KERN_MEMORY_DIAG);
  ...
  kptimer.g_cpu_deadlines =
      zalloc_permanent_tag(deadlines_size, ZALIGN_64, VM_KERN_MEMORY_DIAG);
  for (int i = 0; i < KPTIMER_MAX; i++) {
...
static uint64_t kptimer_get_cpu_deadline(int cpuid, int timerid) {
  return kptimer.g_cpu_deadlines[(cpuid * KPTIMER_MAX) + timerid];
}

static void kptimer_sample_curcpu(unsigned int actionid, unsigned int timerid,
                                  uint32_t flags) {
  ...
  (void)kperf_sample(intbuf, &ctx, actionid, SAMPLE_FLAG_PEND_USER | flags);

  BUF_INFO(PERF_TM_HNDLR | DBG_FUNC_END);
}

static void kptimer_lock(struct kptimer *timer) {
  lck_spin_lock(&timer->kt_lock);
}

static void kptimer_unlock(struct kptimer *timer) {
 ...
 */
static inline uint64_t dead_reckon_deadline(uint64_t now, uint64_t deadline,
                                            uint64_t period) {
  if (deadline < now) {
...
static uint64_t kptimer_fire(struct kptimer *timer, unsigned int timerid,
                             uint64_t deadline, int __unused cpuid,
                             uint64_t now) {
  bool first = false;
  ...
  if (timer->kt_cur_deadline < next_deadline) {
    first = true;
    next_deadline =
        dead_reckon_deadline(now, next_deadline, timer->kt_period_abs);
...
void kptimer_expire(processor_t processor, int cpuid, uint64_t now) {
  ...
  if (min_deadline < UINT64_MAX) {
    running_timer_enter(processor, RUNNING_TIMER_KPERF, NULL, min_deadline,
                        mach_absolute_time());
  }
}
...
static void kptimer_broadcast(void (*fn)(void *)) {
...
static void kptimer_broadcast_ack(void *arg) {
#if defined(__x86_64__)
#pragma unused(arg)
#else  /* defined(__x86_64__) */
...
static void kptimer_sample_pet_remote(void *__unused arg) {
...
void kperf_signal_handler(void) { kptimer_sample_pet_remote(NULL); }
...
static void kptimer_broadcast_pet(void) {
  atomic_fetch_add(&mycounter, 1);
#if defined(__x86_64__)
  (void)mp_cpus_call(CPUMASK_OTHERS, NOSYNC, kptimer_sample_pet_remote, NULL);
#else  /* defined(__x86_64__) */
...
static uint64_t kptimer_earliest_deadline(processor_t processor, uint64_t now) {
  uint64_t min_deadline = UINT64_MAX;
  for (unsigned int i = 0; i < kptimer.g_ntimers; i++) {
    struct kptimer *timer = &kptimer.g_timers[i];
    ...
    cur_deadline =
        dead_reckon_deadline(now, cur_deadline, timer->kt_period_abs);
...
void kptimer_running_setup(processor_t processor, uint64_t now) {
  ...
  if (deadline < UINT64_MAX) {
    running_timer_setup(processor, RUNNING_TIMER_KPERF, NULL, deadline, now);
  }
}

static void kptimer_start_cpu(processor_t processor) {
  uint64_t now = mach_absolute_time();
  uint64_t deadline = kptimer_earliest_deadline(processor, now);
  if (deadline < UINT64_MAX) {
    running_timer_enter(processor, RUNNING_TIMER_KPERF, NULL, deadline, now);
  }
}

static void kptimer_start_remote(void *arg) {
...
void kptimer_stop_curcpu(void) { kptimer_stop_cpu(current_processor()); }

static void kptimer_stop_remote(void *__unused arg) {
...
/*
 * Called when a CPU is brought online.  Handles the cases where the kperf timer
 * may have been either enabled or disabled while the CPU was offline
 * (preventing the enabling/disabling IPIs from reaching this CPU).
 */
void kptimer_curcpu_up(void) {
  ...
  /*
   * If the CPU was taken offline, THEN kperf was enabled, this CPU would have
   * missed the enabling IPI, so fix that here.  Also, if the CPU was taken
   * offline (after having enabled kperf), recompute the deadline (since we may
   * have missed a timer update) and keep the timer enabled.
   */
  if (status == KPERF_SAMPLING_ON) {
    kptimer_start_cpu(processor);
  } else {
    /*
     * Similarly, If the CPU is resuming after having previously armed the kperf
     * timer before going down, and kperf is currently disabled, disable the
     * kperf running timer on this CPU.
     */
    kptimer_stop_cpu(processor);
  }
}
...
int kptimer_get_period(unsigned int timerid, uint64_t *period_abs) {
...
int kptimer_get_action(unsigned int timerid, unsigned int *actionid) {
  if (timerid >= kptimer.g_ntimers) {
    return EINVAL;
  }
...
int kptimer_set_action(unsigned int timerid, unsigned int actionid) {
  if (timerid >= kptimer.g_ntimers) {
    return EINVAL;
  }
  if (kptimer.g_started) {
...
unsigned int kptimer_get_count(void) { return kptimer.g_ntimers; }
...
uint32_t kptimer_get_pet_timerid(void) { return kptimer.g_pet_timerid; }

int kptimer_set_pet_timerid(uint32_t petid) {
  if (kptimer.g_started) {
    return EBUSY;
  }
