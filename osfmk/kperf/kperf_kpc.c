...
void kperf_kpc_thread_sample(struct kpcdata *kpcd, int sample_config) {
  ...
  if (kpc_get_curthread_counters(&kpcd->counterc, kpcd->counterv)) {
    ...
    memset(kpcd->counterv, 0, sizeof(uint64_t) * kpcd->counterc);
  }
  ...
  BUF_INFO(PERF_KPC_THREAD_SAMPLE | DBG_FUNC_END, kpcd->running,
           kpcd->counterc);
}

void kperf_kpc_cpu_sample(struct kpcdata *kpcd, int sample_config) {
  BUF_INFO(PERF_KPC_CPU_SAMPLE | DBG_FUNC_START, sample_config);

  kpcd->running = kpc_get_running();
  kpcd->counterc =
      kpc_get_cpu_counters(0, kpcd->running, &kpcd->curcpu, kpcd->counterv);
  if (!sample_config) {
    kpcd->configc = 0;
  } else {
    kpcd->configc = kpc_get_config_count(kpcd->running);
...
void kperf_kpc_config_log(const struct kpcdata *kpcd) {
  BUF_DATA(PERF_KPC_CONFIG, kpcd->running, kpcd->counterc,
           kpc_get_counter_count(KPC_CLASS_FIXED_MASK), kpcd->configc);
  ...
  for (unsigned int i = 0; i < max; i++) {
    uint32_t flag = (i == 0)
                        ? DBG_FUNC_START
                        : ((i == (max - 1)) ? DBG_FUNC_END : DBG_FUNC_NONE);
    BUF_DATA(PERF_KPC_CFG_REG | flag, kpcd->configv[0 + i * 4],
             kpcd->configv[1 + i * 4], kpcd->configv[2 + i * 4],
             kpcd->configv[3 + i * 4]);
  }
#else  /* __LP64__ */
  ...
  for (unsigned int i = 0; i < max; i++) {
    uint32_t flag = (i == 0)
                        ? DBG_FUNC_START
                        : ((i == (max - 1)) ? DBG_FUNC_END : DBG_FUNC_NONE);
    BUF_DATA(PERF_KPC_CFG_REG32 | flag, kpcd->configv[i * 2] >> 32ULL,
...
static void kperf_kpc_log(uint32_t code, uint32_t code32,
                          const struct kpcdata *kpcd) {
#if __LP64__
#pragma unused(code32)
  ...
  for (unsigned int i = 0; i < max; i++) {
    uint32_t flag = (i == 0)
                        ? DBG_FUNC_START
                        : ((i == (max - 1)) ? DBG_FUNC_END : DBG_FUNC_NONE);
    BUF_DATA(code | flag, kpcd->counterv[0 + i * 4], kpcd->counterv[1 + i * 4],
             kpcd->counterv[2 + i * 4], kpcd->counterv[3 + i * 4]);
  }
#else /* __LP64__ */
#pragma unused(code)
  ...
  for (unsigned int i = 0; i < max; i++) {
    uint32_t flag = (i == 0)
                        ? DBG_FUNC_START
                        : ((i == (max - 1)) ? DBG_FUNC_END : DBG_FUNC_NONE);
    BUF_DATA(code32 | flag, (kpcd->counterv[0 + i * 2] >> 32ULL),
...
void kperf_kpc_cpu_log(const struct kpcdata *kpcd) {
  kperf_kpc_log(PERF_KPC_DATA, PERF_KPC_DATA32, kpcd);
}

void kperf_kpc_thread_log(const struct kpcdata *kpcd) {
