 ...
 * generation count is older than a sampling timer would have incremented a
 * global generation count, the thread samples itself.
 *
 *            |  |
 * thread A   +--+---------|
 *            |  |
...
static void kppet_lock_assert_owned(void) {
  lck_mtx_assert(&kppet_mtx, LCK_MTX_ASSERT_OWNED);
}

static void kppet_lock(void) { lck_mtx_lock(&kppet_mtx); }

static void kppet_unlock(void) { lck_mtx_unlock(&kppet_mtx); }

void kppet_set_period(uint64_t period) { kppet.g_period = period; }

static uint32_t kppet_current_gen(void) {
  /*
...
void kppet_on_cpu(thread_t thread, thread_continue_t continuation,
  ...
  if (thread->kperf_pet_gen != sample_gen) {
    BUF_VERB(PERF_PET_SAMPLE_THREAD | DBG_FUNC_START, sample_gen,
             thread->kperf_pet_gen, kppet_lightweight_start_time,
             kppet.g_period);
 ...
 * once.
 */
static void kppet_setup(void) {
  if (kppet.g_setup) {
    return;
  }

  kern_return_t kr =
      kernel_thread_start(kppet_thread, NULL, &kppet.g_sample_thread);
  if (kr != KERN_SUCCESS) {
...
void kppet_config(unsigned int actionid) {
  ...
  if (actionid > 0) {
    if (!kppet.g_sample) {
      kppet.g_sample = kalloc_type_tag(struct kperf_sample, Z_WAITOK | Z_NOFAIL,
                                       VM_KERN_MEMORY_DIAG);
    ...
    if (kppet.g_sample != NULL) {
      kfree_type(struct kperf_usample_min, kppet.g_sample->usample.usample_min);
...
void kppet_wake_thread(void) { thread_wakeup(&kppet); }

__attribute__((noreturn)) static void kppet_thread(void *__unused param,
  ...
  for (;;) {
    ...
    do {
      (void)lck_mtx_sleep(&kppet_mtx, LCK_SLEEP_DEFAULT, &kppet, THREAD_UNINT);
...
static void kppet_sample_thread(int pid, task_t task, thread_t thread,
                                uint32_t idle_rate) {
...
static kern_return_t kppet_threads_prepare(task_t task) {
  kppet_lock_assert_owned();
 ...
 */
static void kppet_sample_task(task_t task, uint32_t idle_rate) {
  kppet_lock_assert_owned();
  ...
  if (kperf_action_has_task(kppet.g_actionid)) {
    ...
    kperf_sample(kppet.g_sample, &ctx, kppet.g_actionid, SAMPLE_FLAG_TASK_ONLY);
  }
 ...
 */
static kern_return_t kppet_tasks_prepare(void) {
  kppet_lock_assert_owned();
...
int kppet_get_idle_rate(void) { return kppet.g_idle_rate; }
...
void kppet_lightweight_active_update(void) {
  kppet_lightweight_start_time =
      (kperf_is_sampling() && kppet.g_lightweight) ? mach_continuous_time() : 0;
  kperf_on_cpu_update();
}

int kppet_get_lightweight_pet(void) { return kppet.g_lightweight; }
