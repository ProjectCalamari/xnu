...
static void callstack_fixup_user(struct kp_ucallstack *cs, thread_t thread) {
...
static void callstack_fixup_interrupted(struct kp_kcallstack *cs) {
...
void kperf_continuation_sample(struct kp_kcallstack *cs,
...
void kperf_backtrace_sample(struct kp_kcallstack *cs,
                            struct kperf_context *context) {
  assert(cs != NULL);
  assert(context != NULL);
  ...
  cs->kpkc_nframes =
      backtrace(cs->kpkc_word_frames, cs->kpkc_nframes - 1, &ctl, &btinfo);
  if (cs->kpkc_nframes > 0) {
    ...
    if ((context->cur_thread->th_exclaves_state & TH_EXCLAVES_RPC) != 0) {
      cs->kpkc_exclaves_offset =
          exclaves_stack_offset(cs->kpkc_word_frames, cs->kpkc_nframes, true);
    }
...
void kperf_kcallstack_sample(struct kp_kcallstack *cs,
                             struct kperf_context *context) {
  thread_t thread;
  ...
  if (ml_at_interrupt_context()) {
     ...
     */
    kern_return_t kr;
    kr = chudxnu_thread_get_callstack64_kperf(thread, cs->kpkc_frames,
                                              &cs->kpkc_nframes, FALSE);
    if (kr == KERN_SUCCESS) {
      cs->kpkc_flags |= CALLSTACK_VALID;
...
void kperf_ucallstack_sample(struct kp_ucallstack *cs,
                             struct kperf_context *context) {
  ...
  unsigned int nframes =
      backtrace_user(cs->kpuc_frames, maxnframes, &ctl, &btinfo);
  ...
  if (btinfo.btui_error == 0 || btinfo.btui_error == EFAULT) {
    ...
    if (cs->kpuc_nframes < maxnframes && btinfo.btui_async_frame_addr != 0) {
...
static inline uintptr_t scrub_word(uintptr_t *bt, int n_frames, int frame,
                                   bool kern) {
  if (frame < n_frames) {
    if (kern) {
...
static inline uintptr_t scrub_frame(uint64_t *bt, int n_frames, int frame) {
  if (frame < n_frames) {
...
static void callstack_log(uint32_t hdrid, uint32_t dataid, void *vframes,
  ...
  if (flags & CALLSTACK_KERNEL_WORDS) {
    ...
    for (unsigned int i = 0; i < nevts; i++) {
      unsigned int j = i * 4;
      BUF_DATA(dataid, scrub_word(frames, nframes, j + 0, kern),
    ...
    for (unsigned int i = 0; i < nevts; i++) {
      ...
      unsigned int j = i * 4;
      BUF_DATA(dataid, scrub_frame(frames, nframes, j + 0),
...
void kperf_kcallstack_log(struct kp_kcallstack *cs) {
  callstack_log(PERF_CS_KHDR, PERF_CS_KDATA, cs->kpkc_frames, cs->kpkc_nframes,
                cs->kpkc_flags, 0, 0);
...
void kperf_excallstack_log(const stackshottypes_ipcstackentry_s *ipcstack) {
  ...
  if (ipcstack->stacktrace.has_value) {
    address__v_visit(&ipcstack->stacktrace.value,
...
bool kperf_exclave_callstack_pend(struct kperf_context *context,
                                  unsigned int actionid) {
  if ((context->cur_thread->th_exclaves_state & TH_EXCLAVES_RPC) &&
      (os_atomic_load(&context->cur_thread->th_exclaves_inspection_state,
                      relaxed) &
       TH_EXCLAVES_INSPECTION_NOINSPECT) == 0) {
    os_atomic_or(&context->cur_thread->th_exclaves_inspection_state,
                 TH_EXCLAVES_INSPECTION_KPERF, relaxed);
...
int kperf_ucallstack_pend(struct kperf_context *context, uint32_t depth,
                          unsigned int actionid) {
  if (depth < 2) {
    panic("HUH");
  }
  kperf_ast_set_callstack_depth(context->cur_thread, depth);
  return kperf_ast_pend(context->cur_thread, T_KPERF_AST_CALLSTACK, actionid);
}

static kern_return_t chudxnu_kern_read(void *dstaddr, vm_offset_t srcaddr,
                                       vm_size_t size) {
...
static kern_return_t chudxnu_task_read(task_t task, void *kernaddr,
                                       uint64_t usraddr, vm_size_t size) {
...
static inline uint64_t chudxnu_vm_unslide(uint64_t ptr, int kaddr) {
  if (!kaddr) {
    return ptr;
  }
...
// chudxnu_thread_get_callstack gathers a raw callstack along with any
// information needed to fix it up later (in case we stopped program as it was
// saving values into prev stack frame, etc.) after sampling has finished.
//
...
chudxnu_thread_get_callstack64_internal(thread_t thread, uint64_t *callStack,
  ...
  if (is_saved_state64(sstate)) {
    ...
    buffer[bufferIndex++] =
        chudxnu_vm_unslide(currPC, kernel); // save PC in position 0.
    ...
    while (bufferIndex < bufferMaxIndex) {
      ...
      if (kernel) {
        if (((uint64_t)fp > kernStackMax) || ((uint64_t)fp < kernStackMin)) {
          kr = KERN_FAILURE;
        } else {
          kr = chudxnu_kern_read(&frame, (vm_offset_t)fp,
                                 (vm_size_t)sizeof(frame));
          if (kr == KERN_SUCCESS) {
#if defined(HAS_APPLE_PAC)
            /* return addresses on stack will be signed by arm64e ABI */
            pc = (uint64_t)ptrauth_strip((void *)frame[1],
            ...
            /* frame pointers on stack will be signed by arm64e ABI */
            nextFramePointer =
                ptrauth_strip(nextFramePointer, ptrauth_key_frame_pointer);
#endif
          } else {
            pc = 0ULL;
        ...
        kr = chudxnu_task_read(task, &frame, (vm_offset_t)fp,
                               (vm_size_t)sizeof(frame));
        if (kr == KERN_SUCCESS) {
#if defined(HAS_APPLE_PAC)
          /* return addresses on stack will be signed by arm64e ABI */
    ...
    while (bufferIndex < bufferMaxIndex) {
      ...
      if (kernel) {
        if (((uint32_t)fp > kernStackMax) || ((uint32_t)fp < kernStackMin)) {
          kr = KERN_FAILURE;
        } else {
          kr = chudxnu_kern_read(&frame, (vm_offset_t)fp,
                                 (vm_size_t)sizeof(frame));
        ...
        kr = chudxnu_task_read(
            task, &frame32, (((uint64_t)(uint32_t)fp) & 0x00000000FFFFFFFFULL),
            sizeof(frame32));
        if (kr == KERN_SUCCESS) {
    ...
    // Save link register and R13 (sp) at bottom of stack (used for later
    // fixup).
    buffer[bufferIndex++] = chudxnu_vm_unslide(currLR, kernel);
    if (flags & CS_FLAG_EXTRASP) {
      buffer[bufferIndex++] = chudxnu_vm_unslide(currSP, kernel);
...
chudxnu_thread_get_callstack64_kperf(thread_t thread, uint64_t *callStack,
                                     mach_msg_type_number_t *count,
                                     boolean_t user_only) {
  return chudxnu_thread_get_callstack64_internal(thread, callStack, count,
                                                 user_only, 0);
}
#elif __x86_64__

#define VALID_STACK_ADDRESS(supervisor, addr, minKernAddr, maxKernAddr)        \
  (supervisor ? (addr >= minKernAddr && addr <= maxKernAddr) : TRUE)
// don't try to read in the hole
#define VALID_STACK_ADDRESS64(supervisor, addr, minKernAddr, maxKernAddr)      \
  (supervisor                                                                  \
       ? ((uint64_t)addr >= minKernAddr && (uint64_t)addr <= maxKernAddr)      \
       : ((uint64_t)addr != 0ULL &&                                            \
          ((uint64_t)addr <= 0x00007FFFFFFFFFFFULL ||                          \
           (uint64_t)addr >= 0xFFFF800000000000ULL)))
...
static kern_return_t do_kernel_backtrace(thread_t thread,
  ...
  if (KERN_SUCCESS != chudxnu_kern_read(&currPC, (vm_offset_t) & (regs->k_rip),
                                        sizeof(uint64_t))) {
    return KERN_FAILURE;
  }
  if (KERN_SUCCESS != chudxnu_kern_read(&currFP, (vm_offset_t) & (regs->k_rbp),
                                        sizeof(uint64_t))) {
    return KERN_FAILURE;
  }
#else
  ...
  uint32_t prevPC = 0U;
  uint32_t prevFP = 0U;
  if (KERN_SUCCESS != chudxnu_kern_read(&currPC, (vm_offset_t) & (regs->k_eip),
                                        sizeof(uint32_t))) {
    return KERN_FAILURE;
  }
  if (KERN_SUCCESS != chudxnu_kern_read(&currFP, (vm_offset_t) & (regs->k_ebp),
                                        sizeof(uint32_t))) {
    return KERN_FAILURE;
  }
#endif
...
static kern_return_t do_backtrace32(task_t task, thread_t thread,
                                    x86_saved_state32_t *regs, uint64_t *frames,
                                    mach_msg_type_number_t *start_idx,
                                    mach_msg_type_number_t max_idx,
  ...
  while (VALID_STACK_ADDRESS(supervisor, currFP, kernStackMin, kernStackMax)) {
    ...
    /* read our caller */
    if (supervisor) {
      kr = chudxnu_kern_read(&tmpWord, (vm_offset_t)&fp->caller,
                             sizeof(uint32_t));
    } else {
      kr = chudxnu_task_read(task, &tmpWord, (vm_offset_t)&fp->caller,
                             sizeof(uint32_t));
    }
    ...
    if (supervisor) {
      kr =
          chudxnu_kern_read(&tmpWord, (vm_offset_t)&fp->prev, sizeof(uint32_t));
    } else {
      kr = chudxnu_task_read(task, &tmpWord, (vm_offset_t)&fp->prev,
                             sizeof(uint32_t));
    }
...
static kern_return_t do_backtrace64(task_t task, thread_t thread,
                                    x86_saved_state64_t *regs, uint64_t *frames,
                                    mach_msg_type_number_t *start_idx,
                                    mach_msg_type_number_t max_idx,
                                    boolean_t supervisor) {
  ...
  // build a backtrace of this 32 bit state.
  while (
      VALID_STACK_ADDRESS64(supervisor, currFP, kernStackMin, kernStackMax)) {
    // this is the address where caller lives in the user thread
    uint64_t caller = currFP + sizeof(uint64_t);
...
static kern_return_t chudxnu_thread_get_callstack64_internal(
    thread_t thread, uint64_t *callstack, mach_msg_type_number_t *count,
    boolean_t user_only, boolean_t kern_only) {
  kern_return_t kr = KERN_FAILURE;
  ...
  if (!ml_at_interrupt_context() && kernel_task == task) {
    ...
    if (KERN_SUCCESS !=
        chudxnu_kern_read(&kregs, (vm_offset_t) & (thread->kernel_stack),
                          sizeof(void *))) {
      return KERN_FAILURE;
    }
  ...
  *count = 0;
  ...
  if (kregs) {
    ...
    if (KERN_SUCCESS != chudxnu_kern_read(&currPC,
                                          (vm_offset_t) & (kregs->k_rip),
                                          sizeof(uint64_t))) {
      return KERN_FAILURE;
    }
  } else if (regs64) {
  ...
  if (kregs) {
    ...
    // do the backtrace
    kr = do_kernel_backtrace(thread, kregs, callstack, &bufferIndex,
                             bufferMaxIndex);
    ...
    if (KERN_SUCCESS !=
        chudxnu_kern_read(&address, (vm_offset_t) & (kregs->k_rsp), size)) {
      address = 0ULL;
    }

    if (address &&
        KERN_SUCCESS == chudxnu_kern_read(&rsp, (vm_offset_t)address, size) &&
        bufferIndex < bufferMaxIndex) {
    ...
    if (KERN_SUCCESS == chudxnu_kern_read(&rsp, (vm_offset_t)regs64->isf.rsp,
                                          sizeof(uint64_t)) &&
        bufferIndex < bufferMaxIndex) {
    ...
    if (KERN_SUCCESS == chudxnu_kern_read(&esp, (vm_offset_t)regs32->uesp,
                                          sizeof(uint32_t)) &&
        bufferIndex < bufferMaxIndex) {
    ...
    if (KERN_SUCCESS == chudxnu_task_read(task, &rsp,
                                          (addr64_t)u_regs64->isf.rsp,
                                          sizeof(uint64_t)) &&
        bufferIndex < bufferMaxIndex) {
      callstack[bufferIndex++] = rsp;
    }
    ...
    if (KERN_SUCCESS == chudxnu_task_read(task, &esp, (addr64_t)u_regs32->uesp,
                                          sizeof(uint32_t)) &&
        bufferIndex < bufferMaxIndex) {
      callstack[bufferIndex++] = (uint64_t)esp;
    }
