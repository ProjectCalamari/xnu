...
// These are HKDF-Expand parameters for derived seeds. To add a new one, add a
// new struct here.
static derived_seed_param seed_params[] = {{.info = "bootseed_init",
                                            .info_size = 14,
                                            .dst = earlyseed,
...
// Hash the seed to ensure uniformity. But we have a limited-size digest
// available, so we make two invocations: out[0:SHA256_DIGEST_LENGTH]          =
// H(seed || 0) out[SHA256_DIGEST_LENGTH:SEED_SIZE]  = H(seed || 1)
static void wide_hash(const struct ccdigest_info *di, uint8_t *dst,
                      uint8_t *src) {
  uint8_t counter;
...
static void bootseed_init_bootloader(const struct ccdigest_info *di,
                                     uint8_t *dst) {
...
static void bootseed_init_native(const struct ccdigest_info *di, uint8_t *dst) {
  uint8_t seed[SEED_SIZE];
  uint64_t x;
  uint8_t ok;
  size_t i = 0;
  ...
  if (cpuid_leaf7_features() & CPUID_LEAF7_FEATURE_RDSEED) {
    ...
    while (i < n) {
      ...
      if (ok) {
        ...
        // Intel does not recommend pausing between unsuccessful rdrand
        // attempts.
      }
    }
  }
...
static void bootseed_init_native(__unused const struct ccdigest_info *di,
                                 uint8_t *dst) {
  // Even if we don't have any input, the second input needs to be a fixed input
  // of the same size to maintain dual-PRF security for HKDF/HMAC. All zero is
  // fine as long as it is fixed.
  cc_clear(SEED_SIZE, dst);
}

#endif

static void bootseed_init(void) {
  /*
   *  This is a key combiner. HKDF provides dual-PRF security as long as we
   * sample inputs from a set of fixed-length, uniformly random inputs. Ideally
   * those inputs will also be the block size of the underlying digest, which we
   * specify here with SEED_SIZE.
   *
   *  See https://eprint.iacr.org/2023/861 for proof details. The overall
   * construction goes:
   *
   *       H* : {0, 1}* -> {0, 1}^c where c is the block size of the digest
   * underlying HKDF, here 64. n are long enough to require a compression in the
   * underlying hash function. prk = HKDF-Extract(H*(bootloader), H*(native))
  ...
  // Sample the two input seeds from the devicetree and any available RDRAND
  // instructions
  bootseed_init_bootloader(di, bootloader_rand);
  bootseed_init_native(di, native_rand);

  // Combine the input seeds into one root seed of size di->output_size.
  // Eventually we want to use a larger digest here: rdar://119642787 (Move boot
  // seed derivations to a digest that preserves the full width of the
  // devicetree seed)
  int result = cchkdf_extract(di, SEED_SIZE, native_rand, SEED_SIZE,
                              bootloader_rand, prk);
  if (result != CCERR_OK) {
  ...
  for (int i = 0; i < seeds_expected; i++) {
    ...
    result = cchkdf_expand(di, di->output_size, prk, sp.info_size, sp.info,
                           sp.dst_size, sp.dst);
    if (result != CCERR_OK) {
      panic("Early boot random cchkdf_expand %s failed with err %d",
            (const char *)sp.info, result);
    }
  }
...
__attribute__((noinline)) static void early_random_init(void) {
  uint64_t nonce;
  int rc;
  ...
  rc = ccdrbg_init(&erandom.drbg_info,
                   (struct ccdrbg_state *)erandom.drbg_state, sizeof(earlyseed),
                   earlyseed, sizeof(nonce), &nonce, sizeof(ps) - 1, ps);
...
static void read_random_generate(uint8_t *buffer, size_t numbytes);

// This code is used only during early boot (until corecrypto kext is
// loaded), so it's better not to inline it.
__attribute__((noinline)) static void read_erandom_generate(void *buf,
                                                            size_t nbytes) {
  ...
  while (nbytes > 0) {
    ...
    rc = ccdrbg_generate(&erandom.drbg_info,
                         (struct ccdrbg_state *)erandom.drbg_state, n,
                         buffer_bytes, 0, NULL);
    if (rc != CCDRBG_STATUS_OK) {
...
__static_testable __mockable void read_erandom(void *buf, size_t nbytes) {
...
void register_and_init_prng(struct cckprng_ctx *ctx,
                            const struct cckprng_funcs *funcs) {
  ...
  prng_funcs.init_with_getentropy(prng_ctx, MAX_CPUS, sizeof(prngseed),
                                  prngseed, sizeof(nonce), &nonce,
                                  entropy_provide, NULL);
...
__mockable void read_random(void *buffer, u_int numbytes) {
  prng_funcs.refresh(prng_ctx);
  read_random_generate(buffer, numbytes);
}

static void ensure_gsbase(void) {
...
void random_bool_gen_entropy(struct bool_gen *bg, unsigned int *buffer,
                             int count) {
