...
__startup_func static void entropy_analysis_init(uint32_t sample_count) {
  ...
  entropy_analysis_buffer =
      zalloc_permanent(entropy_analysis_buffer_size, ZALIGN(entropy_sample_t));
  entropy_analysis_filter_size =
      (uint32_t)BITMAP_SIZE(entropy_analysis_max_sample_count);
}

static void entropy_analysis_store(entropy_sample_t sample) {
  uint32_t sample_count;
  uint32_t next_sample_count;

  os_atomic_rmw_loop(&entropy_analysis_sample_count, sample_count,
                     next_sample_count, relaxed, {
...
__startup_func void entropy_init(size_t seed_size, uint8_t *seed) {
...
void entropy_collect(void) {
  ...
  (void)os_atomic_cmpxchg(&e->sample_count, sample_count, sample_count + 1,
                          release);
...
// Timestamps with a 2nd differential above the threshold
// ENTROPY_FILTER_THRESHOLD will be marked in the filter bitmap. 2nd
// differentials below the threshold will not be counted nor included in the
// filter bitmap.
//
...
uint32_t entropy_filter(uint32_t sample_count, entropy_sample_t *samples,
                        __assert_only uint32_t filter_count, bitmap_t *filter) {
  assert3u(filter_count, >=, BITMAP_LEN(sample_count));

  bitmap_zero(filter, sample_count);

  // We always keep the first one (or two) sample(s) if we have at least one (or
  // more) samples
  if (sample_count == 0) {
    return 0;
  ...
  for (uint i = 2; i < sample_count; i++) {
    ...
    entropy_sample_t curr_2nd_differential =
        curr_1st_differential - prev_1st_differential;

    if (curr_2nd_differential > ENTROPY_FILTER_THRESHOLD &&
        curr_2nd_differential < ((entropy_sample_t)-ENTROPY_FILTER_THRESHOLD)) {
      bitmap_set(filter, i);
...
static health_test_result_t add_observation(entropy_health_test_t *t,
                                            uint64_t bound) {
  t->observation_count += 1;
  t->stats->max_observation_count =
      MAX(t->stats->max_observation_count, (uint32_t)t->observation_count);
...
static void reset_test(entropy_health_test_t *t, entropy_sample_t observation) {
...
adaptive_proportion_test(entropy_sample_t observation, uint32_t offset) {
  ...
  // window size.
  uint32_t index = (entropy_data.total_sample_count + offset) &
                   ADAPTIVE_PROPORTION_INDEX_MASK;
...
entropy_health_test(uint32_t sample_count, entropy_sample_t *samples,
  ...
  for (uint32_t i = 0; i < sample_count; i += 1) {
    ...
    if (__improbable(repetition_count_test(observation) ==
                     health_test_failure)) {
      result = health_test_failure;
    }

    if (__improbable(adaptive_proportion_test(observation, i) ==
                     health_test_failure)) {
      result = health_test_failure;
    }
  }
...
int32_t entropy_provide(size_t *entropy_size, void *entropy,
                        __unused void *arg) {
  ...
  *entropy_size = 0;
  ...
  percpu_foreach(e, entropy_cpu_data) {
    ...
    uint32_t filtered_sample_count =
        entropy_filter(cpu_sample_count, e->samples, ENTROPY_MAX_FILTER_COUNT,
                       entropy_data.filter);
    ...
    entropy_filter_rejected_sample_count +=
        (cpu_sample_count - filtered_sample_count);

    // The health test depends in part on the current state of
    // the entropy data, so we test the new sample before
    // accumulating it.
    health_test_result_t cpu_health_test_result =
        entropy_health_test(cpu_sample_count, e->samples,
                            ENTROPY_MAX_FILTER_COUNT, entropy_data.filter);
    ...
    SHA512_Update(&entropy_data.sha512_ctx, e->samples,
                  cpu_sample_count * sizeof(e->samples[0]));
  ...
  if (__improbable((entropy_data.total_sample_count -
                    entropy_data.startup_sample_count) < 1024)) {
    goto out;
  }
  ...
  int32_t n = (int32_t)(entropy_data.total_sample_count -
                        entropy_data.read_sample_count);
  ...
  *entropy_size = SHA512_DIGEST_LENGTH;
  ...
  // this context.
  const char label[SHA512_BLOCK_LENGTH - SHA512_DIGEST_LENGTH] =
