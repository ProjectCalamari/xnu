 ...
 */
IPC_KOBJECT_DEFINE(IKOT_IOKIT_IDENT, .iko_op_movable_send = true,
                   .iko_op_no_senders = iokit_ident_no_senders);
IPC_KOBJECT_DEFINE(IKOT_IOKIT_OBJECT, .iko_op_movable_send = true,
                   .iko_op_no_senders = iokit_object_no_senders);
IPC_KOBJECT_DEFINE(IKOT_IOKIT_CONNECT,
                   .iko_op_no_senders = iokit_connect_no_senders);
IPC_KOBJECT_DEFINE(IKOT_UEXT_OBJECT, .iko_op_movable_send = true,
...
MIGEXTERN io_object_t iokit_lookup_object_port(ipc_port_t port) {
  return iokit_lookup_io_object(port, IKOT_IOKIT_OBJECT);
}

MIGEXTERN io_object_t iokit_lookup_connect_port(ipc_port_t port) {
  return iokit_lookup_io_object(port, IKOT_IOKIT_CONNECT);
}

MIGEXTERN io_object_t iokit_lookup_ident_port(ipc_port_t port) {
  return iokit_lookup_io_object(port, IKOT_IOKIT_IDENT);
}

MIGEXTERN io_object_t iokit_lookup_uext_object_port(ipc_port_t port) {
  return iokit_lookup_io_object(port, IKOT_UEXT_OBJECT);
}

static io_object_t iokit_lookup_object_in_space_with_port_name(
    mach_port_name_t name, ipc_kobject_type_t type, ipc_space_t space) {
  io_object_t obj = NULL;
  io_kobject_t kobj;
...
EXTERN io_object_t iokit_lookup_object_with_port_name(mach_port_name_t name,
                                                      ipc_kobject_type_t type,
                                                      task_t task) {
  return iokit_lookup_object_in_space_with_port_name(name, type,
                                                     task->itk_space);
}

EXTERN io_object_t
iokit_lookup_connect_ref_current_task(mach_port_name_t name) {
  return iokit_lookup_object_in_space_with_port_name(name, IKOT_IOKIT_CONNECT,
                                                     current_space());
}

EXTERN io_object_t iokit_lookup_uext_ref_current_task(mach_port_name_t name) {
  return iokit_lookup_object_in_space_with_port_name(name, IKOT_UEXT_OBJECT,
                                                     current_space());
}
...
EXTERN kern_return_t iokit_lookup_raw_current_task(mach_port_name_t name,
   ...
   */
  if (MACH_PORT_VALID(name)) {
    kr = ipc_typed_port_copyin_send(current_space(), name, IOT_ANY, &port);
...
static ipc_port_t iokit_make_port_of_type(io_object_t obj,
                                          ipc_kobject_type_t type) {
...
MIGEXTERN ipc_port_t iokit_make_object_port(io_object_t obj) {
  return iokit_make_port_of_type(obj, IKOT_IOKIT_OBJECT);
}

MIGEXTERN ipc_port_t iokit_make_connect_port(io_object_t obj) {
  return iokit_make_port_of_type(obj, IKOT_IOKIT_CONNECT);
}

MIGEXTERN ipc_port_t iokit_make_ident_port(io_object_t obj) {
...
EXTERN ipc_port_t iokit_alloc_object_port(io_kobject_t obj,
  ...
  if (type == IKOT_UEXT_OBJECT) {
    label = ipc_kobject_label_alloc(IKOT_UEXT_OBJECT, IPC_LABEL_DEXT, IP_NULL);
  }
  return ipc_kobject_alloc_port(obj, label, options);
}

EXTERN void iokit_lock_port(ipc_port_t port) { ip_mq_lock(port); }

EXTERN void iokit_unlock_port(ipc_port_t port) { ip_mq_unlock(port); }

EXTERN void iokit_destroy_object_port(ipc_port_t port,
                                      ipc_kobject_type_t type) {
...
EXTERN mach_port_name_t iokit_make_send_right(task_t task, io_object_t obj,
  ...
  if (IP_VALID(sendPort)) {
    ...
    kr = ipc_object_copyout(task->itk_space, sendPort, MACH_MSG_TYPE_PORT_SEND,
...
kern_return_t iokit_label_dext_task(task_t task) {
 ...
 *		Nothing locked.
 */
void iokit_clear_registered_ports(task_t task) {
  mach_port_t port;
  ipc_kobject_type_t type;
  ...
  for (int i = 0; i < TASK_PORT_REGISTER_MAX; i++) {
    ...
    type = ip_type(port);
    if ((IKOT_IOKIT_OBJECT == type) || (IKOT_IOKIT_CONNECT == type) ||
        (IKOT_IOKIT_IDENT == type) || (IKOT_UEXT_OBJECT == type)) {
      ipc_port_release_send(port);
...
kern_return_t IOMapPages(vm_map_t map, mach_vm_address_t va,
  ...
  prot = (options & kIOMapReadOnly) ? VM_PROT_READ
  ...
  return pmap_map_block(pmap, va, pagenum,
                        (uint32_t)atop_64(round_page_64(length)), prot, 0, 0);
}

kern_return_t IOUnmapPages(vm_map_t map, mach_vm_address_t va,
...
kern_return_t IOProtectCacheMode(vm_map_t __unused map,
                                 mach_vm_address_t __unused va,
                                 mach_vm_size_t __unused length,
