...
void vm_map_store_init_rb(struct vm_map_header *hdr) {
...
bool vm_map_store_lookup_entry_rb(vm_map_t map, vm_map_offset_t address,
...
void vm_map_store_entry_link_rb(struct vm_map_header *mapHdr,
  ...
  if ((tmp_store = RB_INSERT(rb_head, rbh, store)) != NULL) {
    panic("VMSEL: INSERT FAILED: 0x%lx, 0x%lx, 0x%lx, 0x%lx",
          (uintptr_t)entry->vme_start, (uintptr_t)entry->vme_end,
          (uintptr_t)(VME_FOR_STORE(tmp_store))->vme_start,
          (uintptr_t)(VME_FOR_STORE(tmp_store))->vme_end);
  }
}

void vm_map_store_entry_unlink_rb(struct vm_map_header *mapHdr,
                                  vm_map_entry_t entry) {
  struct rb_head *rbh = &(mapHdr->rb_head_store);
...
void vm_map_store_copy_reset_rb(vm_map_copy_t copy, vm_map_entry_t entry,
                                int nentries) {
...
void vm_map_combine_hole(vm_map_t map, vm_map_entry_t hole_entry);
void vm_map_combine_hole(__unused vm_map_t map, vm_map_entry_t hole_entry) {
...
void vm_map_delete_hole(vm_map_t map, vm_map_entry_t hole_entry);
void vm_map_delete_hole(vm_map_t map, vm_map_entry_t hole_entry) {
...
static void check_map_sanity(vm_map_t map, vm_map_entry_t old_hole_entry) {
  ...
  while (map_entry != vm_map_to_entry(map)) {
    ...
    if (map_entry->vme_end != map_entry->vme_next->vme_start) {
      ...
      if (hole_entry->vme_start != map_entry->vme_end) {
        panic("hole_entry not aligned %p(0x%llx), %p (0x%llx), %p", hole_entry,
              (unsigned long long)hole_entry->vme_start, map_entry->vme_next,
              (unsigned long long)map_entry->vme_end, old_hole_entry);
        assert(hole_entry->vme_start == map_entry->vme_end);
      }

      if (hole_entry->vme_end != map_entry->vme_next->vme_start) {
        panic("hole_entry not next aligned %p(0x%llx), %p (0x%llx), %p",
              hole_entry, (unsigned long long)hole_entry->vme_end,
              map_entry->vme_next,
              (unsigned long long)map_entry->vme_next->vme_start,
              old_hole_entry);
 ...
 * For debugging.
 */
static void copy_hole_info(vm_map_entry_t hole_entry,
...
void update_holes_on_entry_deletion(vm_map_t map, vm_map_entry_t old_entry);
void update_holes_on_entry_deletion(vm_map_t map, vm_map_entry_t old_entry) {
  /*
  ...
  if (hole_entry) {
    ...
    if (hole_entry != CAST_TO_VM_MAP_ENTRY(map->holes_list)) {
      if (hole_entry->vme_start > old_entry->vme_start) {
        panic("Hole hint failed: Hole entry start: 0x%llx, entry start: "
              "0x%llx, map hole start: 0x%llx, map hint start: 0x%llx",
      ...
      if (hole_entry->vme_end > old_entry->vme_start) {
        panic("Hole hint failed: Hole entry end: 0x%llx, entry start: 0x%llx, "
              "map hole start: 0x%llx, map hint start: 0x%llx",
...
void update_holes_on_entry_creation(vm_map_t map, vm_map_entry_t new_entry);
void update_holes_on_entry_creation(vm_map_t map, vm_map_entry_t new_entry) {
  vm_map_entry_t hole_entry, next_hole_entry;
#if DEBUG
  struct vm_map_entry old_hole_entry;
   ...
   *
   * Case B: The entry is completely within a hole but NOT aligned with the
   * start/end of the hole. This  will split a hole.
   *
   * Case C: The entry overlaps with the hole. The entry could be extending
   * upwards (C1) or downwards (C2). This will reduce the size of the hole or
   * delete the hole completely if it is smaller than the entry.
   */
  ...
  while (1) {
#if DEBUG
    /*
     * If the entry doesn't exist in the RB tree, we are likely dealing with
     * copy maps where the entries belonging to the copy map are linked into the
     * list of entries silently and then added to the RB-tree later on. So
     * sanity checks are useless in that case.
     */
    check_map_with_hole_sanity =
        vm_map_lookup_entry(map, new_entry->vme_start, &tmp_entry);
#endif /* DEBUG */
    ...
    if (hole_entry->vme_start == new_entry->vme_start &&
      ...
      SAVE_HINT_HOLE_WRITE(map, (struct vm_map_links *)hole_entry);
      return;
    } else if ((new_entry->vme_start <= hole_entry->vme_start) &&
               (hole_entry->vme_start < new_entry->vme_end)) {
      /*
       * Case C1: Entry moving upwards and a part/full hole lies within the
       * bounds of the entry.
       */
      ...
      return;
    } else if ((new_entry->vme_start < hole_entry->vme_end) &&
               (hole_entry->vme_end <= new_entry->vme_end)) {
      /*
       * Case C2: Entry moving downwards and a part/full hole lies within the
       * bounds of the entry.
       */
  ...
  panic("Illegal action: h1: %p, s:0x%llx, e:0x%llx...h2:%p, s:0x%llx, "
        "e:0x%llx...h3:0x%p, s:0x%llx, e:0x%llx",
        VME_PREV(hole_entry),
        (unsigned long long)VME_PREV(hole_entry)->vme_start,
        (unsigned long long)VME_PREV(hole_entry)->vme_end, hole_entry,
        (unsigned long long)hole_entry->vme_start,
        (unsigned long long)hole_entry->vme_end, hole_entry->vme_next,
        (unsigned long long)hole_entry->vme_next->vme_start,
        (unsigned long long)hole_entry->vme_next->vme_end);
}

void update_first_free_rb(vm_map_t map, vm_map_entry_t entry,
                          bool new_entry_creation) {
  if (map->holelistenabled) {
    /*
     * Holes can be used to track ranges all the way up to MACH_VM_MAX_ADDRESS
     * or more (e.g. kernel map).
     */
    vm_map_offset_t max_valid_offset =
        MAX(map->max_offset, (vm_map_offset_t)MACH_VM_MAX_ADDRESS);

    /*
     * Clipping an entry will not result in the creation/deletion/modification
     * of a hole. Those calls pass NULL for their target entry.
     */
    if (entry == NULL) {
      return;
    }
     ...
     */
    if (vm_map_trunc_page(entry->vme_start, VM_MAP_PAGE_MASK(map)) >=
        max_valid_offset) {
      return;
    }
     ...
     *
     * We are updating the hole list after the fact (except in one special case
     * involving copy maps).
     *
     */
