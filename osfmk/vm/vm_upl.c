...
kern_return_t vm_upl_map(vm_map_t map, upl_t upl, vm_address_t *dst_addr) {
...
kern_return_t vm_upl_unmap(vm_map_t map, upl_t upl) {
  if (VM_MAP_NULL == map) {
    return KERN_INVALID_ARGUMENT;
  }
...
kern_return_t vm_upl_map_range(vm_map_t map, upl_t upl,
                               vm_offset_t offset_to_map, vm_size_t size_to_map,
                               vm_prot_t prot_to_map, vm_address_t *dst_addr) {
  ...
  aligned_offset_to_map =
      vm_map_trunc_page(offset_to_map, vm_map_page_mask(map));
  adjusted_offset = offset_to_map - aligned_offset_to_map;
  size_to_map =
      vm_map_round_page(size_to_map + adjusted_offset, vm_map_page_mask(map));

  kr = vm_map_enter_upl_range(map, upl, aligned_offset_to_map, size_to_map,
                              prot_to_map, &map_addr);
  *dst_addr = CAST_DOWN(vm_address_t, (map_addr + adjusted_offset));
  return kr;
}

/* unmap a part of a upl that was mapped in the address space. */
kern_return_t vm_upl_unmap_range(vm_map_t map, upl_t upl,
  ...
  aligned_offset_to_unmap =
      vm_map_trunc_page(offset_to_unmap, vm_map_page_mask(map));
  page_offset = offset_to_unmap - aligned_offset_to_unmap;
  size_to_unmap =
      vm_map_round_page(size_to_unmap + page_offset, vm_map_page_mask(map));

  return vm_map_remove_upl_range(map, upl, aligned_offset_to_unmap,
                                 size_to_unmap);
}

/* Retrieve a upl for an object underlying an address range in a map */

kern_return_t vm_map_get_upl(vm_map_t map, vm_map_offset_t map_offset,
  ...
  kr = vm_map_create_upl(map, map_offset, upl_size, upl, page_list, count,
                         &map_flags, tag);
...
kern_return_t upl_abort_range(upl_t upl, upl_offset_t offset, upl_size_t size,
                              int error, boolean_t *empty) {
  ...
  //	DEBUG4K_UPL("upl %p (u_offset 0x%llx u_size 0x%llx) object %p offset
  //0x%llx size 0x%llx error 0x%x\n", upl, (uint64_t)upl->u_offset,
  //(uint64_t)upl->u_size, upl->map_object, (uint64_t)offset, (uint64_t)size,
  //error);
  ...
  *empty = FALSE;
  ...
  if ((upl->flags & UPL_IO_WIRE) && !(error & UPL_ABORT_DUMP_PAGES)) {
    return upl_commit_range(upl, offset, size, UPL_COMMIT_FREE_ABSENT, NULL, 0,
                            empty);
  }
...
process_upl_to_abort:
  ...
  *empty = FALSE;
  ...
  if (upl->upl_commit_index < UPL_DEBUG_COMMIT_RECORDS) {
    upl->upl_commit_records[upl->upl_commit_index].c_btref =
  ...
  if (upl->flags & UPL_DEVICE_MEMORY) {
    ...
    DEBUG4K_ERROR(
        "upl %p (u_offset 0x%llx u_size 0x%x) offset 0x%x size 0x%x\n", upl,
        upl->u_offset, upl->u_size, offset, size);
    kr = KERN_FAILURE;
    goto done;
  }
  ...
  for (obj_offset = obj_start; obj_offset < obj_end; obj_offset += PAGE_SIZE) {
    vm_page_t t, m;
    unsigned int pg_num;
    boolean_t needed;
      ...
      if ((t = vm_page_lookup(object, target_offset)) != VM_PAGE_NULL) {
        ...
        if (m == VM_PAGE_NULL) {
          m = vm_page_lookup(shadow_object,
                             target_offset + object->vo_shadow_offset);
        }
      }
    }
    if ((upl->flags & UPL_KERNEL_OBJECT)) {
      ...
      if (m->vmp_absent) {
        ...
        if (m->vmp_overwriting) {
          if (VM_PAGE_WIRED(m)) {
            ...
            if (m->vmp_busy) {
              //							printf("*******
              //FBDP %s:%d page %p object %p ofsfet 0x%llx wired and busy\n",
    ...
    if (dwp->dw_mask) {
      if (dwp->dw_mask & ~(DW_clear_busy | DW_PAGE_WAKEUP)) {
        ...
        if (dw_count >= dw_limit) {
          vm_page_do_delayed_work(shadow_object, VM_KERN_MEMORY_NONE, dwp_start,
                                  dw_count);
...
kern_return_t upl_abort(upl_t upl, int error) {
  boolean_t empty;
...
kern_return_t upl_commit_range(upl_t upl, upl_offset_t offset, upl_size_t size,
                               int flags, upl_page_info_t *page_list,
                               mach_msg_type_number_t count, boolean_t *empty) {
  upl_size_t xfer_size, subupl_size;
  vm_object_t shadow_object;
  vm_object_t object;
...
process_upl_to_commit:
  ...
  if (isVectorUPL) {
    ...
    assertf(upl->flags & UPL_INTERNAL,
            "%s: sub-upl %p of vector upl %p has no internal page list",
  ...
  if ((upl->flags & UPL_IO_WIRE) && !(flags & UPL_COMMIT_FREE_ABSENT) &&
      !isVectorUPL && shadow_object->purgable != VM_PURGABLE_VOLATILE &&
  ...
  for (obj_offset = obj_start; obj_offset < obj_end; obj_offset += PAGE_SIZE) {
    ...
    if (upl->flags & UPL_IO_WIRE) {
      ...
      if (flags & UPL_COMMIT_SET_DIRTY) {
        ...
        if (!(flags & UPL_COMMIT_CS_VALIDATED) && m->vmp_cs_validated &&
    ...
    if (m->vmp_overwriting) {
      ...
      if (VM_PAGE_WIRED(m)) {
        ...
        if (m->vmp_busy) {
          //					printf("*******   FBDP %s:%d
          //page %p object %p ofsfet 0x%llx wired and busy\n", __FUNCTION__,
          //__LINE__, m, VM_PAGE_OBJECT(m), m->vmp_offset);
          upl_pages_wired_busy++;
        }
        assert(!m->vmp_absent);
    ...
    if (m->vmp_free_when_done) {
      ...
      if ((flags & UPL_COMMIT_SET_DIRTY) ||
          (m->vmp_pmapped &&
           (pmap_disconnect(VM_PAGE_GET_PHYS_PAGE(m)) & VM_MEM_MODIFIED))) {
        /*
    ...
    if ((upl->flags & UPL_PAGE_SYNC_DONE) ||
        (flags & UPL_COMMIT_CLEAR_PRECIOUS)) {
    ...
    if (hibernate_cleaning_in_progress == FALSE && !m->vmp_dirty &&
    ...
    } else if (should_be_throttled == TRUE &&
               (m->vmp_q_state == VM_PAGE_NOT_ON_Q)) {
      /*
      ...
      if ((flags & UPL_COMMIT_INACTIVATE) && !m->vmp_clustered &&
  ...
  if (upl->flags & UPL_DEVICE_MEMORY) {
    ...
    occupied =
        !fast_path_full_commit && !bitmap_is_empty(upl->lite_list, pages);
  } else {
    occupied = !vm_page_queue_empty(&upl->map_object->memq);
  }
  if (occupied == 0) {
...
kern_return_t upl_commit(upl_t upl, upl_page_info_t *page_list,
  ...
  return upl_commit_range(upl, 0, upl->u_size, 0, page_list, count, &empty);
}
