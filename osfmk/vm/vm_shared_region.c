 ...
 * automatically at exec() time, see vm_map_exec().  The first process of a
 * given environment sets up the shared region and all further processes in that
 ...
 * will then use the shared_region_check_np() and
 * shared_region_map_and_slide_2_np() system calls to validate and/or populate
 * the shared region with the appropriate dyld_shared_cache file.  If the
 * initial call to shared_region_check_np() indicates that the shared region has
 * not been configured, dyld will then call shared_region_map_and_slide_2_np()
 * to configure the shared region.  It's possible that multiple tasks may
 * simultaneously issue this call sequence for the same shared region, but the
 * synchronization done by shared_region_acquire() will ensure that only one
 * task will ultimately configure the shared region.  All other tasks will wait
 * for that task to finish its configuration step, at which point (assuming
 ...
 * sliding of the shared region is performed against a temporary
 * configuration-only vm_map, which is temporarily activated for the calling
 * thread using vm_map_switch_to().  Once mapping and sliding completes
 * successfully, the shared region will be "sealed" by stabilizing all its
 * vm_map_entrys using COPY_DELAY objects, which eliminates the need for later
 * modification of shared region map entries and thus simplifies the shared
 * region's runtime locking requirements. After this sealing step, the original
 * task vm_map will be restored.  Since this entire configuration sequence
 * happens within the context of a single system call, use of the temporary
 * vm_map effectively guarantees that the shared region will not be visible in
 * the task's address space (either to other threads in the task or to other
 * tasks attempting to query the address space e.g. for debugging purposes)
 * until it has been fully configured and sealed.
 *
 * The shared region is only inserted into a task's address space when the
 * shared_region_check_np() system call detects that the shared region has been
 * fully configured.  Only at this point will the placeholder entry inserted at
 * exec() time be replaced with the real shared region submap entry.  This step
 * is required of all tasks; even the task that previously configured the shared
 * region must issue a final shared_region_check_np() system call to obtain the
 * real shared region mapping.
 *
...
TUNABLE_WRITEABLE(int, shared_region_destroy_delay,
...
TUNABLE_WRITEABLE(int, panic_on_dyld_issue, "panic_on_dyld_issue",
                  PANIC_ON_DYLD_ISSUE_DEFAULT);

/*
 * Cached pointer to the most recently mapped shared region from PID 1, which
 * should be the most commonly mapped shared region in the system.  There are
 * many processes which do not use this, for a variety of reasons.
 *
...
#define vm_shared_region_sleep(event, interruptible)                           \
  lck_mtx_sleep_with_inheritor(                                                \
      &vm_shared_region_lock, LCK_SLEEP_DEFAULT, (event_t)(event), *(event),   \
      (interruptible) | THREAD_WAIT_NOREPORT, TIMEOUT_WAIT_FOREVER)
...
vm_shared_region_create(void *root_dir, cpu_type_t cputype,
                        cpu_subtype_t cpu_subtype, boolean_t is_64bit,
                        int target_page_shift, boolean_t reslide,
...
static kern_return_t vm_shared_region_slide_mapping(
    vm_shared_region_t sr, user_addr_t slide_info_addr,
    mach_vm_size_t slide_info_size, mach_vm_offset_t start, mach_vm_size_t size,
    mach_vm_offset_t slid_mapping, uint32_t slide, memory_object_control_t,
    vm_prot_t prot); /* forward */
static kern_return_t
vm_shared_region_insert_placeholder(vm_map_t map,
                                    vm_shared_region_t shared_region);
static kern_return_t
vm_shared_region_insert_submap(vm_map_t map, vm_shared_region_t shared_region,
                               bool overwrite);
 ...
 * necessary.
 */
vm_shared_region_t vm_shared_region_get(task_t task) {
  vm_shared_region_t shared_region;
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: get(%p) <- %p\n",
                             (void *)VM_KERNEL_ADDRPERM(task),
...
static void vm_shared_region_acquire(vm_shared_region_t shared_region) {
  vm_shared_region_lock();
  assert(shared_region->sr_ref_count > 0);
...
static void vm_shared_region_release(vm_shared_region_t shared_region) {
  vm_shared_region_lock();
...
static void vm_shared_region_seal(struct vm_shared_region *sr) {
  vm_map_t sr_map;
...
vm_map_t vm_shared_region_vm_map(vm_shared_region_t shared_region) {
  ipc_port_t sr_handle;
  vm_named_entry_t sr_mem_entry;
  vm_map_t sr_map;

  SHARED_REGION_TRACE_DEBUG(("shared_region: -> vm_map(%p)\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region)));
  assert(shared_region->sr_ref_count > 0);
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: vm_map(%p) <- %p\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region),
 ...
 */
void vm_shared_region_set(task_t task, vm_shared_region_t new_shared_region) {
  vm_shared_region_t old_shared_region;

  SHARED_REGION_TRACE_DEBUG(("shared_region: -> set(%p, %p)\n",
                             (void *)VM_KERNEL_ADDRPERM(task),
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: set(%p) <- old=%p new=%p\n",
                             (void *)VM_KERNEL_ADDRPERM(task),
 ...
 */
static inline bool match_subtype(cpu_type_t cputype, cpu_subtype_t exist,
                                 cpu_subtype_t new) {
  if (exist == new) {
    return true;
  }
  if (cputype == CPU_TYPE_ARM64 && exist == CPU_SUBTYPE_ARM64E &&
 ...
 */
vm_shared_region_t
vm_shared_region_lookup(void *root_dir, cpu_type_t cputype,
                        cpu_subtype_t cpu_subtype, boolean_t is_64bit,
                        int target_page_shift, boolean_t reslide,
                        boolean_t is_driverkit, uint32_t rsr_version) {
  vm_shared_region_t shared_region;
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: -> "
                             "lookup(root=%p,cpu=<%d,%d>,64bit=%d,pgshift=%d,"
                             "reslide=%d,driverkit=%d)\n",
                             (void *)VM_KERNEL_ADDRPERM(root_dir), cputype,
                             cpu_subtype, is_64bit, target_page_shift, reslide,
                             is_driverkit));
  ...
  for (;;) {
    ...
    if (new_shared_region == NULL) {
      ...
      vm_shared_region_unlock();
      new_shared_region = vm_shared_region_create(
          root_dir, cputype, cpu_subtype, is_64bit, target_page_shift, reslide,
          is_driverkit, rsr_version);
    ...
    queue_enter(&vm_shared_region_queue, shared_region, vm_shared_region_t,
                sr_q);
 ...
 */
static void
vm_shared_region_reference_locked(vm_shared_region_t shared_region) {
  LCK_MTX_ASSERT(&vm_shared_region_lock, LCK_MTX_ASSERT_OWNED);

  SHARED_REGION_TRACE_DEBUG(("shared_region: -> reference_locked(%p)\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region)));
  assert(shared_region->sr_ref_count > 0);
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: reference_locked(%p) <- %d\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region),
                             shared_region->sr_ref_count));
}
 ...
 */
void vm_shared_region_reference(vm_shared_region_t shared_region) {
  SHARED_REGION_TRACE_DEBUG(("shared_region: -> reference(%p)\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region)));
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: reference(%p) <- %d\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region),
                             shared_region->sr_ref_count));
}
 ...
 */
void vm_shared_region_deallocate(vm_shared_region_t shared_region) {
  SHARED_REGION_TRACE_DEBUG(("shared_region: -> deallocate(%p)\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region)));
  ...
  if (shared_region->sr_root_dir == NULL) {
     ...
     */
    if (shared_region_persistence && !shared_region->sr_persists) {
      /* make this one persistent */
      shared_region->sr_ref_count++;
      shared_region->sr_persists = TRUE;
    } else if (!shared_region_persistence && shared_region->sr_persists) {
  ...
  assert(shared_region->sr_ref_count > 0);
  shared_region->sr_ref_count--;
  SHARED_REGION_TRACE_DEBUG(("shared_region: deallocate(%p): ref now %d\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region),
                             shared_region->sr_ref_count));
  ...
  if (shared_region->sr_ref_count == 0) {
     ...
     */
    if (shared_region->sr_timer_call == NULL &&
        shared_region_destroy_delay != 0 && !shared_region->sr_stale &&
      ...
      shared_region->sr_timer_call =
          thread_call_allocate((thread_call_func_t)vm_shared_region_timeout,
                               (thread_call_param_t)shared_region);

      /* schedule the timer */
      clock_interval_to_deadline(shared_region_destroy_delay, NSEC_PER_SEC,
                                 &deadline);
      thread_call_enter_delayed(shared_region->sr_timer_call, deadline);

      SHARED_REGION_TRACE_DEBUG(("shared_region: deallocate(%p): armed timer\n",
                                 (void *)VM_KERNEL_ADDRPERM(shared_region)));
       ...
       * it...
       */
      queue_remove(&vm_shared_region_queue, shared_region, vm_shared_region_t,
                   sr_q);
      vm_shared_region_count--;
      vm_shared_region_unlock();
...
void vm_shared_region_timeout(thread_call_param_t param0,
...
vm_shared_region_create(void *root_dir, cpu_type_t cputype,
  ...
  SHARED_REGION_TRACE_INFO(("shared_region: -> "
                            "create(root=%p,cpu=<%d,%d>,64bit=%d,pgshift=%d,"
                            "reslide=%d,driverkit=%d)\n",
                            (void *)VM_KERNEL_ADDRPERM(root_dir), cputype,
                            cpu_subtype, is_64bit, target_page_shift, reslide,
                            is_driverkit));
  ...
  shared_region = kalloc_type(struct vm_shared_region, Z_WAITOK | Z_NOFAIL);
...
#if defined(__arm64__)
  {
...
#if __ARM_MIXED_PAGE_SIZE__
    if (cputype == CPU_TYPE_ARM64 && target_page_shift == FOURK_PAGE_SHIFT) {
    ...
    if ((nested_pmap != PMAP_NULL) && (config_pmap != PMAP_NULL)) {
      ...
      sub_map = vm_map_create_options(nested_pmap, 0, (vm_map_offset_t)size,
                                      VM_MAP_CREATE_PAGEABLE);
      config_map =
          vm_map_create_options(config_pmap, base_address, base_address + size,
                                VM_MAP_CREATE_PAGEABLE);

      if (is_64bit || page_shift_user32 == SIXTEENK_PAGE_SHIFT) {
...
#endif /* __has_feature(ptrauth_calls) */
  kern_return_t kr =
      vm_shared_region_insert_submap(config_map, shared_region, false);
  if (kr != KERN_SUCCESS) {
    SHARED_REGION_TRACE_ERROR(
  ...
  if (shared_region) {
    SHARED_REGION_TRACE_INFO(
        ("shared_region: "
         "create(root=%p,cpu=<%d,%d>,64bit=%d,reslide=%d,driverkit=%d,"
         "base=0x%llx,size=0x%llx) <- "
         "%p mem=(%p,%p) map=%p pmap=%p\n",
         (void *)VM_KERNEL_ADDRPERM(root_dir), cputype, cpu_subtype, is_64bit,
         reslide, is_driverkit, (long long)base_address, (long long)size,
         (void *)VM_KERNEL_ADDRPERM(shared_region),
    ...
    SHARED_REGION_TRACE_INFO(
        ("shared_region: create(root=%p,cpu=<%d,%d>,64bit=%d,driverkit=%d,"
         ...
         (void *)VM_KERNEL_ADDRPERM(root_dir), cputype, cpu_subtype, is_64bit,
         is_driverkit, (long long)base_address, (long long)size));
  }
  return shared_region;
}
...
static void vm_shared_region_destroy(vm_shared_region_t shared_region) {
  ...
  SHARED_REGION_TRACE_INFO(
      ("shared_region: -> destroy(%p) "
       ...
       shared_region->sr_cpu_type, shared_region->sr_cpu_subtype,
       shared_region->sr_64bit, shared_region->sr_driverkit));
   ...
   */
  if (map->pmap) {
    pmap_remove(map->pmap, (vm_map_offset_t)shared_region->sr_base_address,
  ...
  for (uint_t i = 0; i < shared_region->sr_num_auth_section; ++i) {
    ...
    if (si != NULL) {
      ...
      kfree_data(si->si_slide_info_entry, si->si_slide_info_size);
  ...
  if (shared_region->sr_auth_section != NULL) {
    ...
    kfree_type(vm_shared_region_slide_info_t,
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: destroy(%p) <-\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region)));
  shared_region = NULL;
}
 ...
 */
kern_return_t vm_shared_region_start_address(vm_shared_region_t shared_region,
                                             mach_vm_offset_t *start_address) {
  kern_return_t kr;
  mach_vm_offset_t sr_base_address;
  mach_vm_offset_t sr_first_mapping;

  SHARED_REGION_TRACE_DEBUG(("shared_region: -> start_address(%p)\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region)));
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: start_address(%p) <- 0x%llx\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region),
...
kern_return_t vm_shared_region_update_task(task_t task,
                                           vm_shared_region_t shared_region,
                                           mach_vm_offset_t start_address) {
  kern_return_t kr = KERN_SUCCESS;
  uuid_t shared_region_uuid;
  _Static_assert(sizeof(shared_region_uuid) ==
 ...
 */
static kern_return_t find_mapping_to_slide(vm_map_t map, vm_map_address_t addr,
...
static bool shared_region_make_permanent(vm_shared_region_t sr,
                                         vm_prot_t max_prot) {
...
static bool shared_region_tpro_protect(vm_shared_region_t sr,
 ...
 */
static boolean_t task_sign_pointers(task_t task) {
  if (task->map && task->map->pmap && !task->map->pmap->disable_jop) {
    return TRUE;
  }
  return FALSE;
}
 ...
 */
__attribute__((noinline)) kern_return_t
vm_shared_region_auth_remap(vm_shared_region_t sr) {
   ...
   * serializing multiple tasks at the auth_remap step.  But
   * shared_region_pager_match() is slightly racy and can produce duplicate
   * pagers without shared-region-wide synchronization, which is a potential
  ...
  for (i = 0; i < sr->sr_num_auth_section; ++i) {
    ...
    kr = find_mapping_to_slide(
        sr_map, si->si_slid_address - sr->sr_base_address, &tmp_entry_store);
    if (kr != KERN_SUCCESS) {
     ...
     */
    if (tmp_entry->vme_end - tmp_entry->vme_start !=
    ...
    vmk_flags.vmf_permanent =
        shared_region_make_permanent(sr, tmp_entry->max_protection);

    /* Preserve the TPRO flag if task has TPRO enabled */
    vmk_flags.vmf_tpro = (vm_map_tpro(task->map) && tmp_entry->used_for_tpro &&
                          task_has_tpro(task));

    map_addr = si->si_slid_address;
    kr = mach_vm_map_kernel(task->map, vm_sanitize_wrap_addr_ref(&map_addr),
                            si->si_end - si->si_start, 0, vmk_flags,
                            (ipc_port_t)(uintptr_t)sr_pager, 0, TRUE,
                            tmp_entry->protection, tmp_entry->max_protection,
    ...
    assertf(map_addr == si->si_slid_address,
            ...
            (uint64_t)map_addr, (uint64_t)si->si_slid_address, tmp_entry);
  ...
  /*
   * Drop any extra reference to the pager in case we're quitting due to an
   * error above.
   */
  if (sr_pager != MEMORY_OBJECT_NULL) {
    memory_object_deallocate(sr_pager);
...
void vm_shared_region_undo_mappings(
    vm_map_t sr_map, mach_vm_offset_t sr_base_address,
   ...
   */
  for (srfmp = &srf_mappings[0]; srfmp <= srf_mappings_current; srfmp++) {
    ...
    for (j = 0; j < mappings_count; j++) {
      ...
      SHARED_REGION_TRACE_INFO(("shared_region: mapping[%d]: "
                                "address:0x%016llx "
                                "size:0x%016llx "
                                "offset:0x%016llx "
                                "maxprot:0x%x prot:0x%x: "
                                "undoing...\n",
                                j, (long long)mappings[j].sms_address,
      ...
      kr2 = vm_map_remove_guard(sr_map, start, end, VM_MAP_REMOVE_IMMUTABLE,
                                KMEM_GUARD_NONE)
                .kmr_return;
 ...
 */
__attribute__((noinline)) static kern_return_t vm_shared_region_map_file_setup(
    vm_shared_region_t shared_region, int sr_file_mappings_count,
    ...
    mach_vm_offset_t *sfm_min_address, mach_vm_offset_t *sfm_max_address,
    vm_map_t *sr_map_ptr, vm_map_offset_t *lowest_unnestable_addr_ptr,
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: -> map(%p)\n",
                             (void *)VM_KERNEL_ADDRPERM(shared_region)));
  ...
  for (srfmp = &sr_file_mappings[0];
       srfmp < &sr_file_mappings[sr_file_mappings_count]; srfmp++) {
    ...
    } else if (shared_region->sr_slide != 0 && srfmp->slide != 0 &&
               shared_region->sr_slide != srfmp->slide) {
      SHARED_REGION_TRACE_ERROR(
    ...
    if (srfmp->fd == -1) {
      ...
      SHARED_REGION_TRACE_INFO(
          ("shared_region: mapping[0]: "
           ...
           (long long)mappings[0].sms_address, (long long)mappings[0].sms_size,
           (long long)mappings[0].sms_file_offset, mappings[0].sms_max_prot,
      ...
      kr = vm_map_enter(kernel_map, &kaddr, obj_size, 0, vmk_flags, object, 0,
       ...
       */
      int copyin_err = copyin((user_addr_t)mappings->sms_file_offset,
                              (void *)kaddr, mappings->sms_size);
      vm_map_remove(kernel_map, kaddr, kaddr + obj_size);
      if (copyin_err) {
        printf("%s(): for fd==-1 copyin(%p) failed, errno=%d\n", __func__,
               (void *)mappings->sms_file_offset, copyin_err);
        switch (copyin_err) {
        case EPERM:
        case EACCES:
       ...
       */
      target_address =
          (vm_map_offset_t)(mappings[0].sms_address - sr_base_address);
      vmk_flags = VM_MAP_KERNEL_FLAGS_FIXED();
      vmk_flags.vmkf_already = TRUE;
      vmk_flags.vmkf_no_copy_on_read = 1;
      vmk_flags.vmf_permanent =
          shared_region_make_permanent(shared_region, mappings[0].sms_max_prot);

      kr = vm_map_enter(
          sr_map, &target_address,
          vm_map_round_page(mappings[0].sms_size, VM_MAP_PAGE_MASK(sr_map)), 0,
          vmk_flags, object, 0, TRUE, mappings[0].sms_init_prot & VM_PROT_ALL,
          mappings[0].sms_max_prot & VM_PROT_ALL, VM_INHERIT_DEFAULT);
      if (kr != KERN_SUCCESS) {
      ...
      if (os_add_overflow(mappings[0].sms_address, mappings[0].sms_size,
                          &sfm_end) ||
    ...
    for (i = 0; i < mappings_count; i++) {
      SHARED_REGION_TRACE_INFO(
          ("shared_region: mapping[%d]: "
           ...
           "maxprot:0x%x prot:0x%x\n",
           i, (long long)mappings[i].sms_address,
           (long long)mappings[i].sms_size,
           (long long)mappings[i].sms_file_offset, mappings[i].sms_max_prot,
      ...
      if (os_add_overflow(mappings[i].sms_address, mappings[i].sms_size,
                          &sfm_end) ||
          (vm_map_round_page(sfm_end, VM_MAP_PAGE_MASK(sr_map)) <
      ...
      if (__improbable(os_sub_overflow(mappings[i].sms_address, sr_base_address,
                                       &target_address))) {
        kr = KERN_INVALID_ARGUMENT;
        break;
      }
      ...
      vmk_flags.vmkf_no_copy_on_read = 1;
      vmk_flags.vmf_permanent =
          shared_region_make_permanent(shared_region, mappings[i].sms_max_prot);
      vmk_flags.vmf_tpro =
          shared_region_tpro_protect(shared_region, mappings[i].sms_max_prot);
      ...
      if (map_port == MACH_PORT_NULL) {
         ...
         * We have to create the VM object now, so that it can be mapped
         * "copy-on-write".
         */
        obj_size =
        ...
        if (object == VM_OBJECT_NULL) {
          ...
          kr = vm_map_enter(
              sr_map, &target_address,
              vm_map_round_page(mappings[i].sms_size, VM_MAP_PAGE_MASK(sr_map)),
              0, vmk_flags, object, 0, TRUE,
              mappings[i].sms_init_prot & VM_PROT_ALL,
              mappings[i].sms_max_prot & VM_PROT_ALL, VM_INHERIT_DEFAULT);
        }
      } else {
        object = VM_OBJECT_NULL; /* no anonymous memory here */
        kr = mach_vm_map_kernel(
            sr_map, vm_sanitize_wrap_addr_ref(&target_address),
            vm_map_round_page(mappings[i].sms_size, VM_MAP_PAGE_MASK(sr_map)),
            0, vmk_flags, map_port, mappings[i].sms_file_offset, TRUE,
            mappings[i].sms_init_prot & VM_PROT_ALL,
            mappings[i].sms_max_prot & VM_PROT_ALL, VM_INHERIT_DEFAULT);
      }
  ...
  if (kr != KERN_SUCCESS) {
    ...
    SHARED_REGION_TRACE_ERROR(
        ("shared_region: mapping[%d]: "
         ...
         i, (long long)mappings[i].sms_address, (long long)mappings[i].sms_size,
         (long long)mappings[i].sms_file_offset, mappings[i].sms_max_prot,
         mappings[i].sms_init_prot, kr));
    ...
    vm_shared_region_undo_mappings(sr_map, sr_base_address, sr_file_mappings,
                                   srfmp, i);
    return kr;
  }
...
__attribute__((noinline)) static void vm_shared_region_map_file_final(
    vm_shared_region_t shared_region, vm_map_t sr_map,
    mach_vm_offset_t sfm_min_address, mach_vm_offset_t sfm_max_address);
 ...
 */
__attribute__((noinline)) kern_return_t vm_shared_region_map_file(
    vm_shared_region_t shared_region, int sr_file_mappings_count,
    struct _sr_file_mappings *sr_file_mappings) {
  kern_return_t kr = KERN_SUCCESS;
  unsigned int i;
  ...
  if (vmsr_num_slides > 0) {
    ...
    mappings_to_slide = kalloc_type(struct shared_file_mapping_slide_np *,
                                    vmsr_num_slides, Z_WAITOK | Z_ZERO);
  }

  vm_shared_region_acquire(shared_region);

  /*
   * Did someone race in and map this shared region already, or did an earlier
   * mapping fail?
   */
  ...
  kr = vm_shared_region_map_file_setup(
      shared_region, sr_file_mappings_count, sr_file_mappings,
      &mappings_to_slide_cnt, mappings_to_slide, slid_mappings,
      slid_file_controls, &sfm_min_address, &sfm_max_address, &sr_map,
      &lowest_unnestable_addr, vmsr_num_slides);
  if (kr != KERN_SUCCESS) {
    goto done;
  }
  ...
  if (shared_region->sr_num_auth_section > 0) {
    shared_region->sr_auth_section =
        kalloc_type(vm_shared_region_slide_info_t,
                    shared_region->sr_num_auth_section, Z_WAITOK | Z_ZERO);
  }
#endif /* __has_feature(ptrauth_calls) */
  for (i = 0; i < mappings_to_slide_cnt; ++i) {
    kr = vm_shared_region_slide(
        shared_region->sr_slide, mappings_to_slide[i]->sms_file_offset,
        mappings_to_slide[i]->sms_size, mappings_to_slide[i]->sms_slide_start,
        mappings_to_slide[i]->sms_slide_size, slid_mappings[i],
        slid_file_controls[i], mappings_to_slide[i]->sms_max_prot);
    if (kr != KERN_SUCCESS) {
      SHARED_REGION_TRACE_ERROR(
          ("shared_region: region_slide("
           ...
           (long long)mappings_to_slide[i]->sms_slide_size, kr));
      vm_shared_region_undo_mappings(
          sr_map, shared_region->sr_base_address, &sr_file_mappings[0],
  ...
  vm_shared_region_map_file_final(shared_region, sr_map, sfm_min_address,
                                  sfm_max_address);
  vm_shared_region_unlock();
   ...
   */
  if (kr == KERN_SUCCESS) {
    pmap_trim(shared_region->sr_config_map->pmap, sr_map->pmap, sfm_min_address,
              sfm_max_address - sfm_min_address);
  }
#endif
  if (map_switched) {
  ...
  SHARED_REGION_TRACE_DEBUG(("shared_region: map(%p) <- 0x%x \n",
...
vm_shared_region_map_file_final(vm_shared_region_t shared_region,
  ...
  if (!shared_region->sr_uuid_copied) {
    error = copyin((user_addr_t)(shared_region->sr_base_address +
                                 shared_region->sr_first_mapping),
                   (char *)&sr_cache_header, sizeof(sr_cache_header));
    if (error == 0) {
      memcpy(&shared_region->sr_uuid, &sr_cache_header.uuid,
             sizeof(shared_region->sr_uuid));
      shared_region->sr_uuid_copied = TRUE;
      locally_built = sr_cache_header.locallyBuiltCache;
    } else {
#if DEVELOPMENT || DEBUG
      panic("shared_region: copyin shared_cache_header(sr_base_addr:0x%016llx "
            "sr_first_mapping:0x%016llx "
            "offset:0 size:0x%016llx) failed with %d\n",
            (long long)shared_region->sr_base_address,
            (long long)shared_region->sr_first_mapping,
            (long long)sizeof(sr_cache_header), error);
#endif /* DEVELOPMENT || DEBUG */
      shared_region->sr_uuid_copied = FALSE;
    }
  }
  ...
  /*
   * We save a pointer to the shared cache mapped by the "init task", i.e.
   * launchd.  This is used by the stackshot code to reduce output size in the
   * common case that everything maps the same shared cache. One gotcha is that
   * "userspace reboots" can occur which can cause a new shared region to be the
   * primary region.  In that case, launchd re-exec's itself, so we may go
   * through this path multiple times.  We let the most recent one win.
   *
   * Check whether the shared cache is a custom built one and copy in the shared
   * cache layout accordingly.
   */
  bool is_init_task = (task_pid(current_task()) == 1);
  if (shared_region->sr_uuid_copied && is_init_task) {
    /* Copy in the shared cache layout if we're running with a locally built
     * shared cache */
    if (locally_built) {
      KDBG((MACHDBG_CODE(DBG_MACH_SHAREDREGION, PROCESS_SHARED_CACHE_LAYOUT)) |
           DBG_FUNC_START);
      image_array_length = (size_t)(sr_cache_header.imagesTextCount *
                                    sizeof(struct _dyld_cache_image_text_info));
      sr_image_layout = kalloc_data(image_array_length, Z_WAITOK);
      error = copyin((user_addr_t)(shared_region->sr_base_address +
                                   shared_region->sr_first_mapping +
                                   sr_cache_header.imagesTextOffset),
                     (char *)sr_image_layout, image_array_length);
      if (error == 0) {
        if (sr_cache_header.imagesTextCount >= UINT32_MAX) {
          panic("shared_region: sr_cache_header.imagesTextCount >= UINT32_MAX");
        }
        shared_region->sr_images =
            kalloc_data((vm_size_t)(sr_cache_header.imagesTextCount *
                                    sizeof(struct dyld_uuid_info_64)),
                        Z_WAITOK);
        for (size_t index = 0; index < sr_cache_header.imagesTextCount;
             index++) {
          memcpy((char *)&shared_region->sr_images[index].imageUUID,
                 (char *)&sr_image_layout[index].uuid,
                 sizeof(shared_region->sr_images[index].imageUUID));
          shared_region->sr_images[index].imageLoadAddress =
              sr_image_layout[index].loadAddress;
        }

        shared_region->sr_images_count =
            (uint32_t)sr_cache_header.imagesTextCount;
      } else {
#if DEVELOPMENT || DEBUG
        panic(
            "shared_region: copyin shared_cache_layout(sr_base_addr:0x%016llx "
            "sr_first_mapping:0x%016llx "
            "offset:0x%016llx size:0x%016llx) failed with %d\n",
            (long long)shared_region->sr_base_address,
            (long long)shared_region->sr_first_mapping,
            (long long)sr_cache_header.imagesTextOffset,
            (long long)image_array_length, error);
#endif /* DEVELOPMENT || DEBUG */
      }
      KDBG((MACHDBG_CODE(DBG_MACH_SHAREDREGION, PROCESS_SHARED_CACHE_LAYOUT)) |
               DBG_FUNC_END,
...
/*
 * Insert the real shared region submap entry into a task's VM map over the
 * placeholder installed by vm_map_exec().  Note that this function can only be
 * called once per vm_map, and cannot be undone.  This is because it results in
 * the shared region's pmap being nested into [map]'s pmap; on some platforms
 * the security model requires this nesting relationship to be permanent, so the
 * nested pmap cannot be "de-nested" from the top-level pmap or "re-nested"
 * again into the same top-level pmap.
 */
kern_return_t vm_shared_region_insert_submap(vm_map_t map,
                                             vm_shared_region_t shared_region,
                                             bool overwrite) {
  ...
  if (sr_pmap_nesting_start > sr_address) {
    ...
    kr = mach_vm_map_kernel(map, vm_sanitize_wrap_addr_ref(&target_address),
                            mapping_size, 0, vmk_flags, sr_handle, sr_offset,
                            TRUE, cur_prot, max_prot, VM_INHERIT_SHARE);
    if (kr != KERN_SUCCESS) {
      SHARED_REGION_TRACE_ERROR(
          ("shared_region: insert_submap(%p,%p): "
           "vm_map_enter(0x%llx,0x%llx,%p) error 0x%x\n",
           (void *)VM_KERNEL_ADDRPERM(map),
           (void *)VM_KERNEL_ADDRPERM(shared_region), (long long)target_address,
           (long long)mapping_size, (void *)VM_KERNEL_ADDRPERM(sr_handle), kr));
      return kr;
    }
    SHARED_REGION_TRACE_DEBUG(
        ("shared_region: insert_submap(%p,%p): "
  ...
  kr =
      mach_vm_map_kernel(map, vm_sanitize_wrap_addr_ref(&target_address),
                         sr_pmap_nesting_size, 0, vmk_flags, sr_handle,
                         sr_offset, TRUE, cur_prot, max_prot, VM_INHERIT_SHARE);
  if (kr != KERN_SUCCESS) {
    SHARED_REGION_TRACE_ERROR(("shared_region: insert_submap(%p,%p): "
                               "vm_map_enter(0x%llx,0x%llx,%p) error 0x%x\n",
                               (void *)VM_KERNEL_ADDRPERM(map),
                               (void *)VM_KERNEL_ADDRPERM(shared_region),
  ...
  SHARED_REGION_TRACE_DEBUG(
      ("shared_region: insert_submap(%p,%p): "
       ...
       (void *)VM_KERNEL_ADDRPERM(map),
       (void *)VM_KERNEL_ADDRPERM(shared_region), (long long)target_address,
       (long long)sr_pmap_nesting_size, (void *)VM_KERNEL_ADDRPERM(sr_handle),
       kr));
  ...
  if (sr_size > 0) {
    ...
    kr = mach_vm_map_kernel(map, vm_sanitize_wrap_addr_ref(&target_address),
                            mapping_size, 0, VM_MAP_KERNEL_FLAGS_FIXED(),
                            sr_handle, sr_offset, TRUE, cur_prot, max_prot,
                            VM_INHERIT_SHARE);
    if (kr != KERN_SUCCESS) {
      SHARED_REGION_TRACE_ERROR(
          ("shared_region: insert_submap(%p,%p): "
 ...
 * This is intended to be called when a new task is exec'ed and initially
 * associated with a shared region.  Once the userspace dyld initialization
 * sequence successfully queries the shared region start address via the
 * shared_region_check_np syscall, this placeholder will be replaced with the
 * real shared region submap entry.
 */
static kern_return_t
vm_shared_region_insert_placeholder(vm_map_t map,
                                    vm_shared_region_t shared_region) {
  ...
  pmap_set_shared_region(map->pmap,
                         vm_shared_region_vm_map(shared_region)->pmap, address,
                         shared_region->sr_size);

  return vm_map_enter(map, &address, shared_region->sr_size, (vm_map_offset_t)0,
                      vmk_flags, VM_OBJECT_NULL, (vm_object_offset_t)0, FALSE,
 ...
 */
kern_return_t vm_shared_region_enter(struct _vm_map *map, struct task *task,
                                     boolean_t is_64bit, void *fsroot,
                                     cpu_type_t cpu, cpu_subtype_t cpu_subtype,
                                     boolean_t reslide, boolean_t is_driverkit,
  ...
  SHARED_REGION_TRACE_DEBUG(
      ("shared_region: -> "
       ...
       (void *)VM_KERNEL_ADDRPERM(map), (void *)VM_KERNEL_ADDRPERM(task),
       (void *)VM_KERNEL_ADDRPERM(fsroot), cpu, cpu_subtype, is_64bit,
       is_driverkit));
  ...
  shared_region = vm_shared_region_lookup(fsroot, cpu, cpu_subtype, is_64bit,
                                          VM_MAP_PAGE_SHIFT(map), reslide,
                                          is_driverkit, rsr_version);
  if (shared_region == NULL) {
    /* this should not happen ! */
    SHARED_REGION_TRACE_ERROR(("shared_region: -> "
                               "enter(map=%p,task=%p,root=%p,cpu=<%d,%d>,64bit="
                               "%d,reslide=%d,driverkit=%d): "
                               "lookup failed !\n",
                               (void *)VM_KERNEL_ADDRPERM(map),
                               (void *)VM_KERNEL_ADDRPERM(task),
                               (void *)VM_KERNEL_ADDRPERM(fsroot), cpu,
                               cpu_subtype, is_64bit, reslide, is_driverkit));
  ...
  SHARED_REGION_TRACE_DEBUG(
      ("shared_region: enter(%p,%p,%p,%d,%d,%d,%d,%d) <- 0x%x\n",
       (void *)VM_KERNEL_ADDRPERM(map), (void *)VM_KERNEL_ADDRPERM(task),
       (void *)VM_KERNEL_ADDRPERM(fsroot), cpu, cpu_subtype, is_64bit, reslide,
       is_driverkit, kr));
  return kr;
}

void vm_shared_region_remove(task_t task, vm_shared_region_t sr) {
  vm_map_t map;
  ...
  kr = mach_vm_map_kernel(map, vm_sanitize_wrap_addr_ref(&start), size,
                          0,                                   /* mask */
                          vmk_flags, MACH_PORT_NULL, 0, FALSE, /* copy */
                          VM_PROT_NONE, VM_PROT_NONE, VM_INHERIT_DEFAULT);
  if (kr != KERN_SUCCESS) {
    printf("%s:%d vm_map(0x%llx, 0x%llx) error %d\n", __FUNCTION__, __LINE__,
           (uint64_t)sr->sr_base_address, (uint64_t)size, kr);
  }
}

#define SANE_SLIDE_INFO_SIZE (2560 * 1024) /*Can be changed if needed*/

kern_return_t vm_shared_region_sliding_valid(uint32_t slide) {
  kern_return_t kr = KERN_SUCCESS;
...
/*
 * Actually create (really overwrite) the mapping to part of the shared cache
 * which undergoes relocation.  This routine reads in the relocation info from
 * dyld and verifies it. It then creates a (or finds a matching) shared region
 * pager which handles the actual modification of the page contents and installs
 * the mapping using that pager.
 */
kern_return_t vm_shared_region_slide_mapping(
    vm_shared_region_t sr, user_addr_t slide_info_addr,
    mach_vm_size_t slide_info_size, mach_vm_offset_t start, mach_vm_size_t size,
    mach_vm_offset_t slid_mapping, uint32_t slide,
    memory_object_control_t sr_file_control, vm_prot_t prot) {
  kern_return_t kr;
  ...
  if ((kr = vm_shared_region_slide_sanity_check(
           slide_info_entry, slide_info_size)) != KERN_SUCCESS) {
  ...
  si = kalloc_type(struct vm_shared_region_slide_info, Z_WAITOK | Z_NOFAIL);
   ...
   */
  if (sr->sr_cpu_type == CPU_TYPE_ARM64 &&
      sr->sr_cpu_subtype == CPU_SUBTYPE_ARM64E && !(prot & VM_PROT_NOAUTH)) {
   ...
   */
  sr_map = vm_shared_region_vm_map(sr);
  kr = find_mapping_to_slide(sr_map, (vm_map_address_t)slid_mapping,
                             &tmp_entry_store);
  if (kr != KERN_SUCCESS) {
    goto done;
  }
   ...
   */
  if (primary_system_shared_region == NULL ||
      primary_system_shared_region == sr || sr->sr_driverkit) {
  ...
  vmk_flags.vmf_permanent =
      shared_region_make_permanent(sr, tmp_entry->max_protection);
  vmk_flags.vmf_tpro = shared_region_tpro_protect(sr, prot);
  kr = mach_vm_map_kernel(sr_map, vm_sanitize_wrap_addr_ref(&map_addr),
                          tmp_entry->vme_end - tmp_entry->vme_start, 0,
                          vmk_flags, (ipc_port_t)(uintptr_t)sr_pager, 0, TRUE,
                          tmp_entry->protection, tmp_entry->max_protection,
                          tmp_entry->inheritance);
  assertf(kr == KERN_SUCCESS, "kr = 0x%x\n", kr);
  assertf(map_addr == tmp_entry->vme_start,
          "map_addr=0x%llx vme_start=0x%llx tmp_entry=%p\n", (uint64_t)map_addr,
          (uint64_t)tmp_entry->vme_start, tmp_entry);
  ...
  if (sr_pager != NULL) {
     ...
     * The mapping, if it succeeded, is now holding a reference on the memory
     * object.
     */
    memory_object_deallocate(sr_pager);
    sr_pager = MEMORY_OBJECT_NULL;
...
static kern_return_t vm_shared_region_slide_sanity_check_v1(
  ...
  for (; pageIndex < s_info->toc_count; pageIndex++) {
    ...
    if (entryIndex >= s_info->entry_count) {
      printf("No sliding bitmap entry for pageIndex: %d at entryIndex: %d "
             "amongst %d entries\n",
...
static kern_return_t vm_shared_region_slide_sanity_check_v2(
  ...
  /* Ensure that the slide info doesn't reference any data outside of its
   * bounds. */
  ...
  /* Scale by sizeof(uint16_t). Hard-coding the size simplifies the overflow
   * check. */
...
static kern_return_t vm_shared_region_slide_sanity_check_v3(
  ...
  if (s_info->page_size != PAGE_SIZE_FOR_SR_SLIDE) {
    printf("vm_shared_region_slide_sanity_check_v3: s_info->page_size != "
           "PAGE_SIZE_FOR_SR_SL 0x%llx != 0x%llx\n",
  ...
  mach_vm_size_t required_size = sizeof(*s_info) + trailing_size;
  if (required_size < sizeof(*s_info)) {
    printf("vm_shared_region_slide_sanity_check_v3: required_size != "
           "sizeof(*s_info) 0x%llx != 0x%llx\n",
...
static kern_return_t vm_shared_region_slide_sanity_check_v4(
...
static kern_return_t vm_shared_region_slide_sanity_check_v5(
  ...
  if (s_info->page_size != PAGE_SIZE_FOR_SR_SLIDE_16KB) {
    printf("vm_shared_region_slide_sanity_check_v5: s_info->page_size != "
           "PAGE_SIZE_FOR_SR_SL 0x%llx != 0x%llx\n",
  ...
  mach_vm_size_t required_size = sizeof(*s_info) + trailing_size;
  if (required_size < sizeof(*s_info)) {
    printf("vm_shared_region_slide_sanity_check_v5: required_size != "
           "sizeof(*s_info) 0x%llx != 0x%llx\n",
           (uint64_t)required_size, (uint64_t)sizeof(*s_info));
    return KERN_FAILURE;
  }
...
static kern_return_t
vm_shared_region_slide_sanity_check(vm_shared_region_slide_info_entry_t s_info,
...
static kern_return_t
vm_shared_region_slide_page_v1(vm_shared_region_slide_info_t si,
                               vm_offset_t vaddr, uint32_t pageIndex) {
  uint16_t *toc = NULL;
  ...
  if (pageIndex >= s_info->toc_count) {
    ...
    slide_info_entry_toc_t slide_info_entries =
        (slide_info_entry_toc_t)((uintptr_t)s_info + s_info->entry_offset);
        ...
        if (b != 0) {
          for (j = 0; j < 8; ++j) {
            if (b & (1 << j)) {
              ...
              ptr_to_slide = (uint32_t *)((uintptr_t)(vaddr) +
                                          (sizeof(uint32_t) * (i * 8 + j)));
              ...
              *ptr_to_slide += slide;
              if (is_64 && *ptr_to_slide < old_value) {
                 ...
                 * The sliding failed...
                 */
                printf("vm_shared_region_slide() carry over: i=%d j=%d b=0x%x "
                       "slide=0x%x old=0x%x new=0x%x\n",
...
static kern_return_t
rebase_chain_32(uint8_t *page_content, uint16_t start_offset,
  ...
  /* If the offset went past the end of the page, then the slide data is
   * invalid. */
...
static kern_return_t
rebase_chain_64(uint8_t *page_content, uint16_t start_offset,
                uint32_t slide_amount,
                vm_shared_region_slide_info_entry_v2_t s_info) {
...
static kern_return_t
rebase_chain(boolean_t is_64, uint32_t pageIndex, uint8_t *page_content,
             uint16_t start_offset, uint32_t slide_amount,
             vm_shared_region_slide_info_entry_v2_t s_info) {
  kern_return_t kr;
  if (is_64) {
  ...
  if (kr != KERN_SUCCESS) {
    printf("vm_shared_region_slide_page() offset overflow: pageIndex=%u, "
           "start_offset=%u, slide_amount=%u\n",
...
static kern_return_t
vm_shared_region_slide_page_v2(vm_shared_region_slide_info_t si,
                               vm_offset_t vaddr, uint32_t pageIndex) {
  ...
  const uint16_t *page_starts =
      (uint16_t *)((uintptr_t)s_info + s_info->page_starts_offset);
  const uint16_t *page_extras =
      (uint16_t *)((uintptr_t)s_info + s_info->page_extras_offset);

  uint8_t *page_content = (uint8_t *)vaddr;
  uint16_t page_entry;

  if (pageIndex >= s_info->page_starts_count) {
    printf("vm_shared_region_slide_page() did not find page start in slide "
           "info: pageIndex=%u, count=%u\n",
  ...
  if (page_entry & DYLD_CACHE_SLIDE_PAGE_ATTR_EXTRA) {
    ...
    do {
      ...
      if (chain_index >= s_info->page_extras_count) {
        printf("vm_shared_region_slide_page() out-of-bounds extras index: "
               "index=%u, count=%u\n",
               chain_index, s_info->page_extras_count);
        return KERN_FAILURE;
      }
      info = page_extras[chain_index];
      page_start_offset = (uint16_t)((info & DYLD_CACHE_SLIDE_PAGE_VALUE)
                                     << DYLD_CACHE_SLIDE_PAGE_OFFSET_SHIFT);

      kr = rebase_chain(is_64, pageIndex, page_content, page_start_offset,
                        slide_amount, s_info);
      if (kr != KERN_SUCCESS) {
        return KERN_FAILURE;
      }
    ...
    const uint16_t page_start_offset =
        (uint16_t)(page_entry << DYLD_CACHE_SLIDE_PAGE_OFFSET_SHIFT);
    kern_return_t kr;
...
static kern_return_t vm_shared_region_slide_page_v3(
    vm_shared_region_slide_info_t si, vm_offset_t vaddr,
    __unused mach_vm_offset_t uservaddr, uint32_t pageIndex,
#if !__has_feature(ptrauth_calls)
    __unused
#endif /* !__has_feature(ptrauth_calls) */
        uint64_t jop_key) {
  ...
  do {
    ...
    if (isAuthenticated) {
      ...
      if (hasAddressDiversity) {
        // First calculate a new discriminator using the address of where we are
        ...
        discriminator = __builtin_ptrauth_blend_discriminator(
            (void *)(((uintptr_t)uservaddr) + pageOffset), discriminator);
      }

      if (jop_key != 0 && si->si_ptrauth && !arm_user_jop_disabled()) {
#if CONFIG_SPTM
        pmap_batch_sign_user_ptr(rebaseLocation, (void *)value, key,
                                 discriminator, jop_key);
#else  /* CONFIG_SPTM */
        /*
         * these pointers are used in user mode. disable the kernel key
         * diversification so we can sign them for use in user mode.
         */
      ...
      // plus the slide. Regular pointer which needs to fit in 51-bits of value.
...
static kern_return_t
rebase_chainv4(uint8_t *page_content, uint16_t start_offset,
               uint32_t slide_amount,
               vm_shared_region_slide_info_entry_v4_t s_info) {
  const uint32_t last_page_offset = PAGE_SIZE_FOR_SR_SLIDE - sizeof(uint32_t);
...
static kern_return_t
vm_shared_region_slide_page_v4(vm_shared_region_slide_info_t si,
                               vm_offset_t vaddr, uint32_t pageIndex) {
  ...
  if (page_entry & DYLD_CACHE_SLIDE4_PAGE_USE_EXTRA) {
    ...
    do {
      ...
      info = page_extras[chain_index];
      page_start_offset = (uint16_t)((info & DYLD_CACHE_SLIDE4_PAGE_INDEX)
                                     << DYLD_CACHE_SLIDE_PAGE_OFFSET_SHIFT);

      kr =
          rebase_chainv4(page_content, page_start_offset, slide_amount, s_info);
      if (kr != KERN_SUCCESS) {
        return KERN_FAILURE;
      }
...
static kern_return_t vm_shared_region_slide_page_v5(
    vm_shared_region_slide_info_t si, vm_offset_t vaddr,
    __unused mach_vm_offset_t uservaddr, uint32_t pageIndex,
#if !__has_feature(ptrauth_calls)
    __unused
  ...
  do {
    ...
    // A pointer is one of :
    // {
    //   uint64_t    runtimeOffset   : 34,   // offset from the start of the
    //   shared cache
    //               high8           :  8,
    //               unused          : 10,
    ...
    //               diversity       : 16,
    //               addrDiv         :  1,
    //               keyIsData       :  1,   // implicitly always the 'A' key.
    //               0 -> IA.  1 -> DA next            : 11,   // 8-byte stide
    //               auth            :  1;   // == 1
    // }
    ...
    // The new value for a rebase is the low 34-bits of the threaded value plus
    // the base plus slide.
    ...
    if (isAuthenticated) {
      ...
      // the value already has the correct low bits, so just add in the high8 if
      // it exists
      value += high8;
      memcpy(rebaseLocation, &value, sizeof(value));
    }
...
kern_return_t vm_shared_region_slide_page(vm_shared_region_slide_info_t si,
                                          vm_offset_t vaddr,
  ...
  case 5:
    return vm_shared_region_slide_page_v5(si, vaddr, uservaddr, pageIndex,
                                          jop_key);
  default:
    return KERN_FAILURE;
  }
 ...
 */
static void _vm_commpage_init(ipc_port_t *handlep, vm_map_size_t size) {
  vm_named_entry_t mem_entry;
  vm_map_t new_map;

  SHARED_REGION_TRACE_DEBUG(("commpage: -> _init(0x%llx)\n", (long long)size));
 ...
 */
void vm_commpage_text_init(void) {
  SHARED_REGION_TRACE_DEBUG(("commpage text: ->init()\n"));
#if defined(__i386__) || defined(__x86_64__)
  /* create the 32 bit comm text page */
  unsigned int offset = (random() % _PFZ32_SLIDE_RANGE)
                        << PAGE_SHIFT; /* restricting to 32bMAX-2PAGE */
  ...
  offset = (random() % _PFZ64_SLIDE_RANGE)
           << PAGE_SHIFT; /* restricting sliding upto 2Mb range */
 ...
 */
kern_return_t vm_commpage_enter(vm_map_t map, task_t task, boolean_t is64bit) {
#if defined(__arm64__)
#pragma unused(is64bit)
  ...
  SHARED_REGION_TRACE_DEBUG(("commpage: -> enter(%p,%p)\n",
                             (void *)VM_KERNEL_ADDRPERM(map),
                             (void *)VM_KERNEL_ADDRPERM(task)));
  ...
  if (is64bit) {
    ...
    commpage_address = (vm_map_offset_t)(unsigned)_COMM_PAGE32_BASE_ADDRESS;
  ...
  kr = mach_vm_map_kernel(map, vm_sanitize_wrap_addr_ref(&commpage_address),
                          commpage_size, 0, vmk_flags, commpage_handle, 0,
                          FALSE, VM_PROT_READ, VM_PROT_READ, VM_INHERIT_SHARE);
  if (kr != KERN_SUCCESS) {
    SHARED_REGION_TRACE_ERROR(
        ("commpage: enter(%p,0x%llx,0x%llx) "
         "commpage %p mapping failed 0x%x\n",
         (void *)VM_KERNEL_ADDRPERM(map), (long long)commpage_address,
         (long long)commpage_size, (void *)VM_KERNEL_ADDRPERM(commpage_handle),
         kr));
  }
  ...
  kr =
      mach_vm_map_kernel(map, vm_sanitize_wrap_addr_ref(&commpage_text_address),
                         commpage_text_size, 0, vmk_flags, commpage_text_handle,
                         0, FALSE, VM_PROT_READ | VM_PROT_EXECUTE,
                         VM_PROT_READ | VM_PROT_EXECUTE, VM_INHERIT_SHARE);
  if (kr != KERN_SUCCESS) {
    SHARED_REGION_TRACE_ERROR(
        ("commpage text: enter(%p,0x%llx,0x%llx) "
         "commpage text %p mapping failed 0x%x\n",
         (void *)VM_KERNEL_ADDRPERM(map), (long long)commpage_text_address,
   ...
   */
  if (objc_size != 0) {
    kr = mach_vm_map_kernel(map, vm_sanitize_wrap_addr_ref(&objc_address),
                            objc_size, 0, vmk_flags, IPC_PORT_NULL, 0, FALSE,
                            VM_PROT_ALL, VM_PROT_ALL, VM_INHERIT_DEFAULT);
    if (kr != KERN_SUCCESS) {
      SHARED_REGION_TRACE_ERROR(("commpage: enter(%p,0x%llx,0x%llx) "
                                 "objc mapping failed 0x%x\n",
                                 (void *)VM_KERNEL_ADDRPERM(map),
                                 (long long)objc_address, (long long)objc_size,
                                 kr));
    }
  }

  SHARED_REGION_TRACE_DEBUG(("commpage: enter(%p,%p) <- 0x%x\n",
                             (void *)VM_KERNEL_ADDRPERM(map),
...
int vm_shared_region_slide(uint32_t slide, mach_vm_offset_t entry_start_address,
  ...
  SHARED_REGION_TRACE_DEBUG(
      ("vm_shared_region_slide: -> slide %#x, entry_start %#llx, entry_size "
       "%#llx, slide_start %#llx, slide_size %#llx\n",
  ...
  error = vm_shared_region_slide_mapping(
      sr, (user_addr_t)slide_start, slide_size, entry_start_address, entry_size,
      slid_mapping, slide, sr_file_control, prot);
  if (error) {
 ...
 * that to happen.
 */
void vm_shared_region_pivot(void) {
  vm_shared_region_t shared_region = NULL;
  ...
  queue_iterate(&vm_shared_region_queue, shared_region, vm_shared_region_t,
    ...
    if (shared_region->sr_timer_call) {
      ...
      SHARED_REGION_TRACE_DEBUG(("shared_region: pivot(%p): armed timer\n",
                                 (void *)VM_KERNEL_ADDRPERM(shared_region)));
    }
  }
 ...
 */
void vm_shared_region_reslide_stale(boolean_t driverkit) {
#if __has_feature(ptrauth_calls)
  vm_shared_region_t shared_region = NULL;
  ...
  queue_iterate(&vm_shared_region_queue, shared_region, vm_shared_region_t,
                sr_q) {
    assert(shared_region->sr_ref_count > 0);
    if (shared_region->sr_driverkit == driverkit && !shared_region->sr_stale &&
        shared_region->sr_reslide) {
      shared_region->sr_stale = TRUE;
 ...
 */
bool vm_shared_region_is_reslide(__unused struct task *task) {
...
/*
 * This is called from powermanagement code to let kernel know the current
 * source of power. 0 if it is external source (connected to power ) 1 if it is
 * internal power source ie battery
 */
void
#if XNU_TARGET_OS_OSX
...
void *vm_shared_region_root_dir(struct vm_shared_region *sr) {
  void *vnode;
