...
__header_always_inline kmem_flags_t ANYF(kma_flags_t flags) {
  return (kmem_flags_t)flags;
}

__attribute__((overloadable)) __header_always_inline kmem_flags_t
...
__abortlike static void __kmem_invalid_size_panic(vm_map_t map, vm_size_t size,
                                                  uint32_t flags) {
  panic("kmem(map=%p, flags=0x%x): invalid size %zd", map, flags, (size_t)size);
}

__abortlike static void __kmem_invalid_arguments_panic(const char *what,
                                                       vm_map_t map,
...
__abortlike static void __kmem_failed_panic(vm_map_t map, vm_size_t size,
                                            uint32_t flags, kern_return_t kr,
                                            const char *what) {
  panic("kmem_%s(%p, %zd, 0x%x): failed with %d", what, map, (size_t)size,
        flags, kr);
}

__abortlike static void __kmem_entry_not_found_panic(vm_map_t map,
                                                     vm_offset_t addr) {
  panic("kmem(map=%p) no entry found at %p", map, (void *)addr);
}

static inline vm_object_t __kmem_object(kmem_flags_t flags) {
...
static inline pmap_mapping_type_t __kmem_mapping_type(kmem_flags_t flags) {
...
static inline vm_size_t __kmem_guard_left(kmem_flags_t flags) {
  return (flags & KMEM_GUARD_FIRST) ? PAGE_SIZE : 0;
}

static inline vm_size_t __kmem_guard_right(kmem_flags_t flags) {
  return (flags & KMEM_GUARD_LAST) ? PAGE_SIZE : 0;
}

static inline vm_size_t __kmem_guard_size(kmem_flags_t flags) {
  return __kmem_guard_left(flags) + __kmem_guard_right(flags);
}

__pure2 static inline vm_size_t __kmem_entry_orig_size(vm_map_entry_t entry) {
  vm_object_t object = VME_OBJECT(entry);

  if (entry->vme_kernel_object) {
    return entry->vme_end - entry->vme_start - entry->vme_object_or_delta;
  } else {
...
__abortlike static void __mach_vm_range_overflow(mach_vm_offset_t addr,
                                                 mach_vm_offset_t size) {
  panic("invalid vm range: [0x%llx, 0x%llx + 0x%llx) wraps around", addr, addr,
        size);
}

__abortlike static void __mach_vm_range_invalid(mach_vm_offset_t min_address,
                                                mach_vm_offset_t max_address) {
  panic("invalid vm range: [0x%llx, 0x%llx) wraps around", min_address,
        max_address);
}
...
__attribute__((overloadable)) __header_always_inline bool
...
__attribute__((overloadable)) __header_always_inline bool
mach_vm_range_intersects(const struct mach_vm_range *r1,
...
__attribute__((overloadable)) __header_always_inline bool
mach_vm_range_intersects(const struct mach_vm_range *r1, mach_vm_offset_t addr,
                         mach_vm_offset_t size) {
...
bool kmem_range_id_contains(kmem_range_id_t range_id, vm_map_offset_t addr,
                            vm_map_size_t size) {
  return mach_vm_range_contains(&kmem_ranges[range_id],
                                vm_memtag_canonicalize_kernel(addr), size);
}

__abortlike static void kmem_range_invalid_panic(kmem_range_id_t range_id,
 ...
 */
static bool kmem_range_contains_fully(kmem_range_id_t range_id,
                                      vm_map_offset_t addr,
                                      vm_map_size_t size) {
  const struct mach_vm_range *r = &kmem_ranges[range_id];
  mach_vm_offset_t rmin, rmax;
  ...
  if (__improbable(result && ((addr + size < rmin) || (addr + size > rmax)))) {
    kmem_range_invalid_panic(range_id, addr, size);
  }
  return result;
}

vm_map_size_t kmem_range_id_size(kmem_range_id_t range_id) {
  return mach_vm_range_size(&kmem_ranges[range_id]);
}

kmem_range_id_t kmem_addr_get_range(vm_map_offset_t addr, vm_map_size_t size) {
...
bool kmem_is_ptr_range(vm_map_range_id_t range_id) {
  return (range_id >= KMEM_RANGE_ID_FIRST) &&
         (range_id <= KMEM_RANGE_ID_NUM_PTR);
}

__abortlike static void kmem_range_invalid_for_overwrite(vm_map_offset_t addr) {
  panic("Can't overwrite mappings (addr: %p) in kmem ptr ranges", (void *)addr);
}

mach_vm_range_t kmem_validate_range_for_overwrite(vm_map_offset_t addr,
                                                  vm_map_size_t size) {
...
__kmem_entry_validate_panic(vm_map_t map, vm_map_entry_t entry,
                            vm_offset_t addr, vm_size_t size, uint32_t flags,
                            kmem_guard_t guard) {
  const char *what = "???";
  ...
  panic("kmem(map=%p, addr=%p, size=%zd, flags=0x%x): "
        ...
        map, (void *)addr, size, flags, entry, what, guard.kmg_context);
}

static bool __kmem_entry_validate_guard(vm_map_entry_t entry, vm_offset_t addr,
                                        vm_size_t size, kmem_flags_t flags,
                                        kmem_guard_t guard) {
  if (entry->vme_atomic != guard.kmg_atomic) {
    return false;
  }
...
void kmem_entry_validate_guard(vm_map_t map, vm_map_entry_t entry,
...
__abortlike static void __kmem_entry_validate_object_panic(vm_map_t map,
  ...
  panic("kmem(map=%p, flags=0x%x): entry %p %s for the %s object", map, flags,
        entry, verb, what);
}

static bool __kmem_entry_validate_object(vm_map_entry_t entry,
                                         kmem_flags_t flags) {
  if (entry->is_sub_map) {
    return false;
  }
...
vm_size_t kmem_size_guard(vm_map_t map, vm_offset_t addr, kmem_guard_t guard) {
...
static inline uint16_t kmem_hash_backtrace(void *fp) {
  uint64_t bt_count;
  uintptr_t bt[8] = {};
...
kmem_range_id_t kmem_adjust_range_id(uint32_t hash) {
  return (kmem_range_id_t)(KMEM_RANGE_ID_PTR_0 +
                           (hash & KMEM_RANGE_MASK) % kmem_ptr_ranges);
}

static bool kmem_use_sprayqtn(kma_flags_t kma_flags, vm_map_size_t map_size,
                              vm_offset_t mask) {
  /*
   ...
   */
#if DEBUG || DEVELOPMENT
  btref_get_flags_t flags = (kma_flags & KMA_NOPAGEWAIT) ? BTREF_GET_NOWAIT : 0;
  ...
  return (kma_flags & KMA_SPRAYQTN) || (map_size > KMEM_GOBJ_THRESHOLD) ||
         ((kma_flags & KMA_GUARD_FIRST) && (mask > PAGE_MASK));
}

static void kmem_apply_security_policy(vm_map_t map, kma_flags_t kma_flags,
                                       kmem_guard_t guard,
                                       vm_map_size_t map_size, vm_offset_t mask,
  ...
  if (kma_flags & (KMA_DATA | KMA_DATA_SHARED)) {
    ...
    if (kma_flags & KMA_DATA) {
      ...
      range_id = kmem_needs_data_share_range() ? KMEM_RANGE_ID_DATA_SHARED
                                               : KMEM_RANGE_ID_DATA;
    }
...
static kmem_return_t kmem_alloc_guard_internal(
    vm_map_t map, vm_size_t size, vm_offset_t mask, kma_flags_t flags,
    kmem_guard_t guard,
  ...
  assert((flags & (KMA_DATA | KMA_DATA_SHARED)) !=
         (KMA_DATA | KMA_DATA_SHARED));
  ...
  if (size == 0 || (size >> VM_KERNEL_POINTER_SIGNIFICANT_BITS) ||
  ...
  skip_guards = (flags & (KMA_KOBJECT | KMA_COMPRESSOR)) || map->never_faults;
  ...
  if (!(flags & (KMA_VAONLY | KMA_PAGEABLE))) {
    if (alloc_pages) {
      kmr.kmr_return = alloc_pages(fill_size, flags, &wired_page_list);
    } else {
      kmr.kmr_return =
          vm_page_alloc_list(atop(fill_size), flags, &wired_page_list);
    }
  ...
  kmr.kmr_return = vm_map_find_space(map, 0, map_size, mask, vmk_flags, &entry);
  ...
  if (guard_left || guard_right || wired_page_list) {
    ...
    if (guard_right) {
      vm_page_insert(guard_right, object, offset + fill_start + fill_size);
    ...
    if (wired_page_list) {
      kernel_memory_populate_object_and_unlock(
          object, map_addr + fill_start, offset + fill_start, fill_size,
  ...
  if (!(flags & KMA_VAONLY) && (flags & KMA_TAG)) {
    kmr.kmr_ptr = vm_memtag_generate_and_store_tag(
        (caddr_t)kmr.kmr_address + fill_start, fill_size);
...
#if DEBUG || DEVELOPMENT
  VM_DEBUG_CONSTANT_EVENT(vm_kern_request, DBG_VM_KERN_REQUEST, DBG_FUNC_END, 0,
                          0, 0, 0);
#endif /* DEBUG || DEVELOPMENT */
...
__mockable kmem_return_t kmem_alloc_guard(vm_map_t map, vm_size_t size,
                                          vm_offset_t mask, kma_flags_t flags,
                                          kmem_guard_t guard) {
  return kmem_alloc_guard_internal(map, size, mask, flags, guard, NULL);
}

kmem_return_t kmem_alloc_contig_guard(vm_map_t map, vm_size_t size,
                                      vm_offset_t mask, ppnum_t max_pnum,
                                      ppnum_t pnum_mask, kma_flags_t flags,
                                      kmem_guard_t guard) {
  __auto_type alloc_pages =
      ^(vm_size_t fill_size, kma_flags_t kma_flags, vm_page_t *pages) {
        return cpm_allocate(fill_size, pages, max_pnum, pnum_mask, FALSE,
                            kma_flags);
      };
...
kmem_return_t kmem_suballoc(vm_map_t parent, mach_vm_offset_t *addr,
                            vm_size_t size, vm_map_create_options_t vmc_options,
                            int vm_flags, kms_flags_t flags, vm_tag_t tag) {
  ...
  kmr.kmr_return =
      vm_map_enter(parent, &map_addr, size, 0, vmk_flags, (vm_object_t)map, 0,
                   FALSE, VM_PROT_DEFAULT, VM_PROT_ALL, VM_INHERIT_DEFAULT);

  if (kmr.kmr_return != KERN_SUCCESS) {
    if (flags & KMS_NOFAIL) {
      panic("kmem_suballoc(map=%p, size=%zd) failed with %d", parent, size,
            kmr.kmr_return);
    }
   ...
   */
  if (*addr != 0 && parent == kernel_map && startup_phase > STARTUP_SUB_KMEM) {
...
__exported kern_return_t kmem_alloc_external(vm_map_t map, vm_offset_t *addrp,
                                             vm_size_t size);
kern_return_t kmem_alloc_external(vm_map_t map, vm_offset_t *addrp,
                                  vm_size_t size) {
...
__exported kern_return_t kmem_alloc_kobject_external(vm_map_t map,
                                                     vm_offset_t *addrp,
                                                     vm_size_t size);
kern_return_t kmem_alloc_kobject_external(vm_map_t map, vm_offset_t *addrp,
                                          vm_size_t size) {
  if (size && (size >> VM_KERNEL_POINTER_SIGNIFICANT_BITS) == 0) {
...
__exported kern_return_t kmem_alloc_pageable_external(vm_map_t map,
                                                      vm_offset_t *addrp,
                                                      vm_size_t size);
kern_return_t kmem_alloc_pageable_external(vm_map_t map, vm_offset_t *addrp,
                                           vm_size_t size) {
  if (size && (size >> VM_KERNEL_POINTER_SIGNIFICANT_BITS) == 0) {
    return kmem_alloc(map, addrp, size, KMA_PAGEABLE | KMA_DATA_SHARED,
                      vm_tag_bt());
  }
  /* Maintain ABI compatibility: invalid sizes used to be allowed */
  return size ? KERN_NO_SPACE : KERN_INVALID_ARGUMENT;
...
mach_vm_allocate_kernel_sanitize(vm_map_t map, mach_vm_offset_ut addr_u,
  ...
  if (vmk_flags.vmf_fixed) {
    result = vm_sanitize_addr_size(
        addr_u, size_u, VM_SANITIZE_CALLER_VM_ALLOCATE_FIXED, map,
    ...
    *map_addr = 0;
    result =
        vm_sanitize_size(0, size_u, VM_SANITIZE_CALLER_VM_ALLOCATE_ANYWHERE,
                         map, VM_SANITIZE_FLAGS_SIZE_ZERO_SUCCEEDS, map_size);
    if (__improbable(result != KERN_SUCCESS)) {
      return result;
    }
  }
...
kern_return_t mach_vm_allocate_kernel(vm_map_t map, mach_vm_offset_ut *addr_u,
                                      mach_vm_size_ut size_u,
                                      vm_map_kernel_flags_t vmk_flags) {
  vm_map_offset_t map_addr;
  vm_map_size_t map_size;
  ...
  result = mach_vm_allocate_kernel_sanitize(map, *addr_u, size_u, vmk_flags,
                                            &map_addr, &map_size);
  if (__improbable(result != KERN_SUCCESS)) {
  ...
  result = vm_map_enter(map, &map_addr, map_size, (vm_map_offset_t)0, vmk_flags,
...
static void
kernel_memory_populate_pmap_enter(vm_object_t object, vm_address_t addr,
                                  vm_object_offset_t offset, vm_page_t mem,
                                  vm_prot_t prot, int pe_flags,
  ...
  pe_result = pmap_enter_options(
      kernel_pmap, addr + offset, VM_PAGE_GET_PHYS_PAGE(mem), prot,
      VM_PROT_NONE, pe_flags, /* wired */ TRUE, pe_options, NULL, mapping_type);
  ...
  if (pe_result == KERN_RESOURCE_SHORTAGE) {
    ...
    pe_result = pmap_enter_options(kernel_pmap, addr + offset,
                                   VM_PAGE_GET_PHYS_PAGE(mem), prot,
                                   VM_PROT_NONE, pe_flags, /* wired */ TRUE,
                                   pe_options, NULL, mapping_type);
...
void kernel_memory_populate_object_and_unlock(
    vm_object_t object, /* must be locked */
    vm_address_t addr, vm_offset_t offset, vm_size_t size, vm_page_t page_list,
    kma_flags_t flags, vm_tag_t tag, vm_prot_t prot,
    pmap_mapping_type_t mapping_type) {
  vm_page_t mem;
  int pe_flags;
  ...
  for (vm_object_offset_t pg_offset = 0; pg_offset < size;
     ...
     */
    kernel_memory_populate_pmap_enter(object, addr, pg_offset, mem, prot,
                                      pe_flags, mapping_type);
...
kern_return_t kernel_memory_populate(vm_offset_t addr, vm_size_t size,
  ...
  if (kr == KERN_SUCCESS) {
    vm_object_lock(object);
    kernel_memory_populate_object_and_unlock(
        object, addr, addr, size, page_list, flags, tag, VM_PROT_DEFAULT,
        __kmem_mapping_type(ANYF(flags)));
  }
...
void kernel_memory_depopulate(vm_offset_t addr, vm_size_t size,
                              kma_flags_t flags, vm_tag_t tag) {
  vm_object_t object = __kmem_object(ANYF(flags));
...
__abortlike static void
__kmem_realloc_invalid_object_size_panic(vm_map_t map, vm_address_t address,
                                         vm_size_t size, vm_map_entry_t entry) {
  vm_object_t object = VME_OBJECT(entry);
...
__abortlike static void
__kmem_realloc_invalid_pager_panic(vm_map_t map, vm_address_t address,
                                   vm_size_t size, vm_map_entry_t entry) {
  vm_object_t object = VME_OBJECT(entry);
  ...
  panic("kmem_realloc(map=%p, addr=%p, size=%zd, entry=%p): "
        ...
        map, (void *)address, (size_t)size, entry, object, pager, pager_created,
        pager_initialized, pager_ready);
}

static kmem_return_t
kmem_realloc_shrink_guard(vm_map_t map, vm_offset_t req_oldaddr,
                          vm_size_t req_oldsize, vm_size_t req_newsize,
                          kmr_flags_t flags, kmem_guard_t guard,
                          vm_map_entry_t entry) {
...
#if KASAN_CLASSIC
  if (flags & KMR_KASAN_GUARD) {
    kasan_poison_range(oldaddr + newsize, oldsize - newsize, ASAN_VALID);
  }
#endif
#if KASAN_TBI
  if (flags & KMR_TAG) {
    kasan_tbi_mark_free_space((caddr_t)req_oldaddr + newsize,
                              oldsize - newsize);
  }
#endif /* KASAN_TBI */
#endif /* KASAN */
  (void)vm_map_remove_and_unlock(map, oldaddr + newsize, oldaddr + oldsize,
  ...
  if (flags & KMR_KOBJECT) {
    if (flags & KMR_GUARD_LAST) {
      ...
      kernel_memory_depopulate(oldaddr + newsize - PAGE_SIZE, PAGE_SIZE, dflags,
                               guard.kmg_tag);
    }
  } else {
    vm_page_t guard_right = VM_PAGE_NULL;
    ...
    if (object->vo_size != oldsize) {
      __kmem_realloc_invalid_object_size_panic(map, req_oldaddr,
                                               req_oldsize + delta, entry);
    }
...
kmem_return_t kmem_realloc_guard(vm_map_t map, vm_offset_t req_oldaddr,
                                 vm_size_t req_oldsize, vm_size_t req_newsize,
                                 kmr_flags_t flags, kmem_guard_t guard) {
  vm_object_t object;
  vm_size_t oldsize;
  ...
  if (!guard.kmg_atomic) {
    if (!(flags & (KMR_DATA | KMR_DATA_SHARED))) {
      __kmem_invalid_arguments_panic("realloc", map, req_oldaddr, req_oldsize,
                                     flags);
    }
  ...
  if (req_oldaddr == 0ul) {
    kmem_return_t ret =
        kmem_alloc_guard(map, req_newsize, 0, (kma_flags_t)flags, guard);
    vmlp_api_end(KMEM_REALLOC_GUARD, ret.kmr_return);
    return ret;
  }

  if (req_newsize == 0ul) {
    kmem_free_guard(map, req_oldaddr, req_oldsize, (kmf_flags_t)flags, guard);
  ...
  if (oldsize < newsize) {
#if DEBUG || DEVELOPMENT
    VM_DEBUG_CONSTANT_EVENT(vm_kern_request, DBG_VM_KERN_REQUEST,
                            DBG_FUNC_START, newsize - oldsize, 0, 0, 0);
#endif /* DEBUG || DEVELOPMENT */
    kmr.kmr_return = vm_page_alloc_list(atop(newsize - oldsize),
                                        (kma_flags_t)flags, &page_list);
    if (kmr.kmr_return == KERN_SUCCESS) {
      kmem_apply_security_policy(map, (kma_flags_t)flags, guard, newsize, 0,
                                 &vmk_flags, true);
      kmr.kmr_return =
          vm_map_find_space(map, 0, newsize, 0, vmk_flags, &newentry);
    }
    if (__improbable(kmr.kmr_return != KERN_SUCCESS)) {
      if (flags & KMR_REALLOCF) {
        kmem_free_guard(map, req_oldaddr, req_oldsize,
                        flags & (KMF_TAG | KMF_GUARD_FIRST | KMF_GUARD_LAST |
                                 KMF_KASAN_GUARD),
                        guard);
      }
      if (page_list) {
...
again:
  ...
  if (__kmem_entry_orig_size(oldentry) != req_oldsize) {
    __kmem_realloc_invalid_object_size_panic(map, req_oldaddr,
                                             req_oldsize + delta, oldentry);
  }

  if (oldsize == newsize) {
    kmr.kmr_address = req_oldaddr;
    if (oldentry->vme_kernel_object) {
      oldentry->vme_object_or_delta = delta + (-req_newsize & PAGE_MASK);
    } else {
  ...
  if (newsize < oldsize) {
    kmem_return_t ret = kmem_realloc_shrink_guard(
        map, req_oldaddr, req_oldsize, req_newsize, flags, guard, oldentry);
    vmlp_api_end(KMEM_REALLOC_GUARD, kmr.kmr_return);
    return ret;
  }
  ...
  if (flags & KMR_KOBJECT) {
...
#if KASAN
    newentry->vme_object_or_delta = delta + (-req_newsize & PAGE_MASK);
#endif /* KASAN */
  } else {
    if (object->pager_created || object->pager) {
       ...
       */
      __kmem_realloc_invalid_pager_panic(map, req_oldaddr, req_oldsize + delta,
                                         oldentry);
    }
    if (object->vo_size != oldsize) {
      __kmem_realloc_invalid_object_size_panic(map, req_oldaddr,
  ...
  if (flags & KMR_KOBJECT) {
    ...
    pmap_protect(kernel_pmap, oldaddr, oldaddr + oldsize - guard_right_size,
                 VM_PROT_NONE);

    for (vm_object_offset_t offset = 0; offset < oldsize - guard_right_size;
         offset += PAGE_SIZE_64) {
      vm_page_t mem;
      ...
      mem->vmp_busy = true;
      vm_page_remove(mem, true);
      vm_page_insert_wired(mem, object, newaddr + offset, guard.kmg_tag);
      mem->vmp_busy = false;

      kernel_memory_populate_pmap_enter(object, newaddr, offset, mem,
                                        VM_PROT_DEFAULT, 0, mapping_type);
    }

    kernel_memory_populate_object_and_unlock(
        object, newaddr + oldsize - guard_right_size,
        newoffs - guard_right_size, newsize - oldsize, page_list,
        (kma_flags_t)flags, guard.kmg_tag, VM_PROT_DEFAULT, mapping_type);
  } else {
    vm_page_t guard_right = VM_PAGE_NULL;
    ...
    if (flags & KMR_FREEOLD) {
      ...
      for (vm_object_offset_t offset = 0; offset < oldsize - guard_right_size;
        ...
        assertf(!VM_PAGE_PAGEABLE(mem), "mem %p qstate %d", mem,
                mem->vmp_q_state);
        if (vm_page_is_guard(mem)) {
          /* guard pages are not wired */
        } else {
          assertf(VM_PAGE_WIRED(mem), "mem %p qstate %d wirecount %d", mem,
                  mem->vmp_q_state, mem->vmp_wire_count);
          assertf(mem->vmp_wire_count >= 1, "mem %p wirecount %d", mem,
                  mem->vmp_wire_count);
          mem->vmp_wire_count++;
        }
      }
    ...
    for (vm_object_offset_t offset = oldsize - guard_right_size;
         offset < newsize - guard_right_size; offset += PAGE_SIZE_64) {
  ...
  if (flags & KMR_FREEOLD) {
    ...
    (void)vm_map_remove_and_unlock(map, oldaddr, oldaddr + oldsize, vmr_flags,
                                   guard);
  } else {
    vm_map_unlock(map);
  }
  ...
  if ((flags & KMR_KOBJECT) == 0) {
     ...
     */
    kr = vm_map_wire_kernel(map, vm_sanitize_wrap_addr(newaddr),
    ...
    if (flags & KMR_FREEOLD) {
      ...
      for (vm_object_offset_t offset = 0; offset < oldsize - guard_right_size;
        ...
        if (vm_page_is_guard(mem)) {
          ...
          assertf(VM_PAGE_WIRED(mem), "mem %p qstate %d wirecount %d", mem,
                  mem->vmp_q_state, mem->vmp_wire_count);
          assertf(mem->vmp_wire_count >= 2, "mem %p wirecount %d", mem,
                  mem->vmp_wire_count);
          mem->vmp_wire_count--;
...
kern_return_t
mach_vm_map_kernel(vm_map_t target_map, mach_vm_offset_ut *address,
                   mach_vm_size_ut initial_size, mach_vm_offset_ut mask,
                   vm_map_kernel_flags_t vmk_flags, ipc_port_t port,
                   memory_object_offset_ut offset, boolean_t copy,
                   vm_prot_ut cur_protection, vm_prot_ut max_protection,
  ...
  return vm_map_enter_mem_object(target_map, address, initial_size, mask,
                                 vmk_flags, port, offset, copy, cur_protection,
                                 max_protection, inheritance, NULL, 0);
}

kern_return_t
mach_vm_remap_new_kernel(vm_map_t target_map, mach_vm_offset_ut *address,
                         mach_vm_size_ut size, mach_vm_offset_ut mask,
  ...
  return vm_map_remap(target_map, address, size, mask, vmk_flags, src_map,
                      memory_address, copy, cur_protection, max_protection,
                      inheritance);
}
...
__abortlike static void
__kmem_free_invalid_object_size_panic(vm_map_t map, vm_address_t address,
                                      vm_size_t size, vm_map_entry_t entry) {
  vm_object_t object = VME_OBJECT(entry);
  vm_size_t objsize = __kmem_entry_orig_size(entry);
...
__mockable vm_size_t kmem_free_guard(vm_map_t map, vm_offset_t req_addr,
                                     vm_size_t req_size, kmf_flags_t flags,
                                     kmem_guard_t guard) {
  vmr_flags_t vmr_flags = VM_MAP_REMOVE_KUNWIRE;
  ...
  if (flags & KMF_GUESS_SIZE) {
     ...
     */
    __kmem_free_invalid_object_size_panic(map, req_addr, req_size + delta,
                                          entry);
  }
#endif /* KASAN */
#if KASAN_CLASSIC
  ...
  return vm_map_remove_and_unlock(map, addr, addr + size, vmr_flags, guard)
             .kmr_size -
         delta;
}

__exported void kmem_free_external(vm_map_t map, vm_offset_t addr,
                                   vm_size_t size);
void kmem_free_external(vm_map_t map, vm_offset_t addr, vm_size_t size) {
  if (size) {
...
static uint32_t kmem_guard_count(struct kmem_sizeclass *kmem) {
  return kmem->ks_num_elem * KMEM_NUM_GUARDS / KMEM_NUM_SLOTS;
}

static uint32_t kmem_guard_and_quarantine_count(struct kmem_sizeclass *kmem) {
  return kmem->ks_num_elem * (KMEM_NUM_GUARDS + KMEM_NUM_QUARANTINE) /
         KMEM_NUM_SLOTS;
}

static uint32_t kmem_get_front(kmem_range_id_t range_id, bool from_right) {
  assert((range_id >= KMEM_RANGE_ID_FIRST) &&
         (range_id <= KMEM_RANGE_ID_NUM_PTR));
  return (range_id - KMEM_RANGE_ID_FIRST) * 2 + from_right;
}

static inline uint32_t kmem_slot_idx_to_bit(uint32_t slot_idx,
                                            uint32_t size_idx __unused) {
  assert(slot_idx < kmem_size_array[size_idx].ks_num_elem);
  return 1ull << slot_idx;
}

static uint32_t kmem_get_idx_from_size(vm_map_size_t size) {
  assert(size >= KMEM_MIN_SIZE && size <= KMEM_MAX_SIZE);
  return kmem_log2down(size - 1) - KMEM_START_IDX + 1;
}

__abortlike static void kmem_invalid_size_idx(uint32_t idx) {
  panic("Invalid sizeclass idx %u", idx);
}

static vm_map_size_t kmem_get_size_from_idx(uint32_t idx) {
...
static inline uint16_t kmem_get_page_idx(struct kmem_page_meta *meta) {
  uint8_t page_marker = meta->km_page_marker;

  return (page_marker == KMEM_META_PRIMARY) ? 0 : meta->km_page_idx;
}

__abortlike static void kmem_invalid_chunk_len(struct kmem_page_meta *meta) {
  panic("Reading free chunks for meta %p where marker != KMEM_META_PRIMARY",
        meta);
}

static inline uint16_t kmem_get_chunk_len(struct kmem_page_meta *meta) {
...
kmem_invalid_free_chunk_len(struct kmem_page_meta *meta) {
  panic("Reading free chunks for meta %p where marker != KMEM_META_FREE", meta);
}

static inline uint32_t kmem_get_free_chunk_len(struct kmem_page_meta *meta) {
 ...
 */
static struct kmem_page_meta *kmem_addr_to_meta(vm_map_offset_t addr,
...
kmem_addr_to_meta_start(vm_address_t addr, vm_map_range_id_t range_id,
...
__startup_func static void kmem_init_meta_front(struct kmem_page_meta *meta,
                                                kmem_range_id_t range_id,
                                                bool from_right) {
  kernel_memory_populate(trunc_page((vm_map_offset_t)meta), PAGE_SIZE,
                         KMA_KOBJECT | KMA_ZERO | KMA_NOFAIL,
...
__startup_func static void kmem_metadata_init(void) {
  for (kmem_range_id_t i = KMEM_RANGE_ID_FIRST; i <= kmem_ptr_ranges; i++) {
    ...
    kmem_meta_map[i] =
        kmem_suballoc(
            kernel_map, &addr, kmem_meta_size,
            VM_MAP_CREATE_NEVER_FAULTS | VM_MAP_CREATE_DISABLE_HOLELIST,
            VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,
            KMS_PERMANENT | KMS_NOFAIL | KMS_NOSOFTLIMIT, VM_KERN_MEMORY_OSFMK)
            .kmr_submap;
    ...
    meta = kmem_addr_to_meta(kmem_ranges[i].max_address, i, &addr, &meta_idx);
    kmem_init_meta_front(meta, i, 1);
  }
}

__startup_func static void kmem_init_front_head(struct kmem_sizeclass *ks,
                                                uint32_t front) {
...
__startup_func static void kmem_sizeclass_init(void) {
  for (uint32_t i = 0; i < KMEM_NUM_SIZECLASS; i++) {
    ...
    ks->ks_num_chunk =
        roundup(KMEM_NUM_SLOTS * ks->ks_size, KMEM_CHUNK_SIZE_MIN) /
        KMEM_CHUNK_SIZE_MIN;
 ...
 * set up.
 */
__startup_func static void kmem_crypto_init(void) {
  ...
  for (uint32_t i = 0; i < KMEM_NUM_SIZECLASS; i++) {
    ...
    zpercpu_foreach(ctx, ks->ks_rng_ctx) { crypto_random_kmem_init(ctx); }
  }
}
STARTUP(EARLY_BOOT, STARTUP_RANK_MIDDLE, kmem_crypto_init);

__abortlike static void kmem_validate_slot_panic(vm_map_offset_t addr,
                                                 struct kmem_page_meta *meta,
                                                 uint32_t slot_idx,
                                                 uint32_t size_idx) {
  ...
  if (meta->km_sizeclass != size_idx) {
    panic("Metadata's (%p) sizeclass (%u != %u) changed during deletion", meta,
...
__abortlike static void kmem_invalid_slot_for_addr(mach_vm_range_t slot,
                                                   vm_map_offset_t start,
                                                   vm_map_offset_t end) {
  panic("Invalid kmem ptr slot [%p:%p] for allocation [%p:%p]",
        (void *)slot->min_address, (void *)slot->max_address, (void *)start,
        (void *)end);
}

void kmem_validate_slot(vm_map_offset_t addr, struct kmem_page_meta *meta,
                        uint32_t size_idx, uint32_t slot_idx) {
  if ((meta->km_page_marker != KMEM_META_PRIMARY) ||
      (meta->km_sizeclass != size_idx) ||
...
static void kmem_validate_slot_initial(mach_vm_range_t slot,
                                       vm_map_offset_t start,
                                       vm_map_offset_t end,
                                       struct kmem_page_meta *meta,
                                       uint32_t size_idx, uint32_t slot_idx) {
...
uint32_t kmem_addr_get_slot_idx(vm_map_offset_t start, vm_map_offset_t end,
                                vm_map_range_id_t range_id,
                                struct kmem_page_meta **meta,
                                uint32_t *size_idx, mach_vm_range_t slot) {
  vm_map_offset_t chunk_start;
...
static bool kmem_populate_needed(vm_offset_t from, vm_offset_t to) {
#if KASAN
...
static void kmem_populate_meta_locked(vm_offset_t from, vm_offset_t to) {
  vm_offset_t page_addr = trunc_page(from);
...
__abortlike static void
kmem_invalid_meta_panic(struct kmem_page_meta *meta, uint32_t slot_idx,
...
__abortlike static void kmem_slot_has_entry_panic(vm_map_entry_t entry,
                                                  vm_map_offset_t addr) {
  panic("Entry (%p) already exists for addr (%p) being returned", entry,
        (void *)addr);
}

__abortlike static void kmem_slot_not_found(struct kmem_page_meta *meta,
                                            uint32_t slot_idx) {
 ...
 */
__startup_func uint16_t kmem_get_random16(uint16_t upper_limit) {
...
static uint32_t kmem_get_nth_free_slot(struct kmem_page_meta *meta, uint32_t n,
                                       uint32_t bitmap) {
...
static uint32_t kmem_get_next_slot(struct kmem_page_meta *meta,
                                   struct kmem_sizeclass sizeclass,
                                   uint32_t bitmap) {
 ...
 */
static vm_map_offset_t kmem_get_addr_from_meta(struct kmem_page_meta *meta,
                                               vm_map_range_id_t range_id,
                                               struct kmem_sizeclass sizeclass,
                                               vm_map_entry_t *entry) {
  vm_map_offset_t addr;
  ...
  if ((slot_idx >= sizeclass.ks_num_elem) || (meta->km_sizeclass != size_idx) ||
...
__abortlike static void kmem_range_out_of_va(kmem_range_id_t range_id,
                                             uint32_t num_chunks) {
  panic("No more VA to allocate %u chunks in range %u", num_chunks, range_id);
}

static void kmem_init_allocated_chunk(struct kmem_page_meta *meta,
                                      struct kmem_sizeclass sizeclass,
                                      uint32_t size_idx) {
...
static uint32_t
kmem_get_additional_meta(struct kmem_page_meta *meta, uint32_t meta_req,
                         bool from_right,
...
static struct kmem_page_meta *kmem_get_new_chunk(vm_map_range_id_t range_id,
                                                 bool from_right,
                                                 uint32_t size_idx) {
  ...
  for (;;) {
    ...
    meta_num =
        kmem_get_additional_meta(meta, meta_req, from_right, &adj_free_meta);
    ...
    if (adj_free_meta) {
      if (adj_free_meta->km_page_marker != KMEM_META_FREE ||
          kmem_get_free_chunk_len(adj_free_meta) != meta_req - meta_num) {
        return NULL;
      }
    }
...
static void kmem_requeue_meta(struct kmem_page_meta *meta,
 ...
 */
__abortlike static void kmem_invalid_chunk_num(uint32_t chunks) {
  panic("Invalid number of chunks %u\n", chunks);
}

static uint32_t kmem_get_size_idx_for_chunks(uint32_t chunks) {
...
static void kmem_clear_meta_range(struct kmem_page_meta *meta, uint32_t count) {
  bzero(meta, count * sizeof(struct kmem_page_meta));
}

static void kmem_check_meta_range_is_clear(struct kmem_page_meta *meta,
                                           uint32_t count) {
#if MACH_ASSERT
 ...
 */
static void kmem_init_free_chunk(struct kmem_page_meta *meta,
                                 uint32_t num_chunks, uint32_t front) {
  struct kmem_sizeclass *sizeclass;
...
static struct kmem_page_meta *
kmem_get_free_chunk_from_list(struct kmem_sizeclass *org_sizeclass,
                              uint32_t size_idx, uint32_t front) {
  struct kmem_sizeclass *sizeclass;
...
kern_return_t kmem_locate_space(vm_map_size_t size, vm_map_range_id_t range_id,
                                bool from_right, vm_map_offset_t *start_inout,
 ...
 */
static bool kmem_meta_is_from_right(kmem_range_id_t range_id,
                                    struct kmem_page_meta *meta) {
  struct kmem_page_meta *metaf = kmem_meta_hwm[kmem_get_front(range_id, 0)];
  __assert_only struct kmem_page_meta *metab =
      kmem_meta_hwm[kmem_get_front(range_id, 1)];
  struct kmem_page_meta *meta_base = kmem_meta_base[range_id];
...
static void kmem_free_chunk(kmem_range_id_t range_id,
                            struct kmem_page_meta *meta, bool from_right) {
...
static void kmem_free_slot(kmem_range_id_t range_id, mach_vm_range_t slot) {
  struct kmem_page_meta *meta;
  vm_map_offset_t chunk_start;
  ...
  if (num_elem == chunk_elem) {
     ...
     */
    uint32_t front =
        kmem_get_front(range_id, kmem_meta_is_from_right(range_id, meta));

    kmem_requeue_meta(meta, &sizeclass->ks_partial_head[front]);
  }
}

void kmem_free_space(vm_map_offset_t start, vm_map_offset_t end,
                     vm_map_range_id_t range_id, mach_vm_range_t slot) {
...
/*
 * The default percentage of memory that can be mlocked is scaled based on the
 * total amount of memory in the system. These percentages are caclulated
 ...
 * pressure.
 */
static vm_map_size_t wire_limit_percents[] = {80, 80, 80, 80, 82,
                                              85, 88, 91, 94, 97};
#else
static vm_map_size_t wire_limit_percents[] = {70, 73, 76, 79, 82,
                                              85, 88, 91, 94, 97};
#endif /* CONFIG_JETSAM */
 ...
 */
static void kmem_set_user_wire_limits(void) {
  uint64_t available_mem_log;
  uint64_t max_wire_percent;
  size_t wire_limit_percents_length =
      sizeof(wire_limit_percents) / sizeof(vm_map_size_t);
  vm_map_size_t limit;
...
__startup_data struct kmem_range_startup_spec kmem_claims[KMEM_MAX_CLAIMS] = {};
 ...
 */
__startup_func static vm_map_size_t
...
__startup_func void
kmem_range_startup_init(struct kmem_range_startup_spec *sp) {
...
static vm_offset_t kmem_fuzz_start(void) {
 ...
 */
__startup_func void kmem_shuffle(uint16_t *shuffle_buf, uint16_t count) {
...
__startup_func static void kmem_shuffle_claims(void) {
...
__startup_func static void kmem_readjust_ranges(uint32_t cur_idx) {
  assert(cur_idx != 0);
...
__startup_func static void kmem_add_ptr_claims(void) {
...
__startup_func static void kmem_add_extra_claims(void) {
  ...
  for (uint32_t i = 0; i < kmem_claim_count; i++) {
    ...
    total_claims +=
        kmem_claim_to_allocation_size(sp_i.kc_size, /* known_last */ false);
  }
   ...
   */
  data_range_size =
      largest_free_size - sprayqtn_allocation_size - ptr_total_allocation_size;
  ...
  if (kmem_needs_data_share_range()) {
     ...
     */
    shared_data_range_size = data_range_size = trunc_page(data_range_size / 2);
  } else {
  ...
  shared_data_range_size =
      shared_data_range_size
          ? kmem_allocation_to_claim_size(shared_data_range_size)
          : 0;

  /* Check: our allocations should all still fit in the free space */
  assert(sprayqtn_allocation_size + ptr_total_allocation_size +
             kmem_claim_to_allocation_size(data_range_size,
                                           /* known_last */ false) +
             kmem_claim_to_allocation_size(shared_data_range_size,
                                           /* known_last */ false) <=
         largest_free_size);
...
__startup_func static void kmem_scramble_ranges(void) {
   ...
   */
  kmem_ranges[KMEM_RANGE_ID_NONE].min_address =
      MAX(kernel_map->min_offset, VM_MAP_PAGE_SIZE(kernel_map));
  ...
  for (uint32_t i = 0; i < kmem_claim_count; i++) {
    ...
    if (vm_map_locate_space_anywhere(
            kernel_map, sp_allocation_size, 0,
  ...
  for (uint32_t i = 0; i < kmem_claim_count; i++) {
     ...
     */
    if (vm_map_find_space(
            kernel_map, sp.kc_range->min_address, sp_allocation_size, 0,
            VM_MAP_KERNEL_FLAGS_ANYWHERE(.vmkf_no_soft_limit = true),
            &entry) != KERN_SUCCESS) {
  ...
  for (uint32_t i = 0; i < kmem_claim_count; i++) {
    ...
    printf("%-24s: %p - %p (%u%c)\n", sp.kc_name,
           ...
           mach_vm_size_pretty(sp.kc_size), mach_vm_size_unit(sp.kc_size));
  }
#endif /* DEBUG || DEVELOPMENT */
...
__startup_func static void kmem_range_init(void) {
  ...
  for (kmem_range_id_t i = 1; i < KMEM_RANGE_COUNT; i++) {
    ...
    printf("kmem_large_ranges[%d]    : %p - %p (%u%c)\n", i,
           ...
           mach_vm_size_pretty(range_size), mach_vm_size_unit(range_size));
  }
#endif
}
#ifndef __BUILDING_XNU_LIB_UNITTEST__ /* kernel map is not maintained in       \
                                         unit-test */
STARTUP(KMEM, STARTUP_RANK_THIRD, kmem_range_init);
#endif /* __BUILDING_XNU_LIB_UNITTEST__ */

#if DEBUG || DEVELOPMENT
__startup_func static void kmem_log_init(void) {
  /*
...
kmem_gobj_stats kmem_get_gobj_stats(void) {
  ...
  for (uint8_t i = 0; i < kmem_ptr_ranges; i++) {
    ...
    meta_end = kmem_addr_to_meta(range.max_address, range_id, &addr, &meta_idx);
 ...
 */
__startup_func void kmem_init(vm_offset_t start, vm_offset_t end) {
  vm_map_offset_t map_start;
  vm_map_offset_t map_end;

  map_start = vm_map_trunc_page(start, VM_MAP_PAGE_MASK(kernel_map));
  map_end = vm_map_round_page(end, VM_MAP_PAGE_MASK(kernel_map));

  vm_map_will_allocate_early_map(&kernel_map);
#if defined(__arm64__)
  kernel_map =
      vm_map_create_options(pmap_kernel(), VM_MIN_KERNEL_AND_KEXT_ADDRESS,
                            VM_MAX_KERNEL_ADDRESS, VM_MAP_CREATE_DEFAULT);
  /*
   ...
   */
  {
    ...
    while (pmap_virtual_region(region_select, &region_start, &region_size)) {
      ...
      kr = vm_map_enter(
          kernel_map, &map_addr,
          vm_map_round_page(region_size, VM_MAP_PAGE_MASK(kernel_map)),
          (vm_map_offset_t)0,
          VM_MAP_KERNEL_FLAGS_FIXED_PERMANENT(.vmkf_no_pmap_check = true,
                                              .vmkf_no_soft_limit = true),
          VM_OBJECT_NULL, (vm_object_offset_t)0, FALSE, VM_PROT_NONE,
          VM_PROT_NONE, VM_INHERIT_DEFAULT);

      if (kr != KERN_SUCCESS) {
        panic(
            "kmem_init(0x%llx,0x%llx): vm_map_enter(0x%llx,0x%llx) error 0x%x",
  ...
  if (start != VM_MIN_KERNEL_AND_KEXT_ADDRESS) {
    ...
    kr = vm_map_enter(
        ...
        (vm_map_offset_t)0,
        VM_MAP_KERNEL_FLAGS_FIXED(.vmkf_no_pmap_check = true), VM_OBJECT_NULL,
    ...
    if (kr != KERN_SUCCESS) {
      panic("kmem_init(0x%llx,0x%llx): vm_map_enter(0x%llx,0x%llx) error 0x%x",
            ...
            (uint64_t)(map_start - VM_MIN_KERNEL_AND_KEXT_ADDRESS), kr);
    }
  }
#endif
...
kern_return_t copyinmap(vm_map_t map, vm_map_offset_t fromaddr, void *todata,
                        vm_size_t length) {
  kern_return_t kr = KERN_SUCCESS;
...
kern_return_t copyoutmap(vm_map_t map, void *fromdata, vm_map_address_t toaddr,
                         vm_size_t length) {
  kern_return_t kr = KERN_SUCCESS;
  vm_map_switch_context_t switch_ctx;
...
kern_return_t copyoutmap_atomic64(vm_map_t map, uint64_t value,
                                  vm_map_address_t toaddr) {
  kern_return_t kr = KERN_SUCCESS;
  vm_map_switch_context_t switch_ctx;
 ...
 *	facilities that exist. These are basically the same as
 * VM_KERNEL_ADDRPERM() and VM_KERNEL_UNSLIDE_OR_PERM() except they use a
 * different random seed and are exported to KEXTs.
 *
 *	NOTE: USE THE MACRO VERSIONS OF THESE FUNCTIONS (in vm_param.h) FROM
 * WITHIN THE KERNEL
 */

vm_offset_t vm_kernel_addrhash_internal(vm_offset_t addr, uint64_t salt) {
  assert(salt != 0);
...
__exported vm_offset_t vm_kernel_addrhash_external(vm_offset_t addr);
vm_offset_t vm_kernel_addrhash_external(vm_offset_t addr) {
...
void vm_kernel_addrperm_external(vm_offset_t addr, vm_offset_t *perm_addr) {
...
void vm_packing_pointer_invalid(vm_offset_t ptr, vm_packing_params_t params) {
  if (ptr & ((1ul << params.vmpp_shift) - 1)) {
    panic("pointer %p can't be packed: low %d bits aren't 0", (void *)ptr,
          params.vmpp_shift);
  } else if (ptr <= params.vmpp_base) {
    panic("pointer %p can't be packed: below base %p", (void *)ptr,
          (void *)params.vmpp_base);
  } else {
...
void vm_packing_verify_range(const char *subsystem, vm_offset_t min_address,
                             vm_offset_t max_address,
                             vm_packing_params_t params) {
  if (min_address > max_address) {
    panic("%s: %s range invalid min:%p > max:%p", __func__, subsystem,
  ...
  if (min_address <= params.vmpp_base) {
    panic("%s: %s range invalid min:%p <= base:%p", __func__, subsystem,
          (void *)min_address, (void *)params.vmpp_base);
  }

  if (max_address > vm_packing_max_packable(params)) {
    panic("%s: %s range invalid max:%p >= max packable:%p", __func__, subsystem,
          (void *)max_address, (void *)vm_packing_max_packable(params));
  }
}
...
static void kmem_test_for_entry(vm_map_t map, vm_offset_t addr,
...
static bool can_write_at(vm_offset_t offs, uint32_t page) {
  static const int zero;

  return verify_write(&zero, (void *)(offs + ptoa(page) + 128), 1) == 0;
}
#define assert_writeable(offs, page)                                           \
  assertf(can_write_at(offs, page), "can write at %p + ptoa(%d)",              \
          (void *)offs, page)
...
#define peek(offs, page) (*(uint32_t *)((offs) + ptoa(page)))

#define poke(offs, page, v) (*(uint32_t *)((offs) + ptoa(page)) = (v))
...
__attribute__((noinline)) static void kmem_alloc_basic_test(vm_map_t map) {
  kmem_guard_t guard = {
  ...
  addr = kmem_alloc_guard(map, ptoa(10), ptoa(2) - 1,
                          KMA_KOBJECT | KMA_GUARD_FIRST | KMA_GUARD_LAST, guard)
             .kmr_address;
   ...
   */
  addr = kmem_alloc_guard(map, ptoa(10), 0, KMA_PAGEABLE, guard).kmr_address;
  ...
  kmem_free_guard(map, addr, ptoa(10), KMF_GUARD_FIRST | KMF_GUARD_LAST, guard);
  kmem_test_assert_map(map, 0, 0);
}
...
kmem_realloc_basic_test(vm_map_t map, kmr_flags_t kind) {
   ...
   */
  addr = kmem_alloc_guard(map, ptoa(N), 0, (kind & ~KMEM_FREEOLD) | KMA_ZERO,
                          guard)
             .kmr_address;
  assert3u(addr, !=, 0);
   ...
   *	Grow to N + 3 pages
   */
  newaddr = kmem_realloc_guard(map, addr, ptoa(N), ptoa(N + 3), kind | KMR_ZERO,
                               guard)
                .kmr_address;
  assert3u(newaddr, !=, 0);
...
static int kmem_basic_test(__unused int64_t in, int64_t *out) {
  mach_vm_offset_t addr;
  vm_map_t map;

  printf("%s: test running\n", __func__);

  map = kmem_suballoc(kernel_map, &addr, 64U << 20, VM_MAP_CREATE_DEFAULT,
                      VM_FLAGS_ANYWHERE, KMS_NOFAIL | KMS_DATA_SHARED,
                      VM_KERN_MEMORY_DIAG)
            .kmr_submap;
  ...
  printf(
      "%s: kmem_realloc (KMR_FREEOLD | KMR_GUARD_FIRST | KMR_GUARD_LAST) ...\n",
      __func__);
...
static void kmem_test_get_size_idx_for_chunks(uint32_t chunks) {
  __assert_only uint32_t idx = kmem_get_size_idx_for_chunks(chunks);

  assert(chunks >= kmem_size_array[idx].ks_num_chunk);
}

__attribute__((noinline)) static void kmem_test_get_size_idx_for_all_chunks() {
  for (uint32_t i = 0; i < KMEM_NUM_SIZECLASS; i++) {
...
static int kmem_guard_obj_test(__unused int64_t in, int64_t *out) {
  printf("%s: test running\n", __func__);
