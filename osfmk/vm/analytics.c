...
CA_EVENT(vm_swapusage, CA_INT, max_alloced, CA_INT, max_used, CA_INT,
         trial_deployment_id, CA_STATIC_STRING(CA_UUID_LEN), trial_treatment_id,
         CA_STATIC_STRING(CA_UUID_LEN), trial_experiment_id);

CA_EVENT(mlock_failures, CA_INT, over_global_limit, CA_INT, over_user_limit,
         CA_INT, trial_deployment_id, CA_STATIC_STRING(CA_UUID_LEN),
...
static void report_mlock_failures(void) {
  ...
  e->over_user_limit =
      os_atomic_load_wide(&vm_add_wire_count_over_user_limit, relaxed);
...
CA_EVENT(compressor_age, CA_INT, hour1, CA_INT, hour6, CA_INT, hour12, CA_INT,
         hour24, CA_INT, hour36, CA_INT, hour48, CA_INT, hourMax, CA_INT,
         trial_deployment_id, CA_STATIC_STRING(CA_UUID_LEN), trial_treatment_id,
         CA_STATIC_STRING(CA_UUID_LEN), trial_experiment_id);
...
static void report_compressor_age(void) {
  ...
  c_reporting_bucket_t c_buckets[] = {
      ...
      {.count = 0,
       .lower = HR_TO_S(6),
       .upper = HR_TO_S(12)}, /* [6, 12) hours */
      {.count = 0,
       .lower = HR_TO_S(12),
       .upper = HR_TO_S(24)}, /* [12, 24) hours */
      {.count = 0,
       .lower = HR_TO_S(24),
       .upper = HR_TO_S(36)}, /* [24, 36) hours */
      {.count = 0,
       .lower = HR_TO_S(36),
       .upper = HR_TO_S(48)}, /* [36, 48) hours */
      {.count = 0,
       .lower = HR_TO_S(48),
       .upper = C_REPORTING_BUCKETS_MAX}, /* [48, MAX) hours */
  };
  clock_sec_t now;
  clock_nsec_t nsec;
  ...
  for (unsigned q = 0; q < ARRAY_SIZE(c_queues); q++) {
    ...
    while (!queue_end(c_queues[q], (queue_entry_t)c_seg)) {
      for (unsigned b = 0; b < ARRAY_SIZE(c_buckets); b++) {
        ...
        if ((age >= c_buckets[b].lower) && (age < c_buckets[b].upper)) {
          c_buckets[b].count++;
          break;
        }
      }
 ...
 */
static void report_accounting_health(void) {
  /**
   ...
   * unhealthy levels.
   */
  int64_t pages = (vm_page_wire_count + vm_page_free_count +
                   vm_page_inactive_count + vm_page_active_count +
                   VM_PAGE_COMPRESSOR_COUNT + vm_page_speculative_count
...
static void vm_analytics_init(void) {
  vm_analytics_thread_call = thread_call_allocate_with_options(
