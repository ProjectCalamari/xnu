...
kern_return_t shared_region_pager_data_request(
...
kern_return_t shared_region_pager_data_return(
    memory_object_t mem_obj, memory_object_offset_t offset,
...
kern_return_t shared_region_pager_map(memory_object_t mem_obj, vm_prot_t prot);
...
typedef struct shared_region_jop_key_map {
  ...
  os_refcnt_t
      srk_ref_count; /* count of tasks active with this shared_region_id */
...
queue_head_t shared_region_jop_key_queue =
 ...
 */
uint64_t generate_jop_key(void) {
  uint64_t key;
 ...
 */
uint64_t shared_region_find_key(char *shared_region_id) {
  shared_region_jop_key_map_t region;
  uint64_t key;

  lck_mtx_lock(&shared_region_jop_key_lock);
  queue_iterate(&shared_region_jop_key_queue, region,
                shared_region_jop_key_map_t, srk_queue) {
 ...
 */
void shared_region_key_alloc(char *shared_region_id, bool inherit,
...
again:
  ...
  queue_enter_first(&shared_region_jop_key_queue, new,
                    shared_region_jop_key_map_t, srk_queue);
  region = new;
  new = NULL;
 ...
 */
extern void shared_region_key_dealloc(char *shared_region_id) {
  shared_region_jop_key_map_t region;
  ...
  panic("shared_region_key_dealloc() Shared region ID '%s' not found",
        shared_region_id);

done:
  if (os_ref_release_locked(&region->srk_ref_count) == 0) {
    queue_remove(&shared_region_jop_key_queue, region,
                 shared_region_jop_key_map_t, srk_queue);
    --shared_region_key_count;
  } else {
    region = NULL;
...
shared_region_pager_init(memory_object_t mem_obj,
  ...
  PAGER_DEBUG(PAGER_ALL, ("shared_region_pager_init: %p, %p, %x\n", mem_obj,
                          control, pg_size));
...
shared_region_pager_data_request(memory_object_t mem_obj,
  ...
  if (kr != KERN_SUCCESS) {
    ktriage_record(thread_tid(current_thread()),
                   KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_SHARED_REGION,
  ...
  slide_info_page_size =
      pager->srp_slide_info->si_slide_info_entry->version == 1
          ? PAGE_SIZE_FOR_SR_SLIDE
          : pager->srp_slide_info->si_slide_info_entry->page_size;
  ...
  retry_src_fault:
    ...
    vmfr = vm_fault_page(
        src_top_object, pager->srp_backing_offset + offset + cur_offset,
    ...
    case VM_FAULT_MEMORY_SHORTAGE:
      ...
      ktriage_record(thread_tid(current_thread()),
                     KDBG_TRIAGE_EVENTID(
                         KDBG_TRIAGE_SUBSYS_SHARED_REGION, KDBG_TRIAGE_RESERVED,
    ...
    for (offset_in_page = 0; offset_in_page < PAGE_SIZE;
      ...
      bcopy((const char *)(src_vaddr + offset_in_page),
            (char *)(dst_vaddr + offset_in_page), slide_info_page_size);

      offset_in_backing_object = (chunk_offset + pager->srp_backing_offset);
      ...
      offset_in_sliding_range =
          offset_in_backing_object - pager->srp_slide_info->si_start;
      kr = vm_shared_region_slide_page(
          pager->srp_slide_info, dst_vaddr + offset_in_page,
      ...
      if (shared_region_pager_data_request_debug) {
        printf(
            ...
            chunk_offset, pager->srp_slide_info->si_start,
            pager->srp_slide_info->si_end,
            (pager->srp_backing_offset + offset + cur_offset + offset_in_page),
            pager->srp_backing_offset, offset, (uint64_t)cur_offset,
            (uint64_t)offset_in_page, *(uint64_t *)(dst_vaddr + offset_in_page),
            *(uint64_t *)(dst_vaddr + offset_in_page + 8),
            src_page_object->code_signed, src_page->vmp_cs_validated,
            src_page->vmp_cs_tainted, src_page->vmp_cs_nx, kr);
      }
      if (kr != KERN_SUCCESS) {
        ktriage_record(
            ...
            kr /* arg */);
        if (panic_on_dyld_issue) {
          panic("%s(): shared region slide error %d", __func__, kr);
        }
...
void shared_region_pager_dequeue(shared_region_pager_t pager) {
  ...
  queue_remove(&shared_region_pager_queue, pager, shared_region_pager_t,
               srp_queue);
 ...
 * away. Can be called with shared_region_pager_lock held or not, but always
 ...
 *
 */
kern_return_t shared_region_pager_terminate(
#if !DEBUG
    __unused
#endif
...
kern_return_t shared_region_pager_last_unmap(memory_object_t mem_obj) {
  ...
  PAGER_DEBUG(PAGER_ALL, ("shared_region_pager_last_unmap: %p\n", mem_obj));
  ...
  if (pager->srp_is_mapped) {
    ...
    shared_region_pager_count_mapped--;
    count_unmapped =
        (shared_region_pager_count - shared_region_pager_count_mapped);
...
boolean_t shared_region_pager_backing_object(
  ...
  PAGER_DEBUG(PAGER_ALL, ("shared_region_pager_backing_object: %p\n", mem_obj));
...
shared_region_pager_create(vm_object_t backing_object,
  ...
  queue_enter_first(&shared_region_pager_queue, pager, shared_region_pager_t,
                    srp_queue);
  shared_region_pager_count++;
 ...
 */
memory_object_t shared_region_pager_setup(
    vm_object_t backing_object, vm_object_offset_t backing_offset,
    struct vm_shared_region_slide_info *slide_info, uint64_t jop_key) {
  shared_region_pager_t pager;

  /* create new pager */
  pager = shared_region_pager_create(backing_object, backing_offset, slide_info,
                                     jop_key);
  if (pager == SHARED_REGION_PAGER_NULL) {
  ...
  while (!pager->srp_is_ready) {
    lck_mtx_sleep(&shared_region_pager_lock, LCK_SLEEP_DEFAULT,
                  &pager->srp_is_ready, THREAD_UNINT);
  }
  lck_mtx_unlock(&shared_region_pager_lock);
 ...
 * "backing_object" VM object.
 */
memory_object_t shared_region_pager_match(
    vm_object_t backing_object, vm_object_offset_t backing_offset,
    vm_shared_region_slide_info_t slide_info, uint64_t jop_key) {
  shared_region_pager_t pager;
  vm_shared_region_slide_info_t si;

  lck_mtx_lock(&shared_region_pager_lock);
  queue_iterate(&shared_region_pager_queue, pager, shared_region_pager_t,
                srp_queue) {
    ...
    /* If there's no AUTH section then it can't match (slide_info is always
     * !NULL) */
    if (!si->si_ptrauth) {
      continue;
    }
    ...
    if (memcmp(si->si_slide_info_entry, slide_info->si_slide_info_entry,
               si->si_slide_info_size) != 0) {
      continue;
    }
   ...
   *
   * Note slight race condition here since we drop the lock. This could lead to
   * more than one thread calling setup with the same arguments here. That
   * shouldn't break anything, just waste a little memory.
   */
  lck_mtx_unlock(&shared_region_pager_lock);
  return shared_region_pager_setup(backing_object->vo_copy, backing_offset,
                                   slide_info, jop_key);
}

void shared_region_pager_match_task_key(memory_object_t memobj,
...
void shared_region_pager_trim(void) {
  ...
  for (pager = (shared_region_pager_t)queue_last(&shared_region_pager_queue);
    ...
    if (os_ref_get_count_raw(&pager->srp_ref_count) == 2 &&
        pager->srp_is_ready && !pager->srp_is_mapped) {
      /* this pager can be trimmed */
      num_trim++;
      /* remove this pager from the main list ... */
      shared_region_pager_dequeue(pager);
      /* ... and add it to our trim queue */
      queue_enter_first(&trim_queue, pager, shared_region_pager_t, srp_queue);
  ...
  while (!queue_empty(&trim_queue)) {
    queue_remove_first(&trim_queue, pager, shared_region_pager_t, srp_queue);
    pager->srp_queue.next = NULL;
    pager->srp_queue.prev = NULL;
...
static uint64_t shared_region_pager_purge(shared_region_pager_t pager) {
...
uint64_t shared_region_pager_purge_all(void) {
  uint64_t pages_purged;
  shared_region_pager_t pager;
