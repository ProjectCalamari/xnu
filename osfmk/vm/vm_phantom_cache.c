...
int pg_masks[4] = {0x1, 0x2, 0x4, 0x8};

#define vm_phantom_hash(obj_id, offset)                                        \
  (((natural_t)((uintptr_t)obj_id * vm_ghost_bucket_hash) +                    \
    (offset ^ vm_ghost_bucket_hash)) &                                         \
   vm_ghost_hash_mask)
...
void vm_phantom_cache_init(void) {
...
#if !XNU_TARGET_OS_OSX
  num_entries =
      (uint32_t)(((max_mem / PAGE_SIZE) / 10) / VM_GHOST_PAGES_PER_ENTRY);
#else  /* !XNU_TARGET_OS_OSX */
  num_entries =
      (uint32_t)(((max_mem / PAGE_SIZE) / 4) / VM_GHOST_PAGES_PER_ENTRY);
#endif /* !XNU_TARGET_OS_OSX */
  ...
  /*
   * We index this with g_next_index, so don't exceed the width of that
   * bitfield.
   */
  ...
  vm_phantom_cache_size =
      sizeof(struct vm_ghost) * vm_phantom_cache_num_entries;
  vm_phantom_cache_hash_size =
      sizeof(vm_phantom_hash_entry_t) * vm_phantom_cache_num_entries;
...
void vm_phantom_cache_add_ghost(vm_page_t m) {
  vm_ghost_t vpce;
  vm_object_t object;
  int ghost_index;
  int pg_mask;
  ...
  vpce->g_pages_held = pg_mask;
  vpce->g_obj_offset = (m->vmp_offset >> (PAGE_SHIFT + VM_GHOST_PAGE_SHIFT)) &
...
vm_ghost_t vm_phantom_cache_lookup_ghost(vm_page_t m, uint32_t pg_mask) {
  ...
  g_obj_offset = (m->vmp_offset >> (PAGE_SHIFT + VM_GHOST_PAGE_SHIFT)) &
                 VM_GHOST_OFFSET_MASK;
...
void vm_phantom_cache_update(vm_page_t m) {
  int pg_mask;
  vm_ghost_t vpce;
  vm_object_t object;
 ...
 */
static boolean_t is_thrashing(uint32_t added, uint32_t found,
                              uint32_t threshold) {
 ...
 */
boolean_t vm_phantom_cache_check_pressure() {
  ...
  elapsed_msecs_in_eval = vm_compressor_compute_elapsed_msecs(
      cur_ts_sec, cur_ts_nsec, pc_start_of_eval_period_sec,
  ...
  if (pc_need_eval_reset == TRUE) {
     ...
     */
    sample_period_ghost_counts[sample_period_ghost_counts_indx].added =
        sample_period_ghost_added_count;
    sample_period_ghost_counts[sample_period_ghost_counts_indx].found =
        sample_period_ghost_found_count;
    sample_period_ghost_counts[sample_period_ghost_counts_indx].added_ssd =
        sample_period_ghost_added_count_ssd;
    sample_period_ghost_counts[sample_period_ghost_counts_indx].found_ssd =
        sample_period_ghost_found_count_ssd;
    sample_period_ghost_counts[sample_period_ghost_counts_indx].elapsed_ms =
    ...
    /*
     * Since the trashing rate is really a function of the read latency of the
     * disk we have to consider both the SSD and spinning disk case since the
     * file cache could be backed by either or even both flavors.  When the
     * object is first assigned a phantom_object_id, we query the pager to
     * determine if the backing backing media is an SSD and remember that answer
     * in the vm_object.  We use that info to maintains counts for both the SSD
     * and spinning disk cases.
     */
...
#if PHANTOM_CACHE_DEBUG
  sample_period_ghost_counts[sample_period_ghost_counts_indx]
 ...
 */
void vm_phantom_cache_restart_sample(void) { pc_need_eval_reset = TRUE; }
