...
swapfile_pager_data_initialize(memory_object_t mem_obj,
                               memory_object_offset_t offset,
                               memory_object_cluster_size_t data_cnt);
kern_return_t swapfile_pager_map(memory_object_t mem_obj, vm_prot_t prot);
 ...
 * needs to.
 */
kern_return_t swapfile_pager_data_return(
...
kern_return_t
swapfile_pager_data_initialize(__unused memory_object_t mem_obj,
...
swapfile_pager_data_request(memory_object_t mem_obj,
  ...
  assert(os_ref_get_count_raw(&pager->swp_pgr_hdr_ref) >
  ...
  kr = memory_object_upl_request(mo_control, offset, upl_size, &upl, NULL, NULL,
                                 upl_flags, VM_KERN_MEMORY_OSFMK);
  if (kr != KERN_SUCCESS) {
    retval = kr;
    goto done;
  }
  ...
  kr = kmem_alloc(kernel_map, &kernel_mapping, PAGE_SIZE,
                  KMA_DATA_SHARED | KMA_KOBJECT | KMA_PAGEABLE,
                  VM_KERN_MEMORY_NONE);
  if (kr != KERN_SUCCESS) {
    retval = kr;
    goto done;
  ...
  for (cur_offset = 0; cur_offset < length; cur_offset += PAGE_SIZE) {
    ...
    assert(dst_pnum != 0);
    retval = pmap_enter(kernel_pmap, kernel_mapping, dst_pnum,
  ...
  if (upl != NULL) {
    ...
    if (retval != KERN_SUCCESS) {
      ...
      assertf(page_aligned(upl->u_offset) && page_aligned(upl->u_size),
              ...
              upl->u_size);
      upl_commit_range(upl, 0, upl->u_size, UPL_COMMIT_CS_VALIDATED, upl_pl,
                       pl_count, &empty);
    }
...
void swapfile_pager_dequeue(swapfile_pager_t pager) {
  ...
  queue_remove(&swapfile_pager_queue, pager, swapfile_pager_t, pager_queue);
 ...
 *
 */
kern_return_t swapfile_pager_terminate(
#if !DEBUG
    __unused
#endif
...
kern_return_t swapfile_pager_last_unmap(memory_object_t mem_obj) {
  ...
  PAGER_DEBUG(PAGER_ALL, ("swapfile_pager_last_unmap: %p\n", mem_obj));
...
swapfile_pager_t swapfile_pager_create(struct vnode *vp) {
  ...
  queue_iterate(&swapfile_pager_queue, pager2, swapfile_pager_t, pager_queue) {
    if (pager2->swapfile_vnode == vp) {
      break;
    }
  }
  if (!queue_end(&swapfile_pager_queue, (queue_entry_t)pager2)) {
  ...
  queue_enter_first(&swapfile_pager_queue, pager, swapfile_pager_t,
                    pager_queue);
  swapfile_pager_count++;
 ...
 */
memory_object_t swapfile_pager_setup(struct vnode *vp) {
  swapfile_pager_t pager;

  lck_mtx_lock(&swapfile_pager_lock);

  queue_iterate(&swapfile_pager_queue, pager, swapfile_pager_t, pager_queue) {
    if (pager->swapfile_vnode == vp) {
      break;
    }
  }
  if (queue_end(&swapfile_pager_queue, (queue_entry_t)pager)) {
  ...
  while (!pager->is_ready) {
    lck_mtx_sleep(&swapfile_pager_lock, LCK_SLEEP_DEFAULT, &pager->is_ready,
                  THREAD_UNINT);
  }
  lck_mtx_unlock(&swapfile_pager_lock);
...
memory_object_control_t swapfile_pager_control(memory_object_t mem_obj) {
  swapfile_pager_t pager;
