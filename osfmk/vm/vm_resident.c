 ...
 */
#if __has_include(<AppleFeatures.h>)
...
const uint16_t vm_page_inactive_states = BIT(VM_PAGE_ON_INACTIVE_INTERNAL_Q) |
                                         BIT(VM_PAGE_ON_INACTIVE_EXTERNAL_Q) |
                                         BIT(VM_PAGE_ON_INACTIVE_CLEANED_Q);

const uint16_t vm_page_active_or_inactive_states = vm_page_inactive_states |
#if CONFIG_SECLUDED_MEMORY
...
const uint16_t vm_page_non_speculative_pageable_states =
    vm_page_active_or_inactive_states | BIT(VM_PAGE_ON_THROTTLED_Q);

const uint16_t vm_page_pageable_states =
    vm_page_non_speculative_pageable_states | BIT(VM_PAGE_ON_SPECULATIVE_Q);
...
unsigned int PERCPU_DATA(start_color);
...
struct vm_speculative_age_q
    vm_page_queue_speculative[VM_PAGE_RESERVED_SPECULATIVE_AGE_Q + 1];

boolean_t hibernation_vmqueues_inspection =
    FALSE; /* Tracks if the hibernation code is looking at the VM queues.
...
SECURITY_READ_ONLY_EARLY(uint32_t)
vm_packed_from_vm_pages_array_mask = VM_PAGE_PACKED_FROM_ARRAY;
#ifndef __BUILDING_XNU_LIB_UNITTEST__ /* This is not a compile-time constant   \
                                         when building unit-test */
SECURITY_READ_ONLY_EARLY(vm_packing_params_t)
vm_page_packing_params = VM_PACKING_PARAMS(VM_PAGE_PACKED_PTR);
...
SECURITY_READ_ONLY_LATE(vm_page_bucket_t *)
vm_page_buckets; /* Array of buckets */
SECURITY_READ_ONLY_LATE(unsigned int)
vm_page_bucket_count = 0; /* How big is array? */
SECURITY_READ_ONLY_LATE(unsigned int)
vm_page_hash_mask; /* Mask for hash function */
SECURITY_READ_ONLY_LATE(unsigned int)
vm_page_hash_shift; /* Shift for hash function */
SECURITY_READ_ONLY_LATE(uint32_t) vm_page_bucket_hash; /* Basic bucket hash */
SECURITY_READ_ONLY_LATE(unsigned int)
vm_page_bucket_lock_count = 0; /* How big is array of locks? */
...
SECURITY_READ_ONLY_LATE(lck_ticket_t *) vm_page_bucket_locks;

vm_allocation_site_t
    vm_allocation_sites_static[VM_KERN_MEMORY_FIRST_DYNAMIC + 1];
...
void hash_debug(void) {
  ...
  printf("Number used buckets:     %d = %d%%\n", numbuckets,
         100 * numbuckets / vm_page_bucket_count);
...
SECURITY_READ_ONLY_LATE(unsigned int)
vm_color_mask; /* mask is == (vm_colors-1) */
...
LCK_SPIN_DECLARE_ATTR(vm_objects_wired_lock, &vm_page_lck_grp_bucket,
                      &vm_page_lck_attr);
...
vm_page_queue_head_t vm_page_queue_anonymous
    VM_PAGE_PACKED_ALIGNED; /* inactive memory queue for anonymous pages */
...
uint32_t vm_page_donate_target, vm_page_donate_target_high,
    vm_page_donate_target_low;
 ...
 */
static vm_memory_class_t vm_page_get_memory_class(vm_page_t mem __unused,
                                                  ppnum_t pnum __unused) {
 ...
 */
inline bool vm_page_is_restricted(vm_page_t mem) {
...
unsigned int vm_clump_size, vm_clump_mask, vm_clump_shift,
    vm_clump_promote_threshold;

#if DEVELOPMENT || DEBUG
unsigned long vm_clump_stats[MAX_CLUMP_SIZE + 1];
unsigned long vm_clump_allocs, vm_clump_inserts, vm_clump_inrange,
    vm_clump_promotes;
...
static void vm_page_setup_clump(void) {
  ...
  for (vm_clump_shift = 0, n = vm_clump_size; n > 1;
       n >>= 1, vm_clump_shift++) {
    ;
  }
...
void vm_page_free_queue_init(vm_page_free_queue_t free_queue) {
 ...
 */
__pure2 static vm_page_queue_t
...
__pure2 static bool vm_page_free_queue_has_colors(vm_memory_class_t mem_class) {
  switch (mem_class) {
  case VM_MEMORY_CLASS_REGULAR:
    return true;
#if XNU_VM_HAS_LOPAGE
...
static bool vm_page_secluded_pool_eligible(vm_memory_class_t class) {
  switch (class) {
#if XNU_VM_HAS_LOPAGE
  case VM_MEMORY_CLASS_LOPAGE:
    return false;
...
static bool vm_page_secluded_pool_depleted(void) {
...
vm_page_free_queue_foreach(vm_page_queue_t queue, void (^block)(vm_page_t)) {
  ...
  vm_page_queue_iterate(queue, page, vmp_pageq) { block(page); }
}

__attribute__((overloadable)) static void
vm_page_free_queue_foreach(vm_page_free_queue_t queue,
                           void (^block)(vm_page_t)) {
...
static inline uint32_t vm_pages_free_mask_len(void) {
...
static inline int8_t vm_pages_free_mask_bit(ppnum_t pnum) {
  return (int8_t)(pnum & (MAX_COLORS - 1));
}

static inline uint32_t vm_pages_free_mask_index(ppnum_t pnum) {
  return (pnum - pmap_first_pnum) / MAX_COLORS;
}

__pure2 static inline __uint128_t *vm_pages_free_masks(void) {
  return _vm_pages_free_masks;
}

__pure2 static inline bitmap_t *vm_pages_free_masks_as_bitmap(uint32_t index) {
  /*
...
__pure2 static inline int8_t *vm_pages_free_enqueue_idx(uint32_t index) {
 ...
 */
static inline int8_t vm_pages_free_mask_next_bit(uint32_t index, int8_t bit) {
...
static inline bool vm_pages_free_mask_test(uint32_t index, int8_t bit) {
  return bitmap_test(vm_pages_free_masks_as_bitmap(index), bit);
}

static inline void vm_pages_free_mask_set(uint32_t index, int8_t bit) {
  assert(!vm_pages_free_mask_test(index, bit));
  bitmap_set(vm_pages_free_masks_as_bitmap(index), bit);
}

static inline void vm_pages_free_mask_clear(uint32_t index, int8_t bit) {
  assert(vm_pages_free_mask_test(index, bit));
...
vm_page_free_queue_enter(vm_memory_class_t class, vm_page_t mem, ppnum_t pnum) {
  ...
  assert(mem->vmp_pageq.next == 0 && mem->vmp_pageq.prev == 0 &&
         ...
         mem->vmp_next_m == 0 && mem->vmp_object == 0 &&
         mem->vmp_wire_count == 0 && mem->vmp_busy && !mem->vmp_tabled &&
         !mem->vmp_laundry && !mem->vmp_pmapped && !mem->vmp_wpmapped &&
         !mem->vmp_realtime);
 ...
 */
static bool vm_page_free_queue_has_any_waiters(void) {
  uint32_t result = 0;
...
void vm_page_free_wakeup(event_t event, uint32_t n) {
...
#define DONATE_TO_WAITERS(wake, count, waiters_count, limit)                   \
  ...
  if (vm_page_free_wanted_privileged) {
    DONATE_TO_WAITERS(need_priv_wakeup, vmpr_regular,
                      vm_page_free_wanted_privileged, UINT32_MAX);
  }
  ...
  if (vm_page_free_wanted_secluded) {
    DONATE_TO_WAITERS(need_wakeup_secluded, vmpr_secluded,
                      vm_page_free_wanted_secluded, UINT32_MAX);
    unpriv_limit -=
        DONATE_TO_WAITERS(need_wakeup_secluded, vmpr_regular,
                          vm_page_free_wanted_secluded, unpriv_limit);
  ...
  if (vm_page_free_wanted) {
    unpriv_limit -= DONATE_TO_WAITERS(need_wakeup, vmpr_regular,
                                      vm_page_free_wanted, unpriv_limit);
   ...
   */
  if (need_priv_wakeup) {
    vm_page_free_wakeup(&vm_page_free_wanted_privileged, UINT32_MAX);
  }
  if (need_wakeup) {
  ...
  if (need_wakeup_secluded) {
    vm_page_free_wakeup(&vm_page_free_wanted_secluded, need_wakeup_secluded);
  }
#endif /* CONFIG_SECLUDED_MEMORY */
 ...
 */
static void vm_page_free_queue_enter_list(vm_page_list_t list,
                                          vmp_release_options_t opts) {
  ...
  LCK_MTX_ASSERT(&vm_page_queue_lock, (opts & VMP_RELEASE_Q_LOCKED)
   ...
   */
  page_queues_locked = (opts & (VMP_RELEASE_STARTUP | VMP_RELEASE_HIBERNATE |
  ...
  vm_page_list_foreach_consume(mem, &list) {
...
#if XNU_VM_HAS_LOPAGE
    if ((class == VM_MEMORY_CLASS_REGULAR || class == VM_MEMORY_CLASS_LOPAGE) &&
        vm_lopage_refill && vm_lopage_free_count < vm_lopage_free_limit &&
     ...
     */
    if (page_queues_locked && vm_page_secluded_pool_eligible(class) &&
  ...
  VM_DEBUG_CONSTANT_EVENT(vm_page_release, DBG_VM_PAGE_RELEASE, DBG_FUNC_NONE,
                          list.vmpl_count, 0, 0, 0);
...
__attribute__((always_inline)) void
vm_page_free_queue_remove(vm_memory_class_t class, vm_page_t mem, ppnum_t pnum,
    ...
    if (vm_pages_free_masks()) {
      ...
      *vm_pages_free_enqueue_idx(index) = bit;
      ...
      if (bit != -1) {
        assert(vm_pages_free_mask_test(index, bit));
        mem = vm_page_find_canonical(first_pnum + bit);
        queue = vm_page_free_queue_for_class(class, bit & vm_color_mask);
        vm_page_queue_enter_first(queue, mem, vmp_pageq);
      }
    } else
#endif /* CONFIG_SPTM */
 ...
 */
static void vm_page_set_colors(void) {
  ...
  if (PE_parse_boot_argn(
          "colors", &override,
          sizeof(override))) { /* colors specified as a boot-arg? */
    n = override;
  } else if (vm_cache_geometry_colors) { /* do we know what the cache geometry
                                            is? */
...
static uint32_t vm_delayed_count =
    0; /* when non-zero, indicates we may have more pages to init */
static ppnum_t delay_above_pnum = PPNUM_MAX;

/*
 * For x86 first 8 Gig initializes quickly and gives us lots of lowmem + mem
 * above to start off with. If ARM ever uses delayed page initialization, this
 * value may need to be quite different.
 */
 ...
 */
__attribute__((noinline)) static vm_page_t
 ...
 */
void vm_free_delayed_pages(void) {
  vm_page_t p;
  vm_page_t list = NULL;
  uint_t cnt = 0;
  ...
  if (start_free_va < (vm_offset_t)vm_pages_end) {
    ...
    if (free_size > 0) {
      ...
      /*
       * Note there's no locking here, as only this thread will ever change this
       * value. The reader, vm_page_diagnose, doesn't grab any locks for the
       * counts it looks at.
       */
 ...
 */
static void vm_free_delayed_pages_contig(uint_t npages, ppnum_t max_pnum,
                                         ppnum_t pnum_mask) {
  vm_page_t p;
  ppnum_t pnum;
  uint_t cnt = 0;
...
void vm_page_init_local_q(unsigned int num_cpus) {
 ...
 */
void vm_init_before_launchd(void) {
  vm_page_lockspin_queues();
 ...
 */
__startup_func void vm_page_bootstrap(vm_offset_t *startp, vm_offset_t *endp) {
  unsigned int i;
  unsigned int log1;
  unsigned int log2;
  unsigned int size;
  ...
  lck_mtx_init(&vm_page_queue_free_lock, &vm_page_lck_grp_free,
               &vm_page_lck_attr);
  lck_mtx_init(&vm_page_queue_lock, &vm_page_lck_grp_queue, &vm_page_lck_attr);
  lck_mtx_init(&vm_purgeable_queue_lock, &vm_page_lck_grp_purge,
               &vm_page_lck_attr);
  ...
  PE_parse_boot_argn("vm_page_bg_mode", &vm_page_background_mode,
                     sizeof(vm_page_background_mode));
  PE_parse_boot_argn("vm_page_bg_exclude_external",
                     &vm_page_background_exclude_external,
                     sizeof(vm_page_background_exclude_external));
  PE_parse_boot_argn("vm_page_bg_target", &vm_page_background_target,
  ...
  PE_parse_boot_argn("vm_page_donate_mode", &vm_page_donate_mode,
                     sizeof(vm_page_donate_mode));
  ...
  vm_allocation_sites_static[VM_KERN_MEMORY_FIRST_DYNAMIC].tag =
      VM_KERN_MEMORY_ANY;
  vm_allocation_sites[VM_KERN_MEMORY_ANY] =
      &vm_allocation_sites_static[VM_KERN_MEMORY_FIRST_DYNAMIC];

  /*
   *	Steal memory for the map and zone subsystems.
   *
   *	make sure initialize_ram_ranges() has run before we steal pages for the
   * first time on arm
   */
  ...
  vm_page_bucket_lock_count =
      (vm_page_bucket_count + BUCKETS_PER_LOCK - 1) / BUCKETS_PER_LOCK;
  ...
  vm_page_bucket_hash =
      1 << ((log1 + 1) >> 1); /* Get (ceiling of sqrt of table size) */
  vm_page_bucket_hash |=
      1 << ((log1 + 1) >> 2); /* Get (ceiling of quadroot of table size) */
  vm_page_bucket_hash |=
   ...
   * any stomping there.
   */
  vm_page_fake_buckets = (vm_page_bucket_t *)pmap_steal_memory(
      vm_page_bucket_count * sizeof(vm_page_bucket_t), 0);
  vm_page_fake_buckets_start = (vm_map_offset_t)vm_page_fake_buckets;
  vm_page_fake_buckets_end =
      vm_map_round_page((vm_page_fake_buckets_start +
                         (vm_page_bucket_count * sizeof(vm_page_bucket_t))),
                        PAGE_MASK);
  char *cp;
  for (cp = (char *)vm_page_fake_buckets_start;
       cp < (char *)vm_page_fake_buckets_end; cp++) {
    *cp = 0x5a;
  }
#endif /* VM_PAGE_FAKE_BUCKETS */
#endif /* VM_PAGE_BUCKETS_CHECK */
  ...
  vm_page_buckets = (vm_page_bucket_t *)pmap_steal_memory(
      vm_page_bucket_count * sizeof(vm_page_bucket_t), 0);

  kernel_debug_string_early("vm_page_bucket_locks");
  vm_page_bucket_locks = (lck_ticket_t *)pmap_steal_memory(
      vm_page_bucket_lock_count * sizeof(lck_ticket_t), 0);
  ...
  for (i = 0; i < vm_page_bucket_lock_count; i++) {
    lck_ticket_init(&vm_page_bucket_locks[i], &vm_page_lck_grp_bucket);
  }
  ...
  *endp = virtual_space_end;
  ...
  vm_page_wire_count = ((unsigned int)atop_64(max_mem)) - vm_page_free_count -
...
/*
 * This is the early boot time allocator for data structures needed to bootstrap
 * the VM system. On x86 it will allocate large pages if size is sufficiently
 * large. We don't need to do this on ARM yet, due to the combination of a large
 * base page size and smaller RAM devices.
 */
__static_testable void *
pmap_steal_memory_internal(vm_size_t size, vm_size_t alignment,
                           boolean_t might_free, unsigned int flags,
...
#if defined(__x86_64__)
    /*
     * Release remaining unused section of preallocated KVA and the 4K page
     * tables that map it. This makes the VA available for large page mappings.
     */
  ...
  /*
   * Allocate the virtual space for this request. On x86, we'll align to a large
   * page address if the size is big enough to back with at least 1 large page.
   */
#if defined(__x86_64__)
  if (size >= I386_LPGBYTES) {
    virtual_space_start =
        ((virtual_space_start + I386_LPGMASK) & ~I386_LPGMASK);
  }
#endif
  virtual_space_start =
      (virtual_space_start + (alignment - 1)) & ~(alignment - 1);
  addr = virtual_space_start;
  virtual_space_start += size;

  // kprintf("pmap_steal_memory: %08lX - %08lX; size=%08lX\n", (long)addr,
  // (long)virtual_space_start, (long)size);	/* (TEST/DEBUG) */
  ...
  while (map_addr < end) {
    ...
    if ((map_addr & I386_LPGMASK) == 0 &&
      ...
      if (kr != KERN_SUCCESS) {
        panic("pmap_steal_memory: pmap_enter() large failed, new_addr=%#lx, "
              "phys_page=%u",
    ...
    kr = pmap_enter(kernel_pmap, map_addr, phys_page,
                    VM_PROT_READ | VM_PROT_WRITE, VM_PROT_NONE, pmap_flags,
                    FALSE, mapping_type);

    if (kr != KERN_SUCCESS) {
      panic(
          "pmap_steal_memory() pmap_enter failed, map_addr=%#lx, phys_page=%u",
          (unsigned long)map_addr, phys_page);
    }
    map_addr += PAGE_SIZE;
...
__mockable void *pmap_steal_memory(vm_size_t size, vm_size_t alignment) {
  return pmap_steal_memory_internal(size, alignment, FALSE, 0,
                                    PMAP_MAPPING_TYPE_RESTRICTED);
}

void *pmap_steal_freeable_memory(vm_size_t size) {
...
TUNABLE_DT(unsigned int, secluded_mem_mb, "/defaults", "kern.secluded_mem_mb",
           "secluded_mem_mb", 0, TUNABLE_DT_NONE);
...
TUNABLE(secluded_filecache_mode_t, secluded_for_filecache,
        "secluded_for_filecache", SECLUDED_FILECACHE_RDONLY);
uint64_t secluded_shutoff_trigger = 0;
uint64_t secluded_shutoff_headroom =
...
__mockable void pmap_startup(vm_offset_t *startp, vm_offset_t *endp) {
  unsigned int npages;
  ppnum_t phys_page;
  uint64_t mem_sz;
  uint64_t start_ns;
...
#if CONFIG_SPTM
  {
    ...
    _vm_pages_free_masks =
        pmap_steal_memory(count * sizeof(__uint128_t), sizeof(__uint128_t));
...
#endif
  mem_sz += round_page(virtual_space_start) -
            virtual_space_start; /* Account for any slop */
  divisor = PAGE_SIZE + sizeof(struct vm_page);
  npages = (uint32_t)((mem_sz + divisor - 1) /
                      divisor); /* scaled to include the vm_page_ts */
  ...
  vm_page_secluded_target =
      (unsigned int)atop_64(secluded_mem_mb * 1024ULL * 1024ULL);
  ...
  if (vm_page_secluded_target != 0) {
    ...
    assert(secluded_shutoff_trigger == 0 ||
           secluded_shutoff_trigger > secluded_shutoff_headroom);
  }
#endif /* CONFIG_SECLUDED_MEMORY */
  ...
  if (!PE_parse_boot_argn("delay_above_gb", &delay_above_gb,
  ...
  if (delay_above_pnum <= max_valid_low_ppnum) {
    delay_above_pnum =
        max_valid_low_ppnum + ((1024 * 1024 * 1024) >> PAGE_SHIFT);
  }
  ...
  for (uint32_t i = 0; i < npages && pmap_next_page(&phys_page); i++) {
    ...
    if (i == 0) {
      ...
      patch_low_glo_vm_page_info(vm_pages, vm_pages_end, vm_pages_first_pnum);
    }
#else
  ...
  if ((vm_page_t)VM_PAGE_UNPACK_PTR(VM_PAGE_PACK_PTR(vm_page_get(
          vm_pages_count - 1))) != vm_page_get(vm_pages_count - 1)) {
 ...
 * or frees to this zone. It's just here for reporting purposes via zprint
 * command. This needs to be done after all initially delayed pages are put on
 * the free lists.
 */
void vm_pages_array_finalize(void) {
  (void)zone_create_ext(
      "vm pages array", sizeof(struct vm_page),
      ZC_KASAN_NOREDZONE | ZC_KASAN_NOQUARANTINE, ZONE_ID_VM_PAGES,
      ^(zone_t z) {
        ...
        vm_page_array_zone_data_size =
            (uint64_t)vm_pages_end - (uint64_t)vm_pages;
        vm_page_zone_pages =
            atop(round_page((vm_offset_t)vm_page_array_zone_data_size));
...
/*
 * Create the vm_pages zone. This is used for the vm_page structures for the
 * pages that are scavanged from other boot time usages by ml_static_mfree(). As
 * such, this needs to happen in early VM bootstrap.
 */

__startup_func static void vm_page_module_init(void) {
  ...
  vm_page_zone =
      zone_create_ext("vm pages", vm_page_with_ppnum_size,
                      ZC_ALIGNMENT_REQUIRED | ZC_VM, ZONE_ID_ANY, ^(zone_t z) {
                        /*
                         * The number "10" is a small number that is larger than
                         * the number of fictitious pages that any single caller
                         * will attempt to allocate without blocking.
                         *
                         * The largest such number at the moment is kmem_alloc()
                         * when 2 guard pages are asked. 10 is simply a somewhat
                         * larger number, taking into account the 50% hysteresis
                         * the zone allocator uses.
                         *
...
static vm_page_radix_node_t vm_page_radix_node_unpack(vm_page_radix_ptr_t ptr) {
  return (vm_page_radix_node_t)VM_UNPACK_POINTER(ptr, VM_PAGE_PACKED_PTR);
}

static vm_page_radix_ptr_t vm_page_radix_node_pack(vm_page_radix_node_t node) {
...
static uint32_t vm_page_radix_key(uint32_t level, uint32_t index) {
  uint32_t key = index >> (VM_PAGE_RADIX_FANOUT_SHIFT * level);

  return key & (VM_PAGE_RADIX_FANOUT - 1);
}

static vm_page_radix_ptr_t *vm_page_radix_slot(vm_page_radix_node_t node,
                                               uint32_t level, uint32_t index) {
  return node->vmpr_array + vm_page_radix_key(level, index);
}

__startup_func __attribute__((noinline)) static vm_page_radix_node_t
vm_pages_radix_init_root(uint32_t *levelp) {
  ...
  size =
      (vm_page_radix_key(level, max_index) + 1) * sizeof(vm_page_radix_ptr_t);
...
static vm_page_radix_node_t vm_pages_radix_load_root(uint32_t *level) {
...
vm_page_t vm_pages_radix_next(uint32_t *cursor, ppnum_t *pnum) {
...
static int vm_page_radix_verify_test(int64_t in __unused, int64_t *out) {
  uint32_t count = 0;
  vm_page_t mem;
...
__attribute__((noinline)) static void vm_pages_radix_insert(ppnum_t pnum,
                                                            vm_page_t page) {
...
__abortlike static void vm_page_for_ppnum_panic(ppnum_t pnum) {
  if (pnum < pmap_first_pnum) {
    panic("physical page is before the start of DRAM: %#x < %#x)", pnum,
          pmap_first_pnum);
  }
  panic("physical page is beyond the end of managed DRAM: %#x >= %#x)", pnum,
        vm_pages_first_pnum + vm_pages_count);
}

vm_page_t vm_page_find_canonical(ppnum_t pnum) {
  vm_page_radix_ptr_t *slot;
  vm_page_radix_node_t node;
  ...
  if (pnum >= vm_pages_first_pnum + vm_pages_count) {
    /*
     * We could receive requests for pages which are beyond the xnu's managed
     * space. (eg: ECC errors) These need to be handled gracefully, so we return
     * VM_PAGE_NULL here.
     */
    return VM_PAGE_NULL;
  }
 ...
 */
vm_page_t vm_page_create(ppnum_t phys_page, bool canonical,
...
void vm_page_create_canonical(ppnum_t phys_page) {
  vm_page_t m;
...
#define vm_page_hash(object, offset)                                           \
  (((natural_t)((uintptr_t)object * vm_page_bucket_hash) +                     \
    ((uint32_t)atop_64(offset) ^ vm_page_bucket_hash)) &                       \
   vm_page_hash_mask)
 ...
 */
void vm_page_insert(vm_page_t mem, vm_object_t object,
                    vm_object_offset_t offset) {
  vm_page_insert_internal(mem, object, offset, VM_KERN_MEMORY_NONE, FALSE, TRUE,
                          FALSE, FALSE, NULL);
}

void vm_page_insert_wired(vm_page_t mem, vm_object_t object,
                          vm_object_offset_t offset, vm_tag_t tag) {
  vm_page_insert_internal(mem, object, offset, tag, FALSE, TRUE, FALSE, FALSE,
                          NULL);
}

void vm_page_insert_internal(vm_page_t mem, vm_object_t object,
                             vm_object_offset_t offset, vm_tag_t tag,
  ...
  LCK_MTX_ASSERT(&vm_page_queue_lock, queues_lock_held
                                          ? LCK_MTX_ASSERT_OWNED
                                          : LCK_MTX_ASSERT_NOTOWNED);
  ...
  if (insert_in_hash == TRUE) {
    ...
    if (object->internal && (offset >= object->vo_size)) {
      panic("vm_page_insert_internal: (page=%p,obj=%p,off=0x%llx,size=0x%llx) "
            "inserted at offset past object bounds",
    ...
    lck_ticket_lock(bucket_lock, &vm_page_lck_grp_bucket);
...
#if DEVELOPMENT || DEBUG
  if (object->object_is_shared_cache && object->pager != NULL &&
  ...
  if (object->purgable == VM_PURGABLE_DENY && !object->vo_ledger_tag) {
    ...
    vm_object_ledger_tag_ledgers(
        object, &ledger_idx_volatile, &ledger_idx_nonvolatile,
        &ledger_idx_volatile_compressed, &ledger_idx_nonvolatile_compressed,
        &ledger_idx_composite, &ledger_idx_external_wired, &do_footprint);
  }
  if (owner && object->internal &&
      (object->purgable == VM_PURGABLE_NONVOLATILE ||
       object->purgable == VM_PURGABLE_DENY || VM_PAGE_WIRED(mem))) {
    if (delayed_ledger_update) {
      *delayed_ledger_update += PAGE_SIZE;
    } else {
      /* more non-volatile bytes */
      ledger_credit(owner->ledger, ledger_idx_nonvolatile, PAGE_SIZE);
      if (do_footprint) {
        /* more footprint */
        ledger_credit(owner->ledger, task_ledgers.phys_footprint, PAGE_SIZE);
      } else if (ledger_idx_composite != -1) {
        ledger_credit(owner->ledger, ledger_idx_composite, PAGE_SIZE);
      }
    }
  } else if (owner && object->internal &&
             (object->purgable == VM_PURGABLE_VOLATILE ||
              object->purgable == VM_PURGABLE_EMPTY)) {
    assert(!VM_PAGE_WIRED(mem));
    /* more volatile bytes */
    ledger_credit(owner->ledger, ledger_idx_volatile, PAGE_SIZE);
  }
  ...
  if (vm_object_tracking_btlog && object->internal &&
      object->resident_page_count == 0 && object->pager == NULL &&
      object->shadow != NULL && object->shadow->vo_copy == object) {
...
void vm_page_replace(vm_page_t mem, vm_object_t object,
  ...
  vm_page_insert_internal(mem, object, offset, VM_KERN_MEMORY_NONE, FALSE,
...
void vm_page_remove(vm_page_t mem, boolean_t remove_from_hash) {
  vm_page_bucket_t *bucket;
  vm_page_t this;
  lck_ticket_t *bucket_lock;
  int hash_id;
...
#if DEVELOPMENT || DEBUG
  if (m_object->object_is_shared_cache && m_object->pager != NULL &&
  ...
  if (!m_object->internal && m_object->cached_list.next &&
  ...
  if (mem->vmp_reusable) {
    ...
    assert(m_object->reusable_page_count <= m_object->resident_page_count);
  ...
  if (m_object->purgable == VM_PURGABLE_DENY && !m_object->vo_ledger_tag) {
    owner = TASK_NULL;
  } else {
    owner = VM_OBJECT_OWNER(m_object);
    vm_object_ledger_tag_ledgers(
        m_object, &ledger_idx_volatile, &ledger_idx_nonvolatile,
        &ledger_idx_volatile_compressed, &ledger_idx_nonvolatile_compressed,
        &ledger_idx_composite, &ledger_idx_external_wired, &do_footprint);
  }
  if (owner && m_object->internal &&
      (m_object->purgable == VM_PURGABLE_NONVOLATILE ||
       m_object->purgable == VM_PURGABLE_DENY || VM_PAGE_WIRED(mem))) {
    /* less non-volatile bytes */
    ledger_debit(owner->ledger, ledger_idx_nonvolatile, PAGE_SIZE);
    if (do_footprint) {
      /* less footprint */
      ledger_debit(owner->ledger, task_ledgers.phys_footprint, PAGE_SIZE);
    } else if (ledger_idx_composite != -1) {
      ledger_debit(owner->ledger, ledger_idx_composite, PAGE_SIZE);
    }
  } else if (owner && m_object->internal &&
             (m_object->purgable == VM_PURGABLE_VOLATILE ||
              m_object->purgable == VM_PURGABLE_EMPTY)) {
    assert(!VM_PAGE_WIRED(mem));
    /* less volatile bytes */
    ledger_debit(owner->ledger, ledger_idx_volatile, PAGE_SIZE);
  }
...
vm_page_t kdp_vm_page_lookup(vm_object_t object, vm_object_offset_t offset) {
  vm_page_t cur_page;
  int num_traversed = 0;
...
vm_page_t vm_page_lookup(vm_object_t object, vm_object_offset_t offset) {
  vm_page_t mem;
  vm_page_bucket_t *bucket;
  vm_page_queue_entry_t qe;
  lck_ticket_t *bucket_lock = NULL;
  int hash_id;
#if DEBUG_VM_PAGE_LOOKUP
  ...
  if (is_kernel_object(object)) {
    offset = vm_memtag_canonicalize_kernel(offset);
  }
#endif /* KASAN_TBI */
...
void vm_page_rename(vm_page_t mem, vm_object_t new_object,
  ...
  vm_page_insert_internal(mem, new_object, new_offset, tag, TRUE, TRUE, FALSE,
                          FALSE, NULL);
 ...
 */
void vm_page_init(vm_page_t mem, ppnum_t phys_page) {
  assert(phys_page);

#if DEBUG
  if ((phys_page != vm_page_fictitious_addr) &&
      (phys_page != vm_page_guard_addr)) {
   ...
   * try to use initial values which match 0. This minimizes the number of
   * writes needed for boot-time initialization.
   */
...
vm_page_t vm_page_create_fictitious(void) {
  return vm_page_create(vm_page_fictitious_addr, false, Z_WAITOK);
}

vm_page_t vm_page_create_guard(bool canwait) {
  return vm_page_create(vm_page_guard_addr, false,
                        canwait ? Z_WAITOK : Z_NOWAIT);
}

vm_page_t vm_page_create_private(ppnum_t base_page) {
  assert(base_page != vm_page_fictitious_addr &&
         base_page != vm_page_guard_addr);
  return vm_page_create(base_page, false, Z_WAITOK);
}

bool vm_page_is_canonical(const struct vm_page *m) { return m->vmp_canonical; }

bool vm_page_is_fictitious(const struct vm_page *m) {
#if XNU_VM_HAS_LINEAR_PAGES_ARRAY
...
bool vm_page_is_guard(const struct vm_page *m) {
#if XNU_VM_HAS_LINEAR_PAGES_ARRAY
  if (vm_page_in_array(m)) {
    return false;
  }
...
bool vm_page_is_private(const struct vm_page *m) {
  return !vm_page_is_canonical(m) && !vm_page_is_fictitious(m);
}

void vm_page_make_private(vm_page_t m, ppnum_t base_page) {
  LCK_MTX_ASSERT(&vm_page_queue_lock, LCK_MTX_ASSERT_OWNED);
...
void vm_page_reset_private(vm_page_t m) {
 ...
 */
static void vm_page_release_fictitious(vm_page_t m) {
...
void vm_update_darkwake_mode(boolean_t darkwake_mode) {
...
void vm_page_update_special_state(vm_page_t mem) {
...
void vm_page_assign_special_state(vm_page_t mem, vm_page_specialq_t mode) {
  if (mem->vmp_q_state == VM_PAGE_USED_BY_COMPRESSOR) {
    return;
  }
  ...
  case VM_PAGE_SPECIAL_Q_BG: {
    ...
    if (my_task) {
      mem->vmp_on_specialq =
          (proc_get_effective_task_policy(my_task, TASK_POLICY_DARWIN_BG)
               ? VM_PAGE_SPECIAL_Q_BG
               : VM_PAGE_SPECIAL_Q_EMPTY);
    }
    break;
  }
...
void vm_page_remove_from_specialq(vm_page_t mem) {
  vm_object_t m_object;
  ...
  case VM_PAGE_SPECIAL_Q_DONATE: {
    if (mem->vmp_specialq.next && mem->vmp_specialq.prev) {
      vm_page_queue_remove((vm_page_queue_head_t *)&vm_page_queue_donate, mem,
                           vmp_specialq);
      mem->vmp_specialq.next = 0;
      mem->vmp_specialq.prev = 0;
      vm_page_donate_count--;
      if (vm_page_donate_queue_ripe &&
          (vm_page_donate_count < vm_page_donate_target)) {
...
void vm_page_add_to_specialq(vm_page_t mem, boolean_t first) {
  vm_object_t m_object;
  ...
  case VM_PAGE_SPECIAL_Q_DONATE: {
    if (first == TRUE) {
      vm_page_queue_enter_first((vm_page_queue_head_t *)&vm_page_queue_donate,
                                mem, vmp_specialq);
    } else {
      vm_page_queue_enter((vm_page_queue_head_t *)&vm_page_queue_donate, mem,
                          vmp_specialq);
    }
    vm_page_donate_count++;
    if (!vm_page_donate_queue_ripe &&
 ...
 */
static vm_page_t vm_page_grab_finalize(vm_grab_options_t grab_options __unused,
                                       vm_page_t mem) {
  task_t task;
  ...
  {
    VM_DEBUG_EVENT(vm_page_grab, DBG_VM_PAGE_GRAB, DBG_FUNC_NONE, grab_options,
                   0, 0, 0);
  }
...
vm_page_t vm_page_grablo(vm_grab_options_t grab_options) {
  vm_page_t mem = VM_PAGE_NULL;

  if (!vm_lopage_needed) {
    return vm_page_grab_options(grab_options);
  }
  ...
  if (vm_lopage_free_count) {
    ...
    disable_preemption();
#endif
    mem = vm_page_free_queue_grab(grab_options, VM_MEMORY_CLASS_LOPAGE, 1,
                                  VM_PAGE_NOT_ON_Q)
              .vmpl_head;
#if LCK_MTX_USE_ARCH
    enable_preemption();
  ...
  if (mem == VM_PAGE_NULL) {
    if (cpm_allocate(PAGE_SIZE, &mem, atop(PPNUM_MAX), 0, FALSE, KMA_LOMEM) !=
        KERN_SUCCESS) {
      vm_free_page_lock_spin();
  ...
  return vm_page_grab_finalize(grab_options, mem);
}
...
vm_page_grab_secluded(vm_grab_options_t grab_options) {
  ...
  assert((vm_page_secluded_count_free + vm_page_secluded_count_inuse) ==
         vm_page_secluded_count);
  ...
  if (mem->vmp_busy || mem->vmp_cleaning || mem->vmp_laundry) {
...
uint64_t vm_page_secluded_drain(void) {
  ...
  //__FUNCTION__, __LINE__, saved_secluded_count, vm_page_secluded_count,
  //vm_page_secluded_target, num_reclaimed);
 ...
 */
static vm_page_t vm_page_grab_from_cpu(vm_page_t *cpu_list,
                                       scalable_counter_t *counter) {
  vm_page_t mem = _vm_page_list_pop(cpu_list);
...
vm_page_grab_slow(vm_grab_options_t grab_options) {
  ...
  if (target != 0) {
    list = vm_page_free_queue_grab(grab_options, class, target,
...
vm_page_t vm_page_grab_options(vm_grab_options_t options) {
  vm_page_t mem;
...
vm_grab_options_t vm_page_grab_options_for_object(vm_object_t object __unused) {
 ...
 */
static void vm_page_free_queue_steal(vm_grab_options_t options, vm_page_t mem) {
  ppnum_t pnum = VM_PAGE_GET_PHYS_PAGE(mem);
  vm_memory_class_t class = vm_page_get_memory_class(mem, pnum);
...
boolean_t vm_page_wait(int interruptible) {
  /*
  ...
  if (vps_dynamic_priority_enabled) {
     ...
     */
    VM_DEBUG_CONSTANT_EVENT(vm_page_wait_block, DBG_VM_PAGE_WAIT_BLOCK,
                            DBG_FUNC_START, vm_page_free_wanted_privileged,
                            vm_page_free_wanted,
#if CONFIG_SECLUDED_MEMORY
...
#endif /* CONFIG_SECLUDED_MEMORY */
                            0);
    wait_result = lck_mtx_sleep_with_inheritor(
        &vm_page_queue_free_lock, LCK_SLEEP_UNLOCK, wait_event,
        vm_pageout_scan_thread, interruptible, 0);
  } else {
    ...
    VM_DEBUG_CONSTANT_EVENT(vm_page_wait_block, DBG_VM_PAGE_WAIT_BLOCK,
                            DBG_FUNC_END, 0, 0, 0, 0);
  }

out:
  return (wait_result == THREAD_AWAKENED) ||
         (wait_result == THREAD_NOT_WAITING);
}
 ...
 */
static void vm_page_free_prepare(vm_page_t mem) {
   ...
   */
  const sptm_paddr_t paddr = ((uint64_t)VM_PAGE_GET_PHYS_PAGE(mem))
                             << PAGE_SHIFT;
...
void vm_page_free_prepare_queues(vm_page_t mem) {
  vm_object_t m_object;
  ...
  if (VM_PAGE_WIRED(mem)) {
    ...
    if (m_object) {
      ...
      if (m_object->internal && m_object->vo_owner != TASK_NULL &&
          (m_object->purgable == VM_PURGABLE_VOLATILE ||
           m_object->purgable == VM_PURGABLE_EMPTY)) {
        owner = VM_OBJECT_OWNER(m_object);
        vm_object_ledger_tag_ledgers(
 ...
 */
inline static void vm_page_reset_canonical(vm_page_t mem) {
  *mem = (struct vm_page){
      .vmp_offset = (vm_object_offset_t)-1,
      .vmp_q_state = VM_PAGE_NOT_ON_Q,
      .vmp_on_specialq = VM_PAGE_SPECIAL_Q_EMPTY,
...
void vm_page_free_prepare_object(vm_page_t mem, boolean_t remove_from_hash) {
  if (mem->vmp_tabled) {
...
void vm_page_release(vm_page_t mem, vmp_release_options_t options) {
...
void vm_page_free(vm_page_t mem) {
  ...
  if (vm_page_is_canonical(mem)) {
    ...
    vm_page_release(mem, VMP_RELEASE_Q_LOCKED | VMP_RELEASE_SKIP_FREE_CHECK);
  } else {
    vm_page_release_fictitious(mem);
  }
}

void vm_page_free_unlocked(vm_page_t mem, boolean_t remove_from_hash) {
  vm_page_lockspin_queues();
  vm_page_free_prepare_queues(mem);
  vm_page_unlock_queues();
 ...
 */
void vm_page_free_list(vm_page_t freeq, bool prepare_object) {
  LCK_MTX_ASSERT(&vm_page_queue_lock, LCK_MTX_ASSERT_NOTOWNED);
...
void vm_page_wire(vm_page_t mem, vm_tag_t tag, boolean_t check_memorystatus) {
  vm_object_t m_object;
  ...
  if (!VM_PAGE_WIRED(mem)) {
    ...
    if (mem->vmp_unmodified_ro == true) {
      ...
      vm_object_compressor_pager_state_clr(VM_PAGE_OBJECT(mem),
                                           mem->vmp_offset);
    }
    ...
    if (m_object) {
      ...
      if (m_object->all_reusable) {
        ...
        vm_object_reuse_pages(m_object, mem->vmp_offset,
                              mem->vmp_offset + PAGE_SIZE_64, FALSE);
      }
    }
    assert(!mem->vmp_reusable);
 ...
 *	The page's object and the page queues must be locked.
 */
void vm_page_unwire(vm_page_t mem, boolean_t queueit) {
  vm_object_t m_object;
 ...
 */
void vm_page_deactivate(vm_page_t m) { vm_page_deactivate_internal(m, TRUE); }

void vm_page_deactivate_internal(vm_page_t m, boolean_t clear_hw_reference) {
  vm_object_t m_object;
  ...
  //	dbgLog(VM_PAGE_GET_PHYS_PAGE(m), vm_page_free_count, vm_page_wire_count,
  //6);	/* (TEST/DEBUG) */
  LCK_MTX_ASSERT(&vm_page_queue_lock, LCK_MTX_ASSERT_OWNED);
  /*
   ...
   * reference which is held on the object while the page is in the pageout
   * queue... just let the normal laundry processing proceed
   */
  if (m->vmp_laundry || !vm_page_is_canonical(m) ||
      (m->vmp_q_state == VM_PAGE_USED_BY_COMPRESSOR) ||
      (m->vmp_q_state == VM_PAGE_ON_PAGEOUT_Q) || VM_PAGE_WIRED(m)) {
    return;
  }
  ...
  if (!VM_PAGE_INACTIVE(m)) {
    ...
    if (!VM_DYNAMIC_PAGING_ENABLED() && m->vmp_dirty && m_object->internal &&
      ...
      if (m_object->named && os_ref_get_count_raw(&m_object->ref_count) == 1) {
...
void vm_page_activate(vm_page_t m) {
  ...
  vm_page_queues_remove(
      m, ((m->vmp_on_specialq == VM_PAGE_SPECIAL_Q_DONATE) ? TRUE : FALSE));
...
#if CONFIG_SECLUDED_MEMORY
      if (secluded_for_filecache && vm_page_secluded_target != 0 &&
          num_tasks_can_use_secluded_mem == 0 &&
          m_object->eligible_for_secluded && !m->vmp_realtime) {
 ...
 *      The page queues must be locked.
 */
void vm_page_speculate(vm_page_t m, boolean_t new) {
  ...
  if (!VM_PAGE_WIRED(m)) {
    ...
    if (vm_page_speculative_count == 0) {
      ...
      aq->age_ts.tv_nsec =
          (vm_pageout_state.vm_page_speculative_q_age_ms % 1000) * 1000 *
          NSEC_PER_USEC;
      ...
      if (CMP_MACH_TIMESPEC(&ts, &aq->age_ts) >= 0) {
        ...
        aq->age_ts.tv_sec =
            vm_pageout_state.vm_page_speculative_q_age_ms / 1000;
        aq->age_ts.tv_nsec =
            (vm_pageout_state.vm_page_speculative_q_age_ms % 1000) * 1000 *
            NSEC_PER_USEC;
 ...
 *      The page queues must be locked.
 */
void vm_page_speculate_ageit(struct vm_speculative_age_q *aq) {
...
void vm_page_reactivate_all_throttled(void) {
...
void vm_page_reactivate_local(uint32_t lid, boolean_t force,
                              boolean_t nolocks) {
  struct vpl *lq;
  ...
  if (lq->vpl_count) {
    ...
    vm_page_queue_iterate(&lq->vpl_queue, m, vmp_pageq) {
      ...
      if (m->vmp_local_id != lid) {
        panic("vm_page_reactivate_local: found vm_page_t(%p) with wrong cpuid",
              m);
      }
...
void vm_page_part_zero_fill(vm_page_t m, vm_offset_t m_pa, vm_size_t len) {
#if 0
	/*
	 * we don't hold the page queue lock
	 * so this check isn't safe to make
  ...
  if ((m_pa + len) < PAGE_SIZE) {
    vm_page_part_copy(m, m_pa + len, tmp, m_pa + len, PAGE_SIZE - (m_pa + len));
  }
  vm_page_copy(tmp, m);
  VM_PAGE_FREE(tmp);
#endif
 ...
 */
void vm_page_zero_fill(vm_page_t m) {
  int options = 0;
#if 0
	/*
	 * we don't hold the page queue lock
  ...
  //	dbgTrace(0xAEAEAEAE, VM_PAGE_GET_PHYS_PAGE(m), 0);		/*
  //(BRINGUP) */
 ...
 */
void vm_page_part_copy(vm_page_t src_m, vm_offset_t src_pa, vm_page_t dst_m,
                       vm_offset_t dst_pa, vm_size_t len) {
#if 0
	/*
	 * we don't hold the page queue lock
	 * so this check isn't safe to make
...
void vm_page_copy(vm_page_t src_m, vm_page_t dest_m) {
  ...
  if (src_m_object != VM_OBJECT_NULL && src_m_object->code_signed) {
    /*
...
#if DEVELOPMENT || DEBUG
    DTRACE_VM4(codesigned_copy, vm_object_t, src_m_object, vm_object_offset_t,
               src_m->vmp_offset, int, src_m->vmp_cs_validated, int,
               src_m->vmp_cs_tainted);
#endif /* DEVELOPMENT || DEBUG */
  }
  ...
  dest_m->vmp_error =
      VMP_ERROR_GET(src_m); /* sliding src_m might have failed... */
  pmap_copy_page(VM_PAGE_GET_PHYS_PAGE(src_m), VM_PAGE_GET_PHYS_PAGE(dest_m),
                 options);
}

#if MACH_ASSERT
static void _vm_page_print(vm_page_t p) {
  printf("vm_page %p: \n", p);
  ...
  printf("  %slaundry, %sref, %sgobbled, %sprivate\n",
         (p->vmp_laundry ? "" : "!"), (p->vmp_reference ? "" : "!"),
         (p->vmp_gobbled ? "" : "!"), (vm_page_is_private(p) ? "" : "!"));
  printf("  %sbusy, %swanted, %stabled, %sfictitious, %spmapped, %swpmapped\n",
         (p->vmp_busy ? "" : "!"), (p->vmp_wanted ? "" : "!"),
         (p->vmp_tabled ? "" : "!"), (vm_page_is_fictitious(p) ? "" : "!"),
         (p->vmp_pmapped ? "" : "!"), (p->vmp_wpmapped ? "" : "!"));
  printf("  %sfree_when_done, %sabsent, %serror, %sdirty, %scleaning, "
         "%sprecious, %sclustered\n",
         (p->vmp_free_when_done ? "" : "!"), (p->vmp_absent ? "" : "!"),
         (VMP_ERROR_GET(p) ? "" : "!"), (p->vmp_dirty ? "" : "!"),
         (p->vmp_cleaning ? "" : "!"), (p->vmp_precious ? "" : "!"),
         (p->vmp_clustered ? "" : "!"));
  printf("  %soverwriting, %srestart, %sunusual\n",
         (p->vmp_overwriting ? "" : "!"), (p->vmp_restart ? "" : "!"),
         (p->vmp_unusual ? "" : "!"));
  printf("  cs_validated=%d, cs_tainted=%d, cs_nx=%d, %sno_cache\n",
         p->vmp_cs_validated, p->vmp_cs_tainted, p->vmp_cs_nx,
         (p->vmp_no_cache ? "" : "!"));
 ...
 */
static int vm_page_verify_contiguous(vm_page_t pages, unsigned int npages) {
  vm_page_t m;
...
static unsigned int
vm_page_verify_free_list(vm_page_queue_head_t *vm_page_queue,
                         unsigned int color, vm_page_t look_for_page,
  ...
  vm_page_queue_iterate(vm_page_queue, m, vmp_pageq) {
    ...
    if ((vm_page_t)VM_PAGE_UNPACK_PTR(m->vmp_pageq.prev) != prev_m) {
      panic("vm_page_verify_free_list(color=%u, npages=%u): page %p corrupted "
            "prev ptr %p instead of %p",
            color, npages, m, (vm_page_t)VM_PAGE_UNPACK_PTR(m->vmp_pageq.prev),
            prev_m);
    }
    if (!m->vmp_busy) {
    ...
    if (color != (unsigned int)-1) {
      if (VM_PAGE_GET_COLOR(m) != color) {
        panic("vm_page_verify_free_list(color=%u, npages=%u): page %p wrong "
              "color %u instead of %u",
              color, npages, m, VM_PAGE_GET_COLOR(m), color);
      }
      if (m->vmp_q_state != VM_PAGE_ON_FREE_Q) {
        panic("vm_page_verify_free_list(color=%u, npages=%u): page %p - "
              "expecting q_state == VM_PAGE_ON_FREE_Q, found %d",
              color, npages, m, m->vmp_q_state);
      }
    } else {
      if (m->vmp_q_state != VM_PAGE_ON_FREE_LOCAL_Q) {
        panic("vm_page_verify_free_list(npages=%u): local page %p - expecting "
              "q_state == VM_PAGE_ON_FREE_LOCAL_Q, found %d",
  ...
  if (look_for_page != VM_PAGE_NULL) {
    ...
    if (expect_page && !found_page) {
      printf("vm_page_verify_free_list(color=%u, npages=%u): page %p not found "
             "phys=%u\n",
             color, npages, look_for_page,
             VM_PAGE_GET_PHYS_PAGE(look_for_page));
      _vm_page_print(look_for_page);
      for (other_color = 0; other_color < vm_colors; other_color++) {
        if (other_color == color) {
          continue;
        }
        vm_page_verify_free_list(
            &vm_page_queue_free.vmpfq_queues[other_color].qhead, other_color,
            look_for_page, FALSE);
      }
#if XNU_VM_HAS_LOPAGE
      if (color == (unsigned int)-1) {
        vm_page_verify_free_list(&vm_lopage_queue_free, (unsigned int)-1,
                                 look_for_page, FALSE);
      }
#endif /* XNU_VM_HAS_LOPAGE */
    ...
    if (!expect_page && found_page) {
      printf("vm_page_verify_free_list(color=%u, npages=%u): page %p found "
             "phys=%u\n",
             color, npages, look_for_page,
             VM_PAGE_GET_PHYS_PAGE(look_for_page));
    }
...
static void vm_page_verify_free_lists(void) {
  ...
  for (color = 0; color < vm_colors; color++) {
    npages +=
        vm_page_verify_free_list(&vm_page_queue_free.vmpfq_queues[color].qhead,
                                 color, VM_PAGE_NULL, FALSE);
  }
#if XNU_VM_HAS_LOPAGE
  nlopages = vm_page_verify_free_list(&vm_lopage_queue_free, (unsigned int)-1,
                                      VM_PAGE_NULL, FALSE);
#endif /* XNU_VM_HAS_LOPAGE */
 ...
 * is in an object.
 */
boolean_t vm_page_is_relocatable(vm_page_t m,
 ...
 */
kern_return_t vm_page_relocate(vm_page_t m1, int *compressed_pages,
                               vm_relocate_reason_t reloc_reason,
                               vm_page_t *new_page) {
  int refmod = 0;
  ...
  if (VM_PAGE_WIRED(m1) || m1->vmp_gobbled || m1->vmp_laundry ||
      m1->vmp_wanted || m1->vmp_cleaning || m1->vmp_overwriting ||
      m1->vmp_free_when_done || m1->vmp_busy ||
  ...
  if ((m1->vmp_pmapped && !reusable) || m1->vmp_dirty || m1->vmp_precious) {
     ...
     */
    vm_page_insert_internal(m2, object, offset, VM_KERN_MEMORY_NONE, TRUE, TRUE,
                            FALSE, FALSE, NULL);
    ...
    /* unset the busy flag (pages on the free queue are busy) and notify if
     * wanted */
 ...
 *	ordered, ascending list... this is currently true of all our
 * implementations and must remain so... there can be 'holes' in the array...
 * we also can no longer tolerate the vm_page_t's in the list being 'freed' and
 * reclaimed which use to happen via 'vm_page_convert'... that function was no
 * longer being called and was removed...
 *
 *	The basic flow consists of stabilizing some of the interesting state of
 *	a vm_page_t behind the vm_page_queue and vm_page_free locks... we start
 * our sweep at the beginning of the array looking for pages that meet our
 * criterea for a 'stealable' page... currently we are pretty conservative... if
 * the page meets this criterea and is physically contiguous to the previous
 * page in the 'run' we keep developing it.  If we hit a page that doesn't fit,
 * we reset our state and start to develop a new run... if at this point we've
 * already considered at least MAX_CONSIDERED_BEFORE_YIELD pages, we'll drop the
 * 2 locks we hold, and mutex_pause (which will yield the processor), to keep
 * the latency low w/r to other threads trying to acquire free pages (or move
 * pages from q to q), and then continue from the spot we left off... we only
 * make 1 pass through the array.  Once we have a 'run' that is long enough,
 * we'll go into the loop which which steals the pages from the queues they're
 * currently on... pages on the free queue can be stolen directly... pages that
 * are on any of the other queues must be removed from the object they are
 * tabled on... this requires taking the object lock... we do this as a 'try' to
 * prevent deadlocks... if the 'try' fails or if the state of the page behind
 * the vm_object lock is no longer viable, we'll dump the pages we've currently
 * stolen back to the free list, and pick up our scan from the point where we
 * aborted the 'current' run.
 *
 *
 *	Requirements:
 *		- neither vm_page_queue nor vm_free_list lock can be held on
 * entry
 *
...
static unsigned int vm_page_find_contiguous_last_idx = 0,
                    vm_page_lomem_find_contiguous_last_idx = 0;
#if DEBUG
int vm_page_find_contig_debug = 0;
#endif

static vm_page_t vm_page_find_contiguous(unsigned int contig_pages,
...
full_scan_again:
  ...
  for (page_idx = last_idx, start_idx = last_idx;
       npages < contig_pages && page_idx < vm_pages_count; page_idx++) {
    vm_page_t m = NULL;

  retry:
    if (wrapped && npages == 0 && page_idx >= orig_last_idx) {
      /*
    ...
    if (!npages & ((VM_PAGE_GET_PHYS_PAGE(m) & pnum_mask) != 0)) {
       ...
       */
      RESET_STATE_OF_RUN();
    } else if (!vm_page_is_relocatable(m, VM_RELOCATE_REASON_CONTIGUOUS)) {
      /*
    ...
    while (start_idx++ < page_idx) {
      ...
      if (m1->vmp_object == 0) {
        ...
        kr = vm_page_relocate(m1, &compressed_pages,
                              VM_RELOCATE_REASON_CONTIGUOUS, NULL);
        if (kr != KERN_SUCCESS) {
          if (locked_object) {
            vm_object_unlock(locked_object);
            locked_object = VM_OBJECT_NULL;
...
done_scanning:
  ...
  if (vm_page_find_contig_debug) {
    printf("%s(num=%d,low=%d): found %d pages at 0x%llx in %ld.%06ds...  "
           "started at %d...  scanned %d pages...  yielded %d times...  dumped "
           "run %d times... stole %d pages... stole %d compressed pages\n",
           __func__, contig_pages, max_pnum, npages,
           (vm_object_offset_t)start_pnum << PAGE_SHIFT, (long)tv_end_sec,
           tv_end_usec, orig_last_idx, scanned, yielded, dumped_run,
           stolen_pages, compressed_pages);
  }
  ...
  if (list.vmpl_count == 0 && zone_gc_called < 2) {
    printf("%s(num=%d,low=%d): found %d pages at 0x%llx...scanned %d pages...  "
           "yielded %d times...  dumped run %d times... stole %d pages... "
           "stole %d compressed pages... wired count is %d\n",
           __func__, contig_pages, max_pnum, npages,
           (vm_object_offset_t)start_pnum << PAGE_SHIFT, scanned, yielded,
           dumped_run, stolen_pages, compressed_pages, vm_page_wire_count);
 ...
 */
kern_return_t cpm_allocate(vm_size_t size, vm_page_t *list, ppnum_t max_pnum,
                           ppnum_t pnum_mask, boolean_t wire, int flags) {
  vm_page_t pages;
  unsigned int npages;
 ...
 * then call vm_page_do_delayed_work to acquire the vm_page_queue_lock and do
 * the necessary work for each page... we will grab the busy bit on the page if
 * it's not already held so that vm_page_do_delayed_work can drop the object
 * lock if it can't immediately take the vm_page_queue_lock in order to compete
...
kern_return_t vm_page_do_delayed_work(vm_object_t object, vm_tag_t tag,
  ...
  for (j = 0; j < dw_count; j++, dwp++) {
    ...
    if (dwp->dw_mask & DW_vm_page_wire) {
      ...
      if (dwp->dw_mask & DW_vm_page_iopl_wire) {
         ...
         * in the same place that we set vmp_iopl_wired, because this
         ...
         */
        if ((dwp->dw_mask & DW_vm_page_iopl_wire_write) &&
            PMAP_PAGE_IS_USER_EXECUTABLE(m)) {
          if (kr == KERN_SUCCESS) {
            kr = KERN_PROTECTION_FAILURE;
            vm_map_guard_exception(VM_PAGE_GET_PHYS_PAGE(m),
                                   kGUARD_EXC_SEC_IOPL_ON_EXEC_PAGE);
            ktriage_record(
      ...
      queueit =
          (dwp->dw_mask & (DW_vm_page_free | DW_vm_page_deactivate_internal))
              ? FALSE
              : TRUE;
...
__abortlike static void __vm_page_alloc_list_failed_panic(vm_size_t page_count,
                                                          kma_flags_t flags,
                                                          kern_return_t kr) {
...
kern_return_t vm_page_alloc_list(vm_size_t page_count, kma_flags_t flags,
                                 vm_page_t *list) {
  ...
  for (vm_size_t i = 0; i < page_count; i++) {
    for (;;) {
      ...
      /* VM privileged threads should have waited in vm_page_grab() and not get
       * here. */
      assert(!(current_thread()->options & TH_OPT_VMPRIV));

      if ((flags & KMA_NOFAIL) == 0 && ptoa_64(page_count) > max_mem / 4) {
        uint64_t unavailable =
            ptoa_64(vm_page_wire_count + vm_page_free_target);
        if (unavailable > max_mem ||
            ptoa_64(page_count) > (max_mem - unavailable)) {
          kr = KERN_RESOURCE_SHORTAGE;
          goto out;
        }
      }
...
out:
  task = current_task_early();
  if (task != NULL) {
    ledger_credit(task->ledger, task_ledgers.pages_grabbed_kern,
                  page_grab_count);
  }
  counter_add(&vm_page_grab_count_kern, page_grab_count);
...
void vm_page_set_offset(vm_page_t page, vm_object_offset_t offset) {
  page->vmp_offset = offset;
}

vm_page_t vm_page_get_next(vm_page_t page) { return page->vmp_snext; }

vm_object_offset_t vm_page_get_offset(vm_page_t page) {
  return page->vmp_offset;
}

ppnum_t vm_page_get_phys_page(vm_page_t page) {
...
static uint32_t hibernate_teardown_vm_structs(hibernate_page_list_t *,
 ...
 *
 * We use a size value instead of pages because we don't want to take up more
 * space on disk if the system has a 16K page size vs 4K. Also, we are not
 * guaranteed to have that additional space available.
 *
...
static int hibernate_drain_pageout_queue(struct vm_pageout_queue *q) {
  ...
  while (!vm_page_queue_empty(&q->pgo_pending)) {
    ...
    assert_wait_timeout((event_t)(&q->pgo_laundry + 1), THREAD_INTERRUPTIBLE,
    ...
    if (wait_result == THREAD_TIMED_OUT &&
        !vm_page_queue_empty(&q->pgo_pending)) {
...
static int hibernate_flush_queue(vm_page_queue_head_t *q, int qcount) {
  vm_page_t m;
  ...
  while (qcount && !vm_page_queue_empty(q)) {
    ...
    if (!m_object->alive || m->vmp_cleaning || m->vmp_laundry || m->vmp_busy ||
        m->vmp_absent || VMP_ERROR_GET(m)) {
      /*
    ...
    if (tq != NULL) {
      ...
      while (retval == 0) {
        ...
        assert_wait_timeout((event_t)&tq->pgo_laundry, THREAD_INTERRUPTIBLE,
    ...
    if (m_object->internal == TRUE) {
      pmap_disconnect_options(VM_PAGE_GET_PHYS_PAGE(m), PMAP_OPTIONS_COMPRESSOR,
                              NULL);
    }
  ...
  KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 4) | DBG_FUNC_END,
                        hibernate_stats.hibernate_found_dirty, retval, 0, 0, 0);

  return retval;
}

static int hibernate_flush_dirty_pages(int pass) {
  struct vm_speculative_age_q *aq;
  uint32_t i;

  if (vm_page_local_q) {
    zpercpu_foreach_cpu(lid) { vm_page_reactivate_local(lid, TRUE, FALSE); }
  }
  ...
  for (i = 0; i <= vm_page_max_speculative_age_q; i++) {
    ...
    vm_page_queue_iterate(&aq->age_q, m, vmp_pageq) { qcount++; }
    vm_page_unlock_queues();
  ...
  if (hibernate_flush_queue(&vm_page_queue_inactive,
                            vm_page_inactive_count - vm_page_anonymous_count -
                                vm_page_cleaned_count)) {
    return 1;
  }
  /* XXX FBDP TODO: flush secluded queue */
  if (hibernate_flush_queue(&vm_page_queue_anonymous,
  ...
  if (hibernate_skip_external == FALSE &&
...
void hibernate_reset_stats(void) {
  bzero(&hibernate_stats, sizeof(struct hibernate_statistics));
}

int hibernate_flush_memory(void) {
  int retval;

  assert(VM_CONFIG_COMPRESSOR_IS_PRESENT);

  KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 3) | DBG_FUNC_START,
                        vm_page_free_count, 0, 0, 0, 0);

  hibernate_cleaning_in_progress = TRUE;
  hibernate_skip_external = FALSE;

  if ((retval = hibernate_flush_dirty_pages(1)) == 0) {
    KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 10) | DBG_FUNC_START,
                          VM_PAGE_COMPRESSOR_COUNT, 0, 0, 0, 0);

    vm_compressor_flush();

    KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 10) | DBG_FUNC_END,
                          VM_PAGE_COMPRESSOR_COUNT, 0, 0, 0, 0);

    if (consider_buffer_cache_collect != NULL) {
      unsigned int orig_wire_count;

      KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 7) | DBG_FUNC_START, 0,
                            0, 0, 0, 0);
      ...
      KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 7) | DBG_FUNC_END,
                            orig_wire_count - vm_page_wire_count, 0, 0, 0, 0);
    }
  }
  hibernate_cleaning_in_progress = FALSE;

  KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 3) | DBG_FUNC_END,
                        vm_page_free_count,
                        hibernate_stats.hibernate_found_dirty, retval, 0, 0);

  if (retval) {
    HIBLOG("hibernate_flush_memory() failed to finish - "
           "vm_page_compressor_count(%d)\n",
           VM_PAGE_COMPRESSOR_COUNT);
  }

  HIBPRINT("hibernate_flush_memory() considered(%d) reentered_on_q(%d) "
           "found_dirty(%d)\n",
           hibernate_stats.hibernate_considered,
           hibernate_stats.hibernate_reentered_on_q,
           hibernate_stats.hibernate_found_dirty);
  HIBPRINT("   skipped_cleaning(%d) skipped_transient(%d) skipped_precious(%d) "
           "skipped_external(%d) queue_nolock(%d)\n",
           hibernate_stats.hibernate_skipped_cleaning,
           hibernate_stats.hibernate_skipped_transient,
           hibernate_stats.hibernate_skipped_precious,
           hibernate_stats.hibernate_skipped_external,
           hibernate_stats.hibernate_queue_nolock);
  HIBPRINT("   queue_paused(%d) throttled(%d) throttle_timeout(%d) drained(%d) "
           "drain_timeout(%d)\n",
           hibernate_stats.hibernate_queue_paused,
           hibernate_stats.hibernate_throttled,
           hibernate_stats.hibernate_throttle_timeout,
...
static void hibernate_page_list_zero(hibernate_page_list_t *list) {
  uint32_t bank;
...
static boolean_t hibernate_consider_discard(vm_page_t m, boolean_t preflight) {
  ...
  do {
     ...
     */
    discard = (!m->vmp_dirty) || (VM_PURGABLE_VOLATILE == object->purgable) ||
...
static void hibernate_discard_page(vm_page_t m) {
  vm_object_t m_object;
 ...
 */
void hibernate_vm_lock_queues(void) {
  ...
  if (vm_page_local_q) {
    zpercpu_foreach(lq, vm_page_local_q) { VPL_LOCK(&lq->vpl_lock); }
  }
}

void hibernate_vm_unlock_queues(void) {
  if (vm_page_local_q) {
    zpercpu_foreach(lq, vm_page_local_q) { VPL_UNLOCK(&lq->vpl_lock); }
  }
...
#if CONFIG_SPTM
static bool hibernate_sptm_should_force_page_to_wired_pagelist(vm_page_t vmp) {
...
/*
 *  Bits zero in the bitmaps => page needs to be saved. All pages default to be
 * saved, pages known to VM to not need saving are subtracted. Wired pages to be
 * saved are present in page_list_wired, pageable in page_list.
 */

void hibernate_page_list_setall(hibernate_page_list_t *page_list,
                                hibernate_page_list_t *page_list_wired,
                                hibernate_page_list_t *page_list_pal,
                                boolean_t preflight, boolean_t will_discard,
                                uint32_t *pagesOut) {
  ...
  KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 8) | DBG_FUNC_START,
                        count_wire, 0, 0, 0, 0);
  ...
  if (!preflight) {
    percpu_foreach(free_pages_head, free_pages) {
      _vm_page_list_foreach(m, *free_pages_head) {
  ...
  if (vm_pages_free_masks()) {
    ...
    for (int bit = bitmap_first(map, bits); bit >= 0;
         bit = bitmap_next(map, bit)) {
  ...
  while (m && !vm_page_queue_end(&vm_page_queue_throttled,
                                 (vm_page_queue_entry_t)m)) {
    assert(m->vmp_q_state == VM_PAGE_ON_THROTTLED_Q);
  ...
  while (m && !vm_page_queue_end(&vm_page_queue_anonymous,
                                 (vm_page_queue_entry_t)m)) {
    assert(m->vmp_q_state == VM_PAGE_ON_INACTIVE_INTERNAL_Q);
    bool force_to_wired_list =
        false; /* Default to NOT forcing page into the wired page list */
#if CONFIG_SPTM
    ...
    if ((kIOHibernateModeDiscardCleanInactive & gIOHibernateMode) &&
      ...
      /*
       * If the page must be force-added to the wired page list, prevent it from
       * appearing in the unwired page list.
       */
    ...
    /*
     * If the page is NOT being forced into the wired page list, remove it from
     * the wired page list here.
     */
    if (!force_to_wired_list) {
      count_wire--;
      if (!preflight) {
  ...
  while (m &&
         !vm_page_queue_end(&vm_page_queue_cleaned, (vm_page_queue_entry_t)m)) {
  ...
  while (m &&
         !vm_page_queue_end(&vm_page_queue_active, (vm_page_queue_entry_t)m)) {
  ...
  while (m && !vm_page_queue_end(&vm_page_queue_inactive,
                                 (vm_page_queue_entry_t)m)) {
  ...
  for (uint32_t i = 0; i <= vm_page_max_speculative_age_q; i++) {
    ...
    while (m && !vm_page_queue_end(&vm_page_queue_speculative[i].age_q,
                                   (vm_page_queue_entry_t)m)) {
      assertf(m->vmp_q_state == VM_PAGE_ON_SPECULATIVE_Q,
              "Bad page: %p (0x%x:0x%x) on queue %d has state: %d (Discard: "
              "%d, Preflight: %d)",
              m, m->vmp_pageq.next, m->vmp_pageq.prev, i, m->vmp_q_state,
              discard, preflight);
  ...
  if (preflight == FALSE && discard_all == TRUE) {
    ...
    count_discard_vm_struct_pages =
        hibernate_teardown_vm_structs(page_list, page_list_wired);
  ...
  if (!preflight) {
    ...
    for (bank = 0; bank < page_list->bank_count; bank++) {
      ...
      bitmap = (hibernate_bitmap_t *)&bitmap->bitmap[bitmap->bitmapwords];
      bitmap_wired = (hibernate_bitmap_t *)&bitmap_wired
                         ->bitmap[bitmap_wired->bitmapwords];
    }
  }

  // machine dependent adjustments
  hibernate_page_list_setall_machine(page_list, page_list_wired, preflight,
                                     &pages);

  if (!preflight) {
    hibernate_stats.cd_count_wire = count_wire;
    hibernate_stats.cd_discarded =
        count_discard_active + count_discard_inactive +
        count_discard_purgeable + count_discard_speculative +
        count_discard_cleaned + count_discard_vm_struct_pages;
  }
  ...
  HIBLOG("pages %d, wire %d, act %d, inact %d, cleaned %d spec %d, "
         ...
         pages, count_wire, count_active, count_inactive, count_cleaned,
         count_speculative, count_anonymous, count_throttled, count_compressor,
         hibernate_stats.cd_found_xpmapped, discard_all ? "did" : "could",
  ...
  *pagesOut = pages - count_discard_active - count_discard_inactive -
  ...
  if (preflight && will_discard) {
    *pagesOut -= count_compressor + count_throttled + count_anonymous +
                 count_inactive + count_cleaned + count_speculative +
                 count_active;

    /*
     * We try to keep max HIBERNATE_XPMAPPED_LIMIT pages around in the
     * hibernation image even if these are clean and so we need to size the
     * hibernation image accordingly.
     *
     * NB: We have to assume all HIBERNATE_XPMAPPED_LIMIT pages might show up
     * because 'dirty' xpmapped pages aren't distinguishable from other 'dirty'
     * pages in preflight. So we might only see part of the xpmapped pages if we
     * look at 'cd_found_xpmapped' which solely tracks clean xpmapped pages.
     *
     * Since these pages are all cleaned by the time we are in the
     * post-preflight phase, we might see a much larger number in
     * 'cd_found_xpmapped' now than we did in the preflight phase
     */
  ...
  KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 8) | DBG_FUNC_END,
                        count_wire, *pagesOut, 0, 0, 0);
}

void hibernate_page_list_discard(hibernate_page_list_t *page_list) {
  uint64_t start, end, nsec;
  vm_page_t m;
  vm_page_t next;
  uint32_t i;
  ...
  HIBLOG("hibernate_page_list_discard time: %qd ms, discarded act %d inact %d "
         "purgeable %d spec %d cleaned %d\n",
         nsec / 1000000ULL, count_discard_active, count_discard_inactive,
         count_discard_purgeable, count_discard_speculative,
         count_discard_cleaned);
}
...
void hibernate_create_paddr_map(void) {
  unsigned int i;
  ...
  if (hibernate_paddr_map_inited == FALSE) {
    for (i = 0; i < vm_pages_count; i++) {
      ...
      if (ppnm == NULL ||
          VM_PAGE_GET_PHYS_PAGE(vm_page_get(i)) != next_ppnum_in_run) {
...
static ppnum_t hibernate_lookup_paddr(unsigned int indx) {
  struct ppnum_mapping *ppnm = NULL;
...
static uint32_t
hibernate_mark_as_unneeded(addr64_t saddr, addr64_t eaddr,
...
static void hibernate_hash_insert_page(vm_page_t mem) {
  vm_page_bucket_t *bucket;
  int hash_id;
  vm_object_t m_object;
...
static void hibernate_free_range_flush(vm_page_list_t *list) {
  vm_page_free_queue_enter_list(*list, VMP_RELEASE_HIBERNATE);
  *list = (vm_page_list_t){};
}

static void hibernate_free_range(vm_page_list_t *list, int sindx, int eindx) {
...
void hibernate_rebuild_vm_structs(void) {
  ...
  for (cindx = hibernate_teardown_last_valid_compact_indx; cindx >= 0;
       cindx--) {
...
static uint32_t
hibernate_teardown_vm_structs(hibernate_page_list_t *page_list,
                              hibernate_page_list_t *page_list_wired) {
  ...
  HIBLOG("hibernate_teardown: wired_pages %d, free_pages %d, "
         ...
         vm_page_wire_count, vm_page_free_count, vm_page_active_count,
         vm_page_inactive_count, vm_page_speculative_count,
  ...
  unneeded_vm_page_bucket_pages = hibernate_mark_as_unneeded(
      (addr64_t)&vm_page_buckets[0],
      (addr64_t)&vm_page_buckets[vm_page_bucket_count], page_list,
      page_list_wired);
  ...
  for (uint32_t i = 0; i < vm_pages_count; i++) {
    ...
    if (mem->vmp_q_state == VM_PAGE_ON_FREE_Q) {
      vm_page_free_queue_remove(class, mem, pnum, VM_PAGE_ON_FREE_Q);
  ...
  unneeded_vm_pages_pages = hibernate_mark_as_unneeded(
      ...
      (addr64_t)vm_page_get(vm_pages_count - 1), page_list, page_list_wired);
  ...
  if (start_of_unneeded) {
    unneeded_pmap_pages = hibernate_mark_as_unneeded(
        start_of_unneeded, end_of_unneeded, page_list, page_list_wired);
    mark_as_unneeded_pages += unneeded_pmap_pages;
  }
  HIBLOG("hibernate_teardown: mark_as_unneeded_pages %d, %d, %d\n",
         unneeded_vm_page_bucket_pages, unneeded_vm_pages_pages,
         unneeded_pmap_pages);
...
unsigned int vm_page_info(hash_info_bucket_t *info, unsigned int count) {
  unsigned int i;
  lck_ticket_t *bucket_lock;
...
#if VM_PAGE_BUCKETS_CHECK
void vm_page_buckets_check(void) {
  unsigned int i;
  vm_page_t p;
  unsigned int p_hash;
  vm_page_bucket_t *bucket;
...
#if HIBERNATION
  if (hibernate_rebuild_needed || hibernate_rebuild_hash_list) {
    panic("BUCKET_CHECK: hibernation in progress: "
          "rebuild_needed=%d rebuild_hash_list=%p\n",
          hibernate_rebuild_needed, hibernate_rebuild_hash_list);
  }
#endif /* HIBERNATION */
  ...
  for (cp = (char *)vm_page_fake_buckets_start;
    ...
    if (*cp != 0x5a) {
      panic("BUCKET_CHECK: corruption at %p in fake buckets "
            "[0x%llx:0x%llx]\n",
            cp, (uint64_t)vm_page_fake_buckets_start,
    ...
    while (p != VM_PAGE_NULL) {
      ...
      if (!p->vmp_hashed) {
        panic("BUCKET_CHECK: page %p (%p,0x%llx) "
              ...
              "is not hashed\n",
              p, p_object, p->vmp_offset, p_hash, i, bucket);
      }
      ...
      if (p_hash != i) {
        panic("BUCKET_CHECK: corruption in bucket %d "
              ...
              "hash %d\n",
              i, bucket, p, p_object, p->vmp_offset, p_hash);
      }
 ...
 * local queues if they exist... its the only spot in the system where we add
 * pages to those queues...  once on those queues, those pages can only move to
 * one of the global page queues or the free queues... they NEVER move from
 * local q to local q. the 'local' state is stable when vm_page_queues_remove is
 * called since we're behind the global vm_page_queue_lock at this point...  we
 * still need to take the local lock in case this operation is being run on a
 * different CPU then the local queue's identity, but we don't have to worry
 * about the page moving to a global queue or becoming wired while we're
 * grabbing the local lock since those operations would require the global
 * vm_page_queue_lock to be held, and we already own it.
 *
 * this is why its safe to utilze the wire_count field in the vm_page_t as the
 * local_id... 'wired' and local are ALWAYS mutually exclusive conditions.
 */

void vm_page_queues_remove(vm_page_t mem, boolean_t remove_from_specialq) {
  ...
  default: {
     ...
     *              NOTE: vm_page_queues_remove does not deal with removing
     * pages from the pageout queue... the caller is responsible for determing
     * if the page is on that queue, and if so, must either first remove it (it
     * needs both the page queues lock and the object lock to do this via
     * vm_pageout_steal_laundry), or avoid the call to vm_page_queues_remove
     *
     *	we also don't expect to encounter VM_PAGE_ON_FREE_Q,
     * VM_PAGE_ON_FREE_LOCAL_Q, VM_PAGE_ON_FREE_LOPAGE_Q or any of the undefined
     * states
     */
    panic("vm_page_queues_remove - bad page q_state (%p, %d)", mem,
          mem->vmp_q_state);
    break;
  }
  }
...
void vm_page_remove_internal(vm_page_t page) {
...
void vm_page_enqueue_inactive(vm_page_t mem, boolean_t first) {
  vm_object_t m_object;
...
void vm_page_enqueue_active(vm_page_t mem, boolean_t first) {
  vm_object_t m_object;
 ...
 */
void vm_page_check_pageable_safe(vm_page_t page) {
  ...
  if (page_object == compressor_object) {
    panic("vm_page_check_pageable_safe: trying to add page"
          "from compressor object (%p) to pageable queue",
          compressor_object);
  }
}
...
#define KA_SIZE(namelen, subtotalscount)                                       \
  (sizeof(struct vm_allocation_site) + (namelen) + 1 +                         \
   ((subtotalscount) * sizeof(struct vm_allocation_total)))

#define KA_NAME(alloc) ((char *)(&(alloc)->subtotals[(alloc->subtotalscount)]))

#define KA_NAME_LEN(alloc)                                                     \
  (VM_TAG_NAME_LEN_MAX & (alloc->flags >> VM_TAG_NAME_LEN_SHIFT))

vm_tag_t vm_tag_bt(void) {
  uintptr_t *frameptr;
...
#if defined(HAS_APPLE_PAC)
    retaddr =
        (uintptr_t)ptrauth_strip((void *)retaddr, ptrauth_key_return_address);
#endif
...
void vm_tag_alloc_locked(vm_allocation_site_t *site,
...
static void vm_tag_free_locked(vm_tag_t tag) {
  uint64_t avail;
  uint32_t idx;
  uint64_t bit;
...
vm_tag_t vm_tag_alloc(vm_allocation_site_t *site) {
...
#endif /* ARRAY_SIZE */
#define VM_KERN_MEMORY_ELEM(name)                                              \
  [VM_KERN_MEMORY_##name] = "VM_KERN_MEMORY_" #name
...
static void vm_tag_log(vm_object_t object, int64_t delta, void *fp) {
  if (is_kernel_object(object)) {
...
static vm_tag_t vm_tag_str_to_idx(char tagstr[VM_KERN_MEMORY_STR_MAX_LEN]) {
  for (vm_tag_t i = VM_KERN_MEMORY_OSFMK; i < ARRAY_SIZE(vm_kern_memory_names);
       i++) {
...
__startup_func static void vm_btlog_init(void) {
...
void kern_allocation_update_size(kern_allocation_name_t allocation,
                                 int64_t delta, __unused vm_object_t object) {
  uint64_t value;

  value = os_atomic_add(&allocation->total, delta, relaxed);
  if (delta < 0) {
    assertf(value + (uint64_t)-delta > value, "tag %d, site %p",
...
void vm_tag_reset_all_peaks(void) {
...
kern_return_t vm_tag_reset_peak(vm_tag_t tag) {
  if (tag > vm_allocation_tag_highest) {
    return KERN_INVALID_ARGUMENT;
  }
...
void vm_allocation_zones_init(void) {
  vm_offset_t addr;
  vm_size_t size;

  const vm_tag_t early_tags[] = {
      VM_KERN_MEMORY_DIAG,        VM_KERN_MEMORY_KALLOC,
      VM_KERN_MEMORY_KALLOC_DATA, VM_KERN_MEMORY_KALLOC_TYPE,
      VM_KERN_MEMORY_LIBKERN,     VM_KERN_MEMORY_OSFMK,
      VM_KERN_MEMORY_RECOUNT,
  };

  size = VM_MAX_TAG_VALUE * sizeof(vm_allocation_zone_total_t **) +
         ARRAY_COUNT(early_tags) * VM_TAG_SIZECLASSES *
  ...
  for (size_t i = 0; i < ARRAY_COUNT(early_tags); i++) {
    vm_allocation_zone_totals[early_tags[i]] =
        (vm_allocation_zone_total_t *)addr;
    addr += VM_TAG_SIZECLASSES * sizeof(vm_allocation_zone_total_t);
  }
}

__attribute__((noinline)) static vm_tag_t
vm_tag_zone_stats_alloc(vm_tag_t tag, zalloc_flags_t flags) {
  ...
  if (!os_atomic_cmpxchg(&vm_allocation_zone_totals[tag], NULL, stats,
                         release)) {
...
vm_tag_t vm_tag_will_update_zone(vm_tag_t tag, uint32_t zflags) {
...
void vm_tag_update_zone_size(vm_tag_t tag, uint32_t zidx, long delta) {
  vm_allocation_zone_total_t *stats;
  vm_size_t value;
...
void kern_allocation_update_subtotal(kern_allocation_name_t allocation,
                                     vm_tag_t subtag, int64_t delta) {
  ...
  if (subidx >= allocation->subtotalscount) {
    for (subidx = 0; subidx < allocation->subtotalscount; subidx++) {
      total = &allocation->subtotals[subidx];
      if ((VM_KERN_MEMORY_NONE == total->tag) || !total->total) {
...
const char *kern_allocation_get_name(kern_allocation_name_t allocation) {
  return KA_NAME(allocation);
}

kern_allocation_name_t kern_allocation_name_allocate(const char *name,
...
void kern_allocation_name_release(kern_allocation_name_t allocation) {
...
static void vm_page_count_object(mach_memory_info_t *info,
...
static void vm_page_iterate_purgeable_objects(mach_memory_info_t *info,
                                              unsigned int num_info,
                                              vm_page_iterate_proc proc,
                                              purgeable_q_t queue, int group) {
  vm_object_t object;
...
static void vm_page_iterate_objects(mach_memory_info_t *info,
                                    unsigned int num_info,
                                    vm_page_iterate_proc proc) {
  vm_object_t object;

  lck_spin_lock_grp(&vm_objects_wired_lock, &vm_page_lck_grp_bucket);
  queue_iterate(&vm_objects_wired, object, vm_object_t, wired_objq) {
    proc(info, num_info, object);
  }
...
static uint64_t process_account(mach_memory_info_t *info, unsigned int num_info,
                                uint64_t zones_collectable_bytes,
                                boolean_t iterated, bool redact_info __unused) {
  size_t namelen;
  ...
  for (idx = 0; idx <= vm_allocation_tag_highest; idx++) {
    ...
    if (idx < VM_KERN_MEMORY_FIRST_DYNAMIC) {
      ...
      strlcpy(info[idx].name, vm_kern_memory_names[idx],
              MACH_MEMORY_INFO_NAME_MAX_LEN);
    } else if ((namelen = (VM_TAG_NAME_LEN_MAX &
                           (site->flags >> VM_TAG_NAME_LEN_SHIFT)))) {
      info[idx].site = 0;
      info[idx].flags |= VM_KERN_SITE_NAMED;
  ...
  for (idx = 0; idx < count; idx++) {
    ...
    if (!redact_info && vm_allocation_zone_totals &&
        (zone = vm_allocation_zone_totals[idx]) && (nextinfo < num_info)) {
    ...
    if (site->subtotalscount) {
      ...
      for (sub = 0; sub < site->subtotalscount; sub++) {
        ...
        if (info[alloctag].name[0] &&
            alloctag >= VM_KERN_MEMORY_FIRST_DYNAMIC) {
          continue;
        }
...
static void vm_page_diagnose_zone_stats(mach_memory_info_t *info,
                                        zone_stats_t zstats, bool percpu) {
...
static void vm_page_add_info(mach_memory_info_t *info, zone_stats_t stats,
                             bool per_cpu, const char *parent_heap_name,
                             const char *parent_zone_name,
                             const char *view_name) {
  vm_page_diagnose_zone_stats(info, stats, per_cpu);
  snprintf(info->name, sizeof(info->name), "%s%s[%s]", parent_heap_name,
           parent_zone_name, view_name);
}

static void vm_page_diagnose_zone(mach_memory_info_t *info, zone_t z) {
  vm_page_add_info(info, z->z_stats, z->z_percpu, zone_heap_name(z), z->z_name,
                   "raw");
}

static void vm_page_add_view(mach_memory_info_t *info, zone_stats_t stats,
...
static uint32_t vm_page_diagnose_heap_views(mach_memory_info_t *info,
                                            kalloc_heap_t kh,
                                            const char *parent_heap_name,
                                            const char *parent_zone_name) {
  uint32_t i = 0;
  ...
  while (kh) {
    vm_page_add_view(info + i, kh->kh_stats, parent_heap_name, parent_zone_name,
                     kh->kh_name);
    kh = kh->kh_views;
    i++;
  }
...
static uint32_t vm_page_diagnose_heap(mach_memory_info_t *info,
                                      kalloc_heap_t kheap) {
  uint32_t i = 0;
...
static int vm_page_diagnose_kt_heaps(mach_memory_info_t *info) {
  uint32_t idx = 0;
...
kern_return_t vm_page_diagnose(mach_memory_info_t *info, unsigned int num_info,
                               uint64_t zones_collectable_bytes,
                               bool redact_info) {
  uint64_t wired_size;
...
#define SET_COUNT(xcount, xsize, xflags)                                       \
  ...
  strlcpy(counts[xcount].name, vm_kern_count_names[xcount],                    \
          MACH_MEMORY_INFO_NAME_MAX_LEN);                                      \
  MACRO_END;
  ...
  SET_COUNT(VM_KERN_COUNT_STOLEN, ptoa_64(vm_page_stolen_count),
            VM_KERN_SITE_WIRED);
  SET_COUNT(VM_KERN_COUNT_LOPAGE, ptoa_64(vm_lopage_free_count),
            VM_KERN_SITE_WIRED);
  ...
  SET_COUNT(VM_KERN_COUNT_WIRED_STATIC_KERNELCACHE,
            ptoa_64(vm_page_kernelcache_count), 0);
#if CONFIG_SPTM
...
#define SET_MAP(xcount, xsize, xfree, xlargest)                                \
  ...
  if (!redact_info) {
    if (zone_is_data_buffers_kheap(KHEAP_DATA_BUFFERS->kh_heap_id)) {
    ...
    zone_index_foreach(zidx) {
      ...
      for (; zv; zv = zv->zv_next) {
        ...
        has_raw_view = true;
        vm_page_diagnose_zone_stats(counts + i, zv->zv_stats, z->z_percpu);
    ...
    while (map) {
      ...
      for (entry = map->hdr.links.next; map; entry = entry->vme_next) {
        ...
        if (is_kernel_object(VME_OBJECT(entry))) {
          ...
          for (offset = entry->vme_start; offset < entry->vme_end;
               offset += page_size) {
  ...
  vmlp_api_end(VM_PAGE_DIAGNOSE, KERN_SUCCESS);
  return KERN_SUCCESS;
}

#if DEBUG || DEVELOPMENT

kern_return_t vm_kern_allocation_info(uintptr_t addr, vm_size_t *size,
                                      vm_tag_t *tag, vm_size_t *zone_size) {
  kern_return_t ret;
  vm_size_t zsize;
  vm_map_t map;
  vm_map_entry_t entry;
...
// some DEBUG/DEVELOPMENT code to get a process to page out its shared cache
// TEXT pages, only used for DK driver in LPW testing
uint64_t vm_task_evict_shared_cache(task_t task) {
  enum { kMaxKernelDepth = 3 };
  vm_map_t maps[kMaxKernelDepth];
  vm_map_entry_t entries[kMaxKernelDepth];
  vm_map_t map;
  ...
  while (map) {
    ...
    for (entry = map->hdr.links.next; map; entry = entry->vme_next) {
      ...
      if (stackIdx && (VM_PROT_EXECUTE | VM_PROT_READ) == entry->protection) {
...
uint64_t vm_task_pageins(task_t task) { return counter_load(&task->pageins); }

#endif /* DEBUG || DEVELOPMENT */

uint32_t vm_tag_get_kext(vm_tag_t tag, char *name, vm_size_t namelen) {
  vm_allocation_site_t *site;
  uint32_t kmodId;
...
#if CONFIG_SECLUDED_MEMORY
/*
 * Note that there's no locking around other accesses to
 * vm_page_secluded_target. That should be OK, since these are the only place
 * where it can be changed after initialization. Other users (like vm_pageout)
 * may see the wrong value briefly, but will eventually get the correct value.
 * This brief mismatch is OK as pageout and page freeing will auto-adjust the
 * vm_page_secluded_count to match the target over time.
 */
...
void start_secluded_suppression(task_t task) {
...
void stop_secluded_suppression(task_t task) {
  lck_spin_lock(&secluded_suppress_slock);
  if (task->task_suppressed_secluded && --vm_page_secluded_suppress_cnt == 0) {
 ...
 */
void vm_retire_boot_pages(void) {}
