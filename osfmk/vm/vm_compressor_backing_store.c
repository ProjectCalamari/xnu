...
#define SWAP_REUSE                                                             \
  0x8 /* Swap file is on the Q and has a name. Reuse after init-ing.*/
...
struct swapfile {
  ...
  uint8_t
      *swp_bitmap; /* bitmap showing the alloced/freed slots in the swap file */
  ...
  c_segment_t *swp_csegs; /* back pointers to the c_segments. Used during swap
                             reclaim. */
...
static void vm_swapout_finish(c_segment_t c_seg, uint64_t f_offset,
                              uint32_t size, kern_return_t kr);
...
#define VM_MAX_SWAP_FILE_SWAP_ENABLED_NUM                                      \
  (64ULL * (1ULL << 30) / MAX_SWAP_FILE_SIZE)
#define VM_MIN_SWAP_FILE_SWAP_ENABLED_NUM                                      \
  (16ULL * (1ULL << 30) / MAX_SWAP_FILE_SIZE)
 ...
 */
#define VM_MAX_SWAP_FILE_SWAP_ENABLED_NUM                                      \
  (4ULL * (1ULL << 30) / MAX_SWAP_FILE_SIZE)
#define VM_MIN_SWAP_FILE_SWAP_ENABLED_NUM                                      \
  (4ULL * (1ULL << 30) / MAX_SWAP_FILE_SIZE)
...
#define VM_SWAP_SHOULD_DEFRAGMENT()                                            \
  (((vm_swap_force_defrag == TRUE) ||                                          \
    (c_swappedout_sparse_count > (vm_swapfile_total_segs_used / 16)))          \
       ? 1                                                                     \
       : 0)
#define VM_SWAP_SHOULD_PIN(_size) FALSE
#define VM_SWAP_SHOULD_TRIM(swf)                                               \
  ((swf->swp_delayed_trim_count >= VM_SWAPFILE_DELAYED_TRIM_MAX) ? 1 : 0)
...
#define VM_SWAP_SHOULD_DEFRAGMENT()                                            \
  (((vm_swap_force_defrag == TRUE) ||                                          \
    (c_swappedout_sparse_count > (vm_swapfile_total_segs_used / 4)))           \
       ? 1                                                                     \
       : 0)
#define VM_SWAP_SHOULD_PIN(_size)                                              \
  (vm_swappin_avail > 0 && vm_swappin_avail >= (int64_t)(_size))
#define VM_SWAP_SHOULD_TRIM(swf)                                               \
  ((swf->swp_delayed_trim_count >= VM_SWAPFILE_DELAYED_TRIM_MAX) ? 1 : 0)
...
#define VM_SWAP_SHOULD_RECLAIM()                                               \
  (((vm_swap_force_reclaim == TRUE) ||                                         \
    ((vm_swapfile_total_segs_alloced - vm_swapfile_total_segs_used) >=         \
     swapfile_reclaim_threshold_segs))                                         \
       ? 1                                                                     \
       : 0)
#define VM_SWAP_SHOULD_ABORT_RECLAIM()                                         \
  (((vm_swap_force_reclaim == FALSE) &&                                        \
    ((vm_swapfile_total_segs_alloced - vm_swapfile_total_segs_used) <=         \
     swapfile_reclam_minimum_segs))                                            \
       ? 1                                                                     \
       : 0)

#define VM_SWAP_BUSY()                                                         \
  (((c_early_swapout_count + c_regular_swapout_count +                         \
...
boolean_t c_compressed_record_init_done =
    FALSE; /* was the record file opened? */
int c_compressed_record_write_error = 0;
struct vnode *c_compressed_record_vp =
    NULL; /* the file opened for record write */
...
static struct swapfile *vm_swapfile_for_handle(uint64_t f_offset) {
...
void swap_crypt_initialize(void) {
  ...
  error = xts_start(0, NULL, enckey1, keylen1, enckey2, keylen2, 0, 0,
                    &xts_modectx);
  assert(!error);
  ...
  for (i = 0; i < 4096; i++) {
    if (swap_crypt_test_page_encrypt[i] != swap_crypt_test_page_ref[i]) {
      break;
    }
  }
  assert(i != 4096);
  ...
  for (i = 0; i < 4096; i++) {
    if (swap_crypt_test_page_decrypt[i] != swap_crypt_test_page_ref[i]) {
...
void vm_swap_encrypt(c_segment_t c_seg) {
  uint8_t *ptr;
  uint8_t *iv;
  uint64_t ivnum[2];
  int size = 0;
...
void vm_swap_decrypt(c_segment_t c_seg, bool disallow_page_replacement) {
  uint8_t *ptr;
  uint8_t *iv;
  uint64_t ivnum[2];
  int size = 0;
...
uint64_t compressed_swap_chunk_size, vm_swapfile_hiwater_segs,
    swapfile_reclaim_threshold_segs, swapfile_reclam_minimum_segs;
extern bool memorystatus_swap_all_apps;

void vm_compressor_swap_init_swap_file_limit(void) {
  ...
  if (memorystatus_swap_all_apps) {
    if (vm_swap_volume_capacity == 0) {
       ...
       */
      vm_num_swap_files_config = vm_swap_volume_capacity * kFixedPointFactor /
                                 VM_SWAP_MIN_VOLUME_CAPACITY *
                                 VM_MIN_SWAP_FILE_SWAP_ENABLED_NUM /
                                 kFixedPointFactor;
      vm_num_swap_files_config =
          MAX(vm_num_swap_files_config, VM_MIN_SWAP_FILE_SWAP_ENABLED_NUM);
      vm_num_swap_files_config =
          MIN(vm_num_swap_files_config, VM_MAX_SWAP_FILE_SWAP_ENABLED_NUM);
    }
  }
#endif /* CONFIG_JETSAM */
#if DEVELOPMENT || DEBUG
  typeof(vm_num_swap_files_config) parsed_vm_max_num_swap_files = 0;
  if (PE_parse_boot_argn("vm_max_num_swap_files", &parsed_vm_max_num_swap_files,
    ...
    if (parsed_vm_max_num_swap_files > 0) {
      ...
      printf("WARNING: Ignoring vm_max_num_swap_files=%d boot-arg. Value must "
             "be > 0\n",
...
void vm_compressor_swap_init(void) {
  ...
  swapfile_reclaim_threshold_segs =
      ((17 * (MAX_SWAP_FILE_SIZE / compressed_swap_chunk_size)) / 10);
  swapfile_reclam_minimum_segs =
      ((13 * (MAX_SWAP_FILE_SIZE / compressed_swap_chunk_size)) / 10);
  ...
  if (kernel_thread_start_priority((thread_continue_t)vm_swapfile_create_thread,
                                   NULL, BASEPRI_VM, &thread) != KERN_SUCCESS) {
    panic("vm_swapfile_create_thread: create failed");
  }
  thread_set_thread_name(thread, "VM_swapfile_create");
  thread_deallocate(thread);

  if (kernel_thread_start_priority((thread_continue_t)vm_swapfile_gc_thread,
                                   NULL, BASEPRI_VM, &thread) != KERN_SUCCESS) {
  ...
  proc_set_thread_policy_with_tid(kernel_task, thread->thread_id,
                                  TASK_POLICY_INTERNAL, TASK_POLICY_IO,
                                  THROTTLE_LEVEL_COMPRESSOR_TIER2);
  proc_set_thread_policy_with_tid(kernel_task, thread->thread_id,
                                  TASK_POLICY_INTERNAL, TASK_POLICY_PASSIVE_IO,
                                  TASK_POLICY_ENABLE);
...
void c_compressed_record_write(char *buf, int size) {
  if (c_compressed_record_write_error == 0) {
    c_compressed_record_write_error = vm_record_file_write(
        c_compressed_record_vp, c_compressed_record_file_offset, buf, size);
...
void vm_compaction_swapper_do_init(void) {
  struct vnode *vp;
  char *pathname;
  int namelen;
  ...
  if (!compaction_swapper_inited) {
    ...
    if (vp) {
      if (vnode_pager_isSSD(vp) == FALSE) {
         ...
         * Note that the > relationship between these 4 values must be
         * maintained
         */
...
void vm_swap_consider_defragmenting(int flags) {
  ...
  if (compressor_store_stop_compaction == FALSE && !VM_SWAP_BUSY() &&
      (force_defrag || force_reclaim || VM_SWAP_SHOULD_DEFRAGMENT() ||
       VM_SWAP_SHOULD_RECLAIM())) {
  ...
  while (!queue_empty(&c_swappedout_sparse_list_head)) {
    ...
    if (c_seg->c_bytes_used == 0) {
      ...
      if (freezer_incore_cseg_acct) {
        /*
         * TODO(jason): These two are tricky because they're pre-emptive
         * jetsams. The system is not unhealthy, but we know that it's about to
         * become unhealthy once we do this swapin. So we're waking up the
         * memorystatus thread to make space (hopefully) before this segment
         * comes in.
         *
...
TUNABLE(uint64_t, vm_swapfile_creation_delay_ns,
        "vm_swapfile_creation_delay_ns", 15 * NSEC_PER_SEC);

static inline bool vm_swapfile_should_create(uint64_t now) {
  uint64_t delta_failed_creation_ns;
  absolutetime_to_nanoseconds(now - vm_swapfile_last_failed_to_create_ts,
                              &delta_failed_creation_ns);
...
kern_return_t hibernate_pin_swap(boolean_t start) {
  ...
  if (vm_num_swap_files > vm_num_pinned_swap_files) {
    ...
    HIBLOG("hibernate_pin_swap failed - vm_num_swap_files = %d, "
           "vm_num_pinned_swap_files = %d\n",
...
static void vm_swapout_thread_throttle_adjust(void) {
  ...
  case VM_SWAPOUT_T2_PASSIVE:
    ...
    if (SWAPPER_NEEDS_TO_UNTHROTTLE()) {
      ...
      proc_set_thread_policy_with_tid(
          kernel_task, vm_swapout_thread_id, TASK_POLICY_INTERNAL,
          TASK_POLICY_PASSIVE_IO, TASK_POLICY_ENABLE);
  ...
  case VM_SWAPOUT_T0_PASSIVE:
    ...
    if (SWAPPER_NEEDS_TO_CATCHUP()) {
      ...
      proc_set_thread_policy_with_tid(
          kernel_task, vm_swapout_thread_id, TASK_POLICY_INTERNAL,
          TASK_POLICY_PASSIVE_IO, TASK_POLICY_DISABLE);
...
static struct swapout_io_completion *vm_swapout_find_free_soc(void) {
  int i;
...
static struct swapout_io_completion *vm_swapout_find_done_soc(void) {
  int i;
...
static void vm_swapout_complete_soc(struct swapout_io_completion *soc) {
  kern_return_t kr;
...
should_process_swapout_queue(const queue_head_t *swapout_list_head) {
  ...
  if (memorystatus_swap_all_apps &&
    ...
    process_queue = process_queue &&
                    memorystatus_swap_over_trigger(swapout_sleep_threshold);
  }
#endif /* CONFIG_JETSAM */
...
again:
  while (should_process_swapout_queue(swapout_list_head)) {
    ...
    kr = vm_swap_put((vm_offset_t)c_seg->c_store.c_buffer, &soc->swp_f_offset,
                     size, c_seg, soc);
  ...
  c_seg_is_empty:
    if (!(c_early_swapout_count + c_regular_swapout_count +
          c_late_swapout_count)) {
...
#if XNU_TARGET_OS_OSX
  queues_empty = queue_empty(&c_early_swapout_list_head) &&
...
void vm_swapout_iodone(void *io_context, int error) {
  struct swapout_io_completion *soc;
...
static void vm_swapout_finish(c_segment_t c_seg, uint64_t f_offset,
  ...
  if (kr == KERN_SUCCESS) {
    ...
    __assert_only unsigned int new_swapped_count =
        os_atomic_add(&vm_page_swapped_count, c_seg->c_slots_used, relaxed);
    /* Detect overflow */
     ...
     */
    os_atomic_sub(&c_segment_pages_compressed_incore, c_seg->c_slots_used,
                  relaxed);
    ...
    if (c_seg->c_has_donated_pages) {
      os_atomic_sub(&c_segment_pages_compressed_incore_late_swapout,
...
boolean_t vm_swap_create_file() {
  ...
  while (size >= MIN_SWAP_FILE_SIZE) {
    ...
    if (vm_swapfile_preallocate(swf->swp_vp, &size, &swap_file_pin) == 0) {
       ...
       */
      swf->swp_bitmap = kalloc_data(num_bytes_for_bitmap, Z_WAITOK | Z_ZERO);

      swf->swp_csegs =
          kalloc_type(c_segment_t, swf->swp_nsegs, Z_WAITOK | Z_ZERO);
...
kern_return_t vm_swap_get(c_segment_t c_seg, uint64_t f_offset, uint64_t size) {
  struct swapfile *swf = NULL;
  uint64_t file_offset = 0;
  int retval = 0;
  ...
  if (swf == NULL ||
      (!(swf->swp_flags & SWAP_READY) && !(swf->swp_flags & SWAP_RECLAIM))) {
  ...
  if ((retval = vnode_getwithref(swf->swp_vp)) != 0) {
    ...
    retval = vm_swapfile_io(swf->swp_vp, file_offset,
                            (uint64_t)c_seg->c_store.c_buffer,
                            (int)(size / PAGE_SIZE_64), SWAP_READ, NULL);
...
kern_return_t vm_swap_put(vm_offset_t addr, uint64_t *f_offset, uint32_t size,
                          c_segment_t c_seg,
                          struct swapout_io_completion *soc) {
...
retry:
  ...
  while (queue_end(&swf_global_queue, (queue_entry_t)swf) == FALSE) {
    ...
    swf_eligible =
        (swf->swp_flags & SWAP_READY) && (swf->swp_nseginuse < swf->swp_nsegs);
    ...
    if (swf_eligible) {
      while (segidx < swf->swp_nsegs) {
        ...
        (swf->swp_bitmap)[byte_for_segidx] |=
            (uint8_t)(1 << offset_within_byte);
  ...
  if (vm_swapfile_should_create(now)) {
    ...
    waiting = TRUE;
    assert_wait_timeout((event_t)&vm_num_swap_files, THREAD_INTERRUPTIBLE, 1000,
                        1000 * NSEC_PER_USEC);
  } else {
...
issue_io:
  ...
  *f_offset = (swapfile_index << SWAP_DEVICE_SHIFT) | file_offset;
  ...
  if ((error = vnode_getwithref(swf->swp_vp)) != 0) {
    ...
    error = vm_swapfile_io(swf->swp_vp, file_offset, addr,
                           (int)(size / PAGE_SIZE_64), SWAP_WRITE, upl_ctx);
    drop_iocount = TRUE;
  }
...
kern_return_t vm_swap_put_finish(struct swapfile *swf, uint64_t *f_offset,
                                 int error, boolean_t drop_iocount) {
  if (drop_iocount) {
    vnode_put(swf->swp_vp);
  }
...
void vm_swap_free(uint64_t f_offset) {
  struct swapfile *swf = NULL;
...
static void vm_swap_wait_on_trim_handling_in_progress() {
...
static void vm_swap_handle_delayed_trims(boolean_t force_now) {
  struct swapfile *swf = NULL;
  ...
  while (queue_end(&swf_global_queue, (queue_entry_t)swf) == FALSE) {
    if ((swf->swp_flags & SWAP_READY) &&
        (force_now == TRUE || VM_SWAP_SHOULD_TRIM(swf))) {
...
static void vm_swap_do_delayed_trim(struct swapfile *swf) {
  ...
  if ((error = vnode_getwithref(swf->swp_vp)) != 0) {
    printf("vm_swap_do_delayed_trim: vnode_getwithref on swapfile failed with "
           "%d\n",
           error);
    return;
  }
...
void vm_swap_reclaim(void) {
  ...
  kmem_alloc(compressor_map, (vm_offset_t *)&addr, c_seg_bufsize,
             KMA_NOFAIL | KMA_KOBJECT | KMA_DATA_SHARED,
  ...
  ReTry_for_cseg:
    ...
    if (c_seg->c_busy) {
      /*
       * a swapped out c_segment in the process of being freed will remain in
       * the busy state until after the vm_swap_free is called on it...
       * vm_swap_free takes the vm_swap_data_lock, so can't change the swap
       * state until after we drop the vm_swap_data_lock... once we do,
       * vm_swap_free will complete which will allow c_seg_free_locked to clear
       * busy and wake up this thread... at that point, we re-look up the swap
       * state which will now indicate that this c_segment no longer exists.
       */
      c_seg->c_wanted = 1;
    ...
    if (vnode_getwithref(swf->swp_vp)) {
      ...
      if (vm_swapfile_io(swf->swp_vp, f_offset, addr,
                         (int)(c_size / PAGE_SIZE_64), SWAP_READ, NULL)) {
        /*
    ...
    if (vm_swap_put(addr, &f_offset, c_size, c_seg, NULL)) {
      ...
      kernel_memory_populate(c_buffer, c_size, KMA_NOFAIL | KMA_COMPRESSOR,
                             VM_KERN_MEMORY_COMPRESSOR);
...
uint64_t vm_swap_get_total_space(void) {
  ...
  total_space =
      (uint64_t)vm_swapfile_total_segs_alloced * compressed_swap_chunk_size;
...
uint64_t vm_swap_get_used_space(void) {
  ...
  used_space =
      (uint64_t)vm_swapfile_total_segs_used * compressed_swap_chunk_size;
...
uint64_t vm_swap_get_max_configured_space(void) {
  int num_swap_files = (vm_num_swap_files_config ? vm_num_swap_files_config
                                                 : VM_MAX_SWAP_FILE_NUM);
  return num_swap_files * MAX_SWAP_FILE_SIZE;
}

bool vm_swap_low_on_space(void) {
...
bool vm_swap_out_of_space(void) {
  if (vm_num_swap_files == 0 &&
      (!vm_swapfile_can_be_created || !SWAPPER_NEEDS_TO_UNTHROTTLE())) {
    /* We haven't started creating swap files yet */
    return false;
...
boolean_t vm_swap_files_pinned(void) {
  boolean_t result;
...
#if CONFIG_FREEZE
boolean_t vm_swap_max_budget(uint64_t *freeze_daily_budget) {
...
void vm_swap_reset_max_segs_tracking(uint64_t *alloced_max,
                                     uint64_t *used_max) {
  lck_mtx_lock(&vm_swap_data_lock);

  *alloced_max =
      (uint64_t)vm_swapfile_total_segs_alloced_max * compressed_swap_chunk_size;
  *used_max =
      (uint64_t)vm_swapfile_total_segs_used_max * compressed_swap_chunk_size;
