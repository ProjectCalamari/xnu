 ...
 */
#define NO_SANITIZE_UNSIGNED_OVERFLOW                                          \
  __attribute__((no_sanitize("unsigned-integer-overflow")))

/* Don't rewrite this result or telemeter anything. */
static inline __result_use_check vm_sanitize_compat_rewrite_t
vm_sanitize_make_policy_dont_rewrite_err(kern_return_t err) {
  return (vm_sanitize_compat_rewrite_t){
      .compat_kr = err, .should_rewrite = false, .should_telemeter = false};
}
...
vm_sanitize_make_policy_telemeter_dont_rewrite_err(kern_return_t err) {
  return (vm_sanitize_compat_rewrite_t){
      .compat_kr = err, .should_rewrite = false, .should_telemeter = true};
}
...
vm_sanitize_make_policy_telemeter_and_rewrite_err(kern_return_t err) {
  return (vm_sanitize_compat_rewrite_t){
      .compat_kr = err, .should_rewrite = true, .should_telemeter = true};
}
 ...
 */
static bool __unused vm_sanitize_range_overflows_strict_zero(
    vm_address_t start, vm_size_t size, vm_offset_t pgmask) {
  vm_address_t sum;
 ...
 */
static bool __unused vm_sanitize_range_overflows_allow_zero(
    vm_address_t start, vm_size_t size, vm_offset_t pgmask) {
  if (size == 0) {
    return false;
  }
...
vm_sanitize_err_compat_addr_size_vm_allocate_fixed(
    kern_return_t initial_kr, vm_address_t start, vm_size_t size,
    vm_offset_t pgmask, vm_map_t map_or_null __unused) {
  /*
...
VM_SANITIZE_DEFINE_CALLER(
    VM_ALLOCATE_FIXED, .err_compat_addr_size =
                           &vm_sanitize_err_compat_addr_size_vm_allocate_fixed);
...
static vm_sanitize_compat_rewrite_t
vm_sanitize_err_compat_addr_size_vm_deallocate(kern_return_t initial_kr,
                                               vm_address_t start,
                                               vm_size_t size,
                                               vm_offset_t pgmask,
  ...
  if (vm_sanitize_range_overflows_strict_zero(start, size, pgmask) &&
      start + size >= start) {
    return vm_sanitize_make_policy_telemeter_dont_rewrite_err(
        VM_ERR_RETURN_NOW);
  }
  return vm_sanitize_make_policy_dont_rewrite_err(initial_kr);
}
...
static vm_sanitize_compat_rewrite_t
vm_sanitize_err_compat_cur_and_max_prots_vm_map(kern_return_t initial_kr,
...
NO_SANITIZE_UNSIGNED_OVERFLOW
static vm_sanitize_compat_rewrite_t
vm_sanitize_err_compat_addr_size_vm_wire_user(kern_return_t initial_kr,
                                              vm_address_t start,
                                              vm_size_t size,
                                              vm_offset_t pgmask,
                                              vm_map_t map_or_null __unused) {
...
static vm_sanitize_compat_rewrite_t vm_sanitize_err_compat_addr_size_vslock(
    kern_return_t initial_kr __unused, vm_address_t start __unused,
    vm_size_t size __unused, vm_offset_t pgmask __unused,
    vm_map_t map_or_null __unused) {
  /*
...
VM_SANITIZE_DEFINE_CALLER(
    VSUNLOCK, .err_compat_addr_size = &vm_sanitize_err_compat_addr_size_vslock);
...
static vm_sanitize_compat_rewrite_t
vm_sanitize_err_compat_addr_size_vm_map_copyio(kern_return_t initial_kr,
                                               vm_address_t start,
                                               vm_size_t size,
                                               vm_offset_t pgmask,
                                               vm_map_t map_or_null __unused) {
...
NO_SANITIZE_UNSIGNED_OVERFLOW
static vm_sanitize_compat_rewrite_t
vm_sanitize_err_compat_addr_size_vm_map_inherit(kern_return_t initial_kr,
                                                vm_address_t start,
                                                vm_size_t size,
                                                vm_offset_t pgmask,
                                                vm_map_t map_or_null __unused) {
...
NO_SANITIZE_UNSIGNED_OVERFLOW
static vm_sanitize_compat_rewrite_t vm_sanitize_err_compat_addr_size_vm_protect(
    kern_return_t initial_kr, vm_address_t start, vm_size_t size,
    vm_offset_t pgmask, vm_map_t map_or_null __unused) {
  /*
  ...
  if (vm_sanitize_range_overflows_allow_zero(start, size, pgmask)) {
    ...
    } else if (vm_sanitize_range_overflows_allow_zero(
                   aligned_start, aligned_end - aligned_start, pgmask)) {
      /* fall through - this was KERN_INVALID_ARGUMENT historically too */
    } else {
      return vm_sanitize_make_policy_telemeter_and_rewrite_err(
          KERN_INVALID_ADDRESS);
...
NO_SANITIZE_UNSIGNED_OVERFLOW
static vm_sanitize_compat_rewrite_t vm_sanitize_err_compat_addr_size_useracc(
    kern_return_t initial_kr, vm_address_t start, vm_size_t size,
    vm_offset_t pgmask, vm_map_t map_or_null) {
  /*
  ...
  if (vm_sanitize_range_overflows_allow_zero(start, size, pgmask)) {
    ...
    if (vm_sanitize_range_overflows_allow_zero(
            aligned_start, aligned_end - aligned_start, pgmask) ||
        aligned_start < vm_map_min(map_or_null) ||
        aligned_end > vm_map_max(map_or_null) || aligned_start > aligned_end ||
        !have_entry) {
...
VM_SANITIZE_DEFINE_CALLER(
    USERACC, .err_compat_addr_size = &vm_sanitize_err_compat_addr_size_useracc);

/* behavior */

NO_SANITIZE_UNSIGNED_OVERFLOW
static vm_sanitize_compat_rewrite_t
vm_sanitize_err_compat_addr_size_vm_behavior_set(kern_return_t initial_kr,
                                                 vm_address_t start,
                                                 vm_size_t size,
                                                 vm_offset_t pgmask,
...
static vm_sanitize_compat_rewrite_t vm_sanitize_err_compat_addr_size_vm_msync(
    kern_return_t initial_kr, vm_address_t start, vm_size_t size,
    vm_offset_t pgmask, vm_map_t map_or_null __unused) {
  /*
...
NO_SANITIZE_UNSIGNED_OVERFLOW
static vm_sanitize_compat_rewrite_t vm_sanitize_err_compat_addr_size_mincore(
    kern_return_t initial_kr, vm_address_t start, vm_size_t size,
    vm_offset_t pgmask, vm_map_t map_or_null __unused) {
  /*
...
VM_SANITIZE_DEFINE_CALLER(
    MINCORE, .err_compat_addr_size = &vm_sanitize_err_compat_addr_size_mincore);
