...
typedef struct vm_shared_region_slide_info_entry_v1
...
struct vm_shared_region_slide_info_entry_v1 {
  ...
  uint32_t toc_count;  // number of entries in toc (same as number of pages in
                       // r/w mapping)
...
struct vm_shared_region_slide_info_entry_v2 {
  ...
  uint64_t delta_mask; // which (contiguous) set of bits contains the delta to
                       // the next rebase location
  uint64_t value_add;
...
#define DYLD_CACHE_SLIDE_PAGE_ATTR_EXTRA                                       \
  0x8000 // index is into extras array (not starts array)
...
#define DYLD_CACHE_SLIDE_PAGE_VALUE                                            \
  0x3FFF // bitwise negation of DYLD_CACHE_SLIDE_PAGE_ATTRS
#define DYLD_CACHE_SLIDE_PAGE_OFFSET_SHIFT 2

typedef struct vm_shared_region_slide_info_entry_v3
...
typedef struct vm_shared_region_slide_info_entry_v4
...
struct vm_shared_region_slide_info_entry_v4 {
  ...
  uint32_t page_extras_count;
  uint64_t delta_mask; // which (contiguous) set of bits contains the delta to
                       // the next rebase location (0xC0000000)
...
#define DYLD_CACHE_SLIDE4_PAGE_USE_EXTRA                                       \
  0x8000 // index is into extras array (not starts array)
#define DYLD_CACHE_SLIDE4_PAGE_EXTRA_END 0x8000 // last chain entry for page

typedef struct vm_shared_region_slide_info_entry_v5
...
typedef union vm_shared_region_slide_info_entry
 ...
 * the shared cache file and modifies them -- see
...
typedef struct vm_shared_region_slide_info {
  ...
  struct vm_shared_region *
      si_shared_region; /* so we can ref/dealloc for authenticated slide info */
#endif                  /* __has_feature(ptrauth_calls) */
  mach_vm_address_t si_slid_address __kernel_data_semantics;
  mach_vm_offset_t si_start
      __kernel_data_semantics; /* start offset in si_slide_object */
  mach_vm_offset_t si_end __kernel_data_semantics;
  vm_object_t
      si_slide_object; /* The source object for the pages to be modified */
  mach_vm_size_t
      si_slide_info_size; /* size of dyld provided relocation information */
  vm_shared_region_slide_info_entry_t
      si_slide_info_entry; /* dyld provided relocation information */
...
struct vm_shared_region {
  ...
  thread_t
      sr_mapping_in_progress; /* sr_first_mapping will be != -1 when done */
  ...
  uint64_t sr_install_time; /* mach_absolute_time() of installation into global
                               list */
...
extern vm_shared_region_t vm_shared_region_get(struct task *task);
extern void vm_shared_region_deallocate(struct vm_shared_region *shared_region);
extern void vm_shared_region_set(struct task *task,
...
vm_shared_region_update_task(struct task *task,
