...
/*
 * c_segment_info and c_slot_info are used in the serialization protocol of
 * sysctl vm.compressor_segments one c_segment_info is dumped for every
 * c_segment in memory, followed by a number of c_slot_info Every change to this
 * format should increment the version number in VM_C_SEGMENT_INFO_MAGIC
 */
struct c_slot_info {
  uint16_t csi_size;
  uint16_t csi_unused;
...
struct c_segment_info {
  ...
  uint32_t csi_state : 4, csi_swappedin : 1, csi_on_minor_compact_q : 1,
      csi_has_donated_pages : 1, csi_reserved : 25;
 ...
 * a starting header gives the number of entries that follow, the every entry in
 * the vm_map is represented by a vm_map_entry_info
 */
...
struct vm_map_entry_info {
  ...
  uint32_t vmei_is_sub_map : 1, vmei_is_compressor_pager : 1,
      vmei_protection : 3;
