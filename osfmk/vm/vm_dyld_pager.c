...
dyld_pager_data_request(memory_object_t mem_obj, memory_object_offset_t offset,
                        memory_object_cluster_size_t length,
                        vm_prot_t protection_required,
                        memory_object_fault_info_t fault_info);
static kern_return_t
dyld_pager_data_return(memory_object_t mem_obj, memory_object_offset_t offset,
                       memory_object_cluster_size_t data_cnt,
                       memory_object_offset_t *resid_offset, int *io_error,
                       boolean_t dirty, boolean_t kernel_copy, int upl_flags);
static kern_return_t
dyld_pager_data_initialize(memory_object_t mem_obj,
                           memory_object_offset_t offset,
                           memory_object_cluster_size_t data_cnt);
static kern_return_t dyld_pager_map(memory_object_t mem_obj, vm_prot_t prot);
static kern_return_t dyld_pager_last_unmap(memory_object_t mem_obj);
static boolean_t dyld_pager_backing_object(
    memory_object_t mem_obj, memory_object_offset_t mem_obj_offset,
    vm_object_t *backing_object, vm_object_offset_t *backing_offset);
...
/* funciton that calculates delta pointer that remains within the same page by
 * using nospec ISA */
static inline bool _delta_ptr_within_page_nospec(uint64_t **__nonnull ptr,
                                                 uint64_t deltaByteCount,
                                                 bool *crossing_page,
                                                 uintptr_t userVA) {
  ...
  *crossing_page = nospec_delta != deltaByteCount;
  if (*crossing_page) {
    ktriage_record(thread_tid(current_thread()),
                   KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_DYLD_PAGER,
                                       KDBG_TRIAGE_RESERVED,
                                       KDBG_TRIAGE_DYLD_PAGER_DELTA_TOO_LARGE),
                   (uintptr_t)userVA);
    printf(
        "%s(): fixup chain delta crossing to the next page [{%p} + {%lld}]\n",
...
static inline bool _delta_ptr_within_page32_nospec(uint32_t **__nonnull ptr,
                                                   uint32_t deltaByteCount,
                                                   bool *crossing_page,
                                                   uintptr_t userVA) {
  uintptr_t old_page = (uintptr_t)*ptr >> PAGE_SHIFT;
  uintptr_t new_page = ((uintptr_t)*ptr + deltaByteCount) >> PAGE_SHIFT;
 ...
 */
static void dyld_pager_dequeue(__unused dyld_pager_t pager) {
  queue_remove(&dyld_pager_queue, pager, dyld_pager_t, dyld_pager_queue);
 ...
 */
static kern_return_t
dyld_pager_init(memory_object_t mem_obj, memory_object_control_t control,
  ...
  kr = memory_object_change_attributes(control, MEMORY_OBJECT_ATTRIBUTE_INFO,
                                       (memory_object_info_t)&attributes,
                                       MEMORY_OBJECT_ATTR_INFO_COUNT);
  if (kr != KERN_SUCCESS) {
    panic("dyld_pager_init: "
 ...
 */
static kern_return_t
dyld_pager_data_return(__unused memory_object_t mem_obj,
                       __unused memory_object_offset_t offset,
                       __unused memory_object_cluster_size_t data_cnt,
                       __unused memory_object_offset_t *resid_offset,
                       __unused int *io_error, __unused boolean_t dirty,
                       __unused boolean_t kernel_copy, __unused int upl_flags) {
  panic("dyld_pager_data_return: should never happen!");
  return KERN_FAILURE;
}

static kern_return_t
dyld_pager_data_initialize(__unused memory_object_t mem_obj,
                           __unused memory_object_offset_t offset,
                           __unused memory_object_cluster_size_t data_cnt) {
 ...
 */
static kern_return_t fixupPage64(uint64_t userVA, vm_offset_t contents,
                                 void *link_info,
                                 struct dyld_chained_starts_in_segment *segInfo,
                                 uint32_t pageIndex, bool offsetBased) {
  ...
  //  For DYLD_CHAINED_PTR_64 (arm64 and x86_64) and DYLD_CHAINED_PTR_32
  //  (arm64_32) the stride is always 4
  ...
  uint64_t targetAdjust =
      (offsetBased ? hdr->mwli_image_address : hdr->mwli_slide);
  uint64_t delta = 0;
  bool valid_chain = false;
  do {
    if ((uintptr_t)chain < contents ||
        (uintptr_t)chain + sizeof(*chain) > end_contents) {
      ktriage_record(thread_tid(current_thread()),
                     KDBG_TRIAGE_EVENTID(
                         KDBG_TRIAGE_SUBSYS_DYLD_PAGER, KDBG_TRIAGE_RESERVED,
    ...
    /* shifts chain to a delta, chain cannot be used to access outside of page
     * speculatively after this point */
    bool crossing_page = false;
    valid_chain = _delta_ptr_within_page_nospec(&chain, delta, &crossing_page,
                                                (uintptr_t)userVA);
    if (crossing_page) {
      return KERN_FAILURE;
    }
 ...
 */
static kern_return_t fixupPageChain32(
    uint64_t userVA, uint32_t *chain, vm_offset_t contents, void *link_info,
    struct dyld_chained_starts_in_segment *segInfo, uint32_t *bindsArray) {
  struct mwl_info_hdr *hdr = (struct mwl_info_hdr *)link_info;
  uint32_t delta = 0;
  ...
  do {
    ...
    bool crossing_page = false;
    chain_valid = _delta_ptr_within_page32_nospec(&chain, delta, &crossing_page,
                                                  (uintptr_t)userVA);
 ...
 */
static kern_return_t fixupPage32(uint64_t userVA, vm_offset_t contents,
  ...
  if (startOffset & DYLD_CHAINED_PTR_START_MULTI) {
    ...
    while (!chainEnd) {
      /*
       * range check against link_info, note +1 to include data we'll
       * dereference
       */
      if ((uintptr_t)&segInfo->page_start[overflowIndex + 1] >
          (uintptr_t)link_info + link_info_size) {
        ktriage_record(thread_tid(current_thread()),
                       KDBG_TRIAGE_EVENTID(
                           KDBG_TRIAGE_SUBSYS_DYLD_PAGER, KDBG_TRIAGE_RESERVED,
        ...
        printf("%s(): out of range segInfo->page_start[overflowIndex]\n",
               __func__);
        if (panic_on_dyld_issue) {
          panic("%s(): out of range segInfo->page_start[overflowIndex]",
                __func__);
        }
        return KERN_FAILURE;
      }
      chainEnd =
          (segInfo->page_start[overflowIndex] & DYLD_CHAINED_PTR_START_LAST);
      startOffset =
          (segInfo->page_start[overflowIndex] & ~DYLD_CHAINED_PTR_START_LAST);
 ...
 */
static kern_return_t signPointer(uint64_t unsignedAddr, void *loc, bool addrDiv,
                                 uint16_t diversity, ptrauth_key key,
                                 dyld_pager_t pager, uint64_t *signedAddr) {
  // don't sign NULL
  if (unsignedAddr == 0) {
    *signedAddr = 0;
    return KERN_SUCCESS;
  ...
  if (addrDiv) {
    extendedDiscriminator =
        __builtin_ptrauth_blend_discriminator(loc, extendedDiscriminator);
  }
  ...
  case ptrauth_key_asda:
    if (pager->dyld_a_key == 0 || arm_user_jop_disabled()) {
      ...
      *signedAddr = (uintptr_t)pmap_sign_user_ptr(
          (void *)unsignedAddr, key, extendedDiscriminator, pager->dyld_a_key);
    }
    break;
 ...
 */
static kern_return_t
fixupPageAuth64(uint64_t userVA, vm_offset_t contents, dyld_pager_t pager,
                struct dyld_chained_starts_in_segment *segInfo,
                uint32_t pageIndex, bool offsetBased) {
  ...
  struct mwl_info_hdr *hdr = (struct mwl_info_hdr *)link_info;
  uint64_t *bindsArray =
      (uint64_t *)((uintptr_t)link_info + hdr->mwli_binds_offset);
  vm_offset_t end_contents = contents + PAGE_SIZE;
  bool valid_chain = false;
   ...
   */
  if ((uintptr_t)&segInfo->page_start[pageIndex + 1] >
      (uintptr_t)link_info + link_info_size) {
    ktriage_record(
        thread_tid(current_thread()),
        KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_DYLD_PAGER, KDBG_TRIAGE_RESERVED,
                            KDBG_TRIAGE_DYLD_PAGER_PAGE_START_OUT_OF_RANGE),
        (uintptr_t)userVA);
  ...
  do {
    ...
    if (isAuth) {
      ...
      if (isBind) {
        ...
        if (signPointer(bindsArray[bindOrdinal], (void *)uVA, addrDiv,
                        diversity, key, pager, chain) != KERN_SUCCESS) {
          return KERN_FAILURE;
        }
      } else {
        /* note: in auth rebases only have 32-bits, so target is always offset -
         * never vmaddr */
        uint64_t target = (value & 0xFFFFFFFF) + hdr->mwli_image_address;
        if (signPointer(target, (void *)uVA, addrDiv, diversity, key, pager,
                        chain) != KERN_SUCCESS) {
          return KERN_FAILURE;
        }
      }
 ...
 */
static kern_return_t
fixupCachePageAuth64(uint64_t userVA, vm_offset_t contents, dyld_pager_t pager,
                     struct dyld_chained_starts_in_segment *segInfo,
                     uint32_t pageIndex) {
  void *link_info = pager->dyld_link_info;
  uint32_t link_info_size = pager->dyld_link_info_size;
 ...
 */
static kern_return_t fixup_page(vm_offset_t contents, uint64_t userVA,
                                dyld_pager_t pager) {
  void *link_info = pager->dyld_link_info;
  uint32_t link_info_size = pager->dyld_link_info_size;
  struct mwl_info_hdr *hdr = (struct mwl_info_hdr *)link_info;
   ...
   */
  startsInfo = (struct dyld_chained_starts_in_image *)((uintptr_t)hdr +
                                                       hdr->mwli_chains_offset);
  for (segIndex = 0; segIndex < startsInfo->seg_count; ++segIndex) {
    ...
    if (segStartAddress <= userVA && userVA < segEndAddress) {
      ...
      if ((uintptr_t)&seg->page_start[seg->page_count] >
          (uintptr_t)link_info + link_info_size) {
        ktriage_record(thread_tid(current_thread()),
                       KDBG_TRIAGE_EVENTID(
                           KDBG_TRIAGE_SUBSYS_DYLD_PAGER, KDBG_TRIAGE_RESERVED,
  ...
  /*
   * Question for Nick.. or can we make this OK and just return KERN_SUCCESS,
   * nothing to do?
   */
  if (segInfo == NULL) {
    ktriage_record(thread_tid(current_thread()),
  ...
  case DYLD_CHAINED_PTR_32:
    fixupPage32(userVA, contents, link_info, link_info_size, segInfo,
                pageIndex);
    break;
  default:
    ktriage_record(thread_tid(current_thread()),
    ...
    if (panic_on_dyld_issue) {
      panic("%s(): unknown pointer_format %d", __func__,
            hdr->mwli_pointer_format);
    }
    return KERN_FAILURE;
  }
...
dyld_pager_data_request(memory_object_t mem_obj, memory_object_offset_t offset,
  ...
  struct vm_object_fault_info fault_info =
      *((struct vm_object_fault_info *)(uintptr_t)mo_fault_info);
  ...
  upl_size = length;
  upl_flags = UPL_RET_ONLY_ABSENT | UPL_SET_LITE | UPL_NO_SYNC |
              UPL_CLEAN_IN_PLACE | /* triggers UPL_CLEAR_DIRTY */
              UPL_SET_INTERNAL;
  pl_count = 0;
  kr = memory_object_upl_request(mo_control, offset, upl_size, &upl, NULL, NULL,
                                 upl_flags, VM_KERN_MEMORY_SECURITY);
  if (kr != KERN_SUCCESS) {
    ktriage_record(thread_tid(current_thread()),
                   KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_DYLD_PAGER,
                                       KDBG_TRIAGE_RESERVED,
  ...
  for (cur_offset = 0; retval == KERN_SUCCESS && cur_offset < length;
  ...
  retry_src_fault:
    ...
    vmfr = vm_fault_page(src_top_object, offset + cur_offset, VM_PROT_READ,
                         FALSE, FALSE, /* src_page not looked up */
                         &prot, &src_page, &top_page, NULL, &error_code, FALSE,
                         &fault_info);
    switch (vmfr) {
    ...
    src_vaddr = (vm_map_offset_t)phystokv(
        (pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(src_page) << PAGE_SHIFT);
    ...
    if (src_page_object->code_signed) {
      vm_page_validate_cs_mapped(src_page, PAGE_SIZE, 0,
    ...
    /*
     * Figure out what the original user virtual address was, based on the
     * offset.
     */
    userVA = 0;
    ...
    if (r == pager->dyld_num_range) {
      ...
      if (panic_on_dyld_issue) {
        panic("%s(): Range not found for offset 0x%llx", __func__,
              (long long)cur_offset);
      }
      retval = KERN_FAILURE;
    ...
    if (retval != KERN_SUCCESS) {
      ktriage_record(thread_tid(current_thread()),
                     ...
                     userVA);
      if (panic_on_dyld_issue) {
        panic("%s(): dyld pager slide error %d at 0x%llx", __func__, retval,
              (uint64_t)userVA);
      }
    }
 ...
 *
 * Release a reference on this pager and free it when the last reference goes
 * away. Can be called with dyld_pager_lock held or not, but always returns with
 * it unlocked.
 */
static void dyld_pager_deallocate_internal(dyld_pager_t pager, bool locked) {
 ...
 *
 */
static kern_return_t dyld_pager_terminate(
#if !DEBUG
    __unused
#endif
...
static boolean_t dyld_pager_backing_object(memory_object_t mem_obj,
                                           memory_object_offset_t offset,
 ...
 */
static dyld_pager_t dyld_pager_create(
#if !defined(HAS_APPLE_PAC)
    __unused
#endif /* defined(HAS_APPLE_PAC) */
        task_t task,
    vm_object_t backing_object, struct mwl_region *regions, uint32_t region_cnt,
    void *link_info, uint32_t link_info_size) {
  dyld_pager_t pager;
  memory_object_control_t control;
  kern_return_t kr;
  ...
  pager->dyld_link_info =
      link_info; /* pager takes ownership of this pointer here */
  pager->dyld_link_info_size = link_info_size;
#if defined(HAS_APPLE_PAC)
  pager->dyld_a_key =
      (task->map && task->map->pmap && !task->map->pmap->disable_jop)
          ? task->jop_pid
          : 0;
#endif /* defined(HAS_APPLE_PAC) */
  ...
  lck_mtx_lock(&dyld_pager_lock);
  queue_enter_first(&dyld_pager_queue, pager, dyld_pager_t, dyld_pager_queue);
  dyld_pager_count++;
 ...
 */
static memory_object_t dyld_pager_setup(task_t task, vm_object_t backing_object,
  ...
  /* create new pager */
  pager = dyld_pager_create(task, backing_object, regions, region_cnt,
                            link_info, link_info_size);
  if (pager == NULL) {
  ...
  while (!pager->dyld_is_ready) {
    lck_mtx_sleep(&dyld_pager_lock, LCK_SLEEP_DEFAULT, &pager->dyld_is_ready,
                  THREAD_UNINT);
  }
  lck_mtx_unlock(&dyld_pager_lock);
 ...
 */
kern_return_t vm_map_with_linking(task_t task, struct mwl_region *regions,
                                  uint32_t region_cnt, void **link_info,
                                  uint32_t link_info_size,
  ...
  if (file_object == VM_OBJECT_NULL || file_object->internal) {
    printf("%d[%s] %s: invalid object for provided file\n", proc_selfpid(),
           proc_name_address(current_proc()), __func__);
  ...
  pager = dyld_pager_setup(task, backing_object, regions, region_cnt,
                           *link_info, link_info_size);
  ...
  *link_info = NULL; /* ownership of this pointer was given to pager */
  ...
  for (r = 0; r < region_cnt; ++r) {
    ...
    kr = mach_vm_map_kernel(
        map, vm_sanitize_wrap_addr_ref(&map_addr), rp->mwlr_size, 0, vmk_flags,
        (ipc_port_t)(uintptr_t)pager, rp->mwlr_file_offset,
        TRUE, /* copy == TRUE, as this is MAP_PRIVATE so COW may happen */
        rp->mwlr_protections & VM_PROT_DEFAULT,
        rp->mwlr_protections & VM_PROT_DEFAULT, VM_INHERIT_DEFAULT);
    if (kr != KERN_SUCCESS) {
...
static uint64_t dyld_pager_purge(dyld_pager_t pager) {
  ...
  //__FUNCTION__, __LINE__, pager, object, pages_purged,
  //object->resident_page_count);
  vm_object_unlock(object);
  return pages_purged;
}

uint64_t dyld_pager_purge_all(void) {
  uint64_t pages_purged;
  dyld_pager_t pager;
