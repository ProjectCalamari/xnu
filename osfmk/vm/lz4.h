...
static const size_t lz4_hash_table_size =
    LZ4_COMPRESS_HASH_ENTRIES * sizeof(lz4_hash_entry_t);

// Worker function for lz4 encode.  Underlies both the buffer and stream encode
// operations. Performs lz4 encoding of up to 2gb of data, updates dst_ptr and
// src_ptr to point to the first byte of output and input that couldn't be
// completely processed, respectively.
//
// If skip_final_literals is 0, the entire src buffer is encoded, by emitting a
// final sequence of literals at the end of the compressed payload.
//
// If skip_final_literals is not 0, this final literal sequence is not emitted,
// and the src buffer is partially encoded (the length of this literal sequence
// varies).
extern void
lz4_encode_2gb(uint8_t **dst_ptr, size_t dst_size, const uint8_t **src_ptr,
...
extern int lz4_decode_asm(uint8_t **dst_ptr, uint8_t *dst_begin,
...
typedef __attribute__((__ext_vector_type__(16),
                       __aligned__(1))) uint8_t packed_uchar16;
typedef __attribute__((__ext_vector_type__(32),
                       __aligned__(1))) uint8_t packed_uchar32;
typedef __attribute__((__ext_vector_type__(64),
                       __aligned__(1))) uint8_t packed_uchar64;

typedef __attribute__((__ext_vector_type__(4))) uint16_t vector_ushort4;
typedef __attribute__((__ext_vector_type__(4),
                       __aligned__(2))) uint16_t packed_ushort4;
...
UTIL_FUNCTION uint16_t load2(const void *ptr) {
  uint16_t data;
  lz4_memcpy(&data, ptr, sizeof data);
  return data;
}
