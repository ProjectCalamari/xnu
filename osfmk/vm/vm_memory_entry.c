...
IPC_KOBJECT_DEFINE(IKOT_NAMED_ENTRY, .iko_op_movable_send = true,
                   .iko_op_stable = true,
...
kern_return_t mach_make_memory_entry_64(vm_map_t target_map,
  ...
  return mach_make_memory_entry_internal(
      target_map, size_u, offset_u, permission_u,
      VM_NAMED_ENTRY_KERNEL_FLAGS_NONE, object_handle, parent_handle);
}

static inline void
vm_memory_entry_decode_perm(vm_prot_t permission, unsigned int *access,
                            vm_prot_t *protections, bool *mask_protections,
                            bool *use_data_addr, bool *use_4K_compat) {
...
vm_memory_entry_get_offset_in_page(vm_map_offset_t offset,
...
static inline kern_return_t mach_make_memory_entry_cleanup(
    kern_return_t kr, vm_map_t target_map __unused,
    memory_object_size_ut *size_u, vm_map_offset_ut offset_u __unused,
    vm_prot_t permission __unused, vm_named_entry_t user_entry __unused,
    ipc_port_t *object_handle) {
...
mach_make_memory_entry_mem_only_sanitize(vm_map_t target_map,
                                         memory_object_size_ut size_u,
  ...
  return vm_sanitize_addr_size(
      offset_u, size_u, VM_SANITIZE_CALLER_MACH_MAKE_MEMORY_ENTRY, target_map,
      VM_SANITIZE_FLAGS_SIZE_ZERO_FALLTHROUGH, map_start, map_end, map_size);
}

static kern_return_t mach_make_memory_entry_mem_only(
    vm_map_t target_map, memory_object_size_ut *size_u,
    memory_object_offset_ut offset_u, vm_prot_t permission,
    ipc_port_t *object_handle, vm_named_entry_t parent_entry) {
   ...
   * dispatch
   */
  kr = mach_make_memory_entry_mem_only_sanitize(
      target_map, *size_u, offset_u, &map_start, &map_end, &map_size);
  if (__improbable(kr != KERN_SUCCESS)) {
    return mach_make_memory_entry_cleanup(kr, target_map, size_u, offset_u,
                                          permission, user_entry,
                                          object_handle);
  }

  vm_memory_entry_decode_perm(permission, &access, &protections,
                              &mask_protections, &use_data_addr,
                              &use_4K_compat);

  if (use_data_addr || use_4K_compat || parent_entry == NULL) {
    return mach_make_memory_entry_cleanup(KERN_INVALID_ARGUMENT, target_map,
                                          size_u, offset_u, permission,
                                          user_entry, object_handle);
  }
  ...
  if (parent_is_object && object && (access != MAP_MEM_NOOP) &&
      (!(object->nophyscache))) {
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
mach_make_memory_entry_generic_sanitize(vm_map_t target_map,
                                        memory_object_size_ut size_u,
                                        vm_map_offset_ut offset_u,
                                        vm_map_offset_t *map_start,
                                        vm_map_offset_t *map_end,
...
static kern_return_t mach_make_memory_entry_named_create(
    vm_map_t target_map, memory_object_size_ut *size_u,
    vm_map_offset_ut offset_u, vm_prot_t permission,
    vm_named_entry_kernel_flags_t vmne_kflags, ipc_port_t *object_handle) {
  vm_object_t object;
  unsigned int access;
  vm_prot_t protections;
  bool mask_protections;
  ...
  if (VM_SANITIZE_UNSAFE_IS_ZERO(*size_u)) {
    return mach_make_memory_entry_cleanup(KERN_SUCCESS, target_map, size_u,
                                          offset_u, permission, user_entry,
                                          object_handle);
  }
   ...
   * dispatch
   */
  kr = mach_make_memory_entry_generic_sanitize(
      target_map, *size_u, offset_u, &map_start, &map_end, &map_size, &offset);
  if (__improbable(kr != KERN_SUCCESS)) {
    return mach_make_memory_entry_cleanup(kr, target_map, size_u, offset_u,
                                          permission, user_entry,
                                          object_handle);
  ...
  if (map_size > ANON_MAX_SIZE) {
    return mach_make_memory_entry_cleanup(KERN_FAILURE, target_map, size_u,
                                          offset_u, permission, user_entry,
                                          object_handle);
  }
#endif /* __LP64__ */
  ...
  owner = current_task();
  if ((permission & MAP_MEM_PURGABLE) || vmne_kflags.vmnekf_ledger_tag) {
  ...
  if (vmne_kflags.vmnekf_ledger_tag) {
    ...
    kr = vm_object_ownership_change(
        object, vmne_kflags.vmnekf_ledger_tag, owner, /* new owner */
        ledger_flags, FALSE);                         /* task_objq locked? */
    if (kr != KERN_SUCCESS) {
      vm_object_unlock(object);
      vm_object_deallocate(object);
      return mach_make_memory_entry_cleanup(kr, target_map, size_u, offset_u,
  ...
  vm_named_entry_associate_vm_object(user_entry, object, 0, map_size,
...
mach_make_memory_entry_copy(vm_map_t target_map, memory_object_size_ut *size_u,
                            vm_map_offset_ut offset_u, vm_prot_t permission,
  ...
  if (target_map == VM_MAP_NULL) {
    return mach_make_memory_entry_cleanup(KERN_INVALID_TASK, target_map, size_u,
                                          offset_u, permission, user_entry,
                                          object_handle);
  }

  offset_in_page = vm_memory_entry_get_offset_in_page(
      offset, map_start, use_data_addr, use_4K_compat);

  int copyin_flags = VM_MAP_COPYIN_ENTRY_LIST;
  kr = vm_map_copyin_internal(target_map, map_start, map_size, copyin_flags,
                              &copy);
  if (kr != KERN_SUCCESS) {
    return mach_make_memory_entry_cleanup(kr, target_map, size_u, offset_u,
                                          permission, user_entry,
...
static kern_return_t
mach_make_memory_entry_share(vm_map_t target_map, memory_object_size_ut *size_u,
                             vm_map_offset_ut offset_u, vm_prot_t permission,
                             __unused vm_named_entry_kernel_flags_t vmne_kflags,
                             ipc_port_t *object_handle,
                             ipc_port_t parent_handle,
  ...
  kr = mach_make_memory_entry_generic_sanitize(
      target_map, *size_u, offset_u, &map_start, &map_end, &map_size, &offset);
  if (__improbable(kr != KERN_SUCCESS)) {
    kr = mach_make_memory_entry_cleanup(kr, target_map, size_u, offset_u,
                                        permission, user_entry, object_handle);
    vmlp_api_end(MACH_MAKE_MEMORY_ENTRY_SHARE, kr);
    return kr;
  }
  ...
  if (target_map == VM_MAP_NULL) {
    kr = mach_make_memory_entry_cleanup(KERN_INVALID_TASK, target_map, size_u,
                                        offset_u, permission, user_entry,
                                        object_handle);
    vmlp_api_end(MACH_MAKE_MEMORY_ENTRY_SHARE, kr);
    return kr;
  ...
  if (!(permission & MAP_MEM_VM_SHARE)) {
    ...
    if (protections & VM_PROT_WRITE) {
      ...
      kr = vm_map_lookup_and_lock_object(
          ...
          OBJECT_LOCK_EXCLUSIVE, &version, &tmp_object, &obj_off, &prot, &wired,
          NULL, /* fault_info */
          &real_map, &contended);
      if (kr != KERN_SUCCESS) {
    ...
    if ((permission & MAP_MEM_NAMED_REUSE) && parent_entry != NULL &&
  ...
  assert(map_size != 0);
  kr = vm_map_copy_extract(target_map, map_start, map_size, FALSE, /* copy */
                           &copy, &cur_prot, &max_prot, VM_INHERIT_SHARE,
                           vmk_flags);
  if (kr != KERN_SUCCESS) {
    kr = mach_make_memory_entry_cleanup(kr, target_map, size_u, offset_u,
                                        permission, user_entry, object_handle);
  ...
  if (mask_protections) {
    ...
    if (protections == VM_PROT_NONE) {
      ...
      kr = mach_make_memory_entry_cleanup(KERN_PROTECTION_FAILURE, target_map,
                                          size_u, offset_u, permission,
                                          user_entry, object_handle);
      vmlp_api_end(MACH_MAKE_MEMORY_ENTRY_SHARE, kr);
      return kr;
  ...
  if (!(permission & MAP_MEM_VM_SHARE)) {
    ...
    if ((permission & MAP_MEM_NAMED_REUSE) &&
        ...
        parent_entry->offset == 0 && parent_entry->size == map_size &&
      ...
      *size_u =
          vm_sanitize_wrap_size(parent_entry->size - parent_entry->data_offset);
    ...
    if (object == VM_OBJECT_NULL) {
       ...
       * return a failure because the code that follows and other APIs that
       * consume a named-entry expect to have non-null object */
      vm_map_copy_discard(copy);
      kr = mach_make_memory_entry_cleanup(KERN_PROTECTION_FAILURE, target_map,
                                          size_u, offset_u, permission,
                                          user_entry, object_handle);
  ...
  if (permission & MAP_MEM_VM_SHARE) {
    ...
    assert3p(object, ==,
             vm_named_entry_to_vm_object(
                 user_entry)); /* Sanity, this was set above */
...
mach_make_memory_entry_from_parent_entry_sanitize(
    vm_map_t target_map, memory_object_size_ut size_u,
    vm_map_offset_ut offset_u, vm_prot_t permission,
    vm_named_entry_t parent_entry, vm_map_offset_t *map_start,
    vm_map_offset_t *map_end, vm_map_size_t *map_size, vm_map_offset_t *offset,
   ...
   */
  kr = vm_sanitize_addr_size(
      offset_u, size_u, VM_SANITIZE_CALLER_MACH_MAKE_MEMORY_ENTRY, PAGE_MASK,
...
static kern_return_t mach_make_memory_entry_from_parent_entry(
    vm_map_t target_map, memory_object_size_ut *size_u,
    vm_map_offset_ut offset_u, vm_prot_t permission, ipc_port_t *object_handle,
    vm_named_entry_t parent_entry) {
  vm_object_t object;
  unsigned int access;
  vm_prot_t protections;
   ...
   * dispatch
   */
  kr = mach_make_memory_entry_from_parent_entry_sanitize(
      target_map, *size_u, offset_u, permission, parent_entry, &map_start,
      &map_end, &map_size, &offset, &user_entry_offset);
  if (__improbable(kr != KERN_SUCCESS)) {
    return mach_make_memory_entry_cleanup(kr, target_map, size_u, offset_u,
                                          permission, user_entry,
                                          object_handle);
  ...
  if (use_data_addr || use_4K_compat) {
    ...
    if (parent_entry->is_sub_map) {
      panic("Shouldn't be using data address with a parent entry that is a "
            "submap.");
    }
  }
  ...
  if (parent_entry->is_sub_map) {
    ...
    vm_named_entry_associate_vm_object(user_entry, object, user_entry->offset,
                                       user_entry->size,
...
mach_make_memory_entry_sanitize_perm(vm_prot_ut permission_u,
                                     vm_prot_t *permission) {
  return vm_sanitize_memory_entry_perm(
      permission_u, VM_SANITIZE_CALLER_MACH_MAKE_MEMORY_ENTRY,
      VM_SANITIZE_FLAGS_CHECK_USER_MEM_MAP_FLAGS, VM_PROT_IS_MASK, permission);
}

kern_return_t mach_make_memory_entry_internal(
    vm_map_t target_map, memory_object_size_ut *size_u,
    memory_object_offset_ut offset_u, vm_prot_ut permission_u,
    vm_named_entry_kernel_flags_t vmne_kflags, ipc_port_t *object_handle,
    ipc_port_t parent_handle) {
  vm_named_entry_t user_entry __unused = NULL;
  vm_named_entry_t parent_entry;
  ...
  DEBUG4K_MEMENTRY("map %p offset 0x%llx size 0x%llx prot 0x%x\n", target_map,
                   VM_SANITIZE_UNSAFE_UNWRAP(offset_u),
  ...
  if (permission & MAP_MEM_ONLY) {
    return mach_make_memory_entry_mem_only(
        target_map, size_u, offset_u, permission, object_handle, parent_entry);
  }

  if (permission & MAP_MEM_NAMED_CREATE) {
    return mach_make_memory_entry_named_create(
        target_map, size_u, offset_u, permission, vmne_kflags, object_handle);
  }

  if (permission & MAP_MEM_VM_COPY) {
    return mach_make_memory_entry_copy(target_map, size_u, offset_u, permission,
                                       vmne_kflags, object_handle);
  }

  if ((permission & MAP_MEM_VM_SHARE) || parent_entry == NULL ||
   ...
   */
  return mach_make_memory_entry_from_parent_entry(
      target_map, size_u, offset_u, permission, object_handle, parent_entry);
}
...
kern_return_t _mach_make_memory_entry(vm_map_t target_map,
  ...
  return mach_make_memory_entry_64(target_map, size_u, offset_u, permission_u,
                                   object_handle, parent_entry);
}

kern_return_t mach_make_memory_entry(vm_map_t target_map, vm_size_ut *size_u,
  ...
  kr = mach_make_memory_entry_64(target_map, size_u, offset_u, permission_u,
                                 object_handle, parent_entry);
  return kr;
}
...
mach_memory_entry_allocate(ipc_port_t *user_handle_p) {
  ...
  user_entry = kalloc_type(struct vm_named_entry, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
mach_memory_object_memory_entry_64_sanitize(vm_object_size_ut size_u,
                                            vm_prot_ut permission_u,
                                            vm_object_size_t *size,
                                            vm_prot_t *permission) {
  kern_return_t kr;

  kr = vm_sanitize_object_size(
      size_u, VM_SANITIZE_CALLER_MACH_MEMORY_OBJECT_MEMORY_ENTRY,
      VM_SANITIZE_FLAGS_SIZE_ZERO_FAILS, size);
  if (__improbable(kr != KERN_SUCCESS)) {
    return kr;
  }
  kr = vm_sanitize_memory_entry_perm(
      permission_u, VM_SANITIZE_CALLER_MACH_MEMORY_OBJECT_MEMORY_ENTRY,
      VM_SANITIZE_FLAGS_NONE, VM_PROT_NONE, permission);
  if (__improbable(kr != KERN_SUCCESS)) {
    return kr;
  }
...
kern_return_t mach_memory_object_memory_entry_64(
    host_t host, boolean_t internal, vm_object_size_ut size_u,
    vm_prot_ut permission_u, memory_object_t pager, ipc_port_t *entry_handle) {
  vm_named_entry_t user_entry;
   ...
   */
  kr = mach_memory_object_memory_entry_64_sanitize(size_u, permission_u, &size,
                                                   &permission);
  if (__improbable(kr != KERN_SUCCESS)) {
    return vm_sanitize_get_kr(kr);
  }
...
kern_return_t mach_memory_object_memory_entry(host_t host, boolean_t internal,
                                              vm_size_ut size_u,
                                              vm_prot_ut permission_u,
                                              memory_object_t pager,
                                              ipc_port_t *entry_handle) {
  return mach_memory_object_memory_entry_64(host, internal, size_u,
                                            permission_u, pager, entry_handle);
}

kern_return_t mach_memory_entry_purgable_control(ipc_port_t entry_port,
...
kern_return_t memory_entry_purgeable_control_internal(ipc_port_t entry_port,
                                                      vm_purgable_t control,
                                                      int *state) {
  kern_return_t kr;
...
static kern_return_t memory_entry_access_tracking_internal(
    ipc_port_t entry_port, int *access_tracking,
    uint32_t *access_tracking_reads, uint32_t *access_tracking_writes) {
  vm_named_entry_t mem_entry;
  vm_object_t object;
  kern_return_t kr;
  ...
  vm_object_access_tracking(object, access_tracking, access_tracking_reads,
...
kern_return_t
mach_memory_entry_access_tracking(ipc_port_t entry_port, int *access_tracking,
                                  uint32_t *access_tracking_reads,
                                  uint32_t *access_tracking_writes) {
  return memory_entry_access_tracking_internal(entry_port, access_tracking,
...
kern_return_t mach_memory_entry_ownership(ipc_port_t entry_port, task_t owner,
  ...
  if (cur_task == kernel_task) {
...
#if DEVELOPMENT || DEBUG
    if (!transfer_ok && ledger_tag == VM_LEDGER_TAG_DEFAULT &&
        (ledger_flags & VM_LEDGER_FLAG_NO_FOOTPRINT_FOR_DEBUG) &&
      ...
      /*
       * Allow performance tools running on internal builds to hide memory usage
       * from phys_footprint even WITHOUT an entitlement. This can be enabled by
       * per task sysctl vm.task_no_footprint_for_debug=1 with the ledger tag
       * VM_LEDGER_TAG_DEFAULT and flag VM_LEDGER_FLAG_NO_FOOTPRINT_FOR_DEBUG.
       *
       * If the boot-arg "panic_on_no_footprint_for_debug" is set, the kernel
       * will panic here in order to detect any abuse of this feature, which is
       * intended solely for memory debugging purpose.
       */
      if (!init_bootarg) {
        PE_parse_boot_argn("panic_on_no_footprint_for_debug", &to_panic,
                           sizeof(to_panic));
        init_bootarg = true;
      }
      if (to_panic) {
        panic("%s: panic_on_no_footprint_for_debug is triggered by pid %d "
              "procname %s",
              __func__, proc_selfpid(),
              get_bsdtask_info(cur_task)
                  ? proc_name_address(get_bsdtask_info(cur_task))
                  : "?");
      }
    ...
    if (!transfer_ok) {
      ...
      our_id = IOTaskGetEntitlement(current_task(),
                                    "com.apple.developer.memory.transfer-send");
      their_id = IOTaskGetEntitlement(
          owner, "com.apple.developer.memory.transfer-accept");
      if (our_id && their_id &&
          !strcmp(our_id,
                  their_id)) { /* These are guaranteed to be null-terminated */
  ...
  } else if (ledger_tag < 0 || ledger_tag > VM_LEDGER_TAG_MAX) {
    return KERN_INVALID_ARGUMENT;
  }
  if (owner == TASK_NULL) {
  ...
  if (mem_entry->is_sub_map || !mem_entry->is_fully_owned) {
    named_entry_unlock(mem_entry);
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (mem_entry->is_object) {
    ...
    named_entry_unlock(mem_entry);
    kr = vm_object_ownership_change(object, ledger_tag, owner, ledger_flags,
    ...
    for (entry = vm_map_copy_first_entry(copy);
      ...
      if (entry->is_sub_map || object == VM_OBJECT_NULL) {
...
kern_return_t mach_memory_entry_ownership_from_user(ipc_port_t entry_port,
...
kern_return_t mach_memory_entry_get_page_counts(ipc_port_t entry_port,
  ...
  kr = vm_object_get_page_counts(object, offset, size, resident_page_count,
                                 dirty_page_count, swapped_page_count);
...
kern_return_t mach_memory_entry_phys_page_offset(ipc_port_t entry_port,
...
static inline kern_return_t mach_memory_entry_map_size_sanitize_locked(
    vm_map_t map, memory_object_offset_ut *offset_u,
    memory_object_size_ut size_u, vm_named_entry_t mem_entry,
    memory_object_offset_t *offset, memory_object_offset_t *end,
    mach_vm_size_t *map_size) {
  kern_return_t kr;

  if (mem_entry->is_object ||
      (mem_entry->is_copy && (VM_MAP_COPY_PAGE_MASK(mem_entry->backing.copy) ==
                              VM_MAP_PAGE_MASK(map)))) {
    if (__improbable(
            vm_sanitize_add_overflow(*offset_u, mem_entry->offset, offset_u))) {
      return KERN_INVALID_ARGUMENT;
    }
  }
  ...
  kr = vm_sanitize_addr_size(
      *offset_u, size_u, VM_SANITIZE_CALLER_MACH_MEMORY_ENTRY_MAP_SIZE, map,
...
kern_return_t mach_memory_entry_map_size(ipc_port_t entry_port, vm_map_t map,
                                         memory_object_offset_ut offset_u,
                                         memory_object_size_ut size_u,
  ...
  *map_size_out = 0;
   ...
   */
  kr = mach_memory_entry_map_size_sanitize_locked(
      map, &offset_u, size_u, mem_entry, &offset, &end, &map_size);
  if (__improbable(kr != KERN_SUCCESS)) {
    named_entry_unlock(mem_entry);
    return kr;
  }
  ...
  if (VM_MAP_COPY_PAGE_MASK(mem_entry->backing.copy) == VM_MAP_PAGE_MASK(map)) {
    DEBUG4K_SHARE("map %p (%d) mem_entry %p offset 0x%llx + 0x%llx + 0x%llx "
                  "size 0x%llx -> map_size 0x%llx\n",
                  map, VM_MAP_PAGE_MASK(map), mem_entry, mem_entry->offset,
                  mem_entry->data_offset, offset,
                  VM_SANITIZE_UNSAFE_UNWRAP(size_u), map_size);
    named_entry_unlock(mem_entry);
    *map_size_out = map_size;
    return KERN_SUCCESS;
  }
  ...
  DEBUG4K_SHARE(
      "mem_entry %p copy %p (%d) map %p (%d) offset 0x%llx size 0x%llx\n",
      mem_entry, mem_entry->backing.copy,
      VM_MAP_COPY_PAGE_SHIFT(mem_entry->backing.copy), map,
      VM_MAP_PAGE_SHIFT(map), offset, VM_SANITIZE_UNSAFE_UNWRAP(size_u));
  copy_map = mem_entry->backing.copy;
  target_copy_map = VM_MAP_COPY_NULL;
  DEBUG4K_ADJUST("adjusting...\n");
  kr = vm_map_copy_adjust_to_target(copy_map, offset_u, size_u, map, FALSE,
                                    &target_copy_map, &overmap_start,
                                    &overmap_end, &trimmed_start);
  if (kr == KERN_SUCCESS) {
    if (target_copy_map->size != copy_map->size) {
      DEBUG4K_ADJUST("copy %p (%d) map %p (%d) offset 0x%llx size 0x%llx "
                     "overmap_start 0x%llx overmap_end 0x%llx trimmed_start "
                     "0x%llx map_size 0x%llx -> 0x%llx\n",
                     copy_map, VM_MAP_COPY_PAGE_SHIFT(copy_map), map,
                     VM_MAP_PAGE_SHIFT(map), (uint64_t)offset,
                     (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(size_u),
                     (uint64_t)overmap_start, (uint64_t)overmap_end,
                     (uint64_t)trimmed_start, (uint64_t)copy_map->size,
 ...
 */
void mach_memory_entry_port_release(ipc_port_t port) {
  assert(ip_type(port) == IKOT_NAMED_ENTRY);
  ipc_port_release_send(port);
}

vm_named_entry_t mach_memory_entry_from_port(ipc_port_t port) {
  if (IP_VALID(port)) {
...
void mach_memory_entry_describe(vm_named_entry_t named_entry,
                                kobject_description_t desc) {
  vm_object_t vm_object;
  if (named_entry->is_object) {
    vm_object = vm_named_entry_to_vm_object(named_entry);
    vm_object_size_t size =
        vm_object->internal ? vm_object->vo_un1.vou_size : 0;
    snprintf(desc, KOBJECT_DESCRIPTION_LENGTH, "VM-OBJECT(0x%x, %lluKiB)",
             VM_OBJECT_ID(vm_object), BtoKiB(size));
  } else if (named_entry->is_copy) {
    vm_map_copy_t copy_map = named_entry->backing.copy;
    snprintf(desc, KOBJECT_DESCRIPTION_LENGTH, "VM-MAP-COPY(0x%lx, %lluKiB)",
             VM_KERNEL_ADDRHASH(copy_map), BtoKiB(copy_map->size));
  } else if (named_entry->is_sub_map) {
    vm_map_t submap = named_entry->backing.map;
    snprintf(desc, KOBJECT_DESCRIPTION_LENGTH, "VM-SUB-MAP(0x%lx, %lluKiB)",
             VM_KERNEL_ADDRHASH(submap), BtoKiB(submap->size));
  }
}
 ...
 */
static void mach_memory_entry_no_senders(ipc_port_t port,
...
kern_return_t mach_memory_entry_page_op(ipc_port_t entry_port,
                                        vm_object_offset_ut offset_u, int ops,
                                        ppnum_t *phys_entry, int *flags) {
  vm_named_entry_t mem_entry;
  vm_object_t object;
  kern_return_t kr;
  /*
...
kern_return_t mach_memory_entry_range_op(ipc_port_t entry_port,
  ...
  kr = vm_object_range_op(object, offset_beg, offset_end, ops,
                          (uint32_t *)range);
...
kern_return_t memory_entry_check_for_adjustment(vm_map_t src_map,
                                                ipc_port_t port,
  ...
  if (src_map && VM_MAP_PAGE_SHIFT(src_map) < PAGE_SHIFT) {
    ...
    kr = vm_map_copy_adjust_to_target(
        copy_map, vm_sanitize_wrap_addr(0),    /* offset */
...
vm_named_entry_t vm_convert_port_to_named_entry(ipc_port_t port) {
...
vm_object_t vm_convert_port_to_copy_object(ipc_port_t port) {
