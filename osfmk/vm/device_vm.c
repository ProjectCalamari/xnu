...
device_pager_t device_pager_lookup(/* forward */
                                   memory_object_t);

device_pager_t device_object_create(void); /* forward */
 ...
 *
 */
memory_object_t device_pager_setup(__unused memory_object_t device,
                                   uintptr_t device_handle, vm_size_t size,
                                   int flags) {
  ...
  memory_object_create_named((memory_object_t)device_object, size, &control);
 ...
 *
 */
kern_return_t device_pager_populate_object(memory_object_t device,
                                           memory_object_offset_t offset,
                                           ppnum_t page_num, vm_size_t size) {
  device_pager_t device_object;
  ...
  kr = vm_object_populate_with_private(vm_object, offset, page_num, size);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
  ...
  if (!vm_object->phys_contiguous) {
    ...
    kr = vm_object_upl_request(
        vm_object, (vm_object_offset_t)offset, (upl_size_t)size, &upl, NULL,
        &null_size, (UPL_NO_SYNC | UPL_CLEAN_IN_PLACE), VM_KERN_MEMORY_NONE);
    if (kr != KERN_SUCCESS) {
 ...
 *
 */
device_pager_t device_pager_lookup(memory_object_t mem_obj) {
  device_pager_t device_object;
 ...
 *
 */
kern_return_t device_pager_init(memory_object_t mem_obj,
                                memory_object_control_t control,
...
device_pager_data_action(memory_object_t mem_obj, memory_object_offset_t offset,
...
/*ARGSUSED6*/
kern_return_t
device_pager_data_return(memory_object_t mem_obj, memory_object_offset_t offset,
 ...
 *
 */
void device_pager_reference(memory_object_t mem_obj) {
  device_pager_t device_object;

  device_object = device_pager_lookup(mem_obj);
  os_ref_retain_raw(&device_object->dev_pgr_hdr_ref, NULL);
  DTRACE_VM2(device_pager_reference, device_pager_t, device_object,
 ...
 *
 */
void device_pager_deallocate(memory_object_t mem_obj) {
  device_pager_t device_object;
  ...
  DTRACE_VM2(device_pager_deallocate, device_pager_t, device_object,
             unsigned int, device_pager_get_refcount(device_object));
  ...
  if (ref_count == 1) {
    ...
    DTRACE_VM2(device_pager_destroy, device_pager_t, device_object,
               unsigned int, device_pager_get_refcount(device_object));
     ...
     */
    DTRACE_VM2(device_pager_free, device_pager_t, device_object, unsigned int,
               device_pager_get_refcount(device_object));
...
kern_return_t
device_pager_data_initialize(__unused memory_object_t mem_obj,
...
kern_return_t device_pager_terminate(__unused memory_object_t mem_obj) {
  return KERN_SUCCESS;
}
 ...
 *
 */
kern_return_t device_pager_map(memory_object_t mem_obj,
...
kern_return_t device_pager_last_unmap(memory_object_t mem_obj) {
  device_pager_t device_object;
  boolean_t drop_ref;
 ...
 *
 */
device_pager_t device_object_create(void) {
  device_pager_t device_object;

  device_object = zalloc_flags(device_pager_zone, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  DTRACE_VM2(device_pager_create, device_pager_t, device_object, unsigned int,
             device_pager_get_refcount(device_object));

  return device_object;
}

boolean_t is_device_pager_ops(const struct memory_object_pager_ops *pager_ops) {
