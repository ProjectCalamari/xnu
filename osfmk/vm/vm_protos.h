...
extern task_t port_name_to_task_read(mach_port_name_t name);
...
extern int max_task_footprint_mb; /* Per-task limit on physical memory
...
#if MACH_ASSERT
extern void vm_map_pmap_set_process(vm_map_t map, int pid, char *procname);
extern void vm_map_pmap_check_ledgers(pmap_t pmap, ledger_t ledger, int pid,
                                      char *procname);
#endif /* MACH_ASSERT */
 ...
 */
extern kern_return_t vm_region_object_create(vm_map_t target_task,
                                             vm_size_t size,
...
vm_map_apple_protected(vm_map_t map, vm_map_offset_t start, vm_map_offset_t end,
                       vm_object_offset_t crypto_backing_offset,
                       struct pager_crypt_info *crypt_info, uint32_t cryptid);
#endif /* CONFIG_CODE_DECRYPTION */

struct vm_shared_region_slide_info;

#if __has_feature(ptrauth_calls)
extern void shared_region_key_alloc(char *shared_region_id, bool inherit,
...
extern kern_return_t vnode_pager_init(memory_object_t, memory_object_control_t,
                                      memory_object_cluster_size_t);

#if CONFIG_IOSCHED
extern kern_return_t vnode_pager_get_object_devvp(memory_object_t, uintptr_t *);
#endif
 ...
 */
extern kern_return_t
ubc_cs_check_validation_bitmap(struct vnode *vp, memory_object_offset_t offset,
                               int optype);
extern int ubc_map(struct vnode *vp, int flags);
extern void ubc_unmap(struct vnode *vp);

extern void device_pager_reference(memory_object_t);
extern void device_pager_deallocate(memory_object_t);
extern kern_return_t device_pager_init(memory_object_t, memory_object_control_t,
                                       memory_object_cluster_size_t);
...
extern kern_return_t
pager_map_to_phys_contiguous(memory_object_control_t object,
                             memory_object_offset_t offset, addr64_t base_vaddr,
                             vm_size_t size);

struct macx_triggers_args;

extern int macx_swapinfo(memory_object_size_t *total_p,
                         memory_object_size_t *avail_p, vm_size_t *pagesize_p,
...
extern boolean_t cs_validate_range(struct vnode *vp, memory_object_t pager,
                                   memory_object_offset_t offset,
                                   const void *data, vm_size_t size,
                                   unsigned *result);
extern void cs_validate_page(struct vnode *vp, memory_object_t pager,
                             memory_object_offset_t offset, const void *data,
                             int *validated_p, int *tainted_p, int *nx_p);

extern kern_return_t mach_memory_entry_purgable_control(ipc_port_t entry_port,
...
#define SWAPFILENAME_INDEX_LEN                                                 \
  2 /* Doesn't include the terminating NULL character */
...
#define __DEBUG4K(category, fmt, ...)                                          \
  ...
  if (__do_log) {                                                              \
    ...
    printf("DEBUG4K(%s) %d[%s] %p(0x%llx) %s:%d: " fmt,                        \
           debug4k_category_name[__category], __p ? proc_pid(__p) : 0,         \
           __pname, __t, thread_tid(__t), __FUNCTION__, __LINE__,              \
           ##__VA_ARGS__);                                                     \
  }                                                                            \
  MACRO_END
...
_Static_assert(VM_OBJECT_DESTROY_MAX < 8,
               "Need to fit in `no_pager_reason`'s number of bits");
...
extern kern_return_t
memory_object_mark_as_tracked(memory_object_control_t control, bool new_value,
                              bool *old_value);
