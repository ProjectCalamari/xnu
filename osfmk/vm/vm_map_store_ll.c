...
bool first_free_is_valid_ll(vm_map_t map) {
...
void vm_map_store_init_ll(struct vm_map_header *hdr) {
  hdr->links.next = CAST_TO_VM_MAP_ENTRY(hdr);
  VMH_PREV_SET(hdr, CAST_TO_VM_MAP_ENTRY(hdr));
}

void vm_map_store_entry_link_ll(struct vm_map_header *hdr,
...
void vm_map_store_entry_unlink_ll(struct vm_map_header *hdr,
                                  vm_map_entry_t entry) {
  hdr->nentries--;
...
void vm_map_store_copy_reset_ll(vm_map_copy_t copy,
 ...
 */
void update_first_free_ll(vm_map_t map, vm_map_entry_t new_first_free) {
  vm_map_offset_t map_page_mask = VM_MAP_PAGE_MASK(map);
  vm_map_entry_t next;
