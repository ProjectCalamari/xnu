...
void vm_object_tracking_init(void) {
  ...
  if (vm_object_tracking) {
    vm_object_tracking_btlog =
        btlog_create(BTLOG_HASH, VM_OBJECT_TRACKING_NUM_RECORDS);
...
static kern_return_t vm_object_terminate(vm_object_t object);

static void vm_object_do_collapse(vm_object_t object,
                                  vm_object_t backing_object);

static void vm_object_do_bypass(vm_object_t object, vm_object_t backing_object);

static void vm_object_release_pager(memory_object_t pager);
...
#define vm_object_cache_unlock() lck_mtx_unlock(&vm_object_cached_lock_data)
...
static LCK_MTX_DECLARE_ATTR(vm_object_reaper_lock_data, &vm_object_lck_grp,
                            &vm_object_lck_attr);

static queue_head_t
    vm_object_reaper_queue; /* protected by vm_object_reaper_lock() */
unsigned int vm_object_reap_count = 0;
unsigned int vm_object_reap_count_async = 0;

#define vm_object_reaper_lock() lck_mtx_lock(&vm_object_reaper_lock_data)
#define vm_object_reaper_lock_spin()                                           \
  lck_mtx_lock_spin(&vm_object_reaper_lock_data)
#define vm_object_reaper_unlock() lck_mtx_unlock(&vm_object_reaper_lock_data)
...
void vm_object_set_size(vm_object_t object, vm_object_size_t outer_size,
 ...
 */
__startup_func void vm_object_bootstrap(void) {
  vm_size_t vm_object_size;

  assert(sizeof(mo_ipc_object_bits_t) == sizeof(ipc_object_bits_t));

  vm_object_size =
      (sizeof(struct vm_object) + (VM_PAGE_PACKED_PTR_ALIGNMENT - 1)) &
...
#if CONFIG_IOSCHED
void vm_io_reprioritize_init(void) {
  kern_return_t result;

  result = mpsc_daemon_queue_init_with_thread(
      &io_reprioritize_q, io_reprioritize, BASEPRI_KERNEL,
...
void vm_object_reaper_init(void) {
  ...
  kr = kernel_thread_start_priority((thread_continue_t)vm_object_reaper_thread,
                                    NULL, BASEPRI_VM, &thread);
  if (kr != KERN_SUCCESS) {
...
__private_extern__ void vm_object_deallocate(vm_object_t object) {
  ...
  //	if(object)dbgLog(object, object->ref_count, object->can_persist, 3);
  ///* (TEST/DEBUG) */ 	else dbgLog(object, 0, 0, 3);	/* (TEST/DEBUG) */
  ...
  if (os_ref_get_count_raw(&object->ref_count) == 2 && object->named) {
     ...
     */
             ...
             object->internal && object->shadow != VM_OBJECT_NULL) {
    /*
     ...
     */
    if (original_ref_count == 2 && object->named) {
      /* need to take slow path for m_o_last_unmap() */
      atomic_swap = FALSE;
    } else if (original_ref_count == 2 && object->internal &&
      ...
      atomic_swap = OSCompareAndSwap(original_ref_count, original_ref_count - 1,
  ...
  while (object != VM_OBJECT_NULL) {
    ...
    if ((os_ref_get_count_raw(&object->ref_count) > 1) || object->terminating) {
    ...
    if (object->pager_created && !object->pager_initialized) {
      ...
      vm_object_sleep(object, VM_OBJECT_EVENT_PAGER_INIT, THREAD_UNINT,
                      LCK_SLEEP_UNLOCK);
      continue;
    }
...
vm_page_t vm_object_page_grab(vm_object_t object) {
  ...
  while (!vm_page_queue_end(&object->memq, (vm_page_queue_entry_t)next_p) &&
         --p_limit > 0) {
    p = next_p;
...
TUNABLE_WRITEABLE(vm_chead_select_t, vm_chead_select, "vm_chead_select",
                  CSEL_BY_PID);
/* This determines if the grabber-id is set on every page-fault insert or just
 * the first insert */
TUNABLE_WRITEABLE(boolean_t, vm_chead_rehint, "vm_chead_rehint", false);

/*
 * This function is called from vm_page_insert_internal(). When it's called from
 * the context of a vm_fault where a task has just requested a new page/paged-in
 * a existing page, this function records some bits of information about the
 * task. These bits are then going to be used when the page is sent to the
 * compressor to select the compressor-head that will be used. The goal of this
 * is to make pages that come from the same task/coalition be compressed to the
 * same compressor segment, This helps the locality of swap-in and
 * decompression. This optimization relies on a heuristic assumptions that the
 * vm_object is only ever mapped in a single task/coalition. vm_objects that
 * violate this would not benefit from this optimization. See also
 * vm_pageout_select_filling_chead()
 */
void vm_object_set_chead_hint(vm_object_t object) {
  if (!object->internal) {
...
static void vm_object_cache_remove_locked(vm_object_t object) {
...
void vm_object_cache_remove(vm_object_t object) {
  ...
  if (object->cached_list.next && object->cached_list.prev) {
...
void vm_object_cache_add(vm_object_t object) {
  ...
  if (object->cached_list.next == NULL && object->cached_list.prev == NULL) {
...
int vm_object_cache_evict(int num_to_evict, int max_objects_to_examine) {
  ...
  for (;;) { /* loop for as long as we have objects to process */
    ...
    while (!queue_end(&vm_object_cached_list, (queue_entry_t)next_obj) &&
           object_cnt++ < max_objects_to_examine) {
      object = next_obj;
      next_obj = (vm_object_t)queue_next(&next_obj->cached_list);

      assert(object->purgable == VM_PURGABLE_DENY);

      if (sec < object->vo_cache_ts) { // reached the point in the queue beyond
                                       // the time we started
        KDBG_DEBUG(0x130020c, object, object->resident_page_count,
                   object->vo_cache_ts, sec);

        vm_object_cache_aging_ts = object->vo_cache_ts;
        object = VM_OBJECT_NULL; /* this will cause to break away from the outer
                                    loop */
        break;
      }
      ...
      if (vm_page_queue_empty(&object->memq) ||
          object->vo_cache_pages_to_scan == 0) {
        /*
         ...
         */
        KDBG_DEBUG(0x13001fc, object, object->resident_page_count, ep_freed,
                   ep_moved);
     ...
     */
    if ((ep_limit = num_to_evict - (ep_freed + ep_moved)) >
        EVICT_PREPARE_LIMIT) {
    ...
    while (!vm_page_queue_end(&object->memq, (vm_page_queue_entry_t)next_p) &&
           object->vo_cache_pages_to_scan && ep_count < ep_limit) {
      p = next_p;
      next_p = (vm_page_t)vm_page_queue_next(&next_p->vmp_listq);
      ...
      if (!object->internal && object->pager_created && object->pager == NULL) {
        /*
        ...
        //				printf("FBDP %s:%d object %p reason %d
        //page %p offset 0x%llx pmapped %d wpmapped %d xpmapped %d dirty %d
        //precious %d\n", __FUNCTION__, __LINE__, object,
        //object->no_pager_reason, p, p->vmp_offset, p->vmp_pmapped,
        //p->vmp_wpmapped, p->vmp_xpmapped, p->vmp_dirty, p->vmp_precious);
    ...
    KDBG_DEBUG(0x13001f4 | DBG_FUNC_START, object, object->resident_page_count,
               ep_freed, ep_moved);
    ...
    KDBG_DEBUG(0x13001f4 | DBG_FUNC_END, object, object->resident_page_count,
               ep_freed, ep_moved);
    ...
    if (object->vo_cache_pages_to_scan == 0) {
      KDBG_DEBUG(0x1300208, object, object->resident_page_count, ep_freed,
                 ep_moved);
  ...
  //num_to_evict, max_objects_to_examine, ep_freed, ep_moved, ep_skipped);
  return ep_freed;
}
...
static kern_return_t vm_object_terminate(vm_object_t object) {
  ...
  if (!object->internal && object->cached_list.next &&
  ...
  if (object->paging_in_progress != 0 || object->activity_in_progress != 0) {
    /*
...
void vm_object_reap(vm_object_t object) {
  ...
  if (/* object->internal && */
    ...
    kr = vm_object_ownership_change(
        object, VM_LEDGER_TAG_NONE, NULL, /* no owner */
        ledger_flags, FALSE);             /* task_objq not locked */
  ...
  if (object->object_is_shared_cache && object->pager != NULL &&
    ...
    OSAddAtomic(-object->resident_page_count,
  ...
  ref_count = os_ref_release_locked_raw(&object->ref_count, &vm_object_refgrp);
  ...
  if (object->internal) {
    ...
    if (object->purgable == VM_PURGABLE_DENY) {
      ...
      delta = (object->resident_page_count - object->wired_page_count);
      if (delta != 0) {
        assert(vm_page_purgeable_count >= delta);
        OSAddAtomic(-delta, (SInt32 *)&vm_page_purgeable_count);
      }
      if (object->wired_page_count != 0) {
        assert(vm_page_purgeable_wired_count >= object->wired_page_count);
      ...
      panic("object %p in unexpected purgeable state 0x%x", object,
            object->purgable);
    }
    if (object->transposed && object->cached_list.next != NULL &&
        object->cached_list.prev == NULL) {
      /*
...
static inline vm_page_t vm_object_reap_freelist(vm_page_t local_free_q,
                                                bool do_disconnect,
                                                bool set_cache_attr) {
  vm_page_t page;
  if (local_free_q) {
...
void vm_object_reap_pages(vm_object_t object, int reap_type) {
  vm_page_t p;
  vm_page_t next;
  vm_page_t local_free_q = VM_PAGE_NULL;
  int loop_count;
...
restart_after_sleep:
  ...
  while (!vm_page_queue_end(&object->memq, (vm_page_queue_entry_t)next)) {
    ...
    if (--loop_count == 0) {
      ...
      if (local_free_q) {
         ...
         */
        local_free_q = vm_object_reap_freelist(
            local_free_q, disconnect_on_release, set_cache_attr_needed);
      } else {
        mutex_pause(0);
      }
    ...
    case REAP_PURGEABLE:
      ...
      if (p->vmp_pmapped == TRUE) {
        ...
        pmap_disconnect_options(VM_PAGE_GET_PHYS_PAGE(p),
                                PMAP_OPTIONS_NOFLUSH | PMAP_OPTIONS_NOREFMOD,
    ...
    case REAP_TERMINATE:
      ...
      if ((p->vmp_dirty || p->vmp_precious) && !VMP_ERROR_GET(p) &&
          object->alive) {
  ...
  vm_object_reap_freelist(local_free_q, disconnect_on_release,
...
void vm_object_reap_async(vm_object_t object) {
  ...
  queue_enter(&vm_object_reaper_queue, object, vm_object_t, cached_list);
...
void vm_object_reaper_thread(void) {
  vm_object_t object, shadow_object;

  vm_object_reaper_lock_spin();

  while (!queue_empty(&vm_object_reaper_queue)) {
    queue_remove_first(&vm_object_reaper_queue, object, vm_object_t,
                       cached_list);
    ...
    shadow_object = object->pageout ? VM_OBJECT_NULL : object->shadow;
...
kern_return_t vm_object_destroy(vm_object_t object,
  ...
  if (object->named && os_ref_get_count_raw(&object->ref_count) > 2 &&
      object->pager != NULL &&
      vnode_pager_get_forced_unmount(object->pager, &forced_unmount) ==
          KERN_SUCCESS &&
      forced_unmount == false) {
    if (!fbdp_no_panic) {
      panic("FBDP rdar://99829401 object %p refs %d pager %p (no forced "
            "unmount)\n",
            object, os_ref_get_count_raw(&object->ref_count), object->pager);
    }
    DTRACE_VM3(vm_object_destroy_no_forced_unmount, vm_object_t, object, int,
               os_ref_get_count_raw(&object->ref_count), memory_object_t,
               object->pager);
  }
...
/*
 * The "chunk" macros are used by routines below when looking for pages to
 * deactivate.  These exist because of the need to handle shadow chains.  When
 * deactivating pages, we only want to deactive the ones at the top most level
 * in the object chain.  In order to do this efficiently, the specified address
 * range is divided up into "chunks" and we use a bit map to keep track of which
 * pages have already been processed as we descend down the shadow chain.  These
 * chunk macros hide the details of the bit map implementation as much as we
 * can.
 *
 * For convenience, we use a 64-bit data type as the bit map, and therefore a
 * chunk is set to 64 pages.  The bit map is indexed from the low-order end, so
 * that the lowest order bit represents page 0 in the current range and highest
 * order bit represents page 63.
 *
 * For further convenience, we also use negative logic for the page state in the
 * bit map. The bit is set to 1 to indicate it has not yet been seen, and to 0
 * to indicate it has been processed.  This way we can simply test the 64-bit
 * long word to see if it's zero to easily tell if the whole range has been
 * processed.  Therefore, the bit map starts out with all the bits set.  The
 * macros below hide all these details from the caller.
 */
...
#define CHUNK_SIZE                                                             \
  (PAGES_IN_A_CHUNK * PAGE_SIZE_64) /* Size of a chunk in bytes */

typedef uint64_t chunk_state_t;

/*
 * The bit map uses negative logic, so we start out with all 64 bits set to
 * indicate that no pages have been processed yet.  Also, if len is less than
 * the full CHUNK_SIZE, then we mark pages beyond the len as having been
 * "processed" so that we don't waste time looking at pages in that range.  This
 * can save us from unnecessarily chasing down the shadow chain.
 */
 ...
 * page)
 */
static bool page_is_paged_out(vm_object_t object, vm_object_offset_t offset) {
...
__options_decl(deactivate_flags_t, uint32_t,
               {DEACTIVATE_KILL = 0x1, DEACTIVATE_REUSABLE = 0x2,
                DEACTIVATE_ALL_REUSABLE = 0x4, DEACTIVATE_CLEAR_REFMOD = 0x8,
                DEACTIVATE_KILL_NO_WRITE = 0x10});

/*
 * Deactivate the pages in the specified object and range.  If kill_page is set,
 * also discard any page modified state from the pmap.  Update the chunk_state
 * as we go along.  The caller must specify a size that is less than or equal to
 * the CHUNK_SIZE.
 */

static void
deactivate_pages_in_object(vm_object_t object, vm_object_offset_t offset,
                           vm_object_size_t size, deactivate_flags_t flags,
                           chunk_state_t *chunk_state, pmap_flush_context *pfc,
                           struct pmap *pmap, vm_map_offset_t pmap_offset) {
  vm_page_t m;
  int p;
  ...
  /*
   * Examine each page in the chunk.  The variable 'p' is the page number
   * relative to the start of the chunk.  Since this routine is called once for
   * each level in the shadow chain, the chunk_state may have pages marked as
   * having been processed already.  We stop the loop early if we find we've
   * handled all the pages in the chunk.
   */
  ...
  for (p = 0; size && CHUNK_NOT_COMPLETE(*chunk_state); p++,
      size -= PAGE_SIZE_64, offset += PAGE_SIZE_64,
      pmap_offset += PAGE_SIZE_64) {
    /*
     * If this offset has already been found and handled in a higher level
     * object, then don't do anything with it in the current shadow object.
     */
    ...
    /*
     * See if the page at this offset is around.  First check to see if the page
     * is resident, then if not, check the existence map or with the pager.
     */

    if ((m = vm_page_lookup(object, offset)) != VM_PAGE_NULL) {
      /*
       * We found a page we were looking for.  Mark it as "handled" now in the
       * chunk_state so that we won't bother looking for a page at this offset
       * again if there are more shadow objects.  Then deactivate the page.
       */

      MARK_PAGE_HANDLED(*chunk_state, p);

      if ((!VM_PAGE_WIRED(m)) && (!vm_page_is_private(m)) &&
          (!m->vmp_gobbled) && (!m->vmp_busy) && (!m->vmp_laundry) &&
          (!m->vmp_cleaning) && !(m->vmp_free_when_done)) {
        ...
        if ((flags & DEACTIVATE_KILL) && (object->internal)) {
          if (!(flags & DEACTIVATE_KILL_NO_WRITE) &&
              (madvise_free_debug || (madvise_free_debug_sometimes &&
        ...
        if (flags & DEACTIVATE_CLEAR_REFMOD) {
          ...
          pmap_clear_refmod_options(VM_PAGE_GET_PHYS_PAGE(m), clear_refmod_mask,
                                    pmap_options, (void *)pfc);
        }
        ...
        if (dw_count >= dw_limit) {
          ...
          vm_page_do_delayed_work(object, VM_KERN_MEMORY_NONE, dwp_start,
                                  dw_count);
      ...
      if (page_is_paged_out(object, offset)) {
        ...
        /*
         * If we're killing a non-resident page, then clear the page in the
         * existence map so we don't bother paging it back in if it's touched
         * again in the future.
         */
        ...
        if ((flags & DEACTIVATE_KILL) && (object->internal)) {
          ...
          if (pmap != PMAP_NULL) {
             ...
             * longer compressed.
             */
            pmap_remove_options(pmap, pmap_offset, (pmap_offset + PAGE_SIZE),
                                PMAP_OPTIONS_REMOVE);
          }
        }
      }
...
/*
 * Deactive a "chunk" of the given range of the object starting at offset.  A
 * "chunk" will always be less than or equal to the given size.  The total range
 * is divided up into chunks for efficiency and performance related to the locks
 * and handling the shadow chain.  This routine returns how much of the given
 * "size" it actually processed.  It's up to the caler to loop and keep calling
 * this routine until the entire range they want to process has been done. Iff
 * clear_refmod is true, pmap_clear_refmod_options is called for each physical
 * page in this range.
 */

static vm_object_size_t
deactivate_a_chunk(vm_object_t orig_object, vm_object_offset_t offset,
                   vm_object_size_t size, deactivate_flags_t flags,
                   pmap_flush_context *pfc, struct pmap *pmap,
  ...
  /*
   * The chunk_state keeps track of which pages we've already processed if
   * there's a shadow chain on this object.  At this point, we haven't done
   * anything with this range of pages yet, so initialize the state to indicate
   ...
   * object in the shadow chain.  We stop processing early if we've already
   * found all the pages in the range.  Otherwise we stop when we run out of
   * shadow objects.
   */
  ...
  while (object && CHUNK_NOT_COMPLETE(chunk_state)) {
    ...
    deactivate_pages_in_object(object, offset, length, flags, &chunk_state, pfc,
                               pmap, pmap_offset);
    ...
    if (tmp_object) {
      ...
      flags &=
          ~(DEACTIVATE_KILL | DEACTIVATE_REUSABLE | DEACTIVATE_ALL_REUSABLE);
...
/*
 * Move any resident pages in the specified range to the inactive queue.  If
 * kill_page is set, we also clear the modified status of the page and "forget"
 * any changes that have been made to the page.
 */

__private_extern__ void
vm_object_deactivate_pages(vm_object_t object, vm_object_offset_t offset,
                           vm_object_size_t size, boolean_t kill_page,
                           boolean_t reusable_page, boolean_t kill_no_write,
                           struct pmap *pmap, vm_map_offset_t pmap_offset) {
  vm_object_size_t length;
   ...
   * The deactivate_a_chunk() function returns how much of the range it
   * processed. We keep calling this routine until the given size is exhausted.
   */
...
#else
  if (reusable_page && object->internal && object->vo_size != 0 &&
      object->vo_size == size && object->reusable_page_count == 0) {
    all_reusable = TRUE;
    reusable_page = FALSE;
   ...
   * operation. We can't do this if we're killing pages and there's a shadow
   * chain as we don't yet know which pages are in the top object (pages in
   * shadow copies aren't safe to kill). And we can only do this on hardware
   * that supports it.
   */
  if (size > PAGE_SIZE && (!kill_page || !object->shadow)) {
    ...
    refmod_cleared = pmap_clear_refmod_range_options(
        pmap, pmap_offset, pmap_offset + size, pmap_clear_refmod_mask,
        pmap_clear_refmod_options);
    if (refmod_cleared) {
      // We were able to clear all the refmod bits. So deactivate_a_chunk
...
void vm_object_reuse_pages(vm_object_t object, vm_object_offset_t start_offset,
...
#define VM_OBJECT_REUSE_PAGE(object, m, reused)                                \
  MACRO_BEGIN                                                                  \
  if ((m) != VM_PAGE_NULL && (m)->vmp_reusable) {                              \
    assert((object)->reusable_page_count <= (object)->resident_page_count);    \
    assert((object)->reusable_page_count > 0);                                 \
    (object)->reusable_page_count--;                                           \
     ...
     */                                                                        \
    pmap_clear_refmod_options(                                                 \
        VM_PAGE_GET_PHYS_PAGE((m)), 0, /* refmod */                            \
        (PMAP_OPTIONS_CLEAR_REUSABLE | PMAP_OPTIONS_NOFLUSH), NULL);           \
  }                                                                            \
  MACRO_END
  ...
  if (object->all_reusable) {
    panic("object %p all_reusable: can't update pmap stats", object);
    assert(object->reusable_page_count == 0);
    object->all_reusable = FALSE;
    if (end_offset - start_offset == object->vo_size || !allow_partial_reuse) {
      ...
      vm_page_queue_iterate(&object->memq, m, vmp_listq) {
        if (m->vmp_offset < start_offset || m->vmp_offset >= end_offset) {
          m->vmp_reusable = TRUE;
          object->reusable_page_count++;
          assert(object->resident_page_count >= object->reusable_page_count);
          continue;
    ...
    vm_page_stats_reusable.partial_reuse_calls++;
    for (cur_offset = start_offset; cur_offset < end_offset;
 ...
 */
static kern_return_t vm_object_zero_preflight(vm_object_t object,
   ...
   */
  if (object->shadow != NULL || object->vo_copy != NULL) {
...
static void vm_object_zero_page(vm_page_t m) {
  if (m != VM_PAGE_NULL) {
 ...
 */
kern_return_t vm_object_zero(vm_object_t object,
...
__private_extern__ void
vm_object_pmap_protect(vm_object_t object, vm_object_offset_t offset,
                       vm_object_size_t size, pmap_t pmap,
                       vm_map_size_t pmap_page_size, vm_map_offset_t pmap_start,
                       vm_prot_t prot) {
  vm_object_pmap_protect_options(object, offset, size, pmap, pmap_page_size,
                                 pmap_start, prot, 0);
}

__private_extern__ void vm_object_pmap_protect_options(
    vm_object_t object, vm_object_offset_t offset, vm_object_size_t size,
    pmap_t pmap, vm_map_size_t pmap_page_size, vm_map_offset_t pmap_start,
    vm_prot_t prot, int options) {
  pmap_flush_context pmap_flush_context_storage;
   ...
   */
  size_in_object =
      vm_object_round_page(offset + size) - vm_object_trunc_page(offset);
  ...
  assertf(!(offset & (pmap_page_size - 1)) && !(size & (pmap_page_size - 1)),
          "offset 0x%llx size 0x%llx pmap_page_size 0x%llx", offset, size,
  ...
  if (object->phys_contiguous) {
    if (pmap != NULL) {
      ...
      for (phys_addr = phys_start; phys_addr < phys_end;
           phys_addr += PAGE_SIZE_64) {
        pmap_page_protect_options((ppnum_t)(phys_addr >> PAGE_SHIFT), prot,
  ...
  while (TRUE) {
    if (ptoa_64(object->resident_page_count) > size_in_object / 2 &&
        pmap != PMAP_NULL) {
      vm_object_unlock(object);
      if (pmap_page_size < PAGE_SIZE) {
        DEBUG4K_PMAP(
            "pmap %p start 0x%llx end 0x%llx prot 0x%x: pmap_protect()\n", pmap,
            (uint64_t)pmap_start, pmap_start + size, prot);
      }
      pmap_protect_options(pmap, pmap_start, pmap_start + size, prot,
                           options & ~PMAP_OPTIONS_NOFLUSH, NULL);
      return;
    ...
    if (pmap_page_size < PAGE_SIZE) {
      DEBUG4K_PMAP("pmap %p start 0x%llx end 0x%llx prot 0x%x: offset 0x%llx "
                   "size 0x%llx object %p offset 0x%llx size 0x%llx\n",
                   pmap, (uint64_t)pmap_start, pmap_start + size, prot, offset,
                   size, object, offset_in_object, size_in_object);
    }
    ...
    if (ptoa_64(object->resident_page_count / 4) < size_in_object) {
      ...
      vm_page_queue_iterate(&object->memq, p, vmp_listq) {
        if (!vm_page_is_fictitious(p) && (offset_in_object <= p->vmp_offset) &&
            (p->vmp_offset < end)) {
          ...
          if (pmap != PMAP_NULL) {
            ...
            for (curr = start; curr < start + PAGE_SIZE_64;
                 curr += pmap_page_size) {
              if (curr < pmap_start) {
                continue;
              }
              ...
              pmap_protect_options(pmap, curr, curr + pmap_page_size, prot,
                                   options | PMAP_OPTIONS_NOFLUSH,
                                   &pmap_flush_context_storage);
            }
          } else {
            pmap_page_protect_options(VM_PAGE_GET_PHYS_PAGE(p), prot,
                                      options | PMAP_OPTIONS_NOFLUSH,
                                      &pmap_flush_context_storage);
          }
          delayed_pmap_flush = TRUE;
      ...
      for (target_off = offset_in_object; target_off < end;
        ...
        if (p != VM_PAGE_NULL) {
          ...
          if (pmap != PMAP_NULL) {
            vm_map_offset_t curr;
            for (curr = start; curr < start + PAGE_SIZE;
                 curr += pmap_page_size) {
              if (curr < pmap_start) {
                continue;
              }
...
vm_object_copy_slowly(vm_object_t src_object, vm_object_offset_t src_offset,
                      vm_object_size_t size, boolean_t interruptible,
                      vm_object_t *_result_object) /* OUT */
{
  ...
  for (; size != 0; src_offset += PAGE_SIZE_64, new_offset += PAGE_SIZE_64,
                    size -= PAGE_SIZE_64) {
    vm_page_t new_page;
    ...
    do {
      ...
      if (src_object->internal && src_object->shadow == VM_OBJECT_NULL &&
          (src_object->pager == NULL ||
           (vm_object_compressor_pager_state_get(src_object, src_offset) ==
            VM_EXTERNAL_STATE_ABSENT))) {
        boolean_t can_skip_page;

        _result_page = vm_page_lookup(src_object, src_offset);
        if (_result_page == VM_PAGE_NULL) {
           ...
           */
          can_skip_page = TRUE;
        } else if (workaround_41447923 && src_object->pager == NULL &&
                   _result_page != VM_PAGE_NULL && _result_page->vmp_busy &&
      ...
      result = vm_fault_page(src_object, src_offset, VM_PROT_READ, FALSE,
                             FALSE, /* page not looked up */
                             &prot, &_result_page, &top_page, (int *)0,
      ...
      case VM_FAULT_SUCCESS:
        ...
        vm_fault_cleanup(result_page_object, top_page);
...
vm_object_copy_quickly(vm_object_t object,                 /* IN */
...
vm_object_copy_delayed(vm_object_t src_object, vm_object_offset_t src_offset,
                       vm_object_size_t size, boolean_t src_object_shared) {
...
Retry:
  // For iOS, we want to always skip this block. For other OS types, we use the
  // sysctl to control the flow.
#if !XNU_TARGET_OS_IOS
  if (!vm_object_copy_delayed_paging_wait_disable) {
    /*
     * Wait for paging in progress.
     */
    if (!src_object->true_share && (src_object->paging_in_progress != 0 ||
  ...
  if (old_copy != VM_OBJECT_NULL) {
    ...
    if (old_copy->resident_page_count == 0 && !old_copy->pager_created) {
      /*
      ...
      if (old_copy->vo_size < copy_size) {
        ...
        vm_page_queue_iterate(&src_object->memq, p, vmp_listq) {
          if (!vm_page_is_fictitious(p) && p->vmp_offset >= old_copy->vo_size &&
            ...
            if (VM_PAGE_WIRED(p)) {
              ...
              pmap_page_protect_options(
                  ...
                  (p->vmp_xpmapped ? (VM_PROT_READ | VM_PROT_EXECUTE)
                                   : VM_PROT_READ),
        ...
        assertf(page_aligned(copy_size), "object %p size 0x%llx", old_copy,
                (uint64_t)copy_size);
    ...
    assertf(page_aligned(copy_size), "object %p size 0x%llx", new_copy,
            (uint64_t)copy_size);
  ...
  if (old_copy != VM_OBJECT_NULL) {
    ...
    os_ref_release_live_locked_raw(&src_object->ref_count, &vm_object_refgrp);
...
__private_extern__ kern_return_t vm_object_copy_strategically(
    vm_object_t src_object, vm_object_offset_t src_offset,
    vm_object_size_t size, bool forking, vm_object_t *dst_object, /* OUT */
    ...
    boolean_t *dst_needs_copy)                                    /* OUT */
{
  ...
  while (!src_object->internal && !src_object->pager_ready) {
    ...
    wait_result = vm_object_sleep(src_object, VM_OBJECT_EVENT_PAGER_READY,
  ...
  switch (copy_strategy) {
  case MEMORY_OBJECT_COPY_DELAY:
    *dst_object = vm_object_copy_delayed(src_object, src_offset, size,
                                         object_lock_shared);
  ...
  case MEMORY_OBJECT_COPY_NONE:
    result = vm_object_copy_slowly(src_object, src_offset, size, interruptible,
                                   dst_object);
    if (result == KERN_SUCCESS) {
  ...
  default:
    panic("copy_strategically: bad strategy %d for object %p", copy_strategy,
          src_object);
...
vm_object_shadow(vm_object_t *object,        /* IN/OUT */
                 ...
                 vm_object_size_t length, boolean_t always_shadow) {
  vm_object_t source;
  vm_object_t result;
  ...
  assert(source->copy_strategy !=
         MEMORY_OBJECT_COPY_NONE); /* Purgeable objects shouldn't have shadow
                                      objects. */
   ...
   */
  if (vm_object_shadow_check && source->vo_size == length &&
  ...
  *offset = vm_object_trunc_page(*offset);
  ...
  if ((result = vm_object_allocate(length, source->vmo_provenance)) ==
      VM_OBJECT_NULL) {
  ...
  assertf(page_aligned(result->vo_shadow_offset),
          "result %p shadow offset 0x%llx", result, result->vo_shadow_offset);
  ...
  *offset = 0;
  if (offset_adjustment) {
     ...
     * in the new object.
     */
    DEBUG4K_COPY("adjusting offset @ %p from 0x%llx to 0x%llx for object %p "
                 "length: 0x%llx\n",
 ...
 */
vm_object_t vm_object_memory_object_associate(memory_object_t pager,
                                              vm_object_t object,
  ...
  (void)memory_object_init(pager, object->pager_control, PAGE_SIZE);
...
void vm_object_compressor_pager_create(vm_object_t object) {
  ...
  if ((uint32_t)(object->vo_size / PAGE_SIZE) !=
    ...
    printf("vm_object_compressor_pager_create(%p): "
           ...
           object, (uint64_t)object->vo_size, 0x0FFFFFFFFULL * PAGE_SIZE);
#endif /* DEVELOPMENT || DEBUG */
    vm_object_paging_end(object);
    return;
  }
   ...
   */
  {
    ...
    assert((uint32_t)(object->vo_size / PAGE_SIZE) ==
           (object->vo_size / PAGE_SIZE));
    (void)compressor_memory_object_create((memory_object_size_t)object->vo_size,
                                          &pager);
    if (pager == NULL) {
  ...
  pager_object =
      vm_object_memory_object_associate(pager, object, object->vo_size, FALSE);
  if (pager_object != object) {
    panic("vm_object_compressor_pager_create: mismatch (pager: %p, "
          "pager_object: %p, orig_object: %p, orig_object size: 0x%llx)",
...
vm_external_state_t
vm_object_compressor_pager_state_get(vm_object_t object,
                                     vm_object_offset_t offset) {
  if (__probable(not_in_kdp)) {
    vm_object_lock_assert_held(object);
  }
  if (object->internal && object->pager != NULL && !object->terminating &&
      object->alive) {
...
void vm_object_compressor_pager_state_clr(vm_object_t object,
                                          vm_object_offset_t offset) {
  unsigned int num_pages_cleared;
  vm_object_lock_assert_exclusive(object);
  if (object->internal && object->pager != NULL && !object->terminating &&
      object->alive) {
    num_pages_cleared = vm_compressor_pager_state_clr(
        object->pager, offset + object->paging_offset);
    if (num_pages_cleared) {
      vm_compressor_pager_count(object->pager, -num_pages_cleared,
                                FALSE, /* shared */
                                object);
    }
    if (num_pages_cleared &&
...
void vm_object_do_collapse_compressor(vm_object_t object,
  ...
  for (backing_offset = object->vo_shadow_offset;
    ...
    backing_pager_offset = (backing_offset + backing_object->paging_offset);
    backing_pager_offset = vm_compressor_pager_next_compressed(
        backing_object->pager, backing_pager_offset);
    if (backing_pager_offset == (memory_object_offset_t)-1) {
      /* no more compressed pages */
      break;
    }
    backing_offset = (backing_pager_offset - backing_object->paging_offset);
    ...
    vm_compressor_pager_transfer(
        /* destination: */
        object->pager, (new_offset + object->paging_offset),
        /* source: */
...
static void vm_object_do_collapse(vm_object_t object,
  ...
  while (!vm_page_queue_empty(&backing_object->memq)) {
    ...
    if (p->vmp_offset < backing_offset || new_offset >= size) {
      ...
      if (pp == VM_PAGE_NULL) {
        if (vm_object_compressor_pager_state_get(object, new_offset) ==
            VM_EXTERNAL_STATE_EXISTS) {
          /*
  ...
  if (vm_object_collapse_compressor_allowed &&
    ...
    if (object->pager_control != MEMORY_OBJECT_CONTROL_NULL) {
      memory_object_control_collapse(&object->pager_control, object);
    }
  ...
  if (object->shadow) {
    assertf(page_aligned(object->vo_shadow_offset),
            "object %p shadow_offset 0x%llx", object, object->vo_shadow_offset);
    assertf(page_aligned(backing_object->vo_shadow_offset),
            "backing_object %p shadow_offset 0x%llx", backing_object,
            backing_object->vo_shadow_offset);
...
retry:
  ...
  while (TRUE) {
    ...
    if (!backing_object->internal || backing_object->paging_in_progress != 0 ||
    ...
    if (os_ref_get_count_raw(&backing_object->ref_count) == 1 &&
        (vm_object_collapse_compressor_allowed || !object->pager_created ||
         (!backing_object->pager_created)) &&
   ...
   ((rc) && vm_page_lookup((obj), (off)) != VM_PAGE_NULL && (rc)--))
      ...
      if (object->cow_hint != ~(vm_offset_t)0) {
        ...
        hint_offset = (hint_offset > 8 * PAGE_SIZE_64)
                          ? (hint_offset - 8 * PAGE_SIZE_64)
                          : 0;
      }

      if (EXISTS_IN_OBJECT(backing_object, hint_offset + backing_offset,
                           backing_rcount) &&
      ...
      if (backing_rcount && backing_rcount < (object_vcount / 8)) {
        ...
        do {
          ...
          if (offset < object->vo_size && offset != hint_offset &&
      ...
      if (backing_rcount) {
        ...
        while ((offset = (offset + PAGE_SIZE_64 < object->vo_size)
                             ? (offset + PAGE_SIZE_64)
                             : 0) != hint_offset) {
          if (EXISTS_IN_OBJECT(backing_object, offset + backing_offset,
                               backing_rcount) &&
...
__private_extern__ void vm_object_page_remove(vm_object_t object,
                                              vm_object_offset_t start,
                                              vm_object_offset_t end) {
  vm_page_t p, next;
...
vm_object_coalesce(vm_object_t prev_object, vm_object_t next_object,
                   ...
                   vm_object_size_t prev_size, vm_object_size_t next_size) {
  ...
  if ((os_ref_get_count_raw(&prev_object->ref_count) > 1) ||
      prev_object->pager_created || prev_object->phys_contiguous ||
  ...
  if (__improbable(
          os_add3_overflow(prev_offset, prev_size, next_size, &newsize))) {
    vm_object_unlock(prev_object);
    return FALSE;
  }
  ...
  if (newsize > prev_object->vo_size) {
    assertf(page_aligned(newsize), "object %p size 0x%llx", prev_object,
            (uint64_t)newsize);
...
kern_return_t vm_object_populate_with_private(vm_object_t object,
                                              vm_object_offset_t offset,
                                              ppnum_t phys_page,
                                              vm_size_t size) {
  ppnum_t base_page;
  ...
  if (!object->phys_contiguous) {
    ...
    assertf(page_aligned(size), "object %p size 0x%llx", object,
            (uint64_t)size);
    object->vo_size = size;
  }
  vm_object_unlock(object);
...
kern_return_t memory_object_create_named(memory_object_t pager,
  ...
  *control = MEMORY_OBJECT_CONTROL_NULL;
  ...
  object = vm_object_memory_object_associate(pager, VM_OBJECT_NULL, size, TRUE);
  if (object == VM_OBJECT_NULL) {
...
__private_extern__ kern_return_t vm_object_lock_request(
    vm_object_t object, vm_object_offset_t offset, vm_object_size_t size,
    memory_object_return_t should_return, int flags, vm_prot_t prot) {
  ...
  (void)vm_object_update(object, offset, size, NULL, NULL, should_return, flags,
                         prot);
 ...
 */
uint64_t vm_object_purge(vm_object_t object, int flags) {
  ...
  if (object->pager != NULL) {
    ...
    if (object->activity_in_progress == 0 && object->paging_in_progress == 0) {
      /*
      ...
      if (pgcount) {
        ...
        vm_compressor_pager_count(object->pager, -pgcount, FALSE, /* shared */
                                  object);
        vm_object_owner_compressed_update(object, -pgcount);
      }
      if (!(flags & C_DONT_BLOCK)) {
        assert(vm_compressor_pager_get_count(object->pager) == 0);
      }
    } else {
      /*
  ...
  KDBG_RELEASE(VMDBG_CODE(DBG_VM_PURGEABLE_OBJECT_PURGE_ONE) | DBG_FUNC_NONE,
               ...
               object_page_count, total_purged_pgcount, skipped_object);

  return total_purged_pgcount;
}

/*
 * vm_object_purgeable_control() allows the caller to control and investigate
 * the state of a purgeable object.  A purgeable object is created via a call to
...
kern_return_t vm_object_purgable_control(vm_object_t object,
  ...
  if (control == VM_PURGABLE_SET_STATE && object->purgeable_only_by_kernel) {
    return KERN_PROTECTION_FAILURE;
  }
  ...
  case VM_PURGABLE_NONVOLATILE:
    ...
    if (old_state == VM_PURGABLE_VOLATILE || old_state == VM_PURGABLE_EMPTY) {
      /*
  ...
  case VM_PURGABLE_VOLATILE:
    if (object->volatile_fault) {
      ...
      vm_page_queue_iterate(&object->memq, p, vmp_listq) {
        if (p->vmp_busy || VM_PAGE_WIRED(p) || vm_page_is_fictitious(p)) {
          continue;
        }
        refmod = pmap_disconnect(VM_PAGE_GET_PHYS_PAGE(p));
        if ((refmod & VM_MEM_MODIFIED) && !p->vmp_dirty) {
          SET_PAGE_DIRTY(p, FALSE);
        }
      }
    }
    ...
    if (old_state == VM_PURGABLE_NONVOLATILE ||
      ...
      if ((*state & VM_PURGABLE_NO_AGING_MASK) == VM_PURGABLE_NO_AGING) {
      ...
      if (delta != 0) {
        OSAddAtomic(delta, &vm_page_purgeable_count);
      }
      if (object->wired_page_count != 0) {
        OSAddAtomic(object->wired_page_count, &vm_page_purgeable_wired_count);
      }
       ...
       * token queue. So moving priorities will not make pages stay around
       * longer. Reasoning is that the algorithm gives most priority to the most
       * important object. If a new token is added, the most important object'
       * priority is boosted. This biases the system already for purgeable
       * queues that move a lot. It doesn't seem more biasing is neccessary in
       * this case, where no new object is added.
       */
      assert(object->objq.next != NULL &&
             object->objq.prev != NULL); /* object should be on a queue */
      ...
      if (old_queue != queue ||
          (purgeable_when_ripe != object->purgeable_when_ripe)) {
        kern_return_t result;
        ...
        if (object->purgeable_when_ripe) {
          result = vm_purgeable_token_add(queue);
          assert(result == KERN_SUCCESS); /* this should never fail since we
                                             just freed a token */
        }
        vm_page_unlock_queues();
      }
    };
    vm_purgeable_object_add(object, queue,
                            (*state & VM_VOLATILE_GROUP_MASK) >>
...
kern_return_t vm_object_get_page_counts(vm_object_t object,
  ...
  if (resident_page_count != NULL && dirty_page_count == NULL && offset == 0 &&
      object->vo_size == size) {
    /*
     * Fast path when:
  ...
  if (object->resident_page_count <= (size >> PAGE_SHIFT) &&
    ...
    /*
     * Faster path when we don't care about non-resident pages and the object
     * has fewer resident pages than the requested range.
     */
    vm_page_queue_iterate(&object->memq, p, vmp_listq) {
  ...
  for (cur_offset = offset; cur_offset < end_offset;
       cur_offset += PAGE_SIZE_64) {
...
kern_return_t vm_object_transpose(vm_object_t object1, vm_object_t object2,
  ...
  if (object1 == object2 || object1 == VM_OBJECT_NULL ||
  ...
  object1_locked = TRUE;
  if (!object1->alive || object1->terminating || object1->vo_copy ||
      object1->shadow || object1->shadowed ||
  ...
  object2_locked = TRUE;
  if (!object2->alive || object2->terminating || object2->vo_copy ||
      object2->shadow || object2->shadowed ||
...
#define __TRANSPOSE_FIELD(field)                                               \
  ...
  if (object1->pager_control != MEMORY_OBJECT_CONTROL_NULL) {
    memory_object_control_collapse(&object1->pager_control, object1);
  }
  if (object2->pager_control != MEMORY_OBJECT_CONTROL_NULL) {
    memory_object_control_collapse(&object2->pager_control, object2);
  }
...
#endif
  assert((object1->purgable == VM_PURGABLE_DENY) ||
         (object1->objq.next == NULL));
  assert((object1->purgable == VM_PURGABLE_DENY) ||
         (object1->objq.prev == NULL));
  assert((object2->purgable == VM_PURGABLE_DENY) ||
         (object2->objq.next == NULL));
  assert((object2->purgable == VM_PURGABLE_DENY) ||
         (object2->objq.prev == NULL));
...
/*
 * Try to always keep these values an even multiple of PAGE_SIZE. We use these
 * values to derive min_ph_bytes and max_ph_bytes (IMP: bytes not # of pages)
 * and expect those values to always be page-aligned. The derivation could
 * involve operations (e.g. division) that could give us non-page-size aligned
 * values if we start out with values that are odd multiples of PAGE_SIZE.
 */
...
vm_object_cluster_size(vm_object_t object, vm_object_offset_t *start,
  ...
  *io_streaming = 0;
    ...
    if (sequential_run >= (3 * PAGE_SIZE)) {
      ...
      *io_streaming = 1;
      ...
      if (object->pages_created < (20 * (min_ph_size >> PAGE_SHIFT))) {
         ...
         */
        pre_heat_size =
            (max_length * (uint64_t)object->pages_used) / object->pages_created;
  ...
  if (vnode_pager_get_throttle_io_limit(object->pager, &throttle_limit) ==
      KERN_SUCCESS) {
  ...
  if (look_ahead == TRUE) {
     ...
     * address that we want to consider... this is guaranteed not to extend
     * beyond the current EOF...
     */
    assert((vm_size_t)(orig_start - target_start) ==
           (orig_start - target_start));
    tail_size =
        pre_heat_size - (vm_size_t)(orig_start - target_start) - PAGE_SIZE;
  } else {
  ...
  if (look_behind == TRUE) {
    ...
    for (offset = orig_start - PAGE_SIZE_64; head_size;
         offset -= PAGE_SIZE_64, head_size -= PAGE_SIZE) {
      /*
       ...
       * vm_object_compressor_pager_state_get will return
  ...
  if (look_ahead == TRUE) {
    for (offset = orig_start + PAGE_SIZE_64; tail_size;
         offset += PAGE_SIZE_64, tail_size -= PAGE_SIZE) {
      /*
...
kern_return_t vm_object_page_op(vm_object_t object, vm_object_offset_t offset,
                                int ops, ppnum_t *phys_entry, int *flags) {
  vm_page_t dst_page;
  ...
  if (ops & UPL_POP_PHYSICAL) {
    if (object->phys_contiguous) {
      if (phys_entry) {
        *phys_entry = (ppnum_t)(object->vo_shadow_offset >> PAGE_SHIFT);
      }
      vm_object_unlock(object);
      return KERN_SUCCESS;
    } else {
  ...
  while (TRUE) {
    ...
    if ((dst_page->vmp_busy || dst_page->vmp_cleaning) &&
        (((ops & UPL_POP_SET) && (ops & UPL_POP_BUSY)) ||
         (ops & UPL_POP_DUMP))) {
...
kern_return_t vm_object_range_op(vm_object_t object,
                                 vm_object_offset_t offset_beg,
                                 vm_object_offset_t offset_end, int ops,
                                 uint32_t *range) {
  vm_object_offset_t offset;
  vm_page_t dst_page;
...
/*
 * Used to point a pager directly to a range of memory (when the pager may be
 * associated with a non-device vnode).  Takes a virtual address, an offset, and
 * a size.  We currently expect that the virtual address will denote the start
 * of a range that is physically contiguous.
 */
kern_return_t pager_map_to_phys_contiguous(memory_object_control_t object,
  ...
  retval =
      vm_object_populate_with_private(pager_object, offset, page_num, size);
 ...
 * acquiring the lock.
 */
bool vm_object_lock_check_contended(vm_object_t object) {
  if (object == vm_pageout_scan_wants_object) {
    scan_object_collision++;
    mutex_pause(2);
  }
...
boolean_t vm_object_lock_yield_shared(vm_object_t object) {
...
boolean_t vm_object_lock_upgrade(vm_object_t object) {
  boolean_t retval;
 ...
 * The object must be locked
 */
void vm_object_change_wimg_mode(vm_object_t object, unsigned int wimg_mode) {
  vm_object_lock_assert_exclusive(object);
...
static int c_freezer_should_yield() {
...
uint32_t vm_object_compressed_freezer_pageout(vm_object_t object,
  ...
  if (VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
    ...
    while (curr_offset < object->vo_size) {
      curr_offset =
          vm_compressor_pager_next_compressed(object->pager, curr_offset);

      if (curr_offset == (vm_object_offset_t)-1) {
        break;
      }

      retval = vm_compressor_pager_relocate(
          object->pager, curr_offset,
          &(freezer_context_global.freezer_ctx_chead));
   ...
   *
   * Since we are going to drop/grab the object lock repeatedly, we must make
   * sure we won't be stuck in an infinite loop if the same page(s) keep getting
   * decompressed. So we grab a snapshot of the number of pages in the object
   * and we won't process any more than that number of pages.
   */
  ...
  while ((obj_resident_page_count_snapshot--) &&
         !vm_page_queue_empty(&object->memq) &&
         paged_out_count < dirty_budget) {
    p = (vm_page_t)vm_page_queue_first(&object->memq);
    ...
    if (p->vmp_cleaning || vm_page_is_fictitious(p) || p->vmp_busy ||
        p->vmp_absent || p->vmp_unusual || VMP_ERROR_GET(p) ||
        VM_PAGE_WIRED(p)) {
      vm_page_unlock_queues();
    ...
    if (p->vmp_pmapped == TRUE) {
      ...
      vm_page_lockconvert_queues();
      refmod_state =
          pmap_disconnect_options(VM_PAGE_GET_PHYS_PAGE(p), pmap_flags, NULL);
      if (refmod_state & VM_MEM_MODIFIED) {
        SET_PAGE_DIRTY(p, FALSE);
      }
    }
    ...
    /*
     * Grab an activity_in_progress here for vm_pageout_compress_page() to
     * consume.
     *
     * Mark the page busy so no one messes with it while we have the object lock
     * dropped.
     */
    p->vmp_busy = TRUE;
...
void vm_object_pageout(vm_object_t object) {
  vm_page_t p, next;
  ...
  if (!object->internal || object->terminating || !object->alive) {
    vm_object_unlock(object);
    return;
  }
...
ReScan:
  ...
  while (!vm_page_queue_end(&object->memq, (vm_page_queue_entry_t)next)) {
    ...
    if ((p->vmp_q_state == VM_PAGE_ON_THROTTLED_Q) || p->vmp_cleaning ||
        p->vmp_laundry || p->vmp_busy || p->vmp_absent || VMP_ERROR_GET(p) ||
        vm_page_is_fictitious(p) || VM_PAGE_WIRED(p)) {
      /*
    ...
    if (p->vmp_q_state == VM_PAGE_NOT_ON_Q) {
      //			printf("FBDP %s:%d page %p object %p offset
      //0x%llx state %d not on queue\n", __FUNCTION__, __LINE__, p,
      //VM_PAGE_OBJECT(p), p->vmp_offset, p->vmp_q_state);
    ...
    assert(!VMP_ERROR_GET(
        p)); /* XXX there's a window here where we could have an ECC error! */
    ...
    if (p->vmp_pmapped == TRUE) {
      ...
      vm_page_lockconvert_queues();
      refmod_state =
          pmap_disconnect_options(VM_PAGE_GET_PHYS_PAGE(p), pmap_options, NULL);
      if (refmod_state & VM_MEM_MODIFIED) {
        SET_PAGE_DIRTY(p, FALSE);
      }
    }
...
void vm_page_request_reprioritize(vm_object_t o, uint64_t blkno, uint32_t len,
                                  int prio) {
  io_reprioritize_req_t req;
  struct vnode *devvp = NULL;

  if (vnode_pager_get_object_devvp(o->pager, (uintptr_t *)&devvp) !=
      KERN_SUCCESS) {
    return;
  }
  ...
  mpsc_daemon_enqueue(&io_reprioritize_q, &req->iorr_elm,
                      MPSC_QUEUE_DISABLE_PREEMPTION);

  return;
}

void vm_decmp_upl_reprioritize(upl_t upl, int prio) {
  int offset;
  vm_object_t object;
  io_reprioritize_req_t req;
  struct vnode *devvp = NULL;
  ...
  if ((upl->flags & UPL_TRACKED_BY_OBJECT) == 0 ||
      (upl->flags & UPL_EXPEDITE_SUPPORTED) == 0) {
    return;
  }

  /*
   * We dont want to perform any allocations with the upl lock held since that
   * might result in a deadlock. If the system is low on memory, the pageout
   * thread would try to pageout stuff and might wait on this lock. If we are
   * waiting for the memory to be freed up by the pageout thread, it would be a
   * deadlock.
   */

  /* First step is just to get the size of the upl to find out how big the
   * reprio info is */
  if (!upl_try_lock(upl)) {
    return;
  }
  ...
  if (upl->decmp_io_upl == NULL) {
    /* The real I/O upl was destroyed by the time we came in here. Nothing to
     * do. */
    upl_unlock(upl);
    return;
  }
  ...
  io_upl_reprio_info =
      kalloc_data(sizeof(uint64_t) * atop(io_upl_size), Z_WAITOK);
  ...
  if (!object || !object->pager ||
      vnode_pager_get_object_devvp(object->pager, (uintptr_t *)&devvp) !=
          KERN_SUCCESS) {
    upl_unlock(upl);
    goto out;
  }
  ...
  while (offset < io_upl_size) {
    ...
    len = (io_upl_reprio_info[atop(offset)] >> UPL_REPRIO_INFO_SHIFT) &
    ...
    if (blkno != 0 && len != 0) {
      /* Create the request for I/O reprioritization */
      req = zalloc_flags(io_reprioritize_req_zone, Z_WAITOK | Z_NOFAIL);
      req->blkno = blkno;
      req->len = len;
      req->priority = prio;
      req->devvp = devvp;
...
void vm_page_handle_prio_inversion(vm_object_t o, vm_page_t m) {
  upl_t upl;
  upl_page_info_t *pl;
  ...
  queue_iterate(&o->uplq, upl, upl_t, uplq) {
    ...
    for (i = 0; i < num_pages; i++) {
      if (UPL_PAGE_PRESENT(pl, i) &&
          VM_PAGE_GET_PHYS_PAGE(m) == pl[i].phys_addr) {
        if ((upl->flags & UPL_DECMP_REQ) && upl->decmp_io_upl) {
          KDBG((VMDBG_CODE(DBG_VM_PAGE_EXPEDITE)) | DBG_FUNC_NONE,
               VM_KERNEL_UNSLIDE_OR_PERM(upl->upl_creator),
               VM_KERNEL_UNSLIDE_OR_PERM(m), VM_KERNEL_UNSLIDE_OR_PERM(upl),
               upl->upl_priority);
        ...
        KDBG((VMDBG_CODE(DBG_VM_PAGE_EXPEDITE)) | DBG_FUNC_NONE,
             VM_KERNEL_UNSLIDE_OR_PERM(upl->upl_creator),
             VM_KERNEL_UNSLIDE_OR_PERM(m), upl->upl_reprio_info[i],
             upl->upl_priority);
        if (UPL_REPRIO_INFO_BLKNO(upl, i) != 0 &&
            UPL_REPRIO_INFO_LEN(upl, i) != 0) {
          vm_page_request_reprioritize(o, UPL_REPRIO_INFO_BLKNO(upl, i),
                                       UPL_REPRIO_INFO_LEN(upl, i), cur_tier);
        }
        break;
      }
    }
...
void kdp_vm_object_sleep_find_owner(event64_t wait_event,
                                    block_hint_t wait_type,
                                    thread_waitinfo_t *waitinfo) {
  assert(wait_type >= kThreadWaitPagerInit &&
         wait_type <= kThreadWaitPageInThrottle);
...
wait_result_t vm_object_sleep(vm_object_t object,
  ...
  KDBG_FILTERED(VMDBG_CODE(DBG_VM_OBJECT_SLEEP) | DBG_FUNC_START,
                VM_KERNEL_ADDRHIDE(object), reason);

  vm_object_set_wanted(object, reason);
  wait_event = (event_t)((uintptr_t)object + (uintptr_t)reason);
  wr = lck_rw_sleep(&object->Lock, LCK_SLEEP_PROMOTED_PRI | action, wait_event,
                    interruptible);

  KDBG_FILTERED(VMDBG_CODE(DBG_VM_OBJECT_SLEEP) | DBG_FUNC_END,
                VM_KERNEL_ADDRHIDE(object), reason, wr);
  return wr;
}

wait_result_t vm_object_paging_wait(vm_object_t object,
                                    wait_interrupt_t interruptible) {
  wait_result_t wr = THREAD_NOT_WAITING;
  vm_object_lock_assert_exclusive(object);
  while (object->paging_in_progress != 0 || object->activity_in_progress != 0) {
    wr = vm_object_sleep((object), VM_OBJECT_EVENT_PAGING_IN_PROGRESS,
                         interruptible, LCK_SLEEP_EXCLUSIVE);
...
wait_result_t vm_object_paging_only_wait(vm_object_t object,
                                         wait_interrupt_t interruptible) {
  wait_result_t wr = THREAD_NOT_WAITING;
  vm_object_lock_assert_exclusive(object);
  while (object->paging_in_progress != 0) {
    wr = vm_object_sleep(object, VM_OBJECT_EVENT_PAGING_ONLY_IN_PROGRESS,
                         interruptible, LCK_SLEEP_EXCLUSIVE);
    if (wr != THREAD_AWAKENED) {
      break;
    }
...
wait_result_t vm_object_paging_throttle_wait(vm_object_t object,
                                             wait_interrupt_t interruptible) {
  wait_result_t wr = THREAD_NOT_WAITING;
  vm_object_lock_assert_exclusive(object);
  /*
  ...
  while (object->paging_in_progress >= vm_object_pagein_throttle) {
    wr = vm_object_sleep(object, VM_OBJECT_EVENT_PAGEIN_THROTTLE, interruptible,
                         LCK_SLEEP_EXCLUSIVE);
    if (wr != THREAD_AWAKENED) {
      break;
    }
...
wait_result_t vm_object_mapping_wait(vm_object_t object,
                                     wait_interrupt_t interruptible) {
  wait_result_t wr = THREAD_NOT_WAITING;
  vm_object_lock_assert_exclusive(object);
  while (object->mapping_in_progress) {
    wr = vm_object_sleep(object, VM_OBJECT_EVENT_MAPPING_IN_PROGRESS,
                         interruptible, LCK_SLEEP_EXCLUSIVE);
    if (wr != THREAD_AWAKENED) {
      break;
    }
...
void vm_object_wakeup(vm_object_t object, vm_object_wait_reason_t reason) {
  vm_object_lock_assert_exclusive(object);
  assert(reason >= 0 && reason <= VM_OBJECT_EVENT_MAX);
...
void kdp_vm_page_sleep_find_owner(event64_t wait_event,
                                  thread_waitinfo_t *waitinfo) {
...
vm_page_sleep_with_inheritor(lck_rw_t *lck, lck_sleep_action_t lck_sleep_action,
                             event_t event, wait_interrupt_t interruptible);
#endif /* PAGE_SLEEP_WITH_INHERITOR */

wait_result_t vm_page_sleep(vm_object_t object, vm_page_t m,
                            wait_interrupt_t interruptible,
                            lck_sleep_action_t action) {
  wait_result_t ret;

  KDBG_FILTERED((VMDBG_CODE(DBG_VM_PAGE_SLEEP)) | DBG_FUNC_START,
                VM_KERNEL_ADDRHIDE(object), m->vmp_offset,
                VM_KERNEL_ADDRHIDE(m));
#if CONFIG_IOSCHED
  if (object->io_tracking && ((m->vmp_busy == TRUE) ||
                              (m->vmp_cleaning == TRUE) || VM_PAGE_WIRED(m))) {
    /*
     *  Indicates page is busy due to an I/O. Issue a reprioritize request if
     * necessary.
     */
...
#if PAGE_SLEEP_WITH_INHERITOR
  ret = vm_page_sleep_with_inheritor(&object->Lock, action, (event_t)m,
                                     interruptible);
#else
  ret = lck_rw_sleep(&object->Lock, LCK_SLEEP_PROMOTED_PRI | action, (event_t)m,
                     interruptible);
#endif
  KDBG_FILTERED((VMDBG_CODE(DBG_VM_PAGE_SLEEP)) | DBG_FUNC_END,
                VM_KERNEL_ADDRHIDE(object), m->vmp_offset,
                VM_KERNEL_ADDRHIDE(m));
  return ret;
}

void vm_page_wakeup(vm_object_t object, vm_page_t m) {
  assert(m);
  /*
  ...
  if (m->vmp_wanted) {
    KDBG(VMDBG_CODE(DBG_VM_PAGE_WAKEUP) | DBG_FUNC_NONE,
         VM_KERNEL_ADDRHIDE(object), m->vmp_offset, VM_KERNEL_ADDRHIDE(m));
    m->vmp_wanted = false;
    thread_wakeup((event_t)m);
  }
}

void vm_page_wakeup_done(__assert_only vm_object_t object, vm_page_t m) {
  assert(object);
  assert(m->vmp_busy);
  vm_object_lock_assert_exclusive(object);
...
#if PAGE_SLEEP_WITH_INHERITOR
static bool page_worker_unregister_worker(event_t event, thread_t expect_th,
                                          page_worker_token_t *token);
#endif /* PAGE_SLEEP_WITH_INHERITOR */

/* This function duplicates all of what vm_page_wakeup_done() does and adds the
 * option that we're being called from vm_fault_page() in a page that is
 * possibly boosted due to being an inheritor*/
void vm_page_wakeup_done_with_inheritor(vm_object_t object __unused,
                                        vm_page_t m,
                                        page_worker_token_t *token __unused) {
#if PAGE_SLEEP_WITH_INHERITOR
  assert(object);
  assert(m->vmp_busy);
  vm_object_lock_assert_exclusive(object);
  ...
  bool had_inheritor =
      page_worker_unregister_worker((event_t)m, current_thread(), token);

  KDBG(VMDBG_CODE(DBG_VM_PAGE_WAKEUP_DONE) | DBG_FUNC_NONE,
       VM_KERNEL_ADDRHIDE(object), VM_KERNEL_ADDRHIDE(m), m->vmp_wanted,
       had_inheritor);
  m->vmp_busy = FALSE;
 ...
 * The goal of this functionality is to prevent priority inversion that can
 * occur when a low-priority thread is stuck in the compressor and a higher
 * priority thread waits for the same page. Just before vm_fault_page() calls
 * into the compressor it calls page_worker_register_worker() this registers the
 * calling thread as the "page worker" of this page. When another thread then
 * tries to vm_page_sleep() on that page, (wait for it to un-busy) the worker is
 * found and instead of a plain thread_block() (in lck_rw_sleep()) we do
 * lck_rw_sleep_with_inheritor() and give the registered worker thread as the
 * inheritor of the priority boost. The worker thread might have started its
 * work on a low priority, and when a waiter was added, it got boost. When the
 * worker is done getting the page it calls vm_page_wakeup_done_with_inheritor()
 * instead of vm_page_wakeup_done() this unregisters the thread, clears the page
 * busy bit (so that now other threads can use this page), and wakes up any
 * waiters waiting for that page with wakeup_all_with_inheritor(), which removes
 * the priority boost.
 *
 * The worker registration is done in a simple single entry per bucket hash
 * table. A hash collision may occur if two faulting pages end up in the same
 * entry. In this case, the registration of the second one is going to fail and
 * the only repercussions of this is that it would not get the possible boost if
 * anyone is going to wait on it. This implementation was selected over a full
 * hash-table to keep it simple and fast.
 */
...
void page_worker_init(void) {
  page_worker_table_size = PAGE_WORKER_TABLE_BUCKETS;
#if DEVELOPMENT || DEBUG
  PE_parse_boot_argn("page_worker_table_size", &page_worker_table_size,
                     sizeof(page_worker_table_size));
#endif /* DEVELOPMENT || DEBUG */
  /* This checks that the size is a positive power of 2, needed for the hash
   * function */
  assert(page_worker_table_size > 0 &&
         !(page_worker_table_size & (page_worker_table_size - 1)));

  page_worker_table = zalloc_permanent(
      page_worker_table_size * sizeof(struct page_worker), ZALIGN_PTR);
...
static struct page_worker *page_worker_lock_table_entry(event_t event) {
  if (page_worker_table == NULL) {
    return NULL;
  }
...
static struct page_worker *page_worker_lookup(event_t event,
                                              bool try_add_missing) {
  assert(event != NULL);
...
void page_worker_register_worker(event_t event __unused,
  ...
  /* no need to take the thread reference because this is going to get cleared
   * in the same call of vm_page_fault() */
  page_worker_entry_unlock(entry);
  out_token->pwt_did_register_inheritor = true;
}

static bool page_worker_unregister_worker(event_t event,
                                          thread_t expect_th __unused,
                                          page_worker_token_t *token) {
  ...
  assert(token->pwt_did_register_inheritor);
  assert(
      token->pwt_floor_token.thread ==
      THREAD_NULL); /* we shouldn't have done thread_priority_floor_start() */
...
static wait_result_t
vm_page_sleep_with_inheritor(lck_rw_t *lck, lck_sleep_action_t action,
                             event_t event, wait_interrupt_t interruptible) {
  struct page_worker *entry = page_worker_lookup(event, FALSE);
  ...
  if (inheritor == THREAD_NULL) {
    /* no worker was found */
    ret = lck_rw_sleep(lck, LCK_SLEEP_PROMOTED_PRI | action, event,
                       interruptible);
  } else {
    counter_inc(&page_worker_inheritor_sleeps);
    ret = lck_rw_sleep_with_inheritor(lck, action, event, inheritor,
                                      interruptible, TIMEOUT_WAIT_FOREVER);
  }
...
static void io_reprioritize(mpsc_queue_chain_t elm,
                            __assert_only mpsc_daemon_queue_t dq) {
  assert3p(dq, ==, &io_reprioritize_q);
  io_reprioritize_req_t req =
      mpsc_queue_element(elm, struct io_reprioritize_req, iorr_elm);
  vnode_pager_issue_reprioritize_io(req->devvp, req->blkno, req->len,
                                    req->priority);
...
#if VM_OBJECT_ACCESS_TRACKING
void vm_object_access_tracking(vm_object_t object, int *access_tracking_p,
  ...
  *access_tracking_p = object->access_tracking;
  ...
  if (access_tracking) {
    vm_object_pmap_protect_options(object, 0, object->vo_size, PMAP_NULL,
                                   PAGE_SIZE, 0, VM_PROT_NONE, 0);
  }
}
#endif /* VM_OBJECT_ACCESS_TRACKING */

void vm_object_ledger_tag_ledgers(vm_object_t object, int *ledger_idx_volatile,
  ...
  *do_footprint = !object->vo_no_footprint;
  ...
  case VM_LEDGER_TAG_DEFAULT:
    ...
    *ledger_idx_volatile_compressed =
        task_ledgers.tagged_nofootprint_compressed;
    if (*do_footprint) {
  ...
  case VM_LEDGER_TAG_NEURAL:
    ...
    *ledger_idx_volatile_compressed =
    ...
    break;
  default:
    panic("%s: object %p has unsupported ledger_tag %d", __FUNCTION__, object,
          object->vo_ledger_tag);
  }
}

kern_return_t vm_object_ownership_change(vm_object_t object, int new_ledger_tag,
                                         task_t new_owner, int new_ledger_flags,
  ...
  if (new_owner != VM_OBJECT_OWNER_DISOWNED && new_owner != TASK_NULL) {
    if (new_ledger_tag == VM_LEDGER_TAG_NONE &&
        object->purgable == VM_PURGABLE_DENY) {
      /* non-purgeable memory must have a valid non-zero ledger tag */
      return KERN_INVALID_ARGUMENT;
    }
    if (!object->internal && !memory_object_is_vnode_pager(object->pager)) {
  ...
  if (new_ledger_tag < 0 || new_ledger_tag > VM_LEDGER_TAG_MAX) {
    return KERN_INVALID_ARGUMENT;
  }
  if (new_ledger_flags & ~VM_LEDGER_FLAGS_ALL) {
    return KERN_INVALID_ARGUMENT;
  }
  if (object->internal && object->vo_ledger_tag == VM_LEDGER_TAG_NONE &&
      object->purgable == VM_PURGABLE_DENY) {
    /*
     ...
     */
    if (object->resident_page_count != 0 || object->pager != NULL ||
...
#if __arm64__
  if (!new_no_footprint && object->purgable != VM_PURGABLE_DENY &&
      new_owner != TASK_NULL && new_owner != VM_OBJECT_OWNER_DISOWNED &&
  ...
  if (__improbable(vm_debug_events)) {
    DTRACE_VM8(object_ownership_change, vm_object_t, object, task_t, old_owner,
               int, old_ledger_tag, int, old_no_footprint, task_t, new_owner,
               int, new_ledger_tag, int, new_no_footprint, int,
               VM_OBJECT_ID(object));
  }
  ...
  if (old_owner != TASK_NULL &&
    ...
    if (object->internal) {
      ...
      if (resident_count) {
         ...
         */
        ledger_debit(old_owner->ledger, ledger_idx, ptoa_64(resident_count));
        /* adjust old owner's footprint */
        if (object->purgable != VM_PURGABLE_VOLATILE &&
            object->purgable != VM_PURGABLE_EMPTY) {
          if (do_footprint) {
            ledger_debit(old_owner->ledger, task_ledgers.phys_footprint,
                         ptoa_64(resident_count));
          } else if (ledger_idx_composite != -1) {
            ledger_debit(old_owner->ledger, ledger_idx_composite,
                         ptoa_64(resident_count));
          }
        }
      }
      if (wired_count) {
        ...
        ledger_debit(old_owner->ledger, ledger_idx_nonvolatile,
                     ptoa_64(wired_count));
        if (do_footprint) {
          ledger_debit(old_owner->ledger, task_ledgers.phys_footprint,
                       ptoa_64(wired_count));
      ...
      if (compressed_count) {
         ...
         */
        ledger_debit(old_owner->ledger, ledger_idx_compressed,
      ...
      if (wired_count) {
        ledger_debit(old_owner->ledger, ledger_idx_external_wired,
                     ptoa_64(wired_count));
        if (do_footprint) {
          ledger_debit(old_owner->ledger, task_ledgers.phys_footprint,
                       ptoa_64(wired_count));
    ...
    if (old_owner != new_owner) {
      ...
      DTRACE_VM2(object_owner_remove, vm_object_t, object, task_t, old_owner);
      if (!old_task_objq_locked) {
        task_objq_lock(old_owner);
      }
      old_owner->task_owned_objects--;
      queue_remove(&old_owner->task_objq, object, vm_object_t, task_objq);
      switch (object->purgable) {
      case VM_PURGABLE_NONVOLATILE:
      case VM_PURGABLE_EMPTY:
        vm_purgeable_nonvolatile_owner_update(old_owner, -1);
        break;
      case VM_PURGABLE_VOLATILE:
        vm_purgeable_volatile_owner_update(old_owner, -1);
        break;
      default:
        break;
      }
  ...
  if (new_owner != old_owner && new_owner != TASK_NULL &&
  ...
  if (new_owner != TASK_NULL &&
    ...
    if (object->internal) {
      ...
      if (resident_count) {
         ...
         * number of resident pages.
         */
        ledger_credit(new_owner->ledger, ledger_idx, ptoa_64(resident_count));
        /* adjust new owner's footprint */
        if (object->purgable != VM_PURGABLE_VOLATILE &&
            object->purgable != VM_PURGABLE_EMPTY) {
          if (do_footprint) {
            ledger_credit(new_owner->ledger, task_ledgers.phys_footprint,
                          ptoa_64(resident_count));
          } else if (ledger_idx_composite != -1) {
            ledger_credit(new_owner->ledger, ledger_idx_composite,
                          ptoa_64(resident_count));
          }
        }
      }
      if (wired_count) {
        /* wired pages are always nonvolatile */
        ledger_credit(new_owner->ledger, ledger_idx_nonvolatile,
                      ptoa_64(wired_count));
        if (do_footprint) {
          ledger_credit(new_owner->ledger, task_ledgers.phys_footprint,
                        ptoa_64(wired_count));
      ...
      if (compressed_count) {
         ...
         * compressed pages.
         */
        ledger_credit(new_owner->ledger, ledger_idx_compressed,
                      ptoa_64(compressed_count));
        if (object->purgable != VM_PURGABLE_VOLATILE &&
            object->purgable != VM_PURGABLE_EMPTY) {
          if (do_footprint) {
      ...
      /* external but owned object: count wired pages */
      if (wired_count) {
        ledger_credit(new_owner->ledger, ledger_idx_external_wired,
                      ptoa_64(wired_count));
        if (do_footprint) {
          ledger_credit(new_owner->ledger, task_ledgers.phys_footprint,
                        ptoa_64(wired_count));
    ...
    if (new_owner != old_owner) {
      ...
      DTRACE_VM2(object_owner_add, vm_object_t, object, task_t, new_owner);
      assert(new_task_objq_locked);
      new_owner->task_owned_objects++;
      queue_enter(&new_owner->task_objq, object, vm_object_t, task_objq);
      switch (object->purgable) {
      case VM_PURGABLE_NONVOLATILE:
      case VM_PURGABLE_EMPTY:
        vm_purgeable_nonvolatile_owner_update(new_owner, +1);
        break;
      case VM_PURGABLE_VOLATILE:
        vm_purgeable_volatile_owner_update(new_owner, +1);
        break;
      default:
        break;
      }
...
again:
  ...
  for (object = (vm_object_t)queue_first(&task->task_objq);
    ...
    if (task->task_nonvolatile_objects == 0 &&
        task->task_volatile_objects == 0 && task->task_owned_objects == 0) {
  ...
  if (__improbable(task->task_owned_objects != 0)) {
    panic(
        "%s(%p): volatile=%d nonvolatile=%d owned=%d q=%p q_first=%p q_last=%p",
        __FUNCTION__, task, task->task_volatile_objects,
        task->task_nonvolatile_objects, task->task_owned_objects,
        &task->task_objq, queue_first(&task->task_objq),
...
void vm_object_wired_page_update_ledgers(vm_object_t object,
                                         int64_t wired_delta) {
  task_t owner;
  ...
  if (owner) {
    ...
    if (wired_delta > 0) {
      ...
      if (do_footprint) {
        ...
        ledger_credit(owner->ledger, ledger_idx_composite, ptoa(wired_delta));
      }
    } else {
      /* less external wired bytes */
      ledger_debit(owner->ledger, ledger_idx_external_wired,
                   ptoa(-wired_delta));
      if (do_footprint) {
        /* more footprint */
        ledger_debit(owner->ledger, task_ledgers.phys_footprint,
                     ptoa(-wired_delta));
      } else if (ledger_idx_composite != -1) {
        ledger_debit(owner->ledger, ledger_idx_composite, ptoa(-wired_delta));
      }
    }
  }
}
