  ...
  if (lz4sz == 0) {
    ...
    if (didwk) {
      if (__probable(wksz > lz4sz)) {
        ...
        VM_COMPRESSOR_STAT(compressor_stats.lz4_wk_compression_delta +=
                           lz4delta);
        ...
        VM_COMPRESSOR_STAT(compressor_stats.lz4_wk_compression_negative_delta +=
                           (lz4sz - wksz));
        vmcstate.lz4_failure_run_length++;
...
static inline void WKdm_hv(uint32_t *wkbuf) {
  ...
  if (*inw != MZV_MAGIC) {
    if ((*inw | *(inw + 1) | *(inw + 2)) & 0xFFFF0000) {
      panic("WKdm(%p): invalid header 0x%x 0x%x 0x%x", wkbuf, *inw, *(inw + 1),
            *(inw + 2));
    }
  }
...
static inline bool WKdmD(WK_word *src_buf, WK_word *dest_buf, WK_word *scratch,
                         unsigned int bytes, __unused uint32_t *pop_count) {
#if defined(__arm64__)
#endif
  WKdm_hv(src_buf);
#if defined(__arm64__)
...
#endif /* !____ARM_16K_PG__ */
  {
    ...
    VM_COMPRESSOR_STAT_DBG(compressor_stats.wks_dabstime +=
...
static inline int WKdmC(WK_word *src_buf, WK_word *dest_buf, WK_word *scratch,
                        boolean_t *incomp_copy, unsigned int limit,
...
#endif /* !____ARM_16K_PG__ */
  {
    ...
    VM_COMPRESSOR_STAT_DBG(compressor_stats.wks_cabstime +=
...
int metacompressor(const uint8_t *in, uint8_t *cdst, int32_t outbufsz,
                   uint16_t *codec, void *cscratchin, boolean_t *incomp_copy,
  ...
  if (dowk) {
    *codec = CCWK;
    ...
    if (sz == -1) {
      VM_COMPRESSOR_STAT(compressor_stats.wk_compressed_bytes_total +=
                         PAGE_SIZE);
...
lz4compress:
  ...
  if (dolz4) {
    ...
    *codec = CCLZ4;
    ...
    sz = (int)lz4raw_encode_buffer(cdst, outbufsz, in, insize,
                                   &cscratch->lz4state[0]);
...
bool metadecompressor(const uint8_t *source, uint8_t *dest, uint32_t csize,
                      uint16_t ccodec, void *compressor_dscratchin,
                      uint32_t *pop_count_p) {
  ...
  if (dolz4) {
    rval = (int)lz4raw_decode_buffer(dest, PAGE_SIZE, source, csize,
                                     &compressor_dscratch->lz4decodestate[0]);
    ...
    assertf(rval == PAGE_SIZE,
            "LZ4 decode: size != pgsize %d, header: 0x%x, 0x%x, 0x%x", rval,
            *d32, *(d32 + 1), *(d32 + 2));
    success = (rval == PAGE_SIZE);
  } else {
    assert(ccodec == CCWK);

    success = WKdmD(source, dest, &compressor_dscratch->wkdecompscratch[0],
                    csize, &pop_count);
...
uint32_t vm_compressor_get_encode_scratch_size(void) {
  if (vm_compressor_current_codec != VM_COMPRESSOR_DEFAULT_CODEC) {
    return MAX(sizeof(compressor_encode_scratch_t),
...
uint32_t vm_compressor_get_decode_scratch_size(void) {
  if (vm_compressor_current_codec != VM_COMPRESSOR_DEFAULT_CODEC) {
    return MAX(sizeof(compressor_decode_scratch_t),
               WKdm_SCRATCH_BUF_SIZE_INTERNAL);
  } else {
    return WKdm_SCRATCH_BUF_SIZE_INTERNAL;
  }
...
int vm_compressor_algorithm(void) { return vm_compressor_current_codec; }
...
void vm_compressor_algorithm_init(void) {
  ...
  assertf(((new_codec == VM_COMPRESSOR_DEFAULT_CODEC) ||
           (new_codec == CMODE_WK) || (new_codec == CMODE_LZ4) ||
           (new_codec == CMODE_HYB)),
