...
#define CHECKSUM_THE_DATA                                                      \
  ENABLE_COMPRESSOR_CHECKS /* Debug compressor/decompressor data */
#define CHECKSUM_THE_COMPRESSED_DATA                                           \
  ENABLE_COMPRESSOR_CHECKS /* Debug compressor/decompressor compressed data */
...
static_assert((C_SEG_OFFSET_BITS + C_SLOT_C_SIZE_BITS + C_SLOT_C_CODEC_BITS +
               C_SLOT_C_POPCOUNT_BITS + C_SLOT_C_PADDING_BITS +
...
struct c_slot {
  ...
  uint64_t c_packed_ptr : C_SLOT_PACKED_PTR_BITS
      __kernel_ptr_semantics; /* points back to the c_slot_mapping_t in the
                                 pager */
...
#define C_IS_EMPTY                                                             \
  0 /* segment was just allocated and is going to start filling */
...
#define C_ON_SWAPPEDOUTSPARSE_Q                                                \
  6 /* segment is swapped-out but some of its slots were freed */
...
struct c_segment {
  ...
  queue_chain_t
      c_list; /* chain of c_minor_list_head, if c_on_minorcompact_q==1 */
...
#define C_SEG_MAX_LIMIT                                                        \
  (UINT_MAX)          /* this needs to track the size of c_mysegno */
  uint32_t c_mysegno; /* my index in c_segments */

  uint32_t c_creation_ts;   /* time filling the segment has finished, used for
                               checking if segment reached ripe age */
  ...
  uint16_t c_firstemptyslot; /* index of lowest empty slot. used for instance in
                                minor compaction to not have to start from 0 */
  uint16_t c_nextslot;       /* index of the next available slot in either
                                c_slot_fixed_array or c_slot_var_array */
  uint32_t c_nextoffset; /* next available position in the buffer space pointed
                            by c_store.c_buffer */
  uint32_t c_populated_offset; /* how much of the segment is populated from it's
                                  beginning */
  ...
  uint32_t c_agedin_ts; /* time the seg got to age_q after being swapped in.
                           used for stats*/
   ...
   * result in a mess.
   */
  uint32_t c_busy : 1, c_busy_swapping : 1, c_wanted : 1,
      c_on_minorcompact_q
      : 1, /* can also be on the age_q, the majorcompact_q or the swappedin_q */

      c_state : 4, /* what state is the segment in which dictates which q to
                      find it on */
      c_overage_swap : 1, c_has_donated_pages : 1,
#if CONFIG_FREEZE
      c_has_freezer_pages : 1, c_reserved : 21;
#else  /* CONFIG_FREEZE */
      c_reserved : 22;
#endif /* CONFIG_FREEZE */
...
/*
 * the pager holds a buffer of this 32 bit sized object, one for each page in
 * the vm_object, to refer to a specific slot in a specific segment in the
 * compressor
 */
struct c_slot_mapping {
  ...
  uint32_t s_cseg : 22, /* segment number + 1 */
      s_cindx
      : 10; /* index of slot in the segment, see also C_SLOT_MAX_INDEX */
  /* in the case of a single-value (sv) page, s_cseg==C_SV_CSEG_ID and s_cindx
   * is the index into c_segment_sv_hash_table
   */
#else  /* !CONFIG_TRACK_UNMODIFIED_ANON_PAGES */
  uint32_t s_cseg : 21, /* segment number + 1 */
      s_cindx : 10,     /* index in the segment */
      s_uncompressed
      : 1; /* This bit indicates that the page resides uncompressed in a
            * swapfile. This can happen in 2 ways:- 1) Page used to be in the
            * compressor, got decompressed, was not modified, and so was pushed
            * uncompressed to a different swapfile on disk. 2) Page was in its
            * uncompressed form in a swapfile on disk. It got swapped in but was
            * not modified. As we are about to reclaim it, we notice that this
            * bit is set in its current slot. And so we can safely toss this
            * clean anonymous page because its copy exists on disk.
            */
...
#define C_SEG_BUFFER_ADDRESS(c_segno)                                          \
  ((c_buffers + ((uint64_t)c_segno * (uint64_t)c_seg_allocsize)))

#define C_SEG_SLOT_FROM_INDEX(cseg, index)                                     \
  (index < c_seg_fixed_array_len                                               \
       ? &(cseg->c_slot_fixed_array[index])                                    \
       : &(cseg->c_slot_var_array[index - c_seg_fixed_array_len]))

#define C_SEG_OFFSET_TO_BYTES(off) ((off) * (int)sizeof(int32_t))
#define C_SEG_BYTES_TO_OFFSET(bytes) ((bytes) / (int)sizeof(int32_t))

#define C_SEG_UNUSED_BYTES(cseg)                                               \
  (cseg->c_bytes_unused +                                                      \
   (C_SEG_OFFSET_TO_BYTES(cseg->c_populated_offset - cseg->c_nextoffset)))
...
/* round an offset/size up to the next multiple the wkdm write alignment (64
 * byte) */
#define C_SEG_ROUND_TO_ALIGNMENT(offset)                                       \
  (((offset) + C_SEG_OFFSET_ALIGNMENT_MASK) & ~C_SEG_OFFSET_ALIGNMENT_MASK)

#define C_SEG_SHOULD_MINORCOMPACT_NOW(cseg)                                    \
  ((C_SEG_UNUSED_BYTES(cseg) >= (c_seg_bufsize / 4)) ? 1 : 0)

/*
 * the decsion to force a c_seg to be major compacted is based on 2 criteria
 * 1) is the c_seg buffer almost empty (i.e. we have a chance to merge it with
 * another c_seg) 2) are there at least a minimum number of slots unoccupied so
 * that we have a chance of combining this c_seg with another one.
 */
#define C_SEG_SHOULD_MAJORCOMPACT_NOW(cseg)                                    \
  ((((cseg->c_bytes_unused +                                                   \
      (c_seg_bufsize - C_SEG_OFFSET_TO_BYTES(c_seg->c_nextoffset))) >=         \
     (c_seg_bufsize / 8)) &&                                                   \
    ((C_SLOT_MAX_INDEX - cseg->c_slots_used) > (c_seg_bufsize / PAGE_SIZE)))   \
       ? 1                                                                     \
       : 0)

#define C_SEG_ONDISK_IS_SPARSE(cseg)                                           \
  ((cseg->c_bytes_used < cseg->c_bytes_unused) ? 1 : 0)
#define C_SEG_IS_ONDISK(cseg)                                                  \
  ((cseg->c_state == C_ON_SWAPPEDOUT_Q ||                                      \
    cseg->c_state == C_ON_SWAPPEDOUTSPARSE_Q))
#define C_SEG_IS_ON_DISK_OR_SOQ(cseg)                                          \
  ((cseg->c_state == C_ON_SWAPPEDOUT_Q ||                                      \
    cseg->c_state == C_ON_SWAPPEDOUTSPARSE_Q ||                                \
    cseg->c_state == C_ON_SWAPOUT_Q || cseg->c_state == C_ON_SWAPIO_Q))
...
#define C_SEG_MAKE_WRITEABLE(cseg)                                             \
  ...
  if (write_protect_c_segs) {                                                  \
    vm_map_protect(                                                            \
        compressor_map, (vm_map_offset_t)cseg->c_store.c_buffer,               \
        (vm_map_offset_t) &                                                    \
            cseg->c_store.c_buffer[C_SEG_BYTES_TO_OFFSET(c_seg_allocsize)],    \
...
extern void kdp_compressor_busy_find_owner(event64_t wait_event,
                                           thread_waitinfo_t *waitinfo);
extern kern_return_t vm_compressor_kdp_init(void);
extern void vm_compressor_kdp_teardown(void);

/*
 * TODO, there may be a minor optimisation opportunity to replace these
 * divisions with multiplies and shifts
 *
 * By multiplying by 10, the divisors can have more precision w/o resorting to
 * floating point... a divisor specified as 25 is in reality a divide by 2.5 By
 * multiplying by 9, you get a number ~11% smaller which allows us to have
 * another limit point derived from the same base By multiplying by 11, you get
 * a number ~10% bigger which allows us to generate a reset limit derived from
 * the same base which is useful for hysteresis
 */

#define VM_PAGE_COMPRESSOR_COMPACT_THRESHOLD                                   \
  (((AVAILABLE_MEMORY) * 10) /                                                 \
   (vm_compressor_minorcompact_threshold_divisor                               \
        ? vm_compressor_minorcompact_threshold_divisor                         \
        : 10))
#define VM_PAGE_COMPRESSOR_SWAP_THRESHOLD                                      \
  (((AVAILABLE_MEMORY) * 10) /                                                 \
...
#define VM_PAGE_COMPRESSOR_SWAP_UNTHROTTLE_THRESHOLD                           \
  (((AVAILABLE_MEMORY) * 10) /                                                 \
   (vm_compressor_unthrottle_threshold_divisor                                 \
        ? vm_compressor_unthrottle_threshold_divisor                           \
        : 10))
#define VM_PAGE_COMPRESSOR_SWAP_RETHROTTLE_THRESHOLD                           \
  (((AVAILABLE_MEMORY) * 11) /                                                 \
   (vm_compressor_unthrottle_threshold_divisor                                 \
        ? vm_compressor_unthrottle_threshold_divisor                           \
        : 11))
...
#define VM_PAGE_COMPRESSOR_SWAP_HAS_CAUGHTUP_THRESHOLD                         \
  (((AVAILABLE_MEMORY) * 11) / (vm_compressor_catchup_threshold_divisor        \
                                    ...
                                    : 11))
#define VM_PAGE_COMPRESSOR_SWAP_CATCHUP_THRESHOLD                              \
  (((AVAILABLE_MEMORY) * 10) / (vm_compressor_catchup_threshold_divisor        \
                                    ? vm_compressor_catchup_threshold_divisor  \
                                    : 10))
#define VM_PAGE_COMPRESSOR_HARD_THROTTLE_THRESHOLD                             \
  (((AVAILABLE_MEMORY) * 9) / (vm_compressor_catchup_threshold_divisor         \
                                   ? vm_compressor_catchup_threshold_divisor   \
                                   : 9))
...
#define COMPRESSOR_NEEDS_TO_SWAP()                                             \
  ((AVAILABLE_NON_COMPRESSED_MEMORY < VM_PAGE_COMPRESSOR_SWAP_THRESHOLD) ? 1   \
                                                                         : 0)
#endif /* !XNU_TARGET_OS_OSX */

#define HARD_THROTTLE_LIMIT_REACHED()                                          \
  ((AVAILABLE_NON_COMPRESSED_MEMORY <                                          \
    VM_PAGE_COMPRESSOR_HARD_THROTTLE_THRESHOLD)                                \
       ? 1                                                                     \
       : 0)
#define SWAPPER_NEEDS_TO_UNTHROTTLE()                                          \
  ((AVAILABLE_NON_COMPRESSED_MEMORY <                                          \
    VM_PAGE_COMPRESSOR_SWAP_UNTHROTTLE_THRESHOLD)                              \
       ? 1                                                                     \
       : 0)
#define SWAPPER_NEEDS_TO_RETHROTTLE()                                          \
  ((AVAILABLE_NON_COMPRESSED_MEMORY >                                          \
    VM_PAGE_COMPRESSOR_SWAP_RETHROTTLE_THRESHOLD)                              \
       ? 1                                                                     \
       : 0)
#define SWAPPER_NEEDS_TO_CATCHUP()                                             \
  ((AVAILABLE_NON_COMPRESSED_MEMORY <                                          \
    VM_PAGE_COMPRESSOR_SWAP_CATCHUP_THRESHOLD)                                 \
       ? 1                                                                     \
       : 0)
#define SWAPPER_HAS_CAUGHTUP()                                                 \
  ((AVAILABLE_NON_COMPRESSED_MEMORY >                                          \
    VM_PAGE_COMPRESSOR_SWAP_HAS_CAUGHTUP_THRESHOLD)                            \
       ? 1                                                                     \
       : 0)
