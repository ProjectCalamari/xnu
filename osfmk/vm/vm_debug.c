...
kern_return_t
vm32_mach_vm_region_info(__DEBUG_ONLY vm_map_t map,
  ...
  for (;;) {
    ...
    for (cobject = object;; cobject = nobject) {
      ...
      if (used < room) {
        vm_info_object_t *vio = &((vm_info_object_t *)addr)[used];

        vio->vio_object = (natural_t)(uintptr_t)cobject;
        vio->vio_size = (natural_t)cobject->vo_size;
        vio->vio_ref_count = cobject->ref_count;
        vio->vio_resident_page_count = cobject->resident_page_count;
        vio->vio_copy = (natural_t)(uintptr_t)cobject->vo_copy;
        vio->vio_shadow = (natural_t)(uintptr_t)cobject->shadow;
        vio->vio_shadow_offset = (natural_t)cobject->vo_shadow_offset;
        vio->vio_paging_offset = (natural_t)cobject->paging_offset;
        vio->vio_copy_strategy = cobject->copy_strategy;
        vio->vio_last_alloc = (vm_offset_t)cobject->last_alloc;
        vio->vio_paging_in_progress =
            cobject->paging_in_progress + cobject->activity_in_progress;
        vio->vio_pager_created = cobject->pager_created;
        vio->vio_pager_initialized = cobject->pager_initialized;
        vio->vio_pager_ready = cobject->pager_ready;
        vio->vio_can_persist = cobject->can_persist;
        vio->vio_internal = cobject->internal;
        vio->vio_temporary = FALSE;
        vio->vio_alive = cobject->alive;
        vio->vio_purgable = (cobject->purgable != VM_PURGABLE_DENY);
  ...
  if (used == 0) {
    ...
    if (size != vmsize_used) {
      kmem_free(ipc_kernel_map, addr + vmsize_used, size - vmsize_used);
    }
  }
...
kern_return_t
vm32_mach_vm_region_info_64(__DEBUG_ONLY vm_map_t map,
                            __DEBUG_ONLY vm32_offset_ut address_u,
 ...
 */
kern_return_t
vm32_vm_mapped_pages_info(__DEBUG_ONLY vm_map_t map,
  ...
  size = vm_map_round_page(size, VM_MAP_PAGE_MASK(ipc_kernel_map));

  for (;;) {
    (void)mach_vm_allocate_kernel(
        ipc_kernel_map, &addr, size,
        VM_MAP_KERNEL_FLAGS_ANYWHERE(.vm_tag = VM_KERN_MEMORY_IPC));
    (void)vm_map_unwire(
        ipc_kernel_map,
        vm_map_trunc_page(addr, VM_MAP_PAGE_MASK(ipc_kernel_map)),
        vm_map_round_page(addr + size, VM_MAP_PAGE_MASK(ipc_kernel_map)),
        FALSE);

    list = (page_address_array_t)addr;
    space = (unsigned int)(size / sizeof(vm_offset_t));

    actual = pmap_list_resident_pages(pmap, list, space);
    if (actual <= space) {
      break;
    }
  ...
  if (actual == 0) {
    ...
    *pages_count = actual;
    ...
    (void)vm_map_copyin(ipc_kernel_map, (vm_map_address_t)addr,
                        (vm_map_size_t)size_used, TRUE, (vm_map_copy_t *)pages);
    if (vmsize_used != size) {
      (void)kmem_free(ipc_kernel_map, addr + vmsize_used, size - vmsize_used);
    }
  }
...
kern_return_t
host_virtual_physical_table_info(__DEBUG_ONLY host_t host,
  ...
  for (;;) {
    ...
    size = vm_map_round_page(actual * sizeof *info,
                             VM_MAP_PAGE_MASK(ipc_kernel_map));
    kr = kmem_alloc(ipc_kernel_map, &addr, size, KMA_PAGEABLE | KMA_DATA_SHARED,
                    VM_KERN_MEMORY_IPC);
    if (kr != KERN_SUCCESS) {
      return KERN_RESOURCE_SHORTAGE;
    }
