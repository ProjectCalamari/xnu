...
TUNABLE_DEV_WRITEABLE(uint64_t, vm_reclaim_sampling_period_ns,
                      "vm_reclaim_sampling_period_ns",
...
TUNABLE_DT_DEV_WRITEABLE(bool, vm_reclaim_enabled, "/defaults",
                         "kern.vm_reclaim_enabled", "vm_reclaim_enabled",
                         VM_RECLAIM_ENABLED_DEFAULT, TUNABLE_DT_NONE);
TUNABLE_DEV_WRITEABLE(uint32_t, vm_reclaim_autotrim_pct_normal,
                      "vm_reclaim_autotrim_pct_normal", 10);
TUNABLE_DEV_WRITEABLE(uint32_t, vm_reclaim_autotrim_pct_pressure,
                      "vm_reclaim_autotrim_pct_pressure", 5);
TUNABLE_DEV_WRITEABLE(uint32_t, vm_reclaim_autotrim_pct_critical,
                      "vm_reclaim_autotrim_pct_critical", 1);
TUNABLE_DEV_WRITEABLE(uint64_t, vm_reclaim_wma_weight_base,
                      "vm_reclaim_wma_weight_base", 3);
TUNABLE_DEV_WRITEABLE(uint64_t, vm_reclaim_wma_weight_cur,
                      "vm_reclaim_wma_weight_cur", 1);
TUNABLE_DEV_WRITEABLE(uint64_t, vm_reclaim_wma_denom, "vm_reclaim_wma_denom",
                      4);
TUNABLE_DEV_WRITEABLE(uint64_t, vm_reclaim_abandonment_threshold,
                      "vm_reclaim_abandonment_threshold", 512);
#else  /* CONFIG_WORKING_SET_ESTIMATION */
TUNABLE_DT_DEV_WRITEABLE(uint64_t, vm_reclaim_max_threshold, "/defaults",
                         "kern.vm_reclaim_max_threshold",
                         "vm_reclaim_max_threshold", 0, TUNABLE_DT_NONE);
...
#define _vmdr_log_type(type, fmt, ...)                                         \
  os_log_with_type(vm_reclaim_log_handle, type, "vm_reclaim: " fmt,            \
                   ##__VA_ARGS__)
#define vmdr_log(fmt, ...)                                                     \
  _vmdr_log_type(OS_LOG_TYPE_DEFAULT, fmt, ##__VA_ARGS__)
#define vmdr_log_info(fmt, ...)                                                \
  _vmdr_log_type(OS_LOG_TYPE_INFO, fmt, ##__VA_ARGS__)
#define vmdr_log_error(fmt, ...)                                               \
  _vmdr_log_type(OS_LOG_TYPE_ERROR, fmt, ##__VA_ARGS__)
#if DEVELOPMENT || DEBUG
...
reclaim_copyin_head(vm_deferred_reclamation_metadata_t metadata,
                    uint64_t *head);
static kern_return_t
reclaim_copyin_tail(vm_deferred_reclamation_metadata_t metadata,
                    uint64_t *tail);
static kern_return_t
reclaim_copyin_busy(vm_deferred_reclamation_metadata_t metadata,
                    uint64_t *busy);
static kern_return_t
reclaim_handle_copyio_error(vm_deferred_reclamation_metadata_t metadata,
                            int result);
#if CONFIG_WORKING_SET_ESTIMATION
static mach_error_t
...
static kern_return_t reclaim_chunk(vm_deferred_reclamation_metadata_t metadata,
                                   uint64_t bytes_to_reclaim,
                                   uint64_t *bytes_reclaimed_out,
                                   mach_vm_reclaim_count_t chunk_size,
                                   mach_vm_reclaim_count_t *num_reclaimed_out);
...
struct vm_deferred_reclamation_metadata_s {
   ...
   * Since this is a backpointer we don't hold a reference (it's a weak
   * pointer).
   */
  task_t vdrm_task;
  pid_t vdrm_pid;
   ...
   * will set vdrm_task := NULL and drop its ref. Threads operating on the
   * buffer should hold a +1 on the metadata structure to ensure it's validity.
   */
   ...
   */
  uint8_t vdrm_is_registered : 1, __unused1 : 7;
};

#pragma mark Globals
static KALLOC_TYPE_DEFINE(vm_reclaim_metadata_zone,
                          struct vm_deferred_reclamation_metadata_s,
                          KT_DEFAULT);
static LCK_GRP_DECLARE(vm_reclaim_lock_grp, "vm_reclaim");
os_refgrp_decl(static, vm_reclaim_metadata_refgrp, "vm_reclaim_metadata_refgrp",
               NULL);
/*
 ...
 * It should be dropped when acting on a specific metadata entry after acquiring
 * the vdrm_lock.
 */
static TAILQ_HEAD(, vm_deferred_reclamation_metadata_s)
    reclaim_buffers = TAILQ_HEAD_INITIALIZER(reclaim_buffers);
...
static SECURITY_READ_ONLY_LATE(thread_t)
    vm_reclaim_scavenger_thread = THREAD_NULL;
...
vmdr_metadata_alloc(task_t task, user_addr_t buffer, mach_vm_size_t size,
...
static void vmdr_metadata_free(vm_deferred_reclamation_metadata_t metadata) {
...
static mach_vm_size_t vmdr_round_len_to_size(vm_map_t map,
                                             mach_vm_reclaim_count_t count) {
  mach_vm_size_t metadata_size =
...
mach_error_t vm_deferred_reclamation_buffer_allocate_internal(
    task_t task, mach_vm_address_ut *address_u, uint64_t *sampling_period,
    mach_vm_reclaim_count_t len, mach_vm_reclaim_count_t max_len) {
  kern_return_t kr;
  ...
  if (address_u == NULL || sampling_period == NULL || len == 0 ||
      max_len == 0 || max_len < len) {
  ...
  if (rounded_vm_size > VM_RECLAIM_MAX_BUFFER_SIZE) {
    vmdr_log_error("denying request to allocate ringbuffer of size "
                   "%llu KiB (max %llu KiB)\n",
                   rounded_vm_size, VM_RECLAIM_MAX_BUFFER_SIZE);
    return KERN_NO_SPACE;
  }

  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_INIT) | DBG_FUNC_START, task_pid(task), len);
  ...
  vm_map_kernel_flags_t vmk_flags =
      VM_MAP_KERNEL_FLAGS_ANYWHERE_PERMANENT(.vm_tag = VM_MEMORY_MALLOC);
  ...
  kr = mach_vm_map_kernel(map, address_u, size_u, VM_MAP_PAGE_MASK(map),
                          vmk_flags, IPC_PORT_NULL, 0, FALSE, VM_PROT_DEFAULT,
                          VM_PROT_DEFAULT, VM_INHERIT_COPY);
  if (kr != KERN_SUCCESS) {
  ...
  if (!task_is_active(task) || task_is_halting(task)) {
    vmdr_log_error("failed to initialize buffer on dying task %s [%d]",
                   task_best_name(task), task_pid(task));
    kr = KERN_ABORTED;
    goto fail_task;
  }
  if (task->deferred_reclamation_metadata != NULL) {
    vmdr_log_error("tried to overwrite existing reclaim buffer for %s [%d]",
                   task_best_name(task), task_pid(task));
  ...
  vmdr_log_debug("%s [%d] allocated ring with capacity %u/%u\n",
                 task_best_name(task), task_pid(task), len, max_len);
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_INIT) | DBG_FUNC_END, task_pid(task),
       KERN_SUCCESS, address);
  DTRACE_VM3(reclaim_ring_allocate, mach_vm_address_t, address,
             mach_vm_reclaim_count_t, len, mach_vm_reclaim_count_t, max_len);
  return KERN_SUCCESS;
...
fail_task:
  ...
  tmp_kr = mach_vm_deallocate(map, *address_u, size_u);
...
out:
  ...
  *sampling_period = vm_reclaim_sampling_period_abs;
  ...
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_INIT) | DBG_FUNC_END, kr, NULL);
  return kr;
}
...
vmdr_metadata_try_own_locked(vm_deferred_reclamation_metadata_t metadata) {
  kern_return_t kr =
      lck_mtx_gate_try_close(&metadata->vdrm_lock, &metadata->vdrm_gate);
...
vmdr_metadata_own_locked(vm_deferred_reclamation_metadata_t metadata,
  ...
  if (!vmdr_metadata_try_own_locked(metadata)) {
    ...
    wait_result = lck_mtx_gate_wait(&metadata->vdrm_lock, &metadata->vdrm_gate,
                                    LCK_SLEEP_DEFAULT, THREAD_UNINT,
...
static void vmdr_metadata_retain(vm_deferred_reclamation_metadata_t metadata) {
...
void vm_deferred_reclamation_buffer_deallocate(
  ...
  /*
   * First remove the buffer from the global list so no one else can get access
   * to it.
   */
  lck_mtx_lock(&reclaim_buffers_lock);
...
static void
reclaim_kill_with_reason(vm_deferred_reclamation_metadata_t metadata,
                         unsigned reason, mach_exception_data_type_t subcode) {
  ...
  if (task == TASK_NULL || !task_is_active(task) || task_is_halting(task)) {
    ...
    vmdr_metadata_unlock(metadata);
    vmdr_log_error("Unable to deliver guard exception because task "
                   "[%d] is already dead.\n",
                   metadata->vdrm_pid);
    return;
  }
  ...
  if (!fatal) {
    vmdr_log_info("Skipping non fatal guard exception for %s [%d]\n",
                  task_best_name(task), task_pid(task));
    goto out;
  }
  ...
  if (killing_self) {
    ...
    if (p && proc_task(p) != task) {
      vmdr_log_error("Unable to deliver guard exception because proc is gone & "
                     "pid rolled over.\n");
      goto out;
    }
  }
  ...
  if (!p) {
    vmdr_log_error("Unable to deliver guard exception because task does not "
                   "have a proc.\n");
    goto out;
  }
...
static user_addr_t get_head_ptr(vm_deferred_reclamation_metadata_t metadata) {
  return metadata->vdrm_ring_addr +
         offsetof(struct mach_vm_reclaim_ring_s, head);
}

static user_addr_t get_tail_ptr(vm_deferred_reclamation_metadata_t metadata) {
  return metadata->vdrm_ring_addr +
         offsetof(struct mach_vm_reclaim_ring_s, tail);
}

static user_addr_t get_busy_ptr(vm_deferred_reclamation_metadata_t metadata) {
  return metadata->vdrm_ring_addr +
         offsetof(struct mach_vm_reclaim_ring_s, busy);
}
 ...
 * Userspace is not supposed to write to the head or busy pointers, but the
 * kernel must be resilient to that kind of bug in userspace.
 */
...
reclaim_copyin_head(vm_deferred_reclamation_metadata_t metadata,
  ...
  if (kr != KERN_SUCCESS && kr != KERN_MEMORY_ERROR) {
    vmdr_log_error("Unable to copy head ptr from 0x%llx: err=%d\n", head_ptr,
                   result);
  }
  return kr;
}
...
reclaim_copyin_tail(vm_deferred_reclamation_metadata_t metadata,
  ...
  kr = reclaim_handle_copyio_error(metadata, result);
  if (kr != KERN_SUCCESS && kr != KERN_MEMORY_ERROR) {
    vmdr_log_error("Unable to copy tail ptr from 0x%llx: err=%d\n", tail_ptr,
                   result);
  }
  return kr;
}
...
reclaim_copyin_busy(vm_deferred_reclamation_metadata_t metadata,
  ...
  kr = reclaim_handle_copyio_error(metadata, result);
  if (kr != KERN_SUCCESS && kr != KERN_MEMORY_ERROR) {
    vmdr_log_error("Unable to copy busy ptr from 0x%llx: err=%d\n", busy_ptr,
                   result);
  }
  return kr;
}
...
static kern_return_t
reclaim_copyin_reclaimable_bytes(vm_deferred_reclamation_metadata_t metadata,
                                 size_t *reclaimable_bytes_out) {
  int result;
...
#if CONFIG_WORKING_SET_ESTIMATION
static kern_return_t reclaim_copyin_min_reclaimable_bytes(
    vm_deferred_reclamation_metadata_t metadata,
    size_t *min_reclaimable_bytes_out) {
  int result;
  kern_return_t kr = KERN_SUCCESS;
  uint64_t min_reclaimable_bytes;
  user_addr_t ptr =
      (uintptr_t)metadata->vdrm_ring_addr +
...
static bool reclaim_copyout_busy(vm_deferred_reclamation_metadata_t metadata,
                                 uint64_t value) {
  int result;
  kern_return_t kr = KERN_SUCCESS;
  user_addr_t busy_ptr = get_busy_ptr(metadata);
  ...
  if (result) {
    kr = reclaim_handle_copyio_error(metadata, result);
    if (result != EFAULT || !vm_fault_get_disabled()) {
      vmdr_log_error("Unable to copy %llu to busy ptr at 0x%llx: err=%d\n",
...
static bool reclaim_copyout_head(vm_deferred_reclamation_metadata_t metadata,
                                 uint64_t value) {
  int result;
  kern_return_t kr = KERN_SUCCESS;
  user_addr_t head_ptr = get_head_ptr(metadata);
  ...
  if (result) {
    kr = reclaim_handle_copyio_error(metadata, result);
    if (result != EFAULT || !vm_fault_get_disabled()) {
      vmdr_log_error("Unable to copy %llu to head ptr at 0x%llx: err=%d\n",
...
#if CONFIG_WORKING_SET_ESTIMATION
static kern_return_t reclaim_copyout_min_reclaimable_bytes(
    vm_deferred_reclamation_metadata_t metadata, size_t min_reclaimable_bytes) {
  int result;
  kern_return_t kr = KERN_SUCCESS;
  user_addr_t ptr =
      (uintptr_t)metadata->vdrm_ring_addr +
...
static kern_return_t reclaim_chunk(vm_deferred_reclamation_metadata_t metadata,
  ...
  if (busy < head || (busy - head) > kReclaimChunkSize) {
    vmdr_log_error("Userspace modified head or busy pointer! head: %llu "
                   "(0x%llx) | busy: %llu (0x%llx) | tail = %llu (0x%llx)\n",
                   head, get_head_ptr(metadata), busy, get_busy_ptr(metadata),
                   tail, get_tail_ptr(metadata));
  ...
  if (tail < head) {
     ...
     */
    vmdr_log_error("Tail < head! Userspace is likely attempting a "
                   "cancellation; aborting reclamation | head: %llu "
                   "(0x%llx) > tail: %llu (0x%llx) | busy = %llu (0x%llx)\n",
                   head, get_head_ptr(metadata), tail, get_tail_ptr(metadata),
                   busy, get_busy_ptr(metadata));
    kr = KERN_ABORTED;
    goto done;
  }
  ...
  vmdr_log_debug("[%d] reclaiming up to %llu entries (%llu KiB) head=%llu "
                 "busy=%llu tail=%llu len=%u",
                 metadata->vdrm_pid, num_to_reclaim, bytes_reclaimed, head,
                 busy, tail, metadata->vdrm_buffer_len);
  ...
  while (num_copied < num_to_reclaim) {
    ...
    // Clamp the end idx to the buffer. We'll handle wrap-around in our next go
    // around the loop.
    ...
    if (kr != KERN_SUCCESS) {
      if (kr != KERN_MEMORY_ERROR || !vm_fault_get_disabled()) {
        vmdr_log_error("Unable to copyin %llu entries in reclaim "
    ...
    memcpy_start_idx =
        (memcpy_start_idx + num_to_copy) % metadata->vdrm_buffer_len;
  }

  for (num_reclaimed = 0;
       num_reclaimed < num_to_reclaim && bytes_reclaimed < bytes_to_reclaim;
       num_reclaimed++) {
    ...
    if (entry->address != 0 && entry->size != 0) {
      ...
      DTRACE_VM4(vm_reclaim_entry, pid_t, metadata->vdrm_pid, mach_vm_address_t,
                 entry->address, mach_vm_address_t, end,
                 mach_vm_reclaim_action_t, entry->behavior);
      KDBG_FILTERED(VM_RECLAIM_CODE(VM_RECLAIM_ENTRY) | DBG_FUNC_START,
                    metadata->vdrm_pid, start, end, entry->behavior);
      ...
      case VM_RECLAIM_DEALLOCATE:
        kr = vm_map_remove_guard(map, start, end, VM_MAP_REMOVE_GAPS_FAIL,
                                 KMEM_GUARD_NONE)
                 .kmr_return;
        if (kr == KERN_INVALID_VALUE) {
          ...
          reclaim_kill_with_reason(metadata, kGUARD_EXC_DEALLOC_GAP,
                                   entry->address);
          goto done;
        } else if (kr != KERN_SUCCESS) {
          vmdr_log_error("[%d] Killing due to deallocation failure at (0x%llx, "
                         "0x%llx) err=%d\n",
                         metadata->vdrm_pid, start, end, kr);
          reclaim_kill_with_reason(metadata,
                                   kGUARD_EXC_RECLAIM_DEALLOCATE_FAILURE, kr);
          goto done;
        }
        break;
      case VM_RECLAIM_FREE:
         ...
         * age in the LRU.
         */
        kr = vm_map_behavior_set(map, start, end, VM_BEHAVIOR_REUSABLE);
        if (kr != KERN_SUCCESS) {
          vmdr_log_error(
      ...
      KDBG_FILTERED(VM_RECLAIM_CODE(VM_RECLAIM_ENTRY) | DBG_FUNC_END, kr);
    }
  }
  ...
  vmdr_log_debug("[%d] reclaimed %u entries (%llu KiB) head=%llu "
                 "busy=%llu tail=%llu len=%u",
                 metadata->vdrm_pid, num_reclaimed, bytes_reclaimed, head, busy,
                 tail, metadata->vdrm_buffer_len);
...
vmdr_reclaim_from_buffer(vm_deferred_reclamation_metadata_t metadata,
                         mach_vm_size_t bytes_to_reclaim,
                         mach_vm_size_t *num_bytes_reclaimed_out,
                         vm_deferred_reclamation_options_t options) {
  kern_return_t kr = KERN_SUCCESS;
  ...
  while (total_bytes_reclaimed < bytes_to_reclaim) {
    ...
    kr = reclaim_chunk(metadata, bytes_to_reclaim - total_bytes_reclaimed,
                       &cur_bytes_reclaimed, kReclaimChunkSize,
                       &entries_reclaimed);
  ...
  vmdr_log_debug("reclaimed %llu B / %llu B from %d\n", total_bytes_reclaimed,
                 bytes_to_reclaim, metadata->vdrm_pid);
...
kern_return_t vm_deferred_reclamation_buffer_flush_internal(
    task_t task, mach_vm_reclaim_count_t num_entries_to_reclaim,
  ...
  vmdr_log_debug("[%d] flushing %u entries\n", task_pid(task),
                 num_entries_to_reclaim);
  KDBG_FILTERED(VM_RECLAIM_CODE(VM_RECLAIM_FLUSH) | DBG_FUNC_START,
                metadata->vdrm_pid, num_entries_to_reclaim);

  while (total_reclaimed < num_entries_to_reclaim) {
    mach_vm_reclaim_count_t cur_reclaimed;
    uint64_t cur_bytes_reclaimed;
    mach_vm_reclaim_count_t chunk_size =
        MIN(num_entries_to_reclaim - total_reclaimed, kReclaimChunkSize);
  ...
  *bytes_reclaimed_out = bytes_reclaimed;
  KDBG_FILTERED(VM_RECLAIM_CODE(VM_RECLAIM_FLUSH) | DBG_FUNC_END, kr,
                total_reclaimed, bytes_reclaimed);
  DTRACE_VM2(reclaim_flush, mach_vm_reclaim_count_t, num_entries_to_reclaim,
             size_t, bytes_reclaimed);
  return kr;
}

kern_return_t vm_deferred_reclamation_buffer_resize_internal(
    task_t task, mach_vm_reclaim_count_t len,
    mach_vm_size_t *bytes_reclaimed_out) {
  kern_return_t kr;
  ...
  vm_deferred_reclamation_metadata_t metadata =
      vmdr_acquire_task_metadata(task);
  if (metadata == NULL) {
    return KERN_INVALID_TASK;
  }
  ...
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_RESIZE) | DBG_FUNC_START, task_pid(task),
       new_size);
  ...
  do {
    mach_vm_size_t cur_bytes_reclaimed;
    kr = reclaim_chunk(metadata, UINT64_MAX, &cur_bytes_reclaimed,
                       kReclaimChunkSize, &num_entries_reclaimed);
    total_bytes_reclaimed += cur_bytes_reclaimed;
    if (kr != KERN_SUCCESS) {
      goto fail;
    }
  ...
  vmdr_log_debug("[%d] successfully resized buffer | reclaimed: %llu B "
                 ...
                 metadata->vdrm_pid, total_bytes_reclaimed,
  ...
  *bytes_reclaimed_out = total_bytes_reclaimed;
  ...
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_RESIZE) | DBG_FUNC_END, KERN_SUCCESS,
       num_entries_reclaimed, total_bytes_reclaimed);
  DTRACE_VM2(reclaim_ring_resize, mach_vm_reclaim_count_t, old_len,
             mach_vm_reclaim_count_t, len);
  return KERN_SUCCESS;
...
fail:
  ...
  vmdr_metadata_release(metadata);
  *bytes_reclaimed_out = total_bytes_reclaimed;
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_RESIZE) | DBG_FUNC_END, kr,
...
static kern_return_t
vmdr_calculate_autotrim_threshold(vm_deferred_reclamation_metadata_t metadata,
  ...
  vm_pressure_level_t pressure_level =
      os_atomic_load(&memorystatus_vm_pressure_level, relaxed);
  ...
  kr = ledger_get_lifetime_max(task->ledger, task_ledgers.phys_footprint,
                               &phys_footprint_max);
...
#define VMDR_WMA_MIX(base, e)                                                  \
  ((vm_reclaim_wma_weight_base * (base) +                                      \
    (e) * VMDR_WMA_UNIT * vm_reclaim_wma_weight_cur) /                         \
   vm_reclaim_wma_denom)
#endif /* CONFIG_WORKING_SET_ESTIMATION */
...
vmdr_sample_working_set(vm_deferred_reclamation_metadata_t metadata,
  ...
  *trim_threshold_out = 0;
  ...
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_SAMPLE) | DBG_FUNC_START, metadata->vdrm_pid,
       now, metadata->vdrm_last_sample_abs, min_reclaimable_bytes);
  ...
  if (cur_reclaimable_bytes >= metadata->vdrm_kernel_bytes_reclaimed) {
    ...
    reclaim_kill_with_reason(metadata, kGUARD_EXC_RECLAIM_ACCOUNTING_FAILURE,
                             cur_reclaimable_bytes);
    err = KERN_ABORTED;
    goto done;
  }
  ...
  uint64_t samples_elapsed =
      (now - metadata->vdrm_last_sample_abs) / vm_reclaim_sampling_period_abs;
  if (samples_elapsed > vm_reclaim_abandonment_threshold) {
    ...
    /*
     * Compute an exponential moving average of the minimum amount of
     * reclaimable memory in this buffer. Multiple sampling periods may have
     * elapsed since the last sample. By definition, the minimum must be the
     * same for all elapsed periods (otherwise libmalloc would have called down
     * to update accounting)
     */
    for (unsigned int i = 0; i < samples_elapsed; i++) {
      wma = VMDR_WMA_MIX(metadata->vdrm_reclaimable_bytes_wma,
                         min_reclaimable_bytes);
    }
  }

  metadata->vdrm_reclaimable_bytes_wma = wma;
  size_t unneeded_bytes =
      MIN(min_reclaimable_bytes,
  ...
  if (unneeded_bytes >= vm_map_page_size(metadata->vdrm_map) &&
    ...
    *trim_threshold_out =
        vm_map_round_page(unneeded_bytes, vm_map_page_mask(metadata->vdrm_map));
  }
#else  /* !CONFIG_WORKING_SET_ESTIMATION */
  ...
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_SAMPLE) | DBG_FUNC_END, wma,
       min_reclaimable_bytes, cur_reclaimable_bytes, *trim_threshold_out);
  DTRACE_VM5(reclaim_sample, pid_t, metadata->vdrm_pid, uint64_t, wma, size_t,
             min_reclaimable_bytes, size_t, cur_reclaimable_bytes, size_t,
             *trim_threshold_out);
  vmdr_log_debug("sampled buffer with min %lu est %lu trim %llu wma %llu\n",
                 min_reclaimable_bytes, cur_reclaimable_bytes,
                 *trim_threshold_out, wma);
  return err;
}
 ...
 */
static kern_return_t vmdr_trim(vm_deferred_reclamation_metadata_t metadata,
                               mach_vm_size_t bytes_to_reclaim,
                               mach_vm_size_t *bytes_reclaimed,
                               vm_deferred_reclamation_options_t options) {
  kern_return_t kr;
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_TRIM) | DBG_FUNC_START, metadata->vdrm_pid,
       bytes_to_reclaim);

  kr = vmdr_reclaim_from_buffer(metadata, bytes_to_reclaim, bytes_reclaimed,
                                options);

  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_TRIM) | DBG_FUNC_END, kr, bytes_reclaimed);
  DTRACE_VM3(reclaim_trim, pid_t, metadata->vdrm_pid, size_t, bytes_to_reclaim,
             size_t, *bytes_reclaimed);
  return kr;
}
 ...
 * Caller must have buffer owned and unlocked
 */
static kern_return_t vmdr_drain(vm_deferred_reclamation_metadata_t metadata,
                                mach_vm_size_t *bytes_reclaimed,
                                vm_deferred_reclamation_options_t options) {
  kern_return_t kr;
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_DRAIN) | DBG_FUNC_START, metadata->vdrm_pid);

  kr = vmdr_reclaim_from_buffer(metadata, UINT64_MAX, bytes_reclaimed, options);

  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_DRAIN) | DBG_FUNC_END, kr, bytes_reclaimed);
  DTRACE_VM2(reclaim_drain, pid_t, metadata->vdrm_pid, size_t,
             *bytes_reclaimed);
  return kr;
}

mach_error_t vm_deferred_reclamation_update_accounting_internal(
    task_t task, uint64_t *bytes_reclaimed_out) {
  vm_deferred_reclamation_metadata_t metadata =
  ...
  err = vmdr_sample_working_set(metadata, &bytes_to_reclaim,
  ...
  if (bytes_to_reclaim) {
    ...
    err = vmdr_trim(metadata, bytes_to_reclaim, &bytes_reclaimed,
                    RECLAIM_OPTIONS_NONE);
  ...
  KDBG(VM_RECLAIM_CODE(VM_RECLAIM_UPDATE_ACCOUNTING) | DBG_FUNC_END,
       metadata->vdrm_last_sample_abs, bytes_to_reclaim, bytes_reclaimed);
...
vm_deferred_reclamation_task_fork(task_t task,
  ...
  metadata =
      vmdr_metadata_alloc(task, parent->vdrm_ring_addr, parent->vdrm_ring_size,
                          parent->vdrm_buffer_len);
...
static void vmdr_garbage_collect(vm_deferred_reclamation_gc_action_t action,
  ...
  if (kr != KERN_SUCCESS) {
    ...
    wr = lck_mtx_gate_wait(&reclaim_buffers_lock, &vm_reclaim_gc_gate,
                           LCK_SLEEP_DEFAULT, THREAD_UNINT,
                           TIMEOUT_WAIT_FOREVER);
  ...
  lck_mtx_gate_handoff(&reclaim_buffers_lock, &vm_reclaim_gc_gate,
                       GATE_HANDOFF_OPEN_IF_NO_WAITERS);
  lck_mtx_unlock(&reclaim_buffers_lock);
...
OS_NORETURN
static void vm_reclaim_scavenger_thread_continue(__unused void *param,
...
OS_NORETURN
static void vm_reclaim_scavenger_thread_init(__unused void *param,
                                             __unused wait_result_t wr) {
...
__startup_func static void vm_deferred_reclamation_init(void) {
  ...
  kern_return_t kr = kernel_thread_start_priority(
      vm_reclaim_scavenger_thread_init, NULL, BASEPRI_KERNEL,
      &vm_reclaim_scavenger_thread);
  if (kr != KERN_SUCCESS) {
...
bool vm_deferred_reclamation_block_until_task_has_been_reclaimed(task_t task) {
  ...
  sched_cond_signal(&vm_reclaim_scavenger_cond, vm_reclaim_scavenger_thread);
  wait_result_t wr =
      lck_mtx_sleep(&metadata->vdrm_lock, LCK_SLEEP_DEFAULT,
                    (event_t)&metadata->vdrm_waiters, THREAD_ABORTSAFE);
  metadata->vdrm_waiters--;
...
kern_return_t vm_deferred_reclamation_buffer_query_internal(
    task_t task, mach_vm_address_ut *addr_out_u, mach_vm_size_ut *size_out_u) {
