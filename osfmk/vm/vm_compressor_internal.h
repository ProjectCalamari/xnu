...
#define PAGE_REPLACEMENT_DISALLOWED(enable)                                    \
  (enable == TRUE ? lck_rw_lock_shared(&c_master_lock)                         \
                  : lck_rw_done(&c_master_lock))
#define PAGE_REPLACEMENT_ALLOWED(enable)                                       \
  (enable == TRUE ? lck_rw_lock_exclusive(&c_master_lock)                      \
                  : lck_rw_done(&c_master_lock))
...
extern uint64_t vm_compressor_compute_elapsed_msecs(clock_sec_t, clock_nsec_t,
                                                    clock_sec_t, clock_nsec_t);
