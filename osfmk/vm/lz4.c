...
size_t lz4raw_decode_buffer(uint8_t *__restrict dst_buffer, size_t dst_size,
  ...
  if (dst_size > LZ4_GOFAST_SAFETY_MARGIN &&
      src_size > LZ4_GOFAST_SAFETY_MARGIN) {
    if (lz4_decode_asm(&dst, dst_buffer,
                       dst_buffer + dst_size - LZ4_GOFAST_SAFETY_MARGIN, &src,
                       src_buffer + src_size - LZ4_GOFAST_SAFETY_MARGIN)) {
      return 0; // FAIL
    }
  }
#endif
  // DRKTODO: Can the 'C' "safety" decode be eliminated for 4/16K fixed-sized
  // buffers?

  // Finish safe
  if (lz4_decode(&dst, dst_buffer, dst_buffer + dst_size, &src,
                 src_buffer + src_size)) {
    return 0; // FAIL
  }
...
// Compile-time selection, return number of matching bytes 0..N at positions A
// and B.
static inline size_t lz4_nmatch(int N, const uint8_t *a, const uint8_t *b) {
  switch (N) {
  case 4:
    return lz4_nmatch4(a, b);
  case 8:
    return lz4_nmatch8(a, b);
  case 16:
    return lz4_nmatch16(a, b);
  case 32:
    return lz4_nmatch32(a, b);
  case 64:
    return lz4_nmatch64(a, b);
  }
  __builtin_trap(); // FAIL
}

// Store LENGTH in DST using the literal_length/match_length extension scheme: X
// is the sum of all bytes until we reach a byte < 0xff. We are allowed to
// access a constant number of bytes above DST_END. Return incremented DST
// pointer on success, and 0 on failure
static inline uint8_t *lz4_store_length(uint8_t *dst, const uint8_t *const end,
                                        uint32_t L) {
  (void)end;
  while (L >= 17 * 255) {
    lz4_fill16(dst);
...
static inline uint8_t *copy_literal(uint8_t *dst, const uint8_t *restrict src,
                                    uint32_t L) {
  uint8_t *end = dst + L;
  {
    copy16(dst, src);
...
static uint8_t *lz4_emit_match(uint32_t L, uint32_t M, uint32_t D,
                               uint8_t *restrict dst, const uint8_t *const end,
...
void lz4_encode_2gb(uint8_t **dst_ptr, size_t dst_size, const uint8_t **src_ptr,
                    const uint8_t *src_begin, size_t src_size,
  ...
  while (dst < end) {
    ...
    for (match_begin = src; match_begin < src_end; match_begin += 1) {
...
#if LZ4_EARLY_ABORT
      // DRKTODO: Evaluate unrolling further. 2xunrolling had some modest
      // benefits
      if (lz4_do_abort_eval && ((pos) >= LZ4_EARLY_ABORT_EVAL)) {
        ...
        /*            if ((cbytes * LZ4_EARLY_ABORT_MIN_COMPRESSION_FACTOR) >
         * pos)  { */
        /*                    return; */
        /*            } */
        lz4_do_abort_eval = 0;
  ...
  EXPAND_FORWARD:
    ...
    // Expand match backward
    {
      ...
      while (match_begin > match_begin_min &&
             ref_begin[-1] == match_begin[-1]) {
        match_begin -= 1;
        ref_begin -= 1;
      }
    }
    ...
    // Emit match
    dst = lz4_emit_match((uint32_t)(match_begin - src),
...
lz4raw_encode_buffer(uint8_t *__restrict dst_buffer, size_t dst_size,
  ...
  while (src_size > 0) {
    ...
    /* #else */
    /*     for (int i=0;i<LZ4_COMPRESS_HASH_ENTRIES;i++) hash_table[i] =
     * HASH_FILL; */
    /* #endif */
    ...
    // Run the encoder, only the last block emits final literals. Allows
    // concatenation of encoded payloads. Blocks are encoded independently, so
    // src_begin is set to each block origin instead of src_buffer
    uint8_t *dst_start = dst;
    const uint8_t *src_start = src;
    lz4_encode_2gb(&dst, dst_size, &src, src, src_to_encode, hash_table,
    ...
    // Note that there is a potential problem here in case of non compressible
    // data requiring more blocks. We may end up here with src_used very small,
    // or even 0, and will not be able to make progress during compression. We
    // FAIL unless the length of literals remaining at the end is small enough.
...
typedef uint32_t lz4_uint128 __attribute__((ext_vector_type(4)))
__attribute__((__aligned__(1)));

int lz4_decode(uint8_t **dst_ptr, uint8_t *dst_begin, uint8_t *dst_end,
...
#pragma clang diagnostic ignored "-Wcast-align"
    ...
    uint64_t matchDistance =
        *(const uint16_t *)src; // 0x0000 <= matchDistance <= 0xffff
