 ...
 * kernel task can be 32 or 64. mach_vm_allocate makes sense everywhere, and is
 ...
 * always be 32-bit and call into the vm32_vm_allocate APIs. On non-U32/K32
 * platforms, the MIG glue should never call into vm_allocate directly, because
 * the calling task and kernel_task are unlikely to use the same size parameters
 *
 ...
 *	map.
 */
kern_return_t mach_vm_allocate_external(vm_map_t map, mach_vm_offset_ut *addr,
                                        mach_vm_size_ut size, int flags) {
 ...
 */
kern_return_t vm_allocate_external(vm_map_t map, vm_offset_ut *addr,
                                   vm_size_ut size, int flags) {
  return mach_vm_allocate_external(map, addr, size, flags);
}

static __attribute__((always_inline, warn_unused_result)) kern_return_t
mach_vm_deallocate_sanitize(vm_map_t map, mach_vm_offset_ut start_u,
                            mach_vm_size_ut size_u, mach_vm_offset_t *start,
                            mach_vm_offset_t *end, mach_vm_size_t *size) {
 ...
 */
kern_return_t mach_vm_deallocate(vm_map_t map, mach_vm_offset_ut start_u,
  ...
  kr = mach_vm_deallocate_sanitize(map, start_u, size_u, &start, &end, &size);
 ...
 *	size as the kernel).
 */
kern_return_t vm_deallocate(vm_map_t map, vm_offset_ut start, vm_size_ut size) {
 ...
 *	specified map.
 */
kern_return_t mach_vm_inherit(vm_map_t map, mach_vm_offset_ut start_u,
                              mach_vm_size_ut size_u,
  ...
  return vm_map_inherit(map, start_u,
 ...
 */
kern_return_t vm_inherit(vm_map_t map, vm_offset_ut start_u, vm_size_ut size_u,
                         vm_inherit_ut new_inheritance_u) {
...
kern_return_t mach_vm_protect(vm_map_t map, mach_vm_address_ut start_u,
                              mach_vm_size_ut size_u, boolean_t set_maximum,
  ...
  return vm_map_protect(map, start_u,
                        vm_sanitize_compute_ut_end(start_u, size_u),
                        set_maximum, new_protection_u);
}
...
kern_return_t vm_protect(vm_map_t map, vm_offset_ut start_u, vm_size_ut size_u,
                         boolean_t set_maximum, vm_prot_ut new_protection_u) {
 ...
 */
kern_return_t
mach_vm_machine_attribute(vm_map_t map, mach_vm_address_ut addr_u,
                          mach_vm_size_ut size_u,
                          ...
                          vm_machine_attribute_val_t *value) /* IN/OUT */
{
  ...
  return vm_map_machine_attribute(map, addr_u,
 ...
 */
kern_return_t
vm_machine_attribute(vm_map_t map, vm_address_ut addr_u, vm_size_ut size_u,
                     vm_machine_attribute_t attribute,
                     vm_machine_attribute_val_t *value) /* IN/OUT */
{
 ...
 *
 */
kern_return_t mach_vm_read(vm_map_t map, mach_vm_address_ut addr,
                           mach_vm_size_ut size, pointer_ut *data,
 ...
 * that gets returned.
 */
kern_return_t vm_read(vm_map_t map, vm_address_ut addr, vm_size_ut size,
                      pointer_ut *data, mach_msg_type_number_t *data_size) {
 ...
 * manually here.
 */
kern_return_t mach_vm_read_list(vm_map_t map, mach_vm_read_entry_t data_list,
                                natural_t count) {
  mach_msg_type_number_t i;
  kern_return_t error;
  vm_map_copy_t copy;

  if (map == VM_MAP_NULL || count > VM_MAP_ENTRY_MAX) {
    return KERN_INVALID_ARGUMENT;
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    if (map_size != 0) {
      error = vm_map_copyin(map, map_addr, map_size, FALSE, /* src_destroy */
                            &copy);
      if (KERN_SUCCESS == error) {
        error = vm_map_copyout(current_task()->map, &map_addr, copy);
        if (KERN_SUCCESS == error) {
...
kern_return_t vm_read_list(vm_map_t map, vm_read_entry_t data_list,
                           natural_t count) {
  mach_msg_type_number_t i;
  kern_return_t error;
  vm_map_copy_t copy;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (map_size != 0) {
      ...
      if (KERN_SUCCESS == error) {
        error = vm_map_copyout(current_task()->map, &map_addr, copy);
        if (KERN_SUCCESS == error) {
          data_list[i].address = CAST_DOWN(vm_offset_t, map_addr);
          continue;
        }
        vm_map_copy_discard(copy);
      }
...
kern_return_t mach_vm_read_overwrite(vm_map_t map, mach_vm_address_ut address,
  ...
  if (KERN_SUCCESS == error) {
    ...
    error =
        vm_map_copy_overwrite(current_thread()->map, data, copy, size, FALSE);
    if (KERN_SUCCESS == error) {
      *data_size = size;
      return error;
    }
...
kern_return_t vm_read_overwrite(vm_map_t map, vm_address_ut address,
                                vm_size_ut size, vm_address_ut data,
...
kern_return_t mach_vm_update_pointers_with_remote_tags(
    __unused vm_map_t map, __unused mach_vm_offset_list_t in_pointer_list,
  ...
  if (!in_pointer_list || !out_pointer_list ||
      in_pointer_listCnt >= 512
      /* The length of the output pointer list must match the input pointer list
       */
      || !out_pointer_listCnt || *out_pointer_listCnt != in_pointer_listCnt) {
    return KERN_INVALID_ARGUMENT;
  }
 ...
 */
kern_return_t mach_vm_write(vm_map_t map, mach_vm_address_ut address,
                            pointer_ut data_u, mach_msg_type_number_t size) {
  if (map == VM_MAP_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
  ...
  return vm_map_copy_overwrite(map, address, data, size,
 ...
 */
kern_return_t vm_write(vm_map_t map, vm_address_ut address, pointer_ut data,
 ...
 * are "over there").
 */
kern_return_t mach_vm_copy(vm_map_t map, mach_vm_address_ut source_address,
                           mach_vm_size_ut size,
  ...
  if (KERN_SUCCESS == kr) {
    ...
    kr = vm_map_copy_overwrite(map, dest_address, copy, size, FALSE);
...
kern_return_t vm_copy(vm_map_t map, vm_address_ut source_address,
                      vm_size_ut size, vm_address_ut dest_address) {
...
mach_vm_map_external(vm_map_t target_map, mach_vm_offset_ut *address,
  ...
  return mach_vm_map_kernel(target_map, address, initial_size, mask, vmk_flags,
                            port, offset, copy, cur_protection, max_protection,
                            inheritance);
}

/* legacy interface */
__attribute__((always_inline)) kern_return_t vm_map_64_external(
    vm_map_t target_map, vm_offset_ut *address, vm_size_ut size,
    vm_offset_ut mask, int flags, ipc_port_t port,
    memory_object_offset_ut offset, boolean_t copy, vm_prot_ut cur_protection,
    vm_prot_ut max_protection, vm_inherit_ut inheritance) {
  return mach_vm_map_external(target_map, address, size, mask, flags, port,
                              offset, copy, cur_protection, max_protection,
                              inheritance);
}

/* temporary, until world build */
__attribute__((always_inline)) kern_return_t
vm_map_external(vm_map_t target_map, vm_offset_ut *address, vm_size_ut size,
                vm_offset_ut mask, int flags, ipc_port_t port,
                vm_offset_ut offset, boolean_t copy, vm_prot_ut cur_protection,
                vm_prot_ut max_protection, vm_inherit_ut inheritance) {
  return mach_vm_map_external(target_map, address, size, mask, flags, port,
                              offset, copy, cur_protection, max_protection,
                              inheritance);
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
mach_vm_remap_new_external_sanitize(vm_map_t target_map,
                                    vm_prot_ut cur_protection_u,
                                    vm_prot_ut max_protection_u,
                                    vm_prot_t *cur_protection,
                                    vm_prot_t *max_protection) {
  return vm_sanitize_cur_and_max_prots(
...
/*
 * mach_vm_remap_new -
 * Behaves like mach_vm_remap, except that VM_FLAGS_RETURN_DATA_ADDR is always
 * set and {cur,max}_protection are in/out.
 */
kern_return_t
mach_vm_remap_new_external(vm_map_t target_map, mach_vm_offset_ut *address,
   ...
   */
  kr = mach_vm_remap_new_external_sanitize(target_map, *cur_protection_u,
                                           *max_protection_u, &cur_protection,
                                           &max_protection);
  if (__improbable(kr != KERN_SUCCESS)) {
    return vm_sanitize_get_kr(kr);
  }
  ...
  if (copy || max_protection == VM_PROT_READ ||
  ...
  kr = vm_map_remap(target_map, address, size, mask, vmk_flags, src_map,
                    memory_address, copy, cur_protection_u, /* IN/OUT */
...
mach_vm_remap_external(vm_map_t target_map, mach_vm_offset_ut *address,
                       ...
                       boolean_t copy, vm_prot_ut *cur_protection, /* OUT */
...
kern_return_t vm_remap_new_external(vm_map_t target_map, vm_offset_ut *address,
                                    ...
                                    vm_inherit_ut inheritance) {
  return mach_vm_remap_new_external(target_map, address, size, mask, flags,
                                    src_tport, memory_address, copy,
 ...
 * kernel context).
 */
kern_return_t vm_remap_external(vm_map_t target_map, vm_offset_ut *address,
                                vm_size_ut size, vm_offset_ut mask, int flags,
                                vm_map_t src_map, vm_offset_ut memory_address,
                                boolean_t copy,
                                vm_prot_ut *cur_protection, /* OUT */
                                vm_prot_ut *max_protection, /* OUT */
                                vm_inherit_ut inheritance) {
  return mach_vm_remap_external(target_map, address, size, mask, flags, src_map,
                                memory_address, copy, cur_protection,
...
kern_return_t mach_vm_wire_external(host_priv_t host_priv, vm_map_t map,
  ...
  if (VM_SANITIZE_UNSAFE_IS_ZERO(access)) {
    ...
    rc = vm_map_wire_impl(map, start, end, access, VM_KERN_MEMORY_MLOCK, true,
                          NULL, VM_SANITIZE_CALLER_VM_WIRE_USER);
  }
 ...
 *	[ To unwire the pages, specify VM_PROT_NONE. ]
 */
kern_return_t vm_wire(host_priv_t host_priv, vm_map_t map, vm_offset_ut start,
                      vm_size_ut size, vm_prot_ut access) {
...
kern_return_t mach_vm_msync(vm_map_t map, mach_vm_address_ut address_u,
                            mach_vm_size_ut size_u, vm_sync_t sync_flags) {
  if (map == VM_MAP_NULL) {
    return KERN_INVALID_TASK;
  }
...
kern_return_t vm_msync(vm_map_t map, vm_address_ut address_u, vm_size_ut size_u,
                       vm_sync_t sync_flags) {
  return mach_vm_msync(map, address_u, size_u, sync_flags);
}

int vm_toggle_entry_reuse(int toggle, int *old_value) {
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
mach_vm_behavior_set_sanitize(vm_map_t map, mach_vm_offset_ut start_u,
                              mach_vm_size_ut size_u,
                              vm_behavior_ut new_behavior_u,
                              mach_vm_offset_t *start, mach_vm_offset_t *end,
                              mach_vm_size_t *size,
                              vm_behavior_t *new_behavior) {
  mach_vm_offset_t align_mask;
  kern_return_t kr;

  kr = vm_sanitize_behavior(new_behavior_u, VM_SANITIZE_CALLER_VM_BEHAVIOR_SET,
                            new_behavior);
  if (__improbable(kr != KERN_SUCCESS)) {
    return kr;
  }
  ...
  kr =
      vm_sanitize_addr_size(start_u, size_u, VM_SANITIZE_CALLER_VM_BEHAVIOR_SET,
 ...
 */
kern_return_t mach_vm_behavior_set(vm_map_t map, mach_vm_offset_ut start_u,
                                   mach_vm_size_ut size_u,
                                   vm_behavior_ut new_behavior_u) {
  kern_return_t kr;
  mach_vm_offset_t start, end;
  ...
  kr = mach_vm_behavior_set_sanitize(map, start_u, size_u, new_behavior_u,
                                     &start, &end, &size, &new_behavior);
  if (__improbable(kr != KERN_SUCCESS)) {
    return vm_sanitize_get_kr(kr);
  }

  return vm_map_behavior_set(map, start, end, new_behavior);
}
 ...
 *	kernel's).
 */
kern_return_t vm_behavior_set(vm_map_t map, vm_offset_ut start, vm_size_ut size,
...
static inline kern_return_t vm_region_get_kern_return(kern_return_t kr,
                                                      vm_offset_ut addr_u,
                                                      vm_size_ut size_u) {
...
kern_return_t vm_region_64(vm_map_t map, vm_offset_ut *address_u, /* IN/OUT */
                           ...
                           mach_port_t *object_name)              /* OUT */
{
  ...
  kr = mach_vm_region(map, address_u, size_u, flavor, info, count, object_name);

  return vm_region_get_kern_return(kr, *address_u, *size_u);
}

kern_return_t vm_region(vm_map_t map, vm_address_ut *address_u, /* IN/OUT */
                        vm_size_ut *size_u,                     /* OUT */
                        vm_region_flavor_t flavor,              /* IN */
                        vm_region_info_t info,                  /* OUT */
                        mach_msg_type_number_t *count,          /* IN/OUT */
                        mach_port_t *object_name)               /* OUT */
{
  ...
  kr = vm_map_region(map, address_u, size_u, flavor, info, count, object_name);
...
kern_return_t mach_vm_region_recurse(vm_map_t map,
                                     ...
                                     mach_vm_size_ut *size_u, uint32_t *depth,
 ...
 *
 */
kern_return_t vm_region_recurse_64(vm_map_t map, vm_address_ut *address_u,
                                   vm_size_ut *size_u, uint32_t *depth,
...
kern_return_t vm_region_recurse(vm_map_t map,
                                ...
                                mach_msg_type_number_t *infoCnt) /* IN/OUT */
{
  ...
  *infoCnt = VM_REGION_SUBMAP_INFO_COUNT_64;
  ...
  kr =
      vm_map_region_recurse_64(map, address_u, size_u, depth, &info64, infoCnt);
...
kern_return_t mach_vm_purgable_control(vm_map_t map,
...
kern_return_t vm_purgable_control_external(mach_port_t target_tport,
                                           ...
                                           vm_purgable_t control, int *state) {
  return mach_vm_purgable_control_external(target_tport, address, control,
                                           state);
}

kern_return_t mach_vm_page_query(vm_map_t map, mach_vm_offset_ut offset_u,
                                 int *disposition, int *ref_count) {
  kern_return_t kr;
...
kern_return_t vm_map_page_query(vm_map_t map, vm_offset_ut offset,
                                int *disposition, int *ref_count) {
  return mach_vm_page_query(map, offset, disposition, ref_count);
}

static __attribute__((always_inline, warn_unused_result)) kern_return_t
mach_vm_page_range_query_sanitize(mach_vm_offset_ut address_u,
                                  mach_vm_size_ut size_u,
                                  int effective_page_mask,
                                  mach_vm_address_ut dispositions_addr_u,
                                  mach_vm_size_ut dispositions_count_u,
                                  mach_vm_offset_t *start, mach_vm_size_t *size,
  ...
  *dispositions_addr = VM_SANITIZE_UNSAFE_UNWRAP(dispositions_addr_u);
  ...
  if (os_mul_overflow(dispositions_count, sizeof(int), disp_buf_req_size) ||
      os_add_overflow(*dispositions_addr, *disp_buf_req_size, &discard)) {
    return KERN_INVALID_ARGUMENT;
  }

  return vm_sanitize_addr_size(
      address_u, size_u, VM_SANITIZE_CALLER_VM_MAP_PAGE_RANGE_QUERY,
      effective_page_mask, VM_SANITIZE_FLAGS_SIZE_ZERO_FALLTHROUGH, start, &end,
      size);
}

kern_return_t mach_vm_page_range_query(vm_map_t map,
                                       mach_vm_offset_ut address_u,
                                       mach_vm_size_ut size_u,
                                       mach_vm_address_ut dispositions_addr_u,
                                       mach_vm_size_ut *dispositions_count_u) {
  ...
  kr = mach_vm_page_range_query_sanitize(
      address_u, size_u, effective_page_mask, dispositions_addr_u,
      *dispositions_count_u, &start, &size, &dispositions_addr,
      &disp_buf_req_size);
  if (__improbable(kr != KERN_SUCCESS)) {
    return vm_sanitize_get_kr(kr);
  }
  ...
  while (size) {
    ...
    kr = vm_map_page_range_info_internal(
        map, start, vm_map_round_page(start + curr_sz, effective_page_mask),
        effective_page_shift, VM_PAGE_INFO_BASIC, (vm_page_info_t)info, &count);
    ...
    copy_sz =
        MIN(disp_buf_req_size, num_pages * sizeof(int) /* an int per page */);
    ...
    if ((disp_buf_req_size == 0) || (curr_sz >= size)) {
      ...
      curr_sz = MIN(vm_map_round_page(size, effective_page_mask),
                    MAX_PAGE_RANGE_QUERY);
      num_pages = (int)(curr_sz >> effective_page_shift);
    }
  }

  VM_SANITIZE_UT_SET(*dispositions_count_u, disp_buf_total_size / sizeof(int));
...
kern_return_t mach_vm_page_info(vm_map_t map, mach_vm_address_ut address,
 ...
 */
kern_return_t task_wire(vm_map_t map, boolean_t must_wire __unused) {
  if (map == VM_MAP_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
...
kern_return_t vm_region_object_create(__unused vm_map_t target_map,
...
kern_return_t mach_vm_deferred_reclamation_buffer_allocate(
    task_t task, mach_vm_address_ut *address, uint64_t *sampling_period,
    uint32_t initial_capacity, uint32_t max_capacity) {
#if CONFIG_DEFERRED_RECLAIM
  ...
  return vm_deferred_reclamation_buffer_allocate_internal(
      task, address, sampling_period, initial_capacity, max_capacity);
#else
  (void)task;
  (void)address;
  (void)size;
...
kern_return_t mach_vm_deferred_reclamation_buffer_flush(
    task_t task, uint32_t num_entries_to_reclaim,
  ...
  kr = vm_deferred_reclamation_buffer_flush_internal(
      task, num_entries_to_reclaim, &bytes_reclaimed);
...
kern_return_t mach_vm_deferred_reclamation_buffer_resize(
    task_t task, uint32_t new_len, mach_vm_size_ut *bytes_reclaimed_out) {
#if CONFIG_DEFERRED_RECLAIM
  mach_error_t err;
  mach_vm_size_t bytes_reclaimed = 0;
  ...
  err = vm_deferred_reclamation_buffer_resize_internal(task, new_len,
                                                       &bytes_reclaimed);
  *bytes_reclaimed_out = vm_sanitize_wrap_size(bytes_reclaimed);
  return err;
#else
...
#if CONFIG_DEFERRED_RECLAIM
  return vm_deferred_reclamation_buffer_query_internal(task, addr_out_ut,
                                                       size_out_ut);
...
extern void qsort(void *a, size_t n, size_t es,
                  int (*cmp)(const void *, const void *));

static int vm_map_user_range_cmp(const void *e1, const void *e2) {
...
static int mach_vm_range_recipe_v1_cmp(const void *e1, const void *e2) {
...
static inline __result_use_check kern_return_t mach_vm_range_create_v1_sanitize(
    vm_map_t map, mach_vm_range_recipe_v1_ut *recipe_u, uint32_t count,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    kr = vm_sanitize_addr_end(range_u->min_address_u, range_u->max_address_u,
                              VM_SANITIZE_CALLER_MACH_VM_RANGE_CREATE, map,
...
mach_vm_range_create_v1(vm_map_t map, mach_vm_range_recipe_v1_ut *recipe_u,
                        uint32_t new_count) {
  ...
  for (size_t i = 0; i < new_count; i++) {
    ...
    if (i > 0 &&
        recipe[i - 1].range.max_address > recipe[i].range.min_address) {
      vmlp_api_end(MACH_VM_RANGE_CREATE_V1, KERN_INVALID_ARGUMENT);
      return KERN_INVALID_ARGUMENT;
    }
  }
  ...
  table = krealloc_data(table, count * sizeof(struct vm_map_user_range),
  ...
  qsort(table, count + new_count, sizeof(struct vm_map_user_range),
        vm_map_user_range_cmp);
...
out_unlock:
  ...
  if (kr == KERN_SUCCESS) {
    for (size_t i = 0; i < new_count; i++) {
      ...
      vmlp_range_event(map, recipe[i].range.min_address,
                       recipe[i].range.max_address -
                           recipe[i].range.min_address);

      kr2 = vm_map_enter(map, &recipe[i].range.min_address,
                         mach_vm_range_size(&recipe[i].range), 0, vmk_flags,
                         VM_OBJECT_NULL, 0, FALSE, VM_PROT_NONE, VM_PROT_ALL,
                         VM_INHERIT_DEFAULT);
...
kern_return_t mach_vm_range_create(vm_map_t map, mach_vm_range_flavor_t flavor,
...
kern_return_t mach_vm_map(vm_map_t target_map, mach_vm_offset_ut *address,
                          mach_vm_size_ut initial_size, mach_vm_offset_ut mask,
                          int flags, ipc_port_t port,
                          memory_object_offset_ut offset, boolean_t copy,
                          vm_prot_ut cur_protection, vm_prot_ut max_protection,
                          vm_inherit_ut inheritance) {
  return mach_vm_map_external(target_map, address, initial_size, mask, flags,
                              port, offset, copy, cur_protection,
                              max_protection, inheritance);
}

kern_return_t mach_vm_remap(vm_map_t target_map, mach_vm_offset_ut *address,
                            mach_vm_size_ut size, mach_vm_offset_ut mask,
                            int flags, vm_map_t src_map,
                            mach_vm_offset_ut memory_address, boolean_t copy,
                            vm_prot_ut *cur_protection, /* OUT */
                            vm_prot_ut *max_protection, /* OUT */
                            vm_inherit_ut inheritance) {
...
kern_return_t vm_map(vm_map_t target_map, vm_offset_ut *address,
                     vm_size_ut size, vm_offset_ut mask, int flags,
                     ipc_port_t port, vm_offset_ut offset, boolean_t copy,
                     vm_prot_ut cur_protection, vm_prot_ut max_protection,
                     vm_inherit_ut inheritance) {
  return mach_vm_map(target_map, address, size, mask, flags, port, offset, copy,
