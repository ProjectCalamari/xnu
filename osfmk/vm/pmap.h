...
extern kern_return_t copypv(addr64_t source, addr64_t sink, unsigned int size,
                            int which);
...
#define cppvPsrcb 30
#define cppvFsnk                                                               \
  0x000000004 /* Destination requires flushing (only on non-coherent I/O) */
#define cppvFsnkb 29
#define cppvFsrc                                                               \
  0x000000008 /* Source requires flushing (only on non-coherent I/O) */
#define cppvFsrcb 28
...
pmap_steal_memory(vm_size_t size,
                  vm_size_t alignment); /* Early memory allocation */
extern void *
pmap_steal_freeable_memory(vm_size_t size); /* Early memory allocation */

extern uint_t
pmap_free_pages(void); /* report remaining unused physical pages */
#if defined(__arm__) || defined(__arm64__)
extern ppnum_t pmap_first_pnum; /* the first valid physical page on the system
                                   == atop(gDramBase) */
extern uint_t pmap_free_pages_span(
    void); /* report phys address range of unused physical pages */
#endif     /* defined(__arm__) || defined(__arm64__) */

extern void pmap_startup(vm_offset_t *startp,
                         vm_offset_t *endp); /* allocate vm_page structs */

extern void
pmap_init(void); /* Initialization, once we have kernel virtual memory.  */

extern void mapping_adjust(void); /* Adjust free mapping count */

extern void
mapping_free_prime(void); /* Primes the mapping block release list */
...
#else
__enum_decl(pmap_mapping_type_t, uint8_t,
            {PMAP_MAPPING_TYPE_INFER = 0, PMAP_MAPPING_TYPE_DEFAULT,
             PMAP_MAPPING_TYPE_ROZONE, PMAP_MAPPING_TYPE_RESTRICTED});
#endif

/*
 * Report virtual space available for the kernel.
 */
extern void pmap_virtual_space(vm_offset_t *virtual_start,
...
#if MACH_ASSERT
extern void pmap_set_process(pmap_t pmap, int pid, char *procname);
#endif /* MACH_ASSERT */

extern kern_return_t pmap_enter(/* Enter a mapping */
                                pmap_t pmap, vm_map_offset_t v, ppnum_t pn,
                                vm_prot_t prot, vm_prot_t fault_type,
                                unsigned int flags, boolean_t wired,
                                pmap_mapping_type_t mapping_type);

extern kern_return_t pmap_enter_options(pmap_t pmap, vm_map_offset_t v,
                                        ppnum_t pn, vm_prot_t prot,
                                        vm_prot_t fault_type,
                                        unsigned int flags, boolean_t wired,
                                        unsigned int options, void *arg,
                                        pmap_mapping_type_t mapping_type);
extern kern_return_t pmap_enter_options_addr(
    pmap_t pmap, vm_map_offset_t v, pmap_paddr_t pa, vm_prot_t prot,
    vm_prot_t fault_type, unsigned int flags, boolean_t wired,
    unsigned int options, void *arg, pmap_mapping_type_t mapping_type);

extern void pmap_remove_some_phys(pmap_t pmap, ppnum_t pn);

extern void pmap_lock_phys_page(ppnum_t pn);

extern void pmap_unlock_phys_page(ppnum_t pn);
...
extern void pmap_page_protect(/* Restrict access to page. */
                              ppnum_t phys, vm_prot_t prot);

extern void pmap_page_protect_options(/* Restrict access to page. */
                                      ppnum_t phys, vm_prot_t prot,
                                      unsigned int options, void *arg);

extern void(pmap_zero_page)(ppnum_t pn);

extern void(pmap_zero_page_with_options)(ppnum_t pn, int options);

extern void(pmap_zero_part_page)(ppnum_t pn, vm_offset_t offset, vm_size_t len);

extern void(pmap_copy_page)(ppnum_t src, ppnum_t dest, int options);

extern void(pmap_copy_part_page)(ppnum_t src, vm_offset_t src_offset,
                                 ppnum_t dst, vm_offset_t dst_offset,
                                 vm_size_t len);

extern void(pmap_copy_part_lpage)(vm_offset_t src, ppnum_t dst,
                                  vm_offset_t dst_offset, vm_size_t len);

extern void(pmap_copy_part_rpage)(ppnum_t src, vm_offset_t src_offset,
                                  vm_offset_t dst, vm_size_t len);

extern unsigned int(
    pmap_disconnect)(/* disconnect mappings and return reference and change */
                     ppnum_t phys);

extern unsigned int(pmap_disconnect_options)(/* disconnect mappings and return
                                                reference and change */
                                             ppnum_t phys,
                                             unsigned int options, void *arg);

extern kern_return_t(
    pmap_attribute_cache_sync)(/* Flush appropriate
                                * cache based on
                                * page number sent */
                               ppnum_t pn, vm_size_t size,
                               vm_machine_attribute_t attribute,
                               vm_machine_attribute_val_t *value);

extern unsigned int(pmap_cache_attributes)(ppnum_t pn);

/*
 * Set (override) cache attributes for the specified physical page
 */
extern void pmap_set_cache_attributes(ppnum_t, unsigned int);

extern void *pmap_map_compressor_page(ppnum_t);

extern void pmap_unmap_compressor_page(ppnum_t, void *);

/**
 * The following declarations are meant to provide a uniform interface by which
 * the VM layer can pass batches of pages to the pmap layer directly, in the
 * various page list formats natively used by the VM.  If a new type of list is
 * to be added, the various structures and iterator functions below should be
 * updated to understand it, and then it should "just work" with the pmap layer.
 */

/* The various supported page list types. */
__enum_decl(
    unified_page_list_type_t, uint8_t,
    {
        ...
        /* Doubly-linked queue of vm_page_t's associated with a VM object, using
           vmp_listq field. */
        UNIFIED_PAGE_LIST_TYPE_VM_PAGE_OBJ_Q,
        /* Doubly-linked queue of vm_page_t's in a FIFO queue or global free
           list, using vmp_pageq field. */
        UNIFIED_PAGE_LIST_TYPE_VM_PAGE_FIFO_Q,
    });

/* Uniform data structure encompassing the various page list types handled by
 * the VM layer. */
typedef struct {
  union {
    /* Base address and size (in pages) of UPL array for type
     * UNIFIED_PAGE_LIST_TYPE_UPL_ARRAY */
    struct {
      upl_page_info_array_t upl_info;
      unsigned int upl_size;
    } upl;
    /* Head of singly-linked vm_page_t list for
     * UNIFIED_PAGE_LIST_TYPE_VM_PAGE_LIST */
    vm_page_t page_slist;
    /* Head of queue for UNIFIED_PAGE_LIST_TYPE_VM_PAGE_OBJ_Q and
     * UNIFIED_PAGE_LIST_TYPE_VM_PAGE_FIFO_Q */
...
/* Uniform data structure representing an iterator position within a
 * unified_page_list_t object. */
typedef struct {
...
extern void
unified_page_list_iterator_init(const unified_page_list_t *page_list,
...
extern ppnum_t
unified_page_list_iterator_page(const unified_page_list_iterator_t *iter,
                                bool *is_fictitious);

extern vm_page_t
unified_page_list_iterator_vm_page(const unified_page_list_iterator_t *iter);

extern void pmap_batch_set_cache_attributes(const unified_page_list_t *,
                                            unsigned int);
extern void pmap_sync_page_data_phys(ppnum_t pa);
extern void pmap_sync_page_attributes_phys(ppnum_t pa);

/**
 * pmap entry point for performing platform-specific integrity checks and
 * cleanup when the VM is about to free a page.  This function will typically at
 * least validate that the page has no outstanding mappings or other references,
 * and depending upon the platform may also take additional steps to reset page
 * state.
 *
...
extern void(pmap_copy)(/* Copy range of mappings,
                        * if desired. */
                       pmap_t dest, pmap_t source, vm_map_offset_t dest_va,
                       vm_map_size_t size, vm_map_offset_t source_va);

extern kern_return_t(pmap_attribute)(/* Get/Set special memory
                                      * attributes */
                                     pmap_t pmap, vm_map_offset_t va,
                                     vm_map_size_t size,
                                     vm_machine_attribute_t attribute,
                                     vm_machine_attribute_val_t *value);
...
#else /* PMAP_ACTIVATE */
#define PMAP_ACTIVATE_KERNEL(cpu) PMAP_ACTIVATE(pmap_kernel(), THREAD_NULL, cpu)
#endif /* PMAP_ACTIVATE */
...
#define PMAP_BATCH_SET_CACHE_ATTR(object, user_page_list, cache_attr,          \
  ...
  if ((batch_pmap_op)) {                                                       \
    const unified_page_list_t __pmap_batch_list = {                            \
        .upl =                                                                 \
            {                                                                  \
                .upl_info = (user_page_list),                                  \
                .upl_size = (num_pages),                                       \
            },                                                                 \
        .type = UNIFIED_PAGE_LIST_TYPE_UPL_ARRAY,                              \
    };                                                                         \
    pmap_batch_set_cache_attributes(&__pmap_batch_list, (cache_attr));         \
    (object)->set_cache_attr = TRUE;                                           \
  }                                                                            \
  MACRO_END
...
extern void pmap_clear_refmod_options(ppnum_t pn, unsigned int mask,
 ...
 */
extern bool pmap_clear_refmod_range_options(pmap_t pmap, vm_map_address_t start,
...
extern void pmap_protect(/* Change protections. */
                         pmap_t map, vm_map_offset_t s, vm_map_offset_t e,
                         vm_prot_t prot);

extern void pmap_protect_options(/* Change protections. */
                                 pmap_t map, vm_map_offset_t s,
                                 vm_map_offset_t e, vm_prot_t prot,
                                 unsigned int options, void *arg);

extern void(pmap_pageable)(pmap_t pmap, vm_map_offset_t start,
                           vm_map_offset_t end, boolean_t pageable);

extern uint64_t pmap_shared_region_size_min(pmap_t map);

extern void pmap_set_shared_region(pmap_t, pmap_t, addr64_t, uint64_t);
extern kern_return_t pmap_nest(pmap_t, pmap_t, addr64_t, uint64_t);
extern kern_return_t pmap_unnest(pmap_t, addr64_t, uint64_t);

#define PMAP_UNNEST_CLEAN 1

extern kern_return_t pmap_fork_nest(pmap_t old_pmap, pmap_t new_pmap);

extern kern_return_t pmap_unnest_options(pmap_t, addr64_t, uint64_t,
                                         unsigned int);
extern boolean_t pmap_adjust_unnest_parameters(pmap_t, vm_map_offset_t *,
                                               vm_map_offset_t *);
...
#define PMAP_CREATE_KNOWN_FLAGS                                                \
  (PMAP_CREATE_64BIT | PMAP_CREATE_EPT | PMAP_CREATE_TEST)
...
#define PMAP_CREATE_NESTED                                                     \
  0x80 /* pmap will not try to allocate a subpage root table to save space */

/* Define PMAP_CREATE_KNOWN_FLAGS in terms of optional flags */
#define PMAP_CREATE_KNOWN_FLAGS                                                \
  (PMAP_CREATE_64BIT | PMAP_CREATE_STAGE2 | PMAP_CREATE_DISABLE_JOP |          \
   PMAP_CREATE_FORCE_4K_PAGES | PMAP_CREATE_X86_64 | PMAP_CREATE_ROSETTA |     \
   PMAP_CREATE_TEST | PMAP_CREATE_NESTED)

#endif /* __x86_64__ */

#define PMAP_OPTIONS_NOWAIT                                                    \
  0x1 /* don't block, return                                                   \
       * KERN_RESOURCE_SHORTAGE                                                \
       * instead */
#define PMAP_OPTIONS_NOENTER                                                   \
  0x2 /* expand pmap if needed                                                 \
       * but don't enter mapping                                               \
       */
#define PMAP_OPTIONS_COMPRESSOR                                                \
  0x4                              /* credit the compressor for                \
                                    * this operation */
...
#define PMAP_OPTIONS_ALT_ACCT                                                  \
  0x80                            /* use alternate accounting scheme for page */
...
#define PMAP_OPTIONS_COMPRESSOR_IFF_MODIFIED                                   \
  0x800 /* credit the compressor                                               \
         * iff page was modified */
#define PMAP_OPTIONS_PROTECT_IMMEDIATE                                         \
  0x1000 /* allow protections to be                                            \
          * be upgraded */
#define PMAP_OPTIONS_CLEAR_WRITE 0x2000
#define PMAP_OPTIONS_TRANSLATED_ALLOW_EXECUTE                                  \
  0x4000 /* Honor execute for translated processes */
#if defined(__arm__) || defined(__arm64__)
...
/* Indicates that pmap_enter() or pmap_remove() is being called with preemption
 * already disabled. */
#define PMAP_OPTIONS_NOPREEMPT 0x80000

#if CONFIG_SPTM
/* Requests pmap_disconnect() to reset the page frame type (only meaningful for
 * SPTM systems) */
...
#define PMAP_OPTIONS_RESERVED_MASK                                             \
  0xFF000000 /* encoding space reserved for internal pmap use */
...
mach_vm_size_t pmap_query_resident(pmap_t pmap, vm_map_offset_t s,
                                   vm_map_offset_t e,
 ...
 * use.
 */
extern void pmap_trim(pmap_t grand, pmap_t subord, addr64_t vstart,
                      uint64_t size);

extern bool pmap_is_nested(pmap_t pmap);

/*
 * Dump page table contents into the specified buffer.  Returns
 * KERN_INSUFFICIENT_BUFFER_SIZE if insufficient space, KERN_NOT_SUPPORTED if
 * unsupported in the current configuration. This is expected to only be called
 * from kernel debugger context, so synchronization is not required.
 */

extern kern_return_t pmap_dump_page_tables(pmap_t pmap, void *bufp,
                                           void *buf_end,
 ...
 * layer.
 */
bool pmap_has_prot_policy(pmap_t pmap, bool translated_allow_execute,
                          vm_prot_t prot);
...
extern kern_return_t pmap_query_page_info(pmap_t pmap, vm_map_offset_t va,
                                          int *disp);
 ...
 *       writability at page granularity, this mechanism allows specific
 * registers on a read-mostly page to be written using a dedicated guarded mode
 * trap without requiring a full PPL driver extension.
 *
 ...
 *
 * @note On supported devices (determined by
 * pmap_has_iofilter_protected_write()), this function goes over the sorted I/O
 * filter entry table. If there is a hit, the write is performed from Guarded
 * Mode. Otherwise, the write is performed from Normal Mode (kernel mode). Note
 * that you can still hit an exception if the register is owned by PPL but not
 * allowed by an io-filter-entry in the device tree.
 *
 * @note On unsupported devices, this function will panic.
 *
 * @param addr The address of the register.
 * @param value The value to be written.
 * @param width The width of the I/O register, supported values are 1, 2, 4
 * and 8.
 */
extern void pmap_iofilter_protected_write(vm_address_t addr, uint64_t value,
                                          uint64_t width);
...
typedef enum { PMAP_FEAT_UEXEC = 1 } pmap_feature_flags_t;
 ...
 */
extern bool pmap_performs_stage2_translations(const pmap_t pmap);
...
/*
 * All pages allocated by TXM are also kept within the TXM VM object, which
 * allows tracking it for accounting and debugging purposes.
 */
 ...
 */
extern pmap_t pmap_txm_kernel_pmap(void);

/**
 * Acquire the TXM address space object stored within the pmap.
 */
extern TXMAddressSpace_t *pmap_txm_addr_space(const pmap_t pmap);

/**
 * Set the TXM address space object within the pmap.
 */
extern void pmap_txm_set_addr_space(pmap_t pmap,
                                    TXMAddressSpace_t *txm_addr_space);

/**
 * Set the trust level of the TXM address space object within the pmap.
 */
extern void pmap_txm_set_trust_level(pmap_t pmap, CSTrust_t trust_level);

/**
 * Get the trust level of the TXM address space object within the pmap.
 */
extern kern_return_t pmap_txm_get_trust_level_kdp(pmap_t pmap,
 ...
 * on the current pmap.
 */
extern void pmap_txm_acquire_shared_lock(pmap_t pmap);
 ...
 * current pmap.
 */
extern void pmap_txm_release_shared_lock(pmap_t pmap);
 ...
 * on the current pmap.
 */
extern void pmap_txm_acquire_exclusive_lock(pmap_t pmap);
 ...
 * current pmap.
 */
extern void pmap_txm_release_exclusive_lock(pmap_t pmap);

/**
 * Transfer a page to the TXM_DEFAULT type after resolving its mapping from its
 * virtual to physical address.
 */
extern void pmap_txm_transfer_page(const vm_address_t addr);
 ...
 */
extern vm_map_address_t pmap_txm_allocate_page(void);
