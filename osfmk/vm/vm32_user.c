...
kern_return_t vm32_vm_allocate(vm_map_t map, vm32_address_ut *addr32,
                               vm32_size_ut size32, int flags) {
...
kern_return_t vm32_vm_deallocate(vm_map_t map, vm32_offset_ut start32,
                                 vm32_size_ut size32) {
...
kern_return_t vm32_vm_inherit(vm_map_t map, vm32_offset_ut start32,
                              vm32_size_ut size32,
...
kern_return_t vm32_vm_protect(vm_map_t map, vm32_offset_ut start32,
                              vm32_size_ut size32, boolean_t set_maximum,
...
kern_return_t
vm32_vm_machine_attribute(vm_map_t map, vm32_address_ut addr32,
                          vm32_size_ut size32, vm_machine_attribute_t attribute,
...
kern_return_t vm32_vm_read(vm_map_t map, vm32_address_ut addr32,
                           vm32_size_ut size32, pointer_ut *data,
...
kern_return_t vm32_vm_read_list(vm_map_t map, vm32_read_entry_t data_list,
                                natural_t count) {
  ...
  for (i = 0; i < VM_MAP_ENTRY_MAX; i++) {
    data_list[i].address =
        CAST_DOWN_EXPLICIT(vm32_address_t, mdata_list[i].address);
...
kern_return_t vm32_vm_read_overwrite(vm_map_t map, vm32_address_ut addr32,
                                     vm32_size_ut size32,
...
kern_return_t vm32_vm_write(vm_map_t map, vm32_address_ut addr32,
                            pointer_ut data, mach_msg_type_number_t size) {
  mach_vm_offset_ut addr;
...
kern_return_t vm32_vm_copy(vm_map_t map, vm32_address_ut src_addr32,
                           vm32_size_ut size32, vm32_address_ut dst_addr32) {
...
kern_return_t vm32_vm_map_64(vm_map_t target_map, vm32_offset_ut *addr32,
                             vm32_size_ut size32, vm32_offset_ut mask32,
  ...
  result =
      mach_vm_map_external(target_map, &addr, size, mask, flags, port, offset,
...
kern_return_t vm32_vm_map(vm_map_t target_map, vm32_offset_ut *address,
                          vm32_size_ut size, vm32_offset_ut mask, int flags,
                          ipc_port_t port, vm32_offset_ut offset32,
  ...
  return vm32_vm_map_64(target_map, address, size, mask, flags, port, offset,
                        copy, cur_protection, max_protection, inheritance);
}

kern_return_t vm32_vm_remap(vm_map_t target_map, vm32_offset_ut *addr32,
                            vm32_size_ut size32, vm32_offset_ut mask32,
                            boolean_t anywhere, vm_map_t src_map,
                            vm32_offset_ut src_addr32, boolean_t copy,
                            vm_prot_ut *cur_protection,
                            vm_prot_ut *max_protection,
                            vm_inherit_ut inheritance) {
  ...
  result = mach_vm_remap_external(target_map, &addr, size, mask, anywhere,
                                  src_map, src_addr, copy, cur_protection,
...
kern_return_t vm32_vm_msync(vm_map_t map, vm32_address_ut addr32,
                            vm32_size_ut size32, vm_sync_t sync_flags) {
  mach_vm_offset_ut addr;
  mach_vm_size_ut size;
...
kern_return_t vm32_vm_behavior_set(vm_map_t map, vm32_offset_ut start32,
                                   vm32_size_ut size32,
...
static inline kern_return_t vm32_region_get_kern_return(kern_return_t kr,
                                                        vm_offset_ut addr,
                                                        vm_size_ut size) {
...
kern_return_t vm32_vm_region_64(vm_map_t map,
...
kern_return_t vm32_vm_region(vm_map_t map, vm32_address_ut *addr32, /* IN/OUT */
                             vm32_size_ut *size32,                  /* OUT */
                             vm_region_flavor_t flavor,             /* IN */
                             vm_region_info_t info,                 /* OUT */
                             mach_msg_type_number_t *count,         /* IN/OUT */
...
kern_return_t vm32_vm_region_recurse_64(vm_map_t map, vm32_address_ut *addr32,
                                        vm32_size_ut *size32, uint32_t *depth,
...
kern_return_t
vm32_vm_region_recurse(vm_map_t map, vm32_offset_ut *addr32, /* IN/OUT */
                       vm32_size_ut *size32,                 /* OUT */
                       ...
                       mach_msg_type_number_t *infoCnt)      /* IN/OUT */
{
  ...
  *infoCnt = VM_REGION_SUBMAP_INFO_COUNT_64;
  ...
  kr = mach_vm_region_recurse(map, &addr, &size, depth,
                              (vm_region_recurse_info_t)&info64, infoCnt);
...
kern_return_t vm32_vm_purgable_control(vm_map_t map, vm32_offset_ut addr32,
...
kern_return_t vm32_vm_map_page_query(vm_map_t map, vm32_offset_t offset32,
...
kern_return_t vm32_mach_make_memory_entry_64(vm_map_t target_map,
  ...
  return _mach_make_memory_entry(target_map, size, offset, permission,
                                 object_handle, parent_handle);
}

kern_return_t vm32_mach_make_memory_entry(
    vm_map_t target_map, vm32_size_ut *size, vm32_offset_ut offset,
    vm_prot_ut permission, ipc_port_t *object_handle, ipc_port_t parent_entry) {
  ...
  kr = _mach_make_memory_entry(target_map, &mo_size, mo_offset, permission,
                               object_handle, parent_entry);
  *size = vm_sanitize_trunc_size_to_32(mo_size);
  return kr;
}

kern_return_t vm32_task_wire(vm_map_t map, boolean_t must_wire __unused) {
  if (map == VM_MAP_NULL) {
    return KERN_INVALID_ARGUMENT;
  }
...
kern_return_t vm32_vm_map_exec_lockdown(vm_map_t map) {
