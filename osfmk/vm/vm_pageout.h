...
struct freezer_context {
  ...
  char *freezer_ctx_compressor_scratch_buf; /* Scratch buffer for the compressor
                                               algorithm. */
};
...
extern upl_size_t upl_get_size(upl_t upl);
...
#if UPL_DEBUG
extern kern_return_t upl_ubc_alias_set(upl_t upl, uintptr_t alias1,
                                       uintptr_t alias2);
extern int upl_ubc_alias_get(upl_t upl, uintptr_t *al, uintptr_t *al2);
#endif /* UPL_DEBUG */

extern void vm_countdirtypages(void);

extern kern_return_t upl_transpose(upl_t upl1, upl_t upl2);

extern kern_return_t mach_vm_pressure_monitor(boolean_t wait_for_pressure,
                                              unsigned int nsecs_monitored,
                                              unsigned int *pages_reclaimed_p,
                                              unsigned int *pages_wanted_p);

extern kern_return_t vm_set_buffer_cleanup_callout(boolean_t (*func)(int));
...
struct vm_config {
  boolean_t compressor_is_present; /* compressor is initialized and can be used
                                      by the freezer, the sweep or the pager */
  boolean_t compressor_is_active;  /* pager can actively compress pages...
                                      'compressor_is_present' must be set */
  boolean_t swap_is_present;       /* swap is initialized and can be used by the
                                      freezer, the sweep or the pager */
  boolean_t swap_is_active;        /* pager can actively swap out compressed
                                      segments... 'swap_is_present' must be set */
  boolean_t freezer_swap_is_active; /* freezer can swap out frozen tasks...
                                       "compressor_is_present + swap_is_present"
                                       must be set */
};
...
#define VM_PAGER_COMPRESSOR_WITH_SWAP                                          \
  0x4 /* Active in-core compressor + swap backend. */
#define VM_PAGER_FREEZER_DEFAULT                                               \
  0x8 /* Freezer backed by default pager... DEPRECATED */
#define VM_PAGER_FREEZER_COMPRESSOR_NO_SWAP                                    \
  0x10 /* Freezer backed by in-core compressor only i.e. frozen data remain    \
          in-core compressed.*/
#define VM_PAGER_COMPRESSOR_NO_SWAP_PLUS_FREEZER_COMPRESSOR_WITH_SWAP          \
  0x20 /* Active in-core compressor + Freezer backed by in-core compressor     \
          with swap support too.*/

#define VM_PAGER_MAX_MODES 6 /* Total number of vm compressor modes supported  \
                              */

#define VM_CONFIG_COMPRESSOR_IS_PRESENT                                        \
...
#define VM_CONFIG_FREEZER_SWAP_IS_ACTIVE                                       \
