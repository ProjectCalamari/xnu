...
static void vm_purgeable_stats_helper(vm_purgeable_stat_t *stat,
                                      purgeable_q_t queue, int group,
                                      task_t target_task);

#if MACH_ASSERT
static void vm_purgeable_token_check_queue(purgeable_q_t queue) {
  ...
  /*
   * Due to performance impact, perform this check less frequently on
   * DEVELOPMENT kernels. Checking the queue scales linearly with its length, so
   * we compensate by by performing this check less frequently as the queue
   * grows.
   */
  ...
  if (queue->type != PURGEABLE_Q_TYPE_OBSOLETE) {
    ...
    assert((uint32_t)our_inactive_count ==
           vm_page_inactive_count - vm_page_cleaned_count);
  }
}
#endif
...
find_available_token:
  ...
  if (token_free_idx) { /* unused tokens available */
    ...
    while (token_q_allocating) {
      wait_result_t res =
          lck_mtx_sleep(&vm_page_queue_lock, LCK_SLEEP_DEFAULT,
                        (event_t)&token_q_allocating, THREAD_UNINT);
    ...
    if (alloc_size <= TOKEN_COUNT_MAX * sizeof(struct token)) {
      kmr =
          kmem_realloc_guard(kernel_map, (vm_offset_t)tokens, token_q_cur_size,
                             alloc_size, KMR_ZERO | KMR_DATA_SHARED, guard);
    }
    ...
    token_q_max_cnt = (token_idx_t)(token_q_cur_size / sizeof(struct token));
    assert(token_init_idx <
           token_q_max_cnt); /* We must have a free token now */

    /* kmem_realloc_guard() might leave the old region mapped. */
    if (kmem_realloc_should_free((vm_offset_t)old_tokens, kmr)) {
      vm_page_unlock_queues();
      kmem_free_guard(kernel_map, (vm_offset_t)old_tokens, old_token_q_cur_size,
                      KMF_NONE, guard);
      vm_page_lock_queues();
    }
  ...
  KDBG((VMDBG_CODE(DBG_VM_PURGEABLE_TOKEN_ADD)) | DBG_FUNC_NONE, queue->type,
...
static token_idx_t vm_purgeable_token_remove_first(purgeable_q_t queue) {
  ...
  if (token) {
    ...
    KDBG(VMDBG_CODE(DBG_VM_PURGEABLE_TOKEN_DELETE) | DBG_FUNC_NONE, queue->type,
...
static token_idx_t vm_purgeable_token_remove_last(purgeable_q_t queue) {
  LCK_MTX_ASSERT(&vm_page_queue_lock, LCK_MTX_ASSERT_OWNED);
...
/* Call with page queue locked. */
void vm_purgeable_q_advance_all() {
  LCK_MTX_ASSERT(&vm_page_queue_lock, LCK_MTX_ASSERT_OWNED);
  ...
  int i;
  if (token_new_pagecount > (TOKEN_NEW_PAGECOUNT_MAX >>
    ...
    while (queue->token_q_unripe) {
      ...
      if (tokens[queue->token_q_unripe].count == 0) {
        ...
        KDBG(VMDBG_CODE(DBG_VM_PURGEABLE_TOKEN_RIPEN) | DBG_FUNC_NONE,
             queue->type,
             tokens[queue->token_q_head]
                 .count, /* num pages on new first token */
             0, available_for_purge);
 ...
 * Call with page queue locked.
 */
static void vm_purgeable_token_remove_ripe(purgeable_q_t queue) {
  LCK_MTX_ASSERT(&vm_page_queue_lock, LCK_MTX_ASSERT_OWNED);
 ...
 * Call with page queue locked.
 */
static void vm_purgeable_token_choose_and_delete_ripe(purgeable_q_t queue,
  ...
  if (tokens[queue->token_q_head].count == 0) {
    ...
    assert(queue->token_q_unripe ==
           queue->token_q_head); /* queue1 had no unripe
                                  * tokens, remember? */
    ...
    while (token_to_insert_before != 0 &&
    ...
    if ((token_to_insert_before == queue2->token_q_unripe) ||
    ...
    if (token_to_insert_before != 0) {
      ...
      /* if we ran off the end of the list, the token to insert after is the
       * tail */
...
static vm_object_t vm_purgeable_object_find_and_lock(purgeable_q_t queue,
                                                     int group,
                                                     boolean_t pick_ripe) {
  ...
  for (object = (vm_object_t)queue_first(&queue->objq[group]);
    ...
    if (pick_ripe && !object->purgeable_when_ripe) {
...
#if CONFIG_JETSAM
      object_task_importance = proc_get_memstat_priority(
          (struct proc *)get_bsdtask_info(owner), TRUE);
#endif /* CONFIG_JETSAM */
  ...
  KDBG_RELEASE(VMDBG_CODE(DBG_VM_PURGEABLE_OBJECT_PURGE_LOOP) | DBG_FUNC_END,
               ...
               try_lock_failed, try_lock_succeeded,
  ...
  queue_enter(&purgeable_nonvolatile_queue, object, vm_object_t, objq);
...
boolean_t vm_purgeable_object_purge_one(int force_purge_below_group,
  ...
  KDBG(VMDBG_CODE(DBG_VM_PURGEABLE_OBJECT_PURGE) | DBG_FUNC_END, 0, 0,
       available_for_purge);
...
purge_now:
  ...
  //	printf("%sPURGING object %p task %p importance %d queue %d group %d
  //force_purge_below_group %d memorystatus_vm_pressure_level %d\n",
  //forced_purge ? "FORCED " : "", object, object->vo_owner,
  //task_importance_estimate(object->vo_owner), i, group,
  //force_purge_below_group, memorystatus_vm_pressure_level);
  ...
  KDBG(VMDBG_CODE(DBG_VM_PURGEABLE_OBJECT_PURGE) | DBG_FUNC_END,
       ...
       resident_page_count, available_for_purge);
...
void vm_purgeable_object_add(vm_object_t object, purgeable_q_t queue,
  ...
  queue_remove(&purgeable_nonvolatile_queue, object, vm_object_t, objq);
  object->objq.next = NULL;
  object->objq.prev = NULL;
  assert(purgeable_nonvolatile_count > 0);
  purgeable_nonvolatile_count--;
  ...
  if (queue->type != PURGEABLE_Q_TYPE_LIFO) { /* fifo and obsolete are
                                               * fifo-queued */
    queue_enter(&queue->objq[group], object, vm_object_t,
                objq); /* last to die */
  } else {
    queue_enter_first(&queue->objq[group], object, vm_object_t,
                      objq); /* first to die */
  }
...
#if MACH_ASSERT
  queue->debug_count_objects++;
  KDBG(VMDBG_CODE(DBG_VM_PURGEABLE_OBJECT_ADD) | DBG_FUNC_NONE, 0,
       tokens[queue->token_q_head].count, queue->type, group);
#endif
...
#if MACH_ASSERT
  queue->debug_count_objects--;
  KDBG(VMDBG_CODE(DBG_VM_PURGEABLE_OBJECT_REMOVE) | DBG_FUNC_NONE, 0,
       tokens[queue->token_q_head].count, queue->type, group);
#endif
...
void vm_purgeable_stats_helper(vm_purgeable_stat_t *stat, purgeable_q_t queue,
                               int group, task_t target_task) {
  LCK_MTX_ASSERT(&vm_purgeable_queue_lock, LCK_MTX_ASSERT_OWNED);
...
void vm_purgeable_stats(vm_purgeable_info_t info, task_t target_task) {
  ...
  for (group = 0; group < NUM_VOLATILE_GROUPS; group++) {
    vm_purgeable_stats_helper(&(info->fifo_data[group]), queue, group,
                              target_task);
  }
  ...
  for (group = 0; group < NUM_VOLATILE_GROUPS; group++) {
    vm_purgeable_stats_helper(&(info->lifo_data[group]), queue, group,
                              target_task);
  }
...
static void vm_purgeable_account_volatile_queue(purgeable_q_t queue, int group,
                                                task_t task,
  ...
  for (object = (vm_object_t)queue_first(&queue->objq[group]);
    ...
    if (VM_OBJECT_OWNER(object) == task) {
      ...
      acnt_info->pvm_volatile_count +=
          (object->resident_page_count - object->wired_page_count);
...
kern_return_t vm_purgeable_account(task_t task, pvm_account_info_t acnt_info) {
  ...
  for (object = (vm_object_t)queue_first(nonvolatile_q);
    ...
    if (VM_OBJECT_OWNER(object) == task) {
      ...
      if (state == VM_PURGABLE_EMPTY) {
        ...
        acnt_info->pvm_nonvolatile_count +=
            (object->resident_page_count - object->wired_page_count);
  ...
  acnt_info->pvm_volatile_compressed_count =
      (acnt_info->pvm_volatile_compressed_count * PAGE_SIZE);
  acnt_info->pvm_nonvolatile_count =
      (acnt_info->pvm_nonvolatile_count * PAGE_SIZE);
  acnt_info->pvm_nonvolatile_compressed_count =
      (acnt_info->pvm_nonvolatile_compressed_count * PAGE_SIZE);

  return KERN_SUCCESS;
}
#endif /* DEVELOPMENT || DEBUG */

static uint64_t vm_purgeable_queue_purge_task_owned(purgeable_q_t queue,
                                                    int group, task_t task) {
...
uint64_t vm_purgeable_purge_task_owned(task_t task) {
  ...
  queue = &purgeable_queues[PURGEABLE_Q_TYPE_OBSOLETE];
  num_pages_purged += vm_purgeable_queue_purge_task_owned(queue, 0, task);

  queue = &purgeable_queues[PURGEABLE_Q_TYPE_FIFO];
  for (group = 0; group < NUM_VOLATILE_GROUPS; group++) {
    num_pages_purged += vm_purgeable_queue_purge_task_owned(queue, group, task);
  }
...
void vm_purgeable_nonvolatile_enqueue(vm_object_t object, task_t owner) {
  int ledger_flags;
  kern_return_t kr;
  ...
  if (owner != NULL && owner->task_objects_disowning) {
  ...
  kr = vm_object_ownership_change(
      object, object->vo_ledger_tag, /* tag unchanged */
      owner, ledger_flags, FALSE);   /* task_objq_locked */
...
void vm_purgeable_accounting(vm_object_t object, vm_purgable_t old_state) {
  ...
  owner = VM_OBJECT_OWNER(object);
  if (owner == NULL || object->purgable == VM_PURGABLE_DENY) {
    return;
  }
  ...
  if (VM_CONFIG_COMPRESSOR_IS_PRESENT && object->pager != NULL) {
    compressed_page_count = vm_compressor_pager_get_count(object->pager);
  } else {
  ...
  if (old_state == VM_PURGABLE_VOLATILE || old_state == VM_PURGABLE_EMPTY) {
    ...
    ledger_debit(owner->ledger, ledger_idx_volatile,
                 ptoa_64(resident_page_count - wired_page_count));
    /* less compressed volatile bytes in ledger */
    ledger_debit(owner->ledger, ledger_idx_volatile_compressed,
    ...
    ledger_credit(owner->ledger, ledger_idx_nonvolatile_compressed,
                  ptoa_64(compressed_page_count));
    if (do_footprint) {
      /* more footprint */
      ledger_credit(owner->ledger, task_ledgers.phys_footprint,
                    ptoa_64(resident_page_count + compressed_page_count -
                            wired_page_count));
    ...
    ledger_debit(owner->ledger, ledger_idx_nonvolatile,
                 ptoa_64(resident_page_count - wired_page_count));
    /* less compressed non-volatile bytes in ledger */
    ledger_debit(owner->ledger, ledger_idx_nonvolatile_compressed,
                 ptoa_64(compressed_page_count));
    if (do_footprint) {
      /* less footprint */
      ...
      ledger_debit(owner->ledger, ledger_idx_composite,
                   ptoa_64(resident_page_count + compressed_page_count -
                           wired_page_count));
    }

    /* more volatile bytes in ledger */
    ledger_credit(owner->ledger, ledger_idx_volatile,
                  ptoa_64(resident_page_count - wired_page_count));
    /* more compressed volatile bytes in ledger */
    ledger_credit(owner->ledger, ledger_idx_volatile_compressed,
                  ptoa_64(compressed_page_count));
  } else {
...
void vm_purgeable_nonvolatile_owner_update(task_t owner, int delta) {
...
void vm_purgeable_volatile_owner_update(task_t owner, int delta) {
  if (owner == NULL || delta == 0) {
    return;
  }
  ...
  case VM_PURGABLE_NONVOLATILE:
    if (delta > 0) {
      ...
      if (do_footprint) {
        ...
        ledger_credit(owner->ledger, ledger_idx_composite, ptoa_64(delta));
      }
    } else {
      ledger_debit(owner->ledger, ledger_idx_nonvolatile_compressed,
                   ptoa_64(-delta));
      if (do_footprint) {
        ...
        ledger_debit(owner->ledger, ledger_idx_composite, ptoa_64(-delta));
      }
    }
    break;
