...
/*
 * We apply a hard throttle to the demand zero rate of tasks that we believe are
 * running out of control which kicks in when swap space runs out.  64-bit
 * programs have massive address spaces and can leak enormous amounts of memory
 * if they're buggy and can run the system completely out of swap space.  If
 * this happens, we impose a hard throttle on them to prevent them from taking
 * the last bit of memory left.  This helps keep the UI active so that the user
 * has a chance to kill the offending task before the system completely hangs.
 *
 * The hard throttle is only applied when the system is nearly completely out of
 * swap space and is only applied to tasks that appear to be bloated.  When swap
 * runs out, any task using more than vm_hard_throttle_threshold will be
 * throttled.  The throttling is done by giving the thread that's trying to
 * demand zero a page a delay of HARD_THROTTLE_DELAY microseconds before being
 * allowed to try the page fault again.
 */
...
OS_ALWAYS_INLINE
bool vm_fault_get_disabled(void) {
  thread_t t = current_thread();
...
static kern_return_t vm_fault_wire_fast(vm_map_t map, vm_map_offset_t va,
                                        vm_prot_t prot, vm_tag_t wire_tag,
                                        vm_map_entry_t entry, pmap_t pmap,
                                        vm_map_offset_t pmap_addr,
                                        ppnum_t *physpage_p);

static kern_return_t vm_fault_internal(vm_map_t map, vm_map_offset_t vaddr,
                                       vm_prot_t caller_prot, vm_tag_t wire_tag,
                                       pmap_t pmap, vm_map_offset_t pmap_addr,
                                       ppnum_t *physpage_p,
                                       vm_object_fault_info_t fault_info);

static void vm_fault_copy_cleanup(vm_page_t page, vm_page_t top_page);

static void vm_fault_copy_dst_cleanup(vm_page_t page);

#if VM_FAULT_CLASSIFY
extern void vm_fault_classify(vm_object_t object, vm_object_offset_t offset,
...
#define VMRTF_NUM_RECORDS_DEFAULT                                              \
  (VMRTF_DEFAULT_BUFSIZE / sizeof(vm_rtfault_record_t))
TUNABLE(int, vmrtf_num_records, "vm_rtfault_records",
...
LCK_SPIN_DECLARE_ATTR(vm_rtfr_slock, &vm_page_lck_grp_bucket,
                      &vm_page_lck_attr);
 ...
 */
__startup_func void vm_fault_init(void) {
  int i, vm_compressor_temp;
  boolean_t need_default_val = TRUE;
  /*
   * Choose a value for the hard throttle threshold based on the amount of ram.
   * The threshold is computed as a percentage of available memory, and the
   * percentage used is scaled inversely with the amount of memory.  The
   * percentage runs between 10% and 35%.  We use 35% for small memory systems
   * and reduce the value down to 10% for very large memory configurations. This
   * helps give us a definition of a memory hog that makes more sense relative
   * to the amount of ram in the machine. The formula here simply uses the
   * number of gigabytes of ram to adjust the percentage.
   */

  vm_hard_throttle_threshold =
      sane_size * (35 - MIN((int)(sane_size / (1024 * 1024 * 1024)), 25)) / 100;

  /*
   * Configure compressed pager behavior. A boot arg takes precedence over a
   * device tree entry.
   */

  if (PE_parse_boot_argn("vm_compressor", &vm_compressor_temp,
  ...
  if (need_default_val) {
    if (osenvironment_is_diagnostics()) {
      printf("osenvironment == \"diagnostics\". Setting \"vm_compressor_mode\" "
             "to in-core compressor only\n");
  ...
  if (need_default_val) {
    ...
    PE_get_default("kern.vm_compressor", &vm_compressor_mode,
...
#if DEBUG || DEVELOPMENT
  (void)PE_parse_boot_argn("text_corruption_panic", &vmtc_panic_instead,
  ...
  PE_parse_boot_argn("panic_object_not_alive", &panic_object_not_alive,
                     sizeof(panic_object_not_alive));
#endif /* DEBUG || DEVELOPMENT */
}

__startup_func static void vm_rtfault_record_init(void) {
  size_t size;

  vmrtf_num_records = MAX(vmrtf_num_records, 1);
  size = vmrtf_num_records * sizeof(vm_rtfault_record_t);
  vmrtfrs.vm_rtf_records = zalloc_permanent_tag(
      size, ZALIGN(vm_rtfault_record_t), VM_KERN_MEMORY_DIAG);
 ...
 */
void vm_fault_cleanup(vm_object_t object, vm_page_t top_page) {
  thread_pri_floor_t token = {.thread = THREAD_NULL};
  if (top_page != VM_PAGE_NULL && top_page->vmp_busy) {
    /*
     ...
     *
     * TODO: Register a page-worker token when busying the top-level page
     * instead (rdar://154313767)
     */
...
TUNABLE(uint32_t, vm_page_deactivate_behind_min_resident_ratio,
        "vm_deactivate_behind_min_resident_ratio", 3);
/*
...
static void vm_fault_is_sequential(vm_object_t object,
  ...
  if (sequential != orig_sequential) {
    if (!OSCompareAndSwap(orig_sequential, sequential,
                          (UInt32 *)&object->sequential)) {
      /*
 ...
 */
static uint32_t vm_fault_deactivate_behind(vm_object_t object,
                                           vm_object_offset_t offset,
                                           vm_behavior_t behavior) {
...
#if TRACEFAULTPAGE
  dbgTrace(0xBEEF0018, (unsigned int)object,
           (unsigned int)vm_fault_deactivate_behind); /* (TEST/DEBUG) */
#endif
  if (is_kernel_object(object) || !vm_page_deactivate_behind ||
  ...
  default: {
    ...
    if ((uint64_t)sequential_run >= behind &&
        (sequential_run % (VM_DEFAULT_DEACTIVATE_BEHIND_CLUSTER * PAGE_SIZE)) ==
            0) {
      /*
  ...
  for (unsigned n = 0; n < max_pages_in_run; n++) {
    ...
    if (m && !m->vmp_laundry && !m->vmp_busy && !m->vmp_no_cache &&
       ...
       * futher references happen on the page after that remote TLB flushes
       * we'll see a clean, non-referenced page when it eventually gets pulled
       * out of the inactive queue by pageout_scan, which is just fine since the
       * last reference would have happened quite far in the past (TLB caches
       * don't hang around for very long), and of course could just as easily
...
#if TRACEFAULTPAGE
      dbgTrace(0xBEEF0019, (unsigned int)object,
               (unsigned int)m); /* (TEST/DEBUG) */
#endif
    }
...
static int vm_page_throttled(boolean_t page_kept) {
  ...
  if ((vm_page_free_count < vm_page_throttle_limit ||
       (VM_CONFIG_COMPRESSOR_IS_PRESENT && SWAPPER_NEEDS_TO_UNTHROTTLE())) &&
    ...
    if (elapsed_sec <= VM_PAGE_CREATION_THROTTLE_PERIOD_SECS ||
        (thread->t_page_creation_count / elapsed_sec) >=
            VM_PAGE_CREATION_THROTTLE_RATE_PER_SEC) {
      if (elapsed_sec >= (3 * VM_PAGE_CREATION_THROTTLE_PERIOD_SECS)) {
        ...
        thread->t_page_creation_count =
            VM_PAGE_CREATION_THROTTLE_RATE_PER_SEC *
            (VM_PAGE_CREATION_THROTTLE_PERIOD_SECS - 1);
      }
...
__VM_FAULT_THROTTLE_FOR_PAGEOUT_SCAN__(int throttle_delay) {
 ...
 * object == m->vmp_object
 */
static vm_fault_return_t vm_fault_check(vm_object_t object, vm_page_t m,
                                        vm_page_t first_m,
  ...
  if (page_throttle == TRUE) {
    if ((throttle_delay = vm_page_throttled(FALSE))) {
      ...
      VM_DEBUG_EVENT(vmf_check_zfdelay, DBG_VM_FAULT_CHECK_ZFDELAY,
                     DBG_FUNC_NONE, throttle_delay, 0, 0, 0);
 ...
 */
static void vm_fault_cs_clear(vm_page_t m) {
 ...
 */
static void vm_fault_enqueue_throttled_locked(vm_page_t m) {
 ...
 */
static int vm_fault_zero_page(vm_page_t m, boolean_t no_zero_fill) {
...
vm_fault_page_release_page(vm_page_t m, /* Page to release */
...
#define MUST_ASK_PAGER(o, f, s)                                                \
  ((s = vm_object_compressor_pager_state_get((o), (f))) !=                     \
...
#define PAGED_OUT(o, f)                                                        \
...
#if TRACEFAULTPAGE
  dbgTrace(0xBEEF0002, (unsigned int)first_object,
           (unsigned int)first_offset); /* (TEST/DEBUG) */
#endif
  ...
  while (TRUE) {
    ...
    if (!object->alive) {
       ...
       */
#if DEVELOPMENT || DEBUG
      printf("FBDP rdar://93769854 %s:%d object %p internal %d pager %p (%s) "
             "copy %p shadow %p alive %d terminating %d named %d ref %d "
             "shadow_severed %d\n",
             __FUNCTION__, __LINE__, object, object->internal, object->pager,
             object->pager
                 ? object->pager->mo_pager_ops->memory_object_pager_name
                 : "?",
             object->vo_copy, object->shadow, object->alive,
             object->terminating, object->named,
             os_ref_get_count_raw(&object->ref_count), object->shadow_severed);
      if (panic_object_not_alive) {
        panic("FBDP rdar://93769854 %s:%d object %p internal %d pager %p (%s) "
              "copy %p shadow %p alive %d terminating %d named %d ref %d "
              "shadow_severed %d\n",
              __FUNCTION__, __LINE__, object, object->internal, object->pager,
              object->pager
    ...
    if (object->blocked_access) {
      ...
      while (object->blocked_access) {
        vm_object_sleep(object, VM_OBJECT_EVENT_UNBLOCKED, THREAD_UNINT,
                        LCK_SLEEP_EXCLUSIVE);
      }
...
#if TRACEFAULTPAGE
    dbgTrace(0xBEEF0004, (unsigned int)m,
             (unsigned int)object); /* (TEST/DEBUG) */
#endif
    if (m != VM_PAGE_NULL) {
      if (m->vmp_busy) {
        /*
         ...
         */
#if TRACEFAULTPAGE
        dbgTrace(0xBEEF0005, (unsigned int)m,
        ...
        wait_result =
            vm_page_sleep(object, m, interruptible, LCK_SLEEP_DEFAULT);
      ...
      if (m->vmp_error) {
         ...
         */
#if TRACEFAULTPAGE
        dbgTrace(0xBEEF0006, (unsigned int)m,
                 (unsigned int)error_code); /* (TEST/DEBUG) */
#endif
        if (error_code) {
      ...
      if (m->vmp_restart) {
         ...
         */
#if TRACEFAULTPAGE
        dbgTrace(0xBEEF0007, (unsigned int)m,
                 (unsigned int)0); /* (TEST/DEBUG) */
#endif
        VM_PAGE_FREE(m);
      ...
      if (m->vmp_absent) {
         ...
         */
#if TRACEFAULTPAGE
        dbgTrace(0xBEEF0008, (unsigned int)m,
                 (unsigned int)object->shadow); /* (TEST/DEBUG) */
#endif
      ...
      if ((m->vmp_cleaning) &&
         ...
         */
#if TRACEFAULTPAGE
        dbgTrace(0xBEEF0009, (unsigned int)m,
                 (unsigned int)offset); /* (TEST/DEBUG) */
#endif
        /*
        ...
        if (m != VM_PAGE_NULL && m->vmp_cleaning) {
          wait_result =
              vm_page_sleep(object, m, interruptible, LCK_SLEEP_UNLOCK);
    ...
    if (must_be_resident) {
      if (fault_type == VM_PROT_NONE && is_kernel_object(object)) {
        /*
    ...
    look_for_page = (object->pager_created &&
                     (MUST_ASK_PAGER(object, offset, external_state) == TRUE));

#if TRACEFAULTPAGE
    dbgTrace(0xBEEF000C, (unsigned int)look_for_page,
             (unsigned int)object); /* (TEST/DEBUG) */
#endif
    if (!look_for_page && object == first_object && !object->phys_contiguous) {
...
#if TRACEFAULTPAGE
      dbgTrace(0xBEEF000D, (unsigned int)m,
               (unsigned int)object); /* (TEST/DEBUG) */
#endif
      if (m == VM_PAGE_NULL) {
        vm_fault_cleanup(object, first_m);
      ...
      if (fault_info && fault_info->batch_pmap_op == TRUE) {
        vm_page_insert_internal(m, object, vm_object_trunc_page(offset),
                                VM_KERN_MEMORY_NONE, FALSE, TRUE, TRUE, FALSE,
                                NULL);
      } else {
    ...
    if (look_for_page) {
      ...
      if (!object->pager_ready) {
#if TRACEFAULTPAGE
        dbgTrace(0xBEEF000E, (unsigned int)0,
                 (unsigned int)0); /* (TEST/DEBUG) */
#endif
        if (m != VM_PAGE_NULL) {
          VM_PAGE_FREE(m);
        ...
        if (!object->pager_ready) {
          wait_result = vm_object_sleep(object, VM_OBJECT_EVENT_PAGER_READY,
                                        interruptible, LCK_SLEEP_UNLOCK);
          vm_object_deallocate(object);
      ...
      if (!object->internal && !object->phys_contiguous &&
          object->paging_in_progress > vm_object_pagein_throttle) {
        /*
         ...
         */
#if TRACEFAULTPAGE
        dbgTrace(0xBEEF0010, (unsigned int)m,
                 (unsigned int)0); /* (TEST/DEBUG) */
#endif
        if (m != VM_PAGE_NULL) {
          VM_PAGE_FREE(m);
      ...
      if (object->internal) {
        ...
        if (m == VM_PAGE_NULL) {
          ...
          if (fault_info && fault_info->batch_pmap_op == TRUE) {
            vm_page_insert_internal(m, object, vm_object_trunc_page(offset),
                                    VM_KERN_MEMORY_NONE, FALSE, TRUE, TRUE,
                                    FALSE, NULL);
          } else {
            vm_page_insert(m, object, vm_object_trunc_page(offset));
          }
        ...
        vm_object_unlock(object);
        rc = vm_compressor_pager_get(pager, offset + object->paging_offset,
                                     VM_PAGE_GET_PHYS_PAGE(m), &my_fault_type,
                                     c_flags, &compressed_count_delta);
        ...
        if (type_of_fault == NULL) {
          ...
          if (my_fault_type == DBG_COMPRESSOR_FAULT) {
            if ((throttle_delay = vm_page_throttled(TRUE))) {
              VM_DEBUG_EVENT(vmf_compressordelay, DBG_VM_FAULT_COMPRESSORDELAY,
                             DBG_FUNC_NONE, throttle_delay, 0, 1, 0);
              __VM_FAULT_THROTTLE_FOR_PAGEOUT_SCAN__(throttle_delay);
            }
          }
        }
        ...
        case KERN_SUCCESS:
          m->vmp_absent = FALSE;
          m->vmp_dirty = TRUE;
          if (!HAS_DEFAULT_CACHEABILITY(object->wimg_bits & VM_WIMG_MASK)) {
            /*
          ...
          if (((object->purgable != VM_PURGABLE_DENY) ||
            ...
            if (compressed_count_delta) {
              vm_object_owner_compressed_update(object, -1);
            }
          }
...
#if TRACEFAULTPAGE
      dbgTrace(0xBEEF0012, (unsigned int)object,
               (unsigned int)0); /* (TEST/DEBUG) */
#endif
      ...
      if (pager == MEMORY_OBJECT_NULL) {
        ...
        static const enum vm_subsys_error_codes
            object_destroy_errors[VM_OBJECT_DESTROY_MAX + 1] = {
                [VM_OBJECT_DESTROY_UNKNOWN_REASON] =
                    KDBG_TRIAGE_VM_OBJECT_NO_PAGER,
                [VM_OBJECT_DESTROY_UNMOUNT] =
                    KDBG_TRIAGE_VM_OBJECT_NO_PAGER_UNMOUNT,
                [VM_OBJECT_DESTROY_FORCED_UNMOUNT] =
                    KDBG_TRIAGE_VM_OBJECT_NO_PAGER_FORCED_UNMOUNT,
                [VM_OBJECT_DESTROY_UNGRAFT] =
                    KDBG_TRIAGE_VM_OBJECT_NO_PAGER_UNGRAFT,
                [VM_OBJECT_DESTROY_PAGER] =
                    KDBG_TRIAGE_VM_OBJECT_NO_PAGER_DEALLOC_PAGER,
                [VM_OBJECT_DESTROY_RECLAIM] =
                    KDBG_TRIAGE_VM_OBJECT_NO_PAGER_RECLAIM,
            };
        enum vm_subsys_error_codes kdbg_code =
            object_destroy_errors[(vm_object_destroy_reason_t)
                                      object->no_pager_reason];
        ktriage_record(thread_tid(current_thread()),
                       KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_VM,
                                           KDBG_TRIAGE_RESERVED, kdbg_code),
                       0 /* arg */);
        return VM_FAULT_MEMORY_ERROR;
      }
      ...
      if (object->object_is_shared_cache ||
          pager->mo_pager_ops == &dyld_pager_ops) {
        token = thread_priority_floor_start();
        /*
      ...
      rc = memory_object_data_request(
          pager, vm_object_trunc_page(offset) + object->paging_offset,
          PAGE_SIZE, access_required | wants_copy_flag,
...
#if TRACEFAULTPAGE
      dbgTrace(0xBEEF0013, (unsigned int)object,
               (unsigned int)rc); /* (TEST/DEBUG) */
#endif
      vm_object_lock(object);
    ...
    data_requested:
      ...
      if ((interruptible != THREAD_UNINT) &&
          (current_thread()->sched_flags & TH_SFLAG_ABORT)) {
        vm_fault_cleanup(object, first_m);
        thread_interrupt_level(interruptible_state);
  ...
  dont_look_for_page:
     ...
     */
#if TRACEFAULTPAGE
    dbgTrace(0xBEEF0014, (unsigned int)object,
             (unsigned int)m); /* (TEST/DEBUG) */
#endif
    if (object == first_object) {
      first_m = m;
...
#if TRACEFAULTPAGE
  dbgTrace(0xBEEF0015, (unsigned int)object,
           (unsigned int)m); /* (TEST/DEBUG) */
#endif
#if EXTRA_ASSERTIONS
  assert(m->vmp_busy && !m->vmp_absent);
  assert((first_m == VM_PAGE_NULL) ||
         (first_m->vmp_busy && !first_m->vmp_absent && !first_m->vmp_active &&
          !first_m->vmp_inactive && !first_m->vmp_secluded));
  ...
  if (object != first_object) {
#if TRACEFAULTPAGE
    dbgTrace(0xBEEF0016, (unsigned int)object,
             (unsigned int)fault_type); /* (TEST/DEBUG) */
#endif
  ...
  while ((copy_object = first_object->vo_copy) != VM_OBJECT_NULL) {
...
#if TRACEFAULTPAGE
    dbgTrace(0xBEEF0017, (unsigned int)copy_object,
             (unsigned int)fault_type); /* (TEST/DEBUG) */
#endif
    /*
    ...
    if (copy_object->vo_size <= copy_offset) {
       ...
       */
      ;
    } else if ((copy_m = vm_page_lookup(copy_object, copy_offset)) !=
               VM_PAGE_NULL) {
      /*
      ...
      if (copy_m->vmp_busy) {
        ...
        if (copy_m != VM_PAGE_NULL && copy_m->vmp_busy) {
          wait_result = vm_page_sleep(copy_object, copy_m, interruptible,
                                      LCK_SLEEP_UNLOCK);
      ...
      if ((!copy_object->pager_ready) ||
          vm_object_compressor_pager_state_get(copy_object, copy_offset) ==
              VM_EXTERNAL_STATE_ABSENT) {
        vm_page_lockspin_queues();
        assert(!m->vmp_cleaning);
     ...
     */
    vm_object_lock_assert_exclusive(copy_object);
    os_ref_release_live_locked_raw(&copy_object->ref_count, &vm_object_refgrp);
  ...
  *top_page = first_m;
  ...
  if (m != VM_PAGE_NULL) {
    ...
    if (my_fault == DBG_PAGEIN_FAULT) {
      ...
      vm_fault_is_sequential(object, offset, fault_info->behavior);
      vm_fault_deactivate_behind(object, offset, fault_info->behavior);
    } else if (my_fault == DBG_COMPRESSOR_FAULT ||
               my_fault == DBG_COMPRESSOR_SWAPIN_FAULT) {
 ...
 */
static bool vm_fault_cs_need_validation(pmap_t pmap, vm_page_t page,
  ...
  if (fault_page_size == PAGE_SIZE) {
    ...
    if (page->vmp_cs_validated == VMP_CS_ALL_TRUE && !page->vmp_wpmapped) {
...
static bool vm_fault_cs_page_immutable(vm_page_t m,
                                       vm_map_size_t fault_page_size,
                                       vm_map_offset_t fault_phys_offset,
...
static bool vm_fault_cs_page_nx(vm_page_t m, vm_map_size_t fault_page_size,
                                vm_map_offset_t fault_phys_offset) {
 ...
 */
static kern_return_t vm_fault_cs_check_violation(
    bool cs_bypass, vm_object_t object, vm_page_t m, pmap_t pmap,
    vm_prot_t prot, vm_prot_t caller_prot, vm_map_size_t fault_page_size,
    vm_map_offset_t fault_phys_offset, vm_object_fault_info_t fault_info,
    bool map_is_switched, bool map_is_switch_protected, bool *cs_violation) {
#if !CODE_SIGNING_MONITOR
...
#pragma unused(fault_info)
  ...
  if (!cs_bypass && vm_fault_cs_need_validation(
                        pmap, m, object, fault_page_size, fault_phys_offset)) {
    vm_object_lock_assert_exclusive(object);
...
#if CODE_SIGNING_MONITOR
    if (fault_info->csm_associated && csm_enabled() &&
      ...
      /*
       * When we have a code signing monitor, the monitor will evaluate the code
       * signature for any executable page mapping. No need for the VM to also
       * validate the page. In the code signing monitor we trust :)
       */
      vm_cs_defer_to_csm++;
    } else {
  ...
  if (cs_enforcement_enabled && map_is_switched && map_is_switch_protected &&
  ...
  if (cs_bypass) {
    ...
    *cs_violation = FALSE;
  } else if (vm_fault_cs_page_immutable(m, fault_page_size, fault_phys_offset,
                                        prot) &&
...
#if CODE_SIGNING_MONITOR
             /*
              * Executable pages will be validated by the code signing monitor.
              * If the code signing monitor is turned off, then this is a
              * code-signing violation.
              */
             && !csm_enabled()
#endif /* CODE_SIGNING_MONITOR */
  ) {
...
/*
 * Handles a code signing violation by either rejecting the page or forcing a
 * disconnect.
 * @param must_disconnect This value will be set to true if the caller must
 * disconnect this page.
 * @return If this function does not return KERN_SUCCESS, the caller must abort
 * the page fault.
 */
static kern_return_t vm_fault_cs_handle_violation(
    vm_object_t object, vm_page_t m, pmap_t pmap, vm_prot_t prot,
    vm_map_offset_t vaddr, vm_map_size_t fault_page_size,
    vm_map_offset_t fault_phys_offset, bool map_is_switched,
    bool map_is_switch_protected, bool *must_disconnect) {
#if !MACH_ASSERT
#pragma unused(pmap)
#pragma unused(map_is_switch_protected)
  ...
  if (map_is_switched) {
    ...
    if (cs_debug > 5) {
      printf("vm_fault: signed: %s validate: %s tainted: %s wpmapped: %s prot: "
             "0x%x\n",
             object->code_signed ? "yes" : "no",
             VMP_CS_VALIDATED(m, fault_page_size, fault_phys_offset) ? "yes"
                                                                     : "no",
             VMP_CS_TAINTED(m, fault_page_size, fault_phys_offset) ? "yes"
                                                                   : "no",
             m->vmp_wpmapped ? "yes" : "no", (int)prot);
    }
...
#define __PATH_MAX 1024
    ...
    for (shadow = file_object->shadow, shadow_depth = 0;
         shadow != VM_OBJECT_NULL;
         shadow = file_object->shadow, shadow_depth++) {
    ...
    if (file_object->pager != NULL) {
      ...
      if (pathname) {
        ...
        if (vnode_pager_get_object_name(file_object->pager, pathname,
          ...
          vnode_pager_get_object_mtime(file_object->pager, &mtime, &cs_mtime);
        } else {
    ...
    printf(
        ...
        pid, procname, (addr64_t)vaddr, file_offset,
        (pathname ? pathname : "<nil>"), (truncated_path ? "/.../" : ""),
        (truncated_path ? filename : ""), cs_mtime.tv_sec, cs_mtime.tv_nsec,
        ((cs_mtime.tv_sec == mtime.tv_sec && cs_mtime.tv_nsec == mtime.tv_nsec)
             ? "=="
             : "!="),
        mtime.tv_sec, mtime.tv_nsec, object->code_signed,
        VMP_CS_VALIDATED(m, fault_page_size, fault_phys_offset),
        VMP_CS_TAINTED(m, fault_page_size, fault_phys_offset),
        VMP_CS_NX(m, fault_page_size, fault_phys_offset), m->vmp_wpmapped,
        m->vmp_dirty, shadow_depth);

    /*
     * We currently only generate an exit reason if cs_invalid_page directly
     * killed a process. If cs_invalid_page did not kill the process (more the
     * case on desktop), vm_fault_enter will not satisfy the fault and whether
     * the process dies is dependent on whether there is a signal handler
     * registered for SIGSEGV and how that handler will deal with the
     * segmentation fault.
     */
    if (cs_killed) {
      KDBG(BSDDBG_CODE(DBG_BSD_PROC, BSD_PROC_EXITREASON_CREATE) |
               DBG_FUNC_NONE,
           pid, OS_REASON_CODESIGNING, CODESIGNING_EXIT_REASON_INVALID_PAGE);

      codesigning_exit_reason = os_reason_create(
          OS_REASON_CODESIGNING, CODESIGNING_EXIT_REASON_INVALID_PAGE);
      if (codesigning_exit_reason == NULL) {
        ...
        uint32_t reason_buffer_size_estimate =
            kcdata_estimate_required_buffer_size(1, sizeof(*ceri));

        if (os_reason_alloc_buffer_noblock(codesigning_exit_reason,
                                           reason_buffer_size_estimate)) {
          printf("vm_fault_enter: failed to allocate buffer for codesigning "
                 "exit reason\n");
        } else {
          if (KERN_SUCCESS == kcdata_get_memory_addr(
                                  &codesigning_exit_reason->osr_kcd_descriptor,
                                  EXIT_REASON_CODESIGNING_INFO, sizeof(*ceri),
                                  &data_addr)) {
            ...
            if (pathname) {
              strncpy((char *)&ceri->ceri_pathname, pathname,
                      sizeof(ceri->ceri_pathname));
            } else {
              ceri->ceri_pathname[0] = '\0';
            }
            if (filename) {
              strncpy((char *)&ceri->ceri_filename, filename,
                      sizeof(ceri->ceri_filename));
            } else {
            ...
            ceri->ceri_page_codesig_validated =
                VMP_CS_VALIDATED(m, fault_page_size, fault_phys_offset);
            ceri->ceri_page_codesig_tainted =
                VMP_CS_TAINTED(m, fault_page_size, fault_phys_offset);
            ceri->ceri_page_codesig_nx =
                VMP_CS_NX(m, fault_page_size, fault_phys_offset);
...
#if DEBUG || DEVELOPMENT
            panic("vm_fault_enter: failed to allocate kcdata for codesigning "
                  "exit reason");
#else
            printf("vm_fault_enter: failed to allocate kcdata for codesigning "
                   "exit reason\n");
#endif /* DEBUG || DEVELOPMENT */
            /* Free the buffer */
    ...
    if (panic_on_cs_killed && object->object_is_shared_cache) {
      char *tainted_contents;
      vm_map_offset_t src_vaddr;
      src_vaddr = (vm_map_offset_t)phystokv(
          (pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(m) << PAGE_SHIFT);
      tainted_contents = kalloc_data(PAGE_SIZE, Z_WAITOK);
      bcopy((const char *)src_vaddr, tainted_contents, PAGE_SIZE);
      printf("CODE SIGNING: tainted page %p phys 0x%x phystokv 0x%llx copied "
             "to %p\n",
             m, VM_PAGE_GET_PHYS_PAGE(m), (uint64_t)src_vaddr,
             tainted_contents);
      panic("CODE SIGNING: process %d[%s]: "
            ...
            "(signed:%d validated:%d tainted:%d nx:%d"
            "wpmapped:%d dirty:%d depth:%d)\n",
            pid, procname, VM_PAGE_GET_PHYS_PAGE(m), (addr64_t)vaddr,
            file_offset, (pathname ? pathname : "<nil>"),
            (truncated_path ? "/.../" : ""), (truncated_path ? filename : ""),
...
vm_fault_validate_cs(bool cs_bypass, vm_object_t object, vm_page_t m,
                     pmap_t pmap, vm_map_offset_t vaddr, vm_prot_t prot,
                     vm_prot_t caller_prot, vm_map_size_t fault_page_size,
                     vm_map_offset_t fault_phys_offset,
                     vm_object_fault_info_t fault_info, bool *must_disconnect) {
  ...
  kr = vm_fault_cs_check_violation(
      cs_bypass, object, m, pmap, prot, caller_prot, fault_page_size,
      fault_phys_offset, fault_info, map_is_switched, map_is_switch_protected,
      &cs_violation);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
  if (cs_violation) {
    kr = vm_fault_cs_handle_violation(
        object, m, pmap, prot, vaddr, fault_page_size, fault_phys_offset,
 ...
 */
static void vm_fault_enqueue_page(vm_object_t object, vm_page_t m, bool wired,
                                  bool change_wiring, vm_tag_t wire_tag,
                                  bool no_cache, int *type_of_fault,
                                  kern_return_t kr) {
  ...
  if (m->vmp_q_state == VM_PAGE_USED_BY_COMPRESSOR) {
    ...
    if (kr != KERN_SUCCESS) {
      ...
      /* we keep the page queues lock, if we need it later */
               ...
               !VM_PAGE_WIRED(m)) {
      if (vm_page_local_q && (*type_of_fault == DBG_COW_FAULT ||
        ...
        if (!VM_PAGE_WIRED(m)) {
          ...
          if (!VM_PAGE_ACTIVE_OR_INACTIVE(m) || no_cache) {
            /*
            ...
            if (no_cache && (!previously_pmapped || m->vmp_no_cache)) {
              m->vmp_no_cache = TRUE;
...
/*
 * Sets the pmmpped, xpmapped, and wpmapped bits on the vm_page_t and updates
 * accounting.
 * @return true if the page needs to be sync'ed via pmap_sync-page_data_physo
 * before being inserted into the pmap.
 */
static bool vm_fault_enter_set_mapped(vm_object_t object, vm_page_t m,
  ...
  if (fault_type & VM_PROT_WRITE) {
    if (m->vmp_wpmapped == FALSE) {
      ...
      if (!object->internal && object->pager) {
        task_update_logical_writes(current_task(), PAGE_SIZE,
                                   TASK_WRITE_DEFERRED,
                                   vnode_pager_lookup_vnode(object->pager));
      }
      m->vmp_wpmapped = TRUE;
    }
  }
...
static inline kern_return_t vm_fault_pmap_validate_page(
    pmap_t pmap __unused, vm_page_t m __unused, vm_map_offset_t vaddr __unused,
    vm_prot_t prot __unused, vm_object_fault_info_t fault_info __unused,
    bool *page_sleep_needed) {
  assert(page_sleep_needed != NULL);
  *page_sleep_needed = false;
#if CONFIG_SPTM
  /*
   * Reject the executable or debug mapping if the page is already wired for
   * I/O.  The SPTM's security model doesn't allow us to reliably use executable
   * pages for I/O due to both CS integrity protections and the possibility that
   * the pages may be dynamically retyped while wired for I/O. This check is
   * required to happen under the VM object lock in order to synchronize with
   * the complementary check on the I/O wiring path in
   * vm_page_do_delayed_work().
   */
  if (__improbable((m->vmp_cleaning || m->vmp_iopl_wired) &&
                   pmap_will_retype(pmap, vaddr, VM_PAGE_GET_PHYS_PAGE(m), prot,
                                    fault_info->pmap_options |
                                        ((fault_info->fi_xnu_user_debug &&
                                          !VM_PAGE_OBJECT(m)->code_signed)
                                             ? PMAP_OPTIONS_XNU_USER_DEBUG
                                             : 0),
 ...
 */
kern_return_t
pmap_enter_object_options_check(pmap_t pmap, vm_map_address_t virtual_address,
                                vm_map_offset_t fault_phys_offset,
                                vm_object_t obj, ppnum_t pn,
                                vm_prot_t protection, vm_prot_t fault_type,
  ...
  return pmap_enter_options_addr(
      pmap, virtual_address, physical_address, protection, fault_type, flags,
      wired, options | extra_options, NULL, PMAP_MAPPING_TYPE_INFER);
}

kern_return_t pmap_enter_options_check(pmap_t pmap,
  ...
  return pmap_enter_object_options_check(
      pmap, virtual_address, fault_phys_offset, obj,
      VM_PAGE_GET_PHYS_PAGE(page), protection, fault_type, wired, options);
}

kern_return_t pmap_enter_check(pmap_t pmap, vm_map_address_t virtual_address,
                               vm_page_t page, vm_prot_t protection,
                               vm_prot_t fault_type, boolean_t wired) {
  return pmap_enter_options_check(pmap, virtual_address,
                                  0 /* fault_phys_offset */, page, protection,
                                  fault_type, wired, 0 /* options */);
}

/*
 * Try to enter the given page into the pmap.
 * Will retry without execute permission if the code signing monitor is enabled
 * and we encounter a codesigning failure on a non-execute fault.
 */
static kern_return_t vm_fault_attempt_pmap_enter(
    pmap_t pmap, vm_map_offset_t vaddr, vm_map_size_t fault_page_size,
    vm_map_offset_t fault_phys_offset, vm_page_t m, vm_prot_t *prot,
    vm_prot_t caller_prot, vm_prot_t fault_type, bool wired, int pmap_options) {
#if !CODE_SIGNING_MONITOR
#pragma unused(caller_prot)
#endif /* !CODE_SIGNING_MONITOR */
  ...
  if (fault_page_size != PAGE_SIZE) {
    DEBUG4K_FAULT(
        "pmap %p va 0x%llx pa 0x%llx (0x%llx+0x%llx) prot 0x%x fault_type "
        "0x%x\n",
        pmap, (uint64_t)vaddr,
        (uint64_t)((((pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(m)) << PAGE_SHIFT) +
                   fault_phys_offset),
        (uint64_t)(((pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(m)) << PAGE_SHIFT),
        (uint64_t)fault_phys_offset, *prot, fault_type);
    assertf((!(fault_phys_offset & FOURK_PAGE_MASK) &&
             fault_phys_offset < PAGE_SIZE),
            "0x%llx\n", (uint64_t)fault_phys_offset);
  } else {
    assertf(fault_phys_offset == 0, "0x%llx\n", (uint64_t)fault_phys_offset);
  }

  kr = pmap_enter_options_check(pmap, vaddr, fault_phys_offset, m, *prot,
                                fault_type, wired, pmap_options);
   ...
   */
  if (kr == KERN_CODESIGN_ERROR && csm_enabled() && (*prot & VM_PROT_EXECUTE) &&
 ...
 *
 * @param need_retry if not null, avoid making a (potentially) blocking call
 * into the pmap layer. When such a call would be necessary, return true in this
 * boolean instead.
 */
static kern_return_t vm_fault_pmap_enter(
    pmap_t pmap, vm_map_offset_t vaddr, vm_map_size_t fault_page_size,
    vm_map_offset_t fault_phys_offset, vm_page_t m, vm_prot_t *prot,
    vm_prot_t caller_prot, vm_prot_t fault_type, bool wired, int pmap_options,
  ...
  kr = vm_fault_attempt_pmap_enter(pmap, vaddr, fault_page_size,
                                   fault_phys_offset, m, prot, caller_prot,
                                   fault_type, wired, pmap_options);
 ...
 * boolean instead.
 */
static kern_return_t vm_fault_pmap_enter_with_object_lock(
    vm_object_t object, pmap_t pmap, vm_map_offset_t vaddr,
    vm_map_size_t fault_page_size, vm_map_offset_t fault_phys_offset,
    vm_page_t m, vm_prot_t *prot, vm_prot_t caller_prot, vm_prot_t fault_type,
    bool wired, int pmap_options, boolean_t *need_retry,
   ...
   */
  kr = vm_fault_attempt_pmap_enter(
      pmap, vaddr, fault_page_size, fault_phys_offset, m, prot, caller_prot,
      fault_type, wired, pmap_options | PMAP_OPTIONS_NOWAIT);
#if __x86_64__
  if (kr == KERN_INVALID_ARGUMENT && pmap == PMAP_NULL && wired) {
    /*
 ...
 */
static kern_return_t
vm_fault_enter_prepare(vm_page_t m, pmap_t pmap, vm_map_offset_t vaddr,
                       vm_prot_t *prot, vm_prot_t caller_prot,
                       vm_map_size_t fault_page_size,
                       vm_map_offset_t fault_phys_offset, vm_prot_t fault_type,
                       vm_object_fault_info_t fault_info, int *type_of_fault,
                       bool *page_needs_data_sync, bool *page_needs_sleep) {
  kern_return_t kr;
  ...
  kr = vm_fault_pmap_validate_page(pmap, m, vaddr, *prot, fault_info,
                                   page_needs_sleep);
  if (__improbable((kr != KERN_SUCCESS) || *page_needs_sleep)) {
    return kr;
  }
  kr = vm_fault_validate_cs(cs_bypass, object, m, pmap, vaddr, *prot,
                            caller_prot, fault_page_size, fault_phys_offset,
                            fault_info, &is_tainted);
  if (kr == KERN_SUCCESS) {
    /*
     * We either have a good page, or a tainted page that has been accepted by
     * the process. In both cases the page will be entered into the pmap.
     */
    *page_needs_data_sync =
        vm_fault_enter_set_mapped(object, m, *prot, fault_type);
    if ((fault_type & VM_PROT_WRITE) && is_tainted) {
      ...
      if (!cs_bypass) {
        if (pmap_has_prot_policy(pmap,
           ...
           */
          panic("%s: pmap %p vaddr 0x%llx prot 0x%x options 0x%x", __FUNCTION__,
                pmap, (uint64_t)vaddr, *prot, fault_info->pmap_options);
        }
        *prot &= ~VM_PROT_EXECUTE;
      }
    }
 ...
 */
kern_return_t
vm_fault_enter(vm_page_t m, pmap_t pmap, vm_map_offset_t vaddr,
               vm_map_size_t fault_page_size, vm_map_offset_t fault_phys_offset,
               vm_prot_t prot, vm_prot_t caller_prot, boolean_t wired,
               vm_tag_t wire_tag, vm_object_fault_info_t fault_info,
               boolean_t *need_retry, int *type_of_fault,
               uint8_t *object_lock_type, bool *page_needs_sleep) {
  kern_return_t kr;
  vm_object_t object;
  ...
  vm_fault_enqueue_page(object, m, wired, fault_info->fi_change_wiring,
                        wire_tag, fault_info->no_cache, type_of_fault, kr);
  ...
  if (__probable((kr == KERN_SUCCESS) && !(*page_needs_sleep))) {
    ...
    kr = vm_fault_pmap_enter_with_object_lock(
        object, pmap, vaddr, fault_page_size, fault_phys_offset, m, &prot,
        caller_prot, fault_type, wired, pmap_options, need_retry,
        object_lock_type);
  }
...
kern_return_t vm_pre_fault_with_info(vm_map_t map, vm_map_offset_t vaddr,
                                     vm_prot_t prot,
                                     vm_object_fault_info_t fault_info) {
  assert(fault_info != NULL);
  if (pmap_find_phys(map->pmap, vaddr) == 0) {
    return vm_fault_internal(map, vaddr,          /* vaddr */
                             prot,                /* fault_type */
 ...
 */
kern_return_t vm_pre_fault(vm_map_offset_t vaddr, vm_prot_t prot) {
...
kern_return_t vm_fault_external(vm_map_t map, vm_map_offset_t vaddr,
                                vm_prot_t fault_type, boolean_t change_wiring,
  ...
  return vm_fault_internal(map, vaddr, fault_type,
                           ...
                           caller_pmap, caller_pmap_addr, NULL, &fault_info);
}
...
vm_fault(vm_map_t map, vm_map_offset_t vaddr, vm_prot_t fault_type,
  ...
  return vm_fault_internal(map, vaddr, fault_type, wire_tag, caller_pmap,
                           caller_pmap_addr, NULL, &fault_info);
}

static boolean_t current_proc_is_privileged(void) {
 ...
 */
static void vm_fault_complete(
    vm_map_t map, vm_map_t real_map, vm_object_t object, vm_object_t m_object,
    vm_page_t m, vm_map_offset_t offset, vm_map_offset_t trace_real_vaddr,
    vm_object_fault_info_t fault_info, vm_prot_t caller_prot,
#if CONFIG_DTRACE
    vm_map_offset_t real_vaddr,
#else
    __unused vm_map_offset_t real_vaddr,
#endif /* CONFIG_DTRACE */
    int type_of_fault, boolean_t need_retry, kern_return_t kr,
    ppnum_t *physpage_p, vm_prot_t prot, vm_object_t top_object,
    boolean_t need_collapse, vm_map_offset_t cur_offset, vm_prot_t fault_type,
    vm_object_t *written_on_object, memory_object_t *written_on_pager,
  ...
  if (m_object->internal) {
    event_code =
        (MACHDBG_CODE(DBG_MACH_WORKINGSET, VM_REAL_FAULT_ADDR_INTERNAL));
  } else if (m_object->object_is_shared_cache) {
    event_code =
        (MACHDBG_CODE(DBG_MACH_WORKINGSET, VM_REAL_FAULT_ADDR_SHAREDCACHE));
  } else {
    event_code =
        (MACHDBG_CODE(DBG_MACH_WORKINGSET, VM_REAL_FAULT_ADDR_EXTERNAL));
  }
  KDBG_RELEASE(event_code | DBG_FUNC_NONE, trace_real_vaddr,
               (fault_info->user_tag << 16) | (caller_prot << 8) |
                   type_of_fault,
               m->vmp_offset, get_current_unique_pid());
  if (need_retry == FALSE) {
    KDBG_FILTERED(MACHDBG_CODE(DBG_MACH_WORKINGSET, VM_REAL_FAULT_FAST),
                  get_current_unique_pid());
  }
  DTRACE_VM6(real_fault, vm_map_offset_t, real_vaddr, vm_map_offset_t,
             m->vmp_offset, int, event_code, int, caller_prot, int,
             type_of_fault, int, fault_info->user_tag);
  if (kr == KERN_SUCCESS && physpage_p != NULL) {
  ...
  if (need_retry == FALSE && (type_of_fault == DBG_PAGEIND_FAULT ||
                              type_of_fault == DBG_PAGEINV_FAULT ||
  ...
  if (need_retry == FALSE && !m_object->internal &&
      (fault_type & VM_PROT_WRITE)) {
...
static inline int vm_fault_type_for_tracing(boolean_t need_copy_on_read,
                                            int type_of_fault) {
...
kern_return_t vm_fault_internal(
    vm_map_t map, vm_map_offset_t vaddr, vm_prot_t caller_prot,
    vm_tag_t wire_tag, /* if wiring must pass tag != VM_KERN_MEMORY_NONE */
    pmap_t caller_pmap, vm_map_offset_t caller_pmap_addr, ppnum_t *physpage_p,
    vm_object_fault_info_t fault_info) {
  ...
  if (VM_MAP_PAGE_SIZE(original_map) < PAGE_SIZE) {
    ...
    if (fault_page_size < PAGE_SIZE) {
      DEBUG4K_FAULT("map %p vaddr 0x%llx caller_prot 0x%x\n", map,
                    (uint64_t)trace_real_vaddr, caller_prot);
  ...
  KDBG_RELEASE((VMDBG_CODE(DBG_VM_FAULT_INTERNAL)) | DBG_FUNC_START,
               ((uint64_t)trace_vaddr >> 32), trace_vaddr, (map == kernel_map));

  if (get_preemption_level() != 0) {
    KDBG_RELEASE((VMDBG_CODE(DBG_VM_FAULT_INTERNAL)) | DBG_FUNC_END,
                 ((uint64_t)trace_vaddr >> 32), trace_vaddr, KERN_FAILURE);
  ...
  if (cthread->th_vm_faults_disabled) {
    KDBG_RELEASE((MACHDBG_CODE(DBG_MACH_VM, 2)) | DBG_FUNC_END,
                 ((uint64_t)trace_vaddr >> 32), trace_vaddr, KERN_FAILURE);
  ...
  if ((map == kernel_map) && (caller_prot & VM_PROT_WRITE)) {
    if (compressor_map) {
      if ((vaddr >= vm_map_min(compressor_map)) &&
          (vaddr < vm_map_max(compressor_map))) {
        panic("Write fault on compressor map, va: %p type: %u bounds: %p->%p",
              (void *)vaddr, caller_prot, (void *)vm_map_min(compressor_map),
...
RetryFault:
  ...
  kr = vm_map_lookup_and_lock_object(
      &map, vaddr, (fault_type | (need_copy ? VM_PROT_COPY : 0)),
      object_lock_type, &version, &object, &offset, &prot, &wired, fault_info,
      &real_map, &object_is_contended);
  ...
  if (kr != KERN_SUCCESS) {
     ...
     *	ktriage_record(thread_tid(current_thread()),
     * KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_VM, KDBG_TRIAGE_RESERVED,
     * KDBG_TRIAGE_VM_ADDRESS_NOT_FOUND), 0);
     * }
     */
    goto done;
  }
  ...
  if (resilient_media_retry) {
    ...
    if ((object != VM_OBJECT_NULL && object == resilient_media_object &&
         offset == resilient_media_offset && fault_info->resilient_media)
#if MACH_ASSERT
        && (vm_fault_resilient_media_inject_error1_rate == 0 ||
            (++vm_fault_resilient_media_inject_error1 %
             vm_fault_resilient_media_inject_error1_rate) != 0)
#endif /* MACH_ASSERT */
    ) {
      /*
       ...
       */
      //                     printf("RESILIENT_MEDIA %s:%d recovering for object
      //                     %p offset 0x%llx\n", __FUNCTION__, __LINE__,
      //                     object, offset);
      vm_fault_resilient_media_proceed++;
    } else {
      /* not recovering: reset state and retry fault */
      //                     printf("RESILIENT_MEDIA %s:%d no recovery resilient
      //                     %d object %p/%p offset 0x%llx/0x%llx\n",
      //                     __FUNCTION__, __LINE__,
      //                     fault_info->resilient_media, object,
      //                     resilient_media_object, offset,
      ...
      //                     printf("FBDP %s:%d resilient_media_object %p
      //                     deallocate\n", __FUNCTION__, __LINE__,
   ...
   */
  if (fault_type == VM_PROT_READ && (prot & VM_PROT_EXECUTE) &&
      !(prot & VM_PROT_READ) && pmap_enforces_execute_only(pmap)) {
    vm_object_unlock(object);
    vm_map_unlock_read(map);
    if (real_map != map) {
      vm_map_unlock(real_map);
  ...
  fault_phys_offset = (vm_map_offset_t)offset -
                      vm_map_trunc_page((vm_map_offset_t)offset, PAGE_MASK);
  ...
  while (TRUE) {
    ...
    if (m != VM_PAGE_NULL) {
      ...
      if (__improbable(page_sleep_needed)) {
        /*
         * If a prior iteration of the loop requested vm_page_sleep(),
         * re-validate the page to see if it's still needed.
         */
        kr = vm_fault_pmap_validate_page(pmap, m, vaddr, prot, fault_info,
                                         &page_sleep_needed);
      ...
      if (m->vmp_busy || page_sleep_needed) {
        ...
        result = vm_page_sleep(cur_object, m, fault_info->interruptible,
                               LCK_SLEEP_UNLOCK);
    ...
    reclaimed_from_pageout:
      ...
      if (vm_fault_cs_need_validation(map->pmap, m, m_object, PAGE_SIZE, 0) ||
      ...
      if (!need_copy && !fault_info->no_copy_on_read && cur_object != object &&
          !cur_object->internal && !cur_object->pager_trusted &&
          !cur_object->code_signed && vm_protect_privileged_from_untrusted &&
          (current_proc_is_privileged() || vm_kernel_map_is_kernel(map) ||
         ...
         * and paged back in.
         */
        //				printf("COPY-ON-READ %s:%d map %p va
        //0x%llx page %p object %p offset 0x%llx UNTRUSTED: need
        //copy-on-read!\n", __FUNCTION__, __LINE__, map, (uint64_t)vaddr, m,
        //VM_PAGE_OBJECT(m), m->vmp_offset);
        vm_copied_on_read++;
      ...
      FastPmapEnter:
        ...
        if (fault_page_size < PAGE_SIZE) {
          DEBUG4K_FAULT(
              "map %p original %p pmap %p va 0x%llx caller pmap %p va 0x%llx "
              "pa 0x%llx (0x%llx+0x%llx) prot 0x%x caller_prot 0x%x\n",
              map, original_map, pmap, (uint64_t)vaddr, caller_pmap,
              (uint64_t)caller_pmap_addr,
              (uint64_t)((((pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(m))
                          << PAGE_SHIFT) +
                         fault_phys_offset),
              (uint64_t)(((pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(m))
                         << PAGE_SHIFT),
              (uint64_t)fault_phys_offset, prot, caller_prot);
          assertf((!(fault_phys_offset & FOURK_PAGE_MASK) &&
                   fault_phys_offset < PAGE_SIZE),
                  "0x%llx\n", (uint64_t)fault_phys_offset);
        } else {
        ...
        if (__improbable(rtfault && !m->vmp_realtime &&
        ...
        assertf(VM_PAGE_OBJECT(m) == m_object, "m=%p m_object=%p object=%p", m,
                m_object, object);
        assert(VM_PAGE_OBJECT(m) != VM_OBJECT_NULL);
        if (caller_pmap) {
          kr = vm_fault_enter(m, caller_pmap, caller_pmap_addr, fault_page_size,
                              fault_phys_offset, prot, caller_prot, wired,
                              wire_tag, fault_info, need_retry_ptr,
                              &type_of_fault, &object_lock_type,
                              &page_sleep_needed);
        } else {
          kr = vm_fault_enter(
              m, pmap, vaddr, fault_page_size, fault_phys_offset, prot,
              caller_prot, wired, wire_tag, fault_info, need_retry_ptr,
              &type_of_fault, &object_lock_type, &page_sleep_needed);
        }

        vm_fault_complete(
            map, real_map, object, m_object, m, offset, trace_real_vaddr,
            fault_info, caller_prot, real_vaddr,
            vm_fault_type_for_tracing(need_copy_on_read, type_of_fault),
            need_retry, kr, physpage_p, prot, top_object, need_collapse,
            cur_offset, fault_type, &written_on_object, &written_on_pager,
            &written_on_offset);
        top_object = VM_OBJECT_NULL;
        if (need_retry == TRUE) {
          /*
           ...
           */
          (void)pmap_enter_options(pmap, vaddr, 0, 0, 0, 0, 0,
      ...
      if ((cur_object_lock_type == OBJECT_LOCK_SHARED) &&
          vm_fault_cs_need_validation(NULL, m, m_object, PAGE_SIZE, 0)) {
      ...
      if (resilient_media_retry &&
          ...
          (++vm_fault_resilient_media_inject_error2 %
           vm_fault_resilient_media_inject_error2_rate) == 0) {
        /* inject an error */
        cur_m = m;
        m = VM_PAGE_NULL;
      ...
      if (VM_MAP_PAGE_MASK(map) != PAGE_MASK) {
        DEBUG4K_FAULT("map %p vaddr 0x%llx page %p [%p 0x%llx] copied to %p "
                      "[%p 0x%llx]\n",
                      map, (uint64_t)vaddr, cur_m, VM_PAGE_OBJECT(cur_m),
                      cur_m->vmp_offset, m, VM_PAGE_OBJECT(m), m->vmp_offset);
      }
      m_object = object;
       ...
       */
      if (os_ref_get_count_raw(&object->ref_count) > 1 && cur_m->vmp_pmapped) {
         ...
         */
        DEBUG4K_FAULT(
            "pmap_disconnect() page %p object %p offset 0x%llx phys 0x%x\n",
            cur_m, VM_PAGE_OBJECT(cur_m), cur_m->vmp_offset,
       ...
       */
      if (cur_object->pager_created) {
        vm_external_state_t compressor_external_state =
            VM_EXTERNAL_STATE_UNKNOWN;

        if (MUST_ASK_PAGER(cur_object, cur_offset, compressor_external_state) ==
            TRUE) {
          int my_fault_type;
          ...
          kr = vm_compressor_pager_get(
              cur_object->pager,
              (vm_object_trunc_page(cur_offset) + cur_object->paging_offset),
              VM_PAGE_GET_PHYS_PAGE(m), &my_fault_type, c_flags,
              &compressed_count_delta);

          vm_compressor_pager_count(cur_object->pager, compressed_count_delta,
                                    shared_lock, cur_object);
           ...
           * update that now.
           */
          if (object != cur_object && !insert_cur_object) {
            /*
             ...
             * case.
             */
          } else if (((cur_object->purgable == VM_PURGABLE_DENY) &&
                      (!cur_object->vo_ledger_tag)) ||
                     (cur_object->vo_owner == NULL)) {
            /*
             ...
             */
            if (compressed_count_delta) {
              vm_object_owner_compressed_update(cur_object, -1);
            }
          }
      ...
      if (cur_object->shadow == VM_OBJECT_NULL || resilient_media_retry) {
        /*
        ...
        if (cur_object->shadow_severed ||
            ...
            cur_object == compressor_object || is_kernel_object(cur_object)) {
          if (object != cur_object) {
            vm_object_unlock(cur_object);
          }
          vm_object_unlock(object);
        ...
        if (!object->internal) {
          panic("%s:%d should not zero-fill page at offset 0x%llx in external "
                "object %p",
                __FUNCTION__, __LINE__, (uint64_t)offset, object);
        }
#if MACH_ASSERT
        if (resilient_media_retry &&
            vm_fault_resilient_media_inject_error3_rate != 0 &&
            (++vm_fault_resilient_media_inject_error3 %
             vm_fault_resilient_media_inject_error3_rate) == 0) {
          /* inject an error */
          m_object = NULL;
          break;
        ...
        if ((prot & VM_PROT_WRITE) && !(fault_type & VM_PROT_WRITE) &&
        ...
        if (resilient_media_retry) {
          ...
          assertf(!((fault_type & VM_PROT_WRITE) && object->vo_copy),
                  "map %p va 0x%llx wrong path for write fault (fault_type "
                  "0x%x) on object %p with copy %p\n",
         ...
         * represents a significant amount of the zero fill fault handler's
         * work.
         *
         * To improve fault scalability, we'll drop the object lock, if it
         * appears contended, now that we've inserted the page into the vm
         * object. Before dropping the lock, we need to check protection bits
         * and set the mapped bits on the page. Then we can mark the page busy,
         * drop the lock, zero it, and do the pmap enter. We'll need to
         * reacquire the lock to clear the busy bit and wake up any waiters.
         */
        vm_fault_cs_clear(m);
        m->vmp_pmapped = TRUE;
        if (map->no_zero_fill) {
        ...
        }
        {
          ...
          kr = vm_fault_enter_prepare(
              m, destination_pmap, destination_pmap_vaddr, &prot, caller_prot,
              fault_page_size, fault_phys_offset, enter_fault_type, fault_info,
              &type_of_fault, &page_needs_data_sync, &page_sleep_needed);
          ...
          if (object_is_contended) {
            /*
             * At this point the page is in the vm object, but not on a paging
             * queue. Since it's accessible to another thread but its contents
             * are invalid (it hasn't been zeroed) mark it busy before dropping
             * the object lock.
             */
            m->vmp_busy = TRUE;
          ...
          if (object_is_contended) {
            ...
            if (object->vo_copy != saved_copy_object ||
               ...
               */
              if (pmap_has_prot_policy(
                      pmap,
                      fault_info->pmap_options &
                          PMAP_OPTIONS_TRANSLATED_ALLOW_EXECUTE,
                      prot)) {
                /* we should not do CoW on pmap_has_prot_policy mappings */
                panic("%s: map %p va 0x%llx obj %p,%u saved %p,%u: unexpected "
                      "CoW",
                      __FUNCTION__, map, (uint64_t)vaddr, object,
                      object->vo_copy_version, saved_copy_object,
                      saved_copy_version);
              } else {
          ...
          if (object_is_contended) {
            ...
            kr = vm_fault_pmap_enter(
                destination_pmap, destination_pmap_vaddr, fault_page_size,
                fault_phys_offset, m, &prot, caller_prot, enter_fault_type,
                wired, fault_info->pmap_options, need_retry_ptr);
            vm_object_lock(object);
            ...
            kr = vm_fault_pmap_enter_with_object_lock(
                object, destination_pmap, destination_pmap_vaddr,
                fault_page_size, fault_phys_offset, m, &prot, caller_prot,
                enter_fault_type, wired, fault_info->pmap_options,
                need_retry_ptr, &object_lock_type);
          }
        }
      zero_fill_cleanup:
        if (!VM_DYNAMIC_PAGING_ENABLED() &&
        ...
        vm_fault_enqueue_page(object, m, wired, fault_info->fi_change_wiring,
                              wire_tag, fault_info->no_cache, &type_of_fault,
                              kr);
        ...
        vm_fault_complete(
            map, real_map, object, m_object, m, offset, trace_real_vaddr,
            fault_info, caller_prot, real_vaddr, type_of_fault, need_retry, kr,
            physpage_p, prot, top_object, need_collapse, cur_offset, fault_type,
            &written_on_object, &written_on_pager, &written_on_offset);
        top_object = VM_OBJECT_NULL;
        if (need_retry == TRUE) {
          /*
           * vm_fault_enter couldn't complete the PMAP_ENTER...
...
handle_copy_delay:
  ...
  if (__improbable(object == compressor_object || is_kernel_object(object))) {
    /*
  ...
  vm_fault_return_t err = vm_fault_page(
      object, offset, fault_type, (fault_info->fi_change_wiring && !wired),
      FALSE, /* page not looked up */
      &prot, &result_page, &top_page, &type_of_fault, &error_code,
      map->no_zero_fill, fault_info);
   ...
   */
  if (err != VM_FAULT_SUCCESS && err != VM_FAULT_SUCCESS_NO_VM_PAGE) {
    if (err == VM_FAULT_MEMORY_ERROR && fault_info->resilient_media) {
       ...
       */
      //                     printf("RESILIENT_MEDIA %s:%d: object %p offset
      //                     0x%llx recover from media error 0x%x kr 0x%x
      //                     top_page %p result_page %p\n", __FUNCTION__,
      //                     __LINE__, object, offset, error_code, kr, top_page,
      //                     result_page);
      ...
      //                     printf("FBDP %s:%d resilient_media_object %p offset
      //                     0x%llx kept reference\n", __FUNCTION__, __LINE__,
      //                     resilient_media_object, resilient_mmedia_offset);
    ...
    case VM_FAULT_MEMORY_SHORTAGE:
      if (vm_page_wait((fault_info->fi_change_wiring) ? THREAD_UNINT
                                                      : THREAD_ABORTSAFE)) {
        goto RetryFault;
      }
      ktriage_record(thread_tid(current_thread()),
...
#define RELEASE_PAGE(m)                                                        \
  ...
  if ((map != original_map) || !vm_map_verify(map, &version)) {
    ...
    kr = vm_map_lookup_and_lock_object(
        ...
        &version, &retry_object, &retry_offset, &retry_prot, &wired, fault_info,
        &real_map, NULL);
    pmap = real_map->pmap;
  ...
  if ((prot & VM_PROT_WRITE) && m != VM_PAGE_NULL &&
    ...
    if (pmap_has_prot_policy(pmap,
       ...
       * access.
       */
      panic("%s: pmap %p vaddr 0x%llx prot 0x%x options 0x%x m%p obj %p "
            "copyobj %p",
            __FUNCTION__, pmap, (uint64_t)vaddr, prot, fault_info->pmap_options,
            m, m_object, m_object->vo_copy);
    }
    prot &= ~VM_PROT_WRITE;
  }

  if (!need_copy && !fault_info->no_copy_on_read && m != VM_PAGE_NULL &&
      VM_PAGE_OBJECT(m) != object && !VM_PAGE_OBJECT(m)->pager_trusted &&
      vm_protect_privileged_from_untrusted && !VM_PAGE_OBJECT(m)->code_signed &&
     ...
     */
    //		printf("COPY-ON-READ %s:%d map %p vaddr 0x%llx obj %p offset
    //0x%llx found page %p (obj %p offset 0x%llx) UNTRUSTED -> need
    //copy-on-read\n", __FUNCTION__, __LINE__, map, (uint64_t)vaddr, object,
    //offset, m, VM_PAGE_OBJECT(m), m->vmp_offset);
    vm_copied_on_read++;
   ...
   */
  if ((wired && (fault_type != (prot | VM_PROT_WRITE))) || need_copy_on_read) {
    vm_map_unlock_read(map);
    if (real_map != map) {
      vm_map_unlock(real_map);
    }
  ...
  if (m != VM_PAGE_NULL) {
    ...
    if (fault_page_size < PAGE_SIZE) {
      DEBUG4K_FAULT(
          "map %p original %p pmap %p va 0x%llx pa 0x%llx(0x%llx+0x%llx) prot "
          "0x%x caller_prot 0x%x\n",
          map, original_map, pmap, (uint64_t)vaddr,
          (uint64_t)((((pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(m)) << PAGE_SHIFT) +
                     fault_phys_offset),
          (uint64_t)(((pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(m)) << PAGE_SHIFT),
          (uint64_t)fault_phys_offset, prot, caller_prot);
    ...
    if (caller_pmap) {
      kr = vm_fault_enter(m, caller_pmap, caller_pmap_addr, fault_page_size,
                          fault_phys_offset, prot, caller_prot, wired, wire_tag,
                          fault_info, NULL, &type_of_fault, &object_lock_type,
                          &page_sleep_needed);
    } else {
      kr =
          vm_fault_enter(m, pmap, vaddr, fault_page_size, fault_phys_offset,
                         prot, caller_prot, wired, wire_tag, fault_info, NULL,
                         &type_of_fault, &object_lock_type, &page_sleep_needed);
    }
    assert(VM_PAGE_OBJECT(m) == m_object);

    {
      ...
      KDBG_RELEASE(
          event_code | DBG_FUNC_NONE, trace_real_vaddr,
          (fault_info->user_tag << 16) | (caller_prot << 8) |
              vm_fault_type_for_tracing(need_copy_on_read, type_of_fault),
          m->vmp_offset, get_current_unique_pid());
      KDBG_FILTERED(MACHDBG_CODE(DBG_MACH_WORKINGSET, VM_REAL_FAULT_SLOW),
                    get_current_unique_pid());
    ...
    while (vm_map_lookup_entry(map, laddr, &entry)) {
      ...
      if (entry->is_sub_map) {
        ...
        laddr = ((laddr - entry->vme_start) + VME_OFFSET(entry));
        vm_map_lock_read(VME_SUBMAP(entry));
        sub_map = VME_SUBMAP(entry);
        use_pmap = entry->use_pmap;
        entry = VM_MAP_ENTRY_NULL; /* not valid after unlock */
        if (map != real_map) {
          vm_map_unlock_read(map);
        }
        if (use_pmap) {
    ...
    if (vm_map_lookup_entry(map, laddr, &entry) && (!entry->is_sub_map) &&
        (object != VM_OBJECT_NULL) && (VME_OBJECT(entry) == object)) {
      uint16_t superpage;
      ...
      if (superpage && physpage_p) {
        /* for vm_map_wire_and_extract() */
        *physpage_p =
            (ppnum_t)((((vm_map_offset_t)object->vo_shadow_offset) +
                       VME_OFFSET(entry) + (laddr - entry->vme_start)) >>
                      PAGE_SHIFT);
      }
       ...
       */
      assert((uint32_t)((ldelta + hdelta) >> fault_page_shift) ==
             ((ldelta + hdelta) >> fault_page_shift));
      ...
      pmap_paddr_t block_map_pa =
          (pmap_paddr_t)(((vm_map_offset_t)(object->vo_shadow_offset)) +
                         VME_OFFSET(entry) + (laddr - entry->vme_start) -
                         ldelta);
      int block_map_wimg = VM_WIMG_MASK & (int)object->wimg_bits;
      if (caller_pmap) {
      ...
      kr =
          pmap_map_block_addr(block_map_pmap, block_map_va, block_map_pa,
                              (uint32_t)((ldelta + hdelta) >> fault_page_shift),
                              prot, block_map_wimg | superpage, 0);
...
#undef RELEASE_PAGE
   ...
   */
  if ((type_of_fault == DBG_PAGEIND_FAULT) ||
      (type_of_fault == DBG_PAGEINV_FAULT) ||
      (type_of_fault == DBG_COMPRESSOR_SWAPIN_FAULT)) {
    throttle_lowpri_io(1);
  } else {
    if (kr == KERN_SUCCESS && type_of_fault != DBG_CACHE_HIT_FAULT &&
        type_of_fault != DBG_GUARD_FAULT) {
      if ((throttle_delay = vm_page_throttled(TRUE))) {
        if (vm_debug_events) {
          if (type_of_fault == DBG_COMPRESSOR_FAULT) {
            ...
            VM_DEBUG_EVENT(vmf_cowdelay, DBG_VM_FAULT_COWDELAY, DBG_FUNC_NONE,
                           throttle_delay, 0, 0, 0);
          } else {
            VM_DEBUG_EVENT(vmf_zfdelay, DBG_VM_FAULT_ZFDELAY, DBG_FUNC_NONE,
                           throttle_delay, 0, 0, 0);
          }
        }
        __VM_FAULT_THROTTLE_FOR_PAGEOUT_SCAN__(throttle_delay);
  ...
  if (written_on_object) {
    vnode_pager_dirtied(written_on_pager, written_on_offset,
                        written_on_offset + PAGE_SIZE_64);
 ...
 */
kern_return_t vm_fault_wire(vm_map_t map, vm_map_entry_t entry, vm_prot_t prot,
                            vm_tag_t wire_tag, pmap_t pmap,
                            vm_map_offset_t pmap_addr, ppnum_t *physpage_p) {
  vm_map_offset_t va;
  ...
  pmap_pageable(pmap, pmap_addr, pmap_addr + (end_addr - entry->vme_start),
                FALSE);
  ...
  for (va = entry->vme_start; va < end_addr; va += effective_page_size) {
    rc = vm_fault_wire_fast(map, va, prot, wire_tag, entry, pmap,
                            pmap_addr + (va - entry->vme_start), physpage_p);
    if (rc != KERN_SUCCESS) {
      struct vm_object_fault_info fault_info = {
          .interruptible =
      ...
      rc = vm_fault_internal(map, va, prot, wire_tag, pmap,
                             (pmap_addr + (va - entry->vme_start)), physpage_p,
                             &fault_info);
    ...
    if (rc != KERN_SUCCESS) {
      ...
      vm_fault_unwire(map, &tmp_entry, FALSE, pmap, pmap_addr,
                      tmp_entry.vme_end);
 ...
 */
void vm_fault_unwire(vm_map_t map, vm_map_entry_t entry, boolean_t deallocate,
                     pmap_t pmap, vm_map_offset_t pmap_addr,
  ...
  for (va = entry->vme_start; va < end_addr; va += effective_page_size) {
    if (object == VM_OBJECT_NULL) {
      if (pmap) {
        pmap_change_wiring(pmap, pmap_addr + (va - entry->vme_start), FALSE);
      }
      (void)vm_fault(map, va, VM_PROT_NONE, TRUE, VM_KERN_MEMORY_NONE,
                     THREAD_UNINT, pmap, pmap_addr);
    } else {
      vm_prot_t prot;
      ...
      do {
        ...
        result_page = VM_PAGE_NULL;
        result =
            vm_fault_page(object, (VME_OFFSET(entry) + (va - entry->vme_start)),
                          VM_PROT_NONE, TRUE, FALSE, /* page not looked up */
                          &prot, &result_page, &top_page, (int *)0, NULL,
                          map->no_zero_fill, &fault_info);
      ...
      if (result == VM_FAULT_MEMORY_ERROR && is_kernel_object(object)) {
        /*
      ...
      if (deallocate) {
        assert(VM_PAGE_GET_PHYS_PAGE(result_page) != vm_page_fictitious_addr);
        ...
        if (entry->zero_wired_pages && (entry->protection & VM_PROT_WRITE) &&
#if __arm64e__
            !entry->used_for_tpro &&
#endif /* __arm64e__ */
            !entry->used_for_jit) {
  ...
  if (is_kernel_object(object)) {
    ...
    assertf((fault_info.user_tag & VME_ALIAS_MASK) == fault_info.user_tag,
            ...
            (fault_info.user_tag & VME_ALIAS_MASK));
 ...
 */
static kern_return_t vm_fault_wire_fast(__unused vm_map_t map,
                                        vm_map_offset_t va,
                                        __unused vm_prot_t caller_prot,
                                        vm_tag_t wire_tag, vm_map_entry_t entry,
                                        pmap_t pmap, vm_map_offset_t pmap_addr,
                                        ppnum_t *physpage_p) {
  vm_object_t object;
  vm_object_offset_t offset;
...
#define GIVE_UP                                                                \
  ...
  bool page_sleep_needed = false;
  kr = vm_fault_enter(
      m, pmap, pmap_addr, fault_page_size, fault_phys_offset, prot, prot,
      TRUE, /* wired */
      wire_tag, &fault_info, NULL, &type_of_fault,
 ...
 *		requested.
 */
kern_return_t vm_fault_copy(vm_object_t src_object,
                            vm_object_offset_t src_offset,
                            vm_map_size_t *copy_size, /* INOUT */
                            vm_object_t dst_object,
                            vm_object_offset_t dst_offset, vm_map_t dst_map,
                            vm_map_version_t *dst_version, int interruptible) {
  vm_page_t result_page;
...
#define RETURN(x)                                                              \
  ...
  *copy_size -= amount_left;                                                   \
  ...
  RetryDestinationFault:;
    ...
    result = vm_fault_page(dst_object, vm_object_trunc_page(dst_offset),
                           VM_PROT_WRITE | VM_PROT_READ, FALSE,
                           FALSE, /* page not looked up */
                           &dst_prot, &dst_page, &dst_top_page, (int *)0,
                           &error, dst_map->no_zero_fill, &fault_info_dst);
    switch (result) {
    case VM_FAULT_SUCCESS:
      break;
    case VM_FAULT_RETRY:
  ...
  RetrySourceFault:;
    ...
    if (src_object == VM_OBJECT_NULL) {
      ...
      src_page = vm_page_lookup(src_object, vm_object_trunc_page(src_offset));
      if (src_page == dst_page) {
        ...
        result_page = VM_PAGE_NULL;
        result =
            vm_fault_page(src_object, vm_object_trunc_page(src_offset),
                          VM_PROT_READ, FALSE, FALSE, /* page not looked up */
                          &src_prot, &result_page, &src_top_page, (int *)0,
                          &error, FALSE, &fault_info_src);
    ...
    if (!page_aligned(src_offset) || !page_aligned(dst_offset) ||
      ...
      if (result_page == VM_PAGE_NULL) {
        ...
        vm_page_part_zero_fill(dst_page, (vm_offset_t)dst_po,
                               (vm_size_t)part_size);
      } else {
        assert((vm_offset_t)src_po == src_po);
        assert((vm_offset_t)dst_po == dst_po);
        assert((vm_size_t)part_size == part_size);
        vm_page_part_copy(result_page, (vm_offset_t)src_po, dst_page,
                          (vm_offset_t)dst_po, (vm_size_t)part_size);
...
void vm_fault_classify(vm_object_t object, vm_object_offset_t offset,
                       vm_prot_t fault_type) {
  int type, level = 0;
  vm_page_t m;
...
void vm_fault_classify_init(void) {
  int type, level;
...
static inline bool object_supports_coredump(const vm_object_t object) {
...
vm_offset_t kdp_lightweight_fault(vm_map_t map, vm_offset_t cur_target_addr,
                                  bool multi_cpu) {
  vm_map_entry_t entry;
  vm_object_t object;
  ...
  while (TRUE) {
    ...
    if (object->pager_created &&
        (object->paging_in_progress || object->activity_in_progress)) {
      vmlp_api_end(KDP_LIGHTWEIGHT_FAULT, -1);
      return 0;
    }
    ...
    if (multi_cpu) {
      ...
      decomp_ppnum =
          vm_compressor_kdp_state.kc_decompressed_pages_ppnum[my_cpu_no];
      decomp_paddr =
          vm_compressor_kdp_state.kc_decompressed_pages_paddr[my_cpu_no];
    } else {
    ...
    if (object->pager_created &&
        MUST_ASK_PAGER(object, object_offset, compressor_external_state)) {
      if (compressor_external_state == VM_EXTERNAL_STATE_EXISTS) {
        kr = vm_compressor_pager_get(
            object->pager,
            vm_object_trunc_page(object_offset + object->paging_offset),
            decomp_ppnum, &my_fault_type, compressor_flags,
            &compressed_count_delta);
        if (kr == KERN_SUCCESS) {
          vmlp_api_end(KDP_LIGHTWEIGHT_FAULT, 0);
          return decomp_paddr;
 ...
 */
static boolean_t vm_page_validate_cs_fast(vm_page_t page,
                                          vm_map_size_t fault_page_size,
                                          vm_map_offset_t fault_phys_offset) {
  vm_object_t object;
  ...
  if (page->vmp_wpmapped &&
    ...
    if (cs_debug) {
      printf("CODESIGNING: %s: "
             ...
             "was modified\n",
             __FUNCTION__, page, object, page->vmp_offset);
    }
  ...
  kr = vnode_pager_cs_check_validation_bitmap(
      object->pager, page->vmp_offset + object->paging_offset, CS_BITMAP_CHECK);
  if (kr == KERN_SUCCESS) {
...
void vm_page_validate_cs_mapped_slow(vm_page_t page, const void *kaddr) {
  vm_object_t object;
  ...
  tainted = 0;
  nx = 0;
  cs_validate_page(vnode, pager, mo_offset, (const void *)((const char *)kaddr),
                   &validated, &tainted, &nx);
  ...
  if (page->vmp_cs_validated == VMP_CS_ALL_TRUE &&
      page->vmp_cs_tainted == VMP_CS_ALL_FALSE) {
    vnode_pager_cs_check_validation_bitmap(object->pager, mo_offset,
                                           CS_BITMAP_SET);
  }
#endif /* CHECK_CS_VALIDATION_BITMAP */
}

void vm_page_validate_cs_mapped(vm_page_t page, vm_map_size_t fault_page_size,
                                vm_map_offset_t fault_phys_offset,
                                const void *kaddr) {
...
static void vm_page_map_and_validate_cs(vm_object_t object, vm_page_t page) {
  vm_object_offset_t offset;
  ...
  koffset = 0;
  need_unmap = FALSE;
  kr = vm_paging_map_object(page, object, offset, VM_PROT_READ,
                            FALSE, /* can't unlock object ! */
                            &ksize, &koffset, &need_unmap);
  if (kr != KERN_SUCCESS) {
...
void vm_page_validate_cs(vm_page_t page, vm_map_size_t fault_page_size,
                         vm_map_offset_t fault_phys_offset) {
  vm_object_t object;
...
void vm_page_validate_cs_mapped_chunk(vm_page_t page, const void *kaddr,
  ...
  *tainted_p = 0;
  ...
  tainted = 0;
  validated = cs_validate_range(
      vnode, pager, (object->paging_offset + offset + offset_in_page),
      (const void *)((const char *)kaddr + offset_in_page), chunk_size,
      &tainted);
  if (validated) {
    *validated_p = TRUE;
  }
...
static void vm_rtfrecord_lock(void) { lck_spin_lock(&vm_rtfr_slock); }

static void vm_rtfrecord_unlock(void) { lck_spin_unlock(&vm_rtfr_slock); }
...
vm_record_rtfault(thread_t cthread, uint64_t fstart,
                  vm_map_offset_t fault_vaddr, int type_of_fault) {
...
int vmrtf_extract(uint64_t cupid, __unused boolean_t isroot,
                  unsigned long vrecordsz, void *vrecords,
 ...
 */
CA_EVENT(vmtc_telemetry, CA_INT,
         vmtc_num_byte, /* number of corrupt bytes found */
         CA_BOOL,
         vmtc_undiagnosed, /* undiagnosed because more than 1 at a time */
         CA_BOOL, vmtc_not_eligible, /* the page didn't qualify */
         CA_BOOL, vmtc_copyin_fail,  /* unable to copy in the page */
         CA_BOOL,
         vmtc_not_found, /* no corruption found even though CS failed */
 ...
 */
static uint64_t *vmtc_get_page_data(vm_map_offset_t code_addr, vm_page_t page) {
   ...
   */
  if (kernel_memory_allocate(kernel_map, (vm_offset_t *)&buffer, size, size - 1,
                             KMA_KOBJECT,
 ...
 */
static uint64_t *
vmtc_text_page_diagnose_setup(vm_map_offset_t code_addr, vm_page_t page,
 ...
 */
static void vmtc_text_page_diagnose(vm_map_offset_t code_addr,
  ...
  if (copyin((user_addr_t)vm_map_trunc_page(code_addr, size - 1),
  ...
  if (vmtc_last_before_buffer != NULL) {
    kmem_free(kernel_map, (vm_offset_t)vmtc_last_before_buffer,
 ...
 */
static kern_return_t vmtc_revalidate_lookup(vm_map_t map, vm_map_offset_t vaddr,
  ...
  boolean_t wired;
  struct vm_object_fault_info fault_info = {.interruptible = THREAD_UNINT};
...
restart:
  ...
  kr = vm_map_lookup_and_lock_object(
      &map, vaddr, VM_PROT_READ, object_lock_type, &version, &object, &offset,
      &prot, &wired, &fault_info, &real_map, NULL);
  vm_map_unlock_read(map);
 ...
 */
static bool is_page_wired(vm_page_t page) {
  bool result;
  vm_page_lock_queues();
 ...
 */
kern_return_t revalidate_text_page(task_t task, vm_map_offset_t code_addr) {
  kern_return_t kr;
  vm_map_t map;
  ...
  if (!object->code_signed || /* no code signature to check */
      ...
      object
          ->terminating || /* the object and its pages are already going away */
  ...
  if (page->vmp_cs_tainted != VMP_CS_ALL_FALSE) {
     ...
     */
    if (!page->vmp_cleaning && !page->vmp_laundry &&
        !vm_page_is_fictitious(page) && !page->vmp_precious &&
        !page->vmp_absent && !VMP_ERROR_GET(page) && !page->vmp_dirty &&
        !is_page_wired(page)) {
      if (page->vmp_pmapped) {
...
kern_return_t vm_corrupt_text_addr(uintptr_t va) {
