...
#if MACH_ASSERT
bool first_free_is_valid_store(vm_map_t map) {
  return first_free_is_valid_ll(map);
}
#endif

bool vm_map_store_has_RB_support(struct vm_map_header *hdr) {
...
void vm_map_store_init(struct vm_map_header *hdr) {
...
static inline bool _vm_map_store_lookup_entry(vm_map_t map,
...
vm_map_store_lookup_entry(vm_map_t map, vm_map_offset_t address,
                          vm_map_entry_t *entry) /* OUT */
{
...
void _vm_map_store_entry_link(struct vm_map_header *mapHdr,
  ...
  if (__improbable(vm_debug_events)) {
    DTRACE_VM4(map_entry_link, vm_map_t,
               __container_of(mapHdr, struct _vm_map, hdr), vm_map_entry_t,
               entry, vm_address_t, entry->vme_start, vm_address_t,
               entry->vme_end);
  }
  ...
  entry->vme_insertion_bt =
      btref_get(__builtin_frame_address(0), BTREF_GET_NOWAIT);
#endif
}

void vm_map_store_entry_link(vm_map_t map, vm_map_entry_t after_where,
                             vm_map_entry_t entry,
  ...
  if (map->disable_vmentry_reuse == TRUE) {
    ...
    if (alias != VM_MEMORY_MALLOC_NANO && alias != VM_MEMORY_MALLOC_TINY &&
        alias != VM_MEMORY_MALLOC_SMALL && alias != VM_MEMORY_MALLOC_MEDIUM &&
        alias != VM_MEMORY_MALLOC_LARGE && alias != VM_MEMORY_MALLOC_HUGE &&
...
void _vm_map_store_entry_unlink(struct vm_map_header *mapHdr,
                                vm_map_entry_t entry, bool check_permanent) {
  if (__improbable(vm_debug_events)) {
    DTRACE_VM4(map_entry_unlink, vm_map_t,
               __container_of(mapHdr, struct _vm_map, hdr), vm_map_entry_t,
               entry, vm_address_t, entry->vme_start, vm_address_t,
               entry->vme_end);
  }
...
void vm_map_store_entry_unlink(vm_map_t map, vm_map_entry_t entry,
...
void vm_map_store_copy_reset(vm_map_copy_t copy, vm_map_entry_t entry) {
...
void vm_map_store_update_first_free(vm_map_t map,
...
__abortlike static void __vm_map_store_find_space_holelist_corruption(
    vm_map_t map, vm_map_offset_t start, vm_map_entry_t entry) {
...
static void vm_map_store_convert_hole_to_entry(vm_map_t map,
...
static struct vm_map_entry *vm_map_store_find_space_backwards(
    vm_map_t map, vm_map_offset_t end, vm_map_offset_t lowest_addr,
    vm_map_offset_t guard_offset, vm_map_size_t size, vm_map_offset_t mask,
...
static struct vm_map_entry *vm_map_store_find_space_forward(
    vm_map_t map, vm_map_offset_t start, vm_map_offset_t highest_addr,
    vm_map_offset_t guard_offset, vm_map_size_t size, vm_map_offset_t mask,
    vm_map_offset_t *addr_out) {
  const vm_map_offset_t map_mask = VM_MAP_PAGE_MASK(map);
  const bool use_holes = map->holelistenabled;
...
vm_map_store_find_space(vm_map_t map, vm_map_offset_t hint,
  ...
  if (backwards) {
    entry = vm_map_store_find_space_backwards(map, hint, limit, guard_offset,
                                              size, mask, addr_out);
  } else {
    entry = vm_map_store_find_space_forward(map, hint, limit, guard_offset,
                                            size, mask, addr_out);
  }
