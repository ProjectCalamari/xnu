...
/* Zero-out a tagged memory region performing the minimum set of mandatory
 * checks. */
 ...
 */
extern void vm_memtag_store_tag(caddr_t tagged_address, vm_size_t size);

/* Randomly assign a tag to the current chunk of memory. */
extern caddr_t vm_memtag_generate_and_store_tag(caddr_t address,
                                                vm_size_t size);
 ...
 */
extern void vm_memtag_verify_tag(vm_map_address_t tagged_address);

/*
 * Copy metadata between two mappings whenever we are relocating memory.
 */
extern void vm_memtag_relocate_tags(vm_address_t new_address,
                                    vm_address_t old_address, vm_size_t size);

/* Temporarily enable/disable memtag checking. */
extern void vm_memtag_enable_checking(void);
extern void vm_memtag_disable_checking(void);

/*
 * Zeroing operations traditionally happen on large amount of memory (often
 * pages) and tend to span over several different regions with different
 * memtags. Implement variants of bzero that capture both performing this
 * operation without checking (vm_memtag_bzero_unchecked) and by optimizing
 * checking behavior (vm_memtag_bzero_fast_checked)
 */
extern void vm_memtag_bzero_fast_checked(void *tagged_buf, vm_size_t n);
extern void vm_memtag_bzero_unchecked(void *tagged_buf, vm_size_t n);
...
#define vm_memtag_store_tag(a, s)                                              \
  do {                                                                         \
  } while (0)
#define vm_memtag_generate_and_store_tag(a, s) (a)
#define vm_memtag_relocate_tags(n, o, l)                                       \
  do {                                                                         \
  } while (0)
#define vm_memtag_enable_checking()                                            \
  do {                                                                         \
  } while (0)
#define vm_memtag_disable_checking()                                           \
  do {                                                                         \
  } while (0)
...
static inline vm_map_address_t vm_memtag_insert_tag(vm_map_address_t naked_ptr,
                                                    vm_memtag_t tag) {
...
static inline vm_memtag_t vm_memtag_extract_tag(vm_map_address_t tagged_ptr) {
  union vm_memtag_ptr p = {
...
/*
 * when passed a tagged pointer, strip away only the tag bits with their
 * canonical value. Since these are used in a number of frequently called checks
...
extern vm_map_address_t vm_memtag_canonicalize(vm_map_t map,
...
#if KASAN_TBI
#error                                                                         \
    "vm_memtag manipulation APIs should be defined whenever KASAN-TBI is enabled"
#endif /* KASAN_TBI */
