...
static TUNABLE(bool, vm_map_executable_immutable, "vm_map_executable_immutable",
               true);
#else
...
static TUNABLE(int, vm_map_kernel_alloc_limit_mode,
               "vm_map_kernel_alloc_limit_mode",
               VM_MAP_KERNEL_ALLOC_LIMIT_MODE_REJECT);
#else
#define vm_map_kernel_alloc_limit_mode VM_MAP_KERNEL_ALLOC_LIMIT_MODE_REJECT
#endif

SECURITY_READ_ONLY_EARLY(vm_packing_params_t)
vm_map_entry_packing_params = VM_PACKING_PARAMS(VM_ENTRY_PACKED_PTR);
...
extern kern_return_t vm_map_wire_external(vm_map_t map,
...
#endif
    kern_return_t
    vm_map_copyin_common(vm_map_t src_map, vm_map_address_ut src_addr,
...
vm_map_entry_insert(vm_map_t map, vm_map_entry_t insp_entry,
                    vm_map_offset_t start, vm_map_offset_t end,
                    vm_object_t object, vm_object_offset_t offset,
                    vm_map_kernel_flags_t vmk_flags, boolean_t needs_copy,
                    vm_prot_t cur_protection, vm_prot_t max_protection,
                    vm_inherit_t inheritance);

static void vm_map_simplify_range(vm_map_t map, vm_map_offset_t start,
                                  vm_map_offset_t end); /* forward */

static boolean_t vm_map_range_check(vm_map_t map, vm_map_offset_t start,
                                    vm_map_offset_t end, vm_map_entry_t *entry);

static void vm_map_submap_pmap_clean(vm_map_t map, vm_map_offset_t start,
                                     vm_map_offset_t end, vm_map_t sub_map,
                                     vm_map_offset_t offset);

static void vm_map_pmap_enter(vm_map_t map, vm_map_offset_t addr,
                              vm_map_offset_t end_addr, vm_object_t object,
                              vm_object_offset_t offset, vm_prot_t protection);

static void _vm_map_clip_end(struct vm_map_header *map_header,
                             vm_map_entry_t entry, vm_map_offset_t end);

static void _vm_map_clip_start(struct vm_map_header *map_header,
                               vm_map_entry_t entry, vm_map_offset_t start);

static kmem_return_t vm_map_delete(vm_map_t map, vm_map_offset_t start,
                                   vm_map_offset_t end, vmr_flags_t flags,
                                   kmem_guard_t guard, vm_map_zap_t zap);

static void vm_map_copy_insert(vm_map_t map, vm_map_entry_t after_where,
                               vm_map_copy_t copy);

static kern_return_t
vm_map_copy_overwrite_unaligned(vm_map_t dst_map, vm_map_entry_t entry,
                                vm_map_copy_t copy, vm_map_address_t start,
                                boolean_t discard_on_success);

static kern_return_t vm_map_copy_overwrite_aligned(vm_map_t dst_map,
...
static kern_return_t
vm_map_copyin_kernel_buffer(vm_map_t src_map, vm_map_address_t src_addr,
                            vm_map_size_t len, vm_map_copyin_strategy strategy,
                            boolean_t src_destroy,
                            vm_map_copy_t *copy_result); /* OUT */

static kern_return_t
vm_map_copyout_kernel_buffer(vm_map_t map, vm_map_address_t *addr, /* IN/OUT */
                             vm_map_copy_t copy, vm_map_size_t copy_size,
                             boolean_t overwrite, boolean_t consume_on_success);

static void vm_map_fork_share(vm_map_t old_map, vm_map_entry_t old_entry,
                              vm_map_t new_map);

static boolean_t vm_map_fork_copy(vm_map_t old_map, vm_map_entry_t *old_entry_p,
                                  vm_map_t new_map, int vm_map_copyin_flags);

static kern_return_t vm_map_wire_nested(vm_map_t map, vm_map_offset_t start,
                                        vm_map_offset_t end,
                                        vm_prot_t caller_prot, vm_tag_t tag,
                                        boolean_t user_wire, pmap_t map_pmap,
                                        vm_map_offset_t pmap_addr,
                                        ppnum_t *physpage_p);

static kern_return_t vm_map_unwire_nested(vm_map_t map, vm_map_offset_t start,
                                          vm_map_offset_t end,
                                          boolean_t user_wire, pmap_t map_pmap,
                                          vm_map_offset_t pmap_addr);

static kern_return_t vm_map_overwrite_submap_recurse(vm_map_t dst_map,
                                                     vm_map_offset_t dst_addr,
                                                     vm_map_size_t dst_size);

static kern_return_t
vm_map_copy_overwrite_nested(vm_map_t dst_map, vm_map_offset_t dst_addr,
                             vm_map_copy_t copy, boolean_t interruptible,
                             pmap_t pmap, boolean_t discard_on_success);

static kern_return_t
vm_map_remap_extract(vm_map_t map, vm_map_offset_t addr, vm_map_size_t size,
                     boolean_t copy, vm_map_copy_t map_copy,
                     vm_prot_t *cur_protection, vm_prot_t *max_protection,
                     vm_inherit_t inheritance, vm_map_kernel_flags_t vmk_flags);

static void vm_map_region_look_for_page(vm_map_t map, vm_map_offset_t va,
                                        vm_object_t object,
                                        vm_object_offset_t offset,
                                        int max_refcnt, unsigned short depth,
                                        vm_region_extended_info_t extended,
                                        mach_msg_type_number_t count);

static boolean_t vm_map_region_has_obj_ref(vm_map_entry_t entry,
                                           vm_object_t object);

static kern_return_t vm_map_willneed(vm_map_t map, vm_map_offset_t start,
                                     vm_map_offset_t end);

static kern_return_t vm_map_reuse_pages(vm_map_t map, vm_map_offset_t start,
                                        vm_map_offset_t end);

static kern_return_t vm_map_reusable_pages(vm_map_t map, vm_map_offset_t start,
                                           vm_map_offset_t end);

static kern_return_t vm_map_can_reuse(vm_map_t map, vm_map_offset_t start,
                                      vm_map_offset_t end);

static kern_return_t vm_map_zero(vm_map_t map, vm_map_offset_t start,
                                 vm_map_offset_t end);

static kern_return_t
vm_map_random_address_for_size(vm_map_t map, vm_map_offset_t *address,
                               vm_map_size_t size,
                               vm_map_kernel_flags_t vmk_flags);

#if CONFIG_MAP_RANGES

static vm_map_range_id_t vm_map_user_range_resolve(vm_map_t map,
...
#if MACH_ASSERT
static kern_return_t vm_map_pageout(vm_map_t map, vm_map_offset_t start,
                                    vm_map_offset_t end);
#endif /* MACH_ASSERT */

kern_return_t vm_map_corpse_footprint_collect(vm_map_t old_map,
                                              vm_map_entry_t old_entry,
                                              vm_map_t new_map);
void vm_map_corpse_footprint_collect_done(vm_map_t new_map);
void vm_map_corpse_footprint_destroy(vm_map_t map);
kern_return_t vm_map_corpse_footprint_query_page_info(vm_map_t map,
                                                      vm_map_offset_t va,
                                                      int *disposition_p);
static void vm_map_footprint_query_page_info_exclusive(
    vm_map_t map, vm_map_entry_t map_entry, vm_map_offset_t curr_s_offset,
    int *disposition_p);
...
uint8_t vm_map_entry_info_flags(vm_map_entry_t entry);
...
vm_map_kernel_flags_set_vmflags(vm_map_kernel_flags_t *vmk_flags, int vm_flags,
                                vm_tag_t vm_tag) {
...
__attribute__((always_inline, overloadable)) void
vm_map_kernel_flags_set_vmflags(vm_map_kernel_flags_t *vmk_flags,
...
vm_map_kernel_flags_and_vmflags(vm_map_kernel_flags_t *vmk_flags,
                                int vm_flags_mask) {
...
vm_map_kernel_flags_check_vm_and_kflags(vm_map_kernel_flags_t vmk_flags,
                                        int vm_flags_mask) {
  return (vmk_flags.__vm_flags & ~vm_flags_mask) == 0;
}

bool vm_map_kernel_flags_check_vmflags(vm_map_kernel_flags_t vmk_flags,
                                       int vm_flags_mask) {
...
static inline void vm_map_entry_copy_csm_assoc(vm_map_t map __unused,
  ...
  if (new->vme_xnu_user_debug && vm_log_xnu_user_debug) {
    printf("FBDP %d[%s] %s:%d map %p entry %p [ 0x%llx 0x%llx ] "
           "vme_xnu_user_debug\n",
           proc_selfpid(),
           (get_bsdtask_info(current_task())
                ? proc_name_address(get_bsdtask_info(current_task()))
                : "?"),
           __FUNCTION__, __LINE__, map, new, new->vme_start, new->vme_end);
  }
#endif /* DEVELOPMENT || DEBUG */
 ...
 */
static inline void vm_map_entry_copy_code_signing(vm_map_t map,
                                                  vm_map_entry_t new,
                                                  vm_map_entry_t old __unused) {
  if (VM_MAP_POLICY_ALLOW_JIT_COPY(map)) {
    ...
    if (old->used_for_jit) {
      DTRACE_VM3(cs_wx, uint64_t, new->vme_start, uint64_t, new->vme_end,
      ...
      printf("CODE SIGNING: %d[%s] %s: curprot cannot be write+execute. %s\n",
             ...
             (get_bsdtask_info(current_task())
                  ? proc_name_address(get_bsdtask_info(current_task()))
                  : "?"),
             __FUNCTION__, "removing execute access");
...
static inline void vm_map_entry_copy_full(vm_map_entry_t new,
                                          vm_map_entry_t old) {
...
static inline void vm_map_entry_copy(vm_map_t map, vm_map_entry_t new,
                                     vm_map_entry_t old) {
 ...
 */
__attribute__((always_inline)) int vm_map_lock_read_to_write(vm_map_t map) {
...
__attribute__((always_inline)) boolean_t vm_map_try_lock(vm_map_t map) {
...
__attribute__((always_inline)) boolean_t vm_map_try_lock_read(vm_map_t map) {
...
int vm_self_region_page_shift(vm_map_t target_map) {
  ...
  if (effective_page_shift == -1) {
    ...
    effective_page_shift =
        MIN(VM_MAP_PAGE_SHIFT(current_map()), VM_MAP_PAGE_SHIFT(target_map));
  }
  return effective_page_shift;
}

/*
 *	Decide if we want to allow processes to execute from their data or stack
 * areas. override_nx() returns true if we do.  Data/stack execution can be
 * enabled independently for 32 and 64 bit processes.  Set the VM_ABI_32 or
 * VM_ABI_64 flags in allow_data_exec or allow_stack_exec to enable data
 * execution for that type of data area for that particular ABI (or both by
 * or'ing the flags together).  These are initialized in the architecture
 *	specific pmap files since the default behavior varies according to
 * architecture.  The main reason it varies is because of the need to provide
 * binary compatibility with old applications that were written before these
 * restrictions came into being.  In the old days, an app could execute anything
 * it could read, but this has slowly been tightened up over time.  The default
 * behavior is:
 *
 ...
 *
 *	An application on any architecture may override these defaults by
 * explicitly adding PROT_EXEC permission to the page in question with the
 * mprotect(2) system call.  This code here just determines what happens when an
 * app tries to execute from a page that lacks execute permission.
 *
 *	Note that allow_data_exec or allow_stack_exec may also be modified by
 * sysctl to change the default behavior for both 32 and 64 bit apps on a
 * system-wide basis. Furthermore, a Mach-O header flag bit
 * (MH_NO_HEAP_EXECUTION) can be used to forcibly disallow execution from data
 * areas for a particular binary even if the arch normally permits it. As a
 * final wrinkle, a posix_spawn attribute flag can be used to negate this opt-in
 * header bit to support some complicated use cases, notably browsers with
 * out-of-process plugins that are not all NX-safe.
 */

extern int allow_data_exec, allow_stack_exec;

int override_nx(vm_map_t map, uint32_t user_tag) /* map unused on arm */
{
  int current_abi;
  ...
  return (allow_data_exec & current_abi) &&
         (map->map_disallow_data_exec == FALSE);
}
 ...
 * anywhere else.
 */
void vm_map_copy_require(struct vm_map_copy *copy) {
 ...
 */
void vm_map_require(vm_map_t map) {
...
kern_return_t vm_map_set_cache_attr(vm_map_t map, vm_map_offset_t va) {
  ...
  if (!vm_map_lookup_entry(map, va, &map_entry) || map_entry->is_sub_map) {
    /*
 ...
 * The crypt_info block will be copied by vm_map_apple_protected. The data
 * structures referenced in crypt_info must remain valid until
 * crypt_info->crypt_end() is called.
 */
kern_return_t vm_map_apple_protected(vm_map_t map, vm_map_offset_t start,
                                     vm_map_offset_t end,
  ...
  for (map_addr = start_aligned; map_addr < end; map_addr = tmp_entry.vme_end) {
    vm_map_lock(map);
    map_locked = TRUE;

    /* lookup the protected VM object */
    if (!vm_map_lookup_entry(map, map_addr, &map_entry) ||
        map_entry->is_sub_map || VME_OBJECT(map_entry) == VM_OBJECT_NULL) {
      /* that memory is not properly mapped */
      kr = KERN_INVALID_ARGUMENT;
      goto done;
    }
    ...
    unprotected_mem_obj = apple_protect_pager_setup(
        protected_object, VME_OFFSET(&tmp_entry), crypto_backing_offset,
        crypt_info, crypto_start, crypto_end, cache_pager);
    ...
    kr = mach_vm_map_kernel(
        map, vm_sanitize_wrap_addr_ref(&map_addr),
        (tmp_entry.vme_end - tmp_entry.vme_start), (mach_vm_offset_t)0,
        vmk_flags, (ipc_port_t)(uintptr_t)unprotected_mem_obj, 0, TRUE,
        tmp_entry.protection, tmp_entry.max_protection, tmp_entry.inheritance);
    assertf(kr == KERN_SUCCESS, "kr = 0x%x\n", kr);
    assertf(map_addr == tmp_entry.vme_start,
            "map_addr=0x%llx vme_start=0x%llx tmp_entry=%p\n",
            (uint64_t)map_addr, (uint64_t)tmp_entry.vme_start, &tmp_entry);
    ...
    if (vm_map_debug_apple_protect) {
      printf("APPLE_PROTECT: map %p [0x%llx:0x%llx] pager %p:"
             ...
             map, (uint64_t)map_addr,
             (uint64_t)(map_addr + (tmp_entry.vme_end - tmp_entry.vme_start)),
             unprotected_mem_obj, protected_object, VME_OFFSET(&tmp_entry),
             crypto_backing_offset, crypto_start, crypto_end);
    }
#endif /* VM_MAP_DEBUG_APPLE_PROTECT */
    ...
    crypto_backing_offset += (tmp_entry.vme_end - tmp_entry.vme_start);
...
TUNABLE(int, malloc_no_cow_except_fork, "malloc_no_cow_except_fork",
        MALLOC_NO_COW_EXCEPT_FORK_DEFAULT);
 ...
 *	of a new entry.
 */
__startup_func void vm_map_init(void) {

#if MACH_ASSERT
  PE_parse_boot_argn("debug4k_filter", &debug4k_filter, sizeof(debug4k_filter));
#endif /* MACH_ASSERT */

  zone_create_ext(VM_MAP_ZONE_NAME, sizeof(struct _vm_map), VM_MAP_ZFLAGS,
                  ZONE_ID_VM_MAP, NULL);
  ...
  zone_create_ext(VM_MAP_ENTRY_ZONE_NAME, sizeof(struct vm_map_entry),
                  VM_MAP_ENTRY_ZFLAGS, ZONE_ID_VM_MAP_ENTRY, ^(zone_t z) {
                    z->z_elems_rsv =
                        (uint16_t)(32 * (ml_early_cpu_max_number() + 1));
                  });

  zone_create_ext(VM_MAP_HOLES_ZONE_NAME, sizeof(struct vm_map_links),
                  VM_MAP_HOLES_ZFLAGS, ZONE_ID_VM_MAP_HOLES, ^(zone_t z) {
                    z->z_elems_rsv =
                        (uint16_t)(16 * 1024 / zone_elem_outer_size(z));
                  });

  zone_create_ext("VM map copies", sizeof(struct vm_map_copy), ZC_NOENCRYPT,
                  ZONE_ID_VM_MAP_COPY, NULL);
  ...
  printf("VM boostrap: %d maps, %d entries and %d holes available\n",
         zone_count_free(vm_map_zone), zone_count_free(vm_map_entry_zone),
         zone_count_free(vm_map_holes_zone));

  /*
   * Since these are covered by zones, remove them from stolen page accounting.
   */
  VM_PAGE_MOVE_STOLEN(atop_64(map_data_size) + atop_64(kentry_data_size) +
                      atop_64(map_holes_data_size));

#if VM_MAP_DEBUG_APPLE_PROTECT
  PE_parse_boot_argn("vm_map_debug_apple_protect", &vm_map_debug_apple_protect,
                     sizeof(vm_map_debug_apple_protect));
#endif /* VM_MAP_DEBUG_APPLE_PROTECT */
#if VM_MAP_DEBUG_APPLE_FOURK
  PE_parse_boot_argn("vm_map_debug_fourk", &vm_map_debug_fourk,
...
#if DEBUG
  PE_parse_boot_argn("vm_check_map_sanity", &vm_check_map_sanity,
                     sizeof(vm_check_map_sanity));
  if (vm_check_map_sanity) {
    kprintf("VM sanity checking enabled\n");
  } else {
    kprintf("VM sanity checking disabled. Set bootarg vm_check_map_sanity=1 to "
            "enable\n");
  }
#endif /* DEBUG */

#if DEVELOPMENT || DEBUG
  PE_parse_boot_argn("panic_on_unsigned_execute", &panic_on_unsigned_execute,
                     sizeof(panic_on_unsigned_execute));
  PE_parse_boot_argn("panic_on_mlock_failure", &panic_on_mlock_failure,
                     sizeof(panic_on_mlock_failure));
#endif /* DEVELOPMENT || DEBUG */
}

__startup_func static void vm_map_steal_memory(void) {
   ...
   */
  map_data_size =
      zone_get_early_alloc_size(VM_MAP_ZONE_NAME, sizeof(struct _vm_map),
                                VM_MAP_ZFLAGS, VM_MAP_EARLY_COUNT_MAX);

  kentry_data_size = zone_get_early_alloc_size(
      VM_MAP_ENTRY_ZONE_NAME, sizeof(struct vm_map_entry), VM_MAP_ENTRY_ZFLAGS,
      8 * VM_MAP_EARLY_COUNT_MAX);

  map_holes_data_size = zone_get_early_alloc_size(
      VM_MAP_HOLES_ZONE_NAME, sizeof(struct vm_map_links), VM_MAP_HOLES_ZFLAGS,
      8 * VM_MAP_EARLY_COUNT_MAX);
...
__startup_func static void vm_kernel_boostraped(void) {
...
void vm_map_disable_hole_optimization(vm_map_t map) {
...
extern vm_map_t vm_map_create_external(pmap_t pmap, vm_map_offset_t min_off,
                                       vm_map_offset_t max_off,
                                       boolean_t pageable);

vm_map_t vm_map_create_external(pmap_t pmap, vm_map_offset_t min,
                                vm_map_offset_t max, boolean_t pageable) {
...
__startup_func void vm_map_will_allocate_early_map(vm_map_t *owner) {
...
__startup_func void vm_map_relocate_early_maps(vm_offset_t delta) {
 ...
 */
__startup_func void vm_map_relocate_early_elem(uint32_t zone_id,
...
#define relocate(type_t, field)                                                \
  ...
  if (zone_id == ZONE_ID_VM_MAP) {
    ...
    if (VMH_PREV(hdr_ptr)) {
      VMH_PREV_SET(hdr_ptr,
                   (vm_map_entry_t)(((vm_offset_t)VMH_PREV(hdr_ptr)) + delta));
    }
  ...
  if (VML_PREV(links_ptr)) {
    VML_PREV_SET(links_ptr,
                 (vm_map_entry_t)(((vm_offset_t)VML_PREV(links_ptr)) + delta));
  }
  ...
  if (zone_id == ZONE_ID_VM_MAP_ENTRY) {
    ...
    if (((vm_map_entry_t)new_addr)->is_sub_map) {
      ...
      ((vm_map_entry_t)new_addr)->vme_submap += delta >> VME_SUBMAP_SHIFT;
    }
#if MAP_ENTRY_CREATION_DEBUG
...
vm_map_serial_t vm_map_serial_generate(void) {
  vm_map_serial_t serial =
      (void *)os_atomic_inc(&vm_map_serial_current, relaxed);
  return serial;
}

void vm_map_assign_serial(vm_map_t map, vm_map_serial_t serial) {
...
vm_map_t vm_map_create_options(pmap_t pmap, vm_map_offset_t min,
                               vm_map_offset_t max,
  ...
  if (__improbable(startup_phase < STARTUP_SUB_ZALLOC)) {
    if (early_map_count != ~0u &&
        early_map_count != zone_count_allocated(vm_map_zone) + 1) {
    ...
    if (early_map_count != ~0u && pmap && pmap != kernel_pmap) {
      panic("allocating %dth early map for non kernel pmap", early_map_count);
    }
  }
#endif /* DEBUG || DEVELOPMENT */
  ...
  result->user_wire_limit =
      MACH_VM_MAX_ADDRESS; /* default limit is unlimited */
 ...
 */
static void vm_map_adjust_offsets(vm_map_t map, vm_map_offset_t min_off,
                                  vm_map_offset_t max_off) {
...
vm_map_size_t vm_map_adjusted_size(vm_map_t map) {
  ...
  if (map->reserved_regions == FALSE || !vm_map_is_exotic(map) ||
      map->terminated) {
    /*
...
static vm_map_entry_t
_vm_map_entry_create(struct vm_map_header *map_header __unused) {
...
#define vm_map_copy_entry_dispose(copy_entry) vm_map_entry_dispose(copy_entry)

static vm_map_entry_t vm_map_zap_first_entry(vm_map_zap_t list) {
  return list->vmz_head;
}

static vm_map_entry_t vm_map_zap_last_entry(vm_map_zap_t list) {
  assert(vm_map_zap_first_entry(list));
  return __container_of(list->vmz_tail, struct vm_map_entry, vme_next);
}

static void vm_map_zap_append(vm_map_zap_t list, vm_map_entry_t entry) {
  entry->vme_next = VM_MAP_ENTRY_NULL;
  *list->vmz_tail = entry;
  list->vmz_tail = &entry->vme_next;
}

static vm_map_entry_t vm_map_zap_pop(vm_map_zap_t list) {
...
static void vm_map_zap_dispose(vm_map_zap_t list) {
...
boolean_t first_free_is_valid(vm_map_t map) {
  if (!first_free_check) {
    return TRUE;
  }
...
void vm_map_destroy(vm_map_t map) {
  ...
  (void)vm_map_delete(map, 0x0, 0xFFFFFFFFFFFFF000ULL, flags, KMEM_GUARD_NONE,
                      &zap);
 ...
 */
boolean_t vm_map_lookup_entry(vm_map_t map, vm_map_offset_t address,
...
boolean_t vm_map_lookup_entry_or_next(vm_map_t map, vm_map_offset_t address,
                                      vm_map_entry_t *entry) /* OUT */
{
 ...
 */
__abortlike static void vm_map_range_invalid_panic(vm_map_t map,
 ...
 */
static struct mach_vm_range vm_map_get_range(vm_map_t map,
                                             vm_map_address_t *address,
                                             vm_map_kernel_flags_t *vmk_flags,
                                             vm_map_size_t size, bool *is_ptr) {
  ...
  if (map == kernel_map) {
    ...
    if (startup_phase >= STARTUP_SUB_KMEM) {
      ...
      *address = 0ull;
       ...
       */
      if ((range_id == KMEM_RANGE_ID_NONE) || (range_id > KMEM_RANGE_ID_MAX)) {
    ...
    case UMEM_RANGE_ID_LARGE_FILE:
      if (map->large_file_range.min_address !=
          map->large_file_range.max_address) {
...
kern_return_t vm_map_locate_space_anywhere(vm_map_t map, vm_map_size_t size,
  ...
  if (__improbable(map->wait_for_space)) {
     ...
     */
    assert(!map->holelistenabled && !vmk_flags.vmkf_last_free &&
           !vmk_flags.vmkf_keep_map_locked && !vmk_flags.vmkf_map_jit &&
           !vmk_flags.vmf_random_addr && *start_inout <= map->min_offset);
  } else if (vmk_flags.vmkf_last_free) {
    assert(!vmk_flags.vmkf_map_jit && !vmk_flags.vmf_random_addr);
  }
  ...
  if (__improbable(
          !vm_map_is_map_size_valid(map, size, vmk_flags.vmkf_no_soft_limit))) {
   ...
   */
  effective_range =
      vm_map_get_range(map, start_inout, &vmk_flags, size, &is_kmem_ptr_range);
...
again:
  if (vmk_flags.vmkf_last_free) {
    ...
    if (vmk_flags.vmkf_map_jit) {
      if (map->jit_entry_exists && !VM_MAP_POLICY_ALLOW_MULTIPLE_JIT(map)) {
    ...
    if (vmk_flags.vmf_random_addr) {
      ...
      if (kr != KERN_SUCCESS) {
        return kr;
             ...
             !map->disable_vmentry_reuse && map->vmmap_high_start != 0) {
  ...
  entry = vm_map_store_find_space(map, hint, limit, vmk_flags.vmkf_last_free,
                                  guard_offset, size, mask, start_inout);
 ...
 */
static kern_return_t
vm_map_locate_space_fixed(vm_map_t map, vm_map_offset_t start,
                          vm_map_size_t size, vm_map_offset_t mask,
                          vm_map_kernel_flags_t vmk_flags,
                          vm_map_entry_t *entry_out, vm_map_zap_t zap_list) {
  ...
  if (VM_MAP_PAGE_SHIFT(map) < PAGE_SHIFT && !vmk_flags.vmf_overwrite &&
      map->pmap == kernel_pmap && vmk_flags.vm_tag == VM_MEMORY_REALLOC) {
    /*
  ...
  if ((start < effective_min_offset) || (end > effective_max_offset) ||
      (start >= end)) {
    return KERN_INVALID_ADDRESS;
  }
  ...
  if (vmk_flags.vmf_overwrite) {
    ...
    remove_kr =
        vm_map_delete(map, start, end, remove_flags, KMEM_GUARD_NONE, zap_list)
            .kmr_return;
    if (remove_kr) {
 ...
 */
kern_return_t vm_map_find_space(vm_map_t map, vm_map_offset_t hint_address,
                                vm_map_size_t size, vm_map_offset_t mask,
                                vm_map_kernel_flags_t vmk_flags,
                                vm_map_entry_t *o_entry) /* OUT */
{
  ...
  kr = vm_map_locate_space_anywhere(map, size, mask, vmk_flags, &hint_address,
                                    &entry);
  if (kr != KERN_SUCCESS) {
    vm_map_unlock(map);
 ...
 */
__unused static void vm_map_pmap_enter(vm_map_t map, vm_map_offset_t addr,
  ...
  while (addr < end_addr) {
    ...
    if (m == VM_PAGE_NULL || m->vmp_busy || vm_page_is_fictitious(m) ||
        (m->vmp_unusual &&
         (VMP_ERROR_GET(m) || m->vmp_restart || m->vmp_absent))) {
      vm_object_unlock(object);
      return;
    }
    ...
    if (vm_map_pmap_enter_print) {
      ...
      printf("map: %p, addr: %llx, object: %p, offset: %llx\n", map,
             (unsigned long long)addr, object, (unsigned long long)offset);
    }
    type_of_fault = DBG_CACHE_HIT_FAULT;
    bool page_sleep_needed = false;
    kr = vm_fault_enter(
        m, map->pmap, addr, PAGE_SIZE, 0, protection, protection,
        VM_PAGE_WIRED(m), VM_KERN_MEMORY_NONE, /* tag - not wiring */
        &fault_info, NULL,                     /* need_retry */
        &type_of_fault,
...
vm_map_random_address_for_size(vm_map_t map, vm_map_offset_t *address,
  ...
  effective_range =
      vm_map_get_range(map, address, &vmk_flags, size, &is_kmem_ptr);
  ...
  while (tries < MAX_TRIES_TO_GET_RANDOM_ADDRESS) {
    ...
    random_addr = vm_map_trunc_page(effective_range.min_address +
...
static boolean_t vm_memory_malloc_no_cow(int alias) {
  uint64_t alias_mask;
...
kern_return_t vm_map_enter(vm_map_t map, vm_map_offset_t *address, /* IN/OUT */
                           vm_map_size_t size, vm_map_offset_t mask,
                           vm_map_kernel_flags_t vmk_flags, vm_object_t object,
                           vm_object_offset_t offset, boolean_t needs_copy,
                           vm_prot_t cur_protection, vm_prot_t max_protection,
                           vm_inherit_t inheritance) {
  ...
  if (vmk_flags.vmf_4gb_chunk) {
    ...
    chunk_size = (4ULL * 1024 * 1024 *
                  1024); /* max. 4GB chunks for the new allocation */
#else                    /* __LP64__ */
  ...
  if (superpage_size) {
    ...
    inheritance =
        VM_INHERIT_NONE; /* fork() children won't inherit superpages */
  }

  if ((cur_protection & VM_PROT_WRITE) && (cur_protection & VM_PROT_EXECUTE) &&
#if XNU_TARGET_OS_OSX
...
#endif
      (VM_MAP_POLICY_WX_FAIL(map) || VM_MAP_POLICY_WX_STRIP_X(map)) &&
      !entry_for_jit) {
    boolean_t vm_protect_wx_fail = VM_MAP_POLICY_WX_FAIL(map);

    DTRACE_VM3(cs_wx, uint64_t, 0, uint64_t, 0, vm_prot_t, cur_protection);
    printf("CODE SIGNING: %d[%s] %s: curprot cannot be write+execute. %s\n",
           proc_selfpid(),
           (get_bsdtask_info(current_task())
                ? proc_name_address(get_bsdtask_info(current_task()))
  ...
  if (entry_for_jit && cur_protection != VM_PROT_ALL) {
    /*
  ...
  if (resilient_codesign) {
    assert(!is_submap);
    int reject_prot =
        (needs_copy ? VM_PROT_ALLEXEC : (VM_PROT_WRITE | VM_PROT_ALLEXEC));
  ...
  if (entry_for_tpro) {
     ...
     */
    if (!(max_protection & VM_PROT_READ) || !(max_protection & VM_PROT_WRITE) ||
  ...
  if (size == 0 || (offset & MIN(VM_MAP_PAGE_MASK(map), PAGE_MASK_64)) != 0) {
    *address = 0;
    vmlp_api_end(VM_MAP_ENTER, KERN_INVALID_ARGUMENT);
    return KERN_INVALID_ARGUMENT;
  }
...
#define RETURN(value)                                                          \
  ...
  assertf(VM_MAP_PAGE_ALIGNED(*address, VM_MAP_PAGE_MASK(map)), "0x%llx",
          (uint64_t)*address);
  assertf(VM_MAP_PAGE_ALIGNED(size, VM_MAP_PAGE_MASK(map)), "0x%llx",
          (uint64_t)size);
  ...
  if (purgable &&
      (offset != 0 ||
       (object != VM_OBJECT_NULL &&
        (object->vo_size != size || object->purgable == VM_PURGABLE_DENY))
#if __LP64__
       || size > ANON_MAX_SIZE
#endif
       )) {
  ...
  if (anywhere) {
    result = vm_map_locate_space_anywhere(map, size, mask, vmk_flags, address,
                                          &entry);
    start = *address;
  } else {
    start = *address;
    result = vm_map_locate_space_fixed(map, start, size, mask, vmk_flags,
                                       &entry, &zap_old_list);
  }
  ...
  if (result == KERN_MEMORY_PRESENT) {
    ...
    for (; entry->vme_start < end; entry = entry->vme_next) {
      /*
       * Check if the mapping's attributes
       * match the existing map entry.
       */
      if (entry == vm_map_to_entry(map) || entry->vme_start != tmp_start ||
          entry->is_sub_map != is_submap || VME_OFFSET(entry) != tmp_offset ||
          entry->needs_copy != needs_copy ||
          entry->protection != cur_protection ||
          entry->max_protection != max_protection ||
          entry->inheritance != inheritance ||
          entry->iokit_acct != iokit_acct || VME_ALIAS(entry) != alias) {
       ...
       */
      if (is_submap) {
        if (VME_SUBMAP(entry) != (vm_map_t)object) {
          /* not the same submap */
          RETURN(KERN_NO_SPACE);
        }
      } else {
  ...
  if (purgable || entry_for_jit || entry_for_tpro ||
    ...
    if (object == VM_OBJECT_NULL) {
      ...
      if (malloc_no_cow_except_fork && !purgable && !entry_for_jit &&
          !entry_for_tpro && vm_memory_malloc_no_cow(user_alias)) {
    ...
    /* no coalescing if address space uses sub-pages */
  } else if ((is_submap == FALSE) && (object == VM_OBJECT_NULL) &&
             (entry != vm_map_to_entry(map)) && (entry->vme_end == start) &&
             (!entry->is_shared) && (!entry->is_sub_map) &&
             (!entry->in_transition) && (!entry->needs_wakeup) &&
             ...
             !((entry->protection & VM_PROT_EXECUTE) && entry->vme_permanent) &&
...
#endif
             (!entry->csm_associated) && (entry->iokit_acct == iokit_acct) &&
             (!entry->vme_resilient_codesign) &&
             (!entry->vme_resilient_media) && (!entry->vme_atomic) &&
             (entry->vme_no_copy_on_read == no_copy_on_read) &&

             ((entry->vme_end - entry->vme_start) + size <=
              (user_alias == VM_MEMORY_REALLOC ? ANON_CHUNK_SIZE
                                               : NO_COALESCE_LIMIT)) &&

             (entry->wired_count == 0)) { /* implies user_wired_count == 0 */
    if (vm_object_coalesce(VME_OBJECT(entry), VM_OBJECT_NULL, VME_OFFSET(entry),
      ...
      if (__improbable(vm_debug_events)) {
        DTRACE_VM5(map_entry_extend, vm_map_t, map, vm_map_entry_t, entry,
                   vm_address_t, entry->vme_start, vm_address_t, entry->vme_end,
                   vm_address_t, end);
      }
      entry->vme_end = end;
      if (map->holelistenabled) {
  ...
  for (tmp2_start = start; tmp2_start < end; tmp2_start += step) {
    ...
    if (!is_submap && object == VM_OBJECT_NULL && size > chunk_size &&
        max_protection != VM_PROT_NONE && superpage_size == 0) {
      tmp_end = tmp_start + chunk_size;
    } else {
      tmp_end = tmp2_end;
    }
    do {
      if (!is_submap && object != VM_OBJECT_NULL && object->internal &&
          offset + (tmp_end - tmp_start) > object->vo_size) {
        //				printf("FBDP object %p size 0x%llx
        //overmapping offset 0x%llx size 0x%llx\n", object, object->vo_size,
        //offset, (uint64_t)(tmp_end - tmp_start));
        DTRACE_VM5(vm_map_enter_overmap, vm_map_t, map, vm_map_address_t,
                   tmp_start, vm_map_address_t, tmp_end, vm_object_offset_t,
                   offset, vm_object_size_t, object->vo_size);
      }
      new_entry = vm_map_entry_insert(
          map, entry, tmp_start, tmp_end, object, offset, vmk_flags, needs_copy,
      ...
      if (resilient_media && (object == VM_OBJECT_NULL || object->internal)) {
      ...
      if (!is_submap && object != VM_OBJECT_NULL && object->internal &&
      ...
      } else if (!is_submap && iokit_acct && object != VM_OBJECT_NULL &&
                 object->internal) {
        ...
        new_entry->use_pmap = FALSE;
        DTRACE_VM4(vm_map_iokit_mapped_region, vm_map_t, map, vm_map_offset_t,
                   new_entry->vme_start, vm_map_offset_t, new_entry->vme_end,
                   int, VME_ALIAS(new_entry));
        vm_map_iokit_mapped_region(map,
      ...
      if (is_submap) {
        ...
        if (use_pmap && submap->pmap == NULL) {
          ...
          submap->pmap = pmap_create_options(
              ledger, 0,
              submap_is_64bit ? PMAP_CREATE_64BIT | PMAP_CREATE_NESTED
                              : PMAP_CREATE_NESTED);
          if (submap->pmap == NULL) {
             ...
             * need to happen here.
             *
             * We only enter the parent if-block if use_pmap is set to true,
             * which is based on vmkf_nested_pmap. This flag is only set by two
             * functions, vm_shared_region_enter, and vm_commpage_enter. The
             * former performs a shared region lookup, which uses
             * vm_shared_region_create. This path already creates a pmap, so
             * submap->pmap != NULL. The latter doesn't go through the VM layer
             * on arm64 systems anymore. As a result, there is no case on arm64
             * where a nested pmap is actually in this path.
             */
        ...
        if (use_pmap && submap->pmap != NULL) {
          if (VM_MAP_PAGE_SHIFT(map) != VM_MAP_PAGE_SHIFT(submap)) {
            DEBUG4K_ERROR(
                ...
                VM_MAP_PAGE_SHIFT(map), submap, VM_MAP_PAGE_SHIFT(submap));
            kr = KERN_FAILURE;
          } else {
            kr = pmap_nest(map->pmap, submap->pmap, tmp_start,
                           tmp_end - tmp_start);
          }
          if (kr != KERN_SUCCESS) {
            printf("vm_map_enter: "
                   ...
                   "error 0x%x\n",
                   (long long)tmp_start, (long long)tmp_end, kr);
          } else {
            /* we're now nested ! */
            new_entry->use_pmap = TRUE;
            pmap_empty = FALSE;
      ...
      if (superpage_size) {
        ...
        kr = cpm_allocate(SUPERPAGE_SIZE, &pages, 0, SUPERPAGE_NBASEPAGES - 1,
                          TRUE, 0);
        if (kr != KERN_SUCCESS) {
    ...
    } while (tmp_end != tmp2_end && (tmp_start = tmp_end) &&
             (tmp_end = (tmp2_end - tmp_end > chunk_size) ? tmp_end + chunk_size
                                                          : tmp2_end));
  }
...
BailOut:
    ...
    if (map->size_limit != RLIM_INFINITY && map->size > map->size_limit) {
      ...
      printf("%d[%s] %s: map size 0x%llx over RLIMIT_AS 0x%llx\n",
             ...
             (get_bsdtask_info(current_task())
                  ? proc_name_address(get_bsdtask_info(current_task()))
                  : "?"),
             __FUNCTION__, (uint64_t)map->size, (uint64_t)map->size_limit);
      DTRACE_VM2(vm_map_enter_RLIMIT_AS, vm_map_size_t, map->size, uint64_t,
                 map->size_limit);
      ...
      printf("%d[%s] %s: map size 0x%llx over RLIMIT_DATA 0x%llx\n",
             ...
             (get_bsdtask_info(current_task())
                  ? proc_name_address(get_bsdtask_info(current_task()))
                  : "?"),
             __FUNCTION__, (uint64_t)map->size, (uint64_t)map->data_limit);
      DTRACE_VM2(vm_map_enter_RLIMIT_DATA, vm_map_size_t, map->size, uint64_t,
                 map->data_limit);
  ...
  if (result == KERN_SUCCESS) {
...
#if DEBUG
    if (pmap_empty && !(vmk_flags.vmkf_no_pmap_check)) {
      assert(pmap_is_empty(map->pmap, *address, *address + size));
    }
#endif /* DEBUG */
    ...
    if (!is_submap && object != VM_OBJECT_NULL && object->named &&
  ...
  if (result == KERN_SUCCESS) {
    ...
    if ((map->wiring_required) || (superpage_size)) {
      ...
      kr = vm_map_wire_nested(map, start, end, cur_protection,
                              VM_KERN_MEMORY_MLOCK, TRUE, PMAP_NULL, 0, NULL);
      result = kr;
    }
  }
  ...
  if (result != KERN_SUCCESS) {
    if (new_mapping_established) {
      ...
      (void)vm_map_delete(map, *address, *address + size, remove_flags,
    ...
    if (vm_map_zap_first_entry(&zap_old_list)) {
      ...
      if (vm_map_lookup_entry(map, start, &entry1) ||
          vm_map_lookup_entry(map, end, &entry2) || entry1 != entry2) {
        /*
        ...
        while ((entry2 = vm_map_zap_pop(&zap_old_list))) {
          ...
          entry_size = (entry2->vme_end - entry2->vme_start);
...
static kern_return_t vm_map_enter_adjust_offset(vm_object_offset_t *obj_offs,
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_enter_mem_object_sanitize(
    vm_map_t target_map, vm_map_offset_ut address_u,
    vm_map_size_ut initial_size_u, vm_map_offset_ut mask_u,
    vm_object_offset_ut offset_u, vm_prot_ut cur_protection_u,
    vm_prot_ut max_protection_u, vm_inherit_ut inheritance_u,
    vm_map_kernel_flags_t vmk_flags, ipc_port_t port,
    vm_map_address_t *map_addr, vm_map_size_t *map_size, vm_map_offset_t *mask,
    vm_object_offset_t *obj_offs, vm_object_offset_t *obj_end,
    vm_object_size_t *obj_size, vm_prot_t *cur_protection,
    vm_prot_t *max_protection, vm_inherit_t *inheritance) {
  kern_return_t result;

  result = vm_sanitize_cur_and_max_prots(
      cur_protection_u, max_protection_u, VM_SANITIZE_CALLER_ENTER_MEM_OBJ,
      target_map, VM_PROT_IS_MASK, cur_protection, max_protection);
  ...
  if (vmk_flags.vmf_fixed) {
    ...
    result = vm_sanitize_addr_size(
        address_u, initial_size_u, VM_SANITIZE_CALLER_ENTER_MEM_OBJ, target_map,
  ...
  *obj_size = vm_object_round_page(*map_size);
  ...
  if (IP_VALID(port)) {
    result = vm_sanitize_addr_size(offset_u, *obj_size,
                                   VM_SANITIZE_CALLER_ENTER_MEM_OBJ, PAGE_MASK,
...
kern_return_t vm_map_enter_mem_object(
    vm_map_t target_map, vm_map_offset_ut *address_u,
    vm_map_size_ut initial_size_u, vm_map_offset_ut mask_u,
    vm_map_kernel_flags_t vmk_flags, ipc_port_t port,
    vm_object_offset_ut offset_u, boolean_t copy, vm_prot_ut cur_protection_u,
    vm_prot_ut max_protection_u, vm_inherit_ut inheritance_u,
    upl_page_list_ptr_t page_list, unsigned int page_list_count) {
  vm_map_offset_t mask;
   ...
   */
  if ((target_map == VM_MAP_NULL) || (try_prefault && (copy || !page_list))) {
  ...
  result = vm_map_enter_mem_object_sanitize(
      target_map, *address_u, initial_size_u, mask_u, offset_u,
      cur_protection_u, max_protection_u, inheritance_u, vmk_flags, port,
      &map_addr, &map_size, &mask, &obj_offs, &obj_end, &obj_size,
  ...
  if (!IP_VALID(port)) {
    ...
    if (vmk_flags.vmf_return_data_addr || vmk_flags.vmf_return_4k_data_addr) {
      result = vm_map_enter_adjust_offset(&obj_offs, &obj_end,
                                          named_entry->data_offset);
    ...
    if (named_entry->is_copy && (obj_size != named_entry->size) &&
    ...
    // rdar://130307561 (Combine copy, object, and submap fields of
    // vm_named_entry into an enum)
    assert(named_entry->is_copy || named_entry->is_object ||
           named_entry->is_sub_map);
    ...
    if (named_entry->is_sub_map) {
      ...
      result = vm_map_enter(target_map, &map_addr, map_size, mask, vmk_flags,
                            (vm_object_t)(uintptr_t)submap, obj_offs, copy,
                            cur_protection, max_protection, inheritance);
      if (result != KERN_SUCCESS) {
      ...
      if (submap->mapped_in_other_pmaps == FALSE &&
          ...
          vm_map_pmap(submap) != vm_map_pmap(target_map)) {
        /*
    ...
    if (named_entry->is_copy) {
      ...
      int allowed_flags = VM_FLAGS_FIXED | VM_FLAGS_ANYWHERE |
                          VM_FLAGS_OVERWRITE | VM_FLAGS_RETURN_4K_DATA_ADDR |
      ...
      if (VM_MAP_PAGE_SHIFT(target_map) != copy_map->cpy_hdr.page_shift) {
        DEBUG4K_SHARE("copy_map %p offset %llx size 0x%llx pgshift %d -> "
                      "target_map %p pgshift %d\n",
                      copy_map, obj_offs, (uint64_t)map_size,
                      copy_map->cpy_hdr.page_shift, target_map,
      ...
      if (copy_map->cpy_hdr.page_shift != VM_MAP_PAGE_SHIFT(target_map)) {
        ...
        kr = vm_map_copy_adjust_to_target(
            copy_map, obj_offs, initial_size, target_map, copy,
            &target_copy_map, &overmap_start, &overmap_end, &trimmed_start);
        if (kr != KERN_SUCCESS) {
          named_entry_unlock(named_entry);
      ...
      vm_map_kernel_flags_and_vmflags(
          &rsv_flags,
          (VM_FLAGS_FIXED | VM_FLAGS_ANYWHERE | VM_FLAGS_OVERWRITE |
           VM_FLAGS_RETURN_4K_DATA_ADDR | VM_FLAGS_RETURN_DATA_ADDR));

      /* reserve a contiguous range */
      kr = vm_map_enter(target_map, &map_addr,
                        vm_map_round_page(target_size, map_mask), mask,
                        rsv_flags, VM_OBJECT_NULL, 0, FALSE, /* copy */
                        cur_protection, max_protection, inheritance);
      if (kr != KERN_SUCCESS) {
      ...
      for (copy_entry = vm_map_copy_first_entry(target_copy_map);
        ...
        /* sanity check */
        if ((copy_addr + copy_size) > (map_addr + overmap_start + overmap_end +
        ...
        if (copy_entry->is_sub_map) {
          ...
          if (!copy && copy_object != VM_OBJECT_NULL &&
             ...
             */
            // assert(copy_object->copy_strategy ==
            ...
            if (!copy_entry->needs_copy && is_writable) {
              ...
              vm_object_pmap_protect(copy_object, copy_offset, copy_size,
                                     PMAP_NULL, PAGE_SIZE, 0, prot);
            }
          ...
          if (copy_object != VM_OBJECT_NULL && copy_object->named &&
            ...
            if (copy_object->named && pager != MEMORY_OBJECT_NULL &&
                copy_object->copy_strategy != MEMORY_OBJECT_COPY_NONE) {
          ...
          if (copy && copy_object != VM_OBJECT_NULL) {
            ...
            result = vm_object_copy_strategically(
                copy_object, copy_offset, copy_size, false, /* forking */
                &new_object, &new_offset, &do_copy);
            ...
            if (result == KERN_MEMORY_RESTART_COPY) {
              ...
              success = vm_object_copy_quickly(
                  new_object, new_offset, copy_size, &src_needs_copy, &do_copy);
              assert(success);
              result = KERN_SUCCESS;
            }
            if (result != KERN_SUCCESS) {
        ...
        vm_map_kernel_flags_set_vmflags(
            &vmk_remap_flags, vm_map_kernel_flags_vmflags(vmk_flags),
        ...
        kr = vm_map_enter(
            target_map, &copy_addr, copy_size, (vm_map_offset_t)0,
            vmk_remap_flags, copy_object, copy_offset,
            ((copy_object == NULL) ? FALSE : (copy || copy_entry->needs_copy)),
            cur_protection, max_protection, inheritance);
        if (kr != KERN_SUCCESS) {
      ...
      if (kr == KERN_SUCCESS) {
        if (overmap_start) {
          DEBUG4K_SHARE(
              "map %p map_addr 0x%llx offset_in_mapping 0x%llx overmap_start "
              "0x%llx -> *address 0x%llx\n",
              target_map, (uint64_t)map_addr, (uint64_t)offset_in_mapping,
              (uint64_t)overmap_start,
              (uint64_t)(map_addr + offset_in_mapping + overmap_start));
        }
        offset_in_mapping += overmap_start;
      } else if (!vmk_flags.vmf_overwrite) {
        /* deallocate the contiguous range */
        vm_map_remove(target_map, map_addr, map_addr + map_size);
      }
      result = kr;
      goto out;
    }
    ...
    if (named_entry->is_object) {
      ...
      if (vmk_flags.vmf_return_data_addr || vmk_flags.vmf_return_4k_data_addr) {
        ...
        map_size =
            vm_map_round_page(initial_size + offset_in_mapping, map_mask);
      }
  ...
  if (object != VM_OBJECT_NULL && object->named &&
      object->pager != MEMORY_OBJECT_NULL &&
      object->copy_strategy != MEMORY_OBJECT_COPY_NONE) {
    memory_object_t pager;
    vm_prot_t pager_prot;
  ...
  if (copy) {
    ...
    result = vm_object_copy_strategically(object, obj_offs, map_size,
                                          false, /* forking */
                                          &new_object, &new_offset, &copy);
    ...
    if (result == KERN_MEMORY_RESTART_COPY) {
      ...
      new_object = object;
      new_offset = obj_offs;
      success = vm_object_copy_quickly(new_object, new_offset, map_size,
                                       &src_needs_copy, &copy);
      assert(success);
      result = KERN_SUCCESS;
    }
  ...
  result = vm_map_enter(target_map, &map_addr, map_size, (vm_map_offset_t)mask,
                        vmk_flags, object, obj_offs, copy, cur_protection,
      ...
      if (!UPL_VALID_PAGE(page_list, i)) {
        ...
        if (object->phys_contiguous) {
          ...
          if (p == VM_PAGE_NULL) {
            ...
            break;
          } else if (VM_PAGE_GET_PHYS_PAGE(p) != UPL_PHYS_PAGE(page_list, i)) {
            /*
            ...
            assertf(VM_PAGE_GET_PHYS_PAGE(p) == UPL_PHYS_PAGE(page_list, i),
                    ...
                    p, VM_PAGE_GET_PHYS_PAGE(p), UPL_PHYS_PAGE(page_list, i));
         ...
         * something critical.
         */
        kr = pmap_enter_object_options_check(
            target_map->pmap, va, 0, object, UPL_PHYS_PAGE(page_list, i),
            cur_protection, VM_PROT_NONE, TRUE, pmap_options);
        if (kr != KERN_SUCCESS) {
...
kern_return_t vm_map_enter_mem_object_prefault(
    vm_map_t target_map, vm_map_offset_ut *address, vm_map_size_ut initial_size,
    vm_map_offset_ut mask, vm_map_kernel_flags_t vmk_flags, ipc_port_t port,
    vm_object_offset_ut offset, vm_prot_ut cur_protection,
    vm_prot_ut max_protection, upl_page_list_ptr_t page_list,
    unsigned int page_list_count) {
  /* range_id is set by vm_map_enter_mem_object */
  return vm_map_enter_mem_object(target_map, address, initial_size, mask,
                                 vmk_flags, port, offset, FALSE, cur_protection,
                                 max_protection, VM_INHERIT_DEFAULT, page_list,
                                 page_list_count);
}
...
vm_map_enter_mem_object_control_sanitize(
    ...
    vm_object_offset_ut offset_u, vm_prot_ut cur_protection_u,
    vm_prot_ut max_protection_u, vm_inherit_ut inheritance_u,
    vm_map_kernel_flags_t vmk_flags, vm_map_address_t *map_addr,
    vm_map_size_t *map_size, vm_map_offset_t *mask,
    vm_object_offset_t *obj_offs, vm_object_offset_t *obj_end,
    vm_object_size_t *obj_size, vm_prot_t *cur_protection,
    vm_prot_t *max_protection, vm_inherit_t *inheritance) {
  kern_return_t kr;
  ...
  kr = vm_sanitize_cur_and_max_prots(
      cur_protection_u, max_protection_u, VM_SANITIZE_CALLER_ENTER_MEM_OBJ_CTL,
      target_map, cur_protection, max_protection);
...
kern_return_t vm_map_enter_mem_object_control(
    vm_map_t target_map, vm_map_offset_ut *address_u,
    vm_map_size_ut initial_size_u, vm_map_offset_ut mask_u,
    vm_map_kernel_flags_t vmk_flags, memory_object_control_t control,
    vm_object_offset_ut offset_u, boolean_t needs_copy,
    vm_prot_ut cur_protection_u, vm_prot_ut max_protection_u,
  ...
  kr = vm_map_enter_mem_object_control_sanitize(
      target_map, *address_u, initial_size_u, mask_u, offset_u,
      cur_protection_u, max_protection_u, inheritance_u, vmk_flags, &map_addr,
      &map_size, &mask, &obj_offs, &obj_end, &obj_size, &cur_protection,
      &max_protection, &inheritance);
  if (__improbable(kr != KERN_SUCCESS)) {
  ...
  if (needs_copy) {
    ...
    result = vm_object_copy_strategically(
        object, obj_offs, obj_size, false, /* forking */
        &new_object, &new_offset, &needs_copy);
    ...
    if (result == KERN_MEMORY_RESTART_COPY) {
      ...
      new_object = object;
      new_offset = obj_offs;
      success = vm_object_copy_quickly(new_object, new_offset, obj_size,
                                       &src_needs_copy, &needs_copy);
      assert(success);
      result = KERN_SUCCESS;
    }
    /*
  ...
  result = vm_map_enter(target_map, &map_addr, map_size, (vm_map_offset_t)mask,
                        vmk_flags, object, obj_offs, needs_copy, cur_protection,
                        max_protection, inheritance);
...
static void vm_map_clip_unnest(vm_map_t map, vm_map_entry_t entry,
  ...
  if (pmap_adjust_unnest_parameters(map->pmap, &start_unnest, &end_unnest)) {
    ...
    log_unnest_badness(
        map, old_start_unnest, old_end_unnest, VME_SUBMAP(entry)->is_nested_map,
        (entry->vme_start + VME_SUBMAP(entry)->lowest_unnestable_start -
         VME_OFFSET(entry)));
  }

  if (entry->vme_start > start_unnest || entry->vme_end < end_unnest) {
  ...
  if (start_unnest > entry->vme_start) {
    _vm_map_clip_start(&map->hdr, entry, start_unnest);
    if (map->holelistenabled) {
  ...
  if (entry->vme_end > end_unnest) {
    _vm_map_clip_end(&map->hdr, entry, end_unnest);
    if (map->holelistenabled) {
      vm_map_store_update_first_free(map, NULL, FALSE);
    } else {
      vm_map_store_update_first_free(map, map->first_free, FALSE);
  ...
  pmap_unnest(map->pmap, entry->vme_start, entry->vme_end - entry->vme_start);
  if ((map->mapped_in_other_pmaps) &&
      os_ref_get_count_raw(&map->map_refcnt) != 0) {
    /* clean up parent map/maps */
    vm_map_submap_pmap_clean(map, entry->vme_start, entry->vme_end,
                             VME_SUBMAP(entry), VME_OFFSET(entry));
  }
...
__vm_map_clip_atomic_entry_panic(vm_map_t map, vm_map_entry_t entry,
                                 vm_map_offset_t where) {
  panic("vm_map_clip(%p): Attempting to clip an atomic VM map entry "
        "%p [0x%llx:0x%llx] at 0x%llx",
        map, entry, (uint64_t)entry->vme_start, (uint64_t)entry->vme_end,
        (uint64_t)where);
}

__abortlike static void __vm_map_clip_sealed_panic(vm_map_t map,
                                                   vm_map_entry_t entry,
                                                   vm_map_offset_t where) {
...
void vm_map_clip_start(vm_map_t map, vm_map_entry_t entry,
...
#ifndef NO_NESTED_PMAP
  if (entry->is_sub_map && entry->use_pmap && startaddr >= entry->vme_start) {
  ...
  if (startaddr > entry->vme_start) {
    if (!entry->is_sub_map && VME_OBJECT(entry) &&
      ...
      pmap_remove(map->pmap, (addr64_t)(entry->vme_start),
    ...
    DTRACE_VM5(vm_map_clip_start, vm_map_t, map, vm_map_offset_t,
               entry->vme_start, vm_map_offset_t, entry->vme_end,
               vm_map_offset_t, startaddr, int, VME_ALIAS(entry));
...
static void _vm_map_clip_start(struct vm_map_header *map_header,
  ...
  if (__improbable(start >= entry->vme_end)) {
    panic("mapHdr %p entry %p start 0x%llx end 0x%llx new start 0x%llx",
          map_header, entry, entry->vme_start, entry->vme_end, start);
  }
...
void vm_map_clip_end(vm_map_t map, vm_map_entry_t entry,
  ...
  if (entry->is_sub_map && entry->use_pmap) {
    ...
    end_unnest = (endaddr + pmap_shared_region_size_min(map->pmap) - 1) &
  ...
  if (endaddr < entry->vme_end) {
    ...
    DTRACE_VM5(vm_map_clip_end, vm_map_t, map, vm_map_offset_t,
               entry->vme_start, vm_map_offset_t, entry->vme_end,
               vm_map_offset_t, endaddr, int, VME_ALIAS(entry));
...
static void _vm_map_clip_end(struct vm_map_header *map_header,
  ...
  if (__improbable(end <= entry->vme_start)) {
    panic("mapHdr %p entry %p start 0x%llx end 0x%llx new end 0x%llx",
          map_header, entry, entry->vme_start, entry->vme_end, end);
  }
  assert(entry->vme_start < end);
  new_entry->vme_start = entry->vme_end = end;
  VME_OFFSET_SET(new_entry, VME_OFFSET(new_entry) + (end - entry->vme_start));
  assert(new_entry->vme_start < new_entry->vme_end);
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_protect_sanitize(vm_map_t map, vm_map_offset_ut start_u,
                        vm_map_offset_ut end_u, vm_prot_ut new_prot_u,
                        vm_map_offset_t *start, vm_map_offset_t *end,
                        vm_prot_t *new_prot) {
  kern_return_t kr;
  vm_map_size_t size;
  ...
  kr = vm_sanitize_prot(new_prot_u, VM_SANITIZE_CALLER_VM_MAP_PROTECT, map,
                        VM_PROT_COPY, new_prot);
  if (__improbable(kr != KERN_SUCCESS)) {
    return kr;
  }
 ...
 */
kern_return_t vm_map_protect(vm_map_t map, vm_map_offset_ut start_u,
                             vm_map_offset_ut end_u, boolean_t set_max,
  ...
  kr = vm_map_protect_sanitize(map, start_u, end_u, new_prot_u, &start, &end,
                               &new_prot);
  if (__improbable(kr != KERN_SUCCESS)) {
  ...
  if (new_prot & VM_PROT_COPY) {
    ...
    if ((new_prot & VM_PROT_ALLEXEC) && map->pmap != kernel_pmap &&
        (vm_map_cs_enforcement(map)
      ...
      DTRACE_VM3(cs_wx, uint64_t, (uint64_t)start, uint64_t, (uint64_t)end,
                 vm_prot_t, new_prot);
      printf("CODE SIGNING: %d[%s] %s:%d(0x%llx,0x%llx,0x%x) can't have both "
    ...
    new_start = start;
    kr = vm_map_remap(map, vm_sanitize_wrap_addr_ref(&new_start), end - start,
                      0,                        /* mask */
                      kflags, map, start, TRUE, /* copy-on-write remapping! */
...
restart_after_unlock:
  ...
  while (1) {
    ...
    if (entry->superpage_size &&
        (start & (SUPERPAGE_SIZE - 1))) { /* extend request to whole entry */
  ...
  while ((current != vm_map_to_entry(map)) && (current->vme_start < end)) {
    ...
    /* Allow max mask to include execute prot bits if this map doesn't enforce
     * CS */
    if (set_max && (new_prot & VM_PROT_ALLEXEC) &&
        !vm_map_cs_enforcement(map)) {
      new_max = (new_max & ~VM_PROT_ALLEXEC) | (new_prot & VM_PROT_ALLEXEC);
    }
#elif CODE_SIGNING_MONITOR
    if (set_max && (new_prot & VM_PROT_EXECUTE) &&
        (csm_address_space_exempt(map->pmap) == KERN_SUCCESS)) {
    ...
    if (current->used_for_jit &&
        pmap_has_prot_policy(map->pmap, current->translated_allow_execute,
                             current->protection)) {
      vm_map_unlock(map);
      vmlp_api_end(VM_MAP_PROTECT, KERN_PROTECTION_FAILURE);
      return KERN_PROTECTION_FAILURE;
    ...
    if ((new_prot & VM_PROT_WRITE) && (new_prot & VM_PROT_ALLEXEC) &&
#if XNU_TARGET_OS_OSX
        map->pmap != kernel_pmap &&
        (vm_map_cs_enforcement(map)
#if __arm64__
      ...
      DTRACE_VM3(cs_wx, uint64_t, (uint64_t)current->vme_start, uint64_t,
                 (uint64_t)current->vme_end, vm_prot_t, new_prot);
      printf("CODE SIGNING: %d[%s] %s:%d(0x%llx,0x%llx,0x%x) can't have both "
             "write and exec at the same time\n",
             proc_selfpid(),
             (get_bsdtask_info(current_task())
             ...
             __FUNCTION__, __LINE__,
#if DEVELOPMENT || DEBUG
             (uint64_t)current->vme_start, (uint64_t)current->vme_end,
#else  /* DEVELOPMENT || DEBUG */
             (uint64_t)0, (uint64_t)0,
#endif /* DEVELOPMENT || DEBUG */
             new_prot);
  ...
  while ((current != vm_map_to_entry(map)) && (current->vme_start < end)) {
    ...
    if (current->csm_associated && vm_log_xnu_user_debug) {
      printf("FBDP %d[%s] %s(0x%llx,0x%llx,0x%x) on map %p entry %p "
             "[0x%llx:0x%llx 0x%x/0x%x] csm_associated\n",
             proc_selfpid(),
             (get_bsdtask_info(current_task())
                  ? proc_name_address(get_bsdtask_info(current_task()))
                  : "?"),
             __FUNCTION__, (uint64_t)start, (uint64_t)end, new_prot, map,
             current, current->vme_start, current->vme_end, current->protection,
    ...
    if (/* a !csm_associated mapping becoming executable */
        ((!current->csm_associated && !(old_prot & VM_PROT_EXECUTE) &&
          (current->protection & VM_PROT_EXECUTE)) ||
         /* a csm_associated mapping becoming writable */
         (current->csm_associated && !(old_prot & VM_PROT_WRITE) &&
      ...
      if (vm_log_xnu_user_debug) {
        printf("FBDP %d[%s] %s:%d map %p entry %p [ 0x%llx 0x%llx ] prot 0x%x "
               "-> 0x%x cs_associate -> %d user_debug=%d\n",
               proc_selfpid(),
               (get_bsdtask_info(current_task())
                    ? proc_name_address(get_bsdtask_info(current_task()))
                    : "?"),
               __FUNCTION__, __LINE__, map, current, current->vme_start,
               current->vme_end, old_prot, current->protection, kr,
               current->vme_xnu_user_debug);
      }
#endif /* DEVELOPMENT || DEBUG */
    }
    ...
    if (current->protection != old_prot) {
      ...
      if (current->is_sub_map || (VME_OBJECT(current) == NULL) ||
          (VME_OBJECT(current) != compressor_object)) {
        prot &= ~VM_PROT_WRITE;
      } else {
...
#if DEVELOPMENT || DEBUG
      if (!(old_prot & VM_PROT_EXECUTE) && (prot & VM_PROT_EXECUTE) &&
          panic_on_unsigned_execute && (proc_selfcsflags() & CS_KILL)) {
        panic("vm_map_protect(%p,0x%llx,0x%llx) old=0x%x new=0x%x - "
              "<rdar://23770418> code-signing bypass?",
              map, (uint64_t)current->vme_start, (uint64_t)current->vme_end,
              old_prot, prot);
      }
#endif /* DEVELOPMENT || DEBUG */

      if (pmap_has_prot_policy(map->pmap, current->translated_allow_execute,
                               prot)) {
        if (current->wired_count) {
          panic("vm_map_protect(%p,0x%llx,0x%llx) new=0x%x wired=%x", map,
                (uint64_t)current->vme_start, (uint64_t)current->vme_end, prot,
                current->wired_count);
        }
      ...
      if (current->is_sub_map && current->use_pmap) {
        pmap_protect(VME_SUBMAP(current)->pmap, current->vme_start,
                     current->vme_end, prot);
      } else {
        pmap_protect_options(map->pmap, current->vme_start, current->vme_end,
                             prot, pmap_options, NULL);
      }
    }
    current = current->vme_next;
  }
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_inherit_sanitize(vm_map_t map, vm_map_offset_ut start_u,
                        vm_map_offset_ut end_u, vm_inherit_ut new_inheritance_u,
                        vm_map_offset_t *start, vm_map_offset_t *end,
                        vm_inherit_t *new_inheritance) {
  kern_return_t kr;
  vm_map_size_t size;

  kr = vm_sanitize_inherit(new_inheritance_u, VM_SANITIZE_CALLER_VM_MAP_INHERIT,
                           new_inheritance);
  if (__improbable(kr != KERN_SUCCESS)) {
    return kr;
  }
 ...
 */
kern_return_t vm_map_inherit(vm_map_t map, vm_map_offset_ut start_u,
                             vm_map_offset_ut end_u,
  ...
  kr = vm_map_inherit_sanitize(map, start_u, end_u, new_inheritance_u, &start,
                               &end, &new_inheritance);
  if (__improbable(kr != KERN_SUCCESS)) {
    kr = vm_sanitize_get_kr(kr);
...
/*
 * Update the accounting for the amount of wired memory in this map.  If the
 * user has exceeded the defined limits, then we fail.  Wiring on behalf of the
 * kernel never fails.
 */

static kern_return_t add_wire_counts(vm_map_t map, vm_map_entry_t entry,
                                     boolean_t user_wire) {
  vm_map_size_t size;
  ...
  if (user_wire) {
    ...
    /*
     * We're wiring memory at the request of the user.  Check if this is the
     * first time the user is wiring this map entry.
     */
    ...
    if (entry->user_wired_count == 0) {
      ...
      /*
       * Since this is the first time the user is wiring this map entry, check
       * to see if we're exceeding the user wire limits.  There is a per map
       * limit which is the smaller of either the process's rlimit or the global
       * vm_per_task_user_wire_limit which caps this value.  There is also a
       * system-wide limit on the amount of memory all users can wire.  If the
       * user is over either limit, then we fail.
       */

      if (size + map->user_wire_size >
              MIN(map->user_wire_limit, vm_per_task_user_wire_limit) ||
        ...
        if (size + ptoa_64(total_wire_count) > vm_global_user_wire_limit) {
          ...
          if (panic_on_mlock_failure) {
            panic("mlock: Over global wire limit. %llu bytes wired and "
                  "requested to wire %llu bytes more",
...
#if DEVELOPMENT || DEBUG
          if (panic_on_mlock_failure) {
            panic("mlock: Over process wire limit. %llu bytes wired and "
                  "requested to wire %llu bytes more",
      ...
      /*
       * The first time the user wires an entry, we also increment the
       * wired_count and add this to the total that has been wired in the map.
       */
...
/*
 * Update the memory wiring accounting now that the given map entry is being
 * unwired.
 */

static void subtract_wire_counts(vm_map_t map, vm_map_entry_t entry,
                                 boolean_t user_wire) {
  if (user_wire) {
    /*
     * We're unwiring memory at the request of the user.  See if we're removing
     * the last user wire reference.
     */

    if (entry->user_wired_count == 1) {
      /*
       * We're removing the last user wire reference.  Decrement the wired_count
       * and the total user wired memory for this map.
       */
...
static kern_return_t vm_map_wire_nested(vm_map_t map, vm_map_offset_t start,
...
#if XNU_TARGET_OS_OSX
  if (map->pmap == kernel_pmap || !vm_map_cs_enforcement(map)) {
    extra_prots &= ~VM_PROT_COPY_FAIL_IF_EXECUTABLE;
  }
#endif /* XNU_TARGET_OS_OSX */
#if CODE_SIGNING_MONITOR
  ...
  while ((entry != vm_map_to_entry(map)) && (s < end)) {
    ...
    if (entry->in_transition) {
       ...
       */
      wait_result =
          vm_map_entry_wait(map, (user_wire) ? THREAD_ABORTSAFE : THREAD_UNINT);
    ...
    if (entry->is_sub_map) {
      ...
      if (map_pmap == NULL) {
        ...
        if (!vm_map_lookup_entry(map, local_start, &local_entry)) {
          rc = KERN_FAILURE;
          goto done;
        }
      ...
      vm_map_unlock(map);
      rc = vm_map_wire_nested(sub_map, sub_start, sub_end, caller_prot, tag,
                              user_wire, pmap, pmap_addr, NULL);
       ...
       */
      if (!vm_map_lookup_entry(map, local_start, &first_entry)) {
      ...
      last_timestamp = map->timestamp;
      while ((entry != vm_map_to_entry(map)) && (entry->vme_start < e)) {
...
#endif /* MACH_ASSERT */
      DTRACE_VM2(cs_executable_wire, uint64_t, (uint64_t)entry->vme_start,
    ...
    if (entry->needs_copy) {
      ...
      assert(entry->use_pmap);
    } else if (VME_OBJECT(entry)->copy_strategy ==
               MEMORY_OBJECT_COPY_SYMMETRIC) {
      if (wire_and_extract) {
        /*
         * We're supposed to share with the original
       ...
       */
      //			printf("FBDP %s:%d map %p entry %p [ 0x%llx
      //0x%llx ] s 0x%llx end 0x%llx wire&extract=%d\n", __FUNCTION__, __LINE__,
      //map, entry, (uint64_t)entry->vme_start, (uint64_t)entry->vme_end,
      //(uint64_t)s, (uint64_t)end, wire_and_extract);
      vm_map_clip_start(map, entry, s);
      vm_map_clip_end(map, entry, end);
      /* recompute "size" */
      size = entry->vme_end - entry->vme_start;
      ...
      if (VME_OBJECT(entry) != orig_object) {
         ...
         * mapping.
         */
        vm_object_pmap_protect(orig_object, orig_offset, size, map->pmap,
                               VM_MAP_PAGE_SIZE(map), entry->vme_start,
    ...
    if (VME_OBJECT(entry)->copy_strategy == MEMORY_OBJECT_COPY_SYMMETRIC) {
      ...
      if (object->copy_strategy == MEMORY_OBJECT_COPY_SYMMETRIC) {
        assertf(vm_object_round_page(VME_OFFSET(entry) + size) -
                        vm_object_trunc_page(VME_OFFSET(entry)) ==
                    object->vo_size,
                "object %p size 0x%llx entry %p [0x%llx:0x%llx:0x%llx] size "
                "0x%llx\n",
                object, (uint64_t)object->vo_size, entry,
                (uint64_t)entry->vme_start, (uint64_t)entry->vme_end,
                (uint64_t)VME_OFFSET(entry), (uint64_t)size);
        assertf(os_ref_get_count_raw(&object->ref_count) == 1,
                "object %p ref_count %d\n", object,
                os_ref_get_count_raw(&object->ref_count));
        assertf(!entry->needs_copy, "entry %p\n", entry);
    ...
    if (map_pmap) {
      rc = vm_fault_wire(map, &tmp_entry, caller_prot, tag, map_pmap, pmap_addr,
                         physpage_p);
    } else {
      rc = vm_fault_wire(map, &tmp_entry, caller_prot, tag, map->pmap,
                         tmp_entry.vme_start, physpage_p);
    }
    ...
    if (last_timestamp + 1 != map->timestamp) {
       ...
       * after we unlocked the map.
       */
      if (!vm_map_lookup_entry(map, tmp_entry.vme_start, &first_entry)) {
        panic("vm_map_wire: re-lookup failed");
      }
    ...
    if ((entry !=
         vm_map_to_entry(map)) && /* we still have entries in the map */
        (tmp_entry.vme_end !=
         end) && /* AND, we are not at the end of the requested range */
        (entry->vme_start !=
         tmp_entry.vme_end)) { /* AND, the next entry is not contiguous. */
      /* found a "new" hole */
      s = tmp_entry.vme_end;
      rc = KERN_INVALID_ADDRESS;
      goto done;
  ...
  if (rc != KERN_SUCCESS) {
    ...
    vm_map_unwire_nested(map, start, s, user_wire, map_pmap, pmap_addr);
    if (physpage_p) {
      *physpage_p = 0;
    }
  }
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_wire_sanitize(vm_map_t map, vm_map_offset_ut start_u,
  ...
  kr = vm_sanitize_addr_end(start_u, end_u, vm_sanitize_caller, map, flags,
                            start, end, size);
  if (__improbable(kr != KERN_SUCCESS)) {
    return kr;
  }
 ...
 */
kern_return_t vm_map_wire_impl(vm_map_t map, vm_map_offset_ut start_u,
                               vm_map_offset_ut end_u, vm_prot_ut prot_u,
                               vm_tag_t tag, boolean_t user_wire,
                               ppnum_t *physpage_p,
                               vm_sanitize_caller_t vm_sanitize_caller) {
   ...
   * Sanitize any input parameters that are addr/size/prot/inherit
   */
  kr = vm_map_wire_sanitize(map, start_u, end_u, prot_u, vm_sanitize_caller,
                            &start, &end, &size, &prot);
  if (__improbable(kr != KERN_SUCCESS)) {
    if (physpage_p) {
      *physpage_p = 0;
  ...
  return vm_map_wire_nested(map, start, end, prot, tag, user_wire, PMAP_NULL, 0,
                            physpage_p);
}

kern_return_t vm_map_wire_external(vm_map_t map, vm_map_offset_ut start_u,
                                   vm_map_offset_ut end_u, vm_prot_ut prot_u,
                                   boolean_t user_wire) {
...
kern_return_t vm_map_wire_kernel(vm_map_t map, vm_map_offset_ut start_u,
                                 vm_map_offset_ut end_u, vm_prot_ut prot_u,
                                 vm_tag_t tag, boolean_t user_wire) {
  return vm_map_wire_impl(map, start_u, end_u, prot_u, tag, user_wire, NULL,
                          VM_SANITIZE_CALLER_VM_MAP_WIRE);
}

#if XNU_PLATFORM_MacOSX

kern_return_t vm_map_wire_and_extract(vm_map_t map, vm_map_offset_ut start_u,
                                      vm_prot_ut prot_u, boolean_t user_wire,
                                      ppnum_t *physpage_p) {
  vm_tag_t tag = vm_tag_bt();
  ...
  return vm_map_wire_impl(map, start_u, end_u, prot_u, tag, user_wire,
                          physpage_p, VM_SANITIZE_CALLER_VM_MAP_WIRE);
}
  ...
  while ((entry != vm_map_to_entry(map)) && (entry->vme_start < end)) {
    ...
    if (entry->is_sub_map) {
      ...
      if (map_pmap == NULL) {
        ...
        vm_map_reference(sub_map);
        vm_map_unlock(map);
        vm_map_unwire_nested(sub_map, sub_start, sub_end, user_wire, pmap,
                             pmap_addr);
        vm_map_deallocate(sub_map);
        sub_map = VM_MAP_NULL;
        vm_map_lock(map);
        ...
        vm_map_reference(sub_map);
        vm_map_unlock(map);
        vm_map_unwire_nested(sub_map, sub_start, sub_end, user_wire, map_pmap,
                             pmap_addr);
        vm_map_deallocate(sub_map);
        sub_map = VM_MAP_NULL;
        vm_map_lock(map);
    ...
    vm_map_unlock(map);
    if (map_pmap) {
      vm_fault_unwire(map, &tmp_entry, FALSE, map_pmap, pmap_addr,
                      tmp_entry.vme_end);
    } else {
      vm_fault_unwire(map, &tmp_entry, FALSE, map->pmap, tmp_entry.vme_start,
                      tmp_entry.vme_end);
    }
    vm_map_lock(map);
...
kern_return_t vm_map_unwire(vm_map_t map, vm_map_offset_ut start_u,
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_unwire_sanitize(vm_map_t map, vm_map_offset_ut start_u,
                       vm_map_offset_ut end_u,
                       vm_sanitize_caller_t vm_sanitize_caller,
                       vm_map_offset_t *start, vm_map_offset_t *end,
                       vm_map_size_t *size) {
  ...
  return vm_sanitize_addr_end(start_u, end_u, vm_sanitize_caller, map, flags,
                              start, end, size);
}

kern_return_t vm_map_unwire_impl(vm_map_t map, vm_map_offset_ut start_u,
                                 vm_map_offset_ut end_u, boolean_t user_wire,
                                 vm_sanitize_caller_t vm_sanitize_caller) {
  vm_map_offset_t start, end;
  vm_map_size_t size;
   ...
   * Sanitize any input parameters that are addr/size/prot/inherit
   */
  kr = vm_map_unwire_sanitize(map, start_u, end_u, vm_sanitize_caller, &start,
                              &end, &size);
  if (__improbable(kr != KERN_SUCCESS)) {
    return vm_sanitize_get_kr(kr);
  }

  return vm_map_unwire_nested(map, start, end, user_wire, (pmap_t)NULL, 0);
}
 ...
 */
static void vm_map_entry_zap(vm_map_t map, vm_map_entry_t entry,
                             vm_map_zap_t zap) {
  vm_map_offset_t s, e;
...
static void vm_map_submap_pmap_clean(vm_map_t map, vm_map_offset_t start,
  ...
  if (vm_map_lookup_entry(sub_map, offset, &entry)) {
    ...
    if (entry->is_sub_map) {
      vm_map_submap_pmap_clean(sub_map, start, start + remove_size,
                               VME_SUBMAP(entry), VME_OFFSET(entry));
    } else {
      ...
      if (map->mapped_in_other_pmaps &&
        ...
        vm_object_pmap_protect_options(
            VME_OBJECT(entry), (VME_OFFSET(entry) + offset - entry->vme_start),
            remove_size, PMAP_NULL, PAGE_SIZE, entry->vme_start, VM_PROT_NONE,
            PMAP_OPTIONS_REMOVE);
      } else {
        pmap_remove(map->pmap, (addr64_t)start,
  ...
  while ((entry != vm_map_to_entry(sub_map)) &&
    ...
    if (entry->is_sub_map) {
      vm_map_submap_pmap_clean(sub_map, (start + entry->vme_start) - offset,
      ...
      if (map->mapped_in_other_pmaps &&
          ...
          VME_OBJECT(entry) != NULL) {
        vm_object_pmap_protect_options(
            VME_OBJECT(entry), VME_OFFSET(entry), remove_size, PMAP_NULL,
            PAGE_SIZE, entry->vme_start, VM_PROT_NONE, PMAP_OPTIONS_REMOVE);
      } else {
        pmap_remove(
            map->pmap, (addr64_t)((start + entry->vme_start) - offset),
            (addr64_t)(((start + entry->vme_start) - offset) + remove_size));
      }
    }
    entry = entry->vme_next;
  }
 ...
 */
void virt_memory_guard_ast(thread_t thread, mach_exception_data_type_t code,
  ...
  sync_exception_result =
      task_exception_notify(EXC_GUARD, code, subcode, fatal);
  ...
  if (fatal) {
    ...
    exception_info_t info = {.os_reason = OS_REASON_GUARD,
 ...
 */
static bool
vm_map_guard_exception_internal(vm_map_offset_t address, unsigned reason,
...
void vm_map_guard_exception(vm_map_offset_t address, unsigned reason) {
  mach_exception_code_t code;
  mach_exception_data_type_t subcode;
...
static kern_return_t vm_map_delete_submap_recurse(vm_map_t submap,
                                                  vm_map_offset_t submap_start,
                                                  vm_map_offset_t submap_end) {
  ...
  for (; submap_entry != vm_map_to_entry(submap) &&
...
__abortlike static void __vm_map_delete_misaligned_panic(vm_map_t map,
                                                         vm_map_offset_t start,
                                                         vm_map_offset_t end) {
  panic("vm_map_delete(%p,0x%llx,0x%llx): start is not aligned to 0x%x", map,
        (uint64_t)start, (uint64_t)end, VM_MAP_PAGE_SIZE(map));
}

__abortlike static void __vm_map_delete_failed_panic(vm_map_t map,
                                                     vm_map_offset_t start,
                                                     vm_map_offset_t end,
                                                     kern_return_t kr) {
  panic("vm_map_delete(%p,0x%llx,0x%llx): failed unexpected with %d", map,
        (uint64_t)start, (uint64_t)end, kr);
}

__abortlike static void __vm_map_delete_gap_panic(vm_map_t map,
                                                  vm_map_offset_t where,
                                                  vm_map_offset_t start,
                                                  vm_map_offset_t end) {
  panic("vm_map_delete(%p,0x%llx,0x%llx): no map entry at 0x%llx", map,
        (uint64_t)start, (uint64_t)end, (uint64_t)where);
}

__abortlike static void __vm_map_delete_permanent_panic(vm_map_t map,
                                                        vm_map_offset_t start,
                                                        vm_map_offset_t end,
                                                        vm_map_entry_t entry) {
  panic("vm_map_delete(%p,0x%llx,0x%llx): "
        "Attempting to remove permanent VM map entry %p [0x%llx:0x%llx]",
        map, (uint64_t)start, (uint64_t)end, entry, (uint64_t)entry->vme_start,
...
__options_decl(vm_map_delete_state_t, uint32_t,
               {VMDS_NONE = 0x0000,
                ...
                VMDS_FOUND_GAP = 0x0001, VMDS_GAPS_OK = 0x0002,

                VMDS_KERNEL_PMAP = 0x0004, VMDS_NEEDS_LOOKUP = 0x0008,
                VMDS_NEEDS_WAKEUP = 0x0010, VMDS_KERNEL_KMEMPTR = 0x0020});
 ...
 */
#if MACH_ASSERT
static void vm_map_clamp_to_pmap(vm_map_t map, vm_map_address_t *start,
...
static kmem_return_t vm_map_delete(vm_map_t map, vm_map_offset_t start,
                                   vm_map_offset_t end, vmr_flags_t flags,
                                   kmem_guard_t guard, vm_map_zap_t zap_list) {
  ...
  if (vm_map_pmap(map) == kernel_pmap) {
    ...
    if (kmem_is_ptr_range(range_id)) {
      ...
      slot_idx =
          kmem_addr_get_slot_idx(start, end, range_id, &meta, &size_idx, &slot);
    }
  }
  ...
  if (map->corpse_source && !(flags & VM_MAP_REMOVE_TO_OVERWRITE) &&
      !map->terminated) {
    /*
  ...
  interruptible =
      (flags & VM_MAP_REMOVE_INTERRUPTIBLE) ? THREAD_ABORTSAFE : THREAD_UNINT;
  ...
  if ((state & VMDS_GAPS_OK) == 0) {
    ...
    if (end == 0 || end > vm_map_max(map)) {
      ...
      if (state & VMDS_KERNEL_PMAP) {
        __vm_map_delete_gap_panic(map, gap_start, start, end);
      }
      goto out;
    }
    ...
    if (end < start) {
      if (state & VMDS_KERNEL_PMAP) {
        __vm_map_delete_gap_panic(map, vm_map_max(map), start, end);
      }
  ...
  for (vm_map_offset_t s = start; s < end;) {
    ...
    if (clear_in_transition_end) {
      for (vm_map_entry_t it = entry; it != vm_map_to_entry(map) &&
    ...
    if (entry == vm_map_to_entry(map) || s < entry->vme_start) {
      ...
      if (entry == vm_map_to_entry(map) || end <= entry->vme_start) {
        break;
      }
    ...
    if (entry->vme_permanent && entry->is_sub_map) {
      ...
      if (flags & VM_MAP_REMOVE_IMMUTABLE) {
        ...
        submap_kr = vm_map_delete_submap_recurse(VME_SUBMAP(entry),
                                                 submap_start, submap_end);
        if (submap_kr != KERN_SUCCESS) {
          ...
          DTRACE_VM6(vm_map_delete_permanent_deny_submap, vm_map_entry_t, entry,
                     vm_map_offset_t, entry->vme_start, vm_map_offset_t,
                     entry->vme_end, vm_prot_t, entry->protection, vm_prot_t,
                     entry->max_protection, int, VME_ALIAS(entry));
    ...
    if (entry->vme_permanent && entry->is_sub_map) {
       ...
       * removal to proceed.
       */
      DTRACE_VM6(vm_map_delete_permanent_allow_submap, vm_map_entry_t, entry,
                 vm_map_offset_t, entry->vme_start, vm_map_offset_t,
                 entry->vme_end, vm_prot_t, entry->protection, vm_prot_t,
                 entry->max_protection, int, VME_ALIAS(entry));
    ...
    in_transition:
      ...
      if (interruptible && wait_result == THREAD_INTERRUPTED) {
        /*
         * We do not clear the needs_wakeup flag,
    ...
    if (entry->wired_count) {
      ...
      if (tmp_entry.is_sub_map) {
        ...
        sub_end = sub_start + (tmp_entry.vme_end - tmp_entry.vme_start);
        ...
        (void)vm_map_unwire_nested(sub_map, sub_start, sub_end, user_wire, pmap,
                                   pmap_addr);
      } else {
        ...
        if (tmp_entry.vme_kernel_object) {
          pmap_protect_options(map->pmap, tmp_entry.vme_start, entry_end,
                               VM_PROT_NONE, PMAP_OPTIONS_REMOVE, NULL);
        }
        vm_fault_unwire(map, &tmp_entry, tmp_entry.vme_kernel_object, map->pmap,
    ...
    if (!entry->vme_permanent) {
      ...
      printf("%d[%s] %s(0x%llx,0x%llx): "
             "code signing monitor disabled, allowing for permanent executable "
             "entry [0x%llx:0x%llx] "
             "prot 0x%x/0x%x\n",
             proc_selfpid(),
             (get_bsdtask_info(current_task())
                  ? proc_name_address(get_bsdtask_info(current_task()))
                  : "?"),
             __FUNCTION__, (uint64_t)start, (uint64_t)end,
             (uint64_t)entry->vme_start, (uint64_t)entry->vme_end,
             entry->protection, entry->max_protection);
#endif
    } else {
      DTRACE_VM6(vm_map_delete_permanent, vm_map_entry_t, entry,
                 vm_map_offset_t, entry->vme_start, vm_map_offset_t,
                 entry->vme_end, vm_prot_t, entry->protection, vm_prot_t,
                 entry->max_protection, int, VME_ALIAS(entry));
    }
    ...
    if (entry->is_sub_map) {
      assertf(VM_MAP_PAGE_SHIFT(VME_SUBMAP(entry)) >= VM_MAP_PAGE_SHIFT(map),
              ...
              VM_MAP_PAGE_SHIFT(map), entry, VME_SUBMAP(entry),
      ...
      if (entry->use_pmap) {
        ...
        pmap_unnest_options(map->pmap, (addr64_t)entry->vme_start,
                            entry->vme_end - entry->vme_start, pmap_flags);
#endif /* NO_NESTED_PMAP */
        if (map->mapped_in_other_pmaps &&
            os_ref_get_count_raw(&map->map_refcnt) != 0) {
          /* clean up parent map/maps */
       ...
       */
               ...
               os_ref_get_count_raw(&map->map_refcnt) != 0) {
      vm_object_pmap_protect_options(VME_OBJECT(entry), VME_OFFSET(entry),
                                     entry->vme_end - entry->vme_start,
                                     PMAP_NULL, PAGE_SIZE, entry->vme_start,
     ...
     * cleared by now.
     */
    assert(pmap_is_empty(map->pmap, entry->vme_start, entry->vme_end));
#endif /* DEBUG */

    if (entry->iokit_acct) {
      /* alternate accounting */
      DTRACE_VM4(vm_map_iokit_unmapped_region, vm_map_t, map, vm_map_offset_t,
                 entry->vme_start, vm_map_offset_t, entry->vme_end, int,
                 VME_ALIAS(entry));
      vm_map_iokit_unmapped_region(map, (entry->vme_end - entry->vme_start));
    ...
    if (entry->vme_permanent) {
      ...
      if (__improbable(vm_log_map_delete_permanent_prot_none)) {
        printf("%s:%d %d[%s] map %p entry %p [ 0x%llx - 0x%llx ] submap %d "
               "prot 0x%x/0x%x -> 0/0\n",
               __FUNCTION__, __LINE__, proc_selfpid(),
               (get_bsdtask_info(current_task())
                    ? proc_name_address(get_bsdtask_info(current_task()))
                    : "?"),
               map, entry, (uint64_t)entry->vme_start, (uint64_t)entry->vme_end,
               entry->is_sub_map, entry->protection, entry->max_protection);
      }
      DTRACE_VM6(vm_map_delete_permanent_prot_none, vm_map_entry_t, entry,
                 vm_map_offset_t, entry->vme_start, vm_map_offset_t,
                 entry->vme_end, vm_prot_t, entry->protection, vm_prot_t,
                 entry->max_protection, int, VME_ALIAS(entry));
    ...
    if ((flags & VM_MAP_REMOVE_NO_YIELD) == 0 && s < end) {
      ...
      if (lck_rw_lock_yield_exclusive(&map->lock, LCK_RW_YIELD_ANY_WAITER)) {
...
out:
  ...
  if (state & VMDS_FOUND_GAP) {
    DTRACE_VM3(kern_vm_deallocate_gap, vm_map_offset_t, gap_start,
               vm_map_offset_t, save_start, vm_map_offset_t, save_end);
...
kmem_return_t vm_map_remove_and_unlock(vm_map_t map, vm_map_offset_t start,
                                       vm_map_offset_t end, vmr_flags_t flags,
                                       kmem_guard_t guard) {
  kmem_return_t ret;
  VM_MAP_ZAP_DECLARE(zap);
 ...
 */
kmem_return_t vm_map_remove_guard(vm_map_t map, vm_map_offset_t start,
                                  vm_map_offset_t end, vmr_flags_t flags,
                                  kmem_guard_t guard) {
  kmem_return_t ret;
 ...
 *  vm_map_setup:
 *
 *  Perform any required setup on a new task's map. Must be called before the
 * task is enabled for IPC access, since after this point other threads may be
 * able to look up the task port and make VM API calls.
 */
void vm_map_setup(vm_map_t map, task_t task) {
  /*
 ...
 */
kern_return_t vm_map_terminate(vm_map_t map) {
 ...
 */
static vm_map_copy_t vm_map_copy_allocate(uint16_t type) {
...
void vm_map_copy_discard(vm_map_copy_t copy) {
  ...
  case VM_MAP_COPY_KERNEL_BUFFER:
    ...
    if (copy->size > msg_ool_size_small || copy->offset) {
      panic("Invalid vm_map_copy_t sz:%lld, ofst:%lld", (long long)copy->size,
            (long long)copy->offset);
    }
...
__exported extern vm_map_copy_t vm_map_copy_copy(vm_map_copy_t copy);
 ...
 */
vm_map_copy_t vm_map_copy_copy(vm_map_copy_t copy) {
  vm_map_copy_t new_copy;
  ...
  if (copy->type == VM_MAP_COPY_ENTRY_LIST) {
    ...
    vm_map_copy_last_entry(copy)->vme_next = vm_map_copy_to_entry(new_copy);
  }
...
static boolean_t vm_map_entry_is_overwritable(vm_map_t dst_map __unused,
                                              vm_map_entry_t entry) {
  if (!(entry->protection & VM_PROT_WRITE)) {
...
#if !__x86_64__
  if (entry->used_for_jit && vm_map_cs_enforcement(dst_map) &&
      !dst_map->cs_debugged) {
    /*
  ...
  if (entry->vme_permanent) {
    if (entry->is_sub_map) {
       ...
       * entry.
       */
      DTRACE_VM6(vm_map_delete_permanent_deny_overwrite, vm_map_entry_t, entry,
                 vm_map_offset_t, entry->vme_start, vm_map_offset_t,
                 entry->vme_end, vm_prot_t, entry->protection, vm_prot_t,
                 entry->max_protection, int, VME_ALIAS(entry));
      return FALSE;
...
static kern_return_t vm_map_overwrite_submap_recurse(vm_map_t dst_map,
  ...
  dst_end = vm_map_round_page(dst_addr + dst_size, VM_MAP_PAGE_MASK(dst_map));
  vm_map_lock(dst_map);
...
start_pass_1:
  ...
  vm_map_clip_start(dst_map, tmp_entry,
                    vm_map_trunc_page(dst_addr, VM_MAP_PAGE_MASK(dst_map)));
  if (tmp_entry->is_sub_map) {
    /* clipping did unnest if needed */
  ...
  for (entry = tmp_entry;;) {
    ...
    while (entry->is_sub_map) {
      ...
      result = vm_map_overwrite_submap_recurse(sub_map, sub_start,
                                               sub_end - sub_start);
      ...
      vm_map_lock(dst_map);
      if (!vm_map_lookup_entry(dst_map, local_end, &tmp_entry)) {
        vm_map_unlock(dst_map);
        vmlp_api_end(VM_MAP_OVERWRITE_SUBMAP_RECURSE, KERN_INVALID_ADDRESS);
        return KERN_INVALID_ADDRESS;
      }
    ...
    if ((VME_OBJECT(entry) != VM_OBJECT_NULL) &&
        ((!VME_OBJECT(entry)->internal) || (VME_OBJECT(entry)->true_share))) {
 ...
 */
static kern_return_t
vm_map_copy_overwrite_nested(vm_map_t dst_map, vm_map_address_t dst_addr,
                             vm_map_copy_t copy, boolean_t interruptible,
                             pmap_t pmap, boolean_t discard_on_success) {
  vm_map_offset_t dst_end;
  vm_map_entry_t tmp_entry;
  ...
  if (copy->type == VM_MAP_COPY_KERNEL_BUFFER) {
    kr = vm_map_copyout_kernel_buffer(dst_map, &dst_addr, copy, copy->size,
                                      TRUE, discard_on_success);
  ...
  if (!VM_MAP_PAGE_ALIGNED(copy->size, VM_MAP_PAGE_MASK(dst_map)) ||
    ...
    aligned = FALSE;
    dst_end =
        vm_map_round_page(dst_addr + copy->size, VM_MAP_PAGE_MASK(dst_map));
  } else {
  ...
  for (entry = tmp_entry;;) {
    ...
    while (entry->is_sub_map) {
      ...
      if (!(entry->needs_copy)) {
        ...
        kr = vm_map_overwrite_submap_recurse(sub_map, sub_start,
                                             sub_end - sub_start);
      ...
      if (!vm_map_lookup_entry(dst_map, local_end, &entry)) {
        vm_map_unlock(dst_map);
        vmlp_api_end(VM_MAP_COPY_OVERWRITE_NESTED, KERN_INVALID_ADDRESS);
        return KERN_INVALID_ADDRESS;
      }
  ...
  while (TRUE) {
    ...
    for (entry = tmp_entry; copy_size == 0;) {
      ...
      if (entry->in_transition) {
        ...
        if (!vm_map_lookup_entry(dst_map, base_addr, &tmp_entry)) {
          vm_map_unlock(dst_map);
          vmlp_api_end(VM_MAP_COPY_OVERWRITE_NESTED, KERN_INVALID_ADDRESS);
          return KERN_INVALID_ADDRESS;
        }
      ...
      if (entry->is_sub_map) {
        ...
        if (entry->needs_copy) {
          ...
          vm_map_clip_end(dst_map, entry, sub_end);
          vm_map_clip_start(dst_map, entry, sub_start);
          assert(!entry->use_pmap);
        ...
        if (base_addr < entry->vme_start) {
          /* stuff to send */
          copy_size = entry->vme_start - base_addr;
          break;
        }
        sub_start = VME_OFFSET(entry);
        ...
        if (total_size > copy_size) {
          ...
          while (copy_entry != vm_map_copy_to_entry(copy)) {
            entry_size = copy_entry->vme_end - copy_entry->vme_start;
            if ((local_size < copy_size) &&
                ((local_size + entry_size) >= copy_size)) {
            ...
            if (local_size >= copy_size) {
              ...
              copy_entry->vme_next = vm_map_copy_to_entry(copy);
              previous_prev = VMH_PREV(&copy->cpy_hdr);
              VMH_PREV_SET(&copy->cpy_hdr, copy_entry);
              copy->size = copy_size;
              remaining_entries = copy->cpy_hdr.nentries;
        ...
        if ((use_pmap) && (pmap == NULL)) {
          kr = vm_map_copy_overwrite_nested(sub_map, sub_start, copy,
                                            interruptible, sub_map->pmap, TRUE);
        } else if (pmap != NULL) {
          kr = vm_map_copy_overwrite_nested(sub_map, sub_start, copy,
                                            interruptible, pmap, TRUE);
        } else {
          kr = vm_map_copy_overwrite_nested(sub_map, sub_start, copy,
                                            interruptible, dst_map->pmap, TRUE);
        }
        ...
        if (kr != KERN_SUCCESS) {
          if (next_copy != NULL) {
            ...
            VMH_PREV(&copy->cpy_hdr)->vme_next = next_copy;
    ...
    if (aligned) {
      ...
      if ((kr = vm_map_copy_overwrite_aligned(dst_map, tmp_entry, copy,
                                              base_addr, local_pmap)) !=
          KERN_SUCCESS) {
        if (next_copy != NULL) {
          copy->cpy_hdr.nentries += remaining_entries;
          VMH_PREV(&copy->cpy_hdr)->vme_next = next_copy;
      ...
      kr = vm_map_copy_overwrite_unaligned(dst_map, tmp_entry, copy, base_addr,
                                           discard_on_success);
      if (kr != KERN_SUCCESS) {
        if (next_copy != NULL) {
          copy->cpy_hdr.nentries += remaining_entries;
    ...
    vm_map_clip_start(dst_map, tmp_entry,
                      vm_map_trunc_page(base_addr, VM_MAP_PAGE_MASK(dst_map)));
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_copy_addr_size_sanitize(vm_map_t map, vm_map_offset_ut addr_u,
...
kern_return_t vm_map_copy_overwrite(vm_map_t dst_map,
   ...
   * Sanitize any input parameters that are addr/size/prot/inherit
   */
  kr = vm_map_copy_addr_size_sanitize(dst_map, dst_addr_u, copy_size_u,
                                      VM_SANITIZE_CALLER_VM_MAP_COPY_OVERWRITE,
                                      &dst_addr, &dst_end, &copy_size);
  if (__improbable(kr != KERN_SUCCESS)) {
  ...
  if (interruptible || copy->type != VM_MAP_COPY_ENTRY_LIST) {
    /*
     * We can't split the "copy" map if we're interruptible
     * or if we don't have a "copy" map...
     */
  blunt_copy:
    kr = vm_map_copy_overwrite_nested(dst_map, dst_addr, copy, interruptible,
                                      (pmap_t)NULL, TRUE);
    if (kr) {
      ktriage_record(
  ...
  if (copy_page_shift < PAGE_SHIFT || VM_MAP_PAGE_SHIFT(dst_map) < PAGE_SHIFT) {
    goto blunt_copy;
  }
  ...
  if (VM_MAP_PAGE_SHIFT(dst_map) < PAGE_SHIFT) {
    ...
    effective_page_mask = MAX(VM_MAP_COPY_PAGE_MASK(copy), effective_page_mask);
  }
  effective_page_size = effective_page_mask + 1;

  if (copy_size < VM_MAP_COPY_OVERWRITE_OPTIMIZATION_THRESHOLD_PAGES *
                      effective_page_size) {
    /*
  ...
  if (!vm_map_page_aligned(dst_addr, effective_page_mask)) {
    head_addr = dst_addr;
    head_size = (effective_page_size - (copy->offset & effective_page_mask));
    head_size = MIN(head_size, copy_size);
  }
  if (!vm_map_page_aligned(copy->offset + copy_size, effective_page_mask)) {
    /*
     * Mis-alignment at the end.
     * Do an aligned copy up to the last page and
     * then an unaligned copy for the remaining bytes.
     */
    tail_size = ((copy->offset + copy_size) & effective_page_mask);
  ...
  for (; (entry != vm_map_to_entry(dst_map) &&
  ...
  if (head_size) {
    ...
    head_copy->cpy_hdr.entries_pageable = copy->cpy_hdr.entries_pageable;
    ...
    vm_map_copy_entry_unlink(copy, entry);
    vm_map_copy_entry_link(head_copy, vm_map_copy_to_entry(head_copy), entry);

    /*
     * Do the unaligned copy.
     */
    kr = vm_map_copy_overwrite_nested(dst_map, head_addr, head_copy,
                                      interruptible, (pmap_t)NULL, FALSE);
    if (kr != KERN_SUCCESS) {
      ktriage_record(
          thread_tid(current_thread()),
          KDBG_TRIAGE_EVENTID(
  ...
  if (tail_size) {
    ...
    tail_copy->cpy_hdr.entries_pageable = copy->cpy_hdr.entries_pageable;
    ...
    entry = vm_map_copy_last_entry(copy);
    vm_map_copy_entry_unlink(copy, entry);
    vm_map_copy_entry_link(tail_copy, vm_map_copy_last_entry(tail_copy), entry);
  }
   ...
   */
  kr = vm_map_copy_overwrite_nested(dst_map, dst_addr + head_size, copy,
                                    interruptible, (pmap_t)NULL, FALSE);
  if (kr != KERN_SUCCESS) {
    ktriage_record(
        thread_tid(current_thread()),
  ...
  if (tail_size) {
    kr = vm_map_copy_overwrite_nested(dst_map, tail_addr, tail_copy,
                                      interruptible, (pmap_t)NULL, FALSE);
    if (kr) {
      ktriage_record(
          thread_tid(current_thread()),
  ...
  if (kr == KERN_SUCCESS) {
    ...
    if (head_copy) {
      ...
      vm_map_copy_entry_link(copy, vm_map_copy_to_entry(copy), entry);
      copy->offset -= head_size;
      copy->size += head_size;
      vm_map_copy_discard(head_copy);
      head_copy = NULL;
    ...
    if (tail_copy) {
      ...
      vm_map_copy_entry_link(copy, vm_map_copy_last_entry(copy), entry);
      copy->size += tail_size;
      vm_map_copy_discard(tail_copy);
      tail_copy = NULL;
    }
...
static kern_return_t
vm_map_copy_overwrite_unaligned(vm_map_t dst_map, vm_map_entry_t entry,
                                vm_map_copy_t copy, vm_map_offset_t start,
                                boolean_t discard_on_success) {
  vm_map_entry_t copy_entry;
  ...
  vm_map_size_t src_size, dst_size, copy_size, amount_left;
  kern_return_t kr = KERN_SUCCESS;
  ...
  src_offset = copy->offset -
               trunc_page_mask_64(copy->offset, VM_MAP_COPY_PAGE_MASK(copy));
  ...
  while (amount_left > 0) {
    ...
    src_size = copy_entry->vme_end - (copy_entry->vme_start + src_offset);
    ...
    if (dst_object == VM_OBJECT_NULL) {
      ...
      dst_object = vm_object_allocate(
          (vm_map_size_t)entry->vme_end - entry->vme_start, dst_map->serial_id);
     ...
     */
    kr = vm_fault_copy(
        VME_OBJECT(copy_entry), VME_OFFSET(copy_entry) + src_offset, &copy_size,
        dst_object, entry_offset + dst_offset, dst_map, &version, THREAD_UNINT);
    ...
    if ((copy_entry->vme_start + src_offset) == copy_entry->vme_end ||
      ...
      if (copy_entry_next == vm_map_copy_to_entry(copy) && amount_left) {
        /*
         *				not finished copying but run out of
         * source
         */
        vmlp_api_end(VM_MAP_COPY_OVERWRITE_UNALIGNED, KERN_INVALID_ADDRESS);
        return KERN_INVALID_ADDRESS;
    ...
    if (version.main_timestamp == dst_map->timestamp) {
      if (start == entry_end) {
        /*
         *				destination region is split.  Use the
         * version information to avoid a lookup in the normal case.
         */
        entry = entry->vme_next;
        /*
         *				should be contiguous. Fail if we
         * encounter a hole in the destination.
         */
...
static kern_return_t vm_map_copy_overwrite_aligned(vm_map_t dst_map,
  ...
  while ((copy_entry = vm_map_copy_first_entry(copy)) !=
    ...
    if ((entry->vme_start != start) ||
        ((entry->is_sub_map) && !entry->needs_copy)) {
      vm_map_unlock(dst_map);
      vmlp_api_end(VM_MAP_COPY_OVERWRITE_ALIGNED, KERN_INVALID_ADDRESS);
      return KERN_INVALID_ADDRESS;
    }
    ...
    object = VME_OBJECT(entry);
    if ((!entry->is_shared && !entry->vme_permanent && !entry->used_for_jit &&
#if __arm64e__
         !entry->used_for_tpro &&
#endif /* __arm64e__ */
         !(entry->protection & VM_PROT_EXECUTE) &&
         !pmap_has_prot_policy(dst_map->pmap, entry->translated_allow_execute,
                               entry->protection) &&
         ((object == VM_OBJECT_NULL) ||
...
#define __TRADEOFF1_COPY_SIZE (128 * 1024)      /* 128 KB */
      ...
      if (old_object != VM_OBJECT_NULL) {
        ...
        if (entry->is_sub_map) {
          if (entry->use_pmap) {
#ifndef NO_NESTED_PMAP
            pmap_unnest(dst_map->pmap, (addr64_t)entry->vme_start,
                        entry->vme_end - entry->vme_start);
#endif /* NO_NESTED_PMAP */
            if (dst_map->mapped_in_other_pmaps) {
              /* clean up parent */
              /* map/maps */
              vm_map_submap_pmap_clean(dst_map, entry->vme_start,
                                       entry->vme_end, VME_SUBMAP(entry),
                                       VME_OFFSET(entry));
            }
          } else {
            vm_map_submap_pmap_clean(dst_map, entry->vme_start, entry->vme_end,
                                     VME_SUBMAP(entry), VME_OFFSET(entry));
          }
          vm_map_deallocate(VME_SUBMAP(entry));
        } else {
          if (dst_map->mapped_in_other_pmaps) {
            vm_object_pmap_protect_options(
                ...
                entry->vme_end - entry->vme_start, PMAP_NULL, PAGE_SIZE,
                entry->vme_start, VM_PROT_NONE, PMAP_OPTIONS_REMOVE);
          } else {
            pmap_remove_options(dst_map->pmap, (addr64_t)(entry->vme_start),
                                (addr64_t)(entry->vme_end),
                                PMAP_OPTIONS_REMOVE);
          }
          vm_object_deallocate(old_object);
       ...
       * than 2/3 of the pages touched, this optimization actually cost more
       * cycles it also puts a lot of pressure on the pmap layer w/r to mapping
       * structures
       */
    ...
    slow_copy:
      if (entry->needs_copy) {
        VME_OBJECT_SHADOW(entry, (entry->vme_end - entry->vme_start),
                          vm_map_always_shadow(dst_map));
        entry->needs_copy = FALSE;
      }
      ...
      if (dst_object == VM_OBJECT_NULL) {
         ...
         */
        dst_object = vm_object_allocate(entry->vme_end - entry->vme_start,
                                        dst_map->serial_id);
        dst_offset = 0;
        VME_OBJECT_SET(entry, dst_object, false, 0);
      ...
      copy_size = size;
      r = vm_fault_copy(VME_OBJECT(copy_entry), VME_OFFSET(copy_entry),
                        &copy_size, dst_object, dst_offset, dst_map, &version,
                        THREAD_UNINT);
      ...
      start += copy_size;
      vm_map_lock(dst_map);
      if (version.main_timestamp == dst_map->timestamp && copy_size != 0) {
 ...
 */
static kern_return_t
vm_map_copyin_kernel_buffer(vm_map_t src_map, vm_map_offset_t src_addr,
                            vm_map_size_t len, vm_map_copyin_strategy strategy,
                            boolean_t src_destroy, vm_map_copy_t *copy_result) {
  kern_return_t kr;
  vm_map_copy_t copy;
  void *kdata;
...
#pragma unused(strategy)
  ...
  if (src_destroy) {
    ...
    (void)vm_map_remove_guard(
        src_map, vm_map_trunc_page(src_addr, VM_MAP_PAGE_MASK(src_map)),
        vm_map_round_page(src_addr + len, VM_MAP_PAGE_MASK(src_map)), flags,
        KMEM_GUARD_NONE);
  }
...
vm_map_copyout_kernel_buffer(vm_map_t map, vm_map_address_t *addr, /* IN/OUT */
  ...
  if (!overwrite) {
    ...
    *addr = 0;
    kr = vm_map_enter(
        map, addr, vm_map_round_page(copy_size, VM_MAP_PAGE_MASK(map)),
        (vm_map_offset_t)0, vmk_flags, VM_OBJECT_NULL, (vm_object_offset_t)0,
  ...
  if (kr != KERN_SUCCESS) {
    ...
    if (!overwrite) {
       ...
       */
      (void)vm_map_remove(
          map, vm_map_trunc_page(*addr, VM_MAP_PAGE_MASK(map)),
          vm_map_round_page(
              (*addr + vm_map_round_page(copy_size, VM_MAP_PAGE_MASK(map))),
              VM_MAP_PAGE_MASK(map)));
      *addr = 0;
    }
  } else {
...
static void vm_map_copy_insert(vm_map_t map, vm_map_entry_t after_where,
  ...
  while (vm_map_copy_first_entry(copy) != vm_map_copy_to_entry(copy)) {
    entry = vm_map_copy_first_entry(copy);
    vm_map_copy_entry_unlink(copy, entry);
    vm_map_store_entry_link(map, after_where, entry, VM_MAP_KERNEL_FLAGS_NONE);
 ...
 * one to ensure that it hasn't been forged.
 */
void vm_map_copy_remap(vm_map_t map, vm_map_entry_t where, vm_map_copy_t copy,
                       vm_map_offset_t adjustment, vm_prot_t cur_prot,
                       vm_prot_t max_prot, vm_inherit_t inheritance) {
  ...
  for (copy_entry = vm_map_copy_first_entry(copy);
    ...
    vm_map_store_entry_link(map, where, new_entry, VM_MAP_KERNEL_FLAGS_NONE);
    vmlp_range_event_entry(map, new_entry);
 ...
 */
boolean_t vm_map_copy_validate_size(vm_map_t dst_map, vm_map_copy_t copy,
                                    vm_map_size_t *size) {
  if (copy == VM_MAP_COPY_NULL) {
    return FALSE;
  }
...
static kern_return_t
vm_map_copyout_internal(vm_map_t dst_map, vm_map_address_t *dst_addr, /* OUT */
                        vm_map_copy_t copy, vm_map_size_ut copy_size_u,
                        boolean_t consume_on_success, vm_prot_t cur_protection,
                        vm_prot_t max_protection, vm_inherit_t inheritance) {
  ...
  if (copy->type == VM_MAP_COPY_KERNEL_BUFFER) {
    kr = vm_map_copyout_kernel_buffer(dst_map, dst_addr, copy, copy_size, FALSE,
                                      consume_on_success);
    if (kr) {
      ktriage_record(
          thread_tid(current_thread()),
  ...
  if (copy->cpy_hdr.page_shift != VM_MAP_PAGE_SHIFT(dst_map)) {
    ...
    DEBUG4K_ADJUST("adjusting...\n");
    kr = vm_map_copy_adjust_to_target(copy, 0,       /* offset */
                                      copy->size,    /* size */
                                      dst_map, TRUE, /* copy */
                                      &target_copy, &overmap_start,
                                      &overmap_end, &trimmed_start);
    if (kr != KERN_SUCCESS) {
    ...
    DEBUG4K_COPY("copy %p (%d 0x%llx 0x%llx) dst_map %p (%d) target_copy %p "
                 "(%d 0x%llx 0x%llx) overmap_start 0x%llx overmap_end 0x%llx "
                 "trimmed_start 0x%llx\n",
                 copy, copy->cpy_hdr.page_shift, copy->offset,
                 (uint64_t)copy->size, dst_map, VM_MAP_PAGE_SHIFT(dst_map),
                 target_copy, target_copy->cpy_hdr.page_shift,
                 target_copy->offset, (uint64_t)target_copy->size,
                 (uint64_t)overmap_start, (uint64_t)overmap_end,
  ...
  vm_map_lock(dst_map);
  kr = vm_map_locate_space_anywhere(dst_map, size, 0, vmk_flags, &start, &last);
  if (kr != KERN_SUCCESS) {
    vm_map_unlock(dst_map);
    ktriage_record(
        thread_tid(current_thread()),
  ...
  if (copy->cpy_hdr.entries_pageable != dst_map->hdr.entries_pageable) {
    ...
    while (entry != vm_map_copy_to_entry(copy)) {
      ...
      vm_map_copy_entry_link(copy, vm_map_copy_last_entry(copy), new);
      next = entry->vme_next;
      vm_map_entry_dispose(entry);
      entry = next;
    }
  ...
  for (entry = vm_map_copy_first_entry(copy);
       entry != vm_map_copy_to_entry(copy); entry = entry->vme_next) {
    entry->vme_start += adjustment;
    entry->vme_end += adjustment;

    assert(VM_MAP_PAGE_ALIGNED(entry->vme_start, VM_MAP_PAGE_MASK(dst_map)));
    assert(VM_MAP_PAGE_ALIGNED(entry->vme_end, VM_MAP_PAGE_MASK(dst_map)));
    ...
    if (entry->wired_count != 0) {
      ...
      pmap_pageable(dst_map->pmap, entry->vme_start, entry->vme_end, TRUE);
      ...
      while (va < entry->vme_end) {
        ...
        m = vm_page_lookup(object, offset);
        if (m == VM_PAGE_NULL || !VM_PAGE_WIRED(m) || m->vmp_absent) {
          panic("vm_map_copyout: wiring %p", m);
        }

        prot = entry->protection;

        if (override_nx(dst_map, VME_ALIAS(entry)) && prot) {
          prot |= VM_PROT_EXECUTE;
        }
        ...
        if (entry->iokit_acct || (!entry->is_sub_map && !entry->use_pmap)) {
          fault_info.pmap_options |= PMAP_OPTIONS_ALT_ACCT;
        }
        if (entry->vme_xnu_user_debug && !VM_PAGE_OBJECT(m)->code_signed) {
          /*
        ...
        bool page_sleep_needed = false;
        vm_fault_enter(
            m, dst_map->pmap, va, PAGE_SIZE, 0, prot, prot, VM_PAGE_WIRED(m),
...
after_adjustments:
  ...
  *dst_addr = start + (copy->offset - vm_copy_start);
  ...
  if (consume_on_success) {
    ...
    vm_map_copy_remap(dst_map, last, copy, adjustment, cur_protection,
                      max_protection, inheritance);
 ...
 *		Otherwise, the caller is responsible for it.
 */
kern_return_t vm_map_copyout_size(vm_map_t dst_map,
                                  vm_map_address_t *dst_addr, /* OUT */
                                  vm_map_copy_t copy,
                                  vm_map_size_ut copy_size) {
  return vm_map_copyout_internal(
      dst_map, dst_addr, copy, copy_size, TRUE, /* consume_on_success */
 ...
 *		Otherwise, the caller is responsible for it.
 */
kern_return_t vm_map_copyout(vm_map_t dst_map,
                             vm_map_address_t *dst_addr, /* OUT */
                             vm_map_copy_t copy) {
 ...
 *
 */
kern_return_t vm_map_copyin(vm_map_t src_map, vm_map_address_ut src_addr,
                            vm_map_size_ut len, boolean_t src_destroy,
                            vm_map_copy_t *copy_result) /* OUT */
{
  return vm_map_copyin_common(src_map, src_addr, len, src_destroy, FALSE,
                              copy_result, FALSE);
}
...
kern_return_t vm_map_copyin_common(vm_map_t src_map, vm_map_address_ut src_addr,
                                   vm_map_size_ut len, boolean_t src_destroy,
  ...
  return vm_map_copyin_internal(src_map, src_addr, len, flags, copy_result);
}

static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_copyin_sanitize(vm_map_t src_map, vm_map_address_ut src_addr_u,
                       vm_map_size_ut len_u, vm_map_offset_t *src_start,
                       vm_map_offset_t *src_end, vm_map_size_t *len,
  ...
  kr =
      vm_sanitize_addr_size(src_addr_u, len_u, VM_SANITIZE_CALLER_VM_MAP_COPYIN,
                            src_map, flags, src_start, src_end, len);
  if (__improbable(kr != KERN_SUCCESS)) {
    return kr;
  }
...
_vm_map_copyin_select_strategy(vm_map_t src_map, vm_map_size_t len,
                               vm_map_offset_t src_start,
                               vm_map_offset_t src_end, boolean_t use_maxprot,
                               boolean_t preserve_purgeable, int flags) {
  /*
   ...
   * for small regions.
   */
  if ((len <= msg_ool_size_small) && !use_maxprot && !preserve_purgeable &&
       ...
       */
      (src_start >= vm_map_min(src_map) && src_start < vm_map_max(src_map) &&
       src_end >= vm_map_min(src_map) && src_end < vm_map_max(src_map))) {
...
kern_return_t vm_map_copyin_internal(vm_map_t src_map,
                                     vm_map_address_ut src_addr_u,
                                     vm_map_size_ut len_u, int flags,
                                     vm_map_copy_t *copy_result) /* OUT */
{
   ...
   * Sanitize any input parameters that are addr/size/prot/inherit
   */
  kr = vm_map_copyin_sanitize(src_map, src_addr_u, len_u, &src_start, &src_end,
                              &len, &src_addr_unaligned);
  if (__improbable(kr != KERN_SUCCESS)) {
    kr = vm_sanitize_get_kr(kr);
  ...
  vm_map_copyin_strategy strategy = _vm_map_copyin_select_strategy(
      src_map, len, src_start, src_end, use_maxprot, preserve_purgeable, flags);
...
#define RETURN(x)                                                              \
  ...
  RestartCopy:
    if ((src_object == VM_OBJECT_NULL ||
        ...
        vm_object_copy_quickly(VME_OBJECT(new_entry), src_offset, src_size,
                               &src_needs_copy, &new_entry_needs_copy)) {
      ...
      if (src_needs_copy && !tmp_entry->needs_copy) {
        ...
        if (override_nx(src_map, VME_ALIAS(src_entry)) && prot) {
          prot |= VM_PROT_EXECUTE;
        }

        vm_object_pmap_protect(
            src_object, src_offset, src_size,
            (src_entry->is_shared ? PMAP_NULL : src_map->pmap),
            VM_MAP_PAGE_SIZE(src_map), src_entry->vme_start, prot);
    ...
    CopySlowly:
      ...
      result = vm_object_copy_slowly(src_object, src_offset, src_size,
                                     THREAD_UNINT, &new_copy_object);
      ...
      new_object =
          vm_object_copy_delayed(src_object, src_offset, src_size, TRUE);
      if (new_object == VM_OBJECT_NULL) {
        goto CopySlowly;
      }
      ...
      result = vm_object_copy_strategically(
          src_object, src_offset, src_size, (flags & VM_MAP_COPYIN_FORK),
          &new_copy_object, &new_offset, &new_entry_needs_copy);
    ...
    if (result == KERN_SUCCESS &&
      ...
      if (preserve_purgeable && src_object->purgable != VM_PURGABLE_DENY) {
        ...
        if (src_object->purgable != VM_PURGABLE_NONVOLATILE) {
          ...
          vm_object_purgable_control(new_object,
                                     VM_PURGABLE_SET_STATE_FROM_KERNEL, &state);
        }
    ...
    if (result != KERN_SUCCESS && result != KERN_MEMORY_RESTART_COPY) {
      if (vm_map_is_sealed(src_map)) {
        vm_map_lock_read(src_map);
      } else {
        vm_map_lock(src_map);
    ...
    if (src_entry->vme_end < new_entry->vme_end) {
       ...
       */
      assert(
          VM_MAP_PAGE_ALIGNED(src_entry->vme_end, VM_MAP_COPY_PAGE_MASK(copy)));
  ...
  CopySuccessful:;
    ...
    while ((src_start >= src_end) && (src_end != 0)) {
      ...
      if (vm_map_is_sealed(src_map)) {
        ...
        vm_map_simplify_range(src_map, src_base, src_end);
        vm_map_unlock(src_map);
      }
  ...
  if (src_destroy) {
    ...
    (void)vm_map_remove_and_unlock(
        src_map,
        vm_map_trunc_page(src_addr_unaligned, VM_MAP_PAGE_MASK(src_map)),
        src_end, remove_flags, KMEM_GUARD_NONE);
    ...
    vm_map_simplify_range(
        src_map,
        vm_map_trunc_page(src_addr_unaligned, VM_MAP_PAGE_MASK(src_map)),
        vm_map_round_page(src_end, VM_MAP_PAGE_MASK(src_map)));
    vm_map_unlock(src_map);
  }
  ...
  if (VM_MAP_PAGE_SHIFT(src_map) > PAGE_SHIFT &&
    ...
    if (tmp_entry != vm_map_copy_to_entry(copy)) {
      ...
      adjustment =
          (tmp_entry->vme_start -
           vm_map_trunc_page(tmp_entry->vme_start, VM_MAP_PAGE_MASK(src_map)));
      tmp_entry->vme_start -= adjustment;
      VME_OFFSET_SET(tmp_entry, VME_OFFSET(tmp_entry) - adjustment);
      copy_addr -= adjustment;
      assert(tmp_entry->vme_start < tmp_entry->vme_end);
      /* ... adjust for mis-aligned start of copy range */
      adjustment = (vm_map_trunc_page(copy->offset, PAGE_MASK) -
                    vm_map_trunc_page(copy->offset, VM_MAP_PAGE_MASK(src_map)));
      if (adjustment) {
      ...
      assert(
          vm_map_trunc_page(tmp_entry->vme_start, VM_MAP_PAGE_MASK(src_map)) ==
          vm_map_trunc_page(original_start, VM_MAP_PAGE_MASK(src_map)));
    }
    ...
    if (tmp_entry != vm_map_copy_to_entry(copy)) {
      ...
      tmp_entry->vme_end =
          vm_map_round_page(tmp_entry->vme_end, VM_MAP_PAGE_MASK(src_map));
      /* ... adjust for mis-aligned end of copy range */
      adjustment = (vm_map_round_page((copy->offset + copy->size),
                                      VM_MAP_PAGE_MASK(src_map)) -
                    vm_map_round_page((copy->offset + copy->size), PAGE_MASK));
      if (adjustment) {
        assert(page_aligned(adjustment));
        assert(adjustment < VM_MAP_PAGE_SIZE(src_map));
       ...
       * a single 16K page.
       */
      assert(vm_map_round_page(tmp_entry->vme_end, VM_MAP_PAGE_MASK(src_map)) ==
             vm_map_round_page(original_end, VM_MAP_PAGE_MASK(src_map)));
    }
  }
  ...
  while (tmp_entry != vm_map_copy_to_entry(copy)) {
    assert(VM_MAP_PAGE_ALIGNED(copy_addr +
...
kern_return_t vm_map_copy_extract(vm_map_t src_map, vm_map_address_t src_addr,
                                  vm_map_size_t len, boolean_t do_copy,
                                  vm_map_copy_t *copy_result, /* OUT */
  ...
  if (VM_MAP_PAGE_SIZE(src_map) < PAGE_SIZE) {
    DEBUG4K_SHARE("src_map %p src_addr 0x%llx src_end 0x%llx\n", src_map,
                  (uint64_t)src_addr, (uint64_t)(src_addr + len));
  }
  ...
  kr = vm_map_remap_extract(src_map, src_addr, len, do_copy, /* copy */
                            copy, cur_prot,                  /* IN/OUT */
                            max_prot,                        /* IN/OUT */
                            inheritance, vmk_flags);
  if (kr != KERN_SUCCESS) {
    vm_map_copy_discard(copy);
    if ((kr == KERN_INVALID_ADDRESS || kr == KERN_INVALID_ARGUMENT) &&
        src_map->terminated) {
...
static void vm_map_fork_share(vm_map_t old_map, vm_map_entry_t old_entry,
  ...
  if (old_entry->is_sub_map) {
    ...
    if (old_entry->use_pmap) {
      ...
      result = pmap_nest(new_map->pmap, (VME_SUBMAP(old_entry))->pmap,
    ...
    object = vm_object_allocate(
        (vm_map_size_t)(old_entry->vme_end - old_entry->vme_start),
        old_map->serial_id);
     ...
     */
    VME_OBJECT_SHADOW(
        old_entry, (vm_map_size_t)(old_entry->vme_end - old_entry->vme_start),
    ...
    if (!old_entry->needs_copy && is_writable) {
      ...
      if (pmap_has_prot_policy(old_map->pmap,
                               old_entry->translated_allow_execute,
                               old_entry->protection)) {
        panic("%s: map %p pmap %p entry %p 0x%llx:0x%llx prot 0x%x",
              __FUNCTION__, old_map, old_map->pmap, old_entry,
              (uint64_t)old_entry->vme_start, (uint64_t)old_entry->vme_end,
              old_entry->protection);
      }
      ...
      if (pmap_has_prot_policy(old_map->pmap,
                               old_entry->translated_allow_execute, prot)) {
        panic("%s: map %p pmap %p entry %p 0x%llx:0x%llx prot 0x%x",
              __FUNCTION__, old_map, old_map->pmap, old_entry,
              (uint64_t)old_entry->vme_start, (uint64_t)old_entry->vme_end,
              prot);
      ...
      if (old_map->mapped_in_other_pmaps) {
        vm_object_pmap_protect(VME_OBJECT(old_entry), VME_OFFSET(old_entry),
                               (old_entry->vme_end - old_entry->vme_start),
                               PMAP_NULL, PAGE_SIZE, old_entry->vme_start,
                               prot);
      } else {
        pmap_protect(old_map->pmap, old_entry->vme_start, old_entry->vme_end,
                     prot);
      }
    }
  ...
  new_entry =
      vm_map_entry_create(new_map); /* Never the kernel map or descendants */
  ...
  if (old_entry->is_sub_map) {
    ...
    pmap_copy(new_map->pmap, old_map->pmap, new_entry->vme_start,
              old_entry->vme_end - old_entry->vme_start, old_entry->vme_start);
  }
}
...
static boolean_t vm_map_fork_copy(vm_map_t old_map, vm_map_entry_t *old_entry_p,
  ...
  vmlp_range_event_entry(
      old_map,
      old_entry); /* new_map is covered by call to vm_map_copy_insert */
  ...
  if (vm_map_copyin_internal(old_map, start, entry_size, vm_map_copyin_flags,
                             &copy) != KERN_SUCCESS) {
    /*
...
void vm_map_inherit_limits(vm_map_t new_map, const struct _vm_map *old_map) {
 ...
 */
vm_map_t vm_map_fork(ledger_t ledger, vm_map_t old_map, int options) {
  pmap_t new_pmap;
  vm_map_t new_map;
  vm_map_entry_t old_entry;
  ...
  if (options &
      ~(VM_MAP_FORK_SHARE_IF_INHERIT_NONE | VM_MAP_FORK_PRESERVE_PURGEABLE |
        VM_MAP_FORK_CORPSE_FOOTPRINT | VM_MAP_FORK_SHARE_IF_OWNED)) {
    /* unsupported option */
...
#error Unknown architecture.
  ...
  new_map = vm_map_create_options(new_pmap, old_map->min_offset,
                                  old_map->max_offset, map_create_options);
  ...
  for (old_entry = vm_map_first_entry(old_map);
     ...
     */
    if ((options & VM_MAP_FORK_CORPSE_FOOTPRINT) && get_system_inshutdown()) {
    ...
    if (old_entry_inheritance == VM_INHERIT_NONE &&
        ...
        !(!old_entry->is_sub_map && VME_OBJECT(old_entry) != NULL &&
    ...
    if (old_entry_inheritance == VM_INHERIT_COPY &&
        (options & VM_MAP_FORK_SHARE_IF_OWNED) && !old_entry->is_sub_map &&
        VME_OBJECT(old_entry) != VM_OBJECT_NULL) {
      vm_object_t object;
      task_t owner;
      object = VME_OBJECT(old_entry);
      owner = VM_OBJECT_OWNER(object);
      if (owner != TASK_NULL && owner->map == old_map) {
        /*
    ...
    if (old_entry_inheritance != VM_INHERIT_NONE &&
      ...
      footprint_collect_kr =
          vm_map_corpse_footprint_collect(old_map, old_entry, new_map);
    }
    ...
    case VM_INHERIT_COPY:
      ...
      new_entry = vm_map_entry_create(
          new_map); /* never the kernel map or descendants */
      vm_map_entry_copy(old_map, new_entry, old_entry);
      ...
      if (new_entry->used_for_jit == TRUE &&
          new_map->jit_entry_exists == FALSE) {
      ...
      if (!vm_object_copy_quickly(VME_OBJECT(new_entry), VME_OFFSET(old_entry),
                                  (old_entry->vme_end - old_entry->vme_start),
                                  &src_needs_copy, &new_entry_needs_copy)) {
        vm_map_entry_dispose(new_entry);
        goto slow_vm_map_fork_copy;
      ...
      if (src_needs_copy && !old_entry->needs_copy) {
        ...
        vm_object_pmap_protect(
            VME_OBJECT(old_entry), VME_OFFSET(old_entry),
            (old_entry->vme_end - old_entry->vme_start),
            ((old_entry->is_shared || old_map->mapped_in_other_pmaps)
                 ? PMAP_NULL
                 : old_map->pmap),
            VM_MAP_PAGE_SIZE(old_map), old_entry->vme_start, prot);
    ...
    slow_vm_map_fork_copy:
      ...
      if (options & VM_MAP_FORK_PRESERVE_PURGEABLE) {
        vm_map_copyin_flags |= VM_MAP_COPYIN_PRESERVE_PURGEABLE;
      }
      if (vm_map_fork_copy(old_map, &old_entry, new_map, vm_map_copyin_flags)) {
        new_size += entry_size;
      }
      continue;
    }
 ...
 */
kern_return_t vm_map_exec(vm_map_t new_map, task_t task, boolean_t is64bit,
                          void *fsroot, cpu_type_t cpu,
                          cpu_subtype_t cpu_subtype, boolean_t reslide,
                          boolean_t is_driverkit, uint32_t rsr_version) {
  SHARED_REGION_TRACE_DEBUG(
      ("shared_region: task %p: vm_map_exec(%p,%p,%p,0x%x,0x%x): ->\n",
       (void *)VM_KERNEL_ADDRPERM(current_task()),
       (void *)VM_KERNEL_ADDRPERM(new_map), (void *)VM_KERNEL_ADDRPERM(task),
       (void *)VM_KERNEL_ADDRPERM(fsroot), cpu, cpu_subtype));
  (void)vm_commpage_enter(new_map, task, is64bit);

  (void)vm_shared_region_enter(new_map, task, is64bit, fsroot, cpu, cpu_subtype,
                               reslide, is_driverkit, rsr_version);
  ...
  for (size_t i = 0; i < num_regions; ++i) {
    ...
    kr = vm_map_enter(new_map, &address, regions[i].vmrr_size,
                      (vm_map_offset_t)0, vmk_flags, VM_OBJECT_NULL,
    ...
    if (kr != KERN_SUCCESS) {
      os_log_error(OS_LOG_DEFAULT,
                   ...
                   regions[i].vmrr_name, new_map, kr);
      return KERN_FAILURE;
    }
  }
 ...
 */
kern_return_t
vm_map_lookup_and_lock_object(vm_map_t *var_map, /* IN/OUT */
                              vm_map_offset_t vaddr, vm_prot_t fault_type,
                              ...
                              bool *contended)                   /* OUT */
{
  ...
  no_force_copy_if_executable =
      (fault_type & VM_PROT_COPY_FAIL_IF_EXECUTABLE) ? TRUE : FALSE;
...
RetryLookup:
  ...
  if ((entry == vm_map_to_entry(map)) || (vaddr < entry->vme_start) ||
      (vaddr >= entry->vme_end)) {
    vm_map_entry_t tmp_entry;
    ...
    if (!vm_map_lookup_entry(map, vaddr, &tmp_entry)) {
      ...
      if ((*real_map != map) && (*real_map != cow_sub_map_parent)) {
        vm_map_unlock(*real_map);
      }
  ...
  if (entry->is_sub_map) {
    ...
    if ((entry->use_pmap && !((fault_type & VM_PROT_WRITE) || force_copy))) {
    ...
    if (entry->needs_copy && ((fault_type & VM_PROT_WRITE) || force_copy)) {
      if (!mapped_needs_copy) {
        ...
        *var_map = VME_SUBMAP(entry);
        ...
        vm_map_lock_read(VME_SUBMAP(entry));
        *var_map = VME_SUBMAP(entry);
        if ((cow_sub_map_parent != map) && (*real_map != map)) {
          vm_map_unlock(map);
        }
      }
    } else {
    ...
    local_vaddr =
        (local_vaddr - top_entry_saved_start) + top_entry_saved_offset;
    assertf(
        VM_MAP_PAGE_ALIGNED(local_vaddr, fault_page_mask),
        "local_vaddr 0x%llx entry->vme_start 0x%llx fault_page_mask 0x%llx\n",
        (uint64_t)local_vaddr, (uint64_t)top_entry_saved_start,
  ...
  RetrySubMap:
    ...
    start_delta = submap_entry->vme_start > top_entry_saved_offset
                      ? submap_entry->vme_start - top_entry_saved_offset
                      : 0;

    end_delta = (top_entry_saved_offset + start_delta +
                 (old_end - old_start)) <= submap_entry->vme_end
                    ? 0
                    : (top_entry_saved_offset + (old_end - old_start)) -
                          submap_entry->vme_end;

    assertf(!submap_entry->is_sub_map, "Unexpected recursive submap entry %p",
            submap_entry);
    old_start += start_delta;
    old_end -= end_delta;

    if (((fault_type & VM_PROT_WRITE) || force_copy) && cow_sub_map_parent) {
      ...
      local_start = local_vaddr - (cow_parent_vaddr - old_start);
      local_end = local_vaddr + (old_end - cow_parent_vaddr);
      ...
      if ((submap_entry->protection & VM_PROT_EXECUTE) &&
          no_force_copy_if_executable) {
        //				printf("FBDP map %p entry %p start
        //0x%llx end 0x%llx wired %d strat %d\n", map, submap_entry,
        //(uint64_t)local_start, (uint64_t)local_end, submap_entry->wired_count,
        ...
        *real_map = map;
        ...
        DTRACE_VM4(submap_no_copy_executable, vm_map_t, map, vm_object_offset_t,
                   submap_entry_offset, vm_object_size_t, submap_entry_size,
                   int, kr);
        return kr;
      }
      ...
      DTRACE_VM6(submap_copy_strategically, vm_map_t, cow_sub_map_parent,
                 vm_map_offset_t, vaddr, vm_map_t, map, vm_object_size_t,
                 submap_entry_size, int, submap_entry->wired_count, int,
                 sub_object->copy_strategy);
      kr = vm_object_copy_strategically(sub_object, submap_entry_offset,
                                        submap_entry_size, false, /* forking */
                                        &copy_object, &object_copied_offset,
      ...
      if (kr != KERN_SUCCESS) {
        ...
        *real_map = map;
        ...
        ktriage_record(
            ...
            0 /* arg */);
        DTRACE_VM4(submap_copy_error_strategically, vm_object_t, sub_object,
                   vm_object_offset_t, submap_entry_offset, vm_object_size_t,
                   submap_entry_size, int, kr);
      ...
      vm_map_lookup_and_lock_object_copy_strategically_size +=
          submap_entry_size;
      if (submap_entry_size >
          vm_map_lookup_and_lock_object_copy_strategically_max) {
        vm_map_lookup_and_lock_object_copy_strategically_max =
            submap_entry_size;
      }
       ...
       */
      copy_offset =
          (local_vaddr - submap_entry->vme_start + VME_OFFSET(submap_entry));
      ...
      *var_map = cow_sub_map_parent;
      ...
      if (!vm_map_lookup_entry(map, vaddr, &entry)) {
        ...
        *real_map = map;
        ...
        copy_object = VM_OBJECT_NULL;
        vm_map_lock_write_to_read(map);
        DTRACE_VM4(submap_lookup_post_unlock, uint64_t,
                   (uint64_t)entry->vme_start, uint64_t,
                   (uint64_t)entry->vme_end, vm_map_offset_t, vaddr, int,
                   object_copied);
        return KERN_INVALID_ADDRESS;
      }
      ...
      if ((entry->protection & VM_PROT_WRITE) &&
          ...
          (csm_address_space_exempt(map->pmap) != KERN_SUCCESS) &&
#endif
          !(entry->used_for_jit) && VM_MAP_POLICY_WX_STRIP_X(map)) {
        DTRACE_VM3(cs_wx, uint64_t, (uint64_t)entry->vme_start, uint64_t,
                   (uint64_t)entry->vme_end, vm_prot_t, entry->protection);
        printf("CODE SIGNING: %d[%s] %s:%d(0x%llx,0x%llx,0x%x) can't have both "
               "write and exec at the same time\n",
               proc_selfpid(),
               (get_bsdtask_info(current_task())
      ...
      if (object_copied) {
        ...
        assert(VME_OBJECT(entry)->copy_strategy ==
               MEMORY_OBJECT_COPY_SYMMETRIC);
        assert(entry->wired_count == 0);
      ...
      if ((cow_sub_map_parent) && (cow_sub_map_parent != *real_map) &&
          (cow_sub_map_parent != map)) {
        vm_map_unlock(cow_sub_map_parent);
      }
      entry = submap_entry;
  ...
  if (((fault_type & prot) != fault_type)
      ...
      && !((fault_type & VM_PROT_EXECUTE) && !(prot & VM_PROT_EXECUTE) &&
           (prot & VM_PROT_UEXEC))
#endif
  ) {
  protection_failure:
    ...
    *real_map = map;
    ...
    DTRACE_VM3(prot_fault_detailed, vm_prot_t, fault_type, vm_prot_t, prot,
               void *, vaddr);
    /*
     * Noisy (esp. internally) and can be inferred from CrashReports. So OFF for
     * now.
     *
     * ktriage_record(thread_tid(current_thread()),
     * KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_VM, KDBG_TRIAGE_RESERVED,
     * KDBG_TRIAGE_VM_PROTECTION_FAILURE), 0);
     */
  ...
  *wired = (entry->wired_count != 0);
  ...
  if (submap_needed_copy && (prot & VM_PROT_WRITE)) {
    ...
    // proc_name_address(current_task()->bsd_info), map, vaddr);
    prot &= ~VM_PROT_WRITE;
  }
  ...
  if (VME_OBJECT(entry) == VM_OBJECT_NULL) {
    ...
    VME_OBJECT_SET(
        entry,
        vm_object_allocate((vm_map_size_t)(entry->vme_end - entry->vme_start),
                           map->serial_id),
        false, 0);
    VME_OFFSET_SET(entry, 0);
    assert(entry->use_pmap);
  ...
  *object = VME_OBJECT(entry);
  *out_prot = prot;
  KDBG_FILTERED(MACHDBG_CODE(DBG_MACH_WORKINGSET, VM_MAP_LOOKUP_OBJECT),
                VM_KERNEL_UNSLIDE_OR_PERM(*object),
                (unsigned long)VME_ALIAS(entry), 0, 0);
 ...
 */
boolean_t vm_map_verify(vm_map_t map, vm_map_version_t *version) /* REF */
{
  boolean_t result;
...
kern_return_t
vm_map_region_recurse_64(vm_map_t map, vm_map_offset_ut *address_u, /* IN/OUT */
...
recurse_again:
  ...
  for (;;) {
    if (vm_map_lookup_entry(curr_map, curr_address, &tmp_entry)) {
      ...
      if (curr_entry == vm_map_to_entry(curr_map) ||
          (curr_entry->vme_start >= curr_address + curr_max_above)) {
    ...
    if (tmp_entry == vm_map_to_entry(curr_map)) {
    ...
    } else if (tmp_entry->vme_start >= curr_address + curr_max_above) {
      /*
       * tmp_entry is beyond the scope of what we mapped of
       * this submap in the upper level: ignore it.
       */
    } else if ((next_entry == NULL) || (tmp_entry->vme_start + curr_offset <=
      ...
      next_max_above = MIN(next_max_above, next_entry->vme_end - next_address);
      next_max_below = MIN(next_max_below, curr_max_below);
      next_max_below =
          MIN(next_max_below, next_address - next_entry->vme_start);
    }
     ...
     */
    curr_max_above = MIN(curr_max_above, curr_entry->vme_end - curr_address);
    curr_max_below = MIN(curr_max_below, curr_address - curr_entry->vme_start);

    if (!curr_entry->is_sub_map || curr_depth >= user_max_depth) {
      /*
    ...
    curr_entry =
        VM_MAP_ENTRY_NULL; /* no longer valid after unlocking the map */
    if (not_in_kdp) {
  ...
  if (curr_entry == NULL) {
    ...
    if (do_region_footprint && /* we want footprint numbers */
      ...
      if (__improbable(map->pmap == NULL)) {
        /* Some VM tests reach this. (TODO make this more strict,
         * rdar://148290198) */
        panic_on_release_builds("null pmap");
      } else {
        task_ledgers_footprint(map->pmap->ledger, &ledger_resident,
                               &ledger_compressed);
      }
      ...
      if (look_for_pages) {
        ...
        submap_info->pages_resident =
            (unsigned int)(ledger_resident / effective_page_size);
        submap_info->pages_shared_now_private = 0;
        submap_info->pages_swapped_out =
            (unsigned int)(ledger_compressed / effective_page_size);
        ...
        submap_info->object_id =
            VM_OBJECT_ID_FAKE(map, task_ledgers.purgeable_nonvolatile);
  ...
  *address_u = vm_sanitize_wrap_addr(user_address + curr_skip - curr_max_below);
  *size_u = vm_sanitize_wrap_size(curr_max_above + curr_max_below);
  ...
  if (not_in_kdp) {
    if (!curr_entry->is_sub_map) {
      ...
      range_start = MAX((curr_address - curr_max_below), curr_entry->vme_start);
      range_end = MIN((curr_address + curr_max_above), curr_entry->vme_end);
      vm_map_region_walk(
          curr_map, range_start, curr_entry,
          (VME_OFFSET(curr_entry) + (range_start - curr_entry->vme_start)),
          range_end - range_start, &extended, look_for_pages,
      ...
      extended.ref_count =
          os_ref_get_count_raw(&VME_SUBMAP(curr_entry)->map_refcnt);
    }
  }
  ...
  if (look_for_pages) {
    ...
    submap_info->pages_shared_now_private = extended.pages_shared_now_private;
    ...
    if (original_count >= VM_REGION_SUBMAP_INFO_V2_COUNT_64) {
      if (curr_entry->is_sub_map) {
        submap_info->object_id_full =
            (vm_object_id_t)VM_KERNEL_ADDRHASH(VME_SUBMAP(curr_entry));
      } else if (VME_OBJECT(curr_entry)) {
        submap_info->object_id_full =
            (vm_object_id_t)VM_KERNEL_ADDRHASH(VME_OBJECT(curr_entry));
      } else {
...
kern_return_t vm_map_region(vm_map_t map,
                            ...
                            mach_port_t *object_name)      /* OUT */
{
  ...
  case VM_REGION_EXTENDED_INFO__legacy: {
    ...
    vm_map_region_walk(map, start, entry, VME_OFFSET(entry),
                       entry->vme_end - start, extended, TRUE, *count);
...
#define OBJ_RESIDENT_COUNT(obj, entry_size)                                    \
  MIN((entry_size), ((obj)->all_reusable ? (obj)->wired_page_count             \
                                         : (obj)->resident_page_count -        \
                                               (obj)->reusable_page_count))
...
void vm_map_region_top_walk(vm_map_entry_t entry, vm_region_top_info_t top) {
...

  {
    ...
    if (obj->shadow) {
      if (ref_count == 1) {
        top->private_pages_resident = OBJ_RESIDENT_COUNT(obj, entry_size);
      } else {
        top->shared_pages_resident = OBJ_RESIDENT_COUNT(obj, entry_size);
      }
      ...
      if (entry->superpage_size) {
        ...
        if (ref_count == 1 || (ref_count == 2 && obj->named)) {
...
void vm_map_region_walk(vm_map_t map, vm_map_offset_t va, vm_map_entry_t entry,
                        vm_object_offset_t offset, vm_object_size_t range,
                        vm_region_extended_info_t extended,
                        boolean_t look_for_pages,
                        mach_msg_type_number_t count) {
  struct vm_object *obj, *tmp_obj;
  ...
  if ((entry->is_sub_map) || (VME_OBJECT(entry) == VM_OBJECT_NULL) ||
      (VME_OBJECT(entry)->phys_contiguous && !entry->superpage_size)) {
    extended->share_mode = SM_EMPTY;
    extended->ref_count = 0;
    return;
  }
  ...
  if (look_for_pages) {
    for (last_offset = offset + range; offset < last_offset;
      ...
      if (do_region_footprint) {
        ...
        if (map->has_corpse_footprint) {
           ...
           */
          vm_map_corpse_footprint_query_page_info(map, va, &disp);
        } else {
          /*
           * Query the pmap.
           */
          vm_map_footprint_query_page_info_exclusive(map, entry, va, &disp);
        }
      ...
      vm_map_region_look_for_page(map, va, obj, vm_object_trunc_page(offset),
                                  ref_count, 0, extended, count);
    }
  ...
  collect_object_info:
    ...
    if (shadow_object != VM_OBJECT_NULL) {
      ...
      for (; shadow_object != VM_OBJECT_NULL; shadow_depth++) {
  ...
  if (extended->external_pager && extended->ref_count == 2 &&
      extended->share_mode == SM_SHARED) {
    extended->share_mode = SM_PRIVATE;
  } else if (extended->share_mode == SM_SHARED &&
             !(task_self_region_info_flags() &
               VM_REGION_INFO_FLAGS_NO_ALIASED)) {
    vm_map_entry_t cur;
    vm_map_entry_t last;
    int my_refs;
...
static void vm_map_region_look_for_page(
    __unused vm_map_t map, __unused vm_map_offset_t va, vm_object_t object,
    vm_object_offset_t offset, int max_refcnt, unsigned short depth,
    vm_region_extended_info_t extended, mach_msg_type_number_t count) {
  vm_page_t p;
  vm_object_t shadow;
  int ref_count;
  vm_object_t caller_object;
  ...
  while (TRUE) {
    ...
    if (object->internal && object->alive && !object->terminating &&
        object->pager_ready) {
...
static inline boolean_t vm_map_region_has_obj_ref(vm_map_entry_t entry,
                                                  vm_object_t object) {
  vm_object_t cur_obj;
 ...
 *		is often wired down.
 */
void vm_map_simplify_entry(vm_map_t map, vm_map_entry_t this_entry) {
  ...
  if ((this_entry != vm_map_to_entry(map)) &&
      ...
      (prev_entry->translated_allow_execute ==
       this_entry->translated_allow_execute) &&
    ...
    assert(VM_MAP_PAGE_ALIGNED(prev_entry->vme_start, VM_MAP_PAGE_MASK(map)));
...
void vm_map_simplify(vm_map_t map, vm_map_offset_t start) {
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_machine_attribute_sanitize(vm_map_t map, vm_map_offset_ut start_u,
                                  vm_map_offset_ut end_u,
                                  mach_vm_offset_t *start,
                                  mach_vm_offset_t *end, vm_map_size_t *size) {
  vm_sanitize_flags_t flags = VM_SANITIZE_FLAGS_SIZE_ZERO_SUCCEEDS;
 ...
 */
kern_return_t
vm_map_machine_attribute(vm_map_t map, vm_map_offset_ut start_u,
                         vm_map_offset_ut end_u,
                         ...
                         vm_machine_attribute_val_t *value) /* IN/OUT */
{
  ...
  ret = vm_map_machine_attribute_sanitize(map, start_u, end_u, &start, &end,
                                          &sync_size);
  ...
  if (attribute != MATTR_CACHE) {
    ...
    ret = pmap_attribute(map->pmap, start, end - start, attribute, value);
    vm_map_unlock(map);
    vmlp_api_end(VM_MAP_MACHINE_ATTRIBUTE, ret);
    return ret;
  }
  ...
  while (sync_size) {
    if (vm_map_lookup_entry(map, start, &entry)) {
      ...
      if (entry->is_sub_map) {
        ...
        sub_start = (start - entry->vme_start) + VME_OFFSET(entry);
        sub_end = sub_start + sub_size;
        vm_map_machine_attribute(VME_SUBMAP(entry), sub_start, sub_end,
                                 attribute, value);
        ...
        while (range) {
          ...
          if (m && !vm_page_is_fictitious(m)) {
            ret = pmap_attribute_cache_sync(VM_PAGE_GET_PHYS_PAGE(m), PAGE_SIZE,
                                            attribute, value);
 ...
 *	clustered.
 */
kern_return_t vm_map_behavior_set(vm_map_t map, vm_map_offset_t start,
                                  vm_map_offset_t end,
  ...
  if (start > end || start < vm_map_min(map) || end > vm_map_max(map)) {
  ...
  case VM_BEHAVIOR_DONTNEED:
    kr = vm_map_msync(map, start, end - start,
                      VM_SYNC_DEACTIVATE | VM_SYNC_CONTIGUOUS);
    break;

  case VM_BEHAVIOR_FREE:
    kr = vm_map_msync(map, start, end - start,
                      VM_SYNC_KILLPAGES | VM_SYNC_CONTIGUOUS);
    break;
 ...
 * b) or, fault in the pages (zero-fill, decompress etc) if it's an anonymous
 * mapping
 */
static kern_return_t vm_map_willneed(vm_map_t map, vm_map_offset_t start,
                                     vm_map_offset_t end) {
  ...
  KDBG(VMDBG_CODE(DBG_VM_MAP_WILLNEED) | DBG_FUNC_START, start, end);
  struct vm_object_fault_info fault_info = {
      .interruptible = THREAD_UNINT,
  ...
  while (start < end) {
    ...
    vm_prot_t fault_prot =
        entry->protection & VM_PROT_WRITE ? VM_PROT_WRITE : VM_PROT_READ;
    ...
    while (region_size) {
      ...
      kr = vm_pre_fault_with_info(map,
                                  vm_map_trunc_page(addr, effective_page_mask),
                                  fault_prot, &fault_info);
  ...
  KDBG(VMDBG_CODE(DBG_VM_MAP_WILLNEED) | DBG_FUNC_END, start, kr);
  vmlp_api_end(VM_MAP_WILLNEED, kr);
  return kr;
}

static boolean_t vm_map_entry_is_reusable(vm_map_entry_t entry) {
  vm_object_t object;
  ...
  if (/*entry->is_shared ||*/
      entry->is_sub_map || entry->in_transition ||
      entry->protection != VM_PROT_DEFAULT ||
      entry->max_protection != VM_PROT_ALL ||
      entry->inheritance != VM_INHERIT_DEFAULT || entry->no_cache ||
      entry->vme_permanent || entry->superpage_size != FALSE ||
      entry->zero_wired_pages || entry->wired_count != 0 ||
      entry->user_wired_count != 0) {
    return FALSE;
  }
  ...
  if (
		...
		object->ref_count == 1 &&
#endif
      object->vo_copy == VM_OBJECT_NULL && object->shadow == VM_OBJECT_NULL &&
      object->internal && object->purgable == VM_PURGABLE_DENY &&
...
static kern_return_t vm_map_reuse_pages(vm_map_t map, vm_map_offset_t start,
  ...
  for (; entry != vm_map_to_entry(map) && entry->vme_start < end;
    ...
    if (object != VM_OBJECT_NULL) {
      vm_object_lock(object);
      vm_object_reuse_pages(object, start_offset, end_offset, TRUE);
      vm_object_unlock(object);
    }
...
static kern_return_t vm_map_reusable_pages(vm_map_t map, vm_map_offset_t start,
  ...
  for (; entry != vm_map_to_entry(map) && entry->vme_start < end;
    ...
    if (((os_ref_get_count_raw(&object->ref_count) == 1) ||
         ...
         * ledgers.
         */
        !(entry->iokit_acct || (!entry->is_sub_map && !entry->use_pmap))) {
      if (os_ref_get_count_raw(&object->ref_count) != 1) {
    ...
    if (kill_pages != -1) {
      vm_object_deactivate_pages(
          object, start_offset, end_offset - start_offset, kill_pages,
          TRUE /*reusable_pages*/, kill_no_write, map->pmap, pmap_offset);
    } else {
      vm_page_stats_reusable.reusable_pages_shared++;
      DTRACE_VM4(vm_map_reusable_pages_shared, unsigned int, VME_ALIAS(entry),
                 vm_map_t, map, vm_map_entry_t, entry, vm_object_t, object);
    }
    vm_object_unlock(object);
...
static kern_return_t vm_map_pageout(vm_map_t map, vm_map_offset_t start,
  ...
  for (; entry != vm_map_to_entry(map) && entry->vme_start < end;
    ...
    if (entry->is_sub_map) {
      ...
      submap_start = VME_OFFSET(entry);
      submap_end = submap_start + (entry->vme_end - entry->vme_start);

      vm_map_lock_read(submap);

      if (!vm_map_range_check(submap, submap_start, submap_end,
                              &submap_entry)) {
        vm_map_unlock_read(submap);
        vm_map_unlock_read(map);
        vmlp_api_end(VM_MAP_PAGEOUT, KERN_INVALID_ADDRESS);
 ...
 */
static kern_return_t vm_map_zero_entry_preflight(vm_map_entry_t entry) {
  /*
   * Zeroing is restricted to writable non-executable entries and non-JIT
   * regions.
   */
  if (!(entry->protection & VM_PROT_WRITE) ||
      (entry->protection & VM_PROT_EXECUTE) || entry->used_for_jit ||
 ...
 */
static void vm_map_get_bounds_in_object(vm_map_entry_t entry,
                                        vm_map_offset_t start,
                                        vm_map_offset_t end,
...
static kern_return_t vm_map_zero(vm_map_t map, vm_map_offset_t start,
  ...
  while (entry != vm_map_to_entry(map) && entry->vme_start < end) {
     ...
     */
    vm_map_lock_read(map);
    if (last_timestamp != map->timestamp || cur_offset < end_offset) {
      /*
...
vm_map_entry_insert(vm_map_t map, vm_map_entry_t insp_entry,
  ...
  assertf(!os_add_overflow(end - start, offset, &end_offset),
          "size 0x%llx, offset 0x%llx caused overflow", (uint64_t)(end - start),
          offset);
...
static kern_return_t vm_map_remap_extract(
    vm_map_t map, vm_map_offset_t addr, vm_map_size_t size, boolean_t copy,
    vm_map_copy_t map_copy, vm_prot_t *cur_protection, /* IN/OUT */
  ...
  assert(inheritance == VM_INHERIT_NONE || inheritance == VM_INHERIT_COPY ||
  ...
  while (mapped_size != size) {
    ...
    entry_size = (vm_map_size_t)(src_entry->vme_end - src_entry->vme_start);

    if (src_entry->is_sub_map && vmk_flags.vmkf_copy_single_object) {
      vm_map_t submap;
      vm_map_offset_t submap_start;
      ...
      DTRACE_VM7(remap_submap_recurse, vm_map_t, map, vm_map_offset_t, addr,
                 vm_map_size_t, size, boolean_t, copy, vm_map_offset_t,
                 submap_start, vm_map_size_t, submap_size, boolean_t,
                 submap_needs_copy);

      result = vm_map_remap_extract(submap, submap_start, submap_size,
                                    submap_needs_copy, map_copy, cur_protection,
                                    max_protection, inheritance, vmk_flags);
      vm_map_deallocate(submap);

      if (result == KERN_SUCCESS && submap_needs_copy && !copy) {
        /*
        ...
        if (copy_object == VM_OBJECT_NULL) {
          ...
          /* already shareable */
          assert(!copy_entry->needs_copy);
        } else if (copy_entry->needs_copy || copy_object->shadowed ||
                   (copy_object->internal && !copy_object->true_share &&
    ...
    if (src_entry->is_sub_map) {
      ...
      /* protections for submap mapping are irrelevant here */
               ...
               ((src_entry->max_protection & required_max_prot) !=
                required_max_prot)) {
      if (vmk_flags.vmkf_copy_single_object && mapped_size != 0) {
        /*
    ...
    if (src_entry->is_sub_map) {
      ...
      result = vm_map_copy_extract(
          submap, submap_start, submap_size, submap_needs_copy, &submap_copy,
          &submap_curprot, &submap_maxprot, inheritance, vmk_flags);
      ...
      while (vm_map_copy_first_entry(submap_copy) !=
        ...
        _vm_map_store_entry_link(map_header, VMH_PREV(map_header), copy_entry);
      }
      ...
      if (src_entry->iokit_acct) {
        ...
        assertf(!src_entry->use_pmap,
                "map=%p src_entry=%p [0x%llx:0x%llx] 0x%x/0x%x %d", map,
                src_entry, (uint64_t)src_entry->vme_start,
                (uint64_t)src_entry->vme_end, src_entry->protection,
                src_entry->max_protection, VME_ALIAS(src_entry));
      } else {
        /*
      ...
      if (object == VM_OBJECT_NULL) {
        ...
        if (!src_entry->needs_copy && is_writable) {
          ...
          if (pmap_has_prot_policy(map->pmap,
                                   src_entry->translated_allow_execute,
                                   src_entry->protection)) {
            panic("%s: map %p pmap %p entry %p 0x%llx:0x%llx prot 0x%x",
                  __FUNCTION__, map, map->pmap, src_entry,
                  (uint64_t)src_entry->vme_start, (uint64_t)src_entry->vme_end,
                  src_entry->protection);
          }
          ...
          if (pmap_has_prot_policy(map->pmap,
                                   src_entry->translated_allow_execute, prot)) {
            panic("%s: map %p pmap %p entry %p 0x%llx:0x%llx prot 0x%x",
                  __FUNCTION__, map, map->pmap, src_entry,
                  (uint64_t)src_entry->vme_start, (uint64_t)src_entry->vme_end,
                  prot);
          ...
          if (map->mapped_in_other_pmaps) {
            vm_object_pmap_protect(VME_OBJECT(src_entry), VME_OFFSET(src_entry),
                                   entry_size, PMAP_NULL, PAGE_SIZE,
                                   src_entry->vme_start, prot);
#if MACH_ASSERT
            ...
            pmap_protect(vm_map_pmap(map), src_entry->vme_start,
                         src_entry->vme_end, prot);
          }
        }
    ...
    offset = (VME_OFFSET(src_entry) + (src_start - src_entry->vme_start));
  ...
  RestartCopy:
    if (!copy) {
      ...
      if (!src_entry->is_sub_map && VME_OBJECT(src_entry) == VM_OBJECT_NULL) {
      ...
      if (!new_entry->is_sub_map && VME_OBJECT(new_entry) == VM_OBJECT_NULL) {
        /* no accessible memory; nothing to share */
      ...
      if (src_needs_copy && !src_entry->needs_copy) {
        ...
        vm_object_pmap_protect(
            object, offset, entry_size,
            ((src_entry->is_shared || map->mapped_in_other_pmaps) ? PMAP_NULL
                                                                  : map->pmap),
            VM_MAP_PAGE_SIZE(map), src_entry->vme_start, prot);
      ...
      if (src_entry_was_wired > 0 ||
        ...
        result = vm_object_copy_slowly(
            object, offset, (new_entry->vme_end - new_entry->vme_start),
            THREAD_UNINT, &new_copy_object);
        /* VME_OBJECT_SET will reset used_for_jit, so preserve it. */
        saved_used_for_jit = new_entry->used_for_jit;
        VME_OBJECT_SET(new_entry, new_copy_object, false, 0);
    ...
    _vm_map_store_entry_link(map_header, VMH_PREV(map_header), new_entry);
    ...
    if (vmk_flags.vmkf_copy_single_object) {
      if (mapped_size != size) {
        DEBUG4K_SHARE("map %p addr 0x%llx size 0x%llx clipped copy at "
                      "mapped_size 0x%llx\n",
                      map, (uint64_t)addr, (uint64_t)size,
...
bool vm_map_is_exotic(vm_map_t map) { return VM_MAP_IS_EXOTIC(map); }

bool vm_map_is_alien(vm_map_t map) { return VM_MAP_IS_ALIEN(map); }

#if XNU_TARGET_OS_OSX
void vm_map_mark_alien(vm_map_t map) {
...
void vm_map_single_jit(vm_map_t map) {
 ...
 * one to ensure that it hasn't been forged.
 */
static kern_return_t vm_map_copy_to_physcopy(vm_map_copy_t copy_map,
                                             vm_map_t target_map) {
  vm_map_size_t size;
  vm_map_entry_t entry;
  vm_map_entry_t new_entry;
  ...
  DEBUG4K_COPY("copy_map %p (%d %d 0x%llx 0x%llx) BEFORE\n", copy_map,
               copy_map->cpy_hdr.page_shift, copy_map->cpy_hdr.nentries,
               copy_map->offset, (uint64_t)copy_map->size);
  ...
  src_start = 0;
  kr = vm_map_copyout_internal(new_map, &src_start, copy_map, copy_map->size,
  ...
  dst_start = 0;
  kr =
      vm_map_enter(new_map, &dst_start, size, 0, /* mask */
  ...
  for (src_cur = src_start, dst_cur = dst_start; src_cur < src_end;
    ...
    if (kr != KERN_SUCCESS) {
      DEBUG4K_COPY("copyinmap(%p, 0x%llx, %p, 0x%llx) kr 0x%x\n", new_map,
                   (uint64_t)src_cur, kbuf, (uint64_t)bytes, kr);
    }
    ...
    if (kr != KERN_SUCCESS) {
      DEBUG4K_COPY("copyoutmap(%p, %p, 0x%llx, 0x%llx) kr 0x%x\n", new_map,
                   kbuf, (uint64_t)dst_cur, (uint64_t)bytes, kr);
    }
  }
...
void vm_map_copy_adjust_get_target_copy_map(vm_map_copy_t copy_map,
...
void vm_map_copy_adjust_get_target_copy_map(vm_map_copy_t copy_map,
  ...
  for (entry = vm_map_copy_first_entry(copy_map);
       entry != vm_map_copy_to_entry(copy_map); entry = entry->vme_next) {
    ...
    vm_map_copy_entry_link(
        target_copy_map, vm_map_copy_last_entry(target_copy_map), target_entry);
  }
  entry = VM_MAP_ENTRY_NULL;
 ...
 * one to ensure that it hasn't been forged.
 */
static void vm_map_copy_trim(vm_map_copy_t copy_map, uint16_t new_page_shift,
  ...
  for (entry = vm_map_copy_first_entry(copy_map);
       entry != vm_map_copy_to_entry(copy_map); entry = next_entry) {
...
kern_return_t vm_map_copy_adjust_to_target(
    vm_map_copy_t src_copy_map, vm_map_offset_ut offset_u,
    vm_map_size_ut size_u, vm_map_t target_map, boolean_t copy,
    vm_map_copy_t *target_copy_map_p, vm_map_offset_t *overmap_start_p,
    vm_map_offset_t *overmap_end_p, vm_map_offset_t *trimmed_start_p) {
  ...
  kr = vm_map_copy_addr_size_sanitize(
      target_map, offset_u, size_u,
      VM_SANITIZE_CALLER_MACH_MEMORY_ENTRY_MAP_SIZE, &new_start, &new_end,
      &map_size);
  if (__improbable(kr != KERN_SUCCESS)) {
    return vm_sanitize_get_kr(kr);
  }
  ...
  DEBUG4K_ADJUST(
      "copy_map %p (%d offset 0x%llx size 0x%llx) target_map %p (%d) copy %d "
      "offset 0x%llx size 0x%llx target_copy_map %p...\n",
      copy_map, copy_page_shift, (uint64_t)copy_map->offset,
      (uint64_t)copy_map->size, target_map, target_page_shift, copy,
      (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(offset_u),
      (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(size_u), *target_copy_map_p);
  ...
  if (new_end > copy_map->size) {
    DEBUG4K_ERROR("copy_map %p (%d->%d) copy_map->size 0x%llx offset 0x%llx "
                  "size 0x%llx KERN_INVALID_ARGUMENT\n",
                  copy_map, copy_page_shift, target_page_shift,
                  (uint64_t)copy_map->size,
                  (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(offset_u),
                  (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(size_u));
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (new_end < copy_map->size) {
    ...
    DEBUG4K_ADJUST("copy_map %p (%d->%d) copy %d offset 0x%llx size 0x%llx "
                   "target_copy_map %p... trim end from 0x%llx to 0x%llx\n",
                   copy_map, copy_page_shift, target_page_shift, copy,
                   (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(offset_u),
                   (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(size_u), target_copy_map,
                   (uint64_t)new_end, (uint64_t)copy_map->size);
    ...
    vm_map_copy_trim(target_copy_map, target_page_shift, new_end,
                     copy_map->size);
  }
  ...
  if (new_start != 0) {
    DEBUG4K_ADJUST("copy_map %p (%d->%d) copy %d offset 0x%llx size 0x%llx "
                   "target_copy_map %p... trim start from 0x%llx to 0x%llx\n",
                   copy_map, copy_page_shift, target_page_shift, copy,
                   (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(offset_u),
                   (uint64_t)VM_SANITIZE_UNSAFE_UNWRAP(size_u), target_copy_map,
    ...
    vm_map_copy_adjust_get_target_copy_map(copy_map, &target_copy_map);
    copy_map = target_copy_map;
    vm_map_copy_trim(target_copy_map, target_page_shift, 0, new_start);
  }
  *trimmed_start_p = new_start;
  ...
  assertf(target_size == src_copy_map_size - *trimmed_start_p - trimmed_end,
          "target_size 0x%llx src_copy_map_size 0x%llx trimmed_start 0x%llx "
          "trimmed_end 0x%llx\n",
  ...
  if (!overmap_start && !overmap_end && !misalignments) {
    ...
    if (*trimmed_start_p) {
      ...
      *overmap_end_p = 0;
      DEBUG4K_ADJUST(
          "copy_map %p (%d offset 0x%llx size 0x%llx) target_map %p (%d) copy "
          "%d target_copy_map %p (%d offset 0x%llx size 0x%llx) -> trimmed "
          "0x%llx overmap start 0x%llx end 0x%llx KERN_SUCCESS\n",
          copy_map, copy_page_shift, (uint64_t)copy_map->offset,
          (uint64_t)copy_map->size, target_map, target_page_shift, copy,
          *target_copy_map_p, VM_MAP_COPY_PAGE_SHIFT(*target_copy_map_p),
          (uint64_t)(*target_copy_map_p)->offset,
          (uint64_t)(*target_copy_map_p)->size, (uint64_t)*trimmed_start_p,
          (uint64_t)*overmap_start_p, (uint64_t)*overmap_end_p);
      return KERN_SUCCESS;
    }
...
#endif /* MACH_ASSERT */
    DEBUG4K_ADJUST("copy_map %p (%d) target_map %p (%d) copy %d "
                   "target_copy_map %p -> KERN_NOT_SUPPORTED\n",
                   copy_map, copy_page_shift, target_map, target_page_shift,
  ...
  if (misalignments && copy) {
    ...
    *overmap_end_p = target_copy_map->size - target_copy_map_size;
    DEBUG4K_ADJUST(
        "copy_map %p (%d offset 0x%llx size 0x%llx) target_map %p (%d) copy %d "
        "target_copy_map %p (%d offset 0x%llx size 0x%llx)-> trimmed 0x%llx "
        "overmap start 0x%llx end 0x%llx PHYSCOPY\n",
        copy_map, copy_page_shift, (uint64_t)copy_map->offset,
        (uint64_t)copy_map->size, target_map, target_page_shift, copy,
        *target_copy_map_p, VM_MAP_COPY_PAGE_SHIFT(*target_copy_map_p),
        (uint64_t)(*target_copy_map_p)->offset,
  ...
  for (target_entry = vm_map_copy_first_entry(target_copy_map);
    ...
    DEBUG4K_ADJUST("copy %p (%d 0x%llx 0x%llx) entry %p [ 0x%llx 0x%llx ] "
                   "object %p offset 0x%llx BEFORE\n",
                   target_copy_map, VM_MAP_COPY_PAGE_SHIFT(target_copy_map),
                   target_copy_map->offset, (uint64_t)target_copy_map->size,
                   target_entry, (uint64_t)target_entry->vme_start,
                   (uint64_t)target_entry->vme_end, VME_OBJECT(target_entry),
                   VME_OFFSET(target_entry));
    object_offset_start = VME_OFFSET(target_entry);
    if (object_offset_start & target_page_mask) {
      DEBUG4K_ADJUST("copy %p (%d 0x%llx 0x%llx) entry %p [ 0x%llx 0x%llx ] "
                     "object %p offset 0x%llx misaligned at start\n",
                     target_copy_map, VM_MAP_COPY_PAGE_SHIFT(target_copy_map),
                     target_copy_map->offset, (uint64_t)target_copy_map->size,
                     target_entry, (uint64_t)target_entry->vme_start,
                     (uint64_t)target_entry->vme_end, VME_OBJECT(target_entry),
      ...
      if (target_entry == vm_map_copy_first_entry(target_copy_map)) {
        ...
        overmap_start =
            object_offset_start -
            trunc_page_mask_64(object_offset_start, target_page_mask);
        DEBUG4K_ADJUST(
            "entry %p offset 0x%llx copy %d -> overmap_start 0x%llx\n",
            target_entry, VME_OFFSET(target_entry), copy,
            (uint64_t)overmap_start);
    ...
    object_offset_end = VME_OFFSET(target_entry) + target_entry->vme_end -
                        target_entry->vme_start;
    if (object_offset_end & target_page_mask) {
      DEBUG4K_ADJUST("copy %p (%d 0x%llx 0x%llx) entry %p [ 0x%llx 0x%llx ] "
                     "object %p offset 0x%llx misaligned at end\n",
                     target_copy_map, VM_MAP_COPY_PAGE_SHIFT(target_copy_map),
                     target_copy_map->offset, (uint64_t)target_copy_map->size,
                     target_entry, (uint64_t)target_entry->vme_start,
                     (uint64_t)target_entry->vme_end, VME_OBJECT(target_entry),
      ...
      if (target_entry->vme_next == vm_map_copy_to_entry(target_copy_map)) {
         ...
         */
        overmap_end = round_page_mask_64(object_offset_end, target_page_mask) -
                      object_offset_end;
  ...
  *overmap_end_p = overmap_end;
  ...
  DEBUG4K_ADJUST(
      "copy_map %p (%d offset 0x%llx size 0x%llx) target_map %p (%d) copy %d "
      "target_copy_map %p (%d offset 0x%llx size 0x%llx) -> trimmed 0x%llx "
      "overmap start 0x%llx end 0x%llx KERN_SUCCESS\n",
      copy_map, copy_page_shift, (uint64_t)copy_map->offset,
      (uint64_t)copy_map->size, target_map, target_page_shift, copy,
      *target_copy_map_p, VM_MAP_COPY_PAGE_SHIFT(*target_copy_map_p),
      (uint64_t)(*target_copy_map_p)->offset,
...
kern_return_t vm_map_range_physical_size(vm_map_t map, vm_map_address_t start,
                                         mach_vm_size_t size,
  ...
  if (__improbable(os_add_overflow(start, size, &end) ||
    ...
    printf("%s:%d(start=0x%llx, size=0x%llx) pgmask 0x%x: wraparound\n",
           __FUNCTION__, __LINE__, (uint64_t)start, (uint64_t)size,
           VM_MAP_PAGE_MASK(map));
    *phys_size = 0;
    return KERN_INVALID_ARGUMENT;
  }
  ...
  *phys_size = adjusted_size;
  ...
  vmk_flags.vmkf_remap_legacy_mode = true;
  kr = vm_map_copy_extract(map, adjusted_start, adjusted_size, FALSE /* copy */,
                           &copy_map, &cur_prot, &max_prot, VM_INHERIT_DEFAULT,
                           vmk_flags);
  if (kr != KERN_SUCCESS) {
    DEBUG4K_ERROR("map %p start 0x%llx 0x%llx size 0x%llx 0x%llx kr 0x%x\n",
                  map, (uint64_t)start, (uint64_t)adjusted_start, size,
                  (uint64_t)adjusted_size, kr);
    // assert(0);
    *phys_size = 0;
    return kr;
  }
  ...
  DEBUG4K_ADJUST("adjusting...\n");
  kr = vm_map_copy_adjust_to_target(
      copy_map, start - adjusted_start, /* offset */
      size,                             /* size */
      kernel_map, FALSE,                /* copy */
      &target_copy_map, &overmap_start, &overmap_end, &trimmed_start);
  if (kr == KERN_SUCCESS) {
    if (target_copy_map->size != *phys_size) {
      DEBUG4K_ADJUST(
          "map %p (%d) start 0x%llx size 0x%llx adjusted_start 0x%llx "
          "adjusted_end 0x%llx overmap_start 0x%llx overmap_end 0x%llx "
          "trimmed_start 0x%llx phys_size 0x%llx -> 0x%llx\n",
          map, VM_MAP_PAGE_SHIFT(map), (uint64_t)start, (uint64_t)size,
          (uint64_t)adjusted_start, (uint64_t)adjusted_end,
          (uint64_t)overmap_start, (uint64_t)overmap_end,
          (uint64_t)trimmed_start, (uint64_t)*phys_size,
          (uint64_t)target_copy_map->size);
    }
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_remap_sanitize(vm_map_t src_map, vm_map_t target_map,
                      vm_map_address_ut address_u, vm_map_size_ut size_u,
                      vm_map_offset_ut mask_u,
                      vm_map_offset_ut memory_address_u,
                      vm_prot_ut cur_protection_u, vm_prot_ut max_protection_u,
                      vm_inherit_ut inheritance_u,
                      vm_map_kernel_flags_t vmk_flags,
                      vm_map_address_t *target_addr, vm_map_address_t *mask,
                      vm_map_offset_t *memory_address,
                      vm_map_offset_t *memory_end, vm_map_size_t *memory_size,
                      vm_prot_t *cur_protection, vm_prot_t *max_protection,
                      vm_inherit_t *inheritance) {
  kern_return_t result;
  ...
  result = vm_sanitize_cur_and_max_prots(
      cur_protection_u, max_protection_u, VM_SANITIZE_CALLER_VM_MAP_REMAP,
      target_map, cur_protection, max_protection);
  if (__improbable(result != KERN_SUCCESS)) {
    return result;
  }
  ...
  result = vm_sanitize_addr_size(
      memory_address_u, size_u, VM_SANITIZE_CALLER_VM_MAP_REMAP, src_map,
      vm_sanitize_flags, memory_address, memory_end, memory_size);
  if (__improbable(result != KERN_SUCCESS)) {
    return result;
  }
 ...
 */
kern_return_t vm_map_remap(vm_map_t target_map, vm_map_address_ut *address_u,
                           vm_map_size_ut size_u, vm_map_offset_ut mask_u,
                           vm_map_kernel_flags_t vmk_flags, vm_map_t src_map,
                           vm_map_offset_ut memory_address_u, boolean_t copy,
  ...
  if (src_page_mask != target_page_mask) {
    if (copy) {
      DEBUG4K_COPY("src_map %p pgsz 0x%x addr 0x%llx size 0x%llx copy %d -> "
                   "target_map %p pgsz 0x%x\n",
                   src_map, VM_MAP_PAGE_SIZE(src_map),
                   VM_SANITIZE_UNSAFE_UNWRAP(memory_address_u),
                   VM_SANITIZE_UNSAFE_UNWRAP(size_u), copy, target_map,
                   VM_MAP_PAGE_SIZE(target_map));
    } else {
      DEBUG4K_SHARE("src_map %p pgsz 0x%x addr 0x%llx size 0x%llx copy %d -> "
                    "target_map %p pgsz 0x%x\n",
                    src_map, VM_MAP_PAGE_SIZE(src_map),
                    VM_SANITIZE_UNSAFE_UNWRAP(memory_address_u),
                    VM_SANITIZE_UNSAFE_UNWRAP(size_u), copy, target_map,
   ...
   * Sanitize any input parameters that are addr/size/prot/inherit
   */
  result = vm_map_remap_sanitize(
      src_map, target_map, *address_u, size_u, mask_u, memory_address_u,
      *cur_protection_u, *max_protection_u, inheritance_u, vmk_flags,
      &target_addr, &mask, &memory_address, &memory_end, &memory_size,
      &cur_protection, &max_protection, &inheritance);
  if (__improbable(result != KERN_SUCCESS)) {
  ...
  result = vm_map_copy_extract(src_map, memory_address, memory_size, copy,
                               &copy_map, &cur_protection, /* IN/OUT */
  ...
  if (src_page_mask != target_page_mask) {
    ...
    result = vm_map_copy_adjust_to_target(
        copy_map, offset_in_mapping, /* offset */
        initial_size, target_map, copy, &target_copy_map, &overmap_start,
        &overmap_end, &trimmed_start);
    if (result != KERN_SUCCESS) {
  ...
  if (__improbable(!vm_map_is_map_size_valid(target_map, target_size,
                                             vmk_flags.vmkf_no_soft_limit))) {
    vm_map_copy_discard(copy_map);
    return KERN_NO_SPACE;
  }
  ...
  if (!vmk_flags.vmf_fixed) {
    result = vm_map_locate_space_anywhere(target_map, target_size, mask,
                                          vmk_flags, &target_addr, &insp_entry);
  } else {
    /*
     * vm_map_locate_space_fixed will reject overflowing
     * target_addr + target_size values
     */
    result = vm_map_locate_space_fixed(target_map, target_addr, target_size,
                                       mask, vmk_flags, &insp_entry, &zap_list);
  ...
  if (result == KERN_SUCCESS) {
    while (vm_map_copy_first_entry(copy_map) !=
      ...
      if (vmk_flags.vmkf_remap_prot_copy) {
        ...
        if (entry->vme_permanent) {
          ...
          if ((entry->max_protection & VM_PROT_EXECUTE) &&
            ...
            printf("%d[%s] vm_remap(0x%llx,0x%llx) VM_PROT_COPY denied on "
                   "permanent mapping prot 0x%x/0x%x developer %d\n",
                   proc_selfpid(),
                   (get_bsdtask_info(current_task())
                        ? proc_name_address(get_bsdtask_info(current_task()))
                        : "?"),
                   (uint64_t)memory_address, (uint64_t)memory_size,
                   entry->protection, entry->max_protection,
                   developer_mode_state());
            DTRACE_VM6(vm_map_delete_permanent_deny_protcopy, vm_map_entry_t,
                       entry, vm_map_offset_t, entry->vme_start,
                       vm_map_offset_t, entry->vme_end, vm_prot_t,
                       entry->protection, vm_prot_t, entry->max_protection, int,
                       VME_ALIAS(entry));
          }
        } else {
          allow_write = true;
      ...
      if (vmk_flags.vmf_resilient_media && !entry->is_sub_map &&
          (VME_OBJECT(entry) == VM_OBJECT_NULL ||
           VME_OBJECT(entry)->internal)) {
        entry->vme_resilient_media = TRUE;
      }
      assert(VM_MAP_PAGE_ALIGNED(entry->vme_start,
                                 MIN(target_page_mask, PAGE_MASK)));
      assert(VM_MAP_PAGE_ALIGNED(entry->vme_end,
                                 MIN(target_page_mask, PAGE_MASK)));
      assert(VM_MAP_PAGE_ALIGNED(VME_OFFSET(entry),
                                 MIN(target_page_mask, PAGE_MASK)));
      vm_map_store_entry_link(target_map, insp_entry, entry, vmk_flags);
      insp_entry = entry;
    }
  }
  ...
  if (result == KERN_SUCCESS && target_map->wiring_required) {
    result = vm_map_wire_nested(target_map, target_addr,
                                target_addr + target_size, cur_protection,
                                VM_KERN_MEMORY_MLOCK, TRUE, PMAP_NULL, 0, NULL);
  }
  ...
  if (src_page_mask != target_page_mask) {
    DEBUG4K_SHARE(
        "vm_remap(%p 0x%llx 0x%llx copy=%d-> %p 0x%llx 0x%llx  result=0x%x\n",
        src_map, (uint64_t)memory_address, (uint64_t)target_size, copy,
        target_map, (uint64_t)target_addr, (uint64_t)offset_in_mapping, result);
  }
  vm_map_copy_discard(copy_map);
  copy_map = VM_MAP_COPY_NULL;
...
void vm_map_switch_back(vm_map_switch_context_t ctx) {
  thread_t thread = current_thread();
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_rw_user_sanitize(vm_map_t map, vm_map_address_ut addr_u,
                        vm_size_ut size_u,
                        vm_sanitize_caller_t vm_sanitize_caller,
 ...
 *
 */
kern_return_t vm_map_write_user(vm_map_t map, void *src_p,
   ...
   */
  kr = vm_map_rw_user_sanitize(map, dst_addr_u, size_u,
                               VM_SANITIZE_CALLER_VM_MAP_WRITE_USER, &dst_addr,
                               &dst_end, &size);
  if (__improbable(kr != KERN_SUCCESS)) {
    return vm_sanitize_get_kr(kr);
  }
 ...
 *
 */
kern_return_t vm_map_read_user(vm_map_t map, vm_map_address_ut src_addr_u,
                               void *dst_p, vm_size_ut size_u) {
  kern_return_t kr;
   ...
   */
  kr = vm_map_rw_user_sanitize(map, src_addr_u, size_u,
                               VM_SANITIZE_CALLER_VM_MAP_READ_USER, &src_addr,
                               &src_end, &size);
  if (__improbable(kr != KERN_SUCCESS)) {
    return vm_sanitize_get_kr(kr);
  }
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_check_protection_sanitize(vm_map_t map, vm_map_offset_ut start_u,
                                 vm_map_offset_ut end_u,
 ...
 */
boolean_t vm_map_check_protection(vm_map_t map, vm_map_offset_ut start_u,
                                  vm_map_offset_ut end_u,
                                  vm_prot_ut protection_u,
                                  vm_sanitize_caller_t vm_sanitize_caller) {
  vm_map_entry_t entry;
  ...
  kr = vm_map_check_protection_sanitize(map, start_u, end_u, protection_u,
                                        vm_sanitize_caller, &start, &end,
                                        &protection);
  if (__improbable(kr != KERN_SUCCESS)) {
    kr = vm_sanitize_get_kr(kr);
    if (kr == KERN_SUCCESS) {
...
kern_return_t vm_map_purgable_control(vm_map_t map, vm_map_offset_ut address_u,
  ...
  if (control != VM_PURGABLE_SET_STATE && control != VM_PURGABLE_GET_STATE &&
  ...
  if (was_nonvolatile && object->purgable != VM_PURGABLE_NONVOLATILE &&
      map->pmap == kernel_pmap) {
#if DEBUG
...
static void vm_map_footprint_query_page_info_exclusive(
  ...
  if (!map_entry->is_sub_map && object->vo_no_footprint) {
    /* does not count in footprint */
    //		assertf(!map_entry->use_pmap, "offset 0x%llx map_entry %p",
    //(uint64_t) curr_s_offset, map_entry);
  } else if (!map_entry->is_sub_map && !object->internal &&
             object->vo_ledger_tag && VM_OBJECT_OWNER(object) != NULL &&
             VM_OBJECT_OWNER(object)->map == map) {
    /* owned external object: wired pages count in footprint */
    assertf(map_entry->use_pmap, "offset 0x%llx map_entry %p",
            (uint64_t)curr_s_offset, map_entry);
    if ((((curr_s_offset - map_entry->vme_start + VME_OFFSET(map_entry)) /
          effective_page_size) < object->wired_page_count)) {
      /*
  ...
  } else if (!map_entry->is_sub_map && map_entry->iokit_acct &&
             object->internal && object->purgable == VM_PURGABLE_DENY) {
    /*
    ...
    disposition |= VM_PAGE_QUERY_PAGE_DIRTY;
             ...
             (PMAP_QUERY_PAGE_ALTACCT | PMAP_QUERY_PAGE_COMPRESSED_ALTACCT)) {
    /* alternate accounting */
...
kern_return_t vm_map_page_info(vm_map_t map, vm_map_offset_ut offset_u,
                               vm_page_info_flavor_t flavor,
                               vm_page_info_t info,
                               mach_msg_type_number_t *count) {
  return vm_map_page_range_info_internal(
      map, offset_u, /* start of range */
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_page_range_info_sanitize(vm_map_t map, vm_map_offset_ut start_offset_u,
  ...
  retval = vm_sanitize_addr_end(
      start_offset_u, end_offset_u, VM_SANITIZE_CALLER_VM_MAP_PAGE_RANGE_INFO,
      effective_page_mask, VM_SANITIZE_FLAGS_SIZE_ZERO_FALLTHROUGH, start, end,
      &size);
  if (retval != KERN_SUCCESS) {
    return retval;
  }
  ...
  *offset_in_page =
      vm_sanitize_offset_in_page(effective_page_mask, start_offset_u);

  return KERN_SUCCESS;
}

kern_return_t vm_map_page_range_info_internal(vm_map_t map,
  ...
  vm_map_offset_t offset_in_page = 0, offset_in_object = 0,
                  curr_offset_in_object = 0;
  ...
  retval = vm_map_page_range_info_sanitize(map, start_offset_u, end_offset_u,
                                           effective_page_mask, &start, &end,
                                           &offset_in_page);
  if (retval != KERN_SUCCESS) {
  ...
  for (curr_s_offset = start; curr_s_offset < end;) {
    ...
    if (!vm_map_lookup_entry(map, curr_s_offset, &map_entry)) {
      ...
      uint64_t num_pages =
          (curr_e_offset - curr_s_offset) >> effective_page_shift;

      void *info_ptr = (void *)(((uintptr_t)info) +
                                (info_idx * sizeof(struct vm_page_info_basic)));
    ...
    if (map_entry->is_sub_map) {
      ...
      retval = vm_map_page_range_info_internal(
          sub_map, submap_s_offset, submap_e_offset, effective_page_shift,
          VM_PAGE_INFO_BASIC, (vm_page_info_t)submap_info, count);
    ...
    if (do_region_footprint) {
      ...
      if (map->has_corpse_footprint) {
         ...
         * while forking the corpse.
         */
        vm_map_corpse_footprint_query_page_info(map, curr_s_offset,
                                                &disposition);
      } else {
        /*
         ...
         * about this page.
         */
        vm_map_footprint_query_page_info_exclusive(map, map_entry,
                                                   curr_s_offset, &disposition);
      }
      switch (flavor) {
      case VM_PAGE_INFO_BASIC:
        basic_info =
    ...
    for (; curr_s_offset < curr_e_offset;) {
      ...
      for (;;) {
        ...
        if (m != VM_PAGE_NULL) {
          ...
          if (curr_object->internal && curr_object->alive &&
              !curr_object->terminating && curr_object->pager_ready) {
            if (vm_object_compressor_pager_state_get(
                    curr_object, vm_object_trunc_page(curr_offset_in_object)) ==
                VM_EXTERNAL_STATE_EXISTS) {
              /* the pager has that page */
              disposition |= VM_PAGE_QUERY_PAGE_PAGED_OUT;
              break;
      ...
      case VM_PAGE_INFO_BASIC:
        ...
        basic_info->object_id =
            (vm_object_id_t)(uintptr_t)VM_KERNEL_ADDRHASH(curr_object);
        basic_info->offset =
      ...
      disposition = 0;
      offset_in_page = 0; // This doesn't really make sense for any offset other
...
static __attribute__((always_inline, warn_unused_result)) kern_return_t
vm_map_msync_sanitize(vm_map_t map, vm_map_address_ut address_u,
                      vm_map_size_ut size_u, vm_object_offset_t *address,
                      vm_map_size_t *size) {
  vm_object_offset_t end;
  vm_sanitize_flags_t flags = VM_SANITIZE_FLAGS_SIZE_ZERO_SUCCEEDS;

  return vm_sanitize_addr_size(address_u, size_u,
                               VM_SANITIZE_CALLER_VM_MAP_MSYNC, map, flags,
                               address, &end, size);
}
...
kern_return_t vm_map_msync(vm_map_t map, vm_map_address_ut address_u,
                           vm_map_size_ut size_u, vm_sync_t sync_flags) {
  vm_map_entry_t entry;
  ...
  kr = vm_map_msync_sanitize(map, address_u, size_u, &address, &size);
  if (VM_MAP_PAGE_MASK(map) < PAGE_MASK) {
    DEBUG4K_SHARE("map %p address 0x%llx size 0x%llx flags 0x%x\n", map,
                  (uint64_t)address, (uint64_t)size, sync_flags);
  }
  if (__improbable(kr != KERN_SUCCESS)) {
    kr = vm_sanitize_get_kr(kr);
  ...
  while (amount_left > 0) {
    ...
    if (!vm_map_lookup_entry(map, address, &entry)) {
       ...
       * Check for empty map.
       */
      if (entry == vm_map_to_entry(map) && entry->vme_next == entry) {
        vm_map_unlock(map);
        break;
      }
      /*
       ...
       */
      if ((map->hdr.nentries == 0) || (entry->vme_next->vme_start < address)) {
        vm_map_unlock(map);
        break;
      }
      /*
    ...
    if (amount_left + entry->vme_start + offset > entry->vme_end) {
      flush_size = entry->vme_end - (entry->vme_start + offset);
    } else {
    ...
    if (entry->is_sub_map == TRUE) {
      ...
      vm_map_unlock(map);
      if (vm_map_msync(local_map, local_offset, flush_size, sync_flags) ==
    ...
    if (sync_flags & (VM_SYNC_KILLPAGES | VM_SYNC_DEACTIVATE)) {
      ...
      if (kill_pages != -1) {
        ...
        vm_object_deactivate_pages(
            object, start_offset, (vm_object_size_t)(end_offset - start_offset),
            kill_pages, FALSE, /* reusable_pages */
            kill_no_write, map->pmap, pmap_offset);
      }
      vm_object_unlock(object);
      vm_map_unlock(map);
      continue;
     ...
     */
    if ((object->pager == MEMORY_OBJECT_NULL) || (object->internal) ||
        (object->private)) {
      vm_object_unlock(object);
      vm_map_unlock(map);
      continue;
    ...
    do_sync_req =
        vm_object_sync(object, start_offset, (end_offset - start_offset),
...
void vm_named_entry_associate_vm_object(vm_named_entry_t named_entry,
                                        vm_object_t object,
                                        vm_object_offset_t offset,
                                        vm_object_size_t size, vm_prot_t prot) {
  vm_map_copy_t copy;
  vm_map_entry_t copy_entry;
  ...
  DEBUG4K_MEMENTRY(
      "named_entry %p copy %p object %p offset 0x%llx size 0x%llx prot 0x%x\n",
      named_entry, copy, object, offset, size, prot);
}

vm_object_t vm_named_entry_to_vm_object(vm_named_entry_t named_entry) {
  vm_map_copy_t copy;
  vm_map_entry_t copy_entry;
  vm_object_t object;
  ...
  DEBUG4K_MEMENTRY(
      "%p -> %p -> %p [0x%llx 0x%llx 0x%llx 0x%x/0x%x ] -> %p offset 0x%llx "
      "size 0x%llx prot 0x%x\n",
      named_entry, copy, copy_entry, (uint64_t)copy_entry->vme_start,
      (uint64_t)copy_entry->vme_end, copy_entry->vme_offset,
      copy_entry->protection, copy_entry->max_protection, object,
      named_entry->offset, named_entry->size, named_entry->protection);
...
vm_map_t convert_port_entry_to_map(ipc_port_t port) {
  ...
  if ((named_entry->is_sub_map) && (named_entry->protection & VM_PROT_WRITE)) {
...
void vm_map_inspect_deallocate(vm_map_inspect_t map) {
...
void vm_map_disable_NX(vm_map_t map) {
  if (map == NULL) {
    return;
  }
  if (map->pmap == NULL) {
...
void vm_map_disallow_data_exec(vm_map_t map) {
  if (map == NULL) {
    return;
  }
 ...
 * more descriptive.
 */
void vm_map_set_32bit(vm_map_t map) {
#if defined(__arm64__)
...
void vm_map_set_64bit(vm_map_t map) {
#if defined(__arm64__)
 ...
 */
void vm_map_set_jumbo(vm_map_t map) {
 ...
 */
void vm_map_set_extra_jumbo(vm_map_t map) {
#if defined(__arm64__) && !XNU_TARGET_OS_OSX
 ...
 */
void vm_map_set_jit_entitled(vm_map_t map) {
#if defined(__arm64__)
...
void vm_map_set_platform_binary(vm_map_t map, bool is_platform_binary) {
  /* map should be locked by caller, unless still private */
  map->cs_platform_binary = is_platform_binary;
}

bool vm_map_is_platform_binary(vm_map_t map) {
  /* map should be locked by caller, unless still private */
 ...
 */
boolean_t vm_map_tpro(vm_map_t map) {
 ...
 */
void vm_map_set_tpro(vm_map_t map) {
#if defined(__arm64e__)
 ...
 */
boolean_t vm_map_tpro_enforcement(vm_map_t map) {
 ...
 */
void vm_map_set_tpro_enforcement(vm_map_t map) {
 ...
 */
boolean_t vm_map_set_tpro_range(__unused vm_map_t map,
 ...
 */
void vm_map_set_max_addr(vm_map_t map, vm_map_offset_t new_max_offset,
...
vm_map_offset_t vm_compute_max_offset(boolean_t is64) {
#if defined(__arm64__)
  return pmap_max_offset(is64, ARM_PMAP_MAX_OFFSET_DEVICE);
#else
  return is64 ? (vm_map_offset_t)MACH_VM_MAX_ADDRESS
              : (vm_map_offset_t)VM_MAX_ADDRESS;
#endif
}

void vm_map_get_max_aslr_slide_section(vm_map_t map __unused,
...
uint64_t vm_map_get_max_aslr_slide_pages(vm_map_t map) {
#if defined(__arm64__)
...
uint64_t vm_map_get_max_loader_aslr_slide_pages(vm_map_t map) {
#if defined(__arm64__)
...
boolean_t vm_map_is_64bit(vm_map_t map) {
  return map->max_offset > ((vm_map_offset_t)VM_MAX_ADDRESS);
}

boolean_t vm_map_has_hard_pagezero(vm_map_t map,
 ...
 */
kern_return_t vm_map_raise_max_offset(vm_map_t map,
 ...
 */
kern_return_t vm_map_raise_min_offset(vm_map_t map,
                                      vm_map_offset_t new_min_offset) {
  vm_map_entry_t first_entry;

  vmlp_api_start(VM_MAP_RAISE_MIN_OFFSET);

  new_min_offset = vm_map_round_page(new_min_offset, VM_MAP_PAGE_MASK(map));
...
/*
 * Set the limit on the maximum amount of address space and user wired memory
 * allowed for this map. This is basically a copy of the RLIMIT_AS and
 * RLIMIT_MEMLOCK rlimit value maintained by the BSD side of the kernel. The
 * limits are checked in the mach VM side, so we keep a copy so we don't have to
 * reach over to the BSD data structures.
 */

uint64_t vm_map_set_size_limit_count = 0;
kern_return_t vm_map_set_size_limit(vm_map_t map, uint64_t new_size_limit) {
  kern_return_t kr;
  ...
  if (new_size_limit < map->size) {
    ...
    DTRACE_VM2(vm_map_set_size_limit_fail, vm_map_size_t, map->size, uint64_t,
               new_size_limit);
    kr = KERN_FAILURE;
    ...
    /* set new limit */
    DTRACE_VM2(vm_map_set_size_limit, vm_map_size_t, map->size, uint64_t,
               new_size_limit);
...
kern_return_t vm_map_set_data_limit(vm_map_t map, uint64_t new_data_limit) {
  kern_return_t kr;
  ...
  if (new_data_limit < map->size) {
    /* new limit should not be lower than its current size */
    DTRACE_VM2(vm_map_set_data_limit_fail, vm_map_size_t, map->size, uint64_t,
               new_data_limit);
    kr = KERN_FAILURE;
    ...
    /* set new limit */
    DTRACE_VM2(vm_map_set_data_limit, vm_map_size_t, map->size, uint64_t,
               new_data_limit);
...
void vm_map_set_user_wire_limit(vm_map_t map, vm_size_t limit) {
...
void vm_map_switch_protect(vm_map_t map, boolean_t val) {
...
boolean_t vm_map_cs_enforcement(vm_map_t map) {
...
kern_return_t vm_map_cs_wx_enable(__unused vm_map_t map) {
#if CODE_SIGNING_MONITOR
...
kern_return_t vm_map_csm_allow_jit(__unused vm_map_t map) {
#if CODE_SIGNING_MONITOR
...
void vm_map_cs_debugged_set(vm_map_t map, boolean_t val) {
...
void vm_map_cs_enforcement_set(vm_map_t map, boolean_t val) {
...
/*
 * IOKit has mapped a region into this map; adjust the pmap's ledgers
 * appropriately. phys_footprint is a composite limit consisting of iokit +
 * physmem, so we need to bump both counters.
 */
void vm_map_iokit_mapped_region(vm_map_t map, vm_size_t bytes) {
...
void vm_map_iokit_unmapped_region(vm_map_t map, vm_size_t bytes) {
  pmap_t pmap = vm_map_pmap(map);
...
kern_return_t vm_map_sign(vm_map_t map, vm_map_offset_t start,
                          vm_map_offset_t end) {
  vm_map_entry_t entry;
  ...
  while (start < end) {
    ...
    m = vm_page_lookup(object, start - entry_start + entry_offset);
    if (m == VM_PAGE_NULL) {
...
kern_return_t vm_map_partial_reap(vm_map_t map,
  ...
  for (entry = vm_map_first_entry(map); entry != vm_map_to_entry(map);
       entry = next_entry) {
    next_entry = entry->vme_next;
    ...
    if (!entry->is_sub_map && VME_OBJECT(entry) &&
      ...
      *reclaimed_compressed +=
          vm_compressor_pager_get_count(VME_OBJECT(entry)->pager);

      (void)vm_map_delete(map, entry->vme_start, entry->vme_end,
                          VM_MAP_REMOVE_NO_YIELD, KMEM_GUARD_NONE, &zap_list);
    }
  }
...
int vm_map_disconnect_page_mappings(vm_map_t map, boolean_t do_unnest) {
  vm_map_entry_t entry;
  ...
  for (entry = vm_map_first_entry(map); entry != vm_map_to_entry(map);
    ...
    vmlp_range_event_entry(map, entry);
    if (!entry->is_sub_map &&
        ((VME_OBJECT(entry) == 0) || (VME_OBJECT(entry)->phys_contiguous))) {
      continue;
    }
    if (entry->is_sub_map) {
      assert(!entry->use_pmap);
  ...
  vmlp_api_end(VM_MAP_DISCONNECT_PAGE_MAPPINGS,
               (int)(byte_count / VM_MAP_PAGE_SIZE(map)));
  return (int)(byte_count / VM_MAP_PAGE_SIZE(map));
}

kern_return_t vm_map_inject_error(vm_map_t map, vm_map_offset_t vaddr) {
  ...
  result = vm_map_lookup_and_lock_object(
      &map, vaddr, VM_PROT_READ, OBJECT_LOCK_EXCLUSIVE, &version, &object,
      &offset, &prot, &wired, NULL, &real_map, NULL);
  if (object == NULL) {
    result = KERN_MEMORY_ERROR;
  } else if (object->pager) {
    result = vm_compressor_pager_inject_error(object->pager, offset);
  } else {
...
/* iterate over map entries. Call the first argument block for the number of
 * entries and the second for every entry returns: KERN_SUCCESS if iteration
 * completed ok, error code if callback returned an error KERN_FAILURE if there
 * was a race of adding/removing entries during the iteration and the number of
 * entries iterated is different from the number in the first call
 */
static kern_return_t
vm_map_entries_foreach_locked(vm_map_t map,
                              kern_return_t (^count_handler)(int nentries),
  ...
  while (entry != vm_map_to_entry(map)) {
    ...
    ++count;
    if (count > nentries) {
      /* nentries and entries iteration don't agree on how many entries there
       * are, shouldn't really happen */
      return KERN_FAILURE;
    }
  }
  if (count < nentries) {
...
kern_return_t
vm_map_entries_foreach(vm_map_t map,
                       kern_return_t (^count_handler)(int nentries),
                       kern_return_t (^entry_handler)(void *entry)) {
  vm_map_lock_read(map);
  kern_return_t error =
      vm_map_entries_foreach_locked(map, count_handler, entry_handler);
  vm_map_unlock_read(map);
  return error;
}
 ...
 * return true on success, false if there was not enough space in the give
 * buffer argument size in: bytes free in the given buffer, out: bytes written
 */
kern_return_t vm_map_dump_entry_and_compressor_pager(void *pentry, char *buf,
                                                     size_t *size) {
  size_t insize = *size;
  kern_return_t kr;
  size_t offset = 0;
  ...
  *size = offset;
  ...
  kr = vm_compressor_pager_dump(pager, buf + offset, &pager_info_size,
                                &is_compressor, &slot_mapping_count);
  if (kr != KERN_SUCCESS) {
    /* didn't have enough space for everything we want to write, caller needs to
     * retry */
    return kr;
  }
  offset += pager_info_size;
  /* if we got here, is_compressor should be true due to the object->internal
   * check above, so this assignment is just for sanity sake */
...
kern_return_t vm_map_freeze(task_t task, unsigned int *purgeable_count,
                            unsigned int *wired_count,
                            unsigned int *clean_count,
                            unsigned int *dirty_count,
                            unsigned int dirty_budget,
                            unsigned int *shared_count, int *freezer_error_code,
                            boolean_t eval_only) {
  ...
  unsigned int dirty_private_count = 0, dirty_shared_count = 0,
               obj_pages_snapshot = 0;

  vmlp_api_start(VM_MAP_FREEZE);

  *purgeable_count = *wired_count = *clean_count = *dirty_count =
      *shared_count = 0;
...
again:
  ...
  for (entry2 = vm_map_first_entry(map); entry2 != vm_map_to_entry(map);
    ...
    if (!src_object || src_object->phys_contiguous || !src_object->internal) {
      continue;
    }

    /* If eligible, scan the entry, moving eligible pages over to our parent
     * object */
    ...
    if (VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
      ...
      if ((src_object->purgable == VM_PURGABLE_EMPTY) ||
          (src_object->purgable == VM_PURGABLE_VOLATILE)) {
        /*
         * We want to purge objects that may not belong to this task but are
         * mapped in this task alone. Since we already purged this task's
         * purgeable memory at the end of a successful evaluation phase, we want
         * to avoid doing no-op calls on this task's purgeable objects. Hence
         * the check for only volatile objects.
         */
        if (evaluation_phase || src_object->purgable != VM_PURGABLE_VOLATILE ||
      ...
      if (os_ref_get_count_raw(&src_object->ref_count) > 1) {
        if (src_object != cur_shared_object) {
          obj_pages_snapshot =
              (src_object->resident_page_count - src_object->wired_page_count) +
              vm_compressor_pager_get_count(src_object->pager);
          ...
          if (os_ref_get_count_raw(&src_object->ref_count) ==
              cur_shared_obj_ref_cnt) {
            /*
    ...
    uint32_t paged_out_count =
        vm_object_compressed_freezer_pageout(src_object, dirty_budget);
  ...
  *shared_count =
      (unsigned int)((dirty_shared_count * PAGE_SIZE_64) / (1024 * 1024ULL));
  if (evaluation_phase) {
    if (memorystatus_freeze_shared_mb_per_process_max) {
      unsigned int shared_pages_threshold =
          (memorystatus_freeze_shared_mb_per_process_max * 1024 * 1024ULL) /
          PAGE_SIZE_64;
    ...
    if (dirty_shared_count &&
        ((dirty_private_count / dirty_shared_count) <
 ...
 */
boolean_t vm_map_entry_should_cow_for_true_share(vm_map_entry_t entry) {
  vm_object_t object;
  ...
  /*
   * All the criteria match: we have a large object being targeted for
   * "true_share". To limit the adverse side-effects linked with "true_share",
   * tell the caller to try and avoid setting up the entire object for
   * "true_share" by clipping the targeted range and setting it up for
   * copy-on-write.
   */
  return TRUE;
}
...
TUNABLE_WRITEABLE(boolean_t, vm_map_range_overflows_log,
                  "vm_map_range_overflows_log", FALSE);
bool vm_map_range_overflows(vm_map_t map, vm_map_offset_t addr,
                            vm_map_size_t size) {
  ...
  if (__improbable(os_add_overflow(addr, size, &sum) || end <= start)) {
    ...
    if (vm_map_range_overflows_log) {
      printf("%d[%s] vm_map_range_overflows addr 0x%llx size 0x%llx pgmask "
             "0x%llx\n",
             proc_selfpid(), proc_best_name(current_proc()), (uint64_t)addr,
             (uint64_t)size, (uint64_t)pgmask);
    }
    DTRACE_VM4(vm_map_range_overflows, vm_map_t, map, uint32_t, pgmask,
               uint64_t, (uint64_t)addr, uint64_t, (uint64_t)size);
    return true;
  }
  return false;
}

vm_map_offset_t vm_map_round_page_mask(vm_map_offset_t offset,
                                       vm_map_offset_t mask) {
  return VM_MAP_ROUND_PAGE(offset, mask);
}

vm_map_offset_t vm_map_trunc_page_mask(vm_map_offset_t offset,
                                       vm_map_offset_t mask) {
  return VM_MAP_TRUNC_PAGE(offset, mask);
}

boolean_t vm_map_page_aligned(vm_map_offset_t offset, vm_map_offset_t mask) {
  return ((offset)&mask) == 0;
}

int vm_map_page_shift(vm_map_t map) { return VM_MAP_PAGE_SHIFT(map); }

int vm_map_page_size(vm_map_t map) { return VM_MAP_PAGE_SIZE(map); }

vm_map_offset_t vm_map_page_mask(vm_map_t map) { return VM_MAP_PAGE_MASK(map); }

kern_return_t vm_map_set_page_shift(vm_map_t map, int pageshift) {
...
vm_map_query_volatile(vm_map_t map, mach_vm_size_t *volatile_virtual_size_p,
  ...
  for (entry = vm_map_first_entry(map); entry != vm_map_to_entry(map);
    ...
    if (object->pager) {
      volatile_compressed_count += vm_compressor_pager_get_count(object->pager);
    }
    pmap_compressed_bytes = 0;
    pmap_resident_bytes = pmap_query_resident(
        map->pmap, entry->vme_start, entry->vme_end, &pmap_compressed_bytes);
    volatile_pmap_count += (pmap_resident_bytes / PAGE_SIZE);
    volatile_compressed_pmap_count += (pmap_compressed_bytes / PAGE_SIZE);
  }
...
void vm_map_sizes(vm_map_t map, vm_map_size_t *psize, vm_map_size_t *pfree,
...
int vm_map_shadow_max(vm_map_t map) {
  ...
  for (entry = vm_map_first_entry(map); entry != vm_map_to_entry(map);
    ...
    vm_object_lock_shared(object);
    for (shadows = 0; object->shadow != NULL; shadows++, object = next_object) {
...
#if __x86_64__
void vm_map_set_high_start(vm_map_t map, vm_map_offset_t high_start) {
...
kern_return_t vm_map_entry_cs_associate(vm_map_t map, vm_map_entry_t entry,
                                        vm_map_kernel_flags_t vmk_flags) {
  ...
  /*
   * Check for a debug association mapping before we check for used_for_jit.
   * This allows non-RWX JIT on macOS systems to masquerade their mappings as
   * USER_DEBUG pages instead of USER_JIT. These non-RWX JIT pages cannot be
   * marked as USER_JIT since they are mapped with RW or RX permissions, which
   * the page table monitor denies on USER_JIT pages. Given that, if they're not
   * mapped as USER_DEBUG, they will be mapped as USER_EXEC, and that will cause
   * another page table monitor violation when those USER_EXEC pages are mapped
   * as RW.
   *
   * Since these pages switch between RW and RX through mprotect, they mimic
   * what we expect a debugger to do. As the code signing monitor does not
   * enforce mappings on macOS systems, this works in our favor here and allows
   * us to continue to support these legacy-programmed applications without
   * sacrificing security on the page table or the code signing monitor. We
   * don't need to explicitly check for entry_for_jit here and the mapping
   * permissions. If the initial mapping is created with RX, then the
   * application must map it as RW in order to first write to the page (MAP_JIT
   * mappings must be private and anonymous). The switch to RX will cause
   * vm_map_protect to mark the entry as vmkf_remap_prot_copy. Similarly, if the
   * mapping was created as RW, and then switched to RX, vm_map_protect will
   * again mark the entry as a copy, and both these cases lead to this
   * if-statement being entered.
   *
   * For more information: rdar://115313336.
   */
  if (vmk_flags.vmkf_remap_prot_copy) {
    cs_ret = csm_associate_debug_region(map->pmap, entry->vme_start,
                                        entry->vme_end - entry->vme_start);
     ...
     * the end of the function, and if it is not supported, then we still want
     * the VM to perform code-signing enforcement on this entry. That said, if
     * we don't mark this as a xnu_user_debug page when the code-signing monitor
     * is disabled, then it never gets retyped to XNU_USER_DEBUG frame type,
     * which then causes an issue with debugging (since it'll be mapped in as
     * XNU_USER_EXEC in some cases, which will cause a violation when attempted
     * to be mapped as writable).
     */
    ...
    if (vm_log_xnu_user_debug) {
      printf("FBDP %d[%s] %s:%d map %p entry %p [ 0x%llx 0x%llx ]  "
             "vme_xnu_user_debug=%d cs_ret %d\n",
             proc_selfpid(),
             (get_bsdtask_info(current_task())
                  ? proc_name_address(get_bsdtask_info(current_task()))
                  : "?"),
             __FUNCTION__, __LINE__, map, entry, (uint64_t)entry->vme_start,
             (uint64_t)entry->vme_end, entry->vme_xnu_user_debug, cs_ret);
    }
#endif /* DEVELOPMENT || DEBUG */
    goto done;
  }
  ...
  if (entry->used_for_jit) {
    cs_ret = csm_associate_jit_region(map->pmap, entry->vme_start,
                                      entry->vme_end - entry->vme_start);
    goto done;
  }
  ...
  for (;;) {
     ...
     */
    if (!memory_object_backing_object(cs_object->pager, cs_offset,
                                      &backing_object, &backing_offset)) {
  ...
  if (cs_vnode == NULL) {
    ...
    cs_ret = vnode_pager_get_cs_blobs(cs_vnode, &cs_blobs);
    assert(cs_ret == KERN_SUCCESS);
    cs_ret = cs_associate_blob_with_mapping(map->pmap, entry->vme_start,
                                            (entry->vme_end - entry->vme_start),
                                            cs_offset, cs_blobs);
  }
  vm_object_unlock(cs_object);
  ...
  if (cs_ret == KERN_SUCCESS) {
    DTRACE_VM2(vm_map_entry_cs_associate_success, vm_map_offset_t,
               entry->vme_start, vm_map_offset_t, entry->vme_end);
     ...
     * doing it.
     */
    DTRACE_VM3(vm_map_entry_cs_associate_off, vm_map_offset_t, entry->vme_start,
               vm_map_offset_t, entry->vme_end, int, cs_ret);
  } else {
    /*
     * A real error: do not allow
     * execution in this mapping.
     */
    DTRACE_VM3(vm_map_entry_cs_associate_failure, vm_map_offset_t,
               entry->vme_start, vm_map_offset_t, entry->vme_end, int, cs_ret);
    if (vmk_flags.vmkf_overwrite_immutable) {
      /*
...
inline bool vm_map_is_corpse_source(vm_map_t map) {
  bool status = false;
...
inline void vm_map_set_corpse_source(vm_map_t map) {
...
inline void vm_map_unset_corpse_source(vm_map_t map) {
...
struct vm_map_corpse_footprint_region {
  ...
  cf_disp_t cfr_disposition[] __counted_by(
      cfr_num_pages); /* disposition of each page */
} __attribute__((packed));

static cf_disp_t vm_page_disposition_to_cf_disp(int disposition) {
...
static int vm_page_cf_disp_to_disposition(cf_disp_t cf_disp) {
  int disposition;
...
static kmem_guard_t vm_map_corpse_footprint_guard(vm_map_t map) {
  return (kmem_guard_t){
      .kmg_atomic = true,
...
vm_map_corpse_footprint_new_region(
  ...
  footprint_edge = ((uintptr_t)footprint_header + footprint_header->cf_size);
  footprint_region =
      ((struct vm_map_corpse_footprint_region
            *)((char *)footprint_header + footprint_header->cf_last_region));
  ...
  if (((uintptr_t)footprint_header + new_region_offset +
       sizeof(*footprint_region)) >= footprint_edge) {
...
TUNABLE(vm_size_t, vm_map_corpse_footprint_max_buffer_size,
        "vm_cf_max_buf_size",
#if XNU_TARGET_OS_OSX
        MiB(8));
#else  /* !XNU_TARGET_OS_OSX */
...
kern_return_t vm_map_corpse_footprint_collect(vm_map_t old_map,
  ...
  if (!new_map->has_corpse_footprint || old_map->has_corpse_footprint) {
    /*
  ...
  if (new_map->vmmap_corpse_footprint == NULL) {
    ...
    footprint_header->cf_last_region = sizeof(*footprint_header);
    footprint_header->cf_last_zeroes = 0;
  ...
  if ((footprint_region->cfr_vaddr +
       (((vm_map_offset_t)footprint_region->cfr_num_pages) *
        effective_page_size)) != old_entry->vme_start) {
    ...
    num_pages_delta = ((old_entry->vme_start - footprint_region->cfr_vaddr) /
                       effective_page_size) -
    ...
    region_offset_delta_size =
        (sizeof(*footprint_region) +
         ...
         ((footprint_region->cfr_num_pages - footprint_header->cf_last_zeroes) *
          sizeof(cf_disp_t)));
    DTRACE_VM5(corpse_footprint_collect, uint32_t,
               footprint_header->cf_last_region, vm_map_offset_t,
               footprint_region->cfr_vaddr, uint32_t,
               footprint_region->cfr_num_pages, vm_map_offset_t,
               old_entry->vme_start, uin64_t, num_pages_delta);
    if (region_offset_delta_size < num_pages_delta_size ||
        os_add3_overflow(footprint_region->cfr_num_pages,
                         (uint32_t)num_pages_delta, 1, &num_pages_tmp)) {
      /*
       ...
       */
      DTRACE_VM3(corpse_footprint_collect_new_region, vm_map_offset_t,
                 old_entry->vme_start, vm_map_offset_t,
                 footprint_region->cfr_vaddr, uint64_t, num_pages_delta);
      new_footprint_region =
       ...
       * pages.
       */
      DTRACE_VM3(corpse_footprint_collect_zero_gap, vm_map_offset_t,
                 old_entry->vme_start, vm_map_offset_t,
                 footprint_region->cfr_vaddr, uint64_t, num_pages_delta);
  ...
  for (va = old_entry->vme_start; va < old_entry->vme_end;
       va += effective_page_size) {
    int disposition;
    cf_disp_t cf_disp;

    vm_map_footprint_query_page_info_exclusive(old_map, old_entry, va,
                                               &disposition);
    cf_disp = vm_page_disposition_to_cf_disp(disposition);

    DTRACE_VM2(corpse_footprint_collect_page_info, vm_map_offset_t, va,
               cf_disp_t, cf_disp);
    ...
    if (os_add_overflow(footprint_region->cfr_num_pages, 1, &num_pages_tmp)) {
    ...
    next_disp_p =
        (cf_disp_t *)((uintptr_t)footprint_region + sizeof(*footprint_region));
    next_disp_p += footprint_region->cfr_num_pages;
    /* check that we're not going over the edge */
    if ((uintptr_t)next_disp_p >= footprint_edge) {
      goto over_the_edge;
    ...
    *next_disp_p = cf_disp;
    ...
    if ((footprint_header->cf_last_zeroes +
         roundup(((footprint_region->cfr_num_pages -
                  ...
                  sizeof(cf_disp_t)) &
                     (sizeof(int) - 1),
                 sizeof(int))) < (sizeof(*footprint_header))) {
      /*
     ...
     * "zero" dispositions.
     */
    new_footprint_region = vm_map_corpse_footprint_new_region(footprint_header);
    if (new_footprint_region == NULL) {
      goto over_the_edge;
    }
    footprint_region = new_footprint_region;
...
void vm_map_corpse_footprint_collect_done(vm_map_t new_map) {
  ...
  footprint_header =
      (struct vm_map_corpse_footprint_header *)new_map->vmmap_corpse_footprint;
  ...
  actual_size =
      (vm_size_t)(footprint_header->cf_last_region + sizeof(*footprint_region) +
                  (footprint_region->cfr_num_pages * sizeof(cf_disp_t)));

  //	printf("FBDP map %p buf_size 0x%llx actual_size 0x%llx\n", new_map,
  //(uint64_t) buf_size, (uint64_t) actual_size);
  vm_map_corpse_footprint_size_avg =
      (((vm_map_corpse_footprint_size_avg * vm_map_corpse_footprint_count) +
        actual_size) /
  ...
  if (buf_size > actual_size) {
     ...
     */
    kmem_guard_t guard = vm_map_corpse_footprint_guard(new_map);
    kmem_return_t kmr = kmem_realloc_guard(
        kernel_map, (vm_offset_t)footprint_header,
        /* Account for guard page */
        buf_size + PAGE_SIZE, actual_size + PAGE_SIZE,
        KMR_DATA | KMR_GUARD_LAST | KMR_FREEOLD | KMR_KOBJECT, guard);
    assertf(kmr.kmr_return == KERN_SUCCESS,
            "trim: footprint_header %p buf_size 0x%llx actual_size 0x%llx "
            "kr=0x%x\n",
            footprint_header, (uint64_t)buf_size, (uint64_t)actual_size,
            kmr.kmr_return);
    footprint_header = (struct vm_map_corpse_footprint_header *)kmr.kmr_ptr;
    assert3p(footprint_header, !=, NULL);
    new_map->vmmap_corpse_footprint = footprint_header;
 ...
 *
 * This is the equivalent of vm_map_footprint_query_page_info() for a forked
 * corpse.
 */
kern_return_t vm_map_corpse_footprint_query_page_info(vm_map_t map,
                                                      vm_map_offset_t va,
  ...
  if (footprint_header == NULL) {
    *disposition_p = 0;
    //		if (va < SHARED_REGION_BASE_ARM64) printf("FBDP %d query map %p
...
lookup_again:
  ...
  footprint_region =
      (struct vm_map_corpse_footprint_region *)((char *)footprint_header +
                                                footprint_region_offset);
  region_start = footprint_region->cfr_vaddr;
  region_end =
      (region_start + ((vm_map_offset_t)(footprint_region->cfr_num_pages) *
                       effective_page_size));
  if (va < region_start &&
      footprint_region_offset != sizeof(*footprint_header)) {
  ...
  while (va >= region_end) {
    ...
    footprint_region_offset = roundup(footprint_region_offset, sizeof(int));
    footprint_region =
        (struct vm_map_corpse_footprint_region *)((char *)footprint_header +
                                                  footprint_region_offset);
    region_start = footprint_region->cfr_vaddr;
  ...
  *disposition_p = vm_page_cf_disp_to_disposition(cf_disp);
  ...
  //	if (va < SHARED_REGION_BASE_ARM64) printf("FBDP %d query map %p va
  //0x%llx disp 0x%x\n", __LINE__, map, va, *disposition_p);
  /* dtrace -n 'vminfo:::footprint_query_page_info { printf("map 0x%p va 0x%llx
   * disp 0x%x kr 0x%x", arg0, arg1, arg2, arg3); }' */
  DTRACE_VM4(footprint_query_page_info, vm_map_t, map, vm_map_offset_t, va, int,
             *disposition_p, kern_return_t, kr);

  return kr;
}

void vm_map_corpse_footprint_destroy(vm_map_t map) {
  if (map->has_corpse_footprint && map->vmmap_corpse_footprint != NULL) {
    struct vm_map_corpse_footprint_header *footprint_header;
    vm_size_t buf_size;
    ...
    kmem_free_guard(kernel_map, (vm_offset_t)footprint_header,
                    buf_size + PAGE_SIZE, KMF_GUARD_LAST, guard);
 ...
 */
void vm_map_copy_footprint_ledgers(task_t old_task, task_t new_task) {
  ...
  vm_map_copy_ledger(old_task, new_task,
  ...
  vm_map_copy_ledger(old_task, new_task,
                     task_ledgers.tagged_footprint_compressed);
 ...
 */
void vm_map_copy_ledger(task_t old_task, task_t new_task, int ledger_entry) {
  ...
  ledger_disable_panic_on_negative(new_task->ledger, ledger_entry);

  /* adjust "new_task" to match "old_task" */
  ledger_get_balance(old_task->ledger, ledger_entry, &old_balance);
  ledger_get_balance(new_task->ledger, ledger_entry, &new_balance);
 ...
 */
pmap_t vm_map_get_pmap(vm_map_t map) { return vm_map_pmap(map); }

ppnum_t vm_map_get_phys_page(vm_map_t map, vm_offset_t addr) {
  vm_object_offset_t offset;
  vm_object_t object;
  ...
  while (vm_map_lookup_entry(map, map_offset, &entry)) {
    if (entry->is_sub_map) {
      ...
      map_offset = (VME_OFFSET(entry) + (map_offset - entry->vme_start));
      vm_map_unlock(old_map);
      continue;
    }
    ...
    if (VME_OBJECT(entry)->phys_contiguous) {
      ...
      if (VME_OBJECT(entry)->vo_shadow_offset == 0) {
        ...
        vm_map_unlock(map);
        vm_fault(map, map_offset, VM_PROT_NONE, FALSE /* change_wiring */,
                 VM_KERN_MEMORY_NONE, THREAD_UNINT, NULL, 0);
        vm_map_lock(map);
        continue;
      }
      offset = (VME_OFFSET(entry) + (map_offset - entry->vme_start));
      phys_page = (ppnum_t)((VME_OBJECT(entry)->vo_shadow_offset + offset) >>
                            PAGE_SHIFT);
      break;
    }
    offset = (VME_OFFSET(entry) + (map_offset - entry->vme_start));
...
vm_map_range_random_uniform(vm_map_size_t req_size, vm_map_offset_t min_addr,
                            vm_map_offset_t max_addr, vm_map_offset_t offmask) {
...
static vm_map_offset_t vm_map_range_offmask(void) {
  uint32_t pte_depth;
 ...
 */
kern_return_t vm_map_range_configure(vm_map_t map,
                                     __unused bool needs_extra_jumbo_va) {
...
#endif
  data_range = vm_map_range_random_uniform(1ull << 40, default_end,
                                           0x7full << 40, offmask);
  ...
  data_range = vm_map_range_random_uniform(GiB(10), default_end + PAGE_SIZE,
                                           vm_map_max(map), offmask);
  ...
  if (default_end != data_range.min_address) {
    kr = vm_map_enter(map, &default_end, data_range.min_address - default_end,
  ...
  if (data_range.max_address != vm_map_max(map)) {
    ...
    kr = vm_map_enter(map, &data_range.max_address, size, 0,
                      VM_MAP_KERNEL_FLAGS_FIXED_PERMANENT(), VM_OBJECT_NULL, 0,
  ...
  if (needs_extra_jumbo_va) {
    map->large_file_range = vm_map_range_random_uniform(
        TiB(1), MACH_VM_JUMBO_ADDRESS, MACH_VM_MAX_ADDRESS, offmask);
  }
#endif /* XNU_TARGET_OS_IOS && EXTENDED_USER_VA_SUPPORT */
 ...
 *  of a VM map fork.
 */
void vm_map_range_fork(vm_map_t new_map, vm_map_t old_map) {
  ...
  if (old_map->extra_ranges_count) {
    ...
    memcpy(ntable, otable, count * sizeof(struct vm_map_user_range));
 ...
 */
kern_return_t vm_map_get_user_range(vm_map_t map, vm_map_range_id_t range_id,
                                    mach_vm_range_t range) {
...
void vm_map_kernel_flags_update_range_id(vm_map_kernel_flags_t *vmkf,
                                         vm_map_t map,
 ...
 * Check if the vm map entry specified by the virtual address has a device
 * pager. If the vm map entry does not exist or if the map is NULL, this returns
 * FALSE.
 */
boolean_t vm_map_entry_has_device_pager(vm_map_t map, vm_map_offset_t vaddr) {
  vm_map_entry_t entry;
  vm_object_t object;
  boolean_t result;
...
bool vm_map_is_sealed(vm_map_t map) {
  return map->vmmap_sealed == VM_MAP_SEALED;
}

void vm_map_seal(vm_map_t map, bool nested_pmap) {
  vm_map_entry_t entry;
  ...
  for (entry = vm_map_first_entry(map); entry != vm_map_to_entry(map);
    ...
    if (VME_OBJECT(entry) == VM_OBJECT_NULL) {
      object =
          vm_object_allocate(entry->vme_end - entry->vme_start, map->serial_id);
 ...
 */
static void vm_map_testing_require_submap_at_address(
    vm_map_t parent_map, mach_vm_address_t address, vm_map_t *const out_submap,
 ...
 */
static void vm_map_testing_remap_submap_and_unlock(
    vm_map_t parent_map, vm_map_t submap, mach_vm_address_t start,
    mach_vm_address_t end, mach_vm_address_t submap_offset) {
  kern_return_t kr;
  ...
  assertf(vm_map_is_sealed(submap), "submap test: submap must be sealed");
  ...
  assertf(start < end, "submap test: start must precede end");
  ...
  /* Map the submap. */
  new_start = start;
  kr = vm_map_enter(parent_map, &new_start, end - start, 0, vmk_flags,
                    (vm_object_t)(uintptr_t)submap, submap_offset,
                    true /* copy */, cur_prot, max_prot,
                    VM_INHERIT_SHARE /* same as vm_shared_region.c */);
 ...
 * For testing purposes only.
 */
void vm_map_testing_remap_submap(vm_map_t parent_map,
                                 mach_vm_address_t submap_base_address,
                                 mach_vm_address_t start, mach_vm_address_t end,
  ...
  vm_map_testing_require_submap_at_address(parent_map, submap_base_address,
                                           &submap, NULL);

  /* Map the submap as requested. */
  vm_map_testing_remap_submap_and_unlock(parent_map, submap, start, end,
                                         offset);
}

/*
 * Create a new constant/sealed submap. Map it into parent_map at [start, end).
 * The new submap's contents are the map entries initially in the range [start,
 * end). The new submap does not use a nested pmap. On entry: no existing map
 * entry may cross start or end
 *
 * For testing purposes only.
 */
void vm_map_testing_make_sealed_submap(vm_map_t parent_map,
   ...
   */
  submap = vm_map_create_options(NULL /* pmap */, 0, end - start,
                                 VM_MAP_CREATE_PAGEABLE);
  assert(submap);
  ...
  for (vm_map_lookup_entry_or_next(parent_map, start, &entry);
    ...
    assertf((entry->max_protection & VM_PROT_ALLEXEC) == 0,
            "submap test: executable memory is unimplemented");
#if __arm64e__
    assertf(!entry->used_for_tpro, "submap test: TPRO memory is unimplemented");
#endif
    ...
    vm_map_store_entry_link(submap, vm_map_last_entry(submap), entry,
                            VM_MAP_KERNEL_FLAGS_NONE);
  ...
  vm_map_testing_remap_submap_and_unlock(
      parent_map, submap, start, end, /* parent map range */
...
void vm_map_pmap_check_ledgers(pmap_t pmap, ledger_t ledger, int pid,
                               char *procname) {
  ledger_amount_t bal;
  boolean_t do_panic;
...
#define LEDGER_CHECK_BALANCE(__LEDGER)                                         \
  ...
  ledger_get_balance(ledger, task_ledgers.__LEDGER, &bal);                     \
  ledger_get_panic_on_negative(ledger, task_ledgers.__LEDGER,                  \
                               &panic_on_negative);                            \
  if (bal != 0) {                                                              \
    if (panic_on_negative ||                                                   \
        (pmap_ledgers_panic && pmap_ledgers_panic_leeway > 0 &&                \
  ...
  if (do_panic) {
    if (pmap_ledgers_panic) {
      panic("pmap_destroy(%p) %d[%s] has imbalanced ledgers", pmap, pid,
            procname);
    } else {
...
void vm_map_pmap_set_process(vm_map_t map, int pid, char *procname) {
...
vm_size_t vm_map_kernel_max_simple_mappable_size(void) {
#ifdef __arm64__
 ...
 */
__attribute__((always_inline)) bool
vm_map_is_map_size_valid(vm_map_t target_map, vm_size_t size,
                         bool no_soft_limit) {
#ifdef __x86_64__
