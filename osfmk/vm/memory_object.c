...
#define memory_object_should_return_page(m, should_return)                     \
  (should_return != MEMORY_OBJECT_RETURN_NONE &&                               \
   (((m)->vmp_dirty ||                                                         \
     ((m)->vmp_dirty = pmap_is_modified(VM_PAGE_GET_PHYS_PAGE(m)))) ||         \
...
memory_object_lock_page(vm_page_t m, memory_object_return_t should_return,
                        boolean_t should_flush, vm_prot_t prot);
...
memory_object_lock_page(vm_page_t m, memory_object_return_t should_return,
   ...
   *
   *   if (vm_page_deactivate_hint && (should_return !=
   * MEMORY_OBJECT_RETURN_NONE)) return
   * (MEMORY_OBJECT_LOCK_RESULT_MUST_DEACTIVATE);
   */
...
kern_return_t memory_object_lock_request(memory_object_control_t control,
  ...
  if ((prot & ~(VM_PROT_ALL | VM_PROT_ALLEXEC)) != 0 &&
      prot != VM_PROT_NO_CHANGE) {
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (flags & MEMORY_OBJECT_DATA_FLUSH_ALL) {
    if ((should_return != MEMORY_OBJECT_RETURN_NONE) || offset ||
        object->vo_copy) {
  ...
  if (flags & MEMORY_OBJECT_DATA_FLUSH_ALL) {
    ...
    (void)vm_object_update(object, offset, size, resid_offset, io_errno,
                           should_return, flags, prot);
  }
 ...
 *		to the vm_object.
 */
kern_return_t memory_object_destroy(memory_object_control_t control,
...
boolean_t vm_object_sync(vm_object_t object, vm_object_offset_t offset,
...
#define LIST_REQ_PAGEOUT_PAGES(object, data_cnt, po, ro, ioerr, iosync)        \
  ...
  if ((pager = (object)->pager) != MEMORY_OBJECT_NULL) {                       \
    ...
    if (iosync)                                                                \
      ...
      upl_flags = UPL_MSYNC;                                                   \
                                                                               \
    (void)memory_object_data_return(pager, po,                                 \
                                    (memory_object_cluster_size_t)data_cnt,    \
                                    ro, ioerr, FALSE, FALSE, upl_flags);       \
                                                                               \
    vm_object_lock(object);                                                    \
    vm_object_paging_end(object);                                              \
  }                                                                            \
...
static int vm_object_update_extent(vm_object_t object,
                                   ...
                                   int *io_errno, boolean_t should_flush,
                                   memory_object_return_t should_return,
                                   boolean_t should_iosync, vm_prot_t prot) {
  vm_page_t m;
  int retval = 0;
  ...
  for (; offset < offset_end && object->resident_page_count;
       offset += PAGE_SIZE_64) {
    /*
    ...
    if (data_cnt) {
      if ((data_cnt >= MAX_UPL_TRANSFER_BYTES) || (next_offset != offset)) {
        ...
        LIST_REQ_PAGEOUT_PAGES(object, data_cnt, paging_offset, offset_resid,
                               io_errno, should_iosync);
        data_cnt = 0;
      }
    }
    while ((m = vm_page_lookup(object, offset)) != VM_PAGE_NULL) {
      ...
      page_lock_result =
          memory_object_lock_page(m, should_return, should_flush, prot);

      if (data_cnt &&
          page_lock_result != MEMORY_OBJECT_LOCK_RESULT_MUST_RETURN) {
        /*
 ...
 */
kern_return_t vm_object_update(vm_object_t object, vm_object_offset_t offset,
                               vm_object_size_t size,
                               vm_object_offset_t *resid_offset, int *io_errno,
                               memory_object_return_t should_return, int flags,
...
#define RESIDENT_LIMIT (1024 * 32)
  ...
  if (update_cow ||
      (flags & (MEMORY_OBJECT_DATA_PURGE | MEMORY_OBJECT_DATA_SYNC))) {
    int collisions = 0;
  ...
  if ((copy_object != VM_OBJECT_NULL && update_cow) ||
    ...
    if (copy_object != VM_OBJECT_NULL) {
      ...
      copy_offset = (offset >= copy_object->vo_shadow_offset)
                        ? (offset - copy_object->vo_shadow_offset)
                        : 0;
    ...
    RETRY_COW_OF_LOCK_REQUEST:
      ...
      page = VM_PAGE_NULL;
      result = vm_fault_page(copy_object, i, VM_PROT_WRITE | VM_PROT_READ,
                             FALSE, FALSE, /* page not looked up */
                             &prot, &page, &top_page, (int *)0, &error, FALSE,
                             &fault_info);

      switch (result) {
      case VM_FAULT_SUCCESS:
        if (top_page) {
          vm_fault_cleanup(VM_PAGE_OBJECT(page), top_page);
...
BYPASS_COW_COPYIN:
  ...
  if ((object->resident_page_count < RESIDENT_LIMIT) &&
      (atop_64(size) >
       (unsigned)(object->resident_page_count / (8 * MAX_EXTENTS)))) {
    vm_page_t next;
  ...
  for (n = 0; n < num_of_extents; n++) {
    if (vm_object_update_extent(object, extents[n].e_min, extents[n].e_max,
                                resid_offset, io_errno, should_flush,
                                should_return, should_iosync, protection)) {
      data_returned = TRUE;
    }
  }
...
vm_object_set_attributes_common(vm_object_t object, boolean_t may_cache,
 ...
 */
kern_return_t memory_object_change_attributes(memory_object_control_t control,
   ...
   */
  return vm_object_set_attributes_common(object, may_cache, copy_strategy);
}

kern_return_t memory_object_iopl_request(
    ipc_port_t port, memory_object_offset_t offset, upl_size_t *upl_size,
    upl_t *upl_ptr, upl_page_info_array_t user_page_list,
    unsigned int *page_list_count, upl_control_flags_t *flags, vm_tag_t tag) {
  vm_object_t object;
  kern_return_t ret;
  ...
  if (named_entry != NULL) {
    ...
    if (caller_flags & UPL_COPYOUT_FROM) {
      if ((named_entry->protection & VM_PROT_READ) != VM_PROT_READ) {
        return KERN_INVALID_RIGHT;
      }
    } else {
      if ((named_entry->protection & (VM_PROT_READ | VM_PROT_WRITE)) !=
    ...
    if (named_entry->is_sub_map || named_entry->is_copy) {
      return KERN_INVALID_ARGUMENT;
    }
  ...
  ret =
      vm_object_iopl_request(object, offset, *upl_size, upl_ptr, user_page_list,
                             page_list_count, caller_flags, tag);
  vm_object_deallocate(object);
  return ret;
}
...
kern_return_t memory_object_upl_request(memory_object_control_t control,
                                        memory_object_offset_t offset,
                                        upl_size_t size, upl_t *upl_ptr,
                                        upl_page_info_array_t user_page_list,
                                        unsigned int *page_list_count,
                                        int cntrl_flags, int tag) {
  vm_object_t object;
  ...
  return vm_object_upl_request(
      object, offset, size, upl_ptr, user_page_list, page_list_count,
      (upl_control_flags_t)(unsigned int)cntrl_flags, vmtag);
}
...
memory_object_cluster_size(memory_object_control_t control,
  ...
  *start -= object->paging_offset;
  ...
  vm_object_cluster_size(object, (vm_object_offset_t *)start, length,
                         fault_info, io_streaming);
...
kern_return_t host_default_memory_manager(
    host_priv_t host_priv, memory_object_default_t *default_manager,
...
memory_manager_default_reference(void) {
  ...
  while (current_manager == MEMORY_OBJECT_DEFAULT_NULL) {
    ...
    res = lck_mtx_sleep(&memory_manager_default_lock, LCK_SLEEP_DEFAULT,
                        (event_t)&memory_manager_default, THREAD_UNINT);
...
kern_return_t memory_object_page_op(memory_object_control_t control,
                                    memory_object_offset_t offset, int ops,
...
kern_return_t memory_object_range_op(memory_object_control_t control,
                                     memory_object_offset_t offset_beg,
                                     memory_object_offset_t offset_end, int ops,
                                     int *range) {
  vm_object_t object;
  ...
  return vm_object_range_op(object, offset_beg, offset_end, ops,
                            (uint32_t *)range);
}

void memory_object_mark_used(memory_object_control_t control) {
  vm_object_t object;
...
void memory_object_mark_unused(memory_object_control_t control,
...
void memory_object_mark_io_tracking(memory_object_control_t control) {
  vm_object_t object;
...
void memory_object_mark_trusted(memory_object_control_t control) {
  vm_object_t object;
...
kern_return_t memory_object_mark_as_tracked(memory_object_control_t control,
...
void memory_object_mark_eligible_for_secluded(memory_object_control_t control,
  ...
  vm_object_lock(object);
  if (eligible_for_secluded && secluded_for_filecache && /* global boot-arg */
  ...
  } else if (!eligible_for_secluded && object->eligible_for_secluded) {
...
void memory_object_mark_for_realtime(memory_object_control_t control,
                                     bool for_realtime) {
  vm_object_t object;
...
kern_return_t memory_object_pages_resident(memory_object_control_t control,
...
kern_return_t memory_object_signed(memory_object_control_t control,
                                   boolean_t is_signed) {
  vm_object_t object;
...
boolean_t memory_object_is_signed(memory_object_control_t control) {
  boolean_t is_signed;
  vm_object_t object;
...
boolean_t memory_object_is_shared_cache(memory_object_control_t control) {
...
void memory_object_control_reference(__unused memory_object_control_t control) {
  return;
}
 ...
 */
void memory_object_control_deallocate(
    __unused memory_object_control_t control) {}

void memory_object_control_disable(memory_object_control_t *control) {
  assert(*control != VM_OBJECT_NULL);
  *control = VM_OBJECT_NULL;
}

memory_object_t convert_port_to_memory_object(__unused mach_port_t port) {
  return MEMORY_OBJECT_NULL;
}

mach_port_t convert_memory_object_to_port(__unused memory_object_t object) {
  return MACH_PORT_NULL;
}

/* Routine memory_object_reference */
void memory_object_reference(memory_object_t memory_object) {
  (memory_object->mo_pager_ops->memory_object_reference)(memory_object);
}

/* Routine memory_object_deallocate */
void memory_object_deallocate(memory_object_t memory_object) {
  (memory_object->mo_pager_ops->memory_object_deallocate)(memory_object);
}
...
kern_return_t memory_object_terminate(memory_object_t memory_object) {
  return (memory_object->mo_pager_ops->memory_object_terminate)(memory_object);
}
...
kern_return_t memory_object_data_request(
  ...
  return (memory_object->mo_pager_ops->memory_object_data_request)(
      memory_object, offset, length, desired_access, fault_info);
}
...
kern_return_t memory_object_data_return(memory_object_t memory_object,
                                        ...
                                        memory_object_offset_t *resid_offset,
                                        int *io_error, boolean_t dirty,
                                        boolean_t kernel_copy, int upl_flags) {
  return (memory_object->mo_pager_ops->memory_object_data_return)(
      memory_object, offset, size, resid_offset, io_error, dirty, kernel_copy,
      upl_flags);
}
...
kern_return_t memory_object_map(memory_object_t memory_object, vm_prot_t prot) {
  return (memory_object->mo_pager_ops->memory_object_map)(memory_object, prot);
}

/* Routine memory_object_last_unmap */
kern_return_t memory_object_last_unmap(memory_object_t memory_object) {
  return (memory_object->mo_pager_ops->memory_object_last_unmap)(memory_object);
}

boolean_t memory_object_backing_object(memory_object_t memory_object,
                                       memory_object_offset_t offset,
  ...
  return (memory_object->mo_pager_ops->memory_object_backing_object)(
      memory_object, offset, backing_object, backing_offset);
}

upl_t convert_port_to_upl(__unused ipc_port_t port) { return NULL; }

mach_port_t convert_upl_to_port(__unused upl_t upl) { return MACH_PORT_NULL; }
