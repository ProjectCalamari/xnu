...
int get_map_nentries(vm_map_t map) { return map->hdr.nentries; }
...
kern_return_t vnode_pager_cluster_read(/* forward */
                                       vnode_pager_t, vm_object_offset_t,
                                       vm_object_offset_t, uint32_t, vm_size_t);

void vnode_pager_cluster_write(/* forward */
                               vnode_pager_t, vm_object_offset_t, vm_size_t,
                               vm_object_offset_t *, int *, int);

vnode_pager_t vnode_object_create(/* forward */
                                  struct vnode *);

vnode_pager_t vnode_pager_lookup(/* forward */
                                 memory_object_t);

struct vnode *vnode_pager_lookup_vnode(/* forward */
                                       memory_object_t);

ZONE_DEFINE_TYPE(vnode_pager_zone, "vnode pager structures", struct vnode_pager,
                 ZC_NOENCRYPT);
...
int memory_object_control_uiomove(memory_object_control_t control,
                                  memory_object_offset_t offset, void *uio,
                                  int start_offset, int io_requested,
                                  int mark_dirty, int take_reference) {
  vm_object_t object;
  vm_page_t dst_page;
  int xsize;
  int retval = 0;
  ...
  int dirty_count; /* keeps track of number of pages dirtied as part of this
                      uiomove */
  ...
  while (io_requested && retval == 0) {
    ...
    for (cur_run = 0; cur_run < cur_needed;) {
      ...
      if (__improbable(dst_page->vmp_absent)) {
        printf("absent page %p (obj %p offset 0x%llx) -> EIO", dst_page, object,
               offset);
        retval = EIO;
        break;
      }
      ...
      if (mark_dirty) {
        ...
        if (__improbable(PMAP_PAGE_IS_USER_EXECUTABLE(dst_page))) {
           ...
           * See the comment there for the gory details, but it essentially
           * boils down to the same situation of being asked to overwrite page
           * contents that were already marked executable from some prior use of
           * the vnode associated with this VM object.
           */
          pmap_disconnect_options(VM_PAGE_GET_PHYS_PAGE(dst_page),
                                  PMAP_OPTIONS_RETYPE, NULL);
        }
#endif /* CONFIG_SPTM */
        ...
        if (dst_page->vmp_cs_validated && !dst_page->vmp_cs_tainted) {
          /*
           * CODE SIGNING:
    ...
    for (i = 0; i < cur_run; i++) {
      ...
      if ((retval =
               uiomove64((addr64_t)(((addr64_t)(VM_PAGE_GET_PHYS_PAGE(dst_page))
                                     << PAGE_SHIFT) +
                                    start_offset),
                         xsize, uio))) {
        break;
...
bool memory_object_is_vnode_pager(memory_object_t mem_obj) {
  if (mem_obj != NULL && mem_obj->mo_pager_ops == &vnode_pager_ops) {
    return true;
  }
  return false;
}
 ...
 *
 */
memory_object_t vnode_pager_setup(struct vnode *vp,
 ...
 *
 */
kern_return_t vnode_pager_init(memory_object_t mem_obj,
 ...
 *
 */
kern_return_t vnode_pager_data_return(memory_object_t mem_obj,
                                      memory_object_offset_t offset,
                                      memory_object_cluster_size_t data_cnt,
                                      memory_object_offset_t *resid_offset,
                                      int *io_error, __unused boolean_t dirty,
  ...
  vnode_pager_cluster_write(vnode_object, offset, data_cnt, resid_offset,
                            io_error, upl_flags);
...
kern_return_t
vnode_pager_data_initialize(__unused memory_object_t mem_obj,
...
void vnode_pager_dirtied(memory_object_t mem_obj, vm_object_offset_t s_offset,
...
kern_return_t vnode_pager_get_isinuse(memory_object_t mem_obj,
                                      uint32_t *isinuse) {
  vnode_pager_t vnode_object;
...
kern_return_t vnode_pager_get_throttle_io_limit(memory_object_t mem_obj,
                                                uint32_t *limit) {
  vnode_pager_t vnode_object;
...
kern_return_t vnode_pager_get_isSSD(memory_object_t mem_obj, boolean_t *isSSD) {
  vnode_pager_t vnode_object;
...
kern_return_t vnode_pager_get_forced_unmount(memory_object_t mem_obj,
                                             bool *forced_unmount) {
  vnode_pager_t vnode_object;
...
kern_return_t vnode_pager_get_object_size(memory_object_t mem_obj,
...
kern_return_t vnode_pager_get_object_name(
    memory_object_t mem_obj, char *pathname, vm_size_t pathname_len,
    char *filename, vm_size_t filename_len, boolean_t *truncated_path_p) {
  vnode_pager_t vnode_object;
  ...
  return vnode_pager_get_name(vnode_object->vnode_handle, pathname,
                              pathname_len, filename, filename_len,
                              truncated_path_p);
}

kern_return_t vnode_pager_get_object_mtime(memory_object_t mem_obj,
  ...
  return vnode_pager_get_mtime(vnode_object->vnode_handle, mtime, cs_mtime);
}

#if CHECK_CS_VALIDATION_BITMAP
kern_return_t vnode_pager_cs_check_validation_bitmap(
    memory_object_t mem_obj, memory_object_offset_t offset, int optype) {
  vnode_pager_t vnode_object;
  ...
  vnode_object = vnode_pager_lookup(mem_obj);
  return ubc_cs_check_validation_bitmap(vnode_object->vnode_handle, offset,
                                        optype);
}
 ...
 */
kern_return_t
vnode_pager_data_request(memory_object_t mem_obj, memory_object_offset_t offset,
  ...
  assert(offset >= base_offset && offset < base_offset + size);

  return vnode_pager_cluster_read(vnode_object, base_offset, offset,
                                  io_streaming, size);
}
 ...
 *
 */
void vnode_pager_reference(memory_object_t mem_obj) {
  vnode_pager_t vnode_object;
 ...
 *
 */
void vnode_pager_deallocate(memory_object_t mem_obj) {
  vnode_pager_t vnode_object;
 ...
 *
 */
kern_return_t vnode_pager_terminate(
#if !DEBUG
    __unused
#endif
 ...
 *
 */
kern_return_t vnode_pager_map(memory_object_t mem_obj, vm_prot_t prot) {
  vnode_pager_t vnode_object;
  int ret;
  kern_return_t kr;
...
kern_return_t vnode_pager_last_unmap(memory_object_t mem_obj) {
  vnode_pager_t vnode_object;
 ...
 *
 */
void vnode_pager_cluster_write(vnode_pager_t vnode_object,
                               vm_object_offset_t offset, vm_size_t cnt,
                               vm_object_offset_t *resid_offset, int *io_error,
                               int upl_flags) {
  vm_size_t size;
  int errno;
  ...
  if (upl_flags & UPL_MSYNC) {
    ...
    while (cnt) {
      size = (cnt < MAX_UPL_TRANSFER_BYTES)
                 ? cnt
                 : MAX_UPL_TRANSFER_BYTES; /* effective max */

      assert((upl_size_t)size == size);
      vnode_pageout(vnode_object->vnode_handle, NULL, (upl_offset_t)0, offset,
                    (upl_size_t)size, upl_flags, &errno);
    ...
    assert((upl_size_t)size == size);
    vnode_pageout(vnode_object->vnode_handle, NULL,
                  (upl_offset_t)(offset - base_offset), base_offset,
                  (upl_size_t)size, (upl_flags & UPL_IOSYNC) | UPL_VNODE_PAGER,
                  NULL);
  }
}
 ...
 *
 */
kern_return_t vnode_pager_cluster_read(vnode_pager_t vnode_object,
                                       vm_object_offset_t base_offset,
                                       vm_object_offset_t offset,
                                       uint32_t io_streaming, vm_size_t cnt) {
  int local_error = 0;
  int kret;
  int flags = 0;
  ...
  kret = vnode_pagein(vnode_object->vnode_handle, (upl_t)NULL,
                      (upl_offset_t)(offset - base_offset), base_offset,
                      (upl_size_t)cnt, flags, &local_error);
  /*
  ...
  if (kret == 1) {
    ...
    kr = memory_object_upl_request(vnode_object->vn_pgr_hdr.mo_control,
                                   base_offset, (upl_size_t)cnt, &upl, NULL,
                                   &count, uplflags, VM_KERN_MEMORY_NONE);
    if (kr == KERN_SUCCESS) {
      upl_abort(upl, 0);
      upl_deallocate(upl);
    } else {
 ...
 *
 */
vnode_pager_t vnode_object_create(struct vnode *vp) {
  vnode_pager_t vnode_object;
 ...
 *
 */
vnode_pager_t vnode_pager_lookup(memory_object_t name) {
  vnode_pager_t vnode_object;
...
struct vnode *vnode_pager_lookup_vnode(memory_object_t name) {
  vnode_pager_t vnode_object;
  vnode_object = (vnode_pager_t)name;
...
static int fill_vnodeinfoforaddr(vm_map_entry_t entry, uintptr_t *vnodeaddr,
                                 uint32_t *vid, bool *is_map_shared);

int fill_procregioninfo(task_t task, uint64_t arg,
  ...
  if (!vm_map_lookup_entry(map, start, &tmp_entry)) {
    if ((entry = tmp_entry->vme_next) == vm_map_to_entry(map)) {
      if (do_region_footprint && address == tmp_entry->vme_end) {
         ...
         * owned by this task.
         */
        task_ledgers_footprint(task->ledger, &ledger_resident,
                               &ledger_compressed);
        ...
        pinfo->pri_size = (uint64_t)(ledger_resident + ledger_compressed);
        pinfo->pri_depth = 0;
  ...
  vm_map_region_walk(map, start, entry, VME_OFFSET(entry),
                     entry->vme_end - start, &extended, TRUE,
...
int fill_procregioninfo_onlymappedvnodes(task_t task, uint64_t arg,
                                         struct proc_regioninfo_internal *pinfo,
                                         uintptr_t *vnodeaddr, uint32_t *vid) {
  vm_map_t map;
  vm_map_offset_t address = (vm_map_offset_t)arg;
...
int task_find_region_details(task_t task, vm_map_offset_t offset,
                             find_region_details_options_t options,
                             uintptr_t *vp_p, uint32_t *vid_p,
                             bool *is_map_shared_p, uint64_t *start_p,
                             uint64_t *len_p) {
  vm_map_t map;
  vm_map_entry_t entry;
  int rc;
  ...
  *len_p = 0;
  ...
  for (; entry != vm_map_to_entry(map); entry = entry->vme_next) {
    vmlp_range_event_entry(map, entry);
...
static int fill_vnodeinfoforaddr(vm_map_entry_t entry, uintptr_t *vnodeaddr,
  ...
  if (object == VM_OBJECT_NULL) {
    ...
    vm_object_unlock(object);
    return 0;
  } else if (!object->pager_ready || object->terminating || !object->alive ||
             object->pager == NULL) {
    vm_object_unlock(object);
    return 0;
  } else {
    ...
    if (pager_ops == &vnode_pager_ops) {
      kr = vnode_pager_get_object_vnode(memory_object, vnodeaddr, vid);
      if (kr != KERN_SUCCESS) {
        vm_object_unlock(object);
        return 0;
      }
...
kern_return_t vnode_pager_get_object_vnode(memory_object_t mem_obj,
                                           uintptr_t *vnodeaddr,
                                           uint32_t *vid) {
  vnode_pager_t vnode_object;
...
#if CONFIG_IOSCHED
kern_return_t vnode_pager_get_object_devvp(memory_object_t mem_obj,
                                           uintptr_t *devvp) {
  struct vnode *vp;
  uint32_t vid;

  if (vnode_pager_get_object_vnode(mem_obj, (uintptr_t *)&vp,
                                   (uint32_t *)&vid) != KERN_SUCCESS) {
    return KERN_FAILURE;
  }
...
/*
 * Find the underlying vnode object for the given vm_map_entry.  If found,
 * return with the object locked, otherwise return NULL with nothing locked.
 */

vm_object_t find_vnode_object(vm_map_entry_t entry) {
  vm_object_t top_object, object;
  memory_object_t memory_object;
  memory_object_pager_ops_t pager_ops;
  ...
  if (!entry->is_sub_map) {
    ...
    if (top_object) {
      ...
      for (object = top_object; object->shadow != VM_OBJECT_NULL;
           object = object->shadow) {
        vm_object_lock(object->shadow);
        vm_object_unlock(object);
      }

      if (object && !object->internal && object->pager_ready &&
          !object->terminating && object->alive && object->pager != NULL) {
        memory_object = object->pager;
        pager_ops = memory_object->mo_pager_ops;

        /*
         * If this object points to the vnode_pager_ops, then we found what
         * we're looking for.  Otherwise, this vm_map_entry doesn't have an
         * underlying vnode and so we fall through to the bottom and return
         * NULL.
         */
