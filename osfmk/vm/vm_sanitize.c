...
/* avoid includes here; we want these pragmas to also affect included inline
 * functions */
 ...
 * a constant, but instead a variable whose value is determined on boot
...
#if !__OPTIMIZE__
#pragma clang attribute push(                                                  \
    __attribute__((no_sanitize("undefined", "integer", "unsigned-shift-base",  \
                               "nullability", "bounds"))),                     \
    apply_to = function)
#endif
 ...
 */
#define vm_add_no_ubsan(a, b)                                                  \
  ({                                                                           \
    typeof(a + b) TMP;                                                         \
    (void)os_add_overflow(a, b, &TMP);                                         \
    TMP;                                                                       \
  })
#define vm_sub_no_ubsan(a, b)                                                  \
  ({                                                                           \
    typeof(a + b) TMP;                                                         \
    (void)os_sub_overflow(a, b, &TMP);                                         \
    TMP;                                                                       \
  })

static inline kern_return_t
vm_sanitize_apply_err_rewrite_policy(kern_return_t initial_kr,
                                     vm_sanitize_compat_rewrite_t rewrite) {
...
vm_sanitize_size(vm_addr_struct_t offset_u, vm_size_struct_t size_u,
                 vm_sanitize_caller_t vm_sanitize_caller __unused, vm_map_t map,
  ...
  *size = VM_SANITIZE_UNSAFE_UNWRAP(size_u);
  ...
  /*
   * This rounding is a check on the vm_object and thus uses the kernel's
   * PAGE_MASK
   */
...
static __attribute__((warn_unused_result)) kern_return_t
vm_sanitize_err_compat_addr_size(kern_return_t initial_kr,
  ...
  if (vm_sanitize_caller->err_compat_addr_size) {
    compat = (vm_sanitize_caller->err_compat_addr_size)(
        initial_kr, VM_SANITIZE_UNSAFE_UNWRAP(addr_u),
        VM_SANITIZE_UNSAFE_UNWRAP(size_u), pgmask, map_or_null);
  }
  ...
  if (compat.should_telemeter) {
    ...
    if (vm_sanitize_telemeter_to_serial) {
      printf("VM API - [%s] unsanitary addr 0x%llx size 0x%llx pgmask "
             ...
             proc_best_name(current_proc()), VM_SANITIZE_UNSAFE_UNWRAP(addr_u),
             VM_SANITIZE_UNSAFE_UNWRAP(size_u), pgmask,
             vm_sanitize_caller->vmsc_caller_name, initial_kr,
             compat.compat_kr);
    }
#endif /* DEVELOPMENT || DEBUG */

    vm_sanitize_send_telemetry(
        vm_sanitize_caller->vmsc_telemetry_id, VM_SANITIZE_CHECKER_ADDR_SIZE,
        VM_SANITIZE_CHECKER_COUNT_1 /* fixme */,
        vm_sanitize_caller->vmsc_ktriage_id, VM_SANITIZE_UNSAFE_UNWRAP(addr_u),
        VM_SANITIZE_UNSAFE_UNWRAP(size_u), pgmask, 0 /* arg4 */, initial_kr,
        compat.compat_kr);
  }
...
vm_sanitize_addr_size(vm_addr_struct_t addr_u, vm_size_struct_t size_u,
                      vm_sanitize_caller_t vm_sanitize_caller,
                      mach_vm_offset_t pgmask, vm_map_t map_or_null,
  ...
  *size = VM_SANITIZE_UNSAFE_UNWRAP(size_u);
   ...
   */
  if (__improbable((flags & VM_SANITIZE_FLAGS_CHECK_ALIGNED_START) &&
                   (*addr & pgmask))) {
   ...
   */
  if (__improbable((flags & VM_SANITIZE_FLAGS_CHECK_ALIGNED_SIZE) &&
                   (*size & pgmask))) {
    kr = KERN_INVALID_ARGUMENT;
    goto unsanitary;
  }
  ...
  if (flags & VM_SANITIZE_FLAGS_CHECK_ADDR_RANGE) {
     ...
     */
    assert((__builtin_arm_rsr64("ID_AA64MMFR2_EL1") &
            ID_AA64MMFR2_EL1_VARANGE_MASK) == 0);
...
unsanitary:
  ...
  *end = 0;
  *size = 0;
  return vm_sanitize_err_compat_addr_size(kr, vm_sanitize_caller, addr_u,
                                          size_u, pgmask, map_or_null);
}
...
__attribute__((always_inline, warn_unused_result)) kern_return_t
vm_sanitize_prot(vm_prot_ut prot_u,
                 vm_sanitize_caller_t vm_sanitize_caller __unused,
                 vm_map_t map __unused, vm_prot_t extra_mask, vm_prot_t *prot) {
...
vm_sanitize_err_compat_cur_and_max_prots(
    kern_return_t initial_kr, vm_sanitize_caller_t vm_sanitize_caller,
    vm_prot_ut cur_prot_u, vm_prot_ut max_prot_u, vm_prot_t extra_mask,
    vm_prot_t *out_cur, vm_prot_t *out_max) {
  ...
  if (compat.should_telemeter) {
    ...
    if (vm_sanitize_telemeter_to_serial) {
      printf("VM API - [%s] unsanitary vm_prot cur %d max %d "
             ...
             proc_best_name(current_proc()), initial_cur_prot, initial_max_prot,
             vm_sanitize_caller->vmsc_caller_name, initial_kr,
             compat.compat_kr);
    }
#endif /* DEVELOPMENT || DEBUG */
    ...
    vm_sanitize_send_telemetry(
        vm_sanitize_caller->vmsc_telemetry_id, VM_SANITIZE_CHECKER_PROT_CUR_MAX,
        VM_SANITIZE_CHECKER_COUNT_1 /* fixme */,
        vm_sanitize_caller->vmsc_ktriage_id, initial_cur_prot, initial_max_prot,
        extra_mask, 0 /* arg4 */, initial_kr, compat.compat_kr);
  }
...
vm_sanitize_cur_and_max_prots(vm_prot_ut cur_prot_u, vm_prot_ut max_prot_u,
  ...
  kr = vm_sanitize_prot(cur_prot_u, vm_sanitize_caller, map, extra_mask,
                        cur_prot);
  ...
  kr = vm_sanitize_prot(max_prot_u, vm_sanitize_caller, map, extra_mask,
                        max_prot);
  if (__improbable(kr != KERN_SUCCESS)) {
    *cur_prot = VM_PROT_NONE;
    *max_prot = VM_PROT_NONE;
...
unsanitary:
  ...
  *max_prot = VM_PROT_NONE;
  ...
  return vm_sanitize_err_compat_cur_and_max_prots(
      kr, vm_sanitize_caller, cur_prot_u, max_prot_u, extra_mask, cur_prot,
      max_prot);
}
...
vm_sanitize_behavior(vm_behavior_ut behavior_u,
  ...
  *behavior = VM_SANITIZE_UNSAFE_UNWRAP(behavior_u);
  ...
  if (__improbable((*behavior > VM_BEHAVIOR_LAST_VALID) || (*behavior < 0))) {
