...
extern kern_return_t vm_map_exec(vm_map_t new_map, task_t task,
                                 boolean_t is64bit, void *fsroot,
...
#define named_entry_lock_init(object)                                          \
  lck_mtx_init(&(object)->Lock, &vm_object_lck_grp, &vm_object_lck_attr)
#define named_entry_lock_destroy(object)                                       \
  lck_mtx_destroy(&(object)->Lock, &vm_object_lck_grp)
...
struct vm_named_entry {
  ...
  unsigned int                        /* Is backing.xxx : */
      ...
      /* boolean_t */ is_object : 1,  /* ... a VM object (wrapped in a VM map
                                         copy) */
...
#define vme_end links.end
  ...
  unsigned long long
      ...
      /* vm_object_offset_t*/
      vme_offset
      : VME_OFFSET_BITS, /* offset into object */
  ...
  /* Only in task maps: */
        ...
        /* vm_prot_t-like    */ max_protection : 4, /* maximum protection,
                                                       bit3=UEXEC */
        ...
        /* boolean_t         */ vme_permanent : 1, /* mapping can not be removed
                                                    */
        /* boolean_t         */ superpage_size : 1, /* use superpages of a
                                                       certain size */
                                                    /*
        ...
        /* boolean_t         */ csm_associated : 1, /* code signing monitor will
                                                       validate */
        ...
        /* boolean_t         */ translated_allow_execute : 1, /* execute in
                                                                 translated
                                                                 processes */
        /* boolean_t         */ vme_kernel_object : 1,        /* vme_object is a
                                                                 kernel_object */
...
#define VME_ALIAS(entry) ((entry)->vme_alias)

static inline vm_map_t _VME_SUBMAP(vm_map_entry_t entry) {
...
static inline void VME_SUBMAP_SET(vm_map_entry_t entry, vm_map_t submap) {
...
static inline vm_object_t _VME_OBJECT(vm_map_entry_t entry) {
  vm_object_t object;
...
static inline vm_object_offset_t VME_OFFSET(vm_map_entry_t entry) {
...
#define max_offset hdr.links.end                      /* end of range */
  ...
  vm_map_size_t
      user_wire_size; /* current size of user locked memory in this map */
#if __x86_64__
...
#define holes_list f_s._holes
  ...
  unsigned int
      ...
      /* boolean_t */ mapped_in_other_pmaps : 1, /* has this submap been mapped
                                                    in maps that use a different
                                                    pmap */
      /* boolean_t */ switch_protect : 1, /* Protect map from write faults while
                                             switched */
      /* boolean_t */ disable_vmentry_reuse : 1,  /* All vm entries should keep
                                                     using newer and higher
                                                     addresses in the map */
      /* boolean_t */ map_disallow_data_exec : 1, /* Disallow execution from
                                                     data pages on
                                                     exec-permissive
                                                     architectures */
      ...
      /* boolean_t */ is_alien : 1,         /* for platform simulation, i.e.
                                               PLATFORM_IOS on OSX */
      /* boolean_t */ cs_enforcement : 1,   /* code-signing enforcement */
      /* boolean_t */ cs_debugged : 1,      /* code-signed but debugged */
      /* boolean_t */ reserved_regions : 1, /* has reserved regions. The map
                                               size that userspace sees should
                                               ignore these. */
      /* boolean_t */ single_jit : 1,       /* only allow one JIT mapping */
      /* boolean_t */ never_faults : 1, /* this map should never cause faults */
      /* boolean_t */ uses_user_ranges : 1, /* has the map been configured to
                                               use user VM ranges */
      /* boolean_t */ tpro_enforcement : 1, /* enforce TPRO propagation */
      /* boolean_t */ corpse_source : 1, /* map is being used to create a corpse
                                            for diagnostics.*/
      /* boolean_t */ cs_platform_binary : 1, /* map belongs to a platform
                                                 binary */

#define VM_MAP_NOT_SEALED 0 /* map is not sealed and may be freely modified.   \
                             */
#define VM_MAP_WILL_BE_SEALED                                                  \
  1 /* map will be sealed and is subject to limited modification. */
#define VM_MAP_SEALED 2 /* map is sealed and should not be modified. */
      /* unsigned int */ vmmap_sealed : 2, /* sealed state of map, see
                                              definitions above. */
      /* reserved */ res0 : 1,
      /* reserved  */ pad : 6;
...
static inline void VME_PREV_SET(vm_map_entry_t entry, vm_map_entry_t prev) {
  entry->links.prev = VM_PREV_PACK(prev);
}

static inline void VMH_PREV_SET(struct vm_map_header *hdr,
                                vm_map_entry_t prev) {
  hdr->links.prev = VM_PREV_PACK(prev);
}

static inline void VML_PREV_SET(struct vm_map_links *links,
                                vm_map_entry_t prev) {
...
#define VM_MAP_COPY_KERNEL_BUFFER 2
  ...
  union {
    ...
    struct {
      void *
          XNU_PTRAUTH_SIGNED_PTR("vm_map_copy.kdata") kdata; /* KERNEL_BUFFER */
    } buffer_data;
  } c_u;
};
...
extern boolean_t vm_map_lookup_entry(vm_map_t map, vm_map_address_t address,
...
#define vm_map_entry_wait(map, interruptible)                                  \
  _vm_map_entry_wait_helper((map), (interruptible))

#define vm_map_entry_wakeup(map) thread_wakeup((event_t)(&(map)->hdr))

extern void vm_map_inherit_limits(vm_map_t new_map,
                                  const struct _vm_map *old_map);

/* Create a new task map using an existing task map as a template. */
extern vm_map_t vm_map_fork(ledger_t ledger, vm_map_t old_map, int options);
...
extern kern_return_t vm_map_set_cache_attr(vm_map_t map, vm_map_offset_t va);

extern void vm_map_copy_footprint_ledgers(task_t old_task, task_t new_task);
 ...
 */
extern size_t
ml_get_vm_reserved_regions(bool vm_is64bit,
...
extern bool vm_map_is_sealed(vm_map_t map);
 ...
 */
extern kern_return_t vm_map_write_user(vm_map_t map, void *src_p,
                                       vm_map_offset_ut dst_addr_u,
                                       vm_size_ut size_u);

extern kern_return_t vm_map_read_user(vm_map_t map, vm_map_offset_ut src_addr_u,
...
extern vm_map_switch_context_t
vm_map_switch_with_sec_override(vm_map_t, boolean_t sec_override);
static inline vm_map_switch_context_t vm_map_switch_to(vm_map_t map) {
  return vm_map_switch_with_sec_override(map, FALSE);
}
extern void vm_map_switch_back(vm_map_switch_context_t ctx);

extern boolean_t vm_map_cs_enforcement(vm_map_t map);
extern void vm_map_cs_enforcement_set(vm_map_t map, boolean_t val);

extern void vm_map_cs_debugged_set(vm_map_t map, boolean_t val);

extern kern_return_t vm_map_cs_wx_enable(vm_map_t map);
extern kern_return_t vm_map_csm_allow_jit(vm_map_t map);

extern void vm_map_will_allocate_early_map(vm_map_t *map_owner);

extern void vm_map_relocate_early_maps(vm_offset_t delta);

extern void vm_map_relocate_early_elem(uint32_t zone_id, vm_offset_t new_addr,
                                       vm_offset_t delta);

/* wire down a region */

/* never fails */
extern vm_map_t vm_map_create_options(pmap_t pmap, vm_map_offset_t min_off,
...
extern kern_return_t vm_map_enter_mem_object_control(
    ...
    vm_map_offset_ut mask, vm_map_kernel_flags_t vmk_flags,
    memory_object_control_t control, vm_object_offset_ut offset,
    boolean_t needs_copy, vm_prot_ut cur_protection, vm_prot_ut max_protection,
    vm_inherit_ut inheritance);

/* Must be executed on a new task's map before the task is enabled for IPC
 * access */
extern void vm_map_setup(vm_map_t map, task_t task); /* always succeeds */

extern kern_return_t vm_map_terminate(vm_map_t map);

/* Overwrite existing memory with a copy */
extern kern_return_t vm_map_copy_overwrite(vm_map_t dst_map,
...
extern boolean_t vm_map_copy_validate_size(vm_map_t dst_map, vm_map_copy_t copy,
                                           vm_map_size_t *size);

extern kern_return_t vm_map_copyout_size(vm_map_t dst_map,
                                         vm_map_address_t *dst_addr, /* OUT */
                                         vm_map_copy_t copy,
                                         vm_map_size_ut copy_size);

extern void vm_map_disable_NX(vm_map_t map);

extern void vm_map_disallow_data_exec(vm_map_t map);

extern void vm_map_set_64bit(vm_map_t map);

extern void vm_map_set_32bit(vm_map_t map);

extern void vm_map_set_jumbo(vm_map_t map);

#if XNU_PLATFORM_iPhoneOS && EXTENDED_USER_VA_SUPPORT
extern void vm_map_set_extra_jumbo(vm_map_t map);
#endif /* XNU_PLATFORM_iPhoneOS && EXTENDED_USER_VA_SUPPORT */

extern void vm_map_set_jit_entitled(vm_map_t map);

extern void vm_map_set_max_addr(vm_map_t map, vm_map_offset_t new_max_offset,
                                bool extra_jumbo);

extern boolean_t vm_map_has_hard_pagezero(vm_map_t map,
                                          vm_map_offset_t pagezero_size);

extern void vm_commit_pagezero_status(vm_map_t tmap);

extern void vm_map_set_platform_binary(vm_map_t map, bool is_platform_binary);
extern bool vm_map_is_platform_binary(vm_map_t map);

extern boolean_t vm_map_tpro(vm_map_t map);

extern void vm_map_set_tpro(vm_map_t map);

extern void vm_map_set_tpro_enforcement(vm_map_t map);

extern boolean_t vm_map_set_tpro_range(vm_map_t map, vm_map_address_t start,
                                       vm_map_address_t end);

extern boolean_t vm_map_is_64bit(vm_map_t map);

extern kern_return_t vm_map_raise_max_offset(vm_map_t map,
                                             vm_map_offset_t new_max_offset);

extern kern_return_t vm_map_raise_min_offset(vm_map_t map,
                                             vm_map_offset_t new_min_offset);

#if XNU_TARGET_OS_OSX
extern void vm_map_set_high_start(vm_map_t map, vm_map_offset_t high_start);
#endif /* XNU_TARGET_OS_OSX */

extern vm_map_offset_t vm_compute_max_offset(boolean_t is64);

extern void vm_map_get_max_aslr_slide_section(vm_map_t map,
                                              int64_t *max_sections,
                                              int64_t *section_size);

extern uint64_t vm_map_get_max_aslr_slide_pages(vm_map_t map);

extern uint64_t vm_map_get_max_loader_aslr_slide_pages(vm_map_t map);

extern kern_return_t vm_map_set_size_limit(vm_map_t map, uint64_t limit);

extern kern_return_t vm_map_set_data_limit(vm_map_t map, uint64_t limit);

extern void vm_map_set_user_wire_limit(vm_map_t map, vm_size_t limit);

extern void vm_map_switch_protect(vm_map_t map, boolean_t val);

extern boolean_t vm_map_page_aligned(vm_map_offset_t offset,
                                     vm_map_offset_t mask);

extern bool vm_map_range_overflows(vm_map_t map, vm_map_offset_t addr,
                                   vm_map_size_t size);

/* Support for vm_map ranges */
extern kern_return_t vm_map_range_configure(vm_map_t map,
 ...
 */
extern void vm_map_kernel_flags_update_range_id(vm_map_kernel_flags_t *flags,
                                                vm_map_t map,
                                                vm_map_size_t size);
...
extern kern_return_t vm_map_page_info(vm_map_t map, vm_map_offset_ut offset,
                                      vm_page_info_flavor_t flavor,
                                      vm_page_info_t info,
                                      mach_msg_type_number_t *count);

extern kern_return_t vm_map_page_range_info_internal(
    vm_map_t map, vm_map_offset_ut start_offset, vm_map_offset_ut end_offset,
    int effective_page_shift, vm_page_info_flavor_t flavor, vm_page_info_t info,
    mach_msg_type_number_t *count);
...
#define VM_MAP_TRUNC_PAGE(x, pgmask)                                           \
  ((vm_map_offset_t)(x) & ~((signed)(pgmask)))

/*
 * Macros for rounding and truncation of vm_map offsets and sizes
 */
static inline int VM_MAP_PAGE_SHIFT(vm_map_t map) {
...
extern kern_return_t vm_map_sign(vm_map_t map, vm_map_offset_t start,
                                 vm_map_offset_t end);
...
extern kern_return_t
vm_map_freeze(task_t task, unsigned int *purgeable_count,
              unsigned int *wired_count, unsigned int *clean_count,
              unsigned int *dirty_count, unsigned int dirty_budget,
...
extern kern_return_t vm_map_partial_reap(vm_map_t map,
 ...
 * reported.
 */
#define VM_OBJECT_ID_FAKE(map, ledger_id)                                      \
  ((uint32_t)(uintptr_t)VM_KERNEL_ADDRHASH((int *)((map)->pmap->ledger) +      \
                                           (ledger_id)))

#if DEVELOPMENT || DEBUG

extern int vm_map_disconnect_page_mappings(vm_map_t map, boolean_t);
...
extern kern_return_t
vm_map_dump_entry_and_compressor_pager(void *entry, char *buf, size_t *count);

extern void vm_map_testing_make_sealed_submap(vm_map_t parent_map,
                                              mach_vm_address_t start,
                                              mach_vm_address_t end);

extern void vm_map_testing_remap_submap(vm_map_t parent_map,
...
bool vm_map_is_map_size_valid(vm_map_t target_map, vm_size_t size,
                              bool no_soft_limit);
