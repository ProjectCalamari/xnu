...
kern_return_t apple_protect_pager_data_request(
    memory_object_t mem_obj, memory_object_offset_t offset,
    memory_object_cluster_size_t length, vm_prot_t protection_required,
    memory_object_fault_info_t fault_info);
kern_return_t apple_protect_pager_data_return(
    memory_object_t mem_obj, memory_object_offset_t offset,
    memory_object_cluster_size_t data_cnt, memory_object_offset_t *resid_offset,
...
kern_return_t apple_protect_pager_map(memory_object_t mem_obj, vm_prot_t prot);
...
unsigned int apple_protect_pager_count_mapped =
...
apple_protect_pager_t apple_protect_pager_create(
    ...
    struct pager_crypt_info *crypt_info, vm_object_offset_t crypto_start,
    vm_object_offset_t crypto_end, boolean_t cache_pager);
...
apple_protect_pager_init(memory_object_t mem_obj,
  ...
  PAGER_DEBUG(PAGER_ALL, ("apple_protect_pager_init: %p, %p, %x\n", mem_obj,
                          control, pg_size));
 ...
 */
kern_return_t apple_protect_pager_data_return(
    __unused memory_object_t mem_obj, __unused memory_object_offset_t offset,
    __unused memory_object_cluster_size_t data_cnt,
    __unused memory_object_offset_t *resid_offset, __unused int *io_error,
    __unused boolean_t dirty, __unused boolean_t kernel_copy,
    __unused int upl_flags) {
  panic("apple_protect_pager_data_return: should never get called");
  return KERN_FAILURE;
}

kern_return_t apple_protect_pager_data_initialize(
    __unused memory_object_t mem_obj, __unused memory_object_offset_t offset,
    __unused memory_object_cluster_size_t data_cnt) {
...
apple_protect_pager_data_request(memory_object_t mem_obj,
  ...
  assert(os_ref_get_count_raw(&pager->ap_pgr_hdr_ref) >
         1); /* pager is alive and mapped */

  PAGER_DEBUG(PAGER_PAGEIN,
  ...
  retry_src_fault:
    ...
    vmfr = vm_fault_page(
        src_top_object, pager->backing_offset + offset + cur_offset,
        VM_PROT_READ, FALSE, FALSE, /* src_page not looked up */
        &prot, &src_page, &top_page, NULL, &error_code, FALSE, &fault_info);
    switch (vmfr) {
    ...
    for (offset_in_page = 0; offset_in_page < PAGE_SIZE;
         offset_in_page += 4096) {
      if (offset + cur_offset + offset_in_page < pager->crypto_start ||
          offset + cur_offset + offset_in_page >= pager->crypto_end) {
        /* not encrypted: just copy */
        bcopy((const char *)(src_vaddr + offset_in_page),
              (char *)(dst_vaddr + offset_in_page), 4096);
        ...
        if (apple_protect_pager_data_request_debug) {
          printf("apple_protect_data_request"
                 ...
                 "cs_tainted=%d "
                 "cs_nx=%d\n",
                 pager, offset, (uint64_t)cur_offset, (uint64_t)offset_in_page,
                 pager->crypto_start, pager->crypto_end,
                 *(uint64_t *)(dst_vaddr + offset_in_page),
                 *(uint64_t *)(dst_vaddr + offset_in_page + 8),
                 src_page_object->code_signed, src_page->vmp_cs_validated,
                 src_page->vmp_cs_tainted, src_page->vmp_cs_nx);
        }
        ret = 0;
        continue;
      }
      ret = pager->crypt_info->page_decrypt(
          ...
          ((pager->crypto_backing_offset - pager->crypto_start) + /* XXX ? */
      ...
      if (apple_protect_pager_data_request_debug) {
        printf("apple_protect_data_request"
               ...
               pager, offset, (uint64_t)cur_offset, (uint64_t)offset_in_page,
               pager->crypto_start, pager->crypto_end,
               ((pager->crypto_backing_offset - pager->crypto_start) + offset +
                cur_offset + offset_in_page),
               pager->crypto_backing_offset, pager->crypto_start, offset,
               (uint64_t)cur_offset, (uint64_t)offset_in_page,
               *(uint64_t *)(dst_vaddr + offset_in_page),
               *(uint64_t *)(dst_vaddr + offset_in_page + 8),
               src_page_object->code_signed, src_page->vmp_cs_validated,
               src_page->vmp_cs_tainted, src_page->vmp_cs_nx, ret);
      }
      if (ret) {
        break;
      }
  ...
  if (upl != NULL) {
    ...
    if (retval != KERN_SUCCESS) {
      ...
      if (retval == KERN_ABORTED) {
         ...
         */
        wait_result =
            assert_wait_timeout((event_t)apple_protect_pager_data_request,
                                THREAD_UNINT, 10000, /* 10ms */
                                NSEC_PER_USEC);
      ...
      assertf(page_aligned(upl->u_offset) && page_aligned(upl->u_size),
              "upl %p offset 0x%llx size 0x%x", upl, upl->u_offset,
              upl->u_size);
...
void apple_protect_pager_dequeue(apple_protect_pager_t pager) {
  ...
  queue_remove(&apple_protect_pager_queue, pager, apple_protect_pager_t,
               pager_queue);
...
void apple_protect_pager_terminate_internal(apple_protect_pager_t pager) {
  ...
  printf("CRYPT_INFO %s: deallocate %p ref %d\n", __FUNCTION__,
         pager->crypt_info, pager->crypt_info->crypt_refcnt);
...
void apple_protect_pager_deallocate_internal(apple_protect_pager_t pager,
  ...
  count_unmapped =
      (apple_protect_pager_count - apple_protect_pager_count_mapped);
 ...
 *
 */
kern_return_t apple_protect_pager_terminate(
#if !DEBUG
    __unused
#endif
...
kern_return_t apple_protect_pager_last_unmap(memory_object_t mem_obj) {
  ...
  PAGER_DEBUG(PAGER_ALL, ("apple_protect_pager_last_unmap: %p\n", mem_obj));
...
boolean_t apple_protect_pager_backing_object(
  ...
  PAGER_DEBUG(PAGER_ALL, ("apple_protect_pager_backing_object: %p\n", mem_obj));
...
apple_protect_pager_t apple_protect_pager_create(
  ...
  printf("CRYPT_INFO %s: crypt_info %p [%p,%p,%p,%d]\n", __FUNCTION__,
         crypt_info, crypt_info->page_decrypt, crypt_info->crypt_end,
         crypt_info->crypt_ops, crypt_info->crypt_refcnt);
#endif /* CRYPT_INFO_DEBUG */
  ...
  queue_iterate(&apple_protect_pager_queue, pager2, apple_protect_pager_t,
                pager_queue) {
    if ((pager2->crypt_info->page_decrypt != crypt_info->page_decrypt) ||
        (pager2->crypt_info->crypt_end != crypt_info->crypt_end) ||
        (pager2->crypt_info->crypt_ops != crypt_info->crypt_ops)) {
    ...
    if (old_crypt_info) {
      ...
      printf("CRYPT_INFO %s: reference %p ref %d "
             "(create match)\n",
             __FUNCTION__, pager2->crypt_info,
  ...
  if (!queue_end(&apple_protect_pager_queue, (queue_entry_t)pager2)) {
  ...
  queue_enter_first(&apple_protect_pager_queue, pager, apple_protect_pager_t,
                    pager_queue);
  apple_protect_pager_count++;
  ...
  if (old_crypt_info != NULL && old_crypt_info != crypt_info) {
    /* we re-used an old crypt_info instead of using our new one */
#if CRYPT_INFO_DEBUG
    printf("CRYPT_INFO %s: deallocate %p ref %d "
           "(create used old)\n",
           __FUNCTION__, crypt_info, crypt_info->crypt_refcnt);
#endif /* CRYPT_INFO_DEBUG */
...
memory_object_t apple_protect_pager_setup(
  ...
  queue_iterate(&apple_protect_pager_queue, pager, apple_protect_pager_t,
                pager_queue) {
    if ((pager->crypt_info->page_decrypt != crypt_info->page_decrypt) ||
        (pager->crypt_info->crypt_end != crypt_info->crypt_end) ||
        (pager->crypt_info->crypt_ops != crypt_info->crypt_ops)) {
    ...
    if (old_crypt_info) {
      ...
      printf("CRYPT_INFO %s: "
             ...
             __FUNCTION__, crypt_info, crypt_info->page_decrypt,
             crypt_info->crypt_end, crypt_info->crypt_ops,
             crypt_info->crypt_refcnt, old_crypt_info,
             old_crypt_info->page_decrypt, old_crypt_info->crypt_end,
             old_crypt_info->crypt_ops, old_crypt_info->crypt_refcnt, pager);
    ...
    if (pager->backing_object == backing_object &&
      ...
      printf("CRYPT_INFO %s: "
             ...
             __FUNCTION__, pager, pager->crypt_info);
      printf("CRYPT_INFO %s: deallocate %p ref %d "
             "(pager match)\n",
             __FUNCTION__, old_crypt_info, old_crypt_info->crypt_refcnt);
#endif /* CRYPT_INFO_DEBUG */
  ...
  if (queue_end(&apple_protect_pager_queue, (queue_entry_t)pager)) {
    lck_mtx_unlock(&apple_protect_pager_lock);
    ...
    if (new_crypt_info == NULL) {
      ...
      /* create new pager */
      pager = apple_protect_pager_create(backing_object, backing_offset,
                                         crypto_backing_offset, new_crypt_info,
                                         crypto_start, crypto_end, cache_pager);
    }
    if (pager == APPLE_PROTECT_PAGER_NULL) {
      ...
      if (new_crypt_info == old_crypt_info) {
        ...
        printf("CRYPT_INFO %s: deallocate %p ref %d "
               ...
               __FUNCTION__, new_crypt_info, new_crypt_info->crypt_refcnt);
#endif /* CRYPT_INFO_DEBUG */
  ...
  while (!pager->is_ready) {
    lck_mtx_sleep(&apple_protect_pager_lock, LCK_SLEEP_DEFAULT,
                  &pager->is_ready, THREAD_UNINT);
  }
  lck_mtx_unlock(&apple_protect_pager_lock);
...
void apple_protect_pager_trim(void) {
  ...
  for (pager = (apple_protect_pager_t)queue_last(&apple_protect_pager_queue);
       !queue_end(&apple_protect_pager_queue, (queue_entry_t)pager);
       pager = prev_pager) {
    /* get prev elt before we dequeue */
    prev_pager = (apple_protect_pager_t)queue_prev(&pager->pager_queue);

    if (pager->is_cached && os_ref_get_count_raw(&pager->ap_pgr_hdr_ref) == 2 &&
        pager->is_ready && !pager->is_mapped) {
      /* this pager can be trimmed */
      num_trim++;
      /* remove this pager from the main list ... */
      apple_protect_pager_dequeue(pager);
      /* ... and add it to our trim queue */
      queue_enter_first(&trim_queue, pager, apple_protect_pager_t, pager_queue);
  ...
  while (!queue_empty(&trim_queue)) {
    queue_remove_first(&trim_queue, pager, apple_protect_pager_t, pager_queue);
...
#if CRYPT_INFO_DEBUG
  printf("CRYPT_INFO %s: %p ref %d -> %d\n", __FUNCTION__, crypt_info,
         crypt_info->crypt_refcnt, crypt_info->crypt_refcnt + 1);
#endif /* CRYPT_INFO_DEBUG */
...
static uint64_t apple_protect_pager_purge(apple_protect_pager_t pager) {
...
uint64_t apple_protect_pager_purge_all(void) {
  uint64_t pages_purged;
  apple_protect_pager_t pager;
