...
TUNABLE(bool, vps_dynamic_priority_enabled, "vps_dynamic_priority_enabled",
        false);
#else
...
#ifndef VM_PAGEOUT_BURST_INACTIVE_THROTTLE /* maximum iterations of the        \
                                              inactive queue w/o               \
                                              stealing/cleaning a page */
...
#define VM_PAGEOUT_DEADLOCK_RELIEF                                             \
  100 /* number of pages to move to break deadlock */
#endif

#ifndef VM_PAGE_LAUNDRY_MAX
#define VM_PAGE_LAUNDRY_MAX                                                    \
  128UL /* maximum pageouts on a given pageout queue */
...
TUNABLE_DEV_WRITEABLE(unsigned int, vm_page_max_speculative_age_q,
                      "vm_page_max_speculative_age_q",
                      VM_PAGE_DEFAULT_MAX_SPECULATIVE_AGE_Q);
#ifndef VM_PAGE_SPECULATIVE_TARGET
#define VM_PAGE_SPECULATIVE_TARGET(total)                                      \
  ((total) * 1 / (100 / vm_pageout_state.vm_page_speculative_percentage))
...
#define VM_PAGE_FREE_RESERVED(n) ((unsigned)(6 * VM_PAGE_LAUNDRY_MAX) + (n))
...
#else /* !XNU_TARGET_OS_OSX */
#define VM_PAGE_REACTIVATE_LIMIT(avail)                                        \
  (MAX((avail) * 1 / 20, VM_PAGE_REACTIVATE_LIMIT_MAX))
#endif /* !XNU_TARGET_OS_OSX */
...
struct pgo_iothread_state
    pgo_iothread_internal_state[MAX_COMPRESSOR_THREAD_COUNT];
...
static void vm_pageout_iothread_external(struct pgo_iothread_state *,
                                         wait_result_t);
static void vm_pageout_iothread_internal(struct pgo_iothread_state *,
                                         wait_result_t);
static void vm_pageout_adjust_eq_iothrottle(struct pgo_iothread_state *,
                                            boolean_t);
...
TUNABLE(bool, vm_compressor_ebound, "vmcomp_ecluster",
        VM_COMPRESSOR_EBOUND_DEFAULT);
int vm_pgo_pbound = 0;
 ...
 */
void vm_pageout_object_terminate(vm_object_t object) {
  ...
  while (!vm_page_queue_empty(&object->memq)) {
    ...
    m = vm_page_lookup(shadow_object, offset + object->vo_shadow_offset);
 ...
 *
 */
static void vm_pageclean_setup(vm_page_t m, vm_page_t new_m,
 ...
 */
void vm_pageout_initialize_page(vm_page_t m) {
  vm_object_t object;
...
static void vm_pageout_cluster_to_queue(vm_page_t m,
  ...
  if (object->internal == TRUE) {
    ...
    sched_cond_signal(&pgo_iothread_external_state.pgo_wakeup,
                      pgo_iothread_external_state.pgo_iothread);
  }
  VM_PAGE_CHECK(m);
}

void vm_pageout_cluster(vm_page_t m) {
  struct vm_pageout_queue *q;
  vm_object_t object = VM_PAGE_OBJECT(m);
  if (object->internal) {
 ...
 */
void vm_pageout_throttle_up(vm_page_t m) {
  struct vm_pageout_queue *q;
...
static void vm_pageout_throttle_up_batch(struct vm_pageout_queue *q,
                                         int batch_cnt) {
  LCK_MTX_ASSERT(&vm_page_queue_lock, LCK_MTX_ASSERT_OWNED);
...
void record_memory_pressure(void);
void record_memory_pressure(void) {
  ...
  vm_pageout_state.vm_memory_pressure =
      vm_pageout_stats[VM_PAGEOUT_STAT_BEFORE(vm_pageout_stat_now)]
          .freed_speculative +
      vm_pageout_stats[VM_PAGEOUT_STAT_BEFORE(vm_pageout_stat_now)]
          .freed_cleaned +
      vm_pageout_stats[VM_PAGEOUT_STAT_BEFORE(vm_pageout_stat_now)]
  ...
  commpage_set_memory_pressure(
      (unsigned int)vm_pageout_state.vm_memory_pressure);
 ...
 */
unsigned int mach_vm_ctl_page_free_wanted(void) {
...
kern_return_t mach_vm_pressure_monitor(boolean_t wait_for_pressure,
  ...
  if (wait_for_pressure) {
    ...
    while (vm_page_free_count >= vm_page_free_target) {
      wr = assert_wait((event_t)&vm_page_free_wanted, THREAD_INTERRUPTIBLE);
  ...
  pages_reclaimed = 0;
  for (vm_pageout_then = VM_PAGEOUT_STAT_BEFORE(vm_pageout_now);
       vm_pageout_then != vm_pageout_now && units_of_monitor-- != 0;
       vm_pageout_then = VM_PAGEOUT_STAT_BEFORE(vm_pageout_then)) {
...
static void vm_pageout_disconnect_all_pages_in_queue(vm_page_queue_head_t *,
                                                     int);
...
void vm_pageout_disconnect_all_pages() {
  vm_page_lock_queues();
  ...
  vm_pageout_disconnect_all_pages_in_queue(
...
/* NB: assumes the page_queues lock is held on entry, returns with page queue
 * lock held */
void vm_pageout_disconnect_all_pages_in_queue(vm_page_queue_head_t *q,
                                              int qcount) {
  vm_page_t m;
  ...
  while (qcount && !vm_page_queue_empty(q)) {
    ...
    if (!m_object->alive || m->vmp_cleaning || m->vmp_laundry || m->vmp_busy ||
        m->vmp_absent || VMP_ERROR_GET(m) || m->vmp_free_when_done) {
      /*
...
int vm_toggle_task_selfdonate_pages(task_t task) {
  int state = 0;
...
void vm_task_set_selfdonate_pages(task_t task, bool donate) {
...
static size_t vm_pageout_page_queue(vm_page_queue_head_t *, size_t, bool);
...
kern_return_t vm_pageout_anonymous_pages() {
  if (VM_CONFIG_COMPRESSOR_IS_PRESENT) {
    ...
    throttled_pages_moved = vm_pageout_page_queue(
        &vm_page_queue_throttled, vm_page_throttled_count, false);
    anonymous_pages_moved = vm_pageout_page_queue(
        &vm_page_queue_anonymous, vm_page_anonymous_count, false);
    active_pages_moved = vm_pageout_page_queue(&vm_page_queue_active,
                                               vm_page_active_count, false);

    os_log(OS_LOG_DEFAULT,
           "%s: throttled pages moved: %zu, anonymous pages moved: %zu, active "
           "pages moved: %zu",
           __func__, throttled_pages_moved, anonymous_pages_moved,
           active_pages_moved);
...
size_t vm_pageout_page_queue(vm_page_queue_head_t *q, size_t qcount,
                             bool perf_test) {
  vm_page_t m;
  vm_object_t t_object = NULL;
  vm_object_t l_object = NULL;
  ...
  while (qcount && !vm_page_queue_empty(q)) {
    ...
    if (m->vmp_reference == TRUE) {
      ...
      pmap_clear_refmod_options(phys_page, VM_MEM_REFERENCED,
                                PMAP_OPTIONS_NOFLUSH, (void *)NULL);
      goto reenter_pg_on_q;
    }
    if (m->vmp_pmapped == TRUE) {
    ...
    if (!m_object->pager_initialized || m_object->pager == MEMORY_OBJECT_NULL) {
      ...
      if (!m_object->pager_initialized ||
          m_object->pager == MEMORY_OBJECT_NULL) {
        /*
...
#define VM_PAGEOUT_SCAN_HANDLE_REUSABLE_PAGE(m, obj)                           \
  ...
  if ((m)->vmp_reusable || (obj)->all_reusable) {                              \
    vm_object_reuse_pages((obj), (m)->vmp_offset,                              \
                          (m)->vmp_offset + PAGE_SIZE_64, FALSE);              \
  }                                                                            \
  MACRO_END
...
#endif
static void vm_pageout_prepare_to_block(vm_object_t *, int *, vm_page_t *,
                                        int *, int);
...
static void vm_pageout_prepare_to_block(vm_object_t *object,
                                        int *delayed_unlock,
                                        vm_page_t *local_freeq,
                                        int *local_freed, int action) {
  vm_page_unlock_queues();
...
void update_vm_info(void) {
  ...
  vm_pageout_stats[vm_pageout_stat_now].vm_page_swapped_count =
      os_atomic_load(&vm_page_swapped_count, relaxed);
  ...
  vm_pageout_stats[vm_pageout_stat_now].pages_compressed =
      (unsigned int)(tmp64 - last.vm_pageout_compressions);
  ...
  vm_pageout_stats[vm_pageout_stat_now].failed_compressions =
      (unsigned int)(tmp - last.vm_compressor_failed);
  ...
  vm_pageout_stats[vm_pageout_stat_now].pages_grabbed_by_compressor =
      (unsigned int)(tmp64 - last.vm_compressor_pages_grabbed);
  ...
  vm_pageout_stats[vm_pageout_stat_now].phantom_ghosts_found =
      (unsigned int)(tmp - last.vm_phantom_cache_found_ghost);
  ...
  vm_pageout_stats[vm_pageout_stat_now].phantom_ghosts_added =
      (unsigned int)(tmp - last.vm_phantom_cache_added_ghost);
  ...
  vm_pageout_stats[vm_pageout_stat_now].pages_grabbed =
      (unsigned int)(tmp64 - last_vm_page_pages_grabbed);
  ...
  vm_pageout_stats[vm_pageout_stat_now].pages_freed =
      (unsigned int)(tmp - last.vm_page_pages_freed);
  ...
  if (vm_pageout_stats[vm_pageout_stat_now].considered) {
    ...
    vm_pageout_stats[vm_pageout_stat_now].pages_evicted =
        (unsigned int)(tmp - last.vm_pageout_pages_evicted);
    ...
    vm_pageout_stats[vm_pageout_stat_now].pages_purged =
        (unsigned int)(tmp - last.vm_pageout_pages_purged);
    ...
    vm_pageout_stats[vm_pageout_stat_now].freed_speculative =
        (unsigned int)(tmp - last.vm_pageout_freed_speculative);
    ...
    vm_pageout_stats[vm_pageout_stat_now].freed_external =
        (unsigned int)(tmp - last.vm_pageout_freed_external);
    ...
    vm_pageout_stats[vm_pageout_stat_now].inactive_referenced =
        (unsigned int)(tmp - last.vm_pageout_inactive_referenced);
    ...
    vm_pageout_stats[vm_pageout_stat_now].throttled_external_q =
        (unsigned int)(tmp - last.vm_pageout_scan_inactive_throttled_external);
    ...
    vm_pageout_stats[vm_pageout_stat_now].cleaned_dirty_external =
        (unsigned int)(tmp - last.vm_pageout_inactive_dirty_external);
    ...
    vm_pageout_stats[vm_pageout_stat_now].freed_cleaned =
        (unsigned int)(tmp - last.vm_pageout_freed_cleaned);
    ...
    vm_pageout_stats[vm_pageout_stat_now].inactive_nolock =
        (unsigned int)(tmp - last.vm_pageout_inactive_nolock);
    ...
    vm_pageout_stats[vm_pageout_stat_now].skipped_external =
        (unsigned int)(tmp - last.vm_pageout_skipped_external);
    last.vm_pageout_skipped_external = tmp;

    tmp = vm_pageout_vminfo.vm_pageout_skipped_internal;
    vm_pageout_stats[vm_pageout_stat_now].skipped_internal =
        (unsigned int)(tmp - last.vm_pageout_skipped_internal);
    ...
    vm_pageout_stats[vm_pageout_stat_now].reactivation_limit_exceeded =
        (unsigned int)(tmp - last.vm_pageout_reactivation_limit_exceeded);
    ...
    vm_pageout_stats[vm_pageout_stat_now].forced_inactive_reclaim =
        (unsigned int)(tmp - last.vm_pageout_inactive_force_reclaim);
    ...
    vm_pageout_stats[vm_pageout_stat_now].freed_internal =
        (unsigned int)(tmp - last.vm_pageout_freed_internal);
    ...
    vm_pageout_stats[vm_pageout_stat_now].considered_bq_internal =
        (unsigned int)(tmp - last.vm_pageout_considered_bq_internal);
    ...
    vm_pageout_stats[vm_pageout_stat_now].filecache_min_reactivations =
        (unsigned int)(tmp - last.vm_pageout_filecache_min_reactivated);
    ...
    vm_pageout_stats[vm_pageout_stat_now].cleaned_dirty_internal =
        (unsigned int)(tmp - last.vm_pageout_inactive_dirty_internal);
    ...
    vm_pageout_stats[vm_pageout_stat_now].forcereclaimed_sharedcache =
        (unsigned int)(tmp - last.vm_pageout_forcereclaimed_sharedcache);
    ...
    vm_pageout_stats[vm_pageout_stat_now].forcereclaimed_realtime =
        (unsigned int)(tmp - last.vm_pageout_forcereclaimed_realtime);
    ...
    vm_pageout_stats[vm_pageout_stat_now].protected_sharedcache =
        (unsigned int)(tmp - last.vm_pageout_protected_sharedcache);
    ...
    vm_pageout_stats[vm_pageout_stat_now].protected_realtime =
        (unsigned int)(tmp - last.vm_pageout_protected_realtime);
  ...
  KDBG_RELEASE(
      MEMINFO_CODE(DBG_MEMINFO_PGCNT3) | DBG_FUNC_NONE,
      vm_pageout_stats[vm_pageout_stat_now].vm_page_pages_compressed,
      vm_pageout_stats[vm_pageout_stat_now].vm_page_pageable_internal_count,
      vm_pageout_stats[vm_pageout_stat_now].vm_page_pageable_external_count,
      vm_pageout_stats[vm_pageout_stat_now].vm_page_xpmapped_external_count);
  ...
  if (vm_pageout_stats[vm_pageout_stat_now].considered ||
    ...
    KDBG_RELEASE(
        MEMINFO_CODE(DBG_MEMINFO_PGOUT3) | DBG_FUNC_NONE,
        vm_pageout_stats[vm_pageout_stat_now].throttled_internal_q,
        vm_pageout_stats[vm_pageout_stat_now].pages_compressed,
        vm_pageout_stats[vm_pageout_stat_now].pages_grabbed_by_compressor,
        vm_pageout_stats[vm_pageout_stat_now].skipped_external);
    ...
    KDBG_RELEASE(
        MEMINFO_CODE(DBG_MEMINFO_PGOUT4) | DBG_FUNC_NONE,
        vm_pageout_stats[vm_pageout_stat_now].reactivation_limit_exceeded,
        vm_pageout_stats[vm_pageout_stat_now].forced_inactive_reclaim,
        vm_pageout_stats[vm_pageout_stat_now].failed_compressions,
        vm_pageout_stats[vm_pageout_stat_now].freed_internal);
    ...
    KDBG_RELEASE(
        MEMINFO_CODE(DBG_MEMINFO_PGOUT5) | DBG_FUNC_NONE,
        vm_pageout_stats[vm_pageout_stat_now].considered_bq_internal,
        vm_pageout_stats[vm_pageout_stat_now].considered_bq_external,
        vm_pageout_stats[vm_pageout_stat_now].filecache_min_reactivations,
        vm_pageout_stats[vm_pageout_stat_now].cleaned_dirty_internal);
    ...
    KDBG_RELEASE(
        MEMINFO_CODE(DBG_MEMINFO_PGOUT6) | DBG_FUNC_NONE,
        vm_pageout_stats[vm_pageout_stat_now].forcereclaimed_sharedcache,
        vm_pageout_stats[vm_pageout_stat_now].forcereclaimed_realtime,
        vm_pageout_stats[vm_pageout_stat_now].protected_sharedcache,
        vm_pageout_stats[vm_pageout_stat_now].protected_realtime);
 ...
 * DONE_RETURN -- page demand satisfied, work is done -> vm_pageout_scan
 * returns. NEXT_ITERATION -- restart the 'for' loop in vm_pageout_scan aka
 * continue.
 */
 ...
 */
static void vps_deal_with_secluded_page_overflow(vm_page_t *local_freeq,
                                                 int *local_freed) {
#if CONFIG_SECLUDED_MEMORY
  /*
 ...
 */
static void vps_init_page_targets(void) {
  /*
  ...
  vm_pageout_state.vm_page_speculative_target =
      VM_PAGE_SPECULATIVE_TARGET(vm_page_active_count + vm_page_inactive_count);
}
...
static int vps_purge_object() {
  ...
  if (available_for_purge || force_purge) {
    ...
    VM_DEBUG_EVENT(vm_pageout_purgeone, DBG_VM_PAGEOUT_PURGEONE, DBG_FUNC_START,
    ...
    if (vm_purgeable_object_purge_one(force_purge, C_DONT_BLOCK)) {
      ...
      VM_DEBUG_EVENT(vm_pageout_purgeone, DBG_VM_PAGEOUT_PURGEONE, DBG_FUNC_END,
                     vm_page_free_count, 0, 0, 0);
 ...
 * one is empty.
 */
static int vps_age_speculative_queue(boolean_t force_speculative_aging) {
#define DELAY_SPECULATIVE_AGE 1000
  ...
  static int delay_speculative_age =
...
#if DEVELOPMENT || DEBUG
    panic("vm_pageout_scan: vm_page_speculative_count=%d but queues are empty",
          vm_page_speculative_count);
#endif
    /* readjust... */
  ...
  if (vm_page_speculative_count > vm_pageout_state.vm_page_speculative_target ||
    ...
    if (!delay_speculative_age) {
      ...
      ts_fully_aged.tv_sec = (vm_page_max_speculative_age_q *
                              vm_pageout_state.vm_page_speculative_q_age_ms) /
                             1000;
      ts_fully_aged.tv_nsec = ((vm_page_max_speculative_age_q *
                                vm_pageout_state.vm_page_speculative_q_age_ms) %
                               1000) *
                              1000 * NSEC_PER_USEC;
 ...
 */
static int inline vps_object_cache_evict(vm_object_t *object_to_unlock) {
  ...
  if (vm_page_queue_empty(&sq->age_q) && cache_evict_throttle == 0) {
    ...
    if (pages_evicted) {
      ...
      VM_DEBUG_EVENT(vm_pageout_cache_evict, DBG_VM_PAGEOUT_CACHE_EVICT,
                     DBG_FUNC_NONE, vm_page_free_count, pages_evicted,
                     vm_pageout_vminfo.vm_pageout_pages_evicted, 0);
 ...
 */
static void vps_calculate_filecache_min(void) {
...
vps_flow_control_reset_deadlock_timer(struct flow_control *flow_control) {
  ...
  ts.tv_nsec =
      (vm_pageout_state.vm_pageout_deadlock_wait % 1000) * 1000 * NSEC_PER_USEC;
  clock_get_system_nanotime(&sec, &nsec);
 ...
 */
static int vps_flow_control(struct flow_control *flow_control,
                            int *anons_grabbed, vm_object_t *object,
                            int *delayed_unlock, vm_page_t *local_freeq,
                            int *local_freed, int *vm_pageout_deadlock_target,
    ...
    case FCS_DELAYED:
      ...
      if (CMP_MACH_TIMESPEC(&ts, &flow_control->ts) >= 0) {
        ...
        *vm_pageout_deadlock_target =
            vm_pageout_state.vm_pageout_deadlock_relief + vm_page_free_wanted +
  ...
  if (vm_page_free_count >= vm_page_free_target && (vm_page_free_wanted == 0) &&
  ...
  assert_wait_timeout((event_t)&iq->pgo_laundry, THREAD_INTERRUPTIBLE, msecs,
                      1000 * NSEC_PER_USEC);
  ...
  VM_DEBUG_EVENT(vm_pageout_thread_block, DBG_VM_PAGEOUT_THREAD_BLOCK,
                 DBG_FUNC_START, iq->pgo_laundry, iq->pgo_maxlaundry, msecs, 0);
  memoryshot(DBG_VM_PAGEOUT_THREAD_BLOCK, DBG_FUNC_START);

  thread_block(THREAD_CONTINUE_NULL);

  VM_DEBUG_EVENT(vm_pageout_thread_block, DBG_VM_PAGEOUT_THREAD_BLOCK,
                 DBG_FUNC_END, iq->pgo_laundry, iq->pgo_maxlaundry, msecs, 0);
 ...
 * reclaimed.
 */
static int vps_choose_victim_page(vm_page_t *victim_page, int *anons_grabbed,
                                  boolean_t *grab_anonymous,
                                  boolean_t force_anonymous,
                                  boolean_t *is_page_from_bg_q,
  ...
  *is_page_from_bg_q = FALSE;
  ...
  if (vm_page_donate_mode != VM_PAGE_DONATE_DISABLED) {
    if (vm_page_donate_queue_ripe &&
        !vm_page_queue_empty(&vm_page_queue_donate)) {
  ...
  if (vm_page_background_mode != VM_PAGE_BG_DISABLED &&
      (vm_page_background_count > vm_page_background_target)) {
    ...
    if (!VM_PAGE_PAGEABLE(m) || (vm_darkwake_mode && m->vmp_busy)) {
      ...
      if (bg_m_object->internal &&
          (VM_PAGE_Q_THROTTLED(iq) || vm_compressor_out_of_space() == TRUE ||
    ...
    *anons_grabbed = 0;
    ...
    if (VM_CONFIG_SWAP_IS_ACTIVE) {
      ...
      if (vm_page_free_count < (COMPRESSOR_FREE_RESERVED_LIMIT * 2)) {
         ...
         * If we keep going ahead with anonymous pages, we are going to run into
         * a situation where the compressor will be stuck waiting for free pages
         * (if it isn't already).
         *
 ...
 */
static void vps_requeue_page(vm_page_t m, int page_prev_q_state,
 ...
 */
static int vps_switch_object(vm_page_t m, vm_object_t m_object,
                             vm_object_t *object, int page_prev_q_state,
 ...
 */
static void vps_deal_with_throttled_queues(
  ...
  if (cur_object->internal == FALSE) {
     ...
     *  a) The external pageout thread is stuck on the truncate lock for a file
     * that is being extended i.e. written. b) The thread doing the writing is
     * waiting for pages while holding the truncate lock c) Most of the pages in
     * the inactive queue belong to this file.
     *
     ...
     *
     * since we don't know the reason for the external pageout queue being
     * throttled we must suspect that we are deadlocked, so move the current
     * page onto the active queue in an effort to cause a page from the active
     * queue to 'age' to the inactive queue
     *
     * if we don't have jetsam configured (i.e. we have a dynamic pager), set
     * 'force_anonymous' to TRUE to cause us to grab a page from the
     * cleaned/anonymous pool the next time we select a victim page... if we can
     * make enough new free pages, the deadlock will break, the external pageout
     * queue will empty and it will no longer be throttled
     *
     * if we have jetsam configured, keep a count of the pages reactivated this
     * way so that we can try to find clean pages in the active/inactive queues
     * before deciding to jetsam a process
     */
    ...
    *vm_pageout_inactive_external_forced_reactivate_limit -= 1;
    ...
    if (*vm_pageout_inactive_external_forced_reactivate_limit <= 0) {
      *vm_pageout_inactive_external_forced_reactivate_limit =
      ...
      VM_DEBUG_CONSTANT_EVENT(vm_pageout_jetsam, DBG_VM_PAGEOUT_JETSAM,
                              DBG_FUNC_NONE, vm_page_active_count,
                              vm_page_inactive_count, vm_page_free_count,
                              vm_page_free_count);
    }
...
void vm_page_balance_inactive(int max_to_move) {
  vm_page_t m;
  ...
  vm_page_inactive_target =
      VM_PAGE_INACTIVE_TARGET(vm_page_active_count + vm_page_inactive_count +
                              vm_page_speculative_count);

  while (max_to_move-- && (vm_page_inactive_count + vm_page_speculative_count) <
                              vm_page_inactive_target) {
    ...
    /*
     * by not passing in a pmap_flush_context we will forgo any TLB flushing,
     * local or otherwise...
     *
     * a TLB flush isn't really needed here since at worst we'll miss the
     * reference bit being updated in the PTE if a remote processor still has
     * this mapping cached in its TLB when the new reference happens. If no
     * futher references happen on the page after that remote TLB flushes we'll
     * see a clean, non-referenced page when it eventually gets pulled out of
     * the inactive queue by pageout_scan, which is just fine since the last
     * reference would have happened quite far in the past (TLB caches don't
     * hang around for very long), and of course could just as easily have
     * happened before we moved the page
     */
    if (m->vmp_pmapped == TRUE) {
      /*
      ...
      pmap_clear_refmod_options(VM_PAGE_GET_PHYS_PAGE(m), VM_MEM_REFERENCED,
                                PMAP_OPTIONS_NOFLUSH, (void *)NULL);
    }
...
void vm_pageout_scan(void) {
  ...
  struct vm_pageout_queue *eq;
  struct vm_speculative_age_q *sq;
  struct flow_control flow_control = {.state = 0,
                                      .ts = {.tv_sec = 0, .tv_nsec = 0}};
  boolean_t inactive_throttled = FALSE;
  vm_object_t object = NULL; /* object that we're currently working on from
                                previous iterations */
  ...
  reactivate_limit =
      VM_PAGE_REACTIVATE_LIMIT(vm_page_active_count + vm_page_inactive_count);
  ...
  for (;;) {
    ...
    if (lock_yield_check) {
      ...
      if (delayed_unlock++ > delayed_unlock_limit) {
        vm_pageout_prepare_to_block(
            &object, &delayed_unlock, &local_freeq, &local_freed,
            VM_PAGEOUT_PB_CONSIDER_WAKING_COMPACTOR_SWAPPER);
      } else if (vm_pageout_scan_wants_object) {
        vm_page_unlock_queues();
        mutex_pause(0);
        vm_page_lock_queues();
      } else if (vps_yield_for_pgqlockwaiters &&
                 lck_mtx_yield(&vm_page_queue_lock)) {
    ...
    delayed_unlock_limit =
        VM_PAGEOUT_DELAYED_UNLOCK_LIMIT + vm_upl_wait_for_pages;
    ...
    if (vm_page_free_count + local_freed >= vm_page_free_target) {
      ...
      return_from_scan:
        ...
        VM_DEBUG_CONSTANT_EVENT(
            vm_pageout_scan, DBG_VM_PAGEOUT_SCAN, DBG_FUNC_NONE,
            vm_pageout_state.vm_pageout_inactive,
            vm_pageout_state.vm_pageout_inactive_used, 0, 0);
        VM_DEBUG_CONSTANT_EVENT(
            vm_pageout_scan, DBG_VM_PAGEOUT_SCAN, DBG_FUNC_END,
            vm_pageout_vminfo.vm_pageout_freed_speculative,
            vm_pageout_state.vm_pageout_inactive_clean,
            vm_pageout_vminfo.vm_pageout_inactive_dirty_internal,
            vm_pageout_vminfo.vm_pageout_inactive_dirty_external);
     ...
     * vm_pageout_scan iteration and pass that structure to this function
     * instead.
     */
    retval = vps_flow_control(
        &flow_control, &anons_grabbed, &object, &delayed_unlock, &local_freeq,
        &local_freed, &vm_pageout_deadlock_target, inactive_burst_count);
    ...
    vm_pageout_inactive_external_forced_reactivate_limit =
        MIN((vm_page_active_count + vm_page_inactive_count),
            vm_pageout_inactive_external_forced_reactivate_limit);
    loop_count++;
    ...
    m = NULL;
    retval = vps_choose_victim_page(&m, &anons_grabbed, &grab_anonymous,
    ...
    if (m_object != object) {
       ...
       * and then try to acquire the 'm_object' lock. So 'object' has to point
       * to either 'm_object' or NULL.
       */
      retval = vps_switch_object(m, m_object, &object, page_prev_q_state,
                                 avoid_anon_pages, page_from_bg_q);
    ...
    if (vm_pageout_state.vm_page_xpmapped_min_divisor == 0) {
      ...
      vm_pageout_state.vm_page_xpmapped_min =
          (vm_page_pageable_external_count * 10) /
          vm_pageout_state.vm_page_xpmapped_min_divisor;
    }

    if (!m->vmp_no_cache && page_from_bg_q == FALSE &&
      ...
      if (++reactivated_this_call >= reactivate_limit &&
          ...
          !((m->vmp_realtime || object->for_realtime) &&
        ...
        } else if (m->vmp_realtime || object->for_realtime) {
      ...
      reactivate_page:
        if (!object->internal && object->pager != MEMORY_OBJECT_NULL &&
            vnode_pager_get_isinuse(object->pager, &isinuse) == KERN_SUCCESS &&
            !isinuse) {
          /*
  ...
  throttle_inactive:
    if (!VM_DYNAMIC_PAGING_ENABLED() && object->internal && m->vmp_dirty &&
    ...
    if (m->vmp_pmapped == TRUE) {
       ...
       */
      if (!VM_CONFIG_COMPRESSOR_IS_ACTIVE || object->internal == FALSE) {
        pmap_options = 0;
      ...
      refmod_state =
          pmap_disconnect_options(VM_PAGE_GET_PHYS_PAGE(m), pmap_options, NULL);
      if (refmod_state & VM_MEM_MODIFIED) {
        SET_PAGE_DIRTY(m, FALSE);
      }
    }
...
void vm_page_free_reserve(int pages) {
  ...
  free_after_reserve =
      vm_pageout_state.vm_page_free_count_init - vm_page_free_reserved;
  ...
  vm_page_free_target =
      vm_page_free_reserved + VM_PAGE_FREE_TARGET(free_after_reserve);
...
#if XNU_TARGET_OS_OSX
kern_return_t vm_pageout_wait(uint64_t deadline) {
  kern_return_t kr;

  vm_free_page_lock();
  for (kr = KERN_SUCCESS; vm_pageout_running && (KERN_SUCCESS == kr);) {
    vm_pageout_waiter = TRUE;
    if (THREAD_AWAKENED != lck_mtx_sleep_deadline(&vm_page_queue_free_lock,
                                                  LCK_SLEEP_DEFAULT,
                                                  (event_t)&vm_pageout_waiter,
...
OS_NORETURN
static void
vm_pageout_iothread_external_continue(struct pgo_iothread_state *ethr,
  ...
  while (true) {
    ...
    while (!vm_page_queue_empty(&q->pgo_pending)) {
      ...
      if (m == NULL || m->vmp_busy || m->vmp_cleaning || !m->vmp_laundry ||
          (m->vmp_q_state != VM_PAGE_NOT_ON_Q)) {
        /*
       ...
       */
      memory_object_data_return(pager, m->vmp_offset + object->paging_offset,
                                PAGE_SIZE, NULL, NULL, FALSE, FALSE, 0);
    ...
    sched_cond_wait_parameter(
        &(ethr->pgo_wakeup), THREAD_UNINT,
        (thread_continue_t)vm_pageout_iothread_external_continue, ethr);
  }
  /*NOTREACHED*/
}

uint32_t
    vm_compressor_time_thread; /* Set via sysctl 'vm.compressor_timing_enabled'
                                  to record time accrued by this thread. */

#if DEVELOPMENT || DEBUG
static void vm_pageout_record_thread_time(int cqid, int ncomps) {
...
static void *vm_pageout_select_filling_chead(struct pgo_iothread_state *cq,
                                             vm_page_t m) {
  /*
   * Technically we need the pageq locks to manipulate the vmp_on_specialq
   * field. However, this page has been removed from all queues and is only
...
OS_NORETURN
static void vm_pageout_iothread_internal_continue(struct pgo_iothread_state *cq,
                                                  __unused wait_result_t w) {
  struct vm_pageout_queue *q;
  vm_page_t m = NULL;
  ...
  while (true) { /* this top loop is for the compressor_running_perf_test
                    running a full speed without blocking */
#if DEVELOPMENT || DEBUG
    bool benchmark_accounting = false;
    /* If we're running the compressor perf test, only process the benchmark
     * pages. We'll get back to our regular queue once the benchmark is done */
    ...
    if (vm_compressor_ebound &&
      ...
      local_batch_size =
          q->pgo_maxlaundry / (vm_pageout_state.vm_compressor_thread_count * 2);
    }
#else
    local_batch_size =
        q->pgo_maxlaundry / (vm_pageout_state.vm_compressor_thread_count * 2);
...
#endif
    while (true) { /* this loop is for working though all the pages in the
                      pending queue */
      ...
      /* empty the entire content of the thread input q to local_q, but not more
       * than local_batch_size pages */
      while (!vm_page_queue_empty(&q->pgo_pending) &&
             local_cnt < local_batch_size) {
        vm_page_queue_remove_first(&q->pgo_pending, m, vmp_pageq);
        assert(m->vmp_q_state == VM_PAGE_ON_PAGEOUT_Q);
        VM_PAGE_CHECK(m);
       ...
       * disabled when recording data since record data is not protected with a
       * mutex so this may cause races */
      if (pages_left_on_q >= local_batch_size &&
          cq->id < (vm_pageout_state.vm_compressor_thread_count - 1)) {
      ...
      while (local_q) {
        ...
        if (vm_pageout_compress_page(chead, cq->scratch_buf, m) ==
            KERN_SUCCESS) {
#if DEVELOPMENT || DEBUG
          ncomps++;
#endif
          ...
          if (local_freed >= MAX_FREE_BATCH) {
            OSAddAtomic64(local_freed,
                          &vm_pageout_vminfo.vm_pageout_compressions);
        ...
        num_pages_processed++;
#endif             /* DEVELOPMENT || DEBUG */
#if !CONFIG_JETSAM /* Maybe: if there's no JETSAM, be more proactive in waking \
                      up anybody that needs free pages */
        while (vm_page_free_count < COMPRESSOR_FREE_RESERVED_LIMIT) {
          ...
          if (vm_page_free_count < COMPRESSOR_FREE_RESERVED_LIMIT) {
            ...
            wait_result = assert_wait((event_t)&vm_page_free_wanted_privileged,
                                      THREAD_UNINT);
    ...
    if (marked_active == TRUE) {
      ...
      if (vmct_active == 0) {
         ...
         */
        vmct_stats.vmct_cthreads_total +=
            vm_compressor_epoch_stop - vm_compressor_epoch_start;
      }
    }
    ...
    sched_cond_wait_parameter(
        &(cq->pgo_wakeup), THREAD_UNINT,
        (thread_continue_t)vm_pageout_iothread_internal_continue, (void *)cq);
  }
  /*NOTREACHED*/
}

/* resolves the pager and maintain stats in the pager and in the vm_object */
kern_return_t vm_pageout_compress_page(void **current_chead, char *scratch_buf,
                                       vm_page_t m) {
  vm_object_t object;
  memory_object_t pager;
  ...
  retval = vm_compressor_pager_put(pager, m->vmp_offset + object->paging_offset,
                                   VM_PAGE_GET_PHYS_PAGE(m), current_chead,
                                   scratch_buf, &compressed_count_delta, flags);
  ...
  vm_compressor_pager_count(pager, compressed_count_delta,
                            FALSE, /* shared_lock */
                            object);
  ...
  if (retval == KERN_SUCCESS) {
    ...
    if ((object->purgable != VM_PURGABLE_DENY || object->vo_ledger_tag) &&
      ...
      vm_object_owner_compressed_update(object, compressed_count_delta);
    }
...
static void vm_pageout_adjust_eq_iothrottle(struct pgo_iothread_state *ethr,
  ...
  if (ethr->q->pgo_inited == TRUE &&
    ...
    proc_set_thread_policy(ethr->pgo_iothread, TASK_POLICY_EXTERNAL,
                           TASK_POLICY_IO, policy);
...
OS_NORETURN
static void vm_pageout_iothread_external(struct pgo_iothread_state *ethr,
                                         __unused wait_result_t w) {
  ...
  proc_set_thread_policy(self, TASK_POLICY_EXTERNAL, TASK_POLICY_IO,
...
OS_NORETURN
static void vm_pageout_iothread_internal(struct pgo_iothread_state *cthr,
                                         __unused wait_result_t w) {
  thread_t self = current_thread();
...
#if DEVELOPMENT || DEBUG
  vm_pageout_queue_benchmark.pgo_lowpriority =
      vm_pageout_queue_internal.pgo_lowpriority;
...
kern_return_t vm_set_buffer_cleanup_callout(boolean_t (*func)(int)) {
  if (OSCompareAndSwapPtr(NULL, ptrauth_nop_cast(void *, func),
...
/*
 *  This is how the long the system waits 'stuck' in an unchanged non-normal
 * pressure level before resending out notifications for that level again.
 */
int vm_pressure_level_transition_threshold =
    PRESSURE_LEVEL_STUCK_THRESHOLD_MINS;

void vm_pressure_response() {
  ...
  abs_time_since_level_transition =
      curr_ts - vm_pressure_last_level_transition_abs;
  ...
  if (new_level != -1 || force_check) {
    if (new_level != -1) {
      ...
      if (new_level != (int)old_level) {
        VM_DEBUG_CONSTANT_EVENT(vm_pressure_level_change,
                                DBG_VM_PRESSURE_LEVEL_CHANGE, DBG_FUNC_NONE,
    ...
    if (hibernation_vmqueues_inspection || hibernate_cleaning_in_progress) {
       ...
       * because that could collide with checks for non-monotonicity in the
       * scheduler. We do however do all the updates to
       * memorystatus_vm_pressure_level because we _might_ want to use that for
       * decisions regarding which pages or how many pages we want to dump in
       * hibernation.
       */
      return;
    }
    ...
    if ((memorystatus_vm_pressure_level != kVMPressureNormal) ||
        (old_level != memorystatus_vm_pressure_level) || force_check) {
      ...
      vm_pressure_last_level_transition_abs =
          curr_ts; /* renew the window of observation for a stuck pressure level
                    */
    }
  }
}
 ...
 */
kern_return_t mach_vm_wire_level_monitor(int64_t requested_pages) {
...
/*
 * Function called by a kernel thread to either get the current pressure level
 * or wait until memory pressure changes from a given level.
 */
kern_return_t mach_vm_pressure_level_monitor(boolean_t wait_for_pressure,
 ...
 */
void compute_pageout_gc_throttle(__unused void *arg) {
  if (vm_pageout_vminfo.vm_pageout_considered_page !=
      vm_pageout_state.vm_pageout_considered_page_last) {
    vm_pageout_state.vm_pageout_considered_page_last =
        vm_pageout_vminfo.vm_pageout_considered_page;
    sched_cond_signal(&vm_pageout_gc_cond, vm_pageout_gc_thread);
  }
 ...
 */
__dead2 void vm_pageout_garbage_collect(void *step, wait_result_t wr __unused) {
  ...
  while (true) {
    if (step == VM_PAGEOUT_GC_INIT) {
      ...
      vm_deferred_reclamation_gc(RECLAIM_GC_TRIM, &bytes_reclaimed,
      ...
      do {
        ...
        first_try = FALSE;
      } while (buf_large_zfree == TRUE &&
               vm_page_free_count < vm_page_free_target);

      consider_machine_adjust();
    }

    sched_cond_wait_parameter(&vm_pageout_gc_cond, THREAD_UNINT,
                              vm_pageout_garbage_collect,
                              VM_PAGEOUT_GC_COLLECT);
  }
...
void vm_set_restrictions(unsigned int num_cpus) {
  int vm_restricted_to_single_processor = 0;

  if (PE_parse_boot_argn("vm_restricted_to_single_processor",
                         &vm_restricted_to_single_processor,
                         sizeof(vm_restricted_to_single_processor))) {
    kprintf("Overriding vm_restricted_to_single_processor to %d\n",
            vm_restricted_to_single_processor);
    vm_pageout_state.vm_restricted_to_single_processor =
        (vm_restricted_to_single_processor ? TRUE : FALSE);
  } else {
    assert(num_cpus > 0);
 ...
 */
__startup_func void vm_config_init(void) {
  bzero(&vm_config, sizeof(vm_config));

  switch (vm_compressor_mode) {
  case VM_PAGER_DEFAULT:
    printf("mapping deprecated VM_PAGER_DEFAULT to "
  ...
  case VM_PAGER_FREEZER_DEFAULT:
    printf("mapping deprecated VM_PAGER_FREEZER_DEFAULT to "
...
__startup_func static void vm_pageout_create_gc_thread(void) {
  thread_t thread;

  sched_cond_init(&vm_pageout_gc_cond);
  if (kernel_thread_create(vm_pageout_garbage_collect, VM_PAGEOUT_GC_INIT,
                           BASEPRI_DEFAULT, &thread) != KERN_SUCCESS) {
...
void vm_pageout(void) {
  ...
  PE_parse_boot_argn("vmpgo_protect_realtime", &vm_pageout_protect_realtime,
  ...
  if (vm_pageout_state.vm_pageout_burst_inactive_throttle == 0) {
    vm_pageout_state.vm_pageout_burst_inactive_throttle =
  ...
  /* in external_state these cheads are never used, they are used only in
   * internal_state for te compressor */
  ...
  result = kernel_thread_start_priority(
      (thread_continue_t)vm_pageout_iothread_external, (void *)ethr, BASEPRI_VM,
      &(ethr->pgo_iothread));
...
#if VM_PRESSURE_EVENTS
  result = kernel_thread_start_priority((thread_continue_t)vm_pressure_thread,
                                        NULL, BASEPRI_DEFAULT, &thread);
  ...
  pmap_protect(kernel_pmap, vm_page_fake_buckets_start,
               vm_page_fake_buckets_end, VM_PROT_READ);
...
//	vm_tests();
   ...
   * Unreached code!
   *
   * The vm_pageout_continue() call above never returns, so the code below is
   * never executed.  We take advantage of this to declare several DTrace VM
   * related probe points that our kernel doesn't have an analog for.  These are
   * probe points that exist in Solaris and are in the DTrace documentation, so
   * people may have written scripts that use them.  Declaring the probe points
   * here means their scripts will compile and execute which we want for
   * portability of the scripts, but since this section of code is never
   * reached, the probe points will simply never fire.  Yes, this is basically a
   * hack.  The problem is the DTrace probe points were chosen with Solaris
   * specific VM events in mind, not portability to different VM
   * implementations.
   */
...
kern_return_t vm_pageout_internal_start(void) {
...
#define BSD_HOST 1
  ...
  PE_parse_boot_argn("vmcomp_threads",
                     &vm_pageout_state.vm_compressor_thread_count,
  ...
  if (vm_pageout_state.vm_compressor_thread_count <= 0) {
    vm_pageout_state.vm_compressor_thread_count = 1;
  } else if (vm_pageout_state.vm_compressor_thread_count >
...
#if DEVELOPMENT || DEBUG
  // Note: this will be modified at enqueue-time such that the benchmark queue
  // is never throttled
  vm_pageout_queue_benchmark.pgo_maxlaundry =
      vm_pageout_queue_internal.pgo_maxlaundry;
#endif /* DEVELOPMENT || DEBUG */
  ...
  for (int i = 0; i < vm_pageout_state.vm_compressor_thread_count; i++) {
    ...
    result = kernel_thread_start_priority(
        (thread_continue_t)vm_pageout_iothread_internal, (void *)iq, BASEPRI_VM,
        &(iq->pgo_iothread));
...
#if CONFIG_IOSCHED
/*
 * To support I/O Expedite for compressed files we mark the upls with special
 * flags. The way decmpfs works is that we create a big upl which marks all the
 * pages needed to represent the compressed file as busy. We tag this upl with
 * the flag UPL_DECMP_REQ. Decmpfs then issues smaller I/Os for compressed I/Os,
 * deflates them and puts the data into the pages being held in the big original
 * UPL. We mark each of these smaller UPLs with the flag UPL_DECMP_REAL_IO. Any
 * outstanding real I/O UPL is tracked by the big req upl using the decmp_io_upl
 * field (in the upl structure). This link is protected in the forward direction
 * by the req upl lock (the reverse link doesnt need synch. since we never
 * inspect this link unless the real I/O upl is being destroyed).
 */

static void upl_set_decmp_info(upl_t upl, upl_t src_upl) {
...
static upl_t upl_create(int type, int flags, upl_size_t size) {
   ...
   */
  upl =
      kalloc_type(struct upl, struct upl_page_info,
  ...
  if ((type & UPL_CREATE_INTERNAL) && (type & UPL_CREATE_EXPEDITE_SUP)) {
    ...
    upl->upl_reprio_info =
        kalloc_data(sizeof(uint64_t) * pages, Z_WAITOK | Z_ZERO);
...
static void upl_destroy(upl_t upl) {
  uint32_t pages;

  //	DEBUG4K_UPL("upl %p (u_offset 0x%llx u_size 0x%llx) object %p\n", upl,
  //(uint64_t)upl->u_offset, (uint64_t)upl->u_size, upl->map_object);
  ...
  if (upl->flags & UPL_HAS_FS_VERIFY_INFO) {
    assert(upl->u_fs_un.verify_info &&
           ...
           upl->u_fs_un.verify_info->verify_data_len <=
               upl_adjusted_size(upl, PAGE_MASK));
...
void upl_unmark_decmp(upl_t upl) {
...
static boolean_t must_throttle_writes() {
  if (VM_PAGE_Q_BACKING_UP(&vm_pageout_queue_external) &&
      vm_page_pageable_external_count >
          (AVAILABLE_NON_COMPRESSED_MEMORY * 6) / 10) {
    /*
...
__startup_func static void vm_page_delayed_work_init_ctx(void) {
...
struct vm_page_delayed_work *vm_page_delayed_work_get_ctx(void) {
...
void vm_page_delayed_work_finish_ctx(struct vm_page_delayed_work *dwp) {
 ...
 */
OS_NOINLINE OS_NOT_TAIL_CALLED static void
...
vm_object_upl_request(vm_object_t object, vm_object_offset_t offset,
                      upl_size_t size, upl_t *upl_ptr,
                      upl_page_info_array_t user_page_list,
                      unsigned int *page_list_count,
                      upl_control_flags_t cntrl_flags, vm_tag_t tag) {
  ...
  assertf(page_aligned(offset) && page_aligned(size), "offset 0x%llx size 0x%x",
          offset, size);

  VM_DEBUG_CONSTANT_EVENT(vm_object_upl_request, DBG_VM_UPL_REQUEST,
                          DBG_FUNC_START, size, cntrl_flags, 0, 0);
  ...
  if (cntrl_flags & UPL_SET_INTERNAL) {
    if (cntrl_flags & UPL_SET_LITE) {
      upl = upl_create(UPL_CREATE_INTERNAL | UPL_CREATE_LITE | io_tracking_flag,
                       0, size);
    } else {
    ...
    if (cntrl_flags & UPL_SET_LITE) {
      upl = upl_create(UPL_CREATE_EXTERNAL | UPL_CREATE_LITE | io_tracking_flag,
                       0, size);
    } else {
  ...
  *upl_ptr = upl;
  ...
  if (cntrl_flags & UPL_SET_LITE) {
    ...
    assertf(page_aligned(upl->map_object->vo_shadow_offset),
            ...
            upl->map_object->vo_shadow_offset);
  ...
  if ((cntrl_flags & UPL_WILL_MODIFY) && object->vo_copy != VM_OBJECT_NULL) {
     ...
     */
    vm_object_update(object, offset, size, NULL, NULL,
                     FALSE, /* should_return */
                     MEMORY_OBJECT_COPY_SYNC, VM_PROT_NO_CHANGE);
  ...
  while (xfer_size) {
    ...
    if (cntrl_flags & UPL_COPYOUT_FROM) {
      ...
      if (((dst_page = vm_page_lookup(object, dst_offset)) == VM_PAGE_NULL) ||
          vm_page_is_fictitious(dst_page) || dst_page->vmp_absent ||
          VMP_ERROR_GET(dst_page) || dst_page->vmp_cleaning ||
      ...
      if (cntrl_flags & UPL_RET_ONLY_DIRTY) {
        ...
        if (dst_page->vmp_laundry || !(cntrl_flags & UPL_FOR_PAGEOUT)) {
           ...
           */
          if (dst_page->vmp_dirty || dst_page->vmp_precious ||
         ...
         */
        if ((hibernate_cleaning_in_progress == TRUE ||
             (!((refmod_state & VM_MEM_REFERENCED) ||
                dst_page->vmp_reference) ||
              (dst_page->vmp_q_state == VM_PAGE_ON_THROTTLED_Q))) &&
            ((refmod_state & VM_MEM_MODIFIED) || dst_page->vmp_dirty ||
             dst_page->vmp_precious)) {
          goto check_busy;
        }
      dont_return:
        /*
    ...
    check_busy:
      ...
      if (cntrl_flags & UPL_SET_LITE) {
        ...
        if (hw_dirty) {
          ...
          pmap_clear_refmod_options(phys_page, VM_MEM_MODIFIED,
      ...
      while ((cntrl_flags & UPL_WILL_MODIFY) &&
        ...
        last_copy_version = object->vo_copy_version;
        if (object->vo_copy != VM_OBJECT_NULL) {
          vm_object_update(object, dst_offset, /* current offset */
                           xfer_size,          /* remaining size */
                           NULL, NULL, FALSE,  /* should_return */
                           MEMORY_OBJECT_COPY_SYNC, VM_PROT_NO_CHANGE);
      ...
      if (dst_page != VM_PAGE_NULL) {
        ...
        if (object->scan_collisions) {
          ...
          if (dst_page != VM_PAGE_NULL) {
            vm_page_release(dst_page, VMP_RELEASE_NONE);
          }
        ...
        if (dst_page == VM_PAGE_NULL) {
          if ((cntrl_flags & (UPL_RET_ONLY_ABSENT | UPL_NOBLOCK)) ==
              (UPL_RET_ONLY_ABSENT | UPL_NOBLOCK)) {
            /*
             * we don't want to stall waiting for pages to come onto the free
             * list while we're already holding absent pages in this UPL the
             * caller will deal with the empty slots
             */
            if (user_page_list) {
              user_page_list[entry].phys_addr = 0;
            }
          ...
          VM_DEBUG_EVENT(vm_upl_page_wait, DBG_VM_UPL_PAGE_WAIT, DBG_FUNC_START,
                         vm_upl_wait_for_pages, 0, 0, 0);

          VM_PAGE_WAIT();
          OSAddAtomic(-size_in_pages, &vm_upl_wait_for_pages);

          VM_DEBUG_EVENT(vm_upl_page_wait, DBG_VM_UPL_PAGE_WAIT, DBG_FUNC_END,
                         vm_upl_wait_for_pages, 0, 0, 0);
      ...
      if (dst_page->vmp_pmapped) {
        ...
        if (__improbable(PMAP_PAGE_IS_USER_EXECUTABLE(dst_page))) {
          /*
           * Various buffer cache operations may need to reload the page
           * contents even though the page may have an executable frame type
           * from prior use of the vnode associated with the VM object.  For
           * those cases, we need to disconnect all mappings and reset the frame
           * type, regardless of whether UPL_FILE_IO was passed here, as the
           * SPTM will not allow writable CPU or IOMMU mappings of exec-typed
           * pages. NOTE: It's theoretically possible that the retype here could
           * race with setup/teardown of IOMMU mappings by another thread that
           * went through the vm_object_iopl_request() path.  I'm not sure that
           * would ever be expected to happen for an exec page in practice
           * though.  If it does happen, we may need to change
           * vm_page_do_delayed_work() to forbid all IOPLs against executable
           * pages rather than only writable ones.
           */
          refmod_state =
              pmap_disconnect_options(phys_page, PMAP_OPTIONS_RETYPE, NULL);
        } else
#endif /* CONFIG_SPTM */
          if (!(cntrl_flags & UPL_FILE_IO)) {
            /*
    ...
    if (user_page_list) {
      ...
      if (dst_page->vmp_clustered == TRUE) {
        user_page_list[entry].speculative =
            (dst_page->vmp_q_state == VM_PAGE_ON_SPECULATIVE_Q) ? TRUE : FALSE;
      } else {
  ...
  VM_DEBUG_CONSTANT_EVENT(vm_object_upl_request, DBG_VM_UPL_REQUEST,
                          DBG_FUNC_END, page_grab_count, 0, 0, 0);
  if (task != NULL) {
...
/**
 * Helper for determining whether a writable (!UPL_COPYOUT_FROM) UPL is allowed
 * for a given VA region. This is determined not only by the allowed permissions
 * in the relevant vm_map_entry, but also by the code integrity enforcement
 * model present on the system.
 *
 * @param map VM map against which the UPL is being populated.
 * @param entry The source vm_map_entry in [map] against which the UPL is being
 * populated.
 ...
 */
static bool vme_allows_upl_write(vm_map_t map __unused, vm_map_entry_t entry,
...
#if CONFIG_SPTM
  /*
   * For SPTM configurations, reject any attempt to create a writable UPL
   * against any executable region.  Even in cases such as JIT/USER_DEBUG in
   * which the vm_map_entry may allow write access, the SPTM/TXM codesigning
   * model still forbids writable DMA mappings of these pages.
   */
...
/**
 * Helper for determining whether a read-only (UPL_COPYOUT_FROM) UPL is allowed
 * for a given VA region, possibly with the additional requirement of creating a
 * kernel copy of the source buffer. This is determined by the code integrity
 * enforcement model present on the system.
 *
 * @param map VM map against which the UPL is being populated.
 * @param entry The source vm_map_entry in [map] against which the UPL is being
 * populated.
 * @param offset Base offset of UPL request in [map], for debugging purposes.
 * @param copy_required Output parameter indicating whether the UPL should be
 * created against a kernel copy of the source data.
 *
 * @return True if the read-only UPL is allowed for [entry], false otherwise.
 */
static bool vme_allows_upl_read(vm_map_t map __unused,
...
#if CONFIG_SPTM
  /*
   * For SPTM configs, always create a copy when attempting a read-only I/O
   * operation against an executable or debug (which may become executable)
   * mapping.  The SPTM's stricter security enforcements against DMA mappings of
   * executable pages may otherwise trigger an SPTM violation panic.  We expect
   * the added cost of this copy to be manageable as DMA mappings of executable
...
#if !XNU_TARGET_OS_OSX
  /*
   * For all non-Mac targets, create a copy when attempting a read-only I/O
   * operation against a read-only executable region.  These regions are likely
   * to be codesigned and are typically mapped CoW; our wire operation will be
   * treated as a proactive CoW fault which will copy the backing pages and thus
   * cause them to no longer be codesigned.
   */
  if (map->pmap != kernel_pmap && (entry->protection & VM_PROT_EXECUTE) &&
...
kern_return_t vm_map_create_upl(vm_map_t map, vm_map_address_t offset,
                                upl_size_t *upl_size, upl_t *upl,
                                upl_page_info_array_t page_list,
                                unsigned int *count, upl_control_flags_t *flags,
                                vm_tag_t tag) {
...
REDISCOVER_ENTRY:
  ...
  if (VM_MAP_PAGE_SHIFT(map) < PAGE_SHIFT) {
    DEBUG4K_UPL("map %p (%d) offset 0x%llx size 0x%x flags 0x%llx\n", map,
                VM_MAP_PAGE_SHIFT(map), (uint64_t)offset, *upl_size, *flags);
  }
  ...
  if (!entry->is_sub_map) {
    ...
    if (VME_OBJECT(entry) == VM_OBJECT_NULL) {
      ...
      VME_OBJECT_SET(entry,
                     vm_object_allocate((vm_size_t)vm_object_round_page((
    ...
    if (((caller_flags & UPL_COPYOUT_FROM) &&
         !vme_allows_upl_read(map, entry, offset, &copy_required)) ||
        (!(caller_flags & UPL_COPYOUT_FROM) &&
         !vme_allows_upl_write(map, entry, offset))) {
      vm_map_unlock_read(map);
      ret = KERN_PROTECTION_FAILURE;
      goto done;
    }
  ...
  if (__improbable(copy_required)) {
    ...
    kaddr = 0;
    ret = kmem_alloc(kernel_map, &kaddr, ksize, KMA_PAGEABLE | KMA_DATA_SHARED,
                     tag);
    if (ret == KERN_SUCCESS) {
    ...
    if (ret == KERN_SUCCESS) {
      if (ksize > *upl_size) {
        ...
        memset((void *)(kaddr + *upl_size), 0, ksize - *upl_size);
      }
      /* create the UPL from the kernel buffer */
      ret = vm_map_create_upl(kernel_map, (vm_map_address_t)kaddr, upl_size,
...
#if DEVELOPMENT || DEBUG
    DTRACE_VM4(create_upl_from_executable, vm_map_t, map, vm_map_address_t,
               offset, upl_size_t, *upl_size, kern_return_t, ret);
#endif /* DEVELOPMENT || DEBUG */
    goto done;
  }
  ...
  if (!entry->is_sub_map && !entry->needs_copy && *upl_size != 0 &&
    ...
    vm_object_pmap_protect(
        local_object, VME_OFFSET(entry), entry->vme_end - entry->vme_start,
        ((entry->is_shared || map->mapped_in_other_pmaps) ? PMAP_NULL
                                                          : map->pmap),
        VM_MAP_PAGE_SIZE(map), entry->vme_start, prot);
  ...
  if (entry->needs_copy) {
    ...
    if (vm_map_lookup_and_lock_object(&local_map, offset, fault_type,
                                      OBJECT_LOCK_EXCLUSIVE, &version, &object,
                                      &new_offset, &prot, &wired, NULL,
  ...
  if (entry->is_sub_map) {
    ...
    DEBUG4K_UPL("map %p offset 0x%llx (0x%llx) size 0x%x (adjusted 0x%llx "
                "original 0x%llx) submap %p\n",
                map, (uint64_t)offset, (uint64_t)original_offset, *upl_size,
                (uint64_t)adjusted_size, (uint64_t)original_size, submap);
  ...
  if (sync_cow_data &&
      (VME_OBJECT(entry)->shadow || VME_OBJECT(entry)->vo_copy)) {
    local_object = VME_OBJECT(entry);
    local_start = entry->vme_start;
    local_offset = (vm_map_offset_t)VME_OFFSET(entry);
    ...
    if (local_object->shadow && local_object->vo_copy) {
      vm_object_lock_request(
          ...
          *upl_size, FALSE, MEMORY_OBJECT_DATA_SYNC, VM_PROT_NO_CHANGE);
    }
    sync_cow_data = FALSE;
  ...
  if (force_data_sync) {
    ...
    vm_object_lock_request(
        local_object,
        ((vm_object_offset_t)((offset - local_start) + local_offset)),
        (vm_object_size_t)*upl_size, FALSE, MEMORY_OBJECT_DATA_SYNC,
        VM_PROT_NO_CHANGE);
...
#endif /* MACH_ASSERT */
    DTRACE_VM2(cs_executable_create_upl, uint64_t, (uint64_t)entry->vme_start,
  ...
  if (local_object->true_share) {
    ...
    if (!local_object->true_share && vm_object_tracking_btlog) {
    ...
    if (local_object->copy_strategy == MEMORY_OBJECT_COPY_SYMMETRIC) {
  ...
  ret = vm_object_iopl_request(
      local_object,
      ((vm_object_offset_t)((offset - local_start) + local_offset)), *upl_size,
      upl, page_list, count, caller_flags, tag);
  vm_object_deallocate(local_object);
 ...
 * vm_map_enter() API.
 */
kern_return_t vm_map_enter_upl_range(vm_map_t map, upl_t upl,
  ...
  DEBUG4K_UPL("map %p upl %p flags 0x%x object %p offset 0x%llx (uploff: "
              "0x%llx) size 0x%lx (uplsz: 0x%x) \n",
              map, upl, upl->flags, upl->map_object, offset_to_map,
              upl->u_offset, size_to_map, upl->u_size);
  ...
  if ((isVectorUPL = vector_upl_is_valid(upl))) {
    ...
    if (mapped) {
      if (mapped != valid_upls) {
        panic("Only %d of the %d sub-upls within the Vector UPL are alread "
              "mapped",
              mapped, valid_upls);
      } else {
    ...
    if (kr2 != KERN_SUCCESS) {
      os_log(OS_LOG_DEFAULT, "%s: vm_allocate(0x%x) -> %d", __func__,
             vector_upl->u_size, kr2);
      upl_unlock(vector_upl);
      return kr2;
    }
...
process_upl_to_enter:
  if (isVectorUPL) {
    ...
    *dst_addr =
        (vm_map_offset_t)(vector_upl_dst_addr + (vm_map_offset_t)subupl_offset);
  } else {
    /*
  ...
  if ((!(upl->flags & UPL_SHADOWED)) &&
    ...
    upl->map_object->copy_strategy = MEMORY_OBJECT_COPY_NONE;
    upl->map_object->vo_shadow_offset =
        upl_adjusted_offset(upl, PAGE_MASK) - object->paging_offset;
    assertf(page_aligned(upl->map_object->vo_shadow_offset),
            "object %p shadow_offset 0x%llx", upl->map_object,
    ...
    while (size) {
      ...
      if (bitmap_test(upl->lite_list, pg_num)) {
        ...
        vm_page_insert_wired(alias_page, upl->map_object, new_offset,
                             VM_KERN_MEMORY_NONE);
  ...
  if (upl->flags & UPL_SHADOWED) {
    ...
    offset = upl_adjusted_offset(upl, VM_MAP_PAGE_MASK(map)) -
             upl->map_object->paging_offset;
    if (!isVectorUPL) {
      offset += offset_to_map;
    }
  }
  ...
  if (!isVectorUPL) {
    *dst_addr = 0;
     ...
     */
    kr = vm_map_enter(
        map, dst_addr, (vm_map_size_t)size, (vm_map_offset_t)0,
        VM_MAP_KERNEL_FLAGS_DATA_SHARED_ANYWHERE(.vm_tag =
                                                     VM_KERN_MEMORY_OSFMK),
        upl->map_object, offset, FALSE, prot_to_map, VM_PROT_ALL,
        VM_INHERIT_DEFAULT);
  ...
  upl->u_mapped_size =
      (upl_size_t)size; /* When we allow multiple submappings of the UPL */
  ...
  for (addr = *dst_addr; size > 0; size -= PAGE_SIZE, addr += PAGE_SIZE) {
    ...
    if (m) {
      ...
      kr =
          pmap_enter_check(map->pmap, addr, m, prot_to_map, VM_PROT_NONE, TRUE);
  ...
  if (!isVectorUPL) {
    ...
    addr_adjustment =
        (vm_map_offset_t)(upl->u_offset -
                          upl_adjusted_offset(upl, VM_MAP_PAGE_MASK(map)));
    if (addr_adjustment) {
      DEBUG4K_UPL("dst_addr 0x%llx (+ 0x%llx) -> 0x%llx\n", (uint64_t)*dst_addr,
                  (uint64_t)addr_adjustment,
                  (uint64_t)(*dst_addr + addr_adjustment));
...
kern_return_t vm_map_enter_upl(vm_map_t map, upl_t upl,
                               vm_map_offset_t *dst_addr) {
  upl_size_t upl_size = upl_adjusted_size(upl, VM_MAP_PAGE_MASK(map));
  return vm_map_enter_upl_range(map, upl, 0, upl_size, VM_PROT_DEFAULT,
                                dst_addr);
}
 ...
 */
kern_return_t
vm_map_remove_upl_range(vm_map_t map, upl_t upl,
...
process_upl_to_remove:
    ...
    if (curr_upl == vector_upl_max_upls(vector_upl)) {
      ...
      if (kr != KERN_SUCCESS) {
        os_log(OS_LOG_DEFAULT, "%s: vm_deallocate(0x%llx, 0x%x) -> %d",
               __func__, (uint64_t)v_upl_dst_addr, vector_upl->u_size, kr);
      }
      v_upl_dst_addr = 0;
  ...
  if (upl->flags & UPL_PAGE_LIST_MAPPED) {
    ...
    vm_map_remove(map, vm_map_trunc_page(addr, VM_MAP_PAGE_MASK(map)),
...
kern_return_t vm_map_remove_upl(vm_map_t map, upl_t upl) {
  upl_size_t upl_size = upl_adjusted_size(upl, VM_MAP_PAGE_MASK(map));
  return vm_map_remove_upl_range(map, upl, 0, upl_size);
}

void iopl_valid_data(upl_t upl, vm_tag_t tag) {
  vm_object_t object;
  vm_offset_t offset;
  ...
  if ((upl->flags & (UPL_DEVICE_MEMORY | UPL_SHADOWED | UPL_ACCESS_BLOCKED |
                     UPL_IO_WIRE | UPL_INTERNAL)) != UPL_IO_WIRE) {
  ...
  if (object->purgable == VM_PURGABLE_VOLATILE ||
    ...
    panic("iopl_valid_data: object %p purgable %d", object, object->purgable);
  }
  ...
  if (object->vo_size == size &&
      object->resident_page_count == (size / PAGE_SIZE)) {
    nxt_page = (vm_page_t)vm_page_queue_first(&object->memq);
    whole_object = true;
  } else {
    offset = (vm_offset_t)(upl_adjusted_offset(upl, PAGE_MASK) -
  ...
  while (size) {
    if (whole_object) {
      ...
      if (m == VM_PAGE_NULL) {
        panic("iopl_valid_data: missing expected page at offset %lx",
              (long)offset);
      }
    }
    if (m->vmp_busy) {
...
void vm_object_set_pmap_cache_attr(vm_object_t object,
                                   upl_page_info_array_t user_page_list,
  ...
  if (!HAS_DEFAULT_CACHEABILITY(cache_attr)) {
    PMAP_BATCH_SET_CACHE_ATTR(object, user_page_list, cache_attr, num_pages,
                              batch_pmap_op);
  }
}

static bool vm_object_iopl_wire_full(vm_object_t object, upl_t upl,
                                     upl_page_info_array_t user_page_list,
  ...
  while (page_count--) {
    if (dst_page->vmp_busy ||
        ...
        vm_page_is_fictitious(dst_page) || dst_page->vmp_absent ||
        VMP_ERROR_GET(dst_page) || dst_page->vmp_cleaning ||
        dst_page->vmp_restart || dst_page->vmp_laundry) {
      retval = FALSE;
      goto done;
    }
    if ((cntrl_flags & UPL_REQUEST_FORCE_COHERENCY) &&
        dst_page->vmp_written_by_kernel == TRUE) {
      retval = FALSE;
      goto done;
    }
    dst_page->vmp_reference = TRUE;
...
static kern_return_t vm_object_iopl_wire_empty(
    vm_object_t object, upl_t upl, upl_page_info_array_t user_page_list,
    upl_control_flags_t cntrl_flags, vm_tag_t tag,
    vm_object_offset_t *dst_offset, int page_count, int *page_grab_count) {
  vm_page_t dst_page;
  ...
  while (page_count--) {
    while ((dst_page = vm_page_grab_options(grab_options)) == VM_PAGE_NULL) {
      OSAddAtomic(page_count, &vm_upl_wait_for_pages);

      VM_DEBUG_EVENT(vm_iopl_page_wait, DBG_VM_IOPL_PAGE_WAIT, DBG_FUNC_START,
                     vm_upl_wait_for_pages, 0, 0, 0);
      ...
      if (vm_page_wait(interruptible) == FALSE) {
        ...
        VM_DEBUG_EVENT(vm_iopl_page_wait, DBG_VM_IOPL_PAGE_WAIT, DBG_FUNC_END,
                       vm_upl_wait_for_pages, 0, 0, -1);
    ...
    vm_page_insert_internal(dst_page, object, *dst_offset, tag, FALSE, TRUE,
                            TRUE, TRUE, &delayed_ledger_update);
  ...
  if (delayed_ledger_update) {
    ...
    if (object->internal) {
      ...
      ledger_credit(owner->ledger, ledger_idx_nonvolatile,
                    delayed_ledger_update);
      if (do_footprint) {
        /* more footprint */
        ledger_credit(owner->ledger, task_ledgers.phys_footprint,
                      delayed_ledger_update);
      } else if (ledger_idx_composite != -1) {
        ledger_credit(owner->ledger, ledger_idx_composite,
                      delayed_ledger_update);
      }
    } else {
      /* more external wired bytes */
      ledger_credit(owner->ledger, ledger_idx_external_wired,
                    delayed_ledger_update);
      if (do_footprint) {
        /* more footprint */
        ledger_credit(owner->ledger, task_ledgers.phys_footprint,
...
kern_return_t vm_object_iopl_request(vm_object_t object,
                                     vm_object_offset_t offset, upl_size_t size,
                                     upl_t *upl_ptr,
                                     upl_page_info_array_t user_page_list,
                                     unsigned int *page_list_count,
                                     upl_control_flags_t cntrl_flags,
  ...
  *upl_ptr = NULL;
  ...
  size = (upl_size_t)(vm_object_round_page(offset + size) -
                      vm_object_trunc_page(offset));
  offset = vm_object_trunc_page(offset);
  if (size != original_size || offset != original_offset) {
    DEBUG4K_IOKIT("flags 0x%llx object %p offset 0x%llx size 0x%x -> offset "
                  "0x%llx size 0x%x\n",
                  cntrl_flags, object, original_offset, original_size, offset,
                  size);
  }
  ...
  if (cntrl_flags & UPL_NEED_32BIT_ADDR) {
    if ((cntrl_flags & (UPL_SET_IO_WIRE | UPL_SET_LITE)) !=
        (UPL_SET_IO_WIRE | UPL_SET_LITE)) {
      return KERN_INVALID_VALUE;
    }

    if (object->phys_contiguous) {
      if ((offset + object->vo_shadow_offset) >=
          (vm_object_offset_t)max_valid_dma_address) {
  ...
  if ((!object->internal) && (object->paging_offset != 0)) {
    panic(
        "vm_object_iopl_request: external object with non-zero paging offset");
  }

  VM_DEBUG_CONSTANT_EVENT(vm_object_iopl_request, DBG_VM_IOPL_REQUEST,
                          DBG_FUNC_START, size, cntrl_flags, prot, 0);
...
#if CONFIG_IOSCHED
  if (object->io_tracking) {
    /* Check if we're dealing with the kernel object. We do not support expedite
     * on kernel object UPLs */
  ...
  *upl_ptr = upl;
  ...
  if (cntrl_flags & UPL_NOZEROFILLIO) {
    DTRACE_VM4(upl_nozerofillio, vm_object_t, object, vm_object_offset_t,
               offset, upl_size_t, size, upl_t, upl);
  }
  ...
  if (object->phys_contiguous) {
    ...
    upl->highest_page =
        (ppnum_t)((offset + object->vo_shadow_offset + size - 1) >> PAGE_SHIFT);

    if (user_page_list) {
      user_page_list[0].phys_addr =
          (ppnum_t)((offset + object->vo_shadow_offset) >> PAGE_SHIFT);
    ...
    VM_DEBUG_CONSTANT_EVENT(vm_object_iopl_request, DBG_VM_IOPL_REQUEST,
                            DBG_FUNC_END, page_grab_count, KERN_SUCCESS, 0, 0);
    if (task != NULL) {
      ledger_credit(task->ledger, task_ledgers.pages_grabbed_iopl,
                    page_grab_count);
    }
  ...
  if (!(cntrl_flags & UPL_COPYOUT_FROM) && object->vo_copy != VM_OBJECT_NULL) {
    /*
     * Honor copy-on-write obligations
     *
     * The caller is gathering these pages and
  ...
  if (!(cntrl_flags & (UPL_NEED_32BIT_ADDR | UPL_BLOCK_ACCESS)) &&
      object->purgable != VM_PURGABLE_VOLATILE &&
      object->purgable != VM_PURGABLE_EMPTY && object->vo_copy == NULL &&
      size == object->vo_size && offset == 0 && object->shadow == NULL &&
      object->pager == NULL) {
  ...
  if (fast_path_full_req) {
    if (vm_object_iopl_wire_full(object, upl, user_page_list, cntrl_flags,
                                 tag) == TRUE) {
      goto finish;
    }
    /*
    ...
    ret =
        vm_object_iopl_wire_empty(object, upl, user_page_list, cntrl_flags, tag,
                                  &dst_offset, size_in_pages, &page_grab_count);
  ...
  while (xfer_size) {
    ...
    if (dst_page == VM_PAGE_NULL || dst_page->vmp_busy ||
        VMP_ERROR_GET(dst_page) || dst_page->vmp_restart ||
        dst_page->vmp_absent || vm_page_is_fictitious(dst_page)) {
      if (is_kernel_object(object)) {
      ...
      if (dst_page != VM_PAGE_NULL && dst_page->vmp_busy) {
        ...
        vm_object_lock_assert_exclusive(object);
        wait_result =
            vm_page_sleep(object, dst_page, interruptible, LCK_SLEEP_DEFAULT);
        if (wait_result == THREAD_AWAKENED || wait_result == THREAD_RESTART) {
          continue;
        }
        ret = MACH_SEND_INTERRUPTED;
        goto return_err;
      ...
      do {
        ...
        result =
            vm_fault_page(object, dst_offset, prot | VM_PROT_WRITE, FALSE,
                          caller_lookup, &prot, &dst_page, &top_page, (int *)0,
                          &error_code, no_zero_fill, &fault_info);
        ...
        case VM_FAULT_MEMORY_SHORTAGE:
          ...
          VM_DEBUG_EVENT(vm_iopl_page_wait, DBG_VM_IOPL_PAGE_WAIT,
                         DBG_FUNC_START, vm_upl_wait_for_pages, 0, 0, 0);

          if (vm_page_wait(interruptible)) {
            OSAddAtomic(-(size_in_pages - entry), &vm_upl_wait_for_pages);

            VM_DEBUG_EVENT(vm_iopl_page_wait, DBG_VM_IOPL_PAGE_WAIT,
                           DBG_FUNC_END, vm_upl_wait_for_pages, 0, 0, 0);
            vm_object_lock(object);
...
finish:
  ...
  if (cntrl_flags & UPL_BLOCK_ACCESS) {
     ...
     */
    vm_object_pmap_protect(object, offset, (vm_object_size_t)size, PMAP_NULL,
                           PAGE_SIZE, 0, VM_PROT_NONE);
    vm_object_lock(object);
    assert(!object->blocked_access);
    object->blocked_access = TRUE;
    vm_object_unlock(object);
...
return_err:
  ...
  *upl_ptr = NULL;
  ...
  VM_DEBUG_CONSTANT_EVENT(vm_object_iopl_request, DBG_VM_IOPL_REQUEST,
                          DBG_FUNC_END, page_grab_count, ret, 0, 0);
  if (task != NULL) {
    ledger_credit(task->ledger, task_ledgers.pages_grabbed_iopl,
                  page_grab_count);
  }
...
kern_return_t upl_transpose(upl_t upl1, upl_t upl2) {
  kern_return_t retval;
  ...
  if (upl1 == UPL_NULL || upl2 == UPL_NULL || upl1 == upl2 ||
      ((upl1->flags & UPL_VECTOR) == UPL_VECTOR) ||
      ((upl2->flags & UPL_VECTOR) == UPL_VECTOR)) {
    return KERN_INVALID_ARGUMENT;
  }
  ...
  if (retval == KERN_SUCCESS) {
    ...
    if ((upl1->flags & UPL_TRACKED_BY_OBJECT) ||
        (upl2->flags & UPL_TRACKED_BY_OBJECT)) {
...
void upl_range_needed(upl_t upl, int index, int count) {
  int size_in_pages;
...
bool vm_paging_page_inuse[VM_PAGING_NUM_PAGES] = {
    FALSE,
};
...
__startup_func static void vm_paging_map_init(void) {
  kmem_alloc(kernel_map, &vm_paging_base_address, ptoa(VM_PAGING_NUM_PAGES),
             KMA_DATA_SHARED | KMA_NOFAIL | KMA_KOBJECT | KMA_PERMANENT |
                 KMA_PAGEABLE,
             VM_KERN_MEMORY_NONE);
}
 ...
 */
kern_return_t vm_paging_map_object(vm_page_t page, vm_object_t object,
                                   vm_object_offset_t offset,
                                   ...
                                   boolean_t *need_unmap)    /* OUT */
{
  ...
  if (page != VM_PAGE_NULL && *size == PAGE_SIZE) {
    ...
    *address = (vm_map_offset_t)phystokv(
        (pmap_paddr_t)VM_PAGE_GET_PHYS_PAGE(page) << PAGE_SHIFT);
    *need_unmap = FALSE;
    return KERN_SUCCESS;
    ...
    for (;;) {
      for (i = 0; i < VM_PAGING_NUM_PAGES; i++) {
        if (vm_paging_page_inuse[i] == FALSE) {
          page_map_offset = vm_paging_base_address + (i * PAGE_SIZE);
          break;
        }
      }
    ...
    if (page_map_offset != 0) {
       ...
       */
      kr = pmap_enter_check(kernel_pmap, page_map_offset, page, protection,
                            VM_PROT_NONE, TRUE);
      assert(kr == KERN_SUCCESS);
  ...
  map_size = vm_map_round_page(*size, VM_MAP_PAGE_MASK(kernel_map));
  ...
  kr = vm_map_enter(kernel_map, address, map_size, 0,
                    VM_MAP_KERNEL_FLAGS_DATA_SHARED_ANYWHERE(), object,
                    object_offset, FALSE, protection, VM_PROT_ALL,
                    VM_INHERIT_NONE);
  if (kr != KERN_SUCCESS) {
    *address = 0;
    *size = 0;
  ...
  *size = map_size;
  ...
  for (page_map_offset = 0; map_size != 0;
    ...
    kr = pmap_enter_check(kernel_pmap, *address + page_map_offset, page,
                          protection, VM_PROT_NONE, TRUE);
    assert(kr == KERN_SUCCESS);
#if KASAN
 ...
 */
void vm_paging_unmap_object(vm_object_t object, vm_map_offset_t start,
                            vm_map_offset_t end) {
  int i;

  if ((vm_paging_base_address == 0) || (start < vm_paging_base_address) ||
      (end > (vm_paging_base_address + (VM_PAGING_NUM_PAGES * PAGE_SIZE)))) {
    /*
 ...
 */
void vm_pageout_steal_laundry(vm_page_t page, boolean_t queues_locked) {
  if (!queues_locked) {
    vm_page_lockspin_queues();
  }
...
upl_t vector_upl_create(vm_offset_t upl_offset, uint32_t max_upls) {
  upl_t upl;
  ...
  vector_upl_t vector_upl =
      kalloc_type(struct _vector_upl, typeof(vector_upl->upls[0]), max_upls,
...
upl_size_t vector_upl_get_size(const upl_t upl) {
  if (!vector_upl_is_valid(upl)) {
...
void vector_upl_deallocate(upl_t upl) {
  ...
  kfree_type(struct upl_page_info, atop(vector_upl->size),
             vector_upl->pagelist);
  kfree_type(struct _vector_upl, typeof(vector_upl->upls[0]), max_upls,
             vector_upl);
  upl->vector_upl = NULL;
}

boolean_t vector_upl_is_valid(upl_t upl) {
  return upl && (upl->flags & UPL_VECTOR) && upl->vector_upl;
}

boolean_t vector_upl_set_subupl(upl_t upl, upl_t subupl, uint32_t io_size) {
  if (vector_upl_is_valid(upl)) {
    vector_upl_t vector_upl = upl->vector_upl;
    ...
    if (vector_upl) {
      if (subupl) {
        if (io_size) {
          ...
          invalid_upls = os_atomic_inc(&(vector_upl)->invalid_upls, relaxed);
...
void vector_upl_set_pagelist(upl_t upl) {
  if (vector_upl_is_valid(upl)) {
    uint32_t i = 0;
    vector_upl_t vector_upl = upl->vector_upl;
    ...
    if (vector_upl) {
      ...
      vector_upl->pagelist =
          kalloc_type(struct upl_page_info, atop(vector_upl->size), Z_WAITOK);

      for (i = 0; i < vector_upl->num_upls; i++) {
        cur_upl_pagelist_size =
            sizeof(struct upl_page_info) *
            upl_adjusted_size(vector_upl->upls[i].elem, PAGE_MASK) / PAGE_SIZE;
        bcopy(vector_upl->upls[i].elem->page_list,
              (char *)vector_upl->pagelist + pagelist_size,
              cur_upl_pagelist_size);
      ...
      assert(pagelist_size ==
             (sizeof(struct upl_page_info) * (vector_upl->size / PAGE_SIZE)));
    } else {
...
upl_t vector_upl_subupl_byindex(upl_t upl, uint32_t index) {
  if (vector_upl_is_valid(upl)) {
    vector_upl_t vector_upl = upl->vector_upl;
    if (vector_upl) {
...
upl_t vector_upl_subupl_byoffset(upl_t upl, upl_offset_t *upl_offset,
                                 upl_size_t *upl_size) {
  if (vector_upl_is_valid(upl)) {
    uint32_t i = 0;
    vector_upl_t vector_upl = upl->vector_upl;
    ...
    if (vector_upl) {
      ...
      for (i = 0; i < vector_upl->num_upls; i++) {
        ...
        if (*upl_offset <= (subupl_state.offset + subupl_state.size - 1)) {
          ...
          if ((subupl_state.offset + subupl_state.size) <
              (*upl_offset + *upl_size)) {
...
void vector_upl_get_addr(upl_t upl, vm_offset_t *dst_addr) {
  if (vector_upl_is_valid(upl)) {
    vector_upl_t vector_upl = upl->vector_upl;
    if (vector_upl) {
...
void vector_upl_set_addr(upl_t upl, vm_offset_t dst_addr) {
  if (vector_upl_is_valid(upl)) {
    vector_upl_t vector_upl = upl->vector_upl;
    if (vector_upl) {
      if (dst_addr) {
...
void vector_upl_get_iostate(upl_t upl, upl_t subupl, upl_offset_t *offset,
                            upl_size_t *size) {
  if (vector_upl_is_valid(upl)) {
    uint32_t i = 0;
    vector_upl_t vector_upl = upl->vector_upl;
...
void vector_upl_get_iostate_byindex(upl_t upl, uint32_t index,
                                    upl_offset_t *offset, upl_size_t *size) {
  if (vector_upl_is_valid(upl)) {
    vector_upl_t vector_upl = upl->vector_upl;
    if (vector_upl) {
      if (index < vector_upl->num_upls) {
...
void *upl_get_internal_vectorupl(upl_t upl) { return upl->vector_upl; }

upl_page_info_t *upl_get_internal_vectorupl_pagelist(upl_t upl) {
  return upl->vector_upl->pagelist;
}

upl_page_info_t *upl_get_internal_page_list(upl_t upl) {
  return upl->vector_upl ? upl->vector_upl->pagelist : upl->page_list;
}

void upl_clear_dirty(upl_t upl, boolean_t value) {
  if (value) {
    upl->flags |= UPL_CLEAR_DIRTY;
  } else {
    upl->flags &= ~UPL_CLEAR_DIRTY;
...
void upl_set_referenced(upl_t upl, boolean_t value) {
  upl_lock(upl);
  if (value) {
    upl->ext_ref_count++;
  } else {
...
void upl_set_map_exclusive(upl_t upl) {
  upl_lock(upl);
  while (upl->map_addr_owner) {
    upl->flags |= UPL_MAP_EXCLUSIVE_WAIT;
    upl_lock_sleep(upl, &upl->map_addr_owner,
                   ctid_get_thread(upl->map_addr_owner));
  }
  upl->map_addr_owner = thread_get_ctid(current_thread());
  upl_unlock(upl);
}

void upl_clear_map_exclusive(upl_t upl) {
...
#if CONFIG_IOSCHED
void upl_set_blkno(upl_t upl, vm_offset_t upl_offset, int io_size,
                   int64_t blkno) {
  int i, j;
  ...
  for (i = (int)(upl_offset / PAGE_SIZE), j = 0; j < io_size;
       i++, j += PAGE_SIZE) {
...
void inline memoryshot(unsigned int event, unsigned int control) {
  if (vm_debug_events) {
    KERNEL_DEBUG_CONSTANT1(
        (MACHDBG_CODE(DBG_MACH_VM_PRESSURE, event)) | control,
        vm_page_active_count, vm_page_inactive_count, vm_page_free_count,
        vm_page_speculative_count, vm_page_throttled_count);
  } else {
    (void)event;
    (void)control;
  }
...
boolean_t upl_device_page(upl_page_info_t *upl) { return UPL_DEVICE_PAGE(upl); }
...
void vm_countdirtypages(void) {
  ...
  do {
    ...
    if (m == (vm_page_t)0) {
      ...
      !vm_page_queue_end(&vm_page_queue_inactive, (vm_page_queue_entry_t)m));
  vm_page_unlock_queues();
  ...
  do {
    ...
    if (m == (vm_page_t)0) {
      ...
      !vm_page_queue_end(&vm_page_queue_throttled, (vm_page_queue_entry_t)m));
  vm_page_unlock_queues();
  ...
  do {
    ...
    if (m == (vm_page_t)0) {
      ...
      !vm_page_queue_end(&vm_page_queue_anonymous, (vm_page_queue_entry_t)m));
  vm_page_unlock_queues();
...
ppnum_t upl_get_highest_page(upl_t upl) { return upl->highest_page; }

upl_size_t upl_get_size(upl_t upl) { return upl_adjusted_size(upl, PAGE_MASK); }

upl_size_t upl_adjusted_size(upl_t upl, vm_map_offset_t pgmask) {
...
vm_object_offset_t upl_adjusted_offset(upl_t upl, vm_map_offset_t pgmask) {
  return trunc_page_mask_64(upl->u_offset, pgmask);
}

vm_object_offset_t upl_get_data_offset(upl_t upl) {
...
bool upl_has_fs_verify_info(upl_t upl) {
  return upl->flags & UPL_HAS_FS_VERIFY_INFO;
}

void upl_set_fs_verify_info(upl_t upl, uint32_t size) {
...
uint8_t *upl_fs_verify_buf(upl_t upl, uint32_t *size) {
  assert(size);
...
boolean_t upl_has_wired_pages(upl_t upl) {
...
boolean_t VM_PRESSURE_NORMAL_TO_WARNING(void) {
  if (!VM_CONFIG_COMPRESSOR_IS_ACTIVE) {
    ...
    return (AVAILABLE_NON_COMPRESSED_MEMORY <
...
boolean_t VM_PRESSURE_WARNING_TO_CRITICAL(void) {
  if (!VM_CONFIG_COMPRESSOR_IS_ACTIVE) {
    ...
    return vm_compressor_low_on_space() ||
                   (AVAILABLE_NON_COMPRESSED_MEMORY <
                    ((12 * VM_PAGE_COMPRESSOR_SWAP_UNTHROTTLE_THRESHOLD) / 10))
               ? 1
               : 0;
  }
}
...
boolean_t VM_PRESSURE_WARNING_TO_NORMAL(void) {
  if (!VM_CONFIG_COMPRESSOR_IS_ACTIVE) {
    ...
    uint32_t available_pages = memorystatus_get_available_page_count();
    uint32_t target_threshold =
        (((115 * memorystatus_get_soft_memlimit_page_shortage_threshold()) /
          100));
    return available_pages > target_threshold;
  } else {
    return (AVAILABLE_NON_COMPRESSED_MEMORY >
            ((12 * VM_PAGE_COMPRESSOR_COMPACT_THRESHOLD) / 10))
               ? 1
               : 0;
  }
}
...
boolean_t VM_PRESSURE_CRITICAL_TO_WARNING(void) {
  if (!VM_CONFIG_COMPRESSOR_IS_ACTIVE) {
    uint32_t available_pages = memorystatus_get_available_page_count();
    uint32_t target_threshold =
        (((115 * memorystatus_get_critical_page_shortage_threshold()) / 100));
    return available_pages > target_threshold;
  } else {
    return (AVAILABLE_NON_COMPRESSED_MEMORY >
            ((14 * VM_PAGE_COMPRESSOR_SWAP_UNTHROTTLE_THRESHOLD) / 10))
               ? 1
               : 0;
  }
}
...
static kern_return_t move_pages_to_queue(vm_map_t map, user_addr_t start_addr,
                                         size_t buffer_size,
  ...
  *pages_moved = 0;
  ...
  end_addr =
      vm_map_round_page_mask(start_addr + buffer_size, VM_MAP_PAGE_MASK(map));
  ...
  while (curr_addr < end_addr) {
    ...
    if (curr_object) {
      ...
      /* We really only want anonymous memory that's in the top level map and
       * object here. */
      ...
      vm_map_offset_t start_offset =
          (curr_addr - curr_entry->vme_start) + VME_OFFSET(curr_entry);
      vm_map_offset_t end_offset =
          MIN(curr_entry->vme_end, end_addr) -
          (curr_entry->vme_start + VME_OFFSET(curr_entry));
      vm_map_offset_t curr_offset = start_offset;
      vm_page_t curr_page;
      while (curr_offset < end_offset) {
        curr_page =
            vm_page_lookup(curr_object, vm_object_trunc_page(curr_offset));
        if (curr_page != VM_PAGE_NULL) {
          ...
          if (curr_page->vmp_pmapped) {
            pmap_clear_refmod_options(VM_PAGE_GET_PHYS_PAGE(curr_page),
                                      VM_MEM_REFERENCED, PMAP_OPTIONS_NOFLUSH,
                                      (void *)NULL);
          }
...
kern_return_t run_compressor_perf_test(user_addr_t buf, size_t buffer_size,
                                       uint64_t *time,
  ...
  *bytes_compressed = *compressor_growth = 0;
  ...
  err = move_pages_to_queue(map, buf, buffer_size, &compressor_perf_test_queue,
                            &page_count);
  if (err != KERN_SUCCESS) {
    goto out;
  }
  ...
  /*
   * At this point the compressor threads should only process the benchmark
   * queue so we can look at the difference in c_segment_compressed_bytes while
   * the perf test is running to determine how many compressed bytes we ended up
   * using.
   */
  ...
  page_count =
      vm_pageout_page_queue(&compressor_perf_test_queue, page_count, true);
  ...
  /*
   * Depending on when this test is run we could overshoot or be right on the
   * mark with our page_count. So the comparison is of the _less than_ variety.
   */
...
out:
  ...
  absolutetime_to_nanoseconds(
      compressor_perf_test_end - compressor_perf_test_start, time);
  p = (vm_page_t)vm_page_queue_first(&compressor_perf_test_queue);
  while (p && !vm_page_queue_end(&compressor_perf_test_queue,
                                 (vm_page_queue_entry_t)p)) {
