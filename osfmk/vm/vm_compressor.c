...
uint32_t c_seg_max_pages; /* maximum number of pages the compressed data of a
                             segment can take  */
uint32_t c_seg_off_limit; /* if we've reached this size while filling the
                           * segment, don't bother trying to fill anymore
                           * because it's unlikely to succeed, in units of
                           * uint32_t, same as c_nextoffset */
...
#if CONFIG_FREEZE
bool freezer_incore_cseg_acct =
    TRUE; /* Only count incore compressed memory for jetsams. */
#endif    /* CONFIG_FREEZE */
...
#define UNPACK_C_SIZE(cs)                                                      \
  ((cs->c_size == (PAGE_SIZE - 1)) ? PAGE_SIZE : cs->c_size)
#define PACK_C_SIZE(cs, size)                                                  \
  (cs->c_size = ((size == PAGE_SIZE) ? PAGE_SIZE - 1 : size))
...
union c_segu {
  c_segment_t c_seg;
  uintptr_t c_segno; /* index of the next element in the segments free-list,
                        c_free_segno_head is the head */
};
...
c_slot_packing_params = VM_PACKING_PARAMS(C_SLOT_PACKED_PTR);
...
queue_head_t c_early_swappedin_list_head, c_regular_swappedin_list_head,
    c_late_swappedin_list_head;
queue_head_t c_early_swapout_list_head, c_regular_swapout_list_head,
    c_late_swapout_list_head;
...
uint32_t c_early_swappedin_count = 0, c_regular_swappedin_count = 0,
         c_late_swappedin_count = 0;
uint32_t c_early_swapout_count = 0, c_regular_swapout_count = 0,
         c_late_swapout_count = 0;
...
/* a c_segment can be in the minor-compact queue as well as one of the above
 * ones, via c_list */
...
int c_seg_fixed_array_len; /* number of slots in the c_segment inline slots
                              array */
union c_segu
    *c_segments; /* array of all c_segments, not all of it may be populated */
vm_offset_t c_buffers; /* starting address of all compressed data pointed to by
                          c_segment.c_store.c_buffer */
vm_size_t c_buffers_size; /* total size allocated in c_buffers */
caddr_t
    c_segments_next_page; /* next page to populate for extending c_segments */
boolean_t c_segments_busy;
uint32_t
    c_segments_available;  /* how many segments are in populated memory (used or
                              free), populated size of c_segments array */
...
uint32_t c_segment_pages_compressed =
    0; /* Tracks # of uncompressed pages fed into the compressor, including SV
          (single value) pages */
#if CONFIG_FREEZE
int32_t c_segment_pages_compressed_incore =
    0; /* Tracks # of uncompressed pages fed into the compressor that are in
          memory */
int32_t c_segment_pages_compressed_incore_late_swapout =
    0; /* Tracks # of uncompressed pages fed into the compressor that are in
          memory and tagged for swapout */
uint32_t c_segments_incore_limit =
    0; /* Tracks # of segments allowed to be in-core. Based on compressor pool
          size */
#endif /* CONFIG_FREEZE */

uint32_t c_segment_pages_compressed_limit;
uint32_t c_segment_pages_compressed_nearing_limit;
uint32_t c_free_segno_head =
    (uint32_t)-1; /* head of free list of c_segment pointers in c_segments */
...
uint32_t
    age_of_decompressions_during_sample_period[DECOMPRESSION_SAMPLE_MAX_AGE];
...
struct c_sv_hash_entry c_segment_sv_hash_table[C_SV_HASH_SIZE]
...
int c_seg_do_minor_compaction_and_unlock(c_segment_t, boolean_t, boolean_t,
                                         boolean_t);
 ...
 */
static inline union c_segu *c_segments_get(uint32_t segno) {
 ...
 * be initiated.
 */
static bool vm_compressor_needs_to_major_compact(void) {
  ...
  /* second condition:
   *   first term:
   *   - (incore_seg_count * c_seg_max_pages) is the maximum number of pages
   * that all resident segments can hold in their buffers
   *   - VM_PAGE_COMPRESSOR_COUNT is the current size that is actually held by
   * the buffers
   *   -- subtracting these gives the amount of pages that is wasted as holes
   * due to segments not being full second term:
   ...
   * first condition:
   *   compare number of segments being used vs the number of segments that can
   * ever be allocated if we don't have a lot of data in the compressor, then we
   * don't need to bother caring about wasted space in holes
   */
...
uint32_t vm_compressor_incore_fragmentation_wasted_pages(void) {
  /* return one of the components of the calculation in
...
TUNABLE_WRITEABLE(uint64_t, vm_compressor_minor_fragmentation_threshold_pct,
                  "vm_compressor_minor_frag_threshold_pct", 10);

static bool vm_compressor_needs_to_minor_compact(void) {
  ...
  bool is_pressured =
      AVAILABLE_NON_COMPRESSED_MEMORY < VM_PAGE_COMPRESSOR_COMPACT_THRESHOLD;
  if (!is_pressured) {
    return false;
  }
...
uint32_t vm_compressor_pool_size(void) { return VM_PAGE_COMPRESSOR_COUNT; }

uint32_t vm_compressor_fragmentation_level(void) {
  const uint32_t incore_seg_count =
      c_segment_count - c_swappedout_count - c_swappedout_sparse_count;
  if ((incore_seg_count == 0) || (c_seg_max_pages == 0)) {
    return 0;
  }
  return 100 - (vm_compressor_pool_size() * 100 /
                (incore_seg_count * c_seg_max_pages));
}
...
bool vm_compressor_compressed_pages_nearing_limit(void) {
  return vm_compressor_pages_compressed() >
         c_segment_pages_compressed_nearing_limit;
}

static bool vm_compressor_segments_nearing_limit(void) {
  uint64_t segments;
...
bool vm_compressor_out_of_space(void) {
  ...
  if (freezer_incore_cseg_acct) {
    ...
    if (os_sub_overflow(incore_seg_count, c_swappedout_sparse_count,
                        &incore_seg_count)) {
      incore_seg_count = 0;
    }
    incore_compressed_pages =
        os_atomic_load(&c_segment_pages_compressed_incore, relaxed);
  } else {
...
bool vm_compressor_is_thrashing() {
...
int vm_wants_task_throttled(task_t task) {
...
extern boolean_t IOServiceWaitForMatchingResource(const char *property,
                                                  uint64_t timeout);
static void erase_csegbufsz_experiment_property(__unused void *param0,
                                                __unused void *param1) {
  ...
  if (!PERemoveNVRAMProperty(csegbufsizeExperimentProperty)) {
    printf("csegbufsize_experiment_property: Failed to remove %s from NVRAM.",
           csegbufsizeExperimentProperty);
  }
  thread_call_free(csegbufsz_experiment_thread_call);
}

static void erase_csegbufsz_experiment_property_async() {
  csegbufsz_experiment_thread_call = thread_call_allocate_with_priority(
      erase_csegbufsz_experiment_property, NULL, THREAD_CALL_PRIORITY_LOW);
...
static void cleanup_csegbufsz_experiment(__unused void *arg0) {
  char nvram = 0;
...
TUNABLE_DT(uint64_t, swap_vol_min_capacity, "/defaults",
           "kern.swap_min_capacity", "kern.swap_min_capacity", 0,
           TUNABLE_DT_NONE);

static void vm_compressor_set_size(void) {
  /*
   * Note that this function may be called multiple times on systems with app
   * swap because the value of vm_swap_get_max_configured_space() and
   * memorystatus_swap_all_apps can change based the size of the swap volume. On
   * these systems, we'll call this function once early in boot to reserve the
   * maximum amount of VA required for the compressor submap and then one more
   * time in vm_compressor_init after determining the swap volume size. We must
   * not return a larger value the second time around.
   */
  vm_size_t c_segments_arr_size = 0;
  struct c_slot_mapping tmp_slot_ptr;

  /* The segment size can be overwritten by a boot-arg */
  if (!PE_parse_boot_argn("vm_compressor_segment_buffer_size", &c_seg_bufsize,
                          sizeof(c_seg_bufsize))) {
#if CONFIG_JETSAM
  ...
  PE_parse_boot_argn("kern.compressor_pool_multiplier",
                     &compressor_pool_multiplier,
                     sizeof(compressor_pool_multiplier));
  ...
  c_segments_limit =
      (uint32_t)(compressor_pool_size / (vm_size_t)(c_seg_allocsize));
  tmp_slot_ptr.s_cseg = c_segments_limit;
  /* Panic on internal configs*/
  assertf((tmp_slot_ptr.s_cseg == c_segments_limit),
          "vm_compressor_init: overflowed s_cseg field in c_slot_mapping with "
          "c_segno: %d",
          c_segments_limit);

  if (tmp_slot_ptr.s_cseg != c_segments_limit) {
    tmp_slot_ptr.s_cseg = -1;
    c_segments_limit = tmp_slot_ptr.s_cseg -
                       1; /*limited by segment idx bits in c_slot_mapping*/
    compressor_pool_size = (c_segments_limit * (vm_size_t)(c_seg_allocsize));
  }

  c_segments_nearing_limit =
      (uint32_t)(((uint64_t)c_segments_limit * 98ULL) / 100ULL);

  /* an upper limit on how many input pages the compressor can hold */
  c_segment_pages_compressed_limit =
      (c_segments_limit * (c_seg_bufsize / PAGE_SIZE) *
       compressor_pool_multiplier);
  ...
  c_segment_pages_compressed_nearing_limit =
      (uint32_t)(((uint64_t)c_segment_pages_compressed_limit * 98ULL) / 100ULL);
  ...
  if (freezer_incore_cseg_acct) {
    /*
     * Add enough segments to track all frozen c_segs that can be stored in
     * swap.
     */
    c_segments_limit += (uint32_t)(vm_swap_get_max_configured_space() /
                                   (vm_size_t)(c_seg_allocsize));
    tmp_slot_ptr.s_cseg = c_segments_limit;
    /* Panic on internal configs*/
    assertf((tmp_slot_ptr.s_cseg == c_segments_limit),
            "vm_compressor_init: freezer reserve overflowed s_cseg field in "
            "c_slot_mapping with c_segno: %d",
            c_segments_limit);
  }
#endif
  /*
   ...
   */
  c_segments_arr_size = vm_map_round_page(
      (sizeof(union c_segu) * c_segments_limit), VM_MAP_PAGE_MASK(kernel_map));
  c_buffers_size = vm_map_round_page(
      ((vm_size_t)c_seg_allocsize * (vm_size_t)c_segments_limit),
...
bool osenvironment_is_diagnostics(void) {
  DTEntry chosen;
  const char *osenvironment;
  unsigned int size;
  if (kSuccess == SecureDTLookupEntry(0, "/chosen", &chosen)) {
    if (kSuccess == SecureDTGetProperty(chosen, "osenvironment",
                                        (void const **)&osenvironment, &size)) {
...
void vm_compressor_init(void) {
  ...
  if (memorystatus_swap_all_apps) {
     ...
     */
    int error =
        vm_swap_vol_get_capacity(SWAP_VOLUME_NAME, &vm_swap_volume_capacity);
#if DEVELOPMENT || DEBUG
    if (error != 0) {
      panic(
...
#if DEVELOPMENT || DEBUG
  if (PE_parse_boot_argn("-disable_cseg_write_protection", bootarg_name,
                         sizeof(bootarg_name))) {
...
#if CONFIG_FREEZE
  if (PE_parse_boot_argn("-disable_freezer_cseg_acct", bootarg_name,
                         sizeof(bootarg_name))) {
   ...
   */
  uint32_t c_segment_size =
      sizeof(struct c_segment) +
      (c_seg_slot_var_array_min_len * sizeof(struct c_slot));
  ...
  compressor_segment_zone = zone_create("compressor_segment", c_segment_size,
...
#define BSD_HOST 1
    ...
    /*
     * vm_compressor_kdp_state.kc_decompressed_page must be page aligned because
     * we access it through the physical aperture by page number.
     */
    vm_compressor_kdp_state.kc_panic_decompressed_page = buf;
    vm_compressor_kdp_state.kc_panic_decompressed_page_paddr = kvtophys(
        (vm_offset_t)vm_compressor_kdp_state.kc_panic_decompressed_page);
    vm_compressor_kdp_state.kc_panic_decompressed_page_ppnum =
        (ppnum_t)atop(vm_compressor_kdp_state.kc_panic_decompressed_page_paddr);
    buf += PAGE_SIZE;
    bufsize -= PAGE_SIZE;
    ...
    c_compressed_record_ebuf =
        c_compressed_record_sbuf + c_compressed_record_sbuf_size;
  ...
  if (kernel_thread_start_priority(
          (thread_continue_t)vm_compressor_swap_trigger_thread, NULL,
...
#define COMPRESSOR_KDP_BUFSIZE                                                 \
  ((vm_compressor_get_decode_scratch_size() * compressor_cpus) +               \
   (PAGE_SIZE * compressor_cpus)) +                                            \
      (sizeof(*vm_compressor_kdp_state.kc_decompressed_pages_paddr) *          \
       compressor_cpus) +                                                      \
      (sizeof(*vm_compressor_kdp_state.kc_decompressed_pages_ppnum) *          \
       compressor_cpus)
 ...
 */
kern_return_t vm_compressor_kdp_init(void) {
  char *buf;
  kern_return_t err;
  size_t bufsize;
  ...
  buf += sizeof(*vm_compressor_kdp_state.kc_decompressed_pages_paddr) *
         compressor_cpus;
  bufsize -= sizeof(*vm_compressor_kdp_state.kc_decompressed_pages_paddr) *
             compressor_cpus;
  ...
  buf += sizeof(*vm_compressor_kdp_state.kc_decompressed_pages_ppnum) *
         compressor_cpus;
  bufsize -= sizeof(*vm_compressor_kdp_state.kc_decompressed_pages_ppnum) *
             compressor_cpus;

  assert(bufsize == 0);

  for (size_t i = 0; i < compressor_cpus; i++) {
    vm_offset_t offset = (vm_offset_t)&vm_compressor_kdp_state
                             .kc_decompressed_pages[i * PAGE_SIZE];
    vm_compressor_kdp_state.kc_decompressed_pages_paddr[i] = kvtophys(offset);
    vm_compressor_kdp_state.kc_decompressed_pages_ppnum[i] =
        (ppnum_t)atop(vm_compressor_kdp_state.kc_decompressed_pages_paddr[i]);
  }
 ...
 * Stackshot mutex must be held.
 */
void vm_compressor_kdp_teardown(void) {
  extern lck_mtx_t stackshot_subsys_mutex;
  LCK_MTX_ASSERT(&stackshot_subsys_mutex, LCK_MTX_ASSERT_OWNED);
  ...
  kmem_free(kernel_map, (vm_offset_t)vm_compressor_kdp_state.kc_scratch_bufs,
            COMPRESSOR_KDP_BUFSIZE);
...
static uint32_t c_slot_extra_size(c_slot_t cs) {
#pragma unused(cs)
  return 0;
}
...
static void c_seg_validate(c_segment_t c_seg, boolean_t must_be_compact) {
  uint16_t c_indx;
  int32_t bytes_used;
  ...
  for (c_indx = 0; c_indx < c_seg->c_nextslot; c_indx++) {
    ...
    unsigned csvhash;
    if (c_size &&
        cs->c_hash_compressed_data !=
            (csvhash = vmc_hash((char *)&c_seg->c_store.c_buffer[cs->c_offset],
                                c_size))) {
      addr64_t csvphys =
          kvtophys((vm_offset_t)&c_seg->c_store.c_buffer[cs->c_offset]);
      panic("Compressed data doesn't match original %p phys: 0x%llx %d %p %d "
            "%d 0x%x 0x%x",
            c_seg, csvphys, cs->c_offset, cs, c_indx, c_size,
            cs->c_hash_compressed_data, csvhash);
    }
#endif
#if POPCOUNT_THE_COMPRESSED_DATA
    unsigned csvpop;
    if (c_size) {
      ...
      if (cs->c_pop_cdata != (csvpop = vmc_pop(csvaddr, c_size))) {
        panic("Compressed data popcount doesn't match original, bit distance: "
              "%d %p (phys: %p) %p %p 0x%llx 0x%x 0x%x 0x%x",
              (csvpop - cs->c_pop_cdata), (void *)csvaddr,
              (void *)kvtophys(csvaddr), c_seg, cs, (uint64_t)cs->c_offset,
              c_size, csvpop, cs->c_pop_cdata);
      }
    }
#endif
  }
  ...
  if (c_seg->c_bytes_used >
      C_SEG_OFFSET_TO_BYTES((int32_t)c_seg->c_nextoffset)) {
    panic("c_seg_validate: c_bytes_used > c_nextoffset - c_nextoffset = %d,  "
          "c_bytes_used = %d",
          (int32_t)C_SEG_OFFSET_TO_BYTES((int32_t)c_seg->c_nextoffset),
          c_seg->c_bytes_used);
  }
  ...
  if (must_be_compact) {
    if (c_seg->c_bytes_used !=
        C_SEG_OFFSET_TO_BYTES((int32_t)c_seg->c_nextoffset)) {
      panic("c_seg_validate: c_bytes_used doesn't match c_nextoffset - "
            "c_nextoffset = %d,  c_bytes_used = %d",
            (int32_t)C_SEG_OFFSET_TO_BYTES((int32_t)c_seg->c_nextoffset),
            c_seg->c_bytes_used);
    }
  }
...
void c_seg_need_delayed_compaction(c_segment_t c_seg,
...
void c_seg_move_to_sparse_list(c_segment_t c_seg) {
  boolean_t clear_busy = FALSE;
...
int c_seg_do_minor_compaction_and_unlock(c_segment_t c_seg,
   ...
   */
  if (c_seg->c_state == C_ON_MAJORCOMPACT_Q &&
...
void kdp_compressor_busy_find_owner(event64_t wait_event,
...
void c_seg_wait_on_busy(c_segment_t c_seg) {
  c_seg->c_wanted = 1;
 ...
 *
 */
void c_seg_update_task_owner(c_segment_t c_seg, task_t new_owner_task) {
  ...
  if (owner_task) {
    task_update_frozen_to_swap_acct(owner_task, uncompressed_bytes,
                                    DEBIT_FROM_SWAP);
    queue_remove(&owner_task->task_frozen_cseg_q, c_seg, c_segment_t,
                 c_task_list_next_cseg);
  }

  if (new_owner_task) {
    queue_enter(&new_owner_task->task_frozen_cseg_q, c_seg, c_segment_t,
                c_task_list_next_cseg);
...
void c_seg_switch_state(c_segment_t c_seg, int new_state,
  ...
  case C_ON_SWAPPEDIN_Q:
    if (c_seg->c_has_donated_pages) {
      ...
      *donate_swappedin_count -= 1;
      ...
      queue_remove(&c_early_swappedin_list_head, c_seg, c_segment_t,
                   c_age_list);
      c_early_swappedin_count--;
#else  /* CONFIG_FREEZE */
      queue_remove(&c_regular_swappedin_list_head, c_seg, c_segment_t,
                   c_age_list);
  ...
  case C_ON_SWAPOUT_Q:
    assert(new_state == C_ON_AGE_Q || new_state == C_IS_FREE ||
           new_state == C_IS_EMPTY || new_state == C_ON_SWAPIO_Q);
...
#endif /* CONFIG_FREEZE */
    {
      if (c_seg->c_has_donated_pages) {
        ...
        *donate_swapout_count -= 1;
        ...
        queue_remove(&c_regular_swapout_list_head, c_seg, c_segment_t,
                     c_age_list);
...
#if CONFIG_FREEZE
    if (c_seg->c_has_freezer_pages) {
      assert(new_state == C_ON_SWAPPEDOUT_Q ||
             new_state == C_ON_SWAPPEDOUTSPARSE_Q || new_state == C_ON_AGE_Q);
    } else
#endif /* CONFIG_FREEZE */
    {
      if (c_seg->c_has_donated_pages) {
  ...
  case C_ON_SWAPPEDOUT_Q:
    assert(new_state == C_ON_SWAPPEDIN_Q || new_state == C_ON_AGE_Q ||
           new_state == C_ON_SWAPPEDOUTSPARSE_Q || new_state == C_ON_BAD_Q ||
           new_state == C_IS_EMPTY || new_state == C_IS_FREE);
  ...
  case C_ON_SWAPPEDOUTSPARSE_Q:
    assert(new_state == C_ON_SWAPPEDIN_Q || new_state == C_ON_AGE_Q ||
           new_state == C_ON_BAD_Q || new_state == C_IS_EMPTY ||
           new_state == C_IS_FREE);

    queue_remove(&c_swappedout_sparse_list_head, c_seg, c_segment_t,
                 c_age_list);
  ...
  case C_IS_EMPTY:
    assert(old_state == C_ON_SWAPOUT_Q || old_state == C_ON_SWAPPEDOUT_Q ||
           old_state == C_ON_SWAPPEDOUTSPARSE_Q);
  ...
  case C_ON_AGE_Q:
    assert(old_state == C_IS_FILLING || old_state == C_ON_SWAPPEDIN_Q ||
           ...
           old_state == C_ON_MAJORCOMPACT_Q || old_state == C_ON_SWAPPEDOUT_Q ||
           old_state == C_ON_SWAPPEDOUTSPARSE_Q);
  ...
  case C_ON_SWAPPEDIN_Q: {
    ...
    assert(old_state == C_ON_SWAPPEDOUT_Q ||
           old_state == C_ON_SWAPPEDOUTSPARSE_Q || old_state == C_ON_SWAPIO_Q);
    if (c_seg->c_has_donated_pages) {
      /* Error in swapouts could happen while the c_seg is still on the swapio
       * queue */
  ...
  case C_ON_SWAPPEDOUT_Q:
    ...
    if (insert_head == TRUE) {
      queue_enter_first(&c_swappedout_list_head, c_seg, c_segment_t,
                        c_age_list);
    } else {
  ...
  case C_ON_SWAPPEDOUTSPARSE_Q:
    ...
    if (insert_head == TRUE) {
      queue_enter_first(&c_swappedout_sparse_list_head, c_seg, c_segment_t,
                        c_age_list);
    } else {
      queue_enter(&c_swappedout_sparse_list_head, c_seg, c_segment_t,
                  c_age_list);
    }
...
void c_seg_free(c_segment_t c_seg) {
  assert(c_seg->c_busy);
...
void c_seg_free_locked(c_segment_t c_seg) {
  int segno;
  ...
  if (c_buffer) {
    if (pages_populated) {
      kernel_memory_depopulate((vm_offset_t)c_buffer, ptoa(pages_populated),
...
void c_seg_trim_tail(c_segment_t c_seg) {
  c_slot_t cs;
  uint32_t c_size;
  uint32_t c_offset;
  uint32_t c_rounded_size;
  ...
  while (c_seg->c_nextslot) {
    ...
    if (c_size) {
      if (current_nextslot != c_seg->c_nextslot) {
        c_rounded_size =
            C_SEG_ROUND_TO_ALIGNMENT(c_size + c_slot_extra_size(cs));
        c_offset = cs->c_offset + C_SEG_BYTES_TO_OFFSET(c_rounded_size);

        c_seg->c_nextoffset = c_offset;
        c_seg->c_populated_offset =
            (c_offset + (C_SEG_BYTES_TO_OFFSET(PAGE_SIZE) - 1)) &
...
int c_seg_minor_compaction_and_unlock(c_segment_t c_seg, boolean_t clear_busy) {
  ...
  for (i = c_indx + 1; i < c_seg->c_nextslot && c_offset < c_seg->c_nextoffset;
       i++) {
    ...
    c_rounded_size =
        C_SEG_ROUND_TO_ALIGNMENT(c_size + c_slot_extra_size(c_src));

    /* N.B.: This memcpy may be an overlapping copy */
    memcpy(&c_seg->c_store.c_buffer[c_offset],
           &c_seg->c_store.c_buffer[c_src->c_offset], c_rounded_size);
  ...
  if (old_populated_offset > c_seg->c_populated_offset) {
    ...
    gc_size =
        C_SEG_OFFSET_TO_BYTES(old_populated_offset - c_seg->c_populated_offset);
    gc_ptr = &c_seg->c_store.c_buffer[c_seg->c_populated_offset];

    kernel_memory_depopulate((vm_offset_t)gc_ptr, gc_size, KMA_COMPRESSOR,
                             VM_KERN_MEMORY_COMPRESSOR);
  }
...
static void c_seg_alloc_nextslot(c_segment_t c_seg) {
  ...
  if ((c_seg->c_nextslot - c_seg_fixed_array_len) >=
    ...
    if (old_slot_array) {
      memcpy(new_slot_array, old_slot_array, sizeof(struct c_slot) * oldlen);
    }
...
boolean_t c_seg_major_compact_ok(c_segment_t c_seg_dst, c_segment_t c_seg_src) {
  ...
  if (c_seg_dst->c_nextoffset >= c_seg_off_limit ||
      c_seg_dst->c_nextslot >= C_SLOT_MAX_INDEX) {
    /*
 ...
 */
boolean_t c_seg_major_compact(c_segment_t c_seg_dst, c_segment_t c_seg_src) {
  c_slot_mapping_t slot_ptr;
  uint32_t c_rounded_size;
  uint32_t c_size;
  uint16_t dst_slot;
...
#endif
  assertf(c_seg_dst->c_has_donated_pages == c_seg_src->c_has_donated_pages,
          "Mismatched donation status Dst: %p, Src: %p\n", c_seg_dst,
          c_seg_src);
  ...
  for (i = 0; i < c_seg_src->c_nextslot; i++) {
    ...
    int size_left =
        c_seg_bufsize - C_SEG_OFFSET_TO_BYTES(c_seg_dst->c_nextoffset);
    /* we're going to increment c_nextoffset by c_rounded_size so it should not
     * overflow the segment bufsize */
    ...
    assertf(c_seg_dst->c_populated_offset >= c_seg_dst->c_nextoffset,
            "Unexpected segment offsets: %u,%u", c_seg_dst->c_populated_offset,
            c_seg_dst->c_nextoffset);
    if (C_SEG_OFFSET_TO_BYTES(c_seg_dst->c_populated_offset -
                              c_seg_dst->c_nextoffset) <
        (unsigned)combined_size) {
      int size_to_populate;

      /* eagerly populate the entire segment in expectation to fill it */
      assert(c_seg_bufsize >=
             C_SEG_OFFSET_TO_BYTES(c_seg_dst->c_populated_offset));
      size_to_populate =
          c_seg_bufsize - C_SEG_OFFSET_TO_BYTES(c_seg_dst->c_populated_offset);
      ...
      kernel_memory_populate((vm_offset_t)&c_seg_dst->c_store
                                 .c_buffer[c_seg_dst->c_populated_offset],
                             size_to_populate, KMA_NOFAIL | KMA_COMPRESSOR,
                             VM_KERN_MEMORY_COMPRESSOR);

      c_seg_dst->c_populated_offset += C_SEG_BYTES_TO_OFFSET(size_to_populate);
      assert(C_SEG_OFFSET_TO_BYTES(c_seg_dst->c_populated_offset) <=
             c_seg_bufsize);
    }
     ...
     */
    PAGE_REPLACEMENT_DISALLOWED(TRUE);
    memcpy(&c_seg_dst->c_store.c_buffer[c_seg_dst->c_nextoffset],
           &c_seg_src->c_store.c_buffer[c_src->c_offset], combined_size);
    PAGE_REPLACEMENT_DISALLOWED(FALSE);

    c_seg_major_compact_stats[c_seg_major_compact_stats_now].moved_slots++;
    c_seg_major_compact_stats[c_seg_major_compact_stats_now].moved_bytes +=
        combined_size;
    ...
    if (!c_seg_src->c_swappedin) {
      /* Pessimistically lose swappedin status when non-swappedin pages are
       * added. */
...
uint64_t vm_compressor_compute_elapsed_msecs(clock_sec_t end_sec,
...
void compute_swapout_target_age(void) {
  ...
  elapsed_msecs_in_sample = vm_compressor_compute_elapsed_msecs(
      cur_ts_sec, cur_ts_nsec, start_of_sample_period_sec,
  ...
  elapsed_msecs_in_eval = vm_compressor_compute_elapsed_msecs(
      cur_ts_sec, cur_ts_nsec, start_of_eval_period_sec,
  ...
  KERNEL_DEBUG(0xe0400020 | DBG_FUNC_START, elapsed_msecs_in_eval,
               sample_period_compression_count,
               sample_period_decompression_count, 0, 0);

  min_operations_needed_in_this_sample =
      (compressor_thrashing_min_per_10msecs * (uint32_t)elapsed_msecs_in_eval) /
      10;

  if ((sample_period_compression_count - last_eval_compression_count) <
          min_operations_needed_in_this_sample ||
      (sample_period_decompression_count - last_eval_decompression_count) <
          min_operations_needed_in_this_sample) {
    KERNEL_DEBUG(0xe0400020 | DBG_FUNC_END,
                 sample_period_compression_count - last_eval_compression_count,
  ...
  if (sample_period_decompression_count >
    ...
    aging_target =
        (sample_period_decompression_count / 100) * 1; /* 1 percent */
    running_total = 0;

    for (oldest_age_of_csegs_sampled = 0;
         oldest_age_of_csegs_sampled < DECOMPRESSION_SAMPLE_MAX_AGE;
         oldest_age_of_csegs_sampled++) {
      running_total += age_of_decompressions_during_sample_period
          [oldest_age_of_csegs_sampled];

      working_set_approximation += oldest_age_of_csegs_sampled *
    ...
    if (oldest_age_of_csegs_sampled < DECOMPRESSION_SAMPLE_MAX_AGE) {
      working_set_approximation =
          (working_set_approximation * 1000) / elapsed_msecs_in_sample;

      if (working_set_approximation < VM_PAGE_COMPRESSOR_COUNT) {
        running_total = overage_decompressions_during_sample_period;

        for (oldest_age_of_csegs_sampled = DECOMPRESSION_SAMPLE_MAX_AGE - 1;
             oldest_age_of_csegs_sampled; oldest_age_of_csegs_sampled--) {
          running_total += age_of_decompressions_during_sample_period
              [oldest_age_of_csegs_sampled];
        ...
        KERNEL_DEBUG(0xe0400020 | DBG_FUNC_END, swapout_target_age,
                     working_set_approximation, VM_PAGE_COMPRESSOR_COUNT, 2, 0);
      } else {
        KERNEL_DEBUG(0xe0400020 | DBG_FUNC_END, working_set_approximation,
                     VM_PAGE_COMPRESSOR_COUNT, 0, 3, 0);
      }
    } else {
      KERNEL_DEBUG(0xe0400020 | DBG_FUNC_END, working_target, running_total, 0,
                   4, 0);
    }
    ...
    KERNEL_DEBUG(
        0xe0400020 | DBG_FUNC_END, sample_period_decompression_count,
        (compressor_thrashing_threshold_per_10msecs * elapsed_msecs_in_sample) /
            10,
        0, 6, 0);
  }
  ...
  if (compressor_need_sample_reset == TRUE) {
    bzero(age_of_decompressions_during_sample_period,
...
bool vm_compressor_swapout_is_ripe() {
  bool is_ripe = false;
  if (vm_swapout_ripe_segments == TRUE &&
      c_overage_swapped_count < c_overage_swapped_limit) {
    c_segment_t c_seg;
    clock_sec_t now;
    clock_sec_t age;
    clock_nsec_t nsec;
...
static bool compressor_swapout_conditions_met(void) {
  bool should_swap = false;
  if (COMPRESSOR_NEEDS_TO_SWAP()) {
    should_swap = true;
    vmcs_stats.compressor_swap_threshold_exceeded++;
  }
  if (VM_PAGE_Q_THROTTLED(&vm_pageout_queue_external) &&
      vm_page_anonymous_count < (vm_page_inactive_count / 20)) {
    should_swap = true;
    vmcs_stats.external_q_throttled++;
  }
  if (vm_page_free_count <
      (vm_page_free_reserved - (COMPRESSOR_FREE_RESERVED_LIMIT * 2))) {
    should_swap = true;
...
static bool compressor_needs_to_swap() {
  bool should_swap = false;
...
/* run minor compaction right now, if the compaction-swapper thread is not
 * already running */
...
void vm_compressor_do_delayed_compactions(boolean_t flush_all) {
  c_segment_t c_seg;
  int number_compacted = 0;
  boolean_t needs_to_swap = FALSE;
  uint32_t c_swapout_count = 0;

  VM_DEBUG_CONSTANT_EVENT(vm_compressor_do_delayed_compactions,
                          DBG_VM_COMPRESSOR_DELAYED_COMPACT, DBG_FUNC_START,
                          c_minor_count, flush_all, 0, 0);
  ...
  while (!queue_empty(&c_minor_list_head) && needs_to_swap == FALSE) {
    ...
    c_swapout_count =
        c_early_swapout_count + c_regular_swapout_count + c_late_swapout_count;
    if (VM_CONFIG_SWAP_IS_ACTIVE &&
        (number_compacted++ > DELAYED_COMPACTIONS_PER_PASS)) {
      if ((flush_all == TRUE || compressor_needs_to_swap()) &&
          c_swapout_count < C_SWAPOUT_LIMIT) {
        needs_to_swap = TRUE;
      }
  ...
  VM_DEBUG_CONSTANT_EVENT(vm_compressor_do_delayed_compactions,
                          DBG_VM_COMPRESSOR_DELAYED_COMPACT, DBG_FUNC_END,
                          c_minor_count, number_compacted, needs_to_swap, 0);
}

int min_csegs_per_major_compaction = DELAYED_COMPACTIONS_PER_PASS;

static bool vm_compressor_major_compact_cseg(c_segment_t c_seg,
  ...
  while (keep_compacting == TRUE) {
    ...
    if (c_seg_next->c_busy) {
      ...
      if (c_seg->c_wanted) {
        ...
        c_seg_major_compact_stats[c_seg_major_compact_stats_now]
            .bailed_compactions++;
      ...
      VM_DEBUG_CONSTANT_EVENT(vm_compressor_compact_and_swap,
                              DBG_VM_COMPRESSOR_COMPACT_AND_SWAP, DBG_FUNC_NONE,
                              8, (void *)VM_KERNEL_ADDRPERM(c_seg_next), 0, 0);
  ...
  VM_DEBUG_CONSTANT_EVENT(vm_compressor_compact_and_swap,
                          DBG_VM_COMPRESSOR_COMPACT_AND_SWAP, DBG_FUNC_NONE, 10,
                          number_considered, *bail_wanted_cseg, 0);
...
static void vm_compressor_process_major_segments(bool ripe_age_only) {
  ...
  while (!queue_empty(&c_major_list_head)) {
    if (!ripe_age_only) {
      ...
      if ((start_sec - c_seg->c_creation_ts) < vm_ripe_target_age) {
        /*
         * We have found the first segment in our queue that is not ripe.
         * Segments after it will be the same. So let's bail here. Return with
         * c_list_lock held.
         */
        break;
      }
    ...
    if (end_sec > 0 || end_nsec >= report_over_ns) {
      /* we used more than expected */
      c_process_major_reports++;
      printf("%s: moved %d/%d segments from MAJORQ to AGEQ in %lu.%09u seconds "
             "and %d breaks\n",
             __FUNCTION__, count, total, end_sec, end_nsec, breaks);
    }
    if (queue_empty(&c_major_list_head)) {
      /* done */
      break;
...
static void vm_compressor_process_special_swapped_in_segments_locked(void) {
  ...
  yield_after_considered_per_pass =
      MAX(min_csegs_per_major_compaction, DELAYED_COMPACTIONS_PER_PASS);
  while (!queue_empty(special_swappedin_list_head)) {
    ...
    switch_state = vm_compressor_major_compact_cseg(
        c_seg, &number_considered, &bail_wanted_cseg, &bytes_freed);
    assert(c_seg->c_busy);
    assert(!c_seg->c_on_minorcompact_q);
...
/* minimum time that segments can be in swappedin q as a grace period after they
 * were swapped-in before they are added to age-q */
...
vm_compressor_process_regular_swapped_in_segments(boolean_t flush_all) {
  ...
  /* In normal operation, segments are kept in the swapped-in-q for a grace
   * period of 10 seconds so that whoever needed to decompress something from a
   * segment that was just swapped-in would have a chance to decompress more out
   * of it. If the system is in high memory pressure state, this may cause the
   * age-q to be completely empty so that there are no candidate segments for
   * swap-out. In this state we use a lower limit of 1 second. condition 1: the
   * age-q absolute size is too low condition 2: there are more segments in
   * swapped-in-q than in age-q each of these represent a bad situation which we
   * want to try to alleviate by moving more segments from swappped-in-q to
   * age-q so that we have a better selection of who to swap-out
   */
  if (c_age_count < C_AGE_Q_COUNT_LOW_THRESHOLD ||
      c_age_count < c_regular_swappedin_count) {
  ...
  while (!queue_empty(&c_regular_swappedin_list_head)) {
    ...
    if (flush_all == FALSE && (now - c_seg->c_swappedin_ts) < limit) {
      /* swappedin q is sorted by the order of time of addition os if we reached
       * a seg that's too young, we know that all the rest after it are also too
       * young */
      break;
    }
...
void vm_compressor_flush(void) {
  ...
  /* need to wait here since the swap thread may also be running in parallel and
   * handling segments */
  while (!queue_empty(&c_early_swapout_list_head) ||
         !queue_empty(&c_regular_swapout_list_head) ||
         !queue_empty(&c_late_swapout_list_head)) {
    assert_wait_timeout((event_t)&compaction_swapper_running,
                        THREAD_INTERRUPTIBLE, 5000, 1000 * NSEC_PER_USEC);
  ...
  if (vm_swap_put_failures > vm_swap_put_failures_at_start) {
    HIBLOG("vm_compressor_flush failed to clean %llu segments - "
  ...
  HIBLOG("vm_compressor_flush completed - took %qd msecs - vm_num_swap_files = "
         "%d, vm_num_pinned_swap_files = %d, vm_swappin_enabled = %d\n",
         nsec / 1000000ULL, vm_num_swap_files, vm_num_pinned_swap_files,
         vm_swappin_enabled);
}
...
void vm_compressor_do_warmup(void) {
  ...
  if (first_c_segment_to_warm_generation_id ==
      last_c_segment_to_warm_generation_id) {
    first_c_segment_to_warm_generation_id =
        last_c_segment_to_warm_generation_id = 0;
...
void do_fastwake_warmup_all(void) {
  ...
  if (queue_empty(&c_swappedout_list_head) &&
...
void do_fastwake_warmup(queue_head_t *c_queue, boolean_t consider_all_cseg) {
  c_segment_t c_seg = NULL;
  AbsoluteTime startTime, endTime;
  uint64_t nsec;
  ...
  proc_set_thread_policy(current_thread(), TASK_POLICY_INTERNAL, TASK_POLICY_IO,
...
void vm_compressor_compact_and_swap(boolean_t flush_all) {
  c_segment_t c_seg;
  bool switch_state, bail_wanted_cseg = false;
  clock_sec_t now;
  clock_nsec_t nsec;
  mach_timespec_t start_ts, end_ts;
  unsigned int number_considered, wanted_cseg_found,
      yield_after_considered_per_pass, number_yields;
  uint64_t bytes_freed, delta_usec;
  uint32_t c_swapout_count = 0;

  VM_DEBUG_CONSTANT_EVENT(vm_compressor_compact_and_swap,
                          DBG_VM_COMPRESSOR_COMPACT_AND_SWAP, DBG_FUNC_START,
                          c_age_count, c_minor_count, c_major_count,
                          vm_page_free_count);
  ...
  if (fastwake_warmup == TRUE) {
    ...
    KERNEL_DEBUG_CONSTANT(IOKDBG_CODE(DBG_HIBERNATE, 11) | DBG_FUNC_START,
                          c_segment_warmup_count,
                          first_c_segment_to_warm_generation_id,
                          last_c_segment_to_warm_generation_id, 0, 0);
    do_fastwake_warmup(&c_swappedout_list_head, FALSE);
    KERNEL_DEBUG_CONSTANT(
        IOKDBG_CODE(DBG_HIBERNATE, 11) | DBG_FUNC_END, c_segment_warmup_count,
        c_segment_warmup_count - starting_warmup_count, 0, 0, 0);
  ...
  vm_compressor_process_major_segments(
      false /*all segments and not just the ripe-aged ones*/);
#endif /* (XNU_TARGET_OS_OSX && __arm64__) */
  ...
  /**
   * SW: Need to figure out how to properly rate limit this log because it is
   * currently way too noisy. rdar://99379414 (Figure out how to rate limit the
   * fragmentation level logging)
   */
  vm_log_debug("before compaction fragmentation level %u\n",
               vm_compressor_fragmentation_level());

  while (!queue_empty(&c_age_list_head) && !compaction_swapper_abort &&
    ...
    c_swapout_count =
        c_early_swapout_count + c_regular_swapout_count + c_late_swapout_count;
    if (VM_CONFIG_SWAP_IS_ACTIVE && !vm_swap_out_of_space() &&
        c_swapout_count >= C_SWAPOUT_LIMIT) {
      assert_wait_timeout((event_t)&compaction_swapper_running,
                          THREAD_INTERRUPTIBLE, 100, 1000 * NSEC_PER_USEC);
    ...
    if (flush_all == TRUE &&
        c_seg->c_generation_id > c_generation_id_flush_barrier) {
      VM_DEBUG_CONSTANT_EVENT(vm_compressor_compact_and_swap,
                              DBG_VM_COMPRESSOR_COMPACT_AND_SWAP, DBG_FUNC_NONE,
                              5, 0, 0, 0);
      break;
    ...
    if (c_seg->c_busy) {
      VM_DEBUG_CONSTANT_EVENT(vm_compressor_compact_and_swap,
                              DBG_VM_COMPRESSOR_COMPACT_AND_SWAP, DBG_FUNC_NONE,
                              6, (void *)VM_KERNEL_ADDRPERM(c_seg), 0, 0);
    ...
    if (switch_state) {
...
#if (XNU_TARGET_OS_OSX && __arm64__)
        if (flush_all == false &&
            compressor_swapout_conditions_met() == false) {
      ...
      if (c_seg->c_state == C_ON_AGE_Q) {
        ...
        c_seg_major_compact_stats[c_seg_major_compact_stats_now]
            .wasted_space_in_swapouts += c_seg_bufsize - c_seg->c_bytes_used;
        c_seg_major_compact_stats[c_seg_major_compact_stats_now]
            .count_of_swapouts++;
      }
    }
    ...
    /*
     * On systems _with_ general swap, regardless of jetsam, we wake up the
     * swapout thread here. On systems _without_ general swap, it's the
     * responsibility of the memorystatus subsystem to wake up the swapper.
     * TODO: When we have full jetsam support on a swap enabled system, we will
    ...
    if (number_considered >= yield_after_considered_per_pass) {
      if (wanted_cseg_found) {
        ...
        VM_DEBUG_CONSTANT_EVENT(
            vm_compressor_compact_and_swap, DBG_VM_COMPRESSOR_COMPACT_AND_SWAP,
            DBG_FUNC_NONE, 11, number_considered, number_yields, 0);
  ...
  end_ts = major_compact_ts =
      (mach_timespec_t){.tv_sec = (int)now, .tv_nsec = nsec};

  SUB_MACH_TIMESPEC(&end_ts, &start_ts);

  delta_usec = (end_ts.tv_sec * USEC_PER_SEC) +
               (end_ts.tv_nsec / NSEC_PER_USEC) - (number_yields * 100);

  delta_usec = MAX(
      1, delta_usec); /* we could have 0 usec run if conditions weren't right */

  c_seg_major_compact_stats[c_seg_major_compact_stats_now].bytes_freed_rate_us =
      (bytes_freed / delta_usec);
  ...
  VM_DEBUG_CONSTANT_EVENT(vm_compressor_compact_and_swap,
                          DBG_VM_COMPRESSOR_COMPACT_AND_SWAP, DBG_FUNC_END,
                          c_age_count, c_minor_count, c_major_count,
                          vm_page_free_count);
}

static c_segment_t c_seg_allocate(c_segment_t *current_chead,
                                  bool *nearing_limits) {
  c_segment_t c_seg;
  int min_needed;
  int size_to_populate;
  ...
  *nearing_limits = false;
  ...
  if ((c_seg = *current_chead) == NULL) {
    ...
    if (c_free_segno_head == (uint32_t)-1) {
      ...
      /* pages for c_segments are never depopulated, c_segments_available never
       * goes down */
      kernel_memory_populate((vm_offset_t)c_segments_next_page, PAGE_SIZE,
                             KMA_NOFAIL | KMA_KOBJECT,
                             VM_KERN_MEMORY_COMPRESSOR);
      ...
      for (c_segno = c_segments_available + 1;
           c_segno < c_segments_available_new; c_segno++) {
        c_segments_get(c_segno - 1)->c_segno =
            c_segno; /* next free is the one after you */
      }

      lck_mtx_lock_spin_always(c_list_lock);

      c_segments_get(c_segno - 1)->c_segno =
          c_free_segno_head; /* link to the rest of, existing freelist */
      c_free_segno_head = c_segments_available; /* first one in the page that
                                                   was just allocated */
    ...
    assert(c_segments_get(c_segno)->c_segno >
           c_segments_available); /* we just assigned a pointer to it so this is
                                     an indication that it is occupied */
    lck_mtx_unlock_always(c_list_lock);

    for (int i = 0; i < vm_pageout_state.vm_compressor_thread_count; i++) {
#if XNU_TARGET_OS_OSX /* tag:DONATE */
      donate_queue_head = (c_segment_t *)&(
          pgo_iothread_internal_state[i].current_early_swapout_chead);
#else  /* XNU_TARGET_OS_OSX */
      if (memorystatus_swap_all_apps) {
        donate_queue_head = (c_segment_t *)&(
            pgo_iothread_internal_state[i].current_late_swapout_chead);
      } else {
  ...
  size_to_populate =
      c_seg_allocsize - C_SEG_OFFSET_TO_BYTES(c_seg->c_populated_offset);

  if (size_to_populate) {
    min_needed = PAGE_SIZE + (c_seg_allocsize - c_seg_bufsize);

    if (C_SEG_OFFSET_TO_BYTES(c_seg->c_populated_offset - c_seg->c_nextoffset) <
        (unsigned)min_needed) {
      if (size_to_populate > C_SEG_MAX_POPULATE_SIZE) {
        size_to_populate = C_SEG_MAX_POPULATE_SIZE;
      }

      os_atomic_add(&vm_pageout_vminfo.vm_compressor_pages_grabbed,
                    size_to_populate / PAGE_SIZE, relaxed);
...
#endif /* CONFIG_FREEZE */
#endif /* DEVELOPMENT || DEBUG */
uint64_t c_seg_total_donated_bytes =
    0; /* For testing/debugging only for now. Remove and add new counters for
          vm_stat.*/
...
static void c_current_seg_filled(c_segment_t c_seg,
  ...
  unused_bytes = trunc_page_32(
      C_SEG_OFFSET_TO_BYTES(c_seg->c_populated_offset - c_seg->c_nextoffset));

  if (unused_bytes) {
    /* if this is a platform that need an extra page at the end of the segment
     * when running compress then now is the time to depopulate that extra page.
     * it still takes virtual space but doesn't actually waste memory */
    offset_to_depopulate = C_SEG_BYTES_TO_OFFSET(
        round_page_32(C_SEG_OFFSET_TO_BYTES(c_seg->c_nextoffset)));
    ...
    kernel_memory_depopulate(
        ...
        unused_bytes, KMA_COMPRESSOR, VM_KERN_MEMORY_COMPRESSOR);
...
#if CONFIG_FREEZE
  if (current_chead ==
          (c_segment_t *)&(freezer_context_global.freezer_ctx_chead) &&
      VM_CONFIG_SWAP_IS_PRESENT && VM_CONFIG_FREEZER_SWAP_IS_ACTIVE
#if DEVELOPMENT || DEBUG
...
#endif /* XNU_TARGET_OS_OSX */
      if (current_chead == donate_queue_head) {
        /* This is the place where the "donating" task actually does the
         * so-called donation Instead of continueing to take place in memory in
         * the compressor, the segment goes directly to swap-out instead of
         * going to AGE_Q */
        assert(c_seg->c_has_donated_pages);
        new_state = C_ON_SWAPOUT_Q;
  ...
  if (current_chead ==
    ...
    if (freezer_context_global.freezer_ctx_task->donates_own_pages) {
      assert(!c_seg->c_has_donated_pages);
      c_seg->c_has_donated_pages = 1;
      os_atomic_add(&c_segment_pages_compressed_incore_late_swapout,
                    c_seg->c_slots_used, relaxed);
    }
  ...
  if (c_seg->c_state == C_ON_AGE_Q && C_SEG_UNUSED_BYTES(c_seg) >= PAGE_SIZE) {
    /* this is possible if we decompressed a page from the segment before it
     * ended filling */
#if CONFIG_FREEZE
 ...
 */
void c_seg_swapin_requeue(c_segment_t c_seg, boolean_t has_data,
  ...
  if (has_data == TRUE) {
...
#if CONFIG_FREEZE
      /*
       * If a segment has both identities, frozen and donated bits set, the
       * donated bit wins on the swapin path. This is because the segment is
       * being swapped back in and so is in demand and should be given more time
       * to spend in memory before being swapped back out under pressure.
       */
      if (c_seg->c_has_donated_pages) {
    ...
    if (minor_compact_ok == TRUE && !c_seg->c_on_minorcompact_q &&
        C_SEG_UNUSED_BYTES(c_seg) >= PAGE_SIZE) {
      c_seg_need_delayed_compaction(c_seg, TRUE);
    }
  } else {
...
int c_seg_swapin(c_segment_t c_seg, boolean_t force_minor_compaction,
                 boolean_t age_on_swapin_q) {
  vm_offset_t addr = 0;
  uint32_t io_size = 0;
  uint64_t f_offset;
  ...
  if (vm_swap_get(c_seg, f_offset, io_size) != KERN_SUCCESS) {
    ...
    if (c_seg->cseg_hash !=
        vmc_hash((char *)c_seg->c_store.c_buffer, (int)io_size)) {
    ...
    c_seg_swapin_requeue(c_seg, TRUE,
                         force_minor_compaction == TRUE ? FALSE : TRUE,
                         age_on_swapin_q);
    ...
    os_atomic_add(&c_segment_pages_compressed_incore, c_seg->c_slots_used,
                  relaxed);
    if (c_seg->c_has_donated_pages) {
      os_atomic_add(&c_segment_pages_compressed_incore_late_swapout,
                    c_seg->c_slots_used, relaxed);
...
/*
 * TODO: refactor the CAS loops in c_segment_sv_hash_drop_ref() and
 * c_segment_sv_hash_instert() to os_atomic_rmw_loop() [rdar://139546215]
 */

static void c_segment_sv_hash_drop_ref(int hash_indx) {
  ...
  while (1) {
    ...
    if (OSCompareAndSwap64(
            (UInt64)o_sv_he.he_record, (UInt64)n_sv_he.he_record,
            (UInt64 *)&c_segment_sv_hash_table[hash_indx].he_record) == TRUE) {
      if (n_sv_he.he_ref == 0) {
...
static int c_segment_sv_hash_insert(uint32_t data) {
  int hash_sindx;
  int misses;
  struct c_sv_hash_entry o_sv_he, n_sv_he;
  ...
  for (misses = 0; misses < C_SV_HASH_MAX_MISS; misses++) {
    ...
    while (o_sv_he.he_data == data || o_sv_he.he_ref == 0) {
      ...
      if (OSCompareAndSwap64(
              (UInt64)o_sv_he.he_record, (UInt64)n_sv_he.he_record,
              (UInt64 *)&c_segment_sv_hash_table[hash_sindx].he_record) ==
          TRUE) {
        if (n_sv_he.he_ref == 1) {
...
static void c_compressed_record_data(char *src, int c_size) {
 ...
 * @param slot_ptr [OUT] fill the slot-mapping of the c_seg+slot where the page
 * ends up being stored
 * @param current_chead [IN-OUT] current filling c_seg. pointer comes from the
 * current compression thread state On the very first call this is going to
 * point to NULL and this function will fill that pointer with a new filling
 * c_sec if the current filling c_seg doesn't have enough space, it will be
 * replaced in this location with a new filling c_seg
 * @param scratch_buf [IN] pointer from the current thread state, used by the
 * compression codec
 * @return KERN_RESOURCE_SHORTAGE if the compressor has been exhausted
 */
static kern_return_t c_compress_page(char *src, c_slot_mapping_t slot_ptr,
                                     c_segment_t *current_chead,
                                     char *scratch_buf,
...
retry: /* may need to retry if the currently filling c_seg will not have enough
          space */
  ...
  unsigned int avail_space =
      c_seg_bufsize - C_SEG_OFFSET_TO_BYTES((int32_t)cs->c_offset);
...
#endif
  boolean_t incomp_copy =
      FALSE; /* codec indicates it already did copy an incompressible page */
  /* The SW codec case needs 4 bytes for its header and these are not accounted
   * for in the bytes_budget argument. Also, the the SV-not-in-hash case needs 4
   * bytes. */
  ...
  if (max_csize > 0 && max_csize_adj > 0) {
    if (vm_compressor_algorithm() != VM_COMPRESSOR_DEFAULT_CODEC) {
      ...
      if (max_csize >= C_SEG_OFFSET_ALIGNMENT_BOUNDARY) {
        vm_memtag_disable_checking();
        c_size = metacompressor(
            (const uint8_t *)src,
            (uint8_t *)&c_seg->c_store.c_buffer[cs->c_offset], max_csize_adj,
            &ccodec, scratch_buf, &incomp_copy, &inline_popcount);
        vm_memtag_enable_checking();
        assert(inline_popcount == C_SLOT_NO_POPCOUNT);

#if C_SEG_OFFSET_ALIGNMENT_BOUNDARY > 4
        /* The case of HW codec doesn't detect overflow on its own, instead it
         * spills the the next page and we need to detect this happened */
      ...
      vm_memtag_disable_checking();
      cs->c_codec = CCWK;
      __unreachable_ok_push if (PAGE_SIZE == 4096) {
        c_size = WKdm_compress_4k(
            (WK_word *)(uintptr_t)src,
            (WK_word *)(uintptr_t)&c_seg->c_store.c_buffer[cs->c_offset],
            (WK_word *)(uintptr_t)scratch_buf, max_csize_adj);
      }
      else {
        c_size = WKdm_compress_16k(
            (WK_word *)(uintptr_t)src,
            (WK_word *)(uintptr_t)&c_seg->c_store.c_buffer[cs->c_offset],
            (WK_word *)(uintptr_t)scratch_buf, max_csize_adj);
      }
      __unreachable_ok_pop vm_memtag_enable_checking();
#else
      vm_memtag_disable_checking();
      c_size = WKdm_compress_new(
          (const WK_word *)(uintptr_t)src,
          (WK_word *)(uintptr_t)&c_seg->c_store.c_buffer[cs->c_offset],
          (WK_word *)(uintptr_t)scratch_buf, max_csize_adj);
      vm_memtag_enable_checking();
#endif
  ...
  /* c_size is the size written by the codec, or 0 if it's uniform 32 bit value
   * or (-1 if there was not enough space or it was incompressible) */
  assertf(((c_size <= max_csize_adj) && (c_size >= -1)),
          "c_size invalid (%d, %d), cur compressions: %d", c_size,
          max_csize_adj, c_segment_pages_compressed);
  ...
  if (c_size == -1) {
    if (max_csize < PAGE_SIZE) {
       ...
       * right now this assumes that if the space we had is > PAGE_SIZE, then
       * the codec failed due to incompressible input */

      PAGE_REPLACEMENT_DISALLOWED(FALSE);
      goto retry; /* previous c_seg didn't have enough space, we finalized it
                     and can try again with a fresh c_seg */
    }
    c_size = PAGE_SIZE; /* tag:WK-INCOMPRESSIBLE */

    if (incomp_copy ==
        FALSE) { /* codec did not copy the incompressible input */
      vm_memtag_disable_checking();
...
#endif
        /* we didn't write anything to c_buffer and didn't end up using the slot
         * in the c_seg at all, so skip all the book-keeping of the case that we
         * did */
        goto sv_compression;
      }
    }
...
#if RECORD_THE_COMPRESSED_DATA
  c_compressed_record_data((char *)&c_seg->c_store.c_buffer[cs->c_offset],
                           c_size);
#endif
#if CHECKSUM_THE_COMPRESSED_DATA
  cs->c_hash_compressed_data =
      vmc_hash((char *)&c_seg->c_store.c_buffer[cs->c_offset], c_size);
#endif
#if POPCOUNT_THE_COMPRESSED_DATA
  cs->c_pop_cdata =
      vmc_pop((uintptr_t)&c_seg->c_store.c_buffer[cs->c_offset], c_size);
#endif
...
#if CONFIG_FREEZE
  /* TODO: should c_segment_pages_compressed be up here too? See 88598046 for
   * details */
  ...
  /* <csegno=0,indx=0> would mean "empty slot", so use csegno+1, see other
   * usages of s_cseg where it's decremented */
...
sv_compression:
  ...
  if (c_seg->c_nextoffset >= c_seg_off_limit ||
      c_seg->c_nextslot >= C_SLOT_MAX_INDEX) {
    /* condition 1: segment buffer is almost full, don't bother trying to fill
     * it further. condition 2: we can't have any more slots in this c_segment
     * even if we had buffer space */
    c_current_seg_filled(c_seg, current_chead);
    assert(*current_chead == NULL);
  }
...
#if RECORD_THE_COMPRESSED_DATA
  if ((c_compressed_record_cptr - c_compressed_record_sbuf) >=
      c_seg_allocsize) {
    c_compressed_record_write(
        c_compressed_record_sbuf,
        (int)(c_compressed_record_cptr - c_compressed_record_sbuf));
  ...
  KERNEL_DEBUG(0xe0400000 | DBG_FUNC_END, *current_chead, c_size,
               c_segment_input_bytes, c_segment_compressed_bytes, 0);
...
static vm_decompress_result_t c_decompress_page(
    char *dst,
    volatile c_slot_mapping_t
        slot_ptr, /* why volatile? perhaps due to changes across hibernation */
    vm_compressor_options_t flags, int *zeroslot) {
  c_slot_t cs;
  c_segment_t c_seg;
  uint32_t c_segno;
  uint16_t c_indx;
...
ReTry:
  ...
  if (__improbable(c_segno >= c_segments_available)) {
    panic("c_decompress_page: c_segno %d >= c_segments_available %d, "
          "slot_ptr(%p), slot_data(%x)",
...
bypass_busy_check:
  ...
  if (__improbable(c_indx >= c_seg->c_nextslot)) {
    panic("c_decompress_page: c_indx %d >= c_nextslot %d, c_seg(%p), "
          "slot_ptr(%p), slot_data(%x)",
          c_indx, c_seg->c_nextslot, c_seg, slot_ptr,
          *(int *)((void *)slot_ptr));
  }
  ...
  if (__improbable(c_size == 0)) { /* sanity check it's not an empty slot */
    panic("c_decompress_page: c_size == 0, c_seg(%p), slot_ptr(%p), "
          "slot_data(%x)",
          c_seg, slot_ptr, *(int *)((void *)slot_ptr));
  }

  c_rounded_size = C_SEG_ROUND_TO_ALIGNMENT(c_size + c_slot_extra_size(cs));
  /* c_rounded_size should not change after this point so that it remains
   * consistent on all branches */
  ...
  if (dst) { /* would be NULL if we don't want the page content, from free */
...
#if CONFIG_FREEZE
      if (freezer_incore_cseg_acct) {
        if ((c_seg->c_slots_used + c_segment_pages_compressed_incore) >=
            c_segment_pages_compressed_nearing_limit) {
          PAGE_REPLACEMENT_DISALLOWED(FALSE);
          lck_mtx_unlock_always(&c_seg->c_lock);
    ...
    uintptr_t csvaddr = (uintptr_t)&c_seg->c_store.c_buffer[cs->c_offset];
    if (cs->c_pop_cdata != (csvpop = vmc_pop(csvaddr, c_size))) {
      panic("Compressed data popcount doesn't match original, bit distance: %d "
            "%p (phys: %p) %p %p 0x%x 0x%x 0x%x 0x%x",
            (csvpop - cs->c_pop_cdata), (void *)csvaddr,
            (void *)kvtophys(csvaddr), c_seg, cs, cs->c_offset, c_size, csvpop,
            cs->c_pop_cdata);
    }
#endif
...
#if CHECKSUM_THE_COMPRESSED_DATA
    unsigned csvhash;
    if (cs->c_hash_compressed_data !=
        (csvhash = vmc_hash((char *)&c_seg->c_store.c_buffer[cs->c_offset],
                            c_size))) {
      panic("Compressed data doesn't match original %p %p %u %u %u", c_seg, cs,
            c_size, cs->c_hash_compressed_data, csvhash);
    }
#endif
    if (c_size == PAGE_SIZE) { /* tag:WK-INCOMPRESSIBLE */
      ...
      if (vm_compressor_algorithm() != VM_COMPRESSOR_DEFAULT_CODEC) {
        ...
        vm_memtag_disable_checking();
        if (!metadecompressor(
                (const uint8_t *)&c_seg->c_store.c_buffer[cs->c_offset],
        ...
        __unreachable_ok_push if (PAGE_SIZE == 4096) {
          WKdm_decompress_4k(
              (WK_word *)(uintptr_t)&c_seg->c_store.c_buffer[cs->c_offset],
              (WK_word *)(uintptr_t)dst, (WK_word *)(uintptr_t)scratch_buf,
              c_size);
        }
        else {
          WKdm_decompress_16k(
    ...
    if (cs->c_hash_data != vmc_hash(dst, PAGE_SIZE)) {
      ...
      panic("decompressed data doesn't match original cs: %p, hash: 0x%x, "
            "offset: %d, c_size: %d, c_rounded_size: %d, codec: %d, header: "
            "0x%x 0x%x 0x%x",
            cs, cs->c_hash_data, cs->c_offset, c_size, c_rounded_size,
            cs->c_codec, *dinput, *(dinput + 1), *(dinput + 2));
#else  /* defined(__arm64__) */
      panic("decompressed data doesn't match original cs: %p, hash: %d, "
            "offset: 0x%x, c_size: %d",
            cs, cs->c_hash_data, cs->c_offset, c_size);
    ...
    if (c_seg->c_swappedin_ts == 0 && !kdp_mode) {
      ...
      if (age_of_cseg < DECOMPRESSION_SAMPLE_MAX_AGE) {
        os_atomic_inc(&age_of_decompressions_during_sample_period[age_of_cseg],
                      relaxed);
      } else {
    ...
    /*
     * We are freeing an uncompressed page from this c_seg and so balance the
     * ledgers.
     */
    if (C_SEG_IS_ONDISK(c_seg)) {
       ...
       */
      if (c_seg->c_task_owner) {
        task_update_frozen_to_swap_acct(c_seg->c_task_owner, PAGE_SIZE_64,
                                        DEBIT_FROM_SWAP);
      }

      /*
       * We are freeing a page in swap without swapping it in. We bump the
       * in-core count here to simulate a swapin of a page so that we can
       * accurately decrement it below.
       */
      os_atomic_inc(&c_segment_pages_compressed_incore, relaxed);
      if (c_seg->c_has_donated_pages) {
        os_atomic_inc(&c_segment_pages_compressed_incore_late_swapout, relaxed);
      ...
      *zeroslot = 0;
      ...
      goto done; /* this is intended to avoid the decrement of
                    c_segment_pages_compressed_incore below */
#endif           /* CONFIG_FREEZE */
    }
  }
  ...
  assertf(c_segment_pages_compressed_incore >= 0, "-ve incore count %p 0x%x",
          c_seg, c_segment_pages_compressed_incore);
  if (c_seg->c_has_donated_pages) {
    os_atomic_dec(&c_segment_pages_compressed_incore_late_swapout, relaxed);
    assertf(c_segment_pages_compressed_incore_late_swapout >= 0,
            "-ve lateswapout count %p 0x%x", c_seg,
    ...
    if (c_seg->c_bytes_used == 0) {
      if (!(C_SEG_IS_ONDISK(c_seg))) {
        ...
        if (pages_populated) {
          ...
          kernel_memory_depopulate((vm_offset_t)c_seg->c_store.c_buffer,
        ...
        if (!c_seg->c_on_minorcompact_q && c_seg->c_state != C_ON_SWAPIO_Q) {
          if (c_seg->c_state == C_ON_SWAPOUT_Q) {
            /* If we're on the swapout q, we want to get out of it since there's
             * no reason to swapout anymore, so put on AGE Q in the meantime
             * until minor compact */
      ...
      if (c_seg->c_state != C_ON_BAD_Q && c_seg->c_state != C_ON_SWAPOUT_Q &&
          c_seg->c_state != C_ON_SWAPIO_Q &&
          C_SEG_UNUSED_BYTES(c_seg) >= PAGE_SIZE) {
        c_seg_need_delayed_compaction(c_seg, FALSE);
      }
    } else if (c_seg->c_state != C_ON_SWAPPEDOUTSPARSE_Q &&
...
inline bool vm_compressor_is_slot_compressed(int *slot) {
#if !CONFIG_TRACK_UNMODIFIED_ANON_PAGES
#pragma unused(slot)
  return true;
#else  /* !CONFIG_TRACK_UNMODIFIED_ANON_PAGES*/
...
vm_decompress_result_t vm_compressor_get(ppnum_t pn, int *slot,
   ...
   * or (flags & C_DONT_BLOCK) and we found 'c_busy' or 'C_SEG_IS_ONDISK' to be
   * TRUE
   */
  if (zeroslot) {
    *slot = 0;
  ...
  /*
   * returns 0 if we successfully decompressed a page from a segment already in
   * memory returns 1 if we had to first swap in the segment, before
   * successfully decompressing the page returns -1 if we encountered an error
   * swapping in the segment - decompression failed returns -2 if (flags &
   * C_DONT_BLOCK) and we found 'c_busy' or 'C_SEG_IS_ONDISK' to be true
   */
  return retval;
}

vm_decompress_result_t vm_compressor_free(int *slot,
                                          vm_compressor_options_t flags) {
  ...
  if (slot_is_compressed) {
     ...
     * returns -1 if we encountered an error swapping in the segment -
     * decompression failed returns -2 if (flags & C_DONT_BLOCK) and we found
     * 'c_busy' set
     */
...
kern_return_t vm_compressor_put(ppnum_t pn, int *slot, void **current_chead,
                                char *scratch_buf,
                                vm_compressor_options_t flags) {
  char *src;
  kern_return_t kr;
  ...
  /* get the address of the page in the physical apperture in the kernel task
   * virtual memory */
  src = pmap_map_compressor_page(pn);
  assert(src != NULL);

  kr = c_compress_page(src, (c_slot_mapping_t)slot,
                       (c_segment_t *)current_chead, scratch_buf, flags);
  pmap_unmap_compressor_page(pn, src);

  return kr;
}

void vm_compressor_transfer(int *dst_slot_p, int *src_slot_p) {
  ...
  if (src_slot->s_cseg == C_SV_CSEG_ID ||
      !vm_compressor_is_slot_compressed(src_slot_p)) {
...
vm_panic_hibernate_write_image_failed(int err, uint64_t file_size_min,
                                      uint64_t file_size_max,
                                      uint64_t file_size) {
  panic(
      "hibernate_write_image encountered error 0x%x - %u, %u, %d, %d, %d, %d, "
      "%d, %d, %d, %d, %llu, %d, %d, %d, %llu, %llu, %llu\n",
      err, VM_PAGE_COMPRESSOR_COUNT, vm_page_wire_count, c_age_count,
      c_major_count, c_minor_count,
      (c_early_swapout_count + c_regular_swapout_count + c_late_swapout_count),
      c_swappedout_sparse_count, vm_num_swap_files, vm_num_pinned_swap_files,
      vm_swappin_enabled, vm_swap_put_failures,
      (vm_swapfile_last_failed_to_create_ts ? 1 : 0), hibernate_no_swapspace,
      hibernate_flush_timed_out, file_size_min, file_size_max, file_size);
}
#endif /*(__arm64__)*/
...
void vm_compressor_finished_filling(void **current_chead) {
  c_segment_t c_seg;
...
kern_return_t vm_compressor_relocate(void **current_chead, int *slot_p) {
  c_slot_mapping_t slot_ptr;
...
Relookup_src:
  ...
  if (combined_size >
      (uint32_t)(c_seg_bufsize -
                 C_SEG_OFFSET_TO_BYTES((int32_t)c_seg_dst->c_nextoffset))) {
    /*
...
void vm_compressor_inject_error(int *slot) {
  c_slot_mapping_t slot_ptr = (c_slot_mapping_t)slot;
 ...
 * returns true if the segment was written or there's nothing to write for the
 * segno false if there's not enough space argument size input - the size of the
 * input buffer, output - the size written, set to 0 on failure
 */
kern_return_t vm_compressor_serialize_segment_debug_info(
    int segno, char *buf, size_t *size, vm_c_serialize_add_data_t with_data) {
  size_t insize = *size;
  size_t offset = 0;
  *size = 0;
  if (c_segments_get(segno)->c_segno < c_segments_available) {
    /* This check means there's no pointer assigned here so it must be an index
     * in the free list. if this was an active c_segment, .c_seg would be
     * assigned to, which is a pointer, interpreted as an int it would be higher
     * than c_segments_available. See also assert to this effect right after
     * assigning to c_seg in c_seg_allocate()
     */
    return KERN_SUCCESS;
  }
  ...
  if (sizeof(struct c_segment_info) + (nslots * sizeof(struct c_slot_info)) >
      insize) {
...
#endif /* TRACK_C_SEGMENT_UTILIZATION */
  /* This entire data collection races with the compressor threads which can
   * change any of this data members, and specifically can drop the data buffer
   * to swap We don't take the segment lock since that would slow the iteration
   * over the segments down and hurt the "snapshot-ness" of the data. The race
   * risk is acceptable since this is used only for a tester in development. */
...
static uint32_t vm_uncompressed_extract_swap_file(int slot) {
  uint32_t fileidx = (((uint32_t)slot & UNCOMPRESSED_FILEIDX_OFFSET_MASK) >>
                      UNCOMPRESSED_FILEIDX_SHIFT) &
                     UNCOMPRESSED_FILEIDX_MASK;
  return fileidx;
}

static uint32_t vm_uncompressed_extract_swap_offset(int slot) {
  return slot &
         (uint32_t)(~(UNCOMPRESSED_OFFSET_MASK << UNCOMPRESSED_OFFSET_SHIFT));
}

static void vm_uncompressed_return_space_to_swap(int slot) {
  PAGE_REPLACEMENT_ALLOWED(TRUE);
  ...
  if (fileidx == 1) {
...
#endif /*DEVELOPMENT || DEBUG*/
    assertf((uncompressed_file0_space_bitmap[chunkidx] &
             ((uint64_t)1 << chunkoffset)),
    ...
    uint64_t chunkidx = pgidx / 64;
    uint64_t chunkoffset = pgidx % 64;
    assertf((uncompressed_file1_space_bitmap[chunkidx] &
             ((uint64_t)1 << chunkoffset)),
...
static int vm_uncompressed_reserve_space_in_swap() {
  int slot = 0;
  if (uncompressed_file0_free_pages == 0 &&
      uncompressed_file1_free_pages == 0) {
    return -1;
  }
  ...
  if (uncompressed_file0_free_pages) {
    ...
    while (uncompressed_file0_space_bitmap[chunkidx] &
           ((uint64_t)1 << chunkoffset)) {
      chunkoffset++;
    }

    assertf((uncompressed_file0_space_bitmap[chunkidx] &
             ((uint64_t)1 << chunkoffset)) == 0,
            "%llu %llu", chunkidx, chunkoffset);
#if DEVELOPMENT || DEBUG
    uint64_t vaddr = (uint64_t)&uncompressed_file0_space_bitmap[chunkidx];
    uint64_t maxvaddr =
...
#endif /*DEVELOPMENT || DEBUG*/
    uncompressed_file0_space_bitmap[chunkidx] |= ((uint64_t)1 << chunkoffset);
    uncompressed_file0_free_offset =
        ((chunkidx * 64) + chunkoffset) * PAGE_SIZE_64;
    assertf((uncompressed_file0_space_bitmap[chunkidx] &
             ((uint64_t)1 << chunkoffset)),
            "%llu %llu", chunkidx, chunkoffset);

    assert(uncompressed_file0_free_offset <= (1 << UNCOMPRESSED_OFFSET_SHIFT));
    slot = (int)((1 << UNCOMPRESSED_FILEIDX_SHIFT) +
                 uncompressed_file0_free_offset);
    uncompressed_file0_free_pages--;
  } else {
    uint64_t chunkidx = 0;
    ...
    while (uncompressed_file1_space_bitmap[chunkidx] &
           ((uint64_t)1 << chunkoffset)) {
      chunkoffset++;
    }
    assert((uncompressed_file1_space_bitmap[chunkidx] &
            ((uint64_t)1 << chunkoffset)) == 0);
    uncompressed_file1_space_bitmap[chunkidx] |= ((uint64_t)1 << chunkoffset);
    uncompressed_file1_free_offset =
        ((chunkidx * 64) + chunkoffset) * PAGE_SIZE_64;
    slot = (int)((2 << UNCOMPRESSED_FILEIDX_SHIFT) +
                 uncompressed_file1_free_offset);
    uncompressed_file1_free_pages--;
  }
  compressor_ro_uncompressed_swap_usage++;
...
int vm_uncompressed_put(ppnum_t pn, int *slot) {
  int retval = 0;
...
again:
  if (uncompressed_vp0 == NULL) {
    ...
    if (uncompressed_vp0 == NULL) {
      ...
      bzero(uncompressed_file0_space_bitmap,
            sizeof(uint64_t) * MAX_OFFSET_PAGES);

      int i = 0;
      for (; i < MAX_IO_REQ; i++) {
        kmem_alloc(kernel_map, (vm_offset_t *)&uncompressor_io_req[i].addr,
                   PAGE_SIZE_64, KMA_NOFAIL | KMA_KOBJECT,
                   VM_KERN_MEMORY_COMPRESSOR);
      ...
      bzero(uncompressed_file1_space_bitmap,
            sizeof(uint64_t) * MAX_OFFSET_PAGES);
      PAGE_REPLACEMENT_ALLOWED(FALSE);
    } else {
      PAGE_REPLACEMENT_ALLOWED(FALSE);
  ...
  if ((retval = vnode_getwithref(uncompressed_vp)) != 0) {
    vm_log_error(
        "vm_uncompressed_put: vnode_getwithref on swapfile failed with %d\n",
        retval);
  } else {
    int i = 0;
  retry:
    ...
    retval = vm_swapfile_io(uncompressed_vp, uncompress_offset,
                            (uint64_t)uncompressor_io_req[i].addr, 1,
                            SWAP_WRITE, NULL);
    if (retval) {
      *slot = 0;
    } else {
...
int vm_uncompressed_get(ppnum_t pn, int *slot,
                        __unused vm_compressor_options_t flags) {
  int retval = 0;
  struct vnode *uncompressed_vp = NULL;
  uint32_t fileidx = vm_uncompressed_extract_swap_file(*slot);
  ...
  retry:
    ...
    PAGE_REPLACEMENT_ALLOWED(FALSE);
    retval = vm_swapfile_io(uncompressed_vp, uncompress_offset,
                            (uint64_t)uncompressor_io_req[i].addr, 1, SWAP_READ,
                            NULL);
    vnode_put(uncompressed_vp);
    void *addr = pmap_map_compressor_page(pn);
...
int vm_uncompressed_free(int *slot, __unused vm_compressor_options_t flags) {
