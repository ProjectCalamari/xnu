...
kern_return_t
compressor_memory_object_init(memory_object_t mem_obj,
...
kern_return_t compressor_memory_object_data_request(
    ...
    memory_object_cluster_size_t length, __unused vm_prot_t protection_required,
...
kern_return_t
compressor_memory_object_data_initialize(memory_object_t mem_obj,
                                         memory_object_offset_t offset,
                                         memory_object_cluster_size_t size);
kern_return_t compressor_memory_object_map(__unused memory_object_t mem_obj,
...
#define compressor_pager_lookup(_mem_obj_, _cpgr_)                             \
  MACRO_BEGIN                                                                  \
  if (_mem_obj_ == NULL || _mem_obj_->mo_pager_ops != &compressor_pager_ops) { \
    _cpgr_ = NULL;                                                             \
  } else {                                                                     \
...
#define compressor_pager_lock(_cpgr_) lck_mtx_lock(&(_cpgr_)->cpgr_lock)
#define compressor_pager_unlock(_cpgr_) lck_mtx_unlock(&(_cpgr_)->cpgr_lock)
...
#define COMPRESSOR_SLOTS_PER_CHUNK                                             \
  (COMPRESSOR_SLOTS_CHUNK_SIZE / sizeof(compressor_slot_t))
...
unsigned int compressor_pager_slots_chunk_free(compressor_slot_t *chunk,
                                               ...
                                               int *failures);
void compressor_pager_slot_lookup(compressor_pager_t pager, boolean_t do_alloc,
                                  memory_object_offset_t offset,
...
static const size_t compressor_slots_zones_sizes[NUM_SLOTS_ZONES] = {
    16, 64, COMPRESSOR_SLOTS_CHUNK_SIZE};

static const char *compressor_slots_zones_names[NUM_SLOTS_ZONES] = {
    "compressor_slots.16", "compressor_slots.64", "compressor_slots.512"};

static zone_t compressor_slots_zones[NUM_SLOTS_ZONES];

#endif /* defined(__LP64__) */

static void zfree_slot_array(compressor_slot_t *slots, size_t size);
static compressor_slot_t *zalloc_slot_array(size_t size, zalloc_flags_t);

static inline unsigned int
compressor_pager_num_chunks(compressor_pager_t pager) {
  ...
  if (num_chunks * COMPRESSOR_SLOTS_PER_CHUNK < pager->cpgr_num_slots) {
    num_chunks++; /* do the equivalent of ceil() instead of trunc() for the
                     above division */
  }
  return num_chunks;
}
...
kern_return_t compressor_memory_object_init(
    memory_object_t mem_obj, memory_object_control_t control,
...
kern_return_t
compressor_memory_object_last_unmap(__unused memory_object_t mem_obj) {
...
compressor_memory_object_data_initialize(memory_object_t mem_obj,
  ...
  for (cur_offset = offset; cur_offset < offset + size;
 ...
 */
kern_return_t compressor_memory_object_create(memory_object_size_t new_size,
  ...
  if ((uint32_t)(new_size / PAGE_SIZE) != (new_size / PAGE_SIZE)) {
    ...
    panic("%s: size 0x%llx overflow", __FUNCTION__, (uint64_t)new_size);
  ...
  num_chunks = compressor_pager_num_chunks(pager);
  if (num_chunks > 1) {
    /* islots points to an array of chunks pointer. every chunk has
     * 512/sizeof(int)=128 slot_mapping */
    pager->cpgr_slots.cpgr_islots =
        kalloc_type(compressor_slot_t *, num_chunks, Z_WAITOK | Z_ZERO);
  } else if (pager->cpgr_num_slots > 2) {
    pager->cpgr_slots.cpgr_dslots = zalloc_slot_array(
        pager->cpgr_num_slots * sizeof(pager->cpgr_slots.cpgr_dslots[0]),
        Z_WAITOK | Z_ZERO);
  } else {
...
/* check if this pager has a slot_mapping spot for this page, if so give its
 * position, if not, make place for it */
void compressor_pager_slot_lookup(compressor_pager_t pager, boolean_t do_alloc,
                                  memory_object_offset_t offset,
  ...
  /* offset is relative to the pager, first page of the first vm_object that
   * created the pager has an offset of 0 */
  ...
  if (num_chunks > 1) {
    ...
    if (chunk == NULL && do_alloc) {
      t_chunk =
          zalloc_slot_array(COMPRESSOR_SLOTS_CHUNK_SIZE, Z_WAITOK | Z_ZERO);
...
#if defined(__LP64__)
__startup_func static void vm_compressor_slots_init(void) {
  for (unsigned int idx = 0; idx < NUM_SLOTS_ZONES; idx++) {
    compressor_slots_zones[idx] =
        zone_create(compressor_slots_zones_names[idx],
                    compressor_slots_zones_sizes[idx], ZC_VM);
  }
}
STARTUP(ZALLOC, STARTUP_RANK_MIDDLE, vm_compressor_slots_init);
#endif /* defined(__LP64__) */

static compressor_slot_t *zalloc_slot_array(size_t size, zalloc_flags_t flags) {
#if defined(__LP64__)
...
kern_return_t vm_compressor_pager_put(memory_object_t mem_obj,
                                      memory_object_offset_t offset,
                                      ppnum_t ppnum, void **current_chead,
                                      char *scratch_buf,
  ...
  *compressed_count_delta_p = 0;
  ...
  /* we're looking for the slot_mapping that corresponds to the offset, which
   * vm_compressor_put() is then going to set a value into after it allocates
   * the slot. if the slot_mapping doesn't exist, this will create it */
...
kern_return_t vm_compressor_pager_get(memory_object_t mem_obj,
                                      memory_object_offset_t offset,
  ...
  *my_fault_type = DBG_COMPRESSOR_FAULT;
  ...
  if (kr == KERN_SUCCESS) {
    ...
    retval = vm_compressor_get(
        ppnum, slot_p, (unmodified ? (flags | C_PAGE_UNMODIFIED) : flags));
    if (retval <= DECOMPRESS_FIRST_FAIL_CODE) {
      ktriage_record(
          thread_tid(current_thread()),
          KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_VM, KDBG_TRIAGE_RESERVED,
                              KDBG_TRIAGE_VM_COMPRESSOR_DECOMPRESS_FAILED),
          (uintptr_t)retval /* arg */);
      kr = KERN_MEMORY_FAILURE;
      ...
      *my_fault_type = DBG_COMPRESSOR_SWAPIN_FAULT;
      ...
      assert((flags & C_DONT_BLOCK));
      /*
       * Not a fatal failure because we just retry with a blocking get later. So
       * we skip ktriage to avoid noise.
       */
      kr = KERN_FAILURE;
    }
  }
...
unsigned int vm_compressor_pager_state_clr(memory_object_t mem_obj,
                                           memory_object_offset_t offset) {
  compressor_pager_t pager;
  compressor_slot_t *slot_p;
  unsigned int num_slots_freed;
...
vm_external_state_t
vm_compressor_pager_state_get(memory_object_t mem_obj,
                              memory_object_offset_t offset) {
  compressor_pager_t pager;
  compressor_slot_t *slot_p;
...
unsigned int vm_compressor_pager_reap_pages(memory_object_t mem_obj,
                                            vm_compressor_options_t flags) {
  compressor_pager_t pager;
  unsigned int num_chunks;
  int failures;
  ...
  if (num_chunks > 1) {
    ...
    for (i = 0; i < num_chunks; i++) {
      ...
      if (chunk != NULL) {
        num_slots_freed += compressor_pager_slots_chunk_free(
            chunk, COMPRESSOR_SLOTS_PER_CHUNK, flags, &failures);
        if (failures == 0) {
          pager->cpgr_slots.cpgr_islots[i] = NULL;
          zfree_slot_array(chunk, COMPRESSOR_SLOTS_CHUNK_SIZE);
        }
    ...
    chunk = pager->cpgr_slots.cpgr_dslots;
    num_slots_freed += compressor_pager_slots_chunk_free(
        chunk, pager->cpgr_num_slots, flags, NULL);
  } else {
    chunk = &pager->cpgr_slots.cpgr_eslots[0];
    num_slots_freed += compressor_pager_slots_chunk_free(
        chunk, pager->cpgr_num_slots, flags, NULL);
...
void vm_compressor_pager_transfer(memory_object_t dst_mem_obj,
...
vm_compressor_pager_next_compressed(memory_object_t mem_obj,
                                    memory_object_offset_t offset) {
  compressor_pager_t pager;
  unsigned int num_chunks;
  uint32_t page_num;
  ...
  if (num_chunks == 1) {
    ...
    for (slot_idx = page_num; slot_idx < pager->cpgr_num_slots; slot_idx++) {
      if (chunk[slot_idx] != 0) {
        /* found a non-NULL slot in this chunk */
        return (memory_object_offset_t)slot_idx * PAGE_SIZE;
      }
    }
    return (memory_object_offset_t)-1;
  }
  ...
  for (chunk_idx = page_num / COMPRESSOR_SLOTS_PER_CHUNK,
      slot_idx = page_num % COMPRESSOR_SLOTS_PER_CHUNK;
       chunk_idx < num_chunks; chunk_idx++, slot_idx = 0) {
    chunk = pager->cpgr_slots.cpgr_islots[chunk_idx];
    if (chunk == NULL) {
    ...
    for (; slot_idx < COMPRESSOR_SLOTS_PER_CHUNK; slot_idx++) {
      if (chunk[slot_idx] != 0) {
        /* found an occupied slot in this chunk */
        uint32_t next_slot;

        next_slot = ((chunk_idx * COMPRESSOR_SLOTS_PER_CHUNK) + slot_idx);
        if (next_slot >= pager->cpgr_num_slots) {
          /* went beyond end of object */
          return (memory_object_offset_t)-1;
        }
        return (memory_object_offset_t)next_slot * PAGE_SIZE;
      }
    }
  }
  return (memory_object_offset_t)-1;
...
unsigned int vm_compressor_pager_get_count(memory_object_t mem_obj) {
  compressor_pager_t pager;
...
void vm_compressor_pager_count(memory_object_t mem_obj,
...
#if CONFIG_FREEZE
kern_return_t vm_compressor_pager_relocate(memory_object_t mem_obj,
                                           memory_object_offset_t offset,
                                           void **current_chead) {
  /*
...
kern_return_t vm_compressor_pager_inject_error(memory_object_t mem_obj,
                                               memory_object_offset_t offset) {
                         ...
                         unsigned int *slot_count) /* OUT */
{
  ...
  *slot_count = pager->cpgr_num_slots_occupied;
  ...
  /*
   *  size_t insize = *size;
   *  unsigned int needed_size = 0; // pager->cpgr_num_slots_occupied *
   * sizeof(compressor_slot_t) / sizeof(int); if (needed_size > insize) { return
   * KERN_NO_SPACE;
   *  }
