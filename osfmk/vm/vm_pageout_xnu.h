...
#if CONFIG_IOSCHED
extern int upl_get_cached_tier(upl_t upl);
#endif
...
extern ppnum_t upl_get_highest_page(upl_t upl);
...
extern upl_size_t upl_adjusted_size(upl_t upl, vm_map_offset_t page_mask);
extern vm_object_offset_t upl_adjusted_offset(upl_t upl,
                                              vm_map_offset_t page_mask);
extern vm_object_offset_t upl_get_data_offset(upl_t upl);

extern kern_return_t vm_map_create_upl(vm_map_t map, vm_map_address_t offset,
...
extern void vm_page_free_list(vm_page_t mem, bool prepare_object);

extern kern_return_t vm_page_alloc_list(vm_size_t page_count, kma_flags_t flags,
                                        vm_page_t *list);
...
struct vm_pageout_queue {
  vm_page_queue_head_t
      pgo_pending; /* laundry pages to be processed by pager's iothread */
  unsigned int
      pgo_laundry; /* current count of laundry pages on queue or in flight */
  ...
  uint32_t pgo_busy
      ...
      pgo_draining : 1, pgo_inited : 1, pgo_unused_bits : 26;
};
...
/*
 * This function is redeclared with slightly different parameter types in
 * vfs_cluster.c This should be fixed at a later time.
 */
...
extern void vm_pageout_object_terminate(vm_object_t object);

extern void vm_pageout_cluster(vm_page_t m);

extern void vm_pageout_initialize_page(vm_page_t m);
...
struct upl_fs_verify_info {
  uint8_t *verify_data_ptr; /* verification data (hashes) for the data pages in
                               the UPL */
  uint32_t verify_data_len; /* the digest size per page (can vary depending on
                               the type of hash) */
};
...
struct upl {
  ...
  union {
    ...
    struct upl_fs_verify_info
        *verify_info; /* verification data for the data pages in the UPL */
  } u_fs_un;
...
extern void vector_upl_get_iostate_byindex(upl_t, uint32_t, upl_offset_t *,
                                           upl_size_t *);
...
struct vm_pageout_debug {
  ...
  uint32_t
      vm_pageout_cleaned_reactivated; /* debugging; how many cleaned pages are
                                         found to be referenced on pageout (and
                                         are therefore reactivated) */
...
extern struct pgo_iothread_state
