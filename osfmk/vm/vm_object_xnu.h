...
struct vm_object_fault_info {
  ...
  unsigned int
      ...
      /* boolean_t */ fi_no_sleep : 1, __vm_object_fault_info_unused_bits : 19;
  int pmap_options;
};
...
struct vm_object {
  ...
  unsigned int
      wired_page_count; /* number of wired pages
  ...
  uint32_t vo_inherit_copy_none : 1, __vo_unused_padding : 31;
  ...
  unsigned int wimg_bits : 8,  /* cache WIMG bits         */
      ...
      phantom_isssd : 1, volatile_empty : 1, volatile_fault : 1,
      all_reusable : 1, blocked_access : 1, set_cache_attr : 1,
      object_is_shared_cache : 1, purgeable_queue_type : 2,
      purgeable_queue_group : 3, io_tracking : 1, no_tag_update : 1, /*  */
#if CONFIG_SECLUDED_MEMORY
      eligible_for_secluded : 1, can_grab_secluded : 1,
      ...
      vo_ledger_tag : 3, vo_no_footprint : 1;
  ...
  /* This value is used for selecting a chead in the compressor for internal
   * objects. see rdar://140849693 for a possible way to implement the
   * chead_hint functionality in a way that doesn't require these bits */
...
#define VM_OBJECT_PURGEABLE_FAULT_ERROR(object)                                \
  ((object)->volatile_fault && ((object)->purgable == VM_PURGABLE_VOLATILE ||  \
                                (object)->purgable == VM_PURGABLE_EMPTY))

extern const vm_object_t kernel_object_default; /* the default kernel object */

extern const vm_object_t
    compressor_object; /* the single compressor object, allocates pages for
                        * compressed buffers (not the segments) */

extern const vm_object_t
    retired_pages_object; /* pages retired due to ECC, should never be used */
...
#define VM_OBJECT_WIRED_ENQUEUE(object)                                        \
  ...
  queue_enter(&vm_objects_wired, (object), vm_object_t, wired_objq);           \
...
#define VM_OBJECT_WIRED_DEQUEUE(object)                                        \
  ...
  if ((object)->wired_objq.next) {                                             \
    lck_spin_lock_grp(&vm_objects_wired_lock, &vm_page_lck_grp_bucket);        \
    queue_remove(&vm_objects_wired, (object), vm_object_t, wired_objq);        \
    lck_spin_unlock(&vm_objects_wired_lock);                                   \
  }                                                                            \
  MACRO_END
#endif /* VM_TAG_ACTIVE_UPDATE */
...
#define VM_OBJECT_UNWIRED(object)                                              \
  ...
  if (VM_KERN_MEMORY_NONE != (object)->wire_tag) {                             \
    vm_tag_update_size((object)->wire_tag,                                     \
                       -ptoa_64((object)->wired_page_count), (object));        \
...
#define VM_OBJECT_WIRED_PAGE_UPDATE_START(object)                              \
  MACRO_BEGIN {                                                                \
    int64_t __wireddelta = 0;                                                  \
    vm_tag_t __waswired = (object)->wire_tag;
...
#define VM_OBJECT_WIRED_PAGE_UPDATE_END(object, tag)                           \
  if (__wireddelta) {                                                          \
    ...
    assert(!__overflow);                                                       \
    if (!(object)->internal && (object)->vo_ledger_tag &&                      \
...
#define VM_OBJECT_WIRED_PAGE_COUNT(object, delta) __wireddelta += delta;
...
extern void kdp_vm_object_sleep_find_owner(event64_t wait_event,
...
__enum_closed_decl(vm_chead_select_t, uint32_t,
                   {CSEL_MIN = 1, CSEL_BY_PID = 1, CSEL_BY_COALITION = 2,
                    CSEL_MAX = 2});
