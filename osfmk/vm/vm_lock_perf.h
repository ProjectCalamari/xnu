...
#define VMLP_CODE(type, subcode)                                               \
  ((((type) & VMLP_CODE_TYPE_MASK) << VMLP_CODE_TYPE_OFFSET) |                 \
   (((subcode) & VMLP_CODE_SUBCODE_MASK) << VMLP_CODE_SUBCODE_OFFSET))
#define VMLP_EVENTID(type, subcode, function)                                  \
  (MACHDBG_CODE(DBG_MACH_VM_LOCK_PERF, VMLP_CODE((type), (subcode))) |         \
   (function))
...
typedef enum __enum_closed {
  ...
  VMLPAN(VM_MAP_MSYNC),
  VMLPAN(VM_MAP_NON_ALIGNED_TEST), /* now unused; can be removed on next
                                      breaking change */
...
static inline void __vmlp_debug_counter_check(int *__vmlp_debug_counter) {
  if (1 != *__vmlp_debug_counter) {
    panic("vmlp_api_end was run %d times in this function (expected 1).",
          *__vmlp_debug_counter);
  }
}
#define VMLP_DEBUG_COUNTER_DECLARE                                             \
  int __vmlp_debug_counter                                                     \
      __attribute__((cleanup(__vmlp_debug_counter_check))) = 0
...
static inline void __vmlp_api_start(vmlp_api_event_t api) {
  (void)api;
  KDBG(VMLP_EVENTID(VM_LOCK_PERF_API_EVENT, api, DBG_FUNC_START));
}
#define vmlp_api_start(func)                                                   \
  VMLP_DEBUG_COUNTER_DECLARE;                                                  \
  __vmlp_api_start(VMLPAN(func));

static inline void __vmlp_api_end(vmlp_api_event_t api, uint64_t kr) {
  (void)api, (void)kr;
...
static inline void __vmlp_lock_event(vmlp_lock_event_t event, vm_map_t map,
                                     unsigned int timestamp) {
  (void)event, (void)map, (void)timestamp;
  KDBG(VMLP_EVENTID(VM_LOCK_PERF_LOCK_EVENT, event, DBG_FUNC_NONE), map,
       timestamp);
}
static inline void vmlp_lock_event_unlocked(vmlp_lock_event_t event,
                                            vm_map_t map) {
  /*
 ...
 */
static inline void vmlp_lock_event_locked(vmlp_lock_event_t event,
                                          vm_map_t map) {
  /*
...
static inline void vmlp_range_event(vm_map_t map, mach_vm_address_t addr,
                                    mach_vm_size_t size) {
  (void)map, (void)addr, (void)size;
  KDBG(VMLP_EVENTID(VM_LOCK_PERF_RANGE_EVENT, VMLP_EVENT_RANGE, DBG_FUNC_NONE),
       map, map->timestamp, addr, size);
}

static inline void vmlp_range_event_entry(vm_map_t map, vm_map_entry_t entry) {
  vmlp_range_event(map, entry->vme_start, entry->vme_end - entry->vme_start);
}

static inline void vmlp_range_event_none(vm_map_t map) {
  vmlp_range_event(map, 0, 0);
}

static inline void vmlp_range_event_all(vm_map_t map) {
