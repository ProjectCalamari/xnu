...
void pe_init_debug(void) {
  ...
  if (!PE_parse_boot_argn("assertions", &boot_arg_value,
                          sizeof(boot_arg_value))) {
#if MACH_ASSERT
    boot_arg_value = TRUE;
#else
  ...
  if (!PE_parse_boot_argn("statistics", &boot_arg_value,
                          sizeof(boot_arg_value))) {
  ...
  if (!PE_i_can_has_debugger(NULL)) {
    boot_arg_value = FALSE;
  } else if (!PE_parse_boot_argn("diagnostic_api", &boot_arg_value,
                                 sizeof(boot_arg_value))) {
    boot_arg_value = TRUE;
  }
#endif
  gPEKernelConfigurationBitmask |=
      (boot_arg_value ? kPEICanHasDiagnosticAPI : 0);

  int factor = 1;
  boolean_t have_bootarg =
      PE_parse_boot_argn("cpu-factor", &factor, sizeof(factor));
  if (have_bootarg) {
    ...
    if (SecureDTLookupEntry(NULL, "/", &root) == kSuccess) {
      ...
      uint32_t size = 0;
      if (SecureDTGetProperty(root, "target-is-fpga", &prop, &size) ==
          kSuccess) {
...
uint32_t PE_i_can_has_kernel_configuration(void) {
...
__mockable uint32_t PE_get_random_seed(unsigned char *dst_random_seed,
...
#pragma GCC diagnostic pop
  ...
  if ((SecureDTLookupEntry(NULL, "/chosen", &entryP) != kSuccess) ||
      (SecureDTGetProperty(entryP, "random-seed",
                           ...
                           (const void **)(uintptr_t)&random_seed,
                           &size) != kSuccess)) {
    random_seed = NULL;
    size = 0;
  }
