...
SECURITY_READ_ONLY_LATE(kernel_mach_header_t *)
collection_mach_headers[KCNumKinds];
...
kc_index_t kc_kind2index(kc_kind_t type) {
  switch (type) {
  case KCKindPrimary:
...
MARK_AS_FIXUP_TEXT void PE_set_kc_header(kc_kind_t type,
  ...
  for (uint32_t j = 0; j < header->ncmds;
       j++, lc = (struct load_command *)((uintptr_t)lc + lc->cmdsize)) {
...
void PE_set_kc_header_and_base(kc_kind_t type, kernel_mach_header_t *header,
                               void *base, uintptr_t slide) {
  kc_index_t i = kc_kind2index(type);
  assert(!collection_base_pointers[i]);
  assert(!collection_mach_headers[i]);
  collection_mach_headers[i] = header;
...
void PE_set_kc_vp(kc_kind_t type, void *vp) {
  kc_index_t i = kc_kind2index(type);
...
const void *const *PE_get_kc_base_pointers(void) {
 ...
 * the kernel slide.
 */
uintptr_t PE_get_kc_slide(kc_kind_t type) {
...
void *PE_get_kc_baseaddress(kc_kind_t type) {
  kc_index_t i = kc_kind2index(type);
  switch (type) {
#if defined(__arm64__)
  case KCKindPrimary: {
...
bool PE_get_kc_format(kc_kind_t type, kc_format_t *format) {
  switch (type) {
  case KCKindPrimary:
