...
static inline void assert_in_dt_region(vm_offset_t const start,
                                       vm_offset_t const end, void const *p) {
  if ((vm_offset_t)p < start || (vm_offset_t)p > end) {
    panic("Device tree pointer outside of device tree region: pointer %p, "
          "DTEnd %lx",
          p, (unsigned long)DTEnd);
  }
}
#define ASSERT_IN_DT(p)                                                        \
  assert_in_dt_region((vm_offset_t)DTRootNode, (vm_offset_t)DTEnd, (p))

static inline void
assert_prop_in_dt_region(vm_offset_t const start, vm_offset_t const end,
                         DeviceTreeNodeProperty const *prop) {
  vm_offset_t prop_end;

  assert_in_dt_region(start, end, prop);
  assert_in_dt_region(start, end,
                      (uint8_t const *)prop + sizeof(DeviceTreeNodeProperty));
  if (os_add3_overflow((vm_offset_t)prop, sizeof(DeviceTreeNodeProperty),
                       prop->length, &prop_end)) {
    panic("Device tree property overflow: prop %p, length 0x%x", prop,
          prop->length);
  }
  assert_in_dt_region(start, end, (void *)prop_end);
}
#define ASSERT_PROP_IN_DT(prop)                                                \
  assert_prop_in_dt_region((vm_offset_t)DTRootNode, (vm_offset_t)DTEnd, (prop))

#define ASSERT_HEADER_IN_DT_REGION(start, end, p, size)                        \
  assert_in_dt_region((start), (end), (uint8_t const *)(p) + (size))
...
next_prop_region(vm_offset_t const start, vm_offset_t end,
                 DeviceTreeNodeProperty const *prop) {
  uintptr_t next_addr;

  ASSERT_HEADER_IN_DT_REGION(start, end, prop, sizeof(DeviceTreeNodeProperty));

  if (os_add3_overflow((uintptr_t)prop, prop->length,
                       sizeof(DeviceTreeNodeProperty) + 3, &next_addr)) {
    panic("Device tree property overflow: prop %p, length 0x%x", prop,
          prop->length);
  }
...
#define next_prop(prop)                                                        \
  next_prop_region((vm_offset_t)DTRootNode, (vm_offset_t)DTEnd, (prop))

static RealDTEntry skipProperties(RealDTEntry entry) {
  DeviceTreeNodeProperty const *prop;
  unsigned int k;
...
static RealDTEntry skipTree(RealDTEntry root) {
  RealDTEntry entry;
  unsigned int k;
...
static RealDTEntry GetFirstChild(RealDTEntry parent) {
  return skipProperties(parent);
}

static RealDTEntry GetNextChild(RealDTEntry sibling) {
  return skipTree(sibling);
}

static const char *GetNextComponent(const char *cp, char *bp) {
  size_t length = 0;
  char *origbp = bp;
...
static RealDTEntry FindChild(RealDTEntry cur, char *buf) {
  RealDTEntry child;
  unsigned long index;
  char const *str;
  unsigned int dummy;
  ...
  while (1) {
    if (SecureDTGetProperty(child, "name", (void const **)&str, &dummy) !=
        kSuccess) {
      break;
    }
    if (strcmp(str, buf) == 0) {
 ...
 * External Routines
 */
void SecureDTInit(void const *base, size_t size) {
...
bool SecureDTIsLockedDown(void) {
#if CONFIG_SPTM
  return true;
...
int SecureDTFindEntry(const char *propName, const char *propValue,
                      DTEntry *entryH) {
  if (!DTInitialized) {
    return kError;
  }
...
int find_entry(const char *propName, const char *propValue, DTEntry *entryH) {
  ...
  for (k = 0; k < nodeP->nProperties; ++k) {
    DeviceTreeNodeProperty const *propP =
        (DeviceTreeNodeProperty const *)(void const *)startingP;
    ...
    if (strcmp(propP->name, propName) == 0) {
      if (propValue == NULL ||
          strcmp((char const *)(propP + 1), propValue) == 0) {
 ...
 */
static int SecureDTFindNodeWithPropertyEqualToValueHelper(
    const char *const propertyName, const void *const propertyValue,
    const size_t propertyValueSize, const DeviceTreeNode **const devicetreeNode,
    const DeviceTreeNode *const currentNode, size_t *const currentNodeSize) {
  ...
  for (int i = 0; i < currentNode->nProperties; i++) {
    const DeviceTreeNodeProperty *const property =
        (const DeviceTreeNodeProperty *const)current_position;
  ...
  next_property:
    ...
    current_position +=
        sizeof(DeviceTreeNodeProperty) + ((property->length + 3) & ~3);
  }
  ...
  for (int i = 0; i < currentNode->nChildren; i++) {
    const DeviceTreeNode *const child =
        (const DeviceTreeNode *const)current_position;
    size_t child_size;
    const int retval = SecureDTFindNodeWithPropertyEqualToValueHelper(
        propertyName, propertyValue, propertyValueSize, devicetreeNode, child,
        &child_size);
    if (retval == kSuccess) {
      return kSuccess;
    }
...
int SecureDTFindNodeWithPropertyEqualToValue(
    const char *const propertyName, const void *const propertyValue,
  ...
  return SecureDTFindNodeWithPropertyEqualToValueHelper(
      propertyName, propertyValue, propertyValueSize, devicetreeNode,
      DTRootNode, &unused);
};

int SecureDTFindNodeWithPhandle(const uint32_t phandle,
                                const DeviceTreeNode **const devicetreeNode) {
  return SecureDTFindNodeWithPropertyEqualToValue(
      "AAPL,phandle", &phandle, sizeof(phandle), devicetreeNode);
}

int SecureDTFindNodeWithStringProperty(
    const char *const propertyName, const char *const propertyValue,
    const DeviceTreeNode **const devicetreeNode) {
  // The property length for strings that gets encoded in the devicetree blob
  // includes the null-terminator.
  return SecureDTFindNodeWithPropertyEqualToValue(
      propertyName, propertyValue, strlen(propertyValue) + 1, devicetreeNode);
}

int SecureDTLookupEntry(const DTEntry searchPoint, const char *pathName,
                        DTEntry *foundEntry) {
  DTEntryNameBuf buf;
  RealDTEntry cur;
  const char *cp;
...
int SecureDTInitEntryIterator(const DTEntry startEntry, DTEntryIterator iter) {
  if (!DTInitialized) {
    return kError;
  }
...
int SecureDTEnterEntry(DTEntryIterator iter, DTEntry childEntry) {
...
int SecureDTExitEntry(DTEntryIterator iter, DTEntry *currentPosition) {
  DTSavedScopePtr newScope;
...
int SecureDTIterateEntries(DTEntryIterator iter, DTEntry *nextEntry) {
...
int SecureDTRestartEntryIteration(DTEntryIterator iter) {
#if 0
...
static int SecureDTGetPropertyInternal(const DTEntry entry,
                                       const char *propertyName,
                                       void const **propertyValue,
                                       unsigned int *propertySize,
                                       vm_offset_t const region_start,
  ...
  ASSERT_HEADER_IN_DT_REGION(region_start, region_start + region_size, entry,
  ...
  if (entry->nProperties == 0) {
    ...
    for (k = 0; k < entry->nProperties; k++) {
      ...
      if (strcmp(prop->name, propertyName) == 0) {
        *propertyValue =
            (void const *)(((uintptr_t)prop) + sizeof(DeviceTreeNodeProperty));
...
int SecureDTGetProperty(const DTEntry entry, const char *propertyName,
                        void const **propertyValue,
                        unsigned int *propertySize) {
  return SecureDTGetPropertyInternal(
      entry, propertyName, propertyValue, propertySize, (vm_offset_t)DTRootNode,
      (vm_size_t)((uintptr_t)DTEnd - (uintptr_t)DTRootNode));
}

int SecureDTGetPropertyRegion(const DTEntry entry, const char *propertyName,
                              void const **propertyValue,
                              unsigned int *propertySize,
                              vm_offset_t const region_start,
                              vm_size_t region_size) {
  return SecureDTGetPropertyInternal(entry, propertyName, propertyValue,
                                     propertySize, region_start, region_size);
}
...
int SecureDTIterateProperties(DTPropertyIterator iter,
...
int SecureDTRestartPropertyIteration(DTPropertyIterator iter) {
  iter->currentProperty = NULL;
  iter->currentIndex = 0;
  return kSuccess;
}
