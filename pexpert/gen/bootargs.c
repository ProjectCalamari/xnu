...
static int getval(char *s, long long *val, argsep_func_t issep,
...
static boolean_t PE_parse_boot_argn_internal(char *args, const char *arg_string,
                                             void *arg_ptr, int max_len,
    ...
    if (arg_boolean) {
      if (max_len > 0) {
        if (force_string) {
          ...
          argnumcpy(1, arg_ptr,
                    max_len); /* max_len of 0 performs no copy at all*/
        }
        arg_found = TRUE;
      } else if (max_len == 0) {
        arg_found = TRUE;
      ...
      if ('_' == *arg_string) { /* Force a string copy if the argument name
                                   begins with an underscore */
        if (max_len > 0) {
          int hacklen = 17 > max_len ? 17 : max_len;
          argstrcpy2(++cp, (char *)arg_ptr,
                     hacklen - 1); /* Hack - terminate after 16 characters */
          arg_found = TRUE;
        } else if (max_len == 0) {
          arg_found = TRUE;
        }
      ...
      case STR:
        if (*cp == '=') {
          if (max_len > 0) {
            argstrcpy2(++cp, (char *)arg_ptr,
                       max_len - 1); /*max_len of 0 performs no copy at all*/
            arg_found = TRUE;
          } else if (max_len == 0) {
            arg_found = TRUE;
          }
...
boolean_t PE_parse_boot_argn(const char *arg_string, void *arg_ptr,
                             int max_len) {
  return PE_parse_boot_argn_internal(PE_boot_args(), arg_string, arg_ptr,
                                     max_len, FALSE);
}

boolean_t PE_boot_arg_uint64_eq(const char *arg_string, uint64_t value) {
  uint64_t tmp;
...
kern_return_t parse_boot_arg_test(void) {
  ...
  for (int i = 0;
       i < (int)(sizeof(string_test_cases) / sizeof(string_test_cases[0]));
       i++) {
    struct string_test_case *test_case = &string_test_cases[i];

    char result[256] = "NOT_FOUND";
    boolean_t found = PE_parse_boot_argn_internal(
        test_case->args, test_case->argname, result, sizeof(result), TRUE);
    ...
    if (test_case->found) {
      ...
      T_LOG("\"%s\": Looking for \"%s\", expecting not found", test_case->args,
            test_case->argname, test_case->argvalue);
  ...
  for (int i = 0;
       i < (int)(sizeof(integer_test_cases) / sizeof(integer_test_cases[0]));
       i++) {
    struct integer_test_case *test_case = &integer_test_cases[i];

    int result = 0xCAFEFACE;
    boolean_t found = PE_parse_boot_argn_internal(
        test_case->args, test_case->argname, &result, sizeof(result), FALSE);

    if (test_case->found) {
      T_LOG("\"%s\": Looking for \"%s\", expecting %d found", test_case->args,
            test_case->argname, test_case->argvalue);
      T_EXPECT(found, "Should find argument");
...
boolean_t PE_imgsrc_mount_supported() { return TRUE; }
...
boolean_t PE_get_default(const char *property_name, void *property_ptr,
  ...
  if (kSuccess == SecureDTLookupEntry(NULL, "/defaults", &dte)) {
     ...
     */
    if (kSuccess != SecureDTGetProperty(dte, property_name, &property_data,
                                        &property_size)) {
      return FALSE;
    }
   ...
   */
  return IODTGetDefault(property_name, property_ptr, max_property) ? FALSE
                                                                   : TRUE;
}
