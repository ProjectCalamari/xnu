...
#define DEBUG_RECORD_ID_LONG(a, b, c, d, e, f, g, h)                           \
  (((uint64_t)((((h) << 24) & 0xFF000000) | (((g) << 16) & 0x00FF0000) |       \
               (((f) << 8) & 0x0000FF00) | ((e) & 0x000000FF))                 \
    << 32) |                                                                   \
   (uint64_t)((((d) << 24) & 0xFF000000) | (((c) << 16) & 0x00FF0000) |        \
              (((b) << 8) & 0x0000FF00) | ((a) & 0x000000FF)))
#define DEBUG_RECORD_ID_SHORT(a, b, c, d)                                      \
  DEBUG_RECORD_ID_LONG(a, b, c, d, 0, 0, 0, 0)
...
#define DbgIdConsoleHeaderForIOP(which_dbg_processor, which_num)               \
  (DEBUG_RECORD_ID_LONG('C', 'O', 'N', 0, 0, 0, which_dbg_processor, which_num))
...
#define kDbgIdConsoleHeaderOscar                                               \
  DbgIdConsoleHeaderForIOP(DBG_COPROCESSOR_OSCAR, 0)

#define kDbgIdAstrisConnection                                                 \
  DEBUG_RECORD_ID_LONG('A', 'S', 'T', 'R', 'C', 'N', 'X', 'N')
#define kDbgIdAstrisConnectionVers                                             \
  DEBUG_RECORD_ID_LONG('A', 'S', 'T', 'R', 'C', 'V', 'E', 'R')

#define kDbgIdMacOSPanicRegion                                                 \
  DEBUG_RECORD_ID_LONG('M', 'A', 'C', 'P', 'A', 'N', 'I', 'C')

#define kDbgIdUnusedEntry 0x0ULL
#define kDbgIdReservedEntry                                                    \
  DEBUG_RECORD_ID_LONG('R', 'E', 'S', 'E', 'R', 'V', 'E', 'D')
#define kDbgIdFreeReqEntry                                                     \
  DEBUG_RECORD_ID_LONG('F', 'R', 'E', 'E', '-', 'R', 'E', 'Q')
#define kDbgIdFreeAckEntry                                                     \
  DEBUG_RECORD_ID_LONG('F', 'R', 'E', 'E', '-', 'A', 'C', 'K')
...
// This second-level struct should be what the Debug Registry record (e.g.
// kDbgIdCPRHeaderANS) points to.
typedef struct {
  uint32_t rdptr;
  uint32_t wrptr;
 ...
 */
int PE_consistent_debug_register(uint64_t record_id, uint64_t physaddr,
                                 uint64_t length);

/*
 * Lookup an exidting entry from the consistent debug structure, populate the
 * attributes if it exists.
 */
boolean_t PE_consistent_debug_lookup_entry(uint64_t record_id,
                                           uint64_t *phys_addr,
                                           uint64_t *length);
