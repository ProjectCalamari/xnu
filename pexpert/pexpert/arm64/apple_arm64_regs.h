...
#else /* defined(APPLECYCLONE) || defined(APPLETYPHOON) ||                     \
         defined(APPLETWISTER) */
...
#endif /* defined(APPLECYCLONE) || defined(APPLETYPHOON) ||                    \
          defined(APPLETWISTER) */
...
#define ARM64_REG_CYC_OVRD_dsblSnoopPTime                                      \
  (1ULL << 31) /// Don't fetch the timebase from the P-block
#endif         /* APPLEMONSOON */
...
#define CORE_THRTL_CFG2_c1pptThrtlRate_mask                                    \
  (0xFFULL << CORE_THRTL_CFG2_c1pptThrtlRate_shift)
 ...
 *             zero if executed on an E-core / non-PE core / non-AMP
 * architectures.
 */
.macro ARM64_IS_PCORE
#if defined(APPLEMONSOON) || HAS_CLUSTER
    mrs $0,
    MPIDR_EL1 ubfx $0, $0, #MPIDR_CORETYPE_SHIFT, #MPIDR_CORETYPE_WIDTH and $0,
    $0,
    #MPIDR_CORETYPE_ACC_P
#else
    mov $0,
    xzr
#endif
         ...
         *
         * @param arg0 result register; will be non-zero if executed on an
         * E-core, else zero if executed on a P-core / non-PE core / non-AMP
         * architectures.
         */
        .macro ARM64_IS_ECORE
            ...
            mrs $0,
    MPIDR_EL1 ands $0, $0,
    #(MPIDR_CORETYPE_MASK << MPIDR_CORETYPE_SHIFT) csinc $0, xzr, xzr,
    ne
#else
            mov $0,
    xzr
        ...
        /*
         * Reads a system register using the appropriate name for E-cores,
         * P-cores and non-PE cores.
         *
         * @note See also: ARM64_IS_ECORE
         *
         * @param arg0 GPR indicating the core type; non-zero if this is an
         * E-core, else zero
    ...
    :.endmacro
         ...
         /*
          * Writes a system register using the appropriate name for E-cores,
          * P-cores and non-PE cores.
          *
          * @note See also: ARM64_IS_ECORE
          *
