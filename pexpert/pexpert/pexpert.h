 ...
 */
void PE_update_panic_crc(unsigned char *, unsigned int *);
 ...
 */
uint32_t PE_get_random_seed(unsigned char *dst_random_seed,
...
/* Zeroes the panic header, sets the panic magic and initializes the header to
 * be used */
...
#define kPEICanHasAssertions                                                   \
  0x00000001 /* Exceptional conditions should panic() instead of printf() */
#define kPEICanHasStatistics                                                   \
  0x00000002 /* Gather expensive statistics (that don't otherwise change       \
                behavior */
#define kPEICanHasDiagnosticAPI                                                \
  0x00000004 /* Vend API to userspace or kexts that introspect kernel state */
...
#ifdef KERNEL
void PE_install_interrupt_handler(void *nub, int source, void *target,
...
enum { kPEReadTOD, kPEWriteTOD };

enum { kPEWaitForInput = 0x00000001, kPERawInput = 0x00000002 };

/* Private Stuff - eventually put in pexpertprivate.h */
enum { kDebugTypeNone = 0, kDebugTypeDisplay = 1, kDebugTypeSerial = 2 };

/*  Scale factor values for PE_Video.v_scale */
enum { kPEScaleFactorUnknown = 0, kPEScaleFactor1x = 1, kPEScaleFactor2x = 2 };
...
struct PE_Video {
  ...
  unsigned long
      v_length; /* length of video memory (0 for v_rowBytes * v_height) */
  unsigned char
      v_rotate; /* Rotation: 0:normal, 1:right 90, 2:left 180, 3:left 90 */
...
extern boolean_t PE_parse_boot_argn(const char *arg_string, void *arg_ptr,
                                    int max_arg);

extern boolean_t PE_boot_arg_uint64_eq(const char *arg_string, uint64_t value);

extern boolean_t PE_parse_boot_arg_str(const char *arg_string, char *arg_ptr,
                                       int size);

extern boolean_t PE_get_default(const char *property_name, void *property_ptr,
...
#if XNU_KERNEL_PRIVATE
extern kern_return_t __abortlike PE_cpu_start_from_kext(cpu_id_t target,
                                                        vm_offset_t start_paddr,
                                                        vm_offset_t arg_paddr);

extern void PE_cpu_start_internal(cpu_id_t target, vm_offset_t start_paddr,
                                  vm_offset_t arg_paddr);
#endif /* XNU_KERNEL_PRIVATE */

#if !XNU_KERNEL_PRIVATE
extern kern_return_t PE_cpu_start(cpu_id_t target, vm_offset_t start_paddr,
                                  vm_offset_t arg_paddr);
#endif /* !XNU_KERNEL_PRIVATE */
...
extern void PE_set_kc_header(kc_kind_t type, kernel_mach_header_t *header,
                             uintptr_t slide);
void PE_reset_kc_header(kc_kind_t type);
/* set both lowest VA (base) and mach-o header for a given KC type */
extern void PE_set_kc_header_and_base(kc_kind_t type,
                                      kernel_mach_header_t *header, void *base,
                                      uintptr_t slide);
...
#endif
/* returns a pointer to the mach-o header for a give KC type, returns NULL if
 * nothing's been set */
...
/* returns an array of length KCNumKinds of the lowest VAs of each KC type -
 ...
 *
 * @brief
 * Initialize the SOCD client mechanism, used for Xnu to contribute data to the
 * SOCD buffers managed by the SMC in it's SRAM.
 */
 ...
 */
extern void PE_read_socd_client_buffer(vm_offset_t offset, void *out_buff,
                                       vm_size_t size);
 ...
 */
extern void PE_write_socd_client_buffer(vm_offset_t offset, const void *in_buff,
                                        vm_size_t size);
