...
#if HAS_GIC_V3
#define GICR_WAKE_TIMEOUT_NS                                                   \
  (1000000000ULL) // timeout for redistributor wakeup (default 1s)
MACHINE_TIMEOUT(gicr_wake_timeout_ns, "gicr-wake-timeout", GICR_WAKE_TIMEOUT_NS,
                MACHINE_TIMEOUT_UNIT_NSEC, NULL);

static vm_offset_t gicd_base;
static vm_offset_t gicr_base;
static vm_offset_t gicr_size;

static uint32_t _gic_read32(vm_offset_t addr) {
  return *((volatile uint32_t *)addr);
}

static uint64_t _gic_read64(vm_offset_t addr) {
  return *((volatile uint64_t *)addr);
}

static void _gic_write32(vm_offset_t addr, uint32_t value) {
...
#define gicr_write32(offset, data)                                             \
  (_gic_write32(gicr_pe_base + (offset), (data)))
#define gicr_read64(offset) (_gic_read64(gicr_pe_base + (offset)))

static vm_offset_t find_gicr_pe_base() {
  // We only care about aff1 and aff0
  uint32_t phys_id =
      __builtin_arm_rsr64("MPIDR_EL1") & (MPIDR_AFF1_MASK | MPIDR_AFF0_MASK);
  ...
  for (vm_offset_t offset = 0; offset < gicr_size; offset += GICR_PE_SIZE) {
    ...
    uint32_t aff_value =
        (uint32_t)(gicr_typer >> GICR_TYPER_AFFINITY_VALUE_SHIFT) &
        (MPIDR_AFF1_MASK | MPIDR_AFF0_MASK);
  ...
  panic("%s: cannot find GICR base for core %u", __func__,
...
void pe_init_fiq() {
  ...
  if (!gicd_base) {
    ...
    if (prop_size < 4 * sizeof(uint64_t)) {
      panic("%s: incorrect reg property size in GIC DT node; expecting 32 "
            "bytes but got %u bytes",
            __func__, prop_size);
    }
  ...
  if (gicr_waker & GICR_WAKER_CHILDRENASLEEP) {
    ...
    nanoseconds_to_deadline(os_atomic_load(&gicr_wake_timeout_ns, relaxed),
                            &gicr_wake_deadline);
    while (gicr_read32(GICR_WAKER) & GICR_WAKER_CHILDRENASLEEP) {
      // Spin
      if (gicr_wake_timeout_ns > 0 &&
          mach_absolute_time() > gicr_wake_deadline) {
