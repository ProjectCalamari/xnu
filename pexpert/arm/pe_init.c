...
TUNABLE_DT(uint32_t, PE_esdm_fuses, "/chosen", "esdm-fuses", "", 0,
           TUNABLE_DT_NONE);
TUNABLE_DT(uint32_t, PE_vmm_present, "/defaults", "vmm-present", "", 0,
           TUNABLE_DT_NONE);
...
/*
 * This variable is only modified once, when the BSP starts executing. We put it
 * in __DATA_CONST as page protections on kernel text early in startup are
 * read-write. The kernel is locked down later in start-up, said mappings become
 * RO and thus this variable becomes immutable.
 *
...
/*
 * This variable indicates the page protection security policy used by the
 * system. It is intended mostly for debugging purposes.
 */
...
static void check_for_panic_log(void) {
#ifdef PLATFORM_PANIC_LOG_PADDR
  gPanicBase =
      ml_io_map_wcomb(PLATFORM_PANIC_LOG_PADDR, PLATFORM_PANIC_LOG_SIZE);
  panic_text_len =
      PLATFORM_PANIC_LOG_SIZE - sizeof(struct embedded_panic_header);
  ...
  /*
   * DT properties for the panic region are populated by UpdateDeviceTree() in
   * iBoot:
   *
   * chosen {
   ...
   * embedded-panic-log-size is the maximum amount of data to store in the
   * buffer
   */
  ...
  if (kSuccess !=
      SecureDTGetProperty(entry, "reg", (void const **)&reg_prop, &size)) {
    return;
  }
  ...
  if (kSuccess != SecureDTGetProperty(chosen, "embedded-panic-log-size",
                                      (void const **)&panic_region_length,
                                      &size)) {
    return;
  }
  ...
  panic_text_len =
      panic_region_length[0] - sizeof(struct embedded_panic_header);
  ...
  if (PE_consistent_debug_enabled()) {
     ...
     */
    if (PE_consistent_debug_lookup_entry(kDbgIdMacOSPanicRegion,
                                         &macos_panic_physbase,
                                         &macos_panic_physlen)) {
      macos_panic_base = ml_io_map_with_prot(macos_panic_physbase,
                                             macos_panic_physlen, VM_PROT_READ);
      mac_panic_header =
          (struct macos_panic_header *)((void *)macos_panic_base);
  ...
  if (panic_info->eph_magic == EMBEDDED_PANIC_MAGIC) {
    printf("iBoot didn't extract panic log from previous session crash, this "
           "is bad\n");
  }
...
int PE_initialize_console(PE_Video *info, int op) {
  ...
  case kPEReleaseScreen:
    ...
    if (!vc_progress_initialized) {
      ...
      vc_progress_initialize(&default_progress, default_progress_data1x,
                             default_progress_data2x, default_progress_data3x,
...
void PE_init_iokit(void) {
  ...
  if (kSuccess == SecureDTLookupEntry(0, "/chosen/memory-map", &entry)) {
    ...
    if (kSuccess ==
        SecureDTGetProperty(entry, "BootCLUT", (void const **)&map, &size)) {
      bcopy(map[0], appleClut8, sizeof(appleClut8));
    }

    if (kSuccess == SecureDTGetProperty(entry, "Pict-FailedBoot",
                                        (void const **)&map, &size)) {
  ...
  if (show_progress) {
    ...
    *delta *= flip;
    ...
    PE_get_default("progress-dy", &default_progress.dy,
                   sizeof(default_progress.dy));
  ...
  if (kdebug_enable && kdebug_debugid_enabled(IOKDBG_CODE(DBG_BOOTER, 0))) {
    ...
    if (kSuccess == SecureDTLookupEntry(0, "/chosen/iBoot", &entry)) {
      ...
      if (kSuccess == SecureDTGetProperty(entry, "start-time",
                                          (void const **)&value_ptr, &size)) {
        if (size == sizeof(start_time_value)) {
          start_time_value = *value_ptr;
        }
      }

      if (kSuccess == SecureDTGetProperty(entry, "debug-wait-start",
                                          (void const **)&value_ptr, &size)) {
      ...
      if (kSuccess == SecureDTGetProperty(entry, "populate-registry-time",
                                          (void const **)&value_ptr, &size)) {
    ...
    KDBG_RELEASE(IOKDBG_CODE(DBG_BOOTER, 0), start_time_value,
                 debug_wait_start_value, load_kernel_start_value,
                 populate_registry_time_value);
#if CONFIG_SPTM
    KDBG_RELEASE(IOKDBG_CODE(DBG_BOOTER, 1), SPTMArgs->timestamp_sk_bootstrap,
                 SPTMArgs->timestamp_xnu_bootstrap,
                 SPTMArgs->timestamp_txm_bootstrap);
#endif
  }
...
void PE_lockdown_iokit(void) {
   ...
   * cannot start the general-purpose IOKit matching process until after
   * lockdown, as it may involve execution of untrusted/non-entitled kext code.
...
void PE_slide_devicetree(vm_offset_t slide) {
  assert(PE_state.initialized);
  PE_state.deviceTreeHead =
      (void *)((uintptr_t)PE_state.deviceTreeHead + slide);
...
void PE_init_platform(boolean_t vm_initialized, void *args) {
  ...
  if (PE_state.initialized == FALSE) {
    ...
    PE_state.video.v_depth =
        (boot_args_ptr->Video.v_depth >> kBootVideoDepthDepthShift) &
        kBootVideoDepthMask;
    PE_state.video.v_rotate =
        (((boot_args_ptr->Video.v_depth >> kBootVideoDepthRotateShift) &
          kBootVideoDepthMask) + // rotation
         ((boot_args_ptr->Video.v_depth >> kBootVideoDepthBootRotateShift) &
          kBootVideoDepthMask) // add extra boot rotation
         ) %
        4;
    PE_state.video.v_scale =
        ((boot_args_ptr->Video.v_depth >> kBootVideoDepthScaleShift) &
         kBootVideoDepthMask) +
        1;
    PE_state.video.v_display = boot_args_ptr->Video.v_display;
    strlcpy(PE_state.video.v_pixelFormat, "BBBBBBBBGGGGGGGGRRRRRRRR",
  ...
  if (!vm_initialized) {
    ...
    if (kSuccess == (SecureDTFindEntry("name", "device-tree", &entry))) {
      if (kSuccess ==
          SecureDTGetProperty(entry, "model", (void const **)&prop, &size)) {
...
#if defined(TARGET_OS_OSX) && defined(__arm64__)
      if (kSuccess == SecureDTGetProperty(entry, "system-firmware-version",
                                          (void const **)&prop, &size)) {
...
extern boolean_t PE_get_hotkey(__unused unsigned char key) { return FALSE; }
 ...
 */
int PE_stub_poll_input(__unused unsigned int options, char *c) {
  *c = (char)uart_getc();
 ...
 */
boolean_t PE_panic_debugging_enabled() { return panicDebugging; }

void PE_update_panic_crc(unsigned char *buf, unsigned int *size) {
  if (!panic_info || !size) {
    return;
  }
  ...
  *size = *size > panic_text_len ? panic_text_len : *size;
  if (panic_info->eph_magic != EMBEDDED_PANIC_MAGIC) {
    // rdar://88696402 (PanicTest: test case for MAGIC check in
    // PE_update_panic_crc)
    printf("Error!! Current Magic 0x%X, expected value 0x%x\n",
           panic_info->eph_magic, EMBEDDED_PANIC_MAGIC);
  }

  /* CRC everything after the CRC itself - starting with the panic header
   * version */
  panic_info->eph_crc =
      crc32(0L, &panic_info->eph_version,
            (panic_text_len + sizeof(struct embedded_panic_header) -
             offsetof(struct embedded_panic_header, eph_version)));
}
...
void PE_init_panicheader() {
  ...
  /*
   * The panic log begins immediately after the panic header -- debugger
   * synchronization and other functions may log into this region before we've
   * become the exclusive panicking CPU and initialize the header here.
   */
  panic_info->eph_panic_log_offset =
      debug_buf_base ? PE_get_offset_into_panic_region(debug_buf_base) : 0;
 ...
 *
 * NOTE: The purpose of this function is NOT to detect/correct corruption in the
 * panic region, it is to update the panic header to make it consistent when we
 * nest panics.
 */
void PE_update_panicheader_nestedpanic() {
  /*
   * if the panic header pointer is bogus (e.g. someone stomped on it) then
   * bail.
   */
  if (!panic_info) {
   ...
   *
   * note that this should not be possible unless someone stomped on the panic
   * header to zero it out, since by the time we reach this location *someone*
   * should have appended something to the log..
   */
  ...
  if (((panic_info->eph_stackshot_offset == 0) &&
       (panic_info->eph_stackshot_len == 0)) ||
      ((panic_info->eph_stackshot_offset != 0) &&
       (panic_info->eph_stackshot_len != 0))) {
    panic_info->eph_panic_flags |=
        EMBEDDED_PANIC_HEADER_FLAG_INCOHERENT_PANICLOG;
  }
...
boolean_t PE_reboot_on_panic(void) {
  ...
  if (PE_i_can_has_debugger(&debug_flags) && (debug_flags & DB_NMI)) {
...
void PE_sync_panic_buffers(void) {
   ...
   * The iBoot panic region is write-combined on arm64.  We must flush dirty
   * lines from L1/L2 as late as possible before reset, with no further reads of
   * the panic region between the flush and the reset.  Some targets have an
   * additional memcache (L3), and a read may bring dirty lines out of L3 and
   * back into L1/L2, causing the lines to be discarded on reset.  If we can
   * make sure the lines are flushed to L3/DRAM, the platform reset handler will
   * flush any L3.
   */
  if (gPanicBase) {
...
__startup_func vm_size_t PE_init_socd_client(void) {
  DTEntry entry;
  uintptr_t const *reg_prop;
  unsigned int size;
 ...
 */
void PE_read_socd_client_buffer(vm_offset_t offset, void *out_buff,
                                vm_size_t size) {
  volatile uint32_t *client_buff =
      (volatile uint32_t *)(socd_trace_ram_base + offset);
  ...
  if ((offset % 4 != 0) || (size % 4 != 0)) {
    panic("unaligned read of 0x%lu bytes from socd trace ram address 0x%lu",
          size, offset);
  }
 ...
 * 1. Prevents accidentally trusting a value read from socd client buffer. socd
 * client buffer is considered untrusted.
 * 2. Ensures only 4 byte store instructions are used. On some platforms, socd
 * client buffer is backed up by a SRAM that must be written to only 4 bytes at
 * a time.
 */
void PE_write_socd_client_buffer(vm_offset_t offset, const void *in_buff,
                                 vm_size_t size) {
  volatile uint32_t *client_buff =
      (volatile uint32_t *)(socd_trace_ram_base + offset);
  vm_size_t len = size / sizeof(client_buff[0]);
  ...
  /* Perform 4 byte aligned accesses */
  if ((offset % 4 != 0) || (size % 4 != 0)) {
    panic("unaligned write of 0x%lu bytes to socd trace ram address 0x%lu",
          size, offset);
  }
