...
static dbg_record_header_t *consistent_debug_allocate_entry(void) {
  ...
  for (i = 0; i < consistent_debug_registry->top_level_header.num_records;
       i++) {
    dbg_record_header_t *record = &consistent_debug_registry->records[i];
    if (OSCompareAndSwap64(kDbgIdUnusedEntry, kDbgIdReservedEntry,
                           &record->record_id)) {
...
boolean_t PE_consistent_debug_lookup_entry(uint64_t record_id,
  ...
  for (unsigned int i = 0;
       i < consistent_debug_registry->top_level_header.num_records; i++) {
...
int PE_consistent_debug_inherit(void) {
  ...
  if ((SecureDTLookupEntry(NULL, "/chosen", &entryP) == kSuccess)) {
    if (SecureDTGetProperty(entryP, "consistent-debug-root",
                            (void const **)&prop_data, &size) == kSuccess) {
  ...
  consistent_debug_registry = (dbg_registry_t *)ml_map_high_window(
      root_pointer, sizeof(dbg_registry_t));
  return 0;
}
