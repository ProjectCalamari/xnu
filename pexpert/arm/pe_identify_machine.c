...
void pe_identify_machine(boot_args *bootArgs) {
  ...
  while (kSuccess == SecureDTIterateEntries(&iter, &cpu)) {
    ...
    if (SecureDTGetProperty(cpu, "timebase-frequency", &value, &size) ==
        kSuccess) {
      /*
    ...
    if (SecureDTGetProperty(cpu, "memory-frequency", &value, &size) ==
        kSuccess) {
      if (size == 8) {
    ...
    if (SecureDTGetProperty(cpu, "peripheral-frequency", &value, &size) ==
        kSuccess) {
      if (size == 8) {
    ...
    if (SecureDTGetProperty(cpu, "fixed-frequency", &value, &size) ==
        kSuccess) {
      if (size == 8) {
    ...
    if (SecureDTGetProperty(cpu, "clock-frequency", &value, &size) ==
        kSuccess) {
      if (size == 8) {
...
vm_offset_t pe_arm_get_soc_base_phys(void) {
  DTEntry entryP;
  ...
  if (SecureDTFindEntry("name", "arm-io", &entryP) == kSuccess) {
    if (gPESoCDeviceType == 0) {
      SecureDTGetProperty(entryP, "device_type", (void const **)&tmpStr,
                          &prop_size);
      strlcpy(gPESoCDeviceTypeBuffer, tmpStr, SOC_DEVICE_TYPE_BUFFER_SIZE);
      gPESoCDeviceType = gPESoCDeviceTypeBuffer;

      SecureDTGetProperty(entryP, "ranges", (void const **)&ranges_prop,
                          &prop_size);
...
static uint32_t pe_arm_map_interrupt_controller(void) {
  DTEntry entryP;
  ...
  if (SecureDTFindEntry("interrupt-controller", "master", &entryP) ==
      kSuccess) {
  ...
  if (gPicBase == 0) {
    kprintf("pe_arm_map_interrupt_controller: failed to find the "
...
uint32_t pe_arm_init_interrupts(void *args) {
  kprintf("pe_arm_init_interrupts: args: %p\n", args);

  /* Set up mappings for interrupt controller and possibly timers (if they
