 ...
 *
 * Please view doc/arm/arm_serial.md for an in-depth description of these
 * drivers.
 */
...
struct pe_serial_functions {
  ...
  /* Clears this device's IRQs targeting this agent, returning true if at least
   * one IRQ was cleared. */
...
MARK_AS_HIBERNATE_DATA_CONST_LATE static struct pe_serial_functions *gPESF =
    NULL;
 ...
 */
static bool irq_initialized(struct pe_serial_functions *device_fns) {
 ...
 */
static bool irq_available_and_ready(struct pe_serial_functions *device_fns) {
  return device_fns->has_irq && irq_initialized(device_fns);
}

/**
 * Searches through the global serial functions list and returns the serial
 * function for a particular device
 *
 ...
 */
static inline void serial_poll(void) {
#if __arm64__
  if (!uart_hibernation) {
    __builtin_arm_wfe();
  }
...
MARK_AS_HIBERNATE_DATA static volatile apple_uart_registers_t
    *apple_uart_registers = 0;
 ...
 */
static void apple_uart_init(void) {
  ucon_t ucon = {.raw = 0};
...
static void apple_uart_enable_irq(void) {
  ...
  ufcon_t ufcon = {.raw =
                       ml_io_read32((uintptr_t)&apple_uart_registers->ufcon)};
  ufcon.tx_fifo_interrupt_trigger_level_dma_watermark =
      UFCON_TX_FIFO_ITL_0_BYTES;
  ml_io_write32((uintptr_t)&apple_uart_registers->ufcon, ufcon.raw);
...
static bool apple_uart_disable_irq(void) {
...
static bool apple_uart_ack_irq(void) {
...
static inline bool apple_uart_fifo_is_empty(void) {
  const ufstat_t ufstat = {
      .raw = ml_io_read32((uintptr_t)&apple_uart_registers->ufstat)};
  return !(ufstat.tx_fifo_full || ufstat.tx_fifo_count);
}

static void apple_uart_drain_fifo(void) {
...
static void apple_uart_set_baud_rate(uint32_t baud_rate) {
  ...
  for (int _sample_rate = kMaxSampleRate; _sample_rate >= kMinSampleRate;
       _sample_rate--) {
    ...
    if ((ideal_ubr_div - (int)ideal_ubr_div) < 0.00001f) {
      ...
      const float higher_baud_rate =
          kSourceClock / ((ubr_div_rounded_down + 1) * _sample_rate);
      const float lower_baud_rate =
          kSourceClock / ((ubr_div_rounded_up + 1) * _sample_rate);
      if ((((higher_baud_rate - baud_rate) / baud_rate) * 100) <
          kMaxErrorPercentage) {
      ...
      if ((((baud_rate - lower_baud_rate) / baud_rate) * 100) <
          kMaxErrorPercentage) {
  ...
  if (!found_configuration) {
    panic("Unable to find a configuration for the UART that would result in a "
          "nominal baud rate close enough to %u",
          baud_rate);
  }
...
MARK_AS_HIBERNATE_TEXT static unsigned int apple_uart_transmit_ready(void) {
  ufstat_t ufstat = {
      .raw = ml_io_read32((uintptr_t)&apple_uart_registers->ufstat)};
  return !ufstat.tx_fifo_full;
}

MARK_AS_HIBERNATE_TEXT static void apple_uart_transmit_data(uint8_t c) {
  utxh_t utxh = {.txdata = c};
  ml_io_write32((uintptr_t)&apple_uart_registers->utxh, utxh.raw);
}

static unsigned int apple_uart_receive_ready(void) {
  ufstat_t ufstat = {
      .raw = ml_io_read32((uintptr_t)&apple_uart_registers->ufstat)};
  return ufstat.rx_fifo_full || ufstat.rx_fifo_count;
}

static uint8_t apple_uart_receive_data(void) {
...
static void apple_uart_setup(const DeviceTreeNode *const devicetree_node) {
  ...
  if (SecureDTGetProperty(devicetree_node, "reg", (const void **)&reg,
                          &reg_size) != kSuccess) {
    panic(
        "Unable to find the 'reg' property on the Apple UART devicetree node");
  }
  ...
  apple_uart_registers = (apple_uart_registers_t *)ml_io_map(
      soc_base_phys + reg->block_offset, reg->block_size);

  // Check if interrupts are supported.
  const void *unused;
  unsigned int unused_size;
  if (SecureDTGetProperty(devicetree_node, "interrupts", &unused,
                          &unused_size) == kSuccess) {
...
static uint64_t
    dockchannel_drain_deadline; // Deadline for external agent to drain before a
                                // software drain occurs
static bool use_sw_drain;
static uint32_t dock_wstat_mask;
static uint64_t
    prev_dockchannel_spaces; // Previous w_stat level of the DockChannel.
...
static void dockchannel_setup(const DeviceTreeNode *const devicetree_node) {
  ...
  if (SecureDTGetProperty(devicetree_node, "reg", (const void **)&reg,
                          &reg_size) != kSuccess) {
    panic("Unable to find the 'reg' property on the Dock Channels devicetree "
          "node");
  }
  assert(reg_size == sizeof(*reg));
  ...
  const vm_offset_t soc_base_phys = pe_arm_get_soc_base_phys();
  dockchannel_uart_base = ml_io_map(soc_base_phys + reg->channels_block_offset,
                                    reg->channels_block_size);
  dock_agent_base = ml_io_map(soc_base_phys + reg->agents_block_offset,
                              reg->agents_block_size);

  // Configure various Dock Channels settings.
  const uint32_t *max_aop_clk;
  unsigned int max_aop_clk_size;
  if (SecureDTGetProperty(devicetree_node, "max-aop-clk",
                          (const void **)&max_aop_clk,
                          &max_aop_clk_size) == kSuccess) {
  ...
  if (SecureDTGetProperty(devicetree_node, "enable-sw-drain",
                          (const void **)&enable_sw_drain,
                          &enable_sw_drain_size) == kSuccess) {
  ...
  if (SecureDTGetProperty(devicetree_node, "dock-wstat-mask",
                          (const void **)&_dock_wstat_mask,
                          &dock_wstat_mask_size) == kSuccess) {
  ...
  prev_dockchannel_spaces =
      rDOCKCHANNELS_DEV_WSTAT(dockchannel_uart_base, DOCKCHANNEL_UART_CHANNEL) &
      dock_wstat_mask;
...
static int dockchannel_drain_on_stall() {
  ...
  // Check if the DockChannel reader has stalled. If so, empty the DockChannel
  // ourselves. Return number of bytes drained.

  if (mach_absolute_time() >= dockchannel_drain_deadline) {
    // It's been more than DOCKCHANEL_WR_MAX_STALL_US and nobody read from the
    // FIFO Drop a character.
    (void)ml_io_read32(rDOCKCHANNELS_DOCK_RDATA1(dockchannel_uart_base,
                                                 DOCKCHANNEL_UART_CHANNEL));
...
static void dockchannel_clear_intr(void) {
  ml_io_write32(
      rDOCKCHANNELS_AGENT_AP_INTR_CTRL(dock_agent_base),
      ml_io_read32(rDOCKCHANNELS_AGENT_AP_INTR_CTRL(dock_agent_base)) & ~(0x3));
  ml_io_write32(
      rDOCKCHANNELS_AGENT_AP_INTR_STATUS(dock_agent_base),
      ml_io_read32(rDOCKCHANNELS_AGENT_AP_INTR_STATUS(dock_agent_base)) | 0x3);
  ml_io_write32(
      rDOCKCHANNELS_AGENT_AP_ERR_INTR_CTRL(dock_agent_base),
      ml_io_read32(rDOCKCHANNELS_AGENT_AP_ERR_INTR_CTRL(dock_agent_base)) &
          ~(0x3));
  ml_io_write32(
      rDOCKCHANNELS_AGENT_AP_ERR_INTR_STATUS(dock_agent_base),
      ml_io_read32(rDOCKCHANNELS_AGENT_AP_ERR_INTR_STATUS(dock_agent_base)) |
          0x3);
}

static bool dockchannel_disable_irq(void) {
  const uint32_t ap_intr_ctrl =
      ml_io_read32(rDOCKCHANNELS_AGENT_AP_INTR_CTRL(dock_agent_base));
...
static void dockchannel_enable_irq(void) {
  // set interrupt to be when fifo has 255 empty
  ml_io_write32(rDOCKCHANNELS_DEV_WR_WATERMARK(dockchannel_uart_base,
                                               DOCKCHANNEL_UART_CHANNEL),
                0xFF);
  ml_io_write32(
      rDOCKCHANNELS_AGENT_AP_INTR_CTRL(dock_agent_base),
...
static bool dockchannel_ack_irq(void) {
  /* First check if the IRQ is for the kernel */
  const uint32_t ap_intr_status =
      0x1 & ml_io_read32(rDOCKCHANNELS_AGENT_AP_INTR_STATUS(dock_agent_base));
...
MARK_AS_HIBERNATE_TEXT static void dockchannel_transmit_data(uint8_t c) {
  ml_io_write32(
      rDOCKCHANNELS_DEV_WDATA1(dockchannel_uart_base, DOCKCHANNEL_UART_CHANNEL),
      (unsigned)c);

  if (use_sw_drain && !uart_hibernation) {
    os_atomic_dec(&prev_dockchannel_spaces,
                  relaxed); // After writing a byte we have one fewer space than
                            // previously expected.
  }
}

static unsigned int dockchannel_receive_ready(void) {
  return ml_io_read32(rDOCKCHANNELS_DEV_RDATA0(dockchannel_uart_base,
                                               DOCKCHANNEL_UART_CHANNEL)) &
         0x7f;
}

static uint8_t dockchannel_receive_data(void) {
  return (uint8_t)((ml_io_read32(rDOCKCHANNELS_DEV_RDATA1(
                        dockchannel_uart_base, DOCKCHANNEL_UART_CHANNEL)) >>
                    8) &
                   0xff);
}

MARK_AS_HIBERNATE_TEXT static unsigned int dockchannel_transmit_ready(void) {
  uint32_t spaces = ml_io_read32(rDOCKCHANNELS_DEV_WSTAT(
                        dockchannel_uart_base, DOCKCHANNEL_UART_CHANNEL)) &
                    dock_wstat_mask;
  ...
  if (!uart_hibernation) {
    if (use_sw_drain) {
      if (spaces > prev_dockchannel_spaces) {
        ...
        // Note that if the DockFIFO is empty we cannot tell if someone is
        // listening, we can only give them the benefit of the doubt.
        dockchannel_drain_deadline =
            mach_absolute_time() + dockchannel_stall_grace;
      }
...
static void dockchannel_init(void) {
  if (use_sw_drain) {
    nanoseconds_to_absolutetime(DOCKCHANNEL_WR_MAX_STALL_US * NSEC_PER_USEC,
  ...
  // Setup DRAIN timer
  ml_io_write32(rDOCKCHANNELS_DEV_DRAIN_CFG(dockchannel_uart_base,
                                            DOCKCHANNEL_UART_CHANNEL),
                max_dockchannel_drain_period);

  // Drain timer doesn't get loaded with value from drain period register if
  // fifo is already full. Drop a character from the fifo.
  (void)ml_io_read32(rDOCKCHANNELS_DOCK_RDATA1(dockchannel_uart_base,
                                               DOCKCHANNEL_UART_CHANNEL));
}
...
static unsigned int pl011_uart_transmit_ready(void) {
  const uartfr_t uartfr = {.raw = pl011_registers->uartfr.raw};
  return uartfr.txff != 1;
}

static void pl011_uart_transmit_data(uint8_t c) {
  uartdr_t uartdr = {.data = c};
  pl011_registers->uartdr.raw = uartdr.raw;
}

static unsigned int pl011_uart_receive_ready(void) {
  const uartfr_t uartfr = {.raw = pl011_registers->uartfr.raw};
  return uartfr.rxfe != 1;
}

static uint8_t pl011_uart_receive_data(void) {
  const uartdr_t uartdr = {.raw = pl011_registers->uartdr.raw};
  return uartdr.data;
}

static void pl011_uart_init(void) {
...
SECURITY_READ_ONLY_LATE(static struct pe_serial_functions)
pl011_uart_serial_functions = {.init = pl011_uart_init,
...
static void pl011_uart_setup(const DeviceTreeNode *const devicetree_node) {
  ...
  if (SecureDTGetProperty(devicetree_node, "reg", (const void **)&reg,
                          &reg_size) != kSuccess) {
    panic(
        "Unable to find the 'reg' property on the PL011 UART devicetree node");
  }
  assert(reg_size == sizeof(*reg));

  // Create a virtual mapping to that physical address range.
  const vm_offset_t soc_base_phys = pe_arm_get_soc_base_phys();
  pl011_registers = (pl011_registers_t *)ml_io_map(
      soc_base_phys + reg->block_offset, reg->block_size);
 ...
 */
static void uart_puts_force_poll(const char *str);
 ...
 */
static void uart_puts_force_poll_device(const char *str,
 ...
 * driver.
 */
MARK_AS_HIBERNATE_TEXT void serial_hibernation_init(void) {
  uart_hibernation = true;
#if defined(APPLE_UART)
  apple_uart_registers =
      (apple_uart_registers_t *)gHibernateGlobals.hibUartRegPhysBase;
 ...
 * resume.
 */
MARK_AS_HIBERNATE_TEXT static void serial_hibernation_cleanup(void) {
  uart_hibernation = false;
#if defined(APPLE_UART)
  apple_uart_registers =
      (apple_uart_registers_t *)gHibernateGlobals.hibUartRegVirtBase;
#endif /* defined(APPLE_UART) */
#if defined(DOCKCHANNEL_UART)
 ...
 */
static bool get_serial_device_phandle(uint32_t *const phandle) {
  ...
  if (SecureDTFindNodeWithStringProperty("name", "defaults", &defaults_node) !=
      kSuccess) {
  ...
  if (SecureDTGetProperty(defaults_node, "serial-device",
                          (const void **)&defaults_phandle,
                          &defaults_phandle_size) == kSuccess) {
  ...
  if (PE_parse_boot_argn("serial-device", &phandle_bootarg,
                         sizeof(phandle_bootarg))) {
  ...
  if (PE_parse_boot_arg_str("serial-device-name", serial_device_name_buffer,
                            sizeof(serial_device_name_buffer))) {
    // Find the devicetree node with that name.
    const DeviceTreeNode *serial_device_node;
    if (SecureDTFindNodeWithStringProperty("name", serial_device_name_buffer,
                                           &serial_device_node) != kSuccess) {
      panic("Unable to find a devicetree node with the name '%s'.",
            serial_device_name_buffer);
    }

    // Get the phandle of that node.
    const uint32_t *node_phandle;
    unsigned int node_phandle_size;
    if (SecureDTGetProperty(serial_device_node, "AAPL,phandle",
                            (const void **)&node_phandle,
                            &node_phandle_size) != kSuccess) {
...
int serial_init(void) {
  ...
  PE_parse_boot_argn("disable-uart-irq", &disable_uart_irq,
                     sizeof(disable_uart_irq));
  ...
  if (SecureDTGetProperty(serial_device_node, "compatible",
                          (const void **)&compatible,
                          &compatible_size) != kSuccess) {
    panic(
        "The serial device devicetree node doesn't have a 'compatible' string");
  }

  // Call the setup function for the identified serial device driver.
  bool found_matching_driver = false;
  const int n_drivers =
      sizeof(driver_setup_functions) / sizeof(driver_setup_functions[0]);
...
#if HIBERNATION
  /* hibernation needs to know the UART register addresses since it can't
   * directly use this serial driver */
  if (dockchannel_uart_base) {
    gHibernateGlobals.dockChannelRegPhysBase =
  ...
  if (apple_uart_registers) {
    gHibernateGlobals.hibUartRegPhysBase =
        ml_vtophys((vm_offset_t)apple_uart_registers);
 ...
 */
void serial_set_on_demand(bool on_demand) {
  /* Enable or disable transmission. */
  serial_do_transmit = !on_demand;

  /* If on-demand is enabled, report it. */
  if (on_demand) {
    uart_puts_force_poll("On-demand serial mode selected.\n"
 ...
 */
static void serial_wait_for_interrupt(struct pe_serial_functions *fns) {
  /**
 ...
 * @param force Whether we should force this over the device if output has not
 * been enabled yet.
 * @param fns Functions for the device to output over.
 */
static inline void uart_putc_device(char c, bool poll, bool force,
                                    struct pe_serial_functions *fns) {
 ...
 */
MARK_AS_HIBERNATE_TEXT void uart_putc_options(char c, bool poll) {
  struct pe_serial_functions *fns = gPESF;
 ...
 * @param c The character to output.
 */
void uart_putc(char c) { uart_putc_options(c, true); }
 ...
 * @note Interrupts will have already been cleared and disabled by
 * serial_irq_filter.
 * @note This function should only be called from the AppleSerialShim kext.
 */
...
bool serial_irq_filter(serial_device_t device) {
  ...
  /**
   * Disable IRQs until next time a thread waits for an interrupt to prevent an
   * interrupt storm.
   */
