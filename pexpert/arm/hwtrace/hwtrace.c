...
#define panic_trace_error(msg, args...)                                        \
  {                                                                            \
    if (panic_trace_debug == 1)                                                \
      kprintf("panic_trace: " msg "\n", ##args);                               \
    else if (panic_trace_debug == 2)                                           \
      printf("panic_trace: " msg "\n", ##args);                                \
  }
#if PANIC_TRACE_LOG
#define panic_trace_log(msg, args...)                                          \
  {                                                                            \
    if (panic_trace_debug)                                                     \
      panic_trace_error(msg, ##args);                                          \
  }
#else
#define panic_trace_log(msg, args...)
...
TUNABLE_DT_WRITEABLE(panic_trace_t, panic_trace, "/arm-io/cpu-debug-interface",
                     "panic-trace-mode", "panic_trace",
                     DEFAULT_PANIC_TRACE_MODE, TUNABLE_DT_NONE);

/*
 * Panic trace debug state. See 'Logs' section above.
 */
TUNABLE_WRITEABLE(boolean_t, panic_trace_debug, "panic_trace_debug",
                  CT_DFT_LOGS_ON);
...
// The command buffer contains the converted commands from the device tree for
// commanding cpu_halt, enable_trace, etc.
...
#define CPU_SELECTOR(register_offset)                                          \
  ((register_offset & CPU_SELECTOR_MASK) >>                                    \
   CPU_SELECTOR_SHIFT) // Upper 16bits holds the cpu selector
#define MAX_WINDOW_SIZE 0xFFFF
#define DELAY_SHIFT (32)
#define DELAY_MASK (0xFFFFULL << DELAY_SHIFT)
#define DELAY_US(register_offset)                                              \
  ((register_offset & DELAY_MASK) >> DELAY_SHIFT)
...
#define REGISTER_IS_32BIT(register_offset)                                     \
  ((register_offset & REGISTER_32BIT_MASK) != 0)
#define REGISTER_SIZE(register_offset)                                         \
  (REGISTER_IS_32BIT(register_offset) ? sizeof(uint32_t) : sizeof(uintptr_t))
#define CPU_SELECTOR_IS_RANGE(register_offset)                                 \
  ((register_offset & CPU_SELECTOR_ISRANGE_MASK) != 0)
#define CPU_SELECTOR_MIN_CPU(register_offset)                                  \
  ((CPU_SELECTOR(register_offset) & 0xff00) >> 8)
#define CPU_SELECTOR_MAX_CPU(register_offset)                                  \
  (CPU_SELECTOR(register_offset) & 0x00ff)

// Record which CPU is currently running one of our debug commands, so we can
// trap panic reentrancy to PE_arm_debug_panic_hook.
static int running_debug_command_on_cpu_number = -1;

// Determine whether the current debug command is intended for this CPU.
static inline bool is_running_cpu_selected(command_buffer_element_t *command) {
  assert(running_debug_command_on_cpu_number >= 0);
  if (command->cpu_selector_is_range) {
    return running_debug_command_on_cpu_number >=
               command->destination_cpu_selector.range.min_cpu &&
           running_debug_command_on_cpu_number <=
               command->destination_cpu_selector.range.max_cpu;
  } else if (command->destination_cpu_selector.mask == ALL_CPUS) {
    return true;
  } else {
    return !!(command->destination_cpu_selector.mask &
              (1 << running_debug_command_on_cpu_number));
  }
}

// Pointers into debug_command_buffer for each operation. Assumes runtime will
// init them to zero.
...
static void pe_init_debug_command(DTEntry entryP,
                                  command_buffer_element_t **command_buffer,
  ...
  static command_buffer_element_t *next_command_buffer_entry =
      debug_command_buffer;

  // record this pointer but don't assign it to *command_buffer yet, in case we
  // panic while half-initialized
  ...
  if (SecureDTGetProperty(entryP, entry_name, (void const **)&reg_prop,
                          &prop_size) != kSuccess) {
    panic("%s: %s: failed to read property from device tree", __func__,
          entry_name);
  }

  if (prop_size % (2 * sizeof(*reg_prop))) {
    panic("%s: %s: property size %u bytes is not a multiple of %lu", __func__,
          entry_name, prop_size, 2 * sizeof(*reg_prop));
  }
    ...
    if (*reg_prop == RESET_VIRTUAL_ADDRESS_WINDOW) {
      ...
      if (reg_window_size > MAX_WINDOW_SIZE) {
        panic("%s: %s: %#x-byte window at #%lx exceeds maximum size of %#x",
              __func__, entry_name, reg_window_size, base_address_pa,
              MAX_WINDOW_SIZE);
      }
      ...
      panic_trace_log("%s: %s: %#x bytes at %#lx mapped to %#lx\n", __func__,
                      entry_name, reg_window_size, base_address_pa,
                      debug_reg_window);
    } else {
      if ((REGISTER_OFFSET(*reg_prop) + REGISTER_SIZE(*reg_prop)) >
          reg_window_size) {
        panic("%s: %s[%ld]: %#lx(+%lu)-byte offset from %#lx exceeds allocated "
              "size of %#x",
              __func__, entry_name,
              next_command_buffer_entry - command_starting_index,
              REGISTER_OFFSET(*reg_prop), REGISTER_SIZE(*reg_prop),
              base_address_pa, reg_window_size);
      }

      if (next_command_buffer_entry - debug_command_buffer >=
          DEBUG_COMMAND_BUFFER_SIZE - 1) {
        // can't use the very last entry, since we need it to terminate the
        // command
        panic("%s: %s[%ld]: out of space in command buffer", __func__,
              entry_name, next_command_buffer_entry - command_starting_index);
      }

      next_command_buffer_entry->address =
          debug_reg_window + REGISTER_OFFSET(*reg_prop);
      next_command_buffer_entry->address_pa =
          base_address_pa + REGISTER_OFFSET(*reg_prop);
...
#endif
      if ((next_command_buffer_entry->cpu_selector_is_range =
               CPU_SELECTOR_IS_RANGE(*reg_prop))) {
        next_command_buffer_entry->destination_cpu_selector.range.min_cpu =
            (uint8_t)CPU_SELECTOR_MIN_CPU(*reg_prop);
        next_command_buffer_entry->destination_cpu_selector.range.max_cpu =
            (uint8_t)CPU_SELECTOR_MAX_CPU(*reg_prop);
      } else {
        next_command_buffer_entry->destination_cpu_selector.mask =
...
static void pe_run_debug_command(command_buffer_element_t *command_buffer) {
  ...
  // When both the CPUs panic, one will get stuck on the lock and the other CPU
  // will be halted when the first executes the debug command
  ...
  while (command_buffer && command_buffer->address) {
    if (is_running_cpu_selected(command_buffer)) {
      panic_trace_log("%s: cpu %d: reg write 0x%lx (VA 0x%lx):= 0x%lx",
                      __func__, running_debug_command_on_cpu_number,
                      command_buffer->address_pa, command_buffer->address,
                      command_buffer->value);
      if (command_buffer->is_32bit) {
        *((volatile uint32_t *)(command_buffer->address)) =
            (uint32_t)(command_buffer->value);
      } else {
        *((volatile uintptr_t *)(command_buffer->address)) =
            command_buffer->value; // register = value;
      }
      if (command_buffer->delay_us != 0) {
        uint64_t deadline;
        nanoseconds_to_absolutetime(command_buffer->delay_us * NSEC_PER_USEC,
                                    &deadline);
 ...
 */
static boolean_t _is_stress_racks(void) {
  DTEntry ent = NULL;
...
/*
 * Stress racks opt out of panic_trace, unless overridden by the panic_trace
 * boot-arg.
 */
static void panic_trace_apply_stress_rack_policy(void) {
 ...
 */
static void panic_trace_apply_partial_policy(void) {
...
#if DEVELOPMENT || DEBUG
void PE_arm_debug_enable_trace(bool should_log) {
  if (should_log) {
...
#if DEVELOPMENT || DEBUG
static void PE_arm_panic_hook(const char *str __unused) {
  (void)str; // not used
  ...
  if (panic_trace) {
    if (running_debug_command_on_cpu_number == cpu_number()) {
      // This is going to end badly if we don't trap, since we'd be panic-ing
      // during our own code
 ...
 */
void pe_arm_debug_init_early(void *boot_cpu_data) {
  DTEntry entryP;
  uintptr_t const *reg_prop;
  uint32_t prop_size;
   ...
   * report it globally. */
  _panic_trace_always_enabled = (panic_trace & panic_trace_enabled) &&
                                !(panic_trace & panic_trace_partial_policy);
  ...
  if (SecureDTFindEntry("device_type", "cpu-debug-interface", &entryP) ==
      kSuccess) {
    /* Initialize the arm debug interface. */
    if (SecureDTGetProperty(entryP, "reg", (void const **)&reg_prop,
                            &prop_size) == kSuccess) {
      ml_init_arm_debug_interface(
          boot_cpu_data, ml_io_map(gSocPhys + *reg_prop, *(reg_prop + 1)));
    }
...
#if DEVELOPMENT || DEBUG
    const uint32_t dependent_modes =
        (panic_trace_enabled | panic_trace_alt_enabled);
 ...
 */
void pe_arm_debug_init_late(void) {}
