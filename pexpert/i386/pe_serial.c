...
static int legacy_uart_probe(void) {
...
static void legacy_uart_set_baud_rate(__unused int unit, uint32_t baud_rate) {
...
static int legacy_uart_tr0(void) { return IO_READ(LSR) & UART_LSR_THRE; }

static void legacy_uart_td0(int c) { IO_WRITE(THR, c); }

static void legacy_uart_init(void) {
...
static int legacy_uart_rr0(void) {
  unsigned char lsr;
...
static int legacy_uart_rd0(void) { return IO_READ(RBR); }
...
static int mmio_uart_present(void) {
  MMIO_WRITE(SCR, 0x5a);
...
static int mmio_uart_probe(void) {
  unsigned new_mmio_uart_base = 0;

  // if specified, mmio_uart overrides all probing
  if (PE_parse_boot_argn("mmio_uart", &new_mmio_uart_base,
...
static void mmio_uart_set_baud_rate(__unused int unit,
                                    __unused uint32_t baud_rate) {
...
static int mmio_uart_tr0(void) { return MMIO_READ(LSR) & UART_LSR_THRE; }

static void mmio_uart_td0(int c) { MMIO_WRITE(THR, c); }

static void mmio_uart_init(void) {
  /* Disable hardware interrupts */
...
static void lpss_uart_re_init(void) {
  ...
  MMIO_WRITE(
      LCR,
      UART_LCR_DLAB); /* Set DLAB bit to enable reading/writing of DLL, DLH */
...
static int mmio_uart_rd0(void) { return MMIO_READ(RBR); }
...
#define PCIE_MMIO_WRITE(r, v)                                                  \
  ml_phys_write_byte(pcie_mmio_uart_base + PCIE_MMIO_UART_##r, v)
#define PCIE_MMIO_READ(r)                                                      \
  ml_phys_read_byte(pcie_mmio_uart_base + PCIE_MMIO_UART_##r)
...
#define PCIE_UART_ENHANCED_MODE                                                \
  (1U << 4) /* EFR register bit 4 => 1 == enhanced mode (128-bit FIFOs) */
  PCIE_MMIO_UART_SCR = 0x30 /* scratch register (not in datasheet) */
};

static vm_offset_t pcie_mmio_uart_base = 0;

static int pcie_mmio_uart_present(void) {
...
static int pcie_mmio_uart_probe(void) {
  unsigned new_pcie_mmio_uart_base = 0;

  // if specified, pcie_mmio_uart overrides all probing
  if (PE_parse_boot_argn("pcie_mmio_uart", &new_pcie_mmio_uart_base,
...
static void pcie_mmio_uart_set_baud_rate(__unused int unit,
                                         __unused uint32_t baud_rate) {
...
static int pcie_mmio_uart_tr0(void) {
  return PCIE_MMIO_READ(LSR) & UART_LSR_THRE;
}

static void pcie_mmio_uart_td0(int c) { PCIE_MMIO_WRITE(THR, c); }

static void pcie_mmio_uart_init(void) {
...
static int pcie_mmio_uart_rr0(void) {
  unsigned char lsr;
...
static int pcie_mmio_uart_rd0(void) { return PCIE_MMIO_READ(RBR); }
...
int serial_init(void) {
  ...
  if (PE_parse_boot_argn("serialbaud", &new_uart_baud_rate,
...
static void uart_putc(char c) {
  if (uart_initted &&
      (legacy_uart_enabled || lpss_uart_enabled || pcie_uart_enabled)) {
    while (!gPESF->tr0()) {
...
void serial_putc_options(char c, __unused bool poll) { serial_putc(c); }

void serial_putc(char c) { uart_putc(c); }

int serial_getc(void) { return uart_getc(); }
