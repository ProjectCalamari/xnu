...
void PE_init_iokit(void) {
  ...
  if (kSuccess == SecureDTLookupEntry(NULL, "/chosen/memory-map", &entry)) {
    if (kSuccess ==
      ...
      if (sizeof(appleClut8) <= map[1]) {
        bcopy((void *)ml_static_ptovirt(map[0]), appleClut8,
              sizeof(appleClut8));
    ...
    if (kSuccess ==
        SecureDTGetProperty(entry, "FailedCLUT", (void const **)&map, &size)) {
      norootClut_lzss = (uint8_t *)ml_static_ptovirt(map[0]);
    }

    if (kSuccess ==
        SecureDTGetProperty(entry, "FailedImage", (void const **)&map, &size)) {
  ...
  /*
   * x86 only minimally enforces lockdown in hardware.  Additionally, some
   * pre-lockdown functionality such as commpage initialization requires IOKit
   * enumeration of CPUs, which is heavily entangled with the ACPI stack.
   * Therefore, we start the IOKit matching process immediately on x86.
   */
  ...
  if (PE_state.initialized == FALSE) {
    ...
    if (args->Video.v_baseAddr) {
      PE_state.video.v_baseAddr =
          args->Video.v_baseAddr; // remains physical address
      ...
      PE_state.video.v_baseAddr =
          args->VideoV1.v_baseAddr; // remains physical address
...
void PE_display_icon(__unused unsigned int flags, __unused const char *name) {
  if (norootIcon_lzss && norootClut_lzss) {
    ...
    uint32_t y = ((PE_state.video.v_height - height) / 2) +
                 norootIcon_lzss->y_offset_from_center;

    vc_display_lzss_icon(x, y, width, height, &norootIcon_lzss->data[0],
                         norootIcon_lzss->data_size, norootClut_lzss);
...
boolean_t PE_get_hotkey(__unused unsigned char key) { return FALSE; }
...
void PE_call_timebase_callback(void) {
  ...
  num = gPEClockFrequencyInfo.bus_clock_rate_num *
        gPEClockFrequencyInfo.bus_to_dec_rate_num;
  den = gPEClockFrequencyInfo.bus_clock_rate_den *
 ...
 * nest panics.
 *
 * We try to avoid nested panics/asserts on x86 because they are difficult to
 * debug, so log any inconsistencies we find.
 */
void PE_update_panicheader_nestedpanic() {
  ...
  if (!(((panic_info->mph_stackshot_offset == 0) &&
         (panic_info->mph_stackshot_len == 0)) ||
        ((panic_info->mph_stackshot_offset != 0) &&
         (panic_info->mph_stackshot_len != 0)))) {
    kdb_printf("panic_info contains invalid stackshot metadata: "
               "mph_stackshot_offset 0x%x mph_stackshot_len 0x%x\n",
  ...
  /*
   * macOS panic logs contain nested panic data, if we've already closed the
   * panic log, begin the other log.
   */
  if ((panic_info->mph_panic_log_len != 0) &&
      (panic_info->mph_other_log_offset == 0)) {
    panic_info->mph_other_log_offset =
        PE_get_offset_into_panic_region(debug_buf_ptr);

    /* Usually indicative of corruption in the panic region */
    if (panic_info->mph_other_log_len != 0) {
      kdb_printf("panic_info contains invalid other log metadata (zero offset "
                 "but non-zero length), length was 0x%x, zeroing value\n",
...
vm_size_t PE_init_socd_client(void) {
  /* not implemented */
  return 0;
}

void PE_read_socd_client_buffer(__unused vm_offset_t offset,
                                __unused void *out_buff,
                                __unused vm_size_t size) {
  /* not implemented */
}

void PE_write_socd_client_buffer(__unused vm_offset_t offset,
                                 __unused const void *buff,
                                 __unused vm_size_t size) {
  /* not implemented */
}
