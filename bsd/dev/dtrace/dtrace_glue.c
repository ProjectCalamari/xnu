...
void dtrace_sprlock(proc_t *p) { lck_mtx_lock(&p->p_dtrace_sprlock); }

void dtrace_sprunlock(proc_t *p) { lck_mtx_unlock(&p->p_dtrace_sprlock); }
...
int uwrite(proc_t *p, void *buf, user_size_t len, user_addr_t a) {
  ...
  if (map) {
    ...
    ret = mach_vm_region_recurse(map, &address, &sizeOfRegion, &nestingDepth,
    ...
    if (!(info.protection & VM_PROT_WRITE)) {
      ...
      if (info.max_protection & VM_PROT_WRITE) {
        ...
        ret = mach_vm_protect(map, (mach_vm_offset_t)a, (mach_vm_size_t)len, 0,
                              (reprotect & ~VM_PROT_EXECUTE) | VM_PROT_WRITE);
      } else {
        /*
         * The memory is not currently writable, and cannot be made writable. We
         * need to COW this memory.
         *
    ...
    ret = vm_map_write_user(map, buf, (vm_map_address_t)a, (vm_size_t)len);
...
/*
 * dtrace_CRED() can be called from probe context. We cannot simply call
 * kauth_cred_get() since that function may try to resolve a lazy credential
 * binding, which entails taking the proc_lock.
 */
cred_t *dtrace_CRED(void) { return current_thread_ro_unchecked()->tro_cred; }

int PRIV_POLICY_CHOICE(void *cred, int priv, int all) {
#pragma unused(priv, all)
  return kauth_cred_issuser(
      cred); /* XXX TODO: How is this different from PRIV_POLICY_ONLY? */
}

int PRIV_POLICY_ONLY(void *cr, int priv, int boolean) {
...
uid_t crgetuid(const cred_t *cr) {
  cred_t copy_cr = *cr;
  return kauth_cred_getuid(&copy_cr);
}
...
void dtrace_install_cpu_hooks(void) {
  dtrace_cpu_state_changed_hook = dtrace_cpu_state_changed;
}

void dtrace_cpu_state_changed(int cpuid, boolean_t is_running) {
  ...
  LIST_FOREACH(wrapTC, &(cpu_list[cpuid].cpu_cyc_list), entries) {
    ...
    if (suspend) {
      ...
      clock_deadline_for_periodic_event(
          wrapTC->when.cyt_interval, mach_absolute_time(), &wrapTC->deadline);
...
static void _timer_call_apply_cyclic(void *ignore, void *vTChdl) {
#pragma unused(ignore)
  wrap_timer_call_t *wrapTC = (wrap_timer_call_t *)vTChdl;

  (*(wrapTC->hdlr.cyh_func))(wrapTC->hdlr.cyh_arg);

  clock_deadline_for_periodic_event(wrapTC->when.cyt_interval,
                                    mach_absolute_time(), &(wrapTC->deadline));
  timer_call_enter1(&(wrapTC->call), (void *)wrapTC, wrapTC->deadline,
                    TIMER_CALL_SYS_CRITICAL | TIMER_CALL_LOCAL);
}

static cyclic_id_t timer_call_add_cyclic(wrap_timer_call_t *wrapTC,
  ...
  nanoseconds_to_absolutetime(wrapTC->when.cyt_interval,
                              (uint64_t *)&wrapTC->when.cyt_interval);

  now = mach_absolute_time();
  wrapTC->deadline = now;

  clock_deadline_for_periodic_event(wrapTC->when.cyt_interval, now,
                                    &(wrapTC->deadline));

  /* Insert the timer to the list of the running timers on this CPU, and start
   * it. */
  s = dtrace_interrupt_disable();
 ...
 */
static void timer_call_remove_cyclic(wrap_timer_call_t *wrapTC) {
  assert(wrapTC);
...
static void *timer_call_get_cyclic_arg(wrap_timer_call_t *wrapTC) {
  return wrapTC ? wrapTC->hdlr.cyh_arg : NULL;
}

cyclic_id_t cyclic_timer_add(cyc_handler_t *handler, cyc_time_t *when) {
...
void cyclic_timer_remove(cyclic_id_t cyclic) {
  ...
  dtrace_xcall(wrapTC->cpuid, (dtrace_xcall_t)timer_call_remove_cyclic,
               (void *)cyclic);

  kfree_type(wrap_timer_call_t, wrapTC);
}

static void _cyclic_add_omni(cyc_list_t *cyc_list) {
  cyc_time_t cT;
  cyc_handler_t cH;
...
cyclic_id_list_t cyclic_add_omni(cyc_omni_handler_t *omni) {
  cyc_list_t *cyc_list =
      kalloc_type(cyc_list_t, wrap_timer_call_t, NCPU, Z_WAITOK | Z_ZERO);
...
static void _cyclic_remove_omni(cyc_list_t *cyc_list) {
  cyc_omni_handler_t *omni = &cyc_list->cyl_omni;
  void *oarg;
...
void cyclic_remove_omni(cyclic_id_list_t cyc_list) {
  ASSERT(cyc_list != NULL);

  dtrace_xcall(DTRACE_CPUALL, (dtrace_xcall_t)_cyclic_remove_omni,
               (void *)cyc_list);
 ...
 * cleaner and the deadman, but too distant in time and place for the profile
 * provider.
 */
static void _cyclic_apply(void *ignore, void *vTChdl) {
#pragma unused(ignore)
  ...
  clock_deadline_for_periodic_event(wrapTC->when.cyt_interval,
                                    mach_absolute_time(), &(wrapTC->deadline));
  (void)thread_call_enter1_delayed(wrapTC->TChdl, (void *)wrapTC,
                                   wrapTC->deadline);
...
cyclic_id_t cyclic_add(cyc_handler_t *handler, cyc_time_t *when) {
  uint64_t now;

  wrap_thread_call_t *wrapTC =
      kalloc_type(wrap_thread_call_t, Z_ZERO | Z_WAITOK);
  if (NULL == wrapTC) {
    return CYCLIC_NONE;
  }
...
static void noop_cyh_func(void *ignore) {
#pragma unused(ignore)
}

void cyclic_remove(cyclic_id_t cyclic) {
...
int ddi_driver_major(dev_info_t *devi) {
...
#pragma unused(spec_type, node_type, flag)
  ...
  if (NULL ==
      devfs_make_node(dev, DEVFS_CHAR, UID_ROOT, GID_WHEEL, 0666, "%s", name)) {
    return DDI_FAILURE;
  } else {
    return DDI_SUCCESS;
  }
...
major_t getemajor(dev_t d) { return (major_t)major(d); }

minor_t getminor(dev_t d) { return (minor_t)minor(d); }

extern void Debugger(const char *);

void debug_enter(char *c) { Debugger(c); }
...
void *dt_kmem_zalloc_tag(size_t size, int kmflag, vm_tag_t tag) {
#pragma unused(kmflag)
...
void dt_kmem_free(void *buf, size_t size) {
...
__typed_allocators_ignore_pop
    ...
    void *
    dt_kmem_alloc_aligned_tag(size_t size, size_t align, int kmflag,
                              vm_tag_t tag) {
...
void *dt_kmem_zalloc_aligned_tag(size_t size, size_t align, int kmflag,
                                 vm_tag_t tag) {
  void *buf;
...
/*
 * vmem (Solaris "slab" allocator) used by DTrace solely to hand out resource
 * ids
 */
typedef unsigned int u_daddr_t;
#include "blist.h"

/* By passing around blist *handles*, the underlying blist can be resized as
 * needed. */
struct blist_hdl {
  blist_t blist;
};
...
vmem_t *vmem_create(const char *name, void *base, size_t size, size_t quantum,
                    void *ignore5, void *ignore6, vmem_t *source,
...
#pragma unused(name, quantum, ignore5, ignore6, source, qcache_max, vmflag)
  ...
  size = MIN(128, size); /* Clamp to 128 initially, since the underlying data
...
/*
 * dtrace_gethrestime() provides the "walltimestamp", a value that is anchored
 * at January 1, 1970. Because it can be called from probe context, it must take
 * no locks.
 */
...
/*
 * dtrace_gethrtime() provides high-resolution timestamps with machine-dependent
 * origin. Hence its primary use is to specify intervals.
 */

hrtime_t dtrace_abs_to_nano(uint64_t elapsed) {
   ...
   * return (elapsed *
   * (uint64_t)sTimebaseInfo.numer)/(uint64_t)sTimebaseInfo.denom;
   *
   * Provided the final result is representable in 64 bits the following
   * maneuver will deliver that result without intermediate overflow.
   */
...
void *dtrace_casptr(void *target, void *cmp, void *new) {
  if (OSCompareAndSwapPtr(cmp, new, (void **)target)) {
    ...
    return (
        void *)(~(uintptr_t)cmp); /* Must return something *other* than cmp */
  }
}
...
void dtrace_interrupt_enable(dtrace_icookie_t reenable) {
...
/*
 * The dtrace_copyin/out/instr and dtrace_fuword* routines can be called from
 * probe context.
 */
...
static int dtrace_copycheck(user_addr_t uaddr, uintptr_t kaddr, size_t size) {
#pragma unused(kaddr)

  ASSERT(kaddr + size >= kaddr);

  if (uaddr + size < uaddr || /* Avoid address wrap. */
      KERN_FAILURE == dtrace_copyio_preflight(
                          uaddr)) { /* Machine specific setup/constraints. */
...
void dtrace_copyin(user_addr_t src, uintptr_t dst, size_t len,
...
void dtrace_copyinstr(user_addr_t src, uintptr_t dst, size_t len,
                      volatile uint16_t *flags) {
#pragma unused(flags)
  ...
  if (dtrace_copycheck(src, dst, len)) {
    ...
    int error =
        copyinstr((const user_addr_t)src, (char *)dst, (vm_size_t)len, &actual);

    /*
     * ENAMETOOLONG is returned when 'len' bytes have been copied in but the NUL
     * terminator was not encountered. That does not require raising
     * CPU_DTRACE_BADADDR, and we press on. Note that we do *not* stuff a NUL
     * terminator when returning ENAMETOOLONG, that's left to the caller.
     */
...
void dtrace_copyout(uintptr_t src, user_addr_t dst, size_t len,
                    volatile uint16_t *flags) {
#pragma unused(flags)
...
void dtrace_copyoutstr(uintptr_t src, user_addr_t dst, size_t len,
                       volatile uint16_t *flags) {
#pragma unused(flags)
  ...
  if (dtrace_copycheck(dst, src, len)) {
    /*
     * ENAMETOOLONG is returned when 'len' bytes have been copied out but the
     * NUL terminator was not encountered. We raise CPU_DTRACE_BADADDR in that
     * case. Note that we do *not* stuff a NUL terminator when returning
     * ENAMETOOLONG, that's left to the caller.
     */
...
int dtrace_copy_maxsize(void) { return copysize_limit_panic; }

int dtrace_buffer_copyout(const void *kaddr, user_addr_t uaddr,
                          vm_size_t nbytes) {
...
uint8_t dtrace_fuword8(user_addr_t uaddr) {
  uint8_t ret = 0;
...
uint16_t dtrace_fuword16(user_addr_t uaddr) {
  uint16_t ret = 0;
...
uint32_t dtrace_fuword32(user_addr_t uaddr) {
  uint32_t ret = 0;
...
uint64_t dtrace_fuword64(user_addr_t uaddr) {
  uint64_t ret = 0;
 ...
 * Called from the fasttrap provider, so the use of copyin/out requires fewer
 * safegaurds.
 */

int fuword8(user_addr_t uaddr, uint8_t *value) {
...
int fuword16(user_addr_t uaddr, uint16_t *value) {
...
int fuword32(user_addr_t uaddr, uint32_t *value) {
...
int fuword64(user_addr_t uaddr, uint64_t *value) {
...
void fuword32_noerr(user_addr_t uaddr, uint32_t *value) {
  if (copyin((const user_addr_t)uaddr, (char *)value, sizeof(uint32_t))) {
    *value = 0;
  }
}

void fuword64_noerr(user_addr_t uaddr, uint64_t *value) {
  if (copyin((const user_addr_t)uaddr, (char *)value, sizeof(uint64_t))) {
    *value = 0;
  }
}

int suword64(user_addr_t addr, uint64_t value) {
...
int suword32(user_addr_t addr, uint32_t value) {
  if (copyout((const void *)&value, addr, sizeof(value)) != 0) {
    return -1;
  }
...
boolean_t dtrace_tally_fault(user_addr_t uaddr) {
  DTRACE_CPUFLAG_SET(CPU_DTRACE_BADADDR);
  cpu_core[CPU->cpu_id].cpuc_dtrace_illval = uaddr;
...
#define TOTTY 0x02
extern int prf(const char *, va_list, int,
               struct tty *); /* bsd/kern/subr_prf.h */

int vuprintf(const char *format, va_list ap) {
...
const void *bsearch(const void *key, const void *base0, size_t nmemb,
 ...
 * Runtime and ABI
 */
uintptr_t dtrace_caller(int ignore) {
#pragma unused(ignore)
...
int dtrace_getstackdepth(int aframes) {
  ...
  if ((on_intr = CPU_ON_INTR(CPU)) != 0) {
    ...
    stacktop = (struct frame *)(dtrace_get_kernel_stack(current_thread()) +
                                kernel_stack_size);
  }
...
int dtrace_addr_in_module(const void *addr, const struct modctl *ctl) {
