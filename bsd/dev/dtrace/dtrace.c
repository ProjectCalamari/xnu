...
#define t_predcache                                                            \
  t_dtrace_predcache /* Cosmetic. Helps readability of thread.h */
...
static int dtrace_dof_mode; /* See dtrace_impl.h for a description of Darwin's
                               dof modes. */

/*
 * This does't quite fit as an internal variable, as it must be accessed in
 * fbt_provide and sdt_provide. Its clearly not a dtrace tunable variable
 * either...
 */
int dtrace_kernel_symbol_mode; /* See dtrace_impl.h for a description of
                                  Darwin's kernel symbol modes. */
 ...
 *
 */
static LCK_MTX_DECLARE_ATTR(dtrace_lock, &dtrace_lck_grp,
                            &dtrace_lck_attr); /* probe state lock */
static LCK_MTX_DECLARE_ATTR(dtrace_provider_lock, &dtrace_lck_grp,
                            &dtrace_lck_attr); /* provider state lock */
static LCK_MTX_DECLARE_ATTR(dtrace_meta_lock, &dtrace_lck_grp,
                            &dtrace_lck_attr); /* meta-provider state lock */
static LCK_RW_DECLARE_ATTR(dtrace_dof_mode_lock, &dtrace_lck_grp,
                           &dtrace_lck_attr); /* dof mode lock */
...
static void dtrace_provide_nullop(void *arg, const dtrace_probedesc_t *desc) {
#pragma unused(arg, desc)
}

static void dtrace_provide_module_nullop(void *arg, struct modctl *ctl) {
#pragma unused(arg, ctl)
}

static int dtrace_enable_nullop(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id, parg)
  return (0);
}

static void dtrace_disable_nullop(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id, parg)
}

static void dtrace_suspend_nullop(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id, parg)
}

static void dtrace_resume_nullop(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id, parg)
}

static void dtrace_destroy_nullop(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id, parg)
}
...
#define DTRACE_GETSTR(hash, elm) (hash->dth_getstr(elm, hash->dth_stroffs))

#define DTRACE_HASHSTR(hash, elm) dtrace_hash_str(DTRACE_GETSTR(hash, elm))
...
#define DTRACE_HASHEQ(hash, lhs, rhs)                                          \
  (strcmp(DTRACE_GETSTR(hash, lhs), DTRACE_GETSTR(hash, rhs)) == 0)
...
static int dtrace_enabling_match(dtrace_enabling_t *, int *,
                                 dtrace_match_cond_t *cond);
static void dtrace_enabling_matchall_with_cond(dtrace_match_cond_t *cond);
static void dtrace_enabling_matchall(void);
static dtrace_state_t *dtrace_anon_grab(void);
static uint64_t dtrace_helper(int, dtrace_mstate_t *, dtrace_state_t *,
                              uint64_t, uint64_t);
...
static int dtrace_canload_remains(uint64_t, size_t, size_t *, dtrace_mstate_t *,
                                  dtrace_vstate_t *);
...
static int sysctl_dtrace_err_verbose SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern_dtrace, OID_AUTO, err_verbose,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &dtrace_err_verbose, 0,
            sysctl_dtrace_err_verbose, "I", "dtrace error verbose");

static int sysctl_dtrace_buffer_memory_maxsize SYSCTL_HANDLER_ARGS {
 ...
 *
 * Set DTrace maximal size in bytes used by all the consumers' state buffers. By
 * default the limit is PHYS_MEM / 3 for *all* consumers.  Attempting to set a
 * null, a negative value or a value <= to dtrace_buffer_memory_inuse will
 * result in a failure.
 */
SYSCTL_PROC(_kern_dtrace, OID_AUTO, buffer_memory_maxsize,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED,
            &dtrace_buffer_memory_maxsize, 0,
            sysctl_dtrace_buffer_memory_maxsize, "Q",
            "dtrace state buffer memory maxsize");
 ...
 */
SYSCTL_QUAD(_kern_dtrace, OID_AUTO, buffer_memory_inuse,
            CTLFLAG_RD | CTLFLAG_LOCKED, &dtrace_buffer_memory_inuse,
            "dtrace state buffer memory in-use");

static int sysctl_dtrace_difo_maxsize SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg2, req)
  int changed, error;
...
SYSCTL_PROC(_kern_dtrace, OID_AUTO, difo_maxsize,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED, &dtrace_difo_maxsize, 0,
            sysctl_dtrace_difo_maxsize, "Q", "dtrace difo maxsize");

static int sysctl_dtrace_dof_maxsize SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg2, req)
  int changed, error;
...
SYSCTL_PROC(_kern_dtrace, OID_AUTO, dof_maxsize,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED, &dtrace_dof_maxsize, 0,
            sysctl_dtrace_dof_maxsize, "Q", "dtrace dof maxsize");

static int sysctl_dtrace_statvar_maxsize SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg2, req)
  int changed, error;
  dtrace_optval_t value = *(dtrace_optval_t *)arg1;
...
SYSCTL_PROC(_kern_dtrace, OID_AUTO, global_maxsize,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED, &dtrace_statvar_maxsize,
            0, sysctl_dtrace_statvar_maxsize, "Q", "dtrace statvar maxsize");
...
SYSCTL_INT(_kern_dtrace, OID_AUTO, provide_private_probes,
           ...
           "provider must provide the private probes");
...
static int dtrace_canstore_statvar(uint64_t addr, size_t sz, size_t *remain,
  ...
  for (i = 0; i < nsvars; i++) {
    ...
    if (DTRACE_INRANGE(addr, sz, svar->dtsv_data, svar->dtsv_size)) {
      DTRACE_RANGE_REMAIN(remain, addr, svar->dtsv_data, svar->dtsv_size);
      return (1);
    }
  }
 ...
 */
static int dtrace_canstore_remains(uint64_t addr, size_t sz, size_t *remain,
                                   dtrace_mstate_t *mstate,
                                   dtrace_vstate_t *vstate) {
  /*
   ...
   */
  if (dtrace_canstore_statvar(addr, sz, remain, vstate->dtvs_locals,
                              vstate->dtvs_nlocals))
    return (1);

  if (dtrace_canstore_statvar(addr, sz, remain, vstate->dtvs_globals,
 ...
 */
int dtrace_canload(uint64_t addr, size_t sz, dtrace_mstate_t *mstate,
                   dtrace_vstate_t *vstate) {
 ...
 * allowed memory region.
 */
static int dtrace_canload_remains(uint64_t addr, size_t sz, size_t *remain,
                                  dtrace_mstate_t *mstate,
                                  dtrace_vstate_t *vstate) {
  ...
  if (DTRACE_INRANGE(addr, sz, (uintptr_t)mstate->dtms_difo->dtdo_strtab,
    ...
    DTRACE_RANGE_REMAIN(remain, addr, mstate->dtms_difo->dtdo_strtab,
                        mstate->dtms_difo->dtdo_strlen);
    return (1);
  }
 ...
 */
static int dtrace_strcanload(uint64_t addr, size_t sz, size_t *remain,
  ...
  if (dtrace_canload_remains(addr, 0, remain, mstate, vstate)) {
     ...
     */
    strsz = 1 + dtrace_strlen((char *)(uintptr_t)addr, MIN(sz, *remain));
    if (strsz <= *remain) {
      return (1);
    }
  }
 ...
 * region in which a load may be issued given the user's privilege level.
 */
static int dtrace_vcanload(void *src, dtrace_diftype_t *type, size_t *remain,
                           dtrace_mstate_t *mstate, dtrace_vstate_t *vstate) {
  size_t sz;
  ...
  if (type->dtdt_kind == DIF_TYPE_STRING) {
    return (dtrace_strcanload((uintptr_t)src, sz, remain, mstate, vstate));
  }
  return (dtrace_canload_remains((uintptr_t)src, sz, remain, mstate, vstate));
}
 ...
 */
static int64_t dtrace_strtoll(char *input, int base, size_t limit) {
   ...
   */
  if (base == 16 && c == '0' &&
      ((cc = dtrace_load8(pos + 1)) == 'x' || cc == 'X') &&
      isxdigit(ccc = dtrace_load8(pos + 2))) {
    pos += 2;
    c = ccc;
  }
 ...
 */
static int dtrace_strncmp(const char *s1, const char *s2, size_t limit) {
  uint8_t c1, c2;
  volatile uint16_t *flags;
 ...
 */
static size_t dtrace_strlen(const char *s, size_t lim) {
  uint_t len;
 ...
 */
static int dtrace_istoxic(uintptr_t kaddr, size_t size) {
 ...
 */
static void dtrace_bzero(void *dst, size_t len) {
  uchar_t *cp;
...
static void dtrace_add_128(uint64_t *addend1, uint64_t *addend2,
                           uint64_t *sum) {
  uint64_t result[2];
 ...
 */
static int dtrace_priv_proc_common_user(dtrace_state_t *state) {
 ...
 */
static int dtrace_priv_proc_common_zone(dtrace_state_t *state) {
  cred_t *cr, *s_cr = state->dts_cred.dcr_cred;
...
static int dtrace_priv_proc_destructive(dtrace_state_t *state) {
...
static int dtrace_priv_proc_control(dtrace_state_t *state) {
  if (ISSET(current_proc()->p_lflag, P_LNOATTACH))
    goto bad;
  ...
  if (dtrace_priv_proc_common_zone(state) &&
      dtrace_priv_proc_common_user(state) && dtrace_priv_proc_common_nocd())
    return (1);
...
static int dtrace_priv_proc(dtrace_state_t *state) {
  if (ISSET(current_proc()->p_lflag, P_LNOATTACH))
    goto bad;
 ...
 */
static int dtrace_priv_proc_relaxed(dtrace_state_t *state) {
...
static int dtrace_priv_kernel(dtrace_state_t *state) {
  if (dtrace_is_restricted() && !dtrace_are_restrictions_relaxed())
    goto bad;
...
static void dtrace_dynvar_clean(dtrace_dstate_t *dstate) {
  ...
  zpercpu_foreach(dcpu, dstate->dtds_percpu) {
    ...
    } while (dtrace_casptr(&dcpu->dtdsc_dirty, dirty, NULL) != dirty);
  }
...
static dtrace_dynvar_t *dtrace_dynvar(dtrace_dstate_t *dstate, uint_t nkeys,
  ...
  if (op == DTRACE_DYNVAR_DEALLOC) {
    ...
    for (;;) {
      ...
      if (dtrace_casptr((void *)(uintptr_t)lockp, (void *)lock,
                        (void *)(lock + 1)) == (void *)lock)
        break;
    }
...
top:
  ...
  for (dvar = start; dvar != NULL; dvar = dvar->dtdv_next) {
    ...
    for (i = 0; i < nkeys; i++, dkey++) {
      ...
      if (dkey->dttk_size != 0) {
        if (dtrace_bcmp((void *)(uintptr_t)key[i].dttk_value,
                        (void *)(uintptr_t)dkey->dttk_value, dkey->dttk_size))
          goto next;
      } else {
    ...
    if (prev != NULL) {
      ...
      if (dtrace_casptr(&hash[bucket].dtdh_chain, start, dvar->dtdv_next) !=
          start) {
        /*
   ...
   */
  if (sizeof(dtrace_dynvar_t) + sizeof(dtrace_key_t) * (nkeys - 1) + ksize +
          dsize >
      chunksize) {
    dcpu->dtdsc_drops++;
    return (NULL);
    ...
    if (free == NULL) {
      ...
      if (clean == NULL) {
        ...
        case DTRACE_DSTATE_CLEAN: {
          ...
          if (dcpu->dtdsc_dirty != NULL && nstate == DTRACE_DSTATE_EMPTY)
          ...
          (void)dtrace_cas32(sp, DTRACE_DSTATE_CLEAN, nstate);
       ...
       */
      if (dtrace_casptr(&dcpu->dtdsc_clean, clean, NULL) != clean) {
        /*
  ...
  for (i = 0; i < nkeys; i++) {
    ...
    if (kesize != 0) {
      dtrace_bcopy((const void *)(uintptr_t)key[i].dttk_value, (void *)kdata,
                   kesize);
...
/*ARGSUSED*/
static void dtrace_aggregate_min(uint64_t *oval, uint64_t nval, uint64_t arg) {
...
/*ARGSUSED*/
static void dtrace_aggregate_max(uint64_t *oval, uint64_t nval, uint64_t arg) {
#pragma unused(arg) /* __APPLE__ */
  if ((int64_t)nval > (int64_t)*oval)
    *oval = nval;
}

static void dtrace_aggregate_quantize(uint64_t *quanta, uint64_t nval,
                                      uint64_t incr) {
...
static void dtrace_aggregate_lquantize(uint64_t *lquanta, uint64_t nval,
                                       uint64_t incr) {
...
static int dtrace_aggregate_llquantize_bucket(int16_t factor, int16_t low,
                                              int16_t high, int16_t nsteps,
                                              int64_t value) {
...
static void dtrace_aggregate_llquantize(uint64_t *llquanta, uint64_t nval,
                                        uint64_t incr) {
  ...
  llquanta[dtrace_aggregate_llquantize_bucket(factor, low, high, nsteps,
                                              nval)] += incr;
}

/*ARGSUSED*/
static void dtrace_aggregate_avg(uint64_t *data, uint64_t nval, uint64_t arg) {
#pragma unused(arg) /* __APPLE__ */
  data[0]++;
  data[1] += nval;
}
...
/*ARGSUSED*/
static void dtrace_aggregate_stddev(uint64_t *data, uint64_t nval,
                                    uint64_t arg) {
#pragma unused(arg) /* __APPLE__ */
...
/*ARGSUSED*/
static void dtrace_aggregate_count(uint64_t *oval, uint64_t nval,
                                   uint64_t arg) {
...
/*ARGSUSED*/
static void dtrace_aggregate_sum(uint64_t *oval, uint64_t nval, uint64_t arg) {
#pragma unused(arg) /* __APPLE__ */
  *oval += nval;
}
  ...
  while (i < state->dts_nspeculations) {
    ...
    if (current != DTRACESPEC_INACTIVE) {
      if (current == DTRACESPEC_COMMITTINGMANY ||
          current == DTRACESPEC_COMMITTING || current == DTRACESPEC_DISCARDING)
        stat = &state->dts_speculations_busy;
      i++;
      continue;
    }

    if (dtrace_cas32((uint32_t *)&spec->dtsp_state, current,
                     DTRACESPEC_ACTIVE) == current)
      return (i + 1);
  }
...
static void dtrace_speculation_commit(dtrace_state_t *state, processorid_t cpu,
  ...
  } while (dtrace_cas32((uint32_t *)&spec->dtsp_state, current, new) !=
           current);
   ...
   * buffer.
   */
  if ((offs = dtrace_buffer_reserve(dest, src->dtb_offset, sizeof(uint64_t),
                                    state, NULL)) < 0) {
 ...
 * speculation.
 */
static void dtrace_speculation_clean_here(dtrace_state_t *state) {
 ...
 */
static void dtrace_speculation_clean(dtrace_state_t *state) {
  int work = 0;
  uint32_t rv;
  dtrace_specid_t i;
  ...
  dtrace_xcall(DTRACE_CPUALL, (dtrace_xcall_t)dtrace_speculation_clean_here,
               state);
 ...
 */
__attribute__((noinline)) static dtrace_buffer_t *
...
static uintptr_t dtrace_dif_varstr(uintptr_t addr, dtrace_state_t *state,
  ...
  dtrace_strcpy((const void *)addr, (void *)mstate->dtms_scratch_ptr, strsz);
...
static uint64_t dtrace_dif_variable(dtrace_mstate_t *mstate,
    ...
    if (ndx >= sizeof(mstate->dtms_arg) / sizeof(mstate->dtms_arg[0])) {
      ...
      if (pv->dtpv_pops.dtps_getargval != NULL)
        val = pv->dtpv_pops.dtps_getargval(
            pv->dtpv_arg, mstate->dtms_probe->dtpr_id,
            mstate->dtms_probe->dtpr_arg, argndx, aframes);
      /* Special case access of arg5 as passed to dtrace_probe_error() (which
       * see.) */
      else if (mstate->dtms_probe->dtpr_id == dtrace_probeid_error &&
               argndx == 5) {
        return ((dtrace_state_t *)(uintptr_t)(mstate->dtms_arg[0]))
  ...
  case DIF_VAR_CALLER:
    ...
    if (!(mstate->dtms_present & DTRACE_MSTATE_CALLER)) {
      ...
      } else if ((mstate->dtms_caller = dtrace_caller(aframes)) ==
                 (uintptr_t)-1) {
        /*
  ...
  case DIF_VAR_PROBEPROV:
    ...
    return (dtrace_dif_varstr(
        (uintptr_t)mstate->dtms_probe->dtpr_provider->dtpv_name, state,
        mstate));

  case DIF_VAR_PROBEMOD:
    ASSERT(mstate->dtms_present & DTRACE_MSTATE_PROBE);
    return (dtrace_dif_varstr((uintptr_t)mstate->dtms_probe->dtpr_mod, state,
                              mstate));

  case DIF_VAR_PROBEFUNC:
    ASSERT(mstate->dtms_present & DTRACE_MSTATE_PROBE);
    return (dtrace_dif_varstr((uintptr_t)mstate->dtms_probe->dtpr_func, state,
                              mstate));

  case DIF_VAR_PROBENAME:
    ASSERT(mstate->dtms_present & DTRACE_MSTATE_PROBE);
    return (dtrace_dif_varstr((uintptr_t)mstate->dtms_probe->dtpr_name, state,
                              mstate));
  ...
  case DIF_VAR_PTHREAD_SELF:
    ...
    /* Not currently supported, but we should be able to delta the dispatchqaddr
     * and dispatchqoffset to get pthread_self */
    return 0;
  ...
  case DIF_VAR_GID:
    ...
    if (dtrace_CRED() != NULL)
      ...
      /* proc_lock would be taken under kauth_cred_proc_ref() in
       * kauth_cred_get(). */
      DTRACE_CPUFLAG_SET(CPU_DTRACE_ILLOP);
      return -1ULL;
    }
 ...
 */
static char *dtrace_json(uint64_t size, uintptr_t json, char *elemlist,
    ...
    case DTRACE_JSON_STRING:
      ...
      if (cc == '"') {
        ...
        *dd = '\0';
        ...
        if (string_is_key) {
          if (dtrace_strncmp(dest, elem, size) == 0)
            found_key = B_TRUE;
        } else if (found_key) {
          if (nelems > 1) {
            /*
        ...
        state = string_is_key ? DTRACE_JSON_COLON : DTRACE_JSON_COMMA;
...
/* FIXME: awaits lock/mutex work */
  ...
  switch (subr) {
  case DIF_SUBR_RAND:
    regs[rd] = dtrace_xoroshiro128_plus_next(state->dts_rstate[CPU->cpu_id]);
    break;

#if !defined(__APPLE__)
  case DIF_SUBR_MUTEX_OWNED:
    if (!dtrace_canload(tupregs[0].dttk_value, sizeof(kmutex_t), mstate,
                        vstate)) {
      regs[rd] = 0;
      break;
    }
  ...
  case DIF_SUBR_MUTEX_OWNER:
    ...
    m.mx = dtrace_load64(tupregs[0].dttk_value);
    if (MUTEX_TYPE_ADAPTIVE(&m.mi) && MUTEX_OWNER(&m.mi) != MUTEX_NO_OWNER)
  ...
  case DIF_SUBR_RW_READ_HELD: {
    ...
    if (!dtrace_canload(tupregs[0].dttk_value, sizeof(uintptr_t), mstate,
                        vstate)) {
      regs[rd] = 0;
      break;
    }
  ...
  case DIF_SUBR_RW_WRITE_HELD:
    if (!dtrace_canload(tupregs[0].dttk_value, sizeof(krwlock_t), mstate,
                        vstate)) {
      regs[rd] = 0;
      break;
    }
...
/* FIXME: awaits lock/mutex work */
  ...
  case DIF_SUBR_COPYIN: {
    ...
    uint64_t size = tupregs[subr == DIF_SUBR_ALLOCA ? 0 : 1].dttk_value;
     ...
     */
    if (scratch_size < size || !DTRACE_INSCRATCH(mstate, scratch_size)) {
      DTRACE_CPUFLAG_SET(CPU_DTRACE_NOSCRATCH);
      regs[rd] = 0;
      break;
    }
  ...
  case DIF_SUBR_COPYOUT: {
    ...
    if (!dtrace_destructive_disallow && dtrace_priv_proc_control(state) &&
  ...
  case DIF_SUBR_RINDEX: {
    ...
    if (!dtrace_canload((uintptr_t)substr, sublen + 1, mstate, vstate)) {
      regs[rd] = 0;
      break;
    }
  ...
  case DIF_SUBR_STRTOK: {
    ...
    if (addr == 0) {
       ...
       */
      if (!dtrace_strcanload(addr, size, &clim, mstate, vstate)) {
        regs[rd] = 0;
        break;
      }
      limit = addr + clim;
...
#else
      if (!dtrace_canload(tupregs[argi].dttk_value, sizeof(ip4), mstate,
                          vstate)) {
        regs[rd] = 0;
        break;
      }

      dtrace_bcopy((void *)(uintptr_t)tupregs[argi].dttk_value,
...
#define _S6_u8 __u6_addr8
      ...
      if (!dtrace_canload(tupregs[argi].dttk_value, sizeof(struct in6_addr),
                          mstate, vstate)) {
        regs[rd] = 0;
        break;
      }
      ...
      *end-- = '\0';
      ...
      for (i = 0; i < (int)sizeof(struct in6_addr); i++) {
        if (ip6._S6_un._S6_u8[i] == 0 && tryzero == -1 && i % 2 == 0) {
          tryzero = i;
          continue;
        }

        if (tryzero != -1 &&
            (ip6._S6_un._S6_u8[i] != 0 || i == sizeof(struct in6_addr) - 1)) {
          ...
          if (ip6._S6_un._S6_u8[i] == 0 && i == sizeof(struct in6_addr) - 1)
            numzero += 2;
        }
      }
      ...
      if (IN6_IS_ADDR_V4MAPPED(&ip6) || IN6_IS_ADDR_V4COMPAT(&ip6)) {
        for (i = sizeof(struct in6_addr) - 1; i >= (int)DTRACE_V4MAPPED_OFFSET;
             i--) {
          ASSERT(end >= base);
      ...
      for (i = v6end; i >= 0; i -= 2) {
        ...
        val = (ip6._S6_un._S6_u8[i] << 8) + ip6._S6_un._S6_u8[i + 1];
...
#undef _S6_u8
  ...
  case DIF_SUBR_JSON: {
    ...
    *ee++ = '\0';
    ...
    if ((regs[rd] =
             (uintptr_t)dtrace_json(size, json, elemlist, nelems, dest)) != 0)
  ...
  case DIF_SUBR_STRIP:
    ...
    regs[rd] = (uint64_t)dtrace_ptrauth_strip((void *)tupregs[0].dttk_value,
  ...
  case DIF_SUBR_KDEBUG_TRACE: {
    ...
    if (dtrace_destructive_disallow || !dtrace_priv_kernel_destructive(state)) {
      return;
    }
  ...
  case DIF_SUBR_PHYSMEM_READ: {
    ...
    regs[rd] =
        dtrace_physmem_read(tupregs[0].dttk_value, tupregs[1].dttk_value);
#else
    DTRACE_CPUFLAG_SET(CPU_DTRACE_ILLOP);
  ...
  case DIF_SUBR_PHYSMEM_WRITE: {
    ...
    dtrace_physmem_write(tupregs[0].dttk_value, tupregs[1].dttk_value,
                         (size_t)tupregs[2].dttk_value);
#else
    DTRACE_CPUFLAG_SET(CPU_DTRACE_ILLOP);
#endif /* DEBUG || DEVELOPMENT */
    break;
 ...
 */
static uint64_t dtrace_dif_emulate(dtrace_difo_t *difo, dtrace_mstate_t *mstate,
                                   dtrace_vstate_t *vstate,
  ...
  while (pc < textlen && !(*flags & CPU_DTRACE_FAULT)) {
    ...
    case DIF_OP_LDX:
       ...
       */
    case DIF_OP_ULDSB:
      regs[rd] = (int8_t)dtrace_fuword8(regs[r1]);
      break;
    case DIF_OP_ULDSH:
      regs[rd] = (int16_t)dtrace_fuword16(regs[r1]);
      break;
    case DIF_OP_ULDSW:
      regs[rd] = (int32_t)dtrace_fuword32(regs[r1]);
      break;
    case DIF_OP_ULDUB:
      regs[rd] = dtrace_fuword8(regs[r1]);
      break;
    case DIF_OP_ULDUH:
      regs[rd] = dtrace_fuword16(regs[r1]);
      break;
    case DIF_OP_ULDUW:
      regs[rd] = dtrace_fuword32(regs[r1]);
      break;
    case DIF_OP_ULDX:
      regs[rd] = dtrace_fuword64(regs[r1]);
      break;
    case DIF_OP_RET:
      rval = regs[rd];
      pc = textlen;
    ...
    case DIF_OP_SETX:
      ...
      break;
    case DIF_OP_SETS:
      regs[rd] = (uint64_t)(uintptr_t)(strtab + DIF_INSTR_STRING(instr));
      break;
    case DIF_OP_SCMP: {
      ...
      if (s1 != 0 && !dtrace_strcanload(s1, sz, &lim1, mstate, vstate))
        break;
      if (s2 != 0 && !dtrace_strcanload(s2, sz, &lim2, mstate, vstate))
        break;

      cc_r = dtrace_strncmp((char *)s1, (char *)s2, MIN(lim1, lim2));
    ...
    case DIF_OP_LDGA:
      regs[rd] = dtrace_dif_variable(mstate, state, r1, regs[r2]);
      break;
    case DIF_OP_LDGS:
    ...
    case DIF_OP_STGS:
      ...
      if (v->dtdv_type.dtdt_flags & DIF_TF_BYREF) {
        ...
        if (!dtrace_vcanload((void *)(uintptr_t)regs[rd], &v->dtdv_type, &lim,
                             mstate, vstate))
          break;

        dtrace_vcopy((void *)(uintptr_t)regs[rd], (void *)a, &v->dtdv_type,
                     lim);
        break;
      }
    ...
    case DIF_OP_STTS: {
      ...
      dvar = dtrace_dynvar(
          dstate, 2, key,
          v->dtdv_type.dtdt_size > sizeof(uint64_t) ? v->dtdv_type.dtdt_size
                                                    : sizeof(uint64_t),
          regs[rd] ? DTRACE_DYNVAR_ALLOC : DTRACE_DYNVAR_DEALLOC, mstate,
          vstate);
      ...
      if (v->dtdv_type.dtdt_flags & DIF_TF_BYREF) {
        ...
        dtrace_vcopy((void *)(uintptr_t)regs[rd], dvar->dtdv_data,
                     &v->dtdv_type, lim);
      } else {
    ...
    case DIF_OP_CALL:
      dtrace_dif_subr(DIF_INSTR_SUBR(instr), rd, regs, tupregs, ttop, mstate,
                      state);
      break;
      ...
      if (r1 == DIF_TYPE_STRING) {
        ...
        tupregs[ttop].dttk_size =
            dtrace_strlen((char *)(uintptr_t)regs[rd],
                          regs[r2] ? regs[r2] : dtrace_strsize_default) +
            1;
      } else {
        if (regs[r2] > LONG_MAX) {
          *flags |= CPU_DTRACE_ILLOP;
    ...
    case DIF_OP_ALLOCS: {
       ...
       * 0.
       */
      if (size < regs[r1] || !DTRACE_INSCRATCH(mstate, size)) {
        DTRACE_CPUFLAG_SET(CPU_DTRACE_NOSCRATCH);
        regs[rd] = 0;
        break;
      }
    ...
    case DIF_OP_COPYS:
      if (!dtrace_canstore(regs[rd], regs[r2], mstate, vstate)) {
        *flags |= CPU_DTRACE_BADADDR;
        *illval = regs[rd];
        break;
      }
      ...
      dtrace_bcopy((void *)(uintptr_t)regs[r1], (void *)(uintptr_t)regs[rd],
                   (size_t)regs[r2]);
      break;
    ...
    case DIF_OP_STX:
      ...
      *((uint64_t *)(uintptr_t)regs[rd]) = regs[r1];
      break;
    case DIF_OP_STRIP:
      regs[rd] = (uint64_t)dtrace_ptrauth_strip((void *)regs[r1], r2);
      break;
    }
  }
...
__attribute__((noinline)) static void dtrace_action_panic(dtrace_ecb_t *ecb) {
  dtrace_probe_t *probe = ecb->dte_probe;
  ...
  panic("dtrace: panic action at probe %s:%s:%s:%s (ecb %p)",
        probe->dtpr_provider->dtpv_name, probe->dtpr_mod, probe->dtpr_func,
        probe->dtpr_name, (void *)ecb);
 ...
 * kernel.
 */
static void dtrace_action_pidresume(uint64_t pid) {
  if (dtrace_destructive_disallow)
    return;
...
dtrace_action_ustack(dtrace_mstate_t *mstate, dtrace_state_t *state,
  ...
  size =
      (uintptr_t)fps - mstate->dtms_scratch_ptr + (nframes * sizeof(uint64_t));
  ...
  for (i = 0; i < nframes; i++) {
    ...
    sym = (char *)(uintptr_t)dtrace_helper(DTRACE_HELPER_ACTION_USTACK, mstate,
                                           state, pcs[i], fps[i]);
...
dtrace_store_by_ref(dtrace_difo_t *dp, caddr_t tomax, size_t size,
                    size_t *valoffsp, uint64_t *valp, uint64_t end, int intuple,
                    int dtkind) {
  volatile uint16_t *flags;
  uint64_t val = *valp;
 ...
 * error probe.
 */
static dtrace_icookie_t dtrace_probe_enter(dtrace_id_t id) {
  ...
  VERIFY(inprobe == 0 || id == dtrace_probeid_error);
 ...
 */
static void dtrace_probe_exit(dtrace_icookie_t cookie) {
  thread_t thread = current_thread();
 ...
 */
void dtrace_probe(dtrace_id_t id, uint64_t arg0, uint64_t arg1, uint64_t arg2,
  ...
  if (vtime) {
    ...
    dtrace_accum_time = dtrace_get_thread_tracing(
        thread); /* Time spent inside DTrace so far (nanoseconds) */

    if (dtrace_accum_time >= 0) {
      recent_vtime = dtrace_abs_to_nano(dtrace_calc_thread_recent_vtime(
          thread)); /* up to the moment thread vtime */

      recent_vtime = recent_vtime -
                     dtrace_accum_time; /* Time without DTrace contribution */

      dtrace_set_thread_vtime(thread, recent_vtime);
    }
  }

  now = dtrace_gethrtime(); /* must not precede
                               dtrace_calc_thread_recent_vtime() call! */
#endif /* __APPLE__ */
    ...
    *flags &= ~CPU_DTRACE_ERROR;
    ...
    if (state->dts_activity != DTRACE_ACTIVITY_ACTIVE) {
      ...
      if (probe->dtpr_id == dtrace_probeid_begin &&
        ...
        ASSERT(state->dts_activity == DTRACE_ACTIVITY_DRAINING);
        continue;
      }
    }
    ...
    if (ecb->dte_cond) {
      ...
      if ((ecb->dte_cond & DTRACE_COND_USERMODE) &&
          ...
          prov->dtpv_pops.dtps_usermode(prov->dtpv_arg, probe->dtpr_id,
                                        probe->dtpr_arg) == 0)
        continue;
      ...
      if (ecb->dte_cond & DTRACE_COND_OWNER) {
        cred_t *cr;
        cred_t *s_cr = ecb->dte_state->dts_cred.dcr_cred;
        proc_t *proc;
#pragma unused(proc) /* __APPLE__ */
        ...
        if ((cr = dtrace_CRED()) == NULL ||
            ...
            posix_cred_get(s_cr)->cr_gid != posix_cred_get(cr)->cr_sgid ||
#if !defined(__APPLE__)
            (proc = ttoproc(curthread)) == NULL || (proc->p_flag & SNOCD))
#else
...
#pragma unused(cr, s_cr) /* __APPLE__ */
    ...
    if (now - state->dts_alive > dtrace_deadman_timeout) {
      ...
      if (!dtrace_priv_kernel_destructive(state) ||
          !state->dts_cred.dcr_destructive || dtrace_destructive_disallow) {
        void *activity = &state->dts_activity;
        dtrace_activity_t current;

        do {
          current = state->dts_activity;
        } while (dtrace_cas32(activity, current, DTRACE_ACTIVITY_KILLED) !=
                 current);
    ...
    if ((offs = dtrace_buffer_reserve(buf, ecb->dte_needed, ecb->dte_alignment,
                                      state, &mstate)) < 0)
      continue;
    ...
    for (act = ecb->dte_action; !(*flags & CPU_DTRACE_ERROR) && act != NULL;
         act = act->dta_next) {
      size_t valoffs;
      dtrace_difo_t *dp;
      ...
      if (DTRACEACT_ISAGG(act->dta_kind)) {
        ...
        if ((dp = act->dta_difo) != NULL)
          v = dtrace_dif_emulate(dp, &mstate, vstate, state);
         ...
         * dtag_hasarg field.
         */
        dtrace_aggregate(agg, buf, offs, aggbuf, v, val);
        continue;
      }
      ...
      case DTRACEACT_STACK:
        ...
        dtrace_getpcstack(
            (pc_t *)(tomax + valoffs), size / sizeof(pc_t), probe->dtpr_aframes,
            DTRACE_ANCHORED(probe) ? NULL : (uint32_t *)(uintptr_t)arg0);
        continue;
      ...
      case DTRACEACT_USTACK:
         ...
         */
        if (DTRACE_ANCHORED(mstate.dtms_probe) && CPU_ON_INTR(CPU)) {
          int depth = DTRACE_USTACK_NFRAMES(rec->dtrd_arg) + 1;
        ...
        if (DTRACE_USTACK_STRSIZE(rec->dtrd_arg) != 0 &&
           ...
           */
          dtrace_action_ustack(&mstate, state, (uint64_t *)(tomax + valoffs),
                               rec->dtrd_arg);
          continue;
        }

        DTRACE_CPUFLAG_SET(CPU_DTRACE_NOFAULT);
        dtrace_getupcstack((uint64_t *)(tomax + valoffs),
      ...
      case DTRACEACT_SPECULATE: {
        ...
        buf = dtrace_speculation_buffer(state, cpuid, val);
        ...
        offs = dtrace_buffer_reserve(buf, ecb->dte_needed, ecb->dte_alignment,
                                     state, NULL);
      ...
      case DTRACEACT_UADDR: {
        ...
        DTRACE_STORE(uint64_t, tomax, valoffs, (uint64_t)dtrace_proc_selfpid());
        DTRACE_STORE(uint64_t, tomax, valoffs + sizeof(uint64_t), val);
      ...
      case DTRACEACT_EXIT: {
        ...
        if (dtrace_cas32(activity, current, DTRACE_ACTIVITY_DRAINING) !=
            current) {
          *flags |= CPU_DTRACE_DROP;
          continue;
        }
      ...
      if (dp->dtdo_rtype.dtdt_flags & (DIF_TF_BYREF | DIF_TF_BYUREF)) {
        ...
        if (tracememsize != 0 && valoffs + tracememsize < end) {
          end = valoffs + tracememsize;
          tracememsize = 0;
        }

        if (dp->dtdo_rtype.dtdt_flags & DIF_TF_BYREF &&
            !dtrace_vcanload((void *)(uintptr_t)val, &dp->dtdo_rtype, NULL,
                             &mstate, vstate)) {
          continue;
        }

        dtrace_store_by_ref(
            dp, tomax, size, &valoffs, &val, end, act->dta_intuple,
            dp->dtdo_rtype.dtdt_flags & DIF_TF_BYREF ? DIF_TF_BYREF
                                                     : DIF_TF_BYUREF);
    ...
    if (*flags & CPU_DTRACE_FAULT) {
       ...
       */
      for (err = ecb->dte_action, ndx = 0; err != act;
           err = err->dta_next, ndx++)
        continue;

      dtrace_probe_error(
          state, ecb->dte_epid, ndx,
          (mstate.dtms_present & DTRACE_MSTATE_FLTOFFS) ? mstate.dtms_fltoffs
                                                        : -1,
          DTRACE_FLAGS2FLT(*flags), cpu_core[cpuid].cpuc_dtrace_illval);
  ...
  /* FIXME: On Darwin the time spent leaving DTrace from this point to the rti
     is attributed to the current thread. Instead it should accrue to DTrace. */
  if (vtime) {
    thread_t thread = current_thread();
    ...
    if (t >= 0) {
      ...
      /* Return from error recursion. No accumulation, just clear the sign bit
       * on t_dtrace_tracing. */
 ...
 * specified.)
 */
static uint_t dtrace_hash_str(const char *p) {
  unsigned int g;
  uint_t hval = 0;
...
static const char *dtrace_strkey_probe_provider(void *elm, uintptr_t offs) {
#pragma unused(offs)
  dtrace_probe_t *probe = (dtrace_probe_t *)elm;
  return probe->dtpr_provider->dtpv_name;
}

static const char *dtrace_strkey_offset(void *elm, uintptr_t offs) {
  return ((char *)((uintptr_t)(elm) + offs));
}

static const char *dtrace_strkey_deref_offset(void *elm, uintptr_t offs) {
  return *((char **)((uintptr_t)(elm) + offs));
}

static dtrace_hash_t *dtrace_hash_create(dtrace_strkey_f func, uintptr_t arg,
                                         uintptr_t nextoffs,
                                         uintptr_t prevoffs) {
  ...
  hash->dth_tab =
      kmem_zalloc(hash->dth_size * sizeof(dtrace_hashbucket_t *), KM_SLEEP);
 ...
 */
#if !defined(__APPLE__)
static void dtrace_hash_destroy(dtrace_hash_t *hash) {
#if DEBUG
  int i;
  ...
  kmem_free(hash->dth_tab, hash->dth_size * sizeof(dtrace_hashbucket_t *));
...
static void dtrace_hash_add(dtrace_hash_t *hash, void *new) {
...
static void *dtrace_hash_lookup_string(dtrace_hash_t *hash, const char *str) {
...
static dtrace_probe_t *dtrace_hash_lookup(dtrace_hash_t *hash, void *template) {
  return dtrace_hash_lookup_string(hash, DTRACE_GETSTR(hash, template));
}

static int dtrace_hash_collisions(dtrace_hash_t *hash, void *template) {
...
static void dtrace_hash_remove(dtrace_hash_t *hash, void *elm) {
 ...
 */
static int dtrace_badattr(const dtrace_attribute_t *a) {
 ...
 */
static char *dtrace_strref(const char *str) {
...
static void dtrace_strunref(const char *str) {
  ASSERT(str != NULL);
  dtrace_string_t *s = NULL;
...
static int dtrace_badname(const char *s) {
  char c;
  ...
  while ((c = *s++) != '\0') {
    if (!DTRACE_ISALPHA(c) && (c < '0' || c > '9') && c != '-' && c != '_' &&
        c != '.' && c != '`')
      return (1);
  }
...
static void dtrace_cred2priv(cred_t *cr, uint32_t *privp, uid_t *uidp,
                             zoneid_t *zoneidp) {
  uint32_t priv;
...
static void dtrace_errdebug(const char *str) {
 ...
 */
static int dtrace_match_priv(const dtrace_probe_t *prp, uint32_t priv,
                             uid_t uid, zoneid_t zoneid) {
  if (priv != DTRACE_PRIV_ALL) {
     ...
     */
    if ((priv & (DTRACE_PRIV_PROC | DTRACE_PRIV_USER | DTRACE_PRIV_KERNEL)) ==
        0)
      return (0);
 ...
 */
static int dtrace_match_probe(const dtrace_probe_t *prp,
                              const dtrace_probekey_t *pkp, uint32_t priv,
                              uid_t uid, zoneid_t zoneid) {
...
/*ARGSUSED*/
static int dtrace_match_module(const char *s, const char *p, int depth) {
#pragma unused(depth) /* __APPLE__ */
  size_t len;
  if (s == NULL || p == NULL)
    return (0);
...
static int dtrace_match(const dtrace_probekey_t *pkp, uint32_t priv, uid_t uid,
                        zoneid_t zoneid,
                        int (*matched)(dtrace_probe_t *, void *, void *),
                        void *arg1, void *arg2) {
  dtrace_probe_t *probe;
  dtrace_provider_t prov_template = {.dtpv_name =
                                         (char *)(uintptr_t)pkp->dtpk_prov};

  dtrace_probe_t template = {.dtpr_provider = &prov_template,
...
static dtrace_probekey_f *dtrace_probekey_module_func(const char *p) {
  LCK_MTX_ASSERT(&dtrace_lock, LCK_MTX_ASSERT_OWNED);
 ...
 */
static void dtrace_probekey(const dtrace_probedesc_t *pdp,
                            dtrace_probekey_t *pkp) {
  ...
  if (pkp->dtpk_id == DTRACE_IDNONE && pkp->dtpk_pmatch == &dtrace_match_nul &&
...
static void dtrace_probekey_release(dtrace_probekey_t *pkp) {
...
static int dtrace_cond_provider_match(dtrace_probedesc_t *desc, void *data) {
  if (desc == NULL)
    return 1;
...
int dtrace_register(const char *name, const dtrace_pattr_t *pap, uint32_t priv,
  ...
  if (dtrace_badattr(&pap->dtpa_provider) || dtrace_badattr(&pap->dtpa_mod) ||
      dtrace_badattr(&pap->dtpa_func) || dtrace_badattr(&pap->dtpa_name) ||
  ...
  if ((priv & DTRACE_PRIV_KERNEL) &&
    ...
    cmn_err(CE_WARN,
            ...
            "dtps_usermode() op for given privilege attributes",
            name);
    return (EINVAL);
  }
  ...
  if (pops->dtps_provide_module == NULL) {
    ...
    provider->dtpv_pops.dtps_provide_module = dtrace_provide_module_nullop;
  }
  ...
  *idp = (dtrace_provider_id_t)provider;
  ...
  if (dtrace_retained != NULL) {
    ...
    dtrace_match_cond_t cond = {dtrace_cond_provider_match,
                                provider->dtpv_name};
...
int dtrace_unregister(dtrace_provider_id_t id) {
  ...
  dtrace_probe_t template = {.dtpr_provider = old};
  ...
  for (probe = first; probe != NULL; probe = next) {
    ...
    old->dtpv_pops.dtps_destroy(old->dtpv_arg, probe->dtpr_id, probe->dtpr_arg);
...
int dtrace_condense(dtrace_provider_id_t id) {
  ...
  dtrace_probe_t template = {.dtpr_provider = prov};
...
dtrace_id_t dtrace_probe_create(dtrace_provider_id_t prov, const char *mod,
                                const char *func, const char *name, int aframes,
                                void *arg) {
...
static int dtrace_probe_lookup_match(dtrace_probe_t *probe, void *arg1,
                                     void *arg2) {
#pragma unused(arg2)
...
dtrace_id_t dtrace_probe_lookup(dtrace_provider_id_t prid, const char *mod,
  ...
  match = dtrace_match(&pkey, DTRACE_PRIV_ALL, 0, 0, dtrace_probe_lookup_match,
                       &id, NULL);
 ...
 */
static void dtrace_probe_description(const dtrace_probe_t *prp,
                                     dtrace_probedesc_t *pdp) {
  bzero(pdp, sizeof(dtrace_probedesc_t));
  pdp->dtpd_id = prp->dtpr_id;

  /* APPLE NOTE: Darwin employs size bounded string operation. */
  (void)strlcpy(pdp->dtpd_provider, prp->dtpr_provider->dtpv_name,
                DTRACE_PROVNAMELEN);
 ...
 */
static void dtrace_probe_provide(dtrace_probedesc_t *desc,
 ...
 * denoted by offs.
 */
static void dtrace_probe_foreach(uintptr_t offs) {
...
static int dtrace_probe_enable(const dtrace_probedesc_t *desc,
                               dtrace_enabling_t *enab, dtrace_ecbdesc_t *ep) {
  dtrace_probekey_t pkey;
  uint32_t priv;
  uid_t uid;
  zoneid_t zoneid;
  ...
  dtrace_cred2priv(enab->dten_vstate->dtvs_state->dts_cred.dcr_cred, &priv,
                   &uid, &zoneid);

  err = dtrace_match(&pkey, priv, uid, zoneid, dtrace_ecb_create_enable, enab,
                     ep);
...
static void dtrace_dofprov2hprov(dtrace_helper_provdesc_t *hprov,
  ...
  dtrace_dofattr2attr(&hprov->dthpv_pattr.dtpa_mod, dofprov->dofpv_modattr);
  dtrace_dofattr2attr(&hprov->dthpv_pattr.dtpa_func, dofprov->dofpv_funcattr);
  dtrace_dofattr2attr(&hprov->dthpv_pattr.dtpa_name, dofprov->dofpv_nameattr);
  dtrace_dofattr2attr(&hprov->dthpv_pattr.dtpa_args, dofprov->dofpv_argsattr);
}
...
static void dtrace_helper_provide_one(dof_helper_t *dhp, dof_sec_t *sec,
  ...
  str_sec =
      (dof_sec_t *)(uintptr_t)(daddr + dof->dofh_secoff +
   ...
   */
  for (i = 0; i < nprobes; i++) {
    probe = (dof_probe_t *)(uintptr_t)(daddr + prb_sec->dofs_offset +
...
static void dtrace_helper_provide(dof_helper_t *dhp, proc_t *p) {
  uintptr_t daddr = (uintptr_t)dhp->dofhp_dof;
  dof_hdr_t *dof = (dof_hdr_t *)daddr;
  uint32_t i;
  ...
  for (i = 0; i < dof->dofh_secnum; i++) {
    dof_sec_t *sec = (dof_sec_t *)(uintptr_t)(daddr + dof->dofh_secoff +
                                              i * dof->dofh_secsize);
...
static void dtrace_helper_provider_remove_one(dof_helper_t *dhp, dof_sec_t *sec,
                                              proc_t *p) {
  uintptr_t daddr = (uintptr_t)dhp->dofhp_dof;
  dof_hdr_t *dof = (dof_hdr_t *)daddr;
  dof_sec_t *str_sec;
...
static void dtrace_helper_provider_remove(dof_helper_t *dhp, proc_t *p) {
  uintptr_t daddr = (uintptr_t)dhp->dofhp_dof;
  dof_hdr_t *dof = (dof_hdr_t *)daddr;
  uint32_t i;
 ...
 * in <sys/dtrace.h>.
 */
int dtrace_meta_register(const char *name, const dtrace_mops_t *mops, void *arg,
  ...
  *idp = DTRACE_METAPROVNONE;
  ...
  if (mops == NULL || mops->dtms_create_probe == NULL ||
      mops->dtms_provide_proc == NULL || mops->dtms_remove_proc == NULL) {
    cmn_err(CE_WARN,
  ...
  if (dtrace_meta_pid != NULL) {
    ...
    cmn_err(CE_WARN,
            "failed to register meta-register %s: "
            "user-land meta-provider exists",
            name);
  ...
  *idp = (dtrace_meta_provider_id_t)meta;
  ...
  while (help != NULL) {
    for (i = 0; i < help->dthps_nprovs; i++) {
      ...
      if (p == PROC_NULL)
        continue;
      dtrace_helper_provide(&help->dthps_provs[i]->dthp_prov, p);
      proc_rele(p);
    }
 ...
 */
static int dtrace_difo_validate(dtrace_difo_t *dp, dtrace_vstate_t *vstate,
                                uint_t nregs, cred_t *cr) {
  int err = 0;
  uint_t i;
  ...
  kcheckload = cr == NULL || (vstate->dtvs_state->dts_cred.dcr_visible &
                              DTRACE_CRV_KERNEL) == 0;
  ...
  for (pc = 0; pc < dp->dtdo_len && err == 0; pc++) {
    ...
    case DIF_OP_LDX:
      ...
      if (kcheckload)
        dp->dtdo_buf[pc] =
            DIF_INSTR_LOAD(op + DIF_OP_RLDSB - DIF_OP_LDSB, r1, rd);
      break;
    case DIF_OP_RLDSB:
    case DIF_OP_RLDSH:
    case DIF_OP_RLDSW:
  ...
  if (dp->dtdo_len != 0 &&
    ...
    err += efunc(dp->dtdo_len - 1, "expected 'ret' as last DIF instruction\n");
  }
  ...
  for (i = 0; i < dp->dtdo_varlen && err == 0; i++) {
    ...
    if (v->dtdv_kind != DIFV_KIND_ARRAY && v->dtdv_kind != DIFV_KIND_SCALAR) {
 ...
 */
static int dtrace_difo_validate_helper(dtrace_difo_t *dp) {
  int (*efunc)(uint_t pc, const char *, ...) = dtrace_difo_err;
  int err = 0;
  uint_t pc;
  ...
  for (pc = 0; pc < dp->dtdo_len; pc++) {
    ...
    case DIF_OP_LDGS:
      ...
      if (v == DIF_VAR_CURTHREAD || v == DIF_VAR_PID || v == DIF_VAR_PPID ||
          v == DIF_VAR_TID || v == DIF_VAR_EXECNAME || v == DIF_VAR_ZONENAME ||
 ...
 * basis; 0 if not.
 */
static int dtrace_difo_cacheable(dtrace_difo_t *dp) {
  uint_t i;
  ...
  for (i = 0; i < dp->dtdo_len; i++) {
    ...
    if ((op >= DIF_OP_LDSB && op <= DIF_OP_LDX) ||
        ...
        (op >= DIF_OP_RLDSB && op <= DIF_OP_RLDX) || op == DIF_OP_LDGA ||
        op == DIF_OP_STTS)
      return (0);
  }
...
static void dtrace_difo_hold(dtrace_difo_t *dp) {
  uint_t i;
  ...
  for (pc = 0; pc < dp->dtdo_len; pc++) {
    ...
    case DIF_OP_PUSHTR:
      ...
      if ((srd == 0 || sval == 0) && r1 == DIF_TYPE_STRING) {
         ...
         * string size.
         */
        tupregs[ttop++].dttk_size = dtrace_strsize_default;
      } else {
        if (srd == 0)
          return;
...
static void dtrace_difo_init(dtrace_difo_t *dp, dtrace_vstate_t *vstate) {
  ...
  for (i = 0; i < dp->dtdo_varlen; i++) {
    ...
    case DIFV_SCOPE_THREAD:
      while (id >= (uint_t)(otlocals = vstate->dtvs_ntlocals)) {
        ...
        if (osz != 0) {
          bcopy(vstate->dtvs_tlocals, tlocals, osz);
    ...
    case DIFV_SCOPE_LOCAL:
      ...
      if (v->dtdv_type.dtdt_flags & DIF_TF_BYREF)
        dsize = (int)NCPU * (v->dtdv_type.dtdt_size + sizeof(uint64_t));
      else
    ...
    case DIFV_SCOPE_GLOBAL:
      ...
      if (v->dtdv_type.dtdt_flags & DIF_TF_BYREF)
        dsize = v->dtdv_type.dtdt_size + sizeof(uint64_t);
    ...
    if ((svar = (*svarp)[id]) == NULL) {
      ...
      if ((svar->dtsv_size = dsize) != 0) {
        svar->dtsv_data = (uint64_t)(uintptr_t)kmem_zalloc(dsize, KM_SLEEP);
      }
...
static dtrace_difo_t *dtrace_difo_duplicate(dtrace_difo_t *dp,
                                            dtrace_vstate_t *vstate) {
  dtrace_difo_t *new;
  size_t sz;
...
static void dtrace_difo_destroy(dtrace_difo_t *dp, dtrace_vstate_t *vstate) {
  uint_t i;
  ...
  for (i = 0; i < dp->dtdo_varlen; i++) {
    ...
    if (svar->dtsv_size != 0) {
      ...
      kmem_free((void *)(uintptr_t)svar->dtsv_data, svar->dtsv_size);
    }
...
static void dtrace_difo_release(dtrace_difo_t *dp, dtrace_vstate_t *vstate) {
  uint_t i;
...
static dtrace_format_t *dtrace_format_new(char *str) {
...
static uint16_t dtrace_format_add(dtrace_state_t *state, char *str) {
  dtrace_format_t **new;
  uint16_t ndx;
...
static void dtrace_format_remove(dtrace_state_t *state, uint16_t format) {
  dtrace_format_t *fmt;
...
static void dtrace_format_destroy(dtrace_state_t *state) {
  int i;
  ...
  kmem_free(state->dts_formats,
            state->dts_nformats * sizeof(*state->dts_formats));
 ...
 */
static dtrace_predicate_t *dtrace_predicate_create(dtrace_difo_t *dp) {
...
static void dtrace_predicate_hold(dtrace_predicate_t *pred) {
  LCK_MTX_ASSERT(&dtrace_lock, LCK_MTX_ASSERT_OWNED);
...
static void dtrace_predicate_release(dtrace_predicate_t *pred,
                                     dtrace_vstate_t *vstate) {
  dtrace_difo_t *dp = pred->dtp_difo;
 ...
 */
static dtrace_actdesc_t *dtrace_actdesc_create(dtrace_actkind_t kind,
                                               uint32_t ntuple, uint64_t uarg,
                                               uint64_t arg) {
  dtrace_actdesc_t *act;

  ASSERT(!DTRACEACT_ISPRINTFLIKE(kind) || (arg != 0 && arg >= KERNELBASE) ||
...
static void dtrace_actdesc_hold(dtrace_actdesc_t *act) {
  ASSERT(act->dtad_refcnt >= 1);
  act->dtad_refcnt++;
}

static void dtrace_actdesc_release(dtrace_actdesc_t *act,
                                   dtrace_vstate_t *vstate) {
...
static int dtrace_ecb_enable(dtrace_ecb_t *ecb) {
  dtrace_probe_t *probe = ecb->dte_probe;
  ...
  if (probe->dtpr_ecb == NULL) {
    ...
    return (prov->dtpv_pops.dtps_enable(prov->dtpv_arg, probe->dtpr_id,
                                        probe->dtpr_arg));
  } else {
    /*
...
static int dtrace_ecb_resize(dtrace_ecb_t *ecb) {
  dtrace_action_t *act;
  ...
  for (act = ecb->dte_action; act != NULL; act = act->dta_next) {
    ...
    if (DTRACEACT_ISAGG(act->dta_kind)) {
      ...
      if (curneeded == UINT32_MAX) {
        ...
        curneeded =
            P2PHASEUP(ecb->dte_size, sizeof(uint64_t), sizeof(dtrace_aggid_t));

        aggbase = curneeded - sizeof(dtrace_aggid_t);
        ASSERT(IS_P2ALIGNED(aggbase, sizeof(uint64_t)));
      }
...
static dtrace_action_t *dtrace_ecb_aggregation_create(dtrace_ecb_t *ecb,
  ...
  case DTRACEAGG_QUANTIZE:
    ...
    size = (((sizeof(uint64_t) * NBBY) - 1) * 2 + 1) * sizeof(uint64_t);
    break;
  ...
  case DTRACEAGG_LLQUANTIZE: {
    ...
    size = (dtrace_aggregate_llquantize_bucket(factor, low, high, nsteps,
                                               INT64_MAX) +
            2) *
           sizeof(uint64_t);
    break;
...
static void dtrace_ecb_aggregation_destroy(dtrace_ecb_t *ecb,
                                           dtrace_action_t *act) {
  dtrace_aggregation_t *agg = (dtrace_aggregation_t *)act;
  dtrace_state_t *state = ecb->dte_state;
...
static int dtrace_ecb_action_add(dtrace_ecb_t *ecb, dtrace_actdesc_t *desc) {
  ...
  if (DTRACEACT_ISAGG(desc->dtad_kind)) {
    ...
    if (DTRACEACT_ISDESTRUCTIVE(desc->dtad_kind) ||
        (desc->dtad_kind == DTRACEACT_DIFEXPR && dp != NULL &&
         dp->dtdo_destructive)) {
    ...
    case DTRACEACT_DIFEXPR:
      ...
      if (arg == 0) {
        ...
        format = dtrace_format_add(state, (char *)(uintptr_t)arg);
      }
    ...
    case DTRACEACT_MOD:
      if (dp == NULL ||
          ((size = dp->dtdo_rtype.dtdt_size) != sizeof(uint64_t)) ||
    ...
    case DTRACEACT_UMOD:
    case DTRACEACT_UADDR:
      if (dp == NULL || (dp->dtdo_rtype.dtdt_size != sizeof(uint64_t)) ||
          (dp->dtdo_rtype.dtdt_flags & DIF_TF_BYREF))
        return (EINVAL);
    ...
    case DTRACEACT_EXIT:
      if (dp == NULL || (size = dp->dtdo_rtype.dtdt_size) != sizeof(int) ||
          (dp->dtdo_rtype.dtdt_flags & DIF_TF_BYREF))
        return (EINVAL);
      break;
...
static void dtrace_ecb_disable(dtrace_ecb_t *ecb) {
  /*
  ...
  if (probe->dtpr_ecb == NULL) {
    ...
    prov->dtpv_pops.dtps_disable(prov->dtpv_arg, probe->dtpr_id,
                                 probe->dtpr_arg);
    dtrace_sync();
  } else {
    /*
...
static void dtrace_ecb_destroy(dtrace_ecb_t *ecb) {
  dtrace_state_t *state = ecb->dte_state;
  dtrace_vstate_t *vstate = &state->dts_vstate;
  dtrace_predicate_t *pred;
...
static int dtrace_ecb_create_enable(dtrace_probe_t *probe, void *arg1,
                                    void *arg2) {
  dtrace_ecb_t *ecb;
...
static dtrace_ecb_t *dtrace_epid2ecb(dtrace_state_t *state, dtrace_epid_t id) {
  dtrace_ecb_t *ecb;
...
static dtrace_aggregation_t *dtrace_aggid2agg(dtrace_state_t *state,
                                              dtrace_aggid_t id) {
  dtrace_aggregation_t *agg;
#pragma unused(agg) /* __APPLE__ */
  ...
  ASSERT((agg = state->dts_aggregations[id - 1]) == NULL || agg->dtag_id == id);
 ...
 * the same CPU.
 */
static void dtrace_buffer_switch(dtrace_buffer_t *buf) {
 ...
 */
static void dtrace_buffer_activate(dtrace_state_t *state) {
  dtrace_buffer_t *buf;
...
static int dtrace_buffer_canalloc(size_t size) {
...
static int dtrace_buffer_alloc(dtrace_buffer_t *bufs, size_t limit, size_t size,
                               int flags, processorid_t cpu) {
  dtrace_cpu_t *cp;
  dtrace_buffer_t *buf;
 ...
 */
static void dtrace_buffer_drop(dtrace_buffer_t *buf) { buf->dtb_drops++; }
 ...
 * error has occurred.
 */
static intptr_t dtrace_buffer_reserve(dtrace_buffer_t *buf, size_t needed,
  ...
  if (!(buf->dtb_flags & (DTRACEBUF_RING | DTRACEBUF_FILL))) {
    while (offs & (align - 1)) {
       ...
       */
      ASSERT(!((align - (offs & (align - 1))) & (sizeof(uint32_t) - 1)));
...
static void dtrace_buffer_polish(dtrace_buffer_t *buf) {
  ASSERT(buf->dtb_flags & DTRACEBUF_RING);
  LCK_MTX_ASSERT(&dtrace_lock, LCK_MTX_ASSERT_OWNED);
  ...
  if (buf->dtb_offset > buf->dtb_xamot_offset) {
    bzero(buf->dtb_tomax + buf->dtb_offset, buf->dtb_size - buf->dtb_offset);
    bzero(buf->dtb_tomax, buf->dtb_xamot_offset);
  }
}

static void dtrace_buffer_free(dtrace_buffer_t *bufs) {
  int i;
 ...
 */
static dtrace_enabling_t *dtrace_enabling_create(dtrace_vstate_t *vstate) {
  dtrace_enabling_t *enab;
...
static void dtrace_enabling_add(dtrace_enabling_t *enab,
...
static void dtrace_enabling_dump(dtrace_enabling_t *enab) {
  int i;

  for (i = 0; i < enab->dten_ndesc; i++) {
    dtrace_probedesc_t *desc = &enab->dten_desc[i]->dted_probe;

    cmn_err(CE_NOTE, "enabling probe %d (%s:%s:%s:%s)", i, desc->dtpd_provider,
            desc->dtpd_mod, desc->dtpd_func, desc->dtpd_name);
  }
}

static void dtrace_enabling_destroy(dtrace_enabling_t *enab) {
  int i;
  dtrace_ecbdesc_t *ep;
  ...
  kmem_free(enab->dten_desc, enab->dten_maxdesc * sizeof(dtrace_enabling_t *));
...
static int dtrace_enabling_retain(dtrace_enabling_t *enab) {
  dtrace_state_t *state;
...
static int dtrace_enabling_replicate(dtrace_state_t *state,
                                     dtrace_probedesc_t *match,
...
static void dtrace_enabling_retract(dtrace_state_t *state) {
...
static int dtrace_enabling_match(dtrace_enabling_t *enab, int *nmatched,
                                 dtrace_match_cond_t *cond) {
  int i = 0;
 ...
 */
static void dtrace_enabling_prime(dtrace_state_t *state) {
  dtrace_enabling_t *enab;
  int i;
 ...
 */
static void dtrace_enabling_provide(dtrace_provider_t *prv) {
  int i, all = 0;
 ...
 */
static dof_hdr_t *dtrace_dof_create(dtrace_state_t *state) {
  dof_hdr_t *dof;
  dof_sec_t *sec;
  dof_optdesc_t *opt;
...
static dof_hdr_t *dtrace_dof_copyin(user_addr_t uarg, int *errp) {
  dof_hdr_t hdr, *dof;
...
static dof_hdr_t *dtrace_dof_copyin_from_proc(proc_t *p, user_addr_t uarg,
                                              int *errp) {
  dof_hdr_t hdr, *dof;
...
static void dtrace_dof_destroy(dof_hdr_t *dof) {
  kmem_free_aligned(dof, dof->dofh_loadsz);
}

static dof_hdr_t *dtrace_dof_property(const char *name) {
  unsigned int len = 0;
  dof_hdr_t *dof;
...
static dof_sec_t *dtrace_dof_sect(dof_hdr_t *dof, uint32_t type,
                                  dof_secidx_t i) {
  dof_sec_t *sec = (dof_sec_t *)(uintptr_t)((uintptr_t)dof + dof->dofh_secoff +
                                            i * dof->dofh_secsize);
...
static dtrace_probedesc_t *dtrace_dof_probedesc(dof_hdr_t *dof, dof_sec_t *sec,
  ...
  (void)strncpy(desc->dtpd_provider, (char *)(str + probe->dofp_provider),
...
static dtrace_difo_t *dtrace_dof_difo(dof_hdr_t *dof, dof_sec_t *sec,
                                      dtrace_vstate_t *vstate, cred_t *cr) {
  dtrace_difo_t *dp;
  size_t ttl = 0;
  dof_difohdr_t *dofd;
  uintptr_t daddr = (uintptr_t)dof;
  ...
  static const struct {
    ...
    int align;
    const char *msg;
  } difo[] = {{DOF_SECT_DIF, offsetof(dtrace_difo_t, dtdo_buf),
              ...
              {DOF_SECT_STRTAB, offsetof(dtrace_difo_t, dtdo_strtab),
               offsetof(dtrace_difo_t, dtdo_strlen), 0, sizeof(char),
  ...
  for (l = 0; l < n; l++) {
    ...
    if ((subsec = dtrace_dof_sect(dof, DOF_SECT_NONE, dofd->dofd_links[l])) ==
        NULL)
    ...
    for (i = 0; difo[i].section != DOF_SECT_NONE; i++) {
      ...
      *bufp = kmem_alloc(subsec->dofs_size, KM_SLEEP);
      bcopy((char *)(uintptr_t)(daddr + subsec->dofs_offset), *bufp,
            subsec->dofs_size);
...
static dtrace_actdesc_t *dtrace_dof_actdesc(dof_hdr_t *dof, dof_sec_t *sec,
  ...
  for (offs = 0; offs < sec->dofs_size; offs += sec->dofs_entsize) {
    desc = (dof_actdesc_t *)(daddr + (uintptr_t)sec->dofs_offset + offs);
    ...
    if ((DTRACEACT_ISPRINTFLIKE(kind) &&
       ...
       */
      if ((strtab = dtrace_dof_sect(dof, DOF_SECT_STRTAB, desc->dofa_strtab)) ==
          NULL)
        goto err;

      str = (char *)((uintptr_t)dof + (uintptr_t)strtab->dofs_offset);
    ...
    act = dtrace_actdesc_create(kind, desc->dofa_ntuple, desc->dofa_uarg, arg);
    ...
    if ((difosec = dtrace_dof_sect(dof, DOF_SECT_DIFOHDR, desc->dofa_difo)) ==
        NULL)
      goto err;
...
static dtrace_ecbdesc_t *dtrace_dof_ecbdesc(dof_hdr_t *dof, dof_sec_t *sec,
  ...
  if (ecb->dofe_pred != DOF_SECIDX_NONE) {
    if ((sec = dtrace_dof_sect(dof, DOF_SECT_DIFOHDR, ecb->dofe_pred)) == NULL)
      goto err;
  ...
  if (ecb->dofe_actions != DOF_SECIDX_NONE) {
    if ((sec = dtrace_dof_sect(dof, DOF_SECT_ACTDESC, ecb->dofe_actions)) ==
        NULL)
      goto err;
 ...
 */
static int dtrace_dof_slurp(dof_hdr_t *dof, dtrace_vstate_t *vstate, cred_t *cr,
                            dtrace_enabling_t **enabp, uint64_t ubase,
                            int noprobes) {
#pragma unused(ubase) /* __APPLE__ */
   ...
   */
  if (bcmp(&dof->dofh_ident[DOF_ID_MAG0], DOF_MAG_STRING, DOF_MAG_STRLEN) !=
      0) {
   ...
   */
  for (i = 0; i < dof->dofh_secnum; i++) {
    dof_sec_t *sec = (dof_sec_t *)(daddr + (uintptr_t)dof->dofh_secoff +
                                   i * dof->dofh_secsize);
    ...
    if (sec->dofs_type == DOF_SECT_STRTAB &&
        *((char *)daddr + sec->dofs_offset + sec->dofs_size - 1) != '\0') {
 ...
 */
static int dtrace_dof_options(dof_hdr_t *dof, dtrace_state_t *state) {
  uint_t i;
  int rval;
  uint32_t entsize;
  size_t offs;
  ...
  for (i = 0; i < dof->dofh_secnum; i++) {
    dof_sec_t *sec =
        (dof_sec_t *)((uintptr_t)dof + (uintptr_t)dof->dofh_secoff +
                      i * dof->dofh_secsize);
    ...
    for (offs = 0; offs < sec->dofs_size; offs += entsize) {
      desc = (dof_optdesc_t *)((uintptr_t)dof + (uintptr_t)sec->dofs_offset +
                               offs);
      ...
      if ((rval = dtrace_state_option(state, desc->dofo_option,
                                      desc->dofo_value)) != 0) {
 ...
 */
static int dtrace_dstate_init(dtrace_dstate_t *dstate, size_t size) {
  ...
  dstate->dtds_percpu =
      zalloc_percpu(dtrace_state_pcpu_zone, Z_WAITOK | Z_ZERO);
  ...
  zpercpu_foreach_cpu(i) {
    ...
    for (;;) {
      next = (dtrace_dynvar_t *)((uintptr_t)dvar + dstate->dtds_chunksize);
...
static void dtrace_dstate_fini(dtrace_dstate_t *dstate) {
  LCK_MTX_ASSERT(&cpu_lock, LCK_MTX_ASSERT_OWNED);
...
static void dtrace_vstate_fini(dtrace_vstate_t *vstate) {
  /*
  ...
  if (vstate->dtvs_nglobals > 0) {
    kmem_free(vstate->dtvs_globals,
              vstate->dtvs_nglobals * sizeof(dtrace_statvar_t *));
  }

  if (vstate->dtvs_ntlocals > 0) {
    kmem_free(vstate->dtvs_tlocals,
              vstate->dtvs_ntlocals * sizeof(dtrace_difv_t));
  }

  ASSERT((vstate->dtvs_nlocals == 0) ^ (vstate->dtvs_locals != NULL));

  if (vstate->dtvs_nlocals > 0) {
    kmem_free(vstate->dtvs_locals,
              vstate->dtvs_nlocals * sizeof(dtrace_statvar_t *));
  }
}
...
static int dtrace_state_create(dev_t *devp, cred_t *cr,
                               dtrace_state_t **new_state) {
  minor_t minor;
  major_t major;
  char c[30];
  dtrace_state_t *state;
  ...
  *new_state = NULL;
  ...
  if (NULL == state) {
    printf("dtrace_open: couldn't acquire minor number %d. This usually means "
           "that too many DTrace clients are in use at the moment",
           minor);
  ...
  state->dts_aggid_arena = vmem_create(c, (void *)1, INT32_MAX, 1, NULL, NULL,
  ...
  if (cr == NULL || PRIV_POLICY_ONLY(cr, PRIV_ALL, B_FALSE)) {
    if (dtrace_is_restricted() && !dtrace_are_restrictions_relaxed()) {
      ...
      state->dts_cred.dcr_action = DTRACE_CRA_PROC | DTRACE_CRA_PROC_CONTROL |
  ...
  if (cr == NULL || PRIV_POLICY_ONLY(cr, PRIV_ALL, B_FALSE)) {
    ...
    if (PRIV_POLICY_ONLY(cr, PRIV_DTRACE_USER, B_FALSE)) {
      if (PRIV_POLICY_ONLY(cr, PRIV_PROC_OWNER, B_FALSE)) {
        ...
        state->dts_cred.dcr_action |= DTRACE_CRA_PROC_DESTRUCTIVE_ALLUSER;
      }

      if (PRIV_POLICY_ONLY(cr, PRIV_PROC_ZONE, B_FALSE)) {
        state->dts_cred.dcr_visible |= DTRACE_CRV_ALLZONE;

        state->dts_cred.dcr_action |= DTRACE_CRA_PROC_DESTRUCTIVE_ALLZONE;
      }
      ...
      state->dts_cred.dcr_action |= DTRACE_CRA_PROC_DESTRUCTIVE_CREDCHG;
    }
    ...
    if (PRIV_POLICY_ONLY(cr, PRIV_DTRACE_KERNEL, B_FALSE)) {
      ...
      state->dts_cred.dcr_visible |=
          DTRACE_CRV_KERNEL | DTRACE_CRV_ALLPROC | DTRACE_CRV_ALLZONE;

      state->dts_cred.dcr_action |= DTRACE_CRA_KERNEL | DTRACE_CRA_PROC;
      /*
...
static int dtrace_state_go(dtrace_state_t *state, processorid_t *cpu) {
  ...
  if (opt[DTRACEOPT_GRABANON] != DTRACEOPT_UNSET) {
     ...
     * grabbed state.
     */
    state->dts_options[DTRACEOPT_GRABANON] = opt[DTRACEOPT_GRABANON];
  ...
  *cpu = CPU->cpu_id;
  ...
  dtrace_probe(dtrace_probeid_begin, (uint64_t)(uintptr_t)state, 0, 0, 0, 0);
  dtrace_interrupt_enable(cookie);
  /*
   ...
   */
  dtrace_xcall(DTRACE_CPUALL, (dtrace_xcall_t)dtrace_buffer_activate, state);
  goto out;
...
static int dtrace_state_stop(dtrace_state_t *state, processorid_t *cpu) {
  dtrace_icookie_t cookie;
  ...
  cookie = dtrace_interrupt_disable();
  *cpu = CPU->cpu_id;
  dtrace_probe(dtrace_probeid_end, (uint64_t)(uintptr_t)state, 0, 0, 0, 0);
  dtrace_interrupt_enable(cookie);
...
static int dtrace_state_option(dtrace_state_t *state, dtrace_optid_t option,
                               dtrace_optval_t val) {
  LCK_MTX_ASSERT(&dtrace_lock, LCK_MTX_ASSERT_OWNED);
...
static void dtrace_state_destroy(dtrace_state_t *state) {
  dtrace_ecb_t *ecb;
  dtrace_vstate_t *vstate = &state->dts_vstate;
  minor_t minor = getminor(state->dts_dev);
...
int dtrace_keep_kernel_symbols(void) {
  if (dtrace_is_restricted() && !dtrace_are_restrictions_relaxed()) {
    return 0;
  }
...
static void dtrace_helper_trace(dtrace_helper_action_t *helper,
  ...
  size = sizeof(dtrace_helptrace_t) +
         dtrace_helptrace_nlocals * sizeof(uint64_t) - sizeof(uint64_t);
  ...
  for (i = 0; i < vstate->dtvs_nlocals; i++) {
    ...
    ASSERT(svar->dtsv_size >= (int)NCPU * sizeof(uint64_t));
    ent->dtht_locals[i] = ((uint64_t *)(uintptr_t)svar->dtsv_data)[CPU->cpu_id];
  }
}

__attribute__((noinline)) static uint64_t
dtrace_helper(int which, dtrace_mstate_t *mstate, dtrace_state_t *state,
  ...
  for (; helper != NULL; helper = helper->dtha_next) {
    ...
    for (i = 0; i < helper->dtha_nactions; i++) {
      if (trace)
        dtrace_helper_trace(helper, mstate, vstate, i + 1);

      rval = dtrace_dif_emulate(helper->dtha_actions[i], mstate, vstate, state);
  ...
  next:
    if (trace)
      dtrace_helper_trace(helper, mstate, vstate, DTRACE_HELPTRACE_NEXT);
  }

  if (trace)
    dtrace_helper_trace(helper, mstate, vstate, DTRACE_HELPTRACE_DONE);
...
err:
  if (trace)
    dtrace_helper_trace(helper, mstate, vstate, DTRACE_HELPTRACE_ERR);
...
static int dtrace_helper_destroygen(proc_t *p, int gen) {
  ...
  for (;;) {
    ...
    if (dtrace_meta_pid != NULL) {
      ...
      dtrace_helper_provider_remove(&prov->dthp_prov, p);
    }
...
static int dtrace_helper_action_add(proc_t *p, int which,
                                    dtrace_ecbdesc_t *ep) {
  ...
  helper->dtha_actions = kmem_zalloc(
      sizeof(dtrace_difo_t *) * (helper->dtha_nactions = nactions), KM_SLEEP);
...
static void dtrace_helper_provider_register(proc_t *p, dtrace_helpers_t *help,
                                            dof_helper_t *dofhp) {
  LCK_MTX_ASSERT(&dtrace_meta_lock, LCK_MTX_ASSERT_OWNED);
  LCK_MTX_ASSERT(&dtrace_lock, LCK_MTX_ASSERT_NOTOWNED);
...
static int dtrace_helper_provider_add(proc_t *p, dof_helper_t *dofhp, int gen) {
  dtrace_helpers_t *help;
   ...
   */
  for (i = 0; i < help->dthps_nprovs; i++) {
    if (dofhp->dofhp_addr == help->dthps_provs[i]->dthp_prov.dofhp_addr)
      return (EALREADY);
  }
  ...
  if (help->dthps_maxprovs == help->dthps_nprovs) {
    ...
    help->dthps_provs = kmem_zalloc(
        help->dthps_maxprovs * sizeof(dtrace_helper_provider_t *), KM_SLEEP);

    if (tmp_provs != NULL) {
      bcopy(tmp_provs, help->dthps_provs,
            tmp_maxprovs * sizeof(dtrace_helper_provider_t *));
      kmem_free(tmp_provs, tmp_maxprovs * sizeof(dtrace_helper_provider_t *));
    }
  }
...
static int dtrace_helper_provider_validate(dof_hdr_t *dof, dof_sec_t *sec) {
  ...
  if (str_sec == NULL || prb_sec == NULL || arg_sec == NULL || off_sec == NULL)
    return (-1);
  ...
  for (j = 0; j < nprobes; j++) {
     ...
     */
    if (probe->dofpr_offidx + probe->dofpr_noffs < probe->dofpr_offidx ||
        (probe->dofpr_offidx + probe->dofpr_noffs) * off_sec->dofs_entsize >
            off_sec->dofs_size) {
    ...
    if (dof->dofh_ident[DOF_ID_VERSION] != DOF_VERSION_1) {
       ...
       */
      if (enoff_sec == NULL) {
        if (probe->dofpr_enoffidx != 0 || probe->dofpr_nenoffs != 0) {
          dtrace_dof_error(dof, "is-enabled "
                                "offsets with null section");
          return (-1);
        }
      } else if (probe->dofpr_enoffidx + probe->dofpr_nenoffs <
                     probe->dofpr_enoffidx ||
    ...
    if (probe->dofpr_argidx + probe->dofpr_xargc < probe->dofpr_argidx ||
        (probe->dofpr_argidx + probe->dofpr_xargc) * arg_sec->dofs_entsize >
            arg_sec->dofs_size) {
...
static int dtrace_helper_slurp(proc_t *p, dof_hdr_t *dof, dof_helper_t *dhp) {
  dtrace_helpers_t *help;
  dtrace_vstate_t *vstate;
  ...
  if ((rv = dtrace_dof_slurp(dof, vstate, NULL, &enab,
                             dhp != NULL ? dhp->dofhp_addr : 0, B_FALSE)) !=
      0) {
    dtrace_dof_destroy(dof);
    return (rv);
  }
  ...
  for (i = 0; i < enab->dten_ndesc; i++) {
    ...
    if ((rv = dtrace_helper_action_add(p, DTRACE_HELPER_ACTION_USTACK, ep)) !=
        0) {
      /*
 ...
 */
static int dtrace_lazy_dofs_add(proc_t *p, dof_ioctl_data_t *incoming_dofs,
                                int *dofs_claimed) {
  ASSERT(p);
  ...
  *dofs_claimed = 0;
  ...
  if (dtrace_dof_mode == DTRACE_DOF_MODE_LAZY_ON &&
    ...
    unsigned int existing_dofs_count =
        (existing_dofs) ? existing_dofs->dofiod_count : 0;
    unsigned int i,
        merged_dofs_count = incoming_dofs->dofiod_count + existing_dofs_count;
    ...
    uint64_t generation =
        (existing_dofs)
            ? existing_dofs->dofiod_helpers[existing_dofs_count - 1].dofhp_dof +
                  1
            : 1;
    for (i = 0; i < incoming_dofs->dofiod_count; i++) {
      /*
       * We rely on these being the same so we can overwrite dofhp_dof and not
       * lose info.
       */
      ASSERT(incoming_dofs->dofiod_helpers[i].dofhp_dof ==
             incoming_dofs->dofiod_helpers[i].dofhp_addr);
    ...
    if (existing_dofs) {
      ...
      bcopy(&existing_dofs->dofiod_helpers[0], &merged_dofs->dofiod_helpers[0],
    ...
    for (i = 0; i < all_dofs->dofiod_count - 1; i++) {
      ASSERT(all_dofs->dofiod_helpers[i].dofhp_dof <
             all_dofs->dofiod_helpers[i + 1].dofhp_dof);
    }
#endif /* DEBUG */
 ...
 */
static int dtrace_lazy_dofs_remove(proc_t *p, int generation) {
  int rval = EINVAL;
...
void dtrace_lazy_dofs_destroy(proc_t *p) {
  lck_rw_lock_shared(&dtrace_dof_mode_lock);
  dtrace_sprlock(p);
...
static int dtrace_lazy_dofs_proc_iterate_filter(proc_t *p, void *ignored) {
...
static void dtrace_lazy_dofs_process(proc_t *p) {
  /*
  ...
  if (lazy_dofs != NULL) {
    ...
    for (i = 0; i < lazy_dofs->dofiod_count; i++) {
      /*
       * When loading lazy dof, we depend on the generations being sorted in
       * ascending order.
       */
      ASSERT(i >= (lazy_dofs->dofiod_count - 1) ||
             lazy_dofs->dofiod_helpers[i].dofhp_dof <
                 lazy_dofs->dofiod_helpers[i + 1].dofhp_dof);

      dof_helper_t *dhp = &lazy_dofs->dofiod_helpers[i];

      /*
       * We stored the generation in dofhp_dof. Save it, and restore the
       * original value.
       */
      ...
      if (dof != NULL) {
        ...
        if (help->dthps_generation <= generation) {
          ...
          if ((rval = dtrace_helper_slurp(p, dof, dhp)) != generation) {
            dtrace_dof_error(
                NULL, "returned value did not match expected generation");
          }
        }
...
static int dtrace_lazy_dofs_proc_iterate_doit(proc_t *p, void *ignored) {
#pragma unused(ignored)
...
static int dtrace_lazy_dofs_duplicate(proc_t *parent, proc_t *child) {
  LCK_MTX_ASSERT(&dtrace_lock, LCK_MTX_ASSERT_NOTOWNED);
   ...
   */
  ASSERT(parent->p_dtrace_lazy_dofs == NULL ||
         parent->p_dtrace_helpers == NULL);
  /*
...
static dtrace_helpers_t *dtrace_helpers_create(proc_t *p) {
  ...
  help->dthps_actions = kmem_zalloc(
      sizeof(dtrace_helper_action_t *) * DTRACE_NHELPER_ACTIONS, KM_SLEEP);
...
static void dtrace_helpers_destroy(proc_t *p) {
  dtrace_helpers_t *help;
  dtrace_vstate_t *vstate;
  uint_t i;
  ...
  if (help->dthps_maxprovs > 0) {
    if (dtrace_meta_pid != NULL) {
      ...
      for (i = 0; i < help->dthps_nprovs; i++) {
        dtrace_helper_provider_remove(&help->dthps_provs[i]->dthp_prov, p);
      }
    } else {
      lck_mtx_lock(&dtrace_lock);
      ASSERT(help->dthps_deferred == 0 || help->dthps_next != NULL ||
             help->dthps_prev != NULL || help == dtrace_deferred_pid);
    ...
    kmem_free(help->dthps_provs,
              help->dthps_maxprovs * sizeof(dtrace_helper_provider_t *));
  }
...
static void dtrace_helpers_duplicate(proc_t *from, proc_t *to) {
  ...
  for (i = 0; i < DTRACE_NHELPER_ACTIONS; i++) {
    ...
    for (last = NULL; helper != NULL; helper = helper->dtha_next) {
      new = kmem_zalloc(sizeof(dtrace_helper_action_t), KM_SLEEP);
  ...
  if (help->dthps_nprovs > 0) {
    ...
    newhelp->dthps_provs = kmem_alloc(
        newhelp->dthps_nprovs * sizeof(dtrace_helper_provider_t *), KM_SLEEP);
...
void dtrace_proc_fork(proc_t *parent_proc, proc_t *child_proc, int spawn) {
  /*
  ...
  if (!spawn) {
     ...
     */
    if (dtrace_lazy_dofs_duplicate(parent_proc, child_proc) ==
...
void dtrace_proc_exec(proc_t *p) {
  /*
   * Invalidate any predicate evaluation already cached for this thread by
   * DTrace. That's because we've just stored to p_comm and DTrace refers to
   * that when it evaluates the "execname" special variable. uid and gid may
...
static void dtrace_modctl_add(struct modctl *newctl) {
...
static modctl_t *dtrace_modctl_lookup(struct kmod_info *kmod) {
  LCK_MTX_ASSERT(&mod_lock, LCK_MTX_ASSERT_OWNED);
 ...
 */
static void dtrace_modctl_remove(struct modctl *ctl) {
  ASSERT(ctl != NULL);
  LCK_MTX_ASSERT(&mod_lock, LCK_MTX_ASSERT_OWNED);
...
static int dtrace_module_loaded(struct kmod_info *kmod, uint32_t flag) {
  dtrace_provider_t *prv;

  /*
   * If kernel symbols have been disabled, return immediately
   * DTRACE_KERNEL_SYMBOLS_NEVER is a permanent mode, it is safe to test without
   * holding locks
   */
  ...
  if (ctl != NULL) {
    ...
    if (dtrace_err_verbose)
      cmn_err(CE_WARN,
              "dtrace load module already exists '%s %u' is failing against "
              "'%s %u'",
              kmod->name, (uint_t)kmod->id, ctl->mod_modname, ctl->mod_id);
    return (-1);
  } else {
    ctl = kmem_alloc(sizeof(struct modctl), KM_SLEEP);
    if (ctl == NULL) {
      if (dtrace_err_verbose)
        cmn_err(CE_WARN, "dtrace module load '%s %u' is failing ", kmod->name,
                (uint_t)kmod->id);
      lck_mtx_unlock(&mod_lock);
      lck_mtx_unlock(&dtrace_provider_lock);
      return (-1);
  ...
  /*
   * We must hold the dtrace_lock to safely test non permanent
   * dtrace_fbt_symbol_mode(s)
   */
  lck_mtx_lock(&dtrace_lock);
  ...
  /* APPLE NOTE!
   *
   * The cpu_lock mentioned above is only held by dtrace code, Apple's xnu never
   * actually holds it for any reason. Thus the comment above is invalid, we can
   * directly invoke dtrace_enabling_matchall without jumping through all the
   * hoops, and we can avoid the delay call as well.
   */
  lck_mtx_unlock(&dtrace_lock);
...
static int dtrace_cpu_setup(cpu_setup_t what, processorid_t cpu) {
  LCK_MTX_ASSERT(&cpu_lock, LCK_MTX_ASSERT_OWNED);
  lck_mtx_lock(&dtrace_lock);
...
static void dtrace_cpu_setup_initial(processorid_t cpu) {
 ...
 */
/*ARGSUSED*/
static int dtrace_attach(dev_info_t *devi) {
  ...
  /* Darwin uses BSD cloning device driver to automagically obtain minor device
   * number. */
  dtrace_devi = devi;
  ...
  dtrace_arena = vmem_create("dtrace", (void *)1, INT32_MAX, 1, NULL, NULL,
                             NULL, 0, VM_SLEEP | VMC_IDENTIFIER);
  ...
  dtrace_probes =
      kmem_zalloc(sizeof(dtrace_probe_t *) * dtrace_nprobes, KM_SLEEP);

  dtrace_byprov =
      dtrace_hash_create(dtrace_strkey_probe_provider, 0, /* unused */
   ...
   * pseudo provider.
   */
  (void)dtrace_register("dtrace", &dtrace_provider_attr, DTRACE_PRIV_NONE, 0,
  ...
  dtrace_probeid_begin = dtrace_probe_create(
      (dtrace_provider_id_t)dtrace_provider, NULL, NULL, "BEGIN", 1, NULL);
  ...
  dtrace_probeid_error = dtrace_probe_create(
      (dtrace_provider_id_t)dtrace_provider, NULL, NULL, "ERROR", 3, NULL);
#elif defined(__arm64__)
  dtrace_probeid_begin = dtrace_probe_create(
      (dtrace_provider_id_t)dtrace_provider, NULL, NULL, "BEGIN", 2, NULL);
  dtrace_probeid_end = dtrace_probe_create(
      (dtrace_provider_id_t)dtrace_provider, NULL, NULL, "END", 1, NULL);
  dtrace_probeid_error = dtrace_probe_create(
      (dtrace_provider_id_t)dtrace_provider, NULL, NULL, "ERROR", 4, NULL);
#else
#error Unknown Architecture
#endif
  ...
  if (dtrace_helptrace_enabled) {
    ...
    dtrace_helptrace_buffer = kmem_zalloc(dtrace_helptrace_bufsize, KM_SLEEP);
    dtrace_helptrace_next = 0;
  }
...
/*ARGSUSED*/
static int dtrace_open(dev_t *devp, int flag, int otyp, cred_t *cred_p) {
#pragma unused(flag, otyp)
  ...
  if (dtrace_dof_mode == DTRACE_DOF_MODE_LAZY_ON) {
    ...
    proc_iterate(PROC_ALLPROCLIST | PROC_NOWAITTRANS,
                 dtrace_lazy_dofs_proc_iterate_doit, NULL,
                 dtrace_lazy_dofs_proc_iterate_filter, NULL);
...
/*ARGSUSED*/
static int dtrace_close(dev_t dev, int flag, int otyp, cred_t *cred_p) {
#pragma unused(flag, otyp, cred_p) /* __APPLE__ */
  ...
  if (dtrace_opens == 0) {
    ...
    /*
     * If we are the last dtrace client, switch back to lazy (from userspace)
     * symbols
     */
   ...
   * Since closing an active dtrace context is likely to drop that last
   * reference, lets take a shot at cleaning out the orphaned probes now.
   */
...
/*ARGSUSED*/
static int dtrace_ioctl_helper(u_long cmd, caddr_t arg, int *rv) {
#pragma unused(rv)
  /*
...
#endif /* __arm64__*/
  {
    ...
    /*
     * Check that the count didn't change between the first copyin and the
     * second.
     */
    ...
    if (rval == EACCES) {
      ...
      do {
        ...
        if (dof != NULL) {
           ...
           * it may free it now or it may save it and free it later.
           */
          if ((dhp->dofhp_dof = (uint64_t)dtrace_helper_slurp(p, dof, dhp)) ==
              -1ULL) {
            rval = EINVAL;
          }
...
/*ARGSUSED*/
static int dtrace_ioctl(dev_t dev, u_long cmd, user_addr_t arg, int md,
                        cred_t *cr, int *rv) {
#pragma unused(md)
  minor_t minor = getminor(dev);
  dtrace_state_t *state;
  ...
  case DTRACEIOC_AGGDESC: {
    ...
    for (act = agg->dtag_first;; act = act->dta_next) {
      ASSERT(act->dta_intuple || DTRACEACT_ISAGG(act->dta_kind));
    ...
    if (cmd == DTRACEIOC_PROBEMATCH) {
      ...
      for (i = desc.dtpd_id; i <= (dtrace_id_t)dtrace_nprobes; i++) {
        if ((probe = dtrace_probes[i - 1]) != NULL &&
            (m = dtrace_match_probe(probe, &pkey, priv, uid, zoneid)) != 0)
          break;
      }
  ...
  case DTRACEIOC_PROBEARG: {
    ...
    if (prov->dtpv_pops.dtps_getargdesc == NULL) {
      ...
      prov->dtpv_pops.dtps_getargdesc(prov->dtpv_arg, probe->dtpr_id,
                                      probe->dtpr_arg, &desc);
    }
  ...
  case DTRACEIOC_SLEEP: {
    ...
    if (assert_wait_deadline(state, THREAD_ABORTSAFE, abstime) ==
        THREAD_WAITING) {
  ...
  case DTRACEIOC_BUFSNAP: {
    ...
    dtrace_xcall(desc.dtbd_cpu, (dtrace_xcall_t)dtrace_buffer_switch, buf);
...
#pragma unused(old)
     ...
     */
    if (dtrace_buffer_copyout(buf->dtb_xamot, (user_addr_t)desc.dtbd_data,
  ...
  case DTRACEIOC_STATUS: {
    ...
    stat.dtst_killed = (state->dts_activity == DTRACE_ACTIVITY_KILLED);
  ...
  case DTRACEIOC_FORMAT: {
    ...
    if (fmt.dtfd_format == 0 || fmt.dtfd_format > state->dts_nformats) {
      lck_mtx_unlock(&dtrace_lock);
      return (EINVAL);
    }
  ...
  case DTRACEIOC_MODUUIDSLIST: {
    ...
    /*
     * Security restrictions make this operation illegal, if this is enabled
     * DTrace must refuse to provide any fbt probes.
     */
     ...
     * Both NEVER & ALWAYS_FROM_KERNEL are permanent states, it is legal to
     * check for them without holding the dtrace_lock.
     */
    if (dtrace_kernel_symbol_mode == DTRACE_KERNEL_SYMBOLS_NEVER ||
    ...
    while (ctl) {
      /*
       * We assume that userspace symbols will be "better" than kernel level
       * symbols, as userspace can search for dSYM(s) and symbol'd binaries.
       * Even if kernel syms are available, add user syms if the module might
       * use them.
       */
      ASSERT(!MOD_HAS_USERSPACE_SYMBOLS(ctl));
      if (!MOD_SYMBOLS_DONE(ctl) && !MOD_IS_STATIC_KEXT(ctl)) {
  ...
  case DTRACEIOC_PROVMODSYMS: {
    ...
    if (module_symbols_size == 0 ||
        module_symbols_size > (size_t)dtrace_copy_maxsize()) {
    ...
    while (ctl) {
      ...
      if (MOD_HAS_UUID(ctl) && !MOD_SYMBOLS_DONE(ctl) &&
          memcmp(module_symbols->dtmodsyms_uuid, ctl->mod_uuid, sizeof(UUID)) ==
              0) {
        dtrace_provider_t *prv;
        ...
        for (prv = dtrace_provider; prv != NULL; prv = prv->dtpv_next)
          prv->dtpv_pops.dtps_provide_module(prv->dtpv_arg, ctl);
        /*
         * We gave every provider a chance to provide with the user syms, go
         * ahead and clear them
         */
        ctl->mod_user_symbols =
            NULL; /* MUST reset this to clear HAS_USERSPACE_SYMBOLS */
      }
      ctl = ctl->mod_next;
    }
  ...
  case DTRACEIOC_PROCWAITFOR: {
    dtrace_procdesc_t pdesc = {.p_name = {0}, .p_pid = -1};
...
#if !defined(__APPLE__)
/*ARGSUSED*/
static int dtrace_detach(dev_info_t *dip, ddi_detach_cmd_t cmd) {
  dtrace_state_t *state;
...
int _dtrace_open(dev_t dev, int flags, int devtype, struct proc *p) {
#pragma unused(p)
  dev_t locdev = dev;
...
int helper_open(dev_t dev, int flags, int devtype, struct proc *p) {
#pragma unused(dev, flags, devtype, p)
  return 0;
}

int _dtrace_close(dev_t dev, int flags, int devtype, struct proc *p) {
#pragma unused(p)
  return dtrace_close(dev, flags, devtype, CRED());
}

int helper_close(dev_t dev, int flags, int devtype, struct proc *p) {
#pragma unused(dev, flags, devtype, p)
  return 0;
}

int _dtrace_ioctl(dev_t dev, u_long cmd, caddr_t data, int fflag,
                  struct proc *p) {
#pragma unused(p)
  int err, rv = 0;
  user_addr_t uaddrp;
  ...
  /* Darwin's BSD ioctls only return -1 or zero. Overload errno to mimic
   * Solaris. 20 bits suffice. */
  if (err != 0) {
    ASSERT((err & 0xfffff000) == 0);
    return (err & 0xfff); /* ioctl will return -1 and will set errno to an error
                             code < 4096 */
  } else if (rv != 0) {
    ASSERT((rv & 0xfff00000) == 0);
    return (((rv & 0xfffff) << 12)); /* ioctl will return -1 and will set errno
                                        to a value >= 4096 */
  } else
    return 0;
}

int helper_ioctl(dev_t dev, u_long cmd, caddr_t data, int fflag,
                 struct proc *p) {
...
void helper_init(void) {
  ...
  if (0 >= helper_majdevno) {
    ...
    if (NULL == devfs_make_node(makedev(helper_majdevno, 0), DEVFS_CHAR,
                                UID_ROOT, GID_WHEEL, 0666, DTRACEMNR_HELPER)) {
...
static int dtrace_clone_func(dev_t dev, int action) {
#pragma unused(dev)
...
void dtrace_early_init(void) {
   ...
   */
  if (!PE_parse_boot_argn("dtrace_kernel_symbol_mode",
                          &dtrace_kernel_symbol_mode,
...
void dtrace_init(void) {
  if (0 == gDTraceInited) {
    ...
    dtrace_destructive_disallow =
        dtrace_is_restricted() && !dtrace_are_restrictions_relaxed();
    ...
    if (0 != kernel_sysctlbyname("hw.memsize", &dtrace_buffer_memory_maxsize,
      ...
      printf("dtrace_init: failed to retrieve the hw.memsize, defaulted to "
             "%lld bytes\n",
    ...
    if (NULL == devfs_make_node_clone(makedev(gMajDevNo, 0), DEVFS_CHAR,
    ...
    /*
     * The cpu_core structure consists of per-CPU state available in any
     * context. On some architectures, this may mean that the page(s) containing
     * the NCPU-sized array of cpu_core structures must be locked in the TLB --
     * it is up to the platform to assure that this is performed properly.  Note
     * that the structure is sized to avoid false sharing.
     */
    ...
    for (i = 0; i < ncpu; ++i) {
      lck_mtx_init(&cpu_core[i].cpuc_pid_lock, &dtrace_lck_grp,
                   &dtrace_lck_attr);
    }

    cpu_list =
        (dtrace_cpu_t *)kmem_zalloc(ncpu * sizeof(dtrace_cpu_t), KM_SLEEP);
    for (i = 0; i < ncpu; ++i) {
    ...
    for (i = 0; i < ncpu; ++i)
      ...
      dtrace_cpu_setup_initial(
          (processorid_t)i); /* In lieu of register_cpu_setup_func() callback */
    lck_mtx_unlock(&cpu_lock);

    (void)dtrace_abs_to_nano(
        0LL); /* Force once only call to clock_timebase_info (which can take a
                 lock) */
...
void dtrace_postinit(void) {
   ...
   * to go.
   */
  dtrace_attach((dev_info_t *)(uintptr_t)makedev(
      gMajDevNo, 0)); /* Punning a dev_t to a dev_info_t* */
  ...
  if (dtrace_module_loaded(&fake_kernel_kmod,
                           (keepsyms) ? 0 : KMOD_DTRACE_NO_KERNEL_SYMS) != 0) {
 ...
 * from the system.
 */
void register_cpu_setup_func(cpu_setup_func_t *ignore1, void *ignore2) {
#pragma unused(ignore1, ignore2)
}

void unregister_cpu_setup_func(cpu_setup_func_t *ignore1, void *ignore2) {
#pragma unused(ignore1, ignore2)
}
