...
#define SYSTRACE_NARGS                                                         \
  (int)(sizeof(((uthread_t)NULL)->uu_arg) /                                    \
        sizeof(((uthread_t)NULL)->uu_arg[0]))
#define MACHTRACE_NARGS                                                        \
  (int)(sizeof(struct mach_call_args) / sizeof(syscall_arg_t))
...
static LCK_MTX_DECLARE_ATTR(dtrace_systrace_lock, &dtrace_lck_grp,
                            &dtrace_lck_attr); /* probe state lock */

systrace_sysent_t *systrace_sysent = NULL;
void (*systrace_probe)(dtrace_id_t, uint64_t, uint64_t, uint64_t, uint64_t,
                       uint64_t);
...
void systrace_stub(dtrace_id_t id, uint64_t arg0, uint64_t arg1, uint64_t arg2,
                   uint64_t arg3, uint64_t arg4) {
#pragma unused(id, arg0, arg1, arg2, arg3, arg4)
}

int32_t dtrace_systrace_syscall(struct proc *pp, void *uap, int *rv) {
...
#if defined(__x86_64__)
  {
    ...
    if (is_saved_state64(tagged_regs)) {
      ...
      if (code == 0) {
        vm_offset_t params =
            (vm_offset_t)(saved_state32(tagged_regs)->uesp + sizeof(int));
        code = fuword(params);
      }
    }
  }
#elif defined(__arm64__)
  {
    ...
    arm_saved_state_t *arm_regs =
        (arm_saved_state_t *)find_user_regs(current_thread());
...
#error Unknown Architecture
  ...
  sy = (code >= nsysent) ? &systrace_sysent[SYS_invalid]
                         : &systrace_sysent[code];
  ...
  if ((id = sy->stsy_return) != DTRACE_IDNONE) {
    ...
    if (uthread) {
      uthread->t_dtrace_errno = rval; /* Establish t_dtrace_errno now in case
                                         this enabling refers to it. */
    }
    /*
     * "Decode" rv for use in the call to dtrace_probe()
     */
    if (rval == ERESTART) {
      munged_rv0 = -1LL; /* System call will be reissued in user mode. Make
                            DTrace report a -1 return. */
      munged_rv1 = -1LL;
    ...
    /*
     * <http://mail.opensolaris.org/pipermail/dtrace-discuss/2007-January/003276.html>
     * says:
     *
     * "This is a bit of an historical artifact. At first, the syscall provider
     * just had its return value in arg0, and the fbt and pid providers had
     * their return values in arg1 (so that we could use arg0 for the offset of
     * the return site).
     *
     ...
     * to write by replicating the syscall return values in arg1 to match fbt
     * and pid. We debated briefly about removing the return value from arg0,
     * but decided that it would be less confusing to have the same data in two
     * places than to have some non-helpful, non-intuitive value in arg0.
     *
     * This change was made 4/23/2003 according to the DTrace project's putback
     * log."
     */
    (*systrace_probe)(id, munged_rv0, munged_rv0, munged_rv1, (uint64_t)rval,
                      0);
  }
...
void dtrace_systrace_syscall_return(unsigned short code, int rval, int *rv) {
  systrace_sysent_t *sy;
  dtrace_id_t id;
...
static void systrace_init(const struct sysent *actual,
                          systrace_sysent_t **interposed) {
  ...
  if (ssysent == NULL) {
    *interposed = ssysent =
        kmem_zalloc(sizeof(systrace_sysent_t) * NSYSCALL, KM_SLEEP);
  }
...
/*ARGSUSED*/
static void systrace_provide(void *arg, const dtrace_probedesc_t *desc) {
#pragma unused(arg) /* __APPLE__ */
  ...
  for (i = 0; i < NSYSCALL; i++) {
    ...
    if (dtrace_probe_lookup(systrace_id, NULL, syscallnames[i], "entry") != 0) {
      continue;
    }

    (void)dtrace_probe_create(systrace_id, NULL, syscallnames[i], "entry",
                              SYSTRACE_ARTIFICIAL_FRAMES,
                              (void *)((uintptr_t)SYSTRACE_ENTRY(i)));
    (void)dtrace_probe_create(systrace_id, NULL, syscallnames[i], "return",
                              SYSTRACE_ARTIFICIAL_FRAMES,
...
/*ARGSUSED*/
static void systrace_destroy(void *arg, dtrace_id_t id, void *parg) {
...
/*ARGSUSED*/
static int systrace_enable(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg) /* __APPLE__ */
...
#pragma unused(arg, id) /* __APPLE__ */
  ...
  if (disable) {
    /*
     * Usage of volatile protects the if statement below from being optimized
     * away.
     *
     * Compilers are clever and know that const array values can't change in
     * time and the if below is always false. That is because it can't see that
     * DTrace injects dtrace_systrace_syscall dynamically and violates constness
     * of the array.
     */
...
static int systrace_attach(dev_info_t *devi) {
  systrace_probe = (void *)&dtrace_probe;
  membar_enter();

  if (ddi_create_minor_node(devi, "systrace", S_IFCHR, 0, DDI_PSEUDO, 0) ==
          DDI_FAILURE ||
...
static int systrace_detach(dev_info_t *devi, ddi_detach_cmd_t cmd) {
  switch (cmd) {
  case DDI_DETACH:
    break;
  case DDI_SUSPEND:
...
/* APPLE NOTE: From #include <kern/syscall_sw.h> which may be changed for 64
 * bit! */
...
typedef struct {
  unsigned char
      mach_trap_arg_count; /* Number of trap arguments (Arch independant) */
  unsigned char
      mach_trap_u32_words; /* number of 32-bit words to copyin for U32 */
  unsigned char mach_trap_returns_port;
  unsigned char __mach_trap_padding;
  kern_return_t (*mach_trap_function)(void *);
#if CONFIG_REQUIRES_U32_MUNGING || (__arm__ && (__BIGGEST_ALIGNMENT__ > 4))
  mach_munge_t *mach_trap_arg_munge32; /* system call argument munger routine
                                          for 32-bit */
#endif
#if MACH_ASSERT
...
void (*machtrace_probe)(dtrace_id_t, uint64_t, uint64_t, uint64_t, uint64_t,
                        uint64_t);
...
static kern_return_t dtrace_machtrace_syscall(struct mach_call_args *args) {
...
static void machtrace_init(const mach_trap_t *actual,
                           machtrace_sysent_t **interposed) {
  machtrace_sysent_t *msysent = *interposed;
  int i;

  if (msysent == NULL) {
    *interposed = msysent =
        kmem_zalloc(sizeof(machtrace_sysent_t) * NSYSCALL, KM_SLEEP);
  }
...
/*ARGSUSED*/
static void machtrace_provide(void *arg, const dtrace_probedesc_t *desc) {
#pragma unused(arg) /* __APPLE__ */
  ...
  for (i = 0; i < NSYSCALL; i++) {
    ...
    if (dtrace_probe_lookup(machtrace_id, NULL, mach_syscall_name_table[i],
                            "entry") != 0) {
      continue;
    }
...
/*ARGSUSED*/
static void machtrace_destroy(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id) /* __APPLE__ */
  int sysnum = SYSTRACE_SYSNUM((uintptr_t)parg);
...
/*ARGSUSED*/
static int machtrace_enable(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg) /* __APPLE__ */
  ...
  if (mach_trap_table[sysnum].mach_trap_function ==
      machtrace_sysent[sysnum].stsy_underlying) {
    /* It is not possible to write to mach_trap_table[] directly because it is
     * const. */
    vm_offset_t dss = ptrauth_nop_cast(vm_offset_t, &dtrace_machtrace_syscall);
    ml_nofault_copy((vm_offset_t)&dss,
                    (vm_offset_t)&mach_trap_table[sysnum].mach_trap_function,
                    sizeof(vm_offset_t));
  }
...
/*ARGSUSED*/
static void machtrace_disable(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id) /* __APPLE__ */
  ...
  if (disable) {
     ...
     * Compilers are clever and know that const array values can't change in
     * time and the if below is always false. That is because it can't see that
     * DTrace injects dtrace_machtrace_syscall dynamically and violates
     * constness of the array.
     */
    ...
    if (machtrap->mach_trap_function == (mach_call_t)dtrace_machtrace_syscall) {
      ml_nofault_copy((vm_offset_t)&machtrace_sysent[sysnum].stsy_underlying,
                      (vm_offset_t)&machtrap->mach_trap_function,
                      sizeof(vm_offset_t));
    }
    lck_mtx_unlock(&dtrace_systrace_lock);
  }
...
static dtrace_pops_t machtrace_pops = {.dtps_provide = machtrace_provide,
                                       .dtps_provide_module = NULL,
...
static int machtrace_attach(dev_info_t *devi) {
  machtrace_probe = dtrace_probe;
  membar_enter();

  if (ddi_create_minor_node(devi, "machtrace", S_IFCHR, 0, DDI_PSEUDO, 0) ==
          DDI_FAILURE ||
...
static uint64_t systrace_getargval(void *arg, dtrace_id_t id, void *parg,
...
static void systrace_getargdesc(void *arg, dtrace_id_t id, void *parg,
...
#pragma unused(arg, id)
  ...
  if (SYSTRACE_ISENTRY((uintptr_t)parg)) {
    systrace_entry_setargdesc(sysnum, desc->dtargd_ndx, desc->dtargd_native,
                              sizeof(desc->dtargd_native));
  } else {
    systrace_return_setargdesc(sysnum, desc->dtargd_ndx, desc->dtargd_native,
                               sizeof(desc->dtargd_native));
  }
...
static uint64_t machtrace_getarg(void *arg, dtrace_id_t id, void *parg,
                                 int argno, int aframes) {
#pragma unused(arg, id, parg, aframes) /* __APPLE__ */
  uint64_t val = 0;
