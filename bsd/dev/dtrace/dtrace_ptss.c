 ...
 */
struct dtrace_ptss_page_entry *dtrace_ptss_claim_entry_locked(struct proc *p) {
  ...
  while (TRUE) {
    ...
    if (temp == NULL) {
      // Nothing on the free list. Allocate a new page, its okay if multiple
      // threads race here.
      ...
      do {
        page->entries[DTRACE_PTSS_ENTRIES_PER_PAGE - 1].next =
            p->p_dtrace_ptss_free_list;
      } while (!OSCompareAndSwapPtr(
          (void *)page->entries[DTRACE_PTSS_ENTRIES_PER_PAGE - 1].next,
          (void *)&page->entries[0],
    ...
    // Claim temp
    if (!OSCompareAndSwapPtr((void *)temp, (void *)temp->next,
                             (void *volatile *)&p->p_dtrace_ptss_free_list)) {
      continue;
    }
 ...
 */
struct dtrace_ptss_page_entry *dtrace_ptss_claim_entry(struct proc *p) {
 ...
 */
void dtrace_ptss_release_entry(struct proc *p,
                               struct dtrace_ptss_page_entry *e) {
  if (p && p->p_dtrace_ptss_pages && e) {
    do {
      e->next = p->p_dtrace_ptss_free_list;
    } while (
        !OSCompareAndSwapPtr((void *)e->next, (void *)e,
                             (void *volatile *)&p->p_dtrace_ptss_free_list));
  }
}
...
struct dtrace_ptss_page *dtrace_ptss_allocate_page(struct proc *p) {
  ...
  struct dtrace_ptss_page *ptss_page =
      kalloc_type(struct dtrace_ptss_page, Z_ZERO | Z_WAITOK);
  if (ptss_page == NULL) {
    return NULL;
  }
  ...
  kr = mach_vm_map_kernel(map, &addr, size, 0, VM_MAP_KERNEL_FLAGS_ANYWHERE(),
                          IPC_PORT_NULL, 0, FALSE, cur_protection,
                          max_protection, VM_INHERIT_DEFAULT);
  if (kr != KERN_SUCCESS) {
    goto err;
  }
   ...
   * virtual address
   */
  kr = mach_vm_remap(map, &write_addr, size, 0, VM_FLAGS_ANYWHERE, map, addr,
  ...
  kr = mach_vm_protect(map, (mach_vm_offset_t)write_addr, (mach_vm_size_t)size,
  ...
  for (i = 0; i < DTRACE_PTSS_ENTRIES_PER_PAGE; i++) {
    ptss_page->entries[i].addr =
        addr + (i * DTRACE_PTSS_SCRATCH_SPACE_PER_THREAD);
    ptss_page->entries[i].write_addr =
        write_addr + (i * DTRACE_PTSS_SCRATCH_SPACE_PER_THREAD);
...
void dtrace_ptss_free_page(struct proc *p, struct dtrace_ptss_page *ptss_page) {
  ...
  mach_vm_size_t size = PAGE_SIZE; // We need some way to assert that this
                                   // matches vm_map_round_page() !!!
