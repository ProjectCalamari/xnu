...
/*ARGSUSED*/
static void fbt_destroy(void *arg, dtrace_id_t id, void *parg) {
  ...
  for (; fbt != NULL; fbt = fbt->fbtp_next) {
    ...
    if (fbt->fbtp_currentval != fbt->fbtp_patchval) {
      ...
      (void)ml_nofault_copy((vm_offset_t)&fbt->fbtp_patchval,
                            (vm_offset_t)fbt->fbtp_patchpoint,
       ...
       */
      flush_dcache((vm_offset_t)fbt->fbtp_patchpoint,
                   (vm_size_t)sizeof(fbt->fbtp_patchval), 0);
      invalidate_icache((vm_offset_t)fbt->fbtp_patchpoint,
                        (vm_size_t)sizeof(fbt->fbtp_patchval), 0);
...
/*ARGSUSED*/
static void fbt_disable(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id)
  fbt_probe_t *fbt = parg;
  struct modctl *ctl = NULL;
  ...
  for (; fbt != NULL; fbt = fbt->fbtp_next) {
    ...
    if (fbt->fbtp_currentval != fbt->fbtp_savedval) {
      (void)ml_nofault_copy((vm_offset_t)&fbt->fbtp_savedval,
                            (vm_offset_t)fbt->fbtp_patchpoint,
...
/*ARGSUSED*/
static void fbt_suspend(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id)
  fbt_probe_t *fbt = parg;
  struct modctl *ctl = NULL;
  ...
  for (; fbt != NULL; fbt = fbt->fbtp_next) {
     ...
     */
    flush_dcache((vm_offset_t)fbt->fbtp_patchpoint,
                 (vm_size_t)sizeof(fbt->fbtp_savedval), 0);
    invalidate_icache((vm_offset_t)fbt->fbtp_patchpoint,
                      (vm_size_t)sizeof(fbt->fbtp_savedval), 0);
...
/*ARGSUSED*/
static void fbt_resume(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id)
  fbt_probe_t *fbt = parg;
  struct modctl *ctl = NULL;
  ...
  for (; fbt != NULL; fbt = fbt->fbtp_next) {
    ...
    /*
     * Make the patched instruction visible via a data + instruction cache
     * flush.
     */
    flush_dcache((vm_offset_t)fbt->fbtp_patchpoint,
                 (vm_size_t)sizeof(fbt->fbtp_patchval), 0);
...
static void fbt_provide_module_user_syms(struct modctl *ctl) {
  unsigned int i;
  ...
  if (module_symbols) {
    for (i = 0; i < module_symbols->dtmodsyms_count; i++) {
      ...
      fbt_provide_probe(ctl, modname, name,
                        (machine_inst_t *)(uintptr_t)symbol->dtsym_addr,
                        (machine_inst_t *)(uintptr_t)(symbol->dtsym_addr +
                                                      symbol->dtsym_size));
    }
  }
}
static void fbt_provide_kernel_section(struct modctl *ctl,
                                       kernel_section_t *sect,
                                       kernel_nlist_t *sym, uint32_t nsyms,
                                       const char *strings) {
  ...
  for (i = 0; i < nsyms; i++) {
    ...
    fbt_provide_probe(ctl, ctl->mod_modname, name,
                      (machine_inst_t *)sym[i].n_value,
                      (machine_inst_t *)limit);
  }
}

static int fbt_sym_cmp(const void *ap, const void *bp) {
  return (int)(((const kernel_nlist_t *)ap)->n_value -
               ((const kernel_nlist_t *)bp)->n_value);
}

static void fbt_provide_module_kernel_syms(struct modctl *ctl) {
  ...
  syms =
      (kernel_nlist_t *)(linkedit->vmaddr + symtab->symoff - linkedit->fileoff);
  strings =
      (const char *)(linkedit->vmaddr + symtab->stroff - linkedit->fileoff);

  /*
   * Make a copy of the symbol table and sort it to not cross into the next
   * function when disassembling the function
   */
  ...
  for (seg = firstsegfromheader(mh); seg != NULL;
    ...
    if (strcmp(seg->segname, "__KLD") == 0 ||
        strcmp(seg->segname, "__KLDDATA") == 0) {
      continue;
    }

    for (sect = firstsect(seg); sect != NULL; sect = nextsect(seg, sect)) {
      fbt_provide_kernel_section(ctl, sect, sorted_syms, symtab->nsyms,
                                 strings);
    }
  }
...
void fbt_provide_module(void *arg, struct modctl *ctl) {
#pragma unused(arg)
  ASSERT(ctl != NULL);
...
static dtrace_pops_t fbt_pops = {
    ...
    .dtps_getargdesc =
        NULL, /* APPLE NOTE: fbt_getargdesc implemented in userspace */
    .dtps_getargval = NULL,
    .dtps_usermode = NULL,
    .dtps_destroy = fbt_destroy};

static void fbt_cleanup(dev_info_t *devi) {
...
static int fbt_attach(dev_info_t *devi) {
  ...
  if (ddi_create_minor_node(devi, "fbt", S_IFCHR, 0, DDI_PSEUDO, 0) ==
          DDI_FAILURE ||
      dtrace_register("fbt", &fbt_attr, DTRACE_PRIV_KERNEL, NULL, &fbt_pops,
                      NULL, &fbt_id) != 0) {
    fbt_cleanup(devi);
    return DDI_FAILURE;
  }
...
static int _fbt_open(dev_t dev, int flags, int devtype, struct proc *p) {
