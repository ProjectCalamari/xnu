...
#if defined(__arm64__)
typedef kern_return_t (*perfCallback)(int, struct savearea_t *, __unused int,
                                      __unused int);
extern perfCallback tempDTraceTrapHook;
extern kern_return_t fbt_perfCallback(int, struct savearea_t *, __unused int,
                                      __unused int);
#define SDT_PATCHVAL 0xe7eeee7e
#define SDT_AFRAMES 7
#elif defined(__x86_64__)
typedef kern_return_t (*perfCallback)(int, struct savearea_t *, uintptr_t *,
                                      int);
extern perfCallback tempDTraceTrapHook;
...
#define SDT_UNKNOWN_FUNCNAME                                                   \
  "." /* function symbol name when not found in symbol table */
...
/*ARGSUSED*/
static void __sdt_provide_module(void *arg, struct modctl *ctl) {
#pragma unused(arg)
  ...
  /* Nothing to do. Module is either invalid or we haven't found any SDT probe
   * descriptions. */
  ...
  for (sdpd = ctl->mod_sdtdesc; sdpd != NULL; sdpd = sdpd->sdpd_next) {
    ...
    /* Validate probe's provider name.  Do not provide probes for unknown
     * providers. */
    for (prov = sdt_providers; prov->sdtp_name != NULL; prov++) {
    ...
    if (sdpd->sdpd_func == NULL) {
      /*
       * Ignore probes for which we don't have any symbol.  That's likely some
       * problem with
     ...
     */
    if ((id = dtrace_probe_lookup(prov->sdtp_id, modname, sdpd->sdpd_func,
                                  sdp->sdp_name)) != DTRACE_IDNONE) {
      ...
      sdp->sdp_id = dtrace_probe_create(prov->sdtp_id, modname, sdpd->sdpd_func,
                                        sdp->sdp_name, SDT_AFRAMES, sdp);
    ...
    sdp->sdp_hashnext = sdt_probetab[SDT_ADDR2NDX(sdpd->sdpd_offset)];
...
/*ARGSUSED*/
static void sdt_destroy(void *arg, dtrace_id_t id, void *parg) {
...
/*ARGSUSED*/
static int sdt_enable(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id)
  ...
  dtrace_casptr(&tempDTraceTrapHook, NULL,
  ...
  while (sdp != NULL) {
    (void)ml_nofault_copy((vm_offset_t)&sdp->sdp_patchval,
                          (vm_offset_t)sdp->sdp_patchpoint,
     ...
     */
    flush_dcache((vm_offset_t)sdp->sdp_patchpoint,
                 (vm_size_t)sizeof(sdp->sdp_patchval), 0);
    invalidate_icache((vm_offset_t)sdp->sdp_patchpoint,
                      (vm_size_t)sizeof(sdp->sdp_patchval), 0);
...
/*ARGSUSED*/
static void sdt_disable(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id)
  sdt_probe_t *sdp = parg;
  struct modctl *ctl = sdp->sdp_ctl;
  ...
  while (sdp != NULL) {
    (void)ml_nofault_copy((vm_offset_t)&sdp->sdp_savedval,
                          (vm_offset_t)sdp->sdp_patchpoint,
     ...
     */
    flush_dcache((vm_offset_t)sdp->sdp_patchpoint,
                 (vm_size_t)sizeof(sdp->sdp_savedval), 0);
    invalidate_icache((vm_offset_t)sdp->sdp_patchpoint,
                      (vm_size_t)sizeof(sdp->sdp_savedval), 0);
    sdp = sdp->sdp_next;
  }
...
/*ARGSUSED*/
static int sdt_attach(dev_info_t *devi) {
  sdt_provider_t *prov;

  if (ddi_create_minor_node(devi, "sdt", S_IFCHR, 0, DDI_PSEUDO, 0) ==
      DDI_FAILURE) {
  ...
  for (prov = sdt_providers; prov->sdtp_name != NULL; prov++) {
    if (dtrace_register(prov->sdtp_name, prov->sdtp_attr, DTRACE_PRIV_KERNEL,
                        NULL, &sdt_pops, prov, &prov->sdtp_id) != 0) {
      cmn_err(CE_WARN, "failed to register sdt provider %s", prov->sdtp_name);
    }
  }
...
/*ARGSUSED*/
static int sdt_detach(dev_info_t *dip, ddi_detach_cmd_t cmd) {
  sdt_provider_t *prov;
 ...
 */
static char *sdt_strdup_name(const char *name) {
 ...
 */
static kernel_mach_header_t *sdt_get_module_mh(struct modctl *ctl) {
 ...
 */
static uint32_t sdt_find_symbol_table(struct modctl *ctl, kernel_nlist_t **sym,
                                      char **strings) {
  ...
  *sym =
      (kernel_nlist_t *)(orig_le->vmaddr + orig_st->symoff - orig_le->fileoff);
...
void sdt_early_init(void) {
  ...
  /*
   * Detects end of kernel's text in static kernel cache. It is the last text
   * address before the first kext text section start.
   */
  if (kc_format == KCFormatStatic) {
    if ((sec_ks = getsectbynamefromheader(mh, "__PRELINK_INFO",
                                          "__kmod_start")) == NULL) {
 ...
 */
static int sdt_find_module_text_range(struct modctl *ctl, vm_address_t *start,
                                      vm_address_t *end) {
  kc_format_t kc_format;
...
void sdt_load_machsect(struct modctl *ctl) {
  ...
  /* Do not load SDTs when asked to use kernel symbols but symbol table is not
   * available. */
  if (MOD_HAS_KERNEL_SYMBOLS(ctl) &&
      (nsyms = sdt_find_symbol_table(ctl, &sym, &strings)) == 0) {
    printf("SDT: No kernel symbols for %s\n", ctl->mod_modname);
    return;
  }

  /* Locate DTrace SDT section in the object. */
  if ((sec_sdt = getsectbynamefromheader(mh, "__DATA_CONST", "__sdt")) ==
      NULL) {
    return;
  }
  ...
  for (size_t k = 0; k < sec_sdt->size / sizeof(dtrace_sdt_def_t);
       k++, sdtdef++) {
    unsigned long best = 0;

    /*
     * Static KEXTs share __sdt section with kernel after linking. It is
     * required to filter out description and pick only those that belong to
     * requested module or kernel itself.
     */
    ...
    /* Unescape probe name and keep a note of the size of original memory
     * allocation. */
    sdpd->sdpd_name = sdt_strdup_name(sdtdef->dsd_name);
    sdpd->sdpd_namelen = strlen(sdtdef->dsd_name) + 1;

    /* Used only for provider structure lookup so there is no need to make
     * dynamic copy. */
 ...
 *
 * A module contains only partially filled in SDT probe descriptions because
 * symbols were not available at the time when __sdt section was loaded. Fixup
 * descriptons before providing the probes.
 */
static void sdt_provide_module_user_syms(void *arg, struct modctl *ctl) {
  sdt_probedesc_t *sdpd;
  ...
  for (sdpd = ctl->mod_sdtdesc; sdpd != NULL; sdpd = sdpd->sdpd_next) {
    ...
    for (int i = 0; i < mod_sym->dtmodsyms_count; i++) {
      ...
      /*
       * Every function symbol gets extra '_' prepended in the Mach-O symbol
       * table. Strip it away to make a probe's function name match source code.
       */
      if (*name == '_') {
        name += 1;
      }
...
/*ARGSUSED*/
void sdt_provide_module(void *arg, struct modctl *ctl) {
  ASSERT(ctl != NULL);
  ...
  /*
   * The SDT provider's module is not detachable so we don't have to re-provide
   * SDT probes if that happens.  After succesfull providing, the probe
   * descriptions are no longer required.  If module gets re-loaded it will get
   * a new set of probe descriptions from its __sdt section.
   */
