...
static const struct {
  ...
  size_t stat_offset;
    ...
    {"spin-held-", 0, offsetof(lck_grp_t, lck_grp_spincnt),
     offsetof(lck_grp_stats_t, lgss_spin_held)},
    {"spin-miss-", 0, offsetof(lck_grp_t, lck_grp_spincnt),
     offsetof(lck_grp_stats_t, lgss_spin_miss)},
    {"spin-spin-", TIME_EVENT, offsetof(lck_grp_t, lck_grp_spincnt),
     offsetof(lck_grp_stats_t, lgss_spin_spin)},
    {"ticket-held-", 0, offsetof(lck_grp_t, lck_grp_ticketcnt),
     offsetof(lck_grp_stats_t, lgss_ticket_held)},
    {"ticket-miss-", 0, offsetof(lck_grp_t, lck_grp_ticketcnt),
     offsetof(lck_grp_stats_t, lgss_ticket_miss)},
    {"ticket-spin-", TIME_EVENT, offsetof(lck_grp_t, lck_grp_ticketcnt),
     offsetof(lck_grp_stats_t, lgss_ticket_spin)},
    ...
    {"adaptive-wait-", STAT_NEEDED, offsetof(lck_grp_t, lck_grp_mtxcnt),
     offsetof(lck_grp_stats_t, lgss_mtx_wait)},
    {"adaptive-direct-wait-", STAT_NEEDED, offsetof(lck_grp_t, lck_grp_mtxcnt),
     offsetof(lck_grp_stats_t, lgss_mtx_direct_wait)},
    {NULL, false, 0, 0}};
...
static int lockprof_lock_count(lck_grp_t *grp, int kind) {
  return *(int *)((uintptr_t)(grp) + probes[kind].count_offset);
}

static void probe_create(int kind, const char *suffix, const char *grp_name,
                         uint64_t count, uint64_t mult) {
  ...
  lck_grp_foreach(^bool(lck_grp_t *grp) {
    ...
    if (!grp_name || grp_name[0] == '\0' ||
        strcmp(grp_name, grp->lck_grp_name) == 0) {
      snprintf(name, sizeof(name), "%s%llu%s", probes[kind].prefix, count,
               suffix ?: "");

      if (dtrace_probe_lookup(lockprof_id, grp->lck_grp_name, NULL, name) !=
          0) {
        return true;
      }
      ...
      probe->lockprof_id = dtrace_probe_create(
          lockprof_id, grp->lck_grp_name, NULL, name, LOCKPROF_AFRAMES, probe);
...
static void lockprof_provide(void *arg, const dtrace_probedesc_t *desc) {
#pragma unused(arg)
  ...
  if (desc == NULL) {
    ...
    for (i = 0; i < sizeof(cont_defaults) / sizeof(cont_defaults[0]); i++) {
      for (j = 0; probes[j].prefix != NULL; j++) {
        if (probes[j].flags & TIME_EVENT) {
          probe_create(j, cont_defaults[i].suffix, NULL, cont_defaults[i].time,
                       cont_defaults[i].mult);
        }
      }
    }
    return;
  ...
  if (probes[event_id].flags & TIME_EVENT) {
    for (i = 0, mult = 0; suffixes[i].name != NULL; i++) {
      if (suffix && (strncasecmp(suffixes[i].name, suffix,
                                 strlen(suffixes[i].name) + 1) == 0)) {
...
static lck_grp_stat_t *lockprof_stat(lck_grp_t *grp, int kind) {
  return (lck_grp_stat_t *)((uintptr_t)&grp->lck_grp_stats +
                            probes[kind].stat_offset);
}

static int lockprof_enable(void *arg, dtrace_id_t id, void *parg) {
...
static void lockprof_disable(void *arg, dtrace_id_t id, void *parg) {
...
static void lockprof_destroy(void *arg, dtrace_id_t id, void *parg) {
#pragma unused(arg, id)
  lockprof_probe_t *probe = (lockprof_probe_t *)parg;
...
static void lockprof_getargdesc(void *arg, dtrace_id_t id, void *parg,
...
static dtrace_pops_t lockprof_pops = {.dtps_provide = lockprof_provide,
...
static int _lockprof_open(dev_t dev, int flags, int devtype, struct proc *p) {
...
void lockprof_init(void) {
  ...
  if (dtrace_register(LP_NODE, &lockprof_attr, DTRACE_PRIV_KERNEL, NULL,
                      &lockprof_pops, NULL, &lockprof_id) != 0) {
    panic("dtrace: failed to register lockprof provider");
  }

  dev_t dev = makedev(majorno, 0);

  if (devfs_make_node(dev, DEVFS_CHAR, UID_ROOT, GID_WHEEL, 0666, LP_NODE) ==
      NULL) {
