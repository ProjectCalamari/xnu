...
static LCK_MTX_DECLARE_ATTR(fasttrap_cleanup_mtx, &fasttrap_lck_grp,
                            &fasttrap_lck_attr);
...
static fasttrap_provider_t *fasttrap_provider_lookup(proc_t *,
...
/*
 * APPLE NOTE: fasttrap_probe_t's are variable in size. Some quick profiling has
 * shown that the sweet spot for reducing memory footprint is covering the first
...
static const char
    *fasttrap_probe_t_zone_names[FASTTRAP_PROBE_T_ZONE_MAX_TRACEPOINTS] = {
        "", "dtrace.fasttrap_probe_t[1]", "dtrace.fasttrap_probe_t[2]",
        "dtrace.fasttrap_probe_t[3]"};

static int fasttrap_highbit(ulong_t i) {
  int h = 1;
 ...
 */
void fasttrap_sigtrap(proc_t *p, uthread_t t, user_addr_t pc) {
...
static unsigned int fasttrap_pid_cleanup_providers(void) {
  ...
  for (i = 0; i < fasttrap_provs.fth_nent; i++) {
    ...
    while ((fp = *fpp) != NULL) {
       ...
       * or even condense.
       */
      if (fp->ftp_ccount != 0 || fp->ftp_mcount != 0) {
        fp->ftp_marked = 0;
...
static LCK_MTX_DECLARE_ATTR(fasttrap_retired_mtx, &fasttrap_lck_grp,
                            &fasttrap_lck_attr);

#define DEFAULT_RETIRED_SIZE 256

static void fasttrap_tracepoint_cleanup(void) {
  size_t i;
  pid_t pid = 0;
  user_addr_t pc;
  proc_t *p = PROC_NULL;
...
void fasttrap_tracepoint_retire(proc_t *p, fasttrap_tracepoint_t *tp) {
  if (tp->ftt_retired)
    return;
  lck_mtx_lock(&fasttrap_retired_mtx);
  fasttrap_tracepoint_spec_t *s =
      &fasttrap_retired_spec[fasttrap_cur_retired++];
  ...
  if (fasttrap_cur_retired == fasttrap_retired_size) {
    fasttrap_tracepoint_spec_t *new_retired = kmem_zalloc(
        fasttrap_retired_size * 2 * sizeof(*fasttrap_retired_spec), KM_SLEEP);
    memcpy(new_retired, fasttrap_retired_spec,
           sizeof(*fasttrap_retired_spec) * fasttrap_retired_size);
    kmem_free(fasttrap_retired_spec,
              sizeof(*fasttrap_retired_spec) * fasttrap_retired_size);
...
static void fasttrap_pid_cleanup_compute_priority(void) {
  if (fasttrap_total > (fasttrap_max / 100 * 90) ||
      fasttrap_retired > fasttrap_max / 2) {
    thread_precedence_policy_data_t precedence = {
        12 /* BASEPRI_PREEMPT_HIGH */};
    thread_policy_set(fasttrap_cleanup_thread, THREAD_PRECEDENCE_POLICY,
 ...
 */
/*ARGSUSED*/
__attribute__((noreturn)) static void fasttrap_pid_cleanup_cb(void) {
  uint32_t work = 0;
  lck_mtx_lock(&fasttrap_cleanup_mtx);
  msleep(&fasttrap_pid_cleanup_cb, &fasttrap_cleanup_mtx, PRIBIO,
 ...
 */
static void fasttrap_pid_cleanup(uint32_t work) {
  lck_mtx_lock(&fasttrap_cleanup_mtx);
...
static int fasttrap_setdebug(proc_t *p) {
  ...
  if ((proc_getcsflags(p) & (CS_KILL | CS_HARD))) {
    ...
    for (int i = 0; i < DTRACE_NCLIENTS; i++) {
      ...
      (void)mac_proc_check_get_task(state->dts_cred.dcr_cred, &pident,
                                    TASK_FLAVOR_CONTROL);
 ...
 * parent.
 */
static void fasttrap_fork(proc_t *p, proc_t *cp) {
  ...
  for (i = 0; i < fasttrap_tpoints.fth_nent; i++) {
    ...
    lck_mtx_lock(&bucket->ftb_mtx);
    for (tp = bucket->ftb_data; tp != NULL; tp = tp->ftt_next) {
      if (tp->ftt_pid == ppid && tp->ftt_proc->ftpc_acount != 0) {
 ...
 */
static void fasttrap_exec_exit(proc_t *p) {
...
static int fasttrap_tracepoint_enable(proc_t *p, fasttrap_probe_t *probe,
                                      uint_t index) {
...
static void fasttrap_tracepoint_disable(proc_t *p, fasttrap_probe_t *probe,
                                        uint_t index) {
  fasttrap_bucket_t *bucket;
...
#pragma unused(arg, id)
  ...
  case EACCES:
    ...
    sprunlock(p);
    cmn_err(CE_WARN,
            "Failed to install fasttrap probe for pid %d: "
   ...
   * Solaris uses its ulwp_t struct for scratch space to support the pid
   * provider. To mimic this, we allocate on demand scratch space. If this is
   * the first time a probe has been enabled in this process, we need to
   * allocate scratch space for each already existing thread. Now is a good time
   * to do this, as the target process is suspended and the proc_lock is held.
   */
...
#pragma unused(arg, id)
  ...
  if (probe->ftp_prov->ftp_retired != 0 || desc->dtargd_ndx < 0 ||
...
fasttrap_provider_lookup(proc_t *p, fasttrap_provider_type_t provider_type,
                         const char *name, const dtrace_pattr_t *pattr) {
  ...
  for (fp = bucket->ftb_data; fp != NULL; fp = fp->ftp_next) {
    if (fp->ftp_pid == pid && fp->ftp_provider_type == provider_type &&
...
static void fasttrap_provider_free(fasttrap_provider_t *provider) {
  ...
  if (!provider->ftp_retired) {
    ...
    ASSERT(provider->ftp_proc->ftpc_acount < provider->ftp_proc->ftpc_rcount);
  }
...
static void fasttrap_provider_retire(proc_t *p, const char *name, int mprov) {
  ...
  bucket =
      &fasttrap_provs.fth_table[FASTTRAP_PROVS_INDEX(proc_getpid(p), name)];
  lck_mtx_lock(&bucket->ftb_mtx);

  for (fp = bucket->ftb_data; fp != NULL; fp = fp->ftp_next) {
    if (fp->ftp_pid == proc_getpid(p) &&
        strncmp(fp->ftp_name, name, sizeof(fp->ftp_name)) == 0 &&
        !fp->ftp_retired)
      break;
  }
...
static int fasttrap_add_probe(fasttrap_probe_spec_t *pdata) {
  proc_t *p;
  ...
  if (name == NULL) {
    for (i = 0; i < pdata->ftps_noffs; i++) {
      ...
      if (dtrace_probe_lookup(provider->ftp_provid, pdata->ftps_mod,
      ...
      pp->ftp_id = dtrace_probe_create(provider->ftp_provid, pdata->ftps_mod,
                                       pdata->ftps_func, name_str,
    ...
    if (pdata->ftps_noffs < FASTTRAP_PROBE_T_ZONE_MAX_TRACEPOINTS) {
      ...
      pp = kmem_zalloc(offsetof(fasttrap_probe_t, ftp_tps[pdata->ftps_noffs]),
                       KM_SLEEP);
    }
    ...
    pp->ftp_id = dtrace_probe_create(provider->ftp_provid, pdata->ftps_mod,
                                     pdata->ftps_func, name, aframes, pp);
  }
...
/*ARGSUSED*/
static void *fasttrap_meta_provide(void *arg, dtrace_helper_provdesc_t *dhpv,
                                   proc_t *p) {
#pragma unused(arg)
  fasttrap_provider_t *provider;
   ...
   */
  if (strncmp(dhpv->dthpv_provname, FASTTRAP_PID_NAME,
              sizeof(FASTTRAP_PID_NAME)) == 0) {
    cmn_err(CE_WARN,
            "failed to instantiate provider %s: "
   ...
   */
  if (strncmp(dhpv->dthpv_provname, FASTTRAP_OBJC_NAME,
              sizeof(FASTTRAP_OBJC_NAME)) == 0) {
    cmn_err(CE_WARN,
            "failed to instantiate provider %s: "
            "%s is an invalid name",
            dhpv->dthpv_provname, FASTTRAP_OBJC_NAME);
    return (NULL);
  }
  if (strncmp(dhpv->dthpv_provname, FASTTRAP_ONESHOT_NAME,
              sizeof(FASTTRAP_ONESHOT_NAME)) == 0) {
    cmn_err(CE_WARN,
            "failed to instantiate provider %s: "
            "%s is an invalid name",
            dhpv->dthpv_provname, FASTTRAP_ONESHOT_NAME);
    return (NULL);
  }
  ...
  if ((provider = fasttrap_provider_lookup(p, DTFTP_PROVIDER_USDT,
                                           dhpv->dthpv_provname,
                                           &dhpv->dthpv_pattr)) == NULL) {
    cmn_err(CE_WARN,
            "failed to instantiate provider %s for "
            "process %u",
            dhpv->dthpv_provname, (uint_t)proc_getpid(p));
    return (NULL);
  }
...
/*ARGSUSED*/
static void fasttrap_meta_create_probe(void *arg, void *parg,
...
#pragma unused(arg)
  ...
  for (i = 0; i < dhpb->dthpb_noffs; i++) {
    ...
    /*
     * APPLE NOTE: We have linker support when creating DOF to handle all
     * relocations for us. Unfortunately, a side effect of this is that the
     * relocations do not point at exactly the location we want. We need to fix
     * up the addresses here. The fixups vary by arch and type.
     */
...
#if defined(__x86_64__)
    /*
     * Both 32 & 64 bit want to go forward two bytes, to point at a single byte
     * nop.
     */
...
/*ARGSUSED*/
static void fasttrap_meta_remove(void *arg, dtrace_helper_provdesc_t *dhpv,
                                 proc_t *p) {
#pragma unused(arg)
  /*
...
static char *fasttrap_meta_provider_name(void *arg) {
 ...
 */
static int fasttrap_validatestr(char const *str, size_t maxlen) {
  size_t len;
 ...
 */
static int fasttrap_check_cred_priv(cred_t *cr, proc_t *p) {
  int err = 0;
  ...
  struct proc_ident cur_ident =
      proc_ident_with_policy(current_proc(), IDENT_VALIDATION_PROC_EXACT);
...
/*ARGSUSED*/
static int fasttrap_ioctl(dev_t dev, u_long cmd, user_addr_t arg, int md,
                          cred_t *cr, int *rv) {
#pragma unused(dev, md, rv)
  ...
  if (cmd == FASTTRAPIOC_MAKEPROBE) {
     ...
     */
    if (noffs > ((1024 * 1024) - sizeof(fasttrap_probe_spec_t)) /
                    sizeof(probe->ftps_offs[0]))
      return (ENOMEM);
    ...
    if (copyin(arg, probe, size) != 0 || probe->ftps_noffs != noffs) {
      kmem_free(probe, size);
      return (EFAULT);
    }
  ...
  err:
    ...
    while (tp != NULL) {
      if (instr.ftiq_pid == tp->ftt_pid && instr.ftiq_pc == tp->ftt_pc &&
          tp->ftt_proc->ftpc_acount != 0)
        break;
    ...
    bcopy(&tp->ftt_instr, &instr.ftiq_instr, sizeof(instr.ftiq_instr));
    lck_mtx_unlock(&fasttrap_tpoints.fth_table[index].ftb_mtx);
...
static void fasttrap_attach(void) {
  ulong_t nent;
  unsigned int i;
  ...
  fasttrap_tpoints.fth_table = kmem_zalloc(
      fasttrap_tpoints.fth_nent * sizeof(fasttrap_bucket_t), KM_SLEEP);
  ...
  fasttrap_provs.fth_table = kmem_zalloc(
      fasttrap_provs.fth_nent * sizeof(fasttrap_bucket_t), KM_SLEEP);
  ...
  fasttrap_procs.fth_table = kmem_zalloc(
      fasttrap_procs.fth_nent * sizeof(fasttrap_bucket_t), KM_SLEEP);
...
static int _fasttrap_open(dev_t dev, int flags, int devtype, struct proc *p) {
#pragma unused(dev, flags, devtype, p)
  return 0;
}

static int _fasttrap_ioctl(dev_t dev, u_long cmd, caddr_t data, int fflag,
                           struct proc *p) {
  int err, rv = 0;
  user_addr_t uaddrp;
  ...
  /* XXX Darwin's BSD ioctls only return -1 or zero. Overload errno to mimic
  ...
  if (err != 0) {
    ...
    return (
        ((rv & 0xfffff)
         << 12)); /* ioctl returns -1 and errno set to a return value >= 4096 */
  } else
    return 0;
}
...
void fasttrap_init(void) {
   ...
   *
   * The reason is to delay allocating the (rather large) resources as late as
   * possible.
   */
  if (!fasttrap_inited) {
    ...
    if (NULL == devfs_make_node(device, DEVFS_CHAR, UID_ROOT, GID_WHEEL, 0666,
                                "fasttrap")) {
      return;
    }
    ...
    kern_return_t res = kernel_thread_start_priority(
        (thread_continue_t)fasttrap_pid_cleanup_cb, NULL,
        46 /* BASEPRI_BACKGROUND */, &fasttrap_cleanup_thread);
    if (res != KERN_SUCCESS) {
      panic("Could not create fasttrap_cleanup_thread");
    }
    thread_set_thread_name(fasttrap_cleanup_thread,
                           "dtrace_fasttrap_cleanup_thread");

    fasttrap_retired_size = DEFAULT_RETIRED_SIZE;
    fasttrap_retired_spec = kmem_zalloc(
        fasttrap_retired_size * sizeof(*fasttrap_retired_spec), KM_SLEEP);
