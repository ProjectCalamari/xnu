...
extern kern_return_t thread_getstatus_to_user(thread_t act, int flavor,
                                              thread_state_t tstate,
                                              mach_msg_type_number_t *count,
                                              thread_set_status_flags_t);
extern kern_return_t machine_thread_state_convert_to_user(
    thread_t act, int flavor, thread_state_t tstate,
    mach_msg_type_number_t *count, thread_set_status_flags_t);
...
#define UC_FLAVOR_SIZE32                                                       \
  ((ARM_THREAD_STATE_COUNT + ARM_EXCEPTION_STATE_COUNT +                       \
    ARM_VFP_STATE_COUNT) *                                                     \
   sizeof(int))
#define UC_FLAVOR_SIZE64                                                       \
  ((ARM_THREAD_STATE64_COUNT + ARM_EXCEPTION_STATE64_COUNT +                   \
...
static int sendsig_get_state32(thread_t th_act, arm_thread_state_t *ts,
                               mcontext32_t *mcp) {
  void *tstate;
  ...
  if (thread_getstatus(th_act, ARM_THREAD_STATE, (thread_state_t)tstate,
  ...
  tstate = (void *)&mcp->ss;
  state_count = ARM_THREAD_STATE_COUNT;
  if (machine_thread_state_convert_to_user(th_act, ARM_THREAD_STATE,
                                           (thread_state_t)tstate, &state_count,
                                           TSSF_FLAGS_NONE) != KERN_SUCCESS) {
    return EINVAL;
  }

  tstate = (void *)&mcp->es;
  state_count = ARM_EXCEPTION_STATE_COUNT;
  if (thread_getstatus(th_act, ARM_EXCEPTION_STATE, (thread_state_t)tstate,
                       &state_count) != KERN_SUCCESS) {
    return EINVAL;
  }
  ...
  if (thread_getstatus_to_user(th_act, ARM_VFP_STATE, (thread_state_t)tstate,
...
static int sendsig_get_state64(thread_t th_act, arm_thread_state64_t *ts,
                               mcontext64_t *mcp) {
  void *tstate;
  mach_msg_type_number_t state_count;
  ...
  if (thread_getstatus(th_act, ARM_THREAD_STATE64, (thread_state_t)tstate,
                       &state_count) != KERN_SUCCESS) {
    return EINVAL;
  }
  ...
  if (pac_sigreturn_token ||
      task_needs_user_signed_thread_state(current_task())) {
    flags |= TSSF_THREAD_USER_DIV;
  }
  if (machine_thread_state_convert_to_user(th_act, ARM_THREAD_STATE64,
                                           (thread_state_t)tstate, &state_count,
                                           flags) != KERN_SUCCESS) {
    return EINVAL;
  }
  ...
  if (thread_getstatus(th_act, ARM_EXCEPTION_STATE64, (thread_state_t)tstate,
                       &state_count) != KERN_SUCCESS) {
    return EINVAL;
  }
  ...
  if (thread_getstatus_to_user(th_act, ARM_NEON_STATE64, (thread_state_t)tstate,
                               &state_count, TSSF_FLAGS_NONE) != KERN_SUCCESS) {
    return EINVAL;
  }
...
static void sendsig_fill_uctx64(user_ucontext64_t *uctx, int oonstack, int mask,
                                user64_addr_t sp, user64_size_t stack_size,
...
static kern_return_t sendsig_set_thread_state64(
    arm_thread_state64_t *regs, user64_addr_t catcher, int infostyle, int sig,
    user64_addr_t p_sinfo, user64_addr_t p_uctx, user64_addr_t token,
    user64_addr_t trampact, user64_addr_t sp, thread_t th_act) {
  assert(proc_is64bit_data(current_proc()));
  ...
  return thread_setstatus(th_act, ARM_THREAD_STATE64, (void *)regs,
                          ARM_THREAD_STATE64_COUNT);
}
#endif /* defined(__arm64__) */

static void sendsig_fill_uctx32(user_ucontext32_t *uctx, int oonstack, int mask,
                                user_addr_t sp, user_size_t stack_size,
                                user_addr_t p_mctx) {
  bzero(uctx, sizeof(*uctx));
  uctx->uc_onstack = oonstack;
  uctx->uc_sigmask = mask;
...
sendsig_set_thread_state32(arm_thread_state_t *regs, user32_addr_t catcher,
                           int infostyle, int sig, user32_addr_t p_sinfo,
                           user32_addr_t trampact, user32_addr_t sp,
                           thread_t th_act) {
  assert(!proc_is64bit_data(current_proc()));
...
#error Unknown architecture.
  ...
  return thread_setstatus(th_act, ARM_THREAD_STATE, (void *)regs,
                          ARM_THREAD_STATE_COUNT);
}

#if CONFIG_DTRACE
static void sendsig_do_dtrace(uthread_t ut, user_siginfo_t *sinfo, int sig,
                              user_addr_t catcher) {
 ...
 *
 */
void sendsig(struct proc *p, user_addr_t catcher, int sig, int mask,
             __unused uint32_t code, sigset_t siginfo) {
  union {
    struct ts32 {
  ...
  if (ut->uu_pending_sigreturn == 0) {
    ...
    do {
      read_random(&ut->uu_sigreturn_diversifier,
...
#ifdef __arm64__
    int ret = 0;
    if ((ret = sendsig_get_state64(th_act, &ts.ts64.ss,
                                   &user_frame.uf64.mctx)) != 0) {
#if DEVELOPMENT || DEBUG
      printf(
          "process [%s][%d] sendsig_get_state64 failed with ret %d, expected 0",
          p->p_comm, proc_getpid(p), ret);
#endif
      goto bad2;
    }
#else
#error Unsupported architecture
    ...
    int ret = 0;
    if ((ret = sendsig_get_state32(th_act, &ts.ts32.ss,
                                   &user_frame.uf32.mctx)) != 0) {
#if DEVELOPMENT || DEBUG
      printf(
          "process [%s][%d] sendsig_get_state32 failed with ret %d, expected 0",
          p->p_comm, proc_getpid(p), ret);
#endif
      goto bad2;
    }
...
#error Unsupported architecture
  ...
  if (proc_is64bit_data(p)) {
#if defined(__arm64__)
    sendsig_fill_uctx64(&user_frame.uf64.uctx, oonstack, mask, sp,
...
#error Unsupported architecture
    ...
    sendsig_fill_uctx32(&user_frame.uf32.uctx, oonstack, mask, sp,
                        (user32_size_t)stack_size,
...
#error Unsupported architecture
  ...
  default: {
    ...
    if (sinfo.si_code == CLD_EXITED) {
      if (WIFEXITED(status_and_exitcode)) {
        ...
        if (WCOREDUMP(status_and_exitcode)) {
          ...
          status_and_exitcode =
              W_EXITCODE(status_and_exitcode, status_and_exitcode);
        } else {
     ...
     */
    sinfo.si_status = (WEXITSTATUS(status_and_exitcode) & 0x00FFFFFF) |
                      (((uint32_t)(p->p_xhighbits) << 24) & 0xFF000000);
    p->p_xhighbits = 0;
    break;
  }
  }
...
#if DEVELOPMENT || DEBUG
      printf("process [%s][%d] copyout of user_frame to  (sp, size) = (0x%llx, "
             "%zu) failed with ret %d, expected 0\n",
             p->p_comm, proc_getpid(p), sp, sizeof(user_frame.uf64), ret);
#endif
      goto bad;
    }

    if ((kr = sendsig_set_thread_state64(
             &ts.ts64.ss, catcher, infostyle, sig,
             (user64_addr_t) & ((struct user_sigframe64 *)sp)->sinfo,
             (user64_addr_t)p_uctx, token, trampact, sp, th_act)) !=
        KERN_SUCCESS) {
#if DEVELOPMENT || DEBUG
      printf("process [%s][%d] sendsig_set_thread_state64 failed with kr %d, "
             "expected 0",
             p->p_comm, proc_getpid(p), kr);
#endif
      goto bad;
    }
...
#error Unsupported architecture
    ...
    if (sendsig_set_thread_state32(
            &ts.ts32.ss, CAST_DOWN_EXPLICIT(user32_addr_t, catcher), infostyle,
            sig, (user32_addr_t) & ((struct user_sigframe32 *)sp)->sinfo,
            CAST_DOWN_EXPLICIT(user32_addr_t, trampact),
            CAST_DOWN_EXPLICIT(user32_addr_t, sp), th_act) != KERN_SUCCESS) {
      goto bad;
    }
  }
...
static int sigreturn_copyin_ctx32(struct user_ucontext32 *uctx,
                                  mcontext32_t *mctx, user_addr_t uctx_addr) {
  int error;
...
static int sigreturn_set_state32(thread_t th_act, mcontext32_t *mctx) {
  assert(!proc_is64bit_data(current_proc()));
...
#if defined(__arm64__)
static int sigreturn_copyin_ctx64(struct user_ucontext64 *uctx,
                                  mcontext64_t *mctx, user_addr_t uctx_addr) {
  int error;
...
static int sigreturn_set_state64(thread_t th_act, mcontext64_t *mctx,
  ...
  if (thread_setstatus_from_user(th_act, ARM_THREAD_STATE64, (void *)&mctx->ss,
                                 ARM_THREAD_STATE64_COUNT, NULL, 0,
                                 tssf_flags) != KERN_SUCCESS) {
    return EINVAL;
  }
  if (thread_setstatus_from_user(th_act, ARM_NEON_STATE64, (void *)&mctx->ns,
                                 ARM_NEON_STATE64_COUNT, NULL, 0,
                                 TSSF_FLAGS_NONE) != KERN_SUCCESS) {
    return EINVAL;
  }
...
/* ARGSUSED */
int sigreturn(struct proc *p, struct sigreturn_args *uap,
              __unused int *retval) {
  union {
...
#error Unsupported architecture
  ...
  sigreturn_validation =
      atomic_load_explicit(&ps->ps_sigreturn_validation, memory_order_relaxed);
  token_uctx = uap->uctx;
  kr = machine_thread_siguctx_pointer_convert_to_user(th_act, &token_uctx);
  assert(kr == KERN_SUCCESS);
...
#if DEVELOPMENT || DEBUG
      printf("process %s[%d] sigreturn token mismatch: received 0x%llx "
             "expected 0x%llx\n",
...
#if DEVELOPMENT || DEBUG
      printf("process %s[%d] sigreturn token mismatch: received 0x%x expected "
             "0x%x\n",
 ...
 */
int machine_exception(int exception, __unused mach_exception_code_t code,
