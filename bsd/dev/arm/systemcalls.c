...
static int arm_get_syscall_args(uthread_t, struct arm_saved_state *,
                                const struct sysent *);
static int arm_get_u32_syscall_args(uthread_t, arm_saved_state32_t *,
                                    const struct sysent *);
static void arm_prepare_u32_syscall_return(const struct sysent *,
                                           arm_saved_state_t *, uthread_t, int);
static void arm_prepare_syscall_return(const struct sysent *,
                                       struct arm_saved_state *, uthread_t,
                                       int);
...
#define code_is_kdebug_trace(code)                                             \
  (((code) == SYS_kdebug_trace) || ((code) == SYS_kdebug_trace64) ||           \
 ...
 * Outputs:	none
 */
void unix_syscall(struct arm_saved_state *state, thread_t thread_act,
                  struct proc *proc) {
...
#define unix_syscall_kprintf(x...) /* kprintf("unix_syscall: " x) */
  ...
  /*
   * sy_narg is inaccurate on ARM if a 64 bit parameter is specified. Since
   * user_addr_t is currently a 32 bit type, this is really a long word count.
   * See rdar://problem/6104668.
   */
  if (callp->sy_narg != 0) {
  ...
  unix_syscall_kprintf("code %d (pid %d - %s, tid %lld)\n", code, pid,
                       proc->p_comm, thread_tid(current_thread()));

#if CONFIG_MACF
  if (__improbable(proc_syscall_filter_mask(proc) != NULL &&
...
skip_syscall:
  ...
  unix_syscall_kprintf(
      "code %d, error %d, results %x, %x (pid %d - %s, tid %lld)\n", code,
      error, uthread->uu_rval[0], uthread->uu_rval[1], pid,
      get_bsdtask_info(current_task()) ? proc->p_comm : "unknown",
      thread_tid(current_thread()));

#ifdef CONFIG_IOCOUNT_TRACE
  if (uthread->uu_iocount) {
    printf("system call(%d) returned with uu_iocount(%d) != 0", code,
           uthread->uu_iocount);
  }
#endif
#if CONFIG_DTRACE
  ...
  kern_allocation_name_t prior __assert_only = thread_set_allocation_name(NULL);
  ...
  if (kdebug_enable && !code_is_kdebug_trace(code)) {
    KDBG_RELEASE(BSDDBG_CODE(DBG_BSD_EXCP_SC, code) | DBG_FUNC_END, error,
                 uthread->uu_rval[0], uthread->uu_rval[1], pid);
  }
...
#define unix_syscall_return_kprintf(x...) /* kprintf("unix_syscall_retur       \
  ...
  if (kdebug_enable && !code_is_kdebug_trace(code)) {
    KDBG_RELEASE(BSDDBG_CODE(DBG_BSD_EXCP_SC, code) | DBG_FUNC_END, error,
                 uthread->uu_rval[0], uthread->uu_rval[1], proc_getpid(proc));
  }
...
static void arm_prepare_u32_syscall_return(const struct sysent *callp,
                                           arm_saved_state_t *regs,
                                           uthread_t uthread, int error) {
  ...
  if (error == ERESTART) {
    ...
    if (error) {
      ...
      ss32->cpsr |= PSR_CF;
      unix_syscall_return_kprintf(
          "error: setting carry to trigger cerror call\n");
    } else { /* (not error) */
...
static void arm_trace_u32_unix_syscall(int code, arm_saved_state32_t *regs) {
...
static void arm_clear_u32_syscall_error(arm_saved_state32_t *regs) {
  regs->cpsr &= ~PSR_CF;
}

#if defined(__arm64__)
static void arm_prepare_u64_syscall_return(const struct sysent *,
                                           arm_saved_state_t *, uthread_t, int);
static int arm_get_u64_syscall_args(uthread_t, arm_saved_state64_t *,
                                    const struct sysent *);

static int arm_get_syscall_args(uthread_t uthread,
...
static int arm_get_u64_syscall_args(uthread_t uthread,
   ...
   */
  if (callp->sy_narg >
      (int)(sizeof(uthread->uu_arg) / sizeof(uthread->uu_arg[0]))) {
    return -1;
  }

  memcpy(&uthread->uu_arg[0], &regs->x[indirect_offset],
         callp->sy_narg * sizeof(uint64_t));
   ...
   * interested in is whether the task is arm64_32.  We don't appear to
   * guarantee that uu_proc is populated here, which is why this currently uses
   * the thread_t.
   */
...
#error U32 syscalls on ARM64 kernel requires munging
  ...
  if (callp->sy_arg_bytes <= (sizeof(uint32_t) * regparams)) {
    ...
    if (copyin((user_addr_t)regs->sp + 7 * sizeof(int),
               (int *)&uthread->uu_arg[0] + regparams,
...
static unsigned short arm_get_syscall_number(struct arm_saved_state *state) {
  if (is_saved_state32(state)) {
    ...
    if (saved_state64(state)->x[ARM64_SYSCALL_CODE_REG_NUM] != 0) {
      return (unsigned short)saved_state64(state)
          ->x[ARM64_SYSCALL_CODE_REG_NUM];
    } else {
...
static void arm_prepare_syscall_return(const struct sysent *callp,
                                       struct arm_saved_state *state,
                                       uthread_t uthread, int error) {
  if (is_saved_state32(state)) {
...
static void arm_prepare_u64_syscall_return(const struct sysent *callp,
                                           arm_saved_state_t *regs,
                                           uthread_t uthread, int error) {
...
static void arm_trace_u64_unix_syscall(int code, arm_saved_state64_t *regs) {
...
static void arm_trace_unix_syscall(int code, struct arm_saved_state *state) {
  if (is_saved_state32(state)) {
...
static void arm_clear_u64_syscall_error(arm_saved_state64_t *regs) {
  regs->cpsr &= ~PSR64_CF;
}

static void arm_clear_syscall_error(struct arm_saved_state *state) {
  if (is_saved_state32(state)) {
