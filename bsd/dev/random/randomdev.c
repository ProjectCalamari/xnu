...
static int random_stop(__unused struct tty *tp, __unused int rw) { return 0; }

static int random_reset(__unused int uban) { return 0; }

static int random_select(__unused dev_t dev, __unused int which,
...
void random_init(void) {
  ...
  devfs_make_node(makedev(ret, RANDOM_MINOR), DEVFS_CHAR, UID_ROOT, GID_WHEEL,
                  0666, "random");
   ...
   */
  devfs_make_node(makedev(ret, URANDOM_MINOR), DEVFS_CHAR, UID_ROOT, GID_WHEEL,
                  0666, "urandom");
}

int random_ioctl(__unused dev_t dev, u_long cmd, __unused caddr_t data,
...
int random_open(__unused dev_t dev, int flags, __unused int devtype,
...
int random_close(__unused dev_t dev, __unused int flags, __unused int mode,
                 __unused struct proc *p) {
  return 0;
}
 ...
 * prng.
 */
int random_write(dev_t dev, struct uio *uio, __unused int ioflag) {
  int retCode = 0;
  char rdBuffer[256];
...
int getentropy(__unused struct proc *p, struct getentropy_args *gap,
               __unused int *ret) {
