...
static SYSCTL_INT(_machdep, OID_AUTO, wfe_rec_max, CTLFLAG_RW, &wfe_rec_max, 0,
                  "");

static SYSCTL_INT(_machdep, OID_AUTO, wfe_allowed, CTLFLAG_RW, &wfe_allowed, 0,
                  "");

static SYSCTL_INT(_machdep, OID_AUTO, idle_timer_wfe, CTLFLAG_RW,
                  &idle_proximate_timer_wfe, 0, "");

static SYSCTL_INT(_machdep, OID_AUTO, idle_io_wfe_masked, CTLFLAG_RW,
                  &idle_proximate_io_wfe_masked, 0, "");
static SYSCTL_INT(_machdep, OID_AUTO, idle_io_wfe_unmasked, CTLFLAG_RW,
                  &idle_proximate_io_wfe_unmasked, 0, "");

static SYSCTL_INT(_machdep, OID_AUTO, wfe_rec_none, CTLFLAG_RW, &wfe_rec_none,
                  0, "");

extern uint64_t wfe_rec_override_mat;
SYSCTL_QUAD(_machdep, OID_AUTO, wfe_rec_override_mat, CTLFLAG_RW,
            &wfe_rec_override_mat, "");

extern uint64_t wfe_rec_clamp;
SYSCTL_QUAD(_machdep, OID_AUTO, wfe_rec_clamp, CTLFLAG_RW, &wfe_rec_clamp, "");

#endif

static SYSCTL_QUAD(_machdep, OID_AUTO, wake_abstime, CTLFLAG_RD, &wake_abstime,
                   "Absolute Time at the last wakeup");

static int sysctl_time_since_reset SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_machdep, OID_AUTO, time_since_reset,
            CTLFLAG_RD | CTLTYPE_QUAD | CTLFLAG_LOCKED, 0, 0,
            sysctl_time_since_reset, "I",
            "Continuous time since last SOC boot/wake started");

static int sysctl_wake_conttime SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2, oidp)
...
SYSCTL_PROC(_machdep, OID_AUTO, wake_conttime,
            CTLFLAG_RD | CTLTYPE_QUAD | CTLFLAG_LOCKED, 0, 0,
            sysctl_wake_conttime, "I", "Continuous Time at the last wakeup");

#if defined(HAS_IPI)
static int cpu_signal_deferred_timer(__unused struct sysctl_oid *oidp,
...
SYSCTL_PROC(_machdep, OID_AUTO, deferred_ipi_timeout,
            ...
            cpu_signal_deferred_timer, "I",
            "Deferred IPI timeout (nanoseconds)");
...
static int arm_host_info SYSCTL_HANDLER_ARGS {
 ...
 */
static SYSCTL_PROC(_machdep_cpu, OID_AUTO, cores_per_package,
                   CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED,
                   (void *)offsetof(host_basic_info_data_t, physical_cpu_max),
                   sizeof(integer_t), arm_host_info, "I",
 ...
 */
static SYSCTL_PROC(_machdep_cpu, OID_AUTO, core_count,
                   CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED,
                   (void *)offsetof(host_basic_info_data_t, physical_cpu),
                   sizeof(integer_t), arm_host_info, "I",
                   "Number of enabled cores per package");
 ...
 */
static SYSCTL_PROC(_machdep_cpu, OID_AUTO, logical_per_package,
                   CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED,
                   (void *)offsetof(host_basic_info_data_t, logical_cpu_max),
                   sizeof(integer_t), arm_host_info, "I",
                   "CPU logical cpus per package");
 ...
 */
static SYSCTL_PROC(_machdep_cpu, OID_AUTO, thread_count,
                   CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED,
                   (void *)offsetof(host_basic_info_data_t, logical_cpu),
                   sizeof(integer_t), arm_host_info, "I",
                   "Number of enabled threads per package");
 ...
 */
__startup_func static void sysctl_load_brand_string(void) {
  DTEntry node;
  ...
  if (kSuccess != SecureDTGetProperty(node, "product-soc-name",
 ...
 */
static int make_brand_string SYSCTL_HANDLER_ARGS {
  __unused struct sysctl_oid *unused_oidp = oidp;
...
SYSCTL_PROC(_machdep_cpu, OID_AUTO, brand_string,
            ...
            make_brand_string, "A", "CPU brand string");

static int virtual_address_size SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2, oidp)
  int return_value = 64 - T0SZ_BOOT;
  return SYSCTL_OUT(req, &return_value, sizeof(return_value));
}

static SYSCTL_PROC(_machdep, OID_AUTO, virtual_address_size,
                   CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
                   virtual_address_size, "I",
                   "Number of addressable bits in userspace virtual addresses");

#if DEVELOPMENT || DEBUG
extern uint64_t TLockTimeOut;
SYSCTL_QUAD(_machdep, OID_AUTO, tlto, CTLFLAG_RW | CTLFLAG_LOCKED,
            &TLockTimeOut, "Ticket spinlock timeout (MATUs): use with care");
...
SYSCTL_QUAD(_machdep, OID_AUTO, wkdmdretries, CTLFLAG_RW | CTLFLAG_LOCKED,
            &wkdmdretries, "Number of WKDM errata retries");
SYSCTL_QUAD(_machdep, OID_AUTO, wkdmdretriespb, CTLFLAG_RW | CTLFLAG_LOCKED,
            &wkdmdretriespb,
            "Number of retries where payload was on page boundary");
SYSCTL_UINT(_machdep, OID_AUTO, simulate_wkdm2p_error,
            CTLFLAG_RW | CTLFLAG_LOCKED, &simulate_wkdm2p_error, 0, "");
SYSCTL_UINT(_machdep, OID_AUTO, wkdm_isa_2p_war_required,
            CTLFLAG_RW | CTLFLAG_LOCKED, &wkdm_isa_2p_war_required, 0, "");
...
#define SYSCTL_PROC_MACHDEP_CPU_SYSREG(name)                                   \
  static int sysctl_sysreg_##name SYSCTL_HANDLER_ARGS {                        \
    _Pragma("unused(arg1, arg2, oidp)") uint64_t return_value =                \
        __builtin_arm_rsr64(#name);                                            \
    return SYSCTL_OUT(req, &return_value, sizeof(return_value));               \
  }                                                                            \
  SYSCTL_PROC(_machdep_cpu, OID_AUTO, sysreg_##name,                           \
              CTLFLAG_RD | CTLTYPE_QUAD | CTLFLAG_LOCKED, 0, 0,                \
              sysctl_sysreg_##name, "Q",                                       \
...
SYSCTL_QUAD(_machdep, OID_AUTO, report_phy_read_delay, MMIO_TIMEOUT_FLAGS,
            &report_phy_read_delay_to,
            "Maximum time before io/phys read gets reported or panics");
SYSCTL_QUAD(_machdep, OID_AUTO, report_phy_write_delay, MMIO_TIMEOUT_FLAGS,
            &report_phy_write_delay_to,
            "Maximum time before io/phys write gets reported or panics");
SYSCTL_QUAD(_machdep, OID_AUTO, trace_phy_read_delay, MMIO_TIMEOUT_FLAGS,
            &trace_phy_read_delay_to,
            "Maximum time before io/phys read gets ktraced");
SYSCTL_QUAD(_machdep, OID_AUTO, trace_phy_write_delay, MMIO_TIMEOUT_FLAGS,
            &trace_phy_write_delay_to,
            "Maximum time before io/phys write gets ktraced");

SYSCTL_INT(_machdep, OID_AUTO, phy_read_delay_panic,
           CTLFLAG_KERN | CTLFLAG_RW | CTLFLAG_LOCKED, &phy_read_panic, 0,
           "if set, report-phy-read-delay timeout panics");
SYSCTL_INT(_machdep, OID_AUTO, phy_write_delay_panic,
           CTLFLAG_KERN | CTLFLAG_RW | CTLFLAG_LOCKED, &phy_write_panic, 0,
           "if set, report-phy-write-delay timeout panics");
...
SYSCTL_QUAD(_machdep, OID_AUTO, sim_stretched_io_ns,
            ...
            "simulate stretched io in ml_read_io, ml_write_io");
#endif /* ML_IO_SIMULATE_STRETCHED_ENABLED */

#endif /* ML_IO_TIMEOUTS_ENABLED */

int opensource_kernel = 1;
SYSCTL_INT(_kern, OID_AUTO, opensource_kernel,
           CTLFLAG_KERN | CTLFLAG_RD | CTLFLAG_LOCKED, &opensource_kernel, 0,
           "Opensource Kernel");

static int machdep_ptrauth_enabled SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2, oidp)
...
SYSCTL_PROC(_machdep, OID_AUTO, ptrauth_enabled,
            CTLTYPE_INT | CTLFLAG_KERN | CTLFLAG_RD, 0, 0,
...
SYSCTL_ULONG(
    _debug, OID_AUTO, trap_telemetry_capacity_dropped_events,
...
/* A sysctl that can be used to check if the platform supports DRAM ECC and
 * error injection. */
static int dram_ecc_error_injection_capable SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_vm, OID_AUTO, dram_ecc_error_injection_capable,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            &dram_ecc_error_injection_capable, "I", "");
#endif /* DEBUG || DEVELOPMENT */

#if DEBUG || DEVELOPMENT
extern _Atomic unsigned int ipcpv_telemetry_count;
SYSCTL_UINT(_debug, OID_AUTO, ipcpv_telemetry_count,
            CTLFLAG_RD | CTLFLAG_LOCKED, &ipcpv_telemetry_count, 0,
            "Number of ipc policy violation telemetry emitted");
#endif /* DEBUG || DEVELOPMENT */
