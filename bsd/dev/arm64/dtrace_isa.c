 ...
 */
inline void dtrace_membar_producer(void) { __builtin_arm_dmb(DMB_ISH); }

inline void dtrace_membar_consumer(void) { __builtin_arm_dmb(DMB_ISH); }
 ...
 */
static inline bool dtrace_frame_in_kernel_stack(struct frame *fp) {
...
#pragma unused(arg, aframes)
  ...
  for (i = 1; i <= aframes; ++i) {
    ...
    pc = (uintptr_t)ptrauth_strip((void *)fp->retaddr,
...
void dtrace_flush_caches(void) {
  /* TODO There were some problems with flushing just the cache line that had
   * been modified. For now, we'll flush the entire cache, until we figure out
   * how to flush just the patched block.
   */
  FlushPoU_Dcache();
