...
#define FBT_IS_ARM64_ADD_FP_SP(x)                                              \
  (((x) & 0xffc003ff) ==                                                       \
   0x910003fd) /* add fp, sp, #val  (add fp, sp, #0 == mov fp, sp) */
#define FBT_IS_ARM64_RET(x)                                                    \
  (((x) == 0xd65f03c0) || ((x) == 0xd65f0fff)) /* ret, retab */
...
kern_return_t fbt_perfCallback(int, struct arm_saved_state *, __unused int,
                               __unused int);

int fbt_invop(uintptr_t addr, uintptr_t *stack, uintptr_t rval) {
  ...
  for (; fbt != NULL; fbt = fbt->fbtp_hashnext) {
    if ((uintptr_t)fbt->fbtp_patchpoint == addr) {
      if (0 == CPU->cpu_dtrace_invop_underway) {
         ...
         *      ------------------------
         *      fbt entry probe:
         *          Frame from traced function: <previous sp (e.g. 0x1000),
         * return address> fbt return probe: Missing as the return probe has
         * already popped the frame in the callee and traps with LR set to the
         * return address in caller.
         *      ------------------------
         *      arm_context_t
         *      ------------------------
         *      Frame from trap handler:  <previous sp (e.g. 0x1000) , traced PC
         * > The traced function has either never pushed the frame or already
         * popped it.  So there is no frame in the backtrace pointing to the
         * frame on the stack containing the LR in the caller.
         *     ------------------------
         *          |
         *          |
         ...
         *     [Lower addresses]
         *
         * cpu_dtrace_caller compensates for fact that the LR is not stored on
         * stack as explained above.  When walking the stack, when we reach the
         * frame where we extract a PC in the patched function, we put the
         * cpu_dtrace_caller in the backtrace instead.  The next frame we
         * extract will be in the caller's caller, so we output a backtrace
         * starting at the caller and going sequentially up the stack.
         */
        arm_saved_state_t *regs =
            (arm_saved_state_t *)(&((arm_context_t *)stack)->ss);

        CPU->cpu_dtrace_caller = get_saved_state_lr(regs);

        /* When fbt_roffset is non-zero, we know we are handling a return probe
         * point. */
        if (fbt->fbtp_roffset == 0) {
          dtrace_probe(
              fbt->fbtp_id, get_saved_state_reg(regs, 0),
              get_saved_state_reg(regs, 1), get_saved_state_reg(regs, 2),
              get_saved_state_reg(regs, 3), get_saved_state_reg(regs, 4));
        } else {
      ...
      /*
       *       On other architectures, we return a DTRACE constant to let the
       * callback function know what was replaced. On the ARM, since the
       * function prologue/epilogue machine code can vary, we need the actual
       * bytes of the instruction, so return the savedval instead.
       */
...
kern_return_t fbt_perfCallback(int trapno, struct arm_saved_state *regs,
                               __unused int unused1, __unused int unused2) {
  ...
  if (FBT_EXCEPTION_CODE == trapno && !IS_USER_TRAP(regs)) {
    ...
    emul = dtrace_invop(get_saved_state_pc(regs), (uintptr_t *)regs,
                        get_saved_state_reg(regs, 0));
...
void fbt_provide_probe(struct modctl *ctl, const char *modname,
                       const char *symbolName, machine_inst_t *symbolStart,
                       machine_inst_t *instrHigh) {
  int doenable = 0;
  dtrace_id_t thisid;
  ...
  if (thisid != 0) {
     ...
     */
    newfbt->fbtp_id = dtrace_probe_create(fbt_id, modname, symbolName,
                                          FBT_ENTRY, FBT_AFRAMES_ENTRY, newfbt);
    doenable = 0;
  }
...
again:
  ...
  /* Walked onto the start of the next routine? If so, bail out from this
   * function */
  if (FBT_IS_ARM64_FRAME_PUSH(theInstr)) {
    if (!retfbt) {
      kprintf("dtrace: fbt: No return probe for %s, walked to next routine at "
              "0x%016llx\n",
  ...
  if (retfbt == NULL) {
    newfbt->fbtp_id = dtrace_probe_create(
        fbt_id, modname, symbolName, FBT_RETURN, FBT_AFRAMES_RETURN, newfbt);
  } else {
