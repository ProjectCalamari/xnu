...
int fasttrap_tracepoint_init(proc_t *p, fasttrap_tracepoint_t *tp,
...
#pragma unused(type)
  ...
  if (tp->ftt_type == FASTTRAP_T_ARM64_EXCLUSIVE_MEM) {
    kprintf("Detected attempt to place DTrace probe on exclusive memory "
            "instruction (pc = 0x%llx); refusing to trace (or exclusive "
            "operation could never succeed).\n",
            pc);
...
int fasttrap_tracepoint_install(proc_t *p, fasttrap_tracepoint_t *tp) {
  uint32_t instr;
  int size;
...
int fasttrap_tracepoint_remove(proc_t *p, fasttrap_tracepoint_t *tp) {
  uint32_t instr;
  int size = 4;
...
static void fasttrap_return_common(proc_t *p, arm_saved_state_t *regs,
                                   user_addr_t pc, user_addr_t new_pc) {
  ...
  for (id = tp->ftt_retids; id != NULL; id = id->fti_next) {
    ...
    if ((tp->ftt_type != FASTTRAP_T_ARM64_RET ||
         tp->ftt_type != FASTTRAP_T_ARM64_RETAB) &&
    ...
    if (FALSE) {
      ...
      dtrace_probe(probe->ftp_id, pc - id->fti_probe->ftp_faddr,
...
__dead2
#endif
    static void fasttrap_sigsegv(proc_t *p, uthread_t t, user_addr_t addr,
                                 arm_saved_state_t *regs) {
  /* TODO: This function isn't implemented yet. In debug mode, panic the system
   * to find out why we're hitting this point. In other modes, kill the process.
   */
#if DEBUG
...
static void fasttrap_usdt_args64(fasttrap_probe_t *probe,
                                 arm_saved_state64_t *regs64, int argc,
                                 uint64_t *argv) {
  ...
  for (i = 0; i < cap; i++) {
    ...
    if (x < 8) {
      ...
      fasttrap_fuword64_noerr(regs64->sp + (x - 8) * sizeof(uint64_t),
                              &argv[i]);
    }
  }
...
static int condition_true(int cond, int cpsr) {
  int taken = 0;
  int zf = (cpsr & PSR64_Z) ? 1 : 0, nf = (cpsr & PSR64_N) ? 1 : 0,
      cf = (cpsr & PSR64_C) ? 1 : 0, vf = (cpsr & PSR64_V) ? 1 : 0;
  ...
  case 15:
    taken = 1;
    break; /* always "true" for ARM, unpredictable for THUMB. */
  }
 ...
 * cannot clobber a GPR.  Note that we have to specially handle PC-rel
 * loads/stores as well, which have range +/- 1MB (convert to an indirect load).
 * Instruction buffer layout:
 *
 *    [ Thunked instruction sequence ]
 *    [ Trap for return to original code and return probe handling ]
 *
 * This *does* make it impossible for an ldxr/stxr pair to succeed if we trace
 * on or between them... may need to get fancy at some point.
 */
static void fasttrap_pid_probe_thunk_instr64(arm_saved_state_t *state,
                                             fasttrap_tracepoint_t *tp,
                                             proc_t *p, uthread_t uthread,
  ...
  uthread->t_dtrace_astpc = uthread->t_dtrace_scrpc =
      uthread->t_dtrace_scratch->addr;
  ...
  if (uwrite(p, local_scratch, (num_instrs + 1) * sizeof(uint32_t),
             user_scratch_area) != KERN_SUCCESS) {
    fasttrap_sigtrap(p, uthread, pc);
    *pc_out = pc;
    return;
  }

  /* We're stepping (come back to kernel to adjust PC for return to regular
   * code). */
  uthread->t_dtrace_step = 1;

  /* We may or may not be about to run a return probe (but we wouldn't thunk ret
   * lr)*/
 ...
 */
static int64_t sign_extend(int64_t input, uint32_t sign_bit_index) {
 ...
 */
static uint64_t get_saved_state64_regno(arm_saved_state64_t *regs64,
                                        uint32_t regno, int use_xzr) {
...
static void set_saved_state_regno(arm_saved_state_t *state, uint32_t regno,
                                  int use_xzr, register_t value) {
  /* Set PC to register value */
  switch (regno) {
  case 29:
 ...
 */
static uint64_t extract_address_literal_sign_extended(uint32_t instr,
                                                      uint32_t base,
                                                      uint32_t numbits) {
  uint64_t offset;
...
static void do_cbz_cnbz(arm_saved_state64_t *regs64, uint32_t regwidth,
                        uint32_t instr, int is_cbz, user_addr_t *pc_out) {
  uint32_t regno;
  uint64_t regval;
  uint64_t offset;
...
static void do_tbz_tbnz(arm_saved_state64_t *regs64, uint32_t instr, int is_tbz,
                        user_addr_t *pc_out) {
...
static void fasttrap_pid_probe_handle_patched_instr64(
    arm_saved_state_t *state, fasttrap_tracepoint_t *tp __unused,
    uthread_t uthread, proc_t *p, uint_t is_enabled, int *was_simulated) {
   ...
   * All loads are 32bits or greater (no need to handle byte or halfword
   * accesses). LDR Wt, addr LDR Xt, addr LDRSW Xt, addr
   *
   *      LDR St, addr
   *      LDR Dt, addr
   *      LDR Qt, addr
...
#ifndef DEBUG
      new_pc = regs64->pc; /* Bit of a hack, we know about update in
                              fasttrap_sigsegv() */
      break;
#endif
    }
  ...
  case FASTTRAP_T_ARM64_RETAB: {
    ...
    new_pc =
        (user_addr_t)ptrauth_strip((void *)new_pc, ptrauth_key_return_address);
#endif
  ...
  case FASTTRAP_T_ARM64_ADR: {
    ...
    /* Add to PC.  Use unsigned addition so that overflow wraps (rather than
     * being undefined). */
    if (tp->ftt_type == FASTTRAP_T_ARM64_ADRP) {
      ...
      result =
          (regs64->pc & ~0xfffULL) + offset; /* And add to page of current pc */
    } else {
  ...
  case FASTTRAP_T_COMMON: {
    fasttrap_pid_probe_thunk_instr64(state, tp, p, uthread, &tp->ftt_instr, 1,
                                     &new_pc);
    *was_simulated = 0;
    break;
  }
...
int fasttrap_pid_probe(arm_saved_state_t *state) {
    ...
    for (id = tp->ftt_ids; id != NULL; id = id->fti_next) {
      ...
      if (FALSE) {
        ...
        if (id->fti_ptype == DTFTP_ENTRY) {
          ...
          dtrace_probe(probe->ftp_id, get_saved_state_reg(state, 0),
                       get_saved_state_reg(state, 1),
                       get_saved_state_reg(state, 2),
                       get_saved_state_reg(state, 3), arg4);
   ...
   */
  fasttrap_pid_probe_handle_patched_instr64(state, tp, uthread, p, is_enabled,
                                            &was_simulated);
  ...
  if (tp->ftt_retids != NULL) {
     ...
     */
    if (was_simulated) {
      fasttrap_return_common(p, state, (user_addr_t)pc,
                             (user_addr_t)get_saved_state_pc(state));
    } else {
      ASSERT(uthread->t_dtrace_ret != 0);
      ASSERT(uthread->t_dtrace_pc == pc);
      ASSERT(uthread->t_dtrace_scrpc != 0);
      ASSERT(((user_addr_t)get_saved_state_pc(state)) ==
             uthread->t_dtrace_astpc);
    }
  }
...
int fasttrap_return_probe(arm_saved_state_t *regs) {
  proc_t *p = current_proc();
  uthread_t uthread = current_uthread();
...
uint64_t fasttrap_usdt_getarg(void *arg, dtrace_id_t id, void *parg, int argno,
                              int aframes) {
