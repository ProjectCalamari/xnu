...
int sdt_invop(__unused uintptr_t addr, __unused uintptr_t *stack,
              __unused uintptr_t eax) {
#pragma unused(eax)
  ...
  for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
    if ((uintptr_t)sdt->sdp_patchpoint == addr) {
      ...
      dtrace_probe(sdt->sdp_id, get_saved_state_reg(regs, 0),
...
#pragma unused(arg, id, parg) /* __APPLE__ */
  ...
  for (i = 1; i <= aframes; i++) {
    ...
    if (dtrace_invop_callsite_pre != NULL &&
      ...
      arm_saved_state_t *tagged_regs =
          (arm_saved_state_t *)((uintptr_t *)&fp[1]);
