...
static const struct cdevsw mt_cdevsw = {.d_open = mt_cdev_open,
...
static mt_device_t mt_get_device(dev_t devnum) {
...
static void mt_device_lock(mt_device_t dev) { lck_mtx_lock(&dev->mtd_lock); }

static void mt_device_unlock(mt_device_t dev) {
  lck_mtx_unlock(&dev->mtd_lock);
}

static void mt_device_assert_lock_held(__assert_only mt_device_t dev) {
  LCK_MTX_ASSERT(&dev->mtd_lock, LCK_MTX_ASSERT_OWNED);
}

static void mt_device_assert_inuse(__assert_only mt_device_t dev) {
...
int mt_dev_init(void) {
  ...
  for (int i = 0; i < MT_NDEVS; i++) {
    ...
    void *node = devfs_make_node(dev, DEVFS_CHAR, UID_ROOT, GID_WINDOWSERVER,
                                 0666, MT_NODE "/%s", mt_devices[i].mtd_name);
    if (!node) {
...
static int mt_cdev_open(dev_t devnum, __unused int flags, __unused int devtype,
                        __unused proc_t p) {
  int error = 0;
...
static int mt_cdev_close(dev_t devnum, __unused int flags, __unused int devtype,
...
static int mt_ctl_add(mt_device_t dev, user_addr_t uptr) {
  int error;
  uint32_t ctr;
...
static int mt_ctl_counts(mt_device_t dev, user_addr_t uptr) {
  int error;
...
static int mt_ctl_enable(mt_device_t dev, user_addr_t uptr) {
  int error;
...
static int mt_ctl_reset(mt_device_t dev) {
  mt_device_assert_lock_held(dev);
  dev->mtd_reset();
  return 0;
}

static int mt_cdev_ioctl(dev_t devnum, unsigned long cmd, char *arg,
                         __unused int flags, __unused proc_t p) {
  int error = ENODEV;
...
static void _convert_usage_to_counts(struct recount_usage *usage,
                                     uint64_t *counts) {
...
static int mt_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg2)
  ...
  case MT_SUPPORTED:
    return sysctl_io_number(req, (int)mt_core_supported, sizeof(int), NULL,
                            NULL);
  case MT_PMIS:
...
MT_SYSCTL(pmis, MT_PMIS, 0, sizeof(uint64_t), "Q", "number of PMIs seen");
...
MT_SYSCTL(kdebug_test, MT_KDBG_TEST, CTLFLAG_MASKED, sizeof(int), "O",
          "whether task and thread counting is enabled");
MT_SYSCTL(fixed_cpu_perf, MT_FIX_CPU_PERF, CTLFLAG_MASKED, sizeof(uint64_t) * 2,
          "O", "overhead of accessing the current CPU's counters");
