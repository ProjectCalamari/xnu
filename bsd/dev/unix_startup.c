...
SYSCTL_INT(_kern, OID_AUTO, nbuf, CTLFLAG_RD | CTLFLAG_LOCKED, &nbuf_headers, 0,
           "");
SYSCTL_INT(_kern, OID_AUTO, maxnbuf, CTLFLAG_RW | CTLFLAG_LOCKED | CTLFLAG_KERN,
           &max_nbuf_headers, 0, "");
...
static vm_map_size_t bsd_get_bufferhdr_map_size(void) {
  vm_size_t size;

  /* clip the number of buf headers upto 16k */
  if (max_nbuf_headers == 0) {
    max_nbuf_headers = (int)atop_kernel(
        sane_size / 50); /* Get 2% of ram, but no more than we can map */
  }
...
void bsd_startupearly(void) {
  ...
  bufferhdr_map = kmem_suballoc(kernel_map, &bufferhdr_range.min_address, size,
                                VM_MAP_CREATE_NEVER_FAULTS,
                                VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,
                                KMS_PERMANENT | KMS_NOFAIL, VM_KERN_MEMORY_FILE)
                      .kmr_submap;

  kmem_alloc(bufferhdr_map, &(vm_offset_t){bufferhdr_range.min_address}, size,
  ...
  if (vnodes_sized == 0) {
    if (!PE_get_default("kern.maxvnodes", &desiredvnodes,
       ...
       * CONFIG_VNODES is set to 263168 for "medium" configurations (the
       * default) but can be smaller or larger.
       */
...
#if CONFIG_MBUF_MCACHE
  mb_map = kmem_suballoc(kernel_map, &mb_range.min_address,
                         (vm_size_t)(nmbclusters * MCLBYTES), FALSE,
                         VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE,
                         KMS_PERMANENT | KMS_NOFAIL, VM_KERN_MEMORY_MBUF)
               .kmr_submap;
 ...
 */
unsigned int bsd_mbuf_cluster_reserve(boolean_t *overridden) {
...
void bsd_scale_setup(int scale) {
#if defined(__LP64__)
  if (scale > 0) {
    if (!serverperfmode) {
      maxproc *= scale;
