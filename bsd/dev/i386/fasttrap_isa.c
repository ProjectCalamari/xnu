 ...
 * a given instruction is trying to reference. OS X does not have extended
 * registers for 32 bit apps, but the *order* is the same. So for 32 bit state,
 * we will return:
 *
 * REG_RAX -> EAX
 * REG_RCX -> ECX
...
static uint64_t fasttrap_anarg(x86_saved_state_t *regs, int function_entry,
                               int argno) {
  uint64_t value;
...
#pragma unused(type)
  ...
  uint8_t seg, rex = 0;
  unsigned int p_model =
      (p->p_flag & P_LP64) ? DATAMODEL_LP64 : DATAMODEL_ILP32;
  ...
  if (uread(p, &instr[0], first, pc) != 0)
    return (-1);
  if (len > first && uread(p, &instr[first], len - first, pc + first) != 0) {
  ...
  if (instr[start] == FASTTRAP_2_BYTE_OP) {
    ...
    if (reg == 2 || reg == 4) {
      ...
      if (mod != 3 && rm == 4) {
        ...
        if (mod == 0 && rm == 5) {
          ...
          uint8_t base = rm | (FASTTRAP_REX_B(rex) << 3);
    ...
    case FASTTRAP_JG:
      ...
      tp->ftt_dest = pc + tp->ftt_size + (int8_t)instr[start + 1];
      break;
  ...
  if (p_model == DATAMODEL_LP64 && tp->ftt_type == FASTTRAP_T_COMMON) {
    ...
    if (rmindex != -1) {
      ...
      if (mod == 0 && rm == 5) {
        ...
        if (reg != 0) {
          tp->ftt_ripmode =
              FASTTRAP_RIP_1 | (FASTTRAP_RIP_X * FASTTRAP_REX_B(rex));
          rm = 0;
        } else {
          tp->ftt_ripmode =
              FASTTRAP_RIP_2 | (FASTTRAP_RIP_X * FASTTRAP_REX_B(rex));
          rm = 1;
        }

        tp->ftt_modrm = tp->ftt_instr[rmindex];
        tp->ftt_instr[rmindex] = FASTTRAP_MODRM(2, reg, rm);
      }
    }
  }
...
static void fasttrap_return_common(x86_saved_state_t *regs, user_addr_t pc,
                                   pid_t pid, user_addr_t new_pc) {
  x86_saved_state64_t *regs64;
  x86_saved_state32_t *regs32;
  unsigned int p_model;
  int retire_tp = 1;
  ...
  for (id = tp->ftt_retids; id != NULL; id = id->fti_next) {
     ...
     */
    if (tp->ftt_type != FASTTRAP_T_RET && tp->ftt_type != FASTTRAP_T_RET16 &&
    ...
    if (ISSET(current_proc()->p_lflag, P_LNOATTACH)) {
      ...
      dtrace_probe(probe->ftp_id, pc - id->fti_probe->ftp_faddr, regs64->rax,
                   regs64->rdx, 0, 0);
    } else {
      dtrace_probe(probe->ftp_id, pc - id->fti_probe->ftp_faddr, regs32->eax,
                   regs32->edx, 0, 0);
    }
    /* remove the hint */
...
static void fasttrap_usdt_args64(fasttrap_probe_t *probe,
                                 x86_saved_state64_t *regs64, int argc,
                                 uint64_t *argv) {
...
static void fasttrap_usdt_args32(fasttrap_probe_t *probe,
                                 x86_saved_state32_t *regs32, int argc,
                                 uint32_t *argv) {
  int i, x, cap = MIN(argc, probe->ftp_nargs);
  uint32_t *stack = (uint32_t *)(uintptr_t)(regs32->uesp);
 ...
 * FIXME!
 */
static int fasttrap_do_seg(fasttrap_tracepoint_t *tp, x86_saved_state_t *rp,
                           user_addr_t *addr) // 64 bit
{
...
/*
 * Due to variances between Solaris and xnu, I have split this into a 32 bit and
 * 64 bit code path. It still takes an x86_saved_state_t* argument, because it
 * must sometimes call other methods that require a x86_saved_state_t.
 *
 * NOTE!!!!
 *
 ...
 *
 */
static int fasttrap_pid_probe32(x86_saved_state_t *regs) {
    ...
    for (id = tp->ftt_ids; id != NULL; id = id->fti_next) {
      ...
      if (ISSET(current_proc()->p_lflag, P_LNOATTACH)) {
        ...
        if (id->fti_ptype == DTFTP_ENTRY) {
          ...
          dtrace_probe(probe->ftp_id, s1, s2, s3, s4, s5);
          ...
          dtrace_probe(probe->ftp_id, s0, s1, s2, s3, s4);
        } else {
          uint32_t t[5];

          fasttrap_usdt_args32(probe, regs32, sizeof(t) / sizeof(t[0]), t);
  ...
  case FASTTRAP_T_LOOP: {
    ...
    switch (tp->ftt_code) {
    case FASTTRAP_LOOPNZ:
      taken = (regs32->efl & FASTTRAP_EFLAGS_ZF) == 0 && cx != 0;
      break;
    case FASTTRAP_LOOPZ:
      taken = (regs32->efl & FASTTRAP_EFLAGS_ZF) != 0 && cx != 0;
      break;
  ...
  case FASTTRAP_T_CALL:
    if (tp->ftt_code == 0) {
      ...
      if (tp->ftt_index != FASTTRAP_NOREG)
        addr += fasttrap_getreg(regs, tp->ftt_index) << tp->ftt_scale;
 ...
 *
 */
static int fasttrap_pid_probe64(x86_saved_state_t *regs) {
  ...
  if (tp->ftt_ids != NULL) {
    ...
    for (id = tp->ftt_ids; id != NULL; id = id->fti_next) {
      ...
      if (ISSET(current_proc()->p_lflag, P_LNOATTACH)) {
        ...
        cookie = dtrace_interrupt_disable();
        DTRACE_CPUFLAG_SET(CPU_DTRACE_ENTRY);
        dtrace_probe(probe->ftp_id, regs64->rdi, regs64->rsi, regs64->rdx,
                     regs64->rcx, regs64->r8);
        DTRACE_CPUFLAG_CLEAR(CPU_DTRACE_ENTRY);
        dtrace_interrupt_enable(cookie);
      } else if (id->fti_ptype == DTFTP_IS_ENABLED) {
        /*
        ...
        fasttrap_usdt_args64(probe, regs64, sizeof(t) / sizeof(t[0]), t);
  ...
  case FASTTRAP_T_LOOP: {
    ...
    switch (tp->ftt_code) {
    case FASTTRAP_LOOPNZ:
      taken = (regs64->isf.rflags & FASTTRAP_EFLAGS_ZF) == 0 && cx != 0;
      break;
    case FASTTRAP_LOOPZ:
      taken = (regs64->isf.rflags & FASTTRAP_EFLAGS_ZF) != 0 && cx != 0;
      break;
  ...
  case FASTTRAP_T_COMMON: {
    ...
    if (tp->ftt_ripmode != 0) {
      ...
      ASSERT(tp->ftt_ripmode & (FASTTRAP_RIP_1 | FASTTRAP_RIP_2));
...
int fasttrap_pid_probe(x86_saved_state_t *regs) {
  if (is_saved_state64(regs))
...
int fasttrap_return_probe(x86_saved_state_t *regs) {
  x86_saved_state64_t *regs64;
  x86_saved_state32_t *regs32;
  unsigned int p_model;
...
#pragma unused(arg, id, parg, aframes)
  return (fasttrap_anarg((x86_saved_state_t *)find_user_regs(current_thread()),
                         1, argno));
}
 ...
 */
static user_addr_t fasttrap_getreg(x86_saved_state_t *regs, uint_t reg) {
  if (is_saved_state64(regs)) {
    x86_saved_state64_t *regs64 = saved_state64(regs);
    ...
    case REG_R15:
      return regs64->r15;
    case REG_TRAPNO:
      return regs64->isf.trapno;
    case REG_ERR:
      return regs64->isf.err;
    case REG_RIP:
      return regs64->isf.rip;
    case REG_CS:
      return regs64->isf.cs;
    case REG_RFL:
      return regs64->isf.rflags;
    case REG_SS:
      return regs64->isf.ss;
    case REG_FS:
      return regs64->fs;
    case REG_GS:
      return regs64->gs;
    ...
    case REG_FSBASE:
    case REG_GSBASE:
      // Important to distinguish these requests (which should be legal) from
      // other values.
