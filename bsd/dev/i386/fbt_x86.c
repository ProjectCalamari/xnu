...
#define FBT_JMP_SHORT_REL                                                      \
  0xeb /* Jump short, relative, displacement relative to next instr. */
#define FBT_JMP_NEAR_REL                                                       \
  0xe9 /* Jump near, relative, displacement relative to next instr. */
...
kern_return_t fbt_perfCallback(int, x86_saved_state_t *, uintptr_t *,
                               __unused int);
  ...
  for (; fbt != NULL; fbt = fbt->fbtp_hashnext) {
    if ((uintptr_t)fbt->fbtp_patchpoint == addr) {
      ...
      if (fbt->fbtp_roffset == 0) {
        ...
        CPU->cpu_dtrace_caller = *(uintptr_t *)(((uintptr_t)(regs->isf.rsp)) +
                                                sizeof(uint64_t)); // 8(%rsp)
        /* 64-bit ABI, arguments passed in registers. */
        dtrace_probe(fbt->fbtp_id, regs->rdi, regs->rsi, regs->rdx, regs->rcx,
                     regs->r8);
...
kern_return_t fbt_perfCallback(int trapno, x86_saved_state_t *tagged_regs,
  ...
  if (FBT_EXCEPTION_CODE == trapno && !IS_USER_TRAP(saved_state)) {
     ...
     */
    rsp_probe = saved_state->isf
                    .rsp; /* Easy, x86_64 establishes this value in idt64.s */
    ...
    case DTRACE_INVOP_NOP:
      saved_state->isf.rip += DTRACE_INVOP_NOP_SKIP; /* Skip over the patched
                                                        NOP (planted by sdt). */
      retval = KERN_SUCCESS;
      break;
    ...
    case DTRACE_INVOP_MOV_RSP_RBP:
      ...
      saved_state->isf.rip +=
          DTRACE_INVOP_MOV_RSP_RBP_SKIP; /* Skip over the bytes of the patched
                                            mov %rsp,%rbp */
      retval = KERN_SUCCESS;
      break;
    ...
    case DTRACE_INVOP_LEAVE:
       ...
       * and at the slot holding the frame pointer saved by the target's
       * prologue.
       */
      fp = saved_state->rbp;
       ...
       */
      delta = ((uint32_t *)fp) -
              ((uint32_t *)rsp_probe); /* delta is a *word* increment */
      ...
      for (pDst = (uint32_t *)fp; pDst > (((uint32_t *)old_sp)); pDst--)
        *pDst = pDst[-delta];
      ...
      saved_state =
          (x86_saved_state64_t *)(((uintptr_t)saved_state) + (delta << 2));
...
void fbt_provide_probe(struct modctl *ctl, const char *modname,
  ...
  for (j = 0, instr = symbolStart, theInstr = 0;
    ...
    theInstr = instr[0];
    if (theInstr == FBT_PUSH_RBP || theInstr == FBT_RET ||
        theInstr == FBT_RET_IMM16)
      break;
  ...
  if (i1 == FBT_REX_RSP_RBP && i2 == FBT_MOV_RSP_RBP0 &&
      i3 == FBT_MOV_RSP_RBP1) {
...
again:
  ...
  /* Walked onto the start of the next routine? If so, bail out of this
   * function. */
  if (theInstr == FBT_PUSH_RBP)
    return;
  ...
  /*
   * pop %rbp; ret; or leave; ret; or leave; jmp tailCalledFun; -- We have a
   * winner!
   */
  newfbt = kmem_zalloc(sizeof(fbt_probe_t), KM_SLEEP);
  strlcpy((char *)&(newfbt->fbtp_name), symbolName, MAX_FBTP_NAME_CHARS);
  ...
  newfbt->fbtp_roffset = (uintptr_t)(patch_instr - (uint8_t *)symbolStart);
