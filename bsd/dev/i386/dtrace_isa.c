...
void dtrace_probe_error(dtrace_state_t *state, dtrace_epid_t epid, int which,
   ...
   * stash away "illval" here, and special-case retrieving it in
   * DIF_VARIABLE_ARG.
   */
  state->dts_arg_error_illval = illval;
  dtrace_probe(dtrace_probeid_error, (uint64_t)(uintptr_t)state, epid, which,
               fltoffs, fault);
}
 ...
 */
void dtrace_membar_producer(void) { __asm__ volatile("sfence"); }

void dtrace_membar_consumer(void) { __asm__ volatile("lfence"); }
 ...
 */
void dtrace_xcall(processorid_t cpu, dtrace_xcall_t f, void *arg) {
  xcArg_t xcArg;
 ...
 * Runtime and ABI
 */
uint64_t dtrace_getreg(struct regs *savearea, uint_t reg) {
...
uint64_t dtrace_getvmreg(uint_t ndx) {
  uint64_t reg = 0;
  bool failed = false;

  /* Any change in the vmread final opcode must be reflected in
   * dtrace_handle_trap below. */
...
static void dtrace_vmcall(x86_saved_state_t *regs, uint64_t *rflags) {
  uint64_t flags = 0;
  ...
  __asm__ volatile("   movq %12, %%r8  \n"
                   ...
                   : "=a"(regs->ss_64.rax), "=D"(regs->ss_64.rdi),
                     "=S"(regs->ss_64.rsi), "=d"(regs->ss_64.rdx),
                     "=c"(regs->ss_64.rcx), "=r"(regs->ss_64.r8), /* %5 */
                     "=r"(regs->ss_64.r9),                        /* %6 */
                     "=r"(flags)                                  /* %7 */

                   : "a"(regs->ss_64.rax), "D"(regs->ss_64.rdi),
                     "S"(regs->ss_64.rsi), "d"(regs->ss_64.rdx),
                     "c"(regs->ss_64.rcx), "r"(regs->ss_64.r8), /* %12 */
                     "r"(regs->ss_64.r9)                        /* %13 */
...
static inline void dtrace_cpuid(x86_saved_state_t *regs) {
  __asm__ volatile("cpuid"
                   : "=a"(regs->ss_64.rax), "=b"(regs->ss_64.rbx),
                     "=c"(regs->ss_64.rcx), "=d"(regs->ss_64.rdx)

                   : "a"(regs->ss_64.rax), "b"(regs->ss_64.rbx),
                     "c"(regs->ss_64.rcx), "d"(regs->ss_64.rdx));
}

static bool dtrace_applepv_available(uint64_t flag) {
  ...
  if ((regs.ss_64.rcx & _Bit(31)) != 0) {
    for (uint32_t base = 0x40000100; base < 0x40010000; base += 0x100) {
      ...
      /* "apple-pv-xnu" */
      if (regs.ss_64.rbx != 0x6c707061 || regs.ss_64.rcx != 0x76702d65 ||
...
void dtrace_livedump(char *filename, size_t len) {
...
static int dtrace_getustack_common(uint64_t *pcstack, int pcstack_limit,
                                   user_addr_t pc, user_addr_t sp) {
 ...
 */
static int dtrace_adjust_stack(uint64_t **pcstack, int *pcstack_limit,
                               user_addr_t *pc, user_addr_t sp) {
  volatile uint16_t *flags =
      (volatile uint16_t *)&cpu_core[CPU->cpu_id].cpuc_dtrace_flags;
  int64_t missing_tos;
...
void dtrace_getpcstack(pc_t *pcstack, int pcstack_limit, int aframes,
                       uint32_t *intrpc) {
...
uint64_t dtrace_getarg(int arg, int aframes, dtrace_mstate_t *mstate,
...
load:
  if (dtrace_canload((uint64_t)(stack + arg), sizeof(uint64_t), mstate,
                     vstate)) {
 ...
 * Load/Store Safety
 */
void dtrace_toxic_ranges(void (*func)(uintptr_t base, uintptr_t limit)) {
  /*
...
boolean_t dtrace_handle_trap(int trapno, x86_saved_state_t *state) {
  ...
  /*
   * General purpose solution would require pulling in disassembler. Right now
   * there is only one specific case to be handled so it is hardcoded here.
   */
