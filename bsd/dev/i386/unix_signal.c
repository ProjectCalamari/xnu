...
static const xstate_info_t thread_state64[] = {
    [FP] = {x86_FLOAT_STATE64, x86_FLOAT_STATE64_COUNT,
            sizeof(struct mcontext64)},
    [FP_FULL] = {x86_FLOAT_STATE64, x86_FLOAT_STATE64_COUNT,
                 sizeof(struct mcontext64_full)},
    [AVX] = {x86_AVX_STATE64, x86_AVX_STATE64_COUNT,
             sizeof(struct mcontext_avx64)},
    [AVX_FULL] = {x86_AVX_STATE64, x86_AVX_STATE64_COUNT,
                  sizeof(struct mcontext_avx64_full)},
    [AVX512] = {x86_AVX512_STATE64, x86_AVX512_STATE64_COUNT,
                sizeof(struct mcontext_avx512_64)},
    [AVX512_FULL] = {x86_AVX512_STATE64, x86_AVX512_STATE64_COUNT,
                     sizeof(struct mcontext_avx512_64_full)}};
static const xstate_info_t thread_state32[] = {
    [FP] = {x86_FLOAT_STATE32, x86_FLOAT_STATE32_COUNT,
            sizeof(struct mcontext32)},
    [AVX] = {x86_AVX_STATE32, x86_AVX_STATE32_COUNT,
 ...
 */
static void siginfo_user_to_user32_x86(user_siginfo_t *in,
                                       user32_siginfo_t *out) {
...
static void siginfo_user_to_user64_x86(user_siginfo_t *in,
                                       user64_siginfo_t *out) {
  out->si_signo = in->si_signo;
  out->si_errno = in->si_errno;
  out->si_code = in->si_code;
  out->si_pid = in->si_pid;
...
void sendsig(struct proc *p, user_addr_t ua_catcher, int sig, int mask,
  ...
  if (proc_is64bit(p)) {
     ...
     * struct.  This works specifically because the mcontext is layed-out with
     * the variable-sized FP-state as the last member.  However, with the
     * requirement to support passing "full" 64-bit state to the signal handler,
     * that layout has now changed (since the "full" state has a larger "ss"
     * member than the non-"full" structure.  Because of this, and to retain the
     * array-lookup method of determining structure sizes, we OR-in STATE64_FULL
     * to sig_xstate to ensure the proper mcontext size is passed.
     */

    if (thread_getstatus(thread, flavor, (thread_state_t)state, &state_count) !=
        KERN_SUCCESS) {
      goto bad;
    }

    if ((sig_xstate & STATE64_FULL) && mctxp->mctx_avx64.ss.cs != USER64_CS) {
      if ((ut->uu_flag & UT_ALTSTACK) && !oonstack && (sigonstack)) {
        reset_ss = TRUE;
      } else {
        reset_ss = FALSE;
      }
    ...
    if (thread_getstatus(thread, flavor, (thread_state_t)fpstate,
    ...
    if (copyout((caddr_t)&mctx_store, ua_mctxp,
                thread_state64[sig_xstate].mcontext_size)) {
      goto bad;
    }
    ...
    if (copyout((caddr_t)&mctx_store, ua_mctxp,
                thread_state32[sig_xstate].mcontext_size)) {
      goto bad;
    }
...
#define FP_PE 5 /* precision */
  ...
  default: {
     ...
     */
    sinfo64.si_status = (WEXITSTATUS(status_and_exitcode) & 0x00FFFFFF) |
  ...
  if (thread_setstatus(thread, flavor, (thread_state_t)state, state_count) !=
      KERN_SUCCESS) {
    goto bad;
  }
...
int sigreturn(struct proc *p, struct sigreturn_args *uap,
  ...
  if (proc_is64bit(p)) {
    ...
    if ((error = copyin(uctx64.uc_mcontext64, (void *)mctxp,
                        thread_state64[sig_xstate].mcontext_size))) {
      return error;
    }
    ...
    if ((error = copyin(CAST_USER_ADDR_T(uctx32.uc_mcontext), (void *)mctxp,
                        thread_state32[sig_xstate].mcontext_size))) {
      return error;
    }
 ...
 */
int machine_exception(int exception, mach_exception_code_t code,
