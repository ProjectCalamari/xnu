 ...
 * Outputs:	none
 */
__attribute__((noreturn)) void unix_syscall(x86_saved_state_t *state) {
  thread_t thread;
  void *vt;
  ...
  DEBUG_KPRINT_SYSCALL_UNIX("unix_syscall: code=%d(%s) eip=%u\n", code,
                            syscallnames[syscode], (uint32_t)regs->eip);
...
#error U32 syscalls on x86_64 kernel requires munging
    ...
    if (__probable(!code_is_kdebug_trace(code))) {
      uint32_t *uip = vt;
      KDBG_RELEASE(BSDDBG_CODE(DBG_BSD_EXCP_SC, code) | DBG_FUNC_START, uip[0],
                   uip[1], uip[2], uip[3]);
    }
...
#if CONFIG_MACF
  if (__improbable(proc_syscall_filter_mask(p) != NULL &&
                   !bitstr_test(proc_syscall_filter_mask(p), syscode))) {
...
skip_syscall:
  ...
  DEBUG_KPRINT_SYSCALL_UNIX("unix_syscall: error=%d retval=(%u,%u)\n", error,
                            regs->eax, regs->edx);
...
__attribute__((noreturn)) void unix_syscall64(x86_saved_state_t *state) {
  ...
  syscode = (code < nsysent) ? code : SYS_invalid;
  DEBUG_KPRINT_SYSCALL_UNIX("unix_syscall64: code=%d(%s) rip=%llx\n", code,
                            syscallnames[syscode], regs->isf.rip);
  callp = &sysent[syscode];
  ...
  if (callp->sy_narg != 0) {
    ...
    if (__improbable(callp->sy_narg > args_in_regs)) {
      ...
      error = copyin((user_addr_t)(regs->isf.rsp + sizeof(user_addr_t)),
                     (char *)&uthread->uu_arg[args_in_regs], copyin_count);
      if (error) {
        regs->rax = error;
        regs->isf.rflags |= EFL_CF;
        thread_exception_return();
...
skip_syscall:
  ...
  DEBUG_KPRINT_SYSCALL_UNIX("unix_syscall64: error=%d retval=(%llu,%llu)\n",
...
void unix_syscall_return(int error) {
  ...
  if (proc_is64bit(p)) {
    ...
    DEBUG_KPRINT_SYSCALL_UNIX("unix_syscall_return: error=%d retval=(%u,%u)\n",
  ...
  if (!code_is_kdebug_trace(code)) {
    KDBG_RELEASE(BSDDBG_CODE(DBG_BSD_EXCP_SC, code) | DBG_FUNC_END, error,
                 uthread->uu_rval[0], uthread->uu_rval[1], proc_getpid(p));
  }
