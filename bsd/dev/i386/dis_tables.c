...
const char *const dis_addr16[3][8] = {
    ...
    "(%di)",     "",          "(%bx)",     "(%bx,%si)", "(%bx,%di)",
    "(%bp,%si)", "(%bp,%di)", "(%si)",     "(%di",      "(%bp)",
    "(%bx)",     "(%bx,%si)", "(%bx,%di)", "(%bp,%si)", "(%bp,%di)",
    "(%si)",     "(%di)",     "(%bp)",     "(%bx)",
};
...
const char *const dis_CONTROLREG[16] = {"%cr0",   "%cr1",   "%cr2",   "%cr3",
                                        "%cr4",   "%cr5?",  "%cr6?",  "%cr7?",
                                        "%cr8",   "%cr9?",  "%cr10?", "%cr11?",
                                        "%cr12?", "%cr13?", "%cr14?", "%cr15?"};
...
const char *const dis_XMMREG[16] = {"%xmm0",  "%xmm1",  "%xmm2",  "%xmm3",
                                    "%xmm4",  "%xmm5",  "%xmm6",  "%xmm7",
                                    "%xmm8",  "%xmm9",  "%xmm10", "%xmm11",
                                    "%xmm12", "%xmm13", "%xmm14", "%xmm15"};

const char *const dis_YMMREG[16] = {"%ymm0",  "%ymm1",  "%ymm2",  "%ymm3",
                                    "%ymm4",  "%ymm5",  "%ymm6",  "%ymm7",
 ...
 */
const char *const dis_PREDSUFFIX[8] = {"eq",  "lt",  "le",  "unord",
                                       "neq", "nlt", "nle", "ord"};
...
static dis_gather_regs_t dis_vgather[4][2][2] = {
    {/* op 0x90, W.0 */
     {{XMM_OPND, XMM_OPND, XMM_OPND, "d"}, {YMM_OPND, YMM_OPND, YMM_OPND, "d"}},
     /* op 0x90, W.1 */
    ...
    {/* op 0x92, W.0 */
     {{XMM_OPND, XMM_OPND, XMM_OPND, "s"}, {YMM_OPND, YMM_OPND, YMM_OPND, "s"}},
     /* op 0x92, W.1 */
     {{XMM_OPND, XMM_OPND, XMM_OPND, "d"},
      {YMM_OPND, XMM_OPND, YMM_OPND, "d"}}},
    {/* op 0x93, W.0 */
     {{XMM_OPND, XMM_OPND, XMM_OPND, "s"}, {XMM_OPND, YMM_OPND, XMM_OPND, "s"}},
     /* op 0x93, W.1 */
     {{XMM_OPND, XMM_OPND, XMM_OPND, "d"},
 ...
 */
static int dtrace_get_opcode(dis86_t *x, uint_t *high, uint_t *low) {
  int byte;
 ...
 */
static void dtrace_get_SIB(dis86_t *x, uint_t *ss, uint_t *index,
                           uint_t *base) {
  int byte;
 ...
 */
static void dtrace_get_modrm(dis86_t *x, uint_t *mode, uint_t *reg,
                             uint_t *r_m) {
 ...
 */
/*ARGSUSED*/
static void dtrace_rex_adjust(uint_t rex_prefix, uint_t mode, uint_t *reg,
                              uint_t *r_m) {
#pragma unused(mode)
 ...
 */
/*ARGSUSED*/
static void dtrace_vex_adjust(uint_t vex_byte1, uint_t mode, uint_t *reg,
                              uint_t *r_m) {
#pragma unused(mode)
  if (reg != NULL && r_m == NULL) {
    if (!(vex_byte1 & VEX_B))
 ...
 */
static void dtrace_imm_opnd(dis86_t *x, int wbit, int size, int opindex) {
  int i;
  int byte;
  int valsize = 0;
  ...
  if (x->d86_bytes[x->d86_len - 1] & 0x80) {
    for (; i < (int)sizeof(uint64_t); i++)
      x->d86_opnd[opindex].d86_value |= (uint64_t)0xff << (i * 8);
  }
#ifdef DIS_TEXT
 ...
 */
static void dtrace_disp_opnd(dis86_t *x, int wbit, int size, int opindex) {
 ...
 */
/*ARGSUSED*/
static void dtrace_check_override(dis86_t *x, int opindex) {
#ifdef DIS_TEXT
  if (x->d86_seg_prefix) {
    (void)strlcat(x->d86_opnd[opindex].d86_prefix, x->d86_seg_prefix, PFIXLEN);
  }
#else
 ...
 */
/*ARGSUSED*/
static void dtrace_get_operand(dis86_t *x, uint_t mode, uint_t r_m, int wbit,
                               int opindex) {
...
#pragma unused(wbit)
  ...
  if (mode == 1)
    ...
    dispsize = 4;
  else if ((r_m & 7) == EBP_REGNO || (have_SIB && (base & 7) == EBP_REGNO))
    dispsize = 4;

  if (dispsize > 0) {
    dtrace_imm_opnd(x, dispsize == 4 ? LONG_OPND : BYTE_OPND, dispsize,
                    opindex);
    if (x->d86_error)
      return;
  }
  ...
  if (have_SIB == 0) {
    if (x->d86_mode == SIZE32) {
      if (mode == 0)
        (void)strlcat(opnd, dis_addr32_mode0[r_m], OPLEN);
      else
        (void)strlcat(opnd, dis_addr32_mode12[r_m], OPLEN);
    } else {
      if (mode == 0) {
        (void)strlcat(opnd, dis_addr64_mode0[r_m], OPLEN);
        if (r_m == 5) {
          x->d86_opnd[opindex].d86_mode = MODE_RIPREL;
        }
      } else {
        (void)strlcat(opnd, dis_addr64_mode12[r_m], OPLEN);
      }
    }
  } else {
    uint_t need_paren = 0;
 ...
 */
int dtrace_disx86(dis86_t *x, uint_t cpu_mode) {
...
#define NOMEM /* nothing */
  ...
  if (vex_prefix == VEX_2bytes) {
    ...
    switch (vex_p) {
    case VEX_p_66:
      dp = (instable_t *)&dis_opAVX660F[(opcode1 << 4) | opcode2];
      break;
    case VEX_p_F3:
      dp = (instable_t *)&dis_opAVXF30F[(opcode1 << 4) | opcode2];
      break;
    case VEX_p_F2:
      dp = (instable_t *)&dis_opAVXF20F[(opcode1 << 4) | opcode2];
      break;
    default:
    ...
    if ((vex_m != VEX_m_0F) && (vex_m != VEX_m_0F38) && (vex_m != VEX_m_0F3A))
      goto error;
    ...
    case VEX_p_66:
      if (vex_m == VEX_m_0F) {
        ...
        dp = (instable_t *)&dis_opAVX660F38[(opcode1 << 4) | opcode2];
      } else if (vex_m == VEX_m_0F3A) {
        dp = (instable_t *)&dis_opAVX660F3A[(opcode1 << 4) | opcode2];
      } else {
        goto error;
      }
      break;
    case VEX_p_F3:
      if (vex_m == VEX_m_0F) {
        ...
        dp = (instable_t *)&dis_opAVXF30F38[(opcode1 << 4) | opcode2];
      } else {
        goto error;
      }
      break;
    case VEX_p_F2:
      if (vex_m == VEX_m_0F) {
        ...
        dp = (instable_t *)&dis_opAVXF20F3A[(opcode1 << 4) | opcode2];
      } else if (vex_m == VEX_m_0F38) {
        dp = (instable_t *)&dis_opAVXF20F38[(opcode1 << 4) | opcode2];
      } else {
        goto error;
      }
      break;
  ...
  if (dp->it_indirect == (instable_t *)dis_op0F) {
    ...
    if (opcode4 == 0x7 && opcode5 >= 0x1 && opcode5 <= 0x3) {
      ...
      if (dtrace_get_opcode(x, &opcode6, &opcode7) != 0)
        goto error;
      opcode_bytes = 3;
      subcode = ((opcode6 & 0x3) << 1) | ((opcode7 & 0x8) >> 3);
      ...
      if (dp->it_indirect == (instable_t *)dis_op0F38F6) {
        dp = dp->it_indirect;
        if (opnd_size_prefix == 0 && rep_prefix == 0xf3) {
          /* It is adox */
          dp++;
        } else if (opnd_size_prefix != 0x66 && rep_prefix != 0) {
          /* It isn't adcx */
          goto error;
        }
        opnd_size_prefix = 0;
  ...
  if (dp->it_indirect != TERM) {
    ...
    if (opcode1 == 0xD && opcode2 >= 0x8) {
      if (opcode2 == 0xB && mode == 0x3 && opcode3 == 4)
        ...
        dp = (instable_t *)&dis_opFP3[opcode2 - 8][opcode3];
      else
        dp = (instable_t *)&dis_opFP1n2[opcode2 - 8][opcode3];
    } else {
  ...
  case XMMOXMM:
    ...
    off = ((uintptr_t)dp - (uintptr_t)dis_op0F) / sizeof(instable_t);
  ...
  case MG9:
    ...
    off = ((uintptr_t)dp - (uintptr_t)dis_op0FC7) / sizeof(instable_t);
  ...
  case MMOSH:
     ...
     */
    if ((uintptr_t)dp - (uintptr_t)dis_op0F7123 > sizeof(dis_op0F7123))
      goto error;

    if (opnd_size_prefix) {
      off = ((uintptr_t)dp - (uintptr_t)dis_op0F7123) / sizeof(instable_t);
  ...
  if (dp->it_adrmode != CBW && dp->it_adrmode != CWD &&
    ...
    if (dp->it_avxsuf && dp->it_suffix) {
      ...
      if (opcode_bytes == 2 && opcode4 == 4) {
        ...
        (void)strlcat(x->d86_mnem, types[opnd_size], OPLEN);
      }
    }
  }
#endif
  ...
  xmm3p:
    ...
    THREEOPERAND(x, mode, reg, r_m, rex_prefix, wbit, LONG_OPND, 1, 1);
    NOMEM;
    break;

  case XMM3PM_66r:
    THREEOPERAND(x, mode, reg, r_m, rex_prefix, LONG_OPND, XMM_OPND, 1, 0);
    break;
  ...
  case XMMOPM:
    wbit = XMM_OPND;
    THREEOPERAND(x, mode, reg, r_m, rex_prefix, wbit, XMM_OPND, 1, 1);
    ...
    if (dp->it_name[0] == 'c' && dp->it_name[1] == 'm' &&
      ...
      (void)strlcat(x->d86_mnem, dp->it_name + strlen(dp->it_name) - 2, OPLEN);
  ...
  case XMMX2I:
    FOUROPERAND(x, mode, reg, r_m, rex_prefix, XMM_OPND, XMM_OPND, 1);
    NOMEM;
    break;
  ...
  case FMA:
    ...
    if (dp->it_adrmode == FMA) {
      ...
      if (len + 1 < OPLEN) {
        (void)strncpy(x->d86_mnem + len, vex_W != 0 ? "d" : "s", OPLEN - len);
      }
    }
#endif

    if (mode != REG_ONLY) {
      if ((dp == &dis_opAVXF20F[0x10]) || (dp == &dis_opAVXF30F[0x10])) {
        /* vmovsd <m64>, <xmm> */
        /* or vmovss <m64>, <xmm> */
        x->d86_numopnds = 2;
        goto L_VEX_MX;
    ...
    if ((dp == &dis_opAVXF20F[0x2A]) || (dp == &dis_opAVXF30F[0x2A])) {
      /*
  ...
  case VEX_SbVM:
...
#ifdef DIS_TEXT
    (void)strncpy(x->d86_mnem, dp->it_name, OPLEN);
    (void)strlcat(x->d86_mnem + strlen(dp->it_name), vreg->dgr_suffix,
                  OPLEN - strlen(dp->it_name));
#endif
     ...
     * XMM/YMM regs
     */
    dtrace_get_operand(x, REG_ONLY, (0xF - vex_v), vreg->dgr_arg0, 0);
  ...
  case VEX_RRX:
    ...
    if (mode != REG_ONLY) {
      if ((dp == &dis_opAVXF20F[0x11]) || (dp == &dis_opAVXF30F[0x11])) {
        /* vmovsd <xmm>, <m64> */
        /* or vmovss <xmm>, <m64> */
        x->d86_numopnds = 2;
        goto L_VEX_RM;
  ...
  case VEX_RMRX:
    ...
    } else if ((dp == &dis_opAVX660F3A[0x20]) || (dp == &dis_opAVX660F[0xC4])) {
    ...
    if ((dp == &dis_opAVX660F3A[0x4A]) || (dp == &dis_opAVX660F3A[0x4B]) ||
...
#ifdef DIS_TEXT
      if (vex_L)
        (void)strncpy(x->d86_opnd[0].d86_opnd, dis_YMMREG[regnum], OPLEN);
      else
        (void)strncpy(x->d86_opnd[0].d86_opnd, dis_XMMREG[regnum], OPLEN);
#endif
    }
    break;
  ...
  L_VEX_MX:
    ...
    if ((dp == &dis_opAVXF20F[0xE6]) || (dp == &dis_opAVX660F[0x5A]) ||
      ...
      dtrace_get_operand(x, mode, r_m, wbit, 0);
    } else if ((dp == &dis_opAVXF30F[0xE6]) || (dp == &dis_opAVX0F[0x5][0xA]) ||
  ...
  case VEX_XXI:
    ...
    dtrace_get_modrm(x, &mode, &reg, &r_m);
#ifdef DIS_TEXT
    (void)strncpy(x->d86_mnem, dis_AVXvgrp7[opcode2 - 1][reg], OPLEN);
#endif
    dtrace_vex_adjust(vex_byte1, mode, &reg, &r_m);
  ...
  case VEX_RX:
    ...
    if (dp == &dis_opAVX660F3A[0x19] || dp == &dis_opAVX660F3A[0x1d]) {
      x->d86_numopnds = 3;
...
static char *unsigned_ops[] = {"or",    "and",  "xor", "test", "in",  "out",
                               "lcall", "ljmp", "rcr", "rcl",  "ror", "rol",
                               "shl",   "shr",  "sal", "psr",  "psl", 0};

static int isunsigned_op(char *opcode) {
  char *where;
  int i;
  int is_unsigned = 0;
...
static int print_imm(dis86_t *dis, uint64_t usv, uint64_t mask, char *buf,
...
/* ARGSUSED */
void dtrace_disx86_str(dis86_t *dis, uint_t mode, uint64_t pc, char *buf,
                       size_t buflen) {
  uint64_t reltgt = 0;
  uint64_t tgt = 0;
  int curlen;
  ...
  for (i = 0; i < dis->d86_numopnds; i++) {
    ...
    case MODE_OFFSET:
      ...
      if (print_imm(dis, usv, mask, buf, buflen, IMM, TRY_NEG) &&
          (op->d86_mode == MODE_SIGNED || op->d86_mode == MODE_IMPLIED)) {
    ...
    case MODE_RIPREL:
      ...
      (void)print_imm(dis, usv, mask, buf, buflen, DISP, TRY_NEG);
  ...
  if (tgt != 0) {
    if ((dis->d86_flags & DIS_F_NOIMMSYM) == 0 &&
      ...
      (void)strlcat(buf, "\t<", buflen);
      curlen = strlen(buf);
      lookup(dis->d86_data, tgt, buf + curlen, buflen - curlen);
      (void)strlcat(buf, ">", buflen);
    }
    ...
    if (save_usv) {
      (void)strlcat(buf, "\t<", buflen);
      (void)print_imm(dis, save_usv, save_mask, buf, buflen, IMM, POS);
      (void)strlcat(buf, ">", buflen);
    }
  }
