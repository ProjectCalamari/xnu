...
void vm_swapfile_open(const char *path, vnode_t *vp) {
  ...
  if ((error = vnode_open(path, (O_CREAT | O_TRUNC | FREAD | FWRITE),
                          S_IRUSR | S_IWUSR, 0, vp, ctx))) {
...
void vm_swapfile_close(uint64_t path_addr, vnode_t vp) {
  ...
  error =
      unlink1(context, NULLVP, CAST_USER_ADDR_T(path_addr), UIO_SYSSPACE, 0);
...
int vm_record_file_write(vnode_t vp, uint64_t offset, char *buf, int size) {
  int error = 0;
  vfs_context_t ctx;

  ctx = vfs_context_kernel();

  error = vn_rdwr(UIO_WRITE, vp, (caddr_t)buf, size, offset, UIO_SYSSPACE,
                  IO_NODELOCKED, vfs_context_ucred(ctx), (int *)0,
                  vfs_context_proc(ctx));

  return error;
}

int vm_swapfile_io(vnode_t vp, uint64_t offset, uint64_t start, int npages,
                   int flags, void *upl_iodone) {
  int error = 0;
  ...
  upl_size = io_size;
  kr = vm_map_create_upl(kernel_map, start, &upl_size, &upl, NULL, &count,
                         &upl_create_flags, VM_KERN_MEMORY_OSFMK);
  ...
  if (flags & SWAP_READ) {
    vnode_pagein(vp, upl, 0, offset, io_size,
                 upl_control_flags | UPL_IGNORE_VALID_PAGE_CHECK, &error);
    if (error) {
#if DEBUG
      printf("vm_swapfile_io: vnode_pagein failed with %d (vp: %p, offset: "
             "0x%llx, size:%u)\n",
    ...
    vnode_pageout(vp, upl, 0, offset, io_size, upl_control_flags, &error);
    if (error) {
#if DEBUG
      printf("vm_swapfile_io: vnode_pageout failed with %d (vp: %p, offset: "
             "0x%llx, size:%u)\n",
             error, vp, offset, io_size);
#else  /* DEBUG */
  ...
  error =
      vn_rdwr((flags & SWAP_READ) ? UIO_READ : UIO_WRITE, vp, (caddr_t)start,
              io_size, offset, UIO_SYSSPACE,
              IO_SYNC | IO_NODELOCKED | IO_UNIT | IO_NOCACHE | IO_SWAP_DISPATCH,
...
u_int32_t vnode_trim_list(vnode_t vp, struct trim_list *tl,
  ...
  while (tl) {
    ...
    /*
     * We may not get the entire range from tl_offset -> tl_offset+tl_length in
     * a single extent from the blockmap call.  Keep looping/going until we are
     * sure we've hit the whole range or if we encounter an error.
     */
    while (trimmed < tl->tl_length) {
      /*
       * VNOP_BLOCKMAP will tell us the logical to physical block number mapping
       * for the specified offset.  It returns blocks in contiguous chunks, so
       * if the logical range is broken into multiple extents, it must be called
      ...
      if (io_blockno != -1) {
        extents[trim_index].offset =
            (uint64_t)io_blockno * (u_int64_t)blocksize;
      ...
      if (trim_index == MAX_BATCH_TO_TRIM) {
        if (vp->v_mount->mnt_ioflags & MNT_IOFLAGS_CSUNMAP_SUPPORTED) {
          ...
          error = VNOP_IOCTL(devvp, _DKIOCCSUNMAP, (caddr_t)&cs_unmap, 0,
                             vfs_context_kernel());
        } else {
          unmap.extentsCount = trim_index;
          error = VNOP_IOCTL(devvp, DKIOCUNMAP, (caddr_t)&unmap, 0,
                             vfs_context_kernel());
        }
        if (error) {
          goto trim_exit;
...
int vm_swap_vol_get_budget(vnode_t vp, uint64_t *freeze_daily_budget) {
  ...
  if (err == 0) {
    if (vp->v_mount && vp->v_mount->mnt_devvp) {
      devvp = vp->v_mount->mnt_devvp;
      err = VNOP_IOCTL(devvp, DKIOCGETMAXSWAPWRITE,
                       (caddr_t)freeze_daily_budget, 0, ctx);
    } else {
      err = ENODEV;
    }
    vnode_put(vp);
...
int vm_swap_vol_get_capacity(const char *volume_name, uint64_t *capacity) {
  ...
  *capacity = 0;
  ...
  devvp = vp->v_mount->mnt_devvp;
  if ((error = VNOP_IOCTL(devvp, DKIOCGETBLOCKSIZE, (caddr_t)&block_size, 0,
                          ctx))) {
    printf("Unable to get swap volume block size\n");
    goto out;
  }
  if ((error = VNOP_IOCTL(devvp, DKIOCGETBLOCKCOUNT, (caddr_t)&block_count, 0,
                          ctx))) {
