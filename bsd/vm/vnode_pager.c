...
#if CONFIG_IOSCHED
void vnode_pager_issue_reprioritize_io(struct vnode *devvp, uint64_t blkno,
                                       uint32_t len, int priority) {
  u_int32_t blocksize = 0;
  dk_extent_t extent;
  dk_set_tier_t set_tier;
  int error = 0;

  error = VNOP_IOCTL(devvp, DKIOCGETBLOCKSIZE, (caddr_t)&blocksize, 0,
                     vfs_context_kernel());
  if (error) {
    return;
  }
  ...
  error = VNOP_IOCTL(devvp, DKIOCSETTIER, (caddr_t)&set_tier, 0,
                     vfs_context_kernel());
  return;
}
#endif

void vnode_pager_was_dirtied(struct vnode *vp, vm_object_offset_t s_offset,
                             vm_object_offset_t e_offset) {
  cluster_update_state(vp, s_offset, e_offset, TRUE);
}

uint32_t vnode_pager_isinuse(struct vnode *vp) {
...
uint32_t vnode_pager_return_throttle_io_limit(struct vnode *vp,
                                              uint32_t *limit) {
  return cluster_throttle_io_limit(vp, limit);
}

vm_object_offset_t vnode_pager_get_filesize(struct vnode *vp) {
...
kern_return_t vnode_pager_get_name(struct vnode *vp, char *pathname,
                                   vm_size_t pathname_len, char *filename,
                                   vm_size_t filename_len,
                                   boolean_t *truncated_path_p) {
  *truncated_path_p = FALSE;
  if (pathname != NULL) {
    /* get the path name */
    safe_getpath(vp, NULL, pathname, (int)pathname_len, truncated_path_p);
  }
  if ((pathname == NULL || *truncated_path_p) && filename != NULL) {
    /* get the file name */
    const char *name;
...
kern_return_t vnode_pager_get_mtime(struct vnode *vp,
...
kern_return_t vnode_pager_get_cs_blobs(struct vnode *vp, void **blobs) {
...
/*
 * vnode_trim:
 * Used to call the DKIOCUNMAP ioctl on the underlying disk device for the
 * specified vnode. Trims the region at offset bytes into the file, for length
 * bytes.
 *
 * Care must be taken to ensure that the vnode is sufficiently reference counted
 * at the time this function is called; no iocounts or usecounts are taken on
 * the vnode. This function is non-idempotent in error cases;  We cannot
 * un-discard the blocks if only some of them are successfully discarded.
 */
u_int32_t vnode_trim(struct vnode *vp, off_t offset, size_t length) {
  daddr64_t
      io_blockno; /* Block number corresponding to the start of the extent */
  size_t io_bytecount; /* Number of bytes in current extent for the specified
                          range */
  size_t trimmed = 0;
   ...
   * extent from the blockmap call.  Keep looping/going until we are sure we've
   * hit the whole range or if we encounter an error.
   */
  while (trimmed < length) {
    /*
     * VNOP_BLOCKMAP will tell us the logical to physical block number mapping
     * for the specified offset.  It returns blocks in contiguous chunks, so if
     * the logical range is broken into multiple extents, it must be called
     * multiple times, increasing the offset in each call to ensure that the
     * entire range is covered.
     */
    error = VNOP_BLOCKMAP(vp, current_offset, remaining_length, &io_blockno,
                          &io_bytecount, NULL,
    ...
    error =
        VNOP_IOCTL(devvp, DKIOCUNMAP, (caddr_t)&unmap, 0, vfs_context_kernel());
...
pager_return_t vnode_pageout(struct vnode *vp, upl_t upl,
                             upl_offset_t upl_offset,
                             vm_object_offset_t f_offset, upl_size_t size,
  ...
  if (!(flags & UPL_VNODE_PAGER)) {
     ...
     */
    KDBG_RELEASE(VMDBG_CODE(DBG_VM_VNODE_PAGEOUT) | DBG_FUNC_START, size, 1);
    ...
    KDBG_RELEASE(VMDBG_CODE(DBG_VM_VNODE_PAGEOUT) | DBG_FUNC_END, size, 1);
  ...
  if (upl == NULL) {
    ...
    if (vp->v_mount->mnt_vtable->vfc_vfsflags & VFC_VFSVNOP_PAGEOUTV2) {
       ...
       * backed objects... we will not grab the UPL befofe calling
       * VNOP_PAGEOUT... it is the fileystem's responsibility to grab the range
       * we're denoting via 'f_offset' and 'size' into a UPL... this allows the
       * filesystem to first take any locks it needs, before effectively locking
       * the pages into a UPL...
       */
      KDBG_RELEASE(VMDBG_CODE(DBG_VM_VNODE_PAGEOUT) | DBG_FUNC_START, size,
                   (int)f_offset);

      if ((error_ret = VNOP_PAGEOUT(vp, NULL, upl_offset, (off_t)f_offset, size,
                                    flags, ctx))) {
        result = PAGER_ERROR;
      }
      KDBG_RELEASE(VMDBG_CODE(DBG_VM_VNODE_PAGEOUT) | DBG_FUNC_END, size);
    ...
    if (ubc_create_upl_kernel(vp, f_offset, size, &upl, &pl, request_flags,
  ...
  while (isize) {
    ...
    KDBG_RELEASE(VMDBG_CODE(DBG_VM_VNODE_PAGEOUT) | DBG_FUNC_START, xsize,
                 (int)f_offset);

    if ((error = VNOP_PAGEOUT(vp, upl, offset, (off_t)f_offset, xsize, flags,
                              ctx))) {
      if (error_ret == 0) {
        error_ret = error;
      }
    ...
    KDBG_RELEASE(VMDBG_CODE(DBG_VM_VNODE_PAGEOUT) | DBG_FUNC_END, xsize, error);
...
static uint64_t ktriage_encode_v_tag_and_error(vnode_t vp, int error) {
  uint64_t tag = vp->v_tag;
  uint32_t ktriage_error = (uint32_t)error;
  return tag << 32 | ktriage_error;
}

pager_return_t vnode_pagein(struct vnode *vp, upl_t upl,
                            upl_offset_t upl_offset,
                            vm_object_offset_t f_offset, upl_size_t size,
                            int flags, int *errorp) {
  upl_page_info_t *pl;
  ...
  if (upl == (upl_t)NULL) {
    ...
    if (vp->v_mount->mnt_vtable->vfc_vfsflags & VFC_VFSVNOP_PAGEINV2) {
       ...
       * backed objects... we will not grab the UPL befofe calling
       * VNOP_PAGEIN... it is the fileystem's responsibility to grab the range
       * we're denoting via 'f_offset' and 'size' into a UPL... this allows the
       * filesystem to first take any locks it needs, before effectively locking
       * the pages into a UPL... so we pass a NULL into the filesystem instead
       * of a UPL pointer... the 'upl_offset' is used to identify the "must
       * have" page in the extent... the filesystem is free to clip the extent
       * to better fit the underlying FS blocksize if it desires as long as it
       * continues to include the "must have" page... 'f_offset' + 'upl_offset'
       * identifies that page
       */
      if ((error = VNOP_PAGEIN(vp, NULL, upl_offset, (off_t)f_offset, size,
    ...
    ubc_create_upl_kernel(vp, f_offset, size, &upl, &pl,
  ...
  while (last_pg < pages_in_upl) {
    ...
    if (last_pg > start_pg) {
      ...
      if (must_commit) {
        ubc_upl_abort_range(upl, start_pg * PAGE_SIZE, xsize,
     ...
     */
    for (start_pg = last_pg; last_pg < pages_in_upl; last_pg++) {
      if ((!ignore_valid_page_check && upl_valid_page(pl, last_pg)) ||
          !upl_page_present(pl, last_pg)) {
        break;
      }
    }
    if (last_pg > start_pg) {
      ...
      if ((error =
               VNOP_PAGEIN(vp, upl, (upl_offset_t)xoff, (off_t)f_offset + xoff,
                           xsize, flags, vfs_context_current()))) {
        /*
         * Usually this UPL will be aborted/committed by the lower cluster
         * layer.
         *
         * a)	In the case of decmpfs, however, we may return an error (EAGAIN)
         * to avoid a deadlock with another thread already inflating the file.
         *
         * b)	In the case of content protection, EPERM is a valid error and we
         * should respect it.
         *
        ...
        if (must_commit) {
          if (error == EAGAIN) {
            ubc_upl_abort_range(upl, (upl_offset_t)xoff, xsize,
                                UPL_ABORT_FREE_ON_EMPTY | UPL_ABORT_RESTART);
          }
          if (error == EPERM) {
            ubc_upl_abort_range(upl, (upl_offset_t)xoff, xsize,
                                UPL_ABORT_FREE_ON_EMPTY | UPL_ABORT_ERROR);
