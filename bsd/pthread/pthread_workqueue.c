...
static void workq_bound_thread_unpark_continue(void *uth,
                                               wait_result_t wr) __dead2;

static void
workq_bound_thread_initialize_and_unpark_continue(void *uth,
                                                  wait_result_t wr) __dead2;

static void workq_bound_thread_setup_and_run(struct uthread *uth,
                                             int setup_flags) __dead2;

static void workq_schedule_creator(proc_t p, struct workqueue *wq,
                                   workq_kern_threadreq_flags_t flags);

static bool workq_threadreq_admissible(struct workqueue *wq,
                                       struct uthread *uth,
...
static bool workq_schedule_delayed_thread_creation(struct workqueue *wq,
                                                   int flags);

static inline void workq_lock_spin(struct workqueue *wq);

static inline void workq_unlock(struct workqueue *wq);
...
static ZONE_DEFINE(workq_zone_workqueue, "workq.wq", sizeof(struct workqueue),
                   ZC_NONE);
...
static inline uint32_t wq_cooperative_queue_max_size(struct workqueue *wq) {
  return wq->wq_cooperative_queue_has_limited_max_size
             ? 1
             : wq_max_cooperative_threads;
}
...
#pragma unused(arg2)
  ...
  clock_interval_to_absolutetime_interval(v->usecs, NSEC_PER_USEC, &v->abstime);
  return 0;
}
...
SYSCTL_INT(_kern, OID_AUTO, wq_max_constrained_threads,
           CTLFLAG_RW | CTLFLAG_LOCKED, &wq_max_constrained_threads, 0, "");

static int wq_limit_cooperative_threads_for_proc SYSCTL_HANDLER_ARGS {
...
#define WQ_COOPERATIVE_POOL_SIZE_STRICT_PER_QOS -1
  ...
  if (wq != NULL) {
    ...
    if (wq->wq_reqcount > 0 || wq->wq_nthreads > 0) {
      ...
      wq->wq_cooperative_queue_has_limited_max_size =
          (input_pool_size == WQ_COOPERATIVE_POOL_SIZE_STRICT_PER_QOS);
    }
    workq_unlock(wq);
  } else {
...
SYSCTL_PROC(_kern, OID_AUTO, wq_limit_cooperative_threads,
            CTLFLAG_ANYBODY | CTLFLAG_MASKED | CTLFLAG_WR | CTLFLAG_LOCKED |
                CTLTYPE_INT,
            0, 0, wq_limit_cooperative_threads_for_proc, "I",
            "Modify the max pool size of the cooperative pool");

#pragma mark p_wqptr

#define WQPTR_IS_INITING_VALUE ((struct workqueue *)~(uintptr_t)0)

static struct workqueue *proc_get_wqptr_fast(struct proc *p) {
...
static void proc_set_wqptr(struct proc *p, struct workqueue *wq) {
...
static bool proc_init_wqptr_or_wait(struct proc *p) {
  struct workqueue *wq;
...
static inline event_t workq_parked_wait_event(struct uthread *uth) {
...
static inline wq_thactive_t _wq_thactive(struct workqueue *wq) {
  return os_atomic_load_wide(&wq->wq_thactive, relaxed);
}

static inline uint8_t _wq_bucket(thread_qos_t qos) {
...
_wq_thactive_refresh_best_constrained_req_qos(struct workqueue *wq) {
  ...
  req = priority_queue_max(&wq->wq_constrained_queue, struct workq_threadreq_s,
                           tr_entry);
...
#ifdef __LP64__
    WQ_TRACE_WQ(TRACE_wq_thactive_update, wq, (uint64_t)v, (uint64_t)(v >> 64),
                0);
#else
...
static inline wq_thactive_t _wq_thactive_inc(struct workqueue *wq,
                                             thread_qos_t qos) {
  wq_thactive_t v = _wq_thactive_offset_for_qos(qos);
  return os_atomic_add_orig(&wq->wq_thactive, v, relaxed);
}

static inline wq_thactive_t _wq_thactive_dec(struct workqueue *wq,
                                             thread_qos_t qos) {
  wq_thactive_t v = _wq_thactive_offset_for_qos(qos);
  return os_atomic_sub_orig(&wq->wq_thactive, v, relaxed);
}

static inline void _wq_thactive_move(struct workqueue *wq, thread_qos_t old_qos,
...
_wq_thactive_aggregate_downto_qos(struct workqueue *wq, wq_thactive_t v,
                                  thread_qos_t qos, uint32_t *busycount,
 ...
 * for any overrides */
static inline void
_wq_cooperative_queue_scheduled_count_dec(struct workqueue *wq,
                                          thread_qos_t qos) {
  __assert_only uint8_t old_scheduled_count =
      wq->wq_cooperative_queue_scheduled_count[_wq_bucket(qos)]--;
 ...
 * for any overrides */
static inline void
_wq_cooperative_queue_scheduled_count_inc(struct workqueue *wq,
                                          thread_qos_t qos) {
  __assert_only uint8_t old_scheduled_count =
      wq->wq_cooperative_queue_scheduled_count[_wq_bucket(qos)]++;
...
static inline uint32_t _wq_flags(struct workqueue *wq) {
  return os_atomic_load(&wq->wq_flags, relaxed);
}

static inline bool _wq_exiting(struct workqueue *wq) {
...
static bool workq_lock_is_acquired_kdp(struct workqueue *wq) {
...
static inline void workq_lock_held(struct workqueue *wq) {
  LCK_TICKET_ASSERT_OWNED(&wq->wq_lock);
}

static inline bool workq_lock_try(struct workqueue *wq) {
...
static inline thread_qos_t workq_pri_bucket(struct uu_workq_policy req) {
  return MAX(MAX(req.qos_req, req.qos_max), req.qos_override);
}

static inline thread_qos_t workq_pri_override(struct uu_workq_policy req) {
  return MAX(workq_pri_bucket(req), req.qos_bucket);
}

static inline bool workq_thread_needs_params_change(workq_threadreq_t req,
                                                    struct uthread *uth) {
...
static inline bool workq_thread_needs_priority_change(workq_threadreq_t req,
                                                      struct uthread *uth) {
 ...
 */
static void workq_thread_update_bucket(proc_t p, struct workqueue *wq,
                                       struct uthread *uth,
                                       struct uu_workq_policy old_pri,
  ...
  if ((old_bucket != new_bucket) && !workq_thread_is_permanently_bound(uth)) {
  ...
  if (wq->wq_reqcount && !workq_thread_is_permanently_bound(uth) &&
 ...
 */
static void workq_thread_reset_cpupercent(workq_threadreq_t req,
                                          struct uthread *uth) {
  assert(uth == current_uthread());
 ...
 */
static void workq_thread_reset_pri(struct workqueue *wq, struct uthread *uth,
  ...
  if (req && (req->tr_flags & WORKQ_TR_FLAG_WORKLOOP)) {
     ...
     *
     * In all other cases, we cannot safely read and borrow the reference from
     * the kqwl since it can disappear from under us at any time due to the
     * max-ing logic in kqueue_set_preadopted_thread_group.
     *
    ...
    _Atomic(struct thread_group *) *tg_loc =
        kqr_preadopt_thread_group_addr(req);
  ...
  again:
    ret = os_atomic_rmw_loop(tg_loc, old_tg, new_tg, relaxed, {
       ...
       */
      new_tg =
          (thread_group_qos_t)((uintptr_t)old_tg & KQWL_PREADOPT_TG_QOS_MASK);
    });
    ...
    if (ret) {
      ...
      os_atomic_rmw_loop(tg_loc, old_tg, new_tg, relaxed, {
        if (old_tg != thread_group_to_expect) {
           ...
           * the full dance
           */
          thread_group_deallocate_safe(
              KQWL_GET_PREADOPTED_TG(thread_group_to_set));
 ...
 */
void workq_thread_set_max_qos(struct proc *p, workq_threadreq_t kqr) {
...
static inline uint64_t workq_kill_delay_for_idle_thread(struct workqueue *wq) {
...
static inline bool workq_should_kill_idle_thread(struct workqueue *wq,
                                                 struct uthread *uth,
                                                 uint64_t now) {
...
static void workq_death_call_schedule(struct workqueue *wq, uint64_t deadline) {
  ...
  thread_call_enter_delayed_with_leeway(
      wq->wq_death_call, NULL, deadline, wq_reduce_pool_window.abstime / 10,
 ...
 */
static void workq_death_policy_evaluate(struct workqueue *wq,
                                        uint16_t decrement) {
  struct uthread *uth;
  ...
  if (now - uth->uu_save.uus_workq_park_data.idle_stamp > delay) {
    WQ_TRACE_WQ(TRACE_wq_thread_terminate | DBG_FUNC_START, wq,
                wq->wq_thidlecount, 0, 0);
    wq->wq_thdying_count++;
  ...
  workq_death_call_schedule(wq, uth->uu_save.uus_workq_park_data.idle_stamp +
                                    delay);
}
...
void workq_thread_terminate(struct proc *p, struct uthread *uth) {
  ...
  if (!workq_thread_is_permanently_bound(uth)) {
    ...
    if (uth->uu_workq_flags & UT_WORKQ_DYING) {
      WQ_TRACE_WQ(TRACE_wq_thread_terminate | DBG_FUNC_END, wq,
                  wq->wq_thidlecount, 0, 0);
...
static void workq_kill_old_threads_call(void *param0, void *param1 __unused) {
...
static struct uthread *workq_pop_idle_thread(struct workqueue *wq,
                                             uint16_t uu_flags,
                                             bool *needs_wakeup) {
  struct uthread *uth;
 ...
 */
event_t workq_thread_init_and_wq_lock(task_t task, thread_t th) {
 ...
 */
static kern_return_t workq_add_new_idle_thread(proc_t p, struct workqueue *wq,
  ...
  if (kret != KERN_SUCCESS) {
    WQ_TRACE_WQ(TRACE_wq_thread_create_failed | DBG_FUNC_NONE, wq, kret, 1, 0);
    goto out;
  }

  kret = thread_create_workq_waiting(proc_task(p), continuation, &th,
                                     is_permanently_bound);
  if (kret != KERN_SUCCESS) {
    WQ_TRACE_WQ(TRACE_wq_thread_create_failed | DBG_FUNC_NONE, wq, kret, 0, 0);
...
static inline bool workq_thread_is_overcommit(struct uthread *uth) {
  return (uth->uu_workq_flags & UT_WORKQ_OVERCOMMIT) != 0;
}

static inline bool workq_thread_is_nonovercommit(struct uthread *uth) {
  return (uth->uu_workq_flags & (UT_WORKQ_OVERCOMMIT | UT_WORKQ_COOPERATIVE)) ==
         0;
}

static inline bool workq_thread_is_cooperative(struct uthread *uth) {
...
static inline void workq_thread_set_type(struct uthread *uth, uint16_t flags) {
...
static void workq_turnstile_update_inheritor(struct workqueue *wq,
...
static void workq_push_idle_thread(proc_t p, struct workqueue *wq,
  ...
  uth->uu_workq_flags &=
      ~(UT_WORKQ_RUNNING | UT_WORKQ_OVERCOMMIT | UT_WORKQ_COOPERATIVE);
  TAILQ_REMOVE(&wq->wq_thrunlist, uth, uu_workq_entry);
  wq->wq_threads_scheduled--;
  ...
  if (cur_idle >= wq_max_constrained_threads ||
    ...
    WQ_TRACE_WQ(TRACE_wq_thread_terminate | DBG_FUNC_START, wq, cur_idle, 0, 0);
    wq->wq_thdying_count++;
    uth->uu_workq_flags |= UT_WORKQ_DYING;
    uth->uu_workq_flags &= ~UT_WORKQ_IDLE_CLEANUP;
...
static inline bool workq_tr_is_overcommit(workq_tr_flags_t tr_flags) {
  return (tr_flags & WORKQ_TR_FLAG_OVERCOMMIT) != 0;
}

static inline bool workq_tr_is_nonovercommit(workq_tr_flags_t tr_flags) {
  return (tr_flags & (WORKQ_TR_FLAG_OVERCOMMIT | WORKQ_TR_FLAG_COOPERATIVE |
                      WORKQ_TR_FLAG_PERMANENT_BIND)) == 0;
}

static inline bool workq_tr_is_cooperative(workq_tr_flags_t tr_flags) {
  return (tr_flags & WORKQ_TR_FLAG_COOPERATIVE) != 0;
}

#define workq_threadreq_is_overcommit(req)                                     \
  workq_tr_is_overcommit((req)->tr_flags)
#define workq_threadreq_is_nonovercommit(req)                                  \
  workq_tr_is_nonovercommit((req)->tr_flags)
#define workq_threadreq_is_cooperative(req)                                    \
  workq_tr_is_cooperative((req)->tr_flags)

static inline int workq_priority_for_req(workq_threadreq_t req) {
...
static uint64_t
workq_num_cooperative_threads_scheduled_to_qos_internal(struct workqueue *wq,
                                                        thread_qos_t qos) {
...
/* Calculates the number of threads scheduled >= the input QoS */
static uint64_t
workq_num_cooperative_threads_scheduled_to_qos_locked(struct workqueue *wq,
                                                      thread_qos_t qos) {
  workq_lock_held(wq);
...
workq_num_cooperative_threads_scheduled_total(struct workqueue *wq) {
  return workq_num_cooperative_threads_scheduled_to_qos_locked(
      wq, WORKQ_THREAD_QOS_MIN);
}

static bool workq_has_cooperative_thread_requests(struct workqueue *wq) {
  for (thread_qos_t qos = WORKQ_THREAD_QOS_MAX; qos >= WORKQ_THREAD_QOS_MIN;
       qos--) {
    uint8_t bucket = _wq_bucket(qos);
...
static bool _wq_cooperative_queue_refresh_best_req_qos(struct workqueue *wq) {
   ...
   *		b) Highest qos bucket with pending TRs but no scheduled threads
   * for that bucket
   *
  ...
  for (thread_qos_t qos = WORKQ_THREAD_QOS_MAX; qos >= WORKQ_THREAD_QOS_MIN;
       qos--) {
    uint8_t bucket = _wq_bucket(qos);
    uint8_t scheduled_count_for_bucket =
        wq->wq_cooperative_queue_scheduled_count[bucket];
  ...
  wq->wq_cooperative_queue_best_req_qos =
      MAX(highest_qos_with_no_scheduled, highest_qos_req_with_width);
 ...
 *		a) Quantum expires for a thread and it is part of the
 * cooperative pool b) When trying to pick a thread request for the creator
 * thread to represent. c) When a thread is trying to pick a thread request to
 * actually bind to and service.
 *
 * Called with workq lock held.
 */
...
static bool workq_cooperative_allowance(struct workqueue *wq, thread_qos_t qos,
                                        struct uthread *uth,
                                        bool may_start_timer) {
  workq_lock_held(wq);
   ...
   */
  uint64_t aggregate_down_to_qos =
      workq_num_cooperative_threads_scheduled_to_qos_locked(wq, qos);
  passed_admissions =
      (aggregate_down_to_qos < wq_cooperative_queue_max_size(wq));
  WQ_TRACE(TRACE_wq_cooperative_admission | DBG_FUNC_NONE,
           aggregate_down_to_qos, qos, passed_admissions,
           WQ_COOPERATIVE_POOL_SATURATED_UP_TO_QOS);
 ...
 */
static bool workq_threadreq_enqueue(struct workqueue *wq,
                                    workq_threadreq_t req) {
  ...
  if (workq_threadreq_is_cooperative(req)) {
    ...
    struct workq_threadreq_tailq *bucket =
        &wq->wq_cooperative_queue[_wq_bucket(req->tr_qos)];
...
static bool workq_threadreq_dequeue(struct workqueue *wq, workq_threadreq_t req,
  ...
  if (--req->tr_count == 0) {
    ...
    if (workq_threadreq_is_cooperative(req)) {
       ...
       */
      assert(_wq_bucket(req->tr_qos) ==
             _wq_bucket(wq->wq_cooperative_queue_best_req_qos));
...
static void workq_threadreq_destroy(proc_t p, workq_threadreq_t req) {
...
static inline bool workq_thread_call_prepost(struct workqueue *wq,
                                             uint32_t sched, uint32_t pend,
                                             uint32_t fail_mask) {
...
static bool workq_schedule_delayed_thread_creation(struct workqueue *wq,
  ...
  if (!workq_thread_call_prepost(
          wq, WQ_DELAYED_CALL_SCHEDULED, WQ_DELAYED_CALL_PENDED,
          WQ_IMMEDIATE_CALL_PENDED | WQ_IMMEDIATE_CALL_SCHEDULED)) {
    return false;
  }
  ...
  WQ_TRACE_WQ(TRACE_wq_start_add_timer, wq, wq->wq_reqcount, _wq_flags(wq),
...
static void workq_schedule_immediate_thread_creation(struct workqueue *wq) {
  assert(!preemption_enabled());
...
void workq_proc_resumed(struct proc *p) {
  ...
  wq_flags = os_atomic_andnot_orig(&wq->wq_flags,
                                   WQ_PROC_SUSPENDED | WQ_DELAYED_CALL_PENDED |
  ...
  if ((wq_flags & WQ_EXITING) == 0) {
    ...
    if (wq_flags & WQ_IMMEDIATE_CALL_PENDED) {
      ...
      workq_schedule_delayed_thread_creation(
          wq, WORKQ_SCHEDULE_DELAYED_THREAD_CREATION_RESTART);
    }
    enable_preemption();
  }
}
 ...
 */
static bool workq_thread_is_busy(uint64_t now,
                                 _Atomic uint64_t *lastblocked_tsp) {
...
static void workq_add_new_threads_call(void *_p, void *flags) {
  proc_t p = _p;
  struct workqueue *wq = proc_get_wqptr(p);
  ...
  WQ_TRACE_WQ(TRACE_wq_add_timer | DBG_FUNC_END, wq, 0, wq->wq_nthreads,
              wq->wq_thidlecount);
}

#pragma mark thread state tracking

static void workq_sched_callback(int type, thread_t thread) {
  ...
  case SCHED_CALL_BLOCK:
    ...
    if (req_qos == THREAD_QOS_UNSPECIFIED) {
      ...
      old_req_count = _wq_thactive_aggregate_downto_qos(
          wq, old_thactive, req_qos, NULL, &max_busycount);
      /*
    ...
    if (__improbable(kdebug_enable)) {
      __unused uint32_t old =
          _wq_thactive_aggregate_downto_qos(wq, old_thactive, qos, NULL, NULL);
      WQ_TRACE_WQ(TRACE_wq_thread_block | DBG_FUNC_START, wq, old - 1,
                  qos | (req_qos << 8), wq->wq_reqcount << 1 | start_timer);
    }
    break;
  ...
  case SCHED_CALL_UNBLOCK:
    ...
    if (__improbable(kdebug_enable)) {
      __unused uint32_t old =
          _wq_thactive_aggregate_downto_qos(wq, old_thactive, qos, NULL, NULL);
      req_qos = WQ_THACTIVE_BEST_CONSTRAINED_REQ_QOS(old_thactive);
      WQ_TRACE_WQ(TRACE_wq_thread_block | DBG_FUNC_END, wq, old + 1,
                  qos | (req_qos << 8), wq->wq_threads_scheduled);
    }
    break;
  }
}
...
void workq_reference(struct workqueue *wq) { os_ref_retain(&wq->wq_refcnt); }
...
static void workq_deallocate(struct workqueue *wq) {
 ...
 */
int workq_open(struct proc *p, __unused struct workq_open_args *uap,
  ...
  if (wq_init_constrained_limit) {
    ...
    for (thread_qos_t qos = WORKQ_THREAD_QOS_MIN; qos <= WORKQ_THREAD_QOS_MAX;
         qos++) {
  ...
  if (proc_get_wqptr(p) == NULL) {
    ...
    thread_qos_t mgr_priority_hint =
        task_get_default_manager_qos(current_task());
    pthread_priority_t pp =
        _pthread_priority_make_from_thread_qos(mgr_priority_hint, 0, 0);
    ...
    WQ_TRACE_WQ(TRACE_wq_create | DBG_FUNC_NONE, wq, VM_KERNEL_ADDRHIDE(wq), 0,
                0);
    proc_set_wqptr(p, wq);
  }
out:
 ...
 *
 * Function:	Mark the work queue such that new threads will not be added to
 * the work queue after we return.
 *
...
void workq_mark_exiting(struct proc *p) {
   ...
   */
  priority_queue_destroy(&wq->wq_overcommit_queue, struct workq_threadreq_s,
                         tr_entry, ^(workq_threadreq_t e) {
                           workq_threadreq_destroy(p, e);
                         });
  priority_queue_destroy(&wq->wq_constrained_queue, struct workq_threadreq_s,
                         tr_entry, ^(workq_threadreq_t e) {
                           workq_threadreq_destroy(p, e);
                         });
  priority_queue_destroy(&wq->wq_special_queue, struct workq_threadreq_s,
                         tr_entry, ^(workq_threadreq_t e) {
                           workq_threadreq_destroy(p, e);
                         });
 ...
 * Routine:	workq_exit
 *
 * Function:	clean up the work queue structure(s) now that there are no
 * threads left running inside the work queue (except possibly current_thread).
 *
...
void workq_exit(struct proc *p) {
  ...
  if (wq != NULL) {
    ...
    WQ_TRACE_WQ(TRACE_wq_destroy | DBG_FUNC_END, wq, VM_KERNEL_ADDRHIDE(wq), 0,
                0);
...
bool bsdthread_part_of_cooperative_workqueue(struct uthread *uth) {
  return (workq_thread_is_cooperative(uth) ||
          workq_thread_is_nonovercommit(uth)) &&
         (uth->uu_workq_pri.qos_bucket != WORKQ_THREAD_QOS_MANAGER) &&
         (!workq_thread_is_permanently_bound(uth));
}

static bool _pthread_priority_to_policy(pthread_priority_t priority,
...
static int bsdthread_set_self(proc_t p, thread_t th,
...
qos:
  if (flags & (WORKQ_SET_SELF_QOS_FLAG | WORKQ_SET_SELF_QOS_OVERRIDE_FLAG)) {
    ...
    if (!is_wq_thread) {
      ...
      /* overcommit -> non-overcommit */
      if (workq_thread_is_overcommit(uth) &&
          _pthread_priority_is_nonovercommit(priority)) {
        workq_thread_set_type(uth, 0);
        wq->wq_constrained_threads_scheduled++;

        /* non-overcommit -> overcommit */
      } else if (workq_thread_is_nonovercommit(uth) &&
                 _pthread_priority_is_overcommit(priority)) {
        workq_thread_set_type(uth, UT_WORKQ_OVERCOMMIT);
        force_run = (wq->wq_constrained_threads_scheduled-- ==
...
static int bsdthread_add_explicit_override(proc_t p, mach_port_name_t kport,
                                           pthread_priority_t pp,
  ...
  thread_t th = port_name_to_thread(kport, PORT_INTRANS_THREAD_IN_CURRENT_TASK);
  if (th == THREAD_NULL) {
    return ESRCH;
  }

  int rv = proc_thread_qos_add_override(
      proc_task(p), th, 0, qos, TRUE, resource,
...
static int bsdthread_remove_explicit_override(proc_t p, mach_port_name_t kport,
                                              user_addr_t resource) {
  thread_t th = port_name_to_thread(kport, PORT_INTRANS_THREAD_IN_CURRENT_TASK);
  if (th == THREAD_NULL) {
    return ESRCH;
  ...
  int rv = proc_thread_qos_remove_override(
      proc_task(p), th, 0, resource,
      THREAD_QOS_OVERRIDE_TYPE_PTHREAD_EXPLICIT_OVERRIDE);
...
static int workq_thread_add_dispatch_override(proc_t p, mach_port_name_t kport,
                                              pthread_priority_t pp,
  ...
  thread_t thread =
      port_name_to_thread(kport, PORT_INTRANS_THREAD_IN_CURRENT_TASK);
  ...
  WQ_TRACE_WQ(TRACE_wq_override_dispatch | DBG_FUNC_NONE, wq,
              thread_tid(thread), 1, pp);
...
static int workq_thread_reset_dispatch_override(proc_t p, thread_t thread) {
  struct uu_workq_policy old_pri, new_pri;
  struct workqueue *wq = proc_get_wqptr(p);
  struct uthread *uth = get_bsdthread_info(thread);
...
static int workq_thread_allow_kill(__unused proc_t p, thread_t thread,
                                   bool enable) {
...
static int workq_allow_sigmask(proc_t p, sigset_t mask) {
...
static int bsdthread_get_max_parallelism(thread_qos_t qos, unsigned long flags,
                                         int *retval) {
  static_assert(QOS_PARALLELISM_COUNT_LOGICAL ==
                    _PTHREAD_QOS_PARALLELISM_COUNT_LOGICAL,
                "logical");
  static_assert(QOS_PARALLELISM_REALTIME == _PTHREAD_QOS_PARALLELISM_REALTIME,
                "realtime");
  static_assert(QOS_PARALLELISM_CLUSTER_SHARED_RESOURCE ==
                    _PTHREAD_QOS_PARALLELISM_CLUSTER_SHARED_RSRC,
...
static int bsdthread_dispatch_apply_attr(__unused struct proc *p,
                                         thread_t thread, unsigned long flags,
                                         uint64_t value1,
                                         __unused uint64_t value2) {
  ...
  case _PTHREAD_DISPATCH_APPLY_ATTR_CLUSTER_SHARED_RSRC_SET:
    ...
    kr = thread_shared_rsrc_policy_set(thread, apply_worker_index,
                                       ...
                                       SHARED_RSRC_POLICY_AGENT_DISPATCH);
    /*
    ...
    return (kr == KERN_SUCCESS)
               ? 0
               : ((kr == KERN_INVALID_POLICY) ? ENOTSUP : EINVAL);
  case _PTHREAD_DISPATCH_APPLY_ATTR_CLUSTER_SHARED_RSRC_CLEAR:
    kr = thread_shared_rsrc_policy_clear(thread, CLUSTER_SHARED_RSRC_TYPE_RR,
                                         SHARED_RSRC_POLICY_AGENT_DISPATCH);
...
int bsdthread_ctl(struct proc *p, struct bsdthread_ctl_args *uap, int *retval) {
  ...
  case BSDTHREAD_CTL_SET_SELF:
    return bsdthread_set_self(
        p, current_thread(), (pthread_priority_t)uap->arg1,
        (mach_port_name_t)uap->arg2, (enum workq_set_self_flags)uap->arg3);
  ...
  case BSDTHREAD_CTL_DISPATCH_APPLY_ATTR:
    return bsdthread_dispatch_apply_attr(
        p, current_thread(), (unsigned long)uap->arg1, (uint64_t)uap->arg2,
        (uint64_t)uap->arg3);
...
static void __dead2 workq_unpark_select_threadreq_or_park_and_unlock(
    proc_t p, struct workqueue *wq, struct uthread *uth, uint32_t setup_flags);

static void __dead2 workq_select_threadreq_or_park_and_unlock(
    proc_t p, struct workqueue *wq, struct uthread *uth, uint32_t setup_flags);

static void workq_setup_and_run(proc_t p, struct uthread *uth,
                                int flags) __dead2;

#if KDEBUG_LEVEL >= KDEBUG_LEVEL_STANDARD
static inline uint64_t workq_trace_req_id(workq_threadreq_t req) {
 ...
 */
static int workq_reqthreads(struct proc *p, uint32_t reqcount,
                            pthread_priority_t pp, bool cooperative) {
  thread_qos_t qos = _pthread_priority_thread_qos(pp);
  struct workqueue *wq = proc_get_wqptr(p);
  ...
  WQ_TRACE_WQ(TRACE_wq_wqops_reqthreads | DBG_FUNC_NONE, wq, reqcount, pp,
              cooperative);
  ...
  if (workq_tr_is_cooperative(tr_flags) && workq_tr_is_overcommit(tr_flags)) {
    ret = EINVAL;
    goto free_and_exit;
  }
  ...
  WQ_TRACE_WQ(TRACE_wq_thread_request_initiate | DBG_FUNC_NONE, wq,
              workq_trace_req_id(req), req->tr_qos, reqcount);
  ...
  do {
    ...
    while (unpaced > 0 && wq->wq_thidlecount) {
      ...
      reqcount--;
           ...
           (workq_add_new_idle_thread(p, wq, workq_unpark_continue, false,
                                      NULL) == KERN_SUCCESS));
...
#if CONFIG_PREADOPT_TG
    reevaluate_creator_thread_group =
...
void workq_kern_threadreq_modify(struct proc *p, workq_threadreq_t req,
   ...
   * - make its state WORKQ_TR_STATE_NEW (so that
   * workq_threadreq_bind_and_unlock properly attempts a synchronous bind)
   */
  wq->wq_reqcount--;
  req->tr_state = WORKQ_TR_STATE_NEW;
...
void workq_kern_bound_thread_reset_pri(workq_threadreq_t req,
                                       struct uthread *uth) {
...
void workq_kern_threadreq_update_inheritor(struct proc *p,
                                           workq_threadreq_t req,
                                           thread_t owner,
                                           struct turnstile *wl_ts,
                                           turnstile_update_flags_t flags) {
  struct workqueue *wq = proc_get_wqptr_fast(p);
  turnstile_inheritor_t inheritor;
 ...
 */
kern_return_t
workq_kern_threadreq_permanent_bind(struct proc *p,
                                    struct workq_threadreq_s *kqr) {
  kern_return_t ret = 0;
  ...
  if (wq->wq_nthreads >= wq_max_threads) {
    ...
    ret = workq_add_new_idle_thread(
        p, wq, workq_bound_thread_initialize_and_unpark_continue, true,
        &new_thread);
    if (ret == KERN_SUCCESS) {
...
workq_kern_bound_thread_park(struct workq_threadreq_s *kqr) {
  ...
  kqworkloop_bound_thread_park_commit(kqr, workq_parked_wait_event(uth),
                                      workq_bound_thread_unpark_continue);
...
workq_kern_bound_thread_terminate(struct workq_threadreq_s *kqr) {
  ...
  upcall_flags |=
      uth->uu_save.uus_workq_park_data.qos | WQ_FLAG_THREAD_PRIO_QOS;
  ...
  pthread_functions->workq_setup_thread(p, th, vmap, uth->uu_workq_stackaddr,
                                        uth->uu_workq_thport, 0,
                                        WQ_SETUP_EXIT_THREAD, upcall_flags);
  __builtin_unreachable();
}

void workq_kern_threadreq_redrive(struct proc *p,
                                  workq_kern_threadreq_flags_t flags) {
  struct workqueue *wq = proc_get_wqptr_fast(p);
 ...
 * expires again.
 */
void workq_kern_quantum_expiry_reevaluate(proc_t proc, thread_t thread) {
  struct uthread *uth = get_bsdthread_info(thread);
  ...
  if (!thread_supports_cooperative_workqueue(thread)) {
    panic("Quantum expired for thread that doesn't support cooperative "
          "workqueue");
  }
...
void workq_schedule_creator_turnstile_redrive(struct workqueue *wq,
                                              bool locked) {
  if (locked) {
...
static int workq_thread_return(struct proc *p,
                               struct workq_kernreturn_args *uap,
                               struct workqueue *wq) {
  thread_t th = current_thread();
  struct uthread *uth = get_bsdthread_info(th);
  workq_threadreq_t kqr = uth->uu_kqr_bound;
...
handle_stack_events:
  ...
  if (kqr) {
    ...
    if (uth->uu_workq_pri.qos_bucket == WORKQ_THREAD_QOS_MANAGER) {
      ...
      if (uth->uu_workq_flags & UT_WORKQ_OUTSIDE_QOS) {
        ...
        upcall_flags |= uth->uu_workq_pri.qos_req | WQ_FLAG_THREAD_PRIO_QOS;
      }
    }
    error = pthread_functions->workq_handle_stack_events(
        p, th, get_task_map(proc_task(p)), uth->uu_workq_stackaddr,
  ...
  UU_KEVENT_HISTORY_WRITE_ENTRY(uth, {
                                         .uu_error = -1,
                                     });
#endif
  ...
  uth->uu_save.uus_workq_park_data.workloop_params = trp.trp_value;
  workq_select_threadreq_or_park_and_unlock(p, wq, uth, WQ_SETUP_CLEAR_VOUCHER);
  __builtin_unreachable();
}
 ...
 */
int workq_kernreturn(struct proc *p, struct workq_kernreturn_args *uap,
                     int32_t *retval) {
  ...
  case WQOPS_QUEUE_NEWSPISUPP: {
    ...
    int offset = arg2;
    if (arg3 & 0x01) {
      // If we get here, then userspace has indicated support for kevent
      // delivery.
    }
  ...
  case WQOPS_SET_EVENT_MANAGER_PRIORITY: {
    ...
    if (_pthread_priority_has_sched_pri(pri)) {
      pri &=
          (_PTHREAD_PRIORITY_SCHED_PRI_MASK | _PTHREAD_PRIORITY_SCHED_PRI_FLAG);
    } else {
  ...
  case WQOPS_SHOULD_NARROW: {
    ...
    workq_lock_spin(wq);
    bool should_narrow =
        !workq_constrained_allowance(wq, qos, uth, false, false);
    workq_unlock(wq);
...
workq_park_and_unlock(proc_t p, struct workqueue *wq, struct uthread *uth,
   ...
   *		preadoption thread group on creator threads but if it never
   * picked up a thread request and didn't go to userspace, then the thread will
   *		park with a preadoption thread group but no explicitly adopted
   *		voucher or work interval.
   *
   *		We drop the preadoption thread group here before proceeding to
   * park. Note - we may get preempted when we drop the workq lock below.
   *
   * Case 2:
   *		Thread picked up a thread request and bound to it and returned
   * back from userspace and is parking. At this point, preadoption thread group
   * should be NULL since the thread has unbound from the thread request. So
  ...
  if ((uth->uu_workq_flags & UT_WORKQ_IDLE_CLEANUP) &&
    ...
    if (!uth->uu_save.uus_workq_park_data.has_stack) {
      pthread_functions->workq_markfree_threadstack(p, get_machthread(uth),
                                                    get_task_map(proc_task(p)),
  ...
  if (uth->uu_workq_flags & UT_WORKQ_DYING) {
    workq_unpark_for_death_and_unlock(
        p, wq, uth, WORKQ_UNPARK_FOR_DEATH_WAS_IDLE, setup_flags);
    __builtin_unreachable();
  }
...
static inline bool workq_may_start_event_mgr_thread(struct workqueue *wq,
                                                    struct uthread *uth) {
  /*
 ...
 */
static workq_threadreq_t workq_cooperative_queue_best_req(struct workqueue *wq,
                                                          struct uthread *uth) {
  workq_lock_held(wq);
...
workq_threadreq_select_for_creator(struct workqueue *wq) {
  ...
  req_pri = priority_queue_max(&wq->wq_special_queue, struct workq_threadreq_s,
                               tr_entry);
  if (req_pri) {
  ...
  if (req_mgr && workq_may_start_event_mgr_thread(wq, NULL)) {
    ...
    if (mgr_pri & _PTHREAD_PRIORITY_SCHED_PRI_FLAG) {
      ...
      mgr_pri = thread_workq_pri_for_qos(_pthread_priority_thread_qos(mgr_pri));
    }
 ...
 * cooperative pool
 */
static bool workq_adjust_cooperative_constrained_schedule_counts(
    struct workqueue *wq, struct uthread *uth, thread_qos_t old_thread_qos,
   ...
   *
   *					overcommit		non-overcommit
   *	cooperative overcommit			X
   *	case 1				case 2 cooperative		case 3
   *			case 4				case 5 non-overcommit
   *	case 6					X
   *	case 7
   *
...
static workq_threadreq_t workq_threadreq_select(struct workqueue *wq,
                                                struct uthread *uth) {
  workq_threadreq_t req_qos, req_pri, req_tmp, req_mgr;
  ...
  if (pri) {
    ...
    if (req_pri->tr_state != WORKQ_TR_STATE_QUEUED) {
      panic("Invalid thread request (%p) state %d", req_pri, req_pri->tr_state);
    }
  } else {
    req_pri = NULL;
  }

  req_tmp = priority_queue_max(&wq->wq_special_queue, struct workq_threadreq_s,
                               tr_entry);
...
again:
  ...
  if (uth) {
    ...
    if (workq_thread_needs_priority_change(req, uth)) {
      WQ_TRACE_WQ(TRACE_wq_creator_select | DBG_FUNC_NONE, wq, 1,
                  uthread_tid(uth), req->tr_qos);
      workq_thread_reset_pri(wq, uth, req, /*unpark*/ true);
    }
    ...
    if (__improbable(wq->wq_nthreads >= wq_max_threads)) {
      ...
      /* This can drop the workqueue lock, and take it again */
      workq_schedule_immediate_thread_creation(wq);
    } else if ((workq_add_new_idle_thread(p, wq, workq_unpark_continue, false,
                                          NULL) == KERN_SUCCESS)) {
      goto again;
    } else {
      workq_schedule_delayed_thread_creation(wq, 0);
...
workq_select_threadreq_or_park_and_unlock(proc_t p, struct workqueue *wq,
  ...
  if (req_ts) {
    workq_perform_turnstile_operation_locked(wq, ^{
      turnstile_update_inheritor(req_ts, get_machthread(uth),
                                 TURNSTILE_IMMEDIATE_UPDATE |
                                     TURNSTILE_INHERITOR_THREAD);
      turnstile_update_inheritor_complete(req_ts, TURNSTILE_INTERLOCK_HELD);
    });
  }
   ...
   */
  bool cooperative_sched_count_changed =
      workq_adjust_cooperative_constrained_schedule_counts(
          wq, uth, old_pri.qos_req, tr_flags);
  ...
  if (__improbable(thread_unfreeze_base_pri(get_machthread(uth)) &&
                   !is_creator)) {
    if (req_ts) {
      workq_perform_turnstile_operation_locked(wq, ^{
  ...
  wq->wq_fulfilled++;
  schedule_creator =
      workq_threadreq_dequeue(wq, req, cooperative_sched_count_changed);
...
static bool workq_creator_should_yield(struct workqueue *wq,
                                       struct uthread *uth) {
  thread_qos_t qos = workq_pri_override(uth->uu_workq_pri);
  ...
  if (wq->wq_fulfilled - snapshot > conc) {
    ...
    WQ_TRACE_WQ(TRACE_wq_creator_yield, wq, 1, wq->wq_fulfilled, snapshot);
    return true;
  }
  ...
  if (conc <= cnt) {
    ...
    WQ_TRACE_WQ(TRACE_wq_creator_yield, wq, 2, wq->wq_fulfilled, snapshot);
    return true;
  }
...
workq_unpark_continue(void *parameter __unused, wait_result_t wr __unused) {
  ...
  workq_unpark_for_death_and_unlock(p, wq, uth, WORKQ_UNPARK_FOR_DEATH_WAS_IDLE,
                                    WQ_SETUP_NONE);
  __builtin_unreachable();
}
...
workq_setup_and_run(proc_t p, struct uthread *uth, int setup_flags) {
  ...
  if (uth->uu_workq_thport == MACH_PORT_NULL) {
    ...
    uth->uu_workq_thport = ipc_port_copyout_send_pinned(
        convert_thread_to_port_immovable(th), get_task_ipcspace(proc_task(p)));
  }
   ...
   */
  WQ_TRACE_WQ(TRACE_wq_runthread | DBG_FUNC_START, proc_get_wqptr_fast(p), 0, 0,
              0);
  ...
  pthread_functions->workq_setup_thread(p, th, vmap, uth->uu_workq_stackaddr,
                                        uth->uu_workq_thport, 0, setup_flags,
                                        upcall_flags);
...
workq_bound_thread_setup_and_run(struct uthread *uth, int setup_flags) {
  ...
  uint32_t upcall_flags =
      (WQ_FLAG_THREAD_NEWSPI | WQ_FLAG_THREAD_WORKLOOP | WQ_FLAG_THREAD_KEVENT);
...
int fill_procworkqueue(proc_t p, struct proc_workqueueinfo *pwqinfo) {
  ...
  activecount = _wq_thactive_aggregate_downto_qos(wq, act, WORKQ_THREAD_QOS_MIN,
                                                  NULL, NULL);
...
boolean_t workqueue_get_pwq_exceeded(void *v, boolean_t *exceeded_total,
...
uint64_t workqueue_get_task_ss_flags_from_pwq_state_kdp(void *v) {
  ...
  uint64_t total_cooperative_threads;
  total_cooperative_threads =
      workq_num_cooperative_threads_scheduled_to_qos_internal(
          wq, WORKQ_THREAD_QOS_MIN);
  if ((total_cooperative_threads == wq_cooperative_queue_max_size(wq)) &&
      workq_has_cooperative_thread_requests(wq)) {
