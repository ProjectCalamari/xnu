...
#define WQOPS_QUEUE_NEWSPISUPP                                                 \
  0x010 /* this is to check for newer SPI support */
#define WQOPS_QUEUE_REQTHREADS 0x020 /* request number of threads of a prio */
#define WQOPS_QUEUE_REQTHREADS2                                                \
  0x030 /* request a number of threads in a given priority bucket */
#define WQOPS_THREAD_KEVENT_RETURN                                             \
  0x040 /* parks the thread after delivering the passed kevent array */
#define WQOPS_SET_EVENT_MANAGER_PRIORITY                                       \
  0x080 /* max() in the provided priority in the the priority of the event     \
           manager */
#define WQOPS_THREAD_WORKLOOP_RETURN                                           \
  0x100 /* parks the thread after delivering the passed kevent array */
#define WQOPS_SHOULD_NARROW                                                    \
  0x200 /* checks whether we should narrow our concurrency */
#define WQOPS_SETUP_DISPATCH                                                   \
  0x400 /* setup pthread workqueue-related operations */
...
#define WQ_FLAG_THREAD_OVERCOMMIT                                              \
  0x00010000                             /* thread is with overcommit prio */
...
#define WQ_FLAG_THREAD_TSD_BASE_SET                                            \
  0x00200000                               /* tsd base has already been set */
#define WQ_FLAG_THREAD_WORKLOOP 0x00400000 /* workloop thread */
#define WQ_FLAG_THREAD_OUTSIDEQOS                                              \
  0x00800000 /* thread qos changes should not be sent to kernel */
#define WQ_FLAG_THREAD_COOPERATIVE                                             \
  0x01000000 /* thread is part of cooperative thread pool */
...
int __workq_open(void);

int __workq_kernreturn(int op, void *arg2, int arg3, int arg4);

int __kqueue_workloop_ctl(uintptr_t cmd, uint64_t options, void *addr,
                          size_t sz);
