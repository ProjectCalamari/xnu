...
/* These defines come from kern/thread.h but are XNU_KERNEL_PRIVATE so do not
 * get exported to kernel extensions.
 */
...
#define UT_WORKQ_OUTSIDE_QOS                                                   \
  0x10 /* Thread should avoid send QoS changes to kernel */
...
#define UT_WORKQ_WORK_INTERVAL_JOINED                                          \
  0x400 /* Thread has joined a work interval */
#define UT_WORKQ_WORK_INTERVAL_FAILED                                          \
  0x800 /* Thread has failed to join a work interval */
...
typedef union workq_threadreq_param_s {
  struct {
    ...
    uint8_t trp_pri;
    uint8_t trp_pol;
    uint32_t trp_cpupercent : 8, trp_refillms : 24;
  };
  uint64_t trp_value;
...
struct workqueue {
  ...
  uint16_t wq_cooperative_queue_best_req_qos
      : 3, /* UN means no request, returns BG for BG/MT bucket */
      wq_cooperative_queue_has_limited_max_size
      : 1, /* if set, max size of cooperative pool per QoS is 1 */
      wq_exceeded_active_constrained_thread_limit : 1, unused : 11;
...
bool workq_kern_threadreq_initiate(struct proc *p,
                                   struct workq_threadreq_s *kqr,
                                   struct turnstile *ts, thread_qos_t qos,
...
void workq_kern_threadreq_update_inheritor(struct proc *p,
                                           struct workq_threadreq_s *kqr,
                                           thread_t owner, struct turnstile *ts,
