...
__attribute__((noreturn)) static void _sleep_continue(__unused void *parameter,
  ...
  case THREAD_INTERRUPTED:
    if (catch) {
      ...
      } else if ((ut->uu_flag & (UT_CANCELDISABLE | UT_CANCEL | UT_CANCELED)) ==
                 UT_CANCEL) {
...
static int _sleep(caddr_t chan, int pri, const char *wmsg, u_int64_t abstime,
                  int (*continuation)(int), lck_mtx_t *mtx) {
  struct proc *p;
  thread_t self = current_thread();
  struct uthread *ut;
  int sig, catch;
...
int msleep0(void *chan, lck_mtx_t *mtx, int pri, const char *wmsg, int timo,
...
int msleep(void *chan, lck_mtx_t *mtx, int pri, const char *wmsg,
           struct timespec *ts) {
  u_int64_t abstime = 0;
...
int msleep1(void *chan, lck_mtx_t *mtx, int pri, const char *wmsg,
            u_int64_t abstime) {
  return _sleep((caddr_t)chan, pri, wmsg, abstime, (int (*)(int))0, mtx);
}
...
int tsleep0(void *chan, int pri, const char *wmsg, int timo,
            int (*continuation)(int)) {
  u_int64_t abstime = 0;
...
int tsleep1(void *chan, int pri, const char *wmsg, u_int64_t abstime,
            int (*continuation)(int)) {
  return _sleep((caddr_t)chan, pri, wmsg, abstime, continuation,
                (lck_mtx_t *)0);
}
 ...
 */
void wakeup(void *chan) { thread_wakeup((caddr_t)chan); }
 ...
 */
void wakeup_one(caddr_t chan) { thread_wakeup_one((caddr_t)chan); }
 ...
 */
void resetpriority(struct proc *p) {
...
void compute_averunnable(void *arg) {
  ...
  for (i = 0; i < 3; i++) {
    avg->ldavg[i] =
        (cexp[i] * avg->ldavg[i] + nrun * FSCALE * (FSCALE - cexp[i])) >>
        FSHIFT;
  }
}
