...
static TAILQ_HEAD(, kauth_listener)
    kauth_dangling_listeners = TAILQ_HEAD_INITIALIZER(kauth_dangling_listeners);
...
static TAILQ_HEAD(, kauth_scope)
    kauth_scopes = TAILQ_HEAD_INITIALIZER(kauth_scopes);

static int kauth_add_callback_to_scope(kauth_scope_t sp, kauth_listener_t klp);
static void kauth_scope_init(void);
static kauth_scope_t kauth_alloc_scope(const char *identifier,
                                       kauth_scope_callback_t callback,
                                       void *idata);
static kauth_listener_t kauth_alloc_listener(const char *identifier,
                                             kauth_scope_callback_t callback,
                                             void *idata);
#if 0
...
static int kauth_authorize_process_callback(kauth_cred_t _credential,
                                            void *_idata,
...
static int kauth_authorize_generic_callback(kauth_cred_t _credential,
                                            void *_idata,
                                            kauth_action_t _action,
                                            uintptr_t arg0, uintptr_t arg1,
...
static void kauth_scope_init(void) {
  kauth_scope_process = kauth_register_scope(
      KAUTH_SCOPE_PROCESS, kauth_authorize_process_callback, NULL);
  kauth_scope_generic = kauth_register_scope(
      KAUTH_SCOPE_GENERIC, kauth_authorize_generic_callback, NULL);
...
kauth_scope_t kauth_register_scope(const char *identifier,
                                   kauth_scope_callback_t callback,
                                   void *idata) {
...
void kauth_deregister_scope(kauth_scope_t scope) {
  ...
  for (i = 0; i < KAUTH_SCOPE_MAX_LISTENERS; i++) {
    if (scope->ks_listeners[i].kll_listenerp != NULL) {
      TAILQ_INSERT_TAIL(&kauth_dangling_listeners,
                        scope->ks_listeners[i].kll_listenerp, kl_link);
...
kauth_listener_t kauth_listen_scope(const char *identifier,
                                    kauth_scope_callback_t callback,
                                    void *idata) {
  kauth_listener_t klp;
  kauth_scope_t sp;
  ...
  TAILQ_FOREACH(sp, &kauth_scopes, ks_link) {
    if (strncmp(sp->ks_identifier, identifier, strlen(sp->ks_identifier) + 1) ==
        0) {
 ...
 */
int kauth_authorize_action(kauth_scope_t scope, kauth_cred_t credential,
  ...
  if ((scope->ks_flags & KS_F_HAS_LISTENERS) != 0) {
    for (i = 0; i < KAUTH_SCOPE_MAX_LISTENERS; i++) {
      /* XXX - kauth_todo - there is a race here if listener is removed - we
       * will fix this post Tiger. Until the race is fixed our kext clients are
       * responsible for all active requests that may be in their callbacks or
       * on the way to their callbacks before they free kl_callback or kl_idata.
       * We keep copies of these in our kauth_local_listener in an attempt to
       * limit our expose to unlisten race.
       */
      if (scope->ks_listeners[i].kll_listenerp == NULL ||
      ...
      ret = scope->ks_listeners[i].kll_callback(
          credential, scope->ks_listeners[i].kll_idata, action, arg0, arg1,
          arg2, arg3);
      if ((ret == KAUTH_RESULT_DENY) || (result == KAUTH_RESULT_DEFER)) {
        result = ret;
      }
    }
  }
 ...
 */
int kauth_authorize_allow(__unused kauth_cred_t credential,
                          __unused void *idata, __unused kauth_action_t action,
                          __unused uintptr_t arg0, __unused uintptr_t arg1,
                          __unused uintptr_t arg2, __unused uintptr_t arg3) {
...
int kauth_authorize_process(kauth_cred_t credential, kauth_action_t action,
                            struct proc *process, uintptr_t arg1,
                            uintptr_t arg2, uintptr_t arg3) {
  return kauth_authorize_action(kauth_scope_process, credential, action,
                                (uintptr_t)process, arg1, arg2, arg3);
}

static int kauth_authorize_process_callback(kauth_cred_t credential,
                                            __unused void *idata,
...
/*
 * File system operation authorization scope.  This is really only a
 * notification of the file system operation, not an authorization check.  Thus
 * the result is not relevant. arguments passed to KAUTH_FILEOP_OPEN listeners
...
int kauth_authorize_fileop_has_listeners(void) {
  /*
...
int kauth_authorize_fileop(kauth_cred_t credential, kauth_action_t action,
                           uintptr_t arg0, uintptr_t arg1) {
  char *namep = NULL;
  int name_len;
  uintptr_t arg2 = 0;
  ...
  if (action == KAUTH_FILEOP_OPEN || action == KAUTH_FILEOP_CLOSE ||
      action == KAUTH_FILEOP_EXEC || action == KAUTH_FILEOP_WILL_RENAME) {
    ...
    if (action == KAUTH_FILEOP_CLOSE || action == KAUTH_FILEOP_WILL_RENAME) {
      /*
  ...
  kauth_authorize_action(kauth_scope_fileop, credential, action, arg0, arg1,
                         arg2, 0);
...
int kauth_authorize_generic(kauth_cred_t credential, kauth_action_t action) {
  if (credential == NULL) {
    panic("auth against NULL credential");
  }

  return kauth_authorize_action(kauth_scope_generic, credential, action, 0, 0,
                                0, 0);
}

static int kauth_authorize_generic_callback(
    kauth_cred_t credential, __unused void *idata, kauth_action_t action,
    __unused uintptr_t arg0, __unused uintptr_t arg1, __unused uintptr_t arg2,
    __unused uintptr_t arg3) {
  switch (action) {
  case KAUTH_GENERIC_ISSUSER:
    /* XXX == 0 ? */
    return (kauth_cred_getuid(credential) == 0) ? KAUTH_RESULT_ALLOW
                                                : KAUTH_RESULT_DENY;
  }
 ...
 * ACL evaluator.
 *
 * Determines whether the credential has the requested rights for an object
 * secured by the supplied ACL.
 *
 * Evaluation proceeds from the top down, with access denied if any ACE denies
 * any of the requested rights, or granted if all of the requested rights are
 * satisfied by the ACEs so far.
 */
int kauth_acl_evaluate(kauth_cred_t cred, kauth_acl_eval_t eval) {
  ...
  KAUTH_DEBUG("    ACL - %d entries, initial residual %x", eval->ae_count,
              eval->ae_residual);
  for (i = 0, ace = eval->ae_acl; i < eval->ae_count; i++, ace++) {
    ...
    case KAUTH_WKG_GROUP:
      ...
      break;
    /* we short-circuit these here rather than wasting time calling the group
     * membership code */
    ...
    default:
      ...
      KAUTH_DEBUG("    ACL - ACE applicable " K_UUID_FMT " caller " K_UUID_FMT
                  " %smatched",
                  K_UUID_ARG(ace->ace_applicable), K_UUID_ARG(guid),
                  applies ? "" : "not ");

      if (!applies) {
        error = !gotguid ? ENOENT
                         : kauth_cred_ismember_guid(cred, &ace->ace_applicable,
                                                    &applies);
        /*
         ...
         */
        if (error != 0) {
          KAUTH_DEBUG("    ACL[%d] - can't get membership, making pessimistic "
                      "assumption",
                      i);
          switch (ace->ace_flags & KAUTH_ACE_KINDMASK) {
          case KAUTH_ACE_PERMIT:
    ...
    case KAUTH_ACE_PERMIT:
      ...
      KAUTH_DEBUG("    ACL[%d] - rights %x leave residual %x", i, rights,
                  eval->ae_residual);
    ...
    case KAUTH_ACE_DENY:
      ...
      break;
    default:
      KAUTH_DEBUG("    ACL - unknown entry kind %d",
                  ace->ace_flags & KAUTH_ACE_KINDMASK);
      break;
    }
  }
 ...
 * the umask ACL.
 * If isdir is set, the resultant ACL is for a directory, otherwise it is for a
 * file.
 */
int kauth_acl_inherit(vnode_t dvp, kauth_acl_t initial, kauth_acl_t *product,
                      int isdir, vfs_context_t ctx) {
  ...
  if ((initial == NULL || !(initial->acl_flags & KAUTH_ACL_NO_INHERIT)) &&
    ...
    if ((error = vnode_getattr(dvp, &dva, ctx)) != 0) {
      KAUTH_DEBUG(
          "    ERROR - could not get parent directory ACL for inheritance");
      return error;
    }
  ...
  if (inherit != NULL) {
    for (i = 0; i < inherit->acl_entrycount; i++) {
      if (inherit->acl_ace[i].ace_flags &
          (isdir ? KAUTH_ACE_DIRECTORY_INHERIT : KAUTH_ACE_FILE_INHERIT)) {
        entries++;
      }
    }
  }
  ...
  if ((result = kauth_acl_alloc(entries)) == NULL) {
    KAUTH_DEBUG("    ERROR - could not allocate %d-entry result buffer for "
                "inherited ACL",
                entries);
    error = ENOMEM;
    goto out;
  ...
  if (initial != NULL) {
    ...
    KAUTH_DEBUG("    INHERIT - applied %d of %d initial entries", index,
                initial->acl_entrycount);
  }
  if (inherit != NULL) {
    for (i = 0; i < inherit->acl_entrycount; i++) {
      /*
...
int kauth_copyinfilesec(user_addr_t xsecurity, kauth_filesec_t *xsecdestpp) {
  ...
  if (count > 0 && ((error = copyin(xsecurity + KAUTH_FILESEC_SIZE(0),
                                    fsec->fsec_acl.acl_ace,
                                    copysize - KAUTH_FILESEC_SIZE(0))) != 0)) {
 ...
 *		the object itself.
 */
kauth_filesec_t kauth_filesec_alloc(int count) {
  kauth_filesec_t fsp;
 ...
 */
void kauth_filesec_acl_setendian(int kendian, kauth_filesec_t fsec,
                                 kauth_acl_t acl) {
 ...
 */
kauth_acl_t kauth_acl_alloc(int count) {
  /* if the caller hasn't given us a valid size hint, assume the worst */
  if ((count < 0) || (count > KAUTH_ACL_MAX_ENTRIES)) {
    return NULL;
  }
...
void kauth_acl_free(kauth_acl_t aclp) { kfree_data_addr(aclp); }
