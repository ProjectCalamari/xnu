...
static int os_reason_alloc_buffer_internal(os_reason_t cur_reason,
...
static void os_reason_dealloc_buffer(os_reason_t cur_reason) {
  ...
  bzero(&cur_reason->osr_kcd_descriptor,
        sizeof(cur_reason->osr_kcd_descriptor));
}

/*
 * Allocates and initializes a buffer of specified size for the reason. This
 * function may block and should not be called from extremely performance
 * sensitive contexts (i.e. jetsam). Also initializes the kcdata descriptor
 * accordingly. If there is an existing buffer, we dealloc the buffer before
 * allocating a new one and clear the associated kcdata descriptor. If
 * osr_bufsize is passed as 0, we deallocate the existing buffer and then
 * return.
 *
 * Returns:
 * 0 on success
...
static int os_reason_alloc_buffer_internal(os_reason_t cur_reason,
  ...
  cur_reason->osr_kcd_buf =
      kalloc_data_tag(osr_bufsize, flags | Z_ZERO, VM_KERN_MEMORY_REASON);
  ...
  if (kcdata_memory_static_init(&cur_reason->osr_kcd_descriptor,
                                ...
                                KCDATA_BUFFER_BEGIN_OS_REASON, osr_bufsize,
                                KCFLAG_USE_MEMCOPY) != KERN_SUCCESS) {
    os_reason_dealloc_buffer(cur_reason);
...
/*
 * Allocates space and sets description data in kcd_descriptor on the passed
 * reason.
 */
void os_reason_set_description_data(os_reason_t cur_reason, uint32_t type,
  ...
  lck_mtx_lock(&cur_reason->osr_lock);
  if (KERN_SUCCESS != kcdata_get_memory_addr(&cur_reason->osr_kcd_descriptor,
                                             type, reason_data_len,
                                             &osr_data_addr)) {
    panic("os_reason failed to get data address");
  }
  if (KERN_SUCCESS != kcdata_memcpy(&cur_reason->osr_kcd_descriptor,
                                    osr_data_addr, reason_data,
                                    reason_data_len)) {
