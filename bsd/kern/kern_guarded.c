...
extern int writev_uio(struct proc *p, int fd, user_addr_t user_iovp, int iovcnt,
...
static struct fileproc_guard *guarded_fileproc_alloc(guardid_t guard) {
...
static void guarded_fileproc_init(struct fileproc *fp, void *initarg) {
 ...
 */
void guarded_fileproc_copy_guard(struct fileproc *ofp, struct fileproc *nfp) {
  struct gfp_crarg arg = {.gca_guard = ofp->fp_guard->fpg_guard,
 ...
 */
void guarded_fileproc_unguard(struct fileproc *fp) {
...
static int fp_lookup_guarded_locked(proc_t p, int fd, guardid_t guard,
                                    struct fileproc **fpp) {
  int error;
  struct fileproc *fp;
...
int fp_lookup_guarded(proc_t p, int fd, guardid_t guard, struct fileproc **fpp,
                      int locked) {
  int error;
 ...
 * }
 */
int fp_isguarded(struct fileproc *fp, u_int attrs) {
  return fp->fp_guard_attrs && (fp->fp_guard_attrs & attrs) == attrs;
}

extern char *proc_name_address(void *p);

int fp_guard_exception(proc_t p, int fd, struct fileproc *fp, u_int flavor) {
 ...
 */
void fd_guard_ast(thread_t __unused t, mach_exception_code_t code,
   ...
   * and deliver the exception via EXC_CORPSE_NOTIFY. Always kill the process if
 ...
 *	promises above.  Ick.
 */
int guarded_open_np(proc_t p, struct guarded_open_np_args *uap,
                    int32_t *retval) {
  ...
  int error;
  struct gfp_crarg crarg = {.gca_attrs = (uint16_t)uap->guardflags};

  if ((error = copyin(uap->guard, &(crarg.gca_guard),
                      sizeof(crarg.gca_guard))) != 0) {
    return error;
  }
  ...
  return open1(ctx, &nd, uap->flags | O_CLOFORK, &va, guarded_fileproc_init,
               &crarg, retval, AUTH_OPEN_NOAUTHFD);
}
 ...
 *
 * This SPI is extension of guarded_open_np() to include dataprotection class on
 * creation in "dpclass" and dataprotection flags 'dpflags'. Otherwise behaviors
 * are same as in guarded_open_np()
 */
int guarded_open_dprotected_np(proc_t p,
                               struct guarded_open_dprotected_np_args *uap,
                               int32_t *retval) {
  if ((uap->flags & O_CLOEXEC) == 0) {
    return EINVAL;
  }
 ...
 */
int guarded_kqueue_np(proc_t p, struct guarded_kqueue_np_args *uap,
                      int32_t *retval) {
  if (((uap->guardflags & GUARD_REQUIRED) != GUARD_REQUIRED) ||
      ((uap->guardflags & ~GUARD_ALL) != 0)) {
    return EINVAL;
 ...
 */
int guarded_close_np(proc_t p, struct guarded_close_np_args *uap,
...
int change_fdguard_np(proc_t p, struct change_fdguard_np_args *uap,
                      __unused int32_t *retval) {
  ...
  if (0 != uap->nguard) {
    ...
    if (fp->fp_guard_attrs) {
      ...
      if (oldg == fp->fp_guard->fpg_guard &&
        ...
        if ((fp->fp_guard_attrs & GUARD_CLOSE) || (nfdflags & FD_CLOFORK)) {
          fp->fp_flags |= FP_CLOFORK;
        }
 ...
 */
int guarded_write_np(struct proc *p, struct guarded_write_np_args *uap,
                     user_ssize_t *retval) {
  int error;
  guardid_t uguard;
  ...
  return write_internal(p, uap->fd, uap->cbuf, uap->nbyte, 0, 0, &uguard,
                        retval);
}
 ...
 */
int guarded_pwrite_np(struct proc *p, struct guarded_pwrite_np_args *uap,
                      user_ssize_t *retval) {
  int error;
  guardid_t uguard;
  ...
  KERNEL_DEBUG_CONSTANT(
      (BSDDBG_CODE(DBG_BSD_SC_EXTENDED_INFO, SYS_guarded_pwrite_np) |
       DBG_FUNC_NONE),
 ...
 *
 */
int guarded_writev_np(struct proc *p, struct guarded_writev_np_args *uap,
                      user_ssize_t *retval) {
  int error;
  guardid_t uguard;
...
static struct vng_info *new_vgi(unsigned attrs, guardid_t guard) {
...
static struct vng_owner *new_vgo(proc_t p) {
  struct vng_owner *vgo = kalloc_type(struct vng_owner, Z_WAITOK | Z_ZERO);
  vgo->vgo_p = p;
  return vgo;
}

static void vgi_add_vgo(struct vng_info *vgi, struct vng_owner *vgo) {
  vgo->vgo_vgi = vgi;
  TAILQ_INSERT_HEAD(&vgi->vgi_owners, vgo, vgo_link);
}

static boolean_t vgi_remove_vgo(struct vng_info *vgi, struct vng_owner *vgo) {
  TAILQ_REMOVE(&vgi->vgi_owners, vgo, vgo_link);
  vgo->vgo_vgi = NULL;
  return TAILQ_EMPTY(&vgi->vgi_owners);
}

static void free_vgi(struct vng_info *vgi) {
...
static void free_vgo(struct vng_owner *vgo) {
#if DEVELOP || DEBUG
...
static __inline void *vng_lbl_get(struct label *label) {
...
static __inline struct vng_info *vng_lbl_get_withattr(struct label *label,
                                                      unsigned attrmask) {
...
static __inline void vng_lbl_set(struct label *label, void *data) {
  assert(NULL != label);
  lck_rw_assert(&llock, LCK_RW_ASSERT_EXCLUSIVE);
  mac_label_set(label, label_slot, (intptr_t)data);
}

static int vnguard_sysc_getguardattr(proc_t p, struct vnguard_getattr *vga) {
...
static int vnguard_sysc_setguard(proc_t p, const struct vnguard_set *vns) {
  const int fd = vns->vns_fd;

  if ((vns->vns_attrs & ~VNG_ALL) != 0 || 0 == vns->vns_attrs ||
      0 == vns->vns_guard) {
    return EINVAL;
  }
...
static int vng_policy_syscall(proc_t p, int cmd, user_addr_t arg) {
  int error = EINVAL;
 ...
 */
void vng_file_label_destroy(struct fileglob *fg) {
...
static os_reason_t vng_reason_from_pathname(const char *path,
                                            uint32_t pathlen) {
  ...
  if (0 == os_reason_alloc_buffer(r, rsize)) {
    ...
    if (kcdata_get_memory_addr(kcd, EXIT_REASON_USER_DESC, pathlen, &addr) ==
        KERN_SUCCESS) {
 ...
 */
static int vng_guard_violation(const struct vng_info *vgi, unsigned opval,
                               vnode_t vp) {
  int retval = 0;
  ...
  if (vng_policy_flags & (kVNG_POLICY_LOGMSG | kVNG_POLICY_UPRINTMSG)) {
    ...
    TAILQ_FOREACH(vgo, &vgi->vgi_owners, vgo_link) {
      const char fmt[] = "%s[%d]: %s%s: '%s' guarded by %s[%d] (0x%llx)\n";

      if (vng_policy_flags & kVNG_POLICY_LOGMSG) {
        printf(fmt, proc_name_address(p), proc_pid(p), op,
               0 != retval ? " denied" : "", NULL != nm ? nm : "(unknown)",
               proc_name_address(vgo->vgo_p), proc_pid(vgo->vgo_p),
               vgi->vgi_guard);
      }
      if (vng_policy_flags & kVNG_POLICY_UPRINTMSG) {
        uprintf(fmt, proc_name_address(p), proc_pid(p), op,
                0 != retval ? " denied" : "", NULL != nm ? nm : "(unknown)",
                proc_name_address(vgo->vgo_p), proc_pid(vgo->vgo_p),
                vgi->vgi_guard);
      }
  ...
  if (vng_policy_flags &
    ...
    if (vng_policy_flags & (kVNG_POLICY_EXC_CORPSE | kVNG_POLICY_EXC_CORE)) {
      char *path;
      int len = MAXPATHLEN;
      ...
      const bool backtrace_only = !(vng_policy_flags & kVNG_POLICY_EXC_CORE);
      /* not fatal */
...
/* KPI used by APFS Kext to generate fault when someone tries to change
 * permissions on some files */
 ...
 * (Invoked before returning to userland from the syscall handler.)
 */
void vn_guard_ast(thread_t __unused t, mach_exception_data_type_t code,
                  mach_exception_data_type_t subcode) {
  ...
  /*
   * Deliver exception Synchronously if anyone has registered for Sync
   * EXC_GUARD. If Sync exception delivery succeeds, then kill process if the
   * exception is fatal.
   *
   * If Sync exception delivery fails, then deliver the exception via
   * EXC_CORPSE_NOTIFY, the exception would have a corpse for a FATAL one and a
   * corpse-fork for a NON-Fatal exception.
   */
...
static int vng_vnode_check_rename(
    kauth_cred_t __unused cred, struct vnode *__unused dvp,
    struct label *__unused dlabel, struct vnode *vp, struct label *label,
    struct componentname *__unused cnp, struct vnode *__unused tdvp,
    struct label *__unused tdlabel, struct vnode *tvp, struct label *tlabel,
    struct componentname *__unused tcnp) {
  int error = 0;
  if (NULL != label || NULL != tlabel) {
    lck_rw_lock_shared(&llock);
    const struct vng_info *vgi = vng_lbl_get_withattr(label, VNG_RENAME_FROM);
    if (NULL != vgi) {
    ...
    if (0 == error) {
      ...
      if (NULL != vgi) {
        error = vng_guard_violation(vgi, VNG_RENAME_TO, tvp);
      }
    }
    lck_rw_unlock_shared(&llock);
  }
...
static int vng_vnode_check_link(kauth_cred_t __unused cred,
                                struct vnode *__unused dvp,
                                struct label *__unused dlabel, struct vnode *vp,
                                struct label *label,
                                struct componentname *__unused cnp) {
  int error = 0;
  if (NULL != label) {
    lck_rw_lock_shared(&llock);
    const struct vng_info *vgi = vng_lbl_get_withattr(label, VNG_LINK);
    if (vgi) {
...
static int vng_vnode_check_unlink(kauth_cred_t __unused cred,
                                  struct vnode *__unused dvp,
                                  struct label *__unused dlabel,
                                  struct vnode *vp, struct label *label,
                                  struct componentname *__unused cnp) {
  int error = 0;
  if (NULL != label) {
    lck_rw_lock_shared(&llock);
    const struct vng_info *vgi = vng_lbl_get_withattr(label, VNG_UNLINK);
    if (vgi) {
 ...
 */
static int vng_vnode_check_write(kauth_cred_t __unused actv_cred,
                                 kauth_cred_t __unused file_cred,
                                 struct vnode *vp, struct label *label) {
  int error = 0;
  if (NULL != label) {
    lck_rw_lock_shared(&llock);
    const struct vng_info *vgi = vng_lbl_get_withattr(label, VNG_WRITE_OTHER);
    if (vgi) {
      proc_t p = current_proc();
      const struct vng_owner *vgo;
      TAILQ_FOREACH(vgo, &vgi->vgi_owners, vgo_link) {
 ...
 */
static int vng_vnode_check_truncate(kauth_cred_t __unused actv_cred,
                                    kauth_cred_t __unused file_cred,
                                    struct vnode *vp, struct label *label) {
  int error = 0;
  if (NULL != label) {
    lck_rw_lock_shared(&llock);
    const struct vng_info *vgi = vng_lbl_get_withattr(label, VNG_TRUNC_OTHER);
    if (vgi) {
      proc_t p = current_proc();
      const struct vng_owner *vgo;
      TAILQ_FOREACH(vgo, &vgi->vgi_owners, vgo_link) {
...
static int vng_vnode_check_exchangedata(kauth_cred_t __unused cred,
  ...
  if (NULL != flabel || NULL != slabel) {
    lck_rw_lock_shared(&llock);
    const struct vng_info *vgi = vng_lbl_get_withattr(flabel, VNG_EXCHDATA);
    if (NULL != vgi) {
    ...
    if (0 == error) {
      ...
      if (NULL != vgi) {
        error = vng_guard_violation(vgi, VNG_EXCHDATA, svp);
      }
    }
    lck_rw_unlock_shared(&llock);
  }
...
static int vng_vnode_check_open(kauth_cred_t cred, struct vnode *vp,
...
SECURITY_READ_ONLY_EARLY(static struct mac_policy_ops)
vng_policy_ops = {
...
SECURITY_READ_ONLY_LATE(static struct mac_policy_conf)
vng_policy_conf = {.mpc_name = VNG_POLICY_NAME,
...
void vnguard_policy_init(void) {
  if (0 == PE_i_can_has_debugger(NULL)) {
    return;
  }
  vng_policy_flags =
      kVNG_POLICY_LOGMSG | kVNG_POLICY_EXC_CORPSE | kVNG_POLICY_UPRINTMSG;
...
SYSCTL_NODE(_kern, OID_AUTO, vnguard, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "vnguard");
