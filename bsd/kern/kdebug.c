 ...
 */
#define TYPEFILTER_ALLOC_SIZE                                                  \
  MAX(round_page_32(KDBG_TYPEFILTER_BITMAP_SIZE), KDBG_TYPEFILTER_BITMAP_SIZE)

static typefilter_t typefilter_create(void) {
  typefilter_t tf;
  if (KERN_SUCCESS ==
      kmem_alloc(kernel_map, (vm_offset_t *)&tf, TYPEFILTER_ALLOC_SIZE,
...
static mach_port_t typefilter_create_memory_entry(typefilter_t tf) {
  ...
  kern_return_t kr =
      mach_make_memory_entry_64(kernel_map, &size, (memory_object_offset_t)tf,
                                VM_PROT_READ, &memory_entry, MACH_PORT_NULL);
  if (kr != KERN_SUCCESS) {
...
static inline struct kd_dest kd_dest_copyout(user_addr_t buf, size_t size) {
...
static inline int kd_dest_init_write(struct kd_dest *dest, int fd,
                                     struct fileproc **fp_out) {
...
static inline void kd_dest_finish_write(struct kd_dest *dest,
                                        struct fileproc *fp, int fd) {
...
__attribute__((aligned(
    KD_EARLY_BUFFER_SIZE))) static kd_buf kd_early_buffer[KD_EARLY_EVENT_COUNT];
...
struct kd_buffer kd_buffer_trace = {.kdb_event_count = 0,
                                    .kdb_storage_count = 0,
 ...
 */
__attribute__((
    aligned(MAX_CPU_CACHE_LINE_SIZE))) static uint64_t g_curr_str_id = 1;
...
static void _coproc_lock(void) {
  lck_spin_lock_grp(&kd_coproc_spinlock, &kdebug_lck_grp);
}

static void _coproc_unlock(void) { lck_spin_unlock(&kd_coproc_spinlock); }

static void _coproc_list_check(void) {
#if MACH_ASSERT
  _coproc_lock();
...
static void _coproc_list_callback(kd_callback_type type, void *arg) {
...
static kdebug_emit_filter_t _trace_emit_filter(void) {
  if (!kdebug_enable) {
...
static void kdbg_set_tracing_enabled(bool enabled, uint32_t trace_type) {
...
static int create_buffers_trace(unsigned int extra_cpus) {
  int events_per_storage_unit = kd_control_trace.kdebug_events_per_storage_unit;
  int min_storage_units_per_cpu =
      kd_control_trace.kdebug_min_storage_units_per_cpu;
  ...
  kd_control_trace.kdebug_cpus = kd_control_trace.kdc_coprocs
                                     ? kd_control_trace.kdc_coprocs->cpu_id + 1
                                     : kdbg_cpu_count();
  ...
  if (kd_buffer_trace.kdb_event_count < min_event_count) {
    kd_buffer_trace.kdb_storage_count =
        kd_control_trace.alloc_cpus * min_storage_units_per_cpu;
  } else {
    kd_buffer_trace.kdb_storage_count =
        kd_buffer_trace.kdb_event_count / events_per_storage_unit;
  }

  kd_buffer_trace.kdb_event_count =
      kd_buffer_trace.kdb_storage_count * events_per_storage_unit;

  kd_buffer_trace.kd_bufs = NULL;

  int error =
      create_buffers(&kd_control_trace, &kd_buffer_trace, VM_KERN_MEMORY_DIAG);
  if (!error) {
    struct kd_bufinfo *info = kd_buffer_trace.kdb_info;
    struct kd_coproc *cur_iop = kd_control_trace.kdc_coprocs;
    while (cur_iop != NULL) {
      info[cur_iop->cpu_id].continuous_timestamps =
          ISSET(cur_iop->flags, KDCP_CONTINUOUS_TIME);
      cur_iop = cur_iop->next;
    }
    kd_buffer_trace.kdb_storage_threshold =
        kd_buffer_trace.kdb_storage_count / 2;
  }

  return error;
}

static void delete_buffers_trace(void) {
  delete_buffers(&kd_control_trace, &kd_buffer_trace);
}

static int _register_coproc_internal(const char *name,
                                     kdebug_coproc_flags_t flags,
  ...
  coproc->cpu_id =
      kd_coprocs == NULL ? kdbg_cpu_count() : kd_coprocs->cpu_id + 1;
  kd_coprocs = coproc;
...
static inline bool _should_emit_debugid(kdebug_emit_filter_t emit,
                                        uint32_t debugid) {
  switch (emit) {
  case KDEMIT_DISABLE:
    return false;
...
static void _try_wakeup_above_threshold(uint32_t debugid) {
...
__attribute__((always_inline)) static struct kd_storage *
_next_storage_unit(struct kd_bufinfo *info, unsigned int cpu) {
  ...
  do {
    ...
    bool allocated =
        kdebug_storage_alloc(&kd_control_trace, &kd_buffer_trace, cpu);
    if (!allocated) {
...
__attribute__((always_inline)) static kd_buf *
_next_timestamped_coproc_record(unsigned int cpu, uint64_t timestamp) {
  ...
  do {
    ...
    if (os_atomic_cmpxchg(&store->kds_bufindx, store_index, store_index + 1,
                          relaxed)) {
      break;
    }
  } while (true);
...
__attribute__((always_inline)) static void _write_trace_record_coproc_nopreempt(
    uint64_t timestamp, uint32_t debugid, uintptr_t arg1, uintptr_t arg2,
    uintptr_t arg3, uintptr_t arg4, uintptr_t arg5, unsigned int cpu) {
...
static void _write_trace_record(uint32_t debugid, uintptr_t arg1,
                                uintptr_t arg2, uintptr_t arg3, uintptr_t arg4,
                                uintptr_t arg5, kdebug_emit_flags_t flags) {
...
static void kernel_debug_internal(uint32_t debugid, uintptr_t arg1,
...
__attribute__((noinline)) void kernel_debug(uint32_t debugid, uintptr_t arg1,
                                            uintptr_t arg2, uintptr_t arg3,
                                            uintptr_t arg4,
...
__attribute__((noinline)) void kernel_debug1(uint32_t debugid, uintptr_t arg1,
                                             uintptr_t arg2, uintptr_t arg3,
                                             uintptr_t arg4, uintptr_t arg5) {
...
kernel_debug_flags(uint32_t debugid, uintptr_t arg1, uintptr_t arg2,
...
__attribute__((noinline)) void
kernel_debug_filtered(uint32_t debugid, uintptr_t arg1, uintptr_t arg2,
...
void kernel_debug_enter(uint32_t coreid, uint32_t debugid, uint64_t timestamp,
                        ...
                        uintptr_t arg4, uintptr_t threadid) {
  if (kd_control_trace.kdc_flags & KDBG_DISABLE_COPROCS) {
    return;
  }
  ...
  disable_preemption();
  _write_trace_record_coproc_nopreempt(timestamp, debugid, arg1, arg2, arg3,
                                       arg4, threadid, coreid);
  enable_preemption();
}

__pure2 static inline proc_t kdebug_current_proc_unsafe(void) {
...
void kernel_debug_string_simple(uint32_t eventid, const char *str) {
  ...
  kernel_debug_internal(debugid, str_buf[0], str_buf[1], str_buf[2], str_buf[3],
                        thread_id, 0);
  ...
  for (; written < len; i += 4, written += 4 * sizeof(uintptr_t)) {
    ...
    kernel_debug_internal(debugid, str_buf[i], str_buf[i + 1], str_buf[i + 2],
 ...
 */
static void kernel_debug_early_end(void) {
  ...
  kernel_debug_enter(0, TRACE_TIMESTAMPS, 0,
                     ...
                     (uint32_t)tsc_rebase_abs_time, tsc_at_boot, 0, 0);
#endif /* defined(__x86_64__) */
  for (unsigned int i = 0; i < kd_early_index; i++) {
    kernel_debug_enter(0, kd_early_buffer[i].debugid,
                       kd_early_buffer[i].timestamp, kd_early_buffer[i].arg1,
                       kd_early_buffer[i].arg2, kd_early_buffer[i].arg3,
                       kd_early_buffer[i].arg4, 0);
  }
...
static int _kernel_only_event(uint32_t debugid) {
 ...
 */
int kdebug_typefilter(__unused struct proc *p,
  ...
  kern_return_t kr = mach_vm_map_kernel(
      user_map, &user_addr, TYPEFILTER_ALLOC_SIZE, 0,
      VM_MAP_KERNEL_FLAGS_ANYWHERE(), kdbg_typefilter_memory_entry, 0, copy,
...
int kdebug_trace(struct proc *p, struct kdebug_trace_args *uap,
                 int32_t *retval) {
...
int kdebug_trace64(__unused struct proc *p, struct kdebug_trace64_args *uap,
 ...
 */
static uint64_t kernel_debug_string_internal(uint32_t debugid, uint64_t str_id,
                                             void *vstr, size_t str_len) {
  /* str must be word-aligned */
  uintptr_t *str = vstr;
  size_t written = 0;
  uintptr_t thread_id;
  int i;
  uint32_t trace_debugid = TRACEDBG_CODE(DBG_TRACE_STRING, TRACE_STRING_GLOBAL);
...
int kernel_debug_string(uint32_t debugid, uint64_t *str_id, const char *str) {
  ...
  *str_id = kernel_debug_string_internal(debugid, *str_id, str_buf, len_copied);
  return 0;
}

// Support kdebug_trace_string(2).
int kdebug_trace_string(__unused struct proc *p,
  ...
  if (uap->str == USER_ADDR_NULL) {
    ...
    *retval = kernel_debug_string_internal(uap->debugid, uap->str_id, NULL, 0);
    return 0;
  }
...
int kdbg_reinit(unsigned int extra_cpus) {
...
static void _copy_ap_name(unsigned int cpuid, void *dst, size_t size) {
  const char *name = "AP";
...
static errno_t _copy_cpu_map(int map_version, void **dst, size_t *size) {
  _coproc_lock();
  ...
  *size = size_needed;
  ...
  if (*dst == NULL) {
    kern_return_t alloc_ret =
        kmem_alloc(kernel_map, (vm_offset_t *)dst, (vm_size_t)size_needed,
                   KMA_DATA_SHARED | KMA_ZERO, VM_KERN_MEMORY_DIAG);
...
static void _threadmap_init(void) {
...
static int _resolve_iterator(proc_t proc, void *opaque) {
  if (proc == kernproc) {
...
static void _resolve_kernel_task(thread_t thread, void *opaque) {
  struct kd_resolver *resolver = opaque;
  if (resolver->krs_count >= resolver->krs_maxcount) {
    return;
  }
...
static vm_size_t _resolve_threads(kd_threadmap *map, vm_size_t nthreads) {
  struct kd_resolver resolver = {
      .krs_map = map,
      .krs_count = 0,
      .krs_maxcount = nthreads,
  };
...
static kd_threadmap *_thread_map_create_live(size_t maxthreads,
                                             vm_size_t *mapsize,
                                             vm_size_t *mapcount) {
  ...
  *mapcount = nthreads;
  ...
  thread_map =
      kalloc_data_tag(*mapsize, Z_WAITOK | Z_ZERO, VM_KERN_MEMORY_DIAG);
  if (thread_map != NULL) {
...
static void kdbg_clear(void) {
  ...
  if (kd_control_trace.kdc_flags & (KDBG_PIDCHECK | KDBG_PIDEXCLUDE)) {
    proc_list_lock();
    proc_t p;
    ALLPROC_FOREACH(p) { p->p_kdebug = 0; }
    proc_list_unlock();
  }
...
int kdbg_setpid(kd_regtype *kdr) {
  pid_t pid;
  int flag, ret = 0;
  struct proc *p;
...
int kdbg_setpidex(kd_regtype *kdr) {
  pid_t pid;
  int flag, ret = 0;
  struct proc *p;
...
static int kdbg_setreg(kd_regtype *kdr) {
  switch (kdr->type) {
  case KDBG_CLASSTYPE:
...
static int _copyin_event_disable_mask(user_addr_t uaddr, size_t usize) {
  ...
  if (ret != 0) {
    memset(&kd_control_trace.disable_event_match, 0, sizeof(kd_event_matcher));
    return ret;
  }
  return 0;
}

static int _copyout_event_disable_mask(user_addr_t uaddr, size_t usize) {
  if (usize < 2 * sizeof(kd_event_matcher)) {
    return ERANGE;
  }
...
static errno_t _copyout_cpu_map(int map_version, user_addr_t udst,
                                size_t *usize) {
...
int kdbg_readcurthrmap(user_addr_t buffer, size_t *bufsize) {
  kd_threadmap *mapptr;
  vm_size_t mapsize;
  vm_size_t mapcount;
  int ret = 0;
...
static void kdbg_set_nkdbufs_trace(unsigned int req_nkdbufs_trace) {
  /*
  ...
  const uint64_t max_nkdbufs_trace_64 =
      MIN(kmem_range_id_size(range_id), sane_size) / 2 / sizeof(kd_buf);
  /*
...
/*
 * Block until there are `kd_buffer_trace.kdb_storage_threshold` storage units
 * filled with events or `timeout_ms` milliseconds have passed.  If
 * `locked_wait` is true, `ktrace_lock` is held while waiting.  This is
 * necessary while waiting to write events out of the buffers.
 *
...
static bool kdbg_wait(uint64_t timeout_ms) {
  ...
  while (wait_result == THREAD_AWAKENED &&
         kd_control_trace.kdc_storage_used <
             kd_buffer_trace.kdb_storage_threshold) {
    kd_waiter = true;
    ...
    if (deadline_mach) {
      ...
      wait_result =
          lck_spin_sleep(&kd_wait_lock, 0, &kd_waiter, THREAD_ABORTSAFE);
    }
  }

  bool threshold_exceeded = (kd_control_trace.kdc_storage_used >=
                             kd_buffer_trace.kdb_storage_threshold);
...
static void _try_wakeup_waiter(void) {
  ...
  if (lck_spin_try_lock(&kd_wait_lock)) {
    if (kd_waiter && (kd_control_trace.kdc_storage_used >=
                      kd_buffer_trace.kdb_storage_threshold)) {
      kd_waiter = 0;
...
static void _storage_free(struct kd_control *kd_ctrl_page,
...
static bool _reading_set_flags(struct kd_control *ctl,
...
static bool _reading_restore_flags(struct kd_control *ctl,
                                   kdebug_emit_filter_t old_emit,
                                   kdebug_live_flags_t old_live) {
  int intrs_en = kdebug_storage_lock(ctl);
...
static inline void _clear_oldest_lostevents(void) {
  for (unsigned int cpu = 0; cpu < kd_control_trace.kdebug_cpus; cpu++) {
    ...
    if (oldest_ptr.raw != KDS_PTR_NULL) {
      struct kd_storage *store =
          POINTER_FROM_KDS_PTR(kd_buffer_trace.kd_bufs, oldest_ptr);
      store->kds_lostevents = false;
    }
  }
}

static inline bool _event_should_disable(kd_buf *event) {
...
static inline struct kd_storage *_store_read_inc(struct kd_storage *store,
...
static inline uint64_t _store_earliest_timestamp(struct kd_storage *store,
...
static int _read_trace_events_internal(struct kd_dest *dest, size_t event_count,
                                       uint64_t barrier_max, bool wrapped,
                                       bool *should_disable,
  ...
  while (event_count && !out_of_events) {
    ...
    size_t avail_count =
        MIN(event_count, kd_control_trace.kdebug_kdcopybuf_count);
    while (used_count < avail_count) {
      ...
      for (unsigned int cpu = 0; cpu < kd_control_trace.kdebug_cpus; cpu++) {
        ...
        uint64_t t = _store_earliest_timestamp(store, barrier_min, barrier_max,
                                               info, cpu, oldest_ptr);
        if (t < earliest_time) {
          earliest_time = t;
          min_cpu = cpu;
      ...
      if (__improbable(min_info->latest_past_event_timestamp != 0)) {
        if (__improbable(kdbg_debug)) {
          printf("kdebug: PAST EVENT: debugid %#8x: "
                 "time %lld from CPU %u "
                 "(barrier at time %lld)\n",
                 earliest_event->debugid, min_info->latest_past_event_timestamp,
                 min_cpu, barrier_min);
        }
        tempbuf[used_count++] = (kd_buf){
      ...
      if (__improbable(earliest_time < min_info->kd_prev_timebase)) {
        ...
        if (__improbable(kdbg_debug)) {
          printf("kdebug: RETRO EVENT: debugid %#8x: "
                 ...
                 tempbuf[used_count].debugid, earliest_time, min_cpu,
                 min_info->kd_prev_timebase);
        }
...
static int _read_trace_events(struct kd_dest *dest, size_t event_count,
                              size_t *events_written) {
  bool should_disable = false;
  int const prev_kdebug_enable = kdebug_enable;
  *events_written = 0;
  if (!(kd_control_trace.kdc_flags & KDBG_BUFINIT) ||
      kd_buffer_trace.kdcopybuf == NULL) {
    return EINVAL;
  }
  ...
  bool wrapped =
      _reading_set_flags(&kd_control_trace, &old_emit, &old_live_flags);
  bool const no_wrapping = old_live_flags & KDBG_NOWRAP;
  int error = _read_trace_events_internal(
      dest, event_count, barrier_max, wrapped, &should_disable, events_written);
  bool disabled_during_read =
      _reading_restore_flags(&kd_control_trace, old_emit, old_live_flags);
...
static int _read_merged_trace_events(struct kd_dest *dest, size_t event_count,
                                     size_t *events_written) {
  ktrace_assert_lock_held();
...
static int _send_data_vfs(struct kd_dest *dest, const void *src, size_t size) {
  assert(size < INT_MAX);
  assert(dest->kdd_kind == KD_DEST_VFS);
  return vn_rdwr(UIO_WRITE, dest->kdd_vnode, (caddr_t)(uintptr_t)src, (int)size,
                 dest->kdd_cur_offset, UIO_SYSSPACE, IO_NODELOCKED | IO_UNIT,
                 vfs_context_ucred(&dest->kdd_vfs_ctx), (int *)0,
                 vfs_context_proc(&dest->kdd_vfs_ctx));
}

static int _send_data(struct kd_dest *dest, const void *src, size_t size) {
  int error = 0;
...
static int _send_event_chunk_header(struct kd_dest *dest, size_t event_count) {
...
int _send_events(struct kd_dest *dest, const void *src, size_t event_count) {
...
static int _write_legacy_header(bool write_thread_map, struct kd_dest *dest) {
  ...
  cpumap_size = sizeof(kd_cpumap_header) +
                kd_control_trace.kdebug_cpus * sizeof(kd_cpumap);
  ...
  /* The way we are silently embedding a cpumap in the "padding" is by
   * artificially increasing the number of thread entries. However, we'll also
   * need to ensure that the cpumap is embedded in the last 4K page before when
   * the event data is expected. This way the tools can read the data starting
   * the next page boundary on both 4K and 16K systems preserving compatibility
...
static int _kd_sysctl_internal(int op, int value, user_addr_t where,
                               size_t *sizep) {
  size_t size = *sizep;
  kd_regtype kd_Reg;
  ...
  int perm_error =
      read_only ? ktrace_read_check() : ktrace_configure(KTRACE_KDEBUG);
  if (perm_error != 0) {
    return perm_error;
  }
  ...
  case KERN_KDREADTR: {
    ...
    int error =
        _read_merged_trace_events(&copy_dest, event_count, &events_written);
  ...
  case KERN_KDWRITEMAP: {
    ...
    if (op == KERN_KDWRITETR || op == KERN_KDWRITETR_V3) {
      ...
      error =
          _read_merged_trace_events(&write_dest, event_count, &events_written);
...
static int kdebug_sysctl SYSCTL_HANDLER_ARGS {
  int *names = arg1;
  int name_count = arg2;
...
SYSCTL_PROC(_kern, KERN_KDEBUG, kdebug,
            CTLTYPE_NODE | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0, kdebug_sysctl,
            NULL, "");
...
static void sync_flush_callback(void *__unused context, kd_callback_type reason,
                                void *__unused arg) {
...
static void test_coproc_cb(__assert_only void *context,
                           kd_callback_type __unused reason,
                           void *__unused arg) {
...
static int kdbg_test(size_t flavor) {
  ...
  case KDTEST_KERNEL_MACROS:
    /* try each macro */
    KDBG(KDEBUG_TEST_CODE(code));
    code++;
    KDBG(KDEBUG_TEST_CODE(code), 1);
    code++;
    KDBG(KDEBUG_TEST_CODE(code), 1, 2);
    code++;
    KDBG(KDEBUG_TEST_CODE(code), 1, 2, 3);
    code++;
    KDBG(KDEBUG_TEST_CODE(code), 1, 2, 3, 4);
    code++;

    KDBG_RELEASE(KDEBUG_TEST_CODE(code));
    code++;
    KDBG_RELEASE(KDEBUG_TEST_CODE(code), 1);
    code++;
    KDBG_RELEASE(KDEBUG_TEST_CODE(code), 1, 2);
    code++;
    KDBG_RELEASE(KDEBUG_TEST_CODE(code), 1, 2, 3);
    code++;
    KDBG_RELEASE(KDEBUG_TEST_CODE(code), 1, 2, 3, 4);
    code++;

    KDBG_FILTERED(KDEBUG_TEST_CODE(code));
    code++;
    KDBG_FILTERED(KDEBUG_TEST_CODE(code), 1);
    code++;
    KDBG_FILTERED(KDEBUG_TEST_CODE(code), 1, 2);
    code++;
    KDBG_FILTERED(KDEBUG_TEST_CODE(code), 1, 2, 3);
    code++;
    KDBG_FILTERED(KDEBUG_TEST_CODE(code), 1, 2, 3, 4);
    code++;

    KDBG_RELEASE_NOPROCFILT(KDEBUG_TEST_CODE(code));
    code++;
    KDBG_RELEASE_NOPROCFILT(KDEBUG_TEST_CODE(code), 1);
    code++;
    KDBG_RELEASE_NOPROCFILT(KDEBUG_TEST_CODE(code), 1, 2);
    code++;
    KDBG_RELEASE_NOPROCFILT(KDEBUG_TEST_CODE(code), 1, 2, 3);
    code++;
    KDBG_RELEASE_NOPROCFILT(KDEBUG_TEST_CODE(code), 1, 2, 3, 4);
    code++;

    KDBG_DEBUG(KDEBUG_TEST_CODE(code));
    code++;
    KDBG_DEBUG(KDEBUG_TEST_CODE(code), 1);
    code++;
    KDBG_DEBUG(KDEBUG_TEST_CODE(code), 1, 2);
    code++;
    KDBG_DEBUG(KDEBUG_TEST_CODE(code), 1, 2, 3);
    code++;
    KDBG_DEBUG(KDEBUG_TEST_CODE(code), 1, 2, 3, 4);
    code++;
    break;
  ...
  case KDTEST_OLD_TIMESTAMP:
    ...
    code++;
    kernel_debug_enter(dummy_iop, KDEBUG_TEST_CODE(code), kdebug_timestamp(), 0,
                       0, 0, 0, 0);
    code++;
    break;
  ...
  case KDTEST_SETUP_IOP:
    if (!sync_flush_coproc) {
      ktrace_unlock();
      int new_sync_flush_coproc =
          kernel_debug_register_callback(sync_flush_kdcb);
  ...
  case KDTEST_SETUP_COPROCESSOR:
    if (!test_coproc) {
      ktrace_unlock();
      int new_test_coproc =
          kdebug_register_coproc("test_coproc", KDCP_CONTINUOUS_TIME,
                                 test_coproc_cb, (void *)TEST_COPROC_CTX);
  ...
  case KDTEST_ABSOLUTE_TIMESTAMP:;
    ...
    kernel_debug_enter(sync_flush_coproc, KDEBUG_TEST_CODE(0), atime,
                       (uintptr_t)atime, (uintptr_t)(atime >> 32), 0, 0, 0);
    break;

  case KDTEST_CONTINUOUS_TIMESTAMP:;
    uint64_t ctime = mach_continuous_time();
    kernel_debug_enter(test_coproc, KDEBUG_TEST_CODE(1), ctime,
                       (uintptr_t)ctime, (uintptr_t)(ctime >> 32), 0, 0, 0);
    break;

  case KDTEST_PAST_EVENT:;
    uint64_t old_time = 1;
    kernel_debug_enter(test_coproc, KDEBUG_TEST_CODE(1), old_time, 0, 0, 0, 0,
                       0);
    kernel_debug_enter(test_coproc, KDEBUG_TEST_CODE(1), kdebug_timestamp(), 0,
                       0, 0, 0, 0);
    break;
...
static void _deferred_coproc_notify(mpsc_queue_chain_t e,
                                    mpsc_daemon_queue_t queue __unused) {
  ...
  if (kdebug_enable) {
    coproc->callback.func(coproc->callback.context, KD_CALLBACK_KDEBUG_ENABLED,
                          kdbg_typefilter);
  }
}

void kdebug_init(unsigned int n_events, char *filter_desc,
                 enum kdebug_opts opts) {
  assert(filter_desc != NULL);

  kdbg_typefilter = typefilter_create();
  assert(kdbg_typefilter != NULL);
  kdbg_typefilter_memory_entry =
      typefilter_create_memory_entry(kdbg_typefilter);
  assert(kdbg_typefilter_memory_entry != MACH_PORT_NULL);

  (void)mpsc_daemon_queue_init_with_thread_call(
      &_coproc_notify_queue, _deferred_coproc_notify,
      THREAD_CALL_PRIORITY_KERNEL, MPSC_DAEMON_INIT_NONE);

  kdebug_trace_start(n_events, filter_desc, opts);
}

static void kdbg_set_typefilter_string(const char *filter_desc) {
  char *end = NULL;
 ...
 */
void kdebug_trace_start(unsigned int n_events, const char *filter_desc,
                        enum kdebug_opts opts) {
  if (!n_events) {
    kd_early_done = true;
    return;
...
void kdbg_dump_trace_to_file(const char *filename, bool reenable) {
  vfs_context_t ctx;
  vnode_t vp;
  int ret;
  ...
  if (vnode_open(filename, (O_CREAT | FWRITE | O_NOFOLLOW), 0600, 0, &vp,
                 ctx)) {
    goto out;
  }
...
SYSCTL_NODE(_kern, OID_AUTO, kdbg, CTLFLAG_RD | CTLFLAG_LOCKED, 0, "kdbg");

SYSCTL_INT(_kern_kdbg, OID_AUTO, debug, CTLFLAG_RW | CTLFLAG_LOCKED,
