...
static int semundo_adjust(struct proc *p, int *supidx, int semid, int semnum,
                          int adjval);
static void semundo_clear(int semid, int semnum);

/* XXX casting to (sy_call_t *) is bogus, as usual. */
static sy_call_t *const semcalls[] = {(sy_call_t *)semctl, (sy_call_t *)semget,
                                      (sy_call_t *)semop};
...
static __inline__ user_time_t sysv_semtime(void) {
  struct timeval tv;
  microtime(&tv);
  return tv.tv_sec;
}
 ...
 */
static void semid_ds_kernelto32(struct user_semid_ds *in,
                                struct user32_semid_ds *out) {
...
static void semid_ds_kernelto64(struct user_semid_ds *in,
                                struct user64_semid_ds *out) {
  out->sem_perm = in->sem_perm;
  out->sem_base = CAST_DOWN_EXPLICIT(__int32_t, in->sem_base);
  out->sem_nsems = in->sem_nsems;
  out->sem_otime = in->sem_otime; /* XXX loses precision */
 ...
 * XXX is the same.
 */
static void semid_ds_32tokernel(struct user32_semid_ds *in,
                                struct user_semid_ds *out) {
...
static void semid_ds_64tokernel(struct user64_semid_ds *in,
                                struct user_semid_ds *out) {
  out->sem_ctime = in->sem_ctime;
  out->sem_otime = in->sem_otime;
  out->sem_nsems = in->sem_nsems;
 ...
 *
 * Indirect parameters:	uap->which	sem call to invoke (index in array of
 * sem calls) uap->a2		User argument descriptor
 *
 * Returns:	0	Success
 *		!0	Not success
 *
 ...
 *
 */
int semsys(struct proc *p, struct semsys_args *uap, int32_t *retval) {
...
static inline struct semid_kernel *sema_get_by_id(size_t i) {
 ...
 */
static int grow_semu_array(void) {
  ...
  newSemu = krealloc_type(struct sem_undo, seminfo.semmnu, new_size, semu,
                          Z_WAITOK | Z_ZERO);
  if (NULL == newSemu) {
    return 0;
  }
 ...
 * Assumes we already have the subsystem lock.
 */
static int grow_sema_array(void) {
  ...
  newArr = krealloc_type(struct semid_kernel *, old_size / SEMMNI_INC,
                         old_size / SEMMNI_INC + 1, semas, Z_WAITOK | Z_ZERO);
  if (newArr == NULL) {
    return 0;
  }
 ...
 */
static int grow_sem_pool(int new_pool_size) {
  ...
  new_pool_size =
      new_pool_size > limitseminfo.semmns ? limitseminfo.semmns : new_pool_size;

  new_sem_pool =
      krealloc_data(sem_pool, sizeof(struct sem) * seminfo.semmns,
                    sizeof(struct sem) * new_pool_size, Z_WAITOK | Z_ZERO);
  ...
  for (int i = 0; i < seminfo.semmni; i++) {
    ...
    if (semakptr->u.sem_perm.mode & SEM_ALLOC) { /* ID in use */
      semakptr->u.sem_base = new_sem_pool + (semakptr->u.sem_base - sem_pool);
    }
  }
 ...
 */
int semctl(struct proc *p, struct semctl_args *uap, int32_t *retval) {
  int semid = uap->semid;
  ...
  case IPC_SET:
    ...
    semakptr->u.sem_perm.mode =
        (semakptr->u.sem_perm.mode & ~0777) | (sbuf.sem_perm.mode & 0777);
...
int semget(__unused struct proc *p, struct semget_args *uap, int32_t *retval) {
  int semid, eval;
  int key = uap->key;
  int nsems = uap->nsems;
...
#endif
      if ((eval = ipcperm(cred, &semakptr->u.sem_perm, semflg & 0700))) {
        goto semgetout;
      }
  ...
  if (key == IPC_PRIVATE || (semflg & IPC_CREAT)) {
    ...
    semakptr->u.sem_perm._seq = (semakptr->u.sem_perm._seq + 1) & 0x7fff;
    semakptr->u.sem_nsems = nsems;
    semakptr->u.sem_otime = 0;
    semakptr->u.sem_ctime = sysv_semtime();
    semakptr->u.sem_base = &sem_pool[semtot];
    semtot += nsems;
    bzero(semakptr->u.sem_base, sizeof(semakptr->u.sem_base[0]) * nsems);
#if CONFIG_MACF
...
int semop(struct proc *p, struct semop_args *uap, int32_t *retval) {
  int semid = uap->semid;
  int nsops = uap->nsops;
  ...
  for (;;) {
...
#ifdef SEM_DEBUG
      printf("semop:  semakptr=%x, sem_base=%x, semptr=%x, sem[%d]=%d : op=%d, "
             "flag=%s\n",
             semakptr, semakptr->u.sem_base, semptr, sopptr->sem_num,
...
#endif
    for (j = 0; j < i; j++) {
      semakptr->u.sem_base[sops[j].sem_num].semval -= sops[j].sem_op;
    }
     ...
     * wake up.
     */
    eval = msleep((caddr_t)semakptr, &sysv_sem_subsys_mutex,
                  (PZERO - 4) | PCATCH, "semwait", 0);
  ...
  if (do_undos) {
    for (i = 0; i < nsops; i++) {
      ...
      eval = semundo_adjust(p, &supidx, semid, sops[i].sem_num, -adjval);
      if (eval == 0) {
        continue;
      }
      ...
      for (j = i - 1; j >= 0; j--) {
        ...
        if (semundo_adjust(p, &supidx, semid, sops[j].sem_num, adjval) != 0) {
  ...
  if (suptr->un_cnt > 0) {
    while (suptr->un_ent != NULL) {
      ...
      printf("semexit:  %08x id=%d num=%d(adj=%d) ; sem=%d\n", suptr->un_proc,
             semid, semnum, adjval, semakptr->u.sem_base[semnum].semval);
#endif
...
static int sysctl_seminfo(__unused struct sysctl_oid *oidp, void *arg1,
  ...
  if ((sem_pool == NULL) && (semas == NULL) && (semu == NULL) &&
      (semu_list_idx == -1)) {
...
SYSCTL_PROC(_kern_sysv, OID_AUTO, semmni,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &limitseminfo.semmni, 0,
            &sysctl_seminfo, "I", "semmni");

SYSCTL_PROC(_kern_sysv, OID_AUTO, semmns,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &limitseminfo.semmns, 0,
            &sysctl_seminfo, "I", "semmns");

SYSCTL_PROC(_kern_sysv, OID_AUTO, semmnu,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &limitseminfo.semmnu, 0,
            &sysctl_seminfo, "I", "semmnu");

SYSCTL_PROC(_kern_sysv, OID_AUTO, semmsl,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &limitseminfo.semmsl, 0,
            &sysctl_seminfo, "I", "semmsl");

SYSCTL_PROC(_kern_sysv, OID_AUTO, semume,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &limitseminfo.semume, 0,
            &sysctl_seminfo, "I", "semume");

static int IPCS_sem_sysctl(__unused struct sysctl_oid *oidp,
...
SYSCTL_PROC(_kern_sysv_ipcs, OID_AUTO, sem,
            CTLFLAG_RW | CTLFLAG_ANYBODY | CTLFLAG_LOCKED, 0, 0,
            IPCS_sem_sysctl, "S,IPCS_sem_command",
