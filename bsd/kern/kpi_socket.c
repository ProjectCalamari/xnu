...
static errno_t sock_send_internal(socket_t, const struct msghdr *, mbuf_t, int,
                                  size_t *);

#undef sock_accept
#undef sock_socket
errno_t sock_accept(socket_t so, struct sockaddr *__sized_by(fromlen) from,
...
static errno_t sock_accept_common(socket_t sock,
                                  struct sockaddr *__sized_by(fromlen) from,
                                  int fromlen, int flags, sock_upcall callback,
                                  void *cookie, socket_t *new_sock,
                                  bool is_internal);
static errno_t sock_socket_common(int domain, int type, int protocol,
                                  sock_upcall callback, void *context,
                                  socket_t *new_so, bool is_internal);

errno_t sock_accept_common(socket_t sock,
                           struct sockaddr *__sized_by(fromlen) from,
                           int fromlen, int flags, sock_upcall callback,
                           void *cookie, socket_t *new_sock, bool is_internal) {
...
errno_t sock_accept(socket_t sock, struct sockaddr *__sized_by(fromlen) from,
                    int fromlen, int flags, sock_upcall callback, void *cookie,
                    socket_t *new_sock) {
  return sock_accept_common(sock, from, fromlen, flags, callback, cookie,
                            new_sock, false);
}

errno_t sock_accept_internal(socket_t sock,
                             struct sockaddr *__sized_by(fromlen) from,
                             int fromlen, int flags, sock_upcall callback,
                             void *cookie, socket_t *new_sock) {
  return sock_accept_common(sock, from, fromlen, flags, callback, cookie,
...
errno_t sock_bind(socket_t sock, const struct sockaddr *to) {
  int error = 0;
...
errno_t sock_connect(socket_t sock, const struct sockaddr *to, int flags) {
  int error = 0;
  lck_mtx_t *mutex_held;
  struct sockaddr *sa = NULL;
  struct sockaddr_storage ss;
  ...
  if (to->sa_len > sizeof(ss)) {
    sa = kalloc_data(to->sa_len, (flags & MSG_DONTWAIT) ? Z_NOWAIT : Z_WAITOK);
    if (sa == NULL) {
      return ENOBUFS;
    }
  } else {
...
errno_t sock_connectwait(socket_t sock, const struct timeval *tv) {
  lck_mtx_t *mutex_held;
  errno_t retval = 0;
  struct timespec ts;
  ...
  if (tv->tv_sec < 0 || tv->tv_sec > SHRT_MAX / hz || tv->tv_usec < 0 ||
      tv->tv_usec >= 1000000) {
    retval = EDOM;
    goto done;
  }
...
errno_t sock_getpeername(socket_t sock,
                         struct sockaddr *__sized_by(peernamelen) peername,
                         int peernamelen) {
  int error;
...
errno_t sock_getsockname(socket_t sock,
                         struct sockaddr *__sized_by(socknamelen) sockname,
                         int socknamelen) {
  int error;
  struct sockaddr *__single sa = NULL;
...
__private_extern__ int sogetaddr_locked(struct socket *so,
                                        struct sockaddr **psa, int peer) {
  int error;
...
errno_t sock_getaddr(socket_t sock, struct sockaddr **psa, int peer) {
  int error;
...
void sock_freeaddr(struct sockaddr *sa) { free_sockaddr(sa); }

errno_t sock_getsockopt(socket_t sock, int level, int optname, void *optval,
                        int *optlen) {
  int error = 0;
  struct sockopt sopt;
...
errno_t sock_ioctl(socket_t sock, unsigned long request,
                   void *__sized_by(IOCPARM_LEN(request)) argp) {
  return soioctl(sock, request, argp, kernproc); /* will lock socket */
}

errno_t sock_setsockopt(socket_t sock, int level, int optname,
 ...
 */
static uint32_t so_tc_from_dscp(uint8_t dscp) {
  uint32_t tc;
...
errno_t sock_settclassopt(socket_t sock, const void *optval, size_t optlen) {
  errno_t error = 0;
  struct sockopt sopt;
  int sotc;
...
errno_t sock_gettclassopt(socket_t sock, void *optval, size_t *optlen) {
  errno_t error = 0;
  struct sockopt sopt;
...
errno_t sock_listen(socket_t sock, int backlog) {
  if (sock == NULL) {
    return EINVAL;
  }
...
errno_t sock_receive_internal(socket_t sock, struct msghdr *msg, mbuf_t *data,
  ...
  auio = uio_createwithbuffer(((msg != NULL) ? msg->msg_iovlen : 0), 0,
                              UIO_SYSSPACE, UIO_READ, &uio_buf[0],
                              sizeof(uio_buf));
  if (msg != NULL && data == NULL) {
    int i;
    struct iovec *tempp = __unsafe_forge_bidi_indexable(
        struct iovec *, msg->msg_iov, sizeof(struct iovec) * msg->msg_iovlen);

    for (i = 0; i < msg->msg_iovlen; i++) {
      uio_addiov(auio, CAST_USER_ADDR_T((tempp + i)->iov_base),
                 (tempp + i)->iov_len);
    }
    if (uio_resid(auio) < 0) {
      return EINVAL;
  ...
  error = sock->so_proto->pr_usrreqs->pru_soreceive(
      sock, &fromsa, auio, data, (msg && msg->msg_control) ? &control : NULL,
      &flags);
  if (error != 0) {
    goto cleanup;
  }
    ...
    if (msg->msg_name != NULL) {
      ...
      if (msg->msg_namelen > 0 && fromsa != NULL) {
        ...
        SOCKADDR_COPY(fromsa, msg->msg_name,
                      msg->msg_namelen > fromsa->sa_len ? fromsa->sa_len
                                                        : msg->msg_namelen);
      }
    }
...
errno_t sock_receive(socket_t sock, struct msghdr *msg, int flags,
                     size_t *recvdlen) {
  if ((msg == NULL) || (msg->msg_iovlen < 1) ||
      (msg->msg_iov[0].iov_len == 0) || (msg->msg_iov[0].iov_base == NULL)) {
    return EINVAL;
  }
...
errno_t sock_receivembuf(socket_t sock, struct msghdr *msg, mbuf_t *data,
                         int flags, size_t *recvlen) {
  if (data == NULL || recvlen == 0 || *recvlen <= 0 ||
      (msg != NULL && (msg->msg_iov != NULL || msg->msg_iovlen != 0))) {
    return EINVAL;
  }
...
errno_t sock_send_internal(socket_t sock, const struct msghdr *msg, mbuf_t data,
  ...
  if (data == NULL && msg != NULL) {
    ...
    if (auio == NULL) {
      ...
      printf("sock_send_internal: so %p uio_createwithbuffer(%lu) failed, "
             "ENOMEM\n",
  ...
  error = sock->so_proto->pr_usrreqs->pru_sosend(
      sock, msg != NULL ? (struct sockaddr *)msg->msg_name : NULL, auio, data,
      control, flags);
...
errno_t sock_send(socket_t sock, const struct msghdr *msg, int flags,
                  size_t *sentlen) {
...
errno_t sock_sendmbuf(socket_t sock, const struct msghdr *msg, mbuf_t data,
                      int flags, size_t *sentlen) {
  int error;
...
errno_t sock_sendmbuf_can_wait(socket_t sock, const struct msghdr *msg,
                               mbuf_t data, int flags, size_t *sentlen) {
  int error;
  int count = 0;
  int i;
  mbuf_t m;
...
errno_t sock_shutdown(socket_t sock, int how) {
  if (sock == NULL) {
    return EINVAL;
  }
...
errno_t sock_socket_common(int domain, int type, int protocol,
                           sock_upcall callback, void *context,
                           socket_t *new_so, bool is_internal) {
  int error = 0;
...
errno_t sock_socket_internal(int domain, int type, int protocol,
                             sock_upcall callback, void *context,
                             socket_t *new_so) {
  return sock_socket_common(domain, type, protocol, callback, context, new_so,
                            true);
}
...
void sock_retain(socket_t sock) {
  if (sock == NULL) {
    return;
  }
...
void sock_release(socket_t sock) {
  if (sock == NULL) {
    return;
  }
  ...
  if (sock->so_retaincnt < 0) {
    panic("%s: negative retain count (%d) for sock=%p", __func__,
          sock->so_retaincnt, sock);
    /* NOTREACHED */
  }
  /*
  ...
  if ((sock->so_retaincnt == 0) &&
      ((SOCK_DOM(sock) != PF_LOCAL && sock->so_usecount == 2) ||
       (SOCK_DOM(sock) == PF_LOCAL && (sock->so_state & SS_ISCONNECTED) &&
        sock->so_usecount == 3)) &&
...
errno_t sock_setpriv(socket_t sock, int on) {
  if (sock == NULL) {
    return EINVAL;
  }
...
int sock_isconnected(socket_t sock) {
  int retval;
...
int sock_isnonblocking(socket_t sock) {
  int retval;
...
errno_t sock_gettype(socket_t sock, int *outDomain, int *outType,
                     int *outProtocol) {
  socket_lock(sock, 1);
  if (outDomain != NULL) {
 ...
 */
socket_t sock_getlistener(socket_t sock) { return sock->so_head; }

static inline void sock_set_tcp_stream_priority(socket_t sock) {
 ...
 */
void socket_set_traffic_mgt_flags_locked(socket_t sock, u_int8_t flags) {
...
void socket_set_traffic_mgt_flags(socket_t sock, u_int8_t flags) {
  socket_lock(sock, 1);
 ...
 * Caller must have ensured socket is valid and won't be going away.
 */
void socket_clear_traffic_mgt_flags_locked(socket_t sock, u_int8_t flags) {
  u_int32_t soflags1 = 0;
...
void socket_clear_traffic_mgt_flags(socket_t sock, u_int8_t flags) {
  socket_lock(sock, 1);
 ...
 * Caller must have ensured socket is valid and won't be going away.
 */
errno_t socket_defunct(struct proc *p, socket_t so, int level) {
  errno_t retval;
...
void sock_setupcalls_locked(socket_t sock, sock_upcall rcallback,
                            void *rcontext, sock_upcall wcallback,
                            void *wcontext, int locked) {
  if (rcallback != NULL) {
...
errno_t sock_setupcall(socket_t sock, sock_upcall callback, void *context) {
  if (sock == NULL) {
    return EINVAL;
  }
...
errno_t sock_setupcalls(socket_t sock, sock_upcall rcallback, void *rcontext,
...
void sock_catchevents_locked(socket_t sock, sock_evupcall ecallback,
...
errno_t sock_catchevents(socket_t sock, sock_evupcall ecallback, void *econtext,
                         uint32_t emask) {
  if (sock == NULL) {
    return EINVAL;
  }
 ...
 */
int sock_iskernel(socket_t so) { return so && so->last_pid == 0; }
