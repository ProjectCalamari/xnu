...
#define PSEMHASH(table, hash)                                                  \
  (&(table).psem_table[(hash) & (table).psem_table_mask])
...
SYSCTL_NODE(_kern_posix, OID_AUTO, sem, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "Semaphores");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, max, CTLFLAG_RW | CTLFLAG_LOCKED,
            &posix_sem_max, "max");
...
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, miss, CTLFLAG_RD, &psemstats.pstats_miss,
            "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, local_hits, CTLFLAG_RD,
            &psemstats.pstats_local_hits, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, local_miss, CTLFLAG_RD,
            &psemstats.pstats_local_miss, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, global_hits, CTLFLAG_RD,
            &psemstats.pstats_global_hits, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, global_miss, CTLFLAG_RD,
            &psemstats.pstats_global_miss, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, fallback_hits, CTLFLAG_RD,
            &psemstats.pstats_fallback_hits, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, fallback_miss, CTLFLAG_RD,
            &psemstats.pstats_fallback_miss, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, local_collisions, CTLFLAG_RD,
            &psemstats.pstats_local_collisions, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, global_collisions, CTLFLAG_RD,
            &psemstats.pstats_global_collisions, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, neghits, CTLFLAG_RD,
            &psemstats.pstats_neghits, "");
SYSCTL_LONG(_kern_posix_sem, OID_AUTO, longnames, CTLFLAG_RD,
            &psemstats.pstats_longnames, "");
#endif

static int psem_access(struct pseminfo *pinfo, mode_t mode, kauth_cred_t cred);
static int psem_cache_search(struct pseminfo **, struct psemname *,
                             struct psemcache **);
static int psem_delete(struct pseminfo *pinfo);

static int psem_closefile(struct fileglob *fp, vfs_context_t ctx);
static int psem_unlink_internal(struct pseminfo *pinfo,
...
#define PSEM_SUBSYS_ASSERT_HELD()                                              \
  LCK_MTX_ASSERT(&psx_sem_subsys_mutex, LCK_MTX_ASSERT_OWNED)

static int psem_cache_add(struct pseminfo *psemp, struct psemname *pnp,
                          struct psemcache *pcp);
static void psem_cache_delete(struct psemcache *pcp);
int psem_cache_purge_all(void);

static struct psemname psem_cache_hash(char *name, size_t len,
                                       const char *teamid, size_t teamidlen) {
  SIPHASH_CTX ctx;
  struct psemname nd;
  ...
  _Static_assert(sizeof(nd.psem_hash_local) == SIPHASH_DIGEST_LENGTH,
                 "hash field is wrong size for SipHash");
  _Static_assert(sizeof(nd.psem_hash_global) == SIPHASH_DIGEST_LENGTH,
                 "hash field is wrong size for SipHash");

  /*
   * This routine is called before taking the subsystem lock, so we'll prepare
   * hashes for both global and local tables up front.
   */
  SipHash24_Init(&ctx);
 ...
 */
static int psem_cache_is_match(struct psemcache *sem, struct psemname *target,
                               bool local) {
  bool name_matches =
      target->psem_namelen == sem->psem_nlen &&
      !bcmp(target->psem_nameptr, sem->psem_name, target->psem_namelen);

  if (local) {
    bool teamid_matches =
        target->psem_teamidlen == sem->psem_teamidlen &&
...
static int psem_cache_search(struct pseminfo **psemp, struct psemname *pnp,
                             struct psemcache **pcache) {
...
void psem_cache_init(void) {
   ...
   * ID (such as platform binaries). The local table stores all other
   * semaphores.
   */
  psem_global.psem_table =
      hashinit((int)(posix_sem_max / 2), M_SHM, &psem_global.psem_table_mask);
  psem_local.psem_table =
      hashinit((int)(posix_sem_max / 2), M_SHM, &psem_local.psem_table_mask);

  read_frandom(psem_global.psem_siphash_key,
               sizeof(psem_global.psem_siphash_key));
  read_frandom(psem_local.psem_siphash_key,
               sizeof(psem_local.psem_siphash_key));
}
...
static int psem_cache_purge_table(struct psemhashtable *table) {
  struct psemcache *pcp, *tmppcp;
  struct psemhashhead *pcpp;

  for (pcpp = &table->psem_table[table->psem_table_mask];
       pcpp >= table->psem_table; pcpp--) {
 ...
 *		issues.
 */
int sem_open(proc_t p, struct sem_open_args *uap, user_addr_t *retval) {
  int indx, error;
  struct psemname nd;
  struct pseminfo *pinfo;
  ...
  if ((fmode & O_CREAT)) {
    ...
    kret = semaphore_create(kernel_task, &new_pinfo->psem_semobject,
...
static int psem_access(struct pseminfo *pinfo, mode_t mode, kauth_cred_t cred) {
  mode_t mode_req =
      ((mode & FREAD) ? S_IRUSR : 0) | ((mode & FWRITE) ? S_IWUSR : 0);
  ...
  return posix_cred_access(cred, pinfo->psem_uid, pinfo->psem_gid,
                           pinfo->psem_mode, mode_req);
}
...
static int psem_unlink_internal(struct pseminfo *pinfo,
  ...
  AUDIT_ARG(posix_ipc_perm, pinfo->psem_uid, pinfo->psem_gid, pinfo->psem_mode);
...
int sem_unlink(__unused proc_t p, struct sem_unlink_args *uap,
...
int sem_close(proc_t p, struct sem_close_args *uap, __unused int32_t *retval) {
...
int sem_wait(proc_t p, struct sem_wait_args *uap, int32_t *retval) {
  __pthread_testcancel(1);
  return sem_wait_nocancel(p, (struct sem_wait_nocancel_args *)uap, retval);
}

int sem_wait_nocancel(proc_t p, struct sem_wait_nocancel_args *uap,
                      __unused int32_t *retval) {
  int fd = CAST_DOWN_EXPLICIT(int, uap->sem);
  struct fileproc *fp;
  struct pseminfo *pinfo;
  ...
  if ((pinfo->psem_flags & (PSEM_DEFINED | PSEM_ALLOCATED)) != PSEM_ALLOCATED) {
    PSEM_SUBSYS_UNLOCK();
    error = EINVAL;
    goto out;
  }
...
int sem_trywait(proc_t p, struct sem_trywait_args *uap,
                __unused int32_t *retval) {
  int fd = CAST_DOWN_EXPLICIT(int, uap->sem);
  struct fileproc *fp;
  struct pseminfo *pinfo;
...
int sem_post(proc_t p, struct sem_post_args *uap, __unused int32_t *retval) {
  int fd = CAST_DOWN_EXPLICIT(int, uap->sem);
  struct fileproc *fp;
  struct pseminfo *pinfo;
  struct psemnode *pnode;
...
static int psem_close(struct psemnode *pnode) {
  int error = 0;
  struct pseminfo *pinfo;
...
static int psem_closefile(struct fileglob *fg, __unused vfs_context_t ctx) {
  /*
...
int fill_pseminfo(struct psemnode *pnode, struct psem_info *info) {
  struct pseminfo *pinfo;
...
void psem_label_associate(struct fileproc *fp, struct vnode *vp,
  ...
  if (pnode != NULL) {
    psem = pnode->pinfo;
    if (psem != NULL) {
      mac_posixsem_vnode_label_associate(vfs_context_ucred(ctx), psem,
                                         mac_posixsem_label(psem), vp,
                                         mac_vnode_label(vp));
    }
  }
  PSEM_SUBSYS_UNLOCK();
