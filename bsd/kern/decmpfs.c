...
static char *vnpath(vnode_t vp, char *path, int len) {
  int origlen = len;
  path[0] = 0;
...
#if COMPRESSION_DEBUG
#define ErrorLogWithPath(x, args...)                                           \
  do {                                                                         \
...
#define VerboseLogWithPath(x...)                                               \
  do {                                                                         \
  } while (0)
#endif

#define decmpfs_ktriage_record(code, arg)                                      \
  ktriage_record(thread_tid(current_thread()),                                 \
                 KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_DECMPFS,               \
                                     KDBG_TRIAGE_RESERVED, code),              \
                 arg);
...
const char *ktriage_decmpfs_strings[] = {
    ...
    [KTRIAGE_DECMPFS_FETCH_UNCOMPRESSED_DATA_FAILED] =
        "failed to fetch uncompressed data\n",
};
...
static const decmpfs_registration
    *decompressors[CMP_MAX]; /* the registered compressors */
static int
    decompress_channel; /* channel used by decompress_file to wake up waiters */
...
static void *_func_from_offset(uint32_t type, uintptr_t offset,
  ...
  if (ptr != NULL) {
    ...
    ptr = ptrauth_auth_and_resign(ptr, ptrauth_key_asia, discriminator,
                                  ptrauth_key_asia, 0);
  }
  return ptr;
}
...
static void *_decmp_get_func(vnode_t vp, uint32_t type, uintptr_t offset,
                             uint32_t discriminator) {
  /*
   *  this function should be called while holding a shared lock to
   * decompressorsLock, and will return with the lock held
   */
  ...
  if (IOCatalogueMatchingDriversPresent(providesName)) {
    // there is a kext that says it will register for this type, so let's wait
    // for it
    ...
    while (decompressors[type] == NULL) {
      lck_rw_unlock_shared(&decompressorsLock); // we have to unlock to allow
                                                // the kext to register
  ...
  // the compressor hasn't registered, so it never will unless someone manually
  // kextloads it
  ErrorLogWithPath(
...
#define decmp_get_func(vp, type, func)                                         \
  (typeof(decompressors[0]->func))_decmp_get_func(                             \
      vp, type, offsetof_func(func),                                           \
      ptrauth_function_pointer_type_discriminator(                             \
          typeof(decompressors[0]->func)))

#pragma mark--- utilities ---

#if COMPRESSION_DEBUG
static int vnsize(vnode_t vp, uint64_t *size) {
...
ZONE_DEFINE(decmpfs_cnode_zone, "decmpfs_cnode", sizeof(struct decmpfs_cnode),
            ZC_NONE);

decmpfs_cnode *decmpfs_cnode_alloc(void) { return zalloc(decmpfs_cnode_zone); }

void decmpfs_cnode_free(decmpfs_cnode *dp) { zfree(decmpfs_cnode_zone, dp); }
...
void decmpfs_unlock_compressed_data(decmpfs_cnode *cp, __unused int exclusive) {
  void *thread = current_thread();
...
void decmpfs_cnode_set_vnode_state(decmpfs_cnode *cp, uint32_t state,
                                   int skiplock) {
  if (!skiplock) {
...
static void decmpfs_cnode_set_vnode_cmp_type(decmpfs_cnode *cp,
                                             uint32_t cmp_type, int skiplock) {
  if (!skiplock) {
    decmpfs_lock_compressed_data(cp, 1);
  }
...
static void decmpfs_cnode_set_vnode_minimal_xattr(decmpfs_cnode *cp,
                                                  int minimal_xattr,
                                                  int skiplock) {
  if (!skiplock) {
    decmpfs_lock_compressed_data(cp, 1);
...
void decmpfs_cnode_set_vnode_cached_nchildren(decmpfs_cnode *cp,
                                              uint64_t nchildren) {
  while (1) {
...
void decmpfs_cnode_set_vnode_cached_total_size(decmpfs_cnode *cp,
                                               uint64_t total_sz) {
  while (1) {
...
static uint64_t decmpfs_cnode_get_decompression_flags(decmpfs_cnode *cp) {
  return cp->decompression_flags;
}

static void decmpfs_cnode_set_decompression_flags(decmpfs_cnode *cp,
                                                  uint64_t flags) {
  while (1) {
...
uint32_t decmpfs_cnode_cmp_type(decmpfs_cnode *cp) { return cp->cmp_type; }

#pragma mark--- decmpfs state routines ---

static int decmpfs_fetch_compressed_header(vnode_t vp, decmpfs_cnode *cp,
                                           decmpfs_header **hdrOut,
                                           int returnInvalid,
                                           size_t *hdr_size) {
  /*
   *  fetches vp's compression xattr, converting it into a decmpfs_header;
   * returns 0 or errno if returnInvalid == 1, returns the header even if the
   * type was invalid (out of range), and return ERANGE in that case
   */
  ...
  int err = 0;
  char *data = NULL;
  const bool no_additional_data =
      ((cp != NULL) && (cp->cmp_type != 0) && (cp->cmp_minimal_xattr != 0));
  ...
  if (no_additional_data) {
    /* this file's xattr didn't have any extra data when we fetched it, so we
     * can synthesize a header from the data in the cnode */
    ...
    err = vn_getxattr(vp, DECMPFS_XATTR_NAME, NULL, &attr_size,
                      XATTR_NOSECURITY, decmpfs_ctx);
    if (err != 0) {
      goto out;
    }
    alloc_size = attr_size + sizeof(hdr->attr_size);

    if (attr_size < sizeof(decmpfs_disk_header) ||
        attr_size > MAX_DECMPFS_XATTR_SIZE) {
      err = EINVAL;
      goto out;
    }

    /* allocation includes space for the extra attr_size field of a
     * compressed_header */
    ...
    /* read the xattr into our buffer, skipping over the attr_size field at the
     * beginning */
    attr_uio = uio_createwithbuffer(1, 0, UIO_SYSSPACE, UIO_READ, &uio_buf[0],
                                    sizeof(uio_buf));
    uio_addiov(attr_uio, CAST_USER_ADDR_T(data + sizeof(hdr->attr_size)),
               attr_size);

    err = vn_getxattr(vp, DECMPFS_XATTR_NAME, attr_uio, &read_size,
                      XATTR_NOSECURITY, decmpfs_ctx);
    if (err != 0) {
      goto out;
    }
  ...
  if (hdr->compression_type >= CMP_MAX &&
      !decmpfs_type_is_dataless(hdr->compression_type)) {
    if (returnInvalid) {
...
static int decmpfs_fast_get_state(decmpfs_cnode *cp) {
  /*
   *  return the cached state
   *  this should *only* be called when we know that decmpfs_file_is_compressed
   * has already been called, because this implies that the cached state is
   * valid
   */
  ...
  case FILE_TYPE_UNKNOWN:
    /*
     *  we should only get here if decmpfs_file_is_compressed was not called
     * earlier on this vnode, which should not be possible
     */
...
static int decmpfs_fast_file_is_compressed(decmpfs_cnode *cp) {
  int cmp_state = decmpfs_cnode_get_vnode_state(cp);
...
errno_t decmpfs_validate_compressed_file(vnode_t vp, decmpfs_cnode *cp) {
  ...
  if (!decmpfs_type_is_dataless(hdr->compression_type)) {
    ...
    decmpfs_validate_compressed_file_func validate =
        decmp_get_func(vp, hdr->compression_type, validate);
...
int decmpfs_file_is_compressed(vnode_t vp, decmpfs_cnode *cp) {
   ...
   *
   *  to speed up this operation, we cache the result in the cnode, and do as
   * little as possible in the case where the cnode already has a valid cached
   * state
   *
   */
  ...
  case FILE_IS_COMPRESSED:
    return 1;
  case FILE_IS_CONVERTING:
    /* treat the file as compressed, because this gives us a way to block future
     * reads until decompression is done */
    return 1;
  case FILE_TYPE_UNKNOWN:
   ...
   */
  DECMPFS_EMIT_TRACE_ENTRY(DECMPDBG_FILE_IS_COMPRESSED, vp->v_id, is_mounted,
                           is_local_fs);
  ...
  /* lock our cnode data so that another caller doesn't change the state under
   * us */
  decmpfs_lock_compressed_data(cp, 1);
  cnode_locked = 1;
  ...
  if (((ret == FILE_IS_COMPRESSED) || saveInvalid) && hdr) {
    ...
    if (!cnode_locked) {
      /*
       *  we should never get here since the only place ret is set to
       * FILE_IS_COMPRESSED is after the call to decmpfs_lock_compressed_data
       * above
       */
      decmpfs_lock_compressed_data(cp, 1);
      cnode_locked = 1;
    ...
    if (vnode_isdir(vp)) {
      ...
      decmpfs_cnode_set_vnode_cached_nchildren(
          cp, decmpfs_get_directory_entries(hdr));
      if (hdr->compression_type == DATALESS_PKG_CMPFS_TYPE) {
        decmpfs_cnode_set_vnode_cached_total_size(cp,
                                                  DECMPFS_PKG_SIZE(hdr->_size));
      }
    } else {
    ...
    if (ret == FILE_IS_COMPRESSED) {
      ...
      decmpfs_get_decompression_flags_func get_flags =
          decmp_get_func(vp, hdr->compression_type, get_flags);
      if (get_flags) {
    ...
    /* we might have already taken the lock above; if so, skip taking it again
     * by passing cnode_locked as the skiplock parameter */
  ...
  if (VATTR_IS_ACTIVE(vap, va_flags)) {
    ...
    if (vap->va_flags & UF_COMPRESSED) {
      ...
      if (!(old_flags & UF_COMPRESSED)) {
        /*
         * Compression bit was turned on, make sure the file has the
         * DECMPFS_XATTR_NAME attribute. This precludes anyone from using the
         * UF_COMPRESSED bit for anything else, and it enforces an order of
         * operation -- you must first do the setxattr and then the chflags.
         */

        if (VATTR_IS_ACTIVE(vap, va_data_size)) {
          /*
           * don't allow the caller to set the BSD flag and the size in the same
           * call since this doesn't really make sense
           */
        ...
        if (error == 0) {
          ...
          /* the file had a decmpfs attribute but the type was out of range, so
           * don't muck with the file's data size */
        } else {
...
static int wait_for_decompress(decmpfs_cnode *cp) {
  int state;
  ...
  do {
    ...
    msleep((caddr_t)&decompress_channel, &decompress_channel_mtx, PINOD,
...
int decmpfs_hides_rsrc(vfs_context_t ctx, decmpfs_cnode *cp) {
   ...
   *
   *  This routine should only be called after a file has already been through
...
int decmpfs_hides_xattr(vfs_context_t ctx, decmpfs_cnode *cp,
                        const char *xattr) {
  /*
   *  WARNING!!!
   *  callers may (and do) pass NULL for ctx, so we should only use it
  ...
  if (strncmp(xattr, XATTR_RESOURCEFORK_NAME,
              sizeof(XATTR_RESOURCEFORK_NAME) - 1) == 0) {
...
static inline int registration_valid(const decmpfs_registration *registration) {
  return registration && ((registration->decmpfs_registration ==
                           DECMPFS_REGISTRATION_VERSION_V1) ||
                          (registration->decmpfs_registration ==
                           DECMPFS_REGISTRATION_VERSION_V3));
}

errno_t
register_decmpfs_decompressor(uint32_t compression_type,
                              const decmpfs_registration *registration) {
  ...
  lck_rw_lock_exclusive(&decompressorsLock);
  locked = 1;
...
errno_t unregister_decmpfs_decompressor(uint32_t compression_type,
...
static int compression_type_valid(vnode_t vp, decmpfs_header *hdr) {
...
static int decmpfs_fetch_uncompressed_data(vnode_t vp, decmpfs_cnode *cp,
                                           decmpfs_header *hdr, off_t offset,
                                           user_ssize_t size, int nvec,
                                           decmpfs_vector *vec,
                                           uint64_t *bytes_read) {
  /* get the uncompressed bytes for the specified region of vp by calling out to
  ...
  *bytes_read = 0;
  ...
  decmpfs_fetch_uncompressed_data_func fetch =
      decmp_get_func(vp, hdr->compression_type, fetch);
  if (fetch) {
    ...
    if (err == 0) {
      ...
      if (decompression_flags & DECMPFS_FLAGS_FORCE_FLUSH_ON_DECOMPRESS) {
        ...
        for (i = 0; i < nvec; i++) {
          ...
          flush_dcache64((addr64_t)(uintptr_t)vec[i].buf,
                         (unsigned int)vec[i].size, FALSE);
        }
#endif
      }
    } else {
    ...
    decmpfs_ktriage_record(KTRIAGE_DECMPFS_COMPRESSOR_NOT_REGISTERED,
                           hdr->compression_type);
    err = ENOTSUP;
    lck_rw_unlock_shared(&decompressorsLock);
  }
...
static kern_return_t commit_upl(upl_t upl, upl_offset_t pl_offset,
                                size_t uplSize, int flags, int abort) {
  kern_return_t kr = 0;
  ...
  if (abort) {
    ...
    kr = ubc_upl_commit_range(upl, pl_offset, (upl_size_t)uplSize,
                              flags | UPL_COMMIT_CLEAR_DIRTY |
...
errno_t decmpfs_pagein_compressed(struct vnop_pagein_args *ap,
                                  int *is_compressed, decmpfs_cnode *cp) {
   ...
   */
  if (f_offset < 0 || f_offset >= cachedSize || (f_offset & PAGE_MASK_64) ||
      (size & PAGE_MASK) || (pl_offset & PAGE_MASK)) {
  ...
  if (err) {
    ...
    if (verify_block_size & (verify_block_size - 1)) {
      ErrorLogWithPath("verify block size (%zu) is not power of 2, no "
                       "verification will be done\n",
                       verify_block_size);
      err = EINVAL;
    } else if (size % verify_block_size) {
      ErrorLogWithPath(
          "upl size (%zu) is not a multiple of verify block size (%zu)\n",
      ...
      if (fs_bsize & (fs_bsize - 1)) {
        ErrorLogWithPath(
            "FS block size (%u) is greater than PAGE_SIZE (%d) and is not "
            "power of 2, no verification will be done\n",
            fs_bsize, PAGE_SIZE);
        err = EINVAL;
      } else if (fs_bsize > verify_block_size) {
        ErrorLogWithPath("FS block size (%u) is greater than verify block size "
                         "(%zu), no verification will be done\n",
...
decompress:
  /* the mapped data pointer points to the first page of the page list, so we
   * want to start filling in at an offset of pl_offset */
  vec = (decmpfs_vector){
  ...
  if (decmpfs_fast_get_state(cp) == FILE_IS_CONVERTING) {
    ...
    /*
     *  if the file is converting, this must be a recursive call to pagein from
     * underneath a call to decmpfs_decompress_file; pretend that it succeeded
     * but don't do anything since we're just going to write over the pages
     * anyway
     */
    err = 0;
  } else {
    if (verify_block_size <= PAGE_SIZE) {
      err = decmpfs_fetch_uncompressed_data(vp, cp, hdr, uplPos, uplSize, 1,
                                            &vec, &did_read);
      /* zero out whatever wasn't read */
      if (did_read < rounded_uplSize) {
        memset((char *)vec.buf + did_read, 0,
               (size_t)(rounded_uplSize - did_read));
      }
    } else {
      ...
      while (l_uplSize) {
         ...
         */
        pages_left_in_upl =
            (int)(round_page((vm_offset_t)l_uplSize) / PAGE_SIZE);
        ...
        if (start_pg < last_pg) {
          ...
          err = decmpfs_fetch_uncompressed_data(vp, cp, hdr, l_uplPos,
                                                MIN(l_uplSize, inval_size), 1,
                                                &l_vec, &l_did_read);

          if (!err && (l_did_read != inval_size) && (l_uplSize > inval_size)) {
            ErrorLogWithPath(
                "Unexpected size fetch of decompressed data, l_uplSize = %d, "
                "l_did_read = %d, inval_size = %d\n",
      ...
      if (!err && !file_tail_page_valid && (uplSize < rounded_uplSize)) {
        memset((char *)vec.buf + uplSize, 0,
               (size_t)(rounded_uplSize - uplSize));
      }
    }
  }
  if (err) {
    ...
    if (cmp_state == FILE_IS_CONVERTING) {
      ...
      if (cmp_state == FILE_IS_COMPRESSED) {
        ...
        /* a decompress was attempted but it failed, let's try calling fetch
         * again */
        goto decompress;
      }
    }
  ...
  if (!err && verify_block_size) {
    ...
    if ((err = VNOP_VERIFY(vp, uplPos, vec.buf, rounded_uplSize,
                           &cur_verify_block_size, NULL, 0, NULL, NULL))) {
      ErrorLogWithPath("Verification failed with error %d, uplPos = %lld, "
                       "uplSize = %d, did_read = %d, valid_pages = %d, "
                       "invalid_pages = %d, tail_page_valid = %d\n",
                       err, (long long)uplPos, (int)rounded_uplSize,
                       (int)did_read, num_valid_pages, num_invalid_pages,
  ...
  kr = ubc_upl_unmap(pl);
  data = NULL; /* make sure to set data to NULL so we don't try to unmap again
                  below */
  if (kr != KERN_SUCCESS) {
    ...
    if (!err) {
      /* commit our pages */
      kr = commit_upl(pl, pl_offset, (size_t)rounded_uplSize,
                      UPL_COMMIT_FREE_ON_EMPTY, 0 /* commit */);
      /* If there were any pages after the page containing EOF, abort them. */
      if (rounded_uplSize < size) {
        kr = commit_upl(pl, (upl_offset_t)(pl_offset + rounded_uplSize),
                        (size_t)(size - rounded_uplSize),
...
errno_t decmpfs_read_compressed(struct vnop_read_args *ap, int *is_compressed,
                                decmpfs_cnode *cp) {
  ...
  decmpfs_lock_compressed_data(cp, 0);
  cmpdata_locked = 1;
  ...
  decmpfs_adjust_fetch_region_func adjust_fetch =
      decmp_get_func(vp, hdr->compression_type, adjust_fetch);
  if (adjust_fetch) {
    /* give the compressor a chance to adjust the portion of the file that we
     * read */
  ...
  /* If the verify block size is larger than the page size, the UPL needs to
   * aligned to it */
  err = VNOP_VERIFY(vp, uplPos, NULL, 0, &verify_block_size, NULL,
                    VNODE_VERIFY_DEFAULT, NULL, NULL);
  if (err) {
    goto out;
  } else if (verify_block_size) {
    if (verify_block_size & (verify_block_size - 1)) {
      ErrorLogWithPath("verify block size is not power of 2, no verification "
                       "will be done\n");
      verify_block_size = 0;
  ...
  while (uplRemaining > 0) {
    ...
    /* create the upl */
    kr = ubc_create_upl_kernel(vp, curUplPos, (int)curUplSize, &upl, &pli,
                               UPL_SET_LITE, VM_KERN_MEMORY_FILE);
    if (kr != KERN_SUCCESS) {
  ...
  decompress:
    ...
    err = decmpfs_fetch_uncompressed_data(vp, cp, hdr, curUplPos, curUplSize, 1,
                                          &vec, &did_read);
    if (err) {
    ...
    if (!err && verify_block_size) {
      ...
      if ((err = VNOP_VERIFY(vp, curUplPos, data, curUplSize,
                             &cur_verify_block_size, NULL, 0, NULL, NULL))) {
    ...
    if (abort_read) {
        ...
        if (uplOff < 0) {
          ...
          if (count < 0) {
            ...
            VerboseLogWithPath("uplOff %lld count %lld copied %lld\n",
                               (uint64_t)uplOff, (uint64_t)count,
                               (uint64_t)copied);
            if (err) {
      ...
      kr = commit_upl(upl, 0, curUplSize,
                      UPL_COMMIT_FREE_ON_EMPTY | UPL_COMMIT_INACTIVATE, 0);
    }
...
int decmpfs_free_compressed_data(vnode_t vp, decmpfs_cnode *cp) {
  /*
   *  call out to the decompressor to free remove any data associated with this
   * compressed file then delete the file's compression xattr
   */
  decmpfs_header *hdr = NULL;
  size_t alloc_size = 0;
  ...
  if (err) {
    ...
    decmpfs_free_compressed_data_func free_data =
        decmp_get_func(vp, hdr->compression_type, free_data);
    if (free_data) {
...
static int unset_compressed_flag(vnode_t vp) {
  int err = 0;
  struct vnode_attr va;
...
#define MAX_CAS_BSDFLAGS_LOOPS 4
  ...
  for (i = 0; i < MAX_CAS_BSDFLAGS_LOOPS; i++) {
    ...
    err =
        VNOP_IOCTL(vp, FSIOC_CAS_BSDFLAGS, (caddr_t)&cas, FWRITE, decmpfs_ctx);
  ...
  if (err == ENOTTY) {
    ...
    ErrorLogWithPath("FSIOC_CAS_BSDFLAGS err: flags mismatc. actual (%x) "
                     "expected (%x), num retries %d\n",
...
int decmpfs_decompress_file(vnode_t vp, decmpfs_cnode *cp, off_t toSize,
                            int truncate_okay, int skiplock) {
   ...
   */
  DECMPFS_EMIT_TRACE_ENTRY(DECMPDBG_DECOMPRESS_FILE, vp->v_id, (int)toSize,
                           truncate_okay, skiplock);
...
decompress:
  ...
  default: {
    /*
     *  this shouldn't happen since multiple calls to decmpfs_decompress_file
     * lock each other out, and when decmpfs_decompress_file returns, the state
     * should be always be set back to FILE_IS_NOT_COMPRESSED or FILE_IS_UNKNOWN
     */
    err = EINVAL;
    goto out;
  }
  ...
  if (toSize == 0) {
    ...
    /* the caller is trying to grow the file, so we should decompress all the
     * data */
    toSize = hdr->uncompressed_size;
  }
  ...
  while (remaining > 0) {
    ...
    decmpfs_vector vec = {.buf = data,
                          .size = (user_ssize_t)MIN(allocSize, remaining)};
    err = decmpfs_fetch_uncompressed_data(vp, cp, hdr, offset, vec.size, 1,
                                          &vec, &bytes_read);
    if (err != 0) {
      ErrorLogWithPath("decmpfs_fetch_uncompressed_data err %d\n", err);
      goto out;
...
nodecmp:
  ...
  /*
   *  even if free_compressed_data or vnode_getattr/vnode_setattr failed, return
   * success since we succeeded in writing all of the file data to the data fork
   */
  err = 0;
...
out:
  ...
  if (err != 0) {
    /* if there was a failure, reset compression flags to unknown and clear the
     * buffer cache data */
    update_file_state = 1;
  ...
  if (update_file_state) {
    ...
    wakeup((caddr_t)&decompress_channel); /* wake up anyone who might have been
...
/*
 *  The "Type1" compressor stores the data fork directly in the compression
 * xattr
 */

static int decmpfs_validate_compressed_file_Type1(__unused vnode_t vp,
                                                  __unused vfs_context_t ctx,
                                                  decmpfs_header *hdr) {
  int err = 0;

  if (hdr->uncompressed_size + sizeof(decmpfs_disk_header) !=
      (uint64_t)hdr->attr_size) {
    err = EINVAL;
    goto out;
  }
out:
...
static int decmpfs_fetch_uncompressed_data_Type1(
    __unused vnode_t vp, __unused vfs_context_t ctx, decmpfs_header *hdr,
    off_t offset, user_ssize_t size, int nvec, decmpfs_vector *vec,
    uint64_t *bytes_read) {
  int err = 0;
  int i;
...
SECURITY_READ_ONLY_EARLY(static decmpfs_registration)
Type1Reg = {
...
void decmpfs_init(void) {
  ...
  ktriage_register_subsystem_strings(KDBG_TRIAGE_SUBSYS_DECMPFS,
                                     &ktriage_decmpfs_subsystem_strings);
