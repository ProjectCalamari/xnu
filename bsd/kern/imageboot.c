...
#define AUTHDBG(fmt, args...)                                                  \
  do {                                                                         \
    printf("%s: " fmt "\n", __func__, ##args);                                 \
  } while (0)
#define AUTHPRNT(fmt, args...)                                                 \
  do {                                                                         \
    printf("%s: " fmt "\n", __func__, ##args);                                 \
  } while (0)

extern int di_root_image_ext(const char *path, char *devname, size_t devsz,
                             dev_t *dev_p, bool removable);
extern int di_root_image(const char *path, char *devname, size_t devsz,
                         dev_t *dev_p);
extern int di_root_ramfile_buf(void *buf, size_t bufsz, char *devname,
                               size_t devsz, dev_t *dev_p);

static boolean_t imageboot_setup_new(imageboot_type_t type);

void *ubc_getobject_from_filename(const char *filename, struct vnode **vpp,
                                  off_t *file_size);
...
__private_extern__ int imageboot_format_is_valid(const char *root_path) {
  return strncmp(root_path, kIBFilePrefix, strlen(kIBFilePrefix)) == 0;
}

static void vnode_get_and_drop_always(vnode_t vp) {
  vnode_getalways(vp);
  vnode_rele(vp);
  vnode_put(vp);
}

__private_extern__ bool imageboot_desired(void) {
   ...
   * is not requested.
   *
   * [!! Note parens guard the entire logically OR'd set of statements, below.
   * It validates that NONE of the below-mentioned boot-args is present...!!]
   */
...
__private_extern__ imageboot_type_t imageboot_needed(void) {
  ...
  if (imageboot_format_is_valid(root_path)) {
    ...
    panic("%s: Invalid URL scheme for %s", __FUNCTION__, root_path);
  }
 ...
 * this function begins) will be at outgoing_root_path.  If
 * `skip_signature_check` is true, then ignore the chunklisted or authAPFS
 * checks on this image
 */
__private_extern__ int
imageboot_pivot_image(const char *image_path, imageboot_type_t type,
                      const char *mount_path, const char *outgoing_root_path,
                      const bool rooted_dmg, const bool skip_signature_check) {
  int error = 0;
  ...
  if (error) {
     ...
     * since it may result in natural consequences (ergo, cannot unlock
     * filevault prompt).
     */
    return error;
  }
  ...
  (void)PE_parse_boot_argn("-bsdmgroot-ramdisk", &ramdisk_arg,
                           sizeof(ramdisk_arg));
...
#define PIVOTMNT "/System/Volumes/BaseSystem"
  ...
  error = kernel_mount("hfs", NULLVP, NULLVP, PIVOTMNT, fulldevnamepp, 0,
                       (MNT_RDONLY | MNT_DONTBROWSE),
                       (KERNEL_MOUNT_NOAUTH | KERNEL_MOUNT_BASESYSTEMROOT),
                       vfs_context_kernel());
  if (error) {
    printf("mount failed: %d\n", error);
    printf("%s: attempting to mount as apfs...\n", __FUNCTION__);
    error = kernel_mount("apfs", NULLVP, NULLVP, PIVOTMNT, fulldevnamepp, 0,
                         (MNT_RDONLY | MNT_DONTBROWSE),
                         (KERNEL_MOUNT_NOAUTH | KERNEL_MOUNT_BASESYSTEMROOT),
                         vfs_context_kernel());
  }
  ...
  if (error) {
    /*
     * Note that for this particular failure case (failure to mount), the disk
     * image being attached may have failed to quiesce within the alloted time
     * out (20-30 sec). For example, it may be still probing, or APFS container
     * enumeration may have not completed. If so, then we may have fallen into
     * this particular error case. However, failure to complete matching should
     * be an exceptional case as 30 sec. is quite a long time to wait for
     * matching to complete (which would have occurred in di_root_image_ext).
     */
  ...
  /* otherwise, if the mount succeeded, then assert that the DMG is
   * authenticated (either chunklist or authapfs) */
  ...
  rootauth = VNOP_IOCTL(mount_vp, FSIOC_KERNEL_ROOTAUTH, (caddr_t)0x1, 0,
                        vfs_context_kernel());
  if (rootauth) {
     ...
     * Or, if CSR_ALLOW_ANY_RECOVERY_OS is set on Development or Debug build
     * variant.
     */
  ...
  if (error) {
     ...
     * This means that the disk image either was not sealed (authapfs), or it
     * was not hosted on a chunklisted DMG.  Both scenarios may be fatal
...
static void set_fake_bootuuid(mount_t mp) {
  struct vfs_attr va;
  VFSATTR_INIT(&va);
 ...
 */
__private_extern__ int imageboot_mount_image(const char *root_path, int height,
  ...
  DBG_TRACE("%s: old root fsname: %s\n", __FUNCTION__,
            old_rootvnode->v_mount->mnt_vtable->vfc_name);
...
void *ubc_getobject_from_filename(const char *filename, struct vnode **vpp,
  ...
  NDINIT(&ndp, LOOKUP, OP_OPEN, LOCKLEAF, UIO_SYSSPACE,
...
static int imageboot_read_file_internal(const char *path, const off_t offset,
                                        const bool pageable, void **bufp,
                                        size_t *bufszp, off_t *fsizep,
                                        bool no_softlimit) {
  int err = 0;
  struct nameidata ndp = {};
  struct vnode *vp = NULL;
  ...
  NDINIT(&ndp, LOOKUP, OP_OPEN, LOCKLEAF | FOLLOW, UIO_SYSSPACE,
  ...
  /* if fsize is larger than the specified limit (presently 2.5GB) or a
   * NVRAM-configured limit, fail */
  maxsize = IMAGEBOOT_MAX_FILESIZE;
  PE_parse_boot_argn("rootdmg-maxsize", &maxsize, sizeof(maxsize));
  if (maxsize && (maxsize < (size_t)fsize)) {
    AUTHPRNT("file is too large (%lld > %lld)", (long long)fsize,
             (long long)maxsize);
    err = EFBIG;
    goto out;
  }
  ...
  if (pageable) {
    ...
    if (kmem_alloc(kernel_map, &addr, (vm_size_t)fsize, kma_flags,
                   VM_KERN_MEMORY_FILE) == KERN_SUCCESS) {
      buf = (char *)addr;
    } else {
      buf = NULL;
    }
  ...
  if (VNOP_GETNAMEDSTREAM(vp, &rsrc_vp, XATTR_RESOURCEFORK_NAME, NS_OPEN,
                          /*flags*/ 0, ctx) == 0) {
    vnode_ref(rsrc_vp);
    vnode_put(rsrc_vp);
  ...
  while (readsize > 0) {
     ...
     */
    if ((err = vn_rdwr(UIO_READ, vp, (caddr_t)readbuf, (int)chunksize, readoff,
                       UIO_SYSSPACE, IO_NODELOCKED | IO_NOCACHE | IO_RAOFF,
                       kerncred, /*resid*/ NULL, p)) != 0) {
      AUTHPRNT("Cannot read %lld bytes at offset %lld from %s - %d",
               (long long)chunksize, (long long)readoff, path, err);
      goto out;
    }

    /* evict cached pages so they don't accumulate during early boot */
    ubc_msync(vp, readoff, readoff + chunksize, NULL,
              UBC_INVALIDATE | UBC_PUSHALL);

    /* evict potentially-cached decmpfs data if we have a resource fork */
    if (rsrc_vp != NULL) {
      if (vnode_getwithref(rsrc_vp) == 0) {
        ubc_msync(rsrc_vp, 0, ubc_getsize(rsrc_vp), NULL,
                  UBC_INVALIDATE | UBC_PUSHALL);
        vnode_put(rsrc_vp);
      }
    }
...
int imageboot_read_file_from_offset(const char *path, const off_t offset,
                                    void **bufp, size_t *bufszp) {
...
vnode_t imgboot_get_image_file(const char *path, off_t *fsize, int *errp) {
  ...
  NDINIT(&ndp, LOOKUP, OP_OPEN, LOCKLEAF, UIO_SYSSPACE, CAST_USER_ADDR_T(path),
         ctx);
  if ((err = namei(&ndp)) != 0) {
...
static char *imgboot_get_apticket_path(const char *rootpath, size_t *sz) {
...
static int authenticate_root_with_img4(const char *rootpath) {
  errno_t rv;
  vnode_t vp = NULLVP;
  ...
  img4_firmware_flags_t fw_flags =
      IMG4_FIRMWARE_FLAG_BARE | IMG4_FIRMWARE_FLAG_SUBSEQUENT_STAGE;
 ...
 */
static int imageboot_mount_ramdisk(const char *path) {
  int err = 0;
  size_t bufsz = 0;
  void *buf = NULL;
  dev_t dev;
   ...
   */
  err =
      imageboot_read_file_pageable(path, &buf, &bufsz, /* no_softlimit */ true);
  if (err) {
 ...
 */
static char *url_to_path(char *url_path, size_t *sz) {
  char *path = url_path;
...
static boolean_t imageboot_setup_new(imageboot_type_t type) {
  ...
  if (PE_parse_boot_argn("-rootdmg-ramdisk", &imgboot_arg,
                         sizeof(imgboot_arg))) {
    ramdisk_root = TRUE;
  }

  if (PE_parse_boot_argn(IMAGEBOOT_CONTAINER_ARG, root_path, MAXPATHLEN) ==
      TRUE) {
  ...
  if (PE_parse_boot_argn(IMAGEBOOT_AUTHROOT_ARG, root_path, MAXPATHLEN) ==
          FALSE &&
      PE_parse_boot_argn(IMAGEBOOT_ROOT_ARG, root_path, MAXPATHLEN) == FALSE) {
    if (height > 0) {
      panic("%s specified without %s or %s?", IMAGEBOOT_CONTAINER_ARG,
            IMAGEBOOT_AUTHROOT_ARG, IMAGEBOOT_ROOT_ARG);
    }
    goto out;
  }
  ...
  if (error) {
    if (error == EFBIG) {
      panic("root imagefile is too large (err=%d, auth=%d, ramdisk=%d)", error,
            auth_root, ramdisk_root);
    } else {
      panic("Failed to mount root image (err=%d, auth=%d, ramdisk=%d)", error,
            auth_root, ramdisk_root);
    }
  }
...
__private_extern__ void imageboot_setup(imageboot_type_t type) {
  int error = 0;
  char *root_path = NULL;
  ...
  /*
   * New boot-arg scheme:
   *      root-dmg : the dmg that will be the root filesystem, authenticated by
   * default. auth-root-dmg : same as root-dmg. container-dmg : an optional dmg
   * that contains the root-dmg. locker : the locker that will be the root
   * filesystem -- mutually exclusive with any other boot-arg.
   */
   ...
   * there's some sense in which the outer image never needs to be the root
   * filesystem, but it does need very similar treatment: it must not be
   * unmounted, needs a fake device vnode created for it, and should not show up
   * in getfsstat() until exposed with MNT_IMGSRC. We just make it the temporary
   * root.
   */
#if CONFIG_IMAGEBOOT_IMG4
