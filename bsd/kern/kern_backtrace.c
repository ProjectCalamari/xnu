...
SYSCTL_PROC(_kern_backtrace, OID_AUTO, user, CTLFLAG_RW | CTLFLAG_LOCKED,
            (void *)BACKTRACE_USER, sizeof(uint64_t), backtrace_user_sysctl,
            "O", "take user backtrace of current thread");
...
static int backtrace_kernel_sysctl SYSCTL_HANDLER_ARGS {
  ...
  if (scenario == BACKTRACE_KERN_TEST_PACK_UNPACK) {
    packed_size =
        backtrace_pack(BTP_KERN_OFFSET_32, packed_bt, bt_size, bt, len);
  } else {
    packed_size = backtrace_packed(BTP_KERN_OFFSET_32, packed_bt, bt_size, NULL,
                                   &packed_info);
  }
  unsigned int unpacked_len = backtrace_unpack(BTP_KERN_OFFSET_32, unpacked_bt,
                                               bt_len, packed_bt, packed_size);
  if (unpacked_len != len) {
  ...
  for (unsigned int i = 0; i < len; i++) {
    if (unpacked_bt[i] != bt[i]) {
      printf("backtrace_tests: bad address %u: 0x%lx != 0x%lx unpacked", i,
             bt[i], unpacked_bt[i]);
      error = EINVAL;
    }
  }
...
#pragma unused(oidp, arg1, arg2)
  ...
  bt_len =
      req->oldlen > MAX_BACKTRACE ? MAX_BACKTRACE : (unsigned int)req->oldlen;
  bt_size = sizeof(bt[0]) * bt_len;
  bt = kalloc_data(bt_size, Z_WAITOK | Z_ZERO);
  if (!bt) {
    return ENOBUFS;
  ...
  if (resume_scenario) {
    ...
    btinfo = BTUINFO_INIT;
    unsigned int bt_more =
        backtrace_user(bt + bt_filled, bt_space, &ctl, &btinfo);
    error = btinfo.btui_error;
    if (error != 0) {
      goto out;
    }
  ...
  if (btinfo.btui_async_frame_addr != 0 && btinfo.btui_async_start_index != 0) {
    ...
    unsigned int async_filled =
        backtrace_user(bt + start_index, bt_left, &ctl, &btinfo);
    error = btinfo.btui_error;
    if (error != 0) {
      goto out;
    }
