 ...
 */
static int coalition_create_syscall(user_addr_t cidp, uint32_t flags) {
  int error = 0;
  kern_return_t kr;
  uint64_t cid;
  coalition_t coal;
  ...
  kr =
      coalition_create_internal(type, role, privileged, efficient, &coal, &cid);
  if (kr != KERN_SUCCESS) {
 ...
 */
static int coalition_request_terminate_syscall(user_addr_t cidp,
                                               uint32_t flags) {
  kern_return_t kr;
  int error = 0;
  uint64_t cid;
 ...
 */
static int coalition_reap_syscall(user_addr_t cidp, uint32_t flags) {
  kern_return_t kr;
  int error = 0;
  uint64_t cid;
  coalition_t coal;
...
coalition_info_resource_usage(coalition_t coal, user_addr_t buffer,
                              user_size_t bufsize) {
  kern_return_t kr;
...
static int __attribute__((noinline))
coalition_info_get_debug_info(coalition_t coal, user_addr_t buffer,
                              user_size_t bufsize) {
  kern_return_t kr;
...
static int coalition_info_set_name_internal(coalition_t coal,
                                            user_addr_t buffer,
                                            user_size_t bufsize) {
  int error;
...
static int coalition_info_efficiency(coalition_t coal, user_addr_t buffer,
                                     user_size_t bufsize) {
  int error = 0;
  if (coalition_type(coal) != COALITION_TYPE_JETSAM) {
    return EINVAL;
...
static int coalition_ledger_logical_writes_limit(coalition_t coal,
                                                 user_addr_t buffer,
                                                 user_size_t bufsize) {
  int error = 0;
  int64_t limit = 0;
...
int coalition_info(proc_t p, struct coalition_info_args *uap,
...
int coalition_ledger(__unused proc_t p,
                     __unused struct coalition_ledger_args *uap,
                     __unused int32_t *retval) {
  user_addr_t cidp = uap->cid;
  user_addr_t buffer = uap->buffer;
  user_addr_t bufsizep = uap->bufsize;
...
static int coalition_policy_set_suppress(coalition_t coal,
                                         coalition_policy_suppress_t value) {
  int error = 0;
  ...
  case COALITION_POLICY_SUPPRESS_NONE:
    kr = jetsam_coalition_set_policy(coal, TASK_POLICY_DARWIN_BG,
                                     TASK_POLICY_DISABLE);
...
static int coalition_policy_get_suppress(coalition_t coal, int32_t *retval) {
  int value = 0;
  kern_return_t kr =
      jetsam_coalition_get_policy(coal, TASK_POLICY_DARWIN_BG, &value);
...
int sys_coalition_policy_set(proc_t p, struct coalition_policy_set_args *uap,
                             __unused int32_t *retval) {
  ...
  case COALITION_POLICY_SUPPRESS:
    error =
        coalition_policy_set_suppress(coal, (coalition_policy_suppress_t)value);
    break;
  default:
    error = EINVAL;
    break;
...
int sys_coalition_policy_get(proc_t p, struct coalition_policy_get_args *uap,
                             int32_t *retval) {
  uint64_t cid = uap->cid;
  coalition_policy_flavor_t flavor = uap->flavor;
...
SYSCTL_PROC(_kern, OID_AUTO, coalitions,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            sysctl_coalition_get_ids, "Q", "coalition ids of a given process");
...
SYSCTL_PROC(_kern, OID_AUTO, coalition_roles,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            sysctl_coalition_get_roles, "I",
            "coalition roles of a given process");
...
#pragma unused(oidp, arg1, arg2)
  ...
  for (int t = 0; t < COALITION_NUM_TYPES; t++) {
    ...
    if (coal != COALITION_NULL) {
      ...
      coal_dbg("PID:%d, Coalition:%lld, type:%d, pgcount:%lld", pid,
               coalition_id(coal), t, pgcount[t]);
    }
  }
...
SYSCTL_PROC(_kern, OID_AUTO, coalition_page_count,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            sysctl_coalition_get_page_count, "Q",
            "coalition page count of a specified process");
...
SYSCTL_PROC(
    _kern, OID_AUTO, coalition_pid_list,
    CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
    sysctl_coalition_get_pid_list, "I",
    "list of PIDS which are members of the coalition of the current process");
...
SYSCTL_PROC(_kern, OID_AUTO, coalition_notify,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            sysctl_coalition_notify, "Q",
            "get/set coalition notification flag");

extern int unrestrict_coalition_syscalls;
SYSCTL_INT(_kern, OID_AUTO, unrestrict_coalitions, CTLFLAG_RW,
...
#pragma unused(oidp, arg1, arg2)
  ...
  case 2:
    ...
    break;
  case 3:
    kr = energy_id_report_energy(ENERGY_ID_SOURCE_GPU, (energy_id_t)value[1],
                                 (energy_id_t)value[2], value[3]);
    break;
  }
...
SYSCTL_PROC(_kern, OID_AUTO, coalition_gpu_energy_test,
            CTLFLAG_MASKED | CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            sysctl_coalition_gpu_energy_test, "Q", "test coalition gpu energy");
