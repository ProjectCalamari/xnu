...
static ZONE_DEFINE_TYPE(vsockpcb_zone, "vsockpcbzone", struct vsockpcb,
                        ZC_NONE);
...
static errno_t vsock_validate_entitlements(uint16_t protocol, struct proc *p) {
...
static uint32_t vsock_get_peer_space(struct vsockpcb *_Nonnull pcb) {
  VERIFY(pcb != NULL);
  return pcb->peer_buf_alloc - (pcb->tx_cnt - pcb->peer_fwd_cnt);
}

static struct vsockpcb *vsock_get_matching_pcb(struct vsock_address src,
  ...
  LIST_FOREACH(pcb, &vsockinfo[protocol].bound, bound) {
    // Source cid and port must match. Only destination port must match. (Allows
    // for a changing CID during migration)
    socket_lock(pcb->so, 1);
    if ((pcb->so->so_state & SS_ISCONNECTED ||
         pcb->so->so_state & SS_ISCONNECTING) &&
        pcb->local_address.cid == src.cid &&
        pcb->local_address.port == src.port &&
        pcb->remote_address.port == dst.port) {
      preferred = pcb;
      break;
    } else if ((pcb->local_address.cid == src.cid ||
                pcb->local_address.cid == VMADDR_CID_ANY) &&
               pcb->local_address.port == src.port) {
      match = pcb;
    }
...
static errno_t vsock_bind_address_if_free(struct vsockpcb *_Nonnull pcb,
                                          uint32_t local_cid,
                                          uint32_t local_port,
                                          uint32_t remote_cid,
  ...
  const bool check_remote =
      (remote_cid != VMADDR_CID_ANY && remote_port != VMADDR_PORT_ANY);
  ...
  LIST_FOREACH(pcb_match, &vsockinfo[protocol].bound, bound) {
    ...
    if (pcb == pcb_match ||
        ...
        (check_remote && pcb_match->local_address.port == local_port &&
         pcb_match->remote_address.cid == remote_cid &&
         pcb_match->remote_address.port == remote_port)) {
  ...
  if (!taken) {
    pcb->local_address =
        (struct vsock_address){.cid = local_cid, .port = local_port};
    pcb->remote_address =
        (struct vsock_address){.cid = remote_cid, .port = remote_port};
...
static errno_t vsock_bind_address(struct vsockpcb *pcb,
  ...
  if (laddr.cid == VMADDR_CID_HYPERVISOR || laddr.cid == VMADDR_CID_RESERVED ||
      laddr.cid == VMADDR_CID_HOST) {
  ...
  if (laddr.port != VMADDR_PORT_ANY) {
    error = vsock_bind_address_if_free(pcb, laddr.cid, laddr.port, raddr.cid,
                                       raddr.port);
  } else {
    const uint16_t protocol = pcb->so->so_protocol;
    ...
    do {
      ...
      error = vsock_bind_address_if_free(pcb, laddr.cid, *last_port, raddr.cid,
                                         raddr.port);
    } while (error);
...
static void vsock_unbind_pcb_locked(struct vsockpcb *pcb, bool is_locked) {
  if (!pcb) {
    return;
  }
  ...
  if (!is_locked) {
    ...
    // Case where some other thread also called unbind() on this socket while
    // waiting to acquire its lock.
...
static void vsock_unbind_pcb(struct vsockpcb *pcb) {
  vsock_unbind_pcb_locked(pcb, false);
}

static struct sockaddr *vsock_new_sockaddr(struct vsock_address *address) {
  if (!address) {
    return NULL;
  }
  ...
  addr =
      (struct sockaddr_vm *)alloc_sockaddr(sizeof(*addr), Z_WAITOK | Z_NOFAIL);
...
static errno_t vsock_pcb_send_message(struct vsockpcb *pcb,
  ...
  if (src.cid == dst.cid) {
    ...
    socket_unlock(pcb->so, 0);
    error =
        vsock_put_message(src, dst, operation, buf_alloc, fwd_cnt, m, protocol);
    socket_lock(pcb->so, 0);
  } else {
    struct vsock_transport *transport = pcb->transport;
    error = transport->put_message(transport->provider, src, dst, operation,
                                   buf_alloc, fwd_cnt, m);
...
static errno_t vsock_pcb_reset_address(struct vsock_address src,
                                       struct vsock_address dst,
                                       uint16_t protocol) {
  if (dst.cid == VMADDR_CID_ANY || dst.port == VMADDR_PORT_ANY) {
    return EINVAL;
  ...
  if (src.cid == dst.cid) {
    ...
    error = transport->put_message(transport->provider, src, dst, VSOCK_RESET,
                                   0, 0, NULL);
  }
...
static errno_t vsock_pcb_safe_reset_address(struct vsockpcb *pcb,
...
static errno_t vsock_pcb_connect(struct vsockpcb *pcb) {
  return vsock_pcb_send_message(pcb, VSOCK_REQUEST, NULL);
}

static errno_t vsock_pcb_respond(struct vsockpcb *pcb) {
  return vsock_pcb_send_message(pcb, VSOCK_RESPONSE, NULL);
}

static errno_t vsock_pcb_send(struct vsockpcb *pcb, mbuf_t m) {
  return vsock_pcb_send_message(pcb, VSOCK_PAYLOAD, m);
}

static errno_t vsock_pcb_shutdown_send(struct vsockpcb *pcb) {
  return vsock_pcb_send_message(pcb, VSOCK_SHUTDOWN_SEND, NULL);
}

static errno_t vsock_pcb_reset(struct vsockpcb *pcb) {
  return vsock_pcb_send_message(pcb, VSOCK_RESET, NULL);
}

static errno_t vsock_pcb_credit_update(struct vsockpcb *pcb) {
...
vsock_pcb_credit_update_if_needed(struct vsockpcb *_Nonnull pcb) {
  ...
  // Sends a credit update if the credit values have changed since the last sent
  // message.
  if (pcb->so->so_rcv.sb_hiwat != pcb->last_buf_alloc ||
      pcb->fwd_cnt != pcb->last_fwd_cnt) {
    return vsock_pcb_credit_update(pcb);
  }
  return 0;
}

static errno_t vsock_pcb_credit_request(struct vsockpcb *pcb) {
  return vsock_pcb_send_message(pcb, VSOCK_CREDIT_REQUEST, NULL);
}

static errno_t vsock_disconnect_pcb_common(struct vsockpcb *pcb,
                                           bool is_locked) {
  socket_lock_assert_owned(pcb->so);
...
static errno_t vsock_disconnect_pcb_locked(struct vsockpcb *pcb) {
  return vsock_disconnect_pcb_common(pcb, true);
}

static errno_t vsock_disconnect_pcb(struct vsockpcb *pcb) {
  return vsock_disconnect_pcb_common(pcb, false);
}

static errno_t vsock_sockaddr_vm_validate(struct vsockpcb *pcb,
                                          struct sockaddr_vm *addr,
                                          struct proc *p) {
  if (!pcb || !pcb->so || !addr) {
    return EINVAL;
...
static errno_t vsock_put_message_connected(struct vsockpcb *_Nonnull pcb,
                                           enum vsock_operation op, mbuf_t m) {
  VERIFY(pcb != NULL);
  socket_lock_assert_owned(pcb->so);
...
static errno_t vsock_put_message_connecting(struct vsockpcb *_Nonnull pcb,
...
static errno_t vsock_put_message_listening(struct vsockpcb *_Nonnull pcb,
                                           enum vsock_operation op,
                                           struct vsock_address src,
                                           struct vsock_address dst) {
  VERIFY(pcb != NULL);
  ...
  switch (op) {
  case VSOCK_REQUEST:
    addr = (struct sockaddr_vm){.svm_len = sizeof(addr),
                                .svm_family = AF_VSOCK,
                                .svm_reserved1 = 0,
...
errno_t vsock_add_transport(struct vsock_transport *transport) {
  if (transport == NULL || transport->provider == NULL ||
      transport->protocol >= VSOCK_PROTO_MAX) {
    return EINVAL;
  }
  if (!os_atomic_cmpxchg(
          (void *volatile *)&the_vsock_transport[transport->protocol], NULL,
          transport, acq_rel)) {
    return EEXIST;
  }
  return 0;
...
errno_t vsock_remove_transport(struct vsock_transport *transport) {
  if (!os_atomic_cmpxchg(
          (void *volatile *)&the_vsock_transport[transport->protocol],
          transport, NULL, acq_rel)) {
    return ENODEV;
  }
  return 0;
}

errno_t vsock_reset_transport(struct vsock_transport *transport) {
  if (transport == NULL) {
    return EINVAL;
  }
  ...
  LIST_FOREACH_SAFE(pcb, &vsockinfo[transport->protocol].bound, bound,
                    tmp_pcb) {
    // Disconnect this transport's sockets. Listen and bind sockets must stay
    // alive.
    socket_lock(pcb->so, 1);
...
errno_t vsock_put_message(struct vsock_address src, struct vsock_address dst,
  ...
  // Check if the peer's buffer has changed. Update our view of the peer's
  // forwarded bytes.
  int buffers_changed =
      (pcb->peer_buf_alloc != buf_alloc) || (pcb->peer_fwd_cnt) != fwd_cnt;
  pcb->peer_buf_alloc = buf_alloc;
  pcb->peer_fwd_cnt = fwd_cnt;

  // Peer's buffer has enough space for the next packet. Notify any threads
  // waiting for space.
...
static int common_vsock_pcblist(struct sysctl_oid *oidp __unused, void *arg1,
                                int arg2 __unused,
                                struct sysctl_req *_Nonnull req,
                                uint16_t protocol) {
...
static int vsock_pcblist SYSCTL_HANDLER_ARGS {
  return common_vsock_pcblist(oidp, arg1, arg2, req, VSOCK_PROTO_STANDARD);
}

static int vsock_private_pcblist SYSCTL_HANDLER_ARGS {
...
SYSCTL_UINT(_net_vsock, OID_AUTO, sendspace, CTLFLAG_RW | CTLFLAG_LOCKED,
            &vsock_sendspace[VSOCK_PROTO_STANDARD], 0,
            "Maximum outgoing vsock datagram size");
SYSCTL_UINT(_net_vsock, OID_AUTO, recvspace, CTLFLAG_RW | CTLFLAG_LOCKED,
            &vsock_recvspace[VSOCK_PROTO_STANDARD], 0,
            "Maximum incoming vsock datagram size");
SYSCTL_PROC(_net_vsock, OID_AUTO, pcblist,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED,
            __unsafe_forge_single(caddr_t, SOCK_STREAM), 0, vsock_pcblist,
            "S,xvsockpcb", "List of active vsock sockets");
SYSCTL_UINT(_net_vsock, OID_AUTO, pcbcount, CTLFLAG_RD | CTLFLAG_LOCKED,
            (u_int *)&vsockinfo[VSOCK_PROTO_STANDARD].all_pcb_count, 0, "");

// Private namespace.
SYSCTL_NODE(_net, OID_AUTO, vsock_private, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "vsock_private");
SYSCTL_PROC(_net_vsock_private, OID_AUTO, pcblist,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED,
            __unsafe_forge_single(caddr_t, SOCK_STREAM), 0,
            vsock_private_pcblist, "S,xvsockpcb",
...
static int vsock_attach(struct socket *_Nonnull so, int proto, struct proc *p) {
#pragma unused(proto, p)
  ...
  struct vsock_transport *transport =
      os_atomic_load(&the_vsock_transport[protocol], relaxed);
  if (transport == NULL) {
    return ENODEV;
  }
  ...
  pcb->local_address =
      (struct vsock_address){.cid = VMADDR_CID_ANY, .port = VMADDR_PORT_ANY};
  pcb->remote_address =
      (struct vsock_address){.cid = VMADDR_CID_ANY, .port = VMADDR_PORT_ANY};
  so->so_pcb = pcb;
...
static int vsock_control(struct socket *so, u_long cmd,
...
static int vsock_abort(struct socket *so) { return vsock_detach(so); }

static int vsock_bind(struct socket *so, struct sockaddr *nam, struct proc *p) {
#pragma unused(p)
...
static int vsock_listen(struct socket *so, struct proc *p) {
#pragma unused(p)
...
static int vsock_connect(struct socket *so, struct sockaddr *nam,
                         struct proc *p) {
#pragma unused(p)
...
static int vsock_send(struct socket *so, int flags, struct mbuf *m,
                      struct sockaddr *nam, struct mbuf *control, proc_t p) {
#pragma unused(flags, nam, p)
  ...
  // For writes larger than the default `sosendmaxchain` of 65536, vsock_send()
  // is called multiple times per write(). Only the first call to vsock_send()
  // is passed a valid mbuf packet, while subsequent calls are not marked as a
  // packet with a valid length. We should mark all mbufs as a packet and set
  // the correct packet length so that the downstream socket transport layer can
  // correctly generate physical segments.
  ...
  while (len > free_space) {
    ...
    // Check again in case free space was automatically updated in loopback
    // case.
...
static int vsock_shutdown(struct socket *so) {
  struct vsockpcb *pcb = sotovsockpcb(so);
  if (pcb == NULL) {
    return EINVAL;
  }
...
static int vsock_soreceive(struct socket *so, struct sockaddr **psa,
...
static void common_vsock_init(struct protosw *pp, struct domain *dp,
                              uint16_t protocol, lck_grp_t *lock_group) {
#pragma unused(dp)

  static int vsock_initialized[VSOCK_PROTO_MAX] = {0};
  VERIFY((pp->pr_flags & (PR_INITIALIZED | PR_ATTACHED)) == PR_ATTACHED);
  if (!os_atomic_cmpxchg((volatile int *)&vsock_initialized[protocol], 0, 1,
                         acq_rel)) {
    return;
  }
...
static void vsock_init(struct protosw *pp, struct domain *dp) {
  static LCK_GRP_DECLARE(vsock_lock_grp, "vsock");
  common_vsock_init(pp, dp, VSOCK_PROTO_STANDARD, &vsock_lock_grp);
}

static void vsock_private_init(struct protosw *pp, struct domain *dp) {
...
static int vsock_sofreelastref(struct socket *so, int dealloc) {
  socket_lock_assert_owned(so);
...
static int vsock_unlock(struct socket *_Nonnull so, int refcount,
                        void *lr_saved) {
  VERIFY(so != NULL);
...
static void vsock_dinit(struct domain *_Nonnull dp) {
