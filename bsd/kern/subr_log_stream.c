...
TUNABLE(size_t, log_stream_cache_size, "oslog_stream_csize",
        LOG_CACHE_MIN_SIZE);
...
_Static_assert(LOG_CACHE_MIN_SIZE < LOG_STREAM_MIN_SIZE / 2,
               "Minimum log stream cache size is larger than 1/2 of log stream "
               "minimum size");

static inline size_t blocks_size(size_t nblocks) {
  return nblocks << LOG_BLK_EXP;
}

static inline size_t blocks_count(size_t amount) {
  return amount >> LOG_BLK_EXP;
}

static inline size_t blocks_needed(size_t amount) {
...
static inline size_t log_stream_block(const log_stream_t *ls,
                                      uint64_t block_seq) {
  return block_seq % ls->ls_blk_count;
}

static inline size_t log_stream_offset(const log_stream_t *ls,
                                       uint64_t block_seq) {
  return blocks_size(log_stream_block(ls, block_seq));
}

static size_t log_stream_bytes(const log_stream_t *ls, size_t pos) {
  assert(pos >= ls->ls_commited_wraps);
  return pos - ls->ls_commited_wraps;
}

static size_t log_stream_written(const log_stream_t *ls, size_t pos) {
 ...
 */
static log_stream_ticket_t log_stream_reserve(log_stream_t *ls, size_t amount) {
 ...
 */
static void log_stream_commit(log_stream_t *ls, log_stream_ticket_t ticket) {
  assert(!ls->ls_snapshot);
...
static void log_stream_snapshot(const log_stream_t *ls, log_stream_t *snap) {
...
static uint64_t log_stream_waitpoint(const log_stream_t *ls,
                                     log_stream_ticket_t ticket) {
...
static void log_stream_wait(log_stream_t *ls, log_stream_ticket_t ticket) {
...
static inline void log_stream_msg_alter(log_stream_msg_t __unused *msg) {
#ifdef LOG_STREAM_VERIFY
...
static inline void log_stream_msg_verify(const log_stream_msg_t __unused *msg) {
#ifdef LOG_STREAM_VERIFY
  if (__improbable(log_stream_verify)) {
...
static size_t log_stream_msg_size(const log_stream_msg_t *m) {
  assert(LOG_SIZE_VALID(m->lsm_ft.ft_length));
  return sizeof(*m) + m->lsm_ft.ft_length;
}

static size_t log_stream_msg_make(log_stream_msg_t *msg,
...
static void log_session_make(log_stream_session_t *session, log_stream_t *ls,
                             log_stream_ticket_t t) {
...
static void log_session_start(log_stream_t *ls, size_t size,
...
static void log_session_finish(log_stream_session_t *session) {
  log_stream_commit(session->lss_stream, session->lss_ticket);
  bzero(session, sizeof(*session));
}

static size_t log_stream_avail(log_stream_t *ls, size_t pos) {
  log_stream_t snap;
  log_stream_snapshot(ls, &snap);
  return log_stream_written(&snap, pos);
}

static size_t rbuf_copy(uint8_t *rb, size_t rb_size, size_t offset, size_t n,
                        const uint8_t *d) {
...
static void rbuf_set(uint8_t *rb, size_t rb_size, size_t offset, size_t n,
                     uint8_t v) {
  const size_t remains = MIN(n, rb_size - offset);
  assert(remains > 0);
...
static void rbuf_read(const uint8_t *rb, size_t rb_size, size_t offset,
                      size_t n, uint8_t *b) {
...
static size_t log_stream_write(log_stream_t *ls, size_t offset, size_t n,
                               const uint8_t *buf) {
  return rbuf_copy(ls->ls_buf, blocks_size(ls->ls_blk_count), offset, n, buf);
}

static void log_stream_read(log_stream_t *ls, size_t blk_seq, size_t blk_cnt,
                            uint8_t *buf) {
  rbuf_read(ls->ls_buf, blocks_size(ls->ls_blk_count),
            log_stream_offset(ls, blk_seq), blocks_size(blk_cnt), buf);
}

static void log_stream_write_blocks(log_stream_t *ls, size_t blk_seq,
                                    size_t blk_count) {
...
static void log_stream_read_blocks(log_stream_t *ls, size_t blk_seq,
                                   size_t blk_cnt, uint8_t *buf) {
  const size_t blk_id = log_stream_block(ls, blk_seq);
  rbuf_read(ls->ls_blk, ls->ls_blk_count, blk_id, blk_cnt, buf);
}

static void log_session_store(log_stream_session_t *lss, log_stream_msg_t *msg,
                              const void *msg_data) {
...
static size_t log_stream_sync(const log_stream_t *ls, size_t pos) {
  assert(ls->ls_snapshot);
...
static bool log_cache_refill(log_cache_t *lc) {
  ...
  size_t blk_avail =
      MIN(log_stream_written(ls, lc->lc_stream_pos), lc->lc_blk_count);
  assert(blk_avail > 0);
...
static void log_stream_si_wakeup_locked(log_stream_si_t *lsi) {
...
static void log_stream_si_wakeup(log_stream_si_t *lsi) {
...
static void log_stream_si_record(log_stream_si_t *lsi, void *wql, proc_t p) {
...
static void log_stream_si_enable(log_stream_si_t *lsi, log_stream_t *ls) {
  log_stream_si_lock(lsi);
  assert(!lsi->lsi_channel);
...
static void log_stream_si_disable(log_stream_si_t *lsi) {
  log_stream_si_lock(lsi);
  log_stream_si_wakeup_locked(lsi);
...
static bool log_stream_make(log_stream_t *ls, size_t stream_size) {
  ...
  ls->ls_buf = kalloc_data(blocks_size(ls->ls_blk_count), Z_WAITOK | Z_ZERO);
  if (!ls->ls_buf) {
...
static void log_stream_enable(log_stream_t *tgt, log_stream_t *src) {
  /*
...
static void log_stream_disable(log_stream_t *src, log_stream_t *tgt) {
  *tgt = *src;
  src->ls_buf = NULL;
  src->ls_blk = NULL;
...
static void log_stream_teardown(log_stream_t *ls) {
  if (ls->ls_buf) {
...
static bool log_cache_make(log_cache_t *lc, size_t lc_size, log_stream_t *ls) {
  bzero(lc, sizeof(*lc));
...
static void log_cache_move(log_cache_t *src, log_cache_t *tgt) {
  *tgt = *src;
  bzero(src, sizeof(*src));
}

static void log_cache_teardown(log_cache_t *lc) {
  if (lc->lc_blk) {
    kfree_data(lc->lc_blk, lc->lc_blk_count);
  }
  if (lc->lc_buf) {
...
static void log_cache_rewind(log_cache_t *lc) {
  bzero(lc->lc_blk, lc->lc_blk_count);
  bzero(lc->lc_buf, blocks_size(lc->lc_blk_count));
  lc->lc_blk_pos = 0;
}

static void log_cache_consume(log_cache_t *lc, size_t amount) {
  lc->lc_blk_pos += blocks_needed(amount);
}

static size_t log_cache_next_msg(log_cache_t *lc) {
...
static log_stream_msg_t *log_cache_msg(const log_cache_t *lc, size_t blk_id,
                                       size_t *msg_size) {
  assert(blk_id != LOG_CACHE_EMPTY);
  assert(blk_id < lc->lc_blk_count);

  log_stream_msg_t *msg = (log_stream_msg_t *)&lc->lc_buf[blocks_size(blk_id)];
...
static bool log_cache_get(log_cache_t *lc, log_stream_msg_t **log,
                          size_t *log_size) {
...
static int handle_no_logs(log_stream_t *ls, int flag) {
  if (flag & IO_NDELAY) {
    return EWOULDBLOCK;
  }
...
void oslog_stream(bool is_metadata, firehose_tracepoint_id_u ftid,
...
int oslog_streamread(dev_t dev, struct uio *uio, int flag) {
...
int oslog_streamselect(dev_t dev, int rw, void *wql, proc_t p) {
...
int oslog_streamioctl(dev_t dev, u_long com, caddr_t data, __unused int flag,
...
int oslog_streamopen(dev_t dev, __unused int flags, __unused int mode,
                     __unused struct proc *p) {
  if (minor(dev) != 0) {
    return ENXIO;
  }
...
int oslog_streamclose(dev_t dev, __unused int flag, __unused int devtype,
                      __unused struct proc *p) {
  if (minor(dev) != 0) {
    return ENXIO;
  }
...
__startup_func static void oslog_stream_init(void) {
  if (os_log_disabled()) {
  ...
  lck_spin_init(&log_stream_si.lsi_lock, &log_stream_lock_grp, LCK_ATTR_NULL);
