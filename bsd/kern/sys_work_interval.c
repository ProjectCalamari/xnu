...
int work_interval_ctl(__unused proc_t p, struct work_interval_ctl_args *uap,
  ...
  case WORK_INTERVAL_OPERATION_GET_FLAGS:
    ...
    create_params =
        (struct work_interval_create_params){.wicp_port = port_name};

    kret = kern_work_interval_get_flags_from_port(
        port_name, &create_params.wicp_create_flags);
    if (kret != KERN_SUCCESS) {
      return EINVAL;
    }
  ...
  case WORK_INTERVAL_OPERATION_SET_NAME:
    ...
    if ((error = copyinstr(uap->arg, wi_name, sizeof(wi_name), &wi_name_len)) !=
        0) {
      return error;
    }
