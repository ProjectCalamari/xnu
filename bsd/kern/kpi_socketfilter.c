...
#undef sflt_register
static errno_t sflt_register_common(const struct sflt_filter *filter,
                                    int domain, int type, int protocol,
                                    bool is_internal);
errno_t sflt_register(const struct sflt_filter *filter, int domain, int type,
                      int protocol);
...
__private_extern__ int sflt_permission_check(struct inpcb *inp) {
  /* Only IPv4 or IPv6 sockets can bypass filters */
  if (!(inp->inp_vflag & INP_IPV4) && !(inp->inp_vflag & INP_IPV6)) {
    return 0;
  }
  /* Sockets that have incoproc or management entitlements bypass socket
   * filters. */
  if (INP_INTCOPROC_ALLOWED(inp) || INP_MANAGEMENT_ALLOWED(inp)) {
    return 1;
  }
  /* Sockets bound to an intcoproc or management interface bypass socket
   * filters. */
...
static void sflt_retain_locked(struct socket_filter *filter) {
  os_ref_retain_locked(&filter->sf_refcount);
}

static void sflt_release_locked(struct socket_filter *filter) {
...
static void sflt_entry_retain(struct socket_filter_entry *entry) {
  os_ref_retain(&entry->sfe_refcount);
}

static void sflt_entry_release(struct socket_filter_entry *entry) {
  if (os_ref_release(&entry->sfe_refcount) == 0) {
    ...
    if (entry->sfe_next_oncleanup == NULL) {
      if (sock_filter_cleanup_thread == NULL) {
        /* Create a thread */
        kernel_thread_start(sflt_cleanup_thread, NULL,
...
__attribute__((noreturn)) static void sflt_cleanup_thread(void *blah,
                                                          wait_result_t blah2) {
#pragma unused(blah, blah2)
  while (1) {
    ...
    while (sock_filter_cleanup_entries == NULL) {
      ...
      msleep(&sock_filter_cleanup_entries, &sock_filter_cleanup_lock, PWAIT,
             "sflt_cleanup", NULL);
    }
    ...
    for (entry = dead; entry; entry = dead) {
      ...
      if ((entry->sfe_flags & SFEF_NODETACH) == 0 &&
         ...
         */
        entry->sfe_filter->sf_filter.sf_detach(entry->sfe_cookie,
                                               entry->sfe_socket);
...
static int sflt_attach_locked(struct socket *so, struct socket_filter *filter,
                              int socklocked) {
  int error = 0;
  ...
  for (entry = so->so_filt; entry; entry = entry->sfe_next_onfilter) {
    if (entry->sfe_filter->sf_filter.sf_handle == filter->sf_filter.sf_handle) {
      return EEXIST;
    }
  }
  ...
  os_ref_init(&entry->sfe_refcount,
              NULL); /* corresponds to SFEF_ATTACHED flag set */
...
errno_t sflt_attach_internal(socket_t socket, sflt_handle handle) {
...
__private_extern__ void sflt_initsock(struct socket *so) {
  /*
  ...
  if (TAILQ_FIRST(&proto->pr_filter_head) != NULL) {
     ...
     */
    struct socket_filter *__single filter = TAILQ_FIRST(&proto->pr_filter_head);
 ...
 */
__private_extern__ void sflt_termsock(struct socket *so) {
  /*
...
static void sflt_notify_internal(struct socket *so, sflt_event_t event,
                                 void *param, sflt_handle handle) {
  if (so->so_filt == NULL) {
    return;
  }
  ...
  for (entry = so->so_filt; entry; entry = entry->sfe_next_onsocket) {
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      entry->sfe_filter->sf_filter.sf_notify(entry->sfe_cookie, so, event,
                                             param);
...
__private_extern__ void sflt_notify(struct socket *so, sflt_event_t event,
                                    void *param) {
  sflt_notify_internal(so, event, param, 0);
}

static void sflt_notify_after_register(struct socket *so, sflt_event_t event,
                                       sflt_handle handle) {
  sflt_notify_internal(so, event, NULL, handle);
}

__private_extern__ int sflt_ioctl(struct socket *so, u_long cmd,
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_ioctl(entry->sfe_cookie, so, cmd,
                                                    data);
...
__private_extern__ int sflt_bind(struct socket *so,
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_bind(entry->sfe_cookie, so, nam);
...
__private_extern__ int sflt_listen(struct socket *so) {
  if (so->so_filt == NULL || sflt_permission_check(sotoinpcb(so))) {
    return 0;
  }
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_listen(entry->sfe_cookie, so);
...
__private_extern__ int sflt_accept(struct socket *head, struct socket *so,
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_accept(entry->sfe_cookie, head,
                                                     so, local, remote);
...
__private_extern__ int sflt_getsockname(struct socket *so,
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_getsockname(entry->sfe_cookie, so,
                                                          local);
...
__private_extern__ int sflt_getpeername(struct socket *so,
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_getpeername(entry->sfe_cookie, so,
                                                          remote);
...
__private_extern__ int sflt_connectin(struct socket *so,
                                      const struct sockaddr *remote) {
  if (so->so_filt == NULL || sflt_permission_check(sotoinpcb(so))) {
    return 0;
  }
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_connect_in(entry->sfe_cookie, so,
                                                         remote);
...
static int sflt_connectout_common(struct socket *so,
                                  const struct sockaddr *nam) {
  struct socket_filter_entry *__single entry;
  int unlocked = 0;
  int error = 0;
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_connect_out(entry->sfe_cookie, so,
                                                          nam);
...
__private_extern__ int sflt_connectout(struct socket *so,
                                       const struct sockaddr *innam) {
...
__private_extern__ int sflt_setsockopt(struct socket *so,
                                       struct sockopt *sopt) {
  if (so->so_filt == NULL || sflt_permission_check(sotoinpcb(so))) {
    return 0;
  }
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_setoption(entry->sfe_cookie, so,
                                                        sopt);
...
__private_extern__ int sflt_getsockopt(struct socket *so,
                                       struct sockopt *sopt) {
  if (so->so_filt == NULL || sflt_permission_check(sotoinpcb(so))) {
    return 0;
  }
  ...
  for (entry = so->so_filt; entry && error == 0;
    ...
    if ((entry->sfe_flags & SFEF_ATTACHED) &&
      ...
      /* Call the filter */
      error = entry->sfe_filter->sf_filter.sf_getoption(entry->sfe_cookie, so,
                                                        sopt);
...
__private_extern__ int sflt_data_out(struct socket *so,
...
__private_extern__ int sflt_data_in(struct socket *so,
...
static errno_t sflt_register_common(const struct sflt_filter *infilter,
                                    int domain, int type, int protocol,
                                    bool is_internal) {
  const struct sflt_filter *filter =
      (const struct sflt_filter *__indexable)infilter;
  ...
  sock_filt = kalloc_type(struct socket_filter, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  if (protocol == IPPROTO_TCP) {
    ...
    LIST_FOREACH(inp, tcbinfo.ipi_listhead, inp_list) {
      ...
      if (so == NULL || (so->so_state & SS_DEFUNCT) ||
          (!(so->so_flags & SOF_MP_SUBFLOW) && (so->so_state & SS_NOFDREF)) ||
          !SOCK_CHECK_DOM(so, domain) || !SOCK_CHECK_TYPE(so, type)) {
        continue;
      }
...
#undef SOLIST_ADD
  ...
  while (solisthead) {
    ...
    if (so->so_state & SS_ISCONNECTING) {
      sflt_notify_after_register(so, sock_evt_connecting, handle);
    } else if (so->so_state & SS_ISCONNECTED) {
      sflt_notify_after_register(so, sock_evt_connected, handle);
    } else if ((so->so_state &
                (SS_ISDISCONNECTING | SS_CANTRCVMORE | SS_CANTSENDMORE)) ==
               (SS_ISDISCONNECTING | SS_CANTRCVMORE | SS_CANTSENDMORE)) {
      sflt_notify_after_register(so, sock_evt_disconnecting, handle);
    } else if ((so->so_state &
                (SS_CANTRCVMORE | SS_CANTSENDMORE | SS_ISDISCONNECTED)) ==
               (SS_CANTRCVMORE | SS_CANTSENDMORE | SS_ISDISCONNECTED)) {
      sflt_notify_after_register(so, sock_evt_disconnected, handle);
    } else if (so->so_state & SS_CANTSENDMORE) {
      sflt_notify_after_register(so, sock_evt_cantsendmore, handle);
    } else if (so->so_state & SS_CANTRCVMORE) {
      sflt_notify_after_register(so, sock_evt_cantrecvmore, handle);
    }
    socket_unlock(so, 0);
...
errno_t sflt_register_internal(const struct sflt_filter *filter, int domain,
...
errno_t sflt_unregister(sflt_handle handle) {
  ...
  if (filter) {
    if (filter->sf_flags & SFF_INTERNAL) {
      VERIFY(OSDecrementAtomic64(&net_api_stats.nas_sfltr_register_os_count) >
             0);
    }
...
errno_t sock_inject_data_in(socket_t so, const struct sockaddr *from,
  ...
  if (from) {
    if (sbappendaddr(&so->so_rcv, __DECONST_SA(from), data, control, NULL)) {
      sorwakeup(so);
    }
    goto done;
  }
...
errno_t sock_inject_data_out(socket_t so, const struct sockaddr *to,
  ...
  error = sosend(so, __DECONST_SA(to), NULL, data, control, sosendflags);
...
int sockopt_level(sockopt_t sopt) { return sopt->sopt_level; }

int sockopt_name(sockopt_t sopt) { return sopt->sopt_name; }

size_t sockopt_valsize(sockopt_t sopt) { return sopt->sopt_valsize; }

errno_t sockopt_copyin(sockopt_t sopt, void *__sized_by(len) data, size_t len) {
  return sooptcopyin(sopt, data, len, len);
}

errno_t sockopt_copyout(sockopt_t sopt, void *__sized_by(len) data,
                        size_t len) {
...
#if SKYWALK
static bool net_check_compatible_sfltr(void) {
  if (net_api_stats.nas_sfltr_register_count >
      net_api_stats.nas_sfltr_register_os_count) {
    return false;
  }
  return true;
}
