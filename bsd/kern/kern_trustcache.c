...
SYSCTL_NODE(_security_codesigning, OID_AUTO, trustcaches, CTLFLAG_RD, 0,
            "XNU Trust Caches");

SYSCTL_UINT(_security_codesigning_trustcaches, OID_AUTO, num_static, CTLFLAG_RD,
            &num_static_trust_caches, 0,
            "number of static trust caches loaded");

SYSCTL_UINT(_security_codesigning_trustcaches, OID_AUTO, num_engineering,
            CTLFLAG_RD, &num_engineering_trust_caches, 0,
            "number of engineering trust caches loaded");

SYSCTL_UINT(_security_codesigning_trustcaches, OID_AUTO, num_loadable,
            CTLFLAG_RD, &num_loadable_trust_caches, 0,
            "number of loadable trust caches loaded");

#if CONFIG_SPTM
/*
 * We have the TrustedExecutionMonitor environment available. All of our
 * artifacts need to be page-aligned, and transferred to the appropriate TXM
 * type before we call into TXM to load the trust cache.
 *
 * The trust cache runtime is managed independently by TXM. All initialization
 * work is done by the TXM bootstrap and there is nothing more we need to do
 * here.
 */
...
static void get_trust_cache_info(void) {
  txm_call_t txm_call = {.selector = kTXMKernelSelectorGetTrustCacheInfo,
                         .failure_fatal = true,
                         .num_output_args = 4};
...
void trust_cache_runtime_init(void) {
...
static kern_return_t txm_unload_trust_cache(uuid_t uuid) {
  txm_call_t txm_call = {.selector = kTXMKernelSelectorUnloadTrustCache,
                         .num_input_args = 1};
...
static kern_return_t txm_load_trust_cache(
    TCType_t type, const uint8_t *img4_payload, const size_t img4_payload_len,
  ...
  ret = txm_kernel_call(&txm_call, type, payload_addr, img4_payload_len,
                        manifest_addr, img4_manifest_len, 0, 0);
...
out:
  ...
  if (((ret != KERN_SUCCESS) || (reclaim_payload == true)) &&
      (payload_addr != 0)) {
...
txm_load_legacy_trust_cache(__unused const uint8_t *module_data,
                            __unused const size_t module_size) {
  panic("legacy trust caches are not supported on this platform");
}

static kern_return_t
txm_query_trust_cache(TCQueryType_t query_type,
...
static kern_return_t
txm_query_trust_cache_for_rem(const uint8_t cdhash[kTCEntryHashSize],
                              uint8_t *rem_perms) {
#if XNU_HAS_TRUST_CACHE_QUERY_FOR_REM
  txm_call_t txm_call = {.selector = kTXMKernelSelectorQueryTrustCacheForREM,
                         .num_input_args = 1,
                         .num_output_args = 1};
...
static kern_return_t
txm_check_trust_cache_runtime_for_uuid(const uint8_t check_uuid[kUUIDSize]) {
...
/* Immutable part of the runtime */
SECURITY_READ_ONLY_LATE(TrustCacheRuntime_t *)
trust_cache_rt = &ppl_trust_cache_rt;

/* Mutable part of the runtime */
SECURITY_READ_ONLY_LATE(TrustCacheMutableRuntime_t *)
trust_cache_mut_rt = &ppl_trust_cache_mut_rt;

static bool is_internal_iBoot(void) {
  DTEntry node = {0};
  ...
  err = SecureDTGetProperty(node, "iboot-build-variant",
                            (const void **)&variant, &size);
  if (err != kSuccess) {
...
void trust_cache_runtime_init(void) {
  ...
  /*
   * Allow engineering trust caches when the system is booting using a
   * development or debug variant of iBoot.
   */
  ...
  trustCacheInitializeRuntime(
      trust_cache_rt, trust_cache_mut_rt, allow_second_static_cache,
      allow_engineering_caches, false, IMG4_RUNTIME_PMAP_CS);

  /* Locks are initialized in "pmap_bootstrap()" */
}

static kern_return_t ppl_unload_trust_cache(__unused uuid_t uuid) {
  return KERN_NOT_SUPPORTED;
}

static kern_return_t ppl_load_trust_cache(
    TCType_t type, const uint8_t *img4_payload, const size_t img4_payload_len,
    const uint8_t *img4_manifest, const size_t img4_manifest_len,
    const uint8_t *img4_aux_manifest, const size_t img4_aux_manifest_len) {
  kern_return_t ret = KERN_DENIED;
  ...
  if (os_add_overflow(img4_payload_len, sizeof(pmap_img4_payload_t),
                      &payload_len)) {
  ...
  if (aux_manifest_len_aligned != 0) {
    ...
    ret =
        kmem_alloc(kernel_map, &aux_manifest_addr, aux_manifest_len_aligned,
                   KMA_KOBJECT | KMA_DATA | KMA_ZERO, VM_KERN_MEMORY_SECURITY);
    if (ret != KERN_SUCCESS) {
  ...
  ret = pmap_load_trust_cache_with_type(
      type, payload_addr, payload_len, manifest_addr, img4_manifest_len,
...
static kern_return_t
ppl_load_legacy_trust_cache(__unused const uint8_t *module_data,
                            __unused const size_t module_size) {
  panic("legacy trust caches are not supported on this platform");
}

static kern_return_t
ppl_query_trust_cache(TCQueryType_t query_type,
                      const uint8_t cdhash[kTCEntryHashSize],
                      TrustCacheQueryToken_t *query_token) {
  /*
...
static kern_return_t
ppl_check_trust_cache_runtime_for_uuid(const uint8_t check_uuid[kUUIDSize]) {
  return pmap_check_trust_cache_runtime_for_uuid(check_uuid);
}

#else
/*
 * We don't have a monitor environment available. This means someone with a
 * kernel memory exploit will be able to inject a trust cache into the system.
 * There is not much we can do here, since this is older HW.
 */
...
SECURITY_READ_ONLY_LATE(TrustCacheRuntime_t *)
trust_cache_rt = &trust_cache_rt_storage;

/* Mutable part of the runtime */
TrustCacheMutableRuntime_t trust_cache_mut_rt_storage;
SECURITY_READ_ONLY_LATE(TrustCacheMutableRuntime_t *)
trust_cache_mut_rt = &trust_cache_mut_rt_storage;
...
void trust_cache_runtime_init(void) {
  ...
  trustCacheInitializeRuntime(
      trust_cache_rt, trust_cache_mut_rt, allow_second_static_cache,
      allow_engineering_caches, allow_legacy_caches, IMG4_RUNTIME_DEFAULT);

  /* Initialize the read-write lock */
  lck_rw_init(&trust_cache_rt_lock, &trust_cache_lck_grp, 0);
}

static kern_return_t xnu_unload_trust_cache(__unused uuid_t uuid) {
  return KERN_NOT_SUPPORTED;
}

static kern_return_t xnu_load_trust_cache(
    TCType_t type, const uint8_t *img4_payload, const size_t img4_payload_len,
    const uint8_t *img4_manifest, const size_t img4_manifest_len,
    const uint8_t *img4_aux_manifest, const size_t img4_aux_manifest_len) {
  kern_return_t ret = KERN_DENIED;
  ...
  /* Allocate the trust cache data structure -- Z_WAITOK_ZERO means this can't
   * fail */
  TrustCache_t *trust_cache = kalloc_type(TrustCache_t, Z_WAITOK_ZERO);
  assert(trust_cache != NULL);

  /*
   * The manifests aren't needed after the validation is complete, but the
   * payload needs to persist. The caller of this API expects us to make our own
   * allocations. Since we don't need the manifests after validation, we can use
   * the manifests passed in to us but we need to make a new allocation for the
   * payload, since that needs to persist.
   *
  ...
  TCReturn_t tc_ret = amfi->TrustCache.load(
      trust_cache_rt, type, trust_cache, (const uintptr_t)payload,
      img4_payload_len, (const uintptr_t)img4_manifest, img4_manifest_len);
...
static kern_return_t
xnu_load_legacy_trust_cache(__unused const uint8_t *module_data,
                            __unused const size_t module_size) {
  ...
  TCReturn_t tc_ret =
      amfi->TrustCache.loadModule(trust_cache_rt, kTCTypeLegacy, trust_cache,
  ...
  if (tc_ret.error == kTCReturnSuccess) {
    ...
    printf(
        "unable to load legacy trust cache (TCReturn: 0x%02X | 0x%02X | %u)\n",
        tc_ret.component, tc_ret.error, tc_ret.uniqueError);
...
static kern_return_t
xnu_query_trust_cache(TCQueryType_t query_type,
                      const uint8_t cdhash[kTCEntryHashSize],
                      TrustCacheQueryToken_t *query_token) {
  ...
  TCReturn_t tc_ret =
      amfi->TrustCache.query(trust_cache_rt, query_type, cdhash, query_token);
...
static kern_return_t
xnu_check_trust_cache_runtime_for_uuid(const uint8_t check_uuid[kUUIDSize]) {
  kern_return_t ret = KERN_DENIED;
  ...
  TCReturn_t tc_ret =
      amfi->TrustCache.checkRuntimeForUUID(trust_cache_rt, check_uuid, NULL);
...
kern_return_t
check_trust_cache_runtime_for_uuid(const uint8_t check_uuid[kUUIDSize]) {
  kern_return_t ret = KERN_DENIED;
...
kern_return_t unload_trust_cache(uuid_t uuid) {
  kern_return_t ret = KERN_DENIED;
  ...
  if (IOCurrentTaskHasEntitlement("com.apple.private.unload-trust-cache") ==
      false) {
...
kern_return_t load_trust_cache(const uint8_t *img4_object,
                               const size_t img4_object_len,
                               const uint8_t *img4_ext_manifest,
                               const size_t img4_ext_manifest_len) {
  ...
  for (type = kTCTypeLTRS; type < kTCTypeTotal; type += 1) {
    ret = load_trust_cache_with_type(type, img4_object, img4_object_len,
  ...
  ret = load_trust_cache_with_type(kTCTypeDTRS, img4_object, img4_object_len,
                                   img4_ext_manifest, img4_ext_manifest_len,
                                   NULL, 0);
#endif
...
kern_return_t load_trust_cache_with_type(
    TCType_t type, const uint8_t *img4_object, const size_t img4_object_len,
  ...
  if (!img4_object || (img4_object_len == 0)) {
    printf("unable to load trust cache (type: %u): no img4_object provided\n",
           type);
    return KERN_INVALID_ARGUMENT;
  } else if (os_add_overflow((uintptr_t)img4_object, img4_object_len,
                             &length_check)) {
    panic("overflow on the img4 object: %p | %lu", img4_object,
          img4_object_len);
  }
  ...
  if (img4_ext_manifest_len != 0) {
    if (!img4_ext_manifest) {
      printf(
          ...
          type);
      return KERN_INVALID_ARGUMENT;
    } else if (os_add_overflow((uintptr_t)img4_ext_manifest,
                               img4_ext_manifest_len, &length_check)) {
      panic("overflow on the ext manifest: %p | %lu", img4_ext_manifest,
            img4_ext_manifest_len);
    }
  }
  ...
  if (img4_aux_manifest_len != 0) {
    if (!img4_aux_manifest) {
      printf(
          ...
          type);
      return KERN_INVALID_ARGUMENT;
    } else if (os_add_overflow((uintptr_t)img4_aux_manifest,
                               img4_aux_manifest_len, &length_check)) {
      panic("overflow on the ext manifest: %p | %lu", img4_aux_manifest,
            img4_aux_manifest_len);
    }
  }

  /*
   * If we don't have an external manifest provided, we expect the img4_object
   * to have the manifest embedded. In this case, we need to extract the
   * different artifacts out of the object.
   */
  if (img4_ext_manifest_len != 0) {
...
#if CONFIG_SPTM
  ret = txm_load_trust_cache(type, img4_payload, img4_payload_len,
                             img4_manifest, img4_manifest_len,
                             img4_aux_manifest, img4_aux_manifest_len);
#elif PMAP_CS_PPL_MONITOR
  ret = ppl_load_trust_cache(type, img4_payload, img4_payload_len,
                             img4_manifest, img4_manifest_len,
                             img4_aux_manifest, img4_aux_manifest_len);
#else
  ret = xnu_load_trust_cache(type, img4_payload, img4_payload_len,
                             img4_manifest, img4_manifest_len,
                             img4_aux_manifest, img4_aux_manifest_len);
#endif
...
kern_return_t load_legacy_trust_cache(const uint8_t *module_data,
  ...
  if (!module_data || (module_size == 0)) {
    ...
    return KERN_INVALID_ARGUMENT;
  } else if (os_add_overflow((uintptr_t)module_data, module_size,
                             &length_check)) {
...
kern_return_t query_trust_cache(TCQueryType_t query_type,
                                const uint8_t cdhash[kTCEntryHashSize],
                                TrustCacheQueryToken_t *query_token) {
  kern_return_t ret = KERN_NOT_FOUND;
...
kern_return_t query_trust_cache_for_rem(const uint8_t cdhash[kTCEntryHashSize],
  ...
  /*
   * Only when the system is using the Trusted Execution Monitor environment
   * does it support restricted execution mode. For all other monitor
   * environments, or when we don't have a monitor, the return defaults to a not
   * supported.
   */
#if CONFIG_SPTM
...
/*
 * The trust cache management library uses a wrapper data structure to manage
 * each of the trust cache modules. We know the exact number of static trust
 * caches we expect, so we keep around a read-only-late allocation of the data
 * structure for use.
 *
 * Since engineering trust caches are only ever allowed on development builds,
 * they are not protected through the read-only-late property, and instead
...
#else /* x86_64 */
/*
 * We need this duplicate definition because it is hidden behind the
 * MACH_KERNEL_PRIVATE macro definition, which makes it inaccessible to this
...
static inline void *PHYSMAP_PTOV_check(void *paddr) {
  ...
  if (__improbable(pvaddr >= physmap_max)) {
    panic("PHYSMAP_PTOV bounds exceeded, 0x%qx, 0x%qx, 0x%qx", pvaddr,
...
void load_static_trust_cache(void) {
  ...
  err = SecureDTGetProperty(memory_map, "TrustCache", (const void **)&tc_range,
                            &tc_dt_prop_length);
  if (err == kSuccess) {
  ...
  if (tc_segment_length == 0) {
    if (tc_offsets && tc_offsets->num_caches != 0) {
      panic("trust cache segment is zero length but trust caches are "
            "available: %u",
  ...
  if (os_mul_overflow(tc_offsets->num_caches, sizeof(uint32_t),
                      &offsets_length)) {
    panic("overflow on the number of trust caches provided: %u",
          tc_offsets->num_caches);
  } else if (os_add_overflow(offsets_length, sizeof(trust_cache_offsets_t),
                             &struct_length)) {
  ...
  for (uint32_t i = 0; i < tc_offsets->num_caches; i++) {
    ...
    if (os_add_overflow((uintptr_t)tc_offsets, tc_offsets->offsets[i],
                        &tc_module)) {
      panic("trust cache module start overflows: %u | %lu | %u", i,
            (unsigned long)tc_offsets, tc_offsets->offsets[i]);
    } else if (tc_module >= tc_region_end) {
      panic("trust cache module begins after segment ends: %u | %lx | %lx", i,
            (unsigned long)tc_module, tc_region_end);
    }
    ...
    if (tc_type == kTCTypeEngineering) {
      ...
      /* Allocate the trust cache data structure -- Z_WAITOK_ZERO means this
       * can't fail */
      trust_cache = kalloc_type(TrustCache_t, Z_WAITOK_ZERO);
      assert(trust_cache != NULL);
    }

    tc_ret = amfi->TrustCache.loadModule(trust_cache_rt, tc_type, trust_cache,
                                         tc_module, buffer_length);

    if (tc_ret.error != kTCReturnSuccess) {
      printf("unable to load trust cache module: %u (TCReturn: 0x%02X | 0x%02X "
             "| %u)\n",
    ...
    /*
     * The first module is always loaded as a static trust cache. If loading it
     * failed, then this function would've panicked. If we reach here, it means
     * we've loaded a static trust cache on the system.
     */
...
kern_return_t
static_trust_cache_capabilities(uint32_t *num_static_trust_caches_ret,
  ...
  *capabilities1_ret = kTCCapabilityNone;
  ...
  tcRet =
      amfi->TrustCache.getCapabilities(&trust_cache_static0, capabilities0_ret);
  assert(tcRet.error == kTCReturnSuccess);
  *num_static_trust_caches_ret += 1;

#if CONFIG_SECOND_STATIC_TRUST_CACHE
  tcRet =
      amfi->TrustCache.getCapabilities(&trust_cache_static1, capabilities1_ret);
  assert(tcRet.error == kTCReturnSuccess);
  *num_static_trust_caches_ret += 1;
#endif
