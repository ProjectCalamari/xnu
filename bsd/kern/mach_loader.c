...
TUNABLE_DT(uint32_t, force_arm64_32, "/defaults", "force-arm64-32",
           "force-arm64-32", 0, TUNABLE_DT_NONE);
#else
TUNABLE_DT(uint32_t, force_arm64_32, "/defaults", "force-arm64-32",
           "force-arm64-32", 0, TUNABLE_DT_NO_BOOTARG);
#endif
#endif /* ALLOW_FORCING_ARM64_32 */

#if ALLOW_FORCING_ARM64_32 || DEVELOPMENT || DEBUG
/*
 * The binary grading priority for the highest priority override.  Each
...
#if DEVELOPMENT || DEBUG
/*
 * Maxmum number of overrides that can be passed via the bingrade boot-arg
 * property.
 */
#define MAX_BINGRADE_OVERRIDES 4
/*
 * Max size of one bingrade override + 1 comma
 * (technically, sizeof will also include the terminating NUL here, but an
 * overestimation of buffer space is fine).
 */
#define BINGRADE_MAXSTRINGLEN sizeof("0x12345678:0x12345678:0x12345678,")

/*
 * Each binary grading override has a cpu type and cpu subtype to match against
 * the values in the Mach-o header.
 */
...
/*
 * The bingrade_overrides array is an ordered list of binary grading overrides.
 * The first element in the array has the highest priority.  When parsing the
 * `bingrade' boot-arg, elements are added to this array in order.
 */
 ...
 */
static load_return_t parse_machfile(struct vnode *vp, vm_map_t map,
                                    thread_t thread, struct mach_header *header,
                                    off_t file_offset, off_t macho_size,
                                    int depth, int64_t slide,
                                    int64_t dyld_slide, load_result_t *result,
                                    load_result_t *binresult,
                                    struct image_params *imgp);

static load_return_t load_segment(struct load_command *lcp, uint32_t filetype,
                                  void *control, off_t pager_offset,
                                  off_t macho_size, struct vnode *vp,
                                  vm_map_t map, int64_t slide,
                                  load_result_t *result,
                                  struct image_params *imgp);

static load_return_t load_uuid(struct uuid_command *uulp, char *command_end,
                               load_result_t *result);

static load_return_t load_version(struct version_min_command *vmc,
                                  boolean_t *found_version_cmd,
                                  struct image_params *imgp,
                                  load_result_t *result);

static load_return_t load_code_signature(struct linkedit_data_command *lcp,
                                         struct vnode *vp, off_t macho_offset,
                                         off_t macho_size, cpu_type_t cputype,
                                         cpu_subtype_t cpusubtype,
                                         load_result_t *result,
                                         struct image_params *imgp);

#if CONFIG_CODE_DECRYPTION
static load_return_t set_code_unprotect(struct encryption_info_command *lcp,
                                        caddr_t addr, vm_map_t map,
                                        int64_t slide, struct vnode *vp,
                                        off_t macho_offset, cpu_type_t cputype,
                                        cpu_subtype_t cpusubtype);
#endif

static load_return_t load_main(struct entry_point_command *epc, thread_t thread,
                               int64_t slide, load_result_t *result);

static load_return_t setup_driver_main(thread_t thread, int64_t slide,
                                       load_result_t *result);

static load_return_t load_unixthread(struct thread_command *tcp,
                                     thread_t thread, int64_t slide,
                                     boolean_t is_x86_64_compat_binary,
                                     load_result_t *result);

static load_return_t load_threadstate(thread_t thread, uint32_t *ts,
                                      uint32_t total_size, load_result_t *);

static load_return_t
load_threadstack(thread_t thread, uint32_t *ts, uint32_t total_size,
                 mach_vm_offset_t *user_stack, int *customstack,
                 boolean_t is_x86_64_compat_binary, load_result_t *result);

static load_return_t load_threadentry(thread_t thread, uint32_t *ts,
                                      uint32_t total_size,
                                      mach_vm_offset_t *entry_point);

static load_return_t load_dylinker(struct dylinker_command *lcp,
                                   integer_t archbits, vm_map_t map,
                                   thread_t thread, int depth, int64_t slide,
                                   load_result_t *result,
                                   struct image_params *imgp);

#if CONFIG_ROSETTA
static load_return_t load_rosetta(vm_map_t map, thread_t thread,
                                  load_result_t *result,
                                  struct image_params *imgp);
#endif
...
static boolean_t check_if_simulator_binary(struct image_params *imgp,
                                           off_t file_offset, off_t macho_size);
#endif

struct macho_data;

static load_return_t get_macho_vnode(const char *path, integer_t archbits,
...
#if DEVELOPMENT || DEBUG
/*
 * Parse the bingrade boot-arg, adding cputype/cpusubtype/execfeatures tuples to
 * the global binary grading override array.  The bingrade boot-arg must be of
 * the form:
 *
 ...
 *
 * Returns the number of overrides specified in the boot-arg, or 0 if there were
 * no overrides or the syntax of the overrides was found to be invalid.
 */
static int parse_bingrade_override_bootarg(bingrade_t *overrides,
                                           int max_overrides,
                                           char *overrides_arg_string) {
  char bingrade_arg[BINGRADE_MAXSTRINGLEN * MAX_BINGRADE_OVERRIDES + 1];
  int cputypespec_count = 0;

  /* Look for the bingrade boot-arg */
  if (overrides_arg_string != NULL ||
      PE_parse_boot_arg_str("bingrade", bingrade_arg, sizeof(bingrade_arg))) {
    char *bingrade_str = (overrides_arg_string != NULL) ? overrides_arg_string
                                                        : &bingrade_arg[0];
    char *cputypespec;
    ...
    /* If we found the boot-arg, iterate on each OVERRIDESPEC in the
     * BOOTSPEC_BOOTARG */
    while ((cputypespec_count < max_overrides) &&
           ((cputypespec = strsep(&bingrade_str, ",")) != NULL)) {
      ...
      /* If there's a colon present, process the cpu subtype and possibly the
       * execfeatures */
      if (colon != NULL) {
        colon++; /* Move past the colon before parsing */

        char execfeat_buf[16] = {
            0}; /* This *MUST* be preinitialized to zeroes */
        char *second_colon = strchr(colon, ':');
        ptrdiff_t amt_to_copy = 0;

        if (second_colon != NULL) {
          strlcpy(execfeat_buf, second_colon + 1,
                  MIN(strlen(second_colon + 1) + 1, sizeof(execfeat_buf)));
          ...
          /*
           * Note there is no "+ 1" here because we are only copying up to but
           * not including the second colon.  Since cputypestr was initialized
           * to all 0s above, the terminating NUL will already be there.
           */
          amt_to_copy = second_colon - colon;
        } else {
          /* No second colon, so use the wildcard for execfeatures */
          overrides[cputypespec_count].execfeatures =
              EXECFEATURES_OVERRIDE_WILDCARD;
          /*
           * There is no "+ 1" here because colon was already moved forward by 1
           * (above). which allows this computation to include the terminating
           * NUL in the length computed.
           */
        ...
        /*
         * No colon present, set the cpu subtype to 0, the execfeatures to
         * EXECFEATURES_OVERRIDE_WILDCARD and use the whole string as the cpu
         * type
         */
...
size_t bingrade_get_override_string(char *existing_overrides,
                                    size_t existing_overrides_bufsize) {
  ...
  for (int i = 0; i < num_bingrade_overrides; i++) {
    char
        next_override[33]; /* 10char + ':' + 10char + ([future] ':' + 10char) */
    snprintf(next_override, sizeof(next_override), "0x%x:0x%x",
             bingrade_overrides[i].cputype, bingrade_overrides[i].cpusubtype);
    if (i > 0) {
...
#endif /* ALLOW_FORCING_ARM64_32 */
  num_bingrade_overrides = parse_bingrade_override_bootarg(
      bingrade_overrides, MAX_BINGRADE_OVERRIDES, overrides_arg);
  return num_bingrade_overrides;
}
#endif /* DEVELOPMENT || DEBUG */

static inline void widen_segment_command(const struct segment_command *scp32,
...
static void note_all_image_info_section(const struct segment_command_64 *scp,
                                        boolean_t is64, size_t section_size,
                                        const void *sections, int64_t slide,
                                        load_result_t *result) {
  const union {
    struct section s32;
  ...
  for (i = 0; i < scp->nsects; ++i) {
    sectionp = (const void *)((const char *)sections + section_size * i);
...
static inline bool process_is_plugin_host(struct image_params *imgp,
                                          load_result_t *result) {
  ...
  /* Check if override host plugin entitlement is present and posix spawn
   * attribute to disable A keys is passed */
  if (IOVnodeHasEntitlement(imgp->ip_vp, (int64_t)imgp->ip_arch_offset,
    ...
    if (ret) {
      ...
      os_log(OS_LOG_DEFAULT,
             "%s: running binary \"%s\" in keys-off mode due to "
  ...
  /* Disabling library validation is a good signal that this process plans to
   * host plugins */
  ...
  const char *const hardening_exceptions[] = {
      "com.apple.perl5", /* Scripting engines may load third party code and
                            jit*/
      "com.apple.perl", /* Scripting engines may load third party code and jit*/
      "org.python.python", /* Scripting engines may load third party code and
                              jit*/
      "com.apple.expect",  /* Scripting engines may load third party code and
                              jit*/
      "com.tcltk.wish", /* Scripting engines may load third party code and jit*/
      "com.tcltk.tclsh", /* Scripting engines may load third party code and
                            jit*/
      ...
      "com.apple.sh",   /* rdar://138353488: sh re-execs into zsh or bash, which
                           are exempted */
  };
  for (size_t i = 0; i < ARRAY_COUNT(hardening_exceptions); i++) {
    if (strncmp(hardening_exceptions[i], identity,
                strlen(hardening_exceptions[i])) == 0) {
      proc_t p = vfs_context_proc(imgp->ip_vfs_context);
      set_proc_name(imgp, p);
      os_log(OS_LOG_DEFAULT,
             "%s: running binary \"%s\" in keys-off mode due to identity: %s",
             __func__, p->p_name, identity);
      return true;
    }
  }
...
static int grade_binary_override(cpu_type_t __unused exectype,
                                 cpu_subtype_t __unused execsubtype,
  ...
  if (force_arm64_32) {
    ...
    if (exectype == CPU_TYPE_ARM64_32 &&
        execsubtype == CPU_SUBTYPE_ARM64_32_V8) {
  ...
  if (num_bingrade_overrides == -1) {
    num_bingrade_overrides = parse_bingrade_override_bootarg(
        bingrade_overrides, MAX_BINGRADE_OVERRIDES, NULL);
  }
  ...
  for (int i = 0; i < num_bingrade_overrides; i++) {
    if (bingrade_overrides[i].cputype == exectype &&
        bingrade_overrides[i].cpusubtype == execsubtype &&
...
load_return_t load_machfile(struct image_params *imgp,
  ...
  result->is_64bit_addr =
      ((imgp->ip_flags & IMGPF_IS_64BIT_ADDR) == IMGPF_IS_64BIT_ADDR);
  result->is_64bit_data =
      ((imgp->ip_flags & IMGPF_IS_64BIT_DATA) == IMGPF_IS_64BIT_DATA);
  ...
  if (imgp->ip_px_sa != NULL) {
    ...
    if (psa->psa_flags & _POSIX_SPAWN_FORCE_4K_PAGES) {
      pmap_flags |= PMAP_CREATE_FORCE_4K_PAGES;
          ...
          PMAP_CREATE_FORCE_4K_PAGE */

  pmap = pmap_create_options(get_task_ledger(ledger_task), (vm_map_size_t)0,
                             pmap_flags);
  if (pmap == NULL) {
  ...
  /*
   * Compute a random offset for ASLR, and an independent random offset for
   * dyld.
   */
  if (!(imgp->ip_flags & IMGPF_DISABLE_ASLR)) {
    vm_map_get_max_aslr_slide_section(map, &aslr_section_offset,
                                      &aslr_section_size);
    aslr_section_offset = (random() % aslr_section_offset) * aslr_section_size;

    aslr_page_offset = random();
    aslr_page_offset =
        (aslr_page_offset % (vm_map_get_max_aslr_slide_pages(map) - 1)) + 1;
    aslr_page_offset <<= vm_map_page_shift(map);

    dyld_aslr_page_offset = random();
    dyld_aslr_page_offset =
        (dyld_aslr_page_offset %
         (vm_map_get_max_loader_aslr_slide_pages(map) - 1)) +
        1;
  ...
  *result = load_result_null;
  ...
  /*
   * re-set the bitness on the load result since we cleared the load result
   * above.
   */
  result->is_64bit_addr =
      ((imgp->ip_flags & IMGPF_IS_64BIT_ADDR) == IMGPF_IS_64BIT_ADDR);
  ...
  lret = parse_machfile(vp, map, thread, header, file_offset, macho_size, 0,
                        aslr_page_offset, dyld_aslr_page_offset, result, NULL,
                        imgp);
  ...
  /*
   * From now on it's safe to query entitlements via the vnode interface. Let's
   * get figuring out whether we're a security relevant binary out of the way
   * immediately.
   */
...
#if __x86_64__
  /*
   * On x86, for compatibility, don't enforce the hard page-zero restriction for
   * 32-bit binaries.
   */
  if (!result->is_64bit_addr) {
...
#define VM_MAP_HIGH_START_BITS_SHIFT 27
  if (result->is_64bit_addr && (imgp->ip_flags & IMGPF_HIGH_BITS_ASLR)) {
    int random_bits;
    ...
    high_start =
        (((vm_map_offset_t)random_bits) << VM_MAP_HIGH_START_BITS_SHIFT);
  ...
  if (enforce_hard_pagezero && result->is_64bit_addr &&
    ...
    /* 64 bit ARM binary must have "hard page zero" of 4GB to cover the lower 32
     * bit address space */
...
static boolean_t pie_required(cpu_type_t exectype, cpu_subtype_t execsubtype) {
  switch (exectype) {
...
/*
 * Grades the specified CPU type, CPU subtype, CPU features to determine an
 * absolute weight, used in the determination of running the associated binary
 * on this machine.
 *
 * If an override boot-arg is specified, the boot-arg is parsed and its values
 * are stored for later use in overriding the system's hard-coded binary grading
 * values.
 */
int grade_binary(cpu_type_t exectype, cpu_subtype_t execsubtype,
                 cpu_subtype_t execfeatures, bool allow_simulator_binary) {
  extern int ml_grade_binary(cpu_type_t, cpu_subtype_t, cpu_subtype_t, bool);

  int binary_grade;

  if ((binary_grade = grade_binary_override(exectype, execsubtype, execfeatures,
                                            allow_simulator_binary)) < 0) {
    return ml_grade_binary(exectype, execsubtype, execfeatures,
                           allow_simulator_binary);
  }
 ...
 */
static load_return_t
parse_machfile(struct vnode *vp, vm_map_t map, thread_t thread,
               struct mach_header *header, off_t file_offset, off_t macho_size,
               int depth, int64_t aslr_offset, int64_t dyld_aslr_offset,
               load_result_t *result, load_result_t *binresult,
               struct image_params *imgp) {
  uint32_t ncmds;
   ...
   */
  if (((cpu_type_t)(header->cputype & ~CPU_ARCH_MASK) !=
       (cpu_type() & ~CPU_ARCH_MASK))) {
    return LOAD_BADARCH;
  }

  if (!grade_binary(header->cputype, header->cpusubtype & ~CPU_SUBTYPE_MASK,
  ...
  case MH_EXECUTE:
    ...
    if (header->flags & MH_DYLDLINK) {
      ...
      /* Check properties of static executables (disallowed except for
       * development) */
#if !(DEVELOPMENT || DEBUG)
      return LOAD_FAILURE;
#endif
  ...
  if (os_add_overflow(mach_header_sz, header->sizeofcmds, &cmds_size) ||
      ...
      round_page_overflow(cmds_size, &alloc_size) || alloc_size > INT_MAX) {
    return LOAD_BADMACHO;
  }
  ...
  error =
      vn_rdwr(UIO_READ, vp, addr, (int)alloc_size, file_offset, UIO_SYSSPACE, 0,
              vfs_context_ucred(imgp->ip_vfs_context), &resid, p);
  if (error) {
   ...
   *  0: determine if TEXT and DATA boundary can be page-aligned, load platform
   * version 1: thread state, uuid, code signature 2: segments 3: dyld,
   * encryption, check entry point
   */
  ...
  for (pass = 0; pass <= 3; pass++) {
    if (pass == 1) {
      ...
      if (executable_end == writable_start &&
         ...
         */
        adjust = (effective_page_size - (executable_end & effective_page_mask));
        MACHO_PRINTF(("page-unaligned X-W boundary at "
                      "0x%llx; adjust slide from "
                      "0x%llx to 0x%llx%s\n",
                      (uint64_t)executable_end, slide, slide + adjust,
                      (is_pie ? "" : " BUT NO PIE ****** :-(")));
        if (is_pie) {
          slide += adjust;
        }
    ...
    while (ncmds--) {
      ...
      if (os_add_overflow(offset, lcp->cmdsize, &offset) ||
          lcp->cmdsize < sizeof(struct load_command) || offset > cmds_size) {
        ret = LOAD_BADMACHO;
        break;
      }
      ...
      case LC_SEGMENT: {
        ...
        if (pass == 0) {
          ...
          if (scp->initprot == 0 && scp->maxprot == 0 && scp->vmaddr == 0) {
            /* PAGEZERO */
            if (os_add3_overflow(scp->vmaddr, scp->vmsize, slide,
                                 &pagezero_end) ||
                pagezero_end > UINT32_MAX) {
              ret = LOAD_BADMACHO;
              break;
            }
          }
        ...
        if (pass == 1 &&
            !strncmp(scp->segname, "__XHDR", sizeof(scp->segname))) {
          found_xhdr = TRUE;
        }
        ...
        ret = load_segment(lcp, header->filetype, control, file_offset,
                           macho_size, vp, map, slide, result, imgp);
        if (ret == LOAD_SUCCESS && scp->fileoff == 0 && scp->filesize > 0) {
          ...
          if (found_header_segment ||
              ((scp->initprot & (VM_PROT_READ | VM_PROT_EXECUTE)) !=
      ...
      case LC_SEGMENT_64: {
        ...
        if (pass == 1 &&
            !strncmp(scp64->segname, "__XHDR", sizeof(scp64->segname))) {
          found_xhdr = TRUE;
        }
        ...
        if (ret == LOAD_SUCCESS && scp64->fileoff == 0 && scp64->filesize > 0) {
           ...
           * protection. */
          if (found_header_segment ||
              ((scp64->initprot & (VM_PROT_READ | VM_PROT_EXECUTE)) !=
               (VM_PROT_READ | VM_PROT_EXECUTE))) {
            ret = LOAD_BADMACHO;
            break;
          }
...
#endif
        ret = load_unixthread((struct thread_command *)lcp, thread, slide,
                              is_x86_64_compat_binary, result);
        break;
      }
      case LC_MAIN:
        if (pass != 1) {
        ...
        ret =
            load_main((struct entry_point_command *)lcp, thread, slide, result);
        break;
      ...
      case LC_UUID:
        if (pass == 1 && depth == 1) {
          ret = load_uuid((struct uuid_command *)lcp, (char *)addr + cmds_size,
                          result);
        }
        break;
      case LC_CODE_SIGNATURE:
         ...
         */
        ret = load_code_signature((struct linkedit_data_command *)lcp, vp,
                                  file_offset, macho_size, header->cputype,
                                  header->cpusubtype, result, imgp);
        if (ret != LOAD_SUCCESS) {
        ...
        if (got_code_signatures) {
          ...
          while (off < alloc_size && ret == LOAD_SUCCESS) {
            ...
            valid = cs_validate_range(vp, NULL, file_offset + off,
                                      (const void *)((uintptr_t)addr + off),
                                      MIN(PAGE_SIZE, cmds_size), &tainted);
            if (!valid || (tainted & CS_VALIDATE_TAINTED)) {
              if (cs_debug) {
                printf("CODE SIGNING: %s[%d]: invalid initial page at offset "
                       "%lld validated:%d tainted:%d csflags:0x%x\n",
                       vp->v_name, proc_getpid(p),
                       (long long)(file_offset + off), valid, tainted,
                       result->csflags);
              }
              if (cs_process_global_enforcement() ||
      ...
      case LC_ENCRYPTION_INFO_64:
        ...
        ret = set_code_unprotect((struct encryption_info_command *)lcp, addr,
                                 map, slide, vp, file_offset, header->cputype,
                                 header->cpusubtype);
        if (ret != LOAD_SUCCESS) {
          ...
          if (ret == LOAD_DECRYPTFAIL) {
            ...
            KERNEL_DEBUG_CONSTANT(
                BSDDBG_CODE(DBG_BSD_PROC, BSD_PROC_EXITREASON_CREATE) |
                    DBG_FUNC_NONE,
                proc_getpid(p), OS_REASON_EXEC,
                EXEC_EXIT_REASON_FAIRPLAY_DECRYPT, 0, 0);
            load_failure_reason = os_reason_create(
                OS_REASON_EXEC, EXEC_EXIT_REASON_FAIRPLAY_DECRYPT);
          } else {
            KERNEL_DEBUG_CONSTANT(
                BSDDBG_CODE(DBG_BSD_PROC, BSD_PROC_EXITREASON_CREATE) |
                    DBG_FUNC_NONE,
            ...
            load_failure_reason =
                os_reason_create(OS_REASON_EXEC, EXEC_EXIT_REASON_DECRYPT);
          }

          /*
           * Don't signal the process if it was forked and in a partially
           * constructed state as part of a spawn -- it will just be torn down
           * when the exec fails.
           */
          if (!spawn) {
      ...
      if (ret == LOAD_SUCCESS) {
        if (result->user_stack_alloc_size != 0) {
          ...
          mach_vm_size_t user_stack_size =
              vm_map_round_page(result->user_stack_alloc_size, mask);
          result->user_stack = vm_map_round_page(
              vm_end + user_stack_size + reserved_aot_size + slide, mask);
        }
      }
    }
#endif
...
load_return_t validate_potential_simulator_binary(
    cpu_type_t exectype __unused, struct image_params *imgp __unused,
    off_t file_offset __unused, off_t macho_size __unused) {
#if __x86_64__
  ...
  if (bootarg_no32exec && imgp != NULL && exectype == CPU_TYPE_X86) {
    if (imgp->ip_simulator_binary == IMGPF_SB_DEFAULT) {
      boolean_t simulator_binary =
          check_if_simulator_binary(imgp, file_offset, macho_size);
      imgp->ip_simulator_binary =
          simulator_binary ? IMGPF_SB_TRUE : IMGPF_SB_FALSE;
    }
...
static boolean_t check_if_simulator_binary(struct image_params *imgp,
  ...
  error = vn_rdwr(UIO_READ, imgp->ip_vp, ip_vdata, PAGE_SIZE, file_offset,
...
static load_return_t
unprotect_dsmos_segment(uint64_t file_off, uint64_t file_size, struct vnode *vp,
                        off_t macho_offset, vm_map_t map,
                        vm_map_offset_t map_addr, vm_map_size_t map_size) {
  kern_return_t kr;
  uint64_t slice_off;
...
#pragma unused(vp, macho_offset)
    ...
    if (vm_map_debug_apple_protect) {
      ...
      printf("APPLE_PROTECT: %d[%s] map %p "
             "[0x%llx:0x%llx] %s(%s)\n",
             proc_getpid(p), p->p_comm, map, (uint64_t)map_addr,
             (uint64_t)(map_addr + map_size), __FUNCTION__, vp->v_name);
    }
    ...
    kr = vm_map_apple_protected(map, map_addr, map_addr + map_size,
                                crypto_backing_offset, &crypt_info,
...
static load_return_t unprotect_dsmos_segment(__unused uint64_t file_off,
...
map_segment(vm_map_t map, vm_map_offset_t vm_start, vm_map_offset_t vm_end,
            vm_map_kernel_flags_t vmk_flags, memory_object_control_t control,
            vm_map_offset_t file_start, vm_map_offset_t file_end,
            vm_prot_t initprot, vm_prot_t maxprot, load_result_t *result) {
  kern_return_t ret;
  ...
  if (control != MEMORY_OBJECT_CONTROL_NULL) {
    ...
    ret = vm_map_enter_mem_object_control(
        map, &vm_start, file_end - file_start, (mach_vm_offset_t)0, vmk_flags,
        control, file_start, TRUE, /* copy */
        initprot, maxprot, VM_INHERIT_DEFAULT);
  } else {
    ret = mach_vm_map_kernel(map, &vm_start, file_end - file_start,
                             (mach_vm_offset_t)0, vmk_flags, IPC_PORT_NULL,
                             0,    /* offset */
                             TRUE, /* copy */
                             initprot, maxprot, VM_INHERIT_DEFAULT);
  }
...
static load_return_t load_segment(struct load_command *lcp, uint32_t filetype,
  ...
  vm_prot_t initprot;
  vm_prot_t maxprot;
  size_t segment_command_size, total_section_size, single_section_size;
  ...
  if (verbose) {
    MACHO_PRINTF(("+++ load_segment %s "
                  ...
                  "prot %d/%d flags 0x%x\n",
                  scp->segname, (uint64_t)(slide + scp->vmaddr),
                  (uint64_t)(slide + scp->vmaddr + scp->vmsize),
                  pager_offset + scp->fileoff,
                  pager_offset + scp->fileoff + scp->filesize, scp->initprot,
                  scp->maxprot, scp->flags));
  }
  ...
  if (scp->fileoff + scp->filesize < scp->fileoff ||
    ...
    DEBUG4K_ERROR(
        "LOAD_BADMACHO fileoff 0x%llx filesize 0x%llx macho_size 0x%llx\n",
        scp->fileoff, scp->filesize, (uint64_t)macho_size);
    return LOAD_BADMACHO;
  }
  /*
  ...
  if (total_section_size / single_section_size < scp->nsects) {
    DEBUG4K_ERROR("LOAD_BADMACHO 0x%zx 0x%zx %d\n", total_section_size,
                  single_section_size, scp->nsects);
    return LOAD_BADMACHO;
  }
  /*
   ...
   * of the file.
   */
  if (result->cs_end_offset && result->cs_end_offset < (off_t)scp->fileoff &&
      result->cs_end_offset - scp->fileoff < scp->filesize) {
    if (cs_debug) {
      printf("section outside code signature\n");
    }
    DEBUG4K_ERROR(
        "LOAD_BADMACHO end_offset 0x%llx fileoff 0x%llx filesize 0x%llx\n",
        result->cs_end_offset, scp->fileoff, scp->filesize);
    return LOAD_BADMACHO;
  }
  ...
  if (scp->vmaddr == 0 && file_size == 0 && vm_size != 0 &&
    ...
    if (vm_end < vm_offset) {
      DEBUG4K_ERROR(
          "LOAD_BADMACHO vm_end 0x%llx vm_offset 0x%llx vm_size 0x%llx\n",
    ...
    if (fourk_align) {
      ...
      vm_end_aligned = vm_map_trunc_page(vm_end, effective_page_mask);
    } else
#endif /* __arm64__ */
    {
      vm_end = vm_map_round_page(vm_end, PAGE_MASK_64);
      vm_end_aligned = vm_end;
    }
    ret = vm_map_raise_min_offset(map, vm_end_aligned);
    if (ret != KERN_SUCCESS) {
    ...
    if (filetype != MH_DYLINKER && (imgp->ip_flags & IMGPF_ROSETTA) == 0 &&
        scp->vmaddr == 0) {
  ...
  if (fourk_align) {
    /* 4K-align */
    file_start = vm_map_trunc_page(file_offset, FOURK_PAGE_MASK);
    file_end = vm_map_round_page(file_offset + file_size, FOURK_PAGE_MASK);
    vm_start = vm_map_trunc_page(vm_offset, FOURK_PAGE_MASK);
    vm_end = vm_map_round_page(vm_offset + vm_size, FOURK_PAGE_MASK);
    ...
    if (file_offset - file_start > FOURK_PAGE_MASK ||
      ...
      DEBUG4K_ERROR("LOAD_BADMACHO file_start / file_size wrap "
                    ...
                    file_offset, file_offset + file_size, (uint64_t)file_start,
                    (uint64_t)file_end);
      return LOAD_BADMACHO;
    }

    if (!strncmp(scp->segname, "__LINKEDIT", 11) && page_aligned(file_start) &&
      ...
      file_end = vm_map_round_page(file_end, effective_page_mask);
      vm_end = vm_map_round_page(vm_end, effective_page_mask);
    }
  } else
#endif /* __arm64__ */
  {
  ...
  if (vm_size > 0) {
    ...
    if (verbose) {
      MACHO_PRINTF(("++++++ load_segment: "
                    ...
                    "file [0x%llx:0x%llx]\n",
                    (uint64_t)vm_start, (uint64_t)vm_end, (uint64_t)file_start,
                    (uint64_t)file_end));
    }
    ret = map_segment(map, vm_start, vm_end, vmk_flags, control, file_start,
                      file_end, initprot, maxprot, result);
    if (ret) {
  ...
  if (delta_size > 0) {
    ...
    if (verbose) {
      MACHO_PRINTF(("++++++ load_segment: "
                    ...
                    (uint64_t)tmp_start, (uint64_t)tmp_end));
    }
    kr = map_segment(map, tmp_start, tmp_end, vmk_flags,
                     MEMORY_OBJECT_CONTROL_NULL, 0, delta_size, scp->initprot,
                     scp->maxprot, result);
    if (kr != KERN_SUCCESS) {
  ...
  if (scp->flags & SG_PROTECTED_VERSION_1) {
    ret =
        unprotect_dsmos_segment(file_start, file_end - file_start, vp,
                                pager_offset, map, vm_start, vm_end - vm_start);
    if (ret != LOAD_SUCCESS) {
  ...
  if (LOAD_SUCCESS == ret && filetype == MH_DYLINKER &&
    ...
    note_all_image_info_section(
        scp, LC_SEGMENT_64 == lcp->cmd, single_section_size,
        ((const char *)lcp + segment_command_size), slide, result);
  }

  if (result->entry_point != MACH_VM_MIN_ADDRESS) {
    if ((result->entry_point >= vm_offset) &&
        (result->entry_point < (vm_offset + vm_size))) {
      if ((scp->initprot & (VM_PROT_READ | VM_PROT_EXECUTE)) ==
          (VM_PROT_READ | VM_PROT_EXECUTE)) {
        result->validentry = 1;
      } else {
...
static load_return_t load_uuid(struct uuid_command *uulp, char *command_end,
   ...
   * - The UUID part of the command should be completely within the mach-o
   * header
   */
...
static load_return_t load_version(struct version_min_command *vmc,
  ...
  *found_version_cmd = TRUE;
  ...
  case LC_VERSION_MIN_TVOS:
    ...
    break;
#endif /* __x86_64__ */
  /* All LC_VERSION_MIN_* load commands are legacy and we will not be adding any
   * more */
  default:
    sdk = (uint32_t)-1;
    min_sdk = (uint32_t)-1;
...
static load_return_t load_main(struct entry_point_command *epc, thread_t thread,
   ...
   */
  if (epc->stacksize) {
    if (os_add_overflow(epc->stacksize, 4 * PAGE_SIZE,
    ...
    if (os_add_overflow(epc->stacksize, PAGE_SIZE,
                        &result->user_stack_alloc_size)) {
      return LOAD_BADMACHO;
    }
  ...
  result->user_stack =
      (user_addr_t)mach_vm_trunc_page((user_addr_t)addr - slide);
...
static load_return_t load_unixthread(struct thread_command *tcp,
  ...
  ret = load_threadstack(
      thread, (uint32_t *)(((vm_offset_t)tcp) + sizeof(struct thread_command)),
      tcp->cmdsize - sizeof(struct thread_command), &addr, &customstack,
      is_x86_64_compat_binary, result);
  if (ret != LOAD_SUCCESS) {
    return ret;
  }
  ...
  {
    ret = load_threadentry(
        thread,
        (uint32_t *)(((vm_offset_t)tcp) + sizeof(struct thread_command)),
        tcp->cmdsize - sizeof(struct thread_command), &addr);
    if (ret != LOAD_SUCCESS) {
      return ret;
    }
    ...
    ret = load_threadstate(
        thread,
        (uint32_t *)(((vm_offset_t)tcp) + sizeof(struct thread_command)),
        tcp->cmdsize - sizeof(struct thread_command), result);
    if (ret != LOAD_SUCCESS) {
      return ret;
    }
  }
...
load_threadstack(thread_t thread, uint32_t *ts, uint32_t total_size,
  ...
  while (total_size > 0) {
     ...
     */
    {
      ret = thread_userstack(thread, flavor, (thread_state_t)ts, size,
                             user_stack, customstack, result->is_64bit_data);
      if (ret != KERN_SUCCESS) {
        return LOAD_FAILURE;
      }
    }
...
static load_return_t load_threadentry(thread_t thread, uint32_t *ts,
  ...
  *entry_point = MACH_VM_MIN_ADDRESS;
  while (total_size > 0) {
     ...
     * the value of flavor.
     */
    ret = thread_entrypoint(thread, flavor, (thread_state_t)ts, size,
                            entry_point);
    if (ret != KERN_SUCCESS) {
      return LOAD_FAILURE;
    }
...
static load_return_t load_dylinker(struct dylinker_command *lcp,
                                   cpu_type_t cputype, vm_map_t map,
                                   thread_t thread, int depth, int64_t slide,
                                   load_result_t *result,
                                   struct image_params *imgp) {
  const char *name;
  ...
  if (use_alt_dyld) {
    ...
    policy_error =
        proc_uuid_policy_lookup(result->uuid, &policy_flags, &policy_gencount);
    if (policy_error == 0) {
...
#define countof(x) (sizeof(x) / sizeof(x[0]))
  ...
  ret = get_macho_vnode(name, cputype, header, &file_offset, &macho_size,
                        macho_data, &vp, imgp);
  if (ret) {
    goto novp_out;
  }
  ...
  *myresult = load_result_null;
  ...
  ret = parse_machfile(vp, map, thread, header, file_offset, macho_size, depth,
                       slide, 0, myresult, result, imgp);
...
#if (DEVELOPMENT || DEBUG)
static const char *rosetta_runtime_path_alt_x86 =
    "/usr/local/libexec/rosetta/runtime_internal";
static const char *rosetta_runtime_path_alt_arm =
    "/usr/local/libexec/rosetta/runtime_arm_internal";
#endif
...
static load_return_t load_rosetta(vm_map_t map, thread_t thread,
  ...
  if (imgp->ip_flags & IMGPF_ALT_ROSETTA) {
    ...
    policy_error =
        proc_uuid_policy_lookup(result->uuid, &policy_flags, &policy_gencount);
    if (policy_error == 0 &&
        (policy_flags & PROC_UUID_ALT_ROSETTA_POLICY) != 0) {
      use_alt_rosetta = true;
    }
  }
  ...
  ret = get_macho_vnode(rosetta_file_path, CPU_TYPE_ARM64, header, &file_offset,
                        &macho_size, macho_data, &vp, imgp);
  if (ret) {
    goto novp_out;
  }
  ...
  *myresult = load_result_null;
  ...
  ret = parse_machfile(vp, NULL, NULL, header, file_offset, macho_size, 2, 0, 0,
                       myresult, NULL, imgp);
  if (ret != LOAD_SUCCESS) {
    goto out;
  }
  ...
  *myresult = load_result_null;
  ...
  ret = parse_machfile(vp, map, thread, header, file_offset, macho_size, 2,
                       rosetta_load_addr, 0, myresult, result, imgp);
  if (ret == LOAD_SUCCESS) {
    if (result) {
      if (result->threadstate) {
...
static void set_signature_error(struct vnode *vp, struct image_params *imgp,
  ...
  if (reason == OS_REASON_NULL) {
    printf("load_code_signature: %s: failure to allocate exit reason for "
  ...
  if ((reason_error = os_reason_alloc_buffer_noblock(
           reason, kcdata_estimate_required_buffer_size(
                       1, (uint32_t)fatal_failure_desc_len))) == 0 &&
      (kcdata_error = kcdata_get_memory_addr(
           &reason->osr_kcd_descriptor, EXIT_REASON_USER_DESC,
           (uint32_t)fatal_failure_desc_len, &data_addr)) == KERN_SUCCESS) {
    kern_return_t mc_error =
    ...
    printf(
        "load_code_signature: %s: failed to allocate space for reason string "
...
static load_return_t load_code_signature(struct linkedit_data_command *lcp,
  ...
  if (blob != NULL) {
    ...
    if ((blob->csb_cpu_type != cputype && blob->csb_cpu_subtype != cpusubtype &&
         !anyCPU) ||
    ...
    if (error == 0) {
      ...
      if (anyCPU) {
        vnode_lock_spin(vp);
        struct cs_cpu_info cpu_info = {.csb_cpu_type = cputype,
    ...
    /*
     * Revalidation was not possible for this blob. We just continue as if there
     * was no blob, rereading the signature, and ubc_cs_blob_add will do the
     * right thing.
     */
    blob = NULL;
  }
  ...
  resid = 0;
  error = vn_rdwr(UIO_READ, vp, (caddr_t)addr, lcp->datasize,
                  macho_offset + lcp->dataoff, UIO_SYSSPACE, 0,
                  kauth_cred_get(), &resid, current_proc());
  if (error || resid != 0) {
    ret = LOAD_IOERROR;
    goto out;
  }

  if (ubc_cs_blob_add(vp, result->ip_platform, cputype, cpusubtype,
                      macho_offset, &addr, lcp->datasize, imgp, 0, &blob,
...
static load_return_t set_code_unprotect(struct encryption_info_command *eip,
                                        caddr_t addr, vm_map_t map,
                                        int64_t slide, struct vnode *vp,
                                        off_t macho_offset, cpu_type_t cputype,
                                        cpu_subtype_t cpusubtype) {
  ...
  if (eip->cryptsize == 0) {
    printf(
        ...
        __FUNCTION__, __LINE__, vpath, (uint64_t)eip->cryptoff,
        (uint64_t)eip->cryptsize, eip->cryptid);
    zfree(ZV_NAMEI, vpath);
    return LOAD_SUCCESS;
  }
  ...
  if (vm_map_debug_apple_protect) {
    ...
    printf("APPLE_PROTECT: %d[%s] map %p %s(%s) -> 0x%x\n", proc_getpid(p),
           p->p_comm, map, __FUNCTION__, vpath, kr);
  }
#endif /* VM_MAP_DEBUG_APPLE_PROTECT */
  zfree(ZV_NAMEI, vpath);
  ...
  while (ncmds--) {
    ...
    case LC_SEGMENT_64:
      ...
      if ((seg64->fileoff <= eip->cryptoff) &&
        ...
        map_offset = (vm_map_offset_t)(seg64->vmaddr + eip->cryptoff -
                                       seg64->fileoff + slide);
    ...
    case LC_SEGMENT:
      ...
      if ((seg32->fileoff <= eip->cryptoff) &&
          (seg32->fileoff + seg32->filesize >=
           eip->cryptoff + eip->cryptsize)) {
        map_offset = (vm_map_offset_t)(seg32->vmaddr + eip->cryptoff -
                                       seg32->fileoff + slide);
        map_size = eip->cryptsize;
        crypto_backing_offset = macho_offset + eip->cryptoff;
        goto remap_now;
...
remap_now:
  ...
  MACHO_PRINTF(("+++ set_code_unprotect: vm[0x%llx:0x%llx]\n",
                (uint64_t)map_offset, (uint64_t)(map_offset + map_size)));
  kr = vm_map_apple_protected(map, map_offset, map_offset + map_size,
                              crypto_backing_offset, &crypt_info,
                              CRYPTID_APP_ENCRYPTION);
  if (kr) {
 ...
 */
static load_return_t get_macho_vnode(const char *path, cpu_type_t cputype,
                                     struct mach_header *mach_header,
                                     off_t *file_offset, off_t *macho_size,
  ...
  NDINIT(ndp, LOOKUP, OP_OPEN, FOLLOW | LOCKLEAF, UIO_SYSSPACE,
  ...
  if ((error = vnode_authorize(
           vp, NULL, KAUTH_VNODE_EXECUTE | KAUTH_VNODE_READ_DATA, ctx)) != 0) {
    error = LOAD_PROTECT;
    goto bad1;
  }
  ...
  if (is_fat) {
    ...
    error = fatfile_getbestarch_for_cputype(cputype, CPU_SUBTYPE_ANY,
                                            (vm_offset_t)(&header->fat_header),
                                            sizeof(*header), imgp, &fat_arch);
    if (error != LOAD_SUCCESS) {
      goto bad2;
    }
    ...
    error = vn_rdwr(UIO_READ, vp, (caddr_t)&header->mach_header,
                    sizeof(header->mach_header), fat_arch.offset, UIO_SYSSPACE,
                    IO_NODELOCKED, kerncred, &resid, p);
    if (error) {
      error = LOAD_IOERROR;
      goto bad2;
    }
