 ...
 *		size:			How large the fat file header is
 * (including fat_arch array) req_cpu_type:	The required cpu type.
 ...
 **********************************************************************/
static load_return_t fatfile_getarch(vm_offset_t data_ptr, vm_size_t data_size,
  ...
  max_nfat_arch =
      (data_size - sizeof(struct fat_header)) / sizeof(struct fat_arch);
  ...
  for (; nfat_arch-- > 0; arch++) {
     ...
     */
    if (!binary_match(mask_bits, req_cpu_type, req_subcpu_type, testtype,
                      testsubtype)) {
      continue;
    }
  ...
  if (best_arch != NULL &&
      validate_potential_simulator_binary(
          OSSwapBigToHostInt32(best_arch->cputype), imgp,
  ...
  if (best_arch == NULL) {
    ...
    archret->cputype = OSSwapBigToHostInt32(best_arch->cputype);
    archret->cpusubtype = OSSwapBigToHostInt32(best_arch->cpusubtype);
    archret->offset = OSSwapBigToHostInt32(best_arch->offset);
    archret->size = OSSwapBigToHostInt32(best_arch->size);
    archret->align = OSSwapBigToHostInt32(best_arch->align);
...
load_return_t fatfile_getbestarch(vm_offset_t data_ptr, vm_size_t data_size,
   ...
   */
  load_return_t ret =
      fatfile_getarch(data_ptr, data_size, primary_type, CPU_ARCH_MASK,
                      CPU_SUBTYPE_ANY, imgp, archret);
  return ret;
}
...
load_return_t fatfile_getbestarch_for_cputype(
   ...
   */
  return fatfile_getarch(data_ptr, data_size, cputype, 0, cpusubtype, imgp,
                         archret);
}
...
load_return_t fatfile_getarch_with_bits(integer_t archbits,
   ...
   */
  return fatfile_getarch(data_ptr, data_size,
                         (archbits & CPU_ARCH_MASK) |
                             (cpu_type() & ~CPU_ARCH_MASK),
                         0, CPU_SUBTYPE_ANY, NULL, archret);
}
...
load_return_t fatfile_validate_fatarches(vm_offset_t data_ptr,
  ...
  /* now that we know the fat_arch list fits in the buffer, how much does it
   * use? */
  fat_header_size =
      sizeof(struct fat_header) + nfat_arch * sizeof(struct fat_arch);
  ...
  for (i = 0; i < nfat_arch; i++) {
    ...
    if (i_begin < fat_header_size) {
      /* slice is trying to claim part of the file used by fat headers
       * themselves */
      return LOAD_BADMACHO;
    }
