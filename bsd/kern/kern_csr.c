...
static bool _csr_get_dt_bool(DTEntry *entry, char const *name, bool *out) {
  const uint32_t *value;
  unsigned int size;

  if (SecureDTGetProperty(*entry, name, (const void **)&value, &size) !=
      kSuccess) {
    return false;
  }
...
// WARNING: Used extremely early during boot. See csr_bootstrap().
static bool _csr_get_dt_uint64(DTEntry *entry, char const *name,
                               uint64_t *out) {
...
// WARNING: Used extremely early during boot. See csr_bootstrap().
static bool _csr_dt_string_is_equal(DTEntry *entry, const char *name,
                                    const char *str) {
  const void *value;
  unsigned size;
  size_t str_size;
...
static bool _csr_is_recovery_environment(void) {
  DTEntry chosen;
...
static bool _csr_is_restore_environment(void) {
  int notused;

  return PE_parse_boot_argn("-restore", &notused, sizeof(notused));
}

static bool _csr_is_darwinos_ramdisk(void) {
  DTEntry chosen;
...
static bool _csr_is_iuou_or_iuos_device(void) {
  DTEntry entry;
  bool bool_value;

  return (SecureDTLookupEntry(0, "/chosen", &entry) == kSuccess &&
          (_csr_get_dt_bool(&entry, "internal-use-only-unit", &bool_value) &&
           bool_value)) ||
         (SecureDTLookupEntry(0, "/chosen/manifest-properties", &entry) ==
              kSuccess &&
          (_csr_get_dt_bool(&entry, "iuos", &bool_value) && bool_value));
}

static bool _csr_should_allow_device_configuration(void) {
  /*
  ...
  return _csr_is_recovery_environment() ||
         (SecureDTLookupEntry(0, "/chosen", &chosen) == kSuccess &&
          _csr_get_dt_bool(&chosen, "csr-allow-device-configuration",
                           &bool_value) &&
          bool_value);
}
 ...
 *
 */
__startup_func static void csr_bootstrap(void) {
  DTEntry entry;
...
int csr_check(csr_config_t mask) {
  ...
  if (ret == EPERM) {
    // Override the return value if booted from the BaseSystem and the mask does
    // not contain any flag that should always be enforced.
...
int csrctl(__unused proc_t p, struct csrctl_args *args,
