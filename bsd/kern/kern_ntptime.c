...
SYSCTL_INT(_kern, OID_AUTO, log_clock_adjustments, CTLFLAG_RW | CTLFLAG_LOCKED,
           &g_should_log_clock_adjustments, 0,
           "enable kernel clock adjustment logging");
#endif
...
static void ntp_gettime1(struct ntptimeval *ntvp) {
  ...
  if ((unsigned long)atv.tv_sec > last_time_maxerror_update) {
    time_maxerror +=
        (MAXFREQ / 1000) * (atv.tv_sec - last_time_maxerror_update);
...
int ntp_gettime(struct proc *p, struct ntp_gettime_args *uap,
...
int ntp_adjtime(struct proc *p, struct ntp_adjtime_args *uap, int32_t *retval) {
  struct timex ntv = {};
  long freq;
  unsigned int modes;
  int error, ret = 0;
  ...
  if (g_should_log_clock_adjustments) {
    os_log(OS_LOG_DEFAULT,
           "%s: BEFORE modes %u offset %ld freq %ld status %d constant %ld "
           "time_adjtime %lld\n",
           __func__, ntv.modes, ntv.offset, ntv.freq, ntv.status, ntv.constant,
           time_adjtime);
  }
#endif
  /*
  ...
  if (g_should_log_clock_adjustments) {
    os_log(OS_LOG_DEFAULT,
           "%s: AFTER modes %u offset %lld freq %lld status %d constant %ld "
           "time_adjtime %lld\n",
           __func__, modes, time_offset, time_freq, time_status, time_constant,
           time_adjtime);
  }
#endif
...
int64_t ntp_get_freq(void) { return time_freq; }
...
void ntp_update_second(int64_t *adjustment, clock_sec_t secs) {
  ...
  if (old_time_adjtime ||
      ((time_offset || old_offset) && (time_offset != old_offset))) {
    updated = 1;
  } else {
    updated = 0;
  }
  ...
  if (g_should_log_clock_adjustments) {
    ...
    int64_t frac =
        (time_adj > 0) ? ((uint32_t)time_adj) : -((uint32_t)(-time_adj));

    os_log(OS_LOG_DEFAULT,
           "%s:AFTER offset %lld (%lld) freq %lld status %d "
           "constant %ld time_adjtime %lld nano %lld frac %lld adj %lld\n",
           __func__, time_offset,
           (time_offset > 0) ? time_offset >> 32 : -((-time_offset) >> 32),
           time_freq, time_status, time_constant, time_adjtime, nano, frac,
           time_adj);
  }
#endif
...
static int kern_adjtime(struct timeval *delta) {
  struct timeval atv;
  int64_t ltr, ltw;
  boolean_t enable;
  ...
  if (g_should_log_clock_adjustments) {
    os_log(OS_LOG_DEFAULT,
           "%s:AFTER offset %lld freq %lld status %d constant %ld time_adjtime "
           "%lld\n",
           __func__, time_offset, time_freq, time_status, time_constant,
           time_adjtime);
  }
#endif
  NTP_UNLOCK(enable);
...
static void ntp_loop_update_call(void) {
   ...
   * NOTE: clock_update_calendar will call ntp_update_second to compute the next
   * adjustment.
   */
...
static void refresh_ntp_loop(void) {
  ...
  if (--ntp_loop_active == 0) {
     ...
     */
    if (updated) {
      clock_deadline_for_periodic_event(ntp_loop_period, mach_absolute_time(),
                                        &ntp_loop_deadline);

      if (!timer_call_enter(&ntp_loop_update, ntp_loop_deadline,
...
/*
 * This function triggers a timer that each second will calculate the adjustment
 * to provide to clock_calendar to scale the time (used by gettimeofday-family
 * syscalls). The periodic timer will stop when the adjustment will reach a
 * stable value.
 */
static void start_ntp_loop(void) {
  boolean_t enable;
...
static void init_ntp_loop(void) {
  uint64_t abstime;

  ntp_loop_active = 0;
  nanoseconds_to_absolutetime(NTP_LOOP_PERIOD_INTERVAL, &abstime);
  ntp_loop_period = (uint32_t)abstime;
  timer_call_setup(&ntp_loop_update, (timer_call_func_t)ntp_loop_update_call,
                   NULL);
}

void ntp_init(void) { init_ntp_loop(); }
