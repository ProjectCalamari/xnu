 ...
 */
int get_kernel_symfile(__unused proc_t p, __unused char const **symfile);
int get_kernel_symfile(__unused proc_t p, __unused char const **symfile) {
  return KERN_FAILURE;
}
...
static int file_ioctl(void *p1, void *p2, u_long theIoctl, caddr_t result) {
  ...
  return (*bdevsw[major(device)].d_ioctl)(device, theIoctl, result, S_IFBLK,
                                          p2);
}

static int device_ioctl(void *p1, __unused void *p2, u_long theIoctl,
                        caddr_t result) {
  return VNOP_IOCTL(p1, theIoctl, result, 0, p2);
}

static int kern_ioctl_file_extents(struct kern_direct_file_io_ref_t *ref,
                                   u_long theIoctl, off_t offset, off_t end) {
  int error = 0;
  ...
  if (_DKIOCCSPINEXTENT == theIoctl) {
    /* Tell CS the image size, so it knows whether to place the subsequent pins
     * SSD/HDD */
  ...
  for (; offset < end; offset += filechunk) {
    if (ref->vp->v_type == VREG) {
      ...
      error = VNOP_BLOCKMAP(ref->vp, offset, filechunk, &blkno, &filechunk,
                            NULL, VNODE_WRITE | VNODE_BLOCKMAP_NO_TRACK, NULL);
      if (error) {
        break;
      }
      if (-1LL == blkno) {
      ...
      fileblk = offset;
      filechunk =
          (unsigned long)((ref->filelength > ULONG_MAX) ? ULONG_MAX
                                                        : ref->filelength);
    }
    ...
    if (DKIOCUNMAP == theIoctl) {
      ...
      if (error && (ENOTTY != error)) {
        printf("_DKIOCCSPINEXTENT(%d) 0x%qx, 0x%qx\n", error,
               pin.cp_extent.offset, pin.cp_extent.length);
      }
    } else if (_DKIOCCSUNPINEXTENT == theIoctl) {
      pin.cp_extent.offset = fileblk;
      pin.cp_extent.length = filechunk;
...
struct kern_direct_file_io_ref_t *kern_open_file_for_direct_io(
    const char *name, uint32_t iflags,
    kern_get_file_extents_callback_t callback, void *callback_ref,
    off_t set_file_size_min, off_t set_file_size_max, off_t fs_free_size,
    off_t write_file_offset, void *write_file_addr, size_t write_file_len,
    dev_t *partition_device_result, dev_t *image_device_result,
    uint64_t *partitionbase_result, uint64_t *maxiocount_result,
    uint32_t *oflags) {
  ...
  NDINIT(&nd, LOOKUP, OP_OPEN, ndflags, UIO_SYSSPACE,
         CAST_USER_ADDR_T(ref->name), ref->ctx);
  VATTR_INIT(&va);
  ...
  if (write_file_addr && write_file_len) {
    if ((error = kern_write_file(ref, write_file_offset, write_file_addr,
                                 write_file_len, IO_SKIP_ENCRYPTION))) {
  ...
  mpFree <<= 20;
  printf("kern_direct_file(%s): vp size %qd, alloc %qd, mp free %qd, keep free "
         "%qd\n",
  ...
  if (ref->vp->v_type == VREG) {
    ...
    if (ref->wbcranged) {
      uint32_t idx;
      assert(wbc_range.count <=
             (sizeof(wbc_range.extents) / sizeof(wbc_range.extents[0])));
    ...
    if ((set_file_size = set_file_size_max)) {
      ...
      if (fs_free_size) {
        ...
        if ((mpFree < set_file_size) ||
            ((mpFree - set_file_size) < fs_free_size)) {
      ...
      while (TRUE) {
        ...
        error = vnode_setsize(ref->vp, set_file_size, IO_NOZEROFILL | IO_NOAUTH,
                              ref->ctx);
        if ((ENOSPC == error) && set_file_size_min &&
            (set_file_size > set_file_size_min) &&
  ...
  for (; f_offset < ref->filelength; f_offset += filechunk) {
    if (ref->vp->v_type == VREG) {
      ...
      error = VNOP_BLOCKMAP(ref->vp, f_offset, filechunk, &blkno, &filechunk,
                            NULL, VNODE_WRITE | VNODE_BLOCKMAP_NO_TRACK, NULL);
      if (error) {
        goto out;
      }
    ...
    while (physoffset < filechunk) {
      ...
      for (rev = 4096; rev <= getphysreq.length; rev += 4096) {
        callback(callback_ref, getphysreq.offset + getphysreq.length - rev,
                 4096);
      }
#else
  ...
  if (ref->wbcranged) {
    ...
    for (idx = 0; idx < wbc_range.count; idx++) {
      ...
      callback(callback_ref, wbc_range.extents[idx].offset,
               wbc_range.extents[idx].length);
    }
  }
...
out:
  ...
  if (error && ref) {
    if (ref->vp) {
      (void)kern_ioctl_file_extents(ref, _DKIOCCSUNPINEXTENT, 0,
                                    (ref->pinned && ref->cf) ? ref->filelength
                                                             : 0);
...
int kern_write_file(struct kern_direct_file_io_ref_t *ref, off_t offset,
                    void *addr, size_t len, int ioflag) {
  assert(len <= INT32_MAX);
  return vn_rdwr(UIO_WRITE, ref->vp, addr, (int)len, offset, UIO_SYSSPACE,
                 ioflag | IO_SYNC | IO_NODELOCKED | IO_UNIT,
                 vfs_context_ucred(ref->ctx), (int *)0,
                 vfs_context_proc(ref->ctx));
}

int kern_read_file(struct kern_direct_file_io_ref_t *ref, off_t offset,
                   void *addr, size_t len, int ioflag) {
  assert(len <= INT32_MAX);
  return vn_rdwr(UIO_READ, ref->vp, addr, (int)len, offset, UIO_SYSSPACE,
                 ioflag | IO_SYNC | IO_NODELOCKED | IO_UNIT,
                 vfs_context_ucred(ref->ctx), (int *)0,
                 vfs_context_proc(ref->ctx));
}

struct mount *kern_file_mount(struct kern_direct_file_io_ref_t *ref) {
  return ref->vp->v_mount;
}

void kern_close_file_for_direct_io(struct kern_direct_file_io_ref_t *ref,
                                   off_t write_offset, void *addr,
                                   size_t write_length, off_t discard_offset,
                                   off_t discard_end, off_t set_file_size,
                                   bool unlink) {
  int error;
  ...
  if (ref->vp) {
    ...
    discard_offset =
        ((discard_offset + ref->blksize - 1) & ~(((off_t)ref->blksize) - 1));
    ...
    boolean_t will_unmap =
        (!ref->pinned || ref->cf) && (discard_end > discard_offset);
    boolean_t will_unpin = (ref->pinned && ref->cf /* && !will_unmap */);

    (void)kern_ioctl_file_extents(ref, _DKIOCCSUNPINEXTENT, 0,
                                  (will_unpin) ? ref->filelength : 0);

    if (will_unmap) {
      (void)kern_ioctl_file_extents(ref, DKIOCUNMAP, discard_offset,
                                    (ref->cf) ? ref->filelength : discard_end);
    }
    ...
    if (unlink) {
      ...
      error = unlink1(ref->ctx, NULLVP, CAST_USER_ADDR_T(ref->name),
                      UIO_SYSSPACE, 0);
      kprintf("%s: unlink1(%d)\n", __func__, error);
    }
  }
