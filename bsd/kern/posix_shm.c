...
static int pshm_compare(pshm_info_t *a, pshm_info_t *b) {
  int cmp =
      strncmp(a->pshm_hdr.pshm_name, b->pshm_hdr.pshm_name, PSHMNAMLEN + 1);
...
static int pshm_access(pshm_info_t *pinfo, int mode, kauth_cred_t cred,
                       proc_t p);
...
#define PSHM_SUBSYS_ASSERT_HELD()                                              \
  LCK_MTX_ASSERT(&psx_shm_subsys_mutex, LCK_MTX_ASSERT_OWNED)
 ...
 */
void pshm_cache_init(void) { RB_INIT(&pshm_head); }
...
int pshm_cache_purge_all(void) {
  ...
  PSHM_SUBSYS_LOCK();
  RB_FOREACH_SAFE(p, pshmhead, &pshm_head, tmp) { pshm_unlink_internal(p); }
  assert(pshmnument == 0);
  PSHM_SUBSYS_UNLOCK();
 ...
 */
static int pshm_get_name(pshm_info_t *pinfo, const user_addr_t user_addr) {
  size_t bytes_copied = 0;
  int error;

  error = copyinstr(user_addr, &pinfo->pshm_hdr.pshm_name[0], PSHMNAMLEN + 1,
                    &bytes_copied);
  if (error != 0) {
    return error;
  }
 ...
 */
int shm_open(proc_t p, struct shm_open_args *uap, int32_t *retval) {
  int indx;
  int error = 0;
   ...
   * If we find a pre-exisitng one in cache lookup we'll just toss this one
   * later.
   */
  if (fmode & O_CREAT) {
    ...
    have_label = true;
    error = mac_posixshm_check_create(kauth_cred_get(),
                                      new_pinfo->pshm_hdr.pshm_name);
    if (error) {
      goto bad;
    }
#endif
...
#if CONFIG_MACF
    if ((error = mac_posixshm_check_open(kauth_cred_get(), &pinfo->pshm_hdr,
                                         fmode))) {
      goto bad;
    }
#endif
...
#if CONFIG_MACF
    mac_posixshm_label_associate(kauth_cred_get(), &pinfo->pshm_hdr,
                                 pinfo->pshm_hdr.pshm_name);
#endif
  }
 ...
 * only be succesfully done with a non-zero length once per shared memory
 * region.
 */
int pshm_truncate(__unused proc_t p, struct fileproc *fp, __unused int fd,
...
#if CONFIG_MACF
  error =
      mac_posixshm_check_truncate(kauth_cred_get(), &pinfo->pshm_hdr, length);
  if (error) {
    PSHM_SUBSYS_UNLOCK();
    return error;
  }
...
int pshm_stat(pshmnode_t *pnode, void *ub, int isstat64) {
  struct stat *sb =
...
static int pshm_access(pshm_info_t *pinfo, int mode, kauth_cred_t cred,
  ...
  return posix_cred_access(cred, pinfo->pshm_uid, pinfo->pshm_gid,
                           pinfo->pshm_mode, mode_req);
}

int pshm_mmap(__unused proc_t p, vm_map_offset_t user_addr,
              vm_map_size_t user_size, int prot, int flags, struct fileproc *fp,
              off_t file_pos, off_t pageoff, user_addr_t *retval) {
  ...
  pshm_size = vm_map_round_page((vm_map_size_t)pinfo->pshm_length,
...
#if CONFIG_MACF
  error =
      mac_posixshm_check_mmap(kauth_cred_get(), &pinfo->pshm_hdr, prot, flags);
  if (error) {
    PSHM_SUBSYS_UNLOCK();
    return error;
  }
  ...
  if (!(flags & MAP_FIXED)) {
    user_addr = vm_map_round_page(user_addr, vm_map_page_mask(user_map));
    vmk_flags = VM_MAP_KERNEL_FLAGS_ANYWHERE();
  } else {
    if (user_addr != vm_map_round_page(user_addr, vm_map_page_mask(user_map))) {
      error = EINVAL;
      goto out_deref;
    }
  ...
  kret = mach_vm_map_kernel(user_map, &user_addr, user_size, 0, vmk_flags,
  ...
  for (map_pos = 0, pshmobj = SLIST_FIRST(&pinfo->pshm_mobjs); user_size != 0;
       map_pos += pshmobj->pshmo_size,
      pshmobj = SLIST_NEXT(pshmobj, pshmo_next)) {
    if (pshmobj == NULL) {
    ...
    kret = mach_vm_map_kernel(user_map, &user_addr, map_size, 0,
                              VM_MAP_KERNEL_FLAGS_FIXED(.vmf_overwrite = true),
                              pshmobj->pshmo_memobject, file_pos - map_pos,
                              docow, prot, max_prot, VM_INHERIT_SHARE);
    if (kret != KERN_SUCCESS) {
      goto out_deref;
    }
...
out_deref:
  ...
  if (kret != KERN_SUCCESS) {
    if (mapped_size != 0) {
      (void)mach_vm_deallocate(current_map(), user_start_addr, mapped_size);
    }
  }
...
int shm_unlink(proc_t p, struct shm_unlink_args *uap,
   ...
   */
  name_pinfo = kalloc_type(pshm_info_t, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
#if CONFIG_MACF
  error = mac_posixshm_check_unlink(kauth_cred_get(), &pinfo->pshm_hdr,
                                    name_pinfo->pshm_hdr.pshm_name);
  if (error) {
    goto bad_unlock;
  }
#endif
...
static int pshm_closefile(struct fileglob *fg, __unused vfs_context_t ctx) {
  int error = EINVAL;
  pshmnode_t *pnode;
