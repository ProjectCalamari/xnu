...
kern_return_t thread_getstatus(thread_t act, int flavor, thread_state_t tstate,
...
#endif /* SECURE_KERNEL */
__XNU_PRIVATE_EXTERN int sugid_coredump =
 ...
 *		!0	Failure errno
 */
int is_coredump_eligible(proc_t core_proc) {
...
#define COREDUMP_CUSTOM_LOCATION_ENTITLEMENT                                   \
...
cpu_type_t process_cpu_type(proc_t core_proc) {
...
cpu_type_t process_cpu_subtype(proc_t core_proc) {
  cpu_type_t what_we_think;
#if defined(__i386__) || defined(__x86_64__)
  if (IS_64BIT_PROCESS(core_proc)) {
...
static void collectth_state(thread_t th_act, void *tirp) {
  vm_offset_t header;
  size_t hoffset, i;
  mythread_state_flavor_t *flavors;
  ...
  for (i = 0; i < t->flavor_count; i++) {
    *(mythread_state_flavor_t *)(header + hoffset) = flavors[i];
...
#define COREDUMPLOG(fmt, args...)                                              \
  printf("coredump (%s, pid %d): " fmt "\n", core_proc->p_comm,                \
         proc_getpid(core_proc), ##args)
#else
#define COREDUMPLOG(fmt, args...)
#endif
...
static int note_addrable_bits(struct vnode *vp, off_t foffset) {
  ...
  addrable_bits_note_t note = {.version = ADDRABLE_BITS_VER,
                               .addressing_bits =
                                   pmap_user_va_bits(get_task_pmap(t)),
                               .unused = 0};

  return vn_rdwr_64(UIO_WRITE, vp, (vm_offset_t)&note, sizeof(note), foffset,
                    UIO_SYSSPACE, IO_NODELOCKED | IO_UNIT, cred, 0,
                    current_proc());
}
 ...
 * disk at the end of coredump. LC_NOTE's payload has to be written in callbacks
 * here.
 */
static int dump_notes(proc_t __unused core_proc, vm_offset_t header,
                      size_t hoffset, struct vnode *vp, off_t foffset) {
  for (size_t i = 0; i < notes_count; i++) {
    ...
    if (error != KERN_SUCCESS) {
      COREDUMPLOG("failed to write LC_NOTE %s: error %d",
                  core_notes[i].cn_owner, error);
      return error;
    }
 ...
 *		reserve_mb			If non-zero, leave filesystem
 * with at least this much free space. coredump_flags	Extra options (ignore
 * rlimit, run fsync)
 *
 * Returns:	0				Success
 *		!0				Failure errno
...
int coredump(proc_t core_proc, uint32_t reserve_mb, int coredump_flags) {
  ...
  custom_location_entitlement =
      IOCurrentTaskGetEntitlement(COREDUMP_CUSTOM_LOCATION_ENTITLEMENT);
  if (custom_location_entitlement != NULL) {
    ...
    alloced_format =
        kalloc_data(alloced_format_len + 1, Z_ZERO | Z_WAITOK | Z_NOFAIL);
    printed_len = snprintf(alloced_format, alloced_format_len + 1, "%s/%s",
                           dirname, custom_location_entitlement);
  ...
  if (alloced_name == NULL ||
    ...
    snprintf(stack_name, sizeof(stack_name), "/cores/core.%d",
             proc_getpid(core_proc));
    name = stack_name;
  } else {
    name = alloced_name;
  }
  ...
  if ((error = vnode_open(name, (O_CREAT | FWRITE | O_NOFOLLOW), S_IRUSR,
                          VNODE_LOOKUP_NOFOLLOW, &vp, ctx))) {
  ...
  if (vp->v_type != VREG || vnode_getattr(vp, vap, ctx) || vap->va_nlink != 1) {
  ...
  VATTR_INIT(
      vap); /* better to do it here than waste more stack in vnode_setsize */
  ...
  if ((reserve_mb > 0) && ((freespace_mb(vp) - (mapsize >> 20)) < reserve_mb)) {
    COREDUMPLOG(
        "insufficient free space (free=%d MB, needed=%lu MB, reserve=%d MB)",
  ...
  tir1.flavor_count =
      sizeof(thread_flavor_array) / sizeof(mythread_state_flavor_t);
  bcopy(thread_flavor_array, flavors, sizeof(thread_flavor_array));
  tstate_size = 0;
  for (i = 0; i < tir1.flavor_count; i++) {
    tstate_size +=
        sizeof(mythread_state_flavor_t) + (flavors[i].count * sizeof(int));
  }

  {
    ...
    if (os_mul_overflow(segment_count, segment_command_sz, &lhs)) {
      COREDUMPLOG("error: segment size overflow: segment_count=%lu, "
                  "segment_command_sz=%lu",
                  segment_count, segment_command_sz);
      error = ENOMEM;
      goto out;
    }

    /* rhs = (tstate_size + sizeof(struct thread_command)) * thread_count */
    if (os_add_and_mul_overflow(tstate_size, sizeof(struct thread_command),
                                thread_count, &rhs)) {
      COREDUMPLOG("error: thread state size overflow: tstate_size=%lu, "
                  "thread_count=%lu",
    ...
    /* Add notes payload. */
    if (os_mul_overflow(notes_count, sizeof(struct note_command),
                        &notes_size)) {
  ...
  if (os_add_overflow(command_size, mach_header_sz, &header_size)) {
    COREDUMPLOG(
        "error: header size overflow: command_size=%lu, mach_header_sz=%lu",
        command_size, mach_header_sz);
    error = ENOMEM;
    goto out;
  }
  ...
  if (kmem_alloc(kernel_map, &header, (vm_size_t)header_size,
    ...
    COREDUMPLOG("error: failed to allocate memory for header (size=%lu)",
                header_size);
    error = ENOMEM;
    goto out;
  }
  ...
  while (segment_count > 0) {
    ...
    while (1) {
      ...
      if ((kret = mach_vm_region_recurse(
               map, &vmoffset, &vmsize, &nesting_depth,
    ...
    if ((prot | maxprot) == VM_PROT_NONE) {
       ...
       */
      COREDUMPLOG("eliding unreadable segment %llx->%llx", vmoffset,
                  vmoffset + vmsize);
      vmoffset += vmsize;
      continue;
    }
    ...
    if ((prot & VM_PROT_READ) == 0) {
      mach_vm_protect(map, vmoffset, vmsize, FALSE, prot | VM_PROT_READ);
    }
     ...
     */
    int64_t fsize =
        ((maxprot & VM_PROT_READ) == VM_PROT_READ &&
    ...
    if (fsize) {
      int64_t resid = 0;
      const enum uio_seg sflg =
          IS_64BIT_PROCESS(core_proc) ? UIO_USERSPACE64 : UIO_USERSPACE32;

      error = vn_rdwr_64(UIO_WRITE, vp, vmoffset, fsize, foffset, sflg,
                         IO_NODELOCKED | IO_UNIT, cred, &resid, core_proc);
      ...
      if (error) {
         ...
         */
        fsize = 0;
        COREDUMPLOG("failed to write segment %llx->%llx: error %d", vmoffset,
                    vmoffset + vmsize, error);
      } else if (resid) {
        /*
        ...
        vnode_setattr(vp, vap, ctx);
        COREDUMPLOG("partially wrote segment %llx->%llx, resid %lld", vmoffset,
                    vmoffset + vmsize, resid);
      }
    } else {
      COREDUMPLOG("skipping unreadable segment %llx->%llx", vmoffset,
                  vmoffset + vmsize);
    }
    ...
    if (is_64) {
      ...
      sc->fileoff =
          CAST_DOWN_EXPLICIT(uint32_t, foffset); /* will never truncate */
      sc->filesize =
          CAST_DOWN_EXPLICIT(uint32_t, fsize); /* will never truncate */
      sc->maxprot = maxprot;
      sc->initprot = prot;
      sc->nsects = 0;
      sc->flags = 0;
   ...
   */
  error = vn_rdwr(UIO_WRITE, vp, (caddr_t)header,
                  (int)MIN(header_size, INT_MAX), (off_t)0, UIO_SYSSPACE,
...
out2:
  ...
  if (custom_location_entitlement != NULL) {
    kfree_data(custom_location_entitlement,
               custom_location_entitlement_len + 1);
  }
  if (error == 0) {
    error = error1;
  }
