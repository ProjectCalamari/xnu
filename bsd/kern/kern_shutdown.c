...
int get_system_inshutdown() { return system_inshutdown; }

extern int OSKextIsInUserspaceReboot(void);

int get_system_inuserspacereboot() {
  /* set by launchd before performing a userspace reboot */
  return OSKextIsInUserspaceReboot();
}

__abortlike static void panic_kernel(int howto, char *message) {
...
int reboot_kernel(int howto, char *message) {
   ...
   */
  IOSystemShutdownNotification(howto,
                               kIOSystemShutdownNotificationStageProcessExit);
  ...
  if ((howto & RB_QUICK) == RB_QUICK) {
    ...
    IOSystemShutdownNotification(howto,
                                 kIOSystemShutdownNotificationStageRootUnmount);
    ...
    IOSystemShutdownNotification(howto,
                                 kIOSystemShutdownNotificationTerminateDEXTs);
...
static int sd_openlog(vfs_context_t ctx) {
  int error = 0;
  struct timeval tv;

  /* Open shutdown log */
  if ((error = vnode_open(PROC_SHUTDOWN_LOG, (O_CREAT | FWRITE | O_NOFOLLOW),
                          0644, 0, &sd_logvp, ctx))) {
...
static int sd_closelog(vfs_context_t ctx) {
  int error = 0;
  if (sd_logvp != NULLVP) {
...
__printflike(2, 3) static void sd_log(vfs_context_t ctx, const char *fmt, ...) {
  ...
  log_error = vn_rdwr(UIO_WRITE, sd_logvp, (caddr_t)logbuf, len, sd_log_offset,
                      UIO_SYSSPACE, IO_UNIT | IO_NOAUTH, vfs_context_ucred(ctx),
                      &resid, vfs_context_proc(ctx));
...
static int sd_filt1(proc_t p, void *args) {
  ...
  if (((p->p_flag & P_SYSTEM) != 0) || (p->p_ppid == 0) || (p == self) ||
      (p->p_stat == SZOMB) || (p->p_shutdownstate != shutdownstate) ||
...
static int sd_callback1(proc_t p, void *args) {
...
static int sd_filt2(proc_t p, void *args) {
  proc_t self = current_proc();
  struct sd_filterargs *sf = (struct sd_filterargs *)args;
  int delayterm = sf->delayterm;
  int shutdownstate = sf->shutdownstate;
...
static int sd_callback2(proc_t p, void *args) {
  struct sd_iterargs *sd = (struct sd_iterargs *)args;
  int signo = sd->signo;
  int setsdstate = sd->setsdstate;
  int countproc = sd->countproc;
...
static int sd_callback3(proc_t p, void *args) {
  struct sd_iterargs *sd = (struct sd_iterargs *)args;
...
sigterm_loop:
  ...
  if (sdargs.activecount != 0 && proc_shutdown_exitcount != 0) {
    ...
    if (proc_shutdown_exitcount != 0) {
      ...
      ts.tv_sec = 3;
      ts.tv_nsec = 0;
      error = msleep(&proc_shutdown_exitcount, &proc_list_mlock, PWAIT,
                     "shutdownwait", &ts);
      if (error != 0) {
