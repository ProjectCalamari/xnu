...
static const mbuf_flags_t mbuf_flags_mask =
    (MBUF_EXT | MBUF_PKTHDR | MBUF_EOR | MBUF_LOOP | MBUF_BCAST | MBUF_MCAST |
     MBUF_FRAG | MBUF_FIRSTFRAG | MBUF_LASTFRAG | MBUF_PROMISC | MBUF_HASFCS);

/* Unalterable mbuf flags */
static const mbuf_flags_t mbuf_cflags_mask = (MBUF_EXT);

#define MAX_MBUF_TX_COMPL_FUNC 8
mbuf_tx_compl_func mbuf_tx_compl_table[MAX_MBUF_TX_COMPL_FUNC];
...
SYSCTL_NODE(_kern_ipc, OID_AUTO, mbtxcf, CTLFLAG_RW | CTLFLAG_LOCKED, 0, "");
SYSCTL_INT(_kern_ipc_mbtxcf, OID_AUTO, debug, CTLFLAG_RW | CTLFLAG_LOCKED,
           &mbuf_tx_compl_debug, 0, "");
SYSCTL_INT(_kern_ipc_mbtxcf, OID_AUTO, index, CTLFLAG_RD | CTLFLAG_LOCKED,
           &mbuf_tx_compl_index, 0, "");
SYSCTL_QUAD(_kern_ipc_mbtxcf, OID_AUTO, requested, CTLFLAG_RD | CTLFLAG_LOCKED,
            &mbuf_tx_compl_requested, "");
SYSCTL_QUAD(_kern_ipc_mbtxcf, OID_AUTO, callbacks, CTLFLAG_RD | CTLFLAG_LOCKED,
            &mbuf_tx_compl_callbacks, "");
SYSCTL_QUAD(_kern_ipc_mbtxcf, OID_AUTO, aborted, CTLFLAG_RD | CTLFLAG_LOCKED,
            &mbuf_tx_compl_aborted, "");
#endif /* (DEBUG || DEVELOPMENT) */

void *__unsafe_indexable mbuf_data(mbuf_t mbuf) { return m_mtod_current(mbuf); }

errno_t mbuf_data_len(mbuf_t mbuf, void *__sized_by(*out_len) * out_buf,
                      size_t *out_len) {
  size_t len;
  void *buf;
...
errno_t mbuf_attachcluster(mbuf_how_t how, mbuf_type_t type, mbuf_t *mbuf,
  ...
  if ((*mbuf = m_clattach(*mbuf, type, extbuf, extfree, extsize, extarg, how,
                          0)) == NULL) {
    return ENOMEM;
  }
...
errno_t mbuf_ring_cluster_alloc(mbuf_how_t how, mbuf_type_t type, mbuf_t *mbuf,
                                void (*extfree)(caddr_t, u_int, caddr_t),
                                size_t *size) {
  size_t extsize = 0;
  ...
  if ((*mbuf = m_clattach(*mbuf, type, extbuf, extfree, extsize, NULL, how,
                          1)) == NULL) {
...
int mbuf_ring_cluster_is_active(mbuf_t mbuf) {
  return m_ext_paired_is_active(mbuf);
}

errno_t mbuf_ring_cluster_activate(mbuf_t mbuf) {
...
errno_t mbuf_cluster_set_prop(mbuf_t mbuf, u_int32_t oldprop,
                              u_int32_t newprop) {
...
errno_t mbuf_cluster_get_prop(mbuf_t mbuf, u_int32_t *prop) {
...
errno_t mbuf_alloccluster(mbuf_how_t how, size_t *size,
                          char *__sized_by_or_null(*size) * addr) {
...
void mbuf_freecluster(caddr_t addr, size_t size) {
  if (size != MCLBYTES && size != MBIGCLBYTES && size != M16KCLBYTES) {
    panic("%s: invalid size (%ld) for cluster %p", __func__, size,
          (void *)addr);
  }
...
errno_t mbuf_getcluster(mbuf_how_t how, mbuf_type_t type, size_t size,
                        mbuf_t *mbuf) {
  /* Must set *mbuf to NULL in failure case */
  errno_t error = 0;
  int created = 0;
 ...
 */
mbuf_t mbuf_free(mbuf_t mbuf) { return m_free(mbuf); }

/*
 * This function is used to provide m_freem via symbol indirection, please avoid
 * any change in behavior or remove the indirection in config/Unsupported*
 */
void mbuf_freem(mbuf_t mbuf) { m_freem(mbuf); }

int mbuf_freem_list(mbuf_t mbuf) { return m_freem_list(mbuf); }

size_t mbuf_leadingspace(const mbuf_t mbuf) { return M_LEADINGSPACE(mbuf); }
 ...
 * config/Unsupported*
 */
size_t mbuf_trailingspace(const mbuf_t mbuf) { return M_TRAILINGSPACE(mbuf); }

/* Manipulation */
errno_t mbuf_copym(const mbuf_t src, size_t offset, size_t len, mbuf_how_t how,
                   mbuf_t *new_mbuf) {
  /* Must set *mbuf to NULL in failure case */
...
errno_t mbuf_split(mbuf_t src, size_t offset, mbuf_how_t how,
                   mbuf_t *new_mbuf) {
  /* Must set *new_mbuf to NULL in failure case */
...
errno_t mbuf_pulldown(mbuf_t src, size_t *offset, size_t len,
                      mbuf_t *location) {
 ...
 * any change in behavior or remove the indirection in config/Unsupported*
 */
void mbuf_adj(mbuf_t mbuf, int len) { m_adj(mbuf, len); }

errno_t mbuf_adjustlen(mbuf_t m, int amount) {
  /* Verify m_len will be valid after adding amount */
  if (amount > 0) {
    size_t used =
        (size_t)mtod(m, void *) - (size_t)mbuf_datastart(m) + m->m_len;
...
errno_t mbuf_copydata(const mbuf_t m0, size_t off, size_t len,
                      void *out_data __sized_by_or_null(len)) {
...
/* mbuf header */
mbuf_t mbuf_next(const mbuf_t mbuf) { return mbuf->m_next; }

errno_t mbuf_setnext(mbuf_t mbuf, mbuf_t next) {
  if (next && ((next)->m_nextpkt != NULL || (next)->m_type == MT_FREE)) {
    return EINVAL;
  }
  mbuf->m_next = next;
...
mbuf_t mbuf_nextpkt(const mbuf_t mbuf) { return mbuf->m_nextpkt; }

void mbuf_setnextpkt(mbuf_t mbuf, mbuf_t nextpkt) { mbuf->m_nextpkt = nextpkt; }

size_t mbuf_len(const mbuf_t mbuf) { return mbuf->m_len; }

void mbuf_setlen(mbuf_t mbuf, size_t len) { mbuf->m_len = (int32_t)len; }
...
mbuf_type_t mbuf_type(const mbuf_t mbuf) { return mbuf->m_type; }
...
errno_t mbuf_setflags(mbuf_t mbuf, mbuf_flags_t flags) {
  ...
  if ((flags ^ oflags) & mbuf_cflags_mask) {
    ...
    if ((oflags ^ mbuf->m_flags) & M_PKTHDR) {
      ...
      ret = m_reinit(mbuf, (mbuf->m_flags & M_PKTHDR) ? 0 : 1);
    }
  }
...
void *mbuf_pkthdr_header(const mbuf_t mbuf) { return mbuf->m_pkthdr.pkt_hdr; }
...
void mbuf_outbound_finalize(struct mbuf *m, u_int32_t pf, size_t o) {
...
errno_t mbuf_set_csum_requested(mbuf_t mbuf, mbuf_csum_request_flags_t request,
                                u_int32_t value) {
...
errno_t mbuf_get_tso_requested(mbuf_t mbuf, mbuf_tso_request_flags_t *request,
                               u_int32_t *mss) {
  if (mbuf == NULL || (mbuf->m_flags & M_PKTHDR) == 0 || request == NULL ||
      mss == NULL) {
    return EINVAL;
  }
...
errno_t mbuf_get_gso_info(mbuf_t mbuf, mbuf_gso_type_t *type,
                          uint16_t *ret_seg_size, uint16_t *ret_hdr_len) {
  mbuf_tso_request_flags_t flags;
  uint16_t hdr_len = 0;
  uint16_t seg_size = 0;

  if (mbuf == NULL || (mbuf->m_flags & M_PKTHDR) == 0 || type == NULL ||
      ret_seg_size == NULL || ret_hdr_len == NULL) {
    return EINVAL;
  }
...
errno_t mbuf_set_gso_info(mbuf_t mbuf, mbuf_gso_type_t type, uint16_t seg_size,
                          uint16_t hdr_len) {
...
errno_t mbuf_get_lro_info(mbuf_t mbuf, uint8_t *seg_cnt, uint8_t *dup_ack_cnt) {
  errno_t error = EINVAL;
...
errno_t mbuf_set_lro_info(mbuf_t mbuf, uint8_t seg_cnt, uint8_t dup_ack_cnt) {
  errno_t error = EINVAL;

  if (mbuf == NULL || (mbuf->m_flags & M_PKTHDR) == 0 || dup_ack_cnt != 0 ||
      seg_cnt == 1) {
    goto done;
  }
...
errno_t mbuf_get_csum_requested(mbuf_t mbuf, mbuf_csum_request_flags_t *request,
                                u_int32_t *value) {
  *request = mbuf->m_pkthdr.csum_flags;
  *request &= mbuf_valid_csum_request_flags;
  if (value != NULL) {
...
errno_t mbuf_tag_id_find(const char *string, mbuf_tag_id_t *out_id) {
  return net_str_id_find_internal(string, out_id, NSI_MBUF_TAG, 1);
}

errno_t mbuf_tag_allocate(mbuf_t mbuf, mbuf_tag_id_t id, mbuf_tag_type_t type,
                          size_t length, mbuf_how_t how, void **data_p) {
  struct m_tag *tag;
...
errno_t mbuf_tag_find(mbuf_t mbuf, mbuf_tag_id_t id, mbuf_tag_type_t type,
                      size_t *length, void **data_p) {
  struct m_tag *tag;
  u_int32_t mtag_id_first, mtag_id_last;
...
void mbuf_tag_free(mbuf_t mbuf, mbuf_tag_id_t id, mbuf_tag_type_t type) {
  struct m_tag *tag;
  u_int32_t mtag_id_first, mtag_id_last;
...
#define MBUF_DRVAUX_MAXLEN                                                     \
  P2ROUNDDOWN(MLEN - M_TAG_ALIGN(sizeof(struct m_drvaux_tag)), sizeof(uint64_t))

errno_t mbuf_add_drvaux(mbuf_t mbuf, mbuf_how_t how, u_int32_t family,
                        u_int32_t subfamily, size_t length, void **data_p) {
  struct m_drvaux_tag *p;
  struct m_tag *tag;

  if (mbuf == NULL || !(mbuf->m_flags & M_PKTHDR) || length == 0 ||
      length > MBUF_DRVAUX_MAXLEN) {
    return EINVAL;
  }
...
errno_t mbuf_find_drvaux(mbuf_t mbuf, u_int32_t *family_p,
...
void mbuf_del_drvaux(mbuf_t mbuf) {
  struct m_tag *tag;
...
errno_t mbuf_allocpacket(mbuf_how_t how, size_t packetlen,
...
__private_extern__ size_t mbuf_pkt_list_len(mbuf_t m) {
  size_t len = 0;
  mbuf_t n = m;
...
__private_extern__ size_t mbuf_pkt_list_maxlen(mbuf_t m) {
  size_t maxlen = 0;
  mbuf_t n = m;
 ...
 */
errno_t mbuf_copyback(mbuf_t m, size_t off, size_t len0,
...
u_int32_t mbuf_get_mlen(void) { return _MLEN; }

u_int32_t mbuf_get_mhlen(void) { return _MHLEN; }

u_int32_t mbuf_get_minclsize(void) { return MHLEN + MLEN; }

u_int32_t mbuf_get_msize(void) { return _MSIZE; }

u_int32_t mbuf_get_traffic_class_max_count(void) { return MBUF_TC_MAX; }

errno_t mbuf_get_traffic_class_index(mbuf_traffic_class_t tc,
                                     u_int32_t *index) {
...
mbuf_traffic_class_t mbuf_get_traffic_class(mbuf_t m) {
...
errno_t mbuf_set_traffic_class(mbuf_t m, mbuf_traffic_class_t tc) {
  if (m == NULL || !(m->m_flags & M_PKTHDR) || ((u_int32_t)tc >= MBUF_TC_MAX)) {
    return EINVAL;
  }
...
int mbuf_is_traffic_class_privileged(mbuf_t m) {
  if (m == NULL || !(m->m_flags & M_PKTHDR) ||
...
u_int32_t mbuf_get_service_class_max_count(void) { return MBUF_SC_MAX_CLASSES; }

errno_t mbuf_get_service_class_index(mbuf_svc_class_t sc, u_int32_t *index) {
...
mbuf_svc_class_t mbuf_get_service_class(mbuf_t m) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return MBUF_SC_BE;
  }
...
errno_t mbuf_set_service_class(mbuf_t m, mbuf_svc_class_t sc) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
...
errno_t mbuf_pkthdr_aux_flags(mbuf_t m, mbuf_pkthdr_aux_flags_t *flagsp) {
  u_int32_t flags;
...
errno_t mbuf_get_driver_scratch(mbuf_t m, u_int8_t **area, size_t *area_len) {
...
errno_t mbuf_get_unsent_data_bytes(const mbuf_t m, u_int32_t *unsent_data) {
  ...
  *unsent_data = m->m_pkthdr.bufstatus_if + m->m_pkthdr.bufstatus_sndbuf;
  return 0;
}

errno_t mbuf_get_buffer_status(const mbuf_t m,
...
errno_t mbuf_pkt_new_flow(const mbuf_t m, u_int32_t *retval) {
...
errno_t mbuf_last_pkt(const mbuf_t m, u_int32_t *retval) {
  if (m == NULL || retval == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
...
errno_t mbuf_get_timestamp(mbuf_t m, u_int64_t *ts, boolean_t *valid) {
...
errno_t mbuf_set_timestamp(mbuf_t m, u_int64_t ts, boolean_t valid) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
...
errno_t mbuf_get_status(mbuf_t m, kern_return_t *status) {
...
static void driver_mtag_init(mbuf_t m) {
  if ((m->m_pkthdr.pkt_flags & PKTF_DRIVER_MTAG) == 0) {
    m->m_pkthdr.pkt_flags |= PKTF_DRIVER_MTAG;
    bzero(&m->m_pkthdr.driver_mtag, sizeof(m->m_pkthdr.driver_mtag));
  }
}

errno_t mbuf_set_status(mbuf_t m, kern_return_t status) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
...
errno_t mbuf_get_flowid(mbuf_t m, u_int16_t *flowid) {
...
errno_t mbuf_set_flowid(mbuf_t m, u_int16_t flowid) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
...
errno_t mbuf_get_tx_compl_data(mbuf_t m, uintptr_t *arg, uintptr_t *data) {
  if (m == NULL || !(m->m_flags & M_PKTHDR) || arg == NULL || data == NULL) {
    return EINVAL;
  }
...
errno_t mbuf_set_tx_compl_data(mbuf_t m, uintptr_t arg, uintptr_t data) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
...
static u_int32_t get_tx_compl_callback_index(mbuf_tx_compl_func callback) {
  u_int32_t i;
...
mbuf_tx_compl_func m_get_tx_compl_callback(u_int32_t idx) {
...
errno_t mbuf_register_tx_compl_callback(mbuf_tx_compl_func callback) {
  int i;
  errno_t error;
...
errno_t mbuf_unregister_tx_compl_callback(mbuf_tx_compl_func callback) {
  int i;
  errno_t error;
...
errno_t mbuf_get_timestamp_requested(mbuf_t m, boolean_t *requested) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
...
errno_t mbuf_set_timestamp_requested(mbuf_t m, uintptr_t *pktid,
...
void m_do_tx_compl_callback(struct mbuf *m, struct ifnet *ifp) {
  int i;
  ...
  for (i = 0; i < MAX_MBUF_TX_COMPL_FUNC; i++) {
    ...
    if (callback != NULL) {
      callback(m->m_pkthdr.pkt_compl_context, ifp,
               (m->m_pkthdr.pkt_flags & PKTF_TS_VALID)
                   ? m->m_pkthdr.pkt_timestamp
                   : 0,
...
errno_t mbuf_get_keepalive_flag(mbuf_t m, boolean_t *is_keepalive) {
...
errno_t mbuf_set_keepalive_flag(mbuf_t m, boolean_t is_keepalive) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
...
errno_t mbuf_get_wake_packet_flag(mbuf_t m, boolean_t *is_wake_packet) {
...
errno_t mbuf_set_wake_packet_flag(mbuf_t m, boolean_t is_wake_packet) {
  if (m == NULL || !(m->m_flags & M_PKTHDR)) {
    return EINVAL;
  }
