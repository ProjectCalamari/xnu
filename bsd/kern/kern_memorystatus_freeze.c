...
unsigned int memorystatus_freeze_suspended_threshold =
    FREEZE_SUSPENDED_THRESHOLD_DEFAULT;
unsigned int memorystatus_freeze_daily_mb_max = FREEZE_DAILY_MB_MAX_DEFAULT;
uint64_t memorystatus_freeze_budget_pages_remaining =
    0; /* Remaining # of pages that can be frozen to disk */
uint64_t memorystatus_freeze_budget_multiplier =
    100; /* Multiplies the daily budget by 100/multiplier */
boolean_t memorystatus_freeze_degradation =
    FALSE; /* Protected by the freezer mutex. Signals we are in a degraded
              freeze mode. */
...
#endif
boolean_t memorystatus_freeze_dynamic_thread_delay_enabled =
    FREEZE_DYNAMIC_THREAD_DELAY_ENABLED_DEFAULT;
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_dynamic_thread_delay_enabled,
            CTLFLAG_RW | CTLFLAG_LOCKED,
            &memorystatus_freeze_dynamic_thread_delay_enabled, 0, "");

#define FREEZE_APPS_IDLE_DELAY_MULTIPLIER_FAST 1
#define FREEZE_APPS_IDLE_DELAY_MULTIPLIER_SLOW 30
#define FREEZE_APPS_IDLE_DELAY_MULTIPLIER_DEFAULT                              \
  FREEZE_APPS_IDLE_DELAY_MULTIPLIER_FAST
unsigned int memorystatus_freeze_apps_idle_delay_multiplier =
    FREEZE_APPS_IDLE_DELAY_MULTIPLIER_DEFAULT;
...
#define MAX_XPC_SERVICE_PIDS                                                   \
  10 /* Max. # of XPC services per coalition we'll consider freezing. */
...
unsigned int memorystatus_freeze_shared_mb_per_process_max =
    0; /* Max. MB allowed per process to be freezer-eligible. */
#if XNU_TARGET_OS_WATCH
unsigned int memorystatus_freeze_private_shared_pages_ratio =
    1; /* Ratio of private:shared pages for a process to be freezer-eligible. */
#else
unsigned int memorystatus_freeze_private_shared_pages_ratio =
    2; /* Ratio of private:shared pages for a process to be freezer-eligible. */
#endif
unsigned int memorystatus_thaw_count =
    0; /* # of thaws in the current freezer interval */
uint64_t memorystatus_thaw_count_since_boot =
    0; /* The number of thaws since boot */
unsigned int memorystatus_refreeze_eligible_count =
...
static void memorystatus_freeze_thread(void *param __unused,
...
static uint32_t memorystatus_freeze_calculate_new_budget(
    ...
    unsigned int burst_multiple, unsigned int interval_duration_min,
    uint32_t rollover);
static void
memorystatus_freeze_start_normal_throttle_interval(uint32_t new_budget,
...
static bool kill_all_frozen_processes(uint64_t max_band, bool suspended_only,
...
struct memorystatus_freezer_candidate_list memorystatus_global_freeze_list = {
    NULL, 0};
struct memorystatus_freezer_candidate_list memorystatus_global_demote_list = {
    NULL, 0};
/*
 * When enabled, freeze candidates are chosen from the
 * memorystatus_global_freeze_list in order (as opposed to using the older LRU
 * approach).
 */
#if XNU_TARGET_OS_WATCH
...
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_freezer_use_ordered_list,
                              &memorystatus_freezer_use_ordered_list, 0, 1, "");
/*
 * When enabled, demotion candidates are chosen from
 * memorystatus_global_demotion_list
 */
int memorystatus_freezer_use_demotion_list = 0;
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_freezer_use_demotion_list,
                              &memorystatus_freezer_use_demotion_list, 0, 1,
                              "");
...
static void memorystatus_freeze_handle_error(
    proc_t p, const freezer_error_code_t freezer_error_code, bool was_refreeze,
    pid_t pid, const coalition_t coalition, const char *log_prefix);
...
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_count,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_frozen_count, 0, "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_count_webcontent,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_frozen_count_webcontent,
            0, "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_count_xpc_service,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_frozen_count_xpc_service,
            0, "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_thaw_count,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_thaw_count, 0, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_thaw_count_since_boot,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_thaw_count_since_boot,
            "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freeze_pageouts,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_freeze_pageouts, "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_interval,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_freeze_current_interval,
            0, "");

/*
 * Force a new interval with the given budget (no rollover).
 */
static void memorystatus_freeze_force_new_interval(uint64_t new_budget) {
  ...
  now_ts.tv_nsec = nsec;
  memorystatus_freeze_start_normal_throttle_interval(
      (uint32_t)MIN(new_budget, UINT32_MAX), now_ts);
  ...
  memorystatus_freeze_budget_pages_remaining =
      normal_throttle_window->max_pageouts;
}
#if DEVELOPMENT || DEBUG
static int sysctl_memorystatus_freeze_budget_pages_remaining
    SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2, oidp)
  ...
  error = sysctl_io_number(req, memorystatus_freeze_budget_pages_remaining,
                           sizeof(uint64_t), &new_budget, &changed);
  if (changed) {
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_freeze_budget_pages_remaining,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            &sysctl_memorystatus_freeze_budget_pages_remaining, "Q", "");
#else  /* DEVELOPMENT || DEBUG */
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freeze_budget_pages_remaining,
...
SYSCTL_QUAD(_kern, OID_AUTO,
            ...
            "");
SYSCTL_QUAD(
    _kern, OID_AUTO, memorystatus_freezer_error_low_private_shared_ratio_count,
    CTLFLAG_RD | CTLFLAG_LOCKED,
...
SYSCTL_QUAD(_kern, OID_AUTO,
            ...
            "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_error_no_swap_space_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_error_no_swap_space_count, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_error_below_min_pages_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
...
SYSCTL_QUAD(_kern, OID_AUTO,
            ...
            "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_error_elevated_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_error_elevated_count, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_error_other_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_error_other_count, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_process_considered_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_process_considered_count, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_below_threshold_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_below_threshold_count, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_skipped_full_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_skipped_full_count, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_skipped_shared_mb_high_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_skipped_shared_mb_high_count, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_shared_pages_skipped,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_shared_pages_skipped, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_bytes_refrozen,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_bytes_refrozen, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_refreeze_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_refreeze_count, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_freeze_pid_mismatches,
            CTLTYPE_QUAD | CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_freezer_stats.mfs_freeze_pid_mismatches, "");
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_freezer_demote_pid_mismatches,
            CTLTYPE_QUAD | CTLFLAG_RD | CTLFLAG_LOCKED,
 ...
 */
static int calculate_thaw_percentage(uint64_t frozen_count,
                                     uint64_t thaw_count) {
...
static int get_thaw_percentage() {
  uint64_t processes_frozen, processes_thawed;
  processes_frozen =
      os_atomic_load(&memorystatus_freezer_stats.mfs_processes_frozen, relaxed);
  processes_thawed =
      os_atomic_load(&memorystatus_freezer_stats.mfs_processes_thawed, relaxed);
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_freezer_thaw_percentage,
            ...
            &sysctl_memorystatus_freezer_thaw_percentage, "I", "");

static int get_thaw_percentage_fg() {
  uint64_t processes_frozen, processes_thawed_fg;
  processes_frozen =
      os_atomic_load(&memorystatus_freezer_stats.mfs_processes_frozen, relaxed);
  processes_thawed_fg = os_atomic_load(
      &memorystatus_freezer_stats.mfs_processes_thawed_fg, relaxed);
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_freezer_thaw_percentage_fg,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            &sysctl_memorystatus_freezer_thaw_percentage_fg, "I", "");

static int get_thaw_percentage_webcontent() {
  uint64_t processes_frozen_webcontent, processes_thawed_webcontent;
  processes_frozen_webcontent = os_atomic_load(
      &memorystatus_freezer_stats.mfs_processes_frozen_webcontent, relaxed);
  processes_thawed_webcontent = os_atomic_load(
      &memorystatus_freezer_stats.mfs_processes_thawed_webcontent, relaxed);
  return calculate_thaw_percentage(processes_frozen_webcontent,
                                   processes_thawed_webcontent);
}

static int sysctl_memorystatus_freezer_thaw_percentage_webcontent
    SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_freezer_thaw_percentage_webcontent,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            &sysctl_memorystatus_freezer_thaw_percentage_webcontent, "I", "");

static int get_thaw_percentage_bg() {
  ...
  processes_thawed_fg = os_atomic_load(
      &memorystatus_freezer_stats.mfs_processes_thawed_fg, relaxed);
  return calculate_thaw_percentage(processes_frozen,
                                   processes_thawed - processes_thawed_fg);
}
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_freezer_thaw_percentage_bg,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            &sysctl_memorystatus_freezer_thaw_percentage_bg, "I", "");

static int get_thaw_percentage_fg_non_xpc_service() {
  uint64_t processes_frozen, processes_frozen_xpc_service, processes_thawed_fg,
      processes_thawed_fg_xpc_service;
  processes_frozen =
      os_atomic_load(&memorystatus_freezer_stats.mfs_processes_frozen, relaxed);
  processes_frozen_xpc_service = os_atomic_load(
      &memorystatus_freezer_stats.mfs_processes_frozen_xpc_service, relaxed);
  processes_thawed_fg = os_atomic_load(
      &memorystatus_freezer_stats.mfs_processes_thawed_fg, relaxed);
  processes_thawed_fg_xpc_service = os_atomic_load(
      &memorystatus_freezer_stats.mfs_processes_thawed_fg_xpc_service, relaxed);
  /*
   * Since these are all relaxed loads, it's possible (although unlikely) to
   * read a value for frozen/thawed xpc services that's > the value for
   * processes frozen / thawed. Clamp just in case.
   */
  processes_frozen_xpc_service =
      MIN(processes_frozen_xpc_service, processes_frozen);
  processes_thawed_fg_xpc_service =
      MIN(processes_thawed_fg_xpc_service, processes_thawed_fg);
  return calculate_thaw_percentage(
      processes_frozen - processes_frozen_xpc_service,
      processes_thawed_fg - processes_thawed_fg_xpc_service);
}

static int sysctl_memorystatus_freezer_thaw_percentage_fg_non_xpc_service
    SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
...
SYSCTL_PROC(_kern, OID_AUTO,
            memorystatus_freezer_thaw_percentage_fg_non_xpc_service,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            &sysctl_memorystatus_freezer_thaw_percentage_fg_non_xpc_service,
            "I", "");

#define FREEZER_ERROR_STRING_LENGTH 128

EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_freeze_pages_min,
                              &memorystatus_freeze_pages_min, 0, UINT32_MAX,
                              "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_freeze_pages_max,
                              &memorystatus_freeze_pages_max, 0, UINT32_MAX,
                              "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_freeze_processes_max,
                              &memorystatus_frozen_processes_max, 0, UINT32_MAX,
                              "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_freeze_jetsam_band,
                              &memorystatus_freeze_jetsam_band,
                              JETSAM_PRIORITY_BACKGROUND,
                              JETSAM_PRIORITY_FOREGROUND, "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern,
                              memorystatus_freeze_private_shared_pages_ratio,
                              &memorystatus_freeze_private_shared_pages_ratio,
                              0, UINT32_MAX, "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_freeze_min_processes,
                              &memorystatus_freeze_suspended_threshold, 0,
                              UINT32_MAX, "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_freeze_max_candidate_band,
                              &memorystatus_freeze_max_candidate_band,
                              JETSAM_PRIORITY_IDLE, JETSAM_PRIORITY_FOREGROUND,
                              "");
static int sysctl_memorystatus_freeze_budget_multiplier SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2, oidp, req)
  ...
  error = sysctl_io_number(req, memorystatus_freeze_budget_multiplier,
                           sizeof(val), &val, &changed);
  if (error) {
    return error;
  }
  if (changed) {
    ...
    new_budget =
        memorystatus_freeze_calculate_new_budget(0, 1, NORMAL_WINDOW_MINS, 0);
    clock_get_system_nanotime(&sec, &nsec);
    now_ts.tv_sec = (unsigned int)(MIN(sec, UINT32_MAX));
    now_ts.tv_nsec = nsec;
...
EXPERIMENT_FACTOR_LEGACY_PROC(_kern, memorystatus_freeze_budget_multiplier,
                              CTLTYPE_QUAD | CTLFLAG_RW, 0, 0,
                              &sysctl_memorystatus_freeze_budget_multiplier,
                              "Q", "");
/*
 * max. # of frozen process demotions we will allow in our daily cycle.
 */
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_max_freeze_demotions_daily,
                              &memorystatus_max_frozen_demotions_daily, 0,
                              UINT32_MAX, "");

/*
 * min # of thaws needed by a process to protect it from getting demoted into
 * the IDLE band.
 */
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_thaw_count_demotion_threshold,
                              &memorystatus_thaw_count_demotion_threshold, 0,
                              UINT32_MAX, "");

/*
 * min # of global thaws needed for us to consider refreezing these processes.
 */
EXPERIMENT_FACTOR_LEGACY_UINT(_kern, memorystatus_min_thaw_refreeze_threshold,
                              &memorystatus_min_thaw_refreeze_threshold, 0,
                              UINT32_MAX, "");

#if DEVELOPMENT || DEBUG

SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_daily_mb_max,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_freeze_daily_mb_max, 0,
            "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_degraded_mode,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_freeze_degradation, 0,
            "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_threshold,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_freeze_threshold, 0, "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_refreeze_eligible_count,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_refreeze_eligible_count,
            0, "");

/*
 * Max. shared-anonymous memory in MB that can be held by frozen processes in
 * the high jetsam band. "0" means no limit. Default is 10% of system-wide task
 * limit.
 */

SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_shared_mb_max,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_frozen_shared_mb_max, 0,
            "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_shared_mb,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_frozen_shared_mb, 0, "");

SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_shared_mb_per_process_max,
            CTLFLAG_RW | CTLFLAG_LOCKED,
            &memorystatus_freeze_shared_mb_per_process_max, 0, "");

boolean_t memorystatus_freeze_throttle_enabled = TRUE;
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_throttle_enabled,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_freeze_throttle_enabled,
            0, "");

/*
 * When set to true, this keeps frozen processes in the compressor pool in
 * memory, instead of swapping them out to disk. Exposed via the sysctl
 * kern.memorystatus_freeze_to_memory.
 */
boolean_t memorystatus_freeze_to_memory = FALSE;
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_to_memory,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_freeze_to_memory, 0, "");
 ...
 */
static int sysctl_memorystatus_freeze SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  int error, pid = 0;
  proc_t p;
...
again:
  ...
  if (p != NULL) {
    ...
    if (VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
       ...
       *
       * Set the sysctl kern.memorystatus_freeze_to_memory to true to keep
       * compressed data from being swapped out to disk. Note that this disables
       * freezer swap support globally, not just for the process being frozen.
       *
       *
       * We don't care about the global freezer budget or the process's
       * (min/max) budget here. The freeze sysctl is meant to force-freeze a
       * process.
       *
       * We also don't update any global or process stats on this path, so that
       * the jetsam/ freeze logic remains unaffected. The tasks we're performing
       * here are: freeze the process, set the P_MEMSTAT_FROZEN bit, and elevate
       * the process to a higher band (if the freezer is active).
       */
    ...
    /*
     * The jetsam path also verifies that the process is a suspended App. We
     * don't care about that here. We simply ensure that jetsam is not already
     * working on the process and that the process has not explicitly disabled
     * freezing.
     */
    if (state &
        (P_MEMSTAT_TERMINATED | P_MEMSTAT_LOCKED | P_MEMSTAT_FREEZE_DISABLED)) {
      memorystatus_log_error(
          "sysctl_freeze: p_memstat_state check failed, process is%s%s%s\n",
    ...
    KDBG(MEMSTAT_CODE(BSD_MEMSTAT_FREEZE) | DBG_FUNC_START,
         memorystatus_available_pages, pid, max_pages);
    error =
        task_freeze(proc_task(p), &purgeable, &wired, &clean, &dirty, max_pages,
                    &shared, &freezer_error_code, FALSE /* eval only */);
    if (!error ||
        freezer_error_code == FREEZER_ERROR_LOW_PRIVATE_SHARED_RATIO) {
      memorystatus_freezer_stats.mfs_shared_pages_skipped += shared;
    }
    KDBG(MEMSTAT_CODE(BSD_MEMSTAT_FREEZE) | DBG_FUNC_END, purgeable, wired,
         clean, dirty);

    if (error) {
      memorystatus_freeze_handle_error(p, freezer_error_code,
                                       state & P_MEMSTAT_FROZEN, pid, coal,
                                       "sysctl_freeze");
      if (error == KERN_NO_SPACE) {
        /* Make it easy to distinguish between failures due to low compressor/
         * swap space and other failures. */
        error = ENOSPC;
      } else {
        error = EIO;
      }
      ...
      if (!_memstat_proc_is_frozen(p)) {
        ...
        os_atomic_inc(&memorystatus_freezer_stats.mfs_processes_frozen,
                      relaxed);
        if (strcmp(p->p_name, "com.apple.WebKit.WebContent") == 0) {
          memorystatus_frozen_count_webcontent++;
          os_atomic_inc(
              &(memorystatus_freezer_stats.mfs_processes_frozen_webcontent),
              relaxed);
        }
      ...
      if (coal != NULL) {
        ...
        os_atomic_inc(
            &(memorystatus_freezer_stats.mfs_processes_frozen_xpc_service),
            relaxed);
      }
      ...
      if (VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
        ...
        error = memorystatus_update_inactive_jetsam_priority_band(
            pid, MEMORYSTATUS_CMD_ELEVATED_INACTIVEJETSAMPRIORITY_ENABLE,
            memorystatus_freeze_jetsam_band, TRUE);

        if (error) {
          memorystatus_log_error("sysctl_freeze: Elevating frozen process to "
                                 "higher jetsam band failed with %d\n",
                                 error);
        }
      }
    }
    ...
    if ((error == 0) && (coal == NULL)) {
      ...
      if (coalition_is_leader(curr_task, coal)) {
        ntasks = coalition_get_pid_list(coal, COALITION_ROLEMASK_XPC,
                                        COALITION_SORT_DEFAULT, pid_list,
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_freeze,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED | CTLFLAG_MASKED, 0, 0,
            &sysctl_memorystatus_freeze, "I", "");

/*
 * Manual trigger of agressive frozen demotion for dev / debug kernels only.
 */
static int sysctl_memorystatus_demote_frozen_process SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  int error, val;
  /*
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_demote_frozen_processes,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED | CTLFLAG_MASKED, 0, 0,
            &sysctl_memorystatus_demote_frozen_process, "I", "");

static int sysctl_memorystatus_available_pages_thaw SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_thaw,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED | CTLFLAG_MASKED, 0, 0,
            &sysctl_memorystatus_available_pages_thaw, "I", "");
...
#define MAX_FREEZABLE_PROCESSES                                                \
  200 /* Total # of processes in band 0 that we evaluate for freezability */
 ...
 *  - mark its 'freeze_leader_eligible' field with
 * FREEZE_PROC_LEADER_FREEZABLE_UNKNOWN
 *  - continue its freezability evaluation assuming its leader will be freezable
 * too
 *
 ...
 * we update its 'freeze_leader_eligible' to
...
static int memorystatus_freezer_get_status(user_addr_t buffer,
                                           size_t buffer_size,
                                           int32_t *retval) {
  uint32_t proc_count = 0, freeze_eligible_proc_considered = 0, band = 0,
           xpc_index = 0, leader_index = 0;
  ...
  list_size = sizeof(global_freezable_status_t) +
              (sizeof(proc_freezable_status_t) * MAX_FREEZABLE_PROCESSES);
  ...
  list_head =
      (global_freezable_status_t *)kalloc_data(list_size, Z_WAITOK | Z_ZERO);
  if (list_head == NULL) {
    return ENOMEM;
  }
  ...
  list_head->freeze_pages_threshold_crossed =
      (memorystatus_available_pages < memorystatus_freeze_threshold);
  if (memorystatus_freezer_use_ordered_list) {
    list_head->freeze_eligible_procs_available =
        memorystatus_frozen_count < memorystatus_global_freeze_list.mfcl_length;
  } else {
    list_head->freeze_eligible_procs_available =
        ((memorystatus_suspended_count - memorystatus_frozen_count) >
         memorystatus_freeze_suspended_threshold);
  }
  list_head->freeze_scheduled_in_future =
      (curr_time < memorystatus_freezer_thread_next_run_ts);

  list_entry_start =
      (proc_freezable_status_t *)((uintptr_t)list_head +
                                  sizeof(global_freezable_status_t));
  list_entry = list_entry_start;

  bucket = &memstat_bucket[JETSAM_PRIORITY_IDLE];

  entry_count = (memorystatus_global_probabilities_size /
                 sizeof(memorystatus_internal_probabilities_t));

  if (memorystatus_freezer_use_ordered_list) {
    while (iterator.global_freeze_list_index <
           memorystatus_global_freeze_list.mfcl_length) {
      p = memorystatus_freezer_candidate_list_get_proc(
          &memorystatus_global_freeze_list,
          (iterator.global_freeze_list_index)++, NULL);
      if (p != PROC_NULL) {
        break;
      }
    }
  ...
  while ((proc_count <= MAX_FREEZABLE_PROCESSES) && (p) &&
         (list_size < buffer_size)) {
    if (isSysProc(p)) {
      /*
       * Daemon:- We will consider freezing it iff:
       * - it belongs to a coalition and the leader is freeze-eligible (delayed
       * evaluation)
      ...
      task_role_in_coalition =
          task_coalition_role_for_type(curr_task, COALITION_TYPE_JETSAM);
  ...
  continue_eval:
    ...
    if ((state & (P_MEMSTAT_TERMINATED | P_MEMSTAT_LOCKED |
                  P_MEMSTAT_FREEZE_DISABLED | P_MEMSTAT_FREEZE_IGNORE)) ||
    ...
    if (xpc_skip_size_probability_check == TRUE) {
      ...
      list_entry->freeze_leader_eligible =
          FREEZE_PROC_LEADER_FREEZABLE_SUCCESS; /* Apps are freeze eligible and
                                                   their own leaders. */
      list_entry->p_leader_pid = 0; /* Setting this to 0 signifies this isn't a
                                       coalition driven freeze. */
      ...
      if (entry_count) {
        ...
        for (j = 0; j < entry_count; j++) {
          if (strncmp(memorystatus_global_probabilities_table[j].proc_name,
                      p->p_name, MAXCOMLEN) == 0) {
            probability_of_use =
                memorystatus_global_probabilities_table[j].use_probability;
            break;
          }
        }
  ...
  for (xpc_index = 0; xpc_index < freeze_eligible_proc_considered;
       xpc_index++) {
    if (list_entry[xpc_index].freeze_leader_eligible ==
        FREEZE_PROC_LEADER_FREEZABLE_UNKNOWN) {
      ...
      if (leader_proc) {
        if (_memstat_proc_is_frozen(leader_proc)) {
           ...
           */
          list_entry[xpc_index].freeze_leader_eligible =
              FREEZE_PROC_LEADER_FREEZABLE_SUCCESS;
      ...
      for (leader_index = 0; leader_index < freeze_eligible_proc_considered;
           leader_index++) {
        if (list_entry[leader_index].p_pid == leader_pid) {
          if (list_entry[leader_index].freeze_attempted &&
              list_entry[leader_index].p_freeze_error_code == 0) {
            list_entry[xpc_index].freeze_leader_eligible =
                FREEZE_PROC_LEADER_FREEZABLE_SUCCESS;
          } else {
            list_entry[xpc_index].freeze_leader_eligible =
  ...
  memorystatus_log_debug(
      "memorystatus_freezer_get_status: returning %d (%lu - size)\n", error,
 ...
 */
static int memorystatus_freezer_get_procs(user_addr_t buffer,
  ...
  frozen_procs = (global_frozen_procs_t *)kalloc_data(
      sizeof(global_frozen_procs_t), Z_WAITOK | Z_ZERO);
  ...
  p = memorystatus_get_first_proc_locked(&band, FALSE);
  while (p &&
         frozen_procs->gfp_num_frozen < FREEZER_CONTROL_GET_PROCS_MAX_COUNT) {
    if (_memstat_proc_is_frozen(p)) {
      frozen_procs->gfp_procs[frozen_procs->gfp_num_frozen].fp_pid =
          proc_getpid(p);
...
/*
 * If dasd is running an experiment that impacts their freezer candidate
 * selection, we record that in our telemetry.
 */
static memorystatus_freezer_trial_identifiers_v1 dasd_trial_identifiers;

static int memorystatus_freezer_set_dasd_trial_identifiers(user_addr_t buffer,
                                                           size_t buffer_size,
                                                           int32_t *retval) {
 ...
 */
static int memorystatus_freezer_reset_state(int32_t *retval) {
  ...
  os_reason_t jetsam_reason =
      os_reason_create(OS_REASON_JETSAM, JETSAM_REASON_GENERIC);
  if (jetsam_reason == OS_REASON_NULL) {
    memorystatus_log_error("memorystatus_freezer_reset_state -- sync: failed "
  ...
  new_budget = memorystatus_freeze_calculate_new_budget(
      0, normal_throttle_window->burst_multiple, normal_throttle_window->mins,
      0);
  memorystatus_freeze_force_new_interval(new_budget);
...
int memorystatus_freezer_control(int32_t flags, user_addr_t buffer,
                                 size_t buffer_size, int32_t *retval) {
  int err = ENOTSUP;
  ...
  if (flags == FREEZER_CONTROL_GET_PROCS) {
    ...
    err = memorystatus_freezer_set_dasd_trial_identifiers(buffer, buffer_size,
                                                          retval);
...
static bool kill_all_frozen_processes(uint64_t max_band, bool suspended_only,
  ...
  while (next_p) {
    ...
    if (state & (P_MEMSTAT_TERMINATED | P_MEMSTAT_LOCKED)) {
      memorystatus_log("memorystatus: Skipping kill of frozen process %s (%d) "
                       "because it's already exiting.\n",
     ...
     * and we skip anything we've already tried to kill this is guaranteed to
     * terminate.
     */
    band = 0;
    skips = 0;
  ...
  if (!suspended_only && max_band == JETSAM_PRIORITY_MAX) {
    ...
    if (memorystatus_frozen_count - skips > 0) {
      ...
      panic("memorystatus_disable_freeze: Failed to kill all frozen processes, "
            "memorystatus_frozen_count = %d",
            memorystatus_frozen_count);
    }
  }
#endif /* MACH_ASSERT */
...
void memorystatus_disable_freeze(void) {
  ...
  memorystatus_log(
      "memorystatus: Disabling freezer. Will kill all frozen processes\n");

  /*
   * We hold the freezer_mutex (preventing anything from being frozen in
   * parallel) and all frozen processes will be killed by the time we release
   * it. Setting memorystatus_freeze_enabled to false, ensures that no new
   * processes will be frozen once we release the mutex.
   *
   */
  memorystatus_freeze_enabled = false;

  /*
   * Move dirty pages out from the throttle to the active queue since we're not
   * freezing anymore.
   */
  vm_page_reactivate_all_throttled();
  os_reason_t jetsam_reason = os_reason_create(
      OS_REASON_JETSAM, JETSAM_REASON_MEMORY_DISK_SPACE_SHORTAGE);
  if (jetsam_reason == OS_REASON_NULL) {
    memorystatus_log_error("memorystatus_disable_freeze -- sync: failed to "
                           "allocate jetsam reason\n");
  }

  killed = kill_all_frozen_processes(JETSAM_PRIORITY_MAX, false, jetsam_reason,
                                     &memory_reclaimed);

  if (killed) {
    memorystatus_log_info("memorystatus: Killed all frozen processes.\n");
    vm_swap_consider_defragmenting(VM_SWAP_FLAGS_FORCE_DEFRAG |
...
static int sysctl_freeze_enabled SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  ...
  if (!VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
    memorystatus_log_error(
        "memorystatus: Failed attempt to set vm.freeze_enabled sysctl\n");
    return EINVAL;
  }
...
EXPERIMENT_FACTOR_LEGACY_PROC(_vm, freeze_enabled,
                              CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_ANYBODY, NULL,
                              0, sysctl_freeze_enabled, "I", "");

static void schedule_interval_reset(thread_call_t reset_thread_call,
                                    throttle_interval_t *interval) {
  uint64_t interval_expiration_ns =
      interval->ts.tv_sec * NSEC_PER_SEC + interval->ts.tv_nsec;
  uint64_t interval_expiration_absolutetime;
  nanoseconds_to_absolutetime(interval_expiration_ns,
                              &interval_expiration_absolutetime);
  memorystatus_log_info(
      "memorystatus: scheduling new freezer interval at %llu absolute time\n",
      interval_expiration_absolutetime);

  thread_call_enter_delayed(reset_thread_call,
                            interval_expiration_absolutetime);
}
...
CA_EVENT(freezer_interval, CA_INT, budget_remaining, CA_INT,
         error_below_min_pages, CA_INT, error_excess_shared_memory, CA_INT,
         error_low_private_shared_ratio, CA_INT, error_no_compressor_space,
         CA_INT, error_no_swap_space, CA_INT, error_low_probability_of_use,
         CA_INT, error_elevated, CA_INT, error_other, CA_INT, frozen_count,
         CA_INT, pageouts, CA_INT, refreeze_average, CA_INT, skipped_full,
         CA_INT, skipped_shared_mb_high, CA_INT, swapusage, CA_INT, thaw_count,
         CA_INT, thaw_percentage, CA_INT, thaws_per_gb, CA_INT,
         trial_deployment_id, CA_INT, dasd_trial_deployment_id, CA_INT,
         budget_exhaustion_duration_remaining, CA_INT,
         thaw_percentage_webcontent, CA_INT, thaw_percentage_fg, CA_INT,
         thaw_percentage_bg, CA_INT, thaw_percentage_fg_non_xpc_service, CA_INT,
         fg_resume_count, CA_INT, unique_freeze_count, CA_INT,
         unique_thaw_count, CA_STATIC_STRING(CA_UUID_LEN), trial_treatment_id,
 ...
 * via core analytics.
 */
static void memorystatus_freeze_record_interval_analytics(void) {
  ca_event_t event = CA_EVENT_ALLOCATE(freezer_interval);
  CA_EVENT_TYPE(freezer_interval) *e = event->data;
  e->budget_remaining =
      memorystatus_freeze_budget_pages_remaining * PAGE_SIZE / (1UL << 20);
  uint64_t process_considered_count, refrozen_count, below_threshold_count;
  memory_object_size_t swap_size;
  process_considered_count =
      memorystatus_freezer_stats.mfs_process_considered_count;
  if (process_considered_count != 0) {
    e->error_below_min_pages =
        memorystatus_freezer_stats.mfs_error_below_min_pages_count * 100 /
        process_considered_count;
    e->error_excess_shared_memory =
        memorystatus_freezer_stats.mfs_error_excess_shared_memory_count * 100 /
        process_considered_count;
    e->error_low_private_shared_ratio =
        memorystatus_freezer_stats.mfs_error_low_private_shared_ratio_count *
        100 / process_considered_count;
    e->error_no_compressor_space =
        memorystatus_freezer_stats.mfs_error_no_compressor_space_count * 100 /
        process_considered_count;
    e->error_no_swap_space =
        memorystatus_freezer_stats.mfs_error_no_swap_space_count * 100 /
        process_considered_count;
    e->error_low_probability_of_use =
        memorystatus_freezer_stats.mfs_error_low_probability_of_use_count *
        100 / process_considered_count;
    e->error_elevated = memorystatus_freezer_stats.mfs_error_elevated_count *
                        100 / process_considered_count;
    e->error_other = memorystatus_freezer_stats.mfs_error_other_count * 100 /
                     process_considered_count;
  }
  ...
  if (refrozen_count != 0) {
    e->refreeze_average =
        (memorystatus_freezer_stats.mfs_bytes_refrozen / (1UL << 20)) /
        refrozen_count;
  }
  below_threshold_count = memorystatus_freezer_stats.mfs_below_threshold_count;
  if (below_threshold_count != 0) {
    e->skipped_full = memorystatus_freezer_stats.mfs_skipped_full_count * 100 /
                      below_threshold_count;
    e->skipped_shared_mb_high =
        memorystatus_freezer_stats.mfs_skipped_shared_mb_high_count * 100 /
        below_threshold_count;
  }
  ...
  e->thaw_percentage_fg_non_xpc_service =
      get_thaw_percentage_fg_non_xpc_service();

  if (e->pageouts / (1UL << 10) != 0) {
    e->thaws_per_gb = memorystatus_thaw_count / (e->pageouts / (1UL << 10));
  }
  e->budget_exhaustion_duration_remaining =
      memorystatus_freezer_stats.mfs_budget_exhaustion_duration_remaining;
  e->fg_resume_count = os_atomic_load(
      &memorystatus_freezer_stats.mfs_processes_thawed_fg, relaxed);
  e->unique_freeze_count =
      os_atomic_load(&memorystatus_freezer_stats.mfs_processes_frozen, relaxed);
  e->unique_thaw_count =
      os_atomic_load(&memorystatus_freezer_stats.mfs_processes_thawed, relaxed);
  ...
  strlcpy(e->dasd_trial_treatment_id, dasd_trial_identifiers.treatment_id,
          CA_UUID_LEN);
  strlcpy(e->dasd_trial_experiment_id, dasd_trial_identifiers.experiment_id,
          CA_UUID_LEN);
...
static void memorystatus_freeze_reset_interval(void *arg0, void *arg1) {
#pragma unused(arg0, arg1)
  ...
  /* Get unused budget. Clamp to 0. We'll adjust for overused budget in the next
   * interval. */
  budget_rollover = interval->pageouts > interval->max_pageouts
                        ? 0
                        : interval->max_pageouts - interval->pageouts;

  memorystatus_freeze_start_normal_throttle_interval(
      memorystatus_freeze_calculate_new_budget(expiration_period_ts.tv_sec,
                                               interval->burst_multiple,
  ...
  if (!memorystatus_freezer_use_demotion_list) {
    memorystatus_demote_frozen_processes(
        false); /* normal mode...don't force a demotion */
  }
  lck_mtx_unlock(&freezer_mutex);
}
...
bool memorystatus_freeze_process_is_recommended(const proc_t p) {
  ...
  if (entry_count == 0) {
    /*
     * If dasd hasn't supplied a table yet, we default to every app being
  ...
  for (i = 0; i < entry_count; i++) {
     ...
     */
    if (strncmp(memorystatus_global_probabilities_table[i].proc_name, p->p_name,
                MAXCOMLEN) == 0) {
      probability_of_use =
...
__private_extern__ void memorystatus_freeze_init(void) {
  ...
  if (VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
    ...
    if (PE_parse_boot_argn(
            "memorystatus_freezer_use_ordered_list",
            &memorystatus_freezer_use_ordered_list_bootarg,
            sizeof(memorystatus_freezer_use_ordered_list_bootarg))) {
      memorystatus_freezer_use_ordered_list =
          (memorystatus_freezer_use_ordered_list_bootarg != 0);
    }

    int32_t memorystatus_freeze_max_candidate_band_bootarg = 0;
    if (PE_parse_boot_argn(
            "memorystatus_freeze_max_candidate_band",
            &memorystatus_freeze_max_candidate_band_bootarg,
            sizeof(memorystatus_freeze_max_candidate_band_bootarg))) {
      if (memorystatus_freeze_max_candidate_band_bootarg >= 0 &&
          memorystatus_freeze_max_candidate_band_bootarg <= 1000) {
        memorystatus_freeze_max_candidate_band =
            memorystatus_freeze_max_candidate_band_bootarg;
      }
    }
     ...
     */
    memorystatus_freeze_budget_pages_remaining =
        (memorystatus_freeze_daily_mb_max * 1024 * 1024) / PAGE_SIZE;

    result = kernel_thread_start(memorystatus_freeze_thread, NULL, &thread);
    if (result == KERN_SUCCESS) {
      proc_set_thread_policy(thread, TASK_POLICY_INTERNAL, TASK_POLICY_IO,
                             THROTTLE_LEVEL_COMPRESSOR_TIER2);
      proc_set_thread_policy(thread, TASK_POLICY_INTERNAL,
                             TASK_POLICY_PASSIVE_IO, TASK_POLICY_ENABLE);
    ...
    freeze_interval_reset_thread_call = thread_call_allocate_with_options(
        memorystatus_freeze_reset_interval, NULL, THREAD_CALL_PRIORITY_KERNEL,
        THREAD_CALL_OPTIONS_ONCE);
    /* Start a new interval */

    lck_mtx_lock(&freezer_mutex);
    uint32_t budget;
    budget = memorystatus_freeze_calculate_new_budget(
        0, normal_throttle_window->burst_multiple, normal_throttle_window->mins,
        0);
...
void memorystatus_freeze_configure_for_swap() {
  if (!VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
    return;
  }
  ...
  memorystatus_max_frozen_demotions_daily =
      MAX_FROZEN_PROCESS_DEMOTIONS_SWAP_ENABLED_DEFAULT;
   ...
   * Force a new interval. memorystatus_freeze_calculate_new_budget should give
   * us an unlimited budget.
   */
  lck_mtx_lock(&freezer_mutex);
  uint32_t budget;
  budget = memorystatus_freeze_calculate_new_budget(
...
void memorystatus_freeze_disable_swap() {
  ...
  /*
   * Calculate a new budget now that we're constrained by our daily write budget
   * again.
   */
  lck_mtx_lock(&freezer_mutex);
  uint32_t budget;
...
/*
 * Called with both the freezer_mutex and proc_list_lock held & both will be
 * held on return.
 */
static int
memorystatus_freeze_process(proc_t p, coalition_t *coal, /* IN / OUT */
  ...
  if (is_refreeze) {
    ...
    if (memorystatus_frozen_count >= memorystatus_frozen_processes_max) {
      memorystatus_freeze_handle_error(p, FREEZER_ERROR_NO_SLOTS, is_refreeze,
                                       aPid, (coal ? *coal : NULL),
  ...
  if (VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
    ...
    max_pages = MIN(memorystatus_freeze_pages_max,
                    memorystatus_freeze_budget_pages_remaining);
  } else {
    /*
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_FREEZE) | DBG_FUNC_START,
       memorystatus_available_pages, aPid, max_pages);
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_FREEZE) | DBG_FUNC_END, purgeable, wired, clean,
       dirty);

  memorystatus_log_debug(
      "memorystatus_freeze_top_process: task_freeze %s for pid %d [%s] - "
      "memorystatus_pages: %d, purgeable: %d, wired: %d, clean: %d, dirty: %d, "
      "max_pages %llu, shared %d",
      (kr == KERN_SUCCESS) ? "SUCCEEDED" : "FAILED", aPid,
      (*p->p_name ? p->p_name : "(unknown)"), memorystatus_available_pages,
      purgeable, wired, clean, dirty, max_pages, shared);
  ...
  if (KERN_SUCCESS == kr) {
    ...
    /*
     * Still keeping the P_MEMSTAT_LOCKED bit till we are actually done
     * elevating this frozen process to its higher jetsam band.
     */
    proc_list_unlock();
    ...
    if (VM_CONFIG_FREEZER_SWAP_IS_ACTIVE) {
      ...
      int ret;
      ret = memorystatus_update_inactive_jetsam_priority_band(
          proc_getpid(p),
      ...
      for (unsigned int i = 0;
           i < sizeof(throttle_intervals) / sizeof(struct throttle_interval_t);
           i++) {
    ...
    memorystatus_freeze_update_throttle(
        &memorystatus_freeze_budget_pages_remaining);
    memorystatus_log(
        "memorystatus: %sfreezing (%s) pid %d [%s] done, "
        "memorystatus_freeze_budget_pages_remaining %llu %sfroze %u pages\n",
        is_refreeze ? "re" : "",
        ((!coal || !*coal) ? "general" : "coalition-driven"), aPid,
        ((p && *p->p_name) ? p->p_name : "unknown"),
        memorystatus_freeze_budget_pages_remaining, is_refreeze ? "Re" : "",
        dirty);
    ...
    /* Check if we just froze a coalition leader. If so, return the list of XPC
     * services to freeze next. */
    if (coal != NULL && *coal == NULL) {
      curr_task = proc_task(p);
      *coal = task_get_coalition(curr_task, COALITION_TYPE_JETSAM);
      if (coalition_is_leader(curr_task, *coal)) {
        *coalition_list_length = coalition_get_pid_list(
            *coal, COALITION_ROLEMASK_XPC, COALITION_SORT_DEFAULT,
            coalition_list, MAX_XPC_SERVICE_PIDS);
    ...
    memorystatus_freeze_handle_error(
        p, freezer_error_code, p->p_memstat_state & P_MEMSTAT_FROZEN, aPid,
        (coal != NULL) ? *coal : NULL, "memorystatus_freeze_process");
...
/*
 * Synchronously freeze the passed proc. Called with a reference to the proc
 * held.
 *
 * Doesn't deal with:
 ...
 *   we have moved away from that approach to having the kernel choose the
 * optimal candidates to be frozen.
 *
 ...
 */
int memorystatus_freeze_process_sync(proc_t p) {
  int ret = EINVAL;
  ...
  if (p == NULL) {
    memorystatus_log_error(
        "memorystatus_freeze_process_sync: Invalid process\n");
    goto exit;
  }

  if (memorystatus_freeze_enabled == false) {
    memorystatus_log_error(
        "memorystatus_freeze_process_sync: Freezing is DISABLED\n");
    goto exit;
  }

  if (!memorystatus_can_freeze(&memorystatus_freeze_swap_low)) {
    memorystatus_log_info("memorystatus_freeze_process_sync: Low compressor "
                          "and/or low swap space...skipping freeze\n");
    goto exit;
  }
  ...
  if (!memorystatus_freeze_budget_pages_remaining) {
    memorystatus_log_info(
        "memorystatus_freeze_process_sync: exit with NO available budget\n");
    goto exit;
  }
...
static size_t memorystatus_freeze_pid_list(pid_t *pid_list,
 ...
 */
static size_t memorystatus_freeze_top_process(void) {
  int freeze_ret;
  size_t num_frozen = 0;
  coalition_t coal = COALITION_NULL;
  pid_t pid_list[MAX_XPC_SERVICE_PIDS];
  ...
  bzero(&iterator, sizeof(struct memorystatus_freeze_list_iterator));
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_FREEZE_SCAN) | DBG_FUNC_START,
       memorystatus_available_pages);
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_FREEZE_SCAN) | DBG_FUNC_END,
       memorystatus_available_pages);
...
static int sysctl_memorystatus_freeze_top_process SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  int error, val, ret = 0;
  size_t num_frozen;
  /*
...
SYSCTL_PROC(_vm, OID_AUTO, memorystatus_freeze_top_process,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_MASKED, 0, 0,
            &sysctl_memorystatus_freeze_top_process, "I", "");
#endif /* DEVELOPMENT || DEBUG */
...
static inline boolean_t memorystatus_can_freeze_processes(void) {
  ...
  if (memorystatus_suspended_count) {
    ...
    if ((memorystatus_suspended_count - memorystatus_frozen_count) >
        memorystatus_freeze_suspended_threshold) {
      ret = TRUE;
    } else {
      ret = FALSE;
...
memorystatus_can_freeze(boolean_t *memorystatus_freeze_swap_low) {
  ...
  /* Only freeze if we're sufficiently low on memory; this holds off freeze
   * right after boot,  and is generally is a no-op once we've reached steady
   * state. */
 ...
 */
static void memorystatus_demote_frozen_process(proc_t p,
                                               bool urgent_mode __unused) {
  LCK_MTX_ASSERT(&proc_list_mlock, LCK_MTX_ASSERT_OWNED);

  /* We demote to IDLE unless someone has asserted a higher priority on this
   * process. */
...
memorystatus_demote_frozen_processes_using_thaw_count(bool urgent_mode) {
  ...
  while (next_p) {
    ...
    demoted_proc_count++;
    if ((urgent_mode) ||
        (demoted_proc_count == memorystatus_max_frozen_demotions_daily)) {
      break;
    }
  }
...
memorystatus_demote_frozen_processes_using_demote_list(bool urgent_mode) {
  ...
  for (size_t i = 0; i < memorystatus_global_demote_list.mfcl_length; i++) {
    proc_t p = memorystatus_freezer_candidate_list_get_proc(
        &memorystatus_global_demote_list, i,
        &memorystatus_freezer_stats.mfs_demote_pid_mismatches);
...
static void memorystatus_demote_frozen_processes(bool urgent_mode) {
  ...
  if (memorystatus_freezer_use_demotion_list) {
    demoted_proc_count +=
        memorystatus_demote_frozen_processes_using_demote_list(urgent_mode);
    ...
    demoted_proc_count +=
        memorystatus_demote_frozen_processes_using_thaw_count(urgent_mode);
  }
}

/*
 * Calculate a new freezer budget.
 * @param time_since_last_interval_expired_sec How long has it been (in seconds)
 * since the previous interval expired.
...
static uint32_t memorystatus_freeze_calculate_new_budget(
    ...
    unsigned int burst_multiple, unsigned int interval_duration_min,
    uint32_t rollover) {
  uint64_t freeze_daily_budget = 0, freeze_daily_budget_mb = 0,
           daily_budget_pageouts = 0, budget_missed = 0,
           freeze_daily_pageouts_max = 0, new_budget = 0;
  ...
  freeze_daily_pageouts_max =
      ((kFixedPointFactor * memorystatus_freeze_budget_multiplier / 100) *
       freeze_daily_pageouts_max) /
      kFixedPointFactor;

  daily_budget_pageouts =
      (burst_multiple *
       (((uint64_t)interval_duration_min * freeze_daily_pageouts_max) /
        (kNumSecondsInDay / 60)));

  /*
   * Add additional budget for time since the interval expired.
   * For example, if the interval expired n days ago, we should get an
   * additional n days of budget since we didn't use any budget during those n
   * days.
   */
  days_missed = time_since_last_interval_expired_sec * kFixedPointFactor /
                kNumSecondsInDay;
...
/*
 * Mark all non frozen, freezer-eligible processes as skipped for the
 * given reason. Used when we hit some system freeze limit and know that we
 * won't be considering remaining processes. If you're using this for a new
 * reason, make sure to add it to memorystatus_freeze_init_proc so that it gets
 * set for new processes. NB: These processes will retain this skip reason until
 * they are reconsidered by memorystatus_is_process_eligible_for_freeze.
 */
static void memorystatus_freeze_mark_eligible_processes_with_skip_reason(
    memorystatus_freeze_skip_reason_t reason, bool locked) {
  LCK_MTX_ASSERT(&freezer_mutex, LCK_MTX_ASSERT_OWNED);
  LCK_MTX_ASSERT(&proc_list_mlock,
                 locked ? LCK_MTX_ASSERT_OWNED : LCK_MTX_ASSERT_NOTOWNED);
  unsigned int band = JETSAM_PRIORITY_IDLE;
  proc_t p;
  ...
  while (p) {
    ...
    if (!_memstat_proc_is_frozen(p) &&
      ...
      assert(p->p_memstat_freeze_skip_reason ==
 ...
 * Logs the failure, marks the process with the failure reason, and updates
 * freezer stats.
 */
static void memorystatus_freeze_handle_error(
    proc_t p, const freezer_error_code_t freezer_error_code, bool was_refreeze,
...
memorystatus_freeze_start_normal_throttle_interval(uint32_t new_budget,
  ...
  os_atomic_store(&memorystatus_freezer_stats.mfs_processes_thawed_webcontent,
                  0, release);
  os_atomic_store(&memorystatus_freezer_stats.mfs_processes_thawed_fg, 0,
                  release);
  os_atomic_store(
      &memorystatus_freezer_stats.mfs_processes_thawed_fg_xpc_service, 0,
      release);
  os_atomic_store(&memorystatus_freezer_stats.mfs_processes_frozen,
                  memorystatus_frozen_count, release);
  os_atomic_store(&memorystatus_freezer_stats.mfs_processes_frozen_webcontent,
                  memorystatus_frozen_count_webcontent, release);
  os_atomic_store(&memorystatus_freezer_stats.mfs_processes_frozen_xpc_service,
                  memorystatus_frozen_count_xpc_service, release);
  os_atomic_store(&memorystatus_freezer_stats.mfs_processes_fg_resumed, 0,
                  release);
  ...
  schedule_interval_reset(freeze_interval_reset_thread_call,
                          normal_throttle_window);
}

#if DEVELOPMENT || DEBUG

static int sysctl_memorystatus_freeze_calculate_new_budget SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  int error = 0;
  ...
  error =
      sysctl_handle_int(oidp, &time_since_last_interval_expired_sec, 0, req);
  if (error || !req->newptr) {
    return error;
  }
  ...
  new_budget = memorystatus_freeze_calculate_new_budget(
      time_since_last_interval_expired_sec, 1, NORMAL_WINDOW_MINS, 0);
  return copyout(&new_budget, req->oldptr,
                 MIN(sizeof(req->oldlen), sizeof(new_budget)));
}

SYSCTL_PROC(_vm, OID_AUTO, memorystatus_freeze_calculate_new_budget,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_MASKED, 0, 0,
            &sysctl_memorystatus_freeze_calculate_new_budget, "I", "");
...
memorystatus_freeze_out_of_budget(const struct throttle_interval_t *interval) {
  ...
  memorystatus_freezer_stats.mfs_budget_exhaustion_duration_remaining =
      time_left.tv_sec;
  memorystatus_log(
      "memorystatus_freeze: Out of NAND write budget with %u minutes left in "
      "the current freezer interval. %u procs are frozen.\n",
      time_left.tv_sec / 60, memorystatus_frozen_count);

  memorystatus_freeze_mark_eligible_processes_with_skip_reason(
      kMemorystatusFreezeSkipReasonOutOfBudget, false);
}
...
static void memorystatus_freeze_out_of_slots(void) {
  ...
  memorystatus_log("memorystatus_freeze: Out of slots in the freezer. %u procs "
                   "are frozen.\n",
                   memorystatus_frozen_count);

  memorystatus_freeze_mark_eligible_processes_with_skip_reason(
      kMemorystatusFreezeSkipReasonOutOfSlots, true);
}
 ...
 *    - return a budget based on the degraded throttle window's max. pageouts vs
 * current pageouts.
 *
 * 2) check to see if we are in a NEW normal window and update the normal
 * throttle window's params.
 *
 * 3) check what the current normal window allows for a budget.
 *
 * 4) calculate the current rate of pageouts for DEGRADED_WINDOW_MINS duration.
 * If that rate is below what we would normally expect, then we are running low
 * on our daily budget and need to enter degraded perf. mode.
 *
 *    Caller must hold the freezer mutex
...
memorystatus_freeze_update_throttle(uint64_t *budget_pages_allowed) {
  ...
  if (memorystatus_freeze_degradation == FALSE) {
    if (interval->pageouts >= interval->max_pageouts) {
      *budget_pages_allowed = 0;
      ...
      int budget_threshold =
          (freeze_daily_pageouts_max * FREEZE_DEGRADATION_BUDGET_THRESHOLD) /
          100;
      ...
      if (budget_left <= budget_threshold) {
        /*
         * For the current normal window, calculate how much we would pageout in
         * a DEGRADED_WINDOW_MINS duration. And also calculate what we would
         * pageout for the same DEGRADED_WINDOW_MINS duration if we had the full
         * daily pageout budget.
         */

        unsigned int current_budget_rate_allowed =
            ((budget_left / time_left.tv_sec) / 60) * DEGRADED_WINDOW_MINS;
        unsigned int normal_budget_rate_allowed =
            (freeze_daily_pageouts_max / NORMAL_WINDOW_MINS) *
  ...
  memorystatus_log_debug(
      "memorystatus_freeze_update_throttle_interval: throttle updated - %d "
      "frozen (%d max) within %dm; %dm remaining\n",
      interval->pageouts, interval->max_pageouts, interval->mins,
      (interval->ts.tv_sec - now_ts.tv_sec) / 60);
}

SYSCTL_UINT(_kern, OID_AUTO, memorystatus_freeze_apps_idle_delay_multiplier,
            CTLFLAG_RW | CTLFLAG_LOCKED,
            &memorystatus_freeze_apps_idle_delay_multiplier, 0, "");
...
static void memorystatus_freeze_thread(void *param __unused,
  ...
  if (memorystatus_freeze_enabled) {
    if (memorystatus_freezer_use_demotion_list &&
        memorystatus_refreeze_eligible_count > 0) {
      memorystatus_demote_frozen_processes(
          false); /* Normal mode. Consider demoting thawed processes. */
    }
    while (num_frozen < max_to_freeze &&
      ...
      if (memorystatus_freeze_budget_pages_remaining) {
        ...
        memorystatus_demote_frozen_processes(
            true); /* urgent mode..force one demotion */
        break;
      }
    }
  }
  ...
  /*
   * Give applications currently in the aging band a chance to age out into the
   * idle band before running the freezer again.
   */
  if (memorystatus_freeze_dynamic_thread_delay_enabled) {
    if ((num_frozen > 0) || (memorystatus_frozen_count == 0)) {
      memorystatus_freeze_apps_idle_delay_multiplier =
          FREEZE_APPS_IDLE_DELAY_MULTIPLIER_FAST;
    } else {
      memorystatus_freeze_apps_idle_delay_multiplier =
          FREEZE_APPS_IDLE_DELAY_MULTIPLIER_SLOW;
    }
  }
...
int memorystatus_get_process_is_freezable(pid_t pid, int *is_freezable) {
   ...
   * We can check for privileges and allow targeting another process in the
   * future.
   */
  if (p != current_proc()) {
    proc_rele(p);
...
errno_t memorystatus_get_process_is_frozen(pid_t pid, int *is_frozen) {
  proc_t p = PROC_NULL;
...
int memorystatus_set_process_is_freezable(pid_t pid, boolean_t is_freezable) {
   ...
   */
  if (is_freezable && !kauth_cred_issuser(kauth_cred_get()) &&
   ...
   */
  if (p != current_proc() && !kauth_cred_issuser(kauth_cred_get()) &&
      !IOCurrentTaskHasEntitlement(MEMORYSTATUS_ENTITLEMENT)) {
  ...
  if (is_freezable == FALSE) {
    ...
    memorystatus_log_info("memorystatus_set_process_is_freezable: disabling "
                          "freeze for pid %d [%s]\n",
                          proc_getpid(p), (*p->p_name ? p->p_name : "unknown"));
  } else {
    p->p_memstat_state &= ~P_MEMSTAT_FREEZE_DISABLED;
    memorystatus_log_info("memorystatus_set_process_is_freezable: enabling "
                          "freeze for pid %d [%s]\n",
                          proc_getpid(p), (*p->p_name ? p->p_name : "unknown"));
  }
  proc_rele(p);
...
void memorystatus_freeze_init_proc(proc_t p) {
  /* NB: Process is not on the memorystatus lists yet so it's safe to modify the
   * skip reason without the freezer mutex. */
...
static int sysctl_memorystatus_do_fastwake_warmup_all SYSCTL_HANDLER_ARGS {
  if (!req->newptr) {
    return EINVAL;
  }
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_do_fastwake_warmup_all,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED | CTLFLAG_MASKED, 0, 0,
            &sysctl_memorystatus_do_fastwake_warmup_all, "I",
            "Swap-in any compressed data that resides in swapfiles");

/*
 * Takes in a candidate list from the user_addr, validates it, and copies it
 * into the list pointer. Takes ownership over the original value of list.
 * Assumes that list is protected by the freezer_mutex.
 * The caller should not hold any locks.
 */
static errno_t
set_freezer_candidate_list(user_addr_t buffer, size_t buffer_size,
  ...
  if ((buffer == USER_ADDR_NULL) || (buffer_size == 0)) {
    memorystatus_log_error(
        "memorystatus_cmd_grp_set_freeze_priority: NULL or empty list\n");
    return EINVAL;
  }

  if (buffer_size % sizeof(memorystatus_properties_freeze_entry_v1) != 0) {
    memorystatus_log_error(
        "memorystatus_cmd_grp_set_freeze_priority: Invalid list length (caller "
        "might have comiled agsinst invalid headers.)\n");
    return EINVAL;
  }
  ...
  for (size_t i = 0; i < entry_count; i++) {
    ...
    if (entry->version != 1) {
      memorystatus_log_error("memorystatus_cmd_grp_set_freeze_priority: "
                             "Invalid entry version number.");
      error = EINVAL;
      goto out;
    }
    if (i > 0 && entry->priority >= entries[i - 1].priority) {
      memorystatus_log_error("memorystatus_cmd_grp_set_freeze_priority: Entry "
  ...
  tmp_size =
      list->mfcl_length * sizeof(memorystatus_properties_freeze_entry_v1);
...
errno_t memorystatus_cmd_grp_set_freeze_list(user_addr_t buffer,
                                             size_t buffer_size) {
  return set_freezer_candidate_list(buffer, buffer_size,
                                    &memorystatus_global_freeze_list);
}

errno_t memorystatus_cmd_grp_set_demote_list(user_addr_t buffer,
                                             size_t buffer_size) {
  return set_freezer_candidate_list(buffer, buffer_size,
...
void memorystatus_freezer_mark_ui_transition(proc_t p) {
  bool frozen = false, previous_focal_thaw = false, xpc_service = false,
       suspended = false;
  proc_list_lock();
  ...
  if (!suspended) {
    if (frozen) {
      if (!previous_focal_thaw) {
        p->p_memstat_state |= P_MEMSTAT_FROZEN_FOCAL_THAW;
        os_atomic_inc(&(memorystatus_freezer_stats.mfs_processes_thawed_fg),
                      relaxed);
        if (xpc_service) {
          os_atomic_inc(
              &(memorystatus_freezer_stats.mfs_processes_thawed_fg_xpc_service),
              relaxed);
        }
      }
    }
    os_atomic_inc(&(memorystatus_freezer_stats.mfs_processes_fg_resumed),
                  relaxed);
  }
