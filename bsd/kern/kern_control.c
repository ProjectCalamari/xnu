...
static int ctl_send(struct socket *, int, struct mbuf *, struct sockaddr *,
...
SYSCTL_NODE(_net_systm, OID_AUTO, kctl, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "Kernel control family");

struct kctlstat kctlstat;
SYSCTL_PROC(_net_systm_kctl, OID_AUTO, stats,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0, kctl_getstat,
            "S,kctlstat", "");

SYSCTL_PROC(_net_systm_kctl, OID_AUTO, reg_list,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0, kctl_reg_list,
            "S,xkctl_reg", "");

SYSCTL_PROC(_net_systm_kctl, OID_AUTO, pcblist,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0, kctl_pcblist,
            "S,xkctlpcb", "");
...
SYSCTL_INT(_net_systm_kctl, OID_AUTO, debug, CTLFLAG_RW | CTLFLAG_LOCKED,
           &ctl_debug, 0, "");

#if DEVELOPMENT || DEBUG
u_int32_t ctl_panic_debug = 0;
SYSCTL_INT(_net_systm_kctl, OID_AUTO, panicdebug, CTLFLAG_RW | CTLFLAG_LOCKED,
           &ctl_panic_debug, 0, "");
#endif /* DEVELOPMENT || DEBUG */
 ...
 */
__private_extern__ void kern_control_init(struct domain *dp) {
  struct protosw *pr;
  int i;
...
static void kcb_delete(struct ctl_cb *kcb) {
  if (kcb != 0) {
 ...
 */
static int ctl_attach(struct socket *so, int proto, struct proc *p) {
 ...
 */
static void ctl_kcb_increment_use_count(struct ctl_cb *kcb,
                                        lck_mtx_t *mutex_held) {
  LCK_MTX_ASSERT(mutex_held, LCK_MTX_ASSERT_OWNED);
  while (kcb->require_clearing_count > 0) {
    msleep(&kcb->require_clearing_count, mutex_held, PSOCK | PCATCH,
           "kcb_require_clearing", NULL);
  }
  kcb->kcb_usecount++;
}

static void ctl_kcb_require_clearing(struct ctl_cb *kcb,
                                     lck_mtx_t *mutex_held) {
  assert(kcb->kcb_usecount != 0);
  kcb->require_clearing_count++;
  kcb->kcb_usecount--;
  while (kcb->kcb_usecount >
         0) { // we need to wait until no one else is running
    msleep(&kcb->kcb_usecount, mutex_held, PSOCK | PCATCH, "kcb_usecount",
           NULL);
  }
  kcb->kcb_usecount++;
}

static void ctl_kcb_done_clearing(struct ctl_cb *kcb) {
  assert(kcb->require_clearing_count != 0);
  kcb->require_clearing_count--;
  wakeup((caddr_t)&kcb->require_clearing_count);
}

static void ctl_kcb_decrement_use_count(struct ctl_cb *kcb) {
  assert(kcb->kcb_usecount != 0);
  kcb->kcb_usecount--;
...
static int ctl_detach(struct socket *so) {
  struct ctl_cb *__single kcb = (struct ctl_cb *)so->so_pcb;
...
static int ctl_setup_kctl(struct socket *so, struct sockaddr *nam,
                          struct proc *p) {
...
static int ctl_bind(struct socket *so, struct sockaddr *nam, struct proc *p) {
  int error = 0;
  struct ctl_cb *__single kcb = (struct ctl_cb *)so->so_pcb;
...
static int ctl_connect(struct socket *so, struct sockaddr *nam,
                       struct proc *p) {
  int error = 0;
  struct ctl_cb *__single kcb = (struct ctl_cb *)so->so_pcb;
...
static int ctl_disconnect(struct socket *so) {
  struct ctl_cb *__single kcb = (struct ctl_cb *)so->so_pcb;
  ...
  if ((kcb = (struct ctl_cb *)so->so_pcb)) {
    ...
    /* Check for NULL here for the case where ctl_disconnect is racing with
     * itself and the first thread has already cleaned up the structure */
    if (kctl) {
      TAILQ_REMOVE(&kctl->kcb_head, kcb, next);
    }
    kctlstat.kcs_pcbcount--;
...
static void ctl_sbrcv_trim(struct socket *so) {
...
static int ctl_send(struct socket *so, int flags, struct mbuf *m,
...
#pragma unused(addr, p)
  ...
  if (error == 0 && kctl->send) {
    ...
    socket_unlock(so, 0);
    error =
        (*kctl->send)(kctl->kctlref, kcb->sac.sc_unit, kcb->userdata, m, flags);
    socket_lock(so, 0);
  } else {
    m_freem(m);
    if (error == 0) {
...
static int ctl_send_list(struct socket *so, struct mbuf *m, u_int *pktcnt,
                         int flags) {
  int error = 0;
  struct ctl_cb *kcb = (struct ctl_cb *)so->so_pcb;
  struct kctl *kctl;
  ...
  *pktcnt = 0;
  if (kctl->send_list != NULL) {
    ...
    socket_unlock(so, 0);
    error = (*kctl->send_list)(kctl->kctlref, kcb->sac.sc_unit, kcb->userdata,
                               m, flags);
    socket_lock(so, 0);
  } else {
    int space = 0;
    ...
    while (m != NULL && error == 0) {
      ...
      socket_unlock(so, 0);
      error = (*kctl->send)(kctl->kctlref, kcb->sac.sc_unit, kcb->userdata, m,
                            flags);
      socket_lock(so, 0);
      m = nextpkt;
      if (error == 0) {
...
static errno_t ctl_rcvbspace(struct socket *so, size_t datasize,
  ...
  if ((kctlflags & CTL_FLAG_REG_CRIT) == 0) {
     ...
     */
    if (space < (sb->sb_hiwat >> 2) || space < datasize) {
      error = ENOBUFS;
    } else {
      error = 0;
    }
     ...
     */
    autorcvbuf_max =
        min(sb->sb_idealsize + (sb->sb_idealsize >> 2), ctl_autorcvbuf_max);
    ...
    if ((u_int32_t)space >= datasize) {
      ...
      u_int32_t cc = (u_int32_t)MIN(MIN((sb->sb_hiwat + grow), autorcvbuf_max),
                                    UINT32_MAX);
...
errno_t ctl_enqueuembuf(kern_ctl_ref kctlref, u_int32_t unit, struct mbuf *m,
                        u_int32_t flags) {
...
bye:
  if (ctl_debug && error != 0 && (flags & CTL_DATA_CRIT)) {
    printf("%s - crit data err %d len %d hiwat %d cc: %d\n", __func__, error,
           len, so->so_rcv.sb_hiwat, so->so_rcv.sb_cc);
  }
 ...
 */
static int m_space(struct mbuf *m) {
  int space = 0;
  mbuf_ref_t nxt;
...
errno_t ctl_enqueuembuf_list(void *kctlref, u_int32_t unit, struct mbuf *m_list,
  ...
  for (m = m_list; m != NULL; m = nextpkt) {
    ...
    if (m->m_pkthdr.len == 0 && ctl_debug) {
      ...
      printf("%s: %u:%u m_pkthdr.len is 0", __func__, id, unit);
    }
...
errno_t ctl_enqueuedata(void *kctlref, u_int32_t unit,
...
bye:
  if (ctl_debug && error != 0 && (flags & CTL_DATA_CRIT)) {
    printf("%s - crit data err %d len %d hiwat %d cc: %d\n", __func__, error,
           (int)len, so->so_rcv.sb_hiwat, so->so_rcv.sb_cc);
  }
...
static int ctl_ctloutput(struct socket *so, struct sockopt *sopt) {
  struct ctl_cb *__single kcb = (struct ctl_cb *)so->so_pcb;
  struct kctl *__single kctl;
  int error = 0;
  void *data = NULL;
  ...
  case SOPT_SET:
    ...
    if (error == 0) {
      socket_unlock(so, 0);
      error = (*kctl->setopt)(kctl->kctlref, kcb->sac.sc_unit, kcb->userdata,
                              sopt->sopt_name, data, sopt->sopt_valsize);
      socket_lock(so, 0);
    }
  ...
  case SOPT_GET:
    ...
    if (error == 0) {
      ...
      socket_unlock(so, 0);
      error = (*kctl->getopt)(kctl->kctlref, kcb->sac.sc_unit, kcb->userdata,
                              sopt->sopt_name, data, &len);
...
#pragma unused(so, ifp, p)
  ...
  case CTLIOCGINFO: {
    ...
    lck_mtx_lock(&ctl_mtx);
    kctl = ctl_find_by_name(
        __unsafe_null_terminated_from_indexable(ctl_info.ctl_name));
    if (kctl) {
...
static void kctl_tbl_grow(void) {
  ...
  if (kctl_tbl_growing) {
    ...
    do {
      (void)msleep((caddr_t)&kctl_tbl_growing, &ctl_mtx, PSOCK | PCATCH,
                   "kctl_tbl_growing", 0);
...
static kern_ctl_ref kctl_make_ref(struct kctl *kctl) {
  ...
  for (i = 0; i < kctl_tbl_size; i++) {
    if (kctl_table[i] == NULL) {
       ...
       */
      ref = ((kctl_ref_gencnt << KCTLREF_GENCNT_SHIFT) & KCTLREF_GENCNT_MASK) +
...
static void kctl_delete_ref(kern_ctl_ref kctlref) {
  /*
   * Reference is index plus one
   */
...
static struct kctl *kctl_from_ref(kern_ctl_ref kctlref) {
  /*
   * Reference is index plus one
   */
  uintptr_t i = (((uintptr_t)kctlref) & KCTLREF_INDEX_MASK) - 1;
...
errno_t ctl_register(struct kern_ctl_reg *userkctl, kern_ctl_ref *kctlref) {
  ...
  if ((userkctl->ctl_flags & CTL_FLAG_REG_ID_UNIT) == 0) {
    ...
    if (ctl_find_by_name(__unsafe_null_terminated_from_indexable(
            userkctl->ctl_name)) != NULL) {
...
errno_t ctl_deregister(void *kctlref) {
  struct kctl *__single kctl;
 ...
 */
static struct kctl *ctl_find_by_name(const char *__null_terminated name) {
  struct kctl *__single kctl;
...
errno_t ctl_name_by_id(u_int32_t id, char *__counted_by(maxsize) out_name,
                       size_t maxsize) {
  int found = 0;
  struct kctl *__single kctl;
 ...
 */
static struct ctl_cb *kcb_find(struct kctl *kctl, u_int32_t unit) {
...
static int ctl_lock(struct socket *so, int refcount, void *lr) {
  void *__single lr_saved;
  ...
  if (so->so_pcb != NULL) {
    ...
    panic("ctl_lock: so=%p NO PCB! lr=%p lrh= %s", so, lr_saved,
          solockhistory_nr(so));
    /* NOTREACHED */
  }

  if (so->so_usecount < 0) {
    panic("ctl_lock: so=%p so_pcb=%p lr=%p ref=%x lrh= %s", so, so->so_pcb,
...
static int ctl_unlock(struct socket *so, int refcount, void *lr) {
  void *__single lr_saved;
  ...
  if (lr == NULL) {
	...
	if (so->so_usecount < 0) {
    panic("ctl_unlock: so=%p usecount=%x lrh= %s", so, so->so_usecount,
          solockhistory_nr(so));
    /* NOTREACHED */
	}
	if (so->so_pcb == NULL) {
    panic("ctl_unlock: so=%p NO PCB usecount=%x lr=%p lrh= %s", so,
...
static lck_mtx_t *ctl_getlock(struct socket *so, int flags) {
#pragma unused(flags)
  struct ctl_cb *__single kcb = (struct ctl_cb *)so->so_pcb;

  if (so->so_pcb) {
    if (so->so_usecount < 0) {
      panic("ctl_getlock: so=%p usecount=%x lrh= %s", so, so->so_usecount,
            solockhistory_nr(so));
    }
    return &kcb->mtx;
  } else {
    panic("ctl_getlock: so=%p NULL NO so_pcb %s", so, solockhistory_nr(so));
...
#pragma unused(oidp, arg1, arg2)
  ...
  for (i = 0, kctl = TAILQ_FIRST(&ctl_head); i < n && kctl != NULL;
    ...
    xkr->xkr_disconnect = (uint64_t)VM_KERNEL_UNSLIDE(kctl->disconnect);
    xkr->xkr_send = (uint64_t)VM_KERNEL_UNSLIDE(kctl->send);
    xkr->xkr_send_list = (uint64_t)VM_KERNEL_UNSLIDE(kctl->send_list);
...
#pragma unused(oidp, arg1, arg2)
  ...
  for (i = 0, kctl = TAILQ_FIRST(&ctl_head); i < n && kctl != NULL;
    ...
    for (kcb = TAILQ_FIRST(&kctl->kcb_head); i < n && kcb != NULL;
      ...
      sbtoxsockbuf_n(kcb->so ? &kcb->so->so_rcv : NULL, xsbrcv);
      sbtoxsockbuf_n(kcb->so ? &kcb->so->so_snd : NULL, xsbsnd);
...
#pragma unused(oidp, arg1, arg2)
  ...
  error = SYSCTL_OUT(req, &kctlstat, MIN(sizeof(struct kctlstat), req->oldlen));
done:
  lck_mtx_unlock(&ctl_mtx);
  return error;
}

void kctl_fill_socketinfo(struct socket *so, struct socket_info *si) {
  struct ctl_cb *__single kcb = (struct ctl_cb *)so->so_pcb;
  struct kern_ctl_info *__single kcsi = &si->soi_proto.pri_kern_ctl;
  struct kctl *__single kctl = kcb->kctl;
