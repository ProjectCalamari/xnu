...
static sy_call_t *const msgcalls[] = {(sy_call_t *)msgctl, (sy_call_t *)msgget,
...
struct msqid_kernel
    *msqids; /* MSGMNI msqid_kernel structs (wrapping user_msqid_ds structs) */
...
static __inline__ user_time_t sysv_msgtime(void) {
  struct timeval tv;
  microtime(&tv);
  return tv.tv_sec;
}
 ...
 */
static void msqid_ds_kerneltouser32(struct user_msqid_ds *in,
                                    struct user32_msqid_ds *out) {
...
static void msqid_ds_kerneltouser64(struct user_msqid_ds *in,
                                    struct user64_msqid_ds *out) {
  out->msg_perm = in->msg_perm;
  out->msg_qnum = in->msg_qnum;
  out->msg_cbytes = in->msg_cbytes; /* for ipcs */
  out->msg_qbytes = in->msg_qbytes;
 ...
 * the beginning.
 */
static void msqid_ds_user32tokernel(struct user32_msqid_ds *in,
                                    struct user_msqid_ds *out) {
...
static void msqid_ds_user64tokernel(struct user64_msqid_ds *in,
                                    struct user_msqid_ds *out) {
  out->msg_ctime = in->msg_ctime;
  out->msg_rtime = in->msg_rtime;
  out->msg_stime = in->msg_stime;
...
static int msginit(__unused void *dummy) {
 ...
 * msg calls) uap->a2		User argument descriptor
 *
 * Returns:	0	Success
 *              !0	Not success
 *
 ...
 *
 */
int msgsys(struct proc *p, struct msgsys_args *uap, int32_t *retval) {
...
int msgctl(struct proc *p, struct msgctl_args *uap, int32_t *retval) {
  int msqid = uap->msqid;
  int cmd = uap->cmd;
  ...
  case IPC_SET:
...
#endif
      msqbuf.msg_qbytes =
          msginfo.msgmnb; /* silently restrict qbytes to system limit */
    }
    ...
    msqptr->u.msg_perm.mode =
        (msqptr->u.msg_perm.mode & ~0777) | (msqbuf.msg_perm.mode & 0777);
...
int msgget(__unused struct proc *p, struct msgget_args *uap, int32_t *retval) {
  int msqid, eval;
  int key = uap->key;
  ...
  if (key != IPC_PRIVATE) {
    for (msqid = 0; msqid < msginfo.msgmni; msqid++) {
      msqptr = &msqids[msqid];
      if (msqptr->u.msg_qbytes != 0 && msqptr->u.msg_perm._key == key) {
        break;
      }
    }
...
int msgsnd(struct proc *p, struct msgsnd_args *uap, int32_t *retval) {
  __pthread_testcancel(1);
  return msgsnd_nocancel(p, (struct msgsnd_nocancel_args *)uap, retval);
}

int msgsnd_nocancel(struct proc *p, struct msgsnd_nocancel_args *uap,
                    int32_t *retval) {
  int msqid = uap->msqid;
  ...
  for (;;) {
    ...
    if (blocking_resource != NULL) {
      ...
      printf("goodnight\n");
#endif
      eval = msleep(blocking_resource, &sysv_msg_subsys_mutex,
                    (PZERO - 4) | PCATCH, "msgwait", 0);
#ifdef MSG_DEBUG_OK
...
int msgrcv(struct proc *p, struct msgrcv_args *uap, user_ssize_t *retval) {
  __pthread_testcancel(1);
  return msgrcv_nocancel(p, (struct msgrcv_nocancel_args *)uap, retval);
}

int msgrcv_nocancel(struct proc *p, struct msgrcv_nocancel_args *uap,
                    user_ssize_t *retval) {
  int msqid = uap->msqid;
  user_addr_t user_msgp = uap->msgp;
  size_t msgsz = (size_t)uap->msgsz; /* limit to 4G */
    ...
    if (msgtyp == 0) {
...
#if CONFIG_MACF
        eval = mac_sysvmsq_check_msgrcv(kauth_cred_get(), msghdr);
        if (eval) {
          goto msgrcvout;
        }
#endif
      ...
      while ((msghdr = *prev) != NULL) {
        ...
        if (msgtyp == msghdr->msg_type || msghdr->msg_type <= -msgtyp) {
          ...
          if (msgsz < msghdr->msg_ts && (msgflg & MSG_NOERROR) == 0) {
#ifdef MSG_DEBUG_OK
            printf("requested message on the queue is too big (want %ld, got "
                   "%d)\n",
                   msgsz, msghdr->msg_ts);
#endif
            eval = E2BIG;
  ...
  for (len = 0; len < msgsz; len += msginfo.msgssz) {
    ...
    SYSV_MSG_SUBSYS_UNLOCK();
    eval = copyout(&msgpool[next * msginfo.msgssz], user_msgp, tlen);
    SYSV_MSG_SUBSYS_LOCK();
    if (eval != 0) {
#ifdef MSG_DEBUG_OK
...
static int IPCS_msg_sysctl(__unused struct sysctl_oid *oidp,
...
SYSCTL_PROC(_kern_sysv_ipcs, OID_AUTO, msg,
            CTLFLAG_RW | CTLFLAG_ANYBODY | CTLFLAG_LOCKED, 0, 0,
            IPCS_msg_sysctl, "S,IPCS_msg_command",
