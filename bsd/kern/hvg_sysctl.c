...
static int hv_ret_to_errno(hvg_hcall_return_t ret) {
  switch (ret) {
 ...
 */
static int sysctl_trigger_kernel_coredump(__unused struct sysctl_oid *oidp,
                                          __unused void *arg1,
                                          __unused int arg2,
  ...
  char
      buf[3]; // 1 digit for dump option + 1 '\0' from userspace sysctl + 1 '\0'
  ...
  if (req->newptr) {
    ...
    if (!error) {
      ...
      hv_ret =
          hvg_hcall_trigger_dump(&sysctl_vmcore, HVG_HCALL_DUMP_OPTION_REGULAR);
 ...
 */
static int sysctl_get_mabs_offset(__unused struct sysctl_oid *oidp,
 ...
 */
static int sysctl_get_bootsessionuuid(__unused struct sysctl_oid *oidp,
                                      __unused void *arg1, __unused int arg2,
                                      struct sysctl_req *req) {
...
static int sysctl_vmm_present(__unused struct sysctl_oid *oidp,
                              __unused void *arg1, __unused int arg2,
                              struct sysctl_req *req) {
  int vmm_present = 0;
...
static SYSCTL_PROC(_kern_hvg, OID_AUTO, trigger_kernel_coredump,
                   CTLTYPE_STRING | CTLFLAG_RW | CTLFLAG_NOAUTO, NULL, 0,
...
static SYSCTL_PROC(_kern_hvg, OID_AUTO, mabs_offset,
                   ...
                   NULL, 0, sysctl_get_mabs_offset, "Q", "host time offset");

static SYSCTL_PROC(_kern_hvg, OID_AUTO, host_bootsessionuuid,
                   CTLTYPE_STRING | CTLFLAG_RD | CTLFLAG_NOAUTO |
                       CTLFLAG_LOCKED,
                   NULL, 0, sysctl_get_bootsessionuuid, "A",
                   "host boot session UUID");

/* Not dynamic, no need to manually register. */
static SYSCTL_PROC(_kern, OID_AUTO, hv_vmm_present,
                   CTLTYPE_INT | CTLFLAG_ANYBODY | CTLFLAG_KERN |
                       CTLFLAG_LOCKED,
                   NULL, 0, sysctl_vmm_present, "I", "running on a vmm");

__startup_func static void hvg_sysctl_init(void) {
  struct {
