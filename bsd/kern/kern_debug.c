...
static int sysctl_debug_syscall_rejection_mode(struct sysctl_oid __unused *oidp,
                                               void *__unused arg1,
                                               int __unused arg2,
...
SYSCTL_PROC(_kern, OID_AUTO, debug_syscall_rejection_mode,
            CTLFLAG_RW | CTLFLAG_LOCKED, &debug_syscall_rejection_mode, 0,
            sysctl_debug_syscall_rejection_mode, "I",
            "0: ignore, 1: non-fatal, 2: crash");

static size_t const predefined_masks =
    2; // 0: null mask (all 0), 1: all mask (all 1)
...
static size_t const syscall_rejection_mask_count =
    SYSCALL_REJECTION_SELECTOR_MASK_COUNT - predefined_masks;
static syscall_rejection_mask_t
    syscall_rejection_masks[syscall_rejection_mask_count];
...
bool debug_syscall_rejection_handle(int syscall_mach_trap_number) {
  ...
  case SYSCALL_REJECTION_MODE_GUARD: {
    if (flags & SYSCALL_REJECTION_FLAGS_ONCE) {
      int const number = syscall_mach_trap_number < 0
                             ? -syscall_mach_trap_number
                             : (mach_trap_count + syscall_mach_trap_number);
    ...
    mach_exception_subcode_t subcode = syscall_mach_trap_number < 0
                                           ? -syscall_mach_trap_number
                                           : syscall_mach_trap_number;
...
void rejected_syscall_guard_ast(thread_t t, mach_exception_data_type_t code,
...
static void _syscall_rejection_apply_mask(syscall_rejection_mask_t dest,
                                          const syscall_rejection_mask_t src,
                                          bool apply_as_allow) {
  assert(dest != NULL);
  assert(src != NULL);
...
int sys_debug_syscall_reject_config(
    struct proc *p __unused, struct debug_syscall_reject_config_args *args,
    int *retval) {
  int error = 0;
  ...
  *retval = 0;
  ...
  for (int i = 0;
       ...
       (sizeof(args->packed_selectors1) + sizeof(args->packed_selectors2)) * 8;
       i += SYSCALL_REJECTION_SELECTOR_BITS) {
#define s_left_shift(x, n) ((n) < 0 ? ((x) >> -(n)) : ((x) << (n)))

    syscall_rejection_selector_t const selector =
        (syscall_rejection_selector_t)(((i < 64 ? (args->packed_selectors1 >> i)
                                                : 0) |
                                        (i > 64 - SYSCALL_REJECTION_SELECTOR_BITS
                                             ? s_left_shift(
                                                   args->packed_selectors2,
                                                   64 - i)
                                             : 0)) &
    ...
    syscall_rejection_mask_t mask_to_apply =
        syscall_rejection_masks[mask_index - predefined_masks];
  ...
  if ((args->flags & SYSCALL_REJECTION_FLAGS_ONCE)) {
    if (ut->syscall_rejection_once_mask == NULL) {
      ...
      bitmap_or(ut->syscall_rejection_mask, ut->syscall_rejection_mask,
                ut->syscall_rejection_once_mask, mach_trap_count + nsysent);
    }
  }
...
out_locked:
  ...
  if (error == ENOENT &&
      debug_syscall_rejection_mode == SYSCALL_REJECTION_MODE_IGNORE) {
...
static bool _syscall_rejection_add(syscall_rejection_mask_t dst,
                                   char const *name) {
  /*
...
static int
_sysctl_debug_syscall_rejection_masks(struct sysctl_oid __unused *oidp,
                                      void *__unused arg1, int __unused arg2,
                                      struct sysctl_req *req) {
  ...
  if (n != 1 || id < predefined_masks ||
      id > syscall_rejection_mask_count + predefined_masks) {
  ...
  syscall_rejection_mask_t new_mask =
      kalloc_data(SR_MASK_SIZE, Z_WAITOK | Z_ZERO);
  if (new_mask == NULL) {
  ...
  syscall_rejection_mask_t *target_mask =
      &syscall_rejection_masks[id - predefined_masks];
...
SYSCTL_PROC(_kern, OID_AUTO, syscall_rejection_masks,
            CTLTYPE_STRING | CTLFLAG_WR | CTLFLAG_MASKED | CTLFLAG_LOCKED, 0, 0,
            _sysctl_debug_syscall_rejection_masks, "A",
            "system call rejection masks");
...
int sys_debug_syscall_reject_config(
    struct proc *__unused p,
    struct debug_syscall_reject_config_args *__unused args, int __unused *ret) {
  /* not supported. */
  return ENOTSUP;
}

int debug_syscall_reject(struct proc *__unused p,
                         struct debug_syscall_reject_args *__unused args,
                         int *__unused retval) {
  /* not supported. */
  return ENOTSUP;
}

void reset_debug_syscall_rejection_mode(void) { /* not supported. */ }

#endif /* CONFIG_DEBUG_SYSCALL_REJECTION */

#if __arm64__ && (DEBUG || DEVELOPMENT)

static void _spinfor(uint64_t nanoseconds) {
  uint64_t mt = 0;
...
static int
_sysctl_debug_disable_interrupts_test(struct sysctl_oid __unused *oidp,
                                      void *__unused arg1, int __unused arg2,
                                      struct sysctl_req *req) {
  int error = 0;
...
static int
_sysctl_debug_disable_preemption_test(struct sysctl_oid __unused *oidp,
                                      void *__unused arg1, int __unused arg2,
                                      struct sysctl_req *req) {
  int error = 0;
...
SYSCTL_PROC(
    _kern, OID_AUTO, debug_disable_interrupts_test,
    CTLTYPE_QUAD | CTLFLAG_WR | CTLFLAG_MASKED | CTLFLAG_LOCKED, 0, 0,
    _sysctl_debug_disable_interrupts_test, "Q",
    "disable interrupts for specified number of nanoseconds, for testing");

SYSCTL_PROC(
    _kern, OID_AUTO, debug_disable_preemption_test,
    CTLTYPE_QUAD | CTLFLAG_WR | CTLFLAG_MASKED | CTLFLAG_LOCKED, 0, 0,
    _sysctl_debug_disable_preemption_test, "Q",
    "disable preemption for specified number of nanoseconds, for testing");
