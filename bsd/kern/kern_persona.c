...
#define persona_initialized(p)                                                 \
  ((p)->pna_valid == PERSONA_MAGIC || (p)->pna_valid == PERSONA_INIT_TOKEN)
#define persona_valid(p) ((p)->pna_valid == PERSONA_MAGIC)
#define persona_mkinvalid(p) ((p)->pna_valid = ~(PERSONA_MAGIC))

static LIST_HEAD(personalist,
                 persona) all_personas = LIST_HEAD_INITIALIZER(all_personas);
...
extern kern_return_t bank_get_bank_ledger_thread_group_and_persona(
    void *voucher, void *bankledger, void **banktg, uint32_t *persona_id);
void ipc_voucher_release(void *voucher);

struct persona *persona_alloc(uid_t id, const char *login, persona_type_t type,
                              char *path, uid_t uid, int *error) {
...
try_again:
  ...
  persona_dbg("Beginning Initialization of %d:%d (%s)...", id, persona->pna_id,
              persona->pna_login);
  ...
  LIST_FOREACH(tmp, &all_personas, pna_list) {
    ...
    if (strncmp(tmp->pna_login, persona->pna_login, sizeof(tmp->pna_login)) ==
            0 ||
...
out:
  if (err != 0) {
    persona_dbg("ERROR:%d while initializing %d:%d (%s)...", err, id,
...
void persona_init_end(struct persona *persona, int error) {
  ...
  if (error != 0 || persona->pna_valid == PERSONA_ALLOC_TOKEN) {
    persona_dbg("ERROR:%d after initialization of %d (%s)", error,
                persona->pna_id, persona->pna_login);
    /* remove this persona from the global count */
    os_atomic_dec(&g_total_personas, relaxed);
  } else if (error == 0 && persona->pna_valid == PERSONA_INIT_TOKEN) {
    persona->pna_valid = PERSONA_MAGIC;
    LIST_INSERT_HEAD(&all_personas, persona, pna_list);
    persona_dbg("Initialization of %d (%s) Complete.", persona->pna_id,
                persona->pna_login);
  }
...
static void persona_put_and_unlock(struct persona *persona) {
  int destroy = 0;
...
int persona_find(const char *login, uid_t uid, struct persona **persona,
                 size_t *plen) {
...
int persona_find_all(const char *login, uid_t uid, persona_type_t persona_type,
  ...
  persona_dbg("Searching with %d parameters (l:\"%s\", u:%d)", match, login,
              uid);
...
#ifdef PERSONA_DEBUG
    if (m > 0) {
      persona_dbg("ID:%d Matched %d/%d, found:%d, *plen:%d", tmp->pna_id, m,
                  match, (int)found, (int)*plen);
    }
#endif
    persona_unlock(tmp);
  }
...
uid_t current_persona_get_id(void) {
  ...
  if (voucher != IPC_VOUCHER_NULL) {
     ...
     */
    bank_get_bank_ledger_thread_group_and_persona(voucher, NULL, NULL,
                                                  &current_persona_id);
 ...
 */
static struct persona *proc_set_persona_internal(proc_t p,
                                                 struct persona *persona,
  ...
  if (p->p_persona) {
    old_persona =
        proc_reset_persona_internal(p, PROC_REMOVE_PERSONA, NULL, persona);
  }
  ...
  if (new_uid != 0 && (rlim_t)chgproccnt(new_uid, 0) > nproc) {
    pna_err("PID:%d hit proc rlimit in new persona(%d): %s", proc_getpid(p),
            new_uid, persona_desc(persona, 1));

    *rlim_error = EACCES;

    if (old_persona) {
      (void)proc_reset_persona_internal(p, PROC_RESET_OLD_PERSONA, old_persona,
                                        persona);
    }
  ...
  *rlim_error = 0;
  ...
  if (new_uid != old_uid) {
    ...
    persona_dbg("Increment Persona:%d proc_count to: %lu", new_uid, count);
  }
...
int persona_proc_adopt(proc_t p, struct persona *persona, /* consumed */
                       kauth_cred_derive_t derive_fn) {
  int error;
  struct persona *old_persona;
  ...
  persona_dbg("%d adopting Persona %d (%s)", proc_pid(p), persona->pna_id,
              persona_desc(persona, 0));
  ...
  /* Only Multiuser Mode needs to update the session login name to the persona
   * name */
...
int persona_proc_drop(proc_t p) {
  ...
  persona_dbg("PID:%d, %s -> <none>", proc_getpid(p),
              persona_desc(p->p_persona, 0));
...
uid_t persona_get_uid(__unused struct persona *persona) {
  return KAUTH_UID_NONE;
}
...
struct persona *persona_lookup(__unused uid_t id) { return NULL; }

int persona_find(__unused const char *login, __unused uid_t uid,
                 __unused struct persona **persona, __unused size_t *plen) {
  return ENOTSUP;
}

int persona_find_by_type(__unused int persona_type,
                         __unused struct persona **persona,
                         __unused size_t *plen) {
  return ENOTSUP;
}

struct persona *persona_proc_get(__unused pid_t pid) { return NULL; }

uid_t current_persona_get_id(void) { return PERSONA_ID_NONE; }

struct persona *current_persona_get(void) { return NULL; }

struct persona *persona_get(struct persona *persona) { return persona; }

struct persona *proc_persona_get(__unused proc_t p) { return NULL; }

void persona_put(__unused struct persona *persona) { return; }

boolean_t persona_is_adoption_allowed(__unused struct persona *persona) {
  return FALSE;
}
#endif
