 ...
 */
static int sbuf_extendsize(size_t *size) {
 ...
 */
struct sbuf *sbuf_new(struct sbuf *s, char *buf, int length_, int flags) {
 ...
 * 0 on success, -1 on failure.
 */
static int OS_WARN_RESULT sbuf_extend(struct sbuf *s, size_t addlen) {
  char *new_buf;
  size_t new_size;
 ...
 */
static size_t sbuf_capacity(const struct sbuf *s) {
 ...
 */
static int sbuf_ensure_capacity(struct sbuf *s, size_t wanted) {
  size_t size;
 ...
 * overflowed.
 */
int sbuf_bcat(struct sbuf *s, const void *data, size_t len) {
 ...
 */
int sbuf_bcpy(struct sbuf *s, const void *data, size_t len) {
  sbuf_clear(s);
 ...
 */
int sbuf_putc(struct sbuf *s, int c_) {
  char c = (char)c_;
 ...
 */
int sbuf_overflowed(struct sbuf *s) { return !!SBUF_HASOVERFLOWED(s); }
...
#define SBUF_ASSERT_NOT(x) SBUF_ASSERT(!(x))
...
static int sysctl_sbuf_tests SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  ...
  SBUF_TESTING("sbuf_bcpy") {
    ...
    SBUF_SHOULD(
        "succeed if the sbuf is marked as overflowed, but there is space") {
      struct sbuf *s = NULL;
  ...
  SBUF_TESTING("sbuf_finish") {
    SBUF_SHOULD("insert a nul byte, clear the overflowed flag and set the "
                "finished flag") {
      struct sbuf *s = NULL;
...
SYSCTL_PROC(_kern, OID_AUTO, sbuf_test,
            CTLTYPE_STRING | CTLFLAG_RW | CTLFLAG_KERN | CTLFLAG_MASKED, 0, 0,
            sysctl_sbuf_tests, "A", "sbuf tests");
