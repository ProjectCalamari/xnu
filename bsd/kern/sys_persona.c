...
static int kpersona_alloc_syscall(user_addr_t infop, user_addr_t idp,
  ...
  error = 0;
  persona = persona_alloc(id, login, kinfo.persona_type, pna_path,
                          kinfo.persona_uid, &error);
  if (!persona) {
    if (pna_path != NULL) {
      zfree(ZV_NAMEI, pna_path);
    }
...
static int kpersona_getpath_syscall(user_addr_t idp, user_addr_t path) {
  int error;
  uid_t persona_id;
  struct persona *persona;
  size_t pathlen;
...
static void kpersona_populate(struct kpersona_info *kinfo,
                              struct persona *persona) {
  memset(kinfo, 0, sizeof(struct kpersona_info));
...
static int kpersona_info_syscall(user_addr_t idp, user_addr_t infop) {
  ...
  persona_dbg("FOUND: persona: id:%d, login:\"%s\"", persona->pna_id,
              persona->pna_login);
...
static int kpersona_find_syscall(user_addr_t infop, user_addr_t idp,
                                 user_addr_t idlenp) {
  int error;
  struct kpersona_info kinfo;
  const char *login;
  ...
  k_idlen = u_idlen;
  error =
      persona_find_all(login, kinfo.persona_id,
                       (persona_type_t)kinfo.persona_type, persona, &k_idlen);
  if (error) {
    goto out;
  }
  ...
  for (size_t i = 0; i < k_idlen; i++) {
    ...
    error = copyout(&persona[i]->pna_id, idp + (i * sizeof(persona[i]->pna_id)),
 ...
 */
int persona(__unused proc_t p, struct persona_args *pargs,
