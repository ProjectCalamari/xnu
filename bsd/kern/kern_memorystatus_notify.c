 ...
 */
static vm_pressure_level_t
    convert_internal_pressure_level_to_dispatch_level(vm_pressure_level_t);
static boolean_t is_knote_registered_modify_task_pressure_bits(
    struct knote *, int, task_t, vm_pressure_level_t, vm_pressure_level_t);
static void memorystatus_klist_reset_all_for_level(
    vm_pressure_level_t pressure_level_to_clear);
static struct knote *vm_pressure_select_optimal_candidate_to_notify(
    struct klist *candidate_list, int level,
    boolean_t target_foreground_process, uint64_t *next_telemetry_update);
...
/*
 * This value is the threshold that a process must meet to be considered for
 * scavenging.
 */
...
static TUNABLE_DEV_WRITEABLE(uint32_t, vm_pressure_task_footprint_min,
                             "vm_pressure_notify_min_footprint_mb",
                             VM_PRESSURE_MINIMUM_RSIZE);

#if DEVELOPMENT || DEBUG
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_vm_pressure_task_footprint_min,
            CTLFLAG_RW | CTLFLAG_LOCKED, &vm_pressure_task_footprint_min, 0,
            "");
...
static uint64_t memorystatus_jetsam_notification_delay_ns =
    5ull * 1000 * 1000 * 1000; /* nanosec */

#if DEVELOPMENT || DEBUG
SYSCTL_QUAD(_kern, OID_AUTO, memorystatus_jetsam_notification_delay_ns,
...
static int filt_memorystatus(struct knote *kn __unused, long hint) {
  if (hint) {
    switch (hint) {
    ...
    case kMemorystatusPressure:
      if (memorystatus_vm_pressure_level == kVMPressureWarning ||
          memorystatus_vm_pressure_level == kVMPressureUrgent) {
...
static int filt_memorystatusprocess(struct knote *kn,
...
int memorystatus_knote_register(struct knote *kn) {
   ...
   */
  if ((kn->kn_sfflags & EVFILT_MEMORYSTATUS_ALL_MASK) ==
      (unsigned int)kn->kn_sfflags) {
#if XNU_TARGET_OS_OSX
    if (kn->kn_sfflags & NOTE_MEMORYSTATUS_PROC_LIMIT_WARN) {
      kn->kn_sfflags |= NOTE_MEMORYSTATUS_PROC_LIMIT_WARN_ACTIVE;
      kn->kn_sfflags |= NOTE_MEMORYSTATUS_PROC_LIMIT_WARN_INACTIVE;
...
void memorystatus_knote_unregister(struct knote *kn __unused) {
  memorystatus_klist_lock();
...
uint64_t memorystatus_kill_on_sustained_pressure_window_s =
    60 * 10; /* 10 Minutes */
uint64_t memorystatus_kill_on_sustained_pressure_delay_ms =
    500; /* .5 seconds */

SYSCTL_QUAD(_kern_memorystatus, OID_AUTO, kill_on_sustained_pressure_count,
            CTLFLAG_RD | CTLFLAG_LOCKED,
            &memorystatus_kill_on_sustained_pressure_count, "");
SYSCTL_QUAD(_kern_memorystatus, OID_AUTO, kill_on_sustained_pressure_window_s,
            CTLFLAG_RW | CTLFLAG_LOCKED,
            &memorystatus_kill_on_sustained_pressure_window_s, "");
SYSCTL_QUAD(_kern_memorystatus, OID_AUTO, kill_on_sustained_pressure_delay_ms,
            CTLFLAG_RW | CTLFLAG_LOCKED,
...
void memorystatus_notify_init() {
  sustained_pressure_handler_thread_call = thread_call_allocate_with_options(
      sustained_pressure_handler, NULL, THREAD_CALL_PRIORITY_KERNEL_HIGH,
      THREAD_CALL_OPTIONS_ONCE);
  memorystatus_notify_update_telemetry_thread_call =
      thread_call_allocate_with_options(update_footprints_for_telemetry, NULL,
...
inline int memorystatus_send_note(int event_code, void *data,
...
boolean_t memorystatus_warn_process(const proc_t p,
                                    __unused boolean_t is_active,
                                    __unused boolean_t is_fatal,
                                    boolean_t limit_exceeded) {
  /*
   * This function doesn't take a reference to p or lock it. So it better be the
   * current process.
   */
  ...
  SLIST_FOREACH(kn, &memorystatus_klist, kn_selnext) {
    ...
    if (knote_pid == pid) {
       ...
       * via UIKit (see applicationDidReceiveMemoryWarning in UIKit). We'll warn
       * them here if they are near there memory limit. filt_memorystatus() will
       * warn them based on the system pressure level.
       *
       * On desktop, (NOTE_MEMORYSTATUS_PRESSURE_WARN |
       * NOTE_MEMORYSTATUS_PRESSURE_CRITICAL) are only expected to fire for
       * system level warnings. Desktop procesess register for
       * NOTE_MEMORYSTATUS_PROC_LIMIT_WARN if they want to be warned when they
       * approach their limit and for NOTE_MEMORYSTATUS_PROC_LIMIT_CRITICAL to
       * be warned when they exceed their limit.
       *
       ...
       *
       */
      if (platform == PLATFORM_MACOS || platform == PLATFORM_MACCATALYST ||
          platform == PLATFORM_DRIVERKIT) {
        if (!limit_exceeded) {
          if (kn->kn_sfflags & NOTE_MEMORYSTATUS_PROC_LIMIT_WARN) {
            found_knote = TRUE;
            if (!is_fatal) {
          ...
          if (kn->kn_sfflags & NOTE_MEMORYSTATUS_PROC_LIMIT_CRITICAL) {
            ...
            if (!is_fatal) {
               ...
               */
              panic(
                  "Caught sending pid %d a critical warning for a fatal limit.",
                  pid);
            }
          }
        }
...
int memorystatus_send_pressure_note(pid_t pid) {
...
boolean_t memorystatus_bg_pressure_eligible(proc_t p) {
  ...
  /* Foreground processes have already been dealt with at this point, so just
   * test for eligibility */
  if (!(p->p_memstat_state & (P_MEMSTAT_TERMINATED | P_MEMSTAT_LOCKED |
                              P_MEMSTAT_SUSPENDED | P_MEMSTAT_FROZEN))) {
    eligible = TRUE;
  }

  if (p->p_memstat_effectivepriority <
...
void memorystatus_send_low_swap_note(void) {
  ...
  memorystatus_klist_lock();
  SLIST_FOREACH(kn, &memorystatus_klist, kn_selnext) {
    /* We call is_knote_registered_modify_task_pressure_bits to check if the
     * sfflags for the current note contain NOTE_MEMORYSTATUS_LOW_SWAP. Once we
     * find one note in the memorystatus_klist that has the
     * NOTE_MEMORYSTATUS_LOW_SWAP flags in its sfflags set, we call KNOTE with
     * kMemoryStatusLowSwap as the hint to process and update all knotes on the
     * memorystatus_klist accordingly. */
    if (is_knote_registered_modify_task_pressure_bits(
            kn, NOTE_MEMORYSTATUS_LOW_SWAP, NULL, 0, 0) == TRUE) {
 ...
 */
CA_EVENT(memorystatus_pressure_interval, CA_INT, num_processes_registered,
         CA_INT, num_notifications_sent, CA_INT, max_level, CA_INT,
         num_transitions, CA_INT, num_kills, CA_INT, duration);
...
CA_EVENT(memorystatus_proc_notification, CA_INT, footprint_before_notification,
         CA_INT, footprint_1_min_after_first_warning, CA_INT,
         footprint_5_min_after_first_warning, CA_INT,
         footprint_20_min_after_first_warning, CA_INT,
         footprint_1_min_after_first_critical, CA_INT,
         footprint_5_min_after_first_critical, CA_INT,
         footprint_20_min_after_first_critical, CA_INT, order_within_list,
         CA_INT, num_notifications_sent, CA_INT,
         time_between_warning_and_critical, CA_STATIC_STRING(CA_PROCNAME_LEN),
         proc_name);

/* The send timestamps for the first notifications are stored in the knote's
 * kn_sdata field */
...
struct knote_footprint_history {
  ...
  uint32_t kfh_footprint_after_warn_1;      /* 1 minute after first warning
                                               notification */
  uint32_t kfh_footprint_after_warn_5;      /* 5 minutes after first warning
                                               notification */
  uint32_t kfh_footprint_after_warn_20;     /* 20 minutes after first warning
                                               notification */
  uint32_t kfh_footprint_after_critical_1;  /* 1 minute after first critical
                                               notification */
  uint32_t kfh_footprint_after_critical_5;  /* 5 minutes after first critical
                                               notification */
  uint32_t kfh_footprint_after_critical_20; /* 20 minutes after first critical
                                               notification */
  uint16_t kfh_num_notifications;
  uint16_t kfh_notification_order;
} __attribute__((packed));

static_assert(sizeof(struct knote_footprint_history) <= sizeof(uint64_t) * 4,
              "footprint history fits in knote extensions");

static void mark_knote_send_time(struct knote *kn, task_t task,
                                 int knote_pressure_level,
                                 uint16_t order_within_list) {
  uint32_t *timestamps;
  uint32_t index;
  uint64_t curr_ts, curr_ts_seconds;
  struct knote_footprint_history *footprint_history =
      (struct knote_footprint_history *)kn->kn_ext;
  if (knote_pressure_level != NOTE_MEMORYSTATUS_PRESSURE_NORMAL) {
    ...
    index = knote_pressure_level == NOTE_MEMORYSTATUS_PRESSURE_WARN
                ? KNOTE_SEND_TIMESTAMP_WARNING_INDEX
                : KNOTE_SEND_TIMESTAMP_CRITICAL_INDEX;
    if (timestamps[index] == 0) {
      ...
      if (timestamps[index == KNOTE_SEND_TIMESTAMP_WARNING_INDEX
                         ? KNOTE_SEND_TIMESTAMP_CRITICAL_INDEX
                         : KNOTE_SEND_TIMESTAMP_WARNING_INDEX] == 0) {
        /*
         ...
         */
        footprint_history->kfh_starting_footprint = (uint32_t)MIN(
            UINT32_MAX, get_task_phys_footprint(task) / (2UL << 20));
 ...
 *
 * Returns the soonest absolutetime when this footprint history should be
 * updated again.
 */
static uint64_t update_knote_footprint_history(struct knote *kn, task_t task,
                                               uint64_t curr_ts) {
  uint32_t *timestamps = (uint32_t *)&(kn->kn_sdata);
  struct knote_footprint_history *footprint_history =
      (struct knote_footprint_history *)kn->kn_ext;
  uint64_t warning_send_time, critical_send_time,
      minutes_since_warning = UINT64_MAX, minutes_since_critical = UINT64_MAX;
  ...
  uint32_t task_phys_footprint_mb =
      (uint32_t)MIN(UINT32_MAX, get_task_phys_footprint(task) / (2UL << 20));
  uint64_t next_run = UINT64_MAX, absolutetime_in_minute = 0,
           minutes_since_last_notification = 0, curr_ts_s;
  ...
  if (warning_send_time != 0) {
    ...
    if (footprint_history->kfh_footprint_after_warn_1 == 0 &&
        minutes_since_warning >= 1) {
      footprint_history->kfh_footprint_after_warn_1 = task_phys_footprint_mb;
    }
    if (footprint_history->kfh_footprint_after_warn_5 == 0 &&
        minutes_since_warning >= 5) {
      footprint_history->kfh_footprint_after_warn_5 = task_phys_footprint_mb;
    }
    if (footprint_history->kfh_footprint_after_warn_20 == 0 &&
        minutes_since_warning >= 20) {
  ...
  if (critical_send_time != 0) {
    ...
    if (footprint_history->kfh_footprint_after_critical_1 == 0 &&
        minutes_since_critical >= 1) {
      footprint_history->kfh_footprint_after_critical_1 =
          task_phys_footprint_mb;
    }
    if (footprint_history->kfh_footprint_after_critical_5 == 0 &&
        minutes_since_critical >= 5) {
      footprint_history->kfh_footprint_after_critical_5 =
          task_phys_footprint_mb;
    }
    if (footprint_history->kfh_footprint_after_critical_20 == 0 &&
        minutes_since_critical >= 20) {
      footprint_history->kfh_footprint_after_critical_20 =
          task_phys_footprint_mb;
    }
  }

  minutes_since_last_notification =
      MIN(minutes_since_warning, minutes_since_critical);
 ...
 */
static void memorystatus_pressure_interval_send(void) {
  uint64_t duration_nanoseconds;
  CA_EVENT_TYPE(memorystatus_pressure_interval) * evt_data;

  /*
   * Drop the event rather than block for memory. We should be in a normal
   * pressure level now, but we don't want to end up blocked in page_wait if
   * there's a sudden spike in pressure.
   */
  ca_event_t event_wrapper =
      CA_EVENT_ALLOCATE_FLAGS(memorystatus_pressure_interval, Z_NOWAIT);
  if (event_wrapper) {
    absolutetime_to_nanoseconds(mach_absolute_time() -
                                    memstat_cur_interval.start_mt,
    ...
    evt_data->num_kills =
        os_atomic_load(&memstat_cur_interval.num_kills, relaxed);
 ...
 */
static void memorystatus_pressure_proc_telemetry_send(void) {
  struct knote *kn = NULL;
  SLIST_FOREACH(kn, &memorystatus_klist, kn_selnext) {
    proc_t p = PROC_NULL;
    struct knote_footprint_history *footprint_history =
    ...
    uint32_t critical_send_time =
        timestamps[KNOTE_SEND_TIMESTAMP_CRITICAL_INDEX];
    ...
    if (warning_send_time != 0 || critical_send_time != 0) {
       ...
       * there's a sudden spike in pressure.
       */
      ca_event_t event_wrapper = CA_EVENT_ALLOCATE_FLAGS(
          memorystatus_proc_notification, Z_NOWAIT | Z_ZERO);
      if (event_wrapper) {
        event = event_wrapper->data;

        event->footprint_before_notification =
            footprint_history->kfh_starting_footprint;
        event->footprint_1_min_after_first_warning =
            footprint_history->kfh_footprint_after_warn_1;
        event->footprint_5_min_after_first_warning =
            footprint_history->kfh_footprint_after_warn_5;
        event->footprint_20_min_after_first_warning =
            footprint_history->kfh_footprint_after_warn_20;
        event->footprint_1_min_after_first_critical =
            footprint_history->kfh_footprint_after_critical_1;
        event->footprint_5_min_after_first_critical =
            footprint_history->kfh_footprint_after_critical_5;
        event->footprint_20_min_after_first_critical =
            footprint_history->kfh_footprint_after_critical_20;
        event->num_notifications_sent =
            footprint_history->kfh_num_notifications;
        if (warning_send_time != 0 && critical_send_time != 0) {
          event->time_between_warning_and_critical =
              (critical_send_time - warning_send_time) / 60; // Minutes
        }
        ...
        strlcpy(event->proc_name, proc_name_address(p),
                sizeof(event->proc_name));
 ...
 * kn_max - knote
 *
 * knote_pressure_level - to check if the knote is registered for this
 * notification level.
 *
 * task    - task whose bits we'll be modifying
 *
 * pressure_level_to_clear - if the task has been notified of this past level,
 * clear that notification bit so that if/when we revert to that level, the task
 * will be notified again.
 *
 * pressure_level_to_set - the task is about to be notified of this new level.
 * Update the task's bit notification information appropriately.
 *
 */

static boolean_t is_knote_registered_modify_task_pressure_bits(
    struct knote *kn_max, int knote_pressure_level, task_t task,
    vm_pressure_level_t pressure_level_to_clear,
    vm_pressure_level_t pressure_level_to_set) {
  if (kn_max->kn_sfflags & knote_pressure_level) {
    if (pressure_level_to_clear &&
        task_has_been_notified(task, pressure_level_to_clear) == TRUE) {
...
void consider_vm_pressure_events(void) { vm_dispatch_memory_pressure(); }
  ...
  *next_telemetry_update = UINT64_MAX;
  ...
  SLIST_FOREACH(kn, candidate_list, kn_selnext) {
...
#if CONFIG_MEMORYSTATUS
    if (target_foreground_process == TRUE &&
    ...
    *next_telemetry_update = MIN(*next_telemetry_update, telemetry_update);
    ...
    vm_pressure_level_t dispatch_level =
...
#if CONFIG_MEMORYSTATUS
    if (target_foreground_process == FALSE &&
    ...
    /*
     * Privileged listeners are only considered in the multi-level pressure
     * scheme AND only if the pressure is increasing.
     */
    if (level > 0) {
      if (task_has_been_notified(t, level) == FALSE) {
         ...
         */
        if (task_low_mem_privileged_listener(t, FALSE, &privileged_listener) ==
            0) {
       ...
       */
      if ((task_has_been_notified(t, kVMPressureWarning) == FALSE) &&
          (task_has_been_notified(t, kVMPressureCritical) == FALSE)) {
        proc_rele(p);
        continue;
      }
    }
    ...
    if (resident_size >= vm_pressure_task_footprint_min) {
      if (level > 0) {
        ...
        if (pressure_increase) {
          if ((curr_task_importance < selected_task_importance) ||
              ((curr_task_importance == selected_task_importance) &&
               (resident_size > resident_max))) {
            /*
             * We have found a candidate process which is:
             * a) at a lower importance than the current selected process
             * OR
             * b) has importance equal to that of the current selected process
             * but is larger
             */
      ...
      VM_PRESSURE_DEBUG(
          0,
          "[vm_pressure] threshold failed for pid %d with %llu resident...\n",
...
done_scanning:
  if (kn_max) {
    VM_DEBUG_CONSTANT_EVENT(
        vm_pressure_event, DBG_VM_PRESSURE_EVENT, DBG_FUNC_NONE,
        proc_getpid(knote_get_kq(kn_max)->kq_p), resident_max, 0, 0);
    VM_PRESSURE_DEBUG(
        1, "[vm_pressure] sending event to pid %d with %llu resident\n",
...
/*
 * To avoid notification storms in a system with sawtooth behavior of pressure
 * levels eg: Normal -> warning (notify clients) -> critical (notify) -> warning
 * (notify) -> critical (notify) -> warning (notify)...
 *
 * We have 'resting' periods: WARNING_NOTIFICATION_RESTING_PERIOD and
 * CRITICAL_NOTIFICATION_RESTING_PERIOD
 *
 * So it would look like:-
 * Normal -> warning (notify) -> critical (notify) -> warning (notify if it has
 * been RestPeriod since last warning) -> critical (notify if it has been
 * RestPeriod since last critical) -> ...
 *
...
TUNABLE_DEV_WRITEABLE(unsigned int, memstat_sustained_pressure_max_pri,
                      "memstat_sustained_pressure_max_pri",
                      JETSAM_PRIORITY_IDLE);
#if DEVELOPMENT || DEBUG
SYSCTL_UINT(_kern_memorystatus, OID_AUTO, sustained_pressure_max_pri,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
            &memstat_sustained_pressure_max_pri, 0, "");
#endif /* DEVELOPMENT || DEBUG */

#if CONFIG_JETSAM
#define MEMSTAT_PRESSURE_CONFIG_DEFAULT (MEMSTAT_WARNING_KILL_SUSTAINED)
#else
#define MEMSTAT_PRESSURE_CONFIG_DEFAULT                                        \
  (MEMSTAT_WARNING_KILL_IDLE_THROTTLED | MEMSTAT_CRITICAL_PURGE_CACHES)
#endif

TUNABLE_WRITEABLE(memstat_pressure_options_t, memstat_pressure_config,
                  "memorystatus_pressure_config",
                  MEMSTAT_PRESSURE_CONFIG_DEFAULT);
EXPERIMENT_FACTOR_UINT(
    memorystatus_pressure_config, &memstat_pressure_config, 0,
    MEMSTAT_PRESSURE_CONFIG_MAX,
...
static int sysctl_memstat_should_kill_sustained SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_should_kill_on_sustained_pressure,
            ...
            sysctl_memstat_should_kill_sustained, "IU",
            "Whether to kill idle processes under sustained pressure");
#endif

/*
 * TODO(jason): The memorystatus thread should be responsible for this
 * It can just check how long the pressure level has been at warning and the
 * timestamp of the last sustained pressure kill.
 */
static void sustained_pressure_handler(void *arg0 __unused,
                                       void *arg1 __unused) {
   ...
   * consumption in a higher band and this churn is probably doing more harm
   * than good.
   */
  max_kills =
      memstat_get_proccnt_upto_priority(memstat_sustained_pressure_max_pri) * 2;
  memorystatus_log("memorystatus: Pressure level has been elevated for too "
                   "long. killing up to %d idle processes\n",
                   max_kills);
  while (memorystatus_vm_pressure_level != kVMPressureNormal &&
    ...
    if (killed) {
       ...
       */
      delay((int)(memorystatus_kill_on_sustained_pressure_delay_ms *
  ...
  if (memorystatus_vm_pressure_level != kVMPressureNormal) {
    memorystatus_log("memorystatus: Killed %d idle processes due to sustained "
                     "pressure, but device didn't quiesce. Giving up.\n",
                     kill_count);
  }
}
 ...
 */
static size_t memorystatus_klist_length(int level) {
 ...
 */
static void memstat_pressure_interval_start(uint64_t curr_ts) {
  LCK_MTX_ASSERT(&memorystatus_klist_mutex, LCK_MTX_ASSERT_OWNED);
 ...
 */
static void memstat_pressure_interval_end(void) {
  LCK_MTX_ASSERT(&memorystatus_klist_mutex, LCK_MTX_ASSERT_OWNED);
 ...
 */
static void memstat_pressure_interval_update(vm_pressure_level_t new_level) {
  LCK_MTX_ASSERT(&memorystatus_klist_mutex, LCK_MTX_ASSERT_OWNED);
 ...
 */
static void update_footprints_for_telemetry(void *arg0 __unused,
                                            void *arg1 __unused) {
  ...
  if (next_telemetry_update != UINT64_MAX) {
    ...
    thread_call_enter_delayed(memorystatus_notify_update_telemetry_thread_call,
                              next_telemetry_update);
  }
}
...
#define MAX_IDLE_KILLS 100 /* limit the number of idle kills allowed */
  ...
  if (level_snapshot != kVMPressureNormal) {
    ...
    if (level_snapshot == kVMPressureWarning ||
        level_snapshot == kVMPressureUrgent) {
  ...
  if (memstat_pressure_config & MEMSTAT_WARNING_KILL_SUSTAINED) {
    if (memorystatus_vm_pressure_level == kVMPressureNormal &&
        prev_level_snapshot != kVMPressureNormal) {
      memorystatus_log("memorystatus: Pressure has returned to level %d. "
                       "Cancelling scheduled jetsam\n",
                       memorystatus_vm_pressure_level);
      thread_call_cancel(sustained_pressure_handler_thread_call);
    } else if (memorystatus_vm_pressure_level != kVMPressureNormal &&
               prev_level_snapshot == kVMPressureNormal) {
      /*
       ...
       */
      memorystatus_log("memorystatus: Pressure level has increased from %d to "
                       "%d. Scheduling jetsam.\n",
                       prev_level_snapshot, memorystatus_vm_pressure_level);
      uint64_t kill_time;
      nanoseconds_to_absolutetime(
          memorystatus_kill_on_sustained_pressure_window_s * NSEC_PER_SEC,
          &kill_time);
      kill_time += mach_absolute_time();
      thread_call_enter_delayed(sustained_pressure_handler_thread_call,
                                kill_time);
    }
  }
  ...
  while (1) {
    ...
    if (memstat_pressure_config & MEMSTAT_WARNING_KILL_LONG_IDLE &&
      ...
      if (memstat_pressure_config & MEMSTAT_CRITICAL_PURGE_CACHES) {
        ...
        uint64_t delta_ns;
        absolutetime_to_nanoseconds(now - memstat_last_cache_purge_ts,
                                    &delta_ns);
    ...
    kn_max = vm_pressure_select_optimal_candidate_to_notify(
        &memorystatus_klist, level_snapshot, target_foreground_process,
        &next_telemetry_update);
    ...
    if (kn_max == NULL) {
       ...
       *
       * Start the 'resting' window within which clients will not be
       * re-notified.
       */
      ...
      if (level_snapshot != kVMPressureNormal) {
        if (level_snapshot == kVMPressureWarning ||
            level_snapshot == kVMPressureUrgent) {
          nanoseconds_to_absolutetime(
              WARNING_NOTIFICATION_RESTING_PERIOD * NSEC_PER_SEC, &curr_ts);

          /* Next warning notification (if nothing changes) won't be sent
           * before...*/
        ...
        if (level_snapshot == kVMPressureCritical) {
          nanoseconds_to_absolutetime(
              CRITICAL_NOTIFICATION_RESTING_PERIOD * NSEC_PER_SEC, &curr_ts);

          /* Next critical notification (if nothing changes) won't be sent
           * before...*/
    ...
    if (level_snapshot != kVMPressureNormal) {
      if (level_snapshot == kVMPressureWarning ||
          level_snapshot == kVMPressureUrgent) {
        if (is_knote_registered_modify_task_pressure_bits(
                kn_max, NOTE_MEMORYSTATUS_PRESSURE_WARN, task, 0,
                kVMPressureWarning) == TRUE) {
          found_candidate = TRUE;
        }
      } else {
        if (level_snapshot == kVMPressureCritical) {
          if (is_knote_registered_modify_task_pressure_bits(
                  kn_max, NOTE_MEMORYSTATUS_PRESSURE_CRITICAL, task, 0,
                  kVMPressureCritical) == TRUE) {
            found_candidate = TRUE;
          }
        }
      }
    ...
    SLIST_FOREACH_SAFE(kn_cur, &memorystatus_klist, kn_selnext, kn_temp) {
      ...
      if (is_knote_registered_modify_task_pressure_bits(
              kn_cur, knote_pressure_level, task, 0, level_snapshot) == TRUE) {
    ...
    if (level_snapshot != kVMPressureNormal) {
      ...
      if (os_convert_overflow(memstat_cur_interval.num_notifs,
                              &num_notifications)) {
        num_notifications = UINT16_MAX;
      }
      mark_knote_send_time(
          kn_max, task,
          convert_internal_pressure_level_to_dispatch_level(level_snapshot),
          num_notifications);
    ...
    KNOTE(&dispatch_klist, (level_snapshot != kVMPressureNormal)
    ...
    if (memorystatus_manual_testing_on == TRUE &&
        target_foreground_process == TRUE) {
      break;
    }
    ...
    if (memorystatus_manual_testing_on == TRUE) {
      ...
      uint32_t backoff_threshold =
          soft_threshold - ((soft_threshold - critical_threshold) / 2);
...
static uint32_t convert_internal_pressure_level_to_dispatch_level(
    vm_pressure_level_t internal_pressure_level) {
...
void memorystatus_broadcast_jetsam_pressure(
  ...
  if (*waiters > 0) {
    memorystatus_log(
        ...
        pressure_level == kVMPressureForegroundJetsam ? "foreground"
                                                      : "background",
        *waiters);
...
static int sysctl_memorystatus_broadcast_jetsam_pressure SYSCTL_HANDLER_ARGS {
  int error = 0;
  vm_pressure_level_t pressure_level;
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_broadcast_jetsam_pressure,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_MASKED | CTLFLAG_LOCKED, 0, 0,
            &sysctl_memorystatus_broadcast_jetsam_pressure, "I", "");

#endif /* DEVELOPMENT || DEBUG */

static int sysctl_memorystatus_vm_pressure_level SYSCTL_HANDLER_ARGS {
...
#endif /* !XNU_TARGET_OS_OSX */
  uint32_t dispatch_level = convert_internal_pressure_level_to_dispatch_level(
      memorystatus_vm_pressure_level);
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_vm_pressure_level,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            &sysctl_memorystatus_vm_pressure_level, "I", "");
...
static int sysctl_memorypressure_manual_trigger SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern, OID_AUTO, memorypressure_manual_trigger,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED | CTLFLAG_MASKED, 0, 0,
            &sysctl_memorypressure_manual_trigger, "I", "");

SYSCTL_INT(_kern, OID_AUTO, memorystatus_purge_on_warning,
           CTLFLAG_RW | CTLFLAG_LOCKED,
           &vm_pageout_state.memorystatus_purge_on_warning, 0, "");
SYSCTL_INT(_kern, OID_AUTO, memorystatus_purge_on_urgent,
           CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
           &vm_pageout_state.memorystatus_purge_on_urgent, 0, "");
SYSCTL_INT(_kern, OID_AUTO, memorystatus_purge_on_critical,
           CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
           &vm_pageout_state.memorystatus_purge_on_critical, 0, "");

extern int vm_pressure_level_transition_threshold;
SYSCTL_INT(_kern, OID_AUTO, vm_pressure_level_transition_threshold,
           CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
           &vm_pressure_level_transition_threshold, 0, "");

#if DEBUG || DEVELOPMENT
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_vm_pressure_events_enabled,
            CTLFLAG_RW | CTLFLAG_LOCKED, &vm_pressure_events_enabled, 0, "");
...
/*
 * This routine is used for targeted notifications regardless of system memory
 * pressure and regardless of whether or not the process has already been
 * notified. It bypasses and has no effect on the only-one-notification per
 * soft-limit policy.
 *
 * "memnote" is the current user.
 */

static int sysctl_memorystatus_vm_pressure_send SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  ...
  if (fflags == 0) {
    ...
    // printf("memorystatus_vm_pressure_send: using default notification
    // [0x%x]\n", fflags);
  }
  ...
  if (!((fflags == NOTE_MEMORYSTATUS_PRESSURE_NORMAL) ||
    ...
    memorystatus_log_error(
        "memorystatus_vm_pressure_send: notification [0x%x] not supported\n",
        fflags);
    error = 1;
    return error;
  }
  ...
  if (found_knote) {
    KNOTE(&memorystatus_klist, 0);
    memorystatus_log_debug("memorystatus_vm_pressure_send: (value 0x%llx) "
                           "notification [0x%x] sent to process [%d]\n",
                           value, fflags, pid);
    error = 0;
  } else {
    memorystatus_log_error(
        "memorystatus_vm_pressure_send: (value 0x%llx) notification [0x%x] not "
        "sent to process [%d] (none registered?)\n",
        value, fflags, pid);
    error = 1;
  }
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_vm_pressure_send,
