...
static int handle_lowresource(int scope, int action, int policy,
                              int policy_subtype, user_addr_t attrp,
                              proc_t proc, uint64_t target_threadid);
static int handle_cpuuse(int action, user_addr_t attrp, proc_t proc,
                         uint64_t target_threadid);
static int handle_apptype(int scope, int action, int policy, int policy_subtype,
                          user_addr_t attrp, proc_t proc,
                          uint64_t target_threadid);
static int handle_boost(int scope, int action, int policy, int policy_subtype,
                        user_addr_t attrp, proc_t proc,
                        uint64_t target_threadid);
static int handle_no_smt(int scope, int action, proc_t target_proc,
                         uint64_t target_threadid);
static int handle_tecs(int scope, int action, proc_t target_proc,
                       uint64_t target_threadid);
...
#if !defined(XNU_TARGET_OS_OSX)
static int handle_applifecycle(int scope, int action, int policy,
                               int policy_subtype, user_addr_t attrp,
                               proc_t proc, uint64_t target_threadid);
#endif /* !defined(XNU_TARGET_OS_OSX) */
...
int process_policy(__unused struct proc *p, struct process_policy_args *uap,
  ...
  if ((scope != PROC_POLICY_SCOPE_PROCESS) &&
      (scope != PROC_POLICY_SCOPE_THREAD)) {
    return EINVAL;
  }
...
#else
  /*
   * Resoure starvation control can be used by unpriv resource owner but priv at
   * the time of ownership claim. This is checked in low resource handle
   * routine. So bypass the checks here.
   */
  ...
  case PROC_POLICY_RESOURCE_STARVATION:
    error = handle_lowresource(scope, action, policy, policy_subtype, attrp,
                               target_proc, target_threadid);
    break;
  case PROC_POLICY_RESOURCE_USAGE:
...
#if !defined(XNU_TARGET_OS_OSX)
  case PROC_POLICY_APP_LIFECYCLE:
    error = handle_applifecycle(scope, action, policy, policy_subtype, attrp,
                                target_proc, target_threadid);
    break;
#endif /* !defined(XNU_TARGET_OS_OSX) */
  case PROC_POLICY_APPTYPE:
    error = handle_apptype(scope, action, policy, policy_subtype, attrp,
                           target_proc, target_threadid);
    break;
  case PROC_POLICY_BOOST:
    error = handle_boost(scope, action, policy, policy_subtype, attrp,
                         target_proc, target_threadid);
    break;
...
static int handle_lowresource(__unused int scope, int action,
                              __unused int policy, int policy_subtype,
                              __unused user_addr_t attrp, proc_t proc,
...
static int handle_cpuuse(int action, user_addr_t attrp, proc_t proc,
  ...
  if ((action == PROC_POLICY_ACTION_APPLY ||
       action == PROC_POLICY_ACTION_RESTORE) &&
      curp != proc) {
    return EPERM;
  }
  ...
  if (!privileged) {
    privileged =
        (priv_check_cred(kauth_cred_get(), PRIV_PROC_CPUMON_OVERRIDE, 0) == 0);
  }
  ...
  case PROC_POLICY_ACTION_GET:
    error = proc_get_task_ruse_cpu(
        proc_task(proc), &cpuattr.ppattr_cpu_attr, &percentage,
        &cpuattr.ppattr_cpu_attr_interval, &cpuattr.ppattr_cpu_attr_deadline);
    if (error == 0) {
      cpuattr.ppattr_cpu_percentage = percentage;
      cpuattr.ppattr_cpu_attr_interval /= NSEC_PER_SEC;
      error = copyout((proc_policy_cpuusage_attr_t *)&cpuattr,
                      (user_addr_t)attrp, sizeof(proc_policy_cpuusage_attr_t));
    }
    break;

  case PROC_POLICY_ACTION_APPLY:
  case PROC_POLICY_ACTION_SET:
    error = copyin((user_addr_t)attrp, (proc_policy_cpuusage_attr_t *)&cpuattr,
    ...
    error = proc_set_task_ruse_cpu(
        proc_task(proc), (uint16_t)cpuattr.ppattr_cpu_attr,
        (uint8_t)MIN(cpuattr.ppattr_cpu_percentage, UINT8_MAX), interval,
        cpuattr.ppattr_cpu_attr_deadline, privileged);
    break;
...
#if !defined(XNU_TARGET_OS_OSX)
static int handle_applifecycle(__unused int scope, int action,
                               __unused int policy, int policy_subtype,
                               user_addr_t attrp, proc_t proc,
                               __unused uint64_t target_threadid) {
  int error = 0;
...
#if CONFIG_MACF
    /* ToDo - this should be a generic check, since we could potentially hang
  ...
  case PROC_POLICY_APPLIFE_PIDBIND:
    ...
    if (action == PROC_POLICY_ACTION_APPLY) {
      ...
      error = proc_lf_pidbind(current_task(), target_threadid, proc_task(proc),
                              state);
    } else
...
static int handle_apptype(int scope, int action, __unused int policy,
                          int policy_subtype, __unused user_addr_t attrp,
                          proc_t target_proc,
                          __unused uint64_t target_threadid) {
  int error = 0;
  ...
  /* Temporary compatibility with old importance donation interface until
   * libproc is moved to new boost calls */
  switch (policy_subtype) {
...
static int handle_boost(int scope, int action, __unused int policy,
                        int policy_subtype, __unused user_addr_t attrp,
                        proc_t target_proc, __unused uint64_t target_threadid) {
  int error = 0;
...
static int handle_no_smt(__unused int scope, __unused int action,
                         __unused proc_t target_proc,
                         __unused uint64_t target_threadid) {
  return 0;
}
 ...
 */
int proc_pidbackgrounded(pid_t pid, uint32_t *state) {
  proc_t target_proc = PROC_NULL;
  ...
  if (proc_get_effective_task_policy(proc_task(target_proc),
 ...
 */
int proc_get_originatorbgstate(uint32_t *is_backgrounded) {
  uint32_t bgstate;
...
int proc_apply_resource_actions(void *bsdinfo, __unused int type, int action) {
...
int proc_restore_resource_actions(void *bsdinfo, __unused int type,
                                  int action) {
  proc_t p = (proc_t)bsdinfo;
