...
#define K_UUID_ARG(_u)                                                         \
  &_u.g_guid_asint[0], &_u.g_guid_asint[1], &_u.g_guid_asint[2],               \
      &_u.g_guid_asint[3]
#define KAUTH_DEBUG(fmt, args...)                                              \
  do {                                                                         \
...
TAILQ_HEAD(kauth_resolver_unsubmitted_head, kauth_resolver_work)
kauth_resolver_unsubmitted = TAILQ_HEAD_INITIALIZER(kauth_resolver_unsubmitted);
TAILQ_HEAD(kauth_resolver_submitted_head, kauth_resolver_work)
kauth_resolver_submitted = TAILQ_HEAD_INITIALIZER(kauth_resolver_submitted);
TAILQ_HEAD(kauth_resolver_done_head, kauth_resolver_work)
kauth_resolver_done = TAILQ_HEAD_INITIALIZER(kauth_resolver_done);
...
#define KAUTH_CACHES_MAX_SIZE                                                  \
  10000 /* Max # entries for both groups and id caches */
...
static TAILQ_HEAD(kauth_identity_head, kauth_identity)
    kauth_identities = TAILQ_HEAD_INITIALIZER(kauth_identities);
...
kauth_identity_alloc(uid_t uid, gid_t gid, guid_t *guidp, time_t guid_expiry,
                     ntsid_t *ntsidp, time_t ntsid_expiry, size_t supgrpcnt,
                     gid_t *supgrps, time_t groups_expiry, const char *name,
                     int nametype);
static void kauth_identity_register_and_free(struct kauth_identity *kip);
static void kauth_identity_updatecache(struct kauth_identity_extlookup *elp,
...
static int kauth_identity_find_ntsid(ntsid_t *ntsid, struct kauth_identity *kir,
                                     char *getname);
static int kauth_identity_find_nam(char *name, int valid,
                                   struct kauth_identity *kir);
...
kauth_groups = TAILQ_HEAD_INITIALIZER(kauth_groups);
...
__KERNEL_IS_WAITING_ON_EXTERNAL_CREDENTIAL_RESOLVER__(
  ...
  for (;;) {
    ...
    if (kauth_resolver_identity == 0) {
      RESOLVER_FAILED_MESSAGE("kauth external resolver died while while "
 ...
 * Returns:     Nothing.
 */
void kauth_resolver_identity_reset(void) {
  KAUTH_RESOLVER_LOCK();
  if (kauth_resolver_identity != 0) {
...
static int kauth_resolver_submit(struct kauth_identity_extlookup *lkp,
  ...
  if (error == EWOULDBLOCK) {
    ...
    if (workp->kr_flags & KAUTH_REQUEST_UNSUBMITTED) {
       ...
       */
      KAUTH_DEBUG("RESOLVER - request timed out without being collected for "
                  "processing, resolver dead");
 ...
 */
int identitysvc(__unused struct proc *p, struct identitysvc_args *uap,
  ...
  if (!IOCurrentTaskHasEntitlement(IDENTITYSVC_ENTITLEMENT)) {
    KAUTH_DEBUG("RESOLVER - pid %d not entitled to call identitysvc",
  ...
  if (opcode == KAUTH_EXTLOOKUP_REGISTER) {
    ...
    if ((error = kauth_authorize_generic(kauth_cred_get(),
                                         KAUTH_GENERIC_ISSUSER)) != 0) {
      KAUTH_DEBUG(
          "RESOLVER - pid %d refused permission to become identity resolver",
          new_id);
      return error;
    }
    KAUTH_RESOLVER_LOCK();
    if (kauth_resolver_identity != new_id) {
      KAUTH_DEBUG("RESOLVER - new resolver %d taking over from old %d", new_id,
                  kauth_resolver_identity);
      /*
       * We have a new server, so assume that all the old requests have been
       * lost.
       */
      while ((workp = TAILQ_LAST(&kauth_resolver_submitted,
                                 kauth_resolver_submitted_head)) != NULL) {
        TAILQ_REMOVE(&kauth_resolver_submitted, workp, kr_link);
   ...
   */
  if ((kauth_cred_getuid(kauth_cred_get()) != 0) ||
      (proc_getpid(current_proc()) != kauth_resolver_identity)) {
 ...
 *		more information.
 */
static int kauth_resolver_getwork_continue(int result) {
  thread_t thread;
  struct uthread *ut;
  user_addr_t message;
  ...
  if (TAILQ_FIRST(&kauth_resolver_unsubmitted) == NULL) {
    ...
    error = msleep0(&kauth_resolver_unsubmitted, &kauth_resolver_mtx, PCATCH,
...
static int kauth_resolver_getwork2(user_addr_t message) {
   ...
   */
  /* BEFORE FIELD */
  if ((error = copyout(&workp->kr_work, message,
                       offsetof(struct kauth_identity_extlookup, el_extend))) !=
      0) {
   ...
   */
  if (workp->kr_work.el_flags &
      (KAUTH_EXTLOOKUP_VALID_PWNAM | KAUTH_EXTLOOKUP_VALID_GRNAM)) {
    uint64_t uaddr;

    error =
        copyin(message + offsetof(struct kauth_identity_extlookup, el_extend),
               &uaddr, sizeof(uaddr));
    if (!error) {
       ...
       */
      error = copyoutstr(CAST_DOWN(void *, workp->kr_extend), uaddr, MAXPATHLEN,
                         &actual);
    }
    if (error) {
      KAUTH_DEBUG("RESOLVER - error submitting work to resolve");
...
static int kauth_resolver_complete(user_addr_t message) {
  ...
  case KAUTH_EXTLOOKUP_INPROG: {
    ...
    if (!once) {
      printf("kauth_resolver: memberd is not setting valid result codes "
  ...
  case KAUTH_EXTLOOKUP_FATAL:
    ...
    KAUTH_DEBUG("RESOLVER - resolver %d died, waiting for a new one",
                kauth_resolver_identity);
    RESOLVER_FAILED_MESSAGE("resolver %d died, waiting for a new one",
                            kauth_resolver_identity);
    /*
     * Terminate outstanding requests; without an authoritative
  ...
  case KAUTH_EXTLOOKUP_BADRQ:
    KAUTH_DEBUG("RESOLVER - resolver reported invalid request %d",
                extl.el_seqno);
    result = EINVAL;
    break;

  case KAUTH_EXTLOOKUP_FAILURE:
    KAUTH_DEBUG("RESOLVER - resolver reported transient failure for request %d",
                extl.el_seqno);
  ...
  default:
    KAUTH_DEBUG("RESOLVER - resolver returned unexpected status %d",
                extl.el_result);
    RESOLVER_FAILED_MESSAGE("resolver returned unexpected status %d",
                            extl.el_result);
    result = KAUTH_RESOLVER_FAILED_ERRCODE;
    break;
  }
  ...
  if (extl.el_result != KAUTH_EXTLOOKUP_FATAL) {
    ...
    TAILQ_FOREACH(workp, &kauth_resolver_submitted, kr_link) {
      ...
      if (workp->kr_seqno == extl.el_seqno) {
         ...
         */
        want_extend_data =
            (workp->kr_work.el_flags &
             (KAUTH_EXTLOOKUP_WANT_PWNAM | KAUTH_EXTLOOKUP_WANT_GRNAM));
        ...
        if (want_extend_data &&
          ...
          KAUTH_RESOLVER_UNLOCK();
          error = copyinstr(extl.el_extend, CAST_DOWN(void *, workp->kr_extend),
                            MAXPATHLEN, &actual);
          KAUTH_DEBUG("RESOLVER - resolver got name :%*s: len = %d\n",
                      (int)actual, actual ? "null" : (char *)extl.el_extend,
                      actual);
          KAUTH_RESOLVER_LOCK();
        } else if (extl.el_flags & (KAUTH_EXTLOOKUP_VALID_PWNAM |
                                    KAUTH_EXTLOOKUP_VALID_GRNAM)) {
          error = EFAULT;
          KAUTH_DEBUG("RESOLVER - resolver returned mismatching extension "
                      "flags (%d), request contained (%d)",
...
kauth_identity_alloc(uid_t uid, gid_t gid, guid_t *guidp, time_t guid_expiry,
  ...
  if (supgrpcnt) {
    ...
    if (kip->ki_valid & KI_VALID_GID) {
      panic("can't allocate kauth identity with both gid and supplementary "
            "groups");
    }
...
static void kauth_identity_updatecache(struct kauth_identity_extlookup *elp,
  ...
  if (elp->el_flags &
    ...
    speculative_name =
        vfs_addname(tmp, (uint32_t)strnlen(tmp, MAXPATHLEN - 1), 0, 0);
  }
  ...
  if (elp->el_flags & KAUTH_EXTLOOKUP_VALID_UID) {
      ...
      if ((kip->ki_valid & KI_VALID_UID) && (kip->ki_uid == elp->el_uid)) {
        if (elp->el_flags & KAUTH_EXTLOOKUP_VALID_SUPGRPS) {
          ...
          if (elp->el_sup_grp_cnt > NGROUPS) {
            KAUTH_DEBUG(
                "CACHE - invalid sup_grp_cnt provided (%d), truncating to  %d",
                elp->el_sup_grp_cnt, NGROUPS);
            elp->el_sup_grp_cnt = NGROUPS;
          }
          kip->ki_supgrpcnt = elp->el_sup_grp_cnt;
          memcpy(kip->ki_supgrps, elp->el_sup_groups,
                 sizeof(elp->el_sup_groups[0]) * kip->ki_supgrpcnt);
          kip->ki_valid |= KI_VALID_GROUPS;
          kip->ki_groups_expiry =
        ...
        KAUTH_DEBUG("CACHE - refreshed %d is " K_UUID_FMT, kip->ki_uid,
                    K_UUID_ARG(kip->ki_guid));
        break;
      }
    }
    KAUTH_IDENTITY_UNLOCK();
    ...
    if (kip == NULL) {
      kip = kauth_identity_alloc(
          elp->el_uid, KAUTH_GID_NONE,
          (elp->el_flags & KAUTH_EXTLOOKUP_VALID_UGUID) ? &elp->el_uguid : NULL,
          (elp->el_uguid_valid) ? tv.tv_sec + elp->el_uguid_valid : 0,
          (elp->el_flags & KAUTH_EXTLOOKUP_VALID_USID) ? &elp->el_usid : NULL,
          (elp->el_usid_valid) ? tv.tv_sec + elp->el_usid_valid : 0,
          (elp->el_flags & KAUTH_EXTLOOKUP_VALID_SUPGRPS) ? elp->el_sup_grp_cnt
                                                          : 0,
          (elp->el_flags & KAUTH_EXTLOOKUP_VALID_SUPGRPS) ? elp->el_sup_groups
                                                          : NULL,
          (elp->el_member_valid) ? tv.tv_sec + elp->el_member_valid : 0,
          (elp->el_flags & KAUTH_EXTLOOKUP_VALID_PWNAM) ? speculative_name
                                                        : NULL,
          KI_VALID_PWNAM);
      if (kip != NULL) {
        if (rkip != NULL) {
        ...
        KAUTH_DEBUG("CACHE - learned %d is " K_UUID_FMT, kip->ki_uid,
                    K_UUID_ARG(kip->ki_guid));
  ...
  if (elp->el_flags & KAUTH_EXTLOOKUP_VALID_GID &&
      !(elp->el_flags & KAUTH_EXTLOOKUP_VALID_UID)) {
    KAUTH_IDENTITY_LOCK();
    TAILQ_FOREACH(kip, &kauth_identities, ki_link) {
      /* matching record */
    ...
    if (kip == NULL) {
      kip = kauth_identity_alloc(
          ...
          (elp->el_flags & KAUTH_EXTLOOKUP_VALID_SUPGRPS) ? elp->el_sup_groups
                                                          : NULL,
          (elp->el_member_valid) ? tv.tv_sec + elp->el_member_valid : 0,
          (elp->el_flags & KAUTH_EXTLOOKUP_VALID_GRNAM) ? speculative_name
                                                        : NULL,
          KI_VALID_GRNAM);
      if (kip != NULL) {
        if (rkip != NULL) {
...
static int kauth_identity_guid_expired(struct kauth_identity *kip) {
  ...
  microuptime(&tv);
  KAUTH_DEBUG("CACHE - GUID expires @ %ld now %ld", kip->ki_guid_expiry,
              tv.tv_sec);
...
static int kauth_identity_ntsid_expired(struct kauth_identity *kip) {
  ...
  microuptime(&tv);
  KAUTH_DEBUG("CACHE - NTSID expires @ %ld now %ld", kip->ki_ntsid_expiry,
              tv.tv_sec);
 ...
 *		0				Not expired
 */
static int kauth_identity_groups_expired(struct kauth_identity *kip) {
  struct timeval tv;
  ...
  TAILQ_FOREACH(kip, &kauth_identities, ki_link) {
    if ((kip->ki_valid & KI_VALID_UID) && (uid == kip->ki_uid)) {
      ...
      *kir = *kip;
      ...
      if (getname != NULL &&
          (kip->ki_valid & (KI_VALID_PWNAM | KI_VALID_GRNAM))) {
 ...
 *		*klr				Modified, if found
 */
static int kauth_identity_find_gid(uid_t gid, struct kauth_identity *kir,
                                   char *getname) {
  struct kauth_identity *kip;
...
static int kauth_identity_find_guid(guid_t *guidp, struct kauth_identity *kir,
  ...
  TAILQ_FOREACH(kip, &kauth_identities, ki_link) {
    if ((kip->ki_valid & KI_VALID_GUID) &&
        (kauth_guid_equal(guidp, &kip->ki_guid))) {
      kauth_identity_lru(kip);
      /* Copy via structure assignment */
      *kir = *kip;
      /* If a name is wanted and one exists, copy it out */
...
static int kauth_identity_find_ntsid(ntsid_t *ntsid, struct kauth_identity *kir,
  ...
  TAILQ_FOREACH(kip, &kauth_identities, ki_link) {
    if ((kip->ki_valid & KI_VALID_NTSID) &&
        (kauth_ntsid_equal(ntsid, &kip->ki_ntsid))) {
      kauth_identity_lru(kip);
      /* Copy via structure assignment */
      *kir = *kip;
      /* If a name is wanted and one exists, copy it out */
 ...
 */
int kauth_guid_equal(guid_t *guid1, guid_t *guid2) {
...
int kauth_wellknown_guid(guid_t *guid) {
  static char fingerprint[] = {0xab, 0xcd, 0xef, 0xab, 0xcd, 0xef,
 ...
 *		!0				If GUIDs are equal
 */
int kauth_ntsid_equal(ntsid_t *sid1, ntsid_t *sid2) {
 ...
 */
static int kauth_cred_change_egid(kauth_cred_t cred, gid_t new_egid) {
  int i;
  int displaced = 1;
#if radar_4600026
  int is_member;
...
#error Fix radar 4600026 first!!!
   ...
   *  this, we would need to automatically opt-out any SUID/SGID binary, and
   * force it to use initgroups to opt back in.  We take the approach of
   * considering it opt'ed out in any group of 16 displacement instead, since
   * it's a much more conservative approach (i.e. less likely to cause things to
   * break).
   */
  ...
  if (displaced && !(pcred->cr_flags & CRF_NOMEMBERD) &&
      kauth_cred_ismember_gid(cred, new_egid, &is_member) == 0 && is_member) {
    displaced = 0;
  }
#endif /* radar_4600026 */
...
__mockable uid_t kauth_cred_getuid(kauth_cred_t cred) {
  return posix_cred_get(cred)->cr_uid;
}

uid_t kauth_cred_getruid(kauth_cred_t cred) {
...
gid_t kauth_cred_getrgid(kauth_cred_t cred) {
...
/*
 * If there's no resolver, only support a subset of the kauth_cred_x2y()
 * lookups.
 */
static __inline int kauth_cred_cache_lookup(int from, int to, void *src,
                                            void *dst) {
  /* NB: These must match the definitions used by Libinfo's
   * mbr_identifier_translate(). */
  static const uuid_t _user_compat_prefix = {0xff, 0xff, 0xee, 0xee, 0xdd, 0xdd,
                                             0xcc, 0xcc, 0xbb, 0xbb, 0xaa, 0xaa,
                                             0x00, 0x00, 0x00, 0x00};
  static const uuid_t _group_compat_prefix = {
      0xab, 0xcd, 0xef, 0xab, 0xcd, 0xef, 0xab, 0xcd,
      0xef, 0xab, 0xcd, 0xef, 0x00, 0x00, 0x00, 0x00};
 ...
 *		gcount				pointer to the number of groups
 * wanted/returned
 *
 * Returns:	0				Success
 ...
 *
 */
static int kauth_cred_uid2groups(uid_t *uid, gid_t *groups, size_t *gcount) {
  int rv;
 ...
 */
int kauth_cred_guid2grnam(guid_t *guidp, char *grnam) {
 ...
 */
int kauth_cred_guid2uid(guid_t *guidp, uid_t *uidp) {
 ...
 */
int kauth_cred_guid2gid(guid_t *guidp, gid_t *gidp) {
 ...
 *
 * Returns:	0				Success
 *		ENOENT				For now just a stub that always
 * fails
 *
 * Implicit returns:
 *		*dsnode				Modified, if successuful
 */
int kauth_cred_nfs4domain2dsnode(__unused char *nfs4domain,
                                 __unused char *dsnode) {
  return ENOENT;
}
 ...
 */
int kauth_cred_dsnode2nfs4domain(__unused char *dsnode,
                                 __unused char *nfs4domain) {
  return ENOENT;
}
 ...
 *		*uidp				Modified, if successful
 */
int kauth_cred_ntsid2uid(ntsid_t *sidp, uid_t *uidp) {
 ...
 *		*gidp				Modified, if successful
 */
int kauth_cred_ntsid2gid(ntsid_t *sidp, gid_t *gidp) {
 ...
 *		*guidp				Modified, if successful
 */
int kauth_cred_ntsid2guid(ntsid_t *sidp, guid_t *guidp) {
 ...
 *		*guidp				Modified, if successful
 */
int kauth_cred_uid2guid(uid_t uid, guid_t *guidp) {
 ...
 *		*guidp				Modified, if successful
 */
int kauth_cred_getguid(kauth_cred_t cred, guid_t *guidp) {
 ...
 *		*guidp				Modified, if successful
 */
int kauth_cred_gid2guid(gid_t gid, guid_t *guidp) {
 ...
 */
int kauth_cred_uid2ntsid(uid_t uid, ntsid_t *sidp) {
 ...
 *		*sidp				Modified, if successful
 */
int kauth_cred_getntsid(kauth_cred_t cred, ntsid_t *sidp) {
 ...
 *		*sidp				Modified, if successful
 */
int kauth_cred_gid2ntsid(gid_t gid, ntsid_t *sidp) {
 ...
 *		*sidp				Modified, if successful
 */
int kauth_cred_guid2ntsid(guid_t *guidp, ntsid_t *sidp) {
...
static int kauth_cred_cache_lookup(int from, int to, void *src, void *dst) {
  ...
  if (error != 0) {
    ...
    if (!expired) {
      ...
      if (ki.ki_valid & to) {
        KAUTH_DEBUG("CACHE - found matching entry with valid 0x%08x",
                    ki.ki_valid);
        DTRACE_PROC4(kauth__identity__cache__hit, int, from, int, to, void *,
                     src, void *, dst);
        goto found;
      } else {
        /*
        ...
        case KI_VALID_NTSID:
          ...
          case KI_VALID_GID:
            if ((ki.ki_valid & KI_VALID_UID)) {
              KAUTH_DEBUG("CACHE - unexpected entry 0x%08x & %x", ki.ki_valid,
                          KI_VALID_GID);
              return ENOENT;
            }
            break;
  ...
  if (to == KI_VALID_GROUPS) {
    ...
    if (ki.ki_valid & KI_VALID_GROUPS) {
      ...
      if (ki.ki_supgrpcnt > NGROUPS) {
        panic("kauth data structure corrupted. kauth identity 0x%p with %u "
              "groups, greater than max of %d",
              &ki, ki.ki_supgrpcnt, NGROUPS);
      }

      el.el_sup_grp_cnt = (uint32_t)ki.ki_supgrpcnt;

      memcpy(el.el_sup_groups, ki.ki_supgrps,
             sizeof(el.el_sup_groups[0]) * ki.ki_supgrpcnt);
  ...
  DTRACE_PROC3(kauth__id__resolver__submitted, int, from, int, to, uintptr_t,
               src);

  error = kauth_resolver_submit(&el, extend_data);

  DTRACE_PROC2(kauth__id__resolver__returned, int, error,
               struct kauth_identity_extlookup *, &el)
...
static void kauth_groups_updatecache(struct kauth_identity_extlookup *el) {
  ...
  if ((el->el_flags & (KAUTH_EXTLOOKUP_VALID_UID | KAUTH_EXTLOOKUP_VALID_GID |
                       KAUTH_EXTLOOKUP_VALID_MEMBERSHIP)) !=
      (KAUTH_EXTLOOKUP_VALID_UID | KAUTH_EXTLOOKUP_VALID_GID |
       KAUTH_EXTLOOKUP_VALID_MEMBERSHIP)) {
    return;
  }
  ...
  TAILQ_FOREACH(gm, &kauth_groups, gm_link) {
    if ((el->el_uid == gm->gm_uid) && (el->el_gid == gm->gm_gid)) {
      ...
      gm->gm_expiry =
          (el->el_member_valid) ? el->el_member_valid + tv.tv_sec : 0;
      kauth_groups_lru(gm);
      break;
    }
  }
 ...
 */
static void kauth_groups_trimcache(int new_size) {
  struct kauth_group_membership *gm;
...
int kauth_cred_ismember_gid(kauth_cred_t cred, gid_t gid, int *resultp) {
  ...
  KAUTH_GROUPS_LOCK();
  TAILQ_FOREACH(gm, &kauth_groups, gm_link) {
    if ((gm->gm_uid == pcred->cr_gmuid) && (gm->gm_gid == gid) &&
  ...
  bzero(&el, sizeof(el));
  el.el_info_pid = proc_getpid(current_proc());
  el.el_flags = KAUTH_EXTLOOKUP_VALID_UID | KAUTH_EXTLOOKUP_VALID_GID |
                KAUTH_EXTLOOKUP_WANT_MEMBERSHIP;
  el.el_uid = pcred->cr_gmuid;
  el.el_gid = gid;
  el.el_member_valid = 0; /* XXX set by resolver? */

  DTRACE_PROC2(kauth__group__resolver__submitted, int, el.el_uid, int,
               el.el_gid);
 ...
 *					0	Is not member
 */
int kauth_cred_ismember_guid(__unused kauth_cred_t cred, guid_t *guidp,
                             int *resultp) {
  int error = 0;
 ...
 *		an error.
 */
int kauth_cred_gid_subset(kauth_cred_t cred1, kauth_cred_t cred2,
                          int *resultp) {
  int i, err, res = 1;
  gid_t gid;
 ...
 */
int kauth_cred_issuser(kauth_cred_t cred) {
...
ZONE_DEFINE_ID(ZONE_ID_KAUTH_CRED, "cred", struct ucred,
...
SMRH_TRAITS_DEFINE(kauth_cred_traits, struct ucred_rw, crw_link,
                   .domain = &smr_proc_task, .key_hash = kauth_cred_key_hash,
...
static inline void ucred_rw_ref(struct ucred_rw *rw) {
  os_ref_retain_raw(&rw->crw_weak_ref, &ucred_ref_grp);
}

static inline bool ucred_rw_tryref(struct ucred_rw *rw) {
  return os_ref_retain_try_raw(&rw->crw_weak_ref, &ucred_ref_grp);
}

static inline os_ref_count_t ucred_rw_unref(struct ucred_rw *rw) {
  return os_ref_release_raw(&rw->crw_weak_ref, &ucred_ref_grp);
}

static inline void ucred_rw_unref_live(struct ucred_rw *rw) {
  os_ref_release_live_raw(&rw->crw_weak_ref, &ucred_ref_grp);
}

__abortlike static void kauth_cred_panic_over_released(kauth_cred_t cred) {
  panic("kauth_cred_unref: cred %p over-released", cred);
}

__abortlike static void kauth_cred_panic_over_retain(kauth_cred_t cred) {
  panic("kauth_cred_ref: cred %p over-retained", cred);
}

static void kauth_cred_hold(kauth_cred_t cred) {
  unsigned long ref;

  ref = zalloc_ro_update_field_atomic(ZONE_ID_KAUTH_CRED, cred, cr_ref,
                                      ZRO_ATOMIC_ADD_LONG, 1);
  if (ref >= KAUTH_CRED_REF_MAX) {
    kauth_cred_panic_over_retain(cred);
  }
}

static void kauth_cred_drop(kauth_cred_t cred) {
  unsigned long ref;
 ...
 */
__startup_func static void kauth_cred_init(void) {
...
uid_t kauth_getuid(void) { return kauth_cred_getuid(kauth_cred_get()); }

uid_t kauth_getruid(void) { return kauth_cred_getruid(kauth_cred_get()); }

gid_t kauth_getgid(void) { return kauth_cred_getgid(kauth_cred_get()); }

gid_t kauth_getrgid(void) { return kauth_cred_getrgid(kauth_cred_get()); }

kauth_cred_t kauth_cred_get(void) { return current_thread_ro()->tro_cred; }
...
__abortlike static void current_cached_proc_cred_panic(proc_t p) {
  panic("current_cached_proc_cred(%p) called but current_proc() is %p", p,
        current_proc());
}

kauth_cred_t current_cached_proc_cred(proc_t p) {
...
intptr_t current_cached_proc_label_get(int slot) {
#if CONFIG_MACF
...
kauth_cred_t current_cached_proc_cred_ref(proc_t p) {
...
kauth_cred_thread_update_slow(thread_ro_t tro, proc_t proc) {
  ...
  if (my_creds.tro_realcred != cred) {
    ...
    zalloc_ro_update_field(ZONE_ID_THREAD_RO, tro, tro_creds, &my_creds);
  }
 ...
 */
__attribute__((always_inline)) void current_cached_proc_cred_update(void) {
  thread_ro_t tro = current_thread_ro();
...
kauth_cred_t kauth_cred_get_with_ref(void) {
  struct ucred *ucred = kauth_cred_get();
  kauth_cred_ref(ucred);
  return ucred;
}

__abortlike static void kauth_cred_verify_panic(kauth_cred_t cred,
                                                struct ucred_rw *cred_rw) {
  panic("kauth_cred_t backref mismatch: cred:%p cred->cr_rw:%p "
        "cred_rw:%p",
        cred, cred->cr_rw, cred_rw);
}

__pure2 static struct ucred_rw *kauth_cred_rw(kauth_cred_t cred) {
...
static kauth_cred_t __kauth_cred_proc_ref_for_pidversion_slow(pid_t pid,
                                                              uint32_t pidvers,
                                                              bool dovers) {
...
kauth_cred_t kauth_cred_proc_ref_for_pid(pid_t pid) {
  return __kauth_cred_proc_ref_for_pidversion(pid, 0, false);
}

kauth_cred_t kauth_cred_proc_ref_for_pidversion(pid_t pid, uint32_t pidvers) {
 ...
 */
static kauth_cred_t kauth_cred_alloc(kauth_cred_t parent_cred,
...
#if HAS_APPLE_PAC
  {
    ...
    signed_ptr = ptrauth_sign_unauthenticated(
        naked_ptr, ptrauth_key_process_independent_data,
        ptrauth_blend_discriminator(
            &newcred->cr_label, OS_PTRAUTH_DISCRIMINATOR("ucred.cr_label")));
   ...
   */
  found_rw = smr_shash_get_or_insert(&kauth_cred_hash, kauth_cred_key(newcred),
                                     &new_rw->crw_link, &kauth_cred_traits);
...
kauth_cred_t kauth_cred_require(kauth_cred_t cred) {
...
kauth_cred_rw_verify_panic(const struct ucred_rw *cred_rw, kauth_cred_t cred) {
  panic("ucred_rw backref mismatch: cred_rw:%p cred_rw->crw_cred:%p "
        "cred: %p",
        cred_rw, cred_rw->crw_cred, cred);
}

__pure2 static kauth_cred_t kauth_cred_ro(const struct ucred_rw *cred_rw) {
...
__attribute__((noinline)) static void kauth_cred_free(struct smr_node *node) {
...
static kauth_cred_t posix_cred_create_internal(posix_cred_t pcred,
 ...
 *		to be overwritten.
 */
kauth_cred_t kauth_cred_create(kauth_cred_t cred) {
  return posix_cred_create_internal(&cred->cr_posix, cred->cr_audit);
}

kauth_cred_t kauth_cred_derive(kauth_cred_t cred,
...
bool kauth_cred_proc_update(proc_t p, proc_settoken_t action,
                            kauth_cred_derive_t derive_fn) {
  ...
  for (;;) {
    ...
    if (__probable(proc_ucred_locked(p) == cur_cred)) {
      ...
      zalloc_ro_update_field_atomic(ZONE_ID_PROC_RO, proc_get_ro(p),
                                    p_ucred.__smr_ptr, ZRO_ATOMIC_XCHG_LONG,
                                    new_cred);
 ...
 */
bool kauth_cred_model_setresuid(kauth_cred_t model, uid_t ruid, uid_t euid,
                                uid_t svuid, uid_t gmuid) {
 ...
 * Returns:	(kauth_cred_t)			The updated credential
 */
bool kauth_cred_model_setresgid(kauth_cred_t model, gid_t rgid, gid_t egid,
                                gid_t svgid) {
  posix_cred_t pcred = posix_cred_get(model);
  bool updated = false;
 ...
 *	credential when the given gid actually results in changes to the
 * existing credential. The gmuid argument supplies a new uid (or KAUTH_UID_NONE
 * to opt out) which will be used for group membership checking.
 */
/*
 ...
 */
bool kauth_cred_model_setgroups(kauth_cred_t model, gid_t *groups,
                                size_t groupcount, uid_t gmuid) {
  posix_cred_t pcred = posix_cred_get(model);
...
SYSCTL_INT(_kern, OID_AUTO, ds_supgroups_supported, CTLFLAG_RW | CTLFLAG_LOCKED,
           &kauth_external_supplementary_groups_supported, 0, "");
#endif

int kauth_cred_getgroups(kauth_cred_t cred, gid_t *grouplist, size_t *countp) {
...
#if CONFIG_EXT_RESOLVER
  /*
   * If we've not opted out of using the resolver, then convert the cred to a
   * list of supplemental groups. We do this only if there has been a resolver
   * to talk to, since we may be too early in boot, or in an environment that
   * isn't using DS.
   */
  if (kauth_identitysvc_has_registered &&
      kauth_external_supplementary_groups_supported &&
      (pcred->cr_flags & CRF_NOMEMBERD) == 0) {
 ...
 *		flag is set.
 */
bool kauth_cred_model_setuidgid(kauth_cred_t model, uid_t uid, gid_t gid) {
 ...
 * Returns:	(kauth_cred_t)			The updated credential
 */
bool kauth_cred_model_setauditinfo(kauth_cred_t model,
                                   au_session_t *auditinfo_p) {
...
kauth_cred_t kauth_cred_label_update(kauth_cred_t cred, struct label *label) {
  ...
  new_cred =
      kauth_cred_derive(cred, ^bool(kauth_cred_t parent, kauth_cred_t model) {
        mac_cred_label_init(model);
...
void kauth_proc_label_update_execve(struct proc *p, vfs_context_t ctx,
                                    struct vnode *vp, off_t offset,
                                    struct vnode *scriptvp,
                                    struct label *scriptl, struct label *execl,
                                    unsigned int *csflags, void *macextensions,
                                    int *disjoint, int *update_return) {
  kauth_cred_proc_update(
      p, PROC_SETTOKEN_NONE, ^bool(kauth_cred_t parent, kauth_cred_t model) {
        mac_cred_label_init(model);
        mac_cred_label_associate(parent, model);
        mac_cred_label_update_execve(ctx, model, vp, offset, scriptvp, scriptl,
                                     execl, csflags, macextensions, disjoint,
                                     update_return);
        return true;
      });
}
#else
kauth_cred_t kauth_cred_label_update(__unused kauth_cred_t cred,
                                     __unused struct label *label) {
  return NULL;
}

int kauth_proc_label_update(__unused struct proc *p,
                            __unused struct label *label) {
  return 0;
}
#endif

void kauth_cred_ref(kauth_cred_t cred) { ucred_rw_ref(kauth_cred_rw(cred)); }
 ...
 *		performance.
 */
void(kauth_cred_set)(kauth_cred_t *credp, kauth_cred_t new_cred) {
 ...
 */
kauth_cred_t kauth_cred_copy_real(kauth_cred_t cred) {
  kauth_cred_derive_t fn =
...
static uint32_t kauth_cred_ro_hash(const struct ucred *cred, uint32_t seed) {
  uint32_t hash = seed;

  hash =
      os_hash_jenkins_update(&cred->cr_posix, sizeof(struct posix_cred), hash);
  hash =
      os_hash_jenkins_update(&cred->cr_audit, sizeof(struct au_session), hash);
#if CONFIG_MACF
...
static uint32_t kauth_cred_obj_hash(const struct smrq_slink *link,
                                    uint32_t seed) {
...
static bool kauth_cred_obj_equ(const struct smrq_slink *link, smrh_key_t key) {
  const struct ucred_rw *rw;
...
static bool kauth_cred_obj_try_get(void *obj) {
 ...
 *		to be overwritten.
 */
kauth_cred_t posix_cred_create(posix_cred_t pcred) {
  struct au_session audit = {
 ...
 *		case.
 */
posix_cred_t posix_cred_get(kauth_cred_t cred) { return &cred->cr_posix; }
 ...
 *		where this is true.
 */
int posix_cred_access(kauth_cred_t cred, id_t object_uid, id_t object_gid,
                      mode_t object_mode, mode_t mode_req) {
  int is_member;
   ...
   */
  if (kauth_cred_getuid(cred) == object_uid &&
      (mode_req & mode_owner) == mode_req) {
    return 0;
  }
