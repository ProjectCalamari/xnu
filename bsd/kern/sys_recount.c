...
static struct thsc_cpi _usage_to_cpi(struct recount_usage *usage) {
...
static struct thsc_time_cpi _usage_to_time_cpi(struct recount_usage *usage) {
...
static recount_cpu_kind_t _perflevel_index_to_cpu_kind(unsigned int perflevel) {
#if __AMP__
...
static int _selfcounts(thread_selfcounts_kind_t kind, user_addr_t buf,
                       size_t size) {
...
static int _selfcounts_perf_level(thread_selfcounts_kind_t kind,
                                  user_addr_t buf, size_t size) {
  ...
  const size_t counts_len =
      MIN(MIN(recount_topo_count(RCT_TOPO_CPU_KIND), RCT_CPU_KIND_COUNT),
          level_count);
...
int thread_selfcounts(__unused struct proc *p,
                      struct thread_selfcounts_args *uap,
...
int proc_pidthreadcounts(struct proc *p, uint64_t tid, user_addr_t uaddr,
                         size_t usize, int *size_out) {
  struct recount_usage usages[RCT_CPU_KIND_COUNT] = {0};
  ...
  const size_t counts_len =
      MIN(recount_topo_count(RCT_TOPO_CPU_KIND), RCT_CPU_KIND_COUNT);
  ...
  size_t copyout_size =
      MIN(sizeof(uint64_t) + counts_len * sizeof(struct proc_threadcounts_data),
          usize);
...
int proc_pidthreadcounts(__unused struct proc *p, __unused uint64_t tid,
                         __unused user_addr_t uaddr, __unused size_t usize,
                         __unused int *ret_out) {
  return ENOTSUP;
}

int thread_selfcounts(__unused struct proc *p,
                      __unused struct thread_selfcounts_args *uap,
                      __unused int *ret_out) {
  return ENOTSUP;
}
