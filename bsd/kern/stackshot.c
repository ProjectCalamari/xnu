...
EXPERIMENT_FACTOR_LEGACY_UINT(
    _kern, stackshot_estimate_adj, &stackshot_estimate_adj, 0, 100,
    "adjust stackshot estimates up by this percentage");

extern unsigned int stackshot_single_thread;

#if DEVELOPMENT || DEBUG
SYSCTL_UINT(_kern, OID_AUTO, stackshot_single_thread,
            CTLFLAG_RW | CTLFLAG_LOCKED, &stackshot_single_thread, 1,
            "Single-threaded stackshots");
#else
SYSCTL_UINT(_kern, OID_AUTO, stackshot_single_thread,
            CTLFLAG_RD | CTLFLAG_MASKED | CTLFLAG_LOCKED,
            &stackshot_single_thread, 1, "Single-threaded stackshots");
#endif
 ...
 *    stackshot_entitlement_report=0 or 1 or 2  (send CoreAnalytics when called
 * without entitlement(1) or always(2)) stackshot_entitlement_fail=0 or 1 (fail
 * call without entitlement) This only effects requests from userspace.
 *
 * For reporting, we only report a given command once.
 */
SECURITY_READ_ONLY_LATE(uint8_t)
stackshot_entitlement_report = STACKSHOT_ENTITLEMENT_REPORT;
SECURITY_READ_ONLY_LATE(bool)
stackshot_entitlement_refuse = STACKSHOT_ENTITLEMENT_REFUSE;
...
/* use single printable characters; these are in order of the stackshot
 * syscall's checks */
...
CA_EVENT(stackshot_entitlement_report, CA_INT, sshot_count, CA_BOOL,
         sshot_refused, CA_BOOL, sshot_have_entitlement, CA_BOOL,
         sshot_fromtest, CA_STATIC_STRING(2), sshot_progress,
         CA_STATIC_STRING(CA_PROCNAME_LEN), sshot_pcomm, CA_STATIC_STRING(33),
         sshot_pname);
...
static struct stackshot_entitlement_report
    *sshot_report_recent[SSHOT_ENTITLEMENT_RECENT];
...
static void stackshot_entitlement_send_report(
    const struct stackshot_entitlement_report *ser) {
  ...
  static_assert(sizeof(ser_event->sshot_pcomm) == sizeof(ser->ser_pcomm),
                "correct sshot_pcomm/ser_pcomm sizing");
  strlcpy(ser_event->sshot_pcomm, ser->ser_pcomm, sizeof(ser->ser_pcomm));
  static_assert(sizeof(ser_event->sshot_pname) == sizeof(ser->ser_pname),
                "correct sshot_pcomm/ser_pcomm sizing");
  strlcpy(ser_event->sshot_pname, ser->ser_pname, sizeof(ser->ser_pname));
  CA_EVENT_SEND(ca_event);
}

static void sshot_entitlement_schedule_batch(void) {
  static const uint64_t analytics_period_ns =
      SSHOT_ANALYTICS_PERIOD_HOURS * 60 * 60 * NSEC_PER_SEC;
  uint64_t analytics_period_absolutetime;
  nanoseconds_to_absolutetime(analytics_period_ns,
                              &analytics_period_absolutetime);

  thread_call_enter_delayed(sshot_entitlement_thread_call,
                            analytics_period_absolutetime +
                                mach_absolute_time());
}
...
static void stackshot_entitlement_do_report(bool have_entitlement,
                                            enum stackshot_progress progress,
                                            int testval) {
#pragma unused(testval)
  ...
  struct stackshot_entitlement_report *ser =
      kalloc_data(sizeof(*ser), Z_WAITOK | Z_NOFAIL);
  ...
  static_assert(sizeof(p->p_comm) == sizeof(myser.ser_pcomm),
                "correct p_comm/ser_pcomm sizing");
  strlcpy(myser.ser_pcomm, p->p_comm, sizeof(myser.ser_pcomm));
  static_assert(sizeof(p->p_name) == sizeof(myser.ser_pname),
                "correct p_name/ser_pname sizing");
  ...
  // Search the table, looking for a match or a NULL slot.  While we search,
  // track the slot with the oldest use time as an eviction candidate, for LRU
  // behavior
  ...
  for (int i = 0; i < SSHOT_ENTITLEMENT_RECENT; i++) {
    ...
    if (tslot == NULL || ((*tslot)->ser_lastev > cur->ser_lastev)) {
      tslot = curp;
    }
  }
  // Either we have:
  ...
  //   no match, a full table, and tslot points at the entry with the lowest
  //   count
  struct stackshot_entitlement_report *cur =
      NULL; // the entry to bump the count of
  if (match) {
    cur = *tslot;
    tofree = ser;
  } else {
...
static void sshot_entitlement_send_batch(void *arg0, void *arg1) {
#pragma unused(arg0, arg1)
  struct stackshot_entitlement_report *ser =
      kalloc_data(sizeof(*ser) * SSHOT_ENTITLEMENT_RECENT, Z_WAITOK | Z_NOFAIL);
  size_t count = 0;
 ...
 */
static int sysctl_stackshot_entitlement_test SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  ...
  for (int idx = 0; idx < SSHOT_ENTITLEMENT_RECENT; idx++) {
    stackshot_entitlement_do_report(false, STACKSHOT_NOT_ENTITLED,
                                    SSHOT_ENTITLEMENT_REPORT_TEST(idx + 1));
  }
  ...
  // overflow test
  stackshot_entitlement_do_report(false, STACKSHOT_NOT_ENTITLED,
                                  SSHOT_ENTITLEMENT_REPORT_TEST_OVERFLOW);
  stackshot_entitlement_do_report(false, STACKSHOT_NOT_ENTITLED,
                                  SSHOT_ENTITLEMENT_REPORT_TEST_OVERFLOW);
  sshot_entitlement_send_batch(NULL, NULL);
  const uint32_t post_overflow = sshot_report_test_events - post_batch;
  const uint64_t post_overflow_counts =
      sshot_report_test_counts - post_batch_counts;

  os_log_error(OS_LOG_DEFAULT,
               "sysctl_stackshot_entitlement_test: made %d events, %d events "
               "sent, %d counts (both should == events)",
               SSHOT_ENTITLEMENT_RECENT, post_batch, (int)post_batch_counts);
  os_log_error(OS_LOG_DEFAULT,
               "sysctl_stackshot_entitlement_test: overflow, %d events sent "
               "(expect 2), %llx counts (expect %llx)",
               post_overflow, (long long)post_overflow_counts,
               2 * (long long)UINT32_MAX);
  ...
  if (post_batch != SSHOT_ENTITLEMENT_RECENT ||
      post_batch_counts != SSHOT_ENTITLEMENT_RECENT || post_overflow != 2 ||
...
static int sysctl_stackshot_entitlement_status SYSCTL_HANDLER_ARGS {
  int return_value = ((stackshot_entitlement_report & 0xf) |
                      (stackshot_entitlement_refuse ? 0x10 : 0));
...
__startup_func static void atboot_stackshot_entitlement(void) {
  uint32_t boot_arg;
  if (PE_parse_boot_argn(SSHOT_ENTITLEMENT_BOOTARG_REPORT, &boot_arg,
                         sizeof(boot_arg))) {
    /* clamp to valid values */
    boot_arg =
        (boot_arg <= STACKSHOT_REPORT_ALL ? boot_arg : STACKSHOT_REPORT_ALL);
    stackshot_entitlement_report = (uint8_t)boot_arg;
  }
  if (PE_parse_boot_argn(SSHOT_ENTITLEMENT_BOOTARG_FAIL, &boot_arg,
                         sizeof(boot_arg))) {
    stackshot_entitlement_refuse = (boot_arg != 0);
  }
  sshot_entitlement_thread_call = thread_call_allocate_with_options(
      sshot_entitlement_send_batch, NULL, THREAD_CALL_PRIORITY_LOW,
      THREAD_CALL_OPTIONS_ONCE);
}
STARTUP(SYSCTL, STARTUP_RANK_MIDDLE, atboot_stackshot_entitlement);

static int sysctl_stackshot_stats SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  stackshot_stats_t stats;
  proc_t self = current_proc();

  /* root processes and non-root processes with the STATS entitlement can read
   * this */
  ...
  stackshot_get_timing(&stats.ss_last_start, &stats.ss_last_end,
                       &stats.ss_count, &stats.ss_duration);
...
static int stackshot_kern_return_to_bsd_error(kern_return_t kr) {
  switch (kr) {
  case KERN_SUCCESS:
    return 0;
...
/*
 * stack_snapshot_with_config:	Obtains a coherent set of stack traces for
 * specified threads on the sysem, tracing both kernel and user stacks where
 * available. Allocates a buffer from the kernel and maps the buffer into the
 * calling task's address space.
 *
 * Inputs:                      uap->stackshot_config_version - version of the
 * stackshot config that is being passed uap->stackshot_config - pointer to the
 * stackshot config uap->stackshot_config_size- size of the stackshot config
 * being passed Outputs:			EINVAL if there is a problem
 * with the arguments EFAULT if we failed to copy in the arguments succesfully
 *				EPERM if the caller is not privileged
 *				ENOTSUP if the caller is passing a version of
 * arguments that is not supported by the kernel (indicates libsyscall:kernel
 * mismatch) or if the caller is requesting unsupported flags ENOENT if the
 * caller is requesting an existing buffer that doesn't exist or if the
 *				requested PID isn't found
 *				ENOMEM if the kernel is unable to allocate
 * enough memory to serve the request ENOSPC if there isn't enough space in the
 * caller's address space to remap the buffer ESRCH if the target PID isn't
 * found returns KERN_SUCCESS on success
 */
int stack_snapshot_with_config(struct proc *p,
                               struct stack_snapshot_with_config_args *uap,
                               __unused int *retval) {
  int error = 0;
  kern_return_t kr;
  ...
  case STACKSHOT_CONFIG_TYPE:
    ...
    kr = kern_stack_snapshot_internal(uap->stackshot_config_version, &config,
                                      sizeof(stackshot_config_t), TRUE);
...
err:
  if (report == STACKSHOT_REPORT_ALL ||
      (report == STACKSHOT_REPORT_NO_ENT && !has_entitlement)) {
    stackshot_entitlement_do_report(has_entitlement, progress,
                                    SSHOT_ENTITLEMENT_REPORT_NORMAL);
  }
  return error;
}
...
#if CONFIG_TELEMETRY
/*
 * microstackshot:	Catch all system call for microstackshot related
 * operations, including enabling/disabling both global and windowed
 * microstackshots as well as retrieving windowed or global stackshots and the
 * boot profile. Inputs:              uap->tracebuf - address of the user space
 * destination buffer uap->tracebuf_size - size of the user space trace buffer
 *			uap->flags - various flags
 * Outputs:		EPERM if the caller is not privileged
 *			EINVAL if the supplied mss_args is NULL,
 * mss_args.tracebuf is NULL or mss_args.tracebuf_size is not sane ENOMEM if we
 * don't have enough memory to satisfy the request *retval contains the number
 * of bytes traced, if successful and -1 otherwise.
 */
int microstackshot(struct proc *p, struct microstackshot_args *uap,
                   int32_t *retval) {
  int error = 0;
  kern_return_t kr;
  ...
  kr = stack_microstackshot(uap->tracebuf, uap->tracebuf_size, uap->flags,
                            retval);
...
/*
 * kern_stack_snapshot_with_reason:	Obtains a coherent set of stack traces
 * for specified threads on the sysem, tracing both kernel and user stacks where
 * available. Allocates a buffer from the kernel and stores the address of this
 * buffer.
 *
 * Inputs:                              reason - the reason for triggering a
 * stackshot (unused at the moment, but in the future will be saved in the
 * stackshot) Outputs:				EINVAL/ENOTSUP if there is a
 * problem with the arguments EPERM if the caller doesn't pass at least one
 * KERNEL stackshot flag ENOMEM if the kernel is unable to allocate enough
 * memory to serve the request ESRCH if the target PID isn't found returns
 * KERN_SUCCESS on success
 */
int kern_stack_snapshot_with_reason(__unused char *reason) {
  stackshot_config_t config;
  kern_return_t kr;

  config.sc_pid = -1;
  config.sc_flags =
      (STACKSHOT_SAVE_LOADINFO | STACKSHOT_GET_GLOBAL_MEM_STATS |
       STACKSHOT_SAVE_IN_KERNEL_BUFFER | STACKSHOT_KCDATA_FORMAT |
       STACKSHOT_ENABLE_UUID_FAULTING | STACKSHOT_ENABLE_BT_FAULTING |
       STACKSHOT_THREAD_WAITINFO | STACKSHOT_NO_IO_STATS |
       STACKSHOT_COLLECT_SHAREDCACHE_LAYOUT);
  config.sc_delta_timestamp = 0;
  config.sc_out_buffer_addr = 0;
  config.sc_out_size_addr = 0;

  kr = kern_stack_snapshot_internal(STACKSHOT_CONFIG_TYPE, &config,
                                    sizeof(stackshot_config_t), FALSE);
  return stackshot_kern_return_to_bsd_error(kr);
}

#if DEBUG || DEVELOPMENT
static int stackshot_dirty_buffer_test(__unused int64_t in, int64_t *out) {
  uint64_t ss_flags =
      STACKSHOT_KCDATA_FORMAT | STACKSHOT_NO_IO_STATS |
      STACKSHOT_SAVE_KEXT_LOADINFO | STACKSHOT_ACTIVE_KERNEL_THREADS_ONLY |
      STACKSHOT_THREAD_WAITINFO | STACKSHOT_INCLUDE_DRIVER_THREADS_IN_KERNEL;
  unsigned ss_bytes = 0;
  vm_offset_t buf = 0;
  kern_return_t kr;

  // 8MB buffer
  kr = kmem_alloc(kernel_map, &buf, 8 * 1024 * 1024, KMA_ZERO | KMA_DATA_SHARED,
                  VM_KERN_MEMORY_DIAG);
  if (kr != KERN_SUCCESS) {
  ...
  kr = stack_snapshot_from_kernel(0, (char *)buf, 8 * 1024 * 1024, ss_flags, 0,
                                  0, &ss_bytes);
  if (kr != KERN_SUCCESS) {
  ...
  if (ss_bytes == 0) {
    *out = -2;
    printf(
        "stackshot_dirty_buffer_test: stackshot was empty but did not fail\n");
    goto end;
  }
...
static int stackshot_kernel_initiator_test(int64_t in, int64_t *out) {
  ...
  unsigned ss_bytes = 0;
  if (in == 1) {
    kr = kmem_alloc(kernel_map, &buf, 8 * 1024 * 1024,
    ...
    kr = stack_snapshot_from_kernel(0, (char *)buf, 8 * 1024 * 1024, ss_flags,
                                    0, 0, &ss_bytes);
    if (kr != KERN_SUCCESS) {
    ...
    if (ss_bytes == 0) {
      *out = -2;
      printf("stackshot_kernel_initiator_test: stackshot was empty but did not "
             "fail\n");
      goto end;
    }
  } else if (in == 2) {
    ...
    if (kr != KERN_SUCCESS) {
      printf("stackshot_kernel_initiator_test: kern_stack_snapshot_with_reason "
             "failed: %d\n",
             kr);
      goto err;
    }
...
SYSCTL_TEST_REGISTER(stackshot_kernel_initiator,
                     stackshot_kernel_initiator_test);
