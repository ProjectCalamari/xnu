...
struct aio_workq_entry {
  ...
  struct aio_workq_entry
      *lio_leader; /* pointer to the lio leader, can be self */
  ...
  /* Initialized, and possibly freed by aio_work_thread() or at free if
   * cancelled */
...
#define WQ_AIO_TRACE_WQ(x, wq)                                                 \
  ({                                                                           \
    KERNEL_DEBUG_CONSTANT(                                                     \
        BSDDBG_CODE(DBG_BSD_AIO, (x)), proc_getpid((wq)->wa_proc),             \
        (uintptr_t)thread_tid(current_thread()), (wq)->wa_nthreads,            \
        (wq)->wa_thidlecount, (wq)->wa_thdying_count);                         \
  })

/*
 * Notes on aio sleep / wake channels.
 * We currently pick a couple fields within the proc structure that will allow
 * us sleep channels that currently do not collide with any other kernel
 * routines. At this time, for binary compatibility reasons, we cannot create
 * new proc fields.
 */
...
static void aio_workq_remove_entry_locked(aio_workq_t queue,
                                          aio_workq_entry *entryp);
static void aio_workq_add_entry_locked(aio_workq_t queue,
                                       aio_workq_entry *entryp);
...
static int aio_queue_async_request(proc_t procp, user_addr_t aiocbp,
                                   aio_entry_flags_t);
static int aio_validate(proc_t, aio_workq_entry *entryp);

static int do_aio_cancel_locked(proc_t p, int fd, user_addr_t aiocbp,
                                aio_entry_flags_t);
...
static void do_munge_aiocb_user32_to_user(struct user32_aiocb *my_aiocbp,
                                          struct user_aiocb *the_user_aiocbp);
static void do_munge_aiocb_user64_to_user(struct user64_aiocb *my_aiocbp,
                                          struct user_aiocb *the_user_aiocbp);
static aio_workq_entry *aio_create_queue_entry(proc_t procp, user_addr_t aiocbp,
                                               aio_entry_flags_t);
...
static bool workq_aio_entry_remove_locked(struct proc *p,
                                          aio_workq_entry *entryp);

static void workq_aio_kill_old_threads_call(void *param0,
                                            void *param1 __unused);
static void workq_aio_unpark_continue(void *parameter __unused,
                                      wait_result_t wr);

static void workq_aio_mark_exiting(proc_t p);
static void workq_aio_exit(proc_t p);

#define ASSERT_AIO_PROC_LOCK_OWNED(p)                                          \
  LCK_MTX_ASSERT(aio_proc_mutex(p), LCK_MTX_ASSERT_OWNED)
#define ASSERT_AIO_WORKQ_LOCK_OWNED(q)                                         \
  LCK_SPIN_ASSERT(aio_workq_lock(q), LCK_ASSERT_OWNED)
...
extern int dofileread(vfs_context_t ctx, struct fileproc *fp, user_addr_t bufp,
                      user_size_t nbyte, off_t offset, int flags,
                      user_ssize_t *retval);
extern int dofilewrite(vfs_context_t ctx, struct fileproc *fp, user_addr_t bufp,
                       user_size_t nbyte, off_t offset, int flags,
                       user_ssize_t *retval);
...
/* Hash */
static aio_workq_t aio_entry_workq(__unused aio_workq_entry *entryp) {
...
static void aio_workq_remove_entry_locked(aio_workq_t queue,
  ...
  if (entryp->aio_workq_link.tqe_prev == NULL) {
    panic("Trying to remove an entry from a work queue, but it is not on a "
          "queue");
  }
...
static void aio_proc_lock(proc_t procp) { lck_mtx_lock(aio_proc_mutex(procp)); }

static void aio_proc_lock_spin(proc_t procp) {
  lck_mtx_lock_spin(aio_proc_mutex(procp));
}

static bool aio_has_any_work(void) {
  return os_atomic_load(&aio_anchor.aio_total_count, relaxed) != 0;
}

static bool aio_try_proc_insert_active_locked(proc_t procp,
                                              aio_workq_entry *entryp) {
  int old, new;
...
static lck_mtx_t *aio_proc_mutex(proc_t procp) { return &procp->p_mlock; }
...
static lck_spin_t *aio_workq_lock(aio_workq_t wq) { return &wq->aioq_lock; }
 ...
 */
int aio_cancel(proc_t p, struct aio_cancel_args *uap, int *retval) {
 ...
 * THIS MAY BLOCK.
 */
__private_extern__ void _aio_close(proc_t p, int fd) {
  int error;
  ...
  if (error == AIO_NOTCANCELED) {
    ...
    while (aio_proc_has_active_requests_for_file(p, fd)) {
      msleep(&p->AIO_CLEANUP_SLEEP_CHAN, aio_proc_mutex(p), PRIBIO, "aio_close",
             0);
    }
  }
 ...
 * fdatasync, or sync).
 */
int aio_error(proc_t p, struct aio_error_args *uap, int *retval) {
  aio_workq_entry *entryp;
  int error;
 ...
 * fdatasync() call.
 */
int aio_fsync(proc_t p, struct aio_fsync_args *uap, int *retval) {
 ...
 * (uap->aiocbp->aio_buf).
 */
int aio_read(proc_t p, struct aio_read_args *uap, int *retval) {
  int error;
 ...
 */
int aio_return(proc_t p, struct aio_return_args *uap, user_ssize_t *retval) {
  aio_workq_entry *entryp;
  int error = EINVAL;
 ...
 */
__private_extern__ void _aio_exec(proc_t p) {
 ...
 * a process that is terminating (via exit() or exec()).  We cancel any async
 * IOs we can and wait for those already active.  We also disable signaling for
 * cancelled or active aio requests that complete.  This routine MAY block!
 */
__private_extern__ void _aio_exit(proc_t p) {
  ...
  if (error == AIO_NOTCANCELED) {
     ...
     * our channel used to sleep for ALL active requests to complete.  This
     * sleep channel (proc.AIO_CLEANUP_SLEEP_CHAN) is only used when we must
     * wait for all active aio requests.
     */
    ...
    while (aio_has_active_requests_for_process(p)) {
      msleep(&p->AIO_CLEANUP_SLEEP_CHAN, aio_proc_mutex(p), PRIBIO, "aio_exit",
             0);
    }
  }
...
static bool should_cancel(aio_workq_entry *entryp, int fd, user_addr_t aiocbp,
                          aio_entry_flags_t reason) {
...
/*
 * do_aio_cancel_locked - cancel async IO requests (if possible).  We get called
 * by aio_cancel, close, and at exit. There are three modes of operation: 1)
 * cancel all async IOs for a process - fd is 0 and aiocbp is NULL 2) cancel all
 * async IOs for file descriptor - fd is > 0 and aiocbp is NULL 3) cancel one
 * async IO associated with the given aiocbp. Returns -1 if no matches were
 * found, AIO_CANCELED when we cancelled all target async IO requests,
 * AIO_NOTCANCELED if we could not cancel all target async IO requests, and
 * AIO_ALLDONE if all target async IO requests were already complete. WARNING -
 * do not deference aiocbp in this routine, it may point to user land data that
 * has not been copied in (when called from aio_cancel())
 *
...
again:
  ...
  TAILQ_FOREACH_SAFE(entryp, &p->p_aio_activeq, aio_proc_link, tmp) {
    ...
    if (aio_entry_try_workq_remove(p, entryp)) {
      ...
      KERNEL_DEBUG(
          BSDDBG_CODE(DBG_BSD_AIO, AIO_cancel_async_workq) | DBG_FUNC_NONE,
          VM_KERNEL_ADDRPERM(p), VM_KERNEL_ADDRPERM(entryp->uaiocbp), fd, 0, 0);
    ...
    KERNEL_DEBUG(BSDDBG_CODE(DBG_BSD_AIO, AIO_cancel_activeq) | DBG_FUNC_NONE,
                 VM_KERNEL_ADDRPERM(p), VM_KERNEL_ADDRPERM(entryp->uaiocbp), fd,
                 0, 0);
 ...
 * woke us up.
 */
int aio_suspend(proc_t p, struct aio_suspend_args *uap, int *retval) {
  __pthread_testcancel(1);
  return aio_suspend_nocancel(p, (struct aio_suspend_nocancel_args *)uap,
                              retval);
}

int aio_suspend_nocancel(proc_t p, struct aio_suspend_nocancel_args *uap,
                         int *retval) {
  int error;
  int i;
  uint64_t abstime;
  ...
  *retval = -1;
  ...
  if (aiocbpp == NULL ||
      aio_copy_in_list(p, uap->aiocblist, aiocbpp, uap->nent)) {
    error = EAGAIN;
    goto ExitThisRoutine;
  }
...
int aio_write(proc_t p, struct aio_write_args *uap, int *retval __unused) {
  int error;
...
static int aio_copy_in_list(proc_t procp, user_addr_t aiocblist,
                            user_addr_t *aiocbpp, int nent) {
  int result;
...
static int aio_copy_in_sigev(proc_t procp, user_addr_t sigp,
                             struct user_sigevent *sigev) {
  int result = 0;
  ...
  if (proc_is64bit(procp)) {
    ...
    if (result == 0) {
      ...
      sigev->sigev_value.size_equivalent.sival_int =
          sigevent64.sigev_value.size_equivalent.sival_int;
 ...
 */
static int aio_sigev_validate(const struct user_sigevent *sigev) {
 ...
 */
static bool aio_try_enqueue_work_locked(proc_t procp, aio_workq_entry *entryp,
  ...
  KERNEL_DEBUG_CONSTANT(
      BSDDBG_CODE(DBG_BSD_AIO, AIO_work_queued) | DBG_FUNC_START,
      VM_KERNEL_ADDRPERM(procp), VM_KERNEL_ADDRPERM(entryp->uaiocbp),
      entryp->flags, entryp->aiocb.aio_fildes, 0);
  KERNEL_DEBUG_CONSTANT(
      BSDDBG_CODE(DBG_BSD_AIO, AIO_work_queued) | DBG_FUNC_END,
...
static int aio_register_kevent(proc_t procp, aio_workq_entry *entryp) {
...
exit:
  ...
  KERNEL_DEBUG(BSDDBG_CODE(DBG_BSD_AIO, AIO_register_kevent) | DBG_FUNC_END,
               VM_KERNEL_ADDRPERM(procp), VM_KERNEL_ADDRPERM(entryp), error, 0,
               0);
 ...
 */
int lio_listio(proc_t p, struct lio_listio_args *uap, int *retval __unused) {
  ...
  for (int i = 0; i < lio_count; i++) {
    if (aio_try_enqueue_work_locked(p, entries[i], leader)) {
      workq_aio_wakeup_thread(
          p); /* this may drop and reacquire the proc lock */
      /*
  ...
  if (uap->mode == LIO_WAIT && result == 0) {
    ...
    while (leader->lio_pending) {
      ...
      if (msleep(leader, aio_proc_mutex(p), PCATCH | PRIBIO | PSPIN,
                 "lio_listio", 0) != 0) {
        result = EINTR;
        break;
      }
...
/*
 * aio_get_some_work - get the next async IO request that is ready to be
 * executed. aio_fsync complicates matters a bit since we cannot do the fsync
 * until all async IO requests at the time the aio_fsync call came in have
 * completed. NOTE - AIO_LOCK must be held by caller
 */
static aio_workq_entry *aio_get_some_work(void) {
  ...
  waitq_assert_wait64(&queue->aioq_waitq, CAST_EVENT64_T(queue), THREAD_UNINT,
                      0);
  aio_workq_unlock(queue);
...
/*
 * aio_delay_fsync_request - look to see if this aio_fsync request should be
 * delayed. A big, simple hammer: only send it off if it's the most recently
 * filed IO which has not been completed.
 */
static boolean_t aio_delay_fsync_request(aio_workq_entry *entryp) {
 ...
 */
static int aio_validate(proc_t p, aio_workq_entry *entryp) {
  struct fileproc *fp;
  int flag;
  int result;
...
static void do_aio_completion_and_unlock(proc_t p, aio_workq_entry *entryp,
  ...
  if (__improbable(entryp->flags & AIO_EXIT_WAIT)) {
    KERNEL_DEBUG(
        BSDDBG_CODE(DBG_BSD_AIO, AIO_completion_cleanup_wait) | DBG_FUNC_NONE,
    ...
    if (!aio_has_active_requests_for_process(p)) {
      ...
      KERNEL_DEBUG(
          BSDDBG_CODE(DBG_BSD_AIO, AIO_completion_cleanup_wake) | DBG_FUNC_NONE,
          VM_KERNEL_ADDRPERM(p), VM_KERNEL_ADDRPERM(entryp->uaiocbp), 0, 0, 0);
    }
  ...
  if (do_signal) {
    ...
    KERNEL_DEBUG(BSDDBG_CODE(DBG_BSD_AIO, AIO_completion_kevent) |
                     DBG_FUNC_NONE,
                 VM_KERNEL_ADDRPERM(p), VM_KERNEL_ADDRPERM(entryp->uaiocbp),
                 entryp->aiocb.aio_sigevent.sigev_signo, 0, 0);
  ...
  /*
   * A thread in aio_suspend() wants to known about completed IOs.  If it
   * checked the done list before we moved our AIO there, then it already
   * asserted its wait, and we can wake it up without holding the lock.  If it
   * checked the list after we did our move, then it already has seen the AIO
   * that we moved.  Herego, we can do our wakeup without holding the lock.
   */
  wakeup(&p->AIO_SUSPEND_SLEEP_CHAN);
  KERNEL_DEBUG(
      BSDDBG_CODE(DBG_BSD_AIO, AIO_completion_suspend_wake) | DBG_FUNC_NONE,
      VM_KERNEL_ADDRPERM(p), VM_KERNEL_ADDRPERM(entryp->uaiocbp), 0, 0, 0);
...
static int do_aio_read(aio_workq_entry *entryp) {
  ...
  if (fp->fp_glob->fg_flag & FREAD) {
    error = dofileread(&entryp->context, fp, entryp->aiocb.aio_buf,
                       entryp->aiocb.aio_nbytes, entryp->aiocb.aio_offset,
                       FOF_OFFSET, &entryp->returnval);
  } else {
    error = EBADF;
  }
...
static int do_aio_write(aio_workq_entry *entryp) {
  ...
  if (fp->fp_glob->fg_flag & FWRITE) {
    ...
    error = dofilewrite(&entryp->context, fp, entryp->aiocb.aio_buf,
                        entryp->aiocb.aio_nbytes, entryp->aiocb.aio_offset,
                        flags, &entryp->returnval);
  } else {
    error = EBADF;
  }
...
__private_extern__ void aio_init(void) {
  ...
  clock_interval_to_absolutetime_interval(aio_wq_reduce_pool_window.usecs,
                                          NSEC_PER_USEC,
                                          &aio_wq_reduce_pool_window.abstime);
}
 ...
 */
task_t get_aiotask(void) { return current_uthread()->uu_aio_task; }
...
static void do_munge_aiocb_user32_to_user(struct user32_aiocb *my_aiocbp,
  ...
  // LP64
  the_user_aiocbp->aio_sigevent.sigev_notify =
      my_aiocbp->aio_sigevent.sigev_notify;
  the_user_aiocbp->aio_sigevent.sigev_signo =
      my_aiocbp->aio_sigevent.sigev_signo;
...
#endif
    static void
    do_munge_aiocb_user64_to_user(struct user64_aiocb *my_aiocbp,
                                  struct user_aiocb *the_user_aiocbp) {
#if __LP64__
  the_user_aiocbp->aio_fildes = my_aiocbp->aio_fildes;
  the_user_aiocbp->aio_offset = my_aiocbp->aio_offset;
...
static int filt_aioattach(struct knote *kn, struct kevent_qos_s *kev) {
 ...
 */
static int filt_aioevent(struct knote *kn, long hint) {
  aio_workq_entry *entryp;
  int activate = 0;
...
static int filt_aiotouch(struct knote *kn, struct kevent_qos_s *kev) {
  panic("%s: kn %p kev %p (NOT EXPECTED TO BE CALLED!!)", __func__, kn, kev);
}

static int filt_aioprocess(struct knote *kn, struct kevent_qos_s *kev) {
  aio_workq_entry *entryp;
  proc_t p;
  int res = 0;
...
/*
 * The per process workq threads call this function to handle the aio request.
 * The threads belong to the same process so we don't need to change the vm maps
 * as we would for kernel threads.
 */
static int workq_aio_process_entry(aio_workq_entry *entryp) {
  ...
  assert(current_proc() == p &&
         current_thread() != vfs_context_thread(&entryp->context));
 ...
 * workqueue implementation for libdispatch/pthreads. They are stripped down
 * versions of the corresponding functions for libdispatch/pthreads.
 */
...
static inline uint32_t _wa_flags(workq_aio_t wq_aio) {
  return os_atomic_load(&wq_aio->wa_flags, relaxed);
}

static inline bool _wq_exiting(workq_aio_t wq_aio) {
  return _wa_flags(wq_aio) & WQ_EXITING;
}

static inline bool _wq_dead(workq_aio_t wq_aio) {
  return _wa_flags(wq_aio) & AIO_WQ_DEAD;
}

#define AIO_WQPTR_IS_INITING_VALUE ((workq_aio_t) ~(uintptr_t)0)

static workq_aio_t proc_get_aio_wqptr_fast(struct proc *p) {
  return os_atomic_load(&p->p_aio_wqptr, relaxed);
}

static workq_aio_t proc_get_aio_wqptr(struct proc *p) {
  workq_aio_t wq_aio = proc_get_aio_wqptr_fast(p);
  return wq_aio == AIO_WQPTR_IS_INITING_VALUE ? NULL : wq_aio;
}

static void proc_set_aio_wqptr(struct proc *p, workq_aio_t wq_aio) {
...
static bool proc_init_aio_wqptr_or_wait(struct proc *p) {
  workq_aio_t wq_aio;
...
static inline event_t workq_aio_parked_wait_event(struct uthread *uth) {
  return (event_t)&uth->uu_workq_stackaddr;
}

static inline void workq_aio_thread_wakeup(struct uthread *uth) {
...
static void workq_aio_exit(proc_t p) {
  ...
  if (wq_aio->wa_nthreads) {
    ...
    if (wq_aio->wa_nthreads) {
      ...
      while (wq_aio->wa_nthreads) {
        msleep(&wq_aio->wa_nthreads, aio_proc_mutex(p), PRIBIO | PSPIN,
               "aio_workq_exit", 0);
      }
    }
    aio_proc_unlock(p);
  ...
  assertf(TAILQ_EMPTY(&wq_aio->wa_thidlelist),
          "wa_thidlecount = %d, wa_thdying_count = %d", wq_aio->wa_thidlecount,
          wq_aio->wa_thdying_count);
  assertf(wq_aio->wa_thidlecount == 0,
          "wa_thidlecount = %d, wa_thdying_count = %d", wq_aio->wa_thidlecount,
          wq_aio->wa_thdying_count);
  assertf(wq_aio->wa_thdying_count == 0, "wa_thdying_count = %d",
          wq_aio->wa_thdying_count);

  kfree_type(workq_aio_s, wq_aio);
}

static int workq_aio_open(struct proc *p) {
  workq_aio_t wq_aio;
  int error = 0;
  ...
  if (proc_get_aio_wqptr(p) == NULL) {
    ...
    wq_aio->wa_death_call = thread_call_allocate_with_options(
        workq_aio_kill_old_threads_call, wq_aio, THREAD_CALL_PRIORITY_USER,
...
static inline uint64_t workq_kill_delay_for_idle_aio_thread() {
  return aio_wq_reduce_pool_window.abstime;
}

static inline bool workq_should_kill_idle_aio_thread(struct uthread *uth,
                                                     uint64_t now) {
  uint64_t delay = workq_kill_delay_for_idle_aio_thread();
  return now - uth->uu_save.uus_workq_park_data.idle_stamp > delay;
}

static void workq_aio_death_call_schedule(workq_aio_t wq_aio,
                                          uint64_t deadline) {
 ...
 */
static void workq_aio_death_policy_evaluate(workq_aio_t wq_aio,
                                            uint16_t decrement) {
  struct uthread *uth;
  ...
  workq_aio_death_call_schedule(
      wq_aio, uth->uu_save.uus_workq_park_data.idle_stamp + delay);
}
...
static void workq_push_idle_aio_thread(proc_t p, workq_aio_t wq_aio,
                                       struct uthread *uth,
  ...
  struct uthread *tail =
      TAILQ_LAST(&wq_aio->wa_thidlelist, workq_aio_uthread_head);
...
workq_aio_park_and_unlock(proc_t p, workq_aio_t wq_aio, struct uthread *uth,
  ...
  if (uth->uu_workq_flags & UT_WORKQ_DYING) {
    workq_aio_unpark_for_death_and_unlock(
        p, wq_aio, uth, WORKQ_UNPARK_FOR_DEATH_WAS_IDLE, setup_flags);
    __builtin_unreachable();
  }
 ...
 */
static void workq_aio_thread_reset_pri(struct uthread *uth, thread_t src_th) {
  thread_t th = get_machthread(uth);
  thread_qos_t qos =
      (thread_qos_t)proc_get_effective_thread_policy(src_th, TASK_POLICY_QOS);
  int priority = 31;
...
static inline void workq_aio_thread_set_type(struct uthread *uth,
                                             uint16_t flags) {
...
workq_aio_unpark_continue(void *parameter __unused, wait_result_t wr) {
  ...
  if (__probable(uth->uu_workq_flags & UT_WORKQ_RUNNING)) {
    workq_aio_unpark_select_req_or_park_and_unlock(p, wq_aio, uth,
                                                   WQ_SETUP_NONE);
    __builtin_unreachable();
  }
  ...
  workq_aio_unpark_for_death_and_unlock(
      p, wq_aio, uth, WORKQ_UNPARK_FOR_DEATH_WAS_IDLE, WQ_SETUP_NONE);

  __builtin_unreachable();
}

/*
 * Called by thread_create_workq_aio_waiting() during thread initialization,
 * before assert_wait, before the thread has been started.
 */
event_t aio_workq_thread_init_and_wq_lock(task_t task, thread_t th) {
  struct uthread *uth = get_bsdthread_info(th);
 ...
 */
static kern_return_t workq_aio_add_new_thread(proc_t p, workq_aio_t wq_aio) {
  kern_return_t kret;
  thread_t th;
  ...
  kret = thread_create_aio_workq_waiting(proc_task(p),
                                         workq_aio_unpark_continue, &th);

  if (kret != KERN_SUCCESS) {
    WQ_AIO_TRACE(AIO_WQ_aio_thread_create_failed | DBG_FUNC_NONE, wq_aio, kret,
                 0, 0, 0);
    goto out;
  }
...
static void workq_aio_wakeup_thread_internal(proc_t p, bool unlock) {
  workq_aio_t wq_aio = proc_get_aio_wqptr(p);
...
void workq_aio_prepare(struct proc *p) {
  workq_aio_t wq_aio = proc_get_aio_wqptr(p);
...
bool workq_aio_entry_add_locked(struct proc *p, aio_workq_entry *entryp) {
  workq_aio_t wq_aio = proc_get_aio_wqptr(p);
  bool ret = false;
...
bool workq_aio_entry_remove_locked(struct proc *p, aio_workq_entry *entryp) {
  workq_aio_t wq_aio = proc_get_aio_wqptr(p);
