...
memstat_evaluate_health_conditions(memorystatus_system_health_t status) {
...
#if CONFIG_JETSAM
  memstat_evaluate_page_shortage(&status->msh_available_pages_below_soft,
  ...
  status->msh_compressor_is_thrashing =
      !memorystatus_swap_all_apps && vm_compressor_is_thrashing();
#if CONFIG_PHANTOM_CACHE
  status->msh_phantom_cache_pressure =
      os_atomic_load(&memorystatus_phantom_cache_pressure, relaxed);
#else
  status->msh_phantom_cache_pressure = false;
#endif /* CONFIG_PHANTOM_CACHE */
  if (!memorystatus_swap_all_apps && status->msh_phantom_cache_pressure &&
      !(status->msh_compressor_is_thrashing &&
        status->msh_compressor_exhausted)) {
    status->msh_filecache_is_thrashing = true;
  }
  status->msh_pageout_starved =
      os_atomic_load(&memorystatus_pageout_starved, relaxed);
  status->msh_swappable_compressor_segments_over_limit =
      memorystatus_swap_over_trigger(100);
  ...
  status->msh_zone_map_is_exhausted =
      os_atomic_load(&memorystatus_zone_map_is_exhausted, relaxed);
}
...
memstat_is_system_healthy(const memorystatus_system_health_t status) {
  ...
  return !(status->msh_available_pages_below_critical ||
           ...
           status->msh_zone_map_is_exhausted || status->msh_pageout_starved);
#else  /* CONFIG_JETSAM */
  return !(status->msh_zone_map_is_exhausted ||
           status->msh_compressor_exhausted ||
           status->msh_compressor_low_on_space || status->msh_swap_exhausted ||
           status->msh_swap_low_on_space || status->msh_vm_pressure_critical ||
...
memstat_log_system_health(const memorystatus_system_health_t status) {
   ...
   */
  if (prev_status.msh_zone_map_is_exhausted ==
          status->msh_zone_map_is_exhausted &&
      prev_status.msh_compressor_exhausted ==
          status->msh_compressor_exhausted &&
      prev_status.msh_swap_low_on_space == status->msh_swap_low_on_space &&
      prev_status.msh_swap_exhausted == status->msh_swap_exhausted
#if CONFIG_JETSAM
      && prev_status.msh_available_pages_below_idle ==
             status->msh_available_pages_below_idle &&
      prev_status.msh_available_pages_below_soft ==
          status->msh_available_pages_below_soft &&
      prev_status.msh_available_pages_below_critical ==
          status->msh_available_pages_below_critical &&
      prev_status.msh_available_pages_below_reaper ==
          status->msh_available_pages_below_reaper &&
      prev_status.msh_compressor_needs_to_swap ==
          status->msh_compressor_needs_to_swap &&
      prev_status.msh_compressor_is_thrashing ==
          status->msh_compressor_is_thrashing &&
      prev_status.msh_filecache_is_thrashing ==
          status->msh_filecache_is_thrashing &&
      prev_status.msh_phantom_cache_pressure ==
          status->msh_phantom_cache_pressure &&
      prev_status.msh_swapin_queue_over_limit ==
  ...
  if (healthy) {
    ...
    /* Unhealthy */
    memorystatus_log("memorystatus: System is unhealthy! "
                     "memorystatus_available_pages: %llu compressor_size:%u\n",
                     (uint64_t)MEMORYSTATUS_LOG_AVAILABLE_PAGES,
                     vm_compressor_pool_size());
    memorystatus_log(
        ...
        status->msh_available_pages_below_soft,
        status->msh_available_pages_below_reaper,
        status->msh_compressor_needs_to_swap, status->msh_compressor_exhausted,
        status->msh_compressor_is_thrashing, status->msh_filecache_is_thrashing,
...
#else  /* CONFIG_JETSAM */
  memorystatus_log("memorystatus: System is %s. memorystatus_available_pages: "
                   "%llu compressor_size:%u\n",
                   healthy ? "healthy" : "unhealthy",
                   (uint64_t)MEMORYSTATUS_LOG_AVAILABLE_PAGES,
                   vm_compressor_pool_size());
  if (!healthy) {
    memorystatus_log("memorystatus: {"
                     "\"compressor_exhausted\": %d, "
                     "\"zone_map_is_exhausted\": %d, "
                     "\"swap_low\": %d, "
                     "\"swap_exhausted\": %d"
                     ...
                     status->msh_compressor_exhausted,
                     status->msh_zone_map_is_exhausted,
                     status->msh_swap_low_on_space, status->msh_swap_exhausted);
  }
#endif /* CONFIG_JETSAM */
  prev_status = *status;
}

bool memstat_check_system_health(memorystatus_system_health_t status) {
 ...
 * to pick an action. It will continue performing memorystatus actions until
 * this function returns MEMORYSTATUS_KILL_NONE. At that point the thread will
 * block.
 */
...
memorystatus_pick_action(jetsam_state_t state, uint32_t *kill_cause,
  ...
  if (status.msh_available_pages_below_soft || !is_system_healthy) {
    /*
     * If swap is enabled, first check if we're running low or are out of swap
     * space.
     */
    if (memorystatus_swap_all_apps && jetsam_kill_on_low_swap) {
      ...
      } else if (suspended_swappable_apps_remaining &&
    ...
    if (memorystatus_swap_all_apps) {
      if (status.msh_swappable_compressor_segments_over_limit &&
          !vm_swapout_thread_running &&
          !os_atomic_load(&vm_swapout_wake_pending, relaxed)) {
        /*
         * TODO: The swapper will keep running until it has drained the entire
         * early swapout queue. That might be overly aggressive & we should look
         * into tuning it. See rdar://84102304.
         */
        ...
        memorystatus_log_info(
            "memorystatus: Skipping swap wakeup because the swap thread is "
            "already running. vm_swapout_thread_running=%d, "
            "vm_swapout_wake_pending=%d\n",
            vm_swapout_thread_running,
            os_atomic_load(&vm_swapout_wake_pending, relaxed));
      }
    }
  ...
  if (status.msh_available_pages_below_idle && memstat_get_idle_proccnt() > 0 &&
      is_system_healthy) {
    /*
  ...
  *kill_cause = memorystatus_pick_kill_cause(&status);
  ...
  if (status.msh_vm_pressure_critical) {
    /*
     * The system is under critical memory pressure. First terminate any
     * low-risk idle processes. When they are exhausted, purge system memory
     * caches.
     */
    ...
    if (memstat_pressure_config & MEMSTAT_CRITICAL_PURGE_CACHES) {
      uint64_t now = mach_absolute_time();
      uint64_t delta_ns;
      uint64_t last_purge_ts =
          os_atomic_load(&memstat_last_cache_purge_ts, relaxed);
...
static bool memorystatus_aggressive_jetsam_needed_sysproc_aging(
    __unused int jld_eval_aggressive_count, __unused int *jld_idle_kills,
    __unused int jld_idle_kill_candidates, int *total_candidates);

/*
 * kJetsamHighRelaunchCandidatesThreshold defines the percentage of candidates
 * in the idle & deferred bands that need to be bad candidates in order to
 * trigger aggressive jetsam.
 */
TUNABLE_DEV_WRITEABLE(unsigned int, kJetsamHighRelaunchCandidatesThreshold,
                      "jetsam_high_relaunch_candidates_threshold_percent", 100);
#if DEVELOPMENT || DEBUG
SYSCTL_UINT(_kern, OID_AUTO, jetsam_high_relaunch_candidates_threshold_percent,
            CTLFLAG_RW | CTLFLAG_LOCKED,
            &kJetsamHighRelaunchCandidatesThreshold, 100, "");
#endif /* DEVELOPMENT || DEBUG */

/* kJetsamMinCandidatesThreshold defines the minimum number of candidates in the
 * idle/deferred bands to trigger aggressive jetsam. This value basically
 * decides how much memory the system is ready to hold in the lower bands
 * without triggering aggressive jetsam. This number should ideally be tuned
 * based on the memory config of the device.
 */
TUNABLE_DT_DEV_WRITEABLE(unsigned int, kJetsamMinCandidatesThreshold,
                         "/defaults", "kern.jetsam_min_candidates_threshold",
                         "jetsam_min_candidates_threshold", 5,
                         TUNABLE_DT_CHECK_CHOSEN);
#if DEVELOPMENT || DEBUG
SYSCTL_UINT(_kern, OID_AUTO, jetsam_min_candidates_threshold,
            CTLFLAG_RW | CTLFLAG_LOCKED, &kJetsamMinCandidatesThreshold, 5, "");
#endif /* DEVELOPMENT || DEBUG */
...
static bool memorystatus_check_aggressive_jetsam_needed(int *jld_idle_kills) {
   ...
   */
  aggressive_jetsam_needed =
      memorystatus_aggressive_jetsam_needed_sysproc_aging(
          jld_eval_aggressive_count, jld_idle_kills, jld_idle_kill_candidates,
          &total_candidates);

  /*
   * It is also possible that the system is down to a very small number of
   * processes in the candidate bands. In that case, the decisions made by the
   * memorystatus_aggressive_jetsam_needed_* routines would not be useful. In
   * that case, do not trigger aggressive jetsam.
   */
  if (total_candidates < kJetsamMinCandidatesThreshold) {
    memorystatus_log_debug("memorystatus: aggressive: [FAILED] Low Candidate "
   ...
   * parameters have been refreshed. This logic also resets the
   * jld_eval_aggressive_count counter to make sure we reset the aggressive
   * jetsam severity.
   */
  if ((total_candidates == 0) ||
      (jld_now_msecs >
       (jld_timestamp_msecs + memorystatus_jld_eval_period_msecs))) {
...
static bool memorystatus_aggressive_jetsam_needed_sysproc_aging(
    __unused int eval_aggressive_count, __unused int *idle_kills,
    __unused int idle_kill_candidates, int *total_candidates) {
  bool aggressive_jetsam_needed = false;

  /*
   * For the kJetsamAgingPolicySysProcsReclaimedFirst aging policy, we maintain
   * the jetsam relaunch behavior for all daemons. Also, daemons and apps are
   * aged in deferred bands on every dirty->clean transition. For this aging
   * policy, the best way to determine if aggressive jetsam is needed, is to see
   * if the kill candidates are mostly bad candidates. If yes, then we need to
   * go to higher bands to reclaim memory.
   */
  proc_list_lock();
  /* Get total candidate counts for idle and idle deferred bands */
  *total_candidates = memstat_bucket[JETSAM_PRIORITY_IDLE].count +
                      memstat_bucket[system_procs_aging_band].count;
  /* Get counts of bad kill candidates in idle and idle deferred bands */
  int bad_candidates =
      memstat_bucket[JETSAM_PRIORITY_IDLE].relaunch_high_count +
      memstat_bucket[system_procs_aging_band].relaunch_high_count;

  proc_list_unlock();

  /* Check if the number of bad candidates is greater than
   * kJetsamHighRelaunchCandidatesThreshold % */
  aggressive_jetsam_needed = (((bad_candidates * 100) / *total_candidates) >=
                              kJetsamHighRelaunchCandidatesThreshold);

  /*
   * Since the new aging policy bases the aggressive jetsam trigger on
   * percentage of bad candidates, it is prone to being overly aggressive. In
   * order to mitigate that, make sure the system is really under memory
   * pressure before triggering aggressive jetsam.
   */
...
#if DEVELOPMENT || DEBUG
  memorystatus_log_info(
      "memorystatus: aggressive%d: [%s] Bad Candidate Threshold Check (total: "
      "%d, bad: %d, threshold: %d %%); Memory Pressure Check (available_pgs: "
      "%llu, threshold_pgs: %llu)\n",
      eval_aggressive_count, aggressive_jetsam_needed ? "PASSED" : "FAILED",
      *total_candidates, bad_candidates, kJetsamHighRelaunchCandidatesThreshold,
      (uint64_t)MEMORYSTATUS_LOG_AVAILABLE_PAGES,
      (uint64_t)memorystatus_sysproc_aging_aggr_pages);
#endif /* DEVELOPMENT || DEBUG */
  return aggressive_jetsam_needed;
}
...
bool memorystatus_is_process_eligible_for_freeze(proc_t p) {
  ...
  if (isSysProc(p)) {
    ...
    proc_t leader_proc =
        memorystatus_get_coalition_leader_and_role(p, &task_role_in_coalition);
  ...
  if (pages < memorystatus_freeze_pages_min) {
    ...
    p->p_memstat_freeze_skip_reason =
        kMemorystatusFreezeSkipReasonBelowMinPages;
    goto out;
  }

  /* Don't freeze processes that are already exiting on core. It may have
   * started exiting after we chose it for freeze, but before we obtained the
   * proc_list_lock. NB: This is only possible if we're coming in from
   * memorystatus_freeze_process_sync. memorystatus_freeze_top_process holds the
   * proc_list_lock while it traverses the bands.
   */
  ...
  if (!(state & P_MEMSTAT_FROZEN) &&
      p->p_memstat_effectivepriority > memorystatus_freeze_max_candidate_band) {
    /*
...
out:
  if (should_freeze && !(state & P_MEMSTAT_FROZEN)) {
    /*
     * Reset the skip reason. If it's killed before we manage to actually freeze
     * it we failed to consider it early enough.
     */
    p->p_memstat_freeze_skip_reason = kMemorystatusFreezeSkipReasonNone;
    if (!first_consideration) {
      /*
       * We're freezing this for the first time and we previously considered it
       * ineligible. Bump the considered count so that we track this as 1
...
static proc_t memorystatus_freeze_pick_refreeze_process(proc_t last_p) {
  ...
  while (next_p) {
    ...
    if ((p->p_memstat_state & P_MEMSTAT_FROZEN) &&
        !memorystatus_freeze_proc_is_refreeze_eligible(p)) {
    ...
    /*
     * Don't refreeze the last process we just thawed if still within the
     * timeout window
     */
    if (p->p_pid == memorystatus_freeze_last_pid_thawed) {
      uint64_t timeout_delta_abs;
      nanoseconds_to_absolutetime(
          FREEZE_PREVENT_REFREEZE_OF_LAST_THAWED_TIMEOUT_SECONDS * NSEC_PER_SEC,
          &timeout_delta_abs);
      if (mach_absolute_time() <
          (memorystatus_freeze_last_pid_thawed_ts + timeout_delta_abs)) {
        continue;
      }
    }
#endif
...
proc_t memorystatus_freeze_pick_process(
  ...
  if (iterator->refreeze_only ||
    ...
    if (!iterator->refreeze_only) {
       ...
       * candidates, we need to reset the last pointer b/c it's pointing into
       * the wrong band.
       */
    ...
    iterator->last_p =
        memorystatus_freeze_pick_refreeze_process(iterator->last_p);
  ...
  if (memorystatus_freezer_use_ordered_list) {
    while (iterator->global_freeze_list_index <
      ...
      if (p != PROC_NULL && memorystatus_is_process_eligible_for_freeze(p)) {
        ...
        if (p->p_pid == memorystatus_freeze_last_pid_thawed) {
          uint64_t timeout_delta_abs;
          nanoseconds_to_absolutetime(
              FREEZE_PREVENT_REFREEZE_OF_LAST_THAWED_TIMEOUT_SECONDS *
                  NSEC_PER_SEC,
              &timeout_delta_abs);
          if (mach_absolute_time() <
              (memorystatus_freeze_last_pid_thawed_ts + timeout_delta_abs)) {
    ...
    if (iterator->last_p == PROC_NULL) {
      ...
      next_p =
          memorystatus_get_next_proc_locked(&band, iterator->last_p, FALSE);
    }
    while (next_p) {
      p = next_p;
   ...
   * Try to re-freeze.
   */
  if (memorystatus_refreeze_eligible_count >=
 ...
 * of available pages changes. It wakes the freezer thread iff the function
 * returns true. The freezer thread will try to freeze (or refreeze) up to 1
 * process before blocking again.
 *
 * Note the freezer thread is also woken up by memorystatus_on_inactivity.
 */

bool memorystatus_freeze_thread_should_run() {
  /*
  ...
  if (memorystatus_frozen_shared_mb_max &&
...
size_t memorystatus_pick_freeze_count_for_wakeup() {
  size_t num_to_freeze = 0;
  if (!memorystatus_swap_all_apps) {
    num_to_freeze = 1;
  } else {
    /*
     * When app swap is enabled, we want the freezer thread to aggressively
     * freeze all candidates so we clear out space for the fg working set. But
     * we still cap it to the current size of the candidate bands to avoid
     * consuming excessive CPU if there's a lot of churn in the candidate band.
     */
    proc_list_lock();
    for (unsigned int band = JETSAM_PRIORITY_IDLE;
         band <= memorystatus_freeze_max_candidate_band; band++) {
