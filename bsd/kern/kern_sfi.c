...
int sfi_ctl(struct proc *p __unused, struct sfi_ctl_args *uap,
...
static int proc_apply_sfi_managed(proc_t p, void *arg) {
  ...
  if (managed_enabled) {
    KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_PID_SET_MANAGED) |
                              DBG_FUNC_NONE,
                          pid, 0, 0, 0, 0);
  } else {
    KERNEL_DEBUG_CONSTANT(MACHDBG_CODE(DBG_MACH_SFI, SFI_PID_CLEAR_MANAGED) |
                              DBG_FUNC_NONE,
                          pid, 0, 0, 0, 0);
  }

  proc_set_task_policy(
      proc_task(p), TASK_POLICY_ATTRIBUTE, TASK_POLICY_SFI_MANAGED,
      managed_enabled ? TASK_POLICY_ENABLE : TASK_POLICY_DISABLE);

  return PROC_RETURNED;
}

int sfi_pidctl(struct proc *p __unused, struct sfi_pidctl_args *uap,
               int32_t *retval __unused) {
  uint32_t operation = uap->operation;
  pid_t pid = uap->pid;
  int error = 0;
  ...
  case SFI_PIDCTL_OPERATION_PID_SET_FLAGS:
    ...
    if (uap->pid == 0) {
      ...
      proc_iterate(PROC_ALLPROCLIST, proc_apply_sfi_managed,
                   (void *)&uap->sfi_flags, NULL, NULL);
      break;
    }
  ...
  case SFI_PIDCTL_OPERATION_PID_GET_FLAGS:
    ...
    managed_enabled = proc_get_task_policy(
        proc_task(targetp), TASK_POLICY_ATTRIBUTE, TASK_POLICY_SFI_MANAGED);

    proc_rele(targetp);

    out_flags =
        managed_enabled ? SFI_PROCESS_SET_MANAGED : SFI_PROCESS_SET_UNMANAGED;
