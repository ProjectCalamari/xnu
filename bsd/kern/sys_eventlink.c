...
extern uint64_t mach_eventlink_signal_trap(mach_port_name_t port,
...
extern uint64_t mach_eventlink_signal_wait_until_trap(
...
int mach_eventlink_signal(__unused proc_t p,
...
int mach_eventlink_wait_until(__unused proc_t p,
                              struct mach_eventlink_wait_until_args *uap,
                              uint64_t *retval) {
  *retval =
      mach_eventlink_wait_until_trap(uap->eventlink_port, uap->wait_count,
                                     uap->option, uap->clock_id, uap->deadline);
  return 0;
}

int mach_eventlink_signal_wait_until(
    __unused proc_t p, struct mach_eventlink_signal_wait_until_args *uap,
    uint64_t *retval) {
  *retval = mach_eventlink_signal_wait_until_trap(
      uap->eventlink_port, uap->wait_count, uap->signal_count, uap->option,
      uap->clock_id, uap->deadline);
  return 0;
}
