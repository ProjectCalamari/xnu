 ...
 * kernel memory exploit will be able to corrupt code signing state. There is
...
developer_mode_enabled = &developer_mode_storage;

void xnu_toggle_developer_mode(bool state) {
...
kern_return_t xnu_rem_enable(void) { return KERN_NOT_SUPPORTED; }

kern_return_t xnu_rem_state(void) { return KERN_NOT_SUPPORTED; }

#pragma mark Device State

void xnu_update_device_state(void) { /* Does nothing */ }

void xnu_complete_security_boot_mode(__unused uint32_t security_boot_mode) {
  /* Does nothing */
}
...
void xnu_set_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
...
bool xnu_match_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
  bool match = false;
...
void xnu_set_local_signing_public_key(
  ...
  memcpy(local_signing_public_key, public_key,
         sizeof(local_signing_public_key));
}

uint8_t *xnu_get_local_signing_public_key(void) {
...
static uint8_t __attribute__((
    aligned(8))) _xnu_image4_storage[IMG4_PMAP_DATA_SIZE_RECOMMENDED] = {0};

void *xnu_image4_storage_data(size_t *allocated_size) {
  if (allocated_size) {
...
void xnu_image4_set_nonce(const img4_nonce_domain_index_t ndi,
  ...
  IMG4_RUNTIME_PMAP_CS->i4rt_set_nonce(IMG4_RUNTIME_PMAP_CS, ndi, nonce);
}

void xnu_image4_roll_nonce(const img4_nonce_domain_index_t ndi) {
  /*
   * As a hold over from legacy code, AppleImage4 only ever manages nonces
   * from the kernel interface through the PMAP_CS runtime. So even though
   * we don't have a PMAP_CS monitor, we still pass in the PMAP_CS runtime.
  ...
  IMG4_RUNTIME_PMAP_CS->i4rt_roll_nonce(IMG4_RUNTIME_PMAP_CS, ndi);
}

errno_t xnu_image4_copy_nonce(const img4_nonce_domain_index_t ndi,
  ...
  ret = IMG4_RUNTIME_PMAP_CS->i4rt_copy_nonce(IMG4_RUNTIME_PMAP_CS, ndi,
                                              nonce_out);
...
errno_t
xnu_image4_execute_object(img4_runtime_object_spec_index_t obj_spec_index,
  ...
  ret = img4_runtime_execute_object(IMG4_RUNTIME_PMAP_CS, obj_spec, payload,
                                    manifest);
...
errno_t xnu_image4_copy_object(img4_runtime_object_spec_index_t obj_spec_index,
  ...
  /*
   * The object length is used as an in/out parameter, so we require that this
   * parameter is used to specify the length of the buffer.
   */
  ...
  ret = img4_runtime_copy_object(IMG4_RUNTIME_PMAP_CS, obj_spec,
                                 &object_payload, &object_payload_length);
  if (ret != 0) {
...
const void *xnu_image4_get_monitor_exports(void) {
  printf("monitor exports not supported without a monitor\n");
  return NULL;
}

errno_t xnu_image4_set_release_type(__unused const char *release_type) {
  /*
...
errno_t
xnu_image4_set_bnch_shadow(__unused const img4_nonce_domain_index_t ndi) {
  /*
...
kern_return_t xnu_image4_transfer_region(image4_cs_trap_t selector,
                                         __unused vm_address_t region_addr,
                                         __unused vm_size_t region_size) {
  panic("image4 dispatch: transfer without code signing monitor: %llu",
        selector);
}

kern_return_t xnu_image4_reclaim_region(image4_cs_trap_t selector,
                                        __unused vm_address_t region_addr,
                                        __unused vm_size_t region_size) {
  panic("image4 dispatch: reclaim without code signing monitor: %llu",
        selector);
}

errno_t xnu_image4_monitor_trap(image4_cs_trap_t selector,
