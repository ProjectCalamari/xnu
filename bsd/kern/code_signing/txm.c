...
#if CONFIG_SPTM
/*
 * The TrustedExecutionMonitor environment works in tandem with the SPTM to
 * provide code signing and memory isolation enforcement for data structures
 * critical to ensuring that all code executed on the system is authorized to do
 * so.
 *
 * Unless the data is managed by TXM itself, XNU needs to page-align everything,
 * make the relevant type transfer, and then reference the memory as read-only.
 *
 * TXM enforces concurrency on its side, but through the use of try-locks. Upon
 * a failure in acquiring the lock, TXM will panic. As a result, in order to
 * ensure single-threaded behavior, the kernel also has to take some locks on
 * its side befor calling into TXM.
 */
...
SLIST_HEAD(thread_stack_head, _txm_thread_stack)
thread_stacks_head = SLIST_HEAD_INITIALIZER(thread_stacks_head);
...
static void setup_thread_stacks(void) {
  ...
  for (uint32_t i = 0; i < NUM_TXM_THREAD_STACKS; i++) {
    ...
    thread_stack->thread_stack_phys =
        (uintptr_t)kvtophys_nofail(thread_stack->thread_stack_papt);

    /* Resolve the pointer to the thread stack data */
    thread_stack->thread_stack_data =
        (TXMThreadStack_t *)(thread_stack->thread_stack_papt +
                             (PAGE_SIZE - 1024));
...
static txm_thread_stack_t *acquire_thread_stack(void) {
  txm_thread_stack_t *thread_stack = NULL;
  ...
  while (SLIST_EMPTY(&thread_stacks_head) == true) {
    lck_mtx_sleep(&thread_stacks_lock, LCK_SLEEP_DEFAULT, &thread_stack_event,
                  THREAD_UNINT);
  }
...
static void release_thread_stack(txm_thread_stack_t *thread_stack) {
...
static kern_return_t txm_parse_return(TXMReturn_t txm_ret) {
...
static void txm_print_return(TXMKernelSelector_t selector,
                             TXMReturn_t txm_ret) {
  /*
  ...
  if (txm_ret.returnCode == kTXMSuccess) {
    ...
    IOLog("TXM [Error]: TrustCache: selector: %u | 0x%02X | 0x%02X | %u\n",
          selector, txm_ret.tcRet.component, txm_ret.tcRet.error,
          txm_ret.tcRet.uniqueError);
  } else if (txm_ret.returnCode == kTXMReturnCodeSignature) {
    IOLog("TXM [Error]: CodeSignature: selector: %u | 0x%02X | 0x%02X | %u\n",
          selector, txm_ret.csRet.component, txm_ret.csRet.error,
          txm_ret.csRet.uniqueError);
...
static void txm_add_page(void) {
  txm_call_t txm_call = {.selector = kTXMKernelSelectorAddFreeListPage,
                         .failure_fatal = true,
                         .num_input_args = 1};
...
static void txm_kernel_call_registers_setup(txm_call_t *parameters,
...
static TXMReturn_t txm_kernel_call_internal(txm_call_t *parameters,
                                            va_list args) {
...
kern_return_t txm_kernel_call(txm_call_t *parameters, ...) {
  TXMReturn_t txm_ret = (TXMReturn_t){.returnCode = kTXMReturnGeneric};
  ...
  if (parameters->failure_fatal && (ret != KERN_SUCCESS)) {
    panic(
        "received fatal error for a selector from TXM: selector: %u | 0x%0llX",
    ...
    if (ret == KERN_SUCCESS) {
      panic("received fewer than expected return words from TXM: selector: %u "
            "| %llu",
            parameters->selector, parameters->num_return_words);
    }
  }
...
void txm_transfer_region(vm_address_t addr, vm_size_t size) {
  vm_address_t addr_end = 0;
  vm_size_t size_aligned = round_page(size);

  if ((addr & PAGE_MASK) != 0) {
    panic("attempted to transfer non-page-aligned memory to TXM: %p",
          (void *)addr);
  } else if (os_add_overflow(addr, size_aligned, &addr_end)) {
    panic("overflow on range to be transferred to TXM: %p | %lu", (void *)addr,
          size);
  }
...
void txm_reclaim_region(vm_address_t addr, vm_size_t size) {
  vm_address_t addr_end = 0;
  vm_size_t size_aligned = round_page(size);

  if ((addr & PAGE_MASK) != 0) {
    panic("attempted to reclaim non-page-aligned memory from TXM: %p",
          (void *)addr);
  } else if (os_add_overflow(addr, size_aligned, &addr_end)) {
    panic("overflow on range to be reclaimed from TXM: %p | %lu", (void *)addr,
          size);
  }
   ...
   */
  vm_protect(kernel_map, addr, size_aligned, false,
...
void txm_print_logs(void) {
  uint32_t start_index = 0;
  uint32_t end_index = 0;

  /*
   * The design here is very simple. TXM keeps adding slots to its circular
   * buffer and the kernel attempts to read each one and print it, maintaining
   * its own head for the log.
   *
   * This design is by nature lazy. TXM doesn't know or care if the kernel has
   * gone through and printed any of the logs, so it'll just keep writing into
   * its buffer and then circle around when it becomes full.
   *
   * This is fine most of the time since there are a decent amount of slots in
   * the log buffer. We mostly have an issue when TXM is adding so many logs so
   * quickly such that it wraps around and starts overwriting logs which haven't
   * been seen by the kernel. If this were to happen, TXM's log head may circle
   * around the head maintained by the kernel, causing a lot of logs to be
   * missed, since the kernel only attempts the number of logs in-between the
   * two heads.
   *
   * The fix for that is complicated, and until we see an actual impact, we're
   * going to keep the simpler design in place.
   */
  ...
  /*
   * Holding the log lock and printing can cause lots of issues since printing
   * can be rather slow. While we make it a point to keep the logging buffer
   * quiet, some actions (such as loading trust caches) are still very chatty.
   *
   * As a result, we optimize this routine to ensure that the lock itself isn't
   * held for very long. All we need to do within the critical section is
   * calculate the starting and ending index of the log buffer. The actual
   * printing doesn't need to be done with the lock held.
   */
...
SECURITY_READ_ONLY_LATE(CSRestrictedModeState_t *)
txm_restricted_mode_state = NULL;
...
static void get_logging_info(void) {
  txm_call_t txm_call = {.selector = kTXMKernelSelectorGetLogInfo,
                         .failure_fatal = true,
                         .num_output_args = 3};
...
static void get_code_signing_info(void) {
  txm_call_t txm_call = {.selector = kTXMKernelSelectorGetCodeSigningInfo,
                         .failure_fatal = true,
                         .num_output_args = 6};
  txm_kernel_call(&txm_call);
  ...
  num_static_trust_caches =
      os_atomic_load(&txm_metrics->trustCaches.numStatic, relaxed);
  num_engineering_trust_caches =
      os_atomic_load(&txm_metrics->trustCaches.numEngineering, relaxed);
}
...
void txm_enter_lockdown_mode(void) {
  txm_call_t txm_call = {
...
kern_return_t txm_secure_channel_shared_page(uint64_t *secure_channel_phys,
                                             size_t *secure_channel_size) {
#if kTXMKernelAPIVersion >= 5
  txm_call_t txm_call = {.selector = kTXMKernelSelectorGetSecureChannelAddr,
                         .num_output_args = 2};
...
void txm_toggle_developer_mode(bool state) {
  txm_call_t txm_call = {.selector = kTXMKernelSelectorDeveloperModeToggle,
                         .failure_fatal = true,
                         .num_input_args = 1};
...
kern_return_t txm_rem_enable(void) {
  txm_call_t txm_call = {.selector = kTXMKernelSelectorEnableRestrictedMode};
  return txm_kernel_call(&txm_call);
}

kern_return_t txm_rem_state(void) {
...
void txm_update_device_state(void) {
#if kTXMKernelAPIVersion >= 6
  txm_call_t txm_call = {.selector = kTXMSelectorUpdateDeviceState,
                         .failure_fatal = true};
  txm_kernel_call(&txm_call);
#endif
}

void txm_complete_security_boot_mode(__unused uint32_t security_boot_mode) {
#if kTXMKernelAPIVersion >= 6
  txm_call_t txm_call = {.selector = kTXMSelectorCompleteSecurityBootMode,
                         .num_input_args = 1,
                         .failure_fatal = true};
...
bool txm_code_signing_enabled(void) { return code_signing_enabled; }

vm_size_t txm_managed_code_signature_size(void) {
  return managed_signature_size;
}

kern_return_t txm_register_provisioning_profile(const void *profile_blob,
                                                const size_t profile_blob_size,
                                                void **profile_obj) {
  txm_call_t txm_call = {.selector =
                             kTXMKernelSelectorRegisterProvisioningProfile,
                         .num_input_args = 2,
                         .num_output_args = 1};
...
kern_return_t txm_trust_provisioning_profile(__unused void *profile_obj,
                                             __unused const void *sig_data,
                                             __unused size_t sig_size) {
#if kTXMKernelAPIVersion >= 7
  txm_call_t txm_call = {.selector = kTXMKernelSelectorTrustProvisioningProfile,
                         .num_input_args = 3};
...
kern_return_t txm_unregister_provisioning_profile(void *profile_obj) {
  txm_call_t txm_call = {.selector =
                             kTXMKernelSelectorUnregisterProvisioningProfile,
                         .num_input_args = 1,
                         .num_output_args = 2};
...
kern_return_t txm_associate_provisioning_profile(void *sig_obj,
                                                 void *profile_obj) {
  txm_call_t txm_call = {
...
kern_return_t txm_disassociate_provisioning_profile(void *sig_obj) {
  txm_call_t txm_call = {
...
void txm_set_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
  txm_call_t txm_call = {
...
bool txm_match_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
  txm_call_t txm_call = {
...
void txm_set_local_signing_public_key(
...
uint8_t *txm_get_local_signing_public_key(void) {
  txm_call_t txm_call = {
...
void txm_unrestrict_local_signing_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
  txm_call_t txm_call = {
...
kern_return_t txm_register_code_signature(
    ...
    void **sig_obj, vm_address_t *txm_signature_addr) {
  txm_call_t txm_call = {
   ...
   * out the best code directory to use on its own. As a result, this offset
   * here is not used.
   */
  ...
  ret = txm_kernel_call(&txm_call, signature_addr, signature_size,
                        signature_path);
...
exit:
  if ((ret != KERN_SUCCESS) &&
      (signature_size > txm_managed_code_signature_size())) {
...
kern_return_t txm_unregister_code_signature(void *sig_obj) {
  txm_call_t txm_call = {
   ...
   * set of other functions. The unregistration operation is very common when
   * the system is about to reboot because the VFS layer unmounts all volumes.
   *
  ...
  if ((cs_obj->referenceCount > 0) && (get_system_inshutdown() != 0)) {
    printf("TXM [XNU]: unregistration of signature skipped as system is in "
           "shutdown\n");
    return KERN_ABORTED;
  }
...
kern_return_t txm_verify_code_signature(void *sig_obj, uint32_t *trust_level) {
  txm_call_t txm_call = {
  ...
  if ((ret == KERN_SUCCESS) && (trust_level != NULL)) {
    /*
     * Abolsutely gross, but it's not worth linking all of libCodeSignature just
     * for this simple change. We should either return the trust level from TXM,
     * or when we adopt libCodeSignature more broadly, then use an accessor
     * function.
     */
...
kern_return_t txm_reconstitute_code_signature(void *sig_obj,
...
kern_return_t txm_register_address_space(pmap_t pmap, uint16_t addr_space_id,
...
kern_return_t txm_unregister_address_space(pmap_t pmap) {
  txm_call_t txm_call = {
...
kern_return_t txm_setup_nested_address_space(pmap_t pmap,
                                             const vm_address_t region_addr,
                                             const vm_size_t region_size) {
  txm_call_t txm_call = {.selector = kTXMKernelSelectorSetupNestedAddressSpace,
                         .num_input_args = 3};
  TXMAddressSpace_t *txm_addr_space = pmap_txm_addr_space(pmap);
  kern_return_t ret = KERN_DENIED;
...
kern_return_t txm_associate_code_signature(pmap_t pmap, void *sig_obj,
                                           const vm_address_t region_addr,
                                           const vm_size_t region_size,
  ...
  /*
   * Associating a code signature may require exclusive access to the TXM
   * address space lock within TXM.
   */
  pmap_txm_acquire_exclusive_lock(pmap);

  /*
   * If the address space in question is a nested address space, then all
   * associations need to go into the shared region base range. The VM layer is
   * inconsistent with how it makes associations with TXM vs. how it maps pages
   * into the shared region.
   *
   * For TXM, the associations are made without taking the base range into
   * account, but when mappings are entered into the shared region, the base
   * range is taken into account. To normalize this, we add the base range
   * address here.
   */
  ...
  /*
   * The VM tries a bunch of weird mappings within launchd for some platform
   * code which isn't mapped contiguously. These mappings don't succeed, but the
   * failure is fairly harmless since everything seems to work. However, since
   * the call to TXM fails, we make a series of logs. Hence, for launchd, we
  ...
  ret = txm_kernel_call(&txm_call, txm_addr_space, sig_obj,
                        adjusted_region_addr, region_size, region_offset);
  ...
  /*
   * If the main region wasn't set on the address space before hand, but this
   * new call into TXM was successful and sets the main region, it means this
   * signature object is associated with the main region on the address space.
   * With this, we can now set the appropriate trust level on the PMAP.
   */
  if (ret == KERN_SUCCESS) {
...
kern_return_t txm_allow_jit_region(pmap_t pmap) {
  txm_call_t txm_call = {
...
kern_return_t txm_associate_jit_region(pmap_t pmap,
                                       const vm_address_t region_addr,
                                       const vm_size_t region_size) {
  txm_call_t txm_call = {
...
kern_return_t txm_address_space_debugged(pmap_t pmap) {
  TXMAddressSpace_t *txm_addr_space = pmap_txm_addr_space(pmap);
  ...
  pmap_txm_acquire_shared_lock(pmap);
  debug_regions_allowed =
      os_atomic_load(&txm_addr_space->allowsInvalidCode, relaxed);
  pmap_txm_release_shared_lock(pmap);
...
kern_return_t txm_associate_debug_region(pmap_t pmap,
                                         const vm_address_t region_addr,
                                         const vm_size_t region_size) {
...
kern_return_t txm_allow_invalid_code(pmap_t pmap) {
  txm_call_t txm_call = {
...
kern_return_t txm_get_trust_level_kdp(pmap_t pmap, uint32_t *trust_level) {
...
kern_return_t txm_get_jit_address_range_kdp(pmap_t pmap,
                                            uintptr_t *jit_region_start,
                                            uintptr_t *jit_region_end) {
  return pmap_txm_get_jit_address_range_kdp(pmap, jit_region_start,
                                            jit_region_end);
}

kern_return_t txm_address_space_exempt(const pmap_t pmap) {
...
kern_return_t txm_fork_prepare(pmap_t old_pmap, pmap_t new_pmap) {
  /*
...
kern_return_t txm_acquire_signing_identifier(const void *sig_obj,
...
kern_return_t
txm_associate_kernel_entitlements(void *sig_obj,
...
kern_return_t
txm_resolve_kernel_entitlements(pmap_t pmap, const void **kernel_entitlements) {
  txm_call_t txm_call = {
...
kern_return_t txm_accelerate_entitlements(void *sig_obj,
...
void *txm_image4_storage_data(__unused size_t *allocated_size) {
  /*
   * AppleImage4 builds a variant of TXM which TXM should link against
   * statically thereby removing the need for the kernel to allocate some data
   * on behalf of the kernel extension.
   */
  panic("unsupported AppleImage4 interface");
}

void txm_image4_set_nonce(const img4_nonce_domain_index_t ndi,
...
void txm_image4_roll_nonce(const img4_nonce_domain_index_t ndi) {
  txm_call_t txm_call = {
...
errno_t txm_image4_copy_nonce(const img4_nonce_domain_index_t ndi,
...
errno_t
txm_image4_execute_object(img4_runtime_object_spec_index_t obj_spec_index,
...
errno_t txm_image4_copy_object(img4_runtime_object_spec_index_t obj_spec_index,
...
const void *txm_image4_get_monitor_exports(void) {
  txm_call_t txm_call = {
...
errno_t txm_image4_set_release_type(const char *release_type) {
  txm_call_t txm_call = {
...
errno_t txm_image4_set_bnch_shadow(const img4_nonce_domain_index_t ndi) {
  txm_call_t txm_call = {
...
static inline bool
_txm_image4_monitor_trap_supported(image4_cs_trap_t selector) {
  switch (selector) {
...
kern_return_t txm_image4_transfer_region(image4_cs_trap_t selector,
...
kern_return_t txm_image4_reclaim_region(image4_cs_trap_t selector,
                                        vm_address_t region_addr,
                                        vm_size_t region_size) {
  if (_txm_image4_monitor_trap_supported(selector) == true) {
...
errno_t txm_image4_monitor_trap(image4_cs_trap_t selector,
  ...
  kern_return_t ret =
      txm_kernel_call(&txm_call, selector, input_data, input_size, NULL, NULL);
...
#define TXM_METRIC(type, name, field)                                          \
  ...
  SYSCTL_PROC(_txm_metrics_##type, OID_AUTO, name, CTLTYPE_INT | CTLFLAG_RD,   \
              NULL, 0, __txm_metric_##type##_##name, "I",                      \
              "collected data from \'" #type "\':\'" #field "\'");             \
  static int __txm_metric_##type##_##name SYSCTL_HANDLER_ARGS {                \
    if (req->newptr) {                                                         \
      return EPERM;                                                            \
    }                                                                          \
...
SYSCTL_NODE(_txm_metrics, OID_AUTO, memory, CTLFLAG_RD, 0,
            "TXM Metrics - Memory");

#define TXM_ALLOCATOR_METRIC(name, field)                                      \
  SYSCTL_DECL(_txm_metrics_memory_##name);                                     \
  SYSCTL_NODE(_txm_metrics_memory, OID_AUTO, name, CTLFLAG_RD, 0,              \
              "\'" #name "\' allocator");                                      \
...
SYSCTL_NODE(_txm_metrics, OID_AUTO, acceleration, CTLFLAG_RD, 0,
            "TXM Metrics - Acceleration");
...
SYSCTL_NODE(_txm_metrics, OID_AUTO, trustcaches, CTLFLAG_RD, 0,
            "TXM Metrics - Trust Caches");
