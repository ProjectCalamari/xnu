...
/*
 * The Page Protection Layer layer implements the PMAP_CS monitor environment
 * which provides code signing and memory isolation enforcements for data
 * structures which are critical to ensuring that all code executed on the
 * system is authorized to do so.
 *
 * Unless the data is managed by the PPL itself, XNU needs to page-align
 * everything, and then reference the memory as read-only.
 */
...
void code_signing_init() { /* Does nothing */ }

void ppl_enter_lockdown_mode(void) {
  /*
...
kern_return_t
ppl_secure_channel_shared_page(__unused uint64_t *secure_channel_phys,
...
developer_mode_enabled = &ppl_developer_mode_storage;

void ppl_toggle_developer_mode(bool state) {
  pmap_toggle_developer_mode(state);
}

#pragma mark Restricted Execution Mode

kern_return_t ppl_rem_enable(void) { return KERN_NOT_SUPPORTED; }

kern_return_t ppl_rem_state(void) { return KERN_NOT_SUPPORTED; }

#pragma mark Device State

void ppl_update_device_state(void) { /* Does nothing */ }

void ppl_complete_security_boot_mode(__unused uint32_t security_boot_mode) {
  /* Does nothing */
}

#pragma mark Code Signing and Provisioning Profiles

bool ppl_code_signing_enabled(void) { return pmap_cs_enabled(); }

kern_return_t ppl_register_provisioning_profile(const void *profile_blob,
  ...
  if (os_add_overflow(sizeof(*pmap_payload), profile_blob_size,
                      &payload_size)) {
    panic("attempted to load a too-large profile: %lu bytes",
          profile_blob_size);
  }
  ...
  if (ret == KERN_SUCCESS) {
    ...
    *profile_obj = (pmap_cs_profile_t *)phystokv(
        kvtophys_nofail((vm_offset_t)*profile_obj));
  }
...
kern_return_t ppl_trust_provisioning_profile(__unused void *profile_obj,
...
kern_return_t ppl_unregister_provisioning_profile(void *profile_obj) {
  ...
  const pmap_profile_payload_t *pmap_payload =
      ppl_profile_obj->original_payload;
  const vm_address_t payload_addr = (const vm_address_t)pmap_payload;

  /* Get the original payload size */
  vm_size_t payload_size =
      pmap_payload->profile_blob_size + sizeof(*pmap_payload);
...
kern_return_t ppl_associate_provisioning_profile(void *sig_obj,
                                                 void *profile_obj) {
  return pmap_associate_provisioning_profile(sig_obj, profile_obj);
}

kern_return_t ppl_disassociate_provisioning_profile(void *sig_obj) {
  return pmap_disassociate_provisioning_profile(sig_obj);
}

void ppl_set_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
  pmap_set_compilation_service_cdhash(cdhash);
}

bool ppl_match_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
  return pmap_match_compilation_service_cdhash(cdhash);
}

void ppl_set_local_signing_public_key(
    const uint8_t public_key[XNU_LOCAL_SIGNING_KEY_SIZE]) {
  return pmap_set_local_signing_public_key(public_key);
}

uint8_t *ppl_get_local_signing_public_key(void) {
  return pmap_get_local_signing_public_key();
}

void ppl_unrestrict_local_signing_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
  pmap_unrestrict_local_signing(cdhash);
}

vm_size_t ppl_managed_code_signature_size(void) { return pmap_cs_blob_limit; }

kern_return_t ppl_register_code_signature(
    ...
    void **sig_obj, vm_address_t *ppl_signature_addr) {
  ...
  kern_return_t ret = pmap_cs_register_code_signature_blob(
      signature_addr, signature_size, code_directory_offset,
...
kern_return_t ppl_unregister_code_signature(void *sig_obj) {
  return pmap_cs_unregister_code_signature_blob(sig_obj);
}

kern_return_t ppl_verify_code_signature(void *sig_obj, uint32_t *trust_level) {
...
kern_return_t ppl_reconstitute_code_signature(void *sig_obj,
                                              vm_address_t *unneeded_addr,
                                              vm_size_t *unneeded_size) {
  return pmap_cs_unlock_unneeded_code_signature(sig_obj, unneeded_addr,
                                                unneeded_size);
}
...
kern_return_t
ppl_setup_nested_address_space(__unused pmap_t pmap,
                               __unused const vm_address_t region_addr,
                               __unused const vm_size_t region_size) {
  /*
   * We don't need to do anything here from the code-signing-monitor's
   * perspective because the PMAP's base address fields are setup when someone
   * eventually calls pmap_nest on the PMAP object.
   */
  return KERN_SUCCESS;
}

kern_return_t ppl_associate_code_signature(pmap_t pmap, void *sig_obj,
                                           const vm_address_t region_addr,
                                           const vm_size_t region_size,
                                           const vm_offset_t region_offset) {
  return pmap_cs_associate(pmap, sig_obj, region_addr, region_size,
                           region_offset);
}

kern_return_t ppl_allow_jit_region(__unused pmap_t pmap) {
  /* PPL does not support this API */
  return KERN_NOT_SUPPORTED;
}

kern_return_t ppl_associate_jit_region(pmap_t pmap,
                                       const vm_address_t region_addr,
                                       const vm_size_t region_size) {
  return pmap_cs_associate(pmap, PMAP_CS_ASSOCIATE_JIT, region_addr,
                           region_size, 0);
}

kern_return_t ppl_associate_debug_region(pmap_t pmap,
                                         const vm_address_t region_addr,
                                         const vm_size_t region_size) {
  ...
  return pmap_cs_associate(pmap, PMAP_CS_ASSOCIATE_COW, region_addr,
                           region_size, 0);
}

kern_return_t ppl_address_space_debugged(pmap_t pmap) {
  /*
...
kern_return_t ppl_allow_invalid_code(pmap_t pmap) {
  return pmap_cs_allow_invalid(pmap);
}

kern_return_t ppl_get_trust_level_kdp(pmap_t pmap, uint32_t *trust_level) {
  return pmap_get_trust_level_kdp(pmap, trust_level);
}

kern_return_t ppl_get_jit_address_range_kdp(pmap_t pmap,
...
kern_return_t ppl_address_space_exempt(const pmap_t pmap) {
...
kern_return_t ppl_fork_prepare(pmap_t old_pmap, pmap_t new_pmap) {
  return pmap_cs_fork_prepare(old_pmap, new_pmap);
}

kern_return_t ppl_acquire_signing_identifier(const void *sig_obj,
...
kern_return_t
ppl_associate_kernel_entitlements(void *sig_obj,
...
kern_return_t
ppl_resolve_kernel_entitlements(pmap_t pmap, const void **kernel_entitlements) {
  kern_return_t ret = KERN_DENIED;
...
kern_return_t ppl_accelerate_entitlements(void *sig_obj,
...
void *ppl_image4_storage_data(size_t *allocated_size) {
  return pmap_image4_pmap_data(allocated_size);
}

void ppl_image4_set_nonce(const img4_nonce_domain_index_t ndi,
                          const img4_nonce_t *nonce) {
  return pmap_image4_set_nonce(ndi, nonce);
}

void ppl_image4_roll_nonce(const img4_nonce_domain_index_t ndi) {
  return pmap_image4_roll_nonce(ndi);
}

errno_t ppl_image4_copy_nonce(const img4_nonce_domain_index_t ndi,
                              img4_nonce_t *nonce_out) {
  return pmap_image4_copy_nonce(ndi, nonce_out);
}

errno_t
ppl_image4_execute_object(img4_runtime_object_spec_index_t obj_spec_index,
  ...
  /*
   * The PPL will attempt to lockdown both the payload and the manifest before
   * executing the object. In order for that to happen, both the artifacts need
   * to be page-aligned.
   */
  ...
  kr = kmem_alloc(kernel_map, &payload_addr, payload_len_aligned, KMA_KOBJECT,
  ...
  if (manifest != NULL) {
    kr = kmem_alloc(kernel_map, &manifest_addr, manifest_len_aligned,
                    KMA_KOBJECT, VM_KERN_MEMORY_SECURITY);
  ...
  err = pmap_image4_execute_object(obj_spec_index, &payload_aligned,
                                   &manifest_aligned);
  if (err != 0) {
...
errno_t ppl_image4_copy_object(img4_runtime_object_spec_index_t obj_spec_index,
  ...
  /*
   * The PPL will attempt to pin the input buffer in order to ensure that the
   * kernel didn't pass in PPL-owned buffers. The PPL cannot pin the same page
   * more than once, and attempting to do so will panic the system. Hence, we
   * allocate fresh pages for for the PPL to pin.
   *
   * We can send in the address for the length pointer since that is allocated
   * on the stack, so the PPL can pin our stack for the duration of the call as
   * no other thread can be using our stack, meaning the PPL will never attempt
   * to double-pin the page.
   */
  object_len_aligned = round_page(*object_length);

  kr = kmem_alloc(kernel_map, &object_addr, object_len_aligned, KMA_KOBJECT,
                  VM_KERN_MEMORY_SECURITY);
...
const void *ppl_image4_get_monitor_exports(void) {
  /*
...
errno_t ppl_image4_set_release_type(__unused const char *release_type) {
  /*
...
errno_t
ppl_image4_set_bnch_shadow(__unused const img4_nonce_domain_index_t ndi) {
  /*
...
kern_return_t ppl_image4_transfer_region(__unused image4_cs_trap_t selector,
...
kern_return_t ppl_image4_reclaim_region(__unused image4_cs_trap_t selector,
                                        __unused vm_address_t region_addr,
                                        __unused vm_size_t region_size) {
  /* All regions transfers happen internally with the PPL */
  return KERN_SUCCESS;
...
errno_t ppl_image4_monitor_trap(image4_cs_trap_t selector,
