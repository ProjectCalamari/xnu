...
#define TRACKER_GC_RUN_INTERVAL_NSEC                                           \
  (10 * NSEC_PER_SEC)           // GC wakes up periodically
#define TRACKER_GC_IDLE_TO (10) // age out entries when not used for a while
#define TRACKER_GC_EXTENDED_IDLE_TO                                            \
  (120) // extended timeout for entries that are used for policy evaluation
...
SYSCTL_INT(_net_tracker, OID_AUTO, extended_idle_timeout,
           CTLFLAG_RW | CTLFLAG_LOCKED, &tracker_db_extended_idle_timeout, 0,
           "");
...
#define TRACKER_HASH_UUID_TO_BYTE(uuidptr)                                     \
  (((uint8_t *)uuidptr)[0] ^ ((uint8_t *)uuidptr)[1] ^                         \
   ((uint8_t *)uuidptr)[2] ^ ((uint8_t *)uuidptr)[3] ^                         \
   ((uint8_t *)uuidptr)[4] ^ ((uint8_t *)uuidptr)[5] ^                         \
   ((uint8_t *)uuidptr)[6] ^ ((uint8_t *)uuidptr)[7] ^                         \
   ((uint8_t *)uuidptr)[8] ^ ((uint8_t *)uuidptr)[9] ^                         \
   ((uint8_t *)uuidptr)[10] ^ ((uint8_t *)uuidptr)[11] ^                       \
   ((uint8_t *)uuidptr)[12] ^ ((uint8_t *)uuidptr)[13] ^                       \
   ((uint8_t *)uuidptr)[14] ^ ((uint8_t *)uuidptr)[15])

#define TRACKER_HASH_WORD_TO_BYTE(wordptr)                                     \
  (((uint8_t *)wordptr)[0] ^ ((uint8_t *)wordptr)[1] ^                         \
   ((uint8_t *)wordptr)[2] ^ ((uint8_t *)wordptr)[3])

#define TRACKER_HASH(uuidptr, wordptr0, wordptr1, wordptr2, wordptr3)          \
  (TRACKER_HASH_WORD_TO_BYTE(wordptr0) ^ TRACKER_HASH_WORD_TO_BYTE(wordptr1) ^ \
   TRACKER_HASH_WORD_TO_BYTE(wordptr2) ^ TRACKER_HASH_WORD_TO_BYTE(wordptr3) ^ \
...
static KALLOC_TYPE_DEFINE(tracker_hash_entry_zone, struct tracker_hash_entry,
                          NET_KT_DEFAULT);
...
#define ALLOC_ENTRY(flags, entry)                                              \
  if (flags & SO_TRACKER_ATTRIBUTE_FLAGS_DOMAIN_SHORT) {                       \
    tracker_hash_entry_short_t *short_entry = zalloc_flags(                    \
        tracker_hash_entry_short_zone, Z_WAITOK | Z_ZERO | Z_NOFAIL);          \
    if (short_entry == NULL) {                                                 \
    ...
    entry =                                                                    \
        zalloc_flags(tracker_hash_entry_zone, Z_WAITOK | Z_ZERO | Z_NOFAIL);   \
    if (entry == NULL) {                                                       \
...
#define SIZE_OF_ENTRY(entry)                                                   \
  ((entry && entry->metadata.flags & SO_TRACKER_ATTRIBUTE_FLAGS_DOMAIN_SHORT)  \
       ? sizeof(struct tracker_hash_entry_short)                               \
   : entry ? sizeof(struct tracker_hash_entry)                                 \
           : 0)

#define STRLEN_SRC_DOMAIN                                                      \
  strbuflen((char *)src_domain_buffer, src_domain_buffer_size)
#define STRLEN_SRC_DOMAIN_OWNER                                                \
  strbuflen((char *)src_domain_owner_buffer, src_domain_owner_buffer_size)
#define STRLEN_DST_DOMAIN                                                      \
  strbuflen((char *)dst_domain_buffer, dst_domain_buffer_size)
#define STRLEN_DST_DOMAIN_OWNER                                                \
  strbuflen((char *)dst_domain_owner_buffer, dst_domain_owner_buffer_size)
...
#define GET_METADATA_BUFFERS_DST(metadata)                                     \
  ...
  uint8_t *__sized_by(dst_domain_owner_buffer_size) dst_domain_owner_buffer =  \
      NULL;                                                                    \
  if (metadata != NULL) {                                                      \
    if (metadata->flags & SO_TRACKER_ATTRIBUTE_FLAGS_DOMAIN_SHORT) {           \
      tracker_metadata_short_t *short_metadata =                               \
          (tracker_metadata_short_t *)metadata;                                \
...
#define GET_METADATA_BUFFERS_SRC(metadata)                                     \
  ...
  uint8_t *__sized_by(src_domain_owner_buffer_size) src_domain_owner_buffer =  \
      NULL;                                                                    \
  if (metadata != NULL) {                                                      \
    if (metadata->flags & SO_TRACKER_ATTRIBUTE_FLAGS_DOMAIN_SHORT) {           \
      tracker_metadata_short_t *short_metadata =                               \
...
static int tracker_db_init(void) {
  tracker_db_log_handle =
      os_log_create("com.apple.xnu.kern.tracker_db", "tracker_db");

  void *__single hash =
      hashinit(TRACKERHASHSIZE, M_TRACKER, &g_tracker_db.tracker_hashmask);
  g_tracker_db.tracker_hashbase = __unsafe_forge_bidi_indexable(
      struct trackerhashhead *, hash, TRACKERHASHSIZE * sizeof(void *));
  ...
  TRACKER_LOG(LOG_DEBUG, "Initialized: hashsize %d hashmask %lX",
              TRACKERHASHSIZE, g_tracker_db.tracker_hashmask);

  // Spawn thread for gargage collection
  if (kernel_thread_start(tracker_gc_thread_func, NULL, &g_tracker_gc_thread) !=
      KERN_SUCCESS) {
...
static boolean_t copy_metadata(tracker_metadata_t *dst_metadata,
                               tracker_metadata_t *src_metadata) {
  bool is_short = false;
  ...
  if ((src_domain_len > dst_domain_max) ||
      (src_domain_owner_len > dst_domain_max)) {
  ...
  if (src_domain_buffer[0]) {
    ...
    if (dst_domain_len != src_domain_len ||
        strbufcmp((char *)dst_domain_buffer, dst_domain_buffer_size,
                  (char *)src_domain_buffer, src_domain_buffer_size)) {
      if (src_domain_len <= dst_domain_max) {
  ...
  if (src_domain_owner_buffer[0]) {
    ...
    if (dst_domain_owner_len != src_domain_owner_len ||
        strbufcmp((char *)dst_domain_owner_buffer, dst_domain_owner_buffer_size,
                  (char *)src_domain_owner_buffer,
                  src_domain_owner_buffer_size)) {
      if (src_domain_owner_len <= dst_domain_max) {
        bcopy(src_domain_owner_buffer, dst_domain_owner_buffer,
              src_domain_owner_len);
...
static int fill_hash_entry(struct tracker_hash_entry *entry, uuid_t appuuid,
...
static inline void tracker_entry_log(int log_level, char *log_msg,
                                     struct tracker_hash_entry *entry,
                                     uint32_t hash) {
  ...
  TRACKER_LOG(log_level,
              "%s - %s <%s> len %d <%s> len %d <flags %X> "
              "%x%x%x%x-%x%x%x%x-%x%x%x%x-%x%x%x%x (hash 0x%X hashsize %d)",
              log_msg ? log_msg : "n/a", addr_buffer, dst_domain_buffer,
              (int)STRLEN_DST_DOMAIN, dst_domain_owner_buffer,
              (int)STRLEN_DST_DOMAIN_OWNER, entry->metadata.flags, ptr[0],
...
tracker_search_and_insert(struct tracker_db *db,
  ...
  hash_element =
      TRACKER_HASH(&matchentry->app_uuid, &key0, &key1, &key2, &key3) &
      db->tracker_hashmask;
  ...
  LIST_FOREACH(nextentry, trackerhash, entry_link) {
    ...
    if ((nextentry->address_family == AF_INET &&
         matchentry->address_family == AF_INET &&
         nextentry->address.addr46.ia46_addr4.s_addr ==
             matchentry->address.addr46.ia46_addr4.s_addr) ||
        (nextentry->address_family == AF_INET6 &&
         matchentry->address_family == AF_INET6 &&
         IN6_ARE_ADDR_EQUAL(&nextentry->address.addr6,
                            &matchentry->address.addr6))) {
      ...
      if (db->max_link_count == 0 || db->max_link_count < count) {
        ...
        TRACKER_LOG(LOG_DEBUG, "Max link count %d (hash 0x%X)",
                    db->max_link_count, hash_element);
      }

      // If this is for insert and we found an existing entry, update the
      // metadata if different. Different domain aliases may resolve to the same
      // IP, but we only keep one entry for the same IP address.  Therefore, we
      // update to the last metadata.
      if (insert) {
        if (copy_metadata(&nextentry->metadata, &matchentry->metadata) ==
            true) {
          TRACKER_ENTRY_LOG(LOG_DEBUG, "Updated entry", nextentry,
                            hash_element);
          nextentry->lastused = net_uptime();
          return nextentry;
        } else {
          // Failed to update found entry, delete it from db and allow insertion
          // of new entry.
          TRACKER_ENTRY_LOG(LOG_ERR,
                            "Failed to Update entry, deleting found entry",
                            nextentry, hash_element);
          g_tracker_db.tracker_count--;
          if (nextentry->metadata.flags &
              SO_TRACKER_ATTRIBUTE_FLAGS_DOMAIN_SHORT) {
  ...
  if (db->max_link_count == 0 || db->max_link_count < count) {
    db->max_link_count = count;
    TRACKER_LOG(LOG_DEBUG, "Max link count %d (hash 0x%X)", db->max_link_count,
                hash_element);
  }
  ...
  if (insert) {
    ...
    TRACKER_LOG(LOG_DEBUG, "Total entries %d (hashmask 0x%lX)",
                db->tracker_count, db->tracker_hashmask);
  }

  return NULL;
}

static int tracker_retrieve_attribute(u_int8_t *__sized_by(buffer_length)
                                          buffer,
                                      size_t buffer_length, u_int8_t type,
                                      u_int8_t *__sized_by(out_max_size)
                                          out_buffer,
                                      size_t out_size, size_t out_max_size) {
  int cursor = 0;
  size_t value_size = 0;
  u_int8_t *value = NULL;

  cursor =
      necp_buffer_find_tlv(buffer, (u_int32_t)buffer_length, 0, type, NULL, 0);
  if (cursor < 0) {
    TRACKER_LOG(LOG_DEBUG,
                "No tracker attribute of type %d found in parameters", type);
    return ENOENT;
  }
  ...
  if (out_size && value_size != out_size) {
    TRACKER_LOG(
        LOG_ERR,
        "Wrong size for tracker attribute type %d size %zu <got size %zu>",
        type, out_size, value_size);
    return EINVAL;
  }
  if (value_size > out_max_size) {
    TRACKER_LOG(LOG_ERR,
                "Exceeded max size (%zu) - tracker attribute type %d size %zu",
                out_max_size, type, value_size);
    return EINVAL;
  }
  ...
  if (value == NULL) {
    TRACKER_LOG(LOG_ERR,
                "Failed to get value for tracker attribute type %d size %zu",
                type, value_size);
    return EINVAL;
  }
...
static int tracker_add(struct proc *p, struct tracker_action_args *uap,
                       int *retval) {
  ...
  if (buffer_size > sizeof(scratch_pad)) {
    if (buffer_size > TRACKER_BUFFER_ALLOC_MAX) {
      TRACKER_LOG(LOG_ERR,
                  "Failed to allocate buffer, size exceeded max allowed");
      *retval = ENOMEM;
      return ENOMEM;
    }
  ...
  error = tracker_retrieve_attribute(
      buffer, buffer_size, SO_TRACKER_ATTRIBUTE_ADDRESS_FAMILY,
      (u_int8_t *)&address_family, sizeof(address_family),
      sizeof(address_family));
  if (error) {
    TRACKER_LOG(LOG_ERR,
                "Could not retrieve address family TLV from parameters");
    goto cleanup;
  }
  ...
  address_size = (address_family == AF_INET) ? sizeof(struct sockaddr_in)
                                             : sizeof(struct sockaddr_in6);

  // Address (Required)
  error = tracker_retrieve_attribute(
      buffer, buffer_size, SO_TRACKER_ATTRIBUTE_ADDRESS,
      __SA_UTILS_CONV_TO_BYTES(&addrBuffer), address_size, address_size);
  if (error) {
  ...
  if (address_family != addrBuffer.sin6_family) {
    TRACKER_LOG(LOG_ERR,
                "Address family parameter and address parameter family "
                "mismatch <%d != %d>",
  ...
  error = tracker_retrieve_attribute(
      buffer, buffer_size, SO_TRACKER_ATTRIBUTE_FLAGS, (u_int8_t *)&flags,
  ...
  // APP UUID (Required)
  error = tracker_retrieve_attribute(
      buffer, buffer_size, SO_TRACKER_ATTRIBUTE_APP_UUID,
      (u_int8_t *)&entry->app_uuid, sizeof(uuid_t), sizeof(uuid_t));
  if (error) {
  ...
  // Domain (Required)
  error = tracker_retrieve_attribute(buffer, buffer_size,
                                     SO_TRACKER_ATTRIBUTE_DOMAIN,
                                     dst_domain_buffer, 0, dst_domain_max);
  if (error) {
  ...
  if (entry->metadata.flags & SO_TRACKER_ATTRIBUTE_FLAGS_TRACKER) {
    ...
    error = tracker_retrieve_attribute(
        buffer, buffer_size, SO_TRACKER_ATTRIBUTE_DOMAIN_OWNER,
        dst_domain_owner_buffer, 0, dst_domain_max);
    if (error) {
      TRACKER_LOG(LOG_ERR,
                  "Could not retrieve domain owner TLV from parameters");
      error = EINVAL;
      goto cleanup;
    }
  }
...
static size_t tracker_entry_dump_size(struct tracker_hash_entry *entry) {
  size_t len = 0;
  size_t str_len = 0;
  ...
  case AF_INET:
    len +=
        TRACKER_TLV_HDR_LEN + sizeof(entry->address.addr46.ia46_addr4.s_addr);
    break;
  case AF_INET6:
  ...
  default:
    TRACKER_LOG(LOG_ERR,
                "Could not calculate entry dump size - invalid addr family %d",
  ...
  if (dst_domain_max == 0) {
    TRACKER_LOG(LOG_ERR,
                "Could not calculate entry dump size - 0 dst_domain_max");
    return 0;
  }
...
static size_t tracker_entry_dump(struct tracker_hash_entry *entry,
                                 uint8_t *__sized_by(buffer_size) buffer,
                                 size_t buffer_size) {
  ...
  cursor = necp_buffer_write_tlv(
      cursor, SO_TRACKER_ATTRIBUTE_APP_UUID, (u_int32_t)sizeof(entry->app_uuid),
      &entry->app_uuid, buffer, (u_int32_t)buffer_size);
  cursor = necp_buffer_write_tlv(cursor, SO_TRACKER_ATTRIBUTE_ADDRESS_FAMILY,
                                 (u_int32_t)sizeof(entry->address_family),
                                 &entry->address_family, buffer,
                                 (u_int32_t)buffer_size);

  switch (entry->address_family) {
  case AF_INET:
    cursor = necp_buffer_write_tlv(
        cursor, SO_TRACKER_ATTRIBUTE_ADDRESS,
        (u_int32_t)sizeof(entry->address.addr46.ia46_addr4.s_addr),
        &entry->address.addr46.ia46_addr4.s_addr, buffer,
        (u_int32_t)buffer_size);
    break;
  case AF_INET6:
    cursor = necp_buffer_write_tlv(cursor, SO_TRACKER_ATTRIBUTE_ADDRESS,
                                   (u_int32_t)sizeof(entry->address.addr6),
                                   &entry->address.addr6, buffer,
                                   (u_int32_t)buffer_size);
    break;
  default:
  ...
  if (entry->metadata.flags) {
    cursor = necp_buffer_write_tlv(cursor, SO_TRACKER_ATTRIBUTE_FLAGS,
                                   (u_int32_t)sizeof(entry->metadata.flags),
                                   &entry->metadata.flags, buffer,
                                   (u_int32_t)buffer_size);
  }
  ...
  str_len = STRLEN_DST_DOMAIN;
  TRACKER_LOG(LOG_DEBUG, "Dumping domain <%s> len <%zu>", dst_domain_buffer,
              str_len);
  if (str_len) {
    str_len++;
    cursor = necp_buffer_write_tlv(cursor, SO_TRACKER_ATTRIBUTE_DOMAIN,
                                   (u_int32_t)str_len, dst_domain_buffer,
                                   buffer, (u_int32_t)buffer_size);
  }

  str_len = STRLEN_DST_DOMAIN_OWNER;
  TRACKER_LOG(LOG_DEBUG, "Dumping domain owner <%s> len <%zu>",
              dst_domain_owner_buffer, str_len);
  if (str_len) {
    str_len++;
    cursor = necp_buffer_write_tlv(cursor, SO_TRACKER_ATTRIBUTE_DOMAIN_OWNER,
                                   (u_int32_t)str_len, dst_domain_owner_buffer,
                                   buffer, (u_int32_t)buffer_size);
  }
...
static int tracker_dump(struct proc *p, struct tracker_action_args *uap,
                        int *retval, bool by_app) {
#pragma unused(p)
  ...
  if (by_app) {
    ...
    sopt.sopt_p = p;
    if (sooptcopyin(&sopt, app_uuid_tlv, sizeof(app_uuid_tlv),
                    sizeof(app_uuid_tlv)) == 0) {
      if (tracker_retrieve_attribute(
              app_uuid_tlv, sizeof(app_uuid_tlv), SO_TRACKER_ATTRIBUTE_APP_UUID,
              (u_int8_t *)&app_uuid, sizeof(app_uuid), sizeof(app_uuid)) == 0) {
        has_app_uuid = true;
      }
    }
  }
  ...
  for (int i = 0; i < TRACKERHASHSIZE; i++) {
    ...
    LIST_FOREACH_SAFE(entry, hash, entry_link, temp_entry) {
      ...
      if (entry_tlv_size) {
        ...
        if (os_add_overflow(total_size_needed, entry_tlv_size,
                            &total_size_needed)) {
          TRACKER_LOG(
              LOG_ERR,
              "Could not dump entries, failed to calculate total size needed");
          error = EINVAL;
          goto done;
        }
      }
  ...
  if (os_add_overflow(total_size_needed,
                      TRACKER_TLV_HDR_LEN + sizeof(total_mem_size),
                      &total_size_needed)) {
    TRACKER_LOG(LOG_ERR, "Could not dump entries, failed to calculate total "
                         "size needed for memory used)");
    error = EINVAL;
    goto done;
  }

  // pre-append 4-bytes size to start of buffer.
  if (os_add_overflow(total_size_needed, sizeof(uint32_t),
                      &total_size_needed)) {
    TRACKER_LOG(LOG_ERR, "Could not dump entries, failed to add 4-bytes size "
                         "to start of buffer");
    error = EINVAL;
    goto done;
  }
  ...
  if (total_size_needed > uap->buffer_size) {
    TRACKER_LOG(
        LOG_ERR,
        "Could not dump entries, output buffer too small %lu (needed %lu)",
  ...
  for (int i = 0; i < TRACKERHASHSIZE; i++) {
    ...
    LIST_FOREACH_SAFE(entry, hash, entry_link, temp_entry) {
      ...
      entry_tlv_size = tracker_entry_dump(entry, scratch_pad_entry,
                                          sizeof(scratch_pad_entry));
      if (entry_tlv_size <= 0) {
        TRACKER_LOG(LOG_ERR,
                    "Could not dump entry, exceeded entry tlv buffer size");
        continue;
      } else {
        TRACKER_ENTRY_LOG(LOG_DEBUG, "Dumped entry", entry, 0);
      }
      cursor = necp_buffer_write_tlv(
          cursor, SO_TRACKER_ATTRIBUTE_DUMP_ENTRY, (u_int32_t)entry_tlv_size,
          scratch_pad_entry, data_start,
          (u_int32_t)(buffer_size - sizeof(uint32_t)));

      if (os_add_overflow(total_mem_size, SIZE_OF_ENTRY(entry),
                          &total_mem_size)) {
        TRACKER_LOG(
            LOG_ERR,
            "Could not dump entries, failed to calculate total memory used");
        error = EINVAL;
        goto done;
      }
  ...
  cursor = necp_buffer_write_tlv(
      cursor, SO_TRACKER_ATTRIBUTE_MEMORY_USED, sizeof(total_mem_size),
      &total_mem_size, data_start, (u_int32_t)(buffer_size - sizeof(uint32_t)));
  ...
  if (error) {
    TRACKER_LOG(LOG_DEBUG, "Failed to copy out dump buffer (%lu bytes)",
...
int tracker_action(struct proc *p, struct tracker_action_args *uap,
                   int *retval) {
  const task_t __single task = proc_task(p);
  if (task == NULL ||
      !IOTaskHasEntitlement(task, "com.apple.private.ip-domain-table")) {
    TRACKER_LOG(LOG_ERR, "Process (%d) does not hold the necessary entitlement",
                proc_pid(p));
    *retval = EPERM;
    return EPERM;
  }
...
int tracker_lookup(uuid_t app_uuid, struct sockaddr *remote,
                   tracker_metadata_t *metadata) {
  struct tracker_hash_entry matchentry = {};
  ...
  if (remote == NULL || uuid_is_null(app_uuid)) {
    TRACKER_LOG(LOG_DEBUG, "Failed lookup - remote %s null, app_uuid %s null",
                remote ? "is not" : "is",
                !uuid_is_null(app_uuid) ? "is not" : "is");
    return EINVAL;
  }
...
static void tracker_gc_thread_sleep(bool forever) {
  if (forever) {
...
static bool tracker_idle_timed_out(struct tracker_hash_entry *entry,
                                   u_int64_t timeout, u_int64_t current_time) {
...
go_sleep:
  ...
  TRACKER_LOG(LOG_DEBUG,
              "Garbage Collection done...(deleted %d - total count %d)",
