...
errno_t fill_socketinfo(struct socket *so, struct socket_info *si) {
  ...
  case PF_INET6: {
    ...
    if (type == SOCK_STREAM && (protocol == 0 || protocol == IPPROTO_TCP) &&
      ...
      tcpsi->tcpsi_timer[TSI_T_REXMT] = tp->t_timer[TCPT_REXMT];
      tcpsi->tcpsi_timer[TSI_T_PERSIST] = tp->t_timer[TCPT_PERSIST];
      tcpsi->tcpsi_timer[TSI_T_KEEP] = tp->t_timer[TCPT_KEEP];
      tcpsi->tcpsi_timer[TSI_T_2MSL] = tp->t_timer[TCPT_2MSL];
      tcpsi->tcpsi_mss = tp->t_maxseg;
      tcpsi->tcpsi_flags = tp->t_flags;
      tcpsi->tcpsi_tp = (u_int64_t)VM_KERNEL_ADDRHASH(tp);
    }
    break;
  }
  case PF_UNIX: {
    ...
    unsi->unsi_conn_pcb = (uint64_t)VM_KERNEL_ADDRHASH(unp->unp_conn);
    if (unp->unp_conn) {
      unsi->unsi_conn_so =
          (uint64_t)VM_KERNEL_ADDRHASH(unp->unp_conn->unp_socket);
    }
    ...
    if (unp->unp_conn && unp->unp_conn->unp_addr) {
      ...
      SOCKADDR_COPY(unp->unp_conn->unp_addr, &unsi->unsi_caddr.ua_sun, addrlen);
    }
    break;
  }
  case PF_NDRV: {
  ...
  case PF_SYSTEM:
    if (SOCK_PROTO(so) == SYSPROTO_EVENT) {
      ...
      kesi->kesi_vendor_code_filter = ev_pcb->evp_vendor_code_filter;
