...
struct mem_acct {
  int64_t _Atomic ma_allocated; /* Amount of memory accounted towards this
                                   subsystem (ignore temporary per-CPU
                                   accounting from below) */
  int32_t *__zpercpu ma_percpu; /* Per-CPU "bounce-buffer" of accounting that
                                   will be folded in to `ma_allocated` */
  uint64_t ma_hardlimit;        /* hard limit that will not be exceeded */
  uint8_t ma_percent; /* Percent of hard-limit we should start soft-limiting (if
                         != 100 && != 0) */
  uint64_t _Atomic ma_peak;
  char ma_name[MEM_ACCT_NAME_LENGTH]; /* Name of the subsystem using this
                                         instance of memory-accounting module */
};

#define MEM_ACCT_PCPU_MAX                                                      \
  1024 * 1024 /* Update global var after 1MB in the per-cpu var */

static struct mem_acct *memacct[MEM_ACCT_MAX];

static uint64_t mem_acct_softlimit(uint64_t hardlimit, uint8_t percent) {
  return (hardlimit * percent) / 100;
}

static uint64_t mem_acct_presoftlimit(uint64_t hardlimit, uint8_t percent) {
  return (mem_acct_softlimit(hardlimit, percent) * percent) / 100;
}

int mem_acct_limited(const struct mem_acct *macct) {
  uint64_t hardlimit;
  int64_t allocated;
  uint8_t percent;
...
void _mem_acct_add(struct mem_acct *macct, int size) {
  int *pcpu;
  ...
  *pcpu += size;
  ...
  if (*pcpu > MEM_ACCT_PCPU_MAX || *pcpu < -MEM_ACCT_PCPU_MAX) {
    ...
    if (limited != newlimited) {
      os_log(OS_LOG_DEFAULT, "memacct: %s goes from %u to %u for its limit",
...
struct mem_acct *mem_acct_register(const char *__null_terminated name,
  ...
  for (i = 0; i < MEM_ACCT_MAX; i++) {
    ...
    if (strlcmp(memacct[i]->ma_name, name, MEM_ACCT_NAME_LENGTH - 1) == 0) {
      os_log(OS_LOG_DEFAULT, "memacct: subsystem %s already exists", name);
      goto exit;
    }
  }
  ...
  if (percent >= 100) {
    os_log(OS_LOG_DEFAULT, "memacct: percent is > 100");
...
/* Helper functions */
static void memacct_copy_stats(struct memacct_statistics *s,
                               struct mem_acct *a);

SYSCTL_NODE(_kern, OID_AUTO, memacct, CTLFLAG_RW | CTLFLAG_LOCKED,
            sysctl_mem_acct, "Memory Accounting");
...
static int sysctl_subsystem_allocated(struct walkarg *w) {
  ...
  zpercpu_foreach(v, acct->ma_percpu) { value += *v; }
...
static int sysctl_all_statistics(struct sysctl_req *req) {
  ...
  struct memacct_statistics *memstats = kalloc_data(
      sizeof(struct memacct_statistics) * count, Z_WAITOK_ZERO_NOFAIL);
  ...
  error = sysctl_io_opaque(req, memstats,
                           sizeof(struct memacct_statistics) * count, NULL);
  if (error) {
...
static int sysctl_mem_acct_subsystems(struct sysctl_req *req) {
  ...
  error = sysctl_io_opaque(req, names,
                           sizeof(char) * count * MEM_ACCT_NAME_LENGTH, NULL);
  if (error) {
...
static void memacct_copy_stats(struct memacct_statistics *s,
  ...
  zpercpu_foreach(v, a->ma_percpu) { s->allocated += *v; }
  if (a->ma_percent) {
