...
#define PROC_UUID_POLICY_SUBSYS_LOCK()                                         \
  lck_mtx_lock(&proc_uuid_policy_subsys_mutex)
#define PROC_UUID_POLICY_SUBSYS_UNLOCK()                                       \
  lck_mtx_unlock(&proc_uuid_policy_subsys_mutex)

#define PROC_UUID_POLICY_HASH_SIZE 64
u_long proc_uuid_policy_hash_mask;

/* Assume first byte of UUIDs are evenly distributed */
#define UUIDHASH(uuid)                                                         \
  (&proc_uuid_policy_hashtbl[uuid[0] & proc_uuid_policy_hash_mask])
static LIST_HEAD(proc_uuid_policy_hashhead, proc_uuid_policy_entry) *
...
#define BUMP_PROC_UUID_POLICY_GENERATION_COUNT()                               \
  do {                                                                         \
    if (OSIncrementAtomic(&proc_uuid_policy_table_gencount) ==                 \
        (INT32_MAX - 1)) {                                                     \
...
static int proc_uuid_policy_insert(uuid_t uuid, uint32_t flags);

static struct proc_uuid_policy_entry *
proc_uuid_policy_remove_locked(uuid_t uuid, uint32_t flags, int *should_delete);

static int proc_uuid_policy_remove(uuid_t uuid, uint32_t flags);

static struct proc_uuid_policy_entry *
proc_uuid_policy_lookup_locked(uuid_t uuid);

static int proc_uuid_policy_clear(uint32_t flags);

void proc_uuid_policy_init(void) {
  proc_uuid_policy_hashtbl =
      hashinit(PROC_UUID_POLICY_HASH_SIZE, M_PROC_UUID_POLICY,
...
static struct proc_uuid_policy_entry *
proc_uuid_policy_remove_locked(uuid_t uuid, uint32_t flags,
                               int *should_delete) {
...
int proc_uuid_policy_lookup(uuid_t uuid, uint32_t *flags, int32_t *gencount) {
  struct proc_uuid_policy_entry *foundentry = NULL;
  int error;
...
static int proc_uuid_policy_clear(uint32_t flags) {
  ...
  struct proc_uuid_policy_hashhead deletehead =
  ...
  if (proc_uuid_policy_count > 0) {
    for (hashslot = 0; hashslot <= proc_uuid_policy_hash_mask; hashslot++) {
      struct proc_uuid_policy_hashhead *headp =
          &proc_uuid_policy_hashtbl[hashslot];

      LIST_FOREACH_SAFE(searchentry, headp, entries, tmpentry) {
        if ((searchentry->flags & flags) == searchentry->flags) {
          /* We are clearing all flags for this entry, move entry to our delete
           * list */
...
int proc_uuid_policy(struct proc *p __unused, struct proc_uuid_policy_args *uap,
