...
#define ull_assert_notwned(ull)                                                \
  LCK_SPIN_ASSERT(&ull->ull_lock, LCK_ASSERT_NOTOWNED)
...
inline static bool ull_key_match(ulk_t *a, ulk_t *b) {
  if (a->ulk_key_type != b->ulk_key_type) {
    return false;
  }

  if (a->ulk_key_type == ULK_UADDR) {
    return (a->ulk_task == b->ulk_task) && (a->ulk_addr == b->ulk_addr);
  }

  assert(a->ulk_key_type == ULK_XPROC);
  return (a->ulk_object == b->ulk_object) && (a->ulk_offset == b->ulk_offset);
}
...
SYSCTL_INT(_kern, OID_AUTO, ulock_adaptive_spin_usecs,
           CTLFLAG_RW | CTLFLAG_LOCKED, &ulock_adaptive_spin_usecs, 0,
           "ulock adaptive spin duration");

#if DEVELOPMENT || DEBUG
static int ull_simulate_copyin_fault = 0;

static void ull_dump(ull_t *ull) {
  kprintf("ull\t%p\n", ull);
...
#if ULL_TICKET_LOCK
#define ull_bucket_lock(i)                                                     \
  lck_ticket_lock(&ull_bucket[i].ulb_lock, &ull_lck_grp)
#define ull_bucket_unlock(i) lck_ticket_unlock(&ull_bucket[i].ulb_lock)
#else
#define ull_bucket_lock(i)                                                     \
  lck_spin_lock_grp(&ull_bucket[i].ulb_lock, &ull_lck_grp)
#define ull_bucket_unlock(i) lck_spin_unlock(&ull_bucket[i].ulb_lock)
#endif /* ULL_TICKET_LOCK */
static __inline__ uint32_t ull_hash_index(const void *key, size_t length) {
...
#define ULL_INDEX(keyp)                                                        \
  ull_hash_index(keyp, keyp->ulk_key_type == ULK_UADDR ? ULK_UADDR_LEN         \
                                                       : ULK_XPROC_LEN)

static void ulock_initialize(void) {
  ...
  kprintf("%s>thread_max=%d, ull_hash_buckets=%d\n", __FUNCTION__, thread_max,
          ull_hash_buckets);
  assert(ull_hash_buckets >= thread_max / 4);

  ull_bucket =
      zalloc_permanent(sizeof(ull_bucket_t) * ull_hash_buckets, ZALIGN_PTR);
 ...
 */
static int ull_hash_dump(task_t task) {
  int count = 0;
  if (task == TASK_NULL) {
...
static ull_t *ull_alloc(ulk_t *key) {
  ull_t *ull = (ull_t *)zalloc_flags(ull_zone, Z_SET_NOTEARLY);
  assert(ull != NULL);
...
static void ull_free(ull_t *ull) {
 ...
 */
static ull_t *ull_get(ulk_t *key, uint32_t flags, ull_t **unused_ull) {
  ull_t *ull = NULL;
...
static int uaddr_findobj(user_addr_t uaddr, uint64_t *objectp,
                         uint64_t *offsetp) {
  kern_return_t ret;
  vm_page_info_basic_data_t info;
  mach_msg_type_number_t count = VM_PAGE_INFO_BASIC_COUNT;

  ret = vm_map_page_info(current_map(), uaddr, VM_PAGE_INFO_BASIC,
...
inline static int wait_result_to_return_code(wait_result_t wr) {
  int ret = 0;
...
static int ulock_resolve_owner(uint32_t value, thread_t *owner) {
...
int sys_ulock_wait(struct proc *p, struct ulock_wait_args *args,
                   int32_t *retval) {
...
int sys_ulock_wait2(struct proc *p, struct ulock_wait2_args *args,
                    int32_t *retval) {
  ...
  if ((flags & ULF_WAIT_ADAPTIVE_SPIN) && set_owner) {
    ...
    for (;;) {
      if (owner_thread == NULL &&
          ulock_resolve_owner(u32, &owner_thread) != 0) {
        break;
      }
      ...
      if (end == 0) {
        clock_interval_to_deadline(ulock_adaptive_spin_usecs, NSEC_PER_USEC,
                                   &end);
  ...
  if (((ull_simulate_copyin_fault == proc_getpid(p)) ||
       (ull_simulate_copyin_fault == 1)) &&
      (copy_ret == 0)) {
...
#endif
  if (copy_ret != 0) {
    /* copyin() will return an error if the access to the user addr would have
     * faulted, so just return and let the user level code fault it in.
     */
    ret = copy_ret;
    goto out_locked;
  }
  ...
  if (set_owner) {
    if (owner_thread == THREAD_NULL) {
      ...
      /* HACK: don't bail on MACH_PORT_DEAD, to avoid blowing up the no-tsd
       * pthread lock */
      ret = 0;
    }
     ...
     * a) owner_thread is definitely the current owner, because I just read the
     * value b) owner_thread is either: i) holding the user lock or ii) has just
     * unlocked the user lock after I looked and is heading toward the kernel to
     * call ull_wake. If so, it's going to have to wait for the ull mutex.
     *
     * Therefore, I can ask the turnstile to promote its priority, and I can
     * rely on it to come by later to issue the wakeup and lose its promotion.
     */
  ...
  ts = turnstile_prepare((uintptr_t)ull, &ull->ull_turnstile, TURNSTILE_NULL,
                         TURNSTILE_ULOCK);
 ...
 * Must be called with ull_lock held
 */
static void ulock_wait_cleanup(ull_t *ull, thread_t owner_thread,
                               thread_t old_owner, int32_t *retval) {
  ull_assert_owned(ull);
...
int sys_ulock_wake(struct proc *p, struct ulock_wake_args *args,
                   int32_t *retval) {
  int ret = 0;
#if DEVELOPMENT || DEBUG
  uint8_t opcode = (uint8_t)(args->operation & UL_OPCODE_MASK);
...
int ulock_wake(task_t task, uint32_t operation, user_addr_t addr,
               uint64_t wake_value) {
  ...
  if (flags & ULF_WAKE_THREAD) {
    ...
    wake_thread = port_name_to_thread(
        wake_thread_name, PORT_INTRANS_THREAD_IN_CURRENT_TASK |
                              PORT_INTRANS_THREAD_NOT_CURRENT_THREAD);
  ...
  if (flags & ULF_WAKE_THREAD) {
    kern_return_t kr = waitq_wakeup64_thread(
        &ts->ts_waitq, CAST_EVENT64_T(ULOCK_TO_EVENT(ull)), wake_thread,
        THREAD_AWAKENED);
    if (kr != KERN_SUCCESS) {
    ...
    waitq_wakeup64_all(
        &ts->ts_waitq, CAST_EVENT64_T(ULOCK_TO_EVENT(ull)), THREAD_AWAKENED,
     ...
     */
    new_owner = waitq_wakeup64_identify(
        &ts->ts_waitq, CAST_EVENT64_T(ULOCK_TO_EVENT(ull)), THREAD_AWAKENED,
...
void kdp_ulock_find_owner(__unused struct waitq *waitq, event64_t event,
  ...
  case UL_COMPARE_AND_WAIT64_SHARED:
    ...
    break;
  default:
    panic("%s: Invalid ulock opcode %d addr %p", __FUNCTION__, ull->ull_opcode,
          (void *)ull);
    break;
  }
  return;
