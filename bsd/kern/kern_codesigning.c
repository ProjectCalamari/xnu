...
vm_address_t code_signing_allocate(size_t alloc_size) {
  ...
  kern_return_t ret =
      kmem_alloc(kernel_map, &alloc_addr, aligned_size,
...
SYSCTL_NODE(_security, OID_AUTO, codesigning, CTLFLAG_RD, 0,
            "XNU Code Signing");

static SECURITY_READ_ONLY_LATE(bool) cs_config_set = false;
static SECURITY_READ_ONLY_LATE(code_signing_monitor_type_t)
    cs_monitor = CS_MONITOR_TYPE_NONE;
static SECURITY_READ_ONLY_LATE(code_signing_config_t) cs_config = 0;
static uint32_t security_boot_mode_complete = 0;

SYSCTL_UINT(_security_codesigning, OID_AUTO, monitor, CTLFLAG_RD, &cs_monitor,
            0, "code signing monitor type");
SYSCTL_UINT(_security_codesigning, OID_AUTO, config, CTLFLAG_RD, &cs_config, 0,
            "code signing configuration");

SYSCTL_UINT(_security_codesigning, OID_AUTO, security_boot_mode_complete,
            CTLFLAG_RD, &security_boot_mode_complete, 0,
            "security boot mode completion status");

void code_signing_configuration(code_signing_monitor_type_t *monitor_type_out,
  ...
  /*
   * Since we read this variable with load-acquire semantics, if we observe a
   * value of true, it means we should be able to observe writes to cs_monitor
   * and also cs_config.
   */
   ...
   * early in the system boot, much before kernel extensions such as Apple
   * Mobile File Integrity come online. As a result, this function assumes that
   * all the code signing features are enabled, and later on, different
   * components can disable support for different features using
  ...
  if (csm_enabled() == true) {
    ...
    /*
     * Restricted Execution Mode support. The pattern for this code snippet
     * breaks the norm compared to others. For the other features, we consider
     * them enabled by default unless TXM disables them. For REM, given this is
     * a TXM only feature, we consider it disabled unless TXM explicitly tells
     * us it is enabled.
     */
    if (txm_cs_config->systemPolicy->featureSet.restrictedExecutionMode ==
        true) {
  ...
  /*
   * We only ever need to parse for boot-args based exemption state on
   * DEVELOPMENT or DEBUG builds as this state is not respected by any code
   * signing component on RELEASE builds.
   */
...
#define CS_AMFI_MASK_GET_OUT_OF_MY_WAY 0x80
  ...
  if (csm_enabled() == true) {
    ...
    bool override = PE_parse_boot_argn("pmap_cs_allow_any_signature",
    ...
    override = PE_parse_boot_argn("pmap_cs_unrestrict_pmap_cs_disable",
    ...
    override = PE_parse_boot_argn("pmap_cs_allow_modified_code_pages",
  ...
  /*
   * We write the cs_config_set variable with store-release semantics which
   * means no writes before this call will be re-ordered to after this call.
   * Hence, if someone reads this variable with load-acquire semantics, and they
   * observe a value of true, then they will be able to observe the correct
   * values of the cs_monitor and the cs_config variables as well.
   */
...
void disable_code_signing_feature(code_signing_config_t feature) {
  ...
  if (os_atomic_load(&cs_config_set, acquire) == false) {
    panic("attempted to disable code signing feature without init: %u",
          feature);
  }

  /*
   * We require that only a single feature be disabled through a single call to
   * this function. Moreover, we ensure that only valid features are being
   * disabled.
   */
  switch (feature) {
...
kern_return_t secure_channel_shared_page(uint64_t *secure_channel_phys,
  ...
  return CSM_PREFIX(secure_channel_shared_page)(secure_channel_phys,
                                                secure_channel_size);
}

#pragma mark Developer Mode

void enable_developer_mode(void) { CSM_PREFIX(toggle_developer_mode)(true); }

void disable_developer_mode(void) { CSM_PREFIX(toggle_developer_mode)(false); }
...
void update_csm_device_state(void) { CSM_PREFIX(update_device_state)(); }
...
/*
 * AMFI performs full profile validation by itself. XNU only needs to manage
 * provisioning profiles when we have a monitor since the monitor needs to
 * independently verify the profile data as well.
 */
...
static SLIST_HEAD(, _cs_profile)
    all_profiles = SLIST_HEAD_INITIALIZER(all_profiles);
...
static cs_profile_t *search_for_profile_uuid(const uuid_t profile_uuid) {
...
kern_return_t
csm_register_provisioning_profile(const uuid_t profile_uuid,
  ...
  ret = CSM_PREFIX(register_provisioning_profile)(
      profile_blob, profile_blob_size, &monitor_profile_obj);
...
kern_return_t csm_trust_provisioning_profile(const uuid_t profile_uuid,
  ...
  /*
   * We don't explicitly make a check here for if the code-signing-monitor is
   * enabled or not because this function should never be called unless
   * registration of the profile succeeded, which it won't in cases where the
   * CSM is disabled.
   *
   * If this function does somehow get called, it'll result in a panic -- this
   * is good for us to detect and to fix the code path which results in this
   * behavior.
   */
  ...
  ret = CSM_PREFIX(trust_provisioning_profile)(profile->profile_obj, sig_data,
                                               sig_size);
...
kern_return_t csm_associate_provisioning_profile(void *monitor_sig_obj,
  ...
  ret = CSM_PREFIX(associate_provisioning_profile)(monitor_sig_obj,
                                                   profile->profile_obj);

  if (ret == KERN_SUCCESS) {
    /*
     * This seems like an active profile -- let it skip the garbage collector on
     * the next pass. We can modify this field even though we've only taken a
     * shared lock as in this case we're always setting it to a fixed value.
     */
    profile->skip_collector = true;
  }
...
static kern_return_t unregister_provisioning_profile(cs_profile_t *profile) {
  kern_return_t ret = KERN_DENIED;
...
/*
 * AMFI performs full signature validation by itself. For some things, AMFI uses
 * XNU in order to abstract away the underlying implementation for data storage,
 * but for most of these, AMFI doesn't directly interact with them, and they're
 * only required when we have a code signing monitor on the system.
 */

void set_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
  CSM_PREFIX(set_compilation_service_cdhash)(cdhash);
}

bool match_compilation_service_cdhash(const uint8_t cdhash[CS_CDHASH_LEN]) {
...
void unrestrict_local_signing_cdhash(
...
kern_return_t get_trust_level_kdp(__unused pmap_t pmap,
...
kern_return_t get_jit_address_range_kdp(__unused pmap_t pmap,
...
kern_return_t
csm_resolve_os_entitlements_from_proc(__unused const proc_t process,
...
kern_return_t address_space_debugged_state(const proc_t process) {
  ...
  /*
   * For macOS systems only, we allow the execution of unsigned code. On Intel,
   * code doesn't need to be signed, and on ASi, Rosetta binaries don't need to
   * be signed. In these cases, we return successfully from this function
...
bool csm_enabled(void) { return CSM_PREFIX(code_signing_enabled)(); }
...
#else
  /*
   * Lockdown mode is supposed to disable all forms of JIT on the system. For
   * now, we leave JIT enabled by default until some blockers are resolved. The
   * way this code is written, we don't need to change anything once we enforce
   * MAP_JIT to be disabled for lockdown mode.
   */
...
void csm_code_signing_violation(proc_t proc, vm_offset_t addr) {
  ...
  /*
   * For now, the only input into this function is from current_proc(), so using
   * current_thread() over here is alright. If this function ever gets called
   * from another location, we need to then change where we get the user thread
   * from.
   */
  assert(proc == current_proc());

  /*
   * Force exit the process and set it to allow generating crash reports, which
   * is critical for better triaging these issues.
   */

  exception_info_t info = {.os_reason = OS_REASON_CODESIGNING,
...
kern_return_t csm_register_code_signature(
  ...
  return CSM_PREFIX(register_code_signature)(
      signature_addr, signature_size, code_directory_offset, signature_path,
      monitor_sig_obj, monitor_signature_addr);
}
...
kern_return_t csm_reconstitute_code_signature(void *monitor_sig_obj,
  ...
  return CSM_PREFIX(reconstitute_code_signature)(monitor_sig_obj, unneeded_addr,
                                                 unneeded_size);
}
...
kern_return_t csm_setup_nested_address_space(pmap_t pmap,
  ...
  return CSM_PREFIX(setup_nested_address_space)(pmap, region_addr, region_size);
}

kern_return_t csm_associate_code_signature(pmap_t monitor_pmap,
  ...
  return CSM_PREFIX(associate_code_signature)(
      monitor_pmap, monitor_sig_obj, region_addr, region_size, region_offset);
}

kern_return_t csm_allow_jit_region(pmap_t monitor_pmap) {
  if (csm_enabled() == false) {
    return KERN_SUCCESS;
...
kern_return_t csm_associate_jit_region(pmap_t monitor_pmap,
                                       const vm_address_t region_addr,
                                       const vm_size_t region_size) {
  if (csm_enabled() == false) {
    return KERN_NOT_SUPPORTED;
  ...
  return CSM_PREFIX(associate_jit_region)(monitor_pmap, region_addr,
                                          region_size);
}

kern_return_t csm_associate_debug_region(pmap_t monitor_pmap,
                                         const vm_address_t region_addr,
                                         const vm_size_t region_size) {
  if (csm_enabled() == false) {
    return KERN_NOT_SUPPORTED;
  ...
  kern_return_t ret = CSM_PREFIX(associate_debug_region)(
      monitor_pmap, region_addr, region_size);
...
kern_return_t csm_get_jit_address_range_kdp(pmap_t pmap,
  ...
  return CSM_PREFIX(get_jit_address_range_kdp)(pmap, jit_region_start,
                                               jit_region_end);
}

kern_return_t csm_address_space_exempt(const pmap_t pmap) {
  /*
   * These exemptions are actually orthogonal to the code signing enforcement.
   * As a result, we let each monitor explicitly decide how to deal with the
   * exemption in case code signing enforcement is disabled.
   */
...
kern_return_t csm_associate_os_entitlements(void *monitor_sig_obj,
  ...
  return CSM_PREFIX(associate_kernel_entitlements)(monitor_sig_obj,
                                                   os_entitlements);
}
 ...
 * In order to ease the implementation specific, AppleImage4 always depends on
 * these abstracted APIs, regardless of whether the system has a monitor
 * environment or not.
 */
...
kernel_image4_execute_object(img4_runtime_object_spec_index_t obj_spec_index,
  ...
  return CSM_PREFIX(image4_execute_object)(obj_spec_index, payload, manifest);
}

errno_t
kernel_image4_copy_object(img4_runtime_object_spec_index_t obj_spec_index,
                          vm_address_t object_out, size_t *object_length) {
  return CSM_PREFIX(image4_copy_object)(obj_spec_index, object_out,
                                        object_length);
}
...
static errno_t
_kernel_image4_monitor_trap_image_activate(image4_cs_trap_t selector,
  ...
  memcpy((void *)payload_addr, (void *)input.csmx_payload,
         input.csmx_payload_len);

  manifest_addr = code_signing_allocate(input.csmx_manifest_len);
  if (manifest_addr == 0) {
    goto out;
  }
  memcpy((void *)manifest_addr, (void *)input.csmx_manifest,
         input.csmx_manifest_len);

  /* Transfer both regions to the monitor */
  CSM_PREFIX(image4_transfer_region)(selector, payload_addr,
                                     input.csmx_payload_len);
  CSM_PREFIX(image4_transfer_region)(selector, manifest_addr,
                                     input.csmx_manifest_len);
...
out:
  if ((err != 0) && (payload_addr != 0)) {
    ...
    if (manifest_addr != 0) {
      CSM_PREFIX(image4_reclaim_region)(selector, payload_addr,
                                        input.csmx_payload_len);
    }
  ...
  if (manifest_addr != 0) {
    ...
    CSM_PREFIX(image4_reclaim_region)(selector, manifest_addr,
                                      input.csmx_manifest_len);
...
static errno_t _kernel_image4_monitor_trap(image4_cs_trap_t selector,
                                           const void *input_data,
                                           size_t input_size) {
...
errno_t kernel_image4_monitor_trap(image4_cs_trap_t selector,
  ...
  if ((input_data == NULL) || (input_size == 0)) {
    ...
    return EINVAL;
  } else if (os_add_overflow((vm_address_t)input_data, input_size,
                             &length_check)) {
    panic("image4_ dispatch: overflow on input: %p | %lu", input_data,
          input_size);
  }
