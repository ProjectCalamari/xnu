...
#define MCACHE_SIZE(n) __builtin_offsetof(mcache_t, mc_cpu[n])
...
static mcache_bkttype_t mcache_bkttype[] = {
    {1, 4096, 32768, NULL}, {3, 2048, 16384, NULL}, {7, 1024, 12288, NULL},
    {15, 256, 8192, NULL},  {31, 64, 4096, NULL},   {47, 0, 2048, NULL},
    {63, 0, 1024, NULL},    {95, 0, 512, NULL},     {143, 0, 256, NULL},
    {165, 0, 0, NULL},
};

static mcache_t *mcache_create_common(const char *, size_t, size_t,
                                      mcache_allocfn_t, mcache_freefn_t,
                                      mcache_auditfn_t, mcache_logfn_t,
                                      mcache_notifyfn_t, void *, u_int32_t,
                                      int);
static unsigned int mcache_slab_alloc(void *, mcache_obj_t ***, unsigned int,
                                      int);
...
static void mcache_bkt_batch_free(mcache_t *, mcache_bktlist_t *,
                                  mcache_bkt_t *);
...
static void mcache_update(thread_call_param_t __unused,
...
static void mcache_reap_timeout(thread_call_param_t __unused,
                                thread_call_param_t);
...
__private_extern__ void mcache_init(void) {
  ...
  for (i = 0; i < sizeof(mcache_bkttype) / sizeof(*btp); i++) {
    ...
    (void)snprintf(name, sizeof(name), "bkt_%d", btp->bt_bktsize);
    btp->bt_cache = mcache_create(name, (btp->bt_bktsize + 1) * sizeof(void *),
                                  0, 0, MCR_SLEEP);
  }
  ...
  mcache_audit_cache =
      mcache_create("audit", sizeof(mcache_audit_t), 0, 0, MCR_SLEEP);
 ...
 */
__private_extern__ unsigned int mcache_getflags(void) { return mcache_flags; }
 ...
 */
__private_extern__ mcache_t *mcache_create(const char *name, size_t bufsize,
                                           size_t align, u_int32_t flags,
                                           int wait __unused) {
  return mcache_create_common(name, bufsize, align, mcache_slab_alloc,
                              mcache_slab_free, mcache_slab_audit, NULL, NULL,
                              NULL, flags, 1);
}
...
mcache_create_ext(const char *name, size_t bufsize, mcache_allocfn_t allocfn,
                  mcache_freefn_t freefn, mcache_auditfn_t auditfn,
                  mcache_logfn_t logfn, mcache_notifyfn_t notifyfn, void *arg,
                  u_int32_t flags, int wait __unused) {
  return mcache_create_common(name, bufsize, 0, allocfn, freefn, auditfn, logfn,
                              notifyfn, arg, flags, 0);
}

/*
 * Common cache creation routine.
 */
static mcache_t *mcache_create_common(
    const char *name, size_t bufsize, size_t align, mcache_allocfn_t allocfn,
    mcache_freefn_t freefn, mcache_auditfn_t auditfn, mcache_logfn_t logfn,
   ...
   */
  cp = (mcache_t *)P2ROUNDUP((intptr_t)buf + sizeof(void *),
                             CPU_CACHE_LINE_SIZE);
  ...
  *pbuf = buf;
  ...
  cp->mc_bkt_lock_grp = lck_grp_alloc_init(lck_name, LCK_GRP_ATTR_NULL);
  lck_mtx_init(&cp->mc_bkt_lock, cp->mc_bkt_lock_grp, LCK_ATTR_NULL);

  (void)snprintf(lck_name, sizeof(lck_name), "%s.sync", cp->mc_name);
  cp->mc_sync_lock_grp = lck_grp_alloc_init(lck_name, LCK_GRP_ATTR_NULL);
  ...
  if (cp->mc_flags & MCF_DEBUG) {
    printf("mcache_create: %s (%s) arg %p bufsize %lu align %lu "
           ...
           name, need_zone ? "i" : "e", arg, bufsize, cp->mc_align, chunksize,
           btp->bt_bktsize);
  }
  return cp;
}
 ...
 */
__private_extern__ unsigned int mcache_alloc_ext(mcache_t *cp,
                                                 mcache_obj_t **list,
...
retry_alloc:
  ...
  for (;;) {
     ...
     * objects.
     */
    (void)mcache_bkt_batch_alloc(cp, &cp->mc_full, &bkt,
                                 (need <= ccp->cc_bktsize) ? 1 : 2);
    if (bkt != NULL) {
      ...
      if (bkt->bkt_next == NULL) {
        ...
        mcache_cpu_batch_refill(ccp, bkt, ccp->cc_bktsize);
      }
 ...
 */
__private_extern__ void *mcache_alloc(mcache_t *cp, int wait) {
  mcache_obj_t *buf;
...
__private_extern__ void mcache_waiter_inc(mcache_t *cp) {
  os_atomic_inc(&cp->mc_waiter_cnt, relaxed);
}

__private_extern__ void mcache_waiter_dec(mcache_t *cp) {
  os_atomic_dec(&cp->mc_waiter_cnt, relaxed);
}

__private_extern__ boolean_t mcache_bkt_isempty(mcache_t *cp) {
  /*
 ...
 */
static void mcache_notify(mcache_t *cp, u_int32_t event) {
 ...
 */
static void mcache_purge(void *arg) {
  mcache_t *cp = arg;
...
__private_extern__ boolean_t mcache_purge_cache(mcache_t *cp, boolean_t async) {
  /*
 ...
 */
__private_extern__ void mcache_free(mcache_t *cp, void *buf) {
 ...
 */
__private_extern__ void mcache_free_ext(mcache_t *cp, mcache_obj_t *list) {
  ...
  for (;;) {
     ...
     */
    if ((unsigned int)ccp->cc_objs < (unsigned int)ccp->cc_bktsize) {
      /*
 ...
 */
__private_extern__ void mcache_destroy(mcache_t *cp) {
  void **pbuf;
 ...
 */
static unsigned int mcache_slab_alloc(void *arg, mcache_obj_t ***plist,
                                      unsigned int num, int wait) {
#pragma unused(wait)
  mcache_t *cp = arg;
  unsigned int need = num;
  ...
  *list = NULL;
  ...
  for (;;) {
    ...
    base = (void *)P2ROUNDUP((intptr_t)buf + sizeof(u_int64_t), cp->mc_align);
    ...
    *pbuf = buf;
     ...
     */
    if (flags & MCF_DEBUG) {
      VERIFY(((intptr_t)base + rsize) <= ((intptr_t)buf + cp->mc_chunksize));
 ...
 */
static void mcache_slab_free(void *arg, mcache_obj_t *list,
  ...
  for (;;) {
    ...
    if (flags & MCF_DEBUG) {
      VERIFY(((intptr_t)base + rsize) <= ((intptr_t)*pbuf + cp->mc_chunksize));
 ...
 */
static void mcache_slab_audit(void *arg, mcache_obj_t *list, boolean_t alloc) {
  mcache_t *cp = arg;
  size_t rsize = P2ROUNDUP(cp->mc_bufsize, sizeof(u_int64_t));
  void *base, **pbuf;
 ...
 */
static void mcache_cpu_batch_refill(mcache_cpu_t *ccp, mcache_bkt_t *bkt,
                                    int objs) {
 ...
 */
static void mcache_cpu_refill(mcache_cpu_t *ccp, mcache_bkt_t *bkt, int objs) {
  ASSERT((ccp->cc_filled == NULL && ccp->cc_objs == -1) ||
         (ccp->cc_filled && ccp->cc_objs + objs == ccp->cc_bktsize));
  ASSERT(ccp->cc_bktsize > 0);
 ...
 */
static uint32_t mcache_bkt_batch_alloc(mcache_t *cp, mcache_bktlist_t *blp,
                                       mcache_bkt_t **list, uint32_t num) {
  mcache_bkt_t *bkt_list = NULL;
  mcache_bkt_t *bkt;
  uint32_t need = num;
 ...
 */
static void mcache_bkt_batch_free(mcache_t *cp, mcache_bktlist_t *blp,
                                  mcache_bkt_t *bkt) {
  mcache_bkt_t *nbkt;
 ...
 */
static void mcache_cache_bkt_enable(mcache_t *cp) {
  mcache_cpu_t *ccp;
  unsigned int cpu;
 ...
 */
static void mcache_bkt_purge(mcache_t *cp) {
  mcache_cpu_t *ccp;
  mcache_bkt_t *bp, *pbp;
  int objs, pobjs;
  unsigned int cpu;
 ...
 */
static void mcache_bkt_destroy(mcache_t *cp, mcache_bkt_t *bkt, int nobjs) {
  if (nobjs > 0) {
    ...
    if (cp->mc_flags & MCF_DEBUG) {
      ...
      if (cnt != nobjs) {
        panic("mcache_bkt_destroy: %s cp %p corrupted "
              ...
              cp->mc_name, (void *)cp, (void *)bkt, nobjs, cnt);
        /* NOTREACHED */
        __builtin_unreachable();
      }
    }
 ...
 */
static void mcache_bkt_ws_update(mcache_t *cp) {
  MCACHE_LOCK(&cp->mc_bkt_lock);
 ...
 */
static void mcache_bkt_ws_zero(mcache_t *cp) {
  MCACHE_LOCK(&cp->mc_bkt_lock);
 ...
 */
static void mcache_bkt_ws_reap(mcache_t *cp) {
...
static void mcache_reap_timeout(thread_call_param_t dummy __unused,
...
static void mcache_reap_done(void *flag) {
  uint64_t deadline, leeway;

  clock_interval_to_deadline(mcache_reap_interval, NSEC_PER_SEC, &deadline);
  clock_interval_to_absolutetime_interval(mcache_reap_interval_leeway,
                                          NSEC_PER_SEC, &leeway);
  thread_call_enter_delayed_with_leeway(mcache_reap_tcall, flag, deadline,
                                        leeway, THREAD_CALL_DELAY_LEEWAY);
}

static void mcache_reap_start(void *arg) {
  UInt32 *flag = arg;
...
__private_extern__ void mcache_reap(void) {
  ...
  if (mcache_llock_owner == current_thread() || !OSCompareAndSwap(0, 1, flag)) {
    return;
  }
...
__private_extern__ void mcache_reap_now(mcache_t *cp, boolean_t purge) {
  if (purge) {
    mcache_bkt_purge(cp);
    mcache_cache_bkt_enable(cp);
  } else {
...
static void mcache_cache_reap(mcache_t *cp) { mcache_bkt_ws_reap(cp); }

/*
 * Performs period maintenance on a cache.
 */
static void mcache_cache_update(mcache_t *cp) {
 ...
 */
static void mcache_cache_bkt_resize(void *arg) {
  mcache_t *cp = arg;
 ...
 */
static void mcache_cache_enable(void *arg) {
  mcache_t *cp = arg;
...
static void mcache_update_timeout(__unused void *arg) {
  uint64_t deadline, leeway;

  clock_interval_to_deadline(mcache_reap_interval, NSEC_PER_SEC, &deadline);
  clock_interval_to_absolutetime_interval(mcache_reap_interval_leeway,
                                          NSEC_PER_SEC, &leeway);
  thread_call_enter_delayed_with_leeway(mcache_update_tcall, NULL, deadline,
                                        leeway, THREAD_CALL_DELAY_LEEWAY);
}
...
static void mcache_applyall(void (*func)(mcache_t *)) {
  mcache_t *cp;

  MCACHE_LIST_LOCK();
  LIST_FOREACH(cp, &mcache_head, mc_list) { func(cp); }
  MCACHE_LIST_UNLOCK();
}

static void mcache_dispatch(void (*func)(void *), void *arg) {
  ASSERT(func != NULL);
  timeout(func, arg, hz / 1000);
}

__private_extern__ void mcache_buffer_log(mcache_audit_t *mca, void *addr,
                                          mcache_t *cp,
  ...
  transaction->mca_depth =
      (uint16_t)OSBacktrace(stack, MCACHE_STACK_DEPTH + 1) - 1;
  bcopy(&stack[1], transaction->mca_stack, sizeof(transaction->mca_stack));
  ...
  mca->mca_next_trn = (mca->mca_next_trn + 1) % mca_trn_max;
}
...
OS_NOINLINE static void *mcache_verify_set_pattern(u_int64_t old, u_int64_t new,
                                                   void *buf_arg, size_t size) {
  u_int64_t *buf_end = (u_int64_t *)((void *)((char *)buf_arg + size));
  u_int64_t *buf;
  ...
  for (buf = buf_arg; buf < buf_end; buf++) {
    if (*buf != old) {
      mcache_set_pattern(old, buf_arg, (uintptr_t)buf - (uintptr_t)buf_arg);
      return buf;
    }
    *buf = new;
  }
...
__private_extern__ void mcache_audit_free_verify(mcache_audit_t *mca,
  ...
  *oaddr64 = MCACHE_FREE_PATTERN;
  ...
  if ((oaddr64 = mcache_verify_pattern(MCACHE_FREE_PATTERN, (caddr_t)base,
                                       size)) != NULL) {
...
__private_extern__ void mcache_audit_free_verify_set(mcache_audit_t *mca,
                                                     void *base, size_t offset,
                                                     size_t size) {
  void *addr;
  u_int64_t *oaddr64;
...
#define DUMP_TRN_FIELDS(s, x)                                                  \
  s, mca->mca_trns[x].mca_thread, mca->mca_trns[x].mca_depth,                  \
...
__private_extern__ char *mcache_dump_mca(char buf[static DUMP_MCA_BUF_SIZE],
                                         mcache_audit_t *mca) {
  snprintf(buf, DUMP_MCA_BUF_SIZE,
           ...
           mca->mca_cache ? mca->mca_cache->mc_name : "?", mca->mca_next_trn,
