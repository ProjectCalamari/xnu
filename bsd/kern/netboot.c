...
void *IOBSDRegistryEntryForDeviceTree(const char *path);

void IOBSDRegistryEntryRelease(void *entry);

const void *IOBSDRegistryEntryGetData(void *entry, const char *property_name,
                                      int *packet_length);
 ...
 */
static __inline__ char *find_colon(char *str) {
  char *start = str;
  char *colon;
...
static boolean_t parse_image_path(char *path, struct in_addr *iaddr_p,
                                  char const **host, char **mount_dir,
                                  char **image_path) {
  if (path[0] >= '0' && path[0] <= '9') {
    return parse_booter_path(path, iaddr_p, host, mount_dir, image_path);
  }
  return parse_netboot_path(path, iaddr_p, host, mount_dir, image_path);
}

static boolean_t get_root_path(char *root_path) {
  void *entry;
  ...
  if (pkt != NULL) {
    ...
    path = (const char *)dhcpol_find(&options, dhcptag_root_path_e, &len, NULL);
    if (path) {
...
static void save_path(char **str_p, size_t *length_p, char *path) {
...
static struct netboot_info *netboot_info_init(struct in_addr iaddr) {
  boolean_t have_root_path = FALSE;
  struct netboot_info *info = NULL;
  char *root_path = NULL;

  info = (struct netboot_info *)kalloc_type(struct netboot_info,
                                            Z_WAITOK | Z_ZERO);
  ...
  if (have_root_path) {
    ...
    if (parse_image_path(root_path, &server_ip, &server_name, &mount_point,
                         &image_path)) {
      ...
      save_path(&info->image_path, &info->image_path_length, root_path);
...
static void netboot_info_free(struct netboot_info **info_p) {
  ...
  if (info) {
    ...
    kfree_data(info->second_image_path, info->second_image_path_length);
...
boolean_t netboot_rootpath(struct in_addr *server_ip, char *name,
  ...
  if (path_len < S_netboot_info_p->mount_point_length) {
    printf("netboot: path too small %zu < %zu\n", path_len,
           S_netboot_info_p->mount_point_length);
    return FALSE;
  }
...
static boolean_t get_ip_parameters(struct in_addr *iaddr_p,
                                   struct in_addr *netmask_p,
  ...
  if (pkt != NULL) {
    ...
    *iaddr_p = reply->dp_yiaddr;
    ip = (const struct in_addr *)dhcpol_find(&options, dhcptag_subnet_mask_e,
                                             &len, NULL);
    if (ip) {
      *netmask_p = *ip;
    }
    ip = (const struct in_addr *)dhcpol_find(&options, dhcptag_router_e, &len,
                                             NULL);
    if (ip) {
      *router_p = *ip;
    }
...
static int route_cmd(int cmd, struct in_addr d, struct in_addr g,
                     struct in_addr m, uint32_t more_flags,
  ...
  error = rtrequest_scoped(cmd, (struct sockaddr *)&dst, (struct sockaddr *)&gw,
                           (struct sockaddr *)&mask, flags, NULL, ifscope);

  return error;
}

static int default_route_add(struct in_addr router, boolean_t proxy_arp) {
  uint32_t flags = 0;
...
static struct ifnet *find_interface(void) {
...
static int inet_aifaddr(struct socket *so, const char *name,
...
int netboot_setup(void) {
  ...
  if (S_netboot_info_p == NULL || S_netboot_info_p->image_path == NULL) {
    goto done;
  }
  printf("netboot_setup: calling imageboot_mount_image\n");
  error =
      imageboot_mount_image(S_netboot_info_p->image_path, -1, IMAGEBOOT_DMG);
  if (error != 0) {
...
int netboot_root(void) { return S_netboot; }
