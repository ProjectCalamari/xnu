...
static const char *memorystatus_priority_band_name(int32_t priority) {
  switch (priority) {
...
/*
 * Returns the current zone map size and capacity to include in the jetsam
 * snapshot. Defined in zalloc.c
 */
extern void get_zone_map_size(uint64_t *current_size, uint64_t *capacity);

/*
 * Returns the name of the largest zone and its size to include in the jetsam
 * snapshot. Defined in zalloc.c
 */
extern void get_largest_zone_info(char *zone_name, size_t zone_name_len,
                                  uint64_t *zone_size);

static int memstat_set_memlimits_locked(proc_t p, int32_t active_limit,
                                        int32_t inactive_limit,
                                        memlimit_options_t options);
static bool memstat_proc_is_active_locked(proc_t);

static int memorystatus_highwater_enabled =
 ...
 */
static inline bool memstat_update_memlimit_locked(proc_t p, bool use_active) {
 ...
 */
static int _memstat_write_memlimit_to_ledger_locked(proc_t p, bool is_active,
                                                    bool drop_lock) {
  kern_return_t kr;
  ...
  kr = task_set_phys_footprint_limit_internal(
      proc_task(p), (p->p_memstat_memlimit > 0) ? p->p_memstat_memlimit : -1,
...
#define MEMORYSTATUS_REAPER_REAP_RELAUNCH_MASK_UNKNOWN                         \
  (P_MEMSTAT_RELAUNCH_HIGH << 1)
#define MEMORYSTATUS_REAPER_REAP_RELAUNCH_MASK_DEFAULT                         \
  (P_MEMSTAT_RELAUNCH_LOW | MEMORYSTATUS_REAPER_REAP_RELAUNCH_MASK_UNKNOWN)

/*
 * For historical reasons, devices with "medium"-sized memory configs have a
 * critical:idle:pressure ratio of 4:9:15. This ratio is preserved for these
 * devices when a fixed-mb base value has not been provided by EDT/boot-arg; all
 * other devices use a 1:2:3 ratio.
 */
...
/*
 * The size of the freezer snapshot is given by memorystatus_jetsam_snapshot_max
 * / JETSAM_SNAPSHOT_FREEZER_MAX_FACTOR The freezer snapshot can be much smaller
 * than the default snapshot because it only includes apps that have been killed
 * and dasd consumes it every 30 minutes. Since the snapshots are always wired
 * we don't want to overallocate too much.
 */
...
TUNABLE(bool, memorystatus_jetsam_use_freezer_snapshot,
        "kern.jetsam_user_freezer_snapshot", true);

#define MEMORYSTATUS_FREEZE_THRESHOLD_PERCENTAGE 50UL
TUNABLE_DT(uint32_t, memorystatus_freeze_threshold_mb, "/defaults",
           "kern.memstat_freeze_mb", "memorystatus_freeze_threshold_mb", 0,
           TUNABLE_DT_NONE);
#endif /* CONFIG_FREEZE */
 ...
 * of some memorystatus data structures for testing purposes (via
 * memorystatus_control). If there's an owner, then only they may consume the
 * jetsam snapshot & set freezer probabilities. This is used when testing these
 * interface to avoid racing with other processes on the system that typically
 * use them (namely OSAnalytics & dasd).
 */
static pid_t memorystatus_testing_pid = 0;
SYSCTL_INT(_kern, OID_AUTO, memorystatus_testing_pid,
           CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_testing_pid,
           0, "");
...
#if DEVELOPMENT || DEBUG
SYSCTL_UINT(_kern, OID_AUTO, jzs_trigger_band, CTLFLAG_RW | CTLFLAG_LOCKED,
            &jzs_trigger_band, 0,
            "Priority band threshold for taking jetsam zprint snapshot");
#endif /* DEVELOPMENT || DEBUG */
#endif /* JETSAM_ZPRINT_SNAPSHOT */

static void memorystatus_init_jetsam_snapshot_header(
 ...
 */
TUNABLE(uint64_t, memstat_idle_deferral_time_s,
        "memorystatus_idle_deferral_time_s", 10);
TUNABLE(uint64_t, memstat_aging_stuck_time_s, "memorystatus_aging_stuck_time_s",
        30);
...
/* 2GB devices support an entitlement for a higher app memory limit of "almost
 * 2GB". */
...
TUNABLE(bool, memstat_ignore_task_limit_increase,
        "memstat_no_task_limit_increase", false);
...
TUNABLE_WRITEABLE(memorystatus_log_level_t, memorystatus_log_level,
                  "memorystatus_log_level", MEMORYSTATUS_LOG_LEVEL_DEFAULT);

#if DEBUG || DEVELOPMENT
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_log_level,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_log_level,
            MEMORYSTATUS_LOG_LEVEL_DEFAULT, "");
#endif
...
#if DEVELOPMENT || DEBUG
static LCK_MTX_DECLARE(disconnect_page_mappings_mutex,
                       &memorystatus_lock_group);
#endif /* DEVELOPMENT || DEBUG */

/* Idle guard handling */

static void memorystatus_perform_idle_demotion(__unused void *spare1,
                                               __unused void *spare2);
...
boolean_t is_knote_registered_modify_task_pressure_bits(struct knote *, int,
                                                        task_t,
                                                        vm_pressure_level_t,
                                                        vm_pressure_level_t);
void memorystatus_klist_reset_all_for_level(
    vm_pressure_level_t pressure_level_to_clear);
void memorystatus_send_low_swap_note(void);
boolean_t memorystatus_kill_elevated_process(uint32_t cause,
                                             os_reason_t jetsam_reason,
                                             unsigned int band, int aggr_count,
                                             uint32_t *errors,
                                             uint64_t *memory_reclaimed);
static bool memorystatus_kill_proc(proc_t p, uint32_t cause,
                                   os_reason_t jetsam_reason, bool *killed,
...
static bool memstat_kill_top_process(uint32_t cause, os_reason_t jetsam_reason,
                                     ...
                                     memstat_kill_options_t options,
 ...
 *   where we have no more swap / compressor space OR are under critical
 * pressure.
 */
...
SYSCTL_NODE(_kern, OID_AUTO, memorystatus, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "memorystatus subsystem");

/*
 * Idle delay timeout factors for daemons based on relaunch behavior. Only used
 * in kJetsamAgingPolicySysProcsReclaimedFirst aging policy.
 */
...
/*
 * For the kJetsamAgingPolicySysProcsReclaimedFirst aging policy, treat apps as
 * well behaved daemons for aging purposes.
 */
#define kJetsamAppsIdleDelayTimeRatio (kJetsamSysProcsIdleDelayTimeLowRatio)

static uint64_t memorystatus_sysprocs_idle_time(proc_t p) {
  ...
  case P_MEMSTAT_RELAUNCH_LOW:
    idle_delay_time = memorystatus_sysprocs_idle_delay_time /
                      kJetsamSysProcsIdleDelayTimeLowRatio;
    break;
  case P_MEMSTAT_RELAUNCH_MED:
    idle_delay_time = memorystatus_sysprocs_idle_delay_time /
                      kJetsamSysProcsIdleDelayTimeMedRatio;
    break;
  case P_MEMSTAT_RELAUNCH_HIGH:
    idle_delay_time = memorystatus_sysprocs_idle_delay_time /
                      kJetsamSysProcsIdleDelayTimeHighRatio;
    break;
  default:
...
static uint64_t memorystatus_apps_idle_time(__unused proc_t p) {
  return memorystatus_apps_idle_delay_time / kJetsamAppsIdleDelayTimeRatio;
}

static uint64_t _memstat_sysprocs_aging_stuck_delay_time(__unused proc_t p) {
  return memorystatus_aging_stuck_delay_time;
}

static int sysctl_jetsam_set_sysprocs_idle_delay_time SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)

  int error = 0, val = 0, old_time_in_secs = 0;
  uint64_t old_time_in_ns = 0;

  absolutetime_to_nanoseconds(memorystatus_sysprocs_idle_delay_time,
                              &old_time_in_ns);
  ...
  nanoseconds_to_absolutetime((uint64_t)val * NSEC_PER_SEC,
                              &memorystatus_sysprocs_idle_delay_time);

  return 0;
}

SYSCTL_PROC(_kern_memorystatus, OID_AUTO, sysprocs_idle_delay_time_ns,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            sysctl_jetsam_set_sysprocs_idle_delay_time, "I",
            "Aging window for system processes");

static int sysctl_jetsam_set_apps_idle_delay_time SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)

  int error = 0, val = 0, old_time_in_secs = 0;
  uint64_t old_time_in_ns = 0;

  absolutetime_to_nanoseconds(memorystatus_apps_idle_delay_time,
                              &old_time_in_ns);
  old_time_in_secs = (int)(old_time_in_ns / NSEC_PER_SEC);
...
SYSCTL_PROC(_kern_memorystatus, OID_AUTO, apps_idle_delay_time_ns,
            CTLTYPE_INT | CTLFLAG_RW, 0, 0,
            sysctl_jetsam_set_apps_idle_delay_time, "I",
            "Aging window for applications");

SYSCTL_INT(_kern, OID_AUTO, max_task_pmem,
           CTLFLAG_RD | CTLFLAG_LOCKED | CTLFLAG_KERN, &max_task_footprint_mb,
           0, "");

#if __arm64__
int legacy_footprint_bonus_mb =
    50; /* This value was chosen after looking at the top 30 apps
         * that needed the additional room in their footprint when
         * the 'correct' accounting methods were applied to them.
         */

#if DEVELOPMENT || DEBUG
SYSCTL_INT(_kern, OID_AUTO, legacy_footprint_bonus_mb,
           CTLFLAG_RW | CTLFLAG_LOCKED, &legacy_footprint_bonus_mb, 0, "");
#endif /* DEVELOPMENT || DEBUG */
/*
 ...
 */
static void memorystatus_raise_memlimit_locked(proc_t p,
  ...
  memstat_set_memlimits_locked(p, memlimit_mb_active, memlimit_mb_inactive,
                               memlimit_options);
}

void memorystatus_act_on_legacy_footprint_entitlement(
    proc_t p, boolean_t footprint_increase) {
  int memlimit_mb_active = 0, memlimit_mb_inactive = 0;
  ...
  memorystatus_raise_memlimit_locked(p, memlimit_mb_active,
                                     memlimit_mb_inactive);
...
void memorystatus_act_on_ios13extended_footprint_entitlement(proc_t p) {
  proc_list_lock();
  memorystatus_raise_memlimit_locked(
      p, memorystatus_ios13extended_footprint_limit_mb,
...
SYSCTL_INT(_kern, OID_AUTO, memorystatus_level, CTLFLAG_RD | CTLFLAG_LOCKED,
           &memorystatus_level, 0, "");

int memorystatus_get_level(__unused struct proc *p,
                           struct memorystatus_get_level_args *args,
                           __unused int *ret) {
  user_addr_t level = 0;
...
static void memorystatus_thread(void *param __unused,
                                wait_result_t wr __unused);

/* Memory Limits */

static bool memorystatus_kill_specific_process(pid_t victim_pid, uint32_t cause,
                                               os_reason_t jetsam_reason);
static bool memstat_kill_process_sync(pid_t victim_pid, uint32_t cause,
                                      os_reason_t jetsam_reason);

static int memorystatus_cmd_set_memlimit_properties(pid_t pid,
                                                    user_addr_t buffer,
                                                    size_t buffer_size,
...
#if DEBUG || DEVELOPMENT
static int
memorystatus_cmd_set_diag_memlimit_properties(pid_t pid, user_addr_t buffer,
                                              size_t buffer_size,
                                              __unused int32_t *retval);
static int
memorystatus_cmd_get_diag_memlimit_properties(pid_t pid, user_addr_t buffer,
                                              size_t buffer_size,
                                              __unused int32_t *retval);
static int memorystatus_set_diag_memlimit_properties_internal(
    proc_t p, memorystatus_diag_memlimit_properties_t *p_entry);
static int memorystatus_get_diag_memlimit_properties_internal(
    proc_t p, memorystatus_diag_memlimit_properties_t *p_entry);
#endif // DEBUG || DEVELOPMENT
static int
memorystatus_set_memlimit_properties(pid_t pid,
                                     memorystatus_memlimit_properties_t *entry);

static int memorystatus_cmd_get_memlimit_properties(pid_t pid,
                                                    user_addr_t buffer,
                                                    size_t buffer_size,
                                                    __unused int32_t *retval);

static int memorystatus_cmd_get_memlimit_excess_np(pid_t pid, uint32_t flags,
                                                   user_addr_t buffer,
                                                   size_t buffer_size,
                                                   __unused int32_t *retval);

static void memorystatus_get_memlimit_properties_internal(
    proc_t p, memorystatus_memlimit_properties_t *p_entry);
...
uint32_t memorystatus_jld_eval_period_msecs =
    0; /* Init pass sets this based on device memory size */
int memorystatus_jld_max_kill_loops =
    2; /* How many times should we try and kill up to the target band */

/*
 * A FG app can request that the aggressive jetsam mechanism display some
 * leniency in the FG band. This 'lenient' mode is described as:
 * --- if aggressive jetsam kills an app in the FG band and gets back
 * >=AGGRESSIVE_JETSAM_LENIENT_MODE_THRESHOLD memory, it will stop the
 * aggressive march further into and up the jetsam bands.
 *
 * RESTRICTIONS:
 * - Such a request is respected/acknowledged only once while that 'requesting'
 * app is in the FG band i.e. if aggressive jetsam was needed and the 'lenient'
 * mode was deployed then that's it for this special mode while the app is in
 * the FG band.
 *
 * - If the app is still in the FG band and aggressive jetsam is needed again,
 * there will be no stop-and-check the next time around.
 *
 * - Also, the transition of the 'requesting' app away from the FG band will
 * void this special behavior.
 */
...
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_jld_eval_period_msecs,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_jld_eval_period_msecs, 0,
            "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_jld_max_kill_loops,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_jld_max_kill_loops, 0,
            "");
#endif /* DEVELOPMENT || DEBUG */
...
static void memorystatus_init_jetsam_snapshot_locked(
    memorystatus_jetsam_snapshot_t *od_snapshot, uint32_t ods_list_count);
static boolean_t memorystatus_init_jetsam_snapshot_entry_locked(
    proc_t p, memorystatus_jetsam_snapshot_entry_t *entry, uint64_t gencount);
static void
...
static void memorystatus_get_task_phys_footprint_page_counts(
    task_t task, uint64_t *internal_pages, uint64_t *internal_compressed_pages,
    uint64_t *purgeable_nonvolatile_pages,
    uint64_t *purgeable_nonvolatile_compressed_pages,
    uint64_t *alternate_accounting_pages,
    uint64_t *alternate_accounting_compressed_pages,
    uint64_t *iokit_mapped_pages, uint64_t *page_table_pages,
    uint64_t *frozen_to_swap_pages, uint64_t *neural_nofootprint_total_pages);

static void memorystatus_get_task_memory_region_count(task_t task,
                                                      uint64_t *count);
...
static bool memorystatus_kill_processes_aggressive(
    uint32_t cause, int aggr_count, int32_t priority_max, int32_t max_kills,
...
static int memstat_sort_bucket(unsigned int bucket_index,
                               memorystatus_jetsam_sort_order_t sort_order);
static void
memstat_sort_coals_locked(unsigned int bucket_index,
                          memorystatus_jetsam_sort_order_t sort_order);
...
TUNABLE_DT(memorystatus_jetsam_sort_order_t, memstat_jetsam_fg_sort_order,
           "/defaults", "kern.memstat_fg_sort_order", "memstat_fg_sort_order",
           JETSAM_SORT_FG_DEFAULT, TUNABLE_DT_NONE);
 ...
 *    from userspace
 *  - ctd (clear-the-decks): offset applied to non-critical thresholds upon
...
int32_t memstat_reaper_min_age_apps_secs =
    MEMORYSTATUS_REAPER_MIN_AGE_APPS_SECS_DEFAULT;
...
uint32_t memstat_reaper_reap_relaunch_mask =
    MEMORYSTATUS_REAPER_REAP_RELAUNCH_MASK_DEFAULT;
...
uint64_t memstat_oldest_reapable_proc_prio_start =
    MEMSTAT_OLDEST_REAPABLE_PROC_PRIO_START_UNKNOWN;
...
TUNABLE_DT(uint32_t, memorystatus_critical_threshold_mb, "/defaults",
           "kern.memstat_critical_mb", "memorystatus_critical_threshold_mb", 0,
           TUNABLE_DT_NONE);
TUNABLE_DT(uint32_t, memorystatus_idle_threshold_mb, "/defaults",
           "kern.memstat_idle_mb", "memorystatus_idle_threshold_mb", 0,
           TUNABLE_DT_NONE);
TUNABLE_DT(uint32_t, memorystatus_pressure_threshold_mb, "/defaults",
           "kern.memstat_pressure_mb", "memorystatus_pressure_threshold_mb", 0,
           TUNABLE_DT_NONE);
TUNABLE_DT(uint32_t, memorystatus_reaper_threshold_mb, "/defaults",
           "kern.memstat_reaper_mb", "memorystatus_reaper_threshold_mb", 0,
           TUNABLE_DT_NONE);
TUNABLE_DT(uint32_t, memstat_ballast_offset_mb, "/defaults",
           "kern.memstat_ballast_mb", "memstat_ballast_offset_mb", 0,
           TUNABLE_DT_NONE);
 ...
 */
uint32_t _Atomic memorystatus_kill_counts[JETSAM_PRIORITY_MAX + 1]
                                         [JETSAM_REASON_MEMORYSTATUS_MAX - 1];
uint32_t _Atomic memorystatus_idle_exit_kill_count = 0;

TUNABLE_DT(int32_t, memorystatus_reaper_minimum_age_seconds, "/defaults",
           "kern.memstat_reaper_minage_secs",
           "memorystatus_reaper_minimum_age_seconds",
           MEMORYSTATUS_REAPER_MIN_AGE_SECS_DEFAULT, TUNABLE_DT_NONE);
TUNABLE_DT(int32_t, memorystatus_reaper_minimum_age_apps_seconds, "/defaults",
           "kern.memstat_reaper_minapp_secs",
           "memorystatus_reaper_minimum_age_apps_seconds",
           MEMORYSTATUS_REAPER_MIN_AGE_APPS_SECS_DEFAULT, TUNABLE_DT_NONE);
TUNABLE_DT(uint32_t, memorystatus_reaper_rescan_delay_seconds, "/defaults",
           "kern.memstat_reaper_rescan_secs",
           "memorystatus_reaper_rescan_delay_seconds",
           MEMORYSTATUS_REAPER_RESCAN_SECS_DEFAULT, TUNABLE_DT_NONE);
TUNABLE_DT(boolean_t, memorystatus_reaper_enabled, "/defaults",
           "kern.memstat_reaper_enabled", "memorystatus_reaper_enabled",
           MEMORYSTATUS_REAPER_ENABLED_DEFAULT, TUNABLE_DT_NONE);

#if CONFIG_JETSAM
TUNABLE_DT_WRITEABLE(unsigned int, memorystatus_swap_all_apps, "/defaults",
                     "kern.swap_all_apps", "kern.swap_all_apps", false,
                     TUNABLE_DT_NONE);
...
#if DEVELOPMENT || DEBUG
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_available_pages,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_available_pages, 0, "");
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_swapin_trigger_pages,
            CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_swapin_trigger_pages, 0,
            "");
#else
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_available_pages,
            CTLFLAG_RD | CTLFLAG_MASKED | CTLFLAG_LOCKED,
            &memorystatus_available_pages, 0, "");
#endif /* DEVELOPMENT || DEBUG */
SYSCTL_UINT(_kern, OID_AUTO, memorystatus_swap_all_apps,
            CTLFLAG_RD | CTLFLAG_LOCKED, &memorystatus_swap_all_apps, 0, "");

static int memorystatus_cmd_set_jetsam_memory_limit(pid_t pid,
                                                    int32_t high_water_mark,
                                                    __unused int32_t *retval,
                                                    boolean_t is_fatal_limit);

proc_name_t memorystatus_jetsam_proc_name_panic; /* Panic when we are about to
                                                    jetsam this process. */
uint32_t memorystatus_jetsam_proc_cause_panic =
    0; /* If specified, panic only when we are about to jetsam the process above
          for this cause. */
uint32_t memorystatus_jetsam_proc_size_panic =
    0; /* If specified, panic only when we are about to jetsam the process above
          and its footprint is more than this in MB. */

/* If set, kill swappable processes when we're low on swap space. Currently off
 * until we can allocate more swap space (rdar://87800902) */
...
static inline uint32_t roundToNearestMB(uint32_t in) {
  return (in + ((1 << 20) - 1)) >> 20;
}

#if DEVELOPMENT || DEBUG
static int memorystatus_cmd_increase_jetsam_task_limit(pid_t pid,
                                                       uint32_t byte_increase);
#endif
...
static void memorystatus_debug_dump_bucket_locked(unsigned int bucket_index) {
  proc_t p = NULL;
  uint64_t bytes = 0;
  int ledger_limit = 0;
  ...
  memorystatus_log_debug(
      "memorystatus_debug_dump ***START*(PAGE_SIZE_64=%llu)**\n", PAGE_SIZE_64);
  memorystatus_log_debug(
      "bucket [pid]       [pages / MB]     [state]      [EP / RP / AP]   dirty "
      "    deadline [L-limit / C-limit / A-limit / IA-limit] name\n");
  ...
  while (p) {
    ...
    memorystatus_log_debug(
        "%2d     [%5d]     [%5lld /%3lldMB]   0x%-8x   [%2d / %2d / %2d]   "
        "0x%-3x   %10lld    [%3d / %3d%s / %3d%s / %3d%s]   %s\n",
        b, proc_getpid(p),
        (bytes /
         PAGE_SIZE_64), /* task's footprint converted from bytes to pages     */
        (bytes /
         (1024ULL * 1024ULL)), /* task's footprint converted from bytes to MB */
        p->p_memstat_state, p->p_memstat_effectivepriority,
        p->p_memstat_requestedpriority, p->p_memstat_assertionpriority,
        p->p_memstat_dirty, p->p_memstat_idledeadline, ledger_limit,
...
static int sysctl_memorystatus_debug_dump_bucket SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_debug_dump_this_bucket,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
            &memorystatus_debug_dump_this_bucket, 0,
            sysctl_memorystatus_debug_dump_bucket, "I", "");

/* Debug aid to aid determination of limit */

static int sysctl_memorystatus_highwater_enable SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg2)
  proc_t p;
  unsigned int b = 0;
  int error, enable = 0;
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_highwater_enabled,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
            &memorystatus_highwater_enabled, 0,
            sysctl_memorystatus_highwater_enable, "I", "");

SYSCTL_INT(_kern, OID_AUTO, memorystatus_idle_snapshot,
           CTLFLAG_RW | CTLFLAG_LOCKED, &memorystatus_idle_snapshot, 0, "");

#endif /* DEVELOPMENT || DEBUG */

#if CONFIG_JETSAM
#if DEVELOPMENT || DEBUG
static int memstat_page_shortage_threshold_sysctl_handler SYSCTL_HANDLER_ARGS {
  uint32_t threshold;
...
SYSCTL_INT(_kern_memorystatus, OID_AUTO, ballast_offset_pages,
           CTLFLAG_RD | CTLFLAG_LOCKED, &memstat_ballast_offset, 0, "");
SYSCTL_INT(_kern_memorystatus, OID_AUTO, ctd_offset_pages,
           CTLFLAG_RD | CTLFLAG_LOCKED, &memstat_ctd_offset, 0, "");
#endif /* DEBUG || DEVELOPMENT */

EXPERIMENT_FACTOR_LEGACY_UINT(_kern_memorystatus, reaper_enabled,
                              &memstat_reaper_enabled, FALSE, TRUE, "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern_memorystatus, reaper_min_age_secs,
                              &memstat_reaper_min_age_secs, 0, UINT32_MAX, "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern_memorystatus, reaper_min_age_apps_secs,
                              &memstat_reaper_min_age_apps_secs, 0, UINT32_MAX,
                              "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern_memorystatus, reaper_max_priority,
                              &memstat_reaper_max_priority, 0,
                              JETSAM_PRIORITY_MAX, "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern_memorystatus, reaper_reap_relaunch_mask,
                              &memstat_reaper_reap_relaunch_mask, 0, UINT32_MAX,
                              "");
EXPERIMENT_FACTOR_LEGACY_UINT(_kern_memorystatus, reaper_rescan_secs,
                              &memstat_reaper_rescan_secs, 0, UINT32_MAX, "");
...
static int memstat_page_shortage_threshold_experiment_handler
    SYSCTL_HANDLER_ARGS {
  ...
  if ((arg1 == &memstat_soft_threshold || arg1 == &memstat_idle_threshold ||
...
#if DEVELOPMENT || DEBUG
#define MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS                                 \
  (CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_ANYBODY | CTLFLAG_LOCKED)
#else /* RELEASE */
#define MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS                                 \
  (CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_ANYBODY | CTLFLAG_LOCKED | CTLFLAG_MASKED)
#endif /* DEVELOPMENT || DEBUG */

EXPERIMENT_FACTOR_LEGACY_PROC(
    _kern_memorystatus, soft_threshold_mb,
    MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS, &memstat_soft_threshold, 0,
    memstat_page_shortage_threshold_experiment_handler, "IU",
    "The minimum amount of available memory to maintain before killing "
    "processes which have violated there soft memory limit");

EXPERIMENT_FACTOR_LEGACY_PROC(
    _kern_memorystatus, idle_threshold_mb,
    MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS, &memstat_idle_threshold, 0,
    memstat_page_shortage_threshold_experiment_handler, "IU",
    "The minimum amount of available memory to maintain before exiting idle "
    "processes");
EXPERIMENT_FACTOR_LEGACY_PROC(
    _kern_memorystatus, critical_threshold_mb,
    MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS, &memstat_critical_threshold, 0,
    memstat_page_shortage_threshold_experiment_handler, "IU",
    "The minimum amount of available memory to maintain before killing "
    "non-idle "
    "processes");
EXPERIMENT_FACTOR_LEGACY_PROC(
    _kern_memorystatus, reaper_threshold_mb,
    MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS, &memstat_reaper_threshold, 0,
    memstat_page_shortage_threshold_experiment_handler, "IU",
    "The minimum amount of available memory to maintain before killing "
    "long-idle "
    "processes");
EXPERIMENT_FACTOR_LEGACY_PROC(
    _kern_memorystatus, ballast_offset_mb,
    MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS, &memstat_ballast_offset, 0,
    memstat_page_shortage_threshold_experiment_handler, "IU",
    "An offset to apply to all non-critical page shortage thresholds when "
    "ballast is filling");
EXPERIMENT_FACTOR_LEGACY_PROC(
    _kern_memorystatus, clear_the_decks_offset_mb,
    MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS, &memstat_ctd_offset, 0,
    memstat_page_shortage_threshold_experiment_handler, "IU",
    "An offset to apply to all non-critical page shortage thresholds when "
...
int memorystatus_ballast_control(bool drain) {
  ...
  if (drain) {
     ...
     */
    memorystatus_policy_t orig_policy =
        os_atomic_or_orig(&memstat_policy_config,
    ...
    /*
     * Flood the ballast tanks, removing the extra buoyancy by allowing them to
     * be filled with "unavailable" memory.
     */
    memorystatus_policy_t orig_policy = os_atomic_andnot_orig(
        &memstat_policy_config, (memorystatus_policy_t)kPolicyBallastDrain,
        relaxed);
    if (!(orig_policy & kPolicyBallastDrain)) {
      /* already disabled */
      return 0;
    }
    assertf(fast_jetsam_enabled,
            "ballast was drained while fast-jetsam was disabled");
...
static int sysctl_kern_memorystatus_ballast_drain SYSCTL_HANDLER_ARGS {
  int error = 0;

  boolean_t drained =
      os_atomic_load(&memstat_policy_config, relaxed) & kPolicyBallastDrain
          ? TRUE
          : FALSE;
  ...
  /*
   * Writers must be root or have the com.apple.private.kernel.jetsam
   * entitlement
   */
 ...
 */
static int memstat_clear_the_decks(bool clear) {
  if (!fast_jetsam_enabled) {
  ...
  if (clear) {
     ...
     */
    memorystatus_policy_t orig_policy = os_atomic_andnot_orig(
        &memstat_policy_config, (memorystatus_policy_t)kPolicyClearTheDecks,
        relaxed);
    if (!(orig_policy & kPolicyClearTheDecks)) {
      return EALREADY;
    }
    assertf(fast_jetsam_enabled,
            "clear the decks was set while fast-jetsam was disabled");
...
static int sysctl_kern_memorystatus_decks_cleared SYSCTL_HANDLER_ARGS {
  int error = 0;

  boolean_t cleared =
      os_atomic_load(&memstat_policy_config, relaxed) & kPolicyClearTheDecks
          ? TRUE
          : FALSE;
...
SYSCTL_PROC(_kern_memorystatus, OID_AUTO, decks_cleared,
            MEMSTAT_PAGE_SHORTAGE_EXPERIMENT_FLAGS, 0, 0,
            sysctl_kern_memorystatus_decks_cleared, "I",
...
kernel_thread_start_priority(thread_continue_t continuation, void *parameter,
                             integer_t priority, thread_t *new_thread);

#if DEVELOPMENT || DEBUG

static int sysctl_memorystatus_disconnect_page_mappings SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_disconnect_page_mappings,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED | CTLFLAG_MASKED, 0, 0,
            &sysctl_memorystatus_disconnect_page_mappings, "I", "");
 ...
 */
static void
memstat_sort_bucket_locked(unsigned int bucket_index,
                           memorystatus_jetsam_sort_order_t sort_order) {
  LCK_MTX_ASSERT(&proc_list_mlock, LCK_MTX_ASSERT_OWNED);
  ...
  while (p) {
    ...
    if (prev_max_proc != max_proc) {
      ...
      if (insert_after_proc == NULL) {
        ...
        TAILQ_INSERT_AFTER(&current_bucket->list, insert_after_proc, max_proc,
                           p_memstat_list);
      }
...
proc_t memorystatus_get_first_proc_locked(unsigned int *bucket_index,
                                          boolean_t search) {
  memstat_bucket_t *current_bucket;
  proc_t next_p;
...
proc_t memorystatus_get_next_proc_locked(unsigned int *bucket_index, proc_t p,
                                         boolean_t search) {
  memstat_bucket_t *current_bucket;
  proc_t next_p;
...
static jetsam_state_t jetsam_current_thread() {
...
#if CONFIG_JETSAM
static void initialize_entitled_max_task_limit() {
#if !XNU_TARGET_OS_XR
  /**
   ...
   */
  if (!PE_parse_boot_argn("entitled_max_task_pmem", NULL, 0) &&
      memorystatus_swap_all_apps) {
    /*
     ...
     * infinite number of apps open at a time, and cannot swap our way to
     * safety.
     */
  ...
  if (memorystatus_entitled_bincompat_max_task_footprint_mb < 0) {
    memorystatus_log_error(
        "Invalid value (%d) for entitled_bincompat_max_task_pmem. "
        "Setting to 0\n",
        memorystatus_entitled_bincompat_max_task_footprint_mb);
    memorystatus_entitled_bincompat_max_task_footprint_mb = 0;
  }
  ...
  if (memorystatus_entitled_dev_max_task_footprint_mb < -1) {
    memorystatus_log_error(
        "Invalid value (%d) for entitled_max_developer_task_pmem. "
        "Setting to 0\n",
        memorystatus_entitled_dev_max_task_footprint_mb);
    memorystatus_entitled_dev_max_task_footprint_mb = 0;
  } else if (memorystatus_entitled_dev_max_task_footprint_mb == -1) {
    memorystatus_entitled_dev_max_task_footprint_mb =
        (int32_t)(max_mem_actual >> 20);
  }
...
__private_extern__ void memorystatus_init(void) {
  ...
  memorystatus_frozen_shared_mb_max =
      ((MAX_FROZEN_SHARED_MB_PERCENT * max_task_footprint_mb) /
       100); /* 10% of the system wide task limit */
  memorystatus_freeze_shared_mb_per_process_max =
      (memorystatus_frozen_shared_mb_max / 4);
  ...
  memorystatus_max_frozen_demotions_daily =
      MAX_FROZEN_PROCESS_DEMOTIONS_DEFAULT;
  memorystatus_thaw_count_demotion_threshold =
      MIN_THAW_DEMOTION_THRESHOLD_DEFAULT;
  memorystatus_min_thaw_refreeze_threshold =
      MIN_THAW_REFREEZE_THRESHOLD_DEFAULT;
#endif /* CONFIG_FREEZE */
  ...
  memorystatus_idle_demotion_call = thread_call_allocate(
      (thread_call_func_t)memorystatus_perform_idle_demotion, NULL);

  nanoseconds_to_absolutetime(memstat_idle_deferral_time_s * NSEC_PER_SEC,
                              &memorystatus_sysprocs_idle_delay_time);
  nanoseconds_to_absolutetime(memstat_idle_deferral_time_s * NSEC_PER_SEC,
                              &memorystatus_apps_idle_delay_time);
  nanoseconds_to_absolutetime(memstat_aging_stuck_time_s * NSEC_PER_SEC,
                              &memorystatus_aging_stuck_delay_time);
  assert3u(memstat_idle_deferral_time_s, >=,
           kJetsamSysProcsIdleDelayTimeLowRatio);

#if CONFIG_JETSAM
  bzero(memorystatus_jetsam_proc_name_panic,
        sizeof(memorystatus_jetsam_proc_name_panic));
  if (PE_parse_boot_argn("jetsam_proc_name_panic",
                         &memorystatus_jetsam_proc_name_panic,
                         sizeof(memorystatus_jetsam_proc_name_panic))) {
    /*
     * No bounds check to see if this is a valid cause.
     * This is a debugging aid. The callers should know precisely which cause
     * they wish to track.
     */
    PE_parse_boot_argn("jetsam_proc_cause_panic",
                       &memorystatus_jetsam_proc_cause_panic,
                       sizeof(memorystatus_jetsam_proc_cause_panic));
    PE_parse_boot_argn("jetsam_proc_size_panic",
                       &memorystatus_jetsam_proc_size_panic,
                       sizeof(memorystatus_jetsam_proc_size_panic));
  }
   ...
   * make our 'aging' code worry about a mix of processes, some of which need to
   * age and some others that need to stay elevated in the jetsam bands.
   */
  ...
  if (!PE_parse_boot_argn("jetsam_idle_snapshot", &memorystatus_idle_snapshot,
    ...
    PE_get_default("kern.jetsam_idle_snapshot", &memorystatus_idle_snapshot,
                   sizeof(memorystatus_idle_snapshot));
  }

  memorystatus_sysproc_aging_aggr_pages =
      (unsigned int)MEMSTAT_PERCENT_TOTAL_PAGES(
          MEMORYSTATUS_AGGR_SYSPROC_AGING_PERCENTAGE);

  if (max_mem <= MEMORYSTATUS_SMALL_MEMORY_THRESHOLD) {
    memorystatus_delta = (unsigned int)MEMSTAT_PERCENT_TOTAL_PAGES(
        MEMORYSTATUS_DELTA_PERCENTAGE_SMALL);
  } else {
    memorystatus_delta = (unsigned int)MEMSTAT_PERCENT_TOTAL_PAGES(
        MEMORYSTATUS_DELTA_PERCENTAGE_LARGE);
  }

  if (memorystatus_critical_threshold_mb != 0) {
    memstat_critical_threshold =
        atop_32(memorystatus_critical_threshold_mb << 20);
  } else {
    if (max_mem <= MEMORYSTATUS_SMALL_MEMORY_THRESHOLD) {
      memstat_critical_threshold =
          MEMSTAT_PERCENT_TOTAL_PAGES(MEMORYSTATUS_CRITICAL_PERCENTAGE_SMALL);
    } else {
      memstat_critical_threshold =
          MEMSTAT_PERCENT_TOTAL_PAGES(MEMORYSTATUS_CRITICAL_PERCENTAGE_LARGE);
    }
  }
  ...
  if (memorystatus_idle_threshold_mb != 0) {
    ...
    /*
     * For historical reasons, devices with "medium"-sized memory configs have a
     * different critical:idle:pressure ratio
     */
    if ((max_mem > MEMORYSTATUS_SMALL_MEMORY_THRESHOLD) &&
        (max_mem <= MEMORYSTATUS_MEDIUM_MEMORY_THRESHOLD)) {
      memstat_idle_threshold =
          (MEMORYSTATUS_IDLE_RATIO_NUM_MEDIUM * memstat_critical_threshold) /
          MEMORYSTATUS_IDLE_RATIO_DENOM_MEDIUM;
    } else {
      memstat_idle_threshold =
          (MEMORYSTATUS_IDLE_RATIO_NUM * memstat_critical_threshold) /
  ...
  if (memorystatus_pressure_threshold_mb != 0) {
    ...
    if ((max_mem > MEMORYSTATUS_SMALL_MEMORY_THRESHOLD) &&
        (max_mem <= MEMORYSTATUS_MEDIUM_MEMORY_THRESHOLD)) {
      memstat_soft_threshold = (MEMORYSTATUS_PRESSURE_RATIO_NUM_MEDIUM *
                                memstat_critical_threshold) /
                               MEMORYSTATUS_PRESSURE_RATIO_DENOM_MEDIUM;
    } else {
      memstat_soft_threshold =
          (MEMORYSTATUS_PRESSURE_RATIO_NUM * memstat_critical_threshold) /
  ...
  if (memorystatus_reaper_threshold_mb != 0) {
    ...
    if ((max_mem > MEMORYSTATUS_SMALL_MEMORY_THRESHOLD) &&
        (max_mem <= MEMORYSTATUS_MEDIUM_MEMORY_THRESHOLD)) {
      memstat_reaper_threshold =
          (MEMORYSTATUS_REAPER_RATIO_NUM_MEDIUM * memstat_critical_threshold) /
          MEMORYSTATUS_REAPER_RATIO_DENOM_MEDIUM;
    } else {
      memstat_reaper_threshold =
          (MEMORYSTATUS_REAPER_RATIO_NUM * memstat_critical_threshold) /
          MEMORYSTATUS_REAPER_RATIO_DENOM;
    }
  }
  assert3u(memstat_reaper_threshold, <, MEMSTAT_PAGE_SHORTAGE_THRESHOLD_MAX);

  if (memorystatus_reaper_minimum_age_seconds !=
      MEMORYSTATUS_REAPER_SENTINAL_VALUE_MEANING_USE_DEFAULT) {
  ...
  if (memorystatus_reaper_minimum_age_apps_seconds !=
      MEMORYSTATUS_REAPER_SENTINAL_VALUE_MEANING_USE_DEFAULT) {
    memstat_reaper_min_age_apps_secs =
        memorystatus_reaper_minimum_age_apps_seconds;
  } else {
    memstat_reaper_min_age_apps_secs =
        MEMORYSTATUS_REAPER_MIN_AGE_APPS_SECS_DEFAULT;
  ...
  if (memorystatus_reaper_rescan_delay_seconds !=
      MEMORYSTATUS_REAPER_SENTINAL_VALUE_MEANING_USE_DEFAULT) {
  ...
  /* Set the swapin trigger in pages based on the maximum size allocated for
   * each c_seg */
  memorystatus_swapin_trigger_pages = (unsigned int)atop_64(
      memorystatus_swapin_trigger_segments * c_seg_allocsize);
  ...
  memorystatus_jetsam_snapshot =
      kalloc_data(memorystatus_jetsam_snapshot_size, Z_WAITOK | Z_ZERO);
  ...
  memorystatus_jetsam_snapshot_freezer_max =
      memorystatus_jetsam_snapshot_max / JETSAM_SNAPSHOT_FREEZER_MAX_FACTOR;
  memorystatus_jetsam_snapshot_freezer_size =
      sizeof(memorystatus_jetsam_snapshot_t) +
      (sizeof(memorystatus_jetsam_snapshot_entry_t) *
       memorystatus_jetsam_snapshot_freezer_max);
  ...
  nanoseconds_to_absolutetime((uint64_t)JETSAM_SNAPSHOT_TIMEOUT_SECS *
                                  NSEC_PER_SEC,
                              &memorystatus_jetsam_snapshot_timeout);

  memset(&memorystatus_at_boot_snapshot, 0,
         sizeof(memorystatus_jetsam_snapshot_t));

#if CONFIG_FREEZE
  if (memorystatus_freeze_threshold_mb != 0) {
    memorystatus_freeze_threshold =
        (unsigned int)atop_64((uint64_t)memorystatus_freeze_threshold_mb << 20);
  } else {
    memorystatus_freeze_threshold = (unsigned int)MEMSTAT_PERCENT_TOTAL_PAGES(
        MEMORYSTATUS_FREEZE_THRESHOLD_PERCENTAGE);
  }
  ...
  if (!PE_parse_boot_argn("max_jetsam_threads", &max_jetsam_threads,
  ...
  if (PE_parse_boot_argn("memorystatus_kill_on_sustained_pressure", &i,
                         sizeof(i))) {
    if (i) {
  ...
  jetsam_threads =
      zalloc_permanent(sizeof(struct jetsam_state_s) * max_jetsam_threads,
                       ZALIGN(struct jetsam_state_s));

  /* Initialize all the jetsam threads */
  for (i = 0; i < max_jetsam_threads; i++) {
    jetsam_threads[i].inited = false;
    jetsam_threads[i].index = i;
    result = kernel_thread_start_priority(memorystatus_thread, NULL,
                                          95 /* MAXPRI_KERNEL */,
                                          &jetsam_threads[i].thread);
  ...
  jzs_meminfo =
      kalloc_data_tag(jzs_meminfo_size, Z_WAITOK, VM_KERN_MEMORY_DIAG);
#endif /* JETSAM_ZPRINT_SNAPSHOT */
...
#if DEVELOPMENT || DEBUG
  int boot_arg_val = 0;
  if (PE_parse_boot_argn("kern.swap_all_apps", &boot_arg_val,
                         sizeof(boot_arg_val))) {
    if (boot_arg_val) {
...
static void _memstat_record_kill(int32_t priority,
                                 memorystatus_kill_cause_t cause) {
  uint32_t _Atomic *count;
  uint32_t orig;

  /* Check validity of reason / cause */
  if ((priority < JETSAM_PRIORITY_IDLE) || (priority > JETSAM_PRIORITY_MAX) ||
  ...
  if ((priority == JETSAM_PRIORITY_IDLE) &&
      (cause == kMemorystatusKilledIdleExit)) {
    /* rdar://141462516 */
 ...
 */
static int jetsam_do_kill(proc_t p, int jetsam_flags,
                          os_reason_t jetsam_reason) {
  int error = 0;
  error = exit_with_reason(p, W_EXITCODE(0, SIGKILL), (int *)NULL, FALSE, FALSE,
                           jetsam_flags, jetsam_reason);
  return error;
}
 ...
 */
static bool memorystatus_do_kill(proc_t p, uint32_t cause,
                                 os_reason_t jetsam_reason,
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_DO_KILL) | DBG_FUNC_START, victim_pid, cause,
       vm_page_free_count, footprint);
  DTRACE_MEMORYSTATUS4(memorystatus_do_kill, proc_t, p, os_reason_t,
                       jetsam_reason, uint32_t, cause, uint64_t, footprint);

#if CONFIG_JETSAM
  if (*p->p_name && !strncmp(memorystatus_jetsam_proc_name_panic, p->p_name,
                             sizeof(p->p_name))) { /* name */
    if ((!memorystatus_jetsam_proc_cause_panic ||
         cause == memorystatus_jetsam_proc_cause_panic) && /* cause */
        (!memorystatus_jetsam_proc_size_panic ||
         (footprint >> 20) >=
             memorystatus_jetsam_proc_size_panic)) { /* footprint */
      panic("memorystatus_do_kill(): requested panic on jetsam of %s (cause: "
            "%d and footprint: %llu mb)",
...
#pragma unused(cause)
  ...
  if (p->p_memstat_effectivepriority >= JETSAM_PRIORITY_FOREGROUND) {
    memorystatus_log(
        "memorystatus: killing process %d [%s] in high band %s (%d) - "
        "memorystatus_available_pages: %llu\n",
        proc_getpid(p), (*p->p_name ? p->p_name : "unknown"),
        memorystatus_priority_band_name(p->p_memstat_effectivepriority),
        p->p_memstat_effectivepriority,
  ...
  /*
   * The jetsam_reason (os_reason_t) has enough information about the kill
   * cause. We don't really need jetsam_flags anymore, so it's okay that not all
   * possible kill causes have been mapped.
   */
  int jetsam_flags = P_LTERM_JETSAM;
  switch (cause) {
  case kMemorystatusKilledHiwat:
    jetsam_flags |= P_JETSAM_HIWAT;
    break;
  case kMemorystatusKilledVnodes:
    jetsam_flags |= P_JETSAM_VNODE;
    break;
  case kMemorystatusKilledVMPageShortage:
    jetsam_flags |= P_JETSAM_VMPAGESHORTAGE;
    break;
  case kMemorystatusKilledVMCompressorThrashing:
  case kMemorystatusKilledVMCompressorSpaceShortage:
    jetsam_flags |= P_JETSAM_VMTHRASHING;
    break;
  case kMemorystatusKilledFCThrashing:
    jetsam_flags |= P_JETSAM_FCTHRASHING;
    break;
  case kMemorystatusKilledPerProcessLimit:
    jetsam_flags |= P_JETSAM_PID;
    break;
  case kMemorystatusKilledIdleExit:
    jetsam_flags |= P_JETSAM_IDLEEXIT;
    break;
  case kMemorystatusKilledConclaveLimit:
    jetsam_flags |= P_JETSAM_PID;
    break;
  }
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_DO_KILL) | DBG_FUNC_END, victim_pid,
       memstat_effectivepriority, vm_page_free_count, error);

  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_COMPACTOR_RUN) | DBG_FUNC_START, victim_pid,
       cause, vm_page_free_count, footprint);

  if (jetsam_reason->osr_code == JETSAM_REASON_VNODE) {
    /*
     * vnode jetsams are syncronous and not caused by memory pressure.
     * Running the compactor on this thread adds significant latency to the
     * filesystem operation that triggered this jetsam. Kick of compactor thread
     * asyncronously instead.
     */
    vm_wake_compactor_swapper();
  } else {
    /* compact now, except for idle reaper kills.
     * idle reaper kills are done in batches, so we defer compaction until the
     * end of the batch.
     */
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_COMPACTOR_RUN) | DBG_FUNC_END, victim_pid,
       cause, vm_page_free_count);

  os_reason_free(jetsam_reason);
  return error == 0;
}

static int memstat_update_inactive_priority(proc_t p, boolean_t enable,
                                            int jetsam_prio,
    ...
    if (enable) {
      ...
      if (effective_now) {
        if (p->p_memstat_effectivepriority < jetsam_prio) {
          memstat_update_priority_locked(p, jetsam_prio,
...
/*
 * Pin a process to a particular jetsam band when it is in the background i.e.
 * not doing active work. For an application: that means no longer in the FG
 * band For a daemon: that means no longer in its 'requested' jetsam priority
 * band
 */
...
int memorystatus_update_inactive_jetsam_priority_band(pid_t pid,
  ...
  if (p != NULL) {
    error =
        memstat_update_inactive_priority(p, enable, jetsam_prio, effective_now);
    proc_rele(p);
  } else {
    error = ESRCH;
  }
...
static bool _memstat_proc_has_importance_assertion(proc_t p) {
  return (p->p_memstat_state & P_MEMSTAT_TEST_IMP_ASSERTION) ||
         task_has_assertions(proc_task(p));
}
...
_memstat_perform_idle_demotion_for_band(unsigned int demote_prio_band) {
  ...
  while (p) {
    ...
    if (current_time >= p->p_memstat_idledeadline) {
      ...
      next_proc =
          memorystatus_get_next_proc_locked(&demote_prio_band, p, FALSE);

      if ((isSysProc(p) &&
           _memstat_proc_is_dirty(p)) || /* system proc marked dirty*/
          _memstat_proc_has_importance_assertion(
              p)) { /* has outstanding assertions which might indicate
                       outstanding work too */
        ...
        if (isSysProc(p) && _memstat_proc_has_importance_assertion(p)) {
          if (demote_prio_band != system_procs_aging_band_stuck) {
            memorystatus_log_debug(
                "memorystatus_perform_idle_demotion() found stuck process %d "
                "[%s], moving to JETSAM_PRIORITY_AGING_BAND1_STUCK\n",
                proc_getpid(p), (*p->p_name ? p->p_name : "(unknown)"));
            memstat_update_priority_locked(p, JETSAM_PRIORITY_AGING_BAND1_STUCK,
                                           MEMSTAT_PRIORITY_NO_AGING);
            idle_delay_time = _memstat_sysprocs_aging_stuck_delay_time(p);
            KDBG(MEMSTAT_CODE(BSD_MEMSTAT_IDLE_DEMOTE) | DBG_FUNC_NONE,
                 proc_pid(p), JETSAM_PRIORITY_AGING_BAND1_STUCK,
                 p->p_memstat_idledeadline + idle_delay_time);
          } else {
            memorystatus_log("memorystatus_perform_idle_demotion() timed out "
                             "stuck process %d [%s], moving to idle band\n",
        ...
        memstat_update_priority_locked(p, JETSAM_PRIORITY_IDLE,
                                       MEMSTAT_PRIORITY_NO_AGING);
        KDBG(MEMSTAT_CODE(BSD_MEMSTAT_IDLE_DEMOTE) | DBG_FUNC_NONE, proc_pid(p),
             JETSAM_PRIORITY_IDLE, p->p_memstat_idledeadline);
      }
      p = next_proc;
    } else {
...
static void memorystatus_perform_idle_demotion(__unused void *spare1,
  ...
  if (!system_procs_aging_band && !system_procs_aging_band_stuck &&
...
static void _memstat_invalidate_idle_demotion_locked(proc_t p) {
  ...
  memorystatus_log_debug("%s: invalidating demotion to idle band for %s [%d]\n",
 ...
 * no aging procs.
 */
static uint64_t _memstat_find_earliest_idle_deadline(void) {
  memstat_bucket_t *demotion_bucket;
  proc_t oldest_proc = PROC_NULL;
  uint32_t aging_app_count = 0, aging_sysproc_count = 0,
           aging_sysproc_count_stuck = 0;
  LCK_MTX_ASSERT(&proc_list_mlock, LCK_MTX_ASSERT_OWNED);
  assert(system_procs_aging_band || system_procs_aging_band_stuck ||
         applications_aging_band);
  ...
  if (system_procs_aging_band_stuck) {
    aging_sysproc_count_stuck =
        memstat_bucket[system_procs_aging_band_stuck].count;
  }
  if (applications_aging_band) {
    aging_app_count = memstat_bucket[applications_aging_band].count;
  }

  if ((aging_app_count + aging_sysproc_count + aging_sysproc_count_stuck) ==
      0) {
    return 0;
  }
  ...
  if (applications_aging_band && aging_app_count > 0) {
    ...
    if (!oldest_proc || (oldest_app->p_memstat_idledeadline <
                         oldest_proc->p_memstat_idledeadline)) {
...
int memorystatus_add(proc_t p, boolean_t locked) {
  ...
  DTRACE_MEMORYSTATUS2(memorystatus_add, proc_t, p, int32_t,
                       p->p_memstat_effectivepriority);
   ...
   * For coalition-based freezing, we only want to freeze sysprocs that have
 ...
 */
static void _memstat_record_prio_transition(proc_t p, int new_priority) {
  uint64_t now;
...
void memstat_update_priority_locked(proc_t p, int priority,
  ...
  /* Ensure that exit isn't underway, leaving the proc retained but removed from
   * its bucket */
  ...
  DTRACE_MEMORYSTATUS3(memorystatus_update_priority, proc_t, p, int32_t,
                       p->p_memstat_effectivepriority, int, priority);

  old_bucket = &memstat_bucket[p->p_memstat_effectivepriority];

  if (priority == JETSAM_PRIORITY_IDLE &&
      !(_memstat_proc_can_idle_exit(p) && !_memstat_proc_is_dirty(p)) &&
      !(_memstat_proc_is_managed(p) &&
        !_memstat_proc_has_priority_assertion(p))) {
    ...
    memorystatus_log_error("memorystatus: %s [%d] is neither "
                           ...
                           proc_best_name(p), proc_getpid(p),
                           p->p_memstat_dirty, p->p_memstat_state, priority);
  }
  ...
  if (!(options & MEMSTAT_PRIORITY_NO_AGING)) {
    if (_memstat_proc_is_elevated(p)) {
       ...
       * OR
       * - processes that specifically opt-in to the elevated inactive support
       * e.g. docked processes.
       */
    ...
    if (_memstat_proc_is_tracked(p)) {
      ...
      } else if (system_procs_aging_band_stuck &&
                 priority <= system_procs_aging_band_stuck) {
        if (p->p_memstat_effectivepriority == JETSAM_PRIORITY_IDLE) {
          /* process has already aged */
          priority = JETSAM_PRIORITY_IDLE;
        } else {
          /* don't let anyone move anything between sysproc and sysproc stuck
           * inclusive */
          priority = system_procs_aging_band;
        }
      }
  ...
  if (p->p_memstat_effectivepriority != priority) {
    ...
    if ((system_procs_aging_band && priority == system_procs_aging_band) ||
        (system_procs_aging_band_stuck &&
         priority == system_procs_aging_band_stuck) ||
        (applications_aging_band && priority == applications_aging_band)) {
...
#if CONFIG_SECLUDED_MEMORY
  if (secluded_for_apps && task_could_use_secluded_mem(proc_task(p))) {
    task_set_can_use_secluded_mem(proc_task(p),
...
int memorystatus_relaunch_flags_update(proc_t p, int relaunch_flags) {
  ...
  KDBG(BSDDBG_CODE(DBG_BSD_MEMSTAT, BSD_MEMSTAT_RELAUNCH_FLAGS), proc_getpid(p),
       relaunch_flags);
  return 0;
}
...
SYSCTL_PROC(_kern, OID_AUTO, memorystatus_relaunch_flags,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED | CTLFLAG_MASKED, 0, 0,
            sysctl_memorystatus_relaunch_flags, "I",
            "get relaunch flags for current process");
#endif /* DEVELOPMENT || DEBUG */
 ...
 */
static bool current_task_can_use_entitled_range() {
  static const char kInternalJetsamRangeEntitlement[] =
      "com.apple.private.internal-jetsam-range";
 ...
 */
int memorystatus_set_priority(proc_t p, int priority, uint64_t user_data,
                              memstat_priority_options_t options) {
  int ret;
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_UPDATE) | DBG_FUNC_START, proc_getpid(p),
       priority, user_data, options);

  if (priority == -1) {
    /* Use as shorthand for default priority */
    priority = JETSAM_PRIORITY_DEFAULT;
  } else if (priority > JETSAM_PRIORITY_IDLE &&
             priority <= JETSAM_PRIORITY_AGING_BAND2) {
    /*
     * Everything between idle and the aging bands are reserved for internal
     * use. if requested, adjust to JETSAM_PRIORITY_IDLE. Entitled processes
     * (just munch) can use a subset of this range for testing.
     */
  ...
  if ((options & MEMSTAT_PRIORITY_IS_EFFECTIVE) &&
    ...
    ret = EALREADY;
    proc_list_unlock();
    memorystatus_log_error("memorystatus_update: effective change specified "
                           "for pid %d, but change already occurred.\n",
                           proc_getpid(p));
    goto out;
  }

  if ((p->p_memstat_state & (P_MEMSTAT_TERMINATED | P_MEMSTAT_SKIP)) ||
      proc_list_exited(p)) {
    /*
     * This could happen when a process calling posix_spawn() is exiting on the
     * jetsam thread.
     */
    ret = EBUSY;
    proc_list_unlock();
  ...
  if ((options & MEMSTAT_PRIORITY_IS_ASSERTION)) {
    ...
    if (_memstat_proc_is_tracked(p) &&
      ...
      priority =
          MAX(p->p_memstat_assertionpriority, p->p_memstat_requestedpriority);
    }
  } else {
...
#if DEVELOPMENT || DEBUG
static int32_t memstat_increased_limit(int32_t limit, int32_t increase) {
...
static int memstat_set_memlimits_locked(proc_t p, int32_t active_limit,
  ...
  int32_t default_inactive_limit =
      memorystatus_get_default_task_inactive_limit(p);
  ...
  /*
   * Work around a bug in JetsamProperties whereby processes may mistakenly
   * receive ActiveSoftMemoryLimit := -1 by forcing the default task limit to be
   * fatal.
   */
  ...
  memorystatus_log_debug("memorystatus: setting memlimit for %s [%d], "
                         "Active(%dMB %s), Inactive(%dMB, %s)\n",
                         proc_best_name(p), proc_getpid(p), active_limit,
                         ((options & MEMLIMIT_ACTIVE_FATAL) ? "F" : "NF"),
                         inactive_limit,
                         ((options & MEMLIMIT_INACTIVE_FATAL) ? "F" : "NF"));
...
int memorystatus_set_memlimits(proc_t p, int32_t active_limit,
                               int32_t inactive_limit,
                               memlimit_options_t options) {
  int err;
  proc_list_lock();
  err = memstat_set_memlimits_locked(p, active_limit, inactive_limit, options);
  proc_list_unlock();
  return err;
}

int memorystatus_remove(proc_t p) {
  int ret;
  memstat_bucket_t *bucket;
...
static int memorystatus_validate_track_flags(struct proc *target_p,
                                             uint32_t pcontrol) {
  ...
  /* 'Launch in progress' tracking requires that process have enabled dirty
   * tracking too. */
  ...
  if ((pcontrol & PROC_DIRTY_DEFER) && (pcontrol & PROC_DIRTY_DEFER_ALWAYS)) {
    return EINVAL;
  }
...
/*
 * Processes can opt to have their state tracked by the kernel, indicating  when
 * they are busy (dirty) or idle (clean). They may also indicate that they
 * support termination when idle, with the result that they are promoted to
 * their desired, higher, jetsam priority when dirty (and are therefore killed
 * later), and demoted to the low priority idle band when clean (and killed
 * earlier, protecting higher priority procesess).
 *
 * If the deferral flag is set, then newly tracked processes will be protected
 * for an initial period (as determined by
 * memorystatus_sysprocs_idle_delay_time); if they go clean during this time,
 * then they will be moved to a deferred-idle band with a slightly higher
 * priority, guarding against immediate termination under memory pressure and
 * being unable to make forward progress. Finally, when the guard expires, they
 * will be moved to the standard, lowest-priority, idle band. The deferral can
 * be cleared early by clearing the appropriate flag.
 *
 * The deferral timer is active only for the duration that the process is marked
 * as guarded and clean; if the process is marked dirty, the timer will be
 * cancelled. Upon being subsequently marked clean, the deferment will either be
 * re-enabled or the guard state cleared, depending on whether the guard
 * deadline has passed.
 */

int memorystatus_dirty_track(proc_t p, uint32_t pcontrol) {
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_DIRTY_TRACK), proc_getpid(p),
       p->p_memstat_dirty, pcontrol);
  ...
  if (pcontrol & PROC_DIRTY_TRACK) {
    ...
    if (p->p_memstat_state & P_MEMSTAT_MANAGED) {
      ...
      if (!_memstat_proc_cached_memlimit_is_fatal(p)) {
         ...
         * kernel checks, they change the role of a process from app to daemon.
         * And the AGING_IN_PROGRESS bits might still be set i.e. it needs to be
         * demoted correctly from the right aging band (app or sysproc). We
         * can't simply try to invalidate the demotion here because, owing to
         * assertion priorities, we might not be in the aging bands.
         */
        memorystatus_log(
  ...
  memorystatus_log_info(
      "%s [%d] enrolled in ActivityTracking tracked %d / idle-exit %d / defer "
      "%d / dirty %d",
      proc_best_name(p), proc_getpid(p), _memstat_proc_is_tracked(p),
      _memstat_proc_can_idle_exit(p), defer_now, _memstat_proc_is_dirty(p));
...
int memorystatus_dirty_set(proc_t p, boolean_t self, uint32_t pcontrol) {
  int ret = 0;
  bool kill = false;
  bool was_dirty;
  ...
  if (!_memstat_proc_is_tracked(p)) {
     ...
     * Set for termination and marking as clean is OK - see
     * <rdar://problem/10594349>.
     */
    ret = EBUSY;
    goto exit;
  }
  ...
  if (pcontrol && !(p->p_memstat_dirty & flag)) {
    ...
    if ((flag == P_DIRTY_SHUTDOWN) && (!(p->p_memstat_dirty & P_DIRTY))) {
      /* Clearing the dirty shutdown flag, and the process is otherwise clean -
       * kill */
  ...
  if (was_dirty && !now_dirty) {
    if (_memstat_proc_can_idle_exit(p)) {
      /*
       * Legacy mode: P_DIRTY_AGING_IN_PROGRESS means the process is in the
       * aging band OR it might be heading back there once it's clean again. For
       * the legacy case, this only applies if it has some protection window
       * left. P_DIRTY_DEFER: one-time protection window given at launch
       * P_DIRTY_DEFER_ALWAYS: protection window given for every dirty->clean
       * transition. Like non-legacy mode.
       *
       * Non-Legacy mode: P_DIRTY_AGING_IN_PROGRESS means the process is in the
       * aging band. It will always stop over in that band on it's way to IDLE.
       */
      assert(!_memstat_proc_is_aging(p));
      priority = JETSAM_PRIORITY_IDLE;
    } else {
...
int memorystatus_dirty_clear(proc_t p, uint32_t pcontrol) {
  int ret = 0;
  ...
  if (!pcontrol ||
      (pcontrol & (PROC_DIRTY_LAUNCH_IN_PROGRESS | PROC_DIRTY_DEFER |
                   PROC_DIRTY_DEFER_ALWAYS)) == 0) {
    ret = EINVAL;
    goto exit;
  }
...
int memorystatus_dirty_get(proc_t p, boolean_t locked) {
  int ret = 0;
...
int memorystatus_on_terminate(proc_t p) {
  int sig;
  ...
  if ((_memstat_proc_is_tracked(p) && !_memstat_proc_is_dirty(p)) ||
    ...
    /* Dirty, terminated, or state tracking is unsupported; issue SIGTERM to
     * allow cleanup */
    sig = SIGTERM;
  }
...
void memorystatus_on_suspend(proc_t p) {
#if CONFIG_FREEZE
  uint32_t pages;
  memorystatus_get_task_page_counts(proc_task(p), &pages, NULL, NULL);
#endif
...
void memorystatus_on_resume(proc_t p) {
#if CONFIG_FREEZE
  pid_t pid;
#endif
  ...
  if (frozen) {
    ...
    if (p->p_memstat_thaw_count == 0 ||
        p->p_memstat_last_thaw_interval <
            memorystatus_freeze_current_interval) {
      os_atomic_inc(&(memorystatus_freezer_stats.mfs_processes_thawed),
                    relaxed);
      if (strcmp(p->p_name, "com.apple.WebKit.WebContent") == 0) {
        os_atomic_inc(
            &(memorystatus_freezer_stats.mfs_processes_thawed_webcontent),
            relaxed);
      }
    }
...
void memorystatus_on_inactivity(proc_t p) {
#pragma unused(p)
#if CONFIG_FREEZE
...
bool memstat_kill_idle_process(memorystatus_kill_cause_t cause,
  ...
  while (p) {
    ...
    if ((p->p_memstat_dirty &
         (P_DIRTY_ALLOW_IDLE_EXIT | P_DIRTY_IS_DIRTY | P_DIRTY_TERMINATED)) ==
  ...
  if (p) {
    memorystatus_log("memorystatus: killing (idle) %s [%d] due to %s (%u)\n",
                     proc_best_name(p), proc_getpid(p),
                     memstat_kill_cause_name[cause], cause);
...
void memorystatus_respond_to_compressor_exhaustion(void) {
#if CONFIG_JETSAM
  memorystatus_thread_wake();
#else  /* !CONFIG_JETSAM */
  if (kill_on_no_paging_space) {
    ...
    absolutetime_to_nanoseconds(now - last_action_ts,
                                &delta_since_last_no_space_ns);
    if (delta_since_last_no_space_ns >
        no_paging_space_action_throttle_delay_ns) {
      memorystatus_thread_wake();
    }
  }
#endif /* CONFIG_JETSAM */
...
void memorystatus_respond_to_swap_exhaustion(void) {
#if CONFIG_JETSAM
  /*
...
#if CONFIG_JETSAM
static void memorystatus_thread_pool_max() {
...
static void memorystatus_thread_pool_default() {
 ...
 */
static uint32_t _memstat_page_shortage_offset(void) {
  uint32_t offset = 0;
...
uint32_t memorystatus_get_critical_page_shortage_threshold(void) {
  return memstat_critical_threshold;
}

uint32_t memorystatus_get_idle_exit_page_shortage_threshold(void) {
  uint32_t offset = _memstat_page_shortage_offset();
  return memstat_idle_threshold + offset;
}

uint32_t memorystatus_get_soft_memlimit_page_shortage_threshold(void) {
  uint32_t offset = _memstat_page_shortage_offset();
  return memstat_soft_threshold + offset;
}

uint32_t memorystatus_get_reaper_page_shortage_threshold(void) {
  uint32_t offset = _memstat_page_shortage_offset();
  return memstat_reaper_threshold + offset;
}

#if CONFIG_JETSAM
void _memstat_reaper_check_oldest_reapable_proc_info_timeout(void) {
  if (memstat_oldest_reapable_proc_prio_start !=
      MEMSTAT_OLDEST_REAPABLE_PROC_PRIO_START_UNKNOWN) {
    uint64_t curr_ts_matu = mach_absolute_time();
    if (curr_ts_matu > memstat_oldest_reapable_proc_info_expiration_ts_matu) {
      memstat_oldest_reapable_proc_prio_start =
          MEMSTAT_OLDEST_REAPABLE_PROC_PRIO_START_UNKNOWN;
...
void _memstat_reaper_start_sweep(void) {
  if (!memstat_reaper_is_currently_sweeping) {
    memorystatus_log(
        "memorystatus: _memstat_reaper_start_sweep: reaper sweep starting\n");
...
void _memstat_reaper_end_sweep(void) {
  ...
  if (memstat_reaper_is_currently_sweeping) {
    ...
    absolutetime_to_nanoseconds(
        mach_absolute_time() - memstat_reaper_start_ts_matu, &delta_time_nsec);
    memstat_reaper_cumulative_stats.sweep_count++;
    memstat_reaper_cumulative_memory_freed_mb =
        (uint32_t)(memstat_reaper_cumulative_stats.memory_freed_bytes >> 20);
    memorystatus_log(
        "memorystatus: _memstat_reaper_end_sweep: reaper sweep ended, %d "
        "processes killed, %lluMB freed, %llums elapsed, %lluus/process\n",
        ...
        memstat_reaper_current_sweep_stats.kill_count
            ? ((delta_time_nsec / NSEC_PER_USEC) /
               memstat_reaper_current_sweep_stats.kill_count)
            : 0);
    memorystatus_log("memorystatus: _memstat_reaper_end_sweep: reaper totals: "
                     "%d sweeps, %d processes killed, %dMB freed\n",
                     memstat_reaper_cumulative_stats.sweep_count,
                     memstat_reaper_cumulative_stats.kill_count,
                     memstat_reaper_cumulative_memory_freed_mb);
    memstat_reaper_is_currently_sweeping = false;
    nanoseconds_to_absolutetime((memstat_reaper_rescan_secs * NSEC_PER_SEC),
                                &rescan_delta_matu);
    memstat_reaper_can_run_after_ts_matu =
        mach_absolute_time() + rescan_delta_matu;
  }
}

void _memstat_reaper_record_kill(uint64_t bytes_freed) {
  memstat_reaper_current_sweep_stats.kill_count++;
...
const char *_memstat_relaunch_flags_description(uint32_t flags) {
  switch (flags) {
  case P_MEMSTAT_RELAUNCH_UNKNOWN:
    return "-";
  case P_MEMSTAT_RELAUNCH_LOW:
...
const char *_memstat_proc_type_description(proc_t p) {
...
bool memstat_evaluate_page_shortage(bool *should_enforce_memlimits,
  ...
  if (memstat_reaper_enabled) {
    /*
     * Only wake the jetsam thread to do reaper kills if the reaper is currently
     * alreay running a sweep OR if other conditions suggest that we should
     * start a sweep
     */
    ...
    if (memstat_reaper_is_currently_sweeping) {
      ...
      uint64_t curr_ts_matu = mach_absolute_time();
      // if we are not already in the middle of a reaper sweep, do very quick
      // tests to see if we should possibly start one:
      // - the minimum rescan time has passed since the end of the last sweep
      // - we are below the page threshold
      // - the oldest reapable process is old enough to be a reaper candidate
      // now

      if ((curr_ts_matu > memstat_reaper_can_run_after_ts_matu) &&
          (available_page_count <
           memorystatus_get_reaper_page_shortage_threshold())) {
        _memstat_reaper_check_oldest_reapable_proc_info_timeout();

        if (memstat_oldest_reapable_proc_prio_start ==
            MEMSTAT_OLDEST_REAPABLE_PROC_PRIO_START_NONE) {
          memorystatus_log_debug(
              "memorystatus: memstat_evaluate_page_shortage: no known-reapable "
              "processes\n");
        } else {
          if (curr_ts_matu >=
              memstat_oldest_reapable_proc_will_be_reapable_at_ts_matu) {
            requires_action = true;
            if (should_reap) {
              *should_reap = true;
              memorystatus_log_debug(
                  "memorystatus: memstat_evaluate_page_shortage: should start "
                  "reaping long-idle processes\n");
            }
  ...
  if (available_page_count <
      memorystatus_get_idle_exit_page_shortage_threshold()) {
    /*
     * Only wake the jetsam thread if there are idle processes that
     * could exit.
     */
    uint32_t idle_proc_count =
        os_atomic_load(&memstat_bucket[JETSAM_PRIORITY_IDLE].count, relaxed);
...
#if CONFIG_JETSAM
static uint64_t memorystatus_swap_trigger_pages(void) {
  /*
   * The swapout trigger varies based on the current memorystatus_level.
   * When available memory is somewhat high (at
   * memorystatus_available_pages_pressure) we keep more swappable compressor
   * segments in memory. However, as available memory drops to our idle and
   * eventually critical kill thresholds we start swapping more aggressively.
   */
  static uint32_t available_pages_factor[] = {0, 1, 1,  1,  2,  2,  3, 5,
                                              7, 8, 10, 13, 15, 17, 20};
  size_t index = MIN(memorystatus_level,
                     sizeof(available_pages_factor) / sizeof(uint32_t) - 1);
  return available_pages_factor[index] * memorystatus_available_pages / 10;
}

static int sysctl_memorystatus_swap_trigger_pages SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2)
  uint64_t trigger_pages = memorystatus_swap_trigger_pages();
  return SYSCTL_OUT(req, &trigger_pages, sizeof(trigger_pages));
}

SYSCTL_PROC(_kern, OID_AUTO, memorystatus_swap_trigger_pages,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            &sysctl_memorystatus_swap_trigger_pages, "I", "");
...
bool memorystatus_swapin_over_trigger(void) {
  return atop_64(c_late_swappedin_count * c_seg_allocsize) >
         memorystatus_swapin_trigger_pages;
}
#endif /* CONFIG_JETSAM */

#if DEVELOPMENT || DEBUG
SYSCTL_UINT(_vm, OID_AUTO, c_late_swapout_count,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, &c_late_swapout_count, 0,
            "");
SYSCTL_UINT(_vm, OID_AUTO, c_seg_allocsize,
            CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED, &c_seg_allocsize, 0, "");
#if CONFIG_FREEZE
extern int32_t c_segment_pages_compressed_incore_late_swapout;
SYSCTL_INT(_vm, OID_AUTO, c_segment_pages_compressed_incore_late_swapout,
           CTLTYPE_INT | CTLFLAG_RD | CTLFLAG_LOCKED,
           &c_segment_pages_compressed_incore_late_swapout, 0, "");
#endif /* CONFIG_FREEZE */
#endif /* DEVELOPMENT || DEBUG */

static boolean_t memorystatus_should_post_snapshot(int32_t priority,
                                                   uint32_t cause) {
  boolean_t is_idle_priority;

  is_idle_priority = (priority == JETSAM_PRIORITY_IDLE ||
                      priority == JETSAM_PRIORITY_IDLE_DEFERRED);
#if CONFIG_JETSAM
#pragma unused(cause)
  /*
  ...
  boolean_t snapshot_eligible_kill_cause =
      (is_reason_thrashing(cause) || is_reason_zone_map_exhaustion(cause));
  return !is_idle_priority || memorystatus_idle_snapshot ||
...
static boolean_t
memorystatus_act_on_hiwat_processes(uint32_t *errors, uint32_t *hwm_kill,
                                    bool *post_snapshot,
                                    uint64_t *memory_reclaimed) {
 ...
 */
static void memstat_purge_caches(jetsam_state_t state) {
 ...
 * avoid a jetsam.
 */
static void memstat_approaching_fg_band(jetsam_state_t state) {
  memorystatus_log(
...
int jld_max_priority_arr[] = {
    JETSAM_PRIORITY_MAIL,         JETSAM_PRIORITY_MAIL,
    JETSAM_PRIORITY_UI_SUPPORT,   JETSAM_PRIORITY_UI_SUPPORT,
    JETSAM_PRIORITY_DRIVER_APPLE,
};
#define JLD_MAX_PRIORITY_ARR_COUNT                                             \
  (sizeof(jld_max_priority_arr) / sizeof(jld_max_priority_arr[0]))

static bool memorystatus_act_aggressive(jetsam_state_t state, uint32_t cause,
                                        os_reason_t jetsam_reason) {
  boolean_t killed;
  uint32_t errors = 0;
  ...
  int max_kill_pri =
      jld_max_priority_arr[MIN(iteration_no, JLD_MAX_PRIORITY_ARR_COUNT - 1)];
  ...
  elevated_bucket_count =
      memstat_bucket[JETSAM_PRIORITY_ELEVATED_INACTIVE].count;
  proc_list_unlock();
  ...
  while (elevated_bucket_count) {
    ...
    killed = memorystatus_kill_elevated_process(
        cause, jetsam_reason, JETSAM_PRIORITY_ELEVATED_INACTIVE,
        jld_eval_aggressive_count, &errors, &footprint_of_killed_proc);
    if (killed) {
  ...
  killed = memorystatus_kill_processes_aggressive(
      kMemorystatusKilledProcThrashing, jld_eval_aggressive_count, max_kill_pri,
      maximum_kills, &errors, &footprint_of_killed_proc);
 ...
 */
static void memorystatus_thread_init(jetsam_state_t jetsam_thread) {
  char name[32];
  thread_wire_internal(host_priv_self(), current_thread(), TRUE, NULL);
  snprintf(name, 32, "VM_memorystatus_%d", jetsam_thread->index + 1);

  /* Limit all but one thread to the lower jetsam bands, as that's where most of
   * the victims are. */
 ...
 */
static os_reason_t create_jetsam_reason(memorystatus_kill_cause_t cause) {
  os_reason_t jetsam_reason = OS_REASON_NULL;
 ...
 * sets post_snapshot, tracks jld_idle_kills, and notifies if we're appraoching
 * the fg band.
 */
static bool memstat_do_priority_kill(jetsam_state_t state, uint32_t kill_cause,
                                     int32_t max_priority,
                                     memstat_kill_options_t options) {
  os_reason_t jetsam_reason = OS_REASON_NULL;
  bool killed = false;
  ...
  killed = memstat_kill_top_process(kill_cause, jetsam_reason, max_priority,
                                    options, &priority, &state->errors,
                                    &state->memory_reclaimed);
...
static bool memorystatus_do_action(jetsam_state_t state,
                                   memorystatus_action_t action,
                                   memorystatus_kill_cause_t kill_cause) {
  bool killed = false;
  os_reason_t jetsam_reason = OS_REASON_NULL;

  switch (action) {
  case MEMORYSTATUS_KILL_HIWATER:
    killed = memorystatus_act_on_hiwat_processes(
        &state->errors, &state->hwm_kills, &state->post_snapshot,
        &state->memory_reclaimed);
    break;
  ...
  case MEMORYSTATUS_KILL_TOP_PROCESS:
    killed =
        memstat_do_priority_kill(state, kill_cause, JETSAM_PRIORITY_MAX, 0);
    break;
  case MEMORYSTATUS_WAKE_SWAPPER:
    memorystatus_log_info("memorystatus_do_action: Waking up swap thread. "
                          "memorystatus_available_pages: %llu\n",
                          (uint64_t)MEMORYSTATUS_LOG_AVAILABLE_PAGES);
  ...
  case MEMORYSTATUS_PROCESS_SWAPIN_QUEUE:
    memorystatus_log_info("memorystatus_do_action: Processing swapin queue of "
                          "length: %u memorystatus_available_pages: %llu\n",
                          c_late_swappedin_count,
                          (uint64_t)MEMORYSTATUS_LOG_AVAILABLE_PAGES);
    vm_compressor_process_special_swapped_in_segments();
    break;
  case MEMORYSTATUS_KILL_SUSPENDED_SWAPPABLE:
    killed = memstat_do_priority_kill(state, kill_cause,
                                      JETSAM_PRIORITY_BACKGROUND - 1,
                                      MEMSTAT_ONLY_SWAPPABBLE);
    break;
  case MEMORYSTATUS_KILL_SWAPPABLE:
    killed = memstat_do_priority_kill(state, kill_cause, JETSAM_PRIORITY_MAX,
                                      MEMSTAT_ONLY_SWAPPABBLE);
    break;
  case MEMORYSTATUS_KILL_IDLE:
    killed =
        memstat_do_priority_kill(state, kill_cause, JETSAM_PRIORITY_IDLE, 0);
    break;
  case MEMORYSTATUS_KILL_LONG_IDLE:
    killed = memstat_do_priority_kill(state, kill_cause, JETSAM_PRIORITY_IDLE,
                                      MEMSTAT_ONLY_LONG_IDLE);
    break;
  ...
  case MEMORYSTATUS_KILL_NONE:
    panic("memorystatus_do_action: Impossible! memorystatus_do_action called "
          "with action = NONE\n");
  }
  return killed;
}

void memorystatus_post_snapshot() {
  proc_list_lock();
  ...
  if (memorystatus_jetsam_snapshot_count > 0 &&
      (memorystatus_jetsam_snapshot_last_timestamp == 0 ||
       timestamp_now > memorystatus_jetsam_snapshot_last_timestamp +
                           memorystatus_jetsam_snapshot_timeout)) {
    proc_list_unlock();
    int ret = memorystatus_send_note(kMemorystatusSnapshotNote, &snapshot_size,
...
/*
 *  Called by memorystatus_update_jetsam_snapshot_entry_locked to take a zprint
 * snapshot.
 */
static void memorystatus_collect_jetsam_snapshot_zprint(void) {
  ...
  if (new_meminfo_cnt > jzs_meminfo_cnt) {
    jzs_meminfo = krealloc_data_tag(
        jzs_meminfo, jzs_meminfo_cnt * sizeof(mach_memory_info_t),
        new_meminfo_cnt * sizeof(mach_memory_info_t), Z_WAITOK,
        VM_KERN_MEMORY_DIAG);

    jzs_meminfo_cnt = new_meminfo_cnt;
  }

  mach_memory_info_sample(jzs_names, jzs_info, jzs_coalesce, &jzs_zone_cnt,
                          jzs_meminfo, jzs_meminfo_cnt, true);
}
 ...
 */
static void memorystatus_thread_internal(jetsam_state_t state) {
  ...
  if (state->inited == FALSE) {
    /*
     * It's the first time the thread has run, so just mark the thread as
     * privileged and block.
     */
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_SCAN) | DBG_FUNC_START,
       MEMORYSTATUS_LOG_AVAILABLE_PAGES, memorystatus_jld_enabled,
       memorystatus_jld_eval_period_msecs, jld_eval_aggressive_count);
  ...
  memorystatus_log_info("memorystatus: c_segment_count=%u major compaction "
                        "occurred %u seconds ago\n",
                        c_segment_count, major_compact_diff.tv_sec);

  /*
   * Jetsam aware version.
   *
   * The VM pressure notification thread is working its way through clients in
   * parallel.
   *
   * So, while the pressure notification thread is targeting processes in order
   * of increasing jetsam priority, we can hopefully reduce / stop its work by
   * killing any processes that have exceeded their highwater mark.
   *
   * If we run out of HWM processes and our available pages drops below the
   * critical threshold, then, we target the least recently used process in
   * order of increasing jetsam priority (exception: the FG band).
   */
  while (true) {
    bool killed;
    state->memory_reclaimed = 0;
    ...
    memorystatus_action_t action = memorystatus_pick_action(
        state, &cause, highwater_remaining, suspended_swappable_apps_remaining,
        swappable_apps_remaining, &state->jld_idle_kills);
    if (action == MEMORYSTATUS_KILL_NONE) {
      break;
    }

    if (cause == kMemorystatusKilledVMCompressorThrashing ||
    ...
    if (!killed) {
      if (action == MEMORYSTATUS_KILL_HIWATER) {
         ...
         * possible we chose not to because we reclaimed some purgeable memory
         * or hit this thread's priority limit.
         */
        if (state->memory_reclaimed == 0 && !state->limit_to_low_bands) {
          /*
           * We should have found someone to kill. Either we failed because of a
           * transient error or we've run out of candidates and the issue is
           * caused by the kernel.
           */
          memorystatus_log("memorystatus: failed to find a process to kill!\n");
          if (state->errors && !state->errors_cleared) {
            /*
             * It's possible that all of the kill candidates had the error bit
             * set (e.g. because we caught them in exec()). Clear all the error
             * bits and try to kill them one more time in the hopes that they
             * are now killable.
             */
            memorystatus_log(
            ...
            bool is_system_healthy =
                memstat_check_system_health(&health_status);
            if (!is_system_healthy) {
              memorystatus_log(
                  "memorystatus: system still unhealthy after cache purge!\n");
              /*
               * We trimmed the zones above but it's possible there is a bug
               * with working set estimation and we needed a full drain.
               */
              memorystatus_log_fault(
                  "memorystatus: fully draining kernel zone allocator\n");
              zone_gc_drain();
              is_system_healthy = memstat_check_system_health(&health_status);
              if (!is_system_healthy) {
                /*
                 * We've killed everything and purged all xnu caches. There is
...
static void memorystatus_thread(void *param __unused,
  ...
  while (1) {
    ...
    sched_cond_wait(&(jetsam_thread->jt_wakeup_cond), THREAD_UNINT,
                    memorystatus_thread);
  }
}
 ...
 */
void memorystatus_on_ledger_footprint_exceeded(boolean_t warning,
  ...
  if (warning == TRUE) {
    /*
     * This is a warning path which implies that the current process is close,
     * but has not yet exceeded its per-process memory limit.
     */
    if (memorystatus_warn_process(p, memlimit_is_active, memlimit_is_fatal,
                                  FALSE /* not exceeded */) != TRUE) {
      /* Print warning, since it's possible that task has not registered for
       * pressure notifications */
      memorystatus_log_debug(
          "memorystatus_on_ledger_footprint_exceeded: failed to warn %s [%d] "
          "(exiting, or no handler registered?).\n",
          proc_best_name(p), proc_getpid(p));
    }
    return;
  }
  ...
  if (memlimit_is_fatal) {
    /*
     * If this process has no high watermark or has a fatal task limit, then we
     * have been invoked because the task has violated either the system-wide
     * per-task memory limit OR its own task limit.
     */
    jetsam_reason = os_reason_create(OS_REASON_JETSAM,
                                     JETSAM_REASON_MEMORY_PERPROCESSLIMIT);
    if (jetsam_reason == NULL) {
      memorystatus_log_error(
          "task_exceeded footprint: failed to allocate jetsam reason\n");
    } else if (corpse_for_fatal_memkill &&
               proc_send_synchronous_EXC_RESOURCE(p) == FALSE) {
      /* Set OS_REASON_FLAG_GENERATE_CRASH_REPORT to generate corpse */
      jetsam_reason->osr_flags |= OS_REASON_FLAG_GENERATE_CRASH_REPORT;
    }

    if (memstat_kill_process_sync(proc_getpid(p),
                                  kMemorystatusKilledPerProcessLimit,
                                  jetsam_reason) != TRUE) {
      memorystatus_log_error("task_exceeded_footprint: failed to kill the "
     ...
     * This path implies the current process has exceeded a non-fatal (soft)
     * memory limit. Failure to send note is ignored here.
     */
    (void)memorystatus_warn_process(p, memlimit_is_active, memlimit_is_fatal,
                                    TRUE /* exceeded */);
...
void memorystatus_log_exception(const int max_footprint_mb,
                                boolean_t memlimit_is_active,
                                boolean_t memlimit_is_fatal) {
  proc_t p = current_proc();
   ...
   * Hard memory limit is a fatal custom-task-limit or system-wide per-task
   * memory limit.
   */

  memorystatus_log(
      "memorystatus: %s [%d] exceeded mem limit: %s%s %d MB (%s)\n",
      ((p && *p->p_name) ? p->p_name : "unknown"), (p ? proc_getpid(p) : -1),
      (memlimit_is_active ? "Active" : "Inactive"),
...
void memorystatus_on_conclave_limit_exceeded(const int max_footprint_mb) {
  ...
  memorystatus_log("memorystatus: %s [%d] exceeded conclave limit: %d MB \n",
                   ...
                   (p ? proc_getpid(p) : -1), max_footprint_mb);

  jetsam_reason =
      os_reason_create(OS_REASON_JETSAM, JETSAM_REASON_MEMORY_CONCLAVELIMIT);
  if (jetsam_reason == NULL) {
    memorystatus_log_error(
  ...
  if (memstat_kill_process_sync(proc_getpid(p),
                                kMemorystatusKilledConclaveLimit,
                                jetsam_reason) != TRUE) {
    memorystatus_log_error("task_exceeded_conclave: failed to kill the current "
                           "task (exiting?).\n");
  }
}
 ...
 *		False if inactive
 */
static bool memstat_proc_is_active_locked(proc_t p) {
  LCK_MTX_ASSERT(&proc_list_mlock, LCK_MTX_ASSERT_OWNED);
...
static bool memstat_kill_process_sync(pid_t victim_pid, uint32_t cause,
  ...
  if (victim_pid == -1) {
    ...
    killed = memstat_kill_top_process(cause, jetsam_reason, max_priority,
                                      MEMSTAT_SORT_BUCKET, NULL, &errors,
                                      &memory_reclaimed);
  } else {
    killed =
        memorystatus_kill_specific_process(victim_pid, cause, jetsam_reason);
  }
    ...
    if (memorystatus_jetsam_snapshot_count > 0 &&
        (memorystatus_jetsam_snapshot_last_timestamp == 0 ||
         timestamp_now > memorystatus_jetsam_snapshot_last_timestamp +
                             memorystatus_jetsam_snapshot_timeout)) {
      proc_list_unlock();
      int ret = memorystatus_send_note(kMemorystatusSnapshotNote,
                                       &snapshot_size, sizeof(snapshot_size));
      if (!ret) {
        proc_list_lock();
        memorystatus_jetsam_snapshot_last_timestamp = timestamp_now;
        proc_list_unlock();
...
static bool memorystatus_kill_specific_process(pid_t victim_pid, uint32_t cause,
  ...
  absolutetime_to_nanoseconds(killtime - p->p_memstat_prio_start,
                              &time_in_priority_band_secs);
  time_in_priority_band_secs /= NSEC_PER_SEC;

  killed =
      memorystatus_do_kill(p, cause, jetsam_reason, &footprint_of_killed_proc);

  memorystatus_log(
      "%lu.%03d memorystatus: killing_specific_process pid %d [%s] (%s %d "
      "%llus rf:%s type:%s) %lluKB - memorystatus_available_pages: %llu\n",
      (unsigned long)tv_sec, tv_msec, victim_pid,
      ((p && *p->p_name) ? p->p_name : "unknown"),
      memstat_kill_cause_name[cause], (p ? p->p_memstat_effectivepriority : -1),
      time_in_priority_band_secs,
      (p ? _memstat_relaunch_flags_description(p->p_memstat_relaunch_flags)
         : 0),
      _memstat_proc_type_description(p), footprint_of_killed_proc >> 10,
      (uint64_t)MEMORYSTATUS_LOG_AVAILABLE_PAGES);
...
void jetsam_on_ledger_cpulimit_exceeded(void) {
  ...
  if (jetsam_reason == OS_REASON_NULL) {
    memorystatus_log_error(
        "memorystatus: unable to allocate memory for jetsam reason\n");
  }
  ...
  if (retval) {
    memorystatus_log_error(
        "memorystatus: failed to kill current task (exiting?).\n");
  }
}
...
#pragma unused(oidp, arg1, arg2)
  ...
  memorystatus_log_debug(
      "memorystatus: pidwatch old_value = 0x%llx, new_value = 0x%llx\n",
      old_value, new_value);

  return error;
}

SYSCTL_PROC(_kern, OID_AUTO, memorystatus_vm_map_fork_pidwatch,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED | CTLFLAG_MASKED, 0, 0,
            sysctl_memorystatus_vm_map_fork_pidwatch, "Q",
            "get/set pid watched for in vm_map_fork");

/*
 * Record if a watched process fails to qualify for a vm_map_fork().
 */
void memorystatus_abort_vm_map_fork(task_t task) {
  if (memorystatus_vm_map_fork_pidwatch_val != 0) {
    proc_t p = get_bsdtask_info(task);
    if (p != NULL &&
        memorystatus_vm_map_fork_pidwatch_val == (uint64_t)proc_getpid(p)) {
      memorystatus_vm_map_fork_pidwatch_val = -1ull;
    }
  }
}

static void set_vm_map_fork_pidwatch(task_t task, uint64_t x) {
  if (memorystatus_vm_map_fork_pidwatch_val != 0) {
    proc_t p = get_bsdtask_info(task);
    if (p &&
        (memorystatus_vm_map_fork_pidwatch_val == (uint64_t)proc_getpid(p))) {
...
boolean_t memorystatus_allowed_vm_map_fork(task_t task, bool *is_large) {
  ...
  *is_large = false;
  ...
  if (os_atomic_load(&block_corpses, relaxed) != 0) {
    memorystatus_log("memorystatus_allowed_vm_map_fork: corpse for pid %d "
                     "blocked by jetsam).\n",
                     task_pid(task));
    ktriage_record(thread_tid(self),
                   KDBG_TRIAGE_EVENTID(KDBG_TRIAGE_SUBSYS_CORPSE,
...
#define LARGE_CORPSE_LIMIT 1
  if (sane_size > 1 * 1024 * 1024 * 1024) {
    ...
    if (footprint_in_bytes > 200 * 1024 * 1024 &&
        footprint_in_bytes <= 300 * 1024 * 1024 && cnt < LARGE_CORPSE_LIMIT &&
...
void memorystatus_get_task_page_counts(task_t task, uint32_t *footprint,
...
static void memorystatus_get_task_phys_footprint_page_counts(
  ...
  if (neural_nofootprint_total_pages) {
    *neural_nofootprint_total_pages =
        (get_task_neural_nofootprint_total(task) / PAGE_SIZE_64);
  }
 ...
 */
static bool memorystatus_jetsam_snapshot_copy_entry_locked(
    memorystatus_jetsam_snapshot_t *dst_snapshot,
    unsigned int dst_snapshot_size,
    const memorystatus_jetsam_snapshot_entry_t *src_entry) {
  LCK_MTX_ASSERT(&proc_list_mlock, LCK_MTX_ASSERT_OWNED);
  assert(dst_snapshot);
  ...
  memorystatus_jetsam_snapshot_entry_t *dst_entry =
      &dst_snapshot->entries[dst_snapshot->entry_count++];
  memcpy(dst_entry, src_entry, sizeof(memorystatus_jetsam_snapshot_entry_t));
  return true;
}
#endif /* CONFIG_FREEZE */

static bool memorystatus_init_jetsam_snapshot_entry_with_kill_locked(
    memorystatus_jetsam_snapshot_t *snapshot, proc_t p, uint32_t kill_cause,
    uint64_t killtime, memorystatus_jetsam_snapshot_entry_t **entry) {
  LCK_MTX_ASSERT(&proc_list_mlock, LCK_MTX_ASSERT_OWNED);
  memorystatus_jetsam_snapshot_entry_t *snapshot_list = snapshot->entries;
  size_t i = snapshot->entry_count;

  if (memorystatus_init_jetsam_snapshot_entry_locked(
          p, &snapshot_list[i], (snapshot->js_gencount)) == TRUE) {
...
memorystatus_update_jetsam_snapshot_entry_locked(proc_t p, uint32_t kill_cause,
  ...
  for (i = 0; i < memorystatus_jetsam_snapshot_count; i++) {
    if (snapshot_list[i].pid == proc_getpid(p)) {
      ...
      memorystatus_get_task_page_counts(proc_task(p), &pages,
                                        &max_pages_lifetime, &purgeable_pages);
      ...
      memorystatus_get_task_phys_footprint_page_counts(
          proc_task(p), &internal_pages, &internal_compressed_pages,
      ...
      entry->jse_purgeable_nonvolatile_compressed_pages =
          purgeable_nonvolatile_compressed_pages;
      entry->jse_alternate_accounting_pages = alternate_accounting_pages;
      entry->jse_alternate_accounting_compressed_pages =
          alternate_accounting_compressed_pages;
      ...
      entry->jse_neural_nofootprint_total_pages =
          neural_nofootprint_total_pages;
  ...
  if (entry == NULL) {
    ...
    if (memorystatus_jetsam_snapshot_count < memorystatus_jetsam_snapshot_max) {
      ...
      if (memorystatus_init_jetsam_snapshot_entry_with_kill_locked(
              snapshot, p, kill_cause, killtime, &entry)) {
        memorystatus_jetsam_snapshot_count++;

        if (memorystatus_jetsam_snapshot_count >=
            memorystatus_jetsam_snapshot_max) {
          /*
...
exit:
  if (entry) {
    ...
    if (memorystatus_jetsam_use_freezer_snapshot && isApp(p)) {
      /* This is an app kill. Record it in the freezer snapshot so dasd can
       * incorporate this in its recommendations. */
      copied_to_freezer_snapshot =
          memorystatus_jetsam_snapshot_copy_entry_locked(
              memorystatus_jetsam_snapshot_freezer,
              memorystatus_jetsam_snapshot_freezer_max, entry);
      if (copied_to_freezer_snapshot &&
         ...
         */
        memorystatus_log_error(
            "memorystatus: WARNING freezer snapshot buffer is full, count "
            "%zu\n",
            memorystatus_jetsam_snapshot_freezer->entry_count);
      }
    }
    ...
    memorystatus_log_error("memorystatus_update_jetsam_snapshot_entry_locked: "
                           "failed to update pid %d, priority %d, count %d\n",
                           proc_getpid(p), p->p_memstat_effectivepriority,
                           memorystatus_jetsam_snapshot_count);
...
uint32_t memorystatus_get_available_page_count(void) {
  return os_atomic_load(&memorystatus_available_pages, relaxed);
}

void memorystatus_update_available_page_count(uint32_t available_page_count) {
  os_atomic_store(&memorystatus_available_pages, available_page_count, relaxed);
#if VM_PRESSURE_EVENTS
  /*
...
#if CONFIG_FREEZE
  /*
   * We can't grab the freezer_mutex here even though that synchronization would
   * be correct to inspect the # of frozen processes and wakeup the freezer
   * thread. Reason being that we come here into this code with (possibly) the
   * page-queue locks held and preemption disabled. So trying to grab a mutex
   * here will result in the "mutex with preemption disabled" panic.
   */

  if (memorystatus_freeze_thread_should_run()) {
    /*
     * The freezer thread is usually woken up by some user-space call i.e.
     * pid_hibernate(any process). That trigger isn't invoked often enough and
     * so we are enabling this explicit wakeup here.
     */
...
static boolean_t memorystatus_init_jetsam_snapshot_entry_locked(
  ...
  memorystatus_get_task_page_counts(proc_task(p), &pages, &max_pages_lifetime,
                                    &purgeable_pages);
  entry->pages = (uint64_t)pages;
  entry->max_pages_lifetime = (uint64_t)max_pages_lifetime;
  entry->purgeable_pages = (uint64_t)purgeable_pages;
  ...
  entry->jse_gencount = gencount; /* indicates a pass through jetsam thread,
                                     when process was targeted to be killed */

  entry->jse_idle_delta =
      p->p_memstat_idle_delta; /* Most recent timespan spent in idle-band */
  entry->jse_prio_start =
      p->p_memstat_prio_start; /* Time moved to current band */
...
memorystatus_init_snapshot_vmstats(memorystatus_jetsam_snapshot_t *snapshot) {
  ...
  if ((kr = host_statistics64(host_self(), HOST_VM_INFO64,
                              (host_info64_t)&vm_stat, &count)) !=
      KERN_SUCCESS) {
    memorystatus_log_error("memorystatus_init_jetsam_snapshot_stats: "
    ...
    snapshot->stats.total_uncompressed_pages_in_compressor =
        vm_stat.total_uncompressed_pages_in_compressor;
  }

  get_zone_map_size(&snapshot->stats.zone_map_size,
                    &snapshot->stats.zone_map_capacity);

  bzero(snapshot->stats.largest_zone_name,
        sizeof(snapshot->stats.largest_zone_name));
  get_largest_zone_info(snapshot->stats.largest_zone_name,
                        sizeof(snapshot->stats.largest_zone_name),
...
void memorystatus_init_at_boot_snapshot() {
  ...
  memorystatus_at_boot_snapshot.notification_time =
...
static void memorystatus_init_jetsam_snapshot_locked(
  ...
  while (next_p) {
    ...
    if (FALSE == memorystatus_init_jetsam_snapshot_entry_locked(
                     p, &snapshot_list[i], snapshot->js_gencount)) {
      continue;
    }
  ...
  /* Log launchd and kernel_task as well to see more context, even though jetsam
   * doesn't apply to them. */
  if (i < snapshot_max) {
    memorystatus_init_jetsam_snapshot_entry_locked(initproc, &snapshot_list[i],
                                                   snapshot->js_gencount);
    i++;
  }

  if (i < snapshot_max) {
    memorystatus_init_jetsam_snapshot_entry_locked(kernproc, &snapshot_list[i],
                                                   snapshot->js_gencount);
    i++;
  }
...
/* number of entries added to the end of the jetsam snapshot (for launchd and
 * kernel) */
 ...
 * jetsam band.
 */
static int memorystatus_get_sort_order(unsigned int bucket_index, pid_t *pids,
                                       pid_t *order, size_t num_pids) {
  LCK_MTX_ASSERT(&proc_list_mlock, LCK_MTX_ASSERT_OWNED);
 ...
 * function.
 */
static int memorystatus_cmd_test_jetsam_sort(int priority, int sort_order,
  ...
  expected_order =
      kalloc_data_tag(num_pids * sizeof(pid_t), Z_WAITOK, VM_KERN_MEMORY_DIAG);
  actual_order =
      kalloc_data_tag(num_pids * sizeof(pid_t), Z_WAITOK, VM_KERN_MEMORY_DIAG);
  ...
  if (expected_order_user != CAST_USER_ADDR_T(NULL) &&
      expected_order_user_len > 0) {
    bzero(actual_order, num_pids * sizeof(pid_t));
    error = memorystatus_get_sort_order(priority, expected_order, actual_order,
                                        num_pids);
...
#if SOCKETS
static int networking_memstatus_callout(proc_t p, uint32_t status) {
  struct fileproc *fp;
  ...
  fdt_foreach(fp, p) {
    ...
    case DTYPE_NETPOLICY:
      necp_fd_memstatus(p, status, (struct necp_fd_data *)fp_get_data(fp));
      break;
#endif /* NECP */
#if SKYWALK
    case DTYPE_CHANNEL:
      kern_channel_memstatus(p, status, (struct kern_channel *)fp_get_data(fp));
      break;
#endif /* SKYWALK */
    default:
      break;
...
static bool memorystatus_kill_proc(proc_t p, uint32_t cause,
  ...
  if (cause != kMemorystatusKilledVnodes &&
    ...
    if (num_pages_reclaimed) {
      ...
      if (cause == kMemorystatusKilledHiwat) {
        ...
        uint64_t memlimit_in_bytes =
            (((uint64_t)p->p_memstat_memlimit) * 1024ULL *
             1024ULL); /* convert MB to bytes */
        success = (footprint_in_bytes <= memlimit_in_bytes);
      } else {
        success = !(memorystatus_get_available_page_count() <
                    memorystatus_get_soft_memlimit_page_shortage_threshold());
#if CONFIG_SECLUDED_MEMORY
      ...
      if (success) {
        ...
        memorystatus_log_info(
            "memorystatus: reclaimed %llu pages (%llu purged, %llu unsecluded) "
            "from pid %d [%s] and avoided %s\n",
            num_pages_reclaimed, num_pages_purged, num_pages_unsecluded, aPid,
            ((p && *p->p_name) ? p->p_name : "unknown"),
            memstat_kill_cause_name[cause]);

        *killed = false;
        *footprint_out =
            num_pages_reclaimed + num_pages_purged + num_pages_unsecluded;
  ...
  *killed = retval;
  ...
  memorystatus_log(
      "memorystatus: %s pid %d [%s] (%s %d %llus rf:%s type:%s) %lluKB - "
      "memorystatus_available_pages: %llu compressor_size:%u\n",
      kill_reason_string, aPid, proc_best_name(p),
      memstat_kill_cause_name[cause], aPid_ep, time_in_priority_band_secs,
      _memstat_relaunch_flags_description(p->p_memstat_relaunch_flags),
      _memstat_proc_type_description(p), (*footprint_out) >> 10,
...
static bool memstat_kill_top_process(uint32_t cause, os_reason_t jetsam_reason,
                                     int32_t max_priority,
                                     memstat_kill_options_t options,
                                     int32_t *priority_out,
                                     uint32_t *errors_out,
                                     uint64_t *memory_reclaimed_out) {
  pid_t aPid;
  proc_t p = PROC_NULL, next_p = PROC_NULL;
  bool new_snapshot = false, force_new_snapshot = false, killed = false,
       freed_mem = false;
  unsigned int i = 0;
  uint32_t aPid_ep;
  uint64_t footprint_of_killed_proc = 0;
...
#if CONFIG_JETSAM
  if (sort_bucket) {
    (void)memstat_sort_bucket(JETSAM_PRIORITY_FOREGROUND,
                              memstat_jetsam_fg_sort_order);
  }
  ...
  /*
   * And, because we are here under extreme circumstances, we force a snapshot
   * even for IDLE kills.
   */
  ...
  while (next_p && (next_p->p_memstat_effectivepriority <= max_priority)) {
    ...
    if (p->p_memstat_state &
        (P_MEMSTAT_ERROR | P_MEMSTAT_TERMINATED | P_MEMSTAT_SKIP)) {
    ...
    if (only_long_idle) {
      if (!_memstat_proc_is_reapable(p)) {
        memorystatus_log_debug("memorystatus: memstat_kill_top_process: "
                               "skipping non-reapable process %s [%d]\n",
                               proc_best_name(p), p->p_pid);
        continue;
      }
      memorystatus_log_debug("memorystatus: memstat_kill_top_process: found "
                             "reapable long-idle process %s [%d]\n",
                             proc_best_name(p), p->p_pid);
    }
    ...
    if (max_priority == JETSAM_PRIORITY_IDLE &&
       ...
       */
      memorystatus_log_error(
          "memorystatus: skipping idle but not idle-exitable process "
          "%s [%d] (0x%x)\n",
    ...
    if (proc_ref(p, true) == p) {
      /*
       * Mark as terminated so that if exit1() indicates success, but the
       * process (for example) is blocked in task_exception_notify(), it'll be
       * skipped if encountered again - see <rdar://problem/13553476>. This is
       * cheaper than examining P_LEXIT, which requires the acquisition of the
       * proc lock.
       */
      p->p_memstat_state |= P_MEMSTAT_TERMINATED;
    } else {
     ...
     * - priority was not requested (this is something other than an ambient
     * kill)
     * - the priority was requested *and* the targeted process is not at idle
     * priority
     */
    if ((memorystatus_jetsam_snapshot_count == 0) &&
        (force_new_snapshot || memorystatus_idle_snapshot ||
         ((!priority_out) ||
          (priority_out && (aPid_ep != JETSAM_PRIORITY_IDLE))))) {
      memorystatus_init_jetsam_snapshot_locked(NULL, 0);
      new_snapshot = true;
    }
    ...
    freed_mem = memorystatus_kill_proc(
        p, cause, jetsam_reason, &killed,
        &footprint_of_killed_proc); /* purged and/or killed 'p' */
    /* Success? */
    if (freed_mem) {
      if (memory_reclaimed_out) {
...
exit:
  ...
  if (!killed) {
    ...
    if (new_snapshot) {
      proc_list_lock();
      memorystatus_jetsam_snapshot->entry_count =
          memorystatus_jetsam_snapshot_count = 0;
      proc_list_unlock();
    }
#if CONFIG_JETSAM
    if (only_long_idle) {
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_JETSAM) | DBG_FUNC_END,
       MEMORYSTATUS_LOG_AVAILABLE_PAGES, killed ? aPid : 0, killed,
       footprint_of_killed_proc);

  return killed;
}

static void _memstat_refresh_oldest_reapable_proc_info() {
  ...
  /* If we're still within 'memstat_reaper_rescan_secs' of the last process-list
   * walk, don't do another walk yet, and just use the existing information.
   */
  if (mat < memstat_oldest_reapable_proc_info_expiration_ts_matu) {
    memorystatus_log_debug(
        "memorystatus: _memstat_refresh_oldest_reapable_proc_info: re-using "
        "existing data\n");
    return;
  } else {
    memorystatus_log_debug(
        "memorystatus: _memstat_refresh_oldest_reapable_proc_info: rescanning "
        "proc list\n");
  }
  ...
  while (next_p) {
    ...
    /* Since the process list is sorted in priority order, once we find the
     * first process that is out of the reaper's acceptible range, we can skip
     * the rest
     */
    ...
    if (_memstat_proc_is_reapable(p)) {
      ...
      if (proc_prio_start < oldest_prio_start) {
         ...
         * the first process will be the oldest one, and we can bail out and
         * skip the rest
         */
        break;
      }
  ...
  if (memstat_oldest_reapable_proc_prio_start !=
    ...
    nanoseconds_to_absolutetime((memstat_reaper_min_age_secs * NSEC_PER_SEC),
                                &min_age_matu);
    memstat_oldest_reapable_proc_will_be_reapable_at_ts_matu =
        memstat_oldest_reapable_proc_prio_start + min_age_matu;
  }

  nanoseconds_to_absolutetime((memstat_reaper_rescan_secs * NSEC_PER_SEC),
                              &rescan_timeout_duration_matu);
  memstat_oldest_reapable_proc_info_expiration_ts_matu =
      mat + rescan_timeout_duration_matu;
}

static bool _memstat_proc_is_reapable(proc_t proc) {
   ...
   *  - must not have a relaunch probability of High or Medium (per
   * memstat_reaper_reap_relaunch_mask)
   *  - must have been in that priority band longer than the reaper minimum age
   * threshold
   *  - must have been in that priority band longer than the reaper minimum age
   * threshold for applications, if process is an application
   */
  priority_band = proc->p_memstat_effectivepriority;
  if (priority_band > memstat_reaper_max_priority) {
    memorystatus_log_debug("_memstat_proc_is_reapable: %s [%d] not reapable "
                           "because priority (%d) is above threshold (%d)\n",
                           proc_best_name(proc), proc->p_pid, priority_band,
                           memstat_reaper_max_priority);
    return false;
  }

  uint32_t relaunch_flags = proc->p_memstat_relaunch_flags;
  // There's no explicit flag for "unknown" relaunch probability, and we need
  // one for our control bitmask. So if none of the Low Medium or High bits are
  // set, we set the next higher bit as the "unknown relaunch probability" bit
  // and then test all the bits at once, below, with a bitwise-and.
  if ((relaunch_flags & (P_MEMSTAT_RELAUNCH_LOW | P_MEMSTAT_RELAUNCH_MED |
                         P_MEMSTAT_RELAUNCH_HIGH)) == 0) {
    relaunch_flags |=
        MEMORYSTATUS_REAPER_REAP_RELAUNCH_MASK_UNKNOWN; // The bit for 'unknown'
                                                        // is the one just to
                                                        // the left (above) of
                                                        // High, e.g. 0x08
  }
  relaunch_probability_acceptable_mask =
      relaunch_flags & memstat_reaper_reap_relaunch_mask;

  if (relaunch_probability_acceptable_mask == 0) {
    memorystatus_log_debug(
        "_memstat_proc_is_reapable: %s [%d] not reapable because relaunch "
        "probability bitmask (0x%02X) does not match with the "
        "memstat_reaper_reap_relaunch_mask (0x%02X).\n",
        proc_best_name(proc), proc->p_pid, relaunch_flags,
        memstat_reaper_reap_relaunch_mask);
    return false;
  }

  absolutetime_to_nanoseconds(mach_absolute_time() - proc->p_memstat_prio_start,
                              &time_in_priority_band_secs);
  time_in_priority_band_secs /= NSEC_PER_SEC;
  ...
  if (_memstat_proc_is_application(proc)) {
    if ((time_in_priority_band_secs < memstat_reaper_min_age_apps_secs)) {
      memorystatus_log_debug(
          "_memstat_proc_is_reapable: %s [%d] not reapable because it is an "
          "application and age (%llu) is below min age for apps (%d)\n",
          proc_best_name(proc), proc->p_pid, time_in_priority_band_secs,
          memstat_reaper_min_age_apps_secs);
      return false;
    }
  } else {
    if (time_in_priority_band_secs < memstat_reaper_min_age_secs) {
      memorystatus_log_debug("_memstat_proc_is_reapable: %s [%d] not reapable "
                             "because age (%llu) is below min age (%d)\n",
                             proc_best_name(proc), proc->p_pid,
                             time_in_priority_band_secs,
  ...
  memorystatus_log_debug(
      "_memstat_proc_is_reapable: %s [%d] is reapable; priority=%d, age=%d, "
      "relaunch_probability_acceptable_mask=0x%02X, type=%s\n",
      proc_best_name(proc), proc->p_pid, priority_band,
      (uint32_t)(time_in_priority_band_secs),
      relaunch_probability_acceptable_mask,
      _memstat_proc_type_description(proc));
  return true;
}

static bool _memstat_proc_is_application(proc_t proc) {
  bool isApp = false;
 ...
 */
static bool memorystatus_kill_processes_aggressive(
    uint32_t cause, int aggr_count, int32_t priority_max, int max_kills,
    uint32_t *errors, uint64_t *memory_reclaimed) {
  pid_t aPid;
  proc_t p = PROC_NULL, next_p = PROC_NULL;
  ...
  *memory_reclaimed = 0;
  ...
  if (priority_max >= JETSAM_PRIORITY_FOREGROUND) {
     ...
     * coalition footprint.
     */
    memstat_sort_bucket(JETSAM_PRIORITY_FOREGROUND,
                        memstat_jetsam_fg_sort_order);
  }

  jetsam_reason = os_reason_create(OS_REASON_JETSAM, cause);
  if (jetsam_reason == OS_REASON_NULL) {
    memorystatus_log_error("memorystatus_kill_processes_aggressive: failed to "
                           "allocate exit reason\n");
  }
  memorystatus_log(
  ...
  while (next_p) {
    if (proc_list_exited(next_p) ||
        ((unsigned int)(next_p->p_memstat_effectivepriority) !=
         priority_band)) {
      /*
      ...
      memorystatus_log_debug("memorystatus: aggressive%d: rewinding band %d, "
                             "%s(%d) moved or exiting.\n",
                             aggr_count, priority_band,
    ...
    /*
     * Mark as terminated so that if exit1() indicates success, but the process
     * (for example) is blocked in task_exception_notify(), it'll be skipped if
     * encountered again - see <rdar://problem/13553476>. This is cheaper than
     * examining P_LEXIT, which requires the acquisition of the proc lock.
     */
    p->p_memstat_state |= P_MEMSTAT_TERMINATED;
    ...
    if (proc_ref(p, true) == p) {
      if (next_p) {
        while (next_p && (proc_ref(next_p, true) != next_p)) {
          ...
          memorystatus_log_debug(
              ...
              aggr_count, proc_getpid(next_p),
              (*next_p->p_name ? next_p->p_name : "(unknown)"));

          temp_p = next_p;
          next_p =
              memorystatus_get_next_proc_locked(&priority_band, temp_p, TRUE);
        }
      }
      proc_list_unlock();
      ...
      time_in_priority_band_secs /= NSEC_PER_SEC;
      memorystatus_log(
          "memorystatus: %s%d pid %d [%s] (%s %d %llus rf:%s type:%s) - "
          "memorystatus_available_pages: %llu\n",
          ((aPid_ep == JETSAM_PRIORITY_IDLE)
               ? "killing_idle_process_aggressive"
               : "killing_top_process_aggressive"),
          aggr_count, aPid, proc_best_name(p), memstat_kill_cause_name[cause],
          aPid_ep, time_in_priority_band_secs,
          _memstat_relaunch_flags_description(p->p_memstat_relaunch_flags),
      ...
      if (killed) {
        *memory_reclaimed += footprint_of_killed_proc;
        ...
        if (aPid_ep == JETSAM_PRIORITY_FOREGROUND &&
            memorystatus_aggressive_jetsam_lenient == TRUE) {
          if (memorystatus_level > memorystatus_level_snapshot &&
              ((memorystatus_level - memorystatus_level_snapshot) >=
               AGGRESSIVE_JETSAM_LENIENT_MODE_THRESHOLD)) {
#if DEVELOPMENT || DEBUG
            memorystatus_log_info(
                "Disabling Lenient mode after one-time deployment.\n");
#endif /* DEVELOPMENT || DEBUG */
...
static boolean_t memorystatus_kill_hiwat_proc(uint32_t *errors,
  ...
  jetsam_reason =
      os_reason_create(OS_REASON_JETSAM, JETSAM_REASON_MEMORY_HIGHWATER);
  if (jetsam_reason == OS_REASON_NULL) {
    memorystatus_log_error(
  ...
  while (next_p) {
    ...
    memlimit_in_bytes = (((uint64_t)p->p_memstat_memlimit) * 1024ULL *
                         1024ULL); /* convert MB to bytes */
    ...
    if (skip) {
      ...
      freed_mem = memorystatus_kill_proc(
          p, kMemorystatusKilledHiwat, jetsam_reason, &killed,
          &footprint_in_bytes); /* purged and/or killed 'p' */
...
boolean_t memorystatus_kill_elevated_process(uint32_t cause,
  ...
  while (next_p) {
...
#if DEVELOPMENT || DEBUG
    memorystatus_log_info("jetsam: elevated%d process pid %d [%s] - "
                          "memorystatus_available_pages: %d\n",
                          aggr_count, aPid,
                          (*p->p_name ? p->p_name : "unknown"),
                          MEMORYSTATUS_LOG_AVAILABLE_PAGES);
#endif /* DEVELOPMENT || DEBUG */
    ...
    if (proc_ref(p, true) == p) {
      ...
      time_in_priority_band_secs /= NSEC_PER_SEC;
      memorystatus_log(
          "%lu.%03d memorystatus: killing_top_process_elevated%d pid %d [%s] "
          "(%s %d %llus rf:%s type:%s) %lluKB - memorystatus_available_pages: "
          "%llu\n",
          (unsigned long)tv_sec, tv_msec, aggr_count, aPid,
          ((p && *p->p_name) ? p->p_name : "unknown"),
          memstat_kill_cause_name[cause], aPid_ep, time_in_priority_band_secs,
          _memstat_relaunch_flags_description(p->p_memstat_relaunch_flags),
          _memstat_proc_type_description(p), footprint_of_killed_proc >> 10,
...
exit:
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_JETSAM) | DBG_FUNC_END,
       MEMORYSTATUS_LOG_AVAILABLE_PAGES, killed ? aPid : 0, kill_count,
       *memory_reclaimed);

  return killed;
}

bool memorystatus_kill_on_VM_compressor_space_shortage(bool async) {
  if (async) {
    os_atomic_store(&memorystatus_compressor_space_shortage, true, release);
    memorystatus_thread_wake();
    return true;
  } else {
    os_reason_t jetsam_reason = os_reason_create(
        OS_REASON_JETSAM, JETSAM_REASON_MEMORY_VMCOMPRESSOR_SPACE_SHORTAGE);
    if (jetsam_reason == OS_REASON_NULL) {
      memorystatus_log_error(
          "memorystatus_kill_on_VM_compressor_space_shortage -- sync: failed "
          "to allocate jetsam reason\n");
    }

    return memstat_kill_process_sync(
        -1, kMemorystatusKilledVMCompressorSpaceShortage, jetsam_reason);
  }
}
...
bool memorystatus_kill_on_vnode_exhaustion(void) {
  os_reason_t jetsam_reason =
      os_reason_create(OS_REASON_JETSAM, JETSAM_REASON_VNODE);
  if (jetsam_reason == OS_REASON_NULL) {
    memorystatus_log_error(
        "memorystatus_kill_on_vnode_limit: failed to allocate jetsam reason\n");
  }

  return memstat_kill_process_sync(-1, kMemorystatusKilledVnodes,
                                   jetsam_reason);
}

#endif /* CONFIG_JETSAM */

bool memorystatus_kill_on_sustained_pressure() {
  os_reason_t jetsam_reason = os_reason_create(
      OS_REASON_JETSAM, JETSAM_REASON_MEMORY_SUSTAINED_PRESSURE);
  if (jetsam_reason == OS_REASON_NULL) {
    memorystatus_log_error("%s() failed to allocate jetsam reason\n", __func__);
  }

  return memstat_kill_process_sync(-1, kMemorystatusKilledSustainedPressure,
                                   jetsam_reason);
}

bool memstat_kill_with_jetsam_reason_sync(pid_t pid,
                                          os_reason_t jetsam_reason) {
  uint32_t kill_cause =
      jetsam_reason->osr_code <= JETSAM_REASON_MEMORYSTATUS_MAX
          ? (uint32_t)jetsam_reason->osr_code
          : JETSAM_REASON_INVALID;
  return memstat_kill_process_sync(pid, kill_cause, jetsam_reason);
}

bool memorystatus_kill_on_zone_map_exhaustion(pid_t pid) {
  if (pid == -1) {
    ...
    os_reason_t jetsam_reason =
        os_reason_create(OS_REASON_JETSAM, JETSAM_REASON_ZONE_MAP_EXHAUSTION);
    if (jetsam_reason == OS_REASON_NULL) {
      memorystatus_log_error("memorystatus_kill_on_zone_map_exhaustion: failed "
                             "to allocate jetsam reason\n");
    }
    return memstat_kill_process_sync(pid, kMemorystatusKilledZoneMapExhaustion,
                                     jetsam_reason);
  }
}

void memorystatus_on_pageout_scan_end(void) { /* No-op */ }

static size_t memorystatus_priority_list_size(pid_t pid, size_t entry_size) {
  assert((entry_size == sizeof(memorystatus_priority_entry_t)) ||
...
/* Return both allocated and actual size, since there's a race between
 * allocation and list compilation */
static int
memorystatus_get_priority_list(memorystatus_priority_entry_v2_t **list_ptr,
                               size_t *buffer_size, size_t *list_size,
                               size_t entry_size) {
  ...
  *list_size = 0;
  ...
  while (p && (*list_size < *buffer_size)) {
    ...
    if (entry_size == sizeof(memorystatus_priority_entry_t)) {
      entry = (memorystatus_priority_entry_v2_t
                   *)(((memorystatus_priority_entry_t *)entry) + 1);
    } else {
...
static int memorystatus_get_priority_pid(pid_t pid, user_addr_t buffer,
                                         size_t buffer_size) {
  int error = 0;
  bool zombref = false;
...
static int memorystatus_cmd_get_priority_list(pid_t pid, user_addr_t buffer,
                                              size_t buffer_size,
                                              int32_t *retval,
                                              size_t entry_size) {
  int error = 0;
  ...
  if (pid != 0) {
    ...
    error = memorystatus_get_priority_list(&list, &buffer_size, &list_size,
                                           entry_size);
    if (error == 0) {
...
void memorystatus_fast_jetsam_override(bool enable_override) {
#if CONFIG_JETSAM
 ...
 */
static int
memorystatus_get_at_boot_snapshot(memorystatus_jetsam_snapshot_t **snapshot,
                                  size_t *snapshot_size, boolean_t size_only) {
  ...
  *snapshot = &memorystatus_at_boot_snapshot;
  ...
  memorystatus_log_debug("memorystatus_get_at_boot_snapshot: returned "
                         "inputsize (%ld), snapshot_size(%ld), listcount(%d)\n",
                         (long)input_size, (long)*snapshot_size, 0);
  return 0;
}

#if CONFIG_FREEZE
static int memorystatus_get_jetsam_snapshot_freezer(
    memorystatus_jetsam_snapshot_t **snapshot, size_t *snapshot_size,
    boolean_t size_only) {
  size_t input_size = *snapshot_size;
  ...
  *snapshot = memorystatus_jetsam_snapshot_freezer;
  ...
  memorystatus_log_debug(
      "memorystatus_get_jetsam_snapshot_freezer: returned inputsize (%ld), "
      "snapshot_size(%ld), listcount(%ld)\n",
      (long)input_size, (long)*snapshot_size,
      (long)memorystatus_jetsam_snapshot_freezer->entry_count);
...
static int
memorystatus_get_on_demand_snapshot(memorystatus_jetsam_snapshot_t **snapshot,
                                    size_t *snapshot_size,
                                    boolean_t size_only) {
  size_t input_size = *snapshot_size;
  uint32_t ods_list_count =
      memorystatus_list_count + memorystatus_artificial_snapshot_entry_count;
  memorystatus_jetsam_snapshot_t *ods =
      NULL; /* The on_demand snapshot buffer */

  *snapshot_size =
      sizeof(memorystatus_jetsam_snapshot_t) +
      (sizeof(memorystatus_jetsam_snapshot_entry_t) * (ods_list_count));
  ...
  *snapshot = ods;
  ...
  memorystatus_log_debug(
      "memorystatus_get_on_demand_snapshot: returned inputsize (%ld), "
      "snapshot_size(%ld), listcount(%ld)\n",
...
static int
memorystatus_get_jetsam_snapshot(memorystatus_jetsam_snapshot_t **snapshot,
                                 size_t *snapshot_size, boolean_t size_only) {
  size_t input_size = *snapshot_size;
  ...
  *snapshot = memorystatus_jetsam_snapshot;
  ...
  memorystatus_log_debug("memorystatus_get_jetsam_snapshot: returned inputsize "
 ...
 */
static int memorystatus_cmd_get_data_buffer(user_addr_t buffer,
                                            size_t buffer_size, int32_t *retval,
                                            size_t data_size, void *data) {
  boolean_t size_only = (buffer == USER_ADDR_NULL);
  int error;

  /* Nothing to return if there's no data yet, instruct the caller to try again
   * later. */
  if (data == NULL) {
    *retval = -1;
    return EAGAIN;
...
static int memorystatus_cmd_get_jetsam_snapshot(int32_t flags,
                                                user_addr_t buffer,
                                                size_t buffer_size,
                                                int32_t *retval) {
  int error = EINVAL;
  ...
  if (flags == 0) {
    ...
    error =
        memorystatus_get_jetsam_snapshot(&snapshot, &buffer_size, size_only);
  } else {
    if (flags &
        ~(MEMORYSTATUS_SNAPSHOT_ON_DEMAND | MEMORYSTATUS_SNAPSHOT_AT_BOOT |
          MEMORYSTATUS_FLAGS_SNAPSHOT_FREEZER)) {
      /*
    ...
    if (flags & MEMORYSTATUS_SNAPSHOT_ON_DEMAND) {
       ...
       */
      error = memorystatus_get_on_demand_snapshot(&snapshot, &buffer_size,
                                                  size_only);
    } else if (flags & MEMORYSTATUS_SNAPSHOT_AT_BOOT) {
      is_at_boot_snapshot = TRUE;
      error =
          memorystatus_get_at_boot_snapshot(&snapshot, &buffer_size, size_only);
#if CONFIG_FREEZE
    } else if (flags & MEMORYSTATUS_FLAGS_SNAPSHOT_FREEZER) {
      is_freezer_snapshot = true;
      error = memorystatus_get_jetsam_snapshot_freezer(&snapshot, &buffer_size,
                                                       size_only);
#endif /* CONFIG_FREEZE */
    } else {
      /*
    ...
    if ((error = copyout(snapshot, buffer, buffer_size)) == 0) {
      ...
      if (is_default_snapshot) {
        ...
        if (memorystatus_testing_pid != 0 &&
            memorystatus_testing_pid != proc_getpid(current_proc())) {
...
#if DEVELOPMENT || DEBUG
static int memorystatus_cmd_set_testing_pid(int32_t flags) {
  int error = EINVAL;
 ...
 *		  A process could be killed before the kernel can act on it
 * here. If a pid cannot be found in any of the jetsam priority bands, then we
 * simply ignore it.  No harm. But, if the pid has been recycled then it could
 * be an issue. In that scenario, we might move an unsuspecting process to the
 * new priority band. It's not clear how the kernel can safeguard against this,
 * but it would be an extremely rare case anyway. The caller of this api might
 * avoid such race conditions by ensuring that the processes passed in the pid
 * list are suspended.
 */

static int memorystatus_cmd_grp_set_priorities(user_addr_t buffer,
                                               size_t buffer_size) {
  /*
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_GRP_SET_PROP) | DBG_FUNC_START,
       MEMORYSTATUS_FLAGS_GRP_SET_PRIORITY, entry_count);
  ...
  for (i = 0; i < entry_count; i++) {
    ...
    } else if (entries[i].priority > JETSAM_PRIORITY_IDLE &&
               entries[i].priority <= applications_aging_band) {
      /*
       * Everything between idle and the aging bands are reserved for internal
       * use. if requested, adjust to JETSAM_PRIORITY_IDLE. Entitled processes
       * (just munch) can use a subset of this range for testing.
    ...
    } else if ((entries[i].priority < 0) ||
               (entries[i].priority >= MEMSTAT_BUCKET_COUNT)) {
      /* Sanity check */
      error = EINVAL;
      goto out;
    }
   ...
   */
out:
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_GRP_SET_PROP) | DBG_FUNC_END,
       MEMORYSTATUS_FLAGS_GRP_SET_PRIORITY, entry_count, table_count);
...
memorystatus_internal_probabilities_t *memorystatus_global_probabilities_table =
    NULL;
size_t memorystatus_global_probabilities_size = 0;

static int memorystatus_cmd_grp_set_probabilities(user_addr_t buffer,
                                                  size_t buffer_size) {
  int error = 0;
  memorystatus_properties_entry_v1_t *entries = NULL;
  size_t entry_count = 0, i = 0;
  memorystatus_internal_probabilities_t *tmp_table_new = NULL,
                                        *tmp_table_old = NULL;
  ...
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_GRP_SET_PROP) | DBG_FUNC_START,
       MEMORYSTATUS_FLAGS_GRP_SET_PROBABILITY, entry_count);
  ...
  tmp_table_new_size =
      sizeof(memorystatus_internal_probabilities_t) * entry_count;

  if ((tmp_table_new = kalloc_data(tmp_table_new_size, Z_WAITOK | Z_ZERO)) ==
      NULL) {
    error = ENOMEM;
    goto out;
  }
  ...
  for (i = 0; i < entry_count; i++) {
    /* Build the table data  */
    strlcpy(memorystatus_global_probabilities_table[i].proc_name,
            entries[i].proc_name, MAXCOMLEN + 1);
    memorystatus_global_probabilities_table[i].use_probability =
        entries[i].use_probability;
  }
...
out:
  KDBG(MEMSTAT_CODE(BSD_MEMSTAT_GRP_SET_PROP) | DBG_FUNC_END,
       MEMORYSTATUS_FLAGS_GRP_SET_PROBABILITY, entry_count, tmp_table_new_size);
...
static int memorystatus_cmd_grp_set_properties(int32_t flags,
                                               user_addr_t buffer,
                                               size_t buffer_size,
                                               __unused int32_t *retval) {
  int error = 0;

  if ((flags & MEMORYSTATUS_FLAGS_GRP_SET_PRIORITY) ==
      MEMORYSTATUS_FLAGS_GRP_SET_PRIORITY) {
    error = memorystatus_cmd_grp_set_priorities(buffer, buffer_size);
  } else if ((flags & MEMORYSTATUS_FLAGS_GRP_SET_PROBABILITY) ==
             MEMORYSTATUS_FLAGS_GRP_SET_PROBABILITY) {
    error = memorystatus_cmd_grp_set_probabilities(buffer, buffer_size);
#if CONFIG_FREEZE
  } else if ((flags & MEMORYSTATUS_FLAGS_GRP_SET_FREEZE_PRIORITY) ==
             MEMORYSTATUS_FLAGS_GRP_SET_FREEZE_PRIORITY) {
    error = memorystatus_cmd_grp_set_freeze_list(buffer, buffer_size);
  } else if ((flags & MEMORYSTATUS_FLAGS_GRP_SET_DEMOTE_PRIORITY) ==
...
/*
 * This routine is used to update a process's jetsam priority position and
 * stored user_data. It is not used for the setting of memory limits.
 *
 * Flags passed into this call are used to distinguish the motivation behind a
 * jetsam priority transition.  By default, the kernel updates the process's
 * original requested priority when no flag is passed.  But when the
 * MEMORYSTATUS_SET_PRIORITY_ASSERTION flag is used, the kernel updates the
 * process's assertion driven priority.
 *
 * The assertion flag was introduced for use by the device's assertion mediator
 * (eg: runningboardd). When an assertion is controlling a process's jetsam
 * priority, it may conflict with that process's dirty/clean (active/inactive)
 * jetsam state.  The kernel attempts to resolve a priority transition conflict
 * by reviewing the process state and then choosing the maximum jetsam band at
 * play, eg: requested priority versus assertion priority.
 */

static int memorystatus_cmd_set_priority_properties(pid_t pid, uint32_t flags,
                                                    user_addr_t buffer,
                                                    size_t buffer_size,
                                                    __unused int32_t *retval) {
  int error = 0;
  ...
  if ((pid == 0) || (buffer == USER_ADDR_NULL) ||
      (buffer_size != sizeof(memorystatus_priority_properties_t))) {
    return EINVAL;
  }
  ...
  if (error == 0) {
    ...
    error = memorystatus_set_priority(p, mpp_entry.priority,
                                      mpp_entry.user_data, options);
    proc_rele(p);
  }
...
memorystatus_cmd_set_diag_memlimit_properties(pid_t pid, user_addr_t buffer,
  ...
  /* Validate inputs */
  if ((pid == 0) || (buffer == USER_ADDR_NULL) ||
      (buffer_size != sizeof(memorystatus_diag_memlimit_properties_t))) {
    proc_rele(p);
    return EINVAL;
  }
...
static int _memstat_get_process_conclave_mem_limit(pid_t pid, int32_t *retval) {
  kern_return_t error;
  proc_t p = proc_find(pid);
  if (!p) {
    return ESRCH;
...
/*
 * When getting the memlimit settings, we can't simply call
 * task_get_phys_footprint_limit(). That gets the proc's cached memlimit and
 * there is no guarantee that the active/inactive limits will be the same in the
 * no-limit case.  Instead we convert limits <= 0 using
 * task_convert_phys_footprint_limit(). It computes the same limit value that
 * would be written to the task's ledgers via task_set_phys_footprint_limit().
 */
static int memorystatus_cmd_get_memlimit_properties(pid_t pid,
                                                    user_addr_t buffer,
                                                    size_t buffer_size,
...
static int memorystatus_cmd_get_memlimit_excess_np(pid_t pid, uint32_t flags,
  ...
  /* Validate inputs */
  if ((pid == 0) || (buffer == USER_ADDR_NULL) ||
      (buffer_size != sizeof(uint64_t)) || (flags != 0)) {
    return EINVAL;
  }
...
static int memorystatus_cmd_get_pressure_status(int32_t *retval) {
  int error;
...
int memorystatus_get_pressure_status_kdp() {
  return (kVMPressureNormal != memorystatus_vm_pressure_level) ? 1 : 0;
}

/*
 * Every process, including a P_MEMSTAT_INTERNAL process (currently only pid 1),
 * is allowed to set a HWM.
 *
 * This call is inflexible -- it does not distinguish between active/inactive,
 * fatal/non-fatal So, with 2-level HWM preserving previous behavior will map as
 * follows.
 *      - treat the limit passed in as both an active and inactive limit.
 *      - treat the is_fatal_limit flag as though it applies to both active and
 * inactive limits.
 *
 * When invoked via MEMORYSTATUS_CMD_SET_JETSAM_HIGH_WATER_MARK
 *      - the is_fatal_limit is FALSE, meaning the active and inactive limits
 * are non-fatal/soft
 *      - so mapping is (active/non-fatal, inactive/non-fatal)
 *
 * When invoked via MEMORYSTATUS_CMD_SET_JETSAM_TASK_LIMIT
 *      - the is_fatal_limit is TRUE, meaning the process's active and inactive
 * limits are fatal/hard
...
static int
memorystatus_cmd_mark_process_coalition_swappable(pid_t pid,
                                                  __unused int32_t *retval) {
  int error = 0;
  proc_t p = PROC_NULL;
...
static int
memorystatus_cmd_get_process_coalition_is_swappable(pid_t pid,
                                                    int32_t *retval) {
  int error = 0;
  proc_t p = PROC_NULL;
  coalition_t coal = COALITION_NULL;
...
static int memorystatus_cmd_convert_memlimit_mb(pid_t pid, int32_t limit,
                                                int32_t *retval) {
  int error = 0;
  proc_t p;
  p = proc_find(pid);
...
static int _memstat_rearm_proc_memlimit(proc_t proc, void *flagsptr) {
  task_t task = proc_task(proc);
...
static int memorystatus_cmd_rearm_memlimit(pid_t pid, uint32_t flags,
                                           __unused int32_t *retval) {
  if (pid == -1) {
    /* Re-arm all pids */
    proc_iterate(PROC_ALLPROCLIST, _memstat_rearm_proc_memlimit, &flags, NULL,
                 NULL);
  } else {
    /* Re-arm one pid */
...
static int memorystatus_set_diag_memlimit_properties_internal(
  ...
  error = (task_set_diag_footprint_limit_internal(
               proc_task(p), p_entry->memlimit, &old_limit) == KERN_SUCCESS)
              ? KERN_SUCCESS
              : EINVAL;

  memorystatus_log_debug("memorystatus_set_diag_memlimit_properties: new limit "
                         "on pid %d (%lluMB old %lluMB)\n",
                         proc_getpid(p),
                         (p_entry->memlimit > 0 ? p_entry->memlimit : -1),
                         (old_limit));
  DTRACE_MEMORYSTATUS2(
      memorystatus_diag_memlimit_properties_t, proc_t, p, int32_t,
      (p->p_memstat_memlimit > 0 ? p->p_memstat_memlimit : -1));
  return error;
}
...
static int memorystatus_get_diag_memlimit_properties_internal(
  ...
  int error = 0;
  /* Enforce the limit by writing to the ledgers */
  error = (task_get_diag_footprint_limit_internal(
               proc_task(p), &p_entry->memlimit, &p_entry->threshold_enabled) ==
           KERN_SUCCESS)
              ? KERN_SUCCESS
              : EINVAL;
...
bool memorystatus_task_has_increased_memory_limit_entitlement(task_t task) {
  ...
  return IOTaskHasEntitlement(
      task, "com.apple.developer.kernel.increased-memory-limit");
}

bool memorystatus_task_has_increased_debugging_memory_limit_entitlement(
    task_t task) {
  if (memorystatus_entitled_dev_max_task_footprint_mb == 0) {
    // Entitlement is not supported on this device.
    return false;
  ...
  return IOTaskHasEntitlement(
      task, "com.apple.developer.kernel.increased-debugging-memory-limit");
}
...
bool memorystatus_task_has_ios13extended_footprint_limit(task_t task) {
  if (max_mem < 1500ULL * 1024 * 1024 || max_mem > 2ULL * 1024 * 1024 * 1024) {
    /* ios13extended_footprint is only for 2GB devices */
    return false;
  }
  return IOTaskHasEntitlement(
      task, "com.apple.developer.memory.ios13extended_footprint");
}
...
static int32_t memorystatus_get_default_task_active_limit(proc_t p) {
   ...
   */
  if (memorystatus_task_has_increased_debugging_memory_limit_entitlement(
          task)) {
...
#if __arm64__
  if (legacy_footprint_entitlement_mode ==
...
static int memorystatus_set_memlimit_properties(
    pid_t pid, memorystatus_memlimit_properties_t *entry) {
  ...
  int error = memorystatus_set_memlimits(p, memlimit_active, memlimit_inactive,
                                         memlimit_options);
  proc_rele(p);
  return error;
}
 ...
 */
int proc_get_memstat_priority(proc_t p, boolean_t effective_priority) {
  if (p) {
...
static int memorystatus_get_process_is_managed(pid_t pid, int *is_managed) {
  proc_t p = NULL;
...
bool memorystatus_get_proc_is_managed(proc_t proc) {
  proc_list_lock();
...
static int memorystatus_set_process_is_managed(pid_t pid,
...
static int _memstat_get_kill_count(int priority,
                                   memorystatus_kill_cause_t cause,
                                   bool clear) {
...
static int memorystatus_cmd_get_kill_counts(int priority, user_addr_t buffer,
  ...
  if (((buffer_size % sizeof(uint32_t)) != 0) ||
      (priority < JETSAM_PRIORITY_IDLE) || (priority > JETSAM_PRIORITY_MAX)) {
    return EINVAL;
  }

  for (cause = kMemorystatusInvalid; cause <= JETSAM_REASON_MEMORYSTATUS_MAX;
       cause++) {
...
int memorystatus_control(struct proc *p, struct memorystatus_control_args *args,
                         int *ret) {
  int error = EINVAL;
...
#pragma unused(jetsam_reason)
  ...
  /* We don't need entitlements if we're setting / querying the freeze
   * preference or frozen status for a process. */
  ...
  /*
   * On development kernel, we don't need entitlements if we're adjusting the
   * limit. This required for limit adjustment by dyld when roots are detected,
   * see rdar://99669958
   */
#if DEVELOPMENT || DEBUG
  if (args->command == MEMORYSTATUS_CMD_INCREASE_JETSAM_TASK_LIMIT &&
      proc_getpid(p) == args->pid) {
    skip_auth_check = TRUE;
  }
#endif /* DEVELOPMENT || DEBUG */
   ...
   */
  if (args->command == MEMORYSTATUS_CMD_REARM_MEMLIMIT &&
      proc_getpid(p) == args->pid) {
    skip_auth_check = TRUE;
  }
#endif
  ...
  case MEMORYSTATUS_CMD_GET_PRIORITY_LIST:
    error = memorystatus_cmd_get_priority_list(
        args->pid, args->buffer, args->buffersize, ret,
  ...
  case MEMORYSTATUS_CMD_SET_PRIORITY_PROPERTIES:
    error = memorystatus_cmd_set_priority_properties(
        args->pid, args->flags, args->buffer, args->buffersize, ret);
    break;
  case MEMORYSTATUS_CMD_SET_MEMLIMIT_PROPERTIES:
    error = memorystatus_cmd_set_memlimit_properties(args->pid, args->buffer,
                                                     args->buffersize, ret);
    break;
  case MEMORYSTATUS_CMD_GET_MEMLIMIT_PROPERTIES:
    error = memorystatus_cmd_get_memlimit_properties(args->pid, args->buffer,
                                                     args->buffersize, ret);
    break;
  case MEMORYSTATUS_CMD_GET_MEMLIMIT_EXCESS:
    error = memorystatus_cmd_get_memlimit_excess_np(
        args->pid, args->flags, args->buffer, args->buffersize, ret);
    break;
  case MEMORYSTATUS_CMD_GRP_SET_PROPERTIES:
    error = memorystatus_cmd_grp_set_properties(
        (int32_t)args->flags, args->buffer, args->buffersize, ret);
    break;
  case MEMORYSTATUS_CMD_GET_JETSAM_SNAPSHOT:
    error = memorystatus_cmd_get_jetsam_snapshot(
        (int32_t)args->flags, args->buffer, args->buffersize, ret);
    break;
#if JETSAM_ZPRINT_SNAPSHOT
  case MEMORYSTATUS_CMD_GET_JETSAM_ZPRINT_NAMES:
    error = memorystatus_cmd_get_data_buffer(
        args->buffer, args->buffersize, ret,
  ...
  case MEMORYSTATUS_CMD_SET_JETSAM_HIGH_WATER_MARK:
     ...
     */
    error = memorystatus_cmd_set_jetsam_memory_limit(
        args->pid, (int32_t)args->flags, ret, FALSE);
    break;
  case MEMORYSTATUS_CMD_SET_JETSAM_TASK_LIMIT:
     ...
     */
    error = memorystatus_cmd_set_jetsam_memory_limit(
        args->pid, (int32_t)args->flags, ret, TRUE);
    break;
  ...
  case MEMORYSTATUS_CMD_CONVERT_MEMLIMIT_MB:
    error = memorystatus_cmd_convert_memlimit_mb(args->pid,
                                                 (int32_t)args->flags, ret);
    break;
  ...
  case MEMORYSTATUS_CMD_TEST_JETSAM:
    ...
    if (jetsam_reason == OS_REASON_NULL) {
      memorystatus_log_error(
          "memorystatus_control: failed to allocate jetsam reason\n");
    }

    error =
        memstat_kill_process_sync(args->pid, kMemorystatusKilled, jetsam_reason)
            ? 0
            : EINVAL;
    break;
...
#pragma unused(jetsam_reason)
  ...
  case MEMORYSTATUS_CMD_ELEVATED_INACTIVEJETSAMPRIORITY_DISABLE:
    error = memorystatus_update_inactive_jetsam_priority_band(
        args->pid, args->command, JETSAM_PRIORITY_ELEVATED_INACTIVE,
        args->flags ? TRUE : FALSE);
    break;
  ...
  case MEMORYSTATUS_CMD_SET_PROCESS_IS_FREEZABLE:
    error = memorystatus_set_process_is_freezable(args->pid,
                                                  args->flags ? TRUE : FALSE);
    break;
  ...
  case MEMORYSTATUS_CMD_FREEZER_CONTROL:
    error = memorystatus_freezer_control(args->flags, args->buffer,
                                         args->buffersize, ret);
    break;
#endif /* CONFIG_FREEZE */
  ...
  case MEMORYSTATUS_CMD_SET_DIAG_LIMIT:
    error = memorystatus_cmd_set_diag_memlimit_properties(
        args->pid, args->buffer, args->buffersize, ret);
    break;
  case MEMORYSTATUS_CMD_GET_DIAG_LIMIT:
    error = memorystatus_cmd_get_diag_memlimit_properties(
        args->pid, args->buffer, args->buffersize, ret);
    break;
#endif /* DEVELOPMENT || DEBUG */

  case MEMORYSTATUS_CMD_GET_KILL_COUNTS:
    error = memorystatus_cmd_get_kill_counts(args->pid, args->buffer,
                                             args->buffersize, args->flags);
    break;
 ...
 */
static void memstat_insert_list_locked(proc_t before, unsigned int bucket_idx,
                                       pid_t *pid_list, int list_sz) {
  int i;
  proc_t p;
  memstat_bucket_t *bucket;
  ...
  for (i = list_sz - 1; i >= 0; i--) {
    ...
    if (p == NULL || p == before) {
      /*
       * We can encounter p == before when we try to sort a coalition with an
       * in- progress exec of the leader, such that the leader and the exec-ing
...
#define MAX_SORT_PIDS 80
  ...
  assert((sort_order == JETSAM_SORT_LRU) ||
         (sort_order == JETSAM_SORT_FOOTPRINT));
  assert(bucket_index < MEMSTAT_BUCKET_COUNT);
  ...
  while (p) {
    ...
    ntasks =
        coalition_get_pid_list(coal, COALITION_ROLEMASK_XPC,
                               COALITION_SORT_DEFAULT, pid_list, MAX_SORT_PIDS);
...
int memorystatus_update_priority_for_appnap(proc_t p) {
#if !CONFIG_JETSAM
  if (!p || (!isApp(p)) ||
      (p->p_memstat_state & (P_MEMSTAT_INTERNAL | P_MEMSTAT_MANAGED))) {
    /*
  ...
  /*
   * Update priority. We don't want the aging logic because that's only
   * applicable on configs with CONFIG_JETSAM.
   */
  priority =
      proc_get_effective_task_policy(proc_task(p), TASK_POLICY_SUP_ACTIVE)
...
uint64_t memorystatus_available_memory_internal(struct proc *p) {
  ...
  } else if (task_convert_phys_footprint_limit(-1, &memlimit_mb) !=
             KERN_SUCCESS) {
    return 0;
  }
...
int memorystatus_available_memory(
    struct proc *p, __unused struct memorystatus_available_memory_args *args,
    uint64_t *ret) {
...
static int memorystatus_cmd_increase_jetsam_task_limit(pid_t pid,
  ...
  const uint32_t current_memlimit_increase =
      roundToNearestMB(p->p_memlimit_increase);
  /* round to page */
  const int32_t page_aligned_increase = (int32_t)MIN(
      round_page(p->p_memlimit_increase + byte_increase), INT32_MAX);
  ...
  int error = memstat_set_memlimits_locked(p, memlimit_active,
