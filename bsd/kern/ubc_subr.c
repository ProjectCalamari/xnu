...
#undef assert
#endif
#define assert(cond) ((void)((cond) ? 0 : panic("Assert failed: %s", #cond)))
#else
#include <kern/assert.h>
#endif /* DIAGNOSTIC */

static int ubc_info_init_internal(struct vnode *vp, int withfsize,
                                  off_t filesize);
...
static boolean_t cs_valid_range(const void *start, const void *end,
                                const void *lower_bound,
                                const void *upper_bound) {
  if (upper_bound < lower_bound || end < start) {
    return FALSE;
  }
...
uint8_t cs_hash_type(struct cs_hash const *const cs_hash) {
...
static struct cs_hash const *cs_find_md(uint8_t type) {
 ...
 */
static const unsigned char *hashes(const CS_CodeDirectory *cd, uint32_t page,
                                   size_t hash_len, const char *lower_bound,
  ...
  if ((ntohl(cd->version) >= CS_SUPPORTSSCATTER) &&
      (ntohl(cd->scatterOffset))) {
    /* Get first scatter struct */
    const SC_Scatter *scatter =
        (const SC_Scatter *)((const char *)cd + ntohl(cd->scatterOffset));
  ...
  if (!cs_valid_range(hash, hash + hash_len, lower_bound, upper_bound)) {
    hash = NULL;
  }
...
static int cs_validate_codedirectory(const CS_CodeDirectory *cd,
                                     size_t length) {
  ...
  if ((length - ntohl(cd->hashOffset)) / hashtype->cs_size <
      ntohl(cd->nCodeSlots)) {
    return EBADEXEC;
  }
  ...
  if (ntohl(cd->version) >= CS_SUPPORTSSCATTER && cd->scatterOffset) {
    ...
    const SC_Scatter *scatter =
        (const SC_Scatter *)(((const uint8_t *)cd) + ntohl(cd->scatterOffset));
    uint32_t nPages = 0;
    ...
    while (1) {
      ...
      if (((const uint8_t *)scatter) + sizeof(scatter[0]) >
          (const uint8_t *)cd + length) {
        return EBADEXEC;
      }
  ...
  if (ntohl(cd->version) >= CS_SUPPORTSLINKAGE && cd->linkageHashType != 0) {
    ...
    if (ptr_end < ptr || ptr < (uintptr_t)cd ||
        ptr_end > (uintptr_t)cd + length) {
      return EBADEXEC;
    }
  }
...
static int cs_validate_blob(const CS_GenericBlob *blob, size_t length) {
...
static int cs_validate_csblob(const uint8_t *addr, const size_t blob_size,
  ...
  *rder_entitlements = NULL;
  ...
  if (ntohl(blob->magic) == CSMAGIC_EMBEDDED_SIGNATURE) {
    ...
    if ((error = cs_validate_codedirectory(
             (const CS_CodeDirectory *)(const void *)addr, length)) != 0) {
      return error;
    }
...
const CS_GenericBlob *csblob_find_blob_bytes(const uint8_t *addr, size_t length,
  ...
  if (ntohl(blob->magic) == CSMAGIC_EMBEDDED_SIGNATURE) {
    ...
    for (n = 0; n < count; n++) {
      ...
      } else if (((vm_address_t)blob + ntohl(blob->length)) >
                 (vm_address_t)(addr + length)) {
        continue;
      }
      return blob;
    }
    ...
    if (((vm_address_t)blob + ntohl(blob->length)) < (vm_address_t)blob) {
      panic("CODE SIGNING: CS Blob length overflow for code directory blob at: "
            "%p",
            blob);
    } else if (((vm_address_t)blob + ntohl(blob->length)) >
               (vm_address_t)(addr + length)) {
...
const CS_GenericBlob *csblob_find_blob(struct cs_blob *csblob, uint32_t type,
                                       uint32_t magic) {
  if ((csblob->csb_flags & CS_VALID) == 0) {
    return NULL;
  }
  return csblob_find_blob_bytes((const uint8_t *)csblob->csb_mem_kaddr,
                                csblob->csb_mem_size, type, magic);
}

static const uint8_t *find_special_slot(const CS_CodeDirectory *cd,
                                        size_t slotsize, uint32_t slot) {
...
static int csblob_find_special_slot_blob(struct cs_blob *csblob, uint32_t slot,
                                         uint32_t magic,
                                         const CS_GenericBlob **out_start,
                                         size_t *out_length) {
  ...
  blob = csblob_find_blob_bytes((const uint8_t *)csblob->csb_mem_kaddr,
                                csblob->csb_mem_size, slot, magic);

  embedded_hash =
      find_special_slot(code_dir, csblob->csb_hashtype->cs_size, slot);
  ...
  if (embedded_hash == NULL) {
    ...
    if (memcmp(embedded_hash, cshash_zero, csblob->csb_hashtype->cs_size) !=
        0) {
      return EBADEXEC;
    } else {
      return 0;
  ...
  if (memcmp(computed_hash, embedded_hash, csblob->csb_hashtype->cs_size) !=
      0) {
    return EBADEXEC;
  }
  if (out_start) {
...
int csblob_get_entitlements(struct cs_blob *csblob, void **out_start,
                            size_t *out_length) {
  uint8_t computed_hash[CS_HASH_MAX_SIZE];
  ...
  *out_length = 0;
  ...
  embedded_hash = find_special_slot(code_dir, csblob->csb_hashtype->cs_size,
                                    CSSLOT_ENTITLEMENTS);
  ...
  csblob->csb_hashtype->cs_init(&context);
  csblob->csb_hashtype->cs_update(&context, entitlements,
...
int csblob_get_der_entitlements(struct cs_blob *csblob,
                                const CS_GenericBlob **out_start,
                                size_t *out_length) {
  uint8_t computed_hash[CS_HASH_MAX_SIZE];
  ...
  *out_length = 0;
  ...
  csblob->csb_hashtype->cs_init(&context);
  csblob->csb_hashtype->cs_update(&context, der_entitlements,
                                  ntohl(der_entitlements->length));
  csblob->csb_hashtype->cs_final(computed_hash, &context);
...
static bool ubc_cs_blob_pagewise_allocate(__unused vm_size_t size) {
  ...
  /*
   * Small allocations can be maanged by the monitor itself. We only need to
   * allocate page-wise when it is a sufficiently large allocation and the
   * monitor cannot manage it on its own.
   */
...
int csblob_register_profile(__unused struct cs_blob *csblob,
  ...
  kern_return_t kr = csm_register_provisioning_profile(
      profile->uuid, profile->data, profile->size);
  ...
  kr = csm_trust_provisioning_profile(profile->uuid, profile->sig_data,
                                      profile->sig_size);
  ...
  kr = csm_associate_provisioning_profile(csblob->csb_csm_obj, profile->uuid);
...
int csblob_register_profile_uuid(struct cs_blob *csblob,
                                 const uuid_t profile_uuid, void *profile_addr,
                                 vm_size_t profile_size) {
  cs_profile_register_t profile = {.sig_data = NULL,
                                   .sig_size = 0,
                                   .data = profile_addr,
                                   .size = profile_size};
 ...
 *
 */
int ubc_info_init(struct vnode *vp) { return ubc_info_init_internal(vp, 0, 0); }
 ...
 *	vnode_size:???			Other error from vnode_getattr
 */
int ubc_info_init_withsize(struct vnode *vp, off_t filesize) {
 ...
 */
static int ubc_info_init_internal(vnode_t vp, int withfsize, off_t filesize) {
  struct ubc_info *uip;
  void *pager;
  int error = 0;
  kern_return_t kret;
   ...
   */
  kret = memory_object_create_named(pager, (memory_object_size_t)uip->ui_size,
                                    &control);
 ...
 */
static void ubc_info_free(struct ubc_info *uip) {
...
void ubc_info_deallocate(struct ubc_info *uip) { ubc_info_free(uip); }
 ...
 *          EAGAIN if UBC_SETSIZE_NO_FS_REENTRY option is set and new_size < old
 * size Other errors (mapped to errno_t) returned by VM functions
 *
...
errno_t ubc_setsize_ex(struct vnode *vp, off_t nsize, ubc_setsize_opts_t opts) {
  ...
  if (lastoff) {
     ...
     */
    kret = ubc_create_upl_kernel(vp, lastpg, PAGE_SIZE, &upl, &pl,
  ...
  if (olastpgend > lastpg) {
     ...
     *
     */
    kret = memory_object_lock_request(
        control, (memory_object_offset_t)lastpg,
 ...
 */
static int ubc_umcallback(vnode_t vp, __unused void *args) {
  if (UBCINFOEXISTS(vp)) {
 ...
 */
kauth_cred_t ubc_getcred(struct vnode *vp) {
  if (UBCINFOEXISTS(vp)) {
 ...
 *		server.
 */
int ubc_setthreadcred(struct vnode *vp, proc_t p, thread_t thread) {
 ...
 */
int ubc_setcred(struct vnode *vp, kauth_cred_t ucred) {
  struct ubc_info *uip;
 ...
 */
__private_extern__ memory_object_t ubc_getpager(struct vnode *vp) {
  if (UBCINFOEXISTS(vp)) {
 ...
 *		another thread.
 */
memory_object_control_t ubc_getobject(struct vnode *vp, __unused int flags) {
  if (UBCINFOEXISTS(vp)) {
 ...
 */
off_t ubc_blktooff(vnode_t vp, daddr64_t blkno) {
  off_t file_offset = -1;
  int error;
 ...
 *		here instead.
 */
daddr64_t ubc_offtoblk(vnode_t vp, off_t offset) {
  daddr64_t blkno = -1;
  int error = 0;
 ...
 *		0			No
 */
int ubc_pages_resident(vnode_t vp) {
  kern_return_t kret;
   ...
   */
  kret = memory_object_pages_resident(vp->v_ubcinfo->ui_control,
                                      &has_pages_resident);
 ...
 *
 */
errno_t ubc_msync(vnode_t vp, off_t beg_off, off_t end_off, off_t *resid_off,
                  int flags) {
  int retval;
  int io_errno = 0;
  ...
  retval =
      ubc_msync_internal(vp, beg_off, end_off, resid_off, flags, &io_errno);
 ...
 */
static int ubc_msync_internal(vnode_t vp, off_t beg_off, off_t end_off,
                              off_t *resid_off, int flags, int *io_errno) {
  ...
  kret =
      memory_object_lock_request(vp->v_ubcinfo->ui_control, beg_off, tsize,
                                 (memory_object_offset_t *)resid_off, io_errno,
 ...
 *		address space.
 */
__private_extern__ int ubc_map(vnode_t vp, int flags) {
  struct ubc_info *uip;
  int error = 0;
  int need_ref = 0;
  int need_wakeup = 0;
  ...
  if (UBCINFOEXISTS(vp)) {
    ...
    while (ISSET(uip->ui_flags, UI_MAPBUSY)) {
      ...
      (void)msleep(&uip->ui_flags, &vp->v_lock, PRIBIO, "ubc_map", NULL);
    }
    ...
    if (need_ref) {
       ...
       */
      if (vp->v_mount != NULL && ((vp->v_mount->mnt_flag & MNT_ROOTFS) ||
        ...
        //				printf("BUGGYCOW: %s:%d vp %p \"%s\" in
        //mnt %p \"%s\" is untrusted\n", __FUNCTION__, __LINE__, vp, vp->v_name,
        //vp->v_mount, vp->v_mount->mnt_vnodecovered->v_name);
      }
    }
  }
  return error;
 ...
 */
__private_extern__ void ubc_destroy_named(vnode_t vp,
 ...
 *		function will report that the vnode is not in use by ubc.
 */
int ubc_isinuse_locked(struct vnode *vp, int busycount, int locked) {
  int retval = 0;
 ...
 */
__private_extern__ void ubc_unmap(struct vnode *vp) {
  struct ubc_info *uip;
  int need_rele = 0;
  int need_wakeup = 0;
  ...
  if (UBCINFOEXISTS(vp)) {
    ...
    while (ISSET(uip->ui_flags, UI_MAPBUSY)) {
      SET(uip->ui_flags, UI_MAPWAITING);
      (void)msleep(&uip->ui_flags, &vp->v_lock, PRIBIO, "ubc_unmap", NULL);
    }
    SET(uip->ui_flags, UI_MAPBUSY);
    ...
    if (need_rele) {
      ...
      if (want_fsevent && need_fsevent(FSE_CONTENT_MODIFIED, vp)) {
        add_fsevent(FSE_CONTENT_MODIFIED_NO_HLINK, ctx, FSE_ARG_VNODE, vp,
                    FSE_ARG_DONE);
      }
#endif
 ...
 */
kern_return_t ubc_page_op(struct vnode *vp, off_t f_offset, int ops,
                          ppnum_t *phys_entryp, int *flagsp) {
  memory_object_control_t control;
  ...
  return memory_object_page_op(control, (memory_object_offset_t)f_offset, ops,
                               phys_entryp, flagsp);
}
 ...
 *		is returned.
 */
kern_return_t ubc_range_op(struct vnode *vp, off_t f_offset_beg,
                           off_t f_offset_end, int ops, int *range) {
  memory_object_control_t control;
  ...
  return memory_object_range_op(control, (memory_object_offset_t)f_offset_beg,
                                (memory_object_offset_t)f_offset_end, ops,
                                range);
}
 ...
 */
kern_return_t ubc_create_upl_external(struct vnode *vp, off_t f_offset,
                                      int bufsize, upl_t *uplp,
                                      upl_page_info_t **plp, int uplflags) {
  return ubc_create_upl_kernel(vp, f_offset, bufsize, uplp, plp, uplflags,
                               vm_tag_bt());
}

kern_return_t ubc_create_upl_kernel(struct vnode *vp, off_t f_offset,
                                    int bufsize, upl_t *uplp,
                                    upl_page_info_t **plp, int uplflags,
                                    vm_tag_t tag) {
  memory_object_control_t control;
  ...
  *uplp = NULL;
  ...
  if (uplflags & (UPL_UBC_MSYNC | UPL_UBC_PAGEOUT | UPL_UBC_PAGEIN)) {
    if (uplflags & UPL_UBC_MSYNC) {
      ...
      uplflags |= UPL_COPYOUT_FROM | UPL_CLEAN_IN_PLACE | UPL_SET_INTERNAL |
                  UPL_SET_LITE;
      ...
      uplflags |= UPL_FOR_PAGEOUT | UPL_CLEAN_IN_PLACE | UPL_COPYOUT_FROM |
                  UPL_SET_INTERNAL | UPL_SET_LITE;
    } else {
      uplflags |= UPL_RET_ONLY_ABSENT | UPL_NO_SYNC | UPL_CLEAN_IN_PLACE |
                  UPL_SET_INTERNAL | UPL_SET_LITE;
  ...
  kr = memory_object_upl_request(control, f_offset, bufsize, uplp, NULL, NULL,
                                 uplflags, tag);
 ...
 */
upl_size_t ubc_upl_maxbufsize(void) { return MAX_UPL_SIZE_BYTES; }
 ...
 */
kern_return_t ubc_upl_map(upl_t upl, vm_offset_t *dst_addr) {
 ...
 */
kern_return_t ubc_upl_map_range(upl_t upl, vm_offset_t offset_to_map,
                                vm_size_t size_to_map, vm_prot_t prot_to_map,
                                vm_offset_t *dst_addr) {
  return vm_upl_map_range(kernel_map, upl, offset_to_map, size_to_map,
                          prot_to_map, dst_addr);
}
 ...
 */
kern_return_t ubc_upl_unmap(upl_t upl) { return vm_upl_unmap(kernel_map, upl); }
 ...
 *
 * Note: Currrently offset & size are unused as we always initiate the unmap
 * from the very beginning of the UPL's mapping and track the mapped size in the
 * UPL. But we might want to allow unmapping a UPL in the middle, for example,
 * and we can use the offset + size parameters for that purpose.
 */
kern_return_t ubc_upl_unmap_range(upl_t upl, vm_offset_t offset_to_unmap,
 ...
 *		by this function.
 */
kern_return_t ubc_upl_commit(upl_t upl) {
  upl_page_info_t *pl;
  kern_return_t kr;
 ...
 */
kern_return_t ubc_upl_commit_range(upl_t upl, upl_offset_t offset,
                                   upl_size_t size, int flags) {
  upl_page_info_t *pl;
  boolean_t empty;
  kern_return_t kr;
  ...
  kr = upl_commit_range(upl, offset, size, flags, pl,
                        MAX_UPL_SIZE_BYTES >> PAGE_SHIFT, &empty);
 ...
 */
kern_return_t ubc_upl_abort_range(upl_t upl, upl_offset_t offset,
                                  upl_size_t size, int abort_flags) {
  kern_return_t kr;
 ...
 *		referenced; do not use it.
 */
kern_return_t ubc_upl_abort(upl_t upl, int abort_type) {
  kern_return_t kr;
 ...
 */
upl_page_info_t *ubc_upl_pageinfo(upl_t upl) {
  return UPL_GET_INTERNAL_PAGE_LIST(upl);
}

int UBCINFOEXISTS(const struct vnode *vp) {
  return (vp) && ((vp)->v_type == VREG) && ((vp)->v_ubcinfo != UBC_INFO_NULL);
}

void ubc_upl_range_needed(upl_t upl, int index, int count) {
...
boolean_t ubc_was_mapped(const struct vnode *vp, boolean_t *writable) {
...
boolean_t ubc_was_mapped_writable(const struct vnode *vp) {
  boolean_t writable;
...
SYSCTL_UINT(_vm, OID_AUTO, cs_blob_count, CTLFLAG_RD | CTLFLAG_LOCKED,
            &cs_blob_count, 0, "Current number of code signature blobs");
SYSCTL_ULONG(_vm, OID_AUTO, cs_blob_size, CTLFLAG_RD | CTLFLAG_LOCKED,
             &cs_blob_size, "Current size of all code signature blobs");
SYSCTL_UINT(_vm, OID_AUTO, cs_blob_count_peak, CTLFLAG_RD | CTLFLAG_LOCKED,
            &cs_blob_count_peak, 0, "Peak number of code signature blobs");
SYSCTL_ULONG(_vm, OID_AUTO, cs_blob_size_peak, CTLFLAG_RD | CTLFLAG_LOCKED,
             &cs_blob_size_peak, "Peak size of code signature blobs");
SYSCTL_ULONG(_vm, OID_AUTO, cs_blob_size_max, CTLFLAG_RD | CTLFLAG_LOCKED,
             &cs_blob_size_max, "Size of biggest code signature blob");
...
kern_return_t ubc_cs_blob_allocate(vm_offset_t *blob_addr_p,
  ...
  if (ubc_cs_blob_pagewise_allocate(allocation_size) == true) {
    ...
    /* Allocate page-wise */
    kr = kmem_alloc(kernel_map, blob_addr_p, allocation_size,
...
void ubc_cs_blob_deallocate(vm_offset_t blob_addr, vm_size_t blob_size) {
 ...
 */
static errno_t
ubc_cs_blob_reconstruct(struct cs_blob *cs_blob,
 ...
 */
static errno_t ubc_cs_reconstitute_code_signature(
    const struct cs_blob *const blob, vm_address_t *const ret_mem_kaddr,
    vm_size_t *const ret_mem_size, vm_size_t code_directory_size,
  ...
  /*
   * A code signature can contain multiple code directories, each of which
   * contains hashes of pages based on a hashing algorithm. The kernel selects
   * which hashing algorithm is the strongest, and consequently, marks one of
   * these code directories as the best matched one. More often than not, the
   * best matched one is _not_ the first one.
   *
   * However, the CMS blob which cryptographically verifies the code signature
   * is only signed against the first code directory. Therefore, if the CMS blob
   * is present, we also need the first code directory to be able to verify it.
   * Given this, we organize the new cs_blob as following order:
   *
   ...
   * 9. first code directory (if not already the best match, and if cms blob is
   * present)
   *
   * This order is chosen deliberately, as later on, we expect to get rid of the
   * CMS blob and the first code directory once their verification is complete.
   */
  ...
  launch_constraint_self = csblob_find_blob_bytes(
      (const uint8_t *)blob->csb_mem_kaddr, blob->csb_mem_size,
  ...
  cms_blob = csblob_find_blob_bytes((const uint8_t *)blob->csb_mem_kaddr,
                                    blob->csb_mem_size, CSSLOT_SIGNATURESLOT,
                                    CSMAGIC_BLOBWRAPPER);
  if (cms_blob) {
    new_blob_size += sizeof(CS_BlobIndex);
   ...
   * This is only needed if a CMS blob exists and the best code directory isn't
   * already the first one. It is an error if we find a CMS blob but do not find
   * a first code directory.
   */
  if (cms_blob) {
    first_code_directory = csblob_find_blob_bytes(
        (const uint8_t *)blob->csb_mem_kaddr, blob->csb_mem_size,
        CSSLOT_CODEDIRECTORY, CSMAGIC_CODEDIRECTORY);
    if (first_code_directory == best_code_directory) {
      /* We don't need the first code directory anymore, since the best one is
       * already it */
      ...
      printf("CODE SIGNING: Invalid CS Blob: found CMS blob but not a first "
             "code directory\n");
      return EINVAL;
    }
  }
  ...
  if (ret != KERN_SUCCESS) {
    printf("CODE SIGNING: Failed to allocate memory for new code signing blob: "
           "%d\n",
           ret);
    return ENOMEM;
  }
  ...
  if (code_directory_size > 0) {
    ...
    memcpy((void *)(new_blob_addr + blob_offset), best_code_directory,
           new_code_directory_size);
  }
  ...
  if (der_entitlements_blob) {
    ...
    memcpy((void *)(new_blob_addr + blob_offset), der_entitlements_blob,
           ntohl(der_entitlements_blob->length));
  ...
  if (launch_constraint_self) {
    ...
    memcpy((void *)(new_blob_addr + blob_offset), launch_constraint_self,
  ...
  if (launch_constraint_parent) {
    ...
    memcpy((void *)(new_blob_addr + blob_offset), launch_constraint_parent,
  ...
  if (launch_constraint_responsible) {
    blob_index += 1;
    superblob->index[blob_index].offset = htonl(blob_offset);
    superblob->index[blob_index].type =
        htonl(CSSLOT_LAUNCH_CONSTRAINT_RESPONSIBLE);

    memcpy((void *)(new_blob_addr + blob_offset), launch_constraint_responsible,
  ...
  if (library_constraint) {
    ...
    memcpy((void *)(new_blob_addr + blob_offset), library_constraint,
  ...
  if (entitlements_blob) {
    ...
    memcpy((void *)(new_blob_addr + blob_offset), entitlements_blob,
           ntohl(entitlements_blob->length));
  ...
  if (cms_blob) {
    ...
    memcpy((void *)(new_blob_addr + blob_offset), cms_blob,
           ntohl(cms_blob->length));
  ...
  if (first_code_directory) {
    ...
    superblob->index[blob_index].offset = htonl(blob_offset);
    superblob->index[blob_index].type = htonl(CSSLOT_CODEDIRECTORY);
    memcpy((void *)(new_blob_addr + blob_offset), first_code_directory,
           ntohl(first_code_directory->length));
    blob_offset += ntohl(first_code_directory->length);
  }

  /*
   * We only validate the blob in case we copied in the best code directory.
   * In case the code directory size we were passed in wasn't 0, we memset the
   * best code directory to 0 and expect the caller to fill it in. In the same
   * spirit, we expect the caller to validate the code signature after they fill
  ...
  if (code_directory_size == 0) {
    ...
    ret = cs_validate_csblob(
        (const uint8_t *)superblob, new_blob_size, &validated_code_directory,
        &validated_entitlements_blob, &validated_der_entitlements_blob);
 ...
 */
static errno_t
ubc_cs_reconstitute_code_signature_2nd_stage(struct cs_blob *blob) {
  ...
  num_blobs = 1;
  last_needed_blob_offset =
      ntohl(superblob->index[0].offset) + ntohl(blob->csb_cd->length);
  ...
  /*
   * There is a chance that the code directory is marked within the superblob as
   * an alternate code directory. This happens when the first code directory
   * isn't the best one chosen by the kernel, so to be able to access both the
   * first and the best, we save the best one as an alternate one. Since we're
   * getting rid of the first one here, we mark the best one as the first one.
   */
  ...
  code_directory_offset =
      (vm_offset_t)blob->csb_cd - (vm_offset_t)blob->csb_mem_kaddr;
  ...
  ubc_cs_blob_reconstruct(blob, new_superblob, new_superblob_size,
                          code_directory_offset);
  ...
  ret = cs_validate_csblob((const uint8_t *)blob->csb_mem_kaddr,
                           blob->csb_mem_size, &validated_code_directory,
                           &validated_entitlements_blob,
                           &validated_der_entitlements_blob);
  if (ret) {
    printf("unable to validate code signature after 2nd stage reconstitution: "
           "%d\n",
           ret);
    return EINVAL;
  }
...
static int ubc_cs_convert_to_multilevel_hash(struct cs_blob *blob) {
  ...
  uint32_t hashes_per_new_hash_shift =
      (uint32_t)(PAGE_SHIFT - blob->csb_hash_pageshift);

  if (cs_debug > 1) {
    printf("CODE SIGNING: Attempting to convert Code Directory for %lu -> %lu "
           "page shift\n",
  ...
  new_cdsize += (ntohl(old_cd->nCodeSlots) >> hashes_per_new_hash_shift) *
                old_cd->hashSize;

  error = ubc_cs_reconstitute_code_signature(
      blob, &new_blob_addr, &new_blob_size, new_cdsize, &new_cd);
  if (error != 0) {
    printf("CODE SIGNING: Failed to reconsitute code signature: %d\n", error);
    return error;
  }
  entitlements = csblob_find_blob_bytes((uint8_t *)new_blob_addr, new_blob_size,
                                        CSSLOT_ENTITLEMENTS,
                                        CSMAGIC_EMBEDDED_ENTITLEMENTS);
  der_entitlements = csblob_find_blob_bytes(
      (uint8_t *)new_blob_addr, new_blob_size, CSSLOT_DER_ENTITLEMENTS,
      CSMAGIC_EMBEDDED_DER_ENTITLEMENTS);
  ...
  if ((ntohl(new_cd->version) >= CS_SUPPORTSSCATTER) &&
      (ntohl(new_cd->scatterOffset))) {
    SC_Scatter *scatter =
        (SC_Scatter *)((char *)new_cd + ntohl(new_cd->scatterOffset));
  ...
  const unsigned char *src_base =
      (const unsigned char *)old_cd + ntohl(old_cd->hashOffset);
  ...
  error = cs_validate_csblob((const uint8_t *)new_blob_addr, new_blob_size, &cd,
                             &entitlements, &der_entitlements);
  if (error != 0) {
  ...
  blob->csb_der_entitlements_blob =
      der_entitlements; /* may be NULL, not yet validated */
  blob->csb_reconstituted = true;

  /* The blob has some cached attributes of the Code Directory, so update those
   */

  blob->csb_hash_firstlevel_pageshift =
      blob->csb_hash_pageshift; /* Save the original page size */
...
static void cs_blob_cleanup(struct cs_blob *blob) {
  ...
  if (blob->csb_csm_obj != NULL) {
    ...
    if (kr == KERN_SUCCESS) {
      /*
       * If the code signature was monitor managed, the monitor will have freed
       * it itself in the unregistration call. It means we do not need to free
       * the data over here.
       */
      ...
      /*
       * The code-signing-monitor refused to unregister the code signature. It
       * means whatever memory was backing the code signature may not have been
       * released, and attempting to free it down below will not be successful.
       * As a result, all we can do is prevent the kernel from touching the
       * data.
       */
      blob->csb_mem_kaddr = NULL;
      blob->csb_mem_size = 0;
  ...
  if (blob->csb_mem_kaddr) {
    ubc_cs_blob_deallocate((vm_offset_t)blob->csb_mem_kaddr,
                           blob->csb_mem_size);
  }
  blob->csb_mem_kaddr = NULL;
  blob->csb_mem_size = 0;
...
static void cs_blob_ro_free(struct cs_blob *blob) {
  struct cs_blob tmp;
 ...
 */
void cs_blob_free(struct cs_blob *blob) { cs_blob_ro_free(blob); }

static int cs_blob_init_validated(vm_address_t *const addr, vm_size_t size,
                                  struct cs_blob *blob,
  ...
  *addr = 0;
   ...
   */
  length = (size_t)size;
  error = cs_validate_csblob((const uint8_t *)blob->csb_mem_kaddr, length, &cd,
                             &entitlements, &der_entitlements);
  if (error) {
    if (cs_debug) {
    ...
    blob->csb_cd = cd;
    blob->csb_entitlements_blob =
        entitlements; /* may be NULL, not yet validated */
    blob->csb_der_entitlements_blob =
        der_entitlements; /* may be NULL, not yet validated */
    blob->csb_hashtype = cs_find_md(cd->hashType);
    if (blob->csb_hashtype == NULL ||
        blob->csb_hashtype->cs_digest_size > sizeof(hash)) {
    ...
    blob->csb_end_offset =
        (((vm_offset_t)ntohl(cd->codeLimit) + hash_pagemask) & ~hash_pagemask);
    if ((ntohl(cd->version) >= CS_SUPPORTSSCATTER) &&
        (ntohl(cd->scatterOffset))) {
      const SC_Scatter *scatter =
          (const SC_Scatter *)((const char *)cd + ntohl(cd->scatterOffset));
      blob->csb_start_offset =
          ((off_t)ntohl(scatter->base)) * (1U << blob->csb_hash_pageshift);
    } else {
      blob->csb_start_offset = 0;
    }
    ...
    if (ntohl(cd->version) >= CS_SUPPORTSLINKAGE && cd->linkageHashType != 0 &&
      ...
      if (blob->csb_linkage_hashtype != NULL) {
        memcpy(blob->csb_linkage,
               (uint8_t const *)cd + ntohl(cd->linkageOffset), CS_CDHASH_LEN);
      }
    }
#endif
  }
 ...
 */
int cs_blob_create_validated(vm_address_t *const addr, vm_size_t size,
...
#if CONFIG_SUPPLEMENTAL_SIGNATURES
static void cs_blob_supplement_free(struct cs_blob *const blob) {
  void *teamid;
...
static void ubc_cs_blob_adjust_statistics(struct cs_blob const *blob) {
...
static void cs_blob_set_cpu_type(struct cs_blob *blob, cpu_type_t cputype) {
  zalloc_ro_update_field(ZONE_ID_CS_BLOB, blob, csb_cpu_type, &cputype);
}

__abortlike static void panic_cs_blob_backref_mismatch(struct cs_blob *blob,
                                                       struct vnode *vp) {
  panic("cs_blob vnode backref mismatch: blob=%p, vp=%p, "
        "blob->csb_vnode=%p",
        blob, vp, blob->csb_vnode);
}

void cs_blob_require(struct cs_blob *blob, vnode_t vp) {
...
/**
 * Independently verify the authenticity of the code signature through the
 * monitor environment. This is required as otherwise the monitor won't allow
 * associations of the code signature with address spaces.
 *
 * Once we've verified the code signature, we no longer need to keep around any
 * provisioning profiles we may have registered with it. AMFI associates
 * profiles with the monitor during its validation (which happens before the
 * monitor's).
 */
static errno_t verify_code_signature_monitor(struct cs_blob *cs_blob) {
  kern_return_t ret = KERN_DENIED;

  ret = csm_verify_code_signature(cs_blob->csb_csm_obj,
                                  &cs_blob->csb_csm_trust_level);
  ...
  ret = csm_associate_os_entitlements(cs_blob->csb_csm_obj,
 ...
 * will be allocated and managed directory by the monitor, or because the
 * monitor will lockdown the memory associated with the blob.
 */
static errno_t
register_code_signature_monitor(struct vnode *vnode, struct cs_blob *cs_blob,
  ...
  ret = csm_register_code_signature(
      (vm_address_t)cs_blob->csb_mem_kaddr, cs_blob->csb_mem_size,
      code_directory_offset, vnode_path_ptr, &monitor_sig_object,
  ...
  if (ret == KERN_SUCCESS) {
    ...
    if (monitor_signature_addr != (vm_address_t)cs_blob->csb_mem_kaddr) {
      ...
      ubc_cs_blob_deallocate((vm_address_t)cs_blob->csb_mem_kaddr,
                             cs_blob->csb_mem_size);

      /* Reconstruct critical fields in the blob object */
      ubc_cs_blob_reconstruct(cs_blob, monitor_signature_addr,
                              monitor_signature_size, code_directory_offset);
...
static errno_t
validate_main_binary_check(struct cs_blob *csblob,
  ...
  first_cd = (const CS_CodeDirectory *)csblob_find_blob(csblob, slot,
  ...
  for (uint32_t i = 0; i < CSSLOT_ALTERNATE_CODEDIRECTORY_MAX; i++) {
    ...
    alt_cd = (const CS_CodeDirectory *)csblob_find_blob(csblob, slot,
                                                        CSMAGIC_CODEDIRECTORY);
 ...
 * no longer change.
 */
static errno_t accelerate_entitlement_queries(struct cs_blob *cs_blob) {
  ...
  if (ret == KERN_SUCCESS) {
    ...
    ret = amfi->OSEntitlements.adjustContextWithMonitor(
        cs_blob->csb_entitlements, ce_ctx, cs_blob->csb_csm_obj, signing_id,
        cs_blob->csb_flags);
    if (ret != KERN_SUCCESS) {
  ...
  ret = amfi->OSEntitlements.adjustContextWithoutMonitor(
      cs_blob->csb_entitlements, cs_blob);
 ...
 */
static errno_t validate_auxiliary_signed_blobs(struct cs_blob *cs_blob) {
  ...
  for (uint32_t i = 0; i < num_identifiers; i++) {
    errno_t err = csblob_find_special_slot_blob(
        cs_blob, identifiers[i].cs_slot, identifiers[i].cs_magic, NULL, NULL);

    if (err != 0) {
      printf("unable to validate security-critical blob: %d [%u|%u]\n", err,
             identifiers[i].cs_slot, identifiers[i].cs_magic);
...
/**
 * Setup multi-level hashing for the code signature. This isn't supported on
 * most shipping devices, but on ones where it is, it can result in significant
 * savings of memory from the code signature standpoint.
 *
 * Multi-level hashing is used to condense the code directory hashes in order to
 * improve memory consumption. We take four 4K page hashes, and condense them
 * into a single 16K hash, hence reducing the space consumed by the code
 * directory by about ~75%.
 */
static errno_t setup_multilevel_hashing(struct cs_blob *cs_blob) {
   ...
   *
   * Even when the code signing monitor isn't explicitly enabled, there are
   * other reasons for not performing multi-level hashing. For instance, Rosetta
   * creates issues with multi-level hashing on Apple Silicon Macs.
   */
...
/**
 * Once code signature validation is complete, we can remove even more blobs
 * from the code signature as they are no longer needed. This goes on to
 * conserve even more system memory.
 */
static errno_t reconstitute_code_signature_2nd_stage(struct cs_blob *cs_blob) {
  kern_return_t ret = KERN_NOT_SUPPORTED;
  errno_t err = EPERM;
...
#if CODE_SIGNING_MONITOR
  /*
   * When we have a code signing monitor, the code signature is immutable until
   * the monitor decides to unlock parts of it. Therefore, 2nd stage
   * reconstitution takes place in the monitor when we have a monitor available.
   *
   * If the monitor isn't enforcing code signing (in which case the code
   * signature is NOT immutable), then we perform 2nd stage reconstitution
   * within the kernel itself.
   */
  vm_address_t unneeded_addr = 0;
  vm_size_t unneeded_size = 0;

  ret = csm_reconstitute_code_signature(cs_blob->csb_csm_obj, &unneeded_addr,
                                        &unneeded_size);
...
success:
  /*
   * Regardless of whether we are performing 2nd stage reconstitution in the
   * monitor or in the kernel, we remove references to XML entitlements from the
   * blob here. None of the 2nd stage reconstitution code ever keeps these
   * around, and they have been explicitly deprecated and disallowed.
   */
...
/**
 * A code signature blob often contains blob which aren't needed in the kernel.
 * Since the code signature is wired into kernel memory for the time it is used,
 * it behooves us to remove any blobs we have no need for in order to conserve
 * memory.
 *
 * Some platforms support copying the entire SuperBlob stored in kernel memory
 * into userspace memory through the "csops" system call. There is an
 * expectation that when this happens, all the blobs which were a part of the
 * code signature are copied in to userspace memory. As a result, these
 * platforms cannot reconstitute the code signature since, or rather, these
 * platforms cannot remove blobs from the signature, thereby making
 * reconstitution useless.
 */
static errno_t reconstitute_code_signature(struct cs_blob *cs_blob) {
  ...
  /*
   * We can skip reconstitution if the code signing monitor isn't available or
   * not enabled. But if we do have a monitor, then reconsitution becomes
   * required, as there is an expectation of performing 2nd stage reconstitution
  ...
  errno_t err = ubc_cs_reconstitute_code_signature(
      cs_blob, &signature_addr, &signature_size, 0, &code_directory);
  ...
  /* Reconstruct critical fields in the blob object */
  ubc_cs_blob_reconstruct(cs_blob, signature_addr, signature_size,
                          code_directory_offset);
...
int ubc_cs_blob_add(struct vnode *vp, uint32_t platform, cpu_type_t cputype,
                    cpu_subtype_t cpusubtype, off_t base_offset,
                    vm_address_t *addr, vm_size_t size,
                    struct image_params *imgp, __unused int flags,
  ...
  signed_monitor_obj = ptrauth_sign_unauthenticated(
      tmp_blob.csb_csm_obj, ptrauth_key_process_independent_data,
  ...
  error = mac_vnode_check_signature(vp, &tmp_blob, imgp, &cs_flags,
                                    &signer_type, flags, platform);
  ...
  signed_entitlements = ptrauth_sign_unauthenticated(
      tmp_blob.csb_entitlements, ptrauth_key_process_independent_data,
      ptrauth_blend_discriminator(
          &blob_ro->csb_entitlements,
  ...
  for (oblob = ubc_get_cs_blobs(vp); oblob != NULL; oblob = oblob->csb_next) {
    off_t oblob_start_offset, oblob_end_offset;

    if (tmp_blob.csb_signer_type !=
        oblob->csb_signer_type) { // signer type needs to be the same for slices
      vnode_unlock(vp);
      error = EALREADY;
      goto out;
    } else if (tmp_blob.csb_platform_binary) { // platform binary needs to be
                                               // the same for app slices
      if (!oblob->csb_platform_binary) {
        vnode_unlock(vp);
        error = EALREADY;
        goto out;
      }
    } else if (tmp_blob.csb_teamid) { // teamid binary needs to be the same for
                                      // app slices
      if (oblob->csb_platform_binary || oblob->csb_teamid == NULL ||
    ...
    oblob_start_offset = (oblob->csb_base_offset + oblob->csb_start_offset);
    oblob_end_offset = (oblob->csb_base_offset + oblob->csb_end_offset);
    if (blob_start_offset >= oblob_end_offset ||
      ...
      if (blob_start_offset == oblob_start_offset &&
          ...
          !bcmp(tmp_blob.csb_cdhash, oblob->csb_cdhash, CS_CDHASH_LEN)) {
        /*
  ...
  ptrauth_utils_auth_blob_generic(
      &tmp_blob, sizeof(tmp_blob),
      OS_PTRAUTH_DISCRIMINATOR("ubc_cs_blob_add.blocking_op0"),
      PTRAUTH_ADDR_DIVERSIFY, cs_blob_sig);
   ...
   */
  memcpy((void *)&tmp_blob.csb_entitlements, &signed_entitlements,
         sizeof(void *));
#if CODE_SIGNING_MONITOR
  ...
  if (uip->cs_blobs->csb_next) {
    zone_require_ro(ZONE_ID_CS_BLOB, sizeof(struct cs_blob),
                    uip->cs_blobs->csb_next);
  }
  ...
  if (cs_debug > 1) {
    ...
    printf("CODE SIGNING: proc %d(%s) "
           ...
           blob_ro->csb_cpu_type == -1 ? "detached" : "embedded", name,
...
#if CONFIG_SUPPLEMENTAL_SIGNATURES
int ubc_cs_blob_add_supplement(struct vnode *vp, struct vnode *orig_vp,
                               off_t base_offset, vm_address_t *addr,
                               vm_size_t size, struct cs_blob **ret_blob) {
  kern_return_t kr;
  ...
  for (orig_blob = ubc_get_cs_blobs(orig_vp); orig_blob != NULL;
    ...
    if (orig_blob->csb_hashtype == tmp_blob.csb_linkage_hashtype &&
        memcmp(orig_blob->csb_cdhash, tmp_blob.csb_linkage, CS_CDHASH_LEN) ==
            0) {
      // Found match!
...
#if CONFIG_MACF
  unsigned int signer_type = tmp_blob.csb_signer_type;
  error = mac_vnode_check_supplemental_signature(vp, &tmp_blob, orig_vp,
                                                 orig_blob, &signer_type);
  ...
  // copy the platform bit or team-id from the linked signature and whether or
  // not the original is developer code
  tmp_blob.csb_platform_binary = 0;
  tmp_blob.csb_platform_path = 0;
  ...
  if (blob_start_offset >= blob_end_offset || blob_start_offset < 0 ||
      blob_end_offset <= 0) {
...
void csvnode_print_debug(struct vnode *vp) {
  ...
  for (blob = uip->cs_blobs; blob != NULL; blob = blob->csb_next) {
    printf("csvnode: range: %lu -> %lu flags: 0x%08x platform: %s path: %s "
           "team: %s\n",
           (unsigned long)blob->csb_start_offset,
           (unsigned long)blob->csb_end_offset, blob->csb_flags,
...
#if CONFIG_SUPPLEMENTAL_SIGNATURES
struct cs_blob *ubc_cs_blob_get_supplement(struct vnode *vp, off_t offset) {
  struct cs_blob *blob;
...
struct cs_blob *ubc_cs_blob_get(struct vnode *vp, cpu_type_t cputype,
                                cpu_subtype_t cpusubtype, off_t offset) {
  struct cs_blob *blob;
  off_t offset_in_blob;
  ...
  for (blob = ubc_get_cs_blobs(vp); blob != NULL; blob = blob->csb_next) {
...
void ubc_cs_free_and_vnode_unlock(vnode_t vp) {
  ...
  for (blob = cs_blobs; blob != NULL; blob = next_blob) {
...
static void ubc_cs_free(struct ubc_info *uip) {
  ...
  for (blob = uip->cs_blobs; blob != NULL; blob = next_blob) {
    next_blob = blob->csb_next;
    os_atomic_add(&cs_blob_count, -1, relaxed);
    os_atomic_add(&cs_blob_size, -blob->csb_mem_size, relaxed);
    cs_blob_ro_free(blob);
 ...
 */
int ubc_cs_generation_check(struct vnode *vp) {
...
int ubc_cs_blob_revalidate(struct vnode *vp, struct cs_blob *blob,
                           struct image_params *imgp, int flags,
                           uint32_t platform) {
  int error = 0;
  const CS_CodeDirectory *cd = NULL;
  const CS_GenericBlob *entitlements = NULL;
  ...
  if ((blob->csb_flags & CS_VALID) == 0) {
    // If the blob attached to the vnode was invalidated, don't try to
    // revalidate it Blob invalidation only occurs when the file that the blob
    // is attached to is opened for writing, giving us a signal that the file is
    // modified.
    printf("CODESIGNING: can not re-validate a previously invalidated blob, "
           "reboot or create a new file.\n");
    error = EPERM;
    goto out;
  }

  size = blob->csb_mem_size;
  error = cs_validate_csblob((const uint8_t *)blob->csb_mem_kaddr, size, &cd,
                             &entitlements, &der_entitlements);
  if (error) {
    if (cs_debug) {
      printf("CODESIGNING: csblob invalid: %d\n", error);
  ...
  if (blob->csb_reconstituted) {
     ...
     */
    if (cs_debug) {
      printf("CODESIGNING: revalidate: not inline revalidating reconstituted "
             "signature.\n");
    }
...
#if CONFIG_MACF
  error = mac_vnode_check_signature(vp, blob, imgp, &cs_flags, &signer_type,
                                    flags, platform);
  if (cs_debug && error) {
  ...
  vnode_lock_spin(vp);
  struct cs_signer_info signer_info = {.csb_flags = cs_flags,
...
struct cs_blob *ubc_get_cs_blobs(struct vnode *vp) {
  struct ubc_info *uip;
...
#if CONFIG_SUPPLEMENTAL_SIGNATURES
struct cs_blob *ubc_get_cs_supplement(struct vnode *vp) {
  struct ubc_info *uip;
  struct cs_blob *blob;
...
void ubc_get_cs_mtime(struct vnode *vp, struct timespec *cs_mtime) {
  struct ubc_info *uip;
...
static boolean_t cs_validate_hash(struct cs_blob *blobs, memory_object_t pager,
                                  memory_object_offset_t page_offset,
                                  const void *data, vm_size_t *bytes_processed,
                                  unsigned *tainted) {
  union cs_hash_union mdctx;
  ...
  for (blob = blobs; blob != NULL; blob = blob->csb_next) {
    ...
    if (offset < blob->csb_start_offset || offset >= blob->csb_end_offset) {
    ...
    if (cd != NULL) {
      ...
      hash = hashes(cd, (uint32_t)(offset >> blob->csb_hash_pageshift),
                    hashtype->cs_size, lower_bound, upper_bound);
      if (hash != NULL) {
    ...
    *bytes_processed = size;
    ...
    if (blob->csb_hash_firstlevel_pageshift) {
      ...
      for (i = 0; i < size;) {
        ...
        size_t partial_size =
            MIN(size - i, (1U << blob->csb_hash_firstlevel_pageshift));
    ...
    if (bcmp(expected_hash, actual_hash, hashtype->cs_size) != 0) {
      if (cs_debug) {
        printf("CODE SIGNING: cs_validate_page: "
               ...
               pager, page_offset, size, asha1[0], asha1[1], asha1[2], asha1[3],
               asha1[4], esha1[0], esha1[1], esha1[2], esha1[3], esha1[4]);
      }
...
boolean_t cs_validate_range(struct vnode *vp, memory_object_t pager,
                            memory_object_offset_t page_offset,
                            const void *data, vm_size_t dsize,
                            unsigned *tainted) {
  vm_size_t offset_in_range;
  boolean_t all_subranges_validated =
      TRUE; /* turn false if any subrange fails */
  ...
  /*
   * This exemption is specifically useful for systems which want to avoid
   * paying the cost of verifying the integrity of pages, since that is done by
   * computing hashes, which can take some time.
   */
  ...
  *tainted = 0;
  ...
  for (offset_in_range = 0; offset_in_range < dsize;
    ...
    subrange_validated =
        cs_validate_hash(blobs, pager, page_offset + offset_in_range,
                         (const void *)((const char *)data + offset_in_range),
                         &bytes_processed, &subrange_tainted);
...
void cs_validate_page(struct vnode *vp, memory_object_t pager,
                      memory_object_offset_t page_offset, const void *data,
  ...
  *nx_p = VMP_CS_ALL_FALSE;
  ...
  for (offset_in_page = 0; offset_in_page < PAGE_SIZE;
    ...
    subrange_validated =
        cs_validate_hash(blobs, pager, page_offset + offset_in_page,
    ...
    if (offset_in_page == 0 && bytes_processed > PAGE_SIZE - FOURK_PAGE_SIZE) {
    ...
    assertf(bytes_processed <= FOURK_PAGE_SIZE,
            ...
            vp, blobs, (uint64_t)page_offset, (uint64_t)offset_in_page,
...
int ubc_cs_getcdhash(vnode_t vp, off_t offset, unsigned char *cdhash,
                     uint8_t *type) {
...
boolean_t ubc_cs_is_range_codesigned(vnode_t vp, mach_vm_offset_t start,
                                     mach_vm_size_t size) {
   ...
   */
  blob_start =
      (mach_vm_offset_t)(csblob->csb_base_offset + csblob->csb_start_offset);
  blob_end =
      (mach_vm_offset_t)(csblob->csb_base_offset + csblob->csb_end_offset);
 ...
 */
#define USE_CODE_SIGN_BITMAP(vp)                                               \
  ((vp != NULL) && (vp->v_mount != NULL) &&                                    \
   (vp->v_mount->mnt_flag & MNT_ROOTFS) && !root_fs_upgrade_try)
kern_return_t ubc_cs_validation_bitmap_allocate(vnode_t vp) {
  ...
  if (!USE_CODE_SIGN_BITMAP(vp) || (!UBCINFOEXISTS(vp))) {
    ...
    if (uip->cs_valid_bitmap == NULL) {
      ...
      target_bitmap =
          (char *)kalloc_data((vm_size_t)bitmap_size, Z_WAITOK | Z_ZERO);
      if (target_bitmap == 0) {
        kr = KERN_NO_SPACE;
      } else {
        kr = KERN_SUCCESS;
...
kern_return_t ubc_cs_check_validation_bitmap(vnode_t vp,
...
void ubc_cs_validation_bitmap_deallocate(struct ubc_info *uip) {
  if (uip->cs_valid_bitmap != NULL) {
...
kern_return_t
ubc_cs_check_validation_bitmap(__unused struct vnode *vp,
                               __unused memory_object_offset_t offset,
                               __unused int optype) {
  return KERN_INVALID_ARGUMENT;
}

void ubc_cs_validation_bitmap_deallocate(__unused struct ubc_info *uip) {
  return;
}
...
kern_return_t cs_associate_blob_with_mapping(void *pmap, vm_map_offset_t start,
                                             vm_map_size_t size,
  ...
  for (blob = blobs; blob != NULL; blob = blob->csb_next) {
    ...
    if ((off_t)offset < blob_start_offset || (off_t)offset >= blob_end_offset ||
  ...
  if (monitor_sig_obj != NULL) {
    ...
    kr = csm_associate_code_signature(pmap, monitor_sig_obj, start, size,
                                      segment_offset);
  } else {
