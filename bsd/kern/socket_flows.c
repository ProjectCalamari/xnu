 ...
 * struct soflow_hash_entry) is protected by the socket lock. This covers all
 * the socket paths that calls soflow_get_flow() as well as the garbage
 * collection. For the socket detach path, soflow_detach() cannot assume the
 * socket lock is held. Thus, reference counts are added to both struct
 * soflow_db and struct soflow_hash_entry to avoid access after freed issues.
 *
 * The global list, soflow_entry_head, keeps track of all struct
 * soflow_hash_entry entries which is used by garbage collection when detecting
 * idle entries.  This list is protected by the global lock soflow_lck_rw.
 *
 */
...
SYSCTL_LONG(_net_soflow, OID_AUTO, count, CTLFLAG_LOCKED | CTLFLAG_RD,
            &soflow_attached_count, "");
SYSCTL_LONG(_net_soflow, OID_AUTO, high_water_mark, CTLFLAG_LOCKED | CTLFLAG_RD,
            &soflow_attached_high_water_mark, "");
...
#define SOFLOW_LOG(level, so, debug, fmt, ...)                                 \
  do {                                                                         \
    if (soflow_log_level >= level && debug && soflow_log_handle) {             \
      if (level == LOG_ERR) {                                                  \
        os_log_error(                                                          \
            soflow_log_handle, "SOFLOW - %s:%d <pid %d so %llx> " fmt "\n",    \
            __FUNCTION__, __LINE__, so ? SOFLOW_SOCKET_PID(so) : 0,            \
            so ? (uint64_t)VM_KERNEL_ADDRPERM(so) : 0, ##__VA_ARGS__);         \
      } else {                                                                 \
...
#define SOFLOW_HASH(laddr, faddr, lport, fport)                                \
  ((faddr) ^ ((laddr) >> 16) ^ (fport) ^ (lport))

#define SOFLOW_IS_UDP(so)                                                      \
  (so && SOCK_CHECK_TYPE(so, SOCK_DGRAM) && SOCK_CHECK_PROTO(so, IPPROTO_UDP))
#define SOFLOW_GET_SO_PROTO(so) (so ? SOCK_PROTO(so) : IPPROTO_MAX)

#define SOFLOW_SOCKET_PID(so)                                                  \
  ((so->so_flags & SOF_DELEGATED) ? so->e_pid : so->last_pid)

#define SOFLOW_ENABLE_DEBUG(so, entry)                                         \
  ((soflow_log_port == 0 || !entry ||                                          \
    soflow_log_port == ntohs(entry->soflow_lport) ||                           \
    soflow_log_port == ntohs(entry->soflow_fport)) &&                          \
   (soflow_log_pid == 0 || !so || soflow_log_pid == SOFLOW_SOCKET_PID(so)) &&  \
   (soflow_log_proto == 0 || !so ||                                            \
    soflow_log_proto == SOFLOW_GET_SO_PROTO(so)))
...
#define SOFLOW_GC_RUN_INTERVAL_NSEC                                            \
  (10 * NSEC_PER_SEC) // GC wakes up every 10 seconds
...
static boolean_t
soflow_entry_local_address_needs_update(struct soflow_hash_entry *);
static boolean_t
soflow_entry_local_port_needs_update(struct socket *,
                                     struct soflow_hash_entry *);

static void soflow_init(void) {
  ...
  if (kernel_thread_start(soflow_gc_thread_func, NULL, &soflow_gc_thread) !=
      KERN_SUCCESS) {
...
static void soflow_entry_log(int level, struct socket *so,
                             struct soflow_hash_entry *entry, const char *msg) {
#pragma unused(level, msg)
  ...
  SOFLOW_LOG(level, so, entry->soflow_debug,
             "<%s>: %s <%s(%d) entry %p, featureID %llu, filter_ctl 0x%x> "
             "outifp %d lport %d fport %d laddr %s faddr %s hash %X "
             ...
             entry->soflow_filter_control_unit, entry->soflow_outifindex,
             ntohs(entry->soflow_lport), ntohs(entry->soflow_fport), local,
             remote, entry->soflow_flowhash, entry->soflow_rxpackets,
             entry->soflow_rxbytes, entry->soflow_txpackets,
             entry->soflow_txbytes);
}

bool soflow_fill_hash_entry_from_address(struct soflow_hash_entry *entry,
                                         bool isLocal, struct sockaddr *addr,
                                         bool islocalUpdate) {
...
bool soflow_fill_hash_entry_from_inp(struct soflow_hash_entry *entry,
                                     bool isLocal, struct inpcb *inp,
                                     bool islocalUpdate) {
...
static errno_t soflow_db_init(struct socket *so) {
  errno_t error = 0;
  ...
  db->soflow_db_so = so;
  void *__single hash =
      hashinit(SOFLOW_HASH_SIZE, M_CFIL, &db->soflow_db_hashmask);
  db->soflow_db_hashbase = __unsafe_forge_bidi_indexable(
      struct soflow_hash_head *, hash, SOFLOW_HASH_SIZE * sizeof(void *));
...
static void soflow_entry_free(struct soflow_hash_entry *hash_entry) {
  struct socket *so = (hash_entry && hash_entry->soflow_db)
                          ? hash_entry->soflow_db->soflow_db_so
                          : NULL;
  ...
  SOFLOW_ENTRY_LOG(LOG_INFO, so, hash_entry, hash_entry->soflow_debug,
                   "Free entry");
  kfree_type(struct soflow_hash_entry, hash_entry);
}

static void soflow_db_remove_entry(struct soflow_db *db,
  ...
  if (hash_entry->soflow_nstat_context != NULL) {
    SOFLOW_LOG(LOG_INFO, db->soflow_db_so, hash_entry->soflow_debug,
               "<Close nstat> - context %lX",
               (unsigned long)hash_entry->soflow_nstat_context);
  ...
  if (hash_entry->soflow_feat_ctxt != NULL &&
      soflow_feat_detach_entry_func_ptr != NULL) {
...
static void soflow_db_free(struct soflow_db *db) {
  ...
  SOFLOW_LOG(LOG_INFO, db->soflow_db_so, db->soflow_db_debug,
             "<db %p> freeing db (count == %d)", db, db->soflow_db_count);
  ...
  for (int i = 0; i < SOFLOW_HASH_SIZE; i++) {
    ...
    LIST_FOREACH_SAFE(entry, flowhash, soflow_entry_link, temp_entry) {
      SOFLOW_ENTRY_LOG(LOG_INFO, db->soflow_db_so, entry, entry->soflow_debug,
                       "Remove entry");
...
void soflow_detach(struct socket *so) {
...
static boolean_t soflow_match_entries_v4(struct soflow_hash_entry *entry1,
                                         struct soflow_hash_entry *entry2,
                                         boolean_t remoteOnly) {
  if (entry1 == NULL || entry2 == NULL) {
    return false;
  }

  // Ignore local match if remoteOnly or if local has been updated since entry
  // added
  boolean_t lport_matched = (remoteOnly || entry1->soflow_lport_updated ||
                             entry1->soflow_lport == entry2->soflow_lport);
  boolean_t laddr_matched = (remoteOnly || entry1->soflow_laddr_updated ||
                             entry1->soflow_laddr.addr46.ia46_addr4.s_addr ==
                                 entry2->soflow_laddr.addr46.ia46_addr4.s_addr);
  ...
  return lport_matched && entry1->soflow_fport == entry2->soflow_fport &&
         laddr_matched &&
         entry1->soflow_faddr.addr46.ia46_addr4.s_addr ==
             entry2->soflow_faddr.addr46.ia46_addr4.s_addr;
}

static boolean_t soflow_match_entries_v6(struct soflow_hash_entry *entry1,
                                         struct soflow_hash_entry *entry2,
                                         boolean_t remoteOnly) {
  if (entry1 == NULL || entry2 == NULL) {
    return false;
  ...
  boolean_t lport_matched = (remoteOnly || entry1->soflow_lport_updated ||
                             entry1->soflow_lport == entry2->soflow_lport);
  boolean_t laddr_matched =
      (remoteOnly || entry1->soflow_laddr_updated ||
       in6_are_addr_equal_scoped(
           &entry1->soflow_laddr.addr6, &entry2->soflow_laddr.addr6,
           entry1->soflow_laddr6_ifscope, entry2->soflow_laddr6_ifscope));

  // Entries match if local and remote ports and addresses all matched
  return lport_matched && entry1->soflow_fport == entry2->soflow_fport &&
         laddr_matched &&
         in6_are_addr_equal_scoped(
             &entry1->soflow_faddr.addr6, &entry2->soflow_faddr.addr6,
             entry1->soflow_faddr6_ifscope, entry2->soflow_faddr6_ifscope);
}

static struct soflow_hash_entry *
soflow_db_lookup_entry_internal(struct soflow_db *db, struct sockaddr *local,
                                struct sockaddr *remote, boolean_t remoteOnly,
  ...
  matchentry.soflow_debug =
      SOFLOW_ENABLE_DEBUG(db->soflow_db_so, (&matchentry));
  SOFLOW_ENTRY_LOG(LOG_DEBUG, db->soflow_db_so, &matchentry, true,
                   "Looking for entry");

  if (matchentry.soflow_family == AF_INET6) {
    hashkey_faddr = matchentry.soflow_faddr.addr6.s6_addr32[3];
    hashkey_laddr =
        (remoteOnly == false) ? matchentry.soflow_laddr.addr6.s6_addr32[3] : 0;
  } else {
    hashkey_faddr = matchentry.soflow_faddr.addr46.ia46_addr4.s_addr;
    hashkey_laddr = (remoteOnly == false)
                        ? matchentry.soflow_laddr.addr46.ia46_addr4.s_addr
                        : 0;
  }

  hashkey_fport = matchentry.soflow_fport;
  hashkey_lport = (remoteOnly == false || withLocalPort == true)
                      ? matchentry.soflow_lport
                      : 0;

  inp_hash_element =
      SOFLOW_HASH(hashkey_laddr, hashkey_faddr, hashkey_lport, hashkey_fport);
  ...
  LIST_FOREACH(nextentry, flowhash, soflow_entry_link) {
    if (matchentry.soflow_family == AF_INET6) {
      if (soflow_match_entries_v6(nextentry, &matchentry, remoteOnly)) {
        SOFLOW_ENTRY_LOG(LOG_DEBUG, db->soflow_db_so, nextentry,
                         nextentry->soflow_debug, "Found entry v6");
        break;
      }
    } else if (matchentry.soflow_family == AF_INET) {
      if (soflow_match_entries_v4(nextentry, &matchentry, remoteOnly)) {
        SOFLOW_ENTRY_LOG(LOG_DEBUG, db->soflow_db_so, nextentry,
                         nextentry->soflow_debug, "Found entry v4");
        break;
      }
    }
  }
  ...
  if (nextentry == NULL) {
    SOFLOW_ENTRY_LOG(LOG_DEBUG, db->soflow_db_so, &matchentry,
                     matchentry.soflow_debug, "Entry not found");
  }
  return nextentry;
}

static struct soflow_hash_entry *soflow_db_lookup_entry(struct soflow_db *db,
                                                        struct sockaddr *local,
                                                        struct sockaddr *remote,
                                                        boolean_t remoteOnly) {
  struct soflow_hash_entry *entry =
      soflow_db_lookup_entry_internal(db, local, remote, remoteOnly, false);
  if (entry == NULL && remoteOnly == true) {
    entry =
        soflow_db_lookup_entry_internal(db, local, remote, remoteOnly, true);
  }
  return entry;
}

static struct soflow_hash_entry *
soflow_db_lookup_by_feature_context_id(struct soflow_db *db,
                                       u_int64_t feature_context_id) {
  struct soflow_hash_head *flowhash = NULL;
  ...
  if (nextentry == NULL) {
    SOFLOW_LOG(LOG_DEBUG, db->soflow_db_so, db->soflow_db_debug,
               "No entry found for featureID %llu <count %d hash %X %X>",
               feature_context_id, db->soflow_db_count, inp_hash_element,
               (u_int32_t)(feature_context_id & 0x0ffffffff));
  }
  return nextentry;
}

void *soflow_db_get_feature_context(struct soflow_db *db,
                                    u_int64_t feature_context_id) {
  struct soflow_hash_entry *hash_entry = NULL;
  ...
  if (db->soflow_db_count == 1) {
    if (db->soflow_db_only_entry != NULL &&
        db->soflow_db_only_entry->soflow_feat_ctxt != NULL &&
        db->soflow_db_only_entry->soflow_feat_ctxt_id == feature_context_id) {
      SOFLOW_ENTRY_LOG(LOG_DEBUG, db->soflow_db_so, db->soflow_db_only_entry,
                       db->soflow_db_only_entry->soflow_debug,
                       "MATCHED only entry for featureID");
      context = db->soflow_db_only_entry->soflow_feat_ctxt;
    } else {
      SOFLOW_LOG(LOG_DEBUG, db->soflow_db_so, db->soflow_db_debug,
                 "MISMATCHED only entry for featureID %llu (entry %p - cfil %p "
                 "id %llu)",
                 feature_context_id, db->soflow_db_only_entry,
                 db->soflow_db_only_entry
                     ? db->soflow_db_only_entry->soflow_feat_ctxt
                     : NULL,
                 db->soflow_db_only_entry
                     ? db->soflow_db_only_entry->soflow_feat_ctxt_id
                     : 0);
    }
  } else {
...
u_int64_t soflow_db_get_feature_context_id(struct soflow_db *db,
                                           struct sockaddr *local,
                                           struct sockaddr *remote) {
  struct soflow_hash_entry *hash_entry = NULL;
...
static struct soflow_hash_entry *soflow_db_add_entry(struct soflow_db *db,
                                                     struct sockaddr *local,
                                                     struct sockaddr *remote) {
  struct soflow_hash_entry *entry = NULL;
  ...
  entry->soflow_flowhash = SOFLOW_HASH(
      hashkey_laddr, hashkey_faddr, entry->soflow_lport, entry->soflow_fport);
  ...
  SOFLOW_LOG(LOG_INFO, db->soflow_db_so, db->soflow_db_debug, "total count %d",
             db->soflow_db_count);
...
static sa_family_t
soflow_udp_get_address_from_control(struct mbuf *control,
                                    uint8_t *__counted_by(*count) * address_ptr,
                                    int *count) {
  struct cmsghdr *cm;
...
static boolean_t
soflow_entry_local_port_needs_update(struct socket *so,
                                     struct soflow_hash_entry *entry) {
  if (SOFLOW_IS_UDP(so)) {
...
static void soflow_entry_update_local(struct soflow_db *db,
                                      struct soflow_hash_entry *entry,
                                      struct sockaddr *local,
                                      struct mbuf *control,
                                      u_short rcv_ifindex) {
  ...
  if (entry->soflow_outifindex == 0 &&
      (inp->inp_last_outifp != NULL || rcv_ifindex != 0)) {
    entry->soflow_outifindex =
        inp->inp_last_outifp ? inp->inp_last_outifp->if_index : rcv_ifindex;
    SOFLOW_ENTRY_LOG(LOG_INFO, db->soflow_db_so, entry, entry->soflow_debug,
                     "Updated outifp");
  }
  ...
  if (soflow_entry_local_address_needs_update(entry)) {
    ...
    if (local == NULL && control != NULL) {
      ...
      sa_family_t family =
          soflow_udp_get_address_from_control(control, &addr_ptr, &size);
      ...
      if (family != AF_UNSPEC && size && addr_ptr) {
        ...
        case AF_INET:
          if (size == sizeof(struct in_addr)) {
            ...
            (void)memcpy(&address_buf.sin.sin_addr, addr_ptr,
        ...
        case AF_INET6:
          if (size == sizeof(struct in6_addr)) {
            ...
            (void)memcpy(&address_buf.sin6.sin6_addr, addr_ptr,
...
#if defined(NSTAT_EXTENSION_FILTER_DOMAIN_INFO)
static u_int32_t ifnet_to_flags(struct ifnet *ifp, struct socket *so) {
  u_int32_t flags = 0;

  if (ifp != NULL) {
    flags = nstat_ifnet_to_flags(ifp);
    if ((flags & NSTAT_IFNET_IS_WIFI) &&
        ((flags & (NSTAT_IFNET_IS_AWDL | NSTAT_IFNET_IS_LLW)) == 0)) {
...
static bool soflow_nstat_provider_request_vals(nstat_provider_context ctx,
                                               u_int32_t *ifflagsp,
  ...
  if (countsp) {
    ...
    SOFLOW_LOG(LOG_DEBUG, so, hash_entry->soflow_debug,
               "Collected NSTAT counts: rxpackets %llu rxbytes %llu txpackets "
               "%llu txbytes %llu",
               countsp->nstat_rxpackets, countsp->nstat_rxbytes,
               countsp->nstat_txpackets, countsp->nstat_txbytes);
  }

  if (detailsp) {
    bzero(detailsp, sizeof(*detailsp));
    detailsp->nstat_media_stats.ms_total.ts_rxbytes =
        hash_entry->soflow_rxbytes;
    detailsp->nstat_media_stats.ms_total.ts_txbytes =
        hash_entry->soflow_txbytes;
    detailsp->nstat_media_stats.ms_total.ts_rxpackets =
        hash_entry->soflow_rxpackets;
    detailsp->nstat_media_stats.ms_total.ts_txpackets =
        hash_entry->soflow_txpackets;

    SOFLOW_LOG(LOG_DEBUG, so, hash_entry->soflow_debug,
               "Collected NSTAT detailed counts: rxpackets %llu rxbytes %llu "
  ...
  if (metadatap) {
    ...
    if (hash_entry->soflow_family == AF_INET6) {
      in6_ip6_to_sockaddr(&hash_entry->soflow_laddr.addr6,
                          hash_entry->soflow_lport,
                          hash_entry->soflow_laddr6_ifscope, &desc->local.v6,
                          sizeof(desc->local.v6));
      in6_ip6_to_sockaddr(&hash_entry->soflow_faddr.addr6,
                          hash_entry->soflow_fport,
                          hash_entry->soflow_faddr6_ifscope, &desc->remote.v6,
    ...
    if (hash_entry->soflow_debug) {
      ...
      SOFLOW_LOG(
          LOG_DEBUG, so, hash_entry->soflow_debug,
          "Collected NSTAT metadata: eupid %llu epid %d euuid "
          "%x%x%x%x-%x%x%x%x-%x%x%x%x-%x%x%x%x "
          "outifp %d properties 0x%X lport %d fport %d laddr %s faddr %s "
          "rcvbufsize %u rcvbufused %u traffic_class %u",
          desc->eupid, desc->epid, ptr[0], ptr[1], ptr[2], ptr[3], ptr[4],
          ptr[5], ptr[6], ptr[7], ptr[8], ptr[9], ptr[10], ptr[11], ptr[12],
          ptr[13], ptr[14], ptr[15], desc->ifindex, desc->ifnet_properties,
          ntohs(desc->local.v4.sin_port), ntohs(desc->remote.v4.sin_port),
          local, remote, desc->rcvbufsize, desc->rcvbufused,
          desc->traffic_class);
    }
  }
done:
...
soflow_nstat_provider_request_extensions(nstat_provider_context ctx,
                                         int requested_extension, void *buf,
                                         size_t buf_size) {
  struct soflow_hash_entry *hash_entry = (struct soflow_hash_entry *)ctx;
  struct socket *so = (hash_entry && hash_entry->soflow_db)
                          ? hash_entry->soflow_db->soflow_db_so
  ...
  case NSTAT_EXTENDED_UPDATE_TYPE_DOMAIN:
    ...
    if (hash_entry->soflow_debug) {
      SOFLOW_LOG(LOG_DEBUG, so, hash_entry->soflow_debug,
                 "Collected NSTAT domain_info:pid %d domain <%s> owner <%s> "
                 "ctxt <%s> bundle id <%s> is_tracker %d is_non_app_initiated "
                 "%d is_silent %d",
                 so->so_flags & SOF_DELEGATED ? so->e_pid : so->last_pid,
                 domain_info->domain_name, domain_info->domain_owner,
                 domain_info->domain_tracker_ctxt,
                 domain_info->domain_attributed_bundle_id,
                 domain_info->is_tracker, domain_info->is_non_app_initiated,
...
static void soflow_update_flow_stats(struct soflow_hash_entry *hash_entry,
                                     size_t data_size, bool outgoing) {
  struct socket *so = (hash_entry && hash_entry->soflow_db)
                          ? hash_entry->soflow_db->soflow_db_so
                          : NULL;
  ...
  if (hash_entry != NULL) {
    if (outgoing) {
      ...
      hash_entry->soflow_txpackets++;
      SOFLOW_ENTRY_LOG(LOG_DEBUG, so, hash_entry, hash_entry->soflow_debug,
                       "Stats update - Outgoing");
    } else {
      hash_entry->soflow_rxbytes += data_size;
      hash_entry->soflow_rxpackets++;
      SOFLOW_ENTRY_LOG(LOG_DEBUG, so, hash_entry, hash_entry->soflow_debug,
                       "Stats update - Incoming");
    }
  }
}

struct soflow_hash_entry *
soflow_get_flow(struct socket *so, struct sockaddr *local,
                struct sockaddr *remote, struct mbuf *control, size_t data_size,
                soflow_direction_t direction, uint16_t rcv_ifindex) {
  struct soflow_hash_entry *hash_entry = NULL;
  ...
  if (so->so_flow_db != NULL) {
    ...
    if (hash_entry != NULL) {
      ...
      if (hash_entry->soflow_outifindex == 0 ||
          soflow_entry_local_address_needs_update(hash_entry) ||
          soflow_entry_local_port_needs_update(so, hash_entry)) {
        soflow_entry_update_local(so->so_flow_db, hash_entry, local, control,
                                  rcv_ifindex);
      }
      hash_entry->soflow_lastused = net_uptime();
      if (data_size > 0 && direction != SOFLOW_DIRECTION_UNKNOWN) {
        soflow_update_flow_stats(hash_entry, data_size,
                                 direction == SOFLOW_DIRECTION_OUTBOUND);
      }
  ...
  if (!soflow_nstat_disable && SOFLOW_IS_UDP(so) &&
    ...
    hash_entry->soflow_nstat_context = nstat_provider_stats_open(
        (nstat_provider_context)hash_entry, NSTAT_PROVIDER_UDP_SUBFLOW, 0,
        soflow_nstat_provider_request_vals,
        soflow_nstat_provider_request_extensions);
    SOFLOW_LOG(LOG_INFO, so, hash_entry->soflow_debug,
               "<Open nstat> - context %lX",
               (unsigned long)hash_entry->soflow_nstat_context);
#endif
  }
...
void soflow_free_flow(struct soflow_hash_entry *entry) {
  SOFLOW_ENTRY_FREE(entry);
}

static bool soflow_socket_safe_lock(struct inpcb *inp,
...
static struct socket *soflow_validate_dgram_socket(struct socket *so) {
...
static void soflow_gc_thread_sleep(bool forever) {
  if (forever) {
    (void)assert_wait((event_t)&soflow_attached_count, THREAD_INTERRUPTIBLE);
  } else {
    uint64_t deadline = 0;
    nanoseconds_to_absolutetime(SOFLOW_GC_RUN_INTERVAL_NSEC, &deadline);
    clock_absolutetime_interval_to_deadline(deadline, &deadline);

    (void)assert_wait_deadline(&soflow_attached_count, THREAD_INTERRUPTIBLE,
                               deadline);
  }
}
...
static bool soflow_gc_idle_timed_out(struct soflow_hash_entry *hash_entry,
                                     int timeout, u_int64_t current_time) {
  struct socket *so = (hash_entry && hash_entry->soflow_db)
                          ? hash_entry->soflow_db->soflow_db_so
                          : NULL;

  if (hash_entry &&
      (current_time - hash_entry->soflow_lastused >= (u_int64_t)timeout)) {
    SOFLOW_ENTRY_LOG(LOG_INFO, so, hash_entry, hash_entry->soflow_debug,
...
static int soflow_gc_cleanup(struct socket *so) {
  struct soflow_hash_entry *entry = NULL;
  struct soflow_hash_entry *temp_entry = NULL;
  struct soflow_hash_head *flowhash = NULL;
  ...
  for (int i = 0; i < SOFLOW_HASH_SIZE; i++) {
    ...
    LIST_FOREACH_SAFE(entry, flowhash, soflow_entry_link, temp_entry) {
      if (entry->soflow_gc || entry->soflow_feat_gc) {
        if (entry->soflow_feat_ctxt != NULL &&
            soflow_feat_gc_perform_func_ptr != NULL) {
        ...
        if (entry->soflow_gc) {
          SOFLOW_ENTRY_LOG(LOG_INFO, so, entry, entry->soflow_debug,
                           "GC cleanup entry");
          entry->soflow_gc = 0;
          soflow_db_remove_entry(db, entry);
          cleaned++;
...
#pragma unused(v, w)
  ...
  // Go thorough all flows in the flow list and record any socket with expired
  // flows.
  TAILQ_FOREACH(hash_entry, &soflow_entry_head, soflow_entry_list_link) {
    ...
    // Check if we need to perform cleanup due to idle time or feature specified
    // rules
    hash_entry->soflow_gc =
        soflow_gc_idle_timed_out(hash_entry, SOFLOW_GC_IDLE_TO, current_time);
    hash_entry->soflow_feat_gc =
        (soflow_feat_gc_needed_func_ptr != NULL &&
         soflow_feat_gc_needed_func_ptr(so, hash_entry, current_time));
...
void soflow_feat_set_functions(
    soflow_feat_gc_needed_func gc_needed_fn,
...
bool soflow_db_apply(struct soflow_db *db,
                     soflow_entry_apply_func entry_apply_fn, void *context) {
  struct soflow_hash_entry *entry = NULL;
  struct soflow_hash_entry *temp_entry = NULL;
  struct soflow_hash_head *flowhash = NULL;
