...
static void ktriage_lock(void) { lck_mtx_lock(&ktriage_mtx); }

static void ktriage_unlock(void) { lck_mtx_unlock(&ktriage_mtx); }

__startup_func void create_buffers_triage(void) {
  ...
  uint32_t storage_count =
      cpu_count * kd_control_triage.kdebug_min_storage_units_per_cpu;

  kd_buffer_triage.kdb_storage_count = storage_count;
  kd_buffer_triage.kdb_event_count =
      storage_count * kd_control_triage.kdebug_events_per_storage_unit;

  int error = create_buffers(&kd_control_triage, &kd_buffer_triage,
...
static void ktriage_convert_to_string(uint64_t debugid, uintptr_t arg,
  ...
  const char **subsystem_strings =
      ktriage_subsystems_strings[subsystem].strings;
  uint16_t strindx = KDBG_TRIAGE_EXTRACT_CODE(debugid);

  /* fallback if ktriage doesn't know how to parse the given debugid */
  if (subsystem_num_strings < 1 || subsystem_strings == NULL ||
      strindx >= subsystem_num_strings) {
    snprintf(buf, bufsz,
  ...
  snprintf(buf, bufsz, "%s(arg = 0x%lx) %s", subsystem_strings[0], arg,
           subsystem_strings[strindx]);

  return;
}

static void _write_triage_record_nopreempt(uintptr_t debugid, uintptr_t arg,
                                           uintptr_t thread_id) {
  uint64_t now = 0;
  uint32_t bindx;
  kd_buf *kd;
  ...
  const uint32_t events_per_storage =
      kd_control_triage.kdebug_events_per_storage_unit;
  ...
  while (true) {
    ...
    if (kdsp_actual == NULL || bindx >= events_per_storage) {
      if (kdebug_storage_alloc(&kd_control_triage, &kd_buffer_triage, cpu) ==
          false) {
        break;
      }
      continue;
...
static struct kd_storage *_find_triage_min_storage(uint64_t thread_id) {
  ...
  for (unsigned int cpu = 0; cpu < kd_control_triage.kdebug_cpus; cpu++) {
    ...
    struct kd_storage *store =
        POINTER_FROM_KDS_PTR(kd_buffer_triage.kd_bufs, store_ptr);
...
static size_t _read_triage_records(kd_buf *read_buffer, size_t max_count,
                                   uint64_t thread_id) {
  ...
  if (avail_count == 0 || !(kd_control_triage.kdc_flags & KDBG_BUFINIT)) {
    return 0;
  }
...
void ktriage_extract(uint64_t thread_id, void *buf, uint32_t bufsz) {
  ...
  if (record_buf == NULL) {
    ...
    ktriage_lock();
    record_cnt = _read_triage_records(
        record_buf, kd_buffer_triage.kdb_event_count, thread_id);
    ktriage_unlock();
  }
  ...
  for (size_t i = 0; i < record_cnt; i++) {
    ...
    ktriage_convert_to_string(kd->arg4, kd->arg1, local_buf,
                              KDBG_TRIAGE_MAX_STRLEN);
...
int ktriage_register_subsystem_strings(uint8_t subsystem,
                                       ktriage_strings_t *subsystem_strings) {
  if (subsystem == 0 || subsystem > KDBG_TRIAGE_SUBSYS_MAX ||
      subsystem_strings == NULL) {
    return EINVAL;
  }

  ktriage_lock();

  ktriage_subsystems_strings[subsystem].num_strings =
      subsystem_strings->num_strings;
...
const char *vm_triage_strings[] = {
    ...
    [KDBG_TRIAGE_VM_FAULT_COPY_MEMORY_SHORTAGE] =
        "vm_fault_copy hit memory shortage\n",
    [KDBG_TRIAGE_VM_FAULT_OBJCOPYSLOWLY_MEMORY_SHORTAGE] =
        "vm_object_copy_slowly fault hit memory shortage\n",
    [KDBG_TRIAGE_VM_FAULT_OBJIOPLREQ_MEMORY_SHORTAGE] =
        "vm_object_iopl_request fault hit memory shortage\n",
    ...
    [KDBG_TRIAGE_VM_OBJECT_NO_PAGER_RECLAIM] =
        "Object has no pager because the backing vnode was reclaimed\n",
    [KDBG_TRIAGE_VM_OBJECT_NO_PAGER_UNMOUNT] =
        "Object has no pager because the backing vnode was unmounted\n",
    ...
    [KDBG_TRIAGE_VM_OBJECT_NO_PAGER_DEALLOC_PAGER] =
        "Object has no pager because the pager was deallocated\n",
    ...
    [KDBG_TRIAGE_VM_COPYOVERWRITE_PARTIAL_HEAD_NESTED_ERROR] =
        "vm_map_copy_overwrite_nested failed when trying misaligned head "
        "copy\n",
    [KDBG_TRIAGE_VM_COPYOVERWRITE_PARTIAL_TAIL_NESTED_ERROR] =
        "vm_map_copy_overwrite_nested failed when trying misaligned tail "
        "copy\n",
    [KDBG_TRIAGE_VM_COPYOUT_INTERNAL_SIZE_ERROR] =
        "vm_map_copyout_internal failed due to bad size\n",
    [KDBG_TRIAGE_VM_COPYOUT_KERNEL_BUFFER_ERROR] =
        "vm_map_copyout_kernel_buffer failed\n",
    [KDBG_TRIAGE_VM_COPYOUT_INTERNAL_ADJUSTING_ERROR] =
        "vm_map_copyout_internal failed when trying to adjust src-dest "
        "params\n",
    [KDBG_TRIAGE_VM_COPYOUT_INTERNAL_SPACE_ERROR] =
        "vm_map_copyout_internal failed because we couldn't locate space\n",
    [KDBG_TRIAGE_VM_ALLOCATE_KERNEL_BADFLAGS_ERROR] =
        "mach_vm_allocate_kernel failed due to bad flags\n",
    ...
    [KDBG_TRIAGE_VM_ALLOCATE_KERNEL_BADSIZE_ERROR] =
        "mach_vm_allocate_kernel failed due to bad size\n",
...
const char *shared_region_triage_strings[] = {
    ...
    [KDBG_TRIAGE_SHARED_REGION_PAGER_MEMORY_SHORTAGE] =
        "shared_region_pager_data_request hit memory shortage\n",
};
/* Shared Region end */
...
const char *dyld_pager_triage_strings[] = {
    ...
    [KDBG_TRIAGE_DYLD_PAGER_NO_UPL] =
        "dyld_pager_data_request couldn't create a upl\n",
    [KDBG_TRIAGE_DYLD_PAGER_MEMORY_SHORTAGE] =
        "dyld_pager_data_request hit memory shortage\n",
    [KDBG_TRIAGE_DYLD_PAGER_SLIDE_ERROR] =
        "dyld_pager_data_request hit a page sliding error\n",
    [KDBG_TRIAGE_DYLD_PAGER_CHAIN_OUT_OF_RANGE] =
        "dyld_pager_data_request chain out of range\n",
    [KDBG_TRIAGE_DYLD_PAGER_SEG_INFO_OUT_OF_RANGE] =
        "dyld_pager_data_request seg_info out of range\n",
    [KDBG_TRIAGE_DYLD_PAGER_SEG_SIZE_OUT_OF_RANGE] =
        "dyld_pager_data_request seg->size out of range\n",
    [KDBG_TRIAGE_DYLD_PAGER_SEG_PAGE_CNT_OUT_OF_RANGE] =
        "dyld_pager_data_request seg->page_count out of range\n",
    [KDBG_TRIAGE_DYLD_PAGER_NO_SEG_FOR_VA] =
        "dyld_pager_data_request no segment for VA\n",
    [KDBG_TRIAGE_DYLD_PAGER_RANGE_NOT_FOUND] =
        "dyld_pager_data_request no range for offset\n",
    [KDBG_TRIAGE_DYLD_PAGER_DELTA_TOO_LARGE] =
        "dyld_pager_data_request delta * 4 > PAGE_SIZE\n",
    [KDBG_TRIAGE_DYLD_PAGER_PAGE_START_OUT_OF_RANGE] =
        "dyld_pager_data_request segInfo page_start out of range\n",
    [KDBG_TRIAGE_DYLD_PAGER_BAD_POINTER_FMT] =
        "dyld_pager_data_request unkown pointer format\n",
    [KDBG_TRIAGE_DYLD_PAGER_INVALID_AUTH_KEY] =
        "dyld_pager_data_request unkown auth key\n",
    [KDBG_TRIAGE_DYLD_PAGER_BIND_ORDINAL] =
        "dyld_pager_data_request invalid bind ordinal\n",
};
/* Dyld Pager end */
...
const char *apple_protect_pager_triage_strings[] = {
    ...
    [KDBG_TRIAGE_APPLE_PROTECT_PAGER_MEMORY_SHORTAGE] =
        "apple_protect_pager_data_request hit memory shortage\n",
};
...
const char *corpse_triage_strings[] = {
    ...
    [KDBG_TRIAGE_CORPSE_PROC_TOO_BIG] =
        "Process too big for corpse. Corpse disallowed.\n",
    [KDBG_TRIAGE_CORPSE_FAIL_LIBGMALLOC] =
        "Process linked against libgmalloc. Corpse disallowed.\n",
    [KDBG_TRIAGE_CORPSE_BLOCKED_JETSAM] =
        "Jetsams happening in higher bands. Corpse disallowed.\n",
    [KDBG_TRIAGE_CORPSE_LIMIT] =
        "Too many corpses in flight. Corpse disallowed.\n",
    [KDBG_TRIAGE_CORPSES_DISABLED] = "Corpse disabled on system.\n",
    [KDBG_TRIAGE_CORPSE_DISABLED_FOR_PROC] =
        "Corpse disabled for this process.\n",
};
/* Corpse section end */
...
ktriage_strings_t ktriage_subsystems_strings[KDBG_TRIAGE_SUBSYS_MAX + 1] = {
    ...
    [KDBG_TRIAGE_SUBSYS_CLUSTER] = {CLUSTER_MAX_TRIAGE_STRINGS,
                                    cluster_triage_strings},
    [KDBG_TRIAGE_SUBSYS_SHARED_REGION] = {SHARED_REGION_MAX_TRIAGE_STRINGS,
                                          shared_region_triage_strings},
    [KDBG_TRIAGE_SUBSYS_DYLD_PAGER] = {DYLD_PAGER_MAX_TRIAGE_STRINGS,
                                       dyld_pager_triage_strings},
    [KDBG_TRIAGE_SUBSYS_APPLE_PROTECT_PAGER] =
        {APPLE_PROTECT_PAGER_MAX_TRIAGE_STRINGS,
         apple_protect_pager_triage_strings},
    [KDBG_TRIAGE_SUBSYS_CORPSE] = {CORPSE_MAX_TRIAGE_STRINGS,
                                   corpse_triage_strings},
