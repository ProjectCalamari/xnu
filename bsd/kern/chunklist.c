...
static const char *libkern_path =
...
const size_t rev1_chunklist_num_pubkeys =
    sizeof(rev1_chunklist_pubkeys) / sizeof(rev1_chunklist_pubkeys[0]);

static void key_byteswap(void *_dst, const void *_src, size_t len) {
...
static int construct_chunklist_path(char path[static MAXPATHLEN],
...
static int validate_signature(const uint8_t *key_msb, size_t keylen,
                              uint8_t *sig_msb, size_t siglen,
                              uint8_t *digest) {
  int err = 0;
  ...
  rsa_ctx = kalloc_type(rsa_pub_ctx, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  err = rsa_make_pub(rsa_ctx, sizeof(exponent), exponent, CHUNKLIST_PUBKEY_LEN,
                     modulus);
  if (err) {
  ...
  err = rsa_verify_pkcs1v15(rsa_ctx, CC_DIGEST_OID_SHA256, SHA256_DIGEST_LENGTH,
                            digest, siglen, sig, &sig_valid);
  if (err) {
    sig_valid = false;
...
static int validate_root_image(const char *root_path, void *chunklist) {
  int err = 0;
  ...
  for (ch = 0; ch < hdr->cl_chunk_count; ch++) {
    ...
    err = vn_rdwr(UIO_READ, vp, (caddr_t)buf, chk->chunk_size, offset,
    ...
    if (bcmp(sha_digest, chk->chunk_sha256, SHA256_DIGEST_LENGTH) != 0) {
      AUTHPRNT("SHA mismatch on chunk %lu (offset %lld, size %u)", ch, offset,
               chk->chunk_size);
      err = EINVAL;
      goto out;
    }
  ...
  if (offset != fsize) {
    AUTHPRNT(
        "chunklist did not cover entire file (offset = %lld, fsize = %lld)",
        offset, fsize);
    err = EINVAL;
    goto out;
  }
...
static const uuid_t *getuuidfromheader_safe(const void *buf, size_t bufsz,
                                            size_t *uuidsz) {
  ...
  if (mh->magic != MH_MAGIC_64 ||
      (mh->sizeofcmds > bufsz - sizeof(kernel_mach_header_t))) {
 ...
 */
static int validate_chunklist(void *buf, size_t len) {
  int err = 0;
  size_t sigsz = 0;
  size_t sig_end = 0;
  size_t chunks_end = 0;
  ...
  if (os_mul_and_add_overflow(hdr->cl_chunk_count,
                              sizeof(struct chunklist_chunk),
                              hdr->cl_chunk_offset, &chunks_end) ||
  ...
  if (os_add_overflow(hdr->cl_sig_offset, sig_len, &sig_end) ||
      ...
      hdr->cl_sig_offset < chunks_end || hdr->cl_sig_offset > len) {
    AUTHPRNT("invalid signature offset (%llu)", hdr->cl_sig_offset);
    return EINVAL;
  }

  if (sig_end > len || os_sub_overflow(len, hdr->cl_sig_offset, &sigsz) ||
      sigsz != sig_len) {
  ...
  for (size_t i = 0; i < rev1_chunklist_num_pubkeys; i++) {
    ...
    err = validate_signature(key->key, CHUNKLIST_PUBKEY_LEN,
                             (uint8_t *)((uintptr_t)buf + hdr->cl_sig_offset),
                             CHUNKLIST_SIGNATURE_LEN, sha_digest);
    if (err == 0) {
      AUTHDBG("validated rev1 chunklist signature with rev1 key %lu (prod=%d)",
              i, key->is_production);
      ...
      if (!key->is_production) {
        ...
        AUTHDBG(
            "*** allowing DEV rev1 key: this will fail in customer builds ***");
        valid_sig = TRUE;
      }
#endif
      goto out;
 ...
 */
int authenticate_root_with_chunklist(const char *rootdmg_path,
                                     boolean_t *out_enforced) {
  ...
  err =
      imageboot_read_file(chunklist_path, &chunklist_buf, &chunklist_len, NULL);
  if (err) {
...
out:
  ...
  if (err && (csr_check(CSR_ALLOW_ANY_RECOVERY_OS) == 0)) {
    AUTHPRNT(
        "CSR_ALLOW_ANY_RECOVERY_OS set, allowing unauthenticated root image");
    err = 0;
    enforced = FALSE;
  }
#endif
...
int authenticate_libkern_uuid(void) {
  ...
  if (fatfile_validate_fatarches((vm_offset_t)buf, bufsz, fsize) ==
      LOAD_SUCCESS) {
    struct fat_header *fat_header = buf;
    struct fat_arch fat_arch;
    if (fatfile_getbestarch((vm_offset_t)fat_header, bufsz, NULL, &fat_arch,
                            FALSE) != LOAD_SUCCESS) {
      err = EINVAL;
      goto out;
    }
    ...
    err = imageboot_read_file_from_offset(libkern_path, fat_arch.offset, &buf,
                                          &bufsz);
    if (err) {
      goto out;
    }
