...
int kdebug_storage_lock(struct kd_control *kd_ctrl_page) {
  int intrs_en = ml_set_interrupts_enabled(false);
  lck_spin_lock_grp(&kd_ctrl_page->kdc_storage_lock, &kdebug_lck_grp);
  return intrs_en;
}

void kdebug_storage_unlock(struct kd_control *kd_ctrl_page, int intrs_en) {
...
__startup_func void kdebug_startup(void) {
  lck_spin_init(&kd_control_trace.kdc_storage_lock, &kdebug_lck_grp,
                LCK_ATTR_NULL);
  lck_spin_init(&kd_control_triage.kdc_storage_lock, &kdebug_lck_grp,
                LCK_ATTR_NULL);
 ...
 * the kernel intact. TRIAGE mode will explicitly only use
 * mach_continuous_time() for its timestamp.
 */
bool kdebug_using_continuous_time(void) {
  return kd_control_trace.kdc_flags & KDBG_CONTINUOUS_TIME;
}

uint64_t kdebug_timestamp(void) {
...
int create_buffers(struct kd_control *kd_ctrl_page,
                   struct kd_buffer *kd_data_page, vm_tag_t tag) {
  unsigned int i;
  ...
  p_buffer_size = (kdb_storage_count % N_STORAGE_UNITS_PER_BUFFER) *
  ...
  if (kd_ctrl_page->kdebug_kdcopybuf_size > 0 &&
  ...
  for (i = 0; i < f_buffers; i++) {
    if (kmem_alloc(kernel_map, (vm_offset_t *)&kd_bufs[i].kdr_addr,
                   (vm_size_t)f_buffer_size, KMA_DATA | KMA_ZERO,
                   tag) != KERN_SUCCESS) {
      error = ENOSPC;
      goto out;
    }
  ...
  if (p_buffer_size) {
    if (kmem_alloc(kernel_map, (vm_offset_t *)&kd_bufs[i].kdr_addr,
                   (vm_size_t)p_buffer_size, KMA_DATA | KMA_ZERO,
                   tag) != KERN_SUCCESS) {
      error = ENOSPC;
      goto out;
    }
  ...
  for (i = 0; i < kd_data_page->kdb_region_count; i++) {
    ...
    assert(kd_bufs[i].kdr_size <=
           N_STORAGE_UNITS_PER_BUFFER * sizeof(struct kd_storage));
...
void delete_buffers(struct kd_control *kd_ctrl_page,
  ...
  if (kd_bufs) {
    for (i = 0; i < kdb_region_count; i++) {
      if (kd_bufs[i].kdr_addr) {
        kmem_free(kernel_map, (vm_offset_t)kd_bufs[i].kdr_addr,
                  (vm_size_t)kd_bufs[i].kdr_size);
      }
    }
  ...
  if (kd_data_page->kdcopybuf) {
    kmem_free(kernel_map, (vm_offset_t)kd_data_page->kdcopybuf,
              kd_ctrl_page->kdebug_kdcopybuf_size);
...
static void _register_out_of_space(struct kd_control *kd_ctrl_page) {
...
bool kdebug_storage_alloc(struct kd_control *kd_ctrl_page,
  ...
  if (kdbp->kd_list_tail.raw != KDS_PTR_NULL) {
    ...
    if (kdsp_actual->kds_bufindx <
        kd_ctrl_page->kdebug_events_per_storage_unit) {
      goto out;
    }
  }
  ...
  if ((kdsp = kd_ctrl_page->kds_free_list).raw != KDS_PTR_NULL) {
    ...
    for (kdbp_try = &kdbip[0]; kdbp_try < &kdbip[kd_ctrl_page->kdebug_cpus];
         kdbp_try++) {
       ...
       */
      ts = kdbg_get_timestamp(
          &kdsp_actual
               ->kds_records[kd_ctrl_page->kdebug_events_per_storage_unit - 1]);
      if (ts < oldest_ts) {
        oldest_ts = ts;
        kdbp_vict = kdbp_try;
      }
