...
#define BLK_MAX(mp)                                                            \
  ((mp->mnt_vfsstat.f_blocks * mp->mnt_vfsstat.f_bsize) /                      \
   (mp->mnt_devblocksize))
...
struct _disk_conditioner_info_t {
  ...
  struct saved_mount_fields
      mnt_fields; // fields to restore in mount_t when conditioner is disabled

  daddr64_t
      last_blkno; // approx. last transfered block for simulating seek times
...
static double weighted_scale_factor(double scale) {
  // 0 to 1 increasing quickly from 0. This weights smaller blkdiffs higher to
  // add a type of minimum latency I would like to use log(10) / 2.0 + 1, but
  // using different approximation due to no math library y = (x-1)^3 + 1
  double x_m1 = scale - 1;
  return x_m1 * x_m1 * x_m1 + 1;
}

void disk_conditioner_delay(buf_t bp, int extents, int total_size,
  ...
  if (!info->is_ssd) {
    ...
    blkdiff = bp->b_blkno > last_blkno ? bp->b_blkno - last_blkno
                                       : last_blkno - bp->b_blkno;
    internal_info->last_blkno = bp->b_blkno + bp->b_bcount;
  } else {
    blkdiff = BLK_MAX(mp);
  }

  // scale access time by (distance in blocks from previous I/O / maximum
  // blocks)
  access_time_scale =
      weighted_scale_factor((double)blkdiff / (double)BLK_MAX(mp));
  if (__builtin_isnan(access_time_scale)) {
    return;
  }
  // most cases should pass in extents==1 for optimal delay calculation,
  // otherwise just multiply delay by extents
  double temp =
      (((double)extents * (double)info->access_time_usec) * access_time_scale);
  if (temp <= 0) {
    delay_usec = 0;
  } else if (temp >=
             (double)(18446744073709549568ULL)) { /* highest 64-bit unsigned
                                                     integer representable as a
                                                     double */
  ...
  if (info->read_throughput_mbps && (bp->b_flags & B_READ)) {
    delay_usec +=
        (uint64_t)(total_size /
                   ((double)(info->read_throughput_mbps * 1024 * 1024 / 8) /
                    USEC_PER_SEC));
  } else if (info->write_throughput_mbps && !(bp->b_flags & B_READ)) {
    delay_usec +=
        (uint64_t)(total_size /
                   ((double)(info->write_throughput_mbps * 1024 * 1024 / 8) /
                    USEC_PER_SEC));
  }
  ...
  if (!info->is_ssd) {
    ...
    // avoid this delay right after boot (assuming last_io_timestamp is 0 and
    // disk is already spinning)
    if (elapsed.tv_sec > DISK_IDLE_SEC &&
        internal_info->last_io_timestamp.tv_sec != 0) {
...
int disk_conditioner_get_info(mount_t mp, disk_conditioner_info *uinfo) {
...
static inline void
disk_conditioner_restore_mount_fields(mount_t mp,
...
int disk_conditioner_set_info(mount_t mp, disk_conditioner_info *uinfo) {
  struct _disk_conditioner_info_t *internal_info;
  disk_conditioner_info *info;
  struct saved_mount_fields *mnt_fields;

  if (!kauth_cred_issuser(kauth_cred_get()) ||
      !IOCurrentTaskHasEntitlement(DISK_CONDITIONER_SET_ENTITLEMENT)) {
    return EPERM;
  }
  ...
  if (!internal_info) {
    internal_info =
        kalloc_type(struct _disk_conditioner_info_t, Z_WAITOK | Z_ZERO);
  ...
  if (uinfo->ioqueue_depth == 0 ||
      uinfo->ioqueue_depth > mnt_fields->mnt_ioqueue_depth) {
    info->ioqueue_depth = mnt_fields->mnt_ioqueue_depth;
  }
  if (uinfo->maxreadcnt == 0 ||
      uinfo->maxreadcnt > mnt_fields->mnt_maxreadcnt) {
    info->maxreadcnt = mnt_fields->mnt_maxreadcnt;
  }
  if (uinfo->maxwritecnt == 0 ||
      uinfo->maxwritecnt > mnt_fields->mnt_maxwritecnt) {
    info->maxwritecnt = mnt_fields->mnt_maxwritecnt;
  }
  if (uinfo->segreadcnt == 0 ||
      uinfo->segreadcnt > mnt_fields->mnt_segreadcnt) {
    info->segreadcnt = mnt_fields->mnt_segreadcnt;
  }
  if (uinfo->segwritecnt == 0 ||
      uinfo->segwritecnt > mnt_fields->mnt_segwritecnt) {
...
boolean_t disk_conditioner_mount_is_ssd(mount_t mp) {
  struct _disk_conditioner_info_t *internal_info =
      mp->mnt_disk_conditioner_info;
