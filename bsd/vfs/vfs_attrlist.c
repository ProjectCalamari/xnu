...
static int readdirattr(vnode_t, struct fd_vn_data *, uio_t, struct attrlist *,
                       uint64_t, int *, int *, vfs_context_t ctx)
    __attribute__((noinline));

static void vattr_get_alt_data(vnode_t, struct attrlist *, struct vnode_attr *,
                               int, int, int, vfs_context_t)
    __attribute__((noinline));

static void get_error_attributes(vnode_t, struct attrlist *, uint64_t,
                                 user_addr_t, size_t, int, caddr_t,
                                 vfs_context_t) __attribute__((noinline));

static int getvolattrlist(vfs_context_t, vnode_t, struct attrlist *,
                          user_addr_t, size_t, uint64_t, enum uio_seg, int)
    __attribute__((noinline));
...
#define _ATTRLIST_BUF_INIT(a)                                                  \
  do {                                                                         \
    (a)->base = (a)->fixedcursor = (a)->varcursor = NULL;                      \
    (a)->allocated = (a)->needed = 0l;                                         \
    ATTRIBUTE_SET_INIT(&((a)->actual));                                        \
    ATTRIBUTE_SET_INIT(&((a)->valid));                                         \
  } while (0)

static int attrlist_build_path(vnode_t vp, char **outbuf, int *outbuflen,
                               int *outpathlen, char *prefix, int prefix_len,
                               int flags) {
  ...
  do {
    ...
    err = build_path(vp, buf + prefix_len, buflen - prefix_len, &retlen, flags,
                     vfs_context_current());
  } while (err == ENOSPC && proc_support_long_paths(p) && (buflen *= 2) &&
           buflen <= MAXLONGPATHLEN);
  if (err == 0) {
    if (outbuf) {
      *outbuf = buf;
    }
 ...
 */
static void attrlist_pack_fixed(struct _attrlist_buf *ab, void *source,
                                ssize_t count) {
  /*
 ...
 * buffer.  If we are trying to pack two variable width attributes, they are
 * treated as a single variable-width attribute from the POV of the system call
 * caller.
 *
 * Recall that a variable-width attribute has two components: the fixed-width
 * attribute that tells the caller where to look, and the actual variable width
 * data.
 */
static void attrlist_pack_variable2(struct _attrlist_buf *ab,
                                    const void *source, ssize_t count,
   ...
   *
   * Then pack the first variable attribute as space allows.  Note that we
   * advance the variable cursor only if we we had some available space.
   */
...
/*
 * Packing a single variable-width attribute is the same as calling the two, but
 * with an invalid 2nd attribute.
 */
static void attrlist_pack_variable(struct _attrlist_buf *ab, const void *source,
                                   ssize_t count) {
  attrlist_pack_variable2(ab, source, count, NULL, 0);
}

/*
 * Attempt to pack a string. This is a special case of a variable width
 * attribute.
 *
 * If "source" is NULL, then an empty string ("") will be packed.  If "source"
 * is not NULL, but "count" is zero, then "source" is assumed to be a
 * NUL-terminated C-string.  If "source" is not NULL and "count" is not zero,
 * then only the first "count" bytes of "source" will be copied, and a NUL
 * terminator will be added.
 *
 * If the attrlist buffer doesn't have enough room to hold the entire string
 * (including NUL terminator), then copy as much as will fit.  The attrlist
 * buffer's "varcursor" will always be updated based on the entire length of the
 * string (including NUL terminator); this means "varcursor" may end up pointing
 * beyond the end of the allocated buffer space.
 */
static void attrlist_pack_string(struct _attrlist_buf *ab, const char *source,
                                 size_t count) {
  struct attrreference ar;
  ssize_t fit, space;
   ...
   *       buffer is large enough, this will be the string's length; otherwise,
   * it will be equal to "space".
   */
  ...
  if (space > 0) {
    ...
    if (space > fit) {
       ...
       */
      bytes_to_zero = lmin((roundup(fit, 4) - fit), space - fit);
      if (bytes_to_zero) {
...
#define ATTR_PACK_TIME(b, v, is64)                                             \
  do {                                                                         \
    if (is64) {                                                                \
      ...
      struct user32_timespec us = {.tv_sec = (user32_time_t)v.tv_sec,          \
                                   .tv_nsec = (user32_long_t)v.tv_nsec};       \
      ATTR_PACK(&b, us);                                                       \
    }                                                                          \
  } while (0)
...
static struct getvolattrlist_attrtab getvolattrlist_common_tab[] = {
    ...
    {.attr = ATTR_CMN_CRTIME,
     .bits = VFSATTR_BIT(f_create_time),
     .size = ATTR_TIME_SIZE},
    {.attr = ATTR_CMN_MODTIME,
     .bits = VFSATTR_BIT(f_modify_time),
     .size = ATTR_TIME_SIZE},
    {.attr = ATTR_CMN_CHGTIME,
     .bits = VFSATTR_BIT(f_modify_time),
     .size = ATTR_TIME_SIZE},
    {.attr = ATTR_CMN_ACCTIME,
     .bits = VFSATTR_BIT(f_access_time),
     .size = ATTR_TIME_SIZE},
    {.attr = ATTR_CMN_BKUPTIME,
     .bits = VFSATTR_BIT(f_backup_time),
     .size = ATTR_TIME_SIZE},
    ...
    {.attr = ATTR_CMN_EXTENDED_SECURITY,
     .bits = 0,
    ...
    {.attr = ATTR_CMN_RETURNED_ATTRS,
     .bits = 0,
     .size = sizeof(attribute_set_t)},
...
static struct getvolattrlist_attrtab getvolattrlist_vol_tab[] = {
    ...
    {.attr = ATTR_VOL_FSTYPENAME,
     .bits = 0,
     .size = sizeof(struct attrreference)},
    {.attr = ATTR_VOL_FSSUBTYPE, .bits = 0, .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_OWNER, .bits = 0, .size = sizeof(uid_t)},
    {.attr = ATTR_VOL_SIGNATURE,
     .bits = VFSATTR_BIT(f_signature),
     .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_SIZE,
     .bits = VFSATTR_BIT(f_blocks) | VFSATTR_BIT(f_bsize),
     .size = sizeof(off_t)},
    {.attr = ATTR_VOL_SPACEFREE,
     .bits = VFSATTR_BIT(f_bfree) | VFSATTR_BIT(f_bsize),
     .size = sizeof(off_t)},
    {.attr = ATTR_VOL_SPACEAVAIL,
     .bits = VFSATTR_BIT(f_bavail) | VFSATTR_BIT(f_bsize),
     .size = sizeof(off_t)},
    {.attr = ATTR_VOL_MINALLOCATION,
     .bits = VFSATTR_BIT(f_bsize),
     .size = sizeof(off_t)},
    {.attr = ATTR_VOL_ALLOCATIONCLUMP,
     .bits = VFSATTR_BIT(f_bsize),
     .size = sizeof(off_t)},
    {.attr = ATTR_VOL_IOBLOCKSIZE,
     .bits = VFSATTR_BIT(f_iosize),
     .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_OBJCOUNT,
     .bits = VFSATTR_BIT(f_objcount),
     .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_FILECOUNT,
     .bits = VFSATTR_BIT(f_filecount),
     .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_DIRCOUNT,
     .bits = VFSATTR_BIT(f_dircount),
     .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_MAXOBJCOUNT,
     .bits = VFSATTR_BIT(f_maxobjcount),
     .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_MOUNTPOINT,
     .bits = 0,
     .size = sizeof(struct attrreference)},
    {.attr = ATTR_VOL_NAME,
     .bits = VFSATTR_BIT(f_vol_name),
     .size = sizeof(struct attrreference)},
    {.attr = ATTR_VOL_MOUNTFLAGS, .bits = 0, .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_MOUNTEDDEVICE,
     .bits = 0,
     .size = sizeof(struct attrreference)},
    {.attr = ATTR_VOL_ENCODINGSUSED, .bits = 0, .size = sizeof(uint64_t)},
    {.attr = ATTR_VOL_CAPABILITIES,
     .bits = VFSATTR_BIT(f_capabilities),
     .size = sizeof(vol_capabilities_attr_t)},
    {.attr = ATTR_VOL_UUID,
     .bits = VFSATTR_BIT(f_uuid),
     .size = sizeof(uuid_t)},
    {.attr = ATTR_VOL_MOUNTEXTFLAGS, .bits = 0, .size = sizeof(uint32_t)},
    {.attr = ATTR_VOL_SPACEUSED,
     .bits = VFSATTR_BIT(f_bused) | VFSATTR_BIT(f_bsize) | VFSATTR_BIT(f_bfree),
     .size = sizeof(off_t)},
    {.attr = ATTR_VOL_QUOTA_SIZE,
     .bits = VFSATTR_BIT(f_quota) | VFSATTR_BIT(f_bsize),
     .size = sizeof(off_t)},
    {.attr = ATTR_VOL_RESERVED_SIZE,
     .bits = VFSATTR_BIT(f_reserved) | VFSATTR_BIT(f_bsize),
     .size = sizeof(off_t)},
    {.attr = ATTR_VOL_ATTRIBUTES,
     .bits = VFSATTR_BIT(f_attributes),
     .size = sizeof(vol_attributes_attr_t)},
    {.attr = ATTR_VOL_INFO, .bits = 0, .size = 0},
    {.attr = 0, .bits = 0, .size = 0}};

static int getvolattrlist_parsetab(struct getvolattrlist_attrtab *tab,
                                   attrgroup_t attrs, struct vfs_attr *vsp,
 ...
 */
static int getvolattrlist_setupvfsattr(struct attrlist *alp,
                                       struct vfs_attr *vsp, ssize_t *sizep,
                                       int is_64bit) {
  int error;
  if (!alp) {
    return EINVAL;
  ...
  *sizep = sizeof(uint32_t); /* length count */
  if (alp->commonattr) {
    ...
    if ((error = getvolattrlist_parsetab(
             getvolattrlist_common_tab, alp->commonattr, vsp, sizep, is_64bit,
             sizeof(getvolattrlist_common_tab) /
                 sizeof(getvolattrlist_common_tab[0]))) != 0) {
      return error;
    }
  }
  if (alp->volattr &&
      (error = getvolattrlist_parsetab(
           getvolattrlist_vol_tab, alp->volattr, vsp, sizep, is_64bit,
           sizeof(getvolattrlist_vol_tab) /
               sizeof(getvolattrlist_vol_tab[0]))) != 0) {
    return error;
  }
 ...
 */
static void getvolattrlist_fixupattrs(attribute_set_t *asp,
                                      struct vfs_attr *vsp) {
  ...
  if (asp->commonattr) {
    ...
    do {
      if ((tab->attr & asp->commonattr) && (tab->bits != 0) &&
  ...
  if (asp->volattr) {
    ...
    do {
      if ((tab->attr & asp->volattr) && (tab->bits != 0) &&
          ((tab->bits & vsp->f_supported) == 0)) {
...
/*
 * Table-driven setup for all valid common/dir/file/fork attributes against
 * files.
 */
...
/*
 * A zero after the ATTR_ bit indicates that we don't expect the underlying FS
 * to report back with this information, and we will synthesize it at the VFS
 * level.
 */
static struct getattrlist_attrtab getattrlist_common_tab[] = {
    {.attr = ATTR_CMN_NAME,
     .bits = VATTR_BIT(va_name),
     .size = sizeof(struct attrreference),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_DEVID,
     .bits = VATTR_BIT(va_fsid),
     .size = sizeof(dev_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_FSID,
     .bits = VATTR_BIT(va_fsid64),
     .size = sizeof(fsid_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_OBJTYPE,
     .bits = 0,
     .size = sizeof(fsobj_type_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_OBJTAG,
     .bits = 0,
     .size = sizeof(fsobj_tag_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_OBJID,
     .bits = VATTR_BIT(va_fileid) | VATTR_BIT(va_linkid),
     .size = sizeof(fsobj_id_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_OBJPERMANENTID,
     .bits = VATTR_BIT(va_fileid) | VATTR_BIT(va_linkid),
     .size = sizeof(fsobj_id_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_PAROBJID,
     .bits = VATTR_BIT(va_parentid),
     .size = sizeof(fsobj_id_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_SCRIPT,
     .bits = VATTR_BIT(va_encoding),
     .size = sizeof(text_encoding_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_CRTIME,
     .bits = VATTR_BIT(va_create_time),
     .size = ATTR_TIME_SIZE,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_MODTIME,
     .bits = VATTR_BIT(va_modify_time),
     .size = ATTR_TIME_SIZE,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_CHGTIME,
     .bits = VATTR_BIT(va_change_time),
     .size = ATTR_TIME_SIZE,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_ACCTIME,
     .bits = VATTR_BIT(va_access_time),
     .size = ATTR_TIME_SIZE,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_BKUPTIME,
     .bits = VATTR_BIT(va_backup_time),
     .size = ATTR_TIME_SIZE,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_FNDRINFO,
     ...
     .size = 32,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_OWNERID,
     .bits = VATTR_BIT(va_uid),
     .size = sizeof(uid_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_GRPID,
     .bits = VATTR_BIT(va_gid),
     .size = sizeof(gid_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_ACCESSMASK,
     .bits = VATTR_BIT(va_mode),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_FLAGS,
     .bits = VATTR_BIT(va_flags),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_GEN_COUNT,
     .bits = VATTR_BIT(va_write_gencount),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_DOCUMENT_ID,
     .bits = VATTR_BIT(va_document_id),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_USERACCESS,
     .bits = 0,
     .size = sizeof(uint32_t),
     .action = 0},
    {.attr = ATTR_CMN_EXTENDED_SECURITY,
     .bits = VATTR_BIT(va_acl),
     .size = sizeof(struct attrreference),
     .action = KAUTH_VNODE_READ_SECURITY},
    {.attr = ATTR_CMN_UUID,
     .bits = VATTR_BIT(va_uuuid),
     .size = sizeof(guid_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_GRPUUID,
     .bits = VATTR_BIT(va_guuid),
     .size = sizeof(guid_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_FILEID,
     .bits = VATTR_BIT(va_fileid),
     .size = sizeof(uint64_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_PARENTID,
     .bits = VATTR_BIT(va_parentid),
     .size = sizeof(uint64_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_FULLPATH,
     .bits = 0,
     .size = sizeof(struct attrreference),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_ADDEDTIME,
     .bits = VATTR_BIT(va_addedtime),
     .size = ATTR_TIME_SIZE,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_RETURNED_ATTRS,
     .bits = 0,
    ...
    {.attr = ATTR_CMN_DATA_PROTECT_FLAGS,
     .bits = VATTR_BIT(va_dataprotect_class),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = 0, .bits = 0, .size = 0, .action = 0}};

static struct getattrlist_attrtab getattrlist_dir_tab[] = {
    {.attr = ATTR_DIR_LINKCOUNT,
     .bits = VATTR_BIT(va_dirlinkcount),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_DIR_ENTRYCOUNT,
     .bits = VATTR_BIT(va_nchildren),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_DIR_MOUNTSTATUS,
     .bits = 0,
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_DIR_ALLOCSIZE,
     .bits = VATTR_BIT(va_total_alloc) | VATTR_BIT(va_total_size),
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_DIR_IOBLOCKSIZE,
     .bits = VATTR_BIT(va_iosize),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_DIR_DATALENGTH,
     .bits = VATTR_BIT(va_total_size) | VATTR_BIT(va_data_size),
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = 0, .bits = 0, .size = 0, .action = 0}};
static struct getattrlist_attrtab getattrlist_file_tab[] = {
    {.attr = ATTR_FILE_LINKCOUNT,
     .bits = VATTR_BIT(va_nlink),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_TOTALSIZE,
     .bits = VATTR_BIT(va_total_size),
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_ALLOCSIZE,
     .bits = VATTR_BIT(va_total_alloc) | VATTR_BIT(va_total_size),
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_IOBLOCKSIZE,
     .bits = VATTR_BIT(va_iosize),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_CLUMPSIZE,
     .bits = 0,
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_DEVTYPE,
     .bits = VATTR_BIT(va_rdev),
     .size = sizeof(dev_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_DATALENGTH,
     .bits = VATTR_BIT(va_total_size) | VATTR_BIT(va_data_size),
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_DATAALLOCSIZE,
     .bits = VATTR_BIT(va_total_alloc) | VATTR_BIT(va_data_alloc),
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_RSRCLENGTH,
     .bits = 0,
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_RSRCALLOCSIZE,
     .bits = 0,
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = 0, .bits = 0, .size = 0, .action = 0}};
...
static struct getattrlist_attrtab getattrlist_common_extended_tab[] = {
    {.attr = ATTR_CMNEXT_RELPATH,
     .bits = 0,
     .size = sizeof(struct attrreference),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_PRIVATESIZE,
     .bits = VATTR_BIT(va_private_size),
     .size = sizeof(off_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_LINKID,
     .bits = VATTR_BIT(va_fileid) | VATTR_BIT(va_linkid),
     .size = sizeof(uint64_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_NOFIRMLINKPATH,
     .bits = 0,
     .size = sizeof(struct attrreference),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_REALDEVID,
     .bits = VATTR_BIT(va_devid),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_REALFSID,
     .bits = VATTR_BIT(va_fsid64),
     .size = sizeof(fsid_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_CLONEID,
     .bits = VATTR_BIT(va_clone_id),
     .size = sizeof(uint64_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_EXT_FLAGS,
     .bits = VATTR_BIT(va_extflags),
     .size = sizeof(uint64_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_RECURSIVE_GENCOUNT,
     .bits = VATTR_BIT(va_recursive_gencount),
     .size = sizeof(uint64_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_ATTRIBUTION_TAG,
     .bits = VATTR_BIT(va_attribution_tag),
     .size = sizeof(uint64_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMNEXT_CLONE_REFCNT,
     .bits = VATTR_BIT(va_clone_refcnt),
     .size = sizeof(uint32_t),
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = 0, .bits = 0, .size = 0, .action = 0}};
...
static struct getattrlist_attrtab getattrlistbulk_common_tab[] = {
    ...
    {.attr = ATTR_CMN_FSID,
     ...
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_OBJTYPE,
     .bits = VATTR_BIT(va_objtype),
     .size = 0,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_OBJTAG,
     .bits = VATTR_BIT(va_objtag),
     .size = 0,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_CMN_USERACCESS,
     .bits = VATTR_BIT(va_user_access),
     .size = 0,
     .action = 0},
    {.attr = ATTR_CMN_FNDRINFO,
     .bits = VATTR_BIT(va_finderinfo),
     .size = 0,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = 0, .bits = 0, .size = 0, .action = 0}};
...
static struct getattrlist_attrtab getattrlistbulk_file_tab[] = {
    {.attr = ATTR_FILE_RSRCLENGTH,
     .bits = VATTR_BIT(va_rsrc_length),
     .size = 0,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = ATTR_FILE_RSRCALLOCSIZE,
     .bits = VATTR_BIT(va_rsrc_alloc),
     .size = 0,
     .action = KAUTH_VNODE_READ_ATTRIBUTES},
    {.attr = 0, .bits = 0, .size = 0, .action = 0}};
 ...
 *
 * A majority of them are the same attributes that are required for stat(2) and
 * statfs(2).
 */
#define VFS_DFLT_ATTR_VOL                                                      \
  (ATTR_VOL_FSTYPE | ATTR_VOL_FSTYPENAME | ATTR_VOL_SIGNATURE |                \
   ATTR_VOL_SIZE | ATTR_VOL_SPACEFREE | ATTR_VOL_QUOTA_SIZE |                  \
   ATTR_VOL_RESERVED_SIZE | ATTR_VOL_SPACEAVAIL | ATTR_VOL_MINALLOCATION |     \
   ATTR_VOL_ALLOCATIONCLUMP | ATTR_VOL_IOBLOCKSIZE | ATTR_VOL_MOUNTPOINT |     \
   ATTR_VOL_MOUNTFLAGS | ATTR_VOL_MOUNTEDDEVICE | ATTR_VOL_CAPABILITIES |      \
   ATTR_VOL_ATTRIBUTES | ATTR_VOL_ENCODINGSUSED | ATTR_VOL_MOUNTEXTFLAGS)

#define VFS_DFLT_ATTR_CMN                                                      \
  (ATTR_CMN_NAME | ATTR_CMN_DEVID | ATTR_CMN_FSID | ATTR_CMN_OBJTYPE |         \
   ATTR_CMN_OBJTAG | ATTR_CMN_OBJID | ATTR_CMN_PAROBJID | ATTR_CMN_SCRIPT |    \
   ...
   ATTR_CMN_RETURNED_ATTRS | ATTR_CMN_DOCUMENT_ID | ATTR_CMN_GEN_COUNT |       \
   ATTR_CMN_DATA_PROTECT_FLAGS)

#define VFS_DFLT_ATTR_CMN_EXT                                                  \
  (ATTR_CMNEXT_PRIVATESIZE | ATTR_CMNEXT_LINKID | ATTR_CMNEXT_NOFIRMLINKPATH | \
   ATTR_CMNEXT_REALDEVID | ATTR_CMNEXT_REALFSID | ATTR_CMNEXT_CLONEID |        \
   ATTR_CMNEXT_EXT_FLAGS)

#define VFS_DFLT_ATTR_DIR (ATTR_DIR_LINKCOUNT | ATTR_DIR_MOUNTSTATUS)

#define VFS_DFLT_ATTR_FILE                                                     \
  (ATTR_FILE_LINKCOUNT | ATTR_FILE_TOTALSIZE | ATTR_FILE_ALLOCSIZE |           \
   ATTR_FILE_IOBLOCKSIZE | ATTR_FILE_DEVTYPE | ATTR_FILE_DATALENGTH |          \
   ATTR_FILE_DATAALLOCSIZE | ATTR_FILE_RSRCLENGTH | ATTR_FILE_RSRCALLOCSIZE)

static int getattrlist_parsetab(struct getattrlist_attrtab *tab,
                                attrgroup_t attrs, struct vnode_attr *vap,
                                ssize_t *sizep, kauth_action_t *actionp,
 ...
 * the data from a filesystem.
 */
static int getattrlist_setupvattr(struct attrlist *alp, struct vnode_attr *vap,
                                  ssize_t *sizep, kauth_action_t *actionp,
                                  int is_64bit, int isdir, int use_fork) {
  int error;
  ...
  *actionp = 0;
  if (alp->commonattr && (error = getattrlist_parsetab(
                              getattrlist_common_tab, alp->commonattr, vap,
                              sizep, actionp, is_64bit,
                              sizeof(getattrlist_common_tab) /
                                  sizeof(getattrlist_common_tab[0]))) != 0) {
    return error;
  }
  if (isdir && alp->dirattr &&
      (error = getattrlist_parsetab(
           getattrlist_dir_tab, alp->dirattr, vap, sizep, actionp, is_64bit,
  ...
  if (!isdir && alp->fileattr &&
      (error = getattrlist_parsetab(
           getattrlist_file_tab, alp->fileattr, vap, sizep, actionp, is_64bit,
  ...
  if (use_fork && alp->forkattr &&
      (error = getattrlist_parsetab(
           getattrlist_common_extended_tab, alp->forkattr, vap, sizep, actionp,
           is_64bit,
           sizeof(getattrlist_common_extended_tab) /
               sizeof(getattrlist_common_extended_tab[0]))) != 0) {
    return error;
  }
 ...
 * the data from a filesystem.
 */
static int getattrlist_setupvattr_all(struct attrlist *alp,
                                      struct vnode_attr *vap,
                                      enum vtype obj_type, ssize_t *fixedsize,
                                      int is_64bit, int use_fork) {
  int error = 0;
  ...
  if (alp->commonattr) {
    error = getattrlist_parsetab(
        getattrlist_common_tab, alp->commonattr, vap, fixedsize, NULL, is_64bit,
        sizeof(getattrlist_common_tab) / sizeof(getattrlist_common_tab[0]));

    if (!error) {
      /* Ignore any errrors from the bulk table */
      (void)getattrlist_parsetab(getattrlistbulk_common_tab, alp->commonattr,
                                 vap, fixedsize, NULL, is_64bit,
                                 sizeof(getattrlistbulk_common_tab) /
                                     sizeof(getattrlistbulk_common_tab[0]));
    }
  }

  if (!error && (obj_type == VNON || obj_type == VDIR) && alp->dirattr) {
    error = getattrlist_parsetab(
        getattrlist_dir_tab, alp->dirattr, vap, fixedsize, NULL, is_64bit,
        sizeof(getattrlist_dir_tab) / sizeof(getattrlist_dir_tab[0]));
  }

  if (!error && (obj_type != VDIR) && alp->fileattr) {
    error = getattrlist_parsetab(
        getattrlist_file_tab, alp->fileattr, vap, fixedsize, NULL, is_64bit,
        sizeof(getattrlist_file_tab) / sizeof(getattrlist_file_tab[0]));

    if (!error) {
      /*Ignore any errors from the bulk table */
      (void)getattrlist_parsetab(getattrlistbulk_file_tab, alp->fileattr, vap,
                                 fixedsize, NULL, is_64bit,
                                 sizeof(getattrlistbulk_file_tab) /
                                     sizeof(getattrlistbulk_file_tab[0]));
    }
  }
  ...
  if (!error && use_fork && alp->forkattr) {
    error =
        getattrlist_parsetab(getattrlist_common_extended_tab, alp->forkattr,
                             vap, fixedsize, NULL, is_64bit,
                             sizeof(getattrlist_common_extended_tab) /
                                 sizeof(getattrlist_common_extended_tab[0]));

    if (!error) {
      (void)getattrlist_parsetab(
          getattrlistbulk_common_extended_tab, alp->forkattr, vap, fixedsize,
          NULL, is_64bit,
          sizeof(getattrlistbulk_common_extended_tab) /
              sizeof(getattrlistbulk_common_extended_tab[0]));
    }
  }
...
int vfs_setup_vattr_from_attrlist(struct attrlist *alp, struct vnode_attr *vap,
                                  enum vtype obj_vtype,
                                  ssize_t *attrs_fixed_sizep,
                                  vfs_context_t ctx) {
  VATTR_INIT(vap);

  // the caller passes us no options, we assume the caller wants the new fork
  // attr behavior, hence the hardcoded 1
  return getattrlist_setupvattr_all(alp, vap, obj_vtype, attrs_fixed_sizep,
                                    vfs_context_is64bit(ctx), 1);
}
 ...
 * missing attributes from the file system
 */
static void getattrlist_fixupattrs(attribute_set_t *asp, struct vnode_attr *vap,
                                   int use_fork) {
  ...
  if (asp->commonattr) {
    ...
    do {
       ...
       *
       * This array is full of structs that have 4 fields (attr, bits, size,
       * action). The first is used to store the ATTR_CMN_* bit that was being
       * requested from userland.  The second stores the VATTR_BIT corresponding
       * to the field filled in vnode_attr struct.  If it is 0, then we don't
       * typically expect the filesystem to fill in this field.  The third is
       * the size of the field, and the fourth is the type of kauth actions
       * needed.
       *
       * So, for all of the ATTR_CMN bits listed in this array, we iterate
       * through them, and check to see if it was both passed down to the
       * filesystem via the va_active bitfield, and whether or not we expect it
       * to be emitted from the filesystem.  If it wasn't supported, then we
       * un-twiddle the bit and move on.  This is done so that we can uncheck
       * those bits and re-request a vnode_getattr from the filesystem again.
       */
      if ((tab->attr & asp->commonattr) && (tab->bits & vap->va_active) &&
  ...
  if (asp->dirattr) {
    ...
    do {
      if ((tab->attr & asp->dirattr) && (tab->bits & vap->va_active) &&
  ...
  if (asp->fileattr) {
    ...
    do {
      if ((tab->attr & asp->fileattr) && (tab->bits & vap->va_active) &&
          (vap->va_supported & tab->bits) == 0) {
  ...
  if (use_fork && asp->forkattr) {
    ...
    do {
      if ((tab->attr & asp->forkattr) && (tab->bits & vap->va_active) &&
          (vap->va_supported & tab->bits) == 0) {
...
static int setattrlist_setfinderinfo(vnode_t vp, char *fndrinfo,
                                     struct vfs_context *ctx) {
  uio_t auio;
  UIO_STACKBUF(uio_buf, 1);
  int error;

  if ((auio = uio_createwithbuffer(1, 0, UIO_SYSSPACE, UIO_WRITE, uio_buf,
                                   sizeof(uio_buf))) == NULL) {
    error = ENOMEM;
  } else {
...
/*
 * Find something resembling a terminal component name in the mountedonname for
 * vp
 *
 */
static void getattrlist_findnamecomp(const char *mn, const char **np,
                                     ssize_t *nl) {
  int counting;
  const char *cp;
...
static int getvolattrlist(vfs_context_t ctx, vnode_t vp, struct attrlist *alp,
                          user_addr_t attributeBuffer, size_t bufferSize,
                          uint64_t options, enum uio_seg segflg, int is_64bit) {
  struct vfs_attr vs = {};
  ...
  if (!mnt || mnt == dead_mountp) {
    ...
    error = EBADF;
    VFS_DEBUG(ctx, vp,
              "ATTRLIST - ERROR: volume attributes requested on dead mount.");
    goto out;
  }
  ...
  if (!vnode_isvroot(vp)) {
    if (mnt != NULL) {
      ...
      if (error) {
        VFS_DEBUG(ctx, vp,
                  "ATTRLIST - ERROR: volume attributes requested on non-root "
                  "vnode, but got an error getting root.");
        goto out;
      }
      vp = root_vp;
    } else {
      ...
      VFS_DEBUG(ctx, vp,
                "ATTRLIST - ERROR: volume attributes requested on non-root "
                "vnode, but no backpointer to mount.");
      goto out;
    }
  }
   ...
   */
  if ((error = getvolattrlist_setupvfsattr(alp, &vs, &fixedsize, is_64bit)) !=
      0) {
  ...
  if (vs.f_active != 0) {
    ...
    VFS_DEBUG(ctx, vp,
              "ATTRLIST -       calling to get %016llx with supported %016llx",
              vs.f_active, vs.f_supported);
    ...
    if (!VFSATTR_ALL_SUPPORTED(&vs)) {
      ...
      if (VFSATTR_IS_ACTIVE(&vs, f_capabilities)) {
        ...
        if (!VFSATTR_IS_SUPPORTED(&vs, f_capabilities)) {
          ...
          vs.f_capabilities.capabilities[VOL_CAPABILITIES_INTERFACES] =
          ...
          vs.f_capabilities.valid[VOL_CAPABILITIES_INTERFACES] =
              VOL_CAP_INT_ATTRLIST;
  ...
  if (alp->commonattr &
    ...
    if ((error = vnode_getattr(vp, &va, ctx)) != 0) {
      VFS_DEBUG(ctx, vp,
                "ATTRLIST - ERROR: could not fetch attributes from root vnode",
                vp);
      goto out;
    }
#if CONFIG_MACF
    ...
    if (error != 0) {
      VFS_DEBUG(ctx, vp,
                "ATTRLIST - ERROR: MAC framework returned %d for root vnode",
                error);
      goto out;
    }
#endif
  ...
  if (alp->commonattr & ATTR_CMN_NAME) {
    if (vp->v_mount->mnt_vfsstat.f_mntonname[1] == 0x00 &&
      ...
      getattrlist_findnamecomp(vp->v_mount->mnt_vfsstat.f_mntonname, &cnp,
                               &cnl);
    }
    if (alp->commonattr & ATTR_CMN_NAME) {
  ...
  if (return_valid &&
    ...
    if (UIO_SEG_IS_USER_SPACE(segflg)) {
      error = copyout(&num_bytes_valid, CAST_USER_ADDR_T(attributeBuffer),
                      num_bytes_valid);
    } else {
      bcopy(&num_bytes_valid, (void *)attributeBuffer, (size_t)num_bytes_valid);
    }
    goto out;
  }
  ...
  if (ab.base == NULL) {
    error = ENOMEM;
    VFS_DEBUG(ctx, vp,
              "ATTRLIST - ERROR: could not allocate %d for copy buffer",
              ab.allocated);
    goto out;
  }
...
#define HFS_GET_BOOT_INFO (FCNTL_FS_SPECIFIC_BASE + 0x00004)
  ...
  if (alp->commonattr &
      ATTR_CMN_USERACCESS) { /* XXX this is expensive and also duplicate work */
    uint32_t perms = 0;
    if (vnode_isdir(vp)) {
      ...
      if (vnode_authorize(vp, NULL,
                          KAUTH_VNODE_ACCESS | KAUTH_VNODE_LIST_DIRECTORY,
                          ctx) == 0) {
        perms |= R_OK;
      }
      if (vnode_authorize(vp, NULL, KAUTH_VNODE_ACCESS | KAUTH_VNODE_SEARCH,
                          ctx) == 0) {
        perms |= X_OK;
      }
    } else {
      if (vnode_authorize(vp, NULL, KAUTH_VNODE_ACCESS | KAUTH_VNODE_WRITE_DATA,
                          ctx) == 0) {
        perms |= W_OK;
      }
      if (vnode_authorize(vp, NULL, KAUTH_VNODE_ACCESS | KAUTH_VNODE_READ_DATA,
                          ctx) == 0) {
        perms |= R_OK;
      }
      if (vnode_authorize(vp, NULL, KAUTH_VNODE_ACCESS | KAUTH_VNODE_EXECUTE,
                          ctx) == 0) {
        perms |= X_OK;
      }
    }
  ...
  if (alp->volattr & ATTR_VOL_CAPABILITIES) {
     ...
     */
    if (!(vs.f_capabilities.valid[VOL_CAPABILITIES_FORMAT] &
          VOL_CAP_FMT_NO_IMMUTABLE_FILES)) {
      vs.f_capabilities.capabilities[VOL_CAPABILITIES_FORMAT] &=
          ~VOL_CAP_FMT_NO_IMMUTABLE_FILES;
      vs.f_capabilities.valid[VOL_CAPABILITIES_FORMAT] |=
          VOL_CAP_FMT_NO_IMMUTABLE_FILES;
    }
    ...
    /*
     * ATTR_CMN_USERACCESS attribute was previously set by file-system drivers,
     * thus volume capabilitiy VOL_CAP_INT_USERACCESS was conditionally enabled.
     * ATTR_CMN_USERACCESS is now set inside VFS, regardless of underlying
     * volume type thus we always set VOL_CAP_INT_USERACCESS.
     */
    vs.f_capabilities.capabilities[VOL_CAPABILITIES_INTERFACES] |=
  ...
  if (alp->volattr & ATTR_VOL_ATTRIBUTES) {
    ...
    if (vfs_extendedsecurity(mnt)) {
      vs.f_attributes.validattr.commonattr |=
          (ATTR_CMN_EXTENDED_SECURITY | ATTR_CMN_UUID | ATTR_CMN_GRPUUID);
    } else {
      vs.f_attributes.validattr.commonattr &=
          ~(ATTR_CMN_EXTENDED_SECURITY | ATTR_CMN_UUID | ATTR_CMN_GRPUUID);
      vs.f_attributes.nativeattr.commonattr &=
          ~(ATTR_CMN_EXTENDED_SECURITY | ATTR_CMN_UUID | ATTR_CMN_GRPUUID);
    }
  ...
  if (!return_valid && (ab.fixedcursor - ab.base) != fixedsize) {
    panic("packed field size mismatch; allocated %ld but packed %ld for common "
          "%08x vol %08x",
          fixedsize, (long)(ab.fixedcursor - ab.base), alp->commonattr,
          alp->volattr);
  }
  if (!return_valid && ab.varcursor != (ab.base + ab.needed)) {
    panic("packed variable field size mismatch; used %ld but expected %ld",
          (long)(ab.varcursor - ab.base), ab.needed);
  }

  /*
   * In the compatible case, we report the smaller of the required and returned
   * sizes. If the FSOPT_REPORT_FULLSIZE option is supplied, we report the full
   * (required) size of the result buffer, even if we copied less out.  The
   * caller knows how big a buffer they gave us, so they can always check for
   * truncation themselves.
   */
  *(uint32_t *)ab.base = (options & FSOPT_REPORT_FULLSIZE)
                             ? (uint32_t)ab.needed
                             : (uint32_t)lmin(bufferSize, ab.needed);
  ...
  if (UIO_SEG_IS_USER_SPACE(segflg)) {
    ...
    bcopy(ab.base, (void *)attributeBuffer,
          (size_t)ulmin((uint32_t)bufferSize, (uint32_t)ab.needed));
  }
 ...
 * are in ad.
 */
static errno_t attr_pack_common(vfs_context_t ctx, mount_t mp, vnode_t vp,
                                struct attrlist *alp, struct _attrlist_buf *abp,
                                struct vnode_attr *vap, int proc_is64,
                                const char *cnp, ssize_t cnl,
                                const char *fullpathptr, ssize_t fullpathlen,
                                int return_valid, int pack_invalid, int vtype,
                                int is_bulk) {
  uint32_t perms = 0;
  int error = 0;

  if ((alp->commonattr & ATTR_CMN_ERROR) && (!return_valid || pack_invalid)) {
    ATTR_PACK4((*abp), 0);
  ...
  if (alp->commonattr & ATTR_CMN_DEVID) {
    if (mp) { /* caller needs real devid */
      ATTR_PACK4((*abp), mp->mnt_vfsstat.f_fsid.val[0]);
      abp->actual.commonattr |= ATTR_CMN_DEVID;
    } else if (VATTR_IS_ACTIVE(vap, va_fsid) &&
               VATTR_IS_SUPPORTED(vap, va_fsid)) {
      ATTR_PACK4((*abp), vap->va_fsid);
      abp->actual.commonattr |= ATTR_CMN_DEVID;
    } else if (vp) {
      ATTR_PACK4((*abp), vp->v_mount->mnt_vfsstat.f_fsid.val[0]);
      abp->actual.commonattr |= ATTR_CMN_DEVID;
  ...
  if (alp->commonattr & ATTR_CMN_USERACCESS) { /* this is expensive */
    if (vp && !is_bulk) {
      if (vtype == VDIR) {
        ...
        if (vnode_authorize(vp, NULL,
                            KAUTH_VNODE_ACCESS | KAUTH_VNODE_WRITE_DATA,
                            ctx) == 0) {
          perms |= W_OK;
        }
  ...
  if (alp->commonattr & ATTR_CMN_FNDRINFO) {
    size_t fisize =
        lmax(lmin(32, abp->allocated - (abp->fixedcursor - abp->base)), 0);
    ...
    if (vp && !is_bulk) {
      ...
      if ((auio = uio_createwithbuffer(1, 0, UIO_SYSSPACE, UIO_READ, uio_buf,
                                       sizeof(uio_buf))) == NULL) {
        error = ENOMEM;
        goto out;
      }
      uio_addiov(auio, CAST_USER_ADDR_T(abp->fixedcursor), fialloc);
      /* fisize may be reset to 0 after this call */
      error = vn_getxattr(vp, XATTR_FINDERINFO_NAME, auio, &fialloc,
                          XATTR_NOSECURITY, ctx);
      uio_free(auio);
       ...
       */
      if (error && (!return_valid || pack_invalid) &&
          ((error == ENOATTR) || (error == ENOENT) || (error == ENOTSUP) ||
           (error == EPERM))) {
        VFS_DEBUG(
            ctx, vp,
...
static errno_t attr_pack_dir(struct vnode *vp, struct attrlist *alp,
                             struct _attrlist_buf *abp, struct vnode_attr *vap,
                             int return_valid, int pack_invalid) {
  ...
  if (alp->dirattr & ATTR_DIR_MOUNTSTATUS) {
    ...
    if (vp) {
       ...
       */
      if (vp->v_resolve && !(vp->v_resolve->vr_flags & VNT_NO_DIRECT_MOUNT)) {
 ...
 */
static errno_t attr_pack_file(vfs_context_t ctx, struct vnode *vp,
                              struct attrlist *alp, struct _attrlist_buf *abp,
                              struct vnode_attr *vap, int return_valid,
  ...
  if (vp && !is_bulk &&
    ...
    error = vn_getxattr(vp, XATTR_RESOURCEFORK_NAME, NULL, &rsize,
                        XATTR_NOSECURITY, ctx);
    if (error) {
      if ((error == ENOENT) || (error == ENOATTR) || (error == ENOTSUP) ||
    ...
    if (alp->fileattr & (ATTR_FILE_RSRCALLOCSIZE | ATTR_FILE_ALLOCSIZE)) {
      uint32_t blksize;
   ...
   * We infer that if the filesystem does not support va_data_size or
   * va_data_alloc it must not know about alternate forks.  So when we need to
   * gather the total size or total alloc, it's OK to substitute the total size
   * for the data size below.  This is because it is likely a flat filesystem
   * and we must be using AD files to store the rsrc fork and EAs.
   *
   * Additionally, note that getattrlist is barred from being called on
   * resource fork paths. (Search for CN_ALLOWRSRCFORK).  So if the filesystem
   * does support va_data_size, it is guaranteed to represent the data fork's
   * size.  This is an important distinction to make because when we call
   * vnode_getattr on an HFS resource fork vnode, to get the size, it will vend
   * out the resource fork's size (it only gets the size of the passed-in
   * vnode).
   */
...
attr_pack_common_extended(mount_t mp, struct vnode *vp, struct attrlist *alp,
                          struct _attrlist_buf *abp, const char *relpathptr,
                          ssize_t relpathlen, const char *REALpathptr,
                          ssize_t REALpathlen, struct vnode_attr *vap,
                          int return_valid, int pack_invalid) {
...
static void vattr_get_alt_data(vnode_t vp, struct attrlist *alp,
                               struct vnode_attr *vap, int return_valid,
                               int is_bulk,
#if !CONFIG_FIRMLINKS
                               __unused
  ...
  if ((alp->commonattr & (ATTR_CMN_PAROBJID | ATTR_CMN_PARENTID)) && vp) {
     ...
     */
    if (!is_realdev && (vp->v_flag & VFMLINKTARGET) &&
        ((dvp = vp->v_fmlink) != NULL) && (vnode_get(dvp) == 0)) {
  ...
  if ((alp->commonattr & ATTR_CMN_SCRIPT) &&
    ...
    VATTR_RETURN(vap, va_encoding, 0x7e /* kTextEncodingMacUnicode */);
  }
   ...
   * mount point.
   */
  if ((alp->commonattr & ATTR_CMN_NAME) && !VATTR_IS_SUPPORTED(vap, va_name)) {
...
static errno_t calc_varsize(vnode_t vp, struct attrlist *alp,
                            struct vnode_attr *vap, ssize_t *varsizep,
                            struct _attrlist_paths *pathsp, const char **vnamep,
  ...
  *varsizep = 0; /* length count */
      ...
      if (vnode_isvroot(vp)) {
        if (vp->v_mount->mnt_vfsstat.f_mntonname[1] == 0x00 &&
          ...
          *cnlp = strlen(*cnpp);
          ...
          getattrlist_findnamecomp(vp->v_mount->mnt_vfsstat.f_mntonname, cnpp,
                                   cnlp);
        }
      } else {
        *cnpp = *vnamep = vnode_getname(vp);
  ...
  /*
   * Compute the full path to this vnode, if necessary. This attribute is almost
   * certainly not supported by any filesystem, so build the path to this vnode
   * at this time.
   */
  if (vp && (alp->commonattr & ATTR_CMN_FULLPATH)) {
    ...
    if (pathbuf) {
      err = lookup_check_for_resolve_prefix(pathbuf, PATHBUFLEN, PATHBUFLEN,
                                            &resolve_flags, &perfix_len);
      if (err) {
        error = err;
        goto out;
      }
    ...
    err = attrlist_build_path(vp, &(pathsp->fullpathptr), &buflen, &pathlen,
                              pathbuf, (int)perfix_len, 0);
    if (err) {
      error = err;
      goto out;
    }
  ...
  if (vp && (alp->forkattr & ATTR_CMNEXT_RELPATH)) {
    int pathlen;
    int buflen;
    int err = attrlist_build_path(vp, &(pathsp->relpathptr), &buflen, &pathlen,
                                  NULL, 0, BUILDPATH_VOLUME_RELATIVE);
    if (err) {
      error = err;
      goto out;
    }
  ...
  if (vp && (alp->forkattr & ATTR_CMNEXT_NOFIRMLINKPATH)) {
    int pathlen;
    int buflen;
    int err = attrlist_build_path(vp, &(pathsp->REALpathptr), &buflen, &pathlen,
                                  NULL, 0, BUILDPATH_NO_FIRMLINK);
    if (err) {
      error = err;
      goto out;
    }
  ...
  if ((alp->commonattr & ATTR_CMN_EXTENDED_SECURITY) &&
    ...
    /*
     * Since we have a kauth_acl_t (not a kauth_filesec_t), we have to check
     * against KAUTH_FILESEC_NOACL ourselves
     */
    if (vap->va_acl->acl_entrycount == KAUTH_FILESEC_NOACL) {
      *varsizep += roundup((KAUTH_FILESEC_SIZE(0)), 4);
    } else {
      *varsizep +=
          roundup((KAUTH_FILESEC_SIZE(vap->va_acl->acl_entrycount)), 4);
    }
  }
...
static errno_t vfs_attr_pack_internal(mount_t mp, vnode_t vp, uio_t auio,
                                      struct attrlist *alp, uint64_t options,
                                      struct vnode_attr *vap,
                                      __unused void *fndesc, vfs_context_t ctx,
                                      int is_bulk, enum vtype vtype,
                                      ssize_t fixedsize, char *pathbuf) {
  struct _attrlist_buf ab;
  struct _attrlist_paths apaths = {.fullpathptr = NULL,
                                   .fullpathlen = 0,
                                   .fullpathbuflen = 0,
                                   .relpathptr = NULL,
                                   .relpathlen = 0,
  ...
  if (vap->va_active &&
...
#if CONFIG_FIRMLINKS
       /* For firmlink targets we have to overide what the FS returned for
          parentid */
    ...
    vattr_get_alt_data(vp, alp, vap, return_valid, is_bulk, is_realdev, ctx);
    ...
    if (!VATTR_ALL_SUPPORTED(vap)) {
      if (return_valid && pack_invalid) {
        ...
        /* Adjust the requested attributes */
        getattrlist_fixupattrs((attribute_set_t *)&(alp->commonattr), vap,
                               use_fork);
      } else {
        error = EINVAL;
      }
   ...
   * Allocate a target buffer for attribute results.
   *
   * Note that we won't ever copy out more than the caller requested, even
   * though we might have to allocate more than they offer so that the
   * diagnostic checks don't result in a panic if the caller's buffer is too
   * small.
   */
  ab.allocated = fixedsize + varsize;
  ...
  /* common attributes ************************************************/
  error =
      attr_pack_common(ctx, (options & FSOPT_RETURN_REALDEV ? mp : NULL), vp,
                       alp, &ab, vap, proc_is64, cnp, cnl, fullpathptr,
                       fullpathlen, return_valid, pack_invalid, vtype, is_bulk);
  ...
  if (!error && alp->fileattr && (vtype != VDIR)) {
    error = attr_pack_file(ctx, vp, alp, &ab, vap, return_valid, pack_invalid,
                           is_bulk);
  }
  ...
  *(uint32_t *)ab.base = (options & FSOPT_REPORT_FULLSIZE)
                             ? (uint32_t)ab.needed
                             : (uint32_t)lmin(ab.allocated, ab.needed);
...
errno_t vfs_attr_pack_ext(mount_t mp, vnode_t vp, uio_t uio,
                          struct attrlist *alp, uint64_t options,
                          struct vnode_attr *vap, __unused void *fndesc,
                          vfs_context_t ctx) {
  int error;
  ssize_t fixedsize;
  ...
  error =
      vfs_attr_pack_internal(mp, vp, uio, alp, options | FSOPT_REPORT_FULLSIZE,
                             vap, NULL, ctx, 1, v_type, fixedsize, NULL);
...
errno_t vfs_attr_pack(vnode_t vp, uio_t uio, struct attrlist *alp,
                      uint64_t options, struct vnode_attr *vap,
                      __unused void *fndesc, vfs_context_t ctx) {
...
#define FAST_STATFS_CMN_ATTRS                                                  \
  (ATTR_CMN_RETURNED_ATTRS | ATTR_CMN_FSID /* f_fsid */)

#define FAST_STATFS_VOL_ATTRS                                                  \
  (ATTR_VOL_INFO | ATTR_VOL_FSTYPE /* f_type */ |                              \
   ATTR_VOL_MOUNTPOINT /* f_mntonname */ | ATTR_VOL_MOUNTFLAGS /* f_flags */ | \
   ATTR_VOL_MOUNTEDDEVICE /* f_mntfromname */ |                                \
   ATTR_VOL_FSTYPENAME /* f_fstypename */ |                                    \
   ATTR_VOL_FSSUBTYPE /* f_fssubtype */ |                                      \
   ATTR_VOL_MOUNTEXTFLAGS /* f_flags_ext */ | ATTR_VOL_OWNER /* f_owner */)
 ...
 *
 */
static int getattrlist_internal(vfs_context_t ctx, vnode_t vp,
                                struct attrlist *alp,
                                user_addr_t attributeBuffer, size_t bufferSize,
                                uint64_t options, enum uio_seg segflg,
                                char *authoritative_name,
                                struct ucred *file_cred, char *pathbuf) {
  struct vnode_attr *va;
  kauth_action_t action;
  ssize_t fixedsize;
  char *va_name;
  ...
  const int is_fast_statfs =
      (alp->volattr != 0 && alp->fileattr == 0 && alp->dirattr == 0 &&
       alp->forkattr == 0 && (alp->volattr & ~FAST_STATFS_VOL_ATTRS) == 0 &&
  ...
  auio = uio_createwithbuffer(1, 0, segflg, UIO_READ, &uio_buf[0],
                              sizeof(uio_buf));
  ...
  VFS_DEBUG(ctx, vp,
            "%p  ATTRLIST - %s request common %08x vol %08x file %08x dir %08x "
            "fork %08x %sfollow on '%s'",
            vp, vfs_context_proc(ctx)->p_comm, alp->commonattr, alp->volattr,
            alp->fileattr, alp->dirattr, alp->forkattr,
  ...
  if (alp->volattr) {
    ...
    error = getvolattrlist(ctx, vp, alp, attributeBuffer, bufferSize, options,
                           segflg, proc_is64);
    goto out;
  }
  ...
  if ((options & FSOPT_ATTR_CMN_EXTENDED) &&
      (alp->forkattr & (ATTR_FORK_VALIDMASK))) {
    error = EINVAL;
    goto out;
  }
   ...
   */
  if ((error = getattrlist_setupvattr(alp, va, &fixedsize, &action, proc_is64,
                                      (vtype == VDIR), use_fork)) != 0) {
    VFS_DEBUG(ctx, vp, "ATTRLIST - ERROR: setup for request failed");
    goto out;
  }
  ...
  if (va->va_active != 0) {
    ...
    if (VATTR_IS_ACTIVE(va, va_name)) {
       ...
       *
       * N.B. Since authoritative_name implies this is coming from
       * getattrlistbulk, we know the name is authoritative. For /dev/fd, we
       * want to use the file descriptor as the name not the underlying name of
       * the associate vnode in a particular file system.
       */
  ...
  error = vfs_attr_pack_internal(vp->v_mount, vp, auio, alp, options, va, NULL,
                                 ctx, 0, vtype, fixedsize, pathbuf);
...
int fgetattrlist(proc_t p, struct fgetattrlist_args *uap,
  ...
  error = getattrlist_internal(
      ctx, vp, &al, uap->attributeBuffer, uap->bufferSize, uap->options,
      (IS_64BIT_PROCESS(p) ? UIO_USERSPACE64 : UIO_USERSPACE32), NULL,
...
static int getattrlistat_internal(vfs_context_t ctx, user_addr_t path,
                                  struct attrlist *alp,
                                  user_addr_t attributeBuffer,
                                  size_t bufferSize, uint64_t options,
                                  enum uio_seg segflg, enum uio_seg pathsegflg,
                                  int fd) {
  struct nameidata nd;
  vnode_t vp;
  int32_t nameiflags;
  ...
  NDINIT(&nd, LOOKUP, OP_GETATTR, nameiflags, pathsegflg, path, ctx);
  ...
  error = getattrlist_internal(ctx, vp, alp, attributeBuffer, bufferSize,
                               options, segflg, NULL, NOCRED, nd.ni_pathbuf);
...
int getattrlist(proc_t p, struct getattrlist_args *uap,
                __unused int32_t *retval) {
  enum uio_seg segflg;
  struct attrlist al;
  int error;
  ...
  return getattrlistat_internal(
      vfs_context_current(), CAST_USER_ADDR_T(uap->path), &al,
      CAST_USER_ADDR_T(uap->attributeBuffer), uap->bufferSize,
      (uint64_t)uap->options, segflg, segflg, AT_FDCWD);
}

int getattrlistat(proc_t p, struct getattrlistat_args *uap,
                  __unused int32_t *retval) {
  enum uio_seg segflg;
  struct attrlist al;
  int error;
 ...
 * refill.
 */
static int refill_fd_direntries(vfs_context_t ctx, vnode_t dvp,
                                struct fd_vn_data *fvd, int *eofflagp) {
  uio_t rdir_uio;
  UIO_STACKBUF(uio_buf, 1);
  size_t rdirbufsiz;
  size_t rdirbufused;
  ...
  *eofflagp = 0;
  ...
  rdir_uio = uio_createwithbuffer(1, 0, UIO_SYSSPACE, UIO_READ, &uio_buf[0],
                                  sizeof(uio_buf));
...
retry_alloc:
  ...
  eofflag = 0;
  nentries = 0;
  error = vnode_readdir64(dvp, rdir_uio, VNODE_READDIR_EXTENDED, &eofflag,
                          &nentries, ctx);
 ...
 */
static int get_direntry(vfs_context_t ctx, vnode_t dvp, struct fd_vn_data *fvd,
 ...
 */
static void direntry_done(struct fd_vn_data *fvd) {
  struct direntry *dp;
 ...
 */
static void get_error_attributes(vnode_t vp, struct attrlist *alp,
                                 uint64_t options, user_addr_t kern_attr_buf,
                                 size_t kern_attr_buf_siz, int error_attr,
   ...
   */
  fsiz = 0;
  (void)getattrlist_setupvattr(
      &al, NULL, (ssize_t *)&fsiz, &action, proc_is64bit(vfs_context_proc(ctx)),
...
#define MIN_BUF_SIZE_REQUIRED                                                  \
  (sizeof(uint32_t) + sizeof(attribute_set_t) + sizeof(attrreference_t))

/*
 * Read directory entries and get attributes filled in for each directory
 */
static int readdirattr(vnode_t dvp, struct fd_vn_data *fvd, uio_t auio,
                       struct attrlist *alp, uint64_t options, int *count,
                       int *eofflagp, vfs_context_t ctx) {
  ...
  *eofflagp = 0;
  ...
  while (uio_resid(auio) > (user_ssize_t)MIN_BUF_SIZE_REQUIRED) {
     ...
     *
     * Note that we supply NOCROSSMOUNT to the namei call as we attempt to
     * acquire a vnode for this particular entry.  This is because the native
     * call will (likely) attempt to emit attributes based on its own metadata
     * in order to avoid creating vnodes where posssible.  If the native call is
     * not going to  walk up the vnode mounted-on chain in order to find the
     * top-most mount point, then we should not either in this emulated
     * readdir+getattrlist() approach.  We will be responsible for setting
     * DIR_MNTSTATUS_MNTPOINT on that directory that contains a mount point.
     */
    ...
    error = getattrlist_internal(
        ctx, vp, &al, CAST_USER_ADDR_T(kern_attr_buf), kern_attr_buf_siz,
        options | FSOPT_REPORT_FULLSIZE, UIO_SYSSPACE,
        CAST_DOWN_EXPLICIT(char *, name_buffer), NOCRED, NULL);

    nameidone(&nd);

    if (error) {
      get_error_attributes(vp, alp, options, CAST_USER_ADDR_T(kern_attr_buf),
                           kern_attr_buf_siz, error, (caddr_t)name_buffer, ctx);
      error = 0;
    }
    ...
    if (pad_bytes && (entlen + pad_bytes <= bytes_left)) {
      ...
      if (entlen + pad_bytes <= kern_attr_buf_siz) {
        ...
        bzero(kern_attr_buf + entlen, kern_attr_buf_siz - entlen);
        /*
         ...
         * always positive.
         */
        new_resid = (ssize_t)(bytes_left - (entlen + pad_bytes));
      }
      entlen += pad_bytes;
    }
    *((uint32_t *)kern_attr_buf) = (uint32_t)entlen;
    error =
        uiomove(kern_attr_buf, min((int)entlen, (int)kern_attr_buf_siz), auio);
 ...
 */
int getattrlistbulk(proc_t p, struct getattrlistbulk_args *uap,
                    int32_t *retval) {
  struct attrlist al;
  vnode_t dvp = NULLVP;
  struct fileproc *fp;
  ...
  *retval = 0;
  ...
  /*
   * Requested attributes that are available in the direntry struct, with the
   * addition of ATTR_CMN_RETURNED_ATTRS and ATTR_CMN_ERROR, can be let past
   * with just LIST_DIRECTORY. Any other requested attributes require SEARCH as
   * well.
   */
  ...
  if ((al.commonattr &
    ...
    if (fvdata->fv_eofflag && !fvdata->fv_buf) {
      ...
      (void)getattrlist_setupvattr_all(
          &al, va, VNON, NULL, IS_64BIT_PROCESS(p),
          (uap->options & FSOPT_ATTR_CMN_EXTENDED));
      ...
      error = VNOP_GETATTRLISTBULK(dvp, &al, va, auio, NULL, options, &eofflag,
                                   &count, ctx);
      ut->uu_flag &= ~UT_KERN_RAGE_VNODES;
  ...
  if (error == ENOTSUP) {
    ...
    ut->uu_flag |= UT_KERN_RAGE_VNODES;
    error = readdirattr(dvp, fvdata, auio, &al, options, &count, &eofflag, ctx);
    ut->uu_flag &= ~UT_KERN_RAGE_VNODES;
  }
...
static int attrlist_unpack_fixed(char **cursor, char *end, void *buf,
                                 ssize_t size) {
...
#define ATTR_UNPACK(v)                                                         \
  do {                                                                         \
    if ((error = attrlist_unpack_fixed(&cursor, bufend, &v, sizeof(v))) != 0)  \
      goto out;                                                                \
  } while (0);
#define ATTR_UNPACK_CAST(t, v)                                                 \
  do {                                                                         \
    t _f;                                                                      \
    ATTR_UNPACK(_f);                                                           \
    v = (typeof(v))_f;                                                         \
 ...
 * Write attributes.
 */
static int setattrlist_internal(vnode_t vp, struct setattrlist_args *uap,
                                proc_t p, vfs_context_t ctx) {
  struct attrlist al;
  struct vnode_attr va;
  struct attrreference ar;
  kauth_action_t action;
  ...
  VFS_DEBUG(ctx, vp,
            "%p  ATTRLIST - %s set common %08x vol %08x file %08x dir %08x "
            "fork %08x %sfollow on '%s'",
            vp, p->p_comm, al.commonattr, al.volattr, al.fileattr, al.dirattr,
            al.forkattr, (uap->options & FSOPT_NOFOLLOW) ? "no" : "",
            vp->v_name);

  if (al.volattr) {
    if ((al.volattr & ~ATTR_VOL_SETMASK) ||
        (al.commonattr & ~ATTR_CMN_VOLSETMASK) || al.fileattr || al.forkattr) {
      error = EINVAL;
      VFS_DEBUG(ctx, vp,
   ...
   * then exit early.
   */
  if (al.commonattr == 0 && (al.volattr & ~ATTR_VOL_INFO) == 0 &&
      al.dirattr == 0 && al.fileattr == 0 && al.forkattr == 0) {
    error = 0;
    goto out;
  }
   ...
   * size.  We could be more careful by pulling in the fixed-size region,
   * checking the attrref structures, then pulling in the variable section. We
   * need to reconsider this for handling large ACLs, as they should probably be
   * brought directly into a buffer.  Multiple copyins will make this slower
   * though.
   *
   * We could also map the user buffer if it is larger than some sensible
   * mimimum.
   */
  if (uap->bufferSize > attr_max_buffer) {
    VFS_DEBUG(ctx, vp, "ATTRLIST - ERROR: buffer size %d too large",
              uap->bufferSize);
    error = ENOMEM;
    goto out;
  }
  ...
  if (user_buf == NULL) {
    VFS_DEBUG(ctx, vp,
              "ATTRLIST - ERROR: could not allocate %d bytes for buffer",
              uap->bufferSize);
    error = ENOMEM;
    goto out;
  }
  ...
  VFS_DEBUG(ctx, vp, "ATTRLIST - copied in %d bytes of user attributes to %p",
            uap->bufferSize, user_buf);
  ...
  if (al.commonattr & ATTR_CMN_EXTENDED_SECURITY) {
    ...
    if (ar.attr_dataoffset < 0) {
      VFS_DEBUG(ctx, vp, "ATTRLIST - ERROR: bad offset supplied",
                ar.attr_dataoffset);
      error = EINVAL;
      goto out;
    }
    ...
    if (((((char *)rfsec) + KAUTH_FILESEC_SIZE(0)) >
         bufend) ||                                   /* no space for acl */
        (rfsec->fsec_magic != KAUTH_FILESEC_MAGIC) || /* bad magic */
        (KAUTH_FILESEC_COPYSIZE(rfsec) !=
         ar.attr_length) || /* size does not match */
        ((cp + KAUTH_FILESEC_COPYSIZE(rfsec)) >
         bufend)) { /* ACEs overrun buffer */
      error = EINVAL;
    ...
    if (rfsec->fsec_entrycount == KAUTH_FILESEC_NOACL) {
      ...
      VATTR_SET(&va, va_acl, NULL);
               ...
               KAUTH_ACL_MAX_ENTRIES) { /* ACL size invalid */
      error = EINVAL;
  ...
  if (al.volattr & ATTR_VOL_INFO) {
    if (al.volattr & ATTR_VOL_NAME) {
      ...
      if ((ar.attr_dataoffset < 0) || (ar.attr_length == 0) ||
        ...
        VFS_DEBUG(ctx, vp, "ATTRLIST - ERROR: bad offset supplied (2) ",
                  ar.attr_dataoffset);
        error = EINVAL;
        goto out;
      }
  ...
  action = 0;
  if ((va.va_active != 0LL) &&
      ((error = vnode_authattr(vp, &va, &action, ctx)) != 0)) {
    VFS_DEBUG(ctx, vp, "ATTRLIST - ERROR: attribute changes refused: %d",
              error);
    goto out;
  }
  /*
   * We can auth file Finder Info here.  HFS volume FinderInfo is really boot
   * data, and will be auth'ed by the FS.
   */
  if (fndrinfo != NULL) {
    if (al.volattr & ATTR_VOL_INFO) {
      if (vp->v_tag != VT_HFS) {
...
int setattrlist(proc_t p, struct setattrlist_args *uap,
                __unused int32_t *retval) {
  struct vfs_context *ctx;
  struct nameidata nd;
  vnode_t vp = NULL;
...
int setattrlistat(proc_t p, struct setattrlistat_args *uap,
                  __unused int32_t *retval) {
...
int fsetattrlist(proc_t p, struct fsetattrlist_args *uap,
                 __unused int32_t *retval) {
  struct vfs_context *ctx;
  vnode_t vp = NULL;
  int error;
