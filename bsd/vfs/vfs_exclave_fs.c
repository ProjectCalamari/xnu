...
typedef LIST_HEAD(registered_tags_head,
                  registered_fs_tag) registered_tags_head_t;
...
#define OPEN_VNODES_HASH(dev, file_id)                                         \
  (&open_vnodes_hashtbl[HASHFUNC(dev, file_id)])
...
static int exclave_fs_open_internal(uint32_t fs_tag, uint64_t root_id,
                                    ...
                                    uint32_t ov_flags, uint64_t *file_id);
static int vfs_exclave_fs_unregister_internal(vnode_t vp,
                                              bool take_basedir_lock);

static uint32_t hash_fstag(uint32_t tag) { return tag % (rft_hashmask + 1); }

static registered_tags_head_t *get_registered_tags_chain(uint32_t tag) {
 ...
 */
static int get_vnode_info(vnode_t vp, dev_t *dev, fsid_t *fsid,
                          uint64_t *file_id) {
...
static inline bool is_graft(registered_fs_tag_t *rft) {
  return rft->flags & VFS_EXCLAVE_FS_BASE_DIR_GRAFT;
}

static inline bool is_sealed(registered_fs_tag_t *rft) {
  return rft->flags & VFS_EXCLAVE_FS_BASE_DIR_SEALED;
}

static int graft_to_host_inum(fsioc_graft_info_t *gi, uint64_t graft_inum,
                              uint64_t *host_inum) {
...
static int host_to_graft_inum(fsioc_graft_info_t *gi, uint64_t host_inum,
                              uint64_t *graft_inum) {
  if (host_inum == gi->gi_graft_dir) {
    *graft_inum = ROOT_DIR_INO_NUM;
  } else if ((host_inum >= gi->gi_inum_base) &&
             (host_inum < gi->gi_inum_base + gi->gi_inum_len)) {
...
/*
 * Check if a vnode is in an APFS graft and if so obtain information about the
 * graft.
 */
static int get_graft_info(vnode_t vp, bool *is_graft,
                          fsioc_graft_info_t *graft_info) {
  ...
  *is_graft = false;
  ...
  error = VNOP_IOCTL(vp, FSIOC_GET_GRAFT_INFO, (caddr_t)&ggi, 0,
                     vfs_context_kernel());
  if (error) {
    return error;
  }
...
static bool is_fs_writeable(uint32_t fs_tag) {
 ...
 */
static bool is_parent_registered(vnode_t base_vp) {
  ...
  while (vp != NULLVP) {
    ...
    for (i = 0; i <= rft_hashmask; i++) {
      ...
      LIST_FOREACH(rft, head, link) {
        if (rft->vp == vp) {
          printf("vfs_exclave_fs: vnode [%s] has an ancestor which is a "
                 "registered base_dir [%s], fstag %d\n",
 ...
 */
static int set_base_dir(uint32_t fs_tag, vnode_t vp,
                        fsioc_graft_info_t *graft_info, bool is_sealed) {
  dev_t dev;
  int error = 0;
  registered_fs_tag_t *rft;
  ...
  LIST_FOREACH(rft, rfthead, link) {
    if (rft->fstag == fs_tag) {
      // Check if the registered vp is DEAD, it can be the case in edu mode
      // where the original location was unmounted if the vnode is dead
      // unregister it, and continue with setting new base_dir
 ...
 */
static int get_base_dir(uint32_t fs_tag, registered_fs_tag_t *base_dir,
                        vnode_t *vpp) {
  int error = ENOENT;
  registered_fs_tag_t *rft;
...
int vfs_exclave_fs_start(void) {
  ...
  open_vnodes_hashtbl =
      hashinit(open_vnodes_hashsize, M_VNODE, &open_vnodes_hashmask);
  if (open_vnodes_hashtbl == NULL) {
    open_vnodes_hashsize = open_vnodes_hashmask = 0;
    return ENOMEM;
  }

  registered_tags_hash =
      hashinit(REGFSTAG_HASH_WIDTH, M_VNODE /*unused*/, &rft_hashmask);
  ...
  if (PE_parse_boot_argn(EXCLAVE_INTEGRITY_CHECKS_DISABLED_BOOTARG,
                         &bootarg_val, sizeof(bootarg_val))) {
    if (bootarg_val) {
...
static bool exclave_fs_started(void) { return open_vnodes_hashtbl != NULL; }

static void release_open_vnodes(registered_fs_tag_t *);

static void drop_registered_tag(registered_fs_tag_t *rft) {
...
void vfs_exclave_fs_stop(void) {
  ...
  for (i = 0; i <= rft_hashmask; i++) {
    ...
    LIST_FOREACH_SAFE(rft, rfthead, link, nxt) { drop_registered_tag(rft); }
  }
...
int vfs_exclave_fs_register(uint32_t fs_tag, vnode_t vp) {
  ...
  if (error) {
    vnode_rele(vp);
    // if this directory is already registered in this tag do not consider it as
    // an error
    if (error == EALREADY) {
      error = 0;
    }
 ...
 */
static void release_open_vnodes(registered_fs_tag_t *base_dir) {
  int i;
...
int vfs_exclave_fs_get_base_dirs(void *buf, uint32_t *count) {
  ...
  for (i = 0; i <= rft_hashmask; i++) {
    ...
    LIST_FOREACH(base_dir, rfthead, link) {
      ...
      error = get_vnode_info(base_dir->vp, NULL, &out_dir->fsid,
                             &out_dir->base_dir);
      if (error) {
        goto out;
      }
      ...
      out_dir->graft_file =
          is_graft(base_dir) ? base_dir->graft_info.gi_graft_file : 0;
      num_copied++;
    }
  }
...
static int create_exclave_dir(vnode_t base_vp, const char *exclave_id) {
...
int vfs_exclave_fs_root_ex(uint32_t fs_tag, const char *exclave_id,
  ...
  if (strchr(exclave_id, '/') || !strcmp(exclave_id, ".") ||
      !strcmp(exclave_id, "..")) {
  ...
  if (error == ENOENT) {
    ...
    if (!error) {
      error =
          exclave_fs_open_internal(fs_tag, EXCLAVE_FS_BASEDIR_ROOT_ID,
                                   exclave_id, O_DIRECTORY, ov_flags, root_id);
    }
 ...
 */
static int get_open_vnode(registered_fs_tag_t *base_dir, uint64_t file_id,
                          vnode_t *vpp, uint32_t *ov_flags) {
  uint64_t vp_file_id;
 ...
 */
static int increment_vnode_open_count(vnode_t vp, registered_fs_tag_t *base_dir,
                                      uint64_t file_id, uint32_t flags) {
  struct open_vnode *entry;
 ...
 * If base dir is a graft, file_id should be the graft inode number.
 */
static int decrement_vnode_open_count(registered_fs_tag_t *base_dir,
                                      uint64_t file_id, vnode_t *vpp) {
  struct open_vnode *entry;
  vnode_t vp;
  uint64_t vp_file_id;
  int error = 0;
...
static int exclave_fs_open_internal(uint32_t fs_tag, uint64_t root_id,
  ...
  // if we need to create the file, then delete it first (so that we won't reuse
  // the same inode number)
  if ((flags & O_CREAT) && !(flags & O_DIRECTORY)) {
    error = unlink1(vfs_context_kernel(), dvp, CAST_USER_ADDR_T(path),
                    UIO_SYSSPACE, 0);
    if (error) {
      if (error == ENOENT) {
        error = 0;
    ...
    // Add an O_EXCL flag so that create will fail if the file is already there
    // after delete (a possible attack)
    flags |= O_EXCL;
  }
...
static int exclave_fs_io(uint32_t fs_tag, uint64_t file_id, uint64_t offset,
                         uint64_t length, uint8_t *data, bool read) {
  vnode_t vp = NULLVP;
  registered_fs_tag_t base_dir;
  ...
  auio =
      uio_createwithbuffer(1, offset, UIO_SYSSPACE, read ? UIO_READ : UIO_WRITE,
...
int vfs_exclave_fs_write(uint32_t fs_tag, uint64_t file_id,
  ...
  return exclave_fs_io(fs_tag, file_id, file_offset, length, (void *)data,
                       false);
}
...
int vfs_exclave_fs_remove(uint32_t fs_tag, uint64_t root_id, const char *name) {
  ...
  error = unlink1(vfs_context_kernel(), rvp, CAST_USER_ADDR_T(name),
                  UIO_SYSSPACE, 0);
...
static int map_graft_dirents(fsioc_graft_info_t *graft_info, void *dirent_buf,
                             int32_t count) {
  int i, error = 0;
...
int vfs_exclave_fs_readdir(uint32_t fs_tag, uint64_t file_id, void *dirent_buf,
  ...
  error =
      VNOP_GETATTRLISTBULK(dvp, &al, vap, auio, NULL, 0, &eofflag, count, ctx);
  ut->uu_flag &= ~UT_KERN_RAGE_VNODES;
...
SYSCTL_STRING(_kern, OID_AUTO, enospc_exclaves, CTLFLAG_RW | CTLFLAG_LOCKED,
              enospc_exclaves, sizeof(enospc_exclaves),
              "List of comma-separated exclave_ids for writing immediately "
              "returns ENOSPC");
