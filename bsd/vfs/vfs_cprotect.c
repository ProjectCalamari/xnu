...
#endif
  aes_encrypt_ctx
      *cpx_iv_aes_ctx_ptr; // Pointer to context used for generating the IV
...
ZONE_DEFINE(cpx_zone, "cpx", sizeof(struct fcpx), ZC_ZFREE_CLEARMEM);
ZONE_DEFINE(aes_ctz_zone, "AES ctx", sizeof(aes_encrypt_ctx),
            ZC_ZFREE_CLEARMEM);
...
size_t cpx_size(size_t key_len) {
...
cpx_t cpx_alloc(size_t key_len, bool needs_ctx) {
  cpx_t cpx = NULL;
...
#pragma unused(key_len, needs_ctx)
  ...
  // the actual key array is fixed-length, but the amount of usable content can
  // vary, via 'key_len'
...
void cpx_writeprotect(cpx_t cpx) {
  ...
  if (cpx->cpx_flags & CPX_WRITE_PROTECTABLE) {
    mach_vm_protect(kernel_map, (vm_map_offset_t)cpxstart, PAGE_SIZE, false,
                    (VM_PROT_READ));
  }
#else
  (void)cpx;
...
void cpx_free(cpx_t cpx) {
  ...
  if (cpx->cpx_flags & CPX_WRITE_PROTECTABLE) {
    ...
    // If we are here, then we used kmem_alloc to get the page. Must use
    // kmem_free to drop it.
...
uint16_t cpx_max_key_len(const struct cpx *cpx) { return cpx->cpx_max_key_len; }

uint16_t cpx_key_len(const struct cpx *cpx) { return cpx->cpx_key_len; }
...
bool cpx_has_key(const struct cpx *cpx) { return cpx->cpx_key_len > 0; }

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wcast-qual"
void *cpx_key(const struct cpx *cpx) { return (void *)cpx->cpx_cached_key; }
...
static int cp_vfs_callback(mount_t mp, void *arg) {
  ...
  case (CP_TYPE_LOCK_STATE):
    VFS_IOCTL(mp, FIODEVICELOCKED, (void *)(uintptr_t)callback_arg->lock_state,
              0, vfs_context_kernel());
    break;
  case (CP_TYPE_EP_STATE):
    VFS_IOCTL(mp, FIODEVICEEPSTATE, (void *)(uintptr_t)callback_arg->ep_state,
              0, vfs_context_kernel());
    break;
  case (CP_TYPE_CX_STATE):
    VFS_IOCTL(mp, FIODEVICECXSTATE, (void *)(uintptr_t)callback_arg->cx_state,
              0, vfs_context_kernel());
    break;
  default:
    break;
...
int cp_key_store_action(cp_key_store_action_t action) {
  ...
  case CP_ACTION_UNLOCKED:
    ...
    callback_arg.lock_state =
        (action == CP_ACTION_LOCKED ? CP_LOCKED_STATE : CP_UNLOCKED_STATE);
 ...
 */
static cp_key_os_version_t parse_os_version(const char *vers) {
  const char *p = vers;
