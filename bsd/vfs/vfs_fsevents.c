...
_Static_assert(offsetof(struct regular_event, uid) == offsetof(fse_info, uid),
               "kfs_event and fse_info out-of-sync");
_Static_assert(offsetof(struct regular_event, document_id) ==
                   offsetof(fse_info, document_id),
               "kfs_event and fse_info out-of-sync");
...
#define WATCHER_APPLE_SYSTEM_SERVICE                                           \
  0x0010 // fseventsd, coreservicesd, mds, revisiond
...
static LCK_RW_DECLARE_ATTR(
    event_handling_lock, // handles locking for event manipulation and recycling
    &fsevent_rw_group, &fsevent_lock_attr);
static LCK_MTX_DECLARE_ATTR(watch_table_lock, &fsevent_mutex_group,
                            &fsevent_lock_attr);
static LCK_MTX_DECLARE_ATTR(event_buf_lock, &fsevent_mutex_group,
                            &fsevent_lock_attr);
static LCK_MTX_DECLARE_ATTR(event_writer_lock, &fsevent_mutex_group,
                            &fsevent_lock_attr);
...
static int is_ignored_directory(const char *path) {
  if (!path) {
    return 0;
  }

#define IS_TLD(x) strnstr(__DECONST(char *, path), x, MAXPATHLEN)
  if (IS_TLD("/.Spotlight-V100/") || IS_TLD("/.MobileBackups/") ||
...
static void fsevents_internal_init(void) {
  int i;
  ...
  event_zone = zone_create_ext(
      "fs-event-buf", sizeof(kfs_event), ZC_NOGC | ZC_NOCALLOUT, ZONE_ID_ANY,
      ^(zone_t z) {
...
static void lock_watch_table(void) { lck_mtx_lock(&watch_table_lock); }

static void unlock_watch_table(void) { lck_mtx_unlock(&watch_table_lock); }

static void lock_fs_event_list(void) { lck_mtx_lock(&event_buf_lock); }

static void unlock_fs_event_list(void) { lck_mtx_unlock(&event_buf_lock); }

// forward prototype
static void release_event_ref(kfs_event *kfse);

static boolean_t watcher_cares_about_dev(fs_event_watcher *watcher, dev_t dev) {
  unsigned int i;
...
#define is_throw_away(x)                                                       \
  ((x) == FSE_STAT_CHANGED || (x) == FSE_CONTENT_MODIFIED)
...
static inline void kfse_init(kfs_event *kfse, int type, uint64_t time,
                             proc_t p) {
...
int add_fsevent(int type, vfs_context_t ctx, ...) {
...
restart:
  ...
  //
  if (path_override == NULL && type != FSE_CREATE_FILE && type != FSE_DELETE &&
      type != FSE_RENAME && type != FSE_EXCHANGE && type != FSE_CHOWN &&
      type != FSE_DOCID_CHANGED && type != FSE_DOCID_CREATED &&
      type != FSE_CLONE && type != FSE_ACTIVITY &&
    ...
    for (arg_type = va_arg(ap, int32_t); arg_type != FSE_ARG_DONE;
    ...
    if (sTimebaseInfo.denom != sTimebaseInfo.numer) {
      if (sTimebaseInfo.denom == 1) {
        ...
        elapsed =
            (elapsed * sTimebaseInfo.numer) / (uint64_t)sTimebaseInfo.denom;
      }
    }

    if (type == last_event_type && (elapsed < 1000000000) &&
        (last_pid == proc_getpid(p)) &&
        ((vid && vid == last_vid && last_ptr == ptr) ||
         (last_str[0] && last_nlen == nlen && ptr &&
          strcmp(last_str, ptr) == 0))) {
      last_coalesced++;
  ...
  if (kfse &&
      (type == FSE_RENAME || type == FSE_EXCHANGE || type == FSE_CLONE)) {
  ...
  if (kfse == NULL) { // yikes! no free events
...

    {
      ...
      if ((current_tv.tv_sec - last_print.tv_sec) > 10) {
        int ii;
        void *junkptr = zalloc_noblock(event_zone),
             *listhead = kfse_list_head.lh_first;

        printf("add_fsevent: event queue is full! dropping events (num dropped "
               "events: %d; num events outstanding: %d).\n",
        ...
        for (ii = 0; ii < MAX_WATCHERS; ii++) {
          ...
          printf("add_fsevent: watcher %s %p: rd %4d wr %4d q_size %4d flags "
                 "0x%x\n",
                 watcher_table[ii]->proc_name, watcher_table[ii],
  ...
  if (type == FSE_DOCID_CREATED || type == FSE_DOCID_CHANGED) {
    ...
    if (arg_type == FSE_ARG_INO) {
      cur->docid_event.src_ino = (ino64_t)(va_arg(ap, ino64_t));
    }

    arg_type = va_arg(ap, int32_t);
    if (arg_type == FSE_ARG_INO) {
      cur->docid_event.dst_ino = (ino64_t)(va_arg(ap, ino64_t));
    }

    arg_type = va_arg(ap, int32_t);
    if (arg_type == FSE_ARG_INT32) {
      cur->docid_event.docid = (uint64_t)va_arg(ap, uint32_t);
    } else if (arg_type == FSE_ARG_INT64) {
      cur->docid_event.docid = (uint64_t)va_arg(ap, uint64_t);
    }
  ...
  if (type == FSE_ACCESS_GRANTED) {
    ...
    if (atokenp != NULL) {
      memcpy(&cur->access_granted_event.audit_token, atokenp,
             sizeof(cur->access_granted_event.audit_token));
    } else if (vfs_context_copy_audit_token(
                   ctx, &cur->access_granted_event.audit_token) != 0) {
      OSBitOrAtomic16(KFSE_CONTAINS_DROPPED_EVENTS, &cur->flags);
      goto done_with_args;
    }
    ...
    if (path_str != NULL) {
      ...
      pathbuff[0] = '\0';
      if (vn_getpath_no_firmlink(vp, pathbuff, &pathbuff_len) == 0) {
        path_str = pathbuff;
  ...
  for (arg_type = va_arg(ap, int32_t); arg_type != FSE_ARG_DONE;
    ...
    case FSE_ARG_VNODE: {
      ...
      cur->regular_event.mode =
          (int32_t)vnode_vttoif(vnode_vtype(vp)) | va.va_mode;
      cur->regular_event.uid = va.va_uid;
      cur->regular_event.document_id = va.va_document_id;
      if (vp->v_flag & VISHARDLINK) {
        cur->regular_event.mode |= FSE_MODE_HLINK;
        if ((vp->v_type == VDIR && va.va_dirlinkcount == 0) ||
            (vp->v_type == VREG && va.va_nlink == 0)) {
      ...
      if (pathbuff == NULL && path_override == NULL) {
        ...
        pathbuff[0] = '\0';
        if ((ret = vn_getpath_no_firmlink(vp, pathbuff, &pathbuff_len)) != 0 ||
            pathbuff[0] == '\0') {
          OSBitOrAtomic16(KFSE_CONTAINS_DROPPED_EVENTS, &cur->flags);
          ...
          do {
            if (vp->v_parent != NULL) {
              ...
              strlcpy(pathbuff, vp->v_mount->mnt_vfsstat.f_mntonname,
                      MAXPATHLEN);
              break;
            } else {
              vp = NULL;
      ...
      cur->regular_event.str = vfs_addname(pathbuff, pathbuff_len, 0, 0);
      if (cur->regular_event.str == NULL || cur->regular_event.str[0] == '\0') {
        panic("add_fsevent: was not able to add path %s to event %p.", pathbuff,
              cur);
      }
    ...
    case FSE_ARG_STRING:
      ...
      cur->regular_event.len = (int16_t)(va_arg(ap, int32_t) & 0x7fff);
      if (cur->regular_event.len >= 1) {
        cur->regular_event.str =
            vfs_addname(va_arg(ap, char *), cur->regular_event.len, 0, 0);
      } else {
        printf("add_fsevent: funny looking string length: %d\n",
               (int)cur->regular_event.len);
        cur->regular_event.len = 2;
        cur->regular_event.str = vfs_addname("/", cur->regular_event.len, 0, 0);
      }
...
done_with_args:
  ...
  for (i = 0; i < MAX_WATCHERS; i++) {
    ...
    if (type < watcher->num_events && watcher->event_list[type] == FSE_REPORT &&
    ...
    // if (kfse->refcount < 1) {
    //    panic("add_fsevent: line %d: kfse recount %d but should be at least
    //    1", __LINE__, kfse->refcount);
    // }
  }
...
clean_up:
  ...
  if (do_all_links && (kfse->regular_event.mode & FSE_MODE_HLINK) &&
      !(kfse->regular_event.mode & FSE_MODE_LAST_HLINK) &&
      (type == FSE_STAT_CHANGED || type == FSE_CONTENT_MODIFIED ||
       type == FSE_FINDER_INFO_CHANGED || type == FSE_XATTR_MODIFIED)) {
...
#define APFSIOC_NEXT_LINK _IOWR('J', 10, uint64_t)
      ...
      if (link_vp) {
        ...
        if (iret == 0 && mnt_rootvp) {
          iret = VNOP_IOCTL(mnt_rootvp, APFSIOC_NEXT_LINK, (char *)&next_linkid,
                            (int)0, vfs_context_kernel());
        ...
        // continue resolving hardlink paths if there is a valid next_linkid
        // retrieved file systems not supporting APFSIOC_NEXT_LINK will skip
        // replicating events for sibling hardlinks
        if (iret == 0 && next_linkid != 0) {
          ...
          if ((iret = fsgetpath_internal(ctx, fsid0, next_linkid, MAXPATHLEN,
                                         path_override, FSOPT_NOFIRMLINKPATH,
                                         &path_override_len)) == 0) {
            orig_linkcount--;
            ut->uu_flag &= ~UT_KERN_RAGE_VNODES;
            ...
            // encountered a broken link or the linkid has been deleted before
            // retrieving the path
            orig_linkcount--;
            ut->uu_flag &= ~UT_KERN_RAGE_VNODES;
  ...
  if (link_name) {
    ...
    if (do_cache_reset) {
      ...
      if ((ret = VFS_VGET(link_vp->v_mount, (ino64_t)link_parentid, &dvp,
...
int test_fse_access_granted(vnode_t vp, unsigned long type, vfs_context_t ctx) {
  audit_token_t atoken;
  char *pathbuff;
  int error, pathbuff_len;

  if (type == 0) {
    return add_fsevent(FSE_ACCESS_GRANTED, ctx, FSE_ARG_VNODE, vp,
                       FSE_ARG_DONE);
  }
  ...
  if (type == 1) {
    ...
    return add_fsevent(FSE_ACCESS_GRANTED, ctx, FSE_ARG_VNODE, vp,
                       FSE_ARG_AUDIT_TOKEN, &atoken, FSE_ARG_DONE);
  }
  ...
  if (type == 2 || type == 3) {
    ...
    if (type == 2) {
      error = add_fsevent(FSE_ACCESS_GRANTED, ctx, FSE_ARG_STRING, pathbuff_len,
                          pathbuff, FSE_ARG_DONE);
    } else {
      error = add_fsevent(FSE_ACCESS_GRANTED, ctx, FSE_ARG_PATH, pathbuff,
                          FSE_ARG_DONE);
    }
    release_pathbuff(pathbuff);
    return error;
...
static void release_event_ref(kfs_event *kfse) {
  ...
  // holding any locks.
  //
  if (kfse->type != FSE_DOCID_CREATED && kfse->type != FSE_DOCID_CHANGED &&
    ...
    if (dest != NULL) {
      ...
      if (OSAddAtomic(-1, &kfse->regular_event.dest->refcount) == 1) {
...
#define FSEVENTS_WATCHER_ENTITLEMENT "com.apple.private.vfs.fsevents-watcher"
...
static bool watcher_is_entitled(task_t task) {
  //
  ...
  //
  return !!(
      IOTaskHasEntitlement(task, FSEVENTS_WATCHER_ENTITLEMENT) ||
      IOTaskHasEntitlement(task, FSEVENTS_ACCESS_GRANTED_WATCHER_ENTITLEMENT) ||
      IOTaskHasEntitlement(task, FSEVENTS_ACTIVITY_WATCHER_ENTITLEMENT));
}
#if CONFIG_FSE_ACCESS_GRANTED
static bool watcher_is_entitled_for_access_granted(task_t task) {
  return !!IOTaskHasEntitlement(task,
                                FSEVENTS_ACCESS_GRANTED_WATCHER_ENTITLEMENT);
}
#endif
static bool watcher_is_entitled_for_activity(task_t task) {
  return !!IOTaskHasEntitlement(task, FSEVENTS_ACTIVITY_WATCHER_ENTITLEMENT);
}

static int add_watcher(int8_t *event_list, int32_t num_events,
                       int32_t eventq_size, fs_event_watcher **watcher_out,
                       void *fseh) {
  int i;
  fs_event_watcher *watcher;
  ...
  if (num_events > FSE_ACTIVITY && event_list[FSE_ACTIVITY] != FSE_IGNORE &&
  ...
  if (watcher_is_entitled(current_task())) {
    ...
    printf("fsevents: watcher %s (pid: %d) - Using /dev/fsevents directly is "
           "unsupported.  Migrate to FSEventsFramework\n",
...
static void remove_watcher(fs_event_watcher *target) {
  int i, j, counter = 0;
  fs_event_watcher *watcher;
  kfs_event *kfse;
  ...
  for (j = 0; j < MAX_WATCHERS; j++) {
    ...
    // printf("fsevents: removing watcher %p (rd %d wr %d num_readers %d flags
    // 0x%x)\n", watcher, watcher->rd, watcher->wr, watcher->num_readers,
    // watcher->flags);
    ...
    kfree_data(watcher->devices_not_to_watch,
...
static void delayed_event_delivery(__unused void *param0,
  ...
  for (i = 0; i < MAX_WATCHERS; i++) {
    if (watcher_table[i] != NULL &&
        watcher_table[i]->rd != watcher_table[i]->wr) {
...
//
static void schedule_event_wakeup(void) {
  uint64_t deadline;

  if (event_delivery_timer == NULL) {
    event_delivery_timer =
        thread_call_allocate((thread_call_func_t)delayed_event_delivery, NULL);
  }
...
static int watcher_add_event(fs_event_watcher *watcher, kfs_event *kfse) {
  ...
  if (num_pending > (watcher->eventq_size * 3 / 4) &&
      !(watcher->flags & WATCHER_APPLE_SYSTEM_SERVICE)) {
    /* Non-Apple Service is falling behind, start dropping events for this
     * process */
    lck_rw_lock_exclusive(&event_handling_lock);
    while (watcher->rd != watcher->wr) {
      kfse = watcher->event_queue[watcher->rd];
    ...
    printf("fsevents: watcher falling behind: %s (pid: %d) rd: %4d wr: %4d "
           "q_size: %4d flags: 0x%x\n",
...
static int fill_buff(uint16_t type, int32_t size, const void *data, char *buff,
                     int32_t *_buff_idx, int32_t buff_sz, struct uio *uio) {
...
static int copy_out_kfse(fs_event_watcher *watcher, kfs_event *kfse,
                         struct uio *uio) __attribute__((noinline));
...
static int copy_out_kfse(fs_event_watcher *watcher, kfs_event *kfse,
  ...
  if (kfse->type == FSE_INVALID) {
    panic("fsevents: copy_out_kfse: asked to copy out an invalid event (kfse "
          "%p, refcount %d)",
          kfse, kfse->refcount);
  }
...
copy_again:
  ...
  if (kfse->type == FSE_DOCID_CHANGED || kfse->type == FSE_DOCID_CREATED) {
    ...
    error = fill_buff(FSE_ARG_DEV, sizeof(dev_t), &dev, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INO, sizeof(ino64_t), &src_ino, evbuff,
                      &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INO, sizeof(ino64_t), &dst_ino, evbuff,
                      &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INT64, sizeof(uint64_t), &docid, evbuff,
                      &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }
  ...
  if (kfse->type == FSE_ACTIVITY) {
    error = fill_buff(FSE_ARG_INT32, sizeof(cur->activity_event.version),
                      &cur->activity_event.version, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }
    error = fill_buff(FSE_ARG_DEV, sizeof(cur->activity_event.dev),
                      &cur->activity_event.dev, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INO, sizeof(cur->activity_event.ino),
                      &cur->activity_event.ino, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INT64, sizeof(cur->activity_event.origin_id),
                      &cur->activity_event.origin_id, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INT64, sizeof(cur->activity_event.age),
                      &cur->activity_event.age, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INT32, sizeof(cur->activity_event.use_state),
                      &cur->activity_event.use_state, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INT32, sizeof(cur->activity_event.urgency),
                      &cur->activity_event.urgency, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INT64, sizeof(cur->activity_event.size),
                      &cur->activity_event.size, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }
  ...
  if (kfse->type == FSE_ACCESS_GRANTED) {
    ...
    error = fill_buff(FSE_ARG_STRING, cur->access_granted_event.len,
                      cur->access_granted_event.str, evbuff, &evbuff_idx,
                      sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }
    error = fill_buff(FSE_ARG_AUDIT_TOKEN,
                      sizeof(cur->access_granted_event.audit_token),
                      &cur->access_granted_event.audit_token, evbuff,
                      &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }
  ...
  if (watcher->flags & WATCHER_WANTS_COMPACT_EVENTS) {
    ...
    error = fill_buff(FSE_ARG_FINFO, KFSE_INFO_COPYSIZE, &cur->regular_event,
                      evbuff, &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }
  } else {
    error = fill_buff(FSE_ARG_DEV, sizeof(dev_t), &cur->regular_event.dev,
                      evbuff, &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_INO, sizeof(ino64_t), &cur->regular_event.ino,
                      evbuff, &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_MODE, sizeof(int32_t), &cur->regular_event.mode,
                      evbuff, &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }

    error = fill_buff(FSE_ARG_UID, sizeof(uid_t), &cur->regular_event.uid,
                      evbuff, &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }
    ...
    error =
        fill_buff(FSE_ARG_GID, sizeof(gid_t), &cur->regular_event.document_id,
                  evbuff, &evbuff_idx, sizeof(evbuff), uio);
    if (error != 0) {
      goto get_out;
    }
  ...
  error = fill_buff(FSE_ARG_INT64, sizeof(uint64_t), &cur->abstime, evbuff,
                    &evbuff_idx, sizeof(evbuff), uio);
  if (error != 0) {
    goto get_out;
  }
...
static int fmod_watch(fs_event_watcher *watcher, struct uio *uio) {
  int error = 0;
...
restart_watch:
  ...
  while (uio_resid(uio) > 0 && watcher->rd != watcher->wr) {
    ...
    //
    kfse = watcher->event_queue[watcher->rd];
    if (!kfse || kfse->type == FSE_INVALID ||
        kfse->type >= watcher->num_events || kfse->refcount < 1) {
      break;
    }

    if (watcher->event_list[kfse->type] == FSE_REPORT) {
      if (!(watcher->flags & WATCHER_APPLE_SYSTEM_SERVICE) &&
          kfse->type != FSE_DOCID_CREATED && kfse->type != FSE_DOCID_CHANGED &&
          kfse->type != FSE_ACTIVITY &&
...
//
void fsevent_unmount(__unused struct mount *mp, __unused vfs_context_t ctx) {
  ...
  while (fsevent_unmount_dev != 0) {
    error = msleep((caddr_t)&fsevent_unmount_dev, &watch_table_lock, PRIBIO,
    ...
    if (!error && (++waitcount >= 10)) {
      error = EWOULDBLOCK;
      printf("timeout waiting to signal unmount pending for dev %d "
             "(fsevent_unmount_dev %d)\n",
...
static int fseventsf_read(struct fileproc *fp, struct uio *uio,
...
static int fseventsf_ioctl(struct fileproc *fp, u_long cmd, caddr_t data,
                           vfs_context_t ctx) {
  fsevent_handle *fseh = (struct fsevent_handle *)fp_get_data(fp);
  int ret = 0;
  ...
  case FSEVENTS_DEVICE_FILTER_32: {
    ...
    fsevent_dev_filter_args32 *devfilt_args32 =
        (fsevent_dev_filter_args32 *)data;
  ...
  handle_dev_filter: {
    ...
    devices_not_to_watch =
        kalloc_data(new_num_devices * sizeof(dev_t), Z_WAITOK);
    ...
    ret = copyin((user_addr_t)devfilt_args->devices,
                 (void *)devices_not_to_watch, new_num_devices * sizeof(dev_t));
    if (ret) {
...
static int fseventsf_select(struct fileproc *fp, int which, __unused void *wql,
                            vfs_context_t ctx) {
  fsevent_handle *fseh = (struct fsevent_handle *)fp_get_data(fp);
  int ready = 0;
...
#if NOTUSED
static int fseventsf_stat(__unused struct fileproc *fp,
...
static int fseventsf_close(struct fileglob *fg, __unused vfs_context_t ctx) {
...
static void filt_fsevent_detach(struct knote *kn) {
 ...
 */
static int filt_fsevent_common(struct knote *kn, struct kevent_qos_s *kev,
                               long hint) {
  fsevent_handle *fseh = (struct fsevent_handle *)knote_kn_hook_get_raw(kn);
  int activate = 0;
  int32_t rd, wr, amt;
...
static int filt_fsevent_touch(struct knote *kn, struct kevent_qos_s *kev) {
  int res;
...
static int fseventsf_kqfilter(struct fileproc *fp, struct knote *kn,
...
static int fseventsf_drain(struct fileproc *fp, __unused vfs_context_t ctx) {
  int counter = 0;
  fsevent_handle *fseh = (struct fsevent_handle *)fp_get_data(fp);
...
static int fseventsopen(__unused dev_t dev, __unused int flag,
                        __unused int mode, __unused struct proc *p) {
...
static int fseventsclose(__unused dev_t dev, __unused int flag,
                         __unused int mode, __unused struct proc *p) {
  return 0;
}

static int fseventsread(__unused dev_t dev, __unused struct uio *uio,
                        __unused int ioflag) {
  return EIO;
}

static int parse_buffer_and_add_events(const char *buffer, size_t bufsize,
                                       vfs_context_t ctx, size_t *remainder) {
  ...
  while ((ptr + sizeof(int) + sizeof(fse_info) + 1) < buffer + bufsize) {
    ...
    if (type != FSE_RENAME && type != FSE_EXCHANGE && type != FSE_CLONE) {
      ...
      err = add_fsevent(type, ctx, FSE_ARG_STRING, path_len, path,
                        FSE_ARG_FINFO, finfo, FSE_ARG_DONE);
      if (err) {
        break;
      }
      continue;
    ...
    if (dest_finfo->ino) {
      err = add_fsevent(type, ctx, FSE_ARG_STRING, path_len, path,
                        FSE_ARG_FINFO, finfo, FSE_ARG_STRING, dest_path_len,
                        dest_path, FSE_ARG_FINFO, dest_finfo, FSE_ARG_DONE);
    } else {
      err = add_fsevent(type, ctx, FSE_ARG_STRING, path_len, path,
                        FSE_ARG_FINFO, finfo, FSE_ARG_STRING, dest_path_len,
                        dest_path, FSE_ARG_DONE);
    }
...
static int fseventswrite(__unused dev_t dev, struct uio *uio,
                         __unused int ioflag) {
  int error = 0;
  ...
  while (uio_resid(uio)) {
    ...
    error = parse_buffer_and_add_events(write_buffer, offset + count, ctx,
                                        &remainder);
    if (error) {
      break;
    }
    ...
    //
    memmove(write_buffer, (write_buffer + count + offset) - remainder,
            remainder);
    offset = remainder;
  }
...
static int fseventsioctl(__unused dev_t dev, u_long cmd, caddr_t data,
  ...
  handle_clone:
    ...
    event_list =
        kalloc_data(fse_clone_args->num_events * sizeof(int8_t), Z_WAITOK);
    if (event_list == NULL) {
      kfree_type(fsevent_handle, fseh);
      return ENOMEM;
    }

    error = copyin((user_addr_t)fse_clone_args->event_list, (void *)event_list,
     ...
     */
    error = vslock((user_addr_t)fse_clone_args->fd, sizeof(int32_t));
    if (error) {
      kfree_data(event_list, fse_clone_args->num_events * sizeof(int8_t));
      kfree_type(fsevent_handle, fseh);
      return error;
    ...
    error =
        add_watcher(event_list, fse_clone_args->num_events,
                    fse_clone_args->event_queue_depth, &fseh->watcher, fseh);
    if (error) {
      vsunlock((user_addr_t)fse_clone_args->fd, sizeof(int32_t), 0);
      kfree_data(event_list, fse_clone_args->num_events * sizeof(int8_t));
      kfree_type(fsevent_handle, fseh);
      return error;
    }
     ...
     */
    error =
        copyout((void *)&fd, (user_addr_t)fse_clone_args->fd, sizeof(int32_t));
    assert(error == 0);
    ...
    vsunlock((user_addr_t)fse_clone_args->fd, sizeof(int32_t), 1);
    break;
...
void fsevents_init(void) {
  ...
  devfs_make_node(makedev(ret, 0), DEVFS_CHAR, UID_ROOT, GID_WHEEL, 0644,
                  "fsevents");

  fsevents_internal_init();
}

char *get_pathbuff(void) { return zalloc(ZV_NAMEI); }
...
int get_fse_info(struct vnode *vp, fse_info *fse, __unused vfs_context_t ctx) {
  struct vnode_attr va;
...
int vnode_get_fse_info_from_vap(vnode_t vp, fse_info *fse,
...
void create_fsevent_from_kevent(vnode_t vp, uint32_t kevents,
                                struct vnode_attr *vap) {
  int fsevent_type = FSE_CONTENT_MODIFIED,
      len; // the default is the most pessimistic
  ...
  if (kevents & VNODE_EVENT_DELETE) {
    ...
    fsevent_type = FSE_CREATE_FILE; // XXXdbg - should use FSE_RENAME but we
                                    // don't have the destination info;
  } else if (kevents & (VNODE_EVENT_FILE_CREATED | VNODE_EVENT_FILE_REMOVED |
                        VNODE_EVENT_DIR_CREATED | VNODE_EVENT_DIR_REMOVED)) {
    fsevent_type = FSE_STAT_CHANGED; // XXXdbg - because vp is a dir and the
                                     // thing created/removed lived inside it
  } else { // a catch all for VNODE_EVENT_PERMS, VNODE_EVENT_ATTRIB and anything
           // else
  ...
  if (vn_getpath_no_firmlink(vp, pathbuf, &len) == 0) {
    add_fsevent(fsevent_type, vfs_context_current(), FSE_ARG_STRING, len,
                pathbuf, FSE_ARG_FINFO, &fse, FSE_ARG_DONE);
  }
  return;
}
...
void release_pathbuff(char *path) { zfree(ZV_NAMEI, path); }

int add_fsevent(__unused int type, __unused vfs_context_t ctx, ...) {
  return 0;
}

int need_fsevent(__unused int type, __unused vnode_t vp) { return 0; }
