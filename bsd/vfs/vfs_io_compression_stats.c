...
typedef struct {
  uint8_t lz4state[lz4_encode_scratch_size]
...
struct iocs_store_buffer iocs_store_buffer = {
    .buffer = 0, .current_position = 0, .marked_point = 0};

int iocs_sb_bytes_since_last_mark = 0;
int iocs_sb_bytes_since_last_notification = 0;

KALLOC_TYPE_DEFINE(io_compression_stats_zone, struct io_compression_stats,
                   KT_DEFAULT);
...
#define BSD_HOST 1
  ...
  iocs_store_buffer.buffer =
      kalloc_data(IOCS_STORE_BUFFER_SIZE, Z_WAITOK | Z_ZERO);
...
static int sysctl_io_compression_stats_enable SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2, oidp)
  ...
  if ((io_compression_stats_enable == 0) && (enable == 1)) {
    ...
    error = io_compression_stats_allocate_compression_buffers(
        io_compression_stats_block_size);
    if (error == 0) {
      io_compression_stats_enable = enable;
      io_compression_stats_dbg(
          "SUCCESS: setting io_compression_stats_enable to %d",
          io_compression_stats_enable);
    } else {
      io_compression_stats_dbg(
          "FAILED: setting io_compression_stats_enable to %d",
          io_compression_stats_enable);
    }
...
SYSCTL_PROC(_vfs, OID_AUTO, io_compression_stats_enable,
            CTLTYPE_INT | CTLFLAG_RW, 0, 0, &sysctl_io_compression_stats_enable,
            "I", "");

static int sysctl_io_compression_block_size SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2, oidp)
  ...
  if (block_size < IO_COMPRESSION_STATS_MIN_BLOCK_SIZE ||
      block_size > IO_COMPRESSION_STATS_MAX_BLOCK_SIZE) {
    return EINVAL;
  }
  ...
  if (io_compression_stats_block_size != block_size) {
    if (io_compression_stats_enable == 1) {
      ...
      if (error == 0) {
        ...
        io_compression_stats_dbg(
            "SUCCESS: setting io_compression_stats_block_size to %d",
            io_compression_stats_block_size);
      } else {
        /* Failed to allocate buffers, disable IO compression stats */
        io_compression_stats_enable = 0;
        io_compression_stats_dbg(
            "FAILED: setting io_compression_stats_block_size to %d",
            io_compression_stats_block_size);
      }
    } else {
      /* IO compression stats is disabled, only set the
       * io_compression_stats_block_size */
      io_compression_stats_block_size = block_size;
      io_compression_stats_dbg(
          "SUCCESS: setting io_compression_stats_block_size to %d",
...
SYSCTL_PROC(_vfs, OID_AUTO, io_compression_stats_block_size,
            CTLTYPE_INT | CTLFLAG_RW, 0, 0, &sysctl_io_compression_block_size,
            "I", "");

static int32_t iocs_compress_block(uint8_t *block_ptr, uint32_t block_size) {
  disable_preemption();
  ...
  int compressed_block_size =
      (int)lz4raw_encode_buffer(dest_buf, block_size, block_ptr, block_size,
                                (lz4_hash_entry_t *)scratch_buf);
 ...
 */
static uint32_t iocs_compress_buffer(vnode_t vn, uint8_t *buf_ptr,
                                     uint32_t buf_size) {
  uint32_t offset;
  ...
  for (offset = 0; offset < buf_size; offset += block_size) {
    ...
    int current_compressed_block_size =
        iocs_compress_block(buf_ptr + offset, current_block_size);

    if (current_compressed_block_size == 0) {
      compressed_size += current_block_size;
      vnode_updateiocompressionblockstats(vn, current_block_size /
                                                  block_stats_scaling_factor);
    } else if (current_compressed_block_size != -1) {
      compressed_size += current_compressed_block_size;
      vnode_updateiocompressionblockstats(vn, current_compressed_block_size /
                                                  block_stats_scaling_factor);
    }
  }
...
static uint32_t log2down(uint32_t x) { return 31 - __builtin_clz(x); }

/*
 * Once we get the IO compression stats for the entire buffer, we update
 * buffer_size_compressibility_dist, which helps us observe distribution across
 * various io sizes and compression factors. The goal of next two functions is
 * to get the index in this buffer_size_compressibility_dist table.
 */

/*
 * Maps IO size to a bucket between 0 - IO_COMPRESSION_STATS_MAX_SIZE_BUCKET
 * for size < 4096 returns 0 and size > 1MB returns
 * IO_COMPRESSION_STATS_MAX_SIZE_BUCKET (9). For IO sizes in-between we arrive
 * at the index based on log2 function. sizes 4097 - 8192 => index = 1, sizes
 * 8193 - 16384 => index = 2, and so on
 */
#define SIZE_COMPRESSION_DIST_SIZE_BUCKET_MIN 4096
#define SIZE_COMPRESSION_DIST_SIZE_BUCKET_MAX (1024 * 1024)
static uint32_t get_buffer_size_bucket(uint32_t size) {
...
/*
 * Maps compression factor to a bucket between 0 -
 * IO_COMPRESSION_STATS_MAX_COMPRESSION_BUCKET
 */
static uint32_t get_buffer_compressibility_bucket(uint32_t uncompressed_size,
                                                  uint32_t compressed_size) {
  int saved_space_pc =
      (uncompressed_size - compressed_size) * 100 / uncompressed_size;
...
void io_compression_stats(buf_t bp) {
  ...
  if ((io_compression_stats_enable != 1) || (bflags & B_READ) ||
      (buf_cnt <= 0)) {
    return;
  }
  ...
  if (buf_ptr != NULL) {
    ...
    if (compressed_size != 0) {
      vnode_updateiocompressionbufferstats(
          vn, buf_cnt, compressed_size, get_buffer_size_bucket(buf_cnt),
          get_buffer_compressibility_bucket(buf_cnt, compressed_size));
    }
  }

  KDBG_RELEASE(
      FSDBG_CODE(DBG_VFS, DBG_VFS_IO_COMPRESSION_STATS) | DBG_FUNC_NONE,
...
static void iocs_notify_user(void) {
...
static void
construct_iocs_sbe_from_vnode(struct vnode *vp,
                              struct iocs_store_buffer_entry *iocs_sbe) {
  ...
  /*
   * Total path length is path_len, we can copy out IOCS_SBE_PATH_LEN bytes. We
   * are interested in first segment of the path to try and figure out the
   * process writing to the file, and we are interested in the last segment to
   * figure out extention. So, in cases where IOCS_SBE_PATH_LEN < path_len, lets
   * copy out first IOCS_PATH_START_BYTES_TO_COPY bytes and last
   * IOCS_PATH_END_BYTES_TO_COPY (last segment includes the null character).
   */
  if (path_len > IOCS_SBE_PATH_LEN) {
    strncpy(iocs_sbe->path_name, vnpath_scratch_buf,
  ...
  memcpy(&iocs_sbe->iocs, vp->io_compression_stats,
         sizeof(struct io_compression_stats));
}

void vnode_iocs_record_and_free(struct vnode *vp) {
  int notify = 0;
  ...
  assert(iocs_store_buffer.current_position +
             sizeof(struct iocs_store_buffer_entry) <=
         IOCS_STORE_BUFFER_SIZE);

  iocs_sbe =
      (struct iocs_store_buffer_entry *)((uintptr_t)iocs_store_buffer.buffer +
  ...
  if (iocs_store_buffer.current_position +
          sizeof(struct iocs_store_buffer_entry) >
      IOCS_STORE_BUFFER_SIZE) {
  ...
  iocs_sb_bytes_since_last_notification +=
      sizeof(struct iocs_store_buffer_entry);
...
release:
  lck_mtx_unlock(&iocs_store_buffer_lock);
out:
  /* We need to free io_compression_stats whether or not we were able to record
   * it */
...
static int vnode_iocs_callback(struct vnode *vp, void *vctx) {
  ...
  if ((current_offset + sizeof(struct iocs_store_buffer_entry)) < length) {
    if (vp->io_compression_stats != NULL) {
      ...
      uint32_t error =
          copyout(&iocs_sbe_next,
                  (user_addr_t)((unsigned char *)req->oldptr + current_offset),
                  sizeof(struct iocs_store_buffer_entry));
      if (error) {
...
static int vfs_iocs_callback(mount_t mp, void *arg) {
...
static int sysctl_io_compression_dump_stats SYSCTL_HANDLER_ARGS {
#pragma unused(arg1, arg2, oidp)
  ...
  if (iocs_store_buffer.marked_point < iocs_store_buffer.current_position) {
    error = copyout((void *)((uintptr_t)iocs_store_buffer.buffer +
                             iocs_store_buffer.marked_point),
                    req->oldptr,
                    iocs_store_buffer.current_position -
                        iocs_store_buffer.marked_point);
    if (error) {
    ...
    error = copyout(iocs_store_buffer.buffer, req->oldptr + ret_len,
                    iocs_store_buffer.current_position);
    if (error) {
      req->oldidx = 0;
      goto release;
...
SYSCTL_PROC(_vfs, OID_AUTO, io_compression_dump_stats,
            CTLFLAG_WR | CTLTYPE_NODE, 0, 0, sysctl_io_compression_dump_stats,
            "-", "");

errno_t vnode_updateiocompressionblockstats(vnode_t vp, uint32_t size_bucket) {
  if (vp == NULL) {
    return EINVAL;
  }
  ...
  if (vp->io_compression_stats == NULL) {
    io_compression_stats_t iocs =
        zalloc_flags(io_compression_stats_zone, Z_ZERO | Z_NOFAIL);
    vnode_lock_spin(vp);
    /* Re-check with lock */
    if (vp->io_compression_stats == NULL) {
  ...
  OSIncrementAtomic((SInt32 *)&vp->io_compression_stats
                        ->block_compressed_size_dist[size_bucket]);

  return 0;
}
errno_t vnode_updateiocompressionbufferstats(
    __unused vnode_t vp, __unused uint64_t uncompressed_size,
    __unused uint64_t compressed_size, __unused uint32_t size_bucket,
    __unused uint32_t compression_bucket) {
  if (vp == NULL) {
    return EINVAL;
  }

  /* vnode_updateiocompressionblockstats will always be called before
   * vnode_updateiocompressionbufferstats. Hence vp->io_compression_stats should
   * already be allocated */
  if (vp->io_compression_stats == NULL) {
    return EINVAL;
  }

  if ((size_bucket >= IOCS_BUFFER_NUM_SIZE_BUCKETS) ||
      (compression_bucket >= IOCS_BUFFER_NUM_COMPRESSION_BUCKETS)) {
    return EINVAL;
  }

  OSAddAtomic64(uncompressed_size,
                &vp->io_compression_stats->uncompressed_size);
  OSAddAtomic64(compressed_size, &vp->io_compression_stats->compressed_size);

  OSIncrementAtomic(
      (SInt32 *)&vp->io_compression_stats
          ->buffer_size_compression_dist[size_bucket][compression_bucket]);
