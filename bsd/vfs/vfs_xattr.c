...
#define MAKE_SHADOW_NAME(VP, NAME)                                             \
  snprintf((NAME), sizeof((NAME)), (SHADOW_NAME_FMT),                          \
           ((void *)(VM_KERNEL_ADDRPERM(VP))), (VP)->v_id,                     \
...
static int default_getnamedstream(vnode_t vp, vnode_t *svpp, const char *name,
                                  enum nsoperation op, vfs_context_t context);

static int default_makenamedstream(vnode_t vp, vnode_t *svpp, const char *name,
                                   vfs_context_t context);

static int default_removenamedstream(vnode_t vp, const char *name,
                                     vfs_context_t context);

static int getshadowfile(vnode_t vp, vnode_t *svpp, int makestream,
                         size_t *rsrcsize, int *creator, vfs_context_t context);
...
static int default_getxattr(vnode_t vp, const char *name, uio_t uio,
                            size_t *size, int options, vfs_context_t context);
 ...
 */
int vn_getxattr(vnode_t vp, const char *name, uio_t uio, size_t *size,
                int options, vfs_context_t context) {
  int error;
  ...
  if (!(options & XATTR_NOSECURITY)) {
    ...
    if ((error = vnode_authorize(vp, NULL, KAUTH_VNODE_READ_EXTATTRIBUTES,
                                 context))) {
      goto out;
    }
  }
  ...
  if (uio_offset(uio) != 0 && strncmp(name, XATTR_RESOURCEFORK_NAME,
 ...
 */
int vn_setxattr(vnode_t vp, const char *name, uio_t uio, int options,
                vfs_context_t context) {
  int error;
...
#endif
  if ((options & (XATTR_REPLACE | XATTR_CREATE)) ==
 ...
 */
int vn_removexattr(vnode_t vp, const char *name, int options,
                   vfs_context_t context) {
  int error;
 ...
 */
int vn_listxattr(vnode_t vp, uio_t uio, size_t *size, int options,
                 vfs_context_t context) {
  int error;
...
int xattr_validatename(const char *name) {
  size_t namelen;
 ...
 */
int xattr_protected(const char *attrname) {
  return !strncmp(attrname, "com.apple.system.", 17);
}

static void vnode_setasnamedstream_internal(vnode_t vp, vnode_t svp) {
  ...
  vnode_update_identity(
      svp, vp, NULL, 0, 0,
      (VNODE_UPDATE_NAMEDSTREAM_PARENT | VNODE_UPDATE_FORCE_PARENT_REF));
 ...
 */
errno_t vnode_getnamedstream(vnode_t vp, vnode_t *svpp, const char *name,
                             enum nsoperation op, int flags,
                             vfs_context_t context) {
  int error;
 ...
 */
errno_t vnode_makenamedstream(vnode_t vp, vnode_t *svpp, const char *name,
 ...
 */
errno_t vnode_removenamedstream(vnode_t vp, vnode_t svp, const char *name,
                                int flags, vfs_context_t context) {
  int error;
...
errno_t vnode_flushnamedstream(vnode_t vp, vnode_t svp, vfs_context_t context) {
  ...
  while (offset < datasize) {
    ...
    uio_addiov(auio, (uintptr_t)bufptr, iosize);
    error = vn_setxattr(vp, XATTR_RESOURCEFORK_NAME, auio, XATTR_NOSECURITY,
                        context);
    if (error) {
      break;
    }
...
retry_create:
  *creator = 0;
  ...
  if (VNOP_GETATTR(vp, &va, context) != 0 || !VATTR_IS_SUPPORTED(&va, va_uid) ||
      !VATTR_IS_SUPPORTED(&va, va_gid) || !VATTR_IS_SUPPORTED(&va, va_mode)) {
...
static int default_getnamedstream(vnode_t vp, vnode_t *svpp, const char *name,
   ...
   */
  if (strncmp(name, XATTR_RESOURCEFORK_NAME, sizeof(XATTR_RESOURCEFORK_NAME)) !=
      0) {
    *svpp = NULLVP;
    return ENOATTR;
  }
  ...
  if (!creator) {
    ...
    if (svp->v_flag & VISNAMEDSTREAM) {
      ...
      if ((svp->v_flag & VISSHADOW)) {
        /*
         * No VISNAMEDSTREAM, but we did see VISSHADOW, indicating that the
         * other thread is done with this vnode. Just unlock the vnode and try
         * again
         */
        vnode_unlock(svp);
      } else {
...
static bool is_shadow_dir_valid(vnode_t parent_sdvp, vnode_t sdvp,
                                vfs_context_t kernelctx) {
  struct vnode_attr va;
  uint32_t tmp_fsid;
  bool is_valid = false;
  ...
  if (VNOP_GETATTR(sdvp, &va, kernelctx) != 0 ||
      !VATTR_IS_SUPPORTED(&va, va_uid) || !VATTR_IS_SUPPORTED(&va, va_gid) ||
      !VATTR_IS_SUPPORTED(&va, va_mode) || !VATTR_IS_SUPPORTED(&va, va_fsid)) {
    goto out;
  }
  ...
  if ((va.va_uid != 0) || (va.va_gid != 0) ||
      ...
      (va.va_fsid != tmp_fsid) || (va.va_dirlinkcount != 1) ||
      (va.va_acl != (kauth_acl_t)KAUTH_FILESEC_NONE)) {
    goto out;
  }
...
#define ATTR_BUF_SIZE                                                          \
  4096 /* default size of the attr file and how much we'll grow by */
...
typedef struct attr_header {
  ...
  u_int32_t
      total_size; /* file offset of end of attribute header + entries + data */
 ...
 */
static int default_getxattr_doubleagent(vnode_t vp, const char *name, uio_t uio,
                                        size_t *size, int options,
                                        vfs_context_t context, mach_port_t);
static int default_setxattr_doubleagent(vnode_t vp, const char *name, uio_t uio,
                                        int options, vfs_context_t context,
                                        mach_port_t);
 ...
 */
static void close_xattrfile(struct fileglob *xfg, bool have_iocount,
                            bool drop_iocount, vfs_context_t context);

static int open_xattrfile(vnode_t vp, int fileflags, struct fileglob **xfgp,
                          int64_t *file_sizep, bool *created_xattr_filep,
                          vfs_context_t context);

static void remove_xattrfile(struct fileglob *xfg, vnode_t xvp,
                             vfs_context_t context);
 ...
 */
static int default_getxattr(vnode_t vp, const char *name, uio_t uio,
                            size_t *size, __unused int options,
                            vfs_context_t context) {
  mach_port_t port;
  int error;

  if (get_doubleagentd_port(&port) == 0) {
    error = default_getxattr_doubleagent(vp, name, uio, size, options, context,
                                         port);
 ...
 * Set the data of an extended attribute.
 */
static int __attribute__((noinline)) default_setxattr(vnode_t vp,
                                                      const char *name,
                                                      uio_t uio, int options,
                                                      vfs_context_t context) {
  mach_port_t port;
  int error;

  if (get_doubleagentd_port(&port) == 0) {
    error = default_setxattr_doubleagent(vp, name, uio, options, context, port);
    ipc_port_release_send(port);
  } else {
    error = ENOATTR;
  }
 ...
 * Remove an extended attribute.
 */
static int default_removexattr(vnode_t vp, const char *name,
                               __unused int options, vfs_context_t context) {
  mach_port_t port;
  int error;

  if (get_doubleagentd_port(&port) == 0) {
    error = default_removexattr_doubleagent(vp, name, options, context, port);
    ipc_port_release_send(port);
  } else {
    error = ENOATTR;
  }
 ...
 * Retrieve the list of extended attribute names.
 */
static int default_listxattr(vnode_t vp, uio_t uio, size_t *size,
                             __unused int options, vfs_context_t context) {
  mach_port_t port;
  int error;
  ...
  if (get_doubleagentd_port(&port) == 0) {
    error =
        default_listxattr_doubleagent(vp, uio, size, options, context, port);
    ipc_port_release_send(port);
  } else {
    error = 0;
  }
...
static int default_listxattr_doubleagent(vnode_t vp, uio_t uio, size_t *size,
  ...
  if ((error =
           open_xattrfile(vp, FREAD | O_SHLOCK, &xfg, &fsize, NULL, context))) {
    if (error == ENOATTR) {
      error = 0;
    }
    goto out;
...
default_setxattr_doubleagent(vnode_t vp, const char *name, uio_t uio,
  ...
  if (strncmp(name, XATTR_RESOURCEFORK_NAME, sizeof(XATTR_RESOURCEFORK_NAME)) ==
      0) {
    /*
     * For ResourceFork we allow offset to be != 0, so adjust datalen
     * accordingly so doubleagent will adjust the file length accordingly
     *
     */
   ...
   * consumes it.
   */
  kr = doubleagent_allocate_xattr(doubleagentd_port, fileport, fsize, cName,
                                  datalen, options, &error, &value_offset);
  if (kr != KERN_SUCCESS) {
    error = EIO;
  }
  if (error == 0) {
...
static int open_xattrfile(vnode_t vp, int fileflags, struct fileglob **xfgp,
  ...
  filename = &smallname[0];
  alloc_len =
      snprintf(filename, sizeof(smallname), "%s%s", ATTR_FILE_PREFIX, basename);
  if (alloc_len >= sizeof(smallname)) {
    alloc_len++; /* snprintf result doesn't include '\0' */
    filename = kalloc_data(alloc_len, Z_WAITOK);
    copy_len =
        snprintf(filename, alloc_len, "%s%s", ATTR_FILE_PREFIX, basename);
  }
  /*
...
lookup:
  ...
  if (fileflags & (O_EXLOCK | O_SHLOCK)) {
    ...
    error = VNOP_ADVLOCK(xvp, (caddr_t)fg, F_SETLK, &lf, F_FLOCK | F_WAIT,
                         context, NULL);
    if (error == ENOTSUP) {
      error = 0;
    } else if (error) {
...
static void close_xattrfile(struct fileglob *xfg, bool have_iocount,
  ...
  if (have_iocount) {
    ...
    (void)VNOP_ADVLOCK(xvp, (caddr_t)xfg, F_UNLCK, &lf, F_FLOCK, context, NULL);
...
static int make_xattrfile_port(struct fileglob *fg, ipc_port_t *portp) {
  /*
...
static int default_getxattr(__unused vnode_t vp, __unused const char *name,
                            __unused uio_t uio, __unused size_t *size,
                            __unused int options,
                            __unused vfs_context_t context) {
  return ENOTSUP;
}

static int default_setxattr(__unused vnode_t vp, __unused const char *name,
                            __unused uio_t uio, __unused int options,
                            __unused vfs_context_t context) {
  return ENOTSUP;
}

static int default_listxattr(__unused vnode_t vp, __unused uio_t uio,
                             __unused size_t *size, __unused int options,
                             __unused vfs_context_t context) {
  return ENOTSUP;
}

static int default_removexattr(__unused vnode_t vp, __unused const char *name,
                               __unused int options,
                               __unused vfs_context_t context) {
  return ENOTSUP;
}
