 ...
 *    policy lists.
 *  necp_session_policy --> necp_kernel_socket_policy and
 ...
 *    list is traversed to create optimized sub-lists ("Maps") which are used
 * during data-path evaluation. IP policies are sent into
 * necp_kernel_ip_output_policies_map, which hashes incoming packets based on
 * marked socket-layer policies, and removes duplicate or overlapping policies.
 * Socket policies are sent into two maps, necp_kernel_socket_policies_map and
 * necp_kernel_socket_policies_app_layer_map. The app layer map is used for
 * policy checks coming in from user space, and is one list with duplicate and
 * overlapping policies removed. The socket map hashes based on app UUID, and
 * removes duplicate and overlapping policies. necp_kernel_socket_policy -->
 ...
 * Drop All Level
 * ------------------------------------------------
 * The Drop All Level is a sysctl that controls the level at which policies are
 * allowed to override a global drop rule. If the value is 0, no drop rule is
 * applied. If the value is 1, all traffic is dropped. If the value is greater
 * than 1, all kernel policies created by a session with a priority level better
 * than (numerically less than) the Drop All Level will allow matching traffic
 * to not be dropped. The Drop All Level is dynamically interpreted into
 * necp_drop_all_order, which specifies the equivalent assigned session orders
 * to be dropped.
 */
...
u_int32_t necp_drop_unentitled_level =
...
static KALLOC_TYPE_DEFINE(necp_session_policy_zone, struct necp_session_policy,
                          NET_KT_DEFAULT);
...
#define LIST_INSERT_SORTED_ASCENDING(head, elm, field, sortfield, tmpelm)      \
  do {                                                                         \
    if (LIST_EMPTY((head)) ||                                                  \
        (LIST_FIRST(head)->sortfield >= (elm)->sortfield)) {                   \
      LIST_INSERT_HEAD((head), elm, field);                                    \
    } else {                                                                   \
      LIST_FOREACH(tmpelm, head, field) {                                      \
        if (LIST_NEXT(tmpelm, field) == NULL ||                                \
            LIST_NEXT(tmpelm, field)->sortfield >= (elm)->sortfield) {         \
...
#define LIST_INSERT_SORTED_TWICE_ASCENDING(head, elm, field, firstsortfield,   \
                                           secondsortfield, tmpelm)            \
  do {                                                                         \
    if (LIST_EMPTY((head)) ||                                                  \
        (LIST_FIRST(head)->firstsortfield > (elm)->firstsortfield) ||          \
        ((LIST_FIRST(head)->firstsortfield == (elm)->firstsortfield) &&        \
         (LIST_FIRST(head)->secondsortfield >= (elm)->secondsortfield))) {     \
      LIST_INSERT_HEAD((head), elm, field);                                    \
    } else {                                                                   \
      LIST_FOREACH(tmpelm, head, field) {                                      \
        if (LIST_NEXT(tmpelm, field) == NULL ||                                \
            (LIST_NEXT(tmpelm, field)->firstsortfield >                        \
             (elm)->firstsortfield) ||                                         \
            ((LIST_NEXT(tmpelm, field)->firstsortfield ==                      \
              (elm)->firstsortfield) &&                                        \
             (LIST_NEXT(tmpelm, field)->secondsortfield >=                     \
              (elm)->secondsortfield))) {                                      \
          LIST_INSERT_AFTER(tmpelm, elm, field);                               \
          break;                                                               \
        }                                                                      \
...
#define LIST_INSERT_SORTED_THRICE_ASCENDING(                                   \
    head, elm, field, firstsortfield, secondsortfield, thirdsortfield, tmpelm) \
  do {                                                                         \
    if (LIST_EMPTY((head)) ||                                                  \
        (LIST_FIRST(head)->firstsortfield > (elm)->firstsortfield) ||          \
        ((LIST_FIRST(head)->firstsortfield == (elm)->firstsortfield) &&        \
        ...
        ((LIST_FIRST(head)->firstsortfield == (elm)->firstsortfield) &&        \
         (LIST_FIRST(head)->secondsortfield == (elm)->secondsortfield) &&      \
         (LIST_FIRST(head)->thirdsortfield >= (elm)->thirdsortfield))) {       \
      LIST_INSERT_HEAD((head), elm, field);                                    \
    } else {                                                                   \
      LIST_FOREACH(tmpelm, head, field) {                                      \
        if (LIST_NEXT(tmpelm, field) == NULL ||                                \
            ...
            ((LIST_NEXT(tmpelm, field)->firstsortfield ==                      \
             ...
             (LIST_NEXT(tmpelm, field)->secondsortfield ==                     \
              (elm)->secondsortfield) &&                                       \
             (LIST_NEXT(tmpelm, field)->thirdsortfield >=                      \
              (elm)->thirdsortfield))) {                                       \
          LIST_INSERT_AFTER(tmpelm, elm, field);                               \
          break;                                                               \
        }                                                                      \
...
#define IS_NECP_ROUTE_RULE_DENY(x)                                             \
  ((x) == NECP_ROUTE_RULE_DENY_INTERFACE ||                                    \
   (x) == NECP_ROUTE_RULE_DENY_INTERFACE_WITH_TYPE)

#define IS_NECP_ROUTE_RULE_ALLOW_OR_DENY(x)                                    \
  (IS_NECP_ROUTE_RULE_DENY(x) || (x) == NECP_ROUTE_RULE_ALLOW_INTERFACE)
...
#define NECP_KERNEL_CONDITION_LAST_INTERFACE                                   \
  0x010000 // Only set from packets looping between interfaces
...
// Cap the policy size at the max result + conditions size, with room for extra
// TLVs
#define NECP_MAX_POLICY_SIZE                                                   \
  (1024 + NECP_MAX_POLICY_RESULT_SIZE + NECP_MAX_CONDITIONS_ARRAY_SIZE)
...
static LIST_HEAD(necp_domain_filter_list,
                 necp_domain_filter) necp_global_domain_filter_list;

/*
 * Filter id space is split between domain bloom filter id and domain trie
 * filter id. id <= 10000 - bloom filter ids id > 10000 - trie filter ids
 */
...
static LIST_HEAD(necp_domain_trie_list,
                 necp_domain_trie) necp_global_domain_trie_list;
...
#define BUMP_KERNEL_SOCKET_POLICIES_GENERATION_COUNT()                         \
  do {                                                                         \
    if (OSIncrementAtomic(&necp_kernel_socket_policies_gencount) ==            \
        (INT32_MAX - 1)) {                                                     \
...
static LIST_HEAD(_necpkernelsocketconnectpolicies,
                 necp_kernel_socket_policy) necp_kernel_socket_policies;
#define NECP_KERNEL_SOCKET_POLICIES_MAP_NUM_APP_ID_BUCKETS 5
#define NECP_SOCKET_MAP_APP_ID_TO_BUCKET(appid)                                \
  (appid ? (appid % (NECP_KERNEL_SOCKET_POLICIES_MAP_NUM_APP_ID_BUCKETS - 1) + \
            1)                                                                 \
         : 0)
static size_t necp_kernel_socket_policies_map_counts
    [NECP_KERNEL_SOCKET_POLICIES_MAP_NUM_APP_ID_BUCKETS];
static struct necp_kernel_socket_policy **__indexable
    necp_kernel_socket_policies_map
        [NECP_KERNEL_SOCKET_POLICIES_MAP_NUM_APP_ID_BUCKETS];
...
static struct necp_kernel_socket_policy **__indexable
    necp_kernel_socket_policies_app_layer_map;
/*
 * A note on policy 'maps': these are used for boosting efficiency when matching
 * policies. For each dimension of the map, such as an ID, the 0 bucket is
 * reserved for sockets/packets that do not have this parameter, while the other
 * buckets lead to an array of policy pointers that form the list applicable
 * when the (parameter%(NUM_BUCKETS - 1) + 1) == bucket_index.
 *
 * For example, a packet with policy ID of 7, when there are 4 ID buckets, will
 * map to bucket (7%3 + 1) = 2.
 */
...
static LIST_HEAD(_necpkernelipoutputpolicies,
                 necp_kernel_ip_output_policy) necp_kernel_ip_output_policies;
#define NECP_KERNEL_IP_OUTPUT_POLICIES_MAP_NUM_ID_BUCKETS 5
#define NECP_IP_OUTPUT_MAP_ID_TO_BUCKET(id)                                    \
  (id ? (id % (NECP_KERNEL_IP_OUTPUT_POLICIES_MAP_NUM_ID_BUCKETS - 1) + 1) : 0)
static size_t necp_kernel_ip_output_policies_map_counts
    [NECP_KERNEL_IP_OUTPUT_POLICIES_MAP_NUM_ID_BUCKETS];
static struct necp_kernel_ip_output_policy **__indexable
...
necp_handle_policy_add(struct necp_session *session,
                       u_int8_t *__sized_by(tlv_buffer_length) tlv_buffer,
                       size_t tlv_buffer_length, int offset, int *error);
static int necp_handle_policy_dump_all(user_addr_t out_buffer,
                                       size_t out_buffer_length);

#define MAX_RESULT_STRING_LEN 64
static inline const char *__sized_by(MAX_RESULT_STRING_LEN)
    necp_get_result_description(
        char *__sized_by(MAX_RESULT_STRING_LEN) result_string,
        necp_kernel_policy_result result,
        necp_kernel_policy_result_parameter result_parameter);

static struct necp_session_policy *necp_policy_create(
    struct necp_session *session, necp_policy_order order,
    u_int8_t *__sized_by(conditions_array_size) conditions_array,
    u_int32_t conditions_array_size,
    u_int8_t *__sized_by(route_rules_array_size) route_rules_array,
    u_int32_t route_rules_array_size, u_int8_t *__sized_by(result_size) result,
    u_int32_t result_size);
static struct necp_session_policy *
necp_policy_find(struct necp_session *session, necp_policy_id policy_id);
static bool necp_policy_mark_for_deletion(struct necp_session *session,
                                          struct necp_session_policy *policy);
static bool necp_policy_mark_all_for_deletion(struct necp_session *session);
static bool necp_policy_delete(struct necp_session *session,
                               struct necp_session_policy *policy);
static void necp_policy_apply_all(struct necp_session *session);

static necp_kernel_policy_id necp_kernel_socket_policy_add(
    necp_policy_order order, u_int32_t session_order, int session_pid,
    u_int64_t condition_mask, u_int64_t condition_negated_mask,
    necp_app_id cond_app_id, necp_app_id cond_real_app_id,
    char *cond_custom_entitlement __null_terminated, u_int32_t cond_account_id,
    char *cond_domain __null_terminated, u_int32_t cond_domain_filter,
    char *cond_url __null_terminated, pid_t cond_pid, int32_t cond_pid_version,
    uid_t cond_uid, uid_t cond_real_uid, ifnet_t cond_bound_interface,
    struct necp_policy_condition_tc_range cond_traffic_class,
    u_int16_t cond_protocol,
    union necp_sockaddr_union *__single cond_local_start,
    union necp_sockaddr_union *cond_local_end, u_int8_t cond_local_prefix,
    union necp_sockaddr_union *cond_remote_start,
    union necp_sockaddr_union *cond_remote_end, u_int8_t cond_remote_prefix,
    ...
    u_int16_t cond_packet_filter_tags, u_int16_t cond_scheme_port,
    u_int32_t cond_bound_interface_flags, u_int32_t cond_bound_interface_eflags,
    u_int32_t cond_bound_interface_xflags, u_int8_t cond_local_networks_flags,
    necp_kernel_policy_result result,
    necp_kernel_policy_result_parameter result_parameter);
...
necp_socket_find_policy_match_with_info_locked(
    ...
    struct necp_socket_info *info, necp_kernel_policy_filter *return_filter,
    u_int32_t *__counted_by(route_rule_id_array_count)
        return_route_rule_id_array,
    size_t *return_route_rule_id_array_count, size_t route_rule_id_array_count,
    u_int32_t *__counted_by(netagent_array_count) return_netagent_array,
    size_t netagent_array_count,
    u_int32_t *__counted_by(netagent_use_flags_array_count)
        return_netagent_use_flags_array,
    size_t netagent_use_flags_array_count,
    struct necp_client_parameter_netagent_type *__counted_by(
        num_required_agent_types) required_agent_types,
    u_int32_t num_required_agent_types, proc_t proc, u_int16_t pf_tag,
    necp_kernel_policy_id *skip_policy_id, struct rtentry *rt,
...
static necp_kernel_policy_id necp_kernel_ip_output_policy_add(
    necp_policy_order order, necp_policy_order suborder,
    u_int32_t session_order, int session_pid, u_int64_t condition_mask,
    u_int64_t condition_negated_mask, necp_kernel_policy_id cond_policy_id,
    ifnet_t cond_bound_interface, u_int32_t cond_last_interface_index,
    u_int16_t cond_protocol, union necp_sockaddr_union *cond_local_start,
    union necp_sockaddr_union *cond_local_end, u_int8_t cond_local_prefix,
    union necp_sockaddr_union *cond_remote_start,
    union necp_sockaddr_union *cond_remote_end, u_int8_t cond_remote_prefix,
    u_int16_t cond_packet_filter_tags, u_int16_t cond_scheme_port,
...
static bool necp_is_addr_in_range(struct sockaddr *addr,
                                  struct sockaddr *range_start,
                                  struct sockaddr *range_end);
static bool necp_is_range_in_range(struct sockaddr *inner_range_start,
                                   struct sockaddr *inner_range_end,
                                   struct sockaddr *range_start,
                                   struct sockaddr *range_end);
static bool necp_is_addr_in_subnet(struct sockaddr *addr,
                                   struct sockaddr *subnet_addr,
                                   u_int8_t subnet_prefix);
static int necp_addr_compare(struct sockaddr *sa1, struct sockaddr *sa2,
                             int check_port);
static bool necp_buffer_compare_with_bit_prefix(u_int8_t *__indexable p1,
                                                u_int8_t *__indexable p2,
                                                u_int32_t bits);
static bool necp_addr_is_empty(struct sockaddr *addr);
static bool necp_is_loopback(struct sockaddr *local_addr,
                             struct sockaddr *remote_addr, struct inpcb *inp,
                             struct mbuf *packet,
                             u_int32_t bound_interface_index);
...
static LIST_HEAD(necp_uuid_id_mapping_head, necp_uuid_id_mapping) *
    __counted_by(necp_uuid_app_id_hash_num_buckets) necp_uuid_app_id_hashtbl,
    necp_agent_uuid_id_list; // App map is real hash table, agent map is just
                             // mapping
#define APPUUIDHASH(uuid)                                                      \
  (&necp_uuid_app_id_hashtbl                                                   \
       [uuid[0] & necp_uuid_app_id_hash_mask]) // Assume first byte of UUIDs are
                                               // evenly distributed
static u_int32_t necp_create_uuid_app_id_mapping(uuid_t uuid,
                                                 bool *allocated_mapping,
                                                 bool uuid_policy_table);
static bool necp_remove_uuid_app_id_mapping(uuid_t uuid, bool *removed_mapping,
                                            bool uuid_policy_table);
static struct necp_uuid_id_mapping *
necp_uuid_lookup_uuid_with_app_id_locked(u_int32_t agent_id);
...
static LIST_HEAD(necp_agent_type_id_mapping_list,
                 necp_agent_type_id_mapping) necp_agent_type_id_list;
static u_int32_t necp_create_agent_type_to_id_mapping(
    struct necp_policy_condition_agent_type *agent_type);
...
static LIST_HEAD(necp_string_id_mapping_list,
                 necp_string_id_mapping) necp_account_id_list;
static u_int32_t
necp_create_string_to_id_mapping(struct necp_string_id_mapping_list *list,
                                 char *domain __null_terminated);
static bool
necp_remove_string_to_id_mapping(struct necp_string_id_mapping_list *list,
                                 char *domain __null_terminated);
static struct necp_string_id_mapping *
necp_lookup_string_with_id_locked(struct necp_string_id_mapping_list *list,
                                  u_int32_t local_id);

static u_int32_t
necp_create_domain_filter(struct necp_domain_filter_list *list,
                          struct necp_domain_filter_list *owner_list,
                          struct net_bloom_filter *filter);
static bool
necp_remove_domain_filter(struct necp_domain_filter_list *list,
                          struct necp_domain_filter_list *owner_list,
                          u_int32_t filter_id);
static struct necp_domain_filter *
necp_lookup_domain_filter(struct necp_domain_filter_list *list,
                          u_int32_t filter_id);

static u_int32_t
necp_create_domain_trie(struct necp_domain_trie_list *list,
                        struct necp_domain_trie_list *owner_list,
                        struct necp_domain_trie_request *necp_trie_request,
                        size_t necp_trie_request_size);
static bool
necp_remove_domain_trie(struct necp_domain_trie_list *list,
                        __unused struct necp_domain_trie_list *owner_list,
                        u_int32_t id);
...
static Boolean necp_match_domain_with_trie(struct necp_domain_trie_list *list,
                                           u_int32_t id,
                                           char *__sized_by(length) domain,
                                           size_t length);
...
static inline int necp_count_dots(char *__sized_by(length) string,
                                  size_t length);

static char *__null_terminated necp_copy_string(char *__sized_by(length) string,
                                                size_t length);
static bool
necp_update_qos_marking(struct ifnet *ifp,
                        u_int32_t *__counted_by(netagent_array_count)
                            netagent_array,
                        size_t netagent_array_count, u_int32_t route_rule_id);
...
static u_int32_t necp_create_route_rule(
    struct necp_route_rule_list *list,
    u_int8_t *__sized_by(route_rules_array_size) route_rules_array,
    u_int32_t route_rules_array_size, bool *has_socket_only_actions);
static bool necp_remove_route_rule(struct necp_route_rule_list *list,
                                   u_int32_t route_rule_id);
static bool necp_route_is_interface_type_allowed(struct rtentry *route,
                                                 struct ifnet *ifp, proc_t proc,
                                                 struct inpcb *inp);
static bool necp_route_is_allowed(struct rtentry *route, ifnet_t interface,
                                  u_int32_t *__counted_by(netagent_array_count)
                                      netagent_array,
                                  size_t netagent_array_count,
                                  u_int32_t route_rule_id,
                                  u_int32_t *interface_type_denied,
                                  bool *ultra_constrained_denied);
static uint32_t necp_route_get_netagent(
    struct rtentry *route,
    u_int32_t *__counted_by(netagent_array_count) netagent_array,
    size_t netagent_array_count, u_int32_t route_rule_id, bool *remove);
static bool necp_route_rule_matches_agents(u_int32_t route_rule_id);
static uint32_t
necp_route_get_flow_divert(struct rtentry *route,
                           u_int32_t *__counted_by(netagent_array_count)
                               netagent_array,
                           size_t netagent_array_count, u_int32_t route_rule_id,
                           u_int32_t *flow_divert_aggregate_unit);
static struct necp_route_rule *
necp_lookup_route_rule_locked(struct necp_route_rule_list *list,
                              u_int32_t route_rule_id);
static inline void necp_get_parent_is_entitled(task_t task,
...
static LIST_HEAD(necp_aggregate_route_rule_list,
                 necp_aggregate_route_rule) necp_aggregate_route_rules;
static u_int32_t necp_create_aggregate_route_rule(
    u_int32_t *__counted_by(MAX_AGGREGATE_ROUTE_RULES) rule_ids);
...
SYSCTL_INT(_net_necp, NECPCTL_DEDUP_POLICIES, dedup_policies,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_dedup_policies, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_RESTRICT_MULTICAST, restrict_multicast,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_restrict_multicast, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_PASS_LOOPBACK, pass_loopback,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_pass_loopback, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_PASS_KEEPALIVES, pass_keepalives,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_pass_keepalives, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_PASS_INTERPOSE, pass_interpose,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_pass_interpose, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_DEBUG, debug, CTLFLAG_LOCKED | CTLFLAG_RW,
           &necp_debug, 0, "");
SYSCTL_PROC(_net_necp, NECPCTL_DROP_UNENTITLED_LEVEL, drop_unentitled_level,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW,
            &necp_drop_unentitled_level, 0,
            &sysctl_handle_necp_unentitled_level, "IU", "");
SYSCTL_PROC(_net_necp, NECPCTL_DROP_MANAGEMENT_LEVEL, drop_management_level,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW,
            &necp_drop_management_level, 0,
            &sysctl_handle_necp_management_level, "IU", "");
SYSCTL_PROC(_net_necp, NECPCTL_DROP_ALL_LEVEL, drop_all_level,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW, &necp_drop_all_level, 0,
            &sysctl_handle_necp_level, "IU", "");
SYSCTL_LONG(_net_necp, NECPCTL_SOCKET_POLICY_COUNT, socket_policy_count,
            CTLFLAG_LOCKED | CTLFLAG_RD, &necp_kernel_socket_policies_count,
            "");
SYSCTL_LONG(_net_necp, NECPCTL_SOCKET_NON_APP_POLICY_COUNT,
            socket_non_app_policy_count, CTLFLAG_LOCKED | CTLFLAG_RD,
            &necp_kernel_socket_policies_non_app_count, "");
SYSCTL_LONG(_net_necp, NECPCTL_IP_POLICY_COUNT, ip_policy_count,
            CTLFLAG_LOCKED | CTLFLAG_RD, &necp_kernel_ip_output_policies_count,
            "");
SYSCTL_INT(_net_necp, NECPCTL_SESSION_COUNT, session_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_session_count, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_TRIE_COUNT, trie_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_trie_count, 0, "");
...
SYSCTL_INT(_net_necp, OID_AUTO, drop_dest_debug, CTLFLAG_LOCKED | CTLFLAG_RW,
           &necp_drop_dest_debug, 0, "");

static int sysctl_handle_necp_drop_dest_level SYSCTL_HANDLER_ARGS;
SYSCTL_PROC(_net_necp, OID_AUTO, drop_dest_level,
            CTLTYPE_STRUCT | CTLFLAG_LOCKED | CTLFLAG_ANYBODY | CTLFLAG_RW, 0,
            0, &sysctl_handle_necp_drop_dest_level, "S,necp_drop_dest_level",
            "");

static bool necp_address_matches_drop_dest_policy(union necp_sockaddr_union *,
                                                  u_int32_t);

/*
 * data tracing control -
 *
 * necp_data_tracing_level    : 1 for brief trace, 2 for policy details, 3 for
 * condition details necp_data_tracing_port     : match traffic with specified
 * port necp_data_tracing_proto    : match traffic with specified protocol
 * necp_data_tracing_pid      : match traffic with specified pid (only applied
 * at socket level) necp_data_tracing_ifindex  : match traffic on specified
 * ifindex necp_data_tracing_match_all: trace traffic only if ALL specified
 * attributes matched.  Default is 0 to trace traffic if any specified
 * attributes matched. data_tracing_session_order     : match policies in the
 * specified session - log traffic that hit these policies
 * necp_data_tracing_policy_order : match specified policy - log traffic that
 * hit this policy
 */
static int necp_data_tracing_level = 0;
SYSCTL_INT(_net_necp, OID_AUTO, data_tracing_level, CTLFLAG_LOCKED | CTLFLAG_RW,
           &necp_data_tracing_level, 0, "");

static int necp_data_tracing_port = 0;
SYSCTL_INT(_net_necp, OID_AUTO, data_tracing_port, CTLFLAG_LOCKED | CTLFLAG_RW,
           &necp_data_tracing_port, 0, "");

static int necp_data_tracing_proto = 0;
SYSCTL_INT(_net_necp, OID_AUTO, data_tracing_proto, CTLFLAG_LOCKED | CTLFLAG_RW,
           &necp_data_tracing_proto, 0, "");

static int necp_data_tracing_pid = 0;
SYSCTL_INT(_net_necp, OID_AUTO, data_tracing_pid, CTLFLAG_LOCKED | CTLFLAG_RW,
           &necp_data_tracing_pid, 0, "");

static int necp_data_tracing_ifindex = 0;
SYSCTL_INT(_net_necp, OID_AUTO, data_tracing_ifindex,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_data_tracing_ifindex, 0, "");

static int necp_data_tracing_match_all = 0;
SYSCTL_INT(_net_necp, OID_AUTO, data_tracing_match_all,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_data_tracing_match_all, 0, "");

static int necp_data_tracing_session_order = 0;
SYSCTL_INT(_net_necp, OID_AUTO, data_tracing_session_order,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_data_tracing_session_order, 0,
           "");

static int necp_data_tracing_policy_order = 0;
SYSCTL_INT(_net_necp, OID_AUTO, data_tracing_policy_order,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_data_tracing_policy_order, 0, "");
...
#define NECP_DATA_TRACE_PID_MATCHED(pid) (pid == necp_data_tracing_pid)
...
#define NECP_DATA_TRACE_LOCAL_PORT_MATCHED(local_addr)                         \
  (local_addr &&                                                               \
   (ntohs(local_addr->sin.sin_port) == necp_data_tracing_port ||               \
    ntohs(local_addr->sin6.sin6_port) == necp_data_tracing_port))
#define NECP_DATA_TRACE_REMOTE_ORT_MATCHED(remote_addr)                        \
  (remote_addr &&                                                              \
   (ntohs(remote_addr->sin.sin_port) == necp_data_tracing_port ||              \
    ntohs(remote_addr->sin6.sin6_port) == necp_data_tracing_port))
#define NECP_DATA_TRACE_IFINDEX_MATCHED(ifindex)                               \
  (ifindex == necp_data_tracing_ifindex)

#define NECP_ENABLE_DATA_TRACE_OR(local_addr, remote_addr, protocol, pid,      \
                                  ifindex)                                     \
  ((necp_data_tracing_level &&                                                 \
    ((necp_data_tracing_pid && (!pid || NECP_DATA_TRACE_PID_MATCHED(pid))) ||  \
     ...
     (necp_data_tracing_ifindex &&                                             \
      NECP_DATA_TRACE_IFINDEX_MATCHED(ifindex)) ||                             \
     (necp_data_tracing_port &&                                                \
      (NECP_DATA_TRACE_LOCAL_PORT_MATCHED(local_addr) ||                       \
       NECP_DATA_TRACE_REMOTE_ORT_MATCHED(remote_addr)))))                     \
       ? necp_data_tracing_level                                               \
       : 0)

#define NECP_ENABLE_DATA_TRACE_AND(local_addr, remote_addr, protocol, pid,     \
                                   ifindex)                                    \
  ((necp_data_tracing_level &&                                                 \
    ((!necp_data_tracing_pid || !pid || NECP_DATA_TRACE_PID_MATCHED(pid)) &&   \
     (!necp_data_tracing_proto || NECP_DATA_TRACE_PROTO_MATCHED(protocol)) &&  \
...
#define NECP_ENABLE_DATA_TRACE(local_addr, remote_addr, protocol, pid,         \
                               ifindex)                                        \
  (necp_data_tracing_match_all                                                 \
       ? NECP_ENABLE_DATA_TRACE_AND(local_addr, remote_addr, protocol, pid,    \
                                    ifindex)                                   \
       : NECP_ENABLE_DATA_TRACE_OR(local_addr, remote_addr, protocol, pid,     \
                                   ifindex))

#define NECP_DATA_TRACE_ON(debug) (debug)
#define NECP_DATA_TRACE_POLICY_ON(debug) (debug > NECP_DATA_TRACE_LEVEL_BRIEF)
#define NECP_DATA_TRACE_CONDITION_ON(debug)                                    \
  (debug > NECP_DATA_TRACE_LEVEL_POLICY)
#define NECP_DATA_TRACE_DP_ON(debug) (debug > NECP_DATA_TRACE_LEVEL_CONDITION)

const char *necp_get_address_string(union necp_sockaddr_union *address,
                                    char addr_str[MAX_IPv6_STR_LEN]);

#define NECP_DATA_TRACE_LOG_APP_LEVEL(debug, caller, log_msg, policy_id,       \
                                      skip_policy_id)                          \
  if (NECP_DATA_TRACE_ON(debug)) {                                             \
    char laddr_str[MAX_IPv6_STR_LEN];                                          \
    char raddr_str[MAX_IPv6_STR_LEN];                                          \
    NECPDATATRACELOG(LOG_ERR,                                                  \
                     "DATA-TRACE <%s>: %s - fam %d proto %d port <local "      \
                     "%d/%d remote %d/%d> <local %s remote %s> <drop-all "     \
                     "order %d> <pid=%d Application %d Real Application %d "   \
                     "BoundInterface %d> <policy_id %d skip_policy_id %d>",    \
                     caller, log_msg, info.local_addr.sin.sin_family,          \
                     info.protocol, ntohs(info.local_addr.sin.sin_port),       \
                     ntohs(info.local_addr.sin6.sin6_port),                    \
                     ntohs(info.remote_addr.sin.sin_port),                     \
                     ntohs(info.remote_addr.sin6.sin6_port),                   \
                     necp_get_address_string(&info.local_addr, laddr_str),     \
                     necp_get_address_string(&info.remote_addr, raddr_str),    \
                     necp_drop_all_order, info.pid, info.application_id,       \
                     info.real_application_id, info.bound_interface_index,     \
                     policy_id, skip_policy_id);                               \
  }

#define NECP_DATA_TRACE_LOG_SOCKET(debug, socket, caller, log_msg, policy_id,  \
                                   skip_policy_id)                             \
  if (NECP_DATA_TRACE_ON(debug)) {                                             \
    char laddr_str[MAX_IPv6_STR_LEN];                                          \
    char raddr_str[MAX_IPv6_STR_LEN];                                          \
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s %llx>: %s - fam %d proto %d port <local %d/%d remote " \
        "%d/%d> <local %s remote %s> <drop-all order %d> <pid=%d Application " \
        "%d Real Application %d BoundInterface %d> <policy_id %d "             \
        "skip_policy_id %d result %d>",                                        \
        caller, (uint64_t)VM_KERNEL_ADDRPERM(socket), log_msg,                 \
        info.local_addr.sin.sin_family, info.protocol,                         \
        ntohs(info.local_addr.sin.sin_port),                                   \
        ntohs(info.local_addr.sin6.sin6_port),                                 \
        ntohs(info.remote_addr.sin.sin_port),                                  \
        ntohs(info.remote_addr.sin6.sin6_port),                                \
        necp_get_address_string(&info.local_addr, laddr_str),                  \
        necp_get_address_string(&info.remote_addr, raddr_str),                 \
        necp_drop_all_order, info.pid, info.application_id,                    \
        info.real_application_id, info.bound_interface_index, policy_id,       \
        skip_policy_id, inp ? inp->inp_policyresult.results.result : 0);       \
  }

#define NECP_DATA_TRACE_LOG_SOCKET_DP(debug, socket, caller, log_msg,          \
                                      policy_id, skip_policy_id)               \
  if (NECP_DATA_TRACE_ON(debug)) {                                             \
    char laddr_str[MAX_IPv6_STR_LEN];                                          \
    char raddr_str[MAX_IPv6_STR_LEN];                                          \
    NECPDATATRACELOG(                                                          \
        ...
        "%d/%d> <local %s remote %s> <drop-all order %d> <pid=%d Application " \
        "%d Real Application %d BoundInterface %d> <policy_id %d "             \
        "skip_policy_id %d result %d> <input ifindex %d> <allowed_to_receive " \
        "%d><pf_tag %X pass_flags %X>",                                        \
        caller, (uint64_t)VM_KERNEL_ADDRPERM(socket), log_msg,                 \
        info.local_addr.sin.sin_family, info.protocol,                         \
        ntohs(info.local_addr.sin.sin_port),                                   \
        ntohs(info.local_addr.sin6.sin6_port),                                 \
        ...
        info.real_application_id, info.bound_interface_index, policy_id,       \
        skip_policy_id, inp ? inp->inp_policyresult.results.result : 0,        \
        verifyifindex, allowed_to_receive, pf_tag, pass_flags);                \
  }
...
#define NECP_DATA_TRACE_LOG_SOCKET_RESULT(debug, socket, caller, log_msg)      \
  if (NECP_DATA_TRACE_ON(debug)) {                                             \
    ...
    NECPDATATRACELOG(                                                          \
        ...
        "DATA-TRACE <%s %llx>: %s - fam %d proto %d port <local %d/%d remote " \
        "%d/%d> <local %s remote %s> <drop-all order %d> <pid=%d Application " \
        "%d Real Application %d BoundInterface %d> (policy id=%d "             \
        "session_order=%d policy_order=%d result=%s)",                         \
        caller, (uint64_t)VM_KERNEL_ADDRPERM(socket), log_msg,                 \
        info->local_addr.sin.sin_family, info->protocol,                       \
        ntohs(info->local_addr.sin.sin_port),                                  \
        ntohs(info->local_addr.sin6.sin6_port),                                \
        ...
        necp_drop_all_order, info->pid, info->application_id,                  \
        info->real_application_id, info->bound_interface_index,                \
        policy_search_array[i]->id, policy_search_array[i]->session_order,     \
        policy_search_array[i]->order,                                         \
        resultString[policy_search_array[i]->result]);                         \
  }

#define NECP_DATA_TRACE_LOG_SOCKET_BRIEF(                                      \
    debug, socket, caller, log_msg, policy_id, skip_policy_id,                 \
    cached_policy_id, cached_skip_policy_id)                                   \
  if (NECP_DATA_TRACE_ON(debug)) {                                             \
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s %llx>: %s - <policy_id %d skip_policy_id %d> <cached " \
        "policy_id %d skip_policy_id %d>",                                     \
        caller, (uint64_t)VM_KERNEL_ADDRPERM(socket), log_msg, policy_id,      \
        skip_policy_id, cached_policy_id, cached_skip_policy_id);              \
  }
...
#define NECP_DATA_TRACE_LOG_IP4(debug, caller, log_msg)                        \
  if (NECP_DATA_TRACE_ON(debug)) {                                             \
    ...
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s>: %s - fam %d proto %d port <local %d/%d remote "      \
        "%d/%d> <local %s remote %s> <drop-all order %d> <BoundInterface %d> " \
        "<socket policy id %d socket skip id %d> <mbuf %X len %d %d>",         \
        caller, log_msg, local_addr.sin.sin_family, protocol,                  \
        ntohs(local_addr.sin.sin_port), ntohs(local_addr.sin6.sin6_port),      \
        ntohs(remote_addr.sin.sin_port), ntohs(remote_addr.sin6.sin6_port),    \
        necp_get_address_string(&local_addr, laddr_str),                       \
        necp_get_address_string(&remote_addr, raddr_str), necp_drop_all_order, \
        bound_interface_index, socket_policy_id, socket_skip_policy_id,        \
...
#define NECP_DATA_TRACE_LOG_IP6(debug, caller, log_msg)                        \
  if (NECP_DATA_TRACE_ON(debug)) {                                             \
    ...
    NECPDATATRACELOG(                                                          \
        ...
        "DATA-TRACE <%s>: %s - fam %d proto %d port <local %d/%d remote "      \
        "%d/%d> <local %s remote %s> <drop-all order %d> <BoundInterface %d> " \
        "<socket policy id %d socket skip id %d> <mbuf %X len %d %d %d>",      \
        caller, log_msg, local_addr.sin.sin_family, protocol,                  \
        ntohs(local_addr.sin.sin_port), ntohs(local_addr.sin6.sin6_port),      \
        ntohs(remote_addr.sin.sin_port), ntohs(remote_addr.sin6.sin6_port),    \
        necp_get_address_string(&local_addr, laddr_str),                       \
        necp_get_address_string(&remote_addr, raddr_str), necp_drop_all_order, \
        bound_interface_index, socket_policy_id, socket_skip_policy_id,        \
        (unsigned int)packet, ip6->ip6_plen, ntohs(ip6->ip6_plen),             \
        packet ? packet->m_pkthdr.len : 0);                                    \
  }
...
#define NECP_DATA_TRACE_LOG_IP_RESULT(debug, caller, log_msg)                  \
  if (NECP_DATA_TRACE_ON(debug)) {                                             \
    ...
    NECPDATATRACELOG(                                                          \
        ...
        "DATA-TRACE <%s>: %s - fam %d proto %d port <local %d/%d remote "      \
        "%d/%d> <local %s remote %s> <drop-all order %d> <BoundInterface %d> " \
        "(policy id=%d session_order=%d policy_order=%d result=%s)",           \
        caller, log_msg, local_addr->sin.sin_family, protocol,                 \
        ntohs(local_addr->sin.sin_port), ntohs(local_addr->sin6.sin6_port),    \
        ntohs(remote_addr->sin.sin_port), ntohs(remote_addr->sin6.sin6_port),  \
        necp_get_address_string(local_addr, laddr_str),                        \
        necp_get_address_string(remote_addr, raddr_str), necp_drop_all_order,  \
        bound_interface_index, policy_search_array[i]->id,                     \
        policy_search_array[i]->session_order, policy_search_array[i]->order,  \
        resultString[policy_search_array[i]->result]);                         \
  }
...
#define NECP_DATA_TRACE_LOG_POLICY_SOCKET(debug, socket, caller, log_msg)      \
  if (NECP_DATA_TRACE_POLICY_ON(debug)) {                                      \
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s %llx>: %s - policy id=%d session_order=%d "            \
        "policy_order=%d result=%s (cond_policy_id %d) (skip_session_order "   \
        "%d skip_order %d)",                                                   \
        caller, (uint64_t)VM_KERNEL_ADDRPERM(socket), log_msg,                 \
        policy_search_array[i]->id, policy_search_array[i]->session_order,     \
        policy_search_array[i]->order,                                         \
        resultString[policy_search_array[i]->result],                          \
        policy_search_array[i]->condition_mask &                               \
                NECP_KERNEL_CONDITION_POLICY_ID                                \
            ? policy_search_array[i]->cond_policy_id                           \
            : 0,                                                               \
        skip_session_order, skip_order);                                       \
  }
...
#define NECP_DATA_TRACE_LOG_POLICY_IP(debug, caller, log_msg)                  \
  if (NECP_DATA_TRACE_POLICY_ON(debug)) {                                      \
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s>: %s - policy id=%d session_order=%d policy_order=%d " \
        "result=%s (cond_policy_id %d) (skip_session_order %d skip_order %d)", \
        caller, log_msg, policy_search_array[i]->id,                           \
        policy_search_array[i]->session_order, policy_search_array[i]->order,  \
        resultString[policy_search_array[i]->result],                          \
        policy_search_array[i]->condition_mask &                               \
                NECP_KERNEL_CONDITION_POLICY_ID                                \
...
#define NECP_DATA_TRACE_LOG_CONDITION_IP3(debug, caller, negate, name, val1,   \
                                          val2, val3, input1, input2, input3)  \
  if (NECP_DATA_TRACE_CONDITION_ON(debug)) {                                   \
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s>: ------ %smatching <%s> <value (%d / 0x%X) (%d / "    \
        "0x%X) (%d / 0x%X) input (%d / 0x%X) (%d / 0x%X) (%d / 0x%X)>",        \
        caller, negate ? "!" : "", name, val1, val1, val2, val2, val3, val3,   \
        input1, input1, input2, input2, input3, input3);                       \
  }

#define NECP_DATA_TRACE_LOG_CONDITION_IP_STR3(                                 \
    debug, caller, negate, name, val1, val2, val3, input1, input2, input3)     \
  if (NECP_DATA_TRACE_CONDITION_ON(debug)) {                                   \
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s>: ------ %smatching <%s> <value %s %s %s input %s %s " \
        "%s>",                                                                 \
        caller, negate ? "!" : "", name, val1 != NULL ? val1 : "null",         \
        val2 != NULL ? val2 : "null", val3 != NULL ? val3 : "null",            \
        input1 != NULL ? input1 : "null", input2 != NULL ? input2 : "null",    \
        input3 != NULL ? input3 : "null");                                     \
  }

#define NECP_DATA_TRACE_LOG_CONDITION_IP(debug, caller, negate, name, val,     \
                                         input)                                \
  NECP_DATA_TRACE_LOG_CONDITION_IP3(debug, caller, negate, name, val, 0, 0,    \
                                    input, 0, 0)

#define NECP_DATA_TRACE_LOG_CONDITION_IP_STR(debug, caller, negate, name, val, \
                                             input)                            \
  NECP_DATA_TRACE_LOG_CONDITION_IP_STR3(debug, caller, negate, name, val,      \
                                        "n/a", "n/a", input, "n/a", "n/a")

#define NECP_DATA_TRACE_LOG_CONDITION_SOCKET3(debug, socket, caller, negate,   \
                                              name, val1, val2, val3, input1,  \
                                              input2, input3)                  \
  if (NECP_DATA_TRACE_CONDITION_ON(debug)) {                                   \
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s %llx>: ------ %smatching <%s> <value (%d / 0x%X) (%d " \
        "/ 0x%X) (%d / 0x%X) input (%d / 0x%X) (%d / 0x%X) (%d / 0x%X)>",      \
        caller, (uint64_t)VM_KERNEL_ADDRPERM(socket), negate ? "!" : "", name, \
        val1, val1, val2, val2, val3, val3, input1, input1, input2, input2,    \
        input3, input3);                                                       \
  }

#define NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR3(                             \
    debug, socket, caller, negate, name, val1, val2, val3, input1, input2,     \
    input3)                                                                    \
  if (NECP_DATA_TRACE_CONDITION_ON(debug)) {                                   \
    NECPDATATRACELOG(                                                          \
        LOG_ERR,                                                               \
        "DATA-TRACE <%s %llx>: ------ %smatching <%s> <value %s %s %s input "  \
        "%s %s %s>",                                                           \
        caller, (uint64_t)VM_KERNEL_ADDRPERM(socket), negate ? "!" : "", name, \
        val1 != NULL ? val1 : "null", val2 != NULL ? val2 : "null",            \
        val3 != NULL ? val3 : "null", input1 != NULL ? input1 : "null",        \
        input2 != NULL ? input2 : "null", input3 != NULL ? input3 : "null");   \
  }

#define NECP_DATA_TRACE_LOG_CONDITION_SOCKET(debug, socket, caller, negate,    \
                                             name, val, input)                 \
  NECP_DATA_TRACE_LOG_CONDITION_SOCKET3(debug, socket, caller, negate, name,   \
                                        val, 0, 0, input, 0, 0)

#define NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR(debug, socket, caller,        \
                                                 negate, name, val, input)     \
  NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR3(debug, socket, caller, negate,     \
                                            name, val, "n/a", "n/a", input,    \
                                            "n/a", "n/a")

#define NECP_IS_INTCOPROC_ADDRESS(addrv6)                                      \
  (IN6_IS_ADDR_LINKLOCAL(addrv6) &&                                            \
   addrv6->s6_addr32[2] == ntohl(0xaede48ff) &&                                \
   addrv6->s6_addr32[3] == ntohl(0xfe334455))

const char *resultString[NECP_POLICY_RESULT_MAX + 1] = {"INVALID",
                                                        "PASS",
                                                        "SKIP",
                                                        "DROP",
                                                        "SOCKET_DIVERT",
...
#define NECP_DDE_ENTITLEMENT                                                   \
  "com.apple.developer.media-device-discovery-extension"

static int necp_drop_loopback_count = 0;
SYSCTL_INT(_net_necp, OID_AUTO, drop_loopback_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_drop_loopback_count, 0, "");
...
necp_address_is_local_interface_address(union necp_sockaddr_union *addr) {
  ...
  if (remote_address_sanitized.ss_family == AF_INET6) {
    if (in6_embedded_scope ||
        !IN6_IS_SCOPE_EMBED(&SIN6(&remote_address_sanitized)->sin6_addr)) {
...
#define IS_NECP_DEST_IN_LOCAL_NETWORKS(rt, addr, include_local_addresses)      \
  ((rt) != NULL && !((rt)->rt_flags & RTF_GATEWAY) &&                          \
   (include_local_addresses || !((rt)->rt_flags & RTF_LOCAL)) &&               \
   ((rt)->rt_ifa && (rt)->rt_ifa->ifa_ifp &&                                   \
    !((rt)->rt_ifa->ifa_ifp->if_flags & IFF_POINTOPOINT) &&                    \
    !((rt)->rt_ifa->ifa_ifp->if_eflags & IFEF_DIRECTLINK)) &&                  \
   (include_local_addresses || addr == NULL ||                                 \
    !necp_address_is_local_interface_address(addr)))

// Session order allocation
static u_int32_t necp_allocate_new_session_order(u_int32_t priority,
                                                 u_int32_t control_unit) {
  u_int32_t new_order = 0;

  // For now, just allocate 1000 orders for each priority
  if (priority == NECP_SESSION_PRIORITY_UNKNOWN ||
      priority > NECP_SESSION_NUM_PRIORITIES) {
...
static inline u_int32_t necp_get_first_order_for_priority(u_int32_t priority) {
  if (priority == 0) {
    return 0;
  }
...
#pragma unused(arg1, arg2)
  int error = sysctl_handle_int(oidp, oidp->oid_arg1, oidp->oid_arg2, req);
  necp_drop_unentitled_order =
      necp_get_first_order_for_priority(necp_drop_unentitled_level);
  return error;
}

// Use a macro here to avoid computing the kauth_cred_t when
// necp_drop_unentitled_level is 0
static inline u_int32_t _necp_process_drop_order_inner(kauth_cred_t cred) {
...
#define necp_process_drop_order(_cred)                                         \
  (necp_drop_unentitled_order != 0 ? _necp_process_drop_order_inner(_cred)     \
                                   : necp_drop_unentitled_order)
...
#pragma unused(arg1, arg2)
  int error = sysctl_handle_int(oidp, oidp->oid_arg1, oidp->oid_arg2, req);
  necp_drop_management_order =
      necp_get_first_order_for_priority(necp_drop_management_level);
  return error;
}

static inline bool necp_socket_is_connected(struct inpcb *inp) {
  return inp->inp_socket->so_state &
...
static inline int necp_is_platform_binary(proc_t proc) {
  return (proc != NULL) ? (csproc_get_platform_binary(proc) && cs_valid(proc))
                        : 0;
}
...
necp_check_drop_all_bypass_result(proc_t proc) {
  ...
  const bool isConfigd =
      (necp_is_platform_binary(proc) && (signing_id = cs_identity_get(proc)) &&
  ...
  if (task == NULL ||
      !IOTaskHasEntitlement(task, "com.apple.private.necp.drop_all_bypass")) {
    return NECP_DROP_ALL_BYPASS_CHECK_RESULT_FALSE;
  } else {
    return NECP_DROP_ALL_BYPASS_CHECK_RESULT_TRUE;
  }
...
int necp_session_open(struct proc *p, struct necp_session_open_args *uap,
                      int *retval) {
#pragma unused(uap)
  int error = 0;
  ...
  if (uid != 0 && priv_check_cred(kauth_cred_get(),
                                  PRIV_NET_PRIVILEGED_NECP_POLICIES, 0) != 0) {
    NECPLOG0(
        LOG_ERR,
...
static int necp_session_op_close(struct fileglob *fg, vfs_context_t ctx) {
#pragma unused(ctx)
...
static int necp_session_find_from_fd(struct proc *p, int fd,
                                     struct fileproc **fpp,
                                     struct necp_session **session) {
  struct fileproc *__single fp = NULL;
...
static int necp_session_add_policy(struct necp_session *session,
                                   struct necp_session_action_args *uap,
                                   int *retval) {
  int error = 0;
  u_int8_t *__indexable tlv_buffer = NULL;

  if (uap->in_buffer_length == 0 ||
      uap->in_buffer_length > NECP_MAX_POLICY_SIZE || uap->in_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_add_policy invalid input (%zu)",
            (size_t)uap->in_buffer_length);
    error = EINVAL;
    goto done;
  }

  if (uap->out_buffer_length < sizeof(necp_policy_id) || uap->out_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_add_policy invalid output buffer (%zu)",
            (size_t)uap->out_buffer_length);
    error = EINVAL;
    goto done;
  }

  if ((tlv_buffer = (u_int8_t *)kalloc_data(uap->in_buffer_length,
  ...
  necp_policy_id new_policy_id = necp_handle_policy_add(
      session, tlv_buffer, uap->in_buffer_length, 0, &error);
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_session_add_policy failed to add policy (%d)",
            error);
    goto done;
  }
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_session_add_policy policy_id copyout error (%d)",
            error);
    goto done;
  }
...
static int necp_session_get_policy(struct necp_session *session,
                                   struct necp_session_action_args *uap,
                                   int *retval) {
  int error = 0;
  ...
  if (uap->in_buffer_length < sizeof(necp_policy_id) || uap->in_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_get_policy invalid input (%zu)",
            (size_t)uap->in_buffer_length);
    error = EINVAL;
    goto done;
  }
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_session_get_policy policy_id copyin error (%d)",
            error);
    goto done;
  }
  ...
  u_int32_t order_tlv_size =
      sizeof(u_int8_t) + sizeof(u_int32_t) + sizeof(necp_policy_order);
  u_int32_t result_tlv_size =
      (policy->result_size
           ? (sizeof(u_int8_t) + sizeof(u_int32_t) + policy->result_size)
           : 0);
  u_int32_t response_size =
      order_tlv_size + result_tlv_size + policy->conditions_size;

  if (uap->out_buffer_length < response_size || uap->out_buffer == 0) {
    NECPLOG(LOG_ERR,
            "necp_session_get_policy buffer not large enough (%zu < %u)",
            (size_t)uap->out_buffer_length, response_size);
    error = EINVAL;
    goto done;
  }

  if (response_size > NECP_MAX_POLICY_SIZE) {
    NECPLOG(LOG_ERR, "necp_session_get_policy size too large to copy (%u)",
            response_size);
    error = EINVAL;
    goto done;
  }
  ...
  cursor = necp_buffer_write_tlv(cursor, NECP_TLV_POLICY_ORDER,
                                 sizeof(necp_policy_order), &policy->order,
                                 response, response_size);
  if (result_tlv_size) {
    cursor = necp_buffer_write_tlv(cursor, NECP_TLV_POLICY_RESULT,
                                   policy->result_size, (void *)&policy->result,
                                   response, response_size);
  }
  if (policy->conditions_size) {
    memcpy(response + (cursor - response), policy->conditions,
           policy->conditions_size);
  }
...
static int necp_session_delete_policy(struct necp_session *session,
                                      struct necp_session_action_args *uap,
                                      int *retval) {
  int error = 0;

  if (uap->in_buffer_length < sizeof(necp_policy_id) || uap->in_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_delete_policy invalid input (%zu)",
            (size_t)uap->in_buffer_length);
    error = EINVAL;
    goto done;
  }
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_session_delete_policy policy_id copyin error (%d)",
            error);
    goto done;
  }

  struct necp_session_policy *policy =
      necp_policy_find(session, delete_policy_id);
  if (policy == NULL || policy->pending_deletion) {
    NECPLOG(LOG_ERR,
            "necp_session_delete_policy failed to find policy with id %u",
            delete_policy_id);
    error = ENOENT;
    goto done;
  }
...
static int necp_session_apply_all(struct necp_session *session,
                                  struct necp_session_action_args *uap,
                                  int *retval) {
#pragma unused(uap)
  necp_policy_apply_all(session);
...
static int necp_session_list_all(struct necp_session *session,
                                 struct necp_session_action_args *uap,
                                 int *retval) {
  u_int32_t tlv_size =
      (sizeof(u_int8_t) + sizeof(u_int32_t) + sizeof(necp_policy_id));
  ...
  if (num_policies > NECP_MAX_POLICY_LIST_COUNT) {
    NECPLOG(LOG_ERR,
            "necp_session_list_all size too large to copy (%u policies)",
            num_policies);
    error = EINVAL;
    goto done;
  }
  ...
  if (uap->out_buffer_length < response_size || uap->out_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_list_all buffer not large enough (%zu < %u)",
            (size_t)uap->out_buffer_length, response_size);
    error = EINVAL;
    goto done;
  }
  ...
  LIST_FOREACH(policy, &session->policies, chain) {
    if (!policy->pending_deletion && cur_policy_index < num_policies) {
      cursor =
          necp_buffer_write_tlv(cursor, NECP_TLV_POLICY_ID, sizeof(u_int32_t),
                                &policy->local_id, response, response_size);
      cur_policy_index++;
    }
  }
...
static int necp_session_delete_all(struct necp_session *session,
                                   struct necp_session_action_args *uap,
                                   int *retval) {
#pragma unused(uap)
  necp_policy_mark_all_for_deletion(session);
...
static int
necp_session_set_session_priority(struct necp_session *session,
                                  struct necp_session_action_args *uap,
                                  int *retval) {
  int error = 0;
  ...
  if (uap->in_buffer_length < sizeof(necp_session_priority) ||
      uap->in_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_set_session_priority invalid input (%zu)",
            (size_t)uap->in_buffer_length);
    error = EINVAL;
    goto done;
  }
  ...
  error = copyin(uap->in_buffer, &requested_session_priority,
                 sizeof(requested_session_priority));
  if (error != 0) {
    NECPLOG(LOG_ERR,
            "necp_session_set_session_priority priority copyin error (%d)",
            error);
    goto done;
  }
  ...
  if (requested_session_priority == NECP_SESSION_PRIORITY_CONTROL ||
    ...
    if (cred_result != 0) {
      NECPLOG(LOG_ERR,
              "Session does not hold necessary entitlement to claim priority "
              "level %d",
  ...
  if (session->session_priority != requested_session_priority) {
    ...
    session->session_order = necp_allocate_new_session_order(
        session->session_priority, session->control_unit);
    session->dirty = TRUE;
...
static int necp_session_lock_to_process(struct necp_session *session,
                                        struct necp_session_action_args *uap,
                                        int *retval) {
#pragma unused(uap)
...
static int necp_session_dump_all(struct necp_session *session,
                                 struct necp_session_action_args *uap,
                                 int *retval) {
#pragma unused(session)
  ...
  if (uap->out_buffer_length == 0 || uap->out_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_dump_all invalid output buffer (%zu)",
            (size_t)uap->out_buffer_length);
    error = EINVAL;
    goto done;
  }
...
static int necp_session_add_domain_filter(struct necp_session *session,
                                          struct necp_session_action_args *uap,
                                          int *retval) {
  int error = 0;
  ...
  if (in_buffer_length < sizeof(struct net_bloom_filter) ||
      in_buffer_length > NECP_MAX_DOMAIN_FILTER_SIZE || uap->in_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_add_domain_filter invalid input (%zu)",
  ...
  if (out_buffer_length < sizeof(u_int32_t) || uap->out_buffer == 0) {
    NECPLOG(LOG_ERR,
            "necp_session_add_domain_filter buffer not large enough (%zu)",
            (size_t)out_buffer_length);
    error = EINVAL;
    goto done;
  }

  bloom_filter = (struct net_bloom_filter *)kalloc_data(in_buffer_length,
                                                        Z_WAITOK | Z_ZERO);
  if (bloom_filter == NULL) {
    NECPLOG(LOG_ERR,
            "necp_session_add_domain_filter allocate filter error (%zu)",
            in_buffer_length);
    error = ENOMEM;
    goto done;
  }
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_session_add_domain_filter filter copyin error (%d)",
            error);
    goto done;
  }

  size_t expected_filter_size =
      net_bloom_filter_get_size(bloom_filter->b_table_num_bits);
  if (expected_filter_size != in_buffer_length) {
    NECPLOG(LOG_ERR,
            "necp_session_add_domain_filter size mismatch (%zu != %zu)",
            expected_filter_size, in_buffer_length);
    error = EINVAL;
    goto done;
  }

  lck_rw_lock_exclusive(&necp_kernel_policy_lock);
  u_int32_t filter_id = necp_create_domain_filter(
      &necp_global_domain_filter_list, &session->domain_filters, bloom_filter);
  lck_rw_done(&necp_kernel_policy_lock);

  if (filter_id == 0) {
    error = ENOMEM;
  } else {
    // Bloom filter is taken over by the new filter entry, clear the local
    // pointer
    bloom_filter = NULL;

    error = copyout(&filter_id, uap->out_buffer, sizeof(filter_id));
    if (error != 0) {
      NECPLOG(LOG_ERR, "necp_session_add_domain_filter ID copyout error (%d)",
              error);
      goto done;
    }
  }
...
static int
necp_session_remove_domain_filter(struct necp_session *session,
                                  struct necp_session_action_args *uap,
                                  int *retval) {
  int error = 0;
  ...
  if (in_buffer_length < sizeof(u_int32_t) || uap->in_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_remove_domain_filter invalid input (%zu)",
            (size_t)in_buffer_length);
    error = EINVAL;
    goto done;
  }
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_session_remove_domain_filter uuid copyin error (%d)",
            error);
    goto done;
  }

  lck_rw_lock_exclusive(&necp_kernel_policy_lock);
  bool removed = necp_remove_domain_filter(&necp_global_domain_filter_list,
                                           &session->domain_filters, filter_id);
  if (!removed) {
    error = ENOENT;
  }
  lck_rw_done(&necp_kernel_policy_lock);
...
static int
necp_session_remove_all_domain_filters(struct necp_session *session,
                                       struct necp_session_action_args *uap,
                                       int *retval) {
#pragma unused(uap)
  ...
  LIST_FOREACH_SAFE(filter, &session->domain_filters, owner_chain,
                    temp_filter) {
...
static int necp_session_add_domain_trie(struct necp_session *session,
                                        struct necp_session_action_args *uap,
                                        int *retval) {
  int error = 0;
  ...
  if (in_buffer_length < sizeof(struct necp_domain_trie_request) ||
      in_buffer_length > NECP_MAX_DOMAIN_TRIE_SIZE || uap->in_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_add_domain_trie invalid input (%zu)",
            (size_t)in_buffer_length);
    error = EINVAL;
    goto done;
  }
  ...
  if (out_buffer_length < sizeof(u_int32_t) || uap->out_buffer == 0) {
    NECPLOG(LOG_ERR,
            "necp_session_add_domain_trie buffer not large enough (%zu)",
            (size_t)out_buffer_length);
    error = EINVAL;
    goto done;
  }

  domain_trie_request = (struct necp_domain_trie_request *)kalloc_data(
      in_buffer_length, Z_WAITOK | Z_ZERO);
  if (domain_trie_request == NULL) {
    NECPLOG(LOG_ERR,
            "necp_session_add_domain_trie allocate trie request error (%zu)",
            in_buffer_length);
    error = ENOMEM;
    goto done;
  }
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_session_add_domain_trie filter copyin error (%d)",
            error);
    goto done;
  }

  NECPLOG(LOG_INFO,
          "necp_session_add_domain_trie received %zu bytes <trie mem size %d>",
          in_buffer_length, domain_trie_request->total_mem_size);

  lck_rw_lock_exclusive(&necp_kernel_policy_lock);
  u_int32_t id = necp_create_domain_trie(&necp_global_domain_trie_list,
                                         &session->domain_tries,
                                         domain_trie_request, in_buffer_length);
  lck_rw_done(&necp_kernel_policy_lock);
  ...
  if (id == 0) {
    ...
    if (error != 0) {
      NECPLOG(LOG_ERR, "necp_session_add_domain_trie ID copyout error (%d)",
              error);
      goto done;
    }
  }
...
static int necp_session_remove_domain_trie(struct necp_session *session,
                                           struct necp_session_action_args *uap,
                                           int *retval) {
  int error = 0;
  ...
  const size_t in_buffer_length = (size_t)uap->in_buffer_length;
  if (in_buffer_length < sizeof(u_int32_t) || uap->in_buffer == 0) {
    NECPLOG(LOG_ERR, "necp_session_remove_domain_trie invalid input (%zu)",
            (size_t)in_buffer_length);
    error = EINVAL;
    goto done;
  }
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_session_remove_domain_trie uuid copyin error (%d)",
            error);
    goto done;
  }

  lck_rw_lock_exclusive(&necp_kernel_policy_lock);
  bool removed = necp_remove_domain_trie(&necp_global_domain_trie_list,
                                         &session->domain_tries, id);
  if (!removed) {
    error = ENOENT;
  }
  lck_rw_done(&necp_kernel_policy_lock);
...
static int
necp_session_remove_all_domain_tries(struct necp_session *session,
                                     struct necp_session_action_args *uap,
                                     int *retval) {
#pragma unused(uap)
...
static int necp_session_trie_dump_all(struct necp_session *session,
                                      struct necp_session_action_args *uap,
                                      int *retval) {
#pragma unused(session)
  ...
  output_length =
      sizeof(count) +
      (count *
       sizeof(struct necp_domain_trie_request)); // first byte contains count
  if (output_length > uap->out_buffer_length) {
    NECPLOG(LOG_ERR,
            "necp_session_trie_dump_all out_buffer not large enough for %zu",
  ...
  if (output_length > sizeof(request_buffer)) {
    NECPLOG(
        LOG_ERR,
        "necp_session_trie_dump_all temporary buffer not large enough for %zu",
        (size_t)output_length);
    error = EINVAL;
    goto done;
  }
  ...
  LIST_FOREACH(trie, &necp_global_domain_trie_list, chain) {
    if (trie->trie_request != NULL) {
      memcpy((u_int8_t *)ptr, (u_int8_t *)trie->trie_request,
             sizeof(struct necp_domain_trie_request));
...
int necp_session_action(struct proc *p, struct necp_session_action_args *uap,
                        int *retval) {
  ...
  if (session->proc_locked) {
    ...
    proc_getexecutableuuid(current_proc(), session->proc_uuid,
                           sizeof(session->proc_uuid));
...
static void necp_generate_resolver_key(void) {
  s_necp_resolver_key_state.digest_info = ccsha256_di();
  cc_rand_generate(s_necp_resolver_key_state.key,
                   sizeof(s_necp_resolver_key_state.key));
}

static void necp_sign_update_context(const struct ccdigest_info *di,
                                     cchmac_ctx_t ctx, uuid_t client_id,
                                     u_int32_t sign_type, u_int8_t *data,
                                     size_t data_length) {
  ...
  cchmac_update(di, ctx, strlen(context_string),
                __unsafe_null_terminated_to_indexable(context_string));
...
int necp_sign_resolver_answer(uuid_t client_id, u_int32_t sign_type,
                              u_int8_t *data, size_t data_length, u_int8_t *tag,
                              size_t *out_tag_length) {
  if (s_necp_resolver_key_state.digest_info == NULL) {
    return EINVAL;
  }

  if (data == NULL || data_length == 0 || tag == NULL ||
      out_tag_length == NULL) {
    return EINVAL;
  }

  size_t required_tag_length =
      s_necp_resolver_key_state.digest_info->output_size;
  ...
  *out_tag_length = required_tag_length;
  ...
  necp_sign_update_context(s_necp_resolver_key_state.digest_info, ctx,
                           client_id, sign_type, data, data_length);
  cchmac_final(s_necp_resolver_key_state.digest_info, ctx, tag);

  return 0;
}

bool necp_validate_resolver_answer(uuid_t client_id, u_int32_t sign_type,
                                   u_int8_t *__sized_by(data_length) data,
                                   size_t data_length,
                                   u_int8_t *__sized_by(tag_length) tag,
                                   size_t tag_length) {
  if (s_necp_resolver_key_state.digest_info == NULL) {
    return false;
  }

  if (data == NULL || data_length == 0 || tag == NULL || tag_length == 0) {
    return false;
  }
  ...
  return cc_cmp_safe(s_necp_resolver_key_state.digest_info->output_size, tag,
                     actual_tag) == 0;
}
...
static void necp_generate_application_id_key(void) {
  s_necp_application_id_key_state.digest_info = ccsha256_di();
  cc_rand_generate(s_necp_application_id_key_state.key,
...
int necp_sign_application_id(uuid_t client_id, u_int32_t sign_type,
  ...
  if (tag == NULL || out_tag_length == NULL) {
    return EINVAL;
  }

  size_t required_tag_length =
      s_necp_application_id_key_state.digest_info->output_size;
  if (*out_tag_length < required_tag_length) {
    return ERANGE;
  }
  ...
  *out_tag_length = required_tag_length;
  ...
  necp_sign_application_id_update_context(
      s_necp_application_id_key_state.digest_info, ctx, client_id, sign_type);
  cchmac_final(s_necp_application_id_key_state.digest_info, ctx, tag);

  return 0;
}

bool necp_validate_application_id(uuid_t client_id, u_int32_t sign_type,
                                  u_int8_t *__sized_by(tag_length) tag,
                                  size_t tag_length) {
  if (s_necp_application_id_key_state.digest_info == NULL) {
    return false;
  ...
  return cc_cmp_safe(s_necp_application_id_key_state.digest_info->output_size,
                     tag, actual_tag) == 0;
}

void necp_init(void) {
  necp_log_handle = os_log_create("com.apple.xnu.net.necp", "necp");
  necp_data_trace_log_handle =
      os_log_create("com.apple.xnu.net.necp", "necp-data-trace");
  ...
  necp_uuid_app_id_hashtbl = __unsafe_forge_bidi_indexable(
  ...
  memset(&necp_kernel_socket_policies_map, 0,
         sizeof(necp_kernel_socket_policies_map));
  memset(&necp_kernel_ip_output_policies_map, 0,
         sizeof(necp_kernel_ip_output_policies_map));
...
static inline bool necp_buffer_write_tlv_validate(
    u_int8_t *__indexable cursor, u_int8_t type, u_int32_t length,
  ...
  u_int8_t *next_tlv =
      (u_int8_t *)(cursor + sizeof(type) + sizeof(length) + length);
  if (next_tlv <= buffer || // make sure the next TLV start doesn't overflow
      (uintptr_t)(next_tlv - buffer) >
          buffer_length) { // make sure the next TLV has enough room in buffer
    NECPLOG(LOG_ERR,
...
u_int8_t *__counted_by(0) necp_buffer_write_tlv_if_different(
    u_int8_t *__counted_by(0) cursor_, u_int8_t type, u_int32_t length,
    const void *__sized_by(length) value, bool *updated,
    u_int8_t *__sized_by(buffer_length) buffer, u_int32_t buffer_length) {
  // Use __counted_by(0) for cursor_ to avoid using __indexable in parameter
  // list that causes ABI issue
  u_int8_t *cursor = buffer + (cursor_ - buffer);
  if (!necp_buffer_write_tlv_validate(cursor, type, length, buffer,
                                      buffer_length)) {
    // If we can't fit this TLV, return the current cursor
    return cursor;
  }
  u_int8_t *__indexable next_tlv =
      (u_int8_t *)(cursor + sizeof(type) + sizeof(length) + length);
  ...
  if (length > 0) {
    if (*updated || memcmp((u_int8_t *)(cursor + sizeof(type) + sizeof(length)),
                           value, length) != 0) {
      memcpy((u_int8_t *)(cursor + sizeof(type) + sizeof(length)), value,
             length);
      *updated = TRUE;
    }
  }
...
static u_int8_t *__counted_by(0) necp_buffer_write_tlv_with_flags(
    u_int8_t *__counted_by(0) cursor_, u_int8_t type, u_int8_t flags,
    u_int32_t length, const void *__sized_by(length) value,
    u_int8_t *__sized_by(buffer_length) buffer, u_int32_t buffer_length) {
  // Use __counted_by(0) for cursor_ to avoid using __indexable in parameter
  // list that causes ABI issue Add one extra byte to 'length' to account for
  // the flags byte for validation.
  u_int8_t *cursor = buffer + (cursor_ - buffer);
  if (!necp_buffer_write_tlv_validate(cursor, type, length + 1, buffer,
                                      buffer_length)) {
    return NULL;
  }
  u_int8_t *next_tlv = (u_int8_t *)(cursor + sizeof(type) + sizeof(flags) +
                                    sizeof(length) + length);

  // TLV with flags format: type, length, flags, value (added 1 byte for the
  // leading flags)
  *(u_int8_t *)(cursor) = type;
  *(u_int32_t *)(void *)(cursor + sizeof(type)) = length;
  *(u_int8_t *)(cursor + sizeof(type) + sizeof(length)) = flags;
  if (length > 0) {
    memcpy((u_int8_t *)(cursor + sizeof(type) + sizeof(length) + sizeof(flags)),
           value, length);
  }
...
u_int8_t necp_buffer_get_tlv_type(u_int8_t *__counted_by(buffer_length) buffer,
                                  size_t buffer_length, u_int32_t tlv_offset) {
  u_int8_t *__indexable type = NULL;
  uint64_t end_offset = 0;

  if (buffer == NULL ||
      os_add_overflow(tlv_offset, sizeof(u_int8_t), &end_offset) ||
...
u_int32_t necp_buffer_get_tlv_length(u_int8_t *__counted_by(buffer_length)
                                         buffer,
  ...
  if (buffer == NULL ||
      os_add_overflow(tlv_offset, sizeof(u_int8_t) + sizeof(u_int32_t),
                      &end_offset) ||
      buffer_length < end_offset) {
    return 0;
  }
  ...
  length =
      (u_int32_t *)(void *)((u_int8_t *)buffer + tlv_offset + sizeof(u_int8_t));
...
u_int8_t *__sized_by(*value_size)
  ...
  u_int32_t length =
      necp_buffer_get_tlv_length(buffer, buffer_length, tlv_offset);
  if (length == 0) {
    return NULL;
  }

  if (os_add3_overflow(tlv_offset, length, sizeof(u_int8_t) + sizeof(u_int32_t),
                       &end_offset) ||
      buffer_length < end_offset) {
    return NULL;
  }
  ...
  value = (u_int8_t *)((u_int8_t *)buffer + tlv_offset + sizeof(u_int8_t) +
                       sizeof(u_int32_t));
  return value;
}

int necp_buffer_find_tlv(u_int8_t *__sized_by(buffer_length) buffer,
                         u_int32_t buffer_length, int offset, u_int8_t type,
                         int *err, int next) {
  if (err != NULL) {
    *err = ENOENT;
  }
  ...
  while (TRUE) {
    if ((((u_int32_t)cursor) + sizeof(curr_type) + sizeof(curr_length)) >
        buffer_length) {
      return -1;
    }
    if (!next) {
    ...
    if (curr_length > buffer_length - ((u_int32_t)cursor + sizeof(curr_type) +
                                       sizeof(curr_length))) {
      return -1;
    }
...
static int necp_find_tlv(u_int8_t *__sized_by(buffer_length) buffer,
                         u_int32_t buffer_length, int offset, u_int8_t type,
                         int *err, int next) {
  int cursor = -1;
  if (buffer != NULL) {
    cursor =
        necp_buffer_find_tlv(buffer, buffer_length, offset, type, err, next);
  }
  return cursor;
}

static int necp_get_tlv_at_offset(u_int8_t *__sized_by(buffer_length) buffer,
                                  u_int32_t buffer_length, int tlv_offset,
                                  u_int32_t out_buffer_length,
                                  void *__indexable out_buffer,
                                  u_int32_t *value_size) {
  if (buffer == NULL) {
  ...
  if (tlv_length >
      buffer_length - (tlv_offset + sizeof(u_int8_t) + sizeof(u_int32_t))) {
    NECPLOG(LOG_ERR,
            "necp_get_tlv_at_offset buffer_length is too small for TLV of "
            "length %u (%u < %lu)",
            tlv_length, buffer_length,
            tlv_offset + sizeof(u_int8_t) + sizeof(u_int32_t) + tlv_length);
    return EINVAL;
  }
  ...
  if (out_buffer != NULL && out_buffer_length > 0) {
    ...
    if (out_buffer_length < tlv_length) {
      NECPLOG(LOG_ERR,
              "necp_get_tlv_at_offset out_buffer_length is too small for TLV "
              "value (%u < %u)",
    ...
    // Get value pointer
    u_int8_t *__indexable tlv_value =
        necp_buffer_get_tlv_value(buffer, buffer_length, tlv_offset, NULL);
    if (tlv_value == NULL) {
...
static int necp_get_tlv(u_int8_t *__sized_by(buffer_length) buffer,
                        u_int32_t buffer_length, int offset, u_int8_t type,
                        u_int32_t buff_len, void *__indexable buff,
                        u_int32_t *value_size) {
  int error = 0;

  int tlv_offset =
      necp_find_tlv(buffer, buffer_length, offset, type, &error, 0);
  if (tlv_offset < 0) {
    return error;
  }

  return necp_get_tlv_at_offset(buffer, buffer_length, tlv_offset, buff_len,
                                buff, value_size);
}
...
static struct necp_session *necp_create_session(void) {
  ...
  new_session = kalloc_type(struct necp_session, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  lck_mtx_init(&new_session->lock, &necp_kernel_policy_mtx_grp,
               &necp_kernel_policy_mtx_attr);
  ...
  new_session->session_order = necp_allocate_new_session_order(
      new_session->session_priority, control_unit);
...
necp_policy_result_get_type_from_buffer(u_int8_t *__sized_by(length) buffer,
                                        u_int32_t length) {
  return (buffer && length >= sizeof(u_int8_t)) ? buffer[0] : 0;
}

static inline u_int32_t necp_policy_result_get_parameter_length_from_buffer(
    u_int8_t *__sized_by(length) buffer, u_int32_t length) {
  return (buffer && length > sizeof(u_int8_t)) ? (length - sizeof(u_int8_t))
                                               : 0;
}

static inline u_int8_t *__indexable
necp_policy_result_get_parameter_pointer_from_buffer(
    u_int8_t *__indexable buffer, u_int32_t length) {
  return (buffer && length > sizeof(u_int8_t)) ? (buffer + sizeof(u_int8_t))
                                               : NULL;
}

static bool necp_policy_result_requires_route_rules(u_int8_t *__sized_by(length)
                                                        buffer,
                                                    u_int32_t length) {
...
static inline bool _necp_address_is_valid(struct sockaddr *address) {
...
static bool necp_policy_result_is_valid(u_int8_t *__sized_by(length) buffer,
                                        u_int32_t length, bool *is_pass_skip) {
  bool validated = FALSE;
  u_int8_t type = necp_policy_result_get_type_from_buffer(buffer, length);
  u_int32_t parameter_length =
      necp_policy_result_get_parameter_length_from_buffer(buffer, length);
  *is_pass_skip = FALSE;
  switch (type) {
...
static inline u_int8_t
necp_policy_condition_get_type_from_buffer(u_int8_t *__sized_by(length) buffer,
                                           u_int32_t length) {
  return (buffer && length >= sizeof(u_int8_t)) ? buffer[0] : 0;
}

static inline u_int8_t
necp_policy_condition_get_flags_from_buffer(u_int8_t *__sized_by(length) buffer,
                                            u_int32_t length) {
  return (buffer && length >= (2 * sizeof(u_int8_t))) ? buffer[1] : 0;
}

static inline u_int32_t necp_policy_condition_get_value_length_from_buffer(
    u_int8_t *__sized_by(length) buffer, u_int32_t length) {
  return (buffer && length >= (2 * sizeof(u_int8_t)))
             ? (length - (2 * sizeof(u_int8_t)))
             : 0;
}

static inline u_int8_t *__indexable
necp_policy_condition_get_value_pointer_from_buffer(
    u_int8_t *__indexable buffer, u_int32_t length) {
  return (buffer && length > (2 * sizeof(u_int8_t)))
             ? (buffer + (2 * sizeof(u_int8_t)))
             : NULL;
}

static inline bool necp_policy_condition_is_default(u_int8_t *__sized_by(length)
                                                        buffer,
                                                    u_int32_t length) {
  return necp_policy_condition_get_type_from_buffer(buffer, length) ==
         NECP_POLICY_CONDITION_DEFAULT;
}

static inline bool
necp_policy_condition_is_application(u_int8_t *__sized_by(length) buffer,
                                     u_int32_t length) {
  return necp_policy_condition_get_type_from_buffer(buffer, length) ==
...
static inline bool
necp_policy_condition_is_real_application(u_int8_t *__sized_by(length) buffer,
                                          u_int32_t length) {
  return necp_policy_condition_get_type_from_buffer(buffer, length) ==
...
static inline bool
necp_policy_condition_requires_application(u_int8_t *__sized_by(length) buffer,
                                           u_int32_t length) {
...
static inline bool
necp_policy_condition_is_kernel_pid(u_int8_t *__sized_by(length) buffer,
                                    u_int32_t length) {
  u_int8_t type = necp_policy_condition_get_type_from_buffer(buffer, length);
  ...
  if (type == NECP_POLICY_CONDITION_PID) {
    condition_length =
        necp_policy_condition_get_value_length_from_buffer(buffer, length);
    if (condition_length >= sizeof(pid_t)) {
      condition_value =
          (pid_t *)(void *)necp_policy_condition_get_value_pointer_from_buffer(
              buffer, length);
...
static bool necp_policy_condition_is_valid(u_int8_t *__sized_by(length) buffer,
                                           u_int32_t length,
                                           u_int8_t policy_result_type) {
  bool validated = FALSE;
  bool result_cannot_have_ip_layer =
      (policy_result_type == NECP_POLICY_RESULT_SOCKET_DIVERT ||
       ...
       policy_result_type == NECP_POLICY_RESULT_REMOVE_NETAGENT_TYPE)
          ? TRUE
          : FALSE;
  u_int32_t condition_length =
      necp_policy_condition_get_value_length_from_buffer(buffer, length);
  u_int8_t *condition_value =
      necp_policy_condition_get_value_pointer_from_buffer(buffer, length);
  u_int8_t type = necp_policy_condition_get_type_from_buffer(buffer, length);
  ...
  case NECP_POLICY_CONDITION_REAL_APPLICATION: {
    if (!(flags & NECP_POLICY_CONDITION_FLAGS_NEGATIVE) &&
        condition_length >= sizeof(uuid_t) && condition_value != NULL &&
  ...
  case NECP_POLICY_CONDITION_PID: {
    if (condition_length >= sizeof(pid_t) && condition_value != NULL) {
      validated = TRUE;
    }
    break;
  }
  ...
  case NECP_POLICY_CONDITION_REMOTE_ADDR: {
    if (!result_cannot_have_ip_layer &&
        condition_length >= sizeof(struct necp_policy_condition_addr) &&
        necp_address_is_valid(
            &((struct necp_policy_condition_addr *)(void *)condition_value)
                 ->address.sa)) {
      validated = TRUE;
    }
    break;
  ...
  case NECP_POLICY_CONDITION_REMOTE_ADDR_RANGE: {
    if (!result_cannot_have_ip_layer &&
        condition_length >= sizeof(struct necp_policy_condition_addr_range) &&
        necp_address_is_valid(
            &((struct necp_policy_condition_addr_range *)(void *)
                  condition_value)
                 ->start_address.sa) &&
        necp_address_is_valid(
            &((struct necp_policy_condition_addr_range *)(void *)
  ...
  case NECP_POLICY_CONDITION_PACKET_FILTER_TAGS: {
    if (condition_length >= sizeof(u_int16_t)) {
      ...
      if (packet_filter_tags > 0 &&
          packet_filter_tags <= NECP_POLICY_CONDITION_PACKET_FILTER_TAG_MAX) {
        validated = TRUE;
      }
    }
    break;
  ...
  case NECP_POLICY_CONDITION_BOUND_INTERFACE_FLAGS: {
    if (condition_length >=
        sizeof(u_int32_t) *
            NECP_POLICY_CONDITION_BOUND_INTERFACE_FLAGS_IDX_MAX) {
      validated = TRUE;
    }
    break;
  }
...
static bool necp_policy_route_rule_is_default(u_int8_t *__sized_by(length)
                                                  buffer,
                                              u_int32_t length) {
  return necp_policy_condition_get_value_length_from_buffer(buffer, length) ==
             0 &&
         necp_policy_condition_get_flags_from_buffer(buffer, length) == 0;
}

static bool necp_policy_route_rule_is_valid(u_int8_t *__sized_by(length) buffer,
                                            u_int32_t length) {
  bool validated = FALSE;
  u_int8_t type = necp_policy_condition_get_type_from_buffer(buffer, length);
  switch (type) {
...
static int necp_get_posix_error_for_necp_error(int response_error) {
...
necp_handle_policy_add(struct necp_session *session,
                       u_int8_t *__sized_by(tlv_buffer_length) tlv_buffer,
                       size_t tlv_buffer_length, int offset,
                       int *return_error) {
  ...
  // Read policy order
  error = necp_get_tlv(tlv_buffer, tlv_buffer_length, offset,
                       NECP_TLV_POLICY_ORDER, sizeof(order), &order, NULL);
  if (error) {
  ...
  // Read policy result
  cursor = necp_find_tlv(tlv_buffer, tlv_buffer_length, offset,
                         NECP_TLV_POLICY_RESULT, &error, 0);
  if (error || cursor < 0) {
  ...
  error = necp_get_tlv_at_offset(tlv_buffer, tlv_buffer_length, cursor, 0, NULL,
                                 &policy_result_size);
  ...
  if (policy_result == NULL) {
    NECPLOG(LOG_ERR, "Failed to allocate a policy result buffer (size %d)",
            policy_result_size);
    response_error = NECP_ERROR_INTERNAL;
    goto fail;
  }
  error = necp_get_tlv_at_offset(tlv_buffer, tlv_buffer_length, cursor,
                                 policy_result_size, policy_result, NULL);
  if (error) {
  ...
  if (necp_policy_result_requires_route_rules(policy_result,
                                              policy_result_size)) {
    // Read route rules conditions

    for (cursor = necp_find_tlv(tlv_buffer, tlv_buffer_length, offset,
                                NECP_TLV_ROUTE_RULE, &error, 0);
         cursor >= 0;
         cursor = necp_find_tlv(tlv_buffer, tlv_buffer_length, cursor,
                                NECP_TLV_ROUTE_RULE, &error, 1)) {
      u_int32_t route_rule_size = 0;
      necp_get_tlv_at_offset(tlv_buffer, tlv_buffer_length, cursor, 0, NULL,
                             &route_rule_size);
      if (os_add_overflow(
    ...
    if (route_rules_array_size > NECP_MAX_ROUTE_RULES_ARRAY_SIZE) {
      NECPLOG(LOG_ERR, "Route rules length too large: %u",
              route_rules_array_size);
      response_error = NECP_ERROR_INVALID_TLV;
      goto fail;
    }
    route_rules_array =
        (u_int8_t *)kalloc_data(route_rules_array_size, Z_WAITOK);
    if (route_rules_array == NULL) {
      NECPLOG(LOG_ERR,
              "Failed to allocate a policy route rules array (size %d)",
              route_rules_array_size);
      response_error = NECP_ERROR_INTERNAL;
      goto fail;
    }
    ...
    for (cursor = necp_find_tlv(tlv_buffer, tlv_buffer_length, offset,
      ...
      if (route_rule_size > 0 &&
          (sizeof(route_rule_type) + sizeof(route_rule_size) +
           route_rule_size) <=
              (route_rules_array_size - route_rules_array_cursor)) {
        // Add type
        memcpy((route_rules_array + route_rules_array_cursor), &route_rule_type,
               sizeof(route_rule_type));
        route_rules_array_cursor += sizeof(route_rule_type);

        // Add length
        memcpy((route_rules_array + route_rules_array_cursor), &route_rule_size,
               sizeof(route_rule_size));
        route_rules_array_cursor += sizeof(route_rule_size);

        // Add value
        necp_get_tlv_at_offset(
            tlv_buffer, tlv_buffer_length, cursor, route_rule_size,
            (route_rules_array + route_rules_array_cursor), NULL);

        if (!necp_policy_route_rule_is_valid(
                (route_rules_array + route_rules_array_cursor),
                route_rule_size)) {
        ...
        if (necp_policy_route_rule_is_default(
                (route_rules_array + route_rules_array_cursor),
                route_rule_size)) {
          if (has_default_route_rule) {
            NECPLOG0(LOG_ERR, "Failed to validate route rule; contained "
                              "multiple default route rules");
            response_error = NECP_ERROR_ROUTE_RULES_INVALID;
            goto fail;
          }
  ...
  for (cursor = necp_find_tlv(tlv_buffer, tlv_buffer_length, offset,
                              NECP_TLV_POLICY_CONDITION, &error, 0);
       cursor >= 0;
       cursor = necp_find_tlv(tlv_buffer, tlv_buffer_length, cursor,
                              NECP_TLV_POLICY_CONDITION, &error, 1)) {
  ...
  if (conditions_array == NULL) {
    NECPLOG(LOG_ERR, "Failed to allocate a policy conditions array (size %d)",
            conditions_array_size);
    response_error = NECP_ERROR_INTERNAL;
    goto fail;
  }
  ...
  for (cursor = necp_find_tlv(tlv_buffer, tlv_buffer_length, offset,
    ...
    necp_get_tlv_at_offset(tlv_buffer, tlv_buffer_length, cursor, 0, NULL,
                           &condition_size);
    if (condition_size > 0 &&
        (sizeof(condition_type) + sizeof(condition_size) + condition_size) <=
            (conditions_array_size - conditions_array_cursor)) {
      // Add type
      memcpy((conditions_array + conditions_array_cursor), &condition_type,
             sizeof(condition_type));
      conditions_array_cursor += sizeof(condition_type);

      // Add length
      memcpy((conditions_array + conditions_array_cursor), &condition_size,
             sizeof(condition_size));
      conditions_array_cursor += sizeof(condition_size);

      // Add value
      necp_get_tlv_at_offset(
          tlv_buffer, tlv_buffer_length, cursor, condition_size,
          (conditions_array + conditions_array_cursor), NULL);
      if (!necp_policy_condition_is_valid(
              (conditions_array + conditions_array_cursor), condition_size,
      ...
      if (necp_policy_condition_is_default(
              (conditions_array + conditions_array_cursor), condition_size)) {
      ...
      if (has_default_condition && has_non_default_condition) {
        NECPLOG0(LOG_ERR, "Failed to validate conditions; contained default "
                          "and non-default conditions");
        response_error = NECP_ERROR_POLICY_CONDITIONS_INVALID;
        goto fail;
      }

      if (necp_policy_condition_is_application(
              (conditions_array + conditions_array_cursor), condition_size)) {
        has_application_condition = TRUE;
      }

      if (necp_policy_condition_is_real_application(
              (conditions_array + conditions_array_cursor), condition_size)) {
        has_real_application_condition = TRUE;
      }

      if (necp_policy_condition_requires_application(
              (conditions_array + conditions_array_cursor), condition_size)) {
        requires_application_condition = TRUE;
      }

      if (necp_policy_condition_is_kernel_pid(
              (conditions_array + conditions_array_cursor), condition_size)) {
        has_kernel_pid = TRUE;
      }
  ...
  if (has_kernel_pid && !is_pass_skip) {
    NECPLOG0(LOG_ERR, "Failed to validate conditions; kernel pid (0) condition "
                      "allows only Pass/Skip result");
    response_error = NECP_ERROR_POLICY_CONDITIONS_INVALID;
    goto fail;
  }

  if ((policy = necp_policy_create(session, order, conditions_array,
                                   conditions_array_size, route_rules_array,
                                   route_rules_array_size, policy_result,
                                   policy_result_size)) == NULL) {
    response_error = NECP_ERROR_INTERNAL;
    goto fail;
  }
...
static necp_policy_id necp_policy_get_new_id(struct necp_session *session) {
 ...
 *					{
 *						type	:
 *	NECP_POLICY_CONDITION_ALL_INTERFACES len		:	...
 *						value	:	...
 *					}
 *					{
 *						type	:
 *	NECP_POLICY_CONDITION_BOUND_INTERFACES len		:	...
 *						value	:	...
 *					}
 *					...
 *				}
...
#define UNLOCK_AND_REPORT_ERROR(lock, error)                                   \
  lck_rw_done(lock);                                                           \
  REPORT_ERROR(error)
  ...
  errno_t cred_result =
      priv_check_cred(kauth_cred_get(), PRIV_NET_PRIVILEGED_NECP_POLICIES, 0);
  if (cred_result != 0) {
    NECPLOG0(LOG_ERR, "Session does not hold the necessary entitlement to get "
                      "Network Extension Policy information");
  ...
  tlv_buffer_pointers =
      kalloc_type(u_int8_t * __indexable, policy_count, M_WAITOK | Z_ZERO);
  if (tlv_buffer_pointers == NULL) {
    NECPLOG(LOG_DEBUG, "Failed to allocate tlv_buffer_pointers (%lu bytes)",
            sizeof(u_int8_t *) * policy_count);
    UNLOCK_AND_REPORT_ERROR(&necp_kernel_policy_lock, NECP_ERROR_INTERNAL);
  }

  tlv_buffer_lengths = (u_int32_t *)kalloc_data(
      sizeof(u_int32_t) * policy_count, Z_NOWAIT | Z_ZERO);
  if (tlv_buffer_lengths == NULL) {
    NECPLOG(LOG_DEBUG, "Failed to allocate tlv_buffer_lengths (%lu bytes)",
            sizeof(u_int32_t) * policy_count);
    UNLOCK_AND_REPORT_ERROR(&necp_kernel_policy_lock, NECP_ERROR_INTERNAL);
  }

  for (policy_i = 0;
       necp_kernel_socket_policies_app_layer_map != NULL &&
       necp_kernel_socket_policies_app_layer_map[policy_i] != NULL;
       policy_i++) {
    ...
    necp_get_result_description(result_string, policy->result,
                                policy->result_parameter);
    proc_name(policy->session_pid, proc_name_string, MAXCOMLEN);

    u_int16_t proc_name_len =
        strbuflen(proc_name_string, sizeof(proc_name_string) - 1) + 1;
    u_int16_t result_string_len =
        strbuflen(result_string, sizeof(result_string) - 1) + 1;

    if (necp_debug) {
      NECPLOG(LOG_DEBUG, "Policy: process: %s, result: %s", proc_name_string,
              result_string);
    }

    u_int32_t total_allocated_bytes =
        sizeof(u_int8_t) + sizeof(u_int32_t) +
        sizeof(policy->id) + // NECP_TLV_POLICY_ID
        sizeof(u_int8_t) + sizeof(u_int32_t) +
        sizeof(policy->order) + // NECP_TLV_POLICY_ORDER
        sizeof(u_int8_t) + sizeof(u_int32_t) +
        sizeof(policy->session_order) + // NECP_TLV_POLICY_SESSION_ORDER
        sizeof(u_int8_t) + sizeof(u_int32_t) +
        result_string_len + // NECP_TLV_POLICY_RESULT_STRING
        sizeof(u_int8_t) + sizeof(u_int32_t) +
        proc_name_len +                       // NECP_TLV_POLICY_OWNER
        sizeof(u_int8_t) + sizeof(u_int32_t); // NECP_TLV_POLICY_CONDITION

    // We now traverse the condition_mask to see how much space we need to
    // allocate
    ...
    if (condition_mask == NECP_POLICY_CONDITION_DEFAULT) {
      ...
      if (condition_mask & NECP_KERNEL_CONDITION_BOUND_INTERFACE) {
        snprintf(if_name, IFXNAMSIZ, "%s%d",
                 ifnet_name(policy->cond_bound_interface),
                 ifnet_unit(policy->cond_bound_interface));
      ...
      if (condition_mask & NECP_KERNEL_CONDITION_ACCOUNT_ID) {
        account_id_entry = necp_lookup_string_with_id_locked(
            &necp_account_id_list, policy->cond_account_id);
        u_int32_t account_id_len = 0;
        if (account_id_entry) {
          account_id_len = account_id_entry->string
                               ? strlen(account_id_entry->string) + 1
                               : 0;
        }
    ...
    // These are for the condition TLVs (id, length, flags).  The space for
    // "value" is already accounted for above.
    condition_tlv_length +=
        num_conditions *
        (sizeof(u_int8_t) + sizeof(u_int32_t) + sizeof(u_int8_t));
    total_allocated_bytes += condition_tlv_length;

    u_int8_t *__indexable tlv_buffer;
    tlv_buffer =
        (u_int8_t *)kalloc_data(total_allocated_bytes, Z_NOWAIT | Z_ZERO);
    if (tlv_buffer == NULL) {
      NECPLOG(LOG_DEBUG, "Failed to allocate tlv_buffer (%u bytes)",
    ...
    cursor =
        necp_buffer_write_tlv(cursor, NECP_TLV_POLICY_ID, sizeof(policy->id),
                              &policy->id, tlv_buffer, total_allocated_bytes);
    cursor = necp_buffer_write_tlv(cursor, NECP_TLV_POLICY_ORDER,
                                   sizeof(necp_policy_order), &policy->order,
                                   tlv_buffer, total_allocated_bytes);
    cursor = necp_buffer_write_tlv(
        cursor, NECP_TLV_POLICY_SESSION_ORDER, sizeof(policy->session_order),
        &policy->session_order, tlv_buffer, total_allocated_bytes);
    cursor = necp_buffer_write_tlv(cursor, NECP_TLV_POLICY_RESULT_STRING,
                                   result_string_len, result_string, tlv_buffer,
                                   total_allocated_bytes);
    cursor = necp_buffer_write_tlv(cursor, NECP_TLV_POLICY_OWNER, proc_name_len,
                                   proc_name_string, tlv_buffer,
                                   total_allocated_bytes);
...
#define N_QUICK 256
    ...
    if (condition_tlv_length <= N_QUICK) {
      ...
      if (cond_buf == NULL) {
        NECPLOG(LOG_DEBUG, "Failed to allocate cond_buffer (%u bytes)",
                condition_tlv_length);
    ...
    if (condition_mask == NECP_POLICY_CONDITION_DEFAULT) {
      cond_buf_cursor = necp_buffer_write_tlv_with_flags(
          cond_buf_cursor, NECP_POLICY_CONDITION_DEFAULT, cond_flags, 0, "",
          cond_buf, condition_tlv_length);
    } else {
      if (condition_mask & NECP_KERNEL_CONDITION_ALL_INTERFACES) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_ALL_INTERFACES)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_ALL_INTERFACES, cond_flags,
            0, "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_HAS_CLIENT) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_HAS_CLIENT)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_HAS_CLIENT, cond_flags, 0,
            "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_LOCAL_NETWORKS) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_LOCAL_NETWORKS)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_LOCAL_NETWORKS, cond_flags,
            sizeof(policy->cond_local_networks_flags),
            &policy->cond_local_networks_flags, cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_BOUND_INTERFACE) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_BOUND_INTERFACE)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_BOUND_INTERFACE, cond_flags,
            strbuflen(if_name, sizeof(if_name) - 1) + 1, if_name, cond_buf,
            condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_PROTOCOL) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_PROTOCOL)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_IP_PROTOCOL, cond_flags,
            sizeof(policy->cond_protocol), &policy->cond_protocol, cond_buf,
            condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_APP_ID) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_APP_ID)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        struct necp_uuid_id_mapping *entry =
            necp_uuid_lookup_uuid_with_app_id_locked(policy->cond_app_id);
        if (entry != NULL) {
          cond_buf_cursor = necp_buffer_write_tlv_with_flags(
              cond_buf_cursor, NECP_POLICY_CONDITION_APPLICATION, cond_flags,
              sizeof(entry->uuid), entry->uuid, cond_buf, condition_tlv_length);
        }
      }
      if (condition_mask & NECP_KERNEL_CONDITION_REAL_APP_ID) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_REAL_APP_ID)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        struct necp_uuid_id_mapping *entry =
            necp_uuid_lookup_uuid_with_app_id_locked(policy->cond_real_app_id);
        if (entry != NULL) {
          cond_buf_cursor = necp_buffer_write_tlv_with_flags(
              cond_buf_cursor, NECP_POLICY_CONDITION_REAL_APPLICATION,
              cond_flags, sizeof(entry->uuid), entry->uuid, cond_buf,
              condition_tlv_length);
        }
      }
      if ((condition_mask & NECP_KERNEL_CONDITION_DOMAIN) ||
          (condition_mask & NECP_KERNEL_CONDITION_EXACT_DOMAIN)) {
        cond_flags =
            ((condition_negated_mask & NECP_KERNEL_CONDITION_DOMAIN) ||
             (condition_negated_mask & NECP_KERNEL_CONDITION_EXACT_DOMAIN))
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_DOMAIN, cond_flags,
            strlen(policy->cond_domain) + 1,
            __unsafe_null_terminated_to_indexable(policy->cond_domain),
            cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_DOMAIN_FILTER) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_DOMAIN_FILTER)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_DOMAIN_FILTER, cond_flags,
            sizeof(policy->cond_domain_filter), &policy->cond_domain_filter,
            cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_URL) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_URL)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_URL, cond_flags,
            strlen(policy->cond_url) + 1,
            __unsafe_null_terminated_to_indexable(policy->cond_url), cond_buf,
            condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_ACCOUNT_ID) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_ACCOUNT_ID)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        if (account_id_entry != NULL) {
          cond_buf_cursor = necp_buffer_write_tlv_with_flags(
              cond_buf_cursor, NECP_POLICY_CONDITION_ACCOUNT, cond_flags,
              strlen(account_id_entry->string) + 1,
              __unsafe_null_terminated_to_indexable(account_id_entry->string),
              cond_buf, condition_tlv_length);
        }
      }
      if (condition_mask & NECP_KERNEL_CONDITION_PID) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_PID)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        uint8_t pid_buffer[sizeof(policy->cond_pid) +
                           sizeof(policy->cond_pid_version)] = {};
        memcpy(pid_buffer, &policy->cond_pid, sizeof(policy->cond_pid));
        memcpy(pid_buffer + sizeof(policy->cond_pid), &policy->cond_pid_version,
               sizeof(policy->cond_pid_version));
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_PID, cond_flags,
            sizeof(pid_buffer), &pid_buffer, cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_UID) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_UID)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_UID, cond_flags,
            sizeof(policy->cond_uid), &policy->cond_uid, cond_buf,
            condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_REAL_UID) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_REAL_UID)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_REAL_UID, cond_flags,
            sizeof(policy->cond_real_uid), &policy->cond_real_uid, cond_buf,
            condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_TRAFFIC_CLASS) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_TRAFFIC_CLASS)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_TRAFFIC_CLASS, cond_flags,
            sizeof(policy->cond_traffic_class), &policy->cond_traffic_class,
            cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_ENTITLEMENT) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_ENTITLEMENT)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_ENTITLEMENT, cond_flags, 0,
            "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_CUSTOM_ENTITLEMENT) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_CUSTOM_ENTITLEMENT)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_ENTITLEMENT, cond_flags,
            strlen(policy->cond_custom_entitlement) + 1,
            __unsafe_null_terminated_to_indexable(
                policy->cond_custom_entitlement),
            cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_PLATFORM_BINARY) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_PLATFORM_BINARY)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_PLATFORM_BINARY, cond_flags,
            0, "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_SYSTEM_SIGNED_RESULT) {
        cond_flags = (condition_negated_mask &
                      NECP_KERNEL_CONDITION_SYSTEM_SIGNED_RESULT)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_SYSTEM_SIGNED_RESULT,
            cond_flags, 0, "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_SDK_VERSION) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_SDK_VERSION)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_SDK_VERSION, cond_flags,
            sizeof(policy->cond_sdk_version), &policy->cond_sdk_version,
            cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_LOCAL_START) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_LOCAL_START)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        if (condition_mask & NECP_KERNEL_CONDITION_LOCAL_END) {
          struct necp_policy_condition_addr_range range;
          memcpy(&range.start_address, &policy->cond_local_start,
                 sizeof(policy->cond_local_start));
          memcpy(&range.end_address, &policy->cond_local_end,
                 sizeof(policy->cond_local_end));
          cond_buf_cursor = necp_buffer_write_tlv_with_flags(
              cond_buf_cursor, NECP_POLICY_CONDITION_LOCAL_ADDR_RANGE,
              cond_flags, sizeof(range), &range, cond_buf,
              condition_tlv_length);
        } else {
          struct necp_policy_condition_addr addr;
          addr.prefix = policy->cond_local_prefix;
          memcpy(&addr.address, &policy->cond_local_start,
                 sizeof(policy->cond_local_start));
          cond_buf_cursor = necp_buffer_write_tlv_with_flags(
              cond_buf_cursor, NECP_POLICY_CONDITION_LOCAL_ADDR, cond_flags,
              sizeof(addr), &addr, cond_buf, condition_tlv_length);
        }
      }
      if (condition_mask & NECP_KERNEL_CONDITION_REMOTE_START) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_REMOTE_START)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        if (condition_mask & NECP_KERNEL_CONDITION_REMOTE_END) {
          struct necp_policy_condition_addr_range range;
          memcpy(&range.start_address, &policy->cond_remote_start,
                 sizeof(policy->cond_remote_start));
          memcpy(&range.end_address, &policy->cond_remote_end,
                 sizeof(policy->cond_remote_end));
          cond_buf_cursor = necp_buffer_write_tlv_with_flags(
              cond_buf_cursor, NECP_POLICY_CONDITION_REMOTE_ADDR_RANGE,
              cond_flags, sizeof(range), &range, cond_buf,
              condition_tlv_length);
        } else {
          struct necp_policy_condition_addr addr;
          addr.prefix = policy->cond_remote_prefix;
          memcpy(&addr.address, &policy->cond_remote_start,
                 sizeof(policy->cond_remote_start));
          cond_buf_cursor = necp_buffer_write_tlv_with_flags(
              cond_buf_cursor, NECP_POLICY_CONDITION_REMOTE_ADDR, cond_flags,
              sizeof(addr), &addr, cond_buf, condition_tlv_length);
        }
      }
      if (condition_mask & NECP_KERNEL_CONDITION_AGENT_TYPE) {
        cond_flags = (condition_negated_mask & NECP_KERNEL_CONDITION_AGENT_TYPE)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_AGENT_TYPE, cond_flags,
            sizeof(policy->cond_agent_type), &policy->cond_agent_type, cond_buf,
            condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_CLIENT_FLAGS) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_CLIENT_FLAGS)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_CLIENT_FLAGS, cond_flags,
            sizeof(policy->cond_client_flags), &policy->cond_client_flags,
            cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_LOCAL_EMPTY) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_LOCAL_EMPTY)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_FLOW_LOCAL_ADDR_EMPTY,
            cond_flags, 0, "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_REMOTE_EMPTY) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_REMOTE_EMPTY)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_FLOW_REMOTE_ADDR_EMPTY,
            cond_flags, 0, "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_SIGNING_IDENTIFIER) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_SIGNING_IDENTIFIER)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_SIGNING_IDENTIFIER,
            cond_flags, strlen(policy->cond_signing_identifier) + 1,
            __unsafe_null_terminated_to_indexable(
                policy->cond_signing_identifier),
            cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_PACKET_FILTER_TAGS) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_PACKET_FILTER_TAGS)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_PACKET_FILTER_TAGS,
            cond_flags, sizeof(policy->cond_packet_filter_tags),
            &policy->cond_packet_filter_tags, cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_IS_LOOPBACK) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_IS_LOOPBACK)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_FLOW_IS_LOOPBACK, cond_flags,
            0, "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_DELEGATE_IS_PLATFORM_BINARY) {
        cond_flags = (condition_negated_mask &
                      NECP_KERNEL_CONDITION_DELEGATE_IS_PLATFORM_BINARY)
                         ...
                         : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_DELEGATE_IS_PLATFORM_BINARY,
            cond_flags, 0, "", cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_SCHEME_PORT) {
        cond_flags =
            (condition_negated_mask & NECP_KERNEL_CONDITION_SCHEME_PORT)
                ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                : 0;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_SCHEME_PORT, cond_flags,
            sizeof(policy->cond_scheme_port), &policy->cond_scheme_port,
            cond_buf, condition_tlv_length);
      }
      if (condition_mask & NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS) {
        cond_flags = (condition_negated_mask &
                      NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS)
                         ? NECP_POLICY_CONDITION_FLAGS_NEGATIVE
                         : 0;
        ...
        flags[NECP_POLICY_CONDITION_BOUND_INTERFACE_FLAGS_IDX_EFLAGS] =
            policy->cond_bound_interface_eflags;
        flags[NECP_POLICY_CONDITION_BOUND_INTERFACE_FLAGS_IDX_XFLAGS] =
            policy->cond_bound_interface_xflags;
        cond_buf_cursor = necp_buffer_write_tlv_with_flags(
            cond_buf_cursor, NECP_POLICY_CONDITION_BOUND_INTERFACE_FLAGS,
            cond_flags, sizeof(flags), &flags, cond_buf, condition_tlv_length);
      }
    }

    cursor = necp_buffer_write_tlv(cursor, NECP_TLV_POLICY_CONDITION,
                                   cond_buf_cursor - cond_buf, cond_buf,
                                   tlv_buffer, total_allocated_bytes);
    ...
    total_tlv_len +=
        sizeof(u_int8_t) + sizeof(u_int32_t) + (cursor - tlv_buffer);
  }
  ...
  if (out_buffer != 0) {
    if (out_buffer_length < total_tlv_len + sizeof(u_int32_t)) {
      NECPLOG(LOG_DEBUG, "out_buffer_length too small (%lu < %lu)",
              out_buffer_length, total_tlv_len + sizeof(u_int32_t));
      REPORT_ERROR(NECP_ERROR_INVALID_TLV);
    }

    // Allow malloc to wait, since the total buffer may be large and we are not
    // holding any locks
    result_buf = (u_int8_t *)kalloc_data(total_tlv_len + sizeof(u_int32_t),
                                         Z_WAITOK | Z_ZERO);
    if (result_buf == NULL) {
      NECPLOG(LOG_DEBUG, "Failed to allocate result_buffer (%lu bytes)",
              total_tlv_len + sizeof(u_int32_t));
      REPORT_ERROR(NECP_ERROR_INTERNAL);
    }
    ...
    for (int i = 0; i < policy_count; i++) {
      if (tlv_buffer_pointers[i] != NULL) {
        result_buf_cursor = necp_buffer_write_tlv(
            result_buf_cursor, NECP_TLV_POLICY_DUMP, tlv_buffer_lengths[i],
            tlv_buffer_pointers[i], result_buf,
            total_tlv_len + sizeof(u_int32_t));
      }
    }

    int copy_error =
        copyout(result_buf, out_buffer, total_tlv_len + sizeof(u_int32_t));
    if (copy_error) {
      NECPLOG(LOG_DEBUG, "Failed to copy out result_buffer (%lu bytes)",
              total_tlv_len + sizeof(u_int32_t));
      REPORT_ERROR(NECP_ERROR_INTERNAL);
    }
  }
...
static struct necp_session_policy *necp_policy_create(
  ...
  if (session == NULL || conditions_array == NULL || result == NULL ||
      result_size == 0) {
    goto done;
  }
  ...
  LIST_INSERT_SORTED_ASCENDING(&session->policies, new_policy, chain, order,
                               tmp_policy);
...
necp_policy_get_result_type(struct necp_session_policy *policy) {
  return policy ? necp_policy_result_get_type_from_buffer(policy->result,
                                                          policy->result_size)
                : 0;
}
...
necp_policy_get_result_parameter_length(struct necp_session_policy *policy) {
  return policy ? necp_policy_result_get_parameter_length_from_buffer(
                      policy->result, policy->result_size)
                : 0;
}

static bool
necp_policy_get_result_parameter(struct necp_session_policy *policy,
                                 u_int8_t *__sized_by(parameter_buffer_length)
                                     parameter_buffer,
                                 u_int32_t parameter_buffer_length) {
  if (policy) {
    u_int32_t parameter_length =
        necp_policy_result_get_parameter_length_from_buffer(
            policy->result, policy->result_size);
...
static bool necp_policy_unapply(struct necp_session_policy *policy) {
  int i = 0;
  if (policy == NULL) {
    return FALSE;
  }
  ...
  if (!uuid_is_null(policy->applied_app_uuid)) {
    ...
    if (necp_remove_uuid_app_id_mapping(policy->applied_app_uuid,
  ...
  if (policy->applied_account != NULL) {
    necp_remove_string_to_id_mapping(
        &necp_account_id_list,
        __unsafe_null_terminated_from_indexable(policy->applied_account));
...
static bool necp_policy_apply(struct necp_session *session,
                              struct necp_session_policy *policy,
  ...
  while (offset < policy->conditions_size) {
    u_int32_t length = 0;
    u_int8_t *__indexable value = necp_buffer_get_tlv_value(
        policy->conditions, policy->conditions_size, offset, &length);

    u_int8_t condition_type =
        necp_policy_condition_get_type_from_buffer(value, length);
    u_int8_t condition_flags =
        necp_policy_condition_get_flags_from_buffer(value, length);
    bool condition_is_negative =
        condition_flags & NECP_POLICY_CONDITION_FLAGS_NEGATIVE;
    u_int32_t condition_length =
        necp_policy_condition_get_value_length_from_buffer(value, length);
    u_int8_t *condition_value =
        necp_policy_condition_get_value_pointer_from_buffer(value, length);
      ...
      if (condition_length > 0) {
        if (cond_custom_entitlement == NULL) {
          cond_custom_entitlement =
              necp_copy_string((char *)condition_value, condition_length);
          if (cond_custom_entitlement != NULL) {
    ...
    case NECP_POLICY_CONDITION_DOMAIN: {
      ...
      if (condition_length > 0 && cond_domain == NULL) {
        const bool condition_is_exact =
            condition_flags & NECP_POLICY_CONDITION_FLAGS_EXACT;

        u_int64_t mask_value = condition_is_exact
                                   ? NECP_KERNEL_CONDITION_EXACT_DOMAIN
                                   : NECP_KERNEL_CONDITION_DOMAIN;
        cond_domain = necp_create_trimmed_domain((char *)condition_value,
                                                 condition_length);
        if (cond_domain != NULL) {
    ...
    case NECP_POLICY_CONDITION_DOMAIN_FILTER: {
      // Make sure there is only one such rule
      if (condition_length >= sizeof(cond_domain_filter) &&
          cond_domain_filter == 0) {
        memcpy(&cond_domain_filter, condition_value,
               sizeof(cond_domain_filter));
        if (cond_domain_filter != 0) {
          master_condition_mask |= NECP_KERNEL_CONDITION_DOMAIN_FILTER;
          if (condition_is_negative) {
            master_condition_negated_mask |=
                NECP_KERNEL_CONDITION_DOMAIN_FILTER;
          }
          socket_only_conditions = TRUE;
        }
      }
    ...
    case NECP_POLICY_CONDITION_URL: {
      ...
      if (condition_length > 0 && cond_url == NULL) {
        ...
        cond_url = necp_create_trimmed_domain((char *)condition_value,
                                              condition_length);
        if (cond_url != NULL) {
          master_condition_mask |= mask_value;
          if (condition_is_negative) {
    ...
    case NECP_POLICY_CONDITION_ACCOUNT: {
      // Make sure there is only one such rule
      if (condition_length > 0 && condition_length < UINT32_MAX &&
          cond_account_id == 0 && policy->applied_account == NULL) {
        ...
        if (string != NULL) {
          ...
          cond_account_id = necp_create_string_to_id_mapping(
              &necp_account_id_list, __unsafe_null_terminated_from_indexable(
                                         string, &string[condition_length]));
          if (cond_account_id != 0) {
            policy->applied_account =
                string; // Save the string in parent policy
    ...
    case NECP_POLICY_CONDITION_APPLICATION: {
      // Make sure there is only one such rule, because we save the uuid in the
      // policy
      if (condition_length >= sizeof(uuid_t) && cond_app_id == 0) {
        ...
        cond_app_id = necp_create_uuid_app_id_mapping(application_uuid,
    ...
    case NECP_POLICY_CONDITION_REAL_APPLICATION: {
      ...
      if (condition_length >= sizeof(uuid_t) && cond_real_app_id == 0) {
        ...
        cond_real_app_id =
            necp_create_uuid_app_id_mapping(real_application_uuid, NULL, FALSE);
    ...
    case NECP_POLICY_CONDITION_PID: {
      if (condition_length >= sizeof(pid_t)) {
        ...
        if (condition_length >= (sizeof(pid_t) + sizeof(cond_pid_version))) {
          memcpy(&cond_pid_version, (condition_value + sizeof(pid_t)),
    ...
    case NECP_POLICY_CONDITION_TRAFFIC_CLASS: {
      if (condition_length >= sizeof(struct necp_policy_condition_tc_range)) {
        ...
        memcpy(&cond_traffic_class, condition_value,
    ...
    case NECP_POLICY_CONDITION_BOUND_INTERFACE: {
      if (condition_length <= IFXNAMSIZ && condition_length > 0) {
        ...
        interface_name[condition_length - 1] =
            0; // Make sure the string is NULL terminated
        if (ifnet_find_by_name(
                __unsafe_null_terminated_from_indexable(
                    interface_name, &interface_name[condition_length - 1]),
                &cond_bound_interface) == 0) {
    ...
    case NECP_POLICY_CONDITION_LOCAL_NETWORKS: {
      ...
      if (condition_length >= sizeof(u_int8_t)) {
        memcpy(&cond_local_networks_flags, condition_value,
               sizeof(cond_local_networks_flags));
      }
      break;
    }
    case NECP_POLICY_CONDITION_LOCAL_ADDR:
    case NECP_POLICY_CONDITION_FLOW_LOCAL_ADDR: {
      ...
      memcpy(&cond_local_start, &address_struct->address,
             sizeof(address_struct->address));
    ...
    case NECP_POLICY_CONDITION_FLOW_REMOTE_ADDR: {
      ...
      memcpy(&cond_remote_start, &address_struct->address,
             sizeof(address_struct->address));
    ...
    case NECP_POLICY_CONDITION_LOCAL_ADDR_RANGE:
    case NECP_POLICY_CONDITION_FLOW_LOCAL_ADDR_RANGE: {
      struct necp_policy_condition_addr_range *address_struct =
      ...
      memcpy(&cond_local_start, &address_struct->start_address,
             sizeof(address_struct->start_address));
      memcpy(&cond_local_end, &address_struct->end_address,
    ...
    case NECP_POLICY_CONDITION_PACKET_FILTER_TAGS: {
      if (condition_length >= sizeof(u_int16_t)) {
        ...
        memcpy(&cond_packet_filter_tags, condition_value,
  ...
  case NECP_POLICY_RESULT_PASS: {
    ...
    if (necp_policy_result_get_parameter_length_from_buffer(
            policy->result, policy->result_size) > 0) {
      if (necp_policy_get_result_parameter(policy, (u_int8_t *)&pass_flags,
                                           sizeof(pass_flags))) {
  ...
  case NECP_POLICY_RESULT_DROP: {
    ...
    if (necp_policy_result_get_parameter_length_from_buffer(
            policy->result, policy->result_size) > 0) {
      if (necp_policy_get_result_parameter(policy, (u_int8_t *)&drop_flags,
                                           sizeof(drop_flags))) {
        ultimate_result_parameter.drop_flags = drop_flags;
        if (ultimate_result_parameter.drop_flags &
    ...
    if (socket_only_conditions) { // socket_ip_conditions can be TRUE or FALSE
      ...
      socket_layer_non_id_conditions = TRUE;
      ip_output_layer_non_id_conditions = TRUE;
      ip_output_layer_non_id_only = TRUE; // Only apply drop to packets that
                                          // didn't go through socket layer
    }
    break;
  }
  case NECP_POLICY_RESULT_SKIP: {
    u_int32_t skip_policy_order = 0;
    if (necp_policy_get_result_parameter(policy, (u_int8_t *)&skip_policy_order,
  ...
  case NECP_POLICY_RESULT_SOCKET_FILTER: {
    ...
    if (necp_policy_get_result_parameter(policy, (u_int8_t *)&control_unit,
                                         sizeof(control_unit))) {
  ...
  case NECP_POLICY_RESULT_IP_TUNNEL: {
    ...
    u_int32_t tunnel_parameters_length =
    ...
    if (tunnel_parameters_length > sizeof(u_int32_t) &&
        ...
        necp_policy_get_result_parameter(policy, (u_int8_t *)&tunnel_parameters,
                                         sizeof(tunnel_parameters))) {
      ifnet_t __single tunnel_interface = NULL;
      tunnel_parameters
          .interface_name[tunnel_parameters_length - sizeof(u_int32_t) - 1] =
          0; // Make sure the string is NULL terminated
      if (ifnet_find_by_name(__unsafe_null_terminated_from_indexable(
                                 tunnel_parameters.interface_name),
                             &tunnel_interface) == 0) {
        ultimate_result_parameter.tunnel_interface_index =
            tunnel_interface->if_index;
  ...
  case NECP_POLICY_RESULT_REMOVE_NETAGENT: {
    ...
    if (necp_policy_get_result_parameter(policy, (u_int8_t *)&netagent_uuid,
                                         sizeof(netagent_uuid))) {
      ultimate_result_parameter.netagent_id =
          necp_create_agent_uuid_id_mapping(netagent_uuid);
  ...
  case NECP_POLICY_RESULT_REMOVE_NETAGENT_TYPE: {
    ...
    if (necp_policy_get_result_parameter(policy, (u_int8_t *)&netagent_type,
                                         sizeof(netagent_type))) {
      ultimate_result_parameter.netagent_id =
          necp_create_agent_type_to_id_mapping(&netagent_type);
      if (ultimate_result_parameter.netagent_id != 0) {
  ...
  case NECP_POLICY_RESULT_SOCKET_SCOPED: {
    ...
    if (interface_name_length <= IFXNAMSIZ && interface_name_length > 0) {
      ...
      necp_policy_get_result_parameter(policy, (u_int8_t *)interface_name,
                                       interface_name_length);
      interface_name[interface_name_length - 1] =
          0; // Make sure the string is NULL terminated
      if (ifnet_find_by_name(
              __unsafe_null_terminated_from_indexable(
                  interface_name, &interface_name[interface_name_length - 1]),
              &scope_interface) == 0) {
        ultimate_result_parameter.scoped_interface_index =
            scope_interface->if_index;
        socket_layer_non_id_conditions = TRUE;
  ...
  case NECP_POLICY_RESULT_ROUTE_RULES: {
    if (policy->route_rules != NULL && policy->route_rules_size > 0) {
      ...
      u_int32_t route_rule_id = necp_create_route_rule(
          &necp_route_rules, policy->route_rules, policy->route_rules_size,
          &has_socket_only_actions);
      if (route_rule_id > 0) {
        policy->applied_route_rules_id = route_rule_id;
        ultimate_result_parameter.route_rule_id = route_rule_id;
        if (socket_only_conditions ||
            has_socket_only_actions) { // socket_ip_conditions can be TRUE or
                                       // FALSE
          socket_layer_non_id_conditions = TRUE;
        } else if (socket_ip_conditions) {
          socket_layer_non_id_conditions = TRUE;
          ip_output_layer_non_id_conditions = TRUE;
          ip_output_layer_non_id_only =
              TRUE; // Only apply route rules to packets that didn't go through
                    // socket layer
        }
      }
    }
  ...
  if (socket_layer_non_id_conditions) {
    necp_kernel_policy_id policy_id = necp_kernel_socket_policy_add(
        policy->order, session->session_order, session->proc_pid,
        master_condition_mask, master_condition_negated_mask, cond_app_id,
        cond_real_app_id, cond_custom_entitlement, cond_account_id, cond_domain,
        cond_domain_filter, cond_url, cond_pid, cond_pid_version, cond_uid,
        cond_real_uid, cond_bound_interface, cond_traffic_class, cond_protocol,
        &cond_local_start, &cond_local_end, cond_local_prefix,
        &cond_remote_start, &cond_remote_end, cond_remote_prefix,
        &cond_agent_type, &cond_sdk_version, cond_client_flags,
        cond_signing_identifier, cond_packet_filter_tags, cond_scheme_port,
        cond_bound_interface_flags, cond_bound_interface_eflags,
        cond_bound_interface_xflags, cond_local_networks_flags, ultimate_result,
  ...
  if (ip_output_layer_non_id_conditions) {
    ...
    necp_kernel_policy_id policy_id = necp_kernel_ip_output_policy_add(
        policy->order, NECP_KERNEL_POLICY_SUBORDER_NON_ID_CONDITIONS,
        session->session_order, session->proc_pid, condition_mask,
        master_condition_negated_mask, NECP_KERNEL_POLICY_ID_NONE,
        cond_bound_interface, 0, cond_protocol, &cond_local_start,
        &cond_local_end, cond_local_prefix, &cond_remote_start,
        &cond_remote_end, cond_remote_prefix, cond_packet_filter_tags,
        cond_scheme_port, cond_bound_interface_flags,
        cond_bound_interface_eflags, cond_bound_interface_xflags,
        cond_local_networks_flags, ultimate_result, ultimate_result_parameter);
    ...
    policy->kernel_ip_output_policies
        [NECP_KERNEL_POLICY_SUBORDER_NON_ID_CONDITIONS] = policy_id;
  }

  if (ip_output_layer_id_condition) {
    necp_kernel_policy_id policy_id = necp_kernel_ip_output_policy_add(
        policy->order, NECP_KERNEL_POLICY_SUBORDER_ID_CONDITION,
        session->session_order, session->proc_pid,
        NECP_KERNEL_CONDITION_POLICY_ID | NECP_KERNEL_CONDITION_ALL_INTERFACES,
        0, cond_ip_output_layer_id, NULL, 0, 0, NULL, NULL, 0, NULL, NULL, 0, 0,
        0, cond_bound_interface_flags, cond_bound_interface_eflags,
        cond_bound_interface_xflags, cond_local_networks_flags, ultimate_result,
        ultimate_result_parameter);
    ...
    policy
        ->kernel_ip_output_policies[NECP_KERNEL_POLICY_SUBORDER_ID_CONDITION] =
        policy_id;
  }
  ...
  if (ip_output_layer_tunnel_condition_from_id) {
    necp_kernel_policy_id policy_id = necp_kernel_ip_output_policy_add(
        policy->order, NECP_KERNEL_POLICY_SUBORDER_NON_ID_TUNNEL_CONDITION,
        session->session_order, session->proc_pid,
        NECP_KERNEL_CONDITION_POLICY_ID | NECP_KERNEL_CONDITION_LAST_INTERFACE |
            NECP_KERNEL_CONDITION_ALL_INTERFACES,
        0,
        policy->kernel_ip_output_policies
            [NECP_KERNEL_POLICY_SUBORDER_NON_ID_CONDITIONS],
        NULL, cond_last_interface_index, 0, NULL, NULL, 0, NULL, NULL, 0, 0, 0,
        cond_bound_interface_flags, cond_bound_interface_eflags,
        cond_bound_interface_xflags, cond_local_networks_flags,
        secondary_result, secondary_result_parameter);
    ...
    policy->kernel_ip_output_policies
        [NECP_KERNEL_POLICY_SUBORDER_NON_ID_TUNNEL_CONDITION] = policy_id;
  }

  if (ip_output_layer_tunnel_condition_from_id) {
    necp_kernel_policy_id policy_id = necp_kernel_ip_output_policy_add(
        policy->order, NECP_KERNEL_POLICY_SUBORDER_ID_TUNNEL_CONDITION,
        session->session_order, session->proc_pid,
        NECP_KERNEL_CONDITION_POLICY_ID | NECP_KERNEL_CONDITION_LAST_INTERFACE |
            NECP_KERNEL_CONDITION_ALL_INTERFACES,
        0,
    ...
    policy->kernel_ip_output_policies
        [NECP_KERNEL_POLICY_SUBORDER_ID_TUNNEL_CONDITION] = policy_id;
  }
...
static necp_kernel_policy_id necp_kernel_policy_get_new_id(bool socket_level) {
  ...
  if (socket_level) {
    ...
    do {
      ...
      if (necp_last_kernel_socket_policy_id <
    ...
    } while (necp_kernel_socket_policy_find(newid) !=
             NULL); // If already used, keep trying
  } else {
    bool wrapped = FALSE;
    do {
      necp_last_kernel_ip_policy_id++;
      if (necp_last_kernel_ip_policy_id <
          NECP_KERNEL_POLICY_ID_FIRST_VALID_IP) {
        if (wrapped) {
          // Already wrapped, give up
    ...
    } while (necp_kernel_ip_output_policy_find(newid) !=
             NULL); // If already used, keep trying
  }
...
#define NECP_KERNEL_VALID_SOCKET_CONDITIONS                                    \
  (NECP_KERNEL_CONDITION_APP_ID | NECP_KERNEL_CONDITION_REAL_APP_ID |          \
   NECP_KERNEL_CONDITION_DOMAIN | NECP_KERNEL_CONDITION_ACCOUNT_ID |           \
   NECP_KERNEL_CONDITION_PID | NECP_KERNEL_CONDITION_UID |                     \
   NECP_KERNEL_CONDITION_REAL_UID | NECP_KERNEL_CONDITION_ALL_INTERFACES |     \
   NECP_KERNEL_CONDITION_BOUND_INTERFACE |                                     \
   NECP_KERNEL_CONDITION_TRAFFIC_CLASS | NECP_KERNEL_CONDITION_PROTOCOL |      \
   NECP_KERNEL_CONDITION_LOCAL_START | NECP_KERNEL_CONDITION_LOCAL_END |       \
   NECP_KERNEL_CONDITION_LOCAL_PREFIX | NECP_KERNEL_CONDITION_REMOTE_START |   \
   NECP_KERNEL_CONDITION_REMOTE_END | NECP_KERNEL_CONDITION_REMOTE_PREFIX |    \
   NECP_KERNEL_CONDITION_ENTITLEMENT |                                         \
   NECP_KERNEL_CONDITION_CUSTOM_ENTITLEMENT |                                  \
   NECP_KERNEL_CONDITION_AGENT_TYPE | NECP_KERNEL_CONDITION_HAS_CLIENT |       \
   NECP_KERNEL_CONDITION_LOCAL_NETWORKS | NECP_KERNEL_CONDITION_CLIENT_FLAGS | \
   NECP_KERNEL_CONDITION_LOCAL_EMPTY | NECP_KERNEL_CONDITION_REMOTE_EMPTY |    \
   NECP_KERNEL_CONDITION_PLATFORM_BINARY | NECP_KERNEL_CONDITION_SDK_VERSION | \
   NECP_KERNEL_CONDITION_SIGNING_IDENTIFIER |                                  \
   NECP_KERNEL_CONDITION_PACKET_FILTER_TAGS |                                  \
   NECP_KERNEL_CONDITION_IS_LOOPBACK |                                         \
   NECP_KERNEL_CONDITION_DELEGATE_IS_PLATFORM_BINARY |                         \
   NECP_KERNEL_CONDITION_SCHEME_PORT | NECP_KERNEL_CONDITION_DOMAIN_FILTER |   \
   NECP_KERNEL_CONDITION_SYSTEM_SIGNED_RESULT |                                \
...
static necp_kernel_policy_id necp_kernel_socket_policy_add(
  ...
  new_kernel_policy->condition_mask =
      (condition_mask & NECP_KERNEL_VALID_SOCKET_CONDITIONS);
  if ((new_kernel_policy->condition_mask &
       NECP_KERNEL_CONDITION_ALL_INTERFACES) &&
      (new_kernel_policy->condition_mask &
       NECP_KERNEL_CONDITION_BOUND_INTERFACE)) {
  ...
  if ((new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_REAL_APP_ID) &&
      !(new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_APP_ID)) {
  ...
  if (new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_EMPTY) {
    new_kernel_policy->condition_mask &=
        ~(NECP_KERNEL_CONDITION_LOCAL_PREFIX | NECP_KERNEL_CONDITION_LOCAL_END);
  }
  if ((new_kernel_policy->condition_mask &
  ...
  new_kernel_policy->condition_negated_mask =
      condition_negated_mask & new_kernel_policy->condition_mask;
  ...
  if ((new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_DOMAIN) ||
    ...
    new_kernel_policy->cond_domain_dot_count =
        necp_count_dots(__unsafe_null_terminated_to_indexable(cond_domain),
                        strlen(cond_domain));
  }
  ...
  if (new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_START) {
    SOCKADDR_COPY(cond_local_start, &new_kernel_policy->cond_local_start,
                  cond_local_start->sa.sa_len);
  }
  if (new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_END) {
    SOCKADDR_COPY(cond_local_end, &new_kernel_policy->cond_local_end,
                  cond_local_end->sa.sa_len);
  }
  ...
  if (new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_REMOTE_START) {
    SOCKADDR_COPY(cond_remote_start, &new_kernel_policy->cond_remote_start,
                  cond_remote_start->sa.sa_len);
  }
  if (new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_REMOTE_END) {
    SOCKADDR_COPY(cond_remote_end, &new_kernel_policy->cond_remote_end,
                  cond_remote_end->sa.sa_len);
  }
  ...
  if (new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_AGENT_TYPE) {
    memcpy(&new_kernel_policy->cond_agent_type, cond_agent_type,
           sizeof(*cond_agent_type));
  }
  if (new_kernel_policy->condition_mask & NECP_KERNEL_CONDITION_SDK_VERSION) {
    memcpy(&new_kernel_policy->cond_sdk_version, cond_sdk_version,
  ...
  if (new_kernel_policy->condition_mask &
    ...
    new_kernel_policy->cond_bound_interface_eflags =
        cond_bound_interface_eflags;
  ...
  memcpy(&new_kernel_policy->result_parameter, &result_parameter,
  ...
  LIST_INSERT_SORTED_TWICE_ASCENDING(&necp_kernel_socket_policies,
                                     new_kernel_policy, chain, session_order,
                                     order, tmp_kernel_policy);
...
necp_kernel_socket_policy_find(necp_kernel_policy_id policy_id) {
  ...
  LIST_FOREACH_SAFE(kernel_policy, &necp_kernel_socket_policies, chain,
                    tmp_kernel_policy) {
...
static inline const char *__sized_by(MAX_RESULT_STRING_LEN)
  ...
  case NECP_KERNEL_POLICY_RESULT_PASS: {
    snprintf(result_string, MAX_RESULT_STRING_LEN, "Pass (%X)",
             result_parameter.pass_flags);
    break;
  }
  case NECP_KERNEL_POLICY_RESULT_SKIP: {
    snprintf(result_string, MAX_RESULT_STRING_LEN, "Skip (%u)",
             result_parameter.skip_policy_order);
    break;
  }
  case NECP_KERNEL_POLICY_RESULT_DROP: {
    snprintf(result_string, MAX_RESULT_STRING_LEN, "Drop (%X)",
             result_parameter.drop_flags);
    break;
  }
  case NECP_KERNEL_POLICY_RESULT_SOCKET_DIVERT: {
    snprintf(result_string, MAX_RESULT_STRING_LEN, "SocketDivert (%d)",
             result_parameter.flow_divert_control_unit);
    break;
  }
  case NECP_KERNEL_POLICY_RESULT_SOCKET_FILTER: {
    snprintf(result_string, MAX_RESULT_STRING_LEN, "SocketFilter (%d)",
             result_parameter.filter_control_unit);
    break;
  }
  case NECP_KERNEL_POLICY_RESULT_IP_TUNNEL: {
    ifnet_t interface = ifindex2ifnet[result_parameter.tunnel_interface_index];
    snprintf(result_string, MAX_RESULT_STRING_LEN, "IPTunnel (%s%d)",
  ...
  case NECP_KERNEL_POLICY_RESULT_SOCKET_SCOPED: {
    ...
    snprintf(result_string, MAX_RESULT_STRING_LEN, "SocketScoped (%s%d)",
             ifnet_name(interface), ifnet_unit(interface));
    break;
  }
  ...
  case NECP_KERNEL_POLICY_RESULT_ROUTE_RULES: {
    ...
    struct necp_route_rule *route_rule = necp_lookup_route_rule_locked(
    ...
    if (route_rule != NULL) {
      for (index = 0; index < MAX_ROUTE_RULE_INTERFACES; index++) {
        if (route_rule->exception_if_indices[index] != 0) {
          ifnet_t interface =
              ifindex2ifnet[route_rule->exception_if_indices[index]];
          snprintf(interface_names[index], IFXNAMSIZ, "%s%d",
                   ifnet_name(interface), ifnet_unit(interface));
        } else {
      ...
      case NECP_ROUTE_RULE_DENY_INTERFACE_WITH_TYPE:
        snprintf(
            result_string, MAX_RESULT_STRING_LEN,
            "RouteRules (Only "
            "%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s)",
            (route_rule->cellular_action == NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? "Cell "
                : "",
            (route_rule->wifi_action == NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? "WiFi "
                : "",
            (route_rule->wired_action == NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? "Wired "
                : "",
            (route_rule->expensive_action == NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? "Exp "
                : "",
            (route_rule->constrained_action == NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? "Constrained "
                : "",
            (route_rule->ultra_constrained_action ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? "Ultra-Constrained "
                : "",
            (route_rule->companion_action == NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? "Companion "
                : "",
            (route_rule->vpn_action == NECP_ROUTE_RULE_ALLOW_INTERFACE) ? "VPN "
                                                                        : "",
            (route_rule->exception_if_actions[0] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[0]
                : "",
            (route_rule->exception_if_actions[0] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? " "
                : "",
            (route_rule->exception_if_actions[1] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[1]
                : "",
            (route_rule->exception_if_actions[1] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? " "
                : "",
            (route_rule->exception_if_actions[2] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[2]
                : "",
            (route_rule->exception_if_actions[2] ==
            ...
            (route_rule->exception_if_actions[3] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? " "
                : "",
            (route_rule->exception_if_actions[4] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[4]
                : "",
            (route_rule->exception_if_actions[4] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? " "
                : "",
            (route_rule->exception_if_actions[5] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[5]
                : "",
            (route_rule->exception_if_actions[5] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? " "
                : "",
            (route_rule->exception_if_actions[6] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[6]
                : "",
            (route_rule->exception_if_actions[6] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? " "
                : "",
            (route_rule->exception_if_actions[7] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[7]
                : "",
            (route_rule->exception_if_actions[7] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? " "
                : "",
            (route_rule->exception_if_actions[8] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[8]
                : "",
            (route_rule->exception_if_actions[8] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? " "
                : "",
            (route_rule->exception_if_actions[9] ==
             NECP_ROUTE_RULE_ALLOW_INTERFACE)
                ? interface_names[9]
                : "");
        break;
      ...
      case NECP_ROUTE_RULE_QOS_MARKING:
        snprintf(
            result_string, MAX_RESULT_STRING_LEN,
            "RouteRules (QoSMarking "
            "%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s)",
            (route_rule->cellular_action == NECP_ROUTE_RULE_QOS_MARKING)
                ? "Cell "
                : "",
            ...
            (route_rule->wired_action == NECP_ROUTE_RULE_QOS_MARKING) ? "Wired "
                                                                      : "",
            (route_rule->expensive_action == NECP_ROUTE_RULE_QOS_MARKING)
                ? "Exp "
                : "",
            (route_rule->constrained_action == NECP_ROUTE_RULE_QOS_MARKING)
                ? "Constrained "
                : "",
            (route_rule->ultra_constrained_action ==
             NECP_ROUTE_RULE_QOS_MARKING)
                ? "Ultra-Constrained "
                : "",
            (route_rule->companion_action == NECP_ROUTE_RULE_QOS_MARKING)
                ? "Companion "
                : "",
            ...
            (route_rule->exception_if_actions[8] == NECP_ROUTE_RULE_QOS_MARKING)
                ? " "
                : "",
            (route_rule->exception_if_actions[9] == NECP_ROUTE_RULE_QOS_MARKING)
                ? interface_names[9]
                : "");
        break;
      default:
  ...
  case NECP_KERNEL_POLICY_RESULT_USE_NETAGENT: {
    ...
    struct necp_uuid_id_mapping *mapping =
        necp_uuid_lookup_uuid_with_agent_id_locked(
            result_parameter.netagent_id);
    if (mapping != NULL) {
      uuid_unparse(mapping->uuid, uuid_string);
      found_mapping = TRUE;
    }
    snprintf(result_string, MAX_RESULT_STRING_LEN, "UseNetAgent (%s)",
             found_mapping ? uuid_string : "Unknown");
    break;
  }
  case NECP_KERNEL_POLICY_RESULT_NETAGENT_SCOPED: {
    ...
    snprintf(result_string, MAX_RESULT_STRING_LEN, "NetAgentScoped (%s)",
             found_mapping ? uuid_string : "Unknown");
    break;
  }
  case NECP_KERNEL_POLICY_RESULT_REMOVE_NETAGENT: {
    ...
    snprintf(result_string, MAX_RESULT_STRING_LEN, "RemoveNetAgent (%s)",
             found_mapping ? uuid_string : "Unknown");
    break;
  }
  case NECP_KERNEL_POLICY_RESULT_REMOVE_NETAGENT_TYPE: {
    bool found_mapping = FALSE;
    struct necp_agent_type_id_mapping *mapping =
        necp_lookup_agent_type_with_id_locked(result_parameter.netagent_id);
    if (mapping != NULL) {
      found_mapping = TRUE;
    }
    snprintf(result_string, MAX_RESULT_STRING_LEN, "RemoveNetAgentType (%s/%s)",
             found_mapping ? mapping->agent_type.agent_domain : "Unknown",
             found_mapping ? mapping->agent_type.agent_type : "Unknown");
    break;
  }
  default: {
    snprintf(result_string, MAX_RESULT_STRING_LEN, "Unknown %d (%d)", result,
             result_parameter.tunnel_interface_index);
    break;
  }
  }
...
static void necp_kernel_socket_policies_dump_all(void) {
  if (necp_debug) {
    ...
    for (policy_i = 0;
      ...
      proc_name(policy->session_pid, proc_name_string, MAXCOMLEN);
      NECPLOG(LOG_DEBUG,
              "\t%3d. Policy ID: %5d\tProcess: %10.10s\tOrder: "
              "%04d.%04d\tMask: %llx\tResult: %s\n",
              policy_i, policy->id, proc_name_string, policy->session_order,
              policy->order, policy->condition_mask,
              necp_get_result_description(result_string, policy->result,
                                          policy->result_parameter));
    }
    ...
    NECPLOG0(LOG_DEBUG, "-----------\n");
    for (app_i = 0; app_i < NECP_KERNEL_SOCKET_POLICIES_MAP_NUM_APP_ID_BUCKETS;
         app_i++) {
      NECPLOG(LOG_DEBUG, "\tApp Bucket: %d\n", app_i);
      for (policy_i = 0;
           necp_kernel_socket_policies_map[app_i] != NULL &&
           (necp_kernel_socket_policies_map[app_i])[policy_i] != NULL;
           policy_i++) {
...
static inline bool necp_kernel_socket_policy_results_overlap(
    struct necp_kernel_socket_policy *upper_policy,
    struct necp_kernel_socket_policy *lower_policy) {
  if (upper_policy->result == NECP_KERNEL_POLICY_RESULT_DROP) {
    ...
    return TRUE;
             ...
             upper_policy->result ==
                 NECP_KERNEL_POLICY_RESULT_REMOVE_NETAGENT ||
             upper_policy->result ==
                 NECP_KERNEL_POLICY_RESULT_REMOVE_NETAGENT_TYPE) {
    ...
    if (upper_policy->session_order != lower_policy->session_order) {
      ...
      if (upper_policy->result_parameter.skip_policy_order == 0 ||
          lower_policy->order >=
              upper_policy->result_parameter.skip_policy_order) {
  ...
  // A hard pass, flow divert, tunnel, or scope will currently block out lower
  // policies
  return TRUE;
}

static bool necp_kernel_socket_policy_is_unnecessary(
    struct necp_kernel_socket_policy *policy,
    struct necp_kernel_socket_policy **__indexable policy_array,
    int valid_indices) {
  bool can_skip = FALSE;
  ...
  for (i = 0; i < valid_indices; i++) {
    ...
    // For policies in a skip window, we can't mark conflicting policies as
    // unnecessary
    if (can_skip) {
      if (highest_skip_session_order != compared_policy->session_order ||
          (highest_skip_order != 0 &&
           compared_policy->order >= highest_skip_order)) {
        ...
        if (compared_policy->result == NECP_KERNEL_POLICY_RESULT_SKIP) {
          if (compared_policy->result_parameter.skip_policy_order >
              highest_skip_order) {
            highest_skip_order =
                compared_policy->result_parameter.skip_policy_order;
          }
        }
        continue;
      }
    ...
    // The result of the compared policy must be able to block out this policy
    // result
    ...
    if ((policy->condition_mask & NECP_KERNEL_CONDITION_ALL_INTERFACES) &&
        !(compared_policy->condition_mask &
          NECP_KERNEL_CONDITION_ALL_INTERFACES)) {
      continue;
    }

    // If new policy matches Local Networks, compared policy must also
    if (((policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_NETWORKS) &&
         !(compared_policy->condition_mask &
           NECP_KERNEL_CONDITION_LOCAL_NETWORKS)) ||
        policy->cond_local_networks_flags !=
            compared_policy->cond_local_networks_flags) {
      continue;
    }
    ...
    // Compared must be more general than policy, and include only conditions
    // within policy
    if ((policy->condition_mask & compared_policy->condition_mask) !=
        compared_policy->condition_mask) {
      continue;
    }

    // Negative conditions must match for the overlapping conditions
    if ((policy->condition_negated_mask & compared_policy->condition_mask) !=
        (compared_policy->condition_negated_mask &
         compared_policy->condition_mask)) {
      continue;
    }
    ...
    if (compared_policy->condition_mask &
            NECP_KERNEL_CONDITION_CUSTOM_ENTITLEMENT &&
        strcmp(compared_policy->cond_custom_entitlement,
               policy->cond_custom_entitlement) != 0) {
      continue;
    }
    ...
    if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_PID &&
        (compared_policy->cond_pid != policy->cond_pid ||
         compared_policy->cond_pid_version != policy->cond_pid_version)) {
      continue;
    }
    ...
    if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_TRAFFIC_CLASS &&
        !(compared_policy->cond_traffic_class.start_tc <=
              policy->cond_traffic_class.start_tc &&
          compared_policy->cond_traffic_class.end_tc >=
              policy->cond_traffic_class.end_tc)) {
      continue;
    }
    ...
    if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_START) {
      if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_END) {
        if (!necp_is_range_in_range(SA(&policy->cond_local_start),
                                    SA(&policy->cond_local_end),
                                    SA(&compared_policy->cond_local_start),
                                    SA(&compared_policy->cond_local_end))) {
          continue;
        }
      } else if (compared_policy->condition_mask &
                 NECP_KERNEL_CONDITION_LOCAL_PREFIX) {
        if (compared_policy->cond_local_prefix > policy->cond_local_prefix ||
            !necp_is_addr_in_subnet(SA(&policy->cond_local_start),
                                    SA(&compared_policy->cond_local_start),
                                    compared_policy->cond_local_prefix)) {
          continue;
        }
      }
    }
    ...
    if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_REMOTE_START) {
      if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_REMOTE_END) {
        if (!necp_is_range_in_range(SA(&policy->cond_remote_start),
                                    SA(&policy->cond_remote_end),
                                    SA(&compared_policy->cond_remote_start),
                                    SA(&compared_policy->cond_remote_end))) {
          continue;
        }
      } else if (compared_policy->condition_mask &
                 NECP_KERNEL_CONDITION_REMOTE_PREFIX) {
        if (compared_policy->cond_remote_prefix > policy->cond_remote_prefix ||
            !necp_is_addr_in_subnet(SA(&policy->cond_remote_start),
                                    SA(&compared_policy->cond_remote_start),
                                    compared_policy->cond_remote_prefix)) {
          continue;
        }
      }
    }
    ...
    if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_AGENT_TYPE &&
        memcmp(&compared_policy->cond_agent_type, &policy->cond_agent_type,
               sizeof(policy->cond_agent_type)) == 0) {
      continue;
    }

    if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_SDK_VERSION &&
        memcmp(&compared_policy->cond_sdk_version, &policy->cond_sdk_version,
               sizeof(policy->cond_sdk_version)) == 0) {
      continue;
    }

    if (compared_policy->condition_mask &
            NECP_KERNEL_CONDITION_PACKET_FILTER_TAGS &&
        memcmp(&compared_policy->cond_packet_filter_tags,
               &policy->cond_packet_filter_tags,
               sizeof(policy->cond_packet_filter_tags)) == 0) {
      continue;
    }
    ...
    if (compared_policy->condition_mask & NECP_KERNEL_CONDITION_SCHEME_PORT &&
        memcmp(&compared_policy->cond_scheme_port, &policy->cond_scheme_port,
               sizeof(policy->cond_scheme_port)) == 0) {
      continue;
    }

    if (compared_policy->condition_mask &
            NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS &&
        (compared_policy->cond_bound_interface_flags !=
             policy->cond_bound_interface_flags ||
         compared_policy->cond_bound_interface_eflags !=
             policy->cond_bound_interface_eflags ||
         compared_policy->cond_bound_interface_xflags !=
             policy->cond_bound_interface_xflags)) {
      continue;
    }
...
static bool necp_kernel_socket_policies_reprocess(void) {
  ...
  LIST_FOREACH(kernel_policy, &necp_kernel_socket_policies, chain) {
    ...
    if (!(kernel_policy->condition_mask & NECP_KERNEL_CONDITION_APP_ID) ||
      ...
      necp_kernel_socket_policies_non_app_count++;
      for (app_i = 0;
           app_i < NECP_KERNEL_SOCKET_POLICIES_MAP_NUM_APP_ID_BUCKETS;
           app_i++) {
        necp_kernel_socket_policies_map_counts[app_i]++;
      }
    } else {
      necp_kernel_socket_policies_map_counts[NECP_SOCKET_MAP_APP_ID_TO_BUCKET(
          kernel_policy->cond_app_id)]++;
    }
  }
  ...
  for (app_i = 0; app_i < NECP_KERNEL_SOCKET_POLICIES_MAP_NUM_APP_ID_BUCKETS;
    ...
    if (necp_kernel_socket_policies_map_counts[app_i] > 0) {
      ...
      necp_kernel_socket_policies_map[app_i] = kalloc_type(
  ...
  necp_kernel_socket_policies_app_layer_map = kalloc_type(
      struct necp_kernel_socket_policy *,
  ...
  LIST_FOREACH(kernel_policy, &necp_kernel_socket_policies, chain) {
    ...
    if (!necp_dedup_policies ||
        !necp_kernel_socket_policy_is_unnecessary(
            kernel_policy, necp_kernel_socket_policies_app_layer_map,
            app_layer_current_free_index)) {
      necp_kernel_socket_policies_app_layer_map[app_layer_current_free_index] =
          kernel_policy;
      app_layer_current_free_index++;
      necp_kernel_socket_policies_app_layer_map[app_layer_current_free_index] =
          NULL;
    ...
    if (!(kernel_policy->condition_mask & NECP_KERNEL_CONDITION_APP_ID) ||
      ...
      for (app_i = 0;
        ...
        if (!necp_dedup_policies ||
            !necp_kernel_socket_policy_is_unnecessary(
                kernel_policy, necp_kernel_socket_policies_map[app_i],
                bucket_current_free_index[app_i])) {
          (necp_kernel_socket_policies_map[app_i])[(
              bucket_current_free_index[app_i])] = kernel_policy;
          bucket_current_free_index[app_i]++;
          (necp_kernel_socket_policies_map[app_i])[(
              bucket_current_free_index[app_i])] = NULL;
        }
      }
    } else {
...
static u_int32_t necp_get_new_string_id(void) {
  ...
  } while (necp_lookup_string_with_id_locked(&necp_account_id_list, newid) !=
           NULL); // If already used, keep trying
...
static struct necp_string_id_mapping *
necp_lookup_string_to_id_locked(struct necp_string_id_mapping_list *list,
                                char *string __null_terminated) {
...
necp_create_string_to_id_mapping(struct necp_string_id_mapping_list *list,
  ...
  if (existing_mapping != NULL) {
    ...
    if (buffer != NULL) {
      ...
      new_mapping->string =
          __unsafe_null_terminated_from_indexable(buffer, &buffer[length - 1]);
...
static u_int32_t necp_get_new_domain_filter_id(void) {
  ...
  } while (necp_lookup_domain_filter(&necp_global_domain_filter_list, newid) !=
           NULL); // If already used, keep trying
...
necp_create_domain_filter(struct necp_domain_filter_list *list,
  ...
  new_filter =
      kalloc_type(struct necp_domain_filter, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static bool
necp_remove_domain_filter(struct necp_domain_filter_list *list,
                          __unused struct necp_domain_filter_list *owner_list,
                          u_int32_t filter_id) {
...
static u_int32_t necp_get_new_domain_trie_id(void) {
  ...
  } while (necp_lookup_domain_trie(&necp_global_domain_trie_list, newid) !=
           NULL); // If already used, keep trying
...
necp_create_domain_trie(struct necp_domain_trie_list *list,
  ...
  if (net_trie_init_with_mem(
          &new_trie->trie, trie_request->data, trie_request->total_mem_size,
          trie_request->nodes_mem_size, trie_request->maps_mem_size,
          trie_request->bytes_mem_size, trie_request->nodes_count,
          trie_request->maps_count, trie_request->bytes_count) == false) {
...
static void necp_free_domain_trie(struct necp_domain_trie *existing_trie) {
...
static Boolean necp_match_domain_with_trie(struct necp_domain_trie_list *list,
  ...
  Boolean reverse =
      (necp_trie->trie_request->flags & NECP_DOMAIN_TRIE_FLAG_REVERSE_SEARCH);
  Boolean allow_partial_match = (necp_trie->trie_request->flags &
                                 NECP_DOMAIN_TRIE_FLAG_ALLOW_PARTIAL_MATCH);
  return net_trie_search(&necp_trie->trie, (const uint8_t *)domain, length,
                         &metadata, &metadata_length, reverse,
                         allow_partial_match,
                         necp_trie->trie_request->partial_match_terminator,
                         NULL, NULL) != NULL_TRIE_IDX;
}

#define NECP_FIRST_VALID_ROUTE_RULE_ID 1
#define NECP_FIRST_VALID_AGGREGATE_ROUTE_RULE_ID UINT16_MAX
static u_int32_t necp_get_new_route_rule_id(bool aggregate) {
  ...
  if (!aggregate) {
    ...
    } while (necp_lookup_route_rule_locked(&necp_route_rules, newid) !=
             NULL); // If already used, keep trying
  } else {
    ...
    do {
      ...
      if (necp_last_aggregate_route_rule_id <
...
static struct necp_route_rule *necp_lookup_route_rule_by_contents_locked(
    struct necp_route_rule_list *list, u_int8_t default_action,
    u_int8_t cellular_action, u_int8_t wifi_action, u_int8_t wired_action,
    u_int8_t expensive_action, u_int8_t constrained_action,
    u_int8_t companion_action, u_int8_t vpn_action,
    u_int8_t ultra_constrained_action, u_int32_t *__indexable if_indices,
    u_int8_t *__indexable if_actions, uuid_t netagent_uuid,
    uuid_t match_netagent_uuid, u_int32_t control_unit,
    u_int32_t effective_type) {
  struct necp_route_rule *searchentry = NULL;
  struct necp_route_rule *foundentry = NULL;
    ...
    if (searchentry->default_action == default_action &&
      ...
      for (index_a = 0; index_a < MAX_ROUTE_RULE_INTERFACES; index_a++) {
        ...
        for (index_b = 0; index_b < MAX_ROUTE_RULE_INTERFACES; index_b++) {
          ...
          if (searchentry->exception_if_indices[index_a] ==
                  if_indices[index_b] &&
              searchentry->exception_if_actions[index_a] ==
                  if_actions[index_b]) {
            found_index = TRUE;
            break;
          }
...
static u_int32_t necp_create_route_rule(
  ...
  if (route_rules_array == NULL || route_rules_array_size == 0 ||
      has_socket_only_actions == NULL) {
    return 0;
  }

  // Process rules
  while ((offset + sizeof(u_int8_t) + sizeof(u_int32_t)) <
         route_rules_array_size) {
    ifnet_t __single rule_interface = NULL;
    char interface_name[IFXNAMSIZ];
    u_int32_t length = 0;
    u_int8_t *__indexable value = necp_buffer_get_tlv_value(
        route_rules_array, route_rules_array_size, offset, &length);

    if (offset + sizeof(u_int8_t) + sizeof(u_int32_t) + length >
        route_rules_array_size) {
    ...
    if (rule_length == 0) {
      ...
      if ((rule_flags & NECP_ROUTE_RULE_FLAG_ULTRA_CONSTRAINED) ==
          NECP_ROUTE_RULE_FLAG_ULTRA_CONSTRAINED) {
      ...
      // Adjust remaining length to make sure the interface name is picked up
      // below
    ...
    if (rule_length <= IFXNAMSIZ) {
      ...
      interface_name[rule_length - 1] =
          0; // Make sure the string is NULL terminated
      if (ifnet_find_by_name(
              __unsafe_null_terminated_from_indexable(
                  interface_name, &interface_name[rule_length - 1]),
              &rule_interface) == 0) {
  ...
  existing_rule = necp_lookup_route_rule_by_contents_locked(
      list, default_action, cellular_action, wifi_action, wired_action,
      expensive_action, constrained_action, companion_action, vpn_action,
      ultra_constrained_action, if_indices, if_actions, netagent_uuid,
      match_netagent_uuid, control_unit, effective_type);
  if (existing_rule != NULL) {
    ...
    new_rule =
        kalloc_type(struct necp_route_rule, Z_WAITOK | Z_ZERO | Z_NOFAIL);
    ...
    if (!uuid_is_null(match_netagent_uuid)) {
      new_rule->match_netagent_id =
          necp_create_agent_uuid_id_mapping(match_netagent_uuid);
    }
...
static void necp_remove_aggregate_route_rule_for_id(u_int32_t rule_id) {
  if (rule_id) {
    ...
    LIST_FOREACH_SAFE(existing_rule, &necp_aggregate_route_rules, chain,
                      tmp_rule) {
      int index = 0;
  ...
  if (existing_rule != NULL) {
    if (os_ref_release_locked(&existing_rule->refcount) == 0) {
      ...
      necp_remove_agent_uuid_id_mapping_with_agent_id(
          existing_rule->netagent_id);
      necp_remove_agent_uuid_id_mapping_with_agent_id(
          existing_rule->match_netagent_id);
      LIST_REMOVE(existing_rule, chain);
...
static u_int32_t necp_create_aggregate_route_rule(
  ...
  LIST_FOREACH(existing_rule, &necp_aggregate_route_rules, chain) {
    if (memcmp(existing_rule->rule_ids, rule_ids,
               (sizeof(u_int32_t) * MAX_AGGREGATE_ROUTE_RULES)) == 0) {
      lck_rw_done(&necp_route_rule_lock);
  ...
  memcpy(new_rule->rule_ids, rule_ids,
         (sizeof(u_int32_t) * MAX_AGGREGATE_ROUTE_RULES));
...
static u_int32_t necp_get_new_app_uuid_id(void) {
  ...
  } while (necp_uuid_lookup_uuid_with_app_id_locked(newid) !=
           NULL); // If already used, keep trying
...
necp_uuid_lookup_uuid_with_app_id_locked(u_int32_t local_id) {
  ...
  for (uuid_list_head =
           &necp_uuid_app_id_hashtbl[necp_uuid_app_id_hash_num_buckets - 1];
       uuid_list_head >= necp_uuid_app_id_hashtbl; uuid_list_head--) {
...
static u_int32_t necp_create_uuid_app_id_mapping(uuid_t uuid,
  ...
  if (existing_mapping != NULL) {
    ...
    new_mapping = kalloc_type(struct necp_uuid_id_mapping, Z_WAITOK | Z_NOFAIL);
...
static u_int32_t necp_get_new_agent_id(bool uuid) {
  ...
  if (uuid) {
    ...
    } while (necp_uuid_lookup_uuid_with_agent_id_locked(newid) !=
             NULL); // If already used, keep trying
  } else {
    bool wrapped = FALSE;
    do {
    ...
    } while (necp_lookup_agent_type_with_id_locked(newid) !=
             NULL); // If already used, keep trying
  }
...
static struct necp_uuid_id_mapping *necp_uuid_get_null_agent_id_mapping(void) {
...
static struct necp_agent_type_id_mapping *necp_lookup_agent_type_to_id_locked(
    struct necp_policy_condition_agent_type *agent_type) {
  ...
  LIST_FOREACH(searchentry, &necp_agent_type_id_list, chain) {
    if (strlcmp(
            searchentry->agent_type.agent_domain,
            __unsafe_null_terminated_from_indexable(agent_type->agent_domain),
            NETAGENT_DOMAINSIZE) == 0 &&
        strlcmp(searchentry->agent_type.agent_type,
...
static u_int32_t necp_create_agent_type_to_id_mapping(
  ...
  if (existing_mapping != NULL) {
    ...
    new_mapping = kalloc_type(struct necp_agent_type_id_mapping,
                              Z_WAITOK | Z_ZERO | Z_NOFAIL);
    strlcpy(new_mapping->agent_type.agent_domain,
            __unsafe_null_terminated_from_indexable(agent_type->agent_domain),
            NETAGENT_DOMAINSIZE);
    strlcpy(new_mapping->agent_type.agent_type,
            __unsafe_null_terminated_from_indexable(agent_type->agent_type),
            NETAGENT_TYPESIZE);
...
static bool necp_kernel_socket_policies_update_uuid_table(void) {
  ...
  if (necp_uuid_app_id_mappings_dirty) {
    uuid_t dummy_uuid = {};
    if (proc_uuid_policy_kernel(PROC_UUID_POLICY_OPERATION_CLEAR, dummy_uuid,
                                PROC_UUID_NECP_APP_POLICY) < 0) {
      ...
      for (uuid_list_head =
        ...
        LIST_FOREACH(mapping, uuid_list_head, chain) {
          if (mapping->table_usecount > 0 &&
              proc_uuid_policy_kernel(PROC_UUID_POLICY_OPERATION_ADD,
                                      mapping->uuid,
                                      PROC_UUID_NECP_APP_POLICY) < 0) {
...
#define NECP_KERNEL_VALID_IP_OUTPUT_CONDITIONS                                 \
  (NECP_KERNEL_CONDITION_ALL_INTERFACES |                                      \
   NECP_KERNEL_CONDITION_BOUND_INTERFACE | NECP_KERNEL_CONDITION_PROTOCOL |    \
   NECP_KERNEL_CONDITION_LOCAL_START | NECP_KERNEL_CONDITION_LOCAL_END |       \
   NECP_KERNEL_CONDITION_LOCAL_PREFIX | NECP_KERNEL_CONDITION_REMOTE_START |   \
   NECP_KERNEL_CONDITION_REMOTE_END | NECP_KERNEL_CONDITION_REMOTE_PREFIX |    \
   NECP_KERNEL_CONDITION_POLICY_ID | NECP_KERNEL_CONDITION_LAST_INTERFACE |    \
...
static necp_kernel_policy_id necp_kernel_ip_output_policy_add(
  ...
  // Sanitize condition mask
  new_kernel_policy->condition_mask =
      (condition_mask & NECP_KERNEL_VALID_IP_OUTPUT_CONDITIONS);
  if ((new_kernel_policy->condition_mask &
       NECP_KERNEL_CONDITION_ALL_INTERFACES) &&
      (new_kernel_policy->condition_mask &
       NECP_KERNEL_CONDITION_BOUND_INTERFACE)) {
  ...
  LIST_INSERT_SORTED_THRICE_ASCENDING(&necp_kernel_ip_output_policies,
                                      new_kernel_policy, chain, session_order,
                                      order, suborder, tmp_kernel_policy);
...
necp_kernel_ip_output_policy_find(necp_kernel_policy_id policy_id) {
  ...
  LIST_FOREACH_SAFE(kernel_policy, &necp_kernel_ip_output_policies, chain,
                    tmp_kernel_policy) {
    if (kernel_policy->id == policy_id) {
      return kernel_policy;
    }
...
static void necp_kernel_ip_output_policies_dump_all(void) {
  if (necp_debug) {
    ...
    NECPLOG0(LOG_DEBUG, "-----------\n");
    for (id_i = 0; id_i < NECP_KERNEL_IP_OUTPUT_POLICIES_MAP_NUM_ID_BUCKETS;
         id_i++) {
      NECPLOG(LOG_DEBUG, " ID Bucket: %d\n", id_i);
      for (policy_i = 0;
           necp_kernel_ip_output_policies_map[id_i] != NULL &&
           (necp_kernel_ip_output_policies_map[id_i])[policy_i] != NULL;
           policy_i++) {
        policy = (necp_kernel_ip_output_policies_map[id_i])[policy_i];
        proc_name(policy->session_pid, proc_name_string, MAXCOMLEN);
        NECPLOG(LOG_DEBUG,
                "\t%3d. Policy ID: %5d\tProcess: %10.10s\tOrder: "
                "%04d.%04d.%d\tMask: %llx\tResult: %s\n",
                policy_i, policy->id, proc_name_string, policy->session_order,
                policy->order, policy->suborder, policy->condition_mask,
                necp_get_result_description(result_string, policy->result,
                                            policy->result_parameter));
      }
      NECPLOG0(LOG_DEBUG, "-----------\n");
...
static inline bool necp_kernel_ip_output_policy_results_overlap(
    struct necp_kernel_ip_output_policy *upper_policy,
    struct necp_kernel_ip_output_policy *lower_policy) {
  ...
  // All other IP Output policy results (drop, tunnel, hard pass) currently
  // overlap
  return TRUE;
}

static bool necp_kernel_ip_output_policy_is_unnecessary(
    struct necp_kernel_ip_output_policy *policy,
    struct necp_kernel_ip_output_policy **__indexable policy_array,
    int valid_indices) {
  bool can_skip = FALSE;
  u_int32_t highest_skip_session_order = 0;
  u_int32_t highest_skip_order = 0;
  ...
  for (i = 0; i < valid_indices; i++) {
    ...
    // The result of the compared policy must be able to block out this policy
    // result
    if (!necp_kernel_ip_output_policy_results_overlap(compared_policy,
                                                      policy)) {
      continue;
    }
...
static bool necp_kernel_ip_output_policies_reprocess(void) {
  ...
  LIST_FOREACH(kernel_policy, &necp_kernel_ip_output_policies, chain) {
    ...
    if (!(kernel_policy->condition_mask & NECP_KERNEL_CONDITION_POLICY_ID)) {
      ...
      necp_kernel_ip_output_policies_map_counts[NECP_IP_OUTPUT_MAP_ID_TO_BUCKET(
          kernel_policy->cond_policy_id)]++;
    }
  }
  ...
  for (i = 0; i < NECP_KERNEL_IP_OUTPUT_POLICIES_MAP_NUM_ID_BUCKETS; i++) {
    if (necp_kernel_ip_output_policies_map_counts[i] > 0) {
      // Allocate a NULL-terminated array of policy pointers for each bucket
      necp_kernel_ip_output_policies_map[i] = kalloc_type(
  ...
  LIST_FOREACH(kernel_policy, &necp_kernel_ip_output_policies, chain) {
    ...
    if (!(kernel_policy->condition_mask & NECP_KERNEL_CONDITION_POLICY_ID) ||
      ...
      for (i = 0; i < NECP_KERNEL_IP_OUTPUT_POLICIES_MAP_NUM_ID_BUCKETS; i++) {
        if (!necp_dedup_policies ||
            !necp_kernel_ip_output_policy_is_unnecessary(
                kernel_policy, necp_kernel_ip_output_policies_map[i],
                bucket_current_free_index[i])) {
          (necp_kernel_ip_output_policies_map[i])[(
              bucket_current_free_index[i])] = kernel_policy;
          bucket_current_free_index[i]++;
          (necp_kernel_ip_output_policies_map[i])[(
              bucket_current_free_index[i])] = NULL;
        }
      }
    } else {
...
necp_create_trimmed_domain(char *__sized_by(length) string, size_t length) {
  ...
  return __unsafe_null_terminated_from_indexable(trimmed_domain,
                                                 &trimmed_domain[sub.length]);
}
...
static bool necp_check_suffix(struct substring parent, struct substring suffix,
  ...
  // strncasecmp does case-insensitive check for all UTF-8 strings (ignores
  // non-ASCII characters)
  return strbufcasecmp(parent.string + length_difference,
                       parent.length - length_difference, suffix.string,
                       suffix.length) == 0;
}

static bool necp_hostname_matches_domain(struct substring hostname_substring,
  ...
  if (hostname_dot_count == domain_dot_count) {
    ...
    if (hostname_substring.length == domain_substring.length &&
        strbufcasecmp(hostname_substring.string, hostname_substring.length,
                      domain_substring.string, domain_substring.length) == 0) {
      return TRUE;
    }
...
bool net_domain_contains_hostname(char *hostname_string __null_terminated,
                                  char *domain_string __null_terminated) {
  if (hostname_string == NULL || domain_string == NULL) {
    return false;
  }

  struct substring hostname_substring;
  hostname_substring.string =
      __unsafe_null_terminated_to_indexable(hostname_string);
  hostname_substring.length = strlen(hostname_string);

  return necp_hostname_matches_domain(
      hostname_substring,
      necp_count_dots(hostname_substring.string, hostname_substring.length),
      domain_string,
      necp_count_dots(__unsafe_null_terminated_to_indexable(domain_string),
...
static char *__null_terminated necp_copy_string(char *__sized_by(length) string,
  ...
  return __unsafe_null_terminated_from_indexable(copied_string,
                                                 &copied_string[length]);
}

static u_int32_t necp_get_primary_direct_interface_index(void) {
  ...
  TAILQ_FOREACH(ordered_interface, &ifnet_ordered_head, if_ordered_link) {
    const u_int8_t functional_type =
        if_functional_type(ordered_interface, TRUE);
    if (functional_type != IFRTYPE_FUNCTIONAL_UNKNOWN &&
        functional_type != IFRTYPE_FUNCTIONAL_LOOPBACK) {
      // All known, non-loopback functional types represent direct physical
      // interfaces (Wi-Fi, Cellular, Wired)
...
static inline bool necp_task_has_match_entitlement(task_t task) {
  return task != NULL &&
         (IOTaskHasEntitlement(task, "com.apple.private.necp.match") ||
          IOTaskHasEntitlement(task,
                               "com.apple.developer.CaptiveNetworkPlugin"));
}

// Loopback traffic from certain entitled process will be dropped
static inline bool necp_task_has_loopback_drop_entitlement(task_t task) {
  bool drop =
      (task != NULL && IOTaskHasEntitlement(task, NECP_DDE_ENTITLEMENT));
  if (drop) {
...
// Some processes, due to particular entitlements, require using an NECP client
// to access networking. Returns true if the result should be a Drop.
static inline bool
...
static inline bool
necp_check_restricted_multicast_drop(proc_t proc, struct necp_socket_info *info,
  ...
  if ((platform != PLATFORM_IOS || sdk == 0 || (sdk >> 16) < 14 ||
  ...
  NECPLOG(LOG_INFO, "Dropping unentitled multicast (SDK 0x%x, min 0x%x)", sdk,
          min_sdk);

  return true;
}

#define NECP_KERNEL_ADDRESS_TYPE_CONDITIONS                                    \
  (NECP_KERNEL_CONDITION_LOCAL_START | NECP_KERNEL_CONDITION_LOCAL_END |       \
   NECP_KERNEL_CONDITION_LOCAL_PREFIX | NECP_KERNEL_CONDITION_REMOTE_START |   \
   NECP_KERNEL_CONDITION_REMOTE_END | NECP_KERNEL_CONDITION_REMOTE_PREFIX |    \
   NECP_KERNEL_CONDITION_LOCAL_EMPTY | NECP_KERNEL_CONDITION_REMOTE_EMPTY |    \
   NECP_KERNEL_CONDITION_LOCAL_NETWORKS | NECP_KERNEL_CONDITION_SCHEME_PORT)
static void necp_application_fillout_info_locked(
    task_t task, uuid_t application_uuid, uuid_t real_application_uuid,
    uuid_t responsible_application_uuid, char *account __null_terminated,
    char *domain __null_terminated, char *url __null_terminated, pid_t pid,
    int32_t pid_version, uid_t uid, uid_t real_uid, u_int16_t protocol,
    u_int32_t bound_interface_index, u_int32_t traffic_class,
    union necp_sockaddr_union *local_addr,
    union necp_sockaddr_union *remote_addr, u_int16_t local_port,
    u_int16_t remote_port, bool has_client, bool has_system_signed_result,
    proc_t real_proc, proc_t proc, proc_t responsible_proc,
    u_int32_t drop_order, u_int32_t client_flags, u_int16_t scheme_port,
    struct necp_socket_info *info, bool is_loopback, bool is_delegated) {
  ...
  if ((necp_kernel_socket_policies_condition_mask &
       NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS) &&
  ...
  if (necp_kernel_application_policies_condition_mask &
          NECP_KERNEL_CONDITION_APP_ID &&
      !uuid_is_null(application_uuid)) {
    struct necp_uuid_id_mapping *existing_mapping =
  ...
  if (((necp_kernel_application_policies_condition_mask &
    ...
    } else if (responsible_proc != NULL &&
               necp_is_platform_binary(responsible_proc)) {
      info->is_platform_binary = true;
      info->used_responsible_pid = true;
    } else {
  ...
  if (necp_kernel_application_policies_condition_mask &
    ...
    info->real_is_platform_binary =
        (necp_is_platform_binary(real_proc) ? true : false);
  }

  if (necp_kernel_application_policies_condition_mask &
          NECP_KERNEL_CONDITION_ACCOUNT_ID &&
      account != NULL) {
    struct necp_string_id_mapping *existing_mapping =
        necp_lookup_string_to_id_locked(&necp_account_id_list, account);
    if (existing_mapping) {
...
static void
necp_send_application_interface_denied_event(pid_t pid, uuid_t proc_uuid,
...
static void necp_send_network_denied_event(pid_t pid, uuid_t proc_uuid,
  ...
  netpolicy_post_msg(KEV_NETPOLICY_NETDENIED, &ev_netdenied.ev_data,
                     sizeof(ev_netdenied));
}
...
#define NECP_VERIFY_DELEGATION_ENTITLEMENT(_p, _c, _d)                         \
  if (!has_checked_delegation_entitlement) {                                   \
    has_delegation_entitlement =                                               \
        (priv_check_cred(_c, PRIV_NET_PRIVILEGED_SOCKET_DELEGATE, 0) == 0);    \
  ...
  if (!has_delegation_entitlement) {                                           \
    NECPLOG(LOG_ERR,                                                           \
            ...
            "network traffic for other processes by %s",                       \
            proc_name_address(_p), proc_pid(_p), _d);                          \
    break;                                                                     \
  }

int necp_application_find_policy_match_internal(
    proc_t proc, u_int8_t *__sized_by(parameters_size) parameters,
  ...
  necp_drop_all_bypass_check_result_t drop_all_bypass =
  ...
  proc_getexecutableuuid(proc, real_application_uuid,
...
#define NECP_MAX_REQUIRED_AGENTS 16
  ...
  struct necp_client_parameter_netagent_type
      required_agent_types[NECP_MAX_REQUIRED_AGENTS];
  ...
  necp_kernel_policy_result drop_dest_policy_result =
      NECP_KERNEL_POLICY_RESULT_NONE;

  lck_rw_lock_shared(&necp_kernel_policy_lock);
  if (necp_kernel_application_policies_count == 0 &&
      necp_drop_management_order == 0) {
  ...
  while ((offset + sizeof(u_int8_t) + sizeof(u_int32_t)) <= parameters_size) {
    ...
    if (length >
        (parameters_size - (offset + sizeof(u_int8_t) + sizeof(u_int32_t)))) {
    ...
    if (length > 0) {
      ...
      if (value != NULL) {
        ...
        case NECP_CLIENT_PARAMETER_DOMAIN: {
          ...
          ptr[length - 1] = 0;
          domain =
              __unsafe_null_terminated_from_indexable(ptr, &ptr[length - 1]);
          break;
        }
        ...
        case NECP_CLIENT_PARAMETER_LOCAL_ADDRESS: {
          ...
          if (length >= sizeof(struct necp_policy_condition_addr)) {
            ...
            if (necp_address_is_valid(&address_struct->address.sa)) {
              memcpy(&local_addr, &address_struct->address,
                     sizeof(address_struct->address));
            }
          }
          break;
        ...
        case NECP_CLIENT_PARAMETER_REMOTE_ADDRESS: {
          ...
          if (length >= sizeof(struct necp_policy_condition_addr)) {
            struct necp_policy_condition_addr *address_struct =
                (struct necp_policy_condition_addr *)(void *)value;
            if (necp_address_is_valid(&address_struct->address.sa)) {
              memcpy(&remote_addr, &address_struct->address,
                     sizeof(address_struct->address));
            }
          }
          break;
        ...
        case NECP_CLIENT_PARAMETER_PREFER_AGENT_TYPE: {
          ...
          if (length >= sizeof(struct necp_client_parameter_netagent_type)) {
            memcpy(&required_agent_types[num_required_agent_types], value,
        ...
        case NECP_CLIENT_PARAMETER_PREFER_AGENT: {
          ...
          if (length >= sizeof(uuid_t)) {
            if (netagent_get_agent_domain_and_type(
                    value,
                    required_agent_types[num_required_agent_types]
                        .netagent_domain,
                    required_agent_types[num_required_agent_types]
                        .netagent_type)) {
              num_required_agent_types++;
        ...
        case NECP_CLIENT_PARAMETER_RESOLVER_TAG: {
          ...
          struct necp_client_validatable *validatable =
              (struct necp_client_validatable *)value;
          if (length >= sizeof(struct necp_client_validatable)) {
            ...
            if (validatable->signable.sign_type ==
                ...
                validatable->signable.sign_type ==
                    NECP_CLIENT_SIGN_TYPE_SYSTEM_BROWSE_RESULT ||
                validatable->signable.sign_type ==
                    NECP_CLIENT_SIGN_TYPE_SYSTEM_SERVICE_RESOLVER_ANSWER) {
              has_system_signed_result = true;
            }
          }
          break;
        ...
        case NECP_CLIENT_PARAMETER_EXTENDED_FLAGS: {
          if (length >= sizeof(extended_client_flags)) {
            memcpy(&extended_client_flags, value,
                   sizeof(extended_client_flags));
          }
          break;
        }
        default: {
  ...
  if (necp_is_loopback(SA(&local_addr.sa), SA(&remote_addr.sa), NULL, NULL,
  ...
  if (bypass_type == NECP_BYPASS_TYPE_INTCOPROC ||
      (bypass_type == NECP_BYPASS_TYPE_LOOPBACK &&
       necp_pass_loopback == NECP_LOOPBACK_PASS_ALL)) {
    returned_result->policy_id = NECP_KERNEL_POLICY_ID_NO_MATCH;
    returned_result->skip_policy_id = NECP_KERNEL_POLICY_ID_NO_MATCH;
    ...
    if (bypass_type == NECP_BYPASS_TYPE_LOOPBACK) {
      ...
      *flags |= (NECP_CLIENT_RESULT_FLAG_IS_LOCAL |
  ...
  if (drop_order != 0) {
    if (remote_endpoint_type == NECP_CLIENT_ENDPOINT_TYPE_APPLICATION_SERVICE ||
        ...
        ((client_flags & NECP_CLIENT_PARAMETER_FLAG_INBOUND) &&
         remote_address_is_empty)) {
...
#if defined(XNU_TARGET_OS_OSX)
  if (effective_proc->p_responsible_pid > 0 &&
      effective_proc->p_responsible_pid != pid) {
    proc_getresponsibleuuid(effective_proc, responsible_application_uuid,
  ...
  necp_application_fillout_info_locked(
      task, application_uuid, real_application_uuid,
      responsible_application_uuid, account, domain, url, pid, pid_version, uid,
      real_uid, protocol, bound_interface_index, traffic_class, &local_addr,
      &remote_addr, local_port, remote_port, has_client,
      has_system_signed_result, proc, effective_proc, responsible_proc,
      drop_order, client_flags, scheme_port, &info,
      (bypass_type == NECP_BYPASS_TYPE_LOOPBACK), is_delegated);

  int debug = NECP_ENABLE_DATA_TRACE((&info.local_addr), (&info.remote_addr),
                                     info.protocol, info.pid,
  ...
  matched_policy = necp_socket_find_policy_match_with_info_locked(
      necp_kernel_socket_policies_app_layer_map, &info, &filter_control_unit,
      route_rule_id_array, &route_rule_id_array_count,
      MAX_AGGREGATE_ROUTE_RULES, netagent_ids, NECP_MAX_NETAGENTS,
      netagent_use_flags, NECP_MAX_NETAGENTS, required_agent_types,
      num_required_agent_types,
      info.used_responsible_pid ? responsible_proc : effective_proc, 0,
      &skip_policy_id, NULL, &drop_dest_policy_result, &drop_all_bypass,
  ...
  if (bypass_type == NECP_BYPASS_TYPE_LOOPBACK &&
    ...
    returned_result->routed_interface_index = lo_ifp->if_index;
    *flags |=
        (NECP_CLIENT_RESULT_FLAG_IS_LOCAL | NECP_CLIENT_RESULT_FLAG_IS_DIRECT);
    error = 0;
    NECP_DATA_TRACE_LOG_APP_LEVEL(
        debug, "APPLICATION", "RESULT - Loopback PASS <NO MATCH>",
        returned_result->policy_id, returned_result->skip_policy_id);
    goto done;
  }
  ...
  if (matched_policy) {
    ...
    memcpy(&returned_result->routing_result_parameter,
           &matched_policy->result_parameter,
           sizeof(returned_result->routing_result_parameter));
    if (returned_override_euuid != NULL && info.used_responsible_pid &&
        !(matched_policy->condition_mask & NECP_KERNEL_CONDITION_REAL_APP_ID)) {
      uuid_copy(*returned_override_euuid, responsible_application_uuid);
    }
  } else {
    bool drop_all = false;
    if (necp_drop_all_order > 0 || info.drop_order > 0 ||
        drop_dest_policy_result == NECP_KERNEL_POLICY_RESULT_DROP) {
    ...
    if (drop_all &&
        drop_all_bypass == NECP_DROP_ALL_BYPASS_CHECK_RESULT_FALSE) {
      returned_result->policy_id = NECP_KERNEL_POLICY_ID_NO_MATCH;
      returned_result->skip_policy_id = NECP_KERNEL_POLICY_ID_NO_MATCH;
      returned_result->routing_result = NECP_KERNEL_POLICY_RESULT_DROP;
      NECP_DATA_TRACE_LOG_APP_LEVEL(
          debug, "APPLICATION", "RESULT - DROP <NO MATCH>",
          returned_result->policy_id, returned_result->skip_policy_id);
    } else {
      returned_result->policy_id = NECP_KERNEL_POLICY_ID_NO_MATCH;
      returned_result->skip_policy_id = skip_policy_id;
      returned_result->routing_result = NECP_KERNEL_POLICY_RESULT_NONE;
      NECP_DATA_TRACE_LOG_APP_LEVEL(debug, "APPLICATION", "RESULT - NO MATCH",
                                    returned_result->policy_id,
                                    returned_result->skip_policy_id);
    }
  }
  if (necp_check_missing_client_drop(proc, &info) ||
    ...
    returned_result->routing_result = NECP_KERNEL_POLICY_RESULT_DROP;
    NECP_DATA_TRACE_LOG_APP_LEVEL(
        debug, "APPLICATION", "RESULT - DROP <NO CLIENT / MULTICAST>",
        returned_result->policy_id, returned_result->skip_policy_id);
  }
  if (filter_control_unit == NECP_FILTER_UNIT_NO_FILTER) {
    returned_result->filter_control_unit = 0;
  ...
  for (netagent_cursor = 0; netagent_cursor < NECP_MAX_NETAGENTS;
       netagent_cursor++) {
    ...
    if (necp_agent_id_is_uuid(netagent_id)) {
      struct necp_uuid_id_mapping *mapping =
          necp_uuid_lookup_uuid_with_agent_id_locked(netagent_id);
      if (mapping != NULL) {
        uuid_copy(returned_result->netagents[netagent_i], mapping->uuid);
        returned_result->netagent_use_flags[netagent_i] =
            netagent_use_flags[netagent_cursor];
        netagent_i++;
      }
    } else {
      struct necp_agent_type_id_mapping *mapping =
          necp_lookup_agent_type_with_id_locked(netagent_id);
      if (mapping != NULL &&
          removed_netagent_type_i < NECP_MAX_REMOVE_NETAGENT_TYPES &&
          netagent_use_flags[netagent_cursor] & NECP_AGENT_USE_FLAG_REMOVE) {
        memcpy(&returned_result->remove_netagent_types[removed_netagent_type_i],
               &mapping->agent_type, sizeof(mapping->agent_type));
  ...
  if (returned_result->routing_result ==
    ...
    output_bound_interface =
        returned_result->routing_result_parameter.scoped_interface_index;
  } else if (returned_result->routing_result ==
             NECP_KERNEL_POLICY_RESULT_IP_TUNNEL) {
    output_bound_interface =
        returned_result->routing_result_parameter.tunnel_interface_index;
  } else if (returned_result->routing_result ==
  ...
  if (returned_result->routing_result == NECP_KERNEL_POLICY_RESULT_DROP &&
      returned_result->routing_result_parameter.drop_flags &
          NECP_KERNEL_POLICY_DROP_FLAG_LOCAL_NETWORK) {
    if (!(matched_policy->result_parameter.drop_flags &
          NECP_KERNEL_POLICY_DROP_FLAG_SUPPRESS_ALERTS)) {
      // Trigger the event that we dropped due to a local network policy
#if defined(XNU_TARGET_OS_OSX)
      bool should_report_responsible_pid =
          (effective_proc->p_responsible_pid > 0 &&
           effective_proc->p_responsible_pid != pid);
      necp_send_network_denied_event(
          should_report_responsible_pid ? effective_proc->p_responsible_pid
                                        : pid,
          should_report_responsible_pid ? responsible_application_uuid
                                        : application_uuid,
          NETPOLICY_NETWORKTYPE_LOCAL);
#else
      necp_send_network_denied_event(pid, application_uuid,
                                     NETPOLICY_NETWORKTYPE_LOCAL);
#endif
    }
    if (reason != NULL) {
  ...
  if (local_addr.sa.sa_len == 0 ||
      (local_addr.sa.sa_family == AF_INET &&
       local_addr.sin.sin_addr.s_addr == 0) ||
      (local_addr.sa.sa_family == AF_INET6 &&
  ...
  if (remote_addr.sa.sa_len == 0 ||
      (remote_addr.sa.sa_family == AF_INET &&
       remote_addr.sin.sin_addr.s_addr == 0) ||
      (remote_addr.sa.sa_family == AF_INET6 &&
       IN6_IS_ADDR_UNSPECIFIED(&remote_addr.sin6.sin6_addr))) {
    no_remote_addr = TRUE;
  ...
  if (!no_local_addr &&
      (client_flags & NECP_CLIENT_PARAMETER_FLAG_LISTENER) != 0) {
    // Treat the output bound interface as the routed interface for local
    ...
    rt = rtalloc1_scoped(SA(&remote_addr), 0, 0, output_bound_interface);
    ...
    if (rt != NULL && rt->rt_ifp != NULL) {
       ...
       * local address.
       */
      if (bound_interface_index != IFSCOPE_NONE && rt->rt_ifa != NULL &&
          rt->rt_ifa->ifa_ifp &&
         ...
         */
        (void)sa_copy(SA(&remote_addr), &dst, &ifscope);
  ...
  if (returned_result->routed_interface_index != 0 &&
      returned_result->routed_interface_index !=
          lo_ifp->if_index && // Loopback can accept any local address
      !no_local_addr) {
    ...
    if (local_address_sanitized.ss_family == AF_INET6) {
      if (in6_embedded_scope ||
          !IN6_IS_SCOPE_EMBED(&SIN6(&local_address_sanitized)->sin6_addr)) {
        SIN6(&local_address_sanitized)->sin6_scope_id = 0;
      }
    }

    // Validate local address on routed interface
    struct ifaddr *ifa = ifa_ifwithaddr_scoped(
        SA(&local_address_sanitized), returned_result->routed_interface_index);
    if (ifa == NULL) {
  ...
  if (flags != NULL) {
    ...
    if (kernel_is_macos_or_server()) {
      ...
      if (filters & (NET_FILTER_EVENT_SOCKET | NET_FILTER_EVENT_INTERFACE |
                     NET_FILTER_EVENT_IP)) {
    ...
    if ((client_flags & NECP_CLIENT_PARAMETER_FLAG_LISTENER) == 0) {
      ...
      if (rt != NULL && (rt->rt_flags & RTF_LOCAL)) {
        ...
        if (remote_address_sanitized.ss_family == AF_INET6) {
          if (in6_embedded_scope ||
              !IN6_IS_SCOPE_EMBED(
                  &SIN6(&remote_address_sanitized)->sin6_addr)) {
            SIN6(&remote_address_sanitized)->sin6_scope_id = 0;
          }
        }
      ...
      if (rt != NULL && rt->rt_ifp != NULL) {
        ...
        if ((client_flags & NECP_CLIENT_PARAMETER_FLAG_DISCRETIONARY) &&
            (rt->rt_ifp->if_interface_state.valid_bitmask &
             IF_INTERFACE_STATE_LQM_STATE_VALID) &&
            rt->rt_ifp->if_interface_state.lqm_state ==
                IFNET_LQM_THRESH_ABORT) {
          *flags |= NECP_CLIENT_RESULT_FLAG_LINK_QUALITY_ABORT;
        }

        // Check QoS marking (fastlane)
        for (size_t route_rule_index = 0;
             route_rule_index < route_rule_id_array_count; route_rule_index++) {
          if (necp_update_qos_marking(rt->rt_ifp, netagent_ids,
                                      NECP_MAX_NETAGENTS,
                                      route_rule_id_array[route_rule_index])) {
        ...
        if (traffic_class == SO_TC_BK_SYS) {
          ...
          if (ifnet_get_throttle(rt->rt_ifp, &throttle_level) == 0) {
            if (throttle_level == IFNET_THROTTLE_OPPORTUNISTIC) {
              returned_result->routing_result = NECP_KERNEL_POLICY_RESULT_DROP;
              memset(&returned_result->routing_result_parameter, 0,
                     sizeof(returned_result->routing_result_parameter));
            }
          }
        }
    ...
    if (v4Route != NULL) {
      ...
      if (returned_v4_gateway != NULL && v4Route->rt_gateway != NULL &&
          v4Route->rt_gateway->sa_len == sizeof(returned_v4_gateway->u.sin)) {
        memcpy(&returned_v4_gateway->u.sin, v4Route->rt_gateway,
               sizeof(returned_v4_gateway->u.sin));
        memset(&returned_v4_gateway->u.sin.sin_zero, 0,
               sizeof(returned_v4_gateway->u.sin.sin_zero));
      }
      rtfree(v4Route);
      v4Route = NULL;
    }
    ...
    if (v6Route != NULL) {
      if (v6Route->rt_ifp != NULL) {
        *flags |= NECP_CLIENT_RESULT_FLAG_HAS_IPV6;
        ...
        if (ifnet_get_nat64prefix(v6Route->rt_ifp,
                                  returned_result->nat64_prefixes) == 0) {
          *flags |= NECP_CLIENT_RESULT_FLAG_HAS_NAT64;
        }
      }
      if (returned_v6_gateway != NULL && v6Route->rt_gateway != NULL &&
          v6Route->rt_gateway->sa_len == sizeof(returned_v6_gateway->u.sin6)) {
        SOCKADDR_COPY(v6Route->rt_gateway, &returned_v6_gateway->u.sin6,
                      sizeof(returned_v6_gateway->u.sin6));
      }
      rtfree(v6Route);
      v6Route = NULL;
    }
  ...
  // Take two passes through the rule list: first for rules that don't match
  // based on agents, second for rules that match based on agents. Since rules
  // can modify the agent list itself, this makes the logic more deterministic.
  // This allows a non-agent matching rule to remove an agent before it is used
  // for matching later.
  size_t route_rule_index = 0;
  bool second_pass = false;
  while (route_rule_index < route_rule_id_array_count) {
    bool rule_matches_agents =
        necp_route_rule_matches_agents(route_rule_id_array[route_rule_index]);
    ...
    bool route_is_allowed = necp_route_is_allowed(
        rt, NULL, netagent_ids, NECP_MAX_NETAGENTS,
        route_rule_id_array[route_rule_index], &interface_type_denied,
        &ultra_constrained_denied);
    if (!route_is_allowed) {
      ...
      if (interface_type_denied != IFRTYPE_FUNCTIONAL_UNKNOWN) {
        ...
        necp_send_application_interface_denied_event(pid, application_uuid,
        ...
        necp_send_network_denied_event(pid, application_uuid,
                                       NETPOLICY_NETWORKTYPE_ULTRA_CONSTRAINED);
      }
    ...
    // Check if there is a route rule that adds flow divert, if we don't already
    // have a terminal policy result
    if (returned_result->routing_result == NECP_KERNEL_POLICY_RESULT_NONE) {
      u_int32_t flow_divert_control_unit = necp_route_get_flow_divert(
          rt, netagent_ids, NECP_MAX_NETAGENTS,
      ...
      if (flow_divert_control_unit != 0) {
        ...
        returned_result->routing_result_parameter.flow_divert_control_unit =
            flow_divert_control_unit;
      }
      if (flow_divert_aggregate_unit != 0) {
        returned_result->flow_divert_aggregate_unit =
    ...
    bool remove = false;
    u_int32_t netagent_id =
        necp_route_get_netagent(rt, netagent_ids, NECP_MAX_NETAGENTS,
                                route_rule_id_array[route_rule_index], &remove);
    if (netagent_id != 0) {
      struct necp_uuid_id_mapping *mapping =
          necp_uuid_lookup_uuid_with_agent_id_locked(netagent_id);
      if (mapping != NULL) {
        ...
        for (netagent_cursor = 0; netagent_cursor < NECP_MAX_NETAGENTS;
             netagent_cursor++) {
          if (uuid_compare(returned_result->netagents[netagent_cursor],
                           mapping->uuid) == 0) {
            // Found the agent already present
            agent_already_present = true;
            if (remove) {
              // Mark as remove if necessary
              returned_result->netagent_use_flags[netagent_cursor] =
                  NECP_AGENT_USE_FLAG_REMOVE;
            }
          } else if (uuid_is_null(
                         returned_result->netagents[netagent_cursor])) {
            // Found open slot
            if (!agent_already_present) {
              uuid_copy(returned_result->netagents[netagent_cursor],
                        mapping->uuid);
              if (remove) {
                returned_result->netagent_use_flags[netagent_cursor] =
                    NECP_AGENT_USE_FLAG_REMOVE;
  ...
  if (rt != NULL && rt->rt_ifp != NULL) {
    ...
    const bool is_browser =
        ((client_flags & NECP_CLIENT_PARAMETER_FLAG_BROWSE) != 0);
    const bool expensive_prohibited =
        ((client_flags & NECP_CLIENT_PARAMETER_FLAG_PROHIBIT_EXPENSIVE) &&
         IFNET_IS_EXPENSIVE(rt->rt_ifp));
    const bool constrained_prohibited =
        ((client_flags & NECP_CLIENT_PARAMETER_FLAG_PROHIBIT_CONSTRAINED) &&
         IFNET_IS_CONSTRAINED(rt->rt_ifp));
    const bool ultra_constrained_not_allowed =
        (!(client_flags & NECP_CLIENT_PARAMETER_FLAG_ALLOW_ULTRA_CONSTRAINED) &&
         IFNET_IS_ULTRA_CONSTRAINED(rt->rt_ifp) &&
         (task == NULL ||
          (!if_ultra_constrained_default_allowed &&
           !IOTaskHasEntitlement(task, ULTRA_CONSTRAINED_ENTITLEMENT))));

    const bool interface_type_blocked =
        !necp_route_is_interface_type_allowed(rt, NULL, proc, NULL);
    if (!is_listener && !is_browser) {
      if (reason != NULL) {
        if (expensive_prohibited) {
          ...
          *reason = NECP_CLIENT_RESULT_REASON_ULTRA_CONSTRAINED_NOT_ALLOWED;
          necp_send_network_denied_event(
              pid, application_uuid, NETPOLICY_NETWORKTYPE_ULTRA_CONSTRAINED);
        }
      }
      if (expensive_prohibited || constrained_prohibited ||
          ultra_constrained_not_allowed || interface_type_blocked) {
...
static bool necp_is_route_local(union necp_sockaddr_union *remote_addr,
                                Boolean include_local_addresses) {
  struct rtentry *rt = NULL;
  bool is_local = FALSE;
  ...
  is_local =
      IS_NECP_DEST_IN_LOCAL_NETWORKS(rt, remote_addr, include_local_addresses);
...
static bool necp_socket_check_policy(
    struct necp_kernel_socket_policy *kernel_policy, necp_app_id app_id,
    necp_app_id real_app_id, uint8_t is_entitled, u_int32_t account_id,
    struct substring domain, u_int8_t domain_dot_count,
    const char *url __null_terminated, pid_t pid, int32_t pid_version,
    uid_t uid, uid_t real_uid, u_int32_t bound_interface_index,
    u_int32_t traffic_class, u_int16_t protocol,
    union necp_sockaddr_union *local, union necp_sockaddr_union *remote,
    struct necp_client_parameter_netagent_type *__counted_by(
        num_required_agent_types) required_agent_types,
    u_int32_t num_required_agent_types, bool has_client, uint32_t client_flags,
    int is_platform_binary, bool has_signed_result, proc_t proc,
    u_int16_t pf_tag, u_int16_t scheme_port, struct rtentry *rt,
    bool is_loopback, int debug, bool real_is_platform_binary,
    u_int32_t bound_interface_flags, u_int32_t bound_interface_eflags,
    u_int32_t bound_interface_xflags, struct necp_socket_info *info,
    bool is_delegated, struct socket *socket) {
  if (!(kernel_policy->condition_mask & NECP_KERNEL_CONDITION_ALL_INTERFACES)) {
    if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_BOUND_INTERFACE) {
      u_int32_t cond_bound_interface_index =
          kernel_policy->cond_bound_interface
              ? kernel_policy->cond_bound_interface->if_index
              : 0;
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_BOUND_INTERFACE,
          "NECP_KERNEL_CONDITION_BOUND_INTERFACE", cond_bound_interface_index,
          bound_interface_index);
      if (kernel_policy->condition_negated_mask &
          NECP_KERNEL_CONDITION_BOUND_INTERFACE) {
    ...
    if (kernel_policy->condition_mask &
        NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS) {
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS,
          "NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS - flags",
          kernel_policy->cond_bound_interface_flags, bound_interface_flags);
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS,
          "NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS - eflags",
          kernel_policy->cond_bound_interface_eflags, bound_interface_eflags);
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS,
          "NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS - xflags",
          kernel_policy->cond_bound_interface_xflags, bound_interface_xflags);
      if (kernel_policy->condition_negated_mask &
          NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS) {
        if ((kernel_policy->cond_bound_interface_flags &&
             (bound_interface_flags &
              kernel_policy->cond_bound_interface_flags)) ||
            (kernel_policy->cond_bound_interface_eflags &&
             (bound_interface_eflags &
              kernel_policy->cond_bound_interface_eflags)) ||
            (kernel_policy->cond_bound_interface_xflags &&
             (bound_interface_xflags &
              kernel_policy->cond_bound_interface_xflags))) {
    ...
    if (!(kernel_policy->condition_mask &
        ...
        !(kernel_policy->condition_mask &
          NECP_KERNEL_CONDITION_BOUND_INTERFACE_FLAGS)) {
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(debug, socket, "SOCKET", false,
                                           "Requiring no bound interface", 0,
                                           bound_interface_index);
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_APP_ID) {
    ...
    if (kernel_policy->condition_negated_mask &
      ...
      const char *signing_id __null_terminated =
          cs_identity_get(proc ? proc : current_proc());
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_SIGNING_IDENTIFIER,
          "NECP_KERNEL_CONDITION_SIGNING_IDENTIFIER",
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_REAL_APP_ID) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(debug, socket, "SOCKET",
                                         kernel_policy->condition_negated_mask &
                                             NECP_KERNEL_CONDITION_REAL_APP_ID,
                                         "NECP_KERNEL_CONDITION_REAL_APP_ID",
                                         kernel_policy->cond_real_app_id,
                                         real_app_id);
    if (kernel_policy->condition_negated_mask &
        NECP_KERNEL_CONDITION_REAL_APP_ID) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_HAS_CLIENT) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(debug, socket, "SOCKET", false,
                                         "NECP_KERNEL_CONDITION_HAS_CLIENT", 0,
                                         has_client);
    if (!has_client) {
      return FALSE;
    }
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_ENTITLEMENT) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(debug, socket, "SOCKET", false,
                                         "NECP_KERNEL_CONDITION_ENTITLEMENT", 0,
                                         is_entitled);
    if (!is_entitled) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_PLATFORM_BINARY) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        ...
        kernel_policy->condition_negated_mask &
            NECP_KERNEL_CONDITION_PLATFORM_BINARY,
        "NECP_KERNEL_CONDITION_PLATFORM_BINARY", 0, is_platform_binary);
    if (kernel_policy->condition_negated_mask &
        NECP_KERNEL_CONDITION_PLATFORM_BINARY) {
  ...
  if (kernel_policy->condition_mask &
      NECP_KERNEL_CONDITION_SYSTEM_SIGNED_RESULT) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        debug, socket, "SOCKET", false,
        "NECP_KERNEL_CONDITION_SYSTEM_SIGNED_RESULT", 0, is_platform_binary);
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_SDK_VERSION) {
    if (proc != NULL) {
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET3(
          debug, socket, "SOCKET", false, "NECP_KERNEL_CONDITION_SDK_VERSION",
          kernel_policy->cond_sdk_version.platform,
          kernel_policy->cond_sdk_version.min_version,
          kernel_policy->cond_sdk_version.version, proc_platform(proc),
          proc_min_sdk(proc), proc_sdk(proc));
  ...
  if (kernel_policy->condition_mask &
    ...
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR(
        debug, socket, "SOCKET", false,
        "NECP_KERNEL_CONDITION_CUSTOM_ENTITLEMENT", "n/a",
        kernel_policy->cond_custom_entitlement);
    if (kernel_policy->cond_custom_entitlement != NULL) {
      ...
      task_t __single task = proc_task(proc);
      if (task == NULL || !IOTaskHasEntitlementAsBooleanOrObject(
                              task, kernel_policy->cond_custom_entitlement)) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_EXACT_DOMAIN) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR(
        ...
        kernel_policy->condition_negated_mask &
            NECP_KERNEL_CONDITION_EXACT_DOMAIN,
        "NECP_KERNEL_CONDITION_EXACT_DOMAIN", kernel_policy->cond_domain,
        domain.string);
    // Exact match requires the number of dots to match (no suffix matching
    // allowed)
    bool domain_matches =
        (domain_dot_count == kernel_policy->cond_domain_dot_count &&
         necp_hostname_matches_domain(domain, domain_dot_count,
                                      kernel_policy->cond_domain,
                                      kernel_policy->cond_domain_dot_count));
    ...
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR(
        debug, socket, "SOCKET",
        kernel_policy->condition_negated_mask & NECP_KERNEL_CONDITION_DOMAIN,
        "NECP_KERNEL_CONDITION_DOMAIN", kernel_policy->cond_domain,
        domain.string);
    bool domain_matches = necp_hostname_matches_domain(
        domain, domain_dot_count, kernel_policy->cond_domain,
        kernel_policy->cond_domain_dot_count);
    if (domain_matches && socket != NULL) {
      socket->so_flags1 |= SOF1_DOMAIN_MATCHED_POLICY;
    }
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_DOMAIN_FILTER) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        ...
        kernel_policy->cond_domain_filter, 0);
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR(
        debug, socket, "SOCKET",
        kernel_policy->condition_negated_mask &
            NECP_KERNEL_CONDITION_DOMAIN_FILTER,
    ...
    if (NECP_IS_DOMAIN_FILTER_ID(kernel_policy->cond_domain_filter)) {
      struct necp_domain_filter *filter = necp_lookup_domain_filter(
          &necp_global_domain_filter_list, kernel_policy->cond_domain_filter);
      if (filter != NULL && filter->filter != NULL) {
        domain_matches = (domain.string != NULL && domain.length > 0)
                             ? net_bloom_filter_contains(
                                   filter->filter, domain.string, domain.length)
                             : FALSE;
      }
    } else {
      domain_matches = necp_match_domain_with_trie(
          &necp_global_domain_trie_list, kernel_policy->cond_domain_filter,
          domain.string, domain.length);
      if (debug) {
        NECPLOG(LOG_ERR,
                "DATA-TRACE: matching <%s %zu> with trie id %d - matched %d",
                domain.string, domain.length, kernel_policy->cond_domain_filter,
                domain_matches);
      }
    }
    if (domain_matches && socket != NULL) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_URL) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR(
        debug, socket, "SOCKET",
        kernel_policy->condition_negated_mask & NECP_KERNEL_CONDITION_URL,
        "NECP_KERNEL_CONDITION_URL", kernel_policy->cond_url, url);
    bool url_matches =
        (url ? strcasecmp(kernel_policy->cond_url, url) == 0 : false);
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_PID) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        debug, socket, "SOCKET",
        kernel_policy->condition_negated_mask & NECP_KERNEL_CONDITION_PID,
        "NECP_KERNEL_CONDITION_PID", kernel_policy->cond_pid, pid);
    if (kernel_policy->condition_negated_mask & NECP_KERNEL_CONDITION_PID) {
      ...
      if (kernel_policy->cond_pid_version != 0 &&
          pid_version == kernel_policy->cond_pid_version) {
        return FALSE;
      }
    } else {
      if (pid != kernel_policy->cond_pid) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_UID) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        debug, socket, "SOCKET",
        kernel_policy->condition_negated_mask & NECP_KERNEL_CONDITION_UID,
        "NECP_KERNEL_CONDITION_UID", kernel_policy->cond_uid, uid);
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_REAL_UID) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        debug, socket, "SOCKET",
        kernel_policy->condition_negated_mask & NECP_KERNEL_CONDITION_REAL_UID,
        "NECP_KERNEL_CONDITION_REAL_UID", kernel_policy->cond_real_uid,
        real_uid);
    if (kernel_policy->condition_negated_mask &
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_TRAFFIC_CLASS) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET3(
        debug, socket, "SOCKET", false, "NECP_KERNEL_CONDITION_TRAFFIC_CLASS",
        kernel_policy->cond_traffic_class.start_tc,
        kernel_policy->cond_traffic_class.end_tc, 0, traffic_class, 0, 0);
    if (kernel_policy->condition_negated_mask &
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_PROTOCOL) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        debug, socket, "SOCKET",
        kernel_policy->condition_negated_mask & NECP_KERNEL_CONDITION_PROTOCOL,
        "NECP_KERNEL_CONDITION_PROTOCOL", kernel_policy->cond_protocol,
        protocol);
    if (kernel_policy->condition_negated_mask &
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_AGENT_TYPE) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET_STR3(
        debug, socket, "SOCKET", false, "NECP_KERNEL_CONDITION_AGENT_TYPE",
        kernel_policy->cond_agent_type.agent_domain,
        kernel_policy->cond_agent_type.agent_type, "n/a", "n/a", "n/a", "n/a");
    bool matches_agent_type = FALSE;
    for (u_int32_t i = 0; i < num_required_agent_types; i++) {
      struct necp_client_parameter_netagent_type *required_agent_type =
          &required_agent_types[i];
      if ((strbuflen(kernel_policy->cond_agent_type.agent_domain,
                     sizeof(kernel_policy->cond_agent_type.agent_domain)) ==
               0 ||
           strbufcmp(required_agent_type->netagent_domain,
                     sizeof(required_agent_type->netagent_domain),
                     kernel_policy->cond_agent_type.agent_domain,
                     sizeof(kernel_policy->cond_agent_type.agent_domain)) ==
               0) &&
          (strbuflen(kernel_policy->cond_agent_type.agent_type,
                     sizeof(kernel_policy->cond_agent_type.agent_type)) == 0 ||
           strbufcmp(required_agent_type->netagent_type,
                     sizeof(required_agent_type->netagent_type),
                     kernel_policy->cond_agent_type.agent_type,
                     sizeof(kernel_policy->cond_agent_type.agent_type)) == 0)) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_NETWORKS) {
    bool is_local = FALSE;
    bool include_local_addresses =
        (kernel_policy->cond_local_networks_flags &
         NECP_POLICY_LOCAL_NETWORKS_FLAG_INCLUDE_LOCAL_ADDRESSES);

    if (rt != NULL) {
      is_local =
          IS_NECP_DEST_IN_LOCAL_NETWORKS(rt, remote, include_local_addresses);
    } else {
    ...
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        ...
        kernel_policy->condition_negated_mask &
            NECP_KERNEL_CONDITION_LOCAL_NETWORKS,
        "NECP_KERNEL_CONDITION_LOCAL_NETWORKS", 0, is_local);
    if (kernel_policy->condition_negated_mask &
        NECP_KERNEL_CONDITION_LOCAL_NETWORKS) {
      if (is_local) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_START) {
    if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_END) {
      bool inRange =
          necp_is_addr_in_range(SA(local), SA(&kernel_policy->cond_local_start),
                                SA(&kernel_policy->cond_local_end));
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_LOCAL_END,
          "local address range", 0, 0);
      if (kernel_policy->condition_negated_mask &
          NECP_KERNEL_CONDITION_LOCAL_END) {
        if (inRange) {
          return FALSE;
    ...
    } else if (kernel_policy->condition_mask &
               NECP_KERNEL_CONDITION_LOCAL_PREFIX) {
      bool inSubnet = necp_is_addr_in_subnet(
          SA(local), SA(&kernel_policy->cond_local_start),
          kernel_policy->cond_local_prefix);
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_LOCAL_PREFIX,
          "local address with prefix", 0, 0);
      if (kernel_policy->condition_negated_mask &
          NECP_KERNEL_CONDITION_LOCAL_PREFIX) {
        if (inSubnet) {
          return FALSE;
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_REMOTE_START) {
    if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_REMOTE_END) {
      bool inRange = necp_is_addr_in_range(
          SA(remote), SA(&kernel_policy->cond_remote_start),
          SA(&kernel_policy->cond_remote_end));
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_REMOTE_END,
          "remote address range", 0, 0);
      if (kernel_policy->condition_negated_mask &
          NECP_KERNEL_CONDITION_REMOTE_END) {
        if (inRange) {
          return FALSE;
      ...
      bool inSubnet = necp_is_addr_in_subnet(
          SA(remote), SA(&kernel_policy->cond_remote_start),
          kernel_policy->cond_remote_prefix);
      NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
          debug, socket, "SOCKET",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_REMOTE_PREFIX,
          "remote address with prefix", 0, 0);
      if (kernel_policy->condition_negated_mask &
          NECP_KERNEL_CONDITION_REMOTE_PREFIX) {
        if (inSubnet) {
          return FALSE;
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_CLIENT_FLAGS) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(debug, socket, "SOCKET",
                                         ...
                                         "NECP_KERNEL_CONDITION_CLIENT_FLAGS",
                                         kernel_policy->cond_client_flags,
                                         client_flags);
    if (kernel_policy->condition_negated_mask &
        NECP_KERNEL_CONDITION_CLIENT_FLAGS) {
      if ((client_flags & kernel_policy->cond_client_flags) ==
          kernel_policy->cond_client_flags) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_EMPTY) {
    ...
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(debug, socket, "SOCKET",
                                         ...
                                         "NECP_KERNEL_CONDITION_LOCAL_EMPTY", 0,
                                         isEmpty);
    if (kernel_policy->condition_negated_mask &
        NECP_KERNEL_CONDITION_LOCAL_EMPTY) {
      if (isEmpty) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_SCHEME_PORT) {
    u_int16_t remote_port = 0;
    if ((SA(remote))->sa_family == AF_INET ||
        (SA(remote))->sa_family == AF_INET6) {
  ...
  if (kernel_policy->condition_mask &
    ...
    if (kernel_policy->cond_packet_filter_tags &
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_IS_LOOPBACK) {
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(debug, socket, "SOCKET",
                                         kernel_policy->condition_negated_mask &
                                             NECP_KERNEL_CONDITION_IS_LOOPBACK,
                                         "NECP_KERNEL_CONDITION_IS_LOOPBACK", 0,
                                         is_loopback);
    if (kernel_policy->condition_negated_mask &
        NECP_KERNEL_CONDITION_IS_LOOPBACK) {
      if (is_loopback) {
  ...
  if (kernel_policy->condition_mask &
    ...
    NECP_DATA_TRACE_LOG_CONDITION_SOCKET(
        ...
        kernel_policy->condition_negated_mask &
            NECP_KERNEL_CONDITION_DELEGATE_IS_PLATFORM_BINARY,
        "NECP_KERNEL_CONDITION_DELEGATE_IS_PLATFORM_BINARY", 0,
    ...
    if (is_delegated) {
      ...
      // If the connection is not delegated, and the policy did not specify a
      // particular effective process UUID or PID, check the process directly
      if (kernel_policy->condition_negated_mask &
          NECP_KERNEL_CONDITION_DELEGATE_IS_PLATFORM_BINARY) {
        if (is_platform_binary) {
          return FALSE;
...
necp_socket_calc_flowhash_locked(struct necp_socket_info *info) {
  return net_flowhash(info, sizeof(*info),
                      necp_kernel_socket_policies_gencount);
}

static void necp_socket_fillout_info_locked(
    ...
    soflow_direction_t override_direction, u_int32_t drop_order,
    proc_t *socket_proc, struct necp_socket_info *info, bool is_loopback,
    int input_ifindex) {
  ...
  if (inp->inp_flags2 & INP2_WANT_APP_POLICY &&
      necp_kernel_socket_policies_condition_mask &
          NECP_KERNEL_CONDITION_APP_ID) {
    ...
    if (responsible_application_id > 0) {
      ...
      info->real_application_id = info->application_id;
    } else if (necp_kernel_socket_policies_condition_mask &
               NECP_KERNEL_CONDITION_REAL_APP_ID) {
      struct necp_uuid_id_mapping *real_existing_mapping =
          necp_uuid_lookup_app_id_locked(so->last_uuid);
  ...
  if (necp_kernel_socket_policies_condition_mask &
      NECP_KERNEL_CONDITION_ENTITLEMENT) {
    const task_t __single task =
        proc_task(sock_proc != NULL ? sock_proc : curr_proc);
    info->is_entitled = necp_task_has_match_entitlement(task);
    if (!info->is_entitled) {
      // Task does not have entitlement, check the parent task
      necp_get_parent_is_entitled(task, info);
  ...
  if (necp_kernel_socket_policies_condition_mask & NECP_KERNEL_CONDITION_PID) {
    info->pid_version =
        proc_pidversion(sock_proc != NULL ? sock_proc : curr_proc);
  }
  ...
  if ((necp_kernel_socket_policies_condition_mask &
      ...
      (necp_kernel_socket_policies_condition_mask &
       NECP_KERNEL_CONDITION_DELEGATE_IS_PLATFORM_BINARY)) {
    if (info->pid == 0 ||
        necp_is_platform_binary(sock_proc ? sock_proc : curr_proc)) {
      info->is_platform_binary = true;
  ...
  bool needs_address_for_signature =
      ((necp_kernel_socket_policies_condition_mask &
        NECP_KERNEL_CONDITION_SYSTEM_SIGNED_RESULT) &&
       uuid_is_null(inp->necp_client_uuid) &&
       necp_socket_has_resolver_signature(inp));
  if ((necp_data_tracing_level && necp_data_tracing_port) ||
      necp_restrict_multicast || needs_address_for_signature ||
      (necp_kernel_socket_policies_condition_mask &
       NECP_KERNEL_ADDRESS_TYPE_CONDITIONS) ||
      NEED_DGRAM_FLOW_TRACKING(so)) {
    if (override_local_addr != NULL) {
      if (override_local_addr->sa_family == AF_INET6 &&
          override_local_addr->sa_len <= sizeof(struct sockaddr_in6)) {
        SOCKADDR_COPY(override_local_addr, &info->local_addr,
                      override_local_addr->sa_len);
      ...
      } else if (override_local_addr->sa_family == AF_INET &&
                 override_local_addr->sa_len <= sizeof(struct sockaddr_in)) {
        SOCKADDR_COPY(override_local_addr, &info->local_addr,
                      override_local_addr->sa_len);
      }
    } else {
      if (inp->inp_vflag & INP_IPV6) {
        ...
        memcpy(&SIN6(&info->local_addr)->sin6_addr, &inp->in6p_laddr,
        ...
        memcpy(&SIN(&info->local_addr)->sin_addr, &inp->inp_laddr,
               sizeof(struct in_addr));
      }
    }

    if (override_remote_addr != NULL) {
      if (override_remote_addr->sa_family == AF_INET6 &&
          override_remote_addr->sa_len <= sizeof(struct sockaddr_in6)) {
        SOCKADDR_COPY(override_remote_addr, &info->remote_addr,
                      override_remote_addr->sa_len);
      ...
      } else if (override_remote_addr->sa_family == AF_INET &&
                 override_remote_addr->sa_len <= sizeof(struct sockaddr_in)) {
        SOCKADDR_COPY(override_remote_addr, &info->remote_addr,
                      override_remote_addr->sa_len);
      }
    } else {
      if (inp->inp_vflag & INP_IPV6) {
        ...
        memcpy(&SIN6(&info->remote_addr)->sin6_addr, &inp->in6p_faddr,
               sizeof(struct in6_addr));
      } else if (inp->inp_vflag & INP_IPV4) {
        ...
        memcpy(&SIN(&info->remote_addr)->sin_addr, &inp->inp_faddr,
               sizeof(struct in_addr));
      }
    }
  ...
  if (necp_kernel_socket_policies_condition_mask &
      NECP_KERNEL_CONDITION_SYSTEM_SIGNED_RESULT) {
    // For checking sockets, only validate that there is an NECP client present.
    // It will have already checked for the signature.
    if (!uuid_is_null(inp->necp_client_uuid)) {
      info->has_system_signed_result = true;
    } else {
      info->has_system_signed_result =
          necp_socket_resolver_signature_matches_address(inp,
                                                         &info->remote_addr);
    }
  }
  ...
  if (NEED_DGRAM_FLOW_TRACKING(so)) {
    info->soflow_entry =
        soflow_get_flow(so, NULL, &(info->remote_addr.sa), NULL, 0,
                        override_direction, input_ifindex);
  } else {
  ...
  if (necp_kernel_socket_policies_condition_mask &
    ...
    if (NEED_DGRAM_FLOW_TRACKING(so)) {
      // If the socket has a flow entry for this 4-tuple then check if the flow
      // is outgoing and set the inbound flag accordingly. Otherwise use the
      // direction to set the inbound flag.
      ...
      // If the socket is explicitly marked as inbound then set the inbound
      // flag.
...
#define IS_NECP_KERNEL_POLICY_IP_RESULT(result)                                \
  (result == NECP_KERNEL_POLICY_RESULT_PASS ||                                 \
   result == NECP_KERNEL_POLICY_RESULT_DROP ||                                 \
   result == NECP_KERNEL_POLICY_RESULT_IP_TUNNEL ||                            \
   result == NECP_KERNEL_POLICY_RESULT_ROUTE_RULES)
...
necp_socket_find_policy_match_with_info_locked(
    ...
    necp_kernel_policy_result *return_drop_dest_policy_result,
    necp_drop_all_bypass_check_result_t *return_drop_all_bypass,
    u_int32_t *return_flow_divert_aggregate_unit, struct socket *so,
    int debug) {
  struct necp_kernel_socket_policy *matched_policy = NULL;
  ...
  if (netagent_use_flags_array_count > 0 &&
      netagent_use_flags_array_count < netagent_array_count_adjusted) {
  ...
  if (info->domain != NULL) {
    domain_substring = necp_trim_dots_and_stars(
        __unsafe_null_terminated_to_indexable(info->domain),
        info->domain ? strlen(info->domain) : 0);
    domain_dot_count =
        necp_count_dots(domain_substring.string, domain_substring.length);
  }
  ...
  if (necp_pass_interpose > 0 &&
      info->client_flags & NECP_CLIENT_PARAMETER_FLAG_INTERPOSE) {
    return &pass_policy;
  }
  ...
  *return_drop_dest_policy_result = NECP_KERNEL_POLICY_RESULT_NONE;
  ...
  if (policy_search_array != NULL) {
    for (i = 0; policy_search_array[i] != NULL; i++) {
      ...
      if (necp_drop_all_order != 0 &&
          policy_search_array[i]->session_order >= necp_drop_all_order) {
        ...
        if (drop_all_bypass == NECP_DROP_ALL_BYPASS_CHECK_RESULT_FALSE) {
          NECP_DATA_TRACE_LOG_SOCKET_RESULT(
              debug, so, "SOCKET",
      ...
      if (necp_drop_dest_policy.entry_count != 0 &&
          necp_address_matches_drop_dest_policy(
              &info->remote_addr, policy_search_array[i]->session_order)) {
        // We've hit a drop by destination address rule
        *return_drop_dest_policy_result = NECP_KERNEL_POLICY_RESULT_DROP;
        break;
      }
      if (info->drop_order != 0 &&
          policy_search_array[i]->session_order >= info->drop_order) {
        // We've hit a drop order for this socket
        break;
      }
      if (skip_session_order &&
          policy_search_array[i]->session_order >= skip_session_order) {
        // Done skipping
        skip_order = 0;
        skip_session_order = 0;
        // If we didn't skip any policy with IP result, no need to save the skip
        // for IP evaluation.
        if (skip_policy_id && *skip_policy_id != NECP_KERNEL_POLICY_ID_NONE &&
            !skipped_ip_result) {
          *skip_policy_id = NECP_KERNEL_POLICY_ID_NONE;
          NECP_DATA_TRACE_LOG_POLICY_SOCKET(debug, so, "SOCKET",
      ...
      if (skip_order) {
        if (policy_search_array[i]->order < skip_order) {
          // Skip this policy
          // Remember if we skipped an interesting
          // PASS/DROP/IP_TUNNEL/ROUTE_RULES policy. If we didn't, clear out the
          // return value for skip ID when we are done with each session.'
          ...
          NECP_DATA_TRACE_LOG_POLICY_SOCKET(
              debug, so, "SOCKET", "SKIP (session order < skip-order)");
          continue;
        } else {
          // Done skipping
          skip_order = 0;
        ...
        // Skip this policy
        // Remember if we skipped an interesting PASS/DROP/IP_TUNNEL/ROUTE_RULES
        // policy. If we didn't, clear out the return value for skip ID when we
        // are done with each session.'
        if (IS_NECP_KERNEL_POLICY_IP_RESULT(policy_search_array[i]->result)) {
          skipped_ip_result = true;
          NECP_DATA_TRACE_LOG_POLICY_SOCKET(debug, so, "SOCKET",
                                            "SKIPPING POLICY");
        ...
        NECP_DATA_TRACE_LOG_POLICY_SOCKET(debug, so, "SOCKET",
                                          "SKIP (skip-session-order)");
        continue;
      }

      if (necp_socket_check_policy(
              policy_search_array[i], info->application_id,
              info->real_application_id, info->is_entitled, info->account_id,
              domain_substring, domain_dot_count, info->url, info->pid,
              info->pid_version, info->uid, info->real_uid,
              info->bound_interface_index, info->traffic_class, info->protocol,
              &info->local_addr, &info->remote_addr, required_agent_types,
              num_required_agent_types, info->has_client, info->client_flags,
              info->is_platform_binary, info->has_system_signed_result, proc,
              pf_tag, info->scheme_port, rt, info->is_loopback, debug,
              info->real_is_platform_binary, info->bound_interface_flags,
              info->bound_interface_eflags, info->bound_interface_xflags, info,
              info->is_delegated, so)) {
        if (!debug && necp_data_tracing_session_order) {
          if ((necp_data_tracing_session_order ==
               policy_search_array[i]->session_order) &&
              (!necp_data_tracing_policy_order ||
               (necp_data_tracing_policy_order ==
                policy_search_array[i]->order))) {
            NECP_DATA_TRACE_LOG_SOCKET_RESULT(true, so, "SOCKET",
                                              "DEBUG - MATCHED POLICY");
          }
        }

        if (policy_search_array[i]->result ==
            NECP_KERNEL_POLICY_RESULT_SOCKET_FILTER) {
          if (return_filter && *return_filter != NECP_FILTER_UNIT_NO_FILTER) {
            necp_kernel_policy_filter control_unit =
                policy_search_array[i]->result_parameter.filter_control_unit;
            if (control_unit == NECP_FILTER_UNIT_NO_FILTER) {
              *return_filter = control_unit;
              ...
              if ((*return_filter &&
                  ...
                  (control_unit &&
                   !(control_unit & NECP_MASK_PRESERVE_CONNECTIONS))) {
            ...
            if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
              NECPLOG(
                  LOG_DEBUG,
                  "DATA-TRACE: Socket Policy <so %llx>: (Application %d Real "
                  "Application %d BoundInterface %d Proto %d) Filter %d",
                  (uint64_t)VM_KERNEL_ADDRPERM(so), info->application_id,
                  info->real_application_id, info->bound_interface_index,
                  info->protocol, *return_filter);
            }
          }
          continue;
        } else if (policy_search_array[i]->result ==
                   NECP_KERNEL_POLICY_RESULT_ROUTE_RULES) {
          if (return_route_rule_id_array &&
              route_rule_id_count < route_rule_id_array_count) {
            return_route_rule_id_array[route_rule_id_count++] =
                policy_search_array[i]->result_parameter.route_rule_id;
            if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
              NECPLOG(
                  LOG_DEBUG,
                  "DATA-TRACE: Socket Policy <so %llx>: (Application %d Real "
                  "Application %d BoundInterface %d Proto %d) Route Rule %d",
                  (uint64_t)VM_KERNEL_ADDRPERM(so), info->application_id,
                  info->real_application_id, info->bound_interface_index,
                  info->protocol,
                  policy_search_array[i]->result_parameter.route_rule_id);
          ...
          continue;
                   ...
                   policy_search_array[i]->result ==
                       NECP_KERNEL_POLICY_RESULT_NETAGENT_SCOPED) {
          if (netagent_cursor < netagent_array_count_adjusted) {
            bool agent_already_present = false;
            for (size_t netagent_i = 0; netagent_i < netagent_cursor;
                 netagent_i++) {
              if (netagent_ids[netagent_i] ==
                  policy_search_array[i]->result_parameter.netagent_id) {
                // Already present. Mark the "SCOPED" flag if flags are
                // necessary.
                agent_already_present = true;
                if (!(netagent_use_flags[netagent_i] &
                      NECP_AGENT_USE_FLAG_REMOVE) &&
                    policy_search_array[i]->result ==
                        NECP_KERNEL_POLICY_RESULT_NETAGENT_SCOPED) {
            ...
            if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
              NECPLOG(
                  LOG_DEBUG,
                  "DATA-TRACE: Socket Policy <so %llx>: (Application %d Real "
                  "Application %d BoundInterface %d Proto %d) %s Netagent %d",
                  (uint64_t)VM_KERNEL_ADDRPERM(so), info->application_id,
                  info->real_application_id, info->bound_interface_index,
                  info->protocol,
                  policy_search_array[i]->result ==
          ...
          for (size_t netagent_i = 0; netagent_i < netagent_cursor;
            ...
            if (netagent_ids[netagent_i] ==
                policy_search_array[i]->result_parameter.netagent_id) {
              // Already present. Mark the "REMOVE" flag if flags are supported,
          ...
          if (!agent_already_present &&
              netagent_cursor < netagent_array_count_adjusted) {
            // If not present, and flags are supported, add an entry with the
            // "REMOVE" flag
            netagent_ids[netagent_cursor] =
                policy_search_array[i]->result_parameter.netagent_id;
            netagent_use_flags[netagent_cursor] = NECP_AGENT_USE_FLAG_REMOVE;
          ...
          if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
            NECPLOG(
                LOG_DEBUG,
                "DATA-TRACE: Socket Policy <so %llx>: (Application %d Real "
                "Application %d BoundInterface %d Proto %d) Remove Netagent %d",
                (uint64_t)VM_KERNEL_ADDRPERM(so), info->application_id,
                info->real_application_id, info->bound_interface_index,
                info->protocol,
                policy_search_array[i]->result_parameter.netagent_id);
          ...
          if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
            NECPLOG(LOG_DEBUG,
                    "DATA-TRACE: Socket Policy <so %llx>: (Application %d Real "
                    "Application %d BoundInterface %d Proto %d) Remove "
                    "NetagentType %d",
                    (uint64_t)VM_KERNEL_ADDRPERM(so), info->application_id,
                    info->real_application_id, info->bound_interface_index,
                    info->protocol,
          ...
          continue;
                   ...
                   NECP_KERNEL_POLICY_RESULT_SOCKET_DIVERT) {
          u_int32_t control_unit =
              policy_search_array[i]->result_parameter.flow_divert_control_unit;
          if (control_unit & FLOW_DIVERT_IS_TRANSPARENT) {
            /* For transparent proxies, accumulate the control unit and continue
             * to the next policy */
            if (return_flow_divert_aggregate_unit != NULL) {
              *return_flow_divert_aggregate_unit |=
                  (control_unit & ~FLOW_DIVERT_IS_TRANSPARENT);
              if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
                NECPLOG(
                    LOG_DEBUG,
                    "DATA-TRACE: Socket Policy <so %llx>: (Application %d Real "
                    "Application %d BoundInterface %d Proto %d) flow divert %u",
                    (uint64_t)VM_KERNEL_ADDRPERM(so), info->application_id,
                    info->real_application_id, info->bound_interface_index,
                    info->protocol, control_unit);
              }
            }
            continue;
          }
        ...
        if (policy_search_array[i]->result == NECP_KERNEL_POLICY_RESULT_SKIP) {
          NECP_DATA_TRACE_LOG_POLICY_SOCKET(debug, so, "SOCKET",
                                            "MATCHED SKIP POLICY");
          skip_order =
              policy_search_array[i]->result_parameter.skip_policy_order;
          skip_session_order = policy_search_array[i]->session_order + 1;
          if (skip_policy_id && *skip_policy_id == NECP_KERNEL_POLICY_ID_NONE) {
            *skip_policy_id = policy_search_array[i]->id;
            if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
              NECPLOG(LOG_DEBUG,
                      "DATA-TRACE: Socket Policy <so %llx>: MATCHED SKIP "
                      "POLICY (Application %d Real Application %d "
                      "BoundInterface %d Proto %d) set skip_policy_id %d",
                      (uint64_t)VM_KERNEL_ADDRPERM(so), info->application_id,
                      info->real_application_id, info->bound_interface_index,
                      info->protocol, *skip_policy_id);
            }
          }
          continue;
        }
        ...
        NECP_DATA_TRACE_LOG_SOCKET_RESULT(debug, so, "SOCKET",
  ...
  if (return_netagent_array != NULL) {
    if (return_netagent_use_flags_array != NULL) {
      memcpy(return_netagent_array, &netagent_ids,
             sizeof(u_int32_t) * netagent_array_count_adjusted);
      memcpy(return_netagent_use_flags_array, &netagent_use_flags,
             sizeof(u_int32_t) * netagent_array_count_adjusted);
    } else {
      for (size_t netagent_i = 0; netagent_i < netagent_array_count_adjusted;
           netagent_i++) {
...
static bool necp_socket_uses_interface(struct inpcb *inp,
  ...
  TAILQ_FOREACH(ifa, &interface->if_addrhead, ifa_link) {
    if (ifaddr_address(ifa, SA(&address_storage.sa), sizeof(address_storage)) ==
        0) {
      ...
      if (family == AF_INET) {
        if (memcmp(&address_storage.sin.sin_addr, &inp->inp_laddr,
                   sizeof(inp->inp_laddr)) == 0) {
          found_match = TRUE;
          break;
        }
      } else if (family == AF_INET6) {
        if (memcmp(&address_storage.sin6.sin6_addr, &inp->in6p_laddr,
                   sizeof(inp->in6p_laddr)) == 0) {
          found_match = TRUE;
          break;
        }
      }
...
necp_socket_bypass(struct sockaddr *override_local_addr,
                   struct sockaddr *override_remote_addr, struct inpcb *inp) {
  if (necp_is_loopback(override_local_addr, override_remote_addr, inp, NULL,
                       IFSCOPE_NONE)) {
    proc_t curr_proc = current_proc();
    proc_t sock_proc = NULL;
...
static inline void necp_socket_ip_tunnel_tso(struct inpcb *inp) {
...
static inline void necp_unscope(struct inpcb *inp) {
  // If the current policy result is "socket scoped" and the pcb was actually
  // re-scoped as a result, then un-bind the pcb
...
static inline void necp_clear_tunnel(struct inpcb *inp) {
  if (inp->inp_boundifp != NULL && inp->inp_policyresult.results.result ==
                                       NECP_KERNEL_POLICY_RESULT_IP_TUNNEL) {
    inp->inp_flags &= ~INP_BOUND_IF;
    inp->inp_boundifp = NULL;
  }
...
static inline bool
necp_socket_verify_netagents(u_int32_t *__counted_by(NECP_MAX_NETAGENTS)
                                 netagent_ids,
                             int debug, struct socket *so) {
  // Verify netagents
  for (int netagent_cursor = 0; netagent_cursor < NECP_MAX_NETAGENTS;
       netagent_cursor++) {
    struct necp_uuid_id_mapping *mapping = NULL;
    u_int32_t netagent_id = netagent_ids[netagent_cursor];
    if (netagent_id == 0) {
      ...
      if (agent_flags & NETAGENT_FLAG_REGISTERED) {
        if (agent_flags & NETAGENT_FLAG_ACTIVE) {
          ...
          if (agent_flags & NETAGENT_FLAG_KERNEL_ACTIVATED) {
            ...
            if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
              NECPLOG(LOG_ERR,
                      "DATA-TRACE: Socket Policy: <so %llx> Triggering "
                      "inactive agent (%d), error %d",
                      (uint64_t)VM_KERNEL_ADDRPERM(so), netagent_id,
                      trigger_error);
            }
          }
          return false;
...
necp_kernel_policy_id necp_socket_find_policy_match(
    struct inpcb *inp, struct sockaddr *override_local_addr,
    struct sockaddr *override_remote_addr, u_int32_t override_bound_interface) {
  struct socket *so = NULL;
  necp_kernel_policy_filter filter_control_unit = 0;
  ...
  if (necp_drop_management_order == 0 &&
    ...
    if (necp_drop_all_order > 0 || drop_order > 0) {
      ...
      bypass_type =
          necp_socket_bypass(override_local_addr, override_remote_addr, inp);
      if (bypass_type != NECP_BYPASS_TYPE_NONE &&
          bypass_type != NECP_BYPASS_TYPE_DROP) {
  ...
  necp_socket_fillout_info_locked(
      inp, override_local_addr, override_remote_addr, override_bound_interface,
      SOFLOW_DIRECTION_UNKNOWN, drop_order, &socket_proc, &info,
      (bypass_type == NECP_BYPASS_TYPE_LOOPBACK), 0);
  ...
  if (inp->inp_policyresult.policy_id != NECP_KERNEL_POLICY_ID_NONE &&
      inp->inp_policyresult.policy_gencount ==
          necp_kernel_socket_policies_gencount &&
    ...
    if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECPLOG(
          LOG_DEBUG,
          "DATA-TRACE: Socket Policy - INP UPDATE - RESULT - CACHED <MATCHED>: "
          "%p (BoundInterface %d Proto %d) Policy %d Result %d Parameter %d",
          inp->inp_socket, info.bound_interface_index, info.protocol,
          inp->inp_policyresult.policy_id, inp->inp_policyresult.results.result,
          inp->inp_policyresult.results.result_parameter
              .tunnel_interface_index);
    }
    NECP_DATA_TRACE_LOG_SOCKET(
        debug, so, "SOCKET - INP UPDATE", "RESULT - CACHED <MATCHED>",
        inp->inp_policyresult.policy_id, inp->inp_policyresult.skip_policy_id);
  ...
  matched_policy = necp_socket_find_policy_match_with_info_locked(
      necp_kernel_socket_policies_map[NECP_SOCKET_MAP_APP_ID_TO_BUCKET(
          info.application_id)],
      &info, &filter_control_unit, route_rule_id_array,
      &route_rule_id_array_count, MAX_AGGREGATE_ROUTE_RULES, netagent_ids,
      NECP_MAX_NETAGENTS, NULL, 0, NULL, 0, effective_proc, 0, &skip_policy_id,
  ...
  if (bypass_type == NECP_BYPASS_TYPE_LOOPBACK &&
    ...
    inp->inp_policyresult.results.flow_divert_aggregate_unit =
        flow_divert_aggregate_unit;
    inp->inp_policyresult.results.route_rule_id = 0;
    inp->inp_policyresult.results.result = NECP_KERNEL_POLICY_RESULT_PASS;
    if (info.soflow_entry != NULL) {
    ...
    NECP_DATA_TRACE_LOG_SOCKET(
        debug, so, "SOCKET - INP UPDATE", "RESULT - Loopback PASS",
        inp->inp_policyresult.policy_id, inp->inp_policyresult.skip_policy_id);
    return NECP_KERNEL_POLICY_ID_NONE;
  }
  ...
  if (necp_socket_verify_netagents(netagent_ids, debug, so) == false) {
    if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECPLOG(LOG_ERR,
              "DATA-TRACE: Socket Policy: <so %llx> (BoundInterface %d Proto "
              "%d) Dropping packet because agent is not active",
              (uint64_t)VM_KERNEL_ADDRPERM(so), info.bound_interface_index,
              info.protocol);
    }
    ...
    if (info.soflow_entry != NULL) {
      ...
      info.soflow_entry->soflow_policies_gencount =
          necp_kernel_socket_policies_gencount;
      soflow_free_flow(info.soflow_entry);
    }
    ...
    NECP_DATA_TRACE_LOG_SOCKET(
        debug, so, "SOCKET - INP UPDATE", "RESULT - Inactive Agent DROP",
        inp->inp_policyresult.policy_id, inp->inp_policyresult.skip_policy_id);
    return NECP_KERNEL_POLICY_ID_NONE;
  }
  ...
  if (matched_policy) {
    ...
    if (matched_policy->result == NECP_KERNEL_POLICY_RESULT_PASS &&
      ...
      if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
        NECP_DATA_TRACE_LOG_SOCKET(
            debug, so, "SOCKET - INP UPDATE", "socket unscoped for PASS result",
            inp->inp_policyresult.policy_id, skip_policy_id);
      }
    }
    ...
    memcpy(&inp->inp_policyresult.results.result_parameter,
           &matched_policy->result_parameter,
           sizeof(matched_policy->result_parameter));
    if (info.soflow_entry != NULL) {
      info.soflow_entry->soflow_filter_control_unit = filter_control_unit;
      info.soflow_entry->soflow_policies_gencount =
          necp_kernel_socket_policies_gencount;
    ...
    if (necp_socket_is_connected(inp) &&
        (matched_policy->result == NECP_KERNEL_POLICY_RESULT_DROP ||
         (matched_policy->result == NECP_KERNEL_POLICY_RESULT_IP_TUNNEL &&
          !necp_socket_uses_interface(
              inp, matched_policy->result_parameter.tunnel_interface_index)))) {
    ...
    if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECPLOG(LOG_DEBUG,
              "DATA-TRACE: Socket Policy: <so %llx> (BoundInterface %d Proto "
              "%d) Policy %d Skip %d Result %d Parameter %d Filter %d",
              (uint64_t)VM_KERNEL_ADDRPERM(so), info.bound_interface_index,
              info.protocol, matched_policy->id, skip_policy_id,
              matched_policy->result,
              matched_policy->result_parameter.tunnel_interface_index,
              inp->inp_policyresult.results.filter_control_unit);
    }

    if (matched_policy->result == NECP_KERNEL_POLICY_RESULT_DROP &&
        matched_policy->result_parameter.drop_flags &
            NECP_KERNEL_POLICY_DROP_FLAG_LOCAL_NETWORK &&
        !(matched_policy->result_parameter.drop_flags &
          NECP_KERNEL_POLICY_DROP_FLAG_SUPPRESS_ALERTS)) {
      // Trigger the event that we dropped due to a local network policy
    ...
    u_int32_t flow_divert_control_unit =
        necp_route_get_flow_divert(NULL, netagent_ids, NECP_MAX_NETAGENTS,
                                   route_rule_id, &flow_divert_aggregate_unit);
    if (flow_divert_control_unit != 0) {
      inp->inp_policyresult.policy_id = NECP_KERNEL_POLICY_ID_NO_MATCH;
      inp->inp_policyresult.skip_policy_id = NECP_KERNEL_POLICY_ID_NO_MATCH;
      inp->inp_policyresult.policy_gencount =
      ...
      NECP_DATA_TRACE_LOG_SOCKET(debug, so, "SOCKET - INP UPDATE",
      ...
      necp_unscope(inp);
      if (info.client_flags & NECP_CLIENT_PARAMETER_FLAG_LISTENER ||
          info.is_local) {
        necp_clear_tunnel(inp);
      }
      if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      ...
      inp->inp_policyresult.flowhash = flowhash;
      inp->inp_policyresult.results.filter_control_unit =
          filter_control_unit; // We may have matched a filter, so mark it!
      inp->inp_policyresult.results.flow_divert_aggregate_unit =
          flow_divert_aggregate_unit;
      inp->inp_policyresult.results.route_rule_id =
          route_rule_id; // We may have matched a route rule, so mark it!
  ...
  if (necp_check_missing_client_drop(effective_proc, &info) ||
    ...
    if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECP_DATA_TRACE_LOG_SOCKET(debug, so, "SOCKET - INP UPDATE",
                                 "RESULT - DROP <MISSING CLIENT>", 0, 0);
    }
  }
  ...
  if (send_local_network_denied_event &&
      inp->inp_policyresult.network_denied_notifies == 0) {
...
static bool
necp_ip_output_check_policy(struct necp_kernel_ip_output_policy *kernel_policy,
                            necp_kernel_policy_id socket_policy_id,
                            necp_kernel_policy_id socket_skip_policy_id,
                            u_int32_t bound_interface_index,
                            u_int32_t last_interface_index, u_int16_t protocol,
                            union necp_sockaddr_union *local,
                            union necp_sockaddr_union *remote,
                            struct rtentry *rt, u_int16_t pf_tag, int debug) {
    ...
    if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_BOUND_INTERFACE) {
      u_int32_t cond_bound_interface_index =
          kernel_policy->cond_bound_interface
              ? kernel_policy->cond_bound_interface->if_index
              : 0;
      NECP_DATA_TRACE_LOG_CONDITION_IP(
          debug, "IP",
          kernel_policy->condition_negated_mask &
              NECP_KERNEL_CONDITION_BOUND_INTERFACE,
          "NECP_KERNEL_CONDITION_BOUND_INTERFACE", cond_bound_interface_index,
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_POLICY_ID) {
    necp_kernel_policy_id matched_policy_id =
        kernel_policy->result == NECP_KERNEL_POLICY_RESULT_SKIP
            ...
            : socket_policy_id;
    NECP_DATA_TRACE_LOG_CONDITION_IP3(
        debug, "IP", false, "NECP_KERNEL_CONDITION_POLICY_ID",
        kernel_policy->cond_policy_id, 0, 0, matched_policy_id,
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LAST_INTERFACE) {
    NECP_DATA_TRACE_LOG_CONDITION_IP(
        debug, "IP", false, "NECP_KERNEL_CONDITION_LAST_INTERFACE",
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_PROTOCOL) {
    NECP_DATA_TRACE_LOG_CONDITION_IP(debug, "IP",
                                     kernel_policy->condition_negated_mask &
                                         NECP_KERNEL_CONDITION_PROTOCOL,
                                     "NECP_KERNEL_CONDITION_PROTOCOL",
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_LOCAL_NETWORKS) {
    ...
    NECP_DATA_TRACE_LOG_CONDITION_IP(debug, "IP",
                                     kernel_policy->condition_negated_mask &
                                         NECP_KERNEL_CONDITION_LOCAL_NETWORKS,
                                     "NECP_KERNEL_CONDITION_LOCAL_NETWORKS", 0,
                                     is_local);
    if (kernel_policy->condition_negated_mask &
        NECP_KERNEL_CONDITION_LOCAL_NETWORKS) {
      if (is_local) {
  ...
  if (kernel_policy->condition_mask & NECP_KERNEL_CONDITION_SCHEME_PORT) {
    ...
    NECP_DATA_TRACE_LOG_CONDITION_IP(debug, "IP",
                                     kernel_policy->condition_negated_mask &
                                         NECP_KERNEL_CONDITION_SCHEME_PORT,
                                     "NECP_KERNEL_CONDITION_SCHEME_PORT", 0,
                                     remote_port);
    if (kernel_policy->condition_negated_mask &
        NECP_KERNEL_CONDITION_SCHEME_PORT) {
...
necp_ip_output_find_policy_match_locked(
    necp_kernel_policy_id socket_policy_id,
    necp_kernel_policy_id socket_skip_policy_id,
    u_int32_t bound_interface_index, u_int32_t last_interface_index,
    u_int16_t protocol, union necp_sockaddr_union *local_addr,
    union necp_sockaddr_union *remote_addr, struct rtentry *rt,
    u_int16_t pf_tag, u_int32_t *return_route_rule_id,
    necp_kernel_policy_result *return_drop_dest_policy_result,
    necp_drop_all_bypass_check_result_t *return_drop_all_bypass, int debug) {
  u_int32_t skip_order = 0;
  u_int32_t skip_session_order = 0;
  struct necp_kernel_ip_output_policy *matched_policy = NULL;
  struct necp_kernel_ip_output_policy **policy_search_array =
      necp_kernel_ip_output_policies_map[NECP_IP_OUTPUT_MAP_ID_TO_BUCKET(
          socket_policy_id)];
  u_int32_t route_rule_id_array[MAX_AGGREGATE_ROUTE_RULES];
  size_t route_rule_id_count = 0;
  necp_drop_all_bypass_check_result_t drop_all_bypass =
  ...
  if (policy_search_array != NULL) {
    for (int i = 0; policy_search_array[i] != NULL; i++) {
      ...
      if (necp_drop_all_order != 0 &&
        ...
        if (drop_all_bypass == NECP_DROP_ALL_BYPASS_CHECK_RESULT_FALSE) {
          NECP_DATA_TRACE_LOG_IP_RESULT(
              debug, "IP", "RESULT - DROP (session order > drop-all order)");
          break;
        }
      }
      if (necp_drop_dest_policy.entry_count > 0 &&
          necp_address_matches_drop_dest_policy(
              remote_addr, policy_search_array[i]->session_order)) {
        // We've hit a drop by destination address rule
        *return_drop_dest_policy_result = NECP_KERNEL_POLICY_RESULT_DROP;
        NECP_DATA_TRACE_LOG_IP_RESULT(
            debug, "IP", "RESULT - DROP (destination address rule)");
        break;
      }
      if (skip_session_order &&
          policy_search_array[i]->session_order >= skip_session_order) {
      ...
      if (skip_order) {
        if (policy_search_array[i]->order < skip_order) {
          // Skip this policy
          NECP_DATA_TRACE_LOG_POLICY_IP(debug, "IP",
                                        "SKIP (session order < skip-order)");
          continue;
        } else {
          // Done skipping
          skip_order = 0;
      ...
      if (necp_ip_output_check_policy(
              policy_search_array[i], socket_policy_id, socket_skip_policy_id,
              bound_interface_index, last_interface_index, protocol, local_addr,
              remote_addr, rt, pf_tag, debug)) {
        if (!debug && necp_data_tracing_session_order) {
          if ((necp_data_tracing_session_order ==
               policy_search_array[i]->session_order) &&
              (!necp_data_tracing_policy_order ||
        ...
        if (policy_search_array[i]->result ==
            NECP_KERNEL_POLICY_RESULT_ROUTE_RULES) {
          if (return_route_rule_id != NULL &&
              route_rule_id_count < MAX_AGGREGATE_ROUTE_RULES) {
            route_rule_id_array[route_rule_id_count++] =
                policy_search_array[i]->result_parameter.route_rule_id;
          }
          continue;
        } else if (policy_search_array[i]->result ==
...
static inline bool necp_output_bypass(struct mbuf *packet) {
  if (necp_pass_loopback > 0 &&
      necp_is_loopback(NULL, NULL, NULL, packet, IFSCOPE_NONE)) {
    return true;
  }
...
necp_kernel_policy_id necp_ip_output_find_policy_match(
  ...
  if (necp_kernel_ip_output_policies_count == 0 ||
      (socket_policy_id == NECP_KERNEL_POLICY_ID_NONE &&
       necp_kernel_ip_output_policies_non_id_count == 0 &&
       necp_drop_dest_policy.entry_count == 0)) {
    if (necp_drop_all_order > 0) {
  ...
  int debug = NECP_ENABLE_DATA_TRACE((&local_addr), (&remote_addr), protocol, 0,
                                     bound_interface_index);
  NECP_DATA_TRACE_LOG_IP4(debug, "IP4", "START");

  matched_policy = necp_ip_output_find_policy_match_locked(
      socket_policy_id, socket_skip_policy_id, bound_interface_index,
      last_interface_index, protocol, &local_addr, &remote_addr, rt, pf_tag,
      &route_rule_id, &drop_dest_policy_result, &drop_all_bypass, debug);
  if (matched_policy) {
    matched_policy_id = matched_policy->id;
    if (result) {
    ...
    if (result_parameter) {
      memcpy(result_parameter, &matched_policy->result_parameter,
             sizeof(matched_policy->result_parameter));
    }
    ...
    if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECPLOG(LOG_DEBUG,
              "DATA-TRACE: IP Output: RESULT - MATCHED (ID %d BoundInterface "
              "%d LastInterface %d Proto %d) Policy %d Result %d Parameter %d "
              "Route Rule %u",
              socket_policy_id, bound_interface_index, last_interface_index,
              protocol, matched_policy->id, matched_policy->result,
              matched_policy->result_parameter.tunnel_interface_index,
              route_rule_id);
    }
  } else {
    bool drop_all = false;
    /*
     * Apply drop-all only to packets which have never matched a primary policy
     * (check if the packet saved policy id is none or falls within the socket
     * policy id range).
     */
    ...
    if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECPLOG(LOG_DEBUG,
              "DATA-TRACE: IP Output: RESULT - NO MATCH (ID %d BoundInterface "
              "%d LastInterface %d Proto %d)",
              socket_policy_id, bound_interface_index, last_interface_index,
              protocol);
    }
  }
...
necp_kernel_policy_id necp_ip6_output_find_policy_match(
  ...
  if (matched_policy) {
    ...
    if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECPLOG(LOG_DEBUG,
              "DATA-TRACE: IP6 Output: RESULT - MATCHED (ID %d BoundInterface "
              "%d LastInterface %d Proto %d) Policy %d Result %d Parameter %d "
              "Route Rule %u",
              socket_policy_id, bound_interface_index, last_interface_index,
              protocol, matched_policy->id, matched_policy->result,
    ...
    if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECPLOG(LOG_DEBUG,
              "DATA-TRACE: IP6 Output: RESULT - NO MATCH (ID %d BoundInterface "
              "%d LastInterface %d Proto %d)",
              socket_policy_id, bound_interface_index, last_interface_index,
              protocol);
    }
...
static bool necp_is_range_in_range(struct sockaddr *inner_range_start,
  ...
  if (inner_range_start == NULL || inner_range_end == NULL ||
...
static bool necp_is_addr_in_subnet(struct sockaddr *addr,
  ...
  if (addr->sa_family != subnet_addr->sa_family ||
      addr->sa_len != subnet_addr->sa_len) {
    return FALSE;
  }
  ...
  case AF_INET: {
    ...
    return necp_buffer_compare_with_bit_prefix(
        (u_int8_t *)&satosin(addr)->sin_addr,
        (u_int8_t *)&satosin(subnet_addr)->sin_addr, subnet_prefix);
  }
  case AF_INET6: {
    ...
    if (satosin6(addr)->sin6_scope_id && satosin6(subnet_addr)->sin6_scope_id &&
        satosin6(addr)->sin6_scope_id != satosin6(subnet_addr)->sin6_scope_id) {
      return FALSE;
    }
    return necp_buffer_compare_with_bit_prefix(
        (u_int8_t *)&satosin6(addr)->sin6_addr,
        (u_int8_t *)&satosin6(subnet_addr)->sin6_addr, subnet_prefix);
  }
  default: {
    return FALSE;
  }
...
necp_update_qos_marking(struct ifnet *ifp,
  ...
  if (route_rule->match_netagent_id != 0) {
    ...
    bool found_match = FALSE;
    for (size_t agent_index = 0; agent_index < netagent_array_count;
         agent_index++) {
  ...
  qos_marking = (route_rule->default_action == NECP_ROUTE_RULE_QOS_MARKING)
                    ? TRUE
                    : FALSE;
  ...
  for (exception_index = 0; exception_index < MAX_ROUTE_RULE_INTERFACES;
       exception_index++) {
    if (route_rule->exception_if_indices[exception_index] == 0) {
      break;
    }
    if (route_rule->exception_if_actions[exception_index] !=
        NECP_ROUTE_RULE_QOS_MARKING) {
      continue;
    }
    if (route_rule->exception_if_indices[exception_index] == ifp->if_index) {
      ...
      if (necp_debug > 2) {
        NECPLOG(LOG_DEBUG,
                ...
                route_rule->exception_if_indices[exception_index],
                route_rule_id, qos_marking);
      }
      goto done;
    }
  }

  if ((route_rule->cellular_action == NECP_ROUTE_RULE_QOS_MARKING &&
       IFNET_IS_CELLULAR(ifp)) ||
      (route_rule->wifi_action == NECP_ROUTE_RULE_QOS_MARKING &&
       IFNET_IS_WIFI(ifp)) ||
      (route_rule->wired_action == NECP_ROUTE_RULE_QOS_MARKING &&
       IFNET_IS_WIRED(ifp)) ||
      (route_rule->expensive_action == NECP_ROUTE_RULE_QOS_MARKING &&
       IFNET_IS_EXPENSIVE(ifp)) ||
      (route_rule->constrained_action == NECP_ROUTE_RULE_QOS_MARKING &&
       IFNET_IS_CONSTRAINED(ifp)) ||
      (route_rule->ultra_constrained_action == NECP_ROUTE_RULE_QOS_MARKING &&
       IFNET_IS_ULTRA_CONSTRAINED(ifp)) ||
      (route_rule->companion_action == NECP_ROUTE_RULE_QOS_MARKING &&
       IFNET_IS_COMPANION_LINK(ifp)) ||
      (route_rule->vpn_action == NECP_ROUTE_RULE_QOS_MARKING &&
       IFNET_IS_VPN(ifp))) {
    qos_marking = TRUE;
    if (necp_debug > 2) {
      NECPLOG(LOG_DEBUG,
              "QoS Marking: C:%d WF:%d W:%d E:%d Cn:%d Cmpn:%d VPN:%d UlCn:%d "
              "for Rule %d Allowed %d",
              route_rule->cellular_action, route_rule->wifi_action,
              route_rule->wired_action, route_rule->expensive_action,
              route_rule->constrained_action, route_rule->companion_action,
              route_rule->vpn_action, route_rule->ultra_constrained_action,
              route_rule_id, qos_marking);
    }
    goto done;
  }
  ...
  if (necp_debug > 1) {
    NECPLOG(LOG_DEBUG, "QoS Marking: Rule %d ifp %s Allowed %d", route_rule_id,
            ifp ? ifp->if_xname : "", qos_marking);
  }
  return qos_marking;
}

bool necp_lookup_current_qos_marking(int32_t *qos_marking_gencount,
                                     struct rtentry *route,
  ...
  /*
   * This is racy but we do not need the performance hit of taking
  ...
  /*
   * By default, until we have a interface, do not mark and reevaluate the Qos
   * marking policy
   */
  ...
  if (ROUTE_RULE_IS_AGGREGATE(route_rule_id)) {
    struct necp_aggregate_route_rule *aggregate_route_rule =
    ...
    if (aggregate_route_rule != NULL) {
      ...
      for (index = 0; index < MAX_AGGREGATE_ROUTE_RULES; index++) {
        ...
        new_qos_marking =
            necp_update_qos_marking(ifp, NULL, 0, sub_route_rule_id);
...
void necp_socket_update_qos_marking(struct inpcb *inp, struct rtentry *route,
                                    u_int32_t route_rule_id) {
  bool qos_marking =
      inp->inp_socket->so_flags1 & SOF1_QOSMARKING_ALLOWED ? TRUE : FALSE;
  ...
  qos_marking = necp_lookup_current_qos_marking(
      &(inp->inp_policyresult.results.qos_marking_gencount), route, NULL,
      route_rule_id, qos_marking);
...
static bool necp_route_is_lqm_abort(struct ifnet *ifp,
  ...
  if (delegated_ifp != NULL &&
      (delegated_ifp->if_interface_state.valid_bitmask &
       IF_INTERFACE_STATE_LQM_STATE_VALID) &&
...
static bool necp_route_is_allowed_inner(
    struct rtentry *route, struct ifnet *ifp,
    u_int32_t *__counted_by(netagent_array_count) netagent_array,
    size_t netagent_array_count, u_int32_t route_rule_id,
    u_int32_t *interface_type_denied, bool *ultra_constrained_denied) {
  ...
  default_is_allowed =
      IS_NECP_ROUTE_RULE_DENY(route_rule->default_action) ? FALSE : TRUE;
  if (ifp == NULL && route != NULL) {
    ifp = route->rt_ifp;
  }
  if (ifp == NULL) {
    if (necp_debug > 1 && !default_is_allowed) {
      NECPLOG(LOG_DEBUG,
              "Route Allowed: No interface for route, using default for Rule "
              "%d Allowed %d",
              route_rule_id, default_is_allowed);
    }
  ...
  for (exception_index = 0; exception_index < MAX_ROUTE_RULE_INTERFACES;
    ...
    if (route_rule->exception_if_indices[exception_index] == ifp->if_index ||
      ...
      if (route_rule->exception_if_actions[exception_index] ==
        ...
        return false;
      } else if (IS_NECP_ROUTE_RULE_ALLOW_OR_DENY(
                     route_rule->exception_if_actions[exception_index])) {
        if (necp_debug > 1) {
          NECPLOG(LOG_DEBUG,
                  "Route Allowed: Interface match %d for Rule %d Allowed %d",
                  route_rule->exception_if_indices[exception_index],
                  route_rule_id,
                  (IS_NECP_ROUTE_RULE_DENY(
                       route_rule->exception_if_actions[exception_index])
                       ? FALSE
                       : TRUE));
        }
        if (IS_NECP_ROUTE_RULE_DENY(
                route_rule->exception_if_actions[exception_index]) &&
            route_rule->effective_type != 0 && interface_type_denied != NULL) {
          *interface_type_denied = route_rule->effective_type;
        }
        return IS_NECP_ROUTE_RULE_DENY(
                   route_rule->exception_if_actions[exception_index])
                   ? FALSE
                   : TRUE;
      }
  ...
  if (IFNET_IS_COMPANION_LINK(ifp) ||
      (ifp->if_delegated.ifp != NULL &&
       IFNET_IS_COMPANION_LINK(ifp->if_delegated.ifp))) {
  ...
  if (IFNET_IS_ULTRA_CONSTRAINED(ifp)) {
    if (route_rule->ultra_constrained_action ==
        NECP_ROUTE_RULE_DENY_LQM_ABORT) {
      if (necp_route_is_lqm_abort(ifp, delegated_ifp)) {
        // Mark aggregate action as deny
        type_aggregate_action = NECP_ROUTE_RULE_DENY_INTERFACE;
                   ...
                   route_rule->ultra_constrained_action)) {
      if (ultra_constrained_denied != NULL &&
          IS_NECP_ROUTE_RULE_DENY(route_rule->ultra_constrained_action)) {
  ...
  if (type_aggregate_action != NECP_ROUTE_RULE_NONE) {
    if (necp_debug > 1) {
      NECPLOG(LOG_DEBUG,
              "Route Allowed: C:%d WF:%d W:%d E:%d Cmpn:%d VPN:%d for Rule %d "
              "Allowed %d",
              route_rule->cellular_action, route_rule->wifi_action,
              route_rule->wired_action, route_rule->expensive_action,
              route_rule->companion_action, route_rule->vpn_action,
              route_rule_id,
              (IS_NECP_ROUTE_RULE_DENY(type_aggregate_action) ? FALSE : TRUE));
    }
    return IS_NECP_ROUTE_RULE_DENY(type_aggregate_action) ? FALSE : TRUE;
  }

  if (necp_debug > 1 && !default_is_allowed) {
    NECPLOG(LOG_DEBUG, "Route Allowed: Using default for Rule %d Allowed %d",
            route_rule_id, default_is_allowed);
  }
  return default_is_allowed;
}

static bool necp_proc_is_allowed_on_management_interface(proc_t proc) {
  bool allowed = false;
  const task_t __single task = proc_task(proc);
...
necp_log_interface_not_allowed(struct ifnet *ifp, proc_t proc,
                               struct inpcb *inp) {
  char buf[128];
...
static bool necp_route_is_allowed(
    struct rtentry *route, struct ifnet *interface,
    u_int32_t *__counted_by(netagent_array_count) netagent_array,
    size_t netagent_array_count, u_int32_t route_rule_id,
    u_int32_t *interface_type_denied, bool *ultra_constrained_denied) {
  if ((route == NULL && interface == NULL && netagent_array == NULL) ||
      route_rule_id == 0) {
    if (necp_debug > 1) {
      NECPLOG(LOG_DEBUG,
              "Route Allowed: no route or interface, Rule %d Allowed %d",
              route_rule_id, TRUE);
    }
    return TRUE;
  }
  ...
  if (ROUTE_RULE_IS_AGGREGATE(route_rule_id)) {
    ...
    if (aggregate_route_rule != NULL) {
      ...
      for (index = 0; index < MAX_AGGREGATE_ROUTE_RULES; index++) {
        ...
        if (!necp_route_is_allowed_inner(
                route, interface, netagent_array, netagent_array_count,
                sub_route_rule_id, interface_type_denied,
                ultra_constrained_denied)) {
          return FALSE;
        }
      }
    ...
    return necp_route_is_allowed_inner(
        route, interface, netagent_array, netagent_array_count, route_rule_id,
        interface_type_denied, ultra_constrained_denied);
  }
...
static bool necp_route_rule_matches_agents(u_int32_t route_rule_id) {
  struct necp_route_rule *route_rule =
      necp_lookup_route_rule_locked(&necp_route_rules, route_rule_id);
  if (route_rule == NULL) {
    return false;
  }
...
static uint32_t necp_route_get_netagent(
  ...
  for (int exception_index = 0; exception_index < MAX_ROUTE_RULE_INTERFACES;
       exception_index++) {
    if (route_rule->exception_if_indices[exception_index] == 0) {
      break;
    }
    if (route_rule->exception_if_indices[exception_index] == ifp->if_index &&
      ...
      if (route_rule->exception_if_actions[exception_index] ==
        ...
        *remove = (route_rule->exception_if_actions[exception_index] ==
                   NECP_ROUTE_RULE_REMOVE_NETAGENT);
        return route_rule->netagent_id;
      }
      return 0;
  ...
  if ((ifp->if_xflags & IFXF_CONSTRAINED ||
       ifp->if_xflags & IFXF_ULTRA_CONSTRAINED) &&
  ...
  if ((ifp->if_xflags & IFXF_ULTRA_CONSTRAINED) &&
    ...
    if (route_rule->ultra_constrained_action == NECP_ROUTE_RULE_USE_NETAGENT ||
        route_rule->ultra_constrained_action ==
            NECP_ROUTE_RULE_REMOVE_NETAGENT) {
      *remove = (route_rule->ultra_constrained_action ==
                 NECP_ROUTE_RULE_REMOVE_NETAGENT);
      return route_rule->netagent_id;
    }
    return 0;
...
static uint32_t necp_route_get_flow_divert_inner(
    struct rtentry *route,
    u_int32_t *__counted_by(netagent_array_count) netagent_array,
    size_t netagent_array_count, u_int32_t route_rule_id) {
  struct necp_route_rule *route_rule =
...
necp_route_get_flow_divert(struct rtentry *route,
                           ...
                           size_t netagent_array_count, u_int32_t route_rule_id,
                           u_int32_t *flow_divert_aggregate_unit) {
  if ((route == NULL && netagent_array == NULL) || route_rule_id == 0 ||
      flow_divert_aggregate_unit == NULL) {
    return 0;
  }
  ...
  if (ROUTE_RULE_IS_AGGREGATE(route_rule_id)) {
    ...
    if (aggregate_route_rule != NULL) {
      ...
      for (index = 0; index < MAX_AGGREGATE_ROUTE_RULES; index++) {
        ...
        uint32_t control_unit = necp_route_get_flow_divert_inner(
            route, netagent_array, netagent_array_count, sub_route_rule_id);
        if (control_unit & FLOW_DIVERT_IS_TRANSPARENT) {
          // For transparent proxies, accumulate the control unit and continue
          // to the next route rule
          *flow_divert_aggregate_unit |=
              (control_unit & ~FLOW_DIVERT_IS_TRANSPARENT);
          continue;
        }
    ...
    uint32_t control_unit = necp_route_get_flow_divert_inner(
        route, netagent_array, netagent_array_count, route_rule_id);
    if (control_unit & FLOW_DIVERT_IS_TRANSPARENT) {
      // For transparent proxies, accumulate the control unit and let the caller
      // continue
      *flow_divert_aggregate_unit |=
          (control_unit & ~FLOW_DIVERT_IS_TRANSPARENT);
      return 0;
...
bool necp_packet_is_allowed_over_interface(struct mbuf *packet,
                                           struct ifnet *interface) {
  bool is_allowed = true;
  u_int32_t route_rule_id = necp_get_route_rule_id_from_packet(packet);
  if (route_rule_id != 0 && interface != NULL) {
    lck_rw_lock_shared(&necp_kernel_policy_lock);
    is_allowed = necp_route_is_allowed(
        NULL, interface, NULL, 0, necp_get_route_rule_id_from_packet(packet),
        NULL, NULL);
    lck_rw_done(&necp_kernel_policy_lock);
  }
  return is_allowed;
...
static bool
necp_netagents_allow_traffic(u_int32_t *__counted_by(netagent_id_count)
                                 netagent_ids,
                             size_t netagent_id_count) {
  size_t netagent_cursor;
  for (netagent_cursor = 0; netagent_cursor < netagent_id_count;
       netagent_cursor++) {
    struct necp_uuid_id_mapping *mapping = NULL;
    u_int32_t netagent_id = netagent_ids[netagent_cursor];
    if (netagent_id == 0) {
...
static bool necp_packet_filter_tags_receive(u_int16_t pf_tag,
                                            u_int32_t pass_flags) {
  bool allowed_to_receive = TRUE;

  if (pf_tag == PF_TAG_ID_STACK_DROP &&
      (pass_flags & NECP_KERNEL_POLICY_PASS_PF_TAG) !=
...
static bool necp_socket_is_allowed_to_send_recv_internal(
    struct inpcb *inp, struct sockaddr *override_local_addr,
    struct sockaddr *override_remote_addr, ifnet_t input_interface,
    u_int16_t pf_tag, necp_kernel_policy_id *return_policy_id,
    u_int32_t *return_route_rule_id,
    necp_kernel_policy_id *return_skip_policy_id,
    u_int32_t *return_pass_flags) {
  ...
  // If this socket is connected, or we are not taking addresses into account,
  // try to reuse last result
  if ((necp_socket_is_connected(inp) ||
       (override_local_addr == NULL && override_remote_addr == NULL)) &&
    ...
    if (inp->inp_policyresult.policy_gencount !=
        necp_kernel_socket_policies_gencount) {
      policies_have_changed = TRUE;
    } else {
      if (inp->inp_policyresult.results.route_rule_id != 0) {
        lck_rw_lock_shared(&necp_kernel_policy_lock);
        if (!necp_route_is_allowed(route, input_interface, NULL, 0,
                                   inp->inp_policyresult.results.route_rule_id,
                                   &interface_type_denied, NULL)) {
          route_allowed = FALSE;
        }
        lck_rw_done(&necp_kernel_policy_lock);
      }
    ...
    if (!policies_have_changed) {
      if (debug) {
        necp_socket_fillout_info_locked(
            inp, override_local_addr, override_remote_addr, 0,
            input_interface != NULL ? SOFLOW_DIRECTION_INBOUND
                                    : SOFLOW_DIRECTION_OUTBOUND,
            drop_order, &socket_proc, &info,
            (bypass_type == NECP_BYPASS_TYPE_LOOPBACK), verifyifindex);
        debug = NECP_ENABLE_DATA_TRACE((&info.local_addr), (&info.remote_addr),
                                       info.protocol, info.pid,
                                       info.bound_interface_index);
      }
      ...
      if (!route_allowed ||
          ...
          (inp->inp_policyresult.results.result ==
           ...
           input_interface &&
           inp->inp_policyresult.results.result_parameter
                   .tunnel_interface_index != verifyifindex)) {
        allowed_to_receive = FALSE;
        NECP_DATA_TRACE_LOG_SOCKET_DP(
            debug, so, "SOCKET - DATA PATH", "START - RESULT - CACHED DROP",
            return_policy_id ? *return_policy_id : 0,
            return_skip_policy_id ? *return_skip_policy_id : 0);
      } else {
        if (return_policy_id) {
        ...
        if (inp->inp_policyresult.results.result ==
          ...
          pass_flags =
              inp->inp_policyresult.results.result_parameter.pass_flags;
        }
        NECP_DATA_TRACE_LOG_SOCKET_DP(
            debug, so, "SOCKET - DATA PATH", "START - RESULT - CACHED",
            return_policy_id ? *return_policy_id : 0,
            return_skip_policy_id ? *return_skip_policy_id : 0);
      }
      goto done;
  ...
  if (inp->inp_policyresult.policy_id != NECP_KERNEL_POLICY_ID_NONE &&
    ...
    if (inp->inp_policyresult.results.result ==
        ...
        (inp->inp_policyresult.results.result ==
         ...
         inp->inp_policyresult.results.result_parameter
                 .tunnel_interface_index != verifyifindex) ||
        !necp_route_is_interface_type_allowed(route, input_interface, NULL,
                                              inp) ||
        (inp->inp_policyresult.results.route_rule_id != 0 &&
         !necp_route_is_allowed(route, input_interface, NULL, 0,
                                inp->inp_policyresult.results.route_rule_id,
                                &interface_type_denied, NULL))) {
      allowed_to_receive = FALSE;
      NECP_DATA_TRACE_LOG_SOCKET_DP(
          debug, so, "SOCKET - DATA PATH", "RESULT - CACHED <DROP>",
          return_policy_id ? *return_policy_id : 0,
          return_skip_policy_id ? *return_skip_policy_id : 0);
    } else {
      if (return_policy_id) {
      ...
      if (necp_debug > 1 || NECP_DATA_TRACE_POLICY_ON(debug)) {
        NECPLOG(LOG_DEBUG,
                "DATA-TRACE: Socket Policy - Send/Recv - RESULT - CACHED "
                "<MATCHED>: %p (BoundInterface %d Proto %d) Policy %d Skip %d "
                "Result %d Parameter %d",
                inp->inp_socket, info.bound_interface_index, info.protocol,
                inp->inp_policyresult.policy_id,
                inp->inp_policyresult.skip_policy_id,
                inp->inp_policyresult.results.result,
      ...
      NECP_DATA_TRACE_LOG_SOCKET_DP(
          debug, so, "SOCKET - DATA PATH", "RESULT - CACHED <MATCHED>",
          return_policy_id ? *return_policy_id : 0,
          return_skip_policy_id ? *return_skip_policy_id : 0);
    }
    lck_rw_done(&necp_kernel_policy_lock);
  ...
  struct necp_kernel_socket_policy *matched_policy =
      necp_socket_find_policy_match_with_info_locked(
          ...
          &info, &filter_control_unit, route_rule_id_array,
          &route_rule_id_array_count, MAX_AGGREGATE_ROUTE_RULES, netagent_ids,
          NECP_MAX_NETAGENTS, NULL, 0, NULL, 0, effective_proc, pf_tag,
          return_skip_policy_id, inp->inp_route.ro_rt, &drop_dest_policy_result,
          &drop_all_bypass, &flow_divert_aggregate_unit, so, debug);
  ...
  if (bypass_type == NECP_BYPASS_TYPE_LOOPBACK &&
      ...
      (matched_policy == NULL ||
       matched_policy->result != NECP_KERNEL_POLICY_RESULT_SOCKET_DIVERT)) {
    // If policies haven't changed since last evaluation, do not update filter
    // result in order to preserve the very first filter result for the socket.
    // Otherwise, update the filter result to allow content filter to detect and
    // drop pre-existing flows.
    uint32_t current_filter_control_unit =
        inp->inp_policyresult.results.filter_control_unit;
    int32_t current_policies_gencount = inp->inp_policyresult.policy_gencount;
    if (info.soflow_entry != NULL) {
      current_filter_control_unit =
          info.soflow_entry->soflow_filter_control_unit;
    ...
    if (inp->inp_policyresult.results.flow_divert_aggregate_unit !=
        flow_divert_aggregate_unit) {
      inp->inp_policyresult.results.flow_divert_aggregate_unit =
          flow_divert_aggregate_unit;
    }
    ...
    NECP_DATA_TRACE_LOG_SOCKET_DP(
        debug, so, "SOCKET - DATA PATH", "RESULT - loopback",
        return_policy_id ? *return_policy_id : 0,
        return_skip_policy_id ? *return_skip_policy_id : 0);
    goto done;
  }
  ...
  if (necp_socket_verify_netagents(netagent_ids, debug, so) == false) {
    ...
    NECP_DATA_TRACE_LOG_SOCKET_DP(
        debug, so, "SOCKET - DATA PATH", "RESULT - AGENT INACTIVE",
        return_policy_id ? *return_policy_id : 0,
        return_skip_policy_id ? *return_skip_policy_id : 0);
...
skip_agent_check:
  ...
  if (matched_policy != NULL) {
    ...
    if (matched_policy->result == NECP_KERNEL_POLICY_RESULT_DROP ||
        ...
        !necp_route_is_interface_type_allowed(route, input_interface, NULL,
                                              inp) ||
        (route_rule_id != 0 &&
         !necp_route_is_allowed(route, input_interface, netagent_ids,
                                NECP_MAX_NETAGENTS, route_rule_id,
                                &interface_type_denied, NULL)) ||
      ...
      // If policies haven't changed since last evaluation, do not update filter
      // result in order to preserve the very first filter result for the
      // socket.  Otherwise, update the filter result to allow content filter to
      // detect and drop pre-existing flows.
      uint32_t current_filter_control_unit =
          inp->inp_policyresult.results.filter_control_unit;
      int32_t current_policies_gencount = inp->inp_policyresult.policy_gencount;
      if (info.soflow_entry != NULL) {
    ...
    if ((necp_debug > 1 &&
         matched_policy->id != inp->inp_policyresult.policy_id) ||
        NECP_DATA_TRACE_POLICY_ON(debug)) {
      NECPLOG(LOG_DEBUG,
              "DATA-TRACE: Socket Policy - Send/Recv: %p (BoundInterface %d "
              "Proto %d) Policy %d Result %d Parameter %d Allowed %d "
              "<filter_control_unit %d flow_divert_aggregate_unit %d>",
              inp->inp_socket, info.bound_interface_index, info.protocol,
              matched_policy->id, matched_policy->result,
              matched_policy->result_parameter.tunnel_interface_index,
              allowed_to_receive, filter_control_unit,
              flow_divert_aggregate_unit);
    }
  } else {
    bool drop_all = false;
    ...
    if (drop_all &&
        drop_all_bypass == NECP_DROP_ALL_BYPASS_CHECK_RESULT_FALSE) {
      allowed_to_receive = FALSE;
      NECP_DATA_TRACE_LOG_SOCKET_DP(debug, so, "SOCKET - DATA PATH",
                                    "RESULT - DROP - NO MATCH", 0, 0);
    } else {
      if (return_policy_id) {
      ...
      NECP_DATA_TRACE_LOG_SOCKET_DP(
          debug, so, "SOCKET - DATA PATH", "RESULT - NO MATCH",
          return_policy_id ? *return_policy_id : 0,
          return_skip_policy_id ? *return_skip_policy_id : 0);
    }
  }
  ...
  if (!allowed_to_receive &&
      interface_type_denied != IFRTYPE_FUNCTIONAL_UNKNOWN) {
...
bool necp_socket_is_allowed_to_send_recv_v4(
  ...
  return necp_socket_is_allowed_to_send_recv_internal(
      inp, SA(&local), SA(&remote), input_interface, pf_tag, return_policy_id,
      return_route_rule_id, return_skip_policy_id, return_pass_flags);
}

bool necp_socket_is_allowed_to_send_recv_v6(
    struct inpcb *inp, u_int16_t local_port, u_int16_t remote_port,
...
bool necp_socket_is_allowed_to_send_recv(
    struct inpcb *inp, ifnet_t input_interface, u_int16_t pf_tag,
    necp_kernel_policy_id *return_policy_id, u_int32_t *return_route_rule_id,
    necp_kernel_policy_id *return_skip_policy_id,
    u_int32_t *return_pass_flags) {
  return necp_socket_is_allowed_to_send_recv_internal(
      inp, NULL, NULL, input_interface, pf_tag, return_policy_id,
      return_route_rule_id, return_skip_policy_id, return_pass_flags);
}

int necp_mark_packet_from_socket(struct mbuf *packet, struct inpcb *inp,
  ...
  if (NECP_DATA_TRACE_DP_ON(necp_data_tracing_level)) {
    NECP_DATA_TRACE_LOG_SOCKET_BRIEF(
        TRUE, inp->inp_socket, "SOCKET", "START - MARK PACKET", policy_id,
        skip_policy_id, inp->inp_policyresult.policy_id,
        inp->inp_policyresult.skip_policy_id);
  }
  ...
  } else if (inp->inp_policyresult.results.result ==
                 NECP_KERNEL_POLICY_RESULT_PASS ||
             inp->inp_policyresult.results.result ==
                 NECP_KERNEL_POLICY_RESULT_IP_TUNNEL) {
    packet->m_pkthdr.necp_mtag.necp_policy_id = inp->inp_policyresult.policy_id;
  } else if (inp->inp_policyresult.policy_id ==
  ...
  if (route_rule_id != 0) {
    ...
    packet->m_pkthdr.necp_mtag.necp_route_rule_id =
        inp->inp_policyresult.results.route_rule_id;
  }
  packet->m_pkthdr.necp_mtag.necp_app_id =
      (inp->inp_policyresult.app_id >= UINT16_MAX
           ? (inp->inp_policyresult.app_id - UINT16_MAX)
  ...
  } else if (inp->inp_policyresult.skip_policy_id !=
                 NECP_KERNEL_POLICY_ID_NONE &&
             inp->inp_policyresult.skip_policy_id !=
                 NECP_KERNEL_POLICY_ID_NO_MATCH) {
    packet->m_pkthdr.necp_mtag.necp_skip_policy_id =
        inp->inp_policyresult.skip_policy_id;
  } else if (inp->inp_policyresult.results.filter_control_unit ==
  ...
  if (((pass_flags & NECP_KERNEL_POLICY_PASS_PF_TAG) ==
       NECP_KERNEL_POLICY_PASS_PF_TAG) ||
      ((inp->inp_policyresult.results.result_parameter.pass_flags &
        NECP_KERNEL_POLICY_PASS_PF_TAG) == NECP_KERNEL_POLICY_PASS_PF_TAG)) {
    m_pftag(packet)->pftag_tag = PF_TAG_ID_SYSTEM_SERVICE;
  }

  if (NECP_DATA_TRACE_DP_ON(necp_data_tracing_level)) {
    NECP_DATA_TRACE_LOG_SOCKET_BRIEF(
        TRUE, inp->inp_socket, "SOCKET", "RESULT - MARK PACKET",
        packet->m_pkthdr.necp_mtag.necp_policy_id,
        packet->m_pkthdr.necp_mtag.necp_skip_policy_id, 0, 0);
  }

  return 0;
}

int necp_mark_packet_from_ip(struct mbuf *packet,
                             necp_kernel_policy_id policy_id) {
...
int necp_mark_packet_from_ip_with_skip(struct mbuf *packet,
                                       necp_kernel_policy_id policy_id,
                                       necp_kernel_policy_id skip_policy_id) {
  if (packet == NULL || !(packet->m_flags & M_PKTHDR)) {
    return EINVAL;
...
necp_kernel_policy_id necp_get_skip_policy_id_from_packet(struct mbuf *packet) {
  if (packet == NULL || !(packet->m_flags & M_PKTHDR)) {
    return NECP_KERNEL_POLICY_ID_NONE;
  }

  // Check for overloaded value. See necp_mark_packet_from_socket().
  if (packet->m_pkthdr.necp_mtag.necp_skip_policy_id ==
      NECP_KERNEL_POLICY_ID_NO_MATCH) {
    return NECP_KERNEL_POLICY_ID_NONE;
  }
...
u_int16_t necp_get_packet_filter_tags_from_packet(struct mbuf *packet) {
  if (packet == NULL || !(packet->m_flags & M_PKTHDR)) {
    return 0;
  }
...
bool necp_packet_should_skip_filters(struct mbuf *packet) {
  if (packet == NULL || !(packet->m_flags & M_PKTHDR)) {
    return false;
  }
...
int necp_get_app_uuid_from_packet(struct mbuf *packet, uuid_t app_uuid) {
  ...
  if (packet->m_pkthdr.necp_mtag.necp_app_id != 0) {
    lck_rw_lock_shared(&necp_kernel_policy_lock);
    necp_app_id app_id =
        (packet->m_pkthdr.necp_mtag.necp_app_id < UINT16_MAX
             ? (packet->m_pkthdr.necp_mtag.necp_app_id + UINT16_MAX)
             : packet->m_pkthdr.necp_mtag.necp_app_id);
    struct necp_uuid_id_mapping *entry =
...
u_int32_t necp_socket_get_policy_gencount(struct socket *so) {
...
u_int32_t necp_socket_get_flow_divert_control_unit(struct inpcb *inp,
  ...
  if (inp->inp_policyresult.results.result ==
      NECP_KERNEL_POLICY_RESULT_SOCKET_DIVERT) {
    return inp->inp_policyresult.results.result_parameter
        .flow_divert_control_unit;
  }
...
bool necp_socket_should_rescope(struct inpcb *inp) {
  ...
  return inp->inp_policyresult.results.result ==
             NECP_KERNEL_POLICY_RESULT_SOCKET_SCOPED ||
         inp->inp_policyresult.results.result ==
             NECP_KERNEL_POLICY_RESULT_SCOPED_DIRECT;
}
...
u_int32_t necp_socket_get_effective_mtu(struct inpcb *inp,
  ...
  if (inp->inp_policyresult.results.result ==
          NECP_KERNEL_POLICY_RESULT_IP_TUNNEL &&
      (inp->inp_flags & INP_BOUND_IF) && inp->inp_boundifp) {
    u_int bound_interface_index = inp->inp_boundifp->if_index;
    u_int tunnel_interface_index =
        inp->inp_policyresult.results.result_parameter.tunnel_interface_index;

    // The result is IP Tunnel, and is rescoping from one interface to another.
    // Recalculate MTU.
    if (bound_interface_index != tunnel_interface_index) {
      ...
      if (tunnel_interface != NULL) {
        ...
        u_int32_t delegate_tunnel_mtu =
            (tunnel_interface->if_delegated.ifp != NULL)
                ? tunnel_interface->if_delegated.ifp->if_mtu
                : 0;
        const char ipsec_prefix[] = "ipsec";
        if (delegate_tunnel_mtu != 0 &&
            strlcmp(ipsec_prefix, tunnel_interface->if_name,
                    sizeof(ipsec_prefix)) == 0) {
          // For ipsec interfaces, calculate the overhead from the delegate
          // interface
          u_int32_t tunnel_overhead =
              (u_int32_t)(esp_hdrsiz(NULL) + sizeof(struct ip6_hdr));
          if (delegate_tunnel_mtu > tunnel_overhead) {
            delegate_tunnel_mtu -= tunnel_overhead;
          }

          if (delegate_tunnel_mtu < direct_tunnel_mtu) {
            // If the (delegate - overhead) < direct, return (delegate -
            // overhead)
...
ifnet_t necp_get_ifnet_from_result_parameter(
    necp_kernel_policy_result_parameter *result_parameter) {
...
bool necp_packet_can_rebind_to_ifnet(struct mbuf *packet,
                                     struct ifnet *interface,
                                     struct route *new_route, int family) {
  bool found_match = FALSE;
  bool can_rebind = FALSE;
  ifaddr_t ifa;
  union necp_sockaddr_union address_storage;

  if (packet == NULL || interface == NULL || new_route == NULL ||
  ...
  TAILQ_FOREACH(ifa, &interface->if_addrhead, ifa_link) {
    if (ifaddr_address(ifa, SA(&address_storage.sa), sizeof(address_storage)) ==
      ...
      if (family == AF_INET) {
        ...
        if (memcmp(&address_storage.sin.sin_addr, &ip->ip_src,
                   sizeof(ip->ip_src)) == 0) {
          found_match = TRUE;
          break;
        }
        ...
        if (memcmp(&address_storage.sin6.sin6_addr, &ip6->ip6_src,
  ...
  // If source address matched, attempt to construct a route to the destination
  // address
  if (found_match) {
...
static bool necp_addr_is_loopback(struct sockaddr *address) {
  if (address == NULL) {
    return FALSE;
  }
  ...
  if (address->sa_family == AF_INET) {
    ...
    if (!IN6_IS_ADDR_V4MAPPED(&SIN6(address)->sin6_addr)) {
      ...
      return *(const __uint32_t *)(const void *)(&in6_addr_ptr->s6_addr[12]) ==
...
static bool necp_is_loopback(struct sockaddr *local_addr,
  ...
  if (inp != NULL) {
    if ((inp->inp_flags & INP_BOUND_IF) && inp->inp_boundifp &&
        (inp->inp_boundifp->if_flags & IFF_LOOPBACK)) {
      return TRUE;
    }
    if (inp->inp_vflag & INP_IPV4) {
      ...
      if (IN6_IS_ADDR_LOOPBACK(&inp->in6p_laddr) ||
        ...
        return TRUE;
             ...
             lo_ifp->if_index == bound_interface_index) {
    return TRUE;
  }
...
static bool
necp_address_matches_drop_dest_policy(union necp_sockaddr_union *sau,
                                      u_int32_t session_order) {
  ...
  if (necp_drop_dest_debug > 0) {
    if (sau->sa.sa_family == AF_INET) {
      ...
      (void)inet_ntop(AF_INET6, &sau->sin6.sin6_addr, dest_str,
                      sizeof(dest_str));
    } else {
      dest_str[0] = 0;
    }
  ...
  for (u_int32_t i = 0; i < necp_drop_dest_policy.entry_count; i++) {
    struct necp_drop_dest_entry *necp_drop_dest_entry =
        &necp_drop_dest_policy.entries[i];
    struct necp_policy_condition_addr *npca = &necp_drop_dest_entry->cond_addr;

    if (session_order >= necp_drop_dest_entry->order &&
        necp_is_addr_in_subnet(SA(&sau->sa), SA(&npca->address.sa),
                               npca->prefix)) {
      if (necp_drop_dest_debug > 0) {
        ...
        if (sau->sa.sa_family == AF_INET) {
          (void)inet_ntop(AF_INET, &npca->address.sin, subnet_str,
                          sizeof(subnet_str));
          os_log(OS_LOG_DEFAULT, "%s (process %s:%u) %s matches %s/%u",
                 __func__, proc_best_name(p), pid, dest_str, subnet_str,
                 npca->prefix);
        } else if (sau->sa.sa_family == AF_INET6) {
          (void)inet_ntop(AF_INET6, &npca->address.sin6, subnet_str,
                          sizeof(subnet_str));
          os_log(OS_LOG_DEFAULT, "%s (process %s:%u) %s matches %s/%u",
                 __func__, proc_best_name(p), pid, dest_str, subnet_str,
                 npca->prefix);
  ...
  if (necp_drop_dest_debug > 1) {
    ...
    os_log(OS_LOG_DEFAULT, "%s (process %s:%u) %s no match", __func__,
           proc_best_name(p), pid, dest_str);
  }
  return false;
}
...
#pragma unused(arg1, arg2, oidp)
  ...
  if (req->newptr != USER_ADDR_NULL && proc_suser(current_proc()) != 0 &&
      priv_check_cred(kauth_cred_get(), PRIV_NET_PRIVILEGED_NECP_POLICIES, 0) !=
          0) {
    NECPLOG(LOG_ERR, "%s (process %s:%u) not permitted", __func__,
            proc_best_name(p), pid);
    return EPERM;
  }
  if (req->newptr != USER_ADDR_NULL &&
      req->newlen != sizeof(struct necp_drop_dest_policy)) {
    NECPLOG(LOG_ERR, "%s (process %s:%u) bad newlen %lu", __func__,
            proc_best_name(p), pid, req->newlen);
    return EINVAL;
  }

  memcpy(&tmp_drop_dest_policy, &necp_drop_dest_policy,
         sizeof(struct necp_drop_dest_policy));
  error = sysctl_io_opaque(req, &tmp_drop_dest_policy,
                           sizeof(struct necp_drop_dest_policy), &changed);
  if (error != 0) {
    NECPLOG(LOG_ERR, "%s (process %s:%u) sysctl_io_opaque() error %d", __func__,
            proc_best_name(p), pid, error);
    return error;
  }
  ...
  if (tmp_drop_dest_policy.entry_count >= MAX_NECP_DROP_DEST_LEVEL_ADDRS) {
    NECPLOG(LOG_ERR, "%s (process %s:%u) bad entry_count %u", __func__,
            proc_best_name(p), pid, tmp_drop_dest_policy.entry_count);
    return EINVAL;
  }
  for (u_int32_t i = 0; i < tmp_drop_dest_policy.entry_count; i++) {
    struct necp_drop_dest_entry *tmp_drop_dest_entry =
        &tmp_drop_dest_policy.entries[i];
    ...
    case NECP_SESSION_PRIORITY_UNKNOWN:
      if (tmp_drop_dest_policy.entry_count != 0) {
        NECPLOG(LOG_ERR,
                "%s (process %s:%u) NECP_SESSION_PRIORITY_UNKNOWN bad "
                "entry_count %u",
                __func__, proc_best_name(p), pid,
                tmp_drop_dest_policy.entry_count);
        return EINVAL;
      }
      break;
    ...
    case NECP_SESSION_PRIORITY_LOW:
      if (tmp_drop_dest_policy.entry_count == 0) {
        NECPLOG(LOG_ERR, "%s (process %s:%u) priority %u entry_count 0",
                __func__, proc_best_name(p), pid, tmp_drop_dest_entry->level);
        return EINVAL;
      }
      break;
    default: {
      NECPLOG(LOG_ERR, "%s (process %s:%u) bad level %u", __func__,
              proc_best_name(p), pid, tmp_drop_dest_entry->level);
      return EINVAL;
    }
    }
    ...
    case AF_INET: {
      if (npca->prefix > 32) {
        NECPLOG(LOG_ERR, "%s (process %s:%u) AF_INET bad prefix %u", __func__,
                proc_best_name(p), pid, npca->prefix);
        return EINVAL;
      }
      if (npca->address.sin.sin_len != sizeof(struct sockaddr_in)) {
        NECPLOG(LOG_ERR, "%s (process %s:%u) AF_INET bad sin_len %u", __func__,
                proc_best_name(p), pid, npca->address.sin.sin_len);
        return EINVAL;
      }
      if (npca->address.sin.sin_port != 0) {
        NECPLOG(LOG_ERR, "%s (process %s:%u) AF_INET bad sin_port %u, not zero",
                __func__, proc_best_name(p), pid, npca->address.sin.sin_port);
        return EINVAL;
      }
      break;
    }
    case AF_INET6: {
      if (npca->prefix > 128) {
        NECPLOG(LOG_ERR, "%s (process %s:%u) AF_INET6 bad prefix %u", __func__,
                proc_best_name(p), pid, npca->prefix);
        return EINVAL;
      }
      if (npca->address.sin6.sin6_len != sizeof(struct sockaddr_in6)) {
        NECPLOG(LOG_ERR, "%s (process %s:%u) AF_INET6 bad sin6_len %u",
                __func__, proc_best_name(p), pid, npca->address.sin6.sin6_len);
        return EINVAL;
      }
      if (npca->address.sin6.sin6_port != 0) {
        NECPLOG(LOG_ERR,
                "%s (process %s:%u) AF_INET6 bad sin6_port %u, not zero",
                __func__, proc_best_name(p), pid, npca->address.sin6.sin6_port);
        return EINVAL;
      }
      if (npca->address.sin6.sin6_flowinfo != 0) {
        NECPLOG(LOG_ERR,
                "%s (process %s:%u) AF_INET6 bad sin6_flowinfo %u, not zero",
                __func__, proc_best_name(p), pid,
                npca->address.sin6.sin6_flowinfo);
        return EINVAL;
      }
      if (npca->address.sin6.sin6_scope_id != 0) {
        NECPLOG(LOG_ERR,
                "%s (process %s:%u) AF_INET6 bad sin6_scope_id %u, not zero",
                __func__, proc_best_name(p), pid,
                npca->address.sin6.sin6_scope_id);
        return EINVAL;
      }
      break;
    }
  ...
  for (u_int32_t i = 0; i < tmp_drop_dest_policy.entry_count; i++) {
    ...
    memcpy(necp_drop_dest_entry, tmp_drop_dest_entry,
           sizeof(struct necp_drop_dest_entry));

    necp_drop_dest_entry->order =
        necp_get_first_order_for_priority(necp_drop_dest_entry->level);
  }
  lck_rw_done(&necp_kernel_policy_lock);
...
const char *necp_get_address_string(union necp_sockaddr_union *address,
  ...
  if (fam == AF_INET) {
    (void)inet_ntop(AF_INET, &address->sin.sin_addr, addr_str,
                    MAX_IPv6_STR_LEN);
  } else if (fam == AF_INET6) {
    (void)inet_ntop(AF_INET6, &address->sin6.sin6_addr, addr_str,
                    MAX_IPv6_STR_LEN);
  }
