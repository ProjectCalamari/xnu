...
#define FAKE_LOG(__level, __dbgf, __string, ...)                               \
  do {                                                                         \
    if (__level <= if_fake_log_level || FAKE_DBGF_ENABLED(__dbgf)) {           \
      os_log(OS_LOG_DEFAULT, "%s: " __string, __func__, ##__VA_ARGS__);        \
    }                                                                          \
  } while (0)

static boolean_t is_power_of_two(unsigned int val) {
  return (val & (val - 1)) == 0;
}
...
SYSCTL_INT(_net_link_fake, OID_AUTO, multibuflet, CTLFLAG_RW | CTLFLAG_LOCKED,
           &if_fake_multibuflet, 0,
           "Fake interface using multi-buflet packets");
...
SYSCTL_INT(_net_link_fake, OID_AUTO, tso_support, CTLFLAG_RW | CTLFLAG_LOCKED,
           &if_fake_tso_support, 0,
           "Fake interface with support for TSO offload");

#define DEFAULT_EXPIRATION_THRESHOLD 500 /* usec */
static int if_fake_expiration_threshold_us = DEFAULT_EXPIRATION_THRESHOLD;
SYSCTL_INT(_net_link_fake, OID_AUTO, expiration_threshold,
           CTLFLAG_RW | CTLFLAG_LOCKED, &if_fake_expiration_threshold_us,
...
SYSCTL_INT(_net_link_fake, OID_AUTO, separate_frame_header,
           CTLFLAG_RW | CTLFLAG_LOCKED, &if_fake_separate_frame_header, 0,
...
SYSCTL_INT(
    _net_link_fake, OID_AUTO, pktpool_mode, CTLFLAG_RW | CTLFLAG_LOCKED,
...
SYSCTL_INT(_net_link_fake, OID_AUTO, rx_flow_steering_support,
           CTLFLAG_RW | CTLFLAG_LOCKED, &if_fake_rx_flow_steering_support, 0,
...
static int feth_link_layer_aggregation_factor_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  ...
  error = sysctl_io_number(req, if_fake_link_layer_aggregation_factor,
                           sizeof(if_fake_link_layer_aggregation_factor),
                           &new_value, &changed);
  if (error == 0 && changed != 0) {
    if (new_value <= 0 || new_value > FETH_LINK_LAYER_AGGRETATION_FACTOR_MAX) {
      return EINVAL;
    }
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, link_layer_aggregation_factor,
            ...
            feth_link_layer_aggregation_factor_sysctl, "IU",
...
static int feth_tx_headroom_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
  ...
  if (error == 0 && changed != 0) {
    if (new_value > FETH_TX_HEADROOM_MAX || (new_value % 8) != 0) {
      return EINVAL;
    }
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, tx_headroom,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            feth_tx_headroom_sysctl, "IU", "Fake ethernet Tx headroom");
...
static int feth_trailer_length_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
  ...
  error =
      sysctl_io_number(req, if_fake_trailer_length,
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, trailer_length,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            feth_trailer_length_sysctl, "IU",
            "Fake interface frame trailer length");
...
static int feth_tso_buffer_size_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
  ...
  error =
      sysctl_io_number(req, if_fake_tso_buffer_size,
                       sizeof(if_fake_tso_buffer_size), &new_value, &changed);
  if (error == 0 && changed != 0) {
    /* must be a power of 2 between min and max */
    if (new_value > FETH_BUFLET_SIZE_MAX || new_value < FETH_BUFLET_SIZE_MIN ||
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, tso_buf_size,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            feth_tso_buffer_size_sysctl, "IU",
            "Fake interface TSO buffer size");

static int feth_max_mtu_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;

  error = sysctl_io_number(req, if_fake_max_mtu, sizeof(if_fake_max_mtu),
                           &new_value, &changed);
  if (error == 0 && changed != 0) {
    if (new_value > FETH_MAX_MTU_MAX || new_value < ETHERMTU ||
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, max_mtu,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            feth_max_mtu_sysctl, "IU", "Fake interface maximum MTU");

static int feth_buflet_size_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
  ...
  if (error == 0 && changed != 0) {
    /* must be a power of 2 between min and max */
    if (new_value > FETH_BUFLET_SIZE_MAX || new_value < FETH_BUFLET_SIZE_MIN ||
        !is_power_of_two(new_value) || new_value >= if_fake_max_mtu) {
      return EINVAL;
    }
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, buflet_size,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            feth_buflet_size_sysctl, "IU", "Fake interface buflet size");

static unsigned int if_fake_user_access = 0;

static int feth_user_access_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, user_access,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            feth_user_access_sysctl, "IU", "Fake interface user access");
...
static int feth_if_adv_interval_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
  ...
  error =
      sysctl_io_number(req, if_fake_if_adv_interval,
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, if_adv_intvl,
            ...
            "Fake interface will generate interface advisories reports at the "
...
static int feth_fake_tx_drops_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;

  error = sysctl_io_number(req, if_fake_tx_drops, sizeof(if_fake_tx_drops),
                           &new_value, &changed);
  if (error == 0 && changed != 0) {
    if (new_value > FETH_TX_DROPS_MAX || new_value < FETH_TX_DROPS_MIN) {
      return EINVAL;
    }
...
typedef enum {
  ...
  IFF_TX_EXP_POLICY_DROP_AND_NOTIFY =
      1, /* Expiry notification enabled; drop + notify mode */
  IFF_TX_EXP_POLICY_NOTIFY_ONLY =
      2, /* Expiry notification enabled; notify only mode */
  IFF_TX_EXP_POLICY_METADATA =
      3, /* Expiry notification enabled; use packet metadata */
} iff_tx_exp_policy_t;
static iff_tx_exp_policy_t if_fake_tx_exp_policy = IFF_TX_EXP_POLICY_DISABLED;

static int feth_fake_tx_exp_policy_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
  ...
  FAKE_LOG(LOG_DEBUG, FE_DBGF_CONTROL, "if_fake_tx_exp_policy: %u -> %u (%d)",
...
SYSCTL_PROC(
    _net_link_fake, OID_AUTO, tx_exp_policy,
    CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
...
static int feth_fake_tx_completion_mode_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
...
SYSCTL_PROC(
    _net_link_fake, OID_AUTO, tx_completion_mode,
    CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
...
static int feth_fake_llink_cnt_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;

  error = sysctl_io_number(req, if_fake_llink_cnt, sizeof(if_fake_llink_cnt),
                           &new_value, &changed);
  if (error == 0 && changed != 0) {
...
static int feth_fake_qset_cnt_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;

  error = sysctl_io_number(req, if_fake_qset_cnt, sizeof(if_fake_qset_cnt),
                           &new_value, &changed);
  if (error == 0 && changed != 0) {
    if (new_value == 0 || new_value > FETH_MAX_QSETS) {
      return EINVAL;
    }
...
SYSCTL_PROC(_net_link_fake, OID_AUTO, qset_cnt,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            feth_fake_qset_cnt_sysctl, "IU", "Fake interface queue set count");
...
static inline void *__indexable get_bpf_header(mbuf_t m,
...
static void fake_bpf_tap_common(ifnet_t ifp, mbuf_t m,
                                struct ether_header *eh_p, _tap_func func) {
...
static inline void fake_bpf_tap_in(ifnet_t ifp, mbuf_t m,
                                   struct ether_header *eh_p) {
  fake_bpf_tap_common(ifp, m, eh_p, bpf_tap_in);
}

static inline void fake_bpf_tap_out(ifnet_t ifp, mbuf_t m,
                                    struct ether_header *eh_p) {
...
static if_fake_ref ifnet_get_if_fake(ifnet_t ifp);

static inline boolean_t feth_in_bsd_mode(if_fake_ref fakeif) {
  return (fakeif->iff_flags & IFF_FLAGS_BSD_MODE) != 0;
}

static inline void feth_set_detaching(if_fake_ref fakeif) {
  fakeif->iff_flags |= IFF_FLAGS_DETACHING;
}

static inline boolean_t feth_is_detaching(if_fake_ref fakeif) {
  return (fakeif->iff_flags & IFF_FLAGS_DETACHING) != 0;
}

static int feth_enable_dequeue_stall(ifnet_t ifp, uint32_t enable) {
  int error;
...
#if SKYWALK
static inline boolean_t feth_in_wmm_mode(if_fake_ref fakeif) {
  return (fakeif->iff_flags & IFF_FLAGS_WMM_MODE) != 0;
}

static inline boolean_t feth_using_multibuflets(if_fake_ref fakeif) {
  return (fakeif->iff_flags & IFF_FLAGS_MULTIBUFLETS) != 0;
}
static void feth_detach_netif_nexus(if_fake_ref fakeif);

static inline boolean_t feth_has_intf_advisory_configured(if_fake_ref fakeif) {
  return fakeif->iff_adv_interval > 0;
}
#endif /* SKYWALK */

static inline bool feth_supports_tso(if_fake_ref fakeif) {
  return (fakeif->iff_flags & IFF_FLAGS_TSO_SUPPORT) != 0;
}

static inline void feth_set_supports_tso(if_fake_ref fakeif) {
  fakeif->iff_flags |= IFF_FLAGS_TSO_SUPPORT;
}

static inline bool feth_supports_vlan_mtu(if_fake_ref fakeif) {
  return (fakeif->iff_flags & IFF_FLAGS_VLAN_MTU) != 0;
}

static inline void feth_set_supports_vlan_mtu(if_fake_ref fakeif) {
  fakeif->iff_flags |= IFF_FLAGS_VLAN_MTU;
}

static inline bool feth_supports_vlan_tagging(if_fake_ref fakeif) {
  return (fakeif->iff_flags & IFF_FLAGS_VLAN_TAGGING) != 0;
}

static inline void feth_set_supports_vlan_tagging(if_fake_ref fakeif) {
  fakeif->iff_flags |= IFF_FLAGS_VLAN_TAGGING;
}

static inline void feth_set_supports_rx_flow_steering(if_fake_ref fakeif) {
  fakeif->iff_flags |= IFF_FLAGS_RX_FLOW_STEERING;
}

static inline bool feth_supports_rx_flow_steering(if_fake_ref fakeif) {
...
static struct if_clone feth_cloner = IF_CLONE_INITIALIZER(
    FAKE_ETHER_NAME, feth_clone_create, feth_clone_destroy, 0, FETH_MAXUNIT);
...
#define default_media_words_count                                              \
  (sizeof(default_media_words) / sizeof(default_media_words[0]))
...
static inline void feth_lock(void) { lck_mtx_lock(&feth_lck_mtx); }

static inline void feth_unlock(void) { lck_mtx_unlock(&feth_lck_mtx); }

static inline int get_max_mtu(int bsd_mode, unsigned int max_mtu) {
  unsigned int mtu;
...
static inline unsigned int feth_max_mtu(ifnet_t ifp) {
  if_fake_ref fakeif;
...
static void feth_release(if_fake_ref fakeif) {
...
static void feth_retain(if_fake_ref fakeif) {
  OSIncrementAtomic(&fakeif->iff_retain_count);
}

static void feth_packet_pool_init_prepare(if_fake_ref fakeif,
  ...
  if (feth_using_multibuflets(fakeif)) {
    ...
    pp_init->kbi_buflets = pp_init->kbi_packets * pp_init->kbi_max_frags;
...
static errno_t feth_packet_pool_make(if_fake_ref fakeif) {
  struct kern_pbufpool_init pp_init;
  errno_t err;
  ...
  case IFF_PP_MODE_GLOBAL:
    feth_lock();
    if (S_pp == NULL) {
      (void)snprintf((char *)pp_init.kbi_name, sizeof(pp_init.kbi_name), "%s",
                     "feth shared pp");
  ...
  case IFF_PP_MODE_PRIVATE:
    (void)snprintf((char *)pp_init.kbi_name, sizeof(pp_init.kbi_name), "%s pp",
                   fakeif->iff_name);
  ...
  case IFF_PP_MODE_PRIVATE_SPLIT:
    ...
    err = kern_pbufpool_create(&pp_init, &fakeif->iff_rx_pp, NULL);
    if (err != 0) {
      FAKE_LOG(LOG_NOTICE, FE_DBGF_LIFECYCLE, "rx pp create failed %d", err);
      return err;
    }
    pp_init.kbi_flags &= ~(KBIF_IODIR_IN | KBIF_IODIR_OUT |
                           KBIF_BUFFER_ON_DEMAND | KBIF_KERNEL_READONLY);
    ...
    (void)snprintf((char *)pp_init.kbi_name, sizeof(pp_init.kbi_name),
                   "%s tx pp", fakeif->iff_name);
    err = kern_pbufpool_create(&pp_init, &fakeif->iff_tx_pp, NULL);
    if (err != 0) {
      FAKE_LOG(LOG_NOTICE, FE_DBGF_LIFECYCLE, "tx pp create failed %d", err);
      pp_release(fakeif->iff_rx_pp);
      return err;
    }
    break;
...
static void feth_packet_set_trace_tag(kern_packet_t ph, int flag) {
...
static errno_t feth_clone_packet(if_fake_ref dif, kern_packet_t sph,
                                 kern_packet_t *pdph) {
  errno_t err = 0;
  ...
  saddr =
      __unsafe_forge_bidi_indexable(caddr_t, kern_buflet_get_data_address(sbuf),
  ...
  if (dph0 == 0) {
    ...
    ASSERT(kern_buflet_get_object_limit(dbuf0) == PP_BUF_OBJ_SIZE_DEF(pp));
  ...
  if (doff + dlen > dlim0) {
    ...
    if (err != 0) {
      FAKE_LOG(LOG_INFO, FE_DBGF_OUTPUT, "packet clone err %d", err);
      return err;
    }
    dbuf = kern_packet_get_next_buflet(dph, NULL);
  ...
  VERIFY(err == 0);
  err = kern_packet_set_link_header_length(
      dph, kern_packet_get_link_header_length(sph));
  VERIFY(err == 0);
  err = kern_packet_set_service_class(dph, kern_packet_get_service_class(sph));
  VERIFY(err == 0);
...
static inline void feth_copy_buflet(kern_buflet_t sbuf, kern_buflet_t dbuf) {
  errno_t err;
  uint32_t off, len;
  caddr_t saddr, daddr;
  ...
  daddr =
      __unsafe_forge_bidi_indexable(caddr_t, kern_buflet_get_data_address(dbuf),
                                    kern_buflet_get_data_limit(dbuf));
...
static int feth_add_packet_trailer(kern_packet_t ph,
                                   void *__sized_by(trailer_len) trailer,
                                   size_t trailer_len) {
  errno_t err = 0;
  ...
  void *data =
      __unsafe_forge_bidi_indexable(caddr_t, kern_buflet_get_data_address(buf),
...
static int feth_add_packet_fcs(kern_packet_t ph) {
  uint32_t crc = 0;
  int err;
  ...
  while ((buf = kern_packet_get_next_buflet(ph, buf)) != NULL) {
    uint32_t doff = kern_buflet_get_data_offset(buf);
    uint32_t dlen = kern_buflet_get_data_length(buf);
    void *data = __unsafe_forge_bidi_indexable(
                     caddr_t, kern_buflet_get_data_address(buf),
                     kern_buflet_get_data_limit(buf)) +
                 doff;
...
static errno_t feth_copy_packet(if_fake_ref dif, kern_packet_t sph,
                                kern_packet_t *pdph) {
  errno_t err = 0;
  uint16_t i, bufcnt;
  ...
  *pdph = 0;
  ...
  if (__improbable(err != 0)) {
    ...
    while (i-- != 0) {
      ...
      VERIFY(dbuf != NULL);
      baddr = (mach_vm_address_t)kern_buflet_get_data_address(dbuf);
      VERIFY(baddr != 0);
    }
  ...
  if (__probable(err == 0)) {
    err = kern_packet_set_headroom(dph, kern_packet_get_headroom(sph));
    VERIFY(err == 0);
    err = kern_packet_set_link_header_length(
        dph, kern_packet_get_link_header_length(sph));
    VERIFY(err == 0);
    err =
        kern_packet_set_service_class(dph, kern_packet_get_service_class(sph));
    VERIFY(err == 0);
    err = kern_packet_finalize(dph);
    VERIFY(err == 0);
...
static inline void feth_update_pkt_tso_metadata_for_rx(kern_packet_t ph) {
  /*
   ...
   */
  packet_csum_flags_t csum_flags =
      PACKET_CSUM_IP_CHECKED | PACKET_CSUM_IP_VALID | PACKET_CSUM_DATA_VALID |
      PACKET_CSUM_PSEUDO_HDR;
  (void)kern_packet_set_inet_checksum(ph, csum_flags, 0, 0xFFFF, FALSE);
  return;
}

static void feth_rx_submit(if_fake_ref sif, if_fake_ref dif,
                           kern_packet_t *__counted_by(n_pkts) sphs,
                           uint32_t n_pkts) {
  errno_t err = 0;
  ...
  for (uint32_t i = 0; i < n_pkts && rx_slot != NULL; i++) {
    ...
    if (sif->iff_trailer_length != 0) {
      feth_add_packet_trailer(dph, feth_trailer, sif->iff_trailer_length);
    }
    if (sif->iff_fcs != 0) {
    ...
    stats.kcrsi_bytes_transferred += kern_packet_get_data_length(dph);
  ...
  if (last_rx_slot != NULL) {
    ...
    kern_channel_increment_ring_net_stats(rx_ring, dif->iff_ifp, &stats);
  }
...
static void feth_rx_queue_submit(if_fake_ref sif, if_fake_ref dif,
  ...
  if (llink_idx >= dif->iff_llink_cnt) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_INPUT,
             "invalid llink_idx idx %d (max %d) on peer %s", llink_idx,
             dif->iff_llink_cnt, dif->iff_name);
    return;
  }
  llink = &dif->iff_llink[llink_idx];
  if (qset_idx >= llink->fl_qset_cnt) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_INPUT,
             "invalid qset_idx %d (max %d) on peer %s", qset_idx,
             llink->fl_qset_cnt, dif->iff_name);
    return;
  }
  ...
  for (uint32_t i = 0; i < n_pkts; i++) {
    ...
    flags = (i == n_pkts - 1) ? KERN_NETIF_QUEUE_RX_ENQUEUE_FLAG_FLUSH : 0;
    kern_netif_queue_rx_enqueue(queue, dph, 1, flags);
  }
}

static void feth_tx_complete(if_fake_ref fakeif,
                             kern_packet_t *__counted_by(nphs) phs,
                             uint32_t nphs) {
 ...
 */
static bool feth_packet_has_expired(if_fake_ref __unused fakeif,
                                    kern_packet_t ph, uint64_t *out_deadline) {
  uint64_t now;
  ...
  if (if_fake_expiration_threshold_us < time_until_expiration) {
    ...
    FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "Packet has %llu usec until expiration",
...
static errno_t feth_get_packet_notification_details(if_fake_ref fakeif,
                                                    kern_packet_t ph,
                                                    packet_id_t *pkt_id,
                                                    uint32_t *nx_port_id) {
  errno_t err = 0;
  ...
  if (err != 0) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "%s err=%d getting packetid",
             fakeif->iff_name, err);
    return err;
  }
  ...
  if (err != 0) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "%s err=%d getting nx_port_id",
             fakeif->iff_name, err);
    return err;
  }
...
static packet_expiry_action_t feth_get_effective_expn_action(if_fake_ref fakeif,
                                                             kern_packet_t ph) {
  errno_t err;
  ...
  case IFF_TX_EXP_POLICY_METADATA:
    ...
    if (err != 0) {
      if (err != ENOENT) {
        FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "Error %d when getting expiry action",
                 err);
      }
      expiry_action = PACKET_EXPIRY_ACTION_NONE;
    }
 ...
 */
static bool feth_tx_expired_error(if_fake_ref fakeif, kern_packet_t ph) {
  int err = 0;
  ...
  if (feth_packet_has_expired(fakeif, ph,
                              &expn.packet_tx_expiration_deadline)) {
  ...
  if (expiry_action != PACKET_EXPIRY_ACTION_NONE) {
    ...
    err = feth_get_packet_notification_details(fakeif, ph, &expn.packet_id,
                                               &nx_port_id);

    if (err == 0) {
      err = kern_channel_event_transmit_expired(fakeif->iff_ifp, &expn,
                                                nx_port_id);
      FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC,
    ...
    if (err != 0) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "%s err=%d, nx_port_id: 0x%x",
...
static bool feth_tx_complete_error(if_fake_ref fakeif, kern_packet_t ph) {
  int err;
  ...
  if (fakeif->iff_tx_completion_mode == IFF_TX_COMPL_MODE_SYNC) {
    err = kern_packet_set_tx_completion_status(
        ph, CHANNEL_EVENT_PKT_TRANSMIT_STATUS_ERR_RETRY_FAILED);
    VERIFY(err == 0);
    kern_packet_tx_completion(ph, fakeif->iff_ifp);
  } else {
    uint32_t nx_port_id = 0;
    ...
    err = feth_get_packet_notification_details(
        fakeif, ph, &pkt_tx_status.packet_id, &nx_port_id);
    if (err == 0) {
      err = kern_channel_event_transmit_status(fakeif->iff_ifp, &pkt_tx_status,
                                               nx_port_id);
    }
    if (err != 0) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "%s err=%d, nx_port_id: 0x%x",
...
static void feth_if_adv(thread_call_param_t arg0, thread_call_param_t arg1) {
#pragma unused(arg1)
  errno_t error;
  ...
  if (error != 0) {
    FAKE_LOG(LOG_NOTICE, 0, "%s: ifnet_stat() failed %d", fakeif->iff_name,
             error);
    goto done;
  }
  if_adv.header.version = IF_INTERFACE_ADVISORY_VERSION_CURRENT;
  if_adv.header.direction = IF_INTERFACE_ADVISORY_DIRECTION_TX;
  if_adv.header.interface_type = IF_INTERFACE_ADVISORY_INTERFACE_TYPE_WIFI;
  ...
  if (error != 0) {
    FAKE_LOG(LOG_NOTICE, 0, "%s: interface advisory report failed %d",
  ...
  if (!feth_is_detaching(fakeif) && fakeif->iff_channel_connected) {
    uint64_t deadline;
    clock_interval_to_deadline(fakeif->iff_adv_interval, NSEC_PER_MSEC,
                               &deadline);
...
static int feth_if_adv_tcall_create(if_fake_ref fakeif) {
  uint64_t deadline;
  ...
  fakeif->iff_if_adv_tcall = thread_call_allocate_with_options(
      feth_if_adv, (thread_call_param_t)fakeif, THREAD_CALL_PRIORITY_KERNEL,
      THREAD_CALL_OPTIONS_ONCE);
  if (fakeif->iff_if_adv_tcall == NULL) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "%s if_adv tcall alloc failed",
             fakeif->iff_name);
    feth_unlock();
    return ENXIO;
  }
...
**/
static errno_t feth_nxdp_init(kern_nexus_domain_provider_t domprov) {
#pragma unused(domprov)
  return 0;
}

static void feth_nxdp_fini(kern_nexus_domain_provider_t domprov) {
#pragma unused(domprov)
}

static uuid_t feth_nx_dom_prov;

static errno_t feth_register_nexus_domain_provider(void) {
  ...
  err = kern_nexus_register_domain_provider(NEXUS_TYPE_NET_IF,
                                            feth_provider_name, &dp_init,
                                            sizeof(dp_init), &feth_nx_dom_prov);
  if (err != 0) {
...
**/
static if_fake_ref feth_nexus_context(kern_nexus_t nexus) {
  if_fake_ref fakeif;
...
static uint8_t feth_find_tx_ring_by_svc(kern_packet_svc_class_t svc_class) {
  switch (svc_class) {
  case KPKT_SC_VO:
    return 0;
  case KPKT_SC_VI:
...
static errno_t feth_nx_ring_init(kern_nexus_provider_t nxprov,
                                 kern_nexus_t nexus, kern_channel_t channel,
                                 kern_channel_ring_t ring, boolean_t is_tx_ring,
                                 void **ring_ctx) {
  if_fake_ref fakeif;
  int err;
#pragma unused(nxprov, channel, ring_ctx)
  ...
  feth_unlock();
  FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "%s: %s ring init", fakeif->iff_name,
           is_tx_ring ? "TX" : "RX");
  return 0;
}

static void feth_nx_ring_fini(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
...
#pragma unused(nxprov, ring)
  ...
  if (fakeif->iff_rx_ring[0] == ring) {
    ...
    FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "%s: RX ring fini",
             fakeif->iff_name);
    ...
    FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "%s: TX ring fini",
             fakeif->iff_name);
  ...
  if (tcall != NULL) {
    ...
    FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "%s: thread_call_cancel %s",
             fakeif->iff_name, success ? "SUCCESS" : "FAILURE");
    if (!success) {
      feth_lock();
      if (fakeif->iff_doorbell_tcall_active) {
        fakeif->iff_waiting_for_tcall = TRUE;
        FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "%s: *waiting for threadcall",
                 fakeif->iff_name);
        do {
          msleep(fakeif, &feth_lck_mtx, PZERO, "feth threadcall", 0);
        } while (fakeif->iff_doorbell_tcall_active);
        FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "%s: ^threadcall done",
                 fakeif->iff_name);
    ...
    FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "%s: thread_call_free %s",
             fakeif->iff_name, success ? "SUCCESS" : "FAILURE");
    feth_release(fakeif);
    VERIFY(success == TRUE);
  }
...
static errno_t feth_nx_pre_connect(kern_nexus_provider_t nxprov, proc_t proc,
                                   kern_nexus_t nexus, nexus_port_t port,
...
static errno_t feth_nx_connected(kern_nexus_provider_t nxprov,
                                 kern_nexus_t nexus, kern_channel_t channel) {
...
static void feth_nx_pre_disconnect(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus, kern_channel_t channel) {
#pragma unused(nxprov, channel)
  if_fake_ref fakeif;
  ...
  fakeif = feth_nexus_context(nexus);
  FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "%s: pre-disconnect channel %p",
           fakeif->iff_name, channel);
...
static void feth_nx_disconnected(kern_nexus_provider_t nxprov,
                                 kern_nexus_t nexus, kern_channel_t channel) {
#pragma unused(nxprov, channel)
  if_fake_ref fakeif;
...
static errno_t feth_nx_slot_init(kern_nexus_provider_t nxprov,
                                 kern_nexus_t nexus, kern_channel_ring_t ring,
                                 kern_channel_slot_t slot, uint32_t slot_index,
                                 struct kern_slot_prop **slot_prop_addr,
                                 void **slot_context) {
#pragma unused(nxprov, nexus, ring, slot, slot_index, slot_prop_addr,          \
               slot_context)
  return 0;
}

static void feth_nx_slot_fini(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
                              kern_channel_ring_t ring,
                              kern_channel_slot_t slot, uint32_t slot_index) {
#pragma unused(nxprov, nexus, ring, slot, slot_index)
}

static errno_t feth_nx_sync_tx(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
...
#pragma unused(nxprov)
  ...
  fakeif = feth_nexus_context(nexus);
  FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s ring %d flags 0x%x", fakeif->iff_name,
  ...
  if (peer_ifp != NULL) {
    ...
    if (peer_fakeif != NULL) {
      if (feth_is_detaching(peer_fakeif)) {
        FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s peer fakeif %s is detaching",
                 fakeif->iff_name, peer_fakeif->iff_name);
        goto done;
      }
      if (!peer_fakeif->iff_channel_connected) {
        if (fakeif->iff_tx_exp_policy == IFF_TX_EXP_POLICY_DISABLED) {
      ...
      FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s no peer fakeif (peer %p)",
               fakeif->iff_name, peer_ifp);
      goto done;
    }
  } else {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s no peer", fakeif->iff_name);
    goto done;
  }
...
static errno_t feth_nx_sync_rx(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
...
static errno_t feth_nx_tx_dequeue_driver_managed(if_fake_ref fakeif,
                                                 boolean_t doorbell_ctxt) {
  int i;
  errno_t error = 0;
  boolean_t more;
  ...
  for (i = 0; i < IFF_NUM_TX_RINGS_WMM_MODE; i++) {
    ...
    if (error != 0) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: TX refill ring %d (%s) %d",
      ...
      FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: TX refilled ring %d (%s)",
               fakeif->iff_name, ring->ckr_ring_id,
...
static void feth_async_doorbell(thread_call_param_t arg0,
...
#pragma unused(arg1)
  ...
  if (feth_in_wmm_mode(fakeif)) {
    ...
    error = kern_channel_tx_refill(ring, UINT32_MAX, UINT32_MAX, FALSE, &more);
  }
  if (error != 0) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: TX refill failed %d",
             fakeif->iff_name, error);
  } else {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: TX refilled", fakeif->iff_name);
  }
  ...
  if (fakeif->iff_waiting_for_tcall) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: threadcall waking up waiter",
             fakeif->iff_name);
...
static void feth_schedule_async_doorbell(if_fake_ref fakeif) {
  thread_call_t __single tcall;
  ...
  if (tcall != NULL) {
    ...
    tcall = thread_call_allocate_with_options(
        feth_async_doorbell, (thread_call_param_t)fakeif,
        THREAD_CALL_PRIORITY_KERNEL, THREAD_CALL_OPTIONS_ONCE);
    if (tcall == NULL) {
      FAKE_LOG(LOG_NOTICE, FE_DBGF_OUTPUT, "%s tcall alloc failed",
               fakeif->iff_name);
    } else {
...
static errno_t feth_nx_tx_doorbell(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus, kern_channel_ring_t ring,
                                   uint32_t flags) {
#pragma unused(nxprov, ring, flags)
  errno_t error;
  ...
  if ((flags & KERN_NEXUS_TXDOORBELLF_ASYNC_REFILL) == 0) {
    ...
    if (feth_in_wmm_mode(fakeif)) {
      ...
      error = kern_channel_tx_refill(ring, UINT32_MAX, UINT32_MAX, TRUE, &more);
    }
    if (error != 0) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: TX refill (sync) %d",
               fakeif->iff_name, error);
    } else {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: TX refilled (sync)",
               fakeif->iff_name);
    }
  } else {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: schedule async refill",
             fakeif->iff_name);
...
static errno_t feth_netif_prepare(kern_nexus_t nexus, ifnet_t ifp) {
  if_fake_ref fakeif;
...
static errno_t feth_nx_intf_adv_config(void *prov_ctx, bool enable) {
  if_fake_ref fakeif = prov_ctx;

  feth_lock();
  fakeif->iff_intf_adv_enabled = enable;
  feth_unlock();
  FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "%s enable %d", fakeif->iff_name, enable);
  return 0;
}

static errno_t fill_capab_interface_advisory(if_fake_ref fakeif, void *contents,
                                             uint32_t *len) {
  ...
  if (capab->kncia_version != KERN_NEXUS_CAPAB_INTERFACE_ADVISORY_VERSION_1) {
    return EINVAL;
  }
  if (!feth_has_intf_advisory_configured(fakeif)) {
    return ENOTSUP;
...
#pragma unused(td)
  ...
  FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC,
           "%s: notify_steering_info: qset_id 0x%llx, %s", fakeif->iff_name,
           qset->fqs_id, add ? "add" : "remove");
  return 0;
}

static errno_t fill_capab_qset_extensions(if_fake_ref fakeif, void *contents,
                                          uint32_t *len) {
  ...
  if (capab->cqe_version != KERN_NEXUS_CAPAB_QSET_EXTENSIONS_VERSION_1) {
    return EINVAL;
  }
...
static errno_t fill_capab_rx_flow_steering(if_fake_ref fakeif, void *contents,
                                           uint32_t *len) {
  ...
  if (capab->kncrxfs_version != KERN_NEXUS_CAPAB_RX_FLOW_STEERING_VERSION_1) {
    return EINVAL;
  }
...
static errno_t feth_nx_capab_config(kern_nexus_provider_t nxprov,
                                    kern_nexus_t nx, kern_nexus_capab_t capab,
...
static int feth_set_tso_mtu(ifnet_t ifp, uint32_t tso_v4_mtu,
                            uint32_t tso_v6_mtu) {
  int error;
  ...
  if (error != 0) {
    FAKE_LOG(LOG_NOTICE, FE_DBGF_CONTROL,
             "set TSO MTU IPv4 failed on %s, err %d", if_name(ifp), error);
    return error;
  }
  error = ifnet_set_tso_mtu(ifp, AF_INET6, tso_v6_mtu);
  if (error != 0) {
    FAKE_LOG(LOG_NOTICE, FE_DBGF_CONTROL,
             "set TSO MTU IPv6 failed on %s, err %d", if_name(ifp), error);
    return error;
  }
  return 0;
}

static int feth_set_tso_offload(ifnet_t ifp) {
  ...
  if (error != 0) {
    FAKE_LOG(LOG_NOTICE, FE_DBGF_CONTROL,
             "set TSO offload failed on %s, err %d", if_name(ifp), error);
    goto done;
  }
  error =
      feth_set_tso_mtu(ifp, if_fake_tso_buffer_size, if_fake_tso_buffer_size);
done:
  return error;
}

static errno_t create_netif_provider_and_instance(
    if_fake_ref fakeif, struct ifnet_init_eparams *init_params, ifnet_t *ifp,
  ...
  if (feth_in_wmm_mode(fakeif)) {
    ...
    VERIFY(err == 0);
    err = kern_nexus_attr_set(nexus_attr, NEXUS_ATTR_QMAP, NEXUS_QMAP_TYPE_WMM);
    VERIFY(err == 0);
  }

  err = kern_nexus_attr_set(nexus_attr, NEXUS_ATTR_ANONYMOUS, 1);
  VERIFY(err == 0);
  snprintf((char *)provider_name, sizeof(provider_name), "com.apple.netif.%s",
           fakeif->iff_name);
  err = kern_nexus_controller_register_provider(
      controller, feth_nx_dom_prov, provider_name, &prov_init,
      sizeof(prov_init), nexus_attr, provider);
  if (err != 0) {
    FAKE_LOG(LOG_NOTICE, FE_DBGF_LIFECYCLE,
  ...
  err = kern_nexus_controller_alloc_net_provider_instance(
      controller, *provider, fakeif, NULL, instance, &net_init, ifp);
  if (err != 0) {
 ...
 */
static void get_nexus_stats(if_fake_ref fakeif, kern_nexus_t nexus) {
...
static void clear_nexus_stats(if_fake_ref fakeif) {
...
static errno_t feth_nx_qset_init(kern_nexus_provider_t nxprov,
                                 kern_nexus_t nexus, void *llink_ctx,
                                 uint8_t qset_idx, uint64_t qset_id,
                                 kern_netif_qset_t qset, void **qset_ctx) {
#pragma unused(nxprov)
  if_fake_ref fakeif;
  ...
  if (feth_is_detaching(fakeif)) {
    feth_unlock();
    FAKE_LOG(LOG_DEBUG, FE_DBGF_CONTROL, "%s: detaching", fakeif->iff_name);
    return ENXIO;
  }
  if (qset_idx >= fl->fl_qset_cnt) {
    feth_unlock();
    FAKE_LOG(LOG_DEBUG, FE_DBGF_CONTROL, "%s: invalid qset_idx %d",
...
static void feth_nx_qset_fini(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
                              void *qset_ctx) {
#pragma unused(nxprov)
  if_fake_ref fakeif;
...
static errno_t feth_nx_queue_init(kern_nexus_provider_t nxprov,
...
#pragma unused(nxprov)
  ...
  if (tx) {
    if (qidx >= fqs->fqs_tx_queue_cnt) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_CONTROL, "%s: invalid tx qidx %d",
    ...
    if (qidx >= fqs->fqs_rx_queue_cnt) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_CONTROL, "%s: invalid rx qidx %d",
               fakeif->iff_name, qidx);
      feth_unlock();
      return EINVAL;
    }
...
static void feth_nx_queue_fini(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
                               void *queue_ctx) {
...
static void feth_nx_tx_queue_deliver_pkt_chain(
    if_fake_ref fakeif, kern_packet_t sph, struct netif_stats *nifs,
    if_fake_ref peer_fakeif, uint32_t llink_idx, uint32_t qset_idx) {
  kern_packet_t pkts[IFF_MAX_BATCH_SIZE];
  uint32_t n_pkts = 0;

  FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s -> %s", fakeif->iff_name,
  ...
  while (sph != 0) {
    ...
    pkts[n_pkts++] = sph;
    if (n_pkts == IFF_MAX_BATCH_SIZE) {
      feth_rx_queue_submit(fakeif, peer_fakeif, llink_idx, qset_idx, pkts,
                           n_pkts);
      feth_tx_complete(fakeif, pkts, n_pkts);
      n_pkts = 0;
    }
...
static errno_t feth_nx_tx_qset_notify(kern_nexus_provider_t nxprov,
                                      kern_nexus_t nexus, void *qset_ctx,
                                      uint32_t flags) {
#pragma unused(nxprov)
  if_fake_ref fakeif;
  ...
  STATS_INC(nifs, NETIF_STATS_TX_SYNC);
  fakeif = feth_nexus_context(nexus);
  FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s qset %p, idx %d, flags 0x%x",
           fakeif->iff_name, qset, qset->fqs_idx, flags);
  ...
  if (detaching || !connected) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT,
             "%s: detaching %s, channel connected %s", fakeif->iff_name,
             (detaching ? "true" : "false"), (connected ? "true" : "false"));
    feth_unlock();
    return 0;
  }
  peer_ifp = fakeif->iff_peer;
    ...
    if (peer_fakeif != NULL) {
      ...
      if (detaching || !connected) {
        FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT,
                 "peer %s: detaching %s, "
                 "channel connected %s",
                 peer_fakeif->iff_name, (detaching ? "true" : "false"),
                 (connected ? "true" : "false"));
        goto done;
      }
    } else {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "peer_fakeif is NULL");
      goto done;
    }
  } else {
  ...
  for (i = 0; i < qset->fqs_tx_queue_cnt; i++) {
    ...
    err =
        kern_netif_queue_tx_dequeue(queue, UINT32_MAX, UINT32_MAX, &more, &sph);
    if (err != 0 && err != EAGAIN) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT,
    ...
    feth_nx_tx_queue_deliver_pkt_chain(fakeif, sph, nifs, peer_fakeif,
                                       qset->fqs_llink_idx, qset->fqs_idx);
  }
...
static errno_t feth_nx_queue_tx_push(kern_nexus_provider_t nxprov,
                                     kern_nexus_t nexus, void *queue_ctx,
                                     kern_packet_t *ph, uint32_t *packetCount,
                                     uint32_t *byteCount) {
#pragma unused(nxprov)
  if_fake_ref fakeif;
  ifnet_t peer_ifp;
  ...
  *packetCount = *byteCount = 0;
  ...
  feth_nx_tx_queue_deliver_pkt_chain(fakeif, *ph, nifs, peer_fakeif, 0, 0);
...
static errno_t create_non_default_llinks(if_fake_ref fakeif) {
  ...
  if (nx == NULL) {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "%s: nx not found", fakeif->iff_name);
    return ENXIO;
  }
  ...
  for (i = 1; i < if_fake_llink_cnt; i++) {
     ...
     * each llink creation.
     */
    fill_llink_info_and_params(fakeif, i, &llink_init, llink_id, qset_init,
                               if_fake_qset_cnt, 0);
    err = kern_nexus_netif_llink_add(nx, &llink_init);
    if (err != 0) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC, "%s: llink add failed, error %d",
               fakeif->iff_name, err);
      goto fail;
    }
  ...
  for (i = 0; i < fakeif->iff_llink_cnt; i++) {
    ...
    if (error != 0) {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_MISC,
               ...
               "error %d",
               fakeif->iff_name, ll->fl_id, error);
    }
    ll->fl_id = 0;
  }
...
static errno_t create_netif_llink_provider_and_instance(
    if_fake_ref fakeif, struct ifnet_init_eparams *init_params, ifnet_t *ifp,
    uuid_t *provider, uuid_t *instance) {
  errno_t err;
  nexus_controller_t controller = kern_nexus_shared_controller();
  ...
  snprintf((char *)provider_name, sizeof(provider_name), "com.apple.netif.%s",
           fakeif->iff_name);
  err = kern_nexus_controller_register_provider(
      controller, feth_nx_dom_prov, provider_name,
      (struct kern_nexus_provider_init *)&prov_init, sizeof(prov_init),
      nexus_attr, provider);
  if (err != 0) {
    FAKE_LOG(LOG_NOTICE, FE_DBGF_LIFECYCLE,
             "register provider failed, error %d", err);
  ...
  fill_llink_info_and_params(fakeif, 0, &llink_init, NETIF_LLINK_ID_DEFAULT,
                             qsets, if_fake_qset_cnt,
...
static errno_t feth_attach_netif_nexus(if_fake_ref fakeif,
  ...
  if (if_fake_llink_cnt == 0) {
    return create_netif_provider_and_instance(
        fakeif, init_params, ifp, &nx->fnx_provider, &nx->fnx_instance);
  } else {
    return create_netif_llink_provider_and_instance(
        fakeif, init_params, ifp, &nx->fnx_provider, &nx->fnx_instance);
  }
}
...
remove_non_default_llinks(const char *name, fake_nx_t fnx,
  ...
  nx = nx_find(fnx->fnx_instance, FALSE);
  if (nx == NULL) {
    FAKE_LOG(LOG_NOTICE, FE_DBGF_LIFECYCLE, "%s: nx not found", name);
    goto done;
  }
...
static void detach_provider_and_instance(uuid_t provider, uuid_t instance) {
  nexus_controller_t controller = kern_nexus_shared_controller();
  errno_t err;
  ...
  if (!uuid_is_null(instance)) {
    ...
    if (err != 0) {
      ...
      FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "deregister_instance");
    }
  }
  if (!uuid_is_null(provider)) {
    err = kern_nexus_controller_deregister_provider(controller, provider);
    if (err != 0) {
      FAKE_LOG(LOG_NOTICE, FE_DBGF_LIFECYCLE, "deregister_provider %d", err);
    } else {
      FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE, "deregister_provider");
    }
  }
  return;
}
...
static void feth_detach_netif_nexus(if_fake_ref fakeif) {
  ...
  fakeif->iff_llink_cnt = 0;
  feth_unlock();
  remove_non_default_llinks(
      __unsafe_null_terminated_from_indexable(fakeif->iff_name), &fnx, llink,
      llink_cnt);
...
static void feth_ifnet_set_attrs(if_fake_ref fakeif, ifnet_t ifp) {
  ...
  if (error != 0) {
    FAKE_LOG(LOG_NOTICE, FE_DBGF_LIFECYCLE,
             "ifnet_set_offload(%s, 0x%x) failed, %d", ifp->if_xname, offload,
             error);
  } else {
    FAKE_LOG(LOG_DEBUG, FE_DBGF_LIFECYCLE,
             "ifnet_set_offload(%s, 0x%x) succeeded", ifp->if_xname, offload);
  }
...
static void interface_link_event(ifnet_t ifp, u_int32_t event_code) {
  ...
  struct event *data =
      (struct event *)(message + offsetof(struct kern_event_msg, event_data));
...
static int feth_clone_create(struct if_clone *ifc, u_int32_t unit,
                             __unused void *params) {
  bool bsd_mode;
  int error;
  if_fake_ref fakeif;
...
#define FAKE_ETHER_NAME_LEN (sizeof(FAKE_ETHER_NAME) - 1)
  ...
  if ((unsigned int)snprintf(fakeif->iff_name, sizeof(fakeif->iff_name), "%s%d",
  ...
  if (feth_in_bsd_mode(fakeif)) {
    ...
    if (feth_in_wmm_mode(fakeif)) {
      feth_init.output_sched_model = IFNET_SCHED_MODEL_DRIVER_MANAGED;
    }
...
#endif /* SKYWALK */
  fakeif->iff_media_count =
      MIN(default_media_words_count, IF_FAKE_MEDIA_LIST_MAX);
...
static int feth_clone_destroy(ifnet_t ifp) {
  if_fake_ref fakeif;
#if SKYWALK
...
static void feth_enqueue_input(ifnet_t ifp, struct mbuf *m) {
...
static int feth_add_mbuf_trailer(struct mbuf *m,
                                 void *trailer __sized_by(trailer_len),
                                 size_t trailer_len) {
  int ret;
  ASSERT(trailer_len <= FETH_TRAILER_LENGTH_MAX);
...
static int feth_add_mbuf_fcs(struct mbuf *m) {
...
static void feth_output_common(ifnet_t ifp, struct mbuf *m, ifnet_t peer,
                               iff_flags_t flags, bool fcs,
                               void *trailer __sized_by(trailer_len),
                               size_t trailer_len) {
  void *frame_header;
  ...
  if ((flags & IFF_FLAGS_HWCSUM) != 0) {
    ...
    m->m_pkthdr.csum_flags =
        CSUM_DATA_VALID | CSUM_PSEUDO_HDR | CSUM_IP_CHECKED | CSUM_IP_VALID;
  }
  ...
  if ((flags & IFF_FLAGS_SEPARATE_FRAME_HEADER) != 0) {
    ...
    FAKE_LOG(LOG_DEBUG, FE_DBGF_OUTPUT, "%s: frame 0x%llx data 0x%llx len %ld",
             ifp->if_xname, (uint64_t)VM_KERNEL_ADDRPERM(frame_header),
             (uint64_t)VM_KERNEL_ADDRPERM(mtod(m, void *)), mbuf_len(m));
  } else {
    frame_header = mtod(m, void *);
    mbuf_pkthdr_setheader(m, frame_header);
...
static int feth_config(ifnet_t ifp, ifnet_t peer) {
  ...
  if (peer != NULL) {
    ...
    if (feth_is_detaching(fakeif) || feth_is_detaching(peer_fakeif) ||
        peer_fakeif->iff_peer != NULL || fakeif->iff_peer != NULL) {
      error = EBUSY;
      goto done;
    }
#if SKYWALK
    if (fakeif->iff_pp_mode != peer_fakeif->iff_pp_mode) {
      error = EINVAL;
      goto done;
    }
#endif /* SKYWALK */
...
static int feth_set_media(ifnet_t ifp, struct if_fake_request *iffr) {
  if_fake_ref fakeif;
  int error;
...
static int if_fake_request_copyin(user_addr_t user_addr,
...
static int feth_set_drvspec(ifnet_t ifp, uint32_t cmd, u_int32_t len,
  ...
  case IF_FAKE_S_CMD_SET_DEQUEUE_STALL:
    ...
    error = feth_enable_dequeue_stall(ifp, iffr.iffr_dequeue_stall);
    break;
  default:
    error = EOPNOTSUPP;
    break;
...
static int feth_get_drvspec(ifnet_t ifp, u_int32_t cmd, u_int32_t len,
                            user_addr_t user_addr) {
  ...
  case IF_FAKE_G_CMD_GET_PEER:
    ...
    if (peer != NULL) {
      strlcpy(iffr.iffr_peer_name, if_name(peer), sizeof(iffr.iffr_peer_name));
    }
...
static int feth_ioctl(ifnet_t ifp, u_long cmd, void *data) {
  unsigned int count;
  ...
  case SIOCGIFMEDIA64:
    ...
    status =
        (fakeif->iff_peer != NULL) ? (IFM_AVALID | IFM_ACTIVE) : IFM_AVALID;
    ...
    ifmr->ifm_active =
        (fakeif->iff_peer != NULL) ? FAKE_DEFAULT_MEDIA : IFM_ETHER;
    ...
    if (user_addr == USER_ADDR_NULL) {
      ...
      error = copyout(&fakeif->iff_media_list, user_addr, count * sizeof(int));
    }
    feth_unlock();
    break;
  ...
  case SIOCGDRVSPEC64:
    ...
    if (drv_set_command) {
      error = feth_set_drvspec(ifp, drv_cmd, drv_len, user_addr);
    } else {
      error = feth_get_drvspec(ifp, drv_cmd, drv_len, user_addr);
    }
    break;
  ...
  case SIOCDIFADDR:
    if (if_fake_fail_ioctl != 0) {
      FAKE_LOG(LOG_NOTICE, FE_DBGF_LIFECYCLE,
               "%s: failing SIOCDIFADDR with EPWROFF", ifp->if_xname);
      error = EPWROFF;
    }
    break;
  ...
  case SIOCSIFCAP: {
    ...
    feth_lock();
    fakeif = ifnet_get_if_fake(ifp);
    if (fakeif == NULL || (fakeif->iff_flags & IFF_FLAGS_LRO) == 0) {
      feth_unlock();
      return EOPNOTSUPP;
    }
    feth_unlock();
...
__private_extern__ void if_fake_init(void) {
  int error;
