 ...
 */
#define ENTITLEMENT_TEST_CONTROL                                               \
...
#define RPE_FLAG_ENTITLEMENT                                                   \
  0x02                     // can use the port with the required entitlement
...
static struct restricted_port_entry restricted_port_list[] = {
    ...
    },
    {
        .rpe_port = 61315,
        .rpe_flags = RPE_FLAG_ENTITLEMENT | RPE_FLAG_TCP | RPE_FLAG_UDP,
        .rpe_entitlement = "com.apple.private.network.restricted.port.ids_"
                           "cloud_service_connector",
    },
...
#define RPE_ENTRY_COUNT                                                        \
  (sizeof(restricted_port_list) / sizeof(restricted_port_list[0]))

SYSCTL_NODE(_net, OID_AUTO, restricted_port, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "restricted port");
...
SYSCTL_PROC(_net_restricted_port, OID_AUTO, bitmap,
            ...
            &sysctl_restricted_port_bitmap, "", "");

/*
 * In order to set the following sysctl variables the process needs to run as
 * superuser or have the entitlement ENTITLEMENT_TEST_CONTROL
 */
#if (DEBUG || DEVELOPMENT)
static int restricted_port_enforced = 1;
SYSCTL_PROC(_net_restricted_port, OID_AUTO, enforced,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW | CTLFLAG_ANYBODY, 0, 0,
            &sysctl_restricted_port_enforced, "I", "");
#else  /* (DEBUG || DEVELOPMENT) */
const int restricted_port_enforced = 1;
SYSCTL_PROC(_net_restricted_port, OID_AUTO, enforced,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RD, 0, 0,
            &sysctl_restricted_port_enforced, "I", "");
#endif /* (DEBUG || DEVELOPMENT) */
...
SYSCTL_PROC(_net_restricted_port, OID_AUTO, verbose,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW | CTLFLAG_ANYBODY, 0, 0,
            &sysctl_restricted_port_verbose, "I", "");

#if (DEBUG || DEVELOPMENT)

/*
 * Register dynamically a test port set by the unit test program to avoid
 * conflict with a restricted port currently used by its legetimate process. The
 * value must be passed is in host byte order.
 */
...
SYSCTL_PROC(_net_restricted_port, OID_AUTO, test_entitlement,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW | CTLFLAG_ANYBODY, 0, 0,
            &sysctl_restricted_port_test_entitlement, "UI", "");

SYSCTL_PROC(_net_restricted_port, OID_AUTO, test_superuser,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW | CTLFLAG_ANYBODY, 0, 0,
            &sysctl_restricted_port_test_superuser, "UI", "");
#endif /* (DEBUG || DEVELOPMENT) */
...
#pragma unused(arg1, arg2)
  ...
  os_log(OS_LOG_DEFAULT, "%s:%u sysctl net.restricted_port.enforced: %d -> %d",
...
static int sysctl_restricted_port_test_common(struct sysctl_oid *oidp,
...
void restricted_in_port_init(void) {
  unsigned int i;
...
static const char *port_flag_str(uint32_t port_flags) {
  switch (port_flags) {
 ...
 */
bool current_task_can_use_restricted_in_port(in_port_t port, uint8_t protocol,
                                             uint32_t port_flags) {
  unsigned int i;
  ...
  for (i = 0; i < RPE_ENTRY_COUNT; i++) {
    ...
    if (rpe->rpe_flags & RPE_FLAG_SUPERUSER) {
      if (task == kernel_task || proc_suser(current_proc()) == 0) {
        os_log(OS_LOG_DEFAULT,
               "root restricted port %u for protocol %u via %s can be used by "
               "superuser process %s:%u",
               ntohs(port), protocol, port_flag_str(port_flags),
               proc_best_name(p), pid);
        return true;
      }
    ...
    if (rpe->rpe_flags & RPE_FLAG_ENTITLEMENT) {
      ...
      if (task == kernel_task) {
        os_log(OS_LOG_DEFAULT,
               "entitlement restricted port %u for protocol %u via %s cannot "
               "be used by kernel",
               ntohs(port), protocol, port_flag_str(port_flags));
        return false;
      }
      if (!IOCurrentTaskHasEntitlement(rpe->rpe_entitlement)) {
        os_log(OS_LOG_DEFAULT,
               "entitlement restricted port %u for protocol %u via %s cannot "
               "be used by process %s:%u -- IOTaskHasEntitlement(%s) failed",
               ntohs(port), protocol, port_flag_str(port_flags),
               proc_best_name(p), pid, rpe->rpe_entitlement);
        return false;
      }
      os_log(OS_LOG_DEFAULT,
             "entitlement restricted port %u for protocol %u via %s can be "
             "used by process %s:%u",
             ntohs(port), protocol, port_flag_str(port_flags),
             proc_best_name(p), pid);
      return true;
    ...
    os_log(OS_LOG_DEFAULT,
           "root restricted port %u for protocol %u via %s cannot be used by "
           "process %s:%u",
           ntohs(port), protocol, port_flag_str(port_flags), proc_best_name(p),
           pid);
    return false;
  }
  if (restricted_port_verbose > 1) {
