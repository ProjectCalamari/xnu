...
#define PKTAP_LOG(mask, fmt, ...)                                              \
  do {                                                                         \
    if (__improbable(pktap_log & mask))                                        \
      os_log(OS_LOG_DEFAULT, "%s:%d " fmt, __FUNCTION__, __LINE__,             \
             ##__VA_ARGS__);                                                   \
  } while (false)
...
static LIST_HEAD(pktap_list,
                 pktap_softc) pktap_list = LIST_HEAD_INITIALIZER(pktap_list);
...
static struct if_clone pktap_cloner = IF_CLONE_INITIALIZER(
    PKTAP_IFNAME, pktap_clone_create, pktap_clone_destroy, 0, PKTAP_MAXUNIT);
...
errno_t pktap_ioctl(ifnet_t, unsigned long cmd,
                    void *__sized_by(IOCPARM_LEN(cmd)));
void pktap_detach(ifnet_t);
int pktap_filter_evaluate(struct pktap_softc *, struct ifnet *);
void pktap_bpf_tap(struct ifnet *, protocol_family_t, struct mbuf *, u_int32_t,
                   u_int32_t, int);
errno_t pktap_tap_callback(ifnet_t, u_int32_t, bpf_tap_mode);

static void pktap_hexdump(int mask, void *__sized_by(len) addr, size_t len) {
...
__private_extern__ void pktap_init(void) {
  int error = 0;
...
__private_extern__ int pktap_clone_create(struct if_clone *ifc, u_int32_t unit,
...
__private_extern__ int pktap_clone_destroy(struct ifnet *ifp) {
  int error = 0;
 ...
 */
__private_extern__ errno_t pktap_tap_callback(ifnet_t ifp, u_int32_t dlt,
...
__private_extern__ errno_t pktap_if_output(ifnet_t ifp, mbuf_t m) {
  PKTAP_LOG(PKTP_LOG_FUNC, "%s\n", ifp->if_xname);
  mbuf_freem(m);
  return ENOTSUP;
}

__private_extern__ errno_t pktap_demux(ifnet_t ifp, __unused mbuf_t m,
...
pktap_add_proto(__unused ifnet_t ifp, protocol_family_t pf,
                __unused const struct ifnet_demux_desc *dmx,
                __unused u_int32_t cnt) {
  PKTAP_LOG(PKTP_LOG_FUNC, "%s pf %u\n", ifp->if_xname, pf);
  return 0;
}

__private_extern__ errno_t pktap_del_proto(__unused ifnet_t ifp,
                                           __unused protocol_family_t pf) {
  PKTAP_LOG(PKTP_LOG_FUNC, "%s pf %u\n", ifp->if_xname, pf);
  return 0;
}

__private_extern__ errno_t pktap_getdrvspec(ifnet_t ifp, struct ifdrv64 *ifd) {
  errno_t error = 0;
  struct pktap_softc *__single pktap;
  int i;
  ...
  case PKTP_CMD_TAP_COUNT: {
    uint32_t tap_count =
        pktap->pktp_dlt_raw_count + pktap->pktp_dlt_pkttap_count;
    ...
    error =
        copyout(&tap_count, CAST_USER_ADDR_T(ifd->ifd_data), sizeof(tap_count));
    if (error) {
...
__private_extern__ errno_t pktap_setdrvspec(ifnet_t ifp, struct ifdrv64 *ifd) {
  errno_t error = 0;
  struct pktap_softc *__single pktap;
  ...
  case PKTP_CMD_FILTER_SET: {
    ...
    error = copyin(CAST_USER_ADDR_T(ifd->ifd_data), &user_filters,
                   (size_t)ifd->ifd_len);
    if (error) {
...
__private_extern__ errno_t pktap_ioctl(ifnet_t ifp, unsigned long cmd,
                                       void *__sized_by(IOCPARM_LEN(cmd))
                                           data) {
  errno_t error = 0;
...
__private_extern__ void pktap_detach(ifnet_t ifp) {
  struct pktap_softc *__single pktap;
...
__private_extern__ int pktap_filter_evaluate(struct pktap_softc *pktap,
                                             struct ifnet *ifp) {
  int i;
      ...
      if (pktap_filter->filter_param == PKTAP_FILTER_PARAM_IF_TYPE) {
        if (pktap_filter->filter_param_if_type == 0 ||
          ...
          PKTAP_LOG(PKTP_LOG_FILTER, "pass %s match type %u\n", ifp->if_xname,
                    pktap_filter->filter_param_if_type);
          break;
        }
      }
      if (pktap_filter->filter_param == PKTAP_FILTER_PARAM_IF_NAME) {
        if (pktap_filter->filter_ifname_prefix_match == false) {
          match = !strlcmp(pktap_filter->filter_param_if_name, ifp->if_xname,
                           pktap_filter->filter_ifname_len);
        } else {
          match = strprefix(ifp->if_xname,
                            __unsafe_forge_null_terminated(
                                char *, pktap_filter->filter_param_if_name));
        }
...
static void pktap_set_procinfo(struct pktap_header *hdr,
...
__private_extern__ void pktap_finalize_proc_info(struct pktap_header *hdr) {
  int found;
  struct so_procinfo soprocinfo;
  ...
  if (hdr->pth_ipproto == IPPROTO_TCP) {
    found = inp_findinpcb_procinfo(&tcbinfo, hdr->pth_flowid, &soprocinfo);
  } else if (hdr->pth_ipproto == IPPROTO_UDP) {
    found = inp_findinpcb_procinfo(&udbinfo, hdr->pth_flowid, &soprocinfo);
  } else {
    found = inp_findinpcb_procinfo(&ripcbinfo, hdr->pth_flowid, &soprocinfo);
  }
...
static void pktap_v2_set_procinfo(struct pktap_v2_hdr *pktap_v2_hdr,
                                  struct so_procinfo *soprocinfo) {
  uint8_t *region = __unsafe_forge_bidi_indexable(uint8_t *, pktap_v2_hdr,
  ...
  if (soprocinfo->spi_pid != 0 && soprocinfo->spi_pid != -1) {
    if (pktap_v2_hdr->pth_comm_offset != 0) {
      char *ptr = __unsafe_forge_bidi_indexable(
          char *, region + pktap_v2_hdr->pth_comm_offset, PKTAP_MAX_COMM_SIZE);

      strbufcpy(ptr, PKTAP_MAX_COMM_SIZE, soprocinfo->spi_proc_name,
                PKTAP_MAX_COMM_SIZE);
    }
    if (pktap_v2_hdr->pth_uuid_offset != 0) {
      uuid_t *ptr = __unsafe_forge_bidi_indexable(
          uuid_t *, region + pktap_v2_hdr->pth_uuid_offset, sizeof(uuid_t));
  ...
  if (soprocinfo->spi_delegated != 0) {
    ...
    if (soprocinfo->spi_pid != 0 && soprocinfo->spi_pid != -1 &&
      ...
      char *ptr = __unsafe_forge_bidi_indexable(
          char *, region + pktap_v2_hdr->pth_e_comm_offset,
          PKTAP_MAX_COMM_SIZE);
      strbufcpy(ptr, PKTAP_MAX_COMM_SIZE, soprocinfo->spi_e_proc_name,
                PKTAP_MAX_COMM_SIZE);
    }
    if (pktap_v2_hdr->pth_e_uuid_offset != 0) {
      uuid_t *ptr = __unsafe_forge_bidi_indexable(
          uuid_t *, region + pktap_v2_hdr->pth_e_uuid_offset, sizeof(uuid_t));
...
pktap_v2_finalize_proc_info(struct pktap_v2_hdr *pktap_v2_hdr) {
  ...
  if (pktap_v2_hdr->pth_ipproto == IPPROTO_TCP) {
    found =
        inp_findinpcb_procinfo(&tcbinfo, pktap_v2_hdr->pth_flowid, &soprocinfo);
  } else if (pktap_v2_hdr->pth_ipproto == IPPROTO_UDP) {
    found =
        inp_findinpcb_procinfo(&udbinfo, pktap_v2_hdr->pth_flowid, &soprocinfo);
  } else {
    found = inp_findinpcb_procinfo(&ripcbinfo, pktap_v2_hdr->pth_flowid,
                                   &soprocinfo);
  }
  if (found == 1) {
...
__private_extern__ void pktap_fill_proc_info(struct pktap_header *hdr,
                                             protocol_family_t proto,
                                             struct mbuf *m, u_int32_t pre,
                                             int outgoing, struct ifnet *ifp) {
  /*
  ...
  if (outgoing != 0 && m->m_pkthdr.pkt_flowsrc == FLOWSRC_INPCB) {
    ...
    if (proto == PF_INET) {
      ...
      if (ip.ip_p == IPPROTO_TCP) {
        ...
        error = mbuf_copydata(m, pre + hlen, sizeof(struct tcphdr), &th);
        if (error != 0) {
          goto done;
        }
        ...
        error = mbuf_copydata(m, pre + hlen, sizeof(struct udphdr), &uh);
        if (error != 0) {
      ...
      if (pcbinfo != NULL) {
        inp = in_pcblookup_hash_try(pcbinfo, faddr, fport, laddr, lport,
                                    wildcard, outgoing ? NULL : ifp);

        if (inp == NULL && hdr->pth_iftype != IFT_LOOP) {
          PKTAP_LOG(PKTP_LOG_NOPCB, "in_pcblookup_hash_try no pcb %s\n",
                    hdr->pth_ifname);
        }
      } else {
      ...
      if (pcbinfo != NULL) {
        inp = in6_pcblookup_hash_try(
            pcbinfo, faddr, fport, ip6_input_getdstifscope(m), laddr, lport,
            ip6_input_getsrcifscope(m), wildcard, outgoing ? NULL : ifp);

        if (inp == NULL && hdr->pth_iftype != IFT_LOOP) {
          PKTAP_LOG(PKTP_LOG_NOPCB, "in6_pcblookup_hash_try no pcb %s\n",
                    hdr->pth_ifname);
        }
      } else {
...
__private_extern__ void pktap_bpf_tap(struct ifnet *ifp,
                                      protocol_family_t proto, struct mbuf *m,
  ...
  LIST_FOREACH(pktap, &pktap_list, pktp_link) {
    ...
    if (pktap->pktp_dlt_pkttap_count > 0) {
      ...
      static_assert(sizeof(hdr_buffer) ==
                    sizeof(struct pktap_header) + sizeof(u_int32_t));
      ...
      case IFT_OTHER:
        if (ifp->if_family == IFNET_FAMILY_IPSEC ||
          ...
          if (proto == AF_INET6 &&
              (size_t)m_pktlen(m) - 4 < sizeof(struct ip6_hdr)) {
            goto done;
          }

          /*
           * Handle two cases:
           * - The old utun encapsulation with the protocol family in network
           * order
      ...
      if (unknown_if_type) {
        ...
        if (m->m_pkthdr.pkt_ext_flags & PKTF_EXT_LPW ||
            if_is_lpw_enabled(ifp)) {
        ...
        if (data_adjust == 0) {
          bpf_tap_func(pktap->pktp_ifp, DLT_PKTAP, m, &hdr_buffer, hdr_size);
        } else {
          /*
           * We can play just with the length of the first mbuf in the
           * chain because bpf_tap_imp() disregard the packet length
           * of the mbuf packet header.
           */
          if (mbuf_setdata(m, m_mtod_current(m) + data_adjust,
                           m->m_len - data_adjust) == 0) {
            bpf_tap_func(pktap->pktp_ifp, DLT_PKTAP, m, &hdr_buffer, hdr_size);
            mbuf_setdata(m, m_mtod_current(m) - data_adjust,
                         m->m_len + data_adjust);
          }
        }
      }
...
__private_extern__ void pktap_input(struct ifnet *ifp, protocol_family_t proto,
                                    struct mbuf *m, char *frame_header) {
  char *hdr;
  char *start;
...
__private_extern__ void pktap_output(struct ifnet *ifp, protocol_family_t proto,
                                     struct mbuf *m, u_int32_t pre,
                                     u_int32_t post) {
  /* Fast path */
  if (pktap_total_tap_count == 0 ||
...
static void pktap_bpf_tap_packet(struct ifnet *ifp, protocol_family_t proto,
                                 uint32_t dlt, pid_t pid, const char *pname,
                                 pid_t epid, const char *epname,
                                 kern_packet_t pkt,
                                 const void *__sized_by(header_length) header,
                                 size_t header_length, uint8_t ipproto,
                                 uint32_t flowid, uint32_t flags,
                                 tap_packet_func tap_func) {
  struct {
  ...
  if (proto != AF_INET && proto != AF_INET6) {
    PKTAP_LOG(PKTP_LOG_ERROR, "unsupported protocol %d\n", proto);
    return;
  }
  ...
  hdr->pth_protocol_family = proto;
  hdr->pth_svc =
      so_svc2tc((mbuf_svc_class_t)kern_packet_get_service_class(pkt));
  hdr->pth_iftype = ifp->if_type;
  hdr->pth_ifunit = ifp->if_unit;
  hdr_size = sizeof(struct pktap_header);
  if (header != NULL && header_length != 0) {
    if (header_length > sizeof(hdr_buffer.extra.llhdr)) {
      PKTAP_LOG(PKTP_LOG_ERROR, "%s: header %d > %d\n", if_name(ifp),
                (int)header_length, (int)sizeof(hdr_buffer.extra.llhdr));
      return;
    }
  ...
  LIST_FOREACH(pktap, &pktap_list, pktp_link) {
    ...
    if (pktap->pktp_dlt_pkttap_count > 0) {
      (*tap_func)(pktap->pktp_ifp, DLT_PKTAP, pkt, &hdr_buffer, hdr_size);
    }
  }
  lck_rw_done(&pktap_lck_rw);
}

void pktap_input_packet(struct ifnet *ifp, protocol_family_t proto,
                        uint32_t dlt, pid_t pid, const char *pname, pid_t epid,
                        const char *epname, kern_packet_t pkt,
                        const void *__sized_by(header_length) header,
                        size_t header_length, uint8_t ipproto, uint32_t flowid,
                        uint32_t flags) {
  /* Fast path */
  if (pktap_total_tap_count == 0) {
    return;
  ...
  PKTAP_LOG(PKTP_LOG_INPUT, "IN %s proto %u pid %d epid %d\n", ifp->if_xname,
            proto, pid, epid);
  pktap_bpf_tap_packet(ifp, proto, dlt, pid, pname, epid, epname, pkt, header,
                       header_length, ipproto, flowid,
                       PTH_FLAG_DIR_IN |
                           (flags & ~(PTH_FLAG_DIR_IN | PTH_FLAG_DIR_OUT)),
                       bpf_tap_packet_in);
}

void pktap_output_packet(struct ifnet *ifp, protocol_family_t proto,
                         uint32_t dlt, pid_t pid, const char *pname, pid_t epid,
                         const char *epname, kern_packet_t pkt,
                         const void *__sized_by(header_length) header,
                         size_t header_length, uint8_t ipproto, uint32_t flowid,
...
void convert_to_pktap_header_to_v2(struct bpf_packet *bpf_pkt, bool truncate) {
   ...
   */
  extra_src_size = bpf_pkt->bpfp_header_length - sizeof(struct pktap_header);
  ...
  if (pktap_header->pth_ifname[0] != 0) {
    ...
    size_t remaining_space =
        (uintptr_t)(pktap_v2_hdr_space + 1) - (uintptr_t)ptr;
  ...
  if (extra_src_size > 0) {
    uint8_t *extra_src_ptr =
        (uint8_t *)bpf_pkt->bpfp_header + sizeof(*pktap_header);
    uint8_t *extra_dst_ptr =
        (uint8_t *)&pktap_buffer_v2_hdr_extra + pktap_v2_hdr->pth_length;
  ...
  bpf_pkt->bpfp_header_length +=
      pktap_v2_hdr->pth_length - sizeof(struct pktap_header);
