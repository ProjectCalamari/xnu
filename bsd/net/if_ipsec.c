...
static errno_t ipsec_ctl_getopt(kern_ctl_ref kctlref, u_int32_t unit,
                                void *unitinfo, int opt,
                                void *__sized_by(*len) data, size_t *len);
static errno_t ipsec_ctl_setopt(kern_ctl_ref kctlref, u_int32_t unit,
                                void *unitinfo, int opt,
                                void *__sized_by(len) data, size_t len);
...
static errno_t ipsec_proto_pre_output(ifnet_t interface,
                                      protocol_family_t protocol,
                                      mbuf_t *packet,
...
SYSCTL_INT(_net_ipsec, OID_AUTO, verify_interface_creation,
           CTLFLAG_RW | CTLFLAG_LOCKED, &if_ipsec_verify_interface_creation, 0,
           "");

#define IPSEC_IF_VERIFY(_e)                                                    \
  if (__improbable(if_ipsec_verify_interface_creation)) {                      \
    VERIFY(_e);                                                                \
  }
...
SYSCTL_INT(_net_ipsec, OID_AUTO, max_pending_input, CTLFLAG_LOCKED | CTLFLAG_RW,
           &if_ipsec_max_pending_input, 0, "");
SYSCTL_PROC(_net_ipsec, OID_AUTO, ring_size,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW, &if_ipsec_ring_size,
            IPSEC_IF_DEFAULT_RING_SIZE, &sysctl_if_ipsec_ring_size, "I", "");
SYSCTL_PROC(_net_ipsec, OID_AUTO, tx_fsw_ring_size,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW,
            &if_ipsec_tx_fsw_ring_size, IPSEC_IF_DEFAULT_TX_FSW_RING_SIZE,
            &sysctl_if_ipsec_tx_fsw_ring_size, "I", "");
SYSCTL_PROC(_net_ipsec, OID_AUTO, rx_fsw_ring_size,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW,
            &if_ipsec_rx_fsw_ring_size, IPSEC_IF_DEFAULT_RX_FSW_RING_SIZE,
            &sysctl_if_ipsec_rx_fsw_ring_size, "I", "");

static int if_ipsec_debug = 0;
SYSCTL_INT(_net_ipsec, OID_AUTO, debug, CTLFLAG_LOCKED | CTLFLAG_RW,
           &if_ipsec_debug, 0, "");

static errno_t ipsec_register_nexus(void);
...
struct ipsec_pcb {
  ...
  u_int32_t
      ipsec_pcb_data_path_state; /* internal state of interface data path */
...
#define IPSEC_SET_DATA_PATH_READY(_pcb)                                        \
  ((_pcb)->ipsec_pcb_data_path_state |= IPSEC_PCB_DATA_PATH_READY)
#define IPSEC_CLR_DATA_PATH_READY(_pcb)                                        \
  ((_pcb)->ipsec_pcb_data_path_state &= ~IPSEC_PCB_DATA_PATH_READY)
#define IPSEC_IS_DATA_PATH_READY(_pcb)                                         \
  (((_pcb)->ipsec_pcb_data_path_state & IPSEC_PCB_DATA_PATH_READY) != 0)

#if IPSEC_NEXUS
/* Macros to clear/set/test flags. */
static inline void ipsec_flag_set(struct ipsec_pcb *pcb, uint32_t flag) {
  pcb->ipsec_internal_flags |= flag;
}
static inline void ipsec_flag_clr(struct ipsec_pcb *pcb, uint32_t flag) {
  pcb->ipsec_internal_flags &= ~flag;
}

static inline bool ipsec_flag_isset(struct ipsec_pcb *pcb, uint32_t flag) {
...
#pragma unused(arg1, arg2)
  ...
  if (value < IPSEC_IF_MIN_RING_SIZE || value > IPSEC_IF_MAX_RING_SIZE) {
    return EINVAL;
  }
...
static inline bool ipsec_in_wmm_mode(struct ipsec_pcb *pcb) {
...
errno_t ipsec_register_control(void) {
  ...
  kern_ctl.ctl_flags =
      CTL_FLAG_PRIVILEGED | CTL_FLAG_REG_SETUP; /* Require root */
  ...
  if ((result = proto_register_plumber(PF_INET, IFNET_FAMILY_IPSEC,
    ...
    os_log_error(OS_LOG_DEFAULT,
                 "ipsec_register_control - proto_register_plumber(PF_INET, "
                 "IFNET_FAMILY_IPSEC) failed: %d\n",
                 result);
  ...
  if ((result = proto_register_plumber(PF_INET6, IFNET_FAMILY_IPSEC,
    ...
    ctl_deregister(ipsec_kctlref);
    os_log_error(OS_LOG_DEFAULT,
                 "ipsec_register_control - proto_register_plumber(PF_INET6, "
                 "IFNET_FAMILY_IPSEC) failed: %d\n",
                 result);
    return result;
  }
...
static errno_t ipsec_ifnet_set_attrs(ifnet_t ifp) {
   ...
   * if possible following the recommendation of RFC2472 to the 64bit interface
   * ID
   */
...
static errno_t ipsec_nxdp_init(__unused kern_nexus_domain_provider_t domprov) {
  return 0;
}

static void ipsec_nxdp_fini(__unused kern_nexus_domain_provider_t domprov) {
  // Ignore
}
...
static errno_t ipsec_register_nexus(void) {
  ...
  err = kern_nexus_register_domain_provider(
      NEXUS_TYPE_NET_IF, domain_provider_name, &dp_init, sizeof(dp_init),
      &ipsec_nx_dom_prov);
  if (err != 0) {
...
static errno_t ipsec_netif_prepare(kern_nexus_t nexus, ifnet_t ifp) {
  struct ipsec_pcb *__single pcb = kern_nexus_get_context(nexus);
  pcb->ipsec_netif_nexus = nexus;
  return ipsec_ifnet_set_attrs(ifp);
}

static errno_t ipsec_nexus_pre_connect(kern_nexus_provider_t nxprov, proc_t p,
                                       kern_nexus_t nexus,
                                       nexus_port_t nexus_port,
                                       kern_channel_t channel, void **ch_ctx) {
#pragma unused(nxprov, p)
#pragma unused(nexus, nexus_port, channel, ch_ctx)
  return 0;
}

static errno_t ipsec_nexus_connected(kern_nexus_provider_t nxprov,
                                     kern_nexus_t nexus,
...
static void ipsec_nexus_pre_disconnect(kern_nexus_provider_t nxprov,
                                       kern_nexus_t nexus,
                                       kern_channel_t channel) {
#pragma unused(nxprov, channel)
  struct ipsec_pcb *__single pcb = kern_nexus_get_context(nexus);
...
static void ipsec_netif_pre_disconnect(kern_nexus_provider_t nxprov,
                                       kern_nexus_t nexus,
                                       kern_channel_t channel) {
#pragma unused(nxprov, channel)
  struct ipsec_pcb *__single pcb = kern_nexus_get_context(nexus);
...
static void ipsec_nexus_disconnected(kern_nexus_provider_t nxprov,
                                     kern_nexus_t nexus,
                                     kern_channel_t channel) {
#pragma unused(nxprov, channel)
  struct ipsec_pcb *__single pcb = kern_nexus_get_context(nexus);
...
static errno_t ipsec_kpipe_ring_init(kern_nexus_provider_t nxprov,
...
#pragma unused(channel)
  ...
  for (ring_idx = 0; ring_idx < pcb->ipsec_kpipe_count; ring_idx++) {
    if (!uuid_compare(channel->ch_info->cinfo_nx_uuid,
                      pcb->ipsec_kpipe_uuid[ring_idx])) {
      break;
    }
  }
...
static void ipsec_kpipe_ring_fini(kern_nexus_provider_t nxprov,
                                  kern_nexus_t nexus,
                                  kern_channel_ring_t ring) {
#pragma unused(nxprov)
  bool found = false;
...
static errno_t ipsec_kpipe_sync_tx(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus,
                                   kern_channel_ring_t tx_ring,
                                   uint32_t flags) {
#pragma unused(nxprov)
#pragma unused(flags)
  struct ipsec_pcb *__single pcb = kern_nexus_get_context(nexus);
...
static errno_t ipsec_encrypt_kpipe_pkt(ifnet_t interface, kern_packet_t sph,
                                       kern_packet_t dph) {
  uint8_t *sbaddr = NULL;
  int err = 0;
  uint32_t slen = 0;
  ...
  if (__improbable(slen < sizeof(struct ip))) {
    os_log_error(OS_LOG_DEFAULT,
                 "ipsec encrypt kpipe pkt: source "
...
static errno_t ipsec_kpipe_sync_rx(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus,
                                   kern_channel_ring_t rx_ring,
                                   uint32_t flags) {
#pragma unused(nxprov)
#pragma unused(flags)
  struct ipsec_pcb *__single pcb = kern_nexus_get_context(nexus);
  ...
  if (avail == 0) {
    ...
    os_log_error(OS_LOG_DEFAULT, "%s: %s ring %s index %d no room in rx_ring\n",
                 __func__, pcb->ipsec_if_xname, rx_ring->ckr_name, ring_idx);
    ipsec_data_move_end(pcb);
    return 0;
  }
  ...
  struct netif_stats *nifs =
      &NX_NETIF_PRIVATE(pcb->ipsec_netif_nexus)->nif_stats;
  ...
  while (rx_slot != NULL && tx_slot != NULL) {
    ...
    if (tx_pkt_length == 0 || tx_pkt_length > pcb->ipsec_slot_size) {
      os_log_info(OS_LOG_DEFAULT,
                  "ipsec_kpipe_sync_rx %s: "
                  "packet length %zu",
                  pcb->ipsec_ifp->if_xname, tx_pkt_length);
      kern_pbufpool_free(rx_pp, rx_ph);
  ...
  if (rx_pslot) {
    ...
    kern_channel_increment_ring_net_stats(rx_ring, pcb->ipsec_ifp,
                                          &rx_ring_stats);
  }
  ...
  if (tx_pslot) {
    ...
    kern_channel_increment_ring_net_stats(tx_ring, pcb->ipsec_ifp,
                                          &tx_ring_stats);
...
static uint8_t ipsec_find_tx_ring_by_svc(kern_packet_svc_class_t svc_class) {
  switch (svc_class) {
  case KPKT_SC_VO: {
    return 0;
  }
...
static errno_t ipsec_netif_ring_init(kern_nexus_provider_t nxprov,
                                     kern_nexus_t nexus, kern_channel_t channel,
                                     kern_channel_ring_t ring,
                                     boolean_t is_tx_ring, void **ring_ctx) {
#pragma unused(nxprov)
...
static void ipsec_netif_ring_fini(kern_nexus_provider_t nxprov,
                                  kern_nexus_t nexus,
                                  kern_channel_ring_t ring) {
#pragma unused(nxprov)
  struct ipsec_pcb *__single pcb = kern_nexus_get_context(nexus);
...
static bool ipsec_netif_check_policy(ifnet_t interface, mbuf_t data) {
  ...
  if (data && data->m_pkthdr.necp_mtag.necp_policy_id >=
  ...
  case 4: {
    ...
    necp_matched_policy_id = necp_ip_output_find_policy_match(
        data, IP_OUTARGS, &args4, NULL, &necp_result, &necp_result_parameter);
    break;
  }
  case 6: {
    if (interface != NULL) {
    ...
    necp_matched_policy_id = necp_ip6_output_find_policy_match(
        data, IPV6_OUTARGS, &args6, NULL, &necp_result, &necp_result_parameter);
    break;
  }
  default: {
    return false;
...
static errno_t ipsec_netif_sync_tx(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus,
                                   kern_channel_ring_t tx_ring,
                                   uint32_t flags) {
#pragma unused(nxprov)
#pragma unused(flags)
  ...
  while (tx_slot != NULL) {
    ...
    length = MIN(kern_packet_get_data_length(tx_ph), pcb->ipsec_slot_size);
...
static errno_t ipsec_netif_tx_doorbell_one(kern_nexus_provider_t nxprov,
                                           kern_nexus_t nexus,
                                           kern_channel_ring_t ring,
                                           uint32_t flags, uint8_t ring_idx) {
#pragma unused(nxprov)
  struct ipsec_pcb *__single pcb = kern_nexus_get_context(nexus);
   ...
   * variant here.
   */
  rc = kern_channel_tx_refill_canblock(ring, UINT32_MAX, UINT32_MAX, true,
                                       &more);
  if (rc != 0 && rc != EAGAIN && rc != EBUSY) {
    os_log_error(OS_LOG_DEFAULT, "%s: %s ring %s tx refill failed %d\n",
                 __func__, pcb->ipsec_if_xname, ring->ckr_name, rc);
  }
  ...
  if (ring != pcb->ipsec_netif_txring[ring_idx]) {
    ...
    os_log_error(OS_LOG_DEFAULT, "%s: %s ring %s index %d bad netif_txring 3\n",
                 __func__, pcb->ipsec_if_xname, ring->ckr_name, ring_idx);
    return ENXIO;
  }
...
static errno_t ipsec_netif_tx_doorbell(kern_nexus_provider_t nxprov,
                                       kern_nexus_t nexus,
                                       kern_channel_ring_t ring,
...
static errno_t ipsec_transform_kpipe_pkt_to_netif_pkt(
  ...
  if (kpipe_buf_len > pcb->ipsec_slot_size) {
    os_log_info(
        ...
        "%u > pcb ipsec slot size %u",
        pcb->ipsec_ifp->if_xname, kpipe_buf_len, pcb->ipsec_slot_size);
    STATS_INC(nifs, NETIF_STATS_DROP_BADLEN);
    err = EMSGSIZE;
    goto bad;
  }
  ...
  if (__improbable(kpipe_buf_len < sizeof(struct ip))) {
    os_log_info(OS_LOG_DEFAULT,
                "ipsec_transform_kpipe_pkt_to_netif_pkt %s - bad "
                "packet length %u\n",
  ...
  if (__improbable(netif_buf_lim < iphlen)) {
    os_log_info(OS_LOG_DEFAULT,
                "ipsec_transform_kpipe_pkt_to_netif_pkt %s - netif "
  ...
  if (__improbable((id_8[0] & IPSEC_KPIPE_FLAG_WAKE_PKT) ==
                   IPSEC_KPIPE_FLAG_WAKE_PKT)) {
    os_log_info(OS_LOG_DEFAULT,
                "ipsec_transform_kpipe_pkt_to_netif_pkt %s: wake packet "
                "flag is set\n",
                pcb->ipsec_ifp->if_xname);
...
static errno_t ipsec_netif_sync_rx(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus,
                                   kern_channel_ring_t rx_ring,
                                   uint32_t flags) {
#pragma unused(nxprov)
#pragma unused(flags)
  ...
  while (rx_slot != NULL) {
    ...
    if (length < sizeof(struct ip)) {
      ...
      STATS_INC(nifs, NETIF_STATS_DROP);
      os_log_error(OS_LOG_DEFAULT,
                   "ipsec_netif_sync_rx %s: legacy decrypted packet length "
                   "cannot hold IP %zu < %zu\n",
    ...
    if (length > PP_BUF_SIZE_DEF(rx_pp) ||
      ...
      case AF_INET: {
        ...
        int fragment_error =
            ip_fragment(data, pcb->ipsec_ifp, fragment_mtu, TRUE);
        if (fragment_error == 0 && data != NULL) {
          ...
          STATS_INC(nifs, NETIF_STATS_DROP);
          os_log_error(OS_LOG_DEFAULT,
                       "ipsec_netif_sync_rx %s: failed to fragment IPv4 packet "
                       "of length %zu (%d)\n",
      ...
      case AF_INET6: {
        if (length < sizeof(struct ip6_hdr)) {
          ...
          STATS_INC(nifs, NETIF_STATS_DROP);
          os_log_error(OS_LOG_DEFAULT,
                       "ipsec_netif_sync_rx %s: failed to fragment IPv6 packet "
                       "of length %zu < %zu\n",
          ...
          int fragment_error = ip6_do_fragmentation(
              &data, 0, pcb->ipsec_ifp, sizeof(struct ip6_hdr), ip6, NULL,
              fragment_mtu, ip6->ip6_nxt, htonl(ip6_randomid((uint64_t)data)));
          if (fragment_error == 0 && data != NULL) {
            fragment_chain = data;
          } else {
            mbuf_freem(data);
            ...
            STATS_INC(nifs, NETIF_STATS_DROP);
            os_log_error(OS_LOG_DEFAULT,
                         "ipsec_netif_sync_rx %s: failed to fragment IPv6 "
                         "packet of length %zu (%d)\n",
                         pcb->ipsec_ifp->if_xname, length, fragment_error);
          }
        }
        break;
      ...
      default: {
        ...
        STATS_INC(nifs, NETIF_STATS_DROP);
        os_log_error(OS_LOG_DEFAULT,
                     "ipsec_netif_sync_rx %s: uknown legacy decrypted packet "
                     "length %zu > %u\n",
  ...
  for (uint8_t ring_idx = 0; ring_idx < pcb->ipsec_kpipe_count; ring_idx++) {
    ...
    while (rx_slot != NULL && tx_slot != NULL) {
      ...
      error = ipsec_transform_kpipe_pkt_to_netif_pkt(pcb, &tx_ring_stats, nifs,
                                                     tx_ph, rx_ph);
      if (error != 0) {
      ...
      rx_ring_stats.kcrsi_slots_transferred++;
      rx_ring_stats.kcrsi_bytes_transferred +=
          kern_packet_get_data_length(rx_ph);
...
static errno_t ipsec_nexus_ifattach(struct ipsec_pcb *pcb,
  ...
  pp_init.kbi_packets =
      pcb->ipsec_netif_ring_size * (2 * pcb->ipsec_kpipe_count + 1);
  ...
  err = kern_nexus_controller_register_provider(
      controller, ipsec_nx_dom_prov, provider_name, &prov_init,
      sizeof(prov_init), nxa, &pcb->ipsec_nx.if_provider);
  IPSEC_IF_VERIFY(err == 0);
  if (err != 0) {
  ...
  err = kern_nexus_controller_alloc_net_provider_instance(
      controller, pcb->ipsec_nx.if_provider, pcb, NULL,
      &pcb->ipsec_nx.if_instance, &net_init, ifp);
  IPSEC_IF_VERIFY(err == 0);
  if (err != 0) {
...
static void ipsec_detach_provider_and_instance(uuid_t provider,
                                               uuid_t instance) {
  nexus_controller_t controller = kern_nexus_shared_controller();
  errno_t err;
...
static void ipsec_nexus_detach(struct ipsec_pcb *pcb) {
  ipsec_nx_t nx = &pcb->ipsec_nx;
  nexus_controller_t controller = kern_nexus_shared_controller();
  errno_t err;

  if (!uuid_is_null(nx->fsw_device)) {
    err = kern_nexus_ifdetach(controller, nx->fsw_instance, nx->fsw_device);
    if (err != 0) {
      os_log_error(OS_LOG_DEFAULT,
  ...
  ipsec_detach_provider_and_instance(nx->fsw_provider, nx->fsw_instance);
  ipsec_detach_provider_and_instance(nx->if_provider, nx->if_instance);
...
static errno_t ipsec_create_fs_provider_and_instance(struct ipsec_pcb *pcb,
  ...
  err =
      kern_nexus_get_default_domain_provider(NEXUS_TYPE_FLOW_SWITCH, &dom_prov);
  IPSEC_IF_VERIFY(err == 0);
  if (err != 0) {
  ...
  // Reset ring size for flowswitch nexus to limit memory usage. Larger RX than
  // netif.
   ...
   */
  err =
      kern_nexus_attr_set(attr, NEXUS_ATTR_MAX_FRAGS,
...
static errno_t ipsec_flowswitch_attach(struct ipsec_pcb *pcb) {
  nexus_controller_t controller = kern_nexus_shared_controller();
  errno_t err = 0;
  ipsec_nx_t nx = &pcb->ipsec_nx;

  // Allocate flowswitch
  err = ipsec_create_fs_provider_and_instance(
      pcb, "flowswitch", pcb->ipsec_ifp->if_xname, &nx->fsw_provider,
      &nx->fsw_instance);
  if (err != 0) {
    os_log_error(OS_LOG_DEFAULT,
  ...
  err = kern_nexus_ifattach(controller, nx->fsw_instance, NULL, nx->if_instance,
                            FALSE, &nx->fsw_device);
  if (err != 0) {
...
static errno_t ipsec_register_kernel_pipe_nexus(struct ipsec_pcb *pcb) {
  nexus_attr_t __single nxa = NULL;
  ...
  uuid_t dom_prov;
  result =
      kern_nexus_get_default_domain_provider(NEXUS_TYPE_KERNEL_PIPE, &dom_prov);
  if (result) {
    os_log_error(OS_LOG_DEFAULT,
  ...
  ring_size = pcb->ipsec_kpipe_rx_ring_size != 0 ? pcb->ipsec_kpipe_rx_ring_size
              : pcb->ipsec_netif_ring_size != 0  ? pcb->ipsec_netif_ring_size
                                                 : if_ipsec_ring_size;
  result = kern_nexus_attr_set(nxa, NEXUS_ATTR_RX_SLOTS, ring_size);
  VERIFY(result == 0);

  result = kern_nexus_controller_register_provider(
      ipsec_ncd, dom_prov, provider_name, &prov_init, sizeof(prov_init), nxa,
      &ipsec_kpipe_uuid);
  if (result) {
    os_log_error(OS_LOG_DEFAULT,
...
static void ipsec_unregister_kernel_pipe_nexus(void) {
  lck_mtx_lock(&ipsec_lock);
...
static void ipsec_detach_channels(struct ipsec_pcb *pcb,
...
static void ipsec_free_channels(struct ipsec_detached_channels *dc) {
  if (!dc->count) {
    return;
  }
  ...
  for (int i = 0; i < dc->count; i++) {
    errno_t result;
    result =
        kern_nexus_controller_free_provider_instance(ipsec_ncd, dc->uuids[i]);
    VERIFY(!result);
  }
...
static errno_t ipsec_enable_channel(struct ipsec_pcb *pcb, struct proc *proc) {
  struct kern_nexus_init init;
  struct kern_pbufpool_init pp_init;
  uuid_t uuid_null = {0};
  errno_t result;
  ...
  result = kern_pbufpool_create(&pp_init, &pcb->ipsec_kpipe_pp, NULL);
  if (result != 0) {
  ...
  for (unsigned int i = 0; i < pcb->ipsec_kpipe_count; i++) {
    VERIFY(uuid_is_null(pcb->ipsec_kpipe_uuid[i]));
    result = kern_nexus_controller_alloc_provider_instance(
        ipsec_ncd, ipsec_kpipe_uuid, pcb, NULL, &pcb->ipsec_kpipe_uuid[i],
        &init);
    ...
    if (result == 0) {
      ...
      result = kern_nexus_controller_bind_provider_instance(
          ipsec_ncd, (const uint8_t *)pcb->ipsec_kpipe_uuid[i], &port, pid,
          has_proc_uuid ? pcb->ipsec_kpipe_proc_uuid : uuid_null, NULL, 0,
          has_proc_uuid ? NEXUS_BIND_EXEC_UUID : NEXUS_BIND_PID);
    }
    ...
    if (result) {
      ...
      for (int j = 0; j < IPSEC_IF_MAX_RING_COUNT; j++) {
        if (!uuid_is_null(pcb->ipsec_kpipe_uuid[j])) {
          kern_nexus_controller_free_provider_instance(
              ipsec_ncd, pcb->ipsec_kpipe_uuid[j]);
...
static inline int ipsec_find_by_unit(u_int32_t unit) {
...
static inline void ipsec_free_pcb(struct ipsec_pcb *pcb, bool locked) {
#if IPSEC_NEXUS
...
static errno_t ipsec_ctl_connect(kern_ctl_ref kctlref, struct sockaddr_ctl *sac,
  ...
  /* Handle case where ipsec_ctl_setup() was called, but ipsec_ctl_bind() was
   * not */
  if (pcb->ipsec_ctlref == NULL) {
    (void)ipsec_ctl_bind(kctlref, sac, unitinfo);
  }

  snprintf(pcb->ipsec_if_xname, sizeof(pcb->ipsec_if_xname), "ipsec%d",
           pcb->ipsec_unit - 1);
  snprintf(pcb->ipsec_unique_name, sizeof(pcb->ipsec_unique_name), "ipsecid%d",
           pcb->ipsec_unique_id - 1);
  ...
  if (if_ipsec_debug != 0) {
    printf("%s: %s%d use_netif %d kpipe_count %d slot_size %u ring_size %u "
           ...
           __func__, ipsec_init.name, ipsec_init.unit, pcb->ipsec_use_netif,
           pcb->ipsec_kpipe_count, pcb->ipsec_slot_size,
           pcb->ipsec_netif_ring_size, pcb->ipsec_kpipe_tx_ring_size,
  ...
  if (pcb->ipsec_use_netif) {
    ...
    if (result != 0) {
      os_log_error(OS_LOG_DEFAULT,
                   ...
                   result);
      // Do not call ipsec_free_pcb(). We will be attached already, and will be
      // freed later in ipsec_detached().
      *unitinfo = NULL;
      return result;
    }
  ...
  /*
   * Mark the data path as ready.
   * If kpipe nexus is being used then the data path is marked ready only when a
   * kpipe channel is connected.
   */
...
static errno_t ipsec_detach_ip(ifnet_t interface, protocol_family_t protocol,
                               socket_t pf_socket) {
  ...
  if (protocol == PF_INET) {
    ...
    snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s%d", ifnet_name(interface),
...
static void ipsec_remove_address(ifnet_t interface, protocol_family_t protocol,
                                 ifaddr_t address, socket_t pf_socket) {
  errno_t result = 0;
...
static void ipsec_cleanup_family(ifnet_t interface,
                                 protocol_family_t protocol) {
  errno_t result = 0;
   ...
   */
  result = ifnet_get_address_list_family_with_count(
      interface, &addresses, &addresses_count, (sa_family_t)protocol);
  if (result != 0) {
    os_log_error(
        OS_LOG_DEFAULT,
...
static errno_t ipsec_ctl_disconnect(__unused kern_ctl_ref kctlref,
                                    __unused u_int32_t unit, void *unitinfo) {
  struct ipsec_pcb *__single pcb = unitinfo;
  ifnet_t ifp = NULL;
  errno_t result = 0;
...
static errno_t ipsec_ctl_send(__unused kern_ctl_ref kctlref,
                              __unused u_int32_t unit, __unused void *unitinfo,
...
static errno_t ipsec_ctl_setopt(__unused kern_ctl_ref kctlref,
                                __unused u_int32_t unit, void *unitinfo,
                                int opt, void *__sized_by(len) data,
                                size_t len) {
  errno_t result = 0;
  struct ipsec_pcb *__single pcb = unitinfo;
  if (pcb == NULL) {
  ...
  case IPSEC_OPT_INC_IFDATA_STATS_OUT: {
    ...
    if (opt == IPSEC_OPT_INC_IFDATA_STATS_IN) {
      ifnet_stat_increment_in(pcb->ipsec_ifp, (uint32_t)utsp->utsp_packets,
                              (uint32_t)utsp->utsp_bytes,
                              (uint32_t)utsp->utsp_errors);
    } else {
  ...
  case IPSEC_OPT_SET_DELEGATE_INTERFACE: {
    ...
    if (len != 0) { /* if len==0, del_ifp will be NULL causing the delegate to
                       be removed */
      bcopy(data, name, len);
      name[len] = 0;
      result = ifnet_find_by_name(__unsafe_null_terminated_from_indexable(name),
                                  &del_ifp);
    }
    if (result == 0) {
      os_log_error(OS_LOG_DEFAULT,
  ...
  case IPSEC_OPT_OUTPUT_TRAFFIC_CLASS: {
    ...
    os_log_error(OS_LOG_DEFAULT,
                 ...
                 pcb->ipsec_ifp->if_xname, pcb->ipsec_output_service_class);
    break;
  }
  ...
  case IPSEC_OPT_ENABLE_FLOWSWITCH: {
    ...
    if (*(int *)data) {
      flags |= (NETAGENT_FLAG_NEXUS_PROVIDER | NETAGENT_FLAG_NEXUS_LISTENER);
  ...
  case IPSEC_OPT_OUTPUT_DSCP_MAPPING: {
    ...
    ipsec_dscp_mapping_t output_dscp_mapping =
        (ipsec_dscp_mapping_t)(*(int *)data);
    ...
    os_log(OS_LOG_DEFAULT, "%s IPSEC_OPT_OUTPUT_DSCP_MAPPING %s DSCP %d\n",
           __func__, pcb->ipsec_ifp->if_xname, pcb->ipsec_output_dscp_mapping);
    break;
  }
...
static errno_t ipsec_ctl_getopt(__unused kern_ctl_ref kctlref,
                                __unused u_int32_t unit, void *unitinfo,
                                int opt, void *__sized_by(*len) data,
                                size_t *len) {
  errno_t result = 0;
  ...
  case IPSEC_OPT_IFNAME: {
    if (*len <
        MIN(strbuflen(pcb->ipsec_if_xname, sizeof(pcb->ipsec_if_xname)) + 1,
            sizeof(pcb->ipsec_if_xname))) {
      result = EMSGSIZE;
    } else {
      if (pcb->ipsec_ifp == NULL) {
...
static errno_t ipsec_output(ifnet_t interface, mbuf_t data) {
  ...
  case 4: {
    ...
    flags =
        (IP_OUTARGS | /* Passing out args to specify interface */
  ...
  case 6: {
    if (data->m_len < sizeof(*ip6)) {
      os_log_error(OS_LOG_DEFAULT,
                   "ipsec_output: first mbuf length shorter than IPv6 header "
                   "length: %d.\n",
                   data->m_len);
    ...
    error = ipsec6_interface_output(&ipsec_state, interface, &ip6->ip6_nxt,
                                    ipsec_state.m);
    if (error == 0 &&
        ipsec_state.tunneled == 4) { /* tunneled in IPv4 - packet is gone */
      goto done;
    }
    data = ipsec_state.m;
    if (error || data == NULL) {
...
/* Network Interface functions */
static errno_t ipsec_demux(__unused ifnet_t interface, mbuf_t data,
...
static errno_t
ipsec_add_proto(__unused ifnet_t interface, protocol_family_t protocol,
...
static errno_t ipsec_del_proto(__unused ifnet_t interface,
                               __unused protocol_family_t protocol) {
  return 0;
}

static errno_t ipsec_ioctl(ifnet_t interface, u_long command, void *data) {
#if IPSEC_NEXUS
  struct ipsec_pcb *__single pcb = ifnet_softc(interface);
#endif
  errno_t result = 0;
  ...
  case SIOCSIFPEEREGRESSFUNCTIONALTYPE: {
    ...
    peeregressinterfacetype =
        ((struct ifreq *)data)->ifr_ifru.ifru_peer_egress_functional_type;
...
static errno_t ipsec_proto_pre_output(__unused ifnet_t interface,
                                      protocol_family_t protocol,
...
static errno_t ipsec_attach_proto(ifnet_t interface,
                                  protocol_family_t protocol) {
...
void ipsec_set_pkthdr_for_interface(ifnet_t interface, mbuf_t packet,
                                    int family, uint32_t flowid) {
...
void ipsec_set_ip6oa_for_interface(ifnet_t interface,
                                   struct ip6_out_args *ip6oa) {
  struct ipsec_pcb *__single pcb;
...
static void ipsec_data_move_drain(struct ipsec_pcb *pcb) {
  lck_mtx_lock(&pcb->ipsec_pcb_data_move_lock);
