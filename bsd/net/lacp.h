 ...
 */
static __inline__ void lacp_uint16_set(uint8_t *__header_indexable field,
                                       uint16_t value) {
 ...
 *   value, putting it into network byte order
 */
static __inline__ void lacp_uint32_set(uint8_t *__header_indexable field,
                                       uint32_t value) {
 ...
 */
static __inline__ void lacp_actor_partner_tlv_set_system_priority(
    lacp_actor_partner_tlv_ref tlv, lacp_system_priority system_priority) {
  lacp_uint16_set(tlv->lap_system_priority, system_priority);
  return;
}

static __inline__ lacp_system_priority
lacp_actor_partner_tlv_get_system_priority(
    const lacp_actor_partner_tlv_ref tlv) {
...
static __inline__ void
lacp_actor_partner_tlv_set_port(lacp_actor_partner_tlv_ref tlv,
                                lacp_port port) {
...
} la_marker_pdu, *la_marker_pdu_ref, la_marker_response_pdu,
    *la_marker_response_pdu_ref;

static __inline__ void la_marker_pdu_set_requestor_port(la_marker_pdu_ref lmpdu,
                                                        lacp_port port) {
...
la_marker_pdu_get_requestor_transaction_id(la_marker_pdu_ref lmpdu) {
  return (la_marker_transaction_id)lacp_uint32_get(
      lmpdu->lm_requestor_transaction_id);
}
