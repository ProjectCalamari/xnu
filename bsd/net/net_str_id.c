...
static u_int32_t nsi_kind_next[NSI_MAX_KIND] = {
    FIRST_NET_STR_ID, FIRST_NET_STR_ID, FIRST_NET_STR_ID};
static u_int32_t nsi_next_id = FIRST_NET_STR_ID;

__private_extern__ void net_str_id_first_last(u_int32_t *first, u_int32_t *last,
                                              u_int32_t kind) {
...
__private_extern__ errno_t net_str_id_find_internal(const char *string,
                                                    u_int32_t *out_id,
                                                    u_int32_t kind,
                                                    int create) {
  ...
  *out_id = 0;
  ...
  if (entry == NULL) {
    ...
    entry = zalloc_permanent(sizeof(*entry) + string_length, ZALIGN_PTR);
    if (entry == NULL) {
      lck_mtx_unlock(&net_str_id_lock);
      return ENOMEM;
    }
