...
SYSCTL_INT(_net_link_generic_system_low_power, OID_AUTO, verbose,
           CTLFLAG_RW | CTLFLAG_LOCKED, &if_low_power_verbose, 0, "");
SYSCTL_INT(_net_link_generic_system_low_power, OID_AUTO, restricted,
           CTLFLAG_RW | CTLFLAG_LOCKED, &if_low_power_restricted, 0, "");
...
if_low_power_evhdlr_callback(__unused struct eventhandler_entry_arg arg,
  ...
  if (if_low_power_verbose > 0) {
    os_log_info(OS_LOG_DEFAULT, "%s: ifp %s event_code %d", __func__,
  ...
  dlil_post_msg(ifp, KEV_DL_SUBCLASS, KEV_DL_LOW_POWER_MODE_CHANGED,
                (struct net_event_data *)&kev,
                sizeof(struct kev_dl_low_power_mode), FALSE);
}

void if_low_power_evhdlr_init(void) {
  eventhandler_lists_ctxt_init(&if_low_power_evhdlr_ctx);

  (void)EVENTHANDLER_REGISTER(&if_low_power_evhdlr_ctx, if_low_power_event,
                              &if_low_power_evhdlr_callback,
                              eventhandler_entry_dummy_arg,
                              EVENTHANDLER_PRI_ANY);
}

int if_set_low_power(ifnet_t ifp, bool on) {
  int error = 0;
  ...
  os_log(OS_LOG_DEFAULT, "%s: ifp %s low_power mode %d", __func__, if_name(ifp),
         on);
