...
static inline u_int32_t getblock32(const u_int32_t *__bidi_indexable p, int i) {
  return p[i];
}

static inline u_int64_t getblock64(const u_int64_t *__bidi_indexable p, int i) {
  return p[i];
}
...
static inline u_int32_t getblock32(const u_int32_t *__bidi_indexable p, int i) {
  ...
  if (ALIGNED32(p)) {
    ...
    value = (((u_int32_t)bytes[0]) << 24) | (((u_int32_t)bytes[1]) << 16) |
            (((u_int32_t)bytes[2]) << 8) | ((u_int32_t)bytes[3]);
#else  /* LITTLE_ENDIAN */
    value = (((u_int32_t)bytes[3]) << 24) | (((u_int32_t)bytes[2]) << 16) |
            (((u_int32_t)bytes[1]) << 8) | ((u_int32_t)bytes[0]);
#endif /* LITTLE_ENDIAN */
  }
  return value;
}
...
#if BYTE_ORDER == BIG_ENDIAN
    value = (((u_int64_t)bytes[0]) << 56) | (((u_int64_t)bytes[1]) << 48) |
            (((u_int64_t)bytes[2]) << 40) | (((u_int64_t)bytes[3]) << 32) |
            (((u_int64_t)bytes[4]) << 24) | (((u_int64_t)bytes[5]) << 16) |
            (((u_int64_t)bytes[6]) << 8) | ((u_int64_t)bytes[7]);
#else  /* LITTLE_ENDIAN */
    value = (((u_int64_t)bytes[7]) << 56) | (((u_int64_t)bytes[6]) << 48) |
            (((u_int64_t)bytes[5]) << 40) | (((u_int64_t)bytes[4]) << 32) |
            (((u_int64_t)bytes[3]) << 24) | (((u_int64_t)bytes[2]) << 16) |
            (((u_int64_t)bytes[1]) << 8) | ((u_int64_t)bytes[0]);
#endif /* LITTLE_ENDIAN */
  }
  return value;
}
...
static inline u_int32_t mh3_fmix32(u_int32_t h) {
  h ^= h >> 16;
  h *= 0x85ebca6b;
  h ^= h >> 13;
  h *= 0xc2b2ae35;
...
static inline u_int64_t mh3_fmix64(u_int64_t k) {
  k ^= k >> 33;
...
u_int32_t net_flowhash_mh3_x86_32(const void *__sized_by(len) key,
                                  u_int32_t len, const u_int32_t seed) {
...
u_int32_t net_flowhash_mh3_x64_128(const void *__sized_by(len) key,
                                   u_int32_t len, const u_int32_t seed) {
  const u_int8_t *data = (const u_int8_t *)key;
 ...
 * hashbig()
 */
u_int32_t net_flowhash_jhash(const void *__sized_by(len) key, u_int32_t len,
