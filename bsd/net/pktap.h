 ...
 */
#define PKTP_CMD_FILTER_GET                                                    \
  1 /* array of PKTAP_MAX_FILTERS * struct pktap_filter */
#define PKTP_CMD_FILTER_SET                                                    \
  3 /* array of PKTAP_MAX_FILTERS * struct pktap_filter */
#define PKTP_CMD_TAP_COUNT                                                     \
  4 /* uint32_t number of active bpf tap on the interface */
 ...
 *
 * In theory, there could be several types of blocks in a chain before the
 * actual packet
 */
struct pktap_header {
...
#define COPY_PKTAP_COMMON_FIELDS_TO_V2(pktap_v2_hdr_dst, pktap_header_src)     \
  {                                                                            \
    ...
    (pktap_v2_hdr_dst)->pth_frame_pre_length =                                 \
        (uint16_t)(pktap_header_src)->pth_frame_pre_length;                    \
    (pktap_v2_hdr_dst)->pth_frame_post_length =                                \
        (uint16_t)(pktap_header_src)->pth_frame_post_length;                   \
    (pktap_v2_hdr_dst)->pth_iftype = (pktap_header_src)->pth_iftype;           \
    (pktap_v2_hdr_dst)->pth_ipproto =                                          \
        (uint16_t)(pktap_header_src)->pth_ipproto;                             \
    (pktap_v2_hdr_dst)->pth_protocol_family =                                  \
        (pktap_header_src)->pth_protocol_family;                               \
...
#define PTH_FLAG_WAKE_PKT                                                      \
  0x00100000                     /* Packet caused system to ake from sleep */
...
extern void pktap_input_packet(struct ifnet *, protocol_family_t, uint32_t,
                               pid_t, const char *, pid_t, const char *,
                               kern_packet_t,
                               const void *__sized_by(header_len) header,
                               size_t header_len, uint8_t, uint32_t, uint32_t);
...
extern void pktap_input(struct ifnet *, protocol_family_t, struct mbuf *,
                        char *);
...
extern void convert_to_pktap_header_to_v2(struct bpf_packet *bpf_pkt,
                                          bool truncate);
#endif /* BSD_KERNEL_PRIVATE */
#endif /* PRIVATE */
