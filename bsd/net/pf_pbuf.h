...
enum pbuf_type { PBUF_TYPE_ZOMBIE = 0, PBUF_TYPE_MBUF, PBUF_TYPE_MEMORY };

enum pbuf_action { PBUF_ACTION_DESTROY };
...
void pbuf_init_memory(pbuf_t *, const struct pbuf_memory *, struct ifnet *);
...
void pbuf_copy_data(pbuf_t *, int, int, void *__sized_by(buflen),
                    size_t buflen);
void pbuf_copy_back(pbuf_t *, int, int, void *__sized_by(buflen),
                    size_t buflen);
