...
#define DLIL_EWMA(old, new, decay)                                             \
  do {                                                                         \
    u_int32_t _avg;                                                            \
    if ((_avg = (old)) > 0)                                                    \
...
static errno_t dlil_input_async(struct dlil_threading_info *inp,
                                struct ifnet *ifp, struct mbuf *m_head,
                                struct mbuf *m_tail,
                                const struct ifnet_stat_increment_param *s,
                                boolean_t poll, struct thread *tp);
static errno_t dlil_input_sync(struct dlil_threading_info *inp,
                               struct ifnet *ifp, struct mbuf *m_head,
                               struct mbuf *m_tail,
                               const struct ifnet_stat_increment_param *s,
                               boolean_t poll, struct thread *tp);
static void dlil_input_cksum_dbg(struct ifnet *ifp, struct mbuf *m,
                                 char *frame_header, protocol_family_t pf);
static void dlil_input_packet_list_common(struct ifnet *, mbuf_ref_t, u_int32_t,
                                          ifnet_model_t, boolean_t);
...
static int dlil_interface_filters_input(struct ifnet *, mbuf_ref_ref_t, char **,
                                        protocol_family_t, boolean_t);
...
static uint32_t dlil_trim_overcomitted_queue_locked(
    class_queue_t *input_queue, dlil_freeq_t *freeq,
    struct ifnet_stat_increment_param *stat_delta);

static inline mbuf_t handle_bridge_early_input(ifnet_t ifp, mbuf_t m,
                                               u_int32_t cnt);
/*
 * Publicly visible functions.
 */

int dlil_create_input_thread(ifnet_t ifp, struct dlil_threading_info *inp,
  ...
  dlil_rxpoll_input =
      (ifp != NULL && net_rxpoll && (ifp->if_eflags & IFEF_RXPOLL) &&
  ...
  if (ifp == NULL) {
    ...
    inp->dlth_name = tsnprintf(inp->dlth_name_storage,
                               sizeof(inp->dlth_name_storage), "main_input");
    ...
    VERIFY(inp != dlil_main_input_thread);
    inp->dlth_name =
        tsnprintf(inp->dlth_name_storage, sizeof(inp->dlth_name_storage),
    ...
    inp->dlth_name =
        __unsafe_null_terminated_from_indexable(inp->dlth_name_storage);
  }
  ...
  if (error == KERN_SUCCESS) {
    ...
    info.importance = 0;
    kret = thread_policy_set(inp->dlth_thread, THREAD_PRECEDENCE_POLICY,
    ...
    panic_plain("%s: couldn't create %s input thread", __func__, if_name(ifp));
    /* NOTREACHED */
  }
...
void dlil_terminate_input_thread(struct dlil_threading_info *inp) {
  ...
  { /* do something useless that won't get optimized away */
    uint32_t v = 1;
    for (uint32_t i = 0; i < if_input_thread_termination_spin; i++) {
      v = (i + 1) * v;
    }
...
boolean_t dlil_is_rxpoll_input(thread_continue_t func) {
  return func == dlil_rxpoll_input_thread_func;
}

errno_t dlil_input_handler(struct ifnet *ifp, struct mbuf *m_head,
                           struct mbuf *m_tail,
                           const struct ifnet_stat_increment_param *s,
                           boolean_t poll, struct thread *tp) {
...
__private_extern__ void dlil_input_packet_list(struct ifnet *ifp,
                                               struct mbuf *m) {
  return dlil_input_packet_list_common(ifp, m, 0, IFNET_MODEL_INPUT_POLL_OFF,
                                       FALSE);
}

__private_extern__ void dlil_input_packet_list_extended(struct ifnet *ifp,
                                                        struct mbuf *m,
                                                        u_int32_t cnt,
                                                        ifnet_model_t mode) {
 ...
 */
static void dlil_ifproto_input(struct if_proto *ifproto, mbuf_ref_t m) {
  int error;
  ...
  if (ifproto->proto_kpi == kProtoKPI_v1) {
    ...
    while (m != NULL) {
      ...
      error = (*ifproto->kpi.v1.input)(ifproto->ifp, ifproto->protocol_family,
                                       m, frame_header_ptr);
      if (error != 0 && error != EJUSTRETURN) {
        m_drop_if(m, ifproto->ifp, DROPTAP_FLAG_DIR_IN,
                  DROP_REASON_DLIL_IF_FILTER, NULL, 0);
      }
      m = next_packet;
    }
  } else if (ifproto->proto_kpi == kProtoKPI_v2) {
    /* Version 2 protocols support packet lists */
    error = (*ifproto->kpi.v2.input)(ifproto->ifp, ifproto->protocol_family, m);
    if (error != 0 && error != EJUSTRETURN) {
      m_drop_list(m, ifproto->ifp, DROPTAP_FLAG_DIR_IN,
                  DROP_REASON_DLIL_IF_FILTER, NULL, 0);
    }
  }
}
...
static errno_t dlil_input_async(struct dlil_threading_info *inp,
  ...
  if (m_head != NULL) {
    ...
    if (MBUF_QUEUE_IS_OVERCOMMITTED(input_queue)) {
      ...
      os_log_error(OS_LOG_DEFAULT,
                   ...
                   __func__, if_name(ifp), qlimit(input_queue),
                   if_rcvq_burst_limit, s_adj.dropped,
                   inp->dlth_trim_pkts_dropped, inp->dlth_trim_cnt,
                   qlen(input_queue));
    }
  }
  ...
  if (!MBUFQ_EMPTY(&freeq)) {
    m_drop_list(MBUFQ_FIRST(&freeq), ifp, DROPTAP_FLAG_DIR_IN,
                DROP_REASON_DLIL_BURST_LIMIT, NULL, 0);
  }
...
#pragma unused(tp)
  ...
  if (MBUF_QUEUE_IS_OVERCOMMITTED(&inp->dlth_pkts)) {
    ...
    os_log_error(OS_LOG_DEFAULT,
                 "%s %s burst limit %u (sysctl: %u) exceeded. "
                 "%u packets dropped [%u total in %u events]. new qlen %u \n",
                 __func__, if_name(ifp), qlimit(&inp->dlth_pkts),
                 if_rcvq_burst_limit, s_adj.dropped,
                 inp->dlth_trim_pkts_dropped, inp->dlth_trim_cnt,
                 qlen(&inp->dlth_pkts));
  }
  ...
  if (head.cp_mbuf != NULL) {
    dlil_input_packet_list_extended(ifp, head.cp_mbuf, m_cnt,
                                    ifp->if_poll_mode);
  }
...
static void dlil_input_cksum_dbg(struct ifnet *ifp, struct mbuf *m,
  ...
  if ((m->m_pkthdr.csum_flags &
    ...
    if (hlen > rxoff) {
      ...
      if (dlil_verbose) {
        DLIL_PRINTF("%s: partial cksum start offset %d "
                    ...
                    "mbuf 0x%llx\n",
                    if_name(ifp), rxoff, hlen, (uint64_t)VM_KERNEL_ADDRPERM(m));
      }
      return;
    }
    rxoff -= hlen;
      ...
      if (sum != m->m_pkthdr.csum_rx_val) {
        ...
        if (dlil_verbose) {
          DLIL_PRINTF("%s: bad partial cksum value "
                      ...
                      "0x%llx [rx_start %d]\n",
                      if_name(ifp), m->m_pkthdr.csum_rx_val, sum,
                      (uint64_t)VM_KERNEL_ADDRPERM(m),
    ...
    if (hwcksum_dbg_mode & HWCKSUM_DBG_PARTIAL_RXOFF_ADJ) {
      ...
      sum = m_adj_sum16(m, rxoff, aoff, m_pktlen(m) - aoff, sum);
...
#if (DEVELOPMENT || DEBUG)
static void dlil_input_process_wake_packet(ifnet_t ifp,
...
static void dlil_input_packet_list_common(struct ifnet *ifp_param, mbuf_ref_t m,
  ...
  if (bridge_enable_early_input != 0 && ifp != NULL && ifp->if_bridge != NULL) {
  ...
  while (m != NULL) {
    ...
    if ((ifp->if_eflags & IFEF_RXPOLL) && (ifp->if_xflags & IFXF_LEGACY) &&
        poll_thresh != 0 && poll_ival > 0 && (--poll_thresh % poll_ival) == 0) {
      ifnet_poll(ifp);
    }
    ...
    if (ifp != lo_ifp) {
      ...
      if (iorefcnt == 0) {
        if (!ifnet_datamov_begin(ifp)) {
          m_drop(m, DROPTAP_FLAG_DIR_IN, DROP_REASON_DLIL_IF_DATAMOV_BEGIN,
                 NULL, 0);
          goto next;
        }
      }
    ...
    error = (*ifp->if_demux)(ifp, m, frame_header, &protocol_family);
    ifnet_lock_done(ifp);
    if (error != 0) {
    ...
    if ((m->m_flags & M_PROMISC) == 0 && protocol_family == PF_INET6 &&
        IS_INTF_CLAT46(ifp) && dlil_is_clat_needed(protocol_family, m)) {
      char *data = NULL;
      ...
      if (ifp->if_type == IFT_ETHER) {
        ...
        char *fh = __unsafe_forge_bidi_indexable(char *, m->m_pkthdr.pkt_hdr,
                                                 ifnet_hdrlen(ifp));
        if (fh) {
  ...
  skip_clat:
    ...
    if (hwcksum_dbg != 0 && !(ifp->if_flags & IFF_LOOPBACK) &&
      ...
      dlil_input_cksum_dbg(ifp, m, frame_header, protocol_family);
    }
    /*
    ...
    error = dlil_interface_filters_input(ifp, &m, &frame_header,
                                         protocol_family, skip_bridge_filter);
    if (error != 0) {
      if (error != EJUSTRETURN) {
    ...
    if (ifproto == NULL) {
      ...
      m_drop_extended(m, ifp, frame_header, DROPTAP_FLAG_DIR_IN,
                      DROP_REASON_DLIL_NO_PROTO, NULL, 0);
      goto next;
    }
 ...
 */
__attribute__((noreturn)) static void dlil_input_thread_func(void *v,
                                                             wait_result_t w) {
#pragma unused(w)
...
dlil_input_thread_cont(void *v, wait_result_t wres) {
  ...
  while (1) {
    ...
    if (__improbable(embryonic = (inp->dlth_flags & DLIL_INPUT_EMBRYONIC))) {
      inp->dlth_flags &= ~DLIL_INPUT_EMBRYONIC;
    }
     ...
     */
    VERIFY(!(inp->dlth_flags & (DLIL_PROTO_WAITING | DLIL_PROTO_REGISTER)));
    ...
    if (__probable(m != NULL)) {
      dlil_input_packet_list_extended(ifp, m, m_cnt, ifp->if_poll_mode);
    }

    lck_mtx_lock_spin(&inp->dlth_lock);
    VERIFY(inp->dlth_flags & DLIL_INPUT_RUNNING);
    if (!(inp->dlth_flags & ~(DLIL_INPUT_RUNNING | DLIL_INPUT_TERMINATE))) {
      break;
    }
  }
...
static int dlil_interface_filters_input(struct ifnet *ifp, mbuf_ref_ref_t m_p,
                                        char **frame_header_p,
                                        protocol_family_t protocol_family,
  ...
  TAILQ_FOREACH(filter, &ifp->if_flt_head, filt_next) {
    ...
    if (is_vlan_packet && (filter->filt_flags & DLIL_IFF_INTERNAL) == 0) {
      continue;
    }
    /* the bridge has already seen the packet */
    if (skip_bridge && (filter->filt_flags & DLIL_IFF_BRIDGE) != 0) {
      continue;
    }
    if (!filter->filt_skip && filter->filt_input != NULL &&
      ...
      result = (*filter->filt_input)(filter->filt_cookie, ifp, protocol_family,
                                     m_p, frame_header_p);
...
dlil_main_input_thread_cont(void *v, wait_result_t wres) {
  ...
  lck_mtx_lock_spin(&inp->dlth_lock);
  VERIFY(!(inp->dlth_flags & (DLIL_INPUT_TERMINATE | DLIL_INPUT_RUNNING)));
  inp->dlth_flags |= DLIL_INPUT_RUNNING;
  ...
  while (1) {
    ...
    proto_req = (inp->dlth_flags & (DLIL_PROTO_WAITING | DLIL_PROTO_REGISTER));
  ...
  while (1) {
    ...
    if (if_rxpoll && net_timerisset(&ifp->if_poll_sample_holdtime)) {
      ...
      *(&ifp->if_poll_sample_lasttime) = *(&now);
      ...
      DLIL_EWMA(ifp->if_rxpoll_wavg, inp->dlth_wtot, if_rxpoll_decay);
      inp->dlth_wtot = 0;
        ...
        if (net_timercmp(&delta, &dlil_dbgrate, >=)) {
          *(&ifp->if_poll_dbg_lasttime) = *(&now);
          DLIL_PRINTF(
              ...
              "limits [%d/%d]\n",
              if_name(ifp),
              (ifp->if_poll_mode == IFNET_MODEL_INPUT_POLL_ON) ? "ON" : "OFF",
              ifp->if_rxpoll_pavg, ifp->if_rxpoll_pmax, ifp->if_rxpoll_plowat,
              ifp->if_rxpoll_phiwat, ifp->if_rxpoll_wavg, ifp->if_rxpoll_wlowat,
              ifp->if_rxpoll_whiwat, ifp->if_rxpoll_bavg, ifp->if_rxpoll_blowat,
    ...
    if (poll_req != 0 && ifnet_get_ioref(ifp)) {
      ...
      if (dlil_verbose) {
        DLIL_PRINTF(
            ...
            if_name(ifp), (mode == IFNET_MODEL_INPUT_POLL_ON) ? "ON" : "OFF",
            ifp->if_rxpoll_pavg, ifp->if_rxpoll_pmax, ifp->if_rxpoll_plowat,
            ifp->if_rxpoll_phiwat, ifp->if_rxpoll_wavg, ifp->if_rxpoll_wlowat,
            ifp->if_rxpoll_whiwat, ifp->if_rxpoll_bavg, ifp->if_rxpoll_blowat,
            ifp->if_rxpoll_bhiwat);
      ...
      if ((err = ((*ifp->if_input_ctl)(ifp, IFNET_CTL_SET_INPUT_MODEL,
        ...
        DLIL_PRINTF("%s: error setting polling mode "
                    "to %s (%d)\n",
                    if_name(ifp),
                    (mode == IFNET_MODEL_INPUT_POLL_ON) ? "ON" : "OFF", err);
      }
...
out:
  if (dlil_verbose) {
    ...
    DLIL_PRINTF("%s: "
                ...
                __func__, overcommitted_qlen, qlen(input_queue),
                overcommitted_qsize, qsize(input_queue), qlimit(input_queue),
                if_rcvq_burst_limit, target_qlen, if_rcvq_trim_pct,
                pkts_to_drop, dropped_pkts, dropped_bytes);
  }
