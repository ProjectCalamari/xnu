...
SYSCTL_NODE(_net, OID_AUTO, pktsched, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "pktsched");

SYSCTL_UINT(_net_pktsched, OID_AUTO, verbose, CTLFLAG_RW | CTLFLAG_LOCKED,
            &pktsched_verbose, 0, "Packet scheduler verbosity level");

static void pktsched_teardown_noop(__unused struct ifclassq *ifq) { return; }

static int pktsched_request_noop(struct ifclassq *ifq, cqrq_t rq, void *arg) {
#pragma unused(ifq, rq, arg)
  return ENXIO;
}

static int pktsched_getqstats_noop(struct ifclassq *ifq, uint8_t gid,
                                   u_int32_t qid,
...
static int pktsched_enqueue_noop(struct ifclassq *ifq, classq_pkt_t *h,
                                 classq_pkt_t *t, uint32_t cnt, uint32_t bytes,
                                 boolean_t *pdrop) {
  pktsched_pkt_t pkt;
...
static int pktsched_dequeue_noop(struct ifclassq *ifq, u_int32_t maxpktcnt,
                                 u_int32_t maxbytecnt,
                                 classq_pkt_t *first_packet,
                                 classq_pkt_t *last_packet,
                                 u_int32_t *retpktcnt, u_int32_t *retbytecnt,
                                 uint8_t grp_idx) {
#pragma unused(ifq, maxpktcnt, maxbytecnt, first_packet, last_packet,          \
               retpktcnt, retbytecnt, grp_idx)
  return ENXIO;
}

static int pktsched_dequeue_sc_noop(struct ifclassq *ifq, mbuf_svc_class_t svc,
                                    u_int32_t maxpktcnt, u_int32_t maxbytecnt,
                                    classq_pkt_t *first_packet,
                                    classq_pkt_t *last_packet,
                                    u_int32_t *retpktcnt, u_int32_t *retbytecnt,
                                    uint8_t grp_idx) {
#pragma unused(ifq, svc, maxpktcnt, maxbytecnt, first_packet, last_packet,     \
               retpktcnt, retbytecnt, grp_idx)
  return ENXIO;
}

static int pktsched_setup_noop(struct ifclassq *ifq, u_int32_t flags,
                               classq_pkt_type_t ptype) {
#pragma unused(ifq, flags, ptype)
  return ENXIO;
}

static boolean_t pktsched_allow_dequeue_noop(struct ifclassq *ifq) {
#pragma unused(ifq)
  return false;
}
...
static void init_machclk(void) {
  ...
  clock_interval_to_absolutetime_interval(1, NSEC_PER_SEC, &machclk_per_sec);
}

u_int64_t pktsched_abs_to_nsecs(u_int64_t abstime) {
  u_int64_t nsecs;
...
u_int64_t pktsched_nsecs_to_abstime(u_int64_t nsecs) {
  u_int64_t abstime;
...
int pktsched_setup(struct ifclassq *ifq, u_int8_t scheduler, u_int32_t sflags,
                   classq_pkt_type_t ptype) {
  int error = 0;
  u_int32_t rflags;
  pktsched_ops_t *ops;
...
void pktsched_teardown(struct ifclassq *ifq) {
  IFCQ_LOCK_ASSERT_HELD(ifq);
...
int pktsched_getqstats(struct ifclassq *ifq, u_int32_t gid, u_int32_t qid,
                       struct if_ifclassq_stats *ifqs) {
  IFCQ_LOCK_ASSERT_HELD(ifq);
...
void pktsched_pkt_encap(pktsched_pkt_t *pkt, classq_pkt_t *cpkt) {
  ...
  case QP_MBUF:
    pkt->pktsched_plen = (uint32_t)m_pktlen(pkt->pktsched_pkt_mbuf);
    break;
...
void pktsched_pkt_encap_chain(pktsched_pkt_t *pkt, classq_pkt_t *cpkt,
                              classq_pkt_t *tail, uint32_t cnt,
                              uint32_t bytes) {
  pkt->pktsched_pkt = *cpkt;
...
int pktsched_clone_pkt(pktsched_pkt_t *pkt1, pktsched_pkt_t *pkt2) {
  struct mbuf *m1, *m2;
#if SKYWALK
  ...
  ASSERT((pkt1 == pkt2 && pkt1->pktsched_pkt_mbuf == pkt2->pktsched_pkt_mbuf) ||
         (pkt1 != pkt2 && pkt2->pktsched_pkt_mbuf == NULL));
  ...
  case QP_PACKET:
    ...
    err = kern_packet_clone_nosleep(
        SK_PTR_ENCODE(p1, METADATA_TYPE(p1), METADATA_SUBTYPE(p1)), &ph2,
        KPKT_COPY_HEAVY);
...
void pktsched_corrupt_packet(pktsched_pkt_t *pkt) {
  struct mbuf *m = NULL;
  uint8_t *data = NULL;
  uint32_t data_len = 0;
...
void pktsched_free_pkt(pktsched_pkt_t *pkt) {
...
void pktsched_drop_pkt(pktsched_pkt_t *pkt, struct ifnet *ifp,
  ...
  case QP_MBUF: {
    ...
    m_drop_list(m, ifp, flags | DROPTAP_FLAG_DIR_OUT, reason, funcname,
                linenum);
    break;
  }
#if SKYWALK
  case QP_PACKET: {
    ...
    droptap_output_packet(SK_PKT2PH(kpkt), reason, funcname, linenum, flags,
                          ifp, kpkt->pkt_qum.qum_pid, NULL, -1, NULL, 0, 0);
    pktsched_free_pkt(pkt);
    break;
  }
#endif /* SKYWALK */
...
mbuf_svc_class_t pktsched_get_pkt_svc(pktsched_pkt_t *pkt) {
...
void pktsched_get_pkt_vars(pktsched_pkt_t *pkt, volatile uint32_t **flags,
                           uint64_t **timestamp, uint32_t *flowid,
                           uint8_t *flowsrc, uint8_t *proto,
...
struct flowadv_fcentry *pktsched_alloc_fcentry(pktsched_pkt_t *pkt,
                                               struct ifnet *ifp, int how) {
#pragma unused(ifp)
  ...
  case QP_PACKET: {
    ...
    static_assert(sizeof(fce->fce_flowsrc_fidx) ==
                  sizeof(kp->pkt_flowsrc_fidx));
    static_assert(sizeof(fce->fce_flowsrc_token) ==
...
static int pktsched_mbuf_mark_ecn(struct mbuf *m) {
  struct mbuf *m0;
  void *__single hdr;
  int af;
  uint8_t ipv;
  ...
  case AF_INET6: {
    ...
    if ((flowlabel & (IPTOS_ECN_MASK << 20)) == (IPTOS_ECN_NOTECT << 20)) {
      return EINVAL; /* not-ECT */
    }
    if ((flowlabel & (IPTOS_ECN_MASK << 20)) == (IPTOS_ECN_CE << 20)) {
      return 0; /* already marked */
    }
    /*
...
static int pktsched_kpkt_mark_ecn(struct __kern_packet *kpkt) {
  ...
  if ((kpkt->pkt_qum_qflags & QUM_F_FLOW_CLASSIFIED) != 0) {
    ...
    l3_hdr =
        __unsafe_forge_bidi_indexable(uint8_t *, kpkt->pkt_flow_ip_hdr, l3_len);
  } else {
    uint8_t *pkt_buf;
...
int pktsched_mark_ecn(pktsched_pkt_t *pkt) {
  switch (pkt->pktsched_ptype) {
  case QP_MBUF:
...
boolean_t pktsched_is_pkt_l4s(pktsched_pkt_t *pkt) {
  switch (pkt->pktsched_ptype) {
  case QP_MBUF: {
    struct pkthdr *pkth = &(pkt->pktsched_pkt_mbuf->m_pkthdr);
...
static struct m_tag *m_tag_kalloc_aqm(u_int32_t id, u_int16_t type,
...
static void m_tag_kfree_aqm(struct m_tag *tag) {
  struct aqm_tag_container *__single tag_container =
      (struct aqm_tag_container *)tag;
...
void pktsched_register_m_tag(void) {
  int error;
