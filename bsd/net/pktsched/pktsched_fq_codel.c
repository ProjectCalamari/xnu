...
SYSCTL_NODE(_net_classq, OID_AUTO, fq_codel, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "FQ-CODEL parameters");
...
SYSCTL_EXTENSIBLE_NODE(_net_classq_fq_codel, OID_AUTO, params,
                       ...
                       "classq fq codel parameters");

SYSCTL_QUAD(_net_classq_fq_codel, OID_AUTO, fq_empty_purge_delay,
            CTLFLAG_RW | CTLFLAG_LOCKED, &fq_empty_purge_delay,
            "Empty flow queue purge delay (ns)");

SYSCTL_QUAD(_net_classq_fq_codel, OID_AUTO, target_qdelay,
            CTLFLAG_RW | CTLFLAG_LOCKED, &fq_if_def_c_target_qdelay,
            "classic target queue delay in nanoseconds");
...
SYSCTL_INT(_net_classq_fq_codel, OID_AUTO, enable_pacing,
           CTLFLAG_RW | CTLFLAG_LOCKED, &fq_codel_enable_pacing, 0,
           "Enable pacing");
...
static void fq_if_classq_init(fq_if_group_t *fqg, uint32_t priority,
                              uint32_t quantum, uint32_t drr_max,
                              uint32_t svc_class);
static void fq_if_dequeue_class(fq_if_t *, fq_if_classq_t *, uint32_t, int64_t,
                                classq_pkt_t *, classq_pkt_t *, uint32_t *,
                                uint32_t *, flowq_dqlist_t *, bool, uint64_t,
                                bool *, uint64_t *);
...
static void fq_if_empty_old_flow(fq_if_t *fqs, fq_if_classq_t *fq_cl, fq_t *fq,
                                 uint64_t now);
...
static int fq_if_dequeue_sc_separate(struct ifclassq *ifq, mbuf_svc_class_t svc,
...
static void fq_if_calc_target_qdelay(struct ifnet *ifp,
                                     uint64_t *if_target_qdelay,
                                     uint32_t flags);
static void fq_if_calc_update_interval(uint64_t *update_interval,
                                       uint32_t flags);

uint32_t fq_codel_drr_max_values[FQ_IF_MAX_CLASSES] = {
    [FQ_IF_CTL_INDEX] = 8,    [FQ_IF_VO_INDEX] = 8, [FQ_IF_VI_INDEX] = 6,
    [FQ_IF_RV_INDEX] = 6,     [FQ_IF_AV_INDEX] = 6, [FQ_IF_OAM_INDEX] = 4,
    [FQ_IF_RD_INDEX] = 4,     [FQ_IF_BE_INDEX] = 4, [FQ_IF_BK_INDEX] = 2,
...
int fq_if_getqstats(struct ifclassq *ifq, uint8_t gid, u_int32_t qid,
                    struct if_ifclassq_stats *ifqs);
int fq_if_enqueue(struct ifclassq *ifq, classq_pkt_t *h, classq_pkt_t *t,
                  uint32_t cnt, uint32_t bytes, boolean_t *pdrop);
int fq_if_dequeue(struct ifclassq *ifq, u_int32_t maxpktcnt,
                  ...
                  u_int32_t *retbytecnt, uint8_t grp_idx);
int fq_if_dequeue_sc(struct ifclassq *ifq, mbuf_svc_class_t svc,
                     u_int32_t maxpktcnt, u_int32_t maxbytecnt,
...
void pktsched_fq_init(void) {
  ...
  PE_parse_boot_argn("fq_if_def_c_update_interval",
                     &fq_if_def_c_update_interval,
                     sizeof(fq_if_def_c_update_interval));
  PE_parse_boot_argn("fq_if_def_l4s_target_qdelay",
                     &fq_if_def_l4s_target_qdelay,
                     sizeof(fq_if_def_l4s_target_qdelay));
  PE_parse_boot_argn("fq_if_def_l4s_update_interval",
                     &fq_if_def_l4s_update_interval,
                     sizeof(fq_if_def_l4s_update_interval));
  PE_parse_boot_argn("fq_if_ll_c_target_qdelay", &fq_if_ll_c_target_qdelay,
                     sizeof(fq_if_ll_c_target_qdelay));
  PE_parse_boot_argn("fq_if_ll_c_update_interval", &fq_if_ll_c_update_interval,
                     sizeof(fq_if_ll_c_update_interval));
...
static uint32_t fq_if_flow_hash_id(uint32_t flowid) {
...
typedef boolean_t (*fq_getq_flow_t)(fq_if_t *, fq_if_classq_t *, fq_t *,
                                    int64_t, uint32_t, classq_pkt_t *,
                                    classq_pkt_t *, uint32_t *, uint32_t *,
                                    boolean_t *, uint64_t);

static void fq_if_append_mbuf(classq_pkt_t *pkt, classq_pkt_t *next_pkt) {
  pkt->cp_mbuf->m_nextpkt = next_pkt->cp_mbuf;
}

static inline uint64_t fq_codel_get_time(void) {
  struct timespec ts;
  uint64_t now;
...
#if SKYWALK
static void fq_if_append_pkt(classq_pkt_t *pkt, classq_pkt_t *next_pkt) {
  pkt->cp_kpkt->pkt_nextpkt = next_pkt->cp_kpkt;
}
#endif /* SKYWALK */

#if SKYWALK
static boolean_t fq_getq_flow_kpkt(fq_if_t *fqs, fq_if_classq_t *fq_cl,
                                   fq_t *fq, int64_t byte_limit,
                                   uint32_t pkt_limit, classq_pkt_t *head,
                                   classq_pkt_t *tail, uint32_t *byte_cnt,
                                   uint32_t *pkt_cnt, boolean_t *qempty,
                                   uint64_t now) {
  uint32_t plen;
  pktsched_pkt_t pkt;
...
static boolean_t fq_getq_flow_mbuf(fq_if_t *fqs, fq_if_classq_t *fq_cl,
                                   fq_t *fq, int64_t byte_limit,
                                   uint32_t pkt_limit, classq_pkt_t *head,
                                   classq_pkt_t *tail, uint32_t *byte_cnt,
                                   uint32_t *pkt_cnt, boolean_t *qempty,
...
static void fq_if_pacemaker_tcall(thread_call_param_t arg0,
...
fq_if_t *fq_if_alloc(struct ifclassq *ifq, classq_pkt_type_t ptype) {
  ...
  fqs_flows =
      kalloc_type(flowq_list_t, fq_if_hash_table_size, Z_WAITOK | Z_ZERO);
  if (fqs_flows == NULL) {
  ...
  fqs->fqs_pacemaker_tcall = thread_call_allocate_with_options(
      fq_if_pacemaker_tcall, (thread_call_param_t)(ifq->ifcq_ifp),
...
void fq_if_destroy(fq_if_t *fqs) {
...
static inline uint8_t fq_if_service_to_priority(fq_if_t *fqs,
...
void fq_if_classq_init(fq_if_group_t *fqg, uint32_t pri, uint32_t quantum,
...
int fq_if_enqueue(struct ifclassq *ifq, classq_pkt_t *head, classq_pkt_t *tail,
                  uint32_t cnt, uint32_t bytes, boolean_t *pdrop) {
  ...
  if (__improbable(ret != 0)) {
    if (ret == CLASSQEQ_SUCCESS_FC) {
      ...
      *pdrop = FALSE;
      ...
      IFCQ_UNLOCK(ifq);
      *pdrop = TRUE;
      pktsched_drop_pkt(&pkt, ifq->ifcq_ifp, DROP_REASON_AQM_FULL, __func__,
                        __LINE__, 0);
      switch (ret) {
      case CLASSQEQ_DROP:
        ret = ENOBUFS;
...
static inline void fq_dqlist_add(flowq_dqlist_t *fq_dqlist_head, fq_t *fq) {
...
static inline void fq_dqlist_remove(flowq_dqlist_t *fq_dqlist_head, fq_t *fq,
                                    classq_pkt_t *head, classq_pkt_t *tail,
                                    classq_pkt_type_t ptype) {
  ASSERT(fq->fq_in_dqlist);
...
static inline void fq_dqlist_get_packet_list(flowq_dqlist_t *fq_dqlist_head,
                                             classq_pkt_t *head,
                                             classq_pkt_t *tail,
                                             classq_pkt_type_t ptype) {
  fq_t *fq, *tfq;
...
#pragma unused(pri)
  ...
  fq_if_group_t *grp;
  TAILQ_FOREACH(grp, grp_list, fqg_grp_link) { grp->fqg_bitmaps[state] = 0; }
}
 ...
 * due to pacing. Pacemaker will doorbell the driver when current >=
 * next_tx_time. This only applies to L4S traffic at this moment.
 */
static void fq_if_schedule_pacemaker(fq_if_t *fqs, uint64_t now,
...
static int fq_if_dequeue_common(struct ifclassq *ifq, mbuf_svc_class_t svc,
                                u_int32_t maxpktcnt, u_int32_t maxbytecnt,
                                classq_pkt_t *first_packet,
                                classq_pkt_t *last_packet, u_int32_t *retpktcnt,
                                u_int32_t *retbytecnt, uint8_t grp_idx) {
  ...
  for (;;) {
    ...
    if (fqs->grp_bitmaps_zeros(grp_list, svc_pri, FQ_IF_ER) &&
      ...
      if (fqs->grp_bitmaps_zeros(grp_list, svc_pri, FQ_IF_EB)) {
        if (fq_codel_enable_pacing && fq_codel_enable_l4s) {
          /*
           * Move fq_cl in IR back to ER, so that they will inspected with
           * priority the next time the driver dequeues
           */
    ...
    if (fq_cl->fcl_budget <= 0) {
      /* Update the budget */
      fq_cl->fcl_budget +=
          (min(fq_cl->fcl_drr_max, fq_cl->fcl_stat.fcl_flows_cnt) *
           fq_cl->fcl_quantum);
      if (fq_cl->fcl_budget <= 0) {
        goto state_change;
      }
  ...
  state_change:
    ...
    if (!FQ_IF_CLASSQ_IDLE(fq_cl)) {
      ...
      pktsched_bit_clr(pri, &fq_grp->fqg_bitmaps[FQ_IF_ER]);
      VERIFY(((fq_grp->fqg_bitmaps[FQ_IF_ER] | fq_grp->fqg_bitmaps[FQ_IF_EB] |
               fq_grp->fqg_bitmaps[FQ_IF_IB]) &
              (1 << pri)) == 0);
      fq_cl->fcl_budget = 0;
    }
...
int fq_if_dequeue(struct ifclassq *ifq, u_int32_t maxpktcnt,
  ...
  return fq_if_dequeue_common(ifq, MBUF_SC_UNSPEC, maxpktcnt, maxbytecnt,
                              first_packet, last_packet, retpktcnt, retbytecnt,
                              grp_idx);
}
...
int fq_if_dequeue_sc(struct ifclassq *ifq, mbuf_svc_class_t svc,
  ...
  if (fq_if_is_grp_combined(fqs, grp_idx)) {
    return fq_if_dequeue_common(ifq, svc, maxpktcnt, maxbytecnt, first_packet,
                                last_packet, retpktcnt, retbytecnt, grp_idx);
  } else {
    /*
    ...
    return fq_if_dequeue_sc_separate(ifq, svc, maxpktcnt, maxbytecnt,
                                     first_packet, last_packet, retpktcnt,
                                     retbytecnt, grp_idx);
  }
}
...
static int fq_if_dequeue_sc_separate(struct ifclassq *ifq, mbuf_svc_class_t svc,
  ...
  while (total_pktcnt < maxpktcnt && total_bytecnt < maxbytecnt &&
    ...
    fq_if_dequeue_class(fqs, fq_cl, (maxpktcnt - total_pktcnt),
                        (maxbytecnt - total_bytecnt), &head, &tail, &pktcnt,
                        &bytecnt, &fq_dqlist_head, false, now, &all_paced,
                        &next_tx_time);
    if (head.cp_mbuf != NULL) {
      if (first.cp_mbuf == NULL) {
        first = head;
...
static void fq_if_purge_flow(fq_if_t *fqs, fq_t *fq, uint32_t *pktsp,
  ...
  for (;;) {
    ...
    if (__improbable(droptap_verbose > 0)) {
      pktsched_drop_pkt(&pkt, fqs->fqs_ifq->ifcq_ifp,
                        DROP_REASON_AQM_PURGE_FLOW, __func__, __LINE__, 0);
    } else {
      pktsched_free_pkt(&pkt);
    }
  }
...
static void fq_if_purge_classq(fq_if_t *fqs, fq_if_classq_t *fq_cl) {
  fq_t *fq, *tfq;
  uint64_t now;
...
static void fq_if_purge(fq_if_t *fqs) {
  uint64_t now;
  fq_if_group_t *grp;
  int i;
...
static void fq_if_purge_sc(fq_if_t *fqs, cqrq_purge_sc_t *req) {
  fq_t *fq;
  uint64_t now;
  fq_if_group_t *grp;
...
static uint32_t fq_if_calc_quantum(struct ifnet *ifp) {
...
static void fq_if_mtu_update(fq_if_t *fqs) {
#define _FQ_CLASSQ_UPDATE_QUANTUM(_grp, _s, _q)                                \
  (_grp)->fqg_classq[FQ_IF_##_s##_INDEX].fcl_quantum = FQ_CODEL_QUANTUM_##_s(_q)
...
static void fq_if_event(fq_if_t *fqs, cqev_t ev) {
  IFCQ_LOCK_ASSERT_HELD(fqs->fqs_ifq);
...
static void fq_if_classq_suspend(fq_if_t *fqs, fq_if_classq_t *fq_cl) {
...
static void fq_if_classq_resume(fq_if_t *fqs, fq_if_classq_t *fq_cl) {
  VERIFY(FQ_IF_CLASSQ_IDLE(fq_cl));
  fqs->fqs_throttle = 0;
  fq_cl->fcl_stat.fcl_throttle_off++;
  KDBG(AQM_KTRACE_AON_THROTTLE | DBG_FUNC_END, fqs->fqs_ifq->ifcq_ifp->if_index,
       0, 0, 0);
}

static int fq_if_throttle(fq_if_t *fqs, cqrq_throttle_t *tr) {
  struct ifclassq *ifq = fqs->fqs_ifq;
  uint8_t index;
  fq_if_group_t *grp;
...
static inline boolean_t fq_if_is_fq_cl_paced(fq_if_classq_t *fq_cl,
                                             uint64_t now) {
...
static boolean_t fq_if_is_grp_all_paced(fq_if_group_t *grp) {
  fq_if_classq_t *fq_cl;
  uint64_t now;
...
#if (DEBUG || DEVELOPMENT)
static int fq_if_configure_target_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg2)
  ...
  if (fqs->fqs_ifq == NULL || !IFCQ_IS_ENABLED(fqs->fqs_ifq) ||
      fqs->fqs_classq_groups[0] == NULL) {
    return ENXIO;
  }

  target_delay = &fqs->fqs_classq_groups[0]->fqg_target_qdelays[FQ_TFC_C];
  error = sysctl_io_number(req, *target_delay, sizeof(*target_delay),
                           &new_target, &changed);
...
static int fq_if_setup_common(struct ifclassq *ifq, u_int32_t flags,
  ...
  struct ifnet *ifp = ifq->ifcq_ifp;
  struct skoid *fqs_skoid = (struct skoid *)&fqs->fqs_oid;
  skoid_create(fqs_skoid, SKOID_SNODE(_net_classq_fq_codel_params),
...
fq_t *fq_if_hash_pkt(fq_if_t *fqs, fq_if_group_t *fq_grp, uint32_t flowid,
                     mbuf_svc_class_t svc_class, uint64_t now,
                     uint8_t pkt_proto, uint8_t pkt_flowsrc,
                     fq_tfc_type_t tfc_type, bool create) {
  fq_t *fq = NULL;
  ...
  LIST_FOREACH(fq, fq_list, fq_hashlink) {
    if (fq->fq_flowhash == flowid && fq->fq_sc_index == scidx &&
        fq->fq_tfc_type == tfc_type && fq->fq_group == fq_grp) {
      break;
    }
  }
  if (fq == NULL && create) {
...
#endif /* (DEBUG || DEVELOPMENT) */
          tfc_type != FQ_TFC_L4S && !(fqs->fqs_flags & FQS_LEGACY) &&
    ...
    KDBG(AQM_KTRACE_STATS_FLOW_ALLOC, fqs->fqs_ifq->ifcq_ifp->if_index,
         fq->fq_flowhash, AQM_KTRACE_FQ_GRP_SC_IDX(fq), 0);
...
static void fq_if_destroy_flow(fq_if_t *fqs, fq_if_classq_t *fq_cl, fq_t *fq) {
  ...
  KDBG(AQM_KTRACE_STATS_FLOW_DESTROY, fqs->fqs_ifq->ifcq_ifp->if_index,
       fq->fq_flowhash, AQM_KTRACE_FQ_GRP_SC_IDX(fq), 0);
  fq_destroy(fq, fqs->fqs_ptype);
}

inline boolean_t fq_if_at_drop_limit(fq_if_t *fqs) {
  return (IFCQ_LEN(fqs->fqs_ifq) >= fqs->fqs_pkt_droplimit) ? TRUE : FALSE;
}

inline boolean_t fq_if_almost_at_drop_limit(fq_if_t *fqs) {
  /*
...
static inline void fq_if_reuse_empty_flow(fq_if_t *fqs, fq_t *fq,
                                          uint64_t now) {
...
inline void fq_if_move_to_empty_flow(fq_if_t *fqs, fq_if_classq_t *fq_cl,
...
static void fq_if_purge_empty_flow_list(fq_if_t *fqs, uint64_t now,
  ...
  TAILQ_FOREACH_SAFE(fq, &fqs->fqs_empty_list, fq_empty_link, tmp) {
    if (!purge_all &&
        ((now < fq->fq_empty_purge_time) || (i++ == FQ_EMPTY_PURGE_MAX))) {
      break;
    }
    fq_if_purge_empty_flow(fqs, fq);
  }
...
static void fq_if_empty_new_flow(fq_t *fq, fq_if_classq_t *fq_cl) {
  ...
  STAILQ_REMOVE(&fq_cl->fcl_new_flows, fq, flowq, fq_actlink);
...
inline void fq_if_drop_packet(fq_if_t *fqs, uint64_t now) {
  ...
  pktsched_get_pkt_vars(&pkt, &pkt_flags, &pkt_timestamp, NULL, NULL, NULL,
                        NULL, NULL);
...
inline void fq_if_is_flow_heavy(fq_if_t *fqs, fq_t *fq) {
  fq_t *prev_fq;
...
boolean_t fq_if_add_fcentry(fq_if_t *fqs, pktsched_pkt_t *pkt, uint8_t flowsrc,
  ...
  if (fce != NULL) {
    ...
    os_log(OS_LOG_DEFAULT,
           ...
           __func__, fq_cl->fcl_stat.fcl_flow_control, fq->fq_sc_index,
           fce->fce_flowsrc_type, fq->fq_flowhash,
           if_name(fqs->fqs_ifq->ifcq_ifp), fq->fq_bytes);
    KDBG(AQM_KTRACE_STATS_FLOW_CTL | DBG_FUNC_START, fq->fq_flowhash,
         AQM_KTRACE_FQ_GRP_SC_IDX(fq), fq->fq_bytes, fq->fq_min_qdelay);
  }
...
static void fq_if_remove_fcentry(fq_if_t *fqs, struct flowadv_fcentry *fce) {
  STAILQ_REMOVE(&fqs->fqs_fclist, fce, flowadv_fcentry, fce_link);
  STAILQ_NEXT(fce, fce_link) = NULL;
  flowadv_add_entry(fce);
}

void fq_if_flow_feedback(fq_if_t *fqs, fq_t *fq, fq_if_classq_t *fq_cl) {
  ...
  if (fce != NULL) {
    ...
    os_log(OS_LOG_DEFAULT,
           ...
           __func__, fq_cl->fcl_stat.fcl_flow_feedback, fq->fq_sc_index,
           fce->fce_flowsrc_type, fce->fce_flowid,
           if_name(fqs->fqs_ifq->ifcq_ifp), FQ_GROUP(fq)->fqg_index,
           fq->fq_bytes);
    fq_if_remove_fcentry(fqs, fce);
    KDBG(AQM_KTRACE_STATS_FLOW_CTL | DBG_FUNC_END, fq->fq_flowhash,
         AQM_KTRACE_FQ_GRP_SC_IDX(fq), fq->fq_bytes, fq->fq_min_qdelay);
  }
  fq->fq_flags &= ~FQF_FLOWCTL_ON;
}

boolean_t fq_if_report_congestion(fq_if_t *fqs, pktsched_pkt_t *pkt,
                                  uint32_t congestion_cnt, uint32_t l4s_ce_cnt,
                                  uint32_t pkt_cnt) {
  struct flowadv_fcentry *fce;
...
void fq_if_dequeue_class(fq_if_t *fqs, fq_if_classq_t *fq_cl, uint32_t pktlimit,
                         int64_t bytelimit, classq_pkt_t *top,
                         classq_pkt_t *bottom, uint32_t *retpktcnt,
                         uint32_t *retbytecnt, flowq_dqlist_t *fq_dqlist,
                         bool budget_restricted, uint64_t now,
                         bool *fq_cl_paced, uint64_t *next_tx_time) {
  ...
  STAILQ_FOREACH_SAFE(fq, &fq_cl->fcl_new_flows, fq_actlink, tfq) {
    ASSERT((fq->fq_flags & (FQF_NEW_FLOW | FQF_OLD_FLOW)) == FQF_NEW_FLOW);
    uint64_t fq_tx_time;
    ...
    limit_reached = fq_getq_flow_fn(fqs, fq_cl, fq, bytelimit, pktlimit, head,
                                    tail, &bytecnt, &pktcnt, &qempty, now);
    ...
    // TODO: add credit when it's now paced? so that the fq is trated the same
    // as empty
  ...
  STAILQ_FOREACH_SAFE(fq, &fq_cl->fcl_old_flows, fq_actlink, tfq) {
    VERIFY((fq->fq_flags & (FQF_NEW_FLOW | FQF_OLD_FLOW)) == FQF_OLD_FLOW);
    bool destroy = true;
    uint64_t fq_tx_time;
...
void fq_if_teardown(struct ifclassq *ifq) {
  ...
  IFCQ_LOCK_ASSERT_HELD(ifq);
  VERIFY(fqs != NULL);
  VERIFY(ifq->ifcq_type == PKTSCHEDT_FQ_CODEL ||
         ifq->ifcq_type == PKTSCHEDT_FQ_CODEL_NEW);
  fq_if_destroy(fqs);
  ifq->ifcq_disc = NULL;
  ifclassq_detach(ifq);
}

static void fq_export_flowstats(fq_if_t *fqs, fq_t *fq,
...
int fq_if_getqstats(struct ifclassq *ifq, uint8_t gid, u_int32_t qid,
  ...
  flowstat_cnt = min((fcls->fcls_newflows_cnt + fcls->fcls_oldflows_cnt),
                     FQ_IF_MAX_FLOWSTATS);
  i = 0;
...
int fq_if_create_grp(struct ifclassq *ifcq, uint8_t grp_idx, uint8_t flags) {
#define _FQ_CLASSQ_INIT(_grp, _s, _q)                                          \
  fq_if_classq_init(_grp, FQ_IF_##_s##_INDEX, FQ_CODEL_QUANTUM_##_s(_q),       \
                    FQ_CODEL_DRR_MAX(_s), MBUF_SC_##_s);
...
update:
  ...
  fq_if_calc_target_qdelay(ifp, &grp->fqg_target_qdelays[FQ_TFC_C], calc_flags);
  fq_if_calc_target_qdelay(ifp, &grp->fqg_target_qdelays[FQ_TFC_L4S],
                           calc_flags | IF_CLASSQ_L4S);

  fq_if_calc_update_interval(&grp->fqg_update_intervals[FQ_TFC_C], calc_flags);
...
fq_if_group_t *fq_if_find_grp(fq_if_t *fqs, uint8_t grp_idx) {
  fq_if_group_t *grp;
...
void fq_if_destroy_grps(fq_if_t *fqs) {
...
void fq_if_set_grp_combined(struct ifclassq *ifcq, uint8_t grp_idx) {
  fq_if_t *fqs;
  fq_if_group_t *grp;
...
void fq_if_set_grp_separated(struct ifclassq *ifcq, uint8_t grp_idx) {
  fq_if_t *fqs;
  fq_if_group_t *grp;
