...
#define CALLOUT_INIT(c)                                                        \
  do {                                                                         \
    (void)memset((c), 0, sizeof(*(c)));                                        \
  } while (/*CONSTCOND*/ 0)

#define CALLOUT_RESET(c, t, f, a)                                              \
  do {                                                                         \
    (c)->c_arg = (a);                                                          \
    (c)->c_func = (f);                                                         \
    timeout((f), (a), (t));                                                    \
...
static inline boolean_t pktsched_bit_tst(u_int32_t ix,
                                         pktsched_bitmap_t *pData) {
  return (boolean_t)(*pData & (1 << ix));
}

static inline void pktsched_bit_set(u_int32_t ix, pktsched_bitmap_t *pData) {
  *pData |= (1 << ix);
}

static inline void pktsched_bit_clr(u_int32_t ix, pktsched_bitmap_t *pData) {
  *pData &= ~(1 << ix);
}

static inline void pktsched_bit_cpy(u_int32_t ix, pktsched_bitmap_t *pData_dst,
                                    pktsched_bitmap_t *pData_src) {
  *pData_dst ^= (-(*pData_src & (1 << ix)) ^ *pData_dst) & (1 << ix);
}

static inline pktsched_bitmap_t pktsched_ffs(pktsched_bitmap_t pData) {
  return (pktsched_bitmap_t)ffs(pData);
}

static inline pktsched_bitmap_t pktsched_fls(pktsched_bitmap_t pData) {
  return (pktsched_bitmap_t)((sizeof(pktsched_bitmap_t) << 3) -
                             (unsigned long)clz(pData));
}

static inline pktsched_bitmap_t __fls(pktsched_bitmap_t word) {
  VERIFY(word != 0);
  return pktsched_fls(word) - 1;
}

static inline uint32_t pktsched_get_pkt_len(pktsched_pkt_t *pkt) {
  return pkt->pktsched_plen;
}

static inline void pktsched_bit_move(u_int32_t ix, pktsched_bitmap_t *pData_dst,
                                     pktsched_bitmap_t *pData_src) {
...
extern void pktsched_drop_pkt(pktsched_pkt_t *, struct ifnet *, uint32_t,
