...
typedef int (*fq_if_bitmaps_ffs)(fq_grp_tailq_t *, int, fq_if_state,
                                 fq_if_group_t **);
typedef boolean_t (*fq_if_bitmaps_zeros)(fq_grp_tailq_t *, int, fq_if_state);
typedef void (*fq_if_bitmaps_cpy)(fq_grp_tailq_t *, int, fq_if_state,
                                  fq_if_state);
typedef void (*fq_if_bitmaps_clr)(fq_grp_tailq_t *, int, fq_if_state);
typedef void (*fq_if_bitmaps_move)(fq_grp_tailq_t *, int, fq_if_state,
                                   fq_if_state);
...
typedef void (*fq_codel_dq_t)(void *fqs, void *fq, pktsched_pkt_t *pkt,
                              uint64_t now);
...
#define FQS_GROUP(_fqs, _group_idx) (fq_if_find_grp((_fqs), (_group_idx)))

#define FQS_CLASSQ(_fqs, _group_idx, _sc_idx)                                  \
  (FQS_GROUP((_fqs), (_group_idx))->fqg_classq[_sc_idx])

#define FQ_GROUP(_fq) ((_fq)->fq_group)

#define FQ_GRP_LEN(_fq) (FQ_GROUP((_fq))->fqg_len)
#define FQ_GRP_IS_EMPTY(_fq) (FQ_GRP_LEN((_fq)) == 0)
#define FQ_GRP_INC_LEN(_fq) (FQ_GRP_LEN((_fq))++)
#define FQ_GRP_DEC_LEN(_fq) (FQ_GRP_LEN((_fq))--)
#define FQ_GRP_ADD_LEN(_fq, _len) (FQ_GRP_LEN((_fq)) += (_len))
#define FQ_GRP_SUB_LEN(_fq, _len) (FQ_GRP_LEN((_fq)) -= (_len))

#define FQS_GRP_ADD_LEN(_fqs, _grp_idx, _len)                                  \
  (FQS_GROUP(_fqs, grp_idx)->fqg_len += (_len))

#define FQ_GRP_BYTES(_fq) (FQ_GROUP((_fq))->fqg_bytes)
#define FQ_GRP_INC_BYTES(_fq, _len) (FQ_GRP_BYTES((_fq)) += (_len))
#define FQ_GRP_DEC_BYTES(_fq, _len) (FQ_GRP_BYTES((_fq)) -= (_len))

#define FQS_GRP_INC_BYTES(_fqs, grp_idx, _len)                                 \
  (FQS_GROUP(_fqs, grp_idx)->fqg_bytes += (_len))

#define FQ_CLASSQ(_fq) (FQ_GROUP((_fq))->fqg_classq[(_fq)->fq_sc_index])
...
extern struct flowq *fq_if_hash_pkt(fq_if_t *, fq_if_group_t *, u_int32_t,
                                    mbuf_svc_class_t, u_int64_t, uint8_t,
                                    uint8_t, fq_tfc_type_t, bool);
...
extern int fq_if_create_grp(struct ifclassq *ifcq, uint8_t qset_idx,
                            uint8_t flags);
