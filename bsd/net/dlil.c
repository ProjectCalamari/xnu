...
static errno_t ifproto_media_input_v2(struct ifnet *, protocol_family_t,
                                      struct mbuf *);
static errno_t ifproto_media_preout(struct ifnet *, protocol_family_t, mbuf_t *,
...
static errno_t ifp_if_framer(struct ifnet *, struct mbuf **,
                             const struct sockaddr *, IFNET_LLADDR_T,
                             IFNET_FRAME_TYPE_T, u_int32_t *, u_int32_t *);
...
static errno_t ifp_if_framer_extended(struct ifnet *, struct mbuf **,
                                      const struct sockaddr *, IFNET_LLADDR_T,
                                      IFNET_FRAME_TYPE_T, u_int32_t *,
                                      u_int32_t *);
static errno_t ifp_if_set_bpf_tap(struct ifnet *, bpf_tap_mode,
                                  bpf_packet_func);
...
extern uint32_t udp_count_opportunistic(unsigned int ifindex, u_int32_t flags);
extern uint32_t tcp_count_opportunistic(unsigned int ifindex, u_int32_t flags);
...
static inline void ifnet_delay_start_disabled_increment(void) {
...
void ifnet_filter_update_tso(struct ifnet *ifp, boolean_t filter_enable) {
  /*
...
boolean_t ifnet_nx_noauto(ifnet_t ifp) {
  return (ifp->if_xflags & IFXF_NX_NOAUTO) != 0;
}

boolean_t ifnet_nx_noauto_flowswitch(ifnet_t ifp) {
  return ifnet_is_low_latency(ifp);
}

boolean_t ifnet_is_low_latency(ifnet_t ifp) {
  return (ifp->if_xflags & IFXF_LOW_LATENCY) != 0;
}

boolean_t ifnet_needs_compat(ifnet_t ifp) {
...
boolean_t ifnet_needs_fsw_transport_netagent(ifnet_t ifp) {
...
boolean_t ifnet_needs_fsw_ip_netagent(ifnet_t ifp) {
#pragma unused(ifp)
...
boolean_t ifnet_needs_netif_netagent(ifnet_t ifp) {
#pragma unused(ifp)
  return (if_attach_nx & IF_ATTACH_NX_NETIF_NETAGENT) != 0;
}

static boolean_t dlil_detach_nexus_instance(nexus_controller_t controller,
                                            const char *func_str,
                                            uuid_t instance, uuid_t device) {
  errno_t err;
  ...
  err = kern_nexus_controller_free_provider_instance(controller, instance);
  if (err != 0) {
...
static boolean_t dlil_detach_nexus(const char *func_str, uuid_t provider,
                                   uuid_t instance, uuid_t device) {
  boolean_t detached = FALSE;
  nexus_controller_t controller = kern_nexus_shared_controller();
  int err;

  if (dlil_detach_nexus_instance(controller, func_str, instance, device)) {
    detached = TRUE;
  }
  if (provider != NULL && !uuid_is_null(provider)) {
    detached = TRUE;
    err = kern_nexus_controller_deregister_provider(controller, provider);
    if (err != 0) {
...
static errno_t dlil_create_provider_and_instance(nexus_controller_t controller,
                                                 nexus_type_t type, ifnet_t ifp,
                                                 uuid_t *provider,
                                                 uuid_t *instance,
                                                 nexus_attr_t attr) {
  uuid_t dom_prov;
  ...
  const char *type_name = (type == NEXUS_TYPE_NET_IF) ? "netif" : "flowswitch";
  ...
  snprintf((char *)provider_name, sizeof(provider_name), "com.apple.%s.%s",
           type_name, if_name(ifp));
  err = kern_nexus_controller_register_provider(
      controller, dom_prov, provider_name, NULL, 0, attr, provider);
  if (err != 0) {
  ...
  err = kern_nexus_controller_alloc_provider_instance(
      controller, *provider, NULL, NULL, instance, &init);
  if (err != 0) {
...
static boolean_t dlil_attach_netif_nexus_common(ifnet_t ifp,
                                                if_nexus_netif_t netif_nx) {
  nexus_attr_t __single attr = NULL;
  nexus_controller_t controller;
  errno_t err;
  unsigned char *empty_uuid =
  ...
  err = dlil_create_provider_and_instance(controller, NEXUS_TYPE_NET_IF, ifp,
                                          &netif_nx->if_nif_provider,
...
static boolean_t dlil_attach_netif_compat_nexus(ifnet_t ifp,
                                                if_nexus_netif_t netif_nx) {
...
static inline int dlil_siocgifdevmtu(struct ifnet *ifp,
                                     struct ifdevmtu *ifdm_p) {
  struct ifreq ifr;
  int error;
...
static inline int _dlil_get_flowswitch_buffer_size(ifnet_t ifp, uuid_t netif,
                                                   uint32_t *buf_size,
  ...
  *large_buf_size = *use_multi_buflet ? NX_FSW_DEF_LARGE_BUFSIZE : 0;
  ...
  if (__improbable(err != 0)) {
    ...
    DLIL_PRINTF("%s: %s %d %d\n", __func__, if_name(ifp), ifdm.ifdm_max,
                ifdm.ifdm_current);
...
skip_mtu_ioctl:
  ...
  if ((if_max_mtu > NX_FSW_MAXBUFSIZE) && fsw_use_max_mtu_buffer) {
    DLIL_PRINTF("%s: interace (%s) has MAX MTU (%u) > flowswitch "
                "max bufsize(%d)\n",
                __func__, if_name(ifp), if_max_mtu, NX_FSW_MAXBUFSIZE);
    return EINVAL;
  }
  ...
  if (dlil_is_native_netif_nexus(ifp)) {
    err = kern_nexus_get_pbufpool_info(netif, &rx_pp_info, &tx_pp_info);
    if (err != 0) {
      DLIL_PRINTF("%s: can't get pbufpool info for %s\n", __func__,
                  if_name(ifp));
      return ENXIO;
    }
    drv_buf_size = tx_pp_info.kpm_bufsize * tx_pp_info.kpm_max_frags;
    if (if_max_mtu > drv_buf_size) {
      DLIL_PRINTF("%s: interface %s packet pool (rx %d * %d, "
                  "tx %d * %d) can't support max mtu(%d)\n",
                  __func__, if_name(ifp), rx_pp_info.kpm_bufsize,
...
_dlil_attach_flowswitch_nexus(ifnet_t ifp, if_nexus_flowswitch_t nexus_fsw) {
  ...
  err = _dlil_get_flowswitch_buffer_size(ifp, netif, &buf_size, &multi_buflet,
                                         &large_buf_size);
  if (err != 0) {
    goto failed;
  }
  ...
  VERIFY(err == 0);
  err = kern_nexus_attr_set(attr, NEXUS_ATTR_LARGE_BUF_SIZE, large_buf_size);
  VERIFY(err == 0);
  ...
  controller = kern_nexus_shared_controller();
  err = dlil_create_provider_and_instance(controller, NEXUS_TYPE_FLOW_SWITCH,
                                          ifp, &nexus_fsw->if_fsw_provider,
  ...
  err = kern_nexus_ifattach(controller, nexus_fsw->if_fsw_instance, NULL, netif,
                            FALSE, &nexus_fsw->if_fsw_device);
  if (err != 0) {
...
static boolean_t dlil_attach_flowswitch_nexus(ifnet_t ifp) {
   ...
   *
   * ifnet_datamov_begin() is used here to force
   * dlil_quiesce_and_detach_nexuses() (called by another thread) to wait until
   * this function finishes so the flowswitch can be cleaned up by
   ...
   * created flowswitch would be left hanging and ifnet_detach_final() would
   * never wakeup because the existence of the flowswitch prevents the ifnet's
   * ioref from being released.
   */
  if (!ifnet_datamov_begin(ifp)) {
    os_log(OS_LOG_DEFAULT, "%s: %s not attached", __func__, ifp->if_xname);
    goto done;
  }
...
__attribute__((noinline)) static void dlil_netif_detach_notify(ifnet_t ifp) {
...
boolean_t ifnet_add_netagent(ifnet_t ifp) {
  int error;

  error = kern_nexus_interface_add_netagent(ifp);
  os_log(OS_LOG_DEFAULT, "kern_nexus_interface_add_netagent(%s) returned %d",
         ifp->if_xname, error);
  return error == 0;
}

boolean_t ifnet_remove_netagent(ifnet_t ifp) {
  int error;

  error = kern_nexus_interface_remove_netagent(ifp);
  os_log(OS_LOG_DEFAULT, "kern_nexus_interface_remove_netagent(%s) returned %d",
         ifp->if_xname, error);
  return error == 0;
}

boolean_t ifnet_attach_flowswitch_nexus(ifnet_t ifp) {
...
boolean_t ifnet_detach_flowswitch_nexus(ifnet_t ifp) {
  if_nexus_flowswitch nexus_fsw;
...
void ifnet_attach_native_flowswitch(ifnet_t ifp) {
  if (!dlil_is_native_netif_nexus(ifp)) {
    /* not a native netif */
    return;
  }
...
int ifnet_set_flowswitch_rx_callback(ifnet_t ifp, ifnet_fsw_rx_cb_t cb,
                                     void *arg) {
  lck_mtx_lock(&ifp->if_delegate_lock);
  while (ifp->if_fsw_rx_cb_ref > 0) {
    DTRACE_SKYWALK1(wait__fsw, ifnet_t, ifp);
    (void)msleep(&ifp->if_fsw_rx_cb_ref, &ifp->if_delegate_lock, (PZERO + 1),
                 __FUNCTION__, NULL);
...
int ifnet_get_flowswitch_rx_callback(ifnet_t ifp, ifnet_fsw_rx_cb_t *cbp,
                                     void **argp) {
  /*
...
void ifnet_release_flowswitch_rx_callback(ifnet_t ifp) {
  lck_mtx_lock(&ifp->if_delegate_lock);
...
int ifnet_set_delegate_parent(ifnet_t difp, ifnet_t parent) {
...
int ifnet_get_delegate_parent(ifnet_t difp, ifnet_t *parentp) {
  lck_mtx_lock(&difp->if_delegate_lock);
...
void ifnet_release_delegate_parent(ifnet_t difp) {
  lck_mtx_lock(&difp->if_delegate_lock);
...
ifnet_set_detach_notify_locked(ifnet_t ifp, ifnet_detach_notify_cb_t notify,
                               void *arg) {
...
__attribute__((noinline)) void
ifnet_get_detach_notify_locked(ifnet_t ifp, ifnet_detach_notify_cb_t *notifyp,
                               void **argp) {
  ifnet_lock_assert(ifp, IFNET_LCK_ASSERT_EXCLUSIVE);
...
__attribute__((noinline)) void
ifnet_set_detach_notify(ifnet_t ifp, ifnet_detach_notify_cb_t notify,
                        void *arg) {
  ifnet_lock_exclusive(ifp);
...
__attribute__((noinline)) void
ifnet_get_detach_notify(ifnet_t ifp, ifnet_detach_notify_cb_t *notifyp,
                        void **argp) {
  ifnet_lock_exclusive(ifp);
...
int proto_hash_value(u_int32_t protocol_family) {
  /*
...
dlil_post_msg(struct ifnet *ifp, u_int32_t event_subclass, u_int32_t event_code,
  ...
  if (event_subclass == KEV_DL_SUBCLASS) {
    /* Don't update interface generation for frequent link quality and state
     * changes  */
    switch (event_code) {
...
static void dlil_reset_rxpoll_params(ifnet_t ifp) {
  ASSERT(ifp != NULL);
...
#if SKYWALK
static void dlil_filter_event(struct eventhandler_entry_arg arg __unused,
  ...
  if (old_if_enable_fsw_transport_netagent !=
...
void dlil_init(void) {
  ...
  PE_parse_boot_argn("if_link_heuristics", &if_link_heuristics_flags,
                     sizeof(if_link_heuristics_flags));
   ...
   */
  pe_enable_fsw_transport_netagent = PE_get_default(
      "kern.skywalk_netagent_enable", &pe_enable_fsw_transport_netagent,
      sizeof(pe_enable_fsw_transport_netagent));
  pe_disable_fsw_transport_netagent = PE_get_default(
      "kern.skywalk_netagent_disable", &pe_disable_fsw_transport_netagent,
  ...
  VERIFY(
      ...
      (pe_enable_fsw_transport_netagent ^ pe_disable_fsw_transport_netagent));
   ...
   */
  (void)PE_parse_boot_argn("if_attach_nx", &if_attach_nx, sizeof(if_attach_nx));
  if_enable_fsw_transport_netagent =
      ((if_attach_nx & IF_ATTACH_NX_FSW_TRANSPORT_NETAGENT) != 0);

  if_netif_all = ((if_attach_nx & IF_ATTACH_NX_NETIF_ALL) != 0);

  if (pe_disable_fsw_transport_netagent && if_enable_fsw_transport_netagent) {
...
#if (DEVELOPMENT || DEBUG)
  (void)PE_parse_boot_argn("fsw_use_max_mtu_buffer", &fsw_use_max_mtu_buffer,
                           sizeof(fsw_use_max_mtu_buffer));
#endif /* (DEVELOPMENT || DEBUG) */
   ...
   */
  dlil_incr_pending_thread_count();
  if (kernel_thread_start(ifnet_detacher_thread_func, NULL, &thread) !=
      KERN_SUCCESS) {
...
__private_extern__ int dlil_attach_filter(struct ifnet *ifp,
  ...
  if (!ifnet_get_ioref(ifp)) {
    os_log(OS_LOG_DEFAULT, "%s: %s is no longer attached", __func__,
           if_name(ifp));
    retval = ENXIO;
    goto done;
  }
...
static int dlil_detach_filter_internal(interface_filter_t filter,
                                       int detached) {
  int retval = 0;
  ...
  if (detached == 0) {
    ...
    TAILQ_FOREACH(ifp, &ifnet_head, if_link) {
      ...
      TAILQ_FOREACH(entry, &ifp->if_flt_head, filt_next) {
         ...
         */
        if ((filter->filt_flags & DLIL_IFF_BRIDGE) != 0) {
          ifp->if_bridge = NULL;
        }
        if_flt_monitor_leave(ifp);
        lck_mtx_unlock(&ifp->if_flt_lock);
...
__private_extern__ void dlil_detach_filter(interface_filter_t filter) {
  if (filter == NULL) {
    return;
  }
...
__private_extern__ boolean_t dlil_has_ip_filter(void) {
  boolean_t has_filter = ((net_api_stats.nas_ipf_add_count -
                           net_api_stats.nas_ipf_add_os_count) > 0);
...
__private_extern__ boolean_t dlil_has_if_filter(struct ifnet *ifp) {
  boolean_t has_filter = !TAILQ_EMPTY(&ifp->if_flt_head);
  DTRACE_IP1(dlil_has_if_filter, boolean_t, has_filter);
  return has_filter;
}

errno_t dlil_rxpoll_validate_params(struct ifnet_poll_params *p) {
  if (p != NULL) {
...
void dlil_rxpoll_update_params(struct ifnet *ifp, struct ifnet_poll_params *p) {
  u_int64_t sample_holdtime, inbw;

  if ((inbw = ifnet_input_linkrate(ifp)) == 0 && p == NULL) {
    sample_holdtime = 0; /* polling is disabled */
    ifp->if_rxpoll_wlowat = ifp->if_rxpoll_plowat = ifp->if_rxpoll_blowat = 0;
    ifp->if_rxpoll_whiwat = ifp->if_rxpoll_phiwat = ifp->if_rxpoll_bhiwat =
        (u_int32_t)-1;
    ...
    plowat = ((p == NULL || p->packets_lowat == 0) ? rxpoll_tbl[n].plowat
                                                   : p->packets_lowat);
    phiwat = ((p == NULL || p->packets_hiwat == 0) ? rxpoll_tbl[n].phiwat
                                                   : p->packets_hiwat);
    blowat = ((p == NULL || p->bytes_lowat == 0) ? rxpoll_tbl[n].blowat
                                                 : p->bytes_lowat);
    bhiwat = ((p == NULL || p->bytes_hiwat == 0) ? rxpoll_tbl[n].bhiwat
                                                 : p->bytes_hiwat);
    plim = ((p == NULL || p->packets_limit == 0 || if_rxpoll_max != 0)
                ? if_rxpoll_max
                : p->packets_limit);
  ...
  if (dlil_verbose) {
    DLIL_PRINTF("%s: speed %llu bps, sample per %llu nsec, "
                ...
                "bytes limits [%u/%u]\n",
                if_name(ifp), inbw, sample_holdtime, ifp->if_rxpoll_ival,
 ...
 */
errno_t dlil_rxpoll_set_params(struct ifnet *ifp, struct ifnet_poll_params *p,
                               boolean_t locked) {
  errno_t err;
 ...
 * Must be called on an attached ifnet (caller is expected to check.)
 */
errno_t dlil_rxpoll_get_params(struct ifnet *ifp, struct ifnet_poll_params *p) {
  struct dlil_threading_info *inp;
...
errno_t ifnet_input(struct ifnet *ifp, struct mbuf *m_head,
                    const struct ifnet_stat_increment_param *s) {
  return ifnet_input_common(ifp, m_head, NULL, s, FALSE, FALSE);
}

errno_t ifnet_input_extended(struct ifnet *ifp, struct mbuf *m_head,
                             struct mbuf *m_tail,
                             const struct ifnet_stat_increment_param *s) {
  return ifnet_input_common(ifp, m_head, m_tail, s, TRUE, FALSE);
}

errno_t ifnet_input_poll(struct ifnet *ifp, struct mbuf *m_head,
                         struct mbuf *m_tail,
                         const struct ifnet_stat_increment_param *s) {
  return ifnet_input_common(ifp, m_head, m_tail, s, (m_head != NULL), TRUE);
}

static errno_t ifnet_input_common(struct ifnet *ifp, struct mbuf *m_head,
                                  struct mbuf *m_tail,
                                  const struct ifnet_stat_increment_param *s,
                                  boolean_t ext, boolean_t poll) {
  ...
  if (last != m_tail) {
    panic_plain("%s: invalid input packet chain for %s, "
                ...
                __func__, if_name(ifp), m_tail, last);
  }
  ...
  if (ext && s->packets_in != m_cnt) {
    panic_plain("%s: input packet count mismatch for %s, "
                "%d instead of %d\n",
                __func__, if_name(ifp), s->packets_in, m_cnt);
  }
...
static void ifnet_start_common(struct ifnet *ifp, boolean_t resetfc,
                               boolean_t ignore_delay) {
...
void ifnet_start(struct ifnet *ifp) { ifnet_start_common(ifp, FALSE, FALSE); }

void ifnet_start_ignore_delay(struct ifnet *ifp) {
  ifnet_start_common(ifp, FALSE, TRUE);
}

__attribute__((noreturn)) static void ifnet_start_thread_func(void *v,
                                                              wait_result_t w) {
#pragma unused(w)
  ifnet_ref_t ifp = v;
  ...
  (void)snprintf(thread_name, sizeof(thread_name), "ifnet_start_%s",
                 ifp->if_xname);
#if SKYWALK
  /* override name for native Skywalk interface */
  if (ifp->if_eflags & IFEF_SKYWALK_NATIVE) {
    (void)snprintf(thread_name, sizeof(thread_name), "skywalk_doorbell_%s_tx",
                   ifp->if_xname);
  }
#endif /* SKYWALK */
  ASSERT(ifp->if_start_thread == current_thread());
  thread_set_thread_name(current_thread(),
                         __unsafe_null_terminated_from_indexable(thread_name));
...
ifnet_start_thread_cont(void *v, wait_result_t wres) {
  ...
  for (;;) {
    ...
    if ((ifp->if_start_flags & IFSF_NO_DELAY) == 0 && !IFCQ_IS_EMPTY(ifq) &&
        (ifp->if_eflags & IFEF_ENQUEUE_MULTI) && ifp->if_start_delayed == 0 &&
  ...
  if (__probable((ifp->if_start_flags & IFSF_TERMINATING) == 0)) {
    ...
    if (__improbable(ts != NULL)) {
      clock_interval_to_deadline(
          (uint32_t)(ts->tv_nsec + (ts->tv_sec * NSEC_PER_SEC)), 1, &deadline);
    }

    (void)assert_wait_deadline(&ifp->if_start_thread, THREAD_UNINT, deadline);
    lck_mtx_unlock(&ifp->if_start_lock);
    (void)thread_block_parameter(ifnet_start_thread_cont, ifp);
    /* NOTREACHED */
  } else {
...
void ifnet_set_start_cycle(struct ifnet *ifp, struct timespec *ts) {
  if (ts == NULL) {
...
static inline void ifnet_poll_wakeup(struct ifnet *ifp) {
...
void ifnet_poll(struct ifnet *ifp) {
  /*
...
__attribute__((noreturn)) static void ifnet_poll_thread_func(void *v,
                                                             wait_result_t w) {
#pragma unused(w)
  char thread_name[MAXTHREADNAMESIZE];
  ifnet_ref_t ifp = v;
  ...
  /* construct the name for this thread, and then apply it */
  bzero(thread_name, sizeof(thread_name));
  (void)snprintf(thread_name, sizeof(thread_name), "ifnet_poller_%s",
                 ifp->if_xname);
  thread_set_thread_name(ifp->if_poll_thread,
                         __unsafe_null_terminated_from_indexable(thread_name));
...
ifnet_poll_thread_cont(void *v, wait_result_t wres) {
  ...
  for (;;) {
    ...
    ((*ifp->if_input_poll)(ifp, 0, m_lim, &m_head, &m_tail, &m_cnt, &m_totlen));
    ...
    if (m_head != NULL) {
      ...
      if (dlil_verbose > 1) {
        DLIL_PRINTF("%s: polled %d pkts, "
                    "pkts avg %d max %d, wreq avg %d, "
                    "bytes avg %d\n",
                    if_name(ifp), m_cnt, ifp->if_rxpoll_pavg,
      ...
      (void)ifnet_input_common(ifp, m_head, m_tail, &s, TRUE, TRUE);
    } else {
      if (dlil_verbose > 1) {
        DLIL_PRINTF("%s: no packets, "
                    "pkts avg %d max %d, wreq avg %d, "
                    "bytes avg %d\n",
                    if_name(ifp), ifp->if_rxpoll_pavg, ifp->if_rxpoll_pmax,
                    ifp->if_rxpoll_wavg, ifp->if_rxpoll_bavg);
      }

      (void)ifnet_input_common(ifp, NULL, NULL, NULL, FALSE, TRUE);
    }
...
skip:
  ...
  if (__probable((ifp->if_poll_flags & IF_POLLF_TERMINATING) == 0)) {
    ...
    (void)assert_wait_deadline(&ifp->if_poll_thread, THREAD_UNINT, deadline);
    lck_mtx_unlock(&ifp->if_poll_lock);
    (void)thread_block_parameter(ifnet_poll_thread_cont, ifp);
    /* NOTREACHED */
  } else {
...
void ifnet_set_poll_cycle(struct ifnet *ifp, struct timespec *ts) {
  if (ts == NULL) {
...
void ifnet_purge(struct ifnet *ifp) {
  if (ifp != NULL && (ifp->if_eflags & IFEF_TXSTART)) {
    if_qflush(ifp, ifp->if_snd);
  }
}

void ifnet_update_rcv(struct ifnet *ifp, cqev_t ev) {
  switch (ev) {
...
errno_t ifnet_set_output_sched_model(struct ifnet *ifp, u_int32_t model) {
  return ifclassq_change(ifp->if_snd, model);
}

errno_t ifnet_set_sndq_maxlen(struct ifnet *ifp, u_int32_t maxqlen) {
  if (ifp == NULL) {
    return EINVAL;
...
errno_t ifnet_get_sndq_maxlen(struct ifnet *ifp, u_int32_t *maxqlen) {
...
errno_t ifnet_get_sndq_len(struct ifnet *ifp, u_int32_t *pkts) {
  errno_t err;
  ...
  if (ifp == NULL || pkts == NULL) {
    ...
    err = ifclassq_get_len(ifp->if_snd, MBUF_SC_UNSPEC, IF_CLASSQ_ALL_GRPS,
                           pkts, NULL);
  }
...
errno_t ifnet_get_service_class_sndq_len(struct ifnet *ifp, mbuf_svc_class_t sc,
                                         u_int32_t *pkts, u_int32_t *bytes) {
  errno_t err;

  if (ifp == NULL || !MBUF_VALID_SC(sc) || (pkts == NULL && bytes == NULL)) {
    err = EINVAL;
  } else if (!(ifp->if_eflags & IFEF_TXSTART)) {
    err = ENXIO;
  } else {
    err = ifclassq_get_len(ifp->if_snd, sc, IF_CLASSQ_ALL_GRPS, pkts, bytes);
  }

  return err;
}

errno_t ifnet_set_rcvq_maxlen(struct ifnet *ifp, u_int32_t maxqlen) {
  struct dlil_threading_info *inp;
...
errno_t ifnet_get_rcvq_maxlen(struct ifnet *ifp, u_int32_t *maxqlen) {
  struct dlil_threading_info *inp;
...
void ifnet_enqueue_multi_setup(struct ifnet *ifp, uint16_t delay_qlen,
  ...
  if (delay_qlen > 0 && delay_timeout > 0) {
    ...
    kprintf("%s: forced IFEF_ENQUEUE_MULTI qlen %u timeout %u\n", ifp->if_xname,
            (uint32_t)delay_qlen, (uint32_t)delay_timeout);
  } else {
...
static inline errno_t ifnet_enqueue_single(struct ifnet *ifp,
                                           struct ifclassq *ifcq,
                                           classq_pkt_t *p, boolean_t flush,
                                           boolean_t *pdrop) {
#if SKYWALK
  ...
  case QP_MBUF:
    ...
    if ((p->cp_mbuf->m_pkthdr.pkt_flags & PKTF_FLOW_ID) &&
      ...
      if (!(p->cp_mbuf->m_pkthdr.pkt_flags & PKTF_SO_BACKGROUND)) {
     ...
     * class (rdar://51507725).
     */
    if ((p->cp_mbuf->m_flags & M_MCAST) != 0 && IFNET_IS_WIFI_INFRA(ifp)) {
      ...
      if (__improbable(len < sizeof(struct ether_header))) {
        ...
        if ((p->cp_mbuf = m_pullup(p->cp_mbuf, sizeof(struct ether_header))) ==
            NULL) {
          return ENOMEM;
        }
      }
      ...
      if (etype == ETHERTYPE_IP) {
        hlen = sizeof(struct ether_header) + sizeof(struct ip);
        if (len < hlen) {
        ...
        hlen = sizeof(struct ether_header) + sizeof(struct ip6_hdr);
        if (len < hlen) {
      ...
      if (pullup) {
        if ((p->cp_mbuf = m_pullup(p->cp_mbuf, (int)hlen)) == NULL) {
          return ENOMEM;
        }
  ...
  case QP_PACKET:
    ...
    if ((p->cp_kpkt->pkt_link_flags & PKT_LINKF_MCAST) != 0 &&
      ...
      if (__improbable(pktlen < sizeof(struct ether_header))) {
        DTRACE_IP1(pkt__small__ether, __kern_packet *, p->cp_kpkt);
        break;
      }
      ...
      if (etype == ETHERTYPE_IP) {
        if (pktlen < sizeof(struct ether_header) + sizeof(struct ip)) {
          DTRACE_IP1(pkt__small__v4, uint32_t, pktlen);
          break;
        }
        ip_ver = IPVERSION;
      } else if (etype == ETHERTYPE_IPV6) {
        if (pktlen < sizeof(struct ether_header) + sizeof(struct ip6_hdr)) {
          DTRACE_IP1(pkt__small__v6, uint32_t, pktlen);
          break;
        }
        ip_ver = IPV6_VERSION;
      } else {
        DTRACE_IP1(pkt__invalid__etype, uint16_t, etype);
        break;
      }
      mcast_buf = (uint8_t *)(eh + 1);
      /*
  ...
  if (ifp->if_eflags & IFEF_ENQUEUE_MULTI) {
    ...
    if (ifp->if_start_delay_swin > 0) {
      if ((ifp->if_start_delay_swin + dwin) > now_nsec) {
      ...
      } else if ((now_nsec - ifp->if_start_delay_swin) >= (200 * 1000 * 1000)) {
        ...
        if (ifp->if_start_delay_cnt >= ifp->if_start_delay_qlen) {
  ...
  error = ifclassq_enqueue(((ifcq != NULL) ? ifcq : ifp->if_snd), p, p, 1,
                           pktlen, pdrop);
...
ifnet_enqueue_chain(struct ifnet *ifp, struct ifclassq *ifcq,
                    classq_pkt_t *head, classq_pkt_t *tail, uint32_t cnt,
                    uint32_t bytes, boolean_t flush, boolean_t *pdrop) {
  int error;

  /* enqueue the packet (caller consumes object) */
  error = ifclassq_enqueue(ifcq != NULL ? ifcq : ifp->if_snd, head, tail, cnt,
                           bytes, pdrop);
...
int ifnet_enqueue_netem(void *handle, pktsched_pkt_t *__sized_by(n_pkts) pkts,
                        uint32_t n_pkts) {
  ...
  for (i = 0; i < n_pkts - 1; i++) {
    (void)ifnet_enqueue_single(ifp, ifp->if_snd, &pkts[i].pktsched_pkt, FALSE,
                               &pdrop);
  }
  /* flush with the last packet */
  (void)ifnet_enqueue_single(ifp, ifp->if_snd, &pkts[i].pktsched_pkt, TRUE,
                             &pdrop);

  return 0;
}

static inline errno_t ifnet_enqueue_common_single(struct ifnet *ifp,
                                                  struct ifclassq *ifcq,
                                                  classq_pkt_t *pkt,
                                                  boolean_t flush,
                                                  boolean_t *pdrop) {
...
errno_t ifnet_enqueue(struct ifnet *ifp, struct mbuf *m) {
...
errno_t ifnet_enqueue_mbuf(struct ifnet *ifp, struct mbuf *m, boolean_t flush,
                           boolean_t *pdrop) {
  classq_pkt_t pkt;
  ...
  if (ifp == NULL || m == NULL || !(m->m_flags & M_PKTHDR) ||
      m->m_nextpkt != NULL) {
    if (m != NULL) {
      m_drop_if(m, ifp, DROPTAP_FLAG_DIR_OUT, DROP_REASON_DLIL_ENQUEUE_INVALID,
                NULL, 0);
      *pdrop = TRUE;
    }
    return EINVAL;
    ...
    m_drop_if(m, ifp, DROPTAP_FLAG_DIR_OUT,
              DROP_REASON_DLIL_ENQUEUE_IF_NOT_ATTACHED, NULL, 0);
    *pdrop = TRUE;
    return ENXIO;
  } else if (!(ifp->if_flags & IFF_UP)) {
    m_drop_if(m, ifp, DROPTAP_FLAG_DIR_OUT, DROP_REASON_DLIL_ENQUEUE_IF_NOT_UP,
              NULL, 0);
    *pdrop = TRUE;
    return ENETDOWN;
  }
...
errno_t ifnet_enqueue_mbuf_chain(struct ifnet *ifp, struct mbuf *m_head,
                                 struct mbuf *m_tail, uint32_t cnt,
                                 uint32_t bytes, boolean_t flush,
                                 boolean_t *pdrop) {
  ...
  if (!ifnet_is_fully_attached(ifp)) {
    /* flag tested without lock for performance */
    m_drop_list(m_head, ifp, DROPTAP_FLAG_DIR_OUT,
                DROP_REASON_DLIL_ENQUEUE_IF_NOT_ATTACHED, NULL, 0);
    *pdrop = TRUE;
    return ENXIO;
  } else if (!(ifp->if_flags & IFF_UP)) {
    m_drop_list(m_head, ifp, DROPTAP_FLAG_DIR_OUT,
                DROP_REASON_DLIL_ENQUEUE_IF_NOT_UP, NULL, 0);
    *pdrop = TRUE;
    return ENETDOWN;
  }
  ...
  return ifnet_enqueue_chain(ifp, NULL, &head, &tail, cnt, bytes, flush, pdrop);
}

#if SKYWALK
errno_t ifnet_enqueue_pkt(struct ifnet *ifp, struct ifclassq *ifcq,
                          struct __kern_packet *kpkt, boolean_t flush,
                          boolean_t *pdrop) {
  classq_pkt_t pkt;
  ...
  if (__improbable(ifp == NULL || kpkt == NULL)) {
    if (kpkt != NULL) {
      pp_free_packet(__DECONST(struct kern_pbufpool *, kpkt->pkt_qum.qum_pp),
                     SK_PTR_ADDR(kpkt));
      *pdrop = TRUE;
    }
    return EINVAL;
...
errno_t ifnet_enqueue_pkt_chain(struct ifnet *ifp, struct ifclassq *ifcq,
                                struct __kern_packet *k_head,
                                struct __kern_packet *k_tail, uint32_t cnt,
                                uint32_t bytes, boolean_t flush,
                                boolean_t *pdrop) {
  classq_pkt_t head, tail;
  ...
  return ifnet_enqueue_chain(ifp, ifcq, &head, &tail, cnt, bytes, flush, pdrop);
}
#endif /* SKYWALK */

errno_t ifnet_dequeue(struct ifnet *ifp, struct mbuf **mp) {
  errno_t rc;
  ...
  ASSERT(!(ifp->if_eflags & IFEF_SKYWALK_NATIVE));
#endif /* SKYWALK */
  rc = ifclassq_dequeue(ifp->if_snd, MBUF_SC_UNSPEC, 1,
...
errno_t ifnet_dequeue_service_class(struct ifnet *ifp, mbuf_svc_class_t sc,
                                    struct mbuf **mp) {
  errno_t rc;
  classq_pkt_t pkt = CLASSQ_PKT_INITIALIZER(pkt);
  ...
  ASSERT(!(ifp->if_eflags & IFEF_SKYWALK_NATIVE));
#endif /* SKYWALK */
  rc = ifclassq_dequeue(ifp->if_snd, sc, 1, CLASSQ_DEQUEUE_MAX_BYTE_LIMIT, &pkt,
                        NULL, NULL, NULL, 0);
  VERIFY((pkt.cp_ptype == QP_MBUF) || (pkt.cp_mbuf == NULL));
  ifnet_decr_iorefcnt(ifp);
  *mp = pkt.cp_mbuf;
...
errno_t ifnet_dequeue_multi(struct ifnet *ifp, u_int32_t pkt_limit,
...
errno_t ifnet_dequeue_multi_bytes(struct ifnet *ifp, u_int32_t byte_limit,
                                  struct mbuf **head, struct mbuf **tail,
                                  u_int32_t *cnt, u_int32_t *len) {
  errno_t rc;
  classq_pkt_t pkt_head = CLASSQ_PKT_INITIALIZER(pkt_head);
  ...
  ASSERT(!(ifp->if_eflags & IFEF_SKYWALK_NATIVE));
#endif /* SKYWALK */
  rc = ifclassq_dequeue(ifp->if_snd, MBUF_SC_UNSPEC,
                        CLASSQ_DEQUEUE_MAX_PKT_LIMIT, byte_limit, &pkt_head,
                        &pkt_tail, cnt, len, 0);
  VERIFY((pkt_head.cp_ptype == QP_MBUF) || (pkt_head.cp_mbuf == NULL));
  ifnet_decr_iorefcnt(ifp);
  *head = pkt_head.cp_mbuf;
...
errno_t ifnet_dequeue_service_class_multi(
    struct ifnet *ifp, mbuf_svc_class_t sc, u_int32_t pkt_limit,
    struct mbuf **head, struct mbuf **tail, u_int32_t *cnt, u_int32_t *len) {
  errno_t rc;
  classq_pkt_t pkt_head = CLASSQ_PKT_INITIALIZER(pkt_head);
  classq_pkt_t pkt_tail = CLASSQ_PKT_INITIALIZER(pkt_tail);

  if (ifp == NULL || head == NULL || pkt_limit < 1 || !MBUF_VALID_SC(sc)) {
    return EINVAL;
  } else if (!(ifp->if_eflags & IFEF_TXSTART) ||
             !IFNET_MODEL_IS_VALID(ifp->if_output_sched_model)) {
    return ENXIO;
...
#if XNU_TARGET_OS_OSX
errno_t ifnet_framer_stub(struct ifnet *ifp, struct mbuf **m,
                          const struct sockaddr *dest,
                          IFNET_LLADDR_T dest_linkaddr,
                          IFNET_FRAME_TYPE_T frame_type, u_int32_t *pre,
                          u_int32_t *post) {
  if (pre != NULL) {
    *pre = 0;
  }
...
int dlil_post_complete_msg(struct ifnet *ifp, struct kev_msg *event) {
  if (ifp != NULL) {
...
__private_extern__ void dlil_post_sifflags_msg(struct ifnet *ifp) {
  struct kev_msg ev_msg;
  struct net_event_data ev_data;
...
static int dlil_event_internal(struct ifnet *ifp, struct kev_msg *event,
                               bool update_generation) {
  struct ifnet_filter *filter = NULL;
  ...
  TAILQ_FOREACH(filter, &ifp->if_flt_head, filt_next) {
    if (filter->filt_event != NULL) {
      ...
      filter->filt_event(filter->filt_cookie, ifp, filter->filt_protocol,
                         event);
  ...
  if (if_proto_count) {
    ...
    if (if_proto_count <= TMP_IF_PROTO_ARR_SIZE) {
      ...
      tmp_ifproto_arr =
          kalloc_type(struct if_proto *, if_proto_count, Z_WAITOK | Z_ZERO);
  ...
  for (tmp_ifproto_arr_idx = 0; tmp_ifproto_arr_idx < if_proto_count;
    ...
    proto_media_event eventp =
        (proto->proto_kpi == kProtoKPI_v1 ? proto->kpi.v1.event
                                          : proto->kpi.v2.event);
...
errno_t ifnet_event(ifnet_t ifp, struct kern_event_msg *event) {
...
void ifnet_ioctl_async(struct ifnet *ifp, u_long ioctl_code) {
  ...
  if (!ifnet_get_ioref(ifp)) {
    os_log(OS_LOG_DEFAULT,
           "%s:%d %s Failed for ioctl %lu as interface "
           "is not attached",
  ...
  case SIOCADDMULTI:
    ...
    if (!atomic_compare_exchange_strong(&ifp->if_mcast_add_signaled,
  ...
  case SIOCDELMULTI:
    compare_expected = false;
    if (!atomic_compare_exchange_strong(&ifp->if_mcast_del_signaled,
                                        &compare_expected, true)) {
      ifnet_decr_iorefcnt(ifp);
      return;
    }
    break;
  default:
    os_log(OS_LOG_DEFAULT, "%s:%d %s unknown ioctl %lu", __func__, __LINE__,
           if_name(ifp), ioctl_code);
    return;
  }
...
static void ifnet_ioctl_event_callback(struct nwk_wq_entry *nwk_item) {
  struct ifnet_ioctl_event_nwk_wq_entry *p_ev =
      __container_of(nwk_item, struct ifnet_ioctl_event_nwk_wq_entry, nwk_wqe);
  ...
  if ((ret = ifnet_ioctl(ifp, 0, ioctl_code, NULL)) != 0) {
    ...
    os_log(OS_LOG_DEFAULT,
           "%s:%d %s ifnet_ioctl returned successfully "
           "for ioctl %lu",
           __func__, __LINE__, if_name(ifp), ioctl_code);
  }
  ifnet_decr_iorefcnt(ifp);
...
errno_t ifnet_ioctl(ifnet_t ifp, protocol_family_t proto_fam, u_long ioctl_code,
                    void *ioctl_arg) {
  ...
  if_flt_monitor_busy(ifp);
  TAILQ_FOREACH(filter, &ifp->if_flt_head, filt_next) {
    if (filter->filt_ioctl != NULL &&
        (filter->filt_protocol == 0 || filter->filt_protocol == proto_fam)) {
      lck_mtx_unlock(&ifp->if_flt_lock);

      result = filter->filt_ioctl(filter->filt_cookie, ifp, proto_fam,
  ...
  if (proto_fam != 0) {
    ...
    if (proto != NULL) {
      proto_media_ioctl ioctlp =
          (proto->proto_kpi == kProtoKPI_v1 ? proto->kpi.v1.ioctl
                                            : proto->kpi.v2.ioctl);
      result = EOPNOTSUPP;
      if (ioctlp != NULL) {
...
__private_extern__ errno_t dlil_set_bpf_tap(ifnet_t ifp, bpf_tap_mode mode,
...
errno_t dlil_resolve_multi(struct ifnet *ifp, const struct sockaddr *proto_addr,
  ...
  ifnet_lock_done(ifp);
  if (proto != NULL) {
    resolvep = (proto->proto_kpi == kProtoKPI_v1 ? proto->kpi.v1.resolve_multi
                                                 : proto->kpi.v2.resolve_multi);
    if (resolvep != NULL) {
...
__private_extern__ errno_t dlil_send_arp_internal(
    ifnet_t ifp, u_short arpop, const struct sockaddr_dl *sender_hw,
  ...
  if (proto == NULL) {
    ...
    arpp = (proto->proto_kpi == kProtoKPI_v1 ? proto->kpi.v1.send_arp
                                             : proto->kpi.v2.send_arp);
    if (arpp == NULL) {
      result = ENOTSUP;
    } else {
      switch (arpop) {
      ...
      result =
          arpp(ifp, arpop, sender_hw, sender_proto, target_hw, target_proto);
    }
    if_proto_free(proto);
  }
done:
...
static __inline__ int _is_announcement(const struct sockaddr_in *sender_sin,
...
__private_extern__ errno_t dlil_send_arp(ifnet_t ifp, u_short arpop,
  ...
  if (rtflags & RTF_ROUTER) {
    SOCKADDR_COPY(target_proto, &target_proto_sinarp,
  ...
  if (target_proto->sa_family == AF_INET &&
    ...
    if (ifnet_list_get(IFNET_FAMILY_ANY, &ifp_list, &count) == 0) {
      for (ifp_on = 0; ifp_on < count; ifp_on++) {
        ...
        TAILQ_FOREACH(source_ip, &cur_ifp->if_addrhead, ifa_link) {
          IFA_LOCK(source_ip);
          if (source_ip->ifa_addr != NULL &&
              source_ip->ifa_addr->sa_family == AF_INET) {
            /* Copy the source IP address */
            SOCKADDR_COPY(SIN(source_ip->ifa_addr), &source_ip_copy,
                          sizeof(source_ip_copy));
        ...
        /* Send the ARP */
        new_result =
            dlil_send_arp_internal(cur_ifp, arpop, SDL(source_hw->ifa_addr),
                                   SA(&source_ip_copy), NULL, target_proto);
 ...
 */
static int ifnet_lookup(struct ifnet *ifp) {
  ifnet_ref_t _ifp;
 ...
 */
int ifnet_get_ioref(struct ifnet *ifp) {
  bool ret;
...
void ifnet_incr_pending_thread_count(struct ifnet *ifp) {
  lck_mtx_lock_spin(&ifp->if_ref_lock);
  ifp->if_threads_pending++;
  lck_mtx_unlock(&ifp->if_ref_lock);
}

void ifnet_decr_pending_thread_count(struct ifnet *ifp) {
  lck_mtx_lock_spin(&ifp->if_ref_lock);
 ...
 */
void ifnet_incr_iorefcnt(struct ifnet *ifp) { os_ref_retain(&ifp->if_refio); }

void ifnet_decr_iorefcnt(struct ifnet *ifp) {
  /*
...
static void ifnet_decr_iorefcnt_locked(struct ifnet *ifp) {
  /*
   * if there are no more outstanding io references, wakeup the
   * ifnet_detach thread.
   */
...
boolean_t ifnet_datamov_begin(struct ifnet *ifp) {
  boolean_t ret;
...
void ifnet_datamov_end(struct ifnet *ifp) {
  uint32_t datamov;
  /*
   * if there's no more thread moving data, wakeup any
   * drainers that's blocked waiting for this.
   */
  datamov = os_ref_release_raw_relaxed_mask(&ifp->if_datamov, IF_DATAMOV_BITS,
                                            &if_datamovgrp);
...
static void ifnet_datamov_suspend_locked(struct ifnet *ifp) {
...
static void ifnet_datamov_suspend(struct ifnet *ifp) {
  lck_mtx_lock_spin(&ifp->if_ref_lock);
...
boolean_t ifnet_datamov_suspend_if_needed(struct ifnet *ifp) {
  lck_mtx_lock_spin(&ifp->if_ref_lock);
  VERIFY(ifp->if_refflags & (IFRF_ATTACHED | IFRF_DETACHING));
  if (ifp->if_suspend > 0) {
    lck_mtx_unlock(&ifp->if_ref_lock);
...
void ifnet_datamov_drain(struct ifnet *ifp) {
  lck_mtx_lock(&ifp->if_ref_lock);
  VERIFY(ifp->if_refflags & (IFRF_ATTACHED | IFRF_DETACHING));
  ...
  while (os_ref_get_count_mask(&ifp->if_datamov, IF_DATAMOV_BITS) > 1) {
    ...
    (void)msleep(&(ifp->if_datamov), &ifp->if_ref_lock, (PZERO - 1), __func__,
                 NULL);
...
void ifnet_datamov_suspend_and_drain(struct ifnet *ifp) {
  ifnet_datamov_suspend(ifp);
  ifnet_datamov_drain(ifp);
}

void ifnet_datamov_resume(struct ifnet *ifp) {
  lck_mtx_lock(&ifp->if_ref_lock);
  /* data movement must already be suspended */
  VERIFY(ifp->if_suspend > 0);
  if (--ifp->if_suspend == 0) {
...
dlil_attach_protocol(struct if_proto *proto,
                     const struct ifnet_demux_desc *__counted_by(demux_count)
                         demux_list,
                     u_int32_t demux_count, uint32_t *proto_count) {
   ...
   */
  retval =
      ifp->if_add_proto(ifp, proto->protocol_family, demux_list, demux_count);
  if (retval) {
    ifnet_lock_done(ifp);
    goto ioref_done;
  }
...
static void dlil_handle_proto_attach(ifnet_t ifp, protocol_family_t protocol) {
  /*
...
ifnet_attach_protocol(ifnet_t ifp, protocol_family_t protocol,
  ...
  retval = dlil_attach_protocol(ifproto, proto_details->demux_list,
                                proto_details->demux_count, &proto_count);
  ...
  if (retval == EEXIST) {
    ...
    if (dlil_verbose) {
      DLIL_PRINTF("%s: protocol %d already attached\n",
                  ifp != NULL ? if_name(ifp) : "N/A", protocol);
    }
  } else if (retval != 0) {
    ...
    DLIL_PRINTF("%s: attached v1 protocol %d (count = %d)\n",
                ifp != NULL ? if_name(ifp) : "N/A", protocol, proto_count);
  }
  ifnet_head_done();
  if (retval == 0) {
...
errno_t ifnet_attach_protocol_v2(
    ifnet_t ifp, protocol_family_t protocol,
...
errno_t ifnet_detach_protocol(ifnet_t ifp, protocol_family_t proto_family) {
  struct if_proto *proto = NULL;
  int retval = 0;
  ...
  SLIST_REMOVE(&ifp->if_proto_hash[proto_hash_value(proto_family)], proto,
               if_proto, next_hash);
  ...
  if (dlil_verbose) {
    DLIL_PRINTF("%s: detached %s protocol %d\n", if_name(ifp),
                (proto->proto_kpi == kProtoKPI_v1) ? "v1" : "v2", proto_family);
  }
...
static errno_t ifproto_media_input_v1(struct ifnet *ifp,
...
static errno_t ifproto_media_input_v2(struct ifnet *ifp,
                                      protocol_family_t protocol,
                                      struct mbuf *packet) {
...
static errno_t ifproto_media_preout(struct ifnet *ifp,
                                    protocol_family_t protocol, mbuf_t *packet,
                                    const struct sockaddr *dest, void *route,
...
static void ifproto_media_event(struct ifnet *ifp, protocol_family_t protocol,
                                const struct kev_msg *event) {
#pragma unused(ifp, protocol, event)
}

static errno_t ifproto_media_ioctl(struct ifnet *ifp,
                                   protocol_family_t protocol,
...
static errno_t ifproto_media_resolve_multi(ifnet_t ifp,
...
static errno_t ifproto_media_send_arp(struct ifnet *ifp, u_short arpop,
                                      const struct sockaddr_dl *sender_hw,
                                      const struct sockaddr *sender_proto,
                                      const struct sockaddr_dl *target_hw,
                                      const struct sockaddr *target_proto) {
...
void dlil_ifclassq_setup(struct ifnet *ifp, struct ifclassq *ifcq) {
  uint32_t sflags = 0;
  int err;
...
errno_t ifnet_attach(ifnet_t ifp, const struct sockaddr_dl *ll_addr) {
#if SKYWALK
  ...
  if (!(ifp->if_refflags & IFRF_EMBRYONIC)) {
    panic_plain("%s: flags mismatch (embryonic not set) ifp=%p", __func__, ifp);
    /* NOTREACHED */
  }
  lck_mtx_unlock(&ifp->if_ref_lock);
  ...
  VERIFY(ifp->if_proto_hash == NULL);
  ifp->if_proto_hash = kalloc_type(struct proto_hash_entry, PROTO_HASH_SLOTS,
                                   Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  if (ifp->if_index == 0) {
    ...
    if (idx == -1 && if_index >= UINT16_MAX) {
      for (int i = 1; i < if_index; i++) {
        if (ifindex2ifnet[i] == NULL && ifnet_addrs[i - 1] == NULL) {
          idx = i;
          break;
        }
      }
    ...
    if (ll_addr != NULL && ifp->if_type == IFT_ETHER &&
      ...
      bcopy(CONST_LLADDR(ll_addr), dl_if->dl_if_permanent_ether,
            ETHER_ADDR_LEN);
  ...
  if (ifp->if_eflags & IFEF_TXSTART) {
    ...
    ifnet_incr_pending_thread_count(ifp);
    if ((err = kernel_thread_start(ifnet_start_thread_func, ifp,
    ...
    info.importance = 1;
    kret = thread_policy_set(ifp->if_start_thread, THREAD_PRECEDENCE_POLICY,
  ...
  if (dlil_is_rxpoll_input(thfunc)) {
    ...
    bzero(&info, sizeof(info));
    info.importance = 1;
    kret = thread_policy_set(ifp->if_poll_thread, THREAD_PRECEDENCE_POLICY,
                             (thread_policy_t)&info,
                             THREAD_PRECEDENCE_POLICY_COUNT);
    ASSERT(kret == KERN_SUCCESS);
  }
  ...
  if (!LIST_EMPTY(&ifp->if_multiaddrs)) {
    ...
    DLIL_PRINTF("%s: attached with %d suspended link-layer multicast "
                "membership(s)\n",
                if_name(ifp), ifp->if_updatemcasts);
  }
  ...
  if (IFNET_IS_WIFI_INFRA(ifp) && net_qos_policy_wifi_enabled != 0) {
    ...
    error = if_set_qosmarking_mode(ifp, IFRTYPE_QOSMARKING_FASTLANE);
    if (error != 0) {
  ...
  lck_mtx_unlock(&ifp->if_ref_lock);
  DLIL_PRINTF(
      "%s: All kernel threads created for interface %s have been scheduled "
  ...
  if (net_rtref) {
    ...
    (void)ifnet_set_idle_flags_locked(ifp, IFRF_IDLE_NOTIFY, IFRF_IDLE_NOTIFY);
  } else {
  ...
  if (netif_compat || dlil_is_native_netif_nexus(ifp)) {
    if (netif_compat) {
      if (sk_netif_compat_txmodel == NETIF_COMPAT_TXMODEL_ENQUEUE_MULTI) {
        ifnet_enqueue_multi_setup(ifp, sk_tx_delay_qlen, sk_tx_delay_timeout);
      }
...
static void if_purgeaddrs(struct ifnet *ifp) {
#if INET
  in_purgeaddrs(ifp);
#endif /* INET */
  in6_purgeaddrs(ifp);
...
errno_t ifnet_detach(ifnet_t ifp) {
  ...
  if (ifp->if_tcp_kao_cnt > 0) {
    os_log(OS_LOG_DEFAULT, "%s %s tcp_kao_cnt %u not zero", __func__,
           if_name(ifp), ifp->if_tcp_kao_cnt);
  }
   ...
   */
  ifp->if_creation_generation_id =
      os_atomic_inc(&if_creation_generation_count, relaxed);
...
static void ifnet_detaching_enqueue(struct ifnet *ifp) {
  dlil_if_lock_assert();
...
__dead2 static void ifnet_detacher_thread_func(void *v, wait_result_t w) {
#pragma unused(v, w)
  dlil_if_lock();
  (void)assert_wait(&ifnet_delayed_run, THREAD_UNINT);
...
static void ifnet_detach_final(struct ifnet *ifp) {
  ...
  if (!(ifp->if_refflags & IFRF_DETACHING)) {
    panic("%s: flags mismatch (detaching not set) ifp=%p", __func__, ifp);
    /* NOTREACHED */
  }
  ...
  if (os_ref_release_relaxed(&ifp->if_refio) > 0) {
    ...
    while (os_ref_get_count(&ifp->if_refio) > 0) {
      ...
      DLIL_PRINTF("%s: %s waiting for IO references to drain\n", __func__,
                  if_name(ifp));
      (void)msleep(&(ifp->if_refio), &ifp->if_ref_lock, (PZERO - 1),
                   "ifnet_ioref_wait", NULL);
    }
    if (waited) {
  ...
  kfree_type_counted_by(struct proto_hash_entry, ifp->if_proto_hash_count,
                        ifp->if_proto_hash);
  ...
  if (ifp->if_start_thread != THREAD_NULL) {
    ...
    while (ifp->if_start_thread != THREAD_NULL) {
      ...
      (void)msleep(&ifp->if_start_thread, &ifp->if_start_lock, (PZERO - 1),
                   "ifnet_start_thread_exit", NULL);
    }
    lck_mtx_unlock(&ifp->if_start_lock);
    if (dlil_verbose) {
      DLIL_PRINTF("%s: %s starter thread termination complete", __func__,
                  if_name(ifp));
    }
  }
  ...
  if (ifp->if_poll_thread != THREAD_NULL) {
    ...
    while (ifp->if_poll_thread != THREAD_NULL) {
      ...
      (void)msleep(&ifp->if_poll_thread, &ifp->if_poll_lock, (PZERO - 1),
  ...
  if ((inp = ifp->if_inp) != NULL) {
    ...
    if (inp->dlth_affinity) {
      ...
      if (ptp != NULL) {
        ...
        (void)dlil_affinity_set(ptp, THREAD_AFFINITY_TAG_NULL);
        thread_deallocate(ptp);
      }

      /* Tear down workloop thread affinity */
      if (wtp != NULL) {
        (void)dlil_affinity_set(wtp, THREAD_AFFINITY_TAG_NULL);
    ...
    if (inp->dlth_thread != THREAD_NULL) {
      ...
      lck_mtx_lock_spin(&inp->dlth_lock);
      while ((inp->dlth_flags & DLIL_INPUT_TERMINATE_COMPLETE) == 0) {
        (void)msleep(&inp->dlth_flags, &inp->dlth_lock, (PZERO - 1) | PSPIN,
                     inp->dlth_name, NULL);
      }
      lck_mtx_unlock(&inp->dlth_lock);
      ifnet_lock_exclusive(ifp);
...
errno_t ifp_if_output(struct ifnet *ifp, struct mbuf *m) {
#pragma unused(ifp)
  m_freem_list(m);
  return 0;
}

void ifp_if_start(struct ifnet *ifp) { ifnet_purge(ifp); }
...
static void ifp_if_input_poll(struct ifnet *ifp, u_int32_t flags,
                              u_int32_t max_cnt, struct mbuf **m_head,
                              struct mbuf **m_tail, u_int32_t *cnt,
                              u_int32_t *len) {
...
static errno_t ifp_if_ctl(struct ifnet *ifp, ifnet_ctl_cmd_t cmd,
                          u_int32_t arglen, void *arg) {
#pragma unused(ifp, cmd, arglen, arg)
  return EOPNOTSUPP;
}

static errno_t ifp_if_demux(struct ifnet *ifp, struct mbuf *m, char *fh,
...
static errno_t ifp_if_add_proto(struct ifnet *ifp, protocol_family_t pf,
                                const struct ifnet_demux_desc *da,
                                u_int32_t dc) {
#pragma unused(ifp, pf, da, dc)
  return EINVAL;
}

static errno_t ifp_if_del_proto(struct ifnet *ifp, protocol_family_t pf) {
#pragma unused(ifp, pf)
  return EINVAL;
}

static errno_t ifp_if_check_multi(struct ifnet *ifp,
...
static errno_t ifp_if_framer(struct ifnet *ifp, struct mbuf **m,
                             const struct sockaddr *sa, IFNET_LLADDR_T ll,
                             IFNET_FRAME_TYPE_T t, u_int32_t *pre,
                             u_int32_t *post)
#else  /* XNU_TARGET_OS_OSX */
static errno_t ifp_if_framer(struct ifnet *ifp, struct mbuf **m,
                             const struct sockaddr *sa, IFNET_LLADDR_T ll,
...
static errno_t ifp_if_framer_extended(struct ifnet *ifp, struct mbuf **m,
                                      const struct sockaddr *sa,
                                      IFNET_LLADDR_T ll, IFNET_FRAME_TYPE_T t,
...
errno_t ifp_if_ioctl(struct ifnet *ifp, unsigned long cmd, void *arg) {
#pragma unused(ifp, cmd, arg)
  return EOPNOTSUPP;
}

static errno_t ifp_if_set_bpf_tap(struct ifnet *ifp, bpf_tap_mode tm,
                                  bpf_packet_func f) {
...
static void ifp_if_free(struct ifnet *ifp) {
#pragma unused(ifp)
}

static void ifp_if_event(struct ifnet *ifp, const struct kev_msg *e) {
#pragma unused(ifp, e)
}

__private_extern__ void dlil_proto_unplumb_all(struct ifnet *ifp) {
  /*
...
static void ifp_src_route_copyout(struct ifnet *ifp, struct route *dst) {
...
static void ifp_src_route_copyin(struct ifnet *ifp, struct route *src) {
  lck_mtx_lock_spin(&ifp->if_cached_route_lock);
  lck_mtx_convert_spin(&ifp->if_cached_route_lock);
...
static void ifp_src_route6_copyout(struct ifnet *ifp, struct route_in6 *dst) {
  lck_mtx_lock_spin(&ifp->if_cached_route_lock);
  lck_mtx_convert_spin(&ifp->if_cached_route_lock);
...
static void ifp_src_route6_copyin(struct ifnet *ifp, struct route_in6 *src) {
  lck_mtx_lock_spin(&ifp->if_cached_route_lock);
  lck_mtx_convert_spin(&ifp->if_cached_route_lock);

  if (ifp->if_fwd_cacheok) {
    route_copyin((struct route *)src, (struct route *)&ifp->if_src_route6,
                 sizeof(*src));
  } else {
    ROUTE_RELEASE(src);
  }
...
struct rtentry *ifnet_cached_rtlookup_inet(struct ifnet *ifp,
  ...
  if (ROUTE_UNUSABLE(&src_rt) || src_ip.s_addr != dst->sin_addr.s_addr) {
    ...
    src_rt.ro_rt = rtalloc1_scoped(SA(dst), 0, 0, ifp->if_index);
...
struct rtentry *ifnet_cached_rtlookup_inet6(struct ifnet *ifp,
  ...
  if (ROUTE_UNUSABLE(&src_rt) ||
    ...
    bcopy(src_ip6, &src_rt.ro_dst.sin6_addr, sizeof(src_rt.ro_dst.sin6_addr));

    if (src_rt.ro_rt == NULL) {
      src_rt.ro_rt = rtalloc1_scoped(SA(&src_rt.ro_dst), 0, 0, ifp->if_index);
...
void if_lqm_update(struct ifnet *ifp, int lqm, int locked) {
...
static void if_rrc_state_update(struct ifnet *ifp, unsigned int rrc_state) {
  ...
  ifp->if_interface_state.valid_bitmask |= IF_INTERFACE_STATE_RRC_STATE_VALID;
...
errno_t if_state_update(struct ifnet *ifp,
                        struct if_interface_state *if_interface_state) {
  u_short if_index_available = 0;

  ifnet_lock_exclusive(ifp);

  if ((ifp->if_type != IFT_CELLULAR) && (if_interface_state->valid_bitmask &
                                         IF_INTERFACE_STATE_RRC_STATE_VALID)) {
    ifnet_lock_done(ifp);
    return ENOTSUP;
  }
  ...
  if ((if_interface_state->valid_bitmask &
       IF_INTERFACE_STATE_RRC_STATE_VALID) &&
      if_interface_state->rrc_state != IF_INTERFACE_STATE_RRC_STATE_IDLE &&
      if_interface_state->rrc_state != IF_INTERFACE_STATE_RRC_STATE_CONNECTED) {
    ifnet_lock_done(ifp);
    return EINVAL;
  }

  if (if_interface_state->valid_bitmask & IF_INTERFACE_STATE_LQM_STATE_VALID) {
    if_lqm_update(ifp, if_interface_state->lqm_state, 1);
  }
  if (if_interface_state->valid_bitmask & IF_INTERFACE_STATE_RRC_STATE_VALID) {
...
void if_get_state(struct ifnet *ifp,
                  struct if_interface_state *if_interface_state) {
  ifnet_lock_shared(ifp);
  ...
  if (ifp->if_interface_state.valid_bitmask &
      IF_INTERFACE_STATE_RRC_STATE_VALID) {
    if_interface_state->valid_bitmask |= IF_INTERFACE_STATE_RRC_STATE_VALID;
    if_interface_state->rrc_state = ifp->if_interface_state.rrc_state;
  }
  if (ifp->if_interface_state.valid_bitmask &
      IF_INTERFACE_STATE_LQM_STATE_VALID) {
    if_interface_state->valid_bitmask |= IF_INTERFACE_STATE_LQM_STATE_VALID;
    if_interface_state->lqm_state = ifp->if_interface_state.lqm_state;
  }
  if (ifp->if_interface_state.valid_bitmask &
      IF_INTERFACE_STATE_INTERFACE_AVAILABILITY_VALID) {
...
errno_t if_probe_connectivity(struct ifnet *ifp, u_int32_t conn_probe) {
  if (conn_probe > 1) {
    return EINVAL;
  }
  if (conn_probe == 0) {
  ...
  os_log(OS_LOG_DEFAULT, "interface probing on %s set to %u by %s:%d",
         if_name(ifp), conn_probe, proc_best_name(current_proc()),
         proc_selfpid());
...
/* for uuid.c */
static int get_ether_index(int *ret_other_index) {
  ifnet_ref_t ifp;
  int en0_index = 0;
...
int uuid_get_ethernet(u_int8_t *__counted_by(ETHER_ADDR_LEN) node) {
  ...
  if (the_index != 0) {
    ...
    if (dl_if->dl_if_permanent_ether_is_set != 0) {
       ...
       */
      memcpy(node, dl_if->dl_if_permanent_ether, ETHER_ADDR_LEN);
    } else {
...
int dlil_node_present(struct ifnet *ifp, struct sockaddr *sa, int32_t rssi,
...
void dlil_node_absent(struct ifnet *ifp, struct sockaddr *sa) {
  ...
  if (error == 0) {
    ...
    dlil_post_msg(ifp, KEV_DL_SUBCLASS, KEV_DL_NODE_ABSENCE, &kev.link_data,
                  sizeof(kev), FALSE);
  }
}

int dlil_node_present_v2(struct ifnet *ifp, struct sockaddr *sa,
                         struct sockaddr_dl *sdl, int32_t rssi, int lqm,
                         int npm, u_int8_t srvinfo[48]) {
  struct kev_dl_node_presence kev = {};
  struct sockaddr_dl *kev_sdl = NULL;
  struct sockaddr_in6 *kev_sin6 = NULL;
  int ret = 0;
...
const void *dlil_ifaddr_bytes(const struct sockaddr_dl *sdl, size_t *sizep,
                              kauth_cred_t *credp) {
  ...
  if (dlil_lladdr_ckreq) {
    ...
    if (credp && mac_system_check_info(*credp, "net.link.addr")) {
      static const u_int8_t unspec[FIREWIRE_EUI64_LEN] = {[0] = 2};
...
void dlil_report_issues(struct ifnet *ifp, u_int8_t modid[DLIL_MODIDLEN],
  ...
  dlil_post_msg(ifp, KEV_DL_SUBCLASS, KEV_DL_ISSUES, &kev.link_data,
                sizeof(kev), FALSE);
}

errno_t ifnet_getset_opportunistic(ifnet_t ifp, u_long cmd, struct ifreq *ifr,
                                   struct proc *p) {
  ...
  if (cmd == SIOCSIFOPPORTUNISTIC) {
    ...
    if (ifr->ifr_opportunistic.ifo_flags == IFRIFOF_BLOCK_OPPORTUNISTIC) {
    ...
    if (level == IFNET_THROTTLE_OPPORTUNISTIC) {
      ifr->ifr_opportunistic.ifo_flags |= IFRIFOF_BLOCK_OPPORTUNISTIC;
    }
  }
  ...
  if (result == 0) {
    uint32_t flags = 0;
    flags |= (cmd == SIOCSIFOPPORTUNISTIC) ? INPCB_OPPORTUNISTIC_SETCMD : 0;
...
int ifnet_get_throttle(struct ifnet *ifp, u_int32_t *level) {
  struct ifclassq *ifq;
...
int ifnet_set_throttle(struct ifnet *ifp, u_int32_t level) {
  struct ifclassq *ifq;
...
errno_t ifnet_getset_log(ifnet_t ifp, u_long cmd, struct ifreq *ifr,
                         struct proc *p) {
#pragma unused(p)
  errno_t result = 0;
  uint32_t flags;
  ...
  if (cmd == SIOCSIFLOG) {
    if ((result = priv_check_cred(kauth_cred_get(), PRIV_NET_INTERFACE_CONTROL,
                                  0)) != 0) {
      return result;
    }
    ...
    if (result == 0) {
      result = ifnet_set_log(ifp, level, flags, category, subcategory);
    }
  } else {
    result = ifnet_get_log(ifp, &level, &flags, &category, &subcategory);
    if (result == 0) {
...
int ifnet_set_log(struct ifnet *ifp, int32_t level, uint32_t flags,
  ...
  if (ifp->if_output_ctl != NULL) {
    ...
    if (l.flags != 0) {
      err = ifp->if_output_ctl(ifp, IFNET_CTL_SET_LOG, sizeof(l), &l);
    }
  ...
  if (err == 0) {
    ...
    log(LOG_INFO,
        "%s: logging level set to %d flags=0x%x "
        "arg=0x%x, category=%d subcategory=%d\n",
        if_name(ifp), ifp->if_log.level, ifp->if_log.flags, flags, category,
        subcategory);
  }
...
int ifnet_get_log(struct ifnet *ifp, int32_t *level, uint32_t *flags,
                  int32_t *category, int32_t *subcategory) {
  if (level != NULL) {
...
int ifnet_notify_address(struct ifnet *ifp, int af) {
  ...
  return ifp->if_output_ctl(ifp, IFNET_CTL_NOTIFY_ADDRESS, sizeof(na), &na);
}
...
errno_t ifnet_disable_output(struct ifnet *ifp) {
  int err = 0;
...
errno_t ifnet_enable_output(struct ifnet *ifp) {
  if (ifp == NULL) {
    return EINVAL;
  } else if (!(ifp->if_eflags & IFEF_TXSTART) ||
             !ifnet_is_fully_attached(ifp)) {
...
void ifnet_flowadv(uint32_t flowhash) {
 ...
 */
static inline int ifce_cmp(const struct ifnet_fc_entry *fc1,
                           const struct ifnet_fc_entry *fc2) {
  return fc1->ifce_flowhash - fc2->ifce_flowhash;
}

static int ifnet_fc_add(struct ifnet *ifp) {
...
static struct ifnet_fc_entry *ifnet_fc_get(uint32_t flowhash) {
  struct ifnet_fc_entry keyfc, *ifce;
  ifnet_ref_t ifp;
...
static void ifnet_fc_entry_free(struct ifnet_fc_entry *ifce) {
  zfree(ifnet_fc_zone, ifce);
}

static uint32_t ifnet_calc_flowhash(struct ifnet *ifp) {
...
int ifnet_set_netsignature(struct ifnet *ifp, uint8_t family, uint8_t len,
  ...
  case AF_INET6:
    ...
    if (IN6_IFEXTRA(ifp) != NULL) {
      if (len == 0) {
        ...
        bzero(IN6_IFEXTRA(ifp)->netsig, sizeof(IN6_IFEXTRA(ifp)->netsig));
...
int ifnet_get_netsignature(struct ifnet *ifp, uint8_t family, uint8_t *len,
                           uint16_t *flags, uint8_t *__sized_by(*len) data) {
  int error = 0;
...
int ifnet_set_nat64prefix(
  ...
  for (i = 0; i < NAT64_MAX_NUM_PREFIXES; i++) {
    uint32_t prefix_len = prefixes[i].prefix_len;
    struct in6_addr *prefix = &prefixes[i].ipv6_prefix;

    if (prefix_len == 0) {
      clat_log0((LOG_DEBUG, "NAT64 prefixes purged from Interface %s\n",
                 if_name(ifp)));
      /* Allow clearing the signature */
      ...
      clat_log0((LOG_DEBUG, "NAT64 prefixlen is incorrect %d\n", prefix_len));
      error = EINVAL;
      goto out;
    }

    if (IN6_IS_SCOPE_EMBED(prefix)) {
      clat_log0((LOG_DEBUG, "NAT64 prefix has interface/link local scope.\n"));
      error = EINVAL;
      goto out;
    }
    ...
    bcopy(prefix, &IN6_IFEXTRA(ifp)->nat64_prefixes[i].ipv6_prefix,
          sizeof(struct in6_addr));
    clat_log0((LOG_DEBUG, "NAT64 prefix set to %s with prefixlen: %d\n",
...
static uint8_t sumdata[] = {
    };
...
static struct {
  ...
  uint16_t sumrp; /* reference, precomputed */
    ...
    {FALSE, 0, 0, 0x0000},   {FALSE, 1, 0, 0x001f},   {FALSE, 2, 0, 0x8b1f},
    {FALSE, 3, 0, 0x8b27},   {FALSE, 7, 0, 0x790e},   {FALSE, 11, 0, 0xcb6d},
    {FALSE, 20, 0, 0x20dd},  {FALSE, 27, 0, 0xbabd},  {FALSE, 32, 0, 0xf3e8},
    {FALSE, 37, 0, 0x197d},  {FALSE, 43, 0, 0x9eae},  {FALSE, 64, 0, 0x4678},
    {FALSE, 127, 0, 0x9399}, {FALSE, 256, 0, 0xd147}, {FALSE, 325, 0, 0x0358},
};
...
__private_extern__ const char *dlil_kev_dl_code_str(u_int32_t event_code) {
  switch (event_code) {
...
void dlil_dt_tcall_fn(thread_call_param_t arg0, thread_call_param_t arg1) {
#pragma unused(arg1)
...
void ifnet_notify_data_threshold(struct ifnet *ifp) {
  ...
  if (threshold_notify && (bytes - oldbytes) > ifp->if_data_threshold &&
    ...
    if (tival != 0) {
      ...
      (void)thread_call_enter_delayed(ifp->if_dt_tcall, deadline);
    } else {
...
void ifnet_update_stats_per_flow(struct ifnet_stats_per_flow *ifs,
                                 struct ifnet *ifp) {
  tcp_update_stats_per_flow(ifs, ifp);
}

static inline u_int32_t _set_flags(u_int32_t *flags_p, u_int32_t set_flags) {
  return (u_int32_t)OSBitOrAtomic(set_flags, flags_p);
}

static inline u_int32_t _clear_flags(u_int32_t *flags_p,
                                     u_int32_t clear_flags) {
  return (u_int32_t)OSBitAndAtomic(~clear_flags, flags_p);
}

__private_extern__ u_int32_t if_set_eflags(ifnet_t interface,
                                           u_int32_t set_flags) {
  return _set_flags(&interface->if_eflags, set_flags);
}

__private_extern__ void if_clear_eflags(ifnet_t interface,
                                        u_int32_t clear_flags) {
  _clear_flags(&interface->if_eflags, clear_flags);
}

__private_extern__ u_int32_t if_set_xflags(ifnet_t interface,
                                           u_int32_t set_flags) {
  return _set_flags(&interface->if_xflags, set_flags);
}

__private_extern__ u_int32_t if_clear_xflags(ifnet_t interface,
                                             u_int32_t clear_flags) {
  return _clear_flags(&interface->if_xflags, clear_flags);
}

__private_extern__ void ifnet_update_traffic_rule_genid(ifnet_t ifp) {
  os_atomic_inc(&ifp->if_traffic_rule_genid, relaxed);
}

__private_extern__ boolean_t ifnet_sync_traffic_rule_genid(ifnet_t ifp,
                                                           uint32_t *genid) {
...
__private_extern__ void ifnet_update_inet_traffic_rule_count(ifnet_t ifp,
                                                             uint32_t count) {
  os_atomic_store(&ifp->if_inet_traffic_rule_count, count, relaxed);
  ifnet_update_traffic_rule_genid(ifp);
}

__private_extern__ void ifnet_update_eth_traffic_rule_count(ifnet_t ifp,
                                                            uint32_t count) {
...
static bool net_check_compatible_if_filter(struct ifnet *ifp) {
  if (ifp == NULL) {
    if (net_api_stats.nas_iflt_attach_count >
        net_api_stats.nas_iflt_attach_os_count) {
      return false;
    }
  } else {
...
int dlil_dump_top_if_qlen(char *__counted_by(str_len) str, int str_len) {
  char *c = str;
  int k, clen = str_len;
  ...
  if (top_ifcq_ifp != NULL) {
    k = scnprintf(c, clen, "\ntop ifcq_len %u packets by %s\n", top_ifcq_len,
                  top_ifcq_ifp->if_xname);
    DUMP_BUF_CHK();
  }
  if (top_inq_ifp != NULL) {
    k = scnprintf(c, clen, "\ntop inq_len %u packets by %s\n", top_inq_len,
                  top_inq_ifp->if_xname);
    DUMP_BUF_CHK();
  }
done:
  return str_len - clen;
