...
#define TRIE_CHILD_SET(t, i, b, node)                                          \
  {                                                                            \
    if (b >= FIRST_PRINTABLE_ASCII && b <= LAST_PRINTABLE_ASCII) {             \
      (((t)->child_maps +                                                      \
        (CHILD_MAP_SIZE *                                                      \
         TRIE_NODE(t, i).child_map))[(b - FIRST_PRINTABLE_ASCII)]) = node;     \
    } else {                                                                   \
      NET_TRIE_LOG(LOG_ERR, "NETrie - out of printable acsii range <%X>", b);  \
    }                                                                          \
  }

static uint16_t trie_node_alloc(struct net_trie *trie) {
...
static uint16_t trie_child_map_alloc(struct net_trie *trie) {
...
static uint16_t trie_bytes_move(struct net_trie *trie, uint16_t bytes_idx,
                                size_t bytes_size) {
...
static boolean_t
net_trie_has_high_ascii(const uint8_t *__sized_by(string_length) string,
...
boolean_t net_trie_init(struct net_trie *new_trie, size_t prefix_count,
                        size_t leaf_count, size_t bytes_count) {
  ...
  if (new_trie == NULL || prefix_count <= 0 || leaf_count <= 0 ||
      bytes_count <= 0) {
    NET_TRIE_LOG(
        LOG_ERR,
        "%s: NET_TRIE - null trie, no prefix/leaf count or no byte count",
        __FUNCTION__);
    return false;
  }

  if (os_add3_overflow(prefix_count, leaf_count, 1,
                       &nodes_count)) { /* + 1 for the root node */
    NET_TRIE_LOG(LOG_ERR,
                 "%s: NET_TRIE - Overflow while computing the number of nodes",
                 __FUNCTION__);
    return false;
  }
  ...
  if (os_add_overflow(prefix_count, 1,
    ...
    NET_TRIE_LOG(LOG_ERR,
                 "%s: NET_TRIE - Overflow while computing the number of maps",
                 __FUNCTION__);
    return false;
  }
  ...
  if (bytes_count > UINT16_MAX || nodes_count > UINT16_MAX ||
    ...
    NET_TRIE_LOG(LOG_ERR,
                 "%s: NET_TRIE - Invalid bytes count (%lu), nodes count (%lu) "
                 "or maps count (%lu)",
                 __FUNCTION__, bytes_count, nodes_count, maps_count);
    return false;
  }
  ...
  if (os_mul_overflow(sizeof(*new_trie->nodes), (size_t)nodes_count,
      ...
      os_add3_overflow(nodes_mem_size, child_maps_mem_size, bytes_mem_size,
                       &trie_memory_size)) {
    NET_TRIE_LOG(LOG_ERR,
                 "%s: NET_TRIE - Overflow while computing trie memory sizes",
                 __FUNCTION__);
    return false;
  }
  ...
  if (trie_memory_size > MAX_TRIE_MEMORY) {
    NET_TRIE_LOG(
        LOG_ERR,
        "%s: NET_TRIE - Trie memory size (%lu) is too big (maximum is %u)",
        __FUNCTION__, trie_memory_size, MAX_TRIE_MEMORY);
    return false;
  }

  NET_TRIE_LOG(LOG_DEBUG,
               "%s: NET_TRIE - initializing (Nodes count = %lu, child maps "
               "count = %lu, bytes_count = %lu, total memory size %lu)",
               __FUNCTION__, nodes_count, maps_count, bytes_count,
               trie_memory_size);
  ...
  if (memory == NULL) {
    NET_TRIE_LOG(
        LOG_ERR,
        "%s: NET_TRIE - Failed to allocate %lu bytes of memory for the trie",
        __FUNCTION__, trie_memory_size);
    return false;
  }
  ...
  new_trie->bytes =
      (uint8_t *)(void *)(data_memory + nodes_mem_size + child_maps_mem_size);
...
boolean_t net_trie_init_with_mem(struct net_trie *new_trie,
                                 uint8_t *__sized_by(trie_memory_size) memory,
                                 size_t trie_memory_size, size_t nodes_mem_size,
                                 size_t child_maps_mem_size,
                                 size_t bytes_mem_size, uint16_t nodes_count,
  ...
  if (os_add3_overflow(nodes_mem_size, child_maps_mem_size, bytes_mem_size,
                       &test_trie_memory_size) ||
      os_mul_overflow(sizeof(*new_trie->nodes), (size_t)nodes_count,
                      &test_nodes_mem_size) ||
      os_mul3_overflow(sizeof(*new_trie->child_maps), CHILD_MAP_SIZE,
                       (size_t)child_maps_count, &test_child_maps_mem_size) ||
      os_mul_overflow(sizeof(*new_trie->bytes), (size_t)bytes_count,
                      &test_bytes_mem_size)) {
    NET_TRIE_LOG(LOG_ERR,
                 "%s: NET_TRIE - Overflow while validating trie memory sizes",
                 __FUNCTION__);
    return false;
  }
  if (test_trie_memory_size != trie_memory_size) {
    NET_TRIE_LOG(LOG_ERR,
                 "%s: NET_TRIE - passed in mem sizes (nodes %zu maps %zu bytes "
                 "%zu) not equal to total mem %zu",
                 __FUNCTION__, nodes_mem_size, child_maps_mem_size,
                 bytes_mem_size, trie_memory_size);
    return false;
  }
  if (test_nodes_mem_size != nodes_mem_size) {
    NET_TRIE_LOG(LOG_ERR, "%s: NET_TRIE - passed in nodes_count %d not valid",
                 __FUNCTION__, nodes_count);
    return false;
  }
  if (test_child_maps_mem_size != child_maps_mem_size) {
    NET_TRIE_LOG(LOG_ERR, "%s: NET_TRIE - passed in maps_count %d not valid",
                 __FUNCTION__, child_maps_count);
    return false;
  }
  if (test_bytes_mem_size != bytes_mem_size) {
    NET_TRIE_LOG(LOG_ERR, "%s: NET_TRIE - passed in bytes_count %d not valid",
                 __FUNCTION__, bytes_count);
    return false;
  }
  ...
  new_trie->memory = memory;
  new_trie->trie_memory_size = trie_memory_size;
  NET_TRIE_LOG(LOG_DEBUG, "%s: NET_TRIE - initialized with malloc %zu",
               __FUNCTION__, trie_memory_size);
  ...
  NET_TRIE_LOG(
      LOG_DEBUG,
      "%s: NET_TRIE - initialized - mem %X (size %zu) nodes %X (size %zu count "
      "%d) maps %X (size %zu count %d) bytes %X (size %zu count %d)",
      __FUNCTION__, (unsigned int)new_trie->memory, new_trie->trie_memory_size,
      (unsigned int)new_trie->nodes, new_trie->nodes_mem_size,
      new_trie->nodes_count, (unsigned int)new_trie->child_maps,
      new_trie->child_maps_mem_size, new_trie->child_maps_count,
      (unsigned int)new_trie->bytes, new_trie->bytes_mem_size,
      new_trie->bytes_count);
...
uint16_t net_trie_insert(struct net_trie *trie,
                         const uint8_t *__sized_by(string_length) string,
                         size_t string_length,
                         const uint8_t *__sized_by(metadata_length) metadata,
                         size_t metadata_length, boolean_t reverse) {
  if (trie->memory == NULL || string == NULL || string_length == 0) {
    return NULL_TRIE_IDX;
  }

  if (string_length > UINT16_MAX ||
      trie->bytes_free_next + (uint16_t)string_length > trie->bytes_count) {
    NET_TRIE_LOG(LOG_ERR,
                 "%s: NET_TRIE - failed insert - out of allocated memory",
                 __FUNCTION__);
    return NULL_TRIE_IDX;
  }
  ...
  if (net_trie_has_high_ascii(string, string_length)) {
    NET_TRIE_LOG(
        LOG_ERR,
        "%s: NET_TRIE - failed insert - non-printable ASCII not supported",
        __FUNCTION__);
    return NULL_TRIE_IDX;
  }
  ...
  while (child != NULL_TRIE_IDX) {
    ...
    if (node_idx <
      ...
      if (prefix == NULL_TRIE_IDX) {
        NET_TRIE_LOG(LOG_ERR,
                     "%s: NET_TRIE - Ran out of trie nodes while splitting an "
                     "existing node",
                     __FUNCTION__);
        return NULL_TRIE_IDX;
      }
      ...
      if (string_remainder == 0) {
        ...
        if (metadata && metadata_length > 0) {
          ...
          TRIE_NODE(trie, prefix).metadata =
              trie_bytes_move(trie, trie->bytes_free_next, metadata_length);
      ...
      if (TRIE_NODE(trie, prefix).child_map == NULL_TRIE_IDX) {
        NET_TRIE_LOG(LOG_ERR,
                     "%s: NET_TRIE - Ran out of child maps while splitting an "
                     "existing node",
                     __FUNCTION__);
        return NULL_TRIE_IDX;
      }
       ...
       * the prefix */
      TRIE_CHILD_SET(trie, parent,
                     TRIE_BYTE(trie, TRIE_NODE(trie, prefix).start), prefix);
  ...
  if (string_remainder > 0) {
    ...
    if (leaf == NULL_TRIE_IDX) {
      NET_TRIE_LOG(
          LOG_ERR,
          "%s: NET_TRIE - Ran out of trie nodes while inserting a new leaf",
          __FUNCTION__);
      return NULL_TRIE_IDX;
    }
    ...
    if (TRIE_NODE(trie, leaf).start == NULL_TRIE_IDX) {
      NET_TRIE_LOG(LOG_ERR,
                   "%s: NET_TRIE - Ran out of bytes while inserting a new leaf",
                   __FUNCTION__);
      return NULL_TRIE_IDX;
    }
    ...
    if (TRIE_NODE(trie, current).child_map == NULL_TRIE_IDX) {
      ...
      if (TRIE_NODE(trie, current).child_map == NULL_TRIE_IDX) {
        NET_TRIE_LOG(
            LOG_ERR,
            "%s: NET_TRIE - Ran out of child maps while inserting a new leaf",
            __FUNCTION__);
        return NULL_TRIE_IDX;
      }
    }
    TRIE_CHILD_SET(trie, current, TRIE_BYTE(trie, TRIE_NODE(trie, leaf).start),
                   leaf);
    current = leaf;
...
uint16_t net_trie_search(struct net_trie *trie,
  ...
  NET_TRIE_LOG(LOG_DEBUG, "NET_TRIE - search %s len %zu reverse %d", string,
  ...
  while (current != NULL_TRIE_IDX) {
    ...
    if (reverse) {
      for (node_idx = TRIE_NODE(trie, current).start;
           node_idx < node_end && string_idx >= 0 &&
           string[string_idx] == TRIE_BYTE(trie, node_idx);
           node_idx++, string_idx--) {
#if NET_TRIE_DEBUG_SEARCH
      ...
      for (node_idx = TRIE_NODE(trie, current).start;
           node_idx < node_end && string_idx < (int16_t)string_length &&
           string[string_idx] == TRIE_BYTE(trie, node_idx);
           node_idx++, string_idx++) {
#if NET_TRIE_DEBUG_SEARCH
        NET_TRIE_LOG(LOG_DEBUG, "%c", string[string_idx]);
    ...
    // Any char matching the node string are not high Ascii.  Only need to check
    // mismatched char.
    if (string_idx >= 0 && string_idx < (int16_t)string_length &&
        HIGH_ASCII(string[string_idx])) {
...
#if NET_TRIE_DEBUG_SEARCH
    NET_TRIE_LOG(LOG_DEBUG, "NET_TRIE - node_idx %d node_end %d", node_idx,
                 node_end);
#endif

    if (node_idx == node_end) {
      boolean_t exact_matched = ((reverse && string_idx < 0) ||
                                 (string_idx == (int16_t)string_length));
      boolean_t partial_matched =
          (!exact_matched && partial_match_allowed &&
           (string[string_idx] == partial_match_terminator));

#if NET_TRIE_DEBUG_SEARCH
      NET_TRIE_LOG(LOG_DEBUG,
                   "NET_TRIE - reverse %d string_idx %d byte %d leaf %d "
                   "(exact_matched %d partial_matched %d)",
                   reverse, string_idx,
                   string_idx >= 0 && string_idx < (int16_t)string_length
                       ? string[string_idx]
                       : 888,
                   TRIE_NODE(trie, current).is_leaf, exact_matched,
                   partial_matched);
#endif
      ...
      if (TRIE_NODE(trie, current).is_leaf == true) {
        ...
        const uint8_t *data =
            (metadata_idex > 0) ? &TRIE_BYTE(trie, metadata_idex) : NULL;
