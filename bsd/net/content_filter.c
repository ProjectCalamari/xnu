 ...
 *
 * It should be noted that messages about many INET/INET6 sockets can be
 * multiplexed over a single kernel control socket.
 *
 * Notes:
 ...
 *    stored in the INET/INET6 socket structure so the NECP lookup is really
 * simple.
 *
 * 4) The NECP filter control unit is then used to find the corresponding
 *    kernel control socket instance.
 *
 * Note: NECP currently supports a single filter control unit per INET/INET6
 * socket but this restriction may be soon lifted.
 *
 *
 ...
 * created, each pass offset is initially set to 0 so no data is allowed to pass
 * by default.  When the pass offset is set to CFM_MAX_OFFSET via a
 * CFM_OP_DATA_UPDATE then the data flow becomes unrestricted.
 *
 ...
 * so no CFM_OP_DATA_OUT and CFM_OP_DATA_IN event messages are dispatched by
 * default until a CFM_OP_DATA_UPDATE action message with a greater than 0 peek
 * offset is sent by the user space filter agent.  When the peek offset is set
 * to CFM_MAX_OFFSET via a CFM_OP_DATA_UPDATE then the flow of update data
 * events becomes unrestricted.
 *
 ...
 * each new flow identified by its 4-tuple of source address/port and
 * destination address/port, a "struct cfil_info" is created.  Each datagram
 * socket may have multiple flows maintained in a hash table of "struct
 * cfil_info" entries.
 *
 ...
 *
 * Each datagram flow is uniquely identified by it's "struct cfil_info"
 * cfi_sock_id. The highest 32-bits of the cfi_sock_id contains the socket's
 * so_gencnt.  This portion of the cfi_sock_id is used locate the socket during
 * socket lookup.  The lowest 32-bits of the cfi_sock_id contains a hash of the
 * flow's 4-tuple.  This portion of the cfi_sock_id is used as the hash value
 * for the flow hash table lookup within the parent socket.
 *
 * Since datagram sockets may not be connected, flow states may not be
 * maintained in the socket structures and thus have to be saved for each
 * packet.  These saved states will be used for both outgoing and incoming
 * reinjections.  For outgoing packets, destination address/port as well as the
 * current socket states will be saved.  During reinjection, these saved states
 * will be used instead.  For incoming packets, control and address mbufs will
 * be chained to the data.  During reinjection, the whole chain will be queued
...
uint32_t cfil_sock_attached_stats_count =
...
static KALLOC_TYPE_DEFINE(content_filter_zone, struct content_filter,
                          NET_KT_DEFAULT);
...
#define CFI_ADD_TIME_LOG(cfil, t1, t0, op)                                     \
  ...
  if (offset < CFI_MAX_TIME_LOG_ENTRY) {                                       \
    ...
    (cfil)->cfi_op_time[offset] =                                              \
        (uint32_t)(_tdiff.tv_sec * 1000 + _tdiff.tv_usec / 1000);              \
...
struct cfil_info {
  ...
  uint32_t cfi_op_time[CFI_MAX_TIME_LOG_ENTRY]; /* time interval in microseconds
                                                   since first event */
  ...
  boolean_t cfi_isSignatureLatest; /* Indicates if signature covers latest flow
                                      attributes */
...
#define CFIL_QUEUE_VERIFY(x)                                                   \
  if (cfil_debug)                                                              \
  cfil_queue_verify(x)
#define CFIL_INFO_VERIFY(x)                                                    \
  if (cfil_debug)                                                              \
  cfil_info_verify(x)
 ...
 */
#define IS_ICMP(so)                                                            \
  (so && (SOCK_CHECK_TYPE(so, SOCK_RAW) || SOCK_CHECK_TYPE(so, SOCK_DGRAM)) && \
   (SOCK_CHECK_PROTO(so, IPPROTO_ICMP) ||                                      \
    SOCK_CHECK_PROTO(so, IPPROTO_ICMPV6)))
#define IS_RAW(so)                                                             \
  (so && SOCK_CHECK_TYPE(so, SOCK_RAW) && SOCK_CHECK_PROTO(so, IPPROTO_RAW))

#define OPTIONAL_IP_HEADER(so) (!IS_TCP(so) && !IS_UDP(so))
#define GET_SO_PROTOCOL(so) (so ? SOCK_PROTO(so) : IPPROTO_IP)
#define GET_SO_INP_PROTOCOL(so)                                                \
  ((so && sotoinpcb(so)) ? sotoinpcb(so)->inp_ip_p : IPPROTO_IP)
#define GET_SO_PROTO(so)                                                       \
  ((GET_SO_PROTOCOL(so) != IPPROTO_IP) ? GET_SO_PROTOCOL(so)                   \
                                       : GET_SO_INP_PROTOCOL(so))
#define IS_INP_V6(inp) (inp && (inp->inp_vflag & INP_IPV6))

#define UNCONNECTED(inp)                                                       \
  (inp &&                                                                      \
   (((inp->inp_vflag & INP_IPV4) && (inp->inp_faddr.s_addr == INADDR_ANY)) ||  \
    ((inp->inp_vflag & INP_IPV6) &&                                            \
     IN6_IS_ADDR_UNSPECIFIED(&inp->in6p_faddr))))
#define IS_ENTRY_ATTACHED(cfil_info, kcunit)                                   \
  (cfil_info != NULL && (kcunit <= MAX_CONTENT_FILTER) &&                      \
   cfil_info->cfi_entries[kcunit - 1].cfe_filter != NULL)
#define IS_DNS(local, remote)                                                  \
  (check_port(local, 53) || check_port(remote, 53) ||                          \
   check_port(local, 5353) || check_port(remote, 5353))
#define IS_INITIAL_TFO_DATA(so)                                                \
  (so && (so->so_flags1 & SOF1_PRECONNECT_DATA) &&                             \
   (so->so_state & SS_ISCONNECTING))
#define NULLADDRESS(addr)                                                      \
  ((addr.sa.sa_len == 0) ||                                                    \
...
#define SKIP_FILTER_FOR_TCP_SOCKET(so)                                         \
  (so == NULL ||                                                               \
   ...
   !SOCK_CHECK_TYPE(so, SOCK_STREAM) || !SOCK_CHECK_PROTO(so, IPPROTO_TCP) ||  \
   (so->so_flags & SOF_MP_SUBFLOW) != 0 ||                                     \
   (so->so_flags1 & SOF1_CONTENT_FILTER_SKIP) != 0)

/*
 * Special handling for 0.0.0.0-faddr TCP flows.  This flows will be changed to
 * loopback addr by TCP and may result in an immediate TCP RESET and socket
 * close.  This leads to CFIL blocking the owner thread for 1 sec waiting for
 * ack from user-space provider (ack recevied by CFIL but socket already removed
 * from global socket list).  To avoid this, identify these flows and do not
 * perform the close-wait blocking. These flows are identified as destined to
 * Loopback address and were disconnected shortly after connect (before
 * initial-verdict received).
 */
#define IS_LOOPBACK_FADDR(inp)                                                 \
  (inp && ((IS_INP_V6(inp) && IN6_IS_ADDR_LOOPBACK(&inp->in6p_faddr)) ||       \
           (ntohl(inp->inp_faddr.s_addr) == INADDR_LOOPBACK)))

#define SET_NO_CLOSE_WAIT(inp, cfil_info)                                      \
  if (inp && cfil_info && !(cfil_info->cfi_flags & CFIF_INITIAL_VERDICT) &&    \
      IS_LOOPBACK_FADDR(inp)) {                                                \
    cfil_info->cfi_flags |= CFIF_NO_CLOSE_WAIT;                                \
  }

#define IS_NO_CLOSE_WAIT(cfil_info)                                            \
  (cfil_info && (cfil_info->cfi_flags & CFIF_NO_CLOSE_WAIT))
...
#define MATCH_PORT(inp, local, remote)                                         \
  ((inp && ntohs(inp->inp_lport) == cfil_log_port) ||                          \
   (inp && ntohs(inp->inp_fport) == cfil_log_port) ||                          \
...
#define DEBUG_FLOW(inp, so, local, remote)                                     \
  ((cfil_log_port && MATCH_PORT(inp, local, remote)) ||                        \
   (cfil_log_pid && MATCH_PID(so)) || (cfil_log_proto && MATCH_PROTO(so)))
...
#define SO_DELAYED_TCP_TIME_WAIT_GET(so)                                       \
  (so->so_cfil ? (so->so_cfil->cfi_flags & CFIF_SO_DELAYED_TCP_TIME_WAIT)      \
               : false)
...
#define CFIL_STATS_REPORT_RUN_INTERVAL_NSEC                                    \
  (CFIL_STATS_REPORT_INTERVAL_MIN_MSEC * NSEC_PER_MSEC)
...
static struct cfil_stats_report_buffer
    *global_cfil_stats_report_buffers[MAX_CONTENT_FILTER];
static uint32_t global_cfil_stats_counts[MAX_CONTENT_FILTER];

/*
 * UDP Garbage Collection:
 */
#define UDP_FLOW_GC_ACTION_TO                                                  \
  10 // Flow Action Timeout (no action from user space) in seconds
#define UDP_FLOW_GC_MAX_COUNT 100 // Max UDP flows to be handled per run

/*
 * UDP flow queue thresholds
 */
#define UDP_FLOW_GC_MBUF_CNT_MAX                                               \
  (2 << MBSHIFT) // Max mbuf byte count in flow queue (2MB)
#define UDP_FLOW_GC_MBUF_NUM_MAX                                               \
  (UDP_FLOW_GC_MBUF_CNT_MAX >> MCLSHIFT) // Max mbuf count in flow queue (1K)
...
#define DO_PRESERVE_CONNECTIONS                                                \
...
SYSCTL_INT(_net_cfil, OID_AUTO, debug, CTLFLAG_RW | CTLFLAG_LOCKED, &cfil_debug,
           0, "");

SYSCTL_UINT(_net_cfil, OID_AUTO, sock_attached_count,
            CTLFLAG_RD | CTLFLAG_LOCKED, &cfil_sock_attached_count, 0, "");

SYSCTL_UINT(_net_cfil, OID_AUTO, active_count, CTLFLAG_RD | CTLFLAG_LOCKED,
            &cfil_active_count, 0, "");

SYSCTL_UINT(_net_cfil, OID_AUTO, close_wait_timeout,
            CTLFLAG_RW | CTLFLAG_LOCKED, &cfil_close_wait_timeout, 0, "");
...
SYSCTL_PROC(_net_cfil, OID_AUTO, filter_list, CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            sysctl_cfil_filter_list, "S,cfil_filter_stat", "");

SYSCTL_PROC(_net_cfil, OID_AUTO, sock_list, CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            sysctl_cfil_sock_list, "S,cfil_sock_stat", "");
 ...
 */
static int cfil_action_data_pass(struct socket *, struct cfil_info *, uint32_t,
...
static int cfil_data_common(struct socket *, struct cfil_info *, int,
                            struct sockaddr *, struct mbuf *, struct mbuf *,
                            uint32_t);
static int cfil_data_filter(struct socket *, struct cfil_info *, uint32_t, int,
                            struct mbuf *, uint32_t);
static void fill_ip_sockaddr_4_6(union sockaddr_in_4_6 *, struct in_addr,
                                 u_int16_t);
static void fill_ip6_sockaddr_4_6(union sockaddr_in_4_6 *, struct in6_addr *,
                                  u_int16_t, uint32_t);

static int cfil_dispatch_attach_event(struct socket *, struct cfil_info *,
                                      uint32_t, int);
...
static int cfil_service_pending_queue(struct socket *, struct cfil_info *,
                                      uint32_t, int);
static int cfil_data_service_ctl_q(struct socket *, struct cfil_info *,
                                   uint32_t, int);
static void cfil_info_verify(struct cfil_info *);
static int cfil_update_data_offsets(struct socket *, struct cfil_info *,
                                    uint32_t, int, uint64_t, uint64_t);
...
static struct cfil_info *cfil_sock_udp_get_info(struct socket *, uint32_t, bool,
                                                struct soflow_hash_entry *,
                                                struct sockaddr *,
                                                struct sockaddr *);
static errno_t cfil_sock_udp_handle_data(bool, struct socket *,
...
static void cfil_rw_lock_exclusive(lck_rw_t *lck) {
...
static void cfil_rw_unlock_exclusive(lck_rw_t *lck) {
  void *__single lr_saved;
...
static void cfil_rw_lock_shared(lck_rw_t *lck) {
  void *__single lr_saved;
...
static void cfil_rw_unlock_shared(lck_rw_t *lck) {
  void *__single lr_saved;
...
static boolean_t cfil_rw_lock_shared_to_exclusive(lck_rw_t *lck) {
  boolean_t upgraded;
  void *__single lr_saved;
...
static void cfil_rw_lock_exclusive_to_shared(lck_rw_t *lck) {
  void *__single lr_saved;
...
static void cfil_rw_lock_assert_held(lck_rw_t *lck, int exclusive) {
#if !MACH_ASSERT
#pragma unused(lck, exclusive)
#endif
  LCK_RW_ASSERT(lck, exclusive ? LCK_RW_ASSERT_EXCLUSIVE : LCK_RW_ASSERT_HELD);
}
 ...
 */
static unsigned int cfil_data_length(struct mbuf *m, int *retmbcnt,
                                     int *retmbnum) {
  struct mbuf *m0;
...
static struct mbuf *cfil_data_start(struct mbuf *m) {
  struct mbuf *m0;
...
static inline void cfil_queue_init(struct cfil_queue *cfq) {
  cfq->q_start = 0;
  cfq->q_end = 0;
  MBUFQ_INIT(&cfq->q_mq);
}

static inline uint64_t cfil_queue_drain(struct cfil_queue *cfq) {
...
static inline int cfil_queue_empty(struct cfil_queue *cfq) {
  return MBUFQ_EMPTY(&cfq->q_mq);
}

static inline uint64_t cfil_queue_offset_first(struct cfil_queue *cfq) {
  return cfq->q_start;
}

static inline uint64_t cfil_queue_offset_last(struct cfil_queue *cfq) {
  return cfq->q_end;
}

static inline uint64_t cfil_queue_len(struct cfil_queue *cfq) {
...
static void cfil_queue_verify(struct cfil_queue *cfq) {
  mbuf_t chain;
  mbuf_t m;
  mbuf_t n;
  ...
  VERIFY((MBUFQ_EMPTY(&cfq->q_mq) && cfq->q_start == cfq->q_end) ||
         (!MBUFQ_EMPTY(&cfq->q_mq) && cfq->q_start != cfq->q_end));
  ...
  MBUFQ_FOREACH(chain, &cfq->q_mq) {
    ...
    if (m == NULL || m == (void *)M_TAG_FREE_PATTERN ||
        m->m_next == (void *)M_TAG_FREE_PATTERN ||
        m->m_nextpkt == (void *)M_TAG_FREE_PATTERN) {
      panic("%s - mq %p is free at %p", __func__, &cfq->q_mq, m);
    }
    for (n = m; n != NULL; n = n->m_next) {
      if (!m_has_mtype(n, MTF_DATA | MTF_HEADER | MTF_OOBDATA)) {
        panic("%s - %p unsupported type %u", __func__, n, n->m_type);
      }
      chainsize += n->m_len;
    }
    if (mlen != chainsize) {
      panic("%s - %p m_length() %u != chainsize %lu", __func__, m, mlen,
            chainsize);
    }
    queuesize += chainsize;
  }
  OS_ANALYZER_SUPPRESS("81031590") if (queuesize != cfq->q_end - cfq->q_start) {
    panic("%s - %p queuesize %llu != offsetdiffs %llu", __func__, m, queuesize,
          cfq->q_end - cfq->q_start);
  }
}

static void cfil_queue_enqueue(struct cfil_queue *cfq, mbuf_t m, size_t len) {
...
static void cfil_queue_remove(struct cfil_queue *cfq, mbuf_t m, size_t len) {
  CFIL_QUEUE_VERIFY(cfq);
...
static mbuf_t cfil_queue_first(struct cfil_queue *cfq) {
  return MBUFQ_FIRST(&cfq->q_mq);
}

static mbuf_t cfil_queue_next(struct cfil_queue *cfq, mbuf_t m) {
#pragma unused(cfq)
  return MBUFQ_NEXT(m);
}

static void cfil_entry_buf_verify(struct cfe_buf *cfe_buf) {
...
static void cfil_entry_verify(struct cfil_entry *entry) {
...
static void cfil_info_verify(struct cfil_info *cfil_info) {
  int i;
...
static void verify_content_filter(struct content_filter *cfc) {
 ...
 */
static errno_t cfil_ctl_connect(kern_ctl_ref kctlref, struct sockaddr_ctl *sac,
                                void **unitinfo) {
  errno_t error = 0;
  ...
  CFIL_LOG(LOG_INFO, "return %d cfil_active_count %u kcunit %u", error,
           cfil_active_count, sac->sc_unit);

  return error;
}

static void cfil_update_behavior_flags(void) {
...
static errno_t cfil_ctl_disconnect(kern_ctl_ref kctlref, u_int32_t kcunit,
                                   void *unitinfo) {
#pragma unused(kctlref)
  ...
  if (kcunit > MAX_CONTENT_FILTER) {
    CFIL_LOG(LOG_ERR, "kcunit %u > MAX_CONTENT_FILTER (%d)", kcunit,
             MAX_CONTENT_FILTER);
    error = EINVAL;
    goto done;
  }
  ...
  if (content_filters[kcunit - 1] != cfc || cfc->cf_kcunit != kcunit) {
    CFIL_LOG(LOG_ERR, "bad unit info %u)", kcunit);
    cfil_rw_unlock_exclusive(&cfil_lck_rw);
    goto done;
  }
  ...
  while ((entry = TAILQ_FIRST(&cfc->cf_sock_entries)) != NULL) {
    ...
    if (entry->cfe_cfil_info && entry->cfe_cfil_info->cfi_so) {
      ...
      entry->cfe_flags |= CFEF_SENT_SOCK_ATTACHED | CFEF_DATA_START;

      // Before we release global lock, retain the cfil_info -
      // We attempt to retain a valid cfil_info to prevent any deallocation
      // until we are done.  Abort retain if cfil_info has already entered the
      // free code path.
      if (cfil_info == NULL ||
          os_ref_retain_try(&cfil_info->cfi_ref_count) == false) {
       ...
       */
      if ((so->so_cfil == NULL && so->so_flow_db == NULL) ||
          entry->cfe_filter == NULL) {
        cfil_rw_lock_exclusive(&cfil_lck_rw);
        goto release;
      }

      (void)cfil_action_data_pass(so, cfil_info, kcunit, 1, CFM_MAX_OFFSET,
                                  CFM_MAX_OFFSET);

      (void)cfil_action_data_pass(so, cfil_info, kcunit, 0, CFM_MAX_OFFSET,
                                  CFM_MAX_OFFSET);
      ...
      if (entry->cfe_filter == NULL ||
          (so->so_cfil == NULL && soflow_db_get_feature_context(
                                      so->so_flow_db, sock_flow_id) == NULL)) {
        goto release;
      }
 ...
 */
static int cfil_acquire_sockbuf(struct socket *so, struct cfil_info *cfil_info,
                                int outgoing) {
  ...
  while ((sb->sb_flags & SB_LOCK) ||
    ...
    msleep(&sb->sb_flags, mutex_held, PSOCK, "cfil_acquire_sockbuf", NULL);
  ...
  if (cfil_info == NULL) {
    ...
    CFIL_LOG(LOG_ERR, "so %llx drop set", (uint64_t)VM_KERNEL_ADDRPERM(so));
    error = EPIPE;
  }
...
static void cfil_release_sockbuf(struct socket *so, int outgoing) {
  struct sockbuf *sb = outgoing ? &so->so_snd : &so->so_rcv;
  thread_t __single tp = current_thread();
  ...
  if (sb->sb_cfil_thread != NULL && sb->sb_cfil_thread != tp) {
    panic("%s sb_cfil_thread %p not current %p", __func__, sb->sb_cfil_thread,
          tp);
  }
  /*
  ...
  if (!(so->so_flags & SOF_DEFUNCT) && !(sb->sb_flags & SB_LOCK)) {
    panic("%s SB_LOCK not set on %p", __func__, sb);
  }
  /*
...
cfil_sock_id_t cfil_sock_id_from_socket(struct socket *so) {
 ...
 * lock, avoiding potential deadlock with other processes contending for the
 * same resources.  This is also to avoid double locking the pcbinfo for rip
 * sockets since rip_unlock() will lock ripcbinfo if it needs to dispose inpcb
 * when so_usecount is 0.
 */
static bool cfil_socket_safe_lock(struct inpcb *inp,
...
static struct socket *cfil_socket_from_sock_id(cfil_sock_id_t cfil_sock_id,
                                               bool udp_only) {
  struct socket *so = NULL;
...
find_udp:
  ...
  if (so == NULL) {
    ...
    CFIL_LOG(LOG_DEBUG, "no socket for sock_id %llx gencnt %llx flowhash %x",
...
static struct socket *cfil_socket_from_client_uuid(uuid_t necp_client_uuid,
                                                   bool *cfil_attached) {
  struct socket *so = NULL;
  struct inpcb *inp = NULL;
...
static void cfil_info_stats_toggle(struct cfil_info *cfil_info,
                                   struct cfil_entry *entry,
  ...
  if (report_frequency) {
    ...
    if (entry->cfe_stats_report_frequency != report_frequency) {
      ...
      if (entry->cfe_stats_report_frequency <
      ...
      if (cfil_info->cfi_debug && cfil_log_stats) {
        CFIL_LOG(LOG_ERR,
                 "CFIL: VERDICT RECEIVED - STATS FLOW INSERTED: <so %llx "
                 "sockID %llu <%llx>> stats frequency %d msecs",
                 cfil_info->cfi_so
                     ? (uint64_t)VM_KERNEL_ADDRPERM(cfil_info->cfi_so)
                     : 0,
    ...
    if (entry != NULL) {
      ...
      // If cfil_info still has filter(s) asking for stats, no need to remove
      // from list.
    ...
    if (!TAILQ_EMPTY(&cfil_sock_head_stats)) {
      ...
      if (found) {
        ...
        if (cfil_info->cfi_debug && cfil_log_stats) {
          CFIL_LOG(LOG_ERR,
                   "CFIL: VERDICT RECEIVED - STATS FLOW DELETED: <so %llx "
                   "sockID %llu <%llx>> stats frequency reset",
                   cfil_info->cfi_so
                       ? (uint64_t)VM_KERNEL_ADDRPERM(cfil_info->cfi_so)
                       : 0,
                   cfil_info->cfi_sock_id, cfil_info->cfi_sock_id);
...
static errno_t cfil_ctl_send(kern_ctl_ref kctlref, u_int32_t kcunit,
...
#pragma unused(kctlref, flags)
  ...
  case CFM_OP_DATA_UPDATE:
    OSIncrementAtomic(&cfil_stats.cfs_ctl_action_data_update);
    break;
  case CFM_OP_DROP:
  ...
  case CFM_OP_BLESS_CLIENT:
    if (msghdr->cfm_len != sizeof(struct cfil_msg_bless_client)) {
      ...
      error = EINVAL;
      CFIL_LOG(LOG_ERR, "bad len: %u for op %u", msghdr->cfm_len,
               msghdr->cfm_op);
      goto done;
    }
  ...
  if (msghdr->cfm_len != sizeof(struct cfil_msg_action)) {
    OSIncrementAtomic(&cfil_stats.cfs_ctl_action_bad_len);
    error = EINVAL;
    CFIL_LOG(LOG_ERR, "bad len: %u for op %u", msghdr->cfm_len, msghdr->cfm_op);
    goto done;
  }
  cfil_rw_lock_shared(&cfil_lck_rw);
  if (cfc != (void *)content_filters[kcunit - 1]) {
    CFIL_LOG(LOG_ERR, "unitinfo does not match for kcunit %u", kcunit);
    error = EINVAL;
  ...
  if (so == NULL) {
    CFIL_LOG(LOG_NOTICE, "bad sock_id %llx", msghdr->cfm_sock_id);
    error = EINVAL;
    goto done;
  }
  ...
  cfil_info =
      so->so_flow_db != NULL
          ? soflow_db_get_feature_context(so->so_flow_db, msghdr->cfm_sock_id)
          : so->so_cfil;

  // We should not obtain global lock here in order to avoid deadlock down the
  // path. But we attempt to retain a valid cfil_info to prevent any
  // deallocation until we are done.  Abort retain if cfil_info has already
  // entered the free code path.
  ...
  if (cfil_info == NULL) {
    ...
    CFIL_LOG(LOG_NOTICE, "so %llx drop set", (uint64_t)VM_KERNEL_ADDRPERM(so));
    error = EINVAL;
    goto unlock;
  }
  ...
  if (entry->cfe_filter == NULL) {
    CFIL_LOG(LOG_NOTICE, "so %llx no filter", (uint64_t)VM_KERNEL_ADDRPERM(so));
    error = EINVAL;
    goto unlock;
  }
  ...
  if (entry->cfe_flags & CFEF_SENT_SOCK_ATTACHED) {
    ...
    CFIL_LOG(LOG_ERR, "so %llx attached not sent for %u",
             (uint64_t)VM_KERNEL_ADDRPERM(so), kcunit);
    error = EINVAL;
    goto unlock;
  }
  ...
  CFI_ADD_TIME_LOG(cfil_info, &entry->cfe_last_action,
                   &cfil_info->cfi_first_event, msghdr->cfm_op);
  ...
  case CFM_OP_DATA_UPDATE:
    ...
    if (cfil_info->cfi_debug) {
      ...
      CFIL_LOG(LOG_ERR,
               "CFIL: VERDICT RECEIVED: <so %llx sockID %llu <%llx>> <IN "
               "peek:%llu pass:%llu, OUT peek:%llu pass:%llu>",
               (uint64_t)VM_KERNEL_ADDRPERM(so), cfil_info->cfi_sock_id,
               cfil_info->cfi_sock_id, action_msg->cfa_in_peek_offset,
               action_msg->cfa_in_pass_offset, action_msg->cfa_out_peek_offset,
  ...
  case CFM_OP_DROP:
    ...
    CFIL_LOG(LOG_ERR,
             "CFIL: VERDICT DROP RECEIVED: <so %llx sockID %llu <%llx>> <IN "
             "peek:%llu pass:%llu, OUT peek:%llu pass:%llu>",
             (uint64_t)VM_KERNEL_ADDRPERM(so), cfil_info->cfi_sock_id,
             cfil_info->cfi_sock_id, action_msg->cfa_in_peek_offset,
             action_msg->cfa_in_pass_offset, action_msg->cfa_out_peek_offset,
...
static errno_t cfil_ctl_getopt(kern_ctl_ref kctlref, u_int32_t kcunit,
...
#pragma unused(kctlref, opt)
  ...
  case CFIL_OPT_PRESERVE_CONNECTIONS:
    if (*len < sizeof(uint32_t)) {
      CFIL_LOG(LOG_ERR, "CFIL_OPT_PRESERVE_CONNECTIONS len too small %lu",
               *len);
      error = EINVAL;
      goto done;
    }
    if (data != NULL) {
      *(uint32_t *)data =
          (cfc->cf_flags & CFF_PRESERVE_CONNECTIONS) ? true : false;
    }
    break;
  case CFIL_OPT_GET_SOCKET_INFO:
    ...
    struct cfil_opt_sock_info *sock_info = (struct cfil_opt_sock_info *)data;
    ...
    cfil_info = (sock->so_flow_db != NULL)
                    ? soflow_db_get_feature_context(sock->so_flow_db,
                                                    sock_info->cfs_sock_id)
                    : sock->so_cfil;

    if (cfil_info == NULL) {
      CFIL_LOG(LOG_INFO,
               "CFIL: GET_SOCKET_INFO failed: so %llx not attached, cannot "
               "fetch info",
    ...
    if (sock->so_proto == NULL || sock->so_proto->pr_domain == NULL) {
      CFIL_LOG(
          LOG_INFO,
          "CFIL: GET_SOCKET_INFO failed: so %llx NULL so_proto / pr_domain",
          (uint64_t)VM_KERNEL_ADDRPERM(sock));
      error = EINVAL;
      socket_unlock(sock, 1);
      goto return_already_unlocked;
    ...
    if (inp->inp_vflag & INP_IPV6) {
      ...
      cfil_get_flow_address_v6(cfil_info->cfi_hash_entry, inp, &laddr, &faddr,
                               &lport, &fport);
      ...
      cfil_get_flow_address(cfil_info->cfi_hash_entry, inp, &laddr, &faddr,
                            &lport, &fport);
...
static errno_t cfil_ctl_setopt(kern_ctl_ref kctlref, u_int32_t kcunit,
                               void *unitinfo, int opt, void *data,
                               size_t len) {
#pragma unused(kctlref, opt)
  errno_t error = 0;
  ...
  case CFIL_OPT_PRESERVE_CONNECTIONS:
    ...
    CFIL_LOG(LOG_INFO, "CFIL_OPT_PRESERVE_CONNECTIONS got %d (kcunit %d)",
             preserve_connections, kcunit);
...
static void cfil_ctl_rcvd(kern_ctl_ref kctlref, u_int32_t kcunit,
...
static struct m_tag *m_tag_kalloc_cfil_udp(u_int32_t id, u_int16_t type,
...
static void m_tag_kfree_cfil_udp(struct m_tag *tag) {
  struct cflil_tag_container *__single tag_container =
      (struct cflil_tag_container *)tag;

  kfree_type(struct cflil_tag_container, tag_container);
}

void cfil_register_m_tag(void) {
  errno_t error = 0;

  error = m_register_internal_tag_type(
      KERNEL_TAG_TYPE_CFIL_UDP, sizeof(struct cfil_tag), m_tag_kalloc_cfil_udp,
...
void cfil_init(void) {
   ...
   */
  VERIFY(IS_P2ALIGNED(&cfil_stats.cfs_ctl_q_in_enqueued, sizeof(uint32_t)));
  ...
  VERIFY(
      IS_P2ALIGNED(&cfil_stats.cfs_pending_q_out_enqueued, sizeof(uint32_t)));
  ...
  VERIFY(IS_P2ALIGNED(&cfil_stats.cfs_inject_q_in_passed, sizeof(uint32_t)));
  VERIFY(IS_P2ALIGNED(&cfil_stats.cfs_inject_q_out_passed, sizeof(uint32_t)));
  ...
  mbuf_limit = MAX(UDP_FLOW_GC_MBUF_CNT_MAX,
                   (nmbclusters << MCLSHIFT) >> UDP_FLOW_GC_MBUF_SHIFT);
  cfil_udp_gc_mbuf_num_max = (mbuf_limit >> MCLSHIFT);
  cfil_udp_gc_mbuf_cnt_max = mbuf_limit;

  memset(&global_cfil_stats_report_buffers, 0,
         sizeof(global_cfil_stats_report_buffers));
}

struct cfil_info *cfil_info_alloc(struct socket *so,
...
int cfil_info_attach_unit(struct socket *so, uint32_t filter_control_unit,
...
static void cfil_info_free(struct cfil_info *cfil_info) {
  int kcunit;
  uint64_t in_drain = 0;
 ...
 */
boolean_t cfil_sock_connected_pending_verdict(struct socket *so) {
  if (so == NULL || so->so_cfil == NULL) {
    return false;
  }
 ...
 */
boolean_t cfil_sock_is_dead(struct socket *so) {
  struct inpcb *inp = NULL;
 ...
 * The socket is locked.
 *
 * Perform TCP FIN time wait handling if all CFIL data has been processed by
 * filter(s). Otherwise, delay until all data has been processed.
 */
boolean_t cfil_sock_tcp_add_time_wait(struct socket *so) {
  struct inpcb *inp = NULL;
...
boolean_t cfil_filter_present(void) { return cfil_active_count > 0; }

/*
 * Entry point from Sockets layer
 * The socket is locked.
 */
errno_t cfil_sock_attach(struct socket *so, struct sockaddr *local,
                         struct sockaddr *remote, int dir) {
  errno_t error = 0;
  ...
  if (debug) {
    CFIL_LOG(LOG_ERR, "CFIL: TCP (dir %d) - debug flow with port %d", dir,
             cfil_log_port);
  }
  ...
  if (so->so_cfil != NULL) {
    ...
    so->so_cfil->cfi_filter_policy_gencount =
        necp_socket_get_policy_gencount(so);
    so->so_cfil->cfi_debug = debug;
  }
  if (cfil_info_attach_unit(so, filter_control_unit, so->so_cfil) == 0) {
    CFIL_LOG(LOG_ERR, "cfil_info_attach_unit(%u) failed", filter_control_unit);
  ...
  CFIL_LOG(LOG_INFO, "so %llx filter_control_unit %u sockID %llu <%llx>",
           ...
           so->so_cfil->cfi_sock_id, so->so_cfil->cfi_sock_id);
  ...
  if (remote != NULL && (remote->sa_len <= sizeof(union sockaddr_in_4_6))) {
    SOCKADDR_COPY(remote, SA(&so->so_cfil->cfi_so_attach_faddr),
                  remote->sa_len);
  }
 ...
 * The socket is locked.
 */
errno_t cfil_sock_detach(struct socket *so) {
 ...
 */
static void cfil_fill_event_msg_addresses(struct soflow_hash_entry *entry,
                                          struct inpcb *inp,
                                          union sockaddr_in_4_6 *sin_src,
                                          union sockaddr_in_4_6 *sin_dst,
                                          boolean_t outgoing) {
  if ((entry != NULL && entry->soflow_family == AF_INET) || !IS_INP_V6(inp)) {
...
cfil_dispatch_attach_event_sign(cfil_crypto_state_t crypto_state,
  ...
  size_t len = strbuflen(msg->cfs_remote_domain_name,
                         sizeof(msg->cfs_remote_domain_name));
  if (len > 0) {
    extra_data[0].iov_base = msg->cfs_remote_domain_name;
    extra_data[0].iov_len = len;
  }

  // At attach, if local address is already present, no need to re-sign
  ...
  if (cfil_crypto_sign_data(crypto_state, &data, extra_data,
                            sizeof(extra_data) / sizeof(extra_data[0]),
                            msg->cfs_signature,
...
static void cfil_sign_with_domain_name(char *domain_name __null_terminated,
                                       void *ctx) {
  ...
  *(parameters->csp_signature_size) = sizeof(cfil_crypto_signature);
  if (cfil_crypto_sign_data(
          parameters->csp_state, parameters->csp_data, extra_data,
          sizeof(extra_data) / sizeof(extra_data[0]), parameters->csp_signature,
...
cfil_dispatch_data_event_sign(cfil_crypto_state_t crypto_state,
  ...
  if (crypto_state == NULL || msg == NULL || so == NULL || cfil_info == NULL) {
    return false;
  }
  ...
  // At first data, local address may show up for the first time, update address
  // cache and no need to re-sign subsequent data messages anymore.
  if (!NULLADDRESS(data.local)) {
...
cfil_dispatch_closed_event_sign(cfil_crypto_state_t crypto_state,
  ...
  if (crypto_state == NULL || msg == NULL || so == NULL || inp == NULL ||
      cfil_info == NULL) {
    return false;
  }
  ...
  if (cfil_info->cfi_hash_entry != NULL) {
    ...
    soflow_fill_hash_entry_from_address(
        &hash_entry, TRUE, SA(&cfil_info->cfi_so_attach_laddr), FALSE);
    soflow_fill_hash_entry_from_address(
        &hash_entry, FALSE, SA(&cfil_info->cfi_so_attach_faddr), FALSE);
  ...
  struct cfil_sign_parameters parameters = {.csp_state = crypto_state,
                                            .csp_data = &data,
                                            .csp_signature = msg->cfc_signature,
                                            .csp_signature_size =
                                                &msg->cfc_signature_length};
  necp_with_inp_domain_name(so, &parameters, cfil_sign_with_domain_name);
...
static void
cfil_populate_attached_msg_domain_name(char *domain_name __null_terminated,
                                       void *ctx) {
  struct cfil_msg_sock_attached *msg_attached =
      (struct cfil_msg_sock_attached *)ctx;
  ...
  if (domain_name != NULL) {
    strlcpy(msg_attached->cfs_remote_domain_name, domain_name,
            sizeof(msg_attached->cfs_remote_domain_name));
  }
}

static bool cfil_copy_audit_token(pid_t pid, audit_token_t *buffer) {
  bool success = false;
  ...
  if (p != PROC_NULL) {
    ...
    if (t != TASK_NULL) {
      ...
      if (task_info(t, TASK_AUDIT_TOKEN, (task_info_t)&audit_token, &count) ==
          KERN_SUCCESS) {
...
static int cfil_dispatch_attach_event(struct socket *so,
                                      struct cfil_info *cfil_info,
  ...
  CFIL_LOG(LOG_INFO, "so %llx filter_control_unit %u kcunit %u",
           (uint64_t)VM_KERNEL_ADDRPERM(so), entry->cfe_necp_control_unit,
           kcunit);
  ...
  if (hash_entry_ptr != NULL) {
    cfil_fill_event_msg_addresses(hash_entry_ptr, inp, &msg_attached->cfs_src,
                                  &msg_attached->cfs_dst,
                                  conn_dir == CFS_CONNECTION_DIR_OUT);
  }
  msg_attached->cfs_conn_dir = conn_dir;

  if (msg_attached->cfs_e_pid != 0) {
    if (!cfil_copy_audit_token(
            msg_attached->cfs_e_pid,
            (audit_token_t *)&msg_attached->cfs_audit_token)) {
      CFIL_LOG(
          LOG_ERR,
          "CFIL: Failed to get effective audit token for <sockID %llu <%llx>> ",
  ...
  if (msg_attached->cfs_pid != 0) {
    if (msg_attached->cfs_pid == msg_attached->cfs_e_pid) {
      memcpy(&msg_attached->cfs_real_audit_token,
             &msg_attached->cfs_audit_token,
             sizeof(msg_attached->cfs_real_audit_token));
    } else if (!cfil_copy_audit_token(
                   msg_attached->cfs_pid,
                   (audit_token_t *)&msg_attached->cfs_real_audit_token)) {
      CFIL_LOG(LOG_ERR,
               "CFIL: Failed to get real audit token for <sockID %llu <%llx>> ",
               entry->cfe_cfil_info->cfi_sock_id,
               entry->cfe_cfil_info->cfi_sock_id);
    }
  }

  necp_with_inp_domain_name(so, msg_attached,
                            cfil_populate_attached_msg_domain_name);

  if (cfil_info->cfi_debug) {
    cfil_info_log(LOG_ERR, cfil_info, "CFIL: SENDING ATTACH UP");
  }

  cfil_dispatch_attach_event_sign(entry->cfe_filter->cf_crypto_state, cfil_info,
                                  msg_attached);

  error = ctl_enqueuedata(entry->cfe_filter->cf_kcref,
                          entry->cfe_filter->cf_kcunit, msg_attached,
                          sizeof(struct cfil_msg_sock_attached), CTL_DATA_EOR);
...
static int cfil_dispatch_disconnect_event(struct socket *so,
                                          struct cfil_info *cfil_info,
                                          uint32_t kcunit, int outgoing) {
  errno_t error = 0;
  ...
  msg_disconnected.cfm_op =
      outgoing ? CFM_OP_DISCONNECT_OUT : CFM_OP_DISCONNECT_IN;
  msg_disconnected.cfm_sock_id = entry->cfe_cfil_info->cfi_sock_id;
  error = ctl_enqueuedata(entry->cfe_filter->cf_kcref,
                          entry->cfe_filter->cf_kcunit, &msg_disconnected,
                          sizeof(struct cfil_msg_hdr), CTL_DATA_EOR);
  if (error != 0) {
  ...
  microuptime(&entry->cfe_last_event);
  CFI_ADD_TIME_LOG(cfil_info, &entry->cfe_last_event,
                   &cfil_info->cfi_first_event, msg_disconnected.cfm_op);
  ...
  if (error == ENOBUFS) {
    entry->cfe_flags |= CFEF_FLOW_CONTROLLED;
    OSIncrementAtomic(&cfil_stats.cfs_disconnect_event_flow_control);
    ...
    if (error != 0) {
      OSIncrementAtomic(&cfil_stats.cfs_disconnect_event_fail);
    }
...
int cfil_dispatch_closed_event(struct socket *so, struct cfil_info *cfil_info,
                               int kcunit) {
  struct cfil_entry *entry;
  ...
  CFIL_LOG(LOG_INFO, "so %llx kcunit %d", (uint64_t)VM_KERNEL_ADDRPERM(so),
           kcunit);
  ...
  microuptime(&entry->cfe_last_event);
  CFI_ADD_TIME_LOG(cfil_info, &entry->cfe_last_event,
                   &cfil_info->cfi_first_event, CFM_OP_SOCKET_CLOSED);
  ...
  memcpy(msg_closed.cfc_op_time, cfil_info->cfi_op_time,
         sizeof(uint32_t) * CFI_MAX_TIME_LOG_ENTRY);
  memcpy(msg_closed.cfc_op_list, cfil_info->cfi_op_list,
    ...
    if (cfil_info->cfi_so_attach_laddr.sa.sa_len == 0) {
      ...
      if (inp != NULL) {
        ...
        union sockaddr_in_4_6 *src =
            outgoing ? &cfil_info->cfi_so_attach_laddr : NULL;
        union sockaddr_in_4_6 *dst =
            outgoing ? NULL : &cfil_info->cfi_so_attach_laddr;
        cfil_fill_event_msg_addresses(cfil_info->cfi_hash_entry, inp, src, dst,
                                      outgoing);
      }
    }
  ...
  cfil_dispatch_closed_event_sign(entry->cfe_filter->cf_crypto_state, so,
                                  cfil_info, &msg_closed);
   ...
   *       msg_closed.cfc_op_list_ctr  = CFI_MAX_TIME_LOG_ENTRY;       // just
   * in case
   *  }
   *  for (unsigned int i = 0; i < msg_closed.cfc_op_list_ctr ; i++) {
   *       CFIL_LOG(LOG_ERR, "MD: socket %llu event %2u, time + %u msec",
   * msg_closed.cfc_msghdr.cfm_sock_id, (unsigned
   * short)msg_closed.cfc_op_list[i], msg_closed.cfc_op_time[i]);
   *  }
   */

  error = ctl_enqueuedata(entry->cfe_filter->cf_kcref,
                          entry->cfe_filter->cf_kcunit, &msg_closed,
                          sizeof(struct cfil_msg_sock_closed), CTL_DATA_EOR);
  if (error != 0) {
    CFIL_LOG(LOG_ERR, "ctl_enqueuedata() failed: %d", error);
    goto done;
  }
...
static void fill_ip6_sockaddr_4_6(union sockaddr_in_4_6 *sin46,
...
static void fill_ip_sockaddr_4_6(union sockaddr_in_4_6 *sin46,
...
static void cfil_get_flow_address_v6(struct soflow_hash_entry *entry,
                                     struct inpcb *inp, struct in6_addr **laddr,
                                     struct in6_addr **faddr, u_int16_t *lport,
                                     u_int16_t *fport) {
  if (entry != NULL) {
...
static void cfil_get_flow_address(struct soflow_hash_entry *entry,
                                  struct inpcb *inp, struct in_addr *laddr,
                                  struct in_addr *faddr, u_int16_t *lport,
                                  u_int16_t *fport) {
  if (entry != NULL) {
...
static int cfil_dispatch_data_event(struct socket *so,
                                    struct cfil_info *cfil_info,
                                    uint32_t kcunit, int outgoing,
                                    struct mbuf *data, unsigned int copyoffset,
  ...
  data_req->cfd_msghdr.cfm_op = outgoing ? CFM_OP_DATA_OUT : CFM_OP_DATA_IN;
  data_req->cfd_msghdr.cfm_sock_id = entry->cfe_cfil_info->cfi_sock_id;
  ...
  if (data_req->cfd_delegated_pid != 0) {
    if (!cfil_copy_audit_token(
            data_req->cfd_delegated_pid,
            (audit_token_t *)&data_req->cfd_delegated_audit_token)) {
      CFIL_LOG(
          LOG_ERR, "CFIL: Failed to get audit token for <sockID %llu <%llx>> ",
          entry->cfe_cfil_info->cfi_sock_id, entry->cfe_cfil_info->cfi_sock_id);
    }
  }
  ...
  if (OPTIONAL_IP_HEADER(so)) {
    ...
    if ((!outgoing && !IS_INP_V6(inp)) ||
        (outgoing && cfil_dgram_peek_socket_state(data, &inp_flags) &&
         (inp_flags & INP_HDRINCL))) {
  ...
  if (cfil_info->cfi_isSignatureLatest == false) {
    cfil_dispatch_data_event_sign(entry->cfe_filter->cf_crypto_state, so,
                                  cfil_info, data_req);
  }

  microuptime(&tv);
  CFI_ADD_TIME_LOG(cfil_info, &tv, &cfil_info->cfi_first_event,
                   data_req->cfd_msghdr.cfm_op);

  /* Pass the message to the content filter */
  error = ctl_enqueuembuf(entry->cfe_filter->cf_kcref,
                          entry->cfe_filter->cf_kcunit, msg, CTL_DATA_EOR);
  if (error != 0) {
    CFIL_LOG(LOG_ERR, "ctl_enqueuembuf() failed: %d", error);
    mbuf_freem(msg);
    goto done;
  ...
  if (cfil_info->cfi_debug && cfil_log_data) {
    CFIL_LOG(LOG_ERR,
             "CFIL: VERDICT ACTION: so %llx sockID %llu <%llx> outgoing %d: "
             "mbuf %llx copyoffset %u copylen %u (%s)",
             (uint64_t)VM_KERNEL_ADDRPERM(so), cfil_info->cfi_sock_id,
             cfil_info->cfi_sock_id, outgoing,
             (uint64_t)VM_KERNEL_ADDRPERM(data), copyoffset, copylen,
             data_req->cfd_flags & CFD_DATA_FLAG_IP_HEADER ? "IP HDR"
                                                           : "NO IP HDR");
  }
  ...
  if (error == ENOBUFS) {
    entry->cfe_flags |= CFEF_FLOW_CONTROLLED;
    OSIncrementAtomic(&cfil_stats.cfs_data_event_flow_control);
 ...
 */
static int cfil_data_service_ctl_q(struct socket *so,
                                   struct cfil_info *cfil_info, uint32_t kcunit,
                                   int outgoing) {
  errno_t error = 0;
  ...
  if ((entry->cfe_flags & CFEF_SENT_SOCK_ATTACHED) == 0) {
    error = cfil_dispatch_attach_event(
        so, cfil_info, CFI_ENTRY_KCUNIT(cfil_info, entry), cfil_info->cfi_dir);
    if (error != 0) {
      /* We can recover from flow control */
      if (error == ENOBUFS || error == ENOMEM) {
        error = 0;
  ...
  if (cfil_info->cfi_debug && cfil_log_data) {
    CFIL_LOG(
        ...
        entrybuf->cfe_pass_offset, entrybuf->cfe_peeked,
  ...
  while ((data = cfil_queue_first(&entrybuf->cfe_ctl_q)) != NULL &&
    ...
    if (entrybuf->cfe_ctl_q.q_start + datalen <= entrybuf->cfe_pass_offset) {
      /*
       ...
       */
      copylen = (unsigned int)(entrybuf->cfe_pass_offset -
    ...
    if (cfil_info->cfi_debug && cfil_log_data) {
      CFIL_LOG(LOG_ERR,
               "CFIL: SERVICE CTL-Q PASSING: %llx first %llu peeked %llu pass "
               "%llu peek %llu"
               "datalen %u copylen %u",
               (uint64_t)VM_KERNEL_ADDRPERM(tmp), entrybuf->cfe_ctl_q.q_start,
               entrybuf->cfe_peeked, entrybuf->cfe_pass_offset,
               entrybuf->cfe_peek_offset, datalen, copylen);
    }
     ...
     * implicitly
     */
    if (entrybuf->cfe_ctl_q.q_start + copylen > entrybuf->cfe_peeked) {
      entrybuf->cfe_peeked = entrybuf->cfe_ctl_q.q_start + copylen;
    }
    /*
    ...
    if (outgoing) {
      OSAddAtomic64(datalen, &cfil_stats.cfs_pending_q_out_enqueued);
    } else {
      OSAddAtomic64(datalen, &cfil_stats.cfs_pending_q_in_enqueued);
    }
  }
  ...
  for (data = cfil_queue_first(&entrybuf->cfe_ctl_q),
     ...
     */
    if (currentoffset + copyoffset + copylen > entrybuf->cfe_peek_offset) {
    ...
    if (cfil_info->cfi_debug && cfil_log_data) {
      CFIL_LOG(LOG_ERR,
               "CFIL: SERVICE CTL-Q PEEKING: %llx current %llu peeked %llu "
               "pass %llu peek %llu "
               "datalen %u copylen %u copyoffset %u",
               (uint64_t)VM_KERNEL_ADDRPERM(tmp), currentoffset,
               entrybuf->cfe_peeked, entrybuf->cfe_pass_offset,
               entrybuf->cfe_peek_offset, datalen, copylen, copyoffset);
    }
     ...
     */
    error = cfil_dispatch_data_event(so, cfil_info, kcunit, outgoing, data,
                                     copyoffset, copylen);
    if (error != 0) {
    ...
    if (outgoing) {
      OSAddAtomic64(copylen, &cfil_stats.cfs_ctl_q_out_peeked);
    } else {
      OSAddAtomic64(copylen, &cfil_stats.cfs_ctl_q_in_peeked);
    }
  ...
  CFIL_LOG(LOG_DEBUG, "first %llu peeked %llu pass %llu peek %llu",
           entrybuf->cfe_ctl_q.q_start, entrybuf->cfe_peeked,
           entrybuf->cfe_pass_offset, entrybuf->cfe_peek_offset);
 ...
 */
int cfil_data_filter(struct socket *so, struct cfil_info *cfil_info,
                     uint32_t kcunit, int outgoing, struct mbuf *data,
                     uint32_t datalen) {
  errno_t error = 0;
  struct cfil_entry *entry;
  struct cfe_buf *entrybuf;
  ...
  if (outgoing) {
    OSAddAtomic64(datalen, &cfil_stats.cfs_ctl_q_out_enqueued);
  } else {
    OSAddAtomic64(datalen, &cfil_stats.cfs_ctl_q_in_enqueued);
  }
...
static void cfil_strip_ip_header(struct cfil_info *cfil_info, mbuf_t data,
                                 struct socket *so) {
  struct ip *ip = NULL;
  unsigned int hlen = 0;
  ...
  if (inp && (inp->inp_flags & INP_STRIPHDR)) {
    ...
    if (data_start != NULL && (data_start->m_flags & M_PKTHDR)) {
      ...
      if (cfil_info->cfi_debug && cfil_log_data) {
        CFIL_LOG(LOG_ERR,
                 "CFIL: IPHDR STRIPPING: <so %llx>: <hlen %d m_len %d>",
                 (uint64_t)VM_KERNEL_ADDRPERM(so), hlen, data_start->m_len);
      }
 ...
 * content filters
 */
static int cfil_service_inject_queue(struct socket *so,
                                     struct cfil_info *cfil_info,
                                     int outgoing) {
  mbuf_t data;
  ...
  if (cfil_info->cfi_debug && cfil_log_data) {
    CFIL_LOG(
        LOG_ERR, "CFIL: SERVICE INJECT-Q: <so %llx> outgoing %d queue len %llu",
  ...
  while ((data = cfil_queue_first(inject_q)) != NULL) {
    ...
    if (cfil_info->cfi_debug && cfil_log_data) {
      CFIL_LOG(
          ...
          (uint64_t)VM_KERNEL_ADDRPERM(so), (uint64_t)VM_KERNEL_ADDRPERM(data),
          datalen, mbcnt);
    }
    ...
    if (outgoing) {
      OSAddAtomic64(datalen, &cfil_stats.cfs_inject_q_out_passed);
    } else {
  ...
  if (cfil_info && (cfil_info->cfi_flags & CFIF_CLOSE_WAIT)) {
    if (cfil_filters_attached(so) == 0) {
      CFIL_LOG(LOG_INFO, "so %llx waking", (uint64_t)VM_KERNEL_ADDRPERM(so));
      wakeup((caddr_t)cfil_info);
    }
  }

  if (SO_DELAYED_DEAD_GET(so)) {
    // Check to see if all data processed for this socket, if so mark it DEAD
    // now.
  ...
  if (SO_DELAYED_TCP_TIME_WAIT_GET(so)) {
    // Check to see if all data processed for this socket, if so handle the TCP
    // time wait now
...
static int cfil_service_pending_queue(struct socket *so,
                                      struct cfil_info *cfil_info,
                                      uint32_t kcunit, int outgoing) {
  ...
  if (cfil_queue_empty(pending_q)) {
    for (iter_entry = SLIST_NEXT(entry, cfe_order_link); iter_entry != NULL;
         iter_entry = SLIST_NEXT(iter_entry, cfe_order_link)) {
      error = cfil_data_service_ctl_q(
          so, cfil_info, CFI_ENTRY_KCUNIT(cfil_info, iter_entry), outgoing);
  ...
  while ((data = cfil_queue_first(pending_q)) != NULL) {
    ...
    if (cfil_info->cfi_debug && cfil_log_data) {
      CFIL_LOG(LOG_ERR,
               "CFIL: SERVICE PENDING-Q: data %llx datalen %u passlen %llu "
               "curlen %llu",
               (uint64_t)VM_KERNEL_ADDRPERM(data), datalen, passlen, curlen);
    }
    ...
    for (iter_entry = SLIST_NEXT(entry, cfe_order_link); iter_entry != NULL;
         iter_entry = SLIST_NEXT(iter_entry, cfe_order_link)) {
      error = cfil_data_filter(so, cfil_info,
                               CFI_ENTRY_KCUNIT(cfil_info, iter_entry),
                               outgoing, data, datalen);
      /* 0 means passed so we can continue */
      if (error != 0) {
        break;
      }
    ...
    if (error == 0) {
      if (outgoing) {
        cfil_queue_enqueue(&cfil_info->cfi_snd.cfi_inject_q, data, datalen);
        OSAddAtomic64(datalen, &cfil_stats.cfs_inject_q_out_enqueued);
      } else {
        cfil_queue_enqueue(&cfil_info->cfi_rcv.cfi_inject_q, data, datalen);
        OSAddAtomic64(datalen, &cfil_stats.cfs_inject_q_in_enqueued);
      }
    }
  }
...
int cfil_update_data_offsets(struct socket *so, struct cfil_info *cfil_info,
                             uint32_t kcunit, int outgoing,
  ...
  if (pass_offset > entrybuf->cfe_pass_offset) {
    ...
    CFIL_LOG(LOG_INFO, "pass_offset %llu <= cfe_pass_offset %llu", pass_offset,
   ...
   * to be delivered to the filter
   */
  if (entry != NULL && ((entry->cfe_snd.cfe_pass_offset == CFM_MAX_OFFSET &&
    ...
    CFIL_LOG(LOG_INFO, "so %llx detached %u", (uint64_t)VM_KERNEL_ADDRPERM(so),
             kcunit);
    if ((cfil_info->cfi_flags & CFIF_CLOSE_WAIT) &&
        cfil_filters_attached(so) == 0) {
      if (cfil_info->cfi_debug) {
 ...
 */
static int cfil_set_socket_pass_offset(struct socket *so,
                                       struct cfil_info *cfil_info,
                                       int outgoing) {
  struct cfi_buf *cfi_buf;
  struct cfil_entry *entry;
  ...
  if (cfil_info->cfi_debug && cfil_log_data) {
    CFIL_LOG(LOG_ERR, "so %llx outgoing %d", (uint64_t)VM_KERNEL_ADDRPERM(so),
             outgoing);
  }
  ...
  if (cfil_info->cfi_debug && cfil_log_data) {
    CFIL_LOG(LOG_ERR,
             "CFIL: <so %llx, sockID %llu <%llx>> outgoing %d "
             "cfi_pending_first %llu cfi_pending_last %llu",
             (uint64_t)VM_KERNEL_ADDRPERM(so), cfil_info->cfi_sock_id,
             cfil_info->cfi_sock_id, outgoing, cfi_buf->cfi_pending_first,
  ...
  if (cfi_buf->cfi_pending_last - cfi_buf->cfi_pending_first == 0) {
    for (kcunit = 1; kcunit <= MAX_CONTENT_FILTER; kcunit++) {
      ...
      if (first == true || entrybuf->cfe_pass_offset < pass_offset) {
  ...
  if (cfil_info->cfi_debug && cfil_log_data) {
    CFIL_LOG(LOG_ERR,
             "CFIL: <so %llx, sockID %llu <%llx>>, cfi_pass_offset %llu",
             (uint64_t)VM_KERNEL_ADDRPERM(so), cfil_info->cfi_sock_id,
             cfil_info->cfi_sock_id, cfi_buf->cfi_pass_offset);
  }

  return 0;
}

int cfil_action_data_pass(struct socket *so, struct cfil_info *cfil_info,
                          uint32_t kcunit, int outgoing, uint64_t pass_offset,
  ...
  if (error != 0) {
    CFIL_LOG(LOG_INFO, "so %llx %s dropped", (uint64_t)VM_KERNEL_ADDRPERM(so),
             outgoing ? "out" : "in");
    goto release;
  }

  error = cfil_update_data_offsets(so, cfil_info, kcunit, outgoing, pass_offset,
                                   peek_offset);
...
static void cfil_flush_queues(struct socket *so, struct cfil_info *cfil_info) {
  struct cfil_entry *entry;
  int kcunit;
  uint64_t drained;
  ...
  if (cfil_info != NULL) {
    ...
    if (drained) {
      if (cfil_info->cfi_flags & CFIF_DROP) {
        OSIncrementAtomic(&cfil_stats.cfs_flush_out_drop);
      } else {
        OSIncrementAtomic(&cfil_stats.cfs_flush_out_close);
      }
    }
  }
  cfil_release_sockbuf(so, 1);
  ...
  if (cfil_info != NULL) {
    ...
    for (kcunit = 1; kcunit <= MAX_CONTENT_FILTER; kcunit++) {
      ...
      drained += cfil_queue_drain(&entry->cfe_rcv.cfe_ctl_q);
      drained += cfil_queue_drain(&entry->cfe_rcv.cfe_pending_q);
    }
    drained += cfil_queue_drain(&cfil_info->cfi_rcv.cfi_inject_q);

    if (drained) {
      if (cfil_info->cfi_flags & CFIF_DROP) {
        OSIncrementAtomic(&cfil_stats.cfs_flush_in_drop);
      } else {
        OSIncrementAtomic(&cfil_stats.cfs_flush_in_close);
      }
    }
  }
  cfil_release_sockbuf(so, 0);
...
int cfil_action_drop(struct socket *so, struct cfil_info *cfil_info,
                     uint32_t kcunit) {
  errno_t error = 0;
  struct cfil_entry *entry;
  struct proc *p;
  ...
  if (so->so_flow_db == NULL) {
    error = sosetdefunct(p, so,
                         SHUTDOWN_SOCKET_LEVEL_CONTENT_FILTER |
...
int cfil_action_bless_client(uint32_t kcunit, struct cfil_msg_hdr *msghdr) {
  errno_t error = 0;
  struct cfil_info *__single cfil_info = NULL;

  bool cfil_attached = false;
  struct cfil_msg_bless_client *blessmsg =
      (struct cfil_msg_bless_client *)msghdr;

  // Search and lock socket
  struct socket *so =
      cfil_socket_from_client_uuid(blessmsg->cfb_client_uuid, &cfil_attached);
  if (so == NULL) {
    error = ENOENT;
  } else {
...
int cfil_action_set_crypto_key(uint32_t kcunit, struct cfil_msg_hdr *msghdr) {
  struct content_filter *cfc = NULL;
  cfil_crypto_state_t crypto_state = NULL;
  struct cfil_msg_set_crypto_key *keymsg =
      (struct cfil_msg_set_crypto_key *)msghdr;
  ...
  if (crypto_state == NULL) {
    CFIL_LOG(LOG_ERR, "failed to initialize crypto state for unit %u)", kcunit);
    return EINVAL;
  }
...
static int cfil_update_entry_offsets(struct socket *so,
                                     struct cfil_info *cfil_info, int outgoing,
                                     unsigned int datalen) {
  struct cfil_entry *entry;
  struct cfe_buf *entrybuf;
  uint32_t kcunit;
...
int cfil_data_common(struct socket *so, struct cfil_info *cfil_info,
                     int outgoing, struct sockaddr *to, struct mbuf *data,
                     struct mbuf *control, uint32_t flags) {
#pragma unused(to, control, flags)
  ...
  if (cfil_info->cfi_debug && cfil_log_data) {
    CFIL_LOG(LOG_ERR,
             "CFIL: QUEUEING DATA: <so %llx> %s: data %llx len %u flags 0x%x "
             "nextpkt %llx - cfi_pending_last %llu cfi_pending_mbcnt %u   "
             "cfi_pass_offset %llu",
             (uint64_t)VM_KERNEL_ADDRPERM(so), outgoing ? "OUT" : "IN",
             (uint64_t)VM_KERNEL_ADDRPERM(data), datalen, data->m_flags,
             (uint64_t)VM_KERNEL_ADDRPERM(data->m_nextpkt),
             cfi_buf->cfi_pending_last, cfi_buf->cfi_pending_mbcnt,
    ...
    SLIST_FOREACH(iter_entry, &cfil_info->cfi_ordered_entries, cfe_order_link) {
      ...
      if (IS_ENTRY_ATTACHED(cfil_info, kcunit)) {
        if (NEED_DGRAM_FLOW_TRACKING(so) && chain == NULL) {
          /* Datagrams only:
           * Chain addr (incoming only TDB), control (optional) and data into
           * one chain. This full chain will be reinjected into socket after
           * recieving verdict.
           */
        ...
        error =
            cfil_data_filter(so, cfil_info, kcunit, outgoing, data, datalen);
      }
 ...
 */
int cfil_sock_data_out(struct socket *so, struct sockaddr *to,
                       struct mbuf *data, struct mbuf *control, uint32_t flags,
                       struct soflow_hash_entry *flow_entry) {
  int error = 0;
  ...
  if ((so->so_flags & SOF_CONTENT_FILTER) == 0 || so->so_cfil == NULL) {
    ...
    if (!DO_PRESERVE_CONNECTIONS && cfil_active_count > 0 &&
      ...
      if (new_filter_control_unit > 0) {
        cfil_log_printf(so, NULL,
                        "CFIL: outbound TCP data dropped for pre-existing "
                        "un-filtered flow");
        return EPIPE;
      }
    }
  ...
  new_filter_control_unit = necp_socket_get_content_filter_control_unit(so);
  if (new_filter_control_unit > 0 &&
      new_filter_control_unit != so->so_cfil->cfi_filter_control_unit &&
      !SKIP_FILTER_FOR_TCP_SOCKET(so)) {
    if (DO_PRESERVE_CONNECTIONS || (so->so_cfil->cfi_filter_policy_gencount ==
                                    necp_socket_get_policy_gencount(so))) {
      // CFIL state has changed, but preserve the flow intentionally or if this
      // is not a result of NECP policy change
      so->so_cfil->cfi_filter_control_unit = new_filter_control_unit;
    } else {
      cfil_log_printf(
          so, so->so_cfil, "CFIL: outbound TCP data dropped (0x%x -> 0x%x)",
          so->so_cfil->cfi_filter_control_unit, new_filter_control_unit);
      return EPIPE;
    }
  }
  ...
  if (control != NULL) {
    CFIL_LOG(LOG_ERR, "so %llx control", (uint64_t)VM_KERNEL_ADDRPERM(so));
    OSIncrementAtomic(&cfil_stats.cfs_data_out_control);
  }
  if ((flags & MSG_OOB)) {
    CFIL_LOG(LOG_ERR, "so %llx MSG_OOB", (uint64_t)VM_KERNEL_ADDRPERM(so));
  ...
  if (so->so_snd.sb_cfil_thread != NULL) {
    panic("%s sb_cfil_thread %p not NULL", __func__, so->so_snd.sb_cfil_thread);
  }
 ...
 */
int cfil_sock_data_in(struct socket *so, struct sockaddr *from,
                      struct mbuf *data, struct mbuf *control, uint32_t flags,
                      struct soflow_hash_entry *flow_entry) {
  int error = 0;
  int new_filter_control_unit = 0;
  ...
  if (NEED_DGRAM_FLOW_TRACKING(so)) {
    return cfil_sock_udp_handle_data(FALSE, so, NULL, from, data, control,
                                     flags, flow_entry);
  }
  ...
  if ((so->so_flags & SOF_CONTENT_FILTER) == 0 || so->so_cfil == NULL) {
    ...
    if (!DO_PRESERVE_CONNECTIONS && cfil_active_count > 0 &&
      ...
      if (new_filter_control_unit > 0) {
        cfil_log_printf(
            so, NULL,
            "CFIL: inbound TCP data dropped for pre-existing un-filtered flow");
        return EPIPE;
      }
    }
    return 0;
  ...
  if (new_filter_control_unit > 0 &&
    ...
    if (DO_PRESERVE_CONNECTIONS || (so->so_cfil->cfi_filter_policy_gencount ==
      ...
      cfil_log_printf(
          so, so->so_cfil, "CFIL: inbound TCP data dropped (0x%x -> 0x%x)",
          so->so_cfil->cfi_filter_control_unit, new_filter_control_unit);
      return EPIPE;
    }
  }
 ...
 */
int cfil_sock_shutdown(struct socket *so, int *how) {
  int error = 0;
  ...
  CFIL_LOG(LOG_INFO, "so %llx how %d", (uint64_t)VM_KERNEL_ADDRPERM(so), *how);
 ...
 */
void cfil_sock_is_closed(struct socket *so) {
  errno_t error = 0;
  int kcunit;
 ...
 */
void cfil_sock_notify_shutdown(struct socket *so, int how) {
  errno_t error = 0;
  int kcunit;
...
static int cfil_filters_attached(struct socket *so) {
  struct cfil_entry *entry;
  uint32_t kcunit;
  int attached = 0;
 ...
 */
void cfil_sock_close_wait(struct socket *so) {
  lck_mtx_t *mutex_held;
  struct timespec ts;
  int error;
  ...
  while (cfil_filters_attached(so)) {
    ...
    CFIL_LOG(LOG_INFO, "so %llx waiting", (uint64_t)VM_KERNEL_ADDRPERM(so));
    ...
    error = msleep((caddr_t)so->so_cfil, mutex_held, PSOCK | PCATCH,
 ...
 */
int32_t cfil_sock_data_pending(struct sockbuf *sb) {
 ...
 */
int32_t cfil_sock_data_space(struct sockbuf *sb) {
  struct socket *so = sb->sb_so;
  uint64_t pending = 0;
 ...
 */
void cfil_sock_buf_update(struct sockbuf *sb) {
  int outgoing;
  int error;
  struct socket *so = sb->sb_so;
  ...
  CFIL_LOG(LOG_NOTICE, "so %llx outgoing %d", (uint64_t)VM_KERNEL_ADDRPERM(so),
           outgoing);
...
int sysctl_cfil_filter_list(struct sysctl_oid *oidp, void *arg1, int arg2,
...
#pragma unused(oidp, arg1, arg2)
  ...
  for (i = 0; i < MAX_CONTENT_FILTER; i++) {
    ...
    error = SYSCTL_OUT(req, &filter_stat, sizeof(struct cfil_filter_stat));
    if (error != 0) {
      break;
    }
  }
...
#pragma unused(oidp, arg1, arg2)
   ...
   */
  if (req->oldptr == USER_ADDR_NULL) {
    req->oldidx = cfil_sock_attached_count * sizeof(struct cfil_sock_stat);
  ...
  TAILQ_FOREACH(cfi, &cfil_sock_head, cfi_link) {
    ...
    if (so != NULL && so->so_proto != NULL && so->so_proto->pr_domain != NULL) {
      ...
      memcpy(stat.cfs_uuid, so->last_uuid, sizeof(uuid_t));
      if (so->so_flags & SOF_DELEGATED) {
        stat.cfs_e_pid = so->e_pid;
        memcpy(stat.cfs_e_uuid, so->e_uuid, sizeof(uuid_t));
      } else {
        stat.cfs_e_pid = so->last_pid;
        memcpy(stat.cfs_e_uuid, so->last_uuid, sizeof(uuid_t));
      }
    ...
    stat.cfs_snd.cbs_pending_first = cfi->cfi_snd.cfi_pending_first;
    stat.cfs_snd.cbs_pending_last = cfi->cfi_snd.cfi_pending_last;
    stat.cfs_snd.cbs_inject_q_len = cfil_queue_len(&cfi->cfi_snd.cfi_inject_q);
    stat.cfs_snd.cbs_pass_offset = cfi->cfi_snd.cfi_pass_offset;

    stat.cfs_rcv.cbs_pending_first = cfi->cfi_rcv.cfi_pending_first;
    stat.cfs_rcv.cbs_pending_last = cfi->cfi_rcv.cfi_pending_last;
    stat.cfs_rcv.cbs_inject_q_len = cfil_queue_len(&cfi->cfi_rcv.cfi_inject_q);
    stat.cfs_rcv.cbs_pass_offset = cfi->cfi_rcv.cfi_pass_offset;
    ...
    for (i = 0; i < MAX_CONTENT_FILTER; i++) {
      ...
      estat->ces_filter_id =
          entry->cfe_filter ? entry->cfe_filter->cf_kcunit : 0;
      estat->ces_flags = entry->cfe_flags;
      estat->ces_necp_control_unit = entry->cfe_necp_control_unit;

      estat->ces_last_event.tv_sec = (int64_t)entry->cfe_last_event.tv_sec;
      estat->ces_last_event.tv_usec = (int64_t)entry->cfe_last_event.tv_usec;

      estat->ces_last_action.tv_sec = (int64_t)entry->cfe_last_action.tv_sec;
      estat->ces_last_action.tv_usec = (int64_t)entry->cfe_last_action.tv_usec;

      ebuf = &entry->cfe_snd;
      sbuf = &estat->ces_snd;
      sbuf->cbs_pending_first = cfil_queue_offset_first(&ebuf->cfe_pending_q);
      sbuf->cbs_pending_last = cfil_queue_offset_last(&ebuf->cfe_pending_q);
      sbuf->cbs_ctl_first = cfil_queue_offset_first(&ebuf->cfe_ctl_q);
      sbuf->cbs_ctl_last = cfil_queue_offset_last(&ebuf->cfe_ctl_q);
    ...
    error = SYSCTL_OUT(req, &stat, sizeof(struct cfil_sock_stat));
    if (error != 0) {
      break;
    }
  }
 ...
 * UDP Socket Support
 */
static void cfil_hash_entry_log(int level, struct socket *so,
                                struct soflow_hash_entry *entry,
                                uint64_t sockId, const char *msg) {
  ...
  CFIL_LOG(level,
           "<%s>: [%d %s] <%s(%d) %s so %llx %llu %llu age %ld> lport %d fport "
           "%d laddr %s faddr %s hash %X",
           msg, sock_pid, (sock_proc != NULL ? proc_best_name(sock_proc) : ""),
           IS_UDP(so) ? "UDP" : "proto", GET_SO_PROTO(so),
           (entry->soflow_outgoing ? "out" : "in"),
           (uint64_t)VM_KERNEL_ADDRPERM(so), sockId, entry->soflow_feat_ctxt_id,
           diff_time.tv_sec, ntohs(entry->soflow_lport),
           ntohs(entry->soflow_fport), local, remote, entry->soflow_flowhash);
...
static void cfil_inp_log(int level, struct socket *so, const char *msg) {
  struct inpcb *inp = NULL;
  ...
  if (so->so_cfil != NULL) {
    ...
    CFIL_LOG(level,
             "<%s>: [%d %s] <%s %s so %llx flags 0x%x 0x%x %llu age %ld> lport "
             "%d fport %d laddr %s faddr %s",
             msg, sock_pid,
             (sock_proc != NULL ? proc_best_name(sock_proc) : ""),
             IS_UDP(so) ? "UDP" : "TCP",
             (so->so_cfil->cfi_dir == CFS_CONNECTION_DIR_IN ? "in" : "out"),
             (uint64_t)VM_KERNEL_ADDRPERM(so), inp->inp_flags,
             inp->inp_socket->so_flags, so->so_cfil->cfi_sock_id,
             diff_time.tv_sec, ntohs(lport), ntohs(fport), local, remote);
  } else {
    CFIL_LOG(
        level,
        "<%s>: [%d %s] <%s %s so %llx - flags 0x%x 0x%x> lport %d fport %d "
        "laddr %s faddr %s",
        msg, sock_pid, (sock_proc != NULL ? proc_best_name(sock_proc) : ""),
        IS_UDP(so) ? "UDP" : "TCP",
        (so->so_flags1 & SOF1_INBOUND) ? "in" : "out",
        (uint64_t)VM_KERNEL_ADDRPERM(so), inp->inp_flags,
        inp->inp_socket->so_flags, ntohs(lport), ntohs(fport), local, remote);
  }
...
static void cfil_info_log(int level, struct cfil_info *cfil_info,
                          const char *msg) {
  if (cfil_info == NULL) {
    return;
  }
  ...
  if (cfil_info->cfi_hash_entry != NULL) {
    cfil_hash_entry_log(level, cfil_info->cfi_so, cfil_info->cfi_hash_entry,
                        cfil_info->cfi_sock_id, msg);
  } else {
    cfil_inp_log(level, cfil_info->cfi_so, msg);
  }
}

void cfil_log_printf(struct socket *so, struct cfil_info *info,
                     const char *format, ...) {
  va_list args;
  char buffer[256] = {};
  ...
  if (info != NULL) {
    cfil_info_log(LOG_ERR, info,
                  __unsafe_null_terminated_from_indexable(buffer));
  } else if (so != NULL) {
    cfil_inp_log(LOG_ERR, so, __unsafe_null_terminated_from_indexable(buffer));
  }
}

static void cfil_sock_udp_unlink_flow(struct socket *so,
                                      struct soflow_hash_entry *hash_entry,
                                      struct cfil_info *cfil_info) {
  ...
  if (cfil_info->cfi_debug) {
    CFIL_LOG(LOG_ERR, "CFIL <%s>: <so %llx> - use count %d",
             IS_UDP(so) ? "UDP" : "TCP", (uint64_t)VM_KERNEL_ADDRPERM(so),
             so->so_usecount);
  }
...
bool check_port(struct sockaddr *addr, u_short port) {
  struct sockaddr_in *sin = NULL;
  struct sockaddr_in6 *sin6 = NULL;
...
cfil_sock_id_t cfil_sock_id_from_datagram_socket(struct socket *so,
  ...
  return (cfil_sock_id_t)soflow_db_get_feature_context_id(so->so_flow_db, local,
                                                          remote);
}

static struct cfil_info *
cfil_sock_udp_get_info(struct socket *so, uint32_t filter_control_unit,
                       bool outgoing, struct soflow_hash_entry *hash_entry,
  ...
  if (hash_entry->soflow_feat_ctxt != NULL &&
    ...
    if (new_filter_control_unit > 0 &&
      ...
      if (cfil_info->cfi_filter_policy_gencount ==
          hash_entry->soflow_policies_gencount) {
        CFIL_LOG(LOG_NOTICE,
                 "CFIL: UDP(%s) <so %llx> - filter state changed to 0x%x, but "
                 "policies did not. Keeping existing filter state 0x%x",
                 outgoing ? "OUT" : "IN", (uint64_t)VM_KERNEL_ADDRPERM(so),
                 new_filter_control_unit, cfil_info->cfi_filter_control_unit);
      } else if (DO_PRESERVE_CONNECTIONS) {
        CFIL_LOG(LOG_NOTICE,
                 "CFIL: UDP(%s) <so %llx> - filter state changed from 0x%x to "
                 "0x%x, but existing connections are to be preserved",
                 outgoing ? "OUT" : "IN", (uint64_t)VM_KERNEL_ADDRPERM(so),
                 cfil_info->cfi_filter_control_unit, new_filter_control_unit);
        // CFIL state has changed, but preserve the flow intentionally because
        // preserve existing connections is in effect.
        cfil_info->cfi_filter_control_unit = new_filter_control_unit;
      } else {
        cfil_log_printf(
            so, cfil_info, "CFIL: %sbound UDP data dropped (0x%x -> 0x%x)",
            (outgoing ? "out" : "in"), cfil_info->cfi_filter_control_unit,
            new_filter_control_unit);
        return NULL;
      }
    }
    return cfil_info;
  ...
  if (cfil_info == NULL) {
    CFIL_LOG(LOG_ERR, "CFIL: <so %llx> UDP failed to alloc cfil_info",
             (uint64_t)VM_KERNEL_ADDRPERM(so));
    OSIncrementAtomic(&cfil_stats.cfs_sock_attach_no_mem);
    return NULL;
  }
  ...
  if (cfil_info->cfi_debug) {
    CFIL_LOG(LOG_ERR,
             "CFIL: <so %llx> UDP (outgoing %d) - debug flow with port %d",
             (uint64_t)VM_KERNEL_ADDRPERM(so), outgoing, cfil_log_port);
    CFIL_LOG(LOG_ERR,
             "CFIL: <so %llx> UDP so_gencnt %llx entry flowhash %x cfil %p "
             "sockID %llu <%llx>",
             (uint64_t)VM_KERNEL_ADDRPERM(so), so->so_gencnt,
             hash_entry->soflow_flowhash, cfil_info, cfil_info->cfi_sock_id,
             cfil_info->cfi_sock_id);
  }
  ...
  if (cfil_info->cfi_debug) {
    CFIL_LOG(LOG_ERR,
             "CFIL: UDP <so %llx> filter_control_unit %u sockID %llu <%llx> "
             "attached",
             (uint64_t)VM_KERNEL_ADDRPERM(so), filter_control_unit,
             cfil_info->cfi_sock_id, cfil_info->cfi_sock_id);
  }
  ...
  error = cfil_dispatch_attach_event(so, cfil_info, 0,
                                     outgoing ? CFS_CONNECTION_DIR_OUT
                                              : CFS_CONNECTION_DIR_IN);
  /* We can recover from flow control or out of memory errors */
  if (error != 0 && error != ENOBUFS && error != ENOMEM) {
    CFIL_LOG(LOG_ERR,
             "CFIL: UDP <so %llx> cfil_dispatch_attach_event failed <error %d>",
...
errno_t cfil_sock_udp_handle_data(bool outgoing, struct socket *so,
                                  struct sockaddr *local,
                                  struct sockaddr *remote, struct mbuf *data,
                                  struct mbuf *control, uint32_t flags,
                                  struct soflow_hash_entry *hash_entry) {
#pragma unused(outgoing, so, local, remote, data, control, flags)
  ...
  if (hash_entry == NULL) {
    CFIL_LOG(LOG_ERR, "CFIL: <so %llx> NULL soflow_hash_entry",
             (uint64_t)VM_KERNEL_ADDRPERM(so));
    return EPIPE;
  }

  if (hash_entry->soflow_db == NULL) {
    CFIL_LOG(LOG_ERR, "CFIL: <so %llx> NULL soflow_hash_entry db",
             (uint64_t)VM_KERNEL_ADDRPERM(so));
    return EPIPE;
  }
  ...
  cfil_info = cfil_sock_udp_get_info(so, filter_control_unit, outgoing,
                                     hash_entry, local, remote);
  if (cfil_info == NULL) {
    return EPIPE;
  }
  ...
  error =
      cfil_data_common(so, cfil_info, outgoing, remote, data, control, flags);
...
static bool cfil_filters_udp_attached_per_flow(
    struct socket *so, struct soflow_hash_entry *hash_entry, void *context) {
  ...
  for (kcunit = 1; kcunit <= MAX_CONTENT_FILTER; kcunit++) {
    ...
    if (apply_context->need_wait == TRUE) {
      ...
      if (so->so_flow_db == NULL ||
          (cfil_info !=
           soflow_db_get_feature_context(so->so_flow_db, sock_flow_id))) {
        // cfil_info is not valid, do not continue
        return false;
      }
 ...
 * attached flow.
 */
static int cfil_filters_udp_attached(struct socket *so, bool need_wait) {
  ...
  if ((so->so_flags & SOF_CONTENT_FILTER) != 0 && so->so_flow_db != NULL) {
    ...
    soflow_db_apply(so->so_flow_db, cfil_filters_udp_attached_per_flow,
                    (void *)&apply_context);
  }
...
static bool cfil_sock_udp_data_pending_per_flow(
    struct socket *so, struct soflow_hash_entry *hash_entry, void *context) {
#pragma unused(so)
...
int32_t cfil_sock_udp_data_pending(struct sockbuf *sb, bool check_thread) {
  ...
  if ((so->so_flags & SOF_CONTENT_FILTER) != 0 && so->so_flow_db != NULL &&
      (check_thread == FALSE ||
       so->so_snd.sb_cfil_thread != current_thread())) {
    apply_context.sb = sb;
    soflow_db_apply(so->so_flow_db, cfil_sock_udp_data_pending_per_flow,
                    (void *)&apply_context);
...
static bool cfil_sock_udp_notify_shutdown_per_flow(
    struct socket *so, struct soflow_hash_entry *hash_entry, void *context) {
...
int cfil_sock_udp_notify_shutdown(struct socket *so, int how, int drop_flag,
                                  int shut_flag) {
  ...
  if ((so->so_flags & SOF_CONTENT_FILTER) != 0 && so->so_flow_db != NULL) {
    ...
    soflow_db_apply(so->so_flow_db, cfil_sock_udp_notify_shutdown_per_flow,
                    (void *)&apply_context);
  }
...
int cfil_sock_udp_shutdown(struct socket *so, int *how) {
  int error = 0;
...
void cfil_sock_udp_close_wait(struct socket *so) {
  socket_lock_assert_owned(so);
...
static bool cfil_sock_udp_is_closed_per_flow(
    struct socket *so, struct soflow_hash_entry *hash_entry, void *context) {
...
void cfil_sock_udp_is_closed(struct socket *so) {
  socket_lock_assert_owned(so);
...
static bool cfil_sock_udp_buf_update_per_flow(
    struct socket *so, struct soflow_hash_entry *hash_entry, void *context) {
  struct cfil_info *__single cfil_info = NULL;
...
void cfil_sock_udp_buf_update(struct sockbuf *sb) {
  struct socket *so = sb->sb_so;
  ...
  if ((so->so_flags & SOF_CONTENT_FILTER) != 0 && so->so_flow_db != NULL) {
    ...
    soflow_db_apply(so->so_flow_db, cfil_sock_udp_buf_update_per_flow,
                    (void *)sb);
  }
}

void cfil_filter_show(u_int32_t kcunit) {
  struct content_filter *cfc = NULL;
  struct cfil_entry *entry;
  int count = 0;
  ...
  CFIL_LOG(LOG_DEBUG, "CFIL: FILTER SHOW:Filter - total entries shown: %d",
           count);
...
bool cfil_info_action_timed_out(struct cfil_info *cfil_info, int timeout) {
  struct cfil_entry *entry;
   ...
   * an action from user space for a while (the user space filter might have
   * crashed), return action timed out.
   */
  if (cfil_info->cfi_snd.cfi_pending_last >
          cfil_info->cfi_snd.cfi_pass_offset ||
      cfil_info->cfi_rcv.cfi_pending_last >
          cfil_info->cfi_rcv.cfi_pass_offset) {
    ...
    for (int kcunit = 1; kcunit <= MAX_CONTENT_FILTER; kcunit++) {
      ...
      if (cfil_info->cfi_snd.cfi_pending_last >
              entry->cfe_snd.cfe_pass_offset ||
          cfil_info->cfi_rcv.cfi_pending_last >
              entry->cfe_rcv.cfe_pass_offset) {
...
bool cfil_info_buffer_threshold_exceeded(struct cfil_info *cfil_info) {
  if (cfil_info == NULL) {
    return false;
  }
  ...
  if (cfil_info->cfi_snd.cfi_tail_drop_cnt ||
    ...
    if (cfil_info->cfi_debug) {
      CFIL_LOG(LOG_ERR,
               "CFIL: queue threshold exceeded:mbuf max < count: %d bytes: %d "
               "> tail drop count < OUT: %d IN: %d > ",
               cfil_udp_gc_mbuf_num_max, cfil_udp_gc_mbuf_cnt_max,
...
static bool cfil_dgram_gc_needed(struct socket *so,
                                 struct soflow_hash_entry *hash_entry,
...
static bool cfil_dgram_gc_perform(struct socket *so,
                                  struct soflow_hash_entry *hash_entry) {
  struct cfil_info *cfil_info = NULL;
...
static bool cfil_dgram_detach_entry(struct socket *so,
                                    struct soflow_hash_entry *hash_entry) {
  struct cfil_info *cfil_info = NULL;
...
static bool cfil_dgram_detach_db(struct socket *so, struct soflow_db *db) {
#pragma unused(db)
  if (so && so->so_flags & SOF_CONTENT_FILTER) {
    so->so_flags &= ~SOF_CONTENT_FILTER;
    CFIL_LOG(LOG_DEBUG, "CFIL: DGRAM DETACH DB <so %llx>",
             (uint64_t)VM_KERNEL_ADDRPERM(so));
  }
  return true;
}

struct m_tag *cfil_dgram_save_socket_state(struct cfil_info *cfil_info,
                                           struct mbuf *m) {
  struct m_tag *tag = NULL;
  ...
  if (cfil_info == NULL || cfil_info->cfi_so == NULL ||
      cfil_info->cfi_hash_entry == NULL || m == NULL ||
      !(m->m_flags & M_PKTHDR)) {
    return NULL;
  }
  ...
  if (tag) {
    ...
    if (hash_entry->soflow_family == AF_INET6) {
      fill_ip6_sockaddr_4_6(&ctag->cfil_faddr, &hash_entry->soflow_faddr.addr6,
                            hash_entry->soflow_fport,
                            hash_entry->soflow_faddr6_ifscope);
...
struct m_tag *cfil_dgram_get_socket_state(struct mbuf *m,
...
static int cfil_dispatch_stats_event_locked(
    int kcunit, struct cfil_stats_report_buffer *buffer, uint32_t stats_count) {
  struct content_filter *cfc = NULL;
  errno_t error = 0;
  size_t msgsize = 0;
  ...
  msgsize = sizeof(struct cfil_msg_stats_report) +
            (sizeof(struct cfil_msg_sock_stats) * stats_count);
  ...
  if (cfil_log_stats) {
    CFIL_LOG(LOG_DEBUG, "STATS (kcunit %d): msg size %lu - %lu %lu %lu", kcunit,
  ...
  error =
      ctl_enqueuedata(cfc->cf_kcref, cfc->cf_kcunit, buffer,
                      sizeof(struct cfil_stats_report_buffer), CTL_DATA_EOR);
  if (error != 0) {
    CFIL_LOG(LOG_ERR, "ctl_enqueuedata() failed:%d", error);
    goto done;
  }
  ...
  if (error == ENOBUFS) {
    OSIncrementAtomic(&cfil_stats.cfs_stats_event_flow_control);
...
static void cfil_stats_report_thread_sleep(bool forever) {
  if (cfil_log_stats) {
...
static void cfil_stats_report_thread_func(void *v, wait_result_t w) {
#pragma unused(v, w)
...
static bool cfil_stats_collect_flow_stats_for_filter(
    int kcunit, struct cfil_info *cfil_info, struct cfil_entry *entry,
    struct timeval current_tv) {
  ...
  if (cfil_info->cfi_debug && cfil_log_stats) {
    CFIL_LOG(
        LOG_ERR,
        "CFIL: STATS REPORT - elapsed time - ts %llu %llu cur ts %llu %llu "
        "diff %llu %llu(usecs %llu) @freq %llu usecs sockID %llu <%llx>",
        ...
        (unsigned long long)((entry->cfe_stats_report_frequency *
                              NSEC_PER_MSEC) /
                             NSEC_PER_USEC),
        cfil_info->cfi_sock_id, cfil_info->cfi_sock_id);
  }
  ...
  if (diff_time_usecs >=
      (entry->cfe_stats_report_frequency * NSEC_PER_MSEC) / NSEC_PER_USEC) {
    if (cfil_info->cfi_debug && cfil_log_stats) {
    ...
    if (entry->cfe_byte_inbound_count_reported <
            cfil_info->cfi_byte_inbound_count ||
        entry->cfe_byte_outbound_count_reported <
            cfil_info->cfi_byte_outbound_count) {
      ...
      if (entry->cfe_laddr_sent == false) {
        ...
        if (cfil_info->cfi_so_attach_laddr.sa.sa_len == 0) {
          ...
          if (inp != NULL) {
            ...
            union sockaddr_in_4_6 *dst =
                outgoing ? NULL : &cfil_info->cfi_so_attach_laddr;
            cfil_fill_event_msg_addresses(cfil_info->cfi_hash_entry, inp, src,
                                          dst, outgoing);
          }
        }
      ...
      CFI_ADD_TIME_LOG(cfil_info, &current_tv, &cfil_info->cfi_first_event,
                       CFM_OP_STATS);
      return true;
    }
  }
  ...
  do {
    ...
    for (int kcunit = 1; kcunit <= MAX_CONTENT_FILTER; kcunit++) {
      if (global_cfil_stats_report_buffers[kcunit - 1] != NULL) {
        memset(global_cfil_stats_report_buffers[kcunit - 1], 0,
    ...
    TAILQ_FOREACH(cfil_info, &cfil_sock_head_stats, cfi_link_stats) {
      ...
      if (saved_next_sock_id == 0) {
        if (flow_count >= CFIL_STATS_REPORT_MAX_COUNT) {
          // Examine a fixed number of flows each round.  Remember the current
          // flow so we can start from here for next loop
          saved_next_sock_id = cfil_info->cfi_sock_id;
          break;
        }
        ...
        for (int kcunit = 1; kcunit <= MAX_CONTENT_FILTER; kcunit++) {
          ...
          if ((entry->cfe_stats_report_frequency > 0) &&
              cfil_stats_collect_flow_stats_for_filter(kcunit, cfil_info, entry,
                                                       current_tv) == true) {
            flow_reported = true;
          }
        }
    ...
    if (flow_count > 0) {
      ...
      for (int kcunit = 1; kcunit <= MAX_CONTENT_FILTER; kcunit++) {
        if (global_cfil_stats_report_buffers[kcunit - 1] != NULL &&
            global_cfil_stats_counts[kcunit - 1] > 0) {
          cfil_dispatch_stats_event_locked(
              kcunit, global_cfil_stats_report_buffers[kcunit - 1],
              global_cfil_stats_counts[kcunit - 1]);
        }
      }
    } else {
...
go_sleep:
  ...
  cfil_rw_lock_shared(&cfil_lck_rw);
  cfil_stats_report_thread_sleep(cfil_sock_attached_stats_count == 0 ? true
                                                                     : false);
  cfil_rw_unlock_shared(&cfil_lck_rw);
  thread_block_parameter((thread_continue_t)cfil_stats_report, NULL);
  /* NOTREACHED */
