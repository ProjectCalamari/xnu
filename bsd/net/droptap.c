...
SYSCTL_NODE(_net_link, OID_AUTO, droptap, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "droptap virtual interface");
...
SYSCTL_UINT(_net_link_droptap, OID_AUTO, verbose, CTLFLAG_RW | CTLFLAG_LOCKED,
            &droptap_verbose, 0, "");
...
static LIST_HEAD(droptap_list, droptap_softc)
    droptap_list = LIST_HEAD_INITIALIZER(droptap_list);
...
static struct if_clone droptap_cloner =
    IF_CLONE_INITIALIZER(DROPTAP_IFNAME, droptap_clone_create,
                         droptap_clone_destroy, 0, DROPTAP_MAXUNIT);
...
int droptap_clone_create(struct if_clone *ifc, u_int32_t unit,
  ...
  bpf_attach(droptap->dtap_ifp, DLT_PKTAP, sizeof(struct droptap_header), NULL,
             droptap_tap_callback);
...
int droptap_clone_destroy(struct ifnet *ifp) {
  int error = 0;
...
errno_t droptap_tap_callback(ifnet_t ifp, u_int32_t dlt,
...
errno_t droptap_if_output(ifnet_t __unused ifp, mbuf_t __unused m) {
  return ENOTSUP;
}

errno_t droptap_add_proto(__unused ifnet_t ifp, __unused protocol_family_t pf,
                          __unused const struct ifnet_demux_desc *dmx,
                          __unused u_int32_t cnt) {
  return 0;
}

errno_t droptap_del_proto(__unused ifnet_t ifp, __unused protocol_family_t pf) {
  return 0;
}

void droptap_detach(ifnet_t ifp) {
  struct droptap_softc *__single droptap;
...
void droptap_input_packet(kern_packet_t pkt, drop_reason_t reason,
                          const char *funcname, uint16_t linenum,
                          uint16_t flags, struct ifnet *ifp, pid_t pid,
                          const char *pname, pid_t epid, const char *epname,
  ...
  if (funcname != NULL) {
    ...
    snprintf(dtaphdr.dth_dropfunc, sizeof(dtaphdr.dth_dropfunc), "%s",
             funcname);
  ...
  droptap_bpf_tap_packet(pkt, DROPTAP_FLAG_DIR_IN | PTH_FLAG_NEXUS_CHAN,
                         &dtaphdr, dlt, ifp, pid, pname, epid, epname, ipproto,
                         flowid);
}

void droptap_output_packet(kern_packet_t pkt, drop_reason_t reason,
                           const char *funcname, uint16_t linenum,
                           uint16_t flags, struct ifnet *ifp, pid_t pid,
                           const char *pname, pid_t epid, const char *epname,
                           uint8_t ipproto, uint32_t flowid) {
...
static void droptap_bpf_tap_packet(kern_packet_t pkt, uint32_t flags,
                                   struct droptap_header *dtaphdr, uint32_t dlt,
                                   struct ifnet *ifp, pid_t pid,
                                   const char *pname, pid_t epid,
                                   const char *epname, uint8_t ipproto,
                                   uint32_t flowid) {
  ...
  hdr->pth_flags |=
      flags & DROPTAP_FLAG_DIR_OUT ? PTH_FLAG_DIR_OUT : PTH_FLAG_DIR_IN;
  ...
  LIST_FOREACH(droptap, &droptap_list, dtap_link) {
    if (droptap->dtap_dlt_pktap_count > 0) {
      tap_packet_func(droptap->dtap_ifp, DLT_PKTAP, pkt, hdr, hdr_size);
    }
  }
  lck_rw_done(&droptap_lck_rw);
}

void droptap_input_mbuf(struct mbuf *m, drop_reason_t reason,
                        const char *funcname, uint16_t linenum, uint16_t flags,
                        struct ifnet *ifp, char *frame_header) {
  struct droptap_header dtaphdr;
  char *hdr;
  char *start;
...
void droptap_output_mbuf(struct mbuf *m, drop_reason_t reason,
                         const char *funcname, uint16_t linenum, uint16_t flags,
                         struct ifnet *ifp) {
  struct droptap_header dtaphdr;
...
static void droptap_bpf_tap_mbuf(struct mbuf *m, uint16_t flags,
                                 struct droptap_header *dtaphdr,
                                 struct ifnet *ifp) {
  struct droptap_softc *droptap;
  struct pktap_header *hdr;
  ...
  LIST_FOREACH(droptap, &droptap_list, dtap_link) {
    if (droptap->dtap_dlt_pktap_count > 0) {
      bpf_tap_func(droptap->dtap_ifp, DLT_PKTAP, m, hdr, hdr_size);
    }
  }
  lck_rw_done(&droptap_lck_rw);
}
