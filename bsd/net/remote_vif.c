...
static errno_t rvi_add_proto(ifnet_t, protocol_family_t,
...
static errno_t rvi_bpf_tap(ifnet_t, mbuf_t, int, struct rvi_client_t *,
                           struct pktap_header *);
...
static errno_t rvi_ctl_getopt(kern_ctl_ref, uint32_t, void *, int,
...
static inline void rvi_lock_shared(lck_rw_t *mtx) { lck_rw_lock_shared(mtx); }

static inline void rvi_lock_exclusive(lck_rw_t *mtx) {
  lck_rw_lock_exclusive(mtx);
}

static inline void rvi_lock_done_shared(lck_rw_t *mtx) {
  lck_rw_unlock_shared(mtx);
}

static inline void rvi_lock_done_exclusive(lck_rw_t *mtx) {
  lck_rw_unlock_exclusive(mtx);
}

static errno_t rvi_create_if(struct rvi_client_t *client) {
  errno_t err = 0;
  ...
  if (err != 0) {
    os_log(OS_LOG_DEFAULT, "%s: ifnet_allocate for %s%d failed - %d", __func__,
           RVI_IF_NAME, client->_vif, err);
    goto done;
  }
  ...
  if (err != 0) {
    os_log(OS_LOG_DEFAULT, "%s: ifnet_attach for %s%d failed - %d", __func__,
           RVI_IF_NAME, client->_vif, err);
...
static errno_t rvi_destroy_if(struct rvi_client_t *client) {
  errno_t err = 0;
  ...
  if (err != 0) {
    os_log(OS_LOG_DEFAULT, "%s: ifnet_detach for %s%d failed - %d", __func__,
           RVI_IF_NAME, client->_vif, err);
  }
done:
  return err;
...
static void rvi_detach(ifnet_t ifp) {
...
static void rvi_insert_client(struct rvi_client_t *client) {
...
static void rvi_remove_client(uint32_t unit) {
  ...
  if (client == NULL) {
    panic("rvi_ctl_disconnect: received a disconnect notification without a "
          "cache entry");
  }
...
static errno_t rvi_ctl_connect(kern_ctl_ref kctlref, struct sockaddr_ctl *sac,
                               void **unitinfo) {
...
static errno_t rvi_ctl_disconnect(kern_ctl_ref kctlref, uint32_t unit,
                                  void *unitinfo) {
#pragma unused(kctlref)
...
static errno_t rvi_ctl_getopt(kern_ctl_ref kctlref, uint32_t unit,
...
static errno_t rvi_ctl_send(kern_ctl_ref kctlref, uint32_t unit, void *unitinfo,
                            mbuf_t m, int flags) {
#pragma unused(kctlref)
#pragma unused(unit)
#pragma unused(flags)
  ...
  err = rvi_bpf_tap(client->_ifp, m,
                    pktap_hdr.pth_flags & PTH_FLAG_DIR_OUT ? 1 : 0, client,
                    &pktap_hdr);
...
static errno_t rvi_output(ifnet_t ifp, mbuf_t m) {
#pragma unused(ifp)
...
static errno_t rvi_demux(ifnet_t ifp, mbuf_t m, char *header,
...
static errno_t rvi_add_proto(ifnet_t ifp, protocol_family_t pf,
...
static errno_t rvi_del_proto(ifnet_t ifp, protocol_family_t pf) {
#pragma unused(ifp)
#pragma unused(pf)
...
static errno_t rvi_ioctl(ifnet_t ifp, unsigned long cmd, void *data) {
#pragma unused(ifp)
#pragma unused(cmd)
#pragma unused(data)
...
static errno_t rvi_set_bpf_tap(ifnet_t ifp, uint32_t dlt, bpf_tap_mode mode) {
  struct rvi_client_t *__single client;
 ...
 */
static errno_t rvi_bpf_tap(ifnet_t ifp, mbuf_t m, int outgoing,
...
#pragma unused(ifp)
  ...
  if (client->_raw_count > 0 && (pktap_hdr->pth_protocol_family == AF_INET ||
