 ...
 */
int dlil_if_acquire(uint32_t family,
                    const void *uniqueid __sized_by(uniqueid_len),
                    size_t uniqueid_len, const char *ifxname0 __null_terminated,
                    struct ifnet **ifp) {
  ...
  const char *ifxname =
      __unsafe_forge_bidi_indexable(const char *, ifxname0, ifxname_len);
  size_t ifp_name_len;
  ...
  TAILQ_FOREACH(dlifp1, &dlil_ifnet_head, dl_if_link) {
    ...
    if (uniqueid_len != 0 && uniqueid_len == dlifp1->dl_if_uniqueid_len &&
  ...
  if (uniqueid_len) {
    void *new_uniqueid = kalloc_data(uniqueid_len, Z_WAITOK);
  ...
  ifp1->if_name = tsnprintf(dlifp1->dl_if_namestorage,
                            sizeof(dlifp1->dl_if_namestorage), "");
  ifp1->if_xname = tsnprintf(dlifp1->dl_if_xnamestorage,
                             sizeof(dlifp1->dl_if_xnamestorage), "");
  ...
  lck_mtx_init(&ifp1->if_addrconfig_lock, &ifnet_lock_group, &ifnet_lock_attr);
  lck_rw_init(&ifp1->if_llreach_lock, &ifnet_lock_group, &ifnet_lock_attr);
#if INET
  lck_rw_init(&ifp1->if_inetdata_lock, &ifnet_lock_group, &ifnet_lock_attr);
  ...
  lck_rw_init(&ifp1->if_link_status_lock, &ifnet_lock_group, &ifnet_lock_attr);
  ifp1->if_link_status = NULL;
  lck_mtx_init(&ifp1->if_delegate_lock, &ifnet_lock_group, &ifnet_lock_attr);

  /* for send data paths */
  lck_mtx_init(&ifp1->if_start_lock, &ifnet_snd_lock_group, &ifnet_lock_attr);
  lck_mtx_init(&ifp1->if_cached_route_lock, &ifnet_snd_lock_group,
               &ifnet_lock_attr);

  /* for receive data paths */
  lck_mtx_init(&ifp1->if_poll_lock, &ifnet_rcv_lock_group, &ifnet_lock_attr);

  /* thread call allocation is done with sleeping zalloc */
  ifp1->if_dt_tcall = thread_call_allocate_with_options(
      dlil_dt_tcall_fn, ifp1, THREAD_CALL_PRIORITY_KERNEL,
 ...
 * Stats management.
 */
void dlil_input_stats_add(const struct ifnet_stat_increment_param *s,
                          struct dlil_threading_info *inp, struct ifnet *ifp,
                          boolean_t poll) {
...
boolean_t dlil_input_stats_sync(struct ifnet *ifp,
...
#if SKYWALK
errno_t dlil_set_input_handler(struct ifnet *ifp, dlil_input_func fn) {
  return os_atomic_cmpxchg(
             __unsafe_forge_single(void *volatile *, &ifp->if_input_dlil),
             ptrauth_nop_cast(void *, &dlil_input_handler),
             ptrauth_nop_cast(void *, fn), acq_rel)
             ? 0
             : EBUSY;
}

void dlil_reset_input_handler(struct ifnet *ifp) {
...
errno_t dlil_set_output_handler(struct ifnet *ifp, dlil_output_func fn) {
  return os_atomic_cmpxchg(
             __unsafe_forge_single(void *volatile *, &ifp->if_output_dlil),
...
void dlil_reset_output_handler(struct ifnet *ifp) {
  while (!os_atomic_cmpxchg(
      __unsafe_forge_single(void *volatile *, &ifp->if_output_dlil),
...
errno_t dlil_output_handler(struct ifnet *ifp, struct mbuf *m) {
...
#if SKYWALK
errno_t ifnet_set_output_handler(struct ifnet *ifp, ifnet_output_func fn) {
  return os_atomic_cmpxchg(
             __unsafe_forge_single(void *volatile *, &ifp->if_output),
...
void ifnet_reset_output_handler(struct ifnet *ifp) {
  while (!os_atomic_cmpxchg(
      __unsafe_forge_single(void *volatile *, &ifp->if_output),
...
errno_t ifnet_set_start_handler(struct ifnet *ifp, ifnet_start_func fn) {
  return os_atomic_cmpxchg(
             __unsafe_forge_single(void *volatile *, &ifp->if_start),
...
void ifnet_reset_start_handler(struct ifnet *ifp) {
  while (!os_atomic_cmpxchg(
      __unsafe_forge_single(void *volatile *, &ifp->if_start),
