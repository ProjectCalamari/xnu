...
SYSCTL_NODE(_net, OID_AUTO, pktmnglr, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "pktmnglr");
...
static KALLOC_TYPE_DEFINE(packet_mangler_zone, struct packet_mangler,
                          NET_KT_DEFAULT);
...
static errno_t pktmnglr_ipfilter_input(void *cookie, mbuf_t *data, int offset,
                                       u_int8_t protocol);
...
static void pkt_mnglr_rw_lock_exclusive(lck_rw_t *lck) {
...
static void pkt_mnglr_rw_unlock_exclusive(lck_rw_t *lck) {
  void *__single lr_saved;
...
static void pkt_mnglr_rw_lock_shared(lck_rw_t *lck) {
  void *__single lr_saved;
...
static void pkt_mnglr_rw_unlock_shared(lck_rw_t *lck) {
  void *__single lr_saved;
 ...
 */
static errno_t pkt_mnglr_ctl_connect(kern_ctl_ref kctlref,
  ...
  p_pkt_mnglr = zalloc_flags(packet_mangler_zone, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  error = ipf_addv4(&(p_pkt_mnglr->pkt_mnglr_ipfilter),
                    &(p_pkt_mnglr->pkt_mnglr_ipfref));
  if (error) {
    PKT_MNGLR_LOG(LOG_ERR, "Could not register packet mangler's IPv4 Filter");
    goto fail_locked;
  }
  error = ipf_addv6(&(p_pkt_mnglr->pkt_mnglr_ipfilter),
                    &(p_pkt_mnglr->pkt_mnglr_ipfrefv6));
  if (error) {
...
static errno_t pkt_mnglr_ctl_disconnect(kern_ctl_ref kctlref, u_int32_t kcunit,
                                        void *unitinfo) {
#pragma unused(kctlref)
  ...
  if (kcunit > MAX_PACKET_MANGLER) {
    PKT_MNGLR_LOG(LOG_ERR, "kcunit %u > MAX_PACKET_MANGLER (%d)", kcunit,
                  MAX_PACKET_MANGLER);
    error = EINVAL;
    goto done;
  }
  ...
  pkt_mnglr_rw_lock_exclusive(&pkt_mnglr_lck_rw);
  if (packet_manglers[kcunit - 1] != p_pkt_mnglr ||
      p_pkt_mnglr->pkt_mnglr_kcunit != kcunit) {
    PKT_MNGLR_LOG(LOG_ERR, "bad unit info %u", kcunit);
    pkt_mnglr_rw_unlock_exclusive(&pkt_mnglr_lck_rw);
    goto done;
  }
...
static errno_t pkt_mnglr_ctl_getopt(kern_ctl_ref kctlref, u_int32_t kcunit,
...
#pragma unused(kctlref, opt)
  ...
  if (p_pkt_mnglr != (void *)packet_manglers[kcunit - 1]) {
    PKT_MNGLR_LOG(LOG_ERR, "unitinfo does not match for kcunit %u", kcunit);
    error = EINVAL;
    goto done;
  }
  switch (opt) {
...
static errno_t pkt_mnglr_ctl_setopt(kern_ctl_ref kctlref, u_int32_t kcunit,
                                    void *unitinfo, int opt, void *data,
                                    size_t len) {
#pragma unused(kctlref, opt)
  errno_t error = 0;
  ...
  case PKT_MNGLR_OPT_PROTO_ACT_MASK:
    ...
    PKT_MNGLR_LOG(LOG_INFO, "p_pkt_mnglr->proto_action_mask set to :%d",
                  p_pkt_mnglr->proto_action_mask);
    break;
  case PKT_MNGLR_OPT_IP_ACT_MASK:
    if (len < sizeof(uint32_t)) {
  ...
  case PKT_MNGLR_OPT_REMOTE_IP:
    ...
    PKT_MNGLR_LOG(LOG_INFO, "Remote IP registered for address family: %d",
...
static errno_t pktmnglr_ipfilter_input(void *cookie, mbuf_t *data, int offset,
  ...
  if (p_pkt_mnglr->rsaddr.ss_family == AF_INET) {
    ...
    PKT_MNGLR_LOG(LOG_INFO, "Remote IP: %x Source IP: %x in input path",
                  raddr.sin_addr.s_addr, ip.ip_src.s_addr);
  ...
  case IPPROTO_TCP:
    if (ip_pld_len < sizeof(tcp)) {
      PKT_MNGLR_LOG(LOG_ERR, "IP total len not big enough for TCP: %zu",
                    ip_pld_len);
      goto drop_it;
    }
  ...
  case IPPROTO_TCP:
    if (p_pkt_mnglr->proto_action_mask) {
      ...
      if (orig_tcp_optlen) {
        error = mbuf_copydata(*data, (size_t)offset + sizeof(struct tcphdr),
                              orig_tcp_optlen, tcp_opt_buf);
        if (error) {
          PKT_MNGLR_LOG(
              LOG_ERR,
              "Failed to copy tcp options: error %d offset %d optlen %zu",
      ...
      while (tcp_optlen > 0) {
        if (tcp_opt_buf[i] == 0x1) {
          ...
          i++;
          continue;
        } else if ((tcp_opt_buf[i] != 0) &&
                   (tcp_opt_buf[i] != TCP_OPT_MULTIPATH_TCP)) {
          ...
          if (subtype == MPO_DSS) {
            ...
            PKT_MNGLR_LOG(LOG_INFO, "Protocol option mask: %d\n",
                          p_pkt_mnglr->proto_action_mask);
            if (p_pkt_mnglr->proto_action_mask & PKT_MNGLR_TCP_ACT_DSS_DROP) {
              goto drop_it;
            }
          }
      ...
      if (orig_tcp_optlen) {
        error = mbuf_copyback(*data, (size_t)offset + sizeof(struct tcphdr),
