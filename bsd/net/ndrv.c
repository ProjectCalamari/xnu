...
SYSCTL_NODE(_net, OID_AUTO, ndrv, CTLFLAG_RW | CTLFLAG_LOCKED, 0, "");
...
SYSCTL_UINT(_net_ndrv, OID_AUTO, multi_max_count, CTLFLAG_RW | CTLFLAG_LOCKED,
            &ndrv_multi_max_count, 0,
            "Number of allowed multicast addresses per NRDV socket");

/*
 * The locking strategy relies on the PF_NRDRV domain mutex that protects both
 * the PCB list "ndrvl" and the sockets themselves
 */

static int ndrv_do_detach(struct ndrv_cb *);
static int ndrv_do_disconnect(struct ndrv_cb *);
static struct ndrv_cb *ndrv_find_inbound(struct ifnet *ifp,
                                         u_int32_t protocol_family);
static int ndrv_setspec(struct ndrv_cb *np, struct sockopt *sopt);
static int ndrv_delspec(struct ndrv_cb *);
static int ndrv_to_ifnet_demux(struct ndrv_demux_desc *ndrv,
...
static struct ndrv_multiaddr *ndrv_have_multicast(struct ndrv_cb *np,
 ...
 *  to the driver.
 */
static int ndrv_output(struct mbuf *m, struct socket *so) {
...
static errno_t ndrv_input(ifnet_t ifp, protocol_family_t proto_family, mbuf_t m,
                          char *orig_frame_header) {
  struct socket *so;
  struct sockaddr_dl ndrvsrc = {};
  struct ndrv_cb *np;
  char *frame_header = __unsafe_forge_bidi_indexable(char *, orig_frame_header,
                                                     ifnet_hdrlen(ifp));
  int error = 0;
  ...
  if (sbappendaddr(&(so->so_rcv), (struct sockaddr *)&ndrvsrc, m, NULL,
                   &error) != 0) {
    sorwakeup(so);
  }
 ...
 */
static int ndrv_attach(struct socket *so, int proto, __unused struct proc *p) {
  int error;
  struct ndrv_cb *np = sotondrvcb(so);
...
static int ndrv_detach(struct socket *so) {
  struct ndrv_cb *np = sotondrvcb(so);
...
static int ndrv_connect(struct socket *so, struct sockaddr *nam,
  ...
  np->nd_faddr =
      kalloc_type(struct sockaddr_ndrv, Z_WAITOK | Z_NOFAIL | Z_ZERO);
...
static void ndrv_event(struct ifnet *ifp, __unused protocol_family_t protocol,
 ...
 */
static int ndrv_bind(struct socket *so, struct sockaddr *nam,
                     __unused struct proc *p) {
  ...
  if (ifnet_family(ifp) != APPLE_IF_FAM_PPP) {
    ...
    /* We aren't worried about double attaching, that should just return an
     * error */
    socket_unlock(so, 0);
...
static int ndrv_disconnect(struct socket *so) {
  struct ndrv_cb *np = sotondrvcb(so);
 ...
 */
static int ndrv_shutdown(struct socket *so) {
 ...
 */
static int ndrv_send(struct socket *so, __unused int flags, struct mbuf *m,
...
static int ndrv_abort(struct socket *so) {
  struct ndrv_cb *np = sotondrvcb(so);
...
static int ndrv_sockaddr(struct socket *so, struct sockaddr **nam) {
  struct ndrv_cb *np = sotondrvcb(so);
  unsigned int len;
  ...
  *nam = (struct sockaddr *)alloc_sockaddr(len, Z_WAITOK | Z_NOFAIL);

  SOCKADDR_COPY(np->nd_laddr, *nam, len);
  return 0;
}

static int ndrv_peeraddr(struct socket *so, struct sockaddr **nam) {
  struct ndrv_cb *np = sotondrvcb(so);
  unsigned int len;
...
static int ndrv_ctloutput(struct socket *so, struct sockopt *sopt) {
  struct ndrv_cb *np = sotondrvcb(so);
  int error = 0;
...
#ifdef NDRV_DEBUG
  log(LOG_WARNING, "NDRV CTLOUT: %x returns %d\n", sopt->sopt_name, error);
#endif
  return error;
}

static int ndrv_do_detach(struct ndrv_cb *np) {
...
static int ndrv_do_disconnect(struct ndrv_cb *np) {
  struct socket *so = np->nd_socket;
#if NDRV_DEBUG
...
int ndrv_setspec(struct ndrv_cb *np, struct sockopt *sopt) {
  ...
  if (proc_is64bit(sopt->sopt_p)) {
    ...
    error =
        sooptcopyin(sopt, &ndrvSpec64, sizeof(ndrvSpec64), sizeof(ndrvSpec64));
    if (error != 0) {
      return error;
    }
    ...
    error =
        sooptcopyin(sopt, &ndrvSpec32, sizeof(ndrvSpec32), sizeof(ndrvSpec32));
    if (error != 0) {
      return error;
    }
  ...
  struct ifnet_demux_desc *demux_desc =
      kalloc_type(struct ifnet_demux_desc, demux_count, Z_WAITOK | Z_ZERO);
  if (demux_desc == NULL) {
    error = ENOMEM;
  } else {
  ...
  if (proto_param.demux_array != NULL) {
    kfree_type_counted_by(struct ifnet_demux_desc, proto_param.demux_count,
...
int ndrv_to_ifnet_demux(struct ndrv_demux_desc *ndrv,
                        struct ifnet_demux_desc *ifdemux) {
...
int ndrv_delspec(struct ndrv_cb *np) {
  int result = 0;

  if (np->nd_proto_family == PF_NDRV || np->nd_proto_family == 0) {
    return EINVAL;
  }
...
struct ndrv_cb *ndrv_find_inbound(struct ifnet *ifp, u_int32_t protocol) {
  struct ndrv_cb *np;
  ...
  TAILQ_FOREACH(np, &ndrvl, nd_next) {
    if (np->nd_proto_family == protocol && np->nd_if == ifp) {
      return np;
    }
  }
...
static void ndrv_handle_ifp_detach(u_int32_t family, short unit) {
  ...
  TAILQ_FOREACH(np, &ndrvl, nd_next) {
    if (np->nd_family == family && np->nd_unit == unit) {
...
static void ndrv_multiaddr_free(struct ndrv_multiaddr *ndrv_multi) {
...
static int ndrv_do_add_multicast(struct ndrv_cb *np, struct sockopt *sopt) {
  ...
  if (result == 0) {
    ...
    result =
        ifnet_add_multicast(np->nd_if, ndrv_multi->addr, &ndrv_multi->ifma);
  }
...
static void ndrv_cb_remove_multiaddr(struct ndrv_cb *np,
                                     struct ndrv_multiaddr *ndrv_entry) {
...
static void ndrv_dominit(struct domain *dp) {
  struct protosw *pr;
  int i;
