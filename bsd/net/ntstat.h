...
// Note, the nstat_detailed_counts structure is not intended for route
// statistics, hence no equivalent of the nstat_connectattempts and
// nstat_connectsuccesses within nstat_counts
...
// They are mutually exclusive but there is no guarantee that one or the other
// will be set
...
#define NSTAT_MAX_DOMAIN_NAME_LENGTH                                           \
  256 /* As per RFC 2181 for full domain name */
...
enum { NSTAT_SRC_REF_ALL = 0xffffffffffffffffULL, NSTAT_SRC_REF_INVALID = 0 };

/* Source-level filters */
enum { NSTAT_FILTER_NOZEROBYTES = 0x00000001 };

/* Types of extended update information, used in setting initial filters as well
 * as to identify returned extensions */
/* A contiguous range currently limited 1..31 due to being passed as the top 32
 * bits of filter */
enum {
...
#define NSTAT_EXTENDED_UPDATE_TYPE_MAX                                         \
  NSTAT_EXTENDED_UPDATE_TYPE_BLUETOOTH_COUNTS

#define NSTAT_EXTENDED_UPDATE_FLAG_MASK                                        \
  0x00ffffffull /* Maximum of 24 extension types allowed due to restrictions   \
                   on specifying via filter flags */

#define NSTAT_FILTER_ALLOWED_EXTENSIONS_SHIFT                                  \
  40 /* With extensions expediently passed as the top 24 bits of filters       \
        supplied by client, this shift is for extraction */
...
#define NSTAT_FILTER_UDP_INTERFACE_ATTACH                                      \
  0x0000000000020000ull // Subject to removal, do not use
#define NSTAT_FILTER_UDP_FLAGS 0x0000000000020000ull

#define NSTAT_FILTER_TCP_INTERFACE_ATTACH                                      \
  0x0000000000040000ull // Subject to removal, do not use
...
#define NSTAT_FILTER_SOURCE_IS_LISTENER                                        \
  0x0000000002000000ull // NSTAT_SOURCE_IS_LISTENER
#define NSTAT_FILTER_SOURCE_IS_INBOUND                                         \
  0x0000000004000000ull // NSTAT_SOURCE_IS_INBOUND
#define NSTAT_FILTER_SOURCE_IS_OUTBOUND                                        \
  0x0000000008000000ull // NSTAT_SOURCE_IS_OUTBOUND
#define NSTAT_FILTER_SOURCE_ROLE_FLAGS                                         \
  0x000000000E000000ull // All three of the above

/* In this context, boring == no change from previous report */
#define NSTAT_FILTER_SUPPRESS_BORING_CLOSE                                     \
  0x0000000010000000ull /* No final update, only NSTAT_MSG_TYPE_SRC_REMOVED */
#define NSTAT_FILTER_SUPPRESS_BORING_POLL                                      \
  0x0000000020000000ull /* Only for poll-all, not poll specific source */
#define NSTAT_FILTER_SUPPRESS_BORING_FLAGS                                     \
  (NSTAT_FILTER_SUPPRESS_BORING_CLOSE | NSTAT_FILTER_SUPPRESS_BORING_POLL)

#define NSTAT_FILTER_FLAGS_RESERVED_30 0x0000000040000000ull
#define NSTAT_FILTER_FLAGS_RESERVED_31 0x0000000080000000ull

/* The filtering for specific user is a speculative option that hasn't been
 * exploited.  It may be removed */
...
#define NSTAT_FILTER_INITIAL_PROPERTIES                                        \
  0x0000001000000000ull /* For providers that give "properties" on open, apply \
                           the filter to the properties */
/* and permanently discard unless the filter allows through */
#define NSTAT_FILTER_USE_LARGE_BUFFERS                                         \
  0x0000002000000000ull /* Not really a filter, place here until we have other \
                           mechanisms to pass this */
#define NSTAT_FILTER_DELIVER_ONCE                                              \
  0x0000004000000000ull /* Single shot delivery */
#define NSTAT_FILTER_VERSION_2_PROTOCOL 0x0000008000000000ull

#define NSTAT_FILTER_IFNET_AND_CONN_FLAGS                                      \
  (NSTAT_FILTER_IFNET_FLAGS | NSTAT_FILTER_CONN_FLAGS)

#define NSTAT_EXTENSION_FILTER_DOMAIN_INFO                                     \
  (1ull << (NSTAT_EXTENDED_UPDATE_TYPE_DOMAIN +                                \
            NSTAT_FILTER_ALLOWED_EXTENSIONS_SHIFT))
#define NSTAT_EXTENSION_FILTER_NECP_TLV                                        \
  (1ull << (NSTAT_EXTENDED_UPDATE_TYPE_NECP_TLV +                              \
            NSTAT_FILTER_ALLOWED_EXTENSIONS_SHIFT))
#define NSTAT_EXTENSION_FILTER_ORIGINAL_NECP_TLV                               \
  (1ull << (NSTAT_EXTENDED_UPDATE_TYPE_ORIGINAL_NECP_TLV +                     \
            NSTAT_FILTER_ALLOWED_EXTENSIONS_SHIFT))
#define NSTAT_EXTENSION_FILTER_ORIGINAL_DOMAIN_INFO                            \
  (1ull << (NSTAT_EXTENDED_UPDATE_TYPE_ORIGINAL_DOMAIN +                       \
            NSTAT_FILTER_ALLOWED_EXTENSIONS_SHIFT))
#define NSTAT_EXTENSION_FILTER_BLUETOOTH_COUNTS                                \
  (1ull << (NSTAT_EXTENDED_UPDATE_TYPE_BLUETOOTH_COUNTS +                      \
            NSTAT_FILTER_ALLOWED_EXTENSIONS_SHIFT))
#define NSTAT_EXTENSION_FILTER_MASK                                            \
  (NSTAT_EXTENDED_UPDATE_FLAG_MASK << NSTAT_FILTER_ALLOWED_EXTENSIONS_SHIFT)

// Version one is constrained to use only the following
#define NSTAT_FILTER_FLAGS_V1_USAGE                                            \
  (NSTAT_FILTER_ACCEPT_UNKNOWN | NSTAT_FILTER_ACCEPT_LOOPBACK |                \
   NSTAT_FILTER_ACCEPT_CELLULAR | NSTAT_FILTER_ACCEPT_WIFI |                   \
   NSTAT_FILTER_ACCEPT_WIRED | NSTAT_FILTER_ACCEPT_AWDL |                      \
   NSTAT_FILTER_ACCEPT_EXPENSIVE | NSTAT_FILTER_ACCEPT_CELLFALLBACK |          \
   NSTAT_FILTER_ACCEPT_COMPANIONLINK | NSTAT_FILTER_ACCEPT_IS_CONSTRAINED |    \
   NSTAT_FILTER_ACCEPT_IS_LOCAL | NSTAT_FILTER_ACCEPT_IS_NON_LOCAL)

// A note on the header flags
//
// NSTAT_MSG_HDR_FLAG_SUPPORTS_AGGREGATE was used to indicate that user level
// code could cope with multiple counts or descriptor messages within a single
// overall message on the control socket. This ability is now mandatory for user
// level clients.  They may or may not choose to still set
// NSTAT_MSG_HDR_FLAG_SUPPORTS_AGGREGATE, but they must support aggregate
// responses
//
// For messages from the client, NSTAT_MSG_HDR_FLAG_CONTINUATION was used to
// indicate that the results of any NSTAT_SRC_REF_ALL poll could be returned in
// fragments, each fragment except the last one having
// NSTAT_MSG_HDR_FLAG_CONTINUATION set and each intermediate fragment intended
// to elicit a further NSTAT_SRC_REF_ALL poll with the same context as the
// initial one.  This pacing is intended to prevent overload on what amounts to
// a producer/consumer interface.  This style is now mandatory, whether or not
// NSTAT_MSG_HDR_FLAG_CONTINUATION is set, any polls may result in data being
// returned in fragments which each contain just a portion of the requested
// counts/descriptors/updates. The user level clients may choose whether or not
// to set NSTAT_MSG_HDR_FLAG_CONTINUATION in any polls but they must support
// getting poll responses in multiple chunks
//
enum {
...
#define DEFINE_NTSTAT_DATA_ACCESSOR(NTSTAT_TYPE)                               \
  static inline __attribute__((always_inline)) __attribute__((overloadable))   \
...
/* While the only type of extended update is for domain information, we can
 * fully define the structure */
...
static inline __attribute__((always_inline)) struct nstat_sysinfo_keyval *
    __header_indexable
    nstat_sysinfo_get_keyvals(
        struct nstat_msg_sysinfo_counts *__header_indexable counts) {
...
struct nstat_global_counts {
  uint64_t
      nstat_global_count_version; // current version number for this structure

  uint64_t nstat_global_exclusive_lock_uncontended; // Uncontended acquisitions
                                                    // of exlusive lock
  uint64_t nstat_global_exclusive_lock_contended;   // Contended acquisitions of
                                                    // exlusive lock

  uint64_t nstat_global_shared_lock_uncontended; // Uncontended acquisitions of
                                                 // shared lock
  uint64_t nstat_global_shared_lock_contended;   // Contended acquisitions of
                                                 // shared lock
  ...
  uint64_t nstat_global_client_alloc_fails; // total number of failures to
                                            // allocate a client
  ...
  uint64_t nstat_global_src_alloc_fails; // total number of failures to allocate
                                         // a source

  uint64_t nstat_global_tcp_sck_locus_current; // current number of tcp
                                               // nstat_sock_locus overall
  uint64_t nstat_global_tcp_sck_locus_max; // max number of tcp nstat_sock_locus
                                           // overall
  uint64_t nstat_global_tcp_sck_locus_allocs; // total number of tcp
                                              // nstat_sock_locus allocated
  uint64_t
      nstat_global_tcp_sck_locus_alloc_fails; // total number of failures to
                                              // allocate a tcp nstat_sock_locus

  uint64_t
      nstat_global_udp_sck_locus_current;  // current number of udp
                                           // nstat_extended_sock_locus overall
  uint64_t nstat_global_udp_sck_locus_max; // max number of udp
                                           // nstat_extended_sock_locus overall
  uint64_t
      nstat_global_udp_sck_locus_allocs; // total number of udp
                                         // nstat_extended_sock_locus allocated
  uint64_t nstat_global_udp_sck_locus_alloc_fails; // total number of failures
                                                   // to allocate a udp
                                                   // nstat_extended_sock_locus

  uint64_t nstat_global_tu_shad_current; // current number of nstat_tu_shadow
                                         // objects overall
  ...
  uint64_t nstat_global_gshad_current; // current number of generic shadow
                                       // objects overall
  ...
  uint64_t nstat_global_procdetails_current; // current number of procdetails
                                             // objects overall
  uint64_t nstat_global_procdetails_max; // max number of procdetails overall
  uint64_t
      nstat_global_procdetails_allocs; // total number of procdetails allocated
  ...
  uint64_t
      nstat_global_idlecheck_route_src_gone; // total number of route sources
                                             // discovered "gone" in idle check

  // Extra details for sock locus lifecycle
  uint64_t
      nstat_global_tcp_sck_locus_stop_using; // Socket has WNT_STOPUSING when
                                             // creating the initial locus
  uint64_t
      nstat_global_udp_sck_locus_stop_using; // Socket has WNT_STOPUSING when
                                             // creating the initial locus
  uint64_t
      nstat_global_pcb_detach_with_locus; // Expected path, locus on pcb_detach
  uint64_t
      nstat_global_pcb_detach_with_src; // Expected path, locus on pcb_detach,
                                        // an associated source being detached
  uint64_t nstat_global_pcb_detach_without_locus; // Unexpected path, no locus
                                                  // on pcb_detach
  ...
  uint64_t nstat_global_sck_update_last_owner; // nstat_pcb_update_last_owner()
                                               // was called
  uint64_t
      nstat_global_sck_fail_first_owner; // can't set name on sock locus create
  uint64_t nstat_global_sck_fail_last_owner; // nstat_pcb_update_last_owner()
                                             // was called, no name available
  uint64_t nstat_global_tcp_desc_new_name;   // Socket ownership discovered to
                                             // have changed
  uint64_t
      nstat_global_tcp_desc_fail_name; // Socket ownership discovered to have
                                       // changed, fail to get new name
  uint64_t nstat_global_udp_desc_new_name; // Socket ownership discovered to
                                           // have changed
  uint64_t
      nstat_global_udp_desc_fail_name; // Socket ownership discovered to have
                                       // changed, fail to get new name

  // The following are expected to be removed as and when the socket handling
  // code is refined
...
//
// The "net.stats.metrics" systctl can request these metrics either from a
// specific client, the accumulated counts for closed clients, or a summary of
// all the closed and current clients. To collect individual metrics for all
// clients, an initial request is made targeting NSTAT_METRIC_ID_MAX via the
// mr_id field in the request structure.  The returned metrics will be for the
// client with the highest identifer, as returned in the nstat_client_id field.
// The next request should target that returned identifier minus one, which will
// collect the client with the next highest identifier.  This sequence can
// continue until metrics for all clients have been collected
#define NSTAT_METRIC_VERSION 2
#define NSTAT_METRIC_ID_ACCUMULATED                                            \
  0x0 /* Accumulation from all clients that have previously closed */
#define NSTAT_METRIC_ID_GRAND_TOTAL                                            \
  0x1 /* Accumulation from all clients, current and historic */
#define NSTAT_METRIC_ID_MAX                                                    \
  0xffffffff /* Start scanning all clients with this initial value */

struct nstat_metrics_req {
  uint32_t mr_version; // The version of metrics being requested
  uint32_t mr_id; // Identifier for the metrics, a client id, or accumulated or
                  // grand total
};

struct nstat_client_details {
  uint32_t nstat_client_id; // Identifier for this set of metrics, a client id,
                            // or accumulated
  ...
  uint32_t nstat_client_added_src; // Bitmap of providers with individually
                                   // added sources
};
...
struct nstat_metrics {
  ...
  uint32_t
      nstat_first_uint32_count; // Subsequent fields must be uint32_t values
                                // that, if kept per-client, should simply added
                                // to the global counts when the client exit

  // Tracking client requests
  uint32_t nstat_query_request_all; // Client requests for all counts
  uint32_t
      nstat_query_request_one; // Client request for counts on a single source
  uint32_t nstat_query_description_all; // Client requests for all descriptors
  uint32_t nstat_query_description_one; // Client requests for descriptor on a
                                        // single source
  uint32_t nstat_query_update_all;      // Client requests for all updates
  uint32_t
      nstat_query_update_one; // Client requests for update on a single source
  uint32_t nstat_remove_src_found; // Client request to remove a source which is
                                   // still in existence
  uint32_t nstat_remove_src_missed; // Client request to remove a source which
                                    // is no longer there

  // Details for nstat_query_request all/one
  uint32_t nstat_query_request_nobuf;     // No buffers for message send
  uint32_t nstat_query_request_upgrade;   // Successful lock upgrade to handle
                                          // "gone" source
  uint32_t nstat_query_request_noupgrade; // Unsuccessful lock upgrade to handle
                                          // "gone" source
  uint32_t
      nstat_query_request_nodesc; // Can't send a descriptor for "gone" source
  uint32_t nstat_query_request_yield; // Client yields lock due to possibly
  ...
  uint32_t nstat_query_description_yield; // Client yields lock due to possibly
                                          // higher priority processing
  ...
  uint32_t nstat_query_details_upgrade;   // Successful lock upgrade to handle
                                          // "gone" source
  uint32_t nstat_query_details_noupgrade; // Unsuccessful lock upgrade to handle
                                          // "gone" source
  uint32_t nstat_query_details_yield;     // Client yields lock due to possibly
                                          // higher priority processing
  ...
  uint32_t nstat_query_update_upgrade;   // Successful lock upgrade to handle
                                         // "gone" source
  uint32_t nstat_query_update_noupgrade; // Unsuccessful lock upgrade to handle
                                         // "gone" source
  uint32_t
      nstat_query_update_nodesc; // Can't send a descriptor for "gone" source
  uint32_t nstat_query_update_yield; // Client yields lock due to possibly
                                     // higher priority processing
  ...
  uint32_t
      nstat_src_add_no_src_mem;    // fail to get memory for nstat_src structure
  uint32_t nstat_src_add_send_err; // fail to send initial src-added
  uint32_t nstat_src_add_while_cleanup; // fail to add because client is in
                                        // clean up state
  ...
  uint32_t nstat_src_goodbye_successes; // Successful goodbyes (include cases
                                        // messages filtered out)
  uint32_t
      nstat_src_goodbye_failures; // Failed goodbyes, further qualified by..
  ...
  uint32_t nstat_src_goodbye_filtered_details; // Skipped trying to send a
                                               // details message
  ...
  uint32_t nstat_src_goodbye_filtered_update; // Skipped trying to send an
                                              // update message
  ...
  uint32_t nstat_src_goodbye_filtered_counts;  // Skipped trying to send both
                                               // counts and descriptor messages
  uint32_t nstat_src_goodbye_sent_description; // Sent a concluding description
                                               // message
  uint32_t nstat_src_goodbye_failed_description; // Failed to send a description
                                                 // message
  ...
  uint32_t nstat_src_goodbye_filtered_removed; // Skipped on sending a removed
                                               // message

  uint32_t nstat_pcb_event;  // send pcb event code called, one precursor to the
                             // send_event metrics
  uint32_t nstat_send_event; // send event successful
  uint32_t nstat_send_event_fail; // send event fail, likely lack of buffers
  uint32_t
      nstat_send_event_notsup; // send event not supported, old style client

  uint32_t nstat_route_src_gone_idlecheck; // route src gone noted during
                                           // periodic idle check
  uint32_t
      nstat_src_removed_linkage; // removed src linkages on the way to deletion

  uint32_t nstat_src_gone_idlecheck; // Expected to be redundant/removed when
                                     // socket handling code is refined

  uint32_t nstat_last_uint32_count; // Must be the last uint32_t count in the
                                    // structure
...
struct nstat_progress_indicators {
  ...
  u_int32_t np_recentflows_unacked; /* Total of "recent" flows with
                                       unacknowledged data */
  u_int64_t np_recentflows_rxbytes; /* Total of "recent" flows received bytes */
  u_int64_t
      np_recentflows_txbytes; /* Total of "recent" flows transmitted bytes */
  u_int64_t np_recentflows_rxooo; /* Total of "recent" flows received out of
                                     order bytes */
  u_int64_t np_recentflows_rxdup; /* Total of "recent" flows received duplicate
                                     bytes */
  u_int64_t
      np_recentflows_retx; /* Total of "recent" flows retransmitted bytes */
...
struct nstat_progress_req {
  ...
  u_int64_t np_recentflow_maxduration; /* In mach_absolute_time, max duration
                                          for flow to be counted as "recent" */
  u_int64_t np_filter_flags; /* Optional additional filtering, values are
                                interface properties per ntstat.h */
  u_int64_t np_transport_protocol_mask; /* Transport protocol (currently
...
typedef struct nstat_sysinfo_tcp_stats {
  ...
  u_int32_t
      ecn_client_setup; /* Attempts to setup TCP client connection with ECN */
  u_int32_t
      ecn_server_setup; /* Attempts to setup TCP server connection with ECN */
  u_int32_t ecn_client_success; /* Number of successful negotiations of ECN for
                                   a client connection */
  u_int32_t ecn_server_success; /* Number of successful negotiations of ECN for
                                   a server connection */
  u_int32_t ecn_not_supported; /* Number of falbacks to Non-ECN, no support from
                                  peer */
  ...
  u_int32_t ecn_conn_recv_ce;  /* Number of connections using ECN received CE at
                                  least once */
  u_int32_t ecn_conn_recv_ece; /* Number of connections using ECN received ECE
                                  at least once */
  u_int32_t ecn_conn_plnoce; /* Number of connections using ECN seen packet loss
                                but never received CE */
  u_int32_t ecn_conn_pl_ce;  /* Number of connections using ECN seen packet loss
                                and CE */
  u_int32_t ecn_conn_nopl_ce; /* Number of connections using ECN with no packet
                                 loss but received CE */
  u_int32_t ecn_fallback_synloss; /* Number of times we did fall back due to
                                     SYN-Loss */
  u_int32_t ecn_fallback_reorder; /* Number of times we fallback because we
                                     detected the PAWS-issue */
  u_int32_t ecn_fallback_ce; /* Number of times we fallback because we received
                                too many CEs */
  u_int32_t
      tfo_syn_data_rcv; /* Number of SYN+data received with valid cookie */
  ...
  u_int32_t tfo_syn_data_acked; /* Number of times our SYN+data has been
                                   acknowledged */
  u_int32_t
      tfo_syn_loss; /* Number of times SYN+TFO has been lost and we fallback */
  u_int32_t
      tfo_blackhole; /* Number of times SYN+TFO has been lost and we fallback */
  ...
  u_int32_t mptcp_handover_attempt;    /* Total number of MPTCP-attempts using
                                          handover mode */
  u_int32_t mptcp_interactive_attempt; /* Total number of MPTCP-attempts using
                                          interactive mode */
  u_int32_t mptcp_aggregate_attempt;   /* Total number of MPTCP-attempts using
                                          aggregate mode */
  u_int32_t mptcp_fp_handover_attempt; /* Same as previous three but only for
                                          first-party apps */
  u_int32_t mptcp_fp_interactive_attempt;
  u_int32_t mptcp_fp_aggregate_attempt;
  u_int32_t mptcp_heuristic_fallback; /* Total number of MPTCP-connections that
                                         fell back due to heuristics */
  u_int32_t mptcp_fp_heuristic_fallback; /* Same as previous but for first-party
                                            apps */
  u_int32_t
      mptcp_handover_success_wifi; /* Total number of successfull handover-mode
                                      connections that *started* on WiFi */
  u_int32_t
      mptcp_handover_success_cell; /* Total number of successfull handover-mode
                                      connections that *started* on Cell */
  u_int32_t mptcp_interactive_success; /* Total number of interactive-mode
                                          connections that negotiated MPTCP */
  u_int32_t mptcp_aggregate_success;   /* Same as previous but for aggregate */
  u_int32_t mptcp_fp_handover_success_wifi; /* Same as previous four, but for
                                               first-party apps */
  u_int32_t mptcp_fp_handover_success_cell;
  u_int32_t mptcp_fp_interactive_success;
  u_int32_t mptcp_fp_aggregate_success;
  u_int32_t mptcp_handover_cell_from_wifi; /* Total number of connections that
                                              use cell in handover-mode (coming
                                              from WiFi) */
  u_int32_t mptcp_handover_wifi_from_cell; /* Total number of connections that
                                              use WiFi in handover-mode (coming
                                              from cell) */
  u_int32_t mptcp_interactive_cell_from_wifi; /* Total number of connections
                                                 that use cell in interactive
                                                 mode (coming from WiFi) */
  u_int32_t
      mptcp_back_to_wifi; /* Total number of connections that succeed to move
                             traffic away from cell (when starting on cell) */
  u_int64_t mptcp_handover_cell_bytes; /* Total number of bytes sent on cell in
                                          handover-mode (on new subflows,
                                          ignoring initial one) */
  u_int64_t
      mptcp_interactive_cell_bytes; /* Same as previous but for interactive */
  u_int64_t mptcp_aggregate_cell_bytes;
  u_int64_t
      mptcp_handover_all_bytes; /* Total number of bytes sent in handover */
  u_int64_t mptcp_interactive_all_bytes;
  u_int64_t mptcp_aggregate_all_bytes;
  u_int32_t mptcp_wifi_proxy; /* Total number of new subflows that fell back to
                                 regular TCP on cell */
  u_int32_t mptcp_cell_proxy; /* Total number of new subflows that fell back to
                                 regular TCP on WiFi */
  u_int32_t mptcp_triggered_cell; /* Total number of times an MPTCP-connection
                                     triggered cell bringup */
  u_int32_t _padding;
  /* When adding/removing here, also adjust NSTAT_SYSINFO_TCP_STATS_COUNT */
} nstat_sysinfo_tcp_stats;
#define NSTAT_SYSINFO_TCP_STATS_COUNT 71

enum { NSTAT_IFNET_ECN_PROTO_IPV4 = 1, NSTAT_IFNET_ECN_PROTO_IPV6 };
...
#define NSTAT_NET_API_STAT_KEYVAL_COUNT                                        \
  (NSTAT_SYSINFO_API_LAST - NSTAT_SYSINFO_API_FIRST + 1)
...
enum { NSTAT_TX_FLAG_RETRANSMIT = 1 };

enum { NSTAT_RX_FLAG_DUPLICATE = 1, NSTAT_RX_FLAG_OUT_OF_ORDER = 2 };
...
void nstat_route_tx(struct rtentry *rte, u_int32_t packets, u_int32_t bytes,
                    u_int32_t flags);
void nstat_route_rx(struct rtentry *rte, u_int32_t packets, u_int32_t bytes,
                    u_int32_t flags);
void nstat_route_rtt(struct rtentry *rte, u_int32_t rtt, u_int32_t rtt_var);
void nstat_route_update(struct rtentry *rte, uint32_t connect_attempts,
                        uint32_t connect_successes, uint32_t rx_packets,
                        uint32_t rx_bytes, uint32_t rx_duplicatebytes,
                        uint32_t rx_outoforderbytes, uint32_t tx_packets,
                        uint32_t tx_bytes, uint32_t tx_retransmit, uint32_t rtt,
                        uint32_t rtt_var);
...
typedef bool(userland_stats_request_vals_fn)(
...
// Netstats can also request "extension" items, specified by the
// allowed_extensions flag The return value is the amount of space currently
// required for the extension
typedef size_t(userland_stats_request_extension_fn)(
    userland_stats_provider_context *ctx,
    int requested_extension, /* The extension to be returned */
    void *__sized_by(buf_size)
        buf, /* If not NULL, the address for the extension to be returned in */
    size_t buf_size); /* The size of the buffer space, typically matching the
                         return from a previous call with null buffer pointer */

// Things get started with a call to netstats to say that there’s a new
// connection:
nstat_userland_context ntstat_userland_stats_open(
...
void ntstat_userland_stats_event(nstat_userland_context nstat_ctx,
                                 uint64_t event);
...
int nstat_userland_get_snapshot(short proto,
                                void *__sized_by(*snapshot_size) * snapshotp,
                                size_t *snapshot_size, int *countp);
int nstat_userland_list_snapshot(short proto, struct sysctl_req *req,
                                 void *__sized_by(nuserland *
                                                  sizeof(nstat_flow_data))
                                     userlandsnapshot,
                                 int nuserland);
...
typedef void
    *nstat_provider_context; /* This is quoted to the external provider */
typedef void *nstat_context; /* This is quoted by the external provider when
                                calling nstat */

// After nstat_provider_stats_open() has been called (and potentially while the
// open is still executing), netstats can request a refresh of its data The
// various return pointer parameters may be null if the item is not required The
// return code is true for success
typedef bool(nstat_provider_request_vals_fn)(
    nstat_provider_context ctx,
    u_int32_t
        *ifflagsp, /* Flags for being on cell/wifi etc, used for filtering */
...
// allowed_extensions flag The return value is the amount of space currently
// required for the extension
typedef size_t(nstat_provider_request_extensions_fn)(
    nstat_provider_context ctx,
    int requested_extension, /* The extension to be returned */
    void *__sized_by(buf_size)
        buf, /* If not NULL, the address for the extension to be returned in */
...
// Things get started with a call to netstats to say that there’s a new item to
// become a netstats source
nstat_context nstat_provider_stats_open(
    nstat_provider_context ctx, int provider_id,
    u_int64_t properties, /* The bottom 32 bits can be used as per the interface
                             / connection flags ifflagsp */
    nstat_provider_request_vals_fn req_fn,
    nstat_provider_request_extensions_fn req_extensions_fn);

// Note that when the source is closed, netstats will make one last call on the
// request functions to retrieve final values
...
#if defined(__LP64__)
#define locked_add_64(__addr, __count)                                         \
  do {                                                                         \
    *(__addr) += (__count);                                                    \
  } while (0)
#else
