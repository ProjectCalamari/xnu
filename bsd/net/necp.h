 ...
 * Session actions
 */
#define NECP_SESSION_ACTION_POLICY_ADD                                         \
  1 // In: Policy TLVs				Out: necp_policy_id
#define NECP_SESSION_ACTION_POLICY_GET                                         \
  2 // In: necp_policy_id			Out: Policy TLVs
#define NECP_SESSION_ACTION_POLICY_DELETE                                      \
  3 // In: necp_policy_id			Out: None
#define NECP_SESSION_ACTION_POLICY_APPLY_ALL                                   \
  4 // In: None						Out: None
#define NECP_SESSION_ACTION_POLICY_LIST_ALL                                    \
  5 // In: None						Out: TLVs of IDs
#define NECP_SESSION_ACTION_POLICY_DELETE_ALL                                  \
  6 // In: None						Out: None
#define NECP_SESSION_ACTION_SET_SESSION_PRIORITY                               \
  7 // In: necp_session_priority	Out: None
#define NECP_SESSION_ACTION_LOCK_SESSION_TO_PROC                               \
  8 // In: None						Out: None
#define NECP_SESSION_ACTION_REGISTER_SERVICE                                   \
  9 // In: uuid_t					Out: None
#define NECP_SESSION_ACTION_UNREGISTER_SERVICE                                 \
  10 // In: uuid_t					Out: None
#define NECP_SESSION_ACTION_POLICY_DUMP_ALL                                    \
  11 // In: None						Out: uint32_t
     // bytes length, then Policy TLVs
#define NECP_SESSION_ACTION_ADD_DOMAIN_FILTER                                  \
  12 // In: struct net_bloom_filter  Out: uint32_t, ID
#define NECP_SESSION_ACTION_REMOVE_DOMAIN_FILTER                               \
  13 // In: uint32_t, ID             Out: None
#define NECP_SESSION_ACTION_REMOVE_ALL_DOMAIN_FILTERS                          \
  14 // In: None                     Out: None
#define NECP_SESSION_ACTION_ADD_DOMAIN_TRIE                                    \
  15 // In: struct necp_domain_trie_request  Out: uint32_t, ID
#define NECP_SESSION_ACTION_REMOVE_DOMAIN_TRIE                                 \
  16 // In: uint32_t, ID             Out: None
#define NECP_SESSION_ACTION_REMOVE_ALL_DOMAIN_TRIES                            \
  17 // In: None                     Out: None
#define NECP_SESSION_ACTION_TRIE_DUMP_ALL                                      \
  18 // In: None                     Out: uint8_t, count, then array of struct
     // necp_domain_trie_request with no 'data'

/*
 * Control message flags
 */
#define NECP_PACKET_FLAGS_RESPONSE                                             \
...
#define NECP_POLICY_CONDITION_FLAGS_EXACT                                      \
  0x02 // For conditions that would otherwise have more inclusive matching,
 ...
 */
#define NECP_POLICY_CONDITION_DEFAULT                                          \
  0 // N/A, not valid with any other conditions
// Socket/Application conditions
#define NECP_POLICY_CONDITION_APPLICATION                                      \
  1 // uuid_t, uses effective UUID when possible
#define NECP_POLICY_CONDITION_REAL_APPLICATION                                 \
  2 // uuid_t, never uses effective UUID. Only valid with
...
#define NECP_POLICY_CONDITION_LOCAL_ADDR_RANGE                                 \
  14 // necp_policy_condition_addr_range
#define NECP_POLICY_CONDITION_REMOTE_ADDR_RANGE                                \
  15 // necp_policy_condition_addr_range
#define NECP_POLICY_CONDITION_AGENT_TYPE                                       \
  16 // struct necp_policy_condition_agent_type
...
#define NECP_POLICY_CONDITION_FLOW_LOCAL_ADDR_RANGE                            \
  22 // necp_policy_condition_addr_range
#define NECP_POLICY_CONDITION_FLOW_REMOTE_ADDR_RANGE                           \
  23 // necp_policy_condition_addr_range
#define NECP_POLICY_CONDITION_FLOW_IS_LOOPBACK 31 // N/A
// Socket/Application conditions, continued
#define NECP_POLICY_CONDITION_CLIENT_FLAGS                                     \
  24 // u_int32_t, values from NECP_CLIENT_PARAMETER_FLAG_*
...
#define NECP_POLICY_CONDITION_SDK_VERSION                                      \
  28 // struct necp_policy_condition_sdk_version
...
#define NECP_POLICY_CONDITION_DOMAIN_OWNER                                     \
  33 // String, owner of domain (for example, "Google Inc")
#define NECP_POLICY_CONDITION_DOMAIN_CONTEXT                                   \
  34 // String, eTLD+1 leading to networking activities to the domain
#define NECP_POLICY_CONDITION_TRACKER_DOMAIN 35 // String, tracker domain
#define NECP_POLICY_CONDITION_ATTRIBUTED_BUNDLE_IDENTIFIER                     \
  36 // String, app to which traffic is attributed to
#define NECP_POLICY_CONDITION_SCHEME_PORT                                      \
  37 // u_int16_t, the port associated with the scheme for a connection
...
#define NECP_POLICY_CONDITION_BOUND_INTERFACE_FLAGS                            \
  43 // Interface flags: u_int32_t flags, u_int32_t eflags, u_int32_t xflags
...
#define NECP_POLICY_RESULT_SKIP                                                \
  2 // u_int32_t, policy order to skip to. 0 to skip all session policies.
#define NECP_POLICY_RESULT_DROP 3 // N/A
#define NECP_POLICY_RESULT_SOCKET_DIVERT                                       \
  4 // u_int32_t, flow divert control unit
...
#define NECP_POLICY_RESULT_SCOPED_DIRECT                                       \
  16 // N/A, scopes to primary physical interface
#define NECP_POLICY_RESULT_ALLOW_UNENTITLED 17 // N/A
#define NECP_POLICY_RESULT_REMOVE_NETAGENT 18  // netagent uuid_t
#define NECP_POLICY_RESULT_REMOVE_NETAGENT_TYPE                                \
  19 // necp_policy_condition_agent_type
 ...
 */
#define NECP_POLICY_LOCAL_NETWORKS_FLAG_INCLUDE_LOCAL_ADDRESSES                \
  0x01 // Include addresses on local interfaces for Local-Networks condition
...
#define NECP_ROUTE_RULE_DIVERT_SOCKET                                          \
  7 // u_int32_t control unit, followed by string or empty
#define NECP_ROUTE_RULE_DENY_INTERFACE_WITH_TYPE                               \
  8 // u_int32_t effective type, followed by string or empty
...
#define NECP_ROUTE_RULE_FLAG_ULTRA_CONSTRAINED                                 \
  0x90 // Note that this includes the 0x80 bit, so cannot be combined with agent
       // UUID matching.

#define NECP_ROUTE_RULE_FLAG_NETAGENT                                          \
  0x80 // Last bit, reserved to mark that this applies only when an agent UUID
       // is present
...
#define NECP_MASK_PRESERVE_CONNECTIONS                                         \
  0x20000000 // on filter_control_unit value
...
struct necp_aggregate_result {
  ...
  struct necp_policy_condition_agent_type
      remove_netagent_types[NECP_MAX_REMOVE_NETAGENT_TYPES];
 ...
 * but they get entangled with #defines for v4 etc in pfvar.h and it may be
 * better practice to have separate definitions here.
 */
...
// From the necp client parameters: local and remote addresses, euuid, traffic
// class, ifindex
//
...
#define NECP_CLIENT_STATISTICS_TYPE_TCP_VER_1                                  \
  1 // Currently supported version for TCP
#define NECP_CLIENT_STATISTICS_TYPE_UDP_VER_1                                  \
  1 // Currently supported version for UDP
#define NECP_CLIENT_STATISTICS_TYPE_QUIC_VER_1                                 \
  1 // Currently supported version for QUIC

#define NECP_CLIENT_STATISTICS_TYPE_TCP_CURRENT_VER                            \
  NECP_CLIENT_STATISTICS_TYPE_TCP_VER_1
#define NECP_CLIENT_STATISTICS_TYPE_UDP_CURRENT_VER                            \
  NECP_CLIENT_STATISTICS_TYPE_UDP_VER_1
#define NECP_CLIENT_STATISTICS_TYPE_QUIC_CURRENT_VER                           \
  NECP_CLIENT_STATISTICS_TYPE_QUIC_VER_1

#define NECP_CLIENT_STATISTICS_EVENT_INIT 0x00000000 // Register the flow
#define NECP_CLIENT_STATISTICS_EVENT_TIME_WAIT                                 \
  0x00000001 // The flow is effectively finished but waiting on timer
...
struct necp_extra_quic_metadata {
  ...
  u_int32_t cc_alg_index;
  u_int32_t state;
  u_int32_t fallback : 1, unused : 31;
...
#define NECP_CLIENT_STATISTICS_BUFREQ_ID                                       \
  0xbf // Distinguishes from statistics actions taking a necp_all_stats struct

// There is a limit to the number of statistics structures that may be allocated
// per process, subject to change
//
...
typedef struct necp_tcp_tfo_cache {
  ...
  u_int8_t necp_tcp_tfo_heuristics_success
  ...
  u_int8_t necp_tcp_tfo_heuristics_success_req
  ...
  u_int8_t necp_tcp_tfo_heuristics_rst_data
      : 1; // Recevied RST upon SYN with data in the SYN
  u_int8_t necp_tcp_tfo_heuristics_rst_req
...
#define NECP_CLIENT_CACHE_TYPE_ECN_VER_1                                       \
  1 // Currently supported version for ECN
#define NECP_CLIENT_CACHE_TYPE_TFO_VER_1                                       \
  1 // Currently supported version for TFO
...
#define NECP_OPEN_FLAG_OBSERVER                                                \
  0x01 // Observers can query clients they don't own
#define NECP_OPEN_FLAG_BACKGROUND 0x02 // Mark this fd as backgrounded
#define NECP_OPEN_FLAG_PUSH_OBSERVER                                           \
  0x04 // When used with the OBSERVER flag, allows updates to be pushed. Adding
       // clients is not allowed in this mode.

#define NECP_FD_SUPPORTS_GUARD 1

#define NECP_CLIENT_ACTION_ADD                                                 \
  1 // Register a new client. Input: parameters in buffer; Output: client_id
#define NECP_CLIENT_ACTION_REMOVE                                              \
  2 // Unregister a client. Input: client_id, optional struct
    // ifnet_stats_per_flow
#define NECP_CLIENT_ACTION_COPY_PARAMETERS                                     \
  3 // Copy client parameters. Input: client_id; Output: parameters in buffer
#define NECP_CLIENT_ACTION_COPY_RESULT                                         \
  4 // Copy client result. Input: client_id; Output: result in buffer
#define NECP_CLIENT_ACTION_COPY_LIST                                           \
  5 // Copy all client IDs. Output: struct necp_client_list in buffer
#define NECP_CLIENT_ACTION_REQUEST_NEXUS_INSTANCE                              \
  6 // Request a nexus instance from a nexus provider, optional struct
    // necp_stats_bufreq
#define NECP_CLIENT_ACTION_AGENT                                               \
  7 // Interact with agent. Input: client_id, agent parameters
#define NECP_CLIENT_ACTION_COPY_AGENT                                          \
  8 // Copy agent content. Input: agent UUID; Output: struct netagent
#define NECP_CLIENT_ACTION_COPY_INTERFACE                                      \
  9 // Copy interface details. Input: ifindex cast to UUID; Output: struct
    // necp_interface_details
#define NECP_CLIENT_ACTION_SET_STATISTICS 10 // Deprecated
#define NECP_CLIENT_ACTION_COPY_ROUTE_STATISTICS                               \
  11 // Get route statistics. Input: client_id; Output: struct necp_stat_counts
#define NECP_CLIENT_ACTION_AGENT_USE                                           \
  12 // Return the use count and increment the use count. Input/Output: struct
     // necp_agent_use_parameters
#define NECP_CLIENT_ACTION_MAP_SYSCTLS                                         \
  13 // Get the read-only sysctls memory location. Output: mach_vm_address_t
#define NECP_CLIENT_ACTION_UPDATE_CACHE 14 // Update heuristics and cache
#define NECP_CLIENT_ACTION_COPY_CLIENT_UPDATE                                  \
  15 // Fetch an updated client for push-mode observer. Output: Client id,
     // struct necp_client_observer_update in buffer
#define NECP_CLIENT_ACTION_COPY_UPDATED_RESULT                                 \
  16 // Copy client result only if changed. Input: client_id; Output: result in
     // buffer
#define NECP_CLIENT_ACTION_ADD_FLOW                                            \
  17 // Add a flow. Input: client_id; Output: struct necp_client_add_flow
#define NECP_CLIENT_ACTION_REMOVE_FLOW                                         \
  18 // Remove a flow. Input: flow_id, optional struct ifnet_stats_per_flow
#define NECP_CLIENT_ACTION_CLAIM                                               \
  19 // Claim a client that has been added for this unique PID. Input: client_id
#define NECP_CLIENT_ACTION_SIGN                                                \
  20 // Sign a query answer. Input: struct necp_client_signable; Output: struct
     // necp_client_signature
#define NECP_CLIENT_ACTION_GET_INTERFACE_ADDRESS                               \
  21 // Get the best interface local address for given remote address. Input:
     // ifindex, remote sockaddr; Output: matching local sockaddr
#define NECP_CLIENT_ACTION_ACQUIRE_AGENT_TOKEN                                 \
  22 // Get a one-time use token from an agent. Input: agent UUID; Output: token
     // buffer
#define NECP_CLIENT_ACTION_VALIDATE                                            \
  23 // Validate a query answer. Input: struct necp_client_validatable; Output:
     // None
#define NECP_CLIENT_ACTION_GET_SIGNED_CLIENT_ID                                \
  24 // Get a client ID for the appliction along with a signature.
#define NECP_CLIENT_ACTION_SET_SIGNED_CLIENT_ID                                \
  25 // Set a client ID for the appliction along with a signature.
#define NECP_CLIENT_ACTION_COPY_UPDATED_RESULT_FINAL                           \
  26 // Copy client result only if changed, discard data if buffer is too small.
     // Input: client_id; Output: result in buffer
#define NECP_CLIENT_ACTION_GET_FLOW_STATISTICS                                 \
  27 // Get flow statistics for aop flow

#define NECP_CLIENT_PARAMETER_APPLICATION                                      \
  NECP_POLICY_CONDITION_APPLICATION // Requires entitlement
#define NECP_CLIENT_PARAMETER_REAL_APPLICATION                                 \
  NECP_POLICY_CONDITION_REAL_APPLICATION // Requires entitlement
...
#define NECP_CLIENT_PARAMETER_DOMAIN_CONTEXT                                   \
  NECP_POLICY_CONDITION_DOMAIN_CONTEXT
#define NECP_CLIENT_PARAMETER_TRACKER_DOMAIN                                   \
  NECP_POLICY_CONDITION_TRACKER_DOMAIN
#define NECP_CLIENT_PARAMETER_URL NECP_POLICY_CONDITION_URL
#define NECP_CLIENT_PARAMETER_ATTRIBUTED_BUNDLE_IDENTIFIER                     \
  NECP_POLICY_CONDITION_ATTRIBUTED_BUNDLE_IDENTIFIER
#define NECP_CLIENT_PARAMETER_ACCOUNT NECP_POLICY_CONDITION_ACCOUNT
#define NECP_CLIENT_PARAMETER_PID                                              \
  NECP_POLICY_CONDITION_PID // Requires entitlement
#define NECP_CLIENT_PARAMETER_UID                                              \
  NECP_POLICY_CONDITION_UID // Requires entitlement
#define NECP_CLIENT_PARAMETER_BOUND_INTERFACE                                  \
...
#define NECP_CLIENT_PARAMETER_APPLICATION_ID                                   \
  NECP_POLICY_CONDITION_APPLICATION_ID
...
#define NECP_CLIENT_PARAMETER_PROHIBIT_IF_TYPE                                 \
  101 // u_int8_t, see ifru_functional_type in <net/if.h>
#define NECP_CLIENT_PARAMETER_PROHIBIT_AGENT 102 // uuid_t, network agent UUID
#define NECP_CLIENT_PARAMETER_PROHIBIT_AGENT_TYPE                              \
  103 // struct necp_client_parameter_netagent_type

// "Require" will choose an interface with that property, or none if not found
#define NECP_CLIENT_PARAMETER_REQUIRE_IF_TYPE                                  \
  111 // u_int8_t, see ifru_functional_type in <net/if.h>
#define NECP_CLIENT_PARAMETER_REQUIRE_AGENT 112 // uuid_t, network agent UUID
#define NECP_CLIENT_PARAMETER_REQUIRE_AGENT_TYPE                               \
  113 // struct necp_client_parameter_netagent_type

// "Prefer" will choose an interface with an agent, or best otherwise if not
// found
#define NECP_CLIENT_PARAMETER_PREFER_AGENT 122 // uuid_t, network agent UUID
#define NECP_CLIENT_PARAMETER_PREFER_AGENT_TYPE                                \
  123 // struct necp_client_parameter_netagent_type

// "Avoid" will choose an interface without an agent, or best otherwise if
// unavoidable
#define NECP_CLIENT_PARAMETER_AVOID_AGENT 124 // uuid_t, network agent UUID
#define NECP_CLIENT_PARAMETER_AVOID_AGENT_TYPE                                 \
  125 // struct necp_client_parameter_netagent_type
...
#define NECP_CLIENT_PARAMETER_AGENT_ADD_GROUP_MEMBERS                          \
  133 // struct necp_client_group_action
#define NECP_CLIENT_PARAMETER_AGENT_REMOVE_GROUP_MEMBERS                       \
  134 // struct necp_client_group_action
#define NECP_CLIENT_PARAMETER_REPORT_AGENT_ERROR 135 // int32_t

#define NECP_CLIENT_PARAMETER_FALLBACK_MODE                                    \
  140 // u_int8_t, see SO_FALLBACK_MODE_* values
...
#define NECP_CLIENT_PARAMETER_BROWSE_DESCRIPTOR                                \
  202                                          // struct necp_client_endpoint
#define NECP_CLIENT_PARAMETER_RESOLVER_TAG 203 // struct necp_client_validatable
#define NECP_CLIENT_PARAMETER_ADVERTISE_DESCRIPTOR                             \
  204 // struct necp_client_endpoint
#define NECP_CLIENT_PARAMETER_GROUP_DESCRIPTOR 205 // struct necp_client_group

#define NECP_CLIENT_PARAMETER_DELEGATED_UPID                                   \
  210 // u_int64_t, requires entitlement
...
#define NECP_CLIENT_PARAMETER_LOCAL_ADDRESS_PREFERENCE                         \
...
#define NECP_CLIENT_PARAMETER_FLAGS                                            \
  250 // u_int32_t, see NECP_CLIENT_PAREMETER_FLAG_* values
#define NECP_CLIENT_PARAMETER_FLOW_DEMUX_PATTERN                               \
  251 // struct necp_demux_pattern
#define NECP_CLIENT_PARAMETER_EXTENDED_FLAGS                                   \
  252 // u_int64_t, see NECP_CLIENT_PARAMETER_EXTENDED_FLAG_* values

#define NECP_CLIENT_PARAMETER_FLAG_MULTIPATH                                   \
  0x0001 // Get multipath interface results
#define NECP_CLIENT_PARAMETER_FLAG_BROWSE                                      \
  0x0002 // Agent assertions on nexuses are requests to browse
#define NECP_CLIENT_PARAMETER_FLAG_PROHIBIT_EXPENSIVE                          \
  0x0004 // Prohibit expensive interfaces
#define NECP_CLIENT_PARAMETER_FLAG_LISTENER                                    \
  0x0008 // Client is interested in listening for inbound connections
#define NECP_CLIENT_PARAMETER_FLAG_DISCRETIONARY                               \
  0x0010 // Client's traffic is discretionary, and eligible for early defuncting
#define NECP_CLIENT_PARAMETER_FLAG_ECN_ENABLE                                  \
  0x0020 // Client is requesting to enable ECN
#define NECP_CLIENT_PARAMETER_FLAG_ECN_DISABLE                                 \
  0x0040 // Client is requesting to disable ECN
#define NECP_CLIENT_PARAMETER_FLAG_TFO_ENABLE                                  \
  0x0080 // Client is requesting to enable TFO
#define NECP_CLIENT_PARAMETER_FLAG_ONLY_PRIMARY_REQUIRES_TYPE                  \
  0x0100 // Interpret NECP_CLIENT_PARAMETER_REQUIRE_IF_TYPE only for primary
         // interface, and allow exceptions for multipath or listeners
#define NECP_CLIENT_PARAMETER_FLAG_CUSTOM_ETHER                                \
  0x0200 // Client expects to open a custom ethernet channel
#define NECP_CLIENT_PARAMETER_FLAG_CUSTOM_IP                                   \
  0x0400 // Client expects to open a custom IP protocol channel
#define NECP_CLIENT_PARAMETER_FLAG_INTERPOSE                                   \
  0x0800 // Client expects to open an interpose filter channel
#define NECP_CLIENT_PARAMETER_FLAG_PROHIBIT_CONSTRAINED                        \
  0x1000 // Prohibit constrained interfaces
...
#define NECP_CLIENT_PARAMETER_FLAG_KNOWN_TRACKER                               \
  0x10000 // Flow is contacting a known tracker
#define NECP_CLIENT_PARAMETER_FLAG_UNSAFE_SOCKET_ACCESS                        \
  0x20000 // Client allows direct access to sockets
#define NECP_CLIENT_PARAMETER_FLAG_NON_APP_INITIATED                           \
  0x40000 // Networking activities not initiated by application
#define NECP_CLIENT_PARAMETER_FLAG_THIRD_PARTY_WEB_CONTENT                     \
  0x80000 // Third-party web content, not main load
#define NECP_CLIENT_PARAMETER_FLAG_SILENT                                      \
  0x100000 // Private browsing - do not log/track
#define NECP_CLIENT_PARAMETER_FLAG_APPROVED_APP_DOMAIN                         \
  0x200000 // Approved associated app domain; domain is "owned" by app
#define NECP_CLIENT_PARAMETER_FLAG_NO_WAKE_FROM_SLEEP                          \
  0x400000 // Don't wake from sleep on traffic for this client
#define NECP_CLIENT_PARAMETER_FLAG_REUSE_LOCAL                                 \
  0x800000 // Request support for local address/port reuse
#define NECP_CLIENT_PARAMETER_FLAG_ENHANCED_PRIVACY                            \
  0x1000000 // Attempt protocol upgrade or proxy usage for more privacy
#define NECP_CLIENT_PARAMETER_FLAG_WEB_SEARCH_CONTENT                          \
  0x2000000 // Web search traffic
#define NECP_CLIENT_PARAMETER_FLAG_ALLOW_ULTRA_CONSTRAINED                     \
  0x4000000 // Allow ultra-constrained interfaces
#define NECP_CLIENT_PARAMETER_FLAG_HAS_ACCOUNT_ID                              \
  0x8000000 // Client has provided an account identifier
#define NECP_CLIENT_PARAMETER_FLAG_PREFER_COMPANION                            \
  0x10000000 // Client prefers companion proxy for internet access
#define NECP_CLIENT_PARAMETER_FLAG_AVOID_COMPANION                             \
  0x20000000 // Client avoids companion proxy for internet access, potentially
             // falling back
#define NECP_CLIENT_PARAMETER_FLAG_REQUIRE_COMPANION                           \
  0x40000000 // Client requires connecting to companion
#define NECP_CLIENT_PARAMETER_FLAG_PROHIBIT_COMPANION                          \
  0x80000000 // Client prohibits connecting to companion

#define NECP_CLIENT_PARAMETER_EXTENDED_FLAG_AOP2_OFFLOAD                       \
  0x0000000000000001 // Flow is offloaded to AOP2
...
#define NECP_CLIENT_RESULT_FLAGS                                               \
  7 // u_int32_t, see NECP_CLIENT_RESULT_FLAG_* values
#define NECP_CLIENT_RESULT_INTERFACE 8 // struct necp_client_result_interface
#define NECP_CLIENT_RESULT_INTERFACE_OPTION                                    \
  9 // struct necp_client_interface_option
...
#define NECP_CLIENT_RESULT_INTERFACE_TIME_DELTA                                \
  17 // u_int32_t, seconds since interface up/down
#define NECP_CLIENT_RESULT_REASON                                              \
  18 // u_int32_t, see NECP_CLIENT_RESULT_REASON_* values
...
#define NECP_CLIENT_RESULT_DISCOVERED_ENDPOINT                                 \
  202 // struct necp_client_endpoint, result of browse
#define NECP_CLIENT_RESULT_RESOLVED_ENDPOINT                                   \
  203 // struct necp_client_endpoint, result of resolve
...
#define NECP_CLIENT_RESULT_TRAFFIC_MGMT_BG                                     \
  211 // u_int32_t, 1: background, 0: not background
...
#define NECP_CLIENT_RESULT_NAT64                                               \
  214 // struct ipv6_prefix[NAT64_MAX_NUM_PREFIXES]
#define NECP_CLIENT_RESULT_ESTIMATED_THROUGHPUT                                \
  215 // struct necp_client_result_estimated_throughput
#define NECP_CLIENT_RESULT_AGENT_ERROR                                         \
  216 // struct necp_client_result_agent_error
...
#define NECP_CLIENT_RESULT_FLAG_IS_DIRECT                                      \
  0x0002 // Routes to directly accessible peer
...
#define NECP_CLIENT_RESULT_FLAG_FLOW_ASSIGNED                                  \
  0x0040 // Assigned, the flow is active
#define NECP_CLIENT_RESULT_FLAG_FLOW_VIABLE                                    \
  0x0080 // Viable, the flow has a valid route
#define NECP_CLIENT_RESULT_FLAG_PROBE_CONNECTIVITY                             \
  0x0100 // Flow should probe connectivity
#define NECP_CLIENT_RESULT_FLAG_ECN_ENABLED 0x0200 // ECN should be used
#define NECP_CLIENT_RESULT_FLAG_FAST_OPEN_BLOCKED                              \
  0x0400 // Fast open should not be used
#define NECP_CLIENT_RESULT_FLAG_LINK_QUALITY_ABORT                             \
  0x0800 // Link quality is very bad, recommend close connections
#define NECP_CLIENT_RESULT_FLAG_ALLOW_QOS_MARKING                              \
  0x1000                                         // QoS marking is allowed
#define NECP_CLIENT_RESULT_FLAG_HAS_NAT64 0x2000 // Has NAT64 prefix
#define NECP_CLIENT_RESULT_FLAG_INTERFACE_LOW_POWER                            \
  0x4000 // Interface is in low-power mode
#define NECP_CLIENT_RESULT_FLAG_SPECIFIC_LISTENER                              \
  0x8000 // Listener should not listen on all interfaces
#define NECP_CLIENT_RESULT_FLAG_KEXT_FILTER_PRESENT                            \
  0x10000 // Kernel extension filter present
#define NECP_CLIENT_RESULT_FLAG_PF_RULES_PRESENT                               \
  0x20000 // Firewall rules present
#define NECP_CLIENT_RESULT_FLAG_ALF_PRESENT                                    \
  0x40000 // Application Firewall enabled
#define NECP_CLIENT_RESULT_FLAG_PARENTAL_CONTROLS_PRESENT                      \
  0x80000 // Parental Controls present
#define NECP_CLIENT_RESULT_FLAG_IS_GLOBAL_INTERNET                             \
  0x100000 // Routes to global Internet
#define NECP_CLIENT_RESULT_FLAG_LINK_HEURISTICS                                \
  0x200000 // Link heuristics enabled

#define NECP_CLIENT_RESULT_FLAG_FORCE_UPDATE                                   \
  (NECP_CLIENT_RESULT_FLAG_HAS_IPV4 | NECP_CLIENT_RESULT_FLAG_HAS_IPV6 |       \
   NECP_CLIENT_RESULT_FLAG_HAS_NAT64 |                                         \
   NECP_CLIENT_RESULT_FLAG_INTERFACE_LOW_POWER |                               \
   NECP_CLIENT_RESULT_FLAG_LINK_HEURISTICS)
...
#define NECP_CLIENT_RESULT_FAST_OPEN_RCV_PROBE                                 \
  0x02 // DEPRECATED - Fast open receive probe
...
#define NECP_CLIENT_RESULT_REASON_EXPENSIVE_PROHIBITED                         \
  1 // Expensive networks were prohibited
#define NECP_CLIENT_RESULT_REASON_CONSTRAINED_PROHIBITED                       \
  2 // Constrained networks were prohibited
#define NECP_CLIENT_RESULT_REASON_CELLULAR_DENIED                              \
  3 // Denied by a cellular route rule
#define NECP_CLIENT_RESULT_REASON_WIFI_DENIED 4 // Denied by a wifi route rule
#define NECP_CLIENT_RESULT_REASON_LOCAL_NETWORK_PROHIBITED                     \
  5 // Local network access prohibited
#define NECP_CLIENT_RESULT_REASON_ULTRA_CONSTRAINED_NOT_ALLOWED                \
  6 // Ultra constrained networks not allowed
...
#define NECP_CLIENT_FLOW_FLAGS_ALLOW_NEXUS                                     \
  0x01 // Request a nexus instance upon adding a flow
#define NECP_CLIENT_FLOW_FLAGS_USE_CLIENT_ID                                   \
  0x02 // Register the client ID rather than the flow registration ID with
       // network agents
#define NECP_CLIENT_FLOW_FLAGS_BROWSE 0x04 // Create request with a browse agent
#define NECP_CLIENT_FLOW_FLAGS_RESOLVE                                         \
  0x08 // Create request with a resolution agent
#define NECP_CLIENT_FLOW_FLAGS_OVERRIDE_ADDRESS                                \
  0x10 // Flow has a different remote address than the parent flow
#define NECP_CLIENT_FLOW_FLAGS_OVERRIDE_IP_PROTOCOL                            \
  0x20 // Flow has a different IP protocol than the parent flow
#define NECP_CLIENT_FLOW_FLAGS_OPEN_FLOW_ON_BEHALF_OF_CLIENT                   \
  0x40 // Flow is opened on behalf of a client
...
#define NECP_CLIENT_SIGN_TYPE_RESOLVER_ANSWER                                  \
  1 // struct necp_client_host_resolver_answer
#define NECP_CLIENT_SIGN_TYPE_BROWSE_RESULT                                    \
  2 // struct necp_client_browse_result
#define NECP_CLIENT_SIGN_TYPE_SERVICE_RESOLVER_ANSWER                          \
  3 // struct necp_client_service_resolver_answer
...
#define NECP_CLIENT_SIGN_TYPE_SYSTEM_RESOLVER_ANSWER                           \
  4 // struct necp_client_host_resolver_answer
#define NECP_CLIENT_SIGN_TYPE_SYSTEM_BROWSE_RESULT                             \
  5 // struct necp_client_browse_result
#define NECP_CLIENT_SIGN_TYPE_SYSTEM_SERVICE_RESOLVER_ANSWER                   \
  6 // struct necp_client_service_resolver_answer
...
struct necp_client_host_resolver_answer {
  ...
  u_int32_t metadata_hash; // Extra metadata that can be signed along with the
                           // answer, such as a network signature.
  union sockaddr_in_4_6 address_answer; // Can include port
  u_int32_t hostname_length;
  char hostname[0];
} __attribute__((__packed__));
...
struct necp_client_browse_result {
  struct necp_client_signable header;
  u_int32_t metadata_hash; // Extra metadata that can be signed along with the
                           // service, which may include a TXT record or a
                           // network signature
...
#define NECP_FILTER_UNIT_NO_FILTER                                             \
  UINT32_MAX // Reserved filter unit value that prohibits all filters and socket
             // filters

// Type of signed client ID requests.
#define NECP_CLIENT_SIGNED_CLIENT_ID_TYPE_UUID                                 \
  1 // struct necp_client_signed_client_id_uuid
...
/*
 * The sysctl "net.necp.necp_drop_dest_level" controls the global drop rule
 * policy for a set of destinations addresses at the given level -- the drop
 * rule is the last one to be evaluated at this level.
 */
...
#define NECPDATATRACELOG(level, format, ...)                                   \
  do {                                                                         \
    if (level == LOG_ERR) {                                                    \
...
struct necp_all_kstats {
  ...
  struct necp_all_stats *necp_stats_ustats; /* points to user-visible stats (in
                                               shared ustats region) */
};

extern void necp_client_init(void);
extern int necp_application_find_policy_match_internal(
    proc_t proc, u_int8_t *parameters __sized_by(parameters_size),
    u_int32_t parameters_size, struct necp_aggregate_result *returned_result,
    ...
    struct rtentry **returned_route, bool ignore_address, bool has_client,
    uuid_t *returned_override_euuid);
/*
 * TLV utilities
 *
 * Note that these functions (other than necp_buffer_find_tlv) do not check the
 * length of the entire buffer, so the caller must be sure that the entire TLV
 * is within bounds.
 */
...
extern u_int8_t *__counted_by(0)
    necp_buffer_write_tlv(u_int8_t *__counted_by(0) cursor_, u_int8_t type,
                          u_int32_t length,
                          const void *value __sized_by(length),
                          u_int8_t *buffer __sized_by(buffer_length),
                          u_int32_t buffer_length);
extern u_int8_t *__counted_by(0) necp_buffer_write_tlv_if_different(
    u_int8_t *__counted_by(0) cursor_, u_int8_t type, u_int32_t length,
    const void *value __sized_by(length), bool *updated,
    u_int8_t *buffer __sized_by(buffer_length), u_int32_t buffer_length);
extern u_int8_t necp_buffer_get_tlv_type(u_int8_t *__counted_by(buffer_length)
                                             buffer,
                                         size_t buffer_length,
                                         u_int32_t tlv_offset);
extern u_int32_t
necp_buffer_get_tlv_length(u_int8_t *__counted_by(buffer_length) buffer,
                           size_t buffer_length, u_int32_t tlv_offset);
extern u_int8_t *__sized_by(*value_size)
    __necp_buffer_get_tlv_value(u_int8_t *__counted_by(buffer_length) buffer,
                                size_t buffer_length, u_int32_t tlv_offset,
                                u_int32_t *value_size);
extern int necp_buffer_find_tlv(u_int8_t *buffer __sized_by(buffer_length),
                                u_int32_t buffer_length, int offset,
                                u_int8_t type, int *err, int next);

/*
 * Inline shim to safely convert the result of `__necp_buffer_get_tlv_value'
 * to an indexable pointer, when appropriate.
 */
__attribute__((always_inline)) static inline u_int8_t *__header_indexable
necp_buffer_get_tlv_value(u_int8_t *__counted_by(buffer_length) buffer,
                          size_t buffer_length, u_int32_t tlv_offset,
                          u_int32_t *value_size) {
  u_int32_t ensured_value_size = 0;
  u_int8_t *value = __necp_buffer_get_tlv_value(
      buffer, buffer_length, tlv_offset, &ensured_value_size);
  if (value_size) {
...
#define NECPCTL_DROP_ALL_LEVEL                                                 \
  1 /* Drop all packets if no policy matches above this level */
#define NECPCTL_DEBUG 2         /* Log all kernel policy matches */
#define NECPCTL_PASS_LOOPBACK 3 /* Pass all loopback traffic */
#define NECPCTL_PASS_KEEPALIVES                                                \
  4 /* Pass all kernel-generated keepalive traffic */
#define NECPCTL_SOCKET_POLICY_COUNT 5 /* Count of all socket-level policies */
#define NECPCTL_SOCKET_NON_APP_POLICY_COUNT                                    \
  6 /* Count of non-per-app socket-level policies */
...
#define NECPCTL_OBSERVER_MESSAGE_LIMIT                                         \
  16 /* Number of of NECP observer messages allowed to be queued */
#define NECPCTL_SYSCTL_ARENA_COUNT 17 /* Count of sysctl arenas */
#define NECPCTL_DROP_UNENTITLED_LEVEL                                          \
  18 /* Drop unentitled process traffic above this level */
...
#define NECPCTL_CLIENT_TRACING_PID                                             \
  23 /* Apply client tracing only to specified pid */
#define NECPCTL_DROP_MANAGEMENT_LEVEL                                          \
  24                          /* Drop management traffic at this level */
#define NECPCTL_TRIE_COUNT 25 /* Count of tries */

#define NECP_LOOPBACK_PASS_ALL 1 // Pass all loopback traffic
#define NECP_LOOPBACK_PASS_WITH_FILTER                                         \
  2 // Pass all loopback traffic, but activate content filter and/or flow divert
    // if applicable
...
#define NECP_KERNEL_POLICY_RESULT_ALLOW_UNENTITLED                             \
  NECP_POLICY_RESULT_ALLOW_UNENTITLED
#define NECP_KERNEL_POLICY_RESULT_REMOVE_NETAGENT                              \
  NECP_POLICY_RESULT_REMOVE_NETAGENT
#define NECP_KERNEL_POLICY_RESULT_REMOVE_NETAGENT_TYPE                         \
  NECP_POLICY_RESULT_REMOVE_NETAGENT_TYPE
...
#define NECP_KERNEL_POLICY_DROP_FLAG_LOCAL_NETWORK                             \
  NECP_POLICY_DROP_FLAG_LOCAL_NETWORK
#define NECP_KERNEL_POLICY_DROP_FLAG_SUPPRESS_ALERTS                           \
  NECP_POLICY_DROP_FLAG_SUPPRESS_ALERTS
#define NECP_KERNEL_POLICY_DROP_FLAG_DEFUNCT_ALL_FLOWS                         \
  NECP_POLICY_DROP_FLAG_DEFUNCT_ALL_FLOWS
...
struct necp_session_policy {
  ...
  bool pending_update;   // Policy has been modified since creation/last
                         // application
  ...
  u_int8_t *
      conditions __sized_by(conditions_size); // Array of conditions, each with
                                              // a u_int32_t length at start
  u_int32_t route_rules_size;
  u_int8_t *route_rules
      __sized_by(route_rules_size); // Array of route rules, each with a
                                    // u_int32_t length at start
  ...
  necp_kernel_policy_id
      kernel_ip_output_policies[MAX_KERNEL_IP_OUTPUT_POLICIES];
};
...
extern errno_t
necp_set_socket_attributes(struct inp_necp_attributes *attributes,
                           struct sockopt *sopt);
extern errno_t
necp_get_socket_attributes(struct inp_necp_attributes *attributes,
                           struct sockopt *sopt);
extern errno_t
...
extern int necp_set_socket_resolver_signature(struct inpcb *inp,
                                              struct sockopt *sopt);
extern int necp_get_socket_resolver_signature(struct inpcb *inp,
                                              struct sockopt *sopt);
extern bool necp_socket_has_resolver_signature(struct inpcb *inp);
extern bool necp_socket_resolver_signature_matches_address(
...
extern u_int32_t necp_socket_get_effective_mtu(struct inpcb *inp,
                                               u_int32_t current_mtu);

extern bool necp_socket_is_allowed_to_recv_on_interface(struct inpcb *inp,
                                                        ifnet_t interface);

extern bool necp_socket_is_allowed_to_send_recv(
    struct inpcb *inp, ifnet_t input_interface, u_int16_t pf_tag,
...
extern bool necp_socket_is_allowed_to_send_recv_v4(
    struct inpcb *inp, u_int16_t local_port, u_int16_t remote_port,
    struct in_addr *local_addr, struct in_addr *remote_addr,
    ifnet_t input_interface, u_int16_t pf_tag,
    necp_kernel_policy_id *return_policy_id, u_int32_t *return_route_rule_id,
    necp_kernel_policy_id *return_skip_policy_id, u_int32_t *return_pass_flags);
extern bool necp_socket_is_allowed_to_send_recv_v6(
    struct inpcb *inp, u_int16_t local_port, u_int16_t remote_port,
    struct in6_addr *local_addr, struct in6_addr *remote_addr,
    ifnet_t input_interface, u_int16_t pf_tag,
    necp_kernel_policy_id *return_policy_id, u_int32_t *return_route_rule_id,
    necp_kernel_policy_id *return_skip_policy_id, u_int32_t *return_pass_flags);
extern void necp_socket_update_qos_marking(struct inpcb *inp,
                                           struct rtentry *route,
                                           u_int32_t route_rule_id);
extern bool necp_lookup_current_qos_marking(int32_t *qos_marking_gencount,
                                            struct rtentry *route,
                                            struct ifnet *interface,
                                            u_int32_t route_rule_id,
                                            bool old_qos_marking);
extern int necp_mark_packet_from_socket(struct mbuf *packet, struct inpcb *inp,
                                        necp_kernel_policy_id policy_id,
                                        u_int32_t route_rule_id,
                                        necp_kernel_policy_id skip_policy_id,
                                        u_int32_t pass_flags);
...
extern int necp_get_app_uuid_from_packet(struct mbuf *packet, uuid_t app_uuid);

extern necp_kernel_policy_id necp_socket_find_policy_match(
    struct inpcb *inp, struct sockaddr *override_local_addr,
    struct sockaddr *override_remote_addr, u_int32_t override_bound_interface);
extern necp_kernel_policy_id necp_ip_output_find_policy_match(
    struct mbuf *packet, int flags, struct ip_out_args *ipoa,
    struct rtentry *rt, necp_kernel_policy_result *result,
    necp_kernel_policy_result_parameter *result_parameter);
extern necp_kernel_policy_id necp_ip6_output_find_policy_match(
    struct mbuf *packet, int flags, struct ip6_out_args *ip6oa,
    struct rtentry *rt, necp_kernel_policy_result *result,
    necp_kernel_policy_result_parameter *result_parameter);

extern int necp_mark_packet_from_ip(struct mbuf *packet,
                                    necp_kernel_policy_id policy_id);
extern int
necp_mark_packet_from_ip_with_skip(struct mbuf *packet,
                                   necp_kernel_policy_id policy_id,
                                   necp_kernel_policy_id skip_policy_id);
extern int necp_mark_packet_from_interface(struct mbuf *packet,
                                           ifnet_t interface);

extern ifnet_t necp_get_ifnet_from_result_parameter(
    necp_kernel_policy_result_parameter *result_parameter);
extern bool necp_packet_can_rebind_to_ifnet(struct mbuf *packet,
                                            struct ifnet *interface,
                                            struct route *new_route,
                                            int family);

extern bool necp_packet_is_allowed_over_interface(struct mbuf *packet,
                                                  struct ifnet *interface);

extern int necp_mark_packet_as_keepalive(struct mbuf *packet,
                                         bool is_keepalive);
...
extern bool necp_validate_resolver_answer(
    uuid_t client_id, u_int32_t sign_type,
...
extern int necp_sign_application_id(uuid_t client_id, u_int32_t sign_type,
                                    u_int8_t *__counted_by(*out_tag_length) tag,
                                    size_t *out_tag_length);

extern bool necp_validate_application_id(uuid_t client_id, u_int32_t sign_type,
                                         u_int8_t *tag __sized_by(tag_length),
                                         size_t tag_length);

extern void necp_update_all_clients(void); // Handle general re-evaluate event
extern void necp_update_all_clients_immediately_if_needed(
    bool should_update_immediately); // Handle general re-evaluate event

extern void necp_force_update_client(
    uuid_t client_id, uuid_t remove_netagent_uuid,
    u_int32_t agent_generation); // Cause a single client to get an update event

extern bool necp_set_client_as_background(
    proc_t proc, struct fileproc *fp,
    bool background); // Set all clients for an fp as background or not

struct necp_fd_data;
extern void necp_fd_memstatus(
    proc_t proc, uint32_t status,
    struct necp_fd_data *client_fd); // Purge memory of clients for the process
extern void
necp_fd_defunct(proc_t proc,
                struct necp_fd_data
                    *client_fd); // Set all clients for an process as defunct

extern void necp_client_request_in_process_flow_divert(pid_t pid);

extern int necp_client_register_socket_flow(pid_t pid, uuid_t client_id,
                                            struct inpcb *inp);

extern int necp_client_register_socket_listener(pid_t pid, uuid_t client_id,
                                                struct inpcb *inp);

#if SKYWALK
extern int necp_client_get_netns_flow_info(uuid_t client_id,
                                           struct ns_flow_info *flow_info);
#endif /* SKYWALK */

extern int necp_client_assert_bb_radio_manager(uuid_t client_id, bool assert);

extern int necp_client_assign_from_socket(pid_t pid, uuid_t client_id,
                                          struct inpcb *inp);

extern int necp_assign_client_result(uuid_t netagent_uuid, uuid_t client_id,
                                     u_int8_t *assigned_results
                                         __sized_by(assigned_results_length),
                                     size_t assigned_results_length);
extern int necp_assign_client_group_members(
...
extern int necp_stats_ctor(struct skmem_obj_info *oi,
                           struct skmem_obj_info *oim, void *arg,
                           uint32_t skmflag);
extern int necp_stats_dtor(void *addr, void *arg);

/* value to denote invalid flow advisory index */
struct netagent_session;
extern int necp_update_flow_protoctl_event(uuid_t netagent_uuid,
                                           uuid_t client_id,
                                           uint32_t protoctl_event_code,
                                           uint32_t protoctl_event_val,
                                           uint32_t protoctl_event_tcp_seq_num);
...
extern void *__sized_by(*message_length) necp_create_nexus_assign_message(
    uuid_t nexus_instance, nexus_port_t nexus_port,
    void *key __sized_by(key_length), uint32_t key_length,
...
typedef void (*necp_client_flow_cb)(void *handle, int action,
...
extern pid_t necp_client_get_proc_pid_from_arena_info(
    struct skmem_arena_mmap_info *arena_info);

extern void necp_client_early_close(
    uuid_t client_id); // Cause a single client to close stats, etc
...
extern void necp_copy_inp_domain_info(struct inpcb *, struct socket *,
                                      struct nstat_domain_info *);
extern void necp_with_inp_domain_name(
    struct socket *so, void *ctx,
    void (*with_func)(char *domain_name __null_terminated, void *ctx));
extern bool
net_domain_contains_hostname(char *hostname_string __null_terminated,
                             char *domain_string __null_terminated);
#endif /* KERNEL_PRIVATE */
#endif /* KERNEL */

#ifndef KERNEL

extern int necp_match_policy(const uint8_t *parameters, size_t parameters_size,
...
extern int necp_session_action(int necp_fd, uint32_t action, uint8_t *in_buffer,
