...
SYSCTL_INT(_net_link_headless, OID_AUTO, nxattach, CTLFLAG_RW | CTLFLAG_LOCKED,
           &if_headless_nxattach, 0, "headless interface auto-attach nexus");

static int if_headless_debug = 0;
SYSCTL_INT(_net_link_headless, OID_AUTO, debug, CTLFLAG_RW | CTLFLAG_LOCKED,
           &if_headless_debug, 0, "headless interface debug logs");
...
static int headless_tx_headroom_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  ...
  error =
      sysctl_io_number(req, if_headless_tx_headroom,
                       sizeof(if_headless_tx_headroom), &new_value, &changed);
  if (error == 0 && changed != 0) {
    if (new_value > headless_TX_HEADROOM_MAX || (new_value % 8) != 0) {
      return EINVAL;
    }
...
SYSCTL_PROC(_net_link_headless, OID_AUTO, tx_headroom,
            ...
            headless_tx_headroom_sysctl, "IU", "headless ethernet Tx headroom");
...
static int headless_max_mtu_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  ...
  if (error == 0 && changed != 0) {
    if (new_value > headless_MAX_MTU_MAX || new_value < ETHERMTU ||
...
SYSCTL_PROC(_net_link_headless, OID_AUTO, max_mtu,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            headless_max_mtu_sysctl, "IU", "headless interface maximum MTU");

static int headless_buflet_size_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  unsigned int new_value;
  int changed;
  int error;
  ...
  error =
      sysctl_io_number(req, if_headless_buflet_size,
  ...
  if (error == 0 && changed != 0) {
    ...
    if (new_value > headless_BUFLET_SIZE_MAX ||
        new_value < headless_BUFLET_SIZE_MIN || !is_power_of_two(new_value) ||
...
SYSCTL_PROC(_net_link_headless, OID_AUTO, buflet_size,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0,
            headless_buflet_size_sysctl, "IU",
            "headless interface buflet size");
...
static if_headless_ref ifnet_get_if_headless(ifnet_t ifp);

#define HEADLESS_DPRINTF(fmt, ...)                                             \
  {                                                                            \
    if (if_headless_debug != 0)                                                \
      printf("%s " fmt, __func__, ##__VA_ARGS__);                              \
  }

static inline void headless_set_detaching(if_headless_ref headlessif) {
  headlessif->iff_flags |= IFF_FLAGS_DETACHING;
}

static inline boolean_t headless_is_detaching(if_headless_ref headlessif) {
...
static struct if_clone headless_zero_cloner =
    IF_CLONE_INITIALIZER(HEADLESS_ZERO_IFNAME, headless_clone_create,
                         headless_clone_destroy, 0, HEADLESS_MAXUNIT);

static struct if_clone headless_null_cloner =
    IF_CLONE_INITIALIZER(HEADLESS_NULL_IFNAME, headless_clone_create,
                         headless_clone_destroy, 0, HEADLESS_MAXUNIT);
...
static inline void headless_lock(void) { lck_mtx_lock(&headless_lck_mtx); }

static inline void headless_unlock(void) { lck_mtx_unlock(&headless_lck_mtx); }

static inline unsigned int headless_max_mtu(ifnet_t ifp) {
...
static void headless_packet_pool_free(headless_packet_pool_t fpp) {
...
static void headless_release(if_headless_ref headlessif) {
...
static void headless_retain(if_headless_ref headlessif) {
  OSIncrementAtomic(&headlessif->iff_retain_count);
}

static void headless_seg_ctor_fn(const kern_pbufpool_t pp,
                                 const kern_segment_t buf_seg,
                                 const IOSKMemoryDescriptor buf_desc) {
#pragma unused(pp, buf_seg, buf_desc)
}

static void headless_seg_dtor_fn(const kern_pbufpool_t pp,
                                 const kern_segment_t buf_seg,
                                 const IOSKMemoryDescriptor buf_desc) {
#pragma unused(pp, buf_seg, buf_desc)
}

static headless_packet_pool_t headless_packet_pool_alloc(boolean_t multi_buflet,
...
**/
static errno_t headless_nxdp_init(kern_nexus_domain_provider_t domprov) {
#pragma unused(domprov)
  return 0;
}

static void headless_nxdp_fini(kern_nexus_domain_provider_t domprov) {
#pragma unused(domprov)
}

static uuid_t headless_nx_dom_prov;

static errno_t headless_register_nexus_domain_provider(void) {
  ...
  err = kern_nexus_register_domain_provider(
      NEXUS_TYPE_NET_IF, headless_provider_name, &dp_init, sizeof(dp_init),
...
**/
static if_headless_ref headless_nexus_context(kern_nexus_t nexus) {
  if_headless_ref headlessif;
...
static errno_t headless_nx_ring_init(kern_nexus_provider_t nxprov,
...
static void headless_nx_ring_fini(kern_nexus_provider_t nxprov,
                                  kern_nexus_t nexus,
                                  kern_channel_ring_t ring) {
  ...
  if (tcall != NULL) {
    ...
    if (!success) {
      ...
      if (headlessif->iff_tcall_active) {
        ...
        do {
          msleep(headlessif, &headless_lck_mtx, PZERO, "headless threadcall",
                 0);
...
static errno_t headless_nx_pre_connect(kern_nexus_provider_t nxprov,
...
static errno_t headless_nx_connected(kern_nexus_provider_t nxprov,
                                     kern_nexus_t nexus,
                                     kern_channel_t channel) {
...
static void headless_nx_pre_disconnect(kern_nexus_provider_t nxprov,
                                       kern_nexus_t nexus,
                                       kern_channel_t channel) {
#pragma unused(nxprov, channel)
  if_headless_ref headlessif;
...
static void headless_nx_disconnected(kern_nexus_provider_t nxprov,
                                     kern_nexus_t nexus,
                                     kern_channel_t channel) {
#pragma unused(nxprov, channel)
  if_headless_ref headlessif;
...
static errno_t headless_nx_slot_init(
...
static void headless_nx_slot_fini(kern_nexus_provider_t nxprov,
...
static errno_t headless_nx_sync_tx(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus,
...
#pragma unused(nxprov)
  ...
  headless_lock();
  if (headless_is_detaching(headlessif) || !headlessif->iff_channel_connected) {
    headless_unlock();
    return 0;
  }
  headless_unlock();
...
static errno_t headless_nx_sync_rx_null(kern_nexus_provider_t nxprov,
                                        kern_nexus_t nexus,
...
static errno_t headless_nx_sync_rx(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus,
                                   kern_channel_ring_t rx_ring,
                                   uint32_t flags) {
#pragma unused(nxprov)
  ...
  while (rx_slot != NULL) {
    ...
    if (if_headless_create_payload) {
      ...
      uint64_t *addr = __unsafe_forge_bidi_indexable(
          uint64_t *, kern_buflet_get_data_address(buf),
...
static void headless_async_doorbell(thread_call_param_t arg0,
...
static void headless_schedule_async_doorbell(if_headless_ref headlessif) {
  ...
  if (tcall != NULL) {
    ...
    tcall = thread_call_allocate_with_options(
        headless_async_doorbell, (thread_call_param_t)headlessif,
...
static errno_t headless_nx_tx_doorbell(kern_nexus_provider_t nxprov,
                                       kern_nexus_t nexus,
                                       kern_channel_ring_t ring,
                                       uint32_t flags) {
...
static errno_t headless_netif_prepare(kern_nexus_t nexus, ifnet_t ifp) {
  if_headless_ref headlessif;
...
static errno_t create_netif_provider_and_instance(
    if_headless_ref headlessif, struct ifnet_init_eparams *init_params,
    ifnet_t *ifp, uuid_t *provider, uuid_t *instance) {
  errno_t err;
  ...
  err = kern_nexus_controller_register_provider(
      controller, headless_nx_dom_prov, provider_name, &prov_init,
  ...
  err = kern_nexus_controller_alloc_net_provider_instance(
      controller, *provider, headlessif, NULL, instance, &net_init, ifp);
  if (err != 0) {
...
headless_attach_netif_nexus(if_headless_ref headlessif,
  ...
  return create_netif_provider_and_instance(
      headlessif, init_params, ifp, &nx->fnx_provider, &nx->fnx_instance);
}
...
static void headless_detach_netif_nexus(headless_nx_t nx) {
...
static int headless_clone_create(struct if_clone *ifc, u_int32_t unit,
                                 void *params) {
#pragma unused(params)
  ...
  if ((unsigned int)snprintf(headlessif->iff_name, sizeof(headlessif->iff_name),
                             "%s%d", ifc->ifc_name,
                             unit) >= sizeof(headlessif->iff_name)) {
    headless_release(headlessif);
    return EINVAL;
  }
...
static int headless_clone_destroy(ifnet_t ifp) {
  if_headless_ref headlessif;
  headless_nx nx;
...
static int headless_set_media(ifnet_t ifp, struct if_headless_request *iffr) {
  if_headless_ref headlessif;
  int error;
...
static int if_headless_request_copyin(user_addr_t user_addr,
...
static int headless_set_drvspec(ifnet_t ifp, uint64_t cmd, size_t len,
...
static int headless_get_drvspec(ifnet_t ifp, uint64_t cmd, size_t len,
                                user_addr_t user_addr) {
...
static int headless_ioctl(ifnet_t ifp, u_long cmd, void *data) {
  unsigned int count;
  ...
  case SIOCGIFMEDIA64:
    ...
    status =
        (headlessif->iff_peer != NULL) ? (IFM_AVALID | IFM_ACTIVE) : IFM_AVALID;
    ...
    if (user_addr == USER_ADDR_NULL) {
      ...
      error =
          copyout(&headlessif->iff_media_list, user_addr, count * sizeof(int));
    }
    headless_unlock();
    break;
  ...
  case SIOCGDRVSPEC64:
    ...
    if (drv_set_command) {
      error = headless_set_drvspec(ifp, drv_cmd, (size_t)drv_len, user_addr);
    } else {
      error = headless_get_drvspec(ifp, drv_cmd, (size_t)drv_len, user_addr);
    }
    break;
...
void if_headless_init(void) { /* nothing here */ }
#endif /* SKYWALK */
