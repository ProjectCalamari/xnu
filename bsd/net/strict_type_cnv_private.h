...
#error                                                                         \
    "do not include <net/strict_type_cnv_private.h> directly, use <net/sockaddr_utils.h> instead."
#endif
...
#define __STC_DECONST_AND_CONVERT(STAG, STYPE, DTAG, DTYPE, VAL)               \
  ({                                                                           \
    ...
    __WITH_SUPPRESSION(__WCAST_ALIGN,                                          \
                       (DTAG DTYPE * __single)(__sau_deconst_val));            \
  })
...
#define __STC_CONV_TO_BYTES_LEN(VAL, MAX_LEN)                                  \
  ({                                                                           \
    __WITH_SUPPRESSION(__WCAST_ALIGN, __unsafe_forge_bidi_indexable(           \
                                          uint8_t *, (VAL), (MAX_LEN)));       \
  })
...
#define __STC_CONV_FROM_BYTES_LEN(DTAG, DTYPE, VAL, LEN)                       \
  ({                                                                           \
    __WITH_SUPPRESSION(__WCAST_ALIGN,                                          \
                       __unsafe_forge_single(DTAG DTYPE *, (VAL)));            \
  })
...
#if __has_ptrcheck
#define __STC_ENFORCE_MIN_LEN(BTYPE, MIN_LEN, PTR)                             \
  do {                                                                         \
    if ((PTR)) {                                                               \
...
#define __STC_OBJ_TO_BYTES_CNV_IMPL(CV, TAG, TYPE, MAX_LEN, CNV_F)             \
  __STC_CONV_ATTRS__                                                           \
  static inline CV uint8_t *__header_indexable CNV_F(                          \
      const TAG TYPE *ptr __single) {                                          \
    return __STC_CONV_TO_BYTES_LEN(__DECONST(TAG TYPE * __single, ptr),        \
                                   (MAX_LEN));                                 \
  }

#define __STC_BYTES_TO_OBJ_CNV_IMPL(CV, TAG, TYPE, BTYPE, MIN_LEN, CNV_F)      \
  static inline CV TAG TYPE *__single __attribute__((overloadable)) CNV_F(     \
      CV BTYPE *data __header_indexable) {                                     \
    __STC_ENFORCE_MIN_LEN(CV BTYPE, MIN_LEN, data);                            \
    return __STC_CONV_FROM_BYTES_LEN(CV TAG, TYPE, data, (MIN_LEN));           \
  }

/*
 * Converts `[CV] STAG STYPE *__single' to `[CV] DTAG DTYPE *__single', using
 * CNV_F.
 */
#define __STC_OBJ_TO_OBJ_CNV_IMPL(CV, STAG, STYPE, DTAG, DTYPE, CNV_F)         \
  __STC_CONV_ATTRS__                                                           \
  static inline CV DTAG DTYPE *__single __attribute__((overloadable)) CNV_F(   \
      CV STAG STYPE *ptr __single) {                                           \
...
#define __STC_COBJ_TO_OBJ_CNV_IMPL(STAG, STYPE, DTAG, DTYPE, CNV_F)            \
  __STC_CONV_ATTRS__                                                           \
  static inline DTAG DTYPE *__single __attribute__((overloadable)) CNV_F(      \
      const STAG STYPE *ptr __single) {                                        \
 ...
 * 2. "Forward const":    const STAG STYPE *__single -> const DTAG DTYPE
 * *__single
 * 3. "Reverse const":    const DTAG DTYPE *__single -> const STAG STYPE
 * *__single
...
#define __STC_DEFINE_OBJECT_CONVERTERS(STAG, STYPE, DTAG, DTYPE)               \
  /* [0] ("Forward")  */                                                       \
  __STC_OBJ_TO_OBJ_CNV_IMPL(, STAG, STYPE, DTAG, DTYPE,                        \
                            __STC_TY_TY_CNV_F(STYPE, DTYPE))                   \
                                                                               \
  /* [1] ("Reverse") */                                                        \
  __STC_OBJ_TO_OBJ_CNV_IMPL(, DTAG, DTYPE, STAG, STYPE,                        \
                            __STC_TY_TY_CNV_F(DTYPE, STYPE))                   \
                                                                               \
  /* [2] ("Forward const")  */                                                 \
  __STC_OBJ_TO_OBJ_CNV_IMPL(const, STAG, STYPE, DTAG, DTYPE,                   \
                            __STC_CTY_CTY_CNV_F(STYPE, DTYPE))                 \
                                                                               \
  /* [3] ("Reverse const")  */                                                 \
  __STC_OBJ_TO_OBJ_CNV_IMPL(const, DTAG, DTYPE, STAG, STYPE,                   \
                            __STC_CTY_CTY_CNV_F(DTYPE, STYPE))                 \
                                                                               \
  /* [4] ("Forward deconst")  */                                               \
  __STC_COBJ_TO_OBJ_CNV_IMPL(STAG, STYPE, DTAG, DTYPE,                         \
                             __STC_CTY_TY_CNV_F(STYPE, DTYPE))                 \
                                                                               \
  /* [5] ("Reverse deconst")  */                                               \
  __STC_COBJ_TO_OBJ_CNV_IMPL(DTAG, DTYPE, STAG, STYPE,                         \
                             __STC_CTY_TY_CNV_F(DTYPE, STYPE))
...
#define __STC_DEFINE_SELF_CONVERTERS(DTAG, DTYPE)                              \
  /* [0] ("Identity")  */                                                      \
  __STC_CONV_ATTRS__                                                           \
  static inline DTAG DTYPE *__single __STC_TY_ID_CNV_F(DTYPE)(DTAG DTYPE *     \
                                                              sin __single) {  \
    return sin;                                                                \
  }                                                                            \
                                                                               \
  /* [1] ("Const identity")  */                                                \
  __STC_CONV_ATTRS__                                                           \
  static inline const DTAG DTYPE *__single __STC_CTY_CID_CNV_F(DTYPE)(         \
      const DTAG DTYPE *sin __single) {                                        \
    return sin;                                                                \
  }                                                                            \
                                                                               \
  /* [2] ("Deconst identity")  */                                              \
  __STC_CONV_ATTRS__                                                           \
  static inline DTAG DTYPE *__single __STC_CTY_ID_CNV_F(DTYPE)(                \
      const DTAG DTYPE *sin __single) {                                        \
 ...
 * 1. "Const type to const bytes": const TAG TYPE * __single -> const uint8_t
 * *__sized_by(MAX_LEN)
 * 2. "Bytes to type": uint8_t *__sized_by(MIN_LEN) -> TAG TYPE * __single
 * 3. "Const bytes to const type": const uint8_t *__sized_by(MIN_LEN) -> const
 * TAG TYPE * __single
 *
...
#define __STC_DEFINE_BYTE_TO_OBJ_CNVS(TAG, TYPE, MIN_LEN, MAX_LEN)             \
  ...
  __STC_OBJ_TO_BYTES_CNV_IMPL(, TAG, TYPE, MAX_LEN,                            \
                              __STC_COBJ_TO_CBY_CNV(TYPE))                     \
                                                                               \
  /* [2] ("Bytes to type") */                                                  \
  __STC_BYTES_TO_OBJ_CNV_IMPL(, TAG, TYPE, void, MIN_LEN,                      \
                              __STC_BY_TO_OBJ_CNV(TYPE))                       \
                                                                               \
  /* [3] ("Const bytes to const type") */                                      \
  __STC_BYTES_TO_OBJ_CNV_IMPL(const, TAG, TYPE, void, MIN_LEN,                 \
                              __STC_CBY_TO_COBJ_CNV(TYPE))
 ...
 */
__STC_CONV_ATTRS__
static inline const uint8_t *__header_indexable __attribute__((overloadable))
...
__STC_CONV_ATTRS__
static inline uint8_t *__header_indexable __attribute__((overloadable))
 ...
 *
 * The dispatch blocks below are meant to be composed into a single generic
 * selection expression. See the definition of `__SA_UTILS_CONV_TO_SOCKADDR` for
 * an example.
 *
 * Each dispatch block defines several type expressions and the corresponding
 * conversion macros.
 *
 * Wnen compiled with `-fbounds-safety', the dispatch block accepst sized and
 * single pointers. When compiled without `-fbounds-safety', each dispatch block
 * accepts only a pointer to the type.
 */

/*
 * Basic building block for associating a CNV_F function with a pointer to CV
 * TAG TYPE. Depending on whehter `-fbounds-safety' is enabled, the match block
 * is extended to sized pointers.
 */
#if __has_ptrcheck
#define __STC_GENERIC_CV_CNV_CLAUSE(CV, TAG, TYPE, CNV_F)                      \
  CV TAG TYPE *__single : CNV_F,                                               \
                          CV TAG TYPE *__bidi_indexable                        \
      : CNV_F,                                                                 \
        CV TAG TYPE *__indexable : CNV_F
#else /* !__has_ptrcheck */
#define __STC_GENERIC_CV_CNV_CLAUSE(CV, TAG, TYPE, CNV_F) CV TAG TYPE * : CNV_F
#endif /* !__has_ptrcheck */

/**
 * __STC_TYPE_TO_OBJ_CNV_CLAUSE(STAG, STYPE, DTYPE)
 *
 * Matches its argument against `STYPE' (`STAG' indicates struct or union), and
 * upon a type match, converts the argument to `DTYPE'.
 *
...
#define __STC_TYPE_TO_OBJ_CNV_CLAUSE(STAG, STYPE, DTYPE)                       \
  __STC_GENERIC_CV_CNV_CLAUSE(, STAG, STYPE, __STC_TY_TY_CNV_F(STYPE, DTYPE)), \
      __STC_GENERIC_CV_CNV_CLAUSE(const, STAG, STYPE,                          \
                                  __STC_CTY_CTY_CNV_F(STYPE, DTYPE))
 ...
 *
 * When bounds-checks are enabled, the match block treats the `__indexable' and
 * the `__bidi_indexable' pointers the same way (but still has to distinguish
 * between the pointer sizes).
 *
...
#define __STC_BYTES_TO_OBJ_CNV_CLAUSE(TYPE)                                    \
  __STC_GENERIC_CV_CNV_CLAUSE(, , char, __STC_BY_TO_OBJ_CNV(TYPE)),            \
      ...
      __STC_GENERIC_CV_CNV_CLAUSE(const, , uint8_t,                            \
                                  __STC_CBY_TO_COBJ_CNV(TYPE)),                \
...
#define __STC_OBJ_TO_BYTES_CNV_CLAUSE(TAG, TYPE)                               \
  __STC_GENERIC_CV_CNV_CLAUSE(, TAG, TYPE, __STC_OBJ_TO_BY_CNV(TYPE)),         \
      __STC_GENERIC_CV_CNV_CLAUSE(const, TAG, TYPE,                            \
                                  __STC_COBJ_TO_CBY_CNV(TYPE))
 ...
 * To preserve the `const' qualifier, use the `__STC_TYPE_TO_OBJ_CNV_CLAUSE'
 * instead.
 */
 ...
 *
 * The conversion REMOVES the `const' qualifier, if present.
 * To preserve the `const' qualifier, use the `__STC_IDENTITY_CNV_CLAUSE'
 * instead.
 */
 ...
 *
 * Matches a `[const] TAG TYPE *' and converts it to `[const] TAG TYPE *
 * __single'
 *
 * The conversion preserves the `const' qualifier.
 */
...
#define __STC_DEFINE_SUBTYPE_IMPL_6(TAG, TYPENAME, MIN_LEN, MAX_LEN, TAG1,     \
                                    TYPENAME1)                                 \
  __STC_DEFINE_SUBTYPE_IMPL_4(TAG, TYPENAME, MIN_LEN, MAX_LEN)                 \
  __STC_DEFINE_STATIC_CAST(TAG, TYPENAME, TAG1, TYPENAME1)

#define __STC_DEFINE_SUBTYPE_IMPL_8(TAG, TYPENAME, MIN_LEN, MAX_LEN, TAG1,     \
                                    TYPENAME1, TAG2, TYPENAME2)                \
  __STC_DEFINE_SUBTYPE_IMPL_6(TAG, TYPENAME, MIN_LEN, MAX_LEN, TAG1,           \
                              TYPENAME1)                                       \
...
#define __STC_DEFINE_VARIABLE_SIZE_SUBTYPE_4(TAG, TYPENAME, TAG1, TYPENAME1)   \
  __STC_DEFINE_SUBTYPE_IMPL_6(TAG, TYPENAME, sizeof(TAG TYPENAME), 255, TAG1,  \
                              TYPENAME1)

#define __STC_DEFINE_VARIABLE_SIZE_SUBTYPE_6(TAG, TYPENAME, TAG1, TYPENAME1,   \
                                             TAG2, TYPENAME2)                  \
  __STC_DEFINE_SUBTYPE_IMPL_6(TAG, TYPENAME, sizeof(TAG TYPENAME), 255, TAG1,  \
                              TYPENAME1, TAG2, TYPENAME2)
...
#define __STC_DEFINE_FIXED_SIZE_SUBTYPE_2(TAG, TYPENAME)                       \
  __STC_DEFINE_SUBTYPE_IMPL_4(TAG, TYPENAME, sizeof(TAG TYPENAME),             \
                              sizeof(TAG TYPENAME))

#define __STC_DEFINE_FIXED_SIZE_SUBTYPE_4(TAG, TYPENAME, TAG1, TYPENAME1)      \
  __STC_DEFINE_SUBTYPE_IMPL_6(TAG, TYPENAME, sizeof(TAG TYPENAME),             \
                              sizeof(TAG TYPENAME), TAG1, TYPENAME1)

#define __STC_DEFINE_FIXED_SIZE_SUBTYPE_6(TAG, TYPENAME, TAG1, TYPENAME1,      \
                                          TAG2, TYPENAME2)                     \
  __STC_DEFINE_SUBTYPE_IMPL_8(TAG, TYPENAME, sizeof(TAG TYPENAME),             \
                              sizeof(TAG TYPENAME), TAG1, TYPENAME1, TAG2,     \
                              TYPENAME2)
