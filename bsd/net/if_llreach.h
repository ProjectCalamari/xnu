...
/*
 * Per-interface link-layer reachability.  (Currently only for
 * ARP/NDP/Ethernet.)
 */
...
RB_PROTOTYPE_SC_PREV(__private_extern__, ll_reach_tree, if_llreach, ls_link,
                     ifllr_cmp);
...
#define IFLR_LOCK(_iflr) lck_mtx_lock(&(_iflr)->lr_lock)

#define IFLR_LOCK_SPIN(_iflr) lck_mtx_lock_spin(&(_iflr)->lr_lock)

#define IFLR_CONVERT_LOCK(_iflr)                                               \
  do {                                                                         \
    IFLR_LOCK_ASSERT_HELD(_iflr);                                              \
    lck_mtx_convert_spin(&(_iflr)->lr_lock);                                   \
  } while (0)

#define IFLR_UNLOCK(_iflr) lck_mtx_unlock(&(_iflr)->lr_lock)

#define IFLR_ADDREF(_iflr) iflr_addref(_iflr, 0)

#define IFLR_ADDREF_LOCKED(_iflr) iflr_addref(_iflr, 1)

#define IFLR_REMREF(_iflr) iflr_remref(_iflr)
