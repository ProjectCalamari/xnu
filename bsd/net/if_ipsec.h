...
#define IPSEC_OPT_INC_IFDATA_STATS_IN                                          \
  4 /* set to increment stat counters (type struct ipsec_stats_param) */
#define IPSEC_OPT_INC_IFDATA_STATS_OUT                                         \
  5 /* set to increment stat counters (type struct ipsec_stats_param) */
#define IPSEC_OPT_SET_DELEGATE_INTERFACE                                       \
  6 /* set the delegate interface (char[]) */
#define IPSEC_OPT_OUTPUT_TRAFFIC_CLASS                                         \
  7 /* set the traffic class for packets leaving the interface, see            \
       sys/socket.h */
#define IPSEC_OPT_ENABLE_CHANNEL                                               \
  8 /* enable a kernel pipe nexus that allows the owner to open a channel to   \
     * act as a driver, Must be set before connecting */
#define IPSEC_OPT_GET_CHANNEL_UUID                                             \
  9 /* get the uuid of the kernel pipe nexus instance */
#define IPSEC_OPT_ENABLE_FLOWSWITCH                                            \
  10 /* enable a flowswitch nexus that clients can use */
#define IPSEC_OPT_INPUT_FRAG_SIZE                                              \
  11 /* set the maximum size of input packets before fragmenting as a uint32_t \
      */
...
typedef enum {
  IPSEC_DSCP_MAPPING_COPY =
      0, /* Copy DSCP bits from inner IP header to outer IP header */
  IPSEC_DSCP_MAPPING_LEGACY =
      1, /* Copies bits from the outer IP header that are at TOS offset of the
            inner IP header, into the DSCP of the outer IP header  */
