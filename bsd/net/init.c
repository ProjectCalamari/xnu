...
SYSCTL_INT(_net_diagnose, OID_AUTO, on, CTLFLAG_RW | CTLFLAG_LOCKED,
           &net_diagnose_on, 0, "");
...
static struct init_list_entry *LIST_RAN =
    __unsafe_forge_single(struct init_list_entry *, ~(uintptr_t)0);
...
errno_t net_init_add(net_init_func_ptr init_func) {
  ...
  entry = kalloc_type(struct init_list_entry, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
__private_extern__ void net_init_run(void) {
...
#if (DEBUG || DEVELOPMENT)
  (void)PE_parse_boot_argn("net_diagnose_on", &net_diagnose_on,
                           sizeof(net_diagnose_on));
#endif /* (DEBUG || DEVELOPMENT) */
}
