...
static errno_t netsrc_ctlconnect(kern_ctl_ref kctl, struct sockaddr_ctl *sac,
                                 void **uinfo) {
...
static errno_t netsrc_reply(kern_ctl_ref kctl, uint32_t unit,
                            unsigned int version, struct netsrc_rep *reply) {
  switch (version) {
  case NETSRC_CURVERS:
    return ctl_enqueuedata(kctl, unit, reply, sizeof(*reply), CTL_DATA_EOR);
  case NETSRC_VERSION1: {
    if ((reply->nrp_flags & NETSRC_FLAG_ROUTEABLE) == 0) {
      return EHOSTUNREACH;
    }
#define NETSRC_FLAG_V1_MASK                                                    \
  (NETSRC_IP6_FLAG_TENTATIVE | NETSRC_IP6_FLAG_TEMPORARY |                     \
   NETSRC_IP6_FLAG_DEPRECATED | NETSRC_IP6_FLAG_OPTIMISTIC |                   \
   NETSRC_IP6_FLAG_SECURED)
    struct netsrc_repv1 v1 = {.nrp_src = reply->nrp_src,
                              .nrp_flags =
                                  (reply->nrp_flags & NETSRC_FLAG_V1_MASK),
...
static void netsrc_common(struct rtentry *rt, struct netsrc_rep *reply) {
  if (!rt) {
    return;
  }
...
static struct in6_addrpolicy *lookup_policy(struct sockaddr *sa) {
...
static void netsrc_policy_common(struct netsrc_req *request,
...
static errno_t netsrc_ipv6(kern_ctl_ref kctl, uint32_t unit,
  ...
  struct in6_addr storage,
      *in6 = in6_selectsrc(&request->nrq_sin6, NULL, NULL, &ro, NULL, &storage,
  ...
  if (ro.ro_srcia == NULL && in6 != NULL) {
    ro.ro_srcia =
        (struct ifaddr *)(ifa_foraddr6_scoped(in6, reply.nrp_ifindex));
  }
  if (ro.ro_srcia) {
...
static errno_t netsrc_ipv4(kern_ctl_ref kctl, uint32_t unit,
                           struct netsrc_req *request) {
  // Unfortunately, IPv4 doesn't have a function like in6_selectsrc
  // Look up the route
  lck_mtx_lock(rnh_lock);
  struct rtentry *rt = rt_lookup(TRUE, SA(&request->nrq_dst.sa), NULL,
                                 rt_tables[AF_INET], request->nrq_ifscope);
...
static errno_t netsrc_ctlsend(kern_ctl_ref kctl, uint32_t unit, void *uinfo,
                              mbuf_t m, int flags) {
