...
#define EA_LIST(ea)                                                            \
  EA_CH(ea, 0), EA_CH(ea, 1), EA_CH(ea, 2), EA_CH(ea, 3), EA_CH(ea, 4),        \
      EA_CH(ea, 5)
...
#define BOND_LOG(__level, __dbgf, __string, ...)                               \
  do {                                                                         \
    if (__level <= if_bond_log_level || BOND_DBGF_ENABLED(__dbgf)) {           \
...
static __inline__ void bond_assert_lock_held(void) {
  LCK_MTX_ASSERT(&bond_lck_mtx, LCK_MTX_ASSERT_OWNED);
}

static __inline__ void bond_assert_lock_not_held(void) {
  LCK_MTX_ASSERT(&bond_lck_mtx, LCK_MTX_ASSERT_NOTOWNED);
}

static __inline__ void bond_lock(void) { lck_mtx_lock(&bond_lck_mtx); }

static __inline__ void bond_unlock(void) { lck_mtx_unlock(&bond_lck_mtx); }
...
static __inline__ const char *SelectedStateString(SelectedState s) {
...
static inline bool uint32_bit_is_set(uint32_t flags, uint32_t flags_to_test) {
  return (flags & flags_to_test) != 0;
}

static inline void uint32_bit_set(uint32_t *flags_p, uint32_t flags_to_set) {
  *flags_p |= flags_to_set;
}

static inline void uint32_bit_clear(uint32_t *flags_p,
...
static __inline__ bool ifbond_flags_if_detaching(ifbond_ref ifb) {
  return (ifb->ifb_flags & IFBF_IF_DETACHING) != 0;
}

static __inline__ void ifbond_flags_set_if_detaching(ifbond_ref ifb) {
  ifb->ifb_flags |= IFBF_IF_DETACHING;
  return;
}

static __inline__ bool ifbond_flags_lladdr(ifbond_ref ifb) {
  return (ifb->ifb_flags & IFBF_LLADDR) != 0;
}

static __inline__ bool ifbond_flags_change_in_progress(ifbond_ref ifb) {
  return (ifb->ifb_flags & IFBF_CHANGE_IN_PROGRESS) != 0;
}

static __inline__ void ifbond_flags_set_change_in_progress(ifbond_ref ifb) {
  ifb->ifb_flags |= IFBF_CHANGE_IN_PROGRESS;
  return;
}

static __inline__ void ifbond_flags_clear_change_in_progress(ifbond_ref ifb) {
  ifb->ifb_flags &= ~IFBF_CHANGE_IN_PROGRESS;
  return;
}

static __inline__ bool ifbond_flags_promisc(ifbond_ref ifb) {
  return (ifb->ifb_flags & IFBF_PROMISC) != 0;
}

static __inline__ void ifbond_flags_set_promisc(ifbond_ref ifb) {
  ifb->ifb_flags |= IFBF_PROMISC;
  return;
}

static __inline__ void ifbond_flags_clear_promisc(ifbond_ref ifb) {
  ifb->ifb_flags &= ~IFBF_PROMISC;
  return;
}
...
static __inline__ void bondport_flags_set_ntt(bondport_ref p) {
  p->po_flags |= BONDPORT_FLAGS_NTT;
  return;
}

static __inline__ void bondport_flags_clear_ntt(bondport_ref p) {
  p->po_flags &= ~BONDPORT_FLAGS_NTT;
  return;
}

static __inline__ int bondport_flags_ntt(bondport_ref p) {
  return (p->po_flags & BONDPORT_FLAGS_NTT) != 0;
}

static __inline__ void bondport_flags_set_ready(bondport_ref p) {
  p->po_flags |= BONDPORT_FLAGS_READY;
  return;
}

static __inline__ void bondport_flags_clear_ready(bondport_ref p) {
  p->po_flags &= ~BONDPORT_FLAGS_READY;
  return;
}

static __inline__ int bondport_flags_ready(bondport_ref p) {
  return (p->po_flags & BONDPORT_FLAGS_READY) != 0;
}

static __inline__ void bondport_flags_set_selected_changed(bondport_ref p) {
  p->po_flags |= BONDPORT_FLAGS_SELECTED_CHANGED;
  return;
}

static __inline__ void bondport_flags_clear_selected_changed(bondport_ref p) {
  p->po_flags &= ~BONDPORT_FLAGS_SELECTED_CHANGED;
  return;
}

static __inline__ int bondport_flags_selected_changed(bondport_ref p) {
  return (p->po_flags & BONDPORT_FLAGS_SELECTED_CHANGED) != 0;
}

static __inline__ void bondport_flags_set_mux_attached(bondport_ref p) {
  p->po_flags |= BONDPORT_FLAGS_MUX_ATTACHED;
  return;
}

static __inline__ void bondport_flags_clear_mux_attached(bondport_ref p) {
  p->po_flags &= ~BONDPORT_FLAGS_MUX_ATTACHED;
  return;
}

static __inline__ int bondport_flags_mux_attached(bondport_ref p) {
  return (p->po_flags & BONDPORT_FLAGS_MUX_ATTACHED) != 0;
}

static __inline__ void bondport_flags_set_distributing(bondport_ref p) {
  p->po_flags |= BONDPORT_FLAGS_DISTRIBUTING;
  return;
}

static __inline__ void bondport_flags_clear_distributing(bondport_ref p) {
  p->po_flags &= ~BONDPORT_FLAGS_DISTRIBUTING;
  return;
}

static __inline__ int bondport_flags_distributing(bondport_ref p) {
...
static packet_buffer_ref packet_buffer_allocate(int length) {
  packet_buffer_ref m;
  int size;
  ...
  if (size > (int)MHLEN) {
    if (size > (int)MCLBYTES) {
      BOND_LOG(LOG_NOTICE, 0, "packet_buffer_allocate size %d > max %u", size,
               MCLBYTES);
      return NULL;
    }
...
static void *__indexable packet_buffer_byteptr(packet_buffer_ref buf) {
...
** Receive machine
**/
static void bondport_receive_machine(bondport_ref p, LAEvent event,
                                     void *event_data);
/**
** Periodic Transmission machine
**/
static void bondport_periodic_transmit_machine(bondport_ref p, LAEvent event,
                                               void *event_data);
...
static void bondport_transmit_machine(bondport_ref p, LAEvent event,
                                      void *event_data);

/**
** Mux machine
**/
static void bondport_mux_machine(bondport_ref p, LAEvent event,
                                 void *event_data);

/**
** bond, LAG
**/
static void ifbond_activate_LAG(ifbond_ref bond, LAG_ref lag, int active_media);

static void ifbond_deactivate_LAG(ifbond_ref bond, LAG_ref lag);

static int ifbond_all_ports_ready(ifbond_ref bond);

static LAG_ref ifbond_find_best_LAG(ifbond_ref bond, int *active_media);

static int LAG_get_aggregatable_port_count(LAG_ref lag, int *active_media);

static int ifbond_selection(ifbond_ref bond);

static void bond_handle_event(struct ifnet *port_ifp, int event_code);

/**
** bondport
**/

static void bondport_receive_lacpdu(bondport_ref p, lacpdu_ref in_lacpdu_p);

static void bondport_slow_proto_transmit(bondport_ref p, packet_buffer_ref buf);

static bondport_ref bondport_create(struct ifnet *port_ifp,
                                    lacp_port_priority priority, int active,
                                    int short_timeout, int *error);
static void bondport_start(bondport_ref p);

static void bondport_free(bondport_ref p);

static int bondport_aggregatable(bondport_ref p);

static int bondport_remove_from_LAG(bondport_ref p);

static void bondport_set_selected(bondport_ref p, SelectedState s);

static int bondport_matches_LAG(bondport_ref p, LAG_ref lag);

static void bondport_link_status_changed(bondport_ref p);

static void bondport_enable_distributing(bondport_ref p);

static void bondport_disable_distributing(bondport_ref p);

static __inline__ int bondport_collecting(bondport_ref p) {
...
static struct if_clone bond_cloner = IF_CLONE_INITIALIZER(
    BONDNAME, bond_clone_create, bond_clone_destroy, 0, BOND_MAXUNIT);

static int siocsifmtu(struct ifnet *ifp, int mtu) {
  struct ifreq ifr;
...
static int siocgifdevmtu(struct ifnet *ifp, struct ifdevmtu *ifdm_p) {
  struct ifreq ifr;
  int error;
...
static __inline__ void ether_addr_copy(uint8_t *__sized_by(ETHER_ADDR_LEN) dest,
                                       const uint8_t *__sized_by(ETHER_ADDR_LEN)
                                           source) {
  bcopy(source, dest, ETHER_ADDR_LEN);
  return;
}

static __inline__ void ifbond_retain(ifbond_ref ifb) {
  os_ref_retain(&ifb->ifb_retain_count);
}

static __inline__ void ifbond_release(ifbond_ref ifb) {
  ...
  if (ifb->ifb_ifma_slow_proto != NULL) {
    BOND_LOG(LOG_DEBUG, BD_DBGF_LIFECYCLE, "%s: removing multicast",
             ifb->ifb_name);
 ...
 */
static void ifbond_wait(ifbond_ref ifb, const char *msg) {
  int waited = 0;

  /* other add/remove in progress */
  while (ifbond_flags_change_in_progress(ifb)) {
    BOND_LOG(LOG_DEBUG, BD_DBGF_LIFECYCLE, "%s: %s msleep", ifb->ifb_name, msg);
    waited = 1;
  ...
  if (waited) {
    BOND_LOG(LOG_DEBUG, BD_DBGF_LIFECYCLE, "%s: %s woke up", ifb->ifb_name,
             msg);
  }
  return;
}
 ...
 *   a reference on the ifbond_ref.
 */
static void ifbond_signal(ifbond_ref ifb, const char *msg) {
  ifbond_flags_clear_change_in_progress(ifb);
  wakeup((caddr_t)ifb);
  BOND_LOG(LOG_DEBUG, BD_DBGF_LIFECYCLE, "%s: %s wakeup", ifb->ifb_name, msg);
  return;
}
...
static __inline__ int media_active(const struct media_info *mi) {
...
static __inline__ int media_full_duplex(const struct media_info *mi) {
  return (mi->mi_active & IFM_FDX) != 0;
}

static __inline__ int media_type_unknown(const struct media_info *mi) {
  int unknown;
...
static __inline__ int media_ok(const struct media_info *mi) {
  return media_full_duplex(mi) || media_type_unknown(mi);
}

static __inline__ int media_speed(const struct media_info *mi) {
  return link_speed(mi->mi_active);
}

static struct media_info interface_media_info(struct ifnet *ifp) {
...
static int if_siflladdr(struct ifnet *ifp, const struct ether_addr *ea_p) {
  struct ifreq ifr;
...
** bond_globals
**/
static bond_globals_ref bond_globals_create(lacp_system_priority sys_pri,
                                            lacp_system_ref sys) {
  bond_globals_ref b;
...
static int bond_globals_init(void) {
  bond_globals_ref b;
  int i;
  struct ifnet *ifp;
...
static void bond_bpf_tap_common(ifnet_t ifp, mbuf_t m,
...
static inline void bond_bpf_tap_in(ifnet_t ifp, mbuf_t m,
                                   struct ether_header *eh_p) {
  bond_bpf_tap_common(ifp, m, eh_p, bpf_tap_in);
}

static inline void bond_bpf_tap_out(ifnet_t ifp, mbuf_t m,
                                    struct ether_header *eh_p) {
...
static int bond_setmulti(struct ifnet *ifp) {
  ...
  TAILQ_FOREACH(p, &ifb->ifb_port_list, po_port_list) {
    ...
    if (error != 0) {
      BOND_LOG(LOG_NOTICE, 0, "%s: multicast_list_program(%s) failed, %d",
...
static int bond_clone_attach(void) {
  int error;
...
static int ifbond_add_slow_proto_multicast(ifbond_ref ifb) {
  int error;
...
static int bond_clone_create(struct if_clone *ifc, u_int32_t unit,
  ...
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, 0, "%s: failed to add slow_proto multicast, %d",
             ifb->ifb_name, error);
  }
...
static void bond_remove_all_interfaces(ifbond_ref ifb) {
  bondport_ref p;
...
static void bond_remove(ifbond_ref ifb) {
  bond_assert_lock_held();
...
static void bond_if_detach(struct ifnet *ifp) {
  int error;

  error = ifnet_detach(ifp);
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, 0, "%s: ifnet_detach failed, %d", ifp->if_xname,
             error);
  }
  return;
}

static int bond_clone_destroy(struct ifnet *ifp) {
  ifbond_ref ifb;
...
static uint32_t ether_header_hash(struct ether_header *eh_p) {
  uint32_t h;

  /* get 32-bits from destination ether and ether type */
  h = (*((uint16_t *)&eh_p->ether_dhost[4]) << 16) | eh_p->ether_type;
...
static struct mbuf *S_mbuf_skip_to_offset(struct mbuf *m, int32_t *offset) {
  int len;
...
static __inline__ uint32_t make_uint32(u_char c0, u_char c1, u_char c2,
                                       u_char c3) {
...
static __inline__ uint32_t make_uint32(u_char c0, u_char c1, u_char c2,
                                       u_char c3) {
  return ((uint32_t)c3 << 24) | ((uint32_t)c2 << 16) | ((uint32_t)c1 << 8) |
         (uint32_t)c0;
}
#endif /* BYTE_ORDER == LITTLE_ENDIAN */

static int S_mbuf_copy_uint32(struct mbuf *m, int32_t offset, uint32_t *val) {
  struct mbuf *current;
  u_char *current_data;
  struct mbuf *next;
  u_char *next_data;
  ...
  case 1:
    *val =
        make_uint32(current_data[0], next_data[0], next_data[1], next_data[2]);
    break;
  case 2:
    *val = make_uint32(current_data[0], current_data[1], next_data[0],
                       next_data[1]);
    break;
  default:
    *val = make_uint32(current_data[0], current_data[1], current_data[2],
                       next_data[0]);
    break;
  }
  return 0;
...
static uint32_t ip_header_hash(struct mbuf *m) {
  u_char *data;
...
static uint32_t ipv6_header_hash(struct mbuf *m) {
  u_char *data;
  int i;
  int32_t offset;
  struct mbuf *orig_m = m;
  ...
  if ((m->m_len - offset) >= (int)IP6_ADDRS_LEN) {
    ...
    for (i = 0, scan = (uint32_t *)data;
         i < (int)(IP6_ADDRS_LEN / sizeof(uint32_t)); i++, scan++) {
      val ^= *scan;
    }
  } else {
...
static uint32_t get_packet_hash(mbuf_t m) {
  uint32_t flow_hash;
...
static ifnet_t bond_get_port_ifp(ifnet_t ifp, uint32_t hash) {
  uint32_t port_index;
  ifbond_ref ifb;
...
static int bond_output(ifnet_t ifp, mbuf_t m) {
...
static bondport_ref ifbond_lookup_port(ifbond_ref ifb, struct ifnet *port_ifp) {
  bondport_ref p;
  TAILQ_FOREACH(p, &ifb->ifb_port_list, po_port_list) {
...
static bondport_ref bond_lookup_port(struct ifnet *port_ifp) {
  ifbond_ref ifb;
  bondport_ref port;
...
static void bond_receive_lacpdu(struct mbuf *m, struct ifnet *port_ifp) {
...
#define _FORCE_LINK_EVENT_INTERVAL 1
  if (media_type_unknown(&p->po_media_info)) {
    ...
    if ((now - p->po_force_link_event_time) >= _FORCE_LINK_EVENT_INTERVAL) {
  ...
  if (ifbond_selection(ifb)) {
    event_code =
        (ifb->ifb_active_lag == NULL) ? KEV_DL_LINK_OFF : KEV_DL_LINK_ON;
    ...
    if (event_code != ifb->ifb_last_link_event) {
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "%s: (receive) generating LINK event",
               ifb->ifb_name);
      bond_ifp = ifb->ifb_ifp;
      ifb->ifb_last_link_event = event_code;
    }
  ...
  m_freem(m);
  if (need_link_update) {
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "simulating link status changed event");
    bond_handle_event(port_ifp, KEV_DL_LINK_ON);
  }
  return;
}

static void bond_receive_la_marker_pdu(struct mbuf *m, struct ifnet *port_ifp) {
...
static bool is_slow_proto_multicast(struct ether_header *eh_p) {
...
static void bond_handle_slow_proto_multicast(ifnet_t port_ifp, mbuf_t m) {
  u_char subtype = *mtod(m, u_char *);

  if (subtype == IEEE8023AD_SLOW_PROTO_SUBTYPE_LACP) {
    if (m->m_pkthdr.len < LACPDU_MIN_SIZE) {
      BOND_LOG(LOG_DEBUG, BD_DBGF_INPUT, "dropping short LACP frame %d < %d",
    ...
    if (m->m_len < LACPDU_MIN_SIZE) {
      ...
      if (m == NULL) {
        BOND_LOG(LOG_NOTICE, BD_DBGF_INPUT, "m_pullup LACPDU failed");
        return;
      }
    }
    ...
    if (m->m_len < min_size) {
      ...
      if (m == NULL) {
        BOND_LOG(LOG_NOTICE, BD_DBGF_INPUT, "m_pullup LA_MARKER failed");
        return;
      }
    }
...
static void bond_input_packet_list(ifnet_t port_ifp, mbuf_t list) {
  ifbond_ref ifb;
  struct ifnet *ifp;
  bondport_ref p;
...
static int bond_input(ifnet_t port_ifp, __unused protocol_family_t protocol,
                      mbuf_t m) {
  struct ether_header *eh_p;
  void *__single frame_header;
  mblist list;
  ...
  for (scan = m; scan != NULL; scan = next_packet) {
    ...
    frame_header = scan->m_pkthdr.pkt_hdr;
    eh_p = (struct ether_header *)frame_header;
    if ((scan->m_flags & M_MCAST) != 0 && is_slow_proto_multicast(eh_p)) {
...
static __inline__ const char *bondport_get_name(bondport_ref p) {
  return __unsafe_null_terminated_from_indexable(p->po_name);
}

static __inline__ int bondport_get_index(bondport_ref p) {
  return ifnet_index(p->po_ifp);
}

static void bondport_slow_proto_transmit(bondport_ref p,
  ...
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LACP, "(%s) failed %d", bondport_get_name(p),
             error);
  }
  return;
}

static void bondport_timer_process_func(devtimer_ref timer,
  ...
  case devtimer_process_func_event_unlock:
    if (devtimer_valid(timer)) {
      ...
      if (ifbond_selection(p->po_bond)) {
        event_code = (p->po_bond->ifb_active_lag == NULL) ? KEV_DL_LINK_OFF
                                                          : KEV_DL_LINK_ON;
        /* XXX need to take a reference on bond_ifp */
        ...
        if (event_code != p->po_bond->ifb_last_link_event) {
          BOND_LOG(LOG_NOTICE, BD_DBGF_LACP,
                   "%s: (timer) generating LINK event", p->po_bond->ifb_name);
          bond_ifp = p->po_bond->ifb_ifp;
          p->po_bond->ifb_last_link_event = event_code;
        }
      }
...
static bondport_ref bondport_create(struct ifnet *port_ifp,
                                    lacp_port_priority priority, int active,
                                    int short_timeout, int *ret_error) {
  int error = 0;
  bondport_ref p = NULL;
  ...
  *ret_error = 0;
  ...
  if ((u_int32_t)snprintf(p->po_name, sizeof(p->po_name), "%s%d",
                          ifnet_name(port_ifp),
                          ifnet_unit(port_ifp)) >= sizeof(p->po_name)) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE, "name too large");
    *ret_error = EINVAL;
    goto failed;
  }
  error = siocgifdevmtu(port_ifp, &p->po_devmtu);
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE, "SIOCGIFDEVMTU %s failed, %d",
 ...
 */
static void bondport_invalidate_timers(bondport_ref p) {
 ...
 */
static void bondport_cancel_timers(bondport_ref p) {
...
static __inline__ int bond_device_mtu(struct ifnet *ifp, ifbond_ref ifb) {
  return ((int)ifnet_mtu(ifp) > ifb->ifb_altmtu) ? (int)ifnet_mtu(ifp)
                                                 : ifb->ifb_altmtu;
}
...
static int bond_add_interface(struct ifnet *ifp, struct ifnet *port_ifp) {
  ...
  if (devmtu != 0 &&
    ...
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
             "interface %s doesn't support mtu %d", bondport_get_name(p),
             devmtu);
    bondport_free(p);
    return EINVAL;
  }
  ...
  if (TAILQ_EMPTY(&ifb->ifb_port_list)) {
    ...
    if (ifp_offload != port_ifp_offload) {
      ...
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
               ...
               " using 0x%x instead",
               ifb->ifb_name, bondport_get_name(p), ifp_offload,
               port_ifp_offload, offload);
      /*
       * XXX
  ...
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE, "(%s, %s): SIOCSIFMTU %d failed %d",
  ...
  if (error) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
             "(%s, %s): multicast_list_program failed %d", ifb->ifb_name,
             bondport_get_name(p), error);
    goto failed;
  }
  ...
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE, "(%s, %s): SIOCSIFFLAGS failed %d",
             ifb->ifb_name, bondport_get_name(p), error);
    goto failed;
  }

  /* re-program the port's ethernet address */
  error = if_siflladdr(port_ifp, (const struct ether_addr *)IF_LLADDR(ifp));
  if (error == 0) {
    if (memcmp(IF_LLADDR(ifp), IF_LLADDR(port_ifp), ETHER_ADDR_LEN) != 0) {
  ...
  if (error != 0) {
    ...
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE, "(%s, %s): if_siflladdr failed %d",
             ifb->ifb_name, bondport_get_name(p), error);
    error = ifnet_set_promiscuous(port_ifp, 1);
    if (error != 0) {
      /* port doesn't support setting promiscuous mode */
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
               "(%s, %s): set promiscuous failed %d", ifb->ifb_name,
               bondport_get_name(p), error);
      goto failed;
    }
    uint32_bit_set(&control_flags, PORT_CONTROL_FLAGS_PROMISCUOUS_SET);
  } else {
    uint32_bit_set(&control_flags, PORT_CONTROL_FLAGS_LLADDR_SET);
  }
  ...
  if (ifbond_flags_promisc(ifb)) {
    ...
    uint32_bit_set(&control_flags, PORT_CONTROL_FLAGS_BOND_PROMISCUOUS_SET);
  }
...
failed:
  ...
  if (uint32_bit_is_set(control_flags, PORT_CONTROL_FLAGS_LLADDR_SET)) {
    int error1;

    error1 = if_siflladdr(port_ifp, &p->po_saved_addr);
    if (error1 != 0) {
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
               "(%s, %s): if_siflladdr restore failed %d", ifb->ifb_name,
               bondport_get_name(p), error1);
    }
  }
  if (uint32_bit_is_set(control_flags, PORT_CONTROL_FLAGS_PROMISCUOUS_SET)) {
    int error1;

    error1 = ifnet_set_promiscuous(port_ifp, 0);
    if (error1 != 0) {
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
               "(%s, %s): promiscous mode disable failed %d", ifb->ifb_name,
               bondport_get_name(p), error1);
    }
  }
  if (uint32_bit_is_set(control_flags, PORT_CONTROL_FLAGS_PROTO_ATTACHED)) {
    (void)bond_detach_protocol(port_ifp);
  }
  if (uint32_bit_is_set(control_flags, PORT_CONTROL_FLAGS_MTU_SET)) {
    int error1;

    error1 = siocsifmtu(port_ifp, p->po_devmtu.ifdm_current);
    if (error1 != 0) {
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
               "(%s, %s): SIOCSIFMTU %d failed %d", ifb->ifb_name,
               bondport_get_name(p), p->po_devmtu.ifdm_current, error1);
    }
  }
  bond_lock();
  if (uint32_bit_is_set(control_flags, PORT_CONTROL_FLAGS_IN_LIST)) {
...
static int bond_remove_interface(ifbond_ref ifb, struct ifnet *port_ifp) {
  ...
  if (head_port == NULL) {
    ...
    ifb->ifb_altmtu = 0;
             ...
             bcmp(&p->po_saved_addr, IF_LLADDR(ifp), ETHER_ADDR_LEN) == 0) {
  ...
  if (ifb->ifb_mode == IF_BOND_MODE_LACP) {
    ...
    bondport_transmit_machine(p, LAEventStart, TRANSMIT_MACHINE_TX_IMMEDIATE);
  } else {
  ...
  if (last) {
    ...
    ifnet_set_lladdr_and_type(ifp, &head_port->po_saved_addr, ETHER_ADDR_LEN,
                              IFT_ETHER);
    ...
    TAILQ_FOREACH(scan_port, &ifb->ifb_port_list, po_port_list) {
      ...
      error = if_siflladdr(scan_ifp, (const struct ether_addr *)IF_LLADDR(ifp));
      if (error != 0) {
        BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
                 "(%s, %s): if_siflladdr (%s) failed %d", ifb->ifb_name,
                 bondport_get_name(p), bondport_get_name(scan_port), error);
      }
    }
  }

  /* restore the port's ethernet address */
  if (uint32_bit_is_set(p->po_control_flags, PORT_CONTROL_FLAGS_LLADDR_SET)) {
    error = if_siflladdr(port_ifp, &p->po_saved_addr);
    if (error != 0) {
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
               "(%s, %s): if_siflladdr failed %d", ifb->ifb_name,
               bondport_get_name(p), error);
    }
  }
  ...
  if (uint32_bit_is_set(p->po_control_flags,
    ...
    if (error != 0) {
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
               "(%s, %s): disable promiscuous failed %d", ifb->ifb_name,
               bondport_get_name(p), error);
    }
  }
...
static void bond_set_lacp_mode(ifbond_ref ifb) {
  bondport_ref p;
...
static void bond_set_static_mode(ifbond_ref ifb) {
  bondport_ref p;
  lacp_actor_partner_state s;
...
static int bond_set_mode(struct ifnet *ifp, int mode) {
  int error = 0;
  int event_code = 0;
  ifbond_ref ifb;
  ...
  if (mode == IF_BOND_MODE_LACP) {
    ...
    event_code =
        (ifb->ifb_distributing_count == 0) ? KEV_DL_LINK_OFF : KEV_DL_LINK_ON;
  }
  ifb->ifb_last_link_event = event_code;
...
static int bond_get_status(ifbond_ref ifb, struct if_bond_req *ibr_p,
  ...
  count = (ifb->ifb_port_count < ibsr->ibsr_count) ? ifb->ifb_port_count
                                                   : ibsr->ibsr_count;
...
static int bond_set_promisc(struct ifnet *ifp) {
  int error = 0;
  ifbond_ref ifb;
  bool is_promisc;
  bondport_ref p;
  ...
  TAILQ_FOREACH(p, &ifb->ifb_port_list, po_port_list) {
    ...
    port_is_promisc = uint32_bit_is_set(
        p->po_control_flags, PORT_CONTROL_FLAGS_BOND_PROMISCUOUS_SET);
    ...
    if (error != 0) {
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
               "%s: ifnet_set_promiscuous(%s, %d): failed %d", ifb->ifb_name,
               port_ifp->if_xname, val, error);
      continue;
    }
    BOND_LOG(LOG_DEBUG, BD_DBGF_LIFECYCLE,
             "%s: ifnet_set_promiscuous(%s, %d): succeeded", ifb->ifb_name,
             port_ifp->if_xname, val);
    if (is_promisc) {
...
static void bond_get_mtu_values(ifbond_ref ifb, int *ret_min, int *ret_max) {
  int mtu_min = 0;
  int mtu_max = 0;
  bondport_ref p;
...
static int bond_set_mtu_on_ports(ifbond_ref ifb, int mtu) {
  int error = 0;
  bondport_ref p;
  ...
  TAILQ_FOREACH(p, &ifb->ifb_port_list, po_port_list) {
    ...
    if (error != 0) {
      BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE, "%s: SIOCSIFMTU %s failed, %d",
               ifb->ifb_name, bondport_get_name(p), error);
      break;
    }
  }
...
static int bond_set_mtu(struct ifnet *ifp, int mtu, int isdevmtu) {
  int error = 0;
  ifbond_ref ifb;
  int mtu_min;
  int mtu_max;
  ...
  old_max = ((int)ifnet_mtu(ifp) > ifb->ifb_altmtu) ? (int)ifnet_mtu(ifp)
                                                    : ifb->ifb_altmtu;
  if (new_max != old_max) {
...
static int bond_ioctl(struct ifnet *ifp, u_long cmd, void *data) {
  int error = 0;
  ...
  case SIOCSIFBOND:
    user_addr = proc_is64bit(current_proc()) ? ifr->ifr_data64
    ...
    case IF_BOND_OP_REMOVE_INTERFACE:
      port_ifp = ifunit(
          __unsafe_null_terminated_from_indexable(ibr.ibr_ibru.ibru_if_name));
      if (port_ifp == NULL) {
        error = ENXIO;
        break;
      }
...
static void bond_handle_event(struct ifnet *port_ifp, int event_code) {
  ...
  if (ifb->ifb_mode == IF_BOND_MODE_LACP) {
    if (ifbond_selection(ifb)) {
      ...
      if (event_code != ifb->ifb_last_link_event) {
        BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
                 "%s: (event) generating LINK event", ifb->ifb_name);
        bond_ifp = ifb->ifb_ifp;
        ifb->ifb_last_link_event = event_code;
      }
    }
     ...
     * generate a link event
     */
    if (old_distributing_count == 0 && ifb->ifb_distributing_count != 0) {
...
static void bond_event(struct ifnet *port_ifp,
...
static errno_t bond_detached(ifnet_t port_ifp,
                             __unused protocol_family_t protocol) {
...
static int bond_detach_protocol(struct ifnet *ifp) {
  ...
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
             "%s: ifnet_detach_protocol failed, %d", ifp->if_xname, error);
  }
  return error;
}
...
__private_extern__ int bond_family_init(void) {
  int error = 0;

  error = proto_register_plumber(PF_INET, APPLE_IF_FAM_BOND, ether_attach_inet,
                                 ether_detach_inet);
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
             "proto_register_plumber failed for AF_INET error %d", error);
    goto done;
  }
  error = proto_register_plumber(PF_INET6, APPLE_IF_FAM_BOND,
                                 ether_attach_inet6, ether_detach_inet6);
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE,
             "proto_register_plumber failed for AF_INET6 error %d", error);
    goto done;
  }
  error = bond_clone_attach();
  if (error != 0) {
    BOND_LOG(LOG_NOTICE, BD_DBGF_LIFECYCLE, "bond_clone_attach error %d",
             error);
    goto done;
  }
  ...
  TAILQ_FOREACH(bond, &g_bond->ifbond_list, ifb_bond_list) {
    TAILQ_FOREACH(p, &bond->ifb_port_list, po_port_list) {
      ...
      if (ps->ps_port == lacp_actor_partner_tlv_get_port(atlv) &&
          bcmp(&ps_li->li_system, atlv->lap_system, sizeof(ps_li->li_system)) ==
              0) {
        BOND_LOG(LOG_DEBUG, BD_DBGF_LACP,
                 "System " EA_FORMAT " Port 0x%x moved from %s to %s",
                 EA_LIST(&ps_li->li_system), ps->ps_port, bondport_get_name(p),
...
static int ifbond_selection(ifbond_ref bond) {
  ...
  if (lag != bond->ifb_active_lag) {
    ...
    if (lag->lag_active_media != active_media) {
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "LAG PORT SPEED CHANGED from %d to %d",
               link_speed(lag->lag_active_media), link_speed(active_media));
  ...
  TAILQ_FOREACH(p, &bond->ifb_port_list, po_port_list) {
    ...
    if (all_ports_ready && bondport_flags_ready(p) &&
...
static LAG_ref ifbond_find_best_LAG(ifbond_ref bond, int *active_media) {
  ...
  if (bond->ifb_active_lag != NULL) {
    ...
    if (bond->ifb_max_active > 0 && best_count > bond->ifb_max_active) {
  ...
  TAILQ_FOREACH(lag, &bond->ifb_lag_list, lag_list) {
    ...
    if (bond->ifb_max_active > 0 && count > bond->ifb_max_active) {
...
static void ifbond_deactivate_LAG(__unused ifbond_ref bond, LAG_ref lag) {
  bondport_ref p;
...
static void ifbond_activate_LAG(ifbond_ref bond, LAG_ref lag,
                                int active_media) {
  int need = 0;
  bondport_ref p;
...
static int ifbond_all_ports_attached(ifbond_ref bond, bondport_ref this_port) {
  bondport_ref p;
...
static LAG_ref ifbond_get_LAG_matching_port(ifbond_ref bond, bondport_ref p) {
  LAG_ref lag;
...
static void bondport_link_status_changed(bondport_ref p) {
  ...
  if (if_bond_debug) {
    if (media_active(&p->po_media_info)) {
      ...
      BOND_LOG(LOG_NOTICE, BD_DBGF_LACP, "[%s] Link UP %d Mbit/s %s duplex",
               bondport_get_name(p), media_speed(&p->po_media_info),
               duplex_string);
    } else {
      BOND_LOG(LOG_NOTICE, BD_DBGF_LACP, "[%s] Link DOWN",
               bondport_get_name(p));
    }
  }
  if (bond->ifb_mode == IF_BOND_MODE_LACP) {
    if (media_active(&p->po_media_info) && bond->ifb_active_lag != NULL &&
        p->po_lag == bond->ifb_active_lag &&
        p->po_selected != SelectedState_UNSELECTED) {
      if (media_speed(&p->po_media_info) != p->po_lag->lag_active_media) {
        BOND_LOG(LOG_DEBUG, BD_DBGF_LACP,
                 "[%s] Port speed %d differs from LAG %d", bondport_get_name(p),
...
static int bondport_aggregatable(bondport_ref p) {
  ...
  default:
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Port is not selectable",
             bondport_get_name(p));
    return 0;
...
static int bondport_matches_LAG(bondport_ref p, LAG_ref lag) {
  ...
  if (ps_li->li_system_priority == lag_li->li_system_priority &&
      ...
      (bcmp(&ps_li->li_system, &lag_li->li_system, sizeof(lag_li->li_system)) ==
       0)) {
    return 1;
  }
  return 0;
...
static void bondport_add_to_LAG(bondport_ref p, LAG_ref lag) {
...
static void bondport_assign_to_LAG(bondport_ref p) {
  ifbond_ref bond = p->po_bond;
  LAG_ref lag;
...
static void bondport_receive_lacpdu(bondport_ref p, lacpdu_ref in_lacpdu_p) {
  ...
  moved_port = ifbond_list_find_moved_port(
      p, (const lacp_actor_partner_tlv_ref)&in_lacpdu_p->la_actor_tlv);
  if (moved_port != NULL) {
...
static void bondport_set_selected(bondport_ref p, SelectedState s) {
  if (s != p->po_selected) {
    ...
    if (lag != NULL && bond->ifb_active_lag == lag) {
      ...
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] SetSelected: %s (was %s)",
               bondport_get_name(p), SelectedStateString(s),
...
static void bondport_UpdateDefaultSelected(bondport_ref p) {
  bondport_set_selected(p, SelectedState_UNSELECTED);
  return;
}

static void bondport_RecordDefault(bondport_ref p) {
  bzero(&p->po_partner_state, sizeof(p->po_partner_state));
  p->po_actor_state = lacp_actor_partner_state_set_defaulted(p->po_actor_state);
  bondport_assign_to_LAG(p);
  return;
}

static void bondport_UpdateSelected(bondport_ref p, lacpdu_ref lacpdu_p) {
  ...
  if (lacp_actor_partner_tlv_get_port(actor) != ps->ps_port ||
    ...
    bondport_set_selected(p, SelectedState_UNSELECTED);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] updateSelected UNSELECTED",
             bondport_get_name(p));
  }
  return;
}

static void bondport_RecordPDU(bondport_ref p, lacpdu_ref lacpdu_p) {
  lacp_actor_partner_tlv_ref actor;
  ifbond_ref bond = p->po_bond;
  int lacp_maintain = 0;
  partner_state_ref ps;
  ...
  ps_li->li_system_priority = lacp_actor_partner_tlv_get_system_priority(actor);
  ...
  if (lacp_actor_partner_state_active_lacp(ps->ps_state) ||
    ...
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] recordPDU: LACP will maintain",
             bondport_get_name(p));
    lacp_maintain = 1;
  }
  if ((lacp_actor_partner_tlv_get_port(partner) == bondport_get_index(p)) &&
      ...
      bcmp(partner->lap_system, &g_bond->system, sizeof(g_bond->system)) == 0 &&
      ...
      lacp_actor_partner_state_in_sync(actor->lap_state) && lacp_maintain) {
    ps->ps_state = lacp_actor_partner_state_set_in_sync(ps->ps_state);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] recordPDU: LACP partner in sync",
             bondport_get_name(p));
...
static void bondport_UpdateNTT(bondport_ref p, lacpdu_ref lacpdu_p) {
  ifbond_ref bond = p->po_bond;
  lacp_actor_partner_tlv_ref partner;
  ...
  if ((lacp_actor_partner_tlv_get_port(partner) != bondport_get_index(p)) ||
      ...
      (lacp_actor_partner_tlv_get_system_priority(partner) !=
       g_bond->system_priority) ||
      lacp_actor_partner_tlv_get_key(partner) != bond->ifb_key ||
      (updateNTTBits(partner->lap_state) != updateNTTBits(p->po_actor_state))) {
    bondport_flags_set_ntt(p);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] updateNTT: Need To Transmit",
             bondport_get_name(p));
  }
  return;
}

static void bondport_AttachMuxToAggregator(bondport_ref p) {
  if (bondport_flags_mux_attached(p) == 0) {
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Attached Mux To Aggregator",
             bondport_get_name(p));
...
static void bondport_DetachMuxFromAggregator(bondport_ref p) {
  if (bondport_flags_mux_attached(p)) {
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Detached Mux From Aggregator",
             bondport_get_name(p));
...
static void bondport_enable_distributing(bondport_ref p) {
  if (bondport_flags_distributing(p) == 0) {
    ...
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Distribution Enabled",
             bondport_get_name(p));
...
static void bondport_disable_distributing(bondport_ref p) {
  if (bondport_flags_distributing(p)) {
    ...
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Distribution Disabled",
             bondport_get_name(p));
...
**/
static void bondport_receive_machine_initialize(bondport_ref p, LAEvent event,
                                                void *event_data);
static void bondport_receive_machine_port_disabled(bondport_ref p,
                                                   LAEvent event,
                                                   void *event_data);
static void bondport_receive_machine_expired(bondport_ref p, LAEvent event,
                                             void *event_data);
static void bondport_receive_machine_lacp_disabled(bondport_ref p,
                                                   LAEvent event,
                                                   void *event_data);
static void bondport_receive_machine_defaulted(bondport_ref p, LAEvent event,
                                               void *event_data);
static void bondport_receive_machine_current(bondport_ref p, LAEvent event,
                                             void *event_data);

static void bondport_receive_machine_event(bondport_ref p, LAEvent event,
                                           void *event_data) {
  switch (p->po_receive_state) {
...
static void bondport_receive_machine_initialize(bondport_ref p, LAEvent event,
  ...
  case LAEventStart:
    devtimer_cancel(p->po_current_while_timer);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Receive INITIALIZE",
             bondport_get_name(p));
...
static void bondport_receive_machine_port_disabled(bondport_ref p,
  ...
  case LAEventStart:
    devtimer_cancel(p->po_current_while_timer);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Receive PORT_DISABLED",
             bondport_get_name(p));
  ...
  case LAEventMediaChange:
    if (media_active(&p->po_media_info)) {
      ...
      devtimer_set_relative(p->po_current_while_timer, tv,
                            (devtimer_timeout_func)(void (*)(
                                void))bondport_receive_machine_port_disabled,
                            __unsafe_forge_single(void *, LAEventTimeout),
                            NULL);
...
static void bondport_receive_machine_expired(bondport_ref p, LAEvent event,
  ...
  case LAEventStart:
    devtimer_cancel(p->po_current_while_timer);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Receive EXPIRED",
             bondport_get_name(p));
    ...
    p->po_actor_state = lacp_actor_partner_state_set_expired(p->po_actor_state);
    /* start current_while timer */
    tv.tv_sec = LACP_SHORT_TIMEOUT_TIME;
    tv.tv_usec = 0;
    devtimer_set_relative(
        p->po_current_while_timer, tv,
        (devtimer_timeout_func)(void (*)(void))bondport_receive_machine_expired,
...
static void bondport_receive_machine_lacp_disabled(bondport_ref p,
  ...
  case LAEventStart:
    devtimer_cancel(p->po_current_while_timer);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Receive LACP_DISABLED",
             bondport_get_name(p));
...
static void bondport_receive_machine_defaulted(bondport_ref p, LAEvent event,
  ...
  case LAEventStart:
    devtimer_cancel(p->po_current_while_timer);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Receive DEFAULTED",
             bondport_get_name(p));
...
static void bondport_receive_machine_current(bondport_ref p, LAEvent event,
  ...
  case LAEventPacket:
    devtimer_cancel(p->po_current_while_timer);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Receive CURRENT",
             bondport_get_name(p));
    ...
    devtimer_set_relative(
        p->po_current_while_timer, tv,
        (devtimer_timeout_func)(void (*)(void))bondport_receive_machine_current,
        __unsafe_forge_single(void *, LAEventTimeout), NULL);
    break;
  case LAEventTimeout:
    bondport_receive_machine_expired(p, LAEventStart, NULL);
...
static void bondport_periodic_transmit_machine(bondport_ref p, LAEvent event,
  ...
  switch (event) {
  case LAEventStart:
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] periodic_transmit Start",
             bondport_get_name(p));
    OS_FALLTHROUGH;
  case LAEventMediaChange:
    devtimer_cancel(p->po_periodic_timer);
  ...
  case LAEventPacket:
    ...
    if (lacp_actor_partner_state_active_lacp(p->po_actor_state) == 0 &&
        (lacp_actor_partner_state_active_lacp(ps->ps_state) == 0)) {
      devtimer_cancel(p->po_periodic_timer);
      p->po_periodic_interval = 0;
      break;
    }
    ...
    if (p->po_periodic_interval != interval) {
      ...
      devtimer_set_relative(p->po_periodic_timer, tv,
                            (devtimer_timeout_func)(void (*)(
                                void))bondport_periodic_transmit_machine,
                            __unsafe_forge_single(void *, LAEventTimeout),
                            NULL);
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP,
  ...
  case LAEventTimeout:
    ...
    devtimer_set_relative(p->po_periodic_timer, tv,
                          ...
                          __unsafe_forge_single(void *, LAEventTimeout), NULL);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP,
             "[%s] Periodic Transmission Timer: %d secs", bondport_get_name(p),
...
** Transmit machine
**/
static int bondport_can_transmit(bondport_ref p, int32_t current_secs,
  ...
  case LAEventStart:
    ...
    if (event_data == TRANSMIT_MACHINE_TX_IMMEDIATE) {
      ...
      if (devtimer_enabled(p->po_transmit_timer)) {
        BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Transmit Timer Already Set",
                 bondport_get_name(p));
      } else {
        devtimer_set_absolute(
            p->po_transmit_timer, next_tick_time,
            (devtimer_timeout_func)(void (*)(void))bondport_transmit_machine,
            __unsafe_forge_single(void *, LAEventTimeout), NULL);
        BOND_LOG(LOG_DEBUG, BD_DBGF_LACP,
                 "[%s] Transmit Timer Deadline %d secs", bondport_get_name(p),
                 (int)next_tick_time.tv_sec);
      }
      break;
    }
    if (event == LAEventTimeout) {
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Transmit Timer Complete",
               bondport_get_name(p));
    }
    ...
    *((lacp_system_ref)aptlv->lap_system) = g_bond->system;
    lacp_actor_partner_tlv_set_system_priority(aptlv, g_bond->system_priority);
...
static void bondport_mux_machine_detached(bondport_ref p, LAEvent event,
                                          void *event_data);
static void bondport_mux_machine_waiting(bondport_ref p, LAEvent event,
                                         void *event_data);
static void bondport_mux_machine_attached(bondport_ref p, LAEvent event,
                                          void *event_data);

static void bondport_mux_machine_collecting_distributing(bondport_ref p,
                                                         LAEvent event,
                                                         void *event_data);
...
static void bondport_mux_machine_waiting(bondport_ref p, LAEvent event,
  ...
  case LAEventStart:
    devtimer_cancel(p->po_wait_while_timer);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Mux WAITING", bondport_get_name(p));
    p->po_mux_state = MuxState_WAITING;
    OS_FALLTHROUGH;
  default:
  case LAEventSelectedChange:
    ...
    if (p->po_selected == SelectedState_STANDBY) {
      ...
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Mux WAITING: Standby",
               bondport_get_name(p));
      break;
    }
    if (bondport_flags_ready(p)) {
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP,
               "[%s] Mux WAITING: Port is already ready", bondport_get_name(p));
      break;
    }
    if (devtimer_enabled(p->po_wait_while_timer)) {
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Mux WAITING: Timer already set",
               bondport_get_name(p));
      break;
    }
    if (ifbond_all_ports_attached(p->po_bond, p)) {
      devtimer_cancel(p->po_wait_while_timer);
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Mux WAITING: No waiting",
               bondport_get_name(p));
      bondport_flags_set_ready(p);
      goto no_waiting;
    }
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Mux WAITING: 2 seconds",
             bondport_get_name(p));
    tv.tv_sec = LACP_AGGREGATE_WAIT_TIME;
    tv.tv_usec = 0;
    devtimer_set_relative(
        p->po_wait_while_timer, tv,
        (devtimer_timeout_func)(void (*)(void))bondport_mux_machine_waiting,
        __unsafe_forge_single(void *, LAEventTimeout), NULL);
    break;
  case LAEventTimeout:
  ...
  no_waiting:
    if (bondport_flags_ready(p)) {
      BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Mux WAITING: All Ports Ready",
               bondport_get_name(p));
  ...
  default:
    ...
    case SelectedState_SELECTED:
      ...
      if (lacp_actor_partner_state_in_sync(s)) {
        bondport_mux_machine_collecting_distributing(p, LAEventStart, NULL);
      }
      break;
    default:
      bondport_mux_machine_detached(p, LAEventStart, NULL);
...
bondport_mux_machine_collecting_distributing(bondport_ref p, LAEvent event,
  ...
  case LAEventStart:
    devtimer_cancel(p->po_wait_while_timer);
    BOND_LOG(LOG_DEBUG, BD_DBGF_LACP, "[%s] Mux COLLECTING_DISTRIBUTING",
             bondport_get_name(p));
