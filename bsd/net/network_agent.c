...
SYSCTL_NODE(_net, OID_AUTO, netagent, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "NetworkAgent");
SYSCTL_INT(_net_netagent, OID_AUTO, debug, CTLFLAG_LOCKED | CTLFLAG_RW,
           &netagent_debug, 0, "");

static int netagent_registered_count = 0;
SYSCTL_INT(_net_netagent, OID_AUTO, registered_count,
           CTLFLAG_RD | CTLFLAG_LOCKED, &netagent_registered_count, 0, "");
...
#define NETAGENTLOG(level, format, ...)                                        \
  do {                                                                         \
...
static inline __attribute__((always_inline)) __pure uint8_t *
    __bidi_indexable netagent_get_data(const struct netagent *agent) {
  if (agent == NULL) {
    return NULL;
  }

  return __unsafe_forge_bidi_indexable(uint8_t *, agent->netagent_data,
                                       agent->netagent_data_size);
}
#else
...
__pure uint8_t *__bidi_indexable netagent_group_message_get_members(
    const struct netagent_client_group_message *msg, size_t members_length) {
  if (msg == NULL) {
    return NULL;
  }

  return __unsafe_forge_bidi_indexable(uint8_t *, msg->group_members,
                                       members_length);
}
#else
#define netagent_group_message_get_members(msg, members_length)                \
  ((msg)->group_members)
#endif

#if __has_ptrcheck
static inline __attribute__((always_inline)) __pure uint8_t *
    __bidi_indexable netagent_assign_message_get_necp_result(
        const struct netagent_assign_nexus_message *msg, size_t result_length) {
  if (msg == NULL) {
    return NULL;
  }

  return __unsafe_forge_bidi_indexable(uint8_t *, msg->assign_necp_results,
                                       result_length);
}
#else
#define netagent_assign_message_get_necp_result(msg, result_length)            \
  ((msg)->assign_necp_results)
#endif
...
struct netagent_session {
  u_int32_t control_unit; // A control unit of 0 indicates an agent owned by the
                          // kernel
...
static LIST_HEAD(_netagent_list, netagent_registration)
    shared_netagent_list = LIST_HEAD_INITIALIZER(master_netagent_list);
...
#define NETAGENT_LIST_LOCK_EXCLUSIVE()                                         \
  lck_rw_lock_exclusive(&netagent_list_lock)
...
#define NETAGENT_LIST_ASSERT_LOCKED()                                          \
  LCK_RW_ASSERT(&netagent_list_lock, LCK_RW_ASSERT_HELD)
...
#define NETAGENT_SESSION_ASSERT_LOCKED(session)                                \
  LCK_MTX_ASSERT(&session->session_lock, LCK_MTX_ASSERT_OWNED)

#define NETAGENT_LOCK_EXCLUSIVE(registration)                                  \
  lck_rw_lock_exclusive(&registration->agent_lock)
#define NETAGENT_LOCK_SHARED(registration)                                     \
  lck_rw_lock_shared(&registration->agent_lock)
#define NETAGENT_LOCK_SHARED_TO_EXCLUSIVE(registration)                        \
  lck_rw_lock_shared_to_exclusive(&registration->agent_lock)
#define NETAGENT_UNLOCK(registration) lck_rw_done(&registration->agent_lock)
#define NETAGENT_ASSERT_LOCKED(registration)                                   \
  LCK_RW_ASSERT(&registration->agent_lock, LCK_RW_ASSERT_HELD)
...
static errno_t netagent_ctl_connect(kern_ctl_ref kctlref,
                                    struct sockaddr_ctl *sac, void **unitinfo);
static errno_t netagent_ctl_disconnect(kern_ctl_ref kctlref, u_int32_t unit,
                                       void *unitinfo);
static errno_t netagent_ctl_send(kern_ctl_ref kctlref, u_int32_t unit,
                                 void *unitinfo, mbuf_t m, int flags);
static void netagent_ctl_rcvd(kern_ctl_ref kctlref, u_int32_t unit,
                              void *unitinfo, int flags);
static errno_t netagent_ctl_getopt(kern_ctl_ref kctlref, u_int32_t unit,
                                   void *unitinfo, int opt,
                                   void *__sized_by(*len) data, size_t *len);
static errno_t netagent_ctl_setopt(kern_ctl_ref kctlref, u_int32_t unit,
                                   void *unitinfo, int opt,
                                   void *__sized_by(len) data, size_t len);

static int netagent_send_ctl_data(u_int32_t control_unit,
                                  u_int8_t *__sized_by(buffer_size) buffer,
                                  size_t buffer_size);
...
// Register
static void netagent_handle_register_message(struct netagent_session *session,
                                             u_int32_t message_id,
                                             size_t payload_length,
                                             mbuf_t packet, size_t offset);
static errno_t netagent_handle_register_setopt(struct netagent_session *session,
                                               u_int8_t *payload,
                                               size_t payload_length);
...
static void netagent_handle_unregister_message(struct netagent_session *session,
                                               ...
                                               mbuf_t packet, size_t offset);
static errno_t
netagent_handle_unregister_setopt(struct netagent_session *session,
                                  u_int8_t *__sized_by(payload_length) payload,
                                  size_t payload_length);
static errno_t netagent_handle_unregister_all_setopt(
    struct netagent_session *session,
    u_int8_t *__sized_by(payload_length) payload, size_t payload_length);

// Update
static void netagent_handle_update_message(struct netagent_session *session,
                                           u_int32_t message_id,
                                           size_t payload_length, mbuf_t packet,
                                           size_t offset);
static errno_t netagent_handle_update_setopt(struct netagent_session *session,
                                             u_int8_t *payload,
                                             size_t payload_length);

// Assign nexus
static void netagent_handle_assign_nexus_message(
    struct netagent_session *session, u_int32_t message_id,
    size_t payload_length, mbuf_t packet, size_t offset);
static errno_t netagent_handle_assign_nexus_setopt(
    struct netagent_session *session,
    u_int8_t *__sized_by(payload_length) payload, size_t payload_length);

// Assign group
static errno_t netagent_handle_assign_group_setopt(
    struct netagent_session *session,
    u_int8_t *__sized_by(payload_length) payload, size_t payload_length);

// Set/get assert count
static errno_t
netagent_handle_use_count_setopt(struct netagent_session *session,
                                 u_int8_t *__sized_by(payload_length) payload,
                                 size_t payload_length);
static errno_t
netagent_handle_use_count_getopt(struct netagent_session *session,
                                 u_int8_t *__sized_by(*buffer_length) buffer,
                                 size_t *buffer_length);

// Manage tokens
static errno_t
netagent_handle_add_token_setopt(struct netagent_session *session,
                                 u_int8_t *__sized_by(token_length) token,
                                 size_t token_length);
static errno_t netagent_handle_flush_tokens_setopt(
    struct netagent_session *session,
    u_int8_t *__sized_by(payload_length) payload, size_t payload_length);
static errno_t
netagent_handle_token_count_getopt(struct netagent_session *session,
                                   u_int8_t *__sized_by(*buffer_length) buffer,
                                   size_t *buffer_length);
static errno_t netagent_handle_token_low_water_setopt(
    struct netagent_session *session,
    u_int8_t *__sized_by(buffer_length) buffer, size_t buffer_length);
static errno_t netagent_handle_token_low_water_getopt(
    struct netagent_session *session,
    u_int8_t *__sized_by(*buffer_length) buffer, size_t *buffer_length);

// Client error
static errno_t netagent_handle_reset_client_error_setopt(
    struct netagent_session *session,
    u_int8_t *__sized_by(payload_length) payload, size_t payload_length);

// Enable session mode
static errno_t netagent_handle_enable_session_mode_setopt(
    struct netagent_session *session, u_int8_t *payload, size_t payload_length);

// Requires list lock being held
static struct netagent_registration *
netagent_find_agent_with_uuid_and_lock(uuid_t uuid, bool exclusively,
                                       bool ignore_lock);

// Requires session lock being held
static struct netagent_registration *
netagent_session_find_agent_with_uuid_and_lock(struct netagent_session *session,
                                               uuid_t uuid, bool exclusively,
                                               bool ignore_lock);

// Requires session lock being held, and single agent only
static struct netagent_registration *
netagent_session_access_agent_with_lock(struct netagent_session *session,
                                        bool exclusively, bool ignore_lock);

errno_t netagent_init(void) { return netagent_register_control(); }
...
// Kernel events
static void netagent_post_event(uuid_t agent_uuid, u_int32_t event_code,
                                bool update_necp,
...
// Message handling
static u_int8_t *__indexable netagent_buffer_write_message_header(
    u_int8_t *__sized_by(sizeof(struct netagent_message_header) +
                         payload_length) buffer,
    u_int8_t message_type, u_int8_t flags, u_int32_t message_id,
    u_int32_t error, size_t payload_length) {
  memset(buffer, 0, sizeof(struct netagent_message_header));
  ((struct netagent_message_header *)(void *)buffer)->message_type =
      message_type;
  ...
  ((struct netagent_message_header *)(void *)buffer)->message_payload_length =
      (u_int32_t)payload_length;
  return payload_length ? buffer + sizeof(struct netagent_message_header)
                        : NULL;
}

static u_int8_t *__indexable netagent_buffer_write_session_message_header(
    u_int8_t *__sized_by(sizeof(struct netagent_session_message_header) +
                         payload_length) buffer,
    u_int8_t message_type, u_int8_t flags, u_int32_t message_id,
    u_int32_t error, uuid_t message_agent_id, size_t payload_length) {
  memset(buffer, 0, sizeof(struct netagent_session_message_header));
  ((struct netagent_session_message_header *)(void *)buffer)->message_type =
      message_type;
  ((struct netagent_session_message_header *)(void *)buffer)->message_flags =
      flags;
  ((struct netagent_session_message_header *)(void *)buffer)->message_id =
      message_id;
  ((struct netagent_session_message_header *)(void *)buffer)->message_error =
      error;
  uuid_copy(((struct netagent_session_message_header *)(void *)buffer)
                ->message_agent_id,
            message_agent_id);
  ((struct netagent_session_message_header *)(void *)buffer)
      ->message_payload_length = (u_int32_t)payload_length;
  return payload_length
             ? buffer + sizeof(struct netagent_session_message_header)
             : NULL;
}
...
static int netagent_send_ctl_data(u_int32_t control_unit,
                                  u_int8_t *__sized_by(buffer_size) buffer,
                                  size_t buffer_size) {
  if (netagent_kctlref == NULL || control_unit == 0 || buffer == NULL ||
      buffer_size == 0) {
    return EINVAL;
  }

  return ctl_enqueuedata(netagent_kctlref, control_unit, buffer, buffer_size,
                         CTL_DATA_EOR);
}

static int netagent_send_trigger(struct netagent_registration *registration,
                                 struct proc *p, u_int32_t flags,
  ...
  const size_t header_size =
      (session_mode ? sizeof(struct netagent_session_message_header)
  ...
  if (session_mode) {
    (void)netagent_buffer_write_session_message_header(
        trigger, trigger_type, 0, 0, 0, registration->netagent->netagent_uuid,
        sizeof(struct netagent_trigger_message));
  } else {
    (void)netagent_buffer_write_message_header(
        trigger, trigger_type, 0, 0, 0,
        sizeof(struct netagent_trigger_message));
  }

  trigger_message =
      (struct netagent_trigger_message *)(void *)(trigger + header_size);
  ...
  if ((error = netagent_send_ctl_data(registration->control_unit, trigger,
                                      trigger_size))) {
    NETAGENTLOG(LOG_ERR, "Failed to send trigger message on control unit %d",
                registration->control_unit);
  }
...
static int
netagent_send_client_message(struct netagent_registration *registration,
                             uuid_t client_id, u_int8_t message_type) {
  int error = 0;
  ...
  if (session_mode) {
    (void)netagent_buffer_write_session_message_header(
        message, message_type, 0, 0, 0, registration->netagent->netagent_uuid,
        sizeof(struct netagent_client_message));
  } else {
    (void)netagent_buffer_write_message_header(
        message, message_type, 0, 0, 0, sizeof(struct netagent_client_message));
  }

  client_message =
      (struct netagent_client_message *)(void *)(message + header_size);
  uuid_copy(client_message->client_id, client_id);

  if ((error = netagent_send_ctl_data(registration->control_unit, message,
                                      message_size))) {
    NETAGENTLOG(LOG_ERR, "Failed to send client message %d on control unit %d",
                message_type, registration->control_unit);
  }
...
static int
netagent_send_error_message(struct netagent_registration *registration,
                            uuid_t client_id, u_int8_t message_type,
                            int32_t error_code) {
  int error = 0;
  ...
  u_int8_t *message = NULL;
  size_t message_size =
      header_size + sizeof(struct netagent_client_error_message);
  ...
  if (session_mode) {
    (void)netagent_buffer_write_session_message_header(
        message, message_type, 0, 0, 0, registration->netagent->netagent_uuid,
        sizeof(struct netagent_client_error_message));
  } else {
    (void)netagent_buffer_write_message_header(
        message, message_type, 0, 0, 0,
        sizeof(struct netagent_client_error_message));
  }

  client_message =
      (struct netagent_client_error_message *)(void *)(message + header_size);
  uuid_copy(client_message->client_id, client_id);
...
static int
netagent_send_group_message(struct netagent_registration *registration,
                            uuid_t client_id, u_int8_t message_type,
  ...
  size_t message_size = header_size +
                        sizeof(struct netagent_client_group_message) +
                        group_members->group_members_length;
  ...
  client_message =
      (struct netagent_client_group_message *)(void *)(message + header_size);
  uuid_copy(client_message->client_id, client_id);
  memcpy(netagent_group_message_get_members(
             client_message, group_members->group_members_length),
         group_members->group_members, group_members->group_members_length);

  if ((error = netagent_send_ctl_data(registration->control_unit, message,
                                      message_size))) {
    NETAGENTLOG(LOG_ERR,
                "Failed to send client group message %d on control unit %d",
                message_type, registration->control_unit);
  }
...
netagent_send_tokens_needed(struct netagent_registration *registration) {
  ...
  if (session_mode) {
    ...
    (void)netagent_buffer_write_message_header(message, message_type, 0, 0, 0,
                                               0);
  }
  ...
  if ((error = netagent_send_ctl_data(registration->control_unit, message,
    ...
    NETAGENTLOG(
        LOG_ERR,
        "Failed to send client tokens needed message on control unit %d",
        registration->control_unit);
  }
...
static int netagent_send_success_response(struct netagent_session *session,
  ...
  (void)netagent_buffer_write_message_header(response, message_type,
                                             NETAGENT_MESSAGE_FLAGS_RESPONSE,
                                             message_id, 0, 0);

  if ((error = netagent_send_ctl_data(session->control_unit, response,
                                      response_size))) {
...
static errno_t netagent_send_error_response(struct netagent_session *session,
                                            u_int8_t message_type,
                                            u_int32_t message_id,
...
static errno_t netagent_ctl_send(kern_ctl_ref kctlref, u_int32_t unit,
                                 void *unitinfo, mbuf_t packet, int flags) {
#pragma unused(kctlref, unit, flags)
  ...
  case NETAGENT_MESSAGE_TYPE_REGISTER: {
    netagent_handle_register_message(session, header.message_id,
                                     header.message_payload_length, packet,
                                     sizeof(header));
    break;
  }
  case NETAGENT_MESSAGE_TYPE_UNREGISTER: {
    netagent_handle_unregister_message(session, header.message_id,
                                       header.message_payload_length, packet,
                                       sizeof(header));
    break;
  }
  case NETAGENT_MESSAGE_TYPE_UPDATE: {
    netagent_handle_update_message(session, header.message_id,
                                   header.message_payload_length, packet,
                                   sizeof(header));
    break;
  }
  ...
  case NETAGENT_MESSAGE_TYPE_ASSIGN_NEXUS: {
    netagent_handle_assign_nexus_message(session, header.message_id,
                                         header.message_payload_length, packet,
                                         sizeof(header));
    break;
  }
  default: {
    NETAGENTLOG(LOG_ERR, "Received unknown message type %d",
                header.message_type);
    netagent_send_error_response(session, header.message_type,
                                 header.message_id,
...
netagent_create_session(u_int32_t control_unit) {
  ...
  new_session =
      kalloc_type(struct netagent_session, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
netagent_session_t netagent_create(netagent_event_f event_handler,
                                   void *context) {
...
static void netagent_token_free(struct netagent_token *token) {
...
netagent_alloc_registration_memory(uint32_t data_size) {
  ...
  new_registration =
      kalloc_type(struct netagent_registration, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  new_registration->netagent =
      kalloc_data(netagent_alloc_size, Z_WAITOK | Z_NOFAIL);
...
netagent_free_registration_memory(struct netagent_registration *registration) {
  ...
  kfree_data_sized_by(registration->netagent,
                      registration->netagent_alloc_size);
...
netagent_free_registration(struct netagent_registration *registration) {
  ...
  TAILQ_FOREACH_SAFE(search_token, &registration->token_list, token_chain,
                     temp_token) {
  ...
  LIST_FOREACH_SAFE(search_client, &registration->pending_triggers_list,
                    client_chain, temp_client) {
...
static void netagent_unregister_all_session_registrations(
    struct netagent_session *session) {
  TAILQ_HEAD(_netagent_registration_list, netagent_registration)
  deleting_registrations;
  ...
  if (session != NULL) {
    ...
    TAILQ_FOREACH_SAFE(registration, &session->registrations, session_chain,
                       temp_registration) {
  ...
  struct netagent_registration *registration = NULL;
  struct netagent_registration *temp_registration = NULL;
  TAILQ_FOREACH_SAFE(registration, &deleting_registrations, session_chain,
                     temp_registration) {
    TAILQ_REMOVE(&deleting_registrations, registration, session_chain);

    ifnet_clear_netagent(registration->netagent->netagent_uuid);
    netagent_post_event(registration->netagent->netagent_uuid,
                        KEV_NETAGENT_UNREGISTERED, TRUE, false);
...
static void
netagent_unregister_one_session_registration(struct netagent_session *session,
                                             uuid_t agent_id) {
...
static size_t netagent_packet_get_netagent_data_size(mbuf_t packet,
...
static errno_t
netagent_handle_register_inner(struct netagent_session *session,
                               struct netagent_registration *new_registration) {
  NETAGENT_LIST_LOCK_EXCLUSIVE();
  ...
  if (!session->allow_multiple_registrations &&
      !TAILQ_EMPTY(&session->registrations)) {
    NETAGENT_SESSION_UNLOCK(session);
    NETAGENT_LIST_UNLOCK();
    return EINVAL;
  }

  struct netagent_registration *existing_registration =
      netagent_find_agent_with_uuid_and_lock(
          new_registration->netagent->netagent_uuid, false, true);
  if (existing_registration != NULL) {
    NETAGENTLOG0(LOG_ERR, "Existing agent registration UUID conflicts with new "
                          "agent registration");
    NETAGENT_SESSION_UNLOCK(session);
    NETAGENT_LIST_UNLOCK();
    return EEXIST;
  ...
  new_registration->allow_multiple_registrations =
      session->allow_multiple_registrations;
...
errno_t netagent_register(netagent_session_t _session, struct netagent *agent) {
  ...
  __nochk_memcpy(netagent_get_data(new_registration->netagent),
                 netagent_get_data(agent), data_size);
...
static errno_t netagent_handle_register_setopt(struct netagent_session *session,
  ...
  if (payload_length != (sizeof(struct netagent) + data_size)) {
    NETAGENTLOG(LOG_ERR,
                "Mismatch between data size and payload length (%lu != %zu)",
                (sizeof(struct netagent) + data_size), payload_length);
    response_error = EINVAL;
    goto done;
  }
  ...
  __nochk_memcpy(new_registration->netagent, register_netagent,
                 sizeof(struct netagent));
  __nochk_memcpy(netagent_get_data(new_registration->netagent),
                 netagent_get_data(register_netagent), data_size);
...
static void netagent_handle_register_message(struct netagent_session *session,
  ...
  size_t data_size =
      netagent_packet_get_netagent_data_size(packet, offset, &error);
  ...
  NETAGENTLOG0(LOG_DEBUG, "Registered new agent");
  netagent_send_success_response(session, NETAGENT_MESSAGE_TYPE_REGISTER,
                                 message_id);
  netagent_post_event(registered_uuid, KEV_NETAGENT_REGISTERED, TRUE, false);
  return;
fail:
  netagent_send_error_response(session, NETAGENT_MESSAGE_TYPE_REGISTER,
                               message_id, response_error);
}

errno_t netagent_unregister(netagent_session_t _session) {
  struct netagent_session *session = (struct netagent_session *)_session;
  if (session == NULL) {
...
netagent_handle_unregister_setopt(struct netagent_session *session,
  ...
  if (!session->allow_multiple_registrations) {
    ...
    if (payload_length < sizeof(uuid_t)) {
      NETAGENTLOG(LOG_ERR,
                  "Unregister message size too small for UUID: (%zu < %zu)",
...
#pragma unused(payload_length, packet, offset)
  ...
  netagent_send_success_response(session, NETAGENT_MESSAGE_TYPE_UNREGISTER,
                                 message_id);
  return;
fail:
  netagent_send_error_response(session, NETAGENT_MESSAGE_TYPE_UNREGISTER,
                               message_id, response_error);
}
...
netagent_send_cellular_failed_event(struct netagent_registration *registration,
  ...
  if (strlcmp(registration->netagent->netagent_domain, "Cellular",
  ...
  netpolicy_post_msg(KEV_NETPOLICY_IFFAILED, &ev_ifdenied.ev_data,
                     sizeof(ev_ifdenied));
}

static errno_t
netagent_handle_update_inner(struct netagent_session *session,
                             struct netagent_registration *new_registration,
                             size_t data_size, u_int8_t *agent_changed,
  ...
  struct netagent_registration *registration = NULL;
  if (session->allow_multiple_registrations) {
    registration = netagent_session_find_agent_with_uuid_and_lock(
        session, new_registration->netagent->netagent_uuid, true, false);
  } else {
    registration =
        netagent_session_access_agent_with_lock(session, true, false);
  }
  ...
  if (uuid_compare(registration->netagent->netagent_uuid,
                   new_registration->netagent->netagent_uuid) != 0 ||
      memcmp(&registration->netagent->netagent_domain,
             &new_registration->netagent->netagent_domain,
             sizeof(new_registration->netagent->netagent_domain)) != 0 ||
      memcmp(&registration->netagent->netagent_type,
             &new_registration->netagent->netagent_type,
  ...
  new_registration->netagent->netagent_flags |= NETAGENT_FLAG_REGISTERED;
  if (registration->netagent->netagent_data_size ==
          new_registration->netagent->netagent_data_size &&
      memcmp(registration->netagent, new_registration->netagent,
             sizeof(struct netagent)) == 0 &&
      memcmp(netagent_get_data(registration->netagent),
             netagent_get_data(new_registration->netagent), data_size) == 0) {
    ...
    LIST_FOREACH_SAFE(search_client, &registration->pending_triggers_list,
                      client_chain, temp_client) {
      LIST_REMOVE(search_client, client_chain);
      LIST_INSERT_HEAD(&pending_triggers_list_copy, search_client,
                       client_chain);
    }
    NETAGENT_UNLOCK(registration);
    NETAGENT_SESSION_UNLOCK(session);
    ...
    search_client = NULL;
    temp_client = NULL;
    LIST_FOREACH_SAFE(search_client, &pending_triggers_list_copy, client_chain,
                      temp_client) {
      necp_force_update_client(search_client->client_id,
                               registration->netagent->netagent_uuid,
                               registration->generation);
      netagent_send_cellular_failed_event(new_registration,
                                          search_client->client_pid,
                                          search_client->client_proc_uuid);
      LIST_REMOVE(search_client, client_chain);
      kfree_type(struct netagent_client, search_client);
    }
  ...
  TAILQ_INIT(&new_registration->token_list);
  TAILQ_CONCAT(&new_registration->token_list, &registration->token_list,
               token_chain);
  ...
  new_registration->allow_multiple_registrations =
      registration->allow_multiple_registrations;

  if ((new_registration->netagent->netagent_flags & NETAGENT_FLAG_ACTIVE) &&
      !(registration->netagent->netagent_flags & NETAGENT_FLAG_ACTIVE)) {
    netagent_active_count++;
  } else if (!(new_registration->netagent->netagent_flags &
...
errno_t netagent_update(netagent_session_t _session, struct netagent *agent) {
  ...
  should_update_immediately = (NETAGENT_FLAG_UPDATE_IMMEDIATELY ==
                               (new_registration->netagent->netagent_flags &
                                NETAGENT_FLAG_UPDATE_IMMEDIATELY));

  errno_t error =
      netagent_handle_update_inner(session, new_registration, data_size,
                                   &agent_changed, kNetagentErrorDomainPOSIX);
  if (error == 0) {
    netagent_post_event(updated_uuid, KEV_NETAGENT_UPDATED, agent_changed,
                        should_update_immediately);
    if (agent_changed == FALSE) {
      // The session registration does not need the "new_registration" as
      // nothing changed
      netagent_free_registration_memory(new_registration);
    }
  } else {
...
static errno_t netagent_handle_update_setopt(struct netagent_session *session,
  ...
  __nochk_memcpy(new_registration->netagent, update_netagent,
                 sizeof(struct netagent));
  __nochk_memcpy(netagent_get_data(new_registration->netagent),
                 netagent_get_data(update_netagent), data_size);
...
static void netagent_handle_update_message(struct netagent_session *session,
  ...
  error = mbuf_copydata(packet, offset, new_registration->netagent_alloc_size,
                        new_registration->netagent);
  if (error) {
    NETAGENTLOG(LOG_ERR, "Failed to read data into agent structure: %d", error);
    netagent_free_registration_memory(new_registration);
  ...
  response_error = (u_int32_t)netagent_handle_update_inner(
      session, new_registration, data_size, &agent_changed,
  ...
  netagent_send_success_response(session, NETAGENT_MESSAGE_TYPE_UPDATE,
                                 message_id);
  ...
  if (agent_changed == FALSE) {
    // The session registration does not need the "new_registration" as nothing
    // changed
    netagent_free_registration_memory(new_registration);
  }
  ...
  return;
fail:
  netagent_send_error_response(session, NETAGENT_MESSAGE_TYPE_UPDATE,
                               message_id, response_error);
}

errno_t netagent_assign_nexus(netagent_session_t _session,
                              uuid_t necp_client_uuid,
                              void *__sized_by(assigned_results_length)
                                  assign_message,
  ...
  struct netagent_registration *registration =
      netagent_session_access_agent_with_lock(session, false, false);
  if (registration == NULL) {
    NETAGENT_SESSION_UNLOCK(session);
  ...
  int error = necp_assign_client_result(
      netagent_uuid, necp_client_uuid, assign_message, assigned_results_length);
  if (error) {
    // necp_assign_client_result returns POSIX errors; don't error for ENOENT
    NETAGENTLOG((error == ENOENT ? LOG_DEBUG : LOG_ERR),
...
errno_t netagent_update_flow_protoctl_event(
    netagent_session_t _session, uuid_t client_id, uint32_t protoctl_event_code,
  ...
  error = necp_update_flow_protoctl_event(
      netagent_uuid, client_id, protoctl_event_code, protoctl_event_val,
...
static errno_t netagent_handle_assign_nexus_setopt(
  ...
  const size_t header_offset =
      (session->allow_multiple_registrations ? sizeof(uuid_t) : 0);
  ...
  struct netagent_assign_nexus_message *__single assign_nexus_netagent =
      (struct netagent_assign_nexus_message *)(void *)((u_int8_t *)payload +
                                                       header_offset);
  ...
  if (session->allow_multiple_registrations) {
    ...
    uuid_copy(agent_uuid, payload);
    registration = netagent_session_find_agent_with_uuid_and_lock(
        session, agent_uuid, false, false);
  } else {
    registration =
        netagent_session_access_agent_with_lock(session, false, false);
  }
  ...
  size_t assigned_results_length =
      (payload_length - (header_offset + sizeof(client_id)));

  if (assigned_results_length > 0) {
    assigned_results = kalloc_data(assigned_results_length, Z_WAITOK);
    if (assigned_results == NULL) {
      NETAGENTLOG(LOG_ERR, "Failed to allocate assign message (%lu bytes)",
                  assigned_results_length);
      response_error = ENOMEM;
      goto done;
    }
    memcpy(assigned_results,
           netagent_assign_message_get_necp_result(assign_nexus_netagent,
                                                   assigned_results_length),
           assigned_results_length);
  }

  // Note that if the error is 0, NECP has taken over our malloc'ed buffer
  response_error = necp_assign_client_result(
      netagent_uuid, client_id, assigned_results, assigned_results_length);
  if (response_error) {
...
static void netagent_handle_assign_nexus_message(
  ...
  if (assigned_results_length > 0) {
    ...
    error = mbuf_copydata(packet, offset + sizeof(client_id),
                          assigned_results_length, assigned_results);
    if (error) {
      kfree_data(assigned_results, assigned_results_length);
  ...
  // Note that if the error is 0, NECP has taken over our malloc'ed buffer
  error = necp_assign_client_result(netagent_uuid, client_id, assigned_results,
                                    assigned_results_length);
  if (error) {
    kfree_data(assigned_results, assigned_results_length);
  ...
  NETAGENTLOG0(LOG_DEBUG, "Agent assigned nexus properties to client");
  netagent_send_success_response(session, NETAGENT_MESSAGE_TYPE_ASSIGN_NEXUS,
                                 message_id);
  return;
fail:
  netagent_send_error_response(session, NETAGENT_MESSAGE_TYPE_ASSIGN_NEXUS,
                               message_id, response_error);
}
...
static errno_t netagent_handle_assign_group_setopt(
  ...
  struct netagent_assign_nexus_message *assign_message =
      (struct netagent_assign_nexus_message *)(void *)((u_int8_t *)payload +
                                                       header_offset);
  ...
  if (assigned_group_members_length > 0) {
    assigned_group_members =
        (u_int8_t *)kalloc_data(assigned_group_members_length, Z_WAITOK);
    if (assigned_group_members == NULL) {
      NETAGENTLOG(LOG_ERR,
                  "Failed to allocate group assign message (%lu bytes)",
                  assigned_group_members_length);
      response_error = ENOMEM;
      goto done;
    }
    memcpy(assigned_group_members,
           netagent_assign_message_get_necp_result(
               assign_message, assigned_group_members_length),
           assigned_group_members_length);
  }

  // Note that if the error is 0, NECP has taken over our malloc'ed buffer
  response_error = necp_assign_client_group_members(
      netagent_uuid, client_id, assigned_group_members,
      assigned_group_members_length);
  if (response_error != 0) {
...
errno_t netagent_handle_use_count_setopt(struct netagent_session *session,
                                         u_int8_t *__sized_by(payload_length)
                                             payload,
                                         size_t payload_length) {
  errno_t response_error = 0;
...
errno_t netagent_handle_use_count_getopt(struct netagent_session *session,
                                         u_int8_t *__sized_by(*buffer_length)
                                             buffer,
                                         size_t *buffer_length) {
  errno_t response_error = 0;
  uint64_t use_count = 0;
...
netagent_handle_add_token_setopt(struct netagent_session *session,
  ...
  token_struct =
      kalloc_type(struct netagent_token, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  token_struct->token_bytes =
      kalloc_data((token_length - header_offset), Z_WAITOK | Z_NOFAIL);
  token_struct->token_length = (u_int32_t)(token_length - header_offset);
  memcpy(token_struct->token_bytes, token + header_offset,
         (token_length - header_offset));
...
static errno_t
netagent_handle_flush_tokens_setopt(struct netagent_session *session,
                                    u_int8_t *__sized_by(buffer_length) buffer,
                                    size_t buffer_length) {
  errno_t response_error = 0;
...
static errno_t
netagent_handle_enable_session_mode_setopt(struct netagent_session *session,
                                           __unused u_int8_t *payload,
                                           __unused size_t payload_length) {
  errno_t response_error = 0;
...
netagent_session_access_agent_with_lock(struct netagent_session *session,
  ...
  struct netagent_registration *registration =
      TAILQ_FIRST(&session->registrations);
  if (registration != NULL) {
    if (!ignore_lock) {
      if (exclusively) {
        NETAGENT_LOCK_EXCLUSIVE(registration);
...
void netagent_post_updated_interfaces(uuid_t uuid) {
...
static u_int32_t netagent_dump_get_data_size_locked() {
  NETAGENT_LIST_ASSERT_LOCKED();

  struct netagent_registration *search_netagent = NULL;
  u_int32_t total_netagent_data_size = 0;
  // Traverse the shared list to know how much data the client needs to allocate
  // to get the list of agent UUIDs
  LIST_FOREACH(search_netagent, &shared_netagent_list, global_chain) {
    total_netagent_data_size +=
        sizeof(search_netagent->netagent->netagent_uuid);
  }
  return total_netagent_data_size;
}

static void netagent_dump_copy_data_locked(u_int8_t *__sized_by(buffer_length)
                                               buffer,
  ...
  response_size = buffer_length; // We already know that buffer_length is the
                                 // same as total_netagent_data_size.
  cursor = buffer;
  LIST_FOREACH(search_netagent, &shared_netagent_list, global_chain) {
    memcpy(cursor, search_netagent->netagent->netagent_uuid,
           sizeof(search_netagent->netagent->netagent_uuid));
...
int netagent_ioctl(u_long cmd, caddr_t __sized_by(IOCPARM_LEN(cmd)) data) {
  ...
  case SIOCGIFAGENTLIST64: {
    ...
    errno_t cred_result =
        priv_check_cred(kauth_cred_get(), PRIV_NET_PRIVILEGED_NECP_POLICIES, 0);
    if (cred_result != 0) {
      NETAGENTLOG0(LOG_ERR, "Client does not hold the necessary entitlement to "
                            "get netagent information");
      return EINVAL;
    }
    break;
  }
  ...
  case SIOCGIFAGENTDATA32: {
    ...
    struct netagent_registration *registration =
        netagent_find_agent_with_uuid_and_lock(ifsir32->netagent_uuid, false,
                                               false);
    if (registration == NULL) {
      error = ENOENT;
      break;
    ...
    memcpy(ifsir32->netagent_domain, registration->netagent->netagent_domain,
           sizeof(ifsir32->netagent_domain));
    memcpy(ifsir32->netagent_type, registration->netagent->netagent_type,
           sizeof(ifsir32->netagent_type));
    memcpy(ifsir32->netagent_desc, registration->netagent->netagent_desc,
           sizeof(ifsir32->netagent_desc));
    ...
    if (ifsir32->netagent_data_size == 0) {
      ...
      ifsir32->netagent_data_size = registration->netagent->netagent_data_size;
               ...
               ifsir32->netagent_data_size ==
                   registration->netagent->netagent_data_size) {
      // Second pass, client wants data buffer filled out
      error = copyout(netagent_get_data(registration->netagent),
                      ifsir32->netagent_data,
                      registration->netagent->netagent_data_size);
    } else {
      error = EINVAL;
    }
  ...
  case SIOCGIFAGENTDATA64: {
    ...
    struct netagent_registration *registration =
        netagent_find_agent_with_uuid_and_lock(ifsir64->netagent_uuid, false,
                                               false);
    if (registration == NULL) {
      error = ENOENT;
      break;
    ...
    memcpy(ifsir64->netagent_domain, registration->netagent->netagent_domain,
           sizeof(ifsir64->netagent_domain));
    memcpy(ifsir64->netagent_type, registration->netagent->netagent_type,
           sizeof(ifsir64->netagent_type));
    memcpy(ifsir64->netagent_desc, registration->netagent->netagent_desc,
           sizeof(ifsir64->netagent_desc));
    ...
    if (ifsir64->netagent_data_size == 0) {
      ...
      ifsir64->netagent_data_size = registration->netagent->netagent_data_size;
               ...
               ifsir64->netagent_data_size ==
                   registration->netagent->netagent_data_size) {
      // Second pass, client wants data buffer filled out
      error = copyout(netagent_get_data(registration->netagent),
                      ifsir64->netagent_data,
                      registration->netagent->netagent_data_size);
    } else {
      error = EINVAL;
    }
  ...
  case SIOCGIFAGENTLIST32: {
    struct netagentlist_req32 *ifsir32 =
        (struct netagentlist_req32 *)(void *)data;
    if (ifsir32->data_size == 0) {
      // First pass, client wants data size
      ifsir32->data_size = netagent_dump_get_data_size_locked();
    } else if (ifsir32->data != USER_ADDR_NULL && ifsir32->data_size > 0 &&
               ifsir32->data_size == netagent_dump_get_data_size_locked()) {
      // Second pass, client wants data buffer filled out
      u_int8_t *response = NULL;
  ...
  case SIOCGIFAGENTLIST64: {
    struct netagentlist_req64 *ifsir64 =
        (struct netagentlist_req64 *)(void *)data;
    if (ifsir64->data_size == 0) {
      // First pass, client wants data size
      ifsir64->data_size = netagent_dump_get_data_size_locked();
    } else if (ifsir64->data != USER_ADDR_NULL && ifsir64->data_size > 0 &&
               ifsir64->data_size == netagent_dump_get_data_size_locked()) {
      // Second pass, client wants data buffer filled out
      u_int8_t *response = NULL;
...
u_int32_t netagent_get_flags(uuid_t uuid) {
  ...
  NETAGENT_LIST_LOCK_SHARED();
  struct netagent_registration *registration =
      netagent_find_agent_with_uuid_and_lock(uuid, false, false);
  if (registration != NULL) {
...
errno_t netagent_set_flags(uuid_t uuid, u_int32_t flags) {
  errno_t error = 0;
  bool updated = false;

  NETAGENT_LIST_LOCK_SHARED();
  struct netagent_registration *registration =
      netagent_find_agent_with_uuid_and_lock(uuid, true, false);
  if (registration != NULL) {
    ...
    NETAGENTLOG0(LOG_DEBUG, "Attempt to set flags for invalid netagent");
    error = ENOENT;
  }
  NETAGENT_LIST_UNLOCK();
  if (updated) {
...
u_int32_t netagent_get_generation(uuid_t uuid) {
...
bool netagent_get_agent_domain_and_type(uuid_t uuid,
                                        char *__sized_by(NETAGENT_DOMAINSIZE)
                                            domain,
                                        char *__sized_by(NETAGENT_TYPESIZE)
                                            type) {
  bool found = FALSE;
  if (domain == NULL || type == NULL) {
    NETAGENTLOG(
        LOG_ERR,
        "Invalid arguments for netagent_get_agent_domain_and_type %p %p",
        domain, type);
    return FALSE;
  }
  ...
  if (registration != NULL) {
    found = TRUE;
    memcpy(domain, registration->netagent->netagent_domain,
           NETAGENT_DOMAINSIZE);
...
int netagent_kernel_trigger(uuid_t uuid) {
  ...
  struct netagent_registration *registration =
      netagent_find_agent_with_uuid_and_lock(uuid, false, false);
  if (registration == NULL) {
    NETAGENTLOG0(LOG_ERR,
                 "Requested netagent for kernel trigger could not be found");
    error = ENOENT;
    goto done;
  }

  if ((registration->netagent->netagent_flags &
       NETAGENT_FLAG_KERNEL_ACTIVATED) == 0) {
    NETAGENTLOG0(
        LOG_ERR,
  ...
  if ((registration->netagent->netagent_flags & NETAGENT_FLAG_ACTIVE)) {
    ...
    NETAGENTLOG0(LOG_INFO,
                 "Requested netagent for kernel trigger is already active");
    error = 0;
    goto done;
  }

  error = netagent_send_trigger(registration, current_proc(),
                                NETAGENT_TRIGGER_FLAG_KERNEL,
                                NETAGENT_MESSAGE_TYPE_TRIGGER);
  NETAGENTLOG((error ? LOG_ERR : LOG_INFO),
...
int netagent_client_message_with_params(
    uuid_t agent_uuid, uuid_t necp_client_uuid, pid_t pid, void *handle,
    u_int8_t message_type, struct necp_client_agent_parameters *parameters,
  ...
  if (message_type != NETAGENT_MESSAGE_TYPE_CLIENT_TRIGGER &&
    ...
    NETAGENTLOG(LOG_ERR, "Client netagent message type (%d) is invalid",
                message_type);
    return EINVAL;
  }
  ...
  struct netagent_registration *registration =
      netagent_find_agent_with_uuid_and_lock(agent_uuid, false, false);
  if (registration == NULL) {
  ...
  if (message_type == NETAGENT_MESSAGE_TYPE_CLIENT_TRIGGER) {
    if ((registration->netagent->netagent_flags &
         NETAGENT_FLAG_USER_ACTIVATED) == 0) {
      // Agent does not accept user triggers
      // Don't log, since this is a common case used to trigger events that
      // cellular data is blocked, etc.
      error = ENOTSUP;
      ...
      netagent_send_cellular_failed_event(registration, report_pid,
                                          report_proc_uuid);
      goto done;
    }
             ...
             message_type == NETAGENT_MESSAGE_TYPE_ABORT_NEXUS) {
    bool is_nexus_agent =
        ((registration->netagent->netagent_flags &
          (NETAGENT_FLAG_NEXUS_PROVIDER | NETAGENT_FLAG_NEXUS_LISTENER |
           NETAGENT_FLAG_CUSTOM_IP_NEXUS | NETAGENT_FLAG_CUSTOM_ETHER_NEXUS |
    ...
    if ((registration->netagent->netagent_flags & NETAGENT_FLAG_ACTIVE) == 0) {
      ...
      goto done;
             ...
             message_type == NETAGENT_MESSAGE_TYPE_REMOVE_GROUP_MEMBERS) {
    bool is_group_agent = ((registration->netagent->netagent_flags &
                            (NETAGENT_FLAG_SUPPORTS_GROUPS)) != 0);
    if (!is_group_agent) {
      NETAGENTLOG0(
          LOG_ERR,
  ...
  if (registration->control_unit == 0) {
    if (registration->event_handler == NULL) {
      ...
      // We hold the registration lock during the event handler callout, so it
      // is expected that the event handler will not lead to any registrations
      // or unregistrations of network agents. We release the list lock before
      // calling the event handler to allow other threads to access the list
      // while the event is processing.
      NETAGENT_LIST_UNLOCK();
      should_unlock_list = false;
      error = registration->event_handler(message_type, necp_client_uuid, pid,
                                          handle, registration->event_context,
                                          parameters, assigned_results,
                                          assigned_results_length);
      if (error != 0) {
        VERIFY(assigned_results == NULL || *assigned_results == NULL);
        VERIFY(assigned_results_length == NULL ||
    ...
    if (message_type == NETAGENT_MESSAGE_TYPE_CLIENT_ERROR) {
      ...
      if (registration->last_client_error !=
              client_error || // Always notify for an error change
          force_report ||     // Always notify if force reporting was requested
          (client_error == 0 && registration->client_error_count ==
                                    0) || // Only notify once for no-error
          (client_error != 0 && registration->client_error_count <
                                    NETAGENT_MAX_CLIENT_ERROR_COUNT)) {
        if (NETAGENT_LOCK_SHARED_TO_EXCLUSIVE(registration)) {
          ...
          error = netagent_send_error_message(registration, necp_client_uuid,
                                              message_type, client_error);
        } else {
          // If NETAGENT_LOCK_SHARED_TO_EXCLUSIVE fails, it unlocks
          // automatically
          should_unlock_registration = false;
               ...
               message_type == NETAGENT_MESSAGE_TYPE_REMOVE_GROUP_MEMBERS) {
      error = netagent_send_group_message(registration, necp_client_uuid,
                                          message_type,
                                          &parameters->u.group_members);
    } else {
      error = netagent_send_client_message(registration, necp_client_uuid,
                                           message_type);
    }
      ...
      if (NETAGENT_LOCK_SHARED_TO_EXCLUSIVE(registration)) {
        ...
        if (new_pending_client == NULL) {
          ...
          if (parameters != NULL) {
            ...
            uuid_copy(new_pending_client->client_proc_uuid,
                      parameters->u.nexus_request.euuid);
          } else {
            struct proc *p = current_proc();
            if (p != NULL) {
              new_pending_client->client_pid = proc_pid(p);
              proc_getexecutableuuid(
                  p, new_pending_client->client_proc_uuid,
                  sizeof(new_pending_client->client_proc_uuid));
            }
          }
          LIST_INSERT_HEAD(&registration->pending_triggers_list,
                           new_pending_client, client_chain);
        }
      } else {
  ...
  NETAGENTLOG(((error && error != ENOENT) ? LOG_ERR : LOG_INFO),
              "Send message %d for client (error %d)", message_type, error);
  if (message_type == NETAGENT_MESSAGE_TYPE_CLIENT_TRIGGER) {
    uuid_string_t uuid_str;
    uuid_unparse(agent_uuid, uuid_str);
    NETAGENTLOG(LOG_NOTICE, "Triggered network agent %s, error = %d", uuid_str,
                error);
  }
done:
...
int netagent_client_message(uuid_t agent_uuid, uuid_t necp_client_uuid,
                            pid_t pid, void *handle, u_int8_t message_type) {
  size_t dummy_length = 0;
  void *dummy_results __sized_by(dummy_length) = NULL;

  return netagent_client_message_with_params(agent_uuid, necp_client_uuid, pid,
                                             handle, message_type, NULL,
                                             &dummy_results, &dummy_length);
}

int netagent_use(uuid_t agent_uuid, uint64_t *out_use_count) {
  int error = 0;

  NETAGENT_LIST_LOCK_SHARED();
  struct netagent_registration *registration =
      netagent_find_agent_with_uuid_and_lock(agent_uuid, true, false);
  if (registration == NULL) {
    NETAGENTLOG0(LOG_ERR,
...
int netagent_copyout(uuid_t agent_uuid, user_addr_t user_addr,
                     u_int32_t user_size) {
  int error = 0;
  ...
  u_int32_t total_size =
      (sizeof(struct netagent) + registration->netagent->netagent_data_size);
  if (user_size < total_size) {
    NETAGENTLOG(LOG_ERR, "Provided user buffer is too small (%u < %u)",
                user_size, total_size);
    error = EINVAL;
    goto done;
  }

  u_int8_t *ptr = __unsafe_forge_bidi_indexable(
      u_int8_t *, registration->netagent, total_size);
  error = copyout(ptr, user_addr, total_size);

  NETAGENTLOG((error ? LOG_ERR : LOG_DEBUG), "Copied agent content (error %d)",
              error);
done:
  if (registration != NULL) {
    NETAGENT_UNLOCK(registration);
...
#define NETAGENT_TOKEN_EVENT_INTERVAL_NSEC                                     \
  (NSEC_PER_SEC * 10) // Only fire repeated events up to once every 10 seconds

int netagent_acquire_token(uuid_t agent_uuid, user_addr_t user_addr,
                           u_int32_t user_size, int *retval) {
  int error = 0;
    ...
    if (registration->token_low_water != 0) {
      ...
      if (mach_absolute_time() >= registration->need_tokens_event_deadline) {
        ...
        if (event_error == 0) {
          // Reset the deadline
          uint64_t deadline = 0;
          nanoseconds_to_absolutetime(NETAGENT_TOKEN_EVENT_INTERVAL_NSEC,
                                      &deadline);
  ...
  NETAGENTLOG((error ? LOG_ERR : LOG_DEBUG), "Copied token content (error %d)",
              error);
...
int netagent_trigger(struct proc *p, struct netagent_trigger_args *uap,
                     int32_t *retval) {
#pragma unused(p, retval)
  ...
  if ((registration->netagent->netagent_flags & NETAGENT_FLAG_USER_ACTIVATED) ==
      0) {
  ...
  error = netagent_send_trigger(registration, p, NETAGENT_TRIGGER_FLAG_USER,
                                NETAGENT_MESSAGE_TYPE_TRIGGER);
  NETAGENTLOG((error ? LOG_ERR : LOG_INFO), "Triggered netagent (error %d)",
              error);
done:
  if (registration != NULL) {
    NETAGENT_UNLOCK(registration);
