...
static TAILQ_HEAD(, proto_family_str)
    proto_family_head = TAILQ_HEAD_INITIALIZER(proto_family_head);
...
__private_extern__ void proto_unregister_input(protocol_family_t protocol) {
...
static void proto_delayed_attach(struct proto_input_entry *entry) {
  ...
  for (next_entry = entry->next; entry != NULL; entry = next_entry) {
    ...
    for (exist = proto_hash[hash_slot]; exist != NULL; exist = exist->next) {
...
__private_extern__ void proto_input_run(void) {
  struct proto_input_entry *entry;
  struct dlil_threading_info *inp = dlil_main_input_thread;
  mbuf_t packet_list;
  int i, locked = 0;
  ...
  for (i = 0; proto_total_waiting != 0 && i < PROTO_HASH_SLOTS; i++) {
    for (entry = proto_hash[i]; entry != NULL && proto_total_waiting;
         entry = entry->next) {
      if (entry->inject_first != NULL) {
        ...
        if (entry->domain != NULL &&
            !(entry->domain->dom_flags & DOM_REENTRANT)) {
        ...
        if (entry->chain) {
          ...
          for (packet = packet_list; packet != NULL; packet = packet_list) {
...
errno_t proto_input(protocol_family_t protocol, mbuf_t packet_list) {
  struct proto_input_entry *entry;
...
errno_t proto_inject(protocol_family_t protocol, mbuf_t packet_list) {
  struct proto_input_entry *entry;
  mbuf_t last_packet;
  ...
  for (entry = proto_hash[hash_slot]; entry != NULL; entry = entry->next) {
    if (entry->protocol == protocol) {
      break;
    }
  }
...
errno_t proto_register_plumber(protocol_family_t protocol_family,
  ...
  proto_family =
      kalloc_type(struct proto_family_str, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
void proto_unregister_plumber(protocol_family_t protocol_family,
                              ifnet_family_t interface_family) {
  struct proto_family_str *proto_family;
...
__private_extern__ errno_t proto_plumb(protocol_family_t protocol_family,
                                       ifnet_t ifp) {
  struct proto_family_str *proto_family;
  int ret = 0;
...
__private_extern__ errno_t proto_unplumb(protocol_family_t protocol_family,
                                         ifnet_t ifp) {
  struct proto_family_str *proto_family;
  int ret = 0;
