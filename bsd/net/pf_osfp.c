...
pf_osfp_find(struct pf_osfp_list *, struct pf_os_fingerprint *, u_int8_t);
 ...
 */
struct pf_osfp_enlist *pf_osfp_fingerprint(struct pf_pdesc *pd, pbuf_t *pbuf,
                                           int off, const struct tcphdr *tcp) {
  struct ip *__single ip;
  struct ip6_hdr *__single ip6;
  char hdr[60];

  if ((pd->af != PF_INET && pd->af != PF_INET6) || pd->proto != IPPROTO_TCP ||
...
pf_osfp_fingerprint_hdr(const struct ip *ip, const struct ip6_hdr *ip6,
                        const struct tcphdr *__sized_by(tcphdr_max_len) tcp,
...
#pragma unused(tcphdr_max_len)
  ...
  if (ip) {
    ...
    (void)inet_ntop(AF_INET, &ip->ip_src, srcname, (socklen_t)sizeof(srcname));
  } else if (ip6) {
    /* jumbo payload? */
  ...
  for (; tcpopt_cnt > 0; tcpopt_cnt -= optlen, tcpopt_ptr += optlen) {
    ...
    if (*tcpopt_ptr == TCPOPT_NOP) {
      fp.fp_tcpopts =
          (fp.fp_tcpopts << PF_OSFP_TCPOPT_BITS) | PF_OSFP_TCPOPT_NOP;
      optlen = 1;
    } else {
      if (tcpopt_cnt < 2) {
        return NULL;
      ...
      case TCPOPT_MAXSEG:
        if (optlen >= TCPOLEN_MAXSEG) {
          memcpy(&fp.fp_mss, &tcpopt_ptr[2], sizeof(fp.fp_mss));
        }
        fp.fp_tcpopts =
            (fp.fp_tcpopts << PF_OSFP_TCPOPT_BITS) | PF_OSFP_TCPOPT_MSS;
      ...
      case TCPOPT_WINDOW:
        if (optlen >= TCPOLEN_WINDOW) {
          memcpy(&fp.fp_wscale, &tcpopt_ptr[2], sizeof(fp.fp_wscale));
        }
        fp.fp_tcpopts =
            (fp.fp_tcpopts << PF_OSFP_TCPOPT_BITS) | PF_OSFP_TCPOPT_WSCALE;
        break;
      case TCPOPT_SACK_PERMITTED:
        fp.fp_tcpopts =
            (fp.fp_tcpopts << PF_OSFP_TCPOPT_BITS) | PF_OSFP_TCPOPT_SACK;
        break;
      case TCPOPT_TIMESTAMP:
        ...
        fp.fp_tcpopts =
            (fp.fp_tcpopts << PF_OSFP_TCPOPT_BITS) | PF_OSFP_TCPOPT_TS;
        break;
      default:
        return NULL;
      }
  ...
  DPFPRINTF("fingerprinted %s:%d  %d:%d:%d:%d:%llx (%d) "
            ...
            (fp.fp_flags & PF_OSFP_DF) != 0, fp.fp_psize,
            (long long int)fp.fp_tcpopts, fp.fp_optcnt,
  ...
  if ((fpresult = pf_osfp_find(&pf_osfp_list, &fp, PF_OSFP_MAXTTL_OFFSET))) {
...
int pf_osfp_match(struct pf_osfp_enlist *list, pf_osfp_t os) {
  ...
  SLIST_FOREACH(entry, list, fp_entry) {
    ...
    if ((os_class == PF_OSFP_ANY || en_class == os_class) &&
      ...
      DPFPRINTF("osfp matched %s %s %s  %x==%x\n", entry->fp_class_nm,
                entry->fp_version_nm, entry->fp_subtype_nm, os, entry->fp_os);
      return 1;
    }
  }
...
void pf_osfp_initialize(void) {
  pool_init(&pf_osfp_entry_pl, sizeof(struct pf_osfp_entry), 0, 0, 0,
            "pfosfpen", NULL);
  pool_init(&pf_osfp_pl, sizeof(struct pf_os_fingerprint), 0, 0, 0, "pfosfp",
            NULL);
...
/* Add a fingerprint */
int pf_osfp_add(struct pf_osfp_ioctl *fpioc) {
  ...
  DPFPRINTF("adding osfp %s %s %s = %s%d:%d:%d:%s%d:0x%llx %d "
            ...
            : (fpadd.fp_flags & PF_OSFP_WSIZE_DC)  ? "*"
                                                   : "",
            fpadd.fp_wsize, fpadd.fp_ttl, (fpadd.fp_flags & PF_OSFP_DF) ? 1 : 0,
            (fpadd.fp_flags & PF_OSFP_PSIZE_MOD)  ? "%"
            : (fpadd.fp_flags & PF_OSFP_PSIZE_DC) ? "*"
                                                  : "",
            fpadd.fp_psize, (long long int)fpadd.fp_tcpopts, fpadd.fp_optcnt,
            ...
            : (fpadd.fp_flags & PF_OSFP_WSCALE_DC) ? "*"
                                                   : "",
            fpadd.fp_wscale, fpioc->fp_os.fp_os);
...
struct pf_os_fingerprint *pf_osfp_find(struct pf_osfp_list *list,
                                       struct pf_os_fingerprint *find,
                                       u_int8_t ttldiff) {
...
#define MATCH_INT(_MOD, _DC, _field)                                           \
  ...
  SLIST_FOREACH(f, list, fp_next) {
    if (f->fp_tcpopts != find->fp_tcpopts || f->fp_optcnt != find->fp_optcnt ||
        f->fp_ttl < find->fp_ttl || f->fp_ttl - find->fp_ttl > ttldiff ||
...
#define SMART_MSS 1460
        if ((find->fp_wsize % find->fp_mss ||
             find->fp_wsize / find->fp_mss != f->fp_wsize) &&
            (find->fp_wsize % SMART_MSS ||
             find->fp_wsize / SMART_MSS != f->fp_wsize)) {
          continue;
        }
...
#define SMART_MTU (SMART_MSS + MTUOFF)
        if ((find->fp_wsize % (find->fp_mss + MTUOFF) ||
             find->fp_wsize / (find->fp_mss + MTUOFF) != f->fp_wsize) &&
            (find->fp_wsize % SMART_MTU ||
             find->fp_wsize / SMART_MTU != f->fp_wsize)) {
          continue;
        }
      } else if (f->fp_flags & PF_OSFP_WSIZE_MOD) {
        if (f->fp_wsize == 0 || find->fp_wsize % f->fp_wsize) {
          continue;
        }
      } else {
...
struct pf_os_fingerprint *pf_osfp_find_exact(struct pf_osfp_list *list,
                                             struct pf_os_fingerprint *find) {
  struct pf_os_fingerprint *f;

  SLIST_FOREACH(f, list, fp_next) {
    if (f->fp_tcpopts == find->fp_tcpopts && f->fp_wsize == find->fp_wsize &&
        f->fp_psize == find->fp_psize && f->fp_mss == find->fp_mss &&
        f->fp_flags == find->fp_flags && f->fp_optcnt == find->fp_optcnt &&
        f->fp_wscale == find->fp_wscale && f->fp_ttl == find->fp_ttl) {
      return f;
    }
  }
...
void pf_osfp_insert(struct pf_osfp_list *list, struct pf_os_fingerprint *ins) {
...
int pf_osfp_get(struct pf_osfp_ioctl *fpioc) {
  struct pf_os_fingerprint *fp;
  struct pf_osfp_entry *entry;
  ...
  SLIST_FOREACH(fp, &pf_osfp_list, fp_next) {
    SLIST_FOREACH(entry, &fp->fp_oses, fp_entry) {
      if (i++ == num) {
        ...
        fpioc->fp_getnum = num;
        memcpy(&fpioc->fp_os, entry, sizeof(fpioc->fp_os));
