...
#define KAOP_LOCK() lck_mtx_lock(&kaop_lock)
#define KAOP_LOCK_ASSERT_HELD() LCK_MTX_ASSERT(&kaop_lock, LCK_MTX_ASSERT_OWNED)
#define KAOP_LOCK_ASSERT_NOTHELD()                                             \
  LCK_MTX_ASSERT(&kaop_lock, LCK_MTX_ASSERT_NOTOWNED)
#define KAOP_UNLOCK() lck_mtx_unlock(&kaop_lock)

os_log_t kaop_log_handle = NULL;

#define _KAOPLOG(level, type, fmt, ...)                                        \
  do {                                                                         \
    os_log_with_type(kaop_log_handle, type, "%s - " fmt, __func__,             \
                     ##__VA_ARGS__);                                           \
  } while (0)

#define KAOPLOG(fmt, ...)                                                      \
  _KAOPLOG(kaop_log_handle, OS_LOG_TYPE_DEFAULT, fmt, ##__VA_ARGS__)
#define KAOPLOG_DEBUG(fmt, ...)                                                \
  _KAOPLOG(kaop_log_handle, OS_LOG_TYPE_DEBUG, fmt, ##__VA_ARGS__)
#define KAOPLOG_INFO(fmt, ...)                                                 \
  _KAOPLOG(kaop_log_handle, OS_LOG_TYPE_INFO, fmt, ##__VA_ARGS__)
#define KAOPLOG_ERR(fmt, ...)                                                  \
  _KAOPLOG(kaop_log_handle, OS_LOG_TYPE_ERROR, fmt, ##__VA_ARGS__)
...
#define KAOP_PROTOCOL_STATS                                                    \
  (KAOP_IP_STATS | KAOP_IP6_STATS | KAOP_TCP_STATS | KAOP_UDP_STATS)
...
SYSCTL_PROC(_net_aop, OID_AUTO, protocol_stats,
            ...
            KAOP_PROTOCOL_STATS, net_aop_stats_get_sysctl,
            "S,net_aop_protocol_stats", "AOP protocol statistics counter");
SYSCTL_PROC(_net_aop, OID_AUTO, proc_activity_bitmaps,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0,
            KAOP_PROC_ACTIVITY_BITMAPS, net_aop_stats_get_sysctl,
            "S,aop_proc_activity_bitmap", "AOP process activity bitmaps");
...
static errno_t
net_aop_validate_init_params(const struct net_aop_provider_init *init,
                             const uint32_t init_len) {
  errno_t err = 0;

  static_assert(
      __builtin_offsetof(struct net_aop_provider_init, kaopi_version) == 0);
...
static int configure_capab_flow_setup(net_aop_provider_handle_t prov,
  ...
  error = capab_fn(prov->kaop_prov_ctx, NET_AOP_CAPAB_FLOW_SETUP, &capab,
                   &capab_len);
  if (error != 0) {
...
static void unconfigure_capab_flow_setup(net_aop_provider_handle_t prov) {
...
static int configure_capab_flow_stats(net_aop_provider_handle_t prov,
                                      net_aop_config_fn_t capab_fn) {
  ...
  capab_len = sizeof(capab);
  error = capab_fn(prov->kaop_prov_ctx, NET_AOP_CAPAB_FLOW_STATS, &capab,
                   &capab_len);
  if (error != 0) {
...
static void unconfigure_capab_flow_stats(net_aop_provider_handle_t prov) {
...
static int configure_capab_stats(net_aop_provider_handle_t prov,
                                 net_aop_config_fn_t capab_fn) {
  ...
  capab_len = sizeof(capab);
  error =
      capab_fn(prov->kaop_prov_ctx, NET_AOP_CAPAB_STATS, &capab, &capab_len);
  if (error != 0) {
...
static void unconfigure_capab_stats(net_aop_provider_handle_t prov) {
...
static int configure_capab_process_bitmaps(net_aop_provider_handle_t prov,
                                           net_aop_config_fn_t capab_fn) {
...
static void unconfigure_capab_process_bitmaps(net_aop_provider_handle_t prov) {
...
static int net_aop_provider_initialize(net_aop_provider_handle_t prov) {
...
static void net_aop_provider_cleanup(net_aop_provider_handle_t prov) {
  ...
  if (os_ref_release(&prov->kaop_refcnt) != 0) {
    while (os_ref_get_count(&prov->kaop_refcnt) > 0) {
      msleep(&prov->kaop_refcnt, &kaop_lock, (PZERO + 1), __FUNCTION__, NULL);
    }
  }
...
static void net_aop_release_refcnt(net_aop_provider_handle_t prov) {
  KAOP_LOCK();
  if (os_ref_release(&prov->kaop_refcnt) == 0) {
...
int net_aop_get_flow_stats(uint32_t stats_index,
...
int net_aop_get_stats(net_aop_stats_type_t type,
  ...
  if (type == NET_AOP_STATS_TYPE_INVALID || type > NET_AOP_STATS_TYPE_MAX) {
...
int net_aop_get_proc_activity_bitmaps(
    struct aop_proc_activity_bitmap *proc_activity_bitmaps,
  ...
  KAOP_LOCK();
  if ((g_aop_net_provider.kaop_capabilities &
       KAOP_CAPAB_PROC_ACTIVITY_BITMAPS) == 0) {
    KAOP_UNLOCK();
...
net_aop_register_provider(const struct net_aop_provider_init *init,
  ...
  memcpy(&g_aop_net_provider.kaop_ext, init,
         sizeof(g_aop_net_provider.kaop_ext));
...
void net_aop_deregister_provider(net_aop_provider_handle_t prov) {
  if (prov == NULL) {
    return;
  }
...
net_aop_get_protocol_stats(struct net_aop_protocol_stats *aop_proto_stats) {
  ...
  error = net_aop_get_stats(NET_AOP_STATS_TYPE_IP,
                            (uint8_t *)&aop_proto_stats->aop_ip,
                            sizeof(aop_proto_stats->aop_ip));
  if (error != 0) {
    return error;
  }
  error = net_aop_get_stats(NET_AOP_STATS_TYPE_IPV6,
                            (uint8_t *)&aop_proto_stats->aop_ip6,
                            sizeof(aop_proto_stats->aop_ip6));
  if (error != 0) {
    return error;
  }
  error = net_aop_get_stats(NET_AOP_STATS_TYPE_TCP,
                            (uint8_t *)&aop_proto_stats->aop_tcp,
                            sizeof(aop_proto_stats->aop_tcp));
  if (error != 0) {
    return error;
  }
  error = net_aop_get_stats(NET_AOP_STATS_TYPE_UDP,
                            (uint8_t *)&aop_proto_stats->aop_udp,
...
static int net_aop_get_driver_stats(struct aop_driver_stats *driver_stats) {
  int error = 0;
  error = net_aop_get_stats(
      NET_AOP_STATS_TYPE_DRIVER,
      (uint8_t *)(struct aop_driver_stats * __bidi_indexable) driver_stats,
      sizeof(struct aop_driver_stats));
  return error;
}

static int
aop_get_process_activity_bitmaps(struct aop_proc_activity_bitmap **bitmaps,
                                 size_t requested_buffer_space,
                                 size_t *out_len) {
  size_t bitmap_size = 0;
  struct aop_proc_activity_bitmap *__sized_by(bitmap_size)
      proc_activity_bitmap = NULL;
  ...
  if (proc_bitmap_count > 0) {
    size_t requested_count =
        (requested_buffer_space / (sizeof(struct aop_proc_activity_bitmap)));
    requested_count = (requested_count > proc_bitmap_count) ? proc_bitmap_count
                                                            : requested_count;

    size_t required_buffer_len =
        (requested_count * sizeof(struct aop_proc_activity_bitmap));
    proc_activity_bitmap = (struct aop_proc_activity_bitmap *)kalloc_data(
        required_buffer_len, Z_WAITOK | Z_ZERO);
    bitmap_size = required_buffer_len;
    if (proc_activity_bitmap == NULL) {
      return ENOBUFS;
    }

    err = net_aop_get_proc_activity_bitmaps(proc_activity_bitmap,
...
#pragma unused(arg1, arg2)
  ...
  if (!kauth_cred_issuser(kauth_cred_get())) {
    p = current_proc();
    task = proc_task(p);
    bool has_aop_stats_entitlement =
        IOTaskHasEntitlement(task, "com.apple.private.network.aop_stats");
  ...
  if (req->oldptr != USER_ADDR_NULL && buffer_space != 0) {
    if (oidp->oid_arg2 == KAOP_PROTOCOL_STATS) {
      ...
      error =
          aop_get_process_activity_bitmaps(&bitmap, buffer_space, &out_size);
    ...
    if (oidp->oid_arg2 == KAOP_PROTOCOL_STATS) {
      ...
      net_aop_get_proc_activity_bitmaps(NULL, &proc_bitmap_count);
      buffer_space =
          (proc_bitmap_count * sizeof(struct aop_proc_activity_bitmap));
    }
