...
#define AOP_IP_STATS_TABLE(X)                                                  \
  ...
  X(AOP_IP_STATS_NOPROTO, "UnkwnProto",                                        \
    "\t\t%llu packet for unknown/unsupported protocol\n")                      \
  /* Output stats */                                                           \
  X(AOP_IP_STATS_LOCALOUT, "LocalOut", "\t%llu packet sent from this host\n")  \
  X(AOP_IP_STATS_ODROPPED, "DropNoBuf",                                        \
    "\t\t%llu output packet dropped due to no bufs, etc.\n")                   \
  X(AOP_IP_STATS_NOROUTE, "NoRoute",                                           \
    "\t\t%llu output packet discarded due to no route\n")                      \
  X(AOP_IP_STATS_FRAGMENTED, "Fragmented",                                     \
    "\t\t%llu output datagram fragmented\n")                                   \
  X(AOP_IP_STATS_OFRAGMENTS, "OutFraged", "\t\t%llu fragment created\n")       \
  X(AOP_IP_STATS_CANTFRAG, "CantFrag",                                         \
    "\t\t%llu datagram that can't be fragmented\n")                            \
...
#define AOP_IP6_STATS_TABLE(X)                                                 \
  ...
  X(AOP_IP6_STATS_TOOSMALL, "DataTooSmall",                                    \
    "\t\t%llu with size smaller than minimum\n")                               \
  X(AOP_IP6_STATS_TOOSHORT, "PktTooShort",                                     \
    "\t\t%llu with data size < data length\n")                                 \
  X(AOP_IP6_STATS_ADJ, "TotalAdj", "\t\t%llu with data size > data length\n")  \
  X(AOP_IP6_STATS_BADOPTIONS, "BadOptions", "\t\t%llu with bad options\n")     \
  X(AOP_IP6_STATS_BADVERS, "BadVer",                                           \
    "\t\t%llu with incorrect version number\n")                                \
  X(AOP_IP6_STATS_FRAGMENTS, "FrafRcvd", "\t\t%llu fragment received\n")       \
  X(AOP_IP6_STATS_FRAGDROPPED, "FragDrop",                                     \
    "\t\t\t%llu dropped (dup or out of space)\n")                              \
  X(AOP_IP6_STATS_FRAGTIMEOUT, "FragTimeO",                                    \
    "\t\t\t%llu dropped after timeout\n")                                      \
  X(AOP_IP6_STATS_FRAGOVERFLOW, "FragOverFlow", "\t\t\t%llu exceeded limit\n") \
  X(AOP_IP6_STATS_REASSEMBLED, "FragReassembled",                              \
    "\t\t\t%llu reassembled ok\n")                                             \
  X(AOP_IP6_STATS_DELIVERED, "Delivered", "\t\t%llu packet for this host\n")   \
  X(AOP_IP6_STATS_TOOMANYHDR, "TooManyHdr",                                    \
    "\t\t%llu packet discarded due to too may headers\n")                      \
  /* Output stats */                                                           \
  X(AOP_IP6_STATS_LOCALOUT, "LocalOut", "\t%llu packet sent from this host\n") \
  X(AOP_IP6_STATS_ODROPPED, "DropNoBuf",                                       \
    "\t\t%llu output packet dropped due to no bufs, etc.\n")                   \
  X(AOP_IP6_STATS_NOROUTE, "NoRoute",                                          \
    "\t\t%llu output packet discarded due to no route\n")                      \
  X(AOP_IP6_STATS_FRAGMENTED, "Fragmented",                                    \
    "\t\t%llu output datagram fragmented\n")                                   \
  X(AOP_IP6_STATS_OFRAGMENTS, "OutFraged", "\t\t%llu fragment created\n")      \
  X(AOP_IP6_STATS_CANTFRAG, "CantFrag",                                        \
    "\t\t%llu datagram that can't be fragmented\n")                            \
...
#define AOP_TCP_STATS_TABLE(X)                                                 \
  ...
  X(AOP_TCP_STATS_SNDREXMITPACK, "SndDPktReXmt",                               \
    "\t\t%llu data packet retransmitted")                                      \
  X(AOP_TCP_STATS_SNDREXMITBYTE, "SndDByteReXmt", " (%llu byte)\n")            \
  X(AOP_TCP_STATS_MTURESENT, "MTUReSnd",                                       \
    "\t\t%llu resend initiated by MTU discovery\n")                            \
  ...
  X(AOP_TCP_STATS_SYNCHALLENGE, "SYNChallenge",                                \
    "\t\t%llu challenge ACK sent due to unexpected SYN\n")                     \
  X(AOP_TCP_STATS_RSTCHALLENGE, "RSTChallenge",                                \
    "\t\t%llu challenge ACK sent due to unexpected RST\n")                     \
                                                                               \
  /* Input stats */                                                            \
  ...
  X(AOP_TCP_STATS_RCVACKTOOMUCH, "RcvAckUnSnd",                                \
    "\t\t%llu ack for unsent data\n")                                          \
  X(AOP_TCP_STATS_RCVPACK, "RcvPktInSeq",                                      \
    "\t\t%llu packet received in-sequence")                                    \
  X(AOP_TCP_STATS_RCVBYTE, "RcvBInSeq", " (%llu byte)\n")                      \
  X(AOP_TCP_STATS_RCVDUPPACK, "RcvDupPkt",                                     \
    "\t\t%llu completely duplicate packet")                                    \
  X(AOP_TCP_STATS_RCVDUPBYTE, "RcvDupByte", " (%llu byte)\n")                  \
  X(AOP_TCP_STATS_PAWSDROP, "PAWSDrop", "\t\t%llu old duplicate packet\n")     \
  X(AOP_TCP_STATS_RCVMEMDROP, "RcvMemDrop",                                    \
    "\t\t%llu received packet dropped due to low memory\n")                    \
  X(AOP_TCP_STATS_RCVPARTDUPPACK, "RcvDupData",                                \
    "\t\t%llu packet with some dup. data")                                     \
  ...
  X(AOP_TCP_STATS_RCVPACKAFTERWIN, "RcvAftWinPkt",                             \
    "\t\t%llu packet of data after window")                                    \
  X(AOP_TCP_STATS_RCVBYTEAFTERWIN, "RcvAftWinByte", " (%llu byte)\n")          \
  X(AOP_TCP_STATS_RCVWINPROBE, "RcvWinProbPkt", "\t\t%llu window probe\n")     \
  X(AOP_TCP_STATS_RCVWINUPD, "RcvWinUpdPkt",                                   \
    "\t\t%llu window update packet\n")                                         \
  X(AOP_TCP_STATS_RCVAFTERCLOSE, "RcvAftCloPkt",                               \
    "\t\t%llu packet received after close\n")                                  \
  X(AOP_TCP_STATS_BADRST, "BadRST", "\t\t%llu bad reset\n")                    \
  X(AOP_TCP_STATS_RCVBADSUM, "RcvBadCsum",                                     \
    "\t\t%llu discarded for bad checksum\n")                                   \
  X(AOP_TCP_STATS_RCVBADOFF, "RcvBadOff",                                      \
    "\t\t%llu discarded for bad header offset field\n")                        \
  X(AOP_TCP_STATS_RCVSHORT, "RcvTooShort",                                     \
    "\t\t%llu discarded because packet too short\n")                           \
  X(AOP_TCP_STATS_CONNATTEMPT, "ConnInit",                                     \
    "\t\t%llu discarded because packet too short\n")                           \
                                                                               \
  /* Connection stats */                                                       \
  X(AOP_TCP_STATS_CONNECTS, "ConnEst",                                         \
    "\t%llu connection established (including accepts)\n")                     \
  ...
  X(AOP_TCP_STATS_TIMEOUTDROP, "DropTO",                                       \
    "\t\t%llu connection dropped by rexmit timeout\n")                         \
  X(AOP_TCP_STATS_RXTFINDROP, "ReXmtFINDrop",                                  \
    "\t\t%llu connection dropped after retransmitting FIN\n")                  \
  X(AOP_TCP_STATS_PERSISTTIMEO, "PersistTO", "\t%llu persist timeout\n")       \
  X(AOP_TCP_STATS_PERSISTDROP, "PersisStateTO",                                \
    "\t\t%llu connection dropped by persist timeout\n")                        \
  X(AOP_TCP_STATS_KEEPTIMEO, "KATO", "\t%llu keepalive timeout\n")             \
  X(AOP_TCP_STATS_KEEPPROBE, "KAProbe", "\t\t%llu keepalive probe sent\n")     \
  X(AOP_TCP_STATS_KEEPDROPS, "KADrop",                                         \
    "\t\t%llu connection dropped by keepalive\n")                              \
                                                                               \
  /* SACK/RACK related stats */                                                \
  X(AOP_TCP_STATS_SACK_RECOVERY_EPISODE, "SACKRecEpi",                         \
    "\t%llu SACK recovery episode\n")                                          \
  X(AOP_TCP_STATS_SACK_REXMITS, "SACKReXmt",                                   \
    "\t%llu segment rexmit in SACK recovery episodes\n")                       \
  X(AOP_TCP_STATS_SACK_REXMIT_BYTES, "SACKReXmtB",                             \
    "\t%llu byte rexmit in SACK recovery episodes\n")                          \
  X(AOP_TCP_STATS_SACK_RCV_BLOCKS, "SACKRcvBlk",                               \
    "\t%llu SACK option (SACK blocks) received\n")                             \
  X(AOP_TCP_STATS_SACK_SEND_BLOCKS, "SACKSntBlk",                              \
    "\t%llu SACK option (SACK blocks) sent\n")                                 \
  X(AOP_TCP_STATS_SACK_SBOVERFLOW, "SACKSndBlkOF",                             \
    "\t%llu SACK scoreboard overflow\n")                                       \
                                                                               \
  ...
  X(AOP_TCP_STATS_SACK_ACKADV, "SACKAdvAck",                                   \
    "\t%llu time cumulative ack advanced along with SACK\n")                   \
  X(AOP_TCP_STATS_PTO, "ProbTO", "\t%llu probe timeout\n")                     \
  X(AOP_TCP_STATS_RTO_AFTER_PTO, "RTOAfProb",                                  \
    "\t\t%llu time retransmit timeout triggered after probe\n")                \
  X(AOP_TCP_STATS_PROBE_IF, "ProbeIF",                                         \
    "\t\t%llu time probe packets were sent for an interface\n")                \
  X(AOP_TCP_STATS_PROBE_IF_CONFLICT, "ProbeIFConfl",                           \
    "\t\t%llu time couldn't send probe packets for an interface\n")            \
  X(AOP_TCP_STATS_TLP_RECOVERY, "TLPFastRecvr",                                \
    "\t\t%llu time fast recovery after tail loss\n")                           \
  X(AOP_TCP_STATS_TLP_RECOVERLASTPKT, "TLPRecvrLPkt",                          \
    "\t\t%llu time recovered last packet \n")                                  \
  X(AOP_TCP_STATS_PTO_IN_RECOVERY, "PTOInRecvr",                               \
    "\t\t%llu SACK based rescue retransmit\n")                                 \
                                                                               \
  /* DSACK related statistics */                                               \
  X(AOP_TCP_STATS_DSACK_SENT, "DSACKSnd",                                      \
    "\t%llu time DSACK option was sent\n")                                     \
  X(AOP_TCP_STATS_DSACK_RECVD, "DSACKRcv",                                     \
    "\t\t%llu time DSACK option was received\n")                               \
  X(AOP_TCP_STATS_DSACK_DISABLE, "DSACKDisable",                               \
    "\t\t%llu time DSACK was disabled on a connection\n")                      \
  X(AOP_TCP_STATS_DSACK_BADREXMT, "DSACKBadReXmt",                             \
    "\t\t%llu time recovered from bad retransmission using DSACK\n")           \
  X(AOP_TCP_STATS_DSACK_ACKLOSS, "DSACKAckLoss",                               \
    "\t\t%llu time ignored DSACK due to ack loss\n")                           \
  X(AOP_TCP_STATS_DSACK_RECVD_OLD, "DSACKRcvOld",                              \
    "\t\t%llu time ignored old DSACK options\n")                               \
  X(AOP_TCP_STATS_PMTUDBH_REVERTED, "PMTUDBHRevert",                           \
    "\t%llu time PMTU Blackhole detection, size reverted\n")                   \
  X(AOP_TCP_STATS_DROP_AFTER_SLEEP, "DropAPSleep",                             \
    "\t%llu connection were dropped after long sleep\n")                       \
...
#define AOP_UDP_STATS_TABLE(X)                                                 \
  ...
  X(AOP_UDP_STATS_FULLSOCK, "FullSock",                                        \
...
#define DEFINE_STATS_STR_FUNC(type, table)                                     \
  __attribute__((always_inline)) static inline const char *type##_str(         \
      enum _##type value) {                                                    \
    static const char *table[] = {table(EXPAND_TO_STRING)};                    \
    return (table[value]);                                                     \
  }

#define DEFINE_STATS_FMT_FUNC(type, table)                                     \
  __attribute__((always_inline)) static inline const char *type##_fmt(         \
      enum _##type value) {                                                    \
    static const char *table[] = {table(EXPAND_TO_FORMAT)};                    \
    return (table[value]);                                                     \
  }
