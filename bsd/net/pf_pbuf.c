...
void pbuf_init_mbuf(pbuf_t *pbuf, struct mbuf *m, struct ifnet *ifp) {
...
void pbuf_init_memory(pbuf_t *pbuf, const struct pbuf_memory *mp,
                      struct ifnet *ifp) {
...
void pbuf_destroy(pbuf_t *pbuf) {
  if (pbuf->pb_type == PBUF_TYPE_MBUF) {
    ...
    (void)(pbuf->pb_memory.pm_action)(&pbuf->pb_memory, PBUF_ACTION_DESTROY);
  } else {
...
struct mbuf *pbuf_to_mbuf(pbuf_t *pbuf, boolean_t release_ptr) {
  struct mbuf *m = NULL;
  ...
  if (pbuf->pb_type == PBUF_TYPE_MBUF) {
    ...
    if (pbuf->pb_pftag != NULL) {
      ...
      ASSERT(pftag != NULL);
      *pftag = *pbuf->pb_pftag;
      fragtag = ((pftag->pftag_flags & PF_TAG_REASSEMBLED) != 0);
    }

    if (fragtag && pbuf->pb_pf_fragtag != NULL) {
      if (pf_copy_fragment_tag(m, pbuf->pb_pf_fragtag, M_NOWAIT) == NULL) {
        m_freem(m);
        m = NULL;
        goto done;
      }
...
struct mbuf *pbuf_clone_to_mbuf(pbuf_t *pbuf) {
  struct mbuf *m = NULL;
...
void *pbuf_ensure_writable(pbuf_t *pbuf, size_t len) {
  if (pbuf->pb_type == PBUF_TYPE_MBUF) {
    struct mbuf *m = pbuf->pb_mbuf;
...
pbuf_resize_segment(pbuf_t *pbuf, int off, int olen, int nlen) {
  ...
  if (pbuf->pb_type == PBUF_TYPE_MBUF) {
    ...
    delta_len = nlen - olen;
    VERIFY(nm->pm_offset + (nm->pm_len + delta_len) <= nm->pm_buffer_len);
...
void pbuf_copy_back(pbuf_t *pbuf, int off, int len,
                    void *__sized_by(src_buflen) src, size_t src_buflen) {
  VERIFY(off >= 0);
  VERIFY(len >= 0);
...
void pbuf_copy_data(pbuf_t *pbuf, int off, int len,
                    void *__sized_by(dst_buflen) dst, size_t dst_buflen) {
  VERIFY(off >= 0);
  VERIFY(len >= 0);
  VERIFY((u_int)(off + len) <= pbuf->pb_packet_len);
...
uint16_t pbuf_inet_cksum(const pbuf_t *pbuf, uint32_t nxt, uint32_t off,
                         uint32_t len) {
  uint16_t sum = 0;
...
uint16_t pbuf_inet6_cksum(const pbuf_t *pbuf, uint32_t nxt, uint32_t off,
                          uint32_t len) {
  uint16_t sum = 0;
  ...
  if (pbuf->pb_type == PBUF_TYPE_MBUF) {
    ...
    sum = inet6_cksum_buffer(pbuf->pb_data, nxt, off, len, pbuf->pb_contig_len);
  } else {
    panic("%s: bad pb_type: %d", __func__, pbuf->pb_type);
  }
...
mbuf_svc_class_t pbuf_get_service_class(const pbuf_t *pbuf) {
  if (pbuf->pb_type == PBUF_TYPE_MBUF) {
...
void *pbuf_get_packet_buffer_address(const pbuf_t *pbuf) {
  VERIFY(pbuf != NULL);
