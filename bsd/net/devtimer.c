...
#else  /* !DEVTIMER_DEBUG */
static __inline__ void _devtimer_printf(__unused const char *fmt, ...) {}
...
static __inline__ void timeval_add(struct timeval tv1, struct timeval tv2,
...
static __inline__ uint64_t timeval_to_absolutetime(struct timeval tv) {
  uint64_t secs;
  uint64_t usecs;

  clock_interval_to_absolutetime_interval(tv.tv_sec, NSEC_PER_SEC, &secs);
  clock_interval_to_absolutetime_interval(tv.tv_usec, NSEC_PER_USEC, &usecs);
  return secs + usecs;
}

__private_extern__ int devtimer_valid(devtimer_ref timer) {
  return timer->dt_callout != NULL;
}

__private_extern__ void devtimer_retain(devtimer_ref timer) {
  os_ref_retain(&timer->dt_retain_count);
}

__private_extern__ void devtimer_invalidate(devtimer_ref timer) {
...
__private_extern__ void devtimer_release(devtimer_ref timer) {
...
static void devtimer_process(void *param0, void *param1) {
...
__private_extern__ void *devtimer_arg0(devtimer_ref timer) {
...
devtimer_set_absolute(devtimer_ref timer, struct timeval abs_time,
                      devtimer_timeout_func timeout_func, void *arg1,
                      void *arg2) {
  if (timer->dt_callout == NULL) {
  ...
  thread_call_enter1_delayed(timer->dt_callout, &timer->dt_generation,
                             timeval_to_absolutetime(abs_time));
  return;
}

__private_extern__ void
devtimer_set_relative(devtimer_ref timer, struct timeval rel_time,
                      devtimer_timeout_func timeout_func, void *arg1,
                      void *arg2) {
...
__private_extern__ void devtimer_cancel(devtimer_ref timer) {
...
__private_extern__ int devtimer_enabled(devtimer_ref timer) {
  return timer->dt_timeout_func != NULL;
}

__private_extern__ int32_t devtimer_current_secs(void) {
  struct timeval tv;
...
__private_extern__ struct timeval devtimer_current_time(void) {
  struct timeval tv;
  clock_sec_t sec;
  clock_usec_t usec;
