...
static void dlil_output_cksum_dbg(struct ifnet *ifp, struct mbuf *m,
                                  uint32_t hoff, protocol_family_t pf);

#if CONFIG_DTRACE
static void dlil_output_dtrace(ifnet_t ifp, protocol_family_t proto_family,
                               mbuf_t m);
#endif /* CONFIG_DTRACE */
 ...
 * application.
 */
errno_t dlil_output(ifnet_t ifp, protocol_family_t proto_family,
                    mbuf_t packetlist, void *route, const struct sockaddr *dest,
                    int flags, struct flowadv *adv) {
...
preout_again:
  ...
  if (proto_family == PF_INET && IS_INTF_CLAT46(ifp) &&
      (ifp->if_type == IFT_CELLULAR || dlil_is_clat_needed(proto_family, m))) {
    ...
    if (!did_clat46 && proto_family == PF_INET6) {
      ...
      if (proto == NULL) {
        ifnet_lock_done(ifp);
        retval = ENXIO;
        m_drop_if(m, ifp, DROPTAP_FLAG_DIR_OUT, DROP_REASON_DLIL_CLAT64, NULL,
                  0);
        m = NULL;
        goto cleanup;
      }
      ...
      if (ifp->if_type == IFT_ETHER) {
         ...
         */
        rt = rtalloc1_scoped(SA(&dest6), 0, 0, ifp->if_index);
   ...
   */
  if (flags == DLIL_OUTPUT_FLAGS_NONE) {
    proto_media_preout preoutp =
        (proto->proto_kpi == kProtoKPI_v1 ? proto->kpi.v1.pre_output
                                          : proto->kpi.v2.pre_output);
    retval = 0;
    if (preoutp != NULL) {
      retval =
          preoutp(ifp, proto_family, &m, dest, route, frame_type, dst_linkaddr);

      if (retval != 0) {
        if (retval == EJUSTRETURN) {
          goto preout_again;
        }
        m_drop_if(m, ifp, DROPTAP_FLAG_DIR_OUT, DROP_REASON_DLIL_PRE_OUTPUT,
                  NULL, 0);
        m = NULL;
        goto cleanup;
      }
  ...
  do {
    ...
    if (flags == DLIL_OUTPUT_FLAGS_NONE && ifp->if_framer != NULL) {
       ...
       */
      if ((m->m_flags & (M_BCAST | M_LOOP)) && m->m_pkthdr.rcvif == NULL) {
        m->m_pkthdr.rcvif = ifp;
        rcvif_set = 1;
      }
      m_loop_set = m->m_flags & M_LOOP;
      retval =
          ifp->if_framer(ifp, &m, dest, dst_linkaddr, frame_type, &pre, &post);
      if (retval != 0) {
        if (retval != EJUSTRETURN) {
          m_drop_if(m, ifp, DROPTAP_FLAG_DIR_OUT, DROP_REASON_DLIL_IF_FRAMER,
                    NULL, 0);
        }
        goto next;
      }
      ...
      if (hwcksum_dbg != 0 && !(ifp->if_flags & IFF_LOOPBACK)) {
        dlil_output_cksum_dbg(ifp, m, pre, proto_family);
      }
    ...
    if ((flags & DLIL_OUTPUT_FLAGS_SKIP_IF_FILTERS) == 0) {
      ...
      if (retval != 0) {
        if (retval != EJUSTRETURN) {
          m_drop_if(m, ifp, DROPTAP_FLAG_DIR_OUT, DROP_REASON_DLIL_IF_FILTER,
                    NULL, 0);
        }
        goto next;
      }
    ...
    if (TSO_IPV4_NOTOK(ifp, m) || TSO_IPV6_NOTOK(ifp, m)) {
      retval = EMSGSIZE;
      m_drop_if(m, ifp, DROPTAP_FLAG_DIR_OUT, DROP_REASON_DLIL_TSO_NOT_OK, NULL,
                0);
      goto cleanup;
    }
      ...
      *send_tail = m;
      ...
      if (retval == EQFULL || retval == EQSUSPENDED) {
        if (adv != NULL && adv->code == FADV_SUCCESS) {
          adv->code =
              (retval == EQFULL ? FADV_FLOW_CONTROLLED : FADV_SUSPENDED);
        }
        retval = 0;
      }
...
dlil_interface_filters_output(struct ifnet *ifp, struct mbuf **m_p,
  ...
  TAILQ_FOREACH(filter, &ifp->if_flt_head, filt_next) {
    ...
    if (!filter->filt_skip && filter->filt_output != NULL &&
      ...
      result =
          filter->filt_output(filter->filt_cookie, ifp, protocol_family, m_p);
...
#pragma unused(ifp)
  ...
  case PF_INET6:
     ...
     * setting optlen to -1.
     */
    did_sw = in6_finalize_cksum(m, hoff, -1, -1, m->m_pkthdr.csum_flags);
...
dlil_output_dtrace(ifnet_t ifp, protocol_family_t proto_family, mbuf_t m) {
  if (proto_family == PF_INET) {
    ...
    DTRACE_IP6(send, struct mbuf *, m, struct inpcb *, NULL, struct ip6_hdr *,
               ip6, struct ifnet *, ifp, struct ip *, NULL, struct ip6_hdr *,
               ip6);
  }
}
#endif /* CONFIG_DTRACE */
