 ...
 *
 * Each client of NECP provides a set of parameters for a connection or network
 * state evaluation, on which NECP policy evaluation is run. This produces a
 * policy result which can be accessed by the originating process, along with
 * events for when policies results have changed.
 *
 * ------------------------------------------------
 * NECP Client FD
 * ------------------------------------------------
 * A process opens an NECP file descriptor using necp_open(). This is a very
 * simple file descriptor, upon which the process may do the following
 * operations:
 ...
 *
 * Client objects are allocated structures that hang off of the file descriptor.
 * Each client contains:
 *   - Client ID, a UUID that references the client across the system
 *   - Parameters, a buffer of TLVs that describe the client's connection
 * parameters, such as the remote and local endpoints, interface requirements,
 * etc.
 *   - Result, a buffer of TLVs containing the current policy evaluation for the
 * client. This result will be updated whenever a network change occurs that
 * impacts the policy result for that client.
 *
 *                   +--------------+
 *                   |   NECP fd    |
 *                   +--------------+
 ...
 * Client Actions
 * ------------------------------------------------
 *   - Add. Input parameters as a buffer of TLVs, and output a client ID.
 * Allocates a new client structure on the file descriptor.
 *   - Remove. Input a client ID. Removes a client structure from the file
 * descriptor.
 *   - Copy Parameters. Input a client ID, and output parameter TLVs.
 *   - Copy Result. Input a client ID, and output result TLVs. Alternatively,
 * input empty client ID and get next unread client result.
 *   - Copy List. List all client IDs.
 *
 * ------------------------------------------------
 * Client Policy Evaluation
 * ------------------------------------------------
 * Policies are evaluated for clients upon client creation, and upon update
 * events, which are network/agent/policy changes coalesced by a timer.
 *
 * The policy evaluation goes through the following steps:
 *   1. Parse client parameters.
 *   2. Select a scoped interface if applicable. This involves using
 * require/prohibit parameters, along with the local address, to select the most
 * appropriate interface if not explicitly set by the client parameters.
 ...
 * Client Observers
 * ------------------------------------------------
 * If necp_open() is called with the NECP_OPEN_FLAG_OBSERVER flag, and the
 * process passes the necessary privilege check, the fd is allowed to use
 * necp_client_action() to copy client state attached to the file descriptors of
 * other processes, and to list all client IDs on the system.
 */
...
static int necp_close_arenas_timeout_leeway_microseconds =
    1000 * 1000 * 1; // 1s
#endif               /* SKYWALK */
 ...
 *
 * necp_client_tracing_level  : 1 for client trace, 2 for flow trace, 3 for
 * parameter details necp_client_tracing_pid    : match client with pid
 */
...
#define NECP_CLIENT_TRACE_PID_MATCHED(pid) (pid == necp_client_tracing_pid)

#define NECP_ENABLE_CLIENT_TRACE(level)                                        \
  ((necp_client_tracing_level >= level &&                                      \
    (!necp_client_tracing_pid ||                                               \
     NECP_CLIENT_TRACE_PID_MATCHED(client->proc_pid)))                         \
       ? necp_client_tracing_level                                             \
       : 0)
...
#define NECP_CLIENT_FLOW_LOG(client, flow, fmt, ...)                           \
  if (client && flow &&                                                        \
      NECP_ENABLE_CLIENT_TRACE(NECP_CLIENT_TRACE_LEVEL_FLOW)) {                \
    uuid_string_t client_uuid_str = {};                                        \
    uuid_unparse_lower(client->client_id, client_uuid_str);                    \
    uuid_string_t flow_uuid_str = {};                                          \
    uuid_unparse_lower(flow->registration_id, flow_uuid_str);                  \
    NECPLOG(LOG_NOTICE,                                                        \
            "NECP CLIENT FLOW TRACE <pid %d %s> <flow %s>: " fmt "\n",         \
            client ? client->proc_pid : 0, client_uuid_str, flow_uuid_str,     \
            ##__VA_ARGS__);                                                    \
  }
...
#define NECP_ENABLE_SOCKET_TRACE(level)                                        \
  ((necp_client_tracing_level >= level &&                                      \
    (!necp_client_tracing_pid ||                                               \
     NECP_CLIENT_TRACE_PID_MATCHED(NECP_SOCKET_PID(so))))                      \
       ? necp_client_tracing_level                                             \
       : 0)
...
SYSCTL_INT(_net_necp, NECPCTL_CLIENT_FD_COUNT, client_fd_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_client_fd_count, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_OBSERVER_FD_COUNT, observer_fd_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_observer_fd_count, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_CLIENT_COUNT, client_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_client_count, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_SOCKET_FLOW_COUNT, socket_flow_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_socket_flow_count, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_IF_FLOW_COUNT, if_flow_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_if_flow_count, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_OBSERVER_MESSAGE_LIMIT, observer_message_limit,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_observer_message_limit, 256, "");
SYSCTL_INT(_net_necp, NECPCTL_CLIENT_TRACING_LEVEL, necp_client_tracing_level,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_client_tracing_level, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_CLIENT_TRACING_PID, necp_client_tracing_pid,
           CTLFLAG_LOCKED | CTLFLAG_RW, &necp_client_tracing_pid, 0, "");

#if SKYWALK
SYSCTL_INT(_net_necp, NECPCTL_ARENA_COUNT, arena_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_arena_count, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_SYSCTL_ARENA_COUNT, sysctl_arena_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_sysctl_arena_count, 0, "");
SYSCTL_INT(_net_necp, NECPCTL_NEXUS_FLOW_COUNT, nexus_flow_count,
           CTLFLAG_LOCKED | CTLFLAG_RD, &necp_nexus_flow_count, 0, "");
#if (DEVELOPMENT || DEBUG)
SYSCTL_UINT(_net_necp, OID_AUTO, collect_stats_interval_us,
            CTLFLAG_RW | CTLFLAG_LOCKED,
            &necp_collect_stats_timeout_microseconds, 0, "");
SYSCTL_UINT(_net_necp, OID_AUTO, necp_client_stats_rtt_floor,
            CTLFLAG_RW | CTLFLAG_LOCKED, &necp_client_stats_rtt_floor, 0, "");
SYSCTL_UINT(_net_necp, OID_AUTO, necp_client_stats_rtt_ceiling,
            CTLFLAG_RW | CTLFLAG_LOCKED, &necp_client_stats_rtt_ceiling, 0, "");
SYSCTL_INT(_net_necp, OID_AUTO, necp_client_stats_use_route_metrics,
           CTLFLAG_RW | CTLFLAG_LOCKED, &necp_client_stats_use_route_metrics, 0,
           "");
...
struct necp_client_parsed_parameters {
  ...
  struct necp_client_parameter_netagent_type
      required_netagent_types[NECP_MAX_AGENT_PARAMETERS];
  struct necp_client_parameter_netagent_type
      prohibited_netagent_types[NECP_MAX_AGENT_PARAMETERS];
  struct necp_client_parameter_netagent_type
      preferred_netagent_types[NECP_MAX_AGENT_PARAMETERS];
  struct necp_client_parameter_netagent_type
      avoided_netagent_types[NECP_MAX_AGENT_PARAMETERS];
...
static bool necp_find_matching_interface_index(
    struct necp_client_parsed_parameters *parsed_parameters,
    u_int *return_ifindex, bool *validate_agents);

static bool necp_ifnet_matches_local_address(struct ifnet *ifp,
                                             struct sockaddr *sa);

static bool necp_ifnet_matches_parameters(
    struct ifnet *ifp, struct necp_client_parsed_parameters *parsed_parameters,
    u_int32_t override_flags, u_int32_t *preferred_count,
    bool secondary_interface, bool require_scoped_field);
...
#if SKYWALK
  struct necp_arena_info
      *stats_arena;   /* arena where the stats objects came from */
  void *kstats_kaddr; /* kernel snapshot of untrusted userspace stats, for
                         calculating delta */
  mach_vm_address_t ustats_uaddr; /* userspace stats (untrusted) */
  nstat_userland_context stats_handler_context;
  struct flow_stats
      *nexus_stats; /* shared stats objects between necp_client and skywalk */
#endif              /* !SKYWALK */
  u_int64_t last_interface_details __attribute__((aligned(sizeof(u_int64_t))));
};

static int necp_client_flow_id_cmp(struct necp_client_flow_registration *flow0,
                                   struct necp_client_flow_registration *flow1);

RB_HEAD(_necp_client_flow_tree, necp_client_flow_registration);
RB_PROTOTYPE_PREV(_necp_client_flow_tree, necp_client_flow_registration,
                  client_link, necp_client_flow_id_cmp);
RB_GENERATE_PREV(_necp_client_flow_tree, necp_client_flow_registration,
                 client_link, necp_client_flow_id_cmp);

#define NECP_CLIENT_INTERFACE_OPTION_STATIC_COUNT 4
#define NECP_CLIENT_MAX_INTERFACE_OPTIONS 32

#define NECP_CLIENT_INTERFACE_OPTION_EXTRA_COUNT                               \
...
struct necp_client {
  ...
  struct necp_client_interface_option
      interface_options[NECP_CLIENT_INTERFACE_OPTION_STATIC_COUNT];
  struct necp_client_interface_option *__indexable extra_interface_options;
  u_int8_t interface_option_count; // Number in interface_options +
...
#define NECP_CLIENT_ASSERT_LOCKED(_c)                                          \
  LCK_MTX_ASSERT(&_c->lock, LCK_MTX_ASSERT_OWNED)
#define NECP_CLIENT_ASSERT_UNLOCKED(_c)                                        \
  LCK_MTX_ASSERT(&_c->lock, LCK_MTX_ASSERT_NOTOWNED)
...
static void necp_client_add_assertion(struct necp_client *client,
                                      uuid_t netagent_uuid);

static bool necp_client_remove_assertion(struct necp_client *client,
                                         uuid_t netagent_uuid);

static int necp_client_copy_parameters_locked(
    struct necp_client *client,
...
static int necp_client_id_cmp(struct necp_client *client0,
                              struct necp_client *client1);
...
RB_PROTOTYPE_PREV(_necp_client_global_tree, necp_client, global_link,
                  necp_client_id_cmp);
RB_GENERATE_PREV(_necp_client_global_tree, necp_client, global_link,
                 necp_client_id_cmp);

RB_HEAD(_necp_fd_flow_tree, necp_client_flow_registration);
RB_PROTOTYPE_PREV(_necp_fd_flow_tree, necp_client_flow_registration, fd_link,
                  necp_client_flow_id_cmp);
RB_GENERATE_PREV(_necp_fd_flow_tree, necp_client_flow_registration, fd_link,
                 necp_client_flow_id_cmp);

RB_HEAD(_necp_client_flow_global_tree, necp_client_flow_registration);
RB_PROTOTYPE_PREV(_necp_client_flow_global_tree, necp_client_flow_registration,
                  global_link, necp_client_flow_id_cmp);
RB_GENERATE_PREV(_necp_client_flow_global_tree, necp_client_flow_registration,
                 global_link, necp_client_flow_id_cmp);
...
#if SKYWALK
  // Arenas and their mmap info for per-process stats.  Stats objects are
  // allocated from an active arena that is not redirected/defunct.  The
  // stats_arena_active keeps track of such an arena, and it also holds a
  // reference count on the object.  Each flow allocating a stats object also
  // holds a reference the necp_arena_info (where the object got allocated
  // from).  During defunct, we redirect the mapping of the arena such that any
  // attempt to access (read/write) will result in getting zero-filled pages. We
  // then go thru all of the flows for the process and free the stats objects
  // associated with them, followed by destroying the skmem region(s) associated
  // with the arena.  The stats_arena_list keeps track of all current and
  // defunct stats arenas; there could be more than one arena created for the
  // process as the arena destruction happens when its reference count drops to
  // 0.
...
#define NECP_FD_ASSERT_LOCKED(_f)                                              \
  LCK_MTX_ASSERT(&_f->fd_lock, LCK_MTX_ASSERT_OWNED)
#define NECP_FD_ASSERT_UNLOCKED(_f)                                            \
  LCK_MTX_ASSERT(&_f->fd_lock, LCK_MTX_ASSERT_NOTOWNED)
...
#if SKYWALK
static KALLOC_TYPE_DEFINE(necp_arena_info_zone, struct necp_arena_info,
                          NET_KT_DEFAULT);
#endif /* !SKYWALK */
...
static LCK_RW_DECLARE_ATTR(necp_observer_lock, &necp_fd_mtx_grp,
                           &necp_fd_mtx_attr);
static LCK_RW_DECLARE_ATTR(necp_client_tree_lock, &necp_fd_mtx_grp,
                           &necp_fd_mtx_attr);
static LCK_RW_DECLARE_ATTR(necp_flow_tree_lock, &necp_fd_mtx_grp,
                           &necp_fd_mtx_attr);
static LCK_RW_DECLARE_ATTR(necp_collect_stats_list_lock, &necp_fd_mtx_grp,
                           &necp_fd_mtx_attr);

#define NECP_STATS_LIST_LOCK_EXCLUSIVE()                                       \
  lck_rw_lock_exclusive(&necp_collect_stats_list_lock)
#define NECP_STATS_LIST_LOCK_SHARED()                                          \
  lck_rw_lock_shared(&necp_collect_stats_list_lock)
#define NECP_STATS_LIST_UNLOCK() lck_rw_done(&necp_collect_stats_list_lock)

#define NECP_CLIENT_TREE_LOCK_EXCLUSIVE()                                      \
  lck_rw_lock_exclusive(&necp_client_tree_lock)
#define NECP_CLIENT_TREE_LOCK_SHARED()                                         \
  lck_rw_lock_shared(&necp_client_tree_lock)
#define NECP_CLIENT_TREE_UNLOCK() lck_rw_done(&necp_client_tree_lock)
#define NECP_CLIENT_TREE_ASSERT_LOCKED()                                       \
  LCK_RW_ASSERT(&necp_client_tree_lock, LCK_RW_ASSERT_HELD)

#define NECP_FLOW_TREE_LOCK_EXCLUSIVE()                                        \
  lck_rw_lock_exclusive(&necp_flow_tree_lock)
...
#define NECP_FLOW_TREE_ASSERT_LOCKED()                                         \
  LCK_RW_ASSERT(&necp_flow_tree_lock, LCK_RW_ASSERT_HELD)
...
#define NECP_FD_LIST_ASSERT_LOCKED()                                           \
  LCK_RW_ASSERT(&necp_fd_lock, LCK_RW_ASSERT_HELD)

#define NECP_OBSERVER_LIST_LOCK_EXCLUSIVE()                                    \
  lck_rw_lock_exclusive(&necp_observer_lock)
...
// Take NECP_CLIENT_TREE_LOCK when accessing or modifying the
// necp_client_global_tree Take NECP_FLOW_TREE_LOCK when accessing or modifying
// the necp_client_flow_global_tree Take NECP_STATS_LIST_LOCK when accessing or
// modifying the necp_collect_stats_flow_list Take NECP_FD_LOCK when accessing
// or modifying an necp_fd_data entry Take NECP_CLIENT_LOCK when accessing or
// modifying a single necp_client Take NECP_CLIENT_ROUTE_LOCK when accessing or
// modifying a client's route
...
static LCK_RW_DECLARE_ATTR(necp_update_all_clients_lock, &necp_fd_mtx_grp,
                           &necp_fd_mtx_attr);
#define NECP_UPDATE_ALL_CLIENTS_LOCK_EXCLUSIVE()                               \
  lck_rw_lock_exclusive(&necp_update_all_clients_lock)
#define NECP_UPDATE_ALL_CLIENTS_SHARED_TO_EXCLUSIVE()                          \
  lck_rw_lock_shared_to_exclusive(&necp_update_all_clients_lock)
#define NECP_UPDATE_ALL_CLIENTS_SHARED()                                       \
  lck_rw_lock_shared(&necp_update_all_clients_lock)
#define NECP_UPDATE_ALL_CLIENTS_UNLOCK()                                       \
  lck_rw_done(&necp_update_all_clients_lock)

// Array of PIDs that will trigger in-process flow divert, protected by
// NECP_FD_LIST_LOCK
...
static void necp_fd_insert_stats_arena(struct necp_fd_data *fd_data,
                                       struct necp_arena_info *nai);
static void necp_fd_remove_stats_arena(struct necp_fd_data *fd_data,
                                       struct necp_arena_info *nai);
...
static int
necp_stats_initialize(struct necp_fd_data *fd_data, struct necp_client *client,
                      struct necp_client_flow_registration *flow_registration,
                      struct necp_stats_bufreq *bufreq);
static int necp_arena_create(struct necp_fd_data *fd_data, size_t obj_size,
                             size_t obj_cnt, struct proc *p);
static int necp_arena_stats_obj_alloc(struct necp_fd_data *fd_data,
                                      mach_vm_offset_t *off,
                                      struct necp_arena_info **stats_arena,
                                      void **kstats_kaddr, boolean_t cansleep);
static void necp_arena_stats_obj_free(struct necp_fd_data *fd_data,
                                      struct necp_arena_info *stats_arena,
                                      void **kstats_kaddr,
                                      mach_vm_address_t *ustats_uaddr);
static void necp_stats_arenas_destroy(struct necp_fd_data *fd_data,
                                      boolean_t closing);

static int necp_sysctl_arena_initialize(struct necp_fd_data *fd_data,
                                        bool locked);
static void necp_sysctl_arena_destroy(struct necp_fd_data *fd_data);
static void *necp_arena_sysctls_obj(struct necp_fd_data *fd_data,
                                    mach_vm_offset_t *off, size_t *size);
#endif /* !SKYWALK */

static int necp_aop_offload_stats_initialize(
    struct necp_client_flow_registration *flow_registration,
    uuid_t netagent_uuid);
static void necp_aop_offload_stats_destroy(struct necp_client_flow *flow);

void necp_copy_inp_domain_info(struct inpcb *, struct socket *,
                               nstat_domain_info *);
void necp_with_inp_domain_name(
    struct socket *so, void *ctx,
    void (*with_func)(char *domain_name __null_terminated, void *ctx));

#if __has_ptrcheck
static inline __attribute__((always_inline))
__pure struct necp_client_flow_stats *__indexable
necp_client_get_flow_stats(const struct necp_client_add_flow *req) {
  if (req == NULL) {
    return NULL;
  }

  return __unsafe_forge_bidi_indexable(
      struct necp_client_flow_stats *, req->stats_requests,
      sizeof(struct necp_client_flow_stats) * req->stats_request_count);
}
#else
#define necp_client_get_flow_stats(req)                                        \
  ((struct necp_client_flow_stats *)&(req)->stats_requests[0])
#endif

#if __has_ptrcheck
static inline __attribute__((always_inline)) __pure uint8_t *__bidi_indexable
signable_get_data(const struct necp_client_signable *signable,
                  size_t data_length) {
  if (signable == NULL) {
    return NULL;
  }

  return __unsafe_forge_bidi_indexable(uint8_t *, signable->signable_data,
                                       data_length);
}
#else
...
#if __has_ptrcheck
static inline __attribute__((always_inline)) __pure struct sockaddr *
    __single flow_req_get_address(const struct necp_client_add_flow *req,
                                  size_t offset_of_address) {
  if (req == NULL) {
    return NULL;
  }

  uint8_t *__indexable req_ptr = __unsafe_forge_bidi_indexable(
      uint8_t *, req, sizeof(struct necp_client_add_flow));
  return __unsafe_forge_single(struct sockaddr *, req_ptr + offset_of_address);
}
#else
#define flow_req_get_address(req, offset_of_address)                           \
  ((struct sockaddr *)(((uint8_t *)req) + offset_of_address))
#endif

#if __has_ptrcheck
static inline __attribute__((always_inline)) __pure uint8_t *
    __single flow_req_get_proto(const struct necp_client_add_flow *req,
                                size_t offset_of_proto) {
  if (req == NULL) {
    return NULL;
  }
...
#else
#define flow_req_get_proto(req, offset_of_proto)                               \
  ((uint8_t *)(((uint8_t *)req) + offset_of_proto))
#endif

#if __has_ptrcheck
static inline __attribute__((always_inline)) __pure uint8_t *__bidi_indexable
necp_update_get_tlv_buffer(const struct necp_client_observer_update *update,
                           size_t buffer_size) {
  if (update == NULL) {
    return NULL;
  }

  return __unsafe_forge_bidi_indexable(uint8_t *, update->tlv_buffer,
                                       buffer_size);
}
#else
...
#if __has_ptrcheck
static inline __attribute__((always_inline)) __pure char *__bidi_indexable
necp_answer_get_hostname(const struct necp_client_host_resolver_answer *answer,
                         size_t hostname_length) {
  if (answer == NULL) {
    return NULL;
  }

  return __unsafe_forge_bidi_indexable(char *, answer->hostname,
                                       hostname_length);
}
#else
...
static void necp_lock_socket_attributes(void) {
  lck_mtx_lock(&necp_socket_attr_lock);
}

static void necp_unlock_socket_attributes(void) {
  lck_mtx_unlock(&necp_socket_attr_lock);
}

/// NECP file descriptor functions

static void necp_fd_notify(struct necp_fd_data *fd_data, bool locked) {
...
static inline bool necp_client_has_unread_flows(struct necp_client *client) {
  NECP_CLIENT_ASSERT_LOCKED(client);
  struct necp_client_flow_registration *flow_registration = NULL;
  RB_FOREACH(flow_registration, _necp_client_flow_tree,
             &client->flow_registrations) {
...
static int necp_fd_poll(struct necp_fd_data *fd_data, int events, void *wql,
                        struct proc *p, int is_kevent) {
  ...
  if (want_rx) {
    if (fd_data->flags & NECP_OPEN_FLAG_PUSH_OBSERVER) {
      ...
      RB_FOREACH(client, _necp_client_tree, &fd_data->clients) {
        ...
        if (!client->result_read || !client->group_members_read ||
...
static inline void necp_generate_client_id(uuid_t client_id, bool is_flow) {
...
static inline bool necp_client_id_is_flow(uuid_t client_id) {
  return client_id[9] & 0x01;
}

static struct necp_client *necp_find_client_and_lock(uuid_t client_id) {
  ...
  if (necp_client_id_is_flow(client_id)) {
    ...
    struct necp_client_flow_registration *flow = RB_FIND(
        _necp_client_flow_global_tree, &necp_client_flow_global_tree, &find);
    if (flow != NULL) {
      client = flow->client;
    }
...
necp_client_fd_find_client_unlocked(struct necp_fd_data *client_fd,
                                    uuid_t client_id) {
  ...
  if (necp_client_id_is_flow(client_id)) {
    struct necp_client_flow_registration find;
    uuid_copy(find.registration_id, client_id);
    struct necp_client_flow_registration *flow =
        RB_FIND(_necp_fd_flow_tree, &client_fd->flows, &find);
    if (flow != NULL) {
      client = flow->client;
    }
  } else {
...
static struct necp_client *
necp_client_fd_find_client_and_lock(struct necp_fd_data *client_fd,
                                    uuid_t client_id) {
  struct necp_client *client =
      necp_client_fd_find_client_unlocked(client_fd, client_id);
  if (client != NULL) {
    NECP_CLIENT_LOCK(client);
  }
...
static inline int necp_client_id_cmp(struct necp_client *client0,
                                     struct necp_client *client1) {
  return uuid_compare(client0->client_id, client1->client_id);
}

static inline int
necp_client_flow_id_cmp(struct necp_client_flow_registration *flow0,
                        struct necp_client_flow_registration *flow1) {
  return uuid_compare(flow0->registration_id, flow1->registration_id);
}

static int necpop_select(struct fileproc *fp, int which, void *wql,
                         vfs_context_t ctx) {
...
static void necp_fd_knrdetach(struct knote *kn) {
  struct necp_fd_data *fd_data =
      (struct necp_fd_data *)knote_kn_hook_get_raw(kn);
  struct selinfo *si = &fd_data->si;
...
static int necp_fd_knread(struct knote *kn, long hint) {
#pragma unused(kn, hint)
  return 1; /* assume we are ready */
}

static int necp_fd_knrprocess(struct knote *kn, struct kevent_qos_s *kev) {
  struct necp_fd_data *fd_data;
  int revents;
  int res;
...
SECURITY_READ_ONLY_EARLY(struct filterops)
necp_fd_rfiltops = {
    .f_isfd = 1,
...
static int necpop_kqfilter(struct fileproc *fp, struct knote *kn,
...
static uint64_t combine_interface_details(uint32_t interface_index,
                                          uint32_t interface_flags) {
  return ((uint64_t)interface_flags & INTERFACE_FLAGS_MASK)
             << INTERFACE_FLAGS_SHIFT |
         ((uint64_t)interface_index & INTERFACE_INDEX_MASK)
             << INTERFACE_INDEX_SHIFT;
}

#if SKYWALK

static void split_interface_details(uint64_t combined_details,
                                    uint32_t *interface_index,
                                    uint32_t *interface_flags) {
  *interface_index =
      (combined_details >> INTERFACE_INDEX_SHIFT) & INTERFACE_INDEX_MASK;
  *interface_flags =
      (combined_details >> INTERFACE_FLAGS_SHIFT) & INTERFACE_FLAGS_MASK;
}

static void necp_flow_save_current_interface_details(
    struct necp_client_flow_registration *flow_registration) {
  struct necp_client_flow *flow = NULL;
  LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
    if (flow->nexus) {
      uint64_t combined_details = combine_interface_details(
          flow->interface_index, flow->interface_flags);
      os_atomic_store(&flow_registration->last_interface_details,
                      combined_details, release);
      break;
    }
  }
}

static void necp_client_collect_interface_stats(
    struct necp_client_flow_registration *flow_registration,
...
static void necp_client_collect_aop_flow_stats(
    struct necp_client_flow_registration *flow_registration) {
  ...
  struct necp_all_kstats *kstats =
      (struct necp_all_kstats *)flow_registration->kstats_kaddr;
  if (kstats == NULL) {
    return;
  }

  struct necp_stat_counts *prev_tcpstats =
      &(((struct necp_tcp_stats *)&kstats->necp_stats_comm)->necp_tcp_counts);
  ...
  LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
    ...
    if (flow->flow_tag > 0 && flow->aop_stat_index_valid) {
      ...
      if (err != 0) {
        NECPLOG(LOG_ERR,
                "failed to get aop flow stats "
                "for flow id %u with error %d",
                flow->flow_tag, err);
        continue;
      }
      ...
      uint32_t d_rxpackets =
          tcpi->tcpi_rxpackets - prev_tcpstats->necp_stat_rxpackets;
      prev_tcpstats->necp_stat_rxpackets += d_rxpackets;

      uint32_t d_txpackets =
          tcpi->tcpi_txpackets - prev_tcpstats->necp_stat_txpackets;
      prev_tcpstats->necp_stat_txpackets += d_txpackets;

      uint32_t d_rxbytes =
          tcpi->tcpi_rxbytes - prev_tcpstats->necp_stat_rxbytes;
      prev_tcpstats->necp_stat_rxbytes += d_rxbytes;

      uint32_t d_txbytes =
          tcpi->tcpi_txbytes - prev_tcpstats->necp_stat_txbytes;
      prev_tcpstats->necp_stat_txbytes += d_txbytes;

      uint32_t d_rxduplicatebytes = tcpi->tcpi_rxduplicatebytes -
                                    prev_tcpstats->necp_stat_rxduplicatebytes;
      prev_tcpstats->necp_stat_rxduplicatebytes += d_rxduplicatebytes;

      uint32_t d_rxoutoforderbytes = tcpi->tcpi_rxoutoforderbytes -
                                     prev_tcpstats->necp_stat_rxoutoforderbytes;
      prev_tcpstats->necp_stat_rxoutoforderbytes += d_rxoutoforderbytes;

      uint32_t d_txretransmit =
          tcpi->tcpi_txretransmitbytes - prev_tcpstats->necp_stat_txretransmit;
      prev_tcpstats->necp_stat_txretransmit += d_txretransmit;

      uint32_t d_connectattempts = prev_tcpstats->necp_stat_connectattempts -
                                   (tcpi->tcpi_state >= TCPS_SYN_SENT ? 1 : 0);
      prev_tcpstats->necp_stat_connectattempts += d_connectattempts;

      uint32_t d_connectsuccesses =
          prev_tcpstats->necp_stat_connectsuccesses -
          (tcpi->tcpi_state >= TCPS_ESTABLISHED ? 1 : 0);
      ...
      if (route != NULL) {
        nstat_route_update(route, d_connectattempts, d_connectsuccesses,
                           d_rxpackets, d_rxbytes, d_rxduplicatebytes,
                           d_rxoutoforderbytes, d_txpackets, d_txbytes,
                           d_txretransmit, prev_tcpstats->necp_stat_avg_rtt,
                           prev_tcpstats->necp_stat_var_rtt);
      }
...
static void necp_client_collect_nexus_flow_stats(
    struct necp_client_flow_registration *flow_registration) {
  ASSERT(!flow_registration->aop_offload);
  ...
  struct necp_tcp_stats *curr_tcpstats =
      (struct necp_tcp_stats *)kstats->necp_stats_ustats;
  struct necp_tcp_stats *prev_tcpstats =
      (struct necp_tcp_stats *)&kstats->necp_stats_comm;
#define diff_n_update(field)                                                   \
  u_int32_t d_##field = (curr_tcpstats->necp_tcp_counts.necp_stat_##field -    \
                         prev_tcpstats->necp_tcp_counts.necp_stat_##field);    \
  ...
  uint32_t rtt = prev_tcpstats->necp_tcp_counts.necp_stat_avg_rtt =
      curr_tcpstats->necp_tcp_counts.necp_stat_avg_rtt;
  uint32_t rtt_var = prev_tcpstats->necp_tcp_counts.necp_stat_var_rtt =
      curr_tcpstats->necp_tcp_counts.necp_stat_var_rtt;
#undef diff_n_update

  // Do malicious stats detection with the deltas here.
  // RTT check (not necessarily attacks, might just be not measured since we
  // report stats async periodically).
  if (rtt < necp_client_stats_rtt_floor ||
      rtt > necp_client_stats_rtt_ceiling) {
  ...
  if (route != NULL) {
    nstat_route_update(route, d_connectattempts, d_connectsuccesses,
                       d_rxpackets, d_rxbytes, d_rxduplicatebytes,
                       d_rxoutoforderbytes, d_txpackets, d_txbytes,
                       d_txretransmit, rtt, rtt_var);
  }
  NECP_CLIENT_ROUTE_UNLOCK(flow_registration->client);
}

static void necp_client_collect_stats(
    struct necp_client_flow_registration *flow_registration) {
...
// This is called from various places; "closing" here implies the client being
// closed/removed if true, otherwise being defunct.  In the former, we expect
// the caller to not hold the lock; for the latter it must have acquired it.
static void
necp_destroy_flow_stats(struct necp_fd_data *fd_data,
  ...
  if (flow_registration->kstats_kaddr != NULL) {
    ...
    const bool destroyed = necp_client_release_locked(
        client); // Drop the reference held by the stats list
    ASSERT(!destroyed);
    (void)destroyed;
    ...
    necp_arena_stats_obj_free(fd_data, flow_registration->stats_arena,
                              &flow_registration->kstats_kaddr,
...
static void necp_schedule_collect_stats_clients(bool recur) {
  ...
  uint64_t deadline = 0;
  uint64_t leeway = 0;
  clock_interval_to_deadline(necp_collect_stats_timeout_microseconds,
                             NSEC_PER_USEC, &deadline);
  clock_interval_to_absolutetime_interval(
      necp_collect_stats_timeout_leeway_microseconds, NSEC_PER_USEC, &leeway);
...
necp_collect_stats_client_callout(__unused thread_call_param_t dummy,
  ...
  LIST_FOREACH(flow_registration, &necp_collect_stats_flow_list,
               collect_stats_chain) {
...
static void necp_defunct_flow_registration(
    struct necp_client *client,
    struct necp_client_flow_registration *flow_registration,
  ...
  if (!flow_registration->defunct) {
    ...
    LIST_FOREACH(search_flow, &flow_registration->flow_list, flow_chain) {
      ...
      if (search_flow->nexus && !uuid_is_null(search_flow->u.nexus_agent)) {
        should_defunct_flow = true;
      } else if (defunct_socket_flows && search_flow->socket &&
      ...
      if (should_defunct_flow) {
        ...
        if (defunct_list != NULL) {
          ...
          struct necp_flow_defunct *flow_defunct =
              kalloc_type(struct necp_flow_defunct, Z_WAITOK | Z_ZERO);
...
#if SKYWALK
          if (flow_registration->kstats_kaddr != NULL) {
            struct necp_all_stats *ustats_kaddr =
                ((struct necp_all_kstats *)flow_registration->kstats_kaddr)
                    ->necp_stats_ustats;
            struct necp_quic_stats *quicstats =
                (struct necp_quic_stats *)ustats_kaddr;
            if (quicstats != NULL) {
              memcpy(flow_defunct->close_parameters.u.close_token,
                     quicstats->necp_quic_extra.ssr_token,
                     sizeof(flow_defunct->close_parameters.u.close_token));
...
necp_defunct_client_for_policy(struct necp_client *client,
  ...
  RB_FOREACH(flow_registration, _necp_client_flow_tree,
    ...
    necp_defunct_flow_registration(client, flow_registration, defunct_list,
                                   defunct_socket_flows);
  }
}

static void necp_client_free(struct necp_client *client) {
  NECP_CLIENT_ASSERT_UNLOCKED(client);

  kfree_data(client->extra_interface_options,
             sizeof(struct necp_client_interface_option) *
  ...
  kfree_data_counted_by(client->assigned_group_members,
                        client->assigned_group_members_length);
...
necp_client_update_alloc(const void *__sized_by(length) data, size_t length) {
  ...
  client_update =
      kalloc_type(struct necp_client_update, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static void necp_client_update_free(struct necp_client_update *client_update) {
  kfree_data_sized_by(client_update->update, client_update->update_length);
  kfree_type(struct necp_client_update, client_update);
}

static void
necp_client_update_observer_add_internal(struct necp_fd_data *observer_fd,
                                         struct necp_client *client) {
  struct necp_client_update *client_update;
  ...
  client_update =
      necp_client_update_alloc(client->parameters, client->parameters_length);
...
static void
necp_client_update_observer_update_internal(struct necp_fd_data *observer_fd,
                                            struct necp_client *client) {
  NECP_FD_LOCK(observer_fd);
  ...
  struct necp_client_update *client_update =
      necp_client_update_alloc(client->result, client->result_length);
  if (client_update != NULL) {
    uuid_copy(client_update->client_id, client->client_id);
...
static void
necp_client_update_observer_remove_internal(struct necp_fd_data *observer_fd,
                                            struct necp_client *client) {
  NECP_FD_LOCK(observer_fd);
...
static void necp_client_update_observer_add(struct necp_client *client) {
...
static void necp_client_update_observer_update(struct necp_client *client) {
  NECP_OBSERVER_LIST_LOCK_SHARED();
...
static void necp_client_update_observer_remove(struct necp_client *client) {
  NECP_OBSERVER_LIST_LOCK_SHARED();
...
static void necp_destroy_client_flow_registration(
    struct necp_client *client,
    struct necp_client_flow_registration *flow_registration, pid_t pid,
    bool abort) {
  NECP_CLIENT_ASSERT_LOCKED(client);

  bool has_close_parameters = false;
  struct necp_client_agent_parameters close_parameters = {};
  memset(close_parameters.u.close_token, 0,
         sizeof(close_parameters.u.close_token));
#if SKYWALK
  if (flow_registration->kstats_kaddr != NULL) {
    struct necp_all_stats *ustats_kaddr =
        ((struct necp_all_kstats *)flow_registration->kstats_kaddr)
    ...
    if (quicstats != NULL &&
        quicstats->necp_quic_udp_stats.necp_udp_hdr.necp_stats_type ==
            NECP_CLIENT_STATISTICS_TYPE_QUIC) {
      memcpy(close_parameters.u.close_token,
             quicstats->necp_quic_extra.ssr_token,
             sizeof(close_parameters.u.close_token));
  ...
  LIST_FOREACH_SAFE(search_flow, &flow_registration->flow_list, flow_chain,
                    temp_flow) {
    if (search_flow->nexus && !uuid_is_null(search_flow->u.nexus_agent)) {
      ...
      if (!flow_registration->defunct) {
        ...
        int netagent_error = netagent_client_message_with_params(
            search_flow->u.nexus_agent,
            ((flow_registration->flags & NECP_CLIENT_FLOW_FLAGS_USE_CLIENT_ID)
                 ? client->client_id
                 : flow_registration->registration_id),
            pid, client->agent_handle, message_type,
            has_close_parameters ? &close_parameters : NULL, &dummy_results,
            &dummy_length);
        if (netagent_error != 0 && netagent_error != ENOENT) {
          NECPLOG(LOG_ERR,
                  "necp_client_remove close nexus error (%d) MESSAGE TYPE %u",
                  netagent_error, message_type);
        }
      }
      uuid_clear(search_flow->u.nexus_agent);
    }
    if (search_flow->assigned_results != NULL) {
      kfree_data_counted_by(search_flow->assigned_results,
                            search_flow->assigned_results_length);
    }
  ...
  RB_REMOVE(_necp_client_flow_tree, &client->flow_registrations,
            flow_registration);
...
static void necp_destroy_client(struct necp_client *client, pid_t pid,
                                bool abort) {
  NECP_CLIENT_ASSERT_UNLOCKED(client);
  ...
  RB_FOREACH_SAFE(flow_registration, _necp_client_flow_tree,
                  &client->flow_registrations, temp_flow_registration) {
    necp_destroy_client_flow_registration(client, flow_registration, pid,
                                          abort);
  }
  ...
  LIST_FOREACH_SAFE(search_assertion, &client->assertion_list, assertion_chain,
                    temp_assertion) {
    int netagent_error = netagent_client_message(
        search_assertion->asserted_netagent, client->client_id, pid,
...
static bool necp_defunct_client_fd_locked_inner(
    struct necp_fd_data *client_fd,
    struct _necp_flow_defunct_list *defunct_list, bool destroy_stats);
...
necp_process_defunct_list(struct _necp_flow_defunct_list *defunct_list) {
  if (!LIST_EMPTY(defunct_list)) {
    ...
    // For each newly defunct client, send a message to the nexus to remove the
    // flow
    LIST_FOREACH_SAFE(flow_defunct, defunct_list, chain, temp_flow_defunct) {
      if (!uuid_is_null(flow_defunct->nexus_agent)) {
        ...
        void *__sized_by(dummy_length) dummy_results = NULL;
        int netagent_error = netagent_client_message_with_params(
            flow_defunct->nexus_agent, flow_defunct->flow_id,
            flow_defunct->proc_pid, flow_defunct->agent_handle, message_type,
            flow_defunct->has_close_parameters ? &flow_defunct->close_parameters
                                               : NULL,
        ...
        if (netagent_error != 0) {
          ...
          NECPLOG((netagent_error == ENOENT ? LOG_DEBUG : LOG_ERR),
                  "necp_update_client abort nexus error (%d) for pid %d %s",
                  netagent_error, flow_defunct->proc_pid, namebuf);
        }
...
static int necpop_close(struct fileglob *fg, vfs_context_t ctx) {
#pragma unused(ctx)
  ...
  if (fd_data != NULL) {
    ...
    struct necp_client_flow_registration *flow_registration = NULL;
    struct necp_client_flow_registration *temp_flow_registration = NULL;
    RB_FOREACH_SAFE(flow_registration, _necp_fd_flow_tree, &fd_data->flows,
    ...
    TAILQ_FOREACH_SAFE(client_update, &fd_data->update_list, chain,
                       temp_update) {
...
necp_address_is_wildcard(const union necp_sockaddr_union *const addr) {
  return (addr->sa.sa_family == AF_INET &&
         ...
         (addr->sa.sa_family == AF_INET6 &&
          IN6_IS_ADDR_UNSPECIFIED(&addr->sin6.sin6_addr));
}

static int necp_find_fd_data(struct proc *p, int fd, struct fileproc **fpp,
                             struct necp_fd_data **fd_data) {
...
static void necp_client_add_nexus_flow(
    struct necp_client_flow_registration *flow_registration, uuid_t nexus_agent,
    uint32_t interface_index, uint32_t interface_flags, bool aop_offload) {
  struct necp_client_flow *new_flow =
      kalloc_type(struct necp_client_flow, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static void necp_client_add_nexus_flow_if_needed(
    struct necp_client_flow_registration *flow_registration, uuid_t nexus_agent,
    uint32_t interface_index, bool aop_offload) {
  struct necp_client_flow *flow = NULL;
  LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
    if (flow->nexus && uuid_compare(flow->u.nexus_agent, nexus_agent) == 0) {
      return;
    }
  }
  ...
  if (interface_index != IFSCOPE_NONE &&
      interface_index <= (u_int32_t)if_index) {
  ...
  ifnet_head_done();
  necp_client_add_nexus_flow(flow_registration, nexus_agent, interface_index,
                             interface_flags, aop_offload);
}

static struct necp_client_flow *necp_client_add_interface_flow(
    struct necp_client_flow_registration *flow_registration,
    uint32_t interface_index) {
  struct necp_client_flow *new_flow =
      kalloc_type(struct necp_client_flow, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static struct necp_client_flow *necp_client_add_interface_flow_if_needed(
    struct necp_client *client,
    struct necp_client_flow_registration *flow_registration,
    uint32_t interface_index) {
  if (!client->allow_multiple_flows || interface_index == IFSCOPE_NONE) {
...
static void necp_client_add_interface_option_if_needed(
    struct necp_client *client, uint32_t interface_index,
    uint32_t interface_generation, uuid_t *nexus_agent, bool network_provider) {
  ...
  for (u_int32_t option_i = 0; option_i < client->interface_option_count;
       option_i++) {
    if (option_i < NECP_CLIENT_INTERFACE_OPTION_STATIC_COUNT) {
      struct necp_client_interface_option *option =
          &client->interface_options[option_i];
      ...
      struct necp_client_interface_option *option =
          &client->extra_interface_options
               [option_i - NECP_CLIENT_INTERFACE_OPTION_STATIC_COUNT];
      if (option->interface_index == interface_index) {
        if (nexus_agent == NULL) {
          return;
        }
  ...
  // Add a new entry
  if (client->interface_option_count <
      NECP_CLIENT_INTERFACE_OPTION_STATIC_COUNT) {
    // Add to static
    struct necp_client_interface_option *option =
        &client->interface_options[client->interface_option_count];
    ...
    if (client->extra_interface_options == NULL) {
      client->extra_interface_options =
          (struct necp_client_interface_option *)kalloc_data(
              sizeof(struct necp_client_interface_option) *
                  NECP_CLIENT_INTERFACE_OPTION_EXTRA_COUNT,
              Z_WAITOK | Z_ZERO);
    }
...
static bool necp_client_flow_is_viable(proc_t proc, struct necp_client *client,
                                       struct necp_client_flow *flow) {
  struct necp_aggregate_result result;
  bool ignore_address =
      (client->allow_multiple_flows && !flow->nexus && !flow->socket);

  flow->necp_flow_flags = 0;
  int error = necp_application_find_policy_match_internal(
      proc, client->parameters, (u_int32_t)client->parameters_length, &result,
      &flow->necp_flow_flags, NULL, flow->interface_index, &flow->local_addr,
      &flow->remote_addr, NULL, NULL, NULL, ignore_address, true, NULL);
  ...
  for (int i = 0; i < NECP_MAX_NETAGENTS; i++) {
    ...
    if ((flags & NETAGENT_FLAG_REGISTERED) &&
        !(flags & NETAGENT_FLAG_VOLUNTARY) && !(flags & NETAGENT_FLAG_ACTIVE) &&
  ...
  return error == 0 && result.routed_interface_index != IFSCOPE_NONE &&
         result.routing_result != NECP_KERNEL_POLICY_RESULT_DROP;
}

static void necp_flow_add_interface_flows(
    proc_t proc, struct necp_client *client,
    struct necp_client_flow_registration *flow_registration,
    bool send_initial) {
  ...
  for (u_int32_t option_i = 0; option_i < client->interface_option_count;
    ...
    if (option_i < NECP_CLIENT_INTERFACE_OPTION_STATIC_COUNT) {
      struct necp_client_interface_option *option =
          &client->interface_options[option_i];
      struct necp_client_flow *flow = necp_client_add_interface_flow_if_needed(
          client, flow_registration, option->interface_index);
      if (flow != NULL && send_initial) {
        ...
        if (flow->viable && flow->u.cb) {
          ...
          flow->u.cb(flow_registration->interface_handle,
                     NECP_CLIENT_CBACTION_INITIAL, flow->interface_index,
                     flow->necp_flow_flags, &viable);
          flow->viable = viable;
        }
      }
    } else {
...
necp_client_update_flows(proc_t proc, struct necp_client *client,
  ...
  RB_FOREACH(flow_registration, _necp_client_flow_tree,
    ...
    LIST_FOREACH_SAFE(flow, &flow_registration->flow_list, flow_chain,
                      temp_flow) {
      ...
      if (flow->viable && client_updated &&
        ...
        flow->u.cb(flow->u.socket_handle, NECP_CLIENT_CBACTION_VIABLE,
                   flow->interface_index, flow->necp_flow_flags, &flow_viable);
      ...
      if (!flow->viable || flow->invalid) {
        if (client_updated &&
            (flow->socket || (!flow->socket && !flow->nexus)) && flow->u.cb) {
          bool flow_viable = flow->viable;
          flow->u.cb(flow->u.socket_handle, NECP_CLIENT_CBACTION_NONVIABLE,
                     flow->interface_index, flow->necp_flow_flags,
                     &flow_viable);
          flow->viable = flow_viable;
        }
...
#if SKYWALK
      if (defunct_list != NULL) {
        if (flow->invalid && flow->nexus && flow->assigned &&
            !uuid_is_null(flow->u.nexus_agent)) {
          ...
          if (!(flags & NETAGENT_FLAG_REGISTERED)) {
            ...
            necp_defunct_flow_registration(client, flow_registration,
                                           defunct_list, false);
            client_updated = TRUE;
          }
        }
      ...
      if (!flow->viable || flow->invalid) {
        ...
        if (!flow->nexus && !flow->assigned) {
          if (flow->assigned_results != NULL) {
            kfree_data_counted_by(flow->assigned_results,
...
static inline bool necp_netagent_is_requested(
  ...
  if (parameters != NULL) {
    ...
    for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
      ...
      if (uuid_compare(parameters->required_netagents[i], *netagent_uuid) ==
          0) {
        requested = true;
        break;
      }
    ...
    if (!requested) {
      ...
      for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
        if (strbuflen(parameters->required_netagent_types[i].netagent_domain,
                      sizeof(parameters->required_netagent_types[i]
                                 .netagent_domain)) == 0 ||
            strbuflen(
                parameters->required_netagent_types[i].netagent_type,
                sizeof(parameters->required_netagent_types[i].netagent_type)) ==
                0) {
          break;
        }

        if (!fetched_type) {
          if (netagent_get_agent_domain_and_type(
                  *netagent_uuid, netagent_domain, netagent_type)) {
            fetched_type = TRUE;
          } else {
            break;
          }
        ...
        if ((strbuflen(parameters->required_netagent_types[i].netagent_domain,
                       sizeof(parameters->required_netagent_types[i]
                                  .netagent_domain)) == 0 ||
             strbufcmp(netagent_domain, NETAGENT_DOMAINSIZE,
                       parameters->required_netagent_types[i].netagent_domain,
                       sizeof(parameters->required_netagent_types[i]
                                  .netagent_domain)) == 0) &&
            (strbuflen(parameters->required_netagent_types[i].netagent_type,
                       sizeof(parameters->required_netagent_types[i]
                                  .netagent_type)) == 0 ||
             strbufcmp(netagent_type, NETAGENT_TYPESIZE,
                       parameters->required_netagent_types[i].netagent_type,
                       sizeof(parameters->required_netagent_types[i]
                                  .netagent_type)) == 0)) {
          requested = true;
    ...
    for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
      ...
      if (uuid_compare(parameters->preferred_netagents[i], *netagent_uuid) ==
          0) {
        requested = true;
        break;
      }
    ...
    if (!requested) {
      ...
      for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
        if (strbuflen(parameters->preferred_netagent_types[i].netagent_domain,
                      sizeof(parameters->preferred_netagent_types[i]
                                 .netagent_domain)) == 0 ||
            strbuflen(parameters->preferred_netagent_types[i].netagent_type,
                      sizeof(parameters->preferred_netagent_types[i]
                                 .netagent_type)) == 0) {
          break;
        }
        ...
        if ((strbuflen(parameters->preferred_netagent_types[i].netagent_domain,
                       sizeof(parameters->preferred_netagent_types[i]
                                  .netagent_domain)) == 0 ||
             strbufcmp(netagent_domain, NETAGENT_DOMAINSIZE,
                       parameters->preferred_netagent_types[i].netagent_domain,
                       sizeof(parameters->preferred_netagent_types[i]
                                  .netagent_domain)) == 0) &&
            (strbuflen(parameters->preferred_netagent_types[i].netagent_type,
                       sizeof(parameters->preferred_netagent_types[i]
                                  .netagent_type)) == 0 ||
             strbufcmp(netagent_type, NETAGENT_TYPESIZE,
                       parameters->preferred_netagent_types[i].netagent_type,
                       sizeof(parameters->preferred_netagent_types[i]
                                  .netagent_type)) == 0)) {
          requested = true;
          break;
...
static bool necp_netagent_applies_to_client(
    struct necp_client *client,
    const struct necp_client_parsed_parameters *parameters,
    uuid_t *netagent_uuid, bool allow_nexus, uint32_t interface_index,
...
#pragma unused(interface_index, interface_generation)
  ...
  if (uuid_compare(client->failed_trigger_agent.netagent_uuid,
                   *netagent_uuid) == 0) {
    if (client->failed_trigger_agent.generation ==
        netagent_get_generation(*netagent_uuid)) {
      // If this agent was triggered, and failed, and hasn't changed, keep
      // hiding it
      return applies;
    } else {
...
#if SKYWALK
  // Add nexus agent if it is a nexus, and either is not a listener, or the
  // nexus supports listeners
  if (applies && is_nexus_agent &&
      !(parameters->flags &
        NECP_CLIENT_PARAMETER_FLAG_BROWSE) && // Don't add for browse paths
      ((flags & NETAGENT_FLAG_NEXUS_LISTENER) ||
       !(parameters->flags & NECP_CLIENT_PARAMETER_FLAG_LISTENER))) {
    necp_client_add_interface_option_if_needed(
        client, interface_index, interface_generation, netagent_uuid,
...
static void necp_client_add_agent_interface_options(
    struct necp_client *client,
  ...
  if (ifp->if_agentids != NULL) {
    for (u_int32_t i = 0; i < ifp->if_agentcount; i++) {
      ...
      // Relies on the side effect that nexus agents that apply will create
      // flows
      (void)necp_netagent_applies_to_client(
          client, parsed_parameters, &ifp->if_agentids[i], TRUE, ifp->if_index,
...
static void necp_client_add_browse_interface_options(
    struct necp_client *client,
    const struct necp_client_parsed_parameters *parsed_parameters,
    ifnet_t ifp) {
  if (ifp == NULL) {
  ...
  if (ifp->if_agentids != NULL) {
    for (u_int32_t i = 0; i < ifp->if_agentcount; i++) {
      ...
      if ((flags & NETAGENT_FLAG_REGISTERED) &&
          ...
          (!(flags & NETAGENT_FLAG_SPECIFIC_USE_ONLY) ||
           necp_netagent_is_requested(parsed_parameters,
                                      &ifp->if_agentids[i]))) {
        necp_client_add_interface_option_if_needed(
            client, ifp->if_index, ifnet_get_generation(ifp),
...
static inline bool _necp_client_address_is_valid(struct sockaddr *address) {
...
#if SKYWALK
static void necp_client_update_local_port_parameters(
    u_int8_t *__sized_by(parameters_size) parameters, u_int32_t parameters_size,
    uint16_t local_port) {
  size_t offset = 0;
  while ((offset + sizeof(struct necp_tlv_header)) <= parameters_size) {
    u_int8_t type =
        necp_buffer_get_tlv_type(parameters, parameters_size, offset);
    u_int32_t length =
        necp_buffer_get_tlv_length(parameters, parameters_size, offset);

    if (length >
        (parameters_size - (offset + sizeof(struct necp_tlv_header)))) {
      // If the length is larger than what can fit in the remaining parameters
      // size, bail
    ...
    if (length > 0) {
      u_int8_t *__indexable value =
          necp_buffer_get_tlv_value(parameters, parameters_size, offset, NULL);
      if (value != NULL) {
        switch (type) {
        case NECP_CLIENT_PARAMETER_LOCAL_ADDRESS: {
          if (length >= sizeof(struct necp_policy_condition_addr)) {
            struct necp_policy_condition_addr *address_struct =
                (struct necp_policy_condition_addr *)(void *)value;
        ...
        case NECP_CLIENT_PARAMETER_LOCAL_ENDPOINT: {
          if (length >= sizeof(struct necp_client_endpoint)) {
            struct necp_client_endpoint *endpoint =
                (struct necp_client_endpoint *)(void *)value;
            if (necp_client_address_is_valid(&endpoint->u.sa)) {
              if (endpoint->u.sa.sa_family == AF_INET) {
...
static void necp_client_trace_parameter_parsing(struct necp_client *client,
                                                u_int8_t type,
                                                u_int8_t *__sized_by(length)
                                                    value,
                                                u_int32_t length) {
  uint64_t num = 0;
  uint16_t shortBuf;
  ...
  if (value != NULL && length > 0) {
    ...
    int len = NECP_MAX_SOCKET_ATTRIBUTE_STRING_LENGTH < length
                  ? NECP_MAX_SOCKET_ATTRIBUTE_STRING_LENGTH
                  : length;
    memcpy(buffer, value, len);
    buffer[len] = 0;
    NECP_CLIENT_PARAMS_LOG(
        client, "Parsing param - type %d length %d value <%llu (%llX)> %s",
        type, length, num, num, buffer);
  } else {
    NECP_CLIENT_PARAMS_LOG(client, "Parsing param - type %d length %d", type,
                           length);
  }
}

static void necp_client_trace_parsed_parameters(
    struct necp_client *client,
    struct necp_client_parsed_parameters *parsed_parameters) {
  int i;
  ...
  case AF_INET:
    ...
    break;
  case AF_INET6:
    if (parsed_parameters->local_addr.sa.sa_len ==
        sizeof(struct sockaddr_in6)) {
      struct sockaddr_in6 *addr6 = &parsed_parameters->local_addr.sin6;
      inet_ntop(AF_INET6, &(addr6->sin6_addr), local_buffer,
                sizeof(local_buffer));
    }
    break;
  default:
  ...
  case AF_INET:
    if (parsed_parameters->remote_addr.sa.sa_len ==
        sizeof(struct sockaddr_in)) {
      struct sockaddr_in *addr = &parsed_parameters->remote_addr.sin;
      inet_ntop(AF_INET, &(addr->sin_addr), remote_buffer,
  ...
  case AF_INET6:
    if (parsed_parameters->remote_addr.sa.sa_len ==
      ...
      inet_ntop(AF_INET6, &(addr6->sin6_addr), remote_buffer,
                sizeof(remote_buffer));
    }
    break;
  default:
  ...
  NECP_CLIENT_PARAMS_LOG(
      client,
      "Parsed params - valid_fields %X flags %X "
      "extended flags %llX delegated_upid %llu local_addr %s remote_addr %s "
      "required_interface_index %u required_interface_type %d "
      "local_address_preference %d "
      "ip_protocol %d transport_protocol %d ethertype %d effective_pid %d "
      "effective_uuid %s uid %d persona_id %d traffic_class %d",
      parsed_parameters->valid_fields, parsed_parameters->flags,
      parsed_parameters->extended_flags, parsed_parameters->delegated_upid,
      ...
      parsed_parameters->ip_protocol, parsed_parameters->transport_protocol,
      parsed_parameters->ethertype, parsed_parameters->effective_pid, uuid_str,
      parsed_parameters->uid, parsed_parameters->persona_id,
      parsed_parameters->traffic_class);

  NECP_CLIENT_PARAMS_LOG(
      client,
      "Parsed params - tracker flags <known-tracker %X> <non-app-initiated %X> "
      "<silent %X> <app-approved %X>",
      ...
      parsed_parameters->flags &
          NECP_CLIENT_PARAMETER_FLAG_APPROVED_APP_DOMAIN);

  for (i = 0; i < NECP_MAX_INTERFACE_PARAMETERS &&
              parsed_parameters->prohibited_interfaces[i][0];
       i++) {
    NECP_CLIENT_PARAMS_LOG(client, "Parsed prohibited_interfaces[%d] <%s>", i,
                           parsed_parameters->prohibited_interfaces[i]);
  }

  for (i = 0; i < NECP_MAX_AGENT_PARAMETERS &&
              parsed_parameters->required_netagent_types[i].netagent_domain[0];
       i++) {
    NECP_CLIENT_PARAMS_LOG(
        client, "Parsed required_netagent_types[%d] <%s> <%s>", i,
        parsed_parameters->required_netagent_types[i].netagent_domain,
        parsed_parameters->required_netagent_types[i].netagent_type);
  }
  for (i = 0;
       i < NECP_MAX_AGENT_PARAMETERS &&
       parsed_parameters->prohibited_netagent_types[i].netagent_domain[0];
       i++) {
    NECP_CLIENT_PARAMS_LOG(
        client, "Parsed prohibited_netagent_types[%d] <%s> <%s>", i,
        parsed_parameters->prohibited_netagent_types[i].netagent_domain,
        parsed_parameters->prohibited_netagent_types[i].netagent_type);
  }
  for (i = 0; i < NECP_MAX_AGENT_PARAMETERS &&
              parsed_parameters->preferred_netagent_types[i].netagent_domain[0];
       i++) {
    NECP_CLIENT_PARAMS_LOG(
        client, "Parsed preferred_netagent_types[%d] <%s> <%s>", i,
        parsed_parameters->preferred_netagent_types[i].netagent_domain,
        parsed_parameters->preferred_netagent_types[i].netagent_type);
  }
  for (i = 0; i < NECP_MAX_AGENT_PARAMETERS &&
              parsed_parameters->avoided_netagent_types[i].netagent_domain[0];
       i++) {
    NECP_CLIENT_PARAMS_LOG(
        client, "Parsed avoided_netagent_types[%d] <%s> <%s>", i,
        parsed_parameters->avoided_netagent_types[i].netagent_domain,
        parsed_parameters->avoided_netagent_types[i].netagent_type);
  }

  for (i = 0; i < NECP_MAX_AGENT_PARAMETERS &&
              !uuid_is_null(parsed_parameters->required_netagents[i]);
       i++) {
    uuid_unparse_lower(parsed_parameters->required_netagents[i], uuid_str);
    NECP_CLIENT_PARAMS_LOG(client, "Parsed required_netagents[%d] <%s>", i,
                           uuid_str);
  }
  for (i = 0; i < NECP_MAX_AGENT_PARAMETERS &&
              !uuid_is_null(parsed_parameters->prohibited_netagents[i]);
       i++) {
    uuid_unparse_lower(parsed_parameters->prohibited_netagents[i], uuid_str);
    NECP_CLIENT_PARAMS_LOG(client, "Parsed prohibited_netagents[%d] <%s>", i,
                           uuid_str);
  }
  for (i = 0; i < NECP_MAX_AGENT_PARAMETERS &&
              !uuid_is_null(parsed_parameters->preferred_netagents[i]);
       i++) {
    uuid_unparse_lower(parsed_parameters->preferred_netagents[i], uuid_str);
    NECP_CLIENT_PARAMS_LOG(client, "Parsed preferred_netagents[%d] <%s>", i,
                           uuid_str);
  }
  for (i = 0; i < NECP_MAX_AGENT_PARAMETERS &&
              !uuid_is_null(parsed_parameters->avoided_netagents[i]);
       i++) {
    uuid_unparse_lower(parsed_parameters->avoided_netagents[i], uuid_str);
    NECP_CLIENT_PARAMS_LOG(client, "Parsed avoided_netagents[%d] <%s>", i,
                           uuid_str);
  }
}

static bool necp_client_strings_are_equal(
    const char *__sized_by(string1_length) string1, size_t string1_length,
  ...
  return strbufcmp(string1, string1_actual_length, string2,
                   string2_actual_length) == 0;
}

static int necp_client_parse_parameters(
    struct necp_client *client,
    u_int8_t *__sized_by(parameters_size) parameters, u_int32_t parameters_size,
    struct necp_client_parsed_parameters *parsed_parameters) {
  int error = 0;
      ...
      if (value != NULL) {
        ...
        case NECP_CLIENT_PARAMETER_BOUND_INTERFACE: {
          if (length <= IFXNAMSIZ && length > 0) {
            ...
            if (ifnet_find_by_name(
                    __unsafe_null_terminated_from_indexable(
                        interface_name, &interface_name[length - 1]),
                    &bound_interface) == 0) {
              parsed_parameters->required_interface_index =
                  bound_interface->if_index;
        ...
        case NECP_CLIENT_PARAMETER_LOCAL_ADDRESS: {
          if (length >= sizeof(struct necp_policy_condition_addr)) {
            ...
            if (necp_client_address_is_valid(&address_struct->address.sa)) {
              ...
              if ((parsed_parameters->local_addr.sa.sa_family == AF_INET &&
                   parsed_parameters->local_addr.sin.sin_port) ||
                  (parsed_parameters->local_addr.sa.sa_family == AF_INET6 &&
                   parsed_parameters->local_addr.sin6.sin6_port)) {
                parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_REMOTE_ADDRESS: {
          if (length >= sizeof(struct necp_policy_condition_addr)) {
            struct necp_policy_condition_addr *address_struct =
                (struct necp_policy_condition_addr *)(void *)value;
            if (necp_client_address_is_valid(&address_struct->address.sa)) {
              parsed_parameters->remote_addr.sin6 =
                  address_struct->address.sin6;
              parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_PROHIBIT_INTERFACE: {
          ...
          if (length <= IFXNAMSIZ && length > 0) {
            memcpy(parsed_parameters
                       ->prohibited_interfaces[num_prohibited_interfaces],
                   value, length);
            parsed_parameters
                ->prohibited_interfaces[num_prohibited_interfaces][length - 1] =
                0; // Make sure the string is NULL terminated
        ...
        case NECP_CLIENT_PARAMETER_REQUIRE_IF_TYPE: {
          if (parsed_parameters->valid_fields &
              NECP_PARSED_PARAMETERS_FIELD_REQUIRED_IFTYPE) {
            break;
          }
          if (length >= sizeof(u_int8_t)) {
            memcpy(&parsed_parameters->required_interface_type, value,
                   sizeof(u_int8_t));
        ...
        case NECP_CLIENT_PARAMETER_PROHIBIT_IF_TYPE: {
          ...
          if (length >= sizeof(u_int8_t)) {
            memcpy(&parsed_parameters->prohibited_interface_types
        ...
        case NECP_CLIENT_PARAMETER_REQUIRE_AGENT: {
          ...
          if (length >= sizeof(uuid_t)) {
            memcpy(&parsed_parameters->required_netagents[num_required_agents],
                   value, sizeof(uuid_t));
            num_required_agents++;
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_PROHIBIT_AGENT: {
          ...
          if (length >= sizeof(uuid_t)) {
            memcpy(
                &parsed_parameters->prohibited_netagents[num_prohibited_agents],
                value, sizeof(uuid_t));
        ...
        case NECP_CLIENT_PARAMETER_PREFER_AGENT: {
          ...
          if (length >= sizeof(uuid_t)) {
            memcpy(
                &parsed_parameters->preferred_netagents[num_preferred_agents],
                value, sizeof(uuid_t));
        ...
        case NECP_CLIENT_PARAMETER_AVOID_AGENT: {
          ...
          if (length >= sizeof(uuid_t)) {
            memcpy(&parsed_parameters->avoided_netagents[num_avoided_agents],
                   value, sizeof(uuid_t));
            num_avoided_agents++;
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_REQUIRE_AGENT_TYPE: {
          ...
          if (length >= sizeof(struct necp_client_parameter_netagent_type)) {
            memcpy(&parsed_parameters
                        ->required_netagent_types[num_required_agent_types],
                   value, sizeof(struct necp_client_parameter_netagent_type));
        ...
        case NECP_CLIENT_PARAMETER_PROHIBIT_AGENT_TYPE: {
          ...
          if (length >= sizeof(struct necp_client_parameter_netagent_type)) {
            memcpy(&parsed_parameters
                        ->prohibited_netagent_types[num_prohibited_agent_types],
                   value, sizeof(struct necp_client_parameter_netagent_type));
        ...
        case NECP_CLIENT_PARAMETER_PREFER_AGENT_TYPE: {
          ...
          if (length >= sizeof(struct necp_client_parameter_netagent_type)) {
            memcpy(&parsed_parameters
                        ->preferred_netagent_types[num_preferred_agent_types],
                   value, sizeof(struct necp_client_parameter_netagent_type));
        ...
        case NECP_CLIENT_PARAMETER_AVOID_AGENT_TYPE: {
          ...
          if (length >= sizeof(struct necp_client_parameter_netagent_type)) {
            memcpy(&parsed_parameters
                        ->avoided_netagent_types[num_avoided_agent_types],
                   value, sizeof(struct necp_client_parameter_netagent_type));
        ...
        case NECP_CLIENT_PARAMETER_FLAGS: {
          if (length >= sizeof(u_int32_t)) {
            memcpy(&parsed_parameters->flags, value,
                   sizeof(parsed_parameters->flags));
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_IP_PROTOCOL: {
          if (length == sizeof(u_int16_t)) {
            ...
            memcpy(&parsed_parameters->ip_protocol, value,
                   sizeof(parsed_parameters->ip_protocol));
            parsed_parameters->valid_fields |=
                NECP_PARSED_PARAMETERS_FIELD_IP_PROTOCOL;
          }
          break;
        ...
        case NECP_CLIENT_PARAMETER_TRANSPORT_PROTOCOL: {
          if (length >= sizeof(parsed_parameters->transport_protocol)) {
            memcpy(&parsed_parameters->transport_protocol, value,
                   sizeof(parsed_parameters->transport_protocol));
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_PID: {
          if (length >= sizeof(parsed_parameters->effective_pid)) {
            memcpy(&parsed_parameters->effective_pid, value,
                   sizeof(parsed_parameters->effective_pid));
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_DELEGATED_UPID: {
          if (length >= sizeof(parsed_parameters->delegated_upid)) {
            memcpy(&parsed_parameters->delegated_upid, value,
                   sizeof(parsed_parameters->delegated_upid));
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_ETHERTYPE: {
          if (length >= sizeof(parsed_parameters->ethertype)) {
            memcpy(&parsed_parameters->ethertype, value,
                   sizeof(parsed_parameters->ethertype));
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_APPLICATION: {
          if (length >= sizeof(parsed_parameters->effective_uuid)) {
            memcpy(&parsed_parameters->effective_uuid, value,
                   sizeof(parsed_parameters->effective_uuid));
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_TRAFFIC_CLASS: {
          if (length >= sizeof(parsed_parameters->traffic_class)) {
            memcpy(&parsed_parameters->traffic_class, value,
                   sizeof(parsed_parameters->traffic_class));
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_PARENT_ID: {
          if (length == sizeof(parent_id)) {
            ...
            memcpy(&parsed_parameters->parent_uuid, value,
                   sizeof(parsed_parameters->parent_uuid));
            parsed_parameters->valid_fields |=
                NECP_PARSED_PARAMETERS_FIELD_PARENT_UUID;
          }
          break;
        }
        case NECP_CLIENT_PARAMETER_LOCAL_ADDRESS_PREFERENCE: {
          if (length >= sizeof(parsed_parameters->local_address_preference)) {
            memcpy(&parsed_parameters->local_address_preference, value,
                   sizeof(parsed_parameters->local_address_preference));
            parsed_parameters->valid_fields |=
        ...
        case NECP_CLIENT_PARAMETER_ATTRIBUTED_BUNDLE_IDENTIFIER: {
          if (length > 0) {
            parsed_parameters->valid_fields |=
                NECP_PARSED_PARAMETERS_FIELD_ATTRIBUTED_BUNDLE_IDENTIFIER;
          }
          break;
        }
        case NECP_CLIENT_PARAMETER_FLOW_DEMUX_PATTERN: {
          if (parsed_parameters->demux_pattern_count >=
              NECP_MAX_DEMUX_PATTERNS) {
            break;
          }
          if (length >= sizeof(struct necp_demux_pattern)) {
            memcpy(
                &parsed_parameters
                     ->demux_patterns[parsed_parameters->demux_pattern_count],
                value, sizeof(struct necp_demux_pattern));
            parsed_parameters->demux_pattern_count++;
            parsed_parameters->valid_fields |=
                NECP_PARSED_PARAMETERS_FIELD_FLOW_DEMUX_PATTERN;
          }
          break;
        }
        case NECP_CLIENT_PARAMETER_APPLICATION_ID: {
          if (length >= sizeof(necp_application_id_t)) {
            necp_application_id_t *application_id =
                (necp_application_id_t *)(void *)value;
            // UID
            parsed_parameters->uid = application_id->uid;
            parsed_parameters->valid_fields |= NECP_PARSED_PARAMETERS_FIELD_UID;
            // EUUID
            uuid_copy(parsed_parameters->effective_uuid,
                      application_id->effective_uuid);
            parsed_parameters->valid_fields |=
                NECP_PARSED_PARAMETERS_FIELD_EFFECTIVE_UUID;
            // PERSONA
            parsed_parameters->persona_id = application_id->persona_id;
            parsed_parameters->valid_fields |=
                NECP_PARSED_PARAMETERS_FIELD_PERSONA_ID;
          }
          break;
        }
        case NECP_CLIENT_PARAMETER_EXTENDED_FLAGS: {
          if (length >= sizeof(u_int64_t)) {
            memcpy(&parsed_parameters->extended_flags, value,
                   sizeof(parsed_parameters->extended_flags));
            parsed_parameters->valid_fields |=
                NECP_PARSED_PARAMETERS_FIELD_EXTENDED_FLAGS;
          }
          break;
        }
        default: {
  ...
  if (resolver_tag != NULL) {
    struct necp_client_validatable *validatable =
        (struct necp_client_validatable *)resolver_tag;
    if (resolver_tag_length <= sizeof(struct necp_client_validatable)) {
      error = EINVAL;
      NECPLOG(LOG_ERR, "Resolver tag length too short: %u",
              resolver_tag_length);
    } else {
      bool matches = true;

      // Check the client UUID for client-specific results
      if (validatable->signable.sign_type ==
              NECP_CLIENT_SIGN_TYPE_RESOLVER_ANSWER ||
          validatable->signable.sign_type ==
              NECP_CLIENT_SIGN_TYPE_BROWSE_RESULT ||
          validatable->signable.sign_type ==
              NECP_CLIENT_SIGN_TYPE_SERVICE_RESOLVER_ANSWER) {
        if (uuid_compare(parent_id, validatable->signable.client_id) != 0 &&
            uuid_compare(client->client_id, validatable->signable.client_id) !=
                0) {
      ...
      size_t data_length =
          resolver_tag_length - sizeof(struct necp_client_validatable);
      ...
      case NECP_CLIENT_SIGN_TYPE_SYSTEM_RESOLVER_ANSWER: {
        if (data_length < (sizeof(struct necp_client_host_resolver_answer) -
                           sizeof(struct necp_client_signable))) {
          NECPLOG0(LOG_ERR, "Resolver tag invalid length for resolver answer");
          matches = false;
        } else {
          struct necp_client_host_resolver_answer *__single answer_struct =
              (struct necp_client_host_resolver_answer *)&validatable->signable;
          if (data_length != (sizeof(struct necp_client_host_resolver_answer) +
                              answer_struct->hostname_length -
                              sizeof(struct necp_client_signable))) {
            NECPLOG0(LOG_ERR,
                     "Resolver tag invalid length for resolver answer");
            matches = false;
          } else {
            struct sockaddr_in6 sin6 = answer_struct->address_answer.sin6;
            if (answer_struct->hostname_length !=
                    0 && // If the hostname on the signed answer is empty,
                         // ignore
                !necp_client_strings_are_equal(
                    (const char *)client_hostname, hostname_length,
                    necp_answer_get_hostname(answer_struct,
                                             answer_struct->hostname_length),
                    answer_struct->hostname_length)) {
              NECPLOG0(LOG_ERR, "Resolver tag hostname does not match");
              matches = false;
            } else if (answer_struct->address_answer.sa.sa_family !=
                           parsed_parameters->remote_addr.sa.sa_family ||
                       answer_struct->address_answer.sa.sa_len !=
                           parsed_parameters->remote_addr.sa.sa_len) {
              NECPLOG0(LOG_ERR, "Resolver tag address type does not match");
              matches = false;
            } else if (answer_struct->address_answer.sin.sin_port !=
                           0 && // If the port on the signed answer is empty,
                                // ignore
                       answer_struct->address_answer.sin.sin_port !=
                           parsed_parameters->remote_addr.sin.sin_port) {
              NECPLOG0(LOG_ERR, "Resolver tag port does not match");
              matches = false;
            } else if ((answer_struct->address_answer.sa.sa_family == AF_INET &&
                        answer_struct->address_answer.sin.sin_addr.s_addr !=
                            parsed_parameters->remote_addr.sin.sin_addr
                                .s_addr) ||
                       (answer_struct->address_answer.sa.sa_family ==
                            AF_INET6 &&
                        memcmp(&sin6.sin6_addr,
                               &parsed_parameters->remote_addr.sin6.sin6_addr,
      ...
      case NECP_CLIENT_SIGN_TYPE_SYSTEM_BROWSE_RESULT: {
        if (data_length < (sizeof(struct necp_client_browse_result) -
                           sizeof(struct necp_client_signable))) {
          NECPLOG0(LOG_ERR, "Resolver tag invalid length for browse result");
          matches = false;
        } else {
          struct necp_client_browse_result *__single answer_struct =
              (struct necp_client_browse_result *)&validatable->signable;
          if (data_length != (sizeof(struct necp_client_browse_result) +
                              answer_struct->service_length -
                              sizeof(struct necp_client_signable))) {
            NECPLOG0(LOG_ERR, "Resolver tag invalid length for browse result");
            matches = false;
          }
      ...
      case NECP_CLIENT_SIGN_TYPE_SYSTEM_SERVICE_RESOLVER_ANSWER: {
        if (data_length < (sizeof(struct necp_client_service_resolver_answer) -
                           sizeof(struct necp_client_signable))) {
          NECPLOG0(LOG_ERR,
                   "Resolver tag invalid length for service resolver answer");
          matches = false;
        } else {
          struct necp_client_service_resolver_answer *__single answer_struct =
              (struct necp_client_service_resolver_answer *)&validatable
                  ->signable;
          if (data_length !=
              (sizeof(struct necp_client_service_resolver_answer) +
               answer_struct->service_length + answer_struct->hostname_length -
               sizeof(struct necp_client_signable))) {
            NECPLOG0(LOG_ERR,
                     "Resolver tag invalid length for service resolver answer");
            matches = false;
      ...
      default: {
        NECPLOG(LOG_ERR, "Resolver tag unknown sign type: %u",
                validatable->signable.sign_type);
        matches = false;
        break;
      }
      ...
      if (!matches) {
        ...
        const bool validated = necp_validate_resolver_answer(
            validatable->signable.client_id, validatable->signable.sign_type,
            signable_get_data(&validatable->signable, data_length), data_length,
            validatable->signature.signed_tag,
            sizeof(validatable->signature.signed_tag));
        if (!validated) {
          error = EAUTH;
...
static int necp_client_parse_result(u_int8_t *__indexable result,
...
#pragma unused(flow_stats)
  ...
  while ((offset + sizeof(struct necp_tlv_header)) <= result_size) {
    ...
    if (length > 0 &&
        (offset + sizeof(struct necp_tlv_header) + length) <= result_size) {
      u_int8_t *__indexable value =
          necp_buffer_get_tlv_value(result, result_size, offset, NULL);
      if (value != NULL) {
        switch (type) {
...
static struct necp_client_flow_registration *
necp_client_create_flow_registration(struct necp_fd_data *fd_data,
                                     struct necp_client *client) {
  NECP_FD_ASSERT_LOCKED(fd_data);
  NECP_CLIENT_ASSERT_LOCKED(client);

  struct necp_client_flow_registration *new_registration = kalloc_type(
      struct necp_client_flow_registration, Z_WAITOK | Z_ZERO | Z_NOFAIL);

  new_registration->last_interface_details =
      combine_interface_details(IFSCOPE_NONE, NSTAT_IFNET_IS_UNKNOWN_TYPE);
  ...
  RB_INSERT(_necp_client_flow_tree, &client->flow_registrations,
            new_registration);
  ...
  NECP_FLOW_TREE_LOCK_EXCLUSIVE();
  RB_INSERT(_necp_client_flow_global_tree, &necp_client_flow_global_tree,
            new_registration);
  NECP_FLOW_TREE_UNLOCK();
...
#if SKYWALK
  {
    // The uuid caching here is something of a hack, but saves a dynamic lookup
    // with attendant lock hierarchy issues
    uint64_t stats_event_type =
        (uuid_is_null(client->latest_flow_registration_id))
            ? NSTAT_EVENT_SRC_FLOW_UUID_ASSIGNED
            : NSTAT_EVENT_SRC_FLOW_UUID_CHANGED;
    uuid_copy(client->latest_flow_registration_id,
              new_registration->registration_id);

    // With the flow uuid known, push a new statistics update to ensure the uuid
    // gets known by any clients before the flow can close
    if (client->nstat_context != NULL) {
...
static void necp_client_add_socket_flow(
    struct necp_client_flow_registration *flow_registration,
    struct inpcb *inp) {
  struct necp_client_flow *new_flow =
      kalloc_type(struct necp_client_flow, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static int necp_client_register_socket_inner(pid_t pid, uuid_t client_id,
                                             struct inpcb *inp,
                                             bool is_listener) {
  int error = 0;
  ...
  LIST_FOREACH(client_fd, &necp_fd_list, chain) {
    ...
    struct necp_client *client =
        necp_client_fd_find_client_and_lock(client_fd, client_id);
    if (client != NULL) {
      if (!pid || client->proc_pid == pid) {
        if (is_listener) {
          ...
          struct necp_client_flow_registration *flow_registration =
              necp_client_find_flow(client, client_id);
          ...
          } else if (RB_EMPTY(&client->flow_registrations) &&
                     !necp_client_id_is_flow(client_id)) {
            // No flows yet on this client, add a new registration
            flow_registration =
                necp_client_create_flow_registration(client_fd, client);
            if (flow_registration == NULL) {
              error = ENOMEM;
            } else {
              // Add a new flow
...
int necp_client_register_socket_flow(pid_t pid, uuid_t client_id,
                                     struct inpcb *inp) {
  return necp_client_register_socket_inner(pid, client_id, inp, false);
}

int necp_client_register_socket_listener(pid_t pid, uuid_t client_id,
                                         struct inpcb *inp) {
  return necp_client_register_socket_inner(pid, client_id, inp, true);
}

#if SKYWALK
int necp_client_get_netns_flow_info(uuid_t client_id,
...
static void necp_client_add_multipath_interface_flows(
    struct necp_client_flow_registration *flow_registration,
    struct necp_client *client, struct mppcb *mpp) {
...
int necp_client_register_multipath_cb(pid_t pid, uuid_t client_id,
                                      struct mppcb *mpp) {
  int error = 0;
  struct necp_fd_data *client_fd = NULL;
  bool found_client = FALSE;
    ...
    if (client != NULL) {
      if (!pid || client->proc_pid == pid) {
        ...
        if (flow_registration != NULL) {
          // Found the right client and flow registration, add a new flow
          found_client = TRUE;
          necp_client_add_multipath_interface_flows(flow_registration, client,
                                                    mpp);
        } else if (RB_EMPTY(&client->flow_registrations) &&
                   !necp_client_id_is_flow(client_id)) {
          // No flows yet on this client, add a new registration
...
static int necp_client_lookup_bb_radio_manager(struct necp_client *client,
                                               uuid_t netagent_uuid) {
  char netagent_domain[NETAGENT_DOMAINSIZE];
  char netagent_type[NETAGENT_TYPESIZE];
  struct necp_aggregate_result result;
  ...
  error = necp_application_find_policy_match_internal(
      proc, client->parameters, (u_int32_t)client->parameters_length, &result,
      NULL, NULL, 0, NULL, NULL, NULL, NULL, NULL, true, true, NULL);
  ...
  for (int i = 0; i < NECP_MAX_NETAGENTS; i++) {
    ...
    memset(&netagent_domain, 0, NETAGENT_DOMAINSIZE);
    memset(&netagent_type, 0, NETAGENT_TYPESIZE);
    if (netagent_get_agent_domain_and_type(result.netagents[i], netagent_domain,
                                           netagent_type) == FALSE) {
      continue;
    }

    if (strlcmp(netagent_domain, NETAGENT_DOMAIN_RADIO_MANAGER,
                NETAGENT_DOMAINSIZE) != 0) {
      continue;
    }

    if (strlcmp(netagent_type, NETAGENT_TYPE_RADIO_MANAGER,
                NETAGENT_TYPESIZE) != 0) {
      continue;
    }
...
static int
necp_client_assert_bb_radio_manager_common(struct necp_client *client,
                                           bool assert) {
  uuid_t netagent_uuid;
  uint8_t assert_type;
  int error;
  ...
  error = netagent_client_message(netagent_uuid, client->client_id,
...
int necp_client_assert_bb_radio_manager(uuid_t client_id, bool assert) {
  struct necp_client *client;
  int error = 0;
...
static int necp_client_unregister_socket_flow(uuid_t client_id, void *handle) {
  int error = 0;
  struct necp_fd_data *client_fd = NULL;
  bool found_client = FALSE;
  bool client_updated = FALSE;
  ...
  LIST_FOREACH(client_fd, &necp_fd_list, chain) {
    ...
    if (client != NULL) {
      ...
      if (flow_registration != NULL) {
        ...
        struct necp_client_flow *__single search_flow = NULL;
        struct necp_client_flow *temp_flow = NULL;
        LIST_FOREACH_SAFE(search_flow, &flow_registration->flow_list,
                          flow_chain, temp_flow) {
...
static int necp_client_unregister_multipath_cb(uuid_t client_id, void *handle) {
  int error = 0;
  bool found_client = FALSE;
...
int necp_client_assign_from_socket(pid_t pid, uuid_t client_id,
                                   struct inpcb *inp) {
  int error = 0;
  struct necp_fd_data *client_fd = NULL;
  bool found_client = FALSE;
  ...
  LIST_FOREACH(client_fd, &necp_fd_list, chain) {
    ...
    if (client != NULL) {
      struct necp_client_flow_registration *flow_registration =
          necp_client_find_flow(client, client_id);
      if (flow_registration == NULL && RB_EMPTY(&client->flow_registrations) &&
          !necp_client_id_is_flow(client_id)) {
        // No flows yet on this client, add a new registration
        flow_registration =
            necp_client_create_flow_registration(client_fd, client);
        ...
        LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
          if (flow->socket && flow->u.socket_handle == inp) {
            ...
            if (inp->inp_vflag & INP_IPV4) {
              ...
              memcpy(&flow->local_addr.sin.sin_addr, &inp->inp_laddr,
              ...
              memcpy(&flow->remote_addr.sin.sin_addr, &inp->inp_faddr,
                     sizeof(struct in_addr));
            } else if (inp->inp_vflag & INP_IPV6) {
              in6_ip6_to_sockaddr(&inp->in6p_laddr, inp->inp_lport,
                                  inp->inp_lifscope, &flow->local_addr.sin6,
                                  sizeof(flow->local_addr));
              in6_ip6_to_sockaddr(&inp->in6p_faddr, inp->inp_fport,
                                  inp->inp_fifscope, &flow->remote_addr.sin6,
            ...
            void *message = necp_create_nexus_assign_message(
                empty_uuid, 0, NULL, 0,
                (struct necp_client_endpoint *)&flow->local_addr,
                (struct necp_client_endpoint *)&flow->remote_addr, NULL, 0,
                NULL, 0, &message_length);
...
bool necp_socket_is_allowed_to_recv_on_interface(struct inpcb *inp,
                                                 ifnet_t interface) {
  if (interface == NULL || inp == NULL ||
  ...
  if (client != NULL) {
    ...
    int error = necp_client_parse_parameters(
        client, client->parameters, (u_int32_t)client->parameters_length,
        parsed_parameters);
    if (error == 0) {
      if (!necp_ifnet_matches_parameters(interface, parsed_parameters, 0, NULL,
                                         true, false)) {
        allowed = FALSE;
      }
    }
...
int necp_update_flow_protoctl_event(uuid_t netagent_uuid, uuid_t client_id,
      ...
      if (flow_registration != NULL) {
        ...
        LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
          ...
          if ((flow->nexus &&
            ...
            flow->protoctl_event.protoctl_event_tcp_seq_num =
...
static bool necp_assign_client_result_locked(
    struct proc *proc, struct necp_fd_data *client_fd,
    struct necp_client *client,
    struct necp_client_flow_registration *flow_registration,
    uuid_t netagent_uuid, u_int8_t *__indexable assigned_results,
    size_t assigned_results_length, bool notify_fd,
  ...
  LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
    ...
    if (flow->nexus && uuid_compare(flow->u.nexus_agent, netagent_uuid) == 0) {
      ...
      if (assigned_results != NULL && assigned_results_length > 0) {
        int error = necp_client_parse_result(
            assigned_results, (u_int32_t)assigned_results_length, flow,
            assigned_from_userspace_agent
                ? NULL
                : &nexus_stats); // Only assign stats from kernel agents
        VERIFY(error == 0);
      }
...
int necp_assign_client_result(uuid_t netagent_uuid, uuid_t client_id,
                              u_int8_t *__sized_by(assigned_results_length)
                                  assigned_results,
                              size_t assigned_results_length) {
  int error = 0;
  struct necp_fd_data *client_fd = NULL;
    ...
    if (client != NULL) {
      ...
      if (flow_registration != NULL) {
        ...
        if (necp_assign_client_result_locked(
                proc, client_fd, client, flow_registration, netagent_uuid,
...
int necp_assign_client_group_members(
    uuid_t netagent_uuid, uuid_t client_id,
    u_int8_t *__counted_by(assigned_group_members_length)
...
static bool necp_update_parsed_parameters(
    struct necp_client_parsed_parameters *parsed_parameters,
    struct necp_aggregate_result *result) {
  if (parsed_parameters == NULL || result == NULL) {
    return false;
  }
  ...
  for (int i = 0; i < NECP_MAX_NETAGENTS; i++) {
    ...
    if (parsed_parameters->valid_fields &
      ...
      for (int j = 0; j < NECP_MAX_AGENT_PARAMETERS; j++) {
        if (uuid_compare(parsed_parameters->required_netagents[j],
                         result->netagents[i]) == 0) {
          // Already required, break
          break;
        }
        if (uuid_is_null(parsed_parameters->required_netagents[j])) {
          // Add here
          memcpy(&parsed_parameters->required_netagents[j],
                 result->netagents[i], sizeof(uuid_t));
          updated = true;
          break;
        }
      }
      ...
      parsed_parameters->valid_fields |=
          NECP_PARSED_PARAMETERS_FIELD_REQUIRED_AGENT;
      memcpy(&parsed_parameters->required_netagents[0], result->netagents[i],
             sizeof(uuid_t));
      updated = true;
    }
    ...
    if (parsed_parameters->valid_fields &
      ...
      if (netagent_get_agent_domain_and_type(
              result->netagents[i], remove_agent_domain, remove_agent_type)) {
        for (int j = 0; j < NECP_MAX_AGENT_PARAMETERS; j++) {
          if (strbuflen(
                  parsed_parameters->required_netagent_types[j].netagent_domain,
                  sizeof(parsed_parameters->required_netagent_types[j]
                             .netagent_domain)) == 0 &&
              strbuflen(
                  parsed_parameters->required_netagent_types[j].netagent_type,
                  sizeof(parsed_parameters->required_netagent_types[j]
                             .netagent_type)) == 0) {
            break;
          }
          ...
          if (strbufcmp(
                  ...
                  sizeof(parsed_parameters->required_netagent_types[j]
                             .netagent_domain),
                  remove_agent_domain, NETAGENT_DOMAINSIZE) == 0 &&
              strbufcmp(
                  parsed_parameters->required_netagent_types[j].netagent_type,
                  sizeof(parsed_parameters->required_netagent_types[j]
                             .netagent_type),
                  remove_agent_type, NETAGENT_TYPESIZE) == 0) {
            updated = true;
            ...
            if (j == NECP_MAX_AGENT_PARAMETERS - 1) {
              ...
              memset(
                  &parsed_parameters
                       ->required_netagent_types[NECP_MAX_AGENT_PARAMETERS - 1],
                  0, sizeof(struct necp_client_parameter_netagent_type));
              break;
            } else {
              ...
              memmove(&parsed_parameters->required_netagent_types[j],
                      ...
                      sizeof(struct necp_client_parameter_netagent_type) *
                          (NECP_MAX_AGENT_PARAMETERS - (j + 1)));
              memset(
                  &parsed_parameters
                       ->required_netagent_types[NECP_MAX_AGENT_PARAMETERS - 1],
                  0, sizeof(struct necp_client_parameter_netagent_type));
              // Continue, don't increment but look at the new shifted item
              // instead
              continue;
            }
          }
  ...
  if (updated && parsed_parameters->required_interface_index != IFSCOPE_NONE &&
      (parsed_parameters->valid_fields &
       NECP_PARSED_PARAMETERS_FIELD_REQUIRED_IF) == 0) {
...
static inline bool necp_agent_types_match(
    const char *__sized_by(NETAGENT_DOMAINSIZE) agent_domain1,
    const char *__sized_by(NETAGENT_TYPESIZE) agent_type1,
    const char *__sized_by(NETAGENT_DOMAINSIZE) agent_domain2,
    const char *__sized_by(NETAGENT_TYPESIZE) agent_type2) {
  return (strbuflen(agent_domain1, NETAGENT_DOMAINSIZE) == 0 ||
          strbufcmp(agent_domain2, NETAGENT_DOMAINSIZE, agent_domain1,
                    NETAGENT_DOMAINSIZE) == 0) &&
         (strbuflen(agent_type1, NETAGENT_TYPESIZE) == 0 ||
          strbufcmp(agent_type2, NETAGENT_TYPESIZE, agent_type1,
                    NETAGENT_TYPESIZE) == 0);
}

static inline bool necp_calculate_client_result(
    proc_t proc, struct necp_client *client,
    struct necp_client_parsed_parameters *parsed_parameters,
    struct necp_aggregate_result *result, u_int32_t *flags, u_int32_t *reason,
    struct necp_client_endpoint *v4_gateway,
    struct necp_client_endpoint *v6_gateway, uuid_t *override_euuid) {
  ...
  if (necp_find_matching_interface_index(parsed_parameters, &matching_if_index,
    ...
    int error = necp_application_find_policy_match_internal(
        proc, client->parameters, (u_int32_t)client->parameters_length, result,
        flags, reason, matching_if_index, NULL, NULL, v4_gateway, v6_gateway,
        &route, false, true, override_euuid);
    if (error != 0) {
      if (route != NULL) {
        rtfree(route);
      }
    ...
    if (validate_agents) {
      ...
      if (parsed_parameters->valid_fields &
        ...
        for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
          ...
          for (int j = 0; j < NECP_MAX_NETAGENTS; j++) {
            ...
            if (uuid_compare(parsed_parameters->required_netagents[i],
                             result->netagents[j]) == 0) {
      ...
      if (!requirement_failed &&
        ...
        for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
          if (strbuflen(
                  parsed_parameters->required_netagent_types[i].netagent_domain,
                  sizeof(parsed_parameters->required_netagent_types[i]
                             .netagent_domain)) == 0 &&
              strbuflen(
                  parsed_parameters->required_netagent_types[i].netagent_type,
                  sizeof(parsed_parameters->required_netagent_types[i]
          ...
          for (int j = 0; j < NECP_MAX_NETAGENTS; j++) {
            ...
            if (netagent_get_agent_domain_and_type(result->netagents[j],
                                                   policy_agent_domain,
                                                   policy_agent_type)) {
              if (necp_agent_types_match(
                      parsed_parameters->required_netagent_types[i]
                          .netagent_domain,
                      parsed_parameters->required_netagent_types[i]
                          .netagent_type,
      ...
      if (requirement_failed) {
        // Agent requirement failed. Clear out the whole result, make everything
        // fail.
        memset(result, 0, sizeof(*result));
        if (route != NULL) {
          rtfree(route);
...
static bool necp_agent_is_removed_by_type(struct necp_aggregate_result *result,
                                          uuid_t agent_uuid) {
  for (int i = 0; i < NECP_MAX_REMOVE_NETAGENT_TYPES; i++) {
    ...
    if (netagent_get_agent_domain_and_type(agent_uuid, compare_agent_domain,
                                           compare_agent_type)) {
...
#define NECP_PARSED_PARAMETERS_REQUIRED_FIELDS                                 \
  (NECP_PARSED_PARAMETERS_FIELD_REQUIRED_IF |                                  \
   NECP_PARSED_PARAMETERS_FIELD_REQUIRED_IFTYPE |                              \
   NECP_PARSED_PARAMETERS_FIELD_REQUIRED_AGENT |                               \
   NECP_PARSED_PARAMETERS_FIELD_REQUIRED_AGENT_TYPE)
...
static bool
necp_update_client_result(proc_t proc, struct necp_fd_data *client_fd,
                          struct necp_client *client,
                          struct _necp_flow_defunct_list *defunct_list) {
  ...
  int error = necp_client_parse_parameters(client, client->parameters,
  ...
  if (!necp_calculate_client_result(proc, client, parsed_parameters, &result,
                                    &flags, &reason, &v4_gateway, &v6_gateway,
                                    &override_euuid)) {
    kfree_type(struct necp_client_parsed_parameters, parsed_parameters);
    return FALSE;
  }
  ...
  // If the original request was scoped, and the policy result matches, make
  // sure the result is scoped
  if ((result.routing_result == NECP_KERNEL_POLICY_RESULT_NONE ||
      ...
      parsed_parameters->required_interface_index ==
          result.routed_interface_index) {
    result.routing_result = NECP_KERNEL_POLICY_RESULT_SOCKET_SCOPED;
    result.routing_result_parameter.scoped_interface_index =
        result.routed_interface_index;
  }
  ...
  if (defunct_list != NULL &&
    ...
    if (result.routing_result_parameter.drop_flags &
        NECP_KERNEL_POLICY_DROP_FLAG_DEFUNCT_ALL_FLOWS) {
  ...
  if (parsed_parameters->flags & NECP_CLIENT_PARAMETER_FLAG_LISTENER) {
    ...
    // Clients without flows determine viability based on having some routable
    // interface
    flags |= NECP_CLIENT_RESULT_FLAG_SATISFIED;
  }
  ...
  cursor = necp_buffer_write_tlv_if_different(
      cursor, NECP_CLIENT_RESULT_FLAGS, sizeof(flags), &flags, &updated,
      client->result, sizeof(client->result));
  if (reason != 0) {
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_REASON, sizeof(reason), &reason, &updated,
        client->result, sizeof(client->result));
  }
  cursor = necp_buffer_write_tlv_if_different(
      cursor, NECP_CLIENT_RESULT_CLIENT_ID, sizeof(uuid_t), client->client_id,
      &updated, client->result, sizeof(client->result));
  cursor = necp_buffer_write_tlv_if_different(
      cursor, NECP_CLIENT_RESULT_POLICY_RESULT, sizeof(result.routing_result),
      &result.routing_result, &updated, client->result, sizeof(client->result));
  if (result.routing_result_parameter.tunnel_interface_index != 0) {
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_POLICY_RESULT_PARAMETER,
        sizeof(result.routing_result_parameter),
        &result.routing_result_parameter, &updated, client->result,
        sizeof(client->result));
  }
  if (result.filter_control_unit != 0) {
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_FILTER_CONTROL_UNIT,
        sizeof(result.filter_control_unit), &result.filter_control_unit,
        &updated, client->result, sizeof(client->result));
  }
  if (result.flow_divert_aggregate_unit != 0) {
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_FLOW_DIVERT_AGGREGATE_UNIT,
        sizeof(result.flow_divert_aggregate_unit),
        &result.flow_divert_aggregate_unit, &updated, client->result,
        sizeof(client->result));
  }
  if (result.routed_interface_index != 0) {
    ...
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_INTERFACE_INDEX,
  ...
  if (client_fd && client_fd->flags & NECP_OPEN_FLAG_BACKGROUND) {
    ...
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_EFFECTIVE_TRAFFIC_CLASS,
  ...
  if (client_fd->background) {
    ...
    if (has_assigned_flow) {
      ...
      cursor = necp_buffer_write_tlv_if_different(
          cursor, NECP_CLIENT_RESULT_TRAFFIC_MGMT_BG, sizeof(background),
          &background, &updated, client->result, sizeof(client->result));
    }
  }
  ...
  if (client->current_route != NULL) {
    ...
    if (route_mtu != 0) {
      cursor = necp_buffer_write_tlv_if_different(
          cursor, NECP_CLIENT_RESULT_EFFECTIVE_MTU, sizeof(route_mtu),
          &route_mtu, &updated, client->result, sizeof(client->result));
    }
    bool has_remote_addr = parsed_parameters->valid_fields &
                           NECP_PARSED_PARAMETERS_FIELD_REMOTE_ADDR;
    ...
    if (client->current_route->rt_ifp != NULL) {
      int8_t if_lqm =
          client->current_route->rt_ifp->if_interface_state.lqm_state;

      // Upgrade to enhancedLQM for cellular interfaces that support it
      if (client->current_route->rt_ifp->if_type == IFT_CELLULAR &&
          client->current_route->rt_ifp->if_link_status != NULL) {
        struct if_cellular_status_v1 *cell_link_status =
            &client->current_route->rt_ifp->if_link_status->ifsr_u.ifsr_cell
                 .if_cell_u.if_status_v1;

        if (cell_link_status->valid_bitmask &
      ...
      cursor = necp_buffer_write_tlv_if_different(
          cursor, NECP_CLIENT_RESULT_LINK_QUALITY, sizeof(if_lqm), &if_lqm,
          &updated, client->result, sizeof(client->result));
    }
  }
  NECP_CLIENT_ROUTE_UNLOCK(client);
  ...
  if (write_v4_gateway) {
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_GATEWAY, sizeof(struct necp_client_endpoint),
        (uint8_t *)(struct necp_client_endpoint * __bidi_indexable) &
            v4_gateway,
        &updated, client->result, sizeof(client->result));
  }
  ...
  for (int i = 0; i < NAT64_MAX_NUM_PREFIXES; i++) {
    if (result.nat64_prefixes[i].prefix_len != 0) {
      cursor = necp_buffer_write_tlv_if_different(
          cursor, NECP_CLIENT_RESULT_NAT64, sizeof(result.nat64_prefixes),
          result.nat64_prefixes, &updated, client->result,
          sizeof(client->result));
      break;
    }
  }
  ...
  if (result.mss_recommended != 0) {
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_RECOMMENDED_MSS,
  ...
  for (int i = 0; i < NECP_MAX_NETAGENTS; i++) {
    ...
    if (necp_netagent_applies_to_client(client, parsed_parameters,
                                        &netagent.netagent_uuid, TRUE, 0, 0)) {
      cursor = necp_buffer_write_tlv_if_different(
          cursor, NECP_CLIENT_RESULT_NETAGENT, sizeof(netagent), &netagent,
          &updated, client->result, sizeof(client->result));
    }
  }
  ...
  if (result.routed_interface_index != IFSCOPE_NONE &&
      result.routed_interface_index <= (u_int32_t)if_index) {
    direct_interface = ifindex2ifnet[result.routed_interface_index];
             ...
             parsed_parameters->required_interface_index <=
                 (u_int32_t)if_index) {
    // If the request was scoped, but the route didn't match, still grab the
    // agents
    direct_interface =
        ifindex2ifnet[parsed_parameters->required_interface_index];
  } else if (result.routed_interface_index == IFSCOPE_NONE &&
             result.routing_result == NECP_KERNEL_POLICY_RESULT_SOCKET_SCOPED &&
             result.routing_result_parameter.scoped_interface_index !=
                 IFSCOPE_NONE) {
    direct_interface =
        ifindex2ifnet[result.routing_result_parameter.scoped_interface_index];
  }
  ...
  if (result.routing_result == NECP_KERNEL_POLICY_RESULT_IP_TUNNEL &&
      ...
      parsed_parameters->required_interface_index != IFSCOPE_NONE &&
      parsed_parameters->required_interface_index !=
          result.routing_result_parameter.tunnel_interface_index &&
  ...
  if (original_scoped_interface != NULL) {
    ...
    interface_struct.generation =
        ifnet_get_generation(original_scoped_interface);
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_INTERFACE, sizeof(interface_struct),
        &interface_struct, &updated, client->result, sizeof(client->result));
  }
  if (direct_interface != NULL) {
    struct necp_client_result_interface interface_struct;
    ...
    if (ifnet_updown_delta(direct_interface, &updown_delta) == 0) {
      ...
      cursor = necp_buffer_write_tlv_if_different(
          cursor, NECP_CLIENT_RESULT_INTERFACE_TIME_DELTA, sizeof(delta),
          &delta, &ignore_updated, client->result, sizeof(client->result));
    }
  }
  ...
  if (parsed_parameters->flags & NECP_CLIENT_PARAMETER_FLAG_MULTIPATH &&
    ...
    if (direct_interface != NULL) {
      // Add nexus agent
      necp_client_add_agent_interface_options(client, parsed_parameters,
                                              direct_interface);

      // Add interface option in case it is not a nexus
      necp_client_add_interface_option_if_needed(
          client, direct_interface->if_index,
    ...
    if (parsed_parameters->flags & NECP_CLIENT_PARAMETER_FLAG_INBOUND) {
      ...
      TAILQ_FOREACH(multi_interface, &ifnet_head, if_link) {
        if ((multi_interface->if_flags & (IFF_UP | IFF_RUNNING)) &&
            necp_ifnet_matches_parameters(multi_interface, parsed_parameters, 0,
                                          NULL, true, false)) {
      ...
      TAILQ_FOREACH(multi_interface, &ifnet_ordered_head, if_ordered_link) {
        if (multi_interface != direct_interface &&
          ...
          necp_client_add_interface_option_if_needed(
              client, multi_interface->if_index,
    ...
    if (result.routing_result == NECP_KERNEL_POLICY_RESULT_SOCKET_SCOPED) {
      ...
      TAILQ_FOREACH(listen_interface, &ifnet_head, if_link) {
        if ((listen_interface->if_flags & (IFF_UP | IFF_RUNNING)) &&
            necp_ifnet_matches_parameters(listen_interface, parsed_parameters,
                                          0, NULL, true, false)) {
          // Add nexus agents for listeners
          necp_client_add_agent_interface_options(client, parsed_parameters,
                                                  listen_interface);
    ...
    if (result.routing_result == NECP_KERNEL_POLICY_RESULT_SOCKET_SCOPED &&
      ...
      if (direct_interface != NULL) {
        ...
        necp_client_add_browse_interface_options(client, parsed_parameters,
                                                 direct_interface);
      }
    } else {
      ...
      TAILQ_FOREACH(browse_interface, &ifnet_head, if_link) {
        if (necp_ifnet_matches_parameters(browse_interface, parsed_parameters,
                                          0, NULL, true, false)) {
          necp_client_add_browse_interface_options(client, parsed_parameters,
                                                   browse_interface);
        }
    ...
    if (original_scoped_interface->if_agentids != NULL) {
      for (u_int32_t i = 0; i < original_scoped_interface->if_agentcount; i++) {
        ...
        for (int j = 0; j < NECP_MAX_NETAGENTS; j++) {
          ...
          if ((result.netagent_use_flags[j] & NECP_AGENT_USE_FLAG_REMOVE) &&
              uuid_compare(original_scoped_interface->if_agentids[i],
                           result.netagents[j]) == 0) {
            skip_agent = true;
            break;
          }
        ...
        if (!skip_agent &&
            necp_agent_is_removed_by_type(
                &result, original_scoped_interface->if_agentids[i])) {
          skip_agent = true;
        }
        ...
        uuid_copy(netagent.netagent_uuid,
                  original_scoped_interface->if_agentids[i]);
        netagent.generation = netagent_get_generation(netagent.netagent_uuid);
        if (necp_netagent_applies_to_client(
                client, parsed_parameters, &netagent.netagent_uuid, FALSE,
                original_scoped_interface->if_index,
                ifnet_get_generation(original_scoped_interface))) {
          cursor = necp_buffer_write_tlv_if_different(
              cursor, NECP_CLIENT_RESULT_NETAGENT, sizeof(netagent), &netagent,
              &updated, client->result, sizeof(client->result));
    ...
    if (direct_interface->if_agentids != NULL) {
      for (u_int32_t i = 0; i < direct_interface->if_agentcount; i++) {
        ...
        for (int j = 0; j < NECP_MAX_NETAGENTS; j++) {
          ...
          if ((result.netagent_use_flags[j] & NECP_AGENT_USE_FLAG_REMOVE) &&
              uuid_compare(direct_interface->if_agentids[i],
                           result.netagents[j]) == 0) {
            skip_agent = true;
            break;
          }
        ...
        if (!skip_agent && necp_agent_is_removed_by_type(
                               &result, direct_interface->if_agentids[i])) {
          skip_agent = true;
        }
        ...
        netagent.generation = netagent_get_generation(netagent.netagent_uuid);
        if (necp_netagent_applies_to_client(
                client, parsed_parameters, &netagent.netagent_uuid, TRUE,
                direct_interface->if_index,
    ...
    if (delegate_interface->if_agentids != NULL) {
      for (u_int32_t i = 0; i < delegate_interface->if_agentcount; i++) {
        ...
        for (int j = 0; j < NECP_MAX_NETAGENTS; j++) {
          ...
          if ((result.netagent_use_flags[j] & NECP_AGENT_USE_FLAG_REMOVE) &&
              uuid_compare(delegate_interface->if_agentids[i],
                           result.netagents[j]) == 0) {
            skip_agent = true;
            break;
          }
        ...
        if (!skip_agent && necp_agent_is_removed_by_type(
                               &result, delegate_interface->if_agentids[i])) {
          skip_agent = true;
        }
  ...
  if (throughput.up != 0 || throughput.down != 0) {
    cursor = necp_buffer_write_tlv_if_different(
        cursor, NECP_CLIENT_RESULT_ESTIMATED_THROUGHPUT, sizeof(throughput),
        &throughput, &updated, client->result, sizeof(client->result));
  }
  ...
  for (u_int32_t option_i = 0; option_i < client->interface_option_count;
    ...
    if (option_i < NECP_CLIENT_INTERFACE_OPTION_STATIC_COUNT) {
      struct necp_client_interface_option *option =
          &client->interface_options[option_i];
      cursor = necp_buffer_write_tlv_if_different(
          cursor, NECP_CLIENT_RESULT_INTERFACE_OPTION, sizeof(*option), option,
          &updated, client->result, sizeof(client->result));
    } else {
      struct necp_client_interface_option *option =
          &client->extra_interface_options
...
static bool necp_defunct_client_fd_locked_inner(
  ...
  RB_FOREACH(client, _necp_client_tree, &client_fd->clients) {
    ...
    // Prepare close events to be sent to the nexus to effectively remove the
    // flows
    struct necp_client_flow *search_flow = NULL;
    RB_FOREACH(flow_registration, _necp_client_flow_tree,
               &client->flow_registrations) {
    ...
    if (destroy_stats) {
#if SKYWALK
      // Free any remaining stats objects back to the arena where they came
      // from; do this independent of the above defunct check, as the client may
      // have been marked as defunct separately via
      // necp_defunct_client_for_policy().
      RB_FOREACH(flow_registration, _necp_client_flow_tree,
                 &client->flow_registrations) {
...
static inline void
necp_defunct_client_fd_locked(struct necp_fd_data *client_fd,
                              struct _necp_flow_defunct_list *defunct_list,
                              struct proc *proc) {
#pragma unused(proc)
  bool updated_result = FALSE;
  ...
  updated_result =
      necp_defunct_client_fd_locked_inner(client_fd, defunct_list, true);

#if SKYWALK
  // and tear down the currently-active arena's regions now that the redirection
  // and freeing are done
  if (nai != NULL) {
...
static inline void
necp_update_client_fd_locked(struct necp_fd_data *client_fd, proc_t proc,
                             struct _necp_flow_defunct_list *defunct_list) {
  struct necp_client *client = NULL;
  bool updated_result = FALSE;
  NECP_FD_ASSERT_LOCKED(client_fd);
...
#if SKYWALK
static void necp_close_empty_arenas_callout(__unused thread_call_param_t dummy,
                                            __unused thread_call_param_t arg) {
  struct necp_fd_data *client_fd = NULL;
...
static void necp_update_all_clients_callout(__unused thread_call_param_t dummy,
                                            __unused thread_call_param_t arg) {
  struct necp_fd_data *client_fd = NULL;
...
void necp_update_all_clients(void) {
  necp_update_all_clients_immediately_if_needed(false);
}

void necp_update_all_clients_immediately_if_needed(
  ...
  clock_interval_to_absolutetime_interval(leeway_to_use, NSEC_PER_USEC,
                                          &leeway);
  ...
  if (necp_debug > 0) {
    NECPLOG(LOG_DEBUG, "necp_update_all_clients immediate %s update %u",
            should_update_immediately ? "true" : "false", count);
  }
}

bool necp_set_client_as_background(proc_t proc, struct fileproc *fp,
                                   bool background) {
  if (proc == PROC_NULL) {
...
void necp_fd_memstatus(proc_t proc, uint32_t status,
                       struct necp_fd_data *client_fd) {
...
void necp_fd_defunct(proc_t proc, struct necp_fd_data *client_fd) {
  struct _necp_flow_defunct_list defunct_list;
...
void necp_client_request_in_process_flow_divert(pid_t pid) {
  if (pid == 0) {
    return;
  }
...
static void necp_client_remove_agent_from_result(struct necp_client *client,
                                                 uuid_t netagent_uuid) {
  size_t offset = 0;
  ...
  while ((offset + sizeof(struct necp_tlv_header)) <= client->result_length) {
    u_int8_t type =
        necp_buffer_get_tlv_type(result_buffer, client->result_length, offset);
    u_int32_t length = necp_buffer_get_tlv_length(
        result_buffer, client->result_length, offset);
    ...
    if (type == NECP_CLIENT_RESULT_NETAGENT &&
      ...
      struct necp_client_result_netagent *value =
          ((struct necp_client_result_netagent *)(void *)
               necp_buffer_get_tlv_value(result_buffer, client->result_length,
                                         offset, NULL));
      if (uuid_compare(value->netagent_uuid, netagent_uuid) == 0) {
        ...
        memset(result_buffer + client->result_length, 0,
               sizeof(client->result) - client->result_length);
        continue;
      }
    }
...
void necp_force_update_client(uuid_t client_id, uuid_t remove_netagent_uuid,
  ...
  LIST_FOREACH(client_fd, &necp_fd_list, chain) {
    ...
    if (client != NULL) {
      ...
      uuid_copy(client->failed_trigger_agent.netagent_uuid,
...
#if SKYWALK
void necp_client_early_close(uuid_t client_id) {
  NECP_CLIENT_TREE_LOCK_SHARED();
...
#define NECP_PARSED_PARAMETERS_INTERESTING_IFNET_FIELDS                        \
  (NECP_PARSED_PARAMETERS_FIELD_LOCAL_ADDR |                                   \
   NECP_PARSED_PARAMETERS_FIELD_PROHIBITED_IF |                                \
   NECP_PARSED_PARAMETERS_FIELD_REQUIRED_IFTYPE |                              \
   NECP_PARSED_PARAMETERS_FIELD_PROHIBITED_IFTYPE |                            \
...
#define NECP_PARSED_PARAMETERS_SCOPED_FIELDS                                   \
  (NECP_PARSED_PARAMETERS_FIELD_LOCAL_ADDR |                                   \
   NECP_PARSED_PARAMETERS_FIELD_REQUIRED_IFTYPE |                              \
   NECP_PARSED_PARAMETERS_FIELD_REQUIRED_AGENT |                               \
   NECP_PARSED_PARAMETERS_FIELD_PREFERRED_AGENT |                              \
...
#define NECP_PARSED_PARAMETERS_SCOPED_IFNET_FIELDS                             \
  (NECP_PARSED_PARAMETERS_FIELD_LOCAL_ADDR |                                   \
   NECP_PARSED_PARAMETERS_FIELD_REQUIRED_IFTYPE)

#define NECP_PARSED_PARAMETERS_PREFERRED_FIELDS                                \
  (NECP_PARSED_PARAMETERS_FIELD_PREFERRED_AGENT |                              \
   NECP_PARSED_PARAMETERS_FIELD_AVOIDED_AGENT |                                \
   NECP_PARSED_PARAMETERS_FIELD_PREFERRED_AGENT_TYPE |                         \
   NECP_PARSED_PARAMETERS_FIELD_AVOIDED_AGENT_TYPE)

static bool necp_ifnet_matches_type(struct ifnet *ifp, u_int8_t interface_type,
...
static bool necp_ifnet_matches_name(struct ifnet *ifp,
                                    const char *__sized_by(IFXNAMSIZ)
                                        interface_name,
                                    bool check_delegates) {
  struct ifnet *check_ifp = ifp;
  while (check_ifp) {
...
static bool necp_ifnet_matches_agent(struct ifnet *ifp, uuid_t *agent_uuid,
                                     bool check_delegates) {
  struct ifnet *check_ifp = ifp;
...
static bool necp_ifnet_matches_agent_type(
    struct ifnet *ifp, const char *__sized_by(NETAGENT_DOMAINSIZE) agent_domain,
    const char *__sized_by(NETAGENT_TYPESIZE) agent_type,
    bool check_delegates) {
  struct ifnet *check_ifp = ifp;
  ...
  while (check_ifp != NULL) {
    ...
    if (check_ifp->if_agentids != NULL) {
      for (u_int32_t index = 0; index < check_ifp->if_agentcount; index++) {
        ...
        if (netagent_get_agent_domain_and_type(check_ifp->if_agentids[index],
                                               if_agent_domain,
                                               if_agent_type)) {
          if (necp_agent_types_match(agent_domain, agent_type, if_agent_domain,
                                     if_agent_type)) {
            ifnet_lock_done(check_ifp);
            return TRUE;
          }
...
static bool necp_ifnet_matches_local_address(struct ifnet *ifp,
  ...
  if (address.ss_family == AF_INET6) {
    if (in6_embedded_scope || !IN6_IS_SCOPE_EMBED(&SIN6(&address)->sin6_addr)) {
...
necp_interface_type_should_match_unranked_interfaces(u_int8_t interface_type) {
  ...
  // These are the interface types we allow a client to request even if the
  // matching interface isn't currently eligible to be primary (has default
  // route, dns, etc)
...
#define NECP_IFP_IS_ON_ORDERED_LIST(_ifp)                                      \
  ((_ifp)->if_ordered_link.tqe_next != NULL ||                                 \
   (_ifp)->if_ordered_link.tqe_prev != NULL)
...
static bool necp_ifnet_matches_parameters(
  ...
  if (parsed_parameters->valid_fields &
    ...
    if (!necp_ifnet_matches_local_address(
            ifp, SA(&parsed_parameters->local_addr.sa))) {
      return FALSE;
    }
  ...
  if (parsed_parameters->valid_fields & NECP_PARSED_PARAMETERS_FIELD_FLAGS) {
    if (override_flags != 0) {
      ...
      if (!(override_flags &
            NECP_CLIENT_PARAMETER_FLAG_ALLOW_ULTRA_CONSTRAINED) &&
          IFNET_IS_ULTRA_CONSTRAINED(ifp)) {
        return FALSE;
      }
    } else {
      if ((parsed_parameters->flags &
  ...
  if ((!secondary_interface || // Enforce interface type if this is the primary
                               // interface
       !(parsed_parameters->valid_fields &
         NECP_PARSED_PARAMETERS_FIELD_FLAGS) || // or if there are no flags
       !(parsed_parameters->flags &
         NECP_CLIENT_PARAMETER_FLAG_ONLY_PRIMARY_REQUIRES_TYPE)) && // or if the
                                                                    // flags
                                                                    // don't
                                                                    // give an
                                                                    // exception
      (parsed_parameters->valid_fields &
       NECP_PARSED_PARAMETERS_FIELD_REQUIRED_IFTYPE) &&
      !necp_ifnet_matches_type(ifp, parsed_parameters->required_interface_type,
                               FALSE)) {
    return FALSE;
  }
  ...
  if (parsed_parameters->valid_fields &
    ...
    for (int i = 0; i < NECP_MAX_INTERFACE_PARAMETERS; i++) {
      ...
      if (necp_ifnet_matches_type(
              ifp, parsed_parameters->prohibited_interface_types[i], TRUE)) {
        return FALSE;
      }
    }
  }
  ...
  if (parsed_parameters->valid_fields &
      NECP_PARSED_PARAMETERS_FIELD_PROHIBITED_IF) {
    for (int i = 0; i < NECP_MAX_INTERFACE_PARAMETERS; i++) {
      if (strbuflen(parsed_parameters->prohibited_interfaces[i],
                    sizeof(parsed_parameters->prohibited_interfaces[i])) == 0) {
        break;
      }

      if (necp_ifnet_matches_name(
              ifp, parsed_parameters->prohibited_interfaces[i], TRUE)) {
        return FALSE;
      }
    }
  }
  ...
  if (parsed_parameters->valid_fields &
    ...
    for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
      ...
      if (!necp_ifnet_matches_agent(
              ifp, &parsed_parameters->required_netagents[i], FALSE)) {
        return FALSE;
      }
  ...
  if (parsed_parameters->valid_fields &
    ...
    for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
      ...
      if (necp_ifnet_matches_agent(
              ifp, &parsed_parameters->prohibited_netagents[i], TRUE)) {
        return FALSE;
      }
    }
  }
  ...
  if (parsed_parameters->valid_fields &
    ...
    for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
      if (strbuflen(
              ...
              sizeof(parsed_parameters->required_netagent_types[i]
                         .netagent_domain)) == 0 &&
          strbuflen(parsed_parameters->required_netagent_types[i].netagent_type,
                    sizeof(parsed_parameters->required_netagent_types[i]
                               .netagent_type)) == 0) {
        break;
      }

      if (!necp_ifnet_matches_agent_type(
              ifp,
              parsed_parameters->required_netagent_types[i].netagent_domain,
              parsed_parameters->required_netagent_types[i].netagent_type,
              FALSE)) {
  ...
  if (parsed_parameters->valid_fields &
    ...
    for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
      if (strbuflen(
              parsed_parameters->prohibited_netagent_types[i].netagent_domain,
              sizeof(parsed_parameters->prohibited_netagent_types[i]
                         .netagent_domain)) == 0 &&
          strbuflen(
              parsed_parameters->prohibited_netagent_types[i].netagent_type,
              sizeof(parsed_parameters->prohibited_netagent_types[i]
  ...
  if (preferred_count) {
    if (parsed_parameters->valid_fields &
      ...
      for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
        ...
        if (necp_ifnet_matches_agent(
                ifp, &parsed_parameters->preferred_netagents[i], TRUE)) {
          (*preferred_count)++;
          if (require_scoped_field) {
            matched_some_scoped_field = TRUE;
          }
    ...
    if (parsed_parameters->valid_fields &
      ...
      for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
        if (strbuflen(
                parsed_parameters->preferred_netagent_types[i].netagent_domain,
                sizeof(parsed_parameters->preferred_netagent_types[i]
                           .netagent_domain)) == 0 &&
            strbuflen(
                parsed_parameters->preferred_netagent_types[i].netagent_type,
                sizeof(parsed_parameters->preferred_netagent_types[i]
    ...
    if (parsed_parameters->valid_fields &
      ...
      for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
        ...
        if (!necp_ifnet_matches_agent(
                ifp, &parsed_parameters->avoided_netagents[i], TRUE)) {
          (*preferred_count)++;
        }
      }
    }
    ...
    if (parsed_parameters->valid_fields &
      ...
      for (int i = 0; i < NECP_MAX_AGENT_PARAMETERS; i++) {
        if (strbuflen(
                parsed_parameters->avoided_netagent_types[i].netagent_domain,
                sizeof(parsed_parameters->avoided_netagent_types[i]
                           .netagent_domain)) == 0 &&
            strbuflen(
                parsed_parameters->avoided_netagent_types[i].netagent_type,
                sizeof(parsed_parameters->avoided_netagent_types[i]
...
static bool necp_find_matching_interface_index(
  ...
  *return_ifindex = 0;
  ...
  has_preferred_fields = (parsed_parameters->valid_fields &
  ...
  if (!(parsed_parameters->valid_fields &
    ...
    // If the first interface in the list matches, or there are no ordered
    // interfaces, we don't need to scope
    ifp = TAILQ_FIRST(&ifnet_ordered_head);
    if (ifp == NULL || necp_ifnet_matches_parameters(ifp, parsed_parameters, 0,
                                                     NULL, false, false)) {
    ...
    if (parsed_parameters->valid_fields &
        ...
        parsed_parameters->remote_addr.sin6.sin6_scope_id <=
            (u_int32_t)if_index) {
      ifp = ifindex2ifnet[parsed_parameters->remote_addr.sin6.sin6_scope_id];
      if (ifp != NULL && necp_ifnet_matches_parameters(ifp, parsed_parameters,
                                                       0, NULL, false, false)) {
        // Don't set return_ifindex, so the client doesn't need to scope since
        // the v6 scope ID will already route to the correct interface
        ifnet_head_done();
        return TRUE;
      }
    }
  ...
  TAILQ_FOREACH(ifp, &ifnet_ordered_head, if_ordered_link) {
    ...
    if (necp_ifnet_matches_parameters(ifp, parsed_parameters, flags,
                                      &preferred_count, false, false)) {
      if (preferred_count > best_preferred_count || *return_ifindex == 0) {
    ...
    if (has_prohibit_flags && ifp == TAILQ_FIRST(&ifnet_ordered_head)) {
  ...
  if (has_preferred_fields && best_preferred_count == 0 &&
      ((parsed_parameters->valid_fields &
       ...
       (parsed_parameters->valid_fields &
        NECP_PARSED_PARAMETERS_PREFERRED_FIELDS))) {
    // If only has preferred ifnet fields, and nothing was found, clear the
    // interface index and return TRUE
    *return_ifindex = 0;
    return TRUE;
  }

  if (*return_ifindex == 0 && !(parsed_parameters->valid_fields &
                                NECP_PARSED_PARAMETERS_SCOPED_IFNET_FIELDS)) {
    // Has required fields, but not including specific interface fields. Pass
    // for now, and check to see if agents are satisfied by policy.
...
void necp_copy_inp_domain_info(struct inpcb *inp, struct socket *so,
  ...
  if (!domain_info->is_silent) {
    ...
    if (inp->inp_necp_attributes.inp_domain_owner != NULL) {
      strlcpy(domain_info->domain_owner,
...
static size_t necp_find_domain_info_common(
    struct necp_client *client,
    u_int8_t *__sized_by(parameters_size) parameters, size_t parameters_size,
    struct necp_client_flow_registration
        *flow_registration, /* For logging purposes only */
    nstat_domain_info *domain_info) {
  if (client == NULL) {
    return 0;
  }
      ...
      if (value != NULL) {
        ...
        case NECP_CLIENT_PARAMETER_DOMAIN_OWNER: {
          size_t length_to_copy =
              MIN(length, sizeof(domain_info->domain_owner));
          strbufcpy(domain_info->domain_owner,
                    sizeof(domain_info->domain_owner), (const char *)value,
                    length_to_copy);
          break;
        }
        case NECP_CLIENT_PARAMETER_DOMAIN_CONTEXT: {
          size_t length_to_copy =
              MIN(length, sizeof(domain_info->domain_tracker_ctxt));
          strbufcpy(domain_info->domain_tracker_ctxt,
                    sizeof(domain_info->domain_tracker_ctxt),
        ...
        case NECP_CLIENT_PARAMETER_ATTRIBUTED_BUNDLE_IDENTIFIER: {
          size_t length_to_copy =
              MIN(length, sizeof(domain_info->domain_attributed_bundle_id));
          strbufcpy(domain_info->domain_attributed_bundle_id,
                    sizeof(domain_info->domain_attributed_bundle_id),
                    (const char *)value, length_to_copy);
          break;
        }
        case NECP_CLIENT_PARAMETER_REMOTE_ADDRESS: {
  ...
  } else if (domain_info->is_tracker && tracker_domain != NULL &&
             tracker_domain_length > 0) {
    size_t length_to_copy =
        MIN(tracker_domain_length, sizeof(domain_info->domain_name));
    strbufcpy(domain_info->domain_name, sizeof(domain_info->domain_name),
              (const char *)tracker_domain, length_to_copy);
  } else if (domain != NULL && domain_length > 0) {
    size_t length_to_copy =
        MIN(domain_length, sizeof(domain_info->domain_name));
    strbufcpy(domain_info->domain_name, sizeof(domain_info->domain_name),
              (const char *)domain, length_to_copy);
  }

  NECP_CLIENT_FLOW_LOG(
      client, flow_registration,
      "Collected stats - domain <%s> owner <%s> ctxt <%s> bundle id <%s> "
      "is_tracker %d is_non_app_initiated %d is_silent %d",
      domain_info->domain_name, domain_info->domain_owner,
      domain_info->domain_tracker_ctxt,
      domain_info->domain_attributed_bundle_id, domain_info->is_tracker,
      domain_info->is_non_app_initiated, domain_info->is_silent);

  return sizeof(nstat_domain_info);
}

static size_t necp_find_conn_extension_info(
    nstat_provider_context ctx,
    int requested_extension, /* The extension to be returned */
    void *__sized_by(buf_size)
        buf, /* If not NULL, the address for extensions to be returned in */
    size_t buf_size) /* The size of the buffer space, typically matching the
                        return from a previous call with a NULL buf pointer */
{
  // Note, the caller has guaranteed that any buffer has been zeroed, there is
  ...
  case NSTAT_EXTENDED_UPDATE_TYPE_DOMAIN:
    // This is for completeness.  The intent is that domain information can be
    // extracted at user level from the TLV parameters
    if (buf == NULL) {
      return sizeof(nstat_domain_info);
    }
    ...
    return necp_find_domain_info_common(client, client->parameters,
                                        client->parameters_length, NULL,
                                        (nstat_domain_info *)buf);
  ...
  case NSTAT_EXTENDED_UPDATE_TYPE_ORIGINAL_NECP_TLV:
    if (buf == NULL) {
      return (client->original_parameters_source != NULL)
                 ? client->original_parameters_source->parameters_length
                 : 0;
    }
    if ((client->original_parameters_source == NULL) ||
        (buf_size < client->original_parameters_source->parameters_length)) {
      return 0;
    }
    memcpy(buf, client->original_parameters_source->parameters,
           client->original_parameters_source->parameters_length);
  ...
  case NSTAT_EXTENDED_UPDATE_TYPE_ORIGINAL_DOMAIN:
    ...
    return necp_find_domain_info_common(
        client, client->original_parameters_source->parameters,
        client->original_parameters_source->parameters_length, NULL,
        (nstat_domain_info *)buf);
...
static size_t necp_find_extension_info(
    userland_stats_provider_context *ctx,
    int requested_extension, /* The extension to be returned */
    void *__sized_by(buf_size)
        buf, /* If not NULL, the address for extensions to be returned in */
    size_t buf_size) /* The size of the buffer space, typically matching the
                        return from a previous call with a NULL buf pointer */
{
  ...
  struct necp_client_flow_registration *__single flow_registration =
      (struct necp_client_flow_registration *)(void *)ctx;
  struct necp_client *client = flow_registration->client;
  ...
  case NSTAT_EXTENDED_UPDATE_TYPE_DOMAIN:
    ...
    return necp_find_domain_info_common(
        client, client->parameters, client->parameters_length,
        flow_registration, (nstat_domain_info *)buf);
  ...
  case NSTAT_EXTENDED_UPDATE_TYPE_BLUETOOTH_COUNTS: {
    ...
    combined_interface_details =
        os_atomic_load(&flow_registration->last_interface_details, relaxed);
    split_interface_details(combined_interface_details, &route_ifindex,
                            &route_ifflags);
    bool is_companionlink_bluetooth =
        (route_ifflags & NSTAT_IFNET_IS_COMPANIONLINK_BT);

    if (buf == NULL) {
      return (is_companionlink_bluetooth ||
              (route_ifflags & NSTAT_IFNET_PEEREGRESSINTERFACE_IS_CELLULAR))
                 ? sizeof(nstat_interface_counts)
                 : 0;
    }
...
static void necp_find_netstat_data(struct necp_client *client,
                                   union necp_sockaddr_union *remote,
                                   pid_t *effective_pid, uid_t *uid,
                                   uuid_t euuid, uid_t *persona_id,
                                   u_int32_t *traffic_class,
  ...
  while ((offset + sizeof(struct necp_tlv_header)) <= parameters_size) {
    ...
    if (length > 0) {
      ...
      if (value != NULL) {
        ...
        // It is an implementation quirk that the remote address can be found in
        // the necp parameters while the local address must be retrieved from
        // the flowswitch
        case NECP_CLIENT_PARAMETER_REMOTE_ADDRESS: {
          if (length >= sizeof(struct necp_policy_condition_addr)) {
            struct necp_policy_condition_addr *address_struct =
  ...
  while ((offset + sizeof(struct necp_tlv_header)) <= parameters_size) {
    ...
    if (type == NECP_CLIENT_PARAMETER_FLAGS) {
      ...
      u_int8_t *__indexable value =
          necp_buffer_get_tlv_value(parameters, parameters_size, offset, NULL);
      if ((value != NULL) &&
          (length >= sizeof(policy_condition_client_flags))) {
        memcpy(&policy_condition_client_flags, value,
               sizeof(policy_condition_client_flags));
        if (policy_condition_client_flags &
...
static bool necp_request_nexus_tcp_netstats(
    userland_stats_provider_context *ctx, u_int32_t *ifflagsp,
    nstat_progress_digest *digestp, nstat_counts *countsp,
    nstat_detailed_counts *detailed_countsp, void *metadatap) {
  struct necp_client_flow_registration *__single flow_registration =
      (struct necp_client_flow_registration *)(void *)ctx;
  struct necp_client *client = flow_registration->client;
  struct necp_all_stats *ustats_kaddr =
  ...
  if (route_ifindex == IFSCOPE_NONE) {
    ...
    NECPLOG(
        LOG_INFO,
        "req tcp stats, failed to get route details for pid %d curproc %d %s\n",
        client->proc_pid, proc_pid(current_proc()),
  ...
  if (sf == NULL) {
    ...
    NECPLOG(LOG_ERR,
            "req tcp stats, necp_client flow_registration flow_stats missing "
            "for pid %d %s curproc %d %s\n",
            client->proc_pid, namebuf, proc_pid(current_proc()),
            proc_best_name(current_proc()));
  ...
  if (digestp) {
    // The digest is intended to give information that may help give insight
    // into the state of the link
    ...
    digestp->rxduplicatebytes =
        tcpstats->necp_tcp_counts.necp_stat_rxduplicatebytes;
    digestp->rxoutoforderbytes =
        tcpstats->necp_tcp_counts.necp_stat_rxoutoforderbytes;
    ...
    digestp->connstatus.probe_activated =
        tcpstats->necp_tcp_extra.probestatus.probe_activated;
    digestp->connstatus.write_probe_failed =
        tcpstats->necp_tcp_extra.probestatus.write_probe_failed;
    digestp->connstatus.read_probe_failed =
        tcpstats->necp_tcp_extra.probestatus.read_probe_failed;
    digestp->connstatus.conn_probe_failed =
        tcpstats->necp_tcp_extra.probestatus.conn_probe_failed;
  ...
  if (countsp) {
    ...
    countsp->nstat_rxduplicatebytes =
        tcpstats->necp_tcp_counts.necp_stat_rxduplicatebytes;
    countsp->nstat_rxoutoforderbytes =
        tcpstats->necp_tcp_counts.necp_stat_rxoutoforderbytes;
    countsp->nstat_txretransmit =
        tcpstats->necp_tcp_counts.necp_stat_txretransmit;
    ...
    countsp->nstat_connectattempts =
        tcpstats->necp_tcp_extra.state >= TCPS_SYN_SENT ? 1 : 0;
    countsp->nstat_connectsuccesses =
        tcpstats->necp_tcp_extra.state >= TCPS_ESTABLISHED ? 1 : 0;

    // Supplement what the user level has told us with what we know from the
    // flowswitch The nstat_counts structure has only one set of packet counts
    // so set them from the trusted flowswitch as clients may use them to
    // calculate header overhead for cell/wifi/wired counts
  ...
  if (detailed_countsp) {
    detailed_countsp->nstat_media_stats.ms_total.ts_rxbytes =
        tcpstats->necp_tcp_counts.necp_stat_rxbytes;
    detailed_countsp->nstat_media_stats.ms_total.ts_txbytes =
        tcpstats->necp_tcp_counts.necp_stat_txbytes;
    detailed_countsp->nstat_media_stats.ms_total.ts_rxpackets =
        tcpstats->necp_tcp_counts.necp_stat_rxpackets;
    detailed_countsp->nstat_media_stats.ms_total.ts_txpackets =
        tcpstats->necp_tcp_counts.necp_stat_txpackets;

    detailed_countsp->nstat_rxduplicatebytes =
        tcpstats->necp_tcp_counts.necp_stat_rxduplicatebytes;
    detailed_countsp->nstat_rxoutoforderbytes =
        tcpstats->necp_tcp_counts.necp_stat_rxoutoforderbytes;
    detailed_countsp->nstat_txretransmit =
        tcpstats->necp_tcp_counts.necp_stat_txretransmit;

    detailed_countsp->nstat_min_rtt =
        tcpstats->necp_tcp_counts.necp_stat_min_rtt;
    detailed_countsp->nstat_avg_rtt =
        tcpstats->necp_tcp_counts.necp_stat_avg_rtt;
    detailed_countsp->nstat_var_rtt =
        tcpstats->necp_tcp_counts.necp_stat_var_rtt;

    // Supplement what the user level has told us with what we know from the
    // flowswitch The user level statistics don't include a bitmap so use the
    // one within the kernel,
    memcpy(&detailed_countsp->nstat_media_stats.ms_total.ts_bitmap,
           &sf->sf_activity, sizeof(sf->sf_activity));

    struct traffic_stats *ts = media_stats_embedded_ts(
        &detailed_countsp->nstat_media_stats, route_ifflags);
    if (ts) {
  ...
  if (metadatap) {
    ...
    necp_find_netstat_data(client, (union necp_sockaddr_union *)&desc->remote,
                           &effective_pid, &desc->uid, desc->euuid,
                           &desc->persona_id, &desc->traffic_class,
                           &desc->fallback_mode);
    desc->epid = (u_int32_t)effective_pid;

    // Metadata from the flow registration
    // This needs to revisited if multiple flows are created from one flow
    // registration
    struct necp_client_flow *flow = NULL;
    LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
    ...
    desc->ifnet_properties |= (sf->sf_flags & SFLOWF_ONLINK)
    ...
    if (cc_alg_index < TCP_CC_ALGO_COUNT) {
      strbufcpy(desc->cc_algo, sizeof(desc->cc_algo),
                tcp_cc_algo_list[cc_alg_index]->name,
                sizeof(tcp_cc_algo_list[cc_alg_index]->name));
    } else {
      strlcpy(desc->cc_algo, "unknown", sizeof(desc->cc_algo));
    }

    desc->connstatus.probe_activated =
        tcpstats->necp_tcp_extra.probestatus.probe_activated;
    desc->connstatus.write_probe_failed =
        tcpstats->necp_tcp_extra.probestatus.write_probe_failed;
    desc->connstatus.read_probe_failed =
        tcpstats->necp_tcp_extra.probestatus.read_probe_failed;
    desc->connstatus.conn_probe_failed =
        tcpstats->necp_tcp_extra.probestatus.conn_probe_failed;
    ...
    if (NECP_ENABLE_CLIENT_TRACE(NECP_CLIENT_TRACE_LEVEL_FLOW)) {
      ...
      NECPLOG(LOG_NOTICE,
              "Collected stats - TCP - epid %d uid %d euuid %s persona id %d",
              desc->epid, desc->uid, euuid_str, desc->persona_id);
    }
  }
...
static bool necp_request_aop_tcp_netstats(
    userland_stats_provider_context *ctx, u_int32_t *ifflagsp,
    nstat_progress_digest *digestp, nstat_counts *countsp,
    nstat_detailed_counts *detailed_countsp, void *metadatap) {
  struct aop_flow_stats flow_stats = {};
  ...
  if (__improbable(flow->flow_tag != flow_stats.flow_id)) {
    NECPLOG(LOG_ERR, "aop flow stats, flow tag 0x%x != 0x%x", flow->flow_tag,
            flow_stats.flow_id);
    return false;
  }
...
// Called from NetworkStatistics when it wishes to collect latest information
// for a TCP flow. It is a responsibility of NetworkStatistics to have
// previously zeroed any supplied memory.
static bool necp_request_tcp_netstats(userland_stats_provider_context *ctx,
                                      u_int32_t *ifflagsp,
  ...
  struct necp_client_flow_registration *__single flow_registration =
      (struct necp_client_flow_registration *)(void *)ctx;
  if (__probable(!flow_registration->aop_offload)) {
    return necp_request_nexus_tcp_netstats(ctx, ifflagsp, digestp, countsp,
                                           detailed_countsp, metadatap);
  } else {
    return necp_request_aop_tcp_netstats(ctx, ifflagsp, digestp, countsp,
                                         detailed_countsp, metadatap);
  }
}
...
// Called from NetworkStatistics when it wishes to collect latest information
// for a UDP flow.
static bool necp_request_udp_netstats(userland_stats_provider_context *ctx,
                                      u_int32_t *ifflagsp,
                                      nstat_progress_digest *digestp,
                                      nstat_counts *countsp,
                                      nstat_detailed_counts *detailed_countsp,
...
#pragma unused(digestp)
  ...
  if (route_ifindex == IFSCOPE_NONE) {
    ...
    NECPLOG(
        LOG_INFO,
        "req udp stats, failed to get route details for pid %d curproc %d %s\n",
        client->proc_pid, proc_pid(current_proc()),
        proc_best_name(current_proc()));
  }
  ...
  if (sf == NULL) {
    ...
    proc_name(client->proc_pid, namebuf, sizeof(namebuf));
    NECPLOG(LOG_ERR,
            "req udp stats, necp_client flow_registration flow_stats missing "
            "for pid %d %s curproc %d %s\n",
            client->proc_pid, namebuf, proc_pid(current_proc()),
            proc_best_name(current_proc()));
    sf = &ntstat_sk_stats_zero;
  ...
  if (countsp) {
    ...
    countsp->nstat_rxduplicatebytes =
        udpstats->necp_udp_counts.necp_stat_rxduplicatebytes;
    countsp->nstat_rxoutoforderbytes =
        udpstats->necp_udp_counts.necp_stat_rxoutoforderbytes;
    countsp->nstat_txretransmit =
        udpstats->necp_udp_counts.necp_stat_txretransmit;
  ...
  if (detailed_countsp) {
    detailed_countsp->nstat_media_stats.ms_total.ts_rxbytes =
        udpstats->necp_udp_counts.necp_stat_rxbytes;
    detailed_countsp->nstat_media_stats.ms_total.ts_txbytes =
        udpstats->necp_udp_counts.necp_stat_txbytes;
    detailed_countsp->nstat_media_stats.ms_total.ts_rxpackets =
        udpstats->necp_udp_counts.necp_stat_rxpackets;
    detailed_countsp->nstat_media_stats.ms_total.ts_txpackets =
        udpstats->necp_udp_counts.necp_stat_txpackets;
    ...
    detailed_countsp->nstat_min_rtt =
        udpstats->necp_udp_counts.necp_stat_min_rtt;
    detailed_countsp->nstat_avg_rtt =
        udpstats->necp_udp_counts.necp_stat_avg_rtt;
    detailed_countsp->nstat_var_rtt =
        udpstats->necp_udp_counts.necp_stat_var_rtt;
  ...
  if (metadatap) {
    ...
    if (NECP_ENABLE_CLIENT_TRACE(NECP_CLIENT_TRACE_LEVEL_FLOW)) {
      ...
      uuid_unparse(desc->euuid, euuid_str);
      NECPLOG(LOG_NOTICE,
              "Collected stats - UDP - epid %d uid %d euuid %s persona id %d",
              desc->epid, desc->uid, euuid_str, desc->persona_id);
    }
  }
...
// Still to keep the logic separate for future divergence, keeping the routines
// separate. It also seems there are lots of common code between existing
// implementations and it would be good to refactor this logic at some point.
static bool necp_request_quic_netstats(userland_stats_provider_context *ctx,
                                       u_int32_t *ifflagsp,
                                       nstat_progress_digest *digestp,
                                       nstat_counts *countsp,
                                       nstat_detailed_counts *detailed_countsp,
  ...
  if (route_ifindex == IFSCOPE_NONE) {
    ...
    route_ifflags = NSTAT_IFNET_IS_UNKNOWN_TYPE;
    NECPLOG(LOG_INFO,
            "req quic stats, failed to get route details for pid %d curproc %d "
            "%s\n",
            client->proc_pid, proc_pid(current_proc()),
            proc_best_name(current_proc()));
  }
  ...
  if (sf == NULL) {
    ...
    proc_name(client->proc_pid, namebuf, sizeof(namebuf));
    NECPLOG(LOG_ERR,
            "req quic stats, necp_client flow_registration flow_stats missing "
            "for pid %d %s curproc %d %s\n",
            client->proc_pid, namebuf, proc_pid(current_proc()),
            proc_best_name(current_proc()));
    sf = &ntstat_sk_stats_zero;
  ...
  if (digestp) {
    ...
    digestp->rxduplicatebytes =
        quicstats->necp_quic_counts.necp_stat_rxduplicatebytes;
    digestp->rxoutoforderbytes =
        quicstats->necp_quic_counts.necp_stat_rxoutoforderbytes;
    ...
    digestp->connstatus.probe_activated =
        quicstats->necp_quic_extra.probestatus.probe_activated;
    digestp->connstatus.write_probe_failed =
        quicstats->necp_quic_extra.probestatus.write_probe_failed;
    digestp->connstatus.read_probe_failed =
        quicstats->necp_quic_extra.probestatus.read_probe_failed;
    digestp->connstatus.conn_probe_failed =
        quicstats->necp_quic_extra.probestatus.conn_probe_failed;
  ...
  if (countsp) {
    ...
    countsp->nstat_rxoutoforderbytes =
        quicstats->necp_quic_counts.necp_stat_rxoutoforderbytes;
    countsp->nstat_txretransmit =
        quicstats->necp_quic_counts.necp_stat_txretransmit;
    ...
    // TODO: It would be good to expose QUIC stats for CH/SH retransmission and
    // connection state Supplement what the user level has told us with what we
    // know from the flowswitch The nstat_counts structure has only one set of
    // packet counts so set them from the trusted flowswitch as clients may use
    // them to calculate header overhead for cell/wifi/wired counts
    countsp->nstat_rxpackets = sf->sf_ipackets;
    countsp->nstat_txpackets = sf->sf_opackets;
    if (route_ifflags & NSTAT_IFNET_IS_CELLULAR) {
      countsp->nstat_cell_rxbytes = sf->sf_ibytes;
  ...
  if (detailed_countsp) {
    detailed_countsp->nstat_media_stats.ms_total.ts_rxbytes =
        quicstats->necp_quic_counts.necp_stat_rxbytes;
    detailed_countsp->nstat_media_stats.ms_total.ts_txbytes =
        quicstats->necp_quic_counts.necp_stat_txbytes;
    detailed_countsp->nstat_media_stats.ms_total.ts_rxpackets =
        quicstats->necp_quic_counts.necp_stat_rxpackets;
    detailed_countsp->nstat_media_stats.ms_total.ts_txpackets =
        quicstats->necp_quic_counts.necp_stat_txpackets;
    ...
    detailed_countsp->nstat_min_rtt =
        quicstats->necp_quic_counts.necp_stat_min_rtt;
    detailed_countsp->nstat_avg_rtt =
        quicstats->necp_quic_counts.necp_stat_avg_rtt;
    detailed_countsp->nstat_var_rtt =
        quicstats->necp_quic_counts.necp_stat_var_rtt;
  ...
  if (metadatap) {
    ...
    if (NECP_ENABLE_CLIENT_TRACE(NECP_CLIENT_TRACE_LEVEL_FLOW)) {
      ...
      uuid_unparse(desc->euuid, euuid_str);
      NECPLOG(LOG_NOTICE,
              "Collected stats - QUIC - epid %d uid %d euuid %s persona id %d",
              desc->epid, desc->uid, euuid_str, desc->persona_id);
    }
  }
  return true;
...
static void necp_client_inherit_from_parent(struct necp_client *client,
...
static void necp_find_conn_netstat_data(struct necp_client *client,
                                        u_int32_t *ntstat_flags,
                                        pid_t *effective_pid, uuid_t *puuid,
                                        uid_t *uid, uuid_t *euuid,
                                        uid_t *persona_id) {
  ...
  if (ntstat_flags) {
    *ntstat_flags =
        (has_remote_address && has_ip_protocol && has_transport_protocol)
            ? NSTAT_NECP_CONN_HAS_NET_ACCESS
            : 0;
  }
}

static bool necp_request_conn_netstats(nstat_provider_context ctx,
                                       u_int32_t *ifflagsp,
                                       nstat_counts *countsp,
  ...
  if (desc) {
    ...
    // Metadata, that the necp client should have, in TLV format.
    pid_t effective_pid = client->proc_pid;
    necp_find_conn_netstat_data(client, &desc->ifnet_properties, &effective_pid,
                                &desc->puuid, &desc->uid, &desc->euuid,
                                &desc->persona_id);
    desc->epid = (u_int32_t)effective_pid;

    // User level should obtain almost all connection information from an
    // extension leaving little to do here
...
static int necp_skywalk_priv_check_cred(proc_t p, kauth_cred_t cred) {
...
#pragma unused(retval)
  ...
  if (uap->flags & NECP_OPEN_FLAG_OBSERVER ||
    ...
    if (necp_skywalk_priv_check_cred(p, kauth_cred_get()) != 0 &&
      ...
      NECPLOG0(LOG_ERR, "Client does not hold necessary entitlement to observe "
                        "other NECP clients");
      error = EACCES;
      goto done;
    }
  ...
  *retval = fd;
  ...
  if (fd_data->flags & NECP_OPEN_FLAG_PUSH_OBSERVER) {
    ...
    RB_FOREACH(existing_client, _necp_client_global_tree,
...
necp_client_add(struct proc *p, struct necp_fd_data *fd_data,
                struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  ...
  if (fd_data->flags & NECP_OPEN_FLAG_PUSH_OBSERVER) {
    NECPLOG0(LOG_ERR, "NECP client observers with push enabled may not add "
                      "their own clients");
    return EINVAL;
  }

  if (uap->client_id == 0 || uap->client_id_len != sizeof(uuid_t) ||
      buffer_size == 0 || buffer_size > NECP_MAX_CLIENT_PARAMETERS_SIZE ||
      uap->buffer == 0) {
    return EINVAL;
  }
  ...
  client->proc_pid = fd_data->proc_pid; // Save off proc pid in case the client
                                        // will persist past fd
...
#if SKYWALK
  struct necp_client_parsed_parameters parsed_parameters = {};
  int parse_error = necp_client_parse_parameters(
      client, client->parameters, (u_int32_t)client->parameters_length,
      &parsed_parameters);
  ...
  if (parse_error == 0 &&
      ((parsed_parameters.valid_fields &
       ...
       (parsed_parameters.valid_fields &
        NECP_PARSED_PARAMETERS_FIELD_ATTRIBUTED_BUNDLE_IDENTIFIER))) {
    bool has_delegation_entitlement =
        (priv_check_cred(kauth_cred_get(), PRIV_NET_PRIVILEGED_SOCKET_DELEGATE,
                         0) == 0);
    if (!has_delegation_entitlement) {
      if (parsed_parameters.valid_fields &
          NECP_PARSED_PARAMETERS_FIELD_DELEGATED_UPID) {
        NECPLOG(LOG_ERR,
                "%s(%d) does not hold the necessary entitlement to delegate "
                "network traffic for other processes by upid",
                proc_name_address(p), proc_pid(p));
      }
      if (parsed_parameters.valid_fields &
          NECP_PARSED_PARAMETERS_FIELD_ATTRIBUTED_BUNDLE_IDENTIFIER) {
        NECPLOG(LOG_ERR,
                "%s(%d) does not hold the necessary entitlement to set "
                "attributed bundle identifier",
                proc_name_address(p), proc_pid(p));
      }
      error = EPERM;
      goto done;
  ...
  if (parse_error == 0 &&
      parsed_parameters.flags & NECP_CLIENT_PARAMETER_FLAG_INTERPOSE) {
    bool has_nexus_entitlement =
        (necp_skywalk_priv_check_cred(p, kauth_cred_get()) == 0);
    if (!has_nexus_entitlement) {
      NECPLOG(LOG_ERR,
              "%s(%d) does not hold the necessary entitlement to open a custom "
              "nexus client",
              proc_name_address(p), proc_pid(p));
      error = EPERM;
      goto done;
  ...
  if (parse_error == 0 &&
      (parsed_parameters.flags & (NECP_CLIENT_PARAMETER_FLAG_CUSTOM_ETHER |
                                  NECP_CLIENT_PARAMETER_FLAG_CUSTOM_IP))) {
    bool has_custom_protocol_entitlement =
        (priv_check_cred(kauth_cred_get(), PRIV_NET_CUSTOM_PROTOCOL, 0) == 0);
    if (!has_custom_protocol_entitlement) {
      NECPLOG(LOG_ERR,
              "%s(%d) does not hold the necessary entitlement for custom "
              "protocol APIs",
              proc_name_address(p), proc_pid(p));
      error = EPERM;
      goto done;
  ...
  if (parse_error == 0 && (parsed_parameters.extended_flags &
                           NECP_CLIENT_PARAMETER_EXTENDED_FLAG_AOP2_OFFLOAD)) {
    bool has_aop_offload_entitlement =
        IOTaskHasEntitlement(task, "com.apple.private.network.aop2_offload");
    if (!has_aop_offload_entitlement) {
      NECPLOG(LOG_ERR,
              "%s(%d) does not hold the necessary entitlement for aop offload",
              proc_name_address(p), proc_pid(p));
      error = EPERM;
      goto done;
    }
  ...
  if (parse_error == 0 &&
      parsed_parameters.flags & NECP_CLIENT_PARAMETER_FLAG_LISTENER &&
      (parsed_parameters.ip_protocol == IPPROTO_TCP ||
       parsed_parameters.ip_protocol == IPPROTO_UDP)) {
    ...
    proc_name(flow_info.nfi_effective_pid, flow_info.nfi_effective_name,
              MAXCOMLEN);
    ...
    switch (parsed_parameters.local_addr.sa.sa_family) {
    case AF_INET: {
      memcpy(&flow_info.nfi_laddr, &parsed_parameters.local_addr.sa,
             parsed_parameters.local_addr.sa.sa_len);
      netns_addr = (uint32_t *)&parsed_parameters.local_addr.sin.sin_addr;
      netns_addr_len = 4;
      break;
    }
    case AF_INET6: {
      memcpy(&flow_info.nfi_laddr.sin6, &parsed_parameters.local_addr.sin6,
             parsed_parameters.local_addr.sa.sa_len);
    ...
    default: {
      NECPLOG(LOG_ERR, "necp_client_add listener invalid address family (%d)",
    ...
    if (parsed_parameters.local_addr.sin.sin_port == 0) {
      error = netns_reserve_ephemeral(
          &client->port_reservation, netns_addr, netns_addr_len,
          parsed_parameters.ip_protocol,
          &parsed_parameters.local_addr.sin.sin_port, netns_flags, &flow_info);
      if (error) {
        NECPLOG(LOG_ERR, "necp_client_add netns_reserve_ephemeral error (%d)",
                error);
        goto done;
      }
      ...
      necp_client_update_local_port_parameters(
          client->parameters, (u_int32_t)client->parameters_length,
          parsed_parameters.local_addr.sin.sin_port);
    } else {
      error = netns_reserve(&client->port_reservation, netns_addr,
                            netns_addr_len, parsed_parameters.ip_protocol,
                            parsed_parameters.local_addr.sin.sin_port,
                            netns_flags, &flow_info);
      if (error) {
  ...
  if (parsed_parameters.valid_fields &
      NECP_PARSED_PARAMETERS_FIELD_PARENT_UUID) {
    // The parent "should" be found on fd_data without having to search across
    // the whole necp_fd_list It would be nice to do this a little further down
    // where there's another instance of NECP_FD_LOCK but the logic here depends
    // on the parse paramters
    NECP_FD_LOCK(fd_data);
    parent = necp_client_fd_find_client_unlocked(fd_data,
                                                 parsed_parameters.parent_uuid);
    if (parent != NULL) {
      ...
      struct necp_client_flow_registration *flow_registration = NULL;
      RB_FOREACH(flow_registration, _necp_client_flow_tree,
                 &parent->flow_registrations) {
        num_flow_regs++;
      }
    }
    NECP_FD_UNLOCK(fd_data);
    if (parent == NULL) {
      NECPLOG0(LOG_ERR, "necp_client_add, no necp_client_inherit_from_parent "
                        "as can't find parent on fd_data");
    }
  }
  if (parse_error == 0 && parent != NULL &&
      parsed_parameters.valid_fields &
          NECP_PARSED_PARAMETERS_FIELD_FLOW_DEMUX_PATTERN) {
    do {
      ...
      if (parsed_parameters.ip_protocol != IPPROTO_UDP) {
        NECPLOG(
            LOG_INFO,
            "necp_client_add, flow demux pattern not supported for %d protocol",
            parsed_parameters.ip_protocol);
        break;
      }
      ...
      if (parsed_parameters.local_addr.sa.sa_family != AF_INET &&
        ...
        NECPLOG(
            LOG_INFO,
            "necp_client_add, flow demux pattern not supported for %d family",
            parsed_parameters.local_addr.sa.sa_family);
        break;
      }
      if (parsed_parameters.local_addr.sa.sa_family !=
          parsed_parameters.remote_addr.sa.sa_family) {
        NECPLOG0(LOG_INFO,
      ...
      if (parsed_parameters.local_addr.sa.sa_family !=
          parent_parameters.local_addr.sa.sa_family) {
        NECPLOG0(LOG_INFO,
                 "necp_client_add, parent/child address family mismatch");
        break;
      }
      if (SOCKADDR_CMP(&parsed_parameters.local_addr.sa,
                       &parent_parameters.local_addr.sa,
                       parsed_parameters.local_addr.sa.sa_len)) {
        NECPLOG0(LOG_INFO,
                 "necp_client_add, parent/child local address mismatch");
        break;
      }
      if (SOCKADDR_CMP(&parsed_parameters.remote_addr.sa,
                       &parent_parameters.remote_addr.sa,
                       parsed_parameters.remote_addr.sa.sa_len)) {
        NECPLOG0(LOG_INFO,
                 "necp_client_add, parent/child remote address mismatch");
        break;
      }
      if (parsed_parameters.local_addr.sin.sin_port !=
          parent_parameters.local_addr.sin.sin_port) {
        NECPLOG0(LOG_INFO, "necp_client_add, parent/child local port mismatch");
        break;
      }
      if (parsed_parameters.remote_addr.sin.sin_port !=
          parent_parameters.remote_addr.sin.sin_port) {
        NECPLOG0(LOG_INFO,
  ...
  client->nstat_context = nstat_provider_stats_open(
      (nstat_provider_context)client, NSTAT_PROVIDER_CONN_USERLAND,
      (u_int64_t)ntstat_properties, necp_request_conn_netstats,
      necp_find_conn_extension_info);
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_claim(struct proc *p, struct necp_fd_data *fd_data,
                  struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  uuid_t client_id = {};
  struct necp_client *client = NULL;
  ...
  LIST_FOREACH(find_fd, &necp_fd_list, chain) {
    ...
    struct necp_client *find_client =
        necp_client_fd_find_client_and_lock(find_fd, client_id);
    if (find_client != NULL) {
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_remove(struct necp_fd_data *fd_data,
                   struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  uuid_t client_id = {};
  struct ifnet_stats_per_flow flow_ifnet_stats = {};
  ...
  if (uap->buffer != 0 && buffer_size == sizeof(flow_ifnet_stats)) {
    ...
    if (error) {
      NECPLOG(LOG_ERR, "necp_client_remove flow_ifnet_stats copyin error (%d)",
              error);
    ...
    NECPLOG(LOG_ERR, "necp_client_remove unexpected parameters length (%zu)",
            buffer_size);
  }
  ...
  pid_t pid = fd_data->proc_pid;
  struct necp_client *client =
      necp_client_fd_find_client_unlocked(fd_data, client_id);
...
#if SKYWALK
  // If the currently-active arena is idle (has no more flows referring to it),
  // or if there are defunct arenas lingering in the list, schedule a threadcall
  // to do the clean up.  The idle check is done by checking if the reference
  // count is 3: one held by this client (will be released below when we destroy
  // it) when it's non-NULL; the rest held by stats_arena_{active,list}.
  if ((fd_data->stats_arena_active != NULL &&
       fd_data->stats_arena_active->nai_use_count == 3) ||
      (fd_data->stats_arena_active == NULL &&
       !LIST_EMPTY(&fd_data->stats_arena_list))) {
    uint64_t deadline = 0;
    uint64_t leeway = 0;
    clock_interval_to_deadline(necp_close_arenas_timeout_microseconds,
                               NSEC_PER_USEC, &deadline);
    clock_interval_to_absolutetime_interval(
        necp_close_arenas_timeout_leeway_microseconds, NSEC_PER_USEC, &leeway);
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_remove_flow(struct necp_fd_data *fd_data,
                        struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  uuid_t flow_id = {};
  struct ifnet_stats_per_flow flow_ifnet_stats = {};
  ...
  if (uap->client_id == 0 || uap->client_id_len != sizeof(uuid_t)) {
    error = EINVAL;
    NECPLOG(LOG_ERR, "necp_client_remove_flow invalid client_id (length %zu)",
            (size_t)uap->client_id_len);
    goto done;
  }

  error = copyin(uap->client_id, flow_id, sizeof(uuid_t));
  if (error) {
    NECPLOG(LOG_ERR, "necp_client_remove_flow copyin client_id error (%d)",
            error);
    goto done;
  }

  if (uap->buffer != 0 && buffer_size != 0) {
    error = copyin(uap->buffer, &flow_ifnet_stats,
                   MIN(buffer_size, sizeof(flow_ifnet_stats)));
    if (error) {
      NECPLOG(LOG_ERR, "necp_client_remove flow_ifnet_stats copyin error (%d)",
              error);
      // Not fatal
  ...
  struct necp_client *client = NULL;
  struct necp_client_flow_registration *flow_registration =
      necp_client_fd_find_flow(fd_data, flow_id);
  if (flow_registration != NULL) {
#if SKYWALK
    // Cleanup stats per flow
    necp_destroy_flow_stats(fd_data, flow_registration, &flow_ifnet_stats,
                            TRUE);
#endif /* SKYWALK */
    NECP_FLOW_TREE_LOCK_EXCLUSIVE();
    RB_REMOVE(_necp_client_flow_global_tree, &necp_client_flow_global_tree,
  ...
  if (flow_registration != NULL && client != NULL) {
    ...
    if (flow_registration->client == client) {
      ...
      necp_destroy_client_flow_registration(client, flow_registration,
                                            fd_data->proc_pid, abort);
    }
...
// Don't inline the function since it includes necp_client_parsed_parameters on
// the stack
static __attribute__((noinline)) int necp_client_check_tcp_heuristics(
    struct necp_client *client, struct necp_client_flow *flow, u_int32_t *flags,
    u_int8_t *__counted_by(tfo_cookie_maxlen) tfo_cookie,
    u_int8_t tfo_cookie_maxlen, u_int8_t *tfo_cookie_len) {
  struct necp_client_parsed_parameters parsed_parameters;
  int error = 0;
  ...
  if (check_ecn) {
    if (tcp_heuristic_do_ecn_with_address(
            client->current_route->rt_ifp,
  ...
  if ((parsed_parameters.flags & NECP_CLIENT_PARAMETER_FLAG_TFO_ENABLE) ==
    ...
    if (!tcp_heuristic_do_tfo_with_address(
            client->current_route->rt_ifp,
            (union sockaddr_in_4_6 *)&flow->local_addr,
            (union sockaddr_in_4_6 *)&flow->remote_addr, tfo_cookie,
            tfo_cookie_maxlen, tfo_cookie_len)) {
...
static size_t necp_client_calculate_flow_tlv_size(
    struct necp_client_flow_registration *flow_registration) {
  size_t assigned_results_size = 0;
  struct necp_client_flow *flow = NULL;
  LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
    if (flow->assigned || flow_registration->defunct ||
        !necp_client_endpoint_is_unspecified(
...
necp_client_destination_mac_address(struct sockaddr *remote, uint32_t index,
  ...
  if ((tgt_rt->rt_flags & RTF_HOST) && (tgt_rt->rt_flags & RTF_LLINFO) &&
...
static uint8_t *__sized_by(*buflen)
    necp_client_flow_mac_and_gateway(struct necp_client_flow *flow,
                                     size_t *buflen) {
  ...
  *buflen = 0;
  ...
  if (necp_client_destination_mac_address(
          SA(&flow->remote_addr), flow->interface_index, &remote_ether) == 0) {
  ...
  cursor = buffer;
  if (local_ether_set) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_CLIENT_RESULT_LOCAL_ETHER_ADDR, sizeof(struct ether_addr),
        (uint8_t *)(struct ether_addr * __bidi_indexable) & local_ether, buffer,
        valsize);
  }
  if (remote_ether_set) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_CLIENT_RESULT_REMOTE_ETHER_ADDR, sizeof(struct ether_addr),
        (uint8_t *)(struct ether_addr * __bidi_indexable) & remote_ether,
        buffer, valsize);
  }
  *buflen = valsize;
  return buffer;
...
static int necp_client_fillout_flow_tlvs(
    struct necp_client *client, bool client_is_observed,
    struct necp_client_flow_registration *flow_registration,
    struct necp_client_action_args *uap, size_t *assigned_results_cursor) {
  int error = 0;
  struct necp_client_flow *flow = NULL;
  LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
    if (flow->assigned || flow_registration->defunct ||
      ...
      if (flow->nexus) {
        if (flow->check_tcp_heuristics) {
          ...
          if (necp_client_check_tcp_heuristics(client, flow, &flags, tfo_cookie,
                                               tfo_cookie_len,
                                               &tfo_cookie_len) != 0) {
            tfo_cookie_len = 0;
          } else {
      ...
      if (flow->nexus) {
        if (tfo_cookie_len != 0) {
          header_length = sizeof(struct necp_client_nexus_flow_header) -
                          (NECP_TFO_COOKIE_LEN_MAX - tfo_cookie_len);
        } else {
          header_length = sizeof(struct necp_client_nexus_flow_header) -
      ...
      if (flow->nexus) {
        ...
        agent_struct.generation =
            netagent_get_generation(agent_struct.netagent_uuid);
      ...
      length = (header_length - sizeof(struct necp_tlv_header) +
                flow->assigned_results_length);
      if (flow->has_protoctl_event) {
      ...
      error = copyout(&header,
                      uap->buffer + client->result_length +
                          *assigned_results_cursor,
                      header_length);
      if (error) {
        NECPLOG(
            LOG_ERR,
            "necp_client_copy assigned results tlv_header copyout error (%d)",
            error);
        return error;
      }
      *assigned_results_cursor += header_length;
      ...
      if (flow->assigned_results && flow->assigned_results_length) {
        // Write inner TLVs
        error = copyout(flow->assigned_results,
                        uap->buffer + client->result_length +
                            *assigned_results_cursor,
                        flow->assigned_results_length);
        if (error) {
          NECPLOG(LOG_ERR,
                  "necp_client_copy assigned results copyout error (%d)",
                  error);
          return error;
        }
      }
      *assigned_results_cursor += flow->assigned_results_length;
      ...
      if (flow->has_protoctl_event) {
        ...
        error = copyout(&protoctl_event_header,
                        uap->buffer + client->result_length +
                            *assigned_results_cursor,
      ...
      if (flow->nexus && flow->aop_offload) {
        if (buffer != NULL) {
          ...
          *assigned_results_cursor += buflen;
          ...
          if (error) {
            NECPLOG(LOG_ERR,
                    "necp_client_copy mac address results"
                    " tlv_header copyout error (%d)",
                    error);
            return error;
          }
        ...
        if (flow->aop_stat_index_valid) {
          ...
          error = copyout(&flow_stats_header,
                          uap->buffer + client->result_length +
                              *assigned_results_cursor,
                          sizeof(flow_stats_header));
          if (error) {
            NECPLOG(LOG_ERR,
                    "necp_client_copy flow stats index "
                    "tlv header copyout error (%d)",
                    error);
            return error;
          }
...
static int necp_client_copy_internal(struct necp_client *client,
                                     uuid_t client_id, bool client_is_observed,
  ...
  if (uap->action == NECP_CLIENT_ACTION_COPY_PARAMETERS) {
    ...
    *retval = client->parameters_length;
             ...
             client->result_read && client->group_members_read &&
             !necp_client_has_unread_flows(client)) {
    ...
    *retval = 0;
    ...
    if (some_flow_is_defunct && client->legacy_client_is_flow) {
      ...
      u_int8_t *flags_pointer = necp_buffer_get_tlv_value(
          client->result, client->result_length, 0, &value_size);
      if (flags_pointer != NULL && value_size == sizeof(client_flags)) {
        ...
        (void)necp_buffer_write_tlv_if_different(
            client->result, NECP_CLIENT_RESULT_FLAGS, sizeof(client_flags),
            &client_flags, &flags_updated, client->result,
            sizeof(client->result));
      }
    }
    ...
    if (flags_updated) {
      // Revert stored flags
      (void)necp_buffer_write_tlv_if_different(
          client->result, NECP_CLIENT_RESULT_FLAGS, sizeof(original_flags),
          &original_flags, &flags_updated, client->result,
          sizeof(client->result));
    }
    ...
    if (client->assigned_group_members != NULL &&
        client->assigned_group_members_length > 0) {
      error = copyout(client->assigned_group_members,
                      uap->buffer + client->result_length,
                      client->assigned_group_members_length);
      if (error != 0) {
        NECPLOG(LOG_ERR, "necp_client_copy group members copyout error (%d)",
                error);
        return error;
      }
    }
    ...
    size_t assigned_results_cursor =
        client->assigned_group_members_length; // Start with an offset based on
                                               // the group members
    if (necp_client_id_is_flow(client_id)) {
      if (single_flow_registration != NULL) {
        error = necp_client_fillout_flow_tlvs(client, client_is_observed,
                                              single_flow_registration, uap,
                                              &assigned_results_cursor);
        if (error != 0) {
          return error;
        }
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_copy(struct necp_fd_data *fd_data,
                 struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  struct necp_client *client = NULL;
  uuid_t client_id;
  ...
  *retval = 0;
  ...
  if (uap->client_id) {
    if (uap->client_id_len != sizeof(uuid_t)) {
      NECPLOG(LOG_ERR, "Incorrect length (got %zu, expected %zu)",
              (size_t)uap->client_id_len, sizeof(uuid_t));
      return ERANGE;
    }
  ...
  if (is_wildcard) {
    if (uap->action == NECP_CLIENT_ACTION_COPY_RESULT ||
      ...
      RB_FOREACH(find_client, _necp_client_tree, &fd_data->clients) {
        ...
        if (!find_client->result_read || !find_client->group_members_read ||
      ...
      if (client == NULL && fd_data->request_in_process_flow_divert) {
        // No client found that needs update. Check for an event requesting
        // in-process flow divert.
  ...
  if (send_in_process_flow_divert_message) {
    ...
    if (uap->buffer_size < sizeof(request_tlv)) {
      ...
      if (error) {
        NECPLOG(LOG_ERR,
                "necp_client_copy request flow divert TLV copyout error (%d)",
                error);
      } else {
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_copy_client_update(struct necp_fd_data *fd_data,
                               struct necp_client_action_args *uap,
                               int *retval) {
  int error = 0;
  ...
  if (client_update != NULL) {
    ...
    if (error) {
      ...
      if (uap->buffer_size < client_update->update_length) {
        NECPLOG(LOG_ERR, "Buffer size cannot hold update (%zu < %zu)",
                (size_t)uap->buffer_size, client_update->update_length);
        error = EINVAL;
      } else {
        error = copyout(client_update->update, uap->buffer,
                        client_update->update_length);
        if (error) {
...
static int necp_client_copy_parameters_locked(
...
#endif /* !SKYWALK */
  memcpy(&parameters->local_addr, &parsed_parameters.local_addr,
         sizeof(parameters->local_addr));
  memcpy(&parameters->remote_addr, &parsed_parameters.remote_addr,
         sizeof(parameters->remote_addr));
  ...
  parameters->is_listener =
      (parsed_parameters.flags & NECP_CLIENT_PARAMETER_FLAG_LISTENER) ? 1 : 0;
  parameters->is_interpose =
      (parsed_parameters.flags & NECP_CLIENT_PARAMETER_FLAG_INTERPOSE) ? 1 : 0;
  parameters->is_custom_ether =
      (parsed_parameters.flags & NECP_CLIENT_PARAMETER_FLAG_CUSTOM_ETHER) ? 1
                                                                          : 0;
  ...
  flags_pointer = necp_buffer_get_tlv_value(
      client->result, client->result_length, 0, &value_size);
  if (flags_pointer && value_size == sizeof(client_result_flags)) {
    memcpy(&client_result_flags, flags_pointer, value_size);
  }
  parameters->allow_qos_marking =
      (client_result_flags & NECP_CLIENT_RESULT_FLAG_ALLOW_QOS_MARKING) ? 1 : 0;

  if (parsed_parameters.valid_fields &
      NECP_PARSED_PARAMETERS_FIELD_LOCAL_ADDR_PREFERENCE) {
    if (parsed_parameters.local_address_preference ==
        NECP_CLIENT_PARAMETER_LOCAL_ADDRESS_PREFERENCE_DEFAULT) {
      parameters->override_address_selection = false;
    } else if (parsed_parameters.local_address_preference ==
  ...
  if ((parsed_parameters.valid_fields & NECP_PARSED_PARAMETERS_FIELD_FLAGS) &&
      (parsed_parameters.flags &
       NECP_CLIENT_PARAMETER_FLAG_NO_WAKE_FROM_SLEEP)) {
  ...
  if (!parameters->is_listener) {
    if (parsed_parameters.valid_fields &
      ...
      if (parsed_parameters.demux_patterns[0].len == 0) {
        ...
        if (client->validated_parent) {
          ...
          NECP_CLIENT_TREE_LOCK_SHARED();
          struct necp_client *parent =
              necp_find_client_and_lock(client->parent_client_id);
          if (parent != NULL) {
            struct necp_client_flow_registration *parent_flow_registration =
                NULL;
            RB_FOREACH(parent_flow_registration, _necp_client_flow_tree,
                       &parent->flow_registrations) {
              uuid_copy(parameters->parent_flow_uuid,
                        parent_flow_registration->registration_id);
              break;
            }
          ...
          if (parsed_parameters.demux_pattern_count > 0) {
            for (int i = 0; i < parsed_parameters.demux_pattern_count; i++) {
              memcpy(&parameters->demux_patterns[i],
                     &parsed_parameters.demux_patterns[i],
                     sizeof(struct necp_demux_pattern));
            }
            parameters->demux_pattern_count =
                parsed_parameters.demux_pattern_count;
          }
        }
      }
    ...
    if (parsed_parameters.valid_fields &
        NECP_PARSED_PARAMETERS_FIELD_EXTENDED_FLAGS) {
      if (parsed_parameters.extended_flags &
          NECP_CLIENT_PARAMETER_EXTENDED_FLAG_AOP2_OFFLOAD) {
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_list(struct necp_fd_data *fd_data,
                 struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  struct necp_client *find_client = NULL;
  ...
  if (!(fd_data->flags & NECP_OPEN_FLAG_OBSERVER)) {
    NECPLOG0(LOG_ERR, "Client does not hold necessary entitlement to list "
                      "other NECP clients");
    error = EACCES;
    goto done;
  }

  error = copyin(uap->buffer, &requested_client_count,
                 sizeof(requested_client_count));
  if (error) {
    goto done;
  }

  if (os_mul_overflow(sizeof(uuid_t), requested_client_count,
                      &copy_buffer_size)) {
    error = ERANGE;
    goto done;
  }
  ...
  if (requested_client_count > 0 && client_count > 0 && list != NULL) {
    error = copyout(list, uap->buffer + sizeof(client_count), copy_buffer_size);
    if (error) {
      NECPLOG(LOG_ERR, "necp_client_list client count copyout error (%d)",
              error);
      goto done;
    }
  }
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_add_flow(struct necp_fd_data *fd_data,
                     struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  struct necp_client *client = NULL;
  uuid_t client_id;
  ...
  if (uap->client_id == 0 || uap->client_id_len != sizeof(uuid_t)) {
    error = EINVAL;
    NECPLOG(LOG_ERR, "necp_client_add_flow invalid client_id (length %zu)",
            (size_t)uap->client_id_len);
    goto done;
  }
  ...
  if (uap->buffer == 0 || buffer_size < sizeof(struct necp_client_add_flow) ||
    ...
    error = EINVAL;
    NECPLOG(LOG_ERR, "necp_client_add_flow invalid buffer (length %zu)",
            buffer_size);
    goto done;
  }
  ...
  if (buffer_size <= sizeof(struct necp_client_add_flow_default)) {
    ...
    if (error) {
      NECPLOG(LOG_ERR,
              "necp_client_add_flow copyin default_add_request error (%d)",
              error);
      goto done;
    }
    ...
    allocated_add_request = (struct necp_client_add_flow *)kalloc_data(
        buffer_size, Z_WAITOK | Z_ZERO);
  ...
  struct necp_client_flow_registration *new_registration =
      necp_client_create_flow_registration(fd_data, client);
  ...
  // If NECP_CLIENT_FLOW_FLAGS_OPEN_FLOW_ON_BEHALF_OF_CLIENT is set, then set
  // registration_id_to_add to the old value in add_request->registration_id,
  // otherwise use the new value in new_registration->registration_id.
  bool open_flow_on_behalf_of_client =
      (add_request->flags &
       NECP_CLIENT_FLOW_FLAGS_OPEN_FLOW_ON_BEHALF_OF_CLIENT);
  uuid_t registration_id_to_add = {};
  if (open_flow_on_behalf_of_client &&
      !uuid_is_null(add_request->registration_id)) {
  ...
  size_t trailer_offset = (sizeof(struct necp_client_add_flow) +
                           add_request->stats_request_count *
  ...
  if (add_request->flags & NECP_CLIENT_FLOW_FLAGS_OVERRIDE_ADDRESS) {
    ...
    if (buffer_size >= offset_of_address + sizeof(struct sockaddr_in)) {
      ...
      if (buffer_size >= offset_of_address + override_address->sa_len &&
        ...
        SOCKADDR_COPY(override_address, &parameters.remote_addr,
                      override_address->sa_len);
        ...
        if (parameters.remote_addr.sa.sa_family == AF_INET) {
          ...
          memset((uint8_t *)&parameters.local_addr.sin6.sin6_addr, 0,
  ...
  if (add_request->flags & NECP_CLIENT_FLOW_FLAGS_OVERRIDE_IP_PROTOCOL) {
    ...
    if (buffer_size >= offset_of_ip_protocol + sizeof(uint8_t)) {
      uint8_t *__single ip_protocol_p =
          flow_req_get_proto(add_request, offset_of_ip_protocol);
      memcpy(&parameters.ip_protocol, ip_protocol_p, sizeof(uint8_t));
    }
  }

  // If opening the flow on behalf of the client, then replace the pid and
  // parameters.pid with the effective PID so that the client's PID is used for
  // this flow instead of the PID of the process making the requests.
  if (open_flow_on_behalf_of_client) {
  ...
  if (add_request->flags & NECP_CLIENT_FLOW_FLAGS_ALLOW_NEXUS) {
    ...
    if (!found_nexus) {
      ...
      necp_client_add_nexus_flow_if_needed(
          new_registration, add_request->agent_uuid, interface_index,
          parameters.use_aop_offload);

      error = netagent_client_message_with_params(
          add_request->agent_uuid,
          ((new_registration->flags & NECP_CLIENT_FLOW_FLAGS_USE_CLIENT_ID)
               ? client->client_id
               : registration_id_to_add),
          pid, client->agent_handle, NETAGENT_MESSAGE_TYPE_REQUEST_NEXUS,
          (struct necp_client_agent_parameters *)&parameters, &assigned_results,
      ...
      if (error != 0) {
        ...
        if (!necp_assign_client_result_locked(
                ...
                add_request->agent_uuid, assigned_results,
                assigned_results_length, false, false)) {
        ...
        LIST_FOREACH(flow, &new_registration->flow_list, flow_chain) {
          if (flow->nexus &&
            ...
            if (flow->assigned_results == NULL) {
              SOCKADDR_COPY(override_address, &flow->remote_addr,
                            override_address->sa_len);
              uuid_t empty_uuid;
              uuid_clear(empty_uuid);
              size_t message_length;
  ...
  if (error == 0 && add_request->stats_request_count > 0 &&
      necp_arena_initialize(fd_data, true) == 0) {
    struct necp_client_flow_stats *__single stats_request =
        &(necp_client_get_flow_stats(add_request))[0];
    struct necp_stats_bufreq bufreq = {};
  ...
  if (error == 0 && parameters.use_aop_offload) {
    error = necp_aop_offload_stats_initialize(new_registration,
                                              add_request->agent_uuid);
  }
#endif /* !SKYWALK */

  if (error == 0 && (add_request->flags & NECP_CLIENT_FLOW_FLAGS_BROWSE ||
    ...
    TAILQ_FOREACH(interface, &ifnet_head, if_link) {
      ...
      if (interface->if_agentids != NULL) {
        for (u_int32_t i = 0; i < interface->if_agentcount; i++) {
          if (uuid_compare(interface->if_agentids[i],
                           add_request->agent_uuid) == 0) {
    ...
    error = netagent_client_message_with_params(
        ...
        ((new_registration->flags & NECP_CLIENT_FLOW_FLAGS_USE_CLIENT_ID)
             ? client->client_id
             : new_registration->registration_id),
        pid, client->agent_handle, NETAGENT_MESSAGE_TYPE_CLIENT_ASSERT,
        (struct necp_client_agent_parameters *)&parameters, &dummy_results,
        &dummy_length);
    if (error != 0) {
      NECPLOG(LOG_ERR, "netagent_client_message error (%d)", error);
    }
  ...
  if (error != 0) {
    ...
    necp_destroy_client_flow_registration(client, new_registration,
                                          fd_data->proc_pid, true);
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_client_add_flow copyout add_request error (%d)",
            error);
  }
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_request_nexus(struct necp_fd_data *fd_data,
                          struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  struct necp_client *client = NULL;
  uuid_t client_id;
  ...
  error = copyin(uap->client_id, client_id, sizeof(uuid_t));
  if (error) {
    NECPLOG(LOG_ERR, "necp_client_request_nexus copyin client_id error (%d)",
            error);
    goto done;
  }
  ...
  if (proc == PROC_NULL) {
    NECP_FD_UNLOCK(fd_data);
    NECPLOG(LOG_ERR,
            "necp_client_request_nexus process not found for pid %d error (%d)",
            pid, error);
    error = ESRCH;
    goto done;
  }
  ...
  // Using REQUEST_NEXUS indicates that the client only supports one flow per
  // client
  ...
  do {
    ...
    new_registration->flags = (NECP_CLIENT_FLOW_FLAGS_ALLOW_NEXUS |
                               NECP_CLIENT_FLOW_FLAGS_USE_CLIENT_ID);

    necp_client_add_nexus_flow_if_needed(new_registration, nexus_uuid,
                                         interface_index,
                                         parameters.use_aop_offload);

    // Note: Any clients using "request_nexus" are not flow-registration aware.
    // Register the Client ID rather than the Registration ID with the nexus,
    // since the client will send traffic based on the client ID.
    error = netagent_client_message_with_params(
        nexus_uuid,
        ((new_registration->flags & NECP_CLIENT_FLOW_FLAGS_USE_CLIENT_ID)
             ? client->client_id
    ...
    if (assigned_results != NULL) {
      if (!necp_assign_client_result_locked(
              proc, fd_data, client, new_registration, nexus_uuid,
    ...
    if (uap->buffer != 0 && buffer_size == sizeof(struct necp_stats_bufreq) &&
      ...
      if (copy_error) {
        NECPLOG(LOG_ERR, "necp_client_request_nexus copyin bufreq error (%d)",
                copy_error);
      } else {
        (void)necp_stats_initialize(fd_data, client, new_registration, &bufreq);
        ...
        if (copy_error != 0) {
          NECPLOG(LOG_ERR,
                  "necp_client_request_nexus copyout bufreq error (%d)",
                  copy_error);
        }
      }
    }
...
static void necp_client_add_assertion(struct necp_client *client,
  ...
  new_assertion =
      kalloc_type(struct necp_client_assertion, Z_WAITOK | Z_NOFAIL);
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_agent_action(struct necp_fd_data *fd_data,
                         struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  struct necp_client *client = NULL;
  uuid_t client_id;
  ...
  error = copyin(uap->client_id, client_id, sizeof(uuid_t));
  if (error) {
    NECPLOG(LOG_ERR, "necp_client_agent_action copyin client_id error (%d)",
            error);
    goto done;
  }

  if (buffer_size > NECP_MAX_AGENT_ACTION_SIZE) {
    NECPLOG(LOG_ERR, "necp_client_agent_action invalid buffer size (>%u)",
  ...
  if (error) {
    NECPLOG(LOG_ERR, "necp_client_agent_action parameters copyin error (%d)",
            error);
    goto done;
  }
  ...
  if (client != NULL) {
    ...
    while ((offset + sizeof(struct necp_tlv_header)) <= buffer_size) {
      ...
      u_int32_t length =
          necp_buffer_get_tlv_length(parameters, buffer_size, offset);
      ...
      if (length >= sizeof(uuid_t)) {
        u_int8_t *__indexable value =
            necp_buffer_get_tlv_value(parameters, buffer_size, offset, NULL);
        if (value == NULL) {
        ...
        if (type == NECP_CLIENT_PARAMETER_TRIGGER_AGENT ||
          ...
          error = netagent_client_message_with_params(
              agent_uuid, client_id, fd_data->proc_pid, client->agent_handle,
          ...
          if ((length - sizeof(uuid_t)) >= sizeof(agent_params.u.error.error)) {
            memcpy(&agent_params.u.error.error, (value + sizeof(uuid_t)),
                   sizeof(agent_params.u.error.error));
          }
          bool agent_reported = false;
          for (int agent_i = 0; agent_i < NECP_FD_REPORTED_AGENT_COUNT;
               agent_i++) {
            if (uuid_compare(agent_uuid,
                             fd_data->reported_agents.agent_uuid[agent_i]) ==
                0) {
          ...
          if (!agent_reported) {
            ...
            for (int agent_i = 0; agent_i < NECP_FD_REPORTED_AGENT_COUNT;
              ...
              if (uuid_is_null(fd_data->reported_agents.agent_uuid[agent_i])) {
                uuid_copy(fd_data->reported_agents.agent_uuid[agent_i],
                          agent_uuid);
            ...
            if (!saved_agent_uuid) {
              ...
              for (int agent_i = 0; agent_i < NECP_FD_REPORTED_AGENT_COUNT;
                ...
                if (agent_i + 1 < NECP_FD_REPORTED_AGENT_COUNT) {
                  uuid_copy(fd_data->reported_agents.agent_uuid[agent_i],
                            fd_data->reported_agents.agent_uuid[agent_i + 1]);
                } else {
                  uuid_copy(fd_data->reported_agents.agent_uuid[agent_i],
                            agent_uuid);
                }
          ...
          error = netagent_client_message_with_params(
              agent_uuid, client_id, fd_data->proc_pid, client->agent_handle,
              NETAGENT_MESSAGE_TYPE_CLIENT_ERROR, &agent_params, &dummy_results,
              &dummy_length);
          if (error == 0) {
            acted_on_agent = TRUE;
          } else {
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_copy_agent(__unused struct necp_fd_data *fd_data,
                       struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  uuid_t agent_uuid;
  const size_t buffer_size = uap->buffer_size;
  ...
  if (error) {
    NECPLOG(LOG_ERR, "necp_client_copy_agent copyin agent_uuid error (%d)",
            error);
    goto done;
  }
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_agent_use(struct necp_fd_data *fd_data,
                      struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  struct necp_client *client = NULL;
  uuid_t client_id;
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_acquire_agent_token(__unused struct necp_fd_data *fd_data,
                                struct necp_client_action_args *uap,
                                int *retval) {
  int error = 0;
  uuid_t agent_uuid = {};
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_copy_interface(__unused struct necp_fd_data *fd_data,
                           struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  ...
  if (error) {
    NECPLOG(LOG_ERR,
            "necp_client_copy_interface copyin interface_index error (%d)",
            error);
    goto done;
  }
  ...
  if (interface_index == 0) {
    error = ENOENT;
    NECPLOG(LOG_ERR, "necp_client_copy_interface bad interface_index (%d)",
            interface_index);
    goto done;
  }
  ...
  if (interface != NULL) {
    if (interface->if_xname != NULL) {
      strlcpy((char *)&interface_details.name, interface->if_xname,
              sizeof(interface_details.name));
    }
...
#if SKYWALK
    fsw_get_tso_capabilities(interface,
                             &interface_details.tso_max_segment_size_v4,
                             &interface_details.tso_max_segment_size_v6);

    interface_details.hwcsum_flags = interface->if_hwassist & IFNET_CHECKSUMF;
#endif /* SKYWALK */

    u_int8_t ipv4_signature_len =
        sizeof(interface_details.ipv4_signature.signature);
    u_int16_t ipv4_signature_flags;
    if (ifnet_get_netsignature(
            interface, AF_INET, &ipv4_signature_len, &ipv4_signature_flags,
    ...
    v4Route =
        rtalloc1_scoped_locked(SA(&default_address), 0, 0, interface->if_index);
    if (v4Route != NULL) {
    ...
    u_int8_t ipv6_signature_len =
        sizeof(interface_details.ipv6_signature.signature);
    u_int16_t ipv6_signature_flags;
    if (ifnet_get_netsignature(
            interface, AF_INET6, &ipv6_signature_len, &ipv6_signature_flags,
    ...
    TAILQ_FOREACH(ifa, &interface->if_addrhead, ifa_link) {
      ...
      if (ifa->ifa_addr->sa_family == AF_INET) {
        ...
        interface_details.ipv4_netmask =
            (ifatoia(ifa))->ia_sockmask.sin_addr.s_addr;
        if (interface->if_flags & IFF_BROADCAST) {
          interface_details.flags |= NECP_INTERFACE_FLAG_HAS_BROADCAST;
          interface_details.ipv4_broadcast =
              (ifatoia(ifa))->ia_broadaddr.sin_addr.s_addr;
        }
      }
      IFA_UNLOCK(ifa);
    }
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_get_interface_address(__unused struct necp_fd_data *fd_data,
                                  struct necp_client_action_args *uap,
                                  int *retval) {
  int error = 0;
  ...
  if (uap->client_id == 0 || uap->client_id_len != sizeof(u_int32_t) ||
      ...
      buffer_size > sizeof(struct sockaddr_storage) || uap->buffer == 0) {
  ...
  if (error) {
    NECPLOG(
        LOG_ERR,
        "necp_client_get_interface_address copyin interface_index error (%d)",
        error);
    goto done;
  }
  ...
  if (interface_index == IFSCOPE_NONE) {
    error = ENOENT;
    NECPLOG(LOG_ERR,
            "necp_client_get_interface_address bad interface_index (%d)",
            interface_index);
    goto done;
  }
  ...
  if (error) {
    NECPLOG(LOG_ERR,
            "necp_client_get_interface_address copyin address error (%d)",
            error);
    goto done;
  }
  ...
  if (address.ss_family != AF_INET && address.ss_family != AF_INET6) {
    error = EINVAL;
    NECPLOG(LOG_ERR,
            "necp_client_get_interface_address invalid address family (%u)",
            address.ss_family);
    goto done;
  }
  ...
  if (address.ss_len != buffer_size) {
    error = EINVAL;
    NECPLOG(LOG_ERR,
            "necp_client_get_interface_address invalid address length (%u)",
            address.ss_len);
    goto done;
  }
  ...
  error = flow_route_select_laddr((union sockaddr_in_4_6 *)&local_address,
                                  (union sockaddr_in_4_6 *)&address, ifp, rt,
                                  &gencount, 1);
  rtfree(rt);
  rt = NULL;
  ...
  if (error) {
    NECPLOG(
        LOG_ERR,
        "necp_client_get_interface_address local address selection failed (%d)",
        error);
    goto done;
  }
  ...
  if (local_address.ss_len > buffer_size) {
    error = EMSGSIZE;
    NECPLOG(LOG_ERR,
            "necp_client_get_interface_address local address too long for "
            "buffer (%u)",
  ...
  if (error) {
    NECPLOG(LOG_ERR, "necp_client_get_interface_address copyout error (%d)",
            error);
    goto done;
  }
done:
...
int necp_stats_ctor(struct skmem_obj_info *oi, struct skmem_obj_info *oim,
...
int necp_stats_dtor(void *addr, void *arg) {
...
necp_fd_mredirect_stats_arena(struct necp_fd_data *fd_data, struct proc *proc) {
  ...
  if ((nai = fd_data->stats_arena_active) != NULL) {
    ...
    int err = skmem_arena_mredirect(nai->nai_arena, &nai->nai_mmap, proc,
                                    &need_defunct);
    VERIFY(err == 0);
...
static int necp_arena_create(struct necp_fd_data *fd_data, size_t obj_size,
  ...
  name_ptr = tsnprintf(name, sizeof(name), "stats-%u.%s.%d",
                       fd_data->stats_arena_gencnt, proc_name_address(p),
                       fd_data->proc_pid);
  nai->nai_arena =
      skmem_arena_create_for_necp(name_ptr, &srp_ustats, &srp_kstats, &error);
  ...
  if (error != 0) {
    ...
    nai->nai_roff =
        skmem_arena_get_region_offset(nai->nai_arena, SKMEM_REGION_USTATS);
...
static int necp_arena_stats_obj_alloc(struct necp_fd_data *fd_data,
  ...
  kstats_cp = skmem_arena_necp(fd_data->stats_arena_active->nai_arena)
                  ->arc_kstats_cache;
  if ((kstats_obj_tmp = skmem_cache_alloc(
           kstats_cp, (cansleep ? SKMEM_SLEEP : SKMEM_NOSLEEP))) == NULL) {
    return ENOMEM;
  }
  ...
  ustats_obj = __unsafe_forge_bidi_indexable(
      uint8_t *, kstats->necp_stats_ustats, kstats_obj_sz);
...
#pragma unused(fd_data)
  ...
  skmem_cache_free(skmem_arena_necp(stats_arena->nai_arena)->arc_kstats_cache,
                   *kstats_kaddr);
  *kstats_kaddr = NULL;
  *ustats_uaddr = 0;
}
...
static void necp_stats_arenas_destroy(struct necp_fd_data *fd_data,
  ...
  // If reaping (not closing), release reference only for idle active arena; the
  // reference count must be 2 by now, when it's not being referred to by any
  // clients/flows.
  if ((nai = fd_data->stats_arena_active) != NULL &&
      (closing || nai->nai_use_count == 2)) {
    VERIFY(nai->nai_use_count >= 2);
    necp_arena_info_release(nai); // for fd_data
    fd_data->stats_arena_active = NULL;
  }
...
static int necp_arena_initialize(struct necp_fd_data *fd_data, bool locked) {
  int error = 0;
  size_t stats_obj_size =
      MAX(sizeof(struct necp_all_stats), sizeof(struct necp_all_kstats));
...
static int necp_sysctl_arena_initialize(struct necp_fd_data *fd_data,
  ...
  if (fd_data->sysctl_arena == NULL) {
    ...
    name_ptr = tsnprintf(name, sizeof(name), "sysctl.%s.%d",
                         proc_name_address(p), fd_data->proc_pid);
    ...
    if (error != 0) {
      ...
      // doesn't need to be mmap'd at this point, since we simply
      // compute the relative offset.
      fd_data->system_sysctls_roff = skmem_arena_get_region_offset(
          fd_data->sysctl_arena, SKMEM_REGION_SYSCTLS);
...
static int necp_client_stats_bufreq(
    struct necp_fd_data *fd_data, struct necp_client *client,
    struct necp_client_flow_registration *flow_registration,
    struct necp_stats_bufreq *bufreq, struct necp_stats_hdr *out_header) {
  int error = 0;
  NECP_CLIENT_ASSERT_LOCKED(client);
  NECP_FD_ASSERT_LOCKED(fd_data);
  ...
  if ((bufreq->necp_stats_bufreq_id == NECP_CLIENT_STATISTICS_BUFREQ_ID) &&
      ((bufreq->necp_stats_bufreq_type == NECP_CLIENT_STATISTICS_TYPE_TCP &&
        bufreq->necp_stats_bufreq_ver ==
            NECP_CLIENT_STATISTICS_TYPE_TCP_CURRENT_VER) ||
       (bufreq->necp_stats_bufreq_type == NECP_CLIENT_STATISTICS_TYPE_UDP &&
        bufreq->necp_stats_bufreq_ver ==
            NECP_CLIENT_STATISTICS_TYPE_UDP_CURRENT_VER) ||
       (bufreq->necp_stats_bufreq_type == NECP_CLIENT_STATISTICS_TYPE_QUIC &&
        bufreq->necp_stats_bufreq_ver ==
            NECP_CLIENT_STATISTICS_TYPE_QUIC_CURRENT_VER)) &&
    ...
    if (flow_registration->ustats_uaddr == 0) {
      ...
      ASSERT(flow_registration->kstats_kaddr == NULL);
      ASSERT(flow_registration->ustats_uaddr == 0);
      error = necp_arena_stats_obj_alloc(
          fd_data, &off, &flow_registration->stats_arena,
          &flow_registration->kstats_kaddr, FALSE);
      if (error == 0) {
        // upon success, hold a reference for the client; this is released when
        // the client is removed/closed
        ASSERT(flow_registration->stats_arena != NULL);
        necp_arena_info_retain(flow_registration->stats_arena);

        // compute user address based on mapping info and object offset
        flow_registration->ustats_uaddr =
            flow_registration->stats_arena->nai_mmap.ami_mapaddr + off;

        // add to collect_stats list
        NECP_STATS_LIST_LOCK_EXCLUSIVE();
        necp_client_retain_locked(client); // Add a reference to the client
        LIST_INSERT_HEAD(&necp_collect_stats_flow_list, flow_registration,
                         collect_stats_chain);
        NECP_STATS_LIST_UNLOCK();
    ...
    if (flow_registration->ustats_uaddr != 0) {
      ...
      struct necp_all_kstats *kstats =
          (struct necp_all_kstats *)flow_registration->kstats_kaddr;
      kstats->necp_stats_ustats->all_stats_u.tcp_stats.necp_tcp_hdr
          .necp_stats_type = bufreq->necp_stats_bufreq_type;
      kstats->necp_stats_ustats->all_stats_u.tcp_stats.necp_tcp_hdr
          .necp_stats_ver = bufreq->necp_stats_bufreq_ver;
...
static int necp_client_stats_initial(
    struct necp_client_flow_registration *flow_registration,
    uint32_t stats_type, uint32_t stats_ver) {
  // An attempted create
  ...
  int error = 0;
  uint64_t ntstat_properties =
      necp_find_netstat_initial_properties(flow_registration->client);

  switch (stats_type) {
  case NECP_CLIENT_STATISTICS_TYPE_TCP: {
    if (stats_ver == NECP_CLIENT_STATISTICS_TYPE_TCP_VER_1) {
      flow_registration->stats_handler_context = ntstat_userland_stats_open(
          (userland_stats_provider_context *)flow_registration,
          NSTAT_PROVIDER_TCP_USERLAND, ntstat_properties,
          necp_request_tcp_netstats, necp_find_extension_info);
      if (flow_registration->stats_handler_context == NULL) {
        error = EIO;
      }
    } else {
  ...
  case NECP_CLIENT_STATISTICS_TYPE_UDP: {
    if (stats_ver == NECP_CLIENT_STATISTICS_TYPE_UDP_VER_1) {
      flow_registration->stats_handler_context = ntstat_userland_stats_open(
          (userland_stats_provider_context *)flow_registration,
          NSTAT_PROVIDER_UDP_USERLAND, ntstat_properties,
          necp_request_udp_netstats, necp_find_extension_info);
      if (flow_registration->stats_handler_context == NULL) {
        error = EIO;
      }
    } else {
  ...
  case NECP_CLIENT_STATISTICS_TYPE_QUIC: {
    if (stats_ver == NECP_CLIENT_STATISTICS_TYPE_QUIC_VER_1 &&
        flow_registration->flags & NECP_CLIENT_FLOW_FLAGS_ALLOW_NEXUS) {
      flow_registration->stats_handler_context = ntstat_userland_stats_open(
          (userland_stats_provider_context *)flow_registration,
          NSTAT_PROVIDER_QUIC_USERLAND, ntstat_properties,
          necp_request_quic_netstats, necp_find_extension_info);
      if (flow_registration->stats_handler_context == NULL) {
        error = EIO;
      }
    } else {
...
necp_stats_initialize(struct necp_fd_data *fd_data, struct necp_client *client,
  ...
  VERIFY(!(fd_data->stats_arena_active->nai_flags &
           (NAIF_REDIRECT | NAIF_DEFUNCT)));
  ...
  // Setup stats region
  error = necp_client_stats_bufreq(fd_data, client, flow_registration, bufreq,
                                   &stats_hdr);
  if (error) {
    return error;
  }
  ...
  if (flow_registration->stats_handler_context == NULL) {
    error = necp_client_stats_initial(
        flow_registration, stats_hdr.necp_stats_type, stats_hdr.necp_stats_ver);
    if (flow_registration->stats_handler_context != NULL) {
      ntstat_userland_stats_event(flow_registration->stats_handler_context,
                                  NECP_CLIENT_STATISTICS_EVENT_INIT);
    }
    NECP_CLIENT_FLOW_LOG(client, flow_registration,
                         "Initialized stats <error %d>", error);
  }
...
static int necp_aop_offload_stats_initialize(
  ...
  LIST_FOREACH(flow, &flow_registration->flow_list, flow_chain) {
    ...
    if (flow->nexus && uuid_compare(flow->u.nexus_agent, netagent_uuid) == 0) {
      ...
      error = net_aop_setup_flow(flow->flow_tag, true, &flow->stats_index);
      if (error != 0) {
        NECPLOG(LOG_ERR,
                "failed to setup aop flow "
                "stats area, error %d",
                error);
      } else {
...
static void necp_aop_offload_stats_destroy(struct necp_client_flow *flow) {
  ...
  if (flow->flow_tag != 0 && flow->aop_stat_index_valid) {
    error = net_aop_setup_flow(flow->flow_tag, false, &flow->stats_index);
    if (error != 0) {
      NECPLOG(LOG_ERR, "failed to cleanup aop offload stats with error %d",
              error);
    }
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_map_sysctls(__unused struct necp_fd_data *fd_data,
                        struct necp_client_action_args *uap, int *retval) {
  int result = 0;
  if (!retval) {
    retval = &result;
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_copy_route_statistics(__unused struct necp_fd_data *fd_data,
                                  struct necp_client_action_args *uap,
                                  int *retval) {
  int error = 0;
  struct necp_client *client = NULL;
  ...
  if (error) {
    NECPLOG(LOG_ERR,
            "necp_client_copy_route_statistics copyin client_id error (%d)",
            error);
    goto done;
  }
  ...
  if (client != NULL) {
    ...
    if (client->current_route != NULL &&
        client->current_route->rt_stats != NULL) {
      struct nstat_counts *rt_stats = client->current_route->rt_stats;
      route_stats.necp_stat_rxpackets =
          os_atomic_load(&rt_stats->nstat_rxpackets, relaxed);
      route_stats.necp_stat_rxbytes =
          os_atomic_load(&rt_stats->nstat_rxbytes, relaxed);
      route_stats.necp_stat_txpackets =
          os_atomic_load(&rt_stats->nstat_txpackets, relaxed);
      route_stats.necp_stat_txbytes =
          os_atomic_load(&rt_stats->nstat_txbytes, relaxed);
      route_stats.necp_stat_rxduplicatebytes = rt_stats->nstat_rxduplicatebytes;
      route_stats.necp_stat_rxoutoforderbytes =
          rt_stats->nstat_rxoutoforderbytes;
      ...
      if (__probable(necp_client_stats_use_route_metrics == 0)) {
        ...
        route_stats.necp_stat_var_rtt =
            client->current_route->rt_rmx.rmx_rttvar;
      }
    ...
    if (error) {
      NECPLOG(LOG_ERR, "necp_client_copy_route_statistics copyout error (%d)",
              error);
    }
  } else {
    // Unlock
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_update_cache(struct necp_fd_data *fd_data,
                         struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  struct necp_client *client = NULL;
  uuid_t client_id;
  ...
  error = copyin(uap->client_id, client_id, sizeof(uuid_t));
  if (error) {
    NECPLOG(LOG_ERR, "necp_client_update_cache copyin client_id error (%d)",
            error);
    goto done;
  }
  ...
  if (error) {
    NECPLOG(LOG_ERR, "necp_client_update_cache copyin cache buffer error (%d)",
            error);
    goto done_unlock;
  }
  ...
  if (cache_buffer.necp_cache_buf_type == NECP_CLIENT_CACHE_TYPE_ECN &&
    ...
    error = copyin(cache_buffer.necp_cache_buf_addr, &ecn_cache_buffer,
                   sizeof(necp_tcp_ecn_cache));
    if (error) {
      NECPLOG(LOG_ERR,
              "necp_client_update_cache copyin ecn cache buffer error (%d)",
              error);
      goto done_unlock;
    }

    if (client->current_route != NULL &&
        client->current_route->rt_ifp != NULL) {
      if (!client->platform_binary) {
        ecn_cache_buffer.necp_tcp_ecn_heuristics_success = 0;
      }
      tcp_heuristics_ecn_update(&ecn_cache_buffer,
                                client->current_route->rt_ifp,
                                (union sockaddr_in_4_6 *)&flow->local_addr);
    }
  } else if (cache_buffer.necp_cache_buf_type == NECP_CLIENT_CACHE_TYPE_TFO &&
             cache_buffer.necp_cache_buf_ver ==
    ...
    error = copyin(cache_buffer.necp_cache_buf_addr, &tfo_cache_buffer,
                   sizeof(necp_tcp_tfo_cache));
    if (error) {
      NECPLOG(LOG_ERR,
              "necp_client_update_cache copyin tfo cache buffer error (%d)",
              error);
      goto done_unlock;
    }
    ...
    if (client->current_route != NULL &&
      ...
      tcp_heuristics_tfo_update(&tfo_cache_buffer,
                                client->current_route->rt_ifp,
                                (union sockaddr_in_4_6 *)&flow->local_addr,
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_sign(__unused struct necp_fd_data *fd_data,
                 struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  ...
  *retval = 0;
  ...
  const bool has_resolver_entitlement =
      (priv_check_cred(kauth_cred_get(), PRIV_NET_VALIDATED_RESOLVER, 0) == 0);
  if (!has_resolver_entitlement) {
    NECPLOG0(LOG_ERR, "Process does not hold the necessary entitlement to sign "
                      "resolver answers");
    error = EPERM;
    goto done;
  }

  if (uap->client_id == 0 || signable_length < sizeof(*signable) ||
      signable_length > NECP_CLIENT_ACTION_SIGN_MAX_TOTAL_LENGTH) {
    error = EINVAL;
    goto done;
  }

  if (uap->buffer == 0 ||
      return_tag_length != NECP_CLIENT_ACTION_SIGN_TAG_LENGTH) {
    error = EINVAL;
    goto done;
  }
  ...
  if (signable_length <= sizeof(default_signable)) {
    ...
    if ((allocated_signable = (struct necp_client_signable *)kalloc_data(
             signable_length, Z_WAITOK | Z_ZERO)) == NULL) {
      NECPLOG(LOG_ERR, "necp_client_sign allocate signable %zu failed",
              signable_length);
      error = ENOMEM;
      goto done;
    }
  ...
  case NECP_CLIENT_SIGN_TYPE_RESOLVER_ANSWER:
  case NECP_CLIENT_SIGN_TYPE_SYSTEM_RESOLVER_ANSWER: {
    data_length = (sizeof(struct necp_client_host_resolver_answer) -
                   sizeof(struct necp_client_signable));
    if (signable_length < (sizeof(struct necp_client_signable) + data_length)) {
      error = EINVAL;
      goto done;
    }
    struct necp_client_host_resolver_answer *__single signable_struct =
        (struct necp_client_host_resolver_answer *)signable;
    if (signable_struct->hostname_length >
            NECP_CLIENT_ACTION_SIGN_MAX_STRING_LENGTH ||
        signable_length != (sizeof(struct necp_client_signable) + data_length +
                            signable_struct->hostname_length)) {
      error = EINVAL;
      goto done;
    }
  ...
  case NECP_CLIENT_SIGN_TYPE_BROWSE_RESULT:
  case NECP_CLIENT_SIGN_TYPE_SYSTEM_BROWSE_RESULT: {
    data_length = (sizeof(struct necp_client_browse_result) -
                   sizeof(struct necp_client_signable));
    if (signable_length < (sizeof(struct necp_client_signable) + data_length)) {
      error = EINVAL;
      goto done;
    }
    struct necp_client_browse_result *signable_struct =
        (struct necp_client_browse_result *)signable;
    if (signable_struct->service_length >
            NECP_CLIENT_ACTION_SIGN_MAX_STRING_LENGTH ||
        signable_length != (sizeof(struct necp_client_signable) + data_length +
                            signable_struct->service_length)) {
      error = EINVAL;
      goto done;
    }
  ...
  case NECP_CLIENT_SIGN_TYPE_SERVICE_RESOLVER_ANSWER:
  case NECP_CLIENT_SIGN_TYPE_SYSTEM_SERVICE_RESOLVER_ANSWER: {
    data_length = (sizeof(struct necp_client_service_resolver_answer) -
                   sizeof(struct necp_client_signable));
    if (signable_length < (sizeof(struct necp_client_signable) + data_length)) {
      error = EINVAL;
      goto done;
    }
    struct necp_client_service_resolver_answer *__single signable_struct =
        (struct necp_client_service_resolver_answer *)signable;
    if (signable_struct->service_length >
            NECP_CLIENT_ACTION_SIGN_MAX_STRING_LENGTH ||
        signable_struct->hostname_length >
            NECP_CLIENT_ACTION_SIGN_MAX_STRING_LENGTH ||
  ...
  default: {
    NECPLOG(LOG_ERR, "necp_client_sign unknown signable type (%u)",
            signable->sign_type);
    error = EINVAL;
    goto done;
  }
  ...
  error = necp_sign_resolver_answer(signable->client_id, signable->sign_type,
                                    signable_get_data(signable, data_length),
                                    data_length, tag, &tag_size);
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_validate(__unused struct necp_fd_data *fd_data,
                     struct necp_client_action_args *uap, int *retval) {
  int error = 0;
  ...
  *retval = 0;
  ...
  const bool has_resolver_entitlement =
      (priv_check_cred(kauth_cred_get(), PRIV_NET_VALIDATED_RESOLVER, 0) == 0);
  if (!has_resolver_entitlement) {
    NECPLOG0(LOG_ERR, "Process does not hold the necessary entitlement to "
                      "directly validate resolver answers");
    error = EPERM;
    goto done;
  }

  if (uap->client_id == 0 || validatable_length < sizeof(*validatable) ||
      validatable_length > (NECP_CLIENT_ACTION_SIGN_MAX_TOTAL_LENGTH +
  ...
  if (validatable_length <= sizeof(default_validatable)) {
    ...
    if ((allocated_validatable = (struct necp_client_validatable *)kalloc_data(
             validatable_length, Z_WAITOK | Z_ZERO)) == NULL) {
      NECPLOG(LOG_ERR, "necp_client_validate allocate struct %zu failed",
              validatable_length);
      error = ENOMEM;
      goto done;
    }
  ...
  size_t signable_data_len =
      validatable_length - sizeof(struct necp_client_validatable);
  const bool validated = necp_validate_resolver_answer(
      validatable->signable.client_id, validatable->signable.sign_type,
      signable_get_data(&validatable->signable, signable_data_len),
      signable_data_len, validatable->signature.signed_tag,
      sizeof(validatable->signature.signed_tag));
  if (!validated) {
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_get_signed_client_id(__unused struct necp_fd_data *fd_data,
                                 struct necp_client_action_args *uap,
                                 int *retval) {
  int error = 0;
  *retval = 0;
  ...
  if (error) {
    NECPLOG(LOG_ERR,
            "necp_client_get_signed_client_id copyin request_type error (%d)",
            error);
    goto done;
  }
  ...
  if (request_type != NECP_CLIENT_SIGNED_CLIENT_ID_TYPE_UUID) {
    error = ENOENT;
    NECPLOG(LOG_ERR, "necp_client_get_signed_client_id bad request_type (%d)",
            request_type);
    goto done;
  }
  ...
  error = necp_sign_application_id(
      application_uuid, NECP_CLIENT_SIGNED_CLIENT_ID_TYPE_UUID, tag, &tag_size);
  if (tag_size != sizeof(tag)) {
    NECPLOG(LOG_ERR, "necp_client_get_signed_client_id unexpected tag size %zu",
            tag_size);
    error = EINVAL;
    goto done;
  }
  ...
  if (error != 0) {
    NECPLOG(LOG_ERR, "necp_client_get_signed_client_id copyout error (%d)",
            error);
    goto done;
  }
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_set_signed_client_id(__unused struct necp_fd_data *fd_data,
                                 struct necp_client_action_args *uap,
                                 int *retval) {
  int error = 0;
  *retval = 0;
  ...
  bool has_delegation_entitlement =
      task != NULL &&
      IOTaskHasEntitlement(task, kCSWebBrowserNetworkEntitlement);
  if (!has_delegation_entitlement) {
  ...
  if (!has_delegation_entitlement) {
    NECPLOG0(LOG_ERR, "necp_client_set_signed_client_id client lacks the "
  ...
  if (error) {
    NECPLOG(LOG_ERR,
            "necp_client_set_signed_client_id copyin request_type error (%d)",
            error);
    goto done;
  }
  ...
  if (request_type != NECP_CLIENT_SIGNED_CLIENT_ID_TYPE_UUID) {
    error = ENOENT;
    NECPLOG(LOG_ERR, "necp_client_set_signed_client_id bad request_type (%d)",
            request_type);
    goto done;
  }

  error = copyin(uap->buffer, &client_id,
                 sizeof(struct necp_client_signed_client_id_uuid));
  if (error) {
    NECPLOG(LOG_ERR,
            "necp_client_set_signed_client_id copyin request error (%d)",
            error);
    goto done;
  }

  const bool validated = necp_validate_application_id(
      client_id.client_id, NECP_CLIENT_SIGNED_CLIENT_ID_TYPE_UUID,
      client_id.signature_data, sizeof(client_id.signature_data));
  if (!validated) {
    // Return EAUTH to indicate that the signature failed
    error = EAUTH;
    NECPLOG(LOG_ERR,
            "necp_client_set_signed_client_id signature validation failed (%d)",
            error);
    goto done;
  }

  proc_setresponsibleuuid(proc, client_id.client_id,
                          sizeof(client_id.client_id));
...
static int necp_client_copy_flow_stats(
    struct necp_client_flow_registration *flow_registration,
...
static NECP_CLIENT_ACTION_FUNCTION int
necp_client_get_flow_statistics(struct necp_fd_data *fd_data,
                                struct necp_client_action_args *uap,
                                int *retval) {
  int error = 0;
  uuid_t flow_id = {};
  ...
  if (error) {
    NECPLOG(LOG_ERR,
            "necp_client_get_flow_statistics copyin client_id error (%d)",
            error);
    goto done;
  }
  ...
  if (error) {
    NECPLOG(LOG_ERR,
            "necp_client_get_flow_statistics copyin protocol error (%d)",
            error);
    goto done;
  }
  ...
  if (flow_stats.transport_proto != IPPROTO_TCP) {
    NECPLOG(LOG_ERR,
            "necp_client_get_flow_statistics, transport proto %u not supported",
  ...
  if (flow_registration != NULL && client != NULL) {
    ...
    if (flow_registration->client == client) {
      ...
      if (error == 0) {
        ...
        if (error != 0) {
          NECPLOG(LOG_ERR,
                  "necp_client_get_flow_statistics copyout failed (%d)", error);
        }
      }
    }
...
int necp_client_action(struct proc *p, struct necp_client_action_args *uap,
                       int *retval) {
  struct fileproc *__single fp;
  int error = 0;
  int return_value = 0;
...
int necp_match_policy(struct proc *p, struct necp_match_policy_args *uap,
                      int32_t *retval) {
#pragma unused(retval)
  ...
  if (uap->parameters == 0 || uap->parameters_size == 0 ||
      uap->parameters_size > NECP_MAX_MATCH_POLICY_PARAMETER_SIZE ||
      uap->returned_result == 0) {
    error = EINVAL;
    goto done;
  }
  ...
  error = necp_application_find_policy_match_internal(
      p, parameters, buffer_size, &returned_result, NULL, NULL, 0, NULL, NULL,
  ...
  error = copyout(&returned_result, uap->returned_result,
...
static errno_t necp_set_socket_attribute(u_int8_t *__sized_by(buffer_length)
                                             buffer,
                                         size_t buffer_length, u_int8_t type,
                                         char *__null_terminated *buffer_p,
                                         bool *single_tlv) {
  int error = 0;
  int cursor = 0;
  ...
  if (single_tlv != NULL &&
      (buffer_length == sizeof(struct necp_tlv_header) + string_size)) {
    *single_tlv = true;
  }
  if (string_size == 0 ||
      string_size > NECP_MAX_SOCKET_ATTRIBUTE_STRING_LENGTH) {
    // This will clear out the parameter
    goto done;
  }
  ...
  if (local_string == NULL) {
    NECPLOG(LOG_ERR, "Failed to allocate a socket attribute buffer (size %zu)",
            string_size);
    goto fail;
  }
  ...
  necp_lock_socket_attributes();
  if (local_string != NULL) {
    *buffer_p = __unsafe_null_terminated_from_indexable(
        local_string, &local_string[string_size]);
  } else {
    *buffer_p = NULL;
  }
  necp_unlock_socket_attributes();
...
errno_t necp_set_socket_attributes(struct inp_necp_attributes *attributes,
                                   struct sockopt *sopt) {
  int error = 0;
  u_int8_t *buffer = NULL;
  bool single_tlv = false;
  size_t valsize = sopt->sopt_valsize;
  if (valsize == 0 || valsize > ((sizeof(struct necp_tlv_header) +
                                  NECP_MAX_SOCKET_ATTRIBUTE_STRING_LENGTH) *
                                 4)) {
    goto done;
  }
  ...
  // If NECP_TLV_ATTRIBUTE_DOMAIN_CONTEXT is being set/cleared separately from
  // the other attributes, do not clear other attributes.
  error = necp_set_socket_attribute(
      buffer, valsize, NECP_TLV_ATTRIBUTE_DOMAIN_CONTEXT,
      &attributes->inp_domain_context, &single_tlv);
  if (error) {
  ...
  error = necp_set_socket_attribute(buffer, valsize, NECP_TLV_ATTRIBUTE_DOMAIN,
                                    &attributes->inp_domain, NULL);
  if (error) {
    NECPLOG0(LOG_ERR, "Could not set domain TLV for socket attributes");
    goto done;
  }

  error = necp_set_socket_attribute(buffer, valsize,
                                    NECP_TLV_ATTRIBUTE_DOMAIN_OWNER,
                                    &attributes->inp_domain_owner, NULL);
  if (error) {
  ...
  error = necp_set_socket_attribute(buffer, valsize,
                                    NECP_TLV_ATTRIBUTE_TRACKER_DOMAIN,
                                    &attributes->inp_tracker_domain, NULL);
  if (error) {
    NECPLOG0(LOG_ERR, "Could not set tracker domain TLV for socket attributes");
    goto done;
  }

  error = necp_set_socket_attribute(buffer, valsize, NECP_TLV_ATTRIBUTE_ACCOUNT,
                                    &attributes->inp_account, NULL);
  if (error) {
  ...
  NECP_SOCKET_ATTRIBUTE_LOG(
      "NECP ATTRIBUTES SOCKET - domain <%s> owner <%s> context <%s> tracker "
      "domain <%s> account <%s>",
      attributes->inp_domain, attributes->inp_domain_owner,
      attributes->inp_domain_context, attributes->inp_tracker_domain,
      attributes->inp_account);

  if (necp_debug) {
    NECPLOG(LOG_DEBUG,
            "Set on socket: Domain %s, Domain owner %s, Domain context %s, "
            "Tracker domain %s, Account %s",
            attributes->inp_domain, attributes->inp_domain_owner,
            attributes->inp_domain_context, attributes->inp_tracker_domain,
            attributes->inp_account);
  }
...
errno_t necp_get_socket_attributes(struct inp_necp_attributes *attributes,
                                   struct sockopt *sopt) {
  int error = 0;
  size_t valsize = 0;
  u_int8_t *buffer = NULL;
  ...
  if (attributes->inp_domain_owner != NULL) {
    valsize +=
        sizeof(struct necp_tlv_header) + strlen(attributes->inp_domain_owner);
  }
  if (attributes->inp_domain_context != NULL) {
    valsize +=
        sizeof(struct necp_tlv_header) + strlen(attributes->inp_domain_context);
  }
  if (attributes->inp_tracker_domain != NULL) {
    valsize +=
        sizeof(struct necp_tlv_header) + strlen(attributes->inp_tracker_domain);
  }
  ...
  if (attributes->inp_domain != NULL) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_TLV_ATTRIBUTE_DOMAIN, strlen(attributes->inp_domain),
        __terminated_by_to_indexable(attributes->inp_domain), buffer, valsize);
  }

  if (attributes->inp_domain_owner != NULL) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_TLV_ATTRIBUTE_DOMAIN_OWNER,
        strlen(attributes->inp_domain_owner),
        __terminated_by_to_indexable(attributes->inp_domain_owner), buffer,
        valsize);
  }

  if (attributes->inp_domain_context != NULL) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_TLV_ATTRIBUTE_DOMAIN_CONTEXT,
        strlen(attributes->inp_domain_context),
        __terminated_by_to_indexable(attributes->inp_domain_context), buffer,
        valsize);
  }

  if (attributes->inp_tracker_domain != NULL) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_TLV_ATTRIBUTE_TRACKER_DOMAIN,
        strlen(attributes->inp_tracker_domain),
        __terminated_by_to_indexable(attributes->inp_tracker_domain), buffer,
        valsize);
  }

  if (attributes->inp_account != NULL) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_TLV_ATTRIBUTE_ACCOUNT, strlen(attributes->inp_account),
        __terminated_by_to_indexable(attributes->inp_account), buffer, valsize);
  }
...
int necp_set_socket_resolver_signature(struct inpcb *inp,
                                       struct sockopt *sopt) {
  const size_t valsize = sopt->sopt_valsize;
  if (valsize > NECP_CLIENT_ACTION_SIGN_MAX_TOTAL_LENGTH +
                    NECP_CLIENT_ACTION_SIGN_TAG_LENGTH) {
    return EINVAL;
  }
  ...
  if (inp->inp_resolver_signature != NULL) {
    kfree_data_sized_by(inp->inp_resolver_signature,
...
int necp_get_socket_resolver_signature(struct inpcb *inp,
                                       struct sockopt *sopt) {
  int error = 0;
  necp_lock_socket_attributes();
  if (inp->inp_resolver_signature == NULL ||
...
bool necp_socket_has_resolver_signature(struct inpcb *inp) {
  necp_lock_socket_attributes();
  bool has_signature = (inp->inp_resolver_signature != NULL &&
                        inp->inp_resolver_signature_length != 0);
  necp_unlock_socket_attributes();
  return has_signature;
}

bool necp_socket_resolver_signature_matches_address(
    struct inpcb *inp, union necp_sockaddr_union *address) {
  bool matches_address = false;
  necp_lock_socket_attributes();
  if (inp->inp_resolver_signature != NULL &&
      inp->inp_resolver_signature_length > 0 && address->sa.sa_len > 0) {
    struct necp_client_validatable *validatable =
        (struct necp_client_validatable *)inp->inp_resolver_signature;
    if (inp->inp_resolver_signature_length >
            sizeof(struct necp_client_validatable) &&
        validatable->signable.sign_type ==
            NECP_CLIENT_SIGN_TYPE_SYSTEM_RESOLVER_ANSWER) {
      size_t data_length = inp->inp_resolver_signature_length -
                           sizeof(struct necp_client_validatable);
      if (data_length >= (sizeof(struct necp_client_host_resolver_answer) -
                          sizeof(struct necp_client_signable))) {
        struct necp_client_host_resolver_answer *__single answer_struct =
        ...
        if (data_length == (sizeof(struct necp_client_host_resolver_answer) +
            ...
            ((answer_struct->address_answer.sa.sa_family == AF_INET &&
             ...
             (answer_struct->address_answer.sa.sa_family == AF_INET6 &&
              memcmp(&sin6.sin6_addr, &address->sin6.sin6_addr,
                     sizeof(struct in6_addr)) == 0))) {
          // Address matches
          const bool validated = necp_validate_resolver_answer(
              validatable->signable.client_id, validatable->signable.sign_type,
              signable_get_data(&validatable->signable, data_length),
              data_length, validatable->signature.signed_tag,
              sizeof(validatable->signature.signed_tag));
          if (validated) {
            // Answer is validated
 ...
 * Called from soconnectlock/soconnectxlock to directly set the tracker domain
 * and owner for a newly marked tracker socket.
 */
errno_t
necp_set_socket_domain_attributes(struct socket *so,
                                  const char *domain __null_terminated,
                                  const char *domain_owner __null_terminated) {
  int error = 0;
  ...
  if (inp->inp_necp_attributes.inp_domain != NULL) {
    if (strlen(inp->inp_necp_attributes.inp_domain) != strlen(domain) ||
      ...
      buffer_to_free = (inp->inp_necp_attributes.inp_tracker_domain != NULL)
      ...
      necp_lock_socket_attributes();
      inp->inp_necp_attributes.inp_tracker_domain =
          __unsafe_null_terminated_from_indexable((char *)buffer,
                                                  (char *)&buffer[valsize]);
      necp_unlock_socket_attributes();
      if (buffer_to_free != NULL) {
        kfree_data_addr(buffer_to_free);
  ...
  buffer_to_free = (inp->inp_necp_attributes.inp_domain_owner != NULL)
                       ? __unsafe_null_terminated_to_indexable(
                             inp->inp_necp_attributes.inp_domain_owner)
                       : NULL;
  // Protect switching of buffer pointer
...
void *__sized_by(*message_length) necp_create_nexus_assign_message(
    uuid_t nexus_instance, nexus_port_t nexus_port,
    void *__sized_by(key_length) key, uint32_t key_length,
    struct necp_client_endpoint *local_endpoint,
    struct necp_client_endpoint *remote_endpoint,
    struct ether_addr *local_ether_addr, u_int32_t flow_adv_index,
    void *flow_stats, uint32_t flow_id, size_t *message_length) {
  u_int8_t *__indexable buffer = NULL;
  u_int8_t *__indexable cursor = NULL;
  size_t valsize = 0;
  ...
  if (local_endpoint != NULL) {
    valsize +=
        sizeof(struct necp_tlv_header) + sizeof(struct necp_client_endpoint);
  }
  ...
  if (has_nexus_assignment) {
    cursor =
        necp_buffer_write_tlv(cursor, NECP_CLIENT_RESULT_NEXUS_INSTANCE,
                              sizeof(uuid_t), nexus_instance, buffer, valsize);
    cursor = necp_buffer_write_tlv(cursor, NECP_CLIENT_RESULT_NEXUS_PORT,
                                   sizeof(nexus_port_t), &nexus_port, buffer,
                                   valsize);
  }
  if (flow_adv_index != NECP_FLOWADV_IDX_INVALID) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_CLIENT_RESULT_NEXUS_PORT_FLOW_INDEX, sizeof(u_int32_t),
        &flow_adv_index, buffer, valsize);
  }
  if (key != NULL && key_length > 0) {
    cursor = necp_buffer_write_tlv(cursor, NECP_CLIENT_PARAMETER_NEXUS_KEY,
                                   key_length, key, buffer, valsize);
  }
  if (local_endpoint != NULL) {
    cursor = necp_buffer_write_tlv(cursor, NECP_CLIENT_RESULT_LOCAL_ENDPOINT,
                                   sizeof(struct necp_client_endpoint),
                                   (uint8_t *)(struct necp_client_endpoint *
                                               __bidi_indexable) local_endpoint,
                                   buffer, valsize);
  }
  if (remote_endpoint != NULL) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_CLIENT_RESULT_REMOTE_ENDPOINT,
        sizeof(struct necp_client_endpoint),
        (uint8_t *)(struct necp_client_endpoint * __bidi_indexable)
            remote_endpoint,
        buffer, valsize);
  ...
  if (local_ether_addr != NULL) {
    cursor = necp_buffer_write_tlv(
        cursor, NECP_CLIENT_RESULT_LOCAL_ETHER_ADDR, sizeof(struct ether_addr),
        (uint8_t *)(struct ether_addr * __bidi_indexable) local_ether_addr,
        buffer, valsize);
  }
  if (flow_stats != NULL) {
    cursor =
        necp_buffer_write_tlv(cursor, NECP_CLIENT_RESULT_NEXUS_FLOW_STATS,
                              sizeof(void *), &flow_stats, buffer, valsize);
  }
  if (flow_id != 0) {
    cursor =
        necp_buffer_write_tlv(cursor, NECP_CLIENT_RESULT_UNIQUE_FLOW_TAG,
                              sizeof(u_int32_t), &flow_id, buffer, valsize);
  }
...
void necp_inpcb_remove_cb(struct inpcb *inp) {
...
void necp_mppcb_dispose(struct mppcb *mpp) {
  char *__indexable buffer = NULL;
...
void necp_client_init(void) {
  necp_client_update_tcall = thread_call_allocate_with_options(
      necp_update_all_clients_callout, NULL, THREAD_CALL_PRIORITY_KERNEL,
      THREAD_CALL_OPTIONS_ONCE);
  VERIFY(necp_client_update_tcall != NULL);
#if SKYWALK

  necp_client_collect_stats_tcall = thread_call_allocate_with_options(
      necp_collect_stats_client_callout, NULL, THREAD_CALL_PRIORITY_KERNEL,
      THREAD_CALL_OPTIONS_ONCE);
  VERIFY(necp_client_collect_stats_tcall != NULL);

  necp_close_empty_arenas_tcall = thread_call_allocate_with_options(
      necp_close_empty_arenas_callout, NULL, THREAD_CALL_PRIORITY_KERNEL,
      THREAD_CALL_OPTIONS_ONCE);
...
pid_t necp_client_get_proc_pid_from_arena_info(
    struct skmem_arena_mmap_info *arena_info) {
  ASSERT((arena_info->ami_arena->ar_type == SKMEM_ARENA_TYPE_NECP) ||
         (arena_info->ami_arena->ar_type == SKMEM_ARENA_TYPE_SYSTEM));

  if (arena_info->ami_arena->ar_type == SKMEM_ARENA_TYPE_NECP) {
    struct necp_arena_info *__single nai = __unsafe_forge_single(
        struct necp_arena_info *,
        container_of(arena_info, struct necp_arena_info, nai_mmap));
    return nai->nai_proc_pid;
  } else {
    struct necp_fd_data *__single fd_data = __unsafe_forge_single(
        struct necp_fd_data *,
        container_of(arena_info, struct necp_fd_data, sysctl_mmap));
