 ...
 *
 *   Theory of operation:
 *   utun (user tunnel) acts as glue between kernel control sockets and network
 * interfaces. This kernel control will register an interface for every client
 * that connects.
 *   ----------------------------------------------------------------------------------
 */
...
static errno_t utun_ctl_getopt(kern_ctl_ref kctlref, u_int32_t unit,
                               void *unitinfo, int opt,
                               void *__sized_by(*len) data, size_t *len);
static errno_t utun_ctl_setopt(kern_ctl_ref kctlref, u_int32_t unit,
                               void *unitinfo, int opt,
                               void *__sized_by(len) data, size_t len);
...
static errno_t utun_proto_pre_output(ifnet_t interface,
...
SYSCTL_INT(_net_utun, OID_AUTO, max_pending_input, CTLFLAG_LOCKED | CTLFLAG_RW,
           &if_utun_max_pending_input, 0, "");
SYSCTL_PROC(_net_utun, OID_AUTO, ring_size,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW, &if_utun_ring_size,
            UTUN_IF_DEFAULT_RING_SIZE, &sysctl_if_utun_ring_size, "I", "");
SYSCTL_PROC(_net_utun, OID_AUTO, tx_fsw_ring_size,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW,
            &if_utun_tx_fsw_ring_size, UTUN_IF_DEFAULT_TX_FSW_RING_SIZE,
            &sysctl_if_utun_tx_fsw_ring_size, "I", "");
SYSCTL_PROC(_net_utun, OID_AUTO, rx_fsw_ring_size,
            CTLTYPE_INT | CTLFLAG_LOCKED | CTLFLAG_RW,
            &if_utun_rx_fsw_ring_size, UTUN_IF_DEFAULT_RX_FSW_RING_SIZE,
            &sysctl_if_utun_rx_fsw_ring_size, "I", "");

static errno_t utun_register_nexus(void);

static errno_t utun_netif_prepare(__unused kern_nexus_t nexus, ifnet_t ifp);
static errno_t utun_nexus_pre_connect(kern_nexus_provider_t nxprov, proc_t p,
                                      kern_nexus_t nexus,
                                      nexus_port_t nexus_port,
                                      kern_channel_t channel, void **ch_ctx);
static errno_t utun_nexus_connected(kern_nexus_provider_t nxprov,
                                    kern_nexus_t nexus, kern_channel_t channel);
static void utun_netif_pre_disconnect(kern_nexus_provider_t nxprov,
                                      kern_nexus_t nexus,
                                      kern_channel_t channel);
static void utun_nexus_pre_disconnect(kern_nexus_provider_t nxprov,
                                      kern_nexus_t nexus,
                                      kern_channel_t channel);
static void utun_nexus_disconnected(kern_nexus_provider_t nxprov,
                                    kern_nexus_t nexus, kern_channel_t channel);
static errno_t utun_kpipe_ring_init(kern_nexus_provider_t nxprov,
                                    kern_nexus_t nexus, kern_channel_t channel,
                                    kern_channel_ring_t ring,
                                    boolean_t is_tx_ring, void **ring_ctx);
static void utun_kpipe_ring_fini(kern_nexus_provider_t nxprov,
                                 kern_nexus_t nexus, kern_channel_ring_t ring);
static errno_t utun_kpipe_sync_tx(kern_nexus_provider_t nxprov,
                                  kern_nexus_t nexus, kern_channel_ring_t ring,
                                  uint32_t flags);
static errno_t utun_kpipe_sync_rx(kern_nexus_provider_t nxprov,
                                  kern_nexus_t nexus, kern_channel_ring_t ring,
                                  uint32_t flags);
#endif // UTUN_NEXUS
...
struct utun_pcb {
  ...
  u_int32_t
      utun_pcb_data_path_state; /* internal state of interface data path */
...
#define UTUN_HEADER_SIZE(_pcb)                                                 \
  (sizeof(u_int32_t) +                                                         \
   (((_pcb)->utun_external_flags & UTUN_FLAGS_ENABLE_PROC_UUID)                \
        ? sizeof(uuid_t)                                                       \
        : 0))
...
static inline void utun_flag_set(struct utun_pcb *pcb, uint32_t flag) {
  pcb->utun_internal_flags |= flag;
}

static inline void utun_flag_clr(struct utun_pcb *pcb, uint32_t flag) {
  pcb->utun_internal_flags &= ~flag;
}

static inline bool utun_flag_isset(struct utun_pcb *pcb, uint32_t flag) {
  return !!(pcb->utun_internal_flags & flag);
}

static inline bool utun_in_wmm_mode(struct utun_pcb *pcb) {
  return pcb->utun_kpipe_count == UTUN_IF_WMM_RING_COUNT;
}

static uint8_t utun_find_tx_ring_by_svc(kern_packet_svc_class_t svc_class) {
  switch (svc_class) {
  case KPKT_SC_VO: {
    return 0;
  }
...
#pragma unused(arg1, arg2)
  ...
  if (value < UTUN_IF_MIN_RING_SIZE || value > UTUN_IF_MAX_RING_SIZE) {
    return EINVAL;
  }
...
static errno_t utun_netif_ring_init(kern_nexus_provider_t nxprov,
                                    kern_nexus_t nexus, kern_channel_t channel,
                                    kern_channel_ring_t ring,
                                    boolean_t is_tx_ring, void **ring_ctx) {
...
static void utun_netif_ring_fini(kern_nexus_provider_t nxprov,
                                 kern_nexus_t nexus, kern_channel_ring_t ring) {
#pragma unused(nxprov)
  struct utun_pcb *__single pcb = kern_nexus_get_context(nexus);
  bool found = false;
...
static errno_t utun_netif_sync_tx(kern_nexus_provider_t nxprov,
                                  kern_nexus_t nexus,
...
#pragma unused(flags)
  ...
  while (tx_slot != NULL) {
    ...
    uint8_t *tx_baddr = __unsafe_forge_bidi_indexable(
        uint8_t *, kern_buflet_get_data_address(tx_buf),
    ...
    default: {
      os_log_error(OS_LOG_DEFAULT,
                   "utun_netif_sync_tx %s: unknown ip version %u vhl %u "
                   "tx_offset %u len %u header_size %zu\n",
                   pcb->utun_ifp->if_xname, ip_version, vhl, tx_offset,
  ...
  if (tx_pslot) {
    ...
    kern_channel_increment_ring_net_stats(tx_ring, pcb->utun_ifp,
                                          &tx_ring_stats);
...
static errno_t utun_netif_tx_doorbell_one(kern_nexus_provider_t nxprov,
                                          kern_nexus_t nexus,
                                          kern_channel_ring_t ring,
...
#pragma unused(nxprov)
  ...
  if (rc != 0 && rc != EAGAIN && rc != EBUSY) {
    os_log_error(OS_LOG_DEFAULT, "%s: %s ring %s tx refill failed %d\n",
                 __func__, if_name(pcb->utun_ifp), ring->ckr_name, rc);
  }
  ...
  if (ring != pcb->utun_netif_txring[ring_idx]) {
    ...
    os_log_error(OS_LOG_DEFAULT, "%s: %s ring %s index %d bad netif_txring 3\n",
                 __func__, if_name(pcb->utun_ifp), ring->ckr_name, ring_idx);
    return ENXIO;
  }
...
static errno_t utun_netif_tx_doorbell(kern_nexus_provider_t nxprov,
                                      kern_nexus_t nexus,
                                      kern_channel_ring_t ring,
...
static errno_t utun_netif_sync_rx(kern_nexus_provider_t nxprov,
                                  kern_nexus_t nexus,
...
#pragma unused(flags)
  ...
  while (rx_slot != NULL) {
    ...
    if (length < header_offset) {
      ...
      STATS_INC(nifs, NETIF_STATS_DROP);
      os_log_error(OS_LOG_DEFAULT,
                   "utun_netif_sync_rx %s: legacy packet length too short for "
                   "header %zu < %zu\n",
    ...
    VERIFY(rx_buf != NULL);
    void *rx_baddr = __unsafe_forge_bidi_indexable(
        void *, kern_buflet_get_data_address(rx_buf),
  ...
  for (uint8_t ring_idx = 0; ring_idx < pcb->utun_kpipe_count; ring_idx++) {
    ...
    while (rx_slot != NULL && tx_slot != NULL) {
      ...
      if (tx_length < header_offset) {
        ...
        STATS_INC(nifs, NETIF_STATS_DROP);
        os_log_error(OS_LOG_DEFAULT,
                     "utun_netif_sync_rx %s: packet length too short for "
                     "header %u < %zu\n",
                     pcb->utun_ifp->if_xname, tx_length, header_offset);
        continue;
      }

      size_t length = MIN(tx_length - header_offset, pcb->utun_slot_size);
  ...
  if (rx_pslot) {
    ...
    kern_channel_increment_ring_net_stats(rx_ring, pcb->utun_ifp,
                                          &rx_ring_stats);
  }
...
static errno_t utun_nexus_ifattach(struct utun_pcb *pcb,
  ...
  pp_init.kbi_packets =
      pcb->utun_netif_ring_size * (2 * pcb->utun_kpipe_count + 1);
  ...
  err = kern_nexus_controller_register_provider(
      controller, utun_nx_dom_prov, provider_name, &prov_init,
      sizeof(prov_init), nxa, &pcb->utun_nx.if_provider);
  if (err != 0) {
  ...
  err = kern_nexus_controller_alloc_net_provider_instance(
      controller, pcb->utun_nx.if_provider, pcb, NULL,
      &pcb->utun_nx.if_instance, &net_init, ifp);
  if (err != 0) {
...
static void utun_detach_provider_and_instance(uuid_t provider,
                                              uuid_t instance) {
  nexus_controller_t controller = kern_nexus_shared_controller();
  errno_t err;
...
static void utun_nexus_detach(struct utun_pcb *pcb) {
  ...
  utun_detach_provider_and_instance(nx->fsw_provider, nx->fsw_instance);
  utun_detach_provider_and_instance(nx->if_provider, nx->if_instance);
...
static errno_t utun_create_fs_provider_and_instance(struct utun_pcb *pcb,
...
static errno_t utun_flowswitch_attach(struct utun_pcb *pcb) {
  nexus_controller_t controller = kern_nexus_shared_controller();
  errno_t err = 0;
  utun_nx_t nx = &pcb->utun_nx;

  // Allocate flowswitch
  err = utun_create_fs_provider_and_instance(
      pcb, "flowswitch", pcb->utun_ifp->if_xname, &nx->fsw_provider,
      &nx->fsw_instance);
  if (err != 0) {
    os_log_error(OS_LOG_DEFAULT,
...
static errno_t utun_register_kernel_pipe_nexus(struct utun_pcb *pcb) {
  nexus_attr_t __single nxa = NULL;
  errno_t result;
  ...
  uint64_t ring_size = pcb->utun_kpipe_tx_ring_size != 0
                           ? pcb->utun_kpipe_tx_ring_size
                           : if_utun_ring_size;
  result = kern_nexus_attr_set(nxa, NEXUS_ATTR_TX_SLOTS, ring_size);
  VERIFY(result == 0);

  ring_size = pcb->utun_kpipe_rx_ring_size != 0 ? pcb->utun_kpipe_rx_ring_size
                                                : if_utun_ring_size;
  result = kern_nexus_attr_set(nxa, NEXUS_ATTR_RX_SLOTS, ring_size);
  VERIFY(result == 0);

  nexus_domain_provider_name_t domain_provider_name =
      "com.apple.nexus.utun.kpipe";

  result = kern_nexus_controller_register_provider(
      utun_ncd, dom_prov, domain_provider_name, &prov_init, sizeof(prov_init),
      nxa, &utun_kpipe_uuid);
  if (result) {
    os_log_error(OS_LOG_DEFAULT,
...
static void utun_unregister_kernel_pipe_nexus(void) {
  lck_mtx_lock(&utun_lock);
...
static void utun_detach_channels(struct utun_pcb *pcb,
...
static void utun_free_channels(struct utun_detached_channels *dc) {
  if (!dc->count) {
    return;
  }
  ...
  for (int i = 0; i < dc->count; i++) {
    errno_t result;
    result =
        kern_nexus_controller_free_provider_instance(utun_ncd, dc->uuids[i]);
    VERIFY(!result);
  }
...
static errno_t utun_enable_channel(struct utun_pcb *pcb, struct proc *proc) {
  struct kern_nexus_init init;
  struct kern_pbufpool_init pp_init;
  errno_t result;
  ...
  result = kern_pbufpool_create(&pp_init, &pcb->utun_kpipe_pp, NULL);
  if (result != 0) {
    os_log_error(OS_LOG_DEFAULT, "%s pbufbool create failed, error %d\n",
                 __func__, result);
    goto done;
  ...
  for (unsigned int i = 0; i < pcb->utun_kpipe_count; i++) {
    ...
    result = kern_nexus_controller_alloc_provider_instance(
        utun_ncd, utun_kpipe_uuid, pcb, NULL, &pcb->utun_kpipe_uuid[i], &init);
    ...
    if (result == 0) {
      ...
      result = kern_nexus_controller_bind_provider_instance(
          utun_ncd, pcb->utun_kpipe_uuid[i], &port, pid,
          has_proc_uuid ? pcb->utun_kpipe_proc_uuid : uuid_null, NULL, 0,
...
errno_t utun_register_control(void) {
  ...
  kern_ctl.ctl_flags = CTL_FLAG_PRIVILEGED | CTL_FLAG_REG_SETUP |
                       CTL_FLAG_REG_EXTENDED; /* Require root */
  ...
  if ((result = proto_register_plumber(PF_INET, IFNET_FAMILY_UTUN,
    ...
    os_log_error(OS_LOG_DEFAULT,
                 "utun_register_control - proto_register_plumber(PF_INET, "
                 "IFNET_FAMILY_UTUN) failed: %d\n",
                 result);
  ...
  if ((result = proto_register_plumber(PF_INET6, IFNET_FAMILY_UTUN,
    ...
    ctl_deregister(utun_kctlref);
    os_log_error(OS_LOG_DEFAULT,
                 "utun_register_control - proto_register_plumber(PF_INET6, "
                 "IFNET_FAMILY_UTUN) failed: %d\n",
                 result);
    return result;
  }
...
static inline int utun_find_by_unit(u_int32_t unit) {
...
static inline void utun_free_pcb(struct utun_pcb *pcb, bool locked) {
#if UTUN_NEXUS
...
static errno_t utun_ctl_setup(u_int32_t *unit, void **unitinfo) {
  ...
  struct utun_pcb *__single pcb =
      zalloc_flags(utun_pcb_zone, Z_WAITOK | Z_ZERO);
...
static errno_t utun_ctl_bind(kern_ctl_ref kctlref, struct sockaddr_ctl *sac,
  ...
  lck_mtx_init(&pcb->utun_pcb_data_move_lock, &utun_lck_grp, &utun_lck_attr);
#endif // UTUN_NEXUS
...
static errno_t utun_ctl_connect(kern_ctl_ref kctlref, struct sockaddr_ctl *sac,
  ...
  snprintf(pcb->utun_if_xname, sizeof(pcb->utun_if_xname), "utun%d",
           pcb->utun_unit - 1);
  snprintf(pcb->utun_unique_name, sizeof(pcb->utun_unique_name), "utunid%d",
           pcb->utun_unique_id - 1);
    ...
    if (pcb->utun_attach_fsw) {
      ...
      if (result != 0) {
        os_log_error(OS_LOG_DEFAULT,
                     ...
                     result);
        // Do not call utun_free_pcb(). We will be attached already, and will be
        // freed later in utun_detached().
        *unitinfo = NULL;
        return result;
      }
    }
...
#endif // UTUN_NEXUS
  {
    ...
    ifnet_set_flags(pcb->utun_ifp, IFF_UP | IFF_MULTICAST | IFF_POINTOPOINT,
                    0xffff);

    /* The interface must generate its own IPv6 LinkLocal address,
     * if possible following the recommendation of RFC2472 to the 64bit
     * interface ID
     */
...
static errno_t utun_detach_ip(ifnet_t interface, protocol_family_t protocol,
                              socket_t pf_socket) {
...
static void utun_remove_address(ifnet_t interface, protocol_family_t protocol,
  ...
  if (protocol == PF_INET) {
    ...
    snprintf(ifr6.ifr_name, sizeof(ifr6.ifr_name), "%s%d",
             ifnet_name(interface), ifnet_unit(interface));
    result = ifaddr_address(address, SA(&ifr6.ifr_addr), sizeof(ifr6.ifr_addr));
    if (result != 0) {
      os_log_error(OS_LOG_DEFAULT,
...
static void utun_cleanup_family(ifnet_t interface, protocol_family_t protocol) {
  errno_t result = 0;
...
static errno_t utun_ctl_disconnect(__unused kern_ctl_ref kctlref,
...
static errno_t utun_ctl_send(__unused kern_ctl_ref kctlref,
                             __unused u_int32_t unit, void *unitinfo, mbuf_t m,
                             __unused int flags) {
  /*
...
static errno_t utun_ctl_setopt(__unused kern_ctl_ref kctlref,
                               __unused u_int32_t unit, void *unitinfo, int opt,
                               void *__sized_by(len) data, size_t len) {
  struct utun_pcb *__single pcb = unitinfo;
  errno_t result = 0;
  ...
  case UTUN_OPT_ENABLE_CHANNEL: {
    ...
    if (*intp != 0 && *intp != 1 && *intp != UTUN_IF_WMM_RING_COUNT) {
      result = EINVAL;
      break;
    }
    lck_rw_lock_exclusive(&pcb->utun_pcb_lock);
...
static errno_t utun_ctl_getopt(__unused kern_ctl_ref kctlref,
                               __unused u_int32_t unit, void *unitinfo, int opt,
                               void *__sized_by(*len) data, size_t *len) {
  struct utun_pcb *__single pcb = unitinfo;
  errno_t result = 0;
  ...
  case UTUN_OPT_IFNAME:
    if (*len <
        MIN(strbuflen(pcb->utun_if_xname) + 1, sizeof(pcb->utun_if_xname))) {
      result = EMSGSIZE;
    } else {
      if (pcb->utun_ifp == NULL) {
...
static void utun_start(ifnet_t interface) {
  ...
  for (;;) {
    ...
    u_int32_t utun_packet_cnt;
    errno_t error_pc = ctl_getenqueuepacketcount(
        pcb->utun_ctlref, pcb->utun_unit, &utun_packet_cnt);
    if (error_pc != 0) {
      os_log_error(OS_LOG_DEFAULT,
    ...
    if (!can_accept_packets && pcb->utun_ctlref) {
      ...
      if (ctl_getenqueuereadable(pcb->utun_ctlref, pcb->utun_unit,
                                 &difference) == 0) {
        if (difference > 0) {
          // If the low-water mark has not yet been reached, we still need to
...
static errno_t utun_output(ifnet_t interface, mbuf_t data) {
  ...
  if (pcb->utun_ctlref) {
    ...
    result =
        ctl_enqueuembuf(pcb->utun_ctlref, pcb->utun_unit, data, CTL_DATA_EOR);
    if (result != 0) {
      mbuf_freem(data);
...
static errno_t utun_demux(__unused ifnet_t interface, mbuf_t data,
...
static errno_t utun_framer(ifnet_t interface, mbuf_t *packet,
  ...
  *mtod(*packet, protocol_family_t *) =
      *(protocol_family_t *)(uintptr_t)(size_t)frame_type;
  ...
  if (pcb->utun_external_flags & UTUN_FLAGS_ENABLE_PROC_UUID) {
    if (m_pktlen(*packet) >= (int32_t)UTUN_HEADER_SIZE(pcb)) {
      ...
      int uuid_err = necp_get_app_uuid_from_packet(
          *packet, (void *)(header + sizeof(u_int32_t)));
      if (uuid_err != 0) {
        os_log_error(OS_LOG_DEFAULT, "Received app uuid error %d for %s%d\n",
                     uuid_err, ifnet_name(pcb->utun_ifp),
...
static errno_t
utun_add_proto(__unused ifnet_t interface, protocol_family_t protocol,
...
static errno_t utun_del_proto(__unused ifnet_t interface,
                              __unused protocol_family_t protocol) {
  return 0;
}

static errno_t utun_ioctl(ifnet_t interface, u_long command, void *data) {
#if UTUN_NEXUS
  struct utun_pcb *__single pcb = ifnet_softc(interface);
#endif
  errno_t result = 0;
  ...
  case SIOCSIFMTU: {
    ...
    if (pcb->utun_use_netif) {
      ...
      if (((uint64_t)((struct ifreq *)data)->ifr_mtu) + UTUN_IF_HEADROOM_SIZE >
          pcb->utun_slot_size) {
        result = EINVAL;
      } else {
...
static errno_t utun_proto_input(__unused ifnet_t interface,
...
static errno_t utun_proto_pre_output(__unused ifnet_t interface,
                                     protocol_family_t protocol,
                                     __unused mbuf_t *packet,
                                     __unused const struct sockaddr *dest,
                                     __unused void *route, char *frame_type,
                                     __unused char *link_layer_dest) {
  *(protocol_family_t *)(void *)frame_type = protocol;
  return 0;
}

static errno_t utun_attach_proto(ifnet_t interface,
                                 protocol_family_t protocol) {
...
static errno_t utun_pkt_input(struct utun_pcb *pcb, mbuf_t packet) {
#if UTUN_NEXUS
  if (pcb->utun_use_netif) {
    if (!utun_data_move_begin(pcb)) {
      os_log_info(OS_LOG_DEFAULT, "%s: data path stopped for %s\n", __func__,
...
static errno_t utun_nxdp_init(__unused kern_nexus_domain_provider_t domprov) {
  return 0;
}

static void utun_nxdp_fini(__unused kern_nexus_domain_provider_t domprov) {
  // Ignore
}
...
static errno_t utun_ifnet_set_attrs(ifnet_t ifp) {
  /* Set flags and additional information. */
...
static errno_t utun_netif_prepare(kern_nexus_t nexus, ifnet_t ifp) {
  struct utun_pcb *__single pcb = kern_nexus_get_context(nexus);
...
#pragma unused(flags)
  ...
  if (pcb->utun_use_netif) {
    kern_channel_slot_t tx_slot =
        kern_channel_get_next_slot(tx_ring, NULL, NULL);
    if (tx_slot == NULL) {
    ...
    while (tx_slot != NULL) {
      ...
      size_t length =
          MIN(kern_packet_get_data_length(tx_ph), pcb->utun_slot_size);
...
#pragma unused(flags)
  ...
  if (tx_ring == NULL || pcb->utun_netif_nexus == NULL) {
    // Net-If TX ring not set up yet, nothing to read
    lck_rw_unlock_shared(&pcb->utun_pcb_lock);
    utun_data_move_end(pcb);
    return 0;
  ...
  struct netif_stats *nifs =
      &NX_NETIF_PRIVATE(pcb->utun_netif_nexus)->nif_stats;
  ...
  while (rx_slot != NULL && tx_slot != NULL) {
    ...
    if (length < UTUN_HEADER_SIZE(pcb) || length > pcb->utun_slot_size ||
    ...
    VERIFY(rx_buf != NULL);
    rx_baddr = __unsafe_forge_bidi_indexable(
        uint8_t *, kern_buflet_get_data_address(rx_buf),
        kern_buflet_get_data_limit(rx_buf));
    VERIFY(rx_baddr != NULL);
    ...
    default: {
      os_log_error(OS_LOG_DEFAULT,
                   "utun_kpipe_sync_rx %s: unknown ip version %u vhl %u "
                   "header_size %zu\n",
                   pcb->utun_ifp->if_xname, ip_version, vhl,
    ...
    memcpy(rx_baddr + UTUN_HEADER_SIZE(pcb), tx_baddr,
           length - UTUN_HEADER_SIZE(pcb));
    kern_packet_clear_flow_uuid(rx_ph); // zero flow id
...
int utun_pkt_dtls_input(struct utun_pcb *pcb, mbuf_t *pkt,
...
static void utun_data_move_drain(struct utun_pcb *pcb) {
  lck_mtx_lock(&pcb->utun_pcb_data_move_lock);
  ...
  while (pcb->utun_pcb_data_move != 0) {
    (void)msleep(&(pcb->utun_pcb_data_move), &pcb->utun_pcb_data_move_lock,
