...
static LIST_HEAD(iptap_list,
                 iptap_softc) iptap_list = LIST_HEAD_INITIALIZER(iptap_list);
...
static struct if_clone iptap_cloner = IF_CLONE_INITIALIZER(
    IPTAP_IFNAME, iptap_clone_create, iptap_clone_destroy, 0, IPTAP_MAXUNIT);
...
SYSCTL_INT(_net_link_iptap, OID_AUTO, total_tap_count,
           CTLFLAG_RD | CTLFLAG_LOCKED, &iptap_total_tap_count, 0, "");
...
__private_extern__ void iptap_init(void) {
  errno_t error;
...
static void iptap_lock_shared(void) { lck_rw_lock_shared(&iptap_lck_rw); }

static void iptap_lock_exclusive(void) { lck_rw_lock_exclusive(&iptap_lck_rw); }

static void iptap_lock_done(void) { lck_rw_done(&iptap_lck_rw); }

__private_extern__ int iptap_clone_create(struct if_clone *ifc, u_int32_t unit,
                                          void *params) {
#pragma unused(params)
  ...
  bpf_attach(iptap->iptap_ifp, DLT_PKTAP, sizeof(struct pktap_header), NULL,
             iptap_tap_callback);
  bpf_attach(iptap->iptap_ifp, DLT_RAW, 0, NULL, iptap_tap_callback);
...
__private_extern__ int iptap_clone_destroy(struct ifnet *ifp) {
  int error = 0;
 ...
 * - When interface is attached to a BPF device via BIOCSETIF for the default
 * DLT
 * - Whenever a new DLT is selected via BIOCSDLT
 * - When the interface is detached from a BPF device (direction is zero)
 */
__private_extern__ errno_t iptap_tap_callback(ifnet_t ifp, u_int32_t dlt,
...
__private_extern__ errno_t iptap_if_output(ifnet_t ifp, mbuf_t m) {
#pragma unused(ifp)
...
__private_extern__ errno_t iptap_demux(ifnet_t ifp, mbuf_t m, char *header,
...
__private_extern__ errno_t iptap_add_proto(ifnet_t ifp, protocol_family_t pf,
                                           const struct ifnet_demux_desc *dmx,
                                           u_int32_t cnt) {
#pragma unused(ifp)
#pragma unused(pf)
#pragma unused(dmx)
...
__private_extern__ errno_t iptap_del_proto(ifnet_t ifp, protocol_family_t pf) {
#pragma unused(ifp)
#pragma unused(pf)
...
__private_extern__ errno_t iptap_getdrvspec(ifnet_t ifp, struct ifdrv64 *ifd) {
  errno_t error = 0;
  struct iptap_softc *__single iptap;
  ...
  case PKTP_CMD_TAP_COUNT: {
    uint32_t tap_count =
        iptap->iptap_dlt_raw_count + iptap->iptap_dlt_pkttap_count;
...
__private_extern__ errno_t iptap_ioctl(ifnet_t ifp, unsigned long cmd,
                                       void *__sized_by(IOCPARM_LEN(cmd))
                                           data) {
  errno_t error = 0;
...
__private_extern__ void iptap_detach(ifnet_t ifp) {
  struct iptap_softc *__single iptap = NULL;
...
static errno_t iptap_ipf_input(void *arg, mbuf_t *mp, int off, u_int8_t proto) {
#pragma unused(off)
#pragma unused(proto)
  ...
  if (arg == (void *)&iptap_ipf4) {
    ...
    IPTAP_LOG("%s:%d bad cookie 0x%llx &iptap_ipf4 0x%llx "
              "&iptap_ipf6 0x%llx\n",
              __func__, __LINE__, (uint64_t)VM_KERNEL_ADDRPERM(arg),
...
static errno_t iptap_ipf_output(void *arg, mbuf_t *mp, ipf_pktopts_t opt) {
#pragma unused(opt)
...
static void iptap_ipf_detach(void *arg) {
#pragma unused(arg)
}

__private_extern__ void iptap_bpf_tap(struct mbuf *m, u_int32_t proto,
                                      int outgoing) {
  struct iptap_softc *__single iptap;
  ...
  LIST_FOREACH(iptap, &iptap_list, iptap_link) {
    if (iptap->iptap_dlt_raw_count > 0) {
      bpf_tap_func(iptap->iptap_ifp, DLT_RAW, m, NULL, 0);
    }
    if (iptap->iptap_dlt_pkttap_count > 0) {
      struct {
      ...
      if (ifp != NULL) {
        snprintf(hdr->pth_ifname, sizeof(hdr->pth_ifname), "%s", ifp->if_xname);
      }
      ...
      bpf_tap_func(iptap->iptap_ifp, DLT_PKTAP, m, &hdr_buffer, hdr_size);
    }
  }
