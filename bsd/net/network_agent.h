...
#define NETAGENT_MESSAGE_TYPE_REGISTER                                         \
  1 // Pass netagent to set, no return value
#define NETAGENT_MESSAGE_TYPE_UNREGISTER 2 // No value, no return value
#define NETAGENT_MESSAGE_TYPE_UPDATE                                           \
  3                                 // Pass netagent to update, no return value
...
#define NETAGENT_MESSAGE_TYPE_TRIGGER_ASSERT                                   \
  8 // Kernel initiated, no reply expected
#define NETAGENT_MESSAGE_TYPE_TRIGGER_UNASSERT                                 \
  9 // Kernel initiated, no reply expected
#define NETAGENT_MESSAGE_TYPE_REQUEST_NEXUS                                    \
  10 // Kernel initiated, struct netagent_client_message
#define NETAGENT_MESSAGE_TYPE_ASSIGN_NEXUS                                     \
  11 // Pass struct netagent_assign_nexus_message
#define NETAGENT_MESSAGE_TYPE_CLOSE_NEXUS                                      \
  12 // Kernel initiated, struct netagent_client_message
#define NETAGENT_MESSAGE_TYPE_CLIENT_TRIGGER                                   \
  13 // Kernel initiated, struct netagent_client_message
#define NETAGENT_MESSAGE_TYPE_CLIENT_ASSERT                                    \
  14 // Kernel initiated, struct netagent_client_message
#define NETAGENT_MESSAGE_TYPE_CLIENT_UNASSERT                                  \
  15 // Kernel initiated, struct netagent_client_message

#define NETAGENT_OPTION_TYPE_REGISTER                                          \
  NETAGENT_MESSAGE_TYPE_REGISTER // Pass netagent to set, no return value
#define NETAGENT_OPTION_TYPE_UNREGISTER                                        \
  NETAGENT_MESSAGE_TYPE_UNREGISTER // Pass agent uuid in session mode, no return
                                   // value
#define NETAGENT_OPTION_TYPE_UPDATE                                            \
  NETAGENT_MESSAGE_TYPE_UPDATE // Pass netagent to update, no return value
#define NETAGENT_OPTION_TYPE_ASSIGN_NEXUS                                      \
  NETAGENT_MESSAGE_TYPE_ASSIGN_NEXUS // Pass struct
                                     // netagent_assign_nexus_message
#define NETAGENT_OPTION_TYPE_USE_COUNT                                         \
  16 // Pass use count to set, get current use count
#define NETAGENT_MESSAGE_TYPE_ABORT_NEXUS 17 // Kernel private
#define NETAGENT_MESSAGE_TYPE_ADD_GROUP_MEMBERS                                \
  18 // Kernel initiated, struct netagent_client_group_message
#define NETAGENT_MESSAGE_TYPE_REMOVE_GROUP_MEMBERS                             \
  19 // Kernel initiated, struct netagent_client_group_message
#define NETAGENT_MESSAGE_TYPE_ASSIGN_GROUP_MEMBERS                             \
  20 // Pass struct netagent_assign_nexus_message
...
#define NETAGENT_OPTION_TYPE_TOKEN_COUNT                                       \
  23 // Get remaining token count (uint32_t)
#define NETAGENT_OPTION_TYPE_TOKEN_LOW_WATER                                   \
  24 // Set/get token low water mark (uint32_t)
...
#define NETAGENT_OPTION_TYPE_RESET_CLIENT_ERROR                                \
  27 // Call to reset client error and counts
#define NETAGENT_OPTION_TYPE_ENABLE_SESSION_MODE                               \
  28 // Enables registering multiple agents on a single fd "session". Must be
     // set before registering an agent.
#define NETAGENT_OPTION_TYPE_UNREGISTER_ALL                                    \
  29 // Applicable for session mode, unregisters all associated agents.
...
#define NETAGENT_FLAG_KERNEL_ACTIVATED                                         \
  0x0004 // Agent can be activated by kernel activity
#define NETAGENT_FLAG_USER_ACTIVATED                                           \
  0x0008 // Agent can be activated by system call (netagent_trigger)
#define NETAGENT_FLAG_VOLUNTARY 0x0010 // Use of agent is optional
#define NETAGENT_FLAG_SPECIFIC_USE_ONLY                                        \
  0x0020 // Agent should only be used and activated when specifically required
...
#define NETAGENT_FLAG_SUPPORTS_BROWSE                                          \
  0x0100 // Assertions will cause agent to fill in browse endpoints
#define NETAGENT_FLAG_REQUIRES_ASSERT                                          \
  0x0200 // Assertions are expected to be taken against this agent
#define NETAGENT_FLAG_NEXUS_LISTENER 0x0400 // Nexus supports listeners
#define NETAGENT_FLAG_UPDATE_IMMEDIATELY                                       \
  0x0800 // Updates the clients without waiting for a leeway
#define NETAGENT_FLAG_CUSTOM_ETHER_NEXUS                                       \
  0x2000 // Agent provides a custom ethertype nexus
#define NETAGENT_FLAG_CUSTOM_IP_NEXUS 0x4000 // Agent provides a custom IP nexus
#define NETAGENT_FLAG_INTERPOSE_NEXUS                                          \
  0x8000 // Agent provides an interpose nexus
#define NETAGENT_FLAG_SUPPORTS_RESOLVE                                         \
  0x10000 // Assertions will cause agent to fill in resolved endpoints
...
#define NETAGENT_NEXUS_FLAG_ASSERT_UNSUPPORTED                                 \
  0x2 // No calls to assert the agent are required
#define NETAGENT_NEXUS_FLAG_SHOULD_USE_EVENT_RING                              \
  0x4 // indicates that nexus agent should use event rings
#define NETAGENT_NEXUS_FLAG_COMPLETE_RESOLVE_ON_CONNECT                        \
  0x8 // Indicates resolver should mark itself as complete once it has a
      // connected child
#define NETAGENT_NEXUS_FLAG_CANCEL_REMOVED_ENDPOINTS                           \
  0x10 // Indicates resolver should cancel connection attempts to endpoints
       // removed from resolved endpoints list
...
struct netagent_nexus {
  ...
  u_int32_t
      endpoint_resolution_type_pairs[NETAGENT_NEXUS_MAX_RESOLUTION_TYPE_PAIRS *
                                     2];
...
#define NETAGENT_NEXUS_HAS_MAX_BUF_SIZE                                        \
  1 // struct netagent_nexus includes nexus_max_buf_size
...
extern void netagent_post_updated_interfaces(
    uuid_t uuid); // To be called from interface ioctls
...
extern bool
netagent_get_agent_domain_and_type(uuid_t uuid,
                                   char *domain __sized_by(NETAGENT_DOMAINSIZE),
                                   char *type __sized_by(NETAGENT_TYPESIZE));

extern int netagent_kernel_trigger(uuid_t uuid);

extern int netagent_client_message(uuid_t agent_uuid, uuid_t necp_client_uuid,
                                   pid_t pid, void *handle,
                                   u_int8_t message_type);

extern int netagent_client_message_with_params(
...
extern int netagent_copyout(uuid_t uuid, user_addr_t user_addr,
                            u_int32_t user_size);

extern int netagent_acquire_token(uuid_t uuid, user_addr_t user_addr,
...
typedef errno_t (*netagent_event_f)(
    u_int8_t event, uuid_t necp_client_uuid, pid_t pid, void *necp_handle,
    void *context, struct necp_client_agent_parameters *parameters,
    void *__sized_by(*assigned_results_length) * assigned_results,
    size_t *assigned_results_length);

extern netagent_session_t netagent_create(netagent_event_f event_handler,
                                          void *handle);

extern void netagent_destroy(netagent_session_t session);

extern errno_t netagent_register(netagent_session_t session,
                                 struct netagent *agent);

extern errno_t netagent_update(netagent_session_t session,
                               struct netagent *agent);

extern errno_t netagent_unregister(netagent_session_t session);

extern errno_t netagent_assign_nexus(
    netagent_session_t _session, uuid_t necp_client_uuid,
    void *assign_message __sized_by(assigned_results_length),
    size_t assigned_results_length); // Length of assigned_results_length

extern errno_t netagent_update_flow_protoctl_event(
