...
static __inline__ int _ether_cmp(const void *__unsafe_indexable a,
                                 const void *__unsafe_indexable b) {
  ...
  const u_int16_t *__unsafe_indexable a_s =
      __unsafe_forge_bidi_indexable(const u_int16_t *, a, ETHER_ADDR_LEN);
  const u_int16_t *__unsafe_indexable b_s =
      __unsafe_forge_bidi_indexable(const u_int16_t *, b, ETHER_ADDR_LEN);

  if (a_s[0] != b_s[0] || a_s[1] != b_s[1] || a_s[2] != b_s[2]) {
    return 1;
  }
  return 0;
}
