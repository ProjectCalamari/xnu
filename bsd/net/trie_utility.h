...
#define CHILD_MAP_SIZE                                                         \
  (LAST_PRINTABLE_ASCII - FIRST_PRINTABLE_ASCII +                              \
   1) // printable ascii characters only
#define NULL_TRIE_IDX 0xffff
#define TRIE_NODE(t, i) ((t)->nodes[(i)])
#define TRIE_CHILD_GET(t, i, b)                                                \
  ((b >= FIRST_PRINTABLE_ASCII && b <= LAST_PRINTABLE_ASCII)                   \
       ? (((t)->child_maps +                                                   \
           (CHILD_MAP_SIZE *                                                   \
            TRIE_NODE(t, i).child_map))[(b - FIRST_PRINTABLE_ASCII)])          \
       : NULL_TRIE_IDX)
...
typedef boolean_t (*check_metadata_func)(const uint8_t *metadata,
