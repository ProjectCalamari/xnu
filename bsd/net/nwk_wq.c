...
static TAILQ_HEAD(, nwk_wq_entry)
    nwk_wq_head = TAILQ_HEAD_INITIALIZER(nwk_wq_head);
...
void nwk_wq_init(void) {
  ...
  if (kernel_thread_start(nwk_wq_thread_func, NULL, &nwk_wq_thread) !=
      KERN_SUCCESS) {
...
#pragma unused(err)
  for (;;) {
    ...
    while (TAILQ_FIRST(&nwk_wq_head) == NULL) {
      (void)msleep0(&nwk_wq_waitch, &nwk_wq_lock, (PZERO - 1),
                    "nwk_wq_thread_cont", 0, nwk_wq_thread_cont);
      /* NOTREACHED */
    }
    ...
    TAILQ_FOREACH_SAFE(nwk_item, &temp_nwk_wq_head, nwk_wq_link,
                       nwk_item_next) {
...
__dead2 static void nwk_wq_thread_func(void *v, wait_result_t w) {
#pragma unused(v, w)
  lck_mtx_lock(&nwk_wq_lock);
  (void)msleep0(&nwk_wq_waitch, &nwk_wq_lock, (PZERO - 1), "nwk_wq_thread_func",
                0, nwk_wq_thread_cont);
  /*
