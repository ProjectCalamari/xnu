...
// These includes appear in ntstat.h but we include them here first so they
// won't trigger any clang diagnostic errors.
...
// This header defines structures shared with user space, so we need to ensure
// there is no compiler inserted padding in case the user space process isn't
// using the same architecture as the kernel (example: i386 process with x86_64
// kernel).
...
SYSCTL_NODE(_net, OID_AUTO, stats, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "network statistics");
...
SYSCTL_INT(_net_stats, OID_AUTO, lim_min_tx_pkts, CTLFLAG_RW | CTLFLAG_LOCKED,
           &nstat_lim_min_tx_pkts, 0,
           "Low Internet, min transmit packets threshold");

SYSCTL_INT(_net_stats, OID_AUTO, lim_min_rx_pkts, CTLFLAG_RW | CTLFLAG_LOCKED,
           &nstat_lim_min_rx_pkts, 0,
...
#define _NSTAT_LOG(handle, type, fmt, ...)                                     \
  do {                                                                         \
    os_log_with_type(handle, type, "%s - " fmt, __func__, ##__VA_ARGS__);      \
  } while (0)

#define NSTAT_LOG(fmt, ...)                                                    \
  _NSTAT_LOG(nstat_log_handle, OS_LOG_TYPE_DEFAULT, fmt, ##__VA_ARGS__)
#define NSTAT_LOG_DEBUG(fmt, ...)                                              \
  _NSTAT_LOG(nstat_log_handle, OS_LOG_TYPE_DEBUG, fmt, ##__VA_ARGS__)
#define NSTAT_LOG_INFO(fmt, ...)                                               \
  _NSTAT_LOG(nstat_log_handle, OS_LOG_TYPE_INFO, fmt, ##__VA_ARGS__)
#define NSTAT_LOG_ERROR(fmt, ...)                                              \
  _NSTAT_LOG(nstat_log_handle, OS_LOG_TYPE_ERROR, fmt, ##__VA_ARGS__)
...
#define NSTAT_DEBUG_SOCKET_ON(so)                                              \
  ((nstat_debug && (!nstat_debug_pid || NSTAT_DEBUG_SOCKET_PID_MATCHED(so)))   \
       ? nstat_debug                                                           \
       : 0)

#define NSTAT_DEBUG_SOCKET_LOG(so, fmt, ...)                                   \
  if (NSTAT_DEBUG_SOCKET_ON(so)) {                                             \
    NSTAT_LOG_DEBUG("NSTAT_DEBUG_SOCKET <pid %d>: " fmt,                       \
                    (so->so_flags & SOF_DELEGATED ? so->e_pid : so->last_pid), \
                    ##__VA_ARGS__);                                            \
  }
...
static int progress_indicators SYSCTL_HANDLER_ARGS {
  return ntstat_progress_indicators(req);
}

SYSCTL_PROC(
    _net_stats, OID_AUTO, progress,
    CTLTYPE_STRUCT | CTLFLAG_RW | CTLFLAG_LOCKED | CTLFLAG_ANYBODY, 0, 0,
    progress_indicators, "S",
    "Various items that indicate the current state of progress on the link");
 ...
 *
 * Each user space "client" of NetworkStatistics is represented by a
 * nstat_client structure. Communication between user space and kernel is
 * currently via a kernel control socket, but other mechanisms could
 * theoretically be used
 *
 * Each ntstat_client has a queue of nstat_src structures.  These typically
 * represent flows but could represent whatever the relevant provider makes
 * available to be reported to user level. The "nstat_src" name is perhaps a
 * little unfortunate as these don't directly refer to the socket, channel or
 * whatever is the ultimate source of the information but instead map directly
 * to each client's idea of that source.  They therefore contain things like the
 * source reference quoted to the client, which is unique only on a per client
 * basis. Currently each client has source references which are allocated
 * sequentially starting from one
 *
 * There are multiple "providers", one each for things like TCP sockets, QUIC
 * channels, routes etc. The ntstat_src structure contains the nts_provider
 * field to identify the particular provider and the nts_cookie field, which is
 * a pointer to a provider-dependent structure
 *
 * The nstat_src structure has a pointer to a provider dependent "cookie"
 * structure Some, for example the QUIC channel provider, may have a single
 * instance of a structure for all nstat_srcs.  Optionally the provider code may
 * use an nstat_locus structure to allow it to quickly iterate all the
 * associated nstat_srcs.  The alternative is to iterate over the nstat_srcs
 ...
 * 1) The second nstat_client has been in existence for longer than the first
 * one, and the larger nstat_srcref numbers reflect flows that were assigned
 * numbers earlier but which have now closed. 2) The nts_cookie field contains a
 * pointer to a provider dependent structure, here either a nstat_tu_shadow,
 * nstat_sock_locus or rtentry, for channel based or socket based flows or
 * routes respectively 3) The nstat_tu_shadow and nstat_sock_locus structures
 * contain an embedded nstat_locus 4) The nstat_tu_shadow structures are linked,
 * and the links are traversed when collecting all the associated flows when
 * there is a new "watcher".  In contrast, the nstat_sock_locus structures are
 * not linked and the ntstat code must understand how to traverse the relevant
 * structures in the BSD socket code 5) For simplicity, various linkages are not
 * shown.  In the nstat_src we have:
 *    - nts_client is a back pointer to the owning nstat_client
 *    - nts_locus is a pointer to the associated nstat_locus, or NULL if there
 * is no such item
 *    - nts_provider is a pointer to a provider-dependent structure with a list
 ...
 *        +-----------------+   |       +-----------------+   | .  | | |
 * nts_provider    |   |       | nts_provider    |   | .  |     nstat_sock_locus
 * |      inpcb
 *        +-----------------+   |       +-----------------+   | .  |
 * +-----------------+   |   +-----------------+ | nts_cookie      |-->|---+   |
 * nts_coookie     |-->|-.--->-->| nsl_locus       |---+   |                 |
 *        +-----------------+   |   |   +-----------------+   | .  |
 * +-----------------+       |                 | | nts_filter      |   |   |   |
 * nts_filter      |   | .  |    | nsl_link        |---+   |                 |
 *        +-----------------+   |   |   +-----------------+   | .  |
...
static int metrics_collection SYSCTL_HANDLER_ARGS {
  return ntstat_get_metrics(req);
}

SYSCTL_PROC(_net_stats, OID_AUTO, metrics,
            CTLTYPE_STRUCT | CTLFLAG_RW | CTLFLAG_LOCKED | CTLFLAG_ANYBODY, 0,
            0, metrics_collection, "S",
            "Various metrics for NetworkStatistics clients, individually or "
            "collectively");
...
#define NUM_NSTAT_METRICS_UINT32_COUNTS                                        \
  ((__builtin_offsetof(struct nstat_metrics, nstat_last_uint32_count) -        \
    __builtin_offsetof(struct nstat_metrics, nstat_first_uint32_count)) /      \
   sizeof(uint32_t))
...
typedef struct nstat_client {
  ...
  /* A bitmask to indicate whether a provider ever done
  ...
  decl_lck_mtx_data(, ntc_user_mtx); // Mutual exclusion for user level requests
                                     // on this ntc_client
...
typedef struct nstat_provider {
  ...
  errno_t (*nstat_lookup)(const void *__sized_by(length) data, u_int32_t length,
                          nstat_provider_cookie_t *out_cookie);
  int (*nstat_gone)(nstat_provider_cookie_t cookie);
  errno_t (*nstat_counts)(nstat_provider_cookie_t cookie,
                          struct nstat_counts *out_counts, int *out_gone);
  errno_t (*nstat_details)(nstat_provider_cookie_t cookie,
                           struct nstat_detailed_counts *out_details,
                           int *out_gone);
  errno_t (*nstat_watcher_add)(nstat_client *client,
                               nstat_msg_add_all_srcs *req);
  void (*nstat_watcher_remove)(nstat_client *client);
  errno_t (*nstat_copy_descriptor)(nstat_provider_cookie_t cookie,
                                   void *__sized_by(len) data, size_t len);
  void (*nstat_release)(nstat_provider_cookie_t cookie);
  bool (*nstat_reporting_allowed)(nstat_provider_cookie_t cookie,
                                  nstat_provider_filter *filter,
                                  u_int64_t suppression_flags);
  bool (*nstat_cookie_equal)(nstat_provider_cookie_t cookie1,
                             nstat_provider_cookie_t cookie2);
  size_t (*nstat_copy_extension)(nstat_provider_cookie_t cookie,
                                 u_int32_t extension_id, void *buf, size_t len);
} nstat_provider;

typedef struct nstat_src {
  tailq_entry_nstat_src
      nts_client_link; // All sources for the nstat_client, for iterating over.
  tailq_entry_nstat_src
      nts_locus_link; // All sources for the nstat_locus, for iterating over.
  ...
  nstat_provider *nts_provider; // The "provider" for the source, e.g. for
                                // kernel TCP sockets
...
// The merge structures are intended to give a global picture of what may be
// asked for by the current set of clients This is to avoid taking locks to
...
static errno_t nstat_client_send_counts(nstat_client *client, nstat_src *src,
                                        unsigned long long context,
                                        u_int16_t hdr_flags, int *gone);
static int nstat_client_send_description(nstat_client *client, nstat_src *src,
                                         u_int64_t context,
                                         u_int16_t hdr_flags);
static int nstat_client_send_update(nstat_client *client, nstat_src *src,
                                    u_int64_t context, u_int64_t event,
                                    u_int16_t hdr_flags, int *gone);
static errno_t nstat_client_send_removed(nstat_client *client, nstat_src *src,
                                         u_int16_t hdr_flags);
...
static bool nstat_client_reporting_allowed(nstat_client *client, nstat_src *src,
                                           u_int64_t suppression_flags);
static boolean_t nstat_client_begin_query(nstat_client *client,
                                          const nstat_msg_hdr *hdrp);
static u_int16_t nstat_client_end_query(nstat_client *client,
                                        nstat_src *last_src, boolean_t partial);
static void nstat_ifnet_report_lim_stats(void);
static void nstat_net_api_report_stats(void);
static errno_t nstat_set_provider_filter(nstat_client *client,
                                         nstat_msg_add_all_srcs *req);
static errno_t nstat_client_send_event(nstat_client *client, nstat_src *src,
                                       u_int64_t event);
...
static uint32_t nstat_next_client_id =
    3; // Deliberate offset from zero to reserve values 0 and 1
...
#define NSTAT_TRACE(field, client, qual)                                       \
  {                                                                            \
    nstat_trace(offsetof(struct nstat_metrics, field), client,                 \
                (uint64_t)qual);                                               \
  }
...
#define NSTAT_GLOBAL_COUNT_INCREMENT_WITH_MAX(_currentfield, _maxfield)        \
  {                                                                            \
    uint64_t _prev_count =                                                     \
        os_atomic_inc_orig(&nstat_global_counts._currentfield, relaxed);       \
    if (_prev_count >=                                                         \
        os_atomic_load(&nstat_global_counts._maxfield, relaxed)) {             \
      os_atomic_store(&nstat_global_counts._maxfield, _prev_count + 1,         \
                      relaxed);                                                \
    }                                                                          \
  }
...
static int nstat_client_send_update(nstat_client *client, nstat_src *src,
                                    u_int64_t context, u_int64_t event,
                                    u_int16_t hdr_flags, int *gone);
static int nstat_client_send_details(nstat_client *client, nstat_src *src,
                                     u_int64_t context, u_int64_t event,
                                     u_int16_t hdr_flags, int *gone);
static errno_t nstat_client_send_removed(nstat_client *client, nstat_src *src,
                                         u_int16_t hdr_flags);
 ...
 *
 * There is one other lock to consider. Calls from NetworkStatistics clients
 * will first obtain the per-client ntc_user_mtx.  This ensures that only one
 * such client thread can be in progress at a time, making it easier to reason
 * about correctness.
 */
...
#define NSTAT_LOCK_SHARED_TO_EXCLUSIVE()                                       \
  lck_rw_lock_shared_to_exclusive(&nstat_rwlock)
#define NSTAT_LOCK_EXCLUSIVE_TO_SHARED()                                       \
  lck_rw_lock_exclusive_to_shared(&nstat_rwlock)
...
#define NSTAT_LOCK_YIELD_EXCLUSIVE()                                           \
  lck_rw_lock_yield_exclusive(&nstat_rwlock, LCK_RW_YIELD_ANY_WAITER)
#define NSTAT_ASSERT_LOCKED_EXCLUSIVE()                                        \
  LCK_RW_ASSERT(&nstat_rwlock, LCK_RW_ASSERT_EXCLUSIVE)
#define NSTAT_ASSERT_LOCKED_SHARED()                                           \
  LCK_RW_ASSERT(&nstat_rwlock, LCK_RW_ASSERT_SHARED)
#define NSTAT_ASSERT_LOCKED() LCK_RW_ASSERT(&nstat_rwlock, LCK_RW_ASSERT_HELD)
#define NSTAT_ASSERT_UNLOCKED()                                                \
  LCK_RW_ASSERT(&nstat_rwlock, LCK_RW_ASSERT_NOTHELD)

/* some extern definitions */
extern void tcp_report_stats(void);

#if NSTAT_TRACE_ENABLED
static void nstat_trace(int32_t trace_id, nstat_client *client, uint64_t qual) {
...
#if NSTAT_FUZZ_TIMING
static void nstat_random_delay(nstat_client *client) {
...
static void nstat_accumulate_client_metrics(struct nstat_metrics *dest,
                                            nstat_client *client) {
  if (dest->nstat_src_max < client->ntc_metrics.nstat_src_max) {
    dest->nstat_src_max = client->ntc_metrics.nstat_src_max;
  }
  // Most of the counts happen to be consecutive uint32_t values that can be
  // picked up via pointer iteration rather than name
  uint32_t *srcptr = __unsafe_forge_bidi_indexable(
      uint32_t *,
  ...
  uint32_t *destptr = __unsafe_forge_bidi_indexable(
      uint32_t *, (uint32_t *)(void *)&dest->nstat_first_uint32_count,
      (NUM_NSTAT_METRICS_UINT32_COUNTS * sizeof(uint32_t)));
...
static int metrics_for_client_id(uint32_t req_id,
  ...
  if (req_id == NSTAT_METRIC_ID_ACCUMULATED) {
    client_info->nstat_client_details.nstat_client_id =
        NSTAT_METRIC_ID_ACCUMULATED;
    *metrics = nstat_metrics;
  } else {
    NSTAT_LOCK_EXCLUSIVE();
    if (req_id == NSTAT_METRIC_ID_GRAND_TOTAL) {
      client_info->nstat_client_details.nstat_client_id =
          NSTAT_METRIC_ID_GRAND_TOTAL;
      *metrics = nstat_metrics;
      nstat_client *client;
      ...
      for (client = nstat_clients; client; client = client->ntc_next) {
        if (client->ntc_client_id <= req_id) {
          client_info->nstat_client_details.nstat_client_id =
              client->ntc_client_id;
          client_info->nstat_client_details.nstat_client_pid =
              client->ntc_procdetails->pdet_pid;
          client_info->nstat_client_details.nstat_client_watching =
              client->ntc_watching;
          client_info->nstat_client_details.nstat_client_added_src =
              client->ntc_added_src;
...
static int ntstat_get_metrics(struct sysctl_req *req) {
  // The following assumes that the client_info structure is small such that
  // stack allocation is reasonable
  struct nstat_client_info client_info = {};
  int error = 0;
  struct nstat_metrics_req requested;

  if (priv_check_cred(kauth_cred_get(), PRIV_NET_PRIVILEGED_NETWORK_STATISTICS,
                      0) != 0) {
    return EACCES;
  }
...
static void nstat_copy_sa_out(const struct sockaddr *src, struct sockaddr *dst,
                              int maxlen) {
...
static void nstat_ip_to_sockaddr(const struct in_addr *ip, u_int16_t port,
                                 struct sockaddr_in *sin, u_int32_t maxlen) {
...
u_int32_t nstat_ifnet_to_flags(struct ifnet *ifp) {
  ...
  peer_egress_functional_type =
      if_peer_egress_functional_type(ifp, !(IFNET_IS_VPN(ifp)));
...
static void nstat_update_local_flag_from_inpcb_route(const struct inpcb *inp,
                                                     u_int32_t *flags) {
...
static u_int32_t nstat_inpcb_to_flags(const struct inpcb *inp) {
  u_int32_t flags = 0;
...
static void merge_current_event_filters(void) {
  ...
  for (client = nstat_clients; client; client = client->ntc_next) {
    for (provider = NSTAT_PROVIDER_NONE; provider <= NSTAT_PROVIDER_LAST;
         provider++) {
      new_merge.mpf_filters[provider].mf_events |=
          client->ntc_provider_filters[provider].npf_events;
    }
  }
  for (provider = NSTAT_PROVIDER_NONE; provider <= NSTAT_PROVIDER_LAST;
       provider++) {
    // This should do atomic updates of the 64 bit words, where memcpy would be
    // undefined
    merged_filters.mpf_filters[provider].mf_events =
        new_merge.mpf_filters[provider].mf_events;
  }
}

static inline void nstat_src_remove_linkages(nstat_client *client,
                                             nstat_src *src) {
...
static errno_t nstat_client_source_add(u_int64_t context, nstat_client *client,
...
nstat_find_provider_by_id(nstat_provider_id_t id) {
  struct nstat_provider *provider;

  for (provider = nstat_providers; provider != NULL;
...
static errno_t nstat_lookup_entry(nstat_provider_id_t id,
...
static void nstat_client_sanitize_cookie(nstat_client *client,
  ...
  TAILQ_FOREACH(src, &client->ntc_src_queue, nts_client_link) {
    ...
    if (sp->nstat_provider_id == id && sp->nstat_cookie_equal != NULL &&
...
__private_extern__ void nstat_init(void) {
...
static tailq_head_procdetails nstat_procdetails_head =
    TAILQ_HEAD_INITIALIZER(nstat_procdetails_head);

static struct nstat_procdetails *nstat_retain_curprocdetails(void) {
  ...
  if (!procdetails) {
    // No need for paranoia on locking, it would be OK if there are duplicate
    // structs on the list
    procdetails = kalloc_type(struct nstat_procdetails, Z_WAITOK | Z_NOFAIL);
    procdetails->pdet_pid = proc_selfpid();
    procdetails->pdet_upid = upid;
    proc_selfname(procdetails->pdet_procname,
                  sizeof(procdetails->pdet_procname));
    proc_getexecutableuuid(current_proc(), procdetails->pdet_uuid,
                           sizeof(uuid_t));
    ...
    NSTAT_GLOBAL_COUNT_INCREMENT_WITH_MAX(nstat_global_procdetails_current,
...
static void nstat_release_procdetails(struct nstat_procdetails *procdetails) {
  assert(procdetails->pdet_magic == NSTAT_PROCDETAILS_MAGIC);
  // These are harvested later to amortize costs
  OSDecrementAtomic(&procdetails->pdet_refcnt);
}

static void nstat_prune_procdetails(void) {
  ...
  TAILQ_FOREACH_SAFE(procdetails, &nstat_procdetails_head, pdet_link,
                     tmpdetails) {
    assert(procdetails->pdet_magic == NSTAT_PROCDETAILS_MAGIC);
...
static errno_t nstat_route_lookup(const void *data, u_int32_t length,
                                  nstat_provider_cookie_t *out_cookie) {
  ...
  *out_cookie = NULL;
  ...
  if (param->dst.v4.sin_family == 0 || param->dst.v4.sin_family > AF_MAX ||
      (param->mask.v4.sin_family != 0 &&
       param->mask.v4.sin_family != param->dst.v4.sin_family)) {
    return EINVAL;
  }

  if (param->dst.v4.sin_len > sizeof(param->dst) ||
      (param->mask.v4.sin_family &&
       param->mask.v4.sin_len > sizeof(param->mask.v4.sin_len))) {
    return EINVAL;
  }
...
static int nstat_route_gone(nstat_provider_cookie_t cookie) {
  struct rtentry *rt = (struct rtentry *)cookie;
  return ((rt->rt_flags & RTF_UP) == 0) ? 1 : 0;
}

static errno_t nstat_route_counts(nstat_provider_cookie_t cookie,
...
static void nstat_route_release(nstat_provider_cookie_t cookie) {
  rtfree((struct rtentry *)cookie);
}

static u_int32_t nstat_route_watchers = 0;

static int nstat_route_walktree_add(struct radix_node *rn, void *context) {
  errno_t result = 0;
  ...
  if ((rt->rt_flags & RTF_UP) != 0) {
    ...
    NSTAT_LOCK_EXCLUSIVE();
    result =
        nstat_client_source_add(0, client, &nstat_route_provider, rt, NULL);
    NSTAT_UNLOCK_EXCLUSIVE();
    if (result != 0) {
      rtfree_locked(rt);
    }
...
static errno_t nstat_route_add_watcher(nstat_client *client,
                                       nstat_msg_add_all_srcs *req) {
  int i;
  errno_t result = 0;
...
__private_extern__ void nstat_route_new_entry(struct rtentry *rt) {
  ...
  if ((rt->rt_flags & RTF_UP) != 0) {
    ...
    for (client = nstat_clients; client; client = client->ntc_next) {
      if ((client->ntc_watching & (1 << NSTAT_PROVIDER_ROUTE)) != 0) {
        ...
        if (nstat_client_source_add(0, client, &nstat_route_provider, rt,
                                    NULL) != 0) {
          RT_REMREF(rt);
        }
      }
...
static void nstat_route_remove_watcher(__unused nstat_client *client) {
  OSDecrementAtomic(&nstat_route_watchers);
}

static errno_t nstat_route_copy_descriptor(nstat_provider_cookie_t cookie,
...
static bool
nstat_route_reporting_allowed(nstat_provider_cookie_t cookie,
                              nstat_provider_filter *filter,
...
static bool nstat_route_cookie_equal(nstat_provider_cookie_t cookie1,
                                     nstat_provider_cookie_t cookie2) {
...
nstat_route_attach(struct rtentry *rte) {
...
__private_extern__ void nstat_route_detach(struct rtentry *rte) {
  if (rte->rt_stats) {
...
__private_extern__ void nstat_route_connect_attempt(struct rtentry *rte) {
  while (rte) {
...
__private_extern__ void nstat_route_connect_success(struct rtentry *rte) {
  // This route
  while (rte) {
    struct nstat_counts *stats = nstat_route_attach(rte);
    if (stats) {
...
__private_extern__ void nstat_route_tx(struct rtentry *rte, u_int32_t packets,
                                       u_int32_t bytes, u_int32_t flags) {
  while (rte) {
    struct nstat_counts *stats = nstat_route_attach(rte);
    if (stats) {
...
__private_extern__ void nstat_route_rx(struct rtentry *rte, u_int32_t packets,
                                       u_int32_t bytes, u_int32_t flags) {
  while (rte) {
    struct nstat_counts *stats = nstat_route_attach(rte);
    if (stats) {
...
#define NSTAT_EWMA_ATOMIC(_val_addr, _new_val, _decay)                         \
  do {                                                                         \
...
/* atomically compute minimum of current value at _val_addr with _new_val and
 * store  */
#define NSTAT_MIN_ATOMIC(_val_addr, _new_val)                                  \
  do {                                                                         \
    volatile uint32_t _old_val;                                                \
    do {                                                                       \
      _old_val = *_val_addr;                                                   \
...
__private_extern__ void nstat_route_rtt(struct rtentry *rte, u_int32_t rtt,
                                        u_int32_t rtt_var) {
...
__private_extern__ void nstat_route_update(
    struct rtentry *rte, uint32_t connect_attempts, uint32_t connect_successes,
    uint32_t rx_packets, uint32_t rx_bytes, uint32_t rx_duplicatebytes,
    uint32_t rx_outoforderbytes, uint32_t tx_packets, uint32_t tx_bytes,
    uint32_t tx_retransmit, uint32_t rtt, uint32_t rtt_var) {
  const uint32_t decay = 3;
 ...
 *
 * There may be either zero or one of the TCP and UDP variants of the
 * nstat_sock_locus for any socket.  There may be multiple nstat_src structures
 * linked to the locus. At the time of original implementation, it is not
 * expected that the nstat_sock_locus should live beyond the lifetime of the
 * socket.  There is therefore no reference counting and the structures are
 * disposed of when the socket closes.
 */
...
typedef struct nstat_sock_locus {
  ...
  tailq_entry_sock_locus
      nsl_link;          // TCP and UDP sock_locus structures queued here
...
static tailq_head_sock_locus nstat_tcp_sock_locus_head =
    TAILQ_HEAD_INITIALIZER(nstat_tcp_sock_locus_head);
static tailq_head_sock_locus nstat_udp_sock_locus_head =
    TAILQ_HEAD_INITIALIZER(nstat_udp_sock_locus_head);

static bool nstat_tcpudp_reporting_allowed(nstat_provider_cookie_t cookie,
                                           nstat_provider_filter *filter,
                                           bool is_UDP);

static struct nstat_sock_locus *
nstat_sock_locus_alloc_internal(struct inpcb *inp, bool locked) {
  ...
  if (SOCK_PROTO(inp->inp_socket) == IPPROTO_UDP) {
    ...
    NSTAT_GLOBAL_COUNT_INCREMENT_WITH_MAX(nstat_global_tcp_sck_locus_current,
                                          nstat_global_tcp_sck_locus_max);
    // The acquire here may be redundant, it is balanced by a release when the
    // pcb is detached Copied from tucookie precedent, which had a different
    // mechanism but similar constraints
    if (in_pcb_checkstate(inp, WNT_ACQUIRE, locked) == WNT_STOPUSING) {
...
static struct nstat_sock_locus *
nstat_sock_locus_alloc_locked(struct inpcb *inp) {
  return nstat_sock_locus_alloc_internal(inp, true);
}

static void nstat_sock_locus_release(struct nstat_sock_locus *sol,
                                     int inplock) {
...
static size_t nstat_inp_domain_info(struct inpcb *inp,
                                    nstat_domain_info *domain_info,
                                    size_t len) {
  // Note, the caller has guaranteed that the buffer has been zeroed, there is
  ...
  NSTAT_DEBUG_SOCKET_LOG(
      so,
      "NSTAT: <pid %d> Collected stats - domain <%s> owner <%s> ctxt <%s> "
      "bundle id <%s> "
...
static size_t nstat_inp_bluetooth_counts(struct inpcb *inp,
                                         nstat_interface_counts *buf,
                                         size_t len) {
  // Note, the caller has guaranteed that the buffer has been zeroed, there is
  // no need to clear it again
  struct socket *so = inp->inp_socket;
  ...
  if (buf == NULL) {
    ...
    if ((rxbytes == 0) && (txbytes == 0)) {
      // It's more efficient to skip sending counts if they're only going to be
      // zero
      return 0;
    }
  ...
  buf->nstat_rxpackets =
      os_atomic_load(&inp->inp_mstat.ms_bluetooth.ts_rxpackets, relaxed);
  buf->nstat_rxbytes =
      os_atomic_load(&inp->inp_mstat.ms_bluetooth.ts_rxbytes, relaxed);
  buf->nstat_txpackets =
      os_atomic_load(&inp->inp_mstat.ms_bluetooth.ts_txpackets, relaxed);
  buf->nstat_txbytes =
      os_atomic_load(&inp->inp_mstat.ms_bluetooth.ts_txbytes, relaxed);
  return sizeof(nstat_interface_counts);
}

static nstat_provider nstat_tcp_provider;

static errno_t nstat_tcp_lookup(__unused const void *data,
...
static int nstat_tcp_gone(nstat_provider_cookie_t cookie) {
  struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
  struct inpcb *inp;
  struct tcpcb *tp;

  return (!(inp = sol->nsl_inp) || !(tp = intotcpcb(inp)) ||
          inp->inp_state == INPCB_STATE_DEAD)
             ? 1
             : 0;
}

static errno_t nstat_tcp_counts(nstat_provider_cookie_t cookie,
                                struct nstat_counts *out_counts,
                                int *out_gone) {
  struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
  struct inpcb *inp;
  ...
  out_counts->nstat_rxpackets =
      os_atomic_load(&inp->inp_mstat.ms_total.ts_rxpackets, relaxed);
  out_counts->nstat_rxbytes =
      os_atomic_load(&inp->inp_mstat.ms_total.ts_rxbytes, relaxed);
  out_counts->nstat_txpackets =
      os_atomic_load(&inp->inp_mstat.ms_total.ts_txpackets, relaxed);
  out_counts->nstat_txbytes =
      os_atomic_load(&inp->inp_mstat.ms_total.ts_txbytes, relaxed);
  ...
  out_counts->nstat_cell_rxbytes =
      os_atomic_load(&inp->inp_mstat.ms_cellular.ts_rxbytes, relaxed);
  out_counts->nstat_cell_txbytes =
      os_atomic_load(&inp->inp_mstat.ms_cellular.ts_txbytes, relaxed);
  out_counts->nstat_wifi_rxbytes =
      os_atomic_load(&inp->inp_mstat.ms_wifi_infra.ts_rxbytes, relaxed) +
      os_atomic_load(&inp->inp_mstat.ms_wifi_non_infra.ts_rxbytes, relaxed);
  out_counts->nstat_wifi_txbytes =
      os_atomic_load(&inp->inp_mstat.ms_wifi_infra.ts_txbytes, relaxed) +
      os_atomic_load(&inp->inp_mstat.ms_wifi_non_infra.ts_txbytes, relaxed);
  out_counts->nstat_wired_rxbytes =
      os_atomic_load(&inp->inp_mstat.ms_wired.ts_rxbytes, relaxed);
  out_counts->nstat_wired_txbytes =
      os_atomic_load(&inp->inp_mstat.ms_wired.ts_txbytes, relaxed);

  return 0;
}

static errno_t nstat_tcp_details(nstat_provider_cookie_t cookie,
                                 struct nstat_detailed_counts *out_details,
                                 int *out_gone) {
  struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
  struct inpcb *inp;
...
static void nstat_tcp_release(__unused nstat_provider_cookie_t cookie) {}

static errno_t nstat_tcp_add_watcher(nstat_client *client,
                                     nstat_msg_add_all_srcs *req) {
  errno_t result;
  ...
  // so the NSTAT_LOCK_EXCLUSIVE() ensures that sockets are added once and only
  // once
  ...
  if (result == 0) {
    ...
    TAILQ_FOREACH(sol, &nstat_tcp_sock_locus_head, nsl_link) {
      ...
      // Ideally all dead inpcbs should have been removed from the list, but be
      // paranoid
      ...
      nstat_client_source_add(0, client, &nstat_tcp_provider, sol,
                              &sol->nsl_locus);
    }
  }
  NSTAT_UNLOCK_EXCLUSIVE();
...
static void nstat_tcp_remove_watcher(__unused nstat_client *client) {
  OSDecrementAtomic(&nstat_tcp_watchers);
}

__private_extern__ void nstat_tcp_new_pcb(struct inpcb *inp) {
  ...
  for (client = nstat_clients; client; client = client->ntc_next) {
    if ((client->ntc_watching & (1 << NSTAT_PROVIDER_TCP_KERNEL)) != 0) {
      // this client is watching tcp and may or may not be interested in this
      // example Metrics are gathered by nstat_client_source_add(), There's no
      // possible recovery action or logging needed here, no need to check
      // return code
      nstat_client_source_add(0, client, &nstat_tcp_provider, sol,
                              &sol->nsl_locus);
    }
...
__private_extern__ void nstat_pcb_detach(struct inpcb *inp) {
  nstat_client *client;
  nstat_src *src;
  nstat_src *tmpsrc;
  ...
  if (sol) {
    ...
    TAILQ_FOREACH_SAFE(src, &inp->inp_nstat_locus->nsl_locus.ntl_src_queue,
                       nts_locus_link, tmpsrc) {
...
__private_extern__ void nstat_pcb_event(struct inpcb *inp, u_int64_t event) {
  nstat_client *client;
  nstat_src *src;
  struct nstat_sock_locus *sol;
  errno_t result;
  ...
  if (inp == NULL || (inp->inp_nstat_locus == NULL) ||
      (nstat_tcp_watchers == 0 && nstat_udp_watchers == 0)) {
    return;
  }
  if (((merged_filters.mpf_filters[NSTAT_PROVIDER_TCP_KERNEL].mf_events &
        event) == 0) &&
      ((merged_filters.mpf_filters[NSTAT_PROVIDER_UDP_KERNEL].mf_events &
        event) == 0)) {
  ...
  TAILQ_FOREACH(src, &sol->nsl_locus.ntl_src_queue, nts_locus_link) {
    ...
    assert((provider_id == NSTAT_PROVIDER_TCP_KERNEL) ||
           (provider_id == NSTAT_PROVIDER_UDP_KERNEL));

    struct nstat_provider_filter *filter =
        &client->ntc_provider_filters[provider_id];
...
__private_extern__ void nstat_udp_pcb_cache(struct inpcb *inp) {
  if (inp == NULL) {
    return;
  }
  ...
  struct nstat_extended_sock_locus *esol =
      (struct nstat_extended_sock_locus *)inp->inp_nstat_locus;
  if (esol != NULL) {
    ...
    if (inp->inp_vflag & INP_IPV6) {
      in6_ip6_to_sockaddr(&inp->in6p_laddr, inp->inp_lport, inp->inp_lifscope,
                          &esol->nesl_local.v6, sizeof(esol->nesl_local));
      in6_ip6_to_sockaddr(&inp->in6p_faddr, inp->inp_fport, inp->inp_fifscope,
                          &esol->nesl_remote.v6, sizeof(esol->nesl_remote));
    } else if (inp->inp_vflag & INP_IPV4) {
      nstat_ip_to_sockaddr(&inp->inp_laddr, inp->inp_lport,
                           &esol->nesl_local.v4, sizeof(esol->nesl_local));
      nstat_ip_to_sockaddr(&inp->inp_faddr, inp->inp_fport,
                           &esol->nesl_remote.v4, sizeof(esol->nesl_remote));
    } else {
...
__private_extern__ void nstat_udp_pcb_invalidate_cache(struct inpcb *inp) {
  if (inp == NULL) {
    return;
  }
  VERIFY(SOCK_PROTO(inp->inp_socket) == IPPROTO_UDP);
...
__private_extern__ void nstat_pcb_update_last_owner(struct inpcb *inp) {
  ...
  if (sol != NULL) {
    ...
    if (sol->nsl_pid != current_pid) {
      proc_best_name_for_pid(current_pid, sol->nsl_pname,
                             sizeof(sol->nsl_pname));
      if (sol->nsl_pname[0] != '\0') {
...
static errno_t nstat_tcp_copy_descriptor(nstat_provider_cookie_t cookie,
                                         void *__sized_by(len) data,
                                         size_t len) {
  ...
  if (inp->inp_vflag & INP_IPV6) {
    ...
    nstat_ip_to_sockaddr(&inp->inp_laddr, inp->inp_lport, &desc->local.v4,
                         sizeof(desc->local));
    nstat_ip_to_sockaddr(&inp->inp_faddr, inp->inp_fport, &desc->remote.v4,
                         sizeof(desc->remote));
  }

  desc->state = intotcpcb(inp)->t_state;
  desc->ifindex =
      (inp->inp_last_outifp == NULL) ? 0 : inp->inp_last_outifp->if_index;
  ...
  if (so) {
    ...
    if ((desc->pid == sol->nsl_pid) && (sol->nsl_pname[0] != '\0')) {
      ...
      if (desc->pname[0] != '\0') {
        // This may not be fully synchronized but multiple updates should be
        // fine
    ...
    if (nstat_debug) {
      ...
      NSTAT_DEBUG_SOCKET_LOG(
          so, "NSTAT: TCP - pid %d uid %d euuid %s persona id %d", desc->pid,
          desc->uid, euuid_str, desc->persona_id);
    }
  }
...
static bool nstat_tcpudp_reporting_allowed(nstat_provider_cookie_t cookie,
  ...
  if ((filter->npf_flags &
       (NSTAT_FILTER_IFNET_FLAGS | NSTAT_FILTER_SPECIFIC_USER)) != 0) {
    struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
    struct inpcb *inp = sol->nsl_inp;
    ...
    if ((filter->npf_flags & NSTAT_FILTER_IFNET_FLAGS) != 0) {
      ...
      if ((filter->npf_flags & interface_properties) == 0) {
        // For UDP, we could have an undefined interface and yet transfers may
        // have occurred. We allow reporting if there have been transfers of the
        // requested kind. This is imperfect as we cannot account for the
        // expensive attribute over wifi. We also assume that cellular is
        // expensive and we have no way to select for AWDL
        if (is_UDP) {
          do {
            if ((filter->npf_flags & (NSTAT_FILTER_ACCEPT_CELLULAR |
                                      NSTAT_FILTER_ACCEPT_EXPENSIVE)) &&
                (inp->inp_mstat.ms_cellular.ts_rxbytes ||
                 inp->inp_mstat.ms_cellular.ts_txbytes)) {
              break;
            }
            if ((filter->npf_flags & NSTAT_FILTER_ACCEPT_WIFI) &&
                (inp->inp_mstat.ms_wifi_infra.ts_rxbytes ||
                 inp->inp_mstat.ms_wifi_infra.ts_txbytes ||
            ...
            if ((filter->npf_flags & NSTAT_FILTER_ACCEPT_WIRED) &&
                (inp->inp_mstat.ms_wired.ts_rxbytes ||
                 inp->inp_mstat.ms_wired.ts_txbytes)) {
              break;
            }
            if ((filter->npf_flags & NSTAT_FILTER_ACCEPT_COMPANIONLINK_BT) &&
                (inp->inp_mstat.ms_bluetooth.ts_rxbytes ||
                 inp->inp_mstat.ms_bluetooth.ts_txbytes)) {
              break;
            }
            return false;
          } while (0);
    ...
    if (((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER) != 0) && (retval)) {
      ...
      if (so) {
        if (((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER_BY_PID) != 0) &&
          ...
          retval = true;
        } else if (((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER_BY_EPID) !=
                    0) &&
                   (filter->npf_pid == (so->so_flags & SOF_DELEGATED)
                        ? so->e_upid
                        : so->last_pid)) {
          retval = true;
        } else if (((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER_BY_UUID) !=
                    0) &&
                   (memcmp(filter->npf_uuid, so->last_uuid,
                           sizeof(so->last_uuid)) == 0)) {
          retval = true;
        } else if (((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER_BY_EUUID) !=
                    0) &&
                   (memcmp(filter->npf_uuid,
                           (so->so_flags & SOF_DELEGATED) ? so->e_uuid
                                                          : so->last_uuid,
                           sizeof(so->last_uuid)) == 0)) {
          retval = true;
        }
...
static bool nstat_tcp_reporting_allowed(nstat_provider_cookie_t cookie,
                                        nstat_provider_filter *filter,
                                        __unused u_int64_t suppression_flags) {
  return nstat_tcpudp_reporting_allowed(cookie, filter, FALSE);
}

static size_t nstat_tcp_extensions(nstat_provider_cookie_t cookie,
                                   u_int32_t extension_id, void *buf,
                                   size_t len) {
  struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
  struct inpcb *inp = sol->nsl_inp;
...
static errno_t nstat_udp_lookup(__unused const void *data,
                                __unused u_int32_t length,
                                __unused nstat_provider_cookie_t *out_cookie) {
  // Looking up a specific connection is not supported.
  return ENOTSUP;
...
static int nstat_udp_gone(nstat_provider_cookie_t cookie) {
  struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
  struct inpcb *inp;

  return (!(inp = sol->nsl_inp) || inp->inp_state == INPCB_STATE_DEAD) ? 1 : 0;
}

static errno_t nstat_udp_counts(nstat_provider_cookie_t cookie,
                                struct nstat_counts *out_counts,
                                int *out_gone) {
  struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
...
static errno_t nstat_udp_details(nstat_provider_cookie_t cookie,
                                 struct nstat_detailed_counts *out_details,
                                 int *out_gone) {
  struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
...
static void nstat_udp_release(__unused nstat_provider_cookie_t cookie) {}

static errno_t nstat_udp_add_watcher(nstat_client *client,
                                     nstat_msg_add_all_srcs *req) {
  errno_t result;
  ...
  if (result == 0) {
    ...
    TAILQ_FOREACH(sol, &nstat_udp_sock_locus_head, nsl_link) {
      ...
      // There's no need to check the return code from nstat_client_source_add()
      // which will have performed any recovery actions directly
      nstat_client_source_add(0, client, &nstat_udp_provider, sol,
                              &sol->nsl_locus);
    }
  }
  NSTAT_UNLOCK_EXCLUSIVE();
...
static void nstat_udp_remove_watcher(__unused nstat_client *client) {
  OSDecrementAtomic(&nstat_udp_watchers);
}

__private_extern__ void nstat_udp_new_pcb(struct inpcb *inp) {
  struct nstat_sock_locus *sol = NULL;
  inp->inp_start_timestamp = mach_continuous_time();
...
static errno_t nstat_udp_copy_descriptor(nstat_provider_cookie_t cookie,
                                         void *__sized_by(len) data,
                                         size_t len) {
  ...
  struct nstat_extended_sock_locus *esol =
      (struct nstat_extended_sock_locus *)cookie;
  ...
  if (esol->nesl_cached == false) {
    ...
    if (inp->inp_vflag & INP_IPV6) {
      memcpy(&desc->local.v6, &esol->nesl_local.v6, sizeof(desc->local.v6));
      memcpy(&desc->remote.v6, &esol->nesl_remote.v6, sizeof(desc->remote.v6));
    } else if (inp->inp_vflag & INP_IPV4) {
      memcpy(&desc->local.v4, &esol->nesl_local.v4, sizeof(desc->local.v4));
      memcpy(&desc->remote.v4, &esol->nesl_remote.v4, sizeof(desc->remote.v4));
    }
  ...
  if (so) {
    ...
    if (nstat_debug) {
      ...
      uuid_unparse(desc->euuid, euuid_str);
      NSTAT_DEBUG_SOCKET_LOG(
          so, "NSTAT: UDP - pid %d uid %d euuid %s persona id %d", desc->pid,
          desc->uid, euuid_str, desc->persona_id);
    }
  }
...
static bool nstat_udp_reporting_allowed(nstat_provider_cookie_t cookie,
                                        nstat_provider_filter *filter,
                                        __unused u_int64_t suppression_flags) {
  return nstat_tcpudp_reporting_allowed(cookie, filter, TRUE);
}

static size_t nstat_udp_extensions(nstat_provider_cookie_t cookie,
                                   u_int32_t extension_id, void *buf,
                                   size_t len) {
  struct nstat_sock_locus *sol = (struct nstat_sock_locus *)cookie;
  struct inpcb *inp = sol->nsl_inp;
...
// Magic number checking should remain in place until the userland provider has
// been fully proven
#define TU_SHADOW_MAGIC 0xfeedf00d
#define TU_SHADOW_UNMAGIC 0xdeaddeed

static tailq_head_tu_shadow nstat_userprot_shad_head =
...
static errno_t
nstat_userland_tu_lookup(__unused const void *data, __unused u_int32_t length,
                         __unused nstat_provider_cookie_t *out_cookie) {
  // Looking up a specific connection is not supported
  return ENOTSUP;
}

static int nstat_userland_tu_gone(__unused nstat_provider_cookie_t cookie) {
  // Returns non-zero if the source has gone.
  // We don't keep a source hanging around, so the answer is always 0
  return 0;
}

static errno_t nstat_userland_tu_counts(nstat_provider_cookie_t cookie,
                                        struct nstat_counts *out_counts,
                                        int *out_gone) {
  ...
  bool result = (*shad->shad_getvals_fn)(shad->shad_provider_context, NULL,
                                         NULL, out_counts, NULL, NULL);
...
static errno_t
nstat_userland_tu_details(nstat_provider_cookie_t cookie,
                          struct nstat_detailed_counts *out_details,
                          int *out_gone) {
  struct nstat_tu_shadow *shad = (struct nstat_tu_shadow *)cookie;
  assert(shad->shad_magic == TU_SHADOW_MAGIC);
...
static errno_t nstat_userland_tu_copy_descriptor(nstat_provider_cookie_t cookie,
                                                 void *__sized_by(len) data,
                                                 __unused size_t len) {
  struct nstat_tu_shadow *shad = (struct nstat_tu_shadow *)cookie;
  assert(shad->shad_magic == TU_SHADOW_MAGIC);
...
static void nstat_userland_tu_release(__unused nstat_provider_cookie_t cookie) {
...
static bool check_reporting_for_user(nstat_provider_filter *filter, pid_t pid,
                                     pid_t epid, uuid_t *uuid, uuid_t *euuid) {
  bool retval = true;
...
static bool
nstat_userland_tcp_reporting_allowed(nstat_provider_cookie_t cookie,
                                     nstat_provider_filter *filter,
                                     __unused u_int64_t suppression_flags) {
  bool retval = true;
  struct nstat_tu_shadow *shad = (struct nstat_tu_shadow *)cookie;
  ...
  if ((filter->npf_flags & NSTAT_FILTER_IFNET_FLAGS) != 0) {
    ...
    if ((*shad->shad_getvals_fn)(shad->shad_provider_context, &ifflags, NULL,
                                 NULL, NULL, NULL)) {
  ...
  if ((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER) != 0) {
    nstat_tcp_descriptor
        tcp_desc; // Stack allocation - OK or pushing the limits too far?
    if ((*shad->shad_getvals_fn)(shad->shad_provider_context, NULL, NULL, NULL,
                                 NULL, &tcp_desc)) {
      retval = check_reporting_for_user(filter, (pid_t)tcp_desc.pid,
                                        (pid_t)tcp_desc.epid, &tcp_desc.uuid,
                                        &tcp_desc.euuid);
    } else {
...
static size_t nstat_userland_extensions(nstat_provider_cookie_t cookie,
                                        u_int32_t extension_id,
                                        void *__sized_by(len) buf, size_t len) {
  struct nstat_tu_shadow *shad = (struct nstat_tu_shadow *)cookie;
  assert(shad->shad_magic == TU_SHADOW_MAGIC);
  assert(shad->shad_live);
  assert(shad->shad_procdetails->pdet_magic == NSTAT_PROCDETAILS_MAGIC);

  return shad->shad_get_extension_fn(shad->shad_provider_context, extension_id,
                                     buf, len);
}

static bool
nstat_userland_udp_reporting_allowed(nstat_provider_cookie_t cookie,
                                     nstat_provider_filter *filter,
                                     __unused u_int64_t suppression_flags) {
  bool retval = true;
  struct nstat_tu_shadow *shad = (struct nstat_tu_shadow *)cookie;
  ...
  if ((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER) != 0) {
    nstat_udp_descriptor
        udp_desc; // Stack allocation - OK or pushing the limits too far?
    if ((*shad->shad_getvals_fn)(shad->shad_provider_context, NULL, NULL, NULL,
                                 NULL, &udp_desc)) {
      retval = check_reporting_for_user(filter, (pid_t)udp_desc.pid,
                                        (pid_t)udp_desc.epid, &udp_desc.uuid,
                                        &udp_desc.euuid);
    } else {
      retval = false; // No further information, so might as well give up now.
    }
...
static bool
nstat_userland_quic_reporting_allowed(nstat_provider_cookie_t cookie,
                                      nstat_provider_filter *filter,
                                      __unused u_int64_t suppression_flags) {
  bool retval = true;
  struct nstat_tu_shadow *shad = (struct nstat_tu_shadow *)cookie;
  ...
  if ((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER) != 0) {
    nstat_quic_descriptor
        quic_desc; // Stack allocation - OK or pushing the limits too far?
    if ((*shad->shad_getvals_fn)(shad->shad_provider_context, NULL, NULL, NULL,
                                 NULL, &quic_desc)) {
      retval = check_reporting_for_user(filter, (pid_t)quic_desc.pid,
                                        (pid_t)quic_desc.epid, &quic_desc.uuid,
                                        &quic_desc.euuid);
    } else {
      retval = false; // No further information, so might as well give up now.
    }
...
static errno_t nstat_userland_protocol_add_watcher(
    nstat_client *client, nstat_msg_add_all_srcs *req,
    nstat_provider_type_t nstat_provider_type, nstat_provider *nstat_provider,
  ...
  if (result == 0) {
    ...
    TAILQ_FOREACH(shad, &nstat_userprot_shad_head, shad_link) {
      ...
      if (shad->shad_provider == nstat_provider_type) {
        result = nstat_client_source_add(0, client, nstat_provider, shad,
                                         &shad->shad_locus);
        if (result != 0) {
...
static errno_t nstat_userland_tcp_add_watcher(nstat_client *client,
                                              nstat_msg_add_all_srcs *req) {
  return nstat_userland_protocol_add_watcher(
      client, req, NSTAT_PROVIDER_TCP_USERLAND, &nstat_userland_tcp_provider,
      &nstat_userland_tcp_watchers);
}

static errno_t nstat_userland_udp_add_watcher(nstat_client *client,
                                              nstat_msg_add_all_srcs *req) {
  return nstat_userland_protocol_add_watcher(
      client, req, NSTAT_PROVIDER_UDP_USERLAND, &nstat_userland_udp_provider,
      &nstat_userland_udp_watchers);
}

static errno_t nstat_userland_quic_add_watcher(nstat_client *client,
                                               nstat_msg_add_all_srcs *req) {
  return nstat_userland_protocol_add_watcher(
      client, req, NSTAT_PROVIDER_QUIC_USERLAND, &nstat_userland_quic_provider,
      &nstat_userland_quic_watchers);
}

static void nstat_userland_tcp_remove_watcher(__unused nstat_client *client) {
  OSDecrementAtomic(&nstat_userland_tcp_watchers);
}

static void nstat_userland_udp_remove_watcher(__unused nstat_client *client) {
  OSDecrementAtomic(&nstat_userland_udp_watchers);
}

static void nstat_userland_quic_remove_watcher(__unused nstat_client *client) {
  OSDecrementAtomic(&nstat_userland_quic_watchers);
}

static void nstat_init_userland_tcp_provider(void) {
  bzero(&nstat_userland_tcp_provider, sizeof(nstat_userland_tcp_provider));
  nstat_userland_tcp_provider.nstat_descriptor_length =
  ...
  nstat_userland_tcp_provider.nstat_watcher_add =
      nstat_userland_tcp_add_watcher;
  nstat_userland_tcp_provider.nstat_watcher_remove =
      nstat_userland_tcp_remove_watcher;
  nstat_userland_tcp_provider.nstat_copy_descriptor =
      nstat_userland_tu_copy_descriptor;
  nstat_userland_tcp_provider.nstat_reporting_allowed =
...
static void nstat_init_userland_udp_provider(void) {
  ...
  nstat_userland_udp_provider.nstat_descriptor_length =
  ...
  nstat_userland_udp_provider.nstat_watcher_add =
      nstat_userland_udp_add_watcher;
  nstat_userland_udp_provider.nstat_watcher_remove =
      nstat_userland_udp_remove_watcher;
  nstat_userland_udp_provider.nstat_copy_descriptor =
      nstat_userland_tu_copy_descriptor;
  nstat_userland_udp_provider.nstat_reporting_allowed =
...
static void nstat_init_userland_quic_provider(void) {
  ...
  nstat_userland_quic_provider.nstat_descriptor_length =
      sizeof(nstat_quic_descriptor);
  ...
  nstat_userland_quic_provider.nstat_watcher_remove =
      nstat_userland_quic_remove_watcher;
  nstat_userland_quic_provider.nstat_copy_descriptor =
      nstat_userland_tu_copy_descriptor;
  nstat_userland_quic_provider.nstat_reporting_allowed =
...
// Things get started with a call to netstats to say that there’s a new
// connection:
__private_extern__ nstat_userland_context ntstat_userland_stats_open(
    userland_stats_provider_context *ctx, int provider_id, u_int64_t properties,
  ...
  NSTAT_GLOBAL_COUNT_INCREMENT_WITH_MAX(nstat_global_tu_shad_current,
  ...
  for (client = nstat_clients; client; client = client->ntc_next) {
    if ((client->ntc_watching & (1 << provider_id)) != 0) {
      ...
      // Link to it.
      int result =
          nstat_client_source_add(0, client, provider, shad, &shad->shad_locus);
      if (result != 0) {
        // There should be some kind of statistics for failures like this.
        // <rdar://problem/31377195> The kernel ntstat component should keep
        // some internal counters reflecting operational state for eventual AWD
        // reporting
      }
    }
  }
...
ntstat_userland_stats_close(nstat_userland_context nstat_ctx) {
  ...
  TAILQ_FOREACH_SAFE(src, &shad->shad_locus.ntl_src_queue, nts_locus_link,
                     tmpsrc) {
...
static void ntstat_userland_stats_event_locked(struct nstat_tu_shadow *shad,
                                               uint64_t event) {
  nstat_client *client;
  nstat_src *src;
  nstat_provider_id_t provider_id;
...
__private_extern__ void
ntstat_userland_stats_event(nstat_userland_context nstat_ctx, uint64_t event) {
  // This will need refinement for when we do genuine stats filtering
  // See <rdar://problem/23022832> NetworkStatistics should provide opt-in
  // notifications For now it deals only with events that potentially cause any
  // traditional netstat sources to be closed
...
__private_extern__ void nstats_userland_stats_defunct_for_process(int pid) {
...
errno_t nstat_userland_mark_rnf_override(uuid_t target_fuuid,
                                         bool rnf_override) {
  // Note that this can be called multiple times for the same process
  struct nstat_tu_shadow *shad;
  uuid_t fuuid;
  ...
  NSTAT_LOCK_EXCLUSIVE();
  // We set the fallback state regardles of watchers as there may be future ones
  // that need to know
  TAILQ_FOREACH(shad, &nstat_userprot_shad_head, shad_link) {
    assert(shad->shad_magic == TU_SHADOW_MAGIC);
    assert(shad->shad_procdetails->pdet_magic == NSTAT_PROCDETAILS_MAGIC);
    if (shad->shad_get_extension_fn(shad->shad_provider_context,
                                    NSTAT_EXTENDED_UPDATE_TYPE_FUUID, fuuid,
                                    sizeof(fuuid))) {
  ...
  if (shad) {
    if (shad->shad_procdetails->pdet_pid != proc_selfpid()) {
      ...
      ntstat_userland_stats_event_locked(
          shad, rnf_override ? NSTAT_EVENT_SRC_ENTER_CELLFALLBACK
...
static tailq_head_generic_shadow nstat_gshad_head =
    TAILQ_HEAD_INITIALIZER(nstat_gshad_head);

static inline void nstat_retain_gshad(struct nstat_generic_shadow *gshad) {
  assert(gshad->gshad_magic == NSTAT_GENERIC_SHADOW_MAGIC);

  OSIncrementAtomic(&gshad->gshad_refcnt);
}

static void nstat_release_gshad(struct nstat_generic_shadow *gshad) {
  assert(gshad->gshad_magic == NSTAT_GENERIC_SHADOW_MAGIC);
...
static errno_t
nstat_generic_provider_lookup(__unused const void *data,
                              __unused u_int32_t length,
                              __unused nstat_provider_cookie_t *out_cookie) {
  // Looking up a specific connection is not supported
  return ENOTSUP;
...
static int
nstat_generic_provider_gone(__unused nstat_provider_cookie_t cookie) {
  // Returns non-zero if the source has gone.
  // We don't keep a source hanging around, so the answer is always 0
  return 0;
}

static errno_t nstat_generic_provider_counts(nstat_provider_cookie_t cookie,
                                             struct nstat_counts *out_counts,
                                             int *out_gone) {
  ...
  bool result = (*gshad->gshad_getvals_fn)(gshad->gshad_provider_context, NULL,
                                           out_counts, NULL, NULL);
...
static errno_t
nstat_generic_provider_details(nstat_provider_cookie_t cookie,
...
static errno_t
nstat_generic_provider_copy_descriptor(nstat_provider_cookie_t cookie,
                                       void *__sized_by(len) data,
                                       __unused size_t len) {
  struct nstat_generic_shadow *gshad = (struct nstat_generic_shadow *)cookie;
  assert(gshad->gshad_magic == NSTAT_GENERIC_SHADOW_MAGIC);
...
static void
nstat_generic_provider_release(__unused nstat_provider_cookie_t cookie) {
  // Called when a nstat_src is detached.
  struct nstat_generic_shadow *gshad = (struct nstat_generic_shadow *)cookie;
...
static bool
nstat_generic_provider_reporting_allowed(nstat_provider_cookie_t cookie,
                                         nstat_provider_filter *filter,
                                         u_int64_t suppression_flags) {
  struct nstat_generic_shadow *gshad = (struct nstat_generic_shadow *)cookie;
  ...
  // If a provider doesn't support flags, a client shouldn't attempt to use
  // filtering
  if ((filter->npf_flags & NSTAT_FILTER_IFNET_AND_CONN_FLAGS) != 0) {
    u_int32_t ifflags = NSTAT_IFNET_IS_UNKNOWN_TYPE;

    if ((*gshad->gshad_getvals_fn)(gshad->gshad_provider_context, &ifflags,
                                   NULL, NULL, NULL)) {
      if ((filter->npf_flags & ifflags) == 0) {
        return false;
      }
  ...
  if ((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER) != 0) {
    ...
    // Check details that we have readily to hand before asking the provider for
    // descriptor items
    if (((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER_BY_PID) != 0) &&
    ...
    if (((filter->npf_flags & NSTAT_FILTER_SPECIFIC_USER_BY_UUID) != 0) &&
        (memcmp(filter->npf_uuid, &procdetails->pdet_uuid,
                sizeof(filter->npf_uuid)) == 0)) {
      return true;
    }
    if ((filter->npf_flags & (NSTAT_FILTER_SPECIFIC_USER_BY_EPID |
                              NSTAT_FILTER_SPECIFIC_USER_BY_EUUID)) != 0) {
      nstat_udp_descriptor
          udp_desc; // Stack allocation - OK or pushing the limits too far?
      switch (gshad->gshad_provider) {
      case NSTAT_PROVIDER_CONN_USERLAND:
        // Filtering by effective uuid or effective pid is currently not
        // supported
        filter->npf_flags &= ~((uint64_t)(NSTAT_FILTER_SPECIFIC_USER_BY_EPID |
                                          NSTAT_FILTER_SPECIFIC_USER_BY_EUUID));
        NSTAT_LOG("attempt to filter conn provider by effective pid/uuid, not "
                  "supported");
        return true;

      case NSTAT_PROVIDER_UDP_SUBFLOW:
        if ((*gshad->gshad_getvals_fn)(gshad->gshad_provider_context, NULL,
                                       NULL, NULL, &udp_desc)) {
          if (check_reporting_for_user(
                  filter, procdetails->pdet_pid, (pid_t)udp_desc.epid,
...
static size_t nstat_generic_extensions(nstat_provider_cookie_t cookie,
                                       u_int32_t extension_id,
                                       void *__sized_by(len) buf, size_t len) {
  struct nstat_generic_shadow *gshad = (struct nstat_generic_shadow *)cookie;
  assert(gshad->gshad_magic == NSTAT_GENERIC_SHADOW_MAGIC);
  ...
  return gshad->gshad_getextensions_fn(gshad->gshad_provider_context,
                                       extension_id, buf, len);
}

static errno_t nstat_generic_provider_add_watcher(nstat_client *client,
                                                  nstat_msg_add_all_srcs *req) {
  errno_t result;
    ...
    TAILQ_FOREACH(gshad, &nstat_gshad_head, gshad_link) {
      ...
      if (gshad->gshad_provider == provider_id) {
        if (filter->npf_flags & NSTAT_FILTER_INITIAL_PROPERTIES) {
          u_int64_t npf_flags =
              filter->npf_flags & NSTAT_FILTER_IFNET_AND_CONN_FLAGS;
          if ((npf_flags != 0) &&
              ((npf_flags & gshad->gshad_properties) == 0)) {
            // Skip this one
            // Note - no filtering by pid or UUID supported at this point, for
            // simplicity
            continue;
          }
        }
        nstat_retain_gshad(gshad);
        result = nstat_client_source_add(0, client, provider, gshad,
                                         &gshad->gshad_locus);
        if (result != 0) {
          NSTAT_LOG_ERROR("nstat_client_source_add returned %d for "
                          "provider type: %d",
...
static void nstat_userland_conn_remove_watcher(__unused nstat_client *client) {
  OSDecrementAtomic(
      &nstat_generic_provider_watchers[NSTAT_PROVIDER_CONN_USERLAND]);
}

static void nstat_udp_subflow_remove_watcher(__unused nstat_client *client) {
  OSDecrementAtomic(
      &nstat_generic_provider_watchers[NSTAT_PROVIDER_UDP_SUBFLOW]);
}

static void nstat_init_userland_conn_provider(void) {
  bzero(&nstat_userland_conn_provider, sizeof(nstat_userland_conn_provider));
  nstat_userland_conn_provider.nstat_descriptor_length =
      sizeof(nstat_connection_descriptor);
  ...
  nstat_userland_conn_provider.nstat_watcher_add =
      nstat_generic_provider_add_watcher;
  nstat_userland_conn_provider.nstat_watcher_remove =
      nstat_userland_conn_remove_watcher;
  nstat_userland_conn_provider.nstat_copy_descriptor =
      nstat_generic_provider_copy_descriptor;
  nstat_userland_conn_provider.nstat_reporting_allowed =
...
static void nstat_init_udp_subflow_provider(void) {
  ...
  nstat_udp_subflow_provider.nstat_descriptor_length =
      sizeof(nstat_udp_descriptor);
  ...
  nstat_udp_subflow_provider.nstat_watcher_add =
      nstat_generic_provider_add_watcher;
  ...
  nstat_udp_subflow_provider.nstat_copy_descriptor =
      nstat_generic_provider_copy_descriptor;
  nstat_udp_subflow_provider.nstat_reporting_allowed =
      nstat_generic_provider_reporting_allowed;
...
// Things get started with a call from the provider to netstats to say that
// there’s a new source
__private_extern__ nstat_context nstat_provider_stats_open(
    nstat_provider_context ctx, int provider_id, u_int64_t properties,
  ...
  NSTAT_GLOBAL_COUNT_INCREMENT_WITH_MAX(nstat_global_gshad_current,
  ...
  for (client = nstat_clients; client; client = client->ntc_next) {
    if ((client->ntc_watching & (1 << provider_id)) != 0) {
      ...
      u_int64_t npf_flags =
          client->ntc_provider_filters[provider->nstat_provider_id].npf_flags;
      ...
      nstat_retain_gshad(gshad);
      int result = nstat_client_source_add(0, client, provider, gshad,
                                           &gshad->gshad_locus);
      if (result != 0) {
        // There should be some kind of statistics for failures like this.
        // <rdar://problem/31377195> The kernel ntstat component should keep
...
// When the source is closed, netstats will make one last call on the request
// functions to retrieve final values
__private_extern__ void nstat_provider_stats_close(nstat_context nstat_ctx) {
  tailq_head_nstat_src dead_list;
  nstat_src *src;
  nstat_src *tmpsrc;
  ...
  if (gshad->gshad_magic != NSTAT_GENERIC_SHADOW_MAGIC) {
    NSTAT_LOG_ERROR("called with incorrect shadow magic 0x%x",
                    gshad->gshad_magic);
  }
  ...
  TAILQ_FOREACH_SAFE(src, &gshad->gshad_locus.ntl_src_queue, nts_locus_link,
                     tmpsrc) {
...
void nstat_provider_stats_event(__unused nstat_context nstat_ctx,
                                __unused uint64_t event) {
  nstat_src *src;
  struct nstat_generic_shadow *gshad = (struct nstat_generic_shadow *)nstat_ctx;
  ...
  TAILQ_FOREACH(src, &gshad->gshad_locus.ntl_src_queue, nts_locus_link) {
    ...
    if (((client->ntc_watching & (1 << provider_id)) != 0) &&
        (((client->ntc_provider_filters[provider_id].npf_events & event) !=
          0) ||
         ((client->ntc_provider_filters[provider_id].npf_flags &
           NSTAT_FILTER_SUPPRESS_BORING_FLAGS) != 0))) {
...
static errno_t nstat_ifnet_lookup(const void *data, u_int32_t length,
                                  nstat_provider_cookie_t *out_cookie) {
  ...
  cookie = kalloc_type(struct nstat_ifnet_cookie, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static int nstat_ifnet_gone(nstat_provider_cookie_t cookie) {
  struct ifnet *ifp;
  struct nstat_ifnet_cookie *ifcookie = (struct nstat_ifnet_cookie *)cookie;
...
static errno_t nstat_ifnet_counts(nstat_provider_cookie_t cookie,
                                  struct nstat_counts *out_counts,
                                  int *out_gone) {
  struct nstat_ifnet_cookie *ifcookie = (struct nstat_ifnet_cookie *)cookie;
...
static void nstat_ifnet_release(nstat_provider_cookie_t cookie) {
...
static void nstat_ifnet_copy_link_status(struct ifnet *ifp,
  ...
  if (ifp->if_type == IFT_CELLULAR) {
    ...
    if (if_wifi_sr->valid_bitmask & IF_WIFI_CONFIG_FREQUENCY_VALID) {
      ...
      if (if_wifi_sr->config_frequency == IF_WIFI_CONFIG_FREQUENCY_2_4_GHZ) {
        wifi_status->config_frequency =
            NSTAT_IFNET_DESC_WIFI_CONFIG_FREQUENCY_2_4_GHZ;
      } else if (if_wifi_sr->config_frequency ==
...
#define NSTAT_LIM_CONN_TIMEOUT_PERCENT_THRESHOLD                               \
  (10 << 10) /* 10 percent connection timeouts */
#define NSTAT_LIM_PACKET_LOSS_PERCENT_THRESHOLD                                \
  (2 << 10) /* 2 percent packet loss rate */

static boolean_t nstat_lim_activity_check(struct if_lim_perf_stat *st) {
  ...
  st->lim_packet_loss_percent =
      ((st->lim_total_retxpkts << 10) / st->lim_total_txpkts) * 100;

  st->lim_packet_ooo_percent =
      ((st->lim_total_oopkts << 10) / st->lim_total_rxpkts) * 100;

  st->lim_conn_timeout_percent =
      ((st->lim_conn_timeouts << 10) / st->lim_conn_attempts) * 100;
  ...
  if (st->lim_bk_txpkts >= nstat_lim_min_tx_pkts &&
      st->lim_packet_loss_percent >= NSTAT_LIM_PACKET_LOSS_PERCENT_THRESHOLD) {
    st->lim_ul_detected = 1;
  }
  return TRUE;
}
...
static void nstat_ifnet_report_lim_stats(void) {
  ...
  if ((u_int32_t)(uptime - nstat_lim_last_report_time) < nstat_lim_interval) {
    return;
  }
  ...
  TAILQ_FOREACH(ifp, &ifnet_head, if_link) {
    ...
    err = ifnet_get_netsignature(ifp, AF_INET, (u_int8_t *)&st->ifnet_siglen,
                                 NULL, st->ifnet_signature);
    if (err != 0) {
      err = ifnet_get_netsignature(ifp, AF_INET6, (u_int8_t *)&st->ifnet_siglen,
                                   NULL, st->ifnet_signature);
      if (err != 0) {
        continue;
      }
    ...
    bcopy(&ifp->if_lim_stat, &st->lim_stat, sizeof(st->lim_stat));
...
static errno_t nstat_ifnet_copy_descriptor(nstat_provider_cookie_t cookie,
                                           void *__sized_by(len) data,
                                           size_t len) {
  ...
  if (ifp->if_desc.ifd_len < sizeof(desc->description)) {
    memcpy(desc->description, ifp->if_desc.ifd_desc, sizeof(desc->description));
  }
...
static bool nstat_ifnet_cookie_equal(nstat_provider_cookie_t cookie1,
                                     nstat_provider_cookie_t cookie2) {
...
__private_extern__ void nstat_ifnet_threshold_reached(unsigned int ifindex) {
  nstat_client *client;
  nstat_src *src;
  struct ifnet *ifp;
  struct nstat_ifnet_cookie *ifcookie;
...
#pragma mark-- Sysinfo --
static void nstat_set_keyval_scalar(nstat_sysinfo_keyval *kv, int key,
                                    u_int32_t val) {
...
static void nstat_set_keyval_u64_scalar(nstat_sysinfo_keyval *kv, int key,
                                        u_int64_t val) {
  kv->nstat_sysinfo_key = key;
  kv->nstat_sysinfo_flags = NSTAT_SYSINFO_FLAG_SCALAR;
  kv->u.nstat_sysinfo_scalar = val;
...
static void nstat_set_keyval_string(nstat_sysinfo_keyval *kv, int key,
                                    u_int8_t *__counted_by(len) buf,
                                    u_int32_t len) {
  kv->nstat_sysinfo_key = key;
...
static void nstat_sysinfo_send_data_internal(nstat_client *client,
  ...
  syscnt =
      (nstat_msg_sysinfo_counts *)kalloc_data(allocsize, Z_WAITOK | Z_ZERO);
  if (syscnt == NULL) {
    return;
  }
  ...
  case NSTAT_SYSINFO_LIM_STATS: {
    nstat_set_keyval_string(ntstat_sysinfo_next(&iter),
                            ...
                            min(data->u.lim_stats.ifnet_siglen,
                                NSTAT_SYSINFO_KEYVAL_STRING_MAXSIZE));
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_LIM_IFNET_DL_MAX_BANDWIDTH,
        data->u.lim_stats.lim_stat.lim_dl_max_bandwidth);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_LIM_IFNET_UL_MAX_BANDWIDTH,
        data->u.lim_stats.lim_stat.lim_ul_max_bandwidth);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_LIM_IFNET_PACKET_LOSS_PERCENT,
        data->u.lim_stats.lim_stat.lim_packet_loss_percent);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_LIM_IFNET_PACKET_OOO_PERCENT,
  ...
  case NSTAT_SYSINFO_NET_API_STATS: {
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_IF_FLTR_ATTACH,
        data->u.net_api_stats.net_api_stats.nas_iflt_attach_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_IF_FLTR_ATTACH_OS,
        data->u.net_api_stats.net_api_stats.nas_iflt_attach_os_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_IP_FLTR_ADD,
        data->u.net_api_stats.net_api_stats.nas_ipf_add_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_IP_FLTR_ADD_OS,
        data->u.net_api_stats.net_api_stats.nas_ipf_add_os_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_FLTR_ATTACH,
        data->u.net_api_stats.net_api_stats.nas_sfltr_register_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_FLTR_ATTACH_OS,
        data->u.net_api_stats.net_api_stats.nas_sfltr_register_os_total);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_ALLOC_TOTAL,
        data->u.net_api_stats.net_api_stats.nas_socket_alloc_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_ALLOC_KERNEL,
        data->u.net_api_stats.net_api_stats.nas_socket_in_kernel_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_ALLOC_KERNEL_OS,
        data->u.net_api_stats.net_api_stats.nas_socket_in_kernel_os_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_NECP_CLIENTUUID,
        data->u.net_api_stats.net_api_stats.nas_socket_necp_clientuuid_total);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_LOCAL,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_local_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_ROUTE,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_route_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_INET,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_inet_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_INET6,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_inet6_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_SYSTEM,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_system_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_MULTIPATH,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_multipath_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_KEY,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_key_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_NDRV,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_ndrv_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_DOMAIN_OTHER,
        data->u.net_api_stats.net_api_stats.nas_socket_domain_other_total);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET_STREAM,
        data->u.net_api_stats.net_api_stats.nas_socket_inet_stream_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET_DGRAM,
        data->u.net_api_stats.net_api_stats.nas_socket_inet_dgram_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET_DGRAM_CONNECTED,
        data->u.net_api_stats.net_api_stats.nas_socket_inet_dgram_connected);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET_DGRAM_DNS,
        data->u.net_api_stats.net_api_stats.nas_socket_inet_dgram_dns);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET_DGRAM_NO_DATA,
        data->u.net_api_stats.net_api_stats.nas_socket_inet_dgram_no_data);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET6_STREAM,
        data->u.net_api_stats.net_api_stats.nas_socket_inet6_stream_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET6_DGRAM,
    ...
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET6_DGRAM_DNS,
        data->u.net_api_stats.net_api_stats.nas_socket_inet6_dgram_dns);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET6_DGRAM_NO_DATA,
        data->u.net_api_stats.net_api_stats.nas_socket_inet6_dgram_no_data);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET_MCAST_JOIN,
        data->u.net_api_stats.net_api_stats.nas_socket_mcast_join_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_SOCK_INET_MCAST_JOIN_OS,
        data->u.net_api_stats.net_api_stats.nas_socket_mcast_join_os_total);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_NEXUS_FLOW_INET_STREAM,
        data->u.net_api_stats.net_api_stats.nas_nx_flow_inet_stream_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_NEXUS_FLOW_INET_DATAGRAM,
        data->u.net_api_stats.net_api_stats.nas_nx_flow_inet_dgram_total);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_NEXUS_FLOW_INET6_STREAM,
        data->u.net_api_stats.net_api_stats.nas_nx_flow_inet6_stream_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_NEXUS_FLOW_INET6_DATAGRAM,
        data->u.net_api_stats.net_api_stats.nas_nx_flow_inet6_dgram_total);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_IFNET_ALLOC,
        data->u.net_api_stats.net_api_stats.nas_ifnet_alloc_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_IFNET_ALLOC_OS,
        data->u.net_api_stats.net_api_stats.nas_ifnet_alloc_os_total);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_PF_ADDRULE,
        data->u.net_api_stats.net_api_stats.nas_pf_addrule_total);
    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_PF_ADDRULE_OS,
        data->u.net_api_stats.net_api_stats.nas_pf_addrule_os);

    nstat_set_keyval_u64_scalar(
        ntstat_sysinfo_next(&iter), NSTAT_SYSINFO_API_VMNET_START,
  ...
  if (syscnt != NULL) {
    VERIFY(iter.i > 0);
    countsize = offsetof(nstat_sysinfo_counts, nstat_sysinfo_keyvals[iter.i]);
    finalsize += countsize;
    ...
    result = ctl_enqueuedata(client->ntc_kctl, client->ntc_unit, syscnt,
                             finalsize, CTL_DATA_EOR);
    if (result != 0) {
...
__private_extern__ void nstat_sysinfo_send_data(nstat_sysinfo_data *data) {
  if (nstat_debug != 0) {
...
static void nstat_sysinfo_generate_report(void) {
  tcp_report_stats();
...
static void nstat_net_api_report_stats(void) {
   ...
   *   much with time
   */
#define STATCOPY(f) (st->net_api_stats.f = net_api_stats.f)
#define STATDIFF(f)                                                            \
...
#undef STATDIFF
   ...
   */
  memcpy(&net_api_stats_before, &net_api_stats, sizeof(struct net_api_stats));
...
static errno_t nstat_client_connect(kern_ctl_ref kctl, struct sockaddr_ctl *sac,
                                    void **uinfo);
static errno_t nstat_client_disconnect(kern_ctl_ref kctl, u_int32_t unit,
                                       void *uinfo);
static errno_t nstat_client_send(kern_ctl_ref kctl, u_int32_t unit, void *uinfo,
                                 mbuf_t m, int flags);

static errno_t nstat_enqueue_success(uint64_t context, nstat_client *client,
                                     u_int16_t flags) {
...
static errno_t nstat_client_send_event(nstat_client *client, nstat_src *src,
  ...
  if (nstat_client_reporting_allowed(client, src, 0)) {
    if ((client->ntc_flags & NSTAT_FLAG_SUPPORTS_DETAILS) != 0) {
      ...
      if (nstat_debug != 0) {
        NSTAT_LOG_ERROR(
            "nstat_client_send_event() used when updates not supported");
      }
    }
  }
  return result;
...
static errno_t nstat_client_send_goodbye(nstat_client *client, nstat_src *src) {
  ...
  if (nstat_client_reporting_allowed(
          client, src,
          (src->nts_reported) ? NSTAT_FILTER_SUPPRESS_BORING_CLOSE : 0)) {
    hdr_flags = 0;
    if ((client->ntc_flags & NSTAT_FLAG_SUPPORTS_DETAILS) != 0) {
      result = nstat_client_send_details(client, src, 0, 0,
                                         NSTAT_MSG_HDR_FLAG_CLOSING, NULL);
      if (result != 0) {
      ...
      result = nstat_client_send_update(client, src, 0, 0,
                                        NSTAT_MSG_HDR_FLAG_CLOSING, NULL);
      if (result != 0) {
      ...
      result = nstat_client_send_counts(client, src, 0,
                                        NSTAT_MSG_HDR_FLAG_CLOSING, NULL);
      if (result != 0) {
      ...
      result = nstat_client_send_description(client, src, 0,
                                             NSTAT_MSG_HDR_FLAG_CLOSING);
      if (result != 0) {
...
static errno_t nstat_flush_accumulated_msgs(nstat_client *client) {
  errno_t result = 0;
  if (client->ntc_accumulated != NULL &&
      mbuf_len(client->ntc_accumulated) > 0) {
    mbuf_pkthdr_setlen(client->ntc_accumulated,
                       mbuf_len(client->ntc_accumulated));
    result = ctl_enqueuembuf(client->ntc_kctl, client->ntc_unit,
                             client->ntc_accumulated, CTL_DATA_EOR);
    if (result != 0) {
...
static errno_t nstat_accumulate_msg(nstat_client *client,
                                    uint8_t *__sized_by(length) msg,
                                    size_t length) {
  assert(length <= MAX_NSTAT_MSG_HDR_LENGTH);

  if (client->ntc_accumulated &&
      mbuf_trailingspace(client->ntc_accumulated) < length) {
  ...
  if (client->ntc_accumulated == NULL) {
    unsigned int one = 1;
    if (mbuf_allocpacket(MBUF_DONTWAIT, NSTAT_MAX_MSG_SIZE, &one,
                         &client->ntc_accumulated) != 0) {
      if (nstat_debug != 0) {
  ...
  if (result == 0) {
    result = mbuf_copyback(client->ntc_accumulated,
                           mbuf_len(client->ntc_accumulated), length, msg,
                           MBUF_DONTWAIT);
  }
  ...
  if (result != 0) {
    ...
    result = ctl_enqueuedata(client->ntc_kctl, client->ntc_unit, msg, length,
                             CTL_DATA_EOR);
  }
...
static void nstat_idle_check(__unused thread_call_param_t p0,
  ...
  // NSTAT_PROVIDER_IFNET, which will remove the need to scan all possible
  // sources. In the meantime, they should be a minority interest and it is
  // expected that most clients will be skipped in the sequence below
  for (client = nstat_clients; client; client = client->ntc_next) {
    if (((client->ntc_watching & (1 << NSTAT_PROVIDER_ROUTE)) != 0) ||
  ...
  if (nstat_clients) {
    ...
    thread_call_func_delayed((thread_call_func_t)nstat_idle_check, NULL,
                             nstat_idle_time);
  }
...
static void nstat_client_register(void) {
  ...
  strlcpy(nstat_control.ctl_name, NET_STAT_CONTROL_NAME,
          sizeof(nstat_control.ctl_name));
...
static void nstat_client_cleanup_source(nstat_client *client,
...
static bool nstat_client_reporting_allowed(nstat_client *client, nstat_src *src,
  ...
  return src->nts_provider->nstat_reporting_allowed(
      src->nts_cookie,
      &client->ntc_provider_filters[src->nts_provider->nstat_provider_id],
      suppression_flags);
}

static errno_t nstat_client_connect(kern_ctl_ref kctl, struct sockaddr_ctl *sac,
                                    void **uinfo) {
  nstat_client *client = kalloc_type(nstat_client, Z_WAITOK | Z_ZERO);
  if (client == NULL) {
  ...
  *uinfo = client;
  ...
  NSTAT_GLOBAL_COUNT_INCREMENT_WITH_MAX(nstat_global_client_current,
                                        nstat_global_client_max);
#if NSTAT_TRACE_ENABLED
  client->ntc_trace = (nstat_cyclic_trace *)kalloc_data(
      sizeof(nstat_cyclic_trace), Z_WAITOK | Z_ZERO);
#endif
  NSTAT_LOCK_EXCLUSIVE();
...
static errno_t nstat_client_disconnect(__unused kern_ctl_ref kctl,
                                       __unused u_int32_t unit, void *uinfo) {
  u_int32_t watching;
  ...
  for (clientpp = &nstat_clients; *clientpp;
       clientpp = &(*clientpp)->ntc_next) {
    if (*clientpp == client) {
  ...
  for (provider = nstat_providers; provider && watching;
       provider = provider->next) {
...
static nstat_src_ref_t nstat_client_next_src_ref(nstat_client *client) {
...
static errno_t nstat_client_send_counts(nstat_client *client, nstat_src *src,
  ...
  if (src->nts_provider->nstat_counts(src->nts_cookie, &counts.counts, gone) ==
      0) {
    if ((src->nts_filter & NSTAT_FILTER_NOZEROBYTES) &&
        counts.counts.nstat_rxbytes == 0 && counts.counts.nstat_txbytes == 0) {
      result = EAGAIN;
    } else {
      result = ctl_enqueuedata(client->ntc_kctl, client->ntc_unit, &counts,
...
static errno_t nstat_client_append_counts(nstat_client *client, nstat_src *src,
                                          int *gone) {
  /* Some providers may not have any counts to send */
  ...
  errno_t result = 0;
  result =
      src->nts_provider->nstat_counts(src->nts_cookie, &counts.counts, gone);
  if (result != 0) {
    return result;
  }
...
static int nstat_client_send_description(nstat_client *client, nstat_src *src,
  ...
  mbuf_ref_t msg;
  unsigned int one = 1;
  size_t size = offsetof(nstat_msg_src_description, data) +
  ...
  errno_t result = src->nts_provider->nstat_copy_descriptor(
      src->nts_cookie, desc_data_ptr,
      src->nts_provider->nstat_descriptor_length);
  ...
  result =
      ctl_enqueuembuf(client->ntc_kctl, client->ntc_unit, msg, CTL_DATA_EOR);
  if (result != 0) {
...
static errno_t nstat_client_append_description(nstat_client *client,
                                               nstat_src *src) {
  size_t size = offsetof(nstat_msg_src_description, data) +
                src->nts_provider->nstat_descriptor_length;
  ...
  u_int64_t
      buffer[size / sizeof(u_int64_t) + 1]; // u_int64_t to ensure alignment
  bzero(buffer, size);
  ...
  // Query the provider for the provider specific bits
  uint8_t *desc_data_ptr = nstat_get_data(desc);
  result = src->nts_provider->nstat_copy_descriptor(
      src->nts_cookie, desc_data_ptr,
      src->nts_provider->nstat_descriptor_length);
  if (result != 0) {
    return result;
...
static uint64_t nstat_extension_flags_for_source(nstat_client *client,
                                                 nstat_src *src) {
...
static int nstat_client_send_update(nstat_client *client, nstat_src *src,
  ...
  if ((extension_mask != 0) &&
      (src->nts_provider->nstat_copy_extension != NULL)) {
    uint32_t extension_id = 0;
    for (extension_id = NSTAT_EXTENDED_UPDATE_TYPE_MIN;
         extension_id <= NSTAT_EXTENDED_UPDATE_TYPE_MAX; extension_id++) {
      if ((extension_mask & (1ull << extension_id)) != 0) {
        size_t extension_size = src->nts_provider->nstat_copy_extension(
            src->nts_cookie, extension_id, NULL, 0);
        if (extension_size == 0) {
    ...
    size += total_extension_size +
            (sizeof(nstat_msg_src_extended_item_hdr) * num_extensions);
  }
  assert(size <= MAX_NSTAT_MSG_HDR_LENGTH);
  ...
  errno_t result = 0;
  if (src->nts_provider->nstat_descriptor_length != 0 &&
      src->nts_provider->nstat_copy_descriptor) {
    // Query the provider for the provider specific bits
  ...
  if (num_extensions > 0) {
    nstat_msg_src_extended_item_hdr *p_extension_hdr =
        (nstat_msg_src_extended_item_hdr *)mtodo(
            msg, sizeof(nstat_msg_src_update_hdr) +
                     src->nts_provider->nstat_descriptor_length);
    uint32_t extension_id = 0;

    bzero(p_extension_hdr,
    ...
    for (extension_id = NSTAT_EXTENDED_UPDATE_TYPE_MIN;
      ...
      if ((extension_mask & (1ull << extension_id)) != 0) {
        ...
        size_t extension_size = src->nts_provider->nstat_copy_extension(
            src->nts_cookie, extension_id, buf, total_extension_size);
        if ((extension_size == 0) || (extension_size > total_extension_size)) {
          // Something has gone wrong. Instead of attempting to wind back the
          // excess buffer space, mark it as unused
          p_extension_hdr->type = NSTAT_EXTENDED_UPDATE_TYPE_UNKNOWN;
          p_extension_hdr->length =
              total_extension_size +
              (sizeof(nstat_msg_src_extended_item_hdr) * (num_extensions - 1));
          break;
        } else {
          // The extension may be of any size alignment, reported as such in the
          // extension header, but we pad to ensure that whatever comes next is
          // suitably aligned
          ...
          p_extension_hdr =
              (nstat_msg_src_extended_item_hdr *)(void *)((char *)buf +
                                                          extension_size);
          num_extensions--;
        }
      }
  ...
  if (src->nts_provider->nstat_counts) {
    result =
        src->nts_provider->nstat_counts(src->nts_cookie, &desc->counts, gone);
    if (result == 0) {
      if ((src->nts_filter & NSTAT_FILTER_NOZEROBYTES) ==
              NSTAT_FILTER_NOZEROBYTES &&
        ...
        result = ctl_enqueuembuf(client->ntc_kctl, client->ntc_unit, msg,
                                 CTL_DATA_EOR);
      }
    }
  }
...
static int nstat_client_send_details(nstat_client *client, nstat_src *src,
  ...
  if (num_extensions > 0) {
    nstat_msg_src_extended_item_hdr *p_extension_hdr =
        (nstat_msg_src_extended_item_hdr *)mtodo(
            msg, sizeof(nstat_msg_src_details_hdr) +
                     src->nts_provider->nstat_descriptor_length);
    uint32_t extension_id = 0;
  ...
  if (src->nts_provider->nstat_details) {
    result = src->nts_provider->nstat_details(src->nts_cookie,
                                              &desc->detailed_counts, gone);
    if (result == 0) {
      if ((src->nts_filter & NSTAT_FILTER_NOZEROBYTES) ==
              NSTAT_FILTER_NOZEROBYTES &&
...
static errno_t nstat_client_append_update(nstat_client *client, nstat_src *src,
                                          int *gone) {
  if ((src->nts_provider->nstat_descriptor_length == 0 ||
       src->nts_provider->nstat_copy_descriptor == NULL) &&
      src->nts_provider->nstat_counts == NULL) {
...
static errno_t nstat_client_append_details(nstat_client *client, nstat_src *src,
                                           int *gone) {
  if ((src->nts_provider->nstat_descriptor_length == 0 ||
       src->nts_provider->nstat_copy_descriptor == NULL) &&
      src->nts_provider->nstat_details == NULL) {
  ...
  if (num_extensions > 0) {
    nstat_msg_src_extended_item_hdr *p_extension_hdr =
        (nstat_msg_src_extended_item_hdr
             *)(void *)((char *)buffer + sizeof(nstat_msg_src_details_hdr) +
                        src->nts_provider->nstat_descriptor_length);
    uint32_t extension_id = 0;
    bzero(p_extension_hdr,
          total_extension_size +
...
static errno_t nstat_client_handle_add_request(nstat_client *client, mbuf_t m) {
  errno_t result;
  ...
  ssize_t paramlength =
      mbuf_pkthdr_len(m) - offsetof(nstat_msg_add_src_req, param);
  ...
  if (mbuf_pkthdr_len(m) > mbuf_len(m)) {
    ...
    result = mbuf_copydata(m, offsetof(nstat_msg_add_src_req, param),
                           paramlength, data);
    if (result == 0) {
      result = nstat_lookup_entry(req->provider, data, paramlength, &provider,
                                  &cookie);
    }
    kfree_data(data, paramlength);
  } else {
    uint8_t *req_param_ptr = nstat_get_data(req);
    result = nstat_lookup_entry(req->provider, req_param_ptr, paramlength,
                                &provider, &cookie);
  }
  ...
  NSTAT_LOCK_EXCLUSIVE();
  result =
      nstat_client_source_add(req->hdr.context, client, provider, cookie, NULL);
  NSTAT_UNLOCK_EXCLUSIVE();
  ...
  os_atomic_or(&client->ntc_added_src, (1 << provider->nstat_provider_id),
               relaxed);
...
static errno_t nstat_set_provider_filter(nstat_client *client,
  ...
  u_int32_t prev_ntc_watching =
      os_atomic_or_orig(&client->ntc_watching, (1 << provider_id), relaxed);
  ...
  uuid_copy(client->ntc_provider_filters[provider_id].npf_uuid,
            req->target_uuid);
  ...
  // but is a convenient workaround that avoids ntstat.h changes that would
  // require rebuild of all clients Extensions give away additional privacy
  // information and are subject to unconditional privilege check, unconstrained
  // by the value of nstat_privcheck
  if (priv_check_cred(kauth_cred_get(), PRIV_NET_PRIVILEGED_NETWORK_STATISTICS,
                      0) == 0) {
    client->ntc_provider_filters[provider_id].npf_extensions =
        (req->filter >> NSTAT_FILTER_ALLOWED_EXTENSIONS_SHIFT) &
        NSTAT_EXTENDED_UPDATE_FLAG_MASK;
  }
  return 0;
}

static errno_t nstat_client_handle_add_all(nstat_client *client, mbuf_t m) {
  errno_t result = 0;
  ...
  // Traditionally the nstat_privcheck value allowed for easy access to ntstat
  // on the Mac. Keep backwards compatibility while being more stringent with
  // recent providers
  if ((nstat_privcheck != 0) || (req->provider == NSTAT_PROVIDER_UDP_SUBFLOW) ||
      (req->provider == NSTAT_PROVIDER_CONN_USERLAND)) {
  ...
  if (req->filter & NSTAT_FILTER_SUPPRESS_SRC_ADDED) {
    // Suppression of source messages implicitly requires the use of update
    // messages
    client->ntc_flags |= NSTAT_FLAG_SUPPORTS_UPDATES;
  }
  NSTAT_UNLOCK_EXCLUSIVE();

  // rdar://problem/30301300   Different providers require different
  // synchronization to ensure that a new entry does not get double counted due
  // to being added prior to all current provider entries being added.  Hence
  // pass the provider the details in the original request for this to be
  // applied atomically
...
static errno_t nstat_client_source_add(u_int64_t context, nstat_client *client,
  ...
  do {
    ...
    if (client->ntc_flags & NSTAT_FLAG_CLEANUP ||
        src->nts_srcref == NSTAT_SRC_REF_INVALID) {
    ...
    NSTAT_GLOBAL_COUNT_INCREMENT_WITH_MAX(nstat_global_src_current,
                                          nstat_global_src_max);

    client->ntc_metrics.nstat_src_current++;
    if (client->ntc_metrics.nstat_src_current >
        client->ntc_metrics.nstat_src_max) {
...
static errno_t nstat_client_handle_remove_request(nstat_client *client,
                                                  mbuf_t m) {
  nstat_src_ref_t srcref = NSTAT_SRC_REF_INVALID;
  nstat_src *src;

  if (mbuf_copydata(m, offsetof(nstat_msg_rem_src_req, srcref), sizeof(srcref),
                    &srcref) != 0) {
    return EINVAL;
  }
...
static errno_t nstat_client_handle_query_request(nstat_client *client,
                                                 mbuf_t m) {
  ...
  TAILQ_FOREACH_SAFE(src, &client->ntc_src_queue, nts_client_link, tmpsrc) {
    ...
    if (all_srcs || src->nts_srcref == req.srcref) {
      if (nstat_client_reporting_allowed(client, src, 0) &&
        ...
        if (all_srcs) {
          ...
          result =
              nstat_client_send_counts(client, src, req.hdr.context, 0, &gone);
        }
    ...
    if (gone) {
      ...
      if (NSTAT_LOCK_SHARED_TO_EXCLUSIVE()) {
        ...
        // Successfully upgraded the lock, now we can remove the source from the
        // client
...
static errno_t nstat_client_handle_get_src_description(nstat_client *client,
                                                       mbuf_t m) {
    ...
    if (all_srcs || src->nts_srcref == req.srcref) {
      if (nstat_client_reporting_allowed(client, src, 0) &&
        ...
        if (all_srcs) {
          ...
          result =
              nstat_client_send_description(client, src, req.hdr.context, 0);
        }
...
static void nstat_send_error(nstat_client *client, u_int64_t context,
                             u_int32_t error) {
  errno_t result;
...
static errno_t nstat_client_handle_get_update(nstat_client *client, mbuf_t m) {
  nstat_msg_query_src_req req;
  ...
  TAILQ_FOREACH_SAFE(src, &client->ntc_src_queue, nts_client_link, tmpsrc) {
    int gone = 0;
    if (all_srcs) {
      // Check to see if we should handle this source or if we're still skipping
      // to find where to continue
      if ((FALSE == partial || src->nts_seq != client->ntc_seq)) {
        u_int64_t suppression_flags =
            (src->nts_reported) ? NSTAT_FILTER_SUPPRESS_BORING_POLL : 0;
      ...
      if (nstat_client_reporting_allowed(client, src, 0)) {
        result =
            nstat_client_send_update(client, src, req.hdr.context, 0, 0, &gone);
      }
    }
    ...
    if (gone) {
      if (NSTAT_LOCK_SHARED_TO_EXCLUSIVE()) {
        // Successfully upgraded the lock, now we can remove the source from the
        // client pull src out of the list
        nstat_src_remove_linkages(client, src);
        TAILQ_INSERT_TAIL(&dead_list, src, nts_client_link);
        NSTAT_LOCK_EXCLUSIVE_TO_SHARED();
...
static errno_t nstat_client_handle_get_details(nstat_client *client, mbuf_t m) {
  nstat_msg_query_src_req req;
  ...
  TAILQ_FOREACH_SAFE(src, &client->ntc_src_queue, nts_client_link, tmpsrc) {
    ...
    if (all_srcs) {
      ...
      if (nstat_client_reporting_allowed(client, src, 0)) {
        result = nstat_client_send_details(client, src, req.hdr.context, 0, 0,
                                           &gone);
      }
    }
...
static errno_t nstat_client_handle_subscribe_sysinfo(nstat_client *client) {
  errno_t result = priv_check_cred(kauth_cred_get(),
                                   PRIV_NET_PRIVILEGED_NETWORK_STATISTICS, 0);
...
static errno_t nstat_client_send(kern_ctl_ref kctl, u_int32_t unit, void *uinfo,
  ...
  lck_mtx_lock(&client->ntc_user_mtx); /* Prevent misbehaving clients from
                                          overlapping calls */
  switch (hdr->type) {
...
/* Performs interface matching based on NSTAT_IFNET_IS… filter flags provided by
 * an external caller */
static bool nstat_interface_matches_filter_flag(uint32_t filter_flags,
                                                struct ifnet *ifp) {
  bool result = false;

  if (ifp) {
    uint32_t flag_mask = (NSTAT_FILTER_IFNET_FLAGS &
                          ~(NSTAT_IFNET_IS_NON_LOCAL | NSTAT_IFNET_IS_LOCAL));
...
static int progress_indicators_for_interface(
    unsigned int ifindex, uint64_t recentflow_maxduration,
    uint32_t filter_flags, uint64_t transport_protocol_mask,
    struct nstat_progress_indicators *indicators) {
  int error = 0;
  struct inpcb *inp;
  uint64_t min_recent_start_time;
  bool update_tcp_indicators =
      ((transport_protocol_mask & PR_PROTO_TCP) == PR_PROTO_TCP) ||
      (transport_protocol_mask == 0);
  bool update_quic_indicators =
      (transport_protocol_mask & PR_PROTO_QUIC) == PR_PROTO_QUIC;
#if SKYWALK
  struct nstat_tu_shadow *shad;
#endif /* SKYWALK */
  ...
  if (nstat_debug != 0) {
    /* interface index -1 may be passed in to only match against the filters
     * specified in the flags */
    if (ifindex < UINT_MAX) {
      NSTAT_LOG_DEBUG("for interface index %u with flags %x", ifindex,
                      filter_flags);
    } else {
  ...
  if (update_tcp_indicators) {
    ...
    /*
     * For progress indicators we don't need to special case TCP to collect time
     * wait connections
     */
    LIST_FOREACH(inp, tcbinfo.ipi_listhead, inp_list) {
      struct tcpcb *tp = intotcpcb(inp);
      /* radar://57100452
       * The conditional logic implemented below performs an *inclusive* match
       * based on the desired interface index in addition to any filter values.
       * While the general expectation is that only one criteria normally is
       * used for queries, the capability exists satisfy any eccentric future
       * needs.
       */
      if (tp && inp->inp_state != INPCB_STATE_DEAD && inp->inp_last_outifp &&
          /* matches the given interface index, or against any provided filter
             flags */
          (((inp->inp_last_outifp->if_index == ifindex) ||
            nstat_interface_matches_filter_flag(filter_flags,
                                                inp->inp_last_outifp)) &&
           /* perform flow state matching based any provided filter flags */
           (((filter_flags &
              (NSTAT_IFNET_IS_NON_LOCAL | NSTAT_IFNET_IS_LOCAL)) == 0) ||
            ((filter_flags & NSTAT_IFNET_IS_NON_LOCAL) &&
             !(tp->t_flags & TF_LOCAL)) ||
            ((filter_flags & NSTAT_IFNET_IS_LOCAL) &&
             (tp->t_flags & TF_LOCAL))))) {
        struct tcp_conn_status connstatus;

        if (nstat_debug != 0) {
          NSTAT_LOG_DEBUG("*matched non-Skywalk* [filter match: %d]",
                          nstat_interface_matches_filter_flag(
                              filter_flags, inp->inp_last_outifp));
        }
...
#if SKYWALK
  u_int32_t locality_flags =
      (filter_flags & (NSTAT_IFNET_IS_LOCAL | NSTAT_IFNET_IS_NON_LOCAL));
  u_int32_t flag_mask = (NSTAT_FILTER_IFNET_FLAGS &
                         ~(NSTAT_IFNET_IS_NON_LOCAL | NSTAT_IFNET_IS_LOCAL));
  ...
  TAILQ_FOREACH(shad, &nstat_userprot_shad_head, shad_link) {
    ...
    if (consider_shad) {
      ...
      bool result = (*shad->shad_getvals_fn)(
          shad->shad_provider_context, &ifflags, &digest, NULL, NULL, NULL);
      error = (result) ? 0 : EIO;
      if (error) {
        NSTAT_LOG_ERROR("nstat get ifflags and progressdigest returned %d",
                        error);
        continue;
      }
      if (ifflags & (NSTAT_IFNET_FLOWSWITCH_VALUE_UNOBTAINABLE |
                     NSTAT_IFNET_ROUTE_VALUE_UNOBTAINABLE)) {
        NSTAT_LOG_ERROR("nstat get ifflags and progressdigest resulted in "
                        "Skywalk error, ifflags=%x",
                        ifflags);
        continue;
      }
      ...
      if ((digest.ifindex == (u_int32_t)ifindex) ||
        ...
        if (nstat_debug != 0) {
          NSTAT_LOG_DEBUG("*matched Skywalk* [ifindex=%u digest.ifindex=%u "
                          "filter_flags=%x ifflags=%x]",
                          ifindex, digest.ifindex, filter_flags, ifflags);
        }
...
static int tcp_progress_probe_enable_for_interface(unsigned int ifindex,
  ...
  if (nstat_debug != 0) {
    NSTAT_LOG_DEBUG("for interface index %u with flags %d", ifindex,
                    filter_flags);
  }
  ...
  TAILQ_FOREACH(ifp, &ifnet_head, if_link) {
    if ((ifp->if_index == ifindex) ||
      ...
      if (nstat_debug != 0) {
        NSTAT_LOG_DEBUG("*matched* interface index %d, enable: %d",
                        ifp->if_index, enable_flags);
      }
      error = if_probe_connectivity(ifp, enable_flags);
      if (error) {
        NSTAT_LOG_ERROR("(%d) - nstat set tcp probe %d for interface index %d",
                        error, enable_flags, ifp->if_index);
      }
    }
  }
  ifnet_head_done();
...
static int ntstat_progress_indicators(struct sysctl_req *req) {
  struct nstat_progress_indicators indicators = {};
  int error = 0;
  ...
  error = progress_indicators_for_interface(
      (unsigned int)requested.np_ifindex, requested.np_recentflow_maxduration,
      (uint32_t)requested.np_filter_flags, requested.np_transport_protocol_mask,
      &indicators);
  if (error != 0) {
    return error;
  }
...
__private_extern__ int ntstat_tcp_progress_enable(struct sysctl_req *req) {
  int error = 0;
  ...
  error = tcp_progress_probe_enable_for_interface(
      (unsigned int)requested.ifindex, (uint32_t)requested.filter_flags,
      (uint32_t)requested.enable);
...
static int nstat_gather_flow_data(nstat_provider_id_t provider,
                                  nstat_flow_data *__counted_by(n)
                                      flow_data_start,
                                  int n) {
  ...
  TAILQ_FOREACH(shad, &nstat_userprot_shad_head, shad_link) {
    ...
    if (shad->shad_provider == provider) {
      ...
      err = nstat_userland_tu_copy_descriptor(
          (nstat_provider_cookie_t)shad, &flow_data->flow_descriptor,
          sizeof(flow_data->flow_descriptor));
...
static void nstat_userland_to_xinpcb_n(nstat_provider_id_t provider,
                                       nstat_flow_data *flow_data,
                                       struct xinpcb_n *xinp) {
...
static void nstat_userland_to_xsocket_n(nstat_provider_id_t provider,
                                        nstat_flow_data *flow_data,
                                        struct xsocket_n *xso) {
...
static void nstat_userland_to_rcv_xsockbuf_n(nstat_provider_id_t provider,
                                             nstat_flow_data *flow_data,
                                             struct xsockbuf_n *xsbrcv) {
...
static void nstat_userland_to_snd_xsockbuf_n(nstat_provider_id_t provider,
                                             nstat_flow_data *flow_data,
                                             struct xsockbuf_n *xsbsnd) {
...
static void nstat_userland_to_xsockstat_n(nstat_flow_data *flow_data,
                                          struct xsockstat_n *xst) {
  xst->xst_len = sizeof(struct xsockstat_n);
  xst->xst_kind = XSO_STATS;

  // The kernel version supports an array of counts, here we only support one
  // and map to first entry
...
static void nstat_userland_to_xtcpcb_n(nstat_flow_data *flow_data,
                                       struct xtcpcb_n *xt) {
  nstat_tcp_descriptor *desc = &flow_data->flow_descriptor.tcp_descriptor;
...
__private_extern__ int ntstat_userland_count(short proto) {
  int n = 0;
...
nstat_userland_get_snapshot(short proto,
                            void *__sized_by(*snapshot_size) * snapshotp,
                            size_t *snapshot_size, int *countp) {
  int error = 0;
  int n = 0;
  size_t data_size = 0;
  ...
  flow_data = (nstat_flow_data *)kalloc_data(data_size, Z_WAITOK | Z_ZERO);
  if (flow_data) {
...
// nstat_userland_list_snapshot() does most of the work for a sysctl that uses a
// return format as per get_pcblist_n() even though the vast majority of fields
// are unused. Additional items are required in the sysctl output before and
// after the data added by this function.
__private_extern__ int nstat_userland_list_snapshot(
    short proto, struct sysctl_req *req,
    void *__sized_by(n * sizeof(nstat_flow_data)) userlandsnapshot, int n) {
  int error = 0;
  int i;
  nstat_provider_id_t provider;
  void *buf = NULL;
  ...
  if (buf) {
    ...
    struct xsockbuf_n *xsbrcv =
        (struct xsockbuf_n *)ADVANCE64(xso, sizeof(*xso));
    struct xsockbuf_n *xsbsnd =
        (struct xsockbuf_n *)ADVANCE64(xsbrcv, sizeof(*xsbrcv));
    struct xsockstat_n *xsostats =
        (struct xsockstat_n *)ADVANCE64(xsbsnd, sizeof(*xsbsnd));
    struct xtcpcb_n *xt =
        (struct xtcpcb_n *)ADVANCE64(xsostats, sizeof(*xsostats));
...
__private_extern__ void nstat_userland_release_snapshot(void *snapshot,
                                                        int nuserland) {
  if (snapshot != NULL) {
...
__private_extern__ int ntstat_userland_list_n(short proto,
