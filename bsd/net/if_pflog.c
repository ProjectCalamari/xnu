...
static struct if_clone pflog_cloner = IF_CLONE_INITIALIZER(
    PFLOGNAME, pflog_clone_create, pflog_clone_destroy, 0, (PFLOGIFS_MAX - 1));
...
static inline bool pflog_is_enabled(void) {
  uint8_t flags;
...
static int pflog_clone_create(struct if_clone *ifc, u_int32_t unit,
  ...
  if (unit >= PFLOGIFS_MAX) {
    ...
    panic("%s: unit (%d) exceeds max (%d)", __func__, unit, PFLOGIFS_MAX);
    /* NOTREACHED */
  }
...
static int pflog_remove(struct ifnet *ifp) {
  int error = 0;
  ...
  if (pflogif == NULL || (pflogif->sc_flags & IFPFLF_DETACHING) != 0) {
    error = EINVAL;
    goto done;
  }
...
static int pflog_clone_destroy(struct ifnet *ifp) {
  int error = 0;
...
static errno_t pflogoutput(struct ifnet *ifp, struct mbuf *m) {
  printf("%s: freeing data for %s\n", __func__, if_name(ifp));
  m_freem(m);
  return ENOTSUP;
}

static errno_t pflogioctl(struct ifnet *ifp, unsigned long cmd, void *data) {
#pragma unused(data)
  switch (cmd) {
  case SIOCSIFADDR:
  case SIOCAIFADDR:
...
static errno_t pflogdemux(struct ifnet *ifp, struct mbuf *m, char *h,
...
static errno_t pflogaddproto(struct ifnet *ifp, protocol_family_t pf,
                             const struct ifnet_demux_desc *d, u_int32_t cnt) {
#pragma unused(ifp, pf, d, cnt)
  return 0;
}

static errno_t pflogdelproto(struct ifnet *ifp, protocol_family_t pf) {
#pragma unused(ifp, pf)
  return 0;
}

static void pflogfree(struct ifnet *ifp) {
  kfree_type(struct pflog_softc, ifp->if_softc);
  ifp->if_softc = NULL;
  (void)ifnet_release(ifp);
}

int pflog_packet(struct pfi_kif *kif, pbuf_t *pbuf, sa_family_t af,
                 u_int8_t dir, u_int8_t reason, struct pf_rule *rm,
  ...
  if (rm->logif >= PFLOGIFS_MAX || (ifn = pflogifs[rm->logif]) == NULL ||
      !ifn->if_bpf) {
    return 0;
  }
