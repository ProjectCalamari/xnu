...
#define FQ_CLEAR_DELAY_HIGH(_fq_)                                              \
  do {                                                                         \
    if (FQ_IS_DELAY_HIGH(_fq_)) {                                              \
      KDBG(AQM_KTRACE_AON_FLOW_HIGH_DELAY | DBG_FUNC_END, (_fq_)->fq_flowhash, \
           AQM_KTRACE_FQ_GRP_SC_IDX((_fq_)), (_fq_)->fq_bytes,                 \
           (_fq_)->fq_min_qdelay);                                             \
    }                                                                          \
    (_fq_)->fq_flags &= ~FQF_DELAY_HIGH;                                       \
  } while (0)
...
#define FQ_SET_ECN_CAPABLE(_fq_)                                               \
  do {                                                                         \
    (_fq_)->fq_flags |= FQF_ECN_CAPABLE;                                       \
  } while (0)
#define FQ_CLEAR_ECN_CAPABLE(_fq_)                                             \
  do {                                                                         \
    (_fq_)->fq_flags &= ~FQF_ECN_CAPABLE;                                      \
  } while (0)

#define FQ_CONGESTION_FEEDBACK_CAPABLE(_fq_)                                   \
  ((_fq_)->fq_flags & FQF_CONGESTION_FEEDBACK)
#define FQ_ENABLE_CONGESTION_FEEDBACK(_fq_)                                    \
  do {                                                                         \
    (_fq_)->fq_flags |= FQF_CONGESTION_FEEDBACK;                               \
  } while (0)

#define FQ_IS_OVERWHELMING(_fq_) ((_fq_)->fq_flags & FQF_OVERWHELMING)
#define FQ_SET_OVERWHELMING(_fq_)                                              \
  do {                                                                         \
...
#define FQ_CLEAR_OVERWHELMING(_fq_)                                            \
  do {                                                                         \
    if (FQ_IS_OVERWHELMING(_fq_)) {                                            \
...
#define FQF_ECN_CAPABLE                                                        \
  0x100 /* The flow is capable for doing ECN for classic traffic */
#define FQF_CONGESTION_FEEDBACK                                                \
  0x200              /* The flow is capable for doing congestion feedbacks */
...
#define FQF_FLOW_STATE_MASK                                                    \
  (FQF_DELAY_HIGH | FQF_NEW_FLOW | FQF_OLD_FLOW | FQF_FLOWCTL_ON |             \
   FQF_EMPTY_FLOW)
...
#if SKYWALK
#define fq_empty(_q, _ptype)                                                   \
  (((_ptype) == QP_MBUF) ? MBUFQ_EMPTY(&(_q)->fq_mbufq)                        \
...
#define fq_enqueue(_q, _h, _t, _c, _ptype)                                     \
  do {                                                                         \
    ...
    case QP_MBUF:                                                              \
      ...
      MBUFQ_ADD_CRUMB_MULTI(&(_q)->fq_mbufq, (_h).cp_mbuf, (_t).cp_mbuf,       \
                            PKT_CRUMB_FQ_ENQUEUE);                             \
      break;                                                                   \
    case QP_PACKET:                                                            \
      ASSERT((_h).cp_ptype == QP_PACKET);                                      \
      ASSERT((_t).cp_ptype == QP_PACKET);                                      \
      KPKTQ_ENQUEUE_MULTI(&(_q)->fq_kpktq, (_h).cp_kpkt, (_t).cp_kpkt, (_c));  \
      break;                                                                   \
    default:                                                                   \
      VERIFY(0);                                                               \
...
#if SKYWALK
#define fq_dequeue(_q, _p, _ptype)                                             \
  do {                                                                         \
    switch (_ptype) {                                                          \
    case QP_MBUF: {                                                            \
...
extern int fq_codel_enq_legacy(void *fqs, fq_if_group_t *, pktsched_pkt_t *,
                               struct fq_if_classq *);
extern void fq_codel_dq_legacy(void *fqs, void *fq, pktsched_pkt_t *pkt,
                               uint64_t now);
extern void fq_getq_flow_internal(struct fq_codel_sched_data *, fq_t *,
                                  pktsched_pkt_t *);
...
extern void fq_codel_dq(void *fqs_p, void *fq_p, pktsched_pkt_t *pkt,
                        uint64_t now);
