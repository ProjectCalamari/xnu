...
#define IFCQ_LOCK(_ifcq) lck_mtx_lock(&(_ifcq)->ifcq_lock)

#define IFCQ_LOCK_SPIN(_ifcq) lck_mtx_lock_spin(&(_ifcq)->ifcq_lock)

#define IFCQ_CONVERT_LOCK(_ifcq)                                               \
  do {                                                                         \
    IFCQ_LOCK_ASSERT_HELD(_ifcq);                                              \
    lck_mtx_convert_spin(&(_ifcq)->ifcq_lock);                                 \
  } while (0)

#define IFCQ_UNLOCK(_ifcq) lck_mtx_unlock(&(_ifcq)->ifcq_lock)

/*
 * For ifclassq operations
 */
#define IFCQ_TBR_DEQUEUE(_ifcq, _p, _idx)                                      \
  do {                                                                         \
    ifclassq_tbr_dequeue(_ifcq, _p, _idx);                                     \
  } while (0)

#define IFCQ_TBR_DEQUEUE_SC(_ifcq, _sc, _p, _idx)                              \
  do {                                                                         \
...
#define IFCQ_XMIT_ADD(_ifcq, _pkt, _len)                                       \
  do {                                                                         \
    PKTCNTR_ADD(&(_ifcq)->ifcq_xmitcnt, _pkt, _len);                           \
  } while (0)

#define IFCQ_DROP_ADD(_ifcq, _pkt, _len)                                       \
  do {                                                                         \
...
extern int ifclassq_get_len(struct ifclassq *, mbuf_svc_class_t, u_int8_t,
                            u_int32_t *, u_int32_t *);
extern errno_t ifclassq_enqueue(struct ifclassq *, classq_pkt_t *,
                                classq_pkt_t *, u_int32_t, u_int32_t,
                                boolean_t *);
extern errno_t ifclassq_dequeue(struct ifclassq *, mbuf_svc_class_t, u_int32_t,
                                u_int32_t, classq_pkt_t *, classq_pkt_t *,
...
extern int ifclassq_getqstats(struct ifclassq *, u_int8_t, u_int32_t, void *,
                              u_int32_t *);
...
extern void ifclassq_tbr_get(struct ifclassq *, u_int32_t *, u_int64_t *,
                             u_int64_t *);
