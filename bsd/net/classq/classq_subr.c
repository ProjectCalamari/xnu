...
#endif /* SKYWALK */
static int ifclassq_tbr_set_locked(struct ifclassq *ifq,
...
SYSCTL_STRING(_net_classq_scheduler, OID_AUTO, available_models,
              CTLFLAG_RD | CTLFLAG_LOCKED, IFNET_SCHED_MODEL_LIST, 0, "");
#undef X
...
int ifclassq_setup(struct ifclassq *ifq, struct ifnet *ifp, uint32_t sflags) {
  ...
  if (ifp->if_eflags & IFEF_TXSTART) {
    ...
    if (IFCQ_MAXLEN(ifq) != if_sndq_maxlen && IFCQ_TARGET_QDELAY(ifq) == 0) {
      /*
  ...
  static_assert(offsetof(struct skoid, sko_oid_list) ==
                offsetof(ifcq_oid_t, ifcq_oid_list));
  static_assert(offsetof(struct skoid, sko_oid) ==
                offsetof(ifcq_oid_t, ifcq_oid));
  static_assert(offsetof(struct skoid, sko_name) ==
                offsetof(ifcq_oid_t, ifcq_name));

  struct skoid *ifcq_skoid = (struct skoid *)&ifq->ifcq_oid;
  skoid_create(ifcq_skoid, SKOID_SNODE(_net_classq_scheduler), if_name(ifp),
               CTLFLAG_RW);
  skoid_add_handler(ifcq_skoid, "model", CTLFLAG_RW, ifclassq_configure_sysctl,
                    ifq, 0);
#endif /* (DEBUG || DEVELOPMENT) */
...
int ifclassq_change(struct ifclassq *ifq, uint32_t model) {
...
void ifclassq_teardown(struct ifclassq *ifq) {
  IFCQ_LOCK(ifq);
...
int ifclassq_pktsched_setup(struct ifclassq *ifq) {
  struct ifnet *ifp = ifq->ifcq_ifp;
...
#if SKYWALK
  ptype = ((ifp->if_eflags & IFEF_SKYWALK_NATIVE) != 0) ? QP_PACKET : QP_MBUF;
#endif /* SKYWALK */
  ...
  case IFNET_SCHED_MODEL_NORMAL:
    if (ifp->if_family == IFNET_FAMILY_ETHERNET &&
      ...
      err =
          pktsched_setup(ifq, PKTSCHEDT_FQ_CODEL_NEW, ifq->ifcq_sflags, ptype);
    } else {
...
void ifclassq_set_maxlen(struct ifclassq *ifq, u_int32_t maxqlen) {
  IFCQ_LOCK(ifq);
  if (maxqlen == 0) {
...
u_int32_t ifclassq_get_maxlen(struct ifclassq *ifq) { return IFCQ_MAXLEN(ifq); }

int ifclassq_get_len(struct ifclassq *ifq, mbuf_svc_class_t sc,
                     u_int8_t grp_idx, u_int32_t *packets, u_int32_t *bytes) {
  int err = 0;
  ...
  if (__improbable(ifp->if_na_ops != NULL &&
    ...
    err = ifp->if_na_ops->ni_get_len(ifp->if_na, sc, packets, bytes, err);
  }
#endif /* SKYWALK */
...
inline void ifclassq_set_packet_metadata(struct ifclassq *ifq,
...
errno_t ifclassq_enqueue(struct ifclassq *ifq, classq_pkt_t *head,
                         classq_pkt_t *tail, u_int32_t cnt, u_int32_t bytes,
                         boolean_t *pdrop) {
  return ifq->ifcq_ops->ps_enq(ifq, head, tail, cnt, bytes, pdrop);
}

static errno_t ifclassq_dequeue_common_default(
    struct ifclassq *ifq, mbuf_svc_class_t sc, u_int32_t pkt_limit,
    u_int32_t byte_limit, classq_pkt_t *head, classq_pkt_t *tail,
    u_int32_t *cnt, u_int32_t *len, boolean_t drvmgt, u_int8_t grp_idx) {
  struct ifnet *ifp = ifq->ifcq_ifp;
  u_int32_t i = 0, l = 0;
  ...
  if (drvmgt) {
    ...
    err = ifq->ifcq_ops->ps_deq_sc(ifq, sc, pkt_limit, byte_limit, head, tail,
                                   cnt, len, grp_idx);
    IFCQ_UNLOCK(ifq);
    ...
    err = ifq->ifcq_ops->ps_deq(ifq, pkt_limit, byte_limit, head, tail, cnt,
                                len, grp_idx);
    IFCQ_UNLOCK(ifq);
...
errno_t ifclassq_dequeue(struct ifclassq *ifq, mbuf_svc_class_t sc,
                         u_int32_t pkt_limit, u_int32_t byte_limit,
                         classq_pkt_t *head, classq_pkt_t *tail, u_int32_t *cnt,
                         u_int32_t *len, u_int8_t grp_idx) {
  ...
  if (__improbable(ifp->if_na_ops != NULL &&
     ...
     * filters.
     */
    errno_t err = ifclassq_dequeue_common_default(
        ifq, sc, pkt_limit, byte_limit, head, tail, cnt, len, drvmgt, grp_idx);

    return ifp->if_na_ops->ni_dequeue(ifp->if_na, sc, pkt_limit, byte_limit,
                                      head, tail, cnt, len, drvmgt, err);
  }
  return ifclassq_dequeue_common_default(ifq, sc, pkt_limit, byte_limit, head,
                                         tail, cnt, len, drvmgt, grp_idx);
}

void ifclassq_update(struct ifclassq *ifq, cqev_t ev, bool locked) {
  void *ev_p = (void *)&ev;
  ...
  if (IFCQ_TBR_IS_ENABLED(ifq)) {
    struct tb_profile tb = {.rate = ifq->ifcq_tbr.tbr_rate_raw,
                            .percent = ifq->ifcq_tbr.tbr_percent,
                            .depth = 0};
    (void)ifclassq_tbr_set_locked(ifq, &tb, FALSE);
  }
...
int ifclassq_attach(struct ifclassq *ifq, u_int32_t type, void *discipline) {
  IFCQ_LOCK_ASSERT_HELD(ifq);
  VERIFY(ifq->ifcq_disc == NULL);
  ifq->ifcq_type = type;
...
void ifclassq_detach(struct ifclassq *ifq) {
  IFCQ_LOCK_ASSERT_HELD(ifq);
  VERIFY(ifq->ifcq_disc == NULL);
...
int ifclassq_getqstats(struct ifclassq *ifq, u_int8_t gid, u_int32_t qid,
                       void *ubuf, u_int32_t *nbytes) {
  ...
  ifqs = kalloc_type(struct if_ifclassq_stats, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  *(&ifqs->ifqs_dropcnt) = *(&ifq->ifcq_dropcnt);
  ...
  if (err == 0 &&
      (err = copyout(ifqs, (user_addr_t)(uintptr_t)ubuf, sizeof(*ifqs))) == 0) {
    *nbytes = sizeof(*ifqs);
  }
...
const char *__null_terminated ifclassq_ev2str(cqev_t ev) {
...
void ifclassq_tbr_dequeue(struct ifclassq *ifq, classq_pkt_t *pkt,
                          u_int8_t grp_idx) {
  ifclassq_tbr_dequeue_common(ifq, MBUF_SC_UNSPEC, FALSE, pkt, grp_idx);
}

void ifclassq_tbr_dequeue_sc(struct ifclassq *ifq, mbuf_svc_class_t sc,
                             classq_pkt_t *pkt, u_int8_t grp_idx) {
  ifclassq_tbr_dequeue_common(ifq, sc, TRUE, pkt, grp_idx);
}

static void ifclassq_tbr_dequeue_common(struct ifclassq *ifq,
  ...
  *pkt = CLASSQ_PKT_INITIALIZER(*pkt);
   ...
   */
  if (drvmgt) {
    ifq->ifcq_ops->ps_deq_sc(ifq, sc, 1, CLASSQ_DEQUEUE_MAX_BYTE_LIMIT, pkt,
                             NULL, NULL, NULL, grp_idx);
  } else {
    ifq->ifcq_ops->ps_deq(ifq, 1, CLASSQ_DEQUEUE_MAX_BYTE_LIMIT, pkt, NULL,
                          NULL, NULL, grp_idx);
  }
...
#if SKYWALK
    case QP_PACKET:
      tbr->tbr_token -= TBR_SCALE(pkt->cp_kpkt->pkt_length);
      break;
#endif /* SKYWALK */
...
static int ifclassq_tbr_set_locked(struct ifclassq *ifq,
  ...
  if (pktsched_verbose) {
    printf("%s: TBR %s (rate %llu bps depth %u)\n", if_name(ifp),
           (ifq->ifcq_flags & IFCQF_TBR) ? "reconfigured" : "enabled", rate,
           profile->depth);
  }
  ...
  if (tbr->tbr_rate > 0 && (ifp->if_flags & IFF_UP)) {
    struct timespec ts = {0, (long)pktsched_abs_to_nsecs(tbr->tbr_filluptime)};
    if (pktsched_verbose) {
      printf("%s: TBR calculated tokens %lld "
             "filluptime %llu ns\n",
             if_name(ifp), TBR_UNSCALE(tbr->tbr_token),
...
int ifclassq_tbr_set(struct ifclassq *ifq, struct tb_profile *profile,
                     boolean_t update) {
  int error = 0;
...
struct ifclassq *ifclassq_alloc(void) {
...
void ifclassq_retain(struct ifclassq *ifcq) {
  os_ref_retain(&ifcq->ifcq_refcnt);
}

void ifclassq_release(struct ifclassq **pifcq) {
...
int ifclassq_setup_group(struct ifclassq *ifcq, uint8_t grp_idx,
                         uint8_t flags) {
  int err;
  ...
  VERIFY(ifcq->ifcq_type == PKTSCHEDT_FQ_CODEL ||
         ifcq->ifcq_type == PKTSCHEDT_FQ_CODEL_NEW);
...
int ifclassq_request(struct ifclassq *ifcq, enum cqrq rq, void *arg,
                     bool locked) {
  int err = 0;
...
void ifclassq_tbr_get(struct ifclassq *ifcq, u_int32_t *sched_type,
                      u_int64_t *tbr_bw, u_int64_t *tbr_pct) {
  IFCQ_LOCK(ifcq);
...
#pragma unused(oidp, arg2)
  ...
  error = sysctl_io_number(req, ifp->if_output_sched_model,
                           sizeof(ifp->if_output_sched_model), &new_model,
                           &changed);
