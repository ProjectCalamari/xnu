...
SYSCTL_NODE(_net_classq, OID_AUTO, flow_q, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "FQ-CODEL parameters");

SYSCTL_UINT(_net_classq_flow_q, OID_AUTO, pkt_compressor,
            CTLFLAG_RW | CTLFLAG_LOCKED, &pkt_compressor, 0,
            "enable pkt compression");

SYSCTL_QUAD(_net_classq, OID_AUTO, l4s_ce_threshold,
            CTLFLAG_RW | CTLFLAG_LOCKED, &l4s_ce_threshold, "L4S CE threshold");
...
SYSCTL_QUAD(_net_classq_flow_q, OID_AUTO, pkt_pacing_leeway,
            CTLFLAG_RW | CTLFLAG_LOCKED, &pkt_pacing_leeway,
            "packet pacing leeway");

SYSCTL_QUAD(_net_classq_flow_q, OID_AUTO, max_pkt_pacing_interval,
            CTLFLAG_RW | CTLFLAG_LOCKED, &max_pkt_pacing_interval,
            "max packet pacing interval");

SYSCTL_QUAD(_net_classq_flow_q, OID_AUTO, l4s_min_delay_threshold,
            CTLFLAG_RW | CTLFLAG_LOCKED, &l4s_min_delay_threshold,
            "l4s min delay threshold");
...
fq_t *fq_alloc(classq_pkt_type_t ptype) {
  fq_t *fq = NULL;
...
void fq_destroy(fq_t *fq, classq_pkt_type_t ptype) {
...
static inline void fq_detect_dequeue_stall(fq_if_t *fqs, fq_t *flowq,
  ...
  if ((*now) > maxgetqtime) {
    ...
    os_log_error(OS_LOG_DEFAULT,
                 "%s:num: %d, "
                 "scidx: %d, flow: 0x%x, iface: %s grp: %hhu",
                 __func__, fq_cl->fcl_stat.fcl_dequeue_stall,
                 flowq->fq_sc_index, flowq->fq_flowhash,
                 if_name(fqs->fqs_ifq->ifcq_ifp), FQ_GROUP(flowq)->fqg_index);
...
void fq_head_drop(fq_if_t *fqs, fq_t *fq) {
  pktsched_pkt_t pkt;
...
static int fq_compressor(fq_if_t *fqs, fq_t *fq, fq_if_classq_t *fq_cl,
                         pktsched_pkt_t *pkt) {
  ...
  if (ptype == QP_MBUF) {
    ...
    if (__improbable(droptap_verbose > 0)) {
      droptap_output_packet(SK_PKT2PH(kpkt), DROP_REASON_AQM_COMPRESSED, NULL,
                            0, 0, fqs->fqs_ifq->ifcq_ifp, 0, NULL, -1, NULL, 0,
                            0);
    }

    struct kern_pbufpool *pp = __DECONST(
        struct kern_pbufpool *, ((struct __kern_quantum *)kpkt)->qum_pp);
...
int fq_codel_enq_legacy(void *fqs_p, fq_if_group_t *fq_grp, pktsched_pkt_t *pkt,
                        fq_if_classq_t *fq_cl) {
  ...
  fq = fq_if_hash_pkt(fqs, fq_grp, pkt_flowid, pktsched_get_pkt_svc(pkt), now,
                      pkt_proto, pkt_flowsrc, tfc_type, true);
  ...
  KDBG(AQM_KTRACE_STATS_FLOW_ENQUEUE, fq->fq_flowhash,
       AQM_KTRACE_FQ_GRP_SC_IDX(fq), fq->fq_bytes, pktsched_get_pkt_len(pkt));
  ...
  if (__improbable(FQ_IS_DELAY_HIGH(fq) || FQ_IS_OVERWHELMING(fq))) {
    if ((fq->fq_flags & FQF_FLOWCTL_CAPABLE) && (*pkt_flags & PKTF_FLOW_ADV)) {
      fc_adv = 1;
      /*
       * If the flow is suspended or it is not
       * TCP/QUIC, drop the chain.
       */
      if ((pkt_proto != IPPROTO_TCP) && (pkt_proto != IPPROTO_QUIC)) {
        droptype = DTYPE_EARLY;
        fq_cl->fcl_stat.fcl_drop_early += cnt;
        IFCQ_DROP_ADD(fqs->fqs_ifq, cnt, pktsched_get_pkt_len(pkt));
      }
      DTRACE_IP6(flow__adv, fq_if_t *, fqs, fq_if_classq_t *, fq_cl, fq_t *, fq,
                 int, droptype, pktsched_pkt_t *, pkt, uint32_t, cnt);
    } else {
      /*
      ...
      DTRACE_IP6(no__flow__adv, fq_if_t *, fqs, fq_if_classq_t *, fq_cl, fq_t *,
                 fq, int, droptype, pktsched_pkt_t *, pkt, uint32_t, cnt);
    }
  }
  ...
  if (__improbable(droptype == DTYPE_NODROP && fq_if_at_drop_limit(fqs))) {
    ...
    if (fqs->fqs_large_flow == fq) {
       ...
       */
      DTRACE_IP5(large__flow, fq_if_t *, fqs, fq_if_classq_t *, fq_cl, fq_t *,
                 fq, pktsched_pkt_t *, pkt, uint32_t, cnt);
      ...
      if ((fq->fq_flags & FQF_FLOWCTL_CAPABLE) &&
          (*pkt_flags & PKTF_FLOW_ADV) && (pkt_proto != IPPROTO_TCP) &&
      ...
      if (fqs->fqs_large_flow == NULL) {
        ...
        DTRACE_IP5(no__large__flow, fq_if_t *, fqs, fq_if_classq_t *, fq_cl,
         ...
         */
        if (fq_empty(fq, fqs->fqs_ptype) &&
            !(fq->fq_flags & (FQF_NEW_FLOW | FQF_OLD_FLOW))) {
          fq_if_move_to_empty_flow(fqs, fq_cl, fq, now);
          fq = NULL;
        }
      } else {
        DTRACE_IP5(different__large__flow, fq_if_t *, fqs, fq_if_classq_t *,
                   fq_cl, fq_t *, fq, pktsched_pkt_t *, pkt, uint32_t, cnt);
  ...
  if (__probable(droptype == DTYPE_NODROP)) {
    ...
    DTRACE_IP5(fq_enqueue, fq_if_t *, fqs, fq_if_classq_t *, fq_cl, fq_t *, fq,
...
void fq_getq_flow_internal(fq_if_t *fqs, fq_t *fq, pktsched_pkt_t *pkt) {
 ...
 */
static uint64_t fq_get_next_tx_time(fq_if_t *fqs, fq_t *fq) {
  ...
  case QP_MBUF: {
    ...
    if ((m = MBUFQ_FIRST(&fq->fq_mbufq)) != NULL) {
      struct m_tag *tag;
      tag = m_tag_locate(m, KERNEL_MODULE_TAG_ID, KERNEL_TAG_TYPE_AQM);
      if (tag != NULL) {
 ...
 */
boolean_t fq_tx_time_ready(fq_if_t *fqs, fq_t *fq, uint64_t now,
                           uint64_t *ready_time) {
  ...
  KDBG(AQM_KTRACE_TX_NOT_READY, fq->fq_flowhash, AQM_KTRACE_FQ_GRP_SC_IDX(fq),
       now, pkt_tx_time);
  return FALSE;
}

void fq_codel_dq_legacy(void *fqs_p, void *fq_p, pktsched_pkt_t *pkt,
                        uint64_t now) {
  fq_if_t *fqs = (fq_if_t *)fqs_p;
  fq_t *fq = (fq_t *)fq_p;
  fq_if_classq_t *fq_cl = &FQ_CLASSQ(fq);
  ...
  if (fq_codel_enable_pacing && fq_codel_enable_l4s) {
    if (pkt_tx_time > *pkt_timestamp) {
      ...
      DTRACE_SKYWALK5(aqm__miss__pacing__delay, uint64_t, *pkt_timestamp,
                      uint64_t, pkt_tx_time, uint64_t, now, fq_if_t *, fqs,
                      fq_t *, fq);
    }
  ...
  if (num_dequeues == 0) {
    ...
    if (os_add_overflow(num_dequeues, 1, &res)) {
      ...
      os_log_info(OS_LOG_DEFAULT,
                  "%s: dequeue num overflow, "
                  "flow: 0x%x, iface: %s",
                  __func__, fq->fq_flowhash, if_name(fqs->fqs_ifq->ifcq_ifp));
    } else {
      uint64_t product = 0;
      if (os_mul_overflow(fq_cl->fcl_stat.fcl_avg_qdelay, num_dequeues,
                          &product) ||
          os_add_overflow(product, qdelay, &res)) {
        fq_cl->fcl_stat.fcl_avg_qdelay = qdelay;
      } else {
        fq_cl->fcl_stat.fcl_avg_qdelay = res / (num_dequeues + 1);
      }
    }
  }
  ...
  if (fq_codel_enable_l4s && l4s_pkt) {
    ...
    if ((l4s_ce_threshold != 0 && qdelay > l4s_ce_threshold + pacing_delay) ||
        (l4s_ce_threshold == 0 &&
         qdelay > FQ_TARGET_DELAY(fq) + pacing_delay)) {
      ...
      if (__improbable(l4s_local_ce_report != 0) &&
          ...
          fq_if_report_congestion(fqs, pkt, 0, 1,
                                  fq->fq_pkts_since_last_report)) {
  ...
  if (fq->fq_min_qdelay == 0 || (u_int64_t)qdelay < fq->fq_min_qdelay) {
  ...
  if ((fq->fq_flags & FQF_FLOWCTL_ON) && !FQ_IS_DELAY_HIGH(fq) &&
      !FQ_IS_OVERWHELMING(fq)) {
...
int fq_codel_enq(void *fqs_p, fq_if_group_t *fq_grp, pktsched_pkt_t *pkt,
                 fq_if_classq_t *fq_cl) {
  fq_if_t *fqs = (fq_if_t *)fqs_p;
  ...
  if (__probable(droptype == DTYPE_NODROP)) {
    ...
    if (FQ_CONGESTION_FEEDBACK_CAPABLE(fq) && fq->fq_flowsrc == FLOWSRC_INPCB &&
      ...
      KDBG(AQM_KTRACE_CONGESTION_NOTIFIED, fq->fq_flowhash,
           AQM_KTRACE_FQ_GRP_SC_IDX(fq), fq->fq_bytes, fq->fq_congestion_cnt);
...
static boolean_t codel_ok_to_drop(pktsched_pkt_t *pkt, fq_t *fq,
                                  struct codel_status *codel_status,
                                  uint64_t now) {
  ...
  if (codel_status->first_above_time == 0) {
    ...
    ok_to_drop = true;
    KDBG(AQM_KTRACE_OK_TO_DROP, fq->fq_flowhash, sojourn_time, *pkt_enq_time,
         now);
  }
...
static uint64_t codel_control_law(float inv_sqrt, uint64_t t,
                                  uint64_t interval) {
...
static void fq_drop_pkt(struct ifclassq *ifcq, struct ifnet *ifp,
                        pktsched_pkt_t *pkt) {
  IFCQ_DROP_ADD(ifcq, 1, pktsched_get_pkt_len(pkt));
  if (__improbable(droptap_verbose > 0)) {
    pktsched_drop_pkt(pkt, ifp, DROP_REASON_AQM_HIGH_DELAY, __func__, __LINE__,
                      0);
  } else {
    pktsched_free_pkt(pkt);
  }
...
void fq_codel_dq(void *fqs_p, void *fq_p, pktsched_pkt_t *pkt, uint64_t now) {
  fq_if_t *fqs = (fq_if_t *)fqs_p;
  fq_t *fq = (fq_t *)fq_p;
  fq_if_classq_t *fq_cl = &FQ_CLASSQ(fq);
  ...
  pktsched_get_pkt_vars(pkt, &pkt_flags, &pkt_timestamp, NULL, NULL, NULL, NULL,
                        &pkt_tx_time);
  l4s_pkt = pktsched_is_pkt_l4s(pkt);
    ...
    *pkt_timestamp = 0;
    ...
    if (status->dropping) {
      ...
      while (now >= status->drop_next && status->dropping) {
        ...
        if (FQ_CONGESTION_FEEDBACK_CAPABLE(fq) &&
          ...
          if (fq->fq_flowsrc == FLOWSRC_CHANNEL) {
            fq_if_report_congestion(fqs, pkt, 1, 0,
                                    fq->fq_pkts_since_last_report);
            fq_cl->fcl_stat.fcl_congestion_feedback++;
          }
          goto end;
          ...
          fq_cl->fcl_stat.fcl_ce_marked++;
          status->drop_next =
              codel_control_law(fast_inv_sqrt(status->count), status->drop_next,
                                FQ_UPDATE_INTERVAL(fq));
          KDBG(AQM_KTRACE_PKT_DROP, fq->fq_flowhash, status->count,
               status->drop_next, now);
          /*
          ...
          if (!codel_ok_to_drop(pkt, fq, status, now)) {
            ...
            status->drop_next =
                codel_control_law(fast_inv_sqrt(status->count),
                                  status->drop_next, FQ_UPDATE_INTERVAL(fq));
          }
        }
        KDBG(AQM_KTRACE_PKT_DROP, fq->fq_flowhash, status->count,
             status->drop_next, now);
      ...
      status->drop_next = codel_control_law(fast_inv_sqrt(status->count), now,
                                            FQ_UPDATE_INTERVAL(fq));
