...
static struct if_clone redirect_cloner = IF_CLONE_INITIALIZER(
    RD_NAME, redirect_clone_create, redirect_clone_destroy, 0, RD_MAXUNIT);
...
#define RD_LOCK(rd) lck_mtx_lock(&(rd)->rd_lock)
#define RD_UNLOCK(rd) lck_mtx_unlock(&(rd)->rd_lock)
#define RD_LOCK_DESTROY(rd)                                                    \
  lck_mtx_destroy(&(rd)->rd_lock, &redirect_lock_group)

static inline boolean_t redirect_is_usable(if_redirect_t rd) {
  return !rd->rd_detaching && rd->rd_connected;
}

static inline unsigned int redirect_max_mtu(ifnet_t ifp) {
  if_redirect_t rd;
...
static void redirect_free(if_redirect_t rd) {
...
static void redirect_release(if_redirect_t rd) {
...
static void redirect_retain(if_redirect_t rd) {
  OSIncrementAtomic(&rd->rd_retain_count);
}

static void redirect_bpf_tap(ifnet_t ifp, kern_packet_t pkt, bool input) {
  uint32_t dlt;
...
static errno_t redirect_packet_pool_make(if_redirect_t rd) {
  struct kern_pbufpool_init pp_init;
  errno_t err;
...
static int redirect_enqueue_pkt(struct nx_netif *nif, struct __kern_packet *pkt,
                                boolean_t flush, boolean_t *drop) {
  ifnet_t ifp = nif->nif_ifp;
  uint64_t qset_id;
  int err;

  if (NX_LLINK_PROV(nif->nif_nx) && ifp->if_inet_traffic_rule_count > 0 &&
      nxctl_inet_traffic_rule_find_qset_id_with_pkt(ifp->if_xname, pkt,
                                                    &qset_id) == 0) {
...
static int redirect_enqueue_mbuf(struct nx_netif *nif, struct mbuf *m,
                                 boolean_t flush, boolean_t *drop) {
  return ifnet_enqueue_mbuf(nif->nif_ifp, m, flush, drop);
}

static int redirect_tx_submit(ifnet_t delegate_ifp, struct pktq *spktq) {
  ...
  while (KPKTQ_LEN(spktq) > 0) {
    ...
    if (__probable(native)) {
      ...
      netif_ifp_inc_traffic_class_out_pkt(delegate_ifp, pkt->pkt_svc_class, 1,
                                          pkt->pkt_length);
    ...
    if (__probable(err == 0)) {
      ...
      DTRACE_SKYWALK3(enqueue__failed, ifnet_t, delegate_ifp, boolean_t, drop,
                      int, err);
      break;
    }
  }
 ...
 */
static errno_t redirect_nxdp_init(kern_nexus_domain_provider_t domprov) {
#pragma unused(domprov)
  return 0;
}

static void redirect_nxdp_fini(kern_nexus_domain_provider_t domprov) {
#pragma unused(domprov)
}

static uuid_t redirect_nx_dom_prov;

static errno_t redirect_register_nexus_domain_provider(void) {
 ...
 */
static if_redirect_t redirect_nexus_context(kern_nexus_t nexus) {
  if_redirect_t rd;
...
static errno_t redirect_nx_ring_init(kern_nexus_provider_t nxprov,
...
#pragma unused(nxprov, channel, ring_ctx)
  ...
  RD_UNLOCK(rd);
  RDLOG_INFO("%s: %s ring init", rd->rd_name, is_tx_ring ? "TX" : "RX");
  return 0;
}

static void redirect_nx_ring_fini(kern_nexus_provider_t nxprov,
                                  kern_nexus_t nexus,
                                  kern_channel_ring_t ring) {
#pragma unused(nxprov, ring)
  ...
  if (tcall != NULL) {
    ...
    RDLOG_INFO("%s: thread_call_cancel %s", rd->rd_name,
               success ? "SUCCESS" : "FAILURE");
    if (!success) {
      RD_LOCK(rd);
      if (rd->rd_doorbell_tcall_active) {
        rd->rd_waiting_for_tcall = TRUE;
        RDLOG_INFO("%s: *waiting for threadcall", rd->rd_name);
        do {
          msleep(rd, &rd->rd_lock, PZERO, "redirect threadcall", 0);
        } while (rd->rd_doorbell_tcall_active);
        RDLOG_INFO("%s: threadcall done", rd->rd_name);
        rd->rd_waiting_for_tcall = FALSE;
      }
      RD_UNLOCK(rd);
    }
    success = thread_call_free(tcall);
    RDLOG_INFO("%s: thread_call_free %s", rd->rd_name,
               success ? "SUCCESS" : "FAILURE");
    redirect_release(rd);
    VERIFY(success == TRUE);
  }
...
static errno_t redirect_nx_pre_connect(kern_nexus_provider_t nxprov,
...
static errno_t redirect_nx_connected(kern_nexus_provider_t nxprov,
                                     kern_nexus_t nexus,
                                     kern_channel_t channel) {
...
static void redirect_nx_pre_disconnect(kern_nexus_provider_t nxprov,
                                       kern_nexus_t nexus,
                                       kern_channel_t channel) {
#pragma unused(nxprov, channel)
  if_redirect_t rd;
...
static void redirect_nx_disconnected(kern_nexus_provider_t nxprov,
                                     kern_nexus_t nexus,
                                     kern_channel_t channel) {
#pragma unused(nxprov, channel)
  if_redirect_t rd;
...
static errno_t redirect_nx_slot_init(
    kern_nexus_provider_t nxprov, kern_nexus_t nexus, kern_channel_ring_t ring,
...
static void redirect_nx_slot_fini(kern_nexus_provider_t nxprov,
...
static errno_t redirect_nx_sync_tx(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus,
...
#pragma unused(nxprov)
  ...
  KPKTQ_INIT(&tx_pktq);
  while ((tx_slot = kern_channel_get_next_slot(tx_ring, tx_slot, NULL)) !=
         NULL) {
    kern_packet_t sph;
...
static boolean_t pkt_is_for_delegate(if_redirect_t rd,
...
#pragma unused(rd)
  ...
  if ((pkt->pkt_qum_qflags & QUM_F_FLOW_CLASSIFIED) == 0) {
    DTRACE_SKYWALK2(not__classified, if_redirect_t, rd, struct __kern_packet *,
                    pkt);
    return FALSE;
  }
  ...
  hdr = __unsafe_forge_bidi_indexable(
      uint8_t *, pkt->pkt_flow_ip_hdr + pkt->pkt_flow_ip_hlen, l4len);
  if (proto == IPPROTO_ICMPV6) {
    struct icmp6_hdr *icmp6;

    if (l4len < sizeof(*icmp6)) {
      RDLOG_ERR("%s: l4len(%u) < icmp6len(%lu)", rd->rd_name, l4len,
                sizeof(*icmp6));
      DTRACE_SKYWALK3(too__small__v6, if_redirect_t, rd, struct __kern_packet *,
                      pkt, uint32_t, l4len);
      return FALSE;
    }
    ...
    if (icmp6->icmp6_type == ND_ROUTER_ADVERT) {
      DTRACE_SKYWALK3(icmp6__ra, if_redirect_t, rd, struct __kern_packet *, pkt,
                      struct icmp6 *, icmp6);
      return TRUE;
    }
  }
...
static void redirect_rx_cb(void *arg, struct pktq *spktq) {
  ...
  while (KPKTQ_LEN(spktq) > 0) {
    ...
    if (rx_slot == NULL) {
      DTRACE_SKYWALK2(no__slot__drop, if_redirect_t, rd, struct __kern_packet *,
                      spkt);
...
static errno_t redirect_nx_sync_rx(kern_nexus_provider_t nxprov,
                                   kern_nexus_t nexus, kern_channel_ring_t ring,
                                   uint32_t flags) {
...
static void redirect_async_doorbell(thread_call_param_t arg0,
...
static void redirect_schedule_async_doorbell(if_redirect_t rd) {
  ...
  if (tcall != NULL) {
    ...
    tcall = thread_call_allocate_with_options(
        redirect_async_doorbell, (thread_call_param_t)rd,
...
static errno_t redirect_nx_tx_doorbell(kern_nexus_provider_t nxprov,
                                       kern_nexus_t nexus,
                                       kern_channel_ring_t ring,
                                       uint32_t flags) {
...
static errno_t redirect_netif_prepare(kern_nexus_t nexus, ifnet_t ifp) {
  if_redirect_t rd;
...
static void redirect_delegate_adv_config(ifnet_t delegate_ifp, bool enable) {
...
static errno_t redirect_nx_intf_adv_config(void *prov_ctx, bool enable) {
  ...
  if (rd->rd_intf_adv_enabled == enable) {
    RDLOG_ERR("advisory is already %s on %s", enable ? "enable" : "disable",
              if_name(rd->rd_ifp));
...
static errno_t fill_capab_interface_advisory(if_redirect_t rd, void *contents,
                                             uint32_t *len) {
  ...
  if (capab->kncia_version != KERN_NEXUS_CAPAB_INTERFACE_ADVISORY_VERSION_1) {
    DTRACE_SKYWALK2(invalid__ver, uint32_t, capab->kncia_version, uint32_t,
...
static errno_t redirect_nx_capab_config(kern_nexus_provider_t nxprov,
                                        kern_nexus_t nx,
...
static errno_t create_netif_provider_and_instance(
    if_redirect_t rd, struct ifnet_init_eparams *init_params, ifnet_t *ifp,
  ...
  if (err != 0) {
    RDLOG_ERR("%s nexus attribution creation failed, error: %d", rd->rd_name,
              err);
  ...
  err = kern_nexus_controller_register_provider(
      controller, redirect_nx_dom_prov, provider_name, &prov_init,
  ...
  err = kern_nexus_controller_alloc_net_provider_instance(
      controller, *provider, rd, NULL, instance, &net_init, ifp);
  if (err != 0) {
    RDLOG_ERR("%s alloc net provider instance failed %d", rd->rd_name, err);
    DTRACE_SKYWALK2(alloc__provider__instance__failed, if_redirect_t, rd, int,
                    err);
...
static errno_t redirect_attach_netif_nexus(
    if_redirect_t rd, struct ifnet_init_eparams *init_params, ifnet_t *ifp) {
  errno_t error = 0;
  ...
  return create_netif_provider_and_instance(
      rd, init_params, ifp, &nx->rnx_provider, &nx->rnx_instance);
}
...
static void redirect_detach_netif_nexus(if_redirect_t rd) {
...
static if_redirect_t ifnet_get_if_redirect(ifnet_t ifp) {
  return (if_redirect_t)ifnet_softc(ifp);
}

static int redirect_clone_create(struct if_clone *ifc, uint32_t unit,
                                 void *param) {
  int error;
  if_redirect_t rd;
  ...
  if ((params.ircp_type != RD_CREATE_PARAMS_TYPE &&
    ...
    RDLOG_ERR("invalid type(0x%x) or len(0x%d)", params.ircp_type,
              params.ircp_len);
    DTRACE_SKYWALK2(invalid__params, uint16_t, params.ircp_type, uint16_t,
                    params.ircp_len);
    return EINVAL;
  }
  ...
  if ((unsigned int)snprintf(rd->rd_name, sizeof(rd->rd_name), "%s%d",
                             ifc->ifc_name, unit) >= sizeof(rd->rd_name)) {
    redirect_release(rd);
    RDLOG_ERR("invalid ifc_name(%s) or unit(%d)", ifc->ifc_name, unit);
    DTRACE_SKYWALK2(invalid__name__or__unit, char *, ifc->ifc_name, uint32_t,
                    unit);
    return EINVAL;
  }
 ...
 * related info.
 */
static void redirect_cleanup(if_redirect_t rd) {
  redirect_clear_delegate(rd);
  rd->rd_intf_adv_enabled = false;
}

static int redirect_clone_destroy(ifnet_t ifp) {
  if_redirect_t rd;
...
static int if_redirect_request_copyin(user_addr_t user_addr,
                                      struct if_redirect_request *ifrr,
                                      uint64_t len) {
  int error;

  if (user_addr == USER_ADDR_NULL || len < sizeof(*ifrr)) {
    RDLOG_ERR("user_addr(0x%llx) or len(%llu) < %lu", user_addr, len,
              sizeof(*ifrr));
    error = EINVAL;
    goto done;
  }
  ...
  if (ifrr->ifrr_reserved[0] != 0 || ifrr->ifrr_reserved[1] != 0 ||
    ...
    RDLOG_ERR("reserved[0]=0x%llu, reserved[1]=0x%llu"
              "reserved[2]=0x%llu, reserved[3]=0x%llu",
              ifrr->ifrr_reserved[0], ifrr->ifrr_reserved[1],
              ifrr->ifrr_reserved[2], ifrr->ifrr_reserved[3]);
    error = EINVAL;
    goto done;
  }
done:
...
static void redirect_detach_notify(void *arg) {
  if_redirect_t __single rd = arg;

  redirect_clear_delegate(rd);
}

static int redirect_set_delegate(if_redirect_t rd, ifnet_t delegate_ifp) {
  ...
  if (rd->rd_delegate_ifp != NULL) {
    if (rd->rd_delegate_ifp == delegate_ifp) {
      ...
      DTRACE_SKYWALK2(same__ifp, if_redirect_t, rd, ifnet_t, delegate_ifp);
      RD_UNLOCK(rd);
      return EALREADY;
    } else {
  ...
  if (error != 0) {
    ...
    DTRACE_SKYWALK4(set__delegate__parent__failed, if_redirect_t, rd, ifnet_t,
                    delegate_ifp, ifnet_t, ifp, int, error);
    goto fail;
  }
  ...
  if (!ifnet_get_ioref(delegate_ifp)) {
    ...
    DTRACE_SKYWALK2(delegate__detaching, if_redirect_t, rd, ifnet_t,
                    delegate_ifp);
    error = ENXIO;
    goto fail;
  }
  ...
  if (rd->rd_ftype == IFRTYPE_FAMILY_ETHERNET) {
    ...
    error = ifnet_lladdr_copy_bytes(delegate_ifp, mac_addr, ETHER_ADDR_LEN);
    if (error != 0) {
      RDLOG_ERR("failed to get mac addr from %s, error %d",
                if_name(delegate_ifp), error);
      DTRACE_SKYWALK3(lladdr__copy__fail, if_redirect_t, rd, ifnet_t,
                      delegate_ifp, int, error);
      goto fail;
    }
    error = ifnet_set_lladdr(ifp, mac_addr, ETHER_ADDR_LEN);
    if (error != 0) {
      RDLOG_ERR("failed to set mac addr for %s, error %d", if_name(ifp), error);
      DTRACE_SKYWALK3(set__lladdr__fail, if_redirect_t, rd, ifnet_t, ifp, int,
                      error);
      goto fail;
    }
  ...
  if (!ifnet_is_fully_attached(delegate_ifp)) {
    ...
    DTRACE_SKYWALK2(delegate__detached, if_redirect_t, rd, ifnet_t,
                    delegate_ifp);
    error = ENXIO;
    goto fail;
  }
...
static void redirect_clear_delegate_locked(if_redirect_t rd) {
  ifnet_t ifp = rd->rd_ifp;
...
static void redirect_clear_delegate(if_redirect_t rd) {
  RD_LOCK(rd);
  redirect_clear_delegate_locked(rd);
  RD_UNLOCK(rd);
}

static int redirect_ioctl_set_delegate(ifnet_t ifp, user_addr_t user_addr,
                                       uint64_t len) {
  if_redirect_t rd = NULL;
  struct if_redirect_request ifrr;
  ...
  delegate_ifp = ifunit_ref(
      __unsafe_null_terminated_from_indexable(ifrr.ifrr_delegate_name));
  ...
  if (rd->rd_ftype == IFRTYPE_FAMILY_ETHERNET) {
    if (delegate_ifp->if_family != IFNET_FAMILY_ETHERNET) {
      RDLOG_ERR("%s's family %d not compatible "
                "with ethernet functional type",
                if_name(delegate_ifp), delegate_ifp->if_family);
      DTRACE_SKYWALK2(delegate__incompatible__ether, if_redirect_t, rd, ifnet_t,
                      delegate_ifp);
      error = EINVAL;
      goto done;
    }
    if (ifnet_is_low_latency(delegate_ifp)) {
      RDLOG_ERR("low latency %s cannot be a delegate", if_name(delegate_ifp));
      DTRACE_SKYWALK2(delegate__is__ll, if_redirect_t, rd, ifnet_t,
                      delegate_ifp);
      error = EINVAL;
      goto done;
    }
    ...
    if (delegate_ifp->if_family != IFNET_FAMILY_CELLULAR &&
      ...
      RDLOG_ERR("%s's family %d not compatible "
                "with cellular functional type",
                if_name(delegate_ifp), delegate_ifp->if_family);
      DTRACE_SKYWALK2(delegate__incompatible__cell, if_redirect_t, rd, ifnet_t,
                      delegate_ifp);
      error = EINVAL;
      goto done;
    }
  ...
  if (delegate_ifp->if_subfamily == IFNET_SUBFAMILY_REDIRECT) {
    ...
    DTRACE_SKYWALK2(delegate__is__redirect, if_redirect_t, rd, ifnet_t,
                    delegate_ifp);
    error = EINVAL;
    goto done;
  }
...
static int redirect_set_drvspec(ifnet_t ifp, uint64_t cmd, uint64_t len,
...
static int redirect_get_drvspec(ifnet_t ifp, uint64_t cmd, uint64_t len,
                                user_addr_t user_addr) {
...
static errno_t redirect_ioctl(ifnet_t ifp, u_long cmd, void *data) {
  if_redirect_t rd = NULL;
  ...
  case SIOCGDRVSPEC64:
    ...
    if (drv_set_command) {
      error = redirect_set_drvspec(ifp, drv_cmd, drv_len, user_addr);
    } else {
      error = redirect_get_drvspec(ifp, drv_cmd, drv_len, user_addr);
    }
    break;
  case SIOCADDMULTI:
  case SIOCDELMULTI:
...
static void redirect_if_free(ifnet_t ifp) {
  if_redirect_t rd = NULL;
 ...
 */
static errno_t redirect_demux(__unused ifnet_t ifp, mbuf_t data,
...
static errno_t redirect_del_proto(__unused ifnet_t interface,
