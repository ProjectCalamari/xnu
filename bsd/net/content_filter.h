...
#define CFM_OP_BLESS_CLIENT                                                    \
  18 /* mark a client flow as already filtered, passes a uuid */
#define CFM_OP_SET_CRYPTO_KEY                                                  \
  19 /* assign client crypto key for message signing */
...
struct cfil_msg_sock_closed {
  ...
  uint32_t cfc_op_time[CFI_MAX_TIME_LOG_ENTRY]; /* time interval in microseconds
                                                   since first event */
 ...
 */
#define CFIL_DGRAM_HAS_FILTERED_FLOWS(so)                                      \
  ((so->so_flags & SOF_CONTENT_FILTER) && (so->so_flow_db != NULL))
#define CFIL_DGRAM_FILTERED(so)                                                \
  (!IS_TCP(so) && (cfil_active_count > 0) &&                                   \
   (CFIL_DGRAM_HAS_FILTERED_FLOWS(so) ||                                       \
...
extern errno_t cfil_sock_attach(struct socket *so, struct sockaddr *local,
...
extern cfil_sock_id_t
cfil_sock_id_from_datagram_socket(struct socket *so, struct sockaddr *local,
                                  struct sockaddr *remote);

extern struct m_tag *cfil_dgram_get_socket_state(struct mbuf *m,
