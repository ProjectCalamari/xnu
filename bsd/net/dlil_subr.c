...
LCK_RW_DECLARE_ATTR(ifnet_head_lock, &ifnet_head_lock_group,
                    &dlil_lck_attributes);
LCK_MTX_DECLARE_ATTR(dlil_ifnet_lock, &dlil_lock_group, &dlil_lck_attributes);
 ...
 * Utility routines
 */
kern_return_t dlil_affinity_set(struct thread *tp, u_int32_t tag) {
  thread_affinity_policy_data_t policy;

  bzero(&policy, sizeof(policy));
  policy.affinity_tag = tag;
  return thread_policy_set(tp, THREAD_AFFINITY_POLICY, (thread_policy_t)&policy,
...
boolean_t packet_has_vlan_tag(struct mbuf *m) {
  u_int tag = 0;
 ...
 * Monitor functions.
 */
void if_flt_monitor_busy(struct ifnet *ifp) {
...
void if_flt_monitor_unbusy(struct ifnet *ifp) { if_flt_monitor_leave(ifp); }

void if_flt_monitor_enter(struct ifnet *ifp) {
  LCK_MTX_ASSERT(&ifp->if_flt_lock, LCK_MTX_ASSERT_OWNED);

  while (ifp->if_flt_busy) {
    ++ifp->if_flt_waiters;
    (void)msleep(&ifp->if_flt_head, &ifp->if_flt_lock, (PZERO - 1),
                 "if_flt_monitor", NULL);
  }
  if_flt_monitor_busy(ifp);
}

void if_flt_monitor_leave(struct ifnet *ifp) {
  LCK_MTX_ASSERT(&ifp->if_flt_lock, LCK_MTX_ASSERT_OWNED);
...
struct ifaddr *dlil_alloc_lladdr(struct ifnet *ifp,
  ...
  namelen = scnprintf(workbuf, sizeof(workbuf), "%s", if_name(ifp));
  masklen =
      offsetof(struct sockaddr_dl, sdl_data[0]) + ((namelen > 0) ? namelen : 0);
...
#undef ROUNDUP
  ...
  if (socksize > DLIL_SDLMAXLEN ||
    ...
    if (ifa == NULL || ifa == &dl_if->dl_if_lladdr.ifa) {
      dl_if_lladdr_ext =
          zalloc_permanent(sizeof(*dl_if_lladdr_ext), ZALIGN(struct ifaddr));
      ...
      dl_if_lladdr_ext =
          __unsafe_forge_single(struct dl_if_lladdr_xtra_space *, ifa);
      ifa = &dl_if_lladdr_ext->ifa;
    }
  ...
  if (namelen > 0) {
    bcopy(workbuf, addr_sdl->sdl_data,
          min(namelen, sizeof(addr_sdl->sdl_data)));
...
__private_extern__ int dlil_alloc_local_stats(struct ifnet *ifp) {
  int ret = EINVAL;
...
errno_t dlil_if_ref(struct ifnet *ifp) {
  struct dlil_ifnet *dl_if = (struct dlil_ifnet *)ifp;
...
errno_t dlil_if_free(struct ifnet *ifp) {
  struct dlil_ifnet *dl_if = (struct dlil_ifnet *)ifp;
...
void _dlil_if_release(ifnet_t ifp, bool clear_in_use) {
  ...
  ifp->if_name =
      tsnprintf(dlifp->dl_if_namestorage, sizeof(dlifp->dl_if_namestorage),
                "%s", ifp->if_name);
  /* Reset external name (name + unit) */
  ifp->if_xname =
      tsnprintf(dlifp->dl_if_xnamestorage, sizeof(dlifp->dl_if_xnamestorage),
                "%s?", ifp->if_name);
  if (clear_in_use) {
...
__private_extern__ void dlil_if_release(ifnet_t ifp) {
  _dlil_if_release(ifp, false);
}

void if_proto_ref(struct if_proto *proto) {
  os_atomic_inc(&proto->refcount, relaxed);
}

void if_proto_free(struct if_proto *proto) {
  u_int32_t oldval;
...
errno_t dlil_clat46(ifnet_t ifp, protocol_family_t *proto_family, mbuf_t *m) {
   ...
   */
  error =
      (nat464_translate_46(pbuf, off, iph->ip_tos, iph->ip_p, iph->ip_ttl,
                           src_storage, dstsock.sin6_addr, tot_len) == NT_NAT64)
          ? 0
          : -1;
  ...
  error = (nat464_translate_proto(pbuf, (struct nat464_addr *)&osrc,
                                  (struct nat464_addr *)&odst, proto, PF_INET,
                                  PF_INET6, NT_OUT, !is_first_frag) == NT_NAT64)
              ? 0
              : -1;
...
errno_t dlil_clat64(ifnet_t ifp, protocol_family_t *proto_family, mbuf_t *m) {
  ...
  if (IN6_ARE_ADDR_EQUAL(&odst, &ia6_clat_dst->ia_addr.sin6_addr)) {
     ...
     * header if needed
     */
    error = (nat464_translate_64(pbuf, off, tos, &proto, ip6h->ip6_hlim, src,
                                 *dst, tot_len, &is_first_frag) == NT_NAT64)
                ? 0
                : -1;
     ...
     * and related fields.
     */
    error = (nat464_translate_proto(
                 pbuf, (struct nat464_addr *)&osrc, (struct nat464_addr *)&odst,
                 proto, PF_INET6, PF_INET, NT_IN, !is_first_frag) == NT_NAT64)
                ? 0
                : -1;
 ...
 * Thread management
 */
void dlil_clean_threading_info(struct dlil_threading_info *inp) {
 ...
 * Lock management
 */
static errno_t _dlil_get_lock_assertion_type(ifnet_lock_assert_t what,
                                             unsigned int *type) {
  switch (what) {
...
__private_extern__ void dlil_if_lock(void) { lck_mtx_lock(&dlil_ifnet_lock); }
...
__private_extern__ void ifnet_head_lock_assert(ifnet_lock_assert_t what) {
  unsigned int type = 0;
...
__private_extern__ void ifnet_lock_assert(struct ifnet *ifp,
...
__private_extern__ void ifnet_head_done(void) { lck_rw_done(&ifnet_head_lock); }

__private_extern__ void ifnet_head_assert_exclusive(void) {
  LCK_RW_ASSERT(&ifnet_head_lock, LCK_RW_ASSERT_EXCLUSIVE);
}

static errno_t if_mcasts_update_common(struct ifnet *ifp, bool sync) {
  errno_t err;
...
errno_t if_mcasts_update_async(struct ifnet *ifp) {
  return if_mcasts_update_common(ifp, false);
}

errno_t if_mcasts_update(struct ifnet *ifp) {
