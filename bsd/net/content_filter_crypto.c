...
#define CFIL_CRYPTO_LOG_4BYTES(name)                                           \
  CFIL_CRYPTO_LOG(LOG_DEBUG, "%s \t%s: %hhX %hhX %hhX %hhX", prefix, name,     \
                  ptr[0], ptr[1], ptr[2], ptr[3])

#define CFIL_CRYPTO_LOG_8BYTES(name)                                           \
  CFIL_CRYPTO_LOG(                                                             \
      LOG_DEBUG, "%s \t%s: %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX", prefix,   \
      name, ptr[0], ptr[1], ptr[2], ptr[3], ptr[4], ptr[5], ptr[6], ptr[7])

#define CFIL_CRYPTO_LOG_16BYTES(name)                                          \
  CFIL_CRYPTO_LOG(LOG_DEBUG,                                                   \
                  "%s \t%s: %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX "     \
                  "%hhX %hhX %hhX %hhX %hhX %hhX %hhX",                        \
                  prefix, name, ptr[0], ptr[1], ptr[2], ptr[3], ptr[4],        \
...
#define CFIL_CRYPTO_LOG_28BYTES(name)                                          \
  CFIL_CRYPTO_LOG(LOG_DEBUG,                                                   \
                  ...
                  ptr[24], ptr[25], ptr[26], ptr[27])

#define CFIL_CRYPTO_LOG_32BYTES(name, prefix)                                  \
  CFIL_CRYPTO_LOG(                                                             \
      LOG_DEBUG,                                                               \
      "%s \t%s: %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX "  \
      "%hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX %hhX " \
      "%hhX %hhX %hhX %hhX %hhX %hhX",                                         \
      prefix, name, ptr[0], ptr[1], ptr[2], ptr[3], ptr[4], ptr[5], ptr[6],    \
      ptr[7], ptr[8], ptr[9], ptr[10], ptr[11], ptr[12], ptr[13], ptr[14],     \
      ptr[15], ptr[16], ptr[17], ptr[18], ptr[19], ptr[20], ptr[21], ptr[22],  \
      ptr[23], ptr[24], ptr[25], ptr[26], ptr[27], ptr[28], ptr[29], ptr[30],  \
      ptr[31])

static void cfil_crypto_print_data(cfil_crypto_data_t data,
                                   const char *prefix) {
  u_int8_t *ptr = NULL;
...
cfil_crypto_state_t cfil_crypto_init_client(cfil_crypto_key client_key) {
  if (client_key == NULL) {
    return NULL;
  }

  struct cfil_crypto_state *state;
  state = kalloc_type(struct cfil_crypto_state, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
void cfil_crypto_cleanup_state(cfil_crypto_state_t state) {
  if (state != NULL) {
    kfree_type(struct cfil_crypto_state, state);
  }
}

static void cfil_crypto_update_context(
    const struct ccdigest_info *di, cchmac_ctx_t ctx, cfil_crypto_data_t data,
    const struct iovec *__counted_by(extra_data_count) extra_data,
  ...
  for (size_t extra_idx = 0; extra_idx < extra_data_count; extra_idx++) {
    if (extra_data[extra_idx].iov_base != NULL &&
        extra_data[extra_idx].iov_len > 0) {
      cchmac_update(di, ctx, extra_data[extra_idx].iov_len,
                    extra_data[extra_idx].iov_base);
    }
  }
}

int cfil_crypto_sign_data(cfil_crypto_state_t state, cfil_crypto_data_t data,
                          const struct iovec *__counted_by(extra_data_count)
                              extra_data,
                          size_t extra_data_count,
                          cfil_crypto_signature signature,
  ...
  *signature_length = (u_int32_t)required_tag_length;
  ...
  cchmac_init(state->digest_info, ctx, sizeof(state->key), state->key);
  cfil_crypto_update_context(state->digest_info, ctx, data, extra_data,
                             extra_data_count);
  cchmac_final(state->digest_info, ctx, signature);

  if (cfil_log_level >= LOG_DEBUG) {
    cfil_crypto_print_data(data, "SIGN");
    CFIL_CRYPTO_LOG(LOG_DEBUG, "Signed data: datalen %lu",
                    sizeof(struct cfil_crypto_data));
