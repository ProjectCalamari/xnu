...
static errno_t kctl_test_connect(kern_ctl_ref kctlref, struct sockaddr_ctl *sac,
                                 void **unitinfo);
static errno_t kctl_test_disconnect(kern_ctl_ref kctlref, u_int32_t unit,
                                    void *unitinfo);
static errno_t kctl_test_send(kern_ctl_ref kctlref, u_int32_t unit,
                              void *unitinfo, mbuf_t m, int flags);
static errno_t kctl_test_setopt(kern_ctl_ref kctlref, u_int32_t unit,
                                void *unitinfo, int opt, void *data,
                                size_t len);
static errno_t kctl_test_getopt(kern_ctl_ref kctlref, u_int32_t unit,
                                void *unitinfo, int opt, void *data,
                                size_t *len);
...
#pragma unused(unitinfo)
  ...
  os_log(OS_LOG_DEFAULT, "kctl_test_connect: ref %p id %u port %u", kctlref,
         sac->sc_id, sac->sc_unit);

  error = ctl_getenqueuespace(kctlref, sac->sc_unit, &space);
  if (error != 0) {
    os_log(OS_LOG_DEFAULT, "kctl_test_connect; ctl_getenqueuespace failed %d",
           error);
    goto out;
  }
...
#pragma unused(unitinfo)
  ...
  if (error != 0) {
    os_log(OS_LOG_DEFAULT,
           "kctl_test_disconnect; ctl_getenqueuespace failed %d", error);
    goto out;
  }
  os_log(OS_LOG_DEFAULT, "kctl_test_disconnect: ctl_getenqueuespace %ld",
         space);
out:
...
#pragma unused(unitinfo, flags)
  ...
  if (error != 0) {
    os_log(OS_LOG_DEFAULT, "kctl_test_send: ctl_enqueuembuf() failed %d",
           error);
    mbuf_freem(m);
  }
...
#pragma unused(unitinfo)
  ...
  if (error != 0) {
    os_log(OS_LOG_DEFAULT,
           "kctl_test_send_list: ctl_enqueuembuf_list() error %d unsent "
           "packets %u",
           error, unsent_count);
  }
  return error;
...
int kctl_test_init(void) {
  ...
  if (error == 0) {
    ...
    os_log(OS_LOG_DEFAULT,
           "kctl_test_register: OK kctlref %p kctlid %x ctl_sendsize %u "
           "ctl_recvsize %u",
           kctl_test_ref, kctl_test_id, kern_ctl_reg.ctl_sendsize,
           kern_ctl_reg.ctl_recvsize);
  } else {
