...
#define IF_XNAME(ifp)                                                          \
  ((ifp) != NULL ? (ifp)->if_xname : (const char *__null_terminated) "")
...
SYSCTL_OPAQUE(_net_link_generic_system_port_used, OID_AUTO, current_wakeuuid,
              CTLFLAG_RD | CTLFLAG_LOCKED, current_wakeuuid, sizeof(uuid_t),
              "S,uuid_t", "");
...
SYSCTL_INT(_net_link_generic_system_port_used, OID_AUTO, use_test_wakeuuid,
           CTLFLAG_RW | CTLFLAG_LOCKED, &use_test_wakeuuid, 0, "");
...
SYSCTL_OPAQUE(_net_link_generic_system_port_used, OID_AUTO, test_wakeuuid,
              CTLFLAG_RD | CTLFLAG_LOCKED, test_wakeuuid, sizeof(uuid_t),
              "S,uuid_t", "");
...
SYSCTL_PROC(_net_link_generic_system_port_used, OID_AUTO, use_fake_lpw,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &use_fake_lpw, 0,
            &sysctl_use_fake_lpw, "I", "");
...
SYSCTL_PROC(_net_link_generic_system_port_used_mark_wake_packet, OID_AUTO,
            local_port, CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
            &mark_wake_packet_local_port, 0, &sysctl_mark_wake_packet_port, "I",
            "");

static int mark_wake_packet_remote_port = 0;
SYSCTL_PROC(_net_link_generic_system_port_used_mark_wake_packet, OID_AUTO,
            remote_port, CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED,
            &mark_wake_packet_remote_port, 0, &sysctl_mark_wake_packet_port,
            "I", "");

static int mark_wake_packet_ipproto = 0;
SYSCTL_INT(_net_link_generic_system_port_used_mark_wake_packet, OID_AUTO,
           ipproto, CTLFLAG_RW | CTLFLAG_LOCKED, &mark_wake_packet_ipproto, 0,
           "");
...
SYSCTL_INT(_net_link_generic_system_port_used, OID_AUTO, verbose,
           CTLFLAG_RW | CTLFLAG_LOCKED, &if_ports_used_verbose, 0, "");
...
static SYSCTL_PROC(_net_link_generic_system_port_used, OID_AUTO,
                   ...
                   CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
                   sysctl_wakeuuid_not_set_last_time, "S,timeval", "");
...
static SYSCTL_PROC(_net_link_generic_system_port_used, OID_AUTO,
                   ...
                   sysctl_wakeuuid_not_set_last_if, "A", "");

struct timeval wakeuuid_last_update_time;
int sysctl_wakeuuid_last_update_time SYSCTL_HANDLER_ARGS;
static SYSCTL_PROC(_net_link_generic_system_port_used, OID_AUTO,
                   wakeuuid_last_update_time,
                   CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
                   sysctl_wakeuuid_last_update_time, "S,timeval", "");
...
static LCK_MTX_DECLARE(net_port_entry_head_lock,
...
static KALLOC_TYPE_DEFINE(net_port_entry_zone, struct net_port_entry,
                          NET_KT_DEFAULT);

static SLIST_HEAD(net_port_entry_list, net_port_entry)
    net_port_entry_list = SLIST_HEAD_INITIALIZER(&net_port_entry_list);
 ...
 *
 * The hash masks uses the least significant bits so we have to use host byte
 * order when applying the mask because the LSB have more entropy that the MSB
 * (most local ports are in the high dynamic port range)
 */
...
static TAILQ_HEAD(net_port_entry_hash_table,
                  net_port_entry) *__indexable net_port_entry_hash_table = NULL;

/*
 * For some types of physical interface we need to delay the notification of
 * wake packet events until a user land interface controller confirms the AP
 * wake was caused by its packet
 */
...
#define npi_ev_wake_pkt_unattributed                                           \
  npi_ev_wake_pkt_._npi_ev_wake_pkt_unattributed
...
SYSCTL_PROC(_net_link_generic_system_port_used, OID_AUTO, wake_pkt_event_notify,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED | CTLFLAG_MASKED |
                CTLFLAG_ANYBODY,
            0, 0, sysctl_wake_pkt_event_notify, "I", "");

/* Bitmap of the interface families to delay the notification of wake packet
 * events */
...
SYSCTL_PROC(_net_link_generic_system_port_used, OID_AUTO,
            wake_pkt_event_delay_if_families,
...
/*
 * delay_wake_pkt_event hold the current wake packet event that is delayed
 * waiting for confirmation from a userspace agent It can be overwritten as a
 * wake packet makes its way up the stack
 */
static struct net_port_info_wake_pkt_event delay_wake_pkt_event;

int sysctl_last_attributed_wake_event SYSCTL_HANDLER_ARGS;
static SYSCTL_PROC(_net_link_generic_system_port_used, OID_AUTO,
                   last_attributed_wake_event,
                   CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
                   sysctl_last_attributed_wake_event,
                   "S,net_port_info_wake_event", "");

int sysctl_last_unattributed_wake_event SYSCTL_HANDLER_ARGS;
static SYSCTL_PROC(_net_link_generic_system_port_used, OID_AUTO,
                   last_unattributed_wake_event,
                   CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
                   sysctl_last_unattributed_wake_event,
                   "S,net_port_info_una_wake_event", "");

os_log_t wake_packet_log_handle = NULL;

static bool is_wake_pkt_event_delay(uint32_t ifrtype);

static bool _if_need_delayed_wake_pkt_event_inner(struct ifnet *ifp) {
...
static bool if_need_delayed_wake_pkt_event(struct ifnet *ifp) {
  if (ifp != NULL) {
...
void if_ports_used_init(void) {
  ...
  net_port_entry_hash_table = zalloc_permanent(
      NPE_HASH_BUCKET_COUNT * sizeof(*net_port_entry_hash_table), ZALIGN_PTR);
}

bool if_is_lpw_enabled(struct ifnet *ifp) {
...
void if_exit_lpw(struct ifnet *ifp, const char *lpw_exit_reason) {
  ...
  IOPMNetworkStackFullWake(kIOPMNetworkStackFullWakeFlag,
                           "Network.ConnectionNotIdle");
}

static void net_port_entry_list_clear(void) {
  ...
  while ((npe = SLIST_FIRST(&net_port_entry_list)) != NULL) {
    ...
    TAILQ_REMOVE(NPE_HASH_HEAD(npe->npe_npi.npi_local_port), npe,
                 npe_hash_next);
...
static bool get_test_wake_uuid(uuid_string_t wakeuuid_str) {
...
static bool is_wakeuuid_set(void) {
...
void if_ports_used_update_wakeuuid(struct ifnet *ifp) {
  uuid_t wakeuuid;
  ...
  if (__improbable(use_test_wakeuuid)) {
    ...
    wakeuuid_is_set =
        IOPMCopySleepWakeUUIDKey(wakeuuid_str, sizeof(wakeuuid_str));
  }

  if (wakeuuid_is_set) {
    if (uuid_parse(wakeuuid_str, wakeuuid) != 0) {
      os_log(wake_packet_log_handle,
             "if_ports_used_update_wakeuuid: IOPMCopySleepWakeUUIDKey got bad "
             "value %s\n",
             wakeuuid_str);
...
static bool net_port_info_equal(const struct net_port_info *x,
...
static bool net_port_info_has_entry(const struct net_port_info *npi) {
  struct net_port_entry *npe;
  bool found = false;
  int32_t count = 0;
...
static bool net_port_info_add_entry(const struct net_port_info *npi) {
  ...
  if (__improbable(is_wakeuuid_set() == false)) {
    ...
    if (if_ports_used_verbose > 0) {
      os_log(wake_packet_log_handle,
             ...
             __func__, ntohs(npi->npi_local_port), npi->npi_flags,
             npi->npi_if_index, npi->npi_owner_pid, npi->npi_effective_pid);
    }
    return false;
  }
  ...
  if (net_port_info_has_entry(npi) == false) {
    ...
    if (if_ports_used_verbose > 1) {
      os_log(wake_packet_log_handle,
             "%s: num %u for "
             "port: %u flags: 0x%x if: %u pid: %u epid %u",
             __func__, num, ntohs(npi->npi_local_port), npi->npi_flags,
             npi->npi_if_index, npi->npi_owner_pid, npi->npi_effective_pid);
    }
  } else {
...
#pragma unused(oidp, arg1, arg2)
  ...
  error = SYSCTL_OUT(req, test_wakeuuid, MIN(sizeof(uuid_t), req->oldlen));
...
static int sysctl_timeval(struct sysctl_req *req, const struct timeval *tv) {
...
#pragma unused(oidp, arg1, arg2)
  ...
  return SYSCTL_OUT(req, &wakeuuid_not_set_last_if,
                    strbuflen(wakeuuid_not_set_last_if) + 1);
}

int sysctl_if_ports_used_stats SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
...
#pragma unused(oidp, arg1, arg2)
  ...
  if ((error = priv_check_cred(
           kauth_cred_get(), PRIV_NET_PRIVILEGED_NETWORK_STATISTICS, 0)) != 0) {
    return EPERM;
  }
  lck_mtx_lock(&net_port_entry_head_lock);
  ...
  if (req->oldptr == USER_ADDR_NULL) {
    ...
    cnt += cnt >> 4;
    req->oldidx = sizeof(struct xnpigen) + cnt * sizeof(struct net_port_info);
    goto done;
  }
  ...
  SLIST_FOREACH(npe, &net_port_entry_list, npe_list_next) {
    error = SYSCTL_OUT(req, &npe->npe_npi, sizeof(struct net_port_info));
    if (error != 0) {
...
__private_extern__ bool if_ports_used_add_inpcb(const uint32_t ifindex,
  ...
  if (SOCK_PROTO(so) == IPPROTO_TCP) {
    ...
    os_log(wake_packet_log_handle, "%s: unexpected protocol %u for inp %p",
           __func__, SOCK_PROTO(inp->inp_socket), inp);
    return false;
  }
  ...
  if ((inp->inp_vflag & INP_IPV6) != 0 &&
    ...
    memcpy(&npi.npi_local_addr_in6, &inp->in6p_laddr, sizeof(struct in6_addr));
    ...
    memcpy(&npi.npi_local_addr_in6, &inp->in6p_laddr, sizeof(struct in6_addr));
    memcpy(&npi.npi_foreign_addr_in6, &inp->in6p_faddr,
  ...
  if (so->last_pid != 0) {
    proc_name(so->last_pid, npi.npi_owner_pname, sizeof(npi.npi_owner_pname));
...
#if SKYWALK
__private_extern__ bool if_ports_used_add_flow_entry(
    const struct flow_entry *fe, const uint32_t ifindex,
  ...
  if (nfi->nfi_laddr.sa.sa_family == AF_INET) {
    ...
    memcpy(&npi.npi_local_addr_in6, &nfi->nfi_laddr.sin6.sin6_addr,
           sizeof(struct in6_addr));
    memcpy(&npi.npi_foreign_addr_in6, &nfi->nfi_faddr.sin6.sin6_addr,
           sizeof(struct in6_addr));
  ...
  if (proc != PROC_NULL) {
    proc_getexecutableuuid(proc, npi.npi_owner_uuid,
                           sizeof(npi.npi_owner_uuid));
    proc_rele(proc);
  }
...
static void net_port_info_log_npi(const char *s,
                                  const struct net_port_info *npi) {
  ...
  if (npi->npi_flags & NPIF_IPV4) {
    inet_ntop(PF_INET, &npi->npi_local_addr_in.s_addr, lbuf, sizeof(lbuf));
    inet_ntop(PF_INET, &npi->npi_foreign_addr_in.s_addr, fbuf, sizeof(fbuf));
  } else if (npi->npi_flags & NPIF_IPV6) {
    inet_ntop(PF_INET6, &npi->npi_local_addr_in6, lbuf, sizeof(lbuf));
    inet_ntop(PF_INET6, &npi->npi_foreign_addr_in6, fbuf, sizeof(fbuf));
  }
  os_log(wake_packet_log_handle,
         "%s net_port_info if_index %u arch %s family %s proto %s local %s:%u "
         "foreign %s:%u pid: %u epid %u",
         s != NULL ? s : "", npi->npi_if_index,
         (npi->npi_flags & NPIF_SOCKET)    ? "so"
         : (npi->npi_flags & NPIF_CHANNEL) ? "ch"
                                           : "unknown",
         (npi->npi_flags & NPIF_IPV4)   ? "ipv4"
         : (npi->npi_flags & NPIF_IPV6) ? "ipv6"
                                        : "unknown",
         npi->npi_flags & NPIF_TCP   ? "tcp"
         : npi->npi_flags & NPIF_UDP ? "udp"
         : npi->npi_flags & NPIF_ESP ? "esp"
                                     : "unknown",
         lbuf, ntohs(npi->npi_local_port), fbuf, ntohs(npi->npi_foreign_port),
         npi->npi_owner_pid, npi->npi_effective_pid);
}

/*
 * net_port_info_match_npi() returns true for an exact match that does not have
 * "no wake" set
 */
#define NPI_MATCH_IPV4 (NPIF_IPV4 | NPIF_TCP | NPIF_UDP)
#define NPI_MATCH_IPV6 (NPIF_IPV6 | NPIF_TCP | NPIF_UDP)

static bool net_port_info_match_npi(struct net_port_entry *npe,
                                    const struct net_port_info *in_npi,
  ...
  if (npe->npe_npi.npi_if_index != in_npi->npi_if_index &&
      !((npe->npe_npi.npi_flags & NPIF_COMPLINK) &&
        (in_npi->npi_flags & NPIF_COMPLINK))) {
    return false;
  }
  ...
  if (npe->npe_npi.npi_local_port != in_npi->npi_local_port ||
      ((npe->npe_npi.npi_flags & NPI_MATCH_IPV4) !=
           (in_npi->npi_flags & NPI_MATCH_IPV4) &&
       (npe->npe_npi.npi_flags & NPI_MATCH_IPV6) !=
           (in_npi->npi_flags & NPI_MATCH_IPV6))) {
    return false;
  }
  ...
  if (npe->npe_npi.npi_foreign_port == in_npi->npi_foreign_port) {
    if ((npe->npe_npi.npi_flags & NPIF_IPV4) &&
        (npe->npe_npi.npi_flags & NPIF_IPV4)) {
      if (in_npi->npi_local_addr_in.s_addr ==
              npe->npe_npi.npi_local_addr_in.s_addr &&
          in_npi->npi_foreign_addr_in.s_addr ==
              npe->npe_npi.npi_foreign_addr_in.s_addr) {
    ...
    if ((npe->npe_npi.npi_flags & NPIF_IPV6) &&
        (npe->npe_npi.npi_flags & NPIF_IPV6)) {
 ...
 *
 */
static bool net_port_info_find_match(struct net_port_info *in_npi) {
  struct net_port_entry *npe;
  ...
  if (best_match != NULL) {
    ...
    strbufcpy(in_npi->npi_effective_pname,
              best_match->npe_npi.npi_effective_pname);
    uuid_copy(in_npi->npi_owner_uuid, best_match->npe_npi.npi_owner_uuid);
    uuid_copy(in_npi->npi_effective_uuid,
              best_match->npe_npi.npi_effective_uuid);
  }
  lck_mtx_unlock(&net_port_entry_head_lock);
...
#if (DEBUG || DEVELOPMENT)
static void
net_port_info_log_una_wake_event(const char *s,
                                 struct net_port_info_una_wake_event *ev) {
  char lbuf[MAX_IPv6_STR_LEN] = {};
  char fbuf[MAX_IPv6_STR_LEN] = {};

  if (ev->una_wake_pkt_flags & NPIF_IPV4) {
    inet_ntop(PF_INET, &ev->una_wake_pkt_local_addr_._in_a_4.s_addr, lbuf,
              sizeof(lbuf));
    inet_ntop(PF_INET, &ev->una_wake_pkt_foreign_addr_._in_a_4.s_addr, fbuf,
              sizeof(fbuf));
  } else if (ev->una_wake_pkt_flags & NPIF_IPV6) {
    inet_ntop(PF_INET6, &ev->una_wake_pkt_local_addr_._in_a_6.s6_addr, lbuf,
              sizeof(lbuf));
    inet_ntop(PF_INET6, &ev->una_wake_pkt_foreign_addr_._in_a_6.s6_addr, fbuf,
              sizeof(fbuf));
  }
  os_log(wake_packet_log_handle,
         "%s if %s (%u) phy_if %s proto %s local %s:%u foreign %s:%u len: %u "
         "datalen: %u cflags: 0x%x proto: %u lpw: %d",
         s != NULL ? s : "", ev->una_wake_pkt_ifname, ev->una_wake_pkt_if_index,
         ev->una_wake_pkt_phy_ifname,
         ev->una_wake_pkt_flags & NPIF_TCP   ? "tcp"
         : ev->una_wake_pkt_flags & NPIF_UDP ? "udp"
         : ev->una_wake_pkt_flags & NPIF_ESP ? "esp"
                                             : "unknown",
         lbuf, ntohs(ev->una_wake_pkt_local_port), fbuf,
         ntohs(ev->una_wake_pkt_foreign_port), ev->una_wake_pkt_total_len,
         ev->una_wake_pkt_data_len, ev->una_wake_pkt_control_flags,
         ev->una_wake_pkt_proto, ev->una_wake_pkt_flags & NPIF_LPW ? 1 : 0);
}

static void net_port_info_log_wake_event(const char *s,
                                         struct net_port_info_wake_event *ev) {
  char lbuf[MAX_IPv6_STR_LEN] = {};
  char fbuf[MAX_IPv6_STR_LEN] = {};

  if (ev->wake_pkt_flags & NPIF_IPV4) {
    inet_ntop(PF_INET, &ev->wake_pkt_local_addr_._in_a_4.s_addr, lbuf,
              sizeof(lbuf));
    inet_ntop(PF_INET, &ev->wake_pkt_foreign_addr_._in_a_4.s_addr, fbuf,
              sizeof(fbuf));
  } else if (ev->wake_pkt_flags & NPIF_IPV6) {
    inet_ntop(PF_INET6, &ev->wake_pkt_local_addr_._in_a_6.s6_addr, lbuf,
              sizeof(lbuf));
    inet_ntop(PF_INET6, &ev->wake_pkt_foreign_addr_._in_a_6.s6_addr, fbuf,
              sizeof(fbuf));
  }
  os_log(wake_packet_log_handle,
         "%s if %s (%u) phy_if %s proto %s local %s:%u foreign %s:%u len: %u "
         "datalen: %u cflags: 0x%x proc %s eproc %s idle %d lpw %d",
         s != NULL ? s : "", ev->wake_pkt_ifname, ev->wake_pkt_if_index,
         ev->wake_pkt_phy_ifname,
         ev->wake_pkt_flags & NPIF_TCP   ? "tcp"
         : ev->wake_pkt_flags            ? "udp"
         : ev->wake_pkt_flags & NPIF_ESP ? "esp"
                                         : "unknown",
         lbuf, ntohs(ev->wake_pkt_port), fbuf, ntohs(ev->wake_pkt_foreign_port),
         ev->wake_pkt_total_len, ev->wake_pkt_data_len,
         ev->wake_pkt_control_flags, ev->wake_pkt_owner_pname,
         ev->wake_pkt_effective_pname,
 ...
 *
 * 2) We try to attribute a packet to a flow or not based on the physical
 * interface. If we find a flow, then the physical interface is the same as the
 * interface used by the TCP/UDP flow.
 *
 * 3) If the packet is tunneled or redirected we are going to do the attribution
 * again and the physical will be different from the interface used the TCP/UDP
 * flow.
 */
static bool is_wake_pkt_event_delay(uint32_t ifrtype) {
  ...
  if (bitstr_test((bitstr_t *)&npi_wake_packet_event_delay_if_families,
                  ifrtype)) {
    return true;
  }
  return false;
...
static int if_set_wake_physical_interface(struct ifnet *ifp) {
  /*
  ...
  if (last_wake_phy_if_set == true) {
    ...
    os_log(wake_packet_log_handle,
           "if_set_wake_physical_interface ignored on %s because already set "
           "on %s",
...
static void deliver_unattributed_wake_packet_event(
...
#if (DEBUG || DEVELOPMENT)
  net_port_info_log_una_wake_event("unattributed wake packet event",
                                   event_data);
#endif /* (DEBUG || DEVELOPMENT) */
}

static void deliver_attributed_wake_packet_event(
...
static bool is_unattributed_wake_already_notified(struct net_port_info *npi) {
  bool retval = false;
...
static void check_for_existing_delayed_wake_event() {
  /*
...
static void if_notify_unattributed_wake_common(
    struct ifnet *ifp, struct net_port_info *npi,
    struct net_port_info_una_wake_event *event_data) {
...
#if (DEBUG || DEVELOPMENT)
    if (if_ports_used_verbose > 0) {
      net_port_info_log_una_wake_event("delay unattributed wake packet event",
                                       event_data);
    }
#endif /* (DEBUG || DEVELOPMENT) */
...
static void if_notify_unattributed_wake_mbuf(struct ifnet *ifp, struct mbuf *m,
                                             struct net_port_info *npi,
                                             uint32_t pkt_total_len,
                                             uint32_t pkt_data_len,
                                             uint16_t pkt_control_flags,
                                             uint16_t proto) {
  struct net_port_info_una_wake_event event_data = {};
  ...
  event_data.una_wake_pkt_if_info.npi_if_functional_type =
      if_functional_type(ifp, true);
  ...
  event_data.una_wake_pkt_phy_if_info.npi_if_subfamily =
      last_wake_phy_if_subfamily;
  event_data.una_wake_pkt_phy_if_info.npi_if_functional_type =
      last_wake_phy_if_functional_type;

  event_data.una_wake_ptk_len = m->m_pkthdr.len > NPI_MAX_UNA_WAKE_PKT_LEN
                                    ? NPI_MAX_UNA_WAKE_PKT_LEN
                                    : (u_int16_t)m->m_pkthdr.len;
...
static bool is_attributed_wake_already_notified(struct net_port_info *npi) {
...
static void if_notify_wake_packet(struct ifnet *ifp, struct net_port_info *npi,
                                  uint32_t pkt_total_len, uint32_t pkt_data_len,
                                  uint16_t pkt_control_flags) {
  struct net_port_info_wake_event event_data = {};
  ...
  strlcpy(event_data.wake_pkt_ifname, IF_XNAME(ifp),
          sizeof(event_data.wake_pkt_ifname));

  event_data.wake_pkt_if_info.npi_if_family = ifp->if_family;
  event_data.wake_pkt_if_info.npi_if_subfamily = ifp->if_subfamily;
  event_data.wake_pkt_if_info.npi_if_functional_type =
      if_functional_type(ifp, true);
  ...
  event_data.wake_pkt_phy_if_info.npi_if_functional_type =
      last_wake_phy_if_functional_type;
   ...
   */
  if (if_is_lpw_enabled(ifp) && (npi->npi_flags & NPIF_CONNECTION_IDLE) != 0) {
    goto deliver;
  }
...
#if (DEBUG || DEVELOPMENT)
    if (if_ports_used_verbose > 0) {
      net_port_info_log_wake_event("delay attributed wake packet event",
                                   &event_data);
    }
#endif /* (DEBUG || DEVELOPMENT) */
...
static bool is_encapsulated_esp(struct mbuf *m, size_t data_offset) {
  /*
  ...
  if (data_len == 1) {
    ...
    if (error != 0) {
      os_log(wake_packet_log_handle, "%s: mbuf_copydata(ESP_HDR_SIZE) error %d",
             __func__, error);
    } else if (payload[0] == 0 && payload[1] == 0 && payload[2] == 0 &&
               payload[3] == 0) {
      return false;
    }
  }
...
static void log_hexdump(os_log_t log_handle, void *__sized_by(len) data,
                        size_t len) {
  size_t i, j, k;
...
__attribute__((noinline)) static void log_wake_mbuf(struct ifnet *ifp,
                                                    struct mbuf *m) {
  char buffer[64];
  size_t buflen = MIN(mbuf_pkthdr_len(m), sizeof(buffer));

  os_log(wake_packet_log_handle, "wake packet from %s len %d", ifp->if_xname,
         m_pktlen(m));
...
void if_ports_used_match_mbuf(struct ifnet *ifp, protocol_family_t proto_family,
  ...
  if (proto_family == PF_INET) {
    ...
    if (error != 0) {
      os_log(wake_packet_log_handle,
             "if_ports_used_match_mbuf: mbuf_copydata(ip) error %d", error);
      goto failed;
    }
    npi.npi_flags |= NPIF_IPV4;
    ...
    memcpy(&npi.npi_foreign_addr_in6, &ip6_hdr.ip6_src,
           sizeof(struct in6_addr));
    ...
    if (l4_proto == IPPROTO_FRAGMENT) {
      ...
      error = mbuf_copydata(m, sizeof(struct ip6_hdr), sizeof(struct ip6_frag),
                            &ip6_frag);
      if (error != 0) {
        os_log(wake_packet_log_handle,
...
failed:
  ...
  if (found) {
    if_notify_wake_packet(ifp, &npi, pkt_total_len, pkt_data_len,
                          pkt_control_flags);
  } else {
    if_notify_unattributed_wake_mbuf(ifp, m, &npi, pkt_total_len, pkt_data_len,
                                     pkt_control_flags, pkt_proto);
  }
}
...
if_notify_unattributed_wake_pkt(struct ifnet *ifp, struct __kern_packet *pkt,
  ...
  event_data.una_wake_ptk_len =
      pkt->pkt_length - offset > NPI_MAX_UNA_WAKE_PKT_LEN
          ? NPI_MAX_UNA_WAKE_PKT_LEN
          : (u_int16_t)pkt->pkt_length - offset;
...
__attribute__((noinline)) static void log_wake_pkt(struct ifnet *ifp,
...
void if_ports_used_match_pkt(struct ifnet *ifp, struct __kern_packet *pkt) {
  ...
  if ((pkt->pkt_pflags & PKT_F_WAKE_PKT) == 0) {
    if_ports_used_stats.ifpu_match_wake_pkt_no_flag += 1;
    os_log_error(wake_packet_log_handle,
                 "%s: called PKT_F_WAKE_PKT not set from %s", __func__,
                 IF_XNAME(ifp));
    return;
  }
  ...
  pkt_total_len =
      pkt->pkt_flow_ip_hlen + pkt->pkt_flow_tcp_hlen + pkt->pkt_flow_ulen;
  ...
  case IPPROTO_TCP: {
    ...
    struct tcphdr *__single tcp =
        __unsafe_forge_single(struct tcphdr *, pkt->pkt_flow_tcp_hdr);
    if (tcp == NULL) {
      os_log(wake_packet_log_handle,
             "%s: pkt with unassigned TCP header from %s", __func__,
             IF_XNAME(ifp));
      if_ports_used_stats.ifpu_incomplete_tcp_hdr_pkt += 1;
      goto failed;
    }
  ...
  case IPPROTO_UDP: {
    ...
    struct udphdr *__single uh =
        __unsafe_forge_single(struct udphdr *, pkt->pkt_flow_udp_hdr);
    if (uh == NULL) {
      os_log(wake_packet_log_handle,
             "%s: pkt with unassigned UDP header from %s", __func__,
             IF_XNAME(ifp));
      if_ports_used_stats.ifpu_incomplete_udp_hdr_pkt += 1;
      goto failed;
    }
...
failed:
  ...
  if (found) {
    ...
    if_notify_unattributed_wake_pkt(ifp, pkt, &npi, pkt_total_len, pkt_data_len,
                                    pkt_control_flags, pkt_proto);
  }
}
#endif /* SKYWALK */
...
#pragma unused(oidp, arg1, arg2)
  ...
  if (last_wake_pkt_event.npi_wp_code == KEV_POWER_WAKE_PACKET) {
    memcpy(&net_port_info_wake_event,
           &last_wake_pkt_event.npi_ev_wake_pkt_attributed, len);
  }
  lck_mtx_unlock(&net_port_entry_head_lock);
...
#pragma unused(oidp, arg1, arg2)
  ...
  if (last_wake_pkt_event.npi_wp_code == KEV_POWER_UNATTRIBUTED_WAKE) {
    memcpy(&net_port_info_una_wake_event,
           &last_wake_pkt_event.npi_ev_wake_pkt_unattributed, len);
  }
  lck_mtx_unlock(&net_port_entry_head_lock);
...
#pragma unused(oidp, arg1, arg2)
  ...
  os_log(wake_packet_log_handle,
         "sysctl_wake_pkt_event_notify proc %s:%u val %u "
         "last_wake_phy_if_delay_wake_pkt %d last_wake_phy_if_family %u "
         "delay_wake_pkt_event %d",
         proc_best_name(current_proc()), proc_selfpid(), if_family,
         last_wake_phy_if_delay_wake_pkt, last_wake_phy_if_family,
         delay_wake_pkt_event.npi_wp_code);
#if (DEBUG || DEVELOPMENT)
  if (if_ports_used_verbose > 0) {
    if (delay_wake_pkt_event.npi_wp_code == KEV_POWER_WAKE_PACKET) {
      net_port_info_log_wake_event(
          "sysctl_wake_pkt_event_notify",
          &delay_wake_pkt_event.npi_ev_wake_pkt_attributed);
    } else if (delay_wake_pkt_event.npi_wp_code ==
               KEV_POWER_UNATTRIBUTED_WAKE) {
      net_port_info_log_una_wake_event(
          "sysctl_wake_pkt_event_notify",
  ...
  if (last_wake_phy_if_delay_wake_pkt == true &&
      val == last_wake_phy_if_family) {
    last_wake_phy_if_delay_wake_pkt = false;

    if (delay_wake_pkt_event.npi_wp_code == KEV_POWER_WAKE_PACKET) {
      if (is_attributed_wake_already_notified(NULL) == false) {
        deliver_attributed_wake_packet_event(
            &delay_wake_pkt_event.npi_ev_wake_pkt_attributed);
      } else {
        os_log(wake_packet_log_handle,
      ...
      if (is_unattributed_wake_already_notified(NULL)) {
        deliver_unattributed_wake_packet_event(
            &delay_wake_pkt_event.npi_ev_wake_pkt_unattributed);
      } else {
        os_log(
            wake_packet_log_handle,
...
static void if_set_delay_wake_flags(ifnet_t ifp, bool delay) {
  if (delay) {
...
#pragma unused(oidp, arg1, arg2)
  ...
  if (npi_wake_packet_event_delay_if_families != 0) {
    ...
    if (error != 0) {
      os_log_error(wake_packet_log_handle, "%s: ifnet_list_get_all() failed %d",
                   __func__, error);
  ...
  os_log(wake_packet_log_handle,
         "sysctl_wake_pkt_event_delay_if_families proc %s:%u "
         "npi_wake_packet_event_delay_if_families 0x%x -> 0x%x",
         proc_best_name(current_proc()), proc_selfpid(), old_value,
         npi_wake_packet_event_delay_if_families);
...
void init_inband_wake_pkt_tagging_for_family(struct ifnet *ifp) {
  ...
  if (if_ports_used_verbose || ifp->if_family == IFNET_FAMILY_CELLULAR) {
    os_log(wake_packet_log_handle,
           "interface %s initialized INBAND_WAKE_PKT_TAGGING %d "
           "DELAYWAKEPKTEVENT %d",
           ifp->if_xname, ifp->if_xflags & IFXF_INBAND_WAKE_PKT_TAGGING ? 1 : 0,
           ifp->if_xflags & IFXF_DELAYWAKEPKTEVENT ? 1 : 0);
  }
}
...
bool check_wake_mbuf(ifnet_t ifp, protocol_family_t protocol_family,
                     mbuf_ref_t m) {
  uint8_t ipproto = 0;
  size_t offset = 0;
...
bool check_wake_pkt(ifnet_t ifp __unused, struct __kern_packet *pkt) {
  /* The protocol and interface must both be specified */
  if (mark_wake_packet_ipproto == 0 || mark_wake_packet_if[0] == 0) {
    return false;
  }
