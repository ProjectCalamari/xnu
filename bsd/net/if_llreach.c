...
void ifnet_llreach_ifattach(struct ifnet *ifp, boolean_t reuse) {
...
void ifnet_llreach_ifdetach(struct ifnet *ifp) {
#pragma unused(ifp)
  /*
 ...
 */
static __inline int iflr_cmp(const struct if_llreach *a,
                             const struct if_llreach *b) {
  return memcmp(&a->lr_key, &b->lr_key, sizeof(a->lr_key));
}

static __inline int iflr_reachable(struct if_llreach *lr, int cmp_delta,
                                   u_int64_t tval) {
  u_int64_t now;
  u_int64_t expire;
...
int ifnet_llreach_reachable(struct if_llreach *lr) {
  /*
...
int ifnet_llreach_reachable_delta(struct if_llreach *lr, u_int64_t tval) {
  /*
   * Check whether the cache is too old to be trusted.
   */
...
void ifnet_llreach_set_reachable(struct ifnet *ifp, u_int16_t llproto,
...
struct if_llreach *ifnet_llreach_alloc(struct ifnet *ifp, u_int16_t llproto,
                                       void *__sized_by(alen) addr,
                                       unsigned int alen,
                                       u_int32_t llreach_base) {
  struct if_llreach find, *lr;
  struct timeval cnow;
...
void ifnet_llreach_free(struct if_llreach *lr) {
  struct ifnet *ifp;
  ...
  if (!(lr->lr_debug & IFD_ATTACHED)) {
    panic("%s: Attempt to detach an unattached llreach lr=%p", __func__, lr);
    /* NOTREACHED */
  }
  lr->lr_debug &= ~IFD_ATTACHED;
...
u_int64_t ifnet_llreach_up2calexp(struct if_llreach *lr, u_int64_t uptime) {
  ...
  if (uptime != 0) {
     ...
     */
    lr->lr_basecal += (cnow.tv_sec - lr->lr_basecal) - (unow - lr->lr_baseup);

    calendar = lr->lr_basecal + lr->lr_reachable + (uptime - lr->lr_baseup);
  }

  return calendar;
}

u_int64_t ifnet_llreach_up2upexp(struct if_llreach *lr, u_int64_t uptime) {
  return lr->lr_reachable + uptime;
}

int ifnet_llreach_get_defrouter(struct ifnet *ifp, sa_family_t af,
  ...
  VERIFY(ifp != NULL && iflri != NULL && (af == AF_INET || af == AF_INET6));
  ...
  if (rt != NULL) {
    ...
    RT_LOCK(rt);
    if ((rt->rt_flags & RTF_GATEWAY) && (gwrt = rt->rt_gwroute) != NULL &&
...
static struct if_llreach *iflr_alloc(zalloc_flags_t how) {
...
static void iflr_free(struct if_llreach *lr) {
  IFLR_LOCK(lr);
  if (lr->lr_debug & IFD_ATTACHED) {
...
void iflr_addref(struct if_llreach *lr, int locked) {
  if (!locked) {
    IFLR_LOCK(lr);
  } else {
...
void iflr_remref(struct if_llreach *lr) {
  IFLR_LOCK(lr);
  if (lr->lr_refcnt == 0) {
...
void ifnet_lr2ri(struct if_llreach *lr, struct rt_reach_info *ri) {
...
void ifnet_lr2iflri(struct if_llreach *lr, struct ifnet_llreach_info *iflri) {
  IFLR_LOCK_ASSERT_HELD(lr);
...
void ifnet_lr2lri(struct if_llreach *lr, struct if_llreach_info *lri) {
  IFLR_LOCK_ASSERT_HELD(lr);
