...
errno_t ifnet_allocate(const struct ifnet_init_params *init, ifnet_t *ifp);
...
static errno_t ifnet_list_get_common(ifnet_family_t, boolean_t,
                                     ifnet_t *__counted_by(*count) * list,
                                     u_int32_t *count);
static errno_t ifnet_set_lladdr_internal(ifnet_t,
                                         const void *__sized_by(lladdr_len)
                                             lladdr,
                                         size_t lladdr_len, u_char, int);
 ...
 */
static void ifnet_kpi_free(ifnet_t ifp) {
...
errno_t ifnet_allocate_common(const struct ifnet_init_params *init,
                              ifnet_t *ifp, bool is_internal) {
...
errno_t ifnet_allocate_internal(const struct ifnet_init_params *init,
                                ifnet_t *ifp) {
...
static void ifnet_set_broadcast_addr(ifnet_t ifp,
  ...
  if (broadcast_len != 0 && broadcast_addr != NULL) {
    ifp->if_broadcast.ptr = kalloc_data(broadcast_len, Z_WAITOK | Z_NOFAIL);
    ifp->if_broadcast.length = broadcast_len;
    bcopy(broadcast_addr, ifp->if_broadcast.ptr, broadcast_len);
  }
}

errno_t ifnet_allocate_extended(const struct ifnet_init_eparams *einit0,
                                ifnet_t *interface) {
#if SKYWALK
  ...
  if (einit.ver != IFNET_INIT_CURRENT_VERSION || einit.len < sizeof(einit)) {
    return EINVAL;
  }

  if (einit.family == 0 || einit.name == NULL ||
      strlen(einit.name) >= IFNAMSIZ || (einit.type & 0xFFFFFF00) != 0 ||
      einit.type == 0) {
    return EINVAL;
  }
...
#endif /* SKYWALK */
    if (einit.output == NULL || (einit.flags & IFNET_INIT_INPUT_POLL)) {
      return EINVAL;
    }
  ...
  snprintf(if_xname, sizeof(if_xname), "%s%d", einit.name, einit.unit);
  ...
  error =
      dlil_if_acquire(einit.family, einit.uniqueid, einit.uniqueid_len,
  ...
  if (error == 0) {
     ...
     * to write to this.
     */
    char *ifname = __unsafe_forge_bidi_indexable(
        char *, __DECONST(char *, ifp->if_name), IFNAMSIZ);
    const char *einit_name =
        __unsafe_forge_bidi_indexable(const char *, einit.name, IFNAMSIZ);
    ...
    /* Initialize external name (name + unit) */
    char *ifxname = __unsafe_forge_bidi_indexable(
        char *, __DECONST(char *, ifp->if_xname), IFXNAMSIZ);
    ...
    ifnet_set_broadcast_addr(ifp, einit.broadcast_addr, einit.broadcast_len);
    ...
    if (einit.flags & IFNET_INIT_SKYWALK_NATIVE) {
       ...
       */
      if (sk_netif_native_txmodel == NETIF_NATIVE_TXMODEL_ENQUEUE_MULTI) {
    ...
    if ((ifp->if_snd = ifclassq_alloc()) == NULL) {
      panic_plain("%s: ifp=%p couldn't allocate class queues", __func__, ifp);
      /* NOTREACHED */
    }
     ...
     */
    IFCQ_TARGET_QDELAY(ifp->if_snd) = einit.output_target_qdelay * 1000 * 1000;
    ...
    if ((einit.flags & IFNET_INIT_ALLOC_KPI) != 0) {
      ...
      OSIncrementAtomic64(&net_api_stats.nas_ifnet_alloc_os_count);
      INC_ATOMIC_INT64_LIM(net_api_stats.nas_ifnet_alloc_os_total);
    }
...
errno_t ifnet_reference(ifnet_t ifp) { return dlil_if_ref(ifp); }

void ifnet_dispose(ifnet_t ifp) { dlil_if_release(ifp); }

errno_t ifnet_release(ifnet_t ifp) { return dlil_if_free(ifp); }

errno_t ifnet_interface_family_find(const char *module_string,
...
ifnet_subfamily_t ifnet_subfamily(ifnet_t interface) {
...
errno_t ifnet_set_flags(ifnet_t interface, u_int16_t new_flags,
                        u_int16_t mask) {
  bool set_IFF_UP;
  bool change_IFF_UP;
  uint16_t old_flags;
  ...
  if ((old_flags & IFF_MULTICAST) != (interface->if_flags & IFF_MULTICAST)) {
#if INET
 ...
 */
static errno_t ifnet_awdl_check_eflags(ifnet_t ifp, u_int32_t *new_eflags,
                                       u_int32_t *mask) {
  u_int32_t eflags;
  ...
  if (ifp->if_eflags & IFEF_AWDL) {
    ...
    *new_eflags |= IFEF_AWDL_MASK;
    *mask |= IFEF_AWDL_MASK;
  } else if (eflags & IFEF_AWDL_RESTRICTED && !(ifp->if_eflags & IFEF_AWDL)) {
    return EINVAL;
  }
...
errno_t ifnet_set_eflags(ifnet_t interface, u_int32_t new_flags,
                         u_int32_t mask) {
  uint32_t oeflags;
...
errno_t ifnet_set_idle_flags_locked(ifnet_t ifp, u_int32_t new_flags,
                                    u_int32_t mask) {
  if (ifp == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_set_idle_flags(ifnet_t ifp, u_int32_t new_flags, u_int32_t mask) {
  errno_t err;
...
u_int32_t ifnet_idle_flags(ifnet_t ifp) {
  return (ifp == NULL) ? 0 : ifp->if_idle_flags;
}

errno_t ifnet_set_link_quality(ifnet_t ifp, int quality) {
  errno_t err = 0;
...
int ifnet_link_quality(ifnet_t ifp) {
  int lqm;
...
static errno_t ifnet_defrouter_llreachinfo(ifnet_t ifp, sa_family_t af,
...
errno_t ifnet_inet_defrouter_llreachinfo(ifnet_t ifp,
                                         struct ifnet_llreach_info *iflri) {
  return ifnet_defrouter_llreachinfo(ifp, AF_INET, iflri);
}

errno_t ifnet_inet6_defrouter_llreachinfo(ifnet_t ifp,
                                          struct ifnet_llreach_info *iflri) {
...
static const ifnet_offload_t offload_mask =
    (IFNET_CSUM_IP | IFNET_CSUM_TCP | IFNET_CSUM_UDP | IFNET_CSUM_FRAGMENT |
     ...
     IFNET_VLAN_TAGGING | IFNET_VLAN_MTU | IFNET_MULTIPAGES | IFNET_TSO_IPV4 |
     IFNET_TSO_IPV6 | IFNET_TX_STATUS | IFNET_HW_TIMESTAMP |
     IFNET_SW_TIMESTAMP | IFNET_LRO | IFNET_LRO_NUM_SEG);

static const ifnet_offload_t any_offload_csum = IFNET_CHECKSUMF;

static errno_t ifnet_set_offload_common(ifnet_t interface,
...
errno_t ifnet_set_offload_enabled(ifnet_t interface, ifnet_offload_t offload) {
...
errno_t ifnet_set_tso_mtu(ifnet_t interface, sa_family_t family,
                          u_int32_t mtuLen) {
  errno_t error = 0;
...
errno_t ifnet_get_tso_mtu(ifnet_t interface, sa_family_t family,
                          u_int32_t *mtuLen) {
  errno_t error = 0;
...
errno_t ifnet_set_wake_flags(ifnet_t interface, u_int32_t properties,
                             u_int32_t mask) {
  struct kev_msg ev_msg;
  struct net_event_data ev_data;
 ...
 */
errno_t ifnet_set_link_mib_data(ifnet_t interface,
...
errno_t ifnet_get_link_mib_data(ifnet_t interface,
                                void *__sized_by(*mibLen) mibData,
                                uint32_t *mibLen) {
  errno_t result = 0;
...
uint32_t ifnet_get_link_mib_data_length(ifnet_t interface) {
  return (interface == NULL) ? 0 : interface->if_linkmiblen;
}

errno_t ifnet_output(ifnet_t interface, protocol_family_t protocol_family,
...
errno_t ifnet_output_raw(ifnet_t interface, protocol_family_t protocol_family,
                         mbuf_t m) {
...
u_char ifnet_type(ifnet_t interface) {
  return (interface == NULL) ? 0 : interface->if_data.ifi_type;
}

errno_t ifnet_set_addrlen(ifnet_t interface, u_char addrlen) {
  if (interface == NULL) {
    return EINVAL;
  }
...
u_char ifnet_addrlen(ifnet_t interface) {
  return (interface == NULL) ? 0 : interface->if_data.ifi_addrlen;
}

errno_t ifnet_set_hdrlen(ifnet_t interface, u_char hdrlen) {
  if (interface == NULL) {
    return EINVAL;
  }
...
u_char ifnet_hdrlen(ifnet_t interface) {
...
errno_t ifnet_set_baudrate(struct ifnet *ifp, uint64_t baudrate) {
  if (ifp == NULL) {
    return EINVAL;
  }
...
u_int64_t ifnet_baudrate(struct ifnet *ifp) {
  return (ifp == NULL) ? 0 : ifp->if_baudrate;
}

errno_t ifnet_set_bandwidths(struct ifnet *ifp, struct if_bandwidths *output_bw,
...
static void ifnet_set_link_status_outbw(struct ifnet *ifp) {
  struct if_wifi_status_v1 *sr;
  sr = &ifp->if_link_status->ifsr_u.ifsr_wifi.if_wifi_u.if_status_v1;
  if (ifp->if_output_bw.eff_bw != 0) {
    sr->valid_bitmask |= IF_WIFI_UL_EFFECTIVE_BANDWIDTH_VALID;
    sr->ul_effective_bandwidth = ifp->if_output_bw.eff_bw > UINT32_MAX
                                     ? UINT32_MAX
                                     : (uint32_t)ifp->if_output_bw.eff_bw;
  }
  if (ifp->if_output_bw.max_bw != 0) {
    sr->valid_bitmask |= IF_WIFI_UL_MAX_BANDWIDTH_VALID;
    sr->ul_max_bandwidth = ifp->if_output_bw.max_bw > UINT32_MAX
                               ? UINT32_MAX
                               : (uint32_t)ifp->if_output_bw.max_bw;
  }
}

errno_t ifnet_set_output_bandwidths(struct ifnet *ifp,
                                    struct if_bandwidths *bw) {
...
static void ifnet_set_link_status_inbw(struct ifnet *ifp) {
  struct if_wifi_status_v1 *sr;

  sr = &ifp->if_link_status->ifsr_u.ifsr_wifi.if_wifi_u.if_status_v1;
  if (ifp->if_input_bw.eff_bw != 0) {
    sr->valid_bitmask |= IF_WIFI_DL_EFFECTIVE_BANDWIDTH_VALID;
    sr->dl_effective_bandwidth = ifp->if_input_bw.eff_bw > UINT32_MAX
                                     ? UINT32_MAX
                                     : (uint32_t)ifp->if_input_bw.eff_bw;
  }
  if (ifp->if_input_bw.max_bw != 0) {
    sr->valid_bitmask |= IF_WIFI_DL_MAX_BANDWIDTH_VALID;
...
errno_t ifnet_set_input_bandwidths(struct ifnet *ifp,
                                   struct if_bandwidths *bw) {
  struct if_bandwidths old_bw;
...
u_int64_t ifnet_output_linkrate(struct ifnet *ifp) {
...
u_int64_t ifnet_input_linkrate(struct ifnet *ifp) {
  return ifp->if_input_bw.eff_bw;
}

errno_t ifnet_bandwidths(struct ifnet *ifp, struct if_bandwidths *output_bw,
                         struct if_bandwidths *input_bw) {
  if (ifp == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_set_latencies(struct ifnet *ifp, struct if_latencies *output_lt,
...
errno_t ifnet_set_output_latencies(struct ifnet *ifp, struct if_latencies *lt) {
...
errno_t ifnet_set_input_latencies(struct ifnet *ifp, struct if_latencies *lt) {
  struct if_latencies old_lt;
...
errno_t ifnet_latencies(struct ifnet *ifp, struct if_latencies *output_lt,
                        struct if_latencies *input_lt) {
  if (ifp == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_set_poll_params(struct ifnet *ifp, struct ifnet_poll_params *p) {
  errno_t err;
...
errno_t ifnet_poll_params(struct ifnet *ifp, struct ifnet_poll_params *p) {
  errno_t err;
...
errno_t ifnet_stat_increment(struct ifnet *ifp,
...
errno_t ifnet_stat_increment_in(struct ifnet *ifp, u_int32_t packets_in,
...
errno_t ifnet_stat_increment_out(struct ifnet *ifp, u_int32_t packets_out,
...
errno_t ifnet_set_stat(struct ifnet *ifp, const struct ifnet_stats_param *s) {
  if (ifp == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_stat(struct ifnet *ifp, struct ifnet_stats_param *s) {
  if (ifp == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_touch_lastupdown(ifnet_t interface) {
  if (interface == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_updown_delta(ifnet_t interface, struct timeval *updown_delta) {
  if (interface == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_get_address_list(ifnet_t interface,
...
errno_t ifnet_get_address_list_with_count(
    ifnet_t interface, ifaddr_t *__counted_by(*addresses_count) * addresses,
    uint16_t *addresses_count) {
  return ifnet_get_address_list_family_internal(
      interface, addresses, addresses_count, 0, 0, Z_WAITOK, 0);
}
...
errno_t ifnet_get_address_list_family(ifnet_t interface,
  ...
  error = ifnet_get_address_list_family_internal(
      interface, &addresses, &addresses_count, family, 0, Z_WAITOK, 0);
  if (addresses_count > 0) {
    *ret_addresses = __unsafe_null_terminated_from_indexable(
        addresses, &addresses[addresses_count - 1]);
  } else {
    *ret_addresses = NULL;
  }
...
errno_t ifnet_get_address_list_family_with_count(
    ifnet_t interface, ifaddr_t *__counted_by(*addresses_count) * addresses,
...
errno_t ifnet_get_inuse_address_list(ifnet_t interface, ifaddr_t *
                                                            __null_terminated *
                                                            ret_addresses) {
  uint16_t addresses_count = 0;
  ifaddr_t *__counted_by(addresses_count) addresses = NULL;
  ...
  error = ifnet_get_address_list_family_internal(
      interface, &addresses, &addresses_count, 0, 0, Z_WAITOK, 1);
  if (addresses_count > 0) {
    *ret_addresses = __unsafe_null_terminated_from_indexable(
        addresses, &addresses[addresses_count - 1]);
  } else {
  ...
  one:
    ...
    if (interface == NULL || interface == ifp) {
      ...
      TAILQ_FOREACH(ifa, &ifp->if_addrhead, ifa_link) {
        IFA_LOCK(ifa);
        if (family != 0 && ifa->ifa_addr->sa_family != family) {
          IFA_UNLOCK(ifa);
          continue;
        }
...
void ifnet_free_address_list(ifaddr_t *__null_terminated addresses) {
  int i = 0;
...
void ifnet_address_list_free_counted_by_internal(
...
errno_t ifnet_llbroadcast_copy_bytes(ifnet_t interface,
                                     void *__sized_by(buffer_len) addr,
  ...
  *out_len = interface->if_broadcast.length;
  ...
  bcopy(interface->if_broadcast.ptr, addr, interface->if_broadcast.length);

  return 0;
}

static errno_t ifnet_lladdr_copy_bytes_internal(ifnet_t interface,
                                                void *__sized_by(lladdr_len)
                                                    lladdr,
                                                size_t lladdr_len,
                                                kauth_cred_t *credp) {
  size_t bytes_len;
...
errno_t ifnet_lladdr_copy_bytes(ifnet_t interface,
                                void *__sized_by(length) lladdr,
                                size_t length) {
  return ifnet_lladdr_copy_bytes_internal(interface, lladdr, length, NULL);
}

errno_t ifnet_guarded_lladdr_copy_bytes(ifnet_t interface,
                                        void *__sized_by(length) lladdr,
                                        size_t length) {
#if CONFIG_MACF
  ...
  error = ifnet_lladdr_copy_bytes_internal(interface, lladdr, length, credp);
...
static errno_t ifnet_set_lladdr_internal(ifnet_t interface,
                                         const void *__sized_by(lladdr_len)
                                             lladdr,
  ...
  if (error == 0) {
    ...
    dlil_post_msg(interface, KEV_DL_SUBCLASS, KEV_DL_LINK_ADDRESS_CHANGED, NULL,
                  0, FALSE);
  }
...
errno_t ifnet_add_multicast(ifnet_t interface, const struct sockaddr *maddr,
  ...
  case AF_UNSPEC:
    if (maddr->sa_len < ETHER_ADDR_LEN + offsetof(struct sockaddr, sa_data)) {
      return EINVAL;
    }
    break;
  default:
...
errno_t ifnet_remove_multicast(ifmultiaddr_t ifma) {
...
errno_t ifnet_get_multicast_list(ifnet_t ifp, ifmultiaddr_t *
                                                  __null_terminated *
                                                  ret_addresses) {
  int count = 0;
  int cmax = 0;
  ...
  *ret_addresses = NULL;
  ...
  ifnet_lock_shared(ifp);
  LIST_FOREACH(addr, &ifp->if_multiaddrs, ifma_link) { cmax++; }
  ...
  *ret_addresses =
      __unsafe_null_terminated_from_indexable(addresses, &addresses[cmax]);

  return 0;
}

void ifnet_free_multicast_list(ifmultiaddr_t *__null_terminated addresses) {
  int i = 0;
  ...
  for (ifmultiaddr_t *__null_terminated ptr = addresses; *ptr != NULL;
       ptr++, i++) {
    ifmaddr_release(*ptr);
  }

  ifmultiaddr_t *free_addresses =
      __unsafe_null_terminated_to_indexable(addresses);
  kfree_type(ifmultiaddr_t, i + 1, free_addresses);
}

errno_t ifnet_find_by_name(const char *ifname, ifnet_t *ifpp) {
  struct ifnet *ifp;
  size_t namelen;
...
errno_t ifnet_list_get(ifnet_family_t family,
...
static errno_t ifnet_list_get_common(ifnet_family_t family, boolean_t get_all,
                                     ifnet_t *__counted_by(*count) * list,
                                     u_int32_t *count) {
...
void ifnet_list_free(ifnet_t *__null_terminated interfaces) {
  int i = 0;
...
void ifnet_list_free_counted_by_internal(ifnet_t *__counted_by(count)
                                             interfaces,
                                         uint32_t count) {
  if (interfaces == NULL) {
    return;
...
errno_t ifaddr_reference(ifaddr_t ifa) {
  if (ifa == NULL) {
    return EINVAL;
  }
...
errno_t ifaddr_release(ifaddr_t ifa) {
  if (ifa == NULL) {
    return EINVAL;
  }
...
sa_family_t ifaddr_address_family(ifaddr_t ifa) {
...
errno_t ifaddr_address(ifaddr_t ifa, struct sockaddr *out_addr,
                       u_int32_t addr_size) {
  u_int32_t copylen;
  ...
  copylen =
      (addr_size >= ifa->ifa_addr->sa_len) ? ifa->ifa_addr->sa_len : addr_size;
...
errno_t ifaddr_dstaddress(ifaddr_t ifa, struct sockaddr *out_addr,
                          u_int32_t addr_size) {
  u_int32_t copylen;
  ...
  copylen = (addr_size >= ifa->ifa_dstaddr->sa_len) ? ifa->ifa_dstaddr->sa_len
                                                    : addr_size;
...
errno_t ifaddr_netmask(ifaddr_t ifa, struct sockaddr *out_addr,
                       u_int32_t addr_size) {
  u_int32_t copylen;
  ...
  copylen = addr_size >= ifa->ifa_netmask->sa_len ? ifa->ifa_netmask->sa_len
                                                  : addr_size;
...
ifnet_t ifaddr_ifnet(ifaddr_t ifa) {
  struct ifnet *ifp;
...
ifaddr_t ifaddr_findbestforaddr(const struct sockaddr *addr,
                                ifnet_t interface) {
...
errno_t ifaddr_get_ia6_flags(ifaddr_t ifa, u_int32_t *out_flags) {
  sa_family_t family = 0;
...
errno_t ifmaddr_address(ifmultiaddr_t ifma, struct sockaddr *out_addr,
                        u_int32_t addr_size) {
  u_int32_t copylen;
  ...
  copylen = (addr_size >= ifma->ifma_addr->sa_len ? ifma->ifma_addr->sa_len
                                                  : addr_size);
...
errno_t ifmaddr_lladdress(ifmultiaddr_t ifma, struct sockaddr *out_addr,
                          u_int32_t addr_size) {
...
ifnet_t ifmaddr_ifnet(ifmultiaddr_t ifma) {
...
errno_t ifnet_clone_attach(struct ifnet_clone_params *cloner_params,
                           if_clone_t *ifcloner) {
  errno_t error = 0;
  ...
  if (cloner_params == NULL || ifcloner == NULL ||
      cloner_params->ifc_name == NULL || cloner_params->ifc_create == NULL ||
  ...
  if (if_clone_lookup(__terminated_by_to_indexable(cloner_params->ifc_name),
                      namelen, NULL) != NULL) {
    printf("%s: already a cloner for %s\n", __func__, cloner_params->ifc_name);
    error = EEXIST;
    goto fail;
  }
...
errno_t ifnet_clone_detach(if_clone_t ifcloner) {
  errno_t error = 0;
...
static errno_t ifnet_get_local_ports_extended_inner(
    ifnet_t ifp, protocol_family_t protocol, u_int32_t flags,
    u_int8_t bitfield[bitstr_size(IP_PORTRANGE_SIZE)]) {
  u_int32_t ifindex;
  ...
  if (!(flags & IFNET_GET_LOCAL_PORTS_TCPONLY)) {
    udp_get_ports_used(ifp, protocol, flags, bitfield);
  }

  if (!(flags & IFNET_GET_LOCAL_PORTS_UDPONLY)) {
    tcp_get_ports_used(ifp, protocol, flags, bitfield);
  }
...
ifnet_get_local_ports_extended(ifnet_t ifp, protocol_family_t protocol,
  ...
  if (ifp != NULL && ifnet_get_delegate_parent(ifp, &parent_ifp) == 0) {
    ifnet_get_local_ports_extended_inner(parent_ifp, protocol, flags, bitfield);
...
errno_t ifnet_get_local_ports(ifnet_t ifp,
                              u_int8_t bitfield[IP_PORTRANGE_BITFIELD_LEN]) {
  u_int32_t flags = IFNET_GET_LOCAL_PORTS_WILDCARDOK;

  return ifnet_get_local_ports_extended(ifp, PF_UNSPEC, flags, bitfield);
}

errno_t ifnet_notice_node_presence(ifnet_t ifp, struct sockaddr *sa,
...
errno_t ifnet_notice_node_presence_v2(ifnet_t ifp, struct sockaddr *sa,
                                      struct sockaddr_dl *sdl, int32_t rssi,
...
errno_t ifnet_notice_node_absence(ifnet_t ifp, struct sockaddr *sa) {
  if (ifp == NULL || sa == NULL) {
    return EINVAL;
  }
  if (sa->sa_len > sizeof(struct sockaddr_storage)) {
...
errno_t ifnet_notice_primary_elected(ifnet_t ifp) {
  if (ifp == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_tx_compl_status(ifnet_t ifp, mbuf_t m, tx_compl_val_t val) {
#pragma unused(val)
...
errno_t ifnet_tx_compl(ifnet_t ifp, mbuf_t m) {
  m_do_tx_compl_callback(m, ifp);

  return 0;
}

errno_t ifnet_report_issues(ifnet_t ifp, u_int8_t modid[IFNET_MODIDLEN],
...
errno_t ifnet_set_delegate(ifnet_t ifp, ifnet_t delegated_ifp) {
  ifnet_t odifp = NULL;
  ...
  if (delegated_ifp != NULL && ifp != delegated_ifp) {
    ...
    printf("%s: is now delegating %s (type 0x%x, family %u, "
           "sub-family %u)\n",
           ifp->if_xname, delegated_ifp->if_xname, delegated_ifp->if_type,
           delegated_ifp->if_family, delegated_ifp->if_subfamily);
  }
...
errno_t ifnet_get_delegate(ifnet_t ifp, ifnet_t *pdelegated_ifp) {
...
errno_t ifnet_get_keepalive_offload_frames(
  ...
  if (P2ROUNDUP(frame_data_offset, sizeof(u_int32_t)) != frame_data_offset) {
    return EINVAL;
  }
  ...
  *used_frames_count = key_fill_offload_frames_for_savs(
      ifp, frames_array, frames_array_count, frame_data_offset);
  ...
  if (*used_frames_count < frames_array_count) {
    udp_fill_keepalive_offload_frames(ifp, frames_array, frames_array_count,
                                      frame_data_offset, used_frames_count);
  }

  /* If there is more room, collect other TCP keep-alive frames */
  if (*used_frames_count < frames_array_count) {
    tcp_fill_keepalive_offload_frames(ifp, frames_array, frames_array_count,
                                      frame_data_offset, used_frames_count);
  }
...
errno_t ifnet_link_status_report(ifnet_t ifp,
                                 const void *__sized_by(buffer_len) buffer,
                                 size_t buffer_len) {
  ...
  if (ifp->if_type == IFT_CELLULAR) {
     ...
     */
    if (ifsr.ifsr_version != IF_CELLULAR_STATUS_REPORT_CURRENT_VERSION) {
      err = ENOTSUP;
      goto done;
    }
    ...
    if_cell_sr = &ifp->if_link_status->ifsr_u.ifsr_cell.if_cell_u.if_status_v1;
    new_cell_sr = &ifsr.ifsr_u.ifsr_cell.if_cell_u.if_status_v1;
    /* Check if we need to act on any new notifications */
    if ((new_cell_sr->valid_bitmask & IF_CELL_UL_MSS_RECOMMENDED_VALID) &&
        new_cell_sr->mss_recommended != if_cell_sr->mss_recommended) {
    ...
    /* Check version */
    if (ifsr.ifsr_version != IF_WIFI_STATUS_REPORT_CURRENT_VERSION) {
      err = ENOTSUP;
      goto done;
    }
    ...
    if_wifi_sr = &ifp->if_link_status->ifsr_u.ifsr_wifi.if_wifi_u.if_status_v1;
    new_wifi_sr = &ifsr.ifsr_u.ifsr_wifi.if_wifi_u.if_status_v1;
    ifp->if_link_status->ifsr_version = ifsr.ifsr_version;
    ifp->if_link_status->ifsr_len = ifsr.ifsr_len;
     ...
     */
    if (!(new_wifi_sr->valid_bitmask & IF_WIFI_UL_MAX_BANDWIDTH_VALID) &&
        ifp->if_output_bw.max_bw > 0) {
      if_wifi_sr->valid_bitmask |= IF_WIFI_UL_MAX_BANDWIDTH_VALID;
      if_wifi_sr->ul_max_bandwidth = ifp->if_output_bw.max_bw > UINT32_MAX
                                         ? UINT32_MAX
                                         : (uint32_t)ifp->if_output_bw.max_bw;
    }
    if (!(new_wifi_sr->valid_bitmask & IF_WIFI_UL_EFFECTIVE_BANDWIDTH_VALID) &&
        ifp->if_output_bw.eff_bw > 0) {
      if_wifi_sr->valid_bitmask |= IF_WIFI_UL_EFFECTIVE_BANDWIDTH_VALID;
      if_wifi_sr->ul_effective_bandwidth =
          ifp->if_output_bw.eff_bw > UINT32_MAX
              ? UINT32_MAX
              : (uint32_t)ifp->if_output_bw.eff_bw;
    }
    if (!(new_wifi_sr->valid_bitmask & IF_WIFI_DL_MAX_BANDWIDTH_VALID) &&
        ifp->if_input_bw.max_bw > 0) {
      if_wifi_sr->valid_bitmask |= IF_WIFI_DL_MAX_BANDWIDTH_VALID;
      if_wifi_sr->dl_max_bandwidth = ifp->if_input_bw.max_bw > UINT32_MAX
                                         ? UINT32_MAX
                                         : (uint32_t)ifp->if_input_bw.max_bw;
    }
    if (!(new_wifi_sr->valid_bitmask & IF_WIFI_DL_EFFECTIVE_BANDWIDTH_VALID) &&
        ifp->if_input_bw.eff_bw > 0) {
      if_wifi_sr->valid_bitmask |= IF_WIFI_DL_EFFECTIVE_BANDWIDTH_VALID;
...
errno_t ifnet_set_fastlane_capable(ifnet_t interface, boolean_t capable) {
  if (interface == NULL) {
    return EINVAL;
  }

  if_set_qosmarking_mode(interface, capable ? IFRTYPE_QOSMARKING_FASTLANE
                                            : IFRTYPE_QOSMARKING_MODE_NONE);

  return 0;
}

errno_t ifnet_get_fastlane_capable(ifnet_t interface, boolean_t *capable) {
...
errno_t ifnet_get_unsent_bytes(ifnet_t interface, int64_t *unsent_bytes) {
  int64_t bytes;
...
errno_t ifnet_get_buffer_status(const ifnet_t ifp,
  ...
  buf_status->buf_sndbuf =
      ((buf_status->buf_interface != 0) || (ifp->if_sndbyte_unsent != 0)) ? 1
                                                                          : 0;

  return 0;
}

void ifnet_normalise_unsent_data(void) {
  struct ifnet *ifp;
  ...
  TAILQ_FOREACH(ifp, &ifnet_head, if_link) {
    ...
    if (ifp->if_sndbyte_total > 0 || IFCQ_BYTES(ifp->if_snd) > 0) {
...
errno_t ifnet_set_low_power_mode(ifnet_t ifp, boolean_t on) {
  errno_t error;
...
errno_t ifnet_get_low_power_mode(ifnet_t ifp, boolean_t *on) {
  if (ifp == NULL || on == NULL) {
    return EINVAL;
  }
...
errno_t ifnet_set_rx_flow_steering(ifnet_t ifp, boolean_t on) {
  errno_t error = 0;
...
errno_t ifnet_get_rx_flow_steering(ifnet_t ifp, boolean_t *on) {
  if (ifp == NULL || on == NULL) {
    return EINVAL;
  }
...
void ifnet_enable_cellular_thread_group(ifnet_t ifp) {
  VERIFY(ifp != NULL);
