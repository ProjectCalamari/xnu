...
#define WAKE_PKT_EVENT_CONTROL_ENTITLEMENT                                     \
  "com.apple.private.network.wake_pkt.control"
 ...
 *
 * Note: the IFRTYPE_xxx values are the same as the IFNET_xxx used inside the
 * kernel
 */
struct npi_if_info {
...
#define NPI_IF_INFO_IS_UTUN(_npi) ((_npi)->npi_if_family == IFRTYPE_FAMILY_UTUN)

/*
 * Symbolic names for bits of wake_pkt_control_flags and
 * una_wake_pkt_control_flags When the protocol is TCP (flag NPIF_TCP set) the
 * lower 8 bits correspond to the TCP header flag
 */
...
struct net_port_info_wake_event {
  ...
  struct npi_if_info
      wake_pkt_if_info; /* inner-most interface of TCP/UDP packet */

  char wake_pkt_phy_ifname[IFNAMSIZ]; /* name + unit */
  struct npi_if_info
      wake_pkt_phy_if_info; /* outer-most interface of wake packet */
};

/*
 * struct net_port_info_una_wake_event is the event data for
 * KEV_POWER_UNATTRIBUTED_WAKE
 *
 * una_wake_pkt_proto is useful to track unexpected wake packets when NPIF_IPV4
 * or NPIF_IPV6 is set this is the IP protocol -- see IPPROTO_x from
 * netinet/in.h. When NPIF_IPV4 and NPIF_IPV6 are not set the cotent of
 * una_wake_pkt can give clues on the type of unexpected wake packet
 */
#define NPI_MAX_UNA_WAKE_PKT_LEN 102
struct net_port_info_una_wake_event {
  uuid_t una_wake_uuid;
  struct timeval32
      una_wake_pkt_timestamp;      /* when processed by networking stack */
  ...
  uint8_t una_wake_pkt[NPI_MAX_UNA_WAKE_PKT_LEN]; /* initial portion of the
                                                     IPv4/IPv6 packet  */
  ...
  uint32_t
      una_wake_pkt_data_len; /* exclude transport (TCP/UDP) header length */
  uint16_t una_wake_pkt_control_flags; /* see NPICF_xxx above */
  uint16_t una_wake_pkt_proto;         /* IPv4 or IPv6 protocol */

  /* Followings fields added with NPI_HAS_PHYSICAL_LINK */
  struct npi_if_info
      una_wake_pkt_if_info; /* inner-most interface for TCP/UDP tuple  */

  char una_wake_pkt_phy_ifname[IFNAMSIZ]; /* name + unit */
  struct npi_if_info
      una_wake_pkt_phy_if_info; /* outer-most interface of wake packet */
};

#define IFPU_HAS_DELAY_WAKE_EVENT_FIELDS                                       \
  1 /* ifpu_delay_phy_wake_pkt and co are defined */

#define IF_PORTS_USED_STATS_LIST                                               \
  X(uint64_t, ifpu_wakeuid_gen, "wakeuuid generation%s", "", "s")              \
  X(uint64_t, ifpu_wakeuuid_not_set_count,                                     \
    "offload port list quer%s with wakeuuid not set", "y", "ies")              \
  X(uint64_t, ifpu_npe_total, "total offload port entr%s created since boot",  \
    "y", "ies")                                                                \
  ...
  X(uint64_t, ifpu_npi_hash_search_total, "total table entry search%s", "",    \
    "es")                                                                      \
  X(uint64_t, ifpu_npi_hash_search_max, "max hash table entry search%s", "",   \
    "es")                                                                      \
  ...
  X(uint64_t, ifpu_isakmp_natt_wake_pkt, "ISAKMP NAT traversal wake packet%s", \
    "", "s")                                                                   \
  ...
  X(uint64_t, ifpu_dup_wake_pkt_event,                                         \
    "duplicate wake packet event%s in same wake cycle", "", "s")               \
  X(uint64_t, ifpu_wake_pkt_event_error, "wake packet event%s undelivered",    \
    "", "s")                                                                   \
  X(uint64_t, ifpu_unattributed_wake_event,                                    \
    "unattributed wake packet event%s", "", "s")                               \
  X(uint64_t, ifpu_dup_unattributed_wake_event,                                \
    "duplicate unattributed wake packet event%s in same wake cycle", "", "s")  \
  X(uint64_t, ifpu_unattributed_wake_event_error,                              \
    "unattributed wake packet event%s undelivered", "", "s")                   \
  X(uint64_t, ifpu_unattributed_null_recvif,                                   \
    "unattributed wake packet%s received with null interface", "", "s")        \
  X(uint64_t, ifpu_match_wake_pkt_no_flag, "bad packet%s without wake flag",   \
    "", "s")                                                                   \
  X(uint64_t, ifpu_frag_wake_pkt, "pure fragment wake packet%s", "", "s")      \
  X(uint64_t, ifpu_incomplete_tcp_hdr_pkt,                                     \
    "packet%s with incomplete TCP header", "", "s")                            \
  X(uint64_t, ifpu_incomplete_udp_hdr_pkt,                                     \
    "packet%s with incomplete UDP header", "", "s")                            \
  X(uint64_t, ifpu_npi_not_added_no_wakeuuid,                                  \
    "port entr%s not added with wakeuuid not set", "y", "ies")                 \
  X(uint64_t, ifpu_deferred_isakmp_natt_wake_pkt,                              \
    "deferred matching of ISAKMP NAT traversal wake packet%s", "", "s")        \
  X(uint64_t, ifpu_spurious_wake_event,                                        \
    "spurious no wake from sleep packet event%s", "", "s")                     \
  X(uint64_t, ifpu_delayed_attributed_wake_event,                              \
    "delayed attributed wake packet event%s", "", "s")                         \
  X(uint64_t, ifpu_delayed_unattributed_wake_event,                            \
    "delayed unattributed wake packet event%s", "", "s")                       \
  X(uint64_t, ifpu_delayed_wake_event_undelivered,                             \
    "undelivered delayed wake packet event%s", "", "s")                        \
  X(uint64_t, ifpu_connection_idle_wake, "connection idle wake%s", "", "s")    \
  X(uint64_t, ifpu_lpw_connection_idle_wake, "LPW connection idle wake%s", "", \
    "s")                                                                       \
  X(uint64_t, ifpu_lpw_not_idle_wake, "LPW not idle connection wake%s", "",    \
    "s")                                                                       \
  X(uint64_t, ifpu_lpw_to_full_wake, "LPW to full wake transition%s", "", "s") \
  X(uint64_t, ifpu_ignored_phy_wake_pkt,                                       \
    "ignored wake packet%s in same wake cycle", "", "s")                       \
  X(uint64_t, ifpu_delay_phy_wake_pkt, "delayed wake packet%s", "", "s")       \
  X(uint64_t, ifpu_ignored_delayed_attributed_events,                          \
    "ignored delayed attributed event%s", "", "s")                             \
  X(uint64_t, ifpu_ignored_delayed_unattributed_events,                        \
    "ignored delayed unattributed event%s", "", "s")                           \
  X(uint64_t, ifpu_wake_pkt_event_notify_in_vain,                              \
    "wake pkt event notifications%s in vain", "", "s")
...
struct ns_flow_info;
struct flow_entry;
bool if_ports_used_add_flow_entry(const struct flow_entry *fe,
                                  const uint32_t ifindex,
                                  const struct ns_flow_info *nfi,
                                  uint32_t ns_flags);
