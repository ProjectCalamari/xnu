...
#define SET_SNDSB_IDEAL_SIZE(sndsb, size)                                      \
  sndsb->sb_idealsize =                                                        \
      min(max(tcp_sendspace, tp->snd_ssthresh), tcp_autosndbuf_max);
...
void tcp_cc_resize_sndbuf(struct tcpcb *tp) {
  ...
  if (sb->sb_hiwat > tp->snd_ssthresh && (sb->sb_flags & SB_AUTOSIZE)) {
 ...
 */
void tcp_cc_cwnd_init_or_reset(struct tcpcb *tp) {
 ...
 */
int tcp_cc_delay_ack(struct tcpcb *tp, struct tcphdr *th) {
  switch (tcp_delack_enabled) {
  case 1:
  case 2:
    if ((tp->t_flags & TF_RXWIN0SENT) == 0 && (th->th_flags & TH_PUSH) == 0 &&
        (tp->t_unacksegs == 1)) {
      return 1;
    }
    break;
  case 3: {
     ...
     *    Window actually has been growing larger than the initial value by half
     * of it. (this makes sure that during ramp-up we ACK every second MSS until
     * we pass the tcp_recvspace * 1.5-threshold)
     * 6. We haven't waited for half a BDP
     * 7. The amount of unacked data is less than the maximum ACK-burst (256
     * MSS) We try to avoid having the sender end up hitting huge ACK-ranges.
     *
     ...
     */
    if ((tp->t_flags & TF_RXWIN0SENT) == 0 && tp->t_forced_acks == 0 &&
        tp->t_inpcb->inp_socket->so_rcv.sb_cc >
            tp->t_inpcb->inp_socket->so_rcv.sb_lowat &&
...
void tcp_cc_allocate_state(struct tcpcb *tp) {
 ...
 */
inline uint32_t tcp_cc_is_cwnd_nonvalidated(struct tcpcb *tp) {
 ...
 * phase.
 */
inline void tcp_cc_adjust_nonvalidated_cwnd(struct tcpcb *tp) {
 ...
 */
inline uint32_t tcp_get_max_pipeack(struct tcpcb *tp) {
  uint32_t max_pipeack = 0;
  max_pipeack = (tp->t_pipeack_sample[0] > tp->t_pipeack_sample[1])
                    ? tp->t_pipeack_sample[0]
                    : tp->t_pipeack_sample[1];
  max_pipeack = (tp->t_pipeack_sample[2] > max_pipeack)
                    ? tp->t_pipeack_sample[2]
                    : max_pipeack;

  return max_pipeack;
}

inline void tcp_clear_pipeack_state(struct tcpcb *tp) {
