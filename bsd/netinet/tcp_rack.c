...
#define REORDERING_WINDOW_FLOOR                                                \
  (2) /* If min_rtt is too small, at least wait for a reordering window of 2ms \
       */
...
static bool tcp_rack_sent_after(uint32_t t1, uint32_t seq1, uint32_t t2,
                                uint32_t seq2) {
...
void tcp_rack_bad_rexmt_restore(struct tcpcb *tp) {
  ...
  tp->rack.reo_wnd_persist =
      MIN(tp->rack.reo_wnd_persist + 1, TCP_RACK_RECOVERY_PERSIST_MAX);
}
...
/* MUST be called before we have processed dup ACKs and made a decision to enter
 * recovery */
static uint32_t tcp_rack_reordering_window(struct tcpcb *tp, uint32_t dup_acks,
                                           bool in_rto) {
...
static uint32_t tcp_rack_detect_segment_lost(struct tcpcb *tp,
 ...
 * Step 2: Update the state for the most recently sent segment that has been
 * delivered.
 */
void tcp_rack_update_segment_acked(struct tcpcb *tp, uint32_t tsecr,
  ...
  /*
   * RFC 8985 - An ACK can acknowledge retransmitted data and because
   * retransmissions can be spurious, ignore ACKs for such retransmitted
   * segments. Ignore a segment if any of its sequence range has been
   * retransmitted before and if either of two conditions is true:
   * 1. The TSecr of the ACK's timestamp option (if available) indicates the ACK
   * was not acknowledging the last (re)transmission OR tsecr was invalid
   * (greater than tcp_now)
   * 2. If TSecr is not available or ACK arrives immediately after last
   * retransmission, check if the segment was last (re)transmitted less than
   * RACK.min_rtt ago.
   */
  if (retransmitted) {
    if ((tsecr != 0 &&
         (TSTMP_LT(tsecr, xmit_ts) || TSTMP_GT(tsecr, tcp_now))) ||
        rtt < get_base_rtt(tp)) {
  ...
  if (tcp_rack_sent_after(xmit_ts, end_seq, tp->rack.xmit_ts,
                          tp->rack.end_seq)) {
    tp->rack.advanced = 1;
...
void tcp_rack_update_reordering_window(struct tcpcb *tp,
   ...
   * Reordering window persists for 16 loss recoveries (that don't receive
   * DSACK). On receiving DSACK, we reset window persist to 16 as it indicates
   * that reordering is still happening.
   */
 ...
 */
static uint32_t tcp_rack_detect_loss(struct tcpcb *tp, uint32_t dup_acks,
                                     bool *loss_detected) {
  ...
  TAILQ_FOREACH(seg, &tp->t_segs_sent, tx_link) {
     ...
     */
    if (!tcp_rack_sent_after(tp->rack.xmit_ts, tp->rack.end_seq, seg->xmit_ts,
                             seg->end_seq)) {
      break;
    }
    ...
    uint32_t remaining =
        tcp_rack_detect_segment_lost(tp, seg, reordering_window, loss_detected);
    if (remaining) {
      /*
...
bool tcp_rack_detect_loss_and_arm_timer(struct tcpcb *tp, uint32_t dup_acks) {
  ...
  /* Cancel any existing RACK reordering timer as we are going to re-fire it if
   * needed */
  tp->t_timer[TCPT_REORDER] = 0;

  reordering_timeout = tcp_rack_detect_loss(tp, dup_acks, &loss_detected);
  if (reordering_timeout) {
    tp->t_timer[TCPT_REORDER] =
        tcp_offset_from_start(tp, reordering_timeout + REORDERING_WINDOW_FLOOR);
...
void tcp_rack_detect_reordering_acked(struct tcpcb *tp,
  ...
  if (SEQ_LT(seg->end_seq, tp->snd_fack) &&
      (seg->flags &
       (TCP_SEGMENT_SACKED | TCP_SEGMENT_RETRANSMITTED_ATLEAST_ONCE)) == 0) {
    tp->t_reordered_pkts++;
  }
}
