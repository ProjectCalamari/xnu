...
#define TCP_LOG_ENABLE_DEFAULT                                                 \
  (TLEF_CONNECTION | TLEF_DST_LOCAL | TLEF_DST_GW | TLEF_DROP_NECP |           \
   TLEF_DROP_PCB | TLEF_DROP_PKT | TLEF_SYN_RXMT | TLEF_LOG)
...
SYSCTL_UINT(_net_inet_tcp_log, OID_AUTO, enable, CTLFLAG_RW | CTLFLAG_LOCKED,
            &tcp_log_enable_flags, 0, "");

/*
 * The following is a help to describe the values of the flags
 */
#define X(name, value, description, ...) #description ":" #value " "
SYSCTL_STRING(_net_inet_tcp_log, OID_AUTO, enable_usage,
              CTLFLAG_RD | CTLFLAG_LOCKED, TCP_ENABLE_FLAG_LIST, 0, "");
#undef X
 ...
 *  0: off
 *  other: only for interfaces with the corresponding interface family in the
 * bitmap
 */
#if (DEVELOPMENT || DEBUG)
#define TCP_LOG_THFLAGS_IF_FAMILY_DEFAULT                                      \
  (0xfffffffe & ~BIT(IFNET_FAMILY_LOOPBACK))
#else /* (DEVELOPMENT || DEBUG) */
...
SYSCTL_ULONG(_net_inet_tcp_log, OID_AUTO, rate_max, CTLFLAG_RD | CTLFLAG_LOCKED,
             &tcp_log_rate_max, "");
...
#define TCP_LOG_COMMON_ARGS                                                    \
  laddr_buf, ntohs(local_port), faddr_buf, ntohs(foreign_port),                \
      ifp != NULL ? if_name(ifp) : "", tcp_log_rate_exceeded_total
...
#define TCP_LOG_COMMON_PCB_ARGS                                                \
  TCP_LOG_COMMON_ARGS, so != NULL ? so->so_gencnt : 0, tcpstates[tp->t_state], \
      inp->inp_last_proc_name, so->last_pid

/*
 * Returns true when above the rate limit
 */
static bool tcp_log_is_rate_limited(void) {
...
static void tcp_log_inp_addresses(struct inpcb *inp,
                                  char *__sized_by(lbuflen) lbuf,
                                  socklen_t lbuflen,
                                  char *__sized_by(fbuflen) fbuf,
                                  socklen_t fbuflen) {
  /*
...
__attribute__((noinline)) void tcp_log_rtt_info(const char *func_name,
                                                int line_no, struct tcpcb *tp) {
  ...
  ifp = inp->inp_last_outifp != NULL ? inp->inp_last_outifp
        : inp->inp_boundifp != NULL  ? inp->inp_boundifp
                                     : NULL;

  tcp_log_inp_addresses(inp, laddr_buf, sizeof(laddr_buf), faddr_buf,
                        sizeof(faddr_buf));

  os_log(OS_LOG_DEFAULT,
         "tcp_rtt_info (%s:%d) " TCP_LOG_COMMON_PCB_FMT
         "base_rtt: %u ms rttcur: %u ms srtt: %u ms rttvar: %u ms rttmin: %u "
         "ms rxtcur: %u rxtshift: %u",
         func_name, line_no, TCP_LOG_COMMON_PCB_ARGS, get_base_rtt(tp),
         tp->t_rttcur, tp->t_srtt >> TCP_RTT_SHIFT,
         tp->t_rttvar >> TCP_RTTVAR_SHIFT, tp->t_rttmin, tp->t_rxtcur,
         tp->t_rxtshift);
}

__attribute__((noinline)) void tcp_log_rt_rtt(const char *func_name,
                                              int line_no, struct tcpcb *tp,
                                              struct rtentry *rt) {
  struct inpcb *inp = tp->t_inpcb;
  struct socket *so = inp->inp_socket;
...
__attribute__((noinline)) void tcp_log_rtt_change(const char *func_name,
  ...
  rttvar_diff = ABS((tp->t_rttvar - old_rttvar) >> TCP_RTTVAR_SHIFT);
  if (srtt_diff >= 1000 || rttvar_diff >= 500) {
    ...
    os_log(OS_LOG_DEFAULT,
           "tcp_rtt_change (%s:%d) " TCP_LOG_COMMON_PCB_FMT
           "srtt: %u ms old_rtt: %u ms "
           "rttvar: %u old_rttvar: %u ms ",
           func_name, line_no, TCP_LOG_COMMON_PCB_ARGS,
           tp->t_srtt >> TCP_RTT_SHIFT, old_srtt >> TCP_RTT_SHIFT,
           tp->t_rttvar >> TCP_RTTVAR_SHIFT, old_rttvar >> TCP_RTTVAR_SHIFT);
  }
}

__attribute__((noinline)) void tcp_log_keepalive(const char *func_name,
                                                 int line_no, struct tcpcb *tp,
                                                 int32_t idle_time) {
  struct inpcb *inp = tp->t_inpcb;
  struct socket *so = inp->inp_socket;
  ...
  os_log(OS_LOG_DEFAULT,
         "tcp_keepalive (%s:%d) " TCP_LOG_COMMON_PCB_FMT
         "snd_una: %u snd_max: %u "
         "SO_KA: %d RSTALL: %d TFOPRB: %d idle_time: %u "
         "rtimo_probes: %u adaptive_rtimo: %u "
         "KIDLE: %d KINTV: %d KCNT: %d",
         func_name, line_no, TCP_LOG_COMMON_PCB_ARGS, tp->snd_una, tp->snd_max,
         tp->t_inpcb->inp_socket->so_options & SO_KEEPALIVE,
         tp->t_flagsext & TF_DETECT_READSTALL,
         tp->t_tfo_probe_state == TFO_PROBE_PROBING, idle_time,
         tp->t_rtimo_probes, tp->t_adaptive_rtimo, TCP_CONN_KEEPIDLE(tp),
         TCP_CONN_KEEPINTVL(tp), TCP_CONN_KEEPCNT(tp));
}
...
#define TCP_LOG_CONNECT_FMT                                                    \
  "tcp %s: " TCP_LOG_COMMON_PCB_FMT "SYN in/out: %u/%u "                       \
...
#define TCP_LOG_CONNECT_ARGS                                                   \
  event, TCP_LOG_COMMON_PCB_ARGS, tp->t_syn_rcvd, tp->t_syn_sent,              \
      inp->inp_mstat.ms_total.ts_rxbytes, inp->inp_mstat.ms_total.ts_txbytes,  \
      inp->inp_mstat.ms_total.ts_rxpackets,                                    \
      inp->inp_mstat.ms_total.ts_txpackets, P_MS(tp->t_srtt, TCP_RTT_SHIFT),   \
      P_MS(tp->t_rttvar, TCP_RTTVAR_SHIFT), get_base_rtt(tp), error,           \
      so->so_error,                                                            \
      (so->so_flags1 & SOF1_TC_NET_SERV_TYPE) ? so->so_netsvctype              \
                                              : so->so_traffic_class,          \
      inp->inp_flowhash

  if (so->so_head == NULL) {
    os_log(OS_LOG_DEFAULT, TCP_LOG_CONNECT_FMT, TCP_LOG_CONNECT_ARGS);
  } else {
...
#define TCP_LOG_CONN_Q_ARGS                                                    \
  so->so_head->so_qlimit, so->so_head->so_qlen, so->so_head->so_incqlen
...
__attribute__((noinline)) void tcp_log_listen(struct tcpcb *tp, int error) {
  struct inpcb *inp = tp->t_inpcb;
  struct socket *so = inp->inp_socket;
  struct ifnet *ifp;
  char laddr_buf[ADDRESS_STR_LEN];
...
#define TCP_LOG_LISTEN_FMT                                                     \
  "tcp listen: " TCP_LOG_COMMON_PCB_FMT "so_qlimit: %d "                       \
  "error: %d "                                                                 \
  "so_error: %d "                                                              \
  "svc/tc: %u"

#define TCP_LOG_LISTEN_ARGS                                                    \
  TCP_LOG_COMMON_PCB_ARGS, so->so_qlimit, error, so->so_error,                 \
      (so->so_flags1 & SOF1_TC_NET_SERV_TYPE) ? so->so_netsvctype              \
                                              : so->so_traffic_class

  os_log(OS_LOG_DEFAULT, TCP_LOG_LISTEN_FMT, TCP_LOG_LISTEN_ARGS);
#undef TCP_LOG_LISTEN_FMT
#undef TCP_LOG_LISTEN_ARGS
}

static const char *tcp_connection_client_accurate_ecn_state_to_string(
    tcp_connection_client_accurate_ecn_state_t state) {
  switch (state) {
#define ECN_STATE_TO_STRING(_s, _str)                                          \
  case tcp_connection_client_accurate_ecn_##_s: {                              \
    ...
    ECN_STATE_TO_STRING(negotiation_success_ect_mangling_detected,
                        "ECT mangling")
    ECN_STATE_TO_STRING(negotiation_success_ect_bleaching_detected,
                        "ECT bleaching")
...
static const char *tcp_connection_server_accurate_ecn_state_to_string(
    tcp_connection_server_accurate_ecn_state_t state) {
  switch (state) {
#define ECN_STATE_TO_STRING(_s, _str)                                          \
...
__attribute__((noinline)) void tcp_log_connection_summary(const char *func_name,
                                                          int line_no,
                                                          struct tcpcb *tp) {
  struct inpcb *inp;
  struct socket *so;
...
#define TCP_LOG_CONNECTION_SUMMARY_FMT                                         \
  "tcp_connection_summary (%s:%d)" TCP_LOG_COMMON_PCB_FMT                      \
  "Duration: %u.%03u sec "                                                     \
  "Conn_Time: %u.%03u sec "                                                    \
  "bytes in/out: %llu/%llu "                                                   \
  "pkts in/out: %llu/%llu "                                                    \
...
#define TCP_LOG_CONNECTION_SUMMARY_ARGS                                        \
  func_name, line_no, TCP_LOG_COMMON_PCB_ARGS, duration / TCP_RETRANSHZ,       \
      duration % TCP_RETRANSHZ, conntime / TCP_RETRANSHZ,                      \
      conntime % TCP_RETRANSHZ, inp->inp_mstat.ms_total.ts_rxbytes,            \
      inp->inp_mstat.ms_total.ts_txbytes,                                      \
      inp->inp_mstat.ms_total.ts_rxpackets,                                    \
      inp->inp_mstat.ms_total.ts_txpackets, tp->t_stat.rxmitpkts,              \
      tp->t_rcvoopack, tp->t_stat.rxduplicatebytes, tp->t_stat.acks_delayed,   \
      tp->t_stat.delayed_acks_sent, P_MS(tp->t_srtt, TCP_RTT_SHIFT),           \
      P_MS(tp->t_rttvar, TCP_RTTVAR_SHIFT), get_base_rtt(tp), so->so_error,    \
      (so->so_flags1 & SOF1_TC_NET_SERV_TYPE) ? so->so_netsvctype              \
                                              : so->so_traffic_class,          \
      inp->inp_flowhash
...
#define TCP_LOG_CONNECTION_SUMMARY_FMT                                         \
  "tcp_connection_summary " TCP_LOG_COMMON_PCB_FMT "flowctl: %lluus (%llux) "  \
  "SYN in/out: %u/%u "                                                         \
  "FIN in/out: %u/%u "                                                         \
  "RST in/out: %u/%u "                                                         \
...
#define TCP_LOG_CONNECTION_SUMMARY_ARGS                                        \
  TCP_LOG_COMMON_PCB_ARGS, inp->inp_fadv_total_time, inp->inp_fadv_cnt,        \
      tp->t_syn_rcvd, tp->t_syn_sent, tp->t_fin_rcvd, tp->t_fin_sent,          \
      tp->t_rst_rcvd, tp->t_rst_sent,                                          \
      tcp_connection_client_accurate_ecn_state_to_string(                      \
          tp->t_client_accecn_state),                                          \
      tcp_connection_server_accurate_ecn_state_to_string(                      \
 ...
 */
__attribute__((noinline)) void tcp_log_drop_pcb(void *hdr, struct tcphdr *th,
                                                struct tcpcb *tp, bool outgoing,
                                                const char *reason) {
  struct inpcb *inp;
  struct socket *so;
  struct ifnet *ifp;
  ...
  if ((inp->inp_flags2 & INP2_LOGGED_SUMMARY) &&
      ((so->so_state & SS_NOFDREF) || (so->so_flags & SOF_DEFUNCT) ||
  ...
  if (hdr != NULL && th != NULL) {
    ...
    (void)tcp_log_pkt_addresses(hdr, th, outgoing, laddr_buf, sizeof(laddr_buf),
                                faddr_buf, sizeof(faddr_buf));
  } else {
    local_port = inp->inp_lport;
    foreign_port = inp->inp_fport;
    tcp_log_inp_addresses(inp, laddr_buf, sizeof(laddr_buf), faddr_buf,
...
#define TCP_LOG_DROP_PCB_FMT                                                   \
  "tcp drop %s" TCP_LOG_COMMON_PCB_FMT "t_state: %s "                          \
  "so_error: %d "                                                              \
  "reason: %s"

#define TCP_LOG_DROP_PCB_ARGS                                                  \
  direction, TCP_LOG_COMMON_PCB_ARGS, tcpstates[tp->t_state], so->so_error,    \
      reason

  os_log(OS_LOG_DEFAULT, TCP_LOG_DROP_PCB_FMT, TCP_LOG_DROP_PCB_ARGS);
...
#define TCP_LOG_TH_FLAGS_COMMON_ARGS                                           \
  outgoing ? "outgoing" : "incoming", thflags &TH_SYN ? "SYN " : "",           \
      thflags &TH_FIN ? "FIN " : "", thflags &TH_RST ? "RST " : "",            \
      thflags &TH_ACK ? "ACK " : ""

static bool should_log_th_flags(uint8_t thflags, struct tcpcb *tp,
                                bool outgoing, struct ifnet *ifp) {
  /*
   ...
   */
  if (ifp != NULL &&
      (ifp->if_family >= (sizeof(tcp_log_thflags_if_family) << 3) ||
  ...
  if (tp != NULL &&
      ((thflags & TH_SYN) && (tcp_log_enable_flags & TLEF_SYN_RXMT) &&
       ((outgoing && tp->t_syn_sent > 3) ||
        (!outgoing && tp->t_syn_rcvd > 3)))) {
    return true;
  }
  /*
...
__attribute__((noinline)) void tcp_log_th_flags(void *hdr, struct tcphdr *th,
                                                struct tcpcb *tp, bool outgoing,
                                                struct ifnet *ifp) {
  struct inpcb *inp = tp->t_inpcb;
  ...
  if (!tcp_log_pkt_addresses(hdr, th, outgoing, laddr_buf, sizeof(laddr_buf),
                             faddr_buf, sizeof(faddr_buf))) {
    return;
  }
...
#define TCP_LOG_TH_FLAGS_NO_PCB_ARGS                                           \
  TCP_LOG_TH_FLAGS_COMMON_ARGS, TCP_LOG_COMMON_ARGS
...
#define TCP_LOG_TH_FLAGS_PCB_ARGS                                              \
  TCP_LOG_TH_FLAGS_COMMON_ARGS, TCP_LOG_COMMON_PCB_ARGS, tp->t_syn_rcvd,       \
      tp->t_syn_sent

    os_log(OS_LOG_DEFAULT, TCP_LOG_TH_FLAGS_PCB_FMT, TCP_LOG_TH_FLAGS_PCB_ARGS);
#undef TCP_LOG_TH_FLAGS_PCB_FMT
#undef TCP_LOG_TH_FLAGS_PCB_ARGS
  }
}

__attribute__((noinline)) void tcp_log_drop_pkt(void *hdr, struct tcphdr *th,
                                                struct ifnet *ifp,
                                                const char *reason) {
  char laddr_buf[ADDRESS_STR_LEN];
  char faddr_buf[ADDRESS_STR_LEN];
...
#define TCP_LOG_DROP_PKT_FMT                                                   \
  "tcp drop incoming control packet " TCP_LOG_TH_FLAGS_COMMON_FMT "reason: %s"

#define TCP_LOG_DROP_PKT_ARGS                                                  \
  TCP_LOG_TH_FLAGS_COMMON_ARGS, reason != NULL ? reason : ""

  os_log(OS_LOG_DEFAULT, TCP_LOG_DROP_PKT_FMT, TCP_LOG_DROP_PKT_ARGS);
#undef TCP_LOG_DROP_PKT_FMT
#undef TCP_LOG_DROP_PKT_ARGS
}

__attribute__((noinline)) void tcp_log_message(const char *func_name,
                                               int line_no, struct tcpcb *tp,
                                               const char *format, ...) {
  struct inpcb *inp;
  struct socket *so;
...
#define TCP_LOG_MESSAGE_FMT                                                    \
  "tcp (%s:%d) " TCP_LOG_COMMON_PCB_FMT "bytes in/out: %llu/%llu "             \
  "pkts in/out: %llu/%llu "                                                    \
  "%s"
...
#define TCP_LOG_MESSAGE_ARGS                                                   \
  func_name, line_no, TCP_LOG_COMMON_PCB_ARGS,                                 \
      inp->inp_mstat.ms_total.ts_rxbytes, inp->inp_mstat.ms_total.ts_txbytes,  \
      inp->inp_mstat.ms_total.ts_rxpackets,                                    \
      inp->inp_mstat.ms_total.ts_txpackets, message

  os_log(OS_LOG_DEFAULT, TCP_LOG_MESSAGE_FMT, TCP_LOG_MESSAGE_ARGS);
#undef TCP_LOG_MESSAGE_FMT
#undef TCP_LOG_MESSAGE_ARGS
}

#if SKYWALK
__attribute__((noinline)) void tcp_log_fsw_flow(const char *func_name,
                                                int line_no, struct tcpcb *tp,
                                                const char *format, ...) {
  struct inpcb *inp;
  struct socket *so;
...
#define TCP_LOG_FSW_FLOW_MESSAGE_FMT                                           \
  "tcp (%s:%d) " TCP_LOG_COMMON_PCB_FMT "flow %s %s"

#define TCP_LOG_FSW_FLOW_MESSAGE_ARGS                                          \
  func_name, line_no, TCP_LOG_COMMON_PCB_ARGS, flow_uuid_str, message
...
void tcp_log_state_change(const char *func_name, int line_no, struct tcpcb *tp,
                          int new_state) {
  struct inpcb *inp;
  struct socket *so;
  struct ifnet *ifp;
...
#define TCP_LOG_STATE_FMT                                                      \
  "tcp_state_changed (%s:%d) " TCP_LOG_COMMON_PCB_FMT "%s "

#define TCP_LOG_STATE_ARGS                                                     \
  func_name, line_no, TCP_LOG_COMMON_PCB_ARGS, tcpstates[new_state]

  os_log(OS_LOG_DEFAULT, TCP_LOG_STATE_FMT, TCP_LOG_STATE_ARGS);
#undef TCP_LOG_STATE_FMT
#undef TCP_LOG_STATE_ARGS
}

__attribute__((noinline)) void tcp_log_output(const char *func_name,
                                              int line_no, struct tcpcb *tp,
                                              const char *format, ...) {
  struct inpcb *inp;
  struct socket *so;
...
#define TCP_LOG_MESSAGE_ARGS                                                   \
  func_name, line_no, TCP_LOG_COMMON_PCB_ARGS,                                 \
      ...
      inp->inp_mstat.ms_total.ts_rxpackets,                                    \
      inp->inp_mstat.ms_total.ts_txpackets, tp->t_stat.rxmitpkts,              \
      tp->t_stat.txretransmitbytes, message
