...
static int mptcp_validate_join_hmac(struct tcpcb *, u_char *__sized_by(maclen),
                                    int maclen);
static int mptcp_snd_mpprio(struct tcpcb *tp, u_char *cp __ended_by(optend),
                            u_char *optend, int optlen);
static void mptcp_send_remaddr_opt(struct tcpcb *tp,
                                   struct mptcp_remaddr_opt *opt);
static int mptcp_echo_add_addr(struct tcpcb *tp, u_char *__indexable cp,
                               unsigned int optlen);
...
static unsigned
mptcp_setup_first_subflow_syn_opts(struct socket *so,
                                   u_char *opt __ended_by(optend),
                                   u_char *optend __unused, unsigned optlen) {
   ...
   */
  if (ret == 0 && tp->t_rxtshift > mptcp_mpcap_retries &&
      !(mpte->mpte_flags & MPTE_FORCE_ENABLE)) {
    if (!(mp_tp->mpt_flags &
          (MPTCPF_FALLBACK_HEURISTIC | MPTCPF_HEURISTIC_TRAC))) {
...
static unsigned
mptcp_setup_join_subflow_syn_opts(struct socket *so,
                                  u_char *opt __ended_by(optend),
                                  u_char *optend __unused, unsigned optlen) {
  ...
  mptcp_get_rands(tp->t_local_aid, tptomptp(tp), &mpjoin_req.mmjo_rand, NULL);
...
unsigned mptcp_setup_join_ack_opts(struct tcpcb *tp,
                                   u_char *opt __ended_by(optend),
                                   u_char *optend __unused, unsigned optlen) {
  unsigned new_optlen;
...
unsigned mptcp_setup_syn_opts(struct socket *so, u_char *opt __ended_by(optend),
                              u_char *optend, unsigned optlen) {
  unsigned new_optlen;
...
static int mptcp_send_mpfail(struct tcpcb *tp, u_char *opt __ended_by(optend),
                             u_char *optend, unsigned int optlen) {
...
static int mptcp_send_infinite_mapping(struct tcpcb *tp,
                                       u_char *opt __ended_by(optend),
                                       u_char *optend __unused,
                                       unsigned int optlen) {
  ...
  if (mp_tp->mpt_flags & MPTCPF_RECVD_MPFAIL) {
    infin_opt.mdss_dsn =
        (u_int32_t)MPTCP_DATASEQ_LOW32(mp_tp->mpt_dsn_at_csum_fail);
    ...
    infin_opt.mdss_dsn = (u_int32_t)MPTCP_DATASEQ_LOW32(mp_tp->mpt_snduna);
  }
...
static int mptcp_ok_to_fin(struct tcpcb *tp, u_int64_t dsn, u_int32_t datalen) {
  struct mptcb *mp_tp = tptomptp(tp);
...
unsigned int mptcp_setup_opts(struct tcpcb *tp, int32_t off,
                              u_char *opt __ended_by(optend), u_char *optend,
                              unsigned int optlen, int flags, int len,
                              boolean_t *p_mptcp_acknow,
                              boolean_t *do_not_compress) {
  ...
  if ((MAX_TCPOPTLEN - optlen) < sizeof(struct mptcp_mpcapable_opt_common)) {
    os_log_error(
        mptcp_log_handle,
        "%s - %lx: no space left %d flags %x tp->t_mpflags %x len %d\n",
        __func__, (unsigned long)VM_KERNEL_ADDRPERM(mp_tp->mpt_mpte), optlen,
        flags, tp->t_mpflags, len);
    goto ret_optlen;
  }
  ...
  if (len > 0 && off == 0 && tp->t_mpflags & TMPF_SEND_DSN &&
  ...
  if (tp->t_mpflags & TMPF_SND_KEYS &&
    ...
    boolean_t send_data_level_details =
        tp->t_mpflags & TMPF_SEND_DSN ? TRUE : FALSE;
    ...
    if ((MAX_TCPOPTLEN - optlen) < mmco_len) {
      os_log_error(mptcp_log_handle,
                   "%s - %lx: not enough space in TCP option, "
                   "optlen: %u, mmco_len: %d\n",
                   __func__, (unsigned long)VM_KERNEL_ADDRPERM(mp_tp->mpt_mpte),
                   optlen, mmco_len);
      goto ret_optlen;
    ...
    if (send_data_level_details) {
      mptcp_output_getm_data_level_details(so, off, &mptcp_opt.data_len,
                                           &mptcp_opt.csum);
  ...
  if (tp->t_mpflags & TMPF_SND_JACK) {
    *do_not_compress = TRUE;
    ...
    tp->t_timer[TCPT_JACK_RXMT] = tcp_offset_from_start(tp, tcp_jack_rxmt);
  ...
  if ((tp->t_mpflags & TMPF_MPTCP_TRUE) &&
    ...
    if (optlen + rem_opt_len <= MAX_TCPOPTLEN) {
      mptcp_send_remaddr_opt(tp, (struct mptcp_remaddr_opt *)(opt + optlen));
      optlen += rem_opt_len;
    } else {
      tp->t_mpflags &= ~TMPF_SND_REM_ADDR;
    }
...
#define CHECK_DATALEN                                                          \
  {                                                                            \
    ...
    if ((len + optlen + dssoptlen) > tp->t_maxopd) {                           \
      os_log_error(mptcp_log_handle, "%s: nosp %d len %d opt %d %d %d\n",      \
                   __func__, len, dssoptlen, optlen, tp->t_maxseg,             \
                   tp->t_maxopd);                                              \
  ...
  if ((tp->t_mpflags & TMPF_SEND_DSN) && (send_64bit_dsn)) {
    /*
    ...
    dsn_ack_opt.mdss_copt.mdss_flags |= MDSS_M | MDSS_m | MDSS_A;

    CHECK_DATALEN;

    mptcp_output_getm_dsnmap64(so, off, &dsn_ack_opt.mdss_dsn,
                               &dsn_ack_opt.mdss_subflow_seqn,
                               &dsn_ack_opt.mdss_data_len, &dss_csum);

    if ((dsn_ack_opt.mdss_data_len == 0) || (dsn_ack_opt.mdss_dsn == 0)) {
      goto ret_optlen;
    }
    ...
    dsn_ack_opt.mdss_ack = htonl(MPTCP_DATAACK_LOW32(mp_tp->mpt_rcvnxt));

    dsn_ack_opt.mdss_dsn = mptcp_hton64(dsn_ack_opt.mdss_dsn);
    dsn_ack_opt.mdss_subflow_seqn = htonl(dsn_ack_opt.mdss_subflow_seqn);
    dsn_ack_opt.mdss_data_len = htons(dsn_ack_opt.mdss_data_len);
  ...
  if ((tp->t_mpflags & TMPF_SEND_DSN) && (!send_64bit_dsn) &&
    ...
    mptcp_output_getm_dsnmap32(so, off, &dsn_opt.mdss_dsn,
                               ...
                               &dsn_opt.mdss_data_len, &dss_csum);

    if ((dsn_opt.mdss_data_len == 0) || (dsn_opt.mdss_dsn == 0)) {
      goto ret_optlen;
    }
  ...
  if ((tp->t_mpflags & TMPF_MPTCP_ACKNOW) && (!send_64bit_ack) &&
      !(tp->t_mpflags & TMPF_SEND_DSN) && !(tp->t_mpflags & TMPF_SEND_DFIN)) {
  ...
  do_ack32_only:
    ...
    dack_opt.mdss_ack = htonl(MPTCP_DATAACK_LOW32(mp_tp->mpt_rcvnxt));
  ...
  if ((tp->t_mpflags & TMPF_SEND_DSN) && (!send_64bit_dsn) &&
      (!send_64bit_ack) && (tp->t_mpflags & TMPF_MPTCP_ACKNOW)) {
    ...
    dss_ack_opt.mdss_ack = htonl(MPTCP_DATAACK_LOW32(mp_tp->mpt_rcvnxt));

    CHECK_DATALEN;

    mptcp_output_getm_dsnmap32(so, off, &dss_ack_opt.mdss_dsn,
                               &dss_ack_opt.mdss_subflow_seqn,
                               &dss_ack_opt.mdss_data_len, &dss_csum);

    if ((dss_ack_opt.mdss_data_len == 0) || (dss_ack_opt.mdss_dsn == 0)) {
      goto do_ack32_only;
    }
    ...
    dss_ack_opt.mdss_subflow_seqn = htonl(dss_ack_opt.mdss_subflow_seqn);
  ...
  if ((tp->t_mpflags & TMPF_SEND_DSN) && (!send_64bit_dsn) &&
    ...
    dss_ack_opt.mdss_ack = mptcp_hton64(mp_tp->mpt_rcvnxt);
  ...
  if (tp->t_mpflags & TMPF_SEND_DFIN) {
    ...
    dss_ack_opt.mdss_ack = htonl(MPTCP_DATAACK_LOW32(mp_tp->mpt_rcvnxt));
    dss_ack_opt.mdss_dsn = htonl(MPTCP_DATASEQ_LOW32(mp_tp->mpt_sndmax - 1));
...
static int mptcp_sanitize_option(struct tcpcb *tp, int mptcp_subtype) {
  struct mptcb *mp_tp = tptomptp(tp);
  int ret = 1;
...
static void mptcp_do_mpcapable_opt(struct tcpcb *tp,
                                   u_char *cp __ended_by(optend),
                                   u_char *optend __unused, struct tcphdr *th,
                                   uint8_t optlen) {
  ...
  if (!(rsp->mmc_common.mmco_flags & MPCAP_PROPOSAL_SBIT) ||
      rsp->mmc_common.mmco_flags &
          (MPCAP_BBIT | MPCAP_DBIT | MPCAP_EBIT | MPCAP_FBIT | MPCAP_GBIT)) {
    tcpstat.tcps_invalid_mpcap++;
    return;
  }
  ...
  if (rsp->mmc_common.mmco_version < mp_tp->mpt_version) {
    os_log_error(mptcp_log_handle, "local version: %d > peer version %d",
...
static void mptcp_do_mpjoin_opt(struct tcpcb *tp, u_char *cp __ended_by(optend),
                                u_char *optend __unused, struct tcphdr *th,
                                uint8_t optlen) {
  ...
  mptcp_set_raddr_rand(tp->t_local_aid, tptomptp(tp), join_rsp->mmjo_addr_id,
                       join_rsp->mmjo_rand);
  error = mptcp_validate_join_hmac(tp, (u_char *)&join_rsp->mmjo_mac,
                                   HMAC_TRUNCATED_SYNACK);
  if (error) {
...
static int mptcp_validate_join_hmac(struct tcpcb *tp,
                                    u_char *hmac __sized_by(mac_len),
                                    int mac_len) {
  ...
  if (mp_tp->mpt_version == MPTCP_VERSION_0) {
    mptcp_hmac_sha1(mp_tp->mpt_remotekey, mp_tp->mpt_localkey, rem_rand,
                    loc_rand, digest);
  } else {
    uint32_t data[2];
    data[0] = rem_rand;
    data[1] = loc_rand;
    mptcp_hmac_sha256(mp_tp->mpt_remotekey, mp_tp->mpt_localkey, (u_char *)data,
                      8, digest);
  }
  ...
  if (bcmp(digest, hmac, mac_len) == 0) {
    ...
    printf("%s: remote key %llx local key %llx remote rand %x "
           "local rand %x \n",
           __func__, mp_tp->mpt_remotekey, mp_tp->mpt_localkey, rem_rand,
           loc_rand);
    return -1;
  }
}
 ...
 * in MPTCP layer
 */
void mptcp_data_ack_rcvd(struct mptcb *mp_tp, struct tcpcb *tp,
                         u_int64_t full_dack) {
  ...
  if (acked) {
    ...
    if (acked > mp_so->so_snd.sb_cc) {
      if (acked > mp_so->so_snd.sb_cc + 1 ||
        ...
        os_log_error(mptcp_log_handle,
                     ...
                     __func__,
                     (unsigned long)VM_KERNEL_ADDRPERM(mp_tp->mpt_mpte),
                     (uint32_t)acked, mp_so->so_snd.sb_cc, (uint32_t)full_dack,
    ...
    if ((tp->t_mpflags & TMPF_TCP_FALLBACK) &&
      ...
      mp_tp->mpt_flags |= MPTCPF_POST_FALLBACK_SYNC;
      tp->t_inpcb->inp_socket->so_flags1 |= SOF1_POST_FALLBACK_SYNC;
    }
  ...
  if (full_dack == mp_tp->mpt_sndmax && mp_tp->mpt_state >= MPTCPS_FIN_WAIT_1) {
...
void mptcp_update_window_wakeup(struct tcpcb *tp) {
  struct mptcb *mp_tp = tptomptp(tp);
...
static void mptcp_update_window(struct mptcb *mp_tp, u_int64_t ack,
                                u_int64_t seq, u_int32_t tiwin) {
...
static void mptcp_do_dss_opt_ack_meat(u_int64_t full_dack, u_int64_t full_dsn,
...
static void mptcp_do_dss_opt_meat(u_char *cp __ended_by(optend),
                                  u_char *optend __unused, struct tcpcb *tp,
                                  struct tcphdr *th) {
  ...
  case (MDSS_M | MDSS_A): {
    ...
    if (csum_len != 0) {
      csum = *(uint16_t *)(void *)(cp + (dss_ack_rsp->mdss_copt.mdss_len -
                                         csum_len));
    }

    mptcp_update_rcv_state_meat(mp_tp, tp, full_dsn,
                                dss_ack_rsp->mdss_subflow_seqn,
                                dss_ack_rsp->mdss_data_len, csum);
    break;
  }
  case (MDSS_M | MDSS_m): {
    ...
    mptcp_update_rcv_state_meat(mp_tp, tp, full_dsn, dsn64->mdss_subflow_seqn,
                                dsn64->mdss_data_len, csum);
    break;
  }
  case (MDSS_A | MDSS_a): {
  ...
  case (MDSS_M | MDSS_A | MDSS_a): {
    ...
    mp_tp->mpt_flags |= MPTCPF_RCVD_64BITACK;
    MPTCP_EXTEND_DSN(mp_tp->mpt_rcvnxt, dss32_ack64_opt->mdss_dsn, full_dsn);
    ...
    if (csum_len == 0) {
      ...
      mptcp_update_rcv_state_meat(
          mp_tp, tp, full_dsn, dss32_ack64_opt->mdss_subflow_seqn,
          dss32_ack64_opt->mdss_data_len,
          *(uint16_t *)(void *)(cp + dss32_ack64_opt->mdss_copt.mdss_len -
                                csum_len));
    }
    break;
  }
  case (MDSS_M | MDSS_m | MDSS_A | MDSS_a): {
    ...
    if (csum_len == 0) {
      ...
      mptcp_update_rcv_state_meat(
          mp_tp, tp, full_dsn, dss64_ack64->mdss_subflow_seqn,
          dss64_ack64->mdss_data_len,
          *(uint16_t *)(void *)(cp + dss64_ack64->mdss_copt.mdss_len -
                                csum_len));
    }
    break;
  }
...
static void mptcp_do_dss_opt(struct tcpcb *tp, u_char *cp __ended_by(optend),
                             u_char *optend, struct tcphdr *th,
                             uint8_t optlen) {
  struct mptcp_dss_copt *dss_rsp;
...
static void mptcp_do_fastclose_opt(struct tcpcb *tp,
                                   u_char *cp __ended_by(optend),
                                   u_char *optend __unused, struct tcphdr *th,
                                   uint8_t optlen) {
...
static void mptcp_do_mpfail_opt(struct tcpcb *tp, u_char *cp __ended_by(optend),
                                u_char *optend __unused, struct tcphdr *th,
                                uint8_t optlen) {
  struct mptcp_mpfail_opt *fail_opt;
  ...
  error = mptcp_get_map_for_dsn(
      tp->t_inpcb->inp_socket, mp_tp->mpt_dsn_at_csum_fail, &mdss_subflow_seqn);
  if (error == 0) {
...
static boolean_t mptcp_validate_add_addr_hmac(struct tcpcb *tp,
                                              u_char *hmac __sized_by(mac_len),
                                              u_char *msg __sized_by(msg_len),
                                              uint16_t msg_len,
                                              uint16_t mac_len) {
  ...
  mptcp_hmac_sha256(mp_tp->mpt_remotekey, mp_tp->mpt_localkey, msg, msg_len,
                    digest);
...
static void mptcp_do_add_addr_opt_v1(struct tcpcb *tp,
                                     u_char *cp __ended_by(optend),
                                     u_char *optend __unused, uint8_t optlen) {
  ...
  if (addr_opt->maddr_len < MPTCP_V1_ADD_ADDR_OPT_LEN_V6) {
    ...
    if (IN_ZERONET(haddr) || IN_LOOPBACK(haddr) || IN_LINKLOCAL(haddr) ||
        IN_DS_LITE(haddr) || IN_6TO4_RELAY_ANYCAST(haddr) ||
        IN_MULTICAST(haddr) || INADDR_BROADCAST == haddr || IN_PRIVATE(haddr) ||
    ...
    if (addr_opt->maddr_len > MPTCP_V1_ADD_ADDR_OPT_LEN_V4) {
      msg.maddr_port = *(uint16_t *)(void *)(cp + addr_opt->maddr_len -
                                             HMAC_TRUNCATED_ADD_ADDR - 2);
    }
    if (!mptcp_validate_add_addr_hmac(tp, hmac, (u_char *)&msg, msg_len,
                                      HMAC_TRUNCATED_ADD_ADDR)) {
      os_log_error(mptcp_log_handle,
                   "%s - %lx: ADD_ADDR addr: %x invalid HMAC\n", __func__,
                   (unsigned long)VM_KERNEL_ADDRPERM(mpte), addr->s_addr);
      return;
    }
    ...
    if (addr_opt->maddr_len > MPTCP_V1_ADD_ADDR_OPT_LEN_V4) {
      dst->sin_port = *(uint16_t *)(void *)(cp + addr_opt->maddr_len -
                                            HMAC_TRUNCATED_ADD_ADDR - 2);
    } else {
    ...
    if (IN6_IS_ADDR_LINKLOCAL(addr) || IN6_IS_ADDR_MULTICAST(addr) ||
        ...
        IN6_IS_ADDR_V4COMPAT(addr) || IN6_IS_ADDR_V4MAPPED(addr)) {
      ...
      os_log_error(mptcp_log_handle, "%s - %lx: ADD_ADDRv6 invalid addr: %s\n",
                   __func__, (unsigned long)VM_KERNEL_ADDRPERM(mpte), dbuf);
    ...
    if (!mptcp_validate_add_addr_hmac(tp, hmac, (u_char *)&msg, msg_len,
      ...
      os_log_error(mptcp_log_handle,
                   ...
                   (unsigned long)VM_KERNEL_ADDRPERM(mpte), dbuf);
      return;
    }
    ...
    if (addr_opt->maddr_len > MPTCP_V1_ADD_ADDR_OPT_LEN_V6) {
      dst->sin6_port = *(uint16_t *)(void *)(cp + addr_opt->maddr_len -
                                             HMAC_TRUNCATED_ADD_ADDR - 2);
    } else {
...
static void mptcp_do_add_addr_opt_v0(struct mptses *mpte,
                                     u_char *cp __ended_by(optend),
                                     u_char *optend __unused, uint8_t optlen) {
  struct mptcp_add_addr_opt *addr_opt;
...
void tcp_do_mptcp_options(struct tcpcb *tp, u_char *cp __ended_by(optend),
                          u_char *optend, struct tcphdr *th, struct tcpopt *to,
                          uint8_t optlen) {
  int mptcp_subtype = 0;
  struct mptcb *mp_tp = tptomptp(tp);
...
static int mptcp_echo_add_addr(struct tcpcb *tp, u_char *__indexable cp,
  ...
  size_t mpaddr_size = mpte->mpte_last_added_addr_is_v4
                           ? MPTCP_V1_ADD_ADDR_ECHO_OPT_LEN_V4
                           : MPTCP_V1_ADD_ADDR_ECHO_OPT_LEN_V6;
  ...
  if (mpte->mpte_last_added_addr_is_v4) {
    struct in_addr *addr =
        (struct in_addr *)(void *)(cp + sizeof(struct mptcp_add_addr_opt));
    addr->s_addr = mpte->mpte_sub_dst_v4.sin_addr.s_addr;
    mpaddr->maddr_addrid = mpte->sub_dst_addr_id_v4;
  } else {
    struct in6_addr *addr =
        (struct in6_addr *)(void *)(cp + sizeof(struct mptcp_add_addr_opt));
...
static int mptcp_snd_mpprio(struct tcpcb *tp, u_char *cp __ended_by(optend),
                            u_char *optend __unused, int optlen) {
