...
struct tcp_cc_algo tcp_cc_cubic = {.name = "cubic",
                                   .init = tcp_cubic_init,
...
static uint32_t tcp_cubic_update(struct tcpcb *tp, uint32_t rtt) {
  ...
  if (tp->t_ccstate->cub_epoch_start == 0) {
    ...
    if (win < tp->t_ccstate->cub_last_max) {
       ...
       */
      K = ((float)tp->t_ccstate->cub_last_max - win) / tp->t_maxseg /
          tcp_cubic_coeff;
      K = cbrtf(K);
   ...
   */
  elapsed_time = timer_diff(tcp_globals_now(globals), 0,
...
#define TCP_CUBIC_ENABLE_TCPMODE(_tp_)                                         \
  ((!soissrcrealtime((_tp_)->t_inpcb->inp_socket) &&                           \
    (_tp_)->t_ccstate->cub_mean_dev > (tp->t_maxseg << 1))                     \
       ? 1                                                                     \
       : 0)
 ...
 * with tcp_beta_cubic
 *
 * W(t) <- Wmax * tcp_beta_cubic + 3 * ((1 - tcp_beta_cubic)/(1 +
 * tcp_beta_cubic)) * t/RTT
 *
 */
  ...
  if (tp->t_ccstate->cub_tcp_win == 0) {
    ...
    while (tp->t_ccstate->cub_tcp_bytes_acked >= tp->snd_cwnd) {
      ...
      if (tp->snd_cwnd >= tp->t_ccstate->cub_last_max) {
        ...
        float ai_factor =
            (float)3 * (1 - tcp_cubic_beta) / (1 + tcp_cubic_beta);
...
static void tcp_cubic_pre_fr(struct tcpcb *tp) {
  ...
  if (win < tp->t_ccstate->cub_last_max) {
    tp->t_ccstate->cub_last_max =
        (uint32_t)((float)win * tcp_cubic_fast_convergence_factor);
  } else {
  ...
  if (tp->t_ccstate->cub_avg_lastmax == 0) {
    ...
    avg = (avg << 6) - avg;
    tp->t_ccstate->cub_avg_lastmax = (avg + tp->t_ccstate->cub_last_max) >> 6;
  }
  ...
  if (tp->t_ccstate->cub_mean_dev == 0) {
    ...
    dev = dev +
          ((tp->t_ccstate->cub_mean_dev << 4) - tp->t_ccstate->cub_mean_dev);
