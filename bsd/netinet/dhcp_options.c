...
/*
 * To build:
 * xcrun -sdk macosx.internal cc -DTEST_DHCP_OPTIONS -o /tmp/dhcp_options
 * dhcp_options.c -I ..
 */
#include <stdio.h>
#include <stdlib.h>
...
static void ptrlist_init(ptrlist_t *list) {
  bzero(list, sizeof(*list));
  return;
}

static void ptrlist_free(ptrlist_t *list) {
  if (list->array) {
...
static int ptrlist_count(ptrlist_t *list) {
...
static const void *ptrlist_element(ptrlist_t *list, int i) {
  if (list->array == NULL) {
    return NULL;
  }
  if (i < list->count) {
...
static bool ptrlist_grow(ptrlist_t *list, uint32_t n) {
  uint32_t new_size;
  const void **arr;
  ...
  arr =
      krealloc_type(const void *, list->size, new_size, list->array, Z_WAITOK);
  if (arr == NULL) {
    return false;
  }
...
static bool ptrlist_add(ptrlist_t *list, const void *element) {
...
static bool ptrlist_concat(ptrlist_t *list, ptrlist_t *extra) {
...
int dhcpol_count(dhcpol_t *list) { return ptrlist_count((ptrlist_t *)list); }

const void *dhcpol_element(dhcpol_t *list, int i) {
  return ptrlist_element((ptrlist_t *)list, i);
}

void dhcpol_init(dhcpol_t *list) { ptrlist_init((ptrlist_t *)list); }

void dhcpol_free(dhcpol_t *list) { ptrlist_free((ptrlist_t *)list); }
 ...
 *   the end tag.
 */
boolean_t dhcpol_parse_buffer(dhcpol_t *list, const void *buffer, int length) {
  int len;
  const uint8_t *scan;
  uint8_t tag;
 ...
 */
const void *dhcpol_find(dhcpol_t *list, int tag, int *len_p, int *start) {
  int i = 0;
 ...
 */
boolean_t dhcpol_parse_packet(dhcpol_t *options, const struct dhcp *pkt,
                              int len) {
  uint8_t rfc_magic[4] = RFC_OPTIONS_MAGIC;

  dhcpol_init(options); /* make sure it's empty */

  if (len < (sizeof(*pkt) + RFC_MAGIC_SIZE)) {
    dprintf(("dhcp_options: packet is too short: %d < %d\n", len,
             (int)sizeof(*pkt) + RFC_MAGIC_SIZE));
    return FALSE;
  }
...
char test_simple[] = {
    99, 130, 83, 99, 1, 4, 255, 255, 252, 0, 3, 4, 17, 202, 40, 1, 255,
};

char test_vendor[] = {
    99, 130, 83, 99, 1, 4, 255, 255, 252, 0, 3, 4, 17, 202, 40, 1,   43,
    6,  1,   4,  1,  2, 3, 4,   43,  6,   1, 4, 1, 2,  3,   4,  255,
};

char test_no_end[] = {
    0x63, 0x82, 0x53, 0x63, 0x35, 0x01, 0x05, 0x36, 0x04, 0xc0, 0xa8,
    0x01, 0x01, 0x33, 0x04, 0x80, 0x00, 0x80, 0x00, 0x01, 0x04, 0xff,
    0xff, 0xff, 0x00, 0x03, 0x04, 0xc0, 0xa8, 0x01, 0x01, 0x06, 0x0c,
    0x18, 0x1a, 0xa3, 0x21, 0x18, 0x1a, 0xa3, 0x20, 0x18, 0x5e, 0xa3,
    0x21, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

char test_no_magic[] = {0x1};
struct test {
  char *name;
  char *data;
  int len;
...
struct test tests[] = {
    {.name = "empty",
     .data = test_empty,
     .len = sizeof(test_empty),
     .result = TRUE},
    {.name = "simple",
     .data = test_simple,
     .len = sizeof(test_simple),
     .result = TRUE},
    {.name = "vendor",
     .data = test_vendor,
     .len = sizeof(test_vendor),
     .result = TRUE},
    {.name = "no_end",
     .data = test_no_end,
     .len = sizeof(test_no_end),
     .result = TRUE},
    {.name = "no magic",
     .data = test_no_magic,
     .len = sizeof(test_no_magic),
     .result = FALSE},
    {.name = "short",
     .data = test_short,
...
int main(void) {
  ...
  for (i = 0; tests[i].name; i++) {
    ...
    if (dhcpol_parse_packet(&options, pkt, sizeof(*pkt) + tests[i].len) !=
        tests[i].result) {
