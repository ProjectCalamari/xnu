...
static uint64_t microuptime_ns(void) {
...
static uint32_t tcp_pacer_get_packet_interval(struct tcpcb *tp, uint64_t size) {
  uint64_t rate = tp->t_pacer.rate;
  uint64_t interval;

  if (rate == 0) {
    os_log_error(OS_LOG_DEFAULT,
                 "%s: pacer rate shouldn't be 0, CCA is %s (cwnd=%u, smoothed "
                 "rtt=%u ms)",
                 __func__, CC_ALGO(tp)->name, tp->snd_cwnd,
...
#define BURST_SHIFT                                                            \
  (12) /* 1/(2^12) = 0.000244s, we allow a burst queue of at least 250us */
void tcp_update_pacer_state(struct tcpcb *tp) {
