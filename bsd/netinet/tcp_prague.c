...
static void tcp_prague_process_ecn(struct tcpcb *tp, struct tcphdr *th,
                                   uint32_t new_bytes_marked,
...
struct tcp_cc_algo tcp_cc_prague = {.name = "prague",
                                    ...
                                    .set_bytes_acked =
                                        tcp_prague_set_bytes_acked,
                                    .switch_to = tcp_prague_switch_cc};

/*
 * Prague state includes:
 * 1. Exponentially Weighted Moving Average (EWMA) -> alpha, of fraction of CE
 * marks [0,1]
 ...
 * W(t) = C(t-K)^3 + Wmax
 */
static uint32_t cubic_target(struct tcpcb *tp, uint32_t srtt) {
  ...
  if (tp->t_ccstate->cubic_epoch_start == 0) {
    ...
    if (tp->snd_cwnd < tp->t_ccstate->cubic_W_max) {
      ...
      K = ((float)(tp->t_ccstate->cubic_W_max - tp->snd_cwnd)) /
          cubic_c_factor / tp->t_maxseg;
      K = cbrtf(K);
   ...
   * W(t) = C(t-K)^3 + W_max
   */
  uint32_t elapsed_time = timer_diff(tcp_globals_now(globals), 0,
...
static uint32_t reno_friendly_win(struct tcpcb *tp, struct tcphdr *th) {
...
static void tcp_cubic_ack_ca(struct tcpcb *tp, struct tcphdr *th,
                             uint32_t acked) {
  ...
  if (incr_bytes_acked > 0 && tp->t_bytes_acked >= incr_bytes_acked) {
    ...
    tp->snd_cwnd =
        min(tp->snd_cwnd + tp->t_maxseg, TCP_MAXWIN << tp->snd_scale);
  }
}
...
static void tcp_prague_pre_fr(struct tcpcb *tp) {
  ...
  if (win < tp->t_ccstate->cubic_W_max) {
    tp->t_ccstate->cubic_W_max =
        (uint32_t)((float)win * cubic_fast_convergence_factor);
  } else {
...
static void tcp_prague_post_fr(struct tcpcb *tp, __unused struct tcphdr *th) {
  /*
...
static bool rtt_elapsed(uint32_t largest_snd_nxt, uint32_t ack) {
  /*
 ...
 */
static void prague_ai_alpha_rate(struct tcpcb *tp) {
  uint32_t srtt = tp->t_srtt >> TCP_RTT_SHIFT;
  ...
  tp->t_ccstate->prague_alpha_ai =
      (uint64_t)((numer + (divisor >> 1)) / divisor);
}
...
static void prague_update_alpha(struct tcpcb *tp, uint32_t ack,
                                uint32_t packets_marked,
                                uint32_t packets_acked) {
...
static bool prague_cwr(struct tcpcb *tp) {
   ...
   */
  uint64_t reduction = (tp->snd_cwnd * alpha) >> (ALPHA_SHIFT + 1);
...
static void tcp_prague_process_ecn(struct tcpcb *tp, struct tcphdr *th,
  ...
  if (__improbable(packets_marked < tp->t_ccstate->prague_ce_counter ||
    ...
    os_log_error(OS_LOG_DEFAULT,
                 "new CE count (%u) can't be less than current CE count (%u)"
...
int tcp_prague_init(struct tcpcb *tp) {
...
int tcp_prague_cleanup(struct tcpcb *tp) {
#pragma unused(tp)
 ...
 */
void tcp_prague_cwnd_init_or_reset(struct tcpcb *tp) {
  VERIFY(tp->t_ccstate != NULL);
