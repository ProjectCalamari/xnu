...
static void set_dscp_to_wifi_ac_map(const struct dcsp_msc_map *__indexable,
                                    int);
static errno_t dscp_msc_map_from_netsvctype_dscp_map(
    struct netsvctype_dscp_map *__counted_by(count) map, size_t count,
    struct dcsp_msc_map *__counted_by(DSCP_ARRAY_SIZE));

SYSCTL_NODE(_net, OID_AUTO, qos, CTLFLAG_RW | CTLFLAG_LOCKED, 0, "QoS");
...
SYSCTL_PROC(_net_qos, OID_AUTO, reset_dscp_to_wifi_ac_map,
            ...
            sysctl_reset_dscp_to_wifi_ac_map, "I", "");

int net_qos_verbose = 0;
SYSCTL_INT(_net_qos, OID_AUTO, verbose, CTLFLAG_RW | CTLFLAG_LOCKED,
           &net_qos_verbose, 0, "");
 ...
 */
SYSCTL_NODE(_net_qos, OID_AUTO, policy, CTLFLAG_RW | CTLFLAG_LOCKED, 0, "");

int net_qos_policy_restricted = 0;
SYSCTL_INT(_net_qos_policy, OID_AUTO, restricted, CTLFLAG_RW | CTLFLAG_LOCKED,
...
SYSCTL_INT(_net_qos_policy, OID_AUTO, wifi_enabled, CTLFLAG_RW | CTLFLAG_LOCKED,
           &net_qos_policy_wifi_enabled, 0, "");
 ...
 */
static const struct netsvctype_dscp_map
    fastlane_netsvctype_dscp_map[_NET_SERVICE_TYPE_COUNT] = {
 ...
 */
static const struct netsvctype_dscp_map
    rfc4594_netsvctype_dscp_map[_NET_SERVICE_TYPE_COUNT] = {
        {.netsvctype = NET_SERVICE_TYPE_BE, .dscp = _DSCP_DF},
...
/*
 * The DSCP to UP mapping (via mbuf service class) for WiFi follows is the
 * mapping that implemented at the 802.11 driver level when the mbuf service
 * class is MBUF_SC_BE.
 *
...
static const struct dcsp_msc_map default_dscp_to_wifi_ac_map[] = {
    ...
    {.dscp = _DSCP_AF41,
     .msc = MBUF_SC_VI},             /* RFC 2597 Multimedia Conferencing */
    ...
    {.dscp = _DSCP_CS6,
     .msc = MBUF_SC_VO},             /* Wi-Fi WMM Certification: Chariot */
...
static TAILQ_HEAD(,
                  tclass_for_proc) tfp_head = TAILQ_HEAD_INITIALIZER(tfp_head);
 ...
 */
static struct tclass_for_proc *find_tfp_by_pid(pid_t pid) {
 ...
 * Must be called with tclass_lock held
 */
static struct tclass_for_proc *find_tfp_by_pname(const char *pname) {
  struct tclass_for_proc *tfp;

  TAILQ_FOREACH(tfp, &tfp_head, tfp_link) {
    if (strlcmp(tfp->tfp_pname, pname, sizeof(tfp->tfp_pname)) == 0) {
      break;
    }
  }
  return tfp;
...
__private_extern__ void set_tclass_for_curr_proc(struct socket *so) {
  ...
  TAILQ_FOREACH(tfp, &tfp_head, tfp_link) {
    if ((tfp->tfp_pid == pid) ||
        (tfp->tfp_pid == -1 &&
         strlcmp(tfp->tfp_pname, pname, sizeof(tfp->tfp_pname)) == 0)) {
 ...
 */
int purge_tclass_for_proc(void) {
  int error = 0;
 ...
 * Must be called with tclass_lock held
 */
static void free_tclass_for_proc(struct tclass_for_proc *tfp) {
  if (tfp == NULL) {
    return;
  }
  tfp_count--;
 ...
 * Remove all entries
 */
int flush_tclass_for_proc(void) {
  int error = 0;
  struct tclass_for_proc *tfp, *tvar;
...
alloc_tclass_for_proc(pid_t pid, const char *__sized_by(pnamelen) pname,
                      size_t pnamelen) {
  struct tclass_for_proc *tfp;
  ...
  if (pid != -1) {
    ...
    if (pname != NULL) {
      strbufcpy(tfp->tfp_pname, sizeof(tfp->tfp_pname), pname, pnamelen);
    } else {
      tfp->tfp_pname[0] = '\0';
    }
 ...
 */
int set_pid_tclass(struct so_tcdbg *so_tcdbg) {
  int error = EINVAL;
  proc_t p = NULL;
  struct tclass_for_proc *tfp;
    ...
    fdt_foreach(fp, p) {
      ...
      if (tp != NULL) {
        if (ecn_val == IPTOS_ECN_ECT1 || ecn_val == IPTOS_ECN_ECT0) {
          tp->ecn_flags |=
              (ecn_val == IPTOS_ECN_ECT1) ? TE_FORCE_ECT1 : TE_FORCE_ECT0;
        } else {
      ...
      if (netsvctype != _NET_SERVICE_TYPE_UNSPEC) {
        error = sock_setsockopt(so, SOL_SOCKET, SO_NET_SERVICE_TYPE,
                                &netsvctype, sizeof(int));
      }
      if (tclass != SO_TC_UNSPEC) {
        error = sock_setsockopt(so, SOL_SOCKET, SO_TRAFFIC_CLASS, &tclass,
                                sizeof(int));
      }
    }
...
int set_pname_tclass(struct so_tcdbg *so_tcdbg) {
  int error = EINVAL;
  struct tclass_for_proc *tfp;

  lck_mtx_lock(&tclass_lock);

  tfp = find_tfp_by_pname(
      __unsafe_null_terminated_from_indexable(so_tcdbg->so_tcdbg_pname));
  if (tfp == NULL) {
...
static int flush_pid_tclass(struct so_tcdbg *so_tcdbg) {
  pid_t pid = so_tcdbg->so_tcdbg_pid;
  int tclass = so_tcdbg->so_tcdbg_tclass;
  struct fileproc *fp;
  proc_t p;
  ...
  fdt_foreach(fp, p) {
    ...
    so = (struct socket *)fp_get_data(fp);
    error = sock_setsockopt(so, SOL_SOCKET, SO_FLUSH, &tclass, sizeof(tclass));
    if (error != 0) {
      printf("%s: setsockopt(SO_FLUSH) (so=0x%llx, fd=%d, "
             "tclass=%d) failed %d\n",
             __func__, (uint64_t)VM_KERNEL_ADDRPERM(so), fdt_foreach_fd(),
             tclass, error);
    }
  }
...
int get_pid_tclass(struct so_tcdbg *so_tcdbg) {
  int error = EINVAL;
  proc_t p = NULL;
  struct tclass_for_proc *tfp;
  pid_t pid = so_tcdbg->so_tcdbg_pid;
...
int get_pname_tclass(struct so_tcdbg *so_tcdbg) {
  int error = EINVAL;
  struct tclass_for_proc *tfp;
...
static int delete_tclass_for_pid_pname(struct so_tcdbg *so_tcdbg) {
  int error = EINVAL;
  pid_t pid = so_tcdbg->so_tcdbg_pid;
  struct tclass_for_proc *tfp = NULL;
 ...
 */
__private_extern__ int so_set_tcdbg(struct socket *so,
 ...
 */
__private_extern__ int sogetopt_tcdbg(struct socket *so, struct sockopt *sopt) {
  int error = 0;
  struct so_tcdbg so_tcdbg;
  void *buf = NULL;
  ...
  case SO_TCDBG_LIST: {
    ...
    TAILQ_FOREACH(tfp, &tfp_head, tfp_link) {
      ...
      if (tfp->tfp_pid != -1) {
        ...
        ptr->so_tcdbg_pid = -1;
        strbufcpy(ptr->so_tcdbg_pname, tfp->tfp_pname);
      }
...
int so_get_netsvc_marking_level(struct socket *so) {
...
__private_extern__ int so_set_traffic_class(struct socket *so, int optval) {
  int error = 0;
  ...
  if (optval < SO_TC_BE || optval > SO_TC_CTL) {
    ...
    if (error == 0) {
      ...
      if ((SOCK_DOM(so) == PF_INET || SOCK_DOM(so) == PF_INET6) &&
          optval != oldval &&
          (optval == SO_TC_BK_SYS || oldval == SO_TC_BK_SYS)) {
        /*
...
__private_extern__ int so_set_net_service_type(struct socket *so,
                                               int netsvctype) {
  int sotc;
  int error;
...
__private_extern__ void so_set_default_traffic_class(struct socket *so) {
...
__private_extern__ int so_set_opportunistic(struct socket *so, int optval) {
  return so_set_traffic_class(so, (optval == 0) ? SO_TC_BE : SO_TC_BK_SYS);
}

__private_extern__ int so_get_opportunistic(struct socket *so) {
  return so->so_traffic_class == SO_TC_BK_SYS;
}

__private_extern__ int ip_tos_from_control(struct mbuf *control) {
  struct cmsghdr *cm;
 ...
 */
__private_extern__ void so_recv_data_stat(struct socket *so, struct mbuf *m,
                                          size_t off) {
...
__private_extern__ void so_inc_recv_data_stat(struct socket *so, size_t pkts,
                                              size_t bytes) {
  so->so_tc_stats[SO_STATS_DATA].rxpackets += pkts;
  so->so_tc_stats[SO_STATS_DATA].rxbytes += bytes;
}

static inline int so_throttle_best_effort(struct socket *so,
                                          struct ifnet *ifp) {
  uint32_t uptime = (uint32_t)net_uptime();
  return soissrcbesteffort(so) && net_io_policy_throttle_best_effort == 1 &&
         ifp->if_rt_sendts > 0 &&
         (int)(uptime - ifp->if_rt_sendts) <= TCP_BG_SWITCH_TIME;
}

__private_extern__ void set_tcp_stream_priority(struct socket *so) {
  struct inpcb *inp = sotoinpcb(so);
  ...
  VERIFY((SOCK_CHECK_DOM(so, PF_INET) || SOCK_CHECK_DOM(so, PF_INET6)) &&
  ...
  if (old_cc != tp->tcp_cc_index || recvbg != IS_TCP_RECV_BG(so)) {
    SOTHROTTLELOG("throttle[%d]: so 0x%llx [%d,%d] TCP %s send; "
                  "%s recv\n",
                  so->last_pid, (uint64_t)VM_KERNEL_ADDRPERM(so), SOCK_DOM(so),
                  SOCK_TYPE(so),
 ...
 */
__private_extern__ void set_packet_service_class(struct mbuf *m,
                                                 struct socket *so, int sotc,
                                                 uint32_t flags) {
...
__private_extern__ mbuf_svc_class_t so_tc2msc(int tc) {
  mbuf_svc_class_t msc;
...
__private_extern__ int so_svc2tc(mbuf_svc_class_t svc) {
  switch (svc) {
  case MBUF_SC_BK_SYS:
    return SO_TC_BK_SYS;
  case MBUF_SC_BK:
...
static size_t sotc_index(int sotc) {
  switch (sotc) {
  case SO_TC_BK_SYS:
...
uint8_t fastlane_sc_to_dscp(uint32_t svc_class) {
...
uint8_t rfc4594_sc_to_dscp(uint32_t svc_class) {
  uint8_t dscp = _DSCP_DF;
...
mbuf_traffic_class_t rfc4594_dscp_to_tc(uint8_t dscp) {
 ...
 */
static errno_t set_netsvctype_dscp_map(
...
static size_t get_netsvctype_dscp_map(struct netsvctype_dscp_map *__counted_by(
    _NET_SERVICE_TYPE_COUNT) netsvctype_dscp_map) {
  struct net_qos_dscp_map *net_qos_dscp_map;
  int i;
...
int sysctl_default_netsvctype_to_dscp_map SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  int error = 0;

  if (req->oldptr == USER_ADDR_NULL) {
    req->oldidx = _NET_SERVICE_TYPE_COUNT * sizeof(struct netsvctype_dscp_map);
    ...
    error = SYSCTL_OUT(req, netsvctype_dscp_map, MIN(len, req->oldlen));
    if (error != 0) {
      goto done;
    }
  }
...
__private_extern__ errno_t set_packet_qos(struct mbuf *m, struct ifnet *ifp,
                                          boolean_t qos_allowed, int sotc,
                                          int netsvctype, uint8_t *dscp_inout) {
  ...
  if ((ifp->if_eflags & IFEF_QOSMARKING_ENABLED) != 0 &&
    ...
    if (IS_VALID_NET_SERVICE_TYPE(netsvctype) &&
      ...
      if (qos_allowed == FALSE && netsvctype != NET_SERVICE_TYPE_BE &&
      ...
      if (sotcix != SIZE_T_MAX) {
        ...
        if (qos_allowed == FALSE && sotc != SO_TC_BE && sotc != SO_TC_BK &&
            sotc != SO_TC_BK_SYS && sotc != SO_TC_CTL) {
          dscp = _DSCP_DF;
          if (sotc != SO_TC_CTL) {
            m_set_service_class(m, MBUF_SC_BE);
          }
    ...
    if (net_qos_verbose != 0) {
      printf("%s qos_allowed %d sotc %u netsvctype %u dscp %u\n", __func__,
             qos_allowed, sotc, netsvctype, dscp);
    }
...
static void set_dscp_to_wifi_ac_map(const struct dcsp_msc_map *__indexable map,
                                    int clear) {
  int i;
...
static errno_t dscp_msc_map_from_netsvctype_dscp_map(
    struct netsvctype_dscp_map *__counted_by(count) netsvctype_dscp_map,
    size_t count,
    struct dcsp_msc_map *__counted_by(DSCP_ARRAY_SIZE) dcsp_msc_map) {
  errno_t error = 0;
  uint32_t i;
...
int sysctl_dscp_to_wifi_ac_map SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  int error = 0;
  ...
  if (req->oldptr == USER_ADDR_NULL) {
    ...
    for (i = 0; i < DSCP_ARRAY_SIZE; i++) {
      ...
      netsvctype_dscp_map[i].netsvctype = so_svc2tc(wifi_dscp_to_msc_array[i]);
    }
    error = SYSCTL_OUT(req, netsvctype_dscp_map, MIN(len, req->oldlen));
    if (error != 0) {
      goto done;
...
int sysctl_reset_dscp_to_wifi_ac_map SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  int error = 0;
  int val = 0;
...
int net_qos_guideline(struct proc *p, struct net_qos_guideline_args *arg,
                      int *retval) {
#pragma unused(p)
 ...
 */
SYSCTL_NODE(_net_qos, OID_AUTO, custom, CTLFLAG_RW | CTLFLAG_LOCKED, 0, "");
...
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, be,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_BE,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, bk,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_BK,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, sig,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_SIG,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, vi,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_VI,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, vo,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_VO,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, rv,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_RV,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, av,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_AV,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, oam,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_OAM,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");
SYSCTL_PROC(_net_qos_custom_netsvctype_to_dscp, OID_AUTO, rd,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, NET_SERVICE_TYPE_RD,
            sysctl_net_qos_custom_netsvctype_to_dscp, "I", "");

static int sysctl_net_qos_custom_reset SYSCTL_HANDLER_ARGS;
SYSCTL_PROC(_net_qos_custom, OID_AUTO, reset,
            CTLTYPE_INT | CTLFLAG_WR | CTLFLAG_LOCKED, 0, 0,
            sysctl_net_qos_custom_reset, "I", "");

int sysctl_net_qos_custom_netsvctype_to_dscp SYSCTL_HANDLER_ARGS {
#pragma unused(arg1)
  int error = 0;
  ...
  case NET_SERVICE_TYPE_RD:
    break;
  default:
    os_log(OS_LOG_DEFAULT, "%s: unexpected netsvctype %d", __func__, arg2);
    return EINVAL;
  }
  ...
  if (val < 0 || val > _MAX_DSCP) {
    os_log(OS_LOG_DEFAULT, "%s: unexpected DSCP %d", __func__, val);
    return EINVAL;
  }
  ...
  error =
      set_netsvctype_dscp_map(&custom_net_qos_dscp_map, netsvctype_dscp_map);

  return 0;
}

int sysctl_net_qos_custom_reset SYSCTL_HANDLER_ARGS {
...
uint8_t custom_sc_to_dscp(uint32_t svc_class) {
  uint8_t dscp = _DSCP_DF;
