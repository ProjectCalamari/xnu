...
struct mppcb {
  ...
  int32_t
      mpp_inside; /* Indicates whether or not a thread is processing MPTCP */
...
static inline struct mppcb *mpsotomppcb(struct socket *mp_so) {
...
#define MPP_SHOULD_WORKLOOP                                                    \
  0x020 /* MPTCP-stack should call the workloop function */
...
#define MPP_CREATE_SUBFLOWS 0x100 /* This connection needs to create subflows  \
                                   */
#define MPP_INSIDE_SETGETOPT                                                   \
  0x200 /* MPTCP-stack is inside mptcp_setopt/mptcp_getopt */

static inline boolean_t mptcp_should_defer_upcall(struct mppcb *mpp) {
  return !!(mpp->mpp_flags & (MPP_INSIDE_OUTPUT | MPP_INSIDE_INPUT |
                              MPP_INPUT_HANDLE | MPP_WUPCALL));
}
...
#if NECP
extern int necp_client_register_multipath_cb(pid_t pid, uuid_t client_id,
                                             struct mppcb *mpp);
