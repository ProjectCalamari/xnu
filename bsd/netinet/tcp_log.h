...
extern void tcp_log_connection_summary(const char *func_name, int line_no,
                                       struct tcpcb *tp);
extern void tcp_log_th_flags(void *hdr, struct tcphdr *th, struct tcpcb *tp,
...
extern void tcp_log_drop_pcb(void *hdr, struct tcphdr *th, struct tcpcb *tp,
                             bool outgoing, const char *reason);
extern void tcp_log_drop_pkt(void *hdr, struct tcphdr *th, struct ifnet *ifp,
                             const char *reason);
extern void tcp_log_rtt_info(const char *func_name, int line_no,
                             struct tcpcb *tp);
extern void tcp_log_rt_rtt(const char *func_name, int line_no, struct tcpcb *tp,
                           struct rtentry *rt);
extern void tcp_log_rtt_change(const char *func_name, int line_no,
                               struct tcpcb *tp, int old_srtt, int old_rttvar);
extern void tcp_log_keepalive(const char *func_name, int line_no,
                              struct tcpcb *tp, int32_t idle_time);
extern void tcp_log_message(const char *func_name, int line_no,
                            struct tcpcb *tp, const char *format, ...)
    __printflike(4, 5);
extern void tcp_log_fsw_flow(const char *func_name, int line_no,
                             struct tcpcb *tp, const char *format, ...)
    __printflike(4, 5);
extern void tcp_log_state_change(const char *func_name, int line_no,
                                 struct tcpcb *tp, int new_state);
extern void tcp_log_output(const char *func_name, int line_no, struct tcpcb *tp,
...
static inline bool tcp_is_log_enabled(struct tcpcb *tp, uint32_t req_flags) {
  struct inpcb *inp;
  ...
  inp = tp->t_inpcb;
  if (tcp_log_port > 0 && ntohs(inp->inp_lport) != tcp_log_port &&
  ...
  if ((inp->inp_log_flags & TLEF_MASK_DST) == 0) {
    ...
    if (tp->t_state == TCPS_LISTEN &&
        (inp->inp_log_flags & TLEF_MASK_DST) == 0) {
      /*
...
static inline bool tcp_log_summary_needed(struct tcpcb *tp) {
  struct inpcb *inp;
...
#define TCP_LOG_RTT_INFO(tp)                                                   \
  if (tcp_is_log_enabled(tp, TLEF_RTT))                                        \
  tcp_log_rtt_info(__func__, __LINE__, (tp))

#define TCP_LOG_RTM_RTT(tp, rt)                                                \
  if (tcp_is_log_enabled(tp, TLEF_RTT))                                        \
  tcp_log_rt_rtt(__func__, __LINE__, (tp), (rt))

#define TCP_LOG_RTT_CHANGE(tp, old_srtt, old_rttvar)                           \
  if (tcp_is_log_enabled(tp, TLEF_RTT))                                        \
  tcp_log_rtt_change(__func__, __LINE__, (tp), (old_srtt), (old_rttvar))

#define TCP_LOG_KEEP_ALIVE(tp, idle_time)                                      \
  if (tcp_is_log_enabled(tp, TLEF_KEEP_ALIVE))                                 \
  tcp_log_keepalive(__func__, __LINE__, (tp), (idle_time))

#define TCP_LOG_CONNECT(tp, outgoing, error)                                   \
  if (tcp_is_log_enabled(tp, TLEF_CONNECTION))                                 \
  tcp_log_connection((tp),                                                     \
                     __unsafe_forge_null_terminated(                           \
                         const char *, ((outgoing) ? "connect outgoing"        \
                                                   : "connect incoming")),     \
                     (error))

#define TCP_LOG_CONNECTED(tp, error)                                           \
  if (tcp_is_log_enabled(tp, TLEF_CONNECTION))                                 \
  tcp_log_connection((tp), "connected", (error))

#define TCP_LOG_LISTEN(tp, error)                                              \
  if (tcp_is_log_enabled(tp, TLEF_CONNECTION))                                 \
  tcp_log_listen((tp), (error))

#define TCP_LOG_ACCEPT(tp, error)                                              \
  if (tcp_is_log_enabled(tp, TLEF_CONNECTION))                                 \
  tcp_log_connection((tp), "accept", (error))

#define TCP_LOG_CONNECTION_SUMMARY(tp)                                         \
  if (tcp_log_summary_needed(tp))                                              \
  tcp_log_connection_summary(__func__, __LINE__, (tp))

#define TCP_LOG_DROP_NECP(hdr, th, tp, outgoing)                               \
  if (tcp_is_log_enabled(tp, TLEF_DROP_NECP))                                  \
  tcp_log_drop_pcb((hdr), (th), (tp), (outgoing), "NECP")

#define TCP_LOG_DROP_PCB(hdr, th, tp, outgoing, reason)                        \
  if (tcp_is_log_enabled(tp, TLEF_DROP_PCB))                                   \
...
#define TCP_LOG_FSW_FLOW(tp, format, ...)                                      \
  if (tcp_is_log_enabled(tp, TLEF_FSW_FLOW))                                   \
  tcp_log_fsw_flow(__func__, __LINE__, (tp), format, ##__VA_ARGS__)

#define TCP_LOG(tp, format, ...)                                               \
  if (tcp_is_log_enabled(tp, TLEF_LOG))                                        \
  tcp_log_message(__func__, __LINE__, tp, format, ##__VA_ARGS__)

#define TCP_LOG_STATE(tp, new_state)                                           \
  if (tcp_is_log_enabled(tp, TLEF_STATE))                                      \
  tcp_log_state_change(__func__, __LINE__, (tp), (new_state))

#define TCP_LOG_OUTPUT(tp, format, ...)                                        \
  if (tcp_is_log_enabled(tp, TLEF_OUTPUT))                                     \
  tcp_log_output(__func__, __LINE__, tp, format, ##__VA_ARGS__)

#define TCP_LOG_BIND(tp, error)                                                \
  if (tcp_is_log_enabled(tp, TLEF_BIND))                                       \
