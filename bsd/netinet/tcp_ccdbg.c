...
static errno_t tcp_ccdbg_control_disconnect(kern_ctl_ref kctl, u_int32_t unit,
                                            void *uinfo) {
#pragma unused(kctl, unit, uinfo)
  ...
  if (unit == tcp_ccdbg_unit) {
    ...
    if (!OSCompareAndSwap(old_value, new_value, &tcp_ccdbg_unit)) {
      log(LOG_DEBUG, "failed to disconnect tcp_cc debug control");
    }
  }
  return 0;
}

inline void tcp_ccdbg_trace(struct tcpcb *tp, struct tcphdr *th,
                            int32_t event) {
#if !CONFIG_DTRACE
#pragma unused(th)
#endif /* !CONFIG_DTRACE */
  ...
  if (tcp_cc_debug && tcp_ccdbg_unit > 0) {
    ...
    inet_ntop(SOCK_DOM(inp->inp_socket),
              ((SOCK_DOM(inp->inp_socket) == PF_INET)
                   ...
                   : (void *)&inp->in6p_laddr),
              dbg_state.ccd_srcaddr, sizeof(dbg_state.ccd_srcaddr));
    dbg_state.ccd_srcport = ntohs(inp->inp_lport);
    inet_ntop(SOCK_DOM(inp->inp_socket),
              ((SOCK_DOM(inp->inp_socket) == PF_INET)
                   ? (void *)&inp->inp_faddr.s_addr
                   : (void *)&inp->in6p_faddr),
              dbg_state.ccd_destaddr, sizeof(dbg_state.ccd_destaddr));
    ...
    case TCP_CC_ALGO_CUBIC_INDEX:
      dbg_state.u.cubic_state.ccd_last_max = tp->t_ccstate->cub_last_max;
      dbg_state.u.cubic_state.ccd_tcp_win = tp->t_ccstate->cub_tcp_win;
      dbg_state.u.cubic_state.ccd_avg_lastmax = tp->t_ccstate->cub_avg_lastmax;
      dbg_state.u.cubic_state.ccd_mean_deviation = tp->t_ccstate->cub_mean_dev;
      break;
    case TCP_CC_ALGO_BACKGROUND_INDEX:
      dbg_state.u.ledbat_state.led_base_rtt = get_base_rtt(tp);
      break;
    default:
      break;
    }

    ctl_enqueuedata(tcp_ccdbg_ctlref, tcp_ccdbg_unit, &dbg_state,
                    sizeof(dbg_state), 0);
  }
  DTRACE_TCP5(cc, void, NULL, struct inpcb *, inp, struct tcpcb *, tp,
              struct tcphdr *, th, int32_t, event);
}

void tcp_ccdbg_control_register(void) {
