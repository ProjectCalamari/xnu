 ...
 *       @param protocol The protocol type (udp/tcp/icmp/etc...) of the IP
 * packet
 *       @result Return:
...
#define ipf_addv4(filter, filter_ref) ipf_addv4_internal((filter), (filter_ref))
#else
extern errno_t ipf_addv4(const struct ipf_filter *filter,
                         ipfilter_t *filter_ref) __NKE_API_DEPRECATED;
...
#define ipf_addv6(filter, filter_ref) ipf_addv6_internal((filter), (filter_ref))
#else
 ...
 *       @param filter_ref The reference to the filter returned from ipf_addv4
 * or ipf_addv6.
 *       @result 0 on success otherwise the errno error.
 */
extern errno_t ipf_remove(ipfilter_t filter_ref) __NKE_API_DEPRECATED;
 ...
 *       @result 0 on success otherwise the errno error.
 */
extern errno_t ipf_inject_input(mbuf_t data,
                                ipfilter_t filter_ref) __NKE_API_DEPRECATED;
 ...
 *               chance to process the packet. If the filter modified the
 * packet, it should not specify the filter ref to give other filters a chance
...
extern errno_t ipf_inject_output(mbuf_t data, ipfilter_t filter_ref,
                                 ipf_pktopts_t options) __NKE_API_DEPRECATED;
__END_DECLS
