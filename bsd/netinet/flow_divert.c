...
#define FDLOG(level, pcb, format, ...)                                         \
  os_log_with_type(OS_LOG_DEFAULT,                                             \
                   flow_divert_syslog_type_to_oslog_type(level),               \
...
#define FDRETAIN(pcb)                                                          \
  if ((pcb) != NULL)                                                           \
  OSIncrementAtomic(&(pcb)->ref_count)
...
#define FDGRP_RETAIN(grp)                                                      \
  if ((grp) != NULL)                                                           \
  OSIncrementAtomic(&(grp)->ref_count)
#define FDGRP_RELEASE(grp)                                                     \
  if ((grp) != NULL && 1 == OSDecrementAtomic(&(grp)->ref_count))              \
  flow_divert_group_destroy(grp)
...
#define TRIE_CHILD(t, i, b)                                                    \
  (((t)->child_maps + (CHILD_MAP_SIZE * TRIE_NODE(t, i).child_map))[(b)])
#define TRIE_BYTE(t, i) ((t)->bytes[(i)])

#define SO_IS_DIVERTED(s)                                                      \
  (((s)->so_flags & SOF_FLOW_DIVERT) && (s)->so_fd_pcb != NULL)
...
static TAILQ_HEAD(_flow_divert_group_list,
                  flow_divert_group) g_flow_divert_in_process_group_list;
...
static errno_t flow_divert_dup_addr(sa_family_t family, struct sockaddr *addr,
                                    struct sockaddr **dup);

static boolean_t flow_divert_is_sockaddr_valid(struct sockaddr *addr);

static int flow_divert_append_target_endpoint_tlv(mbuf_ref_t connect_packet,
                                                  struct sockaddr *toaddr);

struct sockaddr *flow_divert_get_buffered_target_address(mbuf_ref_t buffer);

static void flow_divert_disconnect_socket(struct socket *so, bool is_connected,
                                          bool delay_if_needed);

static void flow_divert_group_destroy(struct flow_divert_group *group);

static inline uint8_t flow_divert_syslog_type_to_oslog_type(int syslog_type) {
  switch (syslog_type) {
  case LOG_ERR:
...
static inline int flow_divert_pcb_cmp(const struct flow_divert_pcb *pcb_a,
                                      const struct flow_divert_pcb *pcb_b) {
...
static const char *flow_divert_packet_type2str(uint8_t packet_type) {
  switch (packet_type) {
...
static inline void flow_divert_lock_socket(struct socket *so,
                                           struct flow_divert_pcb *fd_cb) {
  socket_lock(so, 0);
  fd_cb->plugin_locked = true;
}

static inline void flow_divert_unlock_socket(struct socket *so,
                                             struct flow_divert_pcb *fd_cb) {
...
flow_divert_group_lookup(uint32_t ctl_unit, struct flow_divert_pcb *fd_cb) {
  ...
  if (g_active_group_count == 0) {
    if (fd_cb != NULL) {
      FDLOG0(LOG_ERR, fd_cb,
             "No active groups, flow divert cannot be used for this socket");
    }
  } else if (ctl_unit == 0 || (ctl_unit >= GROUP_COUNT_MAX &&
                               ctl_unit < FLOW_DIVERT_IN_PROCESS_UNIT_MIN)) {
    FDLOG(LOG_ERR, fd_cb, "Cannot lookup group with invalid control unit (%u)",
          ctl_unit);
  } else if (ctl_unit < FLOW_DIVERT_IN_PROCESS_UNIT_MIN) {
    if (g_flow_divert_groups == NULL) {
      if (fd_cb != NULL) {
        FDLOG0(LOG_ERR, fd_cb,
               "No active non-in-process groups, flow divert cannot be used "
               "for this socket");
      }
    } else {
      ...
      if (group == NULL) {
        if (fd_cb != NULL) {
          FDLOG(LOG_ERR, fd_cb,
                "Group for control unit %u is NULL, flow divert cannot be used "
                "for this socket",
                ctl_unit);
        }
      } else {
    ...
    if (TAILQ_EMPTY(&g_flow_divert_in_process_group_list)) {
      if (fd_cb != NULL) {
        FDLOG0(LOG_ERR, fd_cb,
               "No active in-process groups, flow divert cannot be used for "
               "this socket");
      }
    } else {
      ...
      if (group == NULL) {
        if (fd_cb != NULL) {
          FDLOG(LOG_ERR, fd_cb,
                "Group for control unit %u not found, flow divert cannot be "
        ...
        FDLOG(LOG_ERR, fd_cb,
              "Cannot access group for control unit %u, mismatched PID (%u != "
              "%u)",
              ctl_unit, group_cursor->in_process_pid,
              fd_cb->so ? fd_cb->so->last_pid : 0);
        group = NULL;
      } else {
        FDGRP_RETAIN(group);
      }
...
static errno_t flow_divert_pcb_insert(struct flow_divert_pcb *fd_cb,
  ...
  if (!(group->flags & FLOW_DIVERT_GROUP_FLAG_DEFUNCT)) {
    if (NULL == RB_INSERT(fd_pcb_tree, &group->pcb_tree, fd_cb)) {
      ...
      FDLOG(LOG_ERR, fd_cb, "Group %u already contains a PCB with hash %u",
            group->ctl_unit, fd_cb->hash);
      error = EEXIST;
    }
  } else {
    FDLOG(LOG_ERR, fd_cb, "Group %u is defunct, cannot insert",
          group->ctl_unit);
    error = ENOENT;
  }
  lck_rw_done(&group->lck);
...
static errno_t flow_divert_add_to_group(struct flow_divert_pcb *fd_cb,
                                        uint32_t ctl_unit) {
  errno_t error = 0;
  struct flow_divert_group *group = NULL;
  ...
  do {
    ...
    for (idx = 1; idx < GROUP_COUNT_MAX && error == 0; idx++) {
      ...
      struct flow_divert_group *curr_group =
          flow_divert_group_lookup(idx, NULL);
      if (curr_group != NULL) {
...
static struct flow_divert_pcb *flow_divert_pcb_create(socket_t so) {
...
static void flow_divert_pcb_destroy(struct flow_divert_pcb *fd_cb) {
  FDLOG(LOG_INFO, fd_cb,
        "Destroying, app tx %llu, tunnel tx %llu, tunnel rx %llu",
...
static void flow_divert_pcb_remove(struct flow_divert_pcb *fd_cb) {
  if (fd_cb->group != NULL) {
    struct flow_divert_group *group = fd_cb->group;
    lck_rw_lock_exclusive(&group->lck);
    FDLOG(LOG_INFO, fd_cb, "Removing from group %d, ref count = %d",
          group->ctl_unit, fd_cb->ref_count);
...
static int flow_divert_packet_init(struct flow_divert_pcb *fd_cb,
                                   uint8_t packet_type, mbuf_ref_t *packet) {
...
static int
flow_divert_packet_append_tlv(mbuf_ref_t packet, uint8_t type, uint32_t length,
                              const void __sized_by(length) * value) {
  uint32_t net_length = htonl(length);
  int error = 0;

  error = mbuf_copyback(packet, mbuf_pkthdr_len(packet), sizeof(type), &type,
                        MBUF_DONTWAIT);
  if (error) {
  ...
  error = mbuf_copyback(packet, mbuf_pkthdr_len(packet), sizeof(net_length),
                        &net_length, MBUF_DONTWAIT);
  if (error) {
    FDLOG(LOG_ERR, &nil_pcb, "failed to append the length (%u)", length);
    return error;
  }

  error = mbuf_copyback(packet, mbuf_pkthdr_len(packet), length, value,
                        MBUF_DONTWAIT);
  if (error) {
...
static int flow_divert_packet_find_tlv(mbuf_ref_t packet, int offset,
                                       uint8_t type, int *err, int next) {
...
static int flow_divert_packet_get_tlv(mbuf_ref_t packet, int offset,
                                      uint8_t type, size_t buff_len,
                                      void *buff __sized_by(buff_len),
                                      uint32_t *val_size) {
  int error = 0;
  uint32_t length = 0;
  int tlv_offset = 0;
  ...
  error =
      mbuf_copydata(packet, tlv_offset + sizeof(type), sizeof(length), &length);
  if (error) {
    return error;
  }
  ...
  if (length > (mbuf_pkthdr_len(packet) - data_offset)) {
    FDLOG(LOG_ERR, &nil_pcb,
          "Length of %u TLV (%u) is larger than remaining packet data (%lu)",
          type, length, (mbuf_pkthdr_len(packet) - data_offset));
    return EINVAL;
  }
...
static int flow_divert_packet_compute_hmac(mbuf_ref_t packet,
                                           struct flow_divert_group *group,
                                           uint8_t *hmac) {
  ...
  g_crypto_funcs->cchmac_init_fn(g_crypto_funcs->ccsha1_di, hmac_ctx,
                                 group->token_key_size, group->token_key);

  while (curr_mbuf != NULL) {
    g_crypto_funcs->cchmac_update_fn(g_crypto_funcs->ccsha1_di, hmac_ctx,
                                     mbuf_len(curr_mbuf),
                                     mtod(curr_mbuf, void *));
...
static int flow_divert_packet_verify_hmac(mbuf_ref_t packet,
                                          uint32_t ctl_unit) {
  int error = 0;
  struct flow_divert_group *group = NULL;
  int hmac_offset;
  ...
  if (group == NULL) {
    FDLOG(LOG_ERR, &nil_pcb, "Failed to lookup group for control unit %u",
          ctl_unit);
    return ENOPROTOOPT;
  }
  ...
  hmac_offset =
      flow_divert_packet_find_tlv(packet, 0, FLOW_DIVERT_TLV_HMAC, &error, 0);
  if (hmac_offset < 0) {
    goto done;
  }

  error = flow_divert_packet_get_tlv(packet, hmac_offset, FLOW_DIVERT_TLV_HMAC,
                                     sizeof(packet_hmac), packet_hmac, NULL);
  if (error) {
    goto done;
  }
...
static void flow_divert_add_data_statistics(struct flow_divert_pcb *fd_cb,
                                            size_t data_len, Boolean send) {
...
static errno_t flow_divert_check_no_cellular(struct flow_divert_pcb *fd_cb) {
...
static errno_t flow_divert_check_no_expensive(struct flow_divert_pcb *fd_cb) {
  struct inpcb *inp = sotoinpcb(fd_cb->so);
...
static errno_t flow_divert_check_no_constrained(struct flow_divert_pcb *fd_cb) {
  struct inpcb *inp = sotoinpcb(fd_cb->so);
...
static void flow_divert_update_closed_state(struct flow_divert_pcb *fd_cb,
                                            int how, bool tunnel,
                                            bool flush_snd) {
  if (how != SHUT_RD) {
    ...
    if (tunnel || !(fd_cb->flags & FLOW_DIVERT_CONNECT_STARTED)) {
      ...
      if (flush_snd) {
        /* If the tunnel is not accepting writes any more, then flush the send
         * buffer */
...
static uint16_t trie_node_alloc(struct flow_divert_trie *trie) {
...
static uint16_t trie_child_map_alloc(struct flow_divert_trie *trie) {
...
static uint16_t trie_bytes_move(struct flow_divert_trie *trie,
                                uint16_t bytes_idx, size_t bytes_size) {
...
static uint16_t flow_divert_trie_insert(struct flow_divert_trie *trie,
  ...
  while (child != NULL_TRIE_IDX) {
    ...
    current_end =
        TRIE_NODE(trie, current).start + TRIE_NODE(trie, current).length;

    for (node_idx = TRIE_NODE(trie, current).start;
         node_idx < current_end && string_idx < string_end &&
    ...
    if (node_idx <
      ...
      if (prefix == NULL_TRIE_IDX) {
        FDLOG0(LOG_ERR, &nil_pcb,
               "Ran out of trie nodes while splitting an existing node");
        return NULL_TRIE_IDX;
      }

      /*
       * Prefix points to the portion of the current nodes's string that has
       * matched the input string thus far.
       */
      TRIE_NODE(trie, prefix).start = TRIE_NODE(trie, current).start;
      TRIE_NODE(trie, prefix).length =
          (node_idx - TRIE_NODE(trie, current).start);

      /*
       * Prefix has the current node as the child corresponding to the first
       * byte after the split.
       */
      ...
      /* Parent has the prefix as the child correspoding to the first byte in
       * the prefix */
      TRIE_CHILD(trie, parent, TRIE_BYTE(trie, TRIE_NODE(trie, prefix).start)) =
          prefix;
  ...
  if (string_remainder > 0) {
    ...
    TRIE_NODE(trie, leaf).start =
        trie_bytes_move(trie, string_idx, string_remainder);
    ...
    TRIE_CHILD(trie, current, TRIE_BYTE(trie, TRIE_NODE(trie, leaf).start)) =
        leaf;
    current = leaf;
...
static uint16_t flow_divert_trie_search(struct flow_divert_trie *trie,
                                        const uint8_t *string_bytes
                                            __sized_by(string_bytes_count),
                                        __unused size_t string_bytes_count) {
  uint16_t current = trie->root;
  ...
  while (current != NULL_TRIE_IDX) {
    ...
    for (node_idx = TRIE_NODE(trie, current).start;
         node_idx < node_end && string_bytes[string_idx] != '\0' &&
         string_bytes[string_idx] == TRIE_BYTE(trie, node_idx);
         node_idx++, string_idx++) {
      ;
    }
    ...
    if (node_idx == node_end) {
      if (string_bytes[string_idx] == '\0') {
        return current; /* Got an exact match */
                 ...
                 0 == strlcmp((const char *)string_bytes,
                              APPLE_WEBCLIP_ID_PREFIX, string_idx)) {
...
static int flow_divert_find_proc_by_uuid_callout(proc_t p, void *arg) {
  ...
  if (info->found_signing_id != NULL) {
    if (!info->found_multiple_signing_ids) {
      /* All processes that were found had the same signing identifier, so just
       * claim this first one and be done. */
      info->found_proc = p;
      result = PROC_CLAIMED_DONE;
    } else {
      uuid_string_t uuid_str;
      uuid_unparse(info->target_uuid, uuid_str);
      FDLOG(LOG_WARNING, &nil_pcb,
            "Found multiple processes with UUID %s with different signing "
            "identifiers",
            uuid_str);
    }
  ...
  if (result == PROC_RETURNED_DONE) {
    uuid_string_t uuid_str;
    uuid_unparse(info->target_uuid, uuid_str);
    FDLOG(LOG_WARNING, &nil_pcb, "Failed to find a process with UUID %s",
          uuid_str);
  }
...
static int flow_divert_find_proc_by_uuid_filter(proc_t p, void *arg) {
  struct uuid_search_info *info = (struct uuid_search_info *)arg;
  int include = 0;
  ...
  if (include) {
    ...
    if (signing_id != NULL) {
      FDLOG(LOG_INFO, &nil_pcb, "Found process %d with signing identifier %s",
            proc_getpid(p), signing_id);
      ...
      } else if (strlcmp(info->found_signing_id, signing_id,
                         info->found_signing_id_size)) {
...
static proc_t flow_divert_find_proc_by_uuid(uuid_t uuid) {
  ...
  proc_iterate(PROC_ALLPROCLIST, flow_divert_find_proc_by_uuid_callout, &info,
               flow_divert_find_proc_by_uuid_filter, &info);

  return info.found_proc;
}

static int flow_divert_add_proc_info(struct flow_divert_pcb *fd_cb, proc_t proc,
                                     const char *signing_id __null_terminated,
                                     mbuf_ref_t connect_packet,
                                     bool is_effective) {
  int error = 0;
  uint8_t *cdhash = NULL;
  ...
  if (is_effective) {
    ...
    if (!(fd_cb->group->flags & FLOW_DIVERT_GROUP_FLAG_NO_APP_MAP)) {
      if (proc_cs_id != NULL) {
        ...
        uint16_t result = flow_divert_trie_search(
            &fd_cb->group->signing_id_trie,
            (const uint8_t *)__unsafe_null_terminated_to_indexable(proc_cs_id),
            proc_cs_id_size);
  ...
  /*
   * If signing_id is not NULL then it came from the flow divert token and will
   * be added as part of the token, so there is no need to add it here.
   */
  if (signing_id == NULL && proc_cs_id != NULL) {
    error = flow_divert_packet_append_tlv(
        connect_packet,
        (is_effective ? FLOW_DIVERT_TLV_SIGNING_ID
                      : FLOW_DIVERT_TLV_APP_REAL_SIGNING_ID),
        (uint32_t)strlen(proc_cs_id), __terminated_by_to_indexable(proc_cs_id));
    if (error != 0) {
  ...
  if (cdhash != NULL) {
    error = flow_divert_packet_append_tlv(
        connect_packet,
        (is_effective ? FLOW_DIVERT_TLV_CDHASH
                      : FLOW_DIVERT_TLV_APP_REAL_CDHASH),
        SHA1_RESULTLEN, cdhash);
    if (error) {
  ...
  if (task != TASK_NULL) {
    ...
    kern_return_t rc =
        task_info(task, TASK_AUDIT_TOKEN, (task_info_t)&audit_token, &count);
    if (rc == KERN_SUCCESS) {
      int append_error = flow_divert_packet_append_tlv(
          connect_packet,
          (is_effective ? FLOW_DIVERT_TLV_APP_AUDIT_TOKEN
                        : FLOW_DIVERT_TLV_APP_REAL_AUDIT_TOKEN),
          sizeof(audit_token_t), &audit_token);
      if (append_error) {
        FDLOG(LOG_ERR, fd_cb, "failed to append app audit token: %d",
              append_error);
      }
    }
  }
...
static int flow_divert_add_all_proc_info(
    struct flow_divert_pcb *fd_cb, struct socket *so, proc_t proc,
    const char *signing_id __null_terminated, mbuf_ref_t connect_packet) {
  int error = 0;
  ...
  if (real_proc == PROC_NULL) {
    FDLOG(LOG_ERR, fd_cb, "failed to find the real proc record for %d",
          so->last_pid);
  ...
  error = flow_divert_add_proc_info(fd_cb, src_proc, signing_id, connect_packet,
                                    true);
  if (error != 0) {
    goto done;
  }
  ...
  if (real_src_proc != NULL && real_src_proc != src_proc) {
    error = flow_divert_add_proc_info(fd_cb, real_src_proc, NULL,
                                      connect_packet, false);
    if (error != 0) {
      goto done;
    }
  }
...
static int flow_divert_send_packet(struct flow_divert_pcb *fd_cb,
                                   mbuf_ref_t packet) {
  int error;
  ...
  if (MBUFQ_EMPTY(&fd_cb->group->send_queue)) {
    error = ctl_enqueuembuf(g_flow_divert_kctl_ref, fd_cb->group->ctl_unit,
                            packet, CTL_DATA_EOR);
    if (error) {
      FDLOG(LOG_NOTICE, &nil_pcb,
            "flow_divert_send_packet: ctl_enqueuembuf returned an error: %d",
            error);
    }
  } else {
    error = ENOBUFS;
...
static void flow_divert_append_domain_name(char *domain_name __null_terminated,
                                           void *ctx) {
  ...
  if (domain_name_length > 0 &&
      domain_name_length < FLOW_DIVERT_MAX_NAME_SIZE) {
    int error = flow_divert_packet_append_tlv(
        packet, FLOW_DIVERT_TLV_TARGET_HOSTNAME, (uint32_t)domain_name_length,
        __terminated_by_to_indexable(domain_name));
    if (error) {
...
static int flow_divert_create_connect_packet(struct flow_divert_pcb *fd_cb,
                                             struct sockaddr *to,
                                             struct socket *so, proc_t p,
                                             mbuf_ref_t *out_connect_packet) {
  int error = 0;
  int flow_type = 0;
  ...
  error =
      flow_divert_packet_init(fd_cb, FLOW_DIVERT_PKT_CONNECT, &connect_packet);
  if (error) {
    goto done;
  }

  if (fd_cb->connect_token != NULL && (fd_cb->flags & FLOW_DIVERT_HAS_HMAC)) {
    int find_error = flow_divert_packet_get_tlv(fd_cb->connect_token, 0,
                                                FLOW_DIVERT_TLV_SIGNING_ID, 0,
                                                NULL, &sid_size);
    if (find_error == 0 && sid_size > 0) {
      signing_id = kalloc_data(sid_size + 1, Z_WAITOK | Z_ZERO);
      if (signing_id != NULL) {
        flow_divert_packet_get_tlv(fd_cb->connect_token, 0,
                                   FLOW_DIVERT_TLV_SIGNING_ID, sid_size,
                                   signing_id, NULL);
  ...
  error = flow_divert_add_all_proc_info(
      fd_cb, so, p,
      NULL == signing_id ? NULL
                         : __unsafe_null_terminated_from_indexable(signing_id),
      connect_packet);
  ...
  error = flow_divert_packet_append_tlv(
      connect_packet, FLOW_DIVERT_TLV_TRAFFIC_CLASS,
      sizeof(fd_cb->so->so_traffic_class), &fd_cb->so->so_traffic_class);
  if (error) {
    goto done;
  }
  ...
  error = flow_divert_packet_append_tlv(
      connect_packet, FLOW_DIVERT_TLV_FLOW_TYPE, sizeof(flow_type), &flow_type);
  ...
  if (fd_cb->connect_token != NULL) {
    ...
    necp_with_inp_domain_name(so, connect_packet,
                              flow_divert_append_domain_name);
  }

  if (fd_cb->local_endpoint.sa.sa_family == AF_INET ||
      fd_cb->local_endpoint.sa.sa_family == AF_INET6) {
    error = flow_divert_packet_append_tlv(
        connect_packet, FLOW_DIVERT_TLV_LOCAL_ADDR,
        fd_cb->local_endpoint.sa.sa_len, SA_BYTES(&(fd_cb->local_endpoint.sa)));
    if (error) {
      goto done;
    }
  }
  ...
  if (ifp != NULL) {
    ...
    error = flow_divert_packet_append_tlv(
        connect_packet, FLOW_DIVERT_TLV_OUT_IF_INDEX, sizeof(flow_if_index),
        &flow_if_index);
    if (error) {
      goto done;
    }
  ...
  if (flags != 0) {
    error = flow_divert_packet_append_tlv(connect_packet, FLOW_DIVERT_TLV_FLAGS,
                                          sizeof(flags), &flags);
    if (error) {
      goto done;
    }
  ...
  if (cfil_id != NULL && cfil_id_size > 0 && cfil_id_size <= sizeof(uuid_t)) {
    error =
        flow_divert_packet_append_tlv(connect_packet, FLOW_DIVERT_TLV_CFIL_ID,
                                      (uint32_t)cfil_id_size, cfil_id);
    if (error) {
      goto done;
    }
  }
...
static int flow_divert_send_connect_packet(struct flow_divert_pcb *fd_cb) {
  int error = 0;
  mbuf_ref_t connect_packet = fd_cb->connect_packet;
  mbuf_ref_t saved_connect_packet = NULL;

  if (connect_packet != NULL) {
    error = mbuf_copym(connect_packet, 0, mbuf_pkthdr_len(connect_packet),
                       MBUF_DONTWAIT, &saved_connect_packet);
    if (error) {
...
static int flow_divert_send_connect_result(struct flow_divert_pcb *fd_cb) {
  int error = 0;
  mbuf_ref_t packet = NULL;
  int rbuff_space = 0;

  error =
      flow_divert_packet_init(fd_cb, FLOW_DIVERT_PKT_CONNECT_RESULT, &packet);
  if (error) {
    FDLOG(LOG_ERR, fd_cb, "failed to create a connect result packet: %d",
          error);
    goto done;
  }
  ...
  error = flow_divert_packet_append_tlv(packet, FLOW_DIVERT_TLV_SPACE_AVAILABLE,
                                        sizeof(rbuff_space), &rbuff_space);
  if (error) {
    goto done;
  }
  ...
  if (fd_cb->local_endpoint.sa.sa_family == AF_INET ||
      fd_cb->local_endpoint.sa.sa_family == AF_INET6) {
    error = flow_divert_packet_append_tlv(
        packet, FLOW_DIVERT_TLV_LOCAL_ADDR, fd_cb->local_endpoint.sa.sa_len,
        SA_BYTES(&(fd_cb->local_endpoint.sa)));
    if (error) {
      goto done;
    }
  }
...
static int flow_divert_send_close(struct flow_divert_pcb *fd_cb, int how) {
  int error = 0;
  mbuf_ref_t packet = NULL;
  uint32_t zero = 0;
  ...
  error = flow_divert_packet_append_tlv(packet, FLOW_DIVERT_TLV_ERROR_CODE,
                                        sizeof(zero), &zero);
  if (error) {
  ...
  how = htonl(how);
  error = flow_divert_packet_append_tlv(packet, FLOW_DIVERT_TLV_HOW,
                                        sizeof(how), &how);
  if (error) {
...
static int flow_divert_tunnel_how_closed(struct flow_divert_pcb *fd_cb) {
  if ((fd_cb->flags &
...
/*
 * Determine what close messages if any need to be sent to the tunnel. Returns
 * TRUE if the tunnel is closed for both reads and writes. Returns FALSE
 * otherwise.
 */
static void flow_divert_send_close_if_needed(struct flow_divert_pcb *fd_cb) {
  int how = -1;

  /* Do not send any close messages if there is still data in the send buffer */
  if (fd_cb->so->so_snd.sb_cc == 0) {
    if ((fd_cb->flags &
         (FLOW_DIVERT_READ_CLOSED | FLOW_DIVERT_TUNNEL_RD_CLOSED)) ==
        FLOW_DIVERT_READ_CLOSED) {
      /* Socket closed reads, but tunnel did not. Tell tunnel to close reads */
      how = SHUT_RD;
    }
    if ((fd_cb->flags &
         (FLOW_DIVERT_WRITE_CLOSED | FLOW_DIVERT_TUNNEL_WR_CLOSED)) ==
  ...
  if (how != -1) {
    ...
    if (flow_divert_send_close(fd_cb, how) != ENOBUFS) {
      /* Successfully sent the close packet. Record the ways in which the tunnel
       * has been closed */
      if (how != SHUT_RD) {
        fd_cb->flags |= FLOW_DIVERT_TUNNEL_WR_CLOSED;
      }
...
static errno_t flow_divert_send_data_packet(struct flow_divert_pcb *fd_cb,
                                            mbuf_ref_t data, size_t data_len) {
  mbuf_ref_t packet = NULL;
...
static errno_t
flow_divert_send_datagram_packet(struct flow_divert_pcb *fd_cb, mbuf_ref_t data,
                                 size_t data_len, struct sockaddr *toaddr,
                                 Boolean is_fragment, size_t datagram_size) {
  mbuf_ref_t packet = NULL;
  mbuf_ref_t last = NULL;
  int error = 0;
  ...
  if (is_fragment) {
    error = flow_divert_packet_append_tlv(packet, FLOW_DIVERT_TLV_IS_FRAGMENT,
                                          sizeof(is_fragment), &is_fragment);
    if (error) {
      FDLOG(LOG_ERR, fd_cb,
            "flow_divert_packet_append_tlv(FLOW_DIVERT_TLV_IS_FRAGMENT) "
            "failed: %d",
            error);
      goto done;
    }
  ...
  error = flow_divert_packet_append_tlv(packet, FLOW_DIVERT_TLV_DATAGRAM_SIZE,
                                        sizeof(datagram_size), &datagram_size);
  if (error) {
    FDLOG(LOG_ERR, fd_cb,
          "flow_divert_packet_append_tlv(FLOW_DIVERT_TLV_DATAGRAM_SIZE) "
          "failed: %d",
          error);
    goto done;
  }
...
static errno_t
flow_divert_send_fragmented_datagram(struct flow_divert_pcb *fd_cb,
                                     mbuf_ref_t datagram, size_t datagram_len,
                                     struct sockaddr *toaddr) {
  ...
  while (remaining_len > 0 && next_data != NULL) {
    ...
    error = flow_divert_send_datagram_packet(fd_cb, next_data, to_send,
                                             (first ? toaddr : NULL), TRUE,
                                             (first ? datagram_len : 0));
    if (error) {
      break;
    }
...
static void flow_divert_send_buffered_data(struct flow_divert_pcb *fd_cb,
                                           Boolean force) {
  size_t to_send;
  size_t sent = 0;
  int error = 0;
  ...
  if (buffer == NULL && to_send > 0) {
    FDLOG(LOG_ERR, fd_cb, "Send buffer is NULL, but size is supposed to be %lu",
          to_send);
    return;
  }
  ...
  if (SOCK_TYPE(fd_cb->so) == SOCK_STREAM) {
    ...
    while (buffer) {
      ...
      if (data_len <= FLOW_DIVERT_CHUNK_SIZE) {
        error = flow_divert_send_datagram_packet(fd_cb, data, data_len, toaddr,
                                                 FALSE, 0);
      } else {
        error =
            flow_divert_send_fragmented_datagram(fd_cb, data, data_len, toaddr);
        data = NULL;
      }
      if (error) {
        if (data != NULL) {
...
static int flow_divert_send_app_data(struct flow_divert_pcb *fd_cb,
                                     mbuf_ref_t data, size_t data_size,
                                     struct sockaddr *toaddr) {
  ...
  if (SOCK_TYPE(fd_cb->so) == SOCK_STREAM) {
    ...
    while (sent < to_send && remaining_data != NULL && remaining_size > 0) {
      ...
      if (pkt_data_len < remaining_size) {
        error =
            mbuf_split(pkt_data, pkt_data_len, MBUF_DONTWAIT, &remaining_data);
        if (error) {
    ...
    if (pkt_data != NULL) {
      if (sbspace(&fd_cb->so->so_snd) > 0) {
        if (!sbappendstream(&fd_cb->so->so_snd, pkt_data)) {
          FDLOG(LOG_ERR, fd_cb,
                "sbappendstream failed with pkt_data, send buffer size = %u, "
                "send_window = %u\n",
    ...
    if (remaining_data != NULL) {
      if (sbspace(&fd_cb->so->so_snd) > 0) {
        if (!sbappendstream(&fd_cb->so->so_snd, remaining_data)) {
          FDLOG(LOG_ERR, fd_cb,
                "sbappendstream failed with remaining_data, send buffer size = "
                "%u, send_window = %u\n",
                fd_cb->so->so_snd.sb_cc, fd_cb->send_window);
        }
      } else {
    ...
    if (to_send || data_size == 0) {
      if (data_size <= FLOW_DIVERT_CHUNK_SIZE) {
        send_dgram_error = flow_divert_send_datagram_packet(
            fd_cb, data, data_size, toaddr, FALSE, 0);
      } else {
        send_dgram_error = flow_divert_send_fragmented_datagram(
            fd_cb, data, data_size, toaddr);
        data = NULL;
      }
      if (send_dgram_error) {
        FDLOG(LOG_NOTICE, fd_cb,
              "flow_divert_send_datagram_packet failed with error %d, send "
              "data size = %lu",
              send_dgram_error, data_size);
      } else {
    ...
    if (data != NULL) {
      ...
      if (sbspace(&fd_cb->so->so_snd) > 0) {
        if (toaddr != NULL) {
          int append_error = 0;
          if (!sbappendaddr(&fd_cb->so->so_snd, toaddr, data, NULL,
                            &append_error)) {
            FDLOG(LOG_ERR, fd_cb,
                  "sbappendaddr failed. send buffer size = %u, send_window = "
                  "%u, error = %d",
          ...
          if (!sbappendrecord(&fd_cb->so->so_snd, data)) {
            FDLOG(LOG_ERR, fd_cb,
                  "sbappendrecord failed. send buffer size = %u, send_window = "
                  "%u",
                  fd_cb->so->so_snd.sb_cc, fd_cb->send_window);
          }
        }
        ...
        FDLOG(LOG_ERR, fd_cb,
              "flow_divert_send_datagram_packet failed with error %d, send "
              "data size = %lu, dropping the datagram",
              error, data_size);
        mbuf_freem(data);
      }
    }
...
static int flow_divert_send_read_notification(struct flow_divert_pcb *fd_cb) {
  int error = 0;
  mbuf_ref_t packet = NULL;

  error = flow_divert_packet_init(fd_cb, FLOW_DIVERT_PKT_READ_NOTIFY, &packet);
  if (error) {
    FDLOG(LOG_ERR, fd_cb, "failed to create a read notification packet: %d",
          error);
    goto done;
  }
...
static int flow_divert_send_traffic_class_update(struct flow_divert_pcb *fd_cb,
                                                 int traffic_class) {
  int error = 0;
  mbuf_ref_t packet = NULL;

  error = flow_divert_packet_init(fd_cb, FLOW_DIVERT_PKT_PROPERTIES_UPDATE,
                                  &packet);
  if (error) {
    FDLOG(LOG_ERR, fd_cb, "failed to create a properties update packet: %d",
          error);
    goto done;
  }

  error = flow_divert_packet_append_tlv(packet, FLOW_DIVERT_TLV_TRAFFIC_CLASS,
                                        sizeof(traffic_class), &traffic_class);
  if (error) {
...
static void flow_divert_set_local_endpoint(struct flow_divert_pcb *fd_cb,
                                           struct sockaddr *local_endpoint) {
  struct inpcb *inp = sotoinpcb(fd_cb->so);
...
static void flow_divert_set_remote_endpoint(struct flow_divert_pcb *fd_cb,
...
static uint32_t flow_divert_derive_kernel_control_unit(pid_t pid,
                                                       uint32_t *ctl_unit,
  ...
  *is_aggregate = false;
  if (*ctl_unit == FLOW_DIVERT_IN_PROCESS_UNIT) {
    ...
    if (pid != 0) {
      ...
      // If an in-process group isn't open yet, send a signal up through NECP to
      // request one
      if (!found_unit) {
...
static int flow_divert_try_next_group(struct flow_divert_pcb *fd_cb) {
  int error = 0;
  ...
  do {
    ...
    uint32_t next_ctl_unit = flow_divert_derive_kernel_control_unit(
        0, &policy_control_unit, &(fd_cb->aggregate_unit), &is_aggregate);

    if (fd_cb->control_group_unit == next_ctl_unit) {
      FDLOG0(LOG_NOTICE, fd_cb,
             "Next control unit is the same as the current control unit, "
             "disabling flow divert");
      error = EALREADY;
      break;
    }
    ...
    if (next_group == NULL) {
      FDLOG(LOG_NOTICE, fd_cb, "Group for control unit %u does not exist",
            next_ctl_unit);
      continue;
    }

    FDLOG(LOG_NOTICE, fd_cb, "Moving from %u to %u", fd_cb->control_group_unit,
          next_ctl_unit);
  ...
  if (error) {
    FDLOG(LOG_NOTICE, fd_cb,
          "Failed to send the connect packet to %u, disabling flow divert",
          fd_cb->control_group_unit);
    flow_divert_pcb_remove(fd_cb);
    error = ENOENT;
  }
...
static int flow_divert_disable(struct flow_divert_pcb *fd_cb) {
  ...
  if (do_connect) {
    ...
    error = (*so->so_proto->pr_usrreqs->pru_connect)(
        so, remote_endpoint, (last_proc != NULL ? last_proc : current_proc()));
    if (error) {
      FDLOG(LOG_ERR, fd_cb,
            "Failed to connect using the socket's original protocol: %d",
            error);
      goto done;
    }
  }
  ...
  if (SOCK_TYPE(so) == SOCK_STREAM) {
    ...
    error = (*so->so_proto->pr_usrreqs->pru_send)(
        so, 0, data_to_send, NULL, NULL,
        (last_proc != NULL ? last_proc : current_proc()));

    if (error && error != EWOULDBLOCK) {
      FDLOG(LOG_ERR, fd_cb,
            "Failed to send queued TCP data using the socket's original "
            "protocol: %d",
            error);
    } else {
      error = 0;
    ...
    while (!MBUFQ_EMPTY(&send_queue)) {
      ...
      if (error) {
        FDLOG(LOG_ERR, fd_cb,
              "Failed to send queued UDP data using the socket's original "
              "protocol: %d",
              error);
      }
    }
...
static void flow_divert_scope(struct flow_divert_pcb *fd_cb, int out_if_index,
  ...
  if (current_ifp != NULL) {
    ...
    if (error != 0) {
      FDLOG(LOG_ERR, fd_cb,
            "failed to scope to %d because inp_bindif returned %d",
            out_if_index, error);
      return;
    }
    ...
    if (derive_new_address && fd_cb->original_remote_endpoint != NULL) {
      ...
      if (inp->inp_vflag & INP_IPV6) {
        ...
        error = in6_pcbladdr(inp, fd_cb->original_remote_endpoint,
                             &(fd_cb->local_endpoint.sin6.sin6_addr), NULL);
      } else {
        inp->inp_laddr.s_addr = INADDR_ANY;
        error = in_pcbladdr(inp, fd_cb->original_remote_endpoint,
                            &(fd_cb->local_endpoint.sin.sin_addr), IFSCOPE_NONE,
                            NULL, 0);
      }

      if (error != 0) {
        FDLOG(LOG_WARNING, fd_cb,
              "failed to derive a new local address from %d because "
              "in_pcbladdr returned %d",
              out_if_index, error);
      }
    }
  } else {
...
static void flow_divert_handle_connect_result(struct flow_divert_pcb *fd_cb,
                                              mbuf_ref_t packet, int offset) {
  ...
  error =
      flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_ERROR_CODE,
                                 sizeof(connect_error), &connect_error, NULL);
  if (error) {
  ...
  error = flow_divert_packet_get_tlv(packet, offset,
                                     FLOW_DIVERT_TLV_SPACE_AVAILABLE,
                                     sizeof(send_window), &send_window, NULL);
  if (error) {
    FDLOG(LOG_ERR, fd_cb, "failed to get the send window: %d", error);
    return;
  }

  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_CTL_UNIT,
                                     sizeof(ctl_unit), &ctl_unit, NULL);
  if (error) {
    FDLOG0(LOG_INFO, fd_cb, "No control unit provided in the connect result");
  }

  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_LOCAL_ADDR,
                                     sizeof(local_endpoint),
                                     &(local_endpoint.sin6), NULL);
  if (error) {
    FDLOG0(LOG_INFO, fd_cb, "No local address provided");
  }

  error = flow_divert_packet_get_tlv(
      packet, offset, FLOW_DIVERT_TLV_REMOTE_ADDR, sizeof(remote_endpoint),
      &(remote_endpoint.sin6), NULL);
  if (error) {
    FDLOG0(LOG_INFO, fd_cb, "No remote address provided");
  }

  error =
      flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_OUT_IF_INDEX,
                                 sizeof(out_if_index), &out_if_index, NULL);
  if (error) {
    FDLOG0(LOG_INFO, fd_cb, "No output if index provided");
  }

  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_APP_DATA,
                                     0, NULL, &app_data_length);
  if (error) {
  ...
  if (flow_divert_is_sockaddr_valid(SA(&local_endpoint))) {
    if (local_endpoint.sa.sa_family == AF_INET) {
      ...
      if ((inp->inp_vflag & INP_IPV4) &&
          local_endpoint.sin.sin_addr.s_addr != INADDR_ANY) {
      ...
      if ((inp->inp_vflag & INP_IPV6) &&
          !IN6_IS_ADDR_UNSPECIFIED(&local_endpoint.sin6.sin6_addr)) {
        local_address_is_valid = true;
        fd_cb->local_endpoint = local_endpoint;
        inp->in6p_laddr = sa6_any.sin6_addr;
      } else {
  ...
  if (flow_divert_is_sockaddr_valid(SA(&remote_endpoint)) &&
  ...
  if (app_data_length > 0) {
    ...
    if (app_data != NULL) {
      error =
          flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_APP_DATA,
                                     app_data_length, app_data, NULL);
      if (error == 0) {
        FDLOG(LOG_INFO, fd_cb,
              "Got %u bytes of app data from the connect result",
              app_data_length);
        if (fd_cb->app_data != NULL) {
          kfree_data_sized_by(fd_cb->app_data, fd_cb->app_data_length);
        }
        ...
        FDLOG(LOG_ERR, fd_cb,
              "Failed to copy %u bytes of application data from the connect "
              "result packet",
              app_data_length);
      ...
      FDLOG(LOG_ERR, fd_cb,
            "Failed to allocate a buffer of size %u to hold the application "
            "data from the connect result",
            app_data_length);
    }
  }
...
set_socket_state:
  ...
  if (connect_error || error) {
    if (connect_error &&
        fd_cb->control_group_unit != fd_cb->policy_control_unit) {
      /* The plugin rejected the flow and the control unit is an aggregation of
       * multiple plugins, try to move to the next one */
    ...
    flow_divert_disconnect_socket(
        so, !(fd_cb->flags & FLOW_DIVERT_IMPLICIT_CONNECT), false);
  } else {
#if NECP
...
static void flow_divert_handle_close(struct flow_divert_pcb *fd_cb,
                                     mbuf_ref_t packet, int offset) {
  uint32_t close_error = 0;
  int error = 0;
  int how = 0;
  struct socket *so = fd_cb->so;
  bool is_connected = (SOCK_TYPE(so) == SOCK_STREAM ||
                       !(fd_cb->flags & FLOW_DIVERT_IMPLICIT_CONNECT));

  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_ERROR_CODE,
                                     sizeof(close_error), &close_error, NULL);
  if (error) {
    FDLOG(LOG_ERR, fd_cb, "failed to get the close error: %d", error);
    return;
  }

  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_HOW,
                                     sizeof(how), &how, NULL);
  if (error) {
  ...
  /* Only do this for connected flows because "shutdown by peer" doesn't make
   * sense for unconnected datagram flows */
...
flow_divert_create_control_mbuf(struct flow_divert_pcb *fd_cb) {
  ...
  if ((inp->inp_vflag & INP_IPV4) &&
    ...
    return sbcreatecontrol((caddr_t) & (fd_cb->local_endpoint.sin.sin_addr),
    ...
    return sbcreatecontrol((caddr_t)&pi6, sizeof(pi6), IPV6_PKTINFO,
                           IPPROTO_IPV6);
  }
  return NULL;
}

static int flow_divert_handle_data(struct flow_divert_pcb *fd_cb,
                                   mbuf_ref_t packet, size_t offset) {
  int error = 0;
  struct socket *so = fd_cb->so;
  mbuf_ref_t data = NULL;
  size_t data_size;
  ...
  if (sbspace(&so->so_rcv) == 0) {
    ...
    FDLOG0(LOG_INFO, fd_cb,
           "Receive buffer is full, will send read notification when app reads "
           "some data");
    return error;
  }
  ...
  if (SOCK_TYPE(so) == SOCK_DGRAM) {
    ...
    error = flow_divert_packet_get_tlv(
        packet, (int)offset, FLOW_DIVERT_TLV_REMOTE_ADDR,
        sizeof(remote_address), &remote_address, &val_size);
  ...
  if (so->so_state & SS_CANTRCVMORE) {
    FDLOG(LOG_NOTICE, fd_cb,
          "app cannot receive any more data, dropping %lu bytes of data",
          data_size);
    return error;
  }
  ...
  if (SOCK_TYPE(so) == SOCK_STREAM) {
    ...
    if (got_remote_sa == TRUE) {
      error =
          flow_divert_dup_addr(remote_address.ss_family,
    ...
    if (appended || append_error == 0) {
      ...
      FDLOG(LOG_ERR, fd_cb, "failed to append %lu bytes of data: %d", data_size,
            append_error);
    }
...
static void flow_divert_handle_read_notification(struct flow_divert_pcb *fd_cb,
                                                 mbuf_ref_t packet,
                                                 int offset) {
  uint32_t read_count = 0;
  int error = 0;

  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_READ_COUNT,
                                     sizeof(read_count), &read_count, NULL);
  if (error) {
    FDLOG(LOG_ERR, fd_cb, "failed to get the read count: %d", error);
    return;
  }

  FDLOG(LOG_DEBUG, fd_cb, "received a read notification for %u bytes",
        ntohl(read_count));
...
static void flow_divert_handle_group_init(struct flow_divert_group *group,
                                          mbuf_ref_t packet, int offset) {
  int error = 0;
  uint32_t key_size = 0;
  int log_level = 0;
  ...
  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_TOKEN_KEY,
                                     0, NULL, &key_size);
  if (error) {
  ...
  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_LOG_LEVEL,
  ...
  if (group->flags & FLOW_DIVERT_GROUP_FLAG_DEFUNCT) {
    FDLOG(LOG_ERR, &nil_pcb, "Skipping (re)initialization of defunct group %u",
          group->ctl_unit);
    lck_rw_done(&group->lck);
    return;
  }
  ...
  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_FLAGS,
                                     sizeof(flags), &flags, NULL);
  if (!error) {
    group->flags = flags;
  }

  error = flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_ORDER,
                                     sizeof(order), &order, NULL);
  if (!error) {
...
static void flow_divert_handle_properties_update(struct flow_divert_pcb *fd_cb,
                                                 mbuf_ref_t packet,
                                                 int offset) {
  int error = 0;
  int out_if_index = 0;
  ...
  if (app_data_length > 0) {
    ...
    if (app_data != NULL) {
      ...
      if (error == 0) {
        ...
        FDLOG(LOG_ERR, fd_cb,
              "Failed to copy %u bytes of application data from the properties "
              "update packet",
              app_data_length);
        kfree_data(app_data, app_data_length);
      }
      ...
      FDLOG(LOG_ERR, fd_cb,
            "Failed to allocate a buffer of size %u to hold the application "
            "data from the properties update",
            app_data_length);
    }
  }
}

static void flow_divert_handle_app_map_create(struct flow_divert_group *group,
                                              mbuf_ref_t packet, int offset) {
  ...
  if (group->signing_id_trie.memory != NULL) {
    kfree_data_sized_by(group->signing_id_trie.memory,
                        group->signing_id_trie.memory_size);
  }
  ...
  error =
      flow_divert_packet_get_tlv(packet, offset, FLOW_DIVERT_TLV_PREFIX_COUNT,
                                 sizeof(prefix_count), &prefix_count, NULL);

  if (prefix_count < 0 || error) {
    FDLOG(LOG_ERR, &nil_pcb,
          "Invalid prefix count (%d) or an error occurred while reading the "
          "prefix count: %d",
          prefix_count, error);
    lck_rw_done(&group->lck);
    return;
  ...
  /* Compute the number of signing IDs and the total amount of bytes needed to
   * store them */
  for (cursor = flow_divert_packet_find_tlv(
           packet, offset, FLOW_DIVERT_TLV_SIGNING_ID, &error, 0);
       cursor >= 0;
       cursor = flow_divert_packet_find_tlv(
           packet, cursor, FLOW_DIVERT_TLV_SIGNING_ID, &error, 1)) {
    uint32_t sid_size = 0;
    error = flow_divert_packet_get_tlv(
        packet, cursor, FLOW_DIVERT_TLV_SIGNING_ID, 0, NULL, &sid_size);
    if (error || sid_size == 0) {
      FDLOG(
          LOG_ERR, &nil_pcb,
          "Failed to get the length of the signing identifier at offset %d: %d",
          cursor, error);
      signing_id_count = 0;
      break;
    }
  ...
  if (os_add3_overflow(prefix_count, signing_id_count, 1,
                       &nodes_count)) { /* + 1 for the root node */
    lck_rw_done(&group->lck);
    FDLOG0(LOG_ERR, &nil_pcb, "Overflow while computing the number of nodes");
    return;
  }

  if (os_add_overflow(prefix_count, 1,
                      &maps_count)) { /* + 1 for the root node */
    lck_rw_done(&group->lck);
    FDLOG0(LOG_ERR, &nil_pcb, "Overflow while computing the number of maps");
    return;
  }

  if (bytes_count > UINT16_MAX || nodes_count > UINT16_MAX ||
      maps_count > UINT16_MAX) {
    lck_rw_done(&group->lck);
    FDLOG(LOG_NOTICE, &nil_pcb,
          "Invalid bytes count (%lu), nodes count (%lu) or maps count (%lu)",
          bytes_count, nodes_count, maps_count);
    return;
  }

  FDLOG(LOG_INFO, &nil_pcb,
        "Nodes count = %lu, child maps count = %lu, bytes_count = %lu",
        nodes_count, maps_count, bytes_count);

  if (os_mul_overflow(sizeof(*new_trie.nodes), (size_t)nodes_count,
                      &nodes_mem_size) ||
      os_mul3_overflow(sizeof(*new_trie.child_maps), CHILD_MAP_SIZE,
                       (size_t)maps_count, &child_maps_mem_size) ||
      os_mul_overflow(sizeof(*new_trie.bytes), (size_t)bytes_count,
                      &bytes_mem_size) ||
      os_add3_overflow(nodes_mem_size, child_maps_mem_size, bytes_mem_size,
                       &trie_memory_size)) {
  ...
  new_trie.child_maps =
      (uint16_t *)(void *)((uint8_t *)new_trie.memory + nodes_mem_size);
  ...
  new_trie.bytes = (uint8_t *)(void *)((uint8_t *)new_trie.memory +
                                       nodes_mem_size + child_maps_mem_size);
  ...
  for (cursor = flow_divert_packet_find_tlv(
    ...
    if (error || sid_size == 0) {
      FDLOG(LOG_ERR, &nil_pcb,
            "Failed to get the length of the signing identifier at offset %d "
            "while building: %d",
            cursor, error);
      insert_error = EINVAL;
      break;
    }
    if (sid_size <= UINT16_MAX &&
        new_trie.bytes_free_next + (uint16_t)sid_size <= new_trie.bytes_count) {
      uint16_t new_node_idx;
      error = flow_divert_packet_get_tlv(
          packet, cursor, FLOW_DIVERT_TLV_SIGNING_ID, sid_size,
          &TRIE_BYTE(&new_trie, new_trie.bytes_free_next), NULL);
      if (error) {
        FDLOG(LOG_ERR, &nil_pcb,
      ...
      new_node_idx = flow_divert_trie_insert(
          &new_trie, new_trie.bytes_free_next, sid_size);
...
flow_divert_handle_flow_states_request(struct flow_divert_group *group) {
  ...
  error =
      flow_divert_packet_init(&nil_pcb, FLOW_DIVERT_PKT_FLOW_STATES, &packet);
  if (error || packet == NULL) {
    FDLOG(LOG_ERR, &nil_pcb,
          "flow_divert_packet_init failed: %d, cannot send flow states", error);
    return;
  }
  ...
  if (!MBUFQ_EMPTY(&group->send_queue)) {
    FDLOG0(LOG_WARNING, &nil_pcb,
           "flow_divert_handle_flow_states_request: group send queue is not "
           "empty");
  }
  ...
  SLIST_FOREACH(fd_cb, &tmp_list, tmp_list_entry) {
    ...
    if (fd_cb->so != NULL) {
      ...
      error = flow_divert_packet_append_tlv(packet, FLOW_DIVERT_TLV_FLOW_STATE,
                                            sizeof(state), &state);
      if (error) {
  ...
  error =
      ctl_enqueuembuf(g_flow_divert_kctl_ref, ctl_unit, packet, CTL_DATA_EOR);
  if (error) {
    FDLOG(LOG_NOTICE, &nil_pcb,
          "flow_divert_handle_flow_states_request: ctl_enqueuembuf returned an "
          "error: %d",
          error);
    mbuf_freem(packet);
  }
...
static int flow_divert_input(mbuf_ref_t packet,
                             struct flow_divert_group *group) {
  struct flow_divert_packet_header hdr;
  int error = 0;
  struct flow_divert_pcb *fd_cb;
  ...
  if (mbuf_pkthdr_len(packet) < sizeof(hdr)) {
    FDLOG(LOG_ERR, &nil_pcb,
          "got a bad packet, length (%lu) < sizeof hdr (%lu)",
          mbuf_pkthdr_len(packet), sizeof(hdr));
    error = EINVAL;
    goto done;
  }
  ...
  if (hdr.conn_id == 0) {
    ...
    case FLOW_DIVERT_PKT_FLOW_STATES_REQUEST:
      ...
      break;
    default:
      FDLOG(LOG_WARNING, &nil_pcb, "got an unknown message type: %d",
            hdr.packet_type);
      break;
    }
    goto done;
  ...
  if (fd_cb == NULL) {
    if (hdr.packet_type != FLOW_DIVERT_PKT_CLOSE &&
        hdr.packet_type != FLOW_DIVERT_PKT_READ_NOTIFY) {
      FDLOG(LOG_NOTICE, &nil_pcb,
            "got a %s message from group %d for an unknown pcb: %u",
            flow_divert_packet_type2str(hdr.packet_type), group->ctl_unit,
            hdr.conn_id);
    }
    goto done;
  }
  ...
  if (fd_cb->so != NULL) {
    ...
    case FLOW_DIVERT_PKT_PROPERTIES_UPDATE:
      ...
      break;
    default:
      FDLOG(LOG_WARNING, fd_cb, "got an unknown message type: %d",
            hdr.packet_type);
      break;
    }
...
static void flow_divert_close_all(struct flow_divert_group *group) {
  struct flow_divert_pcb *fd_cb;
  SLIST_HEAD(, flow_divert_pcb) tmp_list;
...
static int flow_divert_disconnectx(struct socket *so, sae_associd_t aid,
...
static int flow_divert_shutdown(struct socket *so) {
  struct flow_divert_pcb *fd_cb = so->so_fd_pcb;
...
static int flow_divert_rcvd(struct socket *so, int flags __unused) {
  struct flow_divert_pcb *fd_cb = so->so_fd_pcb;
  int space = 0;
  ...
  if ((fd_cb->flags & FLOW_DIVERT_NOTIFY_ON_RECEIVED) && (space > 0) &&
...
static int flow_divert_append_target_endpoint_tlv(mbuf_ref_t connect_packet,
  ...
  if (!flow_divert_is_sockaddr_valid(toaddr)) {
    FDLOG(LOG_ERR, &nil_pcb, "Invalid target address, family = %u, length = %u",
          toaddr->sa_family, toaddr->sa_len);
    error = EINVAL;
    goto done;
  }
  ...
  error = flow_divert_packet_append_tlv(
      connect_packet, FLOW_DIVERT_TLV_TARGET_PORT, sizeof(port), &port);
  if (error) {
    goto done;
  }
...
static errno_t flow_divert_ctloutput(struct socket *so, struct sockopt *sopt) {
  struct flow_divert_pcb *fd_cb = so->so_fd_pcb;
...
static errno_t flow_divert_connect_out_internal(struct socket *so,
                                                struct sockaddr *to, proc_t p,
                                                bool implicit) {
  struct flow_divert_pcb *fd_cb = so->so_fd_pcb;
  int error = 0;
  struct inpcb *inp = sotoinpcb(so);
  ...
  if (fd_cb->connect_packet == NULL) {
    ...
    if (!flow_divert_is_sockaddr_valid(to)) {
      FDLOG0(LOG_ERR, fd_cb,
             "Destination address is not valid when creating connect packet");
      error = EINVAL;
      goto done;
    }
    ...
    if (to->sa_family == AF_INET6) {
      ...
      error =
          in6_pcbladdr(inp, to, &(fd_cb->local_endpoint.sin6.sin6_addr), &ifp);
      if (error) {
        FDLOG(LOG_WARNING, fd_cb, "failed to get a local IPv6 address: %d",
              error);
        if (!(fd_cb->flags & FLOW_DIVERT_FLOW_IS_TRANSPARENT) ||
            IN6_IS_ADDR_UNSPECIFIED(&(satosin6(to)->sin6_addr))) {
          error = 0;
        } else {
          goto done;
        }
      ...
      if (IN6_IS_SCOPE_EMBED(&(fd_cb->local_endpoint.sin6.sin6_addr)) &&
        ...
        fd_cb->local_endpoint.sin6.sin6_scope_id =
            ntohs(fd_cb->local_endpoint.sin6.sin6_addr.s6_addr16[1]);
        fd_cb->local_endpoint.sin6.sin6_addr.s6_addr16[1] = 0;
      }

      if (IN6_IS_SCOPE_EMBED(&(to6->sin6_addr)) && in6_embedded_scope &&
      ...
      error = in_pcbladdr(inp, to, &(fd_cb->local_endpoint.sin.sin_addr),
                          IFSCOPE_NONE, &ifp, 0);
      if (error) {
        FDLOG(LOG_WARNING, fd_cb, "failed to get a local IPv4 address: %d",
              error);
        if (!(fd_cb->flags & FLOW_DIVERT_FLOW_IS_TRANSPARENT) ||
            satosin(to)->sin_addr.s_addr == INADDR_ANY) {
          error = 0;
        } else {
          goto done;
        }
      ...
      FDLOG(LOG_WARNING, fd_cb, "target address has an unsupported family: %d",
            to->sa_family);
    }
    ...
    if (SOCK_TYPE(so) == SOCK_STREAM || /* TCP or */
        ...
        ((inp->inp_vflag & INP_IPV6) &&
         !IN6_IS_ADDR_UNSPECIFIED(
             &inp->in6p_laddr)) || /* local address is not un-specified */
        ((inp->inp_vflag & INP_IPV4) && inp->inp_laddr.s_addr != INADDR_ANY)) {
      fd_cb->flags |= FLOW_DIVERT_SHOULD_SET_LOCAL_ADDR;
    }

    error =
        flow_divert_create_connect_packet(fd_cb, to, so, p, &connect_packet);
    if (error) {
      goto done;
    }
...
errno_t flow_divert_connect_out(struct socket *so, struct sockaddr *to,
                                proc_t p) {
#if CONTENT_FILTER
...
static int flow_divert_connectx_out_common(struct socket *so,
                                           struct sockaddr *dst, struct proc *p,
                                           uint32_t ifscope, sae_connid_t *pcid,
                                           struct uio *auio,
  ...
  if (auio != NULL) {
    ...
    error = so->so_proto->pr_usrreqs->pru_sosend(so, NULL, (uio_t)auio, NULL,
                                                 NULL, 0);
    socket_lock(so, 0);
...
static int flow_divert_connectx_out(
    struct socket *so, struct sockaddr *src __unused, struct sockaddr *dst,
    struct proc *p, uint32_t ifscope, sae_associd_t aid __unused,
  ...
  return flow_divert_connectx_out_common(so, dst, p, ifscope, pcid, uio,
                                         bytes_written);
}

static int flow_divert_connectx6_out(
    struct socket *so, struct sockaddr *src __unused, struct sockaddr *dst,
    struct proc *p, uint32_t ifscope, sae_associd_t aid __unused,
    sae_connid_t *pcid, uint32_t flags __unused, void *arg __unused,
    uint32_t arglen __unused, struct uio *uio, user_ssize_t *bytes_written) {
...
static errno_t flow_divert_data_out(struct socket *so, int flags,
                                    mbuf_ref_t data, struct sockaddr *to,
                                    mbuf_ref_t control, struct proc *p) {
  struct flow_divert_pcb *fd_cb = so->so_fd_pcb;
  int error = 0;
  struct inpcb *inp;
#if CONTENT_FILTER
  ...
  if ((fd_cb->flags & FLOW_DIVERT_TUNNEL_WR_CLOSED) &&
      SOCK_TYPE(so) == SOCK_DGRAM) {
    /* The provider considers this datagram flow to be closed, so no data can be
     * sent */
    FDLOG0(LOG_INFO, fd_cb,
           "provider is no longer accepting writes, cannot send data");
    error = EHOSTUNREACH;
    goto done;
  }
...
#if CONTENT_FILTER
  /*
   * If the socket is subject to a UDP Content Filter and no remote address is
   * passed in, retrieve the CFIL saved remote address from the mbuf and use it.
   */
  if (to == NULL && CFIL_DGRAM_FILTERED(so)) {
    ...
    FDLOG(LOG_INFO, fd_cb, "Using remote address from CFIL saved state: %p",
          to);
  }
#endif
...
static int flow_divert_preconnect(struct socket *so) {
  int error = 0;
  struct flow_divert_pcb *fd_cb = so->so_fd_pcb;
...
static void flow_divert_set_udp_protosw(struct socket *so) {
  if (SOCK_DOM(so) == PF_INET) {
...
errno_t flow_divert_implicit_data_out(struct socket *so, int flags,
                                      mbuf_ref_t data, struct sockaddr *to,
                                      mbuf_ref_t control, struct proc *p) {
  struct flow_divert_pcb *fd_cb = so->so_fd_pcb;
  struct inpcb *inp;
...
static errno_t flow_divert_pcb_init_internal(struct socket *so,
                                             uint32_t ctl_unit,
                                             uint32_t aggregate_unit) {
  errno_t error = 0;
  struct flow_divert_pcb *fd_cb = NULL;
  ...
  do {
    uint32_t group_unit = flow_divert_derive_kernel_control_unit(
        so->last_pid, &policy_control_unit, &agg_unit, &is_aggregate);
    if (group_unit == 0 || (group_unit >= GROUP_COUNT_MAX &&
                            group_unit < FLOW_DIVERT_IN_PROCESS_UNIT_MIN)) {
      FDLOG0(LOG_ERR, fd_cb,
             "No valid group is available, cannot init flow divert");
      error = EINVAL;
      break;
    }
...
errno_t flow_divert_pcb_init(struct socket *so) {
  ...
  uint32_t ctl_unit =
      necp_socket_get_flow_divert_control_unit(inp, &aggregate_units);
...
errno_t flow_divert_token_set(struct socket *so, struct sockopt *sopt) {
  ...
  if (g_init_result) {
    FDLOG(LOG_ERR, &nil_pcb,
          "flow_divert_init failed (%d), cannot use flow divert",
          g_init_result);
    error = ENOPROTOOPT;
    goto done;
  }
  ...
  error = flow_divert_packet_get_tlv(token, 0, FLOW_DIVERT_TLV_KEY_UNIT,
                                     sizeof(key_unit), (void *)&key_unit, NULL);
  if (!error) {
    ...
    FDLOG(LOG_ERR, &nil_pcb, "Failed to get the key unit from the token: %d",
          error);
    goto done;
  } else {
    key_unit = 0;
  ...
  error = flow_divert_packet_get_tlv(token, 0, FLOW_DIVERT_TLV_CTL_UNIT,
                                     sizeof(ctl_unit), (void *)&ctl_unit, NULL);
  if (error) {
    FDLOG(LOG_ERR, &nil_pcb,
  ...
  error = flow_divert_packet_get_tlv(token, 0, FLOW_DIVERT_TLV_AGGREGATE_UNIT,
                                     sizeof(aggregate_unit),
                                     (void *)&aggregate_unit, NULL);
  ...
  if (ctl_unit > 0 && ctl_unit < GROUP_COUNT_MAX) {
    hmac_error = flow_divert_packet_verify_hmac(
        token, (key_unit != 0 ? key_unit : ctl_unit));
...
errno_t flow_divert_token_get(struct socket *so, struct sockopt *sopt) {
  ...
  error = flow_divert_packet_append_tlv(token, FLOW_DIVERT_TLV_CTL_UNIT,
                                        sizeof(ctl_unit), &ctl_unit);
  if (error) {
    goto done;
  }

  error = flow_divert_packet_append_tlv(token, FLOW_DIVERT_TLV_FLOW_ID,
                                        sizeof(fd_cb->hash), &fd_cb->hash);
  if (error) {
    goto done;
  }

  if (fd_cb->app_data != NULL) {
    error = flow_divert_packet_append_tlv(token, FLOW_DIVERT_TLV_APP_DATA,
                                          (uint32_t)fd_cb->app_data_length,
                                          fd_cb->app_data);
    if (error) {
      goto done;
    }
  ...
  if (control_group != NULL) {
    ...
    error = flow_divert_packet_append_tlv(token, FLOW_DIVERT_TLV_KEY_UNIT,
                                          sizeof(ctl_unit), &ctl_unit);
    if (!error) {
  ...
  error = flow_divert_packet_append_tlv(token, FLOW_DIVERT_TLV_HMAC,
                                        sizeof(hmac), hmac);
  if (error) {
    goto done;
  }
  ...
  if (sopt->sopt_val == USER_ADDR_NULL) {
    /* If the caller passed NULL to getsockopt, just set the size of the token
     * and return */
  ...
  if (error) {
    token =
        NULL; /* For some reason, soopt_mcopyout() frees the mbuf if it fails */
    goto done;
  }
...
void flow_divert_group_destroy(struct flow_divert_group *group) {
  lck_rw_lock_exclusive(&group->lck);
...
static struct flow_divert_group *flow_divert_allocate_group(u_int32_t unit,
                                                            pid_t pid) {
...
static errno_t flow_divert_kctl_setup(u_int32_t *unit, void **unitinfo) {
    ...
    if (unit_cursor == FLOW_DIVERT_IN_PROCESS_UNIT_MAX) {
      ...
      *unit = unit_cursor;
      ...
      if (group_next != NULL) {
        ...
        TAILQ_INSERT_TAIL(&g_flow_divert_in_process_group_list, new_group,
                          chain);
      }
    ...
    if (kauth_cred_issuser(kauth_cred_get()) == 0) {
      ...
      if (g_flow_divert_groups == NULL) {
        g_flow_divert_groups =
            kalloc_type(struct flow_divert_group *, GROUP_COUNT_MAX,
...
static errno_t flow_divert_kctl_connect(kern_ctl_ref kctlref __unused,
...
static errno_t flow_divert_kctl_disconnect(kern_ctl_ref kctlref __unused,
                                           uint32_t unit, void *unitinfo) {
  struct flow_divert_group *group = NULL;
  errno_t error = 0;
  ...
  if (g_active_group_count == 0) {
    panic("flow divert group %u is disconnecting, but no groups are active "
          "(active count = %u)",
  ...
  if (unit < FLOW_DIVERT_IN_PROCESS_UNIT_MIN) {
    ...
    if (TAILQ_EMPTY(&g_flow_divert_in_process_group_list)) {
      panic("flow divert group %u is disconnecting, but in-process group list "
            "is empty",
            unit);
    }
  ...
  if (g_active_group_count == 0) {
    kfree_type(struct flow_divert_group *, GROUP_COUNT_MAX,
               g_flow_divert_groups);
...
static errno_t flow_divert_kctl_send(__unused kern_ctl_ref kctlref,
                                     uint32_t unit, __unused void *unitinfo,
                                     mbuf_ref_t m, __unused int flags) {
  errno_t error = 0;
...
static void flow_divert_kctl_rcvd(__unused kern_ctl_ref kctlref, uint32_t unit,
                                  __unused void *unitinfo, __unused int flags) {
  struct flow_divert_group *group = flow_divert_group_lookup(unit, NULL);
  if (group == NULL) {
    return;
  }
  ...
  if (!OSTestAndClear(GROUP_BIT_CTL_ENQUEUE_BLOCKED, &group->atomic_bits)) {
    ...
    while (!MBUFQ_EMPTY(&group->send_queue)) {
      ...
      int error = ctl_enqueuembuf(g_flow_divert_kctl_ref, group->ctl_unit,
                                  next_packet, CTL_DATA_EOR);
      if (error) {
        FDLOG(LOG_NOTICE, &nil_pcb,
              "flow_divert_kctl_rcvd: ctl_enqueuembuf returned an error: %d",
              error);
...
void flow_divert_init(void) {
  ...
  memcpy(&g_flow_divert_in_protosw, g_tcp_protosw,
         sizeof(g_flow_divert_in_protosw));
  memcpy(&g_flow_divert_in_usrreqs, g_tcp_protosw->pr_usrreqs,
   ...
   */
  g_flow_divert_in_protosw.pr_filter_head.tqh_first = __unsafe_forge_single(
      struct socket_filter *, (uintptr_t)0xdeadbeefdeadbeef);
  g_flow_divert_in_protosw.pr_filter_head.tqh_last = __unsafe_forge_single(
      struct socket_filter **, (uintptr_t)0xdeadbeefdeadbeef);
  ...
  memcpy(&g_flow_divert_in_udp_usrreqs, g_udp_protosw->pr_usrreqs,
   ...
   * real protocol; if they do, it is a bug and we should panic.
   */
  g_flow_divert_in_udp_protosw.pr_filter_head.tqh_first = __unsafe_forge_single(
      struct socket_filter *, (uintptr_t)0xdeadbeefdeadbeef);
  g_flow_divert_in_udp_protosw.pr_filter_head.tqh_last = __unsafe_forge_single(
      struct socket_filter **, (uintptr_t)0xdeadbeefdeadbeef);
  ...
  memcpy(&g_flow_divert_in6_protosw, g_tcp6_protosw,
         sizeof(g_flow_divert_in6_protosw));
  memcpy(&g_flow_divert_in6_usrreqs, g_tcp6_protosw->pr_usrreqs,
   ...
   * real protocol; if they do, it is a bug and we should panic.
   */
  g_flow_divert_in6_protosw.pr_filter_head.tqh_first = __unsafe_forge_single(
      struct socket_filter *, (uintptr_t)0xdeadbeefdeadbeef);
  g_flow_divert_in6_protosw.pr_filter_head.tqh_last = __unsafe_forge_single(
      struct socket_filter **, (uintptr_t)0xdeadbeefdeadbeef);

  /* UDP6 */
  g_udp6_protosw =
      (struct ip6protosw *)pffindproto(AF_INET6, IPPROTO_UDP, SOCK_DGRAM);

  VERIFY(g_udp6_protosw != NULL);

  memcpy(&g_flow_divert_in6_udp_protosw, g_udp6_protosw,
         sizeof(g_flow_divert_in6_udp_protosw));
  memcpy(&g_flow_divert_in6_udp_usrreqs, g_udp6_protosw->pr_usrreqs,
  ...
  g_flow_divert_in6_udp_protosw.pr_filter_head.tqh_first =
      __unsafe_forge_single(struct socket_filter *,
                            (uintptr_t)0xdeadbeefdeadbeef);
  g_flow_divert_in6_udp_protosw.pr_filter_head.tqh_last = __unsafe_forge_single(
      struct socket_filter **, (uintptr_t)0xdeadbeefdeadbeef);
