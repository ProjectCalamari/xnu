...
void in_stat_set_activity_bitmap(activity_bitmap_t *activity, uint64_t now) {
  ...
  if (slot < IN_STAT_ACTIVITY_BITMAP_TOTAL_SIZE) {
    ...
    *bitmap |= (((u_int64_t)1) << slot);
    ...
    if (slot >= (IN_STAT_ACTIVITY_BITMAP_TOTAL_SIZE * 2)) {
      ...
      uint64_t shift = slot - (IN_STAT_ACTIVITY_BITMAP_TOTAL_SIZE - 1);
      /*
       * Move the start time and bitmap forward to
       * cover the lost time
       */
      activity->start += (shift << IN_STAT_ACTIVITY_TIME_SEC_SHIFT);
      if (shift > IN_STAT_ACTIVITY_BITMAP_FIELD_SIZE) {
        ...
        b0_high = (b1_low << (IN_STAT_ACTIVITY_BITMAP_FIELD_SIZE - shift));
