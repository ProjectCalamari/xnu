...
static inline void rledbat_clear_state(struct tcpcb *tp) {
 ...
 */
static uint32_t rledbat_gain(uint32_t base_rtt) {
  return MIN(GAIN_CONSTANT, tcp_ceil(2 * target_qdelay / (double)base_rtt));
}

/*
 * Congestion avoidance for ledbat++
 */
static void rledbat_congestion_avd(struct tcpcb *tp, uint32_t segment_len,
  ...
  if (curr_rtt <= base_rtt + (uint32_t)target_qdelay) {
     ...
     */
    uint32_t qdelay = curr_rtt > base_rtt ? (curr_rtt - base_rtt) : 0;
...
void tcp_rledbat_data_rcvd(struct tcpcb *tp, struct tcphdr *th,
  ...
  const uint32_t curr_rtt =
      tcp_use_min_curr_rtt ? tp->curr_rtt_min : tp->t_rttcur;
  ...
  if (SEQ_LT(th->th_seq + segment_len, tp->rcv_high) &&
    ...
    if (tp->t_rlstate.slowdown_ts != 0) {
      ...
      tp->t_rlstate.slowdown_ts =
          tcp_globals_now(globals) + TCP_BASE_RTT_INTERVAL;
    }
    return;
  }
  ...
  if (curr_rtt == 0 || base_rtt == 0) {
    update = MIN(segment_len, TCP_CC_CWND_INIT_PKTS * tp->t_maxseg);
    tp->t_rlstate.win += update;
    tp->t_rlstate.win = min(tp->t_rlstate.win, TCP_MAXWIN << tp->rcv_scale);
...
uint32_t tcp_rledbat_get_rlwin(struct tcpcb *tp) {
  ...
  if (tp->t_rlstate.win > tp->t_rlstate.win_ws ||
     ...
     */
    tp->t_rlstate.win_ws = tp->t_rlstate.win_ws - tp->t_rlstate.drained_bytes;
  }
  tp->t_rlstate.drained_bytes = 0;
...
void tcp_rledbat_switch_to(struct tcpcb *tp) {
  ...
  if (tp->t_rlstate.ssthresh == 0) {
    ...
    tp->t_rlstate.ssthresh =
        MIN(tp->t_rlstate.ssthresh, TCP_MAXWIN << TCP_MAX_WINSHIFT);
  }
}
