...
SYSCTL_INT(_net_inet_mptcp, OID_AUTO, mptcp_cap_retr,
           CTLFLAG_RW | CTLFLAG_LOCKED, &mptcp_mpcap_retries, 0,
           "Number of MP Capable SYN Retries");
...
SYSCTL_INT(_net_inet_mptcp, OID_AUTO, rtthist_thresh,
           CTLFLAG_RW | CTLFLAG_LOCKED, &mptcp_rtthist_rtthresh, 0,
           "Rtt threshold");
...
static int sysctl_mptcp_version_check SYSCTL_HANDLER_ARGS {
...
SYSCTL_PROC(_net_inet_tcp, OID_AUTO, mptcp_preferred_version,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &mptcp_preferred_version,
            0, &sysctl_mptcp_version_check, "I", "");

int mptcp_reass_total_qlen = 0;
SYSCTL_INT(_net_inet_mptcp, OID_AUTO, reass_qlen, CTLFLAG_RD | CTLFLAG_LOCKED,
           &mptcp_reass_total_qlen, 0,
           "Total number of MPTCP segments in reassembly queues");

static int mptcp_reass_present(struct socket *mp_so) {
...
static int mptcp_reass(struct socket *mp_so, struct pkthdr *phdr, int *tlenp,
                       struct mbuf *m) {
   ...
   */
  qlimit =
      MIN(MAX(100, mp_so->so_rcv.sb_hiwat >> 10), (tcp_autorcvbuf_max >> 10));
  if (mb_dsn != mp_tp->mpt_rcvnxt && (mp_tp->mpt_reassqlen + 1) >= qlimit) {
 ...
 */
void mptcp_input(struct mptses *mpte, struct mbuf *m) {
  struct socket *mp_so;
  ...
  if (__improbable((m->m_flags & M_PKTHDR) == 0)) {
    m_drop_list(m, NULL, DROPTAP_FLAG_DIR_IN | DROPTAP_FLAG_L2_MISSING,
                DROP_REASON_MPTCP_INPUT_MALFORMED, NULL, 0);
    return;
  }
  ...
  mp_tp->mpt_rcvwnd =
      imax(mptcp_sbspace(mp_tp), (int)(mp_tp->mpt_rcvadv - mp_tp->mpt_rcvnxt));
  ...
  fallback:
    ...
    DTRACE_MPTCP5(receive__degraded, struct mbuf *, m, struct socket *, mp_so,
                  struct sockbuf *, &mp_so->so_rcv, struct sockbuf *,
                  &mp_so->so_snd, struct mptses *, mpte);
  ...
  do {
    ...
    if (todrop > 0) {
      ...
      os_log_info(
          ...
          __func__, (unsigned long)VM_KERNEL_ADDRPERM(mpte), (uint32_t)mb_dsn,
          mb_datalen, (uint32_t)mp_tp->mpt_rcvnxt, mp_tp->mpt_rcvwnd, todrop);
    ...
    if (MPTCP_SEQ_LT(mb_dsn, mp_tp->mpt_rcvnxt)) {
      if (MPTCP_SEQ_LEQ((mb_dsn + mb_datalen), mp_tp->mpt_rcvnxt)) {
        if (freelist == NULL) {
          freelist = m;
        } else {
          tail->m_next = m;
    ...
    DTRACE_MPTCP6(receive, struct mbuf *, m, struct socket *, mp_so,
                  struct sockbuf *, &mp_so->so_rcv, struct sockbuf *,
                  &mp_so->so_snd, struct mptses *, mpte, struct mptcb *, mp_tp);
    count = mp_so->so_rcv.sb_cc - count;
...
boolean_t mptcp_can_send_more(struct mptcb *mp_tp, boolean_t ignore_reinject) {
  ...
  if (!(mp_so->so_flags1 & SOF1_PRECONNECT_DATA) &&
      MPTCP_SEQ_GEQ(mp_tp->mpt_sndnxt, mp_tp->mpt_sndmax)) {
    return FALSE;
  }
 ...
 * MPTCP output.
 */
int mptcp_output(struct mptses *mpte) {
  struct mptcb *mp_tp;
  struct mptsub *mpts;
...
mptcp_choose_subflow(struct mptsub *mpts, struct mptsub *curbest, int *currtt) {
   ...
   */
  if (curbest && sototcpcb(curbest->mpts_socket)->t_rxtshift &&
      tp->t_rxtshift == 0) {
    *currtt = tp->t_srtt;
    return mpts;
  }
...
static struct mptsub *mptcp_return_subflow(struct mptsub *mpts) {
...
static boolean_t mptcp_subflow_is_slow(struct mptses *mpte,
                                       struct mptsub *mpts) {
  struct tcpcb *tp = sototcpcb(mpts->mpts_socket);
 ...
 */
struct mptsub *mptcp_get_subflow(struct mptses *mpte,
                                 struct mptsub **preferred) {
  ...
  TAILQ_FOREACH(mpts, &mpte->mpte_subflows, mpts_entry) {
    ...
    if ((so->so_state & SS_ISDISCONNECTED) ||
        ...
        !TCPS_HAVEESTABLISHED(tp->t_state) || tp->t_state > TCPS_CLOSE_WAIT) {
      continue;
    }
    ...
    if (IFNET_IS_CELLULAR(inp->inp_last_outifp)) {
      second_best = mptcp_choose_subflow(mpts, second_best, &exp_rtt);
    } else {
  ...
  if (mpte->mpte_svctype == MPTCP_SVCTYPE_PURE_HANDOVER) {
    ...
    if (besttp->t_srtt && secondtp->t_srtt && besttp->t_srtt >= rtt_thresh &&
        secondtp->t_srtt < rtt_thresh) {
      tcpstat.tcps_mp_sel_rtt++;
      return mptcp_return_subflow(second_best);
    }

    if (mptcp_subflow_is_slow(mpte, best) && secondtp->t_rxtshift == 0) {
      return mptcp_return_subflow(second_best);
    }

    /* Compare RTOs, select second_best if best's rto exceeds rtothresh */
    if (besttp->t_rxtcur && secondtp->t_rxtcur &&
        besttp->t_rxtcur >= rto_thresh && secondtp->t_rxtcur < rto_thresh) {
...
void mptcp_close_fsm(struct mptcb *mp_tp, uint32_t event) {
  struct socket *mp_so = mptetoso(mp_tp->mpt_mpte);

  socket_lock_assert_owned(mp_so);

  DTRACE_MPTCP2(state__change, struct mptcb *, mp_tp, uint32_t, event);
...
void mptcp_update_dss_rcv_state(struct mptcp_dsn_opt *dss_info,
  ...
  mptcp_update_rcv_state_meat(mp_tp, tp, full_dsn, dss_info->mdss_subflow_seqn,
                              dss_info->mdss_data_len, csum);
}

void mptcp_update_rcv_state_meat(struct mptcb *mp_tp, struct tcpcb *tp,
                                 u_int64_t full_dsn, u_int32_t seqn,
                                 u_int16_t mdss_data_len, uint16_t csum) {
  ...
  if (mdss_data_len == 0) {
    ...
    if ((mp_tp->mpt_flags & MPTCPF_CHECKSUM) && (csum != 0)) {
      os_log_error(mptcp_log_handle, "%s - %lx: Bad checksum %x \n", __func__,
                   (unsigned long)VM_KERNEL_ADDRPERM(mp_tp->mpt_mpte), csum);
    }
...
static uint16_t mptcp_input_csum(struct tcpcb *tp, struct mbuf *m, uint64_t dsn,
...
int mptcp_validate_csum(struct tcpcb *tp, struct mbuf *m, uint64_t dsn,
...
uint16_t mptcp_output_csum(struct mbuf *m, uint64_t dss_val, uint32_t sseq,
                           uint16_t dlen) {
  uint32_t sum = 0;
 ...
 */
boolean_t mptcp_no_rto_spike(struct socket *so) {
...
void mptcp_handle_deferred_upcalls(struct mppcb *mpp, uint32_t flag) {
...
static void mptcp_reset_itfinfo(struct mpt_itf_info *info) {
  memset(info, 0, sizeof(*info));
}

void mptcp_session_necp_cb(void *handle, int action, uint32_t interface_index,
  ...
  os_log(mptcp_log_handle,
         "%s - %lx: action: %u ifindex %u delegated to %u usecount %u "
         "mpt_flags %#x state %u v4 %u v6 %u nat64 %u power %u\n",
         __func__, (unsigned long)VM_KERNEL_ADDRPERM(mpte), action, ifindex,
         ifp && ifp->if_delegated.ifp ? ifp->if_delegated.ifp->if_index
                                      : IFSCOPE_NONE,
  ...
  if (action == NECP_CLIENT_CBACTION_NONVIABLE) {
    ...
    if (dst && dst->sa_family == AF_INET && has_v6 && !has_nat64 && !has_v4) {
      if (found_slot) {
...
void mptcp_set_restrictions(struct socket *mp_so) {
  struct mptses *mpte = mpsotompte(mp_so);
  uint32_t i;
