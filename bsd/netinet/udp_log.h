...
extern void udp_log_message(const char *func_name, int line_no,
                            struct inpcb *inp, const char *format, ...)
    __printflike(4, 5);
extern void udp_log_drop_pcb(void *hdr, struct udphdr *uh, struct inpcb *inp,
                             bool outgoing, const char *format);

static inline bool udp_is_log_enabled(struct inpcb *inp, uint32_t req_flags) {
  if (inp == NULL) {
    return false;
  }
  /*
...
#define UDP_LOG_BIND(inp, error)                                               \
  if (udp_is_log_enabled((inp), ULEF_BIND))                                    \
  udp_log_bind((inp), (error))

#define UDP_LOG_CONNECT(inp, error)                                            \
  if (udp_is_log_enabled((inp), ULEF_CONNECT))                                 \
  udp_log_connect((inp), (error))

#define UDP_LOG_CONNECTION_SUMMARY(inp)                                        \
  if ((inp) != NULL && ((inp)->inp_flags2 & INP2_LOGGING_ENABLED))             \
  udp_log_connection_summary((inp))

#define UDP_LOG(inp, format, ...)                                              \
  if (udp_is_log_enabled((inp), ULEF_LOG))                                     \
  udp_log_message(__func__, __LINE__, (inp), format, ##__VA_ARGS__)

#define UDP_LOG_DROP_NECP(hdr, uh, inp, outgoing)                              \
  if (udp_is_log_enabled(inp, ULEF_DROP_NECP))                                 \
  udp_log_drop_pcb((hdr), (uh), (inp), (outgoing), "NECP")

#define UDP_LOG_DROP_PCB(hdr, uh, inp, outgoing, reason)                       \
  if (udp_is_log_enabled(inp, ULEF_DROP_PCB))                                  \
