...
__private_extern__ struct ipfilter_list ipv4_filters =
    TAILQ_HEAD_INITIALIZER(ipv4_filters);
__private_extern__ struct ipfilter_list ipv6_filters =
    TAILQ_HEAD_INITIALIZER(ipv6_filters);
__private_extern__ struct ipfilter_list tbr_filters =
...
static errno_t ipf_add(const struct ipf_filter *filter, ipfilter_t *filter_ref,
                       struct ipfilter_list *head, bool is_internal);

#if SKYWALK
static bool net_check_compatible_ipf(void);
#endif /* SKYWALK */

__private_extern__ void ipf_ref(void) {
...
__private_extern__ void ipf_unref(void) {
  lck_mtx_lock(&kipf_lock);
  ...
  if (kernel_is_macos_or_server()) {
    net_filter_event_mark(NET_FILTER_EVENT_IP, net_check_compatible_ipf());
  }
#endif /* SKYWALK */
  lck_mtx_unlock(&kipf_lock);
}
...
static errno_t ipf_add(const struct ipf_filter *filter, ipfilter_t *filter_ref,
  ...
  if (filter->name == NULL ||
      (filter->ipf_input == NULL && filter->ipf_output == NULL)) {
    return EINVAL;
  }
...
errno_t ipf_addv4_internal(const struct ipf_filter *filter,
                           ipfilter_t *filter_ref) {
...
errno_t ipf_addv6_internal(const struct ipf_filter *filter,
                           ipfilter_t *filter_ref) {
...
static errno_t ipf_input_detached(void *cookie, mbuf_t *data, int offset,
                                  u_int8_t protocol) {
...
static errno_t ipf_output_detached(void *cookie, mbuf_t *data,
...
errno_t ipf_remove(ipfilter_t filter_ref) {
  ...
  if (match == 0 ||
      (match->ipf_head != &ipv4_filters && match->ipf_head != &ipv6_filters)) {
    return EINVAL;
  }
...
static errno_t ipf_injectv4_out(mbuf_t data, ipfilter_t filter_ref,
                                ipf_pktopts_t options) {
  struct route ro;
  struct ip *ip;
  struct mbuf *m = (struct mbuf *)data;
  ...
  if (options != NULL) {
    ...
    if (options->ippo_flags & IPPOF_BOUND_IF) {
      ...
      ipoa.ipoa_boundif = options->ippo_flags >> IPPOF_SHIFT_IFSCOPE;
    }
  ...
  error = ip_output(m, NULL, &ro, IP_ALLOWBROADCAST | IP_RAWOUTPUT | IP_OUTARGS,
                    imo, &ipoa);
...
static errno_t ipf_injectv6_out(mbuf_t data, ipfilter_t filter_ref,
                                ipf_pktopts_t options) {
  struct route_in6 ro;
  struct ip6_hdr *ip6;
  struct mbuf *m = (struct mbuf *)data;
  ...
  if (options != NULL) {
    ...
    if (options->ippo_flags & IPPOF_BOUND_IF) {
      ...
      ip6oa.ip6oa_boundif = options->ippo_flags >> IPPOF_SHIFT_IFSCOPE;
    }
    if (options->ippo_flags & IPPOF_NO_IFT_CELLULAR) {
...
errno_t ipf_inject_output(mbuf_t data, ipfilter_t filter_ref,
                          ipf_pktopts_t options) {
  struct mbuf *m = (struct mbuf *)data;
  u_int8_t vers;
  errno_t error = 0;
...
__private_extern__ ipfilter_t ipf_get_inject_filter(struct mbuf *m) {
...
static struct m_tag *m_tag_kalloc_ipfilt(u_int32_t id, u_int16_t type,
...
static void m_tag_kfree_ipfilt(struct m_tag *tag) {
  struct ipfilt_tag_container *tag_container =
      (struct ipfilt_tag_container *)tag;
...
void ipfilter_register_m_tag(void) {
  int error;

  error =
      m_register_internal_tag_type(KERNEL_TAG_TYPE_IPFILT, sizeof(ipfilter_t),
...
#if SKYWALK
bool net_check_compatible_ipf(void) {
