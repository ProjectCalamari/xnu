...
static void mpp_lock_assert_held(struct mppcb *mp) {
#if !MACH_ASSERT
#pragma unused(mp)
#endif
...
static void mp_timeout(void *arg) {
#pragma unused(arg)
  ...
  if (gc || t) {
    ...
    TAILQ_FOREACH(mppi, &mppi_head, mppi_entry) {
      if ((gc && mppi->mppi_gc != NULL) || (t && mppi->mppi_timer != NULL)) {
...
void mp_pcbinfo_attach(struct mppcbinfo *mppi) {
  struct mppcbinfo *mppi0;

  lck_mtx_lock(&mp_lock);
  TAILQ_FOREACH(mppi0, &mppi_head, mppi_entry) {
    if (mppi0 == mppi) {
      panic("%s: mppi %p already in the list", __func__, mppi);
      /* NOTREACHED */
    }
  }
...
int mp_pcbinfo_detach(struct mppcbinfo *mppi) {
  struct mppcbinfo *mppi0;
  int error = 0;
...
int mp_pcballoc(struct socket *so, struct mppcbinfo *mppi) {
...
void mp_pcbdetach(struct socket *mp_so) {
...
void mptcp_pcbdispose(struct mppcb *mpp) {
...
static int mp_getaddr_v4(struct socket *mp_so, struct sockaddr **nam,
                         boolean_t peer) {
   ...
   */
  sin = (struct sockaddr_in *)alloc_sockaddr(sizeof(*sin), Z_WAITOK | Z_NOFAIL);
...
static int mp_getaddr_v6(struct socket *mp_so, struct sockaddr **nam,
                         boolean_t peer) {
  struct mptses *mpte = mpsotompte(mp_so);
...
int mp_getsockaddr(struct socket *mp_so, struct sockaddr **nam) {
  struct mptses *mpte = mpsotompte(mp_so);
...
int mp_getpeeraddr(struct socket *mp_so, struct sockaddr **nam) {
  struct mptses *mpte = mpsotompte(mp_so);
