...
#define UDP_LOG_ENABLE_DEFAULT (ULEF_CONNECT | ULEF_DST_LOCAL | ULEF_DST_GW)
#else /* (DEVELOPMENT || DEBUG) */
#define UDP_LOG_ENABLE_DEFAULT (ULEF_CONNECT | ULEF_DST_LOCAL | ULEF_DST_GW)
#endif /* (DEVELOPMENT || DEBUG) */

uint32_t udp_log_enable_flags = UDP_LOG_ENABLE_DEFAULT;
SYSCTL_UINT(_net_inet_udp_log, OID_AUTO, enable, CTLFLAG_RW | CTLFLAG_LOCKED,
            &udp_log_enable_flags, 0, "");

/*
 * The following is a help to describe the values of the flags
 */
#define X(name, value, description, ...) #description ":" #value " "
SYSCTL_STRING(_net_inet_udp_log, OID_AUTO, enable_usage,
              CTLFLAG_RD | CTLFLAG_LOCKED, UDP_ENABLE_FLAG_LIST, 0, "");
#undef X
...
SYSCTL_ULONG(_net_inet_udp_log, OID_AUTO, rate_max, CTLFLAG_RD | CTLFLAG_LOCKED,
             &udp_log_rate_max, "");
...
#define UDP_LOG_COMMON_ARGS                                                    \
  laddr_buf, ntohs(local_port), faddr_buf, ntohs(foreign_port),                \
      ifp != NULL ? if_name(ifp) : "", udp_log_rate_exceeded_total
...
#define UDP_LOG_COMMON_PCB_ARGS                                                \
  UDP_LOG_COMMON_ARGS, so != NULL ? so->so_gencnt : 0,                         \
      so != NULL ? so->so_state : 0, inp->inp_last_proc_name, so->last_pid

/*
 * Returns true when above the rate limit
 */
static bool udp_log_is_rate_limited(void) {
...
static bool udp_log_port_allowed(struct inpcb *inp) {
...
static inline void udp_log_common(struct inpcb *inp, const char *event,
                                  int error) {
  ...
  inp_log_addresses(inp, laddr_buf, sizeof(laddr_buf), faddr_buf,
                    sizeof(faddr_buf));

#define UDP_LOG_CONNECTION_FMT                                                 \
  "udp %s: " UDP_LOG_COMMON_PCB_FMT "bytes in/out: %llu/%llu "                 \
  "pkts in/out: %llu/%llu "                                                    \
  "error: %d "                                                                 \
  "so_error: %d "                                                              \
  "svc/tc: %u "                                                                \
...
#define UDP_LOG_CONNECTION_ARGS                                                \
  event, UDP_LOG_COMMON_PCB_ARGS, inp->inp_mstat.ms_total.ts_rxbytes,          \
      inp->inp_mstat.ms_total.ts_txbytes,                                      \
      inp->inp_mstat.ms_total.ts_rxpackets,                                    \
      inp->inp_mstat.ms_total.ts_txpackets, error, so->so_error,               \
      (so->so_flags1 & SOF1_TC_NET_SERV_TYPE) ? so->so_netsvctype              \
                                              : so->so_traffic_class,          \
      inp->inp_flowhash

  os_log(OS_LOG_DEFAULT, UDP_LOG_CONNECTION_FMT, UDP_LOG_CONNECTION_ARGS);

#undef UDP_LOG_CONNECTION_FMT
#undef UDP_LOG_CONNECTION_ARGS
}

__attribute__((noinline)) void udp_log_bind(struct inpcb *inp, int error) {
...
__attribute__((noinline)) void udp_log_connect(struct inpcb *inp, int error) {
  if (inp == NULL || inp->inp_socket == NULL) {
    return;
  }
  /* Do not log too much */
...
__attribute__((noinline)) void udp_log_connection_summary(struct inpcb *inp) {
  struct socket *so;
  struct ifnet *ifp;
  ...
  if (inp->inp_connect_timestamp > 0) {
    ...
    absolutetime_to_microtime(duration, &connection_secs,
                              &connection_microsecs);
  }
...
#define UDP_LOG_CONNECTION_SUMMARY_FMT                                         \
  "udp_connection_summary " UDP_LOG_COMMON_PCB_FMT "Duration: %lu.%03d sec "   \
...
#define UDP_LOG_CONNECTION_SUMMARY_ARGS                                        \
  UDP_LOG_COMMON_PCB_ARGS, duration_secs, duration_microsecs / 1000,           \
      connection_secs, connection_microsecs / 1000,                            \
      inp->inp_mstat.ms_total.ts_rxbytes, inp->inp_mstat.ms_total.ts_txbytes,  \
      inp->inp_mstat.ms_total.ts_rxpackets,                                    \
      inp->inp_mstat.ms_total.ts_txpackets,                                    \
      so->so_tc_stats[SO_STATS_SBNOSPACE].rxpackets,                           \
      so->so_tc_stats[SO_STATS_SBNOSPACE].rxbytes, so->so_error,               \
      (so->so_flags1 & SOF1_TC_NET_SERV_TYPE) ? so->so_netsvctype              \
                                              : so->so_traffic_class,          \
      inp->inp_flowhash, inp->inp_fadv_total_time, inp->inp_fadv_cnt
...
__attribute__((noinline)) void udp_log_message(const char *func_name,
                                               int line_no, struct inpcb *inp,
                                               const char *format, ...) {
  struct socket *so;
  struct ifnet *ifp;
  char laddr_buf[ADDRESS_STR_LEN];
...
#define UDP_LOG_MESSAGE_FMT "udp (%s:%d): " UDP_LOG_COMMON_PCB_FMT "%s"

#define UDP_LOG_MESSAGE_ARGS                                                   \
  func_name, line_no, UDP_LOG_COMMON_PCB_ARGS, message

  os_log(OS_LOG_DEFAULT, UDP_LOG_MESSAGE_FMT, UDP_LOG_MESSAGE_ARGS);

#undef UDP_LOG_MESSAGE_FMT
#undef UDP_LOG_MESSAGE_ARGS
}

static bool udp_log_pkt_addresses(void *hdr, bool outgoing,
...
__attribute__((noinline)) void udp_log_drop_pcb(void *hdr, struct udphdr *uh,
                                                struct inpcb *inp,
                                                bool outgoing,
                                                const char *reason) {
  struct socket *so;
  ...
  if (hdr != NULL && uh != NULL) {
    ...
    (void)udp_log_pkt_addresses(hdr, outgoing, laddr_buf, sizeof(laddr_buf),
...
#define UDP_LOG_DROP_PCB_FMT                                                   \
  "udp drop %s" UDP_LOG_COMMON_PCB_FMT "so_error: %d "                         \
  "reason: %s"

#define UDP_LOG_DROP_PCB_ARGS                                                  \
  direction, UDP_LOG_COMMON_PCB_ARGS, so->so_error, reason

  os_log(OS_LOG_DEFAULT, UDP_LOG_DROP_PCB_FMT, UDP_LOG_DROP_PCB_ARGS);
