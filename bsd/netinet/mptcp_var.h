...
struct mpt_itf_info {
  uint32_t ifindex;
  uint32_t has_v4_conn : 1, has_v6_conn : 1, has_nat64_conn : 1,
      no_mptcp_support : 1;
};
...
#define __mpte_dst_v6 mpte_u_dst._mpte_dst_v6
  ...
  uint16_t mpte_alternate_port; /* Alternate port for subflow establishment
                                   (network-byte-order) */
...
#define MPTE_FIRSTPARTY                                                        \
  0x04 /* First-party app used multipath_extended entitlement */
#define MPTE_ACCESS_GRANTED                                                    \
  0x08 /* Access to cellular has been granted for this connection */
#define MPTE_FORCE_ENABLE                                                      \
  0x10 /* For MPTCP regardless of heuristics to detect middleboxes */
#define MPTE_IN_WORKLOOP 0x20 /* Are we currently inside the workloop ? */
#define MPTE_WORKLOOP_RELAUNCH                                                 \
  0x40 /* Another event got queued, we should restart the workloop */
#define MPTE_UNICAST_IP                                                        \
  0x80 /* New subflows are only being established towards the unicast IP in    \
          the ADD_ADDR */
#define MPTE_CELL_PROHIBITED                                                   \
  0x100 /* Cell access has been prohibited based on signal quality */
#define MPTE_FORCE_V0                                                          \
  0x200 /* Force MPTCP to use version 0 regradless of tcp cache */
#define MPTE_FORCE_V1                                                          \
  0x400 /* Force MPTCP to use version 1 regradless of tcp cache */
...
#define MPTE_ITFINFO_SIZE 4
  ...
  uint32_t mpte_used_cell : 1, mpte_used_wifi : 1, mpte_initial_cell : 1,
      mpte_triggered_cell, mpte_handshake_success : 1,
...
static inline struct socket *mptetoso(struct mptses *mpte) {
  return mpte->mpte_mppcb->mpp_socket;
}

static inline struct mptses *mptompte(struct mppcb *mp) {
  return (struct mptses *)mp->mpp_pcbe;
}

static inline struct mptses *mpsotompte(struct socket *so) {
  return mptompte(mpsotomppcb(so));
}

static inline boolean_t mpp_try_lock(struct mppcb *mp) {
...
static inline void mpp_lock(struct mppcb *mp) {
...
static inline void mpp_unlock(struct mppcb *mp) {
  VERIFY(!(mp->mpp_flags & MPP_INSIDE_OUTPUT));
  VERIFY(!(mp->mpp_flags & MPP_INSIDE_INPUT));
  VERIFY(!(mp->mpp_flags & MPP_INSIDE_SETGETOPT));
...
static inline lck_mtx_t *mpp_getlock(struct mppcb *mp, int flags) {
...
static inline int mptcp_subflow_cwnd_space(struct socket *so) {
...
static inline bool mptcp_subflows_need_backup_flag(struct mptses *mpte) {
...
#define MPTSF_CLOSED                                                           \
  0x00008000 /* soclose_locked has been called on this subflow */
...
#define MPTSF_FULLY_ESTABLISHED                                                \
  0x00200000 /* Subflow is fully established and it has been confirmed         \
...
#define MPTCPF_FALLBACK_HEURISTIC                                              \
  0x100 /* Send SYN without MP_CAPABLE due to heuristic */
#define MPTCPF_HEURISTIC_TRAC                                                  \
  0x200 /* Tracked this connection in the heuristics as a failure */
...
/* mptcb manipulation */
static inline struct mptcb *tptomptp(struct tcpcb *tp) { return tp->t_mptcb; }
  ...
  {                                                                            \
    if ((MPTCP_DATASEQ_LOW32(x) > y) &&                                        \
      ...
      z |= y;                                                                  \
               ...
               (((u_int32_t)y - ((u_int32_t)MPTCP_DATASEQ_LOW32(x))) >=        \
                (u_int32_t)(1U << 31))) {                                      \
      /*                                                                       \
...
#define MPTCP_CELLICON_TOGGLE_RATE                                             \
  (5 * TCP_RETRANSHZ) /* Only toggle every 5 seconds */
...
extern void mptcpstats_inc_switch(struct mptses *mpte,
                                  const struct mptsub *mpts);
extern void
mptcpstats_update(struct mptcp_itf_stats *stats __counted_by(stats_count),
                  uint16_t stats_count, const struct mptsub *mpts);
extern int mptcpstats_get_index_by_ifindex(
    struct mptcp_itf_stats *stats __counted_by(stats_count),
    uint16_t stats_count, u_short ifindex, boolean_t create);
extern struct mptses *mptcp_drop(struct mptses *mpte, struct mptcb *mp_tp,
                                 u_short errno);
...
extern int mptcp_subflow_output(struct mptses *mpte, struct mptsub *mpts,
                                int flags);
...
extern boolean_t mptcp_can_send_more(struct mptcb *mp_tp,
...
extern void mptcp_hmac_sha256(mptcp_key_t, mptcp_key_t,
                              u_char *__sized_by(msglen), uint16_t msglen,
                              u_char sha_digest[SHA256_DIGEST_LENGTH]);
extern void mptcp_get_mpjoin_hmac(mptcp_addr_id, struct mptcb *,
                                  u_char *__sized_by(digest_len),
                                  uint8_t digest_len);
...
extern struct mptsub *mptcp_get_subflow(struct mptses *mpte,
                                        struct mptsub **preferred);
extern int mptcp_get_map_for_dsn(struct socket *so, uint64_t dsn_fail,
                                 uint32_t *tcp_seq);
...
mptcp_get_session_dst(struct mptses *mpte, boolean_t has_v6, boolean_t has_v4);
extern void mptcp_set_restrictions(struct socket *mp_so);
extern void mptcp_clear_cellicon(void);
extern void mptcp_unset_cellicon(struct mptses *mpte, struct mptsub *mpts,
                                 uint32_t val);
...
#define SYMPTOMS_ADVISORY_USEAPP                                               \
  0xFFFF /* Very ugly workaround to avoid breaking backwards compatibility -   \
            ToDo: Fix it in +1 */
