...
static int mptcp_usr_connectx(struct socket *, struct sockaddr *,
                              ...
                              sae_associd_t, sae_connid_t *, uint32_t, void *,
                              uint32_t, struct uio *, user_ssize_t *);
...
static int mptcp_getconninfo(struct mptses *, sae_connid_t *, uint32_t *,
                             ...
                             user_addr_t, socklen_t *, uint32_t *, user_addr_t,
                             uint32_t *);
...
SYSCTL_INT(_net_inet_mptcp, OID_AUTO, allow_aggregate,
           CTLFLAG_RW | CTLFLAG_LOCKED, &mptcp_developer_mode, 0,
           "Allow the Multipath aggregation mode");

int mptcp_no_first_party = 0;
SYSCTL_INT(_net_inet_mptcp, OID_AUTO, no_first_party,
           CTLFLAG_RW | CTLFLAG_LOCKED, &mptcp_no_first_party, 0,
           "Do not do first-party app exemptions");

static unsigned long mptcp_expected_progress_headstart = 5000;
SYSCTL_ULONG(_net_inet_mptcp, OID_AUTO, expected_progress_headstart,
             CTLFLAG_RW | CTLFLAG_LOCKED, &mptcp_expected_progress_headstart,
             "Headstart to give MPTCP before meeting the progress deadline");

/*
 * Attaches an MPTCP control block to a socket.
 */
static int mptcp_usr_attach(struct socket *mp_so, int proto, struct proc *p) {
#pragma unused(proto)
  int error;
 ...
 */
static int mptcp_usr_detach(struct socket *mp_so) {
 ...
 */
static int mptcp_attach(struct socket *mp_so, struct proc *p) {
#pragma unused(p)
...
static int mptcp_entitlement_check(struct socket *mp_so, uint8_t svctype) {
  ...
  if (soopt_cred_check(mp_so, PRIV_NET_RESTRICTED_MULTIPATH_EXTENDED, TRUE,
                       FALSE) == 0) {
    /*
  ...
  if (mp_so->so_flags & SOF_DELEGATED &&
      soopt_cred_check(mp_so, PRIV_NET_RESTRICTED_MULTIPATH_EXTENDED, TRUE,
                       TRUE) == 0) {
    /*
     * This means the app has the extended entitlement. Thus,
     * it's a first party app and can run without restrictions.
 ...
 */
static int mptcp_connectx(struct mptses *mpte, struct sockaddr *src,
...
#pragma unused(p, aid, flags, arg, arglen)
  ...
  if (dst->sa_family == AF_INET && dst->sa_len != sizeof(mpte->__mpte_dst_v4)) {
    os_log_error(mptcp_log_handle, "%s - %lx: IPv4 dst len %u\n", __func__,
                 (unsigned long)VM_KERNEL_ADDRPERM(mpte), dst->sa_len);
    error = EINVAL;
    goto out;
  }
  ...
  if (src) {
    ...
    if (src->sa_family == AF_INET &&
      ...
      os_log_error(mptcp_log_handle, "%s - %lx: IPv4 src len %u\n", __func__,
                   (unsigned long)VM_KERNEL_ADDRPERM(mpte), src->sa_len);
      error = EINVAL;
      goto out;
    }
  ...
  if (auio != NULL) {
    ...
    error = mp_so->so_proto->pr_usrreqs->pru_sosend(mp_so, NULL, (uio_t)auio,
                                                    NULL, NULL, 0);
 ...
 */
static int mptcp_getassocids(struct mptses *mpte, uint32_t *cnt,
                             user_addr_t aidp) {
  ...
  *cnt = (mpte->mpte_associd != SAE_ASSOCID_ANY) ? 1 : 0;
  ...
  return copyout(&mpte->mpte_associd, aidp, sizeof(mpte->mpte_associd));
}

/*
 * Handle SIOCGCONNIDS ioctl for PF_MULTIPATH domain.
 */
static int mptcp_getconnids(struct mptses *mpte, sae_associd_t aid,
                            uint32_t *cnt, user_addr_t cidp) {
  struct mptsub *mpts;
  int error = 0;
 ...
 */
static int mptcp_getconninfo(struct mptses *mpte, sae_connid_t *cid,
                             uint32_t *flags, uint32_t *ifindex,
                             int32_t *soerror, user_addr_t src,
                             socklen_t *src_len, user_addr_t dst,
                             socklen_t *dst_len, uint32_t *aux_type,
  ...
  *soerror = 0;
  ...
  if (*cid == SAE_CONNID_ALL) {
    ...
    *aux_len = sizeof(mptcp_ci);
    ...
    if (aux_data != USER_ADDR_NULL) {
      ...
      memcpy(mptcp_ci.mptcpci_itfstats, mpte->mpte_itfstats,
             sizeof(mptcp_ci.mptcpci_itfstats));

      TAILQ_FOREACH(mpts, &mpte->mpte_subflows, mpts_entry) {
        if (i >=
            sizeof(mptcp_ci.mptcpci_subflow_connids) / sizeof(sae_connid_t)) {
          break;
        }
  ...
  if (*cid == SAE_CONNID_ANY) {
    ...
    if (inp->inp_vflag & INP_IPV4) {
      error =
          in_getconninfo(so, SAE_CONNID_ANY, flags, ifindex, soerror, src,
                         src_len, dst, dst_len, aux_type, aux_data, aux_len);
    } else {
      error =
          in6_getconninfo(so, SAE_CONNID_ANY, flags, ifindex, soerror, src,
                          src_len, dst, dst_len, aux_type, aux_data, aux_len);
    }
    ...
    if (inp->inp_vflag & INP_IPV4) {
      error = in_getconninfo(so, SAE_CONNID_ANY, flags, ifindex, soerror, src,
                             src_len, dst, dst_len, aux_type, USER_ADDR_NULL,
                             aux_len);
    } else {
      error = in6_getconninfo(so, SAE_CONNID_ANY, flags, ifindex, soerror, src,
                              src_len, dst, dst_len, aux_type, USER_ADDR_NULL,
                              aux_len);
    }
    ...
    *aux_type = CIAUX_TCP;
    if (*aux_len == 0) {
      *aux_len = sizeof(tcp_ci);
       ...
       */
      index = mptcpstats_get_index_by_ifindex(
          mpte->mpte_itfstats, MPTCP_ITFSTATS_SIZE, (u_short)(*cid), false);
      if (index == -1) {
        os_log_error(
            mptcp_log_handle,
            "%s - %lx: Asking for too many ifindex: %u subcount %u, mpts? %s\n",
            __func__, (unsigned long)VM_KERNEL_ADDRPERM(mpte), *cid,
            mpte->mpte_numflows, orig_mpts ? "yes" : "no");
        ...
        tcp_ci.tcpci_tcp_info.tcpi_txbytes += stats->mpis_txbytes;
        tcp_ci.tcpci_tcp_info.tcpi_rxbytes += stats->mpis_rxbytes;

        tcp_ci.tcpci_tcp_info.tcpi_wifi_txbytes += stats->mpis_wifi_txbytes;
        tcp_ci.tcpci_tcp_info.tcpi_wifi_rxbytes += stats->mpis_wifi_rxbytes;

        tcp_ci.tcpci_tcp_info.tcpi_wired_txbytes += stats->mpis_wired_txbytes;
        tcp_ci.tcpci_tcp_info.tcpi_wired_rxbytes += stats->mpis_wired_rxbytes;

        tcp_ci.tcpci_tcp_info.tcpi_cell_txbytes += stats->mpis_cell_txbytes;
        tcp_ci.tcpci_tcp_info.tcpi_cell_rxbytes += stats->mpis_cell_rxbytes;
      }
 ...
 */
static int mptcp_usr_control(struct socket *mp_so, u_long cmd,
...
#pragma unused(ifp, p)
  ...
  case SIOCGASSOCIDS32: { /* struct so_aidreq32 */
    ...
    error = mptcp_getassocids(mpte, &aidr.sar_cnt, aidr.sar_aidp);
    if (error == 0) {
  ...
  case SIOCGASSOCIDS64: { /* struct so_aidreq64 */
    ...
    bcopy(data, &aidr, sizeof(aidr));
    error = mptcp_getassocids(mpte, &aidr.sar_cnt, (user_addr_t)aidr.sar_aidp);
    if (error == 0) {
      bcopy(&aidr, data, sizeof(aidr));
    }
    break;
  ...
  case SIOCGCONNIDS32: { /* struct so_cidreq32 */
    ...
    error = mptcp_getconnids(mpte, cidr.scr_aid, &cidr.scr_cnt, cidr.scr_cidp);
    if (error == 0) {
  ...
  case SIOCGCONNINFO32: { /* struct so_cinforeq32 */
    ...
    error = mptcp_getconninfo(mpte, &cifr.scir_cid, &cifr.scir_flags,
                              &cifr.scir_ifindex, &cifr.scir_error,
                              cifr.scir_src, &cifr.scir_src_len, cifr.scir_dst,
                              &cifr.scir_dst_len, &cifr.scir_aux_type,
                              cifr.scir_aux_data, &cifr.scir_aux_len);
    if (error == 0) {
  ...
  case SIOCGCONNINFO64: { /* struct so_cinforeq64 */
    ...
    bcopy(data, &cifr, sizeof(cifr));
    error = mptcp_getconninfo(
        mpte, &cifr.scir_cid, &cifr.scir_flags, &cifr.scir_ifindex,
        &cifr.scir_error, (user_addr_t)cifr.scir_src, &cifr.scir_src_len,
        (user_addr_t)cifr.scir_dst, &cifr.scir_dst_len, &cifr.scir_aux_type,
        (user_addr_t)cifr.scir_aux_data, &cifr.scir_aux_len);
    if (error == 0) {
      bcopy(&cifr, data, sizeof(cifr));
    }
    break;
...
static int mptcp_disconnect(struct mptses *mpte) {
  struct socket *mp_so;
  struct mptcb *mp_tp;
  int error = 0;
  ...
  if (!(mp_so->so_flags & SOF_PCBCLEARING) &&
      !(mp_so->so_flags & SOF_DEFUNCT)) {
    if (!(mp_so->so_state & (SS_ISCONNECTED | SS_ISCONNECTING))) {
      error = ENOTCONN;
      goto out;
    }
    if (mp_so->so_state & SS_ISDISCONNECTING) {
  ...
  } else if (((mp_so->so_options & SO_LINGER) && mp_so->so_linger == 0) ||
 ...
 */
static int mptcp_usr_disconnect(struct socket *mp_so) {
 ...
 */
static int mptcp_usr_disconnectx(struct socket *mp_so, sae_associd_t aid,
                                 sae_connid_t cid) {
  if (aid != SAE_ASSOCID_ANY && aid != SAE_ASSOCID_ALL) {
    return EINVAL;
  }
...
void mptcp_finish_usrclosed(struct mptses *mpte) {
  struct mptcb *mp_tp = mpte->mpte_mptcb;
  struct socket *mp_so = mptetoso(mpte);
 ...
 */
static struct mptses *mptcp_usrclosed(struct mptses *mpte) {
  struct mptcb *mp_tp = mpte->mpte_mptcb;
 ...
 */
static int mptcp_usr_rcvd(struct socket *mp_so, int flags) {
#pragma unused(flags)
  struct mppcb *mpp = mpsotomppcb(mp_so);
  struct mptses *mpte;
  struct mptsub *mpts;
 ...
 */
static int mptcp_usr_send(struct socket *mp_so, int prus_flags, struct mbuf *m,
 ...
 */
static int mptcp_usr_shutdown(struct socket *mp_so) {
  struct mppcb *mpp = mpsotomppcb(mp_so);
  struct mptses *mpte;
  int error = 0;
 ...
 */
static int mptcp_uiotombuf(struct uio *uio, int how, user_ssize_t space,
                           struct mbuf **top) {
  ...
  while (len > 0) {
    ...
    if (len > MBIGCLBYTES) {
      mb = m_getpackets_internal(&m_needed, 1, how, 1, M16KCLBYTES);
    } else if (len > MCLBYTES) {
      mb = m_getpackets_internal(&m_needed, 1, how, 1, MBIGCLBYTES);
    } else if (len >= (signed)MINCLSIZE) {
      mb = m_getpackets_internal(&m_needed, 1, how, 1, MCLBYTES);
    } else {
 ...
 */
static int mptcp_usr_sosend(struct socket *mp_so, struct sockaddr *addr,
...
#pragma unused(addr)
   ...
   */
  if (resid < 0 || resid > INT_MAX || (flags & MSG_EOR) || control != NULL) {
    error = EINVAL;
    socket_unlock(mp_so, 1);
    goto out;
  }
  ...
  do {
    error = sosendcheck(mp_so, NULL, resid, 0, 0, flags, &sblocked);
    if (error != 0) {
      goto release;
    }
    ...
    do {
       ...
       */
      sendflags = (resid > 0 && space > 0) ? PRUS_MORETOCOME : 0;
       ...
       */
      error = (*mp_so->so_proto->pr_usrreqs->pru_send)(mp_so, sendflags, top,
                                                       NULL, NULL, p);
 ...
 * and sogetoptlock().
 */
static int mptcp_usr_socheckopt(struct socket *mp_so, struct sockopt *sopt) {
#pragma unused(mp_so)
  int error = 0;
 ...
 */
static int mptcp_setopt_apply(struct mptses *mpte, struct mptopt *mpo) {
  struct socket *mp_so;
  struct mptsub *mpts;
  struct mptopt smpo;
  int error = 0;
 ...
 */
static int mptcp_setopt(struct mptses *mpte, struct sockopt *sopt) {
  ...
  if (level == SOL_SOCKET) {
    ...
    case SO_DELEGATED:
      error = sooptcopyin(sopt, &mpte->mpte_epid, sizeof(int), sizeof(int));
      if (error != 0) {
        goto err_out;
      }
    ...
    case SO_DELEGATED_UUID:
      error =
          sooptcopyin(sopt, &mpte->mpte_euuid, sizeof(uuid_t), sizeof(uuid_t));
      if (error != 0) {
        goto err_out;
      }
    ...
    case SO_NECP_CLIENTUUID:
      ...
      error = necp_client_register_multipath_cb(
          mp_so->last_pid, mpsotomppcb(mp_so)->necp_client_uuid,
          mpsotomppcb(mp_so));
      if (error) {
        goto err_out;
      }
      ...
      goto out;
    case SO_NECP_ATTRIBUTES:
      error = necp_set_socket_attributes(
          &mpsotomppcb(mp_so)->inp_necp_attributes, sopt);
      if (error) {
        goto err_out;
      }
    ...
    case TCP_NOTSENT_LOWAT:
      ...
      if (optval < 0) {
        ...
        if (optval == 0) {
          ...
          mp_so->so_flags |= SOF_NOTSENT_LOWAT;
          error = mptcp_set_notsent_lowat(mpte, optval);
        }
    ...
    case MPTCP_EXPECTED_PROGRESS_TARGET: {
      ...
      error = sooptcopyin(sopt, &mach_time_target, sizeof(mach_time_target),
      ...
      if (mach_time_target) {
        ...
        nanoseconds =
            nanoseconds - (mptcp_expected_progress_headstart * NSEC_PER_MSEC);
         ...
         */
        if (mpte->mpte_time_target && mpte->mpte_time_target < time_now &&
            time_now_nanoseconds >
                nanoseconds -
                    (mptcp_expected_progress_headstart * NSEC_PER_MSEC)) {
          os_log(mptcp_log_handle,
                 "%s - %lx: Not rescheduling timer %llu now %llu target %llu\n",
                 __func__, (unsigned long)VM_KERNEL_ADDRPERM(mpte),
                 mpte->mpte_time_target, time_now, mach_time_target);
          goto out;
        }
      }
  ...
  if ((error = sooptcopyin(sopt, &optval, sizeof(optval), sizeof(optval))) !=
      0) {
    goto err_out;
  }
...
static void mptcp_fill_info_bytestats(struct tcp_info *ti,
                                      struct mptses *mpte) {
  struct mptsub *mpts;
  int i;
...
static void mptcp_fill_info(struct mptses *mpte, struct tcp_info *ti) {
  ...
  if (acttp) {
    ti->tcpi_last_outif = (acttp->t_inpcb->inp_last_outifp == NULL)
                              ? 0
 ...
 */
static int mptcp_getopt(struct mptses *mpte, struct sockopt *sopt) {
  ...
  case MPTCP_FORCE_VERSION:
    ...
    break;
  case MPTCP_EXPECTED_PROGRESS_TARGET:
    error = sooptcopyout(sopt, &mpte->mpte_time_target,
                         sizeof(mpte->mpte_time_target));
 ...
 */
int mptcp_ctloutput(struct socket *mp_so, struct sockopt *sopt) {
  struct mppcb *__single mpp = mpsotomppcb(mp_so);
...
const char *mptcp_sopt2str(int level, int optname) {
  switch (level) {
  case SOL_SOCKET:
    switch (optname) {
    case SO_LINGER:
...
static int mptcp_usr_preconnect(struct socket *mp_so) {
  ...
  if (mpts == NULL) {
    os_log_error(mptcp_log_handle, "%s - %lx: invalid preconnect ", __func__,
                 (unsigned long)VM_KERNEL_ADDRPERM(mpte));
    return EINVAL;
  }
