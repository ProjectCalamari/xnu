...
static int mptcp_timer_demux(struct mptses *mpte, uint64_t now_msecs) {
    ...
    if ((now_msecs - mp_tp->mpt_rxtstart) > (mptcp_rto * hz)) {
      ...
      if (mp_tp->mpt_rxtshift > mptcp_nrtos) {
        ...
        os_log_info(mptcp_log_handle, "%s: REXMT %d sndnxt %u\n", __func__,
                    mp_tp->mpt_rxtshift, (uint32_t)mp_tp->mpt_sndnxt);
        mptcp_output(mpte);
      }
    } else {
      resched_timer = 1;
  ...
  case MPTT_TW:
    ...
    if ((now_msecs - mp_tp->mpt_timewait) > (mptcp_tw * hz)) {
      mp_tp->mpt_softerror = ETIMEDOUT;
      DTRACE_MPTCP1(error, struct mptcb *, mp_tp);
    } else {
      resched_timer = 1;
...
uint32_t mptcp_timer(struct mppcbinfo *mppi) {
...
void mptcp_start_timer(struct mptses *mpte, int timer_type) {
  struct timeval now;
...
void mptcp_cancel_timer(struct mptcb *mp_tp, int timer_type) {
...
void mptcp_cancel_all_timers(struct mptcb *mp_tp) {
...
static void mptcp_urgency_timer(void *param0, __unused void *param1) {
  ...
  os_log(mptcp_log_handle, "%s - %lx: timer at %llu now %llu usecount %u\n",
         ...
         mpte->mpte_time_target, time_now, mp_so->so_usecount);
...
static void mptcp_urgency_stop(void *param0, __unused void *param1) {
  struct mptses *mpte = (struct mptses *)param0;
  struct socket *mp_so = mptetoso(mpte);
  ...
  os_log(mptcp_log_handle, "%s - %lx: usecount %u\n", __func__,
         (unsigned long)VM_KERNEL_ADDRPERM(mpte), mp_so->so_usecount);
...
void mptcp_init_urgency_timer(struct mptses *mpte) {
...
void mptcp_set_urgency_timer(struct mptses *mpte) {
  struct socket *mp_so = mptetoso(mpte);
  uint64_t time_now = 0;
  boolean_t ret = FALSE;
...
exit_log:
  os_log(mptcp_log_handle,
         ...
         (unsigned long)VM_KERNEL_ADDRPERM(mpte), mpte->mpte_time_target,
         time_now, mp_so->so_usecount, ret);
}
...
static int mptcp_cancel_urgency_timer(struct mptses *mpte) {
  ...
  os_log(mptcp_log_handle,
         ...
         (unsigned long)VM_KERNEL_ADDRPERM(mpte), mp_so->so_usecount, ret);
