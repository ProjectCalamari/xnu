...
#define MAC_VNODE_CHECK_DYLD_SIM                                               \
  0x1 /* tells the MAC framework that dyld-sim is being loaded */

#define CLEAR_LV_ENTITLEMENT                                                   \
  "com.apple.private.security.clear-library-validation"
#define OVERRIDE_PLUGIN_HOST_ENTITLEMENT                                       \
...
#define CS_OPS_CLEARPLATFORM                                                   \
  13                       /* clear platform binary status (DEVELOPMENT-only) */
...
int csblob_register_profile_uuid(struct cs_blob *, const uuid_t, void *,
                                 vm_size_t);
...
int csblob_get_der_entitlements(struct cs_blob *, const CS_GenericBlob **,
                                size_t *);
#define XNU_HAS_GET_DER_ENTITLEMENTS_UNSAFE 1
const CS_GenericBlob *csblob_get_der_entitlements_unsafe(struct cs_blob *);

const CS_GenericBlob *csblob_find_blob(struct cs_blob *, uint32_t, uint32_t);
const CS_GenericBlob *csblob_find_blob_bytes(const uint8_t *, size_t, uint32_t,
                                             uint32_t);
void *csblob_entitlements_dictionary_copy(struct cs_blob *csblob);
void csblob_entitlements_dictionary_set(struct cs_blob *csblob,
                                        void *entitlements);
...
const uint8_t *csfg_get_supplement_linkage_cdhash(struct fileglob *, uint64_t,
                                                  size_t *);
...
unsigned int csfg_get_supplement_validation_category(struct fileglob *fg,
                                                     uint64_t offset);
