 ...
 * At process fork, free user pages copied from parent, and do not allocate
 * kernel memory.
 *
...
#define DTRACE_PTSS_ENTRIES_PER_PAGE                                           \
  (PAGE_MAX_SIZE / DTRACE_PTSS_SCRATCH_SPACE_PER_THREAD)
...
struct dtrace_ptss_page_entry *
dtrace_ptss_claim_entry(struct proc *p); /* sprlock not held */
struct dtrace_ptss_page_entry *
dtrace_ptss_claim_entry_locked(struct proc *p); /* sprlock held */
