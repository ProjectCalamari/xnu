...
extern int change_fdguard_np(int fd, const guardid_t *guard, u_int guardflags,
                             ...
                             int *fdflagsp);
extern ssize_t guarded_write_np(int fd, const guardid_t *guard, const void *buf,
                                size_t nbyte);
...
#define VNG_ALL                                                                \
  (VNG_RENAME_TO | VNG_RENAME_FROM | VNG_UNLINK | VNG_LINK | VNG_WRITE_OTHER | \
   VNG_TRUNC_OTHER | VNG_EXCHDATA | VNG_PERMISSIONS)
