...
#define DECMPFS_XATTR_NAME                                                     \
  "com.apple.decmpfs" /* extended attribute to use for decmpfs */
...
#define DECMPFS_PKG_CHLD_COUNT(x)                                              \
  ((uint32_t)(((x).value >> DECMPFS_PKG_CHLD_COUNT_SHIFT) &                    \
              DECMPFS_PKG_COUNT_MASK))
#define DECMPFS_PKG_VALUE_FROM_SIZE_COUNT(size, count)                         \
  (((size) & DECMPFS_PKG_SIZE_MASK) |                                          \
   ((uint64_t)(count) << DECMPFS_PKG_CHLD_COUNT_SHIFT))
...
static inline bool decmpfs_type_is_dataless(uint32_t cmp_type) {
  return cmp_type == DATALESS_CMPFS_TYPE || cmp_type == DATALESS_PKG_CMPFS_TYPE;
}

typedef struct __attribute__((packed)) {
  /* this structure represents the xattr on disk; the fields below are
   * little-endian */
...
typedef struct __attribute__((packed)) {
  /* this structure represents the xattr in memory; the fields below are
   * host-endian */
  uint32_t attr_size;
  uint32_t compression_magic;
  ...
  union {
    /*
     * although uncompressed_size remains available for backward-compatibility
     * reasons the uncompressed size and nchildren should be accessed using the
...
decmpfs_get_uncompressed_size(const decmpfs_header *hdr) {
  if (hdr->compression_magic == DECMPFS_MAGIC &&
      hdr->compression_type == DATALESS_PKG_CMPFS_TYPE) {
...
struct decmpfs_cnode {
  uint8_t cmp_state;
  uint8_t
      cmp_minimal_xattr; /* if non-zero, this file's com.apple.decmpfs xattr
                            contained only the minimal decmpfs_disk_header */
  uint32_t cmp_type;
  uint32_t lockcount;
  void *lockowner; /* cnode's lock owner (if a thread is currently holding an
                      exclusive lock) */
  uint64_t uncompressed_size __attribute__((aligned(8)));
  uint64_t nchildren __attribute__((
      aligned(8))); /* for dataless directories (incl. packages) */
  uint64_t total_size __attribute__((
      aligned(8))); /* for dataless directories (incl. packages) */
...
enum {
  ...
  FILE_IS_CONVERTING =
      3 /* file is converting from compressed to decompressed */
};
...
int decmpfs_decompress_file(
    vnode_t vp, decmpfs_cnode *cp, off_t toSize, int truncate_okay,
    int skiplock); /* if toSize == -1, decompress the entire file */
...
/* the following two routines will set *is_compressed to 0 if the file was
 * converted from compressed to decompressed before data could be fetched from
 * the decompressor */
...
typedef int (*decmpfs_validate_compressed_file_func)(vnode_t vp,
                                                     vfs_context_t ctx,
                                                     decmpfs_header *hdr);
typedef void (*decmpfs_adjust_fetch_region_func)(vnode_t vp, vfs_context_t ctx,
                                                 decmpfs_header *hdr,
                                                 off_t *offset,
                                                 user_ssize_t *size);
typedef int (*decmpfs_fetch_uncompressed_data_func)(
    vnode_t vp, vfs_context_t ctx, decmpfs_header *hdr, off_t offset,
    user_ssize_t size, int nvec, decmpfs_vector *vec, uint64_t *bytes_read);
typedef int (*decmpfs_free_compressed_data_func)(vnode_t vp, vfs_context_t ctx,
                                                 decmpfs_header *hdr);
typedef uint64_t (*decmpfs_get_decompression_flags_func)(
    vnode_t vp, vfs_context_t ctx,
    decmpfs_header
        *hdr); // returns flags from the DECMPFS_FLAGS enumeration below
...
errno_t register_decmpfs_decompressor(uint32_t compression_type,
