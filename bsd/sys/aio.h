 ...
 * against file descriptor fd. The aiocbp argument points to the asynchronous
 * I/O control block for a particular request to be canceled.  If aiocbp is
 * NULL, then all outstanding cancelable asynchronous I/O requests against fd
 * shall be canceled.
 */
 ...
 * aiocbp argument. The error status for an asynchronous I/O operation is the
 * errno value that would be set by the corresponding read(), write(),  or
 * fsync() operation.  If the operation has not yet completed, then the error
 ...
 * the file descriptor aio_fildes member of the aiocb structure referenced by
 * the aiocbp argument and queued at the time of the call to aio_fsync() to the
 ...
 * asynchronous operation while it is proceeding.  When the request is queued,
 * the error status for the operation is [EINPROGRESS]. When all data has been
 * successfully transferred, the error status shall be reset to reflect the
 ...
 * the buffer pointed to by aiocbp->aio_buf.  The function call shall return
 * when the read request has been initiated or queued. The aiocbp value may be
 * used as an argument to aio_error() and aio_return() in order to determine the
 ...
 * the function call shall return without having initiated or queued the
 * request. The requested operation takes place at the absolute position in the
 * file as given by aio_offset, as if lseek() were called immediately prior to
 ...
 * status of a given asynchronous operation; thereafter, if the same aiocb
 * structure is used in a call to aio_return() or aio_error(), an error may be
 * returned. When the aiocb structure referred to by aiocbp is used to submit
 * another asynchronous operation, then aio_return() may be successfully used to
 ...
 * in the aiocblist correspond to completed asynchronous I/O operations (that
 * is, the error status for the operation is not equal to [EINPROGRESS]) at the
 ...
 */
int aio_suspend(const struct aiocb *const aiocblist[], int nent,
 ...
 * argument takes one of the values LIO_WAIT or LIO_NOWAIT and determines
 * whether the function returns when the I/O operations have been completed, or
 * as soon as the operations have been queued.  If the mode argument is
 * LIO_WAIT, the function shall wait until all I/O is complete and the sig
 * argument shall be ignored. If the mode argument is LIO_NOWAIT, the function
 * shall return immediately, and asynchronous notification shall occur,
 * according to the sig argument, when all the I/O operations complete.  If sig
 * is NULL, then no asynchronous notification shall occur.
 */
int lio_listio(int mode, struct aiocb *const aiocblist[], int nent,
