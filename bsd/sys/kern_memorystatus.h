...
#define JETSAM_PRIORITY_ENTITLED_MAX                                           \
  9 /* Entitled processes may use bands 1-9 for experimentation */
#define JETSAM_PRIORITY_IDLE_DEFERRED                                          \
  10 /* Keeping this around till all xnu_quick_tests can be moved away from    \
        it.*/
#define JETSAM_PRIORITY_AGING_BAND1 JETSAM_PRIORITY_IDLE_DEFERRED
#define JETSAM_PRIORITY_AGING_BAND1_STUCK                                      \
  15 /* Sysprocs stuck in JETSAM_PRIORITY_IDLE_DEFERRED will be moved to this  \
        band */
...
/*
 * Used to show that a process is so high priority it is not processed by jetsam
 * at all. This is set on launchd and kernel_task.
 */
...
** Allocate a buffer of the size returned by the sysctl, cast to a
* memorystatus_snapshot_t *
*/
...
typedef struct jetsam_snapshot_entry {
  ...
  uint32_t fds;
  memorystatus_freeze_skip_reason_t
      jse_freeze_skip_reason; /* why wasn't this process frozen? */
  uint8_t uuid[16];
  uint64_t user_data;
  uint64_t killed;
  uint64_t pages;
  ...
  uint64_t
      jse_killtime; /* absolute time when jetsam chooses to kill a process */
  uint64_t jse_idle_delta;          /* time spent in idle band */
  uint64_t jse_coalition_jetsam_id; /* we only expose coalition id for
...
typedef struct jetsam_snapshot {
  ...
  memorystatus_kernel_stats_t
      stats; /* system stat when snapshot is initialized */
  size_t entry_count;
...
typedef enum {
  ...
  kMemorystatusKilledSustainedPressure =
      JETSAM_REASON_MEMORY_SUSTAINED_PRESSURE,
  kMemorystatusKilledVMPageoutStarvation =
      JETSAM_REASON_MEMORY_VMPAGEOUT_STARVATION,
...
#define JETSAM_REASON_MEMORY_VMTHRASHING                                       \
  JETSAM_REASON_MEMORY_VMCOMPRESSOR_THRASHING
...
#define MEMORYSTATUS_CMD_SET_JETSAM_HIGH_WATER_MARK                            \
  5 /* Set active memory limit = inactive memory limit, both non-fatal	*/
#define MEMORYSTATUS_CMD_SET_JETSAM_TASK_LIMIT                                 \
  6 /* Set active memory limit = inactive memory limit, both fatal	*/
#define MEMORYSTATUS_CMD_SET_MEMLIMIT_PROPERTIES                               \
  7 /* Set memory limits plus attributes independently			*/
#define MEMORYSTATUS_CMD_GET_MEMLIMIT_PROPERTIES                               \
  8 /* Get memory limits plus attributes */
#define MEMORYSTATUS_CMD_PRIVILEGED_LISTENER_ENABLE                            \
  9 /* Set the task's status as a privileged listener w.r.t memory             \
       notifications  */
#define MEMORYSTATUS_CMD_PRIVILEGED_LISTENER_DISABLE                           \
  10 /* Reset the task's status as a privileged listener w.r.t memory          \
        notifications  */
#define MEMORYSTATUS_CMD_AGGRESSIVE_JETSAM_LENIENT_MODE_ENABLE                 \
  11 /* Enable the 'lenient' mode for aggressive jetsam. See comments in       \
        kern_memorystatus.c near the top. */
#define MEMORYSTATUS_CMD_AGGRESSIVE_JETSAM_LENIENT_MODE_DISABLE                \
  12 /* Disable the 'lenient' mode for aggressive jetsam. */
#define MEMORYSTATUS_CMD_GET_MEMLIMIT_EXCESS                                   \
  13 /* Compute how much a process's phys_footprint exceeds inactive memory    \
        limit */
#define MEMORYSTATUS_CMD_ELEVATED_INACTIVEJETSAMPRIORITY_ENABLE                \
  14 /* Set the inactive jetsam band for a process to                          \
        JETSAM_PRIORITY_ELEVATED_INACTIVE */
#define MEMORYSTATUS_CMD_ELEVATED_INACTIVEJETSAMPRIORITY_DISABLE               \
  15 /* Reset the inactive jetsam band for a process to the default band (0)*/
#define MEMORYSTATUS_CMD_SET_PROCESS_IS_MANAGED                                \
  16 /* (Re-)Set state on a process that marks it as (un-)managed by a system  \
        entity e.g. assertiond */
#define MEMORYSTATUS_CMD_GET_PROCESS_IS_MANAGED                                \
  17 /* Return the 'managed' status of a process */
#define MEMORYSTATUS_CMD_SET_PROCESS_IS_FREEZABLE                              \
  18 /* Is the process eligible for freezing? Apps and extensions can pass in  \
      * FALSE to opt out of freezing, i.e., if they would prefer being                                                    \
      * jetsam'ed in the idle band to being frozen in an elevated band. */
#define MEMORYSTATUS_CMD_GET_PROCESS_IS_FREEZABLE                              \
  19 /* Return the freezable state of a process. */

#define MEMORYSTATUS_CMD_FREEZER_CONTROL 20

#define MEMORYSTATUS_CMD_GET_AGGRESSIVE_JETSAM_LENIENT_MODE                    \
  21 /* Query if the lenient mode for aggressive jetsam is enabled. */

#define MEMORYSTATUS_CMD_INCREASE_JETSAM_TASK_LIMIT                            \
  22 /* Used by DYLD to increase the jetsam active and inactive limits, when   \
        using roots */

#if PRIVATE
#define MEMORYSTATUS_CMD_SET_TESTING_PID                                       \
  23   /* Used by unit tests in the development kernel only. */
#endif /* PRIVATE */

#define MEMORYSTATUS_CMD_GET_PROCESS_IS_FROZEN                                 \
  24 /* Check if the process is frozen. */

#define MEMORYSTATUS_CMD_MARK_PROCESS_COALITION_SWAPPABLE                      \
  25 /* Set the coalition led by this process as swappable. This is a one-way  \
        transition. Swappable coalitions can never be made non-swappable. */
#define MEMORYSTATUS_CMD_GET_PROCESS_COALITION_IS_SWAPPABLE                    \
  26 /* Get the swappable status for this process' coalition. */

#define MEMORYSTATUS_CMD_CONVERT_MEMLIMIT_MB                                   \
  28 /* Given a memlimit value (which may be 0 or -1), convert it to an actual \
        limit in megabytes. */

#define MEMORYSTATUS_CMD_SET_DIAG_LIMIT                                        \
  30 /* Set the diagnostics memory limit */
#define MEMORYSTATUS_CMD_GET_DIAG_LIMIT                                        \
  31 /* Get the diagnostics memory limit */

#define MEMORYSTATUS_CMD_GET_JETSAM_ZPRINT_NAMES                               \
  32 /* Get jetsam snapshot zprint names array */
#define MEMORYSTATUS_CMD_GET_JETSAM_ZPRINT_INFO                                \
  33 /* Get jetsam snapshot zprint zone info */
#define MEMORYSTATUS_CMD_GET_JETSAM_ZPRINT_MEMINFO                             \
  34 /* Get jetsam snapshot zprint wired memory info */
#define MEMORYSTATUS_CMD_REARM_MEMLIMIT                                        \
  36 /* Re-arm memory limit (EXC_RESOURCE) */

#define MEMORYSTATUS_CMD_GET_PRIORITY_LIST_V2                                  \
  35 /* Get priority list with v2 struct */

#define MEMORYSTATUS_CMD_GET_KILL_COUNTS 37 /* Get kill counts */

#define MEMORYSTATUS_CMD_GET_CONCLAVE_LIMIT                                    \
  38 /* Get the conclave memory limit */
...
__enum_decl(
    memorystatus_jetsam_sort_order_t, int,
    {
        ...
        JETSAM_SORT_FOOTPRINT_NOCOAL =
            0x3, /* Sort by footprint ignoring coalitions */
    });
...
#define MEMORYSTATUS_FLAGS_SNAPSHOT_ON_DEMAND                                  \
  0x1 /* A populated snapshot buffer is returned on demand */
#define MEMORYSTATUS_FLAGS_SNAPSHOT_AT_BOOT                                    \
  0x2 /* Returns a snapshot with memstats collected at boot */
#define MEMORYSTATUS_FLAGS_SNAPSHOT_COPY                                       \
  0x4 /* No longer supported. Used to return the previously populated snapshot \
         created by the system */
#define MEMORYSTATUS_FLAGS_GRP_SET_PRIORITY                                    \
  0x8 /* Set jetsam priorities for a group of pids */
#define MEMORYSTATUS_FLAGS_GRP_SET_PROBABILITY                                 \
  0x10 /* Set probability of use for a group of processes */

#if PRIVATE
#define MEMORYSTATUS_FLAGS_SET_TESTING_PID                                     \
  0x20 /* Only used by xnu unit tests. */
#define MEMORYSTATUS_FLAGS_UNSET_TESTING_PID                                   \
  0x40 /* Only used by xnu unit tests. */
#define MEMORYSTATUS_FLAGS_SET_IMP_TESTING_PID                                 \
  0x80 /* Only used by xnu tests. */
#endif /* PRIVATE */

#define MEMORYSTATUS_FLAGS_SNAPSHOT_FREEZER                                    \
  0x80 /* A snapshot buffer containing app kills since last consumption */

#define MEMORYSTATUS_FLAGS_GRP_SET_FREEZE_PRIORITY                             \
  0x100 /* Set a new ordered list of freeze candidates */
#define MEMORYSTATUS_FLAGS_GRP_SET_DEMOTE_PRIORITY                             \
  0x200 /* Set a new ordered list of demote candidates */
...
typedef struct memorystatus_memlimit_properties {
  int32_t
      memlimit_active; /* jetsam memory limit (in MB) when process is active */
  uint32_t memlimit_active_attr;
  int32_t memlimit_inactive; /* jetsam memory limit (in MB) when process is
                                inactive */
...
typedef struct memorystatus_memlimit_properties2 {
  ...
  uint32_t memlimit_increase; /* jetsam memory limit increase (in MB) for active
                                 and inactive states */
  uint32_t memlimit_increase_bytes; /* bytes used to determine the jetsam memory
                                       limit increase, for active and inactive
                                       states */
} memorystatus_memlimit_properties2_t;

#define MEMORYSTATUS_MEMLIMIT_ATTR_FATAL                                       \
  0x1 /* if set, exceeding the memlimit is fatal */
/*
...
typedef struct memorystatus_diag_memlimit_properties {
  uint64_t memlimit; /* max limit for memory usage before an exception is thrown
                        expressed in bytes, but rounded internally to Mbytes */
  bool
      threshold_enabled; /* Current status of the diagnostics threshold, only
                            for get operations, not consider in set threshold */
...
__options_closed_decl(memstat_get_kill_counts_options_t, int,
                      {
                          MEMORYSTATUS_GET_KILL_COUNTS_CLEAR =
                              0x1 /* Flag to clear kill counts list */
                      });

#ifdef BSD_KERNEL_PRIVATE

/*
 * A process will be killed immediately if it crosses a memory limit marked as
 * fatal. Fatal limit types are the
 *	- default system-wide task limit
 *	- per-task custom memory limit
 *
 * A process with a non-fatal memory limit can exceed that limit, but becomes an
 * early candidate for jetsam when the device is under memory pressure.
...
#define P_MEMSTAT_SUSPENDED                                                    \
  0x00000001 /* Process is suspended and likely in the IDLE band */
#define P_MEMSTAT_FROZEN                                                       \
  0x00000002 /* Process has some state on disk. It should be suspended */
#define P_MEMSTAT_FREEZE_DISABLED                                              \
  0x00000004 /* Process isn't freeze-eligible and will not be frozen */
#define P_MEMSTAT_ERROR                                                        \
  0x00000008 /* Process couldn't be jetsammed for some reason. Transient state \
                so jetsam can skip it next time it sees it */
#define P_MEMSTAT_LOCKED                                                       \
  0x00000010 /* Process is being actively worked on behind the proc_list_lock  \
              */
#define P_MEMSTAT_TERMINATED 0x00000020 /* Process is exiting */
#define P_MEMSTAT_FREEZE_IGNORE                                                \
  0x00000040 /* Process was evaluated by freezer and will be ignored till the  \
                next time it goes active and does something */
#define P_MEMSTAT_PRIORITYUPDATED                                              \
  0x00000080 /* Process had its jetsam priority updated */
#define P_MEMSTAT_FOREGROUND                                                   \
  0x00000100 /* Process is in the FG jetsam band...unused??? */
#define P_MEMSTAT_REFREEZE_ELIGIBLE                                            \
  0x00000400 /* Process was once thawed i.e. its state was brought back from   \
                disk. It is now refreeze eligible.*/
#define P_MEMSTAT_MANAGED                                                      \
  0x00000800 /* Process is managed by RunningBoard i.e. is either application  \
                or extension */
#define P_MEMSTAT_INTERNAL                                                     \
  0x00001000 /* Process is a system-critical-not-be-jetsammed process i.e.     \
                launchd */
#define P_MEMSTAT_FATAL_MEMLIMIT                                               \
  0x00002000 /* current fatal state of the process's memlimit */
#define P_MEMSTAT_MEMLIMIT_ACTIVE_FATAL                                        \
  0x00004000 /* if set, exceeding limit is fatal when the process is active */
#define P_MEMSTAT_MEMLIMIT_INACTIVE_FATAL                                      \
  0x00008000 /* if set, exceeding limit is fatal when the process is inactive  \
              */
#define P_MEMSTAT_USE_ELEVATED_INACTIVE_BAND                                   \
  0x00010000 /* if set, the process will go into this band & stay there when   \
              * in the background instead of the aging bands and/or the IDLE                                            \
              * band. */
#define P_MEMSTAT_PRIORITY_ASSERTION                                           \
  0x00020000 /* jetsam priority is being driven by an assertion */
#define P_MEMSTAT_FREEZE_CONSIDERED                                            \
  0x00040000 /* This process has been considered for the freezer. */
#define P_MEMSTAT_SKIP                                                         \
  0x00080000 /* Process is temporarily ineligible for memory pressure kills.   \
                Used only on development & debug kernels to make corpses of    \
                buggy processes */
#define P_MEMSTAT_FROZEN_XPC_SERVICE                                           \
  0x00100000 /* Process is an XPC service. Only used for freezer telemetry. */
#define P_MEMSTAT_FROZEN_FOCAL_THAW                                            \
  0x00200000 /* Process has been thawed while focal in the current freezer     \
                interval. Only used for freezer telemetry. */
#define P_MEMSTAT_TEST_IMP_ASSERTION                                           \
  0x00400000 /* Used for testing to pretend a process has an importance        \
                assertion. */
 ...
 *
 * App -- almost always managed by a system process. Always have dirty tracking
 * OFF. Can include extensions too. System Processes -- not managed by anybody.
 * Always have dirty tracking ON. Can include extensions (here) too.
 */
#define isApp(p)                                                               \
...
extern memorystatus_internal_probabilities_t
...
__options_closed_decl(
    memstat_priority_options_t, uint8_t,
    {
...
extern int memorystatus_set_memlimits(proc_t p, int32_t active_limit,
 ...
 * @return: 0 if successful. EAGAIN if the process can't be removed right now
 * (because it's being frozen) or ESRCH.
 */
...
void memorystatus_on_ledger_footprint_exceeded(int warning,
                                               boolean_t memlimit_is_active,
                                               boolean_t memlimit_is_fatal);
 ...
 * Used by vm_pageout at boot if the swap volume is too small to support app
 * swap. Returns true iff app swap is now disabled. On development or debug
 * kernels, app swap can be enabled via a boot-arg and in that case can not be
 * disabled.
 */
