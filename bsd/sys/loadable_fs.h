...
#define FSUC_MOUNT_FORCE                                                       \
  'M' /* like FSUC_MOUNT, but proceed even on                                  \
       * error. */
/*
 * Return codes from .util program
 */
#define FSUR_RECOGNIZED                                                        \
  (-1)                         /* response to FSUC_PROBE; implies that         \
                                * a mount is possible */
...
#define FSUR_IO_UNCLEAN                                                        \
  (-5)                    /* mount failed, file system not clean               \
                           */
#define FSUR_INVAL (-6)   /* invalid argument */
#define FSUR_LOADERR (-7) /* kern_loader error */
#define FSUR_INITRECOGNIZED                                                    \
  (-8) /* response to FSUC_PROBE or                                            \
