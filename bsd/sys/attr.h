...
#define ATTRIBUTE_SET_INIT(a)                                                  \
  do {                                                                         \
    (a)->commonattr = (a)->volattr = (a)->dirattr = (a)->fileattr =            \
        (a)->forkattr = 0;                                                     \
  } while (0)
...
/* XXX PPD This is derived from HFSVolumePriv.h and should perhaps be referenced
 * from there? */
 ...
 */
#define ATTR_MAX_BUFFER_LONGPATHS                                              \
  (ATTR_MAX_BUFFER - MAXPATHLEN + MAXLONGPATHLEN)
 ...
 *
 * VOL_CAP_FMT_CLONE_MAPPING: When set, this volume supports full clone
 * tracking. See ATTR_CMNEXT_CLONE_REFCNT and ATTR_CMNEXT_CLONEID for more
 * details. Other features like extended directory statistics, for fast
 * directory sizing, and attribution tags may be supported as well. See
