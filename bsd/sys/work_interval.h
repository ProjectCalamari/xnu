 ...
 * Caution (!):
 *
 *   Because the information supplied via this facility directly influences
 * power management decisions, clients should strive to be as accurate as
 * possible. Failure to do so will adversely impact system power and
 * performance.
 *
 ...
 *
 * Another advanced feature for work intervals is the ability to defer the
 * finish calls for the work interval until all auto-joined threads for the work
 * interval have blocked or terminated. This feature is enabled via an optional
 * flag WORK_INTERVAL_FLAG_ENABLE_DEFERRED_FINISH and is valid only if the work
 * interval is configured with the WORK_INTERVAL_FLAG_ENABLE_AUTO_JOIN flag as
 * well. The deferred finish mechanism allows the work interval to defer the
 * finish call for the work interval until all auto-join threads have
 * blocked/terminated (and have therefore un-joined the work interval) or one of
 * the work interval threads calls start for the next frame. The deferred finish
 * works only for workloads that have no temporal overlap across frames i.e.
 * previous frame has to finish before next frame can start. This feature should
 * be used with caution since auto-joined threads would delay finish calls to
 * the performance controller which could lead to poor performance and battery
 * life.
 */

/* Flags to be passed with work_interval_create() */

/* If interval is joinable, create no longer implicitly joins, you must use
 * work_interval_join */
#define WORK_INTERVAL_FLAG_JOINABLE (0x1)
/* Only threads that join the group are measured together, otherwise the group
 * is the creator's home group */
...
/* [Advanced Flag] Read section on "Work Interval Auto Join Support" above for
 * details */
#define WORK_INTERVAL_FLAG_ENABLE_AUTO_JOIN (0x8)
/* [Advanced Flag] Read section on "Work Interval Deferred Finish Support" above
 * for details */
...
/* Specifies that the work interval requests the system to provide just enough
 * performance to be able to finish at the provided deadline and no sooner. */
#define WORK_INTERVAL_FLAG_FINISH_AT_DEADLINE (0x40)

/* Internal-only flag: workinterval will have the SET_WORKLOAD_ID operation
 * called on it immediately after creation. */
#define WORK_INTERVAL_FLAG_HAS_WORKLOAD_ID (0x80)

/* Internal-only flag: Telemetry data will be aggregated from threads while they
 * are joined to the work interval. */
 ...
 *
 * Requires the 'com.apple.private.kernel.work-interval' entitlement
 * (PRIV_WORK_INTERVAL)
 *
 * Note that joining a work interval supersedes automatic thread group
 * management via vouchers
 */
...
/*
 * Notify the power management subsystem that the work for a current interval
 * has completed
 *
 * Only the process which created the work interval may notify
 */
int work_interval_notify(work_interval_t interval_handle, uint64_t start,
                         uint64_t finish, uint64_t deadline,
                         uint64_t next_start, uint32_t flags);
 ...
 * Only the process which created the work interval may notify
 */
int work_interval_notify_simple(work_interval_t interval_handle, uint64_t start,
 ...
 * mach_port_deallocate Only allowed if interval is joinable, otherwise returns
 * ENOTSUP
 *
 * Supersedes automatic thread group management via vouchers
 */
 ...
 *
 * Does NOT consume Mach send right, must deallocate with mach_port_deallocate
 * after using It's safe to deallocate the right after joining, the thread will
 * stay joined
 *
 ...
 * Supersedes automatic thread group management via vouchers
 *
 * If the underlying work interval object is terminated then this may return
 * ENOENT <rdar://problem/31819320>
 */
...
#define WORK_INTERVAL_OPERATION_NOTIFY                                         \
  0x00000003 /* arg is a work_interval_notification_t */
#define WORK_INTERVAL_OPERATION_CREATE2                                        \
  0x00000004 /* arg is a work_interval_create_params */
...
#define WORK_INTERVAL_OPERATION_SET_NAME                                       \
  0x0000000a /* arg is name string (char[WORK_INTERVAL_NAME_MAX])*/
#define WORK_INTERVAL_OPERATION_SET_WORKLOAD_ID                                \
  0x0000000b /* arg is a work_interval_workload_id_params */
...
/* Work interval is allowed to provide complexity values per frame as part of
 * {start, update, finish} calls */
#define WORK_INTERVAL_WORKLOAD_ID_COMPLEXITY_ALLOWED (1u << 3)

/* Flags allowed to be passed in from userspace as part of
...
struct work_interval_workload_id_params {
  ...
  uint64_t wlidp_name;            /* in param (pointer to
                                     char[WORK_INTERVAL_WORKLOAD_ID_NAME_MAX])*/
  uint64_t wlidp_syscall_mask[2]; /* out param (needs to fit
                                     MACH_TRAP_TABLE_COUNT + nsysent bits) */
};
