...
#define KAUTH_NTSID_SIZE(_s)                                                   \
  (KAUTH_NTSID_HDRSIZE + ((_s)->sid_authcount * sizeof(u_int32_t)))
 ...
 * for the duration of a MACF hook.  If a copy of this pointer has to be
 ...
 * The caller must call kauth_cred_unref() to dispose of the returned value.
 */
extern kauth_cred_t kauth_cred_proc_ref_for_pidversion(pid_t pid,
                                                       uint32_t version);
 ...
 */
extern kauth_cred_t kauth_cred_label_update(kauth_cred_t cred,
                                            struct label *label);
...
extern int kauth_cred_ismember_gid(kauth_cred_t _cred, gid_t _gid,
                                   int *_resultp);
extern int kauth_cred_ismember_guid(kauth_cred_t _cred, guid_t *_guidp,
                                    int *_resultp);
...
extern int posix_cred_access(kauth_cred_t cred, id_t object_uid,
                             id_t object_gid, mode_t object_mode,
                             mode_t mode_req);

extern int cantrace(proc_t cur_procp, kauth_cred_t creds, proc_t traced_procp,
                    int *errp);
 ...
 */
extern kauth_cred_t kauth_cred_derive(kauth_cred_t cred,
                                      kauth_cred_derive_t fn);

__enum_decl(
    proc_settoken_t, uint32_t,
    {
 ...
 */
extern bool kauth_cred_proc_update(proc_t p, proc_settoken_t action,
                                   kauth_cred_derive_t fn);

extern bool kauth_cred_model_setresuid(kauth_cred_t model, uid_t ruid,
                                       uid_t euid, uid_t svuid, uid_t gmuid);
extern bool kauth_cred_model_setresgid(kauth_cred_t model, gid_t rgid,
                                       gid_t egid, gid_t svgid);
extern bool kauth_cred_model_setuidgid(kauth_cred_t model, uid_t uid,
                                       gid_t gid);
extern bool kauth_cred_model_setgroups(kauth_cred_t model, gid_t *groups,
...
#ifdef CONFIG_MACF
extern void kauth_proc_label_update_execve(
    struct proc *p, struct vfs_context *ctx, struct vnode *vp, off_t offset,
    struct vnode *scriptvp, struct label *scriptlabel, struct label *execlabel,
    unsigned int *csflags, void *psattr, int *disjoint, int *update_return);
#endif
extern int kauth_cred_gid_subset(kauth_cred_t _cred1, kauth_cred_t _cred2,
                                 int *_resultp);
 ...
 */
#define KAUTH_ACL_SIZE(c)                                                      \
  (__offsetof(struct kauth_acl, acl_ace) +                                     \
   ((u_int32_t)(c) != KAUTH_FILESEC_NOACL ? ((c) * sizeof(struct kauth_ace))   \
                                          : 0))
...
#define KAUTH_FILESEC_SIZE(c)                                                  \
  (__offsetof(struct kauth_filesec, fsec_acl) +                                \
   __offsetof(struct kauth_acl, acl_ace) + (c) * sizeof(struct kauth_ace))
#define KAUTH_FILESEC_COPYSIZE(p)                                              \
  KAUTH_FILESEC_SIZE(((p)->fsec_entrycount == KAUTH_FILESEC_NOACL)             \
                         ? 0                                                   \
                         : (p)->fsec_entrycount)
#define KAUTH_FILESEC_COUNT(s)                                                 \
  (((s) - KAUTH_FILESEC_SIZE(0)) / sizeof(struct kauth_ace))
#define KAUTH_FILESEC_VALID(s)                                                 \
  ((s) >= KAUTH_FILESEC_SIZE(0) &&                                             \
   (((s) - KAUTH_FILESEC_SIZE(0)) % sizeof(struct kauth_ace)) == 0)
...
typedef int (*kauth_scope_callback_t)(kauth_cred_t _credential, void *_idata,
                                      kauth_action_t _action, uintptr_t _arg0,
                                      uintptr_t _arg1, uintptr_t _arg2,
                                      uintptr_t _arg3);
...
#define KAUTH_AEVAL_IN_GROUP                                                   \
  (1 << 1) /* authorizing operation for groupmember */
#define KAUTH_AEVAL_IN_GROUP_UNKNOWN                                           \
  (1 << 2) /* authorizing operation for unknown group membership */
...
extern kauth_scope_t kauth_register_scope(const char *_identifier,
                                          kauth_scope_callback_t _callback,
                                          void *_idata);
extern void kauth_deregister_scope(kauth_scope_t _scope);
__kpi_deprecated("Use EndpointSecurity instead") extern kauth_listener_t
    kauth_listen_scope(const char *_identifier,
                       kauth_scope_callback_t _callback, void *_idata);
__kpi_deprecated("Use EndpointSecurity "
                 "instead") extern void kauth_unlisten_scope(
    kauth_listener_t _scope);
extern int kauth_authorize_action(kauth_scope_t _scope,
...
extern int kauth_authorize_process(kauth_cred_t _credential,
                                   kauth_action_t _action,
                                   struct proc *_process, uintptr_t _arg1,
                                   uintptr_t _arg2, uintptr_t _arg3);
...
extern int kauth_acl_inherit(vnode_t _dvp, kauth_acl_t _initial,
                             kauth_acl_t *_product, int _isdir,
                             vfs_context_t _ctx);

extern int kauth_authorize_allow(kauth_cred_t _credential, void *_idata,
                                 kauth_action_t _action, uintptr_t _arg0,
                                 uintptr_t _arg1, uintptr_t _arg2,
                                 uintptr_t _arg3);

extern int kauth_authorize_generic(kauth_cred_t credential,
                                   kauth_action_t action);

extern int kauth_authorize_fileop_has_listeners(void);

extern int kauth_authorize_fileop(kauth_cred_t _credential,
                                  kauth_action_t _action, uintptr_t _arg0,
                                  uintptr_t _arg1);
...
#define KAUTH_VNODE_GENERIC_READ_BITS                                          \
  (KAUTH_VNODE_READ_DATA | KAUTH_VNODE_READ_ATTRIBUTES |                       \
   KAUTH_VNODE_READ_EXTATTRIBUTES | KAUTH_VNODE_READ_SECURITY)

#define KAUTH_VNODE_GENERIC_WRITE_BITS                                         \
  (KAUTH_VNODE_WRITE_DATA | KAUTH_VNODE_APPEND_DATA | KAUTH_VNODE_DELETE |     \
   KAUTH_VNODE_DELETE_CHILD | KAUTH_VNODE_WRITE_ATTRIBUTES |                   \
   KAUTH_VNODE_WRITE_EXTATTRIBUTES | KAUTH_VNODE_WRITE_SECURITY)

#define KAUTH_VNODE_GENERIC_EXECUTE_BITS (KAUTH_VNODE_EXECUTE)

#define KAUTH_VNODE_GENERIC_ALL_BITS                                           \
  (KAUTH_VNODE_GENERIC_READ_BITS | KAUTH_VNODE_GENERIC_WRITE_BITS |            \
   KAUTH_VNODE_GENERIC_EXECUTE_BITS)

/*
 * Some sets of bits, defined here for convenience.
 */
#define KAUTH_VNODE_WRITE_RIGHTS                                               \
  (KAUTH_VNODE_ADD_FILE | KAUTH_VNODE_ADD_SUBDIRECTORY |                       \
   KAUTH_VNODE_DELETE_CHILD | KAUTH_VNODE_WRITE_DATA |                         \
   KAUTH_VNODE_APPEND_DATA | KAUTH_VNODE_DELETE |                              \
...
#define KAUTH_DEBUG_CTX(_c)                                                    \
  KAUTH_DEBUG("p = %p c = %p", _c->vc_proc, _c->vc_ucred)
...
#define VFS_DEBUG(ctx, vp, fmt, args...)                                       \
  do {                                                                         \
  } while (0)
#endif /* !0 */
#endif /* KERNEL */
