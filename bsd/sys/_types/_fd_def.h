...
int __darwin_check_fd_set_overflow(int, const void *, int)
    __API_AVAILABLE(macosx(10.16), ios(14.0), tvos(14.0), watchos(7.0));
#endif
__END_DECLS

#if !KERNEL
__header_always_inline int __darwin_check_fd_set(int _a, const void *_b) {
#ifdef __clang__
...
__header_always_inline int __darwin_fd_isset(int _fd, const struct fd_set *_p) {
  if (__darwin_check_fd_set(_fd, (const void *)_p)) {
    return _p->fds_bits[(unsigned long)_fd / __DARWIN_NFDBITS] &
           ((__int32_t)(((unsigned long)1)
                        << ((unsigned long)_fd % __DARWIN_NFDBITS)));
  }
...
__header_always_inline void __darwin_fd_set(int _fd, struct fd_set *const _p) {
  if (__darwin_check_fd_set(_fd, (const void *)_p)) {
    (_p->fds_bits[(unsigned long)_fd / __DARWIN_NFDBITS] |=
     ((__int32_t)(((unsigned long)1)
                  << ((unsigned long)_fd % __DARWIN_NFDBITS))));
  }
}
