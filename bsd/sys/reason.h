...
/* think twice about userspace debugging experience before using
 * PX_DEBUG_NO_HONOR */
...
int os_reason_alloc_buffer_noblock(os_reason_t cur_reason,
                                   uint32_t osr_bufsize);
...
void os_reason_set_description_data(os_reason_t cur_reason, uint32_t type,
                                    void *reason_data,
...
#define OS_REASON_FLAG_GENERATE_CRASH_REPORT                                   \
  0x2 /* Create a crash report - the default for userspace requests */
#define OS_REASON_FLAG_FROM_USERSPACE                                          \
  0x4 /* Reason created from a userspace syscall */
#define OS_REASON_FLAG_FAILED_DATA_COPYIN                                      \
  0x8 /* We failed to copyin data from userspace */
#define OS_REASON_FLAG_PAYLOAD_TRUNCATED                                       \
  0x10 /* The payload was truncated because it was longer than allowed */
#define OS_REASON_FLAG_BAD_PARAMS                                              \
  0x20 /* Invalid parameters were passed involved with creating this reason */
#define OS_REASON_FLAG_CONSISTENT_FAILURE                                      \
  0x40 /* Whatever caused this reason to be created will happen again */
#define OS_REASON_FLAG_ONE_TIME_FAILURE                                        \
  0x80 /* Whatever caused this reason to be created was a one time issue */
#define OS_REASON_FLAG_NO_CRASHED_TID                                          \
  0x100 /* Don't include the TID that processed the exit in the crash report   \
         */
#define OS_REASON_FLAG_ABORT                                                   \
  0x200 /* Reason created from abort_* rather than terminate_* */
#define OS_REASON_FLAG_SHAREDREGION_FAULT                                      \
  0x400 /* Fault happened within the shared cache region */
#define OS_REASON_FLAG_CAPTURE_LOGS                                            \
  0x800 /* The report generated for this reason should capture logs */
#define OS_REASON_FLAG_SECURITY_SENSITIVE                                      \
  0x1000 /* Mark as security sensitive for priority treatment */

/*
 * Set of flags that are allowed to be passed from userspace
 */
#define OS_REASON_FLAG_MASK_ALLOWED_FROM_USER                                  \
  (OS_REASON_FLAG_CONSISTENT_FAILURE | OS_REASON_FLAG_ONE_TIME_FAILURE |       \
   OS_REASON_FLAG_NO_CRASH_REPORT | OS_REASON_FLAG_ABORT |                     \
   OS_REASON_FLAG_CAPTURE_LOGS | OS_REASON_FLAG_SECURITY_SENSITIVE)

/*
 * Macros to encode the exit reason namespace and first 32 bits of code in
 * exception code which is used by Report Crash as a hint. It should be only
 * used as a hint since it loses higher 32 bits of exit reason code.
 */
 ...
 *
 * Inputs:              args->reason_namespace - OS_REASON namespace specified
 * for the reason args->reason_code - code in the specified namespace for the
 * reason args->reason_string - additional string formatted information about
 * the request args->reason_flags - options requested for how the process should
 * be terminated (see OS_REASON_FLAG_* above).
 *
 ...
 *                     specific information about why it is being terminated.
 * The payload pointer should point to structured data that can be interpreted
 * by the consumer of exit reason information.
 *
 * Inputs:              args->reason_namespace - OS_REASON namespace specified
 * for the reason args->reason_code - code in the specified namespace for the
 * reason args->payload - pointer to payload structure in user space
 *                      args->payload_size - length of payload buffer (this will
 * be truncated to EXIT_REASON_PAYLOAD_MAX_LEN) args->reason_string - additional
 * string formatted information about the request args->reason_flags - options
 * requested for how the process should be terminated (see OS_REASON_FLAG_*
 * above).
 *
 * Outputs:             Does not return.
 */
 ...
 *                      args->reason_namespace - OS_REASON namespace specified
 * for the reason args->reason_code - code in the specified namespace for the
 * reason args->reason_string - additional string formatted information about
 * the request args->reason_flags - options requested for how the process should
 * be terminated (see OS_REASON_FLAG_* above)
 *
 * Outputs:             returns -1 and sets errno to EINVAL if the PID requested
 * is the same as that of the calling process, invalid or the namespace provided
 * is invalid. returns -1 and sets errno to ESRCH if we couldn't find a live
 * process with the requested PID returns -1 and sets errno to EPERM if the
 * caller is not privileged enough to kill the process with the requested PID
 *                      returns 0 otherwise
 */
int terminate_with_reason(int pid, uint32_t reason_namespace,
                          uint64_t reason_code, const char *reason_string,
                          uint64_t reason_flags);

/*
 * terminate_with_payload: Used to terminate a specific process and pass along
 *                         specific information about why it is being
 * terminated. The payload pointer should point to structured data that can be
 * interpreted by the consumer of exit reason information.
 *
 * Inputs:              args->pid - the PID of the process to be terminated.
 *                      args->reason_namespace - OS_REASON namespace specified
 * for the reason args->reason_code - code in the specified namespace for the
 ...
 *
 * Outputs:             returns -1 and sets errno to EINVAL if the PID requested
 * is the same as that of the calling process, is invalid or the namespace
 * provided is invalid. returns -1 and sets errno to ESRCH if we couldn't find a
 * live process with the requested PID returns -1 and sets errno to EPERM if the
 * caller is not privileged enough to kill the process with the requested PID
 *                      returns 0 otherwise
 */
int terminate_with_payload(int pid, uint32_t reason_namespace,
                           uint64_t reason_code, void *payload,
