...
#define curproc                                                                \
  ((struct proc *)                                                             \
       current_proc()) /* Called from probe context, must blacklist */
...
typedef struct dtrace_cpu {
  ...
  boolean_t cpu_dtrace_invop_underway; /* DTrace gaurds against invalid op
                                          re-entrancy */
...
typedef struct cpu_core {
  ...
  uint8_t cpuc_pad[CPU_CACHE_COHERENCE_SIZE - sizeof(uint64_t) -
                   sizeof(lck_mtx_t) - sizeof(uint16_t) -
...
#define CPU_ON_INTR(cpup)                                                      \
  ml_at_interrupt_context() /* always invoked on current cpu */
...
/* Solaris' modctl structure, greatly simplified, shadowing parts of xnu kmod
 * structure. */
typedef struct modctl {
  ...
  /*
   * SDT probe data are directly stored in modctl. That's fine for XNU as modctl
   * serves different purpose than on Solaris and is allocated/freed as
   * required.
   */
...
#define MODCTL_FBT_PROVIDE_BLACKLISTED_PROBES                                  \
  0x200 // fbt provider must provide blacklisted probes
#define MODCTL_FBT_BLACKLISTED_PROBES_PROVIDED                                 \
  0x400                             // fbt blacklisted probes have been provided
...
#define MOD_HAS_USERSPACE_SYMBOLS(mod)                                         \
  (mod->mod_user_symbols) /* No point in duplicating state in the flags bits   \
                           */
#define MOD_FBT_PROBES_PROVIDED(mod)                                           \
  (mod->mod_flags & MODCTL_FBT_PROBES_PROVIDED)
#define MOD_FBT_INVALID(mod) (mod->mod_flags & MODCTL_FBT_INVALID)
#define MOD_SDT_PROBES_PROVIDED(mod)                                           \
  (mod->mod_flags & MODCTL_SDT_PROBES_PROVIDED)
#define MOD_SDT_INVALID(mod) (mod->mod_flags & MODCTL_SDT_INVALID)
#define MOD_HAS_UUID(mod) (mod->mod_flags & MODCTL_HAS_UUID)
#define MOD_FBT_BLACKLISTED_PROBES_PROVIDED(mod)                               \
  (mod->mod_flags & MODCTL_FBT_BLACKLISTED_PROBES_PROVIDED)
#define MOD_FBT_PROVIDE_BLACKLISTED_PROBES(mod)                                \
  (mod->mod_flags & MODCTL_FBT_PROVIDE_BLACKLISTED_PROBES)
#define MOD_IS_STATIC_KEXT(mod) (mod->mod_flags & MODCTL_IS_STATIC_KEXT)

/* Compound accessors */
#define MOD_FBT_BLACKLISTED_PROBES_DONE(mod)                                   \
  (MOD_FBT_BLACKLISTED_PROBES_PROVIDED(mod) ||                                 \
   !MOD_FBT_PROVIDE_BLACKLISTED_PROBES(mod))
#define MOD_FBT_DONE(mod)                                                      \
  ((MOD_FBT_PROBES_PROVIDED(mod) && MOD_FBT_BLACKLISTED_PROBES_DONE(mod)) ||   \
   MOD_FBT_INVALID(mod))
...
extern int ddi_create_minor_node(dev_info_t *, const char *, int, minor_t,
                                 const char *, int);
...
typedef struct _kthread kthread_t; /* For dtrace_vtime_switch(), dtrace_panicked
                                      and dtrace_errthread */
...
typedef unsigned int
    model_t; /* For dtrace_instr_size_isa() prototype in <sys/dtrace.h> */
...
extern vmem_t *vmem_create(const char *, void *, size_t, size_t, void *, void *,
                           vmem_t *, size_t, int);
...
/*
 * Safe counted string compare against a literal string. The sizeof()
 * intentionally counts the trailing NUL, and so ensures that all the characters
 * in the literal can participate in the comparison.
 */
