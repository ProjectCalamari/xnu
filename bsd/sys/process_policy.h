...
/* defns of scope */
#define PROC_POLICY_SCOPE_PROCESS                                              \
  1 /* the policy setting is for process wide effect */
#define PROC_POLICY_SCOPE_THREAD                                               \
  2 /* the policy setting is for thread inside a proc */
...
#define PROC_POLICY_ACTION_DENYINHERIT                                         \
  3 /* set for no inheritence of the specified policy */
#define PROC_POLICY_ACTION_DENYSELFSET                                         \
  4 /* set for the process to set its own policy */
#define PROC_POLICY_ACTION_ENABLE 5 /* enable policy and its actions */
#define PROC_POLICY_ACTION_DISABLE                                             \
  6 /* disable policy and its actions, also clears any actions that have       \
       already happened */
...
/* attribute values for disk hardware access, bit different as it should reflect
 * IOPOL_XXX */
...
#define PROC_POLICY_GPUACC_DEFAULT                                             \
  PROC_POLICY_GPUACC_FULLACCESS /*  default is complete access */
...
#define PROC_POLICY_NETACC_DEFAULT                                             \
  PROC_POLICY_NETACC_FULLACCESS /*  default is complete access */
...
#define PROC_POLICY_CPUACC_DEFAULT                                             \
  PROC_POLICY_CPUACC_FULLACCESS /*  default is access to all cpus */
...
/* attribute values for the resource usage and low resource - MUST match
 * corresponding task definitions */
...
#define PROC_POLICY_CPUMON_DEFAULTS                                            \
  0xFE /* Set default CPU usage monitor params */
...
typedef struct proc_policy_attribute {
  uint32_t
      ppattr_attribute;   /* the policy attribute to be modified or returned */
...
#define PROC_POLICY_APPLIFE_STATE                                              \
  1 /* sets the app to various lifecycle states */
#define PROC_POLICY_APPLIFE_DEVSTATUS                                          \
  2 /* notes the device in inactive or short/long term */
#define PROC_POLICY_APPLIFE_PIDBIND                                            \
  3    /* a thread is to be bound to another processes app state */
#endif /* !PROCESS_POLICY_OSX */

/* sub policies for PROC_POLICY_APPTYPE */
#define PROC_POLICY_APPTYPE_NONE 0 /* does nothing.. */
#define PROC_POLICY_APPTYPE_MODIFY                                             \
  1 /* sets the app to various lifecycle states */
#if !PROCESS_POLICY_OSX
#define PROC_POLICY_APPTYPE_THREADTHR                                          \
  2    /* notes the device in inactive or short/long term */
#endif /* !PROCESS_POLICY_OSX */
