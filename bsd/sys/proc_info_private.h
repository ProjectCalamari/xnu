...
struct proc_uniqidentifierinfo {
  ...
  int32_t p_orig_ppidversion; /* process's original parent's pid version,
                                 doesn't change if reparented */
...
_Static_assert(sizeof(struct proc_uniqidentifierinfo) == 56,
               "sizeof(struct proc_uniqidentifierinfo) == 56");
...
#define PROC_FLAG_IOS_IMPPROMOTION                                             \
  0x80000 /* Process is daemon which receives importance donation  */
...
#define PROC_FLAG_ROSETTA                                                      \
  0x2000000 /* Process is running translated under Rosetta */

/*
 * Security config.
 * These flags are currently folded inside pbi_flags, but per-feature policies
 * should likely move elsewhere.
 */
...
extern int proc_pidoriginatorpid_uuid(uuid_t uuid, uint32_t buffersize,
                                      pid_t *pid);
#endif
