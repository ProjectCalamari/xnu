...
extern unsigned int
    memorystatus_frozen_count; /* # of processes that are currently frozen. */
extern unsigned int
    memorystatus_frozen_count_webcontent; /* # of webcontent processes that are
                                             currently frozen. */
extern unsigned int
    memorystatus_frozen_count_xpc_service; /* # of xpc services that are
                                              currently frozen. */
...
extern unsigned int
    memorystatus_freeze_shared_mb_per_process_max; /* Max. MB allowed per
                                                      process to be
                                                      freezer-eligible. */
extern unsigned int
    memorystatus_freeze_private_shared_pages_ratio; /* Ratio of private:shared
                                                       pages for a process to be
                                                       freezer-eligible. */
extern unsigned int
    memorystatus_thaw_count; /* # of processes that have been thawed in the
                                current interval. */
extern unsigned int
    memorystatus_refreeze_eligible_count; /* # of processes currently thawed
                                             i.e. have state on disk & in-memory
                                           */
extern unsigned int memorystatus_freeze_max_candidate_band;
extern uint32_t
    memorystatus_freeze_current_interval; /* Monotonically increasing interval
                                             id. */
...
#endif
#define FREEZE_PAGES_MAX_DEFAULT                                               \
  (max_task_footprint_mb == 0 ? INT_MAX                                        \
                              : (max_task_footprint_mb << (20 - PAGE_SHIFT)))
#define FREEZE_PAGES_MAX_SWAP_ENABLED_DEFAULT                                  \
  (memorystatus_entitled_max_task_footprint_mb == 0                            \
       ? INT_MAX                                                               \
       : (memorystatus_entitled_max_task_footprint_mb << (20 - PAGE_SHIFT)))
...
#define FREEZE_DEGRADATION_BUDGET_THRESHOLD                                    \
  25 // degraded perf. when the daily budget left falls below this threshold
     // percentage
...
extern uint64_t
    memorystatus_freeze_budget_pages_remaining; // remaining # of pages that can
                                                // be frozen to disk
extern boolean_t
    memorystatus_freeze_degradation; // protected by the freezer mutex. Signals
                                     // we are in a degraded freeze mode.
...
struct memorystatus_freezer_stats_t {
  ...
  /*
   * # of times that we saw memorystatus_available_pages <=
   * memorystatus_freeze_threshold. Used to normalize skipped_full_count and
   * shared_mb_high_count.
   */
  ...
  /*
   * How many pages have not been sent to swap because they were in a shared
   * object? This is being used to gather telemtry so we can understand the
   * impact we'd have on our NAND budget if we did swap out these pages.
   */
  ...
  /* The number of proceses which have been frozen at least once in the current
   * interval. */
  uint64_t mfs_processes_frozen;
  /* The number of processes which have been thawed at least once in the current
   * interval. */
  uint64_t mfs_processes_thawed;

  /*
   * Telemetry shows that the majority of freezer usage is attributed to
   * webcontent so we track some specific webcontent telemetry here to get more
   * visibility.
   */

  /* The number of webcontent processes which have been frozen at least once in
   * the current interval. */
  uint64_t mfs_processes_frozen_webcontent;
  /* The number of webcontent processes which have been thawed at least once in
   * the current interval. */
  uint64_t mfs_processes_thawed_webcontent;

  /* The number of xpc service processes which have been frozen at least once in
   * the current interval. */
  ...
  /*
   * When we run out of budget, this records how much time is left in the
   * current interval. 0 means we have not run out of budget.
   */
  uint64_t mfs_budget_exhaustion_duration_remaining;

  /* The number of visible resumes in this interval. Mostly used to filter out
   * idle devices. */
