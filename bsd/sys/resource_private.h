 ...
 */
__enum_decl(
    thread_selfcounts_kind_t, uint32_t,
    {
        /*
         * Get the current thread's cycles and instructions -- may return
         * ENOTSUP on certain hardware.
         */
        THSC_CPI = 1,
        /*
         * Same as `THSC_CPI`, except fills in an array indexed by CPU
         * perf-level, with `sysctl hw.nperflevels` entries.
         */
        THSC_CPI_PER_PERF_LEVEL = 2,
        /*
         * Get the current thread's cycles, instructions, user time and system
         * time. Instructions and cycles may be left 0 on certain hardware.
         * System time may be accounted for by user time and left 0 on certain
         * hardware.
         */
        THSC_TIME_CPI = 3,
        /*
        ...
        /*
         * Get the current thread's cycles, instructions, times, and energy
         * usage.
         */
        THSC_TIME_ENERGY_CPI = 5,
        /*
         * Same as `THSC_TIME_ENERGY_CPI`, except fills in an array indexd by
         * CPU perf-level, with `sysctl hw.nperflevels` entries.
         */
...
__enum_decl(darwin_gpu_role_t, uint8_t,
            {
                ...
                /* Allowed to use GPU at Background priority, not visible to
                   user, prioritizes running most-efficiently */
                PRIO_DARWIN_GPU_BACKGROUND = 0x3,
                /* GPU used for non-visible-UI long-running progress-bar
                   workloads, balances between sustainable thermals and perf */
...
#define PRIO_DARWIN_ROLE_TAL_LAUNCH                                            \
  0x5 /* Throttled-launch (for OS X TAL resume) */
#define PRIO_DARWIN_ROLE_DARWIN_BG                                             \
  0x6 /* Throttled for running in the background */
#define PRIO_DARWIN_ROLE_USER_INIT                                             \
  0x7 /* Off-screen doing user-initiated work */
