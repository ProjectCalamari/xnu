...
#define SPARSE_PUSH_LIMIT                                                      \
  4 /* limit on number of concurrent sparse pushes outside of the cl_lockw */
...
struct cl_writebehind {
  ...
  struct cl_wextent
      cl_clusters[MAX_CLUSTERS]; /* packed write behind clusters */
};
...
struct cs_blob {
  ...
  int csb_hash_firstlevel_pageshift; /* First hash this many bytes, then hash
                                        the hashes together */
  ...
  const CS_GenericBlob
      *csb_entitlements_blob; /* raw blob, subrange of csb_mem_kaddr */
  const CS_GenericBlob
   ...
   * stack variable used to setup the blob.
   */
  void *XNU_PTRAUTH_SIGNED_PTR("cs_blob.csb_entitlements") csb_entitlements;

  unsigned int csb_reconstituted; /* signature has potentially been modified
                                     after validation */
...
struct ubc_info {
  ...
  vnode_t XNU_PTRAUTH_SIGNED_PTR(
      "ubc_info.ui_vnode") ui_vnode; /* vnode for this ubc_info */
  ...
  struct cs_blob *XNU_PTRAUTH_SIGNED_PTR(
      "ubc_info.cs_blob") cs_blobs; /* for CODE SIGNING */
#if CONFIG_SUPPLEMENTAL_SIGNATURES
  struct cs_blob *cs_blob_supplement; /* supplemental blob (note that there can
                                         only be one supplement) */
#endif
#if CHECK_CS_VALIDATION_BITMAP
  void *XNU_PTRAUTH_SIGNED_PTR("ubc_info.cs_valid_bitmap")
      cs_valid_bitmap; /* right now: used only for signed files on the read-only
                          root volume */
  uint64_t cs_valid_bitmap_size; /* Save original bitmap size in case the file
                                  * size changes. In the future, we may want to
                                  * reconsider changing the underlying bitmap to
                                  * reflect the new file size changes.
                                  */
...
int ubc_cs_blob_add(vnode_t, uint32_t, cpu_type_t, cpu_subtype_t, off_t,
                    vm_address_t *, vm_size_t, struct image_params *, int,
                    struct cs_blob **, cs_blob_add_flags_t);
#if CONFIG_SUPPLEMENTAL_SIGNATURES
int ubc_cs_blob_add_supplement(vnode_t, vnode_t, off_t, vm_address_t *,
                               vm_size_t, struct cs_blob **);
#endif
