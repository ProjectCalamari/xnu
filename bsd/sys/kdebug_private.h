...
static inline int kdbg_get_cpu(kd_buf *kp) { return (int)kp->cpuid; }
static inline void kdbg_set_timestamp(kd_buf *kp, uint64_t thetime) {
  kp->timestamp = thetime;
}
static inline uint64_t kdbg_get_timestamp(kd_buf *kp) { return kp->timestamp; }
static inline void kdbg_set_timestamp_and_cpu(kd_buf *kp, uint64_t thetime,
                                              int cpu) {
...
static inline void kdbg_set_timestamp_and_cpu(kd_buf *kp, uint64_t thetime,
                                              int cpu) {
  kp->timestamp =
      (thetime & KDBG_TIMESTAMP_MASK) | (((uint64_t)cpu) << KDBG_CPU_SHIFT);
}
#endif // !defined(__LP64__) && !defined(__arm64__)
...
// user space.
#define KDBG_USERFLAGS                                                         \
  (KDBG_NOWRAP | KDBG_CONTINUOUS_TIME | KDBG_DISABLE_COPROCS |                 \
   KDBG_MATCH_DISABLE)
...
#define ENTR_KDASSOCIATE(par_comp, par_opcode, par_act_id, sub_comp,           \
  ...
  do {                                                                         \
    ...
    ENTR_KDTRACEFUNC(KDBG_CODE(DBG_ENERGYTRACE, par_comp, par_opcode),         \
                     par_act_id, kEnTrModAssociate, sub_compcode, sub_act_id); \
  } while (0)
