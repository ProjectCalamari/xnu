...
/**
 * Pages which need to be locked down by the TrustedExecutionMonitor need to
 * made owned by TXM. This function can be used to go through each physical page
 * in a range and transfer it to the relevant TXM type.
 */
 ...
 * of the mappings and make them read-write again. This can only be done when
 * TXM has transferred control of the pages back to the kernel.
 */
void txm_reclaim_region(vm_address_t addr, vm_size_t size);

/**
 * Register an address space with the TrustedExecutionMonitor based on an
 * address space ID. This needs to be done AFTER the SPTM has made its call into
 * TXM for registering an address space ID otherwise the system will panic.
 */
kern_return_t txm_register_address_space(pmap_t pmap, uint16_t addr_space_id,
                                         TXMAddressSpaceFlags_t flags);

/**
 * Unregister an address space from the TrustedExecutionMonitor using the
 * address space object which was previously returned from TXM. This needs to be
 * done AFTER the SPTM has unregistered the address space ID from TXM otherwise
