...
#define DBG_MACH_WORKINGSET                                                    \
  0x32 // private subclass for working set related debugging
...
#define DBG_MACH_MACHDEP_EXCP_SC_x86                                           \
  0xAE // Machine Dependent System Calls on x86
#define DBG_MACH_MACHDEP_EXCP_SC_ARM                                           \
...
#define MACH_PROMOTE                                                           \
  0x7 /* promoted due to resource (replaced by MACH_PROMOTED) */
...
#define MACH_URGENCY                                                           \
  0x15                       /* Urgency (RT/BG/NORMAL) communicated            \
                              * to platform                                    \
                              */
...
#define MACH_SCHED_THREAD_SWITCH                                               \
  0x23 /* attempt direct context switch to hinted thread */
#define MACH_SCHED_SMT_BALANCE 0x24 /* SMT load balancing ASTs */
#define MACH_REMOTE_DEFERRED_AST                                               \
  0x25 /* Deferred AST started against remote processor */
#define MACH_REMOTE_CANCEL_AST                                                 \
  0x26 /* Canceled deferred AST for remote processor */
#define MACH_SCHED_CHANGE_PRIORITY 0x27 /* thread sched priority changed */
#define MACH_SCHED_UPDATE_REC_CORES                                            \
  0x28 /* Change to recommended processor bitmask */
#define MACH_STACK_WAIT                                                        \
  0x29 /* Thread could not be switched-to because of kernel stack shortage */
...
#define MACH_REC_CORES_FAILSAFE                                                \
  0x2e /* recommended processor failsafe kicked in */
...
/* unused  MACH_PROMOTED              0x35 was: thread promoted due to mutex
 * priority promotion */
/* unused  MACH_UNPROMOTED            0x36 was: thread unpromoted due to mutex
 * priority promotion */
/* unused  MACH_PROMOTED_UPDATE       0x37 was: thread already promoted, but
 * promotion priority changed */
#define MACH_QUIESCENT_COUNTER 0x38 /* quiescent counter tick */
#define MACH_TURNSTILE_USER_CHANGE                                             \
  0x39 /* base priority change because of turnstile */
#define MACH_AMP_RECOMMENDATION_CHANGE                                         \
  0x3a /* Thread group recommendation change */
#define MACH_AMP_PERFCTL_POLICY_CHANGE                                         \
  0x3b /* AMP policy for perfctl cluster recommendation */
#define MACH_TURNSTILE_KERNEL_CHANGE                                           \
  0x40 /* sched priority change because of turnstile */
#define MACH_SCHED_WI_AUTO_JOIN 0x41 /* work interval auto join events */
#define MACH_SCHED_WI_DEFERRED_FINISH                                          \
  0x42 /* work interval pending finish events for auto-join thread groups */
...
#define MACH_SCHED_NEXT_PROCESSOR                                              \
  0x49 /* Result of choose_next_rt_processor_for_IPI */
...
#define MACH_INT_HANDLED_EXPIRED 0x56 /* interrupt handling threshold crossed  \
                                       */
/* unused MACH_RT_RESTRICT_DENIED    0x57 was: Denied a thread becoming realtime
 * (with -time-constraint-policy-restrict boot-arg)*/
#define MACH_UPDATE_POWERED_CORES                                              \
  0x58 /* CLPC requested cores powerup/powerdown */
...
#define MACH_MODE_DEMOTE_RT_DISALLOWED                                         \
  0x5b /* Sched mode demotion - rt not allowed */
#define MACH_MODE_UNDEMOTE_THROTTLED                                           \
  0x5c /* Sched mode undemotion - throttling */
#define MACH_MODE_UNDEMOTE_FAILSAFE 0x5d /* Sched mode undemotion - failsafe   \
                                          */
#define MACH_MODE_UNDEMOTE_RT_DISALLOWED                                       \
  0x5e                             /* Sched mode undemotion - rt not allowed */
#define MACH_INT_MASKED_RESET 0x5f /* interrupt masked threshold reset */
#define MACH_RT_DISALLOWED_WORK_INTERVAL                                       \
  0x60 /* RT disallowed due to unmet work interval requirements */
#define MACH_SCHED_WI_EXTERNAL_WAKEUP                                          \
  0x61 /* WI thread woken by a thread outside its same work interval */
...
#define MACH_SCHED_PSET_BITMASKS                                               \
  0x65 /* Migration, rotation, and recommendation bitmasks for each pset */
#define MACH_SUSPEND_DRIVERKIT_USERSPACE                                       \
  0x66 /* one driverkit process is suspended/unsuspended by iokit */
#define MACH_SCHED_PREFERRED_PSET                                              \
  0x67 /* Recommendation change for a thread group at a specific QoS */
...
#define MACH_SCHED_CLUTCH_THREAD_SELECT                                        \
  0x2 /* Thread selection events for Clutch scheduler */
#define MACH_SCHED_CLUTCH_THREAD_STATE 0x3 /* __unused */
#define MACH_SCHED_CLUTCH_TG_BUCKET_PRI                                        \
  0x4 /* Clutch bucket priority update event */
/* Edge Scheduler Tracepoints */
#define MACH_SCHED_EDGE_CLUSTER_OVERLOAD                                       \
  0x5 /* Cluster experienced overload; migrating threads to other clusters */
#define MACH_SCHED_EDGE_STEAL 0x6 /* Per-cluster avg. thread execution time */
#define MACH_SCHED_EDGE_REBAL_RUNNABLE                                         \
  0x7 /* Rebalance runnable threads on a foreign cluster */
#define MACH_SCHED_EDGE_REBAL_RUNNING                                          \
  0x8 /* Rebalance running threads on a foreign cluster */
#define MACH_SCHED_EDGE_SHOULD_YIELD 0x9 /* Edge decisions for thread yield */
#define MACH_SCHED_CLUTCH_THR_COUNT                                            \
  0xa /* Clutch scheduler runnable thread counts */
#define MACH_SCHED_EDGE_LOAD_AVG 0xb /* Per-cluster load average */
#define MACH_SCHED_EDGE_CLUSTER_SHARED_LOAD                                    \
  0xc /* Per-cluster shared resource load */
#define MACH_SCHED_EDGE_RSRC_HEAVY_THREAD 0xd /* Resource heavy thread state   \
                                               */
#define MACH_SCHED_EDGE_SHARED_RSRC_MIGRATE                                    \
  0xe /* Migrating a shared resource thread due to cluster load imbalance */
#define MACH_SCHED_EDGE_STIR_THE_POT                                           \
  0xf /* Rotate running threads on and off P-cores to share time and make      \
         roughly equal forward progress */
...
#define WORKGROUP_INTERVAL_SET_WORKLOAD_ID                                     \
  0x6 /* work interval set workload id */
#define WORKGROUP_INTERVAL_SET_WORKLOAD_ID_NAME                                \
  0x7 /* work interval set workload id (name) */
...
#define MACH_IPC_MSG_RECV_VOUCHER_REFUSED                                      \
  0x5                          /* mach_msg receive, voucher refused */
#define MACH_IPC_KMSG_FREE 0x6 /* kernel free of kmsg data */
#define MACH_IPC_VOUCHER_CREATE                                                \
  0x7 /* Voucher added to global voucher hashtable */
#define MACH_IPC_VOUCHER_CREATE_ATTR_DATA                                      \
  0x8 /* Attr data for newly created voucher */
#define MACH_IPC_VOUCHER_DESTROY                                               \
  0x9 /* Voucher removed from global voucher hashtable */
#define MACH_IPC_KMSG_INFO 0xa /* Send/Receive info for a kmsg */
#define MACH_IPC_KMSG_LINK                                                     \
  0xb /* link a kernel kmsg pointer to user mach_msg_header_t */
#define MACH_IPC_PORT_ENTRY_MODIFY                                             \
  0xc /* A port space gained or lost a port right (reference) */
#define MACH_IPC_DESTROY_GUARDED_DESC                                          \
  0xd /* Unable to receive a guarded descriptor */
...
#define MACH_EXCLAVES_SCHEDULER_YIELD                                          \
  0x0 /* Exclaves scheduler Yield response */
#define MACH_EXCLAVES_SCHEDULER_SPAWNED                                        \
  0x1 /* Exclaves scheduler Spawned response */
#define MACH_EXCLAVES_SCHEDULER_TERMINATED                                     \
  0x2 /* Exclaves scheduler Terminated response */
...
#define MACH_EXCLAVES_SCHEDULER_WAKE 0x4 /* Exclaves scheduler Wake response   \
                                          */
#define MACH_EXCLAVES_SCHEDULER_SUSPENDED                                      \
  0x5 /* Exclaves scheduler Suspended response */
#define MACH_EXCLAVES_SCHEDULER_RESUMED                                        \
  0x6 /* Exclaves scheduler Resumed response */
#define MACH_EXCLAVES_SCHEDULER_INTERRUPTED                                    \
  0x7 /* Exclaves scheduler Interrupted response */
#define MACH_EXCLAVES_SCHEDULER_NOTHING_SCHEDULED                              \
  0x8 /* Exclaves scheduler NothingScheduled response */
#define MACH_EXCLAVES_SCHEDULER_ALL_EXCLAVES_BOOTED                            \
  0x9 /* Exclaves scheduler AllExclavesBooted response */
#define MACH_EXCLAVES_SCHEDULER_EARLY_ALLOC                                    \
  0xa /* Exclaves scheduler PmmEarlyAlloc response */
#define MACH_EXCLAVES_SCHEDULER_WATCHDOG_PANIC_COMPLETE                        \
  0xb /* Exclaves scheduler WatchdogPanicComplete response */
#define MACH_EXCLAVES_SCHEDULER_PANICKING                                      \
  0xc /* Exclaves scheduler Panicking response */
#define MACH_EXCLAVES_SCHEDULER_REQ_RESUME_WITH_HOSTID                         \
  0xd /* Exclaves scheduler ResumeWithHostId request */
#define MACH_EXCLAVES_SCHEDULER_REQ_INTERRUPT_WITH_HOSTID                      \
  0xe /* Exclaves scheduler InterruptWithHostIdrequest */
#define MACH_EXCLAVES_SCHEDULER_REQ_UPDATE_TIMER_OFFSET                        \
  0xf /* Exclaves scheduler UpdateTimerOffset request */
#define MACH_EXCLAVES_SCHEDULER_REQ_BOOT_EXCLAVES                              \
  0x10 /* Exclaves scheduler BootExclaves request */
#define MACH_EXCLAVES_SCHEDULER_REQ_PMM_EARLY_ALLOC_RESPONSE                   \
  0x11 /* Exclaves scheduler PmmEarlyAllocResponse request */
#define MACH_EXCLAVES_SCHEDULER_REQ_WATCHDOG_PANIC                             \
  0x12 /* Exclaves scheduler WatchdogPanic request */
...
#define MACH_BRIDGE_RCV_TS                                                     \
  0x1 /* receive timestamp pair from interrupt handler */
...
#define MACH_BRIDGE_TS_PARAMS                                                  \
  0x4                           /* recompute timestamp conversion parameters */
#define MACH_BRIDGE_SKIP_TS 0x5 /* skip timestamp */
#define MACH_BRIDGE_TS_MISMATCH                                                \
  0x6 /* mismatch between predicted and received remote timestamp */
...
#define STACKSHOT_RECORD_SHORT                                                 \
  0x3 /* ran out of space inside stackshot, growing buffer */
...
#define DBG_CONTENT_PROT                                                       \
  0xCF /* Content Protection Events: see bsd/sys/cprotect.h */
...
#define BSD_MEMSTAT_FREEZE_DISABLE                                             \
  17 /* disable freeze and kill frozen processes */
#define BSD_MEMSTAT_RELAUNCH_FLAGS                                             \
  18 /* flags representing jetsam behavior; based on launchd data */
...
#define DKIO_THROTTLE                                                          \
  0x20 /* Deprecated, still provided so fs_usage doesn't break */
...
#define IMP_MSG                                                                \
  0x12 /* boosting message sent by donating task on donating port */
#define IMP_WATCHPORT                                                          \
  0x13 /* port marked as watchport, and boost was transferred to the watched   \
          task */
#define IMP_THREAD_PROMOTE_ABOVE_TASK                                          \
  0x15 /* Thread is turnstile boosted above task clamp */
...
#define IMP_USYNCH_QOS_OVERRIDE                                                \
  0x1A /* Userspace synchronization applied QoS override to resource owning    \
          thread */
...
#define IMP_TASK_POLICY_BASE_LATENCY_AND_THROUGHPUT_QOS                        \
  0x39 /* latency as value1, throughput as value2 */
#define IMP_TASK_POLICY_OVERRIDE_LATENCY_AND_THROUGHPUT_QOS                    \
  0x3A /* latency as value1, throughput as value2 */
...
#define IMP_TASK_POLICY_QOS_AND_RELPRIO                                        \
  0x38 /* QoS as value1, relative priority as value2 */
...
#define IMP_TASK_POLICY_QOS_IPC_OVERRIDE                                       \
  IMP_TASK_POLICY_QOS_KEVENT_OVERRIDE /* legacy name */
...
#define IMP_EXTERN                                                             \
  0x8 /* boost assertion moved from kernel to userspace responsibility         \
         (externalized) */
...
/* Codes for IMP_MSG */
#define IMP_MSG_SEND                                                           \
  0x1 /* boosting message sent by donating task on donating port */
...
#define IMP_USYNCH_ADD_OVERRIDE 0x0 /* add override for a contended resource   \
                                     */
#define IMP_USYNCH_REMOVE_OVERRIDE                                             \
  0x1 /* remove override for a contended resource */
...
#define BANK_SETTLE_ENERGY                                                     \
  0x3 /* Bank ledger(energy field) rolled up to tasks. */
...
#define KDBG_MIGCODE(msgid)                                                    \
  (((unsigned)DBG_MIG << KDBG_CLASS_OFFSET) |                                  \
...
#define SILICONDBG_CODE(SubClass, code)                                        \
  KDBG_EVENTID(DBG_SILICON, SubClass, code)
...
#define SECURITYDBG_CODE(SubClass, code)                                       \
  KDBG_EVENTID(DBG_SECURITY, SubClass, code)
...
#define ARIADNEDBG_CODE(SubClass, code)                                        \
  KDBG_EVENTID(DBG_ARIADNE, SubClass, code)
...
#define MTDBG_RESOURCES_ON_PROC_EXIT(code)                                     \
  MTDBG_CODE(DBG_MT_RESOURCES_PROC_EXIT, code)
#define MTDBG_RESOURCES_ON_THR_EXIT(code)                                      \
  MTDBG_CODE(DBG_MT_RESOURCES_THR_EXIT, code)
...
#define IMPORTANCE_CODE(SubClass, code)                                        \
  KDBG_EVENTID(DBG_IMPORTANCE, (SubClass), (code))
...
#define TURNSTILE_CODE(SubClass, code)                                         \
  KDBG_EVENTID(DBG_TURNSTILE, (SubClass), (code))
