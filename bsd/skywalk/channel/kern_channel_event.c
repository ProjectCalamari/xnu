...
static inline errno_t __notif_dest_by_ifp(struct __notif_dest *dest,
                                          const ifnet_t ifp) {
...
static inline errno_t __notif_dest_by_nx_uuid(struct __notif_dest *dest,
  ...
  dest->dest_type = __NOTIF_DEST_FSW;
  dest->dest_fsw = fsw;
  dest->dest_desc = (fsw->fsw_ifp != NULL) ? if_name(fsw->fsw_ifp) : desc;
  return 0;
}
...
static inline errno_t kern_channel_packet_event_notify(
    struct __notif_dest *dest, os_channel_event_type_t event_type,
    size_t event_dlen, uint8_t *__sized_by(event_dlen) event_data,
    uint32_t nx_port_id) {
  ...
  SK_DF(SK_VERB_EVENTS,
        "%s[%d] kern_channel_event: %p dest_type: %u len: %zu "
  ...
  case __NOTIF_DEST_NETIF:
    return netif_vp_na_channel_event(dest->dest_netif, nx_port_id, event,
                                     CHANNEL_EVENT_MAX_LEN);
  case __NOTIF_DEST_FSW:
    return fsw_vp_na_channel_event(dest->dest_fsw, nx_port_id, event,
                                   CHANNEL_EVENT_MAX_LEN);
  default:
    return EINVAL;
  }
...
errno_t kern_channel_event_transmit_status_with_packet(const kern_packet_t ph,
                                                       const ifnet_t ifp) {
  errno_t err;
  uint32_t nx_port_id;
  ...
  (void)__packet_get_tx_completion_status(ph, &pkt_tx_status.packet_status);
  ...
  return kern_channel_packet_event_notify(
      &dest, CHANNEL_EVENT_PACKET_TRANSMIT_STATUS, sizeof(pkt_tx_status),
      (uint8_t *)&pkt_tx_status, nx_port_id);
}

errno_t kern_channel_event_transmit_status(
    const ifnet_t ifp, os_channel_event_packet_transmit_status_t *pkt_tx_status,
    uint32_t nx_port_id) {
  errno_t err;
  struct __notif_dest dest = {0, {NULL}, NULL};
  uint8_t *event_data;
   ...
   */
  event_data =
      (uint8_t *__bidi_indexable)(os_channel_event_packet_transmit_status_t *
                                  __bidi_indexable) pkt_tx_status;
  return kern_channel_packet_event_notify(
      &dest, CHANNEL_EVENT_PACKET_TRANSMIT_STATUS, sizeof(*pkt_tx_status),
      event_data, nx_port_id);
}

errno_t kern_channel_event_transmit_status_with_nexus(
    const uuid_t nx_uuid,
    os_channel_event_packet_transmit_status_t *pkt_tx_status,
    uint32_t nx_port_id) {
  errno_t err;
...
errno_t kern_channel_event_transmit_expired(
    const ifnet_t ifp,
   ...
   */
  event_data =
      (uint8_t *__bidi_indexable)(os_channel_event_packet_transmit_expired_t *
                                  __bidi_indexable) pkt_tx_expired;
  return kern_channel_packet_event_notify(
      &dest, CHANNEL_EVENT_PACKET_TRANSMIT_EXPIRED, sizeof(*pkt_tx_expired),
      event_data, nx_port_id);
}

extern errno_t kern_channel_event_transmit_expired_with_nexus(
    const uuid_t nx_uuid,
    os_channel_event_packet_transmit_expired_t *pkt_tx_expired,
    uint32_t nx_port_id) {
  errno_t err;
...
void kern_channel_event_notify(struct __kern_channel_ring *kring) {
...
int kern_channel_event_sync(struct __kern_channel_ring *kring, struct proc *p,
                            uint32_t flags) {
