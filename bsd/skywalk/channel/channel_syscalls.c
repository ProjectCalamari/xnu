...
static int chop_kqfilter(struct fileproc *, struct knote *,
                         struct kevent_qos_s *);
 ...
 * is of type user_addr_t (non-pointer type), which can't be marked as
 * __sized_by.
 */
#define USER_ADDR_TO_PTR(chan_init)                                            \
  __unsafe_forge_bidi_indexable(void *, (chan_init).ci_key,                    \
                                (chan_init).ci_key_len)
...
static int chop_select(struct fileproc *fp, int which, void *wql,
                       vfs_context_t ctx) {
...
static int chop_close(struct fileglob *fg, vfs_context_t ctx) {
#pragma unused(ctx)
  struct kern_channel *ch;
 ...
 */
static int chop_kqfilter(struct fileproc *fp, struct knote *kn,
...
int __channel_open(struct proc *p, struct __channel_open_args *uap,
                   int *retval) {
  ...
  if (__improbable(init.ci_version != CHANNEL_INIT_CURRENT_VERSION)) {
    ...
    goto done;
                 ...
                 (init.ci_key_len == 0 && init.ci_key != USER_ADDR_NULL))) {
    SK_PERR(p, "EINVAL: ci_key_len %i, ci_key %p", init.ci_key_len,
            SK_KVA(init.ci_key));
    err = EINVAL;
    goto done;
  }
  ...
  err = falloc_guarded(p, &fp, &fd, vfs_context_current(), &guard,
                       GUARD_CLOSE | GUARD_DUP | GUARD_SOCKET_IPC |
                           GUARD_FILEPORT | GUARD_WRITE);
  if (__improbable(err != 0)) {
  ...
  if (keylen != 0) {
    ...
    if (__improbable(err != 0)) {
      SK_PERR(p, "copyin err %u: ci_key %p, ci_key_len %u", err,
              SK_KVA(init.ci_key), keylen);
      goto done;
    }
  }
  ...
  if (__improbable((ch = ch_open(&init, p, fd, &err)) == NULL)) {
    ...
    ASSERT(err != 0);
    SK_PERR(p, "ch_open nx_port %d err %u", (int)init.ci_nx_port, err);
    goto done;
  }
  /* in case not processed */
  key = USER_ADDR_TO_PTR(init);
  ...
  if (__improbable(err != 0)) {
    SK_PERR(p, "copyout err %u: init %p", err, SK_KVA(uap->init));
    goto done;
  }
  ...
  *retval = fd;
  ...
  SK_D("%s(%d) nx_port %d fd %d %s", sk_proc_name(p), sk_proc_pid(p),
       (int)init.ci_nx_port, fd, ch->ch_na->na_name);
...
int __channel_get_info(struct proc *p, struct __channel_get_info_args *uap,
                       int *retval) {
#pragma unused(retval)
  ...
  if (__improbable(uap->cinfo == USER_ADDR_NULL ||
    ...
    SK_PERR(p, "EINVAL: cinfo %p, cinfolen %u", SK_KVA(uap->cinfo),
            uap->cinfolen);
    err = EINVAL;
    goto done;
  }
  ...
  if (__improbable(err != 0)) {
    SK_PERR(p, "copyout err %u: cinfo %p", err, SK_KVA(uap->cinfo));
    goto done;
  }
...
SK_LOG_ATTRIBUTE
static void channel_sync_log1(uint64_t verb, const char *sync, struct proc *p,
                              const struct nexus_adapter *na,
                              const struct kern_channel *ch,
                              const struct __kern_channel_ring *kring,
                              ring_id_t i) {
  verb |= SK_VERB_SYNC;
  SK_DF(verb,
        "%s(%d) pre: %s ring %u na \"%s\" (%p) ch %p "
        "th %p h %u kh %u",
        sk_proc_name(p), sk_proc_pid(p), sync, i, na->na_name, SK_KVA(na),
        SK_KVA(ch), SK_KVA(current_thread()), kring->ckr_ring->ring_head,
        kring->ckr_khead);
}

SK_LOG_ATTRIBUTE
static void channel_sync_log2(uint64_t verb, const char *sync, struct proc *p,
                              const struct nexus_adapter *na,
                              const struct __kern_channel_ring *kring,
                              ring_id_t i) {
  verb |= SK_VERB_SYNC;
  SK_DF(verb, "%s(%d) post: %s ring %u na \"%s\" h %u kh %u", sk_proc_name(p),
        sk_proc_pid(p), sync, i, na->na_name, kring->ckr_ring->ring_head,
        kring->ckr_khead);
}
#endif /* SK_LOG */

int __channel_sync(struct proc *p, struct __channel_sync_args *uap,
                   int *retval) {
#pragma unused(retval)
  struct fileproc *__single fp;
  ...
  for (i = qfirst; i < qlast; i++) {
    ...
    if (__improbable((sk_verbose & SK_VERB_SYNC) != 0)) {
      channel_sync_log1((mode == CHANNEL_SYNC_TX) ? SK_VERB_TX : SK_VERB_RX,
                        (mode == CHANNEL_SYNC_TX) ? MODE_TAG_TXSYNC
                                                  : MODE_TAG_RXSYNC,
                        p, na, ch, kring, i);
    }
#endif /* SK_LOG */
    ...
    case CHANNEL_SYNC_TX: {
      ...
      } else if (kring->ckr_na_sync(kring, p, NA_SYNCF_FORCE_RECLAIM) == 0) {
    ...
    case CHANNEL_SYNC_RX: {
      ...
      } else if (kring->ckr_na_sync(kring, p, NA_SYNCF_FORCE_READ) == 0) {
...
#if SK_LOG
    if (__improbable((sk_verbose & SK_VERB_SYNC) != 0)) {
      channel_sync_log2((mode == CHANNEL_SYNC_TX) ? SK_VERB_TX : SK_VERB_RX,
                        (mode == CHANNEL_SYNC_TX) ? MODE_TAG_TXSYNC
                                                  : MODE_TAG_RXSYNC,
                        p, na, kring, i);
    }
...
packet_pool_sync:
  if (flags & (CHANNEL_SYNCF_ALLOC | CHANNEL_SYNCF_LARGE_ALLOC |
               CHANNEL_SYNCF_ALLOC_BUF)) {
    enum txrx type;
    ...
    if (flags & CHANNEL_SYNCF_LARGE_ALLOC) {
      ...
      if (!(flags & CHANNEL_SYNCF_ALLOC)) {
        qfirst++;
      } else if ((qlast - qfirst) > 1 && !(flags & CHANNEL_SYNCF_ALLOC_BUF)) {
        qlast--;
      }
    }
    ASSERT(qfirst != qlast);
    ...
    for (i = qfirst; i < qlast; i++) {
...
#if SK_LOG
      if (__improbable((sk_verbose & SK_VERB_SYNC) != 0)) {
        channel_sync_log1(0, "alloc-sync", p, na, ch, kring, i);
      }
#endif /* SK_LOG */
      if (__improbable(kr_alloc_sync_prologue(kring, p) >=
        ...
        kr_log_bad_ring(kring);
        if (!err) {
          SK_PERR(p, "EFAULT: kr_alloc_sync_prologue()");
          err = EFAULT;
        }
      } else if (kring->ckr_na_sync(kring, p, upp_sync_flags) == 0) {
        kr_alloc_sync_finalize(kring, p);
      } else {
        if (!err) {
          SK_PERR(p, "EIO: ALLOC: ring->ckr_na_sync()");
          err = EIO;
        }
        skywalk_kill_process(p, SKYWALK_KILL_REASON_SYNC_FAILED |
                                    SKYWALK_KILL_REASON_ALLOC_SYNC);
      }
#if SK_LOG
      if (__improbable((sk_verbose & SK_VERB_SYNC) != 0)) {
        channel_sync_log2(0, "alloc-sync", p, na, kring, i);
      }
#endif /* SK_LOG */
      kr_exit(kring);
    }
  ...
  if (flags & CHANNEL_SYNCF_FREE) {
    ...
    for (i = qfirst; i < qlast; i++) {
...
#if SK_LOG
      if (__improbable((sk_verbose & SK_VERB_SYNC) != 0)) {
        channel_sync_log1(0, "free-sync", p, na, ch, kring, i);
      }
#endif /* SK_LOG */
      if (__improbable(kr_free_sync_prologue(kring, p) >=
        ...
        kr_log_bad_ring(kring);
        if (!err) {
          SK_PERR(p, "EFAULT: kr_free_sync_prologue()");
          err = EFAULT;
        }
      } else if (kring->ckr_na_sync(kring, p, upp_sync_flags) == 0) {
        ...
        if (!err) {
          SK_PERR(p, "EIO: FREE: ring->ckr_na_sync()");
          err = EIO;
        }
        skywalk_kill_process(p, SKYWALK_KILL_REASON_SYNC_FAILED |
...
#if SK_LOG
      if (__improbable((sk_verbose & SK_VERB_SYNC) != 0)) {
        channel_sync_log2(0, "free-sync", p, na, kring, i);
      }
#endif /* SK_LOG */
      kr_exit(kring);
    }
...
int __channel_get_opt(struct proc *p, struct __channel_get_opt_args *uap,
                      int *retval) {
#pragma unused(retval)
  struct fileproc *__single fp;
  struct kern_channel *ch = NULL;
  ...
  if (uap->aoptval != USER_ADDR_NULL) {
    ...
    if (err != 0) {
      SK_PERR(p, "copyin err %u: aoptlen %p", err, SK_KVA(uap->aoptlen));
      goto done;
    }
  } else {
    optlen = 0;
...
#if SK_LOG
    if (err != 0) {
      SK_PERR(p, "copyout err %u: aoptlen %p", err, SK_KVA(uap->aoptlen));
    }
#endif
  }
...
int __channel_set_opt(struct proc *p, struct __channel_set_opt_args *uap,
                      int *retval) {
#pragma unused(retval)
  struct fileproc *__single fp;
  struct kern_channel *ch = NULL;
  ...
  if (__improbable(ch->ch_flags & (CHANF_CLOSING | CHANF_DEFUNCT))) {
    ...
    err = ENXIO;
  } else if (__improbable(ch->ch_na == NULL || !NA_IS_ACTIVE(ch->ch_na) ||
