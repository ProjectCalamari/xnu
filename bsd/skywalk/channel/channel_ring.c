...
static void kr_update_user_stats(struct __kern_channel_ring *, uint32_t,
                                 uint32_t);
...
SYSCTL_UINT(_kern_skywalk, OID_AUTO, ring_transfer_decay,
            CTLFLAG_RW | CTLFLAG_LOCKED, &kr_transfer_decay, 0,
            "ilog2 of EWMA decay rate of ring transfers");
...
SYSCTL_UINT(_kern_skywalk, OID_AUTO, disable_panic_on_sync_err,
            CTLFLAG_RW | CTLFLAG_LOCKED, &kr_disable_panic_on_sync_err, 0,
            "disable panic on sync error");
#endif /* (DEVELOPMENT || DEBUG) */

SYSCTL_UINT(_kern_skywalk, OID_AUTO, ring_stat_enable,
            CTLFLAG_RW | CTLFLAG_LOCKED, &kr_stat_enable, 0,
            "enable/disable stats collection for ring");

#define KR_EWMA(old, new, decay)                                               \
  do {                                                                         \
    u_int64_t _avg;                                                            \
...
#define _BUF_DLIM(_buf, _pp)                                                   \
  (BUFLET_HAS_LARGE_BUF(_buf) ? PP_BUF_SIZE_LARGE(_pp) : PP_BUF_SIZE_DEF(_pp))

void kr_init_to_mhints(struct __kern_channel_ring *kring, uint32_t nslots) {
  uint32_t tail;
 ...
 */
int kr_enter(struct __kern_channel_ring *kr, boolean_t can_sleep) {
  ...
  if (!can_sleep) {
    ...
    while (kr->ckr_busy != 0) {
      ...
      SK_DF(SK_VERB_LOCKS,
            "waited for kr \"%s\" "
            "(%p) busy=%u",
            kr->ckr_name, SK_KVA(kr), kr->ckr_busy);
      lck_spin_lock(&kr->ckr_slock);
    }
  }
  ...
  SK_DF(SK_VERB_LOCKS, "kr \"%s\" (%p) right acquired", kr->ckr_name,
        SK_KVA(kr));

  return 0;
}

void kr_exit(struct __kern_channel_ring *kr) {
  uint32_t want = 0;
...
void kr_start(struct __kern_channel_ring *kr) {
  lck_spin_lock(&kr->ckr_slock);
  ASSERT(kr->ckr_busy != 0);
  ...
  SK_DF(SK_VERB_LOCKS, "kr \"%s\" (%p) is started", kr->ckr_name, SK_KVA(kr));
}
 ...
 * later point.
 */
void kr_stop(struct __kern_channel_ring *kr, uint32_t state) {
  uint32_t s;
  ...
  SK_DF(SK_VERB_LOCKS, "kr \"%s\" (0x%p) krflags 0x%x is now stopped s=%u",
        kr->ckr_name, SK_KVA(kr), kr->ckr_flags, state);
}

static void kr_update_user_stats(struct __kern_channel_ring *kring,
                                 uint32_t slot_count, uint32_t byte_count) {
  uint64_t now;
  uint32_t transfer_decay =
      (kr_transfer_decay != 0) ? kr_transfer_decay : kring->ckr_transfer_decay;
  ...
  if (__probable(kring->ckr_user_accumulate_start != 0)) {
    if ((now - kring->ckr_user_accumulate_start) >= kr_accumulate_interval) {
      uint64_t bps;
      uint64_t sps;
      uint64_t sps_ma;
      ...
      /* bytes per sync */
      bps =
          kring->ckr_user_accumulated_bytes / kring->ckr_user_accumulated_syncs;
      KR_EWMA(stats->crsu_bytes_per_sync_ma, bps, transfer_decay);
      stats->crsu_bytes_per_sync = bps;

      /* slots per sync */
      sps =
          kring->ckr_user_accumulated_slots / kring->ckr_user_accumulated_syncs;
...
void kr_update_stats(struct __kern_channel_ring *kring, uint32_t slot_count,
                     uint32_t byte_count) {
  uint64_t now;
  uint64_t diff_secs;
  ...
  if (__probable(kring->ckr_accumulate_start != 0)) {
    ...
    if (diff_secs >= kr_accumulate_interval) {
      ...
      KR_EWMA(stats->crs_bytes_per_second_ma, bps, transfer_decay);
...
boolean_t kr_txempty(struct __kern_channel_ring *kring) {
 ...
 * it under lock.
 */
void kr_log_bad_ring(struct __kern_channel_ring *kring) {
  ...
  for (i = 0; i <= lim; i++) {
    ...
    if (len > kring->ckr_max_pkt_len) {
      SK_RDERR(5, "bad len at slot %u idx %u len %u", i, idx, len);
    }
  }
  ...
  if (errors != 0) {
    ...
    SK_ERR("kr \"%s\" (0x%p) krflags 0x%x crash, "
           "head %u/%u -> %u tail %u/%u -> %u",
           kring->ckr_name, SK_KVA(kring), kring->ckr_flags, ring->ring_head,
...
uint32_t kr_reclaim(struct __kern_channel_ring *kr) {
  int r = 0;
 ...
 */
int kr_txprologue(struct kern_channel *ch, struct __kern_channel_ring *kring,
                  const slot_idx_t head, uint32_t *byte_count,
                  uint64_t *err_reason, struct proc *p) {
  ...
  while (slot_idx != head) {
    ...
    if (__improbable(err != 0)) {
      SK_ERR("%s(%d) kr \"%s\" (%p) slot %u dropped "
             ...
             sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, SK_KVA(kring),
             slot_idx, err, kring->ckr_khead, kring->ckr_ktail,
             kring->ckr_rhead, kring->ckr_rtail, kring->ckr_ring->ring_head,
 ...
 */
int kr_txprologue_upp(struct kern_channel *ch,
                      struct __kern_channel_ring *kring, const slot_idx_t head,
                      uint32_t *byte_count, uint64_t *err_reason,
                      struct proc *p) {
  struct kern_pbufpool *pp = kring->ckr_pp;
  const uint32_t maxfrags = pp->pp_max_frags;
  slot_idx_t slot_idx = kring->ckr_rhead;
    ...
    if (__improbable(err != 0)) {
      if (kqum != NULL) {
        SK_ERR("%s(%d) kr \"%s\" (%p) slot %u "
               ...
               sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, SK_KVA(kring),
               slot_idx, SK_KVA(kqum));
        *err_reason = SKYWALK_KILL_REASON_BAD_BUFLET_CHAIN;
        goto done;
      }
      ...
      SK_ERR("%s(%d) kr \"%s\" (%p) slot %u "
             ...
             "rh %u rt %u | h %u t %u",
             sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, SK_KVA(kring),
             slot_idx, usd->sd_md_idx, kring->ckr_khead, kring->ckr_ktail,
             kring->ckr_rhead, kring->ckr_rtail, kring->ckr_ring->ring_head,
             kring->ckr_ring->ring_tail);
 ...
 */
#define _KR_TXRING_VALIDATE(_kring, _ring, _kh, _kt, _rh, _krt)                \
  do {                                                                         \
    ...
    NM_FAIL_ON(ring->ring_tail != (_krt), SKYWALK_KILL_REASON_TAIL_MISMATCH);  \
  } while (0)
 ...
 */
slot_idx_t kr_txsync_prologue(struct kern_channel *ch,
  ...
  SK_DF(SK_VERB_SYNC | SK_VERB_TX,
        "%s(%d) kr \"%s\", kh %u kt %u | "
        "rh %u rt %u | h %u t %u",
        sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, ckr_khead, ckr_ktail,
   ...
   */
  if (kring->ckr_prologue != NULL &&
      (kring->ckr_prologue(ch, kring, head, &byte_count, &err_reason, p) !=
       0)) {
    goto error;
  }
...
error:
  SK_ERR("%s(%d) kr \"%s\" (%p) krflags 0x%x error: kh %u kt %u | "
         "rh %u rt %u | h %u t %u |",
         sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, SK_KVA(kring),
         kring->ckr_flags, ckr_khead, ckr_ktail, kring->ckr_rhead, ckr_rtail,
         head, ring->ring_tail);
 ...
 * in case of error, in order to force a reinit.
 */
slot_idx_t kr_free_sync_prologue(struct __kern_channel_ring *kring,
                                 struct proc *p) {
  struct __user_channel_ring *ring = kring->ckr_ring;
  slot_idx_t ckr_khead, ckr_ktail, ckr_rtail;
  slot_idx_t head;
 ...
 */
int kr_rxprologue(struct kern_channel *ch, struct __kern_channel_ring *kring,
                  const slot_idx_t head, uint32_t *byte_count,
                  uint64_t *err_reason, struct proc *p) {
#pragma unused(ch, p)
  slot_idx_t slot_idx = kring->ckr_rhead;
  ...
  if (nfree > 0) {
    pp_free_packet_batch(kring->ckr_pp, &kring->ckr_scratch[0], nfree);
  }
  ...
  if (kring->ckr_ready_bytes < *byte_count) {
    SK_ERR("%s(%d) kr \"%s\" (%p) inconsistent ready bytes "
           ...
           sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, SK_KVA(kring),
           kring->ckr_ready_bytes, *byte_count, kring->ckr_khead,
           kring->ckr_ktail, kring->ckr_rhead, kring->ckr_rtail,
           kring->ckr_ring->ring_head, kring->ckr_ring->ring_tail);
 ...
 */
int kr_rxprologue_nodetach(struct kern_channel *ch,
                           struct __kern_channel_ring *kring,
                           const slot_idx_t head, uint32_t *byte_count,
                           uint64_t *err_reason, struct proc *p) {
#pragma unused(ch, p)
    ...
    *err_reason = SKYWALK_KILL_REASON_INCONSISTENT_READY_BYTES;
    ...
    if (kr_disable_panic_on_sync_err == 0) {
      panic("kr(%p), inconsistent, head %u, ready %llu, "
            "cnt %u",
            SK_KVA(kring), head, kring->ckr_ready_bytes, *byte_count);
      /* NOTREACHED */
 ...
 */
int kr_rxprologue_upp(struct kern_channel *ch,
                      struct __kern_channel_ring *kring, const slot_idx_t head,
                      uint32_t *byte_count, uint64_t *err_reason,
                      struct proc *p) {
#pragma unused(ch, p)
 ...
 */
#define _KR_RXRING_VALIDATE(_kring, _ring, _kh, _kt, _rh)                      \
  do {                                                                         \
    slot_idx_t _n = (_kring)->ckr_num_slots;                                   \
    /* kernel sanity checks */                                                 \
    NM_FAIL_ON((_kh) >= _n || (_kt) >= _n, SKYWALK_KILL_REASON_BASIC_SANITY);  \
    /* user sanity checks */                                                   \
    if ((_kt) >= (_kh)) {                                                      \
 ...
 */
slot_idx_t kr_rxsync_prologue(struct kern_channel *ch,
                              struct __kern_channel_ring *kring,
                              struct proc *p) {
#pragma unused(ch)
  struct __user_channel_ring *ring = kring->ckr_ring;
  ...
  SK_DF(SK_VERB_SYNC | SK_VERB_RX,
        ...
        "rh %u rt %u | h %u t %u",
        sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, ckr_khead, ckr_ktail,
        kring->ckr_rhead, kring->ckr_rtail, ring->ring_head, ring->ring_tail);
  /*
...
error:
  SK_ERR("%s(%d) kr \"%s\" (%p) krflags 0x%x error: kh %u kt %u | "
         "rh %u rt %u | h %u t %u",
         sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, SK_KVA(kring),
         kring->ckr_flags, ckr_khead, ckr_ktail, kring->ckr_rhead,
         kring->ckr_rtail, ring->ring_head, ring->ring_tail);
 ...
 * in order to force a reinit.
 */
slot_idx_t kr_alloc_sync_prologue(struct __kern_channel_ring *kring,
                                  struct proc *p) {
  struct __user_channel_ring *ring = kring->ckr_ring;
  slot_idx_t ckr_khead, ckr_ktail;
  slot_idx_t head;
  ...
  SK_DF(SK_VERB_SYNC,
        ...
        "rh %u rt %u | h %u t %u",
        sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, ckr_khead, ckr_ktail,
        kring->ckr_rhead, kring->ckr_rtail, head, ring->ring_tail);
  /*
   * Before storing the new values, we should check they do not
 ...
 */
void kr_txfinalize(struct kern_channel *ch, struct __kern_channel_ring *kring,
...
#pragma unused(ch)
  ...
  if (i != ph_cnt) {
    kern_pbufpool_free_batch(pp, &kring->ckr_scratch[i], ph_cnt - i);
  }
}
 ...
 */
void kr_txfinalize_upp(struct kern_channel *ch,
                       struct __kern_channel_ring *kring, const slot_idx_t head,
                       struct proc *p) {
#pragma unused(ch, p)
  slot_idx_t slot_idx;
 ...
 */
void kr_txsync_finalize(struct kern_channel *ch,
  ...
  *(slot_idx_t *)(uintptr_t)&kring->ckr_ring->ring_tail = kring->ckr_rtail =
      ckr_ktail;
 ...
 */
void kr_rxfinalize(struct kern_channel *ch, struct __kern_channel_ring *kring,
...
#pragma unused(ch)
   ...
   */
  if (__improbable(kring->ckr_netif_mit_stats != NULL) && slot_diff != 0 &&
      byte_count != 0) {
 ...
 */
void kr_rxfinalize_upp(struct kern_channel *ch,
                       struct __kern_channel_ring *kring, const slot_idx_t tail,
                       struct proc *p) {
  const uint32_t maxfrags = kring->ckr_pp->pp_max_frags;
  slot_idx_t slot_idx = kring->ckr_rtail;
  struct kern_pbufpool *pp = kring->ckr_pp;
 ...
 */
void kr_rxsync_finalize(struct kern_channel *ch,
                        struct __kern_channel_ring *kring, struct proc *p) {
#pragma unused(ch, p)
  slot_idx_t ckr_khead, ckr_ktail;
...
void kr_alloc_sync_finalize(struct __kern_channel_ring *kring, struct proc *p) {
#pragma unused(p)
  slot_idx_t ckr_khead, ckr_ktail;
  ...
  *(slot_idx_t *)(uintptr_t)&kring->ckr_ring->ring_tail = kring->ckr_rtail =
      ckr_ktail;
  *(slot_idx_t *)(uintptr_t)&kring->ckr_ring->ring_khead = ckr_khead;
  *(uint32_t *)(uintptr_t)&kring->ckr_ring->ring_alloc_ws = kring->ckr_alloc_ws;
...
void kr_free_sync_finalize(struct __kern_channel_ring *kring, struct proc *p) {
#pragma unused(p)
  slot_idx_t ckr_khead, ckr_ktail;
...
slot_idx_t kr_event_sync_prologue(struct __kern_channel_ring *kring,
                                  struct proc *p) {
  struct __user_channel_ring *ring = kring->ckr_ring;
  slot_idx_t ckr_khead, ckr_ktail;
  ...
  while (slot_idx != head) {
    ...
    if (__improbable(SD_VALID_METADATA(usd))) {
      SK_ERR("%s(%d) kr \"%s\" (%p) slot %u not "
             ...
             " h %u t %u",
             sk_proc_name(p), sk_proc_pid(p), kring->ckr_name, SK_KVA(kring),
             slot_idx, usd->sd_md_idx, ckr_khead, ckr_ktail, kring->ckr_rhead,
             kring->ckr_rtail, ring->ring_head, ring->ring_tail);
      err_reason = SKYWALK_KILL_REASON_SLOT_NOT_DETACHED;
      goto error;
    }
...
void kr_event_sync_finalize(struct kern_channel *ch,
                            struct __kern_channel_ring *kring, struct proc *p) {
#pragma unused(ch)
  struct kern_pbufpool *pp = kring->ckr_pp;
  const uint32_t maxfrags = pp->pp_max_frags;
...
void kr_txkring_reclaim_and_refill(struct __kern_channel_ring *kring,
                                   slot_idx_t index) {
  const slot_idx_t lim = kring->ckr_lim;
 ...
 *  |----------+------------------------------+----------------------------+
 */
int kr_internalize_metadata(struct kern_channel *ch,
                            struct __kern_channel_ring *kring,
                            const uint32_t maxfrags,
...
#pragma unused(kring, maxfrags, p)
  ...
  SK_DF(SK_VERB_MEM, "%s(%d) kring %p uqum %p -> kqum %p", sk_proc_name(p),
        sk_proc_pid(p), SK_KVA(kring), SK_KVA(uqum), SK_KVA(kqum));
  ...
  if (__improbable(dropped)) {
    SK_ERR("%s(%d) kring %p dropped", sk_proc_name(p), sk_proc_pid(p),
           SK_KVA(kring));
    err = ERANGE;
    goto done;
  }
  ...
  if (__probable(ch != NULL)) {
    _UUID_COPY(kpkt->pkt_flowsrc_id, ch->ch_info->cinfo_ch_id);
  }
  ...
  if (__improbable((bcnt == 0) || (bcnt > bmax) ||
    ...
    SK_ERR("%s(%d) kring %p bad bufcnt %d, %d, %d", sk_proc_name(p),
           sk_proc_pid(p), SK_KVA(kring), bcnt, bmax, upkt->pkt_bufs_max);
    err = ERANGE;
    goto done;
  }
  ...
  for (i = 0; i < bcnt; i++) {
    ...
    if (kbuf->buf_flag & BUFLET_FLAG_EXTERNAL) {
      ...
      kbuf_ext = __container_of(kbuf, struct __kern_buflet_ext, kbe_overlay);
      ubuf = __DECONST(struct __user_buflet *, kbuf_ext->kbe_buf_user);
    } else {
      ASSERT(i == 0);
      ubuf = __DECONST(struct __user_buflet *, &uqum->qum_buf[0]);
    }
    ASSERT(ubuf != NULL);
     ...
     */
    if (__improbable(!BUF_IN_RANGE(kbuf) || ubuf->buf_idx != kbuf->buf_idx)) {
      kbuf->buf_dlen = kbuf->buf_doff = 0;
      SK_ERR("%s(%d) kring %p bad bufidx 0x%x, 0x%x", sk_proc_name(p),
             sk_proc_pid(p), SK_KVA(kring), kbuf->buf_idx, ubuf->buf_idx);
      err = ERANGE;
      goto done;
    }
  ...
  static_assert(offsetof(struct __kern_packet, pkt_length) ==
                offsetof(struct __kern_packet, pkt_qum.qum_len));
  if (__improbable(kpkt->pkt_length != len)) {
    SK_ERR("%s(%d) kring %p bad pktlen %d, %d", sk_proc_name(p), sk_proc_pid(p),
  ...
  if (err == 0) {
    ...
    if (__improbable(kpkt->pkt_headroom != bdoff)) {
      SK_ERR("%s(%d) kring %p bad headroom %d, %d", sk_proc_name(p),
             sk_proc_pid(p), SK_KVA(kring), kpkt->pkt_headroom, bdoff);
      err = ERANGE;
      goto done;
    }
    if (__improbable(kpkt->pkt_headroom + kpkt->pkt_l2_len >= bdlim)) {
      SK_ERR("%s(%d) kring %p bad headroom l2len %d, %d", sk_proc_name(p),
             sk_proc_pid(p), SK_KVA(kring), kpkt->pkt_l2_len, bdlim);
      err = ERANGE;
      goto done;
    }
    ...
    if (__probable(PACKET_HAS_PARTIAL_CHECKSUM(kpkt))) {
      ...
      if (__improbable(start > stuff || start > kpkt->pkt_length ||
                       (stuff + sizeof(uint16_t)) > kpkt->pkt_length)) {
        SK_ERR("%s(%d) flags 0x%x start %u stuff %u "
               "len %u",
               sk_proc_name(p), sk_proc_pid(p), kpkt->pkt_csum_flags, start,
               stuff, kpkt->pkt_length);
        err = ERANGE;
        goto done;
      }
    } else {
...
#pragma unused(kring, maxfrags, p)
  ...
  DTRACE_SKYWALK3(externalize, struct __kern_channel_ring *, kring,
                  struct __kern_packet *, kpkt, struct __user_packet *, upkt);
  SK_DF(SK_VERB_MEM, "%s(%d) kring %p kqum %p -> uqum %p", sk_proc_name(p),
        sk_proc_pid(p), SK_KVA(kring), SK_KVA(kqum), SK_KVA(uqum));
  ...
  *(uint16_t *)(uintptr_t)&upkt->pkt_bufs_cnt = bcnt;
  ...
  if (kbuf->buf_addr == 0) {
    ubuf =
        __DECONST(struct __user_buflet *, &kpkt->pkt_qum.qum_user->qum_buf[0]);
    UBUF_INIT(kbuf, ubuf);
  }
...
void kr_externalize_metadata(struct __kern_channel_ring *kring,
                             const uint32_t maxfrags,
                             struct __kern_quantum *kqum, struct proc *p) {
  kr_externalize_metadata_internal(kring, maxfrags, kqum, p);
}
...
char *kr2str(const struct __kern_channel_ring *kr, char *__counted_by(dsz) dst,
             size_t dsz) {
  (void)sk_snprintf(dst, dsz, "%p %s %s flags 0x%b", SK_KVA(kr), kr->ckr_name,
                    sk_ring2str(kr->ckr_tx), kr->ckr_flags, CKRF_BITS);
