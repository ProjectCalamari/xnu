...
#define CHANNEL_EVENT_MAX_PAYLOAD_LEN                                          \
  (sizeof(union __os_channel_event_largest_event_payload))
...
os_channel_event_get_next_event(const os_channel_event_handle_t event_handle,
                                const os_channel_event_t prev_event,
                                os_channel_event_t *event);
extern int os_channel_event_get_event_data(const os_channel_event_t,
                                           struct os_channel_event_data *);
...
#define CHANNEL_EVENT_MAX_LEN                                                  \
  (sizeof(struct __kern_channel_event) + CHANNEL_EVENT_MAX_PAYLOAD_LEN)
...
__BEGIN_DECLS
extern errno_t
kern_channel_event_transmit_status_with_packet(const kern_packet_t,
                                               const ifnet_t);
...
extern errno_t kern_channel_event_transmit_status(
    const ifnet_t, os_channel_event_packet_transmit_status_t *, uint32_t);
/* Post a `packet transmit status' event to a flowswitch */
extern errno_t kern_channel_event_transmit_status_with_nexus(
    const uuid_t, os_channel_event_packet_transmit_status_t *, uint32_t);
/* Post a `packet transmit expired' event to an ifnet device */
extern errno_t kern_channel_event_transmit_expired(
    const ifnet_t, os_channel_event_packet_transmit_expired_t *, uint32_t);
/* Post a `packet transmit expired' event to a flowswitch */
extern errno_t kern_channel_event_transmit_expired_with_nexus(
    const uuid_t, os_channel_event_packet_transmit_expired_t *, uint32_t);
__END_DECLS
#endif /* KERNEL */
