...
#define KRING_EMPTY_TX(_kring, _index) ((_kring)->ckr_rhead == (_index))

#define KRING_FULL_RX(_kring, _index)                                          \
  ((_kring)->ckr_khead == SLOT_NEXT((_index), (_kring)->ckr_lim))

uint32_t kern_channel_notify(const kern_channel_ring_t kring, uint32_t flags) {
#pragma unused(flags)
...
uint32_t kern_channel_reclaim(const kern_channel_ring_t kring) {
...
uint32_t kern_channel_available_slot_count(const kern_channel_ring_t kring) {
  if (kring->ckr_tx == NR_TX) {
    return _kern_channel_available_slot_count_tx(kring, kring->ckr_khead);
  } else {
    return _kern_channel_available_slot_count_rx(kring, kring->ckr_ktail);
  }
}

kern_channel_slot_t kern_channel_get_next_slot(const kern_channel_ring_t kring,
                                               const kern_channel_slot_t slot0,
                                               struct kern_slot_prop *prop) {
  ...
  if (__improbable(slot0 == NULL)) {
    ...
    slot_idx = SLOT_NEXT(KR_SLOT_INDEX(kring, slot0), kring->ckr_lim);
  }
  ...
  if (kring->ckr_tx == NR_TX) {
    if (__improbable(KRING_EMPTY_TX(kring, slot_idx))) {
      SK_DF(SK_VERB_SYNC | SK_VERB_TX,
            ...
            KRNA(kring)->na_name, kring->ckr_name, slot_idx, kring->ckr_khead,
...
static inline void
_kern_channel_advance_slot_tx(const kern_channel_ring_t kring,
                              slot_idx_t index) {
  /* Ensure this is only done by the thread doing a sync syscall */
  VERIFY(sk_is_sync_protected());
...
static inline void
_kern_channel_advance_slot_rx(const kern_channel_ring_t kring,
                              slot_idx_t index) {
...
void kern_channel_advance_slot(const kern_channel_ring_t kring,
                               kern_channel_slot_t slot) {
...
void *kern_channel_get_context(const kern_channel_t ch) { return ch->ch_ctx; }

void *kern_channel_ring_get_context(const kern_channel_ring_t kring) {
  return kring->ckr_ctx;
}

errno_t kern_channel_ring_get_container(const kern_channel_ring_t kring,
                                        kern_packet_t **array,
                                        uint32_t *count) {
  /* Ensure this is only done by the thread doing a sync syscall */
  VERIFY(sk_is_sync_protected());
 ...
 */
void *kern_channel_slot_get_context(const kern_channel_ring_t kring,
...
void kern_channel_increment_ring_stats(
    kern_channel_ring_t kring, struct kern_channel_ring_stat_increment *stats) {
  kr_update_stats(kring, stats->kcrsi_slots_transferred,
                  stats->kcrsi_bytes_transferred);
}

void kern_channel_increment_ring_net_stats(
    kern_channel_ring_t kring, struct ifnet *ifp,
    struct kern_channel_ring_stat_increment *stats) {
  if (kring->ckr_tx == NR_TX) {
    os_atomic_add(&ifp->if_data.ifi_opackets, stats->kcrsi_slots_transferred,
                  relaxed);
    os_atomic_add(&ifp->if_data.ifi_obytes, stats->kcrsi_bytes_transferred,
                  relaxed);
  } else {
    os_atomic_add(&ifp->if_data.ifi_ipackets, stats->kcrsi_slots_transferred,
                  relaxed);
    os_atomic_add(&ifp->if_data.ifi_ibytes, stats->kcrsi_bytes_transferred,
                  relaxed);
  }
...
kern_packet_t kern_channel_slot_get_packet(const kern_channel_ring_t kring,
                                           const kern_channel_slot_t slot) {
...
#pragma unused(kring)
  ...
  if (__improbable(kqum == NULL || (kqum->qum_qflags & QUM_F_DROPPED) != 0)) {
    return 0;
  }
...
errno_t kern_channel_slot_attach_packet(const kern_channel_ring_t kring,
                                        const kern_channel_slot_t slot,
                                        kern_packet_t ph) {
#if (DEVELOPMENT || DEBUG)
  /* catch invalid slot */
...
errno_t kern_channel_slot_detach_packet(const kern_channel_ring_t kring,
                                        const kern_channel_slot_t slot,
                                        kern_packet_t ph) {
#pragma unused(ph)
#if (DEVELOPMENT || DEBUG)
  ...
  ASSERT(SK_PTR_ADDR_KQUM(ph) == SK_PTR_ADDR_KQUM(SD_GET_TAGGED_METADATA(ksd)));
  (void)KR_SLOT_DETACH_METADATA(kring, ksd);

  return 0;
}

static errno_t kern_channel_tx_refill_common(
    const kern_channel_ring_t hw_kring, uint32_t pkt_limit, uint32_t byte_limit,
    boolean_t tx_doorbell_ctxt, boolean_t *pkts_pending, boolean_t canblock) {
#pragma unused(tx_doorbell_ctxt)
  ...
  *pkts_pending = FALSE;
  ...
  if (__improbable(KR_DROP(hw_kring) || !NA_IS_ACTIVE(hw_kring->ckr_na))) {
    kr_exit(hw_kring);
  ...
  sync_err = hw_kring->ckr_na_sync(hw_kring, kernproc, NA_SYNCF_NETIF);
...
out:
  KDBG((SK_KTRACE_CHANNEL_TX_REFILL | DBG_FUNC_END), SK_KVA(hw_kring), rc, 0,
       0);

  return rc;
}

errno_t kern_channel_tx_refill(const kern_channel_ring_t hw_kring,
                               uint32_t pkt_limit, uint32_t byte_limit,
                               boolean_t tx_doorbell_ctxt,
                               boolean_t *pkts_pending) {
  if (NA_OWNED_BY_FSW(hw_kring->ckr_na)) {
    return netif_ring_tx_refill(hw_kring, pkt_limit, byte_limit,
                                tx_doorbell_ctxt, pkts_pending, FALSE);
  } else {
    return kern_channel_tx_refill_common(hw_kring, pkt_limit, byte_limit,
                                         tx_doorbell_ctxt, pkts_pending, FALSE);
  }
}

errno_t kern_channel_tx_refill_canblock(const kern_channel_ring_t hw_kring,
                                        uint32_t pkt_limit, uint32_t byte_limit,
                                        boolean_t tx_doorbell_ctxt,
                                        boolean_t *pkts_pending) {
  if (NA_OWNED_BY_FSW(hw_kring->ckr_na)) {
    return netif_ring_tx_refill(hw_kring, pkt_limit, byte_limit,
                                tx_doorbell_ctxt, pkts_pending, TRUE);
  } else {
    return kern_channel_tx_refill_common(hw_kring, pkt_limit, byte_limit,
                                         tx_doorbell_ctxt, pkts_pending, TRUE);
  }
...
errno_t kern_channel_get_service_class(const kern_channel_ring_t kring,
                                       kern_packet_svc_class_t *svc) {
  if ((KRNA(kring)->na_type != NA_NETIF_DEV) || (kring->ckr_tx == NR_RX) ||
      (kring->ckr_svc == KPKT_SC_UNSPEC)) {
    return ENOTSUP;
  }
  *svc = kring->ckr_svc;
  return 0;
...
static void _kern_channel_flowadv_signal(struct flowadv_fcentry *fce,
                                         flow_adv_type_t type) {
  ...
  if (ch != NULL) {
    if (ch->ch_na != NULL && flow_adv_func(ch, flow_fidx, flow_token)) {
      /* trigger flow advisory kevent */
      na_flowadv_event(&ch->ch_na->na_tx_rings[ch->ch_first[NR_TX]]);
      SK_DF(SK_VERB_FLOW_ADVISORY, "%s(%d) notified of flow update",
            ch->ch_name, ch->ch_pid);
    } else if (ch->ch_na == NULL) {
      SK_DF(SK_VERB_FLOW_ADVISORY, "%s(%d) is closing (flow update ignored)",
            ch->ch_name, ch->ch_pid);
    }
  } else {
    SK_ERR("channel token 0x%x fidx %u on %s not found", ch_token, flow_fidx,
           ifp->if_xname);
  }
done:
  if (type == FLOW_ADV_SIGNAL_RESUME) {
...
void kern_channel_flowadv_clear(struct flowadv_fcentry *fce) {
  _kern_channel_flowadv_signal(fce, FLOW_ADV_SIGNAL_RESUME);
}

void kern_channel_flowadv_set(struct flowadv_fcentry *fce) {
  _kern_channel_flowadv_signal(fce, FLOW_ADV_SIGNAL_SUSPEND);
}

void kern_channel_flowadv_report_congestion_event(struct flowadv_fcentry *fce,
  ...
  if (ch != NULL) {
    if (ch->ch_na != NULL && na_flowadv_report_congestion_event(
                                 ch, flow_fidx, flow_token, congestion_cnt,
                                 l4s_ce_cnt, total_pkt_cnt)) {
      SK_DF(SK_VERB_FLOW_ADVISORY, "%s(%d) notified of flow update",
            ch->ch_name, ch->ch_pid);
    } else if (ch->ch_na == NULL) {
      SK_DF(SK_VERB_FLOW_ADVISORY, "%s(%d) is closing (flow update ignored)",
...
void kern_channel_memstatus(struct proc *p, uint32_t status,
...
#pragma unused(p, status)
  ...
  SK_DF(SK_VERB_CHANNEL, "%s(%d) ch 0x%p flags 0x%x status %d", sk_proc_name(p),
        sk_proc_pid(p), SK_KVA(ch), ch->ch_flags, status);
...
static bool _kern_channel_defunct_eligible(struct kern_channel *ch) {
...
void kern_channel_defunct(struct proc *p, struct kern_channel *ch) {
  ...
  if (_kern_channel_defunct_eligible(ch)) {
    ...
    SK_DF(SK_VERB_CHANNEL, "%s(%d) ch 0x%p flags 0x%x", sk_proc_name(p),
          sk_proc_pid(p), SK_KVA(ch), ch->ch_flags);
     ...
     */
    if (sk_opp_defunct &&
        (!(ch_mode & CHMODE_USER_PACKET_POOL) || !pp_isempty_upp(ch->ch_pp))) {
      if (ch->ch_na->na_type == NA_FLOWSWITCH_VP) {
        struct nx_flowswitch *fsw = VPNA(ch->ch_na)->vpna_fsw;
        STATS_INC(&fsw->fsw_stats, FSW_STATS_CHAN_DEFUNCT_SKIP);
      }
      os_atomic_or(&ch->ch_flags, CHANF_DEFUNCT_SKIP, relaxed);
      /* skip defunct */
      lck_mtx_unlock(&ch->ch_lock);
      return;
    }
     ...
     */
    if ((err = skmem_arena_mredirect(ch->ch_na->na_arena, &ch->ch_mmap, p,
                                     &need_defunct)) == 0 &&
        need_defunct) {
      /*
       ...
       */
      nxdom_prov->nxdom_prov_dom->nxdom_defunct(nxdom_prov, nx, ch, p);
      /*
