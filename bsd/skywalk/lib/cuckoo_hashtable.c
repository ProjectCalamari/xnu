...
SYSCTL_NODE(_kern_skywalk, OID_AUTO, libcuckoo, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "Skywalk Cuckoo Hashtable Library");
...
static inline int cuckoo_node_chain(struct cuckoo_node *node,
...
static inline bool cuckoo_node_del(struct cuckoo_node **pnode,
...
static inline void cuckoo_node_set_next(struct cuckoo_node *node,
                                        struct cuckoo_node *next_node) {
  node->next = next_node;
}

/* We probably won't add RCU soon so use simple pointer reference for now */
static inline struct cuckoo_node *cuckoo_node_next(struct cuckoo_node *node) {
  return node->next;
}
...
static inline void __slot_set(struct _slot *slt, uint32_t hash,
                              struct cuckoo_node *node) {
  slt->_hash = hash;
  slt->_node = node;
}

static inline void __slot_reset(struct _slot *slt) {
  slt->_hash = 0;
  slt->_node = NULL;
}

static inline uint32_t __alt_hash(uint32_t hash) {
...
static inline struct _bucket *__get_bucket(struct cuckoo_hashtable *h,
                                           uint32_t b_i) {
  return &h->_buckets[b_i];
}

static inline struct _bucket *__prim_bucket(struct cuckoo_hashtable *h,
                                            uint32_t hash) {
  return __get_bucket(h, hash & h->_bitmask);
}

static inline struct _bucket *__alt_bucket(struct cuckoo_hashtable *h,
                                           uint32_t hash) {
  return __get_bucket(h, __alt_hash(hash) & h->_bitmask);
}

#if SK_LOG
static inline size_t __bucket_idx(struct cuckoo_hashtable *h,
                                  struct _bucket *b) {
  return ((uintptr_t)b - (uintptr_t)&h->_buckets[0]) / sizeof(struct _bucket);
}
#endif /* SK_LOG */

static inline struct _slot *__bucket_slot(struct _bucket *b,
                                          uint32_t slot_idx) {
  return &b->_slots[slot_idx];
}

static inline bool __slot_empty(struct _slot *s) { return s->_node == NULL; }

static inline uint32_t __align32pow2(uint32_t v) {
  v--;
  v |= v >> 1;
  v |= v >> 2;
  v |= v >> 4;
...
static inline bool __lock_bucket_with_backoff(struct _bucket *b) {
...
static inline int __resize_begin(struct cuckoo_hashtable *h) {
  ...
  while (h->_busy) {
    ...
    int error =
        msleep(&h->_resize_waiters, &h->_lock, (PZERO + 1), __FUNCTION__, NULL);
    if (error == EINTR) {
...
static inline void __resize_end(struct cuckoo_hashtable *h) {
  __unwlock_table(h);
  lck_mtx_lock(&h->_lock);
  h->_busy = false;
...
cuckoo_hashtable_create(struct cuckoo_hashtable_params *p) {
  ...
  buckets =
      sk_alloc_type_array(struct _bucket, n_buckets, Z_WAITOK, cuckoo_tag);
  if (buckets == NULL) {
...
void cuckoo_hashtable_free(struct cuckoo_hashtable *h) {
  uint32_t i;
...
static inline struct cuckoo_node *__find_in_bucket(struct cuckoo_hashtable *h,
                                                   struct _bucket *b, void *key,
                                                   uint32_t hash) {
  uint32_t i;
 ...
 *
 */
static inline int __add_to_bucket(struct cuckoo_hashtable *h, struct _bucket *b,
 ...
 */
static int cuckoo_move(struct cuckoo_hashtable *h, struct cuckoo_node *node,
                       uint32_t hash,
                       struct _bfs_node queue[_CHT_BFS_QUEUE_LEN],
                       uint8_t leaf_node_idx, uint8_t leaf_slot) {
  ...
  while (__probable(curr_node->prev_node_idx != _CHT_BFS_QUEUE_LEN)) {
    ...
    if (alt_bkt != to_bkt || !__slot_empty(__bucket_slot(to_bkt, to_slot))) {
    ...
    cht_log(CHTV_DEBUG, "Move [0x%lx][%d] to [0x%lx][%d]",
            from_bkt - h->_buckets, from_slot, to_bkt - h->_buckets, to_slot);
...
static int cuckoo_probe(struct cuckoo_hashtable *h, struct cuckoo_node *node,
                        uint32_t hash) {
  struct _bfs_node queue[_CHT_BFS_QUEUE_LEN];
  uint8_t head, tail;
  struct _bucket *b;
  ...
  if (tail == head || tail >= _CHT_BFS_QUEUE_END) {
    cht_warn("failed: cuckoo probe out of search space "
             "head %d tail %d (%d/%d, load factor %d%%)",
             head, tail, h->_n_entries, h->_capacity,
             cuckoo_hashtable_load_factor(h));
    ret = ENOMEM;
  } else {
    cht_warn("failed: cuckoo probe path invalidated "
             " (%d/%d, load factor %d%%)",
             h->_n_entries, h->_capacity, cuckoo_hashtable_load_factor(h));
    ret = EAGAIN;
  }
done:
  return ret;
...
static inline void __foreach_node(struct cuckoo_hashtable *h, bool wlocked,
                                  void (^node_handler)(struct cuckoo_node *,
                                                       uint32_t hash)) {
  if (!wlocked) {
    __rlock_table(h);
  }
...
void cuckoo_hashtable_foreach(struct cuckoo_hashtable *h,
                              void (^node_handler)(struct cuckoo_node *,
                                                   uint32_t hash)) {
  __foreach_node(h, false, node_handler);
}

static void cuckoo_dummy_retain(struct cuckoo_node *node) {
#pragma unused(node)
}

static void cuckoo_dummy_release(struct cuckoo_node *node) {
#pragma unused(node)
}

static int cuckoo_resize(struct cuckoo_hashtable *h,
                         enum cuckoo_resize_ops option) {
  int ret = 0;
  ...
  if (option == _CHT_RESIZE_EXPAND) {
    ...
    if (curr_load > _CHT_MAX_LOAD_SHRINK || curr_capacity == h->_rcapacity) {
      goto done;
    }
    new_capacity = curr_capacity / 2;
  }

  cht_info("resize %d/(%d -> %d)", h->_n_entries, curr_capacity, new_capacity);
  ...
  if (__improbable(cuckoo_hashtable_entries(h) !=
    ...
    panic("h %zu add_called %zu tmp_h %zu", cuckoo_hashtable_entries(h),
          add_called, cuckoo_hashtable_entries(tmp_h));
  }
...
static inline int cuckoo_add_no_expand(struct cuckoo_hashtable *h,
                                       struct cuckoo_node *node,
                                       uint32_t hash) {
  struct _bucket *b1, *b2;
  int ret = -1;
...
static inline int __del_from_bucket(struct cuckoo_hashtable *h,
                                    struct _bucket *b, struct cuckoo_node *node,
                                    uint32_t hash) {
  uint32_t i;
...
static inline bool cuckoo_node_looped(struct cuckoo_node *node) {
...
int cuckoo_hashtable_health_check(struct cuckoo_hashtable *h) {
  ...
  for (i = 0; i < h->_n_buckets; i++) {
    ...
    for (j = 0; j < _CHT_BUCKET_SLOTS; j++) {
      ...
      while (node != NULL) {
        seen++;
        if ((__prim_bucket(h, hash) != b) && (__alt_bucket(h, hash) != b)) {
          panic("[%d][%d] stray hash %x node %p", i, j, hash, node);
          healthy = false;
        }

        if (cuckoo_node_looped(node)) {
          panic("[%d][%d] looped hash %x node %p", i, j, hash, node);
          healthy = false;
        }
        node = cuckoo_node_next(node);
      }
