...
static struct sbuf *net_filter_event_description(uint32_t state) {
  struct sbuf *sbuf;
...
net_filter_event_callback(struct eventhandler_entry_arg arg0 __unused,
  ...
  os_log(OS_LOG_DEFAULT, "net_filter_event: new state (0x%x) %s", state,
         sbuf_data(sbuf));
  evhlog(debug,
...
static void net_filter_event_init(void) {
...
void net_filter_event_register(net_filter_event_callback_t callback) {
  net_filter_event_init();
  (void)EVENTHANDLER_REGISTER(&net_filter_evhdlr_ctxt, net_filter_event,
                              callback, eventhandler_entry_dummy_arg,
                              EVENTHANDLER_PRI_ANY);
}

static int net_filter_event_sysctl(struct sysctl_oid *oidp, void *arg1,
...
SYSCTL_PROC(_net, OID_AUTO, filter_state, CTLTYPE_STRING | CTLFLAG_LOCKED, NULL,
            0, net_filter_event_sysctl, "A", "State of the network filters");
