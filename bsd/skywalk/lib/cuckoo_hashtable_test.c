...
static int cht_obj_cmp__(struct cuckoo_node *node, void *key) {
...
static void cht_obj_retain(struct cht_obj *co) {
  (void)os_ref_retain(&co->co_refcnt);
}

static void cht_obj_retain__(struct cuckoo_node *node) {
  struct cht_obj *co = container_of(node, struct cht_obj, co_cnode);
  return cht_obj_retain(co);
}

static void cht_obj_release(struct cht_obj *co) {
  (void)os_ref_release(&co->co_refcnt);
}

static void cht_obj_release__(struct cuckoo_node *node) {
  struct cht_obj *co = container_of(node, struct cht_obj, co_cnode);
  cht_obj_release(co);
}

static int cht_obj_refcnt(struct cht_obj *co) {
...
void cht_test_fini(void) { lck_mtx_destroy(&cht_lock, &sk_lock_group); }

static void cht_obj_init() {
  // init testing objects
  cht_objs = sk_alloc_type_array(struct cht_obj, CHT_OBJ_MAX, Z_WAITOK,
                                 cuckoo_test_tag);
...
static void cht_basic_tests(void) {
  SK_ERR("start");
  ...
  // basic add/del
  struct cht_obj co1 = {.co_cnode = {NULL},
                        .co_key = -1,
                        .co_hash = 1,
                        .co_state = COS_NOT_ADDED,
                        .co_seen = 0};
  struct cht_obj co2 = {.co_cnode = {NULL},
                        .co_key = -2,
                        .co_hash = 1,
                        .co_state = COS_NOT_ADDED,
                        .co_seen = 0};
  ...
  cuckoo_hashtable_foreach(
      h, ^(struct cuckoo_node *curr_node, uint32_t curr_hash) {
...
static void cht_concurrent_ops_begin() {
  ...
  do {
    (void)msleep(&chth_run, &cht_lock, (PZERO - 1), "chthfuncw", NULL);
  } while (!chth_run);
  lck_mtx_unlock(&cht_lock);
}

static void cht_concurrent_ops_done() {
...
static void cht_concurrent_add_init(void) {
  h = cuckoo_hashtable_create(&params_template);
}

static void cht_concurrent_add(void *v, wait_result_t w) {
#pragma unused(v, w)
  ...
  for (uint32_t i = 0; i < objs_to_add; i++) {
    ...
    struct cuckoo_node *node =
        cuckoo_hashtable_find_with_hash(h, &co->co_key, co->co_hash);
    ASSERT(node != NULL);
    ASSERT(node == &co->co_cnode);
    cht_obj_release(co);
  }
...
static void cht_concurrent_add_check(void) {
  __block struct cht_obj *co = NULL;
  struct cuckoo_node *node = NULL;
...
static void cht_concurrent_add_fini(void) {
...
static void cht_concurrent_del_init(void) {
  h = cuckoo_hashtable_create(&params_template);
...
static void cht_concurrent_del(void *v, wait_result_t w) {
#pragma unused(v, w)
  cht_concurrent_ops_begin();
...
static void cht_concurrent_del_check(void) {
  ASSERT(cuckoo_hashtable_entries(h) == 0);
...
static void cht_concurrent_del_fini(void) { cuckoo_hashtable_free(h); }

static void cht_concurrent_duo_init(void) {
...
static void cht_concurrent_duo(void *v, wait_result_t w) {
#pragma unused(v, w)
...
#define DUO_IDX_SHIFT 0x8
  ...
  uint32_t *rands;
  rands = sk_alloc_data(sizeof(uint32_t) * DUO_ITERATIONS, Z_WAITOK,
                        cuckoo_test_tag);
  VERIFY(rands != NULL);
  ...
  for (uint32_t i = 0; i < DUO_ITERATIONS; i++) {
    ...
    if (ops < DUO_OPS_ADD) {
      ...
      if (os_atomic_cmpxchg(&co->co_state, COS_NOT_ADDED, COS_BUSY, acq_rel)) {
        ...
        ASSERT(node == NULL);
        int error =
            cuckoo_hashtable_add_with_hash(h, &co->co_cnode, co->co_hash);
        ASSERT(error == 0);
        ASSERT(cht_obj_refcnt(co) == 2);
...
static void cht_concurrent_duo_check(void) {
  size_t added = 0;
  for (uint32_t i = 0; i < CHT_OBJ_MAX; i++) {
    struct cht_obj *co = &cht_objs[i];
...
static void cht_concurrent_duo_fini(void) {
  for (uint32_t i = 0; i < CHT_OBJ_MAX; i++) {
    struct cht_obj *co = &cht_objs[i];
    if (co->co_state == COS_ADDED) {
      int error = cuckoo_hashtable_del(h, &co->co_cnode, co->co_hash);
...
static void cht_concurrent_tests(void (*cht_concurrent_init)(void),
                                 void (*cht_concurrent_ops)(void *v,
                                                            wait_result_t w),
  ...
  do {
    ...
    (void)msleep(&chth_cnt, &cht_lock, (PZERO - 1), "skmtstartw", &ts);
  ...
  do {
    struct timespec ts = {0, 100 * USEC_PER_SEC};
    (void)msleep(&chth_cnt, &cht_lock, (PZERO - 1), "skmtstopw", &ts);
  } while (chth_cnt != 0);
  chth_run = FALSE;
  lck_mtx_unlock(&cht_lock);
...
static void cuckoo_test_start(void *v, wait_result_t w) {
#pragma unused(v, w)
  lck_mtx_lock(&cht_lock);
  VERIFY(!cht_busy);
  cht_busy = 1;
  ...
  cht_concurrent_tests(cht_concurrent_add_init, cht_concurrent_add,
                       cht_concurrent_add_check, cht_concurrent_add_fini);
  cht_concurrent_tests(cht_concurrent_del_init, cht_concurrent_del,
                       cht_concurrent_del_check, cht_concurrent_del_fini);
  cht_concurrent_tests(cht_concurrent_duo_init, cht_concurrent_duo,
                       cht_concurrent_duo_check, cht_concurrent_duo_fini);
...
static void cuckoo_test_stop(void *v, wait_result_t w) {
#pragma unused(v, w)
...
static int sysctl_cuckoo_test(__unused struct sysctl_oid *oidp,
  ...
  lck_mtx_lock(&cht_lock);
  if ((error = sysctl_io_number(req, cht_enabled, sizeof(int), &newvalue,
                                &changed)) != 0) {
  ...
  if (changed && cht_enabled != newvalue) {
    ...
    do {
      SK_ERR("waiting for %s to complete", newvalue ? "startup" : "shutdown");
      error = msleep(&cht_enabled, &cht_lock, PWAIT | PCATCH, "skmtw", NULL);
      /* BEGIN CSTYLED */
      /*
