...
static LCK_RW_DECLARE_ATTR(nxctl_traffic_rule_lock, &sk_lock_group,
                           &sk_lock_attr);
...
SK_INLINE_ATTRIBUTE
void nxtr_wunlock(void) { lck_rw_unlock_exclusive(&nxctl_traffic_rule_lock); }

SK_INLINE_ATTRIBUTE
void nxtr_rlock(void) { lck_rw_lock_shared(&nxctl_traffic_rule_lock); }
...
void nxctl_traffic_rule_init(void) {
...
void nxctl_traffic_rule_fini(void) {
...
nxctl_traffic_rule_storage_create(void) {
  ...
  rs = sk_alloc_type(struct nxctl_traffic_rule_storage, Z_WAITOK | Z_NOFAIL,
                     nxctl_traffic_rule_tag);
 ...
 */
void nxctl_traffic_rule_clean(struct nxctl *nxctl) {
  struct nxctl_traffic_rule_storage *rs;
...
SK_NO_INLINE_ATTRIBUTE
static void add_traffic_rule_to_nxctl(struct nxctl *nxctl,
...
SK_NO_INLINE_ATTRIBUTE
static void remove_traffic_rule_from_nxctl(struct nxctl *nxctl,
                                           struct nxctl_traffic_rule *ntr) {
  struct nxctl_traffic_rule_storage *rs;
...
void retain_traffic_rule(struct nxctl_traffic_rule *ntr) {
  ...
  DTRACE_SKYWALK2(ntr__retain, struct nxctl_traffic_rule *, ntr, os_ref_count_t,
                  count);
#endif
...
static void link_traffic_rule(struct nxctl *nxctl,
                              struct nxctl_traffic_rule *ntr) {
  /*
...
static void unlink_traffic_rule(struct nxctl *nxctl,
                                struct nxctl_traffic_rule *ntr) {
  if ((ntr->ntr_flags & NTR_FLAG_PERSIST) == 0) {
...
static int find_traffic_rule_by_uuid(uuid_t uuid,
                                     struct nxctl_traffic_rule **ntrp) {
  int i, err;
...
SK_NO_INLINE_ATTRIBUTE
static int add_traffic_rule(struct nxctl *nxctl, const char *ifname,
...
static uint32_t convert_traffic_rule_ioc_flags(uint32_t flags) {
  uint32_t f = 0;
...
SK_NO_INLINE_ATTRIBUTE
static int add_traffic_rule_generic(struct nxctl *nxctl, const char *ifname,
                                    struct ifnet_traffic_descriptor_common *td,
                                    struct ifnet_traffic_rule_action *ra,
                                    uint32_t flags, uuid_t *uuid) {
  struct nxctl_traffic_rule *__single ntr;
  int err;
...
int nxioctl_add_traffic_rule_inet(struct nxctl *nxctl, caddr_t data,
                                  proc_t procp) {
#pragma unused(procp)
  ...
  return add_traffic_rule_generic(
      nxctl, atri_ifname, &args->atri_td.inet_common, &args->atri_ra.ras_common,
      convert_traffic_rule_ioc_flags(args->atri_flags), &args->atri_uuid);
}

int nxioctl_add_traffic_rule_eth(struct nxctl *nxctl, caddr_t data,
                                 proc_t procp) {
#pragma unused(procp)
  ...
  return add_traffic_rule_generic(
      nxctl, atre_ifname, &args->atre_td.eth_common, &args->atre_ra.ras_common,
      convert_traffic_rule_ioc_flags(args->atre_flags), &args->atre_uuid);
}

int nxioctl_remove_traffic_rule(struct nxctl *nxctl, caddr_t data,
                                proc_t procp) {
#pragma unused(procp)
...
int nxioctl_get_traffic_rules(struct nxctl *nxctl, caddr_t data, proc_t procp) {
#pragma unused(nxctl)
  ...
  uaddr =
      proc_is64bit(procp) ? args->gtr_buf64 : CAST_USER_ADDR_T(args->gtr_buf);
