...
#define NXPF_BITS "\020\01ANONYMOUS\02USER_CHANNEL"
...
#define NXPCAP_BITS "\020\01CHECKSUM_PARTIAL\02USER_PKT_POOL\03USER_CHANNEL"
...
#ifdef KERNEL
#define NBR_MATCH_MASK (NBR_MATCH_PID | NBR_MATCH_EXEC_UUID | NBR_MATCH_KEY)
#endif /* KERNEL */
...
#define NXMIB_USERSTACK_STATS                                                  \
  (NXMIB_IP_STATS | NXMIB_IP6_STATS | NXMIB_TCP_STATS | NXMIB_UDP_STATS |      \
   NXMIB_QUIC_STATS)
...
#define NXFLOWREQF_OVERRIDE_ADDRESS_SELECTION                                  \
  0x00000040 /* override system address selection */
#define NXFLOWREQF_USE_STABLE_ADDRESS                                          \
  0x00000080 /* if override local, use stable address */
...
#define NXFLOWREQF_NOWAKEFROMSLEEP                                             \
  0x00000800 /* Don't wake for traffic to this flow */
#define NXFLOWREQF_REUSEPORT                                                   \
  0x00001000                         /* Don't wake for traffic to this flow */
...
#define NXFLOWREQF_BITS                                                        \
  ...
  "\015REUSEPORT\017PARENT\020AOP_OFFLOAD\021CONNECTION_IDLE\022CONNECTION_"   \
  "REUSED"
...
static inline void nx_flow_req_internalize(struct nx_flow_req *req) {
...
static inline void nx_flow_req_externalize(struct nx_flow_req *req) {
...
extern int __nexus_register(int ctl, struct nxprov_reg *reg,
                            const uint32_t reg_len, uuid_t *prov_uuid,
...
extern int __nexus_create(int ctl, const uuid_t prov_uuid,
                          const uint32_t prov_uuid_len, uuid_t *nx_uuid,
...
extern int __os_nexus_ifattach(const nexus_controller_t ctl,
                               const uuid_t nx_uuid, const char *ifname,
                               const uuid_t netif_uuid, boolean_t host,
                               uuid_t *nx_if_uuid);
