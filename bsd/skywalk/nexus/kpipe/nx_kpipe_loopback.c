...
#define KPLO_GENERATE_CTX(addr) (void *)((uintptr_t)(addr) ^ kplo_seed)
#define KPLO_WHICH_RING(_ring) ((_ring) == kplo_rxring ? "RX" : "TX")

#define KPLO_INJECT_ERROR(_err)                                                \
  do {                                                                         \
...
static errno_t kplo_dom_init(kern_nexus_domain_provider_t domprov) {
...
static void kplo_dom_fini(kern_nexus_domain_provider_t domprov) {
#pragma unused(domprov)
  lck_mtx_lock(&kplo_lock);
...
static errno_t kplo_pre_connect(kern_nexus_provider_t nxprov, proc_t p,
                                kern_nexus_t nexus, nexus_port_t nexus_port,
...
#pragma unused(nxprov, p, nexus_port)
  ...
  *ch_ctx = KPLO_GENERATE_CTX(channel);
  SK_DF(SK_VERB_KERNEL_PIPE, "nx_port %u ch %p ch_ctx 0x%llx", nexus_port,
        SK_KVA(channel), (uint64_t)(*ch_ctx));
...
static errno_t kplo_connected(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
...
static void kplo_pre_disconnect(kern_nexus_provider_t nxprov,
                                kern_nexus_t nexus, kern_channel_t channel) {
#pragma unused(nxprov)
  KPLO_VERIFY_CTX(kplo_nx_ctx, kern_nexus_get_context(nexus));
  KPLO_VERIFY_CTX(channel, kern_channel_get_context(channel));
  SK_DF(SK_VERB_KERNEL_PIPE, "called for channel %p", SK_KVA(channel));
}

static void kplo_disconnected(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
                              kern_channel_t channel) {
#pragma unused(nxprov)
  KPLO_VERIFY_CTX(kplo_nx_ctx, kern_nexus_get_context(nexus));
  KPLO_VERIFY_CTX(channel, kern_channel_get_context(channel));
...
static errno_t kplo_ring_init(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
...
static void kplo_ring_fini(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
                           kern_channel_ring_t ring) {
#pragma unused(nxprov)
  KPLO_VERIFY_CTX(kplo_nx_ctx, kern_nexus_get_context(nexus));
  KPLO_VERIFY_CTX(ring, kern_channel_ring_get_context(ring));
  SK_DF(SK_VERB_KERNEL_PIPE, "%s_ring %p", KPLO_WHICH_RING(ring), SK_KVA(ring));
...
static errno_t kplo_slot_init(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
...
#pragma unused(nxprov)
  ...
  *slot_prop_addr = NULL;
  SK_DF(SK_VERB_KERNEL_PIPE, "  slot %p id %u slot_ctx %p [%u]", SK_KVA(slot),
        slot_id, SK_KVA(*pslot_ctx), kplo_drv_slots);
  lck_mtx_unlock(&kplo_lock);
...
static void kplo_slot_fini(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
                           kern_channel_ring_t ring, channel_slot_t slot,
                           uint32_t slot_id) {
#pragma unused(nxprov, nexus, slot_id)
  ...
  SK_DF(SK_VERB_KERNEL_PIPE, "  slot %p id %u [%u]", SK_KVA(slot), slot_id,
        kplo_drv_slots);
  lck_mtx_unlock(&kplo_lock);
}

static errno_t kplo_sync_tx(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
...
#pragma unused(ring, flags)
  ...
  SK_DF(SK_VERB_KERNEL_PIPE | SK_VERB_SYNC | SK_VERB_TX,
        "called with ring \"%s\" krflags 0x%x flags 0x%x", ring->ckr_name,
        ring->ckr_flags, flags);
  ...
  SK_DF(SK_VERB_KERNEL_PIPE | SK_VERB_SYNC | SK_VERB_TX, "%p: %s %x -> %s",
        SK_KVA(txkring), txkring->ckr_name, flags, rxkring->ckr_name);
  SK_DF(SK_VERB_KERNEL_PIPE | SK_VERB_SYNC | SK_VERB_TX,
        "tx before: kh %3u kt %3u | h %3u t %3u", txkring->ckr_khead,
        txkring->ckr_ktail, txkring->ckr_rhead, txkring->ckr_rtail);
  SK_DF(SK_VERB_KERNEL_PIPE | SK_VERB_SYNC | SK_VERB_TX,
        "rx before: kh %3u kt %3u | h %3u t %3u", rxkring->ckr_khead,
  ...
  do {
    ...
    VERIFY(kern_buflet_set_data_length(buf, (uint16_t)(kplo_bufsz + 1)) ==
           ERANGE);
    ...
    VERIFY(kern_buflet_set_data_length(buf, 0) == 0);
    VERIFY(kern_buflet_set_data_offset(buf, (uint16_t)(kplo_bufsz + 1)) ==
           ERANGE);
    VERIFY(kern_buflet_set_data_length(buf, dlen) == 0);
  ...
  SK_DF(SK_VERB_KERNEL_PIPE | SK_VERB_SYNC | SK_VERB_TX,
        "tx after:  kh %3u kt %3u | h %3u t %3u", txkring->ckr_khead,
        txkring->ckr_ktail, txkring->ckr_rhead, txkring->ckr_rtail);
  SK_DF(SK_VERB_KERNEL_PIPE | SK_VERB_SYNC | SK_VERB_TX,
        "rx after:  kh %3u kt %3u | h %3u t %3u", rxkring->ckr_khead,
        rxkring->ckr_ktail, rxkring->ckr_rhead, rxkring->ckr_rtail);
...
static errno_t kplo_sync_rx(kern_nexus_provider_t nxprov, kern_nexus_t nexus,
                            kern_channel_ring_t ring, uint32_t flags) {
  errno_t error;
...
static void kpipe_loopback_start(void) {
  ...
  error = kern_nexus_register_domain_provider(
      NEXUS_TYPE_KERNEL_PIPE, (const uint8_t *)"kpipe_loopback", &dom_init,
      sizeof(dom_init), &kplo_dom_prov_uuid);
  if (error != 0) {
  ...
  SK_DF(SK_VERB_KERNEL_PIPE, "Registered kpipe_loopback domain with uuid %s",
        uuidstr);
  ...
  SK_DF(SK_VERB_KERNEL_PIPE, "Created nexus controller with uuid %s", uuidstr);

  // We don't actually do anything with this.
  uuid_clear(uuidtmp);
  error =
      kern_nexus_get_default_domain_provider(NEXUS_TYPE_KERNEL_PIPE, &uuidtmp);
  if (error) {
  ...
  SK_DF(SK_VERB_KERNEL_PIPE, "Found kernel pipe domain with uuid %s", uuidstr);
  ...
  if (error) {
    SK_ERR("Failed to %s anonymous attribute %d", (kplo_anon ? "set" : "clear"),
           error);
    goto done;
  }

  VERIFY(uuid_is_null(kplo_prov_uuid));
  error = kern_nexus_controller_register_provider(
      kplo_ncd, kplo_dom_prov_uuid,
  ...
  error =
      kern_nexus_controller_read_provider_attr(kplo_ncd, kplo_prov_uuid, nxa);
  if (error != 0) {
    SK_ERR("Failed to read nexus provider attributes %d", error);
    goto done;
  }

  if ((error = kern_nexus_attr_get(nxa, NEXUS_ATTR_TX_RINGS, &kplo_ntxrings)) !=
      0) {
    SK_ERR("Failed to retrieve NEXUS_ATTR_TX_RINGS %d", error);
    goto done;
  }
  if ((error = kern_nexus_attr_get(nxa, NEXUS_ATTR_TX_SLOTS, &kplo_ntxslots)) !=
      0) {
    SK_ERR("Failed to retrieve NEXUS_ATTR_TX_SLOTS %d", error);
    goto done;
  }
  if ((error = kern_nexus_attr_get(nxa, NEXUS_ATTR_RX_RINGS, &kplo_nrxrings)) !=
      0) {
    SK_ERR("Failed to retrieve NEXUS_ATTR_RX_RINGS %d", error);
    goto done;
  }
  if ((error = kern_nexus_attr_get(nxa, NEXUS_ATTR_RX_SLOTS, &kplo_nrxslots)) !=
      0) {
  ...
  if ((error = kern_nexus_attr_get(nxa, NEXUS_ATTR_ANONYMOUS, &kplo_anon)) !=
      0) {
    SK_ERR("Failed to retrieve NEXUS_ATTR_ANONYMOUS %d", error);
    goto done;
  }
  if ((error = kern_nexus_attr_get(nxa, NEXUS_ATTR_PIPES, &kplo_pipes)) != 0) {
  ...
  error = kern_nexus_controller_alloc_provider_instance(
      kplo_ncd, kplo_prov_uuid, KPLO_GENERATE_CTX(kplo_nx_ctx), NULL,
      &kplo_nx_uuid, &nx_init);
  if (error) {
  ...
  SK_DF(SK_VERB_KERNEL_PIPE, "Allocated provider instance uuid %s",
        kplo_nx_uuidstr);
...
static void kpipe_loopback_stop(void) {
  ...
  if (!uuid_is_null(kplo_nx_uuid)) {
    ...
    SK_DF(SK_VERB_KERNEL_PIPE, "Deallocated provider instance uuid %s",
          uuidstr);
    error =
        kern_nexus_controller_free_provider_instance(kplo_ncd, kplo_nx_uuid);
    VERIFY(error == 0);
  ...
  if (!uuid_is_null(kplo_prov_uuid)) {
    uuid_unparse_upper(kplo_prov_uuid, uuidstr);
    SK_DF(SK_VERB_KERNEL_PIPE, "Unregistered nexus controller with uuid %s",
          uuidstr);
    error = kern_nexus_controller_deregister_provider(kplo_ncd, kplo_prov_uuid);
    VERIFY(error == 0);
  ...
  if (kplo_ncd) {
    // XXX opaque violation on kplo_ncd
    uuid_unparse_upper(kplo_ncd->ncd_nxctl->nxctl_uuid, uuidstr);
    SK_DF(SK_VERB_KERNEL_PIPE, "Destroying nexus controller with uuid %s",
          uuidstr);
  ...
  if (!uuid_is_null(kplo_dom_prov_uuid)) {
    ...
    uuid_unparse_upper(kplo_dom_prov_uuid, uuidstr);
    SK_DF(SK_VERB_KERNEL_PIPE, "Unregistered domain provider with uuid %s",
          uuidstr);
    error = kern_nexus_deregister_domain_provider(kplo_dom_prov_uuid);
    VERIFY(error == 0);
...
static int sysctl_kpipe_loopback_enabled(__unused struct sysctl_oid *oidp,
                                         __unused void *arg1, __unused int arg2,
                                         struct sysctl_req *req) {
  int error, newvalue, changed;

  lck_mtx_lock(&kplo_lock);
  if ((error = sysctl_io_number(req, kplo_enabled, sizeof(int), &newvalue,
                                &changed)) != 0) {
    goto done;
  }
  ...
  if (changed && kplo_enabled != newvalue) {
    ...
    if (kernel_thread_start((thread_continue_t)func, NULL, &kpth) !=
        KERN_SUCCESS) {
    ...
    do {
      ...
      error = msleep(&kplo_enabled, &kplo_lock, PWAIT | PCATCH, "kplow", NULL);
      /* BEGIN CSTYLED */
      /*
...
SYSCTL_PROC(_kern_skywalk_kpipe_loopback, OID_AUTO, enabled,
            ...
            sysctl_kpipe_loopback_enabled, "I",
            "Start the loopback kernel pipe");

SYSCTL_STRING(_kern_skywalk_kpipe_loopback, OID_AUTO, nx_uuid,
              CTLFLAG_RD | CTLFLAG_KERN | CTLFLAG_LOCKED, &kplo_nx_uuidstr[0],
