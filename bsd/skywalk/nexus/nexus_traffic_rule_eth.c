...
#define NETHRULEMASKS (sizeof(nxctl_eth_traffic_rule_masks) / sizeof(uint8_t))
 ...
 */
int nxctl_eth_traffic_rule_get_count(const char *ifname, uint32_t *count) {
  int err;
...
/*
 * Used for finding the qset id associated with a ether type and ether remote
 * addr.
 */
int nxctl_eth_traffic_rule_find_qset_id(const char *ifname, uint16_t eth_type,
...
static int parse_eth_hdr(struct __kern_packet *pkt, uint16_t *eth_type,
                         ether_addr_t *eth_raddr) {
...
fail:
  DTRACE_SKYWALK4(classify__failed, ether_header_t *, _l2, size_t,
                  pkt->pkt_length, uint8_t, pkt->pkt_l2_len, int, err);
  return err;
}

int nxctl_eth_traffic_rule_find_qset_id_with_pkt(const char *ifname,
  ...
  return nxctl_eth_traffic_rule_find_qset_id(ifname, eth_type, &eth_raddr,
                                             qset_id);
}
...
int eth_traffic_rule_validate(const char *ifname,
  ...
  if ((tdi->eth_mask & IFNET_TRAFFIC_DESCRIPTOR_ETH_MASK_ETHER_TYPE) != 0) {
    if (tdi->eth_type != ETHERTYPE_PAE && tdi->eth_type != ETHERTYPE_WAI) {
...
static void eth_traffic_rule_storage_create(void) {
  rs = sk_alloc_type(struct nxctl_traffic_rule_eth_storage, Z_WAITOK | Z_NOFAIL,
...
SK_NO_INLINE_ATTRIBUTE
static void eth_traffic_rule_storage_destroy(void) {
...
eth_traffic_rule_if_create(const char *ifname) {
  ...
  rif = sk_alloc_type(struct nxctl_traffic_rule_eth_if, Z_WAITOK | Z_NOFAIL,
                      nxctl_traffic_rule_tag);
  for (i = 0; i < NETHRULEMASKS; i++) {
...
SK_NO_INLINE_ATTRIBUTE
static void eth_traffic_rule_if_destroy(struct nxctl_traffic_rule_eth_if *rif) {
  int i;
...
eth_traffic_rule_match(struct nxctl_traffic_rule_eth *ntre, const char *ifname,
  ...
  if ((mask & IFNET_TRAFFIC_DESCRIPTOR_ETH_MASK_ETHER_TYPE) != 0 &&
    ...
    DTRACE_SKYWALK2(eth_type__mismatch, uint8_t, tdi0->eth_type, uint8_t,
                    tdi->eth_type);
    return FALSE;
  }
  if ((mask & IFNET_TRAFFIC_DESCRIPTOR_ETH_MASK_RADDR) != 0 &&
      _ether_cmp(&tdi0->eth_raddr, &tdi->eth_raddr)) {
    DTRACE_SKYWALK2(eth_raddr__mismatch, ether_addr_t *, &tdi0->eth_raddr,
...
static void eth_update_ifnet_traffic_rule_count(const char *ifname,
                                                uint32_t count) {
  struct ifnet *ifp;
...
void eth_traffic_rule_link(struct nxctl_traffic_rule *ntr) {
  struct nxctl_traffic_rule_eth_if *rif;
  struct nxctl_traffic_rule_eth *ntre = (struct nxctl_traffic_rule_eth *)ntr;
  ...
  for (i = 0; i < NETHRULEMASKS; i++) {
    if (ntre->ntre_td.eth_mask == nxctl_eth_traffic_rule_masks[i]) {
...
int eth_traffic_rule_notify(struct nxctl_traffic_rule *ntr, uint32_t flags) {
  ...
  err = nx_netif_notify_steering_info(
      nif, qset, (struct ifnet_traffic_descriptor_common *)&ntre->ntre_td,
...
int eth_traffic_rule_create(const char *ifname,
  ...
  ntre = sk_alloc_type(struct nxctl_traffic_rule_eth, Z_WAITOK | Z_NOFAIL,
                       nxctl_traffic_rule_tag);
  ntr = &ntre->ntre_common;
...
int eth_traffic_rule_get_all(uint32_t size, uint32_t *count,
                             user_addr_t uaddr) {
  struct nxctl_traffic_rule_eth *ntre = NULL;
  struct nxctl_traffic_rule_eth_if *rif;
  struct nxctl_traffic_rule_eth_iocinfo info;
