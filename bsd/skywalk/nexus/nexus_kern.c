...
static STAILQ_HEAD(, nxdom)
    nexus_domains = STAILQ_HEAD_INITIALIZER(nexus_domains);
...
static int _kern_nexus_ifattach(struct nxctl *nxctl, const uuid_t nx_uuid,
                                struct ifnet *ifp,
                                const uuid_t nx_uuid_attachee, boolean_t host,
                                uuid_t *nx_if_uuid);
...
static STAILQ_HEAD(, kern_nexus_domain_provider)
    nxprov_detaching_head = STAILQ_HEAD_INITIALIZER(nxprov_detaching_head);
...
#define ASSERT_NXDOM_PARAMS(_dom, _var)                                        \
  do {                                                                         \
...
static void nxdom_attach(struct nxdom *nxdom) {
  struct nxdom *nxdom1;
...
static void nxdom_detach(struct nxdom *nxdom) {
  SK_LOCK_ASSERT_HELD();
  ASSERT(nxdom->nxdom_flags & NEXUSDOMF_ATTACHED);
...
static void nxdom_init(struct nxdom *nxdom) {
  ASSERT(nxdom->nxdom_flags & NEXUSDOMF_ATTACHED);
...
static void nxdom_terminate(struct nxdom *nxdom) {
  ASSERT(nxdom->nxdom_flags & NEXUSDOMF_ATTACHED);
...
static void nxdom_fini(struct nxdom *nxdom) {
  ASSERT(nxdom->nxdom_flags & NEXUSDOMF_ATTACHED);
...
int nxdom_prov_add(struct nxdom *nxdom,
  ...
  if (err == 0) {
    ...
    STAILQ_INSERT_TAIL(&nxdom->nxdom_prov_head, nxdom_prov, nxdom_prov_link);
    ...
    SK_D("nxdom_prov %p (%s) dom %s", SK_KVA(nxdom_prov),
         nxdom_prov->nxdom_prov_name, nxdom->nxdom_name);
  } else {
...
void nxdom_prov_del(struct kern_nexus_domain_provider *nxdom_prov) {
  ...
  STAILQ_REMOVE(&nxdom->nxdom_prov_head, nxdom_prov, kern_nexus_domain_provider,
                nxdom_prov_link);
...
struct nxdom *nxdom_find(nexus_type_t type) {
  struct nxdom *nxdom;
...
struct kern_nexus_domain_provider *nxdom_prov_find(const struct nxdom *nxdom,
                                                   const char *name) {
  ...
  if (name != NULL) {
    STAILQ_FOREACH(nxdom_prov, &nxdom->nxdom_prov_head, nxdom_prov_link) {
...
nxdom_prov_find_uuid(const uuid_t dom_prov_uuid) {
  ...
  STAILQ_FOREACH(nxdom, &nexus_domains, nxdom_link) {
    STAILQ_FOREACH(nxdom_prov, &nxdom->nxdom_prov_head, nxdom_prov_link) {
      ...
      if (uuid_compare(nxdom_prov->nxdom_prov_uuid, dom_prov_uuid) == 0) {
        break;
      }
    }
    if (nxdom_prov != NULL) {
...
errno_t kern_nexus_register_domain_provider(
  ...
  bcopy(&nxdom_prov_default[type]->nxdom_prov_cb, &nxdom_prov->nxdom_prov_cb,
        sizeof(struct nxdom_prov_cb));
...
errno_t kern_nexus_deregister_domain_provider(const uuid_t dom_prov_uuid) {
  struct kern_nexus_domain_provider *nxdom_prov = NULL;
  errno_t err = 0;
...
errno_t kern_nexus_get_default_domain_provider(const nexus_type_t type,
                                               uuid_t *dom_prov_uuid) {
  struct kern_nexus_domain_provider *nxdom_prov;
...
static int nxdom_prov_ext_init(struct kern_nexus_domain_provider *nxdom_prov) {
  int err = 0;
...
static void nxdom_prov_ext_fini(struct kern_nexus_domain_provider *nxdom_prov) {
...
static struct nexus_attr *nxa_alloc(zalloc_flags_t how) {
  return zalloc_flags(nxa_zone, how | Z_ZERO);
}

static void nxa_free(struct nexus_attr *nxa) {
  SK_DF(SK_VERB_MEM, "nxa %p FREE", SK_KVA(nxa));
  zfree(nxa_zone, nxa);
}

errno_t kern_nexus_attr_create(nexus_attr_t *nxa) {
  errno_t err = 0;
...
errno_t kern_nexus_attr_clone(const nexus_attr_t nxa, nexus_attr_t *nnxa) {
  errno_t err = 0;
...
errno_t kern_nexus_attr_set(const nexus_attr_t nxa,
                            const nexus_attr_type_t type,
                            const uint64_t value) {
  return __nexus_attr_set(nxa, type, value);
}

errno_t kern_nexus_attr_get(nexus_attr_t nxa, const nexus_attr_type_t type,
                            uint64_t *value) {
  return __nexus_attr_get(nxa, type, value);
}

void kern_nexus_attr_destroy(nexus_attr_t nxa) { nxa_free(nxa); }

static struct nexus_controller *ncd_alloc(zalloc_flags_t how) {
  return zalloc_flags(ncd_zone, how | Z_ZERO);
}

static void ncd_free(struct nexus_controller *ncd) {
  SK_DF(SK_VERB_MEM, "ncd %p FREE", SK_KVA(ncd));
  zfree(ncd_zone, ncd);
}

nexus_controller_t kern_nexus_shared_controller(void) { return &kernnxctl; }

errno_t kern_nexus_controller_create(nexus_controller_t *ncd) {
...
static errno_t nexus_controller_register_provider_validate_init_params(
  ...
  static_assert(
      __builtin_offsetof(struct kern_nexus_provider_init, nxpi_version) == 0);
  ...
  case KERN_NEXUS_PROVIDER_VERSION_1:
    ...
    if (init->nxpi_sync_tx == NULL || init->nxpi_sync_rx == NULL ||
        init->nxpi_pre_connect == NULL || init->nxpi_connected == NULL ||
        init->nxpi_pre_disconnect == NULL || init->nxpi_disconnected == NULL ||
  ...
  case KERN_NEXUS_PROVIDER_VERSION_NETIF:
    ...
    netif_init = __DECONST(struct kern_nexus_netif_provider_init *, init);
...
errno_t kern_nexus_controller_register_provider(
    const nexus_controller_t ncd, const uuid_t dom_prov_uuid,
    const nexus_name_t name, const struct kern_nexus_provider_init *init,
    uint32_t init_len, const nexus_attr_t nxa, uuid_t *prov_uuid) {
  struct kern_nexus_domain_provider *nxdom_prov = NULL;
  ...
  if (ncd == NULL || dom_prov_uuid == NULL || uuid_is_null(dom_prov_uuid)) {
    return EINVAL;
  }
  ...
  err = nexus_controller_register_provider_validate_init_params(init, init_len,
                                                                nxdom_type);
  if (err != 0) {
    SK_UNLOCK();
    err = EINVAL;
  ...
  if ((err = __nexus_provider_reg_prepare(
           &reg, __unsafe_null_terminated_from_indexable(name), nxdom_type,
           nxa)) != 0) {
    SK_UNLOCK();
    goto done;
  }
  ...
  if ((nxprov = nxprov_create_kern(nxctl, nxdom_prov, &reg, init, &err)) ==
      NULL) {
    SK_UNLOCK();
    ASSERT(err != 0);
    goto done;
...
errno_t kern_nexus_controller_deregister_provider(const nexus_controller_t ncd,
...
errno_t kern_nexus_controller_alloc_provider_instance(
  ...
  nxctl = ncd->ncd_nxctl;
  NXCTL_LOCK(nxctl);
  nx = nx_create(nxctl, prov_uuid, NEXUS_TYPE_UNDEFINED, nx_ctx, nx_ctx_release,
                 init->nxi_tx_pbufpool, init->nxi_rx_pbufpool, &err);
  NXCTL_UNLOCK(nxctl);
  if (nx == NULL) {
    ASSERT(err != 0);
    goto done;
...
errno_t kern_nexus_controller_alloc_net_provider_instance(
    const nexus_controller_t ncd, const uuid_t prov_uuid, const void *nx_ctx,
    nexus_ctx_release_fn_t nx_ctx_release, uuid_t *nx_uuid,
  ...
  nx = nx_create(nxctl, prov_uuid, NEXUS_TYPE_NET_IF, nx_ctx, nx_ctx_release,
                 init->nxneti_tx_pbufpool, init->nxneti_rx_pbufpool, &err);
  if (nx == NULL) {
    ASSERT(err != 0);
    goto done;
  }
  ...
  if (NX_LLINK_PROV(nx)) {
    ...
    err =
        nx_netif_default_llink_config(NX_NETIF_PRIVATE(nx), init->nxneti_llink);
    if (err != 0) {
      goto done;
    }
  }
   ...
   */
  err = _kern_nexus_ifattach(
      nxctl, nx->nx_uuid, ifp,
...
errno_t kern_nexus_controller_bind_provider_instance(
    const nexus_controller_t ncd, const uuid_t nx_uuid, nexus_port_t *port,
  ...
  if (ncd == NULL || nx_uuid == NULL || uuid_is_null(nx_uuid) || port == NULL) {
    return EINVAL;
  }

  __nexus_bind_req_prepare(&nbr, nx_uuid, *port, pid, exec_uuid, key, key_len,
                           bind_flags);
...
errno_t kern_nexus_controller_read_provider_attr(const nexus_controller_t ncd,
                                                 const uuid_t prov_uuid,
                                                 nexus_attr_t nxa) {
...
void kern_nexus_controller_destroy(nexus_controller_t ncd) {
  struct nxctl *nxctl;
...
void *kern_nexus_get_context(const kern_nexus_t nx) { return nx->nx_ctx; }

void kern_nexus_stop(const kern_nexus_t nx) {
  SK_LOCK();
  nx_stop(nx);
  SK_UNLOCK();
}

errno_t kern_nexus_get_pbufpool(const kern_nexus_t nx, kern_pbufpool_t *ptx_pp,
...
static int _kern_nexus_ifattach(struct nxctl *nxctl, const uuid_t nx_uuid,
  ...
  __nexus_config_req_prepare(&ncr, nx_uuid, NXCFG_CMD_ATTACH, &nsr,
                             sizeof(nsr));
...
int kern_nexus_ifattach(nexus_controller_t ncd, const uuid_t nx_uuid,
                        struct ifnet *ifp, const uuid_t nx_uuid_attachee,
  ...
  ASSERT(nxctl != NULL);
  NXCTL_LOCK(nxctl);
  err = _kern_nexus_ifattach(nxctl, nx_uuid, ifp, nx_uuid_attachee, host,
                             nx_if_uuid);
  NXCTL_UNLOCK(nxctl);
...
int kern_nexus_ifdetach(const nexus_controller_t ncd, const uuid_t nx_uuid,
  ...
  __nexus_config_req_prepare(&ncr, nx_uuid, NXCFG_CMD_DETACH, &nsr,
                             sizeof(nsr));
...
int kern_nexus_get_netif_instance(struct ifnet *ifp, uuid_t nx_uuid) {
...
int kern_nexus_get_flowswitch_instance(struct ifnet *ifp, uuid_t nx_uuid) {
  struct nexus_netif_adapter *if_na;
...
static void kern_nexus_netagent_add(struct kern_nexus *nx, void *arg0) {
#pragma unused(arg0)
  nx_fsw_netagent_add(nx);
}

static void kern_nexus_netagent_remove(struct kern_nexus *nx, void *arg0) {
#pragma unused(arg0)
  nx_fsw_netagent_remove(nx);
}

static void kern_nexus_netagent_update(struct kern_nexus *nx, void *arg0) {
#pragma unused(arg0)
  nx_fsw_netagent_update(nx);
}

void kern_nexus_register_netagents(void) {
  kern_nexus_walktree(kern_nexus_netagent_add, NULL, FALSE);
}

void kern_nexus_deregister_netagents(void) {
  kern_nexus_walktree(kern_nexus_netagent_remove, NULL, FALSE);
}

void kern_nexus_update_netagents(void) {
  kern_nexus_walktree(kern_nexus_netagent_update, NULL, FALSE);
}

static int _interface_add_remove_netagent(struct ifnet *ifp, bool add) {
  struct nexus_netif_adapter *if_na;
  int err = ENXIO;
...
int kern_nexus_interface_add_netagent(struct ifnet *ifp) {
  return _interface_add_remove_netagent(ifp, true);
}

int kern_nexus_interface_remove_netagent(struct ifnet *ifp) {
  return _interface_add_remove_netagent(ifp, false);
}

int kern_nexus_set_netif_input_tbr_rate(struct ifnet *ifp, uint64_t rate) {
...
int kern_nexus_set_if_netem_params(const nexus_controller_t ncd,
  ...
  __nexus_config_req_prepare(&ncr, nx_uuid, NXCFG_CMD_NETEM, data, data_len);
  bzero(&sopt, sizeof(sopt));
  sopt.sopt_dir = SOPT_SET;
  sopt.sopt_name = NXOPT_NEXUS_CONFIG;
  sopt.sopt_val = (user_addr_t)&ncr;
...
static int _kern_nexus_flow_config(const nexus_controller_t ncd,
                                   const uuid_t nx_uuid, const nxcfg_cmd_t cmd,
...
int kern_nexus_flow_add(const nexus_controller_t ncd, const uuid_t nx_uuid,
                        void *data, size_t data_len) {
  return _kern_nexus_flow_config(ncd, nx_uuid, NXCFG_CMD_FLOW_ADD, data,
                                 data_len);
}

int kern_nexus_flow_del(const nexus_controller_t ncd, const uuid_t nx_uuid,
                        void *data, size_t data_len) {
  return _kern_nexus_flow_config(ncd, nx_uuid, NXCFG_CMD_FLOW_DEL, data,
                                 data_len);
}

static struct kern_nexus_domain_provider *nxdom_prov_alloc(zalloc_flags_t how) {
  SK_LOCK_ASSERT_HELD();

  return zalloc_flags(nxdom_prov_zone, how | Z_ZERO);
}

static void nxdom_prov_free(struct kern_nexus_domain_provider *nxdom_prov) {
  SK_LOCK_ASSERT_HELD();
  ...
  SK_DF(SK_VERB_MEM, "nxdom_prov %p %s", SK_KVA(nxdom_prov),
        ((nxdom_prov->nxdom_prov_flags & NXDOMPROVF_EXT) ? "FREE" : "DESTROY"));
  if (nxdom_prov->nxdom_prov_flags & NXDOMPROVF_EXT) {
    zfree(nxdom_prov_zone, nxdom_prov);
  }
}

void nxdom_prov_retain_locked(struct kern_nexus_domain_provider *nxdom_prov) {
  SK_LOCK_ASSERT_HELD();
...
void nxdom_prov_retain(struct kern_nexus_domain_provider *nxdom_prov) {
  SK_LOCK();
  nxdom_prov_retain_locked(nxdom_prov);
  SK_UNLOCK();
}
...
nxdom_prov_params_default(struct kern_nexus_domain_provider *nxdom_prov,
  ...
  return nxprov_params_adjust(nxdom_prov, req, nxp0, nxp, srp, nxdom, nxdom,
                              nxdom, pp_region_config_flags, NULL);
}

int nxdom_prov_validate_params(struct kern_nexus_domain_provider *nxdom_prov,
                               const struct nxprov_reg *reg,
                               struct nxprov_params *nxp,
                               struct skmem_region_params srp[SKMEM_REGIONS],
  ...
  ASSERT(reg->nxpreg_version == NXPROV_REG_CURRENT_VERSION &&
         nxp0->nxp_namelen != 0 && nxp0->nxp_namelen <= sizeof(nexus_name_t));
  ...
  if (nxdom_prov->nxdom_prov_params != NULL) {
    err = nxdom_prov->nxdom_prov_params(nxdom_prov, req, nxp0, nxp, srp,
                                        pp_region_config_flags);
  } else {
    err = nxdom_prov_params_default(nxdom_prov, req, nxp0, nxp, srp,
                                    pp_region_config_flags);
  }
  return err;
}
...
boolean_t nxdom_prov_release(struct kern_nexus_domain_provider *nxdom_prov) {
  boolean_t lastref;
...
static uint32_t nxprov_bound_var(uint32_t *v, uint32_t dflt, uint32_t lo,
                                 uint32_t hi, const char *msg) {
#pragma unused(msg)
  uint32_t oldv = *v;
  const char *op = NULL;
...
#define NXPROV_PARAMS_ADJUST(flag, param)                                      \
  do {                                                                         \
    uint32_t _v0, _v;                                                          \
    if (req & (flag))                                                          \
      _v = nxp0->nxp_##param;                                                  \
    ...
    if (nxprov_bound_var(&_v, NXDOM_DEF(nxdom_def, param),                     \
                         NXDOM_MIN(nxdom_min, param),                          \
                         NXDOM_MAX(nxdom_max, param), "nxp_" #param) < _v0) {  \
      err = ENOMEM;                                                            \
      goto error;                                                              \
    }                                                                          \
    nxp->nxp_##param = _v;                                                     \
...
int nxprov_params_adjust(
    struct kern_nexus_domain_provider *nxdom_prov, const uint32_t req,
    const struct nxprov_params *nxp0, struct nxprov_params *nxp,
    struct skmem_region_params srp[SKMEM_REGIONS],
  ...
  if (NXDOM_MIN(nxdom_min, capabilities) != 0 &&
    ...
    SK_ERR("%s: caps 0x%x < min 0x%x", nxdom_prov->nxdom_prov_name, capabs,
           NXDOM_MIN(nxdom_min, capabilities));
    err = EINVAL;
    goto error;
  } else if (NXDOM_MAX(nxdom_max, capabilities) != 0 &&
             (capabs & ~NXDOM_MAX(nxdom_max, capabilities))) {
    SK_ERR("%s: caps 0x%x > max 0x%x", nxdom_prov->nxdom_prov_name, capabs,
           NXDOM_MAX(nxdom_max, capabilities));
    err = EINVAL;
    goto error;
  }
  ...
  if (nxp->nxp_max_frags > UINT16_MAX) {
    SK_ERR("invalid configuration for max frags %d", nxp->nxp_max_frags);
    err = EINVAL;
  }
  ...
  SK_D("nxdom \"%s\" (%p) type %d", nxdom_prov->nxdom_prov_dom->nxdom_name,
       SK_KVA(nxdom_prov->nxdom_prov_dom),
       nxdom_prov->nxdom_prov_dom->nxdom_type);
  SK_D("nxp \"%s\" (%p) flags 0x%x", nxp->nxp_name, SK_KVA(nxp),
       nxp->nxp_flags);
  SK_D("  req 0x%x rings %u/%u/%u/%u/%u slots %u/%u/%u/%u/%u buf %u "
       ...
       "tailguard %u large_buf %u",
       req, tx_rings, rx_rings, alloc_rings, free_rings, ev_rings, tx_slots,
       rx_slots, alloc_slots, free_slots, ev_slots, nxp->nxp_buf_size,
       nxp->nxp_md_type, nxp->nxp_md_subtype, stats_size, flowadv_max,
       nexusadv_size, capabs, nxp->nxp_pipes, nxp->nxp_extensions,
       nxp->nxp_max_frags,
  ...
  srp[SKMEM_REGION_RING].srp_r_obj_size = sizeof(struct __user_channel_ring);
  ...
  if (nxp->nxp_max_frags > 1) {
    ...
    buf_cnt =
        MIN((((uint32_t)P2ROUNDUP(NX_MAX_AGGR_PKT_SIZE, nxp->nxp_buf_size) /
              nxp->nxp_buf_size) *
             buf_cnt),
  ...
  pp_regions_params_adjust(srp, nxp->nxp_md_type, nxp->nxp_md_subtype, buf_cnt,
                           (uint16_t)nxp->nxp_max_frags, nxp->nxp_buf_size,
                           nxp->nxp_large_buf_size, buf_cnt,
                           buf_region_segment_size, pp_region_config_flags);
  ...
  if (flowadv_max != 0) {
    static_assert(NX_FLOWADV_DEFAULT * sizeof(struct __flowadv_entry) <=
                  SKMEM_MIN_SEG_SIZE);
  ...
  if (nexusadv_size != 0) {
    srp[SKMEM_REGION_NEXUSADV].srp_r_obj_size =
        nexusadv_size + sizeof(struct __kern_nexus_adv_metadata);
...
error:
  if (overflowline) {
    err = EOVERFLOW;
    SK_ERR("math overflow in %s on line %d", __func__, overflowline);
  }
  return err;
}
...
__attribute__((noreturn)) static void nxprov_detacher(void *v,
                                                      wait_result_t w) {
#pragma unused(v, w)
  SK_LOCK();
  (void)msleep0(&nxprov_detach_wchan, &sk_lock, (PZERO - 1), __func__, 0,
...
static int nxprov_detacher_cont(int err) {
#pragma unused(err)
  struct kern_nexus_domain_provider *nxdom_prov;
