...
int fsw_ethernet_setup(struct nx_flowswitch *fsw, struct ifnet *ifp) {
...
static void fsw_ethernet_ctor(struct nx_flowswitch *fsw,
  ...
  fr->fr_eth.ether_type =
      ((fr->fr_af == AF_INET) ? htons(ETHERTYPE_IP) : htons(ETHERTYPE_IPV6));
  ...
  *(uint8_t *)(uintptr_t)&fr->fr_llhdr.flh_len = ETHER_HDR_LEN;
  ...
  SK_DF(SK_VERB_FLOW_ROUTE, "fr %p eth_type 0x%x eth_src %x:%x:%x:%x:%x:%x",
        SK_KVA(fr), ntohs(fr->fr_eth.ether_type), fr->fr_eth.ether_shost[0],
        fr->fr_eth.ether_shost[1], fr->fr_eth.ether_shost[2],
        fr->fr_eth.ether_shost[3], fr->fr_eth.ether_shost[4],
        fr->fr_eth.ether_shost[5]);
}

static int fsw_ethernet_resolve(struct nx_flowswitch *fsw,
                                struct flow_route *fr,
  ...
  int err = 0;
  uint64_t
      pkt_mflags_restore; /* Save old mbuf flags to restore in error cases */
  ...
  if (__improbable(!IS_DIRECT_HOSTROUTE(tgt_rt) ||
    ...
    SK_ERR("bad gateway route %s on %s (err %d)",
           sk_sa_ntop(tgt_sa, dst_s, sizeof(dst_s)), ifp->if_xname, err);
    goto done;
  }
  ...
  if (__probable((fr->fr_flags & FLOWRTF_HAS_LLINFO) &&
    ...
    os_atomic_or(&fr->fr_flags, (FLOWRTF_RESOLVED | FLOWRTF_HAS_LLINFO),
                 relaxed);
    ...
    probing = FALSE;
    os_atomic_andnot(&fr->fr_flags, (FLOWRTF_RESOLVED | FLOWRTF_HAS_LLINFO),
                     relaxed);
  }

  SK_DF(SK_VERB_FLOW_ROUTE, "%s %s on %s", (probing ? "probing" : "resolving"),
        sk_sa_ntop(tgt_sa, dst_s, sizeof(dst_s)), ifp->if_xname);
  ...
  case AF_INET: {
    ...
    err = arp_lookup_ip(ifp, SIN(tgt_sa), &sdl, sizeof(sdl), NULL, m);
  ...
  case AF_INET6: {
     ...
     */
    if ((tgt_rt->rt_flags & (RTF_UP | RTF_LLINFO)) != (RTF_UP | RTF_LLINFO) ||
        ln == NULL) {
      err = EHOSTUNREACH;
    ...
    if (probing) {
      ...
      if (ln->ln_state == ND6_LLINFO_STALE) {
        ...
        SK_DF(SK_VERB_FLOW_ROUTE, "NUD probe scheduled for %s on %s",
              sk_sa_ntop(tgt_sa, dst_s, sizeof(dst_s)), ifp->if_xname);
      } else {
        RT_UNLOCK(tgt_rt);
      }
    ...
    if (ln->ln_expire == 0 && ln->ln_state == ND6_LLINFO_REACHABLE) {
      if (SDL(tgt_rt->rt_gateway)->sdl_alen != ETHER_ADDR_LEN) {
        err = EHOSTUNREACH;
        SK_ERR("invalid permanent route %s on %s"
               "ln %p (err %d)",
               sk_sa_ntop(rt_key(tgt_rt), dst_s, sizeof(dst_s)), ifp->if_xname,
               SK_KVA(ln), err);
      } else {
        SK_DF(SK_VERB_FLOW_ROUTE,
              "fast-resolve "
    ...
    if (ln->ln_state == ND6_LLINFO_INCOMPLETE &&
        (!ln->ln_asked || !(fr->fr_flags & FLOWRTF_HAS_LLINFO))) {
      ...
      if (m != NULL) {
        ...
        SK_DF(SK_VERB_FLOW_ROUTE, "packet queued while resolving %s on %s",
              sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)),
              ifp->if_xname);
      }
      ...
      SK_DF(SK_VERB_FLOW_ROUTE,
            "soliciting for %s on %s"
            "ln %p state %u",
            sk_sa_ntop(rt_key(tgt_rt), dst_s, sizeof(dst_s)), ifp->if_xname,
            SK_KVA(ln), ln->ln_state);

      /* XXX Refactor this to use same src ip */
      nd6_ns_output(tgt_rt->rt_ifp, NULL, &SIN6(rt_key(tgt_rt))->sin6_addr,
                    NULL, NULL, 0);
...
static void fsw_ethernet_frame(struct nx_flowswitch *fsw, struct flow_route *fr,
                               struct __kern_packet *pkt) {
  ...
  if (__improbable(fr->fr_llhdr.flh_gencnt != fsw->fsw_src_lla_gencnt)) {
    ...
    SK_ERR("fr %p source MAC address updated on %s, "
           ...
           SK_KVA(fr), if_name(fsw->fsw_ifp), old_shost[0], old_shost[1],
           old_shost[2], old_shost[3], old_shost[4], old_shost[5],
  ...
  sk_copy64_16(
      (uint64_t *)(void *)&fr->fr_eth_padded,
...
static sa_family_t fsw_ethernet_demux(struct nx_flowswitch *fsw,
                                      struct __kern_packet *pkt) {
#pragma unused(fsw)
