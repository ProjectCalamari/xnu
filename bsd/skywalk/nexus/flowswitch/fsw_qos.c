...
static inline uint16_t fsw_qos_csum_fixup(uint16_t cksum, uint16_t old,
                                          uint16_t new) {
  uint32_t l;
...
static inline void fsw_qos_set_ip_tos(struct ip *ip, uint8_t dscp) {
  uint8_t old_tos;
  old_tos = ip->ip_tos;
  ip->ip_tos &= IPTOS_ECN_MASK;
  ip->ip_tos |= (u_char)(dscp << IPTOS_DSCP_SHIFT);
  ip->ip_sum =
      fsw_qos_csum_fixup(ip->ip_sum, htons(old_tos), htons(ip->ip_tos));
}

static inline void fsw_qos_set_ipv6_tc(struct ip6_hdr *ip6, uint8_t dscp) {
  ip6->ip6_flow &= ~htonl(IP6FLOW_DSCP_MASK);
  ip6->ip6_flow |= htonl((u_int32_t)dscp << IP6FLOW_DSCP_SHIFT);
}

static inline void fsw_qos_set_pkt_dscp(struct __kern_packet *pkt,
                                        uint8_t dscp) {
  struct ip *__single ip;
  struct ip6_hdr *__single ip6;

  if (pkt->pkt_flow->flow_ip_ver == IPVERSION) {
    ip = __unsafe_forge_single(struct ip *, pkt->pkt_flow->flow_ip_hdr);
    fsw_qos_set_ip_tos(ip, dscp);
  } else {
    ASSERT(pkt->pkt_flow->flow_ip_ver == IPV6_VERSION);
    ip6 = __unsafe_forge_single(struct ip6_hdr *, pkt->pkt_flow->flow_ip_hdr);
    fsw_qos_set_ipv6_tc(ip6, dscp);
  }

  if (pkt->pkt_pflags & PKT_F_MBUF_DATA) {
    if (pkt->pkt_flow->flow_ip_ver == IPVERSION) {
      ip = (struct ip * __single)(void *)(m_mtod_current(pkt->pkt_mbuf) +
                                          pkt->pkt_l2_len);
      fsw_qos_set_ip_tos(ip, dscp);
    } else {
      ip6 = (struct ip6_hdr * __single)(void *)(m_mtod_current(pkt->pkt_mbuf) +
                                                pkt->pkt_l2_len);
      fsw_qos_set_ipv6_tc(ip6, dscp);
    }
  }
...
void fsw_qos_mark(struct nx_flowswitch *fsw, struct flow_entry *fe,
...
boolean_t fsw_qos_default_restricted() { return !!net_qos_policy_restricted; }
