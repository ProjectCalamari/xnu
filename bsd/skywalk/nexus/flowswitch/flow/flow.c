...
int flow_init(void) {
  ...
  if (sk_fo_cache == NULL) {
    sk_fo_cache =
        skmem_cache_create(SK_FO_ZONE_NAME, sk_fo_size, sizeof(uint64_t), NULL,
                           NULL, NULL, NULL, NULL, 0);
    if (sk_fo_cache == NULL) {
      panic("%s: skmem_cache create failed (%s)", __func__, SK_FO_ZONE_NAME);
      /* NOTREACHED */
  ...
  if (sk_fe_cache == NULL) {
    ...
    sk_fe_cache = skmem_cache_create(SK_FE_ZONE_NAME, sk_fe_size, 16, NULL,
                                     NULL, NULL, NULL, NULL, 0);
    if (sk_fe_cache == NULL) {
      panic("%s: skmem_cache create failed (%s)", __func__, SK_FE_ZONE_NAME);
      /* NOTREACHED */
      __builtin_unreachable();
    }
  }
  ...
  static_assert(sizeof(*((struct flow_owner *)0)->fo_flowadv_bmap) ==
                sizeof(bitmap_t));

  sk_fab_size = (sk_fadv_nchunks * sizeof(bitmap_t));
  if (sk_fab_cache == NULL) {
    sk_fab_cache =
        skmem_cache_create(SK_FAB_ZONE_NAME, sk_fab_size, sizeof(uint64_t),
                           NULL, NULL, NULL, NULL, NULL, 0);
    if (sk_fab_cache == NULL) {
      panic("%s: skmem_cache create failed (%s)", __func__, SK_FAB_ZONE_NAME);
      /* NOTREACHED */
      __builtin_unreachable();
    }
  }
