...
#define FOB_LOCK(_fob) lck_mtx_lock(&(_fob)->fob_lock)
#define FOB_LOCK_SPIN(_fob) lck_mtx_lock_spin(&(_fob)->fob_lock)
#define FOB_LOCK_CONVERT(_fob) lck_mtx_convert_spin(&(_fob)->fob_lock)
#define FOB_TRY_LOCK(_fob) lck_mtx_try_lock(&(_fob)->fob_lock)
...
#define FOB_UNLOCK(_fob) lck_mtx_unlock(&(_fob)->fob_lock)
...
#define FO_BUCKET(_fo) __DECONST(struct flow_owner_bucket *, (_fo)->fo_bucket)

RB_PROTOTYPE_SC_PREV(__private_extern__, flow_owner_tree, flow_owner, fo_link,
                     fo_cmp);
...
typedef int (*flow_demux_memcmp_mask_t)(const uint8_t *src1,
                                        const uint8_t *src2,
...
typedef void (*flow_tx_action_t)(struct nx_flowswitch *fsw,
                                 struct flow_entry *fe, uint32_t flags);

#define FLOW_PROC_FLAG_FRAGMENTS 0x0001
typedef void (*flow_rx_action_t)(struct nx_flowswitch *fsw,
                                 struct flow_entry *fe, struct pktq *pkts,
                                 uint32_t rx_bytes, struct mbufq *host_mq,
                                 uint32_t flags);
...
struct flow_entry {
  ...
  struct kern_flow_demux_pattern *
      __counted_by(fe_demux_pattern_count) fe_demux_patterns;
...
#define FLOWENTF_RXSTRC_PENDING                                                \
  0x00000080                             /* Rx steering rule cleanup pending */
...
#define FLOWENTF_EXTRL_PORT 0x00004000 /* port reservation is held externally  \
                                        */
#define FLOWENTF_EXTRL_PROTO                                                   \
  0x00008000 /* proto reservation is held externally */
#define FLOWENTF_EXTRL_FLOWID                                                  \
  0x00010000                       /* flowid reservation is held externally */
...
#define FLOWENTF_BITS                                                          \
  "\020\01INITED\02AOP_OFFLOAD\03RX_"                                          \
  "STEERING\05TRACK\06CONNECTED\07LISTNER\011QOS_MARKING"                      \
...
#define FR_LOCK(_fr) lck_mtx_lock(&(_fr)->fr_lock)
#define FR_TRY_LOCK(_fr) lck_mtx_try_lock(&(_fr)->fr_lock)
...
#define FR_UNLOCK(_fr) lck_mtx_unlock(&(_fr)->fr_lock)

#define FLOWRT_UPD_ETH_DST(_fr, _addr)                                         \
  do {                                                                         \
...
RB_PROTOTYPE_SC_PREV(__private_extern__, flow_route_tree, flow_route, fr_link,
                     fr_cmp);
...
#define FRB_WLOCK(_frb) lck_rw_lock_exclusive(&(_frb)->frb_lock)
#define FRB_WLOCKTORLOCK(_frb)                                                 \
  lck_rw_lock_exclusive_to_shared(&(_frb)->frb_lock)
#define FRB_WTRYLOCK(_frb) lck_rw_try_lock_exclusive(&(_frb)->frb_lock)
#define FRB_WUNLOCK(_frb) lck_rw_unlock_exclusive(&(_frb)->frb_lock)
#define FRB_RLOCK(_frb) lck_rw_lock_shared(&(_frb)->frb_lock)
#define FRB_RLOCKTOWLOCK(_frb)                                                 \
  lck_rw_lock_shared_to_exclusive(&(_frb)->frb_lock)
#define FRB_RTRYLOCK(_frb) lck_rw_try_lock_shared(&(_frb)->frb_lock)
#define FRB_RUNLOCK(_frb) lck_rw_unlock_shared(&(_frb)->frb_lock)
#define FRB_UNLOCK(_frb) lck_rw_done(&(_frb)->frb_lock)
...
#define FRIB_WLOCK(_frib) lck_rw_lock_exclusive(&(_frib)->frib_lock)
#define FRIB_WLOCKTORLOCK(_frib)                                               \
  lck_rw_lock_exclusive_to_shared(&(_frib)->frib_lock)
#define FRIB_WTRYLOCK(_frib) lck_rw_try_lock_exclusive(&(_frib)->frib_lock)
#define FRIB_WUNLOCK(_frib) lck_rw_unlock_exclusive(&(_frib)->frib_lock)
#define FRIB_RLOCK(_frib) lck_rw_lock_shared(&(_frib)->frib_lock)
#define FRIB_RLOCKTOWLOCK(_frib)                                               \
  lck_rw_lock_shared_to_exclusive(&(_frib)->frib_lock)
#define FRIB_RTRYLOCK(_frib) lck_rw_try_lock_shared(&(_frib)->frib_lock)
#define FRIB_RUNLOCK(_frib) lck_rw_unlock_shared(&(_frib)->frib_lock)
#define FRIB_UNLOCK(_frib) lck_rw_done(&(_frib)->frib_lock)
...
struct flow_mgr {
  ...
  void *__sized_by(fm_owner_bucket_tot_sz)
      fm_owner_buckets;          /* cache-aligned fob */
  ...
  void *__sized_by(fm_route_bucket_tot_sz)
      fm_route_buckets;          /* cache-aligned frb */
  ...
  void *__sized_by(fm_route_id_bucket_tot_sz)
      fm_route_id_buckets;          /* cache-aligned frib */
 ...
 */
static inline int flow_key_cmp(const struct flow_key *match,
 ...
 */
static inline int flow_key_cmp_mask(const struct flow_key *match,
                                    const struct flow_key *key,
  ...
  return sk_memcmp_mask_48B((const uint8_t *)match_idx,
                            ...
                            (const uint8_t *)mask_idx);
}

static inline uint32_t flow_key_hash(const struct flow_key *key) {
...
__attribute__((always_inline)) static inline void
flow_pkt2key(struct __kern_packet *pkt, boolean_t input, struct flow_key *key) {
...
__attribute__((always_inline)) static inline struct flow_owner_bucket *
...
__attribute__((always_inline)) static inline struct flow_route_bucket *
...
__attribute__((always_inline)) static inline struct flow_route_id_bucket *
...
flow_mgr_get_fob_idx(struct flow_mgr *fm, struct flow_owner_bucket *bkt) {
...
extern struct flow_owner_bucket *flow_mgr_get_fob_by_pid(struct flow_mgr *,
                                                         pid_t);
extern struct flow_entry *flow_mgr_get_fe_by_uuid_rlock(struct flow_mgr *,
                                                        uuid_t);
extern struct flow_route_bucket *
flow_mgr_get_frb_by_addr(struct flow_mgr *, union sockaddr_in_4_6 *);
extern struct flow_route_id_bucket *flow_mgr_get_frib_by_uuid(struct flow_mgr *,
                                                              uuid_t);
...
extern void flow_owner_attach_nexus_port(struct flow_mgr *, boolean_t, pid_t,
                                         nexus_port_t);
extern uint32_t flow_owner_detach_nexus_port(struct flow_mgr *, boolean_t,
                                             pid_t, nexus_port_t, boolean_t);
extern struct flow_owner *flow_owner_alloc(struct flow_owner_bucket *,
                                           struct proc *, nexus_port_t, bool,
                                           bool, struct nx_flowswitch *,
                                           struct nexus_adapter *, void *,
                                           bool);
extern void flow_owner_free(struct flow_owner_bucket *, struct flow_owner *);
extern struct flow_entry *
flow_owner_create_entry(struct flow_owner *, struct nx_flow_req *, boolean_t,
                        uint32_t, boolean_t, struct flow_route *, int *);
...
extern uint32_t flow_owner_activate_nexus_port(struct flow_mgr *, boolean_t,
                                               pid_t, nexus_port_t,
...
extern struct flow_entry *flow_entry_find_by_uuid(struct flow_owner *, uuid_t);
...
extern bool rx_flow_demux_match(struct nx_flowswitch *, struct flow_entry *,
                                struct __kern_packet *);
...
extern void flow_track_stats(struct flow_entry *, uint64_t, uint64_t, bool,
                             bool);
...
extern void flow_track_abort_tcp(struct flow_entry *fe,
                                 struct __kern_packet *in_pkt,
                                 struct __kern_packet *rst_pkt);
extern void
flow_track_abort_quic(struct flow_entry *fe,
                      uint8_t *__counted_by(QUIC_STATELESS_RESET_TOKEN_SIZE)
                          token);

extern void fsw_host_rx_cb(struct nx_flowswitch *fsw, struct pktq *pktq);
extern void fsw_host_rx_enqueue_mbq(struct nx_flowswitch *fsw,
                                    struct pktq *pktq, struct mbufq *host_mq);
...
extern int flow_route_select_laddr(union sockaddr_in_4_6 *,
                                   union sockaddr_in_4_6 *, struct ifnet *,
                                   struct rtentry *, uint32_t *, int);
extern int flow_route_find(struct kern_nexus *, struct flow_mgr *,
                           struct ifnet *, struct nx_flow_req *,
                           flow_route_ctor_fn_t, flow_route_resolve_fn_t,
                           void *, struct flow_route **);
extern int flow_route_configure(struct flow_route *, struct ifnet *,
                                struct nx_flow_req *);
extern void flow_route_retain(struct flow_route *);
extern void flow_route_release(struct flow_route *);
extern uint32_t flow_route_prune(struct flow_mgr *, struct ifnet *, uint32_t *);
...
extern char *fk2str(const struct flow_key *fk, char *__counted_by(dsz) dst,
                    size_t dsz);
extern char *fe2str(const struct flow_entry *fe, char *__counted_by(dsz) dst,
                    size_t dsz);
#endif /* SK_LOG */
__END_DECLS
