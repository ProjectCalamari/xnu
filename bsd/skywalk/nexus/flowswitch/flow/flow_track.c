...
static void flow_track_tcp_get_wscale(struct flow_track *s,
                                      struct __kern_packet *pkt) {
  const uint8_t *hdr = __unsafe_forge_bidi_indexable(
      uint8_t *, pkt->pkt_flow_tcp_hdr, pkt->pkt_flow_tcp_hlen);
...
static void flow_track_tcp_init(struct flow_entry *fe, struct flow_track *src,
                                struct flow_track *dst,
                                struct __kern_packet *pkt) {
#pragma unused(dst)
...
flow_track_tcp_rtt(struct flow_entry *fe, boolean_t input,
                   struct flow_track *src, struct flow_track *dst,
...
#pragma unused(fe, input) /* KDBG defined as noop in release build */
  ...
  if (dst_last == 0 ||
    ...
    if (ulen > 0 && dst->fse_rtt.frtt_timestamp == 0) {
      ...
      KDBG((SK_KTRACE_FSW_FLOW_TRACK_RTT | DBG_FUNC_START), SK_KVA(fe),
           fe->fe_pid, ntohs(fe->fe_key.fk_sport), input ? 1 : 0);
    }
  }
  ...
  if (tcp_flags & TH_ACK) {
    if (src->fse_rtt.frtt_timestamp != 0 &&
      ...
      if (oldval == 0) {
        ...
        src->fse_rtt.frtt_usec = ((oldval << 4) - oldval + curval) >> 4;
      }

      /* reset RTT tracking session */
      src->fse_rtt.frtt_timestamp = 0;
      src->fse_rtt.frtt_last = 0;
      KDBG((SK_KTRACE_FSW_FLOW_TRACK_RTT | DBG_FUNC_END), SK_KVA(fe),
           fe->fe_pid, ntohs(fe->fe_key.fk_sport), input ? 0 : 1);
...
flow_track_tcp(struct flow_entry *fe, struct flow_track *src,
  ...
  if (__probable((tcp_flags & TH_SYN) == 0 && src->fse_wscale != 0 &&
                 dst->fse_wscale != 0)) {
    sws = src->fse_wscale;
    dws = dst->fse_wscale;
  } else {
  ...
  if (__probable(src->fse_seqlo != 0)) {
    ...
    if (tcp_flags & TH_SYN) {
      ...
      if (dst->fse_flags & FLOWSTATEF_WSCALE) {
        ...
        if (src->fse_flags & FLOWSTATEF_WSCALE) {
           ...
           */
          sws = src->fse_wscale;
          win = (uint16_t)(((u_int32_t)win + (1 << sws) - 1) >> sws);
          dws = dst->fse_wscale;
        } else {
          /* fixup other window */
     ...
     */
    if (src->fse_seqhi == 1 ||
        SEQ_GEQ(end + MAX(1, dst->fse_max_win << dws), src->fse_seqhi)) {
...
#define MAXACKWINDOW (0xffff + 1500) /* 1500 is an arbitrary fudge factor */
  if (SEQ_GEQ(src->fse_seqhi, end) &&
      ...
      (!(tcp_flags & TH_RST) || orig_seq == src->fse_seqlo ||
       (orig_seq == src->fse_seqlo + 1) || (orig_seq + 1 == src->fse_seqlo))) {
    ...
    if (tcp_flags & TH_ACK) {
       ...
       */
      if (dst->fse_state == TCPS_SYN_SENT && !(tcp_flags & TH_RST)) {
    ...
    if ((tcp_flags & TH_RST) && (src->fse_state == TCPS_ESTABLISHED ||
                                 dst->fse_state == TCPS_ESTABLISHED)) {
      /*
    ...
    if (dst->fse_state == TCPS_SYN_SENT && src->fse_state == TCPS_SYN_SENT) {
      src->fse_seqlo = 0;
      src->fse_seqhi = 1;
      src->fse_max_win = 1;
    }
...
boolean_t flow_track_tcp_want_abort(struct flow_entry *fe) {
...
static void flow_track_udp_init(struct flow_entry *fe, struct flow_track *src,
                                struct flow_track *dst,
                                struct __kern_packet *pkt) {
#pragma unused(pkt)
  /*
...
void flow_track_stats(struct flow_entry *fe, uint64_t bytes, uint64_t packets,
                      bool active, bool in) {
  ...
  if (__probable(active)) {
    in_stat_set_activity_bitmap(&fe->fe_stats->fs_activity, net_uptime());
  }
}

int flow_pkt_track(struct flow_entry *fe, struct __kern_packet *pkt, bool in) {
 ...
 */
void flow_track_abort_tcp(struct flow_entry *fe, struct __kern_packet *in_pkt,
  ...
  if (in_pkt != NULL) {
    ...
    if (__probable(in_pkt->pkt_flow_tcp_flags | TH_ACK)) {
      ...
      th->th_seq = 0;
      th->th_ack = in_pkt->pkt_flow_tcp_seq + in_pkt->pkt_flow_ulen;
...
void flow_track_abort_quic(
  ...
  if (fe->fe_key.fk_ipver == IPVERSION) {
    ...
    uh->uh_sum = in6_cksum(m, IPPROTO_UDP, sizeof(struct ip6_hdr), ulen);
    if (uh->uh_sum == 0) {
      uh->uh_sum = 0xffff;
    }
