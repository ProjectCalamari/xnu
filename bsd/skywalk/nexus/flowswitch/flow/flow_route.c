...
void flow_route_init(void) {
  ASSERT(!__flow_route_inited);

  flow_route_size = sizeof(struct flow_route);
  flow_route_cache =
      skmem_cache_create(FR_ZONE_NAME, flow_route_size, sizeof(uint64_t), NULL,
                         NULL, NULL, NULL, NULL, 0);

  __flow_route_inited = 1;
}

void flow_route_fini(void) {
...
struct flow_route_bucket *__sized_by(*tot_sz)
  ...
  *tot_sz = frb_tot_sz = frb_cnt * (*frb_sz);
  ...
  SK_DF(SK_VERB_MEM,
        "frb %p frb_cnt %zu frb_sz %zu "
        "(total %zu bytes) ALLOC",
        SK_KVA(frb), frb_cnt, *frb_sz, frb_tot_sz);

  return frb;
}

void flow_route_buckets_free(struct flow_route_bucket *frb, size_t tot_sz) {
  SK_DF(SK_VERB_MEM, "frb %p FREE", SK_KVA(frb));
  sk_free_type_hash(KT_SK_FRB, tot_sz, frb);
}

void flow_route_bucket_init(struct flow_route_bucket *frb) {
#if !KASAN_CLASSIC
  ASSERT(IS_P2ALIGNED(frb, skmem_cpu_cache_line_size()));
#endif /* !KASAN_CLASSIC */
  lck_rw_init(&frb->frb_lock, &flow_route_lock_group, &flow_route_lock_attr);
  RB_INIT(&frb->frb_head);
}

void flow_route_bucket_destroy(struct flow_route_bucket *frb) {
  ASSERT(RB_EMPTY(&frb->frb_head));
  lck_rw_destroy(&frb->frb_lock, &flow_route_lock_group);
}

static struct flow_route *flow_route_find_by_addr(struct flow_route_bucket *frb,
...
struct flow_route_id_bucket *__sized_by(*tot_sz)
    flow_route_id_buckets_alloc(size_t frib_cnt, size_t *frib_sz,
                                size_t *tot_sz) {
  uint32_t cache_sz = skmem_cpu_cache_line_size();
  ...
  *tot_sz = frib_tot_sz = frib_cnt * (*frib_sz);
  ...
  SK_DF(SK_VERB_MEM,
        "frib %p frib_cnt %zu frib_sz %zu "
        "(total %zu bytes) ALLOC",
        SK_KVA(frib), frib_cnt, *frib_sz, frib_tot_sz);

  return frib;
}

void flow_route_id_buckets_free(struct flow_route_id_bucket *frib,
                                size_t tot_sz) {
  SK_DF(SK_VERB_MEM, "frib %p FREE", SK_KVA(frib));
  sk_free_type_hash(KT_SK_FRIB, tot_sz, frib);
}

void flow_route_id_bucket_init(struct flow_route_id_bucket *frib) {
#if !KASAN_CLASSIC
  ASSERT(IS_P2ALIGNED(frib, skmem_cpu_cache_line_size()));
#endif
  lck_rw_init(&frib->frib_lock, &flow_route_lock_group, &flow_route_lock_attr);
  RB_INIT(&frib->frib_head);
}

void flow_route_id_bucket_destroy(struct flow_route_id_bucket *frib) {
...
static struct flow_route *fr_alloc(boolean_t cansleep) {
  struct flow_route *fr;
...
static void fr_free(struct flow_route *fr) {
...
static inline int fr_cmp(const struct flow_route *a,
                         const struct flow_route *b) {
  int d;

  if ((d = (a->fr_af - b->fr_af)) != 0) {
    return d;
  }
  if ((d = flow_ip_cmp(a->fr_addr_key, b->fr_addr_key, b->fr_addr_len)) != 0) {
    return d;
  }
...
static inline int fr_id_cmp(const struct flow_route *a,
                            const struct flow_route *b) {
  return uuid_compare(a->fr_uuid, b->fr_uuid);
}

static inline int fr_use_stable_address(struct nx_flow_req *req) {
  int use_stable_address = ip6_prefer_tempaddr ? 0 : 1;
  if (req != NULL && (req->nfr_flags & NXFLOWREQF_OVERRIDE_ADDRESS_SELECTION)) {
    use_stable_address =
        (req->nfr_flags & NXFLOWREQF_USE_STABLE_ADDRESS) ? 1 : 0;
  }
  return use_stable_address;
}

int flow_route_configure(struct flow_route *fr, struct ifnet *ifp,
  ...
  if (rt == NULL || !(rt->rt_flags & RTF_UP) || fr->fr_want_configure) {
    ...
    if (rt == NULL) {
      ...
      if (rt->rt_ifp != ifp && rt->rt_ifp != lo_ifp &&
        ...
        SK_ERR("route to %s on %s != %s (err %d)",
               ...
               rt->rt_ifp->if_xname, ifp->if_xname, err);
      }
    }
    ...
    fr->fr_rt_evhdlr_tag = EVENTHANDLER_REGISTER(
        &rt->rt_evhdlr_ctxt, route_event, &flow_route_ev_callback, ee_arg,
  ...
  if (fr->fr_want_configure ||
    ...
    if ((err = flow_route_select_laddr(&fr->fr_laddr, &fr->fr_faddr, ifp, rt,
                                       &fr->fr_laddr_gencnt,
                                       use_stable_address)) != 0) {
    ...
    if (bcmp(&old, &fr->fr_laddr, SA(&old)->sa_len) != 0) {
      SK_ERR("src address is now %s (was %s) to reach %s "
             ...
             sk_sa_ntop(SA(&old), old_s, sizeof(old_s)),
             sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)),
             ifp->if_xname);
    }
  }
...
int flow_route_find(struct kern_nexus *nx, struct flow_mgr *fm,
                    struct ifnet *ifp, struct nx_flow_req *req,
                    flow_route_ctor_fn_t fr_ctor,
                    flow_route_resolve_fn_t fr_resolve, void *arg,
                    struct flow_route **frp) {
#if SK_LOG
  ...
  *frp = NULL;
  ...
  if (fr != NULL) {
    if (__improbable(fr->fr_want_configure ||
        ...
        __improbable(!(fr->fr_flags & FLOWRTF_STABLE_ADDR) !=
                     !use_stable_address)) {
      ...
      if (err != 0) {
        SK_ERR("fr %p error re-configuring dst %s "
               "on %s (err %d) [R]",
               SK_KVA(fr), sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)),
               ifp->if_xname, err);
      }
      FR_UNLOCK(fr);
    }
    if (err == 0) {
      SK_DF(SK_VERB_FLOW_ROUTE,
            "fr %p found for dst %s "
            "on %s [R,%u]",
            SK_KVA(fr), sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)),
            ifp->if_xname, fr->fr_usecnt);
    }
  ...
  if (fr != NULL) {
    ...
    if (err == 0) {
      SK_DF(SK_VERB_FLOW_ROUTE, "fr %p found for dst %s on %s [W,%u]",
            SK_KVA(fr), sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)),
            ifp->if_xname, fr->fr_usecnt);
    }
  ...
  *(struct flow_mgr **)(uintptr_t)&fr->fr_mgr = fm;
  ...
  if ((err = flow_route_configure(fr, ifp, req)) != 0) {
    SK_ERR("fr %p error configuring dst %s on %s (err %d)", SK_KVA(fr),
           sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)), ifp->if_xname,
           err);
    FR_UNLOCK(fr);
    FRB_WUNLOCK(frb); /* writer */
  ...
  *(struct flow_route_id_bucket **)(uintptr_t)&fr->fr_frib = frib;
  ...
  if (!(fr->fr_flags & FLOWRTF_RESOLVED) &&
    ...
    if (fr->fr_flags & FLOWRTF_GATEWAY) {
      SK_ERR("fr %p resolve %s gw %s on %s (err %d)", SK_KVA(fr),
             (err == EJUSTRETURN ? "pending" : "fail"),
             sk_sa_ntop(SA(&fr->fr_gaddr), dst_s, sizeof(dst_s)), ifp->if_xname,
             err);
    } else {
      SK_ERR("fr %p resolve %s dst %s on %s (err %d)", SK_KVA(fr),
             (err == EJUSTRETURN ? "pending" : "fail"),
             sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)), ifp->if_xname,
             err);
    }
...
#if SK_LOG
  if (fr->fr_flags & FLOWRTF_GATEWAY) {
    SK_DF(SK_VERB_FLOW_ROUTE, "add fr %p %s -> %s via gw %s on %s", SK_KVA(fr),
          sk_sa_ntop(SA(&fr->fr_laddr), src_s, sizeof(src_s)),
          sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)),
          sk_sa_ntop(SA(&fr->fr_gaddr), gw_s, sizeof(gw_s)), ifp->if_xname);
  } else {
    SK_DF(SK_VERB_FLOW_ROUTE, "add fr %p %s -> %s on %s", SK_KVA(fr),
          sk_sa_ntop(SA(&fr->fr_laddr), src_s, sizeof(src_s)),
          sk_sa_ntop(SA(&fr->fr_faddr), dst_s, sizeof(dst_s)), ifp->if_xname);
  }
#endif /* SK_LOG */
...
void flow_route_retain(struct flow_route *fr) {
...
static void __flow_route_release(struct flow_route *fr, boolean_t renew) {
...
void flow_route_release(struct flow_route *fr) {
  __flow_route_release(fr, true);
}

static uint32_t flow_route_bucket_purge_common(struct flow_route_bucket *frb,
                                               uint32_t *resid, boolean_t all,
                                               boolean_t early_expire) {
#if SK_LOG
  ...
  RB_FOREACH_SAFE(fr, flow_route_tree, &frb->frb_head, tfr) {
    ...
    if (!all && (fr->fr_usecnt > FLOW_ROUTE_MINREF ||
      ...
      SK_DF(SK_VERB_FLOW_ROUTE,
            "skipping fr %p "
            "refcnt %u expire %llu",
            SK_KVA(fr), fr->fr_usecnt, fr->fr_expire);
      continue;
    }
    lck_spin_unlock(&fr->fr_reflock);
...
#if SK_LOG
    if (fr->fr_flags & FLOWRTF_GATEWAY) {
      SK_DF(SK_VERB_FLOW_ROUTE, "remove fr %p %s -> %s via gw %s [exp %lld]",
            SK_KVA(fr), sk_sa_ntop(SA(&fr->fr_laddr), ss, sizeof(ss)),
            sk_sa_ntop(SA(&fr->fr_faddr), ds, sizeof(ds)),
            sk_sa_ntop(SA(&fr->fr_gaddr), gs, sizeof(gs)),
            (int64_t)(fr->fr_expire - now));
    } else {
      SK_DF(SK_VERB_FLOW_ROUTE, "remove fr %p %s -> %s [exp %lld]", SK_KVA(fr),
...
void flow_route_bucket_purge_all(struct flow_route_bucket *frb) {
  (void)flow_route_bucket_purge_common(frb, NULL, TRUE, FALSE);
}

static uint32_t flow_route_bucket_prune(struct flow_route_bucket *frb,
                                        struct ifnet *ifp, uint32_t *resid) {
  uint64_t now = net_uptime();
  struct flow_route *fr;
  uint32_t i = 0, tot = 0;
  ...
  RB_FOREACH(fr, flow_route_tree, &frb->frb_head) {
    ...
    if (fr->fr_usecnt > FLOW_ROUTE_MINREF ||
        (fr->fr_expire > now && !ifdown && !(fr->fr_flags & FLOWRTF_DELETED))) {
      continue;
    }
    ++i;
  }
  ...
  SK_DF(SK_VERB_FLOW_ROUTE, "purging at least %u idle routes on %s", i,
        ifp->if_xname);
...
uint32_t flow_route_prune(struct flow_mgr *fm, struct ifnet *ifp,
                          uint32_t *tot_resid) {
  uint32_t pruned = 0;
  uint32_t resid;
  uint32_t i;
 ...
 */
static void flow_route_ev_callback(struct eventhandler_entry_arg ee_arg,
                                   struct sockaddr *dst, int route_ev,
                                   struct sockaddr *gw_addr_orig, int flags) {
#pragma unused(dst, flags)
  ...
  if (fm == NULL) {
    SK_ERR("Event %s for dst %s ignored; flow manager not found",
           route_event2str(route_ev), sk_sa_ntop(dst, dst_s, sizeof(dst_s)));
    return;
  }
  ...
  do {
    ...
    case ROUTE_ENTRY_REFRESH:
      ...
      SK_DF(SK_VERB_FLOW_ROUTE, "%s: dst %s route changed", fm->fm_name,
            sk_sa_ntop(dst, dst_s, sizeof(dst_s)));
      break;
    ...
    case ROUTE_ENTRY_DELETED:
      ...
      SK_DF(SK_VERB_FLOW_ROUTE, "%s: dst %s route deleted", fm->fm_name,
            sk_sa_ntop(dst, dst_s, sizeof(dst_s)));
      break;
    ...
    case ROUTE_LLENTRY_STALE:
      ...
      os_atomic_andnot(&fr->fr_flags, FLOWRTF_RESOLVED, relaxed);
      SK_DF(SK_VERB_FLOW_ROUTE, "%s: dst %s llentry stale", fm->fm_name,
            sk_sa_ntop(dst, dst_s, sizeof(dst_s)));
      renew_fr = false;
      break;
    ...
    case ROUTE_LLENTRY_RESOLVED: {
      ...
      if (gw_addr->sdl_alen == ETHER_ADDR_LEN) {
        ...
        SK_DF(SK_VERB_FLOW_ROUTE, "%s: dst %s llentry %s", fm->fm_name,
              sk_sa_ntop(dst, dst_s, sizeof(dst_s)),
              (!(fr->fr_flags & FLOWRTF_HAS_LLINFO) ? "resolved" : "changed"));
...
#if SK_LOG
      if (__improbable((sk_verbose & SK_VERB_FLOW_ROUTE) != 0) &&
          (fr->fr_flags & FLOWRTF_HAS_LLINFO)) {
        SK_DF(SK_VERB_FLOW_ROUTE,
              "%s: fr %p eth_type 0x%x "
              "eth_src %x:%x:%x:%x:%x:%x "
              "eth_dst %x:%x:%x:%x:%x:%x [%s])",
              fm->fm_name, SK_KVA(fr), ntohs(fr->fr_eth.ether_type),
              ...
              fr->fr_eth.ether_dhost[0], fr->fr_eth.ether_dhost[1],
              fr->fr_eth.ether_dhost[2], fr->fr_eth.ether_dhost[3],
              fr->fr_eth.ether_dhost[4], fr->fr_eth.ether_dhost[5],
              sk_sa_ntop(dst, dst_s, sizeof(dst_s)));
      }
#endif /* SK_LOG */
      break;
    ...
    case ROUTE_LLENTRY_DELETED:
       ...
       */
      os_atomic_inc(&fr->fr_want_configure, relaxed);
      os_atomic_andnot(&fr->fr_flags, (FLOWRTF_HAS_LLINFO | FLOWRTF_RESOLVED),
                       relaxed);
      SK_DF(SK_VERB_FLOW_ROUTE, "%s: dst %s llentry deleted", fm->fm_name,
            sk_sa_ntop(dst, dst_s, sizeof(dst_s)));
      break;
    ...
    case ROUTE_LLENTRY_PROBED:
       ...
       * nothing to do here.
       */
      SK_DF(SK_VERB_FLOW_ROUTE, "%s: dst %s llentry probed", fm->fm_name,
            sk_sa_ntop(dst, dst_s, sizeof(dst_s)));
      break;
...
int flow_route_select_laddr(union sockaddr_in_4_6 *src,
                            union sockaddr_in_4_6 *dst, struct ifnet *ifp,
                            struct rtentry *rt, uint32_t *ipaddr_gencnt,
  ...
  case AF_INET: {
    ...
    if (__improbable(rt->rt_ifa->ifa_debug & IFD_DETACHING) != 0) {
      ...
      SK_ERR("route to %s has src address marked detaching "
             "(err %d)",
             sk_ntop(AF_INET, &SIN(dst)->sin_addr, dst_s, sizeof(dst_s)), err);
      ifnet_lock_done(ifp);
      break;
    }
  ...
  case AF_INET6: {
    ...
    if ((in6 = in6_selectsrc_core(SIN6(dst), hints, ifp, 0, &src_storage,
                                  &src_ifp, &err, &ifa, &ro, FALSE)) == NULL) {
      if (err == 0) {
        err = EADDRNOTAVAIL;
      }
      VERIFY(src_ifp == NULL);
      SK_ERR("src address to dst %s on %s not available "
             "(err %d)",
             sk_ntop(AF_INET6, &SIN6(dst)->sin6_addr, dst_s, sizeof(dst_s)),
             ifp->if_xname, err);
      break;
    }
    ...
    if (__improbable(src_ifp != ifp)) {
      ...
      SK_ERR("dst %s, src %s ifp %s != %s (err %d)",
             sk_ntop(AF_INET6, &SIN6(dst)->sin6_addr, dst_s, sizeof(dst_s)),
             sk_ntop(AF_INET6, &SIN6(src)->sin6_addr, src_s, sizeof(src_s)),
  ...
  if (err == 0 && __improbable((sk_verbose & SK_VERB_FLOW_ROUTE) != 0)) {
    SK_DF(SK_VERB_FLOW_ROUTE, "src %s to dst %s on %s",
          ...
          sk_sa_ntop(SA(dst), dst_s, sizeof(dst_s)), ifp->if_xname);
  }
#endif /* SK_LOG */
...
void flow_route_cleanup(struct flow_route *fr) {
#if SK_LOG
  char ss[MAX_IPv6_STR_LEN]; /* dst */
  char ds[MAX_IPv6_STR_LEN]; /* dst */
  char gs[MAX_IPv6_STR_LEN]; /* gw */
...
#if SK_LOG
  if (fr->fr_flags & FLOWRTF_GATEWAY) {
    SK_DF(SK_VERB_FLOW_ROUTE, "clean fr %p %s -> %s via gw %s", SK_KVA(fr),
          sk_sa_ntop(SA(&fr->fr_laddr), ss, sizeof(ss)),
          sk_sa_ntop(SA(&fr->fr_faddr), ds, sizeof(ds)),
          sk_sa_ntop(SA(&fr->fr_gaddr), gs, sizeof(gs)));
  } else if (fr->fr_flags & FLOWRTF_ONLINK) {
    SK_DF(SK_VERB_FLOW_ROUTE, "clean fr %p %s -> %s", SK_KVA(fr),
          sk_sa_ntop(SA(&fr->fr_laddr), ss, sizeof(ss)),
          sk_sa_ntop(SA(&fr->fr_faddr), ds, sizeof(ds)));
  }
#endif /* SK_LOG */
...
static boolean_t _flow_route_laddr_validate(struct flow_ip_addr *src_ip0,
                                            uint8_t ip_v, struct ifnet *ifp,
                                            uint32_t *gencnt) {
  ...
  if (ip_v == IPVERSION) {
    ...
    ifa = (struct ifaddr *)ifa_foraddr_scoped(src_ip._v4.s_addr, scope);
  } else {
    ...
    ifa = (struct ifaddr *)ifa_foraddr6_scoped(&src_ip._v6, scope);
  }
  ...
  if (ip_v == IPV6_VERSION) {
     ...
     */
    struct in6_ifaddr *ia6 = __container_of(ifa, struct in6_ifaddr, ia_ifa);

    /*
     * Fail if IPv6 address is not ready or if the address
     * is reserved * for CLAT46.
     */
    if (__improbable(ia6->ia6_flags & (IN6_IFF_NOTREADY | IN6_IFF_CLAT46)) !=
        0) {
      address_found = FALSE;
      ifnet_lock_done(ifp);
      goto done;
...
boolean_t flow_route_laddr_validate(union sockaddr_in_4_6 *saddr,
                                    struct ifnet *ifp, uint32_t *gencnt) {
  VERIFY(saddr->sa.sa_family == AF_INET || saddr->sa.sa_family == AF_INET6);
...
boolean_t flow_route_key_validate(struct flow_key *fk, struct ifnet *ifp,
                                  uint32_t *gencnt) {
  return _flow_route_laddr_validate(&fk->fk_src, fk->fk_ipver, ifp, gencnt);
}
