...
#define CL_SKIP_ON(t)                                                          \
  if (__improbable(t)) {                                                       \
    ...
    SK_ERR("%s %s", if_name(ifp),                                              \
           sk_dump("buf", pkt_buf + pkt->pkt_headroom,                         \
                   __packet_get_real_data_length(pkt), MIN(128, bdlen)));      \
    error = ENOTSUP;                                                           \
    goto done;                                                                 \
  }
 ...
 *
 */
int flow_pkt_classify(struct __kern_packet *pkt, struct ifnet *ifp,
...
#define udph _l4._udph
  ...
  static_assert(
      (offsetof(struct __flow_l3_ipv6_addrs, _dst) -
       offsetof(struct __flow_l3_ipv6_addrs, _src)) ==
      (offsetof(struct ip6_hdr, ip6_dst) - offsetof(struct ip6_hdr, ip6_src)));

  /* __flow_l4_tcp must mirror tcphdr for the first 16-bytes */
  static_assert(sizeof(struct __flow_l4_tcp) == 16);
  static_assert(
      (offsetof(struct __flow_l4_tcp, _dst) -
       offsetof(struct __flow_l4_tcp, _src)) ==
      (offsetof(struct tcphdr, th_dport) - offsetof(struct tcphdr, th_sport)));
  static_assert(
      (offsetof(struct __flow_l4_tcp, _seq) -
       offsetof(struct __flow_l4_tcp, _src)) ==
      (offsetof(struct tcphdr, th_seq) - offsetof(struct tcphdr, th_sport)));
  static_assert(
      (offsetof(struct __flow_l4_tcp, _ack) -
       offsetof(struct __flow_l4_tcp, _src)) ==
      (offsetof(struct tcphdr, th_ack) - offsetof(struct tcphdr, th_sport)));
  static_assert(
      (offsetof(struct __flow_l4_tcp, _flags) -
       offsetof(struct __flow_l4_tcp, _src)) ==
      (offsetof(struct tcphdr, th_flags) - offsetof(struct tcphdr, th_sport)));
  static_assert(
      (offsetof(struct __flow_l4_tcp, _win) -
       offsetof(struct __flow_l4_tcp, _src)) ==
      (offsetof(struct tcphdr, th_win) - offsetof(struct tcphdr, th_sport)));
  ...
  if (__probable(pkt->pkt_flow_ip_proto == IPPROTO_TCP)) {
    ...
    tcph =
        __DECONST(volatile struct tcphdr *, (volatile uint8_t *)iph + l3hlen);
    ...
    udph =
        __DECONST(volatile struct udphdr *, (volatile uint8_t *)iph + l3hlen);
    l4hlen = sizeof(*udph);
    CL_SKIP_ON(l4hlen > ulen);
  ...
  if (__probable(IS_P2ALIGNED(&tcph->th_sport, 4))) {
    ...
    if (__probable(pkt->pkt_flow_ip_proto == IPPROTO_TCP)) {
      src = __unsafe_forge_bidi_indexable(
          uint32_t *, __DECONST(uint32_t *, &tcph->th_sport),
          sizeof(uint32_t) * 4);
      dst = __unsafe_forge_bidi_indexable(
          uint32_t *, (uint32_t *)(void *)&pkt->pkt_flow_tcp_src,
          sizeof(uint32_t) * 4);
      sk_copy32_16(src, dst);
    } else {
      src = __unsafe_forge_bidi_indexable(
          uint32_t *, __DECONST(uint32_t *, &udph->uh_sport),
          sizeof(uint32_t) * 2);
      dst = __unsafe_forge_bidi_indexable(
          uint32_t *, (uint32_t *)(void *)&pkt->pkt_flow_udp_src,
          sizeof(uint32_t) * 2);
      sk_copy32_8(src, dst);
    }
  } else {
  ...
  if (__probable(error == 0)) {
    SK_DF(SK_VERB_FLOW_CLASSIFY,
          "pkt_length %u l3_ip_len %u "
          "l3_ip_ver 0x%x l3_proto %u l4_sport %u l4_dport %u",
          pkt->pkt_length, l3tlen, pkt->pkt_flow_ip_ver, pkt->pkt_flow_ip_proto,
          ntohs(pkt->pkt_flow_tcp_src), ntohs(pkt->pkt_flow_tcp_dst));
    ...
    if (!input) {
      if (__improbable(pkt->pkt_length != (l3tlen + pkt->pkt_l2_len))) {
        SK_ERR("packet is too long (%u), trimming to "
               "IP + L2 length (%d)",
               pkt->pkt_length, l3tlen + pkt->pkt_l2_len);
        METADATA_SET_LEN(pkt, l3tlen + pkt->pkt_l2_len, bdoff);
      }
      if (__improbable(
              ((pkt->pkt_length > mtu) && (pkt->pkt_proto_seg_sz == 0)) ||
              (pkt->pkt_proto_seg_sz > mtu))) {
        SK_ERR("dropped; length (%u) exceeds MTU (%d) "
               " proto_seg_sz %d",
               pkt->pkt_length, mtu, pkt->pkt_proto_seg_sz);
        SK_ERR("%s", sk_dump("buf", l3_hdr, cls_len, 128));
        error = EMSGSIZE;
        goto fail;
      }
    }
