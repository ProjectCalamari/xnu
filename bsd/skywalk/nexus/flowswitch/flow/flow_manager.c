...
static int fm_cmp(const struct flow_mgr *, const struct flow_mgr *);
...
static int __fe_cuckoo_cmp(struct cuckoo_node *node, void *key0) {
...
static void __fe_cuckoo_retain(struct cuckoo_node *node) {
  struct flow_entry *__single fe =
      __container_of(node, struct flow_entry, fe_cnode);
  return flow_entry_retain(fe);
}

static void __fe_cuckoo_release(struct cuckoo_node *node) {
#pragma unused(node)
  struct flow_entry *__single fe =
      __container_of(node, struct flow_entry, fe_cnode);
...
struct flow_mgr *flow_mgr_create(size_t fe_cnt, size_t fob_cnt, size_t frb_cnt,
                                 size_t frib_cnt) {
  ...
  fm = sk_alloc_type(struct flow_mgr, Z_WAITOK | Z_NOFAIL,
   ...
   */
  fm->fm_owner_buckets =
      flow_owner_buckets_alloc(fob_cnt, &fob_sz, &fob_tot_sz);
   ...
   */
  fm->fm_route_buckets =
      flow_route_buckets_alloc(frb_cnt, &frb_sz, &frb_tot_sz);
  ...
  for (i = 0; i < fm->fm_route_id_buckets_cnt; i++) {
    struct flow_route_id_bucket *frib = flow_mgr_get_frib_at_idx(fm, i);
  ...
  ASSERT(RB_FIND(flow_mgr_tree, &flow_mgr_head, &find) == fm);
#endif /* DEBUG */
...
void flow_mgr_destroy(struct flow_mgr *fm) {
  uint32_t i;
  ...
  if (fm->fm_owner_buckets != NULL) {
    ...
    flow_owner_buckets_free(fm->fm_owner_buckets, fm->fm_owner_bucket_tot_sz);
  ...
  if (fm->fm_route_buckets != NULL) {
    ...
    flow_route_buckets_free(fm->fm_route_buckets, fm->fm_route_bucket_tot_sz);
...
void flow_mgr_terminate(struct flow_mgr *fm) {
  uint32_t i;
 ...
 * the lock.
 */
struct flow_mgr *flow_mgr_find_lock(uuid_t uuid) {
 ...
 */
void flow_mgr_unlock(void) { lck_rw_done(&flow_mgr_lock); }

static inline int fm_cmp(const struct flow_mgr *a, const struct flow_mgr *b) {
  return uuid_compare(a->fm_uuid, b->fm_uuid);
}

static void flow_mgr_clear_embedded_scope_id(struct sockaddr_in6 *addr) {
  struct in6_addr *in6;
  in6 = &addr->sin6_addr;
...
#if CONFIG_MACF
static bool flow_req_check_mac_allowed(struct nx_flow_req *req) {
  int socktype;
  ...
  if (req->nfr_flags & NXFLOWREQF_LISTENER) {
    return mac_skywalk_flow_check_listen(req->nfr_proc, NULL,
                                         SA(&req->nfr_saddr.sa), socktype,
                                         req->nfr_ip_protocol);
  } else {
    return mac_skywalk_flow_check_connect(req->nfr_proc, NULL,
                                          SA(&req->nfr_daddr.sa), socktype,
                                          req->nfr_ip_protocol);
  }
}
#endif /* CONFIG_MACF */

static bool flow_req_needs_netns_reservation(struct nx_flow_req *req) {
  uint8_t proto = req->nfr_ip_protocol;
  return proto == IPPROTO_TCP || proto == IPPROTO_UDP;
}

static bool flow_req_needs_protons_reservation(struct nx_flow_req *req) {
  uint8_t proto = req->nfr_ip_protocol;
  return proto != IPPROTO_TCP && proto != IPPROTO_UDP && proto != IPPROTO_ESP &&
         proto != IPPROTO_AH;
}

static bool flow_req_needs_ipsec_reservation(struct nx_flow_req *req) {
  uint8_t proto = req->nfr_ip_protocol;
  return proto == IPPROTO_ESP || proto == IPPROTO_AH;
}

static void flow_set_port_info(struct ns_flow_info *nfi,
  ...
  proc_name(req->nfr_pid, nfi->nfi_owner_name, sizeof(nfi->nfi_owner_name));
}

static int flow_req_prepare_namespace(struct nx_flow_req *req) {
  ...
  if (flow_req_needs_netns_reservation(req)) {
    if (!NETNS_TOKEN_VALID(&req->nfr_port_reservation)) {
      ...
      err = flow_namespace_create(saddr, req->nfr_ip_protocol, &ns_token,
                                  req->nfr_flags, &nfi);
      if (err != 0) {
      ...
      struct ns_flow_info nfi = {};
      err = netns_get_flow_info(&req->nfr_port_reservation, &nfi);
      /* flow info could be NULL for socket flow */
      if (!err &&
          (req->nfr_pid != nfi.nfi_owner_pid ||
           (req->nfr_epid != -1 && nfi.nfi_effective_pid != req->nfr_epid))) {
        SK_ERR("netns flow info mismatch, "
               "req_(e)pid %d(%d), nfr_(e)pid %d(%d)",
               req->nfr_pid, req->nfr_epid, nfi.nfi_owner_pid,
               nfi.nfi_effective_pid);
        err = EPERM;
        goto fail;
      }
      req->nfr_flags |= NXFLOWREQF_EXT_PORT_RSV;
  ...
  if (flow_req_needs_ipsec_reservation(req)) {
    ...
    err = key_reserve_custom_ipsec(&ipsec_token, saddr, daddr,
                                   req->nfr_ip_protocol);
    if (err != 0) {
      SK_ERR("custom ipsec %u reserve %s failed", req->nfr_ip_protocol,
             sk_sa_ntop(SA(saddr), src_s, sizeof(src_s)));
      goto fail;
    }
  ...
  if (flow_req_needs_protons_reservation(req)) {
    ...
    if (!protons_token_is_valid(req->nfr_proto_reservation)) {
      err = protons_reserve(&ns_token, req->nfr_pid, req->nfr_epid,
                            req->nfr_ip_protocol);
      if (err != 0) {
        SK_ERR("protocol %u namespace failed", req->nfr_ip_protocol);
        goto fail;
      }
...
static int flow_req_prepare(struct nx_flow_req *req, struct kern_nexus *nx,
                            struct flow_mgr *fm, struct ifnet *ifp,
                            flow_route_ctor_fn_t fr_ctor,
  ...
  case AF_INET:
    ...
    if (__probable(protocol == IPPROTO_TCP)) {
      INC_ATOMIC_INT64_LIM(net_api_stats.nas_nx_flow_inet6_stream_total);
    } else {
      INC_ATOMIC_INT64_LIM(net_api_stats.nas_nx_flow_inet6_dgram_total);
    }
    break;
  ...
  case AF_INET6:
    ...
    if (__probable(protocol == IPPROTO_TCP)) {
      INC_ATOMIC_INT64_LIM(net_api_stats.nas_nx_flow_inet_stream_total);
    } else {
      INC_ATOMIC_INT64_LIM(net_api_stats.nas_nx_flow_inet_dgram_total);
    }
    break;
  ...
  if (__improbable(has_saddr && !flow_route_laddr_validate(
                                    saddr, ifp, &req->nfr_saddr_gencnt))) {
    SK_LOG_VAR(char src_s[MAX_IPv6_STR_LEN]);
  ...
  if (req->nfr_flow_demux_count > 0) {
    if (req->nfr_ip_protocol != IPPROTO_UDP) {
      err = EINVAL;
      SK_ERR("invalid ip protocol(%u) for flow demux", req->nfr_ip_protocol);
      goto fail;
    }
  ...
  if (__probable(has_daddr || has_dport)) {
    ...
    err = flow_route_find(nx, fm, ifp, req, fr_ctor, fr_resolve, fr_arg, &fr);
...
static void flow_req_cleanup(struct nx_flow_req *req) {
  if (NETNS_TOKEN_VALID(&req->nfr_port_reservation) &&
      !(req->nfr_flags & NXFLOWREQF_EXT_PORT_RSV)) {
...
SK_LOG_ATTRIBUTE
static void flow_req_dump(char *desc, struct nx_flow_req *req) {
  ...
  if (saddr->sa.sa_family == AF_INET) {
    sipver = IPVERSION;
    (void)sk_ntop(AF_INET, &SIN(saddr)->sin_addr, src_s, sizeof(src_s));
    sport = ntohs(saddr->sin.sin_port);
  } else if (saddr->sa.sa_family == AF_INET6) {
    sipver = IPV6_VERSION;
    (void)sk_ntop(AF_INET6, &SIN6(saddr)->sin6_addr, src_s, sizeof(src_s));
  ...
  if (daddr->sa.sa_family == AF_INET) {
    dipver = IPVERSION;
    (void)sk_ntop(AF_INET, &SIN(daddr)->sin_addr, dst_s, sizeof(dst_s));
    dport = ntohs(daddr->sin.sin_port);
  } else if (daddr->sa.sa_family == AF_INET6) {
    dipver = IPV6_VERSION;
    (void)sk_ntop(AF_INET6, &SIN6(daddr)->sin6_addr, dst_s, sizeof(dst_s));
  ...
  SK_DF(SK_VERB_FLOW,
        ...
        "nx_port=%u,flags 0x%x",
        desc, sk_uuid_unparse(req->nfr_flow_uuid, uuid_s), sipver, dipver,
        src_s, sport, dst_s, dport, protocol, req->nfr_nx_port, req->nfr_flags);
}
#else
#define flow_req_dump(str, req)                                                \
  do {                                                                         \
    ((void)0);                                                                 \
  } while (0)
#endif /* SK_LOG */
 ...
 */
int flow_mgr_flow_add(struct kern_nexus *nx, struct flow_mgr *fm,
                      struct flow_owner *fo, struct ifnet *ifp,
                      struct nx_flow_req *req, flow_route_ctor_fn_t fr_ctor,
                      flow_route_resolve_fn_t fr_resolve, void *fr_arg) {
  struct flow_entry *__single fe;
  int err = 0;
  ...
  VERIFY(NETNS_TOKEN_VALID(&fe->fe_port_reservation) ||
         ...
         req->nfr_flags & NXFLOWREQF_ASIS || (fe->fe_flags & FLOWENTF_CHILD));
  ...
  if (req->nfr_saddr.sa.sa_family == AF_INET6 &&
    ...
    req->nfr_saddr.sin6.sin6_scope_id = ifnet_index(fsw->fsw_ifp);
  }
  if (req->nfr_daddr.sa.sa_family == AF_INET6 &&
      IN6_IS_SCOPE_EMBED(&req->nfr_daddr.sin6.sin6_addr)) {
    req->nfr_daddr.sin6.sin6_scope_id = ifnet_index(fsw->fsw_ifp);
  }
...
struct flow_owner_bucket *flow_mgr_get_fob_by_pid(struct flow_mgr *fm,
                                                  pid_t pid) {
  return flow_mgr_get_fob_at_idx(fm, (pid % fm->fm_owner_buckets_cnt));
}

struct flow_entry *flow_mgr_get_fe_by_uuid_rlock(struct flow_mgr *fm,
                                                 uuid_t uuid) {
  uint32_t i;
...
flow_mgr_get_frb_by_addr(struct flow_mgr *fm, union sockaddr_in_4_6 *daddr) {
...
struct flow_route_id_bucket *flow_mgr_get_frib_by_uuid(struct flow_mgr *fm,
                                                       uuid_t fr_uuid) {
  union {
  ...
  return flow_mgr_get_frib_at_idx(
      fm, ((uint32_t)key % fm->fm_route_id_buckets_cnt));
}

static int flow_hash_mask_add(struct flow_mgr *fm, uint32_t mask, int32_t v) {
...
int flow_mgr_flow_hash_mask_add(struct flow_mgr *fm, uint32_t mask) {
  return flow_hash_mask_add(fm, mask, 1);
}

int flow_mgr_flow_hash_mask_del(struct flow_mgr *fm, uint32_t mask) {
  return flow_hash_mask_add(fm, mask, -1);
}

#if SK_LOG
SK_NO_INLINE_ATTRIBUTE
static void __flow_mgr_find_fe_by_key_prelog(struct flow_key *key) {
...
SK_NO_INLINE_ATTRIBUTE
static void __flow_mgr_find_fe_by_key_epilog(struct flow_entry *fe) {
  SK_LOG_VAR(char dbgbuf[FLOWENTRY_DBGBUF_SIZE]);
  if (fe != NULL) {
...
#else
#define __flow_mgr_find_fe_by_key_prelog(key)                                  \
  do {                                                                         \
    ((void)0);                                                                 \
  } while (0)
#define __flow_mgr_find_fe_by_key_epilog(fe)                                   \
  do {                                                                         \
    ((void)0);                                                                 \
  } while (0)
#endif /* SK_LOG */

struct flow_entry *flow_mgr_find_fe_by_key(struct flow_mgr *fm,
                                           struct flow_key *key) {
  ...
  for (int i = 0; i < FKMASK_IDX_MAX; i++) {
    ...
    if (count == 0 || mask == 0) {
      SK_DF(SK_VERB_FLOW | SK_VERB_LOOKUP, "[%d] mask=%08x count=%zu skiped", i,
            mask, count);
      continue;
    }
    key->fk_mask = mask;
    ...
    SK_DF(SK_VERB_FLOW | SK_VERB_LOOKUP, "[%d] mask=%08x hash %08x node %p", i,
          mask, hash, SK_KVA(node));
    if (node != NULL) {
      ...
      if (__improbable(fe->fe_key.fk_mask == FKMASK_2TUPLE &&
        ...
        flow_entry_release(&fe);
        ASSERT(fe == NULL);
        SK_DF(SK_VERB_FLOW | SK_VERB_LOOKUP, "\tskip v4 only fe");
        continue;
      }
      break;
    }
...
struct flow_entry *flow_mgr_find_conflicting_fe(struct flow_mgr *fm,
                                                struct flow_key *key) {
  struct cuckoo_node *node = NULL;
  struct flow_entry *__single fe = NULL;
  uint32_t hash = 0;
...
void flow_mgr_foreach_flow(struct flow_mgr *fm,
                           void (^flow_handler)(struct flow_entry *fe)) {
  cuckoo_hashtable_foreach(
      fm->fm_flow_table, ^(struct cuckoo_node *node, uint32_t hv) {
#pragma unused(hv)
        struct flow_entry *__single fe;
        fe = __container_of(node, struct flow_entry, fe_cnode);
        flow_handler(fe);
...
bool rx_flow_demux_match(struct nx_flowswitch *fsw, struct flow_entry *fe,
  ...
  uh = __unsafe_forge_bidi_indexable(struct udphdr *,
                                     (struct udphdr *)pkt->pkt_flow_udp_hdr,
                                     sizeof(*uh) + pkt->pkt_flow_ulen);
  if (__improbable(uh == NULL || pkt->pkt_flow_ulen == 0)) {
    return false;
  }

  int udp_payload_offset =
      pkt->pkt_l2_len + pkt->pkt_flow_ip_hlen + sizeof(*uh);
  ...
  for (int index = 0; index < fe->fe_demux_pattern_count; index++) {
    struct flow_demux_pattern *demux_pattern =
        &fe->fe_demux_patterns[index].fdp_demux_pattern;
    ASSERT(demux_pattern->fdp_len > 0);

    if (pkt->pkt_flow_ulen >=
        demux_pattern->fdp_offset + demux_pattern->fdp_len) {
      if (__probable(pkt_payload_len >=
                     demux_pattern->fdp_offset + demux_pattern->fdp_len)) {
        demux_data = (uint8_t *)(uh + 1) + demux_pattern->fdp_offset;
      } else {
        if (pkt->pkt_pflags & PKT_F_MBUF_DATA) {
          m_copydata(pkt->pkt_mbuf,
                     udp_payload_offset + demux_pattern->fdp_offset,
      ...
      if (fe->fe_demux_patterns[index].fdp_memcmp_mask != NULL) {
        result = fe->fe_demux_patterns[index].fdp_memcmp_mask(
            demux_data, demux_pattern->fdp_value, demux_pattern->fdp_mask);
      } else {
        result =
            sk_memcmp_mask(demux_data, demux_pattern->fdp_value,
...
struct flow_entry *rx_lookup_child_flow(struct nx_flowswitch *fsw,
                                        struct flow_entry *parent_fe,
                                        struct __kern_packet *pkt) {
  struct flow_entry *child_fe;
...
struct flow_entry *tx_lookup_child_flow(struct flow_entry *parent_fe,
                                        uuid_t flow_id) {
  struct flow_entry *child_fe;
