...
#define PKT_IS_TRUNC_MBUF(_pkt)                                                \
  (PKT_IS_MBUF(_pkt) && (_pkt->pkt_pflags & PKT_F_TRUNCATED))
#define PKT_IS_WAKE_PKT(_pkt)                                                  \
  ((PKT_IS_MBUF(_pkt) &&                                                       \
    (pkt->pkt_mbuf->m_pkthdr.pkt_flags & PKTF_WAKE_PKT)) ||                    \
   (!PKT_IS_MBUF(_pkt) && (_pkt->pkt_pflags & PKT_F_WAKE_PKT)))

typedef uint16_t (*flow_agg_fix_pkt_sum_func)(uint16_t, uint16_t, uint16_t);

static uint16_t flow_agg_pkt_fix_sum(uint16_t csum, uint16_t old, uint16_t new);

static uint16_t flow_agg_pkt_fix_sum_no_op(uint16_t csum, uint16_t old,
                                           uint16_t new);
...
static const struct ip_tcp_mask ip_tcp_mask __sk_aligned(16) = {
    .ip_m =
        {
            .ip_hl = 0xf,
            .ip_v = 0xf,
...
static const struct ip6_tcp_mask ip6_tcp_mask __sk_aligned(16) = {
    .ip6_m =
        {
...
#if SK_LOG
SK_LOG_ATTRIBUTE
static void _pkt_agg_log(struct __kern_packet *pkt, struct proc *p,
                         bool is_input) {
  SK_LOG_VAR(uint64_t logflags =
                 ((SK_VERB_FSW | SK_VERB_RX) |
  ...
  SK_DF(logflags, "%s(%d) %spkt %p plen %u", sk_proc_name(p), sk_proc_pid(p),
        is_input ? "s" : "d", SK_KVA(pkt), pkt->pkt_length);

  SK_DF(logflags, "%spkt csumf/rxstart/rxval 0x%x/%u/0x%04x",
        is_input ? "s" : "d", pkt->pkt_csum_flags,
        (uint32_t)pkt->pkt_csum_rx_start_off, (uint32_t)pkt->pkt_csum_rx_value);
...
#define pkt_agg_log(_pkt, _p, _is_input)                                       \
  do {                                                                         \
...
SK_LOG_ATTRIBUTE
static void _mbuf_agg_log(struct mbuf *m, struct proc *p, bool is_mbuf) {
  SK_LOG_VAR(uint64_t logflags =
                 ((SK_VERB_FSW | SK_VERB_RX) |
                  (is_mbuf ? SK_VERB_COPY_MBUF : SK_VERB_COPY)));

  SK_DF(logflags, "%s(%d) dest mbuf %p pktlen %u", sk_proc_name(p),
        sk_proc_pid(p), SK_KVA(m), m->m_pkthdr.len);
  ...
  SK_DF(logflags | SK_VERB_DUMP, "%s",
        sk_dump("buf", (uint8_t *)m_mtod_current(m), m->m_len, 128));
}

#define mbuf_agg_log(_m, _p, _is_mbuf)                                         \
  do {                                                                         \
    if (__improbable(sk_verbose != 0)) {                                       \
...
SK_LOG_ATTRIBUTE
static void _mchain_agg_log(struct mbuf *m, struct proc *p, bool is_mbuf) {
  SK_LOG_VAR(uint64_t logflags =
                 ((SK_VERB_FSW | SK_VERB_RX) |
                  (is_mbuf ? SK_VERB_COPY_MBUF : SK_VERB_COPY)));
...
#define mchain_agg_log(_m, _p, _is_mbuf)                                       \
  do {                                                                         \
    if (__improbable(sk_verbose != 0)) {                                       \
 ...
 */
static bool mbuf_csum(struct __kern_packet *pkt, struct mbuf *m, bool verify_l3,
                      uint16_t *data_csum) {
  ...
  if ((m->m_pkthdr.csum_flags & CSUM_RX_FULL_FLAGS) == CSUM_RX_FULL_FLAGS) {
    ...
    struct tcphdr *tcp =
        (struct tcphdr *)(void *)(mtod(m, uint8_t *) + pkt->pkt_l2_len +
    ...
    partial = m_sum16(m, start + pkt->pkt_flow_ip_hlen, pkt->pkt_flow_tcp_hlen);
    ...
    if (pkt->pkt_flow_ip_ver == IPVERSION) {
      ...
      csum =
          in6_pseudo(&pkt->pkt_flow_ipv6_src, &pkt->pkt_flow_ipv6_dst, partial);
    }
    ...
    *data_csum = ~th_sum & 0xffff;
    ...
    __packet_set_inet_checksum(SK_PKT2PH(pkt), PACKET_CSUM_RX_FULL_FLAGS, 0,
                               m->m_pkthdr.csum_rx_val, false);
  ...
  if (verify_l3) {
    ...
    SK_DF(logflags, "IP copy+sum %u(%u) (csum 0x%04x)", start,
          pkt->pkt_flow_ip_hlen, csum);
  ...
  *data_csum = m_sum16(m, start, (plen - start));
  ...
  /* pkt metadata will be transfer to super packet */
  __packet_set_inet_checksum(SK_PKT2PH(pkt), PACKET_CSUM_RX_FULL_FLAGS, 0, csum,
                             false);
...
static inline void _copy_data_sum_dbuf(struct __kern_packet *spkt,
                                       uint16_t soff, uint16_t plen,
                                       uint32_t *partial_sum,
                                       boolean_t *odd_start,
                                       _dbuf_array_t *dbuf, boolean_t do_csum) {
  uint8_t i = 0;
  ...
  while (plen > 0) {
    ...
    if (PKT_IS_TRUNC_MBUF(spkt)) {
      if (do_csum) {
        *partial_sum = m_copydata_sum(spkt->pkt_mbuf, soff, tmplen, dbuf_addr,
                                      *partial_sum, odd_start);
      } else {
        m_copydata(spkt->pkt_mbuf, soff, tmplen, dbuf_addr);
      }
    } else {
      *partial_sum = pkt_copyaddr_sum(SK_PKT2PH(spkt), soff, dbuf_addr, tmplen,
                                      do_csum, *partial_sum, odd_start);
    }
    if (dbuf->dba_is_buflet) {
 ...
 */
static bool copy_pkt_csum_packed(struct __kern_packet *spkt, uint32_t plen,
                                 _dbuf_array_t *dbuf, bool verify_l3,
                                 struct mbuf *currm,
                                 struct __kern_buflet *currp,
                                 uint16_t *data_csum, int *added) {
  ASSERT(data_csum != NULL);

  SK_LOG_VAR(uint64_t logflags = (SK_VERB_FSW | SK_VERB_RX | SK_VERB_COPY));
  ...
  if (currm != NULL) {
    ...
    curr_trailing = currp->buf_dlim - currp->buf_doff - currp->buf_dlen;
    /* XXX -fbounds-safety: use the inline variant to return an __indexable */
  ...
  if (verify_l3) {
    if (PKT_IS_TRUNC_MBUF(spkt)) {
      partial = os_cpu_in_cksum_mbuf(spkt->pkt_mbuf, len, 0, 0);
    } else {
      partial = pkt_sum(SK_PKT2PH(spkt), soff, len);
    }

    csum = __packet_fold_sum(partial);
    SK_DF(logflags, "IP copy+sum %u(%u) (csum 0x%04x)", 0, len, csum);
  ...
  if (PKT_IS_TRUNC_MBUF(spkt)) {
    ...
    if (verify_l4) {
      partial = os_cpu_in_cksum_mbuf(spkt->pkt_mbuf, spkt->pkt_flow_tcp_hlen,
                                     spkt->pkt_flow_ip_hlen, 0);
    }

    /* Now, copy & sum the payload */
    if (tmplen > 0) {
      data_partial = m_copydata_sum(spkt->pkt_mbuf, start, tmplen, curr_ptr, 0,
                                    &odd_start);
      curr_len += tmplen;
    }
    ...
    /* First, simple checksum on the TCP header */
    if (verify_l4) {
      partial = pkt_sum(SK_PKT2PH(spkt), (soff + spkt->pkt_flow_ip_hlen),
                        spkt->pkt_flow_tcp_hlen);
    }

    /* Now, copy & sum the payload */
    if (tmplen > 0) {
      data_partial =
          pkt_copyaddr_sum(SK_PKT2PH(spkt), (soff + start), (uint8_t *)curr_ptr,
                           tmplen, true, 0, &odd_start);
      curr_len += tmplen;
    }
  ...
  if ((len - tmplen) > 0) {
    ASSERT(dbuf != NULL);
    _copy_data_sum_dbuf(spkt, data_off, (len - tmplen), &data_partial,
                        &odd_start, dbuf, true);
    *added = (len - tmplen);
  }
  ...
  *data_csum = __packet_fold_sum(data_partial);
  ...
  if (verify_l4) {
    ...
    if (spkt->pkt_flow_ip_ver == IPVERSION) {
      ...
      csum = in6_pseudo(&spkt->pkt_flow_ipv6_src, &spkt->pkt_flow_ipv6_dst,
                        partial);
    }
    /* pkt metadata will be transfer to super packet */
    __packet_set_inet_checksum(SK_PKT2PH(spkt), PACKET_CSUM_RX_FULL_FLAGS, 0,
                               csum, false);
  } else {
 ...
 */
static bool copy_pkt_csum(struct __kern_packet *pkt, uint32_t plen,
                          _dbuf_array_t *dbuf, uint16_t *data_csum,
                          bool verify_l3) {
  /*
  ...
  if (PACKET_HAS_FULL_CHECKSUM_FLAGS(pkt)) {
    /* copy only */
    _copy_data_sum_dbuf(pkt, PKT_IS_TRUNC_MBUF(pkt) ? 0 : soff, plen, &partial,
                        &odd_start, dbuf, false);
    if (PKT_IS_MBUF(pkt)) {
      ...
      SK_DF(logflags, "HW csumf/rxstart/rxval 0x%x/%u/0x%04x",
            pkt->pkt_csum_flags, pkt->pkt_csum_rx_start_off, csum);
    }
  ...
  if (PKT_IS_TRUNC_MBUF(pkt)) {
    partial =
        m_copydata_sum(pkt->pkt_mbuf, start, len, (daddr + start), 0, NULL);
  } else {
    partial = pkt_copyaddr_sum(SK_PKT2PH(pkt), soff, (daddr + start), len, true,
                               0, NULL);
  }
  ...
  if (verify_l3) {
    csum = __packet_fold_sum(partial);
    SK_DF(logflags, "IP copy+sum %u(%u) (csum 0x%04x)", start, len, csum);
  ...
  if (PKT_IS_TRUNC_MBUF(pkt)) {
    ...
    partial = m_copydata_sum(pkt->pkt_mbuf, start, pkt->pkt_flow_tcp_hlen,
                             (daddr + start), 0, NULL);
    ...
    /* First, copy and sum TCP header */
    partial = pkt_copyaddr_sum(SK_PKT2PH(pkt), (soff + start), (daddr + start),
                               pkt->pkt_flow_tcp_hlen, true, 0, NULL);
  ...
  if (dbuf->dba_is_buflet) {
    VERIFY(kern_buflet_set_data_length(
               dbuf->dba_buflet[0],
               kern_buflet_get_data_length(dbuf->dba_buflet[0]) + copied_len) ==
           0);
  } else {
  ...
  if (data_len > 0) {
    odd_start = false;
    _copy_data_sum_dbuf(pkt, start, data_len, &data_partial, &odd_start, dbuf,
                        l3_csum_ok);
  }
  ...
  *data_csum = __packet_fold_sum(data_partial);
  ...
  SK_DF(logflags, "TCP copy+sum %u(%u) (csum 0x%04x)", pkt->pkt_flow_ip_hlen,
        len, csum);
...
SK_INLINE_ATTRIBUTE
static void flow_agg_init_common(struct nx_flowswitch *fsw, struct flow_agg *fa,
...
static void flow_agg_init_smbuf(struct nx_flowswitch *fsw, struct flow_agg *fa,
...
static void flow_agg_init_spkt(struct nx_flowswitch *fsw, struct flow_agg *fa,
 ...
 */
SK_INLINE_ATTRIBUTE
static bool ipv4_tcp_memcmp(const uint8_t *__counted_by(64) h1,
                            const uint8_t *__counted_by(64) h2) {
  return sk_memcmp_mask_64B(h1, h2, (const uint8_t *)&ip_tcp_mask) == 0;
}

SK_INLINE_ATTRIBUTE
static bool ipv6_tcp_memcmp(const uint8_t *__counted_by(80) h1,
                            const uint8_t *__counted_by(80) h2) {
  return sk_memcmp_mask_80B(h1, h2, (const uint8_t *)&ip6_tcp_mask) == 0;
}

SK_INLINE_ATTRIBUTE
static bool can_agg_fastpath(struct flow_agg *fa, struct __kern_packet *pkt,
  ...
  case IPVERSION:
     ...
     */
    ip_hdr = __unsafe_forge_bidi_indexable(uint8_t *, pkt->pkt_flow_ip_hdr, 64);
    match = ipv4_tcp_memcmp(fa->fa_sptr, ip_hdr);
    break;
  case IPV6_VERSION:
    ip_hdr = __unsafe_forge_bidi_indexable(uint8_t *, pkt->pkt_flow_ip_hdr, 80);
...
static bool can_agg_slowpath(struct flow_agg *fa, struct __kern_packet *pkt,
                             struct fsw_stats *fsws) {
  uint8_t *sl3_hdr = fa->fa_sptr;
  uint8_t *l3_hdr = __unsafe_forge_bidi_indexable(
      uint8_t *, pkt->pkt_flow_ip_hdr, pkt->pkt_flow_ip_hlen);
  uint32_t sl3tlen = 0;
  uint16_t sl3hlen = 0;

  DTRACE_SKYWALK2(aggr__slow, struct __kern_packet *, pkt, uint8_t *, sl3_hdr);
  ...
  if (pkt->pkt_flow_ip_ver == IPVERSION) {
    ...
    if (siph->ip_ttl != iph->ip_ttl) {
      ...
      DTRACE_SKYWALK2(aggr__fail3, uint8_t, siph->ip_ttl, uint8_t, iph->ip_ttl);
      return false;
    }

    if (siph->ip_tos != iph->ip_tos) {
      STATS_INC(fsws, FSW_STATS_RX_AGG_NO_TOS_IP);
      DTRACE_SKYWALK2(aggr__fail4, uint8_t, siph->ip_tos, uint8_t, iph->ip_tos);
      return false;
    }
    ...
    if ((ntohs(siph->ip_off) & (IP_DF | IP_RF)) !=
      ...
      DTRACE_SKYWALK2(aggr__fail5, uint16_t, ntohs(siph->ip_off), uint16_t,
                      ntohs(iph->ip_off));
      return false;
    }

    uint8_t ip_opts_len = pkt->pkt_flow_ip_hlen - sizeof(struct ip);
    if (ip_opts_len > 0 &&
        memcmp((uint8_t *)(siph + 1), (uint8_t *)(iph + 1), ip_opts_len) != 0) {
      STATS_INC(fsws, FSW_STATS_RX_AGG_NO_OPT_IP);
      DTRACE_SKYWALK3(aggr__fail6, uint8_t, ip_opts_len, uint8_t *,
                      (uint8_t *)(siph + 1), uint8_t *, (uint8_t *)(iph + 1));
      return false;
    }
    ...
    if (pkt->pkt_flow_ip_hlen != sizeof(struct ip6_hdr)) {
       ...
       */
      STATS_INC(fsws, FSW_STATS_RX_AGG_NO_HLEN_IP);
      DTRACE_SKYWALK1(aggr__fail7, uint8_t, pkt->pkt_flow_ip_hlen);
      return false;
    }
    ...
    if (memcmp((uint8_t *)&sip6->ip6_flow, (uint8_t *)&ip6->ip6_flow,
      ...
      STATS_INC(fsws, FSW_STATS_RX_AGG_NO_TOS_IP);
      DTRACE_SKYWALK2(aggr__fail8, uint32_t, ntohl(sip6->ip6_flow), uint32_t,
                      ntohl(ip6->ip6_flow));
      return false;
    }
    ...
    if (sip6->ip6_hlim != ip6->ip6_hlim) {
      STATS_INC(fsws, FSW_STATS_RX_AGG_NO_TTL_IP);
      DTRACE_SKYWALK2(aggr__fail9, uint8_t, sip6->ip6_hlim, uint8_t,
                      ip6->ip6_hlim);
      return false;
    }
  ...
  struct tcphdr *tcp = __unsafe_forge_bidi_indexable(
  ...
  if (memcmp(&stcp->th_ack, &tcp->th_ack, sizeof(stcp->th_ack)) != 0 ||
    ...
    DTRACE_SKYWALK4(aggr__fail9, uint32_t, ntohl(stcp->th_ack), uint32_t,
                    ntohl(tcp->th_ack), uint16_t, ntohs(stcp->th_win), uint16_t,
                    ntohs(tcp->th_win));
    return false;
  }
  ...
  if ((stcp->th_flags & ~(TH_PUSH)) != (tcp->th_flags & ~(TH_PUSH))) {
    ...
    DTRACE_SKYWALK2(aggr__fail10, uint8_t, stcp->th_flags, uint8_t,
                    tcp->th_flags);
    return false;
  }
  ...
  if (sl4hlen != pkt->pkt_flow_tcp_hlen) {
    ...
    DTRACE_SKYWALK2(aggr__fail11, uint8_t, sl4hlen, uint8_t,
   ...
   */
  if (tcp_opts_len > 0 &&
      memcmp((uint8_t *)(stcp + 1), (uint8_t *)(tcp + 1), tcp_opts_len) != 0) {
    /*
    ...
    if (tcp_opts_len != TCPOLEN_TSTAMP_APPA) {
      ...
      if (sts_hdr != htonl(TCPOPT_TSTAMP_HDR) ||
        ...
        DTRACE_SKYWALK2(aggr__fail14, uint32_t, sts_hdr, uint32_t, ts_hdr);
        return false;
      }
    }
  }
...
static bool flow_agg_is_ok(struct flow_agg *fa, struct __kern_packet *pkt,
                           struct fsw_stats *fsws) {
  /* Shouldn't exceed the ip_len beyond MIN(custom ip_len, 64K) */
  const uint32_t max_ip_len = MAX_AGG_IP_LEN();
  bool can_agg = false;

  DTRACE_SKYWALK2(aggr__check, struct flow_agg *, fa, struct __kern_packet *,
                  pkt);
  ...
  if (__improbable(ntohl(pkt->pkt_flow_tcp_seq) != fa->fa_tcp_seq)) {
    DTRACE_SKYWALK2(aggr__fail1b, uint32_t, ntohl(pkt->pkt_flow_tcp_seq),
                    uint32_t, fa->fa_tcp_seq);
    STATS_INC(fsws, FSW_STATS_RX_AGG_NO_SEQN_TCP);
    goto done;
  }
  if (__improbable((fa->fa_total + pkt->pkt_flow_ulen) > max_ip_len)) {
    DTRACE_SKYWALK3(aggr__fail1c, uint32_t, fa->fa_total, uint32_t,
                    pkt->pkt_flow_ulen, uint32_t, max_ip_len);
...
static uint16_t flow_agg_pkt_fix_sum(uint16_t csum, uint16_t old,
                                     uint16_t new) {
  return __packet_fix_sum(csum, old, new);
}

static uint16_t flow_agg_pkt_fix_sum_no_op(uint16_t __unused csum,
                                           uint16_t __unused old,
...
flow_agg_pkt_fix_hdr_sum(struct flow_agg *fa,
                         uint8_t *__sized_by(sizeof(uint32_t)) field,
                         uint16_t *csum, uint32_t new) {
  uint32_t old;
  memcpy((uint8_t *)&old, field, sizeof(old));
  memcpy(field, (uint8_t *)&new, sizeof(uint32_t));
  *csum = fa->fa_fix_pkt_sum(
      fa->fa_fix_pkt_sum(*csum, (uint16_t)(old >> 16), (uint16_t)(new >> 16)),
      (uint16_t)(old & 0xffff), (uint16_t)(new & 0xffff));
}

static void flow_agg_merge_hdr(struct flow_agg *fa, struct __kern_packet *pkt,
                               __unused uint16_t data_csum,
                               struct fsw_stats *fsws) {
  ...
  if (pkt->pkt_flow_ip_ver == IPVERSION) {
    ...
    siph->ip_sum =
        fa->fa_fix_pkt_sum(siph->ip_sum, 0, htons(pkt->pkt_flow_ulen));
  ...
  tcp = __unsafe_forge_bidi_indexable(struct tcphdr *,
                                      (struct tcphdr *)pkt->pkt_flow_tcp_hdr,
                                      pkt->pkt_flow_tcp_hlen);
  ...
  if (pkt->pkt_flow_tcp_hlen > sizeof(struct tcphdr) ||
    ...
    if (__improbable(!pkt->pkt_flow_tcp_agg_fast &&
                     memcmp(stcp + 1, tcp + 1,
                            (pkt->pkt_flow_tcp_hlen - sizeof(struct tcphdr))) !=
                         0)) {
    ...
    if ((stcp->th_flags & TH_PUSH) == 0 && (tcp->th_flags & TH_PUSH) != 0) {
      uint16_t old, new;
       ...
       */
      uint16_t *next = __unsafe_forge_single(uint16_t *, th_ack + 1);
      old = *next;
  ...
  stcp->th_sum = fa->fa_fix_pkt_sum(stcp->th_sum, 0, htons(pkt->pkt_flow_ulen));

  /* Update data checksum  */
  if (__improbable(old_l3len & 0x1)) {
    /* swap the byte order, refer to rfc 1071 section 2 */
    stcp->th_sum = fa->fa_fix_pkt_sum(stcp->th_sum, 0, ntohs(data_csum));
  } else {
  ...
  if (fa->fa_sobj_is_pkt) {
    ...
    SK_DF(logflags, "Agg pkt len %u TCP csum 0x%04x", spkt->pkt_length,
          ntohs(stcp->th_sum));
  } else {
    ...
    SK_DF(logflags, "Agg mbuf len %u TCP csum 0x%04x", smbuf->m_pkthdr.len,
          ntohs(stcp->th_sum));
  }
}
 ...
 */
static void pkt_copy_metadata(struct __kern_packet *spkt,
...
static void pkt_finalize(kern_packet_t ph) {
  ...
  DTRACE_SKYWALK2(aggr__finalize, struct __kern_packet *, pkt, uint8_t *, buf);
#endif
}

static inline uint32_t estimate_buf_cnt(struct flow_entry *fe,
                                        uint32_t total_bytes,
                                        uint32_t total_pkts,
                                        uint32_t min_bufsize,
  ...
  hdr_overhead =
      (total_bytes / max_ip_len) *
      (MAX(sizeof(struct ip), sizeof(struct ip6_hdr)) + sizeof(struct tcphdr));

  return ((total_bytes + hdr_overhead) / agg_size) + 1;
}

SK_INLINE_ATTRIBUTE
static inline void _append_dbuf_array_to_kpkt(kern_packet_t ph,
                                              kern_buflet_t pbuf,
...
SK_INLINE_ATTRIBUTE
static inline void _free_dbuf_array(struct kern_pbufpool *pp,
                                    _dbuf_array_t *dbuf_array) {
  for (uint8_t i = 0; i < dbuf_array->dba_num_dbufs; i++) {
    kern_buflet_t buf = dbuf_array->dba_buflet[i];
...
static inline void finalize_super_packet(struct __kern_packet **spkt,
                                         kern_packet_t *sph,
                                         struct flow_agg *fa,
                                         uint32_t *largest_spkt,
                                         uint16_t *spkts, uint16_t bufcnt) {
  (*spkts)++;
  if (bufcnt > 1) {
    (*spkt)->pkt_aggr_type = PKT_AGGR_SINGLE_IP;
  }
...
static inline void converge_aggregation_size(struct flow_entry *fe,
                                             uint32_t largest_agg_size) {
...
SK_NO_INLINE_ATTRIBUTE
static void flow_rx_agg_channel(struct nx_flowswitch *fsw,
                                struct flow_entry *fe, struct pktq *rx_pkts,
                                uint32_t rx_bytes, bool is_mbuf) {
#define __RX_AGG_CHAN_DROP_SOURCE_PACKET(_pkt, _reason, _flags)                \
  do {                                                                         \
    pp_drop_packet_single(_pkt, fsw->fsw_ifp, _flags, _reason, __func__,       \
                          __LINE__);                                           \
    (_pkt) = NULL;                                                             \
    FLOW_AGG_CLEAR(&fa);                                                       \
    prev_csum_ok = false;                                                      \
  ...
  if (__probable(fe->fe_rx_largest_size != 0 && NX_FSW_TCP_RX_AGG_ENABLED())) {
  ...
  err =
      pp_alloc_buflet_batch(dpp, buf_arr, &bh_cnt, SKMEM_NOSLEEP, large_buffer);
  if (__improbable(bh_cnt == 0)) {
    SK_ERR("failed to alloc %u buflets (err %d), use slow path", bh_cnt_tmp,
           err);
  }
  ...
  KPKTQ_FOREACH_SAFE(pkt, rx_pkts, tpkt) {
    ...
    agg_ok = false;
    /* supports TCP only */
    uint32_t thlen = (pkt->pkt_flow_ip_hlen + pkt->pkt_flow_tcp_hlen);
    ...
    if (__improbable(err != 0)) {
      ...
      __RX_AGG_CHAN_DROP_SOURCE_PACKET(pkt, DROP_REASON_FSW_FLOW_TRACK_ERR,
                                       DROPTAP_FLAG_DIR_IN);
      continue;
    }
    ...
    if (prev_csum_ok && sbuf) {
      ...
      if (agg_ok &&
          sbuf->buf_dlim - sbuf->buf_doff - sbuf->buf_dlen >= plen - thlen) {
        /*
         * No need for a new packet, just
         * append to curr_m.
         */
        csum_ok = copy_pkt_csum_packed(pkt, plen, NULL, is_ipv4, NULL, sbuf,
                                       &data_csum, NULL);
    ...
    if (__improbable(bh_cnt_tmp > MAX_BUFLET_COUNT)) {
      ...
      SK_ERR("packet too big: bufcnt %d len %d", bh_cnt_tmp, plen);
      __RX_AGG_CHAN_DROP_SOURCE_PACKET(pkt, DROP_REASON_FSW_GSO_NOMEM_PKT,
                                       DROPTAP_FLAG_DIR_IN);
      continue;
    }
    if (bh_cnt < bh_cnt_tmp) {
      ...
      err = pp_alloc_buflet_batch(dpp, &buf_arr[bh_cnt], &tmp, SKMEM_NOSLEEP,
                                  large_buffer);
      bh_cnt += tmp;
    ...
    if (agg_ok) {
      ...
      csum_ok = copy_pkt_csum_packed(pkt, plen, &dbuf_array, is_ipv4, NULL,
                                     sbuf, &data_csum, &added);
      ...
      if (__improbable(!csum_ok)) {
        ...
        for (j = 0; j < dbuf_array.dba_num_dbufs; j++) {
          VERIFY(kern_buflet_set_data_length(dbuf_array.dba_buflet[j], 0) == 0);
        }
        goto non_agg;
      }
    ...
    non_agg:
      ASSERT(dbuf_array.dba_num_dbufs != 0);
      csum_ok = copy_pkt_csum(pkt, plen, &dbuf_array, &data_csum, is_ipv4);
      if (__improbable(!csum_ok)) {
        STATS_INC(fsws, FSW_STATS_RX_AGG_BAD_CSUM);
    ...
    if (agg_ok) {
      ...
      _append_dbuf_array_to_kpkt(sph, sbuf, &dbuf_array, &sbuf);
    } else {
      /* Finalize the current super packet */
      if (sph != 0) {
        finalize_super_packet(&spkt, &sph, &fa, &largest_spkt, &spkts, bufcnt);
      }
      ...
      spkt->pkt_transport_protocol = fe->fe_transport_protocol;
  ...
  while (bh_cnt > 0) {
    ...
    pp_free_buflet(dpp, __unsafe_forge_single(kern_buflet_t, buf_arr[iter]));
    buf_arr[iter] = 0;
    bh_cnt--;
    iter++;
  }
...
/* streamline a smbuf */
static bool _finalize_smbuf(struct mbuf *smbuf) {
...
SK_NO_INLINE_ATTRIBUTE
static void flow_rx_agg_host(struct nx_flowswitch *fsw, struct flow_entry *fe,
                             struct pktq *rx_pkts, struct mbufq *host_mq,
                             uint32_t rx_bytes, bool is_mbuf) {
#define __RX_AGG_HOST_DROP_SOURCE_PACKET(_pkt, _reason, _flags)                \
  do {                                                                         \
    drop_packets++;                                                            \
  ...
  if (__probable(!is_mbuf)) {
    ...
    try_again:
      if (rx_bytes != 0) {
        mhead_cnt = estimate_buf_cnt(fe, rx_bytes, KPKTQ_LEN(rx_pkts), MCLBYTES,
                                     mhead_bufsize);
      } else {
        /* No payload, thus it's all small-sized ACKs/... */
        mhead_bufsize = MHLEN;
      ...
      mhead = m_allocpacket_internal(&mhead_cnt, mhead_bufsize, &num_segs,
                                     M_NOWAIT, 1, 0);
  ...
  KPKTQ_FOREACH_SAFE(pkt, rx_pkts, tpkt) {
    ...
    if (__improbable(is_mbuf)) {
      ...
      if ((uint32_t)m->m_len < (l2len + thlen)) {
        ...
        if (m == NULL) {
          STATS_INC(fsws, FSW_STATS_RX_DROP_NOMEM_BUF);
          SK_ERR("mbuf pullup failed (err %d)", err);
          __RX_AGG_HOST_DROP_SOURCE_PACKET(pkt, DROP_REASON_FSW_GSO_NOMEM_MBUF,
                                           DROPTAP_FLAG_DIR_IN);
          continue;
        }
      ...
      if (prev_csum_ok && curr_m) {
        ...
        if (agg_ok && M_TRAILINGSPACE(curr_m) >= plen - thlen) {
          /*
           * No need for a new mbuf,
           * just append to curr_m.
           */
          csum_ok = copy_pkt_csum_packed(pkt, plen, NULL, is_ipv4, curr_m, NULL,
                                         &data_csum, NULL);
      ...
      if (__improbable(m == NULL || tot_len > mhead_bufsize)) {
        ...
        err = mbuf_allocpacket(MBUF_DONTWAIT, tot_len, &num_segs, &m);
        if (err != 0) {
          STATS_INC(fsws, FSW_STATS_RX_DROP_NOMEM_BUF);
          SK_ERR("mbuf alloc failed (err %d), "
                 "maxchunks %d, len %d",
                 err, num_segs, tot_len);
          __RX_AGG_HOST_DROP_SOURCE_PACKET(pkt, DROP_REASON_FSW_GSO_NOMEM_MBUF,
                                           DROPTAP_FLAG_DIR_IN);
          continue;
        }
      ...
      if (agg_ok) {
        ...
        csum_ok = copy_pkt_csum_packed(pkt, plen, &dbuf_array, is_ipv4, curr_m,
                                       NULL, &data_csum, &added);
       ...
       */
      m->m_pkthdr.csum_flags |= (pkt->pkt_csum_flags & PACKET_CSUM_RX_FLAGS);
    ...
    if (agg_ok) {
       ...
       */
      if (__improbable(smbuf->m_len < ((m_mtod_current(smbuf) -
  ...
  if (smbufs > 0) {
     ...
     */
    if (smbuf_finalized < smbufs && _finalize_smbuf(smbuf)) {
      FSW_STATS_INC(FSW_STATS_RX_WASTED_16KMBUF);
    }
  ...
  flow_track_stats(fe, (rcvd_bytes - drop_bytes), (rcvd_packets - drop_packets),
                   (rcvd_ulen != 0), true);

  pp_free_pktq(&disposed_pkts);
}

void flow_rx_agg_tcp(struct nx_flowswitch *fsw, struct flow_entry *fe,
...
#pragma unused(flags)
  ...
  if (__improbable((flags & FLOW_PROC_FLAG_FRAGMENTS) != 0)) {
    dp_flow_rx_process(fsw, fe, rx_pkts, rx_bytes, host_mq,
                       FLOW_PROC_FLAG_FRAGMENTS);
    return;
  }
  ...
  if (fe->fe_nx_port == FSW_VP_HOST) {
    ...
    if (sk_fsw_rx_agg_tcp_host != SK_FSW_RX_AGG_TCP_HOST_AUTO) {
      do_rx_agg = (sk_fsw_rx_agg_tcp_host == SK_FSW_RX_AGG_TCP_HOST_ON);
    } else {
      do_rx_agg = !dlil_has_ip_filter() && !dlil_has_if_filter(fsw->fsw_ifp);
    }
