...
const struct flow_key fk_mask_2tuple __sk_aligned(16) = {
...
const struct flow_key fk_mask_3tuple __sk_aligned(16) = {
...
const struct flow_key fk_mask_4tuple __sk_aligned(16) = {
...
const struct flow_key fk_mask_5tuple __sk_aligned(16) = {
...
const struct flow_key fk_mask_ipflow1 __sk_aligned(16) = {
...
const struct flow_key fk_mask_ipflow2 __sk_aligned(16) = {
...
const struct flow_key fk_mask_ipflow3 __sk_aligned(16) = {
...
struct flow_owner *flow_owner_find_by_pid(struct flow_owner_bucket *fob,
                                          pid_t pid, void *context,
                                          bool low_latency) {
  struct flow_owner find = {
      .fo_context = context, .fo_pid = pid, .fo_low_latency = low_latency};
...
struct flow_entry *flow_entry_find_by_uuid(struct flow_owner *fo, uuid_t uuid) {
...
static uint32_t flow_entry_calc_flowid(struct flow_entry *fe) {
  uint32_t flowid;
...
static bool flow_entry_add_child(struct flow_entry *parent_fe,
...
static void flow_entry_remove_all_children(struct flow_entry *parent_fe,
...
static void flow_entry_set_demux_patterns(struct flow_entry *fe,
                                          struct nx_flow_req *req) {
  ASSERT(fe->fe_flags & FLOWENTF_CHILD);
  ASSERT(req->nfr_flow_demux_count > 0);

  fe->fe_demux_patterns = sk_alloc_type_array(
      struct kern_flow_demux_pattern, req->nfr_flow_demux_count,
  ...
  for (int i = 0; i < req->nfr_flow_demux_count; i++) {
    bcopy(&req->nfr_flow_demux_patterns[i],
          &fe->fe_demux_patterns[i].fdp_demux_pattern,
...
convert_flowkey_to_inet_td(struct flow_key *key,
  ...
  if ((key->fk_mask & FKMASK_SRC) != 0) {
    if (td->inet_ipver == IPVERSION) {
      bcopy(&key->fk_src4, &td->inet_laddr.iia_v4addr, sizeof(key->fk_src4));
    } else {
      bcopy(&key->fk_src6, &td->inet_laddr, sizeof(key->fk_src6));
    }
    td->inet_mask |= IFNET_TRAFFIC_DESCRIPTOR_INET_LADDR;
  }
  if ((key->fk_mask & FKMASK_DST) != 0) {
    if (td->inet_ipver == IPVERSION) {
      bcopy(&key->fk_dst4, &td->inet_raddr.iia_v4addr, sizeof(key->fk_dst4));
    } else {
      bcopy(&key->fk_dst6, &td->inet_raddr, sizeof(key->fk_dst6));
    }
...
void flow_qset_select_dynamic(struct nx_flowswitch *fsw, struct flow_entry *fe,
  ...
  if (ifp->if_eth_traffic_rule_count > 0) {
    ...
    struct rtentry *rt =
        (fr->fr_flags & FLOWRTF_GATEWAY) ? fr->fr_rt_gw : fr->fr_rt_dst;
    if (!rt) {
      return;
    }
    ...
    if (err != 0) {
      DTRACE_SKYWALK3(qset__id__not__found, struct nx_flowswitch *, fsw,
                      struct flow_entry *, fe,
                      struct ifnet_traffic_descriptor_inet *, &td);
      return;
    }
  } else {
    DTRACE_SKYWALK2(no__rules, struct nx_flowswitch *, fsw, struct flow_entry *,
                    fe);
    return;
  }

  DTRACE_SKYWALK4(
      qset__id__found, struct nx_flowswitch *, fsw, struct flow_entry *, fe,
...
struct flow_entry *flow_entry_alloc(struct flow_owner *fo,
  ...
  *perr = 0;
  ...
  if (fe != NULL) {
    if ((fe->fe_flags & FLOWENTF_PARENT) &&
      ...
      SK_ERR("entry \"%s\" already exists", fe2str(fe, dbgbuf, sizeof(dbgbuf)));
      /* don't return it */
      flow_entry_release(&fe);
      err = EEXIST;
      goto done;
  ...
  if (fe->fe_adv_idx != FLOWADV_IDX_NONE && fo->fo_nx_port_na != NULL) {
    na_flowadv_entry_alloc(fo->fo_nx_port_na, fe->fe_uuid, fe->fe_adv_idx,
                           fe->fe_flowid);
  }
  ...
  *(struct nx_flowswitch **)(uintptr_t)&fe->fe_fsw = fsw;
  ...
  if (req->nfr_epid != -1 && req->nfr_epid != fo->fo_pid) {
    ...
    proc_name(fe->fe_epid, fe->fe_eproc_name, sizeof(fe->fe_eproc_name));
  } else {
    fe->fe_epid = -1;
  }

  (void)snprintf(fe->fe_proc_name, sizeof(fe->fe_proc_name), "%s", fo->fo_name);
  ...
  if (parent_fe != NULL) {
    ...
    fe->fe_demux_pkt_data = sk_alloc_data(
        FLOW_DEMUX_MAX_LEN, Z_WAITOK | Z_NOFAIL, skmem_tag_flow_demux);
 ...
 *
 * This function provides a high-level interface for configuring RX flow
 * steering rules based on flow entry characteristics. It converts the flow key
 * to a traffic descriptor and configures the underlying netif for hardware
 * steering.
 *
 * Parameters:
 ...
 */
int flow_entry_add_rx_steering_rule(struct nx_flowswitch *fsw,
                                    struct flow_entry *fe) {
  struct ifnet_traffic_descriptor_inet td;
  ...
  SK_DF(SK_VERB_NETIF,
        "Adding RX flow steering rule: fsw=%p, fe=%p, flow_id=%u", SK_KVA(fsw),
        SK_KVA(fe), fe->fe_flowid);

  /* Configure the RX flow steering rule */
  err = nx_netif_configure_rx_flow_steering(
      nx, fe->fe_flowid, (struct ifnet_traffic_descriptor_common *)&td,
  ...
  if (__improbable(err != 0)) {
    ...
    SK_DF(SK_VERB_NETIF, "Successfully added RX flow steering rule: flow_id=%u",
          fe->fe_flowid);
...
void flow_entry_rx_steering_rule_cleanup(struct nx_flowswitch *fsw,
                                         struct flow_entry *fe) {
  ...
  if (fsw->fsw_dev_ch != NULL) {
    nx = fsw->fsw_dev_ch->ch_na->na_nx;
    err = nx_netif_configure_rx_flow_steering(
        nx, fe->fe_flowid, NULL, RX_FLOW_STEERING_ACTION_REMOVE_AOP);
    if (err != 0) {
...
void flow_entry_teardown(struct flow_owner *fo, struct flow_entry *fe) {
#if SK_LOG
  char dbgbuf[FLOWENTRY_DBGBUF_SIZE];
  SK_DF(SK_VERB_FLOW,
        "fe \"%s\" [fo %p] "
        "non_via %d withdrawn %d",
        fe2str(fe, dbgbuf, sizeof(dbgbuf)), SK_KVA(fo), fe->fe_want_nonviable,
        fe->fe_want_withdraw);
#endif /* SK_LOG */
  ...
  if (!(fe->fe_flags & FLOWENTF_TORN_DOWN)) {
    ...
    if (fe->fe_adv_idx != FLOWADV_IDX_NONE) {
      if (fo->fo_nx_port_na != NULL) {
        na_flowadv_entry_free(fo->fo_nx_port_na, fe->fe_uuid, fe->fe_adv_idx,
                              fe->fe_flowid);
      }
...
void flow_entry_destroy(struct flow_owner *fo, struct flow_entry *fe,
  ...
  if (fe->fe_transport_protocol == IPPROTO_QUIC) {
    if (!nolinger && close_params != NULL) {
       ...
       * forge this for now.
       */
      uint8_t *quic_close_params = __unsafe_forge_bidi_indexable(
          uint8_t *, close_params, QUIC_STATELESS_RESET_TOKEN_SIZE);
...
uint32_t flow_entry_refcnt(struct flow_entry *fe) {
  return os_ref_get_count(&fe->fe_refcnt);
}

void flow_entry_retain(struct flow_entry *fe) { os_ref_retain(&fe->fe_refcnt); }

void flow_entry_release(struct flow_entry **pfe) {
  ...
  if (__improbable(os_ref_release(&fe->fe_refcnt) == 0)) {
    ...
    if (fe->fe_demux_patterns != NULL) {
      sk_free_type_array_counted_by(struct kern_flow_demux_pattern,
                                    fe->fe_demux_pattern_count,
                                    fe->fe_demux_patterns);
...
struct flow_entry_dead *flow_entry_dead_alloc(zalloc_flags_t how) {
...
void flow_entry_dead_free(struct flow_entry_dead *fed) {
  SK_DF(SK_VERB_MEM, "fed %p FREE", SK_KVA(fed));
  zfree(sk_fed_zone, fed);
}

static void fe_stats_init(struct flow_entry *fe) {
...
static void fe_stats_update(struct flow_entry *fe) {
  struct sk_stats_flow *sf = &fe->fe_stats->fs_stats;
...
void flow_entry_stats_get(struct flow_entry *fe, struct sk_stats_flow *sf) {
...
struct flow_entry *fe_alloc(boolean_t can_block) {
  struct flow_entry *fe;

  static_assert((offsetof(struct flow_entry, fe_key) % 16) == 0);

  fe = skmem_cache_alloc(sk_fe_cache, can_block ? SKMEM_SLEEP : SKMEM_NOSLEEP);
  if (fe == NULL) {
    return NULL;
  }
...
static void fe_free(struct flow_entry *fe) {
  ASSERT(fe->fe_flags & FLOWENTF_TORN_DOWN);
  ASSERT(fe->fe_flags & FLOWENTF_DESTROYED);
  ASSERT(!(fe->fe_flags & FLOWENTF_LINGERING));
...
static __inline__ int fe_id_cmp(const struct flow_entry *a,
                                const struct flow_entry *b) {
  return uuid_compare(a->fe_uuid, b->fe_uuid);
}

#if SK_LOG
SK_NO_INLINE_ATTRIBUTE
char *fk2str(const struct flow_key *fk, char *__counted_by(dsz) dst,
             size_t dsz) {
  int af;
...
SK_NO_INLINE_ATTRIBUTE
char *fe2str(const struct flow_entry *fe, char *__counted_by(dsz) dst,
             size_t dsz) {
  ...
  (void)sk_snprintf(dst, dsz,
                    "%p proc %s(%d)%s nx_port %d flow_uuid %s"
                    " flags 0x%b %s tp_proto=0x%02u",
                    SK_KVA(fe), fe->fe_proc_name, fe->fe_pid, fe->fe_eproc_name,
                    (int)fe->fe_nx_port, sk_uuid_unparse(fe->fe_uuid, uuidstr),
                    fe->fe_flags, FLOWENTF_BITS, keybuf,
