...
struct flow_owner_bucket *__sized_by(*tot_sz)
  ...
  *tot_sz = fob_tot_sz = fob_cnt * (*fob_sz);
  fob = sk_alloc_type_hash(KT_SK_FOB, fob_tot_sz, Z_WAITOK,
  ...
  SK_DF(SK_VERB_MEM,
        "fob %p fob_cnt %zu fob_sz %zu "
        "(total %zu bytes) ALLOC",
        SK_KVA(fob), fob_cnt, *fob_sz, fob_tot_sz);

  return fob;
}

void flow_owner_buckets_free(struct flow_owner_bucket *fob, size_t tot_sz) {
  SK_DF(SK_VERB_MEM, "fob %p FREE", SK_KVA(fob));
  sk_free_type_hash(KT_SK_FOB, tot_sz, fob);
}

void flow_owner_bucket_init(struct flow_owner_bucket *fob) {
#if !KASAN_CLASSIC
  ASSERT(IS_P2ALIGNED(fob, skmem_cpu_cache_line_size()));
#endif /* !KASAN_CLASSIC */
  lck_mtx_init(&fob->fob_lock, &flow_owner_lock_group, &flow_owner_lock_attr);
  RB_INIT(&fob->fob_owner_head);
}

void flow_owner_bucket_destroy(struct flow_owner_bucket *fob) {
  /*
  ...
  while (!RB_EMPTY(&fob->fob_owner_head)) {
    ...
    (void)msleep(&fob->fob_dtor_waiters, &fob->fob_lock, (PZERO - 1),
                 __FUNCTION__, NULL);
  }
  while (fob->fob_busy_flags & FOBF_CLOSE_BUSY) {
    ...
    (void)msleep(&fob->fob_close_waiters, &fob->fob_lock, (PZERO - 1),
                 __FUNCTION__, NULL);
  }
...
static uint32_t flow_owner_bucket_purge_common(struct flow_owner_bucket *fob,
  ...
  RB_FOREACH_SAFE(fo, flow_owner_tree, &fob->fob_owner_head, tfo) {
    ...
    if (!if_idle || nx_port == NEXUS_PORT_ANY) {
      RB_FOREACH_SAFE(fe, flow_entry_id_tree, &fo->fo_flow_entry_id_head, tfe) {
    ...
    ASSERT(nx_port != NEXUS_PORT_ANY || RB_EMPTY(&fo->fo_flow_entry_id_head));
...
void flow_owner_bucket_purge_all(struct flow_owner_bucket *fob) {
  (void)flow_owner_bucket_purge_common(fob, NEXUS_PORT_ANY, TRUE);
}

static uint32_t flow_owner_bucket_activate_nx_port_common(
    struct flow_owner_bucket *fob, nexus_port_t nx_port,
...
uint32_t flow_owner_activate_nexus_port(struct flow_mgr *fm,
                                        boolean_t pid_bound, pid_t pid,
                                        nexus_port_t nx_port,
                                        struct nexus_adapter *nx_port_na,
  ...
  if (pid_bound) {
    ...
    fo_cnt = flow_owner_bucket_activate_nx_port_common(fob, nx_port, nx_port_na,
                                                       mode);
  } else {
    uint32_t i;
    /*
...
static void flow_owner_bucket_attach_common(struct flow_owner_bucket *fob,
                                            nexus_port_t nx_port) {
  struct flow_owner *fo;
...
void flow_owner_attach_nexus_port(struct flow_mgr *fm, boolean_t pid_bound,
...
uint32_t flow_owner_detach_nexus_port(struct flow_mgr *fm, boolean_t pid_bound,
                                      pid_t pid, nexus_port_t nx_port,
                                      boolean_t if_idle) {
  struct flow_owner_bucket *fob;
  uint32_t purged = 0;
...
#define FO_BMASK64(_beg, _end)                                                 \
  ((((uint64_t)0xffffffffffffffff) >> (63 - (_end))) & ~((1ULL << (_beg)) - 1))

struct flow_owner *flow_owner_alloc(struct flow_owner_bucket *fob,
                                    struct proc *p, nexus_port_t nx_port,
                                    bool nx_port_pid_bound, bool flowadv,
  ...
  if (fo != NULL) {
    if (flowadv) {
      ...
      for (i = 0; i < sk_fadv_nchunks; i++) {
        ...
        if (i == (sk_fadv_nchunks - 1)) {
          end = ((sk_max_flows - 1) % FO_FLOWADV_CHUNK);
        }
    ...
    *(struct nx_flowswitch **)(uintptr_t)&fo->fo_fsw = fsw;
    ...
    SK_DF(SK_VERB_FLOW,
          "%s(%d) fob %p added fo %p "
          "nx_port %d nx_port_pid_bound %d ll %d nx_port_na %p",
          fo->fo_name, fo->fo_pid, SK_KVA(fob), SK_KVA(fo), (int)nx_port,
          nx_port_pid_bound, fo->fo_low_latency, SK_KVA(nx_port_na));
  }

  return fo;
}

void flow_owner_free(struct flow_owner_bucket *fob, struct flow_owner *fo) {
  FOB_LOCK_ASSERT_HELD(fob);
  ...
  *(struct flow_owner_bucket **)(uintptr_t)&fo->fo_bucket = NULL;
  ...
  SK_DF(SK_VERB_FLOW, "%s(%d) fob %p removed fo %p nx_port %d", fo->fo_name,
        fo->fo_pid, SK_KVA(fob), SK_KVA(fo), (int)fo->fo_nx_port);

  fo_free(fo);
}

int flow_owner_flowadv_index_alloc(struct flow_owner *fo,
                                   flowadv_idx_t *fadv_idx) {
  ...
  nchunks = P2ROUNDUP(fo->fo_flowadv_max, FO_FLOWADV_CHUNK) / FO_FLOWADV_CHUNK;
  ...
  if (idx == FLOWADV_IDX_NONE) {
    SK_ERR("%s(%d) flow advisory table full: num %u max %u", fo->fo_name,
           fo->fo_pid, fo->fo_num_flowadv, fo->fo_flowadv_max);
...
void flow_owner_flowadv_index_free(struct flow_owner *fo,
                                   flowadv_idx_t fadv_idx) {
...
int flow_owner_destroy_entry(struct flow_owner *fo, uuid_t uuid, bool nolinger,
                             void *close_params) {
...
static inline int fo_cmp(const struct flow_owner *a,
                         const struct flow_owner *b) {
...
static struct flow_owner *fo_alloc(boolean_t can_block) {
  struct flow_owner *fo;

  fo = skmem_cache_alloc(sk_fo_cache, can_block ? SKMEM_SLEEP : SKMEM_NOSLEEP);
  if (fo == NULL) {
    return NULL;
  }
...
static void fo_free(struct flow_owner *fo) {
