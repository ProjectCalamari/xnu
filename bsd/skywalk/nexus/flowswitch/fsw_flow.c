...
struct flow_owner *fsw_flow_add(struct nx_flowswitch *fsw,
  ...
  *error = 0;
  ...
  while (fob->fob_busy_flags & (FOBF_OPEN_BUSY | FOBF_CLOSE_BUSY)) {
    ...
    if ((*error = msleep(&fob->fob_open_waiters, &fob->fob_lock,
      ...
      SK_ERR("%s(%d) binding for uuid %s was interrupted", sk_proc_name(p), pid,
  ...
  if (__improbable((fob->fob_busy_flags & FOBF_DEAD) != 0)) {
    SK_ERR("%s(%d) binding for flow_uuid %s aborted due to "
           "dead owner",
           sk_proc_name(p), pid, sk_uuid_unparse(req.nfr_flow_uuid, uuidstr));
    *error = ENXIO;
    goto done;
  }
  ...
  do {
    ...
    if (fo == NULL && nx_port == NEXUS_PORT_ANY) {
       ...
       */
      if ((*error = NX_DOM(nx)->nxdom_bind_port(nx, &nx_port, &nxb, NULL)) !=
          0) {
      ...
      SK_DF(SK_VERB_FLOW,
            "%s(%d) flow_uuid %s associated with "
            "ephemeral nx_port %d",
            sk_proc_name(p), pid, sk_uuid_unparse(req.nfr_flow_uuid, uuidstr),
            (int)nx_port);
      ...
      if (nx_port != NEXUS_PORT_ANY && nx_port != fo->fo_nx_port) {
        *error = EINVAL;
        break;
      }
    ...
    ASSERT(fo == NULL);
    fo =
        flow_owner_alloc(fob, p, nx_port, nx_port_pid_bound, (max_flowadv != 0),
                         fsw, NULL, fo_context, low_latency);
    if (fo == NULL) {
      *error = ENOMEM;
      break;
    }
  ...
  *error = flow_mgr_flow_add(nx, fm, fo, fsw->fsw_ifp, &req,
  ...
  if (__improbable(*error != 0)) {
    SK_ERR("%s(%d) failed to add flow_uuid %s (err %d)", sk_proc_name(p), pid,
           sk_uuid_unparse(req.nfr_flow_uuid, uuidstr), *error);
    if (fo != NULL) {
      if (new_mapping) {
        FOB_LOCK_CONVERT(fob);
  ...
  /* Configure RX flow steering if flow was added successfully and AOP offload
   * is requested */
  if (aop_fe != NULL) {
...
int fsw_flow_del(struct nx_flowswitch *fsw, struct nx_flow_req *req,
  ...
  while (fob->fob_busy_flags & (FOBF_OPEN_BUSY | FOBF_CLOSE_BUSY)) {
    ...
    (void)msleep(&fob->fob_close_waiters, &fob->fob_lock, (PZERO - 1) | PSPIN,
                 __FUNCTION__, NULL);
  }
   ...
   */
  error = flow_owner_destroy_entry(fo, req->nfr_flow_uuid, nolinger, params);
   ...
   * this port available.
   */
  if (RB_EMPTY(&fo->fo_flow_entry_id_head) && fo->fo_nx_port_destroyed &&
      fo->fo_nx_port_pid_bound) {
  ...
  if (__improbable((sk_verbose & SK_VERB_FLOW) != 0)) {
    uuid_string_t uuidstr;
    if (fo != NULL) {
      SK_DF(SK_VERB_FLOW, "%s(%d) flow_uuid %s (err %d)", fo->fo_name,
            fo->fo_pid, sk_uuid_unparse(req->nfr_flow_uuid, uuidstr), error);
    } else {
...
int fsw_flow_config(struct nx_flowswitch *fsw, struct nx_flow_req *req) {
  struct flow_mgr *fm = fsw->fsw_flow_mgr;
...
static void fsw_flow_route_ctor(void *arg, struct flow_route *fr) {
...
static int fsw_flow_route_resolve(void *arg, struct flow_route *fr,
