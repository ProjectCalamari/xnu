...
static packet_svc_class_t fsw_netagent_tc2sc(uint32_t tc, uint32_t *scp) {
  uint32_t sc;
  int ret = 0;
...
static int fsw_netagent_flow_add(struct nx_flowswitch *fsw, uuid_t flow_uuid,
  ...
  if (fsw_netagent_tc2sc(cparams->traffic_class, &req.nfr_svc_class) != 0) {
    error = EINVAL;
    goto done;
  }
  ...
  if (cparams->is_demuxable_parent) {
    ...
    if (cparams->demux_pattern_count > 0) {
      ...
      static_assert(sizeof(struct necp_demux_pattern) ==
  ...
  assign_message = necp_create_nexus_assign_message(
      fsw->fsw_nx->nx_uuid, req.nfr_nx_port, fo->fo_key, sizeof(fo->fo_key),
  ...
  if (__improbable((sk_verbose & SK_VERB_FLOW) != 0)) {
    ...
    SK_DF(SK_VERB_FLOW,
          "pid %d connect flow_uuid %s nx_port %d "
          "(err %d)",
          pid, sk_uuid_unparse(flow_uuid, uuidstr), (int)req.nfr_nx_port,
          error);
  }
#endif /* SK_LOG */
  if (error != 0) {
...
static int fsw_netagent_flow_del(struct nx_flowswitch *fsw, uuid_t flow_uuid,
                                 pid_t pid, bool nolinger, void *context,
                                 void *params) {
  struct nx_flow_req req;
  int error;
  ...
  if (__improbable((sk_verbose & SK_VERB_FLOW) != 0)) {
    ...
    SK_DF(SK_VERB_FLOW, "pid %d flow_uuid %s (err %d)", pid,
          sk_uuid_unparse(flow_uuid, uuidstr), error);
  }
#endif /* SK_LOG */
...
static int fsw_netagent_event(u_int8_t event, uuid_t flow_uuid, pid_t pid,
...
int fsw_netagent_register(struct nx_flowswitch *fsw, struct ifnet *ifp) {
  ...
  if (ifnet_needs_fsw_ip_netagent(ifp)) {
    ...
    agent.agent.netagent_flags |=
        (NETAGENT_FLAG_CUSTOM_IP_NEXUS | NETAGENT_FLAG_NEXUS_LISTENER);
  }
  ...
  if (ifp->if_bridge != NULL) {
    /* see rdar://107076453 */
    SK_ERR("%s is bridged, not adding netagent", if_name(ifp));
    goto done;
  }
...
void fsw_netagent_unregister(struct nx_flowswitch *fsw, struct ifnet *ifp) {
