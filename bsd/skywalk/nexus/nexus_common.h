...
__nexus_attr_from_params(nexus_attr_t nxa, const struct nxprov_params *p) {
  ...
  nxa->nxa_checksum_offload =
      (p->nxp_capabilities & NXPCAP_CHECKSUM_PARTIAL) ? 1 : 0;
  nxa->nxa_user_packet_pool =
      (p->nxp_capabilities & NXPCAP_USER_PACKET_POOL) ? 1 : 0;
...
__attribute__((always_inline)) static inline int
__nexus_provider_reg_prepare(struct nxprov_reg *reg,
                             const uint8_t *__null_terminated name,
  ...
  if (nxa != NULL) {
    ...
    if (nxa->nxa_requested & NXA_REQ_QMAP) {
      ...
      if ((nxa->nxa_qmap == NEXUS_QMAP_TYPE_WMM) &&
          (reg->nxpreg_params.nxp_tx_rings != NEXUS_NUM_WMM_QUEUES)) {
        err = EINVAL;
        goto done;
      }
    ...
    if (nxa->nxa_requested & NXA_REQ_MAX_FRAGS) {
      if ((type != NEXUS_TYPE_NET_IF) && (type != NEXUS_TYPE_FLOW_SWITCH)) {
        err = EINVAL;
        goto done;
      }
    ...
    if (nxa->nxa_requested & NXA_REQ_REJECT_ON_CLOSE) {
      ...
      p->nxp_reject_on_close = (nxa->nxa_reject_on_close != 0);
    }
    if (nxa->nxa_requested & NXA_REQ_LARGE_BUF_SIZE) {
      reg->nxpreg_requested |= NXPREQ_LARGE_BUF_SIZE;
      p->nxp_large_buf_size = (uint32_t)nxa->nxa_large_buf_size;
    }
  }
done:
  return err;
...
__nexus_bind_req_prepare(struct nx_bind_req *nbr, const uuid_t nx_uuid,
                         const nexus_port_t port, const pid_t pid,
                         const uuid_t exec_uuid, const void *key,
...
__nexus_config_req_prepare(struct nx_cfg_req *ncr, const uuid_t nx_uuid,
                           const nxcfg_cmd_t cmd, const void *arg,
