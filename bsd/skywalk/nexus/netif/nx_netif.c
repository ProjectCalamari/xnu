...
static int
nx_netif_prov_params_adjust(const struct kern_nexus_domain_provider *,
                            const struct nxprov_params *,
...
static errno_t
nx_netif_interface_advisory_notify(void *,
                                   const struct ifnet_interface_advisory *);

struct nxdom nx_netif_dom_s = {
    .nxdom_prov_head = STAILQ_HEAD_INITIALIZER(nx_netif_dom_s.nxdom_prov_head),
...
SYSCTL_UINT(_kern_skywalk_netif, OID_AUTO, netif_queue_transfer_decay,
            CTLFLAG_RW | CTLFLAG_LOCKED, &nq_transfer_decay, NQ_TRANSFER_DECAY,
            "ilog2 of EWMA decay rate of netif queue transfers");
SYSCTL_UINT(_kern_skywalk_netif, OID_AUTO, netif_queue_stat_accumulate_interval,
            ...
            NQ_ACCUMULATE_INTERVAL,
            "accumulation interval for netif queue stats");
#endif /* !DEVELOPMENT && !DEBUG */

SYSCTL_UINT(_kern_skywalk_netif, OID_AUTO, netif_queue_stat_enable,
            CTLFLAG_RW | CTLFLAG_LOCKED, &sk_netif_queue_stat_enable, 0,
            "enable/disable stats collection for netif queue");
...
static void nx_netif_dom_init(struct nxdom *nxdom) {
  SK_LOCK_ASSERT_HELD();
...
static void nx_netif_dom_terminate(struct nxdom *nxdom) {
  ...
  STAILQ_FOREACH_SAFE(nxdom_prov, &nxdom->nxdom_prov_head, nxdom_prov_link,
                      tnxdp) {
    (void)nxdom_prov_del(nxdom_prov);
  }
}

static void nx_netif_dom_fini(struct nxdom *nxdom) {
#pragma unused(nxdom)
}

int nx_netif_prov_init(struct kern_nexus_domain_provider *nxdom_prov) {
#pragma unused(nxdom_prov)
  SK_D("initializing %s", nxdom_prov->nxdom_prov_name);
  return 0;
}

static int nx_netif_na_notify_drop(struct __kern_channel_ring *kring,
                                   struct proc *p, uint32_t flags) {
#pragma unused(kring, p, flags)
  return ENXIO;
}

int nx_netif_prov_nx_stop(struct kern_nexus *nx) {
  uint32_t r;
  ...
  for (r = 0; r < na_get_nrings(na, NR_TX); r++) {
    while (!os_atomic_cmpxchg(
        (void *volatile *)&na->na_tx_rings[r].ckr_na_notify,
  ...
  for (r = 0; r < na_get_nrings(na, NR_RX); r++) {
    while (!os_atomic_cmpxchg(
        (void *volatile *)&na->na_rx_rings[r].ckr_na_notify,
...
nx_netif_compat_adjust_ring_size(struct nxprov_adjusted_params *adj,
  ...
  if (IFNET_IS_CELLULAR(ifp) && (ifp->if_unit != 0)) {
    ...
    *(adj->adj_tx_slots) = sk_netif_compat_aux_cell_tx_ring_sz;
    ...
    if (ifname[0] == 'a' && ifname[1] == 'p' && ifname[2] == '\0') {
      /* Wi-Fi Access Point */
...
nx_netif_prov_params_adjust(const struct kern_nexus_domain_provider *nxdom_prov,
                            const struct nxprov_params *nxp,
   ...
   */
  if (strbufcmp(nxdom_prov->nxdom_prov_name, NEXUS_PROVIDER_NET_IF_COMPAT) ==
      0) {
    /*
    ...
    *(adj->adj_rx_rings) += 1;
    ...
    if ((*(adj->adj_buf_size) < PKT_MAX_PROTO_HEADER_SIZE)) {
      SK_ERR("buf size too small, min (%d)", PKT_MAX_PROTO_HEADER_SIZE);
      return EINVAL;
    }
    static_assert(sizeof(struct __kern_netif_intf_advisory) ==
                  NX_INTF_ADV_SIZE);
...
int nx_netif_prov_params(struct kern_nexus_domain_provider *nxdom_prov,
  ...
  return nxprov_params_adjust(nxdom_prov, req, nxp0, nxp, srp, nxdom, nxdom,
                              nxdom, pp_region_config_flags,
                              nx_netif_prov_params_adjust);
}

int nx_netif_prov_mem_new(struct kern_nexus_domain_provider *nxdom_prov,
...
#pragma unused(nxdom_prov)
  ...
  SK_DF(SK_VERB_NETIF, "nx %p (\"%s\":\"%s\") na \"%s\" (%p)", SK_KVA(nx),
  ...
  na->na_arena = skmem_arena_create_for_nexus(
      na, NX_PROV(nx)->nxprov_region_params, &nx->nx_tx_pp, &nx->nx_rx_pp,
      pp_truncated_buf, kernel_only, &nx->nx_adv, &err);
  ASSERT(na->na_arena != NULL || err != 0);
  ASSERT(nx->nx_tx_pp == NULL ||
         (nx->nx_tx_pp->pp_md_type == NX_DOM(nx)->nxdom_md_type &&
          nx->nx_tx_pp->pp_md_subtype == NX_DOM(nx)->nxdom_md_subtype));

  return err;
}

SK_NO_INLINE_ATTRIBUTE
static int nx_netif_get_llink_info(struct sockopt *sopt,
  ...
  if (nlir->nlir_version != NETIF_LLINK_INFO_VERSION) {
    SK_ERR("nlir version mismatch: %d != %d", nlir->nlir_version,
           NETIF_LLINK_INFO_VERSION);
    err = ENOTSUP;
    goto done;
  }
...
int nx_netif_prov_config(struct kern_nexus_domain_provider *nxdom_prov,
                         struct kern_nexus *nx, struct nx_cfg_req *ncr,
                         int sopt_dir, struct proc *p, kauth_cred_t cred) {
#pragma unused(nxdom_prov)
  struct sockopt sopt;
  int err = 0;
  ...
  if ((err = skywalk_priv_check_cred(p, cred, PRIV_SKYWALK_REGISTER_NET_IF)) !=
      0) {
    goto done;
  }
  ...
  case NXCFG_CMD_FLOW_DEL: {
    static_assert(offsetof(struct nx_flow_req, _nfr_kernel_field_end) ==
  ...
  SK_DF(err ? SK_VERB_ERROR : SK_VERB_NETIF, "nexus %p (%s) cmd %d err %d",
        SK_KVA(nx), NX_DOM_PROV(nx)->nxdom_prov_name, ncr->nc_cmd, err);
  return err;
}

void nx_netif_prov_fini(struct kern_nexus_domain_provider *nxdom_prov) {
#pragma unused(nxdom_prov)
  SK_D("destroying %s", nxdom_prov->nxdom_prov_name);
}

int nx_netif_prov_nx_ctor(struct kern_nexus *nx) {
  struct nx_netif *n;
  char name[64];
  ...
  n->nif_nx = nx;
  SK_D("create new netif %p for nexus %p", SK_KVA(NX_NETIF_PRIVATE(nx)),
       SK_KVA(nx));
  return 0;
}

void nx_netif_prov_nx_dtor(struct kern_nexus *nx) {
  struct nx_netif *n = NX_NETIF_PRIVATE(nx);

  SK_LOCK_ASSERT_HELD();

  SK_D("nexus %p (%s) netif %p", SK_KVA(nx), NX_DOM_PROV(nx)->nxdom_prov_name,
       SK_KVA(n));
...
int nx_netif_prov_nx_mem_info(struct kern_nexus *nx, struct kern_pbufpool **tpp,
...
static size_t __netif_mib_get_stats(struct kern_nexus *nx, void *out,
                                    size_t len) {
...
static size_t __netif_mib_get_llinks(struct kern_nexus *nx,
                                     void *__sized_by(len) out, size_t len) {
  struct nx_netif *nif = NX_NETIF_PRIVATE(nx);
...
static size_t __netif_mib_get_queue_stats(struct kern_nexus *nx,
                                          void *__sized_by(len) out,
                                          size_t len) {
  struct nx_netif *nif = NX_NETIF_PRIVATE(nx);
  uint8_t *itr = out;
  size_t actual_space = 0;
  ...
  STAILQ_FOREACH(llink, &nif->nif_llink_list, nll_link) {
    ...
    ASSERT(qset_cnt <= NETIF_LLINK_MAX_QSETS);
    SLIST_FOREACH(qset, &llink->nll_qset_list, nqs_list) {
      int queue_cnt = qset->nqs_num_rx_queues + qset->nqs_num_tx_queues;
      for (uint16_t k = 0; k < queue_cnt; k++) {
        struct netif_qstats_info *nqi = (struct netif_qstats_info *)(void *)itr;
...
size_t nx_netif_prov_nx_mib_get(struct kern_nexus *nx,
...
static int nx_netif_dom_bind_port(struct kern_nexus *nx, nexus_port_t *nx_port,
  ...
  if (__improbable(first == last)) {
    ...
    if (error == 0) {
      error = nx_port_bind_info(nx, port, nxb, info);
      SK_DF(SK_VERB_NETIF, "found port %d, bind err %d", port, error);
    }
  }
  NETIF_WUNLOCK(nif);
  ...
  SK_DF(error ? SK_VERB_ERROR : SK_VERB_NETIF,
        "+++ netif %p nx_port %d, total %u active %u (err %d)", SK_KVA(nif),
        (int)*nx_port, NX_NETIF_MAXPORTS, nx->nx_active_ports, error);

  return error;
}

static int nx_netif_dom_unbind_port(struct kern_nexus *nx,
...
static int nx_netif_dom_connect(struct kern_nexus_domain_provider *nxdom_prov,
                                struct kern_nexus *nx, struct kern_channel *ch,
                                struct chreq *chr, struct nxbind *nxb,
                                struct proc *p) {
#pragma unused(nxdom_prov)
  int err = 0;
  ...
  if (err == 0) {
     ...
     */
    skmem_arena_nexus_sd_set_noidle(skmem_arena_nexus(ch->ch_na->na_arena), 1);
  }
...
#pragma unused(nxdom_prov)
  ...
  SK_D("channel %p -!- nexus %p (%s:\"%s\":%u:%d)", SK_KVA(ch), SK_KVA(nx),
...
static void nx_netif_dom_defunct(struct kern_nexus_domain_provider *nxdom_prov,
                                 struct kern_nexus *nx, struct kern_channel *ch,
                                 struct proc *p) {
...
#pragma unused(nxdom_prov)
  ...
  if (ch->ch_na->na_type == NA_NETIF_VP && ifp != NULL &&
     ...
     */
    DTRACE_SKYWALK2(release__vpna__ifp, struct nexus_adapter *, ch->ch_na,
                    struct ifnet *, ifp);
    ifnet_decr_iorefcnt(ifp);
    ch->ch_na->na_ifp = NULL;
  }
  SK_D("%s(%d): ch %p -/- nx %p (%s:\"%s\":%u:%d)", ch->ch_name, ch->ch_pid,
       SK_KVA(ch), SK_KVA(nx), nxdom_prov->nxdom_prov_name, ch->ch_na->na_name,
       ch->ch_info->cinfo_nx_port, (int)ch->ch_info->cinfo_ch_ring_id);
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_ctl_attach(struct kern_nexus *nx, struct nx_spec_req *nsr,
                               struct proc *p) {
  struct nx_netif *n = NX_NETIF_PRIVATE(nx);
  ...
  if ((compat && SKYWALK_NATIVE(ifp)) || (!compat && !SKYWALK_NATIVE(ifp))) {
...
#if (DEVELOPMENT || DEBUG)
      skoid_create(&n->nif_skoid, SKOID_SNODE(_kern_skywalk_netif),
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_clean(struct nx_netif *nif, boolean_t quiesce_needed) {
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_ctl_detach(struct kern_nexus *nx, struct nx_spec_req *nsr) {
  struct nx_netif *nif = NX_NETIF_PRIVATE(nx);
  int err = 0;
  ...
  if (nsr != NULL && uuid_is_null(nsr->nsr_if_uuid)) {
    err = EINVAL;
             ...
             uuid_compare(nsr->nsr_if_uuid, nif->nif_uuid) != 0) {
    err = ESRCH;
  ...
  if (nsr != NULL) {
    ...
    SK_DF(err ? SK_VERB_ERROR : SK_VERB_NETIF,
          "nexus %p (%s) if_uuid %s flags 0x%x err %d", SK_KVA(nx),
          NX_DOM_PROV(nx)->nxdom_prov_name,
          sk_uuid_unparse(nsr->nsr_if_uuid, ifuuidstr), nsr->nsr_flags, err);
  } else {
    SK_DF(err ? SK_VERB_ERROR : SK_VERB_NETIF, "nexus %p (%s) err %d",
          SK_KVA(nx), NX_DOM_PROV(nx)->nxdom_prov_name, err);
  }
#endif /* SK_LOG */
 ...
 */
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_ctl_flow_check(struct nx_netif *nif, nxcfg_cmd_t cmd,
...
#pragma unused(nif)
  ...
  if (need_check) {
    ...
    error = priv_check_cred(cred, PRIV_NET_PRIVILEGED_SOCKET_DELEGATE, 0);
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_ctl_flow_add(struct nx_netif *nif, struct proc *p,
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_ctl_flow_del(struct nx_netif *nif, struct proc *p,
                                 struct nx_flow_req *req) {
  int err;
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_ctl(struct kern_nexus *nx, nxcfg_cmd_t nc_cmd, void *data,
                        struct proc *p) {
  struct nx_netif *nif = NX_NETIF_PRIVATE(nx);
...
static void nx_netif_llink_notify(struct kern_nexus *nx,
                                  struct netif_llink *llink, uint32_t flags) {
#pragma unused(flags)
  struct netif_qset *qset;
...
static void nx_netif_llink_notify_all(struct kern_nexus *nx, uint32_t flags) {
  struct nx_netif *nif;
  struct netif_llink *llink;
 ...
 * starter thread.
 */
static void nx_netif_doorbell_internal(struct ifnet *ifp, uint32_t flags) {
...
static void nx_netif_doorbell(struct ifnet *ifp) {
 ...
 */
static int nx_netif_na_txsync(struct __kern_channel_ring *kring, struct proc *p,
                              uint32_t flags) {
#pragma unused(p)
  ...
  SK_DF(SK_VERB_NETIF | SK_VERB_SYNC | SK_VERB_TX,
        "%s(%d) kr \"%s\" (%p) krflags 0x%x ring %u flags 0%x", sk_proc_name(p),
        sk_proc_pid(p), kring->ckr_name, SK_KVA(kring), kring->ckr_flags,
        kring->ckr_ring_id, flags);

  if (__improbable(!ifnet_is_fully_attached(ifp))) {
    SK_ERR("kr %p ifp %s (%p), interface not attached", SK_KVA(kring),
           if_name(ifp), SK_KVA(ifp));
    return ENXIO;
  }
  ...
  sync_only = ((flags & NA_SYNCF_SYNC_ONLY) != 0) || !KR_KERNEL_ONLY(kring);
...
static int nx_netif_na_rxsync(struct __kern_channel_ring *kring, struct proc *p,
                              uint32_t flags) {
#pragma unused(p)
  int ret;
 ...
 */
int nx_netif_common_intr(struct __kern_channel_ring *kring, struct proc *p,
                         uint32_t flags, uint32_t *work_done) {
  struct netif_stats *nifs = &NX_NETIF_PRIVATE(KRNA(kring)->na_nx)->nif_stats;
  int (*notify)(struct __kern_channel_ring *kring, struct proc *,
                uint32_t flags);
  int ret;
  ...
  SK_DF(SK_VERB_NETIF | SK_VERB_INTR |
        ...
        "na \"%s\" (%p) kr \"%s\" (%p) krflags 0x%x", KRNA(kring)->na_name,
        SK_KVA(KRNA(kring)), kring->ckr_name, SK_KVA(kring), kring->ckr_flags);
...
static int nx_netif_na_notify_tx(struct __kern_channel_ring *kring,
                                 struct proc *p, uint32_t flags) {
  return nx_netif_mit_tx_intr(kring, p, flags, NULL);
}

static int nx_netif_na_notify_rx(struct __kern_channel_ring *kring,
                                 struct proc *p, uint32_t flags) {
  int ret;
...
static int nx_netif_na_notify_rx_redirect(struct __kern_channel_ring *kring,
                                          struct proc *p, uint32_t flags) {
  struct netif_stats *nifs = &NX_NETIF_PRIVATE(KRNA(kring)->na_nx)->nif_stats;
  uint32_t work_done;
...
void nx_netif_mit_config(struct nexus_netif_adapter *nifna, boolean_t *tx_mit,
                         boolean_t *tx_mit_simple, boolean_t *rx_mit,
                         boolean_t *rx_mit_simple) {
...
static int nx_netif_na_activate(struct nexus_adapter *na,
  ...
  SK_DF(SK_VERB_NETIF, "na \"%s\" (%p) %s [%s]", na->na_name, SK_KVA(na),
        ifp->if_xname, na_activate_mode2str(mode));

  switch (mode) {
  case NA_ACTIVATE_MODE_ON:
    ASSERT(SKYWALK_CAPABLE(ifp));

    nx_netif_mit_config(nifna, &tx_mit, &tx_mit_simple, &rx_mit,
                        &rx_mit_simple);
    ...
    if (tx_mit) {
      ...
      mit_tmp = skn_alloc_type_array(tx_on, struct nx_netif_mit, nrings,
                                     Z_WAITOK, skmem_tag_netif_mit);
      if (mit_tmp == NULL) {
    ...
    if (rx_mit) {
      ...
      mit_tmp = skn_alloc_type_array(rx_on, struct nx_netif_mit, nrings,
                                     Z_WAITOK, skmem_tag_netif_mit);
      if (mit_tmp == NULL) {
        SK_ERR("RX mitigation allocation failed");
        if (nifna->nifna_tx_mit != NULL) {
          skn_free_type_array_counted_by(rx_fail, struct nx_netif_mit,
                                         nifna->nifna_tx_mit_count,
                                         nifna->nifna_tx_mit);
        }
        error = ENOMEM;
    ...
    for (r = 0; r < na_get_nrings(na, NR_TX); r++) {
      na->na_tx_rings[r].ckr_netif_notify = na->na_tx_rings[r].ckr_na_notify;
      na->na_tx_rings[r].ckr_na_notify = nx_netif_na_notify_tx;
      if (nifna->nifna_tx_mit != NULL) {
        nx_netif_mit_init(nif, ifp, &nifna->nifna_tx_mit[r],
    ...
    for (r = 0; r < na_get_nrings(na, NR_RX); r++) {
      na->na_rx_rings[r].ckr_netif_notify = na->na_rx_rings[r].ckr_na_notify;
      na->na_rx_rings[r].ckr_na_notify = IFNET_IS_REDIRECT(ifp)
                                             ? nx_netif_na_notify_rx_redirect
                                             : nx_netif_na_notify_rx;
      if (nifna->nifna_rx_mit != NULL) {
        nx_netif_mit_init(nif, ifp, &nifna->nifna_rx_mit[r],
  ...
  case NA_ACTIVATE_MODE_OFF:
    ...
    while (ifp->if_start_active != 0) {
      ...
      (void)msleep(&ifp->if_start_waiters, &ifp->if_start_lock, (PZERO - 1),
                   na->na_name, NULL);
    }
    ...
    /* reset all TX notify callbacks */
    for (r = 0; r < na_get_nrings(na, NR_TX); r++) {
      na->na_tx_rings[r].ckr_na_notify = na->na_tx_rings[r].ckr_netif_notify;
    ...
    /* reset all RX notify callbacks */
    for (r = 0; r < na_get_nrings(na, NR_RX); r++) {
      na->na_rx_rings[r].ckr_na_notify = na->na_rx_rings[r].ckr_netif_notify;
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_attach(struct kern_nexus *nx, struct ifnet *ifp) {
  struct nx_netif *nif = NX_NETIF_PRIVATE(nx);
  ...
  *(nexus_stats_type_t *)(uintptr_t)&devna->na_stats_type =
  ...
  if ((retval = NX_DOM_PROV(nx)->nxdom_prov_mem_new(NX_DOM_PROV(nx), nx,
                                                    devna)) != 0) {
  ...
  *(uint32_t *)(uintptr_t)&devna->na_flowadv_max = nxp->nxp_flowadv_max;
  ...
  (void)snprintf(hostna->na_name, sizeof(hostna->na_name), "%s^",
                 devna->na_name);
 ...
 * initialized here.
 */
static void nx_netif_flags_init(struct nx_netif *nif) {
 ...
 */
static void nx_netif_flags_fini(struct nx_netif *nif) {
  ifnet_t ifp = nif->nif_ifp;
...
SK_NO_INLINE_ATTRIBUTE
static void nx_netif_callbacks_init(struct nx_netif *nif) {
  ifnet_t ifp = nif->nif_ifp;
  ...
  if (ifnet_is_low_latency(ifp)) {
    ifnet_set_detach_notify_locked(ifp, nx_netif_llw_detach_notify, ifp->if_na);
  }
}

SK_NO_INLINE_ATTRIBUTE
static void nx_netif_callbacks_fini(struct nx_netif *nif) {
  ifnet_t ifp = nif->nif_ifp;
...
configure_capab_interface_advisory(struct nx_netif *nif,
  ...
  capab.kncia_version = KERN_NEXUS_CAPAB_INTERFACE_ADVISORY_VERSION_1;
  ...
  *__DECONST(void **, &(capab.kncia_kern_context)) = nx;
  ...
  error = capab_fn(NX_PROV(nx), nx, KERN_NEXUS_CAPAB_INTERFACE_ADVISORY, &capab,
                   &capab_len);
  if (error != 0) {
    DTRACE_SKYWALK2(interface__advisory__capab__error, struct nx_netif *, nif,
                    int, error);
    return;
  }
...
static void unconfigure_capab_interface_advisory(struct nx_netif *nif) {
...
static void configure_capab_qset_extensions(struct nx_netif *nif,
                                            nxprov_capab_config_fn_t capab_fn) {
  ...
  capab_len = sizeof(capab);
  error = capab_fn(NX_PROV(nx), nx, KERN_NEXUS_CAPAB_QSET_EXTENSIONS, &capab,
                   &capab_len);
  if (error != 0) {
    DTRACE_SKYWALK2(qset__extensions__capab__error, struct nx_netif *, nif, int,
                    error);
    return;
  }
...
static void unconfigure_capab_qset_extensions(struct nx_netif *nif) {
...
int nx_netif_notify_steering_info(struct nx_netif *nif, struct netif_qset *qset,
  ...
  err = qset_ext->qe_notify_steering_info(qset_ext->qe_prov_ctx, qset->nqs_ctx,
                                          td, add);
  return err;
}
...
configure_capab_rx_flow_steering(struct nx_netif *nif,
  ...
  bzero(&capab, sizeof(capab));
  capab.kncrxfs_version = KERN_NEXUS_CAPAB_RX_FLOW_STEERING_VERSION_1;
  capab_len = sizeof(capab);
  error = capab_fn(NX_PROV(nx), nx, KERN_NEXUS_CAPAB_RX_FLOW_STEERING, &capab,
                   &capab_len);
  if (error != 0) {
    DTRACE_SKYWALK2(rx__flow__steering__capab__error, struct nx_netif *, nif,
                    int, error);
    return;
  }
...
static void unconfigure_capab_rx_flow_steering(struct nx_netif *nif) {
...
int nx_netif_configure_rx_flow_steering(
    struct kern_nexus *nx, uint32_t id,
    struct ifnet_traffic_descriptor_common *td,
  ...
  err = rx_flow_steering->config_fn(rx_flow_steering->prov_ctx, id, td, action);
  return err;
}

static void nx_netif_capabilities_init(struct nx_netif *nif) {
  struct kern_nexus *nx = nif->nif_nx;
  nxprov_capab_config_fn_t capab_fn;
...
static void nx_netif_capabilities_fini(struct nx_netif *nif) {
  unconfigure_capab_interface_advisory(nif);
  unconfigure_capab_qset_extensions(nif);
  unconfigure_capab_rx_flow_steering(nif);
}

static void nx_netif_verify_tso_config(struct nx_netif *nif) {
  ifnet_t ifp = nif->nif_ifp;
  ...
  VERIFY(PP_BUF_SIZE_DEF(nif->nif_nx->nx_tx_pp) >= max(tso_v4_mtu, tso_v6_mtu));
}

void na_netif_finalize(struct nexus_netif_adapter *nifna, struct ifnet *ifp) {
  struct nx_netif *nif = nifna->nifna_netif;
  struct kern_nexus *nx = nif->nif_nx;
  struct nexus_adapter *devna = nx_port_get_na(nx, NEXUS_PORT_NET_IF_DEV);
...
void nx_netif_reap(struct nexus_netif_adapter *nifna, struct ifnet *ifp,
 ...
 * prevent the detach from completing indefinitely. Note that this is only
 * needed for low latency VPNAs. Userspace do get notified about interface
 * detach events for other NA types (custom ether and filter) and will do the
 * necessary cleanup. The cleanup is done in two phases: 1) VPNAs channels are
 * defuncted. This releases the resources held by VPNAs and causes the device
 * channel to be closed. All ifnet references held by VPNAs are also released.
 ...
 */
void nx_netif_llw_detach_notify(void *arg) {
...
void nx_netif_copy_stats(struct nexus_netif_adapter *nifna,
...
int nx_netif_na_special(struct nexus_adapter *na, struct kern_channel *ch,
...
int nx_netif_na_special_common(struct nexus_adapter *na,
                               struct kern_channel *ch, struct chreq *chr,
                               nxspec_cmd_t spec_cmd) {
  int error = 0;
  ...
  SK_DF(error ? SK_VERB_ERROR : SK_VERB_NETIF,
        ...
        "spec_cmd %u (err %d)",
        SK_KVA(ch), na->na_name, SK_KVA(na), na->na_flags, SK_KVA(ch->ch_nexus),
        spec_cmd, error);
 ...
 */
int nx_netif_na_find(struct kern_nexus *nx, struct kern_channel *ch,
...
#pragma unused(ch)
  ...
  *nap = NULL; /* default */
  ...
  SK_D("name \"%s\" spec_uuid \"%s\" port %d mode 0x%x pipe_id %u "
       ...
       (int)chr->cr_port, chr->cr_mode, chr->cr_pipe_id, (int)chr->cr_ring_id,
       chr->cr_ring_set, chr->cr_endpoint, create,
  ...
  case NEXUS_PORT_NET_IF_DEV:
     ...
     * user memory regions.
     */
    if (p != kernproc && (!skywalk_netif_direct_allowed(ifp->if_xname) ||
                          (kauth_cred_issuser(kauth_cred_get()) == 0 &&
                           (anon || nif->nif_dev_nxb == NULL || nxb == NULL ||
                            !nxb_is_equal(nif->nif_dev_nxb, nxb))))) {
      DTRACE_SKYWALK2(direct__not__allowed, struct ifnet *, ifp, struct chreq *,
                      chr);
      err = ENOTSUP;
      goto done;
    }
...
int nx_netif_dev_krings_create(struct nexus_adapter *na,
                               struct kern_channel *ch) {
  int ret;
...
void nx_netif_dev_krings_delete(struct nexus_adapter *na,
...
struct nx_netif *nx_netif_alloc(zalloc_flags_t how) {
  struct nx_netif *n;
...
static void nx_netif_destroy(struct nx_netif *n) {
...
void nx_netif_release(struct nx_netif *n) {
  SK_LOCK_ASSERT_HELD();
...
void nx_netif_retain(struct nx_netif *n) {
  SK_LOCK_ASSERT_HELD();
...
void nx_netif_free(struct nx_netif *n) { nx_netif_release(n); }

static int nx_netif_interface_advisory_report(
    struct kern_nexus *nx, const struct ifnet_interface_advisory *advisory) {
   ...
   */
  intf_adv->cksum =
      os_cpu_copy_in_cksum(advisory, &intf_adv->adv, sizeof(*advisory), 0);
...
static errno_t nx_netif_interface_advisory_notify(
    void *kern_ctx, const struct ifnet_interface_advisory *advisory) {
  static_assert(offsetof(struct ifnet_interface_advisory, version) ==
                offsetof(struct ifnet_interface_advisory, header.version));
  static_assert(offsetof(struct ifnet_interface_advisory, direction) ==
                offsetof(struct ifnet_interface_advisory, header.direction));
  static_assert(
      offsetof(struct ifnet_interface_advisory, _reserved) ==
  ...
  if (__improbable(
          (advisory->header.version < IF_INTERFACE_ADVISORY_VERSION_MIN) ||
          (advisory->header.version > IF_INTERFACE_ADVISORY_VERSION_MAX))) {
    SK_ERR("Invalid advisory version %d", advisory->header.version);
    return EINVAL;
  }
  if (__improbable(
          (advisory->header.direction != IF_INTERFACE_ADVISORY_DIRECTION_TX) &&
          (advisory->header.direction != IF_INTERFACE_ADVISORY_DIRECTION_RX))) {
    SK_ERR("Invalid advisory direction %d", advisory->header.direction);
    return EINVAL;
  }
  if (__improbable(
...
void nx_netif_config_interface_advisory(struct kern_nexus *nx, bool enable) {
 ...
 */
static boolean_t packet_is_filterable(struct nexus_netif_adapter *nifna,
 ...
 * are enabled.
 */
static void get_filterable_packets(struct nexus_netif_adapter *nifna,
  ...
  *passthrough_chain = passthrough_head;
  ...
  DTRACE_SKYWALK6(filterable, struct nexus_netif_adapter *, nifna, int, fcnt,
                  int, pcnt, int, dcnt, struct __kern_packet *, fpkt_head,
                  struct __kern_packet *, passthrough_head);
}
 ...
 * unmodified.
 */
void netif_receive(struct nexus_netif_adapter *nifna,
                   struct __kern_packet *pkt_chain,
  ...
  if (nif->nif_filter_cnt > 0) {
    ...
    get_filterable_packets(nifna, pkt_chain, &fpkt_chain, &passthrough_chain);
    if (fpkt_chain != NULL) {
  ...
  if (na->na_rx != NULL) {
    ...
    DTRACE_SKYWALK2(no__rx__cb, struct nx_netif *, nif, struct __kern_packet *,
                    pkt_chain);
...
static slot_idx_t netif_rate_limit(struct __kern_channel_ring *r, uint64_t rate,
                                   slot_idx_t begin, slot_idx_t end,
  ...
  if (__improbable(r->ckr_tbr_token == CKR_TBR_TOKEN_INVALID)) {
    ...
    r->ckr_tbr_token += ((sec * USEC_PER_SEC + usec) * rate / USEC_PER_SEC);
  ...
  *rate_limited = FALSE;
  ...
  SK_DF(SK_VERB_FSW | SK_VERB_RX, "ckr %p %s rate limited at %d", r,
        r->ckr_name, i);
...
SK_NO_INLINE_ATTRIBUTE
static struct __kern_packet *consume_pkts(struct __kern_channel_ring *ring,
                                          slot_idx_t end) {
...
int netif_rx_notify(struct __kern_channel_ring *ring, struct proc *p,
                    uint32_t flags) {
  ...
  KDBG((SK_KTRACE_NETIF_RX_NOTIFY_DEFAULT | DBG_FUNC_START), SK_KVA(ring));
  ...
  if (err != 0) {
    ...
    SK_DF(SK_VERB_FSW,
          ...
          "(%d)",
          KRNA(ring)->na_name, SK_KVA(KRNA(ring)), ring->ckr_name, SK_KVA(ring),
          ring->ckr_flags, err);
    goto out;
  }
  ...
  if (__improbable(ring->ckr_khead == ktail)) {
    SK_DF(SK_VERB_FSW | SK_VERB_NOTIFY | SK_VERB_RX,
          ...
          "\"%s\" (%p)",
          KRNA(ring)->na_name, SK_KVA(KRNA(ring)));
    goto put_out;
  }
  ktail = netif_rate_limit(ring, nif->nif_input_rate, ring->ckr_rhead, ktail,
                           &ring->ckr_rate_limited);

  pkt_chain = consume_pkts(ring, ktail);
  if (pkt_chain != NULL) {
    netif_receive(nifna, pkt_chain, &stats);

    if (ring->ckr_netif_mit_stats != NULL && stats.nps_pkts != 0 &&
        stats.nps_bytes != 0) {
      ring->ckr_netif_mit_stats(ring, stats.nps_pkts, stats.nps_bytes);
    }
  }
...
out:
  KDBG((SK_KTRACE_NETIF_RX_NOTIFY_DEFAULT | DBG_FUNC_END), SK_KVA(ring), err);
  return err;
}
 ...
 */
static channel_ring_notify_t netif_hwna_get_notify(netif_mode_t mode) {
...
static uint32_t netif_mode_to_flag(netif_mode_t mode) {
  uint32_t flag = 0;
...
static void netif_hwna_config_mode(struct nexus_adapter *hwna,
                                   netif_mode_t mode,
                                   void (*rx)(struct nexus_adapter *,
  ...
  ASSERT(hwna->na_type == NA_NETIF_DEV || hwna->na_type == NA_NETIF_COMPAT_DEV);
...
void netif_hwna_set_mode(struct nexus_adapter *hwna, netif_mode_t mode,
                         void (*rx)(struct nexus_adapter *,
                                    struct __kern_packet *,
                                    struct nexus_pkt_stats *)) {
...
void netif_hwna_clear_mode(struct nexus_adapter *hwna) {
  return netif_hwna_config_mode(hwna, NETIF_MODE_NONE, NULL, FALSE);
}

static void netif_inject_rx(struct nexus_adapter *na,
                            struct __kern_packet *pkt_chain) {
  struct nexus_netif_adapter *nifna = NIFNA(na);
  struct nx_netif *nif = nifna->nifna_netif;
  struct netif_stats *nifs = &nif->nif_stats;
  ...
  if (r->ckr_netif_mit_stats != NULL && stats.nps_pkts != 0 &&
      stats.nps_bytes != 0) {
 ...
 */
errno_t nx_netif_filter_rx_cb(struct nexus_netif_adapter *nifna,
                              struct __kern_packet *fpkt_chain,
                              uint32_t flags) {
#pragma unused(flags)
  struct nx_netif *nif = nifna->nifna_netif;
  ...
  pkt_chain =
      nx_netif_filter_pkt_to_pkt_chain(nifna, fpkt_chain, NETIF_CONVERT_RX);
  if (pkt_chain == NULL) {
    return ENOMEM;
  }
  if (nif->nif_flow_cnt > 0) {
    ...
    err = nx_netif_demux(nifna, pkt_chain, &remain, NULL, NETIF_FLOW_INJECT);
    if (remain == NULL) {
      return err;
    }
    pkt_chain = remain;
  ...
  if (na->na_rx != NULL) {
    ...
    int dropcnt = 0;
    nx_netif_free_packet_chain(pkt_chain, &dropcnt);
    STATS_ADD(nifs, NETIF_STATS_FILTER_DROP_NO_RX_CB, dropcnt);
    STATS_ADD(nifs, NETIF_STATS_DROP, dropcnt);
  }
  return 0;
}
 ...
 */
errno_t nx_netif_filter_tx_cb(struct nexus_netif_adapter *nifna,
                              struct __kern_packet *fpkt_chain,
                              uint32_t flags) {
#pragma unused(flags)
  struct nx_netif *nif = nifna->nifna_netif;
  ...
  if (NETIF_IS_COMPAT(nif)) {
    ...
    m_chain =
        nx_netif_filter_pkt_to_mbuf_chain(nifna, fpkt_chain, NETIF_CONVERT_TX);
    if (m_chain == NULL) {
      return ENOMEM;
    }
    /*
    ...
    err = nx_netif_filter_tx_processed_mbuf_enqueue(nifna, sc, m_chain);
  } else {
    struct __kern_packet *pkt_chain;
    kern_packet_svc_class_t sc;

    pkt_chain =
        nx_netif_filter_pkt_to_pkt_chain(nifna, fpkt_chain, NETIF_CONVERT_TX);
    if (pkt_chain == NULL) {
      return ENOMEM;
    }
    /*
    ...
    err = nx_netif_filter_tx_processed_pkt_enqueue(nifna, sc, pkt_chain);
  }
...
void nx_netif_vp_region_params_adjust(struct nexus_adapter *na,
...
static bool netif_use_starter_thread(struct ifnet *ifp, uint32_t flags) {
#if (DEVELOPMENT || DEBUG)
  ...
  if (!SKYWALK_NATIVE(ifp) || NA(ifp) == NULL ||
      ...
      (flags & NETIF_XMIT_FLAG_PACING) != 0 || sk_is_rx_notify_protected() ||
      sk_is_async_transmit_protected() ||
      (sk_is_sync_protected() && (flags & NETIF_XMIT_FLAG_HOST) != 0)) {
    DTRACE_SKYWALK2(use__starter__thread, struct ifnet *, ifp, uint32_t, flags);
    ifnet_start(ifp);
    return true;
  }
...
void netif_transmit(struct ifnet *ifp, uint32_t flags) {
...
static struct ifclassq *netif_get_default_ifcq(struct nexus_adapter *hwna) {
  struct nx_netif *nif;
...
static errno_t netif_deq_packets(struct nexus_adapter *hwna,
                                 struct ifclassq *ifcq, uint32_t pkt_limit,
                                 uint32_t byte_limit,
                                 struct __kern_packet **head,
                                 boolean_t *pkts_pending,
                                 kern_packet_svc_class_t sc, uint32_t *pkt_cnt,
                                 uint32_t *bytes, uint8_t qset_idx) {
  ...
  svc = (sch_model & IFNET_SCHED_DRIVER_MANGED_MODELS) ? (mbuf_svc_class_t)sc
                                                       : MBUF_SC_UNSPEC;
  ...
  ifclassq_get_len(ifcq, (mbuf_svc_class_t)sc, qset_idx, &pkts_cnt, &bytes_cnt);
...
SK_LOG_ATTRIBUTE
static void netif_no_ring_space_log(const struct nexus_adapter *na,
                                    const kern_channel_ring_t ring) {
  SK_DF(SK_VERB_SYNC | SK_VERB_TX,
        "no ring space: na \"%s\" [%u] "
        "\"%s\"(kh %u kt %u | rh %u rt %u)",
        na->na_name, ring->ckr_ring_id, ring->ckr_name, ring->ckr_khead,
        ring->ckr_ktail, ring->ckr_rhead, ring->ckr_rtail);
}
#endif /* SK_LOG */
 ...
 */
errno_t netif_ring_tx_refill(const kern_channel_ring_t ring, uint32_t pkt_limit,
                             uint32_t byte_limit, boolean_t tx_doorbell_ctxt,
  ...
  *pkts_pending = FALSE;
  ...
  if (__improbable(pkt_limit == 0 || byte_limit == 0)) {
    SK_ERR("invalid limits plim %d, blim %d", pkt_limit, byte_limit);
    rc = EINVAL;
    goto out;
  }

  if (__improbable(!ifnet_is_fully_attached(ifp))) {
    SK_ERR("hwna %p ifp %s (%p), interface not attached", SK_KVA(hwna),
           if_name(ifp), SK_KVA(ifp));
    rc = ENXIO;
    goto out;
  }
  ...
  if (__improbable(KR_DROP(ring) || !NA_IS_ACTIVE(ring->ckr_na))) {
...
#if SK_LOG
    if (__improbable((sk_verbose & (SK_VERB_SYNC | SK_VERB_TX)) != 0)) {
  ...
  if (tx_doorbell_ctxt && ((hwna->na_flags & NAF_VIRTUAL_DEVICE) == 0)) {
    pkt_limit = MIN(pkt_limit, nx_netif_doorbell_max_dequeue);
  }

  rc = netif_deq_packets(hwna, NULL, pkt_limit, byte_limit, &head, pkts_pending,
                         ring->ckr_svc, NULL, NULL, 0);
  ...
  if (rc != 0) {
     ...
     */
    SK_DF(SK_VERB_SYNC | SK_VERB_TX, "nothing to dequeue, err %d", rc);
...
sync_ring:
  ...
  kr_exit(ring);
out:
  KDBG((SK_KTRACE_NETIF_RING_TX_REFILL | DBG_FUNC_END), SK_KVA(ring), rc, 0,
       npkts);

  return rc;
}

#define NQ_EWMA(old, new, decay)                                               \
  do {                                                                         \
    u_int64_t _avg;                                                            \
...
void kern_netif_increment_queue_stats(kern_netif_queue_t queue,
  ...
  if (stats->nq_min_pkts == 0 || pkt_count < stats->nq_min_pkts) {
  ...
  if (__probable(queue->nq_accumulate_start != 0)) {
    ...
    if (diff_secs >= nq_accumulate_interval) {
      ...
      NQ_EWMA(stats->nq_bytes_ps_ma, bps, nq_transfer_decay);
...
void kern_netif_queue_rx_enqueue(kern_netif_queue_t queue,
                                 kern_packet_t ph_chain, uint32_t count,
                                 uint32_t flags) {
#pragma unused(count)
...
errno_t kern_netif_queue_tx_dequeue(kern_netif_queue_t queue,
                                    uint32_t pkt_limit, uint32_t byte_limit,
                                    boolean_t *pending,
                                    kern_packet_t *ph_chain) {
  struct netif_queue *q = queue;
  struct netif_llink *llink = q->nq_qset->nqs_llink;
  struct netif_stats *nifs = &llink->nll_nif->nif_stats;
  struct nexus_adapter *hwna;
  ...
  rc = netif_deq_packets(hwna, q->nq_qset->nqs_ifcq, pkt_limit, byte_limit,
                         &pkt_chain, pending, q->nq_svc, &pkt_cnt, &bytes,
                         q->nq_qset->nqs_idx);
...
errno_t kern_netif_qset_tx_queue_len(kern_netif_qset_t qset, uint32_t svc,
...
errno_t kern_nexus_netif_llink_remove(struct kern_nexus *nx,
...
errno_t kern_netif_queue_get_service_class(kern_netif_queue_t queue,
