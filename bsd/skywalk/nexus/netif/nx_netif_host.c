...
#define DBG_FUNC_NX_NETIF_HOST_ENQUEUE SKYWALKDBG_CODE(DBG_SKYWALK_NETIF, 2)
...
#define SK_IFCAP_CSUM                                                          \
  (IFCAP_HWCSUM | IFCAP_CSUM_PARTIAL | IFCAP_CSUM_ZERO_INVERT)

static bool nx_netif_host_is_gso_needed(struct nexus_adapter *na) {
...
static void nx_netif_host_adjust_if_capabilities(struct nexus_adapter *na,
                                                 bool activate) {
  struct nx_netif *nif = ((struct nexus_netif_adapter *)na)->nifna_netif;
  ...
  if (activate) {
    ...
    if (NA_KERNEL_ONLY(na)) {
      if (na->na_type == NA_NETIF_HOST) { /* native */
        ifp->if_hwassist |= IFNET_MULTIPAGES | (nif->nif_hwassist &
                                                (IFNET_CHECKSUMF | IFNET_TSOF));
        ifp->if_capabilities |=
            (nif->nif_capabilities & (SK_IFCAP_CSUM | IFCAP_TSO));
        ifp->if_capenable |= (nif->nif_capenable & (SK_IFCAP_CSUM | IFCAP_TSO));
        /*
        ...
        ifp->if_hwassist |=
            (nif->nif_hwassist & (IFNET_CHECKSUMF | IFNET_TSOF));
        ifp->if_capabilities |=
            (nif->nif_capabilities & (SK_IFCAP_CSUM | IFCAP_TSO));
        ifp->if_capenable |= (nif->nif_capenable & (SK_IFCAP_CSUM | IFCAP_TSO));
      }
    ...
    if ((sk_fsw_tx_agg_tcp != 0) && (na->na_type == NA_NETIF_HOST)) {
      ifp->if_hwassist &= ~IFNET_TSOF;
      ifp->if_capabilities &= ~IFCAP_TSO;
      ifp->if_capenable &= ~IFCAP_TSO;
    }
    ...
    ifp->if_hwassist |=
        (nif->nif_hwassist & (IFNET_CHECKSUMF | IFNET_TSOF | IFNET_MULTIPAGES));
    ifp->if_capabilities |=
        (nif->nif_capabilities & (SK_IFCAP_CSUM | IFCAP_TSO));
    ifp->if_capenable |= (nif->nif_capenable & (SK_IFCAP_CSUM | IFCAP_TSO));
  }
...
int nx_netif_host_na_activate(struct nexus_adapter *na,
                              na_activate_mode_t mode) {
  struct ifnet *ifp = na->na_ifp;
  int error = 0;

  ASSERT(na->na_type == NA_NETIF_HOST || na->na_type == NA_NETIF_COMPAT_HOST);
...
int nx_netif_host_krings_create(struct nexus_adapter *na,
  ...
  if (ret == 0) {
    ...
    for (i = 0; i < na_get_nrings(na, NR_RX); i++) {
      ...
      nx_mbq_safe_init(kring, &kring->ckr_rx_queue, NX_MBQ_NO_LIMIT,
 ...
 */
void nx_netif_host_krings_delete(struct nexus_adapter *na,
  ...
  for (i = 0; i < na_get_nrings(na, NR_RX); i++) {
    ...
    SK_DF(SK_VERB_NETIF,
          ...
          "krflags 0x%x with qlen %u",
          na->na_name, SK_KVA(na), kring->ckr_name, SK_KVA(kring),
          kring->ckr_flags, nx_mbq_len(q));
    nx_mbq_purge(q);
    if (!defunct) {
...
int nx_netif_host_na_rxsync(struct __kern_channel_ring *kring, struct proc *p,
                            uint32_t flags) {
 ...
 */
int nx_netif_host_na_txsync(struct __kern_channel_ring *kring, struct proc *p,
                            uint32_t flags) {
#pragma unused(kring, p, flags)
  return 0;
}

int nx_netif_host_na_special(struct nexus_adapter *na, struct kern_channel *ch,
 ...
 */
static void nx_netif_host_catch_tx(struct nexus_adapter *na, bool activate) {
  struct ifnet *ifp = na->na_ifp;
  int err = 0;
  ...
  if (activate) {
     ...
     */
    if (na->na_type == NA_NETIF_HOST) {
      err = ifnet_set_output_handler(ifp, nx_netif_host_is_gso_needed(na)
                                              ? netif_gso_dispatch
...
static int get_af_from_mbuf(struct mbuf *m) {
  /*
 ...
 */
int nx_netif_host_output(struct ifnet *ifp, struct mbuf *m_chain) {
  ...
  struct mbuf *m_head = m_chain, *m = NULL, *drop_list = NULL,
              *free_list = NULL;
  ...
  while (m_head) {
    ...
    if (currentkring != kring) {
      if (currentkring != NULL) {
        KDBG((SK_KTRACE_NETIF_HOST_ENQUEUE | DBG_FUNC_END),
             SK_KVA(currentkring), error);
      }
      currentkring = kring;
      KDBG((SK_KTRACE_NETIF_HOST_ENQUEUE | DBG_FUNC_START),
           SK_KVA(currentkring));
    }
    if (__improbable(!NA_IS_ACTIVE(hwna) || !NA_IS_ACTIVE(hostna))) {
      STATS_INC(nifs, NETIF_STATS_DROP_NA_INACTIVE);
      SK_ERR("\"%s\" (%p) not in skywalk mode anymore", hwna->na_name,
             SK_KVA(hwna));
      error = ENXIO;
      pkt_drop = TRUE;
      goto out;
    ...
    if (__improbable(KR_DROP(&hostna->na_rx_rings[0]) || KR_DROP(kring))) {
      ...
      SK_DF(SK_VERB_NETIF, "kr \"%s\" (%p) krflags 0x%x or %s in drop mode",
            kring->ckr_name, SK_KVA(kring), kring->ckr_flags, ifp->if_xname);
      error = ENXIO;
      pkt_drop = TRUE;
      goto out;
    }
    if (__improbable(((unsigned)m_pktlen(m) + ifp->if_tx_headroom) >
      ...
      SK_ERR("\"%s\" (%p) from_host, drop packet size %u > %u", hwna->na_name,
             SK_KVA(hwna), m_pktlen(m), kring->ckr_max_pkt_len);
      pkt_drop = TRUE;
      goto out;
    }
    /*
    ...
    if (!netif_chain_enqueue_enabled(ifp)) {
      if (qset != NULL) {
        error = ifnet_enqueue_pkt(ifp, qset->nqs_ifcq, kpkt, false, &pkt_drop);
  ...
  if (__probable(!KPKTQ_EMPTY(&pkt_q))) {
    ...
    if (qset != NULL) {
      error = ifnet_enqueue_pkt_chain(ifp, qset->nqs_ifcq, pkt_chain_head,
                                      pkt_chain_tail, n_pkts, n_bytes, false,
                                      &pkt_drop);
      nx_netif_qset_release(&qset);
    } else {
      /* callee consumes packet */
...
static inline int get_l2_hlen(struct mbuf *m, uint8_t *l2len) {
  /*
   * -fbounds-safety: Although m_pkthdr.pkt_hdr is a void * without
...
SK_LOG_ATTRIBUTE
static void nx_netif_mbuf_to_kpkt_log(struct __kern_packet *kpkt, uint32_t len,
                                      uint32_t poff) {
  uint8_t *baddr;
  uint32_t pkt_len;
  ...
  SK_DF(SK_VERB_HOST | SK_VERB_TX,
        "mlen %u dplen %u"
        " hr %u l2 %u poff %u",
        len, kpkt->pkt_length, kpkt->pkt_headroom, kpkt->pkt_l2_len, poff);
...
nx_netif_mbuf_to_kpkt(struct nexus_adapter *na, struct mbuf *m) {
  ...
  if (__improbable(ph == 0)) {
    ...
    SK_DF(SK_VERB_MEM,
          ...
          pp->pp_name, SK_KVA(pp), if_name(na->na_ifp));
    return NULL;
  }

  copysum = ((m->m_pkthdr.csum_flags & (CSUM_DATA_VALID | CSUM_PARTIAL)) ==
  ...
  nif->nif_pkt_copy_from_mbuf(NR_TX, ph, poff, m, 0, len, copysum,
