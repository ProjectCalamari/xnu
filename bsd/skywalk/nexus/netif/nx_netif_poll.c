...
#define NETIF_POLL_EWMA(old, new, decay)                                       \
  do {                                                                         \
    uint32_t _avg;                                                             \
    if ((_avg = (old)) > 0)                                                    \
...
static inline void nx_netif_rxpoll_set_mode(struct ifnet *ifp,
                                            ifnet_model_t mode) {
  errno_t err;
  uint64_t ival;
  struct timespec ts;
  ...
  if ((err = ((*ifp->if_input_ctl)(ifp, IFNET_CTL_SET_INPUT_MODEL, sizeof(p),
                                   &p))) != 0) {
    SK_ERR("%s: error setting polling mode to %s (%d)", if_name(ifp),
      ...
      if (net_timercmp(&delta, &netif_poll_dbgrate, >=)) {
        *(&ifp->if_poll_dbg_lasttime) = *(&now);
        SK_DF(SK_VERB_NETIF_POLL,
              ...
              ifp->if_rxpoll_phiwat, ifp->if_rxpoll_bavg, ifp->if_rxpoll_blowat,
...
skip:
  ...
  if (poll_thresh != 0 && poll_ival > 0 && (--poll_thresh % poll_ival) == 0) {
    lck_mtx_lock_spin(&ifp->if_poll_lock);
    ifp->if_poll_req++;
    lck_mtx_unlock(&ifp->if_poll_lock);
  }
 ...
 */
errno_t netif_rxpoll_set_params(struct ifnet *ifp, struct ifnet_poll_params *p,
                                boolean_t locked) {
  errno_t err;
...
static inline void netif_rxpoll_poll_driver(
    struct ifnet *ifp, uint32_t m_lim, struct ifnet_stat_increment_param *s,
    struct timespec *start_time, struct timespec *poll_duration) {
  ...
  (void)ifnet_input_poll(ifp, m_head, m_tail, (m_head != NULL) ? s : NULL);
  nanouptime(&now);
  net_timersub(&now, start_time, poll_duration);

  SK_DF(SK_VERB_NETIF_POLL,
        "%s: polled %d pkts, pkts avg %d max %d, "
...
netif_rxpoll_compat_thread_cont(void *v, wait_result_t wres) {
  ...
  for (;;) {
    ...
    if (poll) {
      netif_rxpoll_poll_driver(ifp, m_lim, &s, &start_time, &poll_duration);
      /*
    ...
    if (__improbable((ifp->if_poll_flags & IF_POLLF_TERMINATING) != 0)) {
      break;
    }
  ...
  if (__probable((ifp->if_poll_flags & IF_POLLF_TERMINATING) == 0)) {
    ...
    if (ts != NULL) {
      ...
      clock_interval_to_deadline((uint32_t)interval, NSEC_PER_USEC, &deadline);
    }
  ...
  terminate:
    ...
    lck_mtx_unlock(&ifp->if_poll_lock);
    SK_DF(SK_VERB_NETIF_POLL, "%s: poller thread terminated", if_name(ifp));
