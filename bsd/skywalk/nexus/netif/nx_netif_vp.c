...
static int netif_vp_na_channel_event_notify(struct nexus_adapter *,
                                            struct __kern_channel_event *,
                                            uint16_t);

static void netif_vp_dump_packet(struct __kern_packet *pkt) {
  uint8_t *baddr;
  ...
  DTRACE_SKYWALK2(dump__packet, struct __kern_packet *, pkt, uint8_t *, baddr);
}

static int netif_copy_or_attach_pkt(struct __kern_channel_ring *ring,
  ...
  if (pkt->pkt_qum.qum_pp == ring->ckr_pp) {
    ...
    dpkt = nx_netif_pkt_to_pkt(NIFNA(ring->ckr_na), pkt,
                               ring->ckr_na->na_type == NA_NETIF_VP
                                   ? NETIF_CONVERT_RX
                                   : NETIF_CONVERT_TX);
...
static errno_t netif_deliver_pkt(struct nexus_adapter *na,
...
#pragma unused(flags)
  ...
  if (__improbable(KR_DROP(ring))) {
    ...
    STATS_ADD(nifs, NETIF_STATS_VP_DROP_USER_RING_DISABLED, dropcnt);
    STATS_ADD(nifs, NETIF_STATS_DROP, dropcnt);
    DTRACE_SKYWALK2(ring__drop, struct __kern_channel_ring *, ring, int,
                    dropcnt);
  ...
  DTRACE_SKYWALK4(delivered, struct nexus_adapter *, na,
                  struct __kern_channel_ring *, ring, struct __kern_packet *,
                  pkt, int, cnt);

  if (pkt != NULL) {
    nx_netif_free_packet_chain(pkt, &dropcnt);
    STATS_ADD(nifs, NETIF_STATS_VP_DROP_USER_RING_NO_SPACE, dropcnt);
    STATS_ADD(nifs, NETIF_STATS_DROP, dropcnt);
    DTRACE_SKYWALK2(deliver__drop, struct nexus_adapter *, na, int, dropcnt);
  }
  if (last_slot != NULL) {
...
static errno_t netif_deliver_cb(void *arg, void *chain, uint32_t flags) {
  return netif_deliver_pkt(arg, chain, flags);
}

static int netif_hwna_rx_get_pkts(struct __kern_channel_ring *ring,
                                  struct proc *p, uint32_t flags,
                                  struct __kern_packet **chain) {
  int err, cnt = 0;
  sk_protect_t protect;
  ...
  if (__improbable(ring->ckr_khead == ktail)) {
    SK_DF(SK_VERB_VP, "spurious wakeup on hwna %s (%p)", KRNA(ring)->na_name,
          SK_KVA(KRNA(ring)));
  ...
  DTRACE_SKYWALK2(rx__notify, struct __kern_channel_ring *, ring, int, cnt);
  *chain = pkt_chain;
out:
  sk_sync_unprotect(protect);
  kr_exit(ring);
...
int netif_llw_rx_notify(struct __kern_channel_ring *ring, struct proc *p,
                        uint32_t flags) {
  int err;
  struct __kern_packet *__single pkt_chain = NULL;
  ...
  return nx_netif_demux(NIFNA(KRNA(ring)), pkt_chain, NULL, NULL,
                        NETIF_FLOW_SOURCE);
}

static void netif_change_pending(struct nx_netif *nif) {
  SK_LOCK_ASSERT_HELD();
  while ((nif->nif_flags & NETIF_FLAG_CHANGE_PENDING) != 0) {
    DTRACE_SKYWALK1(change__pending__wait, struct nx_netif *, nif);
    (void)msleep(&nif->nif_flags, &sk_lock, (PZERO - 1), __func__, NULL);
    DTRACE_SKYWALK1(change__pending__wake, struct nx_netif *, nif);
  }
  nif->nif_flags |= NETIF_FLAG_CHANGE_PENDING;
}

static void netif_change_done(struct nx_netif *nif) {
  SK_LOCK_ASSERT_HELD();
...
static errno_t netif_hwna_setup(struct nx_netif *nif) {
  ...
  if (ch == NULL) {
    SK_ERR("%s: failed to open nx %p (err %d)", if_name(nif->nif_ifp),
           SK_KVA(nx), err);
    netif_change_done(nif);
    return err;
  }
  ...
  nif->nif_hw_ch = ch;
  SK_DF(SK_VERB_VP, "%s: hw channel opened %p, %s:%s", if_name(nif->nif_ifp),
        SK_KVA(ch), NX_DOM(nx)->nxdom_name, NX_DOM_PROV(nx)->nxdom_prov_name);
  netif_change_done(nif);
  return 0;
}

static void netif_hwna_teardown(struct nx_netif *nif) {
  struct kern_nexus *nx = nif->nif_nx;
  ...
  SK_DF(SK_VERB_VP, "%s: hw channel closing %p, %s:%s", if_name(nif->nif_ifp),
        SK_KVA(ch), NX_DOM(nx)->nxdom_name, NX_DOM_PROV(nx)->nxdom_prov_name);
  ...
  nif->nif_hw_ch = NULL;
  SK_DF(SK_VERB_VP, "%s: hw channel closed, %s:%s", if_name(nif->nif_ifp),
        NX_DOM(nx)->nxdom_name, NX_DOM_PROV(nx)->nxdom_prov_name);
  netif_change_done(nif);
}

static int netif_vp_na_activate_on(struct nexus_adapter *na) {
  errno_t err;
  ...
  NETIF_WLOCK(nif);
  err = nx_port_get_info(nif->nif_nx, na->na_nx_port, NX_PORT_INFO_TYPE_NETIF,
                         &npi, sizeof(npi));
  NETIF_WUNLOCK(nif);
  if (err != 0) {
...
static int netif_vp_na_activate_off(struct nexus_adapter *na) {
  errno_t err;
  struct nexus_netif_adapter *nifna;
  struct nx_netif *nif;
...
static int netif_vp_na_activate(struct nexus_adapter *na,
  ...
  SK_DF(SK_VERB_VP, "na \"%s\" (%p) %s err %d", na->na_name, SK_KVA(na),
        na_activate_mode2str(mode), err);
  return err;
}

/*
 * A variation of netif data path for llw, which sends packets to the
 * appropriate HW queue directly, bypassing AQM.
 */
static int
...
#pragma unused(p)
  struct __kern_packet *pkt = pkt_chain, *next;
  struct __kern_packet *dpkt, *__single dpkt_head = NULL,
                                        **dpkt_tail = &dpkt_head;
...
#pragma unused(ring)
  ...
  err = nxprov->nxprov_netif_ext.nxnpi_queue_tx_push(
      nxprov, nif->nif_nx, drvq->nq_ctx, &ph, &pkt_count, &byte_count);

  STATS_ADD(nifs, NETIF_STATS_VP_LL_SENT, pkt_count);
  DTRACE_SKYWALK3(ll__sent, struct __kern_channel_ring *, ring, uint32_t,
                  pkt_count, uint32_t, byte_count);
  ...
  if (ph != 0) {
    ...
    STATS_ADD(nifs, NETIF_STATS_DROP, dropcnt);
    DTRACE_SKYWALK2(ll__dropped, struct __kern_channel_ring *, ring, int,
                    dropcnt);
  }
...
static int netif_vp_send_pkt_chain_common(struct nexus_netif_adapter *dev_nifna,
                                          struct __kern_packet *pkt_chain,
                                          boolean_t compat) {
  ...
  while (pkt != NULL) {
    ...
    if (err != 0) {
      ...
      DTRACE_SKYWALK2(enqueue__failed, struct nexus_netif_adapter *, dev_nifna,
                      boolean_t, drop);
    } else {
...
static int netif_vp_send_pkt_chain(struct nexus_netif_adapter *dev_nifna,
                                   struct __kern_packet *pkt_chain,
                                   struct proc *p) {
  struct nexus_adapter *na = &dev_nifna->nifna_up;

  if (NETIF_IS_LOW_LATENCY(dev_nifna->nifna_netif)) {
    return netif_vp_send_pkt_chain_low_latency(dev_nifna, pkt_chain, p);
  }
...
static boolean_t validate_packet(struct nexus_netif_adapter *nifna,
                                 struct __kern_packet *pkt) {
  struct nx_netif *nif = nifna->nifna_netif;
  ...
  if (outbound_check == 0 ||
      (NETIF_IS_LOW_LATENCY(nif) && outbound_check != OUTBOUND_CHECK_FORCED)) {
    return TRUE;
  }
  if (!nx_netif_validate_macaddr(nif, pkt, NETIF_FLOW_OUTBOUND)) {
    return FALSE;
  }
  if (!nx_netif_flow_match(nif, pkt, nifna->nifna_flow, NETIF_FLOW_OUTBOUND)) {
    return FALSE;
  }
  return TRUE;
}

static int netif_vp_na_txsync(struct __kern_channel_ring *kring, struct proc *p,
                              uint32_t flags) {
#pragma unused(flags)
  kern_channel_slot_t last_slot = NULL, slot = NULL;
  ...
  for (;;) {
    ...
    if (__improbable(ph == 0)) {
      ...
      DTRACE_SKYWALK2(bad__slot, struct __kern_channel_ring *, kring,
                      kern_channel_slot_t, slot);
      last_slot = slot;
      continue;
    }
    ...
    if (NA_CHANNEL_EVENT_ATTACHED(KRNA(kring))) {
      __packet_set_tx_nx_port(SK_PKT2PH(pkt), KRNA(kring)->na_nx_port,
                              nifna->nifna_gencnt);
    }
  ...
  DTRACE_SKYWALK4(injected, struct nexus_netif_adapter *, nifna,
                  struct __kern_channel_ring *, kring, struct __kern_packet *,
                  head, int, cnt);
  if (last_slot != NULL) {
    kern_channel_advance_slot(kring, last_slot);
  }

  dev_nifna = NIFNA(nx_port_get_na(KRNA(kring)->na_nx, NEXUS_PORT_NET_IF_DEV));
...
static int netif_vp_na_rxsync(struct __kern_channel_ring *kring, struct proc *p,
                              uint32_t flags) {
...
static int netif_vp_na_krings_create(struct nexus_adapter *na,
                                     struct kern_channel *ch) {
  ASSERT(na->na_type == NA_NETIF_VP);
...
static void netif_vp_na_krings_delete(struct nexus_adapter *na,
                                      struct kern_channel *ch,
                                      boolean_t defunct) {
  ASSERT(na->na_type == NA_NETIF_VP);
...
#pragma unused(tx_pp)
  ...
  srp[SKMEM_REGION_TXAKSD].srp_r_obj_cnt = na_get_nrings(na, NR_TX) +
  ...
  srp[SKMEM_REGION_RXFKSD].srp_r_obj_size = MAX(nslots, afslots) * SLOT_DESC_SZ;
  ...
  nx = na->na_nx;
  nif = nx->nx_arg;
  if (vp_zerocopy != 0 && NETIF_IS_LOW_LATENCY(nif) && nx->nx_tx_pp != NULL &&
      (nx->nx_rx_pp == NULL || nx->nx_tx_pp == nx->nx_rx_pp) &&
      !PP_KERNEL_ONLY(nx->nx_tx_pp)) {
...
static int netif_vp_na_mem_new(struct kern_nexus *nx,
...
#pragma unused(nx)
  ...
  na->na_arena =
      skmem_arena_create_for_nexus(na, srp, tx_pp != NULL ? &tx_pp : NULL, NULL,
                                   FALSE, FALSE, &nx->nx_adv, &err);
  ASSERT(na->na_arena != NULL || err != 0);
  return err;
}

static void netif_vp_na_dtor(struct nexus_adapter *na) {
...
int netif_vp_na_create(struct kern_nexus *nx, struct chreq *chr,
  ...
  (void)snprintf(na->na_name, sizeof(na->na_name), "netif_vp:%d", chr->cr_port);
  ...
  na_set_nrings(
      na, NR_A,
      ((nxp->nxp_max_frags > 1) && (sk_channel_buflet_alloc != 0)) ? 2 : 1);

  slots = vp_tx_slots != 0 ? vp_tx_slots : NX_DOM(nx)->nxdom_tx_slots.nb_def;
  na_set_nslots(na, NR_TX, slots);

  slots = vp_rx_slots != 0 ? vp_rx_slots : NX_DOM(nx)->nxdom_rx_slots.nb_def;
...
static int netif_vp_na_channel_event_notify(struct nexus_adapter *vpna,
                                            struct __kern_channel_event *ev,
                                            uint16_t ev_len) {
  int err;
  char *baddr;
  kern_packet_t ph;
  ...
  err = __buflet_set_data_length(buf, (ev_len + __KERN_CHANNEL_EVENT_OFFSET));
  VERIFY(err == 0);
...
nx_netif_find_port_vpna(struct nx_netif *netif, uint32_t nx_port_id) {
  ...
  if (__improbable(!nx_port_is_valid(nx, port))) {
    SK_ERR("%s[%d] port no longer valid", if_name(netif->nif_ifp), port);
    return NULL;
  }
...
errno_t netif_vp_na_channel_event(struct nx_netif *nif, uint32_t nx_port_id,
  ...
  SK_DF(SK_VERB_EVENTS,
        ...
        if_name(nif->nif_ifp), nx_port_id, event, event_len, event->ev_type,
        event->ev_flags, event->_reserved, event->ev_dlen);
