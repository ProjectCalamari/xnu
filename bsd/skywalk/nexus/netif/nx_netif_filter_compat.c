...
static void nx_netif_filter_tx_mbuf_enqueue(struct nexus_netif_adapter *nifna,
                                            struct mbuf *m_chain) {
  ...
  if (nif->nif_filter_cnt == 0) {
    ...
    DTRACE_SKYWALK2(mbuf__default__drop, struct nx_netif *, nif, uint32_t,
                    dropcnt);
  ...
  fpkt_chain =
      nx_netif_mbuf_to_filter_pkt_chain(nifna, m_chain, NETIF_CONVERT_TX);
  if (fpkt_chain == NULL) {
    return;
  }
...
static struct mbuf *get_next_mbuf(struct nx_mbq mbqs[MBUF_TC_MAX], int *curr,
                                  int end) {
  int i;
  struct mbuf *m = NULL;
...
SK_NO_INLINE_ATTRIBUTE
static struct mbuf *nx_netif_filter_tx_processed_mbuf_dequeue(
    struct nexus_netif_adapter *nifna, mbuf_svc_class_t sc, uint32_t pkt_limit,
    uint32_t byte_limit) {
  struct nx_netif *nif = nifna->nifna_netif;
  int curr, end;
  ...
  DTRACE_SKYWALK4(processed__mbuf__dequeue, struct nexus_netif_adapter *, nifna,
                  uint32_t, cnt, uint32_t, bytes, struct mbuf *, m_head);
  return m_head;
}
...
nx_netif_filter_tx_processed_mbuf_enqueue(struct nexus_netif_adapter *nifna,
  ...
  DTRACE_SKYWALK4(processed__mbuf__enqueue, struct nexus_netif_adapter *, nifna,
                  struct nx_mbq *, q, uint32_t, qlen, uint32_t, cnt);
  nx_mbq_unlock(q);
  return 0;
}
...
nx_netif_tx_processed_mbuf_get_len(struct nexus_netif_adapter *nifna,
                                   mbuf_svc_class_t sc, uint32_t *packets,
  ...
  DTRACE_SKYWALK6(processed__mbuf__qlen, struct nexus_netif_adapter *, nifna,
                  struct nx_mbq *, q, uint32_t, qlen, size_t, qsize, uint32_t,
                  (packets != NULL) ? *packets : 0, uint32_t,
                  (bytes != NULL) ? *bytes : 0);
  return err;
}

static void fix_dequeue_mbuf_return_args(struct mbuf *m_chain,
 ...
 */
errno_t nx_netif_compat_tx_dequeue(struct nexus_netif_adapter *nifna,
...
#pragma unused(drvmgt)
   ...
   */
  m_chain = nx_netif_filter_tx_processed_mbuf_dequeue(nifna, sc, pkt_limit,
                                                      byte_limit);

  fix_dequeue_mbuf_return_args(m_chain, head, tail, cnt, len, orig_err, &err);
  return err;
}
 ...
 */
errno_t nx_netif_compat_tx_get_len(struct nexus_netif_adapter *nifna,
  ...
  return nx_netif_tx_processed_mbuf_get_len(nifna, sc, packets, bytes,
                                            orig_err);
}
