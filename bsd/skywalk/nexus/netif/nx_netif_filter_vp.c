...
static void netif_filter_dump_packet(struct __kern_packet *pkt) {
  uint8_t *baddr;
...
static errno_t netif_filter_deliver(struct nexus_adapter *na,
...
static errno_t netif_filter_rx_cb(void *arg, struct __kern_packet *pkt_chain,
                                  uint32_t flags) {
...
static errno_t netif_filter_tx_cb(void *arg, struct __kern_packet *pkt_chain,
                                  uint32_t flags) {
  struct nexus_adapter *__single na = arg;
  struct __kern_channel_ring *ring =
...
static errno_t netif_filter_cb(void *arg, struct __kern_packet *pkt_chain,
                               uint32_t flags) {
  errno_t err;
...
static int netif_filter_na_activate(struct nexus_adapter *na,
  ...
  if (mode == NA_ACTIVATE_MODE_ON) {
    err = nx_netif_filter_add(nifna->nifna_netif, na->na_nx_port, na,
                              netif_filter_cb, &nf);
    if (err != 0) {
      return err;
    }
    ...
    err = nx_netif_filter_remove(nifna->nifna_netif, nifna->nifna_filter);
    VERIFY(err == 0);
    nifna->nifna_filter = NULL;
    os_atomic_andnot(&na->na_flags, NAF_ACTIVE, relaxed);
  }

  SK_DF(SK_VERB_FILTER, "na \"%s\" (%p) %s", na->na_name, SK_KVA(na),
        na_activate_mode2str(mode));
  return 0;
}

static int netif_filter_na_krings_create(struct nexus_adapter *na,
                                         struct kern_channel *ch) {
  ASSERT(na->na_type == NA_NETIF_FILTER);
  return na_rings_mem_setup(na, FALSE, ch);
}

static void netif_filter_na_krings_delete(struct nexus_adapter *na,
                                          struct kern_channel *ch,
                                          boolean_t defunct) {
  ASSERT(na->na_type == NA_NETIF_FILTER);
...
static int netif_filter_region_params_setup(
    struct nexus_adapter *na, struct skmem_region_params srp[SKMEM_REGIONS]) {
  uint32_t max_mtu;
  ...
  srp[SKMEM_REGION_TXAKSD].srp_r_obj_size = MAX(nslots, afslots) * SLOT_DESC_SZ;
...
static int netif_filter_na_mem_new(struct nexus_adapter *na) {
  ...
  if (nif->nif_filter_pp == NULL) {
    ...
    filter_pp_name = tsnprintf(pp_name, sizeof(pp_name), "%s_netif_filter_pp",
                               if_name(nif->nif_ifp));
    if (srp[SKMEM_REGION_KMD].srp_max_frags > 1) {
      pp_flags |= PPCREATEF_ONDEMAND_BUF;
    }
    pp = pp_create(filter_pp_name, srp, NULL, NULL, NULL, NULL, NULL, pp_flags);
    if (pp == NULL) {
  ...
  na->na_arena = skmem_arena_create_for_nexus(na, srp, &nif->nif_filter_pp,
...
static int netif_filter_na_rxsync(struct __kern_channel_ring *kring,
...
static int netif_filter_na_txsync(struct __kern_channel_ring *kring,
                                  struct proc *p, uint32_t flags) {
#pragma unused(p, flags)
  kern_channel_slot_t last_slot = NULL, slot = NULL;
  ...
  iflags =
      (ringid == NETIF_FILTER_RING_INBOUND) ? NETIF_FILTER_RX : NETIF_FILTER_TX;
  ...
  DTRACE_SKYWALK4(injected, struct nexus_netif_adapter *, nifna,
                  struct __kern_channel_ring *, kring, int, cnt, uint32_t,
                  iflags);
  ...
  for (i = 0; i < KPKT_TC_MAX; i++) {
    ...
    err = nx_netif_filter_inject(dev_nifna, nifna->nifna_filter, q->fp_head,
                                 iflags);
    if (err != 0) {
      DTRACE_SKYWALK3(inject__failed, struct nexus_netif_adapter *, nifna,
...
static void netif_filter_na_dtor(struct nexus_adapter *na) {
  struct kern_nexus *nx = na->na_nx;
...
int netif_filter_na_create(struct kern_nexus *nx, struct chreq *chr,
  ...
  slots =
      MAX(NX_DOM(nx)->nxdom_tx_slots.nb_def, NX_DOM(nx)->nxdom_rx_slots.nb_def);

  na_set_nslots(na, NR_TX, filter_tx_slots != 0 ? filter_tx_slots : slots);
  na_set_nslots(na, NR_RX, filter_rx_slots != 0 ? filter_rx_slots : slots);
