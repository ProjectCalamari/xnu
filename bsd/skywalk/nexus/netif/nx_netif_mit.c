...
#define MIT_EWMA(old, new, gdecay, sdecay)                                     \
  do {                                                                         \
    uint32_t _avg;                                                             \
    if ((_avg = (old)) > 0) {                                                  \
...
SYSCTL_UINT(_kern_skywalk_netif, OID_AUTO, ad_mit_gdecay,
            CTLFLAG_RW | CTLFLAG_LOCKED, &netif_ad_mit_gdecay,
            NETIF_AD_MIT_GDECAY, "");
SYSCTL_UINT(_kern_skywalk_netif, OID_AUTO, ad_mit_sdecay,
            CTLFLAG_RW | CTLFLAG_LOCKED, &netif_ad_mit_sdecay,
            NETIF_AD_MIT_SDECAY, "");
SYSCTL_PROC(_kern_skywalk_netif, OID_AUTO, ad_mit_freeze,
            CTLTYPE_QUAD | CTLFLAG_RW | CTLFLAG_LOCKED,
            &netif_mit_mode_holdtime, NETIF_MIT_MODE_HOLDTIME,
            sysctl_mit_mode_holdtime, "Q", "");
#endif /* !DEVELOPMENT && !DEBUG */

void nx_netif_mit_init(struct nx_netif *nif, const struct ifnet *ifp,
...
#pragma unused(nif)
  ...
  static_assert(sizeof(mit_cfg_tbl_native_cellular) <=
  ...
  if (kr->ckr_tx == NR_TX) {
    if (simple) {
      (void)snprintf(mit->mit_name, sizeof(mit->mit_name), "skywalk_%s_tx_%u",
                     ifp->if_xname, kr->ckr_ring_id);
    } else {
      (void)snprintf(mit->mit_name, sizeof(mit->mit_name),
                     "skywalk_mit_%s_tx_%u", ifp->if_xname, kr->ckr_ring_id);
    }
    oid_name =
        tsnprintf(oid_name_buf, sizeof(oid_name_buf), "tx_%u", kr->ckr_ring_id);
  } else {
    if (simple) {
      (void)snprintf(mit->mit_name, sizeof(mit->mit_name), "skywalk_%s_rx_%u",
                     ifp->if_xname, kr->ckr_ring_id);
    } else {
      (void)snprintf(mit->mit_name, sizeof(mit->mit_name),
                     "skywalk_mit_%s_rx_%u", ifp->if_xname, kr->ckr_ring_id);
    }
    oid_name =
        tsnprintf(oid_name_buf, sizeof(oid_name_buf), "rx_%u", kr->ckr_ring_id);
  }
  ...
  if ((ifp->if_eflags & IFEF_SKYWALK_NATIVE) &&
    ...
    mit->mit_cfg_idx_max =
        sizeof(mit_cfg_tbl_native_cellular) / sizeof(*mit->mit_cfg);
  } else if (ifp->if_eflags & IFEF_SKYWALK_NATIVE) {
    bcopy(mit_cfg_tbl_native,
          (void *)__DECONST(struct mit_cfg_tbl *, mit->mit_tbl),
          sizeof(mit->mit_tbl));
    mit->mit_cfg_idx_max = sizeof(mit_cfg_tbl_native) / sizeof(*mit->mit_cfg);
  } else {
    bcopy(mit_cfg_tbl_compat,
          (void *)__DECONST(struct mit_cfg_tbl *, mit->mit_tbl),
          sizeof(mit->mit_tbl));
    mit->mit_cfg_idx_max = sizeof(mit_cfg_tbl_compat) / sizeof(*mit->mit_cfg);
  }
  ...
  if (ifp->if_rx_mit_ival != 0) {
    ...
    SK_D("mit interval updated: %s cfg %u ival %u", mit->mit_name, 0,
         mit->mit_tbl[0].cfg_ival);
  }
  ...
  if (kernel_thread_start(nx_netif_mit_thread_func, mit, &mit->mit_thread) !=
      KERN_SUCCESS) {
  ...
  info.importance = 0;
  kret =
      thread_policy_set(mit->mit_thread, THREAD_PRECEDENCE_POLICY,
  ...
  skoid_add_uint(&mit->mit_skoid, "interval", CTLFLAG_RW, &mit->mit_interval);
...
nx_netif_mit_update_interval(struct nx_netif_mit *mit, boolean_t reset) {
  ...
  if ((net_timercmp(&delta, &mit->mit_mode_holdtime, >)) || reset) {
    ...
    SK_DF(SK_VERB_NETIF_MIT, "%s: resetting [mode %u->%u]", mit->mit_name,
          mit->mit_mode, mode);
...
void nx_netif_mit_cleanup(struct nx_netif_mit *mit) {
  if (mit->mit_thread != THREAD_NULL) {
    ...
    (void)thread_wakeup_thread((caddr_t)&mit->mit_flags, mit->mit_thread);
    MIT_SPIN_UNLOCK(mit);
...
int nx_netif_mit_tx_intr(struct __kern_channel_ring *kr, struct proc *p,
                         uint32_t flags, uint32_t *work_done) {
  struct nexus_netif_adapter *nifna = (struct nexus_netif_adapter *)KRNA(kr);
  struct netif_stats *nifs = &NX_NETIF_PRIVATE(KRNA(kr)->na_nx)->nif_stats;
  ...
  if (__probable(nifna->nifna_tx_mit == NULL)) {
    ...
    struct nx_netif_mit *mit = &nifna->nifna_tx_mit[kr->ckr_ring_id];
    ASSERT(mit->mit_flags & NETIF_MITF_INITIALIZED);
    MIT_SPIN_LOCK(mit);
    mit->mit_requests++;
    if (!(mit->mit_flags & (NETIF_MITF_RUNNING | NETIF_MITF_TERMINATING |
                            NETIF_MITF_TERMINATED))) {
      (void)thread_wakeup_thread((caddr_t)&mit->mit_flags, mit->mit_thread);
    }
    MIT_SPIN_UNLOCK(mit);
  }
...
int nx_netif_mit_rx_intr(struct __kern_channel_ring *kr, struct proc *p,
                         uint32_t flags, uint32_t *work_done) {
  struct nexus_netif_adapter *nifna = (struct nexus_netif_adapter *)KRNA(kr);
  struct netif_stats *nifs = &NX_NETIF_PRIVATE(KRNA(kr)->na_nx)->nif_stats;
  ...
  if (__improbable(nifna->nifna_rx_mit != NULL)) {
    struct nx_netif_mit *mit = &nifna->nifna_rx_mit[kr->ckr_ring_id];
    ASSERT(mit->mit_flags & NETIF_MITF_INITIALIZED);
    MIT_SPIN_LOCK(mit);
    mit->mit_requests++;
    if (!(mit->mit_flags & (NETIF_MITF_RUNNING | NETIF_MITF_TERMINATING |
...
#pragma unused(w)
  ...
  thread_set_thread_name(
      current_thread(), __unsafe_null_terminated_from_indexable(mit->mit_name));
...
nx_netif_mit_s_thread_cont(void *v, wait_result_t wres) {
  ...
  irq_stat =
      (kr->ckr_tx == NR_TX) ? NETIF_STATS_TX_IRQ_MIT : NETIF_STATS_RX_IRQ_MIT;
  ...
  for (;;) {
     ...
     */
    if (error == EBUSY) {
      uint32_t ival = MAX(netif_busy_mit_delay, NETIF_BUSY_MIT_DELAY);
  ...
  if (__probable((mit->mit_flags & NETIF_MITF_TERMINATING) == 0)) {
    ...
    if (kr->ckr_rate_limited) {
      SK_DF(SK_VERB_NETIF_MIT, "%s: posting wait deadline for MIT",
            mit->mit_name);
      clock_interval_to_deadline(1, NSEC_PER_MSEC, &deadline);
    }
    mit->mit_flags &= ~NETIF_MITF_RUNNING;
    (void)assert_wait_deadline(&mit->mit_flags, THREAD_UNINT, deadline);
    MIT_SPIN_UNLOCK(mit);
    (void)thread_block_parameter(nx_netif_mit_s_thread_cont, mit);
    /* NOTREACHED */
  } else {
  terminate:
    ...
    mit->mit_flags &=
        ~(NETIF_MITF_READY | NETIF_MITF_RUNNING | NETIF_MITF_TERMINATING);
...
nx_netif_mit_thread_cont(void *v, wait_result_t wres) {
  ...
  for (;;) {
    ...
    if (ival != 0 || error == EBUSY) {
      if (error == EBUSY) {
        ival = MAX(netif_busy_mit_delay, NETIF_BUSY_MIT_DELAY);
        MIT_SPIN_LOCK(mit);
        mit->mit_requests++;
        MIT_SPIN_UNLOCK(mit);
      }
...
static void nx_netif_mit_stats(struct __kern_channel_ring *kr, uint64_t pkts,
                               uint64_t bytes) {
  ...
  if (net_timerisset(&mit->mit_sample_time)) {
    ...
    *(&mit->mit_sample_lasttime) = *(&now);
    ...
    MIT_EWMA(mit->mit_bytes_avg, btot, netif_ad_mit_gdecay,
             netif_ad_mit_sdecay);
    ...
    MIT_EWMA(mit->mit_packets_avg, ptot, netif_ad_mit_gdecay,
             netif_ad_mit_sdecay);
    ...
    SK_RDF(SK_VERB_NETIF_MIT, 2,
           "%s [%u]: pavg %u bavg %u "
           "delay %u usec",
           mit->mit_name, mit->mit_cfg_idx, mit->mit_packets_avg,
           mit->mit_bytes_avg,
    ...
    if (mode != mit->mit_mode || cfg_idx != mit->mit_cfg_idx) {
      ...
      SK_DF(SK_VERB_NETIF_MIT,
            ...
            mit->mit_name, mit->mit_cfg_idx, cfg_idx, mit->mit_packets_avg,
            mit->mit_bytes_avg, mit->mit_mode, mode,
            (mit->mit_mode == MIT_MODE_ADVANCED_STATIC
                 ? 0
