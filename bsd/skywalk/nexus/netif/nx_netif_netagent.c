...
SYSCTL_INT(_kern_skywalk_netif, OID_AUTO, netagent, CTLFLAG_RW | CTLFLAG_LOCKED,
           &nif_netagent, 0, "");
#endif /* !DEVELOPMENT && !DEBUG */

SK_NO_INLINE_ATTRIBUTE
static int get_mac_addr(struct nx_netif *nif, struct ether_addr *addr) {
 ...
 */
SK_NO_INLINE_ATTRIBUTE
static void get_ipv6_ula(struct in6_addr *addr) {
  uint8_t buf[16];
...
SK_NO_INLINE_ATTRIBUTE
static void get_ipv6_sockaddr(struct sockaddr_in6 *sin6) {
...
SK_NO_INLINE_ATTRIBUTE
static int validate_ipv6_sockaddr(struct sockaddr_in6 *sin6) {
...
SK_NO_INLINE_ATTRIBUTE
static boolean_t flow_ipv6_ula_match(struct netif_agent_flow *naf,
                                     struct nx_flow_req *nfr) {
  struct in6_addr *s1, *s2, *d1, *d2;

  if (naf->naf_pid != nfr->nfr_pid) {
    DTRACE_SKYWALK2(pid__mismatch, pid_t, naf->naf_pid, pid_t, nfr->nfr_pid);
    return FALSE;
  }
  ...
  if (!IN6_ARE_ADDR_EQUAL(s1, s2)) {
    DTRACE_SKYWALK2(saddr__mismatch, struct in6_addr *, s1, struct in6_addr *,
                    s2);
    return FALSE;
  }
  ...
  if (!IN6_ARE_ADDR_EQUAL(d1, d2)) {
    DTRACE_SKYWALK2(daddr__mismatch, struct in6_addr *, d1, struct in6_addr *,
                    d2);
    return FALSE;
  }
  return TRUE;
...
static uint16_t forbidden_ethertypes[] = {
    ETHERTYPE_IP,   ETHERTYPE_ARP, ETHERTYPE_REVARP,      ETHERTYPE_VLAN,
    ETHERTYPE_IPV6, ETHERTYPE_PAE, ETHERTYPE_RSN_PREAUTH,
};
#define FORBIDDEN_ETHERTYPES                                                   \
  (sizeof(forbidden_ethertypes) / sizeof(forbidden_ethertypes[0]))

SK_NO_INLINE_ATTRIBUTE
static int validate_ethertype(uint16_t ethertype) {
  uint32_t i;
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_fill_port_info(struct nx_netif *nif,
                                            struct nx_flow_req *nfr,
...
#pragma unused(nif)
  ...
  if ((nfr->nfr_flags & (NXFLOWREQF_CUSTOM_ETHER | NXFLOWREQF_IPV6_ULA)) == 0) {
    return 0;
  }
  npi =
      sk_alloc_data(sizeof(*npi), Z_WAITOK | Z_NOFAIL, skmem_tag_nx_port_info);
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_flow_bind(struct nx_netif *nif,
                                       struct nx_flow_req *nfr) {
  uuid_t uuid_key;
  ...
  nxb.nxb_key =
      sk_alloc_data(sizeof(uuid_key), Z_WAITOK | Z_NOFAIL, skmem_tag_nx_key);
  ...
  if (err != 0) {
    ...
    SK_ERR("%s(%d) failed to bind flow_uuid %s to a "
           "nx_port (err %d)",
           sk_proc_name(p), pid, sk_uuid_unparse(nfr->nfr_flow_uuid, uuidstr),
           err);
    return err;
  }
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_flow_unbind(struct nx_netif *nif,
                                         struct nx_flow_req *nfr) {
  int err;
  struct kern_nexus *nx = nif->nif_nx;
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_check_flags(struct nx_netif *nif,
                                         struct nx_flow_req *nfr,
                                         boolean_t add) {
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_listener_flow_add(struct nx_netif *nif,
                                               struct nx_flow_req *nfr) {
  int err;
 ...
 */
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_flow_find(struct nx_netif *nif,
                                       struct nx_flow_req *nfr) {
...
SK_NO_INLINE_ATTRIBUTE
static void nx_netif_netagent_fill_flow_info(struct netif_agent_flow *naf,
                                             struct nx_flow_req *nfr) {
...
int nx_netif_netagent_flow_add(struct nx_netif *nif, struct nx_flow_req *nfr) {
  int err;
  struct netif_agent_flow *naf;
  struct netif_stats *nifs = &nif->nif_stats;
  ...
  if (err == 0) {
    ...
    DTRACE_SKYWALK2(found__flow, struct nx_netif *, nif, struct nx_flow_req *,
                    nfr);
  ...
  if (err != 0) {
    ...
    DTRACE_SKYWALK3(bind__failed, struct nx_netif *, nif, struct nx_flow_req *,
                    nfr, int, err);
    sk_free_type(struct netif_agent_flow, naf);
    lck_mtx_unlock(&nif->nif_agent_lock);
    return err;
  ...
  if ((nfr->nfr_flags & NXFLOWREQF_IPV6_ULA) != 0) {
    ...
    SK_DF(SK_VERB_NETIF, "IPv6 local: %s",
          sk_ntop(AF_INET6, &nfr->nfr_saddr.sin6.sin6_addr, local,
                  sizeof(local)));
    SK_DF(SK_VERB_NETIF, "IPv6 remote: %s",
          sk_ntop(AF_INET6, &nfr->nfr_daddr.sin6.sin6_addr, remote,
                  sizeof(remote)));
  }
#endif /* SK_LOG */
  lck_mtx_unlock(&nif->nif_agent_lock);
...
int nx_netif_netagent_flow_del(struct nx_netif *nif, struct nx_flow_req *nfr) {
  int err;
  ...
  /* no-op for listener */
  if ((nfr->nfr_flags & NXFLOWREQF_LISTENER) != 0) {
    DTRACE_SKYWALK2(listener, struct nx_netif *, nif, struct nx_flow_req *,
                    nfr);
    return 0;
  }
  lck_mtx_lock(&nif->nif_agent_lock);
  ...
  SLIST_REMOVE(&nif->nif_agent_flow_list, naf, netif_agent_flow, naf_link);
  sk_free_type(struct netif_agent_flow, naf);
  nif->nif_agent_flow_cnt--;
  lck_mtx_unlock(&nif->nif_agent_lock);
  return 0;
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_agent_flow_purge(struct nx_netif *nif) {
  ...
  DTRACE_SKYWALK2(agent__flows__purge, struct nx_netif *, nif, uint32_t, cnt);
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_handle_interpose_flow_add(
    struct nx_netif *nif, uuid_t flow_uuid, pid_t pid,
    struct necp_client_nexus_parameters *cparams,
...
#pragma unused(cparams)
  ...
  message = necp_create_nexus_assign_message(
      ...
      sizeof(nfr.nfr_bind_key), NULL, NULL, NULL, 0, NULL, 0, &len);
  if (message == NULL) {
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_handle_custom_ether_flow_add(
    struct nx_netif *nif, uuid_t flow_uuid, pid_t pid,
    struct necp_client_nexus_parameters *cparams,
    void *__sized_by(*results_length) * results, size_t *results_length) {
  int err;
  ...
  message = necp_create_nexus_assign_message(
      nif->nif_nx->nx_uuid, nfr.nfr_nx_port, nfr.nfr_bind_key,
      sizeof(nfr.nfr_bind_key), NULL, NULL, &nfr.nfr_etheraddr, 0, NULL, 0,
      &len);
  if (message == NULL) {
    (void)nx_netif_netagent_flow_del(nif, &nfr);
    return ENOMEM;
...
#define IS_V6_ADDR(addr) ((addr)->sin6.sin6_family == AF_INET6)

SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_handle_ipv6_ula_flow_add(
    struct nx_netif *nif, uuid_t flow_uuid, pid_t pid,
    struct necp_client_nexus_parameters *cparams,
    void *__sized_by(*results_length) * results, size_t *results_length) {
  int err;
  ...
  if (cparams->is_listener) {
    ...
    if (IS_V6_ADDR(&cparams->local_addr)) {
      bcopy(&cparams->local_addr, &nfr.nfr_saddr, sizeof(nfr.nfr_saddr));
    }
    if (IS_V6_ADDR(&cparams->remote_addr)) {
      bcopy(&cparams->remote_addr, &nfr.nfr_daddr, sizeof(nfr.nfr_daddr));
    }
  ...
  if (cparams->is_listener) {
    ...
    message = necp_create_nexus_assign_message(
        zero_nx_uuid, NEXUS_PORT_ANY, NULL, 0, &local_endpoint, NULL,
    ...
    message = necp_create_nexus_assign_message(
        nif->nif_nx->nx_uuid, nfr.nfr_nx_port, nfr.nfr_bind_key,
        sizeof(nfr.nfr_bind_key), &local_endpoint, &remote_endpoint,
        &nfr.nfr_etheraddr, 0, NULL, 0, &len);
  }
  if (message == NULL) {
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_handle_flow_add(
    struct nx_netif *nif, uuid_t flow_uuid, pid_t pid,
    struct necp_client_nexus_parameters *cparams,
    void *__sized_by(*results_length) * results, size_t *results_length) {
  int err = 0;
  ...
  if (cparams->is_interpose) {
    err = nx_netif_netagent_handle_interpose_flow_add(
        nif, flow_uuid, pid, cparams, results, results_length);
  } else if (cparams->is_custom_ether) {
    err = nx_netif_netagent_handle_custom_ether_flow_add(
        nif, flow_uuid, pid, cparams, results, results_length);
  } else if (NETIF_IS_LOW_LATENCY(nif)) {
    err = nx_netif_netagent_handle_ipv6_ula_flow_add(
        nif, flow_uuid, pid, cparams, results, results_length);
  }
  if (err != 0) {
    ASSERT(*results == NULL);
...
SK_NO_INLINE_ATTRIBUTE
static int nx_netif_netagent_handle_flow_del(struct nx_netif *nif,
                                             uuid_t flow_uuid, pid_t pid,
                                             boolean_t abort) {
#pragma unused(abort)
  struct nx_flow_req nfr;
...
static int nx_netif_netagent_event(u_int8_t event, uuid_t flow_uuid, pid_t pid,
                                   void *context, void *ctx,
                                   struct necp_client_agent_parameters *cparams,
                                   void *__sized_by(*results_length) * results,
  ...
  case NETAGENT_EVENT_NEXUS_FLOW_INSERT:
    ...
    *results_length = 0;
    err = nx_netif_netagent_handle_flow_add(nif, flow_uuid, pid,
                                            &cparams->u.nexus_request, results,
                                            results_length);
    break;

  case NETAGENT_EVENT_NEXUS_FLOW_REMOVE:
  case NETAGENT_EVENT_NEXUS_FLOW_ABORT:
    err = nx_netif_netagent_handle_flow_del(
        nif, flow_uuid, pid, (event == NETAGENT_EVENT_NEXUS_FLOW_REMOVE));
    break;
...
static int nx_netif_agent_register(struct nx_netif *nif, uint32_t features) {
  ...
  (void)snprintf(agent.agent.netagent_type, sizeof(agent.agent.netagent_type),
                 "%s", "NetIf");
  (void)snprintf(agent.agent.netagent_desc, sizeof(agent.agent.netagent_desc),
                 "%s", "Userspace Networking");
  ...
  if (NETIF_IS_LOW_LATENCY(nif)) {
    agent.nexus_data.nexus_flags |= NETAGENT_NEXUS_FLAG_SHOULD_USE_EVENT_RING;
  }
...
static void nx_netif_agent_unregister(struct nx_netif *nif) {
...
static uint32_t nx_netif_agent_get_features(struct nx_netif *nif) {
  uint32_t features = 0;

  if ((nif->nif_filter_flags & NETIF_FILTER_FLAG_INITIALIZED) != 0) {
    features |=
        (NETAGENT_FLAG_INTERPOSE_NEXUS | NETAGENT_FLAG_NETWORK_PROVIDER);
  }
  if ((nif->nif_flow_flags & NETIF_FLOW_FLAG_INITIALIZED) != 0) {
    if (NETIF_IS_LOW_LATENCY(nif)) {
...
void nx_netif_agent_init(struct nx_netif *nif) {
  int err;
  ...
  if (err != 0) {
    SK_DF(SK_VERB_ERROR, "%s: agent register failed: err %d", if_name(ifp),
          err);
    return;
  }
  ...
  if (err != 0) {
    ...
    SK_DF(SK_VERB_ERROR, "%s: agent add failed: err %d", if_name(ifp), err);
    return;
  }
...
void nx_netif_agent_fini(struct nx_netif *nif) {
  ifnet_t ifp = nif->nif_ifp;
