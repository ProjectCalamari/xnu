...
nx_netif_qset_create(struct netif_llink *, uint8_t,
                     struct kern_nexus_netif_llink_qset_init *);
...
static void nx_netif_driver_queue_init(struct netif_qset *,
                                       struct netif_queue *,
                                       kern_packet_svc_class_t, bool);
static struct netif_llink *
nx_netif_llink_create_locked(struct nx_netif *,
                             struct kern_nexus_netif_llink_init *);
...
void nx_netif_llink_retain(struct netif_llink *llink) {
  os_ref_retain(&llink->nll_refcnt);
}

SK_NO_INLINE_ATTRIBUTE
static void nx_netif_llink_free(struct netif_llink **pllink) {
  ...
  *pllink = NULL;
  SLIST_FOREACH_SAFE(qset, &llink->nll_qset_list, nqs_list, tqset) {
    SLIST_REMOVE(&llink->nll_qset_list, qset, netif_qset, nqs_list);
...
SK_NO_INLINE_ATTRIBUTE
void nx_netif_llink_release(struct netif_llink **pllink) {
...
/* retains a reference for the callee */
static struct netif_qset *nx_netif_qset_alloc(uint8_t nrxqs, uint8_t ntxqs) {
...
SK_NO_INLINE_ATTRIBUTE
void nx_netif_qset_retain(struct netif_qset *qset) {
  /*
...
SK_NO_INLINE_ATTRIBUTE
void nx_netif_qset_release(struct netif_qset **pqset) {
...
SK_NO_INLINE_ATTRIBUTE
static void nx_netif_qset_free(struct netif_qset **pqset) {
  struct netif_qset *qset = *pqset;
  uint8_t i;
...
SK_NO_INLINE_ATTRIBUTE
static void nx_netif_qset_destroy(struct netif_qset *qset) {
  VERIFY(qset->nqs_llink->nll_state == NETIF_LLINK_STATE_DESTROYED);
  nx_netif_qset_free(&qset);
}

SK_NO_INLINE_ATTRIBUTE
static void nx_netif_qset_setup_ifclassq(struct netif_llink *llink,
                                         struct netif_qset *qset) {
  uint8_t flags = 0;
...
SK_NO_INLINE_ATTRIBUTE
static void nx_netif_qset_teardown_ifclassq(struct netif_qset *qset) {
  ASSERT((qset->nqs_flags & NETIF_QSET_FLAG_AQM) != 0);
...
#define _NETIF_QSET_MAX_TXQS 4
  kern_packet_svc_class_t svc[_NETIF_QSET_MAX_TXQS] = {KPKT_SC_BE, KPKT_SC_BK,
                                                       KPKT_SC_VI, KPKT_SC_VO};
  ...
  if (ifp->if_output_sched_model & IFNET_SCHED_DRIVER_MANGED_MODELS) {
    ...
    for (i = 0; i < qset->nqs_num_tx_queues; i++) {
      nx_netif_driver_queue_init(qset, NETIF_QSET_TX_QUEUE(qset, i), svc[i],
                                 false);
    }
  } else {
    for (i = 0; i < qset->nqs_num_tx_queues; i++) {
      nx_netif_driver_queue_init(qset, NETIF_QSET_TX_QUEUE(qset, i),
...
nx_netif_qset_create(struct netif_llink *llink, uint8_t idx,
  ...
  qset =
      nx_netif_qset_alloc(qset_init->nlqi_num_rxqs, qset_init->nlqi_num_txqs);
  nx_netif_qset_init(qset, llink, idx, qset_init);
  return qset;
}

static uint16_t nx_netif_generate_internal_llink_id(struct nx_netif *nif) {
  struct netif_llink *llink;
...
nx_netif_llink_initialize(struct netif_llink *llink, struct nx_netif *nif,
  ...
  for (i = 0; i < llink_init->nli_num_qsets; i++) {
    if (llink->nll_ifcq == NULL &&
        (llink_init->nli_qsets[i].nlqi_flags & KERN_NEXUS_NET_LLINK_QSET_AQM)) {
    ...
    struct netif_qset *qset =
        nx_netif_qset_create(llink, i, &llink_init->nli_qsets[i]);
...
static void nx_netif_driver_queue_destroy(struct netif_queue *drvq) {
  VERIFY(drvq->nq_qset->nqs_llink->nll_state == NETIF_LLINK_STATE_DESTROYED);
...
static void nx_netif_driver_queue_init(struct netif_qset *qset,
                                       struct netif_queue *drvq,
                                       kern_packet_svc_class_t svc,
                                       bool is_rx) {
  lck_mtx_init(&drvq->nq_lock, &netif_llink_lock_group, &netif_llink_lock_attr);
...
SK_NO_INLINE_ATTRIBUTE
static void nx_netif_llink_destroy_locked(struct nx_netif *nif,
                                          struct netif_llink **pllink) {
  struct netif_stats *nifs = &nif->nif_stats;
...
int nx_netif_llink_add(struct nx_netif *nif,
                       struct kern_nexus_netif_llink_init *llink_init,
                       struct netif_llink **pllink) {
  int err;
  ...
  *pllink = NULL;
  ...
  STAILQ_FOREACH(llink, &nif->nif_llink_list, nll_link) {
    if (llink->nll_link_id == llink_init->nli_link_id) {
      SK_ERR("duplicate llink_id 0x%llu", llink_init->nli_link_id);
      STATS_INC(nifs, NETIF_STATS_LLINK_DUP_ID_GIVEN);
      DTRACE_SKYWALK1(dup__id__given, uint64_t, llink_init->nli_link_id);
...
int nx_netif_llink_remove(struct nx_netif *nif,
...
static void nx_netif_default_llink_add(struct nx_netif *nif) {
  ...
  if (NX_LLINK_PROV(nif->nif_nx)) {
    ...
    if (NETIF_IS_LOW_LATENCY(nif)) {
      qset.nlqi_flags |= KERN_NEXUS_NET_LLINK_QSET_LOW_LATENCY;
    } else {
      qset.nlqi_flags |= KERN_NEXUS_NET_LLINK_QSET_AQM;
    }
    qset.nlqi_num_rxqs = (uint8_t)na_get_nrings(devna, NR_RX);
    qset.nlqi_num_txqs = (uint8_t)na_get_nrings(devna, NR_TX);
...
static void nx_netif_default_llink_remove(struct nx_netif *nif) {
  struct netif_llink *__single llink;
...
int netif_qset_enqueue(struct netif_qset *qset, bool chain,
                       struct __kern_packet *pkt_chain,
                       struct __kern_packet *tail, uint32_t cnt, uint32_t bytes,
  ...
  if (chain) {
    ...
    netif_ifp_inc_traffic_class_out_pkt(ifp, pkt_chain->pkt_svc_class, cnt,
                                        bytes);
    ...
    while (pkt != NULL) {
      ...
      netif_ifp_inc_traffic_class_out_pkt(ifp, pkt->pkt_svc_class, 1,
                                          pkt->pkt_length);
...
struct netif_qset *nx_netif_get_default_qset_noref(struct nx_netif *nif) {
  struct netif_qset *qset;
  struct netif_stats *nifs = &nif->nif_stats;
...
static void nx_netif_qset_hint_decode(uint64_t hint, uint16_t *link_id_internal,
                                      uint16_t *qset_idx) {
...
static struct netif_qset *nx_netif_get_default_qset(struct nx_netif *nif) {
  struct netif_qset *qset;
 ...
 */
struct netif_qset *nx_netif_find_qset(struct nx_netif *nif, uint64_t hint) {
  ...
  if (!NX_LLINK_PROV(nif->nif_nx) || (nx_netif_random_qset == 0 && hint == 0)) {
    goto def_qset;
  }
  ...
  random_id = random();
  STAILQ_FOREACH(llink, &nif->nif_llink_list, nll_link) {
    if (nx_netif_random_qset != 0 && (random_id % nif->nif_llink_cnt) == i) {
      break;
  ...
  SLIST_FOREACH(qset, &llink->nll_qset_list, nqs_list) {
    if (nx_netif_random_qset != 0 && (random_id % llink->nll_qset_cnt) == j) {
      break;
  ...
  if (nx_netif_random_qset != 0) {
    SK_DF(SK_VERB_LLINK,
          "%s: random qset: qset %p, ifcq %p, "
          "llink_idx %d, qset_idx %d",
          if_name(nif->nif_ifp), qset, qset->nqs_ifcq, i, j);

    DTRACE_SKYWALK5(random__qset, struct nx_netif *, nif, struct netif_qset *,
                    qset, struct ifclassq *, qset->nqs_ifcq, int, i, int, j);
  } else {
    SK_DF(SK_VERB_LLINK,
          "%s: non-default qset: qset %p, ifcq %p, "
          " ll_id_internal 0x%x, qset_idx %d",
          if_name(nif->nif_ifp), qset, qset->nqs_ifcq, ll_id_internal,
          qset_idx);

    DTRACE_SKYWALK5(nondef__qset, struct nx_netif *, nif, struct netif_qset *,
                    qset, struct ifclassq *, qset->nqs_ifcq, uint16_t,
                    ll_id_internal, uint16_t, qset_idx);
  }
  return qset;
...
def_qset:
  ...
  DTRACE_SKYWALK4(def__qset, struct nx_netif *, nif, struct netif_qset *, qset,
                  struct ifclassq *, qset->nqs_ifcq, uint64_t, hint);
  return qset;
}

struct netif_qset *nx_netif_find_qset_with_pkt(struct ifnet *ifp,
...
void nx_netif_llink_init(struct nx_netif *nif) {
  ifnet_t ifp = nif->nif_ifp;

#if (DEVELOPMENT || DEBUG)
  if (__improbable(nx_netif_disable_llink != 0)) {
    SK_DF(SK_VERB_LLINK, "%s: llink is disabled", if_name(nif->nif_ifp));
    return;
  }
#endif /* (DEVELOPMENT || DEBUG) */

  if (!SKYWALK_NATIVE(ifp)) {
    SK_DF(SK_VERB_LLINK, "%s: llink is supported on native devices only",
          if_name(ifp));
    return;
  }
  ASSERT(!NETIF_LLINK_ENABLED(nif));
...
void nx_netif_llink_fini(struct nx_netif *nif) {
  if (!NETIF_LLINK_ENABLED(nif)) {
    SK_DF(SK_VERB_LLINK, "%s: llink not initialized", if_name(nif->nif_ifp));
    return;
  }
...
int nx_netif_validate_llink_config(struct kern_nexus_netif_llink_init *init,
                                   bool default_llink) {
  ...
  default_llink_flag = ((init->nli_flags & KERN_NEXUS_NET_LLINK_DEFAULT) != 0);
  ...
  for (i = 0; i < init->nli_num_qsets; i++) {
    if (qsinit[i].nlqi_flags & KERN_NEXUS_NET_LLINK_QSET_DEFAULT) {
      if (has_default_qset) {
    ...
    if ((qsinit[i].nlqi_flags & KERN_NEXUS_NET_LLINK_QSET_WMM_MODE) &&
...
int nx_netif_default_llink_config(struct nx_netif *nif,
  ...
  nif->nif_default_llink_params =
      sk_alloc_type(struct kern_nexus_netif_llink_init, Z_WAITOK | Z_NOFAIL,
                    nx_netif_tag_llink_cfg);

  qsinit = sk_alloc_type_array(struct kern_nexus_netif_llink_qset_init,
                               init->nli_num_qsets, Z_WAITOK,
                               nx_netif_tag_llink_cfg);
  if (qsinit == NULL) {
  ...
  memcpy(nif->nif_default_llink_params, init,
         __builtin_offsetof(struct kern_nexus_netif_llink_init, nli_qsets));
  for (i = 0; i < init->nli_num_qsets; i++) {
...
void nx_netif_llink_config_free(struct nx_netif *nif) {
...
static int nx_netif_llink_ext_init_queues(struct kern_nexus *nx,
                                          struct netif_llink *llink) {
  ...
  SLIST_FOREACH(qset, &llink->nll_qset_list, nqs_list) {
    ...
    err = nxnpi->nxnpi_qset_init(nxprov, nx, llink->nll_ctx, qset->nqs_idx,
                                 qset->nqs_id, qset, &qset->nqs_ctx);
    if (err != 0) {
      STATS_INC(nifs, NETIF_STATS_LLINK_QSET_INIT_FAIL);
      SK_ERR("nx: %p, qset: %d, qset init err %d", SK_KVA(nx), qset->nqs_idx,
             err);
      goto out;
    }
    ...
    for (i = 0; i < qset->nqs_num_rx_queues; i++) {
      ...
      err = nxnpi->nxnpi_queue_init(nxprov, nx, qset->nqs_ctx, i, false, drvq,
                                    &drvq->nq_ctx);
      if (err != 0) {
        STATS_INC(nifs, NETIF_STATS_LLINK_RXQ_INIT_FAIL);
        SK_ERR("nx: %p qset: %d queue_init err %d", SK_KVA(nx), qset->nqs_idx,
               err);
        goto out;
      }
    ...
    for (i = 0; i < qset->nqs_num_tx_queues; i++) {
      ...
      ASSERT((drvq->nq_flags & NETIF_QUEUE_EXT_INITED) == 0);
      err = nxnpi->nxnpi_queue_init(nxprov, nx, qset->nqs_ctx, i, true, drvq,
                                    &drvq->nq_ctx);
      if (err != 0) {
...
static void nx_netif_llink_ext_fini_queues(struct kern_nexus *nx,
                                           struct netif_llink *llink) {
  struct kern_nexus_provider *nxprov = NX_PROV(nx);
  struct kern_nexus_netif_provider_init *nxnpi;
  struct netif_qset *qset;
...
int nx_netif_llink_ext_init_default_queues(struct kern_nexus *nx) {
  struct nx_netif *nif = NX_NETIF_PRIVATE(nx);
  return nx_netif_llink_ext_init_queues(nx, nif->nif_default_llink);
}

void nx_netif_llink_ext_fini_default_queues(struct kern_nexus *nx) {
  struct nx_netif *nif = NX_NETIF_PRIVATE(nx);
