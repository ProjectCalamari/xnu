...
static int netif_flow_get_buf_pkt(struct __kern_packet *pkt, size_t minlen,
...
static int netif_flow_get_buf_mbuf(struct mbuf *m, size_t minlen,
                                   uint8_t *__sized_by(*len) * buf,
                                   uint32_t *len) {
  /*
   * XXX
...
static int netif_flow_get_buf(struct __kern_packet *pkt, size_t minlen,
...
static int netif_flow_ethertype_info(struct __kern_packet *pkt,
...
#pragma unused(flags)
  ...
  err = netif_flow_get_buf(pkt, sizeof(ether_header_t), &buf, &len);
  if (err != 0) {
    DTRACE_SKYWALK2(get__buf__failed, struct __kern_packet *, pkt, int, err);
    return err;
  }
  ...
  if (kern_packet_get_vlan_tag(SK_PKT2PH(pkt), &tag) == 0) {
    DTRACE_SKYWALK2(hw__vlan, struct __kern_packet *, pkt, uint16_t, tag);
  } else if (etype == ETHERTYPE_VLAN) {
    struct ether_vlan_header *evh;

    DTRACE_SKYWALK2(encap__vlan, struct __kern_packet *, pkt, uint8_t *, buf);
    if ((pkt->pkt_pflags & PKT_F_MBUF_DATA) != 0) {
      struct mbuf *m = pkt->pkt_mbuf;

      if (mbuf_len(m) < sizeof(*evh)) {
        DTRACE_SKYWALK1(mbuf__too__small, struct mbuf *, m);
        return EINVAL;
      }
    } else {
      if (len < sizeof(*evh)) {
        DTRACE_SKYWALK2(pkt__too__small, struct __kern_packet *, pkt, uint32_t,
                        len);
        return EINVAL;
      }
    }
  ...
  if (EVL_VLANOFTAG(tag) != 0) {
    DTRACE_SKYWALK2(vlan__non__zero, struct __kern_packet *, pkt, uint16_t,
                    tag);
    return ENOTSUP;
  }
  DTRACE_SKYWALK4(extracted__info, struct __kern_packet *, pkt, uint8_t *, buf,
                  uint16_t, tag, uint16_t, etype);
  fd->fd_ethertype = etype;
  return 0;
}

static boolean_t netif_flow_ethertype_match(struct netif_flow_desc *fd1,
                                            struct netif_flow_desc *fd2) {
  return fd1->fd_ethertype == fd2->fd_ethertype;
}

static int netif_flow_ipv6_ula_info(struct __kern_packet *pkt,
                                    struct netif_flow_desc *fd,
                                    uint32_t flags) {
  ether_header_t *eh;
  uint32_t len;
  ...
  err = netif_flow_get_buf(pkt, sizeof(*eh) + sizeof(*ip6h), &buf, &len);
  if (err != 0) {
    DTRACE_SKYWALK2(get__buf__failed, struct __kern_packet *, pkt, int, err);
    return err;
  }
...
static boolean_t netif_flow_ipv6_ula_match(struct netif_flow_desc *fd1,
                                           struct netif_flow_desc *fd2) {
  return IN6_ARE_ADDR_EQUAL(&fd1->fd_laddr, &fd2->fd_laddr) &&
         IN6_ARE_ADDR_EQUAL(&fd1->fd_raddr, &fd2->fd_raddr);
}

static int netif_flow_list_lookup(struct netif_flowtable *ft,
...
static int netif_flow_list_insert(struct netif_flowtable *ft,
                                  struct netif_flow *f) {
  struct netif_list_flowtable *__single lft = ft->ft_internal;
  struct netif_flow *nf;
...
static void netif_flow_list_remove(struct netif_flowtable *ft,
                                   struct netif_flow *f) {
  struct netif_list_flowtable *__single lft = ft->ft_internal;
...
netif_flow_list_table_alloc(struct netif_flowtable_ops *ops) {
  ...
  ft = skn_alloc_type(flowtable, struct netif_flowtable, Z_WAITOK | Z_NOFAIL,
...
static void netif_flow_list_table_free(struct netif_flowtable *ft) {
...
static void nx_netif_flow_deliver(struct nx_netif *nif, struct netif_flow *f,
                                  void *data, uint32_t flags) {
#pragma unused(nif)
  f->nf_cb_func(f->nf_cb_arg, data, flags);
}

void nx_netif_snoop(struct nx_netif *nif, struct __kern_packet *pkt,
                    boolean_t inbound) {
  ...
  if (inbound) {
    pktap_input_packet(nif->nif_ifp, AF_INET6, DLT_EN10MB, -1, NULL, -1, NULL,
                       SK_PKT2PH(pkt), NULL, 0, 0, 0, PTH_FLAG_NEXUS_CHAN);
  } else {
    pktap_output_packet(nif->nif_ifp, AF_INET6, DLT_EN10MB, -1, NULL, -1, NULL,
                        SK_PKT2PH(pkt), NULL, 0, 0, 0, PTH_FLAG_NEXUS_CHAN);
  }
}
 ...
 */
boolean_t nx_netif_validate_macaddr(struct nx_netif *nif,
                                    struct __kern_packet *pkt, uint32_t flags) {
  ...
  DTRACE_SKYWALK4(dump__buf, struct nx_netif *, nif, struct __kern_packet *,
                  pkt, void *, buf, uint32_t, len);
  ...
  if (NETIF_IS_LOW_LATENCY(nif)) {
    ...
    if (mbcast) {
      DTRACE_SKYWALK4(mbcast__pkt__llw, struct nx_netif *, nif,
                      struct __kern_packet *, pkt, void *, buf, uint32_t, len);
      goto done;
    }
    /* only validate macaddr for outbound packets */
    if (!outbound) {
      DTRACE_SKYWALK4(skip__check__llw, struct nx_netif *, nif,
                      struct __kern_packet *, pkt, void *, buf, uint32_t, len);
      return TRUE;
    }
  } else {
    if (mbcast) {
      if (outbound) {
        ...
        DTRACE_SKYWALK4(mbcast__src, struct nx_netif *, nif,
                        struct __kern_packet *, pkt, void *, buf, uint32_t,
                        len);
        goto done;
      } else {
        /* allow multicast/broadcast as dest macaddr */
        DTRACE_SKYWALK4(mbcast__dest, struct nx_netif *, nif,
                        struct __kern_packet *, pkt, void *, buf, uint32_t,
                        len);
        return TRUE;
      }
  ...
  if (ifnet_lladdr_copy_bytes(ifp, local_addr, sizeof(local_addr)) != 0) {
    ...
    DTRACE_SKYWALK2(bad__addr__len, struct nx_netif *, nif, struct ifnet *,
                    ifp);
    return FALSE;
  }
  ...
  if (!valid) {
    ...
    DTRACE_SKYWALK2(bad__addr, struct nx_netif *, nif, struct __kern_packet *,
                    pkt);
  }
  return valid;
}
 ...
 */
boolean_t nx_netif_flow_match(struct nx_netif *nif, struct __kern_packet *pkt,
  ...
  if (!match) {
    ...
    DTRACE_SKYWALK3(not__match, struct nx_netif *, nif, struct netif_flow *, f,
                    struct __kern_packet *, pkt);
  }
  return match;
}

struct netif_flow *nx_netif_flow_classify(struct nx_netif *nif,
                                          struct __kern_packet *pkt,
                                          uint32_t flags) {
  struct netif_stats *nifs = &nif->nif_stats;
...
void nx_netif_flow_release(struct nx_netif *nif, struct netif_flow *nf) {
  lck_mtx_lock(&nif->nif_flow_lock);
  if (--nf->nf_refcnt == 0) {
    wakeup(&nf->nf_refcnt);
  }
...
errno_t nx_netif_demux(struct nexus_netif_adapter *nifna,
                       struct __kern_packet *pkt_chain,
                       struct __kern_packet **remain,
  ...
  while (pkt != NULL) {
    ...
    ASSERT((pkt->pkt_pflags & PKT_F_PKT_DATA) == 0);
    plen = ((pkt->pkt_pflags & PKT_F_MBUF_DATA) != 0) ? m_pktlen(pkt->pkt_mbuf)
                                                      : pkt->pkt_length;
    ...
    if (nf != NULL) {
      ...
      if (prev_nf != NULL) {
        if (prev_nf != nf) {
          DTRACE_SKYWALK5(deliver, struct nx_netif *, nif, struct netif_flow *,
                          prev_nf, struct __kern_packet *, head, int, c,
                          uint32_t, flags);
  ...
  if (head != NULL) {
    ASSERT(prev_nf != NULL);
    DTRACE_SKYWALK5(deliver__last, struct nx_netif *, nif, struct netif_flow *,
                    prev_nf, struct __kern_packet *, pkt, int, c, uint32_t,
                    flags);
  ...
  DTRACE_SKYWALK5(demux__delivered, struct nx_netif *, nif, int, delivered, int,
                  r, int, bytes, int, rbytes);
  return 0;
}

SK_NO_INLINE_ATTRIBUTE
static errno_t nx_netif_flowtable_init(struct nx_netif *nif,
                                       netif_flowtable_type_t type) {
  struct netif_flowtable *ft;
  struct netif_flowtable_ops *fops;
...
SK_NO_INLINE_ATTRIBUTE
static void nx_netif_flowtable_fini(struct nx_netif *nif) {
 ...
 */
static inline void nx_netif_flow_log(struct nx_netif *nif,
                                     struct netif_flow *nf, boolean_t add) {
  int i;
  struct netif_stats *nifs = &nif->nif_stats;

  os_log(OS_LOG_DEFAULT,
         "netif flowstats (%s): if %s, nx_port %d, "
         "ethertype 0x%x, src %s, dst %s",
         add ? "add" : "remove", if_name(nif->nif_ifp), nf->nf_port,
         nf->nf_desc.fd_ethertype, ip6_sprintf(&nf->nf_desc.fd_laddr),
         ip6_sprintf(&nf->nf_desc.fd_raddr));
  for (i = 0; i < __NETIF_STATS_MAX; i++) {
    if (STATS_VAL(nifs, i) == 0) {
      continue;
    }
    os_log(OS_LOG_DEFAULT, "%s: %llu", netif_stats_str(i), STATS_VAL(nifs, i));
  }
}

errno_t nx_netif_flow_add(struct nx_netif *nif, nexus_port_t port,
  ...
  if (++nif->nif_flow_cnt == 1) {
    ...
    ft_type = NETIF_IS_LOW_LATENCY(nif) ? FT_TYPE_IPV6_ULA : FT_TYPE_ETHERTYPE;

    err = nx_netif_flowtable_init(nif, ft_type);
    if (err != 0) {
      STATS_INC(nifs, NETIF_STATS_VP_FLOW_TABLE_INIT_FAIL);
      DTRACE_SKYWALK1(flowtable__init__fail, struct nx_netif *, nif);
      goto fail;
    }
    new_table = TRUE;
  }
  ...
  lck_mtx_unlock(&nif->nif_flow_lock);
  SK_DF(SK_VERB_VP, "flow add successful: if %s, nif %p", if_name(nif->nif_ifp),
        SK_KVA(nif));
...
fail:
  ...
  lck_mtx_unlock(&nif->nif_flow_lock);
  SK_ERR("flow add failed: if %s, nif %p, err %d", if_name(nif->nif_ifp),
         SK_KVA(nif), err);
  return err;
}

errno_t nx_netif_flow_remove(struct nx_netif *nif, struct netif_flow *nf) {
  struct netif_flowtable_ops *fops;
  struct netif_flowtable *ft;
  struct netif_stats *nifs = &nif->nif_stats;
  ...
  while (nf->nf_refcnt > 0) {
    ...
    (void)msleep(&nf->nf_refcnt, &nif->nif_flow_lock, (PZERO + 1), __FUNCTION__,
                 NULL);
  }
  ...
  SK_DF(SK_VERB_VP, "flow remove: if %s, nif %p", if_name(nif->nif_ifp),
        SK_KVA(nif));
...
void nx_netif_flow_init(struct nx_netif *nif) {
  ...
  if (ifp->if_family != IFNET_FAMILY_ETHERNET) {
    SK_DF(SK_VERB_VP,
          "%s: flows not supported on "
          "interface family %d",
          if_name(ifp), ifp->if_family);
    return;
  }
  ASSERT(nif->nif_flow_flags == 0);
  lck_mtx_init(&nif->nif_flow_lock, &nexus_lock_group, &nexus_lock_attr);
...
void nx_netif_flow_fini(struct nx_netif *nif) {
  if ((nif->nif_flow_flags & NETIF_FLOW_FLAG_INITIALIZED) == 0) {
    SK_DF(SK_VERB_VP, "%s: flows not initialized", if_name(nif->nif_ifp));
    return;
  }
  nif->nif_flow_flags &= ~NETIF_FLOW_FLAG_INITIALIZED;
  ...
  SK_DF(SK_VERB_VP, "%s: flows uninitialization done", if_name(nif->nif_ifp));
}

static void nx_netif_flow_set_enable(struct nx_netif *nif, boolean_t set) {
  /*
  ...
  lck_mtx_lock(&nif->nif_flow_lock);
  if (set) {
    SK_DF(SK_VERB_VP, "%s: flow enable, nif %p", if_name(nif->nif_ifp),
          SK_KVA(nif));
    nif->nif_flow_flags |= NETIF_FLOW_FLAG_ENABLED;
  } else {
    SK_DF(SK_VERB_VP, "%s: flow disable, nif %p", if_name(nif->nif_ifp),
          SK_KVA(nif));
    nif->nif_flow_flags &= ~NETIF_FLOW_FLAG_ENABLED;
  }
  lck_mtx_unlock(&nif->nif_flow_lock);
...
void nx_netif_flow_enable(struct nx_netif *nif) {
  nx_netif_flow_set_enable(nif, TRUE);
}

void nx_netif_flow_disable(struct nx_netif *nif) {
