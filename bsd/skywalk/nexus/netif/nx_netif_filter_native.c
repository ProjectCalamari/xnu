...
static void nx_netif_filter_tx_pkt_enqueue(struct nexus_netif_adapter *nifna,
  ...
  if (nif->nif_filter_cnt == 0) {
    ...
    DTRACE_SKYWALK2(pkt__default__drop, struct nx_netif *, nif, int, dropcnt);
    STATS_ADD(nifs, NETIF_STATS_FILTER_DROP_DEFAULT, dropcnt);
    STATS_ADD(nifs, NETIF_STATS_DROP, dropcnt);
    return;
  }
  fpkt_chain =
      nx_netif_pkt_to_filter_pkt_chain(nifna, pkt_chain, NETIF_CONVERT_TX);
  if (fpkt_chain == NULL) {
    return;
  }
...
static struct __kern_packet *get_next_pkt(struct nx_pktq pktqs[KPKT_TC_MAX],
                                          int *curr, int end) {
  int i;
...
SK_NO_INLINE_ATTRIBUTE
static struct __kern_packet *nx_netif_filter_tx_processed_pkt_dequeue(
    struct nexus_netif_adapter *nifna, kern_packet_svc_class_t sc,
  ...
  DTRACE_SKYWALK4(processed__pkt__dequeue, struct nexus_netif_adapter *, nifna,
                  uint32_t, cnt, uint32_t, bytes, struct __kern_packet *,
                  p_head);
  return p_head;
}
...
nx_netif_filter_tx_processed_pkt_enqueue(struct nexus_netif_adapter *nifna,
  ...
  if (__improbable((qlen = nx_pktq_len(q)) > nx_pktq_limit(q))) {
    ...
    DTRACE_SKYWALK4(q__full, struct nexus_netif_adapter *, nifna,
                    struct nx_pktq *, q, uint32_t, qlen, struct __kern_packet *,
                    p_chain);
  ...
  DTRACE_SKYWALK4(processed__pkt__enqueue, struct nexus_netif_adapter *, nifna,
                  struct nx_pktq *, q, uint32_t, qlen, uint32_t, cnt);
  nx_pktq_unlock(q);
  return 0;
}
...
nx_netif_tx_processed_pkt_get_len(struct nexus_netif_adapter *nifna,
                                  kern_packet_svc_class_t sc, uint32_t *packets,
                                  uint32_t *bytes, errno_t orig_err) {
  struct nx_netif *nif = nifna->nifna_netif;
  struct nx_pktq *q;
  uint32_t qlen = 0;
  size_t qsize = 0;
  ...
  DTRACE_SKYWALK6(processed__pkt__qlen, struct nexus_netif_adapter *, nifna,
                  struct nx_pktq *, q, uint32_t, qlen, size_t, qsize, uint32_t,
                  (packets != NULL) ? *packets : 0, uint32_t,
                  (bytes != NULL) ? *bytes : 0);
  return err;
}

static void fix_dequeue_pkt_return_args(struct __kern_packet *p_chain,
                                        classq_pkt_t *head, classq_pkt_t *tail,
                                        uint32_t *cnt, uint32_t *len,
                                        errno_t orig_err, errno_t *err) {
  struct __kern_packet *__single p_tail = NULL;
  uint32_t c = 0, l = 0;
 ...
 */
errno_t nx_netif_native_tx_dequeue(struct nexus_netif_adapter *nifna,
                                   uint32_t sc, uint32_t pkt_limit,
                                   uint32_t byte_limit, classq_pkt_t *head,
                                   classq_pkt_t *tail, uint32_t *cnt,
                                   uint32_t *len, boolean_t drvmgt,
                                   errno_t orig_err) {
#pragma unused(drvmgt)
   ...
   */
  p_chain = nx_netif_filter_tx_processed_pkt_dequeue(nifna, sc, pkt_limit,
                                                     byte_limit);

  fix_dequeue_pkt_return_args(p_chain, head, tail, cnt, len, orig_err, &err);
  return err;
}
 ...
 */
errno_t nx_netif_native_tx_get_len(struct nexus_netif_adapter *nifna,
                                   uint32_t sc, uint32_t *packets,
                                   uint32_t *bytes, errno_t orig_err) {
  struct nx_netif *nif = nifna->nifna_netif;
  ...
  return nx_netif_tx_processed_pkt_get_len(nifna, sc, packets, bytes, orig_err);
}
