...
struct netif_filter {
  ...
  uint32_t nf_refcnt;
  void *nf_cb_arg;
  errno_t (*nf_cb_func)(void *, struct __kern_packet *, uint32_t);
};
...
typedef enum { FT_TYPE_ETHERTYPE, FT_TYPE_IPV6_ULA } netif_flowtable_type_t;
...
typedef boolean_t netif_flow_match_t(struct netif_flow_desc *,
                                     struct netif_flow_desc *);
typedef int netif_flow_info_t(struct __kern_packet *, struct netif_flow_desc *,
                              uint32_t);
typedef int netif_flow_insert_t(struct netif_flowtable *, struct netif_flow *);
typedef void netif_flow_remove_t(struct netif_flowtable *, struct netif_flow *);
typedef struct netif_flowtable *
netif_flow_table_alloc_t(struct netif_flowtable_ops *);
...
#define NETIF_QSET_FLAG_DEFAULT                                                \
  0x0001                           /* default queue set of the logical link */
...
#define NETIF_IS_LOW_LATENCY(n) (((n)->nif_flags & NETIF_FLAG_LOW_LATENCY) != 0)
#define NETIF_IS_COMPAT(n) (((n)->nif_flags & NETIF_FLAG_COMPAT) != 0)
...
typedef enum { NETIF_MODE_NONE, NETIF_MODE_FSW, NETIF_MODE_LLW } netif_mode_t;
...
#define NETIF_WLOCK(_nif) lck_rw_lock_exclusive(&(_nif)->nif_lock)
#define NETIF_WUNLOCK(_nif) lck_rw_unlock_exclusive(&(_nif)->nif_lock)
#define NETIF_WLOCKTORLOCK(_nif)                                               \
  lck_rw_lock_exclusive_to_shared(&(_nif)->nif_lock)
#define NETIF_RLOCK(_nif) lck_rw_lock_shared(&(_nif)->nif_lock)
...
#define NETIF_RUNLOCK(_nif) lck_rw_unlock_shared(&(_nif)->nif_lock)
#define NETIF_UNLOCK(_nif) lck_rw_done(&(_nif)->nif_lock)
...
#define SKYWALK_SET_CAPABLE(ifp)                                               \
  do {                                                                         \
...
#define SKYWALK_CLEAR_CAPABLE(ifp)                                             \
  do {                                                                         \
    ifnet_lock_exclusive(ifp);                                                 \
    (ifp)->if_capabilities &= ~IFCAP_SKYWALK;                                  \
    (ifp)->if_capenable &= ~IFCAP_SKYWALK;                                     \
...
#define SKYWALK_NATIVE(ifp) ((ifp)->if_eflags & IFEF_SKYWALK_NATIVE)
...
#define MIT_SPIN_LOCK(_mit) lck_spin_lock(&(_mit)->mit_lock)
...
#define MIT_SPIN_UNLOCK(_mit) lck_spin_unlock(&(_mit)->mit_lock)
...
extern int nx_netif_na_find(struct kern_nexus *, struct kern_channel *,
                            ...
                            struct nexus_adapter **, boolean_t create);
extern int nx_netif_na_special(struct nexus_adapter *, struct kern_channel *,
...
extern size_t nx_netif_prov_nx_mib_get(struct kern_nexus *nx,
                                       ...
                                       void *__sized_by(len), size_t len,
                                       struct proc *);
...
extern errno_t nx_netif_filter_add(struct nx_netif *, nexus_port_t, void *,
                                   errno_t (*)(void *, struct __kern_packet *,
                                               uint32_t),
                                   struct netif_filter **);
...
extern errno_t nx_netif_demux(struct nexus_netif_adapter *,
                              struct __kern_packet *, struct __kern_packet **,
                              struct nexus_pkt_stats *, uint32_t);
extern errno_t nx_netif_flow_add(struct nx_netif *, nexus_port_t,
                                 struct netif_flow_desc *, void *,
                                 errno_t (*)(void *, void *, uint32_t),
                                 struct netif_flow **);
...
extern struct netif_flow *
nx_netif_flow_classify(struct nx_netif *, struct __kern_packet *, uint32_t);
...
nx_netif_mbuf_to_filter_pkt_chain(struct nexus_netif_adapter *, struct mbuf *,
                                  uint32_t);
extern struct mbuf *
...
extern struct __kern_packet *
nx_netif_pkt_to_filter_pkt(struct nexus_netif_adapter *, struct __kern_packet *,
                           uint32_t);
extern struct __kern_packet *
...
extern struct mbuf *nx_netif_pkt_to_mbuf(struct nexus_netif_adapter *,
                                         struct __kern_packet *, uint32_t);
extern struct __kern_packet *nx_netif_pkt_to_pkt(struct nexus_netif_adapter *,
                                                 struct __kern_packet *,
                                                 uint32_t);

extern void nx_netif_mbuf_chain_info(struct mbuf *, struct mbuf **, uint32_t *,
                                     uint32_t *);
...
extern void nx_netif_mit_config(struct nexus_netif_adapter *, boolean_t *,
...
extern int netif_rx_notify(struct __kern_channel_ring *, struct proc *p,
                           uint32_t);
extern int netif_llw_rx_notify(struct __kern_channel_ring *, struct proc *p,
                               uint32_t);
extern void netif_receive(struct nexus_netif_adapter *, struct __kern_packet *,
...
extern int netif_ring_tx_refill(const kern_channel_ring_t, uint32_t, uint32_t,
                                boolean_t, boolean_t *, boolean_t);
extern void netif_hwna_set_mode(struct nexus_adapter *, netif_mode_t,
                                void (*)(struct nexus_adapter *,
                                         struct __kern_packet *,
                                         struct nexus_pkt_stats *));
...
extern int
nx_netif_notify_steering_info(struct nx_netif *, struct netif_qset *,
                              struct ifnet_traffic_descriptor_common *, bool);

/*
 * Rx flow steering functions
 */
extern int
nx_netif_configure_rx_flow_steering(struct kern_nexus *, uint32_t,
