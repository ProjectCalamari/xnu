...
static int nx_netif_default_drop_sysctl SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  ...
  err =
      sysctl_io_number(req, nx_netif_filter_default_drop,
...
static errno_t nx_netif_default_cb(struct nexus_netif_adapter *nifna,
...
errno_t nx_netif_filter_inject(struct nexus_netif_adapter *nifna,
                               struct netif_filter *f,
                               struct __kern_packet *pkt_chain,
                               uint32_t flags) {
  ...
  if ((nif->nif_filter_flags & NETIF_FILTER_FLAG_ENABLED) == 0) {
    DTRACE_SKYWALK2(disabled, struct nexus_netif_adapter *, nifna,
                    struct netif_filter *, f);
  ...
  if (f != NULL) {
    ...
    if (__improbable(f == NULL)) {
       ...
       *    acquired.
       */
      DTRACE_SKYWALK1(removed, struct nexus_netif_adapter *, nifna);
...
drop:
  ...
  if (drop_stat != -1) {
    ...
    DTRACE_SKYWALK3(inject__drop, struct nexus_netif_adapter *, nifna, uint32_t,
                    flags, int, dropcnt);
    return err;
  }
...
errno_t nx_netif_filter_add(struct nx_netif *nif, nexus_port_t port,
                            void *cb_arg,
                            errno_t (*cb_func)(void *, struct __kern_packet *,
                                               uint32_t),
...
static void nx_netif_filter_flushq(struct nx_netif *nif) {
  struct netif_stats *nifs = &nif->nif_stats;
  uint32_t flushed = 0;
  int i;
  ...
  if (flushed > 0) {
    DTRACE_SKYWALK2(filter__flush, struct nx_netif *, nif, uint32_t, flushed);
    STATS_ADD(nifs, NETIF_STATS_FILTER_TX_FLUSH, flushed);
  }
}

errno_t nx_netif_filter_remove(struct nx_netif *nif, struct netif_filter *nf) {
  struct netif_stats *nifs = &nif->nif_stats;
  ...
  while (nf->nf_refcnt > 0) {
    ...
    (void)msleep(&nf->nf_refcnt, &nif->nif_filter_lock, (PZERO + 1),
                 __FUNCTION__, NULL);
  }
...
static void nx_netif_filter_initq(struct nx_netif *nif) {
  int i;
...
void nx_netif_filter_init(struct nx_netif *nif) {
  ifnet_t ifp = nif->nif_ifp;

  if (!ifnet_needs_netif_netagent(ifp)) {
    SK_DF(SK_VERB_FILTER,
          "%s: filters not supported due "
          "to missing if_attach_nx flag",
          if_name(ifp));
    return;
  }
  if (NETIF_IS_LOW_LATENCY(nif)) {
    SK_DF(SK_VERB_FILTER,
          "%s: filters not supported on "
...
static void nx_netif_filter_finiq(struct nx_netif *nif) {
  int i;
...
void nx_netif_filter_fini(struct nx_netif *nif) {
  if ((nif->nif_filter_flags & NETIF_FILTER_FLAG_INITIALIZED) == 0) {
    SK_DF(SK_VERB_FILTER, "%s: filters not initialized", if_name(nif->nif_ifp));
    return;
  }
  nif->nif_filter_flags &= ~NETIF_FILTER_FLAG_INITIALIZED;
...
static void nx_netif_filter_set_enable(struct nx_netif *nif, boolean_t set) {
  /*
  ...
  lck_mtx_lock(&nif->nif_filter_lock);
  if (set) {
    SK_DF(SK_VERB_FILTER, "%s: filter enabled, nif %p", if_name(nif->nif_ifp),
          SK_KVA(nif));
    nif->nif_filter_flags |= NETIF_FILTER_FLAG_ENABLED;
  } else {
    SK_DF(SK_VERB_FILTER, "%s: filter disabled, nif %p", if_name(nif->nif_ifp),
          SK_KVA(nif));
    nif->nif_filter_flags &= ~NETIF_FILTER_FLAG_ENABLED;
  }
  lck_mtx_unlock(&nif->nif_filter_lock);
...
void nx_netif_filter_enable(struct nx_netif *nif) {
  nx_netif_filter_set_enable(nif, TRUE);
}

void nx_netif_filter_disable(struct nx_netif *nif) {
