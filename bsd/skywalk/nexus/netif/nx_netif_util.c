...
struct __kern_packet *nx_netif_alloc_packet(struct kern_pbufpool *pp,
                                            uint32_t sz, kern_packet_t *php) {
  kern_packet_t ph;
...
SK_NO_INLINE_ATTRIBUTE
void nx_netif_free_packet(struct __kern_packet *pkt) {
  pp_free_packet_single(pkt);
}

SK_NO_INLINE_ATTRIBUTE
void nx_netif_free_packet_chain(struct __kern_packet *pkt_chain, int *cnt) {
  pp_free_packet_chain(pkt_chain, cnt);
}

static void __check_convert_flags(uint32_t flags) {
...
SK_NO_INLINE_ATTRIBUTE
static void fill_vlan_info(struct __kern_packet *fpkt) {
  struct mbuf *m;
  ...
  if ((fpkt->pkt_pflags & PKT_F_MBUF_DATA) != 0) {
    ...
    if ((pkt->pkt_pflags & PKT_F_MBUF_DATA) != 0) {
      ...
      DTRACE_SKYWALK1(tag__from__inner__mbuf, uint16_t, tag);
    } else {
...
nx_netif_mbuf_to_filter_pkt(struct nexus_netif_adapter *nifna, struct mbuf *m,
                            uint32_t flags) {
  ...
  off = ((flags & NETIF_CONVERT_TX) != 0) ? (uint8_t)ifp->if_tx_headroom : 0;
  ...
  if (__improbable((off + mlen) > PP_BUF_SIZE_DEF(pp))) {
    ...
    DTRACE_SKYWALK5(mbuf__truncated, struct nexus_netif_adapter *, nifna,
                    struct mbuf *, m, uint8_t, off, int, mlen, uint32_t,
                    PP_BUF_SIZE_DEF(pp));
    STATS_INC(nifs, NETIF_STATS_FILTER_PKT_TRUNCATED);
  }
  fpkt = nx_netif_alloc_packet(pp, off + mlen, &fph);
  if (__improbable(fpkt == NULL)) {
    DTRACE_SKYWALK2(alloc__fail, struct nexus_netif_adapter *, nifna,
                    struct mbuf *, m);
  ...
  if (__improbable((m->m_flags & M_HASFCS) != 0)) {
    if (type != NR_RX) {
       ...
       */
      DTRACE_SKYWALK2(bad__flags, struct nexus_netif_adapter *, nifna,
                      struct mbuf *, m);
      goto drop;
    }
    if (mlen > ETHER_CRC_LEN) {
      ...
      DTRACE_SKYWALK3(bad__pkt__size, struct nexus_netif_adapter *, nifna,
                      struct mbuf *, m, int, mlen);
      goto drop;
    }
  }
  /*
  ...
  VERIFY(err == 0);
  nif->nif_pkt_copy_from_mbuf(type, fph, off, m, 0, mlen, FALSE, 0);
...
drop:
  if (fpkt != NULL) {
    ...
    ASSERT(fpkt->pkt_mbuf == NULL && (fpkt->pkt_pflags & PKT_F_MBUF_DATA) == 0);
...
nx_netif_mbuf_to_filter_pkt_chain(struct nexus_netif_adapter *nifna,
  ...
  DTRACE_SKYWALK2(pkt__chain, struct __kern_packet *, pkt_head, int, c);
  return pkt_head;
}
...
nx_netif_pkt_to_filter_pkt(struct nexus_netif_adapter *nifna,
  ...
  if (__improbable((off + plen) > PP_BUF_SIZE_DEF(pp))) {
    ...
    DTRACE_SKYWALK5(pkt__truncated, struct nexus_netif_adapter *, nifna,
                    struct __kern_packet *, pkt, uint8_t, off, int, plen,
   ...
   */
  if (m != NULL) {
    nif->nif_pkt_copy_from_mbuf(type, fph, off, m, 0, plen, FALSE, 0);
  } else {
    nif->nif_pkt_copy_from_pkt(type, fph, off, ph, pkt->pkt_headroom, plen,
                               FALSE, 0, 0, FALSE);
  }
...
drop:
  if (fpkt != NULL) {
    ...
    ASSERT(fpkt->pkt_pkt == NULL && (fpkt->pkt_pflags & PKT_F_PKT_DATA) == 0);
    nx_netif_free_packet(fpkt);
  }
  STATS_INC(nifs, NETIF_STATS_DROP);
  nx_netif_free_packet(pkt);
...
struct mbuf *nx_netif_pkt_to_mbuf(struct nexus_netif_adapter *nifna,
                                  struct __kern_packet *pkt, uint32_t flags) {
  struct nx_netif *nif = nifna->nifna_netif;
  ifnet_t ifp = nif->nif_ifp;
  ...
  nif->nif_pkt_copy_to_mbuf(type, ph, pkt->pkt_headroom, m, 0, (uint32_t)len,
                            FALSE, 0);
...
struct __kern_packet *nx_netif_pkt_to_pkt(struct nexus_netif_adapter *nifna,
                                          struct __kern_packet *pkt,
                                          uint32_t flags) {
  struct nx_netif *nif = nifna->nifna_netif;
  ...
  if (type == NR_TX) {
    ...
    ASSERT(na->na_type == NA_NETIF_VP || na->na_type == NA_NETIF_DEV);
  ...
  if (__improbable((off + len) > PP_BUF_SIZE_DEF(pp))) {
    ...
    DTRACE_SKYWALK5(pkt__too__large, struct nexus_netif_adapter *, nifna,
  ...
  if (m != NULL) {
    nif->nif_pkt_copy_from_mbuf(type, dph, off, m, 0, len, FALSE, 0);
  } else {
    nif->nif_pkt_copy_from_pkt(type, dph, off, ph, pkt->pkt_headroom, len,
                               FALSE, 0, 0, FALSE);
  }
...
void nx_netif_mbuf_chain_info(struct mbuf *m_head, struct mbuf **m_tail,
...
void nx_netif_pkt_chain_info(struct __kern_packet *p_head,
                             struct __kern_packet **p_tail, uint32_t *cnt,
                             uint32_t *bytes) {
...
int nx_netif_get_max_mtu(ifnet_t ifp, uint32_t *max_mtu) {
  struct ifreq ifr;
  int err;
...
void nx_netif_pktap_output(ifnet_t ifp, int af, struct __kern_packet *pkt) {
  uint32_t dlt;
  ...
  pktap_output_packet(ifp, af, dlt, -1, NULL, -1, NULL, SK_PKT2PH(pkt), NULL, 0,
                      pkt->pkt_flow_ip_proto, pkt->pkt_flow_token, flags);
}
