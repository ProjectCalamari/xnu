...
typedef void(nxctl_traffic_rule_link_cb_t)(struct nxctl_traffic_rule *ntr);

typedef void(nxctl_traffic_rule_unlink_cb_t)(struct nxctl_traffic_rule *ntr);
...
typedef int(nxctl_traffic_rule_notify_cb_t)(struct nxctl_traffic_rule *ntr,
                                            uint32_t flags);

/*
 * Callback for a rule type to get rule count.
 */
typedef int(nxctl_traffic_rule_get_count_cb_t)(const char *ifname,
                                               uint32_t *count);
...
typedef void(nxctl_traffic_rule_destroy_cb_t)(struct nxctl_traffic_rule *ntr);

/*
 * This is used for copying all rules for a type (including generic
 * and type-specific info) to userspace.
 */
typedef int(nxctl_traffic_rule_get_all_cb_t)(uint32_t size, uint32_t *count,
                                             user_addr_t uaddr);
