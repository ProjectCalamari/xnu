...
void nx_pktq_safe_init(struct __kern_channel_ring *kr, struct nx_pktq *q,
...
void nx_pktq_init(struct nx_pktq *q, uint32_t lim) {
  __nx_pktq_init(q, lim, NULL);
}

void nx_pktq_concat(struct nx_pktq *q1, struct nx_pktq *q2) {
  uint32_t qlen;
  uint64_t qsize;
...
boolean_t nx_pktq_empty(struct nx_pktq *q) {
  return qempty(&q->nx_pktq_q) && qhead(&q->nx_pktq_q) == NULL;
}

void nx_pktq_purge(struct nx_pktq *q) { _flushq(&q->nx_pktq_q); }

void nx_pktq_safe_purge(struct nx_pktq *q) {
  nx_pktq_lock(q);
  _flushq(&q->nx_pktq_q);
  nx_pktq_unlock(q);
}

void nx_pktq_safe_destroy(struct nx_pktq *q) {
  VERIFY(nx_pktq_empty(q));
  lck_mtx_destroy(&q->nx_pktq_lock, q->nx_pktq_grp);
}

void nx_pktq_destroy(struct nx_pktq *q) { VERIFY(nx_pktq_empty(q)); }
