...
#define NINETRULEMASKS (sizeof(nxctl_inet_traffic_rule_masks) / sizeof(uint8_t))
 ...
 */
int nxctl_inet_traffic_rule_get_count(const char *ifname, uint32_t *count) {
  int err;
 ...
 */
int nxctl_inet_traffic_rule_find_qset_id(
    const char *ifname, struct ifnet_traffic_descriptor_inet *td,
    uint64_t *qset_id) {
 ...
 * This function populates struct ifnet_traffic_descriptor_inet instead of
 * struct __flow.
 */
static int fill_inet_td(struct __kern_packet *pkt,
...
#define udph _l4._udph
  ...
  case 4:
    if (cls_len < sizeof(struct ip)) {
      SK_ERR("cls_len < sizeof(struct ip) (%d < %lu)", cls_len,
             sizeof(struct ip));
      err = EINVAL;
      goto fail;
    }
    ...
    if (l3hlen < sizeof(struct ip)) {
      SK_ERR("l3hlen < sizeof(struct ip) (%d < %lu)", l3hlen,
             sizeof(struct ip));
      err = EINVAL;
      goto fail;
    }
  ...
  if (td->inet_proto == IPPROTO_TCP) {
    if (cls_len < l3hlen + sizeof(*tcph) || ulen < sizeof(*tcph)) {
      SK_ERR("cls_len < l3hlen + sizeof(*tcph) || ulen < sizeof(*tcph) "
             "(%d < %d + %lu || %d < %lu)",
             cls_len, l3hlen, sizeof(*tcph), ulen, sizeof(*tcph));
      err = EINVAL;
      goto fail;
    }
    ...
    if (cls_len < l3hlen + sizeof(*udph) || ulen < sizeof(*udph)) {
      SK_ERR("cls_len < l3hlen + sizeof(*udph) || ulen < sizeof(*udph) "
             "(%d < %d + %lu || %d < %lu)",
             cls_len, l3hlen, sizeof(*udph), ulen, sizeof(*udph));
      err = EINVAL;
      goto fail;
    }
    l4hlen = sizeof(*udph);
  ...
  return 0;
fail:
  DTRACE_SKYWALK5(classify__failed, struct ip *, iph, size_t, pkt_len, uint8_t,
                  pkt->pkt_l2_len, struct ifnet_traffic_descriptor_inet *, td,
                  int, err);
  bzero(td, sizeof(*td));
  return err;
#undef iph
...
int nxctl_inet_traffic_rule_find_qset_id_with_pkt(const char *ifname,
...
void inet_traffic_rule_init(kern_allocation_name_t rule_tag) {
  ASSERT(nxctl_traffic_rule_tag == NULL);
  nxctl_traffic_rule_tag = rule_tag;
}

int inet_traffic_rule_validate(const char *ifname,
  ...
  if (td->itd_len != sizeof(*tdi)) {
    SK_ERR("invalid td len: expected %lu, actual %d", sizeof(*tdi),
           td->itd_len);
    return EINVAL;
  }
  if (td->itd_flags == 0 ||
      (td->itd_flags & ~(IFNET_TRAFFIC_DESCRIPTOR_FLAG_INBOUND |
...
static void inet_traffic_rule_storage_create(void) {
...
SK_NO_INLINE_ATTRIBUTE
static void inet_traffic_rule_storage_destroy(void) {
...
inet_traffic_rule_if_create(const char *ifname) {
  ...
  rif = sk_alloc_type(struct nxctl_traffic_rule_inet_if, Z_WAITOK | Z_NOFAIL,
...
SK_NO_INLINE_ATTRIBUTE
static boolean_t
inet_traffic_rule_match(struct nxctl_traffic_rule_inet *ntri,
                        const char *ifname, uint32_t flags,
                        struct ifnet_traffic_descriptor_inet *tdi) {
  ...
  if (tdi0->inet_ipver != tdi->inet_ipver) {
    DTRACE_SKYWALK2(ipver__mismatch, uint8_t, tdi0->inet_ipver, uint8_t,
                    tdi->inet_ipver);
    return FALSE;
  }
  if ((mask & IFNET_TRAFFIC_DESCRIPTOR_INET_PROTO) != 0 &&
      tdi0->inet_proto != tdi->inet_proto) {
    DTRACE_SKYWALK2(proto__mismatch, uint8_t, tdi0->inet_proto, uint8_t,
                    tdi->inet_proto);
    return FALSE;
  }
  if (tdi0->inet_ipver == IPVERSION) {
    if ((mask & IFNET_TRAFFIC_DESCRIPTOR_INET_LADDR) != 0 &&
      ...
      DTRACE_SKYWALK2(v4laddr__mismatch, in_addr_t, tdi0->inet_laddr.iia_v4addr,
                      in_addr_t, tdi->inet_laddr.iia_v4addr);
      return FALSE;
    }
    if ((mask & IFNET_TRAFFIC_DESCRIPTOR_INET_RADDR) != 0 &&
        tdi0->inet_raddr.iia_v4addr != tdi->inet_raddr.iia_v4addr) {
      DTRACE_SKYWALK2(v4raddr__mismatch, in_addr_t, tdi0->inet_raddr.iia_v4addr,
    ...
    if ((mask & IFNET_TRAFFIC_DESCRIPTOR_INET_LADDR) != 0 &&
      ...
      DTRACE_SKYWALK2(v6laddr__mismatch, struct in6_addr *, &tdi0->inet_laddr,
                      struct in6_addr *, &tdi->inet_laddr);
      return FALSE;
    }
    if ((mask & IFNET_TRAFFIC_DESCRIPTOR_INET_RADDR) != 0 &&
        !inet_v6addr_cmp(&tdi0->inet_raddr, &tdi->inet_raddr)) {
      DTRACE_SKYWALK2(v6raddr__mismatch, struct in6_addr *, &tdi0->inet_raddr,
  ...
  if ((mask & IFNET_TRAFFIC_DESCRIPTOR_INET_LPORT) != 0 &&
    ...
    DTRACE_SKYWALK2(lport__mismatch, uint8_t, tdi0->inet_lport, uint8_t,
                    tdi->inet_lport);
    return FALSE;
  }
  if ((mask & IFNET_TRAFFIC_DESCRIPTOR_INET_RPORT) != 0 &&
      tdi0->inet_rport != tdi->inet_rport) {
    DTRACE_SKYWALK2(rport__mismatch, uint8_t, tdi0->inet_rport, uint8_t,
                    tdi->inet_rport);
    return FALSE;
  }
  return TRUE;
...
int inet_traffic_rule_find(const char *ifname,
                           struct ifnet_traffic_descriptor_common *td,
...
int inet_traffic_rule_find_by_uuid(uuid_t uuid,
...
static void inet_update_ifnet_traffic_rule_count(const char *ifname,
                                                 uint32_t count) {
  struct ifnet *ifp;
...
void inet_traffic_rule_link(struct nxctl_traffic_rule *ntr) {
  struct nxctl_traffic_rule_inet_if *rif;
  struct nxctl_traffic_rule_inet *ntri = (struct nxctl_traffic_rule_inet *)ntr;
  ...
  for (i = 0; i < NINETRULEMASKS; i++) {
    if (ntri->ntri_td.inet_mask == nxctl_inet_traffic_rule_masks[i]) {
...
void inet_traffic_rule_unlink(struct nxctl_traffic_rule *ntr) {
  struct nxctl_traffic_rule_inet_if *rif;
  struct nxctl_traffic_rule_inet *ntri = (struct nxctl_traffic_rule_inet *)ntr;
  struct nxctl_traffic_rule_inet_head *list = NULL;
  int i;
 ...
 */
int inet_traffic_rule_notify(struct nxctl_traffic_rule *ntr, uint32_t flags) {
  struct ifnet *ifp;
  struct nx_netif *nif;
  ...
  err = nx_netif_notify_steering_info(
      nif, qset, (struct ifnet_traffic_descriptor_common *)&ntri->ntri_td,
...
int inet_traffic_rule_get_count(const char *ifname, uint32_t *count) {
  struct nxctl_traffic_rule_inet_if *rif;
  int err;
...
int inet_traffic_rule_create(const char *ifname,
                             struct ifnet_traffic_descriptor_common *td,
                             struct ifnet_traffic_rule_action *ra,
                             uint32_t flags, struct nxctl_traffic_rule **ntrp) {
  struct nxctl_traffic_rule_inet *ntri;
  ...
  ntri = sk_alloc_type(struct nxctl_traffic_rule_inet, Z_WAITOK | Z_NOFAIL,
                       nxctl_traffic_rule_tag);
  ntr = &ntri->ntri_common;
...
void inet_traffic_rule_destroy(struct nxctl_traffic_rule *ntr) {
  struct nxctl_traffic_rule_inet *ntri;
...
int inet_traffic_rule_get_all(uint32_t size, uint32_t *count,
                              user_addr_t uaddr) {
  struct nxctl_traffic_rule_inet *ntri = NULL;
  struct nxctl_traffic_rule_inet_if *rif;
  struct nxctl_traffic_rule_inet_iocinfo info;
