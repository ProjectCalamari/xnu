...
static STAILQ_HEAD(, nxctl) nxctl_head = STAILQ_HEAD_INITIALIZER(nxctl_head);
static STAILQ_HEAD(, kern_nexus_provider)
    nxprov_head = STAILQ_HEAD_INITIALIZER(nxprov_head);
...
static struct nxctl *nxctl_alloc(struct proc *, struct fileproc *,
                                 zalloc_flags_t);
static void nxctl_free(struct nxctl *);

static struct kern_nexus_provider *
nxprov_create_common(struct nxctl *, struct kern_nexus_domain_provider *,
                     struct nxprov_reg *,
...
static struct kern_nexus_provider *
nxprov_alloc(struct kern_nexus_domain_provider *, zalloc_flags_t);
...
struct nxctl *nxctl_create(struct proc *p, struct fileproc *fp,
                           const uuid_t nxctl_uuid, int *err) {
...
void nxctl_close(struct nxctl *nxctl) {
  ...
  SK_D("nxctl %p UUID %s flags 0x%x", SK_KVA(nxctl),
       sk_uuid_unparse(nxctl->nxctl_uuid, uuidstr), nxctl->nxctl_flags);
#endif /* SK_LOG */
...
int nxctl_set_opt(struct nxctl *nxctl, struct sockopt *sopt) {
#pragma unused(nxctl)
  int err = 0;
...
int nxctl_get_opt(struct nxctl *nxctl, struct sockopt *sopt) {
#pragma unused(nxctl)
  int err = 0;
...
static int nxctl_get_nexus_prov_list(struct nxctl *nxctl,
                                     struct sockopt *sopt) {
  ...
  if (nres != NULL) {
    if (err == 0 && tmp_ptr != USER_ADDR_NULL) {
      if (sopt->sopt_p != kernproc) {
        err = copyout(nres, tmp_ptr, ncregs * sizeof(*nres));
      } else {
        caddr_t tmp;
        tmp = __unsafe_forge_bidi_indexable(
            caddr_t, CAST_DOWN(caddr_t, tmp_ptr), ncregs * sizeof(*nres));
...
/* Hoisted out of line to reduce kernel stack footprint */
SK_NO_INLINE_ATTRIBUTE
static int nxctl_get_nexus_prov_entry(struct nxctl *nxctl,
                                      struct sockopt *sopt) {
  ...
  SK_LOCK();
  STAILQ_FOREACH(nxprov, &nxprov_head, nxprov_link) {
    if (uuid_compare(nxprov->nxprov_uuid, nre.npre_prov_uuid) == 0) {
      /*
       * Return only entries that are visible to the caller,
       * unless it has PRIV_SKYWALK_OBSERVE_ALL.
       */
      if (nxprov->nxprov_ctl != nxctl) {
        if (skywalk_priv_check_cred(sopt->sopt_p, nxctl->nxctl_cred,
                                    PRIV_SKYWALK_OBSERVE_ALL) != 0) {
          nxprov = NULL;
          break;
        }
...
/* Hoisted out of line to reduce kernel stack footprint */
SK_NO_INLINE_ATTRIBUTE
static int nxctl_get_nexus_list(struct nxctl *nxctl, struct sockopt *sopt) {
  user_addr_t tmp_ptr = USER_ADDR_NULL;
  ...
  if (uuids != NULL) {
    if (err == 0 && nxprov != NULL && tmp_ptr != USER_ADDR_NULL) {
      ...
      if (cnt_uuid > 0) {
        if (sopt->sopt_p != kernproc) {
          err = copyout(uuids, tmp_ptr, cnt_uuid * sizeof(uuid_t));
        } else {
          caddr_t tmp;
          tmp = __unsafe_forge_bidi_indexable(
              caddr_t, CAST_DOWN(caddr_t, tmp_ptr), cnt_uuid * sizeof(uuid_t));
...
/* Hoisted out of line to reduce kernel stack footprint */
SK_NO_INLINE_ATTRIBUTE
static int nxctl_nexus_bind(struct nxctl *nxctl, struct sockopt *sopt) {
  ...
  if (m_key) {
    ...
    if (sopt->sopt_p != kernproc) {
       ...
       */
      void *nb_key =
          __unsafe_forge_bidi_indexable(void *, nbr.nb_key, nbr.nb_key_len);
  ...
  if (nx == NULL ||
      (disable_nxctl_check == 0 && nx->nx_prov->nxprov_ctl != nxctl &&
  ...
  if (m_key) {
    ...
    ASSERT(key != NULL);
    ASSERT(nbr.nb_key_len != 0 && nbr.nb_key_len <= NEXUS_MAX_KEY_LEN);
    /*
  ...
  SK_D("nexus %p nxb %p port %u flags 0x%x pid %d "
       ...
       SK_KVA(nx), SK_KVA(nxb), nbr.nb_port, nxb->nxb_flags, nxb->nxb_pid,
       nxb->nxb_uniqueid, sk_uuid_unparse(nxb->nxb_exec_uuid, exec_uuidstr),
       (nxb->nxb_key != NULL) ? SK_KVA(nxb->nxb_key) : 0, nxb->nxb_key_len);
...
/* Hoisted out of line to reduce kernel stack footprint */
SK_NO_INLINE_ATTRIBUTE
static int nxctl_nexus_unbind(struct nxctl *nxctl, struct sockopt *sopt) {
...
/* Hoisted out of line to reduce kernel stack footprint */
SK_NO_INLINE_ATTRIBUTE
static int nxctl_nexus_config(struct nxctl *nxctl, struct sockopt *sopt) {
  struct kern_nexus *nx = NULL;
  ...
  if (NX_DOM_PROV(nx)->nxdom_prov_config != NULL) {
    err = NX_DOM_PROV(nx)->nxdom_prov_config(NX_DOM_PROV(nx), nx, &ncr,
                                             sopt->sopt_dir, sopt->sopt_p,
                                             nxctl->nxctl_cred);
  } else {
    err = EPERM;
  }
...
struct nxbind *nxb_alloc(zalloc_flags_t how) {
...
void nxb_free(struct nxbind *nxb) {
 ...
 */
boolean_t nxb_is_equal(struct nxbind *nxb0, struct nxbind *nxb1) {
  ...
  if ((nxb0->nxb_flags & NXBF_MATCH_KEY) &&
      (nxb0->nxb_key_len != nxb1->nxb_key_len || nxb1->nxb_key == NULL ||
       timingsafe_bcmp(nxb1->nxb_key, nxb0->nxb_key, nxb1->nxb_key_len) != 0)) {
    return FALSE;
  }
...
void nxb_move(struct nxbind *snxb, struct nxbind *dnxb) {
...
/* Hoisted out of line to reduce kernel stack footprint */
SK_NO_INLINE_ATTRIBUTE
static int nxctl_get_channel_list(struct nxctl *nxctl, struct sockopt *sopt) {
  user_addr_t tmp_ptr = USER_ADDR_NULL;
  uint32_t nuuids = 0, ncuuids = 0;
  uuid_t *puuid, *uuids = NULL;
  size_t uuids_sz;
...
static void nxctl_init(struct nxctl *nxctl, struct proc *p,
                       struct fileproc *fp) {
  uuid_t p_uuid;
...
static struct nxctl *nxctl_alloc(struct proc *p, struct fileproc *fp,
                                 zalloc_flags_t how) {
...
static void nxctl_free(struct nxctl *nxctl) {
...
static void nxctl_retain_locked(struct nxctl *nxctl) {
  SK_LOCK_ASSERT_HELD();
...
void nxctl_retain(struct nxctl *nxctl) {
  SK_LOCK();
  nxctl_retain_locked(nxctl);
  SK_UNLOCK();
}

static int nxctl_release_locked(struct nxctl *nxctl) {
...
int nxctl_release(struct nxctl *nxctl) {
  int lastref;
 ...
 */
void nxctl_dtor(struct nxctl *arg) {
...
int nxprov_advise_connect(struct kern_nexus *nx, struct kern_channel *ch,
                          struct proc *p) {
  ...
  err = nxprov->nxprov_ext.nxpi_pre_connect(
      nxprov, p, nx, ch->ch_info->cinfo_nx_port, ch, &ch->ch_ctx);
  if (err != 0) {
  ...
  if (err != 0) {
    SK_D("ch %p flags %x nx %p connected error %d", SK_KVA(ch), ch->ch_flags,
         SK_KVA(nx), err);
    goto done;
  }
  os_atomic_or(&ch->ch_flags, CHANF_EXT_CONNECTED, relaxed);
  SK_D("ch %p flags %x nx %p connected", SK_KVA(ch), ch->ch_flags, SK_KVA(nx));
...
void nxprov_advise_disconnect(struct kern_nexus *nx, struct kern_channel *ch) {
  struct kern_nexus_provider *nxprov = NX_PROV(nx);
  ...
  if (ch->ch_flags & (CHANF_EXT_CONNECTED | CHANF_EXT_PRECONNECT)) {
    ...
    SK_D("ch %p flags %x nx %p disconnected", SK_KVA(ch), ch->ch_flags,
         SK_KVA(nx));
...
nxprov_create_common(struct nxctl *nxctl,
  ...
  pp_region_config_flags =
      PP_REGION_CONFIG_MD_MAGAZINE_ENABLE | PP_REGION_CONFIG_BUF_IODIR_BIDIR;
  /*
  ...
  if (init != NULL) {
    if (init->nxpi_flags & NXPIF_MONOLITHIC) {
      pp_region_config_flags |= PP_REGION_CONFIG_BUF_MONOLITHIC;
    }

    if (init->nxpi_flags & NXPIF_INHIBIT_CACHE) {
      pp_region_config_flags |= PP_REGION_CONFIG_BUF_NOCACHE;
    }
  }
  ...
  if ((*err = nxdom_prov_validate_params(nxdom_prov, reg, &nxp, srp, override,
                                         pp_region_config_flags)) != 0) {
    goto done;
  }
  ...
  if (init != NULL) {
    if (init->nxpi_version == KERN_NEXUS_PROVIDER_VERSION_NETIF) {
      ...
      bcopy(init, &nxprov->nxprov_netif_ext, sizeof(nxprov->nxprov_netif_ext));
    } else {
      ASSERT(!NXPROV_LLINK(nxprov));
      ASSERT(init->nxpi_version == KERN_NEXUS_PROVIDER_CURRENT_VERSION);
  ...
  } else if (nxdom_prov->nxdom_prov_dom->nxdom_type != NEXUS_TYPE_NET_IF) {
    /*
...
struct kern_nexus_provider *nxprov_create(struct proc *p, struct nxctl *nxctl,
                                          struct nxprov_reg *reg, int *err) {
  ...
  *err = 0;
  ...
  ASSERT(nxp->nxp_type != NEXUS_TYPE_NET_IF ||
         strbufcmp(nxdom_prov->nxdom_prov_name,
                   sizeof(nxdom_prov->nxdom_prov_name),
                   NEXUS_PROVIDER_NET_IF_COMPAT,
                   sizeof(NEXUS_PROVIDER_NET_IF_COMPAT)) == 0);
...
int nxprov_destroy(struct nxctl *nxctl, const uuid_t nxprov_uuid) {
  struct kern_nexus_provider *nxprov = NULL;
  int err = 0;
...
int nxprov_close(struct kern_nexus_provider *nxprov, boolean_t locked) {
  int err = 0;
  ...
  SK_D("nxprov %p UUID %s flags 0x%x", SK_KVA(nxprov),
       sk_uuid_unparse(nxprov->nxprov_uuid, uuidstr), nxprov->nxprov_flags);
#endif /* SK_LOG */
  ...
  if (nxprov->nxprov_flags & NXPROVF_CLOSED) {
    ...
    STAILQ_FOREACH_SAFE(nx, &nxprov->nxprov_nx_head, nx_prov_link, tnx) {
      nx_retain_locked(nx);
...
static void nxprov_detach(struct kern_nexus_provider *nxprov,
                          boolean_t locked) {
  if (!locked) {
    SK_LOCK();
  }
...
static struct kern_nexus_provider *
nxprov_alloc(struct kern_nexus_domain_provider *nxdom_prov,
             zalloc_flags_t how) {
...
static void nxprov_free(struct kern_nexus_provider *nxprov) {
...
static void nxprov_retain_locked(struct kern_nexus_provider *nxprov) {
  SK_LOCK_ASSERT_HELD();
...
void nxprov_retain(struct kern_nexus_provider *nxprov) {
  SK_LOCK();
  nxprov_retain_locked(nxprov);
  SK_UNLOCK();
}

static int nxprov_release_locked(struct kern_nexus_provider *nxprov) {
...
int nxprov_release(struct kern_nexus_provider *nxprov) {
  int lastref;
...
struct nxprov_params *nxprov_params_alloc(zalloc_flags_t how) {
  return zalloc_flags(nxprov_params_zone, how | Z_ZERO);
}

void nxprov_params_free(struct nxprov_params *nxp) {
  SK_DF(SK_VERB_MEM, "nxp %p FREE", SK_KVA(nxp));
  zfree(nxprov_params_zone, nxp);
}

static int nx_check_pp(struct kern_nexus_provider *nxprov,
   ...
   */
  if (nxdom_prov->nxdom_prov_dom->nxdom_md_type != pp->pp_md_type ||
      nxdom_prov->nxdom_prov_dom->nxdom_md_subtype != pp->pp_md_subtype) {
    SK_ERR("Mismatch in metadata type/subtype "
           "(%u/%u != %u/%u)",
           pp->pp_md_type, nxdom_prov->nxdom_prov_dom->nxdom_md_type,
           pp->pp_md_subtype, nxdom_prov->nxdom_prov_dom->nxdom_md_subtype);
    return EINVAL;
  }
...
struct kern_nexus *nx_create(struct nxctl *nxctl, const uuid_t nxprov_uuid,
                             const nexus_type_t dom_type, const void *nx_ctx,
                             nexus_ctx_release_fn_t nx_ctx_release,
                             struct kern_pbufpool *tx_pp,
  ...
  *err = 0;
  ...
  if (dom_type != NEXUS_TYPE_UNDEFINED &&
    ...
    SK_ERR("Mismatch in domain type (0x%u != 0x%u)", dom_type,
  ...
  lck_rw_init(&nx->nx_ch_if_adv_lock, &nexus_lock_group, &nexus_lock_attr);
  ...
  SK_D("nexus %p (%s:%s) UUID %s", SK_KVA(nx),
       nxdom_prov->nxdom_prov_dom->nxdom_name, nxdom_prov->nxdom_prov_name,
...
int nx_destroy(struct nxctl *nxctl, const uuid_t nx_uuid) {
  struct kern_nexus *nx = NULL;
  struct kern_nexus find;
  int err = 0;
...
static inline int nx_cmp(const struct kern_nexus *a,
                         const struct kern_nexus *b) {
  return uuid_compare(a->nx_uuid, b->nx_uuid);
}

struct kern_nexus *nx_find(const uuid_t nx_uuid, boolean_t locked) {
  struct kern_nexus *nx = NULL;
  struct kern_nexus find;
...
int nx_close(struct kern_nexus *nx, boolean_t locked) {
  int err = 0;
...
void nx_stop(struct kern_nexus *nx) {
...
void nx_detach(struct kern_nexus *nx) {
  struct kern_nexus_provider *nxprov = nx->nx_prov;
...
int nx_advisory_alloc(struct kern_nexus *nx, const char *name,
                      struct skmem_region_params *srp_nexusadv,
                      nexus_advisory_type_t type) {
  ...
  if ((nx->nx_adv.nxv_reg =
           skmem_region_create(name, srp_nexusadv, NULL, NULL, NULL)) == NULL) {
    return ENOMEM;
  }

  nx->nx_adv.nxv_adv = skmem_region_alloc(
      nx->nx_adv.nxv_reg, &maddr, NULL, NULL, (SKMEM_NOSLEEP | SKMEM_PANIC),
  ...
  if (type == NEXUS_ADVISORY_TYPE_FLOWSWITCH) {
    ...
    nx->nx_adv.netif_nxv_adv->nna_version = NX_NETIF_ADVISORY_CURRENT_VERSION;
  }
  return 0;
}

void nx_advisory_free(struct kern_nexus *nx) {
  if (nx->nx_adv.nxv_reg != NULL) {
    ASSERT(nx->nx_adv.nxv_adv != NULL);
    skmem_region_free(nx->nx_adv.nxv_reg, nx->nx_adv.nxv_adv, NULL);
...
static struct kern_nexus *nx_alloc(zalloc_flags_t how) {
  SK_LOCK_ASSERT_HELD();

  return zalloc_flags(nx_zone, how | Z_ZERO);
}

static void nx_free(struct kern_nexus *nx) {
...
void nx_retain_locked(struct kern_nexus *nx) {
  SK_LOCK_ASSERT_HELD();
...
void nx_retain(struct kern_nexus *nx) {
  SK_LOCK();
  nx_retain_locked(nx);
  SK_UNLOCK();
}

int nx_release_locked(struct kern_nexus *nx) {
...
int nx_release(struct kern_nexus *nx) {
  int lastref;
...
static int nx_init_rings(struct kern_nexus *nx, struct kern_channel *ch) {
  struct kern_nexus_provider *nxprov = NX_PROV(nx);
  ...
  for_rx_tx(t) {
    ...
    for (i = 0; i < na_get_nrings(na, t); i++) {
      ...
      if ((err = nxprov->nxprov_ext.nxpi_ring_init(nxprov, nx, ch, kring,
                                                   (kring->ckr_tx == NR_TX),
                                                   &kring->ckr_ctx)) != 0) {
        SK_D("ch %p flags %x nx %p kr \"%s\" "
             "(%p) krflags %x ring_init error %d",
             SK_KVA(ch), ch->ch_flags, SK_KVA(nx), kring->ckr_name,
             SK_KVA(kring), kring->ckr_flags, err);
        kring->ckr_ctx = NULL;
        undo = TRUE;
        break;
      }
...
static void nx_fini_rings(struct kern_nexus *nx, struct kern_channel *ch) {
  struct kern_nexus_provider *nxprov = NX_PROV(nx);
  struct nexus_adapter *na = ch->ch_na;
  int ksd_releases = 0;
  enum txrx t;
...
static int nx_init_slots(struct kern_nexus *nx,
  ...
  for (i = 0; i < kring->ckr_num_slots; i++) {
    ...
    if ((err = nxprov->nxprov_ext.nxpi_slot_init(nxprov, nx, kring, &slot[i], i,
                                                 &slot_ctx_prop,
                                                 &slot_ctx_arg)) != 0) {
      SK_D("nx %p kr \"%s\" (%p) krflags %x slot %u "
           "slot_init error %d",
           SK_KVA(nx), kring->ckr_name, SK_KVA(kring), kring->ckr_flags, i,
           err);
      break;
    }
...
static void nx_fini_slots(struct kern_nexus *nx,
                          struct __kern_channel_ring *kring) {
  struct kern_nexus_provider *nxprov = NX_PROV(nx);
  struct __slot_desc *slot = kring->ckr_ksds;
  uint32_t i;
  ...
  for (i = 0; i < kring->ckr_slot_ctxs_set; i++) {
    ...
    if (nxprov->nxprov_ext.nxpi_slot_fini != NULL) {
      nxprov->nxprov_ext.nxpi_slot_fini(nxprov, nx, kring, &slot[i], i);
    }
...
int nx_port_find(struct kern_nexus *nx, nexus_port_t first, nexus_port_t last,
  ...
  *nx_port = NEXUS_PORT_ANY;
  ...
  SK_DF(SK_VERB_NXPORT, "nx %p nx_port %d (err %d)", SK_KVA(nx), (int)*nx_port,
        err);

  return err;
}

static int nx_port_grow(struct kern_nexus *nx, nexus_port_size_t grow) {
  ...
  if ((ports = sk_realloc_type_array(struct nx_port_info, old_num_ports,
                                     num_ports, nx->nx_ports, Z_WAITOK,
  ...
  for (i = (nx->nx_num_ports / NX_PORT_CHUNK); i < (num_ports / NX_PORT_CHUNK);
       i++) {
  ...
  SK_DF(SK_VERB_NXPORT, "!!! nx %p ports %u/%u, %u ports added", SK_KVA(nx),
        nx->nx_active_ports, nx->nx_num_ports, grow);

  return 0;
}

int nx_port_alloc(struct kern_nexus *nx, nexus_port_t nx_port,
                  struct nxbind *nxb, struct nexus_adapter **na,
                  struct proc *p) {
  ...
  SK_DF(SK_VERB_NXPORT, "nx %p nx_port %d, ports %u/%u (err %d)", SK_KVA(nx),
        (int)nx_port, nx->nx_active_ports, nx->nx_num_ports, err);

  return err;
}

void nx_port_defunct(struct kern_nexus *nx, nexus_port_t nx_port) {
  struct nx_port_info *npi = &nx->nx_ports[nx_port];

  npi->npi_nah = NPI_NA_ENCODE(npi->npi_nah, NEXUS_PORT_STATE_DEFUNCT);
}

void nx_port_free(struct kern_nexus *nx, nexus_port_t nx_port) {
  struct nx_port_info *npi = NULL;
  bitmap_t *bmap;
  uint32_t i, j;
  ...
  SK_DF(SK_VERB_NXPORT, "--- nx %p nx_port %d, ports %u/%u", SK_KVA(nx),
        (int)nx_port, nx->nx_active_ports, nx->nx_num_ports);
}

int nx_port_bind_info(struct kern_nexus *nx, nexus_port_t nx_port,
  ...
  SK_DF(err ? SK_VERB_ERROR : SK_VERB_NXPORT,
        "+++ nx %p nx_port %d, ports %u/%u (err %d)", SK_KVA(nx), (int)nx_port,
        nx->nx_active_ports, nx->nx_num_ports, err);

  return err;
}

int nx_port_bind(struct kern_nexus *nx, nexus_port_t nx_port,
                 struct nxbind *nxb0) {
 ...
 */
static int nx_port_info_size(struct nx_port_info_header *info, size_t *sz) {
...
int nx_port_unbind(struct kern_nexus *nx, nexus_port_t nx_port) {
  struct nx_port_info *npi = NULL;
  struct nxbind *nxb;
  uint32_t i, j;
  bitmap_t *bmap;
...
struct nexus_adapter *nx_port_get_na(struct kern_nexus *nx,
                                     nexus_port_t nx_port) {
...
int nx_port_get_info(struct kern_nexus *nx, nexus_port_t port,
                     nx_port_info_type_t type, void *__sized_by(len) info,
                     uint32_t len) {
...
bool nx_port_is_valid(struct kern_nexus *nx, nexus_port_t nx_port) {
  return nx_port < nx->nx_num_ports;
}

bool nx_port_is_defunct(struct kern_nexus *nx, nexus_port_t nx_port) {
  ASSERT(nx_port_is_valid(nx, nx_port));

  return NPI_IS_DEFUNCT(&nx->nx_ports[nx_port]);
}

void nx_port_free_all(struct kern_nexus *nx) {
  ...
  sk_free_type_array_counted_by(struct nx_port_info, nx->nx_num_ports,
                                nx->nx_ports);
  nx->nx_ports = NULL;
  nx->nx_num_ports = 0;
}

void nx_port_foreach(struct kern_nexus *nx,
...
SYSCTL_PROC(_kern_skywalk, OID_AUTO, nexus_provider_list,
            ...
            nexus_provider_list_sysctl, "S,nexus_provider_info_t", "");

SYSCTL_PROC(_kern_skywalk, OID_AUTO, nexus_channel_list,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, 0,
            nexus_channel_list_sysctl, "S,nexus_channel_entry_t", "");

SYSCTL_PROC(_kern_skywalk, OID_AUTO, llink_list,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, NXMIB_LLINK_LIST,
            nexus_mib_get_sysctl, "S,nx_llink_info", "A list of logical links");

SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, flow,
            CTLTYPE_STRUCT | CTLFLAG_RW | CTLFLAG_LOCKED | CTLFLAG_ANYBODY |
                CTLFLAG_KERN,
...
SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, flow_owner,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, NXMIB_FLOW_OWNER,
            nexus_mib_get_sysctl, "S,sk_stats_flow_owner", "Nexus flow owners");

SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, flow_route,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, NXMIB_FLOW_ROUTE,
            nexus_mib_get_sysctl, "S,sk_stats_flow_route", "Nexus flow routes");

SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, net_if,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, NXMIB_NETIF_STATS,
            nexus_mib_get_sysctl, "S,sk_stats_net_if",
            "Nexus netif statistics collected in kernel");

SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, flow_switch,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, NXMIB_FSW_STATS,
            nexus_mib_get_sysctl, "S,sk_stats_flow_switch",
...
SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, userstack,
            ...
            NXMIB_USERSTACK_STATS, nexus_mib_get_sysctl, "S,sk_stats_userstack",
            "Nexus userstack statistics counter");

SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, flow_adv,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0, NXMIB_FLOW_ADV,
            nexus_mib_get_sysctl, "S,sk_stats_flow_adv",
            "Nexus flow advisory dump");

SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, netif_queue,
            CTLTYPE_STRUCT | CTLFLAG_RD | CTLFLAG_LOCKED, 0,
            NXMIB_NETIF_QUEUE_STATS, nexus_mib_get_sysctl,
            "S,netif_qstats_info", "A list of netif queue stats entries");

/*
 * Provider list sysctl
 */
static void nexus_provider_info_populate(struct kern_nexus_provider *nxprov,
  ...
  /* instance UUID list */
  uuids = __unsafe_forge_bidi_indexable(uuid_t *, info->npi_instance_uuids,
                                        sizeof(uuid_t) *
                                            info->npi_instance_uuids_count);
  STAILQ_FOREACH(nx, &nxprov->nxprov_nx_head, nx_prov_link) {
...
#pragma unused(arg1, arg2, oidp)
  ...
  STAILQ_FOREACH(nxprov, &nxprov_head, nxprov_link) {
    ...
    info_size = NEXUS_PROVIDER_INFO_SIZE(nxprov->nxprov_nx_count);
    if (scan != NULL) {
 ...
 * Channel list sysctl
 */
static uint32_t channel_ring_count(struct kern_channel *ch, enum txrx which) {
 ...
 */
static void populate_ring_entries(
    struct __kern_channel_ring *__counted_by(last) kring, ring_id_t first,
    ring_id_t last, nexus_channel_ring_entry *__counted_by(entry_count) entries,
  ...
  for (i = first; i < last; i++, scan++) {
    ...
    DTRACE_SKYWALK1(populate__ring, struct __kern_channel_ring *, ring);
    if (kr_stat_enable == 0) {
      bzero(&scan->ncre_stats, sizeof(scan->ncre_stats));
      bzero(&scan->ncre_user_stats, sizeof(scan->ncre_user_stats));
    } else {
      scan->ncre_stats = ring->ckr_stats;
      scan->ncre_stats.crs_seconds_since_last_update =
          now - scan->ncre_stats.crs_last_update_net_uptime;
...
static uint32_t nexus_channel_get_flags(uint32_t ch_mode, uint32_t ch_flags) {
  uint32_t flags = 0;
...
SK_NO_INLINE_ATTRIBUTE
static void nexus_channel_entry_populate(struct kern_channel *ch,
...
nexus_channel_info_populate(struct kern_nexus *nx,
                            nexus_channel_info *__sized_by(buffer_size) info,
                            size_t buffer_size) {
  struct kern_channel *ch = NULL;
  size_t info_size;
  caddr_t scan = NULL;
  ...
  STAILQ_FOREACH(ch, &nx->nx_ch_head, ch_link) {
    ...
    ring_count = channel_ring_count(ch, NR_TX) + channel_ring_count(ch, NR_RX);
...
#pragma unused(arg1, arg2, oidp)
  ...
  RB_FOREACH(nx, kern_nexus_tree, &nx_head) {
    ...
    info_size = nexus_channel_info_populate(nx, (void *)scan, buffer_space);
    if (scan != NULL) {
      if (buffer_space < info_size) {
        /* supplied buffer too small, stop copying */
        error = ENOMEM;
...
#pragma unused(arg1, arg2)
  ...
  if (req->newptr == USER_ADDR_NULL) {
     ...
     */
    if (oidp->oid_arg2 == NXMIB_FLOW && !kauth_cred_issuser(kauth_cred_get())) {
  ...
  RB_FOREACH(nx, kern_nexus_tree, &nx_head) {
    ...
    if (req->oldptr == USER_ADDR_NULL || buffer_space) {
      size = nx_dp->nxdom_prov_nx_mib_get(nx, &filter, scan, buffer_space, p);
    }
...
void kern_nexus_walktree(kern_nexus_walktree_f_t *f, void *arg0,
  ...
  RB_FOREACH(nx, kern_nexus_tree, &nx_head) { (*f)(nx, arg0); }
...
void nx_interface_advisory_notify(struct kern_nexus *nx) {
  ...
  STAILQ_FOREACH(ch, &nx->nx_ch_if_adv_head, ch_link_if_adv) {
    ...
    ASSERT(na != NULL);
    na_post_event(&na->na_tx_rings[ch->ch_first[NR_TX]], TRUE, FALSE, FALSE,
                  CHAN_FILT_HINT_IF_ADV_UPD);
    if (nxdom_type == NEXUS_TYPE_NET_IF) {
