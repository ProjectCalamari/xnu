...
static int nxop_ioctl(struct fileproc *fp, u_long cmd, caddr_t data,
                      vfs_context_t ctx) {
  struct nxctl *nxctl;
...
static int nxop_close(struct fileglob *fg, vfs_context_t ctx) {
#pragma unused(ctx)
  struct nxctl *nxctl;
  int error = 0;
...
int __nexus_open(struct proc *p, struct __nexus_open_args *uap, int *retval) {
  ...
  if (__improbable(uap->init == USER_ADDR_NULL ||
    ...
    SK_PERR(p, "EINVAL: init 0x%llx, init_len %u", uap->init, uap->init_len);
    err = EINVAL;
    goto done;
  }
  ...
  if (__improbable(err != 0)) {
    SK_PERR(p, "copyout err %d, init %p", err, SK_KVA(uap->init));
    goto done;
  }
  ...
  *retval = fd;
  ...
  SK_D("%s(%d) fd %d guard 0x%llx", sk_proc_name(p), sk_proc_pid(p), fd, guard);
...
int __nexus_register(struct proc *p, struct __nexus_register_args *uap,
                     int *retval) {
#pragma unused(retval)
  ...
  if (__improbable(uap->reg == USER_ADDR_NULL || uap->reg_len < sizeof(reg) ||
                   uap->prov_uuid == USER_ADDR_NULL ||
                   uap->prov_uuid_len < sizeof(uuid_t))) {
    SK_PERR(p,
            "EINVAL: reg %p, reg_len %u, prov_uuid %p, "
            "prov_uuid_len %u",
            SK_KVA(uap->reg), uap->reg_len, SK_KVA(uap->prov_uuid),
            uap->prov_uuid_len);
    return EINVAL;
  }
  ...
  if (__improbable(err != 0)) {
    SK_PERR(p, "copyout err %d, prov_uuid %p", err, SK_KVA(uap->prov_uuid));
    goto done;
  }
...
int __nexus_deregister(struct proc *p, struct __nexus_deregister_args *uap,
                       int *retval) {
#pragma unused(retval)
  struct fileproc *__single fp;
  struct nxctl *nxctl = NULL;
  ...
  if (__improbable(uap->prov_uuid_len < sizeof(uuid_t))) {
    SK_PERR(p, "EINVAL: prov_len %u < %lu", uap->prov_uuid_len, sizeof(uuid_t));
    return EINVAL;
  }

  err = copyin(uap->prov_uuid, (caddr_t)&nxprov_uuid, sizeof(uuid_t));
  if (__improbable(err != 0)) {
    SK_PERR(p, "copyin err %d, prov_uuid %p", err, SK_KVA(uap->prov_uuid));
    return err;
  }
...
int __nexus_create(struct proc *p, struct __nexus_create_args *uap,
                   int *retval) {
#pragma unused(retval)
  struct fileproc *__single fp;
  ...
  if (__improbable(uap->prov_uuid_len < sizeof(uuid_t) ||
    ...
    SK_PERR(p,
            "EINVAL: prov_uuid_len %u, nx_uuid_len %u, "
            "nx_uuid %p",
            uap->prov_uuid_len, uap->nx_uuid_len, SK_KVA(uap->nx_uuid));
    return EINVAL;
  }
  ...
  lck_mtx_lock(&nxctl->nxctl_lock);
  nx = nx_create(nxctl, nxprov_uuid, NEXUS_TYPE_UNDEFINED, NULL, NULL, NULL,
                 NULL, &err);
  lck_mtx_unlock(&nxctl->nxctl_lock);
  ...
  if (__improbable(err != 0)) {
    SK_PERR(p, "copyout err %d, nx_uuid %p", err, SK_KVA(uap->nx_uuid));
    goto done;
  }
...
int __nexus_destroy(struct proc *p, struct __nexus_destroy_args *uap,
                    int *retval) {
#pragma unused(retval)
  struct fileproc *__single fp;
  struct nxctl *nxctl = NULL;
  ...
  if (__improbable(uap->nx_uuid == USER_ADDR_NULL ||
                   uap->nx_uuid_len < sizeof(uuid_t))) {
    SK_PERR(p, "EINVAL: nx_uuid %p, nx_uuid_len %u", SK_KVA(uap->nx_uuid),
            uap->nx_uuid_len);
    return EINVAL;
  }
  ...
  if (__improbable(err != 0)) {
    SK_PERR(p, "copyin err %d, nx_uuid %p", err, SK_KVA(uap->nx_uuid));
    return err;
  }
...
int __nexus_get_opt(struct proc *p, struct __nexus_get_opt_args *uap,
                    int *retval) {
#pragma unused(retval)
  struct fileproc *__single fp;
  struct nxctl *nxctl = NULL;
  ...
  if (uap->aoptval != USER_ADDR_NULL) {
    ...
    if (__improbable(err != 0)) {
      SK_PERR(p, "copyin err %d, aoptlen %p", err, SK_KVA(uap->aoptlen));
      goto done;
    }
  } else {
    optlen = 0;
...
#if SK_LOG
    if (__improbable(err != 0)) {
      SK_PERR(p, "copyout err %d, aoptlen %p", err, SK_KVA(uap->aoptlen));
    }
#endif /* SK_LOG */
  }
...
int __nexus_set_opt(struct proc *p, struct __nexus_set_opt_args *uap,
                    int *retval) {
#pragma unused(retval)
  struct fileproc *__single fp;
  struct nxctl *nxctl = NULL;
