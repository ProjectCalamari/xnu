...
#define SK_ALIGN64_CASSERT(type, field)                                        \
  _Static_assert((__builtin_offsetof(type, field) % sizeof(uint64_t)) == 0,    \
                 "incorrect alignment")
...
#define IF_ATTACH_NX_FSW_TRANSPORT_NETAGENT                                    \
  0x04                                    /* enable fsw TCP/UDP netagent */
...
enum SK_VERB_FLAGS { SK_VERB_FLAGS_TABLE(EXPAND_TO_ENUMERATION) };
...
#define _SK_LOG(_type, _flag, _fmt, ...)                                       \
  do {                                                                         \
...
#define SK_ERR(_fmt, ...)                                                      \
  _SK_LOG(OS_LOG_TYPE_ERROR, SK_VERB_ERROR, _fmt, ##__VA_ARGS__)

/* error log with proc info (captured by default) */
#define SK_PERR(_p, _fmt, ...)                                                 \
  do {                                                                         \
...
#define SK_D(_fmt, ...)                                                        \
  _SK_LOG(OS_LOG_TYPE_DEFAULT, SK_VERB_DEFAULT, _fmt, ##__VA_ARGS__)

/* debug log (enabled sk_verbose flag) */
#define SK_DF(_flag, _fmt, ...)                                                \
  _SK_LOG(OS_LOG_TYPE_DEFAULT, (uint64_t)_flag, _fmt, ##__VA_ARGS__)

/* SK_DF with proc info */
#define SK_PDF(_flag, _p, _fmt, ...)                                           \
  do {                                                                         \
    SK_DF(_flag, "%s(%d): " _fmt, sk_proc_name(_p), sk_proc_pid(_p),           \
          ##__VA_ARGS__);                                                      \
  } while (0)

/* rate limited, lps indicates how many per second */
#define _SK_RD(_flag, _lps, _fmt, ...)                                         \
  do {                                                                         \
...
#define SK_RDF(_flag, _lps, _fmt, ...) _SK_RD(_flag, _lps, _fmt, ##__VA_ARGS__)
