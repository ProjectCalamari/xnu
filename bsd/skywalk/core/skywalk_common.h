...
#define SK_ABORT_WITH_CAUSE(msg, cause)                                        \
  do {                                                                         \
 ...
 */
__attribute__((always_inline)) static inline void __sk_copy64_8(uint64_t *src,
                                                                uint64_t *dst) {
  *dst = *src; /* [#0*8] */
}
 ...
 */
static inline void __sk_copy64_16(uint64_t *__counted_by(2) src,
                                  uint64_t *__counted_by(2) dst) {
  dst[0] = src[0]; /* [#0*8] */
  dst[1] = src[1]; /* [#1*8] */
}
...
__sk_vcopy64_16(uint64_t *__counted_by(2) src, uint64_t *__counted_by(2) dst) {
  ...
  /* BEGIN CSTYLED */
  __asm__ __volatile__("ldr	q0, [%[src]]		\n\t"
                       "str	q0, [%[dst]]		\n\t"
                       :
                       : [src] "r"((uint64_t *__unsafe_indexable)src),
                         [dst] "r"((uint64_t *__unsafe_indexable)dst)
                       : "v0", "memory");
  /* END CSTYLED */
}
...
__sk_vcopy64_32(uint64_t *__counted_by(4) src, uint64_t *__counted_by(4) dst) {
  /* no need to save/restore registers on arm64 (SPILL_REGISTERS) */
  /* BEGIN CSTYLED */
  __asm__ __volatile__("ldp	q0, q1, [%[src]]	\n\t"
                       "stp	q0, q1, [%[dst]]	\n\t"
                       :
                       : [src] "r"((uint64_t *__unsafe_indexable)src),
                         [dst] "r"((uint64_t *__unsafe_indexable)dst)
 ...
 */
__attribute__((always_inline)) static inline void __sk_zero_16(void *p) {
  /*
   * Use 16-bytes store pair using 64-bit zero register on arm64;
   * no need to save/restore registers on arm64 (SPILL_REGISTERS).
   */
  /* BEGIN CSTYLED */
  __asm__ __volatile__("stp	xzr, xzr, [%[p]]	\n\t"
                       :
                       : [p] "r"(p)
                       : "memory");
  /* END CSTYLED */
 ...
 */
__attribute__((always_inline)) static inline void __sk_zero_32(void *p) {
  /*
   * Use 32-bytes store pair using zeroed v0 register on arm64;
   * no need to save/restore registers on arm64 (SPILL_REGISTERS).
   */
  /* BEGIN CSTYLED */
  __asm__ __volatile__("eor.16b v0, v0, v0		\n\t"
                       "stp	 q0, q0, [%[p]]		\n\t"
                       :
                       : [p] "r"(p)
                       : "v0", "memory", "cc");
 ...
 */
__attribute__((always_inline)) static inline void __sk_zero_48(void *p) {
  /*
 ...
 */
__attribute__((always_inline)) static inline void __sk_zero_128(void *p) {
  /*
 ...
 */
__attribute__((always_inline)) static inline void
sk_copy64_4x(uint32_t *__sized_by(l) src, uint32_t *__sized_by(l) dst,
             size_t l) {
#if (DEVELOPMENT || DEBUG)
 ...
 */
__attribute__((always_inline)) static inline void
sk_copy64_8x(uint64_t *__sized_by(l) src, uint64_t *__sized_by(l) dst,
             size_t l) {
#if (DEVELOPMENT || DEBUG)
  if (__probable(l <= sk_copy_thres)) {
#else
 ...
 */
__attribute__((always_inline)) static inline void
sk_copy64_32x(uint64_t *__sized_by(l) src, uint64_t *__sized_by(l) dst,
              size_t l) {
#if (DEVELOPMENT || DEBUG)
  if (__probable(l <= sk_copy_thres)) {
#else
 ...
 */
__attribute__((always_inline)) static inline void
sk_copy64_64x(uint64_t *__sized_by(l) src, uint64_t *__sized_by(l) dst,
              size_t l) {
#if (DEVELOPMENT || DEBUG)
  if (__probable(l <= sk_copy_thres)) {
#else
...
#define _sk_alloc_type(probename, type, flags, name)                           \
  ({                                                                           \
    ...
    __auto_type ret = kalloc_type_tag(type, Z_ZERO | (flags), (name)->tag);    \
    DTRACE_SKYWALK3(probename, char *, #type, int, (flags), void *, ret);      \
    ret;                                                                       \
  })
...
#define _sk_alloc_type_array(probename, type, count, flags, name)              \
  ({                                                                           \
    __auto_type ret =                                                          \
        kalloc_type_tag(type, (count), Z_ZERO | (flags), (name)->tag);         \
    DTRACE_SKYWALK4(probename, char *, #type, size_t, (count), int, (flags),   \
                    void *, ret);                                              \
    ret;                                                                       \
  })
...
#define _sk_alloc_type_hash(probename, heap, size, flags, name)                \
  ({                                                                           \
    __auto_type ret = kalloc_type_var_impl(                                    \
        (heap), (size), __zone_flags_mix_tag((flags) | Z_ZERO, (name)->tag),   \
        NULL);                                                                 \
    DTRACE_SKYWALK4(probename, char *, (heap)->kt_name + 5, size_t, (size),    \
                    int, (flags), void *, ret);                                \
    ret;                                                                       \
  })

#define _sk_realloc_type_array(probename, type, oldcount, newcount, elem,      \
                               flags, name)                                    \
  ({                                                                           \
    __auto_type ret = krealloc_type_tag(type, (oldcount), (newcount), (elem),  \
                                        Z_ZERO | (flags), (name)->tag);        \
    DTRACE_SKYWALK5(probename, void *, (elem), size_t, (oldcount), size_t,     \
                    (newcount), int, (flags), void *, ret);                    \
    ret;                                                                       \
  })

#define _sk_alloc_type_header_array(probename, htype, type, count, flags,      \
                                    name)                                      \
  ({                                                                           \
    __auto_type ret =                                                          \
        kalloc_type_tag(htype, type, (count), Z_ZERO | (flags), (name)->tag);  \
    DTRACE_SKYWALK5(probename, char *, #htype, char *, #type, size_t, (count), \
                    int, (flags), void *, ret);                                \
    ret;                                                                       \
  })
...
#define _sk_free_type_array(probename, type, count, elem)                      \
  {                                                                            \
    DTRACE_SKYWALK3(probename, char *, #type, size_t, (count), void *,         \
                    (elem));                                                   \
...
#define _sk_free_type_hash(probename, heap, size, elem)                        \
  {                                                                            \
    DTRACE_SKYWALK3(probename, char *, (heap)->kt_name + 5, size_t, (size),    \
                    void *, (elem));                                           \
...
#define _sk_free_type_header_array(probename, htype, type, count, elem)        \
  {                                                                            \
    DTRACE_SKYWALK4(probename, char *, #htype, char *, #type, size_t, (count), \
                    void *, (elem));                                           \
...
#define _sk_alloc_data(probename, size, flags, name)                           \
  ({                                                                           \
    ...
    DTRACE_SKYWALK3(probename, size_t, (size), int, (flags), void *, ret);     \
    ret;                                                                       \
  })
...
#define _sk_realloc_data(probename, elem, oldsize, newsize, flags, name)       \
  ({                                                                           \
    void *ret;                                                                 \
                                                                               \
    ret = krealloc_data_tag((elem), (oldsize), (newsize), Z_ZERO | (flags),    \
                            (name)->tag);                                      \
    DTRACE_SKYWALK5(probename, void *, (elem), size_t, (oldsize), size_t,      \
                    (newsize), int, (flags), void *, ret);                     \
    ret;                                                                       \
  })
...
#define sk_alloc_type_header_array(htype, type, count, flags, tag)             \
  _sk_alloc_type_header_array(sk_alloc_type_header_array, htype, type, count,  \
                              flags, tag)

#define sk_realloc_type_array(type, oldsize, newsize, elem, flags, tag)        \
  _sk_realloc_type_array(sk_realloc_type_array, type, oldsize, newsize, elem,  \
                         flags, tag)

#define sk_free_type(type, elem) _sk_free_type(sk_free_type, type, elem)
...
#define sk_free_type_array_counted_by(type, count, elem)                       \
  _sk_free_type_array_counted_by(sk_free_type_array_counted_by, type, count,   \
                                 elem)
...
#define sk_free_type_header_array(htype, type, count, elem)                    \
  _sk_free_type_header_array(sk_free_type_header_array, htype, type, count,    \
                             elem)
...
#define sk_realloc_data(elem, oldsize, newsize, flags, tag)                    \
  _sk_realloc_data(sk_realloc_data, elem, oldsize, newsize, flags, tag)

#define sk_free_data(elem, size) _sk_free_data(sk_free_data, elem, size)
...
#define skn_realloc(name, elem, oldsize, newsize, flags, tag)                  \
  _sk_realloc(sk_realloc_##name, elem, oldsize, newsize, flags, tag)
...
#define skn_alloc_type_array(name, type, count, flags, tag)                    \
  _sk_alloc_type_array(sk_alloc_type_array_##name, type, count, flags, tag)

#define skn_alloc_type_hash(name, heap, size, flags, tag)                      \
  _sk_alloc_type_hash(sk_alloc_type_hash_##name, heap, size, flags, tag)

#define skn_alloc_type_header_array(name, htype, type, count, flags, tag)      \
  _sk_alloc_type_header_array(sk_alloc_type_header_array_##name, htype, type,  \
                              count, flags, tag)
...
#define skn_free_type_array(name, type, count, elem)                           \
  _sk_free_type_array(sk_free_type_array_##name, type, count, elem)

#define skn_free_type_array_counted_by(name, type, count, elem)                \
  _sk_free_type_array_counted_by(sk_free_type_array_##name, type, count, elem)
...
#define skn_free_type_header_array(name, htype, type, count, elem)             \
  _sk_free_type_header_array(sk_free_type_header_array_##name, htype, type,    \
                             count, elem)
...
#define skn_realloc_data(name, elem, oldsize, newsize, flags, tag)             \
  _sk_realloc_data(sk_realloc_data_##name, elem, oldsize, newsize, flags, tag)
 ...
 */
static inline int __sk_memcmp_mask_scalar(const uint8_t *__counted_by(n) src1,
                                          const uint8_t *__counted_by(n) src2,
                                          const uint8_t *__counted_by(n)
                                              byte_mask,
                                          size_t n) {
  uint32_t result = 0;
 ...
 *  @param n number of bytes
 */
static inline int __sk_memcmp_mask(const uint8_t *__counted_by(n) src1,
                                   const uint8_t *__counted_by(n) src2,
                                   const uint8_t *__counted_by(n) byte_mask,
                                   size_t n) {
  uint32_t result = 0;
  size_t i = 0;
  for (; i + 64 <= n; i += 64) {
    result |= sk_memcmp_mask_64B(src1 + i, src2 + i, byte_mask + i);
  }
  for (; i + 32 <= n; i += 32) {
    result |= sk_memcmp_mask_32B(src1 + i, src2 + i, byte_mask + i);
  }
  for (; i + 16 <= n; i += 16) {
    result |= sk_memcmp_mask_16B(src1 + i, src2 + i, byte_mask + i);
  }
  if (i < n) {
    if (n >= 16) {
      /* Compare the last 16 bytes with vector code. */
      result |=
          sk_memcmp_mask_16B(src1 + n - 16, src2 + n - 16, byte_mask + n - 16);
    } else {
