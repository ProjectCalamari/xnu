...
__attribute__((always_inline)) static inline struct sysctl_oid *
...
_skoid_oid_init(struct skoid *skoid, struct sysctl_oid *oid,
  ...
  if (&skoid->sko_oid == oid) {
    ...
    sko_name = tsnprintf(skoid->sko_name, sizeof(skoid->sko_name), "%s", name);
 ...
 */
void skoid_create(struct skoid *skoid, struct sysctl_oid_list *parent,
                  const char *name, int kind) {
  struct sysctl_oid *oid = &skoid->sko_oid;

  _skoid_oid_init(skoid, oid, parent, CTLTYPE_NODE | CTLFLAG_LOCKED | kind,
...
_skoid_add_property(struct skoid *skoid, const char *name, int kind,
                    const char *fmt, void *arg1, int arg2,
                    int(*handler) SYSCTL_HANDLER_ARGS) {
  struct sysctl_oid *oid;

  oid = _skoid_oid_alloc();
  _skoid_oid_init(skoid, oid, &skoid->sko_oid_list, CTLFLAG_LOCKED | kind, arg1,
                  arg2, name, handler, fmt);
  sysctl_register_oid(oid);
}
 ...
 */
void skoid_add_int(struct skoid *skoid, const char *name, int kind,
                   int *int_ptr) {
 ...
 */
void skoid_add_uint(struct skoid *skoid, const char *name, int kind,
 ...
 */
void skoid_add_handler(struct skoid *skoid, const char *name, int kind,
                       int(*proc) SYSCTL_HANDLER_ARGS, void *proc_arg1,
                       int proc_arg2) {
 ...
 */
void skoid_destroy(struct skoid *skoid) {
  /*
  ...
  SLIST_FOREACH_SAFE(oid, &skoid->sko_oid_list, oid_link, oid_tmp) {
    ...
    if ((oid->oid_kind & CTLTYPE) == CTLTYPE_NODE) {
      panic("leaked skoid sub-node detected %p %s", oid, oid->oid_name);
