...
SYSCTL_NODE(_kern, OID_AUTO, skywalk, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "Skywalk parameters");
SYSCTL_NODE(_kern_skywalk, OID_AUTO, stats, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "Skywalk statistics");
...
SYSCTL_UINT(_kern_skywalk, OID_AUTO, sk_opp_defunct,
            CTLFLAG_RW | CTLFLAG_LOCKED, &sk_opp_defunct, 0,
            "Defunct opportunistically");
SYSCTL_UINT(_kern_skywalk, OID_AUTO, sk_cksum_tx, CTLFLAG_RW | CTLFLAG_LOCKED,
            &sk_cksum_tx, 0,
            "Advertise (and perform) outbound checksum offload");
...
__startup_func void __sk_tag_make(const struct sk_tag_spec *spec) {
  *spec->skt_var = kern_allocation_name_allocate(spec->skt_name, 0);
}

boolean_t skywalk_netif_direct_enabled(void) { return sk_netif_direct_cnt > 0; }

boolean_t skywalk_netif_direct_allowed(const char *ifname) {
  uint32_t i;
...
#if (DEVELOPMENT || DEBUG)
static void parse_netif_direct(void) {
  ...
  curr = 0;
  len = strbuflen(buf);
  for (i = 0; i < len + 1 && sk_netif_direct_cnt < SK_NETIF_DIRECT_MAX; i++) {
    if (buf[i] != ',' && buf[i] != '\0') {
      continue;
    }
    ...
    if (iflen > 0 && iflen < IFXNAMSIZ) {
      (void)strbufcpy(sk_netif_direct[sk_netif_direct_cnt], IFXNAMSIZ,
                      buf + curr, IFXNAMSIZ);
      sk_netif_direct_cnt++;
    }
    curr = i + 1;
...
int skywalk_init(void) {
  ...
  (void)PE_parse_boot_argn("sk_cksum_tx", &sk_cksum_tx, sizeof(sk_cksum_tx));
  (void)PE_parse_boot_argn("sk_cksum_rx", &sk_cksum_rx, sizeof(sk_cksum_rx));
  (void)PE_parse_boot_argn("sk_txring_sz", &sk_txring_sz, sizeof(sk_txring_sz));
  (void)PE_parse_boot_argn("sk_rxring_sz", &sk_rxring_sz, sizeof(sk_rxring_sz));
  ...
  (void)PE_parse_boot_argn("sk_max_flows", &sk_max_flows, sizeof(sk_max_flows));
  ...
  (void)PE_parse_boot_argn("sk_compat_rx_mbq_limit",
                           &sk_netif_compat_rx_mbq_limit,
                           sizeof(sk_netif_compat_rx_mbq_limit));
  (void)PE_parse_boot_argn("sk_netif_tx_mit", &sk_netif_tx_mit,
                           sizeof(sk_netif_tx_mit));
  (void)PE_parse_boot_argn("sk_netif_rx_mit", &sk_netif_rx_mit,
                           sizeof(sk_netif_rx_mit));
  ...
  sk_fadv_nchunks =
      (uint32_t)P2ROUNDUP(sk_max_flows, FO_FLOWADV_CHUNK) / FO_FLOWADV_CHUNK;
  ...
  if (error == 0) {
    ...
    sk_dump_buf =
        sk_alloc_data(SK_DUMP_BUF_SIZE, Z_WAITOK | Z_NOFAIL, skmem_tag_dump);
 ...
 */
boolean_t skywalk_check_platform_binary(proc_t p) {
  return (csproc_get_platform_binary(p) == 0) ? FALSE : TRUE;
}

static int sk_priv_chk(proc_t p, kauth_cred_t cred, int priv) {
#pragma unused(p)
  int ret = EPERM;
  ...
  if (ret != 0) {
    ...
    if (__priv_check) {
      SK_DF(SK_VERB_PRIV,
            "%s(%d) insufficient privilege %d "
            "(\"%s\") err %d",
            sk_proc_name(p), sk_proc_pid(p), priv, pstr, ret);
    } else {
      SK_DF(SK_VERB_PRIV,
            "%s(%d) IGNORING missing privilege "
            "%d (\"%s\") err %d",
            sk_proc_name(p), sk_proc_pid(p), priv, pstr, ret);
    }
#endif /* SK_LOG */
...
int skywalk_priv_check_cred(proc_t p, kauth_cred_t cred, int priv) {
  return sk_priv_chk(p, cred, priv);
}

#if CONFIG_MACF
int skywalk_mac_system_check_proc_cred(proc_t p, const char *info_type) {
  int ret;
 ...
 */
int skywalk_nxctl_check_privileges(proc_t p, kauth_cred_t cred) {
  int ret = 0;
  ...
  do {
     ...
     */
    if ((ret = sk_priv_chk(p, cred, PRIV_SKYWALK_OBSERVE_STATS)) == 0) {
      break;
    }
    if ((ret = sk_priv_chk(p, cred, PRIV_SKYWALK_OBSERVE_ALL)) == 0) {
      break;
    }
    if ((ret = sk_priv_chk(p, cred, PRIV_SKYWALK_REGISTER_USER_PIPE)) == 0) {
      break;
    }
    if ((ret = sk_priv_chk(p, cred, PRIV_SKYWALK_REGISTER_KERNEL_PIPE)) == 0) {
      break;
    }
    if ((ret = sk_priv_chk(p, cred, PRIV_SKYWALK_REGISTER_NET_IF)) == 0) {
      break;
    }
    if ((ret = sk_priv_chk(p, cred, PRIV_SKYWALK_REGISTER_FLOW_SWITCH)) == 0) {
      break;
    }
    /* none set, so too bad */
    ret = EPERM;
  ...
  if (ret != 0) {
    SK_ERR("%s(%d) insufficient privilege to open nexus controller "
           "err %d",
           sk_proc_name(p), sk_proc_pid(p), ret);
  }
#endif /* !DEVELOPMENT && !DEBUG */
done:
  return ret;
...
void sk_gen_guard_id(boolean_t isch, const uuid_t uuid, guardid_t *guard) {
...
#define GUARD_NX_SIG 0x4e58 /* 'NX' */
  ...
  bcopy((void *)&__u._u64, guard, MIN(sizeof(*guard), sizeof(uint64_t)));
}

extern char *__counted_by(sizeof(uuid_string_t))
 ...
 *   length to be dumped.
 */
const char *__counted_by(SK_DUMP_BUF_SIZE)
    sk_dump(const char *label, const void *__sized_by(len) obj, int len,
            int dumplen) {
  int i, j, i0, n = 0;
 ...
 */
const char *sk_proc_name(struct proc *p) {
  if (p == PROC_NULL) {
    return "proc_null";
  }
...
const char *sk_ntop(int af, const void *addr,
                    char *__counted_by(addr_strlen) addr_str,
                    size_t addr_strlen) {
...
const char *sk_sa_ntop(struct sockaddr *sa,
                       char *__counted_by(addr_strlen) addr_str,
                       size_t addr_strlen) {
  const char *__null_terminated str = NULL;
  ...
  switch (sa->sa_family) {
  case AF_INET:
    str = sk_ntop(AF_INET, &SIN(sa)->sin_addr.s_addr, addr_str,
                  (socklen_t)addr_strlen);
    break;

  case AF_INET6:
    str = sk_ntop(AF_INET6, &SIN6(sa)->sin6_addr, addr_str,
                  (socklen_t)addr_strlen);
    break;
...
bool sk_sa_has_addr(struct sockaddr *sa) {
  switch (sa->sa_family) {
  case AF_INET:
...
bool sk_sa_has_port(struct sockaddr *sa) {
  switch (sa->sa_family) {
  case AF_INET:
    ASSERT(sa->sa_len == sizeof(struct sockaddr_in));
...
uint16_t sk_sa_get_port(struct sockaddr *sa) {
  switch (sa->sa_family) {
  case AF_INET:
    ASSERT(sa->sa_len == sizeof(struct sockaddr_in));
...
void skywalk_kill_process(struct proc *p, uint64_t reason_code) {
  ...
  if (exit_reason == OS_REASON_NULL) {
    ...
    SK_ERR("%s(%d) aborted for reason 0x%llX", sk_proc_name(p), sk_proc_pid(p),
           reason_code);
  }
...
#define SK_MEMCMP_MASK_VERIFY(t, l, lr)                                        \
  do {                                                                         \
...
#define SK_MEMCMP_MASK_MATCH_VERIFY(t, l)                                      \
  do {                                                                         \
    if (sk_memcmp_mask_##l##B(hdr1, hdr2, t##_m) != 0) {                       \
...
#define SK_MEMCMP_MASK_MISMATCH_VERIFY(t, l)                                   \
  do {                                                                         \
    if (sk_memcmp_mask_##l##B(hdr1, hdr2, t##_m) == 0) {                       \
      panic_plain("\nbroken: " #t " using sk_memcmp_mask_" #l                  \
...
#define SK_MEMCMP_BYTEMASK_VERIFY(t)                                           \
  do {                                                                         \
...
static inline int skywalk_memcmp_mask_ref(const uint8_t *__sized_by(n) src1,
                                          const uint8_t *__sized_by(n) src2,
                                          const uint8_t *__sized_by(n)
                                              byte_mask,
                                          size_t n) {
  uint32_t result = 0;
...
static void skywalk_memcmp_mask_self_tests(void) {
  ...
  static const uint8_t fk_t2_m[] = {
      0x0a, 0x00, 0x00, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  static const uint8_t fk_t3_m[] = {
      0x0f, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  static const uint8_t fk_i1_m[] = {
      0x02, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  static const uint8_t fk_i2_m[] = {
      0x07, 0x00, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  static const uint8_t fk_i3_m[] = {
      0x17, 0x00, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
      0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
