...
struct ns_flow_info {
  /* rule (flow) UUID */
  uuid_t nfi_flow_uuid __attribute((aligned(sizeof(uint64_t))));
 ...
 */
extern int netns_reserve(netns_token *token,
                         uint32_t *__sized_by(addr_len) addr, uint8_t addr_len,
                         uint8_t proto, in_port_t port, uint32_t flags,
...
/*
 * Access the flow info associated with a token, by filling out the local
 * struct. Returns 0 on success, or an error otherwise.
 */
...
extern int netns_change_addr(netns_token *token,
                             uint32_t *__sized_by(new_addr_len) new_addr,
 ...
 */
extern errno_t
netns_get_local_ports(ifnet_t ifp, protocol_family_t protocol, u_int32_t flags,
 ...
 */
extern uint32_t netns_find_anyres_byaddr(struct ifaddr *ifa, uint8_t proto);

/*
 * Return count of existing port reservations in the coresponding namespace,
 * IPv4.
 */
extern uint32_t netns_lookup_reservations_count_in(struct in_addr addr,
                                                   uint8_t proto);
 ...
 * IPv6.
 */
extern uint32_t netns_lookup_reservations_count_in6(struct in6_addr addr,
                                                    uint8_t proto);
...
netns_reserve_in(netns_token *token, struct in_addr addr, uint8_t proto,
  ...
  return netns_reserve(token, &addr.s_addr, sizeof(struct in_addr), proto, port,
                       flags, nfi);
}
...
netns_reserve_in6(netns_token *token, struct in6_addr addr, uint8_t proto,
  ...
  return netns_reserve(token, &addr.s6_addr32[0], sizeof(struct in6_addr),
                       proto, port, flags, nfi);
}

__attribute__((always_inline)) static inline int
netns_change_addr_in(netns_token *token, struct in_addr addr) {
  return netns_change_addr(token, &addr.s_addr, sizeof(struct in_addr));
}
...
netns_change_addr_in6(netns_token *token, struct in6_addr addr) {
  ...
  return netns_change_addr(token, &addr.s6_addr32[0], sizeof(struct in6_addr));
}
...
#define NETNS_CONFIGURATION_FLAGS                                              \
  (NETNS_NOWAKEFROMSLEEP | NETNS_RECVANYIF | NETNS_EXTBGIDLE |                 \
   NETNS_REUSEPORT | NETNS_CONNECTION_IDLE)
