...
#define PROTONS_LOCK() lck_mtx_lock(&protons_lock)
#define PROTONS_UNLOCK() lck_mtx_unlock(&protons_lock)
...
static struct protons_token *protons_token_alloc(bool can_block) {
...
static void protons_token_free(struct protons_token *pt) {
  PROTONS_LOCK_ASSERT_HELD();
...
bool protons_token_is_valid(struct protons_token *pt) {
...
bool protons_token_has_matching_pid(struct protons_token *pt, pid_t pid,
                                    pid_t epid) {
  ASSERT(pt != NULL);
  return pt->pt_pid == pid && pt->pt_epid == epid;
}

static struct protons_token *protons_find_token_with_protocol(uint8_t proto) {
...
int protons_token_get_use_count(struct protons_token *pt) {
  /* minus one refcnt in RB tree*/
  return os_ref_get_count(&pt->pt_refcnt) - 1;
}

static void protons_token_release(struct protons_token *pt) {
...
static int protons_reserve_locked(struct protons_token **ptp, pid_t pid,
                                  pid_t epid, uint8_t proto) {
  ...
  *ptp = NULL;
  ...
  if (pt != NULL) {
    ...
    if (pt->pt_pid != pid || pt->pt_epid != epid) {
      SK_ERR("proto %u existed with pid %d epid %d", proto, pt->pt_pid,
             pt->pt_epid);
      (void)os_ref_release(&pt->pt_refcnt);
      pt = NULL;
      return EEXIST;
...
int protons_reserve(struct protons_token **ptp, pid_t pid, pid_t epid,
                    uint8_t proto) {
  int err = 0;
  PROTONS_LOCK();
...
void protons_release(struct protons_token **ptp) {
...
static void protons_init_netinet_protocol(void) {
  PROTONS_LOCK();
