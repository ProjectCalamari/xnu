...
#define FLOWIDNS_FLOWID_SET_DOMAIN(_dom, _fid)                                 \
  do {                                                                         \
    (_fid) &= ~FLOWIDNS_FLOWID_DOMAIN_MASK;                                    \
    (_fid) |= ((_dom) << FLOWIDNS_FLOWID_DOMAIN_SHIFT);                        \
  } while (0)

#define FLOWIDNS_FLOWID_GET_DOMAIN(_dom, _fid)                                 \
  do {                                                                         \
...
static struct flowidns_flowid_tree_node *flowidns_fftn_alloc(bool can_block) {
...
static void flowidns_fftn_free(struct flowidns_flowid_tree_node *fftn) {
...
void flowidns_allocate_flowid(flowidns_domain_id_t domain,
                              struct flowidns_flow_key *pflow_key,
                              flowidns_flowid_t *pflowid) {
  ...
  VERIFY(domain >= FLOWIDNS_DOMAIN_MIN && domain <= FLOWIDNS_DOMAIN_MAX);
...
retry:
  ...
  if (dup != NULL) {
    ...
    SK_ERR("duplicate flowid 0x%x generated, retrying %d", flowid, retry_cnt);
    /*
...
void flowidns_release_flowid(flowidns_flowid_t flowid) {
...
int flowidns_init() {
  ...
  for (domain = FLOWIDNS_DOMAIN_MIN; domain <= FLOWIDNS_DOMAIN_MAX; domain++) {
    bzero(&flowidns_domain_array[domain], sizeof(struct flowidns_domain));
    flowidns_domain_array[domain].fd_id = domain;
    lck_mtx_init(&(flowidns_domain_array[domain].fd_mtx), &flowidns_lock_group,
                 NULL);
...
void flowidns_fini(void) {
  ...
  for (domain = FLOWIDNS_DOMAIN_MIN; domain <= FLOWIDNS_DOMAIN_MAX; domain++) {
    ...
    RB_FOREACH_SAFE(fftn, flowidns_flowid_tree,
                    &(flowidns_domain_array[domain].fd_flowid_tree), fftn_tmp) {
      RB_REMOVE(flowidns_flowid_tree,
                &(flowidns_domain_array[domain].fd_flowid_tree), fftn);
      flowidns_fftn_free(fftn);
    }
...
SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, flowidns,
            ...
            flowidns_stats_sysctl, "-",
            "flowid allocations (struct sk_stats_flowidns_header, "
            "skywalk/os_stats_private.h)");

static int flowidns_dump_domain(struct sysctl_req *req,
                                struct flowidns_domain *domain) {
  struct flowidns_flowid_tree_node *fftn;
  ...
  RB_FOREACH(fftn, flowidns_flowid_tree, &domain->fd_flowid_tree) {
    ...
    static_assert(sizeof(fftn->fftn_flowkey.ffk_laddr) ==
                  sizeof(record.sfr_laddr));
    static_assert(sizeof(fftn->fftn_flowkey.ffk_raddr) ==
