...
#define NS_VERB_PROTO(proto)                                                   \
  ((proto == IPPROTO_TCP) ? SK_VERB_NS_TCP : SK_VERB_NS_UDP)
#define NS_VERB_IP(addr_len)                                                   \
  ((addr_len == sizeof(struct in_addr)) ? SK_VERB_NS_IPV4 : SK_VERB_NS_IPV6)
#define PROTO_STR(proto) ((proto == IPPROTO_TCP) ? "tcp" : "udp")
#define LEN_TO_AF(len) (((len == sizeof(struct in_addr)) ? AF_INET : AF_INET6))
#define NS_PORT_ERR(_fmt, ...)                                                 \
  do {                                                                         \
    proc_t _p = current_proc();                                                \
    SK_ERR("%s(%d) port %u: " _fmt, sk_proc_name(_p), sk_proc_pid(_p), port,   \
           ##__VA_ARGS__);                                                     \
  } while (0);
...
#define NETNS_LOCK() lck_mtx_lock(&netns_lock)
#define NETNS_LOCK_SPIN() lck_mtx_lock_spin(&netns_lock)
#define NETNS_LOCK_CONVERT()                                                   \
  do {                                                                         \
    NETNS_LOCK_ASSERT_HELD();                                                  \
    lck_mtx_convert_spin(&netns_lock);                                         \
  } while (0)
#define NETNS_UNLOCK() lck_mtx_unlock(&netns_lock)
...
ns_reservation_tree_find(struct ns_reservation_tree *, const in_port_t);
...
RB_HEAD(netns_namespaces_tree, ns)
netns_namespaces = RB_INITIALIZER(netns_namespaces);
...
LIST_HEAD(, ns_token)
netns_unbound_tokens = LIST_HEAD_INITIALIZER(netns_unbound_tokens);

/* List of all tokens currently allocated in the system */
LIST_HEAD(, ns_token)
netns_all_tokens = LIST_HEAD_INITIALIZER(netns_all_tokens);
 ...
 */
static struct ns *_netns_get_ns(uint32_t *__sized_by(addr_len),
                                uint8_t addr_len, uint8_t, bool);
static inline boolean_t
_netns_is_wildcard_addr(const uint32_t *__sized_by(addr_len), uint8_t addr_len);
...
static int _netns_reserve_kpi_common(struct ns *, netns_token *,
                                     uint32_t *__sized_by(addr_len),
                                     uint8_t addr_len, uint8_t, in_port_t *,
                                     uint32_t, struct ns_flow_info *);
static void _netns_set_ifnet_internal(struct ns_token *, struct ifnet *);

static struct ns_reservation *netns_ns_reservation_alloc(in_port_t port,
                                                         uint32_t flags) {
...
static void netns_ns_reservation_free(struct ns_reservation *res) {
  skmem_cache_free(netns_ns_reservation_cache, res);
}

static struct ns *netns_ns_alloc(zalloc_flags_t how) {
  struct ns *namespace;
...
static void netns_ns_free(struct ns *namespace) {
  struct ns_reservation *res;
  ...
  SK_DF(NS_VERB_IP(namespace->ns_addr_len) | NS_VERB_PROTO(namespace->ns_proto),
        "freeing %s ns for IP %s", PROTO_STR(namespace->ns_proto),
        sk_ntop(LEN_TO_AF(namespace->ns_addr_len), namespace->ns_addr,
  ...
  RB_FOREACH_SAFE(res, ns_reservation_tree, &namespace->ns_reservations,
    ...
    RB_REMOVE(ns_reservation_tree, &namespace->ns_reservations, res);
  }

  VERIFY(RB_EMPTY(&namespace->ns_reservations));

  if (netns_global_wild[NETNS_NS_GLOBAL_IDX(
          namespace->ns_proto, namespace->ns_addr_len)] == namespace) {
    netns_global_wild[NETNS_NS_GLOBAL_IDX(namespace->ns_proto,
                                          namespace->ns_addr_len)] = NULL;
  }
  if (netns_global_non_wild[NETNS_NS_GLOBAL_IDX(
          namespace->ns_proto, namespace->ns_addr_len)] == namespace) {
...
static void netns_ns_cleanup(struct ns *namespace) {
  if (namespace->ns_is_freeable && RB_EMPTY(&namespace->ns_reservations)) {
...
static struct ns_token *netns_ns_token_alloc(boolean_t with_nfi) {
  ...
  if (with_nfi) {
    token->nt_flow_info =
        skmem_cache_alloc(netns_ns_flow_info_cache, SKMEM_SLEEP);
...
static void netns_ns_token_free(struct ns_token *token) {
  NETNS_LOCK_ASSERT_HELD();
  NETNS_LOCK_CONVERT();
...
__attribute__((always_inline)) static inline int ns_cmp(const struct ns *a,
                                                        const struct ns *b) {
  int d;
  ...
  if ((d = flow_ip_cmp(a->ns_addr_key, b->ns_addr_key, b->ns_addr_len)) != 0) {
    return d;
  }
 ...
 */
__attribute__((always_inline)) static inline struct ns_reservation *
ns_reservation_tree_find(struct ns_reservation_tree *tree,
 ...
 * created.
 */
static struct ns *_netns_get_ns(uint32_t *__sized_by(addr_len) addr,
                                uint8_t addr_len, uint8_t proto, bool create) {
  ...
  if (create && namespace == NULL) {
    SK_DF(NS_VERB_IP(addr_len) | NS_VERB_PROTO(proto),
          "allocating %s ns for IP %s", PROTO_STR(proto),
          sk_ntop(LEN_TO_AF(addr_len), addr, tmp_ip_str, sizeof(tmp_ip_str)));
    NETNS_LOCK_CONVERT();
    ...
    if (_netns_is_wildcard_addr(addr, addr_len) &&
        netns_global_wild[NETNS_NS_GLOBAL_IDX(proto, addr_len)] == NULL) {
      netns_global_wild[NETNS_NS_GLOBAL_IDX(proto, addr_len)] = namespace;
    }
  }
...
_netns_is_wildcard_addr(const uint32_t *__sized_by(addr_len) addr,
                        uint8_t addr_len) {
  boolean_t wildcard;
  ...
  case sizeof(struct in6_addr):
    wildcard = (addr[0] == 0 && addr[1] == 0 && addr[2] == 0 && addr[3] == 0);
    break;
...
__attribute__((always_inline)) static boolean_t
_netns_is_port_used(struct ns *gns, struct ns_reservation *curr_res,
                    in_port_t port) {
 ...
 */
static int _netns_reserve_common(struct ns *namespace, in_port_t port,
                                 uint32_t flags) {
  ...
  exist = RB_INSERT(ns_reservation_tree, &namespace->ns_reservations, res);
  ...
  SK_DF(NS_VERB_IP(addr_len) | NS_VERB_PROTO(proto),
        ...
        sk_ntop(LEN_TO_AF(namespace->ns_addr_len), namespace->ns_addr,
                tmp_ip_str, sizeof(tmp_ip_str)),
        PROTO_STR(proto), port, flags, NETNS_REF_COUNT(res, NETNS_SKYWALK),
        NETNS_REF_COUNT(res, NETNS_LISTENER), NETNS_REF_COUNT(res, NETNS_BSD),
  ...
  if (namespace !=
    ...
    boolean_t is_wild = _netns_is_wildcard_addr(namespace->ns_addr, addr_len);
    struct ns *gns = netns_global_wild[NETNS_NS_GLOBAL_IDX(proto, addr_len)];

    if (NETNS_IS_SKYWALK(flags)) {
      if ((!is_wild || exist != NULL) && gns != NULL &&
          (skres = ns_reservation_tree_find(&gns->ns_reservations, port)) !=
              NULL &&
      ...
      if (is_wild) {
        gns = netns_global_non_wild[NETNS_NS_GLOBAL_IDX(proto, addr_len)];
        VERIFY(gns != NULL);

        if (_netns_is_port_used(netns_global_non_wild[NETNS_NS_GLOBAL_IDX(
                                    proto, NETNS_ADDRLEN_V4)],
                                res, port) ||
            _netns_is_port_used(netns_global_non_wild[NETNS_NS_GLOBAL_IDX(
                                    proto, NETNS_ADDRLEN_V6)],
                                res, port)) {
          /*
      ...
      if (is_wild) {
         ...
         * intentional.
         */
        if ((skres = ns_reservation_tree_find(&gns->ns_reservations, port)) !=
                NULL &&
            (NETNS_REF_COUNT(skres, NETNS_SKYWALK) |
             NETNS_REF_COUNT(skres, NETNS_LISTENER)) != 0) {
    ...
    case NETNS_SKYWALK:
      ...
      if (NETNS_REF_COUNT(res, NETNS_LISTENER) == 0 &&
        ...
        gns = netns_global_wild[NETNS_NS_GLOBAL_IDX(proto, addr_len)];
        if (gns != NULL &&
            (skres = ns_reservation_tree_find(&gns->ns_reservations, port)) !=
                NULL &&
            NETNS_REF_COUNT(skres, NETNS_LISTENER) != 0) {
        ...
        if (addr_len == sizeof(struct in_addr)) {
          ...
          gns = netns_global_wild[NETNS_NS_GLOBAL_IDX(proto, NETNS_ADDRLEN_V6)];
          if (gns != NULL &&
              (skres = ns_reservation_tree_find(&gns->ns_reservations, port)) !=
                  NULL &&
              NETNS_REF_COUNT(skres, NETNS_LISTENER) != 0) {
    ...
    case NETNS_LISTENER:
      if (NETNS_REF_COUNT(res, NETNS_BSD) > 0 ||
          ...
          NETNS_REF_COUNT(res, NETNS_LISTENER) > 0 ||
          _netns_is_port_used(
              netns_global_wild[NETNS_NS_GLOBAL_IDX(proto, NETNS_ADDRLEN_V4)],
              res, port) ||
          _netns_is_port_used(
              netns_global_wild[NETNS_NS_GLOBAL_IDX(proto, NETNS_ADDRLEN_V6)],
              res, port) ||
          _netns_is_port_used(netns_global_non_wild[NETNS_NS_GLOBAL_IDX(
                                  proto, NETNS_ADDRLEN_V4)],
                              res, port) ||
    ...
    case NETNS_PF:
      if (NETNS_REF_COUNT(res, NETNS_SKYWALK) > 0 ||
        ...
        NS_PORT_ERR("ERROR - %s got ADDRINUSE",
                    ((flags & NETNS_OWNER_MASK) == NETNS_PF) ? "PF" : "BSD");
        err = EADDRINUSE;
      }
      break;
 ...
 */
static void _netns_release_common(struct ns *namespace, in_port_t port,
                                  uint32_t flags) {
  struct ns_reservation *res;
  uint32_t refs;
  int i;
...
netns_clear_ifnet(struct ns_token *nstoken) {
  ...
  if (nstoken->nt_ifp != NULL) {
    ...
    SK_DF(NS_VERB_IP(nstoken->nt_addr_len) | NS_VERB_PROTO(nstoken->nt_proto),
          "%s:%s:%d // removed from ifnet %d",
          sk_ntop(LEN_TO_AF(nstoken->nt_addr_len), nstoken->nt_addr, tmp_ip_str,
                  sizeof(tmp_ip_str)),
 ...
 */
static int _netns_reserve_kpi_common(struct ns *ns, netns_token *token,
                                     uint32_t *__sized_by(addr_len) addr,
                                     uint8_t addr_len, uint8_t proto,
                                     in_port_t *port, uint32_t flags,
  ...
  SK_DF(NS_VERB_IP(addr_len) | NS_VERB_PROTO(proto),
        ...
        sk_ntop(LEN_TO_AF(addr_len), addr, tmp_ip_str, sizeof(tmp_ip_str)),
        PROTO_STR(proto), hport, flags, NETNS_TOKEN_VALID(token) ? "" : "in");
  ...
  if (NETNS_TOKEN_VALID(token)) {
    if (flags & NETNS_PRERESERVED) {
      ...
      VERIFY((nt->nt_flags & NETNS_RESERVATION_FLAGS | NETNS_PRERESERVED) ==
      ...
      if ((nt->nt_flags & NETNS_CONFIGURATION_FLAGS) ==
        ...
        SK_DF(NS_VERB_IP(nt->nt_addr_len) | NS_VERB_PROTO(nt->nt_proto),
              ...
              sk_ntop(LEN_TO_AF(nt->nt_addr_len), nt->nt_addr, tmp_ip_str,
                      sizeof(tmp_ip_str)),
              PROTO_STR(nt->nt_proto), nt->nt_port, nt->nt_flags, flags);
        nt->nt_flags &= ~NETNS_CONFIGURATION_FLAGS;
        nt->nt_flags |= flags & NETNS_CONFIGURATION_FLAGS;
      }
      SK_DF(NS_VERB_IP(addr_len) | NS_VERB_PROTO(proto),
  ...
  if (!_netns_is_wildcard_addr(ns->ns_addr, addr_len)) {
    ...
    nwns = netns_global_non_wild[NETNS_NS_GLOBAL_IDX(proto, addr_len)];
...
int netns_init(void) {
  ...
  netns_ns_reservation_cache = skmem_cache_create(
      NETNS_NS_RESERVATION_ZONE_NAME, netns_ns_reservation_size,
  ...
  netns_ns_token_cache =
      skmem_cache_create(NETNS_NS_TOKEN_ZONE_NAME, netns_ns_token_size,
                         sizeof(uint64_t), NULL, NULL, NULL, NULL, NULL, 0);
  ...
  netns_ns_flow_info_cache =
      skmem_cache_create(NETNS_NS_FLOW_INFO_ZONE_NAME, netns_ns_flow_info_size,
                         sizeof(uint64_t), NULL, NULL, NULL, NULL, NULL, 0);
  ...
  netns_init_global_ns(&netns_global_non_wild[NETNS_NS_GLOBAL_IDX(
                           IPPROTO_TCP, NETNS_ADDRLEN_V4)],
                       IPPROTO_TCP, sizeof(struct in_addr));

  netns_init_global_ns(&netns_global_non_wild[NETNS_NS_GLOBAL_IDX(
                           IPPROTO_UDP, NETNS_ADDRLEN_V4)],
                       IPPROTO_UDP, sizeof(struct in_addr));

  netns_init_global_ns(&netns_global_non_wild[NETNS_NS_GLOBAL_IDX(
                           IPPROTO_TCP, NETNS_ADDRLEN_V6)],
                       IPPROTO_TCP, sizeof(struct in6_addr));

  netns_init_global_ns(&netns_global_non_wild[NETNS_NS_GLOBAL_IDX(
                           IPPROTO_UDP, NETNS_ADDRLEN_V6)],
                       IPPROTO_UDP, sizeof(struct in6_addr));
...
void netns_uninit(void) {
  if (__netns_inited == 1) {
    struct ns *namespace;
    struct ns *temp_namespace;
    int i;

    RB_FOREACH_SAFE(namespace, netns_namespaces_tree, &netns_namespaces,
                    temp_namespace) {
      RB_REMOVE(netns_namespaces_tree, &netns_namespaces, namespace);
      netns_n_namespaces--;
      netns_ns_free(namespace);
...
void netns_reap_caches(boolean_t purge) {
...
boolean_t netns_is_enabled(void) { return __netns_inited == 1; }

int netns_reserve(netns_token *token, uint32_t *__sized_by(addr_len) addr,
                  uint8_t addr_len, uint8_t proto, in_port_t port,
                  uint32_t flags, struct ns_flow_info *nfi) {
  int err = 0;
#if SK_LOG
  char tmp_ip_str[MAX_IPv6_STR_LEN];
#endif /* SK_LOG */
  ...
  SK_DF(NS_VERB_IP(addr_len) | NS_VERB_PROTO(proto), "%s:%s:%d // flags 0x%x",
        sk_ntop(LEN_TO_AF(addr_len), addr, tmp_ip_str, sizeof(tmp_ip_str)),
        PROTO_STR(proto), ntohs(port), flags);

  /*
   * Check wether the process is allowed to bind to a restricted port
   */
  if (!current_task_can_use_restricted_in_port(port, proto, flags)) {
    *token = NULL;
    return EADDRINUSE;
  }

  NETNS_LOCK_SPIN();
  err = _netns_reserve_kpi_common(NULL, token, addr, addr_len, proto, &port,
                                  flags, nfi);
  NETNS_UNLOCK();
...
int netns_reserve_ephemeral(netns_token *token,
                            uint32_t *__sized_by(addr_len) addr,
                            uint8_t addr_len, uint8_t proto, in_port_t *pport,
                            uint32_t flags, struct ns_flow_info *nfi) {
  int err = 0;
  ...
  boolean_t use_randomport =
      (proto == IPPROTO_TCP) ? tcp_use_randomport : udp_use_randomport;
#if SK_LOG
  char tmp_ip_str[MAX_IPv6_STR_LEN];
#endif /* SK_LOG */
  ...
  if (first == last) {
    ...
    if (use_randomport) {
      ...
      if (first > last) {
        rand_port = namespace->ns_last_ephemeral_port_down - 1;
        if (rand_port < last || rand_port > first) {
          rand_port = last;
        }
        count_up = false;
      } else {
        rand_port = namespace->ns_last_ephemeral_port_up + 1;
  ...
  while (true) {
    ...
    if (!IS_RESTRICTED_IN_PORT(n_last_port)) {
      err = _netns_reserve_kpi_common(namespace, token, addr, addr_len, proto,
                                      &n_last_port, flags, nfi);
...
void netns_release(netns_token *token) {
  struct ns *ns;
  struct ns_token *nt;
  uint8_t proto, addr_len;
#if SK_LOG
  ...
  *token = NULL;
  ...
  SK_DF(NS_VERB_IP(addr_len) | NS_VERB_PROTO(proto), "releasing %s:%s:%d",
        sk_ntop(LEN_TO_AF(nt->nt_addr_len), nt->nt_addr, tmp_ip_str,
                sizeof(tmp_ip_str)),
        PROTO_STR(proto), nt->nt_port);

  if (!_netns_is_wildcard_addr(nt->nt_addr, addr_len)) {
    /* Remove from global non-wild namespace */

    ns = netns_global_non_wild[NETNS_NS_GLOBAL_IDX(proto, addr_len)];
    VERIFY(ns != NULL);
...
int netns_change_addr(netns_token *token, uint32_t *__sized_by(addr_len) addr,
                      uint8_t addr_len) {
  int err = 0;
...
#if SK_LOG
  sk_ntop(LEN_TO_AF(nt->nt_addr_len), nt->nt_addr, tmp_ip_str_1,
          sizeof(tmp_ip_str_1));
  sk_ntop(LEN_TO_AF(addr_len), addr, tmp_ip_str_2, sizeof(tmp_ip_str_2));
#endif /* SK_LOG */
  SK_DF(NS_VERB_IP(addr_len) | NS_VERB_PROTO(proto),
        "changing address for %s:%d from %s to %s", PROTO_STR(proto),
        nt->nt_port, tmp_ip_str_1, tmp_ip_str_2);
  ...
  old_namespace = _netns_get_ns(nt->nt_addr, nt->nt_addr_len, proto, false);
  ...
  if ((err = _netns_reserve_common(new_namespace, nt->nt_port, nt->nt_flags))) {
    NETNS_LOCK_CONVERT();
    netns_ns_cleanup(new_namespace);
    SK_ERR("port %u reservation collision under new namespace", nt->nt_port);
    goto done;
  }
  ...
  if (!_netns_is_wildcard_addr(nt->nt_addr, nt->nt_addr_len)) {
     ...
     */
    global_namespace =
        netns_global_non_wild[NETNS_NS_GLOBAL_IDX(proto, nt->nt_addr_len)];
    VERIFY(global_namespace != NULL);

    _netns_release_common(global_namespace, nt->nt_port, nt->nt_flags);
  }
  ...
  if (!_netns_is_wildcard_addr(addr, addr_len)) {
    ...
    if ((err = _netns_reserve_common(global_namespace, nt->nt_port,
                                     nt->nt_flags)) != 0) {
...
static void _netns_set_ifnet_internal(struct ns_token *nt, struct ifnet *ifp) {
#if SK_LOG
  char tmp_ip_str[MAX_IPv6_STR_LEN];
#endif /* SK_LOG */
  ...
  if (ifp != NULL && ifnet_get_ioref(ifp)) {
    ...
    SK_DF(NS_VERB_IP(nt->nt_addr_len) | NS_VERB_PROTO(nt->nt_proto),
          ...
          sk_ntop(LEN_TO_AF(nt->nt_addr_len), nt->nt_addr, tmp_ip_str,
                  sizeof(tmp_ip_str)),
          PROTO_STR(nt->nt_proto), nt->nt_port, ifp->if_index);
  } else {
    LIST_INSERT_HEAD(&netns_unbound_tokens, nt, nt_ifp_link);
  }
}

void netns_set_ifnet(netns_token *token, ifnet_t ifp) {
  struct ns_token *nt;
#if SK_LOG
  char tmp_ip_str[MAX_IPv6_STR_LEN];
#endif /* SK_LOG */
  ...
  if (nt->nt_ifp == ifp) {
    SK_DF(NS_VERB_IP(nt->nt_addr_len) | NS_VERB_PROTO(nt->nt_proto),
          ...
          sk_ntop(LEN_TO_AF(nt->nt_addr_len), nt->nt_addr, tmp_ip_str,
                  sizeof(tmp_ip_str)),
          PROTO_STR(nt->nt_proto), nt->nt_port, ifp ? ifp->if_index : -1);
    NETNS_UNLOCK();
    return;
  }
...
void netns_ifnet_detach(ifnet_t ifp) {
  ...
  LIST_FOREACH_SAFE(token, &ifp->if_netns_tokens, nt_ifp_link, tmp_token) {
...
static void _netns_set_state(netns_token *token, uint32_t state) {
  struct ns_token *nt;
#if SK_LOG
  char tmp_ip_str[MAX_IPv6_STR_LEN];
#endif /* SK_LOG */
...
void netns_half_close(netns_token *token) {
  _netns_set_state(token, NETNS_STATE_HALFCLOSED);
}

void netns_withdraw(netns_token *token) {
  _netns_set_state(token, NETNS_STATE_WITHDRAWN);
}

int netns_get_flow_info(netns_token *token, struct ns_flow_info *nfi) {
  if (__netns_inited == 0) {
    return ENOTSUP;
  }

  NETNS_LOCK();
  if (!NETNS_TOKEN_VALID(token) || nfi == NULL) {
    NETNS_UNLOCK();
    return EINVAL;
  }
...
void netns_change_flags(netns_token *token, uint32_t set_flags,
 ...
 */
static inline void
netns_local_port_scan_flow_entry(struct flow_entry *fe,
                                 protocol_family_t protocol, u_int32_t flags,
  ...
  if (token->nt_addr_len == sizeof(struct in_addr)) {
    ...
    iswildcard = IN6_IS_ADDR_UNSPECIFIED(&token->nt_in6addr);
  }
  ...
  if (token->nt_ifp != NULL && (token->nt_ifp->if_eflags & IFEF_AWDL) != 0) {
    ...
    if (fr == NULL || fr->fr_rt_dst == NULL ||
      ...
      if (protocol == PF_INET) {
        sk_ntop(PF_INET, &nfi->nfi_laddr.sin.sin_addr, lbuf, sizeof(lbuf));
        sk_ntop(PF_INET, &nfi->nfi_faddr.sin.sin_addr, fbuf, sizeof(fbuf));
        lport = nfi->nfi_laddr.sin.sin_port;
        fport = nfi->nfi_faddr.sin.sin_port;
      } else {
        sk_ntop(PF_INET6, &nfi->nfi_laddr.sin6.sin6_addr.s6_addr, lbuf,
                sizeof(lbuf));
        sk_ntop(PF_INET6, &nfi->nfi_faddr.sin6.sin6_addr, fbuf, sizeof(fbuf));
        lport = nfi->nfi_laddr.sin6.sin6_port;
        fport = nfi->nfi_faddr.sin6.sin6_port;
      }

      os_log(wake_packet_log_handle,
             "netns_local_port_scan_flow_entry: route is down %s %s:%u %s:%u "
             "ifp %s proc %s:%d",
             token->nt_proto == IPPROTO_TCP ? "tcp" : "udp", lbuf, ntohs(lport),
             fbuf, ntohs(fport), token->nt_ifp->if_xname, pname,
             nfi->nfi_owner_pid);
  ...
  if (!(flags & IFNET_GET_LOCAL_PORTS_NOWAKEUPOK) &&
    ...
    os_log(wake_packet_log_handle,
           "netns_local_port_scan_flow_entry: no wake from sleep %s %s:%u "
           "%s:%u ifp %s proc %s:%d",
           token->nt_proto == IPPROTO_TCP ? "tcp" : "udp", lbuf, ntohs(lport),
           fbuf, ntohs(fport),
           token->nt_ifp != NULL ? token->nt_ifp->if_xname : "", pname,
           nfi->nfi_owner_pid);
  }
#endif /* DEBUG || DEVELOPMENT */
  ...
  if (token->nt_ifp != NULL && token->nt_flow_info != NULL) {
    /*
     * When the flow has "no wake from sleep" option, do not set the port in the
     * bitmap except if explicetely requested by the driver. We always add the
     * flow to the list of port in order to report spurious wakes
     */
    if ((flags & IFNET_GET_LOCAL_PORTS_NOWAKEUPOK) ||
...
netns_get_if_local_ports(ifnet_t ifp, protocol_family_t protocol,
  ...
  flow_mgr_foreach_flow(fsw->fsw_flow_mgr, ^(struct flow_entry *_fe) {
    netns_local_port_scan_flow_entry(_fe, protocol, flags, bitfield);
  });
  NETNS_UNLOCK();
  FSW_UNLOCK(fsw);
done:
...
errno_t netns_get_local_ports(ifnet_t ifp, protocol_family_t protocol,
                              u_int32_t flags,
                              u_int8_t bitfield[IP_PORTRANGE_BITFIELD_LEN]) {
  if (__netns_inited == 0) {
    return 0;
  ...
  if (ifp != NULL) {
    ...
    for (i = 0; i < count; i++) {
      ...
      netns_get_if_local_ports(ifp_list[i], protocol, flags, bitfield);
    }
...
uint32_t netns_find_anyres_byaddr(struct ifaddr *ifa, uint8_t proto) {
  int result = 0;
  int ifa_addr_len;
  struct ns_token *token;
  ...
  ifa_addr_len = (ifa_addr->sa_family == AF_INET) ? sizeof(struct in_addr)
  ...
  LIST_FOREACH(token, &ifp->if_netns_tokens, nt_ifp_link) {
    ...
    if (ifa_addr->sa_family == AF_INET) {
      ...
      if (IN6_ARE_ADDR_EQUAL(IFA_IN6(ifa), &token->nt_in6addr)) {
        result = 1;
        break;
      }
    }
...
static uint32_t _netns_lookup_ns_n_reservations(uint32_t *__sized_by(addr_len)
                                                    addr,
                                                uint8_t addr_len,
                                                uint8_t proto) {
...
uint32_t netns_lookup_reservations_count_in(struct in_addr addr,
                                            uint8_t proto) {
  return _netns_lookup_ns_n_reservations(&addr.s_addr, sizeof(struct in_addr),
                                         proto);
}

uint32_t netns_lookup_reservations_count_in6(struct in6_addr addr,
                                             uint8_t proto) {
  if (IN6_IS_SCOPE_EMBED(&addr)) {
    addr.s6_addr16[1] = 0;
  }
  return _netns_lookup_ns_n_reservations(&addr.s6_addr32[0],
                                         sizeof(struct in6_addr), proto);
}
...
SYSCTL_NODE(_kern_skywalk, OID_AUTO, netns, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "Netns interface");
...
static int netns_ctl_write_ns(struct sysctl_req *req, struct ns *namespace,
                              boolean_t is_global) {
  struct ns_reservation *res;
  ...
  memcpy(response_header.ncdh_addr, namespace->ns_addr, namespace->ns_addr_len);
  ...
  RB_FOREACH(res, ns_reservation_tree, &namespace->ns_reservations) {
    ...
    response_record.ncdr_listener_refs = NETNS_REF_COUNT(res, NETNS_LISTENER);
    response_record.ncdr_skywalk_refs = NETNS_REF_COUNT(res, NETNS_SKYWALK);
    response_record.ncdr_bsd_refs = NETNS_REF_COUNT(res, NETNS_BSD);
    response_record.ncdr_pf_refs = NETNS_REF_COUNT(res, NETNS_PF);
    err = SYSCTL_OUT(req, &response_record, sizeof(response_record));
    if (err) {
      return err;
    }
  }
