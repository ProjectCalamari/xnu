...
#define SKMEM_SYSCTL_KERN_IPC_LIST X(uint32_t, throttle_best_effort, 0)
...
#define SYSCTL_SKMEM_UPDATE_AT_OFFSET(offset, value)                           \
  do {                                                                         \
    if (offset >= 0 &&                                                         \
      ...
      skmem_sysctl *__single swptr = skmem_get_sysctls_obj(NULL);              \
      void *offp =                                                             \
          (u_int8_t * __bidi_indexable)(skmem_sysctl * __bidi_indexable)       \
              swptr +                                                          \
          offset;                                                              \
      if (swptr && ((uintptr_t)offp) % _Alignof(typeof(value)) == 0) {         \
...
#define SYSCTL_SKMEM_INT(parent, oid, sysctl_name, access, ptr, offset, descr) \
  SYSCTL_OID(parent, oid, sysctl_name, CTLTYPE_INT | access, ptr, offset,      \
             skmem_sysctl_handle_int, "I", descr);                             \
...
#define SYSCTL_SKMEM_TCP_INT(oid, sysctl_name, access, variable_type,          \
  ...
  SYSCTL_SKMEM_INT(_net_inet_tcp, oid, sysctl_name, access, &variable_name,    \
                   offsetof(skmem_sysctl, tcp.sysctl_name), descr)

#define SYSCTL_SKMEM_KERN_IPC_INT(oid, sysctl_name, access, variable_type,     \
                                  variable_name, initial_value, descr)         \
  variable_type variable_name = initial_value;                                 \
  SYSCTL_SKMEM_INT(_kern_ipc, oid, sysctl_name, access, &variable_name,        \
                   offsetof(skmem_sysctl, kern.ipc.sysctl_name), descr)
#endif /* KERNEL */
