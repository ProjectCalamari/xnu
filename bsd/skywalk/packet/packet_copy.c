...
SYSCTL_NODE(_kern_skywalk, OID_AUTO, packet, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "Skywalk packet");
 ...
 */
void pkt_copy_from_pkt(const enum txrx t, kern_packet_t dph,
                       const uint16_t doff, kern_packet_t sph,
                       const uint16_t soff, const uint32_t len,
  ...
  case NR_RX:
    ...
    if (__probable(do_sum)) {
      ...
      partial = __packet_copy_and_sum((sbaddr + start), (dbaddr + start),
                                      (len - start), 0);
      csum = __packet_fold_sum(partial);

      __packet_set_inet_checksum(dph, PACKET_CSUM_PARTIAL, start, csum, FALSE);
    } else {
    ...
    SK_DF(SK_VERB_COPY | SK_VERB_RX,
          ...
          sk_proc_name(current_proc()), sk_proc_pid(current_proc()), len,
          (copysum ? (len - start) : 0), csum, start);
    SK_DF(SK_VERB_COPY | SK_VERB_RX,
          "   pkt  %p doff %u csumf/rxstart/rxval 0x%x/%u/0x%04x", SK_KVA(dpkt),
          doff, dpkt->pkt_csum_flags, (uint32_t)dpkt->pkt_csum_rx_start_off,
    ...
    if (copysum) {
      ...
      if (IS_P2ALIGNED(dbaddr + stuff, sizeof(csum))) {
        *(uint16_t *)(uintptr_t)(dbaddr + stuff) = csum;
        ...
        bcopy((void *)&csum, dbaddr + stuff, sizeof(csum));
      }
    } else {
      _pkt_copy(sbaddr, dbaddr, len);
    }
  ...
  SK_DF(SK_VERB_COPY | SK_VERB_DUMP, "%s(%d) %s %s",
        ...
        (t == NR_RX) ? "RX" : "TX", sk_dump("buf", dbaddr, len, 128));
#endif
}
 ...
 */
uint32_t static inline _pkt_copyaddr_sum(kern_packet_t sph, uint16_t soff,
                                         uint8_t *__sized_by(len) dbaddr,
                                         uint32_t len, boolean_t do_csum,
  ...
  while (len != 0) {
    ...
    if (__improbable(sbuf == NULL)) {
      panic("%s: bad packet, %p [off %d, len %d]", __func__, SK_KVA(spkt), off0,
            len0);
      /* NOTREACHED */
      __builtin_unreachable();
    }
    ...
    if (__probable(do_csum)) {
      ...
      if (clen != 0) {
        partial = __packet_copy_and_sum(sbaddr, dbaddr, clen, partial);
      }
 ...
 * as well as to perform the final 1's complement on it.
 */
static inline boolean_t _pkt_copypkt_sum(kern_packet_t sph, uint16_t soff,
                                         kern_packet_t dph, uint16_t doff,
                                         uint32_t len, uint32_t *csum_partial,
                                         boolean_t do_csum) {
  uint8_t odd = 0;
...
uint32_t pkt_sum(kern_packet_t sph, uint16_t soff, uint16_t len) {
  uint8_t odd = 0;
  uint32_t sum = 0, partial;
  boolean_t needs_swap, started_on_odd = FALSE;
  uint8_t *sbaddr = NULL;
  ...
  while (len != 0) {
    ...
    if (clen != 0) {
      partial = __packet_cksum(sbaddr, clen, partial);
    }
 ...
 * [ 0 ... soff ... soff + start/stuff ... soff + len ... ]
 */
void pkt_copy_multi_buflet_from_pkt(const enum txrx t, kern_packet_t dph,
                                    const uint16_t doff, kern_packet_t sph,
                                    const uint16_t soff, const uint32_t len,
                                    const boolean_t copysum,
  ...
  case NR_RX:
    ...
    if (__probable(do_sum)) {
       ...
       */
      if (__probable(start != 0)) {
        rc = _pkt_copypkt_sum(sph, soff, dph, doff, start, NULL, FALSE);
        ASSERT(rc);
      }
      _pkt_copypkt_sum(sph, (soff + start), dph, (doff + start), (len - start),
                       &partial, TRUE);
      csum = __packet_fold_sum(partial);
      __packet_set_inet_checksum(dph, PACKET_CSUM_PARTIAL, start, csum, FALSE);
      METADATA_ADJUST_LEN(dpkt, start, doff);
    } else {
      rc = _pkt_copypkt_sum(sph, soff, dph, doff, len, NULL, FALSE);
      ASSERT(rc);
      dpkt->pkt_csum_rx_start_off = spkt->pkt_csum_rx_start_off;
      dpkt->pkt_csum_rx_value = spkt->pkt_csum_rx_value;
      dpkt->pkt_csum_flags |= spkt->pkt_csum_flags & PACKET_CSUM_RX_FLAGS;
  ...
  case NR_TX:
    if (copysum) {
      ...
      rc = _pkt_copypkt_sum(sph, (soff + start), dph, (doff + start),
                            (len - start), &partial, TRUE);
      ASSERT(rc);
      csum = __packet_fold_sum_final(partial);
      ...
      if (IS_P2ALIGNED(baddr + stuff, sizeof(csum))) {
        *(uint16_t *)(uintptr_t)(baddr + stuff) = csum;
        ...
        bcopy((void *)&csum, baddr + stuff, sizeof(csum));
      }
      METADATA_ADJUST_LEN(dpkt, start, doff);
    } else {
      rc = _pkt_copypkt_sum(sph, soff, dph, doff, len, NULL, FALSE);
...
static inline uint32_t _convert_mbuf_csum_flags(uint32_t mbuf_flags) {
 ...
 */
void pkt_copy_from_mbuf(const enum txrx t, kern_packet_t ph,
                        const uint16_t poff, struct mbuf *m,
                        const uint16_t moff, const uint32_t len,
                        const boolean_t copysum, const uint16_t start) {
  ...
  case NR_RX:
    ...
    if (__probable(((m->m_pkthdr.csum_flags & CSUM_RX_FULL_FLAGS) !=
      ...
      partial =
          m_copydata_sum(m, start, (len - start), (baddr + start), 0, NULL);
      csum = __packet_fold_sum(partial);

      __packet_set_inet_checksum(ph, PACKET_CSUM_PARTIAL, start, csum, FALSE);
    } else {
    ...
    SK_PDF(SK_VERB_COPY_MBUF | SK_VERB_RX, current_proc(),
           "RX len %u, copy+sum %u (csum 0x%04x), start %u", len,
           (copysum ? (len - start) : 0), csum, start);
    SK_DF(SK_VERB_COPY_MBUF | SK_VERB_RX,
          "   mbuf %p csumf/rxstart/rxval 0x%x/%u/0x%04x", SK_KVA(m),
          m->m_pkthdr.csum_flags, (uint32_t)m->m_pkthdr.csum_rx_start,
          (uint32_t)m->m_pkthdr.csum_rx_val);
    SK_DF(SK_VERB_COPY_MBUF | SK_VERB_RX,
          "   pkt  %p poff %u csumf/rxstart/rxval 0x%x/%u/0x%04x", SK_KVA(pkt),
          poff, pkt->pkt_csum_flags, (uint32_t)pkt->pkt_csum_rx_start_off,
  ...
  case NR_TX:
    ...
    pkt->pkt_policy_id = (uint32_t)necp_get_policy_id_from_packet(m);
    pkt->pkt_skip_policy_id = (uint32_t)necp_get_skip_policy_id_from_packet(m);

    if ((m->m_pkthdr.pkt_flags & PKTF_TX_COMPL_TS_REQ) != 0) {
      if ((m->m_pkthdr.pkt_flags & PKTF_DRIVER_MTAG) != 0) {
        __packet_set_tx_completion_data(ph, m->m_pkthdr.drv_tx_compl_arg,
                                        m->m_pkthdr.drv_tx_compl_data);
      }
      pkt->pkt_tx_compl_context = m->m_pkthdr.pkt_compl_context;
      pkt->pkt_tx_compl_callbacks = m->m_pkthdr.pkt_compl_callbacks;
      /*
    ...
    SK_PDF(SK_VERB_COPY_MBUF | SK_VERB_TX, current_proc(),
           "TX len %u, copy+sum %u (csum 0x%04x), start %u", len,
           (copysum ? (len - start) : 0), csum, start);
    SK_DF(SK_VERB_COPY_MBUF | SK_VERB_TX,
          "   mbuf %p csumf/txstart/txstuff 0x%x/%u/%u", SK_KVA(m),
          m->m_pkthdr.csum_flags, (uint32_t)m->m_pkthdr.csum_tx_start,
 ...
 */
static inline uint32_t m_copypkt_sum(mbuf_t m, int soff, kern_packet_t dph,
                                     uint16_t doff, uint32_t len,
                                     boolean_t do_cscum) {
  boolean_t needs_swap, started_on_odd = FALSE;
  int off0 = soff;
  uint32_t len0 = len;
  ...
  while (len != 0) {
    ...
    if (count) {
      partial = __packet_copy_and_sum(sbaddr, dbaddr, count, partial);
      sbaddr += count;
      dbaddr += count;
      if (__improbable(partial & 0xc0000000)) {
        if (needs_swap) {
 ...
 * [ 0 ... moff ... moff + start/stuff ... moff + len ... ]
 */
void pkt_copy_multi_buflet_from_mbuf(const enum txrx t, kern_packet_t ph,
                                     const uint16_t poff, struct mbuf *m,
                                     const uint16_t moff, const uint32_t len,
                                     const boolean_t copysum,
  ...
  case NR_RX:
    ...
    if (__probable(((m->m_pkthdr.csum_flags & CSUM_RX_FULL_FLAGS) !=
      ...
      partial =
          m_copypkt_sum(m, start, ph, (poff + start), (len - start), TRUE);
      csum = __packet_fold_sum(partial);
      __packet_set_inet_checksum(ph, PACKET_CSUM_PARTIAL, start, csum, FALSE);
...
static inline uint32_t _convert_pkt_csum_flags(uint32_t pkt_flags) {
 ...
 */
void pkt_copy_to_mbuf(const enum txrx t, kern_packet_t ph, const uint16_t poff,
  ...
  case NR_RX:
    ...
    while (curr_m != NULL && remaining_len != 0) {
      uint32_t tmp_len = MIN(remaining_len, (uint32_t)M_TRAILINGSPACE(curr_m));
      dp = (uint8_t *)m_mtod_end(curr_m);
      if (__probable(do_sum)) {
        partial =
            __packet_copy_and_sum((baddr + copied_len), dp, tmp_len, partial);
      } else {
    ...
    SK_PDF(SK_VERB_COPY_MBUF | SK_VERB_RX, current_proc(),
           ...
           (copysum ? (len - start) : 0), csum, start);
    SK_DF(SK_VERB_COPY_MBUF | SK_VERB_RX,
          "   mbuf %p moff %u csumf/rxstart/rxval 0x%x/%u/0x%04x", SK_KVA(m),
          moff, m->m_pkthdr.csum_flags, (uint32_t)m->m_pkthdr.csum_rx_start,
          (uint32_t)m->m_pkthdr.csum_rx_val);
    SK_DF(SK_VERB_COPY_MBUF | SK_VERB_RX,
          "   pkt  %p poff %u csumf/rxstart/rxval 0x%x/%u/0x%04x", SK_KVA(pkt),
          poff, pkt->pkt_csum_flags, (uint32_t)pkt->pkt_csum_rx_start_off,
  ...
  case NR_TX:
    ...
    if (copysum) {
      ...
      partial = __packet_copy_and_sum((baddr + start), (dp + start),
                                      (len - start), 0);
      csum = __packet_fold_sum_final(partial);

      /* RFC1122 4.1.3.4: Invert 0 to -0 (for UDP) */
      if (csum == 0 && (pkt->pkt_csum_flags & PACKET_CSUM_ZERO_INVERT)) {
        csum = 0xffff;
      }
    ...
    SK_PDF(SK_VERB_COPY_MBUF | SK_VERB_TX, current_proc(),
           ...
           (copysum ? (len - start) : 0), csum, start);
    SK_DF(SK_VERB_COPY_MBUF | SK_VERB_TX,
          "   pkt  %p poff %u csumf/txstart/txstuff 0x%x/%u/%u", SK_KVA(pkt),
          poff, pkt->pkt_csum_flags, (uint32_t)pkt->pkt_csum_tx_start_off,
 ...
 * [ 0 ... poff ... poff + start/stuff ... poff + len ... ]
 */
void pkt_copy_multi_buflet_to_mbuf(const enum txrx t, kern_packet_t ph,
                                   const uint16_t poff, struct mbuf *m,
                                   const uint16_t moff, const uint32_t len,
                                   const boolean_t copysum,
                                   const uint16_t start) {
  ...
  case NR_RX:
    ...
    while (curr_m != NULL && remaining_len != 0) {
      ...
      if (__probable(do_sum)) {
        partial = _pkt_copyaddr_sum(ph, soff, dp, tmp_len, TRUE, partial, NULL);
      } else {
        pkt_copyaddr_sum(ph, soff, dp, tmp_len, FALSE, 0, NULL);
      }
  ...
  case NR_TX:
    ...
    if (copysum) {
      ...
      partial = _pkt_copyaddr_sum(ph, (poff + start), (dp + start),
                                  (len - start), TRUE, 0, NULL);
      csum = __packet_fold_sum_final(partial);
 ...
 * as well as to perform the final 1's complement on it.
 */
uint32_t m_copydata_sum(struct mbuf *m, int off, int len,
                        void *__sized_by(len) vp, uint32_t initial_sum,
                        boolean_t *odd_start) {
  boolean_t needs_swap, started_on_odd = FALSE;
  ...
  for (; len0 > 0; m = m->m_next) {
    ...
    if (count) {
      partial = __packet_copy_and_sum(datap, cp, count, (uint32_t)partial);
      datap += count;
      cp += count;
      len0 -= count;
...
uint32_t pkt_add_trailers(kern_packet_t ph, const uint32_t len,
                          const uint16_t start) {
  struct __kern_packet *pkt = SK_PTR_ADDR_KPKT(ph);
  uint32_t extra;
  uint8_t *baddr;
  ...
  pkt->pkt_csum_rx_value =
      (uint16_t)__packet_cksum((baddr + start), ((len + extra) - start), 0);
  pkt->pkt_csum_rx_start_off = start;

  return extra;
}

uint32_t pkt_add_trailers_mbuf(struct mbuf *m, const uint16_t start) {
  uint32_t extra;
...
void pkt_copypkt_sum(kern_packet_t sph, uint16_t soff, kern_packet_t dph,
                     uint16_t doff, uint16_t len, uint32_t *partial,
                     boolean_t do_csum) {
  VERIFY(_pkt_copypkt_sum(sph, soff, dph, doff, len, partial, do_csum));
}

uint32_t pkt_copyaddr_sum(kern_packet_t sph, uint16_t soff,
                          uint8_t *__sized_by(len) dbaddr, uint32_t len,
                          boolean_t do_csum, uint32_t initial_sum,
                          boolean_t *odd_start) {
  return _pkt_copyaddr_sum(sph, soff, dbaddr, len, do_csum, initial_sum,
                           odd_start);
}

uint32_t pkt_mcopypkt_sum(mbuf_t m, int soff, kern_packet_t dph, uint16_t doff,
                          uint16_t len, boolean_t do_cscum) {
  return m_copypkt_sum(m, soff, dph, doff, len, do_cscum);
}

void pkt_copy(void *__sized_by(len) src, void *__sized_by(len) dst,
              size_t len) {
