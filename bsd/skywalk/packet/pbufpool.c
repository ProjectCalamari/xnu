...
static uint32_t pp_alloc_packet_common(struct kern_pbufpool *, uint16_t,
                                       uint64_t *__counted_by(num),
                                       uint32_t num, boolean_t, alloc_cb_func_t,
                                       const void *, uint32_t);
static void pp_free_packet_array(struct kern_pbufpool *,
                                 uint64_t *__counted_by(num) array,
                                 uint32_t num);
static int pp_metadata_ctor_no_buflet(struct skmem_obj_info *,
                                      struct skmem_obj_info *, void *,
                                      uint32_t);
...
static int pp_metadata_construct(struct __kern_quantum *,
                                 struct __user_quantum *, obj_idx_t,
                                 struct kern_pbufpool *, uint32_t, uint16_t,
                                 bool, struct skmem_obj **);
...
static struct __kern_quantum *pp_metadata_init(struct __metadata_preamble *,
                                               struct kern_pbufpool *, uint16_t,
                                               uint32_t, struct skmem_obj **);
static struct __metadata_preamble *
pp_metadata_fini(struct __kern_quantum *, struct kern_pbufpool *,
                 struct mbuf **, struct __kern_packet **, struct skmem_obj **,
...
static mach_vm_address_t pp_alloc_buffer_common(const kern_pbufpool_t pp,
                                                struct skmem_obj_info *oi,
                                                uint32_t skmflag, bool large);
static inline uint32_t pp_alloc_buflet_common(struct kern_pbufpool *pp,
                                              uint64_t *__counted_by(num) array,
 ...
 */
#define KERN_PBUFPOOL_U_HASH_INDEX(_i, _m) (((_i) * 2654435761U) & (_m))
#define KERN_PBUFPOOL_U_HASH(_pp, _i)                                          \
  (&(_pp)->pp_u_hash_table[KERN_PBUFPOOL_U_HASH_INDEX(                         \
      _i, KERN_PBUFPOOL_U_HASH_SIZE - 1)])
#define KERN_PBUFPOOL_U_BFT_HASH(_pp, _i)                                      \
  (&(_pp)->pp_u_bft_hash_table[KERN_PBUFPOOL_U_HASH_INDEX(                     \
      _i, KERN_PBUFPOOL_U_HASH_SIZE - 1)])
...
static SKMEM_TAG_DEFINE(skmem_tag_pbufpool_bft_hash,
                        SKMEM_TAG_PBUFPOOL_BFT_HASH);
...
int pp_init(void) {
  ...
  static_assert(PKT_F_COMMON_MASK ==
   ...
   */
  static_assert(sizeof(struct __metadata_preamble) != 0 &&
                (sizeof(struct __metadata_preamble) % 16) == 0);

  static_assert(NX_PBUF_FRAGS_MIN == 1 &&
   ...
   */
  static_assert(offsetof(struct __metadata_preamble, _mdp_next) ==
                offsetof(struct skmem_obj, mo_next));
  static_assert(offsetof(struct __buflet, __buflet_next) ==
                offsetof(struct skmem_obj, mo_next));
  ...
  pp_opt_cache =
      skmem_cache_create("pkt.opt", sizeof(struct __packet_opt),
                         sizeof(uint64_t), NULL, NULL, NULL, NULL, NULL, 0);
  pp_flow_cache = skmem_cache_create("pkt.flow", sizeof(struct __flow),
                                     16, /* 16-bytes aligned */
                                     NULL, NULL, NULL, NULL, NULL, 0);
  pp_compl_cache =
      skmem_cache_create("pkt.compl", sizeof(struct __packet_compl),
                         sizeof(uint64_t), NULL, NULL, NULL, NULL, NULL, 0);
...
static struct kern_pbufpool *pp_alloc(zalloc_flags_t how) {
...
static void pp_free(struct kern_pbufpool *pp) {
...
void pp_retain_locked(struct kern_pbufpool *pp) {
  PP_LOCK_ASSERT_HELD(pp);
...
void pp_retain(struct kern_pbufpool *pp) {
  PP_LOCK(pp);
  pp_retain_locked(pp);
  PP_UNLOCK(pp);
}

boolean_t pp_release_locked(struct kern_pbufpool *pp) {
...
boolean_t pp_release(struct kern_pbufpool *pp) {
  boolean_t lastref;
...
void pp_close(struct kern_pbufpool *pp) {
  PP_LOCK(pp);
  ASSERT(pp->pp_refcnt > 0);
 ...
 * size for the srp_array. This is same as marking it
 * __counted_by(SKMEM_REGIONS)
 */
void pp_regions_params_adjust(
    struct skmem_region_params srp_array[SKMEM_REGIONS],
  ...
  struct skmem_region_params *srp, *kmd_srp, *buf_srp, *kbft_srp, *lbuf_srp;
  uint32_t md_size = 0;
  ...
  bool md_magazine_enable =
      ((flags & PP_REGION_CONFIG_MD_MAGAZINE_ENABLE) != 0);
  ...
  if (config_buflet) {
     ...
     */
    kbft_srp->srp_r_obj_cnt = buf_srp->srp_c_obj_cnt + lbuf_srp->srp_c_obj_cnt;
    kbft_srp->srp_r_obj_size =
        MAX(sizeof(struct __kern_buflet_ext), sizeof(struct __user_buflet));
    kbft_srp->srp_cflags = kmd_srp->srp_cflags;
    skmem_region_params_config(kbft_srp);
    ASSERT(kbft_srp->srp_c_obj_cnt >=
...
static int pp_metadata_construct(struct __kern_quantum *kqum,
                                 struct __user_quantum *uqum, obj_idx_t midx,
                                 struct kern_pbufpool *pp, uint32_t skmflag,
                                 uint16_t bufcnt, bool raw,
                                 struct skmem_obj **blist) {
  ...
  if (raw) {
    ...
    pflags = (PKT_F_OPT_ALLOC | PKT_F_FLOW_ALLOC | PKT_F_TX_COMPL_ALLOC);
  } else {
    ASSERT((kpkt->pkt_pflags & PKT_F_OPT_ALLOC) && kpkt->pkt_com_opt != NULL);
    opt = kpkt->pkt_com_opt;
    ASSERT((kpkt->pkt_pflags & PKT_F_FLOW_ALLOC) && kpkt->pkt_flow != NULL);
    flow = kpkt->pkt_flow;
  ...
  KPKT_CTOR(kpkt, pflags, opt, flow, compl, midx, upkt, pp, 0, pp->pp_max_frags,
            0);
  ...
  for (i = 0; i < bufcnt; i++) {
    ...
    if (!PP_HAS_BUFFER_ON_DEMAND(pp)) {
      ...
      KBUF_CTOR(kbuf, baddr, SKMEM_OBJ_IDX_REG(&oib), SKMEM_OBJ_BUFCTL(&oib),
                pp, false);
      baddr = 0;
    } else {
      /*
  ...
  SK_DF(SK_VERB_MEM, "pp %p pkt %p bufcnt %d buf %p", SK_KVA(pp), SK_KVA(kqum),
        bufcnt, SK_KVA(baddr));
  return 0;
...
static int pp_metadata_ctor_common(struct skmem_obj_info *oi0,
                                   struct skmem_obj_info *oim0,
                                   struct kern_pbufpool *pp, uint32_t skmflag,
                                   bool no_buflet) {
  ...
  kqum = SK_PTR_ADDR_KQUM((uintptr_t)SKMEM_OBJ_ADDR(oi) + METADATA_PREAMBLE_SZ);

  if (__probable(!PP_KERNEL_ONLY(pp))) {
    ASSERT(oim != NULL && SKMEM_OBJ_ADDR(oim) != NULL);
    ASSERT(SKMEM_OBJ_SIZE(oi) == SKMEM_OBJ_SIZE(oim));
    uqum =
        SK_PTR_ADDR_UQUM((uintptr_t)SKMEM_OBJ_ADDR(oim) + METADATA_PREAMBLE_SZ);
  } else {
    ASSERT(oim == NULL);
    uqum = NULL;
  }
  ...
  if (oim != NULL) {
    ...
    mdp->mdp_redzone =
        (SKMEM_OBJ_ROFF(oim) + METADATA_PREAMBLE_SZ) ^ __ch_umd_redzone_cookie;
  }
  ...
  if (PP_HAS_BUFFER_ON_DEMAND(pp) && bufcnt != 0) {
    (void)skmem_cache_batch_alloc(PP_KBFT_CACHE_DEF(pp), &blist,
                                  PP_KBFT_CACHE_DEF(pp)->skm_objsize, bufcnt,
                                  skmflag);
  }

  error = pp_metadata_construct(kqum, uqum, SKMEM_OBJ_IDX_REG(oi), pp, skmflag,
                                bufcnt, TRUE, &blist);
...
static int pp_metadata_ctor_no_buflet(struct skmem_obj_info *oi0,
                                      struct skmem_obj_info *oim0, void *arg,
                                      uint32_t skmflag) {
  return pp_metadata_ctor_common(oi0, oim0, arg, skmflag, true);
}

static int pp_metadata_ctor_max_buflet(struct skmem_obj_info *oi0,
                                       struct skmem_obj_info *oim0, void *arg,
                                       uint32_t skmflag) {
  return pp_metadata_ctor_common(oi0, oim0, arg, skmflag, false);
}

__attribute__((always_inline)) static void pp_metadata_destruct_common(
    struct __kern_quantum *kqum, struct kern_pbufpool *pp, bool raw,
    struct skmem_obj **blist_def, struct skmem_obj **blist_nocache_def,
  ...
  struct skmem_obj *__single p_blist_nocache_def = NULL,
                             *__single p_blist_nocache_large = NULL;
  ...
  *__DECONST(uint16_t *, &kpkt->pkt_bufs_cnt) = 0;
    ...
    if (BUFLET_HAS_LARGE_BUF(nbuf)) {
      ...
      if (nbuf->buf_ctl->bc_usecnt > 1) {
        *pp_blist_nocache_large = (struct skmem_obj *)(void *)nbuf;
        pp_blist_nocache_large = &((struct skmem_obj *)(void *)nbuf)->mo_next;
      } else {
        *pp_blist_large = (struct skmem_obj *)(void *)nbuf;
        pp_blist_large = &((struct skmem_obj *)(void *)nbuf)->mo_next;
      }
    } else {
      if (nbuf->buf_ctl->bc_usecnt > 1) {
        *pp_blist_nocache_def = (struct skmem_obj *)(void *)nbuf;
        pp_blist_nocache_def = &((struct skmem_obj *)(void *)nbuf)->mo_next;
      } else {
        *pp_blist_def = (struct skmem_obj *)(void *)nbuf;
        pp_blist_def = &((struct skmem_obj *)(void *)nbuf)->mo_next;
      }
    }
  ...
  if (raw) {
    ...
    if (kpkt->pkt_com_opt != NULL) {
      ...
      skmem_cache_free(pp_opt_cache, kpkt->pkt_com_opt);
      kpkt->pkt_com_opt = NULL;
    }
    ASSERT(kpkt->pkt_flow != NULL || !(kpkt->pkt_pflags & PKT_F_FLOW_ALLOC));
    ...
    if (kpkt->pkt_tx_compl != NULL) {
      ASSERT(kpkt->pkt_pflags & PKT_F_TX_COMPL_ALLOC);
      skmem_cache_free(pp_compl_cache, kpkt->pkt_tx_compl);
      kpkt->pkt_tx_compl = NULL;
    }
    kpkt->pkt_pflags = 0;
  }
...
pp_free_kbft_list(struct kern_pbufpool *pp, struct skmem_obj *blist_def,
                  struct skmem_obj *blist_nocache_def,
  ...
  if (blist_nocache_large != NULL) {
    skmem_cache_batch_free_nocache(PP_KBFT_CACHE_LARGE(pp),
                                   blist_nocache_large);
  }
}
...
pp_metadata_destruct(struct __kern_quantum *kqum, struct kern_pbufpool *pp,
  ...
  struct skmem_obj *__single blist_nocache_def = NULL,
                             *__single blist_nocache_large = NULL;

  pp_metadata_destruct_common(kqum, pp, raw, &blist_def, &blist_nocache_def,
                              &blist_large, &blist_nocache_large);
  pp_free_kbft_list(pp, blist_def, blist_nocache_def, blist_large,
                    blist_nocache_large);
}

static void pp_metadata_dtor(void *addr, void *arg) {
  pp_metadata_destruct(SK_PTR_ADDR_KQUM((uintptr_t)addr + METADATA_PREAMBLE_SZ),
                       arg, TRUE);
}

static void pp_buf_seg_ctor(struct sksegment *sg, IOSKMemoryBufferRef md,
                            void *arg) {
...
static void pp_buf_seg_dtor(struct sksegment *sg, IOSKMemoryBufferRef md,
                            void *arg) {
  struct kern_pbufpool *__single pp = arg;
...
static int pp_buflet_metadata_ctor_common(struct skmem_obj_info *oi0,
                                          struct skmem_obj_info *oim0,
...
static int pp_buflet_default_buffer_metadata_ctor(struct skmem_obj_info *oi0,
                                                  struct skmem_obj_info *oim0,
                                                  void *arg, uint32_t skmflag) {
  return pp_buflet_metadata_ctor_common(oi0, oim0, arg, skmflag, false);
}

static int pp_buflet_large_buffer_metadata_ctor(struct skmem_obj_info *oi0,
                                                struct skmem_obj_info *oim0,
                                                void *arg, uint32_t skmflag) {
  return pp_buflet_metadata_ctor_common(oi0, oim0, arg, skmflag, true);
}

static void pp_buflet_metadata_dtor(void *addr, void *arg) {
  ...
  ASSERT(((struct __kern_buflet_ext *)kbft)->kbe_buf_upp_link.sle_next == NULL);
  ASSERT(kbft->buf_addr != 0);
  ASSERT(kbft->buf_idx != OBJ_IDX_NONE);
  ASSERT(kbft->buf_ctl != NULL);

  KBUF_DTOR(kbft, usecnt);
  SK_DF(SK_VERB_MEM, "pp %p buf %p usecnt %u", SK_KVA(pp), SK_KVA(objaddr),
        usecnt);
  if (__probable(usecnt == 0)) {
    skmem_cache_free(large ? PP_BUF_CACHE_LARGE(pp) : PP_BUF_CACHE_DEF(pp),
                     objaddr);
  }
}
...
pp_create(const char *name, struct skmem_region_params srp_array[SKMEM_REGIONS],
  ...
  if ((ppcreatef & PPCREATEF_KERNEL_ONLY) == 0) {
    ...
    if (kbft_srp != NULL) {
      ...
      ASSERT(ubft_srp->srp_c_obj_size == kbft_srp->srp_c_obj_size);
      ASSERT(ubft_srp->srp_c_obj_cnt == kbft_srp->srp_c_obj_cnt);
      ASSERT(ubft_srp->srp_c_seg_size == kbft_srp->srp_c_seg_size);
  ...
  ASSERT(md_type > NEXUS_META_TYPE_INVALID && md_type <= NEXUS_META_TYPE_MAX);
  ASSERT(max_frags >= 1);
  ASSERT(md_type == NEXUS_META_TYPE_PACKET);
  ASSERT(md_size >= (METADATA_PREAMBLE_SZ + NX_METADATA_PACKET_SZ(max_frags)));
  ...
  (void)snprintf((char *)pp->pp_name, sizeof(pp->pp_name), "skywalk.pp.%s",
                 name);
  ...
  if (umd_srp != NULL && (pp->pp_umd_region = skmem_region_create(
                              name, umd_srp, NULL, NULL, NULL)) == NULL) {
    SK_ERR("\"%s\" (%p) failed to create %s region", pp->pp_name, SK_KVA(pp),
           umd_srp->srp_name);
    goto failed;
  }

  if ((pp->pp_kmd_region =
           skmem_region_create(name, kmd_srp, NULL, NULL, NULL)) == NULL) {
    SK_ERR("\"%s\" (%p) failed to create %s region", pp->pp_name, SK_KVA(pp),
           kmd_srp->srp_name);
    goto failed;
  }
  ...
  if (PP_HAS_BUFFER_ON_DEMAND(pp)) {
    ...
    if (!PP_KERNEL_ONLY(pp)) {
      VERIFY((ubft_srp != NULL) && (ubft_srp->srp_c_obj_cnt > 0));
    }
  }
  /*
  ...
  if (PP_HAS_BUFFER_ON_DEMAND(pp) && !PP_KERNEL_ONLY(pp)) {
    if ((pp->pp_ubft_region =
             skmem_region_create(name, ubft_srp, NULL, NULL, NULL)) == NULL) {
      SK_ERR("\"%s\" (%p) failed to create %s region", pp->pp_name, SK_KVA(pp),
             ubft_srp->srp_name);
      goto failed;
    }
  ...
  if (PP_HAS_BUFFER_ON_DEMAND(pp)) {
    if ((pp->pp_kbft_region =
             skmem_region_create(name, kbft_srp, NULL, NULL, NULL)) == NULL) {
      SK_ERR("\"%s\" (%p) failed to create %s region", pp->pp_name, SK_KVA(pp),
             kbft_srp->srp_name);
      goto failed;
    }
  ...
  if (PP_HAS_BUFFER_ON_DEMAND(pp)) {
    pp->pp_kmd_cache = skmem_cache_create(
        cache_name, md_size, 0, pp_metadata_ctor_no_buflet, pp_metadata_dtor,
        NULL, pp, pp->pp_kmd_region, md_cflags);
  } else {
    pp->pp_kmd_cache = skmem_cache_create(
        cache_name, md_size, 0, pp_metadata_ctor_max_buflet, pp_metadata_dtor,
        NULL, pp, pp->pp_kmd_region, md_cflags);
  }

  if (pp->pp_kmd_cache == NULL) {
    SK_ERR("\"%s\" (%p) failed to create \"%s\" cache", pp->pp_name, SK_KVA(pp),
           cname);
    goto failed;
  }
  ...
  if (pp->pp_kbft_region != NULL) {
    ...
    PP_KBFT_CACHE_DEF(pp) = skmem_cache_create(
        cache_name, kbft_srp->srp_c_obj_size, 0,
        pp_buflet_default_buffer_metadata_ctor, pp_buflet_metadata_dtor, NULL,
        pp, pp->pp_kbft_region, md_cflags);

    if (PP_KBFT_CACHE_DEF(pp) == NULL) {
      SK_ERR("\"%s\" (%p) failed to create \"%s\" cache", pp->pp_name,
             SK_KVA(pp), cname);
      goto failed;
    }
    ...
    if (PP_HAS_LARGE_BUF(pp)) {
      ...
      cache_name = tsnprintf(cname, sizeof(cname), "kbft_large.%s", name);
      PP_KBFT_CACHE_LARGE(pp) = skmem_cache_create(
          cache_name, kbft_srp->srp_c_obj_size, 0,
          pp_buflet_large_buffer_metadata_ctor, pp_buflet_metadata_dtor, NULL,
          pp, pp->pp_kbft_region, md_cflags);
  ...
  if ((PP_BUF_REGION_DEF(pp) = skmem_region_create(
           name, buf_srp, pp_buf_seg_ctor, pp_buf_seg_dtor, pp)) == NULL) {
    SK_ERR("\"%s\" (%p) failed to create %s region", pp->pp_name, SK_KVA(pp),
           buf_srp->srp_name);
    goto failed;
  }
  ...
  if (PP_HAS_LARGE_BUF(pp)) {
    PP_BUF_REGION_LARGE(pp) = skmem_region_create(
        name, lbuf_srp, pp_buf_seg_ctor, pp_buf_seg_dtor, pp);
  ...
  buf_def_cflags = buf_srp->srp_cflags & SKMEM_REGION_CR_NOMAGAZINES
                       ? SKMEM_CR_NOMAGAZINES
                       : 0;
  if ((PP_BUF_CACHE_DEF(pp) = skmem_cache_create(
           cache_name, def_buf_obj_size, 0, NULL, NULL, NULL, pp,
           PP_BUF_REGION_DEF(pp), buf_def_cflags)) == NULL) {
    SK_ERR("\"%s\" (%p) failed to create \"%s\" cache", pp->pp_name, SK_KVA(pp),
           cname);
    goto failed;
  }
  ...
  if (PP_BUF_REGION_LARGE(pp) != NULL) {
    ...
    if ((PP_BUF_CACHE_LARGE(pp) = skmem_cache_create(
             cache_name, lbuf_srp->srp_c_obj_size, 0, NULL, NULL, NULL, pp,
...
void pp_destroy(struct kern_pbufpool *pp) {
  PP_LOCK_ASSERT_HELD(pp);
...
static int pp_init_upp_locked(struct kern_pbufpool *pp, boolean_t can_block) {
  int i, err = 0;
  ...
  if (can_block) {
    pp->pp_u_hash_table = sk_alloc_type_array(
        struct kern_pbufpool_u_bkt, KERN_PBUFPOOL_U_HASH_SIZE, Z_WAITOK,
        skmem_tag_pbufpool_hash);
    pp->pp_u_hash_table_size = KERN_PBUFPOOL_U_HASH_SIZE;
  } else {
    pp->pp_u_hash_table = sk_alloc_type_array(
        struct kern_pbufpool_u_bkt, KERN_PBUFPOOL_U_HASH_SIZE, Z_NOWAIT,
        skmem_tag_pbufpool_hash);
    pp->pp_u_hash_table_size = KERN_PBUFPOOL_U_HASH_SIZE;
  }
  if (pp->pp_u_hash_table == NULL) {
...
static void pp_destroy_upp_locked(struct kern_pbufpool *pp) {
  PP_LOCK_ASSERT_HELD(pp);
  if (pp->pp_u_hash_table != NULL) {
    ...
    kfree_type_counted_by(struct kern_pbufpool_u_bkt, pp->pp_u_hash_table_size,
                          pp->pp_u_hash_table);
  }
  ASSERT(pp->pp_u_bufinuse == 0);
}

int pp_init_upp(struct kern_pbufpool *pp, boolean_t can_block) {
  int err = 0;
...
__attribute__((always_inline)) static void
pp_insert_upp_bft_locked(struct kern_pbufpool *pp, struct __kern_buflet *kbft,
                         pid_t pid) {
...
void static inline pp_insert_upp_common(struct kern_pbufpool *pp,
                                        struct __kern_quantum *kqum,
                                        pid_t pid) {
  ...
  kbft = __unsafe_forge_single(struct __kern_buflet *,
                               (kern_buflet_t)kqum->qum_buf[0].buf_nbft_addr);
  if (kbft != NULL) {
...
void pp_insert_upp_locked(struct kern_pbufpool *pp, struct __kern_quantum *kqum,
                          pid_t pid) {
  pp_insert_upp_common(pp, kqum, pid);
}

void pp_insert_upp(struct kern_pbufpool *pp, struct __kern_quantum *kqum,
                   pid_t pid) {
  PP_LOCK(pp);
  pp_insert_upp_common(pp, kqum, pid);
  PP_UNLOCK(pp);
...
__attribute__((always_inline)) static struct __kern_buflet *
pp_remove_upp_bft_locked(struct kern_pbufpool *pp, obj_idx_t bft_idx) {
  ...
  SLIST_FOREACH_SAFE(kbft, &bkt->upp_head, kbe_buf_upp_link, tbft) {
    if (((kern_buflet_t)kbft)->buf_bft_idx_reg == bft_idx) {
      SLIST_REMOVE(&bkt->upp_head, kbft, __kern_buflet_ext, kbe_buf_upp_link);
...
struct __kern_buflet *pp_remove_upp_bft(struct kern_pbufpool *pp,
                                        obj_idx_t md_idx, int *err) {
...
pp_remove_upp_bft_chain_locked(struct kern_pbufpool *pp,
  ...
  struct __user_packet *upkt =
      __DECONST(struct __user_packet *, kpkt->pkt_qum.qum_user);
  ...
  do {
    ...
    if (__improbable(kbft == NULL)) {
      ...
      SK_ERR("unallocated next buflet (%d), %p", bft_idx, SK_KVA(pbft));
      return ERANGE;
    }
    ASSERT(kbft->buf_flag & BUFLET_FLAG_EXTERNAL);
...
struct __kern_quantum *pp_remove_upp_locked(struct kern_pbufpool *pp,
                                            obj_idx_t md_idx, int *err) {
  ...
  SLIST_FOREACH_SAFE(kqum, &bkt->upp_head, qum_upp_link, tqum) {
    if (METADATA_IDX(kqum) == md_idx) {
      SLIST_REMOVE(&bkt->upp_head, kqum, __kern_quantum, qum_upp_link);
...
struct __kern_quantum *pp_remove_upp(struct kern_pbufpool *pp, obj_idx_t md_idx,
                                     int *err) {
  struct __kern_quantum *kqum;
...
struct __kern_quantum *pp_find_upp(struct kern_pbufpool *pp, obj_idx_t md_idx) {
  struct __kern_quantum *kqum, *tqum;
  struct kern_pbufpool_u_bkt *bkt;
...
pp_purge_upp_locked(struct kern_pbufpool *pp, pid_t pid) {
  ...
  for (i = 0; i < KERN_PBUFPOOL_U_HASH_SIZE; i++) {
    ...
    SLIST_FOREACH_SAFE(kqum, &bkt->upp_head, qum_upp_link, tqum) {
      ...
      kqum->qum_ksd = NULL;
      pp_free_packet(__DECONST(struct kern_pbufpool *, kqum->qum_pp),
                     (uint64_t)kqum);
      ASSERT(pp->pp_u_bufinuse != 0);
      pp->pp_u_bufinuse--;
    }
...
void pp_purge_upp(struct kern_pbufpool *pp, pid_t pid) {
  PP_LOCK(pp);
...
static int pp_init_upp_bft_locked(struct kern_pbufpool *pp,
                                  boolean_t can_block) {
  int i, err = 0;
  ...
  if (can_block) {
    pp->pp_u_bft_hash_table = sk_alloc_type_array(
        struct kern_pbufpool_u_bft_bkt, KERN_PBUFPOOL_U_HASH_SIZE, Z_WAITOK,
        skmem_tag_pbufpool_bft_hash);
    pp->pp_u_bft_hash_table_size = KERN_PBUFPOOL_U_HASH_SIZE;
  } else {
    pp->pp_u_bft_hash_table = sk_alloc_type_array(
        struct kern_pbufpool_u_bft_bkt, KERN_PBUFPOOL_U_HASH_SIZE, Z_NOWAIT,
        skmem_tag_pbufpool_bft_hash);
    pp->pp_u_bft_hash_table_size = KERN_PBUFPOOL_U_HASH_SIZE;
  }
  if (pp->pp_u_bft_hash_table == NULL) {
...
static void pp_destroy_upp_bft_locked(struct kern_pbufpool *pp) {
  PP_LOCK_ASSERT_HELD(pp);
  if (pp->pp_u_bft_hash_table != NULL) {
    /* purge anything that's left */
...
void pp_insert_upp_bft(struct kern_pbufpool *pp, struct __kern_buflet *kbft,
                       pid_t pid) {
  PP_LOCK(pp);
  pp_insert_upp_bft_locked(pp, kbft, pid);
  PP_UNLOCK(pp);
...
boolean_t pp_isempty_upp(struct kern_pbufpool *pp) {
  boolean_t isempty;
...
__attribute__((always_inline)) static inline struct __kern_quantum *
pp_metadata_init(struct __metadata_preamble *mdp, struct kern_pbufpool *pp,
  ...
  if (PP_HAS_BUFFER_ON_DEMAND(pp) && bufcnt != 0 &&
      pp_metadata_construct(kqum, uqum, METADATA_IDX(kqum), pp, skmflag, bufcnt,
                            FALSE, blist) != 0) {
    return NULL;
  }
...
pp_alloc_packet_common(struct kern_pbufpool *pp, uint16_t bufcnt,
                       uint64_t *__counted_by(num) array, uint32_t num,
                       boolean_t tagged, alloc_cb_func_t cb, const void *ctx,
                       uint32_t skmflag) {
  ...
  if (PP_HAS_BUFFER_ON_DEMAND(pp) && bufcnt != 0 && allocp != 0) {
    (void)skmem_cache_batch_alloc(PP_KBFT_CACHE_DEF(pp), &blist,
                                  PP_KBFT_CACHE_DEF(pp)->skm_objsize,
  ...
  while (plist != NULL) {
    ...
    if (tagged) {
      *array_cp =
          SK_PTR_ENCODE(kqum, METADATA_TYPE(kqum), METADATA_SUBTYPE(kqum));
    } else {
...
uint64_t pp_alloc_packet(struct kern_pbufpool *pp, uint16_t bufcnt,
                         uint32_t skmflag) {
  uint64_t kpkt = 0;

  (void)pp_alloc_packet_common(pp, bufcnt, &kpkt, 1, FALSE, NULL, NULL,
                               skmflag);

  return kpkt;
}

int pp_alloc_packet_batch(struct kern_pbufpool *pp, uint16_t bufcnt,
                          uint64_t *__counted_by(*size) array, uint32_t *size,
                          boolean_t tagged, alloc_cb_func_t cb, const void *ctx,
                          uint32_t skmflag) {
  uint32_t i, n;
  int err;
  ...
  i = pp_alloc_packet_common(pp, bufcnt, array, n, tagged, cb, ctx, skmflag);
  /*
...
int pp_alloc_pktq(struct kern_pbufpool *pp, uint16_t bufcnt, struct pktq *pktq,
                  uint32_t num, alloc_cb_func_t cb, const void *ctx,
                  uint32_t skmflag) {
  struct __metadata_preamble *mdp;
  ...
  while (plist != NULL) {
    ...
    mdp = (struct __metadata_preamble *)(void *)plist;
    kpkt = (struct __kern_packet *)pp_metadata_init(mdp, pp, bufcnt, skmflag,
                                                    &blist);
    if (kpkt == NULL) {
      if (blist != NULL) {
        skmem_cache_batch_free(PP_KBFT_CACHE_DEF(pp), blist);
...
uint64_t pp_alloc_packet_by_size(struct kern_pbufpool *pp, uint32_t size,
                                 uint32_t skmflag) {
  uint32_t bufcnt = pp->pp_max_frags;
  uint64_t kpkt = 0;

  if (PP_HAS_BUFFER_ON_DEMAND(pp)) {
    bufcnt = SK_ROUNDUP(size, PP_BUF_SIZE_DEF(pp)) / PP_BUF_SIZE_DEF(pp);
    ASSERT(bufcnt <= UINT16_MAX);
  }

  (void)pp_alloc_packet_common(pp, (uint16_t)bufcnt, &kpkt, 1, TRUE, NULL, NULL,
                               skmflag);

  return kpkt;
}

__attribute__((always_inline)) static inline struct __metadata_preamble *
pp_metadata_fini(struct __kern_quantum *kqum, struct kern_pbufpool *pp,
                 struct mbuf **mp, struct __kern_packet **kpp,
                 struct skmem_obj **blist_def,
                 struct skmem_obj **blist_nocache_def,
                 struct skmem_obj **blist_large,
  ...
  if ((kpkt->pkt_pflags & PKT_F_TX_COMPL_TS_REQ) != 0) {
    __packet_perform_tx_completion_callbacks(SK_PKT2PH(kpkt), NULL);
  }
  if ((kpkt->pkt_pflags & PKT_F_MBUF_DATA) != 0) {
    ...
    if (kpp != NULL) {
      ...
      *kpp = kpkt->pkt_pkt;
      ...
      ASSERT((kpkt->pkt_pkt->pkt_pflags & PKT_F_PKT_DATA) == 0);
...
void pp_free_packet_chain(struct __kern_packet *pkt_chain, int *npkt) {
  struct __metadata_preamble *mdp;
  struct skmem_obj *__single obj_mdp;
  struct skmem_obj *__single top = NULL;
  struct skmem_obj *__single blist_def = NULL,
                             *__single blist_nocache_def = NULL;
  struct skmem_obj *__single blist_large = NULL,
                             *__single blist_nocache_large = NULL;
  ...
  for (pkt = pkt_chain; pkt != NULL; pkt = next) {
    ...
    mdp = pp_metadata_fini(SK_PTR_ADDR_KQUM(pkt), pp, mp, kpp, &blist_def,
                           &blist_nocache_def, &blist_large,
                           &blist_nocache_large);
...
void pp_free_pktq(struct pktq *pktq) {
...
void pp_drop_pktq(struct pktq *pktq, struct ifnet *ifp, uint16_t flags,
                  drop_reason_t reason, const char *funcname,
                  uint16_t linenum) {
...
pp_free_packet_array(struct kern_pbufpool *pp,
                     uint64_t *__counted_by(num) array, uint32_t num) {
  struct __metadata_preamble *mdp;
  ...
  for (i = 0; i < num; i++) {
    ...
    mdp = pp_metadata_fini(SK_PTR_ADDR_KQUM(array[i]), pp, mp, kpp, &blist_def,
                           &blist_nocache_def, &blist_large,
                           &blist_nocache_large);
...
void pp_free_packet(struct kern_pbufpool *pp, uint64_t kqum) {
  pp_free_packet_array(pp, &kqum, 1);
}

void pp_free_packet_batch(const kern_pbufpool_t pp,
                          uint64_t *__counted_by(size) array, uint32_t size) {
  pp_free_packet_array(pp, array, size);
}

void pp_free_packet_single(struct __kern_packet *pkt) {
  ASSERT(pkt->pkt_nextpkt == NULL);
  pp_free_packet(__DECONST(struct kern_pbufpool *, pkt->pkt_qum.qum_pp),
                 SK_PTR_ADDR(pkt));
}

void pp_drop_packet_single(struct __kern_packet *pkt, struct ifnet *ifp,
                           uint16_t flags, drop_reason_t reason,
                           const char *funcname, uint16_t linenum) {
  drop_func_t dropfunc;
...
static mach_vm_address_t pp_alloc_buffer_common(const kern_pbufpool_t pp,
  ...
  struct skmem_cache *skm =
      large ? PP_BUF_CACHE_LARGE(pp) : PP_BUF_CACHE_DEF(pp);
  ...
  if (__improbable(mtbf != 0 && (net_uptime_ms() % mtbf) == 0 &&
    ...
    net_update_uptime();
    if (baddr != 0) {
      skmem_cache_free(skm, __unsafe_forge_single(struct skmem_obj *, baddr));
      baddr = 0;
    }
  }
#endif /* (DEVELOPMENT || DEBUG) */
  ...
  if (__improbable(baddr == 0)) {
    SK_DF(SK_VERB_MEM, "failed to alloc buffer, pp %p", SK_KVA(pp));
    return 0;
  }
  skmem_cache_get_obj_info(
      skm, __unsafe_forge_single(struct skmem_obj *, baddr), oi, NULL);
  ASSERT(SKMEM_OBJ_BUFCTL(oi) != NULL);
  ASSERT((mach_vm_address_t)SKMEM_OBJ_ADDR(oi) == baddr);
  return baddr;
}

errno_t pp_alloc_buffer(const kern_pbufpool_t pp, mach_vm_address_t *baddr,
                        kern_segment_t *seg, kern_obj_idx_seg_t *idx,
                        uint32_t skmflag) {
  struct skmem_obj_info oib;
...
void pp_free_buffer(const kern_pbufpool_t pp, mach_vm_address_t addr) {
  ASSERT(pp != NULL && addr != 0);
  skmem_cache_free(PP_BUF_CACHE_DEF(pp),
                   __unsafe_forge_single(struct skmem_obj *, addr));
}
...
pp_alloc_buflet_common(struct kern_pbufpool *pp,
  ...
  if (large) {
    allocd = skmem_cache_batch_alloc(PP_KBFT_CACHE_LARGE(pp), &list,
                                     PP_KBFT_CACHE_LARGE(pp)->skm_objsize, num,
                                     skmflag);
  } else {
    allocd = skmem_cache_batch_alloc(PP_KBFT_CACHE_DEF(pp), &list,
                                     PP_KBFT_CACHE_DEF(pp)->skm_objsize, num,
                                     skmflag);
  }
...
errno_t pp_alloc_buflet(struct kern_pbufpool *pp, kern_buflet_t *kbft,
                        uint32_t skmflag, bool large) {
  uint64_t bft;
...
errno_t pp_alloc_buflet_batch(struct kern_pbufpool *pp,
                              uint64_t *__counted_by(*size) array,
  ...
  if (kbft->buf_flag & BUFLET_FLAG_EXTERNAL) {
    ...
    if (kbft->buf_ctl->bc_usecnt > 1) {
      skmem_cache_free_nocache(BUFLET_HAS_LARGE_BUF(kbft)
                                   ? PP_KBFT_CACHE_LARGE(pp)
                                   : PP_KBFT_CACHE_DEF(pp),
                               (void *)kbft);
    } else {
      skmem_cache_free(BUFLET_HAS_LARGE_BUF(kbft) ? PP_KBFT_CACHE_LARGE(pp)
                                                  : PP_KBFT_CACHE_DEF(pp),
                       (void *)kbft);
    }
    ...
    SK_DF(SK_VERB_MEM, "pp %p buf %p usecnt %u", SK_KVA(pp), SK_KVA(objaddr),
          usecnt);
    if (__probable(usecnt == 0)) {
      skmem_cache_free(BUFLET_HAS_LARGE_BUF(kbft) ? PP_BUF_CACHE_LARGE(pp)
                                                  : PP_BUF_CACHE_DEF(pp),
                       objaddr);
    }
  }
...
void pp_free_buflet(const kern_pbufpool_t pp, kern_buflet_t kbft) {
  ASSERT(kbft->buf_flag & BUFLET_FLAG_EXTERNAL);
  ASSERT(pp != NULL && kbft != NULL);
  pp_free_buflet_common(pp, kbft);
}

void pp_reap_caches(boolean_t purge) {
