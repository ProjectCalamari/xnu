...
#define KBUF_CTOR(_kbuf, _baddr, _bidxreg, _bc, _pp, _large)                   \
  do {                                                                         \
    static_assert(sizeof((_kbuf)->buf_addr) == sizeof(mach_vm_address_t));     \
    /* kernel variant (deconst) */                                             \
    BUF_CTOR(_kbuf, _baddr, _bidxreg,                                          \
             (_large) ? PP_BUF_SIZE_LARGE(_pp) : PP_BUF_SIZE_DEF(_pp), 0, 0,   \
             (_kbuf)->buf_nbft_addr, (_kbuf)->buf_nbft_idx,                    \
             (_kbuf)->buf_flag);                                               \
...
#define KBUF_EXT_CTOR(_kbuf, _ubuf, _baddr, _bidxreg, _bc, _bft_idx_reg, _pp,  \
                      _large)                                                  \
  do {                                                                         \
    ...
    *__DECONST(struct __user_buflet **,                                        \
               &((struct __kern_buflet_ext *)(_kbuf))->kbe_buf_user) =         \
        (_ubuf);                                                               \
    KBUF_CTOR(_kbuf, _baddr, _bidxreg, _bc, _pp, _large);                      \
  } while (0)

#define KBUF_INIT(_kbuf)                                                       \
  do {                                                                         \
...
#define KBUF_EXT_INIT(_kbuf, _pp)                                              \
  do {                                                                         \
    ASSERT((_kbuf)->buf_ctl != NULL);                                          \
    ...
    *__DECONST(uint32_t *, &(_kbuf)->buf_dlim) =                               \
    ...
    (_kbuf)->buf_dlen = 0;                                                     \
    (_kbuf)->buf_doff = 0;                                                     \
    ((struct __kern_buflet_ext * __unsafe_indexable)(_kbuf))->kbe_buf_pid =    \
        (pid_t) - 1;                                                           \
    ((struct __kern_buflet_ext * __unsafe_indexable)(_kbuf))                   \
        ->kbe_buf_upp_link.sle_next = NULL;                                    \
  } while (0)

/* initialize struct __user_buflet from struct __kern_buflet */
#define UBUF_INIT(_kbuf, _ubuf)                                                \
  do {                                                                         \
    BUF_CTOR(_ubuf, 0, (_kbuf)->buf_idx, (_kbuf)->buf_dlim, (_kbuf)->buf_dlen, \
             (_kbuf)->buf_doff, (_kbuf)->buf_nbft_addr, (_kbuf)->buf_nbft_idx, \
             (_kbuf)->buf_flag);                                               \
    BUF_BFT_IDX_REG(_ubuf, (_kbuf)->buf_bft_idx_reg);                          \
  } while (0)

#define KBUF_EXTERNALIZE(_kbuf, _ubuf, _pp)                                    \
  do {                                                                         \
    ASSERT((_kbuf)->buf_dlim == BUFLET_HAS_LARGE_BUF(_kbuf)                    \
               ? PP_BUF_SIZE_LARGE((_pp))                                      \
               : PP_BUF_SIZE_DEF((_pp)));                                      \
    ASSERT((_kbuf)->buf_addr != 0);                                            \
  ...
  do {                                                                         \
    ...
    *__DECONST(uint16_t *, &(_dkb)->buf_flag) &= ~BUFLET_FLAG_EXTERNAL;        \
    if (__probable((_dkb)->buf_ctl != NULL)) {                                 \
      skmem_bufctl_use(__DECONST(struct skmem_bufctl *, (_dkb)->buf_ctl));     \
    }                                                                          \
  } while (0)
...
#define KQUM_CTOR(_kqum, _midx, _uqum, _pp, _qflags)                           \
  do {                                                                         \
    ASSERT((uintptr_t)(_kqum) != (uintptr_t)(_uqum));                          \
    static_assert(sizeof(METADATA_IDX(_kqum)) == sizeof(obj_idx_t));           \
    /* kernel variant (deconst) */                                             \
    _KQUM_CTOR(_kqum, (PP_KERNEL_ONLY(_pp) ? QUM_F_KERNEL_ONLY : 0) | _qflags, \
               0, 0, OBJ_IDX_NONE, PP_BUF_SIZE_DEF((_pp)), _midx);             \
...
#define KQUM_INIT(_kqum, _flags)                                               \
  do {                                                                         \
    ASSERT((_kqum)->qum_ksd == NULL);                                          \
    ASSERT((_kqum)->qum_pid == (pid_t) - 1);                                   \
    /* kernel variant (deconst) */                                             \
    _KQUM_INIT(_kqum,                                                          \
               (PP_KERNEL_ONLY((_kqum)->qum_pp) ? QUM_F_KERNEL_ONLY : 0) |     \
                   _flags,                                                     \
               0, METADATA_IDX(_kqum));                                        \
    /* no need to initialize user variant as it is done in externalize */      \
  } while (0)

__attribute__((always_inline)) inline boolean_t _UUID_MATCH(uuid_t u1,
                                                            uuid_t u2) {
 ...
 */
#define _QUM_INTERNALIZE(_uq, _kq)                                             \
  do {                                                                         \
    _QUM_COPY(_uq, _kq);                                                       \
 ...
 */
#define _QUM_EXTERNALIZE(_kq, _uq)                                             \
  do {                                                                         \
    static_assert(offsetof(struct __quantum, __q_flags) == 24);                \
...
#define _PKT_COM_INIT(_p, _pflags)                                             \
  do {                                                                         \
...
#define _PKT_CTOR(_p, _pflags, _bufcnt, _maxfrags)                             \
  do {                                                                         \
...
#define KPKT_CLEAR_MBUF_PKT_DATA(_pk)                                          \
  do {                                                                         \
    static_assert(offsetof(struct __kern_packet, pkt_mbuf) ==                  \
...
#define KPKT_CLEAR_MBUF_DATA(_pk)                                              \
  do {                                                                         \
    (_pk)->pkt_pflags &= ~PKT_F_MBUF_MASK;                                     \
    (_pk)->pkt_mbuf = NULL;                                                    \
  } while (0)

#define KPKT_CLEAR_PKT_DATA(_pk)                                               \
  do {                                                                         \
    (_pk)->pkt_pflags &= ~PKT_F_PKT_MASK;                                      \
    (_pk)->pkt_pkt = NULL;                                                     \
  } while (0)

#define KPKT_CLEAR_FLOW_INIT(_fl)                                              \
  do {                                                                         \
    static_assert(sizeof((_fl)->flow_init_data) == 128);                       \
    sk_zero_128(&(_fl)->flow_init_data[0]);                                    \
  } while (0)

#define KPKT_CLEAR_FLOW_ALL(_fl)                                               \
  do {                                                                         \
    bzero(_fl, sizeof(struct __flow));                                         \
  } while (0)

#define _KPKT_CTOR_PRIV_VARS(_p, _opt, _flow, _txcomp)                         \
  do {                                                                         \
    (_p)->pkt_com_opt = (_opt);                                                \
    (_p)->pkt_flow = (_flow);                                                  \
...
#define _KPKT_INIT_PRIV_VARS(_p)                                               \
  do {                                                                         \
...
#define _KPKT_INIT_TX_COMPL_DATA(_p)                                           \
  do {                                                                         \
 ...
 */
#define _PKT_COPY_OPT_DATA(_skp, _dkp)                                         \
  do {                                                                         \
    if (__improbable(((_skp)->pkt_pflags & PKT_F_OPT_DATA) != 0)) {            \
      ...
      sk_copy64_40(                                                            \
          (uint64_t *)(struct __packet_opt * __header_bidi_indexable)(_skp)    \
              ->pkt_com_opt,                                                   \
          (uint64_t *)(struct __packet_opt * __header_bidi_indexable)(_dkp)    \
              ->pkt_com_opt);                                                  \
    }                                                                          \
  } while (0)
 ...
 * after __p_flags.
 */
#define _PKT_COPY(_skp, _dkp)                                                  \
  do {                                                                         \
    static_assert(sizeof(struct __packet) == 32);                              \
    static_assert(sizeof(struct __packet_com) == 32);                          \
    static_assert(offsetof(struct __packet, __p_flags) == 24);                 \
    /* copy __packet excluding pkt_pflags */                                   \
    sk_copy64_24((uint64_t *)(struct __packet * __header_bidi_indexable) &     \
                     (_skp)->pkt_com,                                          \
                 (uint64_t *)(struct __packet * __header_bidi_indexable) &     \
                     (_dkp)->pkt_com);                                         \
 ...
 */
#define _PKT_COPY_TX_PORT_DATA(_skp, _dkp)                                     \
  do {                                                                         \
 ...
 */
#define _PKT_INTERNALIZE(_up, _kp)                                             \
  do {                                                                         \
    ...
    if (__improbable(((_kp)->pkt_pflags & PKT_F_OPT_DATA) != 0)) {             \
      ...
      sk_copy64_40(                                                            \
          ...
          (uint64_t *)(struct __packet_opt * __header_bidi_indexable)(_kp)     \
              ->pkt_com_opt);                                                  \
    }                                                                          \
  } while (0)
 ...
 */
#define _PKT_EXTERNALIZE(_kp, _up)                                             \
  do {                                                                         \
    static_assert(sizeof(struct __packet) == 32);                              \
    static_assert(sizeof(struct __packet_com) == 32);                          \
    static_assert(offsetof(struct __packet, __p_flags) == 24);                 \
...
#define SK_PTR_ADDR_KQUM(_ph)                                                  \
  __unsafe_forge_single(struct __kern_quantum *, (_ph))
#define SK_PTR_ADDR_KPKT(_ph)                                                  \
  __unsafe_forge_single(struct __kern_packet *, (_ph))
 ...
 */
#define METADATA_SET_LEN(_md, _len, _doff)                                     \
  do {                                                                         \
    struct __kern_quantum *_q = (struct __kern_quantum *)(void *)(_md);        \
    _q->qum_len = (_len);                                                      \
...
#define METADATA_ADJUST_LEN(_md, _len, _doff)                                  \
  do {                                                                         \
    struct __kern_packet *_p = (struct __kern_packet *)(void *)(_md);          \
    struct __kern_buflet *_kbft;                                               \
    PKT_GET_FIRST_BUFLET(_p, _p->pkt_bufs_cnt, _kbft);                         \
...
typedef void(pkt_copy_from_pkt_t)(const enum txrx, kern_packet_t,
                                  const uint16_t, kern_packet_t, const uint16_t,
                                  const uint32_t, const boolean_t,
                                  const uint16_t, const uint16_t,
                                  const boolean_t);

typedef void(pkt_copy_from_mbuf_t)(const enum txrx, kern_packet_t,
                                   const uint16_t, struct mbuf *,
                                   const uint16_t, const uint32_t,
                                   const boolean_t, const uint16_t);

typedef void(pkt_copy_to_mbuf_t)(const enum txrx, kern_packet_t, const uint16_t,
                                 struct mbuf *, const uint16_t, const uint32_t,
                                 const boolean_t, const uint16_t);
...
extern void pkt_copypkt_sum(kern_packet_t, uint16_t, kern_packet_t, uint16_t,
                            uint16_t, uint32_t *, boolean_t);
extern uint32_t pkt_copyaddr_sum(kern_packet_t sph, uint16_t soff,
...
extern uint32_t pkt_mcopypkt_sum(mbuf_t, int, kern_packet_t, uint16_t, uint16_t,
                                 boolean_t);
extern uint32_t m_copydata_sum(struct mbuf *m, int off, int len,
