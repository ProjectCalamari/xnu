...
struct kern_pbufpool {
   ...
   */
  struct kern_pbufpool_u_bkt *
      __counted_by(pp_u_hash_table_size) pp_u_hash_table;
   ...
   */
  struct kern_pbufpool_u_bft_bkt *
      __counted_by(pp_u_bft_hash_table_size) pp_u_bft_hash_table;
...
#define PP_KERNEL_ONLY(_pp) (((_pp)->pp_flags & PPF_KERNEL) != 0)

#define PP_HAS_TRUNCATED_BUF(_pp) (((_pp)->pp_flags & PPF_TRUNCATED_BUF) != 0)

#define PP_HAS_BUFFER_ON_DEMAND(_pp)                                           \
  (((_pp)->pp_flags & PPF_BUFFER_ON_DEMAND) != 0)

#define PP_BATCH_CAPABLE(_pp) (((_pp)->pp_flags & PPF_BATCH) != 0)

#define PP_DYNAMIC(_pp) (((_pp)->pp_flags & PPF_DYNAMIC) != 0)

#define PP_HAS_LARGE_BUF(_pp) (((_pp)->pp_flags & PPF_LARGE_BUF) != 0)

#define PP_LOCK(_pp) lck_mtx_lock(&_pp->pp_lock)
...
#define PP_UNLOCK(_pp) lck_mtx_unlock(&_pp->pp_lock)
...
#define PP_BUF_OBJ_SIZE_DEF(_pp) ((_pp)->pp_buf_obj_size[PBUFPOOL_BUF_IDX_DEF])
...
pp_regions_params_adjust(struct skmem_region_params srp_array[SKMEM_REGIONS],
                         nexus_meta_type_t, nexus_meta_subtype_t, uint32_t,
...
extern int pp_alloc_packet_batch(struct kern_pbufpool *, uint16_t,
                                 uint64_t *__counted_by(*size), uint32_t *size,
                                 boolean_t, alloc_cb_func_t, const void *,
                                 uint32_t);
...
extern void pp_free_packet_batch(struct kern_pbufpool *,
                                 uint64_t *__counted_by(size) array,
                                 uint32_t size);
extern void pp_free_packet_single(struct __kern_packet *);
extern void pp_drop_packet_single(struct __kern_packet *, struct ifnet *,
                                  uint16_t, drop_reason_t, const char *,
                                  uint16_t);
extern void pp_free_packet_chain(struct __kern_packet *, int *);
extern void pp_free_pktq(struct pktq *);
extern void pp_drop_pktq(struct pktq *, struct ifnet *, uint16_t, drop_reason_t,
                         const char *, uint16_t);
