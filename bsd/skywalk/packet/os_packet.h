...
#define PACKET_CSUM_RX_FLAGS (PACKET_CSUM_RX_FULL_FLAGS | PACKET_CSUM_PARTIAL)

#define PACKET_CSUM_TSO_FLAGS (PACKET_CSUM_TSO_IPV4 | PACKET_CSUM_TSO_IPV6)

#define PACKET_HAS_FULL_CHECKSUM_FLAGS(_p)                                     \
  (((_p)->pkt_csum_flags & PACKET_CSUM_RX_FULL_FLAGS) ==                       \
   PACKET_CSUM_RX_FULL_FLAGS)

#define PACKET_TX_CSUM_OFFLOAD_FLAGS                                           \
  (PACKET_CSUM_IP | PACKET_CSUM_TCP | PACKET_CSUM_UDP | PACKET_CSUM_TCPIPV6 |  \
   PACKET_CSUM_UDPIPV6 | PACKET_CSUM_ZERO_INVERT)
...
#define PKT_AGGR_IP_CHAIN                                                      \
  0x01 /* buflet chain of discrete IP packets containing contiguous L4 frames  \
        */
#define PKT_AGGR_SINGLE_IP 0x02 /* buflet chain representing single IP packet  \
                                 */
#define PKT_AGGR_SINGLE_IP_PACKED                                              \
  0x03 /* buflet chain representing single IP packet in packed format */
 ...
 */
/* Rx Group */
#define PKT_TRACE_RX_DRV_START                                                 \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x001) | DBG_FUNC_START)
#define PKT_TRACE_RX_DRV_END                                                   \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x001) | DBG_FUNC_END)

#define PKT_TRACE_RX_FSW_START                                                 \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x002) | DBG_FUNC_START)
#define PKT_TRACE_RX_FSW_END                                                   \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x002) | DBG_FUNC_END)

#define PKT_TRACE_RX_CHN_START                                                 \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x003) | DBG_FUNC_START)
#define PKT_TRACE_RX_CHN_END                                                   \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x003) | DBG_FUNC_END)

/* Tx Group */
#define PKT_TRACE_TX_FSW_START                                                 \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x010) | DBG_FUNC_START)
#define PKT_TRACE_TX_FSW_END                                                   \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x010) | DBG_FUNC_END)

#define PKT_TRACE_TX_AQM_START                                                 \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x011) | DBG_FUNC_START)
#define PKT_TRACE_TX_AQM_END                                                   \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x011) | DBG_FUNC_END)

#define PKT_TRACE_TX_DRV_START                                                 \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x012) | DBG_FUNC_START)
#define PKT_TRACE_TX_DRV_END                                                   \
  (SKYWALKDBG_CODE(DBG_SKYWALK_PACKET, 0x012) | DBG_FUNC_END)

/*
 * @enum packet_expiry_action_t
 * @abstract The desired action(s) to take upon the expiration of the packet
 * @discussion Bitfield property to express the desired actions to take when the
 * packet expires. At the moment the actions are only taken for the TX packets.
 *
...
extern int os_packet_set_compression_generation_count(const packet_t,
                                                      const uint32_t);
extern int os_packet_get_compression_generation_count(const packet_t,
                                                      uint32_t *);
...
extern int os_packet_set_inet_checksum(const packet_t,
                                       const packet_csum_flags_t,
                                       const uint16_t, const uint16_t);
#define HAS_OS_PACKET_ADD_CSUMF 1
extern void os_packet_add_inet_csum_flags(const packet_t,
                                          const packet_csum_flags_t);
...
extern int os_packet_set_expiry_action(const packet_t,
                                       const packet_expiry_action_t);
extern int os_packet_get_expiry_action(const packet_t,
                                       packet_expiry_action_t *);
...
extern int os_packet_set_app_metadata(const packet_t,
                                      const packet_app_metadata_type_t,
                                      const uint8_t);
...
extern uint32_t os_copy_and_inet_checksum(const void *, void *, uint32_t,
                                          uint32_t);
...
extern errno_t
kern_packet_set_transport_traffic_background(const kern_packet_t);
extern boolean_t
kern_packet_get_transport_traffic_background(const kern_packet_t);
extern errno_t kern_packet_set_transport_traffic_realtime(const kern_packet_t);
extern boolean_t
kern_packet_get_transport_traffic_realtime(const kern_packet_t);
/* deprecated */
...
extern kern_packet_svc_class_t
kern_packet_get_service_class(const kern_packet_t);
extern errno_t
kern_packet_get_service_class_index(const kern_packet_svc_class_t, uint32_t *);
...
extern errno_t kern_packet_set_compression_generation_count(const kern_packet_t,
                                                            const uint32_t);
extern errno_t kern_packet_get_compression_generation_count(const kern_packet_t,
                                                            uint32_t *);
extern boolean_t kern_packet_is_high_priority(const kern_packet_t);
...
extern errno_t kern_packet_get_token(const kern_packet_t,
                                     void *__sized_by(*len), uint16_t *len);
...
extern void kern_packet_set_trace_tag(const kern_packet_t ph,
                                      packet_trace_tag_t tag);
...
extern void kern_packet_get_tso_flags(const kern_packet_t,
                                      packet_tso_flags_t *);
 ...
 * Expiry notification
 */
extern errno_t kern_packet_set_expiry_action(const kern_packet_t,
                                             const packet_expiry_action_t);
extern errno_t kern_packet_get_expiry_action(const kern_packet_t,
                                             packet_expiry_action_t *);
extern errno_t kern_packet_check_for_expiry_and_notify(const kern_packet_t ph,
                                                       ifnet_t ifp,
                                                       uint16_t origin,
                                                       uint16_t status);
...
extern uint32_t kern_copy_and_inet_checksum(const void *__sized_by(len),
...
extern errno_t kern_buflet_set_buffer_offset(const kern_buflet_t,
                                             const uint32_t);
...
extern errno_t kern_pbufpool_alloc_nosleep(const kern_pbufpool_t pbufpool,
                                           const uint32_t bufcnt,
                                           kern_packet_t *packet);
extern errno_t kern_pbufpool_alloc_batch_nosleep(
    const kern_pbufpool_t pbufpool, const uint32_t bufcnt,
    kern_packet_t *__counted_by(*size) array, uint32_t *size);
...
extern void kern_pbufpool_free_batch(const kern_pbufpool_t pbufpool,
                                     kern_packet_t *__counted_by(size) array,
                                     uint32_t size);
...
extern errno_t kern_pbufpool_alloc_buffer_nosleep(
    const kern_pbufpool_t pbufpool, mach_vm_address_t *buffer,
