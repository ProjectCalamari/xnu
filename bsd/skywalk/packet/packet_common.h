...
#define QUM_GET_NEXT_BUFLET(_qum, _pbuf, _buf)                                 \
  do {                                                                         \
    ASSERT((_pbuf) == NULL || (_pbuf) == (_qum)->qum_buf);                     \
    (_buf) = (((_pbuf) == NULL) ? (_qum)->qum_buf : NULL);                     \
  } while (0)

#define PKT_GET_FIRST_BUFLET(_pkt, _bcnt, _buf)                                \
  do {                                                                         \
    ...
    if (__probable((_pkt)->pkt_qum_buf.buf_addr != 0)) {                       \
      ...
      (_buf) = __unsafe_forge_single(                                          \
          struct __kern_buflet *,                                              \
          __DECONST(void *, (_pkt)->pkt_qum_buf.buf_nbft_addr));               \
    }                                                                          \
  } while (0)

#define _PKT_GET_NEXT_BUFLET(_pkt, _bcnt, _pbuf, _buf)                         \
  do {                                                                         \
    if ((_pbuf) == NULL) {                                                     \
      PKT_GET_FIRST_BUFLET(_pkt, _bcnt, _buf);                                 \
    } else {                                                                   \
      (_buf) = __unsafe_forge_single(                                          \
          struct __kern_buflet *, __DECONST(void *, (_pbuf)->buf_nbft_addr));  \
    }                                                                          \
  } while (0)

#ifndef KERNEL
#define PKT_GET_NEXT_BUFLET(_pkt, _bcnt, _pbuf, _buf)                          \
  do {                                                                         \
...
#define PKT_DECOMPOSE_NX_PORT_ID(_nx_port_id, _nx_port, _gencnt)               \
  do {                                                                         \
...
__attribute__((always_inline)) static inline errno_t
__packet_get_token(const uint64_t ph, void *__sized_by(*len) token,
                   uint16_t *len) {
#ifdef KERNEL
  struct __packet_opt *po = PKT_ADDR(ph)->pkt_com_opt;
#else  /* !KERNEL */
...
__packet_opt_set_token(struct __packet_opt *po,
                       ...
                       const uint16_t len, const uint8_t type,
                       volatile uint64_t *pflags) {
  static_assert((__builtin_offsetof(struct __packet_opt, __po_token) % 8) == 0);
  if (len != 0) {
    if (token == NULL || len > PKT_OPT_MAX_TOKEN_SIZE || type == 0) {
      return EINVAL;
    }
...
__attribute__((always_inline)) static inline errno_t
__packet_set_token(const uint64_t ph, const void *__sized_by(len) token,
                   const uint16_t len) {
  PKT_TYPE_ASSERT(ph, NEXUS_META_TYPE_PACKET);
#ifdef KERNEL
  return __packet_opt_set_token(PKT_ADDR(ph)->pkt_com_opt, token, len,
                                PKT_OPT_TOKEN_TYPE_OPAQUE,
...
__packet_set_packetid(const uint64_t ph, const packet_id_t *pktid) {
  PKT_TYPE_ASSERT(ph, NEXUS_META_TYPE_PACKET);
#ifdef KERNEL
  return __packet_opt_set_token(
      PKT_ADDR(ph)->pkt_com_opt, pktid, sizeof(packet_id_t),
      PKT_OPT_TOKEN_TYPE_PACKET_ID, &PKT_ADDR(ph)->pkt_pflags);
#else  /* !KERNEL */
  return __packet_opt_set_token(
      &PKT_ADDR(ph)->pkt_com_opt, pktid, sizeof(packet_id_t),
      PKT_OPT_TOKEN_TYPE_PACKET_ID, &PKT_ADDR(ph)->pkt_pflags);
...
__packet_set_inet_checksum(const uint64_t ph, const packet_csum_flags_t flags,
                           const uint16_t start, const uint16_t stuff_val,
                           boolean_t tx) {
  PKT_TYPE_ASSERT(ph, NEXUS_META_TYPE_PACKET);
...
__attribute__((always_inline)) static inline void
__packet_add_inet_csum_flags(const uint64_t ph,
...
__attribute__((always_inline)) static inline packet_csum_flags_t
...
#ifdef KERNEL
  VERIFY(bcnt != 0 || PP_HAS_BUFFER_ON_DEMAND(PKT_ADDR(ph)->pkt_qum.qum_pp));
#else  /* !KERNEL */
  /*
...
#ifdef KERNEL
  VERIFY((bprev != NULL || bcnt == 0) && (bcnt < PKT_ADDR(ph)->pkt_bufs_max));
#else  /* !KERNEL */
...
#ifdef KERNEL
  ASSERT(bcnt != 0 || PP_HAS_BUFFER_ON_DEMAND(PKT_ADDR(ph)->pkt_qum.qum_pp));
#else  /* !KERNEL */
  /*
   * Handle the case where the metadata region gets
   * redirected to anonymous zero-filled pages at
...
__packet_finalize(const uint64_t ph) {
  ...
  if (__improbable((PKT_ADDR(ph)->pkt_headroom + PKT_ADDR(ph)->pkt_l2_len) >=
                   bdlim0)) {
    err = ERANGE;
    goto done;
  }
...
__packet_finalize_with_mbuf(struct __kern_packet *pkt) {
  ...
  if (__improbable((pkt->pkt_headroom + pkt->pkt_l2_len) >= bdlim)) {
    err = ERANGE;
    goto done;
  }
  ...
  if (__improbable(pkt->pkt_pflags & PKT_F_OPT_DATA)) {
    ...
    if ((pkt->pkt_pflags & PKT_F_OPT_EXPIRE_TS) && po->__po_expire_ts == 0) {
      err = EINVAL;
      goto done;
    }
    if ((pkt->pkt_pflags & PKT_F_OPT_TOKEN) && po->__po_token_len == 0) {
      err = EINVAL;
      goto done;
    }
  }
...
extern uint32_t os_cpu_copy_in_cksum(const void *__sized_by(len),
                                     void *__sized_by(len), uint32_t len,
                                     uint32_t);

__attribute__((always_inline)) static inline uint32_t
__packet_copy_and_sum(const void *__sized_by(len) src,
                      void *__sized_by(len) dst, uint32_t len, uint32_t sum0) {
...
__attribute__((always_inline)) static inline void
__packet_fix_hdr_sum(uint8_t *__sized_by(4) field, uint16_t *csum,
                     uint32_t new) {
  uint32_t old;
  memcpy(&old, field, sizeof(old));
  memcpy(field, &new, sizeof(uint32_t));
  *csum = __packet_fix_sum(
      __packet_fix_sum(*csum, (uint16_t)(old >> 16), (uint16_t)(new >> 16)),
      (uint16_t)(old & 0xffff), (uint16_t)(new & 0xffff));
}

__attribute__((always_inline)) static inline void *__header_indexable
__buflet_get_data_address(const void *buf) {
  return __unsafe_forge_bidi_indexable(
      void *, (void *)(BLT_ADDR(buf)->buf_addr), BLT_ADDR(buf)->buf_dlim);
}
...
__buflet_set_data_address(const void *buf, const void *addr) {
  ...
  /* full bounds checking will be performed during finalize */
  if (__probable((uintptr_t)addr >= (uintptr_t)BLT_ADDR(buf)->buf_objaddr)) {
    static_assert(sizeof(BLT_ADDR(buf)->buf_addr) == sizeof(mach_vm_address_t));
    /* deconst */
...
#ifdef KERNEL
__attribute__((always_inline)) static inline struct sksegment *
...
__attribute__((always_inline)) static inline packet_trace_id_t
...
#ifdef KERNEL
__attribute__((always_inline)) static inline packet_trace_tag_t
...
__packet_perform_tx_completion_callbacks(const kern_packet_t ph, ifnet_t ifp) {
  ...
  while (kpkt->pkt_tx_compl_callbacks != 0) {
    ...
    if (__probable(cb != NULL)) {
      cb(kpkt->pkt_tx_compl_context, ifp, ts, cb_arg, cb_data, tx_status);
    }
  }
  kpkt->pkt_pflags &= ~PKT_F_TX_COMPL_TS_REQ;
}

static inline void *__packet_get_priv(const kern_packet_t ph) {
  PKT_TYPE_ASSERT(ph, NEXUS_META_TYPE_PACKET);
  return PKT_ADDR(ph)->pkt_priv;
}

static inline void __packet_set_priv(const uint64_t ph, void *priv) {
  PKT_TYPE_ASSERT(ph, NEXUS_META_TYPE_PACKET);
