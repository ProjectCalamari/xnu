...
#ifdef KERNEL
#define BUF_CTOR(_buf, _baddr, _bidx, _dlim, _dlen, _doff, _nbaddr, _nbidx,    \
                 _bflag)                                                       \
  do {                                                                         \
...
#define BUF_INIT(_buf, _dlen, _doff)                                           \
  do {                                                                         \
    (_buf)->buf_dlen = (_dlen);                                                \
    (_buf)->buf_doff = (_doff);                                                \
  } while (0)
...
#define METADATA_IDX(_md) (METADATA_PREAMBLE(_md)->mdp_idx)

#define METADATA_TYPE(_md) (METADATA_PREAMBLE(_md)->mdp_type)

#define METADATA_SUBTYPE(_md) (METADATA_PREAMBLE(_md)->mdp_subtype)
...
#ifdef KERNEL
#define _KQUM_CTOR(_kqum, _flags, _len, _baddr, _bidx, _dlim, _qidx)           \
  do {                                                                         \
...
#define _KQUM_INIT(_kqum, _flags, _len, _qidx)                                 \
  do {                                                                         \
    (_kqum)->qum_flow_id_val64[0] = 0;                                         \
    (_kqum)->qum_flow_id_val64[1] = 0;                                         \
    (_kqum)->qum_qflags = (_flags);                                            \
...
#define PKT_F_OPT_DATA                                                         \
  (PKT_F_OPT_GROUP_START | PKT_F_OPT_GROUP_END | PKT_F_OPT_EXPIRE_TS |         \
   PKT_F_OPT_TOKEN | PKT_F_OPT_VLTAG | PKT_F_OPT_EXP_ACTION |                  \
...
#define PKT_F_USER_MASK                                                        \
  (PKT_F_BACKGROUND | PKT_F_REALTIME | PKT_F_REXMT | PKT_F_LAST_PKT |          \
   PKT_F_OPT_DATA | PKT_F_PROMISC | PKT_F_TRUNCATED | PKT_F_WAKE_PKT |         \
   PKT_F_L4S | PKT_F_ULPN)
...
#define PKT_F_COMMON_MASK                                                      \
  (PKT_F_BACKGROUND | PKT_F_REALTIME | PKT_F_REXMT | PKT_F_LAST_PKT |          \
   PKT_F_FLOW_ID | PKT_F_FLOW_ADV | PKT_F_TX_COMPL_TS_REQ | PKT_F_TS_VALID |   \
   PKT_F_NEW_FLOW | PKT_F_START_SEQ | PKT_F_KEEPALIVE | PKT_F_WAKE_PKT)
...
#define SK_PTR_ADDR_UQUM(_ph)                                                  \
  (__unsafe_forge_single(struct __user_quantum *, SK_PTR_ADDR(_ph)))
#define SK_PTR_ADDR_UPKT(_ph)                                                  \
  (__unsafe_forge_single(struct __user_packet *, SK_PTR_ADDR(_ph)))
