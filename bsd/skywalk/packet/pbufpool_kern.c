...
static errno_t kern_pbufpool_alloc_common(const kern_pbufpool_t, const uint32_t,
                                          kern_packet_t *, uint32_t);
static errno_t kern_pbufpool_alloc_batch_common(
    const kern_pbufpool_t, const uint32_t, kern_packet_t *__counted_by(*size),
    uint32_t *size, alloc_cb_func_t, const void *, uint32_t);

#define KBI_INVALID_CB_PAIRS(cb1, cb2)                                         \
  (!(init->kbi_##cb1 == NULL && init->kbi_##cb2 == NULL) &&                    \
   ((init->kbi_##cb1 == NULL) ^ (init->kbi_##cb2 == NULL)))

errno_t kern_pbufpool_create(const struct kern_pbufpool_init *init,
                             kern_pbufpool_t *ppp,
  ...
  if (ppp == NULL || init == NULL ||
      ...
      init->kbi_packets == 0 ||
      (init->kbi_buflets != 0 && init->kbi_buflets < init->kbi_packets &&
  ...
  *ppp = NULL;
  ...
  md_type = ((init->kbi_flags & KBIF_QUANTUM) ? NEXUS_META_TYPE_QUANTUM
  ...
  VERIFY((init->kbi_max_frags != 0) && (init->kbi_max_frags <= UINT16_MAX));
   ...
   */
  if (init->kbi_flags & (KBIF_PERSISTENT | KBIF_MONOLITHIC |
                         KBIF_INHIBIT_CACHE | KBIF_PHYS_CONTIGUOUS)) {
  ...
  pp_regions_params_adjust(srp, NEXUS_META_TYPE_PACKET, NEXUS_META_SUBTYPE_RAW,
                           pkt_cnt, max_frags, init->kbi_bufsize, 0, buf_cnt,
                           init->kbi_buf_seg_size, pp_region_flags);
  ...
  if ((pp = pp_create(__unsafe_null_terminated_from_indexable(init->kbi_name),
                      srp, init->kbi_buf_seg_ctor, init->kbi_buf_seg_dtor,
                      init->kbi_ctx, init->kbi_ctx_retain,
                      init->kbi_ctx_release, ppcreatef)) == NULL) {
    err = ENOMEM;
    goto done;
  }
 ...
 */
void *__single kern_pbufpool_get_context(const kern_pbufpool_t pp) {
...
kern_segment_idx_t kern_segment_get_index(const kern_segment_t seg) {
  return seg->sg_index;
}

static errno_t kern_pbufpool_alloc_common(const kern_pbufpool_t pp,
  ...
  *pph = 0;
  ...
  if (__probable(kqum != NULL)) {
    *pph = SK_PTR_ENCODE(kqum, METADATA_TYPE(kqum), METADATA_SUBTYPE(kqum));
  }

  return (kqum != NULL) ? 0 : ENOMEM;
}

errno_t kern_pbufpool_alloc(const kern_pbufpool_t pp, const uint32_t bufcnt,
                            kern_packet_t *pph) {
  return kern_pbufpool_alloc_common(pp, bufcnt, pph, SKMEM_SLEEP);
}

errno_t kern_pbufpool_alloc_nosleep(const kern_pbufpool_t pp,
                                    const uint32_t bufcnt, kern_packet_t *pph) {
  return kern_pbufpool_alloc_common(pp, bufcnt, pph, SKMEM_NOSLEEP);
}

static errno_t kern_pbufpool_alloc_batch_common(
    const kern_pbufpool_t pp, const uint32_t bufcnt,
  ...
  return pp_alloc_packet_batch(pp, (uint16_t)bufcnt, array, size, TRUE, cb, ctx,
                               skmflag);
}

errno_t kern_pbufpool_alloc_batch(const kern_pbufpool_t pp,
                                  const uint32_t bufcnt,
                                  kern_packet_t *__counted_by(*size) array,
                                  uint32_t *size) {
  return kern_pbufpool_alloc_batch_common(pp, bufcnt, array, size, NULL, NULL,
                                          SKMEM_SLEEP);
}

errno_t kern_pbufpool_alloc_batch_callback(const kern_pbufpool_t pp,
                                           const uint32_t bufcnt,
                                           kern_packet_t *__counted_by(*size)
                                               array,
                                           uint32_t *size, alloc_cb_func_t cb,
                                           const void *ctx) {
  return kern_pbufpool_alloc_batch_common(pp, bufcnt, array, size, cb, ctx,
                                          SKMEM_SLEEP);
}

errno_t kern_pbufpool_alloc_batch_nosleep(const kern_pbufpool_t pp,
                                          const uint32_t bufcnt,
                                          kern_packet_t *__counted_by(*size)
                                              array,
                                          uint32_t *size) {
...
errno_t kern_pbufpool_alloc_batch_nosleep_callback(
    const kern_pbufpool_t pp, const uint32_t bufcnt,
    kern_packet_t *__counted_by(*size) array, uint32_t *size,
...
void kern_pbufpool_free(const kern_pbufpool_t pp, kern_packet_t ph) {
  pp_free_packet(pp, SK_PTR_ADDR(ph));
}

void kern_pbufpool_free_batch(const kern_pbufpool_t pp,
                              kern_packet_t *__counted_by(size) array,
                              uint32_t size) {
...
void kern_pbufpool_free_chain(const kern_pbufpool_t pp, kern_packet_t chain) {
...
errno_t kern_pbufpool_alloc_buffer(const kern_pbufpool_t pp,
                                   mach_vm_address_t *buf, kern_segment_t *sg,
                                   kern_obj_idx_seg_t *sg_idx) {
  return pp_alloc_buffer(pp, buf, sg, sg_idx, 0);
}

errno_t kern_pbufpool_alloc_buffer_nosleep(const kern_pbufpool_t pp,
...
void kern_pbufpool_free_buffer(const kern_pbufpool_t pp,
                               mach_vm_address_t baddr) {
  pp_free_buffer(pp, baddr);
}

void kern_pbufpool_destroy(kern_pbufpool_t pp) {
  VERIFY(pp->pp_flags & PPF_EXTERNAL);
  pp_close(pp);
}

errno_t kern_pbufpool_alloc_buflet(const kern_pbufpool_t pp,
                                   kern_buflet_t *pbuf) {
  return pp_alloc_buflet(pp, pbuf, SKMEM_SLEEP, false);
}

errno_t kern_pbufpool_alloc_buflet_nosleep(const kern_pbufpool_t pp,
                                           kern_buflet_t *pbuf) {
