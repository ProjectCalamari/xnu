...
#define KPKTQ_ENQUEUE_MULTI(q, p, n, c)                                        \
  do {                                                                         \
    KPKTQ_NEXT(n) = NULL;                                                      \
    *(q)->kq_last = (p);                                                       \
  ...
  do {                                                                         \
    if (KPKTQ_FIRST(q) == (p)) {                                               \
      ...
      while (KPKTQ_NEXT(_p) != (p))                                            \
        _p = KPKTQ_NEXT(_p);                                                   \
      if ((KPKTQ_NEXT(_p) = KPKTQ_NEXT(KPKTQ_NEXT(_p))) == NULL) {             \
...
#define KPKTQ_FOREACH(p, q) for ((p) = KPKTQ_FIRST(q); (p); (p) = KPKTQ_NEXT(p))

#define KPKTQ_FOREACH_SAFE(p, q, tvar)                                         \
  for ((p) = KPKTQ_FIRST(q); (p) && ((tvar) = KPKTQ_NEXT(p), 1); (p) = (tvar))
