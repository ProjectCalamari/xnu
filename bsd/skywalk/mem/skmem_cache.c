...
static LCK_MTX_DECLARE_ATTR(skmem_cache_lock, &skmem_lock_grp,
                            &skmem_lock_attr);
...
#define SKM_DEPOT_LOCK(_skm) lck_mtx_lock(&(_skm)->skm_dp_lock)
#define SKM_DEPOT_LOCK_SPIN(_skm) lck_mtx_lock_spin(&(_skm)->skm_dp_lock)
#define SKM_DEPOT_CONVERT_LOCK(_skm) lck_mtx_convert_spin(&(_skm)->skm_dp_lock)
#define SKM_DEPOT_LOCK_TRY(_skm) lck_mtx_try_lock(&(_skm)->skm_dp_lock)
...
#define SKM_DEPOT_UNLOCK(_skm) lck_mtx_unlock(&(_skm)->skm_dp_lock)

#define SKM_RESIZE_LOCK(_skm) lck_mtx_lock(&(_skm)->skm_rs_lock)
...
#define SKM_RESIZE_UNLOCK(_skm) lck_mtx_unlock(&(_skm)->skm_rs_lock)

#define SKM_CPU_LOCK(_cp) lck_mtx_lock(&(_cp)->cp_lock)
#define SKM_CPU_LOCK_SPIN(_cp) lck_mtx_lock_spin(&(_cp)->cp_lock)
#define SKM_CPU_CONVERT_LOCK(_cp) lck_mtx_convert_spin(&(_cp)->cp_lock)
...
#define SKM_CPU_UNLOCK(_cp) lck_mtx_unlock(&(_cp)->cp_lock)
...
static int skmem_magazine_ctor(struct skmem_obj_info *, struct skmem_obj_info *,
                               void *, uint32_t);
...
static uint32_t skmem_depot_batch_alloc(struct skmem_cache *,
                                        struct skmem_maglist *, uint32_t *,
                                        struct skmem_mag * __bidi_indexable *,
                                        uint32_t);
...
static void skmem_cache_batch_free_common(struct skmem_cache *,
                                          struct skmem_obj *, uint32_t);
...
static void skmem_cpu_batch_reload(struct skmem_cpu_cache *, struct skmem_mag *,
                                   int);
...
SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, cache,
            ...
            skmem_cache_mib_get_sysctl, "S,sk_stats_cache",
 ...
 */
void skmem_cache_test_start(uint32_t i) {
 ...
 */
void skmem_cache_test_stop(void) {
 ...
 */
void skmem_cache_pre_init(void) {
  vm_size_t skm_size;
...
void skmem_cache_init(void) {
  ...
  static_assert(offsetof(struct skmem_bufctl, bc_addr) ==
                offsetof(struct skmem_bufctl_audit, bc_addr));
  static_assert(offsetof(struct skmem_bufctl, bc_addrm) ==
                offsetof(struct skmem_bufctl_audit, bc_addrm));
  static_assert(offsetof(struct skmem_bufctl, bc_slab) ==
                offsetof(struct skmem_bufctl_audit, bc_slab));
  static_assert(offsetof(struct skmem_bufctl, bc_lim) ==
                offsetof(struct skmem_bufctl_audit, bc_lim));
  static_assert(offsetof(struct skmem_bufctl, bc_flags) ==
                offsetof(struct skmem_bufctl_audit, bc_flags));
  static_assert(offsetof(struct skmem_bufctl, bc_idx) ==
                offsetof(struct skmem_bufctl_audit, bc_idx));
  static_assert(offsetof(struct skmem_bufctl, bc_usecnt) ==
                offsetof(struct skmem_bufctl_audit, bc_usecnt));
  static_assert(sizeof(struct skmem_bufctl) ==
                offsetof(struct skmem_bufctl_audit, bc_thread));
  ...
  if (skmem_clear_min == 0) {
    ...
    skmem_clear_min = (uint32_t)P2ROUNDUP(skmem_clear_min, cpu_cache_line_size);
  }
  ...
  if (skmem_debug & SKMEM_DEBUG_AUDIT) {
    ...
    skmem_bufctl_cache =
        skmem_cache_create("bufctl.audit", bc_size, sizeof(uint64_t), NULL,
                           NULL, NULL, NULL, NULL, 0);
  } else {
    bc_size = sizeof(struct skmem_bufctl);
    skmem_bufctl_cache = skmem_cache_create("bufctl", bc_size, sizeof(uint64_t),
                                            NULL, NULL, NULL, NULL, NULL, 0);
  }

  /* create a cache for slab structures */
  skmem_slab_cache =
      skmem_cache_create("slab", sizeof(struct skmem_slab), sizeof(uint64_t),
                         NULL, NULL, NULL, NULL, NULL, 0);
  ...
  for (i = 0; i < sizeof(skmem_magtype) / sizeof(*mtp); i++) {
    ...
    if (mtp->mt_align != 0 && ((mtp->mt_align & (mtp->mt_align - 1)) != 0 ||
                               mtp->mt_align < (int)cpu_cache_line_size)) {
      panic("%s: bad alignment %d", __func__, mtp->mt_align);
      /* NOTREACHED */
      __builtin_unreachable();
    }
    mt_cname = tsnprintf(mtp->mt_cname, sizeof(mtp->mt_cname), "mg.%d",
                         mtp->mt_magsize);

    /* create a cache for this magazine type */
    mtp->mt_cache = skmem_cache_create(
        mt_cname, SKMEM_MAG_SIZE(mtp->mt_magsize), mtp->mt_align,
   ...
   */
  skmem_cache_reap_tc = thread_call_allocate_with_options(
      skmem_cache_reap_func, NULL, THREAD_CALL_PRIORITY_KERNEL,
      THREAD_CALL_OPTIONS_ONCE);
  skmem_cache_update_tc = thread_call_allocate_with_options(
      skmem_cache_update_func, NULL, THREAD_CALL_PRIORITY_KERNEL,
      THREAD_CALL_OPTIONS_ONCE);
...
skmem_cache_create(const char *name, size_t bufsize, size_t bufalign,
                   skmem_ctor_fn_t ctor, skmem_dtor_fn_t dtor,
                   skmem_reclaim_fn_t reclaim, void *private,
                   struct skmem_region *region, uint32_t cflags) {
  ...
  if (region == NULL) {
     ...
     * as computed by skmem_region_params_config() above.
     */
    VERIFY(region->skr_c_obj_size >= (bufsize + sizeof(uint64_t) + bufalign));
#endif /* KASAN */
  ...
  uintptr_t diff =
      P2ROUNDUP((intptr_t)buf + skm_align_off, CHANNEL_CACHE_ALIGN_MAX) -
      (intptr_t)buf;
  ...
  (void)snprintf(skm->skm_name, sizeof(skm->skm_name), "%s.%s",
                 SKMEM_CACHE_PREFIX, name);
  ...
  if (chunksize > objsize) {
    panic("%s: (bufsize %lu, chunksize %lu) > objsize %lu", __func__, bufsize,
          chunksize, objsize);
    /* NOTREACHED */
    __builtin_unreachable();
  }
  ...
  skm->skm_hash_table =
      sk_alloc_type_array(struct skmem_bufctl_bkt, skm->skm_hash_initial,
                          Z_WAITOK | Z_NOFAIL, skmem_tag_bufctl_hash);
  ...
  for (cpuid = 0; cpuid < ncpu; cpuid++) {
    ...
    lck_mtx_init(&ccp->cp_lock, &skmem_cpu_lock_grp, &skmem_lock_attr);
    ccp->cp_rounds = -1;
    ccp->cp_prounds = -1;
  }
  ...
  SK_DF(SK_VERB_MEM_CACHE, "\"%s\": skm %p mode 0x%x", skm->skm_name,
        SK_KVA(skm), skm->skm_mode);
 ...
 * Destroy a cache.
 */
void skmem_cache_destroy(struct skmem_cache *skm) {
  uint32_t cpuid;
  ...
  if (skm->skm_sl_bufinuse != 0) {
    panic("%s: '%s' (%p) not empty (%llu unfreed)", __func__, skm->skm_name,
          (void *)skm, skm->skm_sl_bufinuse);
    /* NOTREACHED */
    __builtin_unreachable();
  }
  ...
  if (skm->skm_hash_table != NULL) {
    ...
    sk_free_type_array(struct skmem_bufctl_bkt, skm->skm_hash_size, htable);
  ...
  for (cpuid = 0; cpuid < ncpu; cpuid++) {
    lck_mtx_destroy(&skm->skm_cpu_cache[cpuid].cp_lock, &skmem_cpu_lock_grp);
  }
  ...
  SK_DF(SK_VERB_MEM_CACHE, "\"%s\": skm %p", skm->skm_name, SK_KVA(skm));
 ...
 */
void skmem_cache_get_obj_info(struct skmem_cache *skm, void *buf,
                              struct skmem_obj_info *oi,
                              struct skmem_obj_info *oim) {
  ...
  if (__improbable(bc == NULL)) {
    panic("%s: %s failed to get object info for %p", __func__, skm->skm_name,
          buf);
    /* NOTREACHED */
    __builtin_unreachable();
  }
   ...
   */
  sl = bc->bc_slab;
  SKMEM_OBJ_ADDR(oi) = __unsafe_forge_bidi_indexable(
      void *, bc->bc_addr, (uint32_t)skm->skm_objsize);
  ...
  SKMEM_OBJ_IDX_REG(oi) = (sl->sl_seg->sg_index * sl->sl_chunks) + bc->bc_idx;
  ...
  if (oim != NULL) {
    ...
    if (bc->bc_addrm != NULL) {
      SKMEM_OBJ_ADDR(oim) =
          __unsafe_forge_bidi_indexable(void *, bc->bc_addrm, oi->oi_size);
 ...
 */
static int skmem_magazine_ctor(struct skmem_obj_info *oi,
 ...
 */
static void skmem_magazine_destroy(struct skmem_cache *skm,
                                   struct skmem_mag *mg, int nrounds) {
  int round;
 ...
 */
static uint32_t skmem_depot_batch_alloc(
    struct skmem_cache *skm, struct skmem_maglist *ml, uint32_t *count,
    struct skmem_mag * __bidi_indexable * list, uint32_t num) {
 ...
 */
static void skmem_depot_batch_free(struct skmem_cache *skm,
                                   struct skmem_maglist *ml, uint32_t *count,
                                   struct skmem_mag *mg) {
 ...
 */
static void skmem_depot_ws_update(struct skmem_cache *skm) {
 ...
 */
static void skmem_depot_ws_zero(struct skmem_cache *skm) {
  SKM_DEPOT_LOCK_SPIN(skm);
 ...
 */
static void skmem_depot_ws_reap(struct skmem_cache *skm) {
  struct skmem_mag *mg, *nmg;
  uint32_t f, e, reap;

  reap = f = MIN(skm->skm_full.ml_reaplimit, skm->skm_full.ml_min);
  if (reap != 0) {
    (void)skmem_depot_batch_alloc(skm, &skm->skm_full, &skm->skm_depot_full,
                                  &mg, reap);
    while (mg != NULL) {
      nmg = SLIST_NEXT(mg, mg_link);
      SLIST_NEXT(mg, mg_link) = NULL;
      skmem_magazine_destroy(skm, mg, mg->mg_magtype->mt_magsize);
      mg = nmg;
    }
  }
  ...
  if (reap != 0) {
    (void)skmem_depot_batch_alloc(skm, &skm->skm_empty, &skm->skm_depot_empty,
                                  &mg, reap);
    while (mg != NULL) {
      nmg = SLIST_NEXT(mg, mg_link);
      SLIST_NEXT(mg, mg_link) = NULL;
 ...
 */
static void skmem_cache_update(struct skmem_cache *skm, uint32_t arg) {
#pragma unused(arg)
 ...
 */
static void skmem_cpu_batch_reload(struct skmem_cpu_cache *cp,
                                   struct skmem_mag *mg, int rounds) {
 ...
 */
static void skmem_cpu_reload(struct skmem_cpu_cache *cp, struct skmem_mag *mg,
                             int rounds) {
  ASSERT((cp->cp_loaded == NULL && cp->cp_rounds == -1) ||
         (cp->cp_loaded && cp->cp_rounds + rounds == cp->cp_magsize));
  ASSERT(cp->cp_magsize > 0);
 ...
 */
uint32_t skmem_cache_batch_alloc(struct skmem_cache *skm,
                                 struct skmem_obj **list,
                                 size_t SK_FB_ARG objsize, uint32_t num,
                                 uint32_t skmflag) {
  ...
  *list = NULL;
  ...
  for (;;) {
     ...
     */
    (void)skmem_depot_batch_alloc(skm, &skm->skm_full, &skm->skm_depot_full,
                                  &mg, (need <= cp->cp_magsize) ? 1 : 2);
    if (mg != NULL) {
      SLIST_HEAD(, skmem_mag) mg_list = SLIST_HEAD_INITIALIZER(mg_list);

      if (cp->cp_ploaded != NULL) {
        SLIST_INSERT_HEAD(&mg_list, cp->cp_ploaded, mg_link);
      }
      if (SLIST_NEXT(mg, mg_link) == NULL) {
        ...
        if (cp->cp_loaded != NULL) {
          SLIST_INSERT_HEAD(&mg_list, cp->cp_loaded, mg_link);
        }
        skmem_cpu_batch_reload(cp, mg, cp->cp_magsize);
      }
      skmem_depot_batch_free(skm, &skm->skm_empty, &skm->skm_depot_empty,
                             SLIST_FIRST(&mg_list));
      continue;
    }
  ...
  if (__probable(num > 1 && (skm->skm_mode & SKM_MODE_BATCH) != 0)) {
    ...
    while (skm->skm_ctor != NULL && rlist != NULL) {
       ...
       */
      if (skm->skm_ctor(oi, ((SKMEM_OBJ_ADDR(oim) != NULL) ? oim : NULL),
                        skm->skm_private, skmflag) != 0) {
        VERIFY(rlist->mo_next == rlistn);
        os_atomic_add(&skm->skm_sl_alloc_fail, rlistc - c, relaxed);
        if (rlistp != NULL) {
          rlistp->mo_next = NULL;
        }
        if (rlist == rtop) {
    ...
    *list = rtop;
    ...
    if (skm->skm_ctor != NULL &&
        skm->skm_ctor(&oi, ((SKMEM_OBJ_ADDR(&oim) != NULL) ? &oim : NULL),
                      skm->skm_private, skmflag) != 0) {
    ...
    need = 0;
    *list = buf;
    ASSERT(!(skm->skm_mode & SKM_MODE_BATCH) || (*list)->mo_next == NULL);
  }

done:
  /* if auditing is enabled, record this transaction */
  if (__improbable(*top != NULL && (skm->skm_mode & SKM_MODE_AUDIT) != 0)) {
    skmem_audit_buf(
        skm, __unsafe_forge_bidi_indexable(struct skmem_obj *, *top, objsize));
  }
 ...
 */
void skmem_cache_free(struct skmem_cache *skm, void *buf) {
  if (skm->skm_mode & SKM_MODE_BATCH) {
    ((struct skmem_obj *)buf)->mo_next = NULL;
  }
 ...
 */
void skmem_cache_free_nocache(struct skmem_cache *skm, void *buf) {
  if (skm->skm_mode & SKM_MODE_BATCH) {
    ((struct skmem_obj *)buf)->mo_next = NULL;
  }
  skmem_cache_batch_free_common(skm, (struct skmem_obj *)buf,
                                SKMEM_CACHE_FREE_NOCACHE);
}

void skmem_cache_batch_free(struct skmem_cache *skm, struct skmem_obj *list) {
  skmem_cache_batch_free_common(skm, list, 0);
}

void skmem_cache_batch_free_nocache(struct skmem_cache *skm,
                                    struct skmem_obj *list) {
  skmem_cache_batch_free_common(skm, list, SKMEM_CACHE_FREE_NOCACHE);
}

static void skmem_cache_batch_free_common(struct skmem_cache *skm,
                                          struct skmem_obj *list,
                                          uint32_t flags) {
  struct skmem_cpu_cache *cp = SKMEM_CPU_CACHE(skm);
  struct skmem_magtype *mtp;
  ...
  for (;;) {
     ...
     */
    if ((unsigned int)cp->cp_rounds < (unsigned int)cp->cp_magsize) {
      /*
      ...
      if (region->skr_mode & SKR_MODE_MEMTAG) {
        ...
        cp->cp_loaded->mg_round[cp->cp_rounds++] =
            __unsafe_forge_bidi_indexable(struct skmem_obj *, tagged_address,
                                          skm->skm_objsize);
      } else {
    ...
    if (mg != NULL) {
      ...
      skmem_depot_batch_free(skm, &skm->skm_full, &skm->skm_depot_full,
                             SLIST_FIRST(&mg_list));
      continue;
    }
    ...
    if (mg != NULL) {
       ...
       */
      ASSERT(SLIST_NEXT(mg, mg_link) == NULL);
      skmem_depot_batch_free(skm, &skm->skm_empty, &skm->skm_depot_empty, mg);
      continue;
    }
...
nocache:
   ...
   */
  if (__probable((skm->skm_mode & SKM_MODE_BATCH) && list->mo_next != NULL)) {
 ...
 */
uint32_t skmem_cache_magazine_max(uint32_t chunksize) {
  struct skmem_magtype *mtp;
  ...
  for (;;) {
    ...
    if (mtp == skmem_cache_magsize_last || chunksize >= mtp->mt_maxbuf) {
      break;
    }
    ++mtp;
 ...
 */
boolean_t skmem_allow_magazines(void) {
 ...
 */
static void skmem_cache_magazine_purge(struct skmem_cache *skm) {
 ...
 */
static void skmem_cache_magazine_enable(struct skmem_cache *skm, uint32_t arg) {
#pragma unused(arg)
  struct skmem_cpu_cache *cp;
  uint32_t cpuid;
  ...
  SK_DF(SK_VERB_MEM_CACHE, "skm %p chunksize %u magsize %d", SK_KVA(skm),
        (uint32_t)skm->skm_chunksize, SKMEM_CPU_CACHE(skm)->cp_magsize);
}
 ...
 */
static int skmem_cache_resize_enter(struct skmem_cache *skm,
                                    boolean_t can_sleep) {
  SKM_RESIZE_LOCK(skm);
  ...
  if (!can_sleep) {
    ...
    while (skm->skm_rs_busy != 0) {
      ...
      SK_DF(SK_VERB_MEM_CACHE,
            "waited for skm \"%s\" "
            "(%p) busy=%u",
            skm->skm_name, SK_KVA(skm), skm->skm_rs_busy);
      SKM_RESIZE_LOCK(skm);
    }
  }
 ...
 */
static void skmem_cache_resize_exit(struct skmem_cache *skm) {
  uint32_t want;
 ...
 */
static void skmem_cache_magazine_resize(struct skmem_cache *skm) {
  ...
  if (skm->skm_chunksize < mtp->mt_maxbuf) {
    ...
    skm->skm_magtype = ++mtp;
    skm->skm_cpu_mag_size = skm->skm_magtype->mt_magsize;
    skm->skm_depot_contention_prev = skm->skm_depot_contention + INT_MAX;
    SKM_DEPOT_UNLOCK(skm);
 ...
 */
static void skmem_cache_hash_rescale(struct skmem_cache *skm) {
  ...
  new_table = sk_alloc_type_array(struct skmem_bufctl_bkt, new_size, Z_NOWAIT,
  ...
  SK_DF(SK_VERB_MEM_CACHE, "skm %p old_size %u new_size %u [%u moved]",
        SK_KVA(skm), (uint32_t)old_size, (uint32_t)new_size, moved);
 ...
 */
static void skmem_cache_applyall(void (*func)(struct skmem_cache *, uint32_t),
                                 uint32_t arg) {
  ...
  SKMEM_CACHE_LOCK();
  TAILQ_FOREACH(skm, &skmem_cache_head, skm_link) { func(skm, arg); }
  SKMEM_CACHE_UNLOCK();
}
 ...
 */
static void skmem_cache_reclaim(struct skmem_cache *skm, uint32_t lowmem) {
  /*
 ...
 */
static void skmem_cache_reap_func(thread_call_param_t dummy,
 ...
 */
void skmem_cache_reap_now(struct skmem_cache *skm, boolean_t purge) {
 ...
 */
void skmem_reap_caches(boolean_t purge) {
 ...
 */
static void skmem_cache_update_func(thread_call_param_t dummy,
                                    thread_call_param_t arg) {
 ...
 */
__attribute__((noinline, cold, not_tail_called)) static inline void
skmem_audit_buf(struct skmem_cache *skm, struct skmem_obj *list) {
  ...
  while (list != NULL) {
    ...
    if (__improbable(bc == NULL)) {
      panic("%s: %s failed to get bufctl for %p", __func__, skm->skm_name, buf);
      /* NOTREACHED */
      __builtin_unreachable();
    }
...
static size_t skmem_cache_mib_get_stats(struct skmem_cache *skm,
  ...
  (void)snprintf(sca->sca_name, sizeof(sca->sca_name), "%s", skm->skm_name);
