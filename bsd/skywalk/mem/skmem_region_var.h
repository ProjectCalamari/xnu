...
#define SKSEGMENT_IN_FREELIST(_sg)                                             \
  ((_sg)->sg_link.tqe_next != NULL || (_sg)->sg_link.tqe_prev != NULL)
...
typedef void (*sksegment_ctor_fn_t)(struct sksegment *, IOSKMemoryBufferRef,
                                    void *);
typedef void (*sksegment_dtor_fn_t)(struct sksegment *, IOSKMemoryBufferRef,
                                    void *);
...
#define skr_seg_max_cnt skr_params.srp_seg_cnt /* max # of segments */
  ...
  struct sksegment_bkt *
      __counted_by(skr_hash_size) skr_hash_table;  /* alloc'd segment htable */
...
#define SKR_LOCK(_skr) lck_mtx_lock(&(_skr)->skr_lock)
...
#define SKR_UNLOCK(_skr) lck_mtx_unlock(&(_skr)->skr_lock)
...
#define SKMEM_REGION_CR_MEMTAG                                                 \
  0x10000 /* enable memory tagging in this region */
...
extern void *__sized_by(objsize)
    skmem_region_alloc(struct skmem_region *, void *__sized_by(*msize) *,
                       struct sksegment **, struct sksegment **, uint32_t,
                       uint32_t objsize, uint32_t *msize);
