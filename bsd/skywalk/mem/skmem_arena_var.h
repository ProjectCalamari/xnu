...
#define AR_LOCK(_ar) lck_mtx_lock(&(_ar)->ar_lock)
...
#define AR_UNLOCK(_ar) lck_mtx_unlock(&(_ar)->ar_lock)

#define AR_MEM_TOTAL(_ar, _id) ((_ar)->ar_regions[_id]->skr_memtotal)
#define AR_MEM_INUSE(_ar, _id) ((_ar)->ar_regions[_id]->skr_meminuse)
#define AR_MEM_WIRED_INUSE(_ar, _id) ((_ar)->ar_regions[_id]->skr_w_meminuse)
#define AR_MEM_SEGSIZE(_ar, _id) ((_ar)->ar_regions[_id]->skr_seg_size)
#define AR_MEM_SEGCNT(_ar, _id) ((_ar)->ar_regions[_id]->skr_seg_max_cnt)
#define AR_MEM_OBJCNT_R(_ar, _id) ((_ar)->ar_regions[_id]->skr_r_obj_cnt)
#define AR_MEM_OBJCNT_C(_ar, _id) ((_ar)->ar_regions[_id]->skr_c_obj_cnt)
#define AR_MEM_OBJSIZE_R(_ar, _id) ((_ar)->ar_regions[_id]->skr_r_obj_size)
#define AR_MEM_OBJSIZE_C(_ar, _id) ((_ar)->ar_regions[_id]->skr_c_obj_size)
 ...
 */
__attribute__((always_inline)) static inline struct skmem_arena_nexus *
...
extern struct skmem_arena *
skmem_arena_create_for_necp(const char *, struct skmem_region_params *,
                            struct skmem_region_params *, int *);
...
extern int skmem_arena_mredirect(struct skmem_arena *,
                                 struct skmem_arena_mmap_info *, struct proc *,
                                 boolean_t *);
extern int skmem_arena_defunct(struct skmem_arena *);
extern void skmem_arena_get_stats(struct skmem_arena *, uint64_t *, uint64_t *);
