...
#define SKMEM_MEMTAG_STRIP_TAG(addr, size)                                     \
  __unsafe_forge_bidi_indexable(                                               \
      void *, vm_memtag_canonicalize_kernel((vm_offset_t)addr), size)
...
#define SKMEM_SLAB_MEMBER(sl, buf)                                             \
  (((size_t)(buf) -                                                            \
    (size_t)vm_memtag_canonicalize_kernel((vm_offset_t)(sl)->sl_base)) <       \
...
#define SKMEM_MAG_SIZE(n) offsetof(struct skmem_mag, mg_round[n])
...
typedef int (*skmem_ctor_fn_t)(struct skmem_obj_info *, struct skmem_obj_info *,
                               void *, uint32_t);
...
struct skmem_cache {
  ...
  struct skmem_bufctl_bkt *
      __counted_by(skm_hash_size) skm_hash_table; /* alloc'd buffer htable */
...
#define SKMEM_CACHE_SIZE(n) offsetof(struct skmem_cache, skm_cpu_cache[n])
...
extern uint32_t skmem_slab_batch_alloc(struct skmem_cache *,
                                       struct skmem_obj **, uint32_t, uint32_t);
extern int skmem_slab_alloc(struct skmem_cache *, struct skmem_obj_info *,
                            struct skmem_obj_info *, uint32_t);
extern void skmem_audit_bufctl(struct skmem_bufctl *);
#define SKM_SLAB_LOCK(_skm) lck_mtx_lock(&(_skm)->skm_sl_lock)
...
#define SKM_SLAB_UNLOCK(_skm) lck_mtx_unlock(&(_skm)->skm_sl_lock)
#define SKMEM_CACHE_HASH_INDEX(_a, _s, _m) (((_a) >> (_s)) & (_m))
#define SKMEM_CACHE_HASH(_skm, _buf)                                           \
  (&(_skm)->skm_hash_table[SKMEM_CACHE_HASH_INDEX(                             \
      (uintptr_t)_buf, (_skm)->skm_hash_shift, (_skm)->skm_hash_mask)])
...
extern struct skmem_cache *skmem_cache_create(const char *, size_t, size_t,
                                              skmem_ctor_fn_t, skmem_dtor_fn_t,
                                              skmem_reclaim_fn_t, void *,
...
extern uint32_t skmem_cache_batch_alloc(struct skmem_cache *,
                                        struct skmem_obj **list, size_t objsize,
                                        uint32_t, uint32_t);
...
extern void skmem_cache_batch_free_nocache(struct skmem_cache *,
                                           struct skmem_obj *);
