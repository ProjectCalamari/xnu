 ...
 * Create a slab.
 */
static struct skmem_slab *skmem_slab_create(struct skmem_cache *skm,
                                            uint32_t skmflag) {
  ...
  SK_DF(SK_VERB_MEM_CACHE, "skm %p sl %p", SK_KVA(skm), SK_KVA(sl));
  SK_DF(SK_VERB_MEM_CACHE, "  [%u] [%p-%p)", sl->sl_seg->sg_index, SK_KVA(slab),
        SK_KVA(slab + objsize));
...
slab_alloc_failure:
  skmem_region_free(
      skr, slab,
      __unsafe_forge_bidi_indexable(void *, slabm, skr->skr_c_obj_size));
 ...
 * Destroy a slab.
 */
static void skmem_slab_destroy(struct skmem_cache *skm, struct skmem_slab *sl) {
  ...
  SK_DF(SK_VERB_MEM_CACHE, "skm %p sl %p", SK_KVA(skm), SK_KVA(sl));
  SK_DF(SK_VERB_MEM_CACHE, "  [%u] [%p-%p)", sl->sl_seg->sg_index, SK_KVA(slab),
        SK_KVA((uintptr_t)slab + skm->skm_objsize));
 ...
 */
int skmem_slab_alloc_locked(struct skmem_cache *skm, struct skmem_obj_info *oi,
...
again:
  ...
  if (sl == NULL) {
    ...
    if (!(flags & SKMEM_NOSLEEP)) {
      ...
      if (os_add_overflow(boff_total, (boff * NSEC_PER_USEC), &boff_total)) {
     ...
     */
    if (!(flags & SKMEM_NOSLEEP) && boff_total <= SKMEM_SLAB_MAX_BACKOFF) {
      retry = TRUE;
      ++retries;
      flags |= SKMEM_FAILOK;
    } else {
      if (!(flags & SKMEM_NOSLEEP)) {
        panic_plain("\"%s\": failed to allocate "
                    ...
                    "msec, %u retries\n\n%s",
                    skm->skm_name, (boff_total / NSEC_PER_USEC), retries,
    ...
    if ((sl = skmem_slab_create(skm, flags)) == NULL) {
      if (retry) {
        SK_ERR("\"%s\": failed to allocate "
               ...
               skm->skm_name, (flags & SKMEM_NOSLEEP) ? "non-" : "", boff,
               (boff_total / NSEC_PER_USEC), retries);
        VERIFY(boff > 0 && ((uint32_t)boff <= (SKMEM_SLAB_BACKOFF_THRES * 2)));
        delay((uint32_t)boff * NSEC_PER_USEC);
        SKM_SLAB_LOCK(skm);
        goto again;
      } else {
        SK_RDERR(4,
                 "\"%s\": failed to allocate slab "
                 "(%ssleeping mode)",
                 skm->skm_name, (flags & SKMEM_NOSLEEP) ? "non-" : "");
        SKM_SLAB_LOCK(skm);
      }
      return ENOMEM;
    }
    ...
    SKM_SLAB_LOCK(skm);
    skm->skm_sl_create++;
    if ((skm->skm_sl_bufinuse += sl->sl_chunks) > skm->skm_sl_bufmax) {
  ...
  if (region->skr_mode & SKR_MODE_MEMTAG) {
     ...
     */
    buf =
        __unsafe_forge_bidi_indexable(void *, tagged_address, skm->skm_objsize);
  } else {
    bufsize = bc->bc_lim;
    buf = bc->bc_addr;
  }
  ...
  if (oim != NULL) {
    ...
    if (bc->bc_addrm != NULL) {
      SKMEM_OBJ_ADDR(oim) = __unsafe_forge_bidi_indexable(void *, bc->bc_addrm,
                                                          SKMEM_OBJ_SIZE(oi));
  ...
  if (SLIST_EMPTY(&sl->sl_head)) {
    ...
    if (new_slab) {
      skm->skm_sl_partial++;
      ASSERT(skm->skm_sl_partial != 0);
      TAILQ_INSERT_HEAD(&skm->skm_sl_partial_list, sl, sl_link);
    }
  }
 ...
 */
int skmem_slab_alloc_pseudo_locked(struct skmem_cache *skm,
   ...
   */
  uintptr_t diff =
      P2ROUNDUP((intptr_t)obj + sizeof(u_int64_t), skm->skm_bufalign) -
      (uintptr_t)obj;
   ...
   * choice.
   */
  void **pbuf = __unsafe_forge_bidi_indexable(
      void **, (intptr_t)buf - sizeof(void *), sizeof(void *));
  *pbuf = obj;
  ...
  SKMEM_OBJ_ADDR(oi) = buf;
#if KASAN
  SKMEM_OBJ_SIZE(oi) = (uint32_t)skm->skm_objsize - (uint32_t)skm->skm_bufalign;
#else
 ...
 */
int skmem_slab_alloc(struct skmem_cache *skm, struct skmem_obj_info *oi,
                     struct skmem_obj_info *oim, uint32_t skmflag) {
  int err;
 ...
 */
uint32_t skmem_slab_batch_alloc(struct skmem_cache *skm,
                                struct skmem_obj **list, uint32_t num,
                                uint32_t skmflag) {
  uint32_t need = num;
 ...
 */
void skmem_slab_free_locked(struct skmem_cache *skm, void *buf) {
  struct skmem_bufctl *bc, *tbc;
  struct skmem_bufctl_bkt *bcb;
  ...
  if (region->skr_mode & SKR_MODE_MEMTAG) {
    tagged_addr = (vm_map_address_t)vm_memtag_generate_and_store_tag(
        buf, skm->skm_objsize);
  }
#endif /* CONFIG_KERNEL_TAGGING */
 ...
 */
void skmem_slab_free_pseudo_locked(struct skmem_cache *skm, void *buf) {
  struct skmem_region *skr = skm->skm_region;
   ...
   */
  void **pbuf = __unsafe_forge_bidi_indexable(
      void **, ((intptr_t)obj - sizeof(void *)), sizeof(void *));
 ...
 */
void skmem_slab_free(struct skmem_cache *skm, void *buf) {
  if (skm->skm_mode & SKM_MODE_BATCH) {
    ((struct skmem_obj *)buf)->mo_next = NULL;
  }
 ...
 */
void skmem_slab_batch_free(struct skmem_cache *skm, struct skmem_obj *list) {
 ...
 */
__attribute__((noinline, cold, not_tail_called)) inline void
