...
void skmem_test_init(void) {
  lck_mtx_init(&skmt_lock, &sk_lock_group, &sk_lock_attr);
}

void skmem_test_fini(void) { lck_mtx_destroy(&skmt_lock, &sk_lock_group); }

bool skmem_test_enabled(void) {
  bool enabled;
...
static uint16_t skmem_reference_sum(void *buffer, int len, int sum0) {
  uint16_t *w;
  int sum = sum0;
...
static void skmem_buflet_tests(uint32_t flags) {
  ...
  if (flags & KBIF_BUFFER_ON_DEMAND) {
    ...
    for (i = 0, phcnt = 0; i < maxphcnt; i++) {
      ...
      while ((err = kern_pbufpool_alloc_nosleep(pp, 0, &ph)) != 0) {
        VERIFY(err == ENOMEM);
        if (phcnt < 32) {
          SK_ERR("[a] retrying alloc for packet %u", phcnt);
  ...
  for (;;) {
    ...
    if (err == ENOMEM) {
      ...
      SK_ERR("batch alloc for %u packets only returned %u", maxphcnt, phcnt);
      break;
    } else {
      VERIFY(err == 0);
      break;
  ...
  VERIFY(kern_pbufpool_alloc_batch_callback(pp, 1, phary, &phcnt, NULL,
                                            &skmt_alloccb_ctx) == EINVAL);
  VERIFY(kern_pbufpool_alloc_batch_callback(
             pp, 1, phary, &phcnt, skmem_test_alloccb, &skmt_alloccb_ctx) == 0);
  ...
  for (i = 0; i < phcnt; i++) {
    ...
    VERIFY((buflet = kern_packet_get_next_buflet(phary[i], NULL)) != NULL);
    ...
    VERIFY(kern_packet_clone(phary[i], &pharyc[i], KPKT_COPY_LIGHT) == 0 ||
           !(flags & KBIF_BUFFER_ON_DEMAND));
    if (pharyc[i] != 0) {
      ...
      VERIFY((buflet2 = kern_packet_get_next_buflet(pharyc[i], NULL)) != NULL);
      ...
      VERIFY(kern_buflet_set_data_limit(
                 buflet2, (uint16_t)kern_buflet_get_object_limit(buflet2) +
                              1) == ERANGE);
      VERIFY(kern_buflet_set_data_limit(
                 buflet2,
                 (uint16_t)kern_buflet_get_object_limit(buflet2) - 16) == 0);
      VERIFY(
          kern_buflet_set_data_address(
              buflet2, (const void *)((uintptr_t)kern_buflet_get_object_address(
                                          buflet2) -
                                      1)) == ERANGE);
      VERIFY(
          kern_buflet_set_data_address(
      ...
      VERIFY(kern_buflet_set_data_length(
                 buflet2, kern_buflet_get_data_length(buflet2) - 32) == 0);
      VERIFY(kern_buflet_get_object_segment(buflet, &buf_idx_seg) ==
             kern_buflet_get_object_segment(buflet2, &buf2_idx_seg));
  ...
  for (i = 0; i < mbcnt; i++) {
    ...
    mblen = BUFLEN;
    VERIFY(mbuf_ring_cluster_alloc(MBUF_WAITOK, MBUF_TYPE_HEADER, &mbary[i],
                                   skmem_test_mbfreecb, &mblen) == 0);
    VERIFY(mblen == BUFLEN);
    VERIFY(mbary[i] != NULL);
  ...
  for (i = 0; i < phcnt; i++) {
    VERIFY(kern_packet_clone(phary[i], &pharyc[i], KPKT_COPY_HEAVY) == 0);
    struct __kern_packet *kpkt = SK_PTR_ADDR_KPKT(phary[i]);
    struct __kern_packet *kpkt2 = SK_PTR_ADDR_KPKT(pharyc[i]);
    kern_buflet_t buflet, buflet2;
    /*
    ...
    VERIFY(memcmp(mtod(kpkt->pkt_mbuf, void *), mtod(kpkt2->pkt_mbuf, void *),
                  mbuf_len(kpkt->pkt_mbuf)) == 0);
    VERIFY(__packet_finalize(pharyc[i]) == 0);
    ++phcloned;
  }
  ...
  for (i = 0; i < phcnt; i++) {
    ...
    VERIFY(mbary[i] != NULL);
    VERIFY(__packet_initialize_with_mbuf(kpkt2, mbary[i], 0, 0) == 0);
  ...
  (void)snprintf((char *)pp_init_mb.kbi_name, sizeof(pp_init_mb.kbi_name), "%s",
                 "skmem_buflet_tests_mb");
  pp_init_mb.kbi_flags = flags;
  pp_init_mb.kbi_max_frags = 4;
  pp_init_mb.kbi_packets = 64;
  pp_init_mb.kbi_bufsize = 512;
  pp_init_mb.kbi_buflets = pp_init_mb.kbi_packets * pp_init_mb.kbi_max_frags;
...
static void skmem_test_alloccb(kern_packet_t ph, uint32_t idx,
                               const void *ctx) {
  VERIFY(ph != 0);
...
static void skmem_packet_tests(uint32_t flags) {
  ...
  ref_buffer = (uint8_t *)kalloc_data(SKMEM_TEST_BUFSIZE, Z_WAITOK | Z_ZERO);

  bzero(&pp_init_mb, sizeof(pp_init_mb));
  pp_init_mb.kbi_version = KERN_PBUFPOOL_CURRENT_VERSION;
  pp_init_mb.kbi_buf_seg_size = skmem_usr_buf_seg_size;
  (void)snprintf((char *)pp_init_mb.kbi_name, sizeof(pp_init_mb.kbi_name), "%s",
                 "skmem_packet_tests_mb");
  ...
  if (csum_eeo_ref != csum) {
    SK_ERR("pkt_copypkt_sum: csum_eeo_mismatch 0x%x, "
           "0x%x, %p",
           csum_eeo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)));
  }
  ...
  if (test_unaligned) {
    ...
    if (csum_eoe_ref != csum) {
      SK_ERR("pkt_copypkt_sum: csum_eoe_mismatch 0x%x, "
             "0x%x, %p",
             csum_eoe_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)));
    }
    ...
    if (csum_eoo_ref != csum) {
      SK_ERR("pkt_copypkt_sum: csum_eoo_mismatch 0x%x, "
             "0x%x, %p",
             csum_eoo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)));
    }
    ...
    if (csum_oeo_ref != csum) {
      SK_ERR("pkt_copypkt_sum: csum_oeo_mismatch 0x%x, "
             "0x%x, %p",
             csum_oeo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)));
    }
    ...
    if (csum_ooo_ref != csum) {
      SK_ERR("pkt_copypkt_sum: csum_ooo_mismatch 0x%x, "
             "0x%x, %p",
             csum_ooo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)));
    }
    ...
    if (csum_ooe_ref != csum) {
      SK_ERR("pkt_copypkt_sum: csum_ooe_mismatch 0x%x, "
             "0x%x, %p",
             csum_ooe_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)));
    }
    ...
    if (csum_ooe_ref != csum) {
      SK_ERR("pkt_copypkt_sum: csum_oee_mismatch 0x%x, "
             "0x%x, %p",
             csum_oee_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)));
    }
  ...
  if (csum_eee_ref != csum) {
    SK_ERR("pkt_copypkt_sum: csum_eee_mismatch 0x%x, "
           "0x%x, %p",
           csum_eee_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)));
  }
  ...
  if (csum_eeo_ref != csum) {
    SK_ERR("pkt_copyaddr_sum: csum_eeo_mismatch "
           "0x%x, 0x%x, %p, %p",
           csum_eeo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(buffer));
  }
  VERIFY(csum_eeo_ref == csum);
  ...
  if (csum_eee_ref != csum) {
    SK_ERR("pkt_copyaddr_sum: csum_eee_mismatch "
           "0x%x, 0x%x, %p, %p",
           csum_eee_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(buffer));
  }
  VERIFY(csum_eee_ref == csum);
  ...
  if (test_unaligned) {
    ...
    if (csum_eee_ref != csum) {
      SK_ERR("pkt_mcopypkt_sum: csum_eee_mismatch "
             "0x%x, 0x%x, %p, %p",
             csum_eee_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(m));
    }
    VERIFY(csum_eee_ref == csum);
    kern_pbufpool_free(pp_mb, ph_mb);
    ph_mb = 0;
    ...
    if (csum_eoe_ref != csum) {
      SK_ERR("pkt_mcopypkt_sum: csum_eoe_mismatch "
             "0x%x, 0x%x, %p, %p",
             csum_eoe_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(m));
    }
    VERIFY(csum_eoe_ref == csum);
    kern_pbufpool_free(pp_mb, ph_mb);
    ph_mb = 0;
    ...
    if (csum_eoo_ref != csum) {
      SK_ERR("pkt_mcopypkt_sum: csum_eoo_mismatch "
             "0x%x, 0x%x, %p, %p",
             csum_eoo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(m));
    }
    VERIFY(csum_eoo_ref == csum);
    kern_pbufpool_free(pp_mb, ph_mb);
    ph_mb = 0;
  ...
  if (csum_eeo_ref != csum) {
    SK_ERR("pkt_mcopypkt_sum: csum_eeo_mismatch "
           "0x%x, 0x%x, %p, %p",
           csum_eeo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(m));
  }
  VERIFY(csum_eeo_ref == csum);
  kern_pbufpool_free(pp_mb, ph_mb);
  ph_mb = 0;
  ...
  if (test_unaligned) {
    ...
    if (csum_oeo_ref != csum) {
      SK_ERR("pkt_mcopypkt_sum: csum_oeo_mismatch "
             "0x%x, 0x%x, %p, %p",
             csum_oeo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(m));
    }
    VERIFY(csum_oeo_ref == csum);
    kern_pbufpool_free(pp_mb, ph_mb);
    ph_mb = 0;
    ...
    if (csum_oee_ref != csum) {
      SK_ERR("pkt_mcopypkt_sum: csum_oee_mismatch "
             "0x%x, 0x%x, %p, %p",
             csum_oee_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(m));
    }
    VERIFY(csum_oee_ref == csum);
    kern_pbufpool_free(pp_mb, ph_mb);
    ph_mb = 0;
    ...
    if (csum_ooe_ref != csum) {
      SK_ERR("pkt_mcopypkt_sum: csum_ooe_mismatch "
             "0x%x, 0x%x, %p, %p",
             csum_ooe_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(m));
    }
    VERIFY(csum_ooe_ref == csum);
    kern_pbufpool_free(pp_mb, ph_mb);
    ph_mb = 0;
    ...
    if (csum_ooo_ref != csum) {
      SK_ERR("pkt_mcopypkt_sum: csum_ooo_mismatch "
             "0x%x, 0x%x, %p, %p",
             csum_ooo_ref, csum, SK_KVA(SK_PTR_ADDR_KQUM(ph_mb)), SK_KVA(m));
    }
    VERIFY(csum_ooo_ref == csum);
    kern_pbufpool_free(pp_mb, ph_mb);
    ph_mb = 0;
...
static void skmem_basic_tests(void) {
  ...
  skmem_buflet_tests(KBIF_PERSISTENT | KBIF_PHYS_CONTIGUOUS | KBIF_USER_ACCESS);
  skmem_buflet_tests(KBIF_PERSISTENT | KBIF_MONOLITHIC | KBIF_USER_ACCESS);
  skmem_buflet_tests(KBIF_PERSISTENT | KBIF_BUFFER_ON_DEMAND);
  skmem_buflet_tests(KBIF_PERSISTENT | TEST_OPTION_INHIBIT_CACHE);
  skmem_buflet_tests(KBIF_PERSISTENT | KBIF_MONOLITHIC | KBIF_BUFFER_ON_DEMAND);
  skmem_buflet_tests(KBIF_PERSISTENT | KBIF_MONOLITHIC | KBIF_USER_ACCESS |
  ...
  skmem_buflet_tests(KBIF_MONOLITHIC | KBIF_USER_ACCESS | KBIF_NO_MAGAZINES);
  skmem_buflet_tests(KBIF_MONOLITHIC | KBIF_PHYS_CONTIGUOUS | KBIF_USER_ACCESS);
  ...
  skmem_buflet_tests(KBIF_VIRTUAL_DEVICE | KBIF_PERSISTENT | KBIF_MONOLITHIC |
  ...
  skmem_buflet_tests(KBIF_VIRTUAL_DEVICE | KBIF_MONOLITHIC | KBIF_USER_ACCESS);
  ...
  skmem_packet_tests(KBIF_PERSISTENT | KBIF_MONOLITHIC | KBIF_PHYS_CONTIGUOUS |
                     KBIF_USER_ACCESS);
  skmem_packet_tests(KBIF_PERSISTENT | KBIF_MONOLITHIC | KBIF_USER_ACCESS);
  skmem_packet_tests(KBIF_PERSISTENT | KBIF_BUFFER_ON_DEMAND);
  skmem_packet_tests(KBIF_PERSISTENT | TEST_OPTION_INHIBIT_CACHE);
  skmem_packet_tests(KBIF_PERSISTENT | KBIF_MONOLITHIC | KBIF_BUFFER_ON_DEMAND);
  skmem_packet_tests(KBIF_PERSISTENT | KBIF_MONOLITHIC | KBIF_USER_ACCESS |
                     TEST_OPTION_INHIBIT_CACHE);
  skmem_packet_tests(KBIF_PERSISTENT | KBIF_MONOLITHIC | KBIF_BUFFER_ON_DEMAND |
                     TEST_OPTION_INHIBIT_CACHE);
  ...
  skmem_packet_tests(KBIF_MONOLITHIC | KBIF_USER_ACCESS | KBIF_NO_MAGAZINES);
  skmem_packet_tests(KBIF_MONOLITHIC | KBIF_USER_ACCESS | KBIF_PHYS_CONTIGUOUS);
  ...
  skmem_packet_tests(KBIF_VIRTUAL_DEVICE | KBIF_PERSISTENT |
                     KBIF_PHYS_CONTIGUOUS);
  skmem_packet_tests(KBIF_VIRTUAL_DEVICE | KBIF_PERSISTENT | KBIF_MONOLITHIC |
                     KBIF_USER_ACCESS);
  skmem_packet_tests(KBIF_VIRTUAL_DEVICE | KBIF_PERSISTENT |
                     KBIF_BUFFER_ON_DEMAND);
  skmem_packet_tests(KBIF_VIRTUAL_DEVICE | KBIF_PERSISTENT |
  ...
  skmem_packet_tests(KBIF_VIRTUAL_DEVICE | KBIF_MONOLITHIC |
                     KBIF_PHYS_CONTIGUOUS);
  skmem_packet_tests(KBIF_VIRTUAL_DEVICE | KBIF_MONOLITHIC | KBIF_USER_ACCESS |
                     KBIF_PHYS_CONTIGUOUS);
  skmem_packet_tests(KBIF_VIRTUAL_DEVICE | KBIF_MONOLITHIC | KBIF_USER_ACCESS);
  skmem_packet_tests(KBIF_VIRTUAL_DEVICE | KBIF_MONOLITHIC |
                     KBIF_BUFFER_ON_DEMAND);
...
static void skmem_advanced_tests(int n, int32_t th_max, uint32_t mode,
                                 boolean_t nosleep, uint32_t flags) {
  struct kern_pbufpool_init pp_init;
  kern_packet_t mph = 0;
  ...
  case 0:
    ...
    VERIFY(kern_pbufpool_create(&pp_init, &skmth_pp, NULL) == 0);
    break;
  ...
  case 1:
    ...
    pp_init.kbi_max_frags = 1;
    pp_init.kbi_flags |=
        KBIF_MONOLITHIC | KBIF_USER_ACCESS | KBIF_VIRTUAL_DEVICE;
    VERIFY(kern_pbufpool_create(&pp_init, &skmth_pp, NULL) == 0);
    break;
  ...
  case 2:
    ...
    pp_init.kbi_bufsize = SKMEM_TEST_BUFSIZE;
    pp_init.kbi_max_frags = 1;
    pp_init.kbi_flags |= KBIF_MONOLITHIC | KBIF_USER_ACCESS | KBIF_PERSISTENT;
    VERIFY(kern_pbufpool_create(&pp_init, &skmth_pp, NULL) == 0);
    break;
  ...
  SK_ERR("%d: th_max %d mode %u nosleep %u nomagazines %u", n, th_max, mode,
         nosleep, !!(flags & KBIF_NO_MAGAZINES));
  ...
  do {
    ...
    (void)msleep(&skmth_cnt, &skmt_lock, (PZERO - 1), "skmtstartw", &ts);
  ...
  do {
    struct timespec ts = {.tv_sec = 0, .tv_nsec = 100 * USEC_PER_SEC};
    (void)msleep(&skmth_cnt, &skmt_lock, (PZERO - 1), "skmtstopw", &ts);
  } while (skmth_cnt != 0);
  skmth_run = FALSE;
  lck_mtx_unlock(&skmt_lock);
...
__attribute__((noreturn)) static void skmem_test_func(void *v,
                                                      wait_result_t w) {
#pragma unused(w)
  ...
  do {
    (void)msleep(&skmth_run, &skmt_lock, (PZERO - 1), "skmtfuncw", NULL);
  } while (!skmth_run);
  lck_mtx_unlock(&skmt_lock);
  ...
  for (c = 0; c < 41; c++) {
    ...
    if (skmth_info[i].sti_nosleep) {
      errno_t err = kern_pbufpool_alloc_nosleep(skmth_pp, 1, &ph);
      VERIFY(ph != 0 || err != 0);
    } else {
    ...
    if (skmth_info[i].sti_mph != 0) {
      ...
      if (skmth_info[i].sti_nosleep) {
        errno_t err;
        err = kern_packet_clone_nosleep(
            skmth_info[i].sti_mph, &skmth_info[i].sti_mpc, KPKT_COPY_LIGHT);
        VERIFY(skmth_info[i].sti_mpc != 0 || err != 0);
      } else {
        VERIFY(kern_packet_clone(skmth_info[i].sti_mph, &skmth_info[i].sti_mpc,
                                 KPKT_COPY_LIGHT) == 0);
      }
      if (skmth_info[i].sti_mpc != 0) {
        VERIFY(
            !(QUM_ADDR(skmth_info[i].sti_mpc)->qum_qflags & QUM_F_FINALIZED));
        VERIFY((buflet = kern_packet_get_next_buflet(skmth_info[i].sti_mph,
                                                     NULL)) != NULL);
        VERIFY((buflet2 = kern_packet_get_next_buflet(skmth_info[i].sti_mpc,
                                                      NULL)) != NULL);
        VERIFY(kern_buflet_get_object_address(buflet) ==
               kern_buflet_get_object_address(buflet2));
        VERIFY(kern_buflet_get_data_address(buflet) ==
...
static int skmem_test_ctor(struct skmem_obj_info *oi,
...
#pragma unused(skmflag)
  ...
  sto->sto_val[0] = (uint64_t)(void *)sto ^ (uint64_t)(void *)&sto->sto_val[0];
  sto->sto_val[1] = (uint64_t)(void *)sto ^ (uint64_t)(void *)&sto->sto_val[1];
  os_atomic_inc(&skmem_test_objs, relaxed);

  return 0;
}

static void skmem_test_dtor(void *addr, void *arg) {
...
static void skmem_tests(uint32_t align) {
...
static void skmem_test_start(void *v, wait_result_t w) {
...
static void skmem_test_stop(void *v, wait_result_t w) {
#pragma unused(v, w)
...
static int sysctl_skmem_test(__unused struct sysctl_oid *oidp,
                             __unused void *arg1, __unused int arg2,
                             struct sysctl_req *req) {
  int error, newvalue, changed;

  lck_mtx_lock(&skmt_lock);
  if ((error = sysctl_io_number(req, skmt_enabled, sizeof(int), &newvalue,
                                &changed)) != 0) {
    goto done;
  }
  ...
  if (changed && skmt_enabled != newvalue) {
    ...
    do {
      ...
      error = msleep(&skmt_enabled, &skmt_lock, PWAIT | PCATCH, "skmtw", NULL);
      /* BEGIN CSTYLED */
      /*
