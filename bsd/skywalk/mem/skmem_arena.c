...
static void skmem_arena_init_common(struct skmem_arena *ar,
                                    skmem_arena_type_t type, size_t ar_zsize,
                                    const char *ar_str, const char *name);
...
SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, arena,
            ...
            skmem_arena_mib_get_sysctl, "S,sk_stats_arena",
...
static TAILQ_HEAD(, skmem_arena)
    skmem_arena_head = TAILQ_HEAD_INITIALIZER(skmem_arena_head);

#define SKMEM_ARENA_LOCK() lck_mtx_lock(&skmem_arena_lock)
...
#define SKMEM_ARENA_UNLOCK() lck_mtx_unlock(&skmem_arena_lock)
...
static int skmem_arena_sd_setup(const struct nexus_adapter *na,
                                struct skmem_region_params srp[SKMEM_REGIONS],
                                struct skmem_arena *ar, boolean_t kernel_only,
                                boolean_t tx) {
  ...
  if (ksd_skr == NULL) {
    SK_ERR("\"%s\" ar 0x%p flags 0x%x failed to create %s region", ar->ar_name,
           SK_KVA(ar), ar->ar_flags, srp[ksd_type].srp_name);
    err = ENOMEM;
    goto failed;
  }
  ar->ar_regions[ksd_type] = ksd_skr;
  if (!kernel_only) {
    usd_skr = skmem_region_create(name, &srp[usd_type], NULL, NULL, NULL);
    if (usd_skr == NULL) {
      err = ENOMEM;
      goto failed;
    }
  ...
  name = tsnprintf(cname, sizeof(cname), "%s_ksd.%.*s", tx ? "txa" : "rxf",
                   (int)sizeof(na->na_name), na->na_name);
  ASSERT(ar->ar_regions[ksd_type] != NULL);
  *cachep = skmem_cache_create(name, srp[ksd_type].srp_c_obj_size, 0, NULL,
                               NULL, NULL, NULL, ar->ar_regions[ksd_type],
                               SKMEM_CR_NOMAGAZINES);
  if (*cachep == NULL) {
    SK_ERR("\"%s\" ar %p flags 0x%x failed to create %s", ar->ar_name,
           SK_KVA(ar), ar->ar_flags, cname);
    err = ENOMEM;
    goto failed;
  }
  return 0;
...
SK_NO_INLINE_ATTRIBUTE
static void skmem_arena_sd_teardown(struct skmem_arena *ar, boolean_t tx) {
  struct skmem_arena_nexus *arn = (struct skmem_arena_nexus *)ar;
  struct skmem_cache **cachep;
...
static bool skmem_arena_pp_setup(struct skmem_arena *ar,
                                 struct skmem_region_params srp[SKMEM_REGIONS],
                                 const char *name, struct kern_pbufpool *rx_pp,
                                 struct kern_pbufpool *tx_pp,
                                 boolean_t kernel_only,
                                 boolean_t pp_truncated_buf) {
  struct skmem_arena_nexus *arn = (struct skmem_arena_nexus *)ar;
  ...
  if (rx_pp == NULL && tx_pp == NULL) {
    ...
    rx_pp = pp_create(name, srp, NULL, NULL, NULL, NULL, NULL, ppcreatef);
    if (rx_pp == NULL) {
      SK_ERR("\"%s\" ar %p flags 0x%x failed to create pp", ar->ar_name,
    ...
    ASSERT(
        !(!kernel_only && (PP_KERNEL_ONLY(rx_pp) || (PP_KERNEL_ONLY(tx_pp)))));
  ...
  if (rx_pp == tx_pp) {
    ...
    ar->ar_regions[SKMEM_REGION_BUF_LARGE] = PP_BUF_REGION_LARGE(rx_pp);
    ...
    if (rx_pp->pp_kbft_region != NULL) {
      ...
      ar->ar_regions[SKMEM_REGION_KBFT] = rx_pp->pp_kbft_region;
    }
    ...
    skmem_region_retain(PP_BUF_REGION_DEF(rx_pp));
    ar->ar_regions[SKMEM_REGION_RXBUF_DEF] = PP_BUF_REGION_DEF(rx_pp);
    ar->ar_regions[SKMEM_REGION_RXBUF_LARGE] = PP_BUF_REGION_LARGE(rx_pp);
    if (PP_BUF_REGION_LARGE(rx_pp) != NULL) {
      skmem_region_retain(PP_BUF_REGION_LARGE(rx_pp));
    }
    skmem_region_retain(PP_BUF_REGION_DEF(tx_pp));
    ar->ar_regions[SKMEM_REGION_TXBUF_DEF] = PP_BUF_REGION_DEF(tx_pp);
    ar->ar_regions[SKMEM_REGION_TXBUF_LARGE] = PP_BUF_REGION_LARGE(tx_pp);
    ...
    if (rx_pp->pp_kbft_region != NULL) {
      ...
      skmem_region_retain(rx_pp->pp_kbft_region);
      ar->ar_regions[SKMEM_REGION_RXKBFT] = rx_pp->pp_kbft_region;
    }
  ...
  if (kernel_only) {
    if ((arn->arn_mode & AR_NEXUS_MODE_EXTERNAL_PPOOL) == 0) {
      ...
      if (tx_pp->pp_kbft_region != NULL) {
        ASSERT(tx_pp->pp_kbft_region->skr_mirror == NULL);
      }
    }
  } else {
    ASSERT(rx_pp == tx_pp);
    ...
    if (rx_pp->pp_kbft_region != NULL) {
      ASSERT(rx_pp->pp_kbft_region->skr_mirror != NULL);
      ASSERT(rx_pp->pp_ubft_region != NULL);
      ASSERT(rx_pp->pp_ubft_region->skr_mode & SKR_MODE_MIRRORED);
      ar->ar_regions[SKMEM_REGION_UBFT] = rx_pp->pp_ubft_region;
...
static void skmem_arena_init_common(struct skmem_arena *ar,
  ...
  lck_mtx_init(&ar->ar_lock, &skmem_arena_lock_grp, LCK_ATTR_NULL);
  (void)snprintf(ar->ar_name, sizeof(ar->ar_name), "%s.%s.%s",
                 SKMEM_ARENA_PREFIX, ar_str, name);
}

/*
 * Create a nexus adapter arena.
 */
struct skmem_arena *skmem_arena_create_for_nexus(
...
#define SRP_CFLAGS(_id) (srp[_id].srp_cflags)
  ...
  *perr = 0;
  ...
  ar = &arn->arn_cmn;
  skmem_arena_init_common(ar, SKMEM_ARENA_TYPE_NEXUS, AR_NEXUS_SIZE, "nexus",
                          name);
  ...
  if ((na->na_flags & NAF_USER_PKT_POOL) == 0) {
    ASSERT(SRP_CFLAGS(SKMEM_REGION_TXAUSD) & SKMEM_REGION_CR_UREADONLY);
    ASSERT(SRP_CFLAGS(SKMEM_REGION_RXFUSD) & SKMEM_REGION_CR_UREADONLY);
  } else {
    ASSERT(!(SRP_CFLAGS(SKMEM_REGION_TXAUSD) & SKMEM_REGION_CR_UREADONLY));
    ASSERT(!(SRP_CFLAGS(SKMEM_REGION_RXFUSD) & SKMEM_REGION_CR_UREADONLY));
  }
  ...
  if (nxv != NULL && nxv->nxv_reg != NULL) {
    ...
    if (nxv->nxv_adv_type == NEXUS_ADVISORY_TYPE_FLOWSWITCH) {
      ...
      panic_plain("%s: invalid advisory type %d", __func__, nxv->nxv_adv_type);
      /* NOTREACHED */
    }
  ...
  for (i = 0; i < SKMEM_REGIONS; i++) {
    ...
    if (srp[i].srp_c_obj_cnt == 0) {
      ASSERT(i == SKMEM_REGION_GUARD_HEAD || i == SKMEM_REGION_USTATS ||
             i == SKMEM_REGION_KSTATS || i == SKMEM_REGION_INTRINSIC ||
             i == SKMEM_REGION_FLOWADV || i == SKMEM_REGION_NEXUSADV ||
             i == SKMEM_REGION_SYSCTLS || i == SKMEM_REGION_GUARD_TAIL);
      continue;
    }
     ...
     */
    if (kernel_only &&
        (i == SKMEM_REGION_GUARD_HEAD || i == SKMEM_REGION_SCHEMA ||
         i == SKMEM_REGION_RING || i == SKMEM_REGION_GUARD_TAIL)) {
      continue;
    }
    ...
    /* otherwise create it */
    if ((ar->ar_regions[i] =
             skmem_region_create(name, &srp[i], NULL, NULL, NULL)) == NULL) {
      SK_ERR("\"%s\" ar %p flags 0x%x failed to "
             "create %s region",
             ar->ar_name, SK_KVA(ar), ar->ar_flags, srp[i].srp_name);
      goto failed;
    }
  }
  ...
  if (ar->ar_regions[SKMEM_REGION_SCHEMA] != NULL) {
    name = tsnprintf(cname, sizeof(cname), "schema.%.*s",
                     (int)sizeof(na->na_name), na->na_name);
    if ((arn->arn_schema_cache = skmem_cache_create(
             name, srp[SKMEM_REGION_SCHEMA].srp_c_obj_size, 0, NULL, NULL, NULL,
             NULL, ar->ar_regions[SKMEM_REGION_SCHEMA],
  ...
  name = tsnprintf(cname, sizeof(cname), "ring.%.*s", (int)sizeof(na->na_name),
                   na->na_name);
  ASSERT(ar->ar_regions[SKMEM_REGION_RING] != NULL || kernel_only);
  if ((ar->ar_regions[SKMEM_REGION_RING] != NULL) &&
      (arn->arn_ring_cache = skmem_cache_create(
           name, srp[SKMEM_REGION_RING].srp_c_obj_size, 0, NULL, NULL, NULL,
           NULL, ar->ar_regions[SKMEM_REGION_RING], SKMEM_CR_NOMAGAZINES)) ==
          NULL) {
    SK_ERR("\"%s\" ar %p flags 0x%x failed to create %s", ar->ar_name,
           SK_KVA(ar), ar->ar_flags, cname);
    goto failed;
  ...
  if (ar->ar_regions[SKMEM_REGION_USTATS] != NULL) {
    ...
    if ((obj = skmem_region_alloc(skr, &maddr, NULL, NULL, SKMEM_SLEEP,
                                  skr->skr_c_obj_size, &msize)) == NULL) {
      SK_ERR("\"%s\" ar %p flags 0x%x failed to alloc stats", ar->ar_name,
             SK_KVA(ar), ar->ar_flags);
      goto failed;
    }
  ...
  if (ar->ar_regions[SKMEM_REGION_FLOWADV] != NULL) {
    struct skmem_region *skr = ar->ar_regions[SKMEM_REGION_FLOWADV];
    void *obj;
  ...
  if (skmem_arena_create_finalize(ar) != 0) {
    SK_ERR("\"%s\" ar %p flags 0x%x failed to finalize", ar->ar_name,
           SK_KVA(ar), ar->ar_flags);
    goto failed;
  }
...
void skmem_arena_nexus_sd_set_noidle(struct skmem_arena_nexus *arn, int cnt) {
...
boolean_t skmem_arena_nexus_sd_idle(struct skmem_arena_nexus *arn) {
  struct skmem_arena *ar = &arn->arn_cmn;
  boolean_t idle;
...
static void skmem_arena_nexus_teardown(struct skmem_arena_nexus *arn,
                                       boolean_t defunct) {
  struct skmem_arena *ar = &arn->arn_cmn;
  ...
  if (arn->arn_flowadv_obj != NULL) {
    ...
    /* XXX -fbounds-safety */
    void *obj = __unsafe_forge_bidi_indexable(void *, arn->arn_flowadv_obj,
                                              skr->skr_c_obj_size);
...
skmem_arena_create_for_necp(const char *name,
  ...
  *perr = 0;
  ...
  ar = &arc->arc_cmn;
  skmem_arena_init_common(ar, SKMEM_ARENA_TYPE_NECP, AR_NECP_SIZE, "necp",
                          name);
  ...
  if ((ar->ar_regions[SKMEM_REGION_USTATS] =
           skmem_region_create(name, srp_ustats, NULL, NULL, NULL)) == NULL) {
    goto failed;
  }

  if ((ar->ar_regions[SKMEM_REGION_KSTATS] =
           skmem_region_create(name, srp_kstats, NULL, NULL, NULL)) == NULL) {
    goto failed;
  }
  ...
  if ((arc->arc_kstats_cache = skmem_cache_create(
           cache_name, srp_kstats->srp_c_obj_size, 0, necp_stats_ctor, NULL,
           NULL, NULL, ar->ar_regions[SKMEM_REGION_KSTATS],
           SKMEM_CR_NOMAGAZINES)) == NULL) {
    goto failed;
  }
...
failed:
  SK_ERR("\"%s\" ar %p flags 0x%x failed to create %s region", ar->ar_name,
         SK_KVA(ar), ar->ar_flags, srp_kstats->srp_name);
  AR_LOCK_ASSERT_HELD(ar);
  skmem_arena_destroy(ar);
  *perr = ENOMEM;
...
static void skmem_arena_necp_teardown(struct skmem_arena_necp *arc,
                                      boolean_t defunct) {
 ...
 */
struct skmem_arena_necp *skmem_arena_necp(struct skmem_arena *ar) {
 ...
 */
struct skmem_arena *skmem_arena_create_for_system(const char *name, int *perr) {
  ...
  *perr = 0;
  ...
  ar = &ars->ars_cmn;
  skmem_arena_init_common(ar, SKMEM_ARENA_TYPE_SYSTEM, AR_SYSTEM_SIZE, "system",
                          name);
  ...
  ASSERT((skrsys->skr_mode &
          (SKR_MODE_MMAPOK | SKR_MODE_NOMAGAZINES | SKR_MODE_KREADONLY |
           SKR_MODE_UREADONLY | SKR_MODE_MONOLITHIC | SKR_MODE_SHAREOK)) ==
...
static void skmem_arena_system_teardown(struct skmem_arena_system *ars,
                                        boolean_t defunct) {
 ...
 */
struct skmem_arena_system *skmem_arena_system(struct skmem_arena *ar) {
...
void *skmem_arena_system_sysctls_obj_addr(struct skmem_arena *ar) {
  ASSERT(ar->ar_type == SKMEM_ARENA_TYPE_SYSTEM);
  return skmem_arena_system(ar)->ars_sysctls_obj;
}

size_t skmem_arena_system_sysctls_obj_size(struct skmem_arena *ar) {
  ASSERT(ar->ar_type == SKMEM_ARENA_TYPE_SYSTEM);
 ...
 * Destroy a region.
 */
static void skmem_arena_destroy(struct skmem_arena *ar) {
  AR_LOCK_ASSERT_HELD(ar);

  SK_DF(SK_VERB_MEM_ARENA, "\"%s\" ar %p flags 0x%x", ar->ar_name, SK_KVA(ar),
        ar->ar_flags);
 ...
 */
static void skmem_arena_teardown(struct skmem_arena *ar, boolean_t defunct) {
  uint32_t i;

  switch (ar->ar_type) {
  case SKMEM_ARENA_TYPE_NEXUS:
    skmem_arena_nexus_teardown((struct skmem_arena_nexus *)ar, defunct);
    break;

  case SKMEM_ARENA_TYPE_NECP:
    skmem_arena_necp_teardown((struct skmem_arena_necp *)ar, defunct);
    break;

  case SKMEM_ARENA_TYPE_SYSTEM:
    skmem_arena_system_teardown((struct skmem_arena_system *)ar, defunct);
    break;
...
static int skmem_arena_create_finalize(struct skmem_arena *ar) {
  ...
  if (ar->ar_ar == NULL) {
    SK_ERR("\"%s\" ar %p flags 0x%x failed to create IOSKArena of"
           "%u regions",
           ar->ar_name, SK_KVA(ar), ar->ar_flags, regcnt);
    err = ENOMEM;
    goto failed;
  }
...
static void skmem_arena_free(struct skmem_arena *ar) {
...
void skmem_arena_retain(struct skmem_arena *ar) {
  AR_LOCK(ar);
 ...
 */
__attribute__((always_inline)) static inline boolean_t
...
boolean_t skmem_arena_release(struct skmem_arena *ar) {
  boolean_t lastref;
 ...
 */
int skmem_arena_mmap(struct skmem_arena *ar, struct proc *p,
 ...
 */
static boolean_t skmem_arena_munmap_common(struct skmem_arena *ar,
                                           struct skmem_arena_mmap_info *ami) {
  ...
  if (ami->ami_mapref != NULL) {
    ...
    SLIST_REMOVE(&ar->ar_map_head, ami, skmem_arena_mmap_info, ami_link);
    ...
    if (ami->ami_redirect) {
      ...
      ar->ar_maprdrcnt--;
    } else if (ar->ar_maprdrcnt != 0 && ar->ar_maprdrcnt == ar->ar_mapcnt) {
      /*
 ...
 */
void skmem_arena_munmap_channel(struct skmem_arena *ar,
  ...
  if (skmem_arena_munmap_common(ar, &ch->ch_mmap)) {
     ...
     */
    nxdom_prov->nxdom_prov_dom->nxdom_defunct_finalize(nxdom_prov, nx, ch,
                                                       TRUE);
  }
}
 ...
 */
void skmem_arena_munmap(struct skmem_arena *ar,
                        struct skmem_arena_mmap_info *ami) {
 ...
 */
int skmem_arena_mredirect(struct skmem_arena *ar,
                          struct skmem_arena_mmap_info *ami, struct proc *p,
...
#pragma unused(p)
  ...
  *need_defunct = FALSE;
  ...
  SK_DF(((err != 0) ? SK_VERB_ERROR : SK_VERB_DEFAULT),
        ...
        "err %d",
        sk_proc_name(p), sk_proc_pid(p), ar->ar_name, SK_KVA(ar), ar->ar_flags,
        !(ar->ar_flags & ARF_ACTIVE), *need_defunct, err);
 ...
 * Defunct a region.
 */
int skmem_arena_defunct(struct skmem_arena *ar) {
  AR_LOCK(ar);
 ...
 */
void skmem_arena_get_stats(struct skmem_arena *ar, uint64_t *mem_total,
                           uint64_t *mem_inuse) {
  uint32_t i;
 ...
 */
mach_vm_offset_t skmem_arena_get_region_offset(struct skmem_arena *ar,
                                               skmem_region_id_t id) {
...
static void skmem_reap_pbufpool_caches(struct kern_pbufpool *pp,
                                       boolean_t purge) {
 ...
 */
static void skmem_arena_reap_locked(struct skmem_arena *ar, boolean_t purge) {
...
void skmem_arena_reap(struct skmem_arena *ar, boolean_t purge) {
  AR_LOCK(ar);
...
#if SK_LOG
SK_LOG_ATTRIBUTE
static void skmem_arena_create_region_log(struct skmem_arena *ar) {
  char label[32];
  int i;

  switch (ar->ar_type) {
  case SKMEM_ARENA_TYPE_NEXUS:
    SK_D("\"%s\" ar %p flags 0x%x rx_pp %p tx_pp %p", ar->ar_name, SK_KVA(ar),
         ar->ar_flags, SK_KVA(skmem_arena_nexus(ar)->arn_rx_pp),
         SK_KVA(skmem_arena_nexus(ar)->arn_tx_pp));
    break;

  case SKMEM_ARENA_TYPE_NECP:
  case SKMEM_ARENA_TYPE_SYSTEM:
    SK_D("\"%s\" ar %p flags 0x%x", ar->ar_name, SK_KVA(ar), ar->ar_flags);
    break;
  }
  ...
  for (i = 0; i < SKMEM_REGIONS; i++) {
    ...
    (void)snprintf(label, sizeof(label), "REGION_%s:", skmem_region_id2name(i));
    SK_D("  %-16s %6u KB s:[%2u x %6u KB] "
         "o:[%4u x %6u -> %4u x %6u]",
         label, (uint32_t)AR_MEM_TOTAL(ar, i) >> 10,
         (uint32_t)AR_MEM_SEGCNT(ar, i), (uint32_t)AR_MEM_SEGSIZE(ar, i) >> 10,
         (uint32_t)AR_MEM_OBJCNT_R(ar, i), (uint32_t)AR_MEM_OBJSIZE_R(ar, i),
         (uint32_t)AR_MEM_OBJCNT_C(ar, i), (uint32_t)AR_MEM_OBJSIZE_C(ar, i));
  }
}
#endif /* SK_LOG */

static size_t skmem_arena_mib_get_stats(struct skmem_arena *ar,
  ...
  AR_LOCK(ar);
  (void)snprintf(sar->sar_name, sizeof(sar->sar_name), "%s", ar->ar_name);
  ...
  SLIST_FOREACH(ami, &ar->ar_map_head, ami_link) {
    if (ami->ami_arena->ar_type == SKMEM_ARENA_TYPE_NEXUS) {
      ...
      ch = __unsafe_forge_single(
          struct kern_channel *,
          container_of(ami, struct kern_channel, ch_mmap));
      proc_pid = ch->ch_pid;
    } else {
      ASSERT((ami->ami_arena->ar_type == SKMEM_ARENA_TYPE_NECP) ||
             (ami->ami_arena->ar_type == SKMEM_ARENA_TYPE_SYSTEM));
      proc_pid = necp_client_get_proc_pid_from_arena_info(ami);
    }
...
SK_NO_INLINE_ATTRIBUTE
char *ar2str(const struct skmem_arena *ar, char *__counted_by(dsz) dst,
             size_t dsz) {
  (void)sk_snprintf(dst, dsz, "%p %s flags 0x%b", SK_KVA(ar), ar->ar_name,
                    ar->ar_flags, ARF_BITS);
