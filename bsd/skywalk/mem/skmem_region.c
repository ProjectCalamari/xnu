...
static void sksegment_freelist_insert(struct skmem_region *, struct sksegment *,
                                      boolean_t);
static struct sksegment *sksegment_freelist_remove(struct skmem_region *,
                                                   struct sksegment *, uint32_t,
                                                   boolean_t);
...
static struct sksegment *sksegment_alloc_with_idx(struct skmem_region *,
                                                  uint32_t);
static void *__sized_by(seg_size)
    skmem_region_alloc_common(struct skmem_region *, struct sksegment *,
                              uint32_t seg_size);
static void *__sized_by(seg_size)
    skmem_region_mirror_alloc(struct skmem_region *, struct sksegment *,
                              uint32_t seg_size, struct sksegment **);
...
SYSCTL_PROC(_kern_skywalk_stats, OID_AUTO, region,
            ...
            skmem_region_mib_get_sysctl, "S,sk_stats_region",
...
#define SKMEM_REGION_LOCK() lck_mtx_lock(&skmem_region_lock)
...
#define SKMEM_REGION_UNLOCK() lck_mtx_unlock(&skmem_region_lock)
...
#define SKMEM_REGION_HASH(_skr, _addr)                                         \
  (&(_skr)->skr_hash_table[SKMEM_REGION_HASH_INDEX(                            \
      (uintptr_t)_addr, (_skr)->skr_hash_shift, (_skr)->skr_hash_mask)])
...
void skmem_region_init(void) {
  ...
  skmem_seg_size = (uint32_t)P2ROUNDUP(skmem_seg_size, SKMEM_MIN_SEG_SIZE);
  ...
  skmem_md_seg_size =
      (uint32_t)P2ROUNDUP(skmem_md_seg_size, SKMEM_MIN_SEG_SIZE);
   ...
   */
  randomize_seg_size =
      !PE_parse_boot_argn("skmem_drv_buf_seg_size", &skmem_drv_buf_seg_size,
                          sizeof(skmem_drv_buf_seg_size));
  ...
  (void)PE_parse_boot_argn("skmem_usr_buf_seg_size", &skmem_usr_buf_seg_size,
                           sizeof(skmem_usr_buf_seg_size));
  if (skmem_usr_buf_seg_size < skmem_seg_size) {
    skmem_usr_buf_seg_size = skmem_seg_size;
  }
  skmem_usr_buf_seg_size =
      (uint32_t)P2ROUNDUP(skmem_usr_buf_seg_size, SKMEM_MIN_SEG_SIZE);
  VERIFY((skmem_usr_buf_seg_size % SKMEM_PAGE_SIZE) == 0);

  SK_D("seg_size %u, md_seg_size %u, drv_buf_seg_size %u [eff %u], "
       "usr_buf_seg_size %u",
       skmem_seg_size, skmem_md_seg_size, skmem_drv_buf_seg_size,
       skmem_drv_buf_seg_eff_size, skmem_usr_buf_seg_size);

  TAILQ_INIT(&skmem_region_head);

  skmem_region_update_tc = thread_call_allocate_with_options(
      skmem_region_update_func, NULL, THREAD_CALL_PRIORITY_KERNEL,
  ...
  skmem_sg_cache = skmem_cache_create("sg", sg_size, sizeof(uint64_t), NULL,
 ...
 */
void skmem_region_reap_caches(boolean_t purge) {
 ...
 */
void skmem_region_params_config(struct skmem_region_params *srp) {
  ...
  if (srp->srp_r_seg_size == 0) {
    ...
    srp->srp_r_seg_size =
        (uint32_t)P2ROUNDUP(srp->srp_r_seg_size, SKMEM_MIN_SEG_SIZE);
  }

  seglim = srp->srp_r_seg_size;
  VERIFY(seglim != 0 && (seglim % SKMEM_PAGE_SIZE) == 0);

  SK_DF(SK_VERB_MEM, "%s: seglim %zu objsize %zu objcnt %zu", srp->srp_name,
        seglim, objsize, objcnt);
  ...
  if (!((objsize < cache_line_size) && (objsize < seglim) &&
    ...
    while (objsize < seglim && (seglim % objsize) != 0) {
      SK_DF(SK_VERB_MEM, "%s: objsize %zu -> %zu", srp->srp_name, objsize,
  ...
  while (objsize > seglim) {
    SK_DF(SK_VERB_MEM, "%s: seglim %zu -> %zu", srp->srp_name, seglim,
  ...
  if (!(srp->srp_cflags & SKMEM_REGION_CR_NOMAGAZINES)) {
    uint32_t magazine_max_objs = skmem_cache_magazine_max((uint32_t)objsize);
    SK_DF(SK_VERB_MEM, "%s: objcnt %zu -> %zu", srp->srp_name, objcnt,
          objcnt + magazine_max_objs);
    objcnt += magazine_max_objs;
  }

  SK_DF(SK_VERB_MEM,
        "%s: seglim %zu objsize %zu "
        "objcnt %zu",
  ...
  if (seglim > segsize) {
    ...
    while (seglim > segsize && seglim > SKMEM_MIN_SEG_SIZE) {
      ...
      SK_DF(SK_VERB_MEM, "%s: segsize %zu (%zu*%zu) seglim [-] %zu -> %zu",
            srp->srp_name, segsize, objsize, objcnt, seglim,
            P2ROUNDUP(seglim - SKMEM_MIN_SEG_SIZE, SKMEM_MIN_SEG_SIZE));
      seglim = P2ROUNDUP(seglim - SKMEM_MIN_SEG_SIZE, SKMEM_MIN_SEG_SIZE);
    }
  ...
  SK_DF(SK_VERB_MEM, "%s: segsize %zu seglim %zu", srp->srp_name, segsize,
        seglim);
  ...
  SK_DF(SK_VERB_MEM, "%s: segcnt %zu segsize %zu", srp->srp_name, segcnt,
        segsize);
  ...
  if (__improbable(sk_verbose != 0)) {
    ...
    (void)snprintf(label, sizeof(label),
                   "REGION_%s:", skmem_region_id2name(srp->srp_id));
    SK_D("%-16s o:[%4u x %6u -> %4u x %6u]", label,
         (uint32_t)srp->srp_r_obj_cnt, (uint32_t)srp->srp_r_obj_size,
         (uint32_t)srp->srp_c_obj_cnt, (uint32_t)srp->srp_c_obj_size);
  }
#endif /* SK_LOG */
}
 ...
 * Create a region.
 */
struct skmem_region *skmem_region_create(const char *name,
                                         struct skmem_region_params *srp,
                                         sksegment_ctor_fn_t ctor,
                                         sksegment_dtor_fn_t dtor,
                                         void *private) {
  ...
  if (!pseudo) {
    ...
    skr->skr_hash_table =
        sk_alloc_type_array(struct sksegment_bkt, size, Z_WAITOK | Z_NOFAIL,
  ...
  lck_mtx_init(&skr->skr_lock, &skmem_region_lock_grp, &skmem_region_lock_attr);
  ...
  (void)snprintf(skr->skr_name, sizeof(skr->skr_name), "%s.%s.%s",
                 SKMEM_REGION_PREFIX, srp->srp_name, name);

  SK_DF(SK_VERB_MEM_REGION, "\"%s\": skr %p ", skr->skr_name, SK_KVA(skr));
  ...
  if (!(skr->skr_mode & SKR_MODE_PSEUDO)) {
    ...
    skr->skr_seg_bmap =
        sk_alloc_data(size, Z_WAITOK | Z_NOFAIL, skmem_tag_segment_bmap);
  ...
  if (skr->skr_mode & SKR_MODE_PSEUDO) {
    char zone_name[64];
    (void)snprintf(zone_name, sizeof(zone_name), "%s.reg.%s", SKMEM_ZONE_PREFIX,
                   name);
    ...
    if ((skr->skr_reg =
             IOSKRegionCreate(&skr->skr_regspec, (IOSKSize)skr->skr_seg_size,
                              (IOSKCount)skr->skr_seg_max_cnt)) == NULL) {
      SK_ERR("\%s\": [%u * %u] cflags 0x%x skr_reg failed", skr->skr_name,
  ...
  SK_DF(SK_VERB_MEM_REGION, "  [TOTAL] seg (%u*%u) obj (%u*%u) cflags 0x%x",
        (uint32_t)skr->skr_seg_size, (uint32_t)skr->skr_seg_max_cnt,
...
static void skmem_region_destroy(struct skmem_region *skr) {
  ...
  if (skr->skr_seginuse != 0) {
    panic("%s: '%s' (%p) not empty (%u unfreed)", __func__, skr->skr_name,
          (void *)skr, skr->skr_seginuse);
    /* NOTREACHED */
    __builtin_unreachable();
  }
  ...
  if (skr->skr_hash_table != NULL) {
    ...
    sk_free_type_array(struct sksegment_bkt, skr->skr_hash_size, htable);
 ...
 */
void skmem_region_mirror(struct skmem_region *skr, struct skmem_region *mskr) {
  ASSERT(mskr != NULL);
  SK_DF(SK_VERB_MEM_REGION, "skr master %p, slave %p ", SK_KVA(skr),
        SK_KVA(mskr));
...
void skmem_region_slab_config(struct skmem_region *skr, struct skmem_cache *skm,
                              bool attach) {
  int i;

  SKR_LOCK(skr);
  if (attach) {
    for (i = 0; i < SKR_MAX_CACHES && skr->skr_cache[i] != NULL; i++) {
      ;
    }
    ...
    for (i = 0; i < SKR_MAX_CACHES && skr->skr_cache[i] != skm; i++) {
      ;
    }
    VERIFY(i < SKR_MAX_CACHES);
 ...
 */
static void *__sized_by(objsize)
 ...
 * land.
 */
void *__sized_by(objsize)
    skmem_region_alloc(struct skmem_region *skr,
                       void *__sized_by(*msize) * maddr,
                       struct sksegment **retsg, struct sksegment **retsgm,
                       uint32_t skmflag, uint32_t SK_FB_ARG objsize,
                       uint32_t *SK_FB_ARG msize) {
    ...
    if (sg == NULL) {
      ...
      if (skr->skr_mode & SKR_MODE_SLAB) {
        ...
        for (int i = 0; i < SKR_MAX_CACHES; i++) {
          ...
          skmem_cache_reap_now(skr->skr_cache[i], TRUE);
        }
        SKR_LOCK(skr);
        /*
      ...
      ++skr->skr_seg_waiters;
      (void)msleep(&skr->skr_seg_free, &skr->skr_lock, (PZERO - 1),
                   skr->skr_name, NULL);
    }
  }
  ...
  retry:
     ...
     */
    if (sksegment_freelist_remove(skr, sg, skmflag, FALSE) == NULL) {
      ...
      if (skmflag & SKMEM_NOSLEEP) {
        ...
        if (++retries > SKMEM_WDT_MAXTIME) {
          panic_plain("\"%s\": failed to "
                      ...
                      "after %u retries\n\n%s",
                      skr->skr_name, SKMEM_WDT_MAXTIME, skmem_dump(skr));
          /* NOTREACHED */
          __builtin_unreachable();
        } else {
  ...
  if (sg == NULL) {
    ...
    if (skmflag & SKMEM_PANIC) {
      VERIFY((skmflag & (SKMEM_NOSLEEP | SKMEM_FAILOK)) == SKMEM_NOSLEEP);
      /*
...
#if SK_LOG
  SK_DF(SK_VERB_MEM_REGION, "skr %p sg %p", SK_KVA(skr), SK_KVA(sg));
  if (skr->skr_mirror == NULL ||
      !(skr->skr_mirror->skr_mode & SKR_MODE_MIRRORED)) {
    SK_DF(SK_VERB_MEM_REGION, "  [%u] [%p-%p)", sg->sg_index,
          SK_KVA(sg->sg_start), SK_KVA(sg->sg_end));
  } else {
    SK_DF(SK_VERB_MEM_REGION, "  [%u] [%p-%p) mirrored", sg->sg_index,
          SK_KVA(sg->sg_start), SK_KVA(sg->sg_end));
  }
#endif /* SK_LOG */
...
static void *__sized_by(seg_size)
    skmem_region_mirror_alloc(struct skmem_region *skr, struct sksegment *sg0,
                              uint32_t SK_FB_ARG seg_size,
                              struct sksegment **__single retsg) {
 ...
 */
void skmem_region_free(struct skmem_region *skr, void *addr, void *maddr) {
  struct sksegment_bkt *sgb;
  ...
  if (skr->skr_mirror == NULL ||
      !(skr->skr_mirror->skr_mode & SKR_MODE_MIRRORED)) {
    SK_DF(SK_VERB_MEM_REGION, "  [%u] [%p-%p)", sg->sg_index, SK_KVA(addr),
          SK_KVA((uintptr_t)addr + skr->skr_seg_size));
  } else {
    SK_DF(SK_VERB_MEM_REGION, "  [%u] [%p-%p) mirrored", sg->sg_index,
          SK_KVA(addr), SK_KVA((uintptr_t)addr + skr->skr_seg_size));
  }
#endif /* SK_LOG */
  ...
  if (skr->skr_seg_waiters != 0) {
    SK_DF(SK_VERB_MEM_REGION, "sg %p waking up %u waiters", SK_KVA(sg),
          skr->skr_seg_waiters);
 ...
 * Retain a segment.
 */
void skmem_region_retain(struct skmem_region *skr) {
  SKR_LOCK(skr);
  skmem_region_retain_locked(skr);
  SKR_UNLOCK(skr);
}
 ...
 */
boolean_t skmem_region_release(struct skmem_region *skr) {
  boolean_t lastref;
 ...
 */
static void skmem_region_depopulate(struct skmem_region *skr) {
  struct sksegment *sg, *tsg;
 ...
 */
static int sksegment_cmp(const struct sksegment *sg1,
                         const struct sksegment *sg2) {
 ...
 */
static struct sksegment *sksegment_create(struct skmem_region *skr,
                                          uint32_t i) {
  ...
  SK_DF(SK_VERB_MEM_REGION, "  [%u] [%p-%p) 0x%x", i, SK_KVA(sg->sg_start),
        SK_KVA(sg->sg_end), skr->skr_mode);
 ...
 */
static void sksegment_destroy(struct skmem_region *skr, struct sksegment *sg) {
 ...
 */
static void sksegment_freelist_insert(struct skmem_region *skr,
                                      struct sksegment *sg,
 ...
 */
static struct sksegment *sksegment_freelist_remove(struct skmem_region *skr,
                                                   struct sksegment *sg,
                                                   uint32_t skmflag,
                                                   boolean_t purging) {
#pragma unused(skmflag)
   ...
   */
  if (__improbable(mtbf != 0 && !purging && (net_uptime_ms() % mtbf) == 0 &&
                   !(skmflag & SKMEM_PANIC))) {
    SK_ERR("skr \"%s\" %p sg %p MTBF failure", skr->skr_name, SK_KVA(skr),
           SK_KVA(sg));
    net_update_uptime();
    return NULL;
  }
  ...
  if ((sg->sg_md = IOSKMemoryBufferCreate(skr->skr_seg_size, &skr->skr_bufspec,
                                          &segstart)) == NULL) {
    ASSERT(sg->sg_type == SKSEG_TYPE_FREE);
    if (skmflag & SKMEM_PANIC) {
      ...
      panic_plain("\"%s\": skr 0x%p sg 0x%p (idx %u) unable "
                  ...
                  skr->skr_name, skr, sg, sg->sg_index);
      /* NOTREACHED */
      __builtin_unreachable();
    }
  ...
  sg->sg_state = IOSKBufferIsWired(sg->sg_md) ? SKSEG_STATE_MAPPED_WIRED
                                              : SKSEG_STATE_MAPPED;
 ...
 */
static struct sksegment *sksegment_freelist_grow(struct skmem_region *skr) {
  struct sksegment *sg = NULL;
  uint32_t i, j, idx;
 ...
 */
static struct sksegment *sksegment_alloc_with_idx(struct skmem_region *skr,
                                                  uint32_t idx) {
  struct sksegment *sg;
  ...
  if (!bit_test(skr->skr_seg_bmap[idx / BMAPSZ], idx % BMAPSZ)) {
    panic("%s: '%s' (%p) idx %u (out of %u) is already allocated", __func__,
          skr->skr_name, (void *)skr, idx, (skr->skr_seg_max_cnt - 1));
    /* NOTREACHED */
    __builtin_unreachable();
  }
 ...
 */
static void skmem_region_hash_rescale(struct skmem_region *skr) {
   ...
   */
  new_size =
      MAX(skr->skr_hash_initial, (1 << (flsll(3 * skr->skr_seginuse + 4) - 2)));
  ...
  new_table = sk_alloc_type_array(struct sksegment_bkt, new_size, Z_NOWAIT,
                                  skmem_tag_segment_hash);
  ...
  SK_DF(SK_VERB_MEM_REGION, "skr %p old_size %zu new_size %zu [%u moved]",
        SK_KVA(skr), old_size, new_size, moved);
 ...
 */
static void skmem_region_applyall(void (*func)(struct skmem_region *)) {
  struct skmem_region *skr;

  net_update_uptime();

  SKMEM_REGION_LOCK();
  TAILQ_FOREACH(skr, &skmem_region_head, skr_link) { func(skr); }
  SKMEM_REGION_UNLOCK();
}

static void skmem_region_update(struct skmem_region *skr) {
   ...
   */
  if (skr->skr_seg_waiters != 0) {
    SK_DF(SK_VERB_MEM_REGION, "waking up %u waiters to reclaim",
          skr->skr_seg_waiters);
    skr->skr_seg_waiters = 0;
    wakeup(&skr->skr_seg_free);
  }
 ...
 */
static void skmem_region_update_func(thread_call_param_t dummy,
...
boolean_t skmem_region_for_pp(skmem_region_id_t id) {
  int i;
...
void skmem_region_get_stats(struct skmem_region *skr,
                            struct sk_stats_region *sreg) {
  bzero(sreg, sizeof(*sreg));

  (void)snprintf(sreg->sreg_name, sizeof(sreg->sreg_name), "%s", skr->skr_name);
...
static size_t skmem_region_mib_get_stats(struct skmem_region *skr,
...
#if SK_LOG
const char *skmem_region_id2name(skmem_region_id_t id) {
  const char *name;
  switch (id) {
  ...
  const char *__null_terminated s =
      __unsafe_null_terminated_from_indexable(name);
...
#if (DEVELOPMENT || DEBUG)
uint64_t skmem_region_get_mtbf(void) { return skmem_region_mtbf; }

void skmem_region_set_mtbf(uint64_t newval) {
...
static int skmem_region_mtbf_sysctl(struct sysctl_oid *oidp, void *arg1,
...
#pragma unused(oidp, arg1, arg2)
  ...
  if ((error = sysctl_io_number(req, skmem_region_mtbf, sizeof(uint64_t),
                                &newval, &changed)) == 0) {
    if (changed) {
