...
uint32_t bsd_post_tests_count =
    sizeof(bsd_post_tests) / sizeof(xnupost_test_data_t);
...
int bsd_list_tests() {
  ...
  while (no_load_counter > 0) {
    ...
    if (absolute_break_counter <= 0) {
      printf("bsd_list_tests: WARNING: Waiting beyond normal time for "
             "stabilizing kext loading\n");
      break;
    }
    ...
    if (prev_load_time == last_loaded_timestamp) {
      no_load_counter -= 1;
      printf(
          "bsd_list_tests: INFO: no new kexts loaded. remaining checks: %d\n",
          no_load_counter);
    }
...
struct kcdata_subtype_descriptor kc_xnupost_test_def[] = {
    {.kcs_flags = KCS_SUBTYPE_FLAGS_NONE,
     .kcs_elem_type = KC_ST_UINT16,
     .kcs_elem_offset = 0,
     .kcs_elem_size = sizeof(uint16_t),
     .kcs_name = "config"},
    {.kcs_flags = KCS_SUBTYPE_FLAGS_NONE,
     .kcs_elem_type = KC_ST_UINT16,
     .kcs_elem_offset = 1 * sizeof(uint16_t),
     .kcs_elem_size = sizeof(uint16_t),
     .kcs_name = "test_num"},
    {.kcs_flags = KCS_SUBTYPE_FLAGS_NONE,
     .kcs_elem_type = KC_ST_INT32,
     .kcs_elem_offset = 2 * sizeof(uint16_t),
     .kcs_elem_size = sizeof(int32_t),
     .kcs_name = "retval"},
    {.kcs_flags = KCS_SUBTYPE_FLAGS_NONE,
     .kcs_elem_type = KC_ST_INT32,
     .kcs_elem_offset = 2 * sizeof(uint16_t) + sizeof(int32_t),
     .kcs_elem_size = sizeof(int32_t),
     .kcs_name = "expected_retval"},
    {.kcs_flags = KCS_SUBTYPE_FLAGS_NONE,
    ...
    {.kcs_flags = KCS_SUBTYPE_FLAGS_ARRAY,
     ...
     .kcs_elem_offset =
         2 * (sizeof(uint16_t) + sizeof(int32_t) + sizeof(uint64_t)),
     .kcs_elem_size = KCS_SUBTYPE_PACK_SIZE(XNUPOST_TNAME_MAXLEN * sizeof(char),
                                            sizeof(char)),
     .kcs_name = "test_name"}};

const uint32_t kc_xnupost_test_def_count =
    sizeof(kc_xnupost_test_def) / sizeof(struct kcdata_subtype_descriptor);

kern_return_t xnupost_copyout_test(xnupost_test_t t, mach_vm_address_t outaddr);

int xnupost_copyout_test(xnupost_test_t t, mach_vm_address_t outaddr) {
  ...
  kret =
      copyout(&t->xt_expected_retval, (user_addr_t)outaddr, sizeof(uint32_t));
  if (kret) {
    return kret;
  }
  outaddr += sizeof(uint32_t);
...
uint32_t xnupost_get_estimated_testdata_size(void) {
  ...
  uint32_t elem_size =
      kc_xnupost_test_def[kc_xnupost_test_def_count - 1].kcs_elem_offset +
...
int xnupost_export_testdata(void *outp, size_t size_in, uint32_t *lenp) {
...
#define RET_IF_OP_FAIL                                                         \
  do {                                                                         \
    if (kret != KERN_SUCCESS) {                                                \
      return (kret == KERN_NO_ACCESS)                                          \
                 ? EACCES                                                      \
                 : ((kret == KERN_RESOURCE_SHORTAGE) ? ENOMEM : EINVAL);       \
    }                                                                          \
  } while (0)

  kret = kcdata_memory_static_init(&kcd, (mach_vm_address_t)outp,
                                   KCDATA_BUFFER_BEGIN_XNUPOST_CONFIG, size,
                                   KCFLAG_USE_COPYOUT);
  RET_IF_OP_FAIL;

  /* add mach timebase info */
  clock_timebase_info(&timebase);
  kret = kcdata_get_memory_addr(&kcd, KCDATA_TYPE_TIMEBASE, sizeof(timebase),
                                &user_addr);
  RET_IF_OP_FAIL;
  ...
  kret = kcdata_get_memory_addr(&kcd, STACKSHOT_KCTYPE_OSVERSION,
                                length_to_copy, &user_addr);
  RET_IF_OP_FAIL;
  ...
  kret = kcdata_get_memory_addr(&kcd, STACKSHOT_KCTYPE_BOOTARGS, length_to_copy,
                                &user_addr);
  RET_IF_OP_FAIL;
  ...
  kret = kcdata_add_type_definition(&kcd, XNUPOST_KCTYPE_TESTCONFIG,
                                    kctype_name, &kc_xnupost_test_def[0],
  ...
  kret = kcdata_get_memory_addr_for_array(&kcd, XNUPOST_KCTYPE_TESTCONFIG,
                                          elem_size, total_tests, &user_addr);
  RET_IF_OP_FAIL;

  for (i = 0; i < bsd_post_tests_count; i++) {
    tmp_entry_addr =
        (mach_vm_address_t)((uint64_t)(user_addr) + (uint64_t)(i * elem_size));
