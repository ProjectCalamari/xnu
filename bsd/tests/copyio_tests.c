...
static const char copyio_test_string[] = {'T',  'e', 's', 't', ' ', 'S',
                                          't',  'r', 'i', 'n', 'g', '!',
                                          '\0', 'A', 'B', 'C'};
...
struct copyio_test_data {
  ...
  /* The start of a page-sized region that guaranteed to be unmapped in
   * userspace. */
  user_addr_t unmapped_addr;
  /* The start of a page-sized region mapped at the largest possible userspace
   * address. */
...
static void copyio_thread_call_fn(void *arg, wait_result_t __unused res) {
  struct copyio_test_thread_data *tdata = arg;
  tdata->ret = tdata->fn(tdata->data);
  semaphore_signal(tdata->done);
}

static int copyio_test_run_in_thread(copyio_thread_fn_t fn,
                                     struct copyio_test_data *data) {
...
static void copyio_test_protect(struct copyio_test_data *data, vm_prot_t prot) {
  __assert_only kern_return_t ret = mach_vm_protect(
      data->user_map, data->user_addr, copyio_test_buf_size, false, prot);
  assert(ret == KERN_SUCCESS);
}

static int copyin_from_kernel(struct copyio_test_data *data) {
...
static void copyin_test(struct copyio_test_data *data) {
  ...
  T_EXPECT_EQ_INT(
      err, 0,
      "copyin() from kernel address in kernel_task thread should succeed");
  cmp = memcmp(data->kern_addr, in_buf, copyio_test_buf_size);
  T_EXPECT_EQ_INT(cmp, 0,
                  "copyin() from kernel address should correctly copy in data");
...
static int copyout_to_kernel(struct copyio_test_data *data) {
  char *out_buf = data->buf1;
  return copyout(out_buf, (uintptr_t)data->kern_addr, copyio_test_buf_size);
}

static void copyout_test(struct copyio_test_data *data) {
  char *out_buf = data->buf1;
  ...
  T_EXPECT_EQ_INT(
      err, 0,
      "copyout() to kernel address in kernel_task thread should succeed");
  cmp = memcmp(out_buf, data->kern_addr, copyio_test_buf_size);
  T_EXPECT_EQ_INT(cmp, 0,
                  "copyout() to kernel address should correctly copy out data");
...
static int copyinstr_from_kernel(struct copyio_test_data *data) {
  char *in_buf = data->buf1;
  size_t *lencopied = data->thread_ptr;
  return copyinstr((user_addr_t)data->kern_addr, in_buf, copyio_test_buf_size,
                   lencopied);
}

static void copyinstr_test(struct copyio_test_data *data) {
  char *in_buf = data->buf1;
  ...
  size_t lencopied;
  int err =
      copyinstr(data->user_addr, in_buf, copyio_test_buf_size, &lencopied);
  T_EXPECT_EQ_INT(err, 0, "copyinstr() with valid parameters should succeed");
  T_EXPECT_EQ_ULONG(
      lencopied, strlen(copyio_test_string) + 1,
      "copyinstr() with a large enough buffer should read entire string");

  int cmp = strncmp(in_buf, copyio_test_string, lencopied);
  T_EXPECT_EQ_INT(
      cmp, 0, "copyinstr() should correctly copy string up to NULL terminator");
  ...
  err =
      copyinstr(data->unmapped_addr, in_buf, copyio_test_buf_size, &lencopied);
  T_EXPECT_EQ_INT(
      err, EFAULT,
  ...
  T_EXPECT_EQ_INT(err, EFAULT,
                  "copyinstr() from kernel address in kernel_task thread "
                  "should return EFAULT");
#else
  T_EXPECT_EQ_INT(
      err, 0,
      "copyinstr() from kernel address in kernel_task thread should succeed");
  T_EXPECT_EQ_ULONG(
      lencopied, strlen(copyio_test_string) + 1,
      "copyinstr() from kernel address should read entire string");
  cmp = strncmp(in_buf, copyio_test_string, lencopied);
  T_EXPECT_EQ_INT(cmp, 0,
                  "copyinstr() from kernel address should correctly copy "
                  "string up to NULL terminator");
  cmp = memcmp(in_buf, copyio_test_string, sizeof(copyio_test_string));
  T_EXPECT_NE_INT(
      cmp, 0,
      "copyinstr() from kernel address should not read past NULL terminator");
#endif
  ...
  char *kern_unterminated_addr =
      (char *)data->kern_addr + copyio_test_buf_size - unterminated_size;
  memset(kern_unterminated_addr, 'A', unterminated_size);

  user_addr_t user_unterminated_addr =
      data->user_addr + copyio_test_buf_size - unterminated_size;
  err = copyinstr(user_unterminated_addr, in_buf, copyio_test_buf_size,
                  &lencopied);
  T_EXPECT_EQ_INT(err, EFAULT,
                  "copyinstr() from userspace region without NULL terminator "
                  "should return EFAULT");
}

static int copyoutstr_to_kernel(struct copyio_test_data *data) {
  size_t *lencopied = data->thread_ptr;
  return copyoutstr(copyio_test_string, (user_addr_t)data->kern_addr,
                    sizeof(copyio_test_string), lencopied);
}

static void copyoutstr_test(struct copyio_test_data *data) {
  bzero(data->kern_addr, sizeof(copyio_test_string));

  size_t lencopied;
  int err = copyoutstr(copyio_test_string, data->user_addr,
                       sizeof(copyio_test_string), &lencopied);
  T_EXPECT_EQ_INT(err, 0, "copyoutstr() with valid parameters should succeed");
  T_EXPECT_EQ_ULONG(lencopied, strlen(copyio_test_string) + 1,
                    "copyoutstr() should copy string up to NULL terminator");

  int cmp =
      strncmp(data->kern_addr, copyio_test_string, sizeof(copyio_test_string));
  ...
  T_EXPECT_NE_INT(cmp, 0,
                  "copyoutstr() should stop copying at NULL terminator");
...
#if defined(__arm64__)
  T_EXPECT_EQ_INT(err, EFAULT,
                  "copyoutstr() to kernel address in kernel_task thread should "
                  "return EFAULT");
#else
  T_EXPECT_EQ_INT(
      err, 0,
      "copyoutstr() to kernel address in kernel_task thread should succeed");
  T_EXPECT_EQ_ULONG(lencopied, strlen(copyio_test_string) + 1,
                    "copyoutstr() to kernel address should copy string up to "
                    "NULL terminator");
  cmp =
      strncmp(data->kern_addr, copyio_test_string, sizeof(copyio_test_string));
  T_EXPECT_EQ_INT(
      cmp, 0, "copyoutstr() to kernel address should correctly copy out data");
#endif
  ...
  copyio_test_protect(data, VM_PROT_READ);
  err = copyoutstr(copyio_test_string, data->user_addr,
                   strlen(copyio_test_string), &lencopied);
  T_EXPECT_EQ_INT(err, EFAULT,
...
static int copyin_atomic32_from_kernel(struct copyio_test_data *data) {
...
static int copyout_atomic32_to_kernel(struct copyio_test_data *data) {
  return copyout_atomic32((uint32_t)data->thread_data,
                          (user_addr_t)data->kern_addr);
}

static int copyout_atomic64_to_kernel(struct copyio_test_data *data) {
...
#define copyin_atomic_test(data, word_t, copyin_fn, copyin_from_kernel_fn)     \
  do {                                                                         \
    ...
    T_EXPECT_EQ_INT(err, 0,                                                    \
                    #copyin_fn "() with valid parameters should succeed");     \
                                                                               \
    ...
    for (unsigned int offset = 1; offset < sizeof(word_t); offset++) {         \
      ...
      T_EXPECT_EQ_INT(err, EINVAL,                                             \
                      #copyin_fn "() from unaligned userspace address should " \
                                 "return EINVAL (offset = %u)",                \
                      offset);                                                 \
    };                                                                         \
    err = copyin_fn(data->unmapped_addr, &word_in);                            \
    T_EXPECT_EQ_INT(                                                           \
        err, EFAULT,                                                           \
        #copyin_fn "() from unmapped userspace address should return EFAULT"); \
                                                                               \
    ...
    T_EXPECT_EQ_INT(                                                           \
        err, EFAULT,                                                           \
        #copyin_fn                                                             \
        "() from kernel address in kernel_task threads should return EFAULT"); \
    err = copyin_from_kernel_fn(data);                                         \
    T_EXPECT_EQ_INT(                                                           \
        err, EFAULT,                                                           \
        #copyin_fn                                                             \
        "() from kernel address in other threads should return EFAULT");       \
                                                                               \
    copyio_test_protect(data, VM_PROT_WRITE);                                  \
    err = copyin_fn(data->user_addr, &word_in);                                \
    T_EXPECT_EQ_INT(err, EFAULT,                                               \
                    #copyin_fn                                                 \
                    "() from write-only address should return EFAULT");        \
    copyio_test_protect(data, VM_PROT_READ | VM_PROT_WRITE);                   \
  } while (0)
...
#define copyout_atomic_test(data, word_t, copyout_fn, copyout_to_kernel_fn)    \
  do {                                                                         \
    ...
    T_EXPECT_EQ_INT(err, 0,                                                    \
                    #copyout_fn "() with valid parameters should succeed");    \
                                                                               \
    ...
    for (unsigned int offset = 1; offset < sizeof(word_t); offset++) {         \
      ...
      T_EXPECT_EQ_INT(err, EINVAL,                                             \
                      #copyout_fn "() to unaligned userspace address should "  \
                                  "return EINVAL (offset = %u)",               \
                      offset);                                                 \
    };                                                                         \
    ...
    T_EXPECT_EQ_INT(err, EFAULT,                                               \
                    #copyout_fn                                                \
                    "() to unmapped userspace address should return EFAULT");  \
    err = copyout_fn(word_out, (uintptr_t)data->kern_addr);                    \
    T_EXPECT_EQ_INT(err, EFAULT,                                               \
                    #copyout_fn "() to kernel address should return EFAULT");  \
                                                                               \
    ...
    T_EXPECT_EQ_INT(                                                           \
        err, EFAULT,                                                           \
        #copyout_fn                                                            \
        "() to kernel address in kernel_task thread should return EFAULT");    \
    err = copyout_to_kernel_fn(data);                                          \
    T_EXPECT_EQ_INT(                                                           \
        err, EFAULT,                                                           \
        #copyout_fn                                                            \
        "() to kernel address in other threads should return EFAULT");         \
                                                                               \
    copyio_test_protect(data, VM_PROT_READ);                                   \
    err = copyout_fn(word_out, data->user_addr);                               \
    T_EXPECT_EQ_INT(err, EFAULT,                                               \
                    #copyout_fn                                                \
                    "() to read-only address should return EFAULT");           \
    copyio_test_protect(data, VM_PROT_READ | VM_PROT_WRITE);                   \
  } while (0)
...
copyin_atomic32_wait_if_equals_from_kernel(struct copyio_test_data *data) {
  return copyin_atomic32_wait_if_equals((uintptr_t)data->kern_addr,
                                        (uint32_t)data->thread_data);
}

static void copyin_atomic32_wait_if_equals_test(struct copyio_test_data *data) {
  bzero(data->kern_addr, sizeof(uint32_t));
  int err = copyin_atomic32_wait_if_equals(data->user_addr, 0);
  T_EXPECT_EQ_INT(
      err, 0, "copyin_atomic32_wait_if_equals() should return 0 when equals");
  ...
  for (unsigned int offset = 1; offset < sizeof(uint32_t); offset++) {
    ...
    T_EXPECT_EQ_INT(err, EINVAL,
                    "copyin_atomic32_wait_if_equals() on unaligned userspace "
                    "address should return EINVAL (offset = %u)",
                    offset);
  }
  err = copyin_atomic32_wait_if_equals(data->unmapped_addr, 0);
  T_EXPECT_EQ_INT(err, EFAULT,
                  "copyin_atomic32_wait_if_equals() on unmapped userspace "
                  "address should return EFAULT");

  data->thread_data = 0;

  err = copyio_test_run_in_thread(copyin_atomic32_wait_if_equals_from_kernel,
                                  data);
  T_EXPECT_EQ_INT(err, EFAULT,
                  "copyin_atomic32_wait_if_equals() from kernel address in "
                  "kernel_task thread should return EFAULT");
  err = copyin_atomic32_wait_if_equals_from_kernel(data);
  T_EXPECT_EQ_INT(err, EFAULT,
                  "copyin_atomic32_wait_if_equals() from kernel address in "
                  "other threads should return EFAULT");

  copyio_test_protect(data, VM_PROT_WRITE);
  err = copyin_atomic32_wait_if_equals(data->user_addr, 0);
  T_EXPECT_EQ_INT(err, EFAULT,
                  "copyin_atomic32_wait_if_equals() on write-only address "
                  "should return EFAULT");
  copyio_test_protect(data, VM_PROT_READ | VM_PROT_WRITE);
}
...
kern_return_t copyio_test(void) {
  ...
  ret = mach_vm_allocate_kernel(data.user_map, &user_addr,
                                copyio_test_buf_size + PAGE_SIZE,
  ...
  ret = mach_vm_remap(kernel_map, &kern_addr, copyio_test_buf_size,
                      ...
                      data.user_map, data.user_addr, false, &cur_protection,
