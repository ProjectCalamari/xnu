 ...
 */
static int rbt_cmp(struct rbt_test_node *a, struct rbt_test_node *b) {
  if (!a && !b) {
    return 0;
  } else if (a && !b) {
    return -1;
...
static size_t rb_tree_insert_nodes(struct _rb_test_tree *tree, size_t count) {
  unsigned int idx = 0;
  ...
  for (idx = 0; idx < count; idx++) {
    ...
    T_EXPECT_EQ_INT(node->rbt_flags & RBT_MASK_IN_USE, 0,
                    "Trying to insert a tree node that is already in use");
...
static size_t rb_tree_remove_nodes(struct _rb_test_tree *tree, size_t count) {
  unsigned int idx = 0;
  if (RBT_TEST_NODE_COUNT < count) {
    count = RBT_TEST_NODE_COUNT;
  }
  ...
  for (idx = 0; idx < count; idx++) {
    rbt_test_node_t node = &test_nodes[idx];
    T_EXPECT_EQ_INT(node->rbt_flags & RBT_MASK_IN_USE, 1,
                    "Trying to remove a tree node that is not in use");
    T_EXPECT_EQ_INT(node->rbt_id, test_node_ids[idx],
                    "The node id does not match the node index");
...
static int rb_tree_test_run(__unused int64_t in, int64_t *out) {
