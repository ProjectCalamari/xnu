 ...
 * security context. The context state is maintained in client and server
 * structures. On the client side, each user of an NFS mount is assigned their
 * own context, identified by UID, on their first use of the mount, and it
 * persists until the unmount or until the context is renewed.  Each user
 * context has a corresponding server context which the server maintains until
 * the client destroys it, or until the context expires.
 *
 ...
 * forwarded via Mach upcall to the gssd, which invokes the GSS-API to
 * authenticate the user to the server (and vice-versa). The client and server
 * also receive a unique session key that can be used to digitally sign the
 * credentials and verifier or optionally to provide data integrity and/or
 * privacy.
 *
...
#define GSS_SVC_CTX_TTL                                                        \
  ((uint64_t)max(2 * GSS_CTX_PEND, nfsrv_gss_context_ttl) * NSEC_PER_SEC)
...
static errno_t rpc_gss_prepend_32(mbuf_t *mb, uint32_t value) {
  int error;
  uint32_t *data;
...
static errno_t rpc_gss_data_create(mbuf_t *mbp_head, uint32_t seqnum) {
  int error;
  mbuf_t mb;
 ...
 * On successful return mb_head will point to the rpc_gss_integ_data_t of length
 * len. Note mb_head will now point to a 4 byte sequence number. len does not
 * include any extra xdr padding. Returns 0 on success, else an errno_t
 */

static errno_t rpc_gss_integ_data_create(gss_ctx_id_t ctx, mbuf_t *mb_head,
                                         uint32_t seqnum, uint32_t *len) {
  uint32_t error;
  uint32_t major;
  uint32_t length;
  gss_buffer_desc mic;
  ...
  /* Append GSS mic token by advancing rpc_gss_data_t length + NFSX_UNSIGNED
...
/*
 * Create an rpc_gss_priv_data_t out of the supplied raw arguments or results in
 * mb_head. On successful return mb_head will point to a wrap token of lenght
 * len. Note len does not include any xdr padding Returns 0 on success, else an
 * errno_t
 */
static errno_t rpc_gss_priv_data_create(gss_ctx_id_t ctx, mbuf_t *mb_head,
                                        uint32_t seqnum, uint32_t *len) {
  uint32_t error;
  uint32_t major;
  struct nfsm_chain nmc;
...
void nfs_gss_svc_init(void) {
  nfs_gss_svc_ctx_hashtbl =
      hashinit(SVC_CTX_HASHSZ, M_TEMP, &nfs_gss_svc_ctx_hash);

  nfs_gss_svc_ctx_timer_call =
      thread_call_allocate(nfs_gss_svc_ctx_timer, NULL);
}
 ...
 */
static struct nfs_gss_svc_ctx *nfs_gss_svc_ctx_find(uint32_t handle) {
 ...
 */
static void nfs_gss_svc_ctx_insert(struct nfs_gss_svc_ctx *cp) {
  struct nfs_gss_svc_ctx_hashhead *head;
...
retry:
  ...
  if (!nfs_gss_timer_on) {
    ...
    nfs_interval_timer_start(
        nfs_gss_svc_ctx_timer_call,
        min(GSS_TIMER_PERIOD, max(GSS_CTX_TTL_MIN, nfsrv_gss_context_ttl)) *
            MSECS_PER_SEC);
  }
 ...
 */
void nfs_gss_svc_ctx_timer(__unused void *param1, __unused void *param2) {
    ...
    LIST_FOREACH_SAFE(cp, &nfs_gss_svc_ctx_hashtbl[i], gss_svc_entries, next) {
      ...
      if (timenow > cp->gss_svc_incarnation +
        ...
        if (cp->gss_svc_seqbits) {
          kfree_data(cp->gss_svc_seqbits,
                     nfs_gss_seqbits_size(cp->gss_svc_seqwin));
        }
 ...
 */
int nfs_gss_svc_cred_get(struct nfsrv_descript *nd, struct nfsm_chain *nmc) {
  ...
  nd->nd_sec = service == RPCSEC_GSS_SVC_NONE        ? RPCAUTH_KRB5
               : service == RPCSEC_GSS_SVC_INTEGRITY ? RPCAUTH_KRB5I
               : service == RPCSEC_GSS_SVC_PRIVACY   ? RPCAUTH_KRB5P
                                                     : 0;
  ...
  if (proc == RPCSEC_GSS_INIT) {
     ...
     */
    cp = kalloc_type(struct nfs_gss_svc_ctx, Z_WAITOK | Z_ZERO | Z_NOFAIL);
  ...
  if (proc == RPCSEC_GSS_DATA || proc == RPCSEC_GSS_DESTROY) {
    ...
    major =
        gss_krb5_verify_mic_mbuf((uint32_t *)&error, cp->gss_svc_ctx_id,
                                 nmc->nmc_mhead, 0, header_len, &cksum, NULL);
    ...
    case RPCSEC_GSS_SVC_PRIVACY:
      ...
      /* Get the wrap token (current mbuf in the chain starting at the current
       * offset) */
      start = nmc->nmc_ptr - mtod(nmc->nmc_mcur, caddr_t);

      /* split out the wrap token */
      argsize = arglen;
      error = gss_normalize_mbuf(nmc->nmc_mcur, start, &argsize, &reply_mbuf,
                                 &pad_mbuf, 0);
      if (error) {
        goto nfsmout;
      }
      ...
      major = gss_krb5_unwrap_mbuf((uint32_t *)&error, cp->gss_svc_ctx_id,
                                   &reply_mbuf, 0, arglen, NULL, NULL);
      if (major != GSS_S_COMPLETE) {
 ...
 */
int nfs_gss_svc_verf_put(struct nfsrv_descript *nd, struct nfsm_chain *nmc) {
  struct nfs_gss_svc_ctx *cp;
  int error = 0;
  ...
  major = gss_krb5_get_mic((uint32_t *)&error, cp->gss_svc_ctx_id, 0, &seqbuf,
                           &cksum);
  if (major != GSS_S_COMPLETE) {
    return error;
  }
 ...
 */
int nfs_gss_svc_prepare_reply(struct nfsrv_descript *nd,
                              struct nfsm_chain *nmc) {
 ...
 */
int nfs_gss_svc_protect_reply(struct nfsrv_descript *nd, mbuf_t mrep __unused) {
  struct nfs_gss_svc_ctx *cp = nd->nd_gss_context;
  ...
  if (nd->nd_sec == RPCAUTH_KRB5I) {
    error = rpc_gss_integ_data_create(cp->gss_svc_ctx_id, &results,
                                      nd->nd_gss_seqnum, &reslen);
  } else {
    /* RPCAUTH_KRB5P */
    error = rpc_gss_priv_data_create(cp->gss_svc_ctx_id, &results,
                                     nd->nd_gss_seqnum, &reslen);
  }
 ...
 */
int nfs_gss_svc_ctx_init(struct nfsrv_descript *nd, struct nfsrv_sock *slp,
                         mbuf_t *mrepp) {
  struct nfs_gss_svc_ctx *cp = NULL;
  int error = 0;
  int autherr = 0;
  ...
  *mrepp = NULL;
  ...
  case RPCSEC_GSS_CONTINUE_INIT:
    ...
    nfsm_chain_get_opaque(error, nmreq, cp->gss_svc_tokenlen,
                          cp->gss_svc_token);
    ...
    cp->gss_svc_seqbits = kalloc_data(nfs_gss_seqbits_size(cp->gss_svc_seqwin),
                                      Z_WAITOK | Z_ZERO);
  ...
  *mrepp = nmrep.nmc_mhead;
  ...
  if (cp->gss_svc_proc == RPCSEC_GSS_INIT ||
    ...
    if (cp->gss_svc_token != NULL) {
      nfsm_chain_add_opaque(error, &nmrep, cp->gss_svc_token,
                            cp->gss_svc_tokenlen);
 ...
 */
static int nfs_gss_svc_gssd_upcall(struct nfs_gss_svc_ctx *cp) {
  kern_return_t kr;
  mach_port_t mp;
  int retry_cnt = 0;
...
retry:
  ...
  kr = mach_gss_accept_sec_context(
      mp, (gssd_byte_buffer)itoken,
      (mach_msg_type_number_t)cp->gss_svc_tokenlen, svcname, 0,
      &cp->gss_svc_context, &cp->gss_svc_cred_handle, &ret_flags,
      &cp->gss_svc_uid, cp->gss_svc_gids, &cp->gss_svc_ngroups, &octx,
      (mach_msg_type_number_t *)&lucidlen, &otoken, &otokenlen,
      &cp->gss_svc_major, &cp->gss_svc_minor);
  ...
  if (kr != KERN_SUCCESS) {
    ...
    if (kr == MIG_SERVER_DIED && cp->gss_svc_context == 0 &&
      ...
      if (cp->gss_svc_tokenlen > 0) {
        nfs_gss_mach_alloc_buffer(cp->gss_svc_token, cp->gss_svc_tokenlen,
                                  &itoken);
      }
      goto retry;
    }
  ...
  if (lucidlen > 0) {
    ...
    error =
        nfs_gss_mach_vmcopyout((vm_map_copy_t)octx, lucidlen, lucid_ctx_buffer);
    if (error) {
      vm_map_copy_discard((vm_map_copy_t)octx);
      vm_map_copy_discard((vm_map_copy_t)otoken);
  ...
  if (otokenlen > 0) {
    ...
    error = nfs_gss_mach_vmcopyout((vm_map_copy_t)otoken, otokenlen,
                                   cp->gss_svc_token);
    if (error) {
      vm_map_copy_discard((vm_map_copy_t)otoken);
 ...
 */
static int nfs_gss_svc_seqnum_valid(struct nfs_gss_svc_ctx *cp, uint32_t seq) {
 ...
 */
void nfs_gss_svc_ctx_deref(struct nfs_gss_svc_ctx *cp) {
  lck_mtx_lock(&cp->gss_svc_mtx);
...
static void host_release_special_port(mach_port_t mp) {
  if (IPC_PORT_VALID(mp)) {
 ...
 * complete.
 */
static void nfs_gss_mach_alloc_buffer(u_char *buf, size_t buflen,
                                      vm_map_copy_t *addr) {
  kern_return_t kr;
  ...
  *addr = NULL;
  ...
  kr = kmem_alloc(ipc_kernel_map, &kmem_buf, tbuflen, KMA_DATA,
                  VM_KERN_MEMORY_FILE);
  if (kr != 0) {
 ...
 */
static int nfs_gss_mach_vmcopyout(vm_map_copy_t in, uint32_t len, u_char *out) {
 ...
 */
static int nfs_gss_mchain_length(mbuf_t mhead) {
  mbuf_t mb;
  int len = 0;
 ...
 */
static int nfs_gss_seqbits_size(uint32_t win) {
 ...
 */
static int nfs_gss_append_chain(struct nfsm_chain *nmc, mbuf_t mc) {
  int error = 0;
  mbuf_t mb, tail;
 ...
 */
static void nfs_gss_nfsm_chain(struct nfsm_chain *nmc, mbuf_t mc) {
  mbuf_t mb, tail;
