...
int corecrypto_available(void) { return g_crypto_funcs ? 1 : 0; }
...
size_t ccpad_cts3_decrypt(const struct ccmode_cbc *cbc, cccbc_ctx *cbc_key,
                          cccbc_iv *iv, size_t nbytes, const void *in,
                          void *out) {
  if (g_crypto_funcs) {
    return (*g_crypto_funcs->ccpad_cts3_decrypt_fn)(cbc, cbc_key, iv, nbytes,
                                                    in, out);
  }
  return 0;
}

size_t ccpad_cts3_encrypt(const struct ccmode_cbc *cbc, cccbc_ctx *cbc_key,
                          cccbc_iv *iv, size_t nbytes, const void *in,
                          void *out) {
  if (g_crypto_funcs) {
    return (*g_crypto_funcs->ccpad_cts3_encrypt_fn)(cbc, cbc_key, iv, nbytes,
                                                    in, out);
  }
  return 0;
}
