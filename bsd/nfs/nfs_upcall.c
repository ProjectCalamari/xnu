...
#define NFS_UC_HASH(x)                                                         \
  ((((uint32_t)(uintptr_t)(x)) >> 3) % nfsrv_uc_thread_count)
 ...
 */
static void nfsrv_uc_thread(void *arg, wait_result_t wr __unused) {
  ...
  while (!nfsrv_uc_shutdown) {
    ...
    while (!nfsrv_uc_shutdown && TAILQ_EMPTY(myqueue->ucq_queue)) {
      ...
      error = msleep(myqueue, &myqueue->ucq_lock, PSOCK, "nfsd_upcall_handler",
                     NULL);
      myqueue->ucq_flags &= ~NFS_UC_QUEUE_SLEEPING;
      if (error) {
 ...
 */
void nfsrv_uc_dequeue(struct nfsrv_sock *slp) {
  ...
  while (ap->nua_flags & NFS_UC_RECEIVE) {
    msleep(&ap->nua_flags, &myqueue->ucq_lock, PSOCK, "nfsrv_uc_dequeue_wait",
           NULL);
  }
...
void nfsrv_uc_init(void) {
  for (int i = 0; i < NFS_UC_HASH_SZ; i++) {
    ...
    lck_mtx_init(&nfsrv_uc_queue_tbl[i].ucq_lock, &nfsrv_uc_group,
                 LCK_ATTR_NULL);
 ...
 */
static void nfsrv_uc_start(void) {
  int32_t i;
  int error;
  ...
  while (nfsrv_uc_shutdown || nfsrv_uc_thread_count > 0) {
    msleep(&nfsrv_uc_thread_count, &nfsrv_uc_shutdown_lock, PSOCK,
  ...
  for (i = 0; i < NFS_UC_HASH_SZ; i++) {
    error =
        kernel_thread_start(nfsrv_uc_thread, (void *)(uintptr_t)i,
                            &nfsrv_uc_queue_tbl[nfsrv_uc_thread_count].ucq_thd);
    if (!error) {
      nfsrv_uc_thread_count++;
    } else {
 ...
 */
static void nfsrv_uc_stop(void) {
  int32_t i;
...
void nfsrv_uc_cleanup(void) {
  ...
  /*
   * Every thing should be dequeued at this point or will be as sockets are
   * closed but to be safe, we'll make sure.
   */
  for (i = 0; i < NFS_UC_HASH_SZ; i++) {
 ...
 */
static void nfsrv_uc_proxy(socket_t so, void *arg, int waitflag) {
 ...
 */
void nfsrv_uc_addsock(struct nfsrv_sock *slp, int start) {
  int on = 1;
  ...
  if (nfsrv_uc_thread_count) {
    arg = kalloc_type(struct nfsrv_uc_arg, Z_WAITOK | Z_ZERO | Z_NOFAIL);
