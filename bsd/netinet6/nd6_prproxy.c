...
static KALLOC_TYPE_DEFINE(ndprl_zone, struct nd6_prproxy_prelist,
                          NET_KT_DEFAULT); /* nd6_prproxy_prelist zone */

static KALLOC_TYPE_DEFINE(solsrc_zone, struct nd6_prproxy_solsrc,
                          NET_KT_DEFAULT); /* nd6_prproxy_solsrc zone */

static KALLOC_TYPE_DEFINE(soltgt_zone, struct nd6_prproxy_soltgt,
                          NET_KT_DEFAULT); /* nd6_prproxy_soltgt zone */

/* The following is protected by ndpr_lock */
RB_GENERATE_PREV(prproxy_sols_tree, nd6_prproxy_soltgt, soltgt_link,
                 soltgt_cmp);
...
SYSCTL_UINT(_net_inet6_icmp6, OID_AUTO, nd6_maxsolstgt,
            CTLFLAG_RW | CTLFLAG_LOCKED, &nd6_max_tgt_sols,
...
SYSCTL_UINT(_net_inet6_icmp6, OID_AUTO, nd6_maxproxiedsol,
            CTLFLAG_RW | CTLFLAG_LOCKED, &nd6_max_src_sols,
...
static struct nd6_prproxy_prelist *nd6_ndprl_alloc(zalloc_flags_t how) {
  return zalloc_flags(ndprl_zone, how | Z_ZERO);
}

static void nd6_ndprl_free(struct nd6_prproxy_prelist *ndprl) {
...
nd6_prproxy_prelist_setroute(boolean_t enable,
  ...
  struct nd6_prproxy_prelist *__single up, *__single down, *__single ndprl_tmp;
  ...
  SLIST_FOREACH_SAFE(down, down_head, ndprl_le, ndprl_tmp) {
    struct nd_prefix *__single pr_up;
    rtentry_ref_t rt;
    boolean_t prproxy, set_allmulti = FALSE;
    int allmulti_sw = FALSE;
    ifnet_ref_t ifp = NULL;
 ...
 */
int nd6_if_prproxy(struct ifnet *ifp, boolean_t enable) {
  SLIST_HEAD(, nd6_prproxy_prelist) up_head;
  SLIST_HEAD(, nd6_prproxy_prelist) down_head;
  struct nd6_prproxy_prelist *__single up, *__single down;
  struct nd_prefix *__single pr;
  ...
  for (pr = nd_prefix.lh_first; pr; pr = pr->ndpr_next) {
    ...
    if (IN6_IS_ADDR_LINKLOCAL(&pr->ndpr_prefix.sin6_addr) ||
        ...
        (enable && (pr->ndpr_stateflags & NDPRF_PRPROXY)) ||
        (pr->ndpr_stateflags & NDPRF_IFSCOPE) || pr->ndpr_ifp != ifp) {
      NDPR_UNLOCK(pr);
      continue;
    }
     ...
     */
    if (enable && (pr->ndpr_stateflags & NDPRF_ONLINK) && nd6_need_cache(ifp)) {
  ...
  SLIST_FOREACH(up, &up_head, ndprl_le) {
    ...
    for (fwd = nd_prefix.lh_first; fwd; fwd = fwd->ndpr_next) {
      ...
      if (!(fwd->ndpr_stateflags & NDPRF_ONLINK) ||
          !(fwd->ndpr_stateflags & NDPRF_IFSCOPE) || fwd->ndpr_plen != pr_len ||
          !in6_are_prefix_equal(&fwd->ndpr_prefix.sin6_addr,
                                fwd->ndpr_prefix.sin6_scope_id, &pr_addr,
                                pr_ifscope, pr_len)) {
        NDPR_UNLOCK(fwd);
        continue;
      }
 ...
 */
boolean_t nd6_prproxy_isours(struct mbuf *m, struct ip6_hdr *ip6,
                             struct route_in6 *ro6, unsigned int ifscope) {
  rtentry_ref_t rt;
  ...
  if (ROUTE_UNUSABLE(ro6)) {
    ...
    VERIFY(in6_are_addr_equal_scoped(&ip6->ip6_dst, &ro6->ro_dst.sin6_addr,
                                     ip6_input_getdstifscope(m),
                                     ro6->ro_dst.sin6_scope_id));
 ...
 */
void nd6_proxy_find_fwdroute(struct ifnet *ifp, struct route_in6 *ro6) {
  struct in6_addr *__single dst6 = &ro6->ro_dst.sin6_addr;
  uint32_t dst_ifscope = ro6->ro_dst.sin6_scope_id;
  ifnet_ref_t fwd_ifp = NULL;
  struct nd_prefix *__single pr;
  rtentry_ref_t rt;
  ...
  if ((rt = ro6->ro_rt) != NULL) {
    ...
    if (!(rt->rt_flags & RTF_PROXY) || rt->rt_ifp == ifp) {
      nd6log2(debug,
              ...
              if_name(ifp), ip6_sprintf(dst6), if_name(rt->rt_ifp));
      RT_UNLOCK(rt);
      /* look it up below */
    } else {
      RT_UNLOCK(rt);
  ...
  for (pr = nd_prefix.lh_first; pr; pr = pr->ndpr_next) {
    ...
    for (fwd = nd_prefix.lh_first; fwd; fwd = fwd->ndpr_next) {
      NDPR_LOCK(fwd);
      if (!(fwd->ndpr_stateflags & NDPRF_ONLINK) || fwd->ndpr_ifp == ifp ||
  ...
  if ((rt = rtalloc1_scoped_locked(SA(&ro6->ro_dst), 0,
    ...
    if (rt->rt_ifp != fwd_ifp || !(rt->rt_flags & RTF_PROXY)) {
      ...
      nd6log2(debug,
              ...
              "for dst %s\n",
              if_name(rt->rt_ifp), ip6_sprintf(dst6));
      RT_UNLOCK(rt);
   ...
   */
  if (fwd_ifp != NULL &&
      (rt = rtalloc1_scoped_locked(SA(&ro6->ro_dst), 1, RTF_PRCLONING,
                                   fwd_ifp->if_index)) != NULL) {
    RT_LOCK(rt);
 ...
 */
void nd6_prproxy_prelist_update(struct nd_prefix *pr_cur,
  ...
  if (pr_up == NULL) {
    NDPR_LOCK(pr_cur);
    bcopy(&pr_cur->ndpr_prefix.sin6_addr, &pr_addr, sizeof(pr_addr));
    ...
    for (pr = nd_prefix.lh_first; pr; pr = pr->ndpr_next) {
      NDPR_LOCK(pr);
      if (!(pr->ndpr_stateflags & NDPRF_ONLINK) ||
          !(pr->ndpr_stateflags & NDPRF_PRPROXY) || pr->ndpr_plen != pr_len ||
          !in6_are_prefix_equal(&pr->ndpr_prefix.sin6_addr,
                                pr->ndpr_prefix.sin6_scope_id, &pr_addr,
                                pr_ifscope, pr_len)) {
        NDPR_UNLOCK(pr);
        continue;
      }
    ...
    NDPR_LOCK(pr_up);
    bcopy(&pr_up->ndpr_prefix.sin6_addr, &pr_addr, sizeof(pr_addr));
  ...
  for (pr = nd_prefix.lh_first; pr; pr = pr->ndpr_next) {
    NDPR_LOCK(pr);
    if (!(pr->ndpr_stateflags & NDPRF_ONLINK) ||
        !(pr->ndpr_stateflags & NDPRF_IFSCOPE) || pr->ndpr_plen != pr_len ||
        !in6_are_prefix_equal(&pr->ndpr_prefix.sin6_addr,
                              pr->ndpr_prefix.sin6_scope_id, &pr_addr,
                              pr_ifscope, pr_len)) {
      NDPR_UNLOCK(pr);
 ...
 */
boolean_t nd6_prproxy_ifaddr(struct in6_ifaddr *ia) {
  struct nd_prefix *__single pr;
  ...
  for (pr = nd_prefix.lh_first; pr; pr = pr->ndpr_next) {
    ...
    if ((pr->ndpr_stateflags & NDPRF_ONLINK) &&
        (pr->ndpr_stateflags & NDPRF_PRPROXY) &&
        in6_are_prefix_equal(&pr->ndpr_prefix.sin6_addr,
                             pr->ndpr_prefix.sin6_scope_id, &addr, pr_scope_id,
                             pr_len)) {
      NDPR_UNLOCK(pr);
      proxied = TRUE;
      break;
 ...
 */
void nd6_prproxy_ns_output(struct ifnet *ifp, struct ifnet *exclifp,
                           struct in6_addr *daddr, struct in6_addr *taddr,
                           struct llinfo_nd6 *ln) {
  SLIST_HEAD(, nd6_prproxy_prelist) ndprl_head;
  struct nd6_prproxy_prelist *__single ndprl, *__single ndprl_tmp;
  struct nd_prefix *__single pr, *__single fwd;
  ifnet_ref_t fwd_ifp;
  struct in6_addr pr_addr;
  u_char pr_len;
  uint32_t pr_scope_id;
  ...
  if (exclifp == NULL) {
    ...
    nd6log2(debug,
            ...
            "(except %s)\n",
            if_name(ifp), ip6_sprintf(taddr), if_name(exclifp));
  }
  ...
  for (pr = nd_prefix.lh_first; pr; pr = pr->ndpr_next) {
    ...
    if (!(pr->ndpr_stateflags & NDPRF_ONLINK) ||
        !(pr->ndpr_stateflags & NDPRF_PRPROXY) ||
        !in6_are_masked_addr_scope_equal(&pr->ndpr_prefix.sin6_addr,
                                         pr_scope_id, taddr, taddr_ifscope,
                                         &pr->ndpr_mask)) {
      NDPR_UNLOCK(pr);
      continue;
    }
    ...
    for (fwd = nd_prefix.lh_first; fwd; fwd = fwd->ndpr_next) {
      NDPR_LOCK(fwd);
      if (!(fwd->ndpr_stateflags & NDPRF_ONLINK) || fwd->ndpr_ifp == ifp ||
          fwd->ndpr_ifp == exclifp || fwd->ndpr_plen != pr_len ||
          !in6_are_prefix_equal(&fwd->ndpr_prefix.sin6_addr,
                                fwd->ndpr_prefix.sin6_scope_id, &pr_addr,
                                pr_scope_id, pr_len)) {
        NDPR_UNLOCK(fwd);
  ...
  SLIST_FOREACH_SAFE(ndprl, &ndprl_head, ndprl_le, ndprl_tmp) {
    ...
    if (pr->ndpr_stateflags & NDPRF_ONLINK) {
      ...
      nd6log2(debug,
              ...
              "on %s\n",
              if_name(fwd_ifp), ip6_sprintf(taddr), if_name(ifp));

      nd6_ns_output(fwd_ifp, daddr, taddr, NULL, NULL, 0);
    } else {
      NDPR_UNLOCK(pr);
    }
    NDPR_REMREF(pr);
  ...
  nd6_ns_output(ifp, daddr, taddr, ln, NULL, 0);
}
 ...
 */
void nd6_prproxy_ns_input(struct ifnet *ifp, struct in6_addr *saddr,
                          char *__sized_by(lladdrlen) lladdr, int lladdrlen,
                          struct in6_addr *daddr, struct in6_addr *taddr,
  ...
  if (solrec) {
    ...
    nd6_cache_lladdr(ifp, saddr, lladdr, lladdrlen, ND_NEIGHBOR_SOLICIT, 0,
                     NULL);
  }
  ...
  SLIST_FOREACH_SAFE(ndprl, &ndprl_head, ndprl_le, ndprl_tmp) {
    ...
    if (pr->ndpr_stateflags & NDPRF_ONLINK) {
      ...
      nd6log2(debug,
              ...
              if_name(fwd_ifp), ndprl->ndprl_sol ? "NUD/AR" : "DAD",
              ip6_sprintf(saddr), ip6_sprintf(daddr), ip6_sprintf(taddr),
              if_name(ifp));

      nd6_ns_output(fwd_ifp, ndprl->ndprl_sol ? taddr : NULL, taddr, NULL,
                    nonce, noncelen);
    } else {
      NDPR_UNLOCK(pr);
    }
 ...
 */
void nd6_prproxy_na_input(struct ifnet *ifp, struct in6_addr *saddr,
                          struct in6_addr *daddr0, struct in6_addr *taddr,
                          int flags) {
  SLIST_HEAD(, nd6_prproxy_prelist) ndprl_head;
  struct nd6_prproxy_prelist *__single ndprl, *__single ndprl_tmp;
  struct nd_prefix *__single pr;
    ...
    if (send_na) {
      if (!ndprl->ndprl_sol) {
        nd6log2(debug,
                ...
                if_name(fwd_ifp), ip6_sprintf(saddr), ip6_sprintf(&daddr),
        ...
        nd6log2(debug,
                ...
                "%s\n",
                if_name(fwd_ifp), ip6_sprintf(saddr), ip6_sprintf(&daddr),
                ip6_sprintf(daddr0), ip6_sprintf(taddr), if_name(ifp));
      }
...
static struct nd6_prproxy_solsrc *nd6_solsrc_alloc(int how) {
  return zalloc_flags(solsrc_zone, how | Z_ZERO);
}

static void nd6_solsrc_free(struct nd6_prproxy_solsrc *ssrc) {
  zfree(solsrc_zone, ssrc);
}

static void nd6_prproxy_sols_purge(struct nd_prefix *pr, u_int64_t max_stgt) {
  struct nd6_prproxy_soltgt *__single soltgt, *__single tmp;
  u_int64_t expire = (max_stgt > 0) ? net_uptime() : 0;

  NDPR_LOCK_ASSERT_HELD(pr);

  /* Either trim all or those that have expired or are idle */
  RB_FOREACH_SAFE(soltgt, prproxy_sols_tree, &pr->ndpr_prproxy_sols, tmp) {
    VERIFY(pr->ndpr_prproxy_sols_cnt > 0);
 ...
 */
void nd6_prproxy_sols_reap(struct nd_prefix *pr) {
 ...
 * Caller is responsible for holding prefix lock.
 */
void nd6_prproxy_sols_prune(struct nd_prefix *pr, u_int32_t max_stgt) {
 ...
 */
static boolean_t nd6_solsrc_enq(struct nd_prefix *pr, struct ifnet *ifp,
                                struct in6_addr *saddr,
                                struct in6_addr *taddr) {
  struct nd6_prproxy_soltgt find;
  struct nd6_prproxy_soltgt *__single soltgt;
  struct nd6_prproxy_solsrc *__single ssrc;
 ...
 */
static boolean_t nd6_solsrc_deq(struct nd_prefix *pr, struct in6_addr *taddr,
...
static struct nd6_prproxy_soltgt *nd6_soltgt_alloc(int how) {
  struct nd6_prproxy_soltgt *__single soltgt;
...
static void nd6_soltgt_free(struct nd6_prproxy_soltgt *soltgt) {
  struct nd6_prproxy_solsrc *__single ssrc, *__single tssrc;
...
static void nd6_soltgt_prune(struct nd6_prproxy_soltgt *soltgt,
                             u_int32_t max_ssrc) {
 ...
 */
static __inline int soltgt_cmp(const struct nd6_prproxy_soltgt *a,
