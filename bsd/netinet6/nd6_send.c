...
#pragma unused(oidp, arg1)
  ...
  buffer =
      (char *)kalloc_data(SYSCTL_CGA_PARAMETERS_BUFFER_SIZE, Z_WAITOK | Z_ZERO);
  if (buffer == NULL) {
  ...
  if (req->oldptr != USER_ADDR_NULL && req->oldlen > 0) {
    ...
    if (iov->iov_len > 0) {
      ...
      if (&oldp[sizeof(cfg.cga_prepare)] <= fin) {
        bcopy(&cfg.cga_prepare, oldp, sizeof(cfg.cga_prepare));
      }
      ...
      if (&oldp[iov->iov_len] < fin) {
        bcopy(
            __unsafe_forge_bidi_indexable(void *, iov->iov_base, iov->iov_len),
            oldp, iov->iov_len);
      }
      oldp += iov->iov_len;
