...
static void in6_cga_node_lock_assert(int owned) {
#if !MACH_ASSERT
#pragma unused(owned)
#endif
...
static boolean_t in6_cga_is_prepare_valid(const struct in6_cga_prepare *prepare,
  ...
  SHA1Init(&ctx);
  SHA1Update(&ctx, &prepare->cga_modifier.octets, IN6_CGA_MODIFIER_LENGTH);
 ...
 * @return void
 */
static void in6_cga_generate_iid(const struct in6_cga_prepare *prepare,
                                 const struct iovec *pubkey,
                                 u_int8_t collisions, struct in6_addr *in6,
                                 struct ifnet *ifp) {
  SHA1_CTX ctx;
  u_int8_t sha1[SHA1_RESULTLEN];
...
int in6_cga_start(const struct in6_cga_nodecfg *cfg) {
  ...
  bcopy(
      __unsafe_forge_bidi_indexable(void *, privkey.iov_base, privkey.iov_len),
      privkeycopy, privkey.iov_len);
  ...
  bcopy(__unsafe_forge_bidi_indexable(void *, pubkey.iov_base, pubkey.iov_len),
        pubkeycopy, pubkey.iov_len);
...
int in6_cga_generate(struct in6_cga_prepare *prepare, u_int8_t collisions,
  ...
  if (prepare == NULL) {
    ...
    prepare->cga_security_level = in6_cga.cga_prepare.cga_security_level;
  }
