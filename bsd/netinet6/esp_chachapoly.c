...
#if (((8 * (ESP_CHACHAPOLY_KEY_LEN + ESP_CHACHAPOLY_SALT_LEN)) !=              \
...
int esp_chachapoly_mature(struct secasvar *sav) {
  ...
  if (sav->key_enc == NULL) {
    esp_log_err("ChaChaPoly key is missing, SPI 0x%08x", ntohl(sav->spi));
    return 1;
  }
  ...
  esp_log_default(
      "ChaChaPoly Mature SPI 0x%08x%s %s dir %u state %u mode %u",
      ntohl(sav->spi), (((sav->flags & SADB_X_EXT_IIV) != 0) ? " IIV" : ""),
...
size_t esp_chachapoly_schedlen(__unused const struct esp_algorithm *algo) {
  return sizeof(esp_chachapoly_ctx_s);
}

int esp_chachapoly_schedule(__unused const struct esp_algorithm *algo,
                            struct secasvar *sav) {
  esp_chachapoly_ctx_t esp_ccp_ctx;
  int rc = 0;

  ESP_CHECK_ARG(sav);
  if (_KEYLEN(sav->key_enc) !=
      ESP_CHACHAPOLY_KEY_LEN + ESP_CHACHAPOLY_SALT_LEN) {
  ...
  if (sav->ivlen !=
      (esp_ccp_ctx->ccp_implicit_iv ? 0 : ESP_CHACHAPOLY_IV_LEN)) {
    esp_log_err("ChaChaPoly Invalid ivlen %u, SPI 0x%08x", sav->ivlen,
                ntohl(sav->spi));
    return EINVAL;
  }
  ...
  if (rc != 0) {
    esp_log_err("ChaChaPoly chacha20poly1305_init failed %d, SPI 0x%08x", rc,
                ntohl(sav->spi));
    return rc;
  }
  ...
  esp_log_default(
      "ChaChaPoly Schedule SPI 0x%08x%s %s dir %u state %u mode %u",
...
int esp_chachapoly_ivlen(const struct esp_algorithm *algo,
                         struct secasvar *sav) {
  ESP_CHECK_ARG(algo);

  if (sav != NULL &&
      ((sav->sched_enc != NULL &&
        ((esp_chachapoly_ctx_t)sav->sched_enc)->ccp_implicit_iv) ||
...
int esp_chachapoly_encrypt_finalize(struct secasvar *sav, unsigned char *tag,
                                    size_t tag_bytes) {
  esp_chachapoly_ctx_t esp_ccp_ctx;
  int rc = 0;
  ...
  if (tag_bytes != ESP_CHACHAPOLY_ICV_LEN) {
    esp_log_err("ChaChaPoly Invalid tag_bytes %zu, SPI 0x%08x", tag_bytes,
                ntohl(sav->spi));
    return EINVAL;
  }
...
int esp_chachapoly_decrypt_finalize(struct secasvar *sav, unsigned char *tag,
                                    size_t tag_bytes) {
  esp_chachapoly_ctx_t esp_ccp_ctx;
  int rc = 0;
  ...
  if (rc != 0) {
    esp_packet_log_err(
        "ChaChaPoly chacha20poly1305_verify failed %d, SPI 0x%08x", rc,
        ntohl(sav->spi));
    return rc;
  }
  return 0;
...
int esp_chachapoly_encrypt(struct mbuf *m, // head of mbuf chain
                           ...
                           __unused size_t plen, struct secasvar *sav,
  ...
  const int32_t ivoff =
      (int32_t)off + (int32_t)sizeof(struct newesp); // IV offset
  ...
  _Static_assert(ESP_CHACHAPOLY_SALT_LEN + ESP_CHACHAPOLY_IV_LEN ==
                     sizeof(nonce),
                 "Bad nonce length");
  esp_chachapoly_ctx_t esp_ccp_ctx;
  ...
  if (ivlen != (esp_ccp_ctx->ccp_implicit_iv ? 0 : ESP_CHACHAPOLY_IV_LEN)) {
    esp_log_err("ChaChaPoly Invalid ivlen %u, SPI 0x%08x", ivlen,
                ntohl(sav->spi));
    m_freem(m);
    return EINVAL;
  }
  if (sav->ivlen != ivlen) {
    esp_log_err("ChaChaPoly Invalid sav->ivlen %u, SPI 0x%08x", sav->ivlen,
                ntohl(sav->spi));
    m_freem(m);
    return EINVAL;
  }
  ...
  if (rc != 0) {
    esp_log_err("ChaChaPoly chacha20poly1305_reset failed %d, SPI 0x%08x", rc,
                ntohl(sav->spi));
    m_freem(m);
    return rc;
  }
  ...
  if (!esp_ccp_ctx->ccp_implicit_iv) {
    ...
    _Static_assert(4 + sizeof(esp_hdr.esp_seq) == ESP_CHACHAPOLY_IV_LEN,
                   "Bad IV length");
    memcpy(((uint8_t *)nonce) + ESP_CHACHAPOLY_SALT_LEN + 4, &esp_hdr.esp_seq,
  ...
  rc = chacha20poly1305_aad(&esp_ccp_ctx->ccp_ctx, sizeof(esp_hdr),
                            (void *)&esp_hdr);
  if (rc != 0) {
    esp_log_err("ChaChaPoly chacha20poly1305_aad failed %d, SPI 0x%08x", rc,
                ntohl(sav->spi));
    m_freem(m);
    return rc;
  }
  ...
  while (s != NULL && soff < m->m_pkthdr.len) {
    ...
    if ((len = (size_t)(s->m_len - sn)) != 0) {
      ...
      rc = chacha20poly1305_encrypt(&esp_ccp_ctx->ccp_ctx, len, sp, sp);
      if (rc != 0) {
        m_freem(m);
  ...
  if (s == NULL && soff != m->m_pkthdr.len) {
    esp_log_err("ChaChaPoly not enough mbufs %d %d, SPI 0x%08x", soff,
                m->m_pkthdr.len, ntohl(sav->spi));
    m_freem(m);
    return EFBIG;
  }
  return 0;
...
int esp_chachapoly_decrypt(struct mbuf *m, // head of mbuf chain
  ...
  if (esp_ccp_ctx->ccp_implicit_iv) {
    ...
    // copy IV from packet
    m_copydata(m, ivoff, ESP_CHACHAPOLY_IV_LEN,
               ((uint8_t *)nonce) + ESP_CHACHAPOLY_SALT_LEN);
  }
  _Static_assert(ESP_CHACHAPOLY_SALT_LEN + ESP_CHACHAPOLY_IV_LEN ==
                     sizeof(nonce),
                 "Bad nonce length");
  ...
  while (s != NULL && soff < m->m_pkthdr.len) {
    ...
    if ((len = (size_t)(s->m_len - sn)) != 0) {
      ...
      rc = chacha20poly1305_decrypt(&esp_ccp_ctx->ccp_ctx, len, sp, sp);
      if (rc != 0) {
        m_freem(m);
        esp_packet_log_err("chacha20poly1305_decrypt failed %d, SPI 0x%08x", rc,
                           ntohl(sav->spi));
        return rc;
      }
    }
  ...
  if (s == NULL && soff != m->m_pkthdr.len) {
    esp_packet_log_err("not enough mbufs %d %d, SPI 0x%08x", soff,
                       m->m_pkthdr.len, ntohl(sav->spi));
    m_freem(m);
    return EFBIG;
  }
...
int esp_chachapoly_encrypt_data(
  ...
  if (!esp_ccp_ctx->ccp_implicit_iv) {
    ...
    if (__improbable(out_ivlen != ESP_CHACHAPOLY_IV_LEN)) {
      cc_clear(sizeof(nonce), nonce);
      esp_log_err("ChaChaPoly Invalid ivlen %zu, SPI 0x%08x", out_ivlen,
                  ntohl(sav->spi));
      return EINVAL;
    }
  ...
  rc = chacha20poly1305_encrypt(&esp_ccp_ctx->ccp_ctx, input_data_len,
                                input_data, output_data);
  if (rc != 0) {
    esp_log_err("ChaChaPoly chacha20poly1305_encrypt failed %d, SPI 0x%08x", rc,
                ntohl(sav->spi));
    return rc;
  }
...
int esp_chachapoly_decrypt_data(struct secasvar *sav,
  ...
  if (esp_ccp_ctx->ccp_implicit_iv) {
    ...
    if (ivlen != ESP_CHACHAPOLY_IV_LEN) {
      esp_log_err("ChaChaPoly Invalid ivlen %zu, SPI 0x%08x", ivlen,
                  ntohl(sav->spi));
      return EINVAL;
    }
    memcpy(((uint8_t *)nonce) + ESP_CHACHAPOLY_SALT_LEN, iv,
  ...
  rc = chacha20poly1305_decrypt(&esp_ccp_ctx->ccp_ctx, input_data_len,
                                input_data, output_data);
  if (rc != 0) {
    esp_log_err("chacha20poly1305_decrypt failed %d, SPI 0x%08x", rc,
                ntohl(sav->spi));
    return rc;
  }
