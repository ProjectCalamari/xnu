...
void nd6_rti_list_wait(const char *func) {
  ...
  while (nd6_rti_list_busy) {
    nd6log2(debug,
            ...
            func);
    (void)msleep(&nd6_rti_list_busy, nd6_mutex, (PZERO - 1), func, NULL);
    LCK_MTX_ASSERT(nd6_mutex, LCK_MTX_ASSERT_OWNED);
  }
...
static struct nd_route_info *nd6_rti_lookup(struct nd_route_info *rti) {
...
void nd6_rtilist_update(struct nd_route_info *new_rti,
...
void nd6_rti_purge(struct nd_route_info *new_rti) {
  VERIFY(new_rti != NULL);
  LCK_MTX_ASSERT(nd6_mutex, LCK_MTX_ASSERT_OWNED);
...
void nd6_rti_delreq(struct nd_route_info *rti) {
  VERIFY(rti != NULL);
  LCK_MTX_ASSERT(nd6_mutex, LCK_MTX_ASSERT_OWNED);
...
void nd6_rti_select(struct nd_route_info *rti, struct ifnet *ifp) {
  VERIFY(rti != NULL);
  VERIFY(ifp != NULL);
  LCK_MTX_ASSERT(nd6_mutex, LCK_MTX_ASSERT_OWNED);
