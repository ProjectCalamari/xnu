...
static kern_return_t kextpgo_trap(void) {
  return DebuggerTrapWithState(DBOP_RESET_PGO_COUNTERS, NULL, NULL, NULL, 0,
                               NULL, FALSE, 0, NULL);
}

static kern_return_t pgo_reset_counters(void) {
  kern_return_t r;
  boolean_t istate;
...
int grab_pgo_data(struct proc *p, struct grab_pgo_data_args *uap,
                  register_t *retval) {
  char *buffer = NULL;
  uint64_t size64 = 0;
  int err = 0;
  ...
  if (uap->flags & ~PGO_ALL_FLAGS || uap->size < 0 ||
  ...
  *retval = 0;
  ...
  if (uap->uuid) {
    ...
    if (uap->buffer == 0 && uap->size == 0) {
      ...
      err = OSKextGrabPgoData(uuid, &size64, NULL, 0, 0,
      ...
      ssize_t size = size64;
      if (((uint64_t)size) != size64 || size < 0) {
        err = ERANGE;
        goto out;
      }
      ...
      *retval = size;
      ...
      err = OSKextGrabPgoData(uuid, &size64, NULL, 0, false,
                              !!(uap->flags & PGO_METADATA));
