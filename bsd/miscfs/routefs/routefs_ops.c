...
static int routefs_mount(struct mount *mp, __unused vnode_t devvp,
                         __unused user_addr_t data, vfs_context_t ctx);
static int routefs_start(__unused struct mount *mp, __unused int flags,
                         __unused vfs_context_t ctx);
static int routefs_unmount(struct mount *mp, int mntflags,
                           __unused vfs_context_t ctx);
static int routefs_root(struct mount *mp, struct vnode **vpp,
                        __unused vfs_context_t ctx);
static int routefs_statfs(struct mount *mp, struct vfsstatfs *sbp,
                          __unused vfs_context_t ctx);
static int routefs_vfs_getattr(__unused mount_t mp, struct vfs_attr *fsap,
                               __unused vfs_context_t ctx);
static int routefs_sync(__unused struct mount *mp, __unused int waitfor,
                        __unused vfs_context_t ctx);
static int routefs_vget(__unused struct mount *mp, __unused ino64_t ino,
                        __unused struct vnode **vpp,
                        __unused vfs_context_t ctx);
static int routefs_fhtovp(__unused struct mount *mp, __unused int fhlen,
                          __unused unsigned char *fhp,
                          __unused struct vnode **vpp,
                          __unused vfs_context_t ctx);
static int routefs_vptofh(__unused struct vnode *vp, __unused int *fhlenp,
                          __unused unsigned char *fhp,
                          __unused vfs_context_t ctx);
static int routefs_sysctl(__unused int *name, __unused u_int namelen,
                          __unused user_addr_t oldp, __unused size_t *oldlenp,
                          __unused user_addr_t newp, __unused size_t newlen,
                          __unused vfs_context_t ctx);
...
static boolean_t _fs_alreadyMounted =
    FALSE; /* atleast a mount of this filesystem is present */

static int routefs_init(__unused struct vfsconf *vfsp) { return 0; }
...
static int routefs_mount(struct mount *mp, __unused vnode_t devvp,
  ...
  routefs_mp_p =
      kalloc_type(struct routefs_mount, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
static int routefs_root(struct mount *mp, struct vnode **vpp,
  ...
  if (vnode_getwithvid(routefs_mp_p->route_rvp, routefs_mp_p->route_vpvid) !=
      0) {
    /* only one in the path., since no vnodes with this, you can hold across
     * this call */
    ROUTEFS_LOCK();
    if (vnode_getwithref(routefs_mp_p->route_rvp) == 0) {
      vnode_rele(routefs_mp_p->route_rvp);
      ...
      error = vnode_lookup(routefs_mp_p->route_path, FREAD | O_DIRECTORY,
                           &routefs_mp_p->route_rvp, ctx);
      if (error == 0) {
...
static int routefs_vfs_getattr(__unused mount_t mp, struct vfs_attr *fsap,
  ...
  if (VFSATTR_IS_ACTIVE(fsap, f_blocks) || VFSATTR_IS_ACTIVE(fsap, f_bused)) {
    fsap->f_blocks =
        (sizeof(struct routefs_mount) + fsap->f_bsize) / fsap->f_bsize;
  ...
  if (VFSATTR_IS_ACTIVE(fsap, f_capabilities)) {
    fsap->f_capabilities.capabilities[VOL_CAPABILITIES_FORMAT] =
        ...
        VOL_CAP_FMT_NO_ROOT_TIMES | VOL_CAP_FMT_CASE_SENSITIVE |
        VOL_CAP_FMT_CASE_PRESERVING | VOL_CAP_FMT_FAST_STATFS |
        VOL_CAP_FMT_2TB_FILESIZE | VOL_CAP_FMT_HIDDEN_FILES;
    ...
    fsap->f_capabilities.valid[VOL_CAPABILITIES_FORMAT] =
        VOL_CAP_FMT_PERSISTENTOBJECTIDS | VOL_CAP_FMT_SYMBOLICLINKS |
        VOL_CAP_FMT_HARDLINKS | VOL_CAP_FMT_JOURNAL |
        VOL_CAP_FMT_JOURNAL_ACTIVE | VOL_CAP_FMT_NO_ROOT_TIMES |
        VOL_CAP_FMT_SPARSE_FILES | VOL_CAP_FMT_ZERO_RUNS |
        VOL_CAP_FMT_CASE_SENSITIVE | VOL_CAP_FMT_CASE_PRESERVING |
        VOL_CAP_FMT_FAST_STATFS | VOL_CAP_FMT_2TB_FILESIZE |
        VOL_CAP_FMT_OPENDENYMODES | VOL_CAP_FMT_HIDDEN_FILES |
        VOL_CAP_FMT_PATH_FROM_ID | VOL_CAP_FMT_NO_VOLUME_SIZES;
    fsap->f_capabilities.valid[VOL_CAPABILITIES_INTERFACES] =
        VOL_CAP_INT_SEARCHFS | VOL_CAP_INT_ATTRLIST | VOL_CAP_INT_NFSEXPORT |
        VOL_CAP_INT_READDIRATTR | VOL_CAP_INT_EXCHANGEDATA |
        VOL_CAP_INT_COPYFILE | VOL_CAP_INT_ALLOCATE | VOL_CAP_INT_VOL_RENAME |
        VOL_CAP_INT_ADVLOCK | VOL_CAP_INT_FLOCK |
        VOL_CAP_INT_EXTENDED_SECURITY | VOL_CAP_INT_USERACCESS |
        VOL_CAP_INT_MANLOCK | VOL_CAP_INT_EXTENDED_ATTR |
  ...
  if (VFSATTR_IS_ACTIVE(fsap, f_attributes)) {
    fsap->f_attributes.validattr.commonattr =
        ...
        ATTR_CMN_OBJTAG | ATTR_CMN_OBJID | ATTR_CMN_PAROBJID |
    ...
    fsap->f_attributes.validattr.volattr =
        ...
        ATTR_VOL_SPACEAVAIL | ATTR_VOL_MINALLOCATION | ATTR_VOL_OBJCOUNT |
        ATTR_VOL_MAXOBJCOUNT | ATTR_VOL_MOUNTPOINT | ATTR_VOL_MOUNTFLAGS |
        ATTR_VOL_MOUNTEDDEVICE | ATTR_VOL_CAPABILITIES | ATTR_VOL_ATTRIBUTES;
    fsap->f_attributes.validattr.dirattr =
        ATTR_DIR_LINKCOUNT | ATTR_DIR_MOUNTSTATUS;
    fsap->f_attributes.validattr.fileattr =
        ATTR_FILE_LINKCOUNT | ATTR_FILE_TOTALSIZE | ATTR_FILE_IOBLOCKSIZE |
    ...
    fsap->f_attributes.nativeattr.volattr =
        ...
        ATTR_VOL_MAXOBJCOUNT | ATTR_VOL_MOUNTPOINT | ATTR_VOL_MOUNTFLAGS |
        ATTR_VOL_MOUNTEDDEVICE | ATTR_VOL_CAPABILITIES | ATTR_VOL_ATTRIBUTES;
    fsap->f_attributes.nativeattr.dirattr = ATTR_DIR_MOUNTSTATUS;
...
int routefs_kernel_mount(char *routepath) {
  ...
  error =
      vnode_lookup(args.route_path, FREAD | O_DIRECTORY, &args.route_rvp, ctx);
  if (error) {
    goto out;
  }
  ...
  error = kernel_mount(fsname, NULLVP, NULLVP, mounthere, &args, 0,
                       MNT_DONTBROWSE, KERNEL_MOUNT_NOAUTH, ctx);
  if (error) {
    goto out;
  }
...
static const struct vnodeopv_entry_desc routefs_vnodeop_entries[] = {
    {.opve_op = &vnop_default_desc,
     .opve_impl = (VOPFUNC)(void *)vn_default_error},
    {.opve_op = &vnop_lookup_desc,
     .opve_impl = (VOPFUNC)routefserr_lookup}, /* lookup */
    ...
    {.opve_op = &vnop_getattr_desc,
     .opve_impl = (VOPFUNC)err_getattr}, /* getattr */
    ...
    {.opve_op = &vnop_reclaim_desc,
     .opve_impl = (VOPFUNC)err_reclaim}, /* reclaim */
    ...
    {.opve_op = &vnop_setlabel_desc,
     .opve_impl = (VOPFUNC)routefserr_setlabel}, /* setlabel */
#endif
    {.opve_op = (struct vnodeop_desc *)NULL,
     .opve_impl = (int (*)(void *))NULL}};

const struct vnodeopv_desc routefs_vnodeop_opv_desc = {
    .opv_desc_vector_p = &routefs_vnodeop_p,
    .opv_desc_ops = routefs_vnodeop_entries};
