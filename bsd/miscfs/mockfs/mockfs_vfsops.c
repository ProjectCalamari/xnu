 ...
 */
int mockfs_unmount(__unused struct mount *mp, __unused int mntflags,
                   __unused vfs_context_t ctx);
int mockfs_root(mount_t mp, vnode_t *vpp, __unused vfs_context_t ctx);
int mockfs_sync(__unused struct mount *mp, __unused int waitfor,
                __unused vfs_context_t ctx);
int mockfs_init(__unused struct vfsconf *vfsc);

/*
 * mockfs_mountroot:
 *   Given a mount (mp) and a vnode for the root device (rvp), builds a fake
 * filesystem for rvp.  This consists of three nodes; a directory node (to serve
 * as a mountpoint for devfs), a file node meant to serve as an executable
 * frontend for rootvp (we will assume that rootvp is an executable, that the
 * kernel can subsequently run), and the root node for the mockfs filesystem.
 * The structure of mockfs is memory-backed; only the contents of the file node
 * refer to the backing device.
 *
...
int mockfs_mountroot(mount_t mp, vnode_t rvp, __unused vfs_context_t ctx) {
  ...
  /*
   * TODO: Validate that the device at least LOOKS like a mach-o (has a sane
   * header); this would prevent us from causing EBADMACHO panics further along
   * the boot path.
   */
  ...
  /*
   * If rvp is a memory device (with a few caveats), we can point to the same
   * physical memory as the device and avoid pointless paging/copying; query the
   * device node for the information we need to determine if we can do this.
   */
  ...
  lck_mtx_init(&mockfs_mount_data->mockfs_mnt_mtx, &mockfs_mtx_grp,
               LCK_ATTR_NULL);

  /*
   * All of the needed nodes/structures have been set up; now we just need to
   * establish the relationships between the various mockfs nodes.
   */
...
/*
 * mockfs_unmount:
 *   Given a mount (mp), and associated flags (mntflags), performs the necessary
 * teardown to destroy the mount.
 *
 * Returns 0 on success, or an error.
 */
int mockfs_unmount(struct mount *mp, int mntflags, __unused vfs_context_t ctx) {
  ...
  /*
   * Reclaim the vnodes for the mount (forcibly, if requested; given that mockfs
   * only support mountroot at the moment, this should ALWAYS be forced),
   */
  ...
  /*
   * Past this point, errors are likely to be unrecoverable, so panic if we're
   * given any excuse; we need to teardown the mockfs_mnt data now, so that VFS
   * can cleanup the mount structure.  Note that clearing mockfs_root before
   * destroying the fsnode tree is related to an implementation detail of
   * mockfs_fsnode_destroy (which will refuse to destroy the root node).
   */
...
/*
 * mockfs_root:
 *   Given a mount (mp), returns the root vnode (*vpp) for that mount with an
 * iocount.
 *
 * Returns 0 on success, or an error.
 */
int mockfs_root(mount_t mp, vnode_t *vpp, __unused vfs_context_t ctx) {
  ...
  rvalue =
      mockfs_fsnode_vnode(((mockfs_mount_t)mp->mnt_data)->mockfs_root, vpp);
  return rvalue;
}
...
int mockfs_init(__unused struct vfsconf *vfsc) { return 0; }
