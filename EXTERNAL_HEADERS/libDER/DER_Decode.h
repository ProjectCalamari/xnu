 ...
 */
DERReturn DERDecodeItem(const DERItem *der,       /* data to decode */
 ...
 *  because the encoded length would be greater than the size of the buffer
 * passed in. Only works with:
 *
 *  -- definite length encoding
 ...
 *
 */
DER_bounds_safety_replaced(DERDecodeItemPartialBufferGetLength) DERReturn
    DERDecodeItemPartialBuffer(const DERItem *der,      /* data to decode */
                               DERDecodedInfo *decoded, /* RETURNED */
                               DERBool allowPartialBuffer);

/*
 * Same as above, but returns a DERDecodedInfo with a length no larger than the
 * buffer. The actual encoded length can be retrieved from encodedLength
 * parameter. encodedLength can be NULL to achieve the same behavior as
 * DERDecodeItemPartialBuffer, with allowPartialBuffer=false
 *
 * NOTE: The DERDecoded length will never be larger than the input buffer.
 * This is a key difference from DERDecodeItemPartialBuffer which could return
 * invalid length.
 *
 */
DERReturn
DERDecodeItemPartialBufferGetLength(const DERItem *der, /* data to decode */
                                    DERDecodedInfo *decoded, /* RETURNED */
...
DERReturn DERParseInteger64(const DERItem *contents,
                            DERLong *value); /* RETURNED */

DERReturn DERParseIntegerSigned(const DERItem *contents,
                                DERSignedInt *value); /* RETURNED */

DERReturn DERParseInteger64Signed(const DERItem *contents,
...
__attribute__((warn_unused_result)) DERReturn
DERDecodeSeqNext(DERSequence *derSeq, DERDecodedInfo *decoded); /* RETURNED */
 ...
 * This function implements the oft-repeated while(DERDecodeSeqNext ==
 * DR_Success) pattern. To stop decoding, set stop to true. If an error (that
 * should cause a stop) occurs, return a non-success status.
 */
#if !defined(_MSC_VER)
DERReturn DERDecodeSequenceWithBlock(
    const DERItem *der,
    DERReturn (^operation)(DERDecodedInfo *content, bool *stop));
#else
DERReturn DERDecodeSequenceWithBlock(
    const DERItem *der,
    DERReturn (*operation)(DERDecodedInfo *content, bool *stop));
 ...
 */
#if !defined(_MSC_VER)
DERReturn DERDecodeSequenceContentWithBlock(
    const DERItem *der,
    DERReturn (^operation)(DERDecodedInfo *content, bool *stop));
#else
DERReturn DERDecodeSequenceContentWithBlock(
...
DERReturn DERParseSequenceContentToObject(
    const DERItem *content, DERShort numItems, /* size of itemSpecs[] */
    const DERItemSpec *DER_counted_by(numItems) itemSpecs,
    void *DER_sized_by(destSize) dest,     /* DERDecodedInfo(s) here RETURNED */
    DERSize destSize, DERSize sizeToZero); /* optional */

/* same as DERParseSequenceContentToObject, but without a destination size */
DER_bounds_safety_replaced(DERParseSequenceContentToObject) DERReturn
