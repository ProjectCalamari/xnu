...
DERReturn DEREncodeLength(DERSize length,
                          DERByte *DER_counted_by(*inOutLen)
                              buf,            /* encoded length goes here */
                          DERSize *inOutLen); /* IN/OUT */

DERReturn DEREncodeLengthSized(DERSize length,
                               DERByte *DER_counted_by(*outLen)
                                   buf, /* encoded length goes here */
                               DERSize inLen, DERSize *outLen);

/* calculate size of encoded length */
DERSize DERLengthOfItem(DERTag tag, DERSize length);

/* encode item */
DERReturn DEREncodeItem(DERTag tag, DERSize length,
                        const DERByte *DER_counted_by(length) src,
                        DERByte *DER_counted_by(*inOutLen)
                            derOut,         /* encoded item goes here */
                        DERSize *inOutLen); /* IN/OUT */

DERReturn DEREncodeItemSized(DERTag tag, DERSize length,
                             const DERByte *DER_counted_by(length) src,
                             DERByte *DER_counted_by(*outLen)
                                 derOut, /* encoded item goes here */
                             DERSize inLen, DERSize *outLen);
...
DERReturn DEREncodeSequenceFromObject(
    ...
    const void *DER_sized_by(srcSize) src, /* generally a ptr to a struct full
                                            * of DERItems */
    DERSize srcSize, DERShort numItems,    /* size of itemSpecs[] */
    const DERItemSpec *DER_counted_by(numItems) itemSpecs,
    DERByte *DER_counted_by(*outLen) derOut, /* encoded data written here */
    DERSize inLen, DERSize *outLen);         /* OUT */

/* same as DEREncodeSequenceFromObject, but without a source size */
DER_bounds_safety_replaced(DEREncodeSequenceFromObject) DERReturn
    DEREncodeSequence(DERTag topTag, /* ASN1_CONSTR_SEQUENCE, ASN1_CONSTR_SET */
                      const void *DER_unsafe_indexable
                          src,           /* generally a ptr to a struct full of
                                          *    DERItems */
                      DERShort numItems, /* size of itemSpecs[] */
                      const DERItemSpec *DER_counted_by(numItems) itemSpecs,
                      DERByte *DER_counted_by(*inOutLen)
                          derOut,         /* encoded data written here */
                      DERSize *inOutLen); /* IN/OUT */

/* precalculate the length of an encoded sequence: can return an error for
 * overflow, and returns its result by argument */
DERReturn DERLengthOfEncodedSequenceFromObject(
    DERTag topTag, /* ASN1_CONSTR_SEQUENCE, ASN1_CONSTR_SET */
    const void *DER_sized_by(srcSize) src, /* generally a ptr to a struct full
                                            * of DERItems */
    DERSize srcSize, DERShort numItems,    /* size of itemSpecs[] */
    const DERItemSpec *DER_counted_by(numItems) itemSpecs, DERSize *result);

/* same, but without a source size, and without a chance to report errors:
 * returns 0 if the size can't possibly be valid for this pointer */
DER_bounds_safety_replaced(DERLengthOfEncodedSequenceFromObject) DERSize
    DERLengthOfEncodedSequence(
        DERTag topTag, /* ASN1_CONSTR_SEQUENCE, ASN1_CONSTR_SET */
        const void *DER_unsafe_indexable src, /* generally a ptr to a struct
                                               * full of DERItems */
        DERShort numItems,                    /* size of itemSpecs[] */
        const DERItemSpec *DER_counted_by(numItems) itemSpecs);
