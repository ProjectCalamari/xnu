...
DERReturn
DEREncodeSHA1DigestInfoSized(const DERByte *DER_counted_by(sha1DigestLen)
                                 sha1Digest,
                             DERSize sha1DigestLen,
                             DERByte *DER_counted_by(*resultLen)
                                 result, /* encoded result RETURNED here */
                             DERSize inputLen, DERSize *resultLen); /* OUT */

DERReturn DEREncodeSHA1DigestInfo(const DERByte *DER_counted_by(sha1DigestLen)
                                      sha1Digest,
                                  DERSize sha1DigestLen,
                                  DERByte *DER_counted_by(*resultLen)
                                      result, /* encoded result RETURNED here */
...
DERReturn
DEREncodeSHA256DigestInfoSized(const DERByte *DER_counted_by(sha256DigestLen)
                                   sha256Digest,
...
DERReturn
DEREncodeSHA256DigestInfo(const DERByte *DER_counted_by(sha256DigestLen)
                              sha256Digest,
                          DERSize sha256DigestLen,
                          DERByte *DER_counted_by(*resultLen)
                              result, /* encoded result RETURNED here */
...
typedef enum { WD_MD2 = 1, WD_MD5 = 2 } WhichDigest;

DERReturn
DEREncodeMDDigestInfoSized(WhichDigest whichDigest,
...
DERReturn DEREncodeMDDigestInfo(WhichDigest whichDigest,
                                const DERByte *DER_counted_by(mdDigestLen)
                                    mdDigest,
                                DERSize mdDigestLen,
                                DERByte *DER_counted_by(*resultLen)
                                    result, /* encoded result RETURNED here */
