...
#define ROUND_TO_STACK(len) (((len) + STACK_INCR - 1) / STACK_INCR * STACK_INCR)
 ...
 */
#define END(name) FUNCTION_EPILOGUE
 ...
 */
#define IMPORT(name) .reference name
 ...
 */
#define P_ABS(name, value) .set name, value
 ...
 */
#define BSS(name, size) .comm name, size

/*
 * P_BSS -- declare private zero'ed storage
 */
#define P_BSS(name, size) .lcomm name, size
...
#elif defined(__x86_64__)
#define PICIFY(var) movq var @GOTPCREL(% rip), % r11
#endif
