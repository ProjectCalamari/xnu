...
/*
     ...
     Contains:   Autoconfiguration of TARGET_ conditionals for Mac OS X and
   iPhone

                 Note:  TargetConditionals.h in 3.4 Universal Interfaces works
                        with all compilers.  This header only recognizes
   compilers known to run on Mac OS X.
 ...
 * for 32-bit mode TARGET_CPU_PPC64        - Compiler is generating PowerPC
 * instructions for 64-bit mode TARGET_CPU_68K          - Compiler is generating
 * 680x0 instructions TARGET_CPU_X86          - Compiler is generating x86
 * instructions for 32-bit mode TARGET_CPU_X86_64       - Compiler is generating
 * x86 instructions for 64-bit mode TARGET_CPU_ARM          - Compiler is
 * generating ARM instructions for 32-bit mode TARGET_CPU_ARM64        -
 * Compiler is generating ARM instructions for 64-bit mode TARGET_CPU_MIPS -
 * Compiler is generating MIPS instructions TARGET_CPU_SPARC        - Compiler
 * is generating Sparc instructions TARGET_CPU_ALPHA        - Compiler is
 * generating Dec Alpha instructions
 */
 ...
 *  run.  Indention is used to show which conditionals are evolutionary
 * subclasses.
 *
 ...
 *    TARGET_OS_UNIX               - Generated code will run on some Unix (not
 * macOS) TARGET_OS_LINUX              - Generated code will run on Linux
 *    TARGET_OS_MAC                - Generated code will run on a variant of
 * macOS TARGET_OS_OSX                - Generated code will run on macOS
 *      TARGET_OS_IPHONE             - Generated code will run on a variant of
 * iOS (firmware, devices, simulator) TARGET_OS_IOS                - Generated
 * code will run on iOS TARGET_OS_MACCATALYST        - Generated code will run
 * on macOS TARGET_OS_TV                 - Generated code will run on tvOS
 *        TARGET_OS_WATCH              - Generated code will run on watchOS
 *        TARGET_OS_VISION             - Generated code will run on visionOS
 *        TARGET_OS_BRIDGE             - Generated code will run on bridge
 * devices TARGET_OS_SIMULATOR          - Generated code will run on an iOS,
 * tvOS, watchOS, or visionOS simulator TARGET_OS_DRIVERKIT          - Generated
 * code will run on macOS, iOS, tvOS, watchOS, or visionOS
 *
 *    TARGET_OS_EMBEDDED           - DEPRECATED: Use TARGET_OS_IPHONE and/or
 * TARGET_OS_SIMULATOR instead TARGET_IPHONE_SIMULATOR      - DEPRECATED: Same
 * as TARGET_OS_SIMULATOR TARGET_OS_NANO               - DEPRECATED: Same as
 * TARGET_OS_WATCH
 *
 *    +--------------------------------------------------------------------------------------+
 *    |                                    TARGET_OS_MAC | | +-----+
 ...
 * integers TARGET_RT_64_BIT        - Generated code uses 64-bit pointers
 *      TARGET_RT_MAC_CFM       - TARGET_OS_MAC is true and CFM68K or PowerPC
 * CFM (TVectors) are used TARGET_RT_MAC_MACHO     - TARGET_OS_MAC is true and
 * Mach-O/dlyd runtime is used
 */
 ...
 *
 * DYNAMIC_TARGETS_ENABLED indicates that the core TARGET_OS macros were enabled
 * via clang preprocessor extensions. If this value is not set, the macro
 * enablements will fall back to the static behavior. It is disabled by default.
 */

#if defined(__is_target_environment) && defined(__has_extension)
#if __is_target_environment(kernelkit) &&                                      \
    !__has_extension(define_target_os_macros)
...
#if (__is_target_arch(x86_64) || __arm64__) && __is_target_vendor(apple) &&    \
    __is_target_os(ios) && __is_target_environment(macabi)
...
// This has to always be defined in the header due to limitations in
// define_target_os_macros
...
/*
    ...
    on the command line to the compiler (e.g. -DTARGET_CPU_MIPS=1
   -DTARGET_OS_UNIX=1)

    The long term solution is to add suppport for __is_target_arch and
   __is_target_os to your compiler.
*/
...
#define TARGET_ABI_USES_IOS_VALUES                                             \
