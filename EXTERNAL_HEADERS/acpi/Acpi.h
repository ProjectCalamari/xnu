...
typedef struct facs_descriptor {
  ...
  UINT32 FirmwareWakingVector; /* 32-bit physical address of the Firmware Waking
                                  Vector */
...
/*
 * Common FACS - This is a version-independent FACS structure used for internal
 * use only
 */
...
#define ACPI_FADT_COMMON                                                       \
  ...
  UINT8 Prefer_PM_Profile; /* Conveys preferred power management profile to    \
                              OSPM. */                                         \
  ...
  UINT8 CstCnt;         /* Support for the _CST object and C States change     \
                           notification.*/                                     \
  ...
  UINT8 DutyWidth;  /* Processor's duty cycle value bit width in P_CNT         \
                       register.*/                                             \
  ...
  UINT16 IapcBootArch; /* IA-PC Boot Architecture Flags. See Table 5-10 for    \
                          description*/                                        \
...
typedef struct fadt_descriptor {
  ...
  UINT8 SleepButton : 1; /* 05:    Sleep button is handled as a generic feature,
                            or not present */
  ...
  UINT8 ResetRegSup
      : 1; /* 10:    System reset via the FADT RESET_REG supported */
  UINT8 SealedCase
      : 1; /* 11:    No internal expansion capabilities and case is sealed */
  UINT8 Headless
      : 1; /* 12:    No local video capabilities or local input devices */
  UINT8 CpuSwSleep : 1; /* 13:    Must execute native instruction after writing
                           SLP_TYPx register */

  UINT8 PciExpWak
      : 1; /* 14:    System supports PCIEXP_WAKE (STS/EN) bits (ACPI 3.0) */
  UINT8 UsePlatformClock
      : 1; /* 15:    OSPM should use platform-provided timer (ACPI 3.0) */
  UINT8 S4RtcStsValid
      : 1; /* 16:    Contents of RTC_STS valid after S4 wake (ACPI 3.0) */
  UINT8 RemotePowerOnCapable
      : 1; /* 17:    System is compatible with remote power on (ACPI 3.0) */
  UINT8 ForceApicClusterModel
      : 1; /* 18:    All local APICs must use cluster model (ACPI 3.0) */
  UINT8 ForceApicPhysicalDestinationMode
      : 1; /* 19:    All local xAPICs must use physical dest mode (ACPI 3.0) */
  ...
  UINT8 ResetValue;     /* Value to write to the ResetRegister port to reset the
                           system */
  UINT8 Reserved4[3];   /* These three bytes must be zero */
  UINT64 XFirmwareCtrl; /* 64-bit physical address of FACS */
  UINT64 XDsdt;         /* 64-bit physical address of DSDT */
  ACPI_GENERIC_ADDRESS
      XPm1aEvtBlk; /* Extended Power Mgt 1a AcpiEvent Reg Blk address */
  ACPI_GENERIC_ADDRESS
      XPm1bEvtBlk; /* Extended Power Mgt 1b AcpiEvent Reg Blk address */
  ACPI_GENERIC_ADDRESS
      XPm1aCntBlk; /* Extended Power Mgt 1a Control Reg Blk address */
  ACPI_GENERIC_ADDRESS
      XPm1bCntBlk; /* Extended Power Mgt 1b Control Reg Blk address */
  ACPI_GENERIC_ADDRESS
      XPm2CntBlk; /* Extended Power Mgt 2 Control Reg Blk address */
  ACPI_GENERIC_ADDRESS
      XPmTmrBlk; /* Extended Power Mgt Timer Ctrl Reg Blk address */
  ACPI_GENERIC_ADDRESS
      XGpe0Blk; /* Extended General Purpose AcpiEvent 0 Reg Blk address */
  ACPI_GENERIC_ADDRESS
 ...
 */
#define ACPI_FLAG_OFFSET(d, f, o)                                              \
  (UINT8)(ACPI_OFFSET(d, f) + sizeof(((d *)0)->f) + o)
/*
