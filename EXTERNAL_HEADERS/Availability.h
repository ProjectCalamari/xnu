...
/*
   ...
   For these macros to function properly, a program must specify the OS version
   range it is targeting.  The min OS version is specified as an option to the
   compiler: -mmacosx-version-min=10.x when building for Mac OS X, and
   -miphoneos-version-min=y.z when building for the iPhone.  The upper bound for
   the OS version is rarely needed, but it can be set on the command line via:
   -D__MAC_OS_X_VERSION_MAX_ALLOWED=10x0 for Mac OS X and
   __IPHONE_OS_VERSION_MAX_ALLOWED = y0z00 for iOS.

   Examples:

       A function available in Mac OS X 10.5 and later, but not on the phone:

           extern void mymacfunc()
   ...
   It is also possible to use the *_VERSION_MIN_REQUIRED in source code to make
   one source base that can be compiled to target a range of OS versions.  It is
   best to not use the _MAC_* and __IPHONE_* macros for comparisons, but rather
   their values. That is because you might get compiled on an old OS that does
   not define a later OS version macro, and in the C preprocessor undefined
   values evaluate to zero in expresssions, which could cause the #if expression
   to evaluate in an unexpected way.
...
/* __MAC_NA is not defined to a value but is uses as a token by macros to
 * indicate that the API is unavailable */
...
/* __IPHONE_NA is not defined to a value but is uses as a token by macros to
 * indicate that the API is unavailable */
...
#define __WATCHOS_DEPRECATED(_start, _dep, _msg)                               \
  __WATCHOS_AVAILABLE(_start)                                                  \
  __OS_AVAILABILITY_MSG(watchos, deprecated = _dep, _msg)
#endif
#endif
 ...
 *    __API_DEPRECATED("No longer supported", macos(10.4, 10.8), ios(2.0, 3.0),
 * watchos(2.0, 3.0), tvos(9.0, 10.0))
 *
 *    __API_DEPRECATED_WITH_REPLACEMENT("-setName:", tvos(10.0, 10.4),
 * ios(9.0, 10.0))
 *    __API_DEPRECATED_WITH_REPLACEMENT("SomeClassName", macos(10.4, 10.6),
 * watchos(2.0, 3.0))
 */
#define __API_DEPRECATED(...)                                                  \
  __API_DEPRECATED_MSG_GET_MACRO(__VA_ARGS__, __API_DEPRECATED_MSG5,           \
                                 __API_DEPRECATED_MSG4, __API_DEPRECATED_MSG3, \
                                 __API_DEPRECATED_MSG2,                        \
                                 __API_DEPRECATED_MSG1)(__VA_ARGS__)
#define __API_DEPRECATED_WITH_REPLACEMENT(...)                                 \
  __API_DEPRECATED_REP_GET_MACRO(__VA_ARGS__, __API_DEPRECATED_REP5,           \
                                 __API_DEPRECATED_REP4, __API_DEPRECATED_REP3, \
                                 __API_DEPRECATED_REP2,                        \
                                 __API_DEPRECATED_REP1)(__VA_ARGS__)
...
#define __API_UNAVAILABLE(...)                                                 \
  __API_UNAVAILABLE_GET_MACRO(__VA_ARGS__, __API_UNAVAILABLE3,                 \
                              __API_UNAVAILABLE2,                              \
                              __API_UNAVAILABLE1)(__VA_ARGS__)
