...
/*
        ...
        Contains:   This file defines structured error handling and assertion
   macros for programming in C. Originally used in QuickDraw GX and later
   enhanced. These macros are used throughout Apple's software.

                                New code may not want to begin adopting these
   macros and instead use existing language functionality.

                                See "Living In an Exceptional World" by Sean
   Parent (develop, The Apple Technical Journal, Issue 11, August/September
   1992) <http://developer.apple.com/dev/techsupport/develop/issue11toc.shtml>
   or <http://www.mactech.com/articles/develop/issue_11/Parent_final.html> for
   the methodology behind these error handling and assertion macros.
 ...
 *         In debug builds, if assertion evaluates to false, calls
 ...
 *         In production builds, if the assertion expression evaluates to false,
 * goto exceptionLabel In debug builds, if the assertion expression evaluates to
 * false, calls DEBUG_ASSERT_MESSAGE and jumps to exceptionLabel
 *
 *      In addition the following suffixes are available:
 *
 *         _noerr     Adds "!= 0" to assertion.  Useful for asserting and
 * OSStatus or OSErr is noErr (zero) _action    Adds statement to be executued
 * if assertion fails _quiet     Suppress call to DEBUG_ASSERT_MESSAGE _string
 * Allows you to add explanitory message to DEBUG_ASSERT_MESSAGE
 *
 *        For instance, require_noerr_string(resultCode, label, msg) will do
 * nothing if resultCode is zero, otherwise it will call DEBUG_ASSERT_MESSAGE
 * with msg and jump to label.
 *
 *  Configuration:
 *
 *      By default all macros generate "production code" (i.e non-debug).  If
 *      DEBUG_ASSERT_PRODUCTION_CODE is defined to zero or DEBUG is defined to
 * non-zero while this header is included, the macros will generated debug code.
 *
 *      If DEBUG_ASSERT_COMPONENT_NAME_STRING is defined, all debug messages
 * will be prefixed with it.
 *
 *      By default, all messages write to stderr.  If you would like to write a
 * custom error message formater, defined DEBUG_ASSERT_MESSAGE to your function
 * name.
 *
 *      Each individual macro will only be defined if it is not already defined,
 * so you can redefine their behavior singly by providing your own definition
 * before this file is included.
 *
 *      If you define __ASSERTMACROS__ before this file is included, then
 * nothing in this file will take effect.
 *
 *      Prior to Mac OS X 10.6 the macro names used in this file conflicted with
 * some user code, including libraries in boost and the proposed C++ standards
 * efforts, and there was no way for a client of this header to resolve this
 * conflict. Because of this, most of the macros have been changed so that they
 * are prefixed with
 *      __ and contain at least one capital letter, which should alleviate the
 * current and future conflicts.  However, to allow current sources to continue
 * to compile, compatibility macros are defined at the end with the old names.
 * A tops script at the end of this file will convert all of the old macro names
 * used in a directory to the new names.  Clients are recommended to migrate
 * over to these new macros as they update their sources because a future
 * release of Mac OS X will remove the old macro definitions ( without the
 * double-underscore prefix ).  Clients who want to compile without the old
 * macro definitions can define the macro
 *      __ASSERT_MACROS_DEFINE_VERSIONS_WITHOUT_UNDERSCORES to 0 before this
 * file is included.
 */
...
/*
 *  To activate the additional assertion code and messages for non-production
 * builds, #define DEBUG_ASSERT_PRODUCTION_CODE to zero before including this
 * file.
 *
 *  If you do not define DEBUG_ASSERT_PRODUCTION_CODE, the default value 1 will
 * be used (production code = no assertion code and no messages).
 */
...
/*
 *  DEBUG_ASSERT_MESSAGE(component, assertion, label, error, file, line,
 * errorCode)
 *
 *  Summary:
 *    All assertion messages are routed through this macro. If you wish to use
 * your own routine to display assertion messages, you can override
 * DEBUG_ASSERT_MESSAGE by #defining DEBUG_ASSERT_MESSAGE before including this
 * file.
 *
 *  Parameters:
 *
 ...
 *
 *  Here is an example of a DEBUG_ASSERT_MESSAGE macro and a routine which
 * displays assertion messsages:
 *
 *      #define DEBUG_ASSERT_COMPONENT_NAME_STRING "MyCoolProgram"
 *
 *      #define DEBUG_ASSERT_MESSAGE(componentNameString, assertionString, \
 *                                   exceptionLabelString, errorString,
 ...
 *
 *      static void
 *      MyProgramDebugAssert(const char *componentNameString, const char
 * *assertionString, const char *exceptionLabelString, const char *errorString,
 *                           const char *fileName, long lineNumber, int
 * errorCode)
 *      {
 *          if ( (assertionString != NULL) && (*assertionString != '\0') )
 *              fprintf(stderr, "Assertion failed: %s: %s\n",
 * componentNameString, assertionString); else fprintf(stderr, "Check failed:
 * %s:\n", componentNameString); if ( exceptionLabelString != NULL )
 *              fprintf(stderr, "    %s\n", exceptionLabelString);
 *          if ( errorString != NULL )
 *              fprintf(stderr, "    %s\n", errorString);
 *          if ( fileName != NULL )
 ...
 *      }
 *
 *  If you do not define DEBUG_ASSERT_MESSAGE, a simple printf to stderr will be
 * used.
 */
#ifndef DEBUG_ASSERT_MESSAGE
#ifdef KERNEL
#include <libkern/libkern.h>
#define DEBUG_ASSERT_MESSAGE(name, assertion, label, message, file, line,      \
                             value)                                            \
...
#define __Debug_String(message)                                                \
  do {                                                                         \
    DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING, "", 0, message,   \
                         __FILE__, __LINE__, 0);                               \
  } while (0)
#endif
#endif
...
#define __Check(assertion)                                                     \
  do {                                                                         \
    if (__builtin_expect(!(assertion), 0)) {                                   \
      DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING, #assertion, 0,  \
...
#define __nCheck_String(assertion, message)                                    \
  __Check_String(!(assertion), message)
#endif
 ...
 *    Production builds: does nothing and produces no code.
 *
 *    Non-production builds: if the errorCode expression does not equal 0
 * (noErr), call DEBUG_ASSERT_MESSAGE.
 *
 *  Parameters:
 *
 *    errorCode:
  ...
  do {                                                                         \
    ...
    if (__builtin_expect(0 != evalOnceErrorCode, 0)) {                         \
      DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING,                 \
                           #errorCode " == 0 ", 0, 0, __FILE__, __LINE__,      \
                           evalOnceErrorCode);                                 \
    }                                                                          \
  } while (0)
#endif
...
#define __Check_noErr_String(errorCode, message)                               \
  do {                                                                         \
    ...
    if (__builtin_expect(0 != evalOnceErrorCode, 0)) {                         \
      DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING,                 \
                           #errorCode " == 0 ", 0, message, __FILE__,          \
                           __LINE__, evalOnceErrorCode);                       \
    }                                                                          \
  } while (0)
#endif
#endif
...
#define __nVerify_String(assertion, message)                                   \
  __Verify_String(!(assertion), message)
#endif
 ...
 *
 *    Non-production builds: if the errorCode expression does not equal 0
 * (noErr), call DEBUG_ASSERT_MESSAGE and then execute the action statement or
 ...
 *
 *    Non-production builds: if the assertion expression evaluates to false,
 *    call DEBUG_ASSERT_MESSAGE and then execute the action statement or
 * compound statement (block).
 *
 *  Parameters:
 *
...
#define __Verify_Action(assertion, action)                                     \
  if (__builtin_expect(!(assertion), 0)) {                                     \
    DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING, #assertion, 0, 0, \
                         __FILE__, __LINE__, 0);                               \
    action;                                                                    \
  } else                                                                       \
    do {                                                                       \
...
#define __Require(assertion, exceptionLabel)                                   \
  do {                                                                         \
    if (__builtin_expect(!(assertion), 0)) {                                   \
      DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING, #assertion,     \
                           #exceptionLabel, 0, __FILE__, __LINE__, 0);         \
      goto exceptionLabel;                                                     \
    }                                                                          \
  } while (0)
#endif
...
#ifndef __nRequire
#define __nRequire(assertion, exceptionLabel)                                  \
  __Require(!(assertion), exceptionLabel)
#endif
...
#define __nRequire_Quiet(assertion, exceptionLabel)                            \
  __Require_Quiet(!(assertion), exceptionLabel)
#endif
...
#define __Require_String(assertion, exceptionLabel, message)                   \
  do {                                                                         \
    if (__builtin_expect(!(assertion), 0)) {                                   \
      DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING, #assertion,     \
                           #exceptionLabel, message, __FILE__, __LINE__, 0);   \
      goto exceptionLabel;                                                     \
    }                                                                          \
  } while (0)
#endif
 ...
 *
 *    Non-production builds: if the errorCode expression does not equal 0
 * (noErr), call DEBUG_ASSERT_MESSAGE and then goto exceptionLabel.
 *
 *  Parameters:
 *
 *    errorCode:
...
#define __Require_noErr(errorCode, exceptionLabel)                             \
  do {                                                                         \
    ...
    if (__builtin_expect(0 != evalOnceErrorCode, 0)) {                         \
      DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING,                 \
                           #errorCode " == 0 ", #exceptionLabel, 0, __FILE__,  \
                           __LINE__, evalOnceErrorCode);                       \
      goto exceptionLabel;                                                     \
    }                                                                          \
  } while (0)
 ...
 *
 *    Non-production builds: if the errorCode expression does not equal 0
 * (noErr), call DEBUG_ASSERT_MESSAGE, execute the action statement or compound
 * statement (block), and then goto exceptionLabel.
 *
 *  Parameters:
 *
...
#define __Require_noErr_String(errorCode, exceptionLabel, message)             \
  do {                                                                         \
    ...
    if (__builtin_expect(0 != evalOnceErrorCode, 0)) {                         \
      DEBUG_ASSERT_MESSAGE(DEBUG_ASSERT_COMPONENT_NAME_STRING,                 \
                           #errorCode " == 0 ", #exceptionLabel, message,      \
                           __FILE__, __LINE__, evalOnceErrorCode);             \
      goto exceptionLabel;                                                     \
    }                                                                          \
  } while (0)
#endif
...
#if DEBUG_ASSERT_PRODUCTION_CODE
#define __Require_noErr_Action_String(errorCode, exceptionLabel, action,       \
                                      message)                                 \
  do {                                                                         \
    if (__builtin_expect(0 != (errorCode), 0)) {                               \
      {                                                                        \
 ...
 *
 *  Discussion:
 *     This declares an array with a size that is determined by a compile-time
 * expression. If false, it declares a negatively sized array, which generates a
 * compile-time error.
 *
 * Examples:
 ...
 *  Note: This only works in places where extern declarations are allowed (e.g.
 * global scope).
 */
 ...
 *
 *	A future release of Mac OS X will no longer do this, and will require
 that clients move to the
 *  new macros as defined above.  However, in the interim both the new and old
 macros will work, unless
 *  clients define a macro __ASSERT_MACROS_DEFINE_VERSIONS_WITHOUT_UNDERSCORES
 before this file is included
 *  in their compilations.  Clients who do not want the older macros defined can
 accomplish this by adding
 *    #define __ASSERT_MACROS_DEFINE_VERSIONS_WITHOUT_UNDERSCORES 0
 *  at the top of their sources, or my adding
 -D__ASSERT_MACROS_DEFINE_VERSIONS_WITHOUT_UNDERSCORES=0 to the
 *  gcc compilation options.
 *
 *  To aid users of these macros in converting their sources, the following tops
 script will convert usages
 *  of the old macros into the new equivalents.  To do so, in Terminal go into
...
/* If we haven't set this yet, it defaults to on.  In the next release, this
 * will default to off. */
...
#define require_noerr_action_string(errorCode, exceptionLabel, action,         \
                                    message)                                   \
  __Require_noErr_Action_String(errorCode, exceptionLabel, action, message)
#endif
...
#define verify_noerr_action(errorCode, action)                                 \
  __Verify_noErr_Action(errorCode, action)
#endif
