 ...
 */
OS_ENUM(CEVersion, int64_t, kCEVersionInvalid = 0, kCEVersionZero = 1,
        kCEVersionOne = 2);
...
/*!
 * @function CEValidate
 * Validates if the provided blob conforms to one of the entitlement
 * specification understood by CoreEntitlements
 * @param rt
 * Active runtime
 * @param result
 ...
 */
CEError_t CEValidate(const CERuntime_t rt, CEValidationResult *result,
                     const uint8_t *__ended_by(blob_end) blob,
                     const uint8_t *blob_end) __result_use_check;
/*!
 ...
 * This function will return kCENoError if the entitlements are valid
 */
CEError_t CEValidateWithOptions(const CERuntime_t rt,
                                CEValidationOptions *options,
 ...
 * Creates and returns a managed query context for the validated blob against
 * which you can perform queries
 * @param rt
 ...
 * @note
 * The returned managed context must be subsequently released with
 * CEAcquireManagedContext
 */
CEError_t CEAcquireManagedContext(const CERuntime_t rt,
                                  CEValidationResult validationResult,
                                  CEQueryContext_t *ctx) __result_use_check;
 ...
 */
OS_ENUM(CEQueryOpOpcode, int64_t, kCEOpNoop = 0, kCEOpSelectKey = 1,
        kCEOpSelectIndex = 2, kCEOpMatchString = 3, kCEOpMatchStringPrefix = 4,
        kCEOpMatchBool = 5, kCEOpStringValueAllowed = 6, kCEOpMatchInteger = 7,
        kCEOpStringPrefixValueAllowed = 8, kCEOpSelectKeyWithPrefix = 9,
        kCEOpIntegerValueAllowed = 10, kCEOpMatchType = 11, kCEOpMatchData = 12,
 ...
 * The opcode specified _which_ operation to perform, while the parameters
 * specify how to perform it. Operations are passed by value and may be safely
 * reused.
 */
...
typedef struct CEPrepareOptions {
  ...
  bool materialize;
  /*
   Controls if CEPrepareQuery should fail on keys in dynamic operations that are
   too long
   */
 ...
 *
 * @returns
 * This function will return kCENoError if the query is satisfiable, otherwise
 * kCEQueryCannotBeSatisfied.
 *
 * @note
 * As stated previously, the query only succeeds if it is satisfiable by the
 * context, meaning that the operations executed in the passed in order leave
 * the VM in the valid state. An invalid state may arise from a variety of
 * situations, like trying to select a value for a key that doesn't exist, or a
 * failing string matching operations.
 */
CEError_t CEContextQuery(CEQueryContext_t ctx,
                         const CEQueryOperation_t *__counted_by(queryLength)
                             query,
                         size_t queryLength) __result_use_check;

/*!
 * @function CEPrepareQuery
 * Prepares the query for execution by materializing dynamic operations if
 * needed
 *
 * @params options
 ...
 * The number of operations in the query
 */
CEError_t CEPrepareQuery(CEPrepareOptions_t options,
                         CEQueryOperation_t *__counted_by(queryLength) query,
                         size_t queryLength);

/*!
 * @function CEContextIsSubset
 * Checks if the subset <-> superset relation holds between two context.
 * The logic relations used to establish that relation correspond to the normal
 ...
 *
 * @returns
 * This function will return kCENoError if the relation holds, otherwise
 * kCEQueryCannotBeSatisfied.
 */
