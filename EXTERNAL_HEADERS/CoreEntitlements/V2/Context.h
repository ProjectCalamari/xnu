...
typedef struct _CEElementIndex {
  /*
   * Offset of the key and value from the beginning of the index. Spec
   * validation doesn't really care if the offset is larger than UINT32_MAX, but
   * we enforce that for acceleration, a context becomes ineligible if it is
   * very large.
   */
  uint32_t keyOffset;
...
/*
 * We need to be very conservative with how much memory we consume for
 * acceleration of a context. Many other components are tied to the idea of each
 * indexed element only consuming 8 bytes (such as PPL/AMFI/TXM).
 */
_Static_assert(sizeof(CEElementIndex_t) == 8, "sizeof(CEElementIndex_t) != 8");

/*
 * Each context type maps to a set of constraints, which are validated when a
 * context is initialized. Each context can be limited in the set of types it
 * supports along with some other bespoke rules.
 */
typedef struct _CEContextConstraints {
   ...
   * we let array values contain elements of different types. Otherwise, the
   * type of the first element in the array is considered the de-facto type for
   ...
   *
   * 1. Version 0 encoding is not expected to have sorted keys, whereas version
   * 1 and above are.
   */
