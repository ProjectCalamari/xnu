...
/**
 * The environment defined `CEEnvironmentStackCheck` function is called before
 * the library begins iterating into a nested type such as an array or a
 * dictionary. This provides the environment an opportunity to catch stack
 * overflows, and instead return safely.
 *
 * A return of `kCEReturnSuccess` means the library is safe to proceed with the
 * iteration, and a return of any other value halts the iteration and returns an
 * error.
 */
extern CEReturn_t CEEnvironmentStackCheck(void);

#else

static inline
    __attribute__((always_inline)) CEReturn_t CEEnvironmentStackCheck(void) {
