...
_Static_assert(sizeof(TCReturn_t) == sizeof(uint32_t),
               "TCReturn_t is not 32 bits large");

static inline TCReturn_t buildTCRet(uint8_t component, uint8_t error,
                                    uint16_t uniqueError) {
  TCReturn_t ret = {
      .component = component, .error = error, .uniqueError = uniqueError};
