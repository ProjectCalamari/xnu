...
#define cchmac_ctx_size(STATE_SIZE, BLOCK_SIZE)                                \
  (cc_pad_align(ccdigest_ctx_size(STATE_SIZE, BLOCK_SIZE)) + (STATE_SIZE))
#define cchmac_di_size(_di_)                                                   \
  (cchmac_ctx_size((_di_)->state_size, (_di_)->block_size))

#define cchmac_ctx_n(STATE_SIZE, BLOCK_SIZE)                                   \
  ccn_nof_size(cchmac_ctx_size((STATE_SIZE), (BLOCK_SIZE)))

#define cchmac_ctx_decl(STATE_SIZE, BLOCK_SIZE, _name_)                        \
  cc_ctx_decl_vla(struct cchmac_ctx, cchmac_ctx_size(STATE_SIZE, BLOCK_SIZE),  \
                  _name_)
#define cchmac_ctx_clear(STATE_SIZE, BLOCK_SIZE, _name_)                       \
  cc_clear(cchmac_ctx_size(STATE_SIZE, BLOCK_SIZE), _name_)
#define cchmac_di_decl(_di_, _name_)                                           \
  cchmac_ctx_decl((_di_)->state_size, (_di_)->block_size, _name_)
#define cchmac_di_clear(_di_, _name_)                                          \
  cchmac_ctx_clear((_di_)->state_size, (_di_)->block_size, _name_)
...
/* Accesors for ostate fields, this is all cchmac_ctx_t adds to the
 * ccdigest_ctx_t. */
#define cchmac_ostate(_di_, HC)                                                \
  ((ccdigest_state_t)(((cchmac_ctx_t)(HC))->b +                                \
                      cc_pad_align(ccdigest_di_size(_di_))))
...
void cchmac(const struct ccdigest_info *di, size_t key_len, const void *key,
