...
/* A word about configuration macros:
    ...
    Conditional configuration macros specific to corecrypto should be named
   CORECRYPTO_xxx or CCxx_yyy and be defined to be either 0 or 1 in this file.
   You can add an #ifndef #error construct at the end of this file to make sure
   it's always defined.

    They should always be tested using the #if directive, never the #ifdef
   directive.

    No other conditional macros shall ever be used (except in this file)

    Configuration Macros that are defined outside of corecrypto (eg: KERNEL,
   DEBUG, ...) shall only be used in this file to define CCxxx macros.

    External macros should be assumed to be either undefined, defined with no
   value, or defined as true or false. We shall strive to build with -Wundef
   whenever possible, so the following construct should be used to test external
   macros in this file:
    ...
    It is acceptable to define a conditional CC_xxxx macro in an implementation
   file, to be used only in this file.

    The current code is not guaranteed to follow those rules, but should be
   fixed to.
...
#define __cc_printflike(fmtarg, firstvararg)                                   \
  __attribute__((format(printf, fmtarg, firstvararg)))
#endif

// Do not set this macros to 1, unless you are developing/testing for Linux
// under macOS
...
// This macro can be used to enable prints when a condition in the macro
// "cc_require" is false. This is especially useful to confirm that negative
// testing fails at the intended location
...
// ARMv6-M
                         ...
                         defined(__TARGET_ARCH_6S_M) || defined(__armv6m__))
...
#if (CC_USE_L4 || CC_RTKIT || CC_RTKITROM || CC_USE_SEPROM || CC_EFI ||        \
     CC_LINUX || defined(_WIN32) || CC_BASEBAND || CC_SGX || CC_ARM_ARCH_6M)
#define CC_DARWIN 0
#elif CC_TXM
#define CC_DARWIN 0
#elif CC_SPTM
...
#if !defined(__APPLE__) || CC_RTKIT || CC_RTKITROM || CC_USE_SEPROM ||         \
    defined(__CC_ARM) || defined(__hexagon__) || CC_EFI
...
// Use this macro to strictly disable assembly regardless of
// cpu/os/compiler/etc. Our assembly code is not gcc compatible. Clang defines
// the __GNUC__ macro as well.
#if !defined(CC_USE_ASM)
#if defined(_WIN32) || CC_EFI || CC_BASEBAND || CC_XNU_KERNEL_PRIVATE ||       \
    (defined(__GNUC__) && !defined(__clang__)) || CC_LINUX
#define CC_USE_ASM 0
#else
#define CC_USE_ASM 1
#endif
...
#ifndef CC_LOG
#define CC_LOG                                                                 \
  (CC_DARWIN && !CC_KERNEL && !CC_IBOOT && !CC_DRIVERKIT && !CC_EXCLAVE)
#endif
...
// Only apply the `CC_CONST` attribute to functions with no side-effects where
// the output is a strict function of pass by value input vars with no exterior
// side-effects. Specifically, do not apply CC_CONST if the function has any
// arguments that are pointers (directly, or indirectly)
...
#define cc_unsafe_forge_bidi_indexable(P, S)                                   \
  __builtin_unsafe_forge_bidi_indexable(P, S)
...
#if defined(ENABLE_CRYPTOKIT_PRIVATE_DEFINITIONS) &&                           \
    ENABLE_CRYPTOKIT_PRIVATE_DEFINITIONS
...
#define CC_WORKSPACE_OVERRIDE(f, o)                                            \
  CC_WORKSPACE_OVERRIDE_PRAGMA(workspace - override f o)
#else
#define CC_WORKSPACE_OVERRIDE(f, o)
#endif
