...
#define __CC_BRIDGEOS_DEPRECATED(_dep, _msg)                                   \
  __attribute__((availability(bridgeos, deprecated = _dep, replacement = _msg)))
#endif
#endif
...
#define cc_deprecate_with_replacement(replacement_message, ios_version,        \
                                      macos_version, tvos_version,             \
                                      watchos_version, bridgeos_version)       \
...
/* Manage asserts here because a few functions in header public files do use
 * asserts */
#if CORECRYPTO_DEBUG
...
#define cc_pad_align(_size_)                                                   \
  ((_size_ + CC_MAX_ALIGNMENT - 1) & (~(CC_MAX_ALIGNMENT - 1)))

/* number of array elements used in a cc_ctx_decl */
#define cc_ctx_n(_type_, _size_)                                               \
  ((_size_ + sizeof(_type_) - 1) / sizeof(_type_))

// VLA warning opt-outs to help transition away from VLAs.
#if defined(__KEIL__)
#define CC_IGNORE_VLA_WARNINGS #pragma push #pragma diag_suppress 1057

#define CC_RESTORE_VLA_WARNINGS #pragma pop
#else
#define CC_IGNORE_VLA_WARNINGS                                                 \
  _Pragma("GCC diagnostic push") _Pragma("GCC diagnostic ignored \"-Wvla\"")

#define CC_RESTORE_VLA_WARNINGS _Pragma("GCC diagnostic pop")
#endif

/* sizeof of a context declared with cc_ctx_decl */
#define cc_ctx_sizeof(_type_, _size_)                                          \
  CC_IGNORE_VLA_WARNINGS                                                       \
  sizeof(_type_[cc_ctx_n(_type_, _size_)]) CC_RESTORE_VLA_WARNINGS

/*
  1. _alloca cannot be removed because this header file is compiled with both
  MSVC++ and with clang.
  2. The _MSC_VER version of cc_ctx_decl() is not compatible with the way *_decl
  macros as used in CommonCrypto, AppleKeyStore and SecurityFrameworks. To
  observe the incompatibilities and errors, use below definition. Corecrypto
  itself, accepts both definitions #define cc_ctx_decl(_type_, _size_, _name_)
  _type_ _name_ ## _array[cc_ctx_n(_type_, (_size_))]; _type_ *_name_ = _name_
  ## _array
  3. Never use sizeof() operator for the variables declared with cc_ctx_decl(),
  because it is not be compatible with the _MSC_VER version of cc_ctx_decl().
 */
#if defined(_MSC_VER)

#include <malloc.h>
#define cc_ctx_decl(_type_, _size_, _name_)                                    \
  _type_ *_name_ = (_type_ *)_alloca(sizeof(_type_) * cc_ctx_n(_type_, _size_))
...
/*!
 ...
 @discussion It is used to clear sensitive data, particularly when the are
...
CC_INLINE CC_NONNULL((2)) void cc_xor(size_t size, void *cc_sized_by(size) r,
                                      const void *cc_sized_by(size) s,
                                      const void *cc_sized_by(size) t) {
...
/*!
 @brief cc_cmp_safe(num, pt1, pt2) compares two array ptr1 and ptr2 of num
 bytes.
 @discussion The execution time/cycles is independent of the data and therefore
 guarantees no leak about the data. However, the execution time depends on num.
 @param num  number of bytes in each array
 @param ptr1 input array
 @param ptr2 input array
 @return  returns 0 if the num bytes starting at ptr1 are identical to the num
 bytes starting at ptr2 and 1 if they are different or if num is 0 (empty
 arrays).
 */
CC_NONNULL((2, 3))
int cc_cmp_safe(size_t num, const void *cc_sized_by(num) ptr1,
                const void *cc_sized_by(num) ptr2);
...
/*
 When building with "-nostdinc" (i.e. iboot), ptrauth.h is in a non-standard
 location. This requires a new flag to be used when building iboot: -ibuiltininc
...
#define CC_SPTR(_sn_, _n_)                                                     \
  __ptrauth(ptrauth_key_process_independent_code, 1,                           \
            ptrauth_string_discriminator("cc_" #_sn_ #_n_)) _n_
#else
