...
typedef int (*ccentropy_get_seed_fn_t)(ccentropy_ctx_t *ctx, size_t seed_nbytes,
                                       void *seed);
...
/*!
  ...
  abort) otherwise.
*/
int ccentropy_get_seed(ccentropy_ctx_t *ctx, size_t seed_nbytes, void *seed);
...
/*!
  ...
  all implementations.
*/
int ccentropy_add_entropy(ccentropy_ctx_t *ctx, uint32_t entropy_nsamples,
...
*/
int ccentropy_rng_init(ccentropy_rng_ctx_t *ctx, struct ccrng_state *rng_ctx,
                       size_t seed_max_nbytes);
...
typedef struct ccentropy_digest_ctx {
  ...
  ccdigest_ctx_decl(MAX_DIGEST_STATE_SIZE, MAX_DIGEST_BLOCK_SIZE, digest_ctx);
...
/*!
  ...
  @param entropy_mbits_per_sample An estimate of per-sample entropy measured in
  millibits.
