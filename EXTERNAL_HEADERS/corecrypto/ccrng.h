...
#define CCRNG_STATE_COMMON                                                     \
  int (*CC_SPTR(ccrng_state, generate))(struct ccrng_state * rng,              \
                                        size_t outlen, void *out);

/*!
 @type      struct ccrng_state
 @abstract  Default state structure. Do not instantiate. ccrng() returns a
...
/*!
 ...
 @result A handle to a secure RNG, or null if one cannot be initialized
 successfully
...
/*!
 ...
 @abstract   Generate `outlen` bytes of output, stored in `out`, using
 ccrng_state `rng`.

 @param rng  `struct ccrng_state` representing the state of the RNG.
 @param outlen  Amount of random bytes to generate.
 @param out  Pointer to memory where random bytes are stored, of size at least
 `outlen`.
