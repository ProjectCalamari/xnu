...
/*
 * The maximum length of the entropy_input,  additional_input
 * (max_additional_input_length) , personalization string
 * (max_personalization_string_length) and max_number_of_bits_per_request  are
 * implementation dependent but shall fit in a 32 bit register and be be less
 * than or equal to the specified maximum length for the selected DRBG mechanism
 * (NIST 800-90A Section 10).
 */
...
#define CCDRBG_MAX_REQUEST_SIZE                                                \
  ((uint32_t)1 << 16) // this is the absolute maximum in NIST 800-90A
#define CCDRBG_RESEED_INTERVAL                                                 \
  ((uint64_t)1 << 48) // must be able to fit the NIST maximum of 2^48

/*
 * The entropyLength is forced to be greater or equal than the security
 * strength. Nonce is not forced. It either needs to have 0.5*security strength
 * entropy. Or, a vale that is repeated less than a 0.5*security strength bit
 * random string. see below or NIST  800-90A for the definition of security
 * strength
 */

int ccdrbg_init(const struct ccdrbg_info *info, struct ccdrbg_state *drbg,
                size_t entropyLength, const void *entropy, size_t nonceLength,
                const void *nonce, size_t psLength, const void *ps);

/*
 *  The entropyLength is forced to be greater or equal than the security
 * strength.
 */
int ccdrbg_reseed(const struct ccdrbg_info *info, struct ccdrbg_state *drbg,
                  size_t entropyLength, const void *entropy,
                  size_t additionalLength, const void *additional);

int ccdrbg_generate(const struct ccdrbg_info *info, struct ccdrbg_state *drbg,
 ...
 * the maximum security strengh of drbg equals to the block size of the
 * corresponding ECB.
 */
...
void ccdrbg_factory_nistctr(struct ccdrbg_info *info,
                            const struct ccdrbg_nistctr_custom *custom);

/*
 * NIST SP 800-90 HMAC_DRBG
 * the maximum security strengh of drbg is half of output size of the input hash
 * function and it internally is limited to 256 bits
 */
...
void ccdrbg_factory_nisthmac(struct ccdrbg_info *info,
