...
struct ccmode_ecb {
  ...
  size_t block_size;
  int (*CC_SPTR(ccmode_ecb, init))(const struct ccmode_ecb *ecb, ccecb_ctx *ctx,
                                   size_t key_nbytes, const void *key);
  int (*CC_SPTR(ccmode_ecb, ecb))(const ccecb_ctx *ctx, size_t nblocks,
                                  const void *in, void *out);
  void (*CC_SPTR(ccmode_ecb, roundkey))(const ccecb_ctx *ctx, unsigned r,
                                        void *key);
  cc_impl_t impl;
};
 ...
 *
 * corecrypto supports 6 stateless en(de)cryption modes and 2 stateful
 * authenticated en(de)cryption modes stateless modes CBC, CFB, CFB8, CTR, OFB,
 * XTS: They provide 3 interface functions that do not return errors codes 1-
 * ccmod_xxx_init() 2- ccmod_xxx_decrypt() 3- ccmod_xxx_encrypt()
 *
 * stateful modes CCM and GCM: They provide 7 interface functions that return
 * error codes if a function is called out of state 1- ccmod_xxx_init() 2-
 * ccmod_xxx_setiv() 3- ccmod_xxx_aad() 4- ccmod_xxx_decrypt() 5-
 * ccmod_xxx_encrypt() 6- ccmod_xxx_finalize() 7- ccmod_xxx_reset()
 *
...
struct ccmode_cbc {
  ...
  size_t block_size;
  int (*CC_SPTR(ccmode_cbc, init))(const struct ccmode_cbc *cbc, cccbc_ctx *ctx,
                                   size_t key_len, const void *key);
  /* cbc encrypt or decrypt nblocks from in to out, iv will be used and updated.
   */
  int (*CC_SPTR(ccmode_cbc, cbc))(const cccbc_ctx *ctx, cccbc_iv *iv,
...
struct ccmode_cfb {
  ...
  size_t block_size;
  int (*CC_SPTR(ccmode_cfb, init))(const struct ccmode_cfb *cfb, cccfb_ctx *ctx,
                                   size_t key_len, const void *key,
                                   const void *iv);
  int (*CC_SPTR(ccmode_cfb, cfb))(cccfb_ctx *ctx, size_t nbytes, const void *in,
                                  void *out);
  const void *custom;
};
...
struct ccmode_cfb8 {
  ...
  size_t block_size;
  int (*CC_SPTR(ccmode_cfb8, init))(const struct ccmode_cfb8 *cfb8,
                                    cccfb8_ctx *ctx, size_t key_len,
                                    const void *key, const void *iv);
  int (*CC_SPTR(ccmode_cfb8, cfb8))(cccfb8_ctx *ctx, size_t nbytes,
                                    const void *in, void *out);
  const void *custom;
};
...
struct ccmode_ctr {
  ...
  int (*CC_SPTR(ccmode_ctr, init))(const struct ccmode_ctr *mode,
                                   ccctr_ctx *ctx, size_t key_len,
                                   const void *key, const void *iv);
  int (*CC_SPTR(ccmode_ctr, setctr))(const struct ccmode_ctr *mode,
                                     ccctr_ctx *ctx, const void *ctr);
  int (*CC_SPTR(ccmode_ctr, ctr))(ccctr_ctx *ctx, size_t nbytes, const void *in,
                                  void *out);
  const void *custom;
};
...
struct ccmode_ofb {
  ...
  size_t block_size;
  int (*CC_SPTR(ccmode_ofb, init))(const struct ccmode_ofb *ofb, ccofb_ctx *ctx,
                                   size_t key_len, const void *key,
                                   const void *iv);
  int (*CC_SPTR(ccmode_ofb, ofb))(ccofb_ctx *ctx, size_t nbytes, const void *in,
                                  void *out);
  const void *custom;
};
...
struct ccmode_xts {
  size_t size;       /* first argument to ccxts_ctx_decl(). Size of the ctx data
                        structure */
  size_t tweak_size; /* first argument to ccxts_tweak_decl(). Size of the tweak
                        structure, not the expected tweak size */
  size_t block_size;
  ...
  void *(*CC_SPTR(ccmode_xts, xts))(const ccxts_ctx *ctx, ccxts_tweak *tweak,
                                    size_t nblocks, const void *in, void *out);
...
struct ccmode_gcm {
  ...
  size_t block_size;
  int (*CC_SPTR(ccmode_gcm, init))(const struct ccmode_gcm *gcm, ccgcm_ctx *ctx,
                                   size_t key_nbytes, const void *key);
  int (*CC_SPTR(ccmode_gcm, set_iv))(ccgcm_ctx *ctx, size_t iv_nbytes,
                                     const void *iv);
  int (*CC_SPTR(ccmode_gcm,
                gmac))(ccgcm_ctx *ctx, size_t nbytes,
                       const void *in); // could just be gcm with NULL out
  int (*CC_SPTR(ccmode_gcm, gcm))(ccgcm_ctx *ctx, size_t nbytes, const void *in,
                                  void *out);
  int (*CC_SPTR(ccmode_gcm, finalize))(ccgcm_ctx *key, size_t tag_nbytes,
                                       void *tag);
...
struct ccmode_ccm {
  ...
  size_t block_size;
  int (*CC_SPTR(ccmode_ccm, init))(const struct ccmode_ccm *ccm, ccccm_ctx *ctx,
                                   size_t key_len, const void *key);
  int (*CC_SPTR(ccmode_ccm, set_iv))(ccccm_ctx *ctx, ccccm_nonce *nonce_ctx,
                                     size_t nonce_len, const void *nonce,
                                     size_t mac_size, size_t auth_len,
                                     size_t data_len);
  int (*CC_SPTR(ccmode_ccm,
                cbcmac))(ccccm_ctx *ctx, ccccm_nonce *nonce_ctx, size_t nbytes,
                         const void *in); // could just be ccm with NULL out
  int (*CC_SPTR(ccmode_ccm, ccm))(ccccm_ctx *ctx, ccccm_nonce *nonce_ctx,
                                  size_t nbytes, const void *in, void *out);
  int (*CC_SPTR(ccmode_ccm, finalize))(ccccm_ctx *key, ccccm_nonce *nonce_ctx,
                                       void *mac);
...
struct ccmode_omac {
  ...
  size_t block_size;
  int (*CC_SPTR(ccmode_omac, init))(const struct ccmode_omac *omac,
                                    ccomac_ctx *ctx, size_t tweak_len,
                                    size_t key_len, const void *key);
  int (*CC_SPTR(ccmode_omac, omac))(ccomac_ctx *ctx, size_t nblocks,
                                    const void *tweak, const void *in,
                                    void *out);
  const void *custom;
};
...
struct ccmode_siv {
  ...
  int (*CC_SPTR(ccmode_siv, set_nonce))(
      ccsiv_ctx *ctx, size_t nbytes,
      const uint8_t *in); // could just be ccm with NULL out
  int (*CC_SPTR(ccmode_siv,
                auth))(ccsiv_ctx *ctx, size_t nbytes,
                       const uint8_t *in); // could just be ccm with NULL out
  int (*CC_SPTR(ccmode_siv, crypt))(ccsiv_ctx *ctx, size_t nbytes,
...
/* This provides an implementation of SIV using AES CTR mode with HMAC as the
 MAC, allowing for a tagging mechanism with collision resistant tags. This is a
 modification of the standard specified in https://tools.ietf.org/html/rfc5297
 also in
 http://csrc.nist.gov/groups/ST/toolkit/BCM/documents/proposedmodes/siv/siv.pdf
...
struct ccmode_siv_hmac {
  ...
  int (*CC_SPTR(ccmode_siv_hmac, init))(const struct ccmode_siv_hmac *sivhmac,
                                        ccsiv_hmac_ctx *ctx, size_t key_len,
                                        const uint8_t *key,
                                        const size_t tag_size);
  int (*CC_SPTR(ccmode_siv_hmac, set_nonce))(ccsiv_hmac_ctx *ctx, size_t nbytes,
                                             const uint8_t *in);
  int (*CC_SPTR(ccmode_siv_hmac, auth))(ccsiv_hmac_ctx *ctx, size_t nbytes,
                                        const uint8_t *in);
  int (*CC_SPTR(ccmode_siv_hmac, crypt))(ccsiv_hmac_ctx *ctx, size_t nbytes,
                                         const uint8_t *in, uint8_t *out);
