...
struct ccdigest_info {
  ...
  void (*CC_SPTR(ccdigest_info, compress))(ccdigest_state_t state,
                                           size_t nblocks, const void *data);
  void (*CC_SPTR(ccdigest_info, final))(const struct ccdigest_info *di,
                                        ccdigest_ctx_t ctx,
                                        unsigned char *digest);
  cc_impl_t impl;
  void (*CC_SPTR(ccdigest_info,
                 compress_parallel))(ccdigest_state_t state1, size_t nblocks1,
                                     const void *data1, ccdigest_state_t state2,
                                     size_t nblocks2, const void *data2);
};
...
/* Return sizeof a ccdigest_ctx for a given size_t _state_size_ and
   size_t _block_size_. */
#define ccdigest_ctx_size(_state_size_, _block_size_)                          \
  ((_state_size_) + sizeof(uint64_t) + (_block_size_) + sizeof(unsigned int))
/* Return sizeof a ccdigest_ctx for a given struct ccdigest_info *_di_. */
#define ccdigest_di_size(_di_)                                                 \
  (ccdigest_ctx_size((_di_)->state_size, (_di_)->block_size))
...
/* Declare a ccdigest_ctx for a given size_t _state_size_ and
   ...
   stack. */
#define ccdigest_ctx_decl(_state_size_, _block_size_, _name_)                  \
  cc_ctx_decl(struct ccdigest_ctx,                                             \
              ccdigest_ctx_size(_state_size_, _block_size_), _name_)
#define ccdigest_ctx_clear(_state_size_, _block_size_, _name_)                 \
  cc_clear(ccdigest_ctx_size(_state_size_, _block_size_), _name_)
/* Declare a ccdigest_ctx for a given size_t _state_size_ and
   size_t _block_size_, named _name_.  Can be used on the stack. */
#define ccdigest_di_decl(_di_, _name_)                                         \
  cc_ctx_decl_vla(struct ccdigest_ctx, ccdigest_di_size(_di_), _name_)
...
#define ccdigest_state_u8(_di_, _ctx_)                                         \
  ccdigest_u8(ccdigest_state((_di_), (_ctx_)))
#define ccdigest_state_u32(_di_, _ctx_)                                        \
  ccdigest_u32(ccdigest_state((_di_), (_ctx_)))
#define ccdigest_state_u64(_di_, _ctx_)                                        \
  ccdigest_u64(ccdigest_state((_di_), (_ctx_)))
#define ccdigest_state_ccn(_di_, _ctx_)                                        \
  ccdigest_ccn(ccdigest_state((_di_), (_ctx_)))

#define ccdigest_nbits(_di_, _ctx_)                                            \
  (*((uint64_t *)((ccdigest_ctx_t)(_ctx_))->state))
#define ccdigest_state(_di_, _ctx_)                                            \
  ((ccdigest_state_t)(((ccdigest_ctx_t)(_ctx_))->state + sizeof(uint64_t)))
#define ccdigest_data(_di_, _ctx_)                                             \
  (((ccdigest_ctx_t)(_ctx_))->state + (_di_)->state_size + sizeof(uint64_t))
#define ccdigest_num(_di_, _ctx_)                                              \
  (*((unsigned int *)(((ccdigest_ctx_t)(_ctx_))->state + (_di_)->state_size +  \
                      sizeof(uint64_t) + (_di_)->block_size)))
...
CC_INLINE
void ccdigest_final(const struct ccdigest_info *di, ccdigest_ctx_t ctx,
                    unsigned char *digest) {
  di->final(di, ctx, digest);
}

void ccdigest(const struct ccdigest_info *di, size_t len, const void *data,
              void *digest);

/*!
  @function ccdigest_parallel
  @abstract Hashes two inputs of the same size, in parallel where hardware
...
void ccdigest_parallel(const struct ccdigest_info *di, size_t data_nbytes,
                       const void *data1, void *digest1, const void *data2,
                       void *digest2);
...
#define CC_DIGEST_OID_SHA224                                                   \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x04")
#define CC_DIGEST_OID_SHA256                                                   \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x01")
#define CC_DIGEST_OID_SHA384                                                   \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x02")
#define CC_DIGEST_OID_SHA512                                                   \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x03")
#define CC_DIGEST_OID_SHA512_256                                               \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x06")
#define CC_DIGEST_OID_RMD160 OID_DEF("\x06\x05\x2B\x24\x03\x02\x01")
#define CC_DIGEST_OID_SHA3_224                                                 \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x07")
#define CC_DIGEST_OID_SHA3_256                                                 \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x08")
#define CC_DIGEST_OID_SHA3_384                                                 \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x09")
#define CC_DIGEST_OID_SHA3_512                                                 \
  OID_DEF("\x06\x09\x60\x86\x48\x01\x65\x03\x04\x02\x0A")
