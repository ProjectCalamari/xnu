...
#elif __has_builtin(__builtin___memcpy_chk) && !defined(_MSC_VER) &&           \
    !CC_SGX && !CC_ARM_ARCH_6M
#define cc_memcpy(dst, src, len)                                               \
  __builtin___memcpy_chk((dst), (src), (len), __builtin_object_size((dst), 1))
#define cc_memcpy_nochk(dst, src, len)                                         \
...
#elif __has_builtin(__builtin___memmove_chk) && !defined(_MSC_VER) &&          \
    !CC_SGX && !CC_ARM_ARCH_6M
#define cc_memmove(dst, src, len)                                              \
  __builtin___memmove_chk((dst), (src), (len), __builtin_object_size((dst), 1))
#else
...
#elif __has_builtin(__builtin___memset_chk) && !defined(_MSC_VER) &&           \
    !CC_SGX && !CC_ARM_ARCH_6M
#define cc_memset(dst, val, len)                                               \
  __builtin___memset_chk((dst), (val), (len), __builtin_object_size((dst), 1))
#else
...
CC_INLINE uint64_t cc_load64_le(const uint8_t cc_sized_by(8) * y) {
  return (((uint64_t)(y[7])) << 56) | (((uint64_t)(y[6])) << 48) |
         (((uint64_t)(y[5])) << 40) | (((uint64_t)(y[4])) << 32) |
         (((uint64_t)(y[3])) << 24) | (((uint64_t)(y[2])) << 16) |
         (((uint64_t)(y[1])) << 8) | ((uint64_t)(y[0]));
}
...
CC_INLINE uint32_t cc_load32_le(const uint8_t cc_sized_by(4) * y) {
  return (((uint32_t)(y[3])) << 24) | (((uint32_t)(y[2])) << 16) |
         (((uint32_t)(y[1])) << 8) | ((uint32_t)(y[0]));
}
...
#define CC_PROVIDES_ABORT                                                      \
  (!(CC_BASEBAND || CC_EFI || CC_RTKITROM || CC_USE_SEPROM))
...
/*!
  ...
  @abstract Abort execution iff the platform provides a function like @p abort()
  or @p panic()

  @discussion If the platform does not provide a means to abort execution, this
  function does nothing; therefore, callers should return an error code after
