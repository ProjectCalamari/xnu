...
#define cccmac_ctx_size(_mode_)                                                \
  (cccmac_hdr_size + cccmac_iv_size(_mode_) + cccmac_cbc_size(_mode_))
#define cccmac_ctx_n(_mode_) ccn_nof_size(cccmac_ctx_size(_mode_))

#define cccmac_mode_decl(_mode_, _name_)                                       \
  cc_ctx_decl_vla(struct cccmac_ctx, cccmac_ctx_size(_mode_), _name_)
#define cccmac_mode_clear(_mode_, _name_)                                      \
  cc_clear(cccmac_ctx_size(_mode_), _name_)
...
#define cccmac_mode_sym_ctx(_mode_, HC)                                        \
  (cccbc_ctx *)(cccmac_mode_ctx_start(_mode_, HC))
#define cccmac_mode_iv(_mode_, HC)                                             \
  (cccbc_iv *)(cccmac_mode_ctx_start(_mode_, HC) + cccmac_cbc_size(_mode_))
 ...
 */
int cccmac_one_shot_generate(const struct ccmode_cbc *cbc, size_t key_nbytes,
                             const void *cc_sized_by(key_nbytes) key,
                             size_t data_nbytes,
                             const void *cc_sized_by(data_nbytes) data,
                             size_t mac_nbytes,
                             void *cc_sized_by(mac_nbytes) mac);
...
/*!
 ...
 @discussion Only supports CMAC_BLOCKSIZE block ciphers
 */
int cccmac_one_shot_verify(const struct ccmode_cbc *cbc, size_t key_nbytes,
                           const void *cc_sized_by(key_nbytes) key,
                           size_t data_nbytes,
                           const void *cc_sized_by(data_nbytes) data,
                           size_t expected_mac_nbytes,
                           const void *cc_sized_by(expected_mac_nbytes)
                               expected_mac);
...
int cccmac_init(const struct ccmode_cbc *cbc, cccmac_ctx_t ctx,
...
int cccmac_update(cccmac_ctx_t ctx, size_t data_nbytes,
                  const void *cc_sized_by(data_nbytes) data);
...
/*!
 ...
 @discussion Only supports CMAC_BLOCKSIZE block ciphers
 */
int cccmac_final_generate(cccmac_ctx_t ctx, size_t mac_nbytes,
                          void *cc_sized_by(mac_nbytes) mac);
...
/*!
 ...
 @discussion Only supports CMAC_BLOCKSIZE block ciphers
 */
int cccmac_final_verify(cccmac_ctx_t ctx, size_t expected_mac_nbytes,
                        const void *cc_sized_by(expected_mac_nbytes)
                            expected_mac);
