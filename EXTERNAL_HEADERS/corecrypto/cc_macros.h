...
#define __CC_DEBUG_REQUIRE_MESSAGE(name, assertion, label, message, file,      \
                                   line, value)                                \
  {                                                                            \
    char *___t = cc_strstr(file);                                              \
    cc_printf("require: %s, %s%s:%d\n", assertion,                             \
              (message != 0) ? message : "", ___t == NULL ? file : ___t,       \
              line);                                                           \
  }
    ...
    (!CORECRYPTO_DEBUG_ENABLE_CC_REQUIRE_PRINTS)
  ...
  do {                                                                         \
    if (__builtin_expect(!(assertion), 0)) {                                   \
      __CC_DEBUG_REQUIRE_MESSAGE(__CC_DEBUG_ASSERT_COMPONENT_NAME_STRING,      \
                                 #assertion, #exceptionLabel, 0, __FILE__,     \
                                 __LINE__, 0);                                 \
      goto exceptionLabel;                                                     \
    }                                                                          \
  } while (0)
