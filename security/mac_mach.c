...
static struct proc *mac_task_get_proc(struct task *task) {
...
int mac_task_check_expose_task(struct task *task, mach_task_flavor_t flavor) {
  int error;
...
int mac_task_check_task_id_token_get_task(struct task *task,
...
int mac_task_check_set_host_special_port(struct task *task, int id,
...
int mac_task_check_get_task_special_port(struct task *task, struct task *target,
                                         int which) {
#pragma unused(task)
  int error;
  struct proc *target_proc = NULL;
...
int mac_task_check_set_task_special_port(struct task *task, struct task *target,
                                         int which, struct ipc_port *port) {
#pragma unused(task)
  int error;
  ...
  struct proc_ident pident =
      proc_ident_with_policy(targetp, IDENT_VALIDATION_PROC_EXACT);
  proc_rele(targetp);
...
#pragma unused(task)
  ...
  for (exception = FIRST_EXCEPTION; exception < EXC_TYPES_COUNT; exception++) {
    if (exception_mask & (1 << exception)) {
      MAC_CHECK(proc_check_set_task_exception_port, cred, &pident, exception,
                new_behavior);
      if (error) {
        break;
      }
...
#pragma unused(task)
  ...
  for (exception = FIRST_EXCEPTION; exception < EXC_TYPES_COUNT; exception++) {
    if (exception_mask & (1 << exception)) {
      MAC_CHECK(proc_check_set_thread_exception_port, cred, &pident, exception,
                new_behavior);
      if (error) {
        break;
      }
...
void mac_thread_userret(struct thread *td) { MAC_PERFORM(thread_userret, td); }

void mac_thread_telemetry(struct thread *t, int err, void *data,
                          size_t length) {
...
/**** Exception Policy
 *
 * Note that the functions below do not fully follow the usual convention for
 * mac policy functions in the kernel. Besides avoiding confusion in how the mac
 * function names are mixed with the actual policy function names, we diverge
 * because the exception policy is somewhat special: It is used in places where
 * allocation and association must be separate, and its labels do not only
 * belong to one type of object as usual, but to two (on exception actions and
 * on tasks as crash labels).
 */
...
struct label *mac_exc_create_label(struct exception_action *action) {
  return mac_labelzone_alloc_for_owner(
      action ? &action->label : NULL, MAC_WAITOK, ^(struct label *label) {
        // Policy initialization of the label, typically performs allocations as
        // well. (Unless the policy's full data really fits into a pointer
        // size.)
...
void mac_exc_associate_action_label(struct exception_action *action,
                                    struct label *label) {
...
int mac_exc_update_action_label(struct exception_action *action,
...
int mac_exc_inherit_action_label(struct exception_action *parent,
...
int mac_exc_action_check_exception_send(struct task *victim_task,
  ...
  MAC_CHECK(exc_action_check_exception_send, label, action,
            mac_exc_label(action));
...
int mac_schedule_telemetry(void) { return telemetry_macf_mark_curthread(); }
