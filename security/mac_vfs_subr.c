...
int vnode_label(struct mount *mp, struct vnode *dvp, struct vnode *vp,
  ...
  if (exit_fast) {
    if (flags & VNODE_LABEL_CREATE) {
      error = mac_vnode_notify_create(ctx, mp, dvp, vp, cnp);
    }
    return error;
  }
  ...
  if ((vp->v_lflag & VL_LABEL) == 0) {
    ...
    while (vp->v_lflag & VL_LABEL) {
      ...
      error =
          msleep(&vp->v_label, &vp->v_lock, PVFS | PDROP, "vnode_label", &ts);
      vnode_lock_spin(vp);
...
int mac_vnop_setxattr(struct vnode *vp, const char *name, char *buf,
                      size_t len) {
  vfs_context_t ctx;
  ...
  auio = uio_createwithbuffer(1, 0, UIO_SYSSPACE, UIO_WRITE, &uio_buf[0],
                              sizeof(uio_buf));
...
int mac_vnop_getxattr(struct vnode *vp, const char *name, char *buf, size_t len,
                      size_t *attrlen) {
...
int mac_vnop_removexattr(struct vnode *vp, const char *name) {
  vfs_context_t ctx = vfs_context_current();
  int options = XATTR_NOSECURITY;
  int error;
