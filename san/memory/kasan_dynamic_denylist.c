...
static access_t
    denylisted_types; /* bitmap of access types with denylist entries */

static void dyn_denylist_lock(boolean_t *b) {
  *b = ml_set_interrupts_enabled(false);
  simple_lock(&_dyn_denylist_lock, LCK_GRP_NULL);
}

static void dyn_denylist_unlock(boolean_t b) {
...
struct denylist_hash_entry {
  ...
  struct denylist_entry
      *dle;       // denylist entry that this caller is an instance of
...
static struct denylist_hash_entry *alloc_hash_entry(void) {
...
static unsigned hash_addr(uintptr_t addr) {
  addr ^= (addr >> 7); /* mix in some of the bits likely to select the kext */
  return (unsigned)addr & HASH_MASK;
}

static struct denylist_hash_entry *denylist_hash_lookup(uintptr_t addr) {
...
static int NOINLINE range_tree_cmp(const struct range_tree_entry *e1,
                                   const struct range_tree_entry *e2) {
...
void kasan_dyn_denylist_load_kext(uintptr_t addr, const char *kextname) {
  int i;
    ...
    if (cmd->cmd == LC_SEGMENT_KERNEL) {
      ...
      if (is_exec) {
        struct range_tree_entry *e =
            kalloc_type(struct range_tree_entry, Z_WAITOK | Z_ZERO | Z_NOFAIL);
...
void kasan_dyn_denylist_unload_kext(uintptr_t addr) {
  int i;
 ...
 */
static const NOINLINE char *addr_to_func(uintptr_t addr,
                                         const kernel_mach_header_t *mh) {
  int i;
...
bool OS_NOINLINE kasan_is_denylisted(access_t type) {
  uint32_t nframes = 0;
  ...
  for (uint32_t i = 0; i < nframes; i++) {
    ...
    if (dlhe) {
      ...
      //              dle->kext_name ?: "" , dle->func_name ?: "",
      //              VM_KERNEL_UNSLIDE(bt[i]), mask);
      dyn_denylist_unlock(flag);
      return true;
    }
  }
  ...
  for (uint32_t i = 0; i < nframes; i++) {
    ...
    // VM_KERNEL_UNSLIDE(bt[i]), funcname, kextname);
    ...
    for (size_t j = 0; j < denylist_entries; j++) {
      ...
      if (dle->kext_name && kextname &&
          strncmp(kextname, dle->kext_name, KMOD_MAX_NAME) != 0) {
        /* wrong kext name */
        continue;
      }

      if (dle->func_name && funcname &&
          strncmp(funcname, dle->func_name, 128) != 0) {
        /* wrong func name */
        continue;
      }
...
static void add_denylist_entry(const char *kext, const char *func,
                               access_t type) {
  assert(kext || func);
...
static inline access_t map_type(const char *str) {
  if (strlen(str) == 0) {
    return TYPE_NORMAL;
  }
...
bool kasan_is_denylisted(access_t __unused type) { return false; }
#endif
