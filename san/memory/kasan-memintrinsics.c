...
void *__asan_memcpy(void *dst, const void *src, size_t sz) {
  kasan_check_range(src, sz, TYPE_MEMR);
  kasan_check_range(dst, sz, TYPE_MEMW);
  return __nosan_memcpy(dst, src, sz);
}

void *__asan_memset(void *dst, int c, size_t sz) {
  kasan_check_range(dst, sz, TYPE_MEMW);
...
int __asan_bcmp(const void *a, const void *b, size_t len) {
  kasan_check_range(a, len, TYPE_MEMR);
  kasan_check_range(b, len, TYPE_MEMR);
#if CONFIG_KCOV
  kcov_trace_cmp_func(__builtin_return_address(0), KCOV_CMP_FUNC_MEMCMP, a, len,
                      b, len, false);
#endif
  return __nosan_bcmp(a, b, len);
}

int __asan_memcmp(const void *a, const void *b, size_t n) {
  kasan_check_range(a, n, TYPE_MEMR);
  kasan_check_range(b, n, TYPE_MEMR);
#if CONFIG_KCOV
  kcov_trace_cmp_func(__builtin_return_address(0), KCOV_CMP_FUNC_MEMCMP, a, n,
                      b, n, false);
#endif
...
#if CONFIG_KCOV
  kcov_trace_cmp_func(__builtin_return_address(0), KCOV_CMP_FUNC_STRCMP, s1, l1,
                      s2, l2, false);
#else
  (void)l1;
  (void)l2;
...
#if CONFIG_KCOV
  kcov_trace_cmp_func(__builtin_return_address(0), KCOV_CMP_FUNC_STRNCMP, s1,
                      l1, s2, l2, false);
#else
  (void)l1;
  (void)l2;
...
int __asan_strlcmp(const char *__counted_by(n) s1, const char *s2, size_t n) {
  size_t l1 = __asan_strnlen(s1, n);
  size_t l2 = __asan_strlen(s2);
#if CONFIG_KCOV
  kcov_trace_cmp_func(__builtin_return_address(0), KCOV_CMP_FUNC_STRNCMP, s1,
...
int __asan_strbufcmp(const char *__counted_by(s1len) s1, size_t s1len,
                     const char *__counted_by(s2len) s2, size_t s2len) {
  size_t l1 = __asan_strnlen(s1, s1len);
  size_t l2 = __asan_strnlen(s2, s2len);
#if CONFIG_KCOV
  kcov_trace_cmp_func(__builtin_return_address(0), KCOV_CMP_FUNC_STRBUFCMP, s1,
                      l1, s2, l2, false);
#else
  (void)l1;
  (void)l2;
