...
_Static_assert((VM_MEMTAG_PTR_SIZE > VM_KERNEL_POINTER_SIGNIFICANT_BITS),
               "Kernel pointers leave no room for tagging");
...
#if HIBERNATION
// if we're building a kernel with hibernation support, hibernate_write_image
...
_Static_assert(KASAN_OFFSET == KASAN_OFFSET_ARM64,
               "KASan inconsistent shadow offset");
_Static_assert(VM_MAX_KERNEL_ADDRESS < KASAN_SHADOW_MIN,
               "KASan shadow overlaps with kernel VM");
_Static_assert((VM_MIN_KERNEL_ADDRESS >> KASAN_SCALE) + KASAN_OFFSET_ARM64 >=
                   KASAN_SHADOW_MIN,
               "KASan shadow does not cover kernel VM");
_Static_assert((VM_MAX_KERNEL_ADDRESS >> KASAN_SCALE) + KASAN_OFFSET_ARM64 <
                   KASAN_SHADOW_MAX,
               "KASan shadow does not cover kernel VM");
...
#define KASAN_ARM64_STATIC_VALID_MAP                                           \
  (KASAN_ARM64_MAP | KASAN_ARM64_MAP_STATIC_VALID_PAGE)
...
#define KASAN_ARM64_MAP_STATIC_EARLY                                           \
  (KASAN_ARM64_STATIC_VALID_MAP | KASAN_ARM64_NO_PHYSMAP)
 ...
 * physmap.
 */
static vm_map_address_t kasan_arm64_phystokv(uintptr_t pa,
                                             __unused bool early) {
#if CONFIG_SPTM
  return phystokv(pa);
#else
...
#if CONFIG_SPTM
static uintptr_t kasan_arm64_kvtophys(vm_map_address_t va) {
  sptm_paddr_t pa;
 ...
 */
static uintptr_t kasan_arm64_alloc_page(void) {
...
static uintptr_t kasan_arm64_alloc_zero_page(bool early) {
...
static uintptr_t kasan_arm64_alloc_valid_page(bool early) {
  uintptr_t mem = kasan_arm64_alloc_page();
  kasan_impl_fill_valid_range(kasan_arm64_phystokv(mem, early), ARM_PGBYTES);
  return mem;
}

static void kasan_arm64_align_to_page(vm_offset_t *addrp, vm_offset_t *sizep) {
  vm_offset_t addr_aligned = vm_map_trunc_page(*addrp, ARM_PGMASK);
  *sizep = vm_map_round_page(*sizep + (*addrp - addr_aligned), ARM_PGMASK);
  *addrp = addr_aligned;
}

static uint64_t *kasan_arm64_lookup_l1(uint64_t *base, vm_offset_t address) {
  return base + L1_TABLE_T1_INDEX(address, TCR_EL1_BOOT);
}

static uint64_t *kasan_arm64_lookup_l2(uint64_t *base, vm_offset_t address) {
  return base + L2_TABLE_INDEX(address);
}

static uint64_t *kasan_arm64_lookup_l3(uint64_t *base, vm_offset_t address) {
 ...
 */
static void kasan_arm64_pte_map(vm_offset_t shadow_base, uint64_t *base,
                                uint8_t options) {
#if CONFIG_SPTM
  const sptm_paddr_t root_pt_paddr =
      (sptm_paddr_t)kasan_arm64_kvtophys((vm_map_address_t)base);
...
#if CONFIG_SPTM
  assert((*pte & ARM_TTE_VALID) &&
         ((*pte & ARM_TTE_TYPE_MASK) == ARM_TTE_TYPE_TABLE));
#else
  if (*pte & ARM_TTE_VALID) {
    assert((*pte & ARM_TTE_TYPE_MASK) == ARM_TTE_TYPE_TABLE);
  } else {
    *pte = ((uint64_t)kasan_arm64_alloc_zero_page(early) & ARM_TTE_TABLE_MASK) |
           ARM_TTE_VALID | ARM_TTE_TYPE_TABLE;
  }
#endif /* CONFIG_SPTM */
...
#if CONFIG_SPTM
    const sptm_tte_t tte =
        (sptm_tte_t)((uint64_t)kasan_arm64_alloc_zero_page(early) &
                     ARM_TTE_TABLE_MASK) |
        ARM_TTE_VALID | ARM_TTE_TYPE_TABLE;
  ...
  if (static_valid) {
    ...
    newpte =
        (uint64_t)kasan_arm64_alloc_valid_page(early) | ARM_PTE_AP(AP_RWNA);
  }

  newpte |= ARM_PTE_TYPE_VALID | ARM_PTE_AF | ARM_PTE_SH(SH_OUTER_MEMORY) |
            ARM_PTE_ATTRINDX(CACHE_ATTRINDX_DEFAULT) | ARM_PTE_NX | ARM_PTE_PNX;
...
static void kasan_map_shadow_internal(vm_offset_t address, vm_size_t size,
                                      uint8_t options) {
  size = (size + KASAN_SIZE_ALIGNMENT) & ~KASAN_SIZE_ALIGNMENT;
  vm_offset_t shadow_base =
      vm_map_trunc_page(SHADOW_FOR_ADDRESS(address), ARM_PGMASK);
  vm_offset_t shadow_top =
      vm_map_round_page(SHADOW_FOR_ADDRESS(address + size), ARM_PGMASK);
...
void kasan_map_shadow(vm_offset_t address, vm_size_t size, bool static_valid) {
 ...
 */
static void kasan_arm64_do_map_shadow_early(vm_offset_t address, vm_size_t size,
                                            uint8_t options) {
  kasan_arm64_align_to_page(&address, &size);
  vm_size_t j;

  for (j = 0; j < size; j += ARM_PGBYTES) {
    vm_offset_t virt_shadow_target =
        (vm_offset_t)SHADOW_FOR_ADDRESS(address + j);

    assert(virt_shadow_target >= KASAN_SHADOW_MIN);
    assert(virt_shadow_target < KASAN_SHADOW_MAX);

    kasan_arm64_pte_map(virt_shadow_target, (uint64_t *)bootstrap_pgtable_phys,
                        options);
  }
...
static void kasan_map_shadow_early(vm_offset_t address, vm_size_t size) {
  kasan_arm64_do_map_shadow_early(address, size, KASAN_ARM64_MAP_EARLY);
}

static void kasan_map_shadow_static_early(vm_offset_t address, vm_size_t size) {
...
void kasan_arch_init(void) {
  ...
  kasan_map_shadow_internal(VM_MIN_KERNEL_ADDRESS,
                            VM_MAX_KERNEL_ADDRESS - VM_MIN_KERNEL_ADDRESS + 1,
                            KASAN_ARM64_PREALLOCATE_TRANSLATION);
#endif
}

#if CONFIG_SPTM
/*
 * Steal memory for the shadow, and shadow map the bootstrap page tables so we
 * can run until kasan_init().
 */
void kasan_bootstrap(boot_args *args, vm_offset_t pgtable,
                     sptm_bootstrap_args_xnu_t *sptm_boot_args) {
  uintptr_t tosteal;
  ...
  kasan_impl_fill_valid_range(
      kasan_arm64_phystokv(unmutable_valid_access_page, false), ARM_PGBYTES);
  ...
  const size_t size_to_map =
      phystokv(sptm_boot_args->first_avail_phys) - sptm_boot_args->physmap_base;
   ...
   * cover memory allocated during early bootstrap, and not all of the
   * iBoot-loaded images. Map the rest here.
   */
  kasan_map_shadow_static_early(sptm_boot_args->executables_papt_start,
                                sptm_boot_args->executables_papt_end -
                                    sptm_boot_args->executables_papt_start);
#endif /* ARM_LARGE_MEMORY */

  vm_offset_t intstack_virt = (vm_offset_t)&intstack;
  vm_offset_t excepstack_virt = (vm_offset_t)&excepstack;
  vm_offset_t intstack_size =
      (vm_offset_t)&intstack_top - (vm_offset_t)&intstack;
  vm_offset_t excepstack_size =
      (vm_offset_t)&excepstack_top - (vm_offset_t)&excepstack;
  ...
  if ((vm_offset_t)args->deviceTreeP < (vm_offset_t)&_mh_execute_header) {
    kasan_map_shadow_early((vm_offset_t)args->deviceTreeP,
...
/*
 * Steal memory for the shadow, and shadow map the bootstrap page tables so we
 * can run until kasan_init(). Called while running with identity (V=P) map
 * active.
 */
void kasan_bootstrap(boot_args *args, vm_offset_t pgtable) {
  uintptr_t tosteal;
