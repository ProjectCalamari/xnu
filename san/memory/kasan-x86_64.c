...
static split_addr_t split_address(vm_offset_t address) {
  split_addr_t addr;
...
#define ROUND_SUPERPAGE(x)                                                     \
  ((((uintptr_t)(x)) + I386_LPGBYTES - 1) & ~(I386_LPGMASK))

static uintptr_t alloc_superpage(void) {
  uintptr_t mem;
...
static uintptr_t alloc_page_zero(void) {
  uintptr_t mem = alloc_page();
  bzero_phys(mem, I386_PGBYTES);
  return mem;
}

static void kasan_map_shadow_superpage_zero(vm_offset_t address,
                                            vm_size_t size) {
  ...
  for (j = 0; j < size; j += I386_LPGBYTES * 8) {
    ...
    if (L3 == NULL) {
      ...
      IdlePML4[addr.pml4] = pmem | INTEL_PTE_VALID | INTEL_PTE_WRITE;
    } else {
    ...
    if (L2 == NULL) {
      ...
      L3[addr.pdpt] = pmem | INTEL_PTE_VALID | INTEL_PTE_WRITE;
    } else {
    ...
    if (L1 == NULL) {
      L2[addr.pd] = (uint64_t)zero_superpage_phys | INTEL_PTE_VALID |
                    INTEL_PTE_PS | INTEL_PTE_NX;
    } else {
      panic("Unexpected shadow mapping, addr =  %lx, sz = %lu", address, size);
    }
...
void kasan_map_shadow(vm_offset_t address, vm_size_t size, bool cannot_poison) {
  size = kasan_granule_round(size);
  vm_offset_t shadow_base =
      vm_map_trunc_page(SHADOW_FOR_ADDRESS(address), PAGE_MASK);
  vm_offset_t shadow_top =
      vm_map_round_page(SHADOW_FOR_ADDRESS(address + size), PAGE_MASK);
  ...
  for (; shadow_base < shadow_top; shadow_base += I386_PGBYTES) {
    ...
    if ((pde & (INTEL_PTE_VALID | INTEL_PTE_PS)) ==
        (INTEL_PTE_VALID | INTEL_PTE_PS)) {
    ...
    if (L1 == NULL) {
      ...
      L2[addr.pd] = pmem | INTEL_PTE_VALID | INTEL_PTE_WRITE;
    } else {
    ...
    if (pte == NULL) {
      ...
      if (cannot_poison) {
        ...
        newpte = (vm_offset_t)alloc_page_zero() | INTEL_PTE_WRITE;
      }
      L1[addr.pt] = newpte | INTEL_PTE_VALID | INTEL_PTE_NX;
 ...
 */
void kasan_reserve_memory(void *_args) {
  ...
  total_pages = 0;
  for (i = 0, mptr_tmp = mptr; i < mcount;
       i++, mptr_tmp = (EfiMemoryRange *)(((vm_offset_t)mptr_tmp) + msize)) {
    total_pages += mptr_tmp->NumberOfPages;
  }

  to_steal = (unsigned long)(total_pages * STOLEN_MEM_PERCENT) / 100 +
             (STOLEN_MEM_BYTES / I386_PGBYTES);
  ...
  for (i = 0, mptr_tmp = mptr; i < mcount;
    ...
    top = (ppnum_t)((mptr_tmp->PhysicalStart >> I386_PGSHIFT) +
                    mptr_tmp->NumberOfPages - 1);
    ...
    if ((mptr_tmp->Type == kEfiConventionalMemory) &&
      ...
      shadow_pbase =
          mptr_tmp->PhysicalStart + (mptr_tmp->NumberOfPages << I386_PGSHIFT);
...
void kasan_lock_init(void) { simple_lock_init(&kasan_vm_lock, 0); }
