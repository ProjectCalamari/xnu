...
void NOINLINE kasan_impl_late_init(void) { kasan_init_fakestack(); }
...
void NOINLINE kasan_init_globals(vm_offset_t base, vm_size_t size) {
  ...
  for (; glob < glob_end; glob++) {
     ...
     */
    kasan_poison(glob->addr, glob->size, 0,
                 glob->size_with_redzone - glob->size, ASAN_GLOBAL_RZ);
  }
}

/* Reporting */
static const char *kasan_classic_access_to_str(access_t type) {
  if (type & TYPE_READ) {
    return "load from";
...
size_t kasan_impl_decode_issue(char *logbuf, size_t bufsize, uptr p, uptr width,
                               access_t access, violation_t reason) {
...
static inline bool kasan_poison_active(uint8_t flags) {
  switch (flags) {
  case ASAN_GLOBAL_RZ:
...
/*
 * Create a poisoned redzone at the top and at the end of a (marked) valid
 * range. Parameters: base: starting address (including the eventual left red
 * zone) size: size of the valid range leftrz: size (multiple of KASAN_GRANULE)
 * of the left redzone rightrz: size (multiple of KASAN_GRANULE) of the right
 * redzone flags: select between different poisoning options (e.g. stack vs
 * heap)
 */
 ...
 * is poisoned.
 */
static bool NOINLINE kasan_range_poisoned(vm_offset_t base, vm_size_t size,
                                          vm_offset_t *first_invalid) {
  uint8_t *shadow;
  vm_size_t i;
...
static kasan_alloc_header_t header_for_user_addr(vm_offset_t addr) {
  return (void *)(addr - sizeof(struct kasan_alloc_header));
}

void kasan_zmem_add(vm_address_t addr, vm_size_t size, vm_offset_t esize,
                    vm_offset_t offs, vm_offset_t rzsize) {
...
void kasan_zmem_remove(vm_address_t addr, vm_size_t size, vm_offset_t esize,
                       vm_offset_t offs, vm_offset_t rzsize) {
  uint8_t *shadow = SHADOW_FOR_ADDRESS(addr);
...
void kasan_alloc(vm_address_t addr, vm_size_t size, vm_size_t req,
                 vm_size_t rzsize, bool percpu, void *fp) {
...
void kasan_free(vm_address_t addr, vm_size_t size, vm_size_t req,
                vm_size_t rzsize, bool percpu, void *fp) {
  uint8_t *shadow = SHADOW_FOR_ADDRESS(addr);
...
void kasan_alloc_large(vm_address_t addr, vm_size_t req_size) {
 ...
 */
void kasan_check_alloc(vm_offset_t addr, vm_size_t size, vm_size_t req) {
  kasan_alloc_header_t h = header_for_user_addr(addr);
  ...
  if (!kasan_check_shadow(addr - h->left_rz, h->left_rz, ASAN_HEAP_LEFT_RZ) ||
      !kasan_check_shadow(addr + h->user_size, rightrz_sz,
                          ASAN_HEAP_RIGHT_RZ)) {
    kasan_violation(addr, req, TYPE_ZFREE, REASON_BAD_METADATA);
  }
...
struct kasan_quarantine_result kasan_quarantine(vm_address_t addr,
                                                vm_size_t size) {
  kasan_alloc_header_t h = header_for_user_addr(addr);
 ...
 * lives relative to the start of the buffer, but it's always the word
 * immediately before the start of the array data, so for naturally-aligned
 * objects we need to search at most 2 shadow bytes.
 */
...
SYSCTL_UINT(_kern_kasan, OID_AUTO, quarantine, CTLFLAG_RW, &quarantine_enabled,
            0, "");
