...
static inline size_t next_entry(size_t x) { return (x + 1) % ubsan_log_size; }

void ubsan_log_append(ubsan_violation_t *violation) {
  if (!ubsan_logging) {
    return;
  }
...
static size_t ubsan_log_recorded(size_t head, size_t tail) {
  if (head >= tail) {
    return head - tail;
  }
...
static int sysctl_ubsan_log_dump SYSCTL_HANDLER_ARGS {
...
static int sysctl_ubsan_log_entries SYSCTL_HANDLER_ARGS {
#pragma unused(oidp, arg1, arg2)
  size_t head, tail;
  ...
  int changed = 0;
  int err =
      sysctl_io_number(req, nentries, sizeof(nentries), &nentries, &changed);
...
SYSCTL_COMPAT_UINT(_kern_ubsan, OID_AUTO, logsize, CTLFLAG_RD, NULL,
                   (unsigned)ubsan_log_size, "");

SYSCTL_PROC(_kern_ubsan, OID_AUTO, logentries, CTLTYPE_INT | CTLFLAG_RW, 0, 0,
            sysctl_ubsan_log_entries, "I", "");

SYSCTL_PROC(_kern_ubsan, OID_AUTO, log,
            CTLTYPE_STRING | CTLFLAG_RD | CTLFLAG_MASKED, 0, 0,
            sysctl_ubsan_log_dump, "A", "");
