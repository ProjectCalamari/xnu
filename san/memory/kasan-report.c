...
static size_t kasan_dump_shadow(uptr p, char *buf, size_t len) {
  int i, j;
  size_t n = 0;
  ...
  for (i = 0; i < 1 + before + after; i++, shadow += CONTEXT_BLOCK_SIZE) {
    if ((vm_map_round_page(shadow, PAGE_MASK) != shadow_page) &&
...
static void kasan_report_internal(uptr p, uptr width, access_t access,
                                  violation_t reason, bool dopanic) {
...
static void kasan_panic_report_internal(uptr p, uptr width, access_t access,
                                        violation_t reason) {
  kasan_report_internal(p, width, access, reason, true);
}

static void kasan_log_report_internal(uptr p, uptr width, access_t access,
                                      violation_t reason) {
  kasan_report_internal(p, width, access, reason, false);
}

/* Pretty print a crash report. */
void NOINLINE OS_NORETURN kasan_crash_report(uptr p, uptr width,
                                             access_t access,
                                             violation_t reason) {
  kasan_handle_test();
...
static void kasan_log_report(uptr p, uptr width, access_t access,
                             violation_t reason) {
 ...
 */
void kasan_violation(uintptr_t addr, size_t size, access_t access,
                     violation_t reason) {
