...
#define FAKESTACK_UNUSED                                                       \
  0 /* waiting to be collected at next gc - forced by clang */
...
_Static_assert(sizeof(struct fakestack_header) <= FAKESTACK_HEADER_SZ,
...
/*
 * Enter a fakestack critical section in a reentrant-safe fashion. Returns true
 * on success with the kasan lock held.
 */
static bool thread_enter_fakestack(boolean_t *flags) {
...
static bool ptr_is_on_stack(uptr ptr) {
...
void kasan_fakestack_gc(thread_t thread) {
  ...
  struct fakestack_header_list *head =
      &kasan_get_thread_data(thread)->fakestack_head;
...
static uint8_t **fakestack_flag_ptr(vm_offset_t ptr, vm_size_t sz) {
  uint8_t **x = (uint8_t **)ptr;
  size_t idx = sz / 8;
  return &x[idx - 1];
}

static uptr ALWAYS_INLINE kasan_fakestack_alloc(int sz_class, size_t realsz) {
  ...
  if (ret) {
    ...
    size_t rightrz =
        sz - validsz - 32; /* 16 bytes, plus whatever is left over */
...
static void NOINLINE kasan_fakestack_free(int sz_class, uptr dst,
                                          size_t realsz) {
...
void NOINLINE kasan_fakestack_drop(thread_t thread) {
  boolean_t flags;
  if (!thread_enter_fakestack(&flags)) {
    panic("expected success entering fakestack");
  }
...
void NOINLINE kasan_init_fakestack(void) {
  ...
  for (int i = 0; i < FAKESTACK_NUM_SZCLASS; i++) {
    ...
    fakestack_zones[i] =
        zone_create_ext(fakestack_names[i], sz,
...
static int sysctl_fakestack_enable(__unused struct sysctl_oid *oidp,
                                   __unused void *arg1, int __unused arg2,
                                   struct sysctl_req *req) {
  int ch, err, val;

  err = sysctl_io_number(req, fakestack_enabled, sizeof(fakestack_enabled),
                         &val, &ch);
  if (err == 0 && ch) {
...
void kasan_unpoison_fakestack(thread_t __unused thread) {
  assert(__asan_option_detect_stack_use_after_return == 0);
}

void NOINLINE kasan_fakestack_drop(thread_t __unused thread) {
  assert(__asan_option_detect_stack_use_after_return == 0);
}

void kasan_fakestack_gc(thread_t __unused thread) {
  assert(__asan_option_detect_stack_use_after_return == 0);
}
...
static uptr kasan_fakestack_alloc(int __unused sz_class,
                                  size_t __unused realsz) {
  assert(__asan_option_detect_stack_use_after_return == 0);
  return 0;
}

static void OS_NORETURN kasan_fakestack_free(int __unused sz_class,
                                             uptr __unused dst,
                                             size_t __unused realsz) {
  assert(__asan_option_detect_stack_use_after_return == 0);
...
#define FAKESTACK_DECLARE(szclass)                                             \
  uptr __asan_stack_malloc_##szclass(size_t sz) {                              \
    return kasan_fakestack_alloc(szclass, sz);                                 \
  }                                                                            \
  void FAKESTACK_NORETURN __asan_stack_free_##szclass(uptr dst, size_t sz) {   \
    kasan_fakestack_free(szclass, dst, sz);                                    \
  }
