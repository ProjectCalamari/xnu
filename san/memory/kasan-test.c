...
#define TEST_NOFAULT(t)                                                        \
  do {                                                                         \
    TEST_BARRIER();                                                            \
    t->result = 1;                                                             \
    TEST_BARRIER();                                                            \
  } while (0)
#define TEST_DONE(t, res)                                                      \
  do {                                                                         \
    t->result = (res);                                                         \
    kasan_handle_test();                                                       \
  } while (0)
...
static int OS_NOINLINE test_denylist_helper(void) {
  return kasan_is_denylisted(TYPE_TEST);
}

static int OS_NOINLINE test_denylist(struct kasan_test *t) {
  TEST_START(t);
...
static int OS_NOINLINE test_denylist_str(struct kasan_test *t) {
  TEST_START(t);
  char a1[8];
...
static void OS_NOINLINE force_fakestack(char *x) {
  __asm__ __volatile__("" ::"r"(x) : "memory");
}

OS_NOINLINE
static int test_fakestack_helper(struct kasan_test *t, char *x) {
  TEST_START(t);
...
static int test_fakestack(struct kasan_test *t) {
  char x[8];
...
int *uaf_ptr;
static int *NOINLINE stack_uaf_helper(void) {
  int x;
  uaf_ptr = &x;
  return uaf_ptr;
}
...
static int kasan_run_test(struct kasan_test *test_list, int testno, int fail) {
...
static void kasan_test(int testno, int fail) {
  int i = 1;
  int pass = 0, total = 0;
  int ret;
...
void __kasan_runtests(struct kasan_test *kext_tests, int numtests) {
  int i;
...
static int sysctl_kasan_test(__unused struct sysctl_oid *oidp,
                             __unused void *arg1, int arg2,
...
SYSCTL_PROC(_kern_kasan, OID_AUTO, test,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 0, sysctl_kasan_test,
            "I", "");

SYSCTL_PROC(_kern_kasan, OID_AUTO, fail,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, 0, 1, sysctl_kasan_test,
            "I", "");
