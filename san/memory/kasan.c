 ...
 *
 * For each 8-byte granule in the address space, one byte is reserved in the
 * shadow table. Cost: ~13% of memory + 20-30MB of quarantine/redzones. See
 * kasan-classic.c for details.
 *
...
/*
 * Return true if 'thread' holds the kasan lock. Only safe if 'thread' ==
 * current thread
 */
...
bool kasan_check_enabled(access_t access) {
  return kasan_enabled && (kasan_enabled_checks & access) &&
         !kasan_is_denylisted(access);
}

void kasan_poison_range(vm_offset_t base, vm_size_t size, uint8_t flags) {
...
void NOINLINE kasan_unpoison(void *base, vm_size_t size) {
  /* size=size, leftsz=0, rightsz=0 */
  kasan_poison((vm_offset_t)base, size, 0, 0, 0);
}

void NOINLINE kasan_unpoison_stack(uintptr_t base, size_t size) {
  assert(base > 0);
  assert(size > 0);
...
void NOINLINE kasan_unpoison_curstack(bool whole_stack) {
...
void NOINLINE __asan_handle_no_return(void) {
...
void NOINLINE kasan_load_kext(vm_offset_t base, vm_size_t __unused size,
                              const void *bundleid) {
  ...
  sect = getsectdatafromheader((void *)base, KASAN_GLOBAL_SEGNAME,
                               KASAN_GLOBAL_SECTNAME, &sectsz);
  if (sect) {
...
void NOINLINE kasan_unload_kext(vm_offset_t base, vm_size_t size) {
  unsigned long sectsz;
  void *sect;
...
/*
 * It is not possible to fully enable/disable kasan. Try to disable as much
 * checks as possible to allow panic code path to create a coredump without
 * recursing into KASAN failures.
 *
 * Compiler inlined checks require us to keep running with kasan_enabled = 1 so
 * the shadow map gets properly updated.
 */
void NOINLINE kasan_kdp_disable(void) {
  kasan_enabled_checks = 0;
  kasan_impl_kdp_disable();
}

static void NOINLINE kasan_init_xnu_globals(void) {
...
void NOINLINE kasan_late_init(void) {
#if KASAN_DYNAMIC_DENYLIST
...
void NOINLINE kasan_notify_stolen(vm_offset_t top) {
  kasan_map_shadow(kernel_vtop, top - kernel_vtop, KASAN_MAY_POISON);
}

static void NOINLINE kasan_debug_touch_mappings(vm_offset_t base,
                                                vm_size_t sz) {
#if KASAN_DEBUG
  vm_size_t i;
  uint8_t tmp1, tmp2;
...
static void NOINLINE kasan_notify_address_internal(vm_offset_t address,
                                                   vm_size_t size,
                                                   bool cannot_poison) {
 ...
 */
int kasan_traverse_mappings(pmap_traverse_callback cb, void *ctx) {
...
#endif
SYSCTL_COMPAT_UINT(_kern_kasan, OID_AUTO, dynamicbl, CTLFLAG_RD, NULL,
                   KASAN_DYNAMIC_DENYLIST, "");
