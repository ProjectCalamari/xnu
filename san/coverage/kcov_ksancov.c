...
static void kcov_ksancov_bookmark_on_demand_module(uint32_t *start,
                                                   uint32_t *stop);
...
SYSCTL_UINT(_kern_kcov_od, OID_AUTO, config, CTLFLAG_RD, &ksancov_od_config, 0,
            "");
SYSCTL_UINT(_kern_kcov_od, OID_AUTO, allocated_entries, CTLFLAG_RD,
            &ksancov_od_allocated_count, 0, "");
SYSCTL_UINT(_kern_kcov_od, OID_AUTO, modules_count, CTLFLAG_RD,
            &ksancov_od_modules_count, 0, "");
SYSCTL_UINT(_kern_kcov_od, OID_AUTO, enabled_count, CTLFLAG_RD,
            &ksancov_od_enabled_count, 0, "");
SYSCTL_UINT(_kern_kcov_od, OID_AUTO, support_enabled, CTLFLAG_RD,
            &ksancov_od_support_enabled, 0, "");
SYSCTL_UINT(_kern_kcov_od, OID_AUTO, logging_enabled, CTLFLAG_RW,
            &ksancov_od_logging_enabled, 0, "");

#define ksancov_od_log(...)                                                    \
  do {                                                                         \
...
__startup_func void ksancov_init(void) {
  unsigned arg;
  ...
  if (ksancov_od_config & KSANCOV_OD_ENABLE_SUPPORT) {
    ...
    ksancov_od_module_entries = kalloc_type_tag(
        struct ksancov_od_module_entry, ksancov_od_allocated_count,
        Z_WAITOK_ZERO_NOFAIL, VM_KERN_MEMORY_DIAG);
    ksancov_od_module_handles = kalloc_type_tag(
        struct ksancov_od_module_handle, ksancov_od_allocated_count,
        Z_WAITOK_ZERO_NOFAIL, VM_KERN_MEMORY_DIAG);
  } else {
    ksancov_od_support_enabled = 0;
  }
...
void kcov_ksancov_init_thread(ksancov_dev_t *dev) { *dev = NULL; }

#define GUARD_SEEN (uint32_t)0x80000000
#define GUARD_IDX_MASK (uint32_t)0x0fffffff

static void trace_pc_guard_pcs(struct ksancov_dev *dev, uintptr_t pc) {
  ...
  ksancov_trace_pc_ent_t *entries =
      (ksancov_trace_pc_ent_t *)dev->trace->kt_entries;
  entries[idx] = pc;
}

#if CONFIG_STKSZ
static void trace_pc_guard_pcs_stk(struct ksancov_dev *dev, uintptr_t pc,
                                   uint32_t stksize) {
  if (os_atomic_load(&dev->trace->kt_head, relaxed) >= dev->maxpcs) {
    return; /* overflow */
  }
  ...
  ksancov_trace_stksize_ent_t *entries =
      (ksancov_trace_stksize_ent_t *)dev->trace->kt_entries;
  entries[idx] = {.pc = pc, .stksize = stksize};
}
#endif

static void trace_pc_guard_counter(struct ksancov_dev *dev, uint32_t *guardp) {
...
void kcov_ksancov_trace_guard(uint32_t *guardp, void *caller) {
  /*
...
void kcov_ksancov_trace_pc(kcov_thread_data_t *data, uint32_t *guardp,
                           void *caller, uintptr_t sp) {
#pragma unused(sp)
  uintptr_t pc;
  ...
  /*
   * Coverage sanitizer is disabled in the code called below. This allows
   * calling back to the kernel without the risk of killing machine with
   * recursive calls.
   */
  switch (dev->mode) {
  case KS_MODE_TRACE:
  ...
  default:
      /*
       * Treat all unsupported tracing modes as no-op. It is not destructive for
       * the kernel itself just coverage sanitiser will not record anything in
       * such case.
       */
      ;
  }
...
void kcov_ksancov_trace_pc_guard_init(uint32_t *start, uint32_t *stop) {
  ...
  /* only invoke kcov_ksancov_bookmark_on_demand_module if we assigned new
   * guards */
...
void kcov_ksancov_trace_cmp(kcov_thread_data_t *data, uint32_t type,
                            uint64_t arg1, uint64_t arg2, void *caller) {
  ksancov_dev_t dev = data->ktd_device;
  ...
  /*
   * Treat all unsupported tracing modes as no-op. It is not destructive for the
   * kernel itself just coverage sanitiser will not record anything in such
   * case.
   */
  if (dev->cmps_mode != KS_CMPS_MODE_TRACE &&
      dev->cmps_mode != KS_CMPS_MODE_TRACE_FUNC) {
    return;
  }
  ...
  size_t max_entries = (dev->cmps_sz - sizeof(ksancov_trace_t)) /
                       sizeof(ksancov_cmps_trace_ent_t);
  ...
  ksancov_cmps_trace_ent_t *entries =
      (ksancov_cmps_trace_ent_t *)dev->cmps_trace->kt_entries;
  entries[idx].pc = pc;
...
void kcov_ksancov_trace_cmp_func(kcov_thread_data_t *data, uint32_t type,
                                 const void *arg1, size_t len1,
                                 const void *arg2, size_t len2, void *caller,
                                 bool always_log) {
  ...
  uint32_t idx =
      os_atomic_add_orig(&dev->cmps_trace->kt_head,
                         space / sizeof(ksancov_cmps_trace_ent_t), relaxed);
  if (__improbable(idx >= max_entries)) {
    return;
  }
...
void kcov_ksancov_pcs_init(uintptr_t *start, uintptr_t *stop) {
#if USE_PC_TABLE
...
static void kcov_ksancov_bookmark_on_demand_module(uint32_t *start,
  ...
  if (gate_sz == 0) {
    ksancov_od_log(
        "ksancov: Dropping %s: not instrumented with gated callbacks\n",
        summary.name);
    return;
  }
  ...
  if (ksancov_od_modules_count >= ksancov_od_allocated_count) {
    ...
    ksancov_od_log("ksancov: Reallocating entries: %u -> %u\n",
                   old_ksancov_od_allocated_count, ksancov_od_allocated_count);

    ksancov_od_module_entries = krealloc_type_tag(
        struct ksancov_od_module_entry, old_ksancov_od_allocated_count,
        ksancov_od_allocated_count, ksancov_od_module_entries,
        Z_WAITOK_ZERO | Z_REALLOCF, VM_KERN_MEMORY_DIAG);

    ksancov_od_module_handles = krealloc_type_tag(
        struct ksancov_od_module_handle, old_ksancov_od_allocated_count,
        ksancov_od_allocated_count, ksancov_od_module_handles,
        Z_WAITOK_ZERO | Z_REALLOCF, VM_KERN_MEMORY_DIAG);
  }
...
bool kcov_ksancov_must_instrument(uintptr_t addr) {
   ...
   *  2) ksancov_od_module_handles[...] that are constant after being added to
   * the array, with only the gate field changing 3) atomically the gate value
   */
  unsigned int modules_count =
      os_atomic_load(&ksancov_od_modules_count, relaxed);
  for (unsigned int idx = 0; idx < modules_count; idx++) {
    struct ksancov_od_module_handle *handle = &ksancov_od_module_handles[idx];
    if (unslided_addr >= handle->text_start &&
        unslided_addr < handle->text_end && handle->gate) {
...
static ksancov_dev_t create_dev(dev_t dev) {
  ksancov_dev_t d;

  d = kalloc_type_tag(struct ksancov_dev, Z_WAITOK_ZERO_NOFAIL,
                      VM_KERN_MEMORY_DIAG);
  d->mode = KS_MODE_NONE;
...
static void free_dev(ksancov_dev_t d) {
  ...
  if ((d->cmps_mode == KS_CMPS_MODE_TRACE ||
       d->cmps_mode == KS_CMPS_MODE_TRACE_FUNC) &&
      d->cmps_trace) {
...
static void *ksancov_do_map(uintptr_t base, size_t sz, vm_prot_t prot) {
  kern_return_t kr;
  mach_port_t mem_entry = MACH_PORT_NULL;
  mach_vm_address_t user_addr = 0;
  memory_object_size_t size = sz;

  kr = mach_make_memory_entry_64(kernel_map, &size, (mach_vm_offset_t)base,
                                 MAP_MEM_VM_SHARE | prot, &mem_entry,
                                 MACH_PORT_NULL);
  if (kr != KERN_SUCCESS) {
    return NULL;
  }

  kr = mach_vm_map_kernel(get_task_map(current_task()), &user_addr, size, 0,
                          VM_MAP_KERNEL_FLAGS_ANYWHERE(), mem_entry, 0, FALSE,
                          prot, prot, VM_INHERIT_SHARE);
 ...
 */
static int ksancov_map(ksancov_dev_t d, uintptr_t *bufp, size_t *sizep) {
  uintptr_t addr;
  size_t size = d->sz;
 ...
 */
static int ksancov_map_edgemap(uintptr_t *bufp, size_t *sizep) {
  uintptr_t addr;
  size_t size;
  ...
  size = sizeof(ksancov_edgemap_t) +
         ksancov_edgemap->ke_nedges * sizeof(uintptr_t);
...
static int ksancov_open(dev_t dev, int flags, int devtype, proc_t p) {
#pragma unused(flags, devtype, p)
  ...
  if (ksancov_edgemap == NULL) {
    ...
    kern_return_t kr = kmem_alloc(kernel_map, &buf, sz,
                                  KMA_DATA_SHARED | KMA_ZERO | KMA_PERMANENT,
                                  VM_KERN_MEMORY_DIAG);
    if (kr) {
...
static int ksancov_trace_alloc(ksancov_dev_t d, ksancov_mode_t mode,
                               size_t maxpcs) {
  ...
  kern_return_t kr = kmem_alloc(
      kernel_map, &buf, sz, KMA_DATA_SHARED | KMA_ZERO, VM_KERN_MEMORY_DIAG);
  if (kr != KERN_SUCCESS) {
    return ENOMEM;
  }

  struct ksancov_trace *trace = (struct ksancov_trace *)buf;
  trace->kt_hdr.kh_magic =
      (mode == KS_MODE_TRACE) ? KSANCOV_TRACE_MAGIC : KSANCOV_STKSIZE_MAGIC;
...
static int ksancov_counters_alloc(ksancov_dev_t d) {
  if (d->mode != KS_MODE_NONE) {
    return EBUSY; /* trace/counters already created */
  }
  ...
  uintptr_t buf;
  size_t sz = sizeof(struct ksancov_counters) +
              ksancov_edgemap->ke_nedges * sizeof(uint8_t);
 ...
 */
static int ksancov_attach(ksancov_dev_t d, thread_t th) {
  if (d->mode == KS_MODE_NONE) {
    return EINVAL; /* not configured */
  }
 ...
 */
static void ksancov_detach(ksancov_dev_t d) {
  if (d->thread == THREAD_NULL) {
    /* no thread attached */
    return;
  }
...
static int ksancov_cmps_trace_alloc(ksancov_dev_t d, ksancov_cmps_mode_t mode,
                                    size_t maxcmps) {
 ...
 */
static int ksancov_cmps_map(ksancov_dev_t d, uintptr_t *bufp, size_t *sizep) {
  uintptr_t addr;
...
static int ksancov_close(dev_t dev, int flags, int devtype, proc_t p) {
#pragma unused(flags, devtype, p)
  const int minor_num = minor(dev);
  ...
  if (d->cmps_mode != KS_CMPS_MODE_NONE && d->cmps_hdr != NULL) {
    os_atomic_store(&d->cmps_hdr->kh_enabled, 0,
                    relaxed); /* stop tracing cmps */
  }
...
static void ksancov_testpanic(volatile uint64_t guess) {
...
static int ksancov_handle_on_demand_cmd(struct ksancov_on_demand_msg *kmsg) {
  struct ksancov_od_module_entry *entry = NULL;
  struct ksancov_od_module_handle *handle = NULL;
...
static int ksancov_ioctl(dev_t dev, unsigned long cmd, caddr_t _data, int fflag,
                         proc_t p) {
#pragma unused(fflag, p)
  ...
  case KSANCOV_IOC_STKSIZE:
    lck_mtx_lock(&d->lock);
    ksancov_mode_t mode =
        (cmd == KSANCOV_IOC_TRACE) ? KS_MODE_TRACE : KS_MODE_STKSIZE;
  ...
  case KSANCOV_IOC_CMPS_TRACE_FUNC:
    lck_mtx_lock(&d->lock);
    ksancov_cmps_mode_t cmp_mode = (cmd == KSANCOV_IOC_CMPS_TRACE)
                                       ? KS_CMPS_MODE_TRACE
...
static int ksancov_dev_clone(dev_t dev, int action) {
#pragma unused(dev)
...
static const struct cdevsw ksancov_cdev = {.d_open = ksancov_open,
