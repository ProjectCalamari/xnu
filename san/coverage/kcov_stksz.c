...
static uint32_t stksz_threshold =
    KERNEL_STACK_SIZE + 1; /* Larger than stack == disabled. */
...
SYSCTL_NODE(_kern_kcov, OID_AUTO, stksz, CTLFLAG_RW | CTLFLAG_LOCKED, 0,
            "stksz");
SYSCTL_INT(_kern_kcov_stksz, OID_AUTO, threshold, CTLFLAG_RW | CTLFLAG_LOCKED,
           &stksz_threshold, KERNEL_STACK_SIZE + 1, "stack size threshold");
SYSCTL_INT(_kern_kcov_stksz, OID_AUTO, delta, CTLFLAG_RW | CTLFLAG_LOCKED,
           &stksz_delta, KERNEL_STACK_SIZE + 1, "stack delta");

static int sysctl_kcov_stksz_enabled SYSCTL_HANDLER_ARGS {
  int err;
  int value = *(int *)arg1;
...
SYSCTL_PROC(_kern_kcov_stksz, OID_AUTO, enabled,
            CTLTYPE_INT | CTLFLAG_RW | CTLFLAG_LOCKED, &stksz_enabled, 0,
            sysctl_kcov_stksz_enabled, "I", "stack size recording enabled");

/* Constants to make code below simpler. */
const uint32_t debugid_above =
    MACHDBG_CODE(DBG_MACH_KCOV, KCOV_STKSZ_THRESHOLD_ABOVE);
const uint32_t debugid_below =
    MACHDBG_CODE(DBG_MACH_KCOV, KCOV_STKSZ_THRESHOLD_BELOW);
const uint32_t debugid_delta = MACHDBG_CODE(DBG_MACH_KCOV, KCOV_STKSZ_DELTA);

void kcov_stksz_init_thread(kcov_stksz_thread_t *data) {
  data->kst_pc = 0;
  data->kst_stksz = 0;
...
void kcov_stksz_update_stack_size(thread_t th, kcov_thread_data_t *kcdata,
