...
static inline int ksancov_open(void) { return open(KSANCOV_PATH, 0); }

static inline int ksancov_map(int fd, uintptr_t *buf, size_t *sz) {
  int ret;
...
static inline int ksancov_map_edgemap(int fd, uintptr_t *buf, size_t *sz) {
  int ret;
  struct ksancov_buf_desc mc = {0};
...
static inline size_t ksancov_nedges(int fd) {
  size_t nedges;
...
static inline int ksancov_mode_trace(int fd, size_t entries) {
  int ret;
...
static inline int ksancov_mode_stksize(int fd, size_t entries) {
  int ret;
...
static inline int ksancov_mode_counters(int fd) {
  int ret;
...
static inline int ksancov_thread_self(int fd) {
  int ret;
  uintptr_t th = 0;
...
static inline int ksancov_start(void *buf) {
  ksancov_header_t *hdr = (ksancov_header_t *)buf;
  atomic_store_explicit(&hdr->kh_enabled, 1, memory_order_relaxed);
  return 0;
}

static inline int ksancov_stop(void *buf) {
  ksancov_header_t *hdr = (ksancov_header_t *)buf;
  atomic_store_explicit(&hdr->kh_enabled, 0, memory_order_relaxed);
  return 0;
}

static inline int ksancov_reset(void *buf) {
  ksancov_header_t *hdr = (ksancov_header_t *)buf;
  if (hdr->kh_magic == KSANCOV_TRACE_MAGIC ||
      hdr->kh_magic == KSANCOV_STKSIZE_MAGIC ||
      hdr->kh_magic == KSANCOV_CMPS_TRACE_MAGIC) {
...
static inline uintptr_t ksancov_edge_addr(ksancov_edgemap_t *kemap,
                                          size_t idx) {
  assert(kemap);
...
static inline size_t ksancov_trace_max_ent(ksancov_trace_t *trace) {
  assert(trace);
  return trace->kt_maxent;
}

static inline size_t ksancov_trace_head(ksancov_trace_t *trace) {
  assert(trace);
...
static inline uintptr_t ksancov_trace_entry(ksancov_trace_t *trace, size_t i) {
  assert(trace);
...
static inline uintptr_t ksancov_stksize_pc(ksancov_trace_t *trace, size_t i) {
  assert(trace);
  ...
  ksancov_trace_stksize_ent_t *entries =
      (ksancov_trace_stksize_ent_t *)trace->kt_entries;
  return entries[i].pc;
}

static inline uint32_t ksancov_stksize_size(ksancov_trace_t *trace, size_t i) {
  assert(trace);
  assert(trace->kt_hdr.kh_magic == KSANCOV_STKSIZE_MAGIC);
  if (i >= trace->kt_head) {
    return 0;
...
static inline int ksancov_cmps_mode_trace(int fd, size_t entries,
...
static inline int ksancov_cmps_map(int fd, uintptr_t *buf, size_t *sz) {
  int ret;
  struct ksancov_buf_desc mc = {0};
...
ksancov_cmps_trace_entry(ksancov_trace_t *trace, size_t i) {
  ...
  ksancov_cmps_trace_ent_t *entries =
      (ksancov_cmps_trace_ent_t *)trace->kt_entries;
  return &entries[i];
}
...
static inline int
_ksancov_on_demand_operation(int fd, const char *bundle,
                             ksancov_on_demand_operation_t op,
 ...
 */
static inline int ksancov_on_demand_get_gate(int fd, const char *bundle,
                                             uint64_t *gate) {
  assert(gate);
 ...
 */
static inline int ksancov_on_demand_set_gate(int fd, const char *bundle,
                                             uint64_t value) {
  struct ksancov_on_demand_msg msg = {};
  msg.gate = value;
 ...
 */
static inline int ksancov_on_demand_get_range(int fd, const char *bundle,
