 ...
 */
static int ksancov_set_mode(int fd, ksancov_mode_t mode, int max_entries) {
  int ret = 0;
 ...
 */
static int ksancov_cmps_set_mode(int fd, ksancov_cmps_mode_t mode,
                                 int max_entries) {
  int ret = 0;
...
/*
 * Initialize coverage state from provided options. Shared mappings with kernel
 * are established here.
 */
static int ksancov_init_state(int fd, ksancov_mode_t mode,
                              ksancov_cmps_mode_t cmps_mode, int max_entries,
                              ksancov_state_t *state) {
  uintptr_t addr;
  size_t sz;
  int ret = 0;
...
static int ksancov_print_state(ksancov_state_t *state) {
  ...
  if (state->ks_cmps_mode == KS_CMPS_MODE_TRACE ||
      state->ks_cmps_mode == KS_CMPS_MODE_TRACE_FUNC) {
    static const char *type_map[KCOV_CMP_SIZE8 + 1] = {
        "8 bits", NULL, "16 bits", NULL, "32 bits", NULL, "64 bits"};

    size_t head = ksancov_trace_head(state->ks_cmps_trace);
    fprintf(stderr, "cmps head = %lu\n", head);

    for (uint32_t i = 0; i < head;) {
      ksancov_cmps_trace_ent_t *entry =
          ksancov_cmps_trace_entry(state->ks_cmps_trace, i);
      if (KCOV_CMP_IS_FUNC(entry->type)) {
        size_t space =
            ksancov_cmps_trace_func_space(entry->len1_func, entry->len2_func);
        i += space / sizeof(ksancov_cmps_trace_ent_t);
        fprintf(stderr, "0x%llx [func %u %u] '%s' '%s'\n", entry->pc,
                entry->len1_func, entry->len2_func,
        ...
        fprintf(stderr, "0x%llx [%s] 0x%llx 0x%llx\n", entry->pc,
                type_map[type], entry->args[0], entry->args[1]);
        ++i;
      }
    }
  }
...
static int ksancov_on_demand_set_enabled(int fd, const char *bundle,
                                         bool enabled) {
  int ret = 0;
...
int main(int argc, char *argv[]) {
  ...
  static struct option opts[] = {{"entries", required_argument, NULL, 'n'},
  ...
  ret = ksancov_init_state(fd, ksan_mode, ksan_cmps_mode, max_entries,
                           &ksan_state);
  if (ret) {
    perror("ksancov init\n");
    return ret;
  ...
  /* Execute binary (when provided) with enabled coverage collection. Run
   * getppid() otherwise. */
  if (path) {
    int pid = fork();
    if (pid == 0) {
