...
T_DECL(ledger_entry_v2,
       ...
       T_META_LTEPHASE(LTE_POSTINIT), T_META_OWNER("skwok2"),
  ...
  T_QUIET;
  T_ASSERT_NE(lei_v2, NULL, "malloc(ledger_entry_info_v2) of size %u",
              malloc_size);

  T_ASSERT_GE(ledger(LEDGER_ENTRY_INFO_V2, (caddr_t)(uintptr_t)getpid(),
                     (caddr_t)lei_v2, (caddr_t)&ledger_count),
              0, "ledger(LEDGER_ENTRY_INFO_V2)");
  ...
  if (retrieved_lifetime_max) {
    T_PASS(
        "successfully retrieved at least one entry which support lifetime max");
  } else {
