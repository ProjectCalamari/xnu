...
static void *get_sysctl_value_byname(const char *name, size_t *len) {
  int rc = -1;
  char *val = NULL;
  size_t val_len = 0;
...
static bool cs_enforcement_disabled(void) {
  int *cs_enforcement = NULL;
  size_t sysctl_val_len = 0;
  bool cs_enforcement_disable = false;

  cs_enforcement = (int *)get_sysctl_value_byname("vm.cs_process_enforcement",
                                                  &sysctl_val_len);
...
static bool pmap_cs_enabled(void) {
  ...
  const char *pmap_cs_absent_platforms[] = {"T7000", "T7001", "S8000", "S8001",
                                            "S8003", "T8002", "T8004"};

  kern_version =
      (char *)get_sysctl_value_byname("kern.version", &sysctl_val_len);
  ...
  for (i = 0; i < sizeof(pmap_cs_absent_platforms) /
                      sizeof(pmap_cs_absent_platforms[0]);
       i++) {
  ...
  /*
   * If we reach this point, it means the platform kernel has PMAP_CS code
   * present. However the code is disabled by default on certain SoCs. Moreover,
   * the code can be disabled through an explicit boot-arg as well.
   */
  ...
  const char *pmap_cs_disabled_platforms[] = {"T8010", "T8011", "T8012",
                                              "T8015"};

  for (i = 0; i < sizeof(pmap_cs_disabled_platforms) /
                      sizeof(pmap_cs_disabled_platforms[0]);
       i++) {
    if (strstr(kern_version, pmap_cs_disabled_platforms[i]) &&
        !strstr(bootargs, "pmap_cs=1")) {
...
exit:
  ...
  if (pmap_cs_enabled == false) {
    T_LOG("INFO: PMAP_CS is either not available or is disabled on this "
          "platform: %s",
          unavailable_reason);
  }
...
static bool pmap_cs_unsigned_pages_allowed(void) {
  char *bootargs = NULL;
  ...
  /*
   * Checking for boot-args can be tricky, since `strstr` will return based on
   * the first match for the boot-arg.
   *
   * For example: boot-args="pmap_cs_unrestrict_pmap_cs_disable=1
   * pmap_cs_unrestrict_pmap_cs_disable=0" The following code will only catch
   * the first one, and believe the boot-arg is set, even though the kernel will
   * parse both, and consider the latter as the actual value.
   *
   * This can be potentially fixed with `strrstr`, but that isn't standard in
   * the C library, so we don't use it.
   */

  if (strstr(bootargs, "pmap_cs_unrestrict_pmap_cs_disable=1")) {
    pmap_cs_unsigned_pages_allow = true;
    goto exit;
  } else if (strstr(bootargs, "amfi=1") || strstr(bootargs, "amfi=3") ||
             strstr(bootargs, "amfi=-1")) {
    /* Any of these boot-args enable pmap_cs_unrestrict_pmap_cs_disable, but it
     * can be overridden */
  ...
  if (strstr(bootargs, "pmap_cs_allow_modified_code_pages=1")) {
    ...
    /* cs_enforcement_disable enables pmap_cs_allow_modified_code_pages, but it
     * can be overridden */
...
static void pre_test(void) {
  ...
  if (pmap_cs_enabled()) {
    /*
     * When PMAP_CS is enabled, VM layer delegates all executable code signing
     * enforcement to it, and doesn't participate in executable code validation.
     * If PMAP_CS isn't allowing unsigned code pages to execute, then we expect
     * a SIGBUS error from the helper.
     */
    ...
    /*
     * When PMAP_CS isn't enabled, VM layer handles all code signing
     * enforcement, including that for executable code. If VM layer isn't
     * allowing unsigned code pages to execute, then we expect a SIGKILL error
     * from the helper.
     */
    if (!cs_enforcement_disabled()) {
      expected_code = 9;
    } else {
      T_LOG("WANRING: unsigned code pages are allowed as code signing "
            "enforcement is disabled");
    }
  }

#if defined(__arm64__)
  panic_on_unsigned = (int *)get_sysctl_value_byname(
      "vm.panic_on_unsigned_execute", &sysctl_val_len);
  if (panic_on_unsigned) {
    if (*panic_on_unsigned == 1) {
      panic_on_unsigned_orig = 1;
      *panic_on_unsigned = 0;
      T_EXPECT_POSIX_SUCCESS(sysctlbyname("vm.panic_on_unsigned_execute", NULL,
                                          0, panic_on_unsigned, sizeof(int)),
...
static void post_test(void) {
  ...
  if (panic_on_unsigned_orig == 1) {
    T_EXPECT_POSIX_SUCCESS(sysctlbyname("vm.panic_on_unsigned_execute", NULL, 0,
                                        &panic_on_unsigned_orig, sizeof(int)),
...
static void check_executable(char *exec_path) {
  int ret = -1;
  struct stat sb;
...
T_DECL(code_signing, "testing code siging with unsigned syscall code - \
    rdar://problem/23770418",
       T_META_RUN_CONCURRENTLY(true),
       T_META_IGNORECRASHES(".*vm_test_code_signing_helper.*"),
       T_META_ENABLED(false) /* rdar://98779213 */,
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "_NSGetExecutablePath: %s, size: %d", binary_path,
              path_size);
