...
T_GLOBAL_META(T_META_NAMESPACE("xnu.iokit"), T_META_RUN_CONCURRENTLY(true),
              T_META_ASROOT(true), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("IOKit"),
              T_META_OWNER("souvik_b"));

static size_t absoluteDifference(size_t first, size_t second) {
  if (first > second) {
    return first - second;
  } else {
    return second - first;
...
static void notificationReceived(void *refcon __unused,
                                 io_iterator_t iter __unused,
                                 uint32_t msgType __unused,
                                 void *msgArg __unused) {
...
static void notificationReceived2(void *refcon, io_iterator_t iter __unused,
                                  uint32_t msgType __unused,
                                  void *msgArg __unused) {
...
T_HELPER_DECL(ioserviceusernotification_race_helper,
              "ioserviceusernotification_race_helper") {
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    service = IOServiceGetMatchingService(
        kIOMainPortDefault,
        IOServiceMatching("TestIOServiceUserNotificationUserClient"));
    ...
    T_ASSERT_MACH_SUCCESS(
        IOServiceAddInterestNotification(notifyPort, service, kIOBusyInterest,
                                         notificationReceived, NULL, &iter),
        "add notification");

    struct Context *c = calloc(1, sizeof(struct Context));

    // The second notification object is released after a notification is
    // received
    T_ASSERT_MACH_SUCCESS(
        IOServiceAddInterestNotification(notifyPort2, service, kIOBusyInterest,
                                         notificationReceived2, c, &iter2),
        "add notification 2");
...
// the test involves multiple sleep points. adding together they consume at
// least
// ((WAIT_TIME1_MS + WAIT_TIME2_MS) * NUM_HELPER_INVOCATIONS + WAIT_TIME3_MS *
// EXTERNAL_METHOD_GROUP_SIZE + WAIT_TIME4_MS) ms this (plus some leeway) should
// not exceed 30s
static_assert(((WAIT_TIME1_MS + WAIT_TIME2_MS) * NUM_HELPER_INVOCATIONS +
               WAIT_TIME3_MS * EXTERNAL_METHOD_GROUP_SIZE + WAIT_TIME4_MS) <
              28 * MSEC_PER_SEC);
...
T_DECL(ioserviceusernotification_race, "Test IOServiceUserNotification race",
  ...
  char *helper_args[] = {test_path, "-n",
                         "ioserviceusernotification_race_helper", NULL};
  io_iterator_t notificationIters[NUM_NOTIFICATION_ITERS];
  pid_t childPids[NUM_HELPER_INVOCATIONS] = {};
  size_t leaks = 1, outCount = 1;

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(proc_pidpath(getpid(), test_path, MAXPATHLEN),
                         "get pid path");
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      IOTestServiceFindService("TestIOServiceUserNotification", &service),
      "Find service");
  T_QUIET;
  ...
  for (size_t i = 0; i < NUM_HELPER_INVOCATIONS; i++) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(
        IOConnectCallMethod(connect, 0, NULL, 0, NULL, 0, NULL, 0, NULL, NULL),
    ...
    while (true) {
      for (size_t k = 0; k < EXTERNAL_METHOD_GROUP_SIZE; k++) {
        T_QUIET;
        T_ASSERT_MACH_SUCCESS(IOConnectCallMethod(connect, 0, NULL, 0, NULL, 0,
                                                  NULL, 0, NULL, NULL),
                              "call external method");
  ...
  for (size_t i = 0;
       i < sizeof(notificationIters) / sizeof(notificationIters[0]); i++) {
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(
        IOServiceAddInterestNotification(notifyPort, service, kIOBusyInterest,
  ...
  for (size_t i = 0;
       i < sizeof(notificationIters) / sizeof(notificationIters[0]); i++) {
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(IOObjectRelease(notificationIters[i]),
                          "remove notification");
    notificationIters[i] = MACH_PORT_NULL;
  }

  T_ASSERT_MACH_SUCCESS(IOServiceOpen(service, mach_task_self(), 1, &connect),
                        "open service");

  T_ASSERT_MACH_SUCCESS(IOConnectCallMethod(connect, 1, NULL, 0, NULL, 0,
                                            &leaks, &outCount, NULL, NULL),
                        "call external method");
