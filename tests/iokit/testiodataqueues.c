...
// PT: These two files must be included before
...
T_DECL(iodataqueues, "Test IODataQueues", T_META_TAG_VM_PREFERRED) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(IOTestServiceFindService("TestIODataQueues", &service),
                         "Find service");
  T_QUIET;
  ...
  for (int cycle = 0; cycle < 2; cycle++) {
    ret = IOCircularDataQueueCreateWithConnection(
        kIOCircularDataQueueCreateConsumer, connect, 53, &queue);
