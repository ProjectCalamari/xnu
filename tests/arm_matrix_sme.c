...
static unsigned int sme_version(void) {
...
static size_t sme_za_size(void) { return arm_sme_svl_b() * arm_sme_svl_b(); }

static size_t sme_z_size(void) { return arm_sme_svl_b() * SME_Z_VECTORS; }

static size_t sme_p_size(void) { return arm_sme_svl_b() * SME_P_VECTORS / 8; }

static size_t sme_zt0_size(void) {
  if (sme_version() >= 2) {
    return 64;
  } else {
    return 0;
...
static size_t sme_tpidr2_size(void) { return sizeof(uint64_t); }

static inline uint8_t *sme_za(void *addr) { return addr; }

static inline const uint8_t *const_sme_za(const void *addr) { return addr; }

static inline uint8_t *sme_z(void *addr) {
  return sme_za(addr) + sme_za_size();
}

static inline const uint8_t *const_sme_z(const void *addr) {
  return const_sme_za(addr) + sme_za_size();
}

static inline uint8_t *sme_p(void *addr) { return sme_z(addr) + sme_z_size(); }

static inline const uint8_t *const_sme_p(const void *addr) {
  return const_sme_z(addr) + sme_z_size();
}

static inline uint8_t *sme_zt0(void *addr) {
  return sme_p(addr) + sme_p_size();
}

static inline const uint8_t *const_sme_zt0(const void *addr) {
  return const_sme_p(addr) + sme_p_size();
}

static size_t sme_data_size(void) {
  return sme_za_size() + sme_z_size() + sme_p_size() + sme_zt0_size() +
         sme_tpidr2_size();
}

static inline void set_sme_tpidr2_el0(void *addr, uint64_t val) {
  uint64_t *ptr = (uint64_t *)(sme_zt0(addr) + sme_zt0_size());
  *ptr = val;
}

static inline uint64_t get_sme_tpidr2_el0(const void *addr) {
  const uint64_t *ptr =
      (const uint64_t *)(const_sme_zt0(addr) + sme_zt0_size());
  return *ptr;
}

static void *sme_alloc_data(void) { return malloc(sme_data_size()); }

static bool sme_is_available(void) { return sme_version() > 0; }

static void sme_start(void) { asm volatile("smstart"); }

static void sme_stop(void) { asm volatile("smstop"); }

static void sme_load_one_vector(const void *addr) {
...
static void sme_load_data(const void *addr) {
  ...
  for (register uint16_t i asm("w12") = 0; i < svl_b; i += 16) {
    asm volatile("ldr    za[%w[i],  #0], [%[addr],  #0, mul vl]"
                 "\n"
                 ...
                 "\n"
                 :
                 : [i] "r"(i), [addr] "r"(za + (i * svl_b)));
  }

  asm volatile("ldr    z0, [%[z],   #0, mul vl]"
               "\n"
               "ldr    z1, [%[z],   #1, mul vl]"
               "\n"
               "ldr    z2, [%[z],   #2, mul vl]"
  ...
  asm volatile("ldr     p0, [%[p],  #0, mul vl]"
               "\n"
               "ldr     p1, [%[p],  #1, mul vl]"
               "\n"
               "ldr     p2, [%[p],  #2, mul vl]"
  ...
  if (sme_zt0_size()) {
    ...
    asm volatile("ldr	zt0, [%[zt0]]" : : [zt0] "r"(zt0));
  }

  __builtin_arm_wsr64("TPIDR2_EL0", get_sme_tpidr2_el0(addr));
}

static void sme_store_data(void *addr) {
  ...
  for (register uint16_t i asm("w12") = 0; i < svl_b; i += 16) {
    asm volatile("str    za[%w[i],  #0], [%[addr],  #0, mul vl]"
                 "\n"
  ...
  asm volatile("str    z0, [%[z],   #0, mul vl]"
               "\n"
               "str    z1, [%[z],   #1, mul vl]"
               "\n"
               "str    z2, [%[z],   #2, mul vl]"
  ...
  asm volatile("str     p0, [%[p],  #0, mul vl]"
               "\n"
               "str     p1, [%[p],  #1, mul vl]"
               "\n"
               "str     p2, [%[p],  #2, mul vl]"
  ...
  if (sme_zt0_size()) {
    ...
    asm volatile("str	zt0, [%[zt0]]" : : [zt0] "r"(zt0));
  }

  set_sme_tpidr2_el0(addr, __builtin_arm_rsr64("TPIDR2_EL0"));
}

static kern_return_t sme_thread_get_state(thread_act_t thread, void *addr) {
  uint8_t *za = sme_za(addr);
  uint8_t *z = sme_z(addr);
  uint8_t *p = sme_p(addr);
  uint16_t svl_b = arm_sme_svl_b();
  ...
  kern_return_t err = thread_get_state(thread, ARM_SME_STATE,
                                       (thread_state_t)&sme_state, &sme_count);
  if (err) {
    return err;
  }
  ...
  err = thread_get_state(thread, ARM_SME_ZA_STATE1, (thread_state_t)&za_state,
                         &za_count);
  if (err) {
    return err;
  }
  ...
  err = thread_get_state(thread, ARM_SVE_Z_STATE1, (thread_state_t)&z_state1,
                         &z_streaming_count);
  if (err) {
    return err;
  }
  err = thread_get_state(thread, ARM_SVE_Z_STATE2, (thread_state_t)&z_state2,
                         &z_streaming_count);
  if (err) {
    return err;
  }
  ...
  err = thread_get_state(thread, ARM_SVE_P_STATE, (thread_state_t)&p_state,
                         &p_streaming_count);
  if (err) {
    return err;
  }
  ...
  if (sme_zt0_size()) {
    ...
    err = thread_get_state(thread, ARM_SME2_STATE, (thread_state_t)&sme2_state,
                           &sme2_count);
    if (err) {
      return err;
    }
...
static kern_return_t sme_thread_set_state(thread_act_t thread,
                                          const void *addr) {
  const uint8_t *za = const_sme_za(addr);
  const uint8_t *z = const_sme_z(addr);
  const uint8_t *p = const_sme_p(addr);
  ...
  kern_return_t err = thread_set_state(
      thread, ARM_SME_STATE, (thread_state_t)&sme_state, ARM_SME_STATE_COUNT);
  if (err) {
    return err;
  }

  err = thread_set_state(thread, ARM_SVE_Z_STATE1, (thread_state_t)&z_state1,
                         ARM_SVE_Z_STATE_COUNT);
  if (err) {
    return err;
  }

  err = thread_set_state(thread, ARM_SVE_Z_STATE2, (thread_state_t)&z_state2,
                         ARM_SVE_Z_STATE_COUNT);
  if (err) {
    return err;
  }

  err = thread_set_state(thread, ARM_SVE_P_STATE, (thread_state_t)&p_state,
                         ARM_SVE_P_STATE_COUNT);
  if (err) {
    return err;
  }

  err = thread_set_state(thread, ARM_SME_ZA_STATE1, (thread_state_t)&za_state,
                         ARM_SME_ZA_STATE_COUNT);
  if (err) {
    return err;
  }
  ...
  if (sme_zt0_size()) {
    ...
    err = thread_set_state(thread, ARM_SME2_STATE, (thread_state_t)&sme2_state,
                           ARM_SME2_STATE_COUNT);
    if (err) {
      return err;
    }
  }
