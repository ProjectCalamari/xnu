...
void xfsz_signal(__unused int signo) { siglongjmp(xfsz_jmpbuf, 1); }
...
T_DECL(pwrite, "Tests avoiding SIGXFSZ with pwrite and odd offsets",
  ...
  struct stat f_stat;
  struct rlimit crl;
  static const int offs[] = {-1, -1 * 1024, -1 * 1024 * 16,
                             -1 * 1024 * 1024 * 16, 0};
  ...
  for (x = 0; offs[x] != 0; x++) {
    ...
    T_ASSERT_TRUE(((ret == -1) && (errno == EINVAL)), "negative offset %d",
                  offs[x]);
  }
  ...
  if (!sigsetjmp(xfsz_jmpbuf, 1)) {
    ...
    T_ASSERT_TRUE(((ret == -1) && (errno == EFBIG)), "large offset %d", 13);
  } else {
    signal(SIGXFSZ, SIG_DFL);
    T_FAIL("%s unexpected SIGXFSZ with offset %lX", "<rdar://problem/28581610>",
           LONG_MAX);
  }
