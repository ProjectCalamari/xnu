...
static int trigger_sleep(int for_secs) {
  ...
  T_ASSERT_POSIX_ZERO((spawn_ret = posix_spawn(&pid, pmset1_args[0], NULL, NULL,
                                               pmset1_args, environ)),
                      NULL);
  ...
  T_ASSERT_POSIX_ZERO((spawn_ret = posix_spawn(&pid, pmset2_args[0], NULL, NULL,
                                               pmset2_args, environ)),
                      NULL);
...
T_DECL(kevent_continuous_time_periodic_tick,
       "kevent(EVFILT_TIMER with NOTE_MACH_CONTINUOUS_TIME)",
  ...
  T_LOG("EV_SET(&kev, 1, EVFILT_TIMER, EV_ADD, NOTE_SECONDS | "
        "NOTE_MACH_CONTINUOUS_TIME, 4, 0, 0, 0);");
  ...
  T_WITH_ERRNO;
  T_ASSERT_EQ(kevent64(kq, NULL, 0, &kev, 1, 0, NULL), 1,
              "kevent() should have returned one event");
  T_LOG("event = {.ident = %llx, .filter = %d, .flags = %d, .fflags = %d, "
        ".data = %lld, .udata = %lld}",
        kev.ident, kev.filter, kev.flags, kev.fflags, kev.data, kev.udata);
  T_ASSERT_EQ(kev.flags & EV_ERROR, 0, "event should not have EV_ERROR set: %s",
              kev.flags & EV_ERROR ? strerror((int)kev.data) : "no error");
  ...
  T_LOG("ct progressed %llu ms, abs progressed %llu ms", ct_ms_progressed,
        tick_to_ms(abs_now - abs_then));

  if (run_sleep_tests) {
    T_ASSERT_GT(llabs((int64_t)ct_ms_progressed - (int64_t)ab_ms_progressed),
                500LL, "should have > 500ms difference between MCT and MAT");
  } else {
    T_ASSERT_LT(llabs((int64_t)ct_ms_progressed - (int64_t)ab_ms_progressed),
                10LL, "should have < 10ms difference between MCT and MAT");
  }

  if (sleep_secs < 4) {
    T_ASSERT_LT(llabs((int64_t)ct_ms_progressed - 4000), 100LL,
                "mach_continuous_time should progress ~4 seconds (+/- 100ms) "
                "between sleeps");
  }
...
T_DECL(kevent_continuous_time_absolute,
       "kevent(EVFILT_TIMER with NOTE_MACH_CONTINUOUS_TIME and NOTE_ABSOLUTE)",
       T_META_LTEPHASE(LTE_POSTINIT), T_META_TAG_VM_PREFERRED) {
  mach_timebase_info(&tb_info);
  ...
  T_LOG("EV_SET(&kev, 2, EVFILT_TIMER, EV_ADD, NOTE_MACH_CONTINUOUS_TIME | "
        "NOTE_ABSOLUTE | NOTE_USECONDS, fire_at, 0);");
  ...
  T_ASSERT_EQ(kevent64(kq, NULL, 0, &event, 1, 0, &timeout), 1,
              "kevent() should have returned one event");
  T_LOG("event = {.ident = %llx, .filter = %d, .flags = %d, .fflags = %d, "
        ".data = %lld, .udata = %lld}",
        event.ident, event.filter, event.flags, event.fflags, event.data,
        event.udata);
  T_ASSERT_EQ(event.flags & EV_ERROR, 0,
              "event should not have EV_ERROR set: %s",
              event.flags & EV_ERROR ? strerror((int)event.data) : "no error");

  uint64_t elapsed_ms = tick_to_ms(mach_continuous_time() - cnt_now);
  int64_t missed_by =
      tick_to_ns((int64_t)mach_continuous_time() - (int64_t)cnt_then) / 1000000;

  // ~1/2 second is about as good as we'll get
  T_ASSERT_LT(llabs(missed_by), 500LL,
              "timer should pop 3 sec in the future, popped after %lldms",
              elapsed_ms);
...
T_DECL(kevent_continuous_time_pops,
       "kevent(EVFILT_TIMER with NOTE_MACH_CONTINUOUS_TIME with multiple pops)",
       T_META_LTEPHASE(LTE_POSTINIT), T_META_TAG_VM_PREFERRED) {
  ...
  T_LOG("EV_SET(&kev, 3, EVFILT_TIMER, EV_ADD, NOTE_MACH_CONTINUOUS_TIME, 100, "
        "0);");
  ...
  struct kevent64_s event = {0};
  T_ASSERT_EQ(kevent64(kq, NULL, 0, &event, 1, 0, NULL), 1,
              "kevent() should have returned one event");
  T_LOG("event = {.ident = %llx, .filter = %d, .flags = %d, .fflags = %d, "
        ".data = %lld, .udata = %llu}",
        event.ident, event.filter, event.flags, event.fflags, event.data,
        event.udata);
  T_ASSERT_EQ(event.flags & EV_ERROR, 0, "should not have EV_ERROR set: %s",
              event.flags & EV_ERROR ? strerror((int)event.data) : "no error");
  ...
  if (run_sleep_tests) {
    T_ASSERT_LT(llabs((int64_t)ms_elapsed - 2000LL), 500LL,
                "slept for %llums, expected 2000ms (astris is connected?)",
                ms_elapsed);
  }
