 ...
 */
static void __unused test_kevent(int type) {
  int sockets[2] = {-1};
  int kq = -1;
  ...
  T_ASSERT_POSIX_SUCCESS(fcntl(sockets[0], F_SETFL, flags | O_NONBLOCK),
                         "fcntl");
...
static void __unused test_kevent_lowat(int type) {
  int sockets[2] = {-1};
  int kq = -1;
  struct kevent evlist = {0};
  struct kevent chlist = {0};
  ...
  EV_SET(&chlist, sockets[0], EVFILT_WRITE, EV_ADD | EV_ERROR, NOTE_LOWAT, 100,
         0);
  ...
  result =
      setsockopt(sockets[0], SOL_SOCKET, SO_SNDLOWAT, &lowat, sizeof(lowat));
...
T_DECL(uipc_kevent, "Tests the UNIX Domain kevent filter",
