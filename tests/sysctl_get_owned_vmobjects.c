...
static void main_test(void) {
  ...
  kr = vm_allocate(__self, &tmp_buf, tmp_size,
  ...
  T_QUIET;
  T_EXPECT_NE(tmp_buf, (vm_address_t)0,
              "failed to allocate temporary purgable buffer\n");

  kr = vm_allocate(__self, &tmp_buf2, tmp_size,
                   VM_FLAGS_ANYWHERE | VM_FLAGS_PURGABLE);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "vm_allocate(%zu) error 0x%x (%s)",
              (size_t)tmp_size, kr, mach_error_string(kr));
  T_QUIET;
  T_EXPECT_NE(tmp_buf2, (vm_address_t)0,
              "failed to allocate temporary purgable buffer\n");
  ...
  ret = sysctlbyname(g_sysctl_name, (void *)tmp_buf, &out_size, &task_name,
                     sizeof(task_name));
  T_EXPECT_EQ(
      ret, -1,
      "expected failure with task_name == MACH_PORT_NULL in new parameters\n");
  ...
  T_EXPECT_EQ(errno, EINVAL,
              "expected EINVAL with 0 output size and valid pointer\n");

  /* we should get the number of entries we should allocate for */
  out_size = 0;
  output_size = 0;
  ret = sysctlbyname(g_sysctl_name, NULL, &out_size, &task_name,
                     sizeof(task_name));
  T_QUIET;
  T_EXPECT_EQ(ret, 0, "failed getting the number of entries\n");
  T_EXPECT_EQ(out_size, 2 * sizeof(vm_object_query_data_t) + sizeof(int64_t),
              "expected one entry\n");
  ...
  ret = sysctlbyname(g_sysctl_name, out_buffer, &out_size, &task_name,
                     sizeof(task_name));

  T_QUIET;
  T_EXPECT_EQ(ret, 0, "sysctlbyname failed\n");
  T_EXPECT_EQ(out_size, 1 * sizeof(vm_object_query_data_t) + sizeof(int64_t),
              "sysctl return size is incorrect\n");
  T_EXPECT_EQ(out_buffer->entries, 1ULL, "should have 1 vm object\n");
  T_EXPECT_NE(out_buffer->data[0].object_id, 0ULL,
              "vm_object_id should not be 0\n");
  ...
  kr = mach_vm_region_recurse(__self, &addr, &addr_size, &nestingDepth,
                              (vm_region_info_t)&regionInfo, &count);
  T_QUIET;
  T_EXPECT_EQ(kr, KERN_SUCCESS, "mach_vm_region_recurse(%zu) error 0x%x (%s)\n",
              tmp_size, kr, mach_error_string(kr));
  T_EXPECT_EQ(regionInfo.object_id_full, out_buffer->data[0].object_id,
  ...
  T_EXPECT_EQ(kr, KERN_SUCCESS, "mach_vm_region_recurse(%zu) error 0x%x (%s)\n",
              tmp_size, kr, mach_error_string(kr));
  T_EXPECT_EQ(regionInfo.object_id_full, out_buffer->data[1].object_id,
  ...
  /* corpse */
  {
    ...
    /* corpse_port can be a valid NULL if out of resources, corpse limit, or
     * corpses disabled */
    if (corpse_port != MACH_PORT_NULL) {
      ...
      /* we should get the number of entries we should allocate for */
      corpse_out_size = 0;
      ret = sysctlbyname(g_sysctl_name, NULL, &corpse_out_size, &corpse_port,
                         sizeof(corpse_port));

      T_QUIET;
      T_EXPECT_EQ(ret, 0, "failed getting the number of entries for corpse\n");
      T_EXPECT_EQ(corpse_out_size, out_size,
                  "corpse output size matchrd the parent process\n");
      T_EXPECT_EQ(corpse_out_size,
                  2 * sizeof(vm_object_query_data_t) + sizeof(int64_t),
                  "corpse expected one entry\n");
      ...
      T_QUIET;
      T_EXPECT_NE(
          corpse_out_buffer, NULL,
          "failed to allocate the output buffer for sysctlbyname for corpse\n");

      /* get the list for the current process */
      corpse_out_size = corpse_output_size;
      memset(corpse_out_buffer, 0, corpse_output_size);
      ret = sysctlbyname(g_sysctl_name, corpse_out_buffer, &corpse_out_size,
                         &corpse_port, sizeof(corpse_port));
      ...
      T_EXPECT_EQ(corpse_out_size,
                  2 * sizeof(vm_object_query_data_t) + sizeof(int64_t),
                  "corpse sysctl return size is incorrect\n");
      T_EXPECT_EQ(corpse_out_buffer->entries, 2ULL,
                  "corpse should have 2 vm objects\n");
      T_EXPECT_NE(corpse_out_buffer->data[0].object_id, 0ULL,
                  "corpse vm_object_id should not be 0\n");
