...
T_GLOBAL_META(T_META_NAMESPACE("xnu.exec"), T_META_CHECK_LEAKS(false),
...
static void do_exec(void) {
  ...
  snprintf(echo_arg, sizeof(echo_arg),
...
static void *thread_main(void *arg) {
  ...
  if (exec_mode == EXEC_SECOND) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "dprintf()");
    do_exec();
  }
...
void run_test(void) {
  ...
  if (child_pid == 0) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "read()");

    T_LOG("mode: %d, %d: Parent[%d]: got: '%s' from execing child, trying to "
          "kill and wait\n",
    ...
    case KILL_ONCE:
      ...
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(waitedpid, "waitpid()");
    ...
    T_LOG("mode: %d, %d: Parent[%d]: waitpid returned: %d, errno %d (%s), exit "
          "signal %d, after %d loops\n",
          kill_mode, exec_mode, getpid(), waitedpid, errno, strerror(errno),
          WTERMSIG(status), killcount);
  }
}

T_DECL(exec_exit_race_once_first,
       "Exec-exit race, one kill, exec on first thread") {
  int rv = pipe(fd);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rv, "pipe()");
...
T_DECL(exec_exit_race_many_first,
...
T_DECL(exec_exit_race_once_second,
...
T_DECL(exec_exit_race_many_second,
