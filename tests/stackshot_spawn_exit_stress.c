...
T_GLOBAL_META(T_META_NAMESPACE("xnu.stackshot"),
              T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("stackshot"),
              T_META_OWNER("jonathan_w_adams"), T_META_CHECK_LEAKS(false),
              T_META_ASROOT(true));
...
static void *loop(__attribute__((unused)) void *arg) { exit(0); }

T_HELPER_DECL(spawn_children_helper, "spawn_children helper") {
  pthread_t pthread;

  T_QUIET;
  T_ASSERT_POSIX_ZERO(pthread_create(&pthread, NULL, loop, NULL),
                      "pthread_create");
...
static void take_stackshot(void) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "set flags on stackshot config");
...
retry:
  ...
  if (ret == EBUSY || ret == ETIMEDOUT) {
    if (retries_remaining > 0) {
      ...
      T_QUIET;
      T_ASSERT_POSIX_ZERO(
          ret, "called stackshot_capture_with_config (no retries remaining)");
    }
  } else {
    T_QUIET;
  ...
  T_QUIET;
  T_EXPECT_POSIX_ZERO(ret, "deallocated stackshot config");
}

T_DECL(stackshot_spawn_exit,
       "tests taking many stackshots while children processes are "
       "spawning+exiting",
       T_META_TIMEOUT(120), T_META_TAG_VM_PREFERRED) {
  char path[PATH_MAX];
  ...
  uint64_t stop_time =
      clock_gettime_nsec_np(CLOCK_UPTIME_RAW) + TEST_DURATION_NS;

  dispatch_queue_t stackshot_queue =
  ...
  // <rdar://problem/39739547> META option for T_HELPER_DECL to not output test
  // begin on start
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(posix_spawn_file_actions_addopen(
  ...
  while (clock_gettime_nsec_np(CLOCK_UPTIME_RAW) < stop_time) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(sp_ret, "spawned process '%s' with PID %d", args[0],
                        pid);
    ...
    if (children_unreaped >= REAP_INTERVAL) {
      while (children_unreaped) {
        T_QUIET;
        T_ASSERT_POSIX_SUCCESS(waitpid(-1, &status, 0),
                               "waitpid returned child pid");
        children_unreaped--;
      }
    }
