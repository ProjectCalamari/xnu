...
T_DECL(so_linger_negative, "SO_LINGER negative") {
  int s = -1;
  T_ASSERT_POSIX_SUCCESS(s = socket(AF_LOCAL, SOCK_DGRAM, 0),
  ...
  T_EXPECT_EQ(set_l.l_linger, get_l.l_linger,
              "SO_LINGER negative l_linger %d == %d", set_l.l_linger,
              get_l.l_linger);
}
...
T_DECL(so_linger_500, "SO_LINGER 500") {
  ...
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("kern.clockrate", &clkinfo, &oldlen, NULL, 0),
  ...
  T_EXPECT_EQ(set_l.l_linger, get_l.l_linger, "SO_LINGER 500 l_linger %d == %d",
              set_l.l_linger, get_l.l_linger);
}
...
T_DECL(so_linger_sec_negative, "SO_LINGER_SEC negative") {
  ...
  set_l.l_onoff = 1;
  set_l.l_linger = -1;
  T_LOG("l_linger %d * clkinfo.hz %d = %d", set_l.l_linger, clkinfo.hz,
        (set_l.l_linger * clkinfo.hz));

  T_ASSERT_POSIX_SUCCESS(
      setsockopt(s, SOL_SOCKET, SO_LINGER_SEC, &set_l, sizeof(struct linger)),
      "setsockopt SO_LINGER_SEC");

  struct linger get_l = {};
  socklen_t len = sizeof(struct linger);
  T_ASSERT_POSIX_SUCCESS(getsockopt(s, SOL_SOCKET, SO_LINGER_SEC, &get_l, &len),
                         "getsockopt SO_LINGER_SEC");

  T_EXPECT_EQ(set_l.l_linger, get_l.l_linger,
              "SO_LINGER_SEC negative l_linger %d == %d", set_l.l_linger,
              get_l.l_linger);
}
...
T_DECL(so_linger_sec_5_seconds, "SO_LINGER_SEC 5 seconds") {
  ...
  T_EXPECT_EQ(set_l.l_linger, get_l.l_linger,
              "SO_LINGER_SEC 5 seconds l_linger %d == %d", set_l.l_linger,
              get_l.l_linger);
}
