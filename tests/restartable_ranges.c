...
static void noop_signal(int signo __unused) {}

static void *task_restartable_ranges_thread(void *_ctx) {
  int *stepp = _ctx;
...
static void wait_for_step(int which) {
...
static void *arena_alloc(void) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(mprotect(p, PAGE_MAX_SIZE, PROT_READ | PROT_WRITE),
                         "arena_alloc");
...
static void arena_free(void *p) {
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(mprotect(p, PAGE_MAX_SIZE, PROT_NONE), "arena_free");
}

static void task_restartable_ranges_race_fail(int signo) {
  T_FAIL("test crashed with signal %s after %d syncs", strsignal(signo), syncs);
  T_END;
}
...
T_DECL(task_restartable_ranges_race, "test for 88873668") {
  ...
  dispatch_async_f(dispatch_get_global_queue(QOS_CLASS_BACKGROUND, 0), obj,
                   fake_msgSend);

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      mprotect(&arena[1], (N_BUCKETS - 1) * PAGE_MAX_SIZE, PROT_NONE),
      "arena_init");
