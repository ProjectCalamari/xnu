...
static size_t ptoa(size_t num_pages) {
  static size_t page_size = 0;
  int ret;
  size_t page_size_size = sizeof(page_size);
  if (page_size == 0) {
    ret = sysctlbyname(g_sysctl_vm_page_size_name, &page_size, &page_size_size,
                       NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "Unable to get page size");
  }
...
T_DECL(global_no_user_wire_amount, "no_user_wire_amount <= 32G",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "no_user_wire sysctl failed");
  T_QUIET;
  T_EXPECT_LE(no_wire, 32 * 2ULL << 30, "no_user_wire_amount is too big.");
}
...
T_DECL(user_wire_amount, "max_mem > user_wire_amount >= 0.7 * max_mem",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "memsize sysctl failed");
  ret = sysctlbyname(g_sysctl_wire_name, &wire, &wire_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "user_wire sysctl failed");
  T_QUIET;
  T_ASSERT_LT(wire, max_mem, "wire limit is too big");
  T_QUIET;
  T_ASSERT_GE(wire, max_mem * 70 / 100, "wire limit is too small.");
}
 ...
 * changes correctly.
 */
static void set_no_wire_limit(vm_map_size_t value, uint64_t max_mem) {
  vm_map_size_t wire;
  size_t wire_size = sizeof(wire);
  int ret;
  ret = sysctlbyname(g_sysctl_no_wire_name, NULL, 0, &value, sizeof(value));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "no_user_wire sysctl set failed");
  ret = sysctlbyname(g_sysctl_wire_name, &wire, &wire_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "user_wire sysctl failed");
  T_QUIET;
  T_ASSERT_EQ(max_mem - wire, value, "no wire size is incorrect");
}
 ...
 * changes correctly.
 */
static void set_wire_limit(vm_map_size_t value, uint64_t max_mem) {
  vm_map_size_t no_wire;
  size_t no_wire_size = sizeof(no_wire);
  int ret;
  ret = sysctlbyname(g_sysctl_wire_name, NULL, 0, &value, sizeof(value));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "user_wire sysctl set failed");
  ret = sysctlbyname(g_sysctl_no_wire_name, &no_wire, &no_wire_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "no_user_wire sysctl failed");
  T_QUIET;
  T_ASSERT_EQ(max_mem - value, no_wire, "no wire size is incorrect");
}

T_DECL(set_global_no_user_wire_amount,
       "Setting no_user_wire_amount changes global_user_wire_amount",
  ...
  ret = sysctlbyname(g_sysctl_memsize_name, &max_mem, &max_mem_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "max_mem sysctl failed");
  ret = sysctlbyname(g_sysctl_no_wire_name, &no_wire, &no_wire_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "no_user_wire sysctl failed");
  ret = sysctlbyname(g_sysctl_wire_name, &wire, &wire_size, NULL, 0);
  ...
  T_ASSERT_POSIX_SUCCESS(ret, "user_wire sysctl failed");
  T_QUIET;
  T_ASSERT_EQ(max_mem - wire, no_wire, "no wire size is incorrect");
...
T_DECL(set_user_wire_limit, "Set user_wire_limit", T_META_ASROOT(true),
  ...
  ret = sysctlbyname(g_sysctl_per_task_wire_name, &original_wire, &wire_size,
                     NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "user_wire sysctl get failed");
  ...
  T_ASSERT_POSIX_SUCCESS(ret, "user_wire sysctl get failed");
  T_QUIET;
  T_ASSERT_EQ(wire, original_wire + wire_delta,
              "user_wire sysctl didn't set the correct value.");

  // Cleanup
  ret = sysctlbyname(g_sysctl_per_task_wire_name, NULL, 0, &original_wire,
                     wire_size);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "user_wire sysctl set failed");
}
 ...
 */
static void *wire_to_limit(size_t limit, size_t *size) {
...
#endif /* __x86_64__ */
  while (true) {
    ret = sysctlbyname(g_sysctl_current_wired_count_name, &current_wired,
                       &current_wired_size, NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "get current wired count failed");
#if __x86_64__
    ret = sysctlbyname(g_sysctl_current_free_count_name, &current_free,
                       &current_free_size, NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "get current free count failed");
#else
    current_free = 0;
#endif /* __x86_64__ */
    offset_from_limit = ptoa(current_wired + current_free + wiggle_room_pages);
    T_QUIET;
    T_ASSERT_GE(limit, offset_from_limit,
                "more pages are wired than the limit.");
    buffer_size = limit - offset_from_limit;
    buffer = malloc(buffer_size);
    T_QUIET;
    T_ASSERT_NOTNULL(buffer, "Unable to allocate buffer");
...
T_DECL(wire_stress_test,
  ...
  ret =
      sysctlbyname(g_sysctl_wire_name, &wire_limit, &wire_limit_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "user_wire sysctl failed");
  buffer = wire_to_limit(wire_limit, &buffer_size);
  ret = clock_gettime(CLOCK_MONOTONIC, &start);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "Unable to get current time.");
  while (true) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "Unable to unlock memory.");
  free(buffer);
}
#endif /* TARGET_OS_OSX */
