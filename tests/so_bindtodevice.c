...
static void test_so_bindtodevice(int domain, int type, int proto) {
  ...
  T_ASSERT_POSIX_SUCCESS(
      getsockopt(fd, boundif_level, boundif_name, &intval, &len),
  ...
  len = sizeof(ifname);
  T_ASSERT_POSIX_SUCCESS(
      getsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, ifname, &len),
  ...
  intval = (int)ifindex;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(fd, boundif_level, boundif_name, &intval, len),
  ...
  len = IFNAMSIZ;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, ifname, len), NULL);
  T_LOG("set SO_BINDTODEVICE `\"\"` %s: len %d", boundif_str, intval);

  T_ASSERT_POSIX_SUCCESS(
      getsockopt(fd, boundif_level, boundif_name, &intval, &len), NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, ifname, len),
  ...
  T_ASSERT_POSIX_FAILURE(
      setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, ifname, len), EINVAL,
      "set SO_BINDTODEVICE: \"%s\", len %u (IFNAMSIZ + 1) expected EINVAL",
      ifname, len);
#endif
}
