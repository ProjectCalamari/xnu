 ...
 *
 * Also test that suspended-spawn correctly looks like a SIGSTOP while it's
 * suspended.
 *
 * <rdar://problem/26184412> posix_spawn non-exec with
 * POSIX_SPAWN_START_SUSPENDED, then killing instead of SIGCONT-ing causes
 * unkillable hung processes
 */

static void spawn_and_signal(int signal) {
  ...
  if (signal == SIGKILL) {
    T_ASSERT_EQ(WIFSIGNALED(status), 1,
                "child should have exited due to signal");
...
T_DECL(suspended_spawn_continue,
       "Tests spawning a suspended process and continuing it",
       T_META_TIMEOUT(2), T_META_TAG_VM_PREFERRED) {
  spawn_and_signal(SIGCONT);
}

T_DECL(suspended_spawn_kill,
