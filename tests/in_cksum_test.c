/* <rdar://problem/49479689> arm64 os_cpu_in_cksum_mbuf sometimes incorrect with
...
static void log_hexdump(const void *inp, size_t len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    off += (unsigned)snprintf(buf + off, sizeof(buf) - off, " %02x",
                              (((const uint8_t *)inp)[i]) & 0xff);
    if (i % 16 == 15) {
      T_LOG("%s", buf);
    }
  }
 ...
 */
static uint16_t dumb_in_cksum(const uint8_t *buf, size_t len) {
  uint32_t partial = 0;
  while (len > 1) {
    uint16_t val = buf[1];
    val <<= 8;
...
static uint16_t split_in_cksum(const uint8_t *buf, int nsegs,
                               const uint32_t *seglens, const uint8_t *aligns,
                               uint8_t *tmpbuf) {
  uint32_t partial = 0;
...
static void test_checksum(const uint8_t *data, uint32_t len) {
      ...
      for (aligns[1] = 0; aligns[1] < MAXALIGN; aligns[1]++) {
        ...
        if (osum != dsum) {
          ...
          T_LOG("len %d seg[0] %d seg[1] %d align[0] %d align[1] %d\n", len,
                seglens[0], seglens[1], aligns[0], aligns[1]);
        }
        T_QUIET;
        T_ASSERT_EQ(osum, dsum, "checksum mismatch got 0x%04x expecting 0x%04x",
                    htons(osum), htons(dsum));
      }
    }
  }
  T_PASS("OK len %d", len);
...
static void test_one_random_packet(uint32_t maxlen) {
 ...
 * It is known to cause a problem at splits 44 and 46 with second alignment of 1
 * or 3
 */
...
T_DECL(in_cksum_49479689a,
       "tests os_cpu_in_cksum with known problem packet in various random "
       "segmentation and memory alignment",
...
T_DECL(in_cksum_49479689b,
       "tests os_cpu_in_cksum with many random packets in various random "
       "segmentation and memory alignment",
       T_META_TAG_VM_NOT_PREFERRED) {
  for (int i = 0; i < 100; i++) {
