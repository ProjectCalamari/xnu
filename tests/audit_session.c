...
#define VALID_AU_SESSION_FLAGS                                                 \
  (AU_SESSION_FLAG_IS_INITIAL | AU_SESSION_FLAG_HAS_GRAPHIC_ACCESS |           \
   AU_SESSION_FLAG_HAS_TTY | AU_SESSION_FLAG_IS_REMOTE |                       \
   AU_SESSION_FLAG_HAS_CONSOLE_ACCESS | AU_SESSION_FLAG_HAS_AUTHENTICATED)

T_GLOBAL_META(T_META_RUN_CONCURRENTLY(true),
              T_META_NAMESPACE("xnu.audit.session"));

static void get_asid_auid(au_asid_t *asidp, au_id_t *auidp) {
  audit_token_t token = INVALID_AUDIT_TOKEN_VALUE;
  mach_msg_type_number_t count = TASK_AUDIT_TOKEN_COUNT;
  T_ASSERT_MACH_SUCCESS(task_info(mach_task_self(), TASK_AUDIT_TOKEN,
                                  (task_info_t)&token, &count),
                        "obtain audit token for self");
  T_LOG("Task audit token for pid %d has asid %d auid %d", (int)getpid(),
        (int)token.val[6], (int)token.val[0]);
  if (asidp) {
...
static void tlog_aia(struct auditinfo_addr *aiap, const char *label) {
  T_LOG("%s:", label);
...
T_DECL(getaudit_addr, "getaudit_addr smoke test") {
  ...
  T_EXPECT_BITS_NOTSET(aia.ai_flags, ~(au_asflgs_t)VALID_AU_SESSION_FLAGS,
                       NULL);
  T_EXPECT_BITS_SET(aia.ai_flags,
                    (au_asflgs_t)AU_SESSION_FLAG_HAS_AUTHENTICATED, NULL);
}
...
T_DECL(auditon_getsflags, "auditon(A_GETSFLAGS) smoke test") {
  ...
  T_EXPECT_BITS_SET(flags, (au_asflgs_t)AU_SESSION_FLAG_HAS_AUTHENTICATED,
                    NULL);
}
...
T_DECL(auditon_getpinfo_addr, "auditon(A_GETPINFO_ADDR) smoke test") {
  ...
  T_ASSERT_POSIX_SUCCESS(rv_from_auditon,
                         "auditon(2) A_GETPINFO_ADDR succeeds");
  ...
  // any ap_mask
  // any ap_termid
  T_EXPECT_BITS_NOTSET(apia.ap_flags, ~(au_asflgs_t)VALID_AU_SESSION_FLAGS,
                       NULL);
  T_EXPECT_BITS_SET(apia.ap_flags,
                    (au_asflgs_t)AU_SESSION_FLAG_HAS_AUTHENTICATED, NULL);
}
...
T_DECL(auditon_getsinfo_addr, "auditon(A_GETSINFO_ADDR) smoke test") {
  ...
  T_ASSERT_POSIX_SUCCESS(rv_from_auditon,
                         "auditon(2) A_GETSINFO_ADDR succeeds");
  T_EXPECT_EQ(aia.ai_asid, asid, NULL);
  T_EXPECT_EQ(aia.ai_auid, auid, NULL);
  T_EXPECT_NE(aia.ai_auid, AU_DEFAUDITID, NULL);
  // any ap_mask
...
T_DECL(auditon_getsinfo_addr_rootasid,
       "auditon(A_GETSINFO_ADDR) for root session") {
...
T_DECL(auditon_getsinfo_addr_asid1, "auditon(A_GETSINFO_ADDR) for asid 1") {
  ...
  T_ASSERT_POSIX_FAILURE(rv_from_auditon, EINVAL,
                         "auditon(2) A_GETSINFO_ADDR fails");
}
...
// fact denied.
static void new_session_flow(au_asid_t asid, enum termid_mode termid_mode,
                             uint32_t termid_type, enum auid_mode auid_mode) {
  if (geteuid() != 0) {
  ...
  struct auditinfo_addr aia1b =
      {}; // passed into setaudit_addr, potentially modified
  ...
  // For unknown reasons, AU_SESSION_FLAG_HAS_TTY cannot be set after session
  // creation, but the terminal ID can, which seems inconsistent.
  ...
  T_ASSERT_POSIX_SUCCESS(rv_from_setaudit_addr,
                         "setaudit_addr(2) succeeds at creating a new session");
  ...
  // Don't check other fields of aia1b, the contract is only well-defined for
  // the asid.

  T_ASSERT_POSIX_SUCCESS(
      getaudit_addr(&aia1c, sizeof(aia1c)),
      "getaudit_addr(2) succeeds at obtaining new session aia");
  ...
  T_ASSERT_POSIX_SUCCESS(auditon(A_GETSFLAGS, &flags1c, sizeof(flags1c)),
                         "auditon(2) A_GETSFLAGS succeeds");
  ...
  if (termid_mode != TERMIDM_NOUPDATE) {
    ...
    T_ASSERT_POSIX_SUCCESS(
        setaudit_addr(&aia2b, sizeof(aia2b)),
        "setaudit_addr(2) succeeds at updating the session with a terminal ID");
    tlog_aia(&aia2b, "aia2b");
    T_EXPECT_EQ(aia2b.ai_asid, aia2a.ai_asid, NULL);
    // Don't check other fields of aia2b, the contract is only well-defined for
    // the asid.

    T_ASSERT_POSIX_SUCCESS(
        getaudit_addr(&aia2c, sizeof(aia2c)),
        "getaudit_addr(2) succeeds at obtaining updated session aia");
  ...
  if (auid_mode != AUIDM_NOUPDATE) {
    if (auid_mode == AUIDM_UPDATE_SETAUDIT_ADDR) {
      ...
      T_ASSERT_POSIX_SUCCESS(
          setaudit_addr(&aia3b, sizeof(aia3b)),
          "setaudit_addr(2) succeeds at updating the session as authenticated");
      tlog_aia(&aia3b, "aia3b");
      T_EXPECT_EQ(aia3b.ai_asid, aia3a.ai_asid, NULL);
      // Don't check other fields of aia3b, the contract is only well-defined
      // for the asid.
    } else {
      ...
      T_ASSERT_POSIX_SUCCESS(auditon(A_SETPMASK, &api, sizeof(api)),
                             "auditon(2) A_SETPMASK succeeds");

      struct auditinfo_addr new_aia = {};
      T_ASSERT_POSIX_SUCCESS(
          getaudit_addr(&new_aia, sizeof(new_aia)),
          "getaudit_addr(2) after auditon(2) A_SETPMASK succeeds");
      ...
      T_EXPECT_EQ(new_aia.ai_termid.at_addr[0], aia2c.ai_termid.at_addr[0],
                  NULL);
      T_EXPECT_EQ(new_aia.ai_termid.at_addr[1], aia2c.ai_termid.at_addr[1],
                  NULL);
      T_EXPECT_EQ(new_aia.ai_termid.at_addr[2], aia2c.ai_termid.at_addr[2],
                  NULL);
      T_EXPECT_EQ(new_aia.ai_termid.at_addr[3], aia2c.ai_termid.at_addr[3],
                  NULL);
      ...
      T_ASSERT_POSIX_SUCCESS(
          setauid(&new_auid),
          "setauid(2) succeeds at updating the auid of the session");
      T_EXPECT_EQ(new_auid, test_auid_a, NULL);

      T_ASSERT_POSIX_SUCCESS(getaudit_addr(&new_aia, sizeof(new_aia)),
                             "getaudit_addr(2) after setauid(2) succeeds");
      tlog_aia(&new_aia, "new_aia");
      T_EXPECT_EQ(new_aia.ai_asid, aia2c.ai_asid, NULL);
      ...
      T_ASSERT_POSIX_SUCCESS(
          auditon(A_GETSFLAGS, &new_flags, sizeof(new_flags)),
          "auditon(2) A_GETSFLAGS succeeds");
      T_EXPECT_EQ(new_flags, flags1c, NULL);
      new_flags |= AU_SESSION_FLAG_HAS_AUTHENTICATED;
      T_ASSERT_POSIX_SUCCESS(
          auditon(A_SETSFLAGS, &new_flags, sizeof(new_flags)),
          "auditon(2) A_SETSFLAGS succeeds");
    ...
    T_ASSERT_POSIX_SUCCESS(
        getaudit_addr(&aia3c, sizeof(aia3c)),
        "getaudit_addr(2) succeeds at obtaining updated session aia");
  ...
  T_ASSERT_POSIX_FAILURE(setaudit_addr(&aia4b, sizeof(aia4b)), EINVAL,
                         "setaudit_addr(2) refuses changing auid once set");
  ...
  T_ASSERT_POSIX_FAILURE(setauid(&new_auid), EINVAL,
                         "setauid(2) refuses changing auid once set");
  ...
  T_ASSERT_POSIX_FAILURE(
      setaudit_addr(&aia5b, sizeof(aia5b)), EINVAL,
      "setaudit_addr(2) refuses changing termid port once set");
  ...
  aia6a.ai_termid.at_type =
      aia6a.ai_termid.at_type == AU_IPv4 ? AU_IPv6 : AU_IPv4;
  struct auditinfo_addr aia6b = {};
  bcopy(&aia6a, &aia6b, sizeof(aia6b));
  T_ASSERT_POSIX_FAILURE(
      setaudit_addr(&aia6b, sizeof(aia6b)), EINVAL,
      "setaudit_addr(2) refuses changing termid type once set");
  ...
  T_ASSERT_POSIX_FAILURE(
      setaudit_addr(&aia7b, sizeof(aia7b)), EINVAL,
      "setaudit_addr(2) refuses changing termid addr once set");
  ...
  T_ASSERT_POSIX_FAILURE(
      setaudit_addr(&aia8b, sizeof(aia8b)), EINVAL,
      "setaudit_addr(2) refuses changing protected flags once set");
  ...
  au_asflgs_t new_flags = -1UL;
  T_ASSERT_POSIX_SUCCESS(auditon(A_GETSFLAGS, &new_flags, sizeof(new_flags)),
                         "auditon(2) A_GETSFLAGS succeeds");
  T_EXPECT_BITS_SET(new_flags, AU_SESSION_FLAG_IS_REMOTE, NULL);
  new_flags &= ~(au_asflgs_t)AU_SESSION_FLAG_IS_REMOTE;
  T_ASSERT_POSIX_FAILURE(
      auditon(A_SETSFLAGS, &new_flags, sizeof(new_flags)), EINVAL,
      "auditon(2) A_SETSFLAGS refuses changing protected flags once set");
  ...
  if (termid_type == AU_IPv4) {
    T_ASSERT_POSIX_SUCCESS(
        auditon(A_GETPINFO, &api, sizeof(api)),
        "auditon(2) A_GETPINFO succeeds for IPv4 terminal ID");
    ...
    T_ASSERT_POSIX_FAILURE(auditon(A_GETPINFO, &api, sizeof(api)), EINVAL,
                           "auditon(2) A_GETPINFO fails for IPv6 terminal ID");
  }
  ...
  auditpinfo_addr_t apia = {};
  apia.ap_pid = getpid();
  T_ASSERT_POSIX_SUCCESS(auditon(A_GETPINFO_ADDR, &apia, sizeof(apia)),
                         "auditon(2) A_GETPINFO_ADDR succeeds");
  T_EXPECT_EQ(apia.ap_pid, getpid(), NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(auditon(A_GETSINFO_ADDR, &aia9, sizeof(aia9)),
                         "auditon(2) A_GETSINFO_ADDR succeeds");
  tlog_aia(&aia9, "aia9");
  T_EXPECT_EQ(aia9.ai_asid, aia3c.ai_asid, NULL);
  T_EXPECT_EQ(aia9.ai_auid, aia3c.ai_auid, NULL);
  // Masks on the session without process context are undefined, don't check
  // them
...
// auid_mode = { AUIDM_NOUPDATE | AUIDM_UPDATE_SETAUDIT_ADDR |
// AUIDM_UPDATE_SETAUID }

T_DECL(new_session_1141,
       "new session asid=AU_ASSIGN_ASID termid=noupdate IPv4 auid=noupdate") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_NOUPDATE, AU_IPv4, AUIDM_NOUPDATE);
}

T_DECL(
    new_session_1142,
    "new session asid=AU_ASSIGN_ASID termid=noupdate IPv4 auid=setaudit_addr") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_NOUPDATE, AU_IPv4,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}

T_DECL(new_session_1143,
       "new session asid=AU_ASSIGN_ASID termid=noupdate IPv4 auid=setauid") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_NOUPDATE, AU_IPv4,
                   AUIDM_UPDATE_SETAUID);
}

T_DECL(new_session_1161,
       "new session asid=AU_ASSIGN_ASID termid=noupdate IPv6 auid=noupdate") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_NOUPDATE, AU_IPv6, AUIDM_NOUPDATE);
}

T_DECL(
    new_session_1162,
    "new session asid=AU_ASSIGN_ASID termid=noupdate IPv6 auid=setaudit_addr") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_NOUPDATE, AU_IPv6,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}

T_DECL(new_session_1163,
       "new session asid=AU_ASSIGN_ASID termid=noupdate IPv6 auid=setauid") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_NOUPDATE, AU_IPv6,
                   AUIDM_UPDATE_SETAUID);
}
...
T_DECL(
    new_session_1241,
    "new session asid=AU_ASSIGN_ASID termid=setaudit_addr IPv4 auid=noupdate") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_NOUPDATE);
}

T_DECL(new_session_1242, "new session asid=AU_ASSIGN_ASID termid=setaudit_addr "
                         "IPv4 auid=setaudit_addr") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}
...
T_DECL(
    new_session_1243,
    "new session asid=AU_ASSIGN_ASID termid=setaudit_addr IPv4 auid=setauid") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_UPDATE_SETAUID);
}
...
T_DECL(
    new_session_1261,
    "new session asid=AU_ASSIGN_ASID termid=setaudit_addr IPv6 auid=noupdate") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_NOUPDATE);
}

T_DECL(new_session_1262, "new session asid=AU_ASSIGN_ASID termid=setaudit_addr "
                         "IPv6 auid=setaudit_addr") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}
...
T_DECL(
    new_session_1263,
    "new session asid=AU_ASSIGN_ASID termid=setaudit_addr IPv6 auid=setauid") {
  new_session_flow(AU_ASSIGN_ASID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_UPDATE_SETAUID);
}

T_DECL(new_session_2141,
       "new session asid=AU_DEFAUDITSID termid=noupdate IPv4 auid=noupdate") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_NOUPDATE, AU_IPv4, AUIDM_NOUPDATE);
}

T_DECL(
    new_session_2142,
    "new session asid=AU_DEFAUDITSID termid=noupdate IPv4 auid=setaudit_addr") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_NOUPDATE, AU_IPv4,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}

T_DECL(new_session_2143,
       "new session asid=AU_DEFAUDITSID termid=noupdate IPv4 auid=setauid") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_NOUPDATE, AU_IPv4,
                   AUIDM_UPDATE_SETAUID);
}

T_DECL(new_session_2161,
       "new session asid=AU_DEFAUDITSID termid=noupdate IPv6 auid=noupdate") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_NOUPDATE, AU_IPv6, AUIDM_NOUPDATE);
}

T_DECL(
    new_session_2162,
    "new session asid=AU_DEFAUDITSID termid=noupdate IPv6 auid=setaudit_addr") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_NOUPDATE, AU_IPv6,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}

T_DECL(new_session_2163,
       "new session asid=AU_DEFAUDITSID termid=noupdate IPv6 auid=setauid") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_NOUPDATE, AU_IPv6,
                   AUIDM_UPDATE_SETAUID);
}
...
T_DECL(
    new_session_2241,
    "new session asid=AU_DEFAUDITSID termid=setaudit_addr IPv4 auid=noupdate") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_NOUPDATE);
}

T_DECL(new_session_2242, "new session asid=AU_DEFAUDITSID termid=setaudit_addr "
                         "IPv4 auid=setaudit_addr") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}
...
T_DECL(
    new_session_2243,
    "new session asid=AU_DEFAUDITSID termid=setaudit_addr IPv4 auid=setauid") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_UPDATE_SETAUID);
}
...
T_DECL(
    new_session_2261,
    "new session asid=AU_DEFAUDITSID termid=setaudit_addr IPv6 auid=noupdate") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_NOUPDATE);
}

T_DECL(new_session_2262, "new session asid=AU_DEFAUDITSID termid=setaudit_addr "
                         "IPv6 auid=setaudit_addr") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}
...
T_DECL(
    new_session_2263,
    "new session asid=AU_DEFAUDITSID termid=setaudit_addr IPv6 auid=setauid") {
  new_session_flow(AU_DEFAUDITSID, TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_UPDATE_SETAUID);
}

T_DECL(new_session_3141,
       "new session asid=getpid() termid=noupdate IPv4 auid=noupdate") {
  new_session_flow(getpid(), TERMIDM_NOUPDATE, AU_IPv4, AUIDM_NOUPDATE);
}

T_DECL(new_session_3142,
       "new session asid=getpid() termid=noupdate IPv4 auid=setaudit_addr") {
  new_session_flow(getpid(), TERMIDM_NOUPDATE, AU_IPv4,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}

T_DECL(new_session_3143,
       "new session asid=getpid() termid=noupdate IPv4 auid=setauid") {
  new_session_flow(getpid(), TERMIDM_NOUPDATE, AU_IPv4, AUIDM_UPDATE_SETAUID);
}

T_DECL(new_session_3161,
       "new session asid=getpid() termid=noupdate IPv6 auid=noupdate") {
  new_session_flow(getpid(), TERMIDM_NOUPDATE, AU_IPv6, AUIDM_NOUPDATE);
}

T_DECL(new_session_3162,
       "new session asid=getpid() termid=noupdate IPv6 auid=setaudit_addr") {
  new_session_flow(getpid(), TERMIDM_NOUPDATE, AU_IPv6,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}

T_DECL(new_session_3163,
       "new session asid=getpid() termid=noupdate IPv6 auid=setauid") {
  new_session_flow(getpid(), TERMIDM_NOUPDATE, AU_IPv6, AUIDM_UPDATE_SETAUID);
}

T_DECL(new_session_3241,
       "new session asid=getpid() termid=setaudit_addr IPv4 auid=noupdate") {
  new_session_flow(getpid(), TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_NOUPDATE);
}

T_DECL(
    new_session_3242,
    "new session asid=getpid() termid=setaudit_addr IPv4 auid=setaudit_addr") {
  new_session_flow(getpid(), TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}

T_DECL(new_session_3243,
       "new session asid=getpid() termid=setaudit_addr IPv4 auid=setauid") {
  new_session_flow(getpid(), TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv4,
                   AUIDM_UPDATE_SETAUID);
}

T_DECL(new_session_3261,
       "new session asid=getpid() termid=setaudit_addr IPv6 auid=noupdate") {
  new_session_flow(getpid(), TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_NOUPDATE);
}

T_DECL(
    new_session_3262,
    "new session asid=getpid() termid=setaudit_addr IPv6 auid=setaudit_addr") {
  new_session_flow(getpid(), TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_UPDATE_SETAUDIT_ADDR);
}

T_DECL(new_session_3263,
       "new session asid=getpid() termid=setaudit_addr IPv6 auid=setauid") {
  new_session_flow(getpid(), TERMIDM_UPDATE_SETAUDIT_ADDR, AU_IPv6,
                   AUIDM_UPDATE_SETAUID);
}
...
T_HELPER_DECL(check_child_session, "Check child aia against file") {
  ...
  snprintf(path, MAXPATHLEN, "%s/" NEW_SESSION_CHECK_CHILD_FILENAME,
           dt_tmpdir());
  ...
  struct auditinfo_addr aia;
  T_ASSERT_POSIX_SUCCESS(getaudit_addr(&aia, sizeof(aia)),
                         "getaudit_addr(2) succeeds");
  ...
  T_EXPECT_EQ(aia.ai_termid.at_addr[0], expected_aia.ai_termid.at_addr[0],
              NULL);
  T_EXPECT_EQ(aia.ai_termid.at_addr[1], expected_aia.ai_termid.at_addr[1],
              NULL);
  T_EXPECT_EQ(aia.ai_termid.at_addr[2], expected_aia.ai_termid.at_addr[2],
              NULL);
  T_EXPECT_EQ(aia.ai_termid.at_addr[3], expected_aia.ai_termid.at_addr[3],
              NULL);
...
T_DECL(new_session_check_child_aia,
       "new session is inherited by child processes") {
...
T_HELPER_DECL(child_create_session, "Create a session in a child process") {
  ...
  snprintf(tmppath, MAXPATHLEN, "%s/" NEW_SESSION_CLEANUP_FILENAME "~",
           dt_tmpdir());
  int fd = open(tmppath, O_CREAT | O_TRUNC | O_RDWR, 0755);
  T_ASSERT_POSIX_SUCCESS(fd, "open %s by pid %d for writing", tmppath,
                         getpid());
  ssize_t bytes_written = write(fd, &aia, sizeof(aia));
  T_ASSERT_EQ(bytes_written, (ssize_t)sizeof(aia), NULL);
  pid_t pid = getpid();
  ...
  // Atomically move it into place so that we can reliably for it over in the
  // other helper.
  char path[MAXPATHLEN];
  snprintf(path, MAXPATHLEN, "%s/" NEW_SESSION_CLEANUP_FILENAME, dt_tmpdir());
  T_ASSERT_POSIX_SUCCESS(rename(tmppath, path), "move %s to %s by pid %d",
                         tmppath, path, getpid());

  T_END;
}

T_HELPER_DECL(child_session_disappeared,
              "Check that session created in other helper disappeared") {
  ...
  T_ASSERT_POSIX_FAILURE(auditon(A_GETSINFO_ADDR, &aia, sizeof(aia)), EINVAL,
                         "auditon(2) A_GETSINFO_ADDR cannot find the session");
...
T_DECL(audit_session_self, "audit_session_self(2) smoke test") {
  ...
  T_ASSERT_TRUE(MACH_PORT_VALID(session_port),
                "audit_session_self(2) returns valid send right");
...
static void audit_session_join_cleanup(void) {
...
T_DECL(audit_session_join,
       "audit_session_join(2) and port/session lifecycle test") {
  int cond, rv_from_auditon = auditon(A_GETCOND, &cond, sizeof(cond));
  if (rv_from_auditon == -1 && errno == ENOSYS) {
    T_SKIP("Kernel support for auditon(2) not available");
  }
  ...
  T_ASSERT_POSIX_SUCCESS(
      audit_session_port(original_asid, &original_session_port),
      "audit_session_port(2) succeeds");
  T_ASSERT_TRUE(MACH_PORT_VALID(original_session_port), NULL);

  // Join original session
  T_ASSERT_POSIX_SUCCESS(audit_session_join(original_session_port),
                         "audit_session_join(2) succeeds");
  au_asid_t asid;
  au_id_t auid;
  get_asid_auid(&asid, &auid);
  ...
  T_ASSERT_POSIX_SUCCESS(
      auditon(A_GETSINFO_ADDR, &aia, sizeof(aia)),
      "auditon(2) A_GETSINFO_ADDR can still find the new session");
  ...
  T_ASSERT_POSIX_SUCCESS(audit_session_join(new_session_port),
                         "audit_session_join(2) succeeds");
  get_asid_auid(&asid, &auid);
  ...
  T_ASSERT_POSIX_FAILURE(
      auditon(A_GETSINFO_ADDR, &aia, sizeof(aia)), EINVAL,
      "auditon(2) A_GETSINFO_ADDR cannot find the new session");
}
...
T_DECL(setaudit_addr_join, "join session the BSD way using setaudit_addr(2)") {
  ...
  T_ASSERT_POSIX_SUCCESS(auditon(A_GETSINFO_ADDR, &aia, sizeof(aia)),
                         "auditon(2) A_GETSINFO_ADDR can find the old session");
  T_ASSERT_EQ(aia.ai_asid, original_asid, NULL);
  T_ASSERT_EQ(aia.ai_auid, original_auid, NULL);

  // Switch back to original session
  T_ASSERT_POSIX_SUCCESS(
      setaudit_addr(&aia, sizeof(aia)),
      "setaudit_addr(2) succeeds at joining the original session");
