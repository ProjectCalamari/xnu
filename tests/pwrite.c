...
T_GLOBAL_META(T_META_NAMESPACE("xnu.syscall.pwrite"),
              T_META_RUN_CONCURRENTLY(true));

static void test_file_equals(int fd, const char *expected_buffer, size_t size) {
  ...
  T_ASSERT_EQ(read(fd, read_buffer, size), (ssize_t)size,
              "Read expected buffer");
  T_ASSERT_EQ(strncmp(read_buffer, expected_buffer, size), 0,
              "Buffer as expected");
  free(read_buffer);
}

T_DECL(pwrite_regular_file, "test pwrite() on a regular file.") {
...
static void test_pwrite_should_fail(int fd, int expected_errno) {
  char input_buffer = 'A';
...
T_DECL(pwrite_socket, "test pwrite() on a socket.") {
  int sockets[2];
...
T_DECL(pwrite_unix_shared_memory, "test pwrite() on unix shared memory.") {
...
T_DECL(pwrite_kqueue, "test pwrite() on kqueue.") {
  int queue = kqueue();
...
T_DECL(pwrite_pipe, "test pwrite() on pipe.") {
  int pipe_fds[2];
...
T_DECL(pwrite_write_to_dev_null, "test pwrite() from null.") {
