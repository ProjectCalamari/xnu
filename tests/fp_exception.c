...
T_GLOBAL_META(T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_OWNER("devon_andrade"), T_META_RUN_CONCURRENTLY(true),
...
static size_t exc_arithmetic_handler(__unused mach_port_t task,
  ...
  T_ASSERT_EQ(codes_64[0], (mach_exception_data_type_t)EXC_ARM_FP_DZ,
...
T_DECL(armv8_fp_exception, "Test that ARMv8 floating point exceptions generate "
                           "Mach exceptions, verify default FPCR value.") {
#ifndef __arm64__
  ...
  if (fpcr != FPCR_INIT) {
    T_FAIL("The floating point control register has a non-default value"
           "%" PRIx64,
           fpcr);
  }
...
#define DSB_ISH 0xb
  ...
  if (__builtin_arm_rsr64("FPCR") != fpcr_divexc) {
    T_SKIP("Running on a device that doesn't support floating point "
           "exceptions, skipping...");
  }

  /* Check if floating-point exceptions are enabled */
  kernel_bootargs_len = sizeof(kernel_bootargs);
  kern_return_t kr = sysctlbyname("kern.bootargs", kernel_bootargs,
                                  &kernel_bootargs_len, NULL, 0);
  if (kr != 0) {
  ...
  /**
   * This should cause a floating point divide-by-zero exception to get
   * triggered.
   *
