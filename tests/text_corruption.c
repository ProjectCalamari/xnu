 ...
 * - call atan
 *
 * The first and last of each should exit normally. The middle one should exit
 * with SIGILL.
 *
...
T_DECL(text_corruption_recovery, "test detection/recovery of text corruption",
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, SIGILL, "Call of corrupted foo");
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, SIGILL, "Call of corrupted atan");
