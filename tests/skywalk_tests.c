...
T_GLOBAL_META(T_META_NAMESPACE("xnu.skywalk"),
...
#define BATS_TESTS                                                             \
  ...
  X(oneslotus,                                                                 \
    "test sends one slot of data on user pipe loopback using select")          \
  ...
  X(oneslotuk,                                                                 \
    "test sends one slot of data on user pipe loopback using kqueue")          \
  X(oneslotkk, "test sends one slot of data on kpipe loopback using kqueue")   \
  X(oneslotuk_defunct, "test sends one slot of data on user pipe loopback "    \
                       "using kqueue with one end of the pipe defuncted")      \
  ...
  X(nslotsus,                                                                  \
    "test sends TX_SLOTS of data on user pipe loopback using select")          \
  ...
  X(nslotsuk,                                                                  \
    "test sends TX_SLOTS of data on user pipe loopback using kqueue")          \
  X(nslotskk, "test sends TX_SLOTS of data on kpipe loopback using kqueue")    \
  X(mslotsus,                                                                  \
    "test sends 1000000 slots of data on user pipe loopback using select")     \
  X(mslotsks,                                                                  \
    "test sends 1000000 slots of data on kpipe loopback using select")         \
  X(mslotsup,                                                                  \
    "test sends 1000000 slots of data on user pipe loopback using poll")       \
  X(mslotskp, "test sends 1000000 slots of data on kpipe loopback using poll") \
  X(mslotsuk,                                                                  \
    "test sends 1000000 slots of data on user pipe loopback using kqueue")     \
  X(mslotskk,                                                                  \
    "test sends 1000000 slots of data on kpipe loopback using kqueue")         \
  ...
  X(kqueue_lowat_chan_bytes,                                                   \
    "test kqueue low watermark (byte watermark on channel)")                   \
  X(kqueue_lowat_chan_slots,                                                   \
    "test kqueue low watermark (slot watermark on channel)")                   \
  X(kqueue_lowat_note_bytes,                                                   \
    "test kqueue low watermark (byte watermark on knote)")                     \
  X(kqueue_lowat_note_slots,                                                   \
    "test kqueue low watermark (slot watermark on knote)")                     \
  ...
  X(debug_verify_u, "test confirms that skywalk is storing checksums of "      \
                    "slots received on a upipe when in SKF_VERIFY debug mode") \
  X(utun27302538a,                                                             \
    "test cleaning up utun kpipe while channel is in kevent (case a)")         \
  X(utun27302538b,                                                             \
    "test cleaning up utun kpipe while channel is in kevent (case b)")         \
  X(utun27302538c,                                                             \
    "test cleaning up utun kpipe while channel is in kevent (case c)")         \
  X(utun27302538d,                                                             \
    "test cleaning up utun kpipe while channel is in kevent (case d)")         \
  ...
  X(netifcompatb, "Test setup and teardown netifcompat on feth0 with "         \
                  "deferred close channel")                                    \
  X(netifcompatc, "Test setup and teardown netifcompat on feth0 with "         \
                  "deferred detach and close channel")                         \
  X(fswloopfsw, "Test creates a flowswitch and attaches it to itself")         \
  X(fswloop2ff,                                                                \
    "Test attaches a flowswitch to a flowswitch without any netif")            \
  ...
  X(reass_default_setting, "UDP fragmentation reassembly (channel flow Rx) "   \
                           "(without forcing ip_reass sysctl)")                \
  X(reass_timeout, "Test send partial fragment to flowswitch and check for "   \
                   "ICMPv6 time exceeded reply")                               \
  X(reass_bad_fraglen,                                                         \
    "Test send fragment with bad fragment length (!= 8*) to flowswitch and "   \
    "check for ICMPv6 param header reply")                                     \
  X(reass_atomic,                                                              \
    "Test send atomic ICMP echo fragment to flowswitch and check for reply")   \
 ...
 *       X(writenfd, "test writing to a guarded nexus fd") due to
 * rdar://133461652
 */
...
#define BATSUTUN_TESTS                                                         \
  ...
  X(utunloopy4u1,                                                              \
    "open 2 utuns with netif and floods ipv4 udp packets in one direction")    \
  X(utunloopy4u2,                                                              \
    "open 2 utuns with netif and floods ipv4 udp packets in two directions")   \
  X(utunloopy4t1,                                                              \
    "open 2 utuns with netif and floods ipv4 tcp packets in one direction")    \
  X(utunloopy4t2,                                                              \
    "open 2 utuns with netif and floods ipv4 tcp packets in two directions")
...
#define SHUTDOWN_TESTS                                                         \
  ...
  X(mslotskk,                                                                  \
    "test sends 1000000 slots of data on kpipe loopback using kqueue")         \
  X(mmslotsus,                                                                 \
    "test sends 10000000 slots of data on user pipe loopback using select")    \
  X(mmslotsks,                                                                 \
    "test sends 10000000 slots of data on kpipe loopback using select")        \
  X(mmslotsup,                                                                 \
    "test sends 10000000 slots of data on user pipe loopback using poll")      \
  X(mmslotskp,                                                                 \
    "test sends 10000000 slots of data on kpipe loopback using poll")          \
  X(mmslotsuk,                                                                 \
    "test sends 10000000 slots of data on user pipe loopback using kqueue")    \
  X(mmslotskk,                                                                 \
    "test sends 10000000 slots of data on kpipe loopback using kqueue")        \
  X(closecfd, "test closing guarded channel fd")                               \
  X(writecfd, "test writing to channel fd")                                    \
  X(readcfd, "test reading from channel fd")                                   \
  X(closenfd, "test closing guarded nexus fd")                                 \
  ...
  X(debug_verify_u, "test confirms that skywalk is storing checksums of "      \
                    "slots received on a upipe when in SKF_VERIFY debug mode") \
  X(debug_verify_k, "test confirms that skywalk is storing checksums of "      \
                    "slots received on a kpipe when in SKF_VERIFY debug mode") \
  X(utun27302538a,                                                             \
    "test cleaning up utun kpipe while channel is in kevent (case a)")         \
  X(utun27302538b,                                                             \
    "test cleaning up utun kpipe while channel is in kevent (case b)")         \
  ...
  X(utunloopy4t2,                                                              \
    "open 2 utuns with netif and floods ipv4 tcp packets in two directions")   \
  X(ipsecloopy4u1,                                                             \
    "open 2 ipsecs with netif and floods ipv4 udp packets in one direction")   \
  X(ipsecloopy4u2,                                                             \
    "open 2 ipsecs with netif and floods ipv4 udp packets in two directions")  \
  X(ipsecloopy4t1,                                                             \
    "open 2 ipsecs with netif and floods ipv4 tcp packets in one direction")   \
  X(ipsecloopy4t2,                                                             \
    "open 2 ipsecs with netif and floods ipv4 tcp packets in two directions")  \
  X(bindupipekey, "test binds a channel to a non-anonymous user pipe nexus")   \
  X(netifcompata, "Test setup and teardown netifcompat on feth0")              \
  X(netifcompatb, "Test setup and teardown netifcompat on feth0 with "         \
                  "deferred close channel")                                    \
  X(netifcompatc, "Test setup and teardown netifcompat on feth0 with "         \
                  "deferred detach and close channel")                         \
  X(teardownr,                                                                 \
    "setup complicated topology tear it down randomly (1000 iterations)")      \
  X(teardownz, "setup complicated topology tear it down with each stage in "   \
               "an out of order position")                                     \
  X(fsw29301703a, "Test open 63 channels to a flowswitch")                     \
  X(fsw29301703b, "Test open 200 channels to a flowswitch")                    \
  X(fsw29301703c, "Open too many channels to a flowswitch")                    \
...
#define RDAR_145328590_FAILING_TESTS                                           \
  ...
  X(flowswitch_ns_reserve,                                                     \
    "test confirms that flowswitches can reserve L4 ports")                    \
  X(flowswitch_ns_reserve2,                                                    \
    "thorough test of netns for both BSD & flowswitch, IPv4/v6")               \
...
#define EXPAND_TO_T_DECL_COMMON(test, desc)                                    \
  {                                                                            \
    ...
    skywalk_test_driver_run(&skt_##test, argc, (skt_##test).skt_argv, memfail, \
                            ignorefail, shutdown, itersecs);                   \
  }
 ...
 */
#define X(test, desc, ...)                                                     \
  T_DECL_REF(memfail_##test, test, desc, T_META_ENVVAR("memfail=127"),         \
...
#define X(test, desc, ...)                                                     \
  T_DECL_REF(pll_memfail_##test, pll_##test, desc,                             \
             T_META_ENVVAR("memfail=127"), T_META_ENVVAR("ignorefail=true"));
BATSPLL_TESTS;
#undef X
 ...
 */
T_DECL_REF(noop_memcleanup, noop,
           "run noop test to cleanup memory failure sysctl",
 ...
 * here we used 1. That's because the legacy tests ran a group of tests, but
 * here it is only for a single test.
 */
#define X(test, desc, ...)                                                     \
  T_DECL(shutdown_##test, desc, T_META_ENVVAR("ignorefail=true"),              \
         T_META_ENVVAR("shutdown=true"), T_META_ENVVAR("itersecs=1"))          \
  EXPAND_TO_T_DECL_COMMON(test, desc)
SHUTDOWN_TESTS;
#undef X
