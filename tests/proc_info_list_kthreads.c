...
T_DECL(
    ...
    T_META_ASROOT(_ASROOT), T_META_CHECK_LEAKS(false), T_META_TAG_VM_PREFERRED,
   ...
   * thread ID. However, there is a TOCTOU race between asking for the thread
   * count and asking for the array of identifiers.
   *
  ...
  while (!thread_count && (attempt < MAX_TRIES)) {
    ...
    T_QUIET;
    T_WITH_ERRNO;
    T_ASSERT_GT(buf_used, 0,
                "proc_pidinfo(PROC_PIDTASKINFO) returned a value > 0");
    T_QUIET;
    T_ASSERT_EQ(buf_used, (int)sizeof(ti),
    ...
    T_QUIET;
    T_WITH_ERRNO;
    T_ASSERT_NOTNULL(thread_list_tmp, "malloc(size = %d) failed",
                     allocated_size);

    buf_used = proc_pidinfo(0, PROC_PIDLISTTHREADIDS, 0, thread_list_tmp,
                            (int)allocated_size);
    T_LOG(
        "proc_pidinfo(PROC_PIDLISTTHREADIDS) buf_used = %d, expected_size = %d",
        buf_used, expected_size);
    ...
    if (buf_used == expected_size) {
      ...
      T_QUIET;
      T_WITH_ERRNO;
      T_ASSERT_NOTNULL(thread_list, "realloc(size = %d) failed", buf_used);
    } else if (buf_used > expected_size) {
      if (buf_used < allocated_size) {
        ...
        T_LOG("expected %d threads, but saw an extra thread: %d",
              expected_size / (int)sizeof(uint64_t),
              buf_used / (int)sizeof(uint64_t));
  ...
  T_QUIET;
  T_ASSERT_LE(attempt, MAX_TRIES, "attempt <= MAX_TRIES");
  T_QUIET;
  T_ASSERT_NOTNULL(thread_list, "thread_list != NULL");
  T_QUIET;
  T_ASSERT_GT(thread_count, 0, "thread_count > 0");

  struct proc_threadinfo pthinfo_64;
  for (int i = 0; i < thread_count; i++) {
    bzero(&pthinfo_64, sizeof(struct proc_threadinfo));
    int retval =
        proc_pidinfo(0, PROC_PIDTHREADID64INFO, thread_list[i],
                     (void *)&pthinfo_64, (uint32_t)sizeof(pthinfo_64));
    T_QUIET;
    T_WITH_ERRNO;
    T_EXPECT_GT(retval, 0, "proc_pidinfo(PROC_PIDTASKINFO) returned %d",
                retval);
    T_QUIET;
    T_EXPECT_EQ(retval, (int)sizeof(pthinfo_64),
                "proc_pidinfo(PROC_PIDTASKINFO) returned size %d == %lu",
