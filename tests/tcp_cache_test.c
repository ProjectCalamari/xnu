...
T_DECL(tcp_cache_list_sysctl, "Test retrieving TCP cache list via sysctl") {
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(buffer, "malloc buffer");
  ...
  for (size_t i = 0; i < num_entries; i++) {
    ...
    if (entry->tc_key.tck_src.thk_family == AF_INET) {
      T_LOG("  tc_key.tck_src IP: %s",
            inet_ntoa(entry->tc_key.tck_src.thk_ip.addr));
    } else if (entry->tc_key.tck_src.thk_family == AF_INET6) {
      char addr_str[INET6_ADDRSTRLEN];
      inet_ntop(AF_INET6, &entry->tc_key.tck_src.thk_ip.addr6, addr_str,
                sizeof(addr_str));
    ...
    if (entry->tc_key.tck_family == AF_INET) {
      ...
      char addr_str[INET6_ADDRSTRLEN];
      inet_ntop(AF_INET6, &entry->tc_key.tck_dst.addr6, addr_str,
                sizeof(addr_str));
    ...
    if (entry->tc_tfo_cookie_len > 0) {
      ...
      for (int j = 0; j < entry->tc_tfo_cookie_len && j < TFO_COOKIE_LEN_MAX;
           j++) {
    ...
    // Log MPTCP info
    T_LOG("  tc_mptcp_version_confirmed: %u",
          entry->tc_mptcp_version_confirmed);
...
T_DECL(tcp_heuristics_list_sysctl,
       "Test retrieving TCP heuristics list via sysctl") {
  size_t size = 0;
  int ret;
  ...
  for (size_t i = 0; i < num_entries; i++) {
    ...
    if (entry->th_key.thk_family == AF_INET) {
      ...
      char addr_str[INET6_ADDRSTRLEN];
      inet_ntop(AF_INET6, &entry->th_key.thk_ip.addr6, addr_str,
                sizeof(addr_str));
    ...
    T_LOG("  th_mptcp_heuristic_disabled: %u",
          entry->th_mptcp_heuristic_disabled);
    T_LOG(""); // Empty line between entries
  }
