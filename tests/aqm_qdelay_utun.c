...
static void nsec_to_str(unsigned long long nsec, char *buf) {
  const char *u;
...
static int create_tun() {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      bind(tun_fd, (struct sockaddr *)&kernctl_addr, sizeof(kernctl_addr)),
      NULL);

  const int enable = 1;
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(setsockopt(tun_fd, SYSPROTO_CONTROL,
                                    UTUN_OPT_ENABLE_NETIF, &enable,
                                    sizeof(enable)),
                         NULL);

  T_QUIET;
  T_ASSERT_POSIX_FAILURE(setsockopt(tun_fd, SYSPROTO_CONTROL,
                                    UTUN_OPT_ENABLE_FLOWSWITCH, &enable,
                                    sizeof(enable)),
                         EINVAL, NULL);

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      connect(tun_fd, (struct sockaddr *)&kernctl_addr, sizeof(kernctl_addr)),
      NULL);
  ;
...
static int aqmstats_setup(char *iface) {
  unsigned int ifindex;
  int s;

  ifindex = if_nametoindex(iface);
  T_QUIET;
  T_ASSERT_TRUE(ifindex != 0, "interface index for utun");

  ifcqs = malloc(sizeof(*ifcqs));
  T_ASSERT_TRUE(ifcqs != 0, "Allocated ifcqs");

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(s = socket(AF_INET, SOCK_DGRAM, 0), NULL);
  ...
  ifqr.ifqr_slot = 0;
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ioctl(s, SIOCGIFQUEUESTATS, (char *)&ifqr), NULL);
...
static void aqmstats_cleanup() { free(ifcqs); }
...
T_DECL(aqm_qdelay, "This test checks the min/max/avg AQM queuing delay",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      getsockopt(tun_fd, SYSPROTO_CONTROL, UTUN_OPT_IFNAME, ifname, &optlen),
      NULL);
  ...
  int sock_fd;
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sock_fd = socket(AF_INET, SOCK_DGRAM, 0), NULL);
  struct sockaddr_in sin;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(bind(sock_fd, (struct sockaddr *)&sin, sizeof(sin)),
                         NULL);
  ...
  if (scheduler == PKTSCHEDT_FQ_CODEL) {
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ioctl(stats_fd, SIOCGIFQUEUESTATS, (char *)&ifqr),
                           NULL);
  ...
  if (scheduler == PKTSCHEDT_FQ_CODEL) {
    ...
    T_LOG("min/max/avg qdelay %10s    %10s    %10s", min, max, avg);
    T_ASSERT_TRUE(ifcqs->ifqs_fq_codel_stats.fcls_min_qdelay <=
                          ifcqs->ifqs_fq_codel_stats.fcls_avg_qdelay &&
                      ifcqs->ifqs_fq_codel_stats.fcls_min_qdelay <=
                          ifcqs->ifqs_fq_codel_stats.fcls_max_qdelay,
                  "min qdelay check");
    T_ASSERT_TRUE(ifcqs->ifqs_fq_codel_stats.fcls_avg_qdelay <=
                      ifcqs->ifqs_fq_codel_stats.fcls_max_qdelay,
                  "avg qdelay check");
  }
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(close(sock_fd), NULL);
  T_QUIET;
