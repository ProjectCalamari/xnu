...
static void sendPackets(int s, struct sockaddr *dst, unsigned int numMsg,
                        size_t bufferLen) {
  ssize_t count = 0;
  struct msghdr msg = {};
  struct iovec vec = {};
  ...
  for (unsigned int i = 0; i < numMsg; i++) {
    ssize_t n;
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(n = sendmsg(s, &msg, 0), "sendmsg()");
    T_LOG("Sent %ld bytes\n", n);
    count += 1;
  }
...
static void recvPackets_x(int s, unsigned int numMsg, size_t buflen,
                          socklen_t cmsgLen) {
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(msgList = calloc(numMsg, sizeof(struct msghdr_x)),
                   "msgList calloc()");
  T_QUIET;
  T_ASSERT_NOTNULL(srcAddrs = calloc(numMsg, sizeof(struct sockaddr_in)),
                   "srcAddrs calloc()");
  T_QUIET;
  T_ASSERT_NOTNULL(vec = calloc(numMsg, sizeof(struct iovec)), "vec calloc()");
  T_QUIET;
  T_ASSERT_NOTNULL(buffers = calloc(numMsg, buflen), "buffers calloc()");
  T_QUIET;
  T_ASSERT_NOTNULL(cmsgBuf = calloc(numMsg, ALIGN(cmsgLen)),
                   "cmsgBuf calloc()");
  ...
  while (true) {
    ...
    if ((count % numMsg) == 0) {
      for (unsigned int i = 0; i < numMsg; i++) {
        ...
        T_QUIET;
        T_EXPECT_TRUE((uintptr_t)msg->msg_control % sizeof(uint32_t) == 0,
                      NULL);
      }
    }

    ssize_t n =
        recvmsg_x(s, msgList + (count % numMsg), numMsg - (count % numMsg), 0);
    if (n < 0) {
      if (errno == EINTR) {
    ...
    for (unsigned int i = count; i < count + (u_int)n; i++) {
      ...
      T_LOG("Received packet #%d %lu bytes with recvmsg_x(), msg_namelen = %u, "
            "msg_controllen = %d -> %d, msg_flags = 0x%x\n",
            i + 1, msg->msg_datalen, msg->msg_namelen, cmsgLen,
      ...
      for (cmsg = CMSG_FIRSTHDR(msg); cmsg; cmsg = CMSG_NXTHDR(msg, cmsg)) {
        T_QUIET;
        T_EXPECT_TRUE((uintptr_t)cmsg % sizeof(uint32_t) == 0, NULL);
...
T_DECL(recvmsg_x_test, "exercise revcmsg_x() with various parameter",
  ...
  for (int dontTrunc = 0; dontTrunc <= 1; dontTrunc++) {
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(setsockopt(recvSocket, SOL_SOCKET, SO_DONTTRUNC,
                                      (void *)&dontTrunc, sizeof(dontTrunc)),
...
T_DECL(recvmsg_x_empty_packet, "exercise revcmsg_x() with an empty packet",
  ...
  T_EXPECT_POSIX_SUCCESS(socketpair(AF_UNIX, SOCK_DGRAM, 0, sockets),
                         "socketpair");
  ...
  T_LOG("received = %zd\n", received);
  T_LOG("%zu %zu %zu\n", headers[0].msg_datalen, headers[1].msg_datalen,
        headers[2].msg_datalen);
