...
__attribute__((aligned(PAD_SIZE))) static const_state_t pager_state = {
...
static fault_state_t *fault_state_create(void) {
 ...
 */
static void fault_state_destroy(void *fault_state) {
  if (fault_state == NULL) {
 ...
 */
static void access_failed_handler(int signum) {
 ...
 */
static bool does_access_fault(access_type_t access_type, void *addr,
                              uint32_t value) {
  ...
  new_action.sa_handler =
      access_failed_handler; /* A handler for write failures. */
...
static bool does_read_fault(void *addr) {
  return does_access_fault(ACCESS_READ, addr, 0);
}

static bool does_write_fault(void *addr, uint32_t value) {
  return does_access_fault(ACCESS_WRITE, addr, value);
}

static bool has_pager_support(void) {
  uint32_t enabled = false;
  size_t output_size = sizeof(enabled);

  (void)sysctlbyname("vm.pmap_tpro_pagers", &enabled, &output_size, NULL, 0);
  return enabled;
}
...
static void cleanup(void) {
  ...
  if (!(fault_state = pthread_getspecific(fault_state_key))) {
    ...
    T_ASSERT_POSIX_ZERO(pthread_setspecific(fault_state_key, NULL),
                        "Remove fault_state");
    fault_state_destroy(fault_state);
  }

  if (key_created) {
    T_ASSERT_POSIX_ZERO(pthread_key_delete(fault_state_key),
...
static void thread_self_restrict_test(void (^test)(void)) {
  int err = 0;
...
static void fork_child_test(const_page_t *state) {
  pid_t pid;
  int statloc;
  ...
  if (pid == 0) {
    ...
    os_thread_self_restrict_tpro_to_rw();
    T_EXPECT_EQ(os_thread_self_restrict_tpro_is_writable(), true,
                "TPRO region configured as read-write in child");
    T_EXPECT_EQ(does_write_fault((void *)&state->bytes, 0x47474747), 0,
                "write to pager backed memory in child (no fault)");
    T_EXPECT_EQ(does_write_fault(MAGIC(state), 0x46464646), 0,
                "write to pager backed memory in child (no fault)");
    exit(0);
  }
...
static void pager_test(const_page_t *state) {
  kern_return_t kr;
  uint32_t pre;
   ...
   */
  T_EXPECT_EQ(os_thread_self_restrict_tpro_is_writable(), false,
              "TPRO region starts read-only");
  T_EXPECT_EQ(does_read_fault(MAGIC(state)), 0,
              "read from pager backed memory");
  T_EXPECT_EQ(does_write_fault(MAGIC(state), 0x43434343), 1,
              "write to pager backed memory (detect fault)");
  ...
  os_thread_self_restrict_tpro_to_rw();
  T_EXPECT_EQ(os_thread_self_restrict_tpro_is_writable(), true,
              "TPRO region configured as read-write");
  T_EXPECT_EQ(does_write_fault(MAGIC(state), 0x44444444), 0,
              "write to pager backed memory (no fault)");
  ...
  T_EXPECT_EQ(does_write_fault(MAGIC(state), 0x45454545), 1,
              "write to pager backed memory (detect fault)");
   ...
   */
  kr = mach_vm_remap(mach_task_self(), &addr, PAGE_SIZE, 0, /* mask */
                     VM_FLAGS_ANYWHERE, mach_task_self(),
                     (mach_vm_address_t)state, FALSE, /* copy */
  ...
  os_thread_self_restrict_tpro_to_ro();
  T_EXPECT_EQ(os_thread_self_restrict_tpro_is_writable(), false,
              "TPRO configured as read-only");
  T_EXPECT_EQ(curprot, VM_PROT_READ, "TPRO region should be VM_PROT_READ");
  T_EXPECT_EQ(does_write_fault(MAGIC(copy_state), 0x46464646), 1,
              "write to remapped region (detect fault)");
  os_thread_self_restrict_tpro_to_rw();
  T_EXPECT_EQ(does_write_fault(MAGIC(copy_state), 0x46464646), 0,
              "write to remapped region (no fault)");
  T_EXPECT_EQ(0x46464646, state->magic,
              "write into copied region should be observed");

  /*
   * Ensure that if we remap the region that we do not observe writes to
   * the new copy in __DATA_CONST itself.
   */
  kr = mach_vm_remap(mach_task_self(), (mach_vm_address_t *)&copy_state,
                     PAGE_SIZE, 0, /* mask */
                     VM_FLAGS_ANYWHERE, mach_task_self(),
                     (mach_vm_address_t)state, TRUE, /* copy */
                     &curprot, &maxprot, VM_INHERIT_DEFAULT);
  ...
  T_EXPECT_EQ(does_write_fault(MAGIC(copy_state), 0x46464646), 0,
              "write to pager backed memory (no fault)");
  T_EXPECT_EQ(pre, state->magic,
              "write into copied region should not be observed");
  ...
  kr = mach_make_memory_entry_64(
      mach_task_self(), &me_size, (mach_vm_address_t)state,
      MAP_MEM_VM_SHARE | VM_PROT_READ | VM_PROT_WRITE, &cow_port,
      MACH_PORT_NULL);
  T_EXPECT_POSIX_SUCCESS(kr, "mach_make_memory_entry_64(MAP_MEM_VM_SHARE)");

  pre = state->magic;
  T_EXPECT_EQ(does_write_fault(MAGIC(copy_state), 0x48484849), 0,
              "write to mapped copy region (no fault)");
  T_EXPECT_EQ(pre, state->magic,
              "write into copied region should not be observed");

  copy_state = NULL;
  kr = mach_vm_map(mach_task_self(), (mach_vm_address_t *)&copy_state,
                   PAGE_SIZE, 0,                   /* mask */
                   VM_FLAGS_ANYWHERE, cow_port, 0, /* offset */
                   TRUE,                           /* copy */
  ...
  pre = state->magic;
  os_thread_self_restrict_tpro_to_ro();
  T_EXPECT_EQ(does_write_fault(MAGIC(copy_state), 0x48484848), 0,
              "write to mapped copy region (no fault)");
  T_EXPECT_EQ(pre, state->magic,
              "write into copied region should not be observed");
   ...
   */
  os_thread_self_restrict_tpro_to_ro();
  T_EXPECT_EQ(does_write_fault(MAGIC(state), 0x49494949), 1,
              "write to pager backed memory (detect fault)");
  os_thread_self_restrict_tpro_to_rw();
  T_EXPECT_EQ(does_write_fault(MAGIC(state), 0x4a4a4a4a), 0,
              "write to pager backed memory (no fault)");
   ...
   */
  mach_vm_behavior_set(mach_task_self(), (mach_vm_address_t)state, PAGE_SIZE,
                       VM_BEHAVIOR_PAGEOUT);
  ...
  os_thread_self_restrict_tpro_to_ro();
  T_EXPECT_EQ(does_write_fault(MAGIC(state), 0x49494949), 1,
              "write to pager backed memory after pageout (detect fault)");
  os_thread_self_restrict_tpro_to_rw();
  T_EXPECT_EQ(does_write_fault(MAGIC(state), 0x4a4a4a4a), 0,
              "write to pager backed memory after pageout (no fault)");

  /*
   * Try and reprotect the region. We should fail
   */
  kr = vm_protect(mach_task_self(), (mach_vm_address_t)state, PAGE_SIZE, FALSE,
                  VM_PROT_DEFAULT);
  T_EXPECT_POSIX_ERROR(kr, KERN_PROTECTION_FAILURE,
                       "vm_protect(RW) should fail");

  os_thread_self_restrict_tpro_to_ro();
}

static void mmap_test(const_page_t *state) {
  void *mapping;
   ...
   * overwritten.
   */
  mapping = mmap(state, PAGE_SIZE, PROT_READ | PROT_WRITE,
                 MAP_FIXED | MAP_PRIVATE | MAP_ANON, -1, 0);
  T_ASSERT_EQ(mapping, MAP_FAILED, "Map over TPRO range should fail");
  os_thread_self_restrict_tpro_to_ro();
}

static void vm_allocate_test(const_page_t *state) {
  kern_return_t kr;
   ...
   */
  kr =
      mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)state, PAGE_SIZE);
  T_EXPECT_POSIX_ERROR(kr, KERN_SUCCESS, "vm_deallocate should succeed");

  kr = mach_vm_allocate(mach_task_self(), (mach_vm_address_t *)&addr, PAGE_SIZE,
                        VM_FLAGS_FIXED);
  T_EXPECT_POSIX_ERROR(kr, KERN_NO_SPACE,
                       "vm_allocate should fail with KERN_NO_SPACE");
  ...
  T_QUIET;
  T_EXPECT_POSIX_ERROR(kr, KERN_SUCCESS, "mach_vm_region should succeed");

  T_ASSERT_EQ(info.protection, VM_PROT_NONE,
              "Entry should have no permissions");
  T_ASSERT_EQ(info.max_protection, VM_PROT_NONE,
              "Entry should have no permissions");
  os_thread_self_restrict_tpro_to_ro();
}
...
T_DECL(thread_self_restrict_pagers,
  ...
  thread_self_restrict_test(^{
     ...
     */
    os_thread_self_restrict_tpro_to_ro();
    T_EXPECT_EQ(does_write_fault(&pager_state.ro[0], 0x41414141), 1,
                "write to __DATA_CONST should succeed (no fault)");
    os_thread_self_restrict_tpro_to_rw();
    T_EXPECT_EQ(does_write_fault(&pager_state.ro[0], 0x41414141), 0,
                "write to __DATA_CONST should fail (detect fault)");
  });
#else
