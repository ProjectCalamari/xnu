...
T_DECL(adjust_quantum_nonovercommit_to_overcommit_switch, "rdar://75084197",
  ...
  dispatch_async(dq, ^{
    ...
    pthread_priority_t overcommit =
        (pthread_priority_t)_pthread_getspecific_direct(
            _PTHREAD_TSD_SLOT_PTHREAD_QOS_CLASS) |
        _PTHREAD_PRIORITY_OVERCOMMIT_FLAG;
    T_ASSERT_POSIX_ZERO(
        _pthread_set_properties_self(_PTHREAD_SET_SELF_QOS_FLAG, overcommit, 0),
        NULL);
...
T_DECL(cooperative_to_overcommit_switch,
       "Switching from cooperative queue to another type should not panic",
       T_META_TAG_VM_PREFERRED) {
  dispatch_queue_t cooperative_dq =
      dispatch_get_global_queue(QOS_CLASS_UTILITY, DISPATCH_QUEUE_COOPERATIVE);
  ...
  dispatch_queue_t dq =
      dispatch_queue_create("serial IN overcommit queue", attr);
...
T_DECL(maintenance_bg_coalesced, "BG and MT coalescing should work",
       T_META_TAG_VM_PREFERRED) {
  dispatch_queue_t dq = dispatch_get_global_queue(QOS_CLASS_MAINTENANCE,
                                                  DISPATCH_QUEUE_COOPERATIVE);
  T_ASSERT_NE(dq, NULL, "global_queue");
