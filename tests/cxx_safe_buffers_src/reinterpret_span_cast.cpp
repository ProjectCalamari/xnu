...
static void tests() {
  {
    ...
    std::span<const std::byte> nonwritable_sp =
        os::reinterpret_span_cast<const std::byte>(sp);
    CHECK(writable_sp.size() == sp.size_bytes() &&
          nonwritable_sp.size() == sp.size_bytes());
  }

  {
    // convert span<byte> to span<A>
    std::vector<std::byte> vec{std::byte{0}, std::byte{1}, std::byte{2},
...
static void trapping_test() {
  ...
  T_ASSERT_FALSE(dt_waitpid(pid, &status, &signal, 0),
                 "wait for child (%d) complete with signal %d", pid, signal);
  // child process must trigger an execution trap
  T_ASSERT_TRUE(WIFSIGNALED(signal),
                "Child process successfully triggered an execution trap");
}
