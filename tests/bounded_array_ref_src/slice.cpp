...
template <typename T>
using tracking_bounded_array_ref =
    libkern::bounded_array_ref<T, tracking_policy>;
...
template <typename T> static void tests() {
  ...
  }
  {
    ...
    CHECK(tracking_policy::message ==
          "bounded_array_ref: invalid slice provided, the indices are of "
          "bounds for the bounded_array_ref");
  }
  ...
  // Slices overflowing a uint32_t
  {
    ...
    CHECK(tracking_policy::did_trap);
    CHECK(tracking_policy::message == "bounded_array_ref: n + m is larger than "
                                      "the size of any bounded_array_ref");
  }
