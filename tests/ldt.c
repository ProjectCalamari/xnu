...
#ifndef STANDALONE
T_GLOBAL_META(T_META_NAMESPACE("xnu.intel"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("intel"),
              T_META_OWNER("seth_goldberg"), T_META_CHECK_LEAKS(false));
#endif
 ...
 *     64-bit code segments is not allowed (just use the existing 0x2b
 * selector.) (3) Once you know which selector is associated with the desired
 * code, use a trampoline (or thunk) to (a) switch to a stack that's located
 * below 4GiB and (b) save ABI-mandated caller-saved state so that if it's
 * trashed by compatibility-mode code, it can be restored before returning to
 * 64-bit mode (if desired), and finally (c) long-jump or long-call (aka far
 * call) to the segment and desired offset (this example uses an offset of 0 for
 *     simplicity.)
 * (4) Once in compatibility mode, if a framework call or system call is
 * required, the code must trampoline back to 64-bit mode to do so.  System
 * calls from compatibility mode code are not supported and will result in
 * invalid opcode exceptions.  This example includes a simple 64-bit trampoline
 * (which must be located in the low 4GiB of virtual address space, since it's
 * executed by compatibility-mode code.)  Note that since the 64-bit ABI
 * mandates that the stack must be aligned to a 16-byte boundary, the sample
 * trampoline performs that rounding, to simplify compatibility-mode code.
 * Additionally, since 64-bit native code makes use of thread-local storage, the
 * user-mode GSbase must be restored.  This sample includes two ways to do
 * that-- (a) by calling into a C implementation that associates the
 * thread-local storage pointer with a stack range (which will be unique for
 * each thread.), and (b) by storing the original GSbase in a block of memory
 * installed into GSbase before calling into compatibility-mode code.  A special
 * machdep system call restores GSbase as needed.  Note that the sample
 * trampoline does not save and restore %gs (or most other register state, so
 * that is an area that may be tailored to the application's requirements.) (5)
 * Once running in compatibility mode, should synchronous or asynchronous
 ...
 *     servers.  Alternatively, BSD signal handlers can be used.  Note that once
 * a process installs a custom LDT, *ALL* future signal deliveries will include
 *     ucontext pointers to mcontext structures that include enhanced thread
 *     state embedded (e.g. the %ds, %es, %ss, and GSBase registers) [This
 * assumes that the SA_SIGINFO is passed to sigaction(2) when registering
 * handlers]. The mcontext size (part of the ucontext) can be used to
 * differentiate between different mcontext flavors (e.g. those with/without
 * full thread state plus x87 FP state, AVX state, or AVX2/3 state).
 */

/*
 * This test exercises the custom LDT functionality exposed via the
 * i386_{get,set}_ldt system calls.
 *
 * Tests include:
 * (1a) Exception handling (due to an exception or another thread sending a
 * signal) while running in compatibility mode; (1b) Signal handling while
 * running in compatibility mode; (2)  Thunking back to 64-bit mode and
 * executing a framework function (e.g. printf) (3)  Ensuring that transitions
 * to compatibility mode and back to 64-bit mode do not negatively impact system
 * calls and framework calls in 64-bit mode (4)  Use of thread_get_state /
 * thread_set_state to configure a thread to execute in compatibility mode with
 * the proper LDT code segment (this is effectively what the exception handler
 * does when the passed-in new_state is changed (or what the BSD signal handler
 * return handling does when the mcontext is modified).) (5)  Ensure that
 * compatibility mode code cannot make system calls via sysenter or old-style
 * int {0x80..0x82}. (6)  Negative testing to ensure errors are returned if the
 * consumer tries to set a disallowed segment type / Long flag. [TBD]
 */
...
typedef uint64_t (*compat_tramp_t)(far_call_t *fcp, void *lowmemstk,
                                   uint64_t arg_for_32bit, uint64_t callback,
...
kern_return_t catch_mach_exception_raise_state_identity(
    ...
    mach_msg_type_number_t code_count, int *flavor, thread_state_t old_state,
...
/*
 * Though we've directed all exceptions through the
 * catch_mach_exception_raise_state_identity entry point, we still must provide
 * these two other entry points, otherwise a linker error will occur.
 */
kern_return_t catch_mach_exception_raise(
    mach_port_t exception_port, mach_port_t thread, mach_port_t task,
    exception_type_t exception, mach_exception_data_t code,
    ...
    mach_msg_type_number_t old_stateCnt, thread_state_t new_state,
    mach_msg_type_number_t *new_stateCnt) {
#pragma unused(exception_port, thread, task, exception, code, codeCnt, flavor, \
...
kern_return_t catch_mach_exception_raise_state(
    ...
    mach_msg_type_number_t old_stateCnt, thread_state_t new_state,
    mach_msg_type_number_t *new_stateCnt) {
#pragma unused(exception_port, exception, code, codeCnt, flavor, old_state,    \
               old_stateCnt, new_state, new_stateCnt)
  fprintf(stderr, "Unexpected exception handler called: %s\n", __func__);
  return KERN_FAILURE;
}

static void handle_arithmetic_exception(_STRUCT_X86_THREAD_FULL_STATE64 *xtfs64,
  ...
  fprintf(stderr, "cs=0x%x rip=0x%x gs=0x%x ss=0x%x rsp=0x%llx\n",
          (unsigned)xtfs64->__ss64.__cs, (unsigned)xtfs64->__ss64.__rip,
          (unsigned)xtfs64->__ss64.__gs, (unsigned)xtfs64->__ss,
          xtfs64->__ss64.__rsp);
  *ip_skip_countp = 2;
}

static void handle_badinsn_exception(_STRUCT_X86_THREAD_FULL_STATE64 *xtfs64,
                                     uint64_t __unused *ip_skip_countp) {
  ...
  fprintf(stderr,
          "cs=%x rip=%x gs=%x ss=0x%x rsp=0x%llx | handling between 0x%llx and "
          "0x%llx\n",
          (unsigned)xtfs64->__ss64.__cs, (unsigned)xtfs64->__ss64.__rip,
          (unsigned)xtfs64->__ss64.__gs, (unsigned)xtfs64->__ss,
          xtfs64->__ss64.__rsp, start_addr, end_addr);
...
#pragma unused(exception_port, thread, task)
  ...
  _STRUCT_X86_THREAD_FULL_STATE64 *xtfs64 =
      (_STRUCT_X86_THREAD_FULL_STATE64 *)(void *)old_state;
  _STRUCT_X86_THREAD_FULL_STATE64 *new_xtfs64 =
      (_STRUCT_X86_THREAD_FULL_STATE64 *)(void *)new_state;
  ...
  default:
    fprintf(stderr,
            "Unsupported catch_mach_exception_raise_state_identity: code "
            "0x%llx sub 0x%llx\n",
  ...
  if ((unsigned)xtfs64->__ss64.__cs == COMPAT_MODE_CS_SELECTOR) {
    ...
    *new_xtfs64 = *xtfs64;
    ...
    fprintf(stderr, "new cs=0x%x rip=0x%llx\n",
            (unsigned)new_xtfs64->__ss64.__cs, new_xtfs64->__ss64.__rip);
    return KERN_SUCCESS;
  } else {
...
static void *handle_exceptions(void *arg) {
...
static void init_task_exception_server(void) {
  kern_return_t kr;
  ...
  kr = task_set_exception_ports(
      me, EXC_MASK_BAD_INSTRUCTION | EXC_MASK_ARITHMETIC, ePort,
      (exception_behavior_t)(EXCEPTION_STATE_IDENTITY | MACH_EXCEPTION_CODES),
      x86_THREAD_FULL_STATE64);
  if (kr != KERN_SUCCESS) {
    fprintf(stderr,
            "abort: error setting task exception ports on task=[%d], "
            "handler=[%d]: %d\n",
            me, ePort, kr);
    exit(1);
  }
  ...
  if (pthread_create(&handler_thread, &attr, handle_exceptions,
                     (void *)(uintptr_t)ePort) != 0) {
...
static void signal_handler(int signo, siginfo_t *sinfop, void *ucontext) {
  uint64_t rip_skip_count = 0;
  ...
  if (uctxp->uc_mcsize == sizeof(_STRUCT_MCONTEXT_AVX512_64) ||
    ...
    if (ENV_set_ldt_in_sighandler == TRUE && !saw_ud2) {
      ...
      /*
       * When we return here, the sigreturn processing code will try to copy a
       * FULL thread context from the signal stack, which will likely cause the
       * resumed thread to fault and be terminated.
       */
      return;
    }
     ...
     * BSD-signalling-sending-full-thread-state support.  It's likely the latter
     * case.
     */
#ifndef STANDALONE
    T_ASSERT_FAIL(
        "This system doesn't support BSD signals with full thread state.");
#else
    fprintf(stderr, "This system doesn't support BSD signals with full thread "
                    "state.  Aborting.\n");
    abort();
#endif
...
static void setup_signal_handling(void) {
  ...
  struct sigaction sa = {.__sigaction_u = {.__sa_sigaction = signal_handler},
                         .sa_flags = SA_SIGINFO};
...
static void teardown_signal_handling(void) {
...
#ifdef DEBUG
static void dump_desc(union ldt_entry *entp) {
  printf("base %p lim %p type 0x%x dpl %x present %x opsz %x granular %x\n",
         (void *)(uintptr_t)(entp->code.base00 + (entp->code.base16 << 16) +
                             (entp->code.base24 << 24)),
         (void *)(uintptr_t)(entp->code.limit00 + (entp->code.limit16 << 16)),
         entp->code.type, entp->code.dpl, entp->code.present, entp->code.opsz,
         entp->code.granular);
}
#endif

static int map_lowmem_stack(void **lowmemstk) {
  void *addr;
  int err;
...
#else
    fprintf(stderr,
            "This kernel returned a virtual address > 4G (%p) despite "
            "MAP_32BIT.  Aborting.\n",
            addr);
    exit(1);
#endif
...
static int map_32bit_code_impl(uint8_t *code_src, size_t code_len,
                               void **codeptr, size_t szlimit) {
  void *addr;
  ...
  printf("Mapping code @%p..%p => %p..%p\n", (void *)code_src,
         (void *)((uintptr_t)code_src + (unsigned)code_len), addr,
         (void *)((uintptr_t)addr + (unsigned)code_len));
#endif
...
static int map_32bit_trampoline(compat_tramp_t *lowmemtrampp) {
  extern int compat_mode_trampoline_len;

  return map_32bit_code_impl((uint8_t *)&compat_mode_trampoline,
                             (size_t)compat_mode_trampoline_len,
                             (void **)lowmemtrampp, FIXED_TRAMP_MAXLEN);
}
...
static uint64_t call_compatmode(uint32_t stackaddr, uint64_t compat_arg,
                                uint64_t callback) {
  uint64_t rv;
  ...
  rv = thunkit(&input_desc, (void *)(uintptr_t)stackaddr, compat_arg, callback,
               thunk64_addr);
...
static uint64_t get_cursp(void) {
  uint64_t curstk;
  __asm__ __volatile__("movq %%rsp, %0" : "=r"(curstk)::"memory");
  return curstk;
}

static void hello_from_32bit(void) {
 ...
 */
static void *thread_32bit(void *arg) {
  ...
  /*
   * Associate GSbase with the compat-mode stack (which will be used for long
   * mode thunk calls as well.)
   */
...
static void join_32bit_thread(pthread_t *thridp, thread_arg_t *cmargp) {
...
static int create_worker_thread(thread_arg_t *cmargp, uint32_t stackaddr,
                                pthread_t *cmthreadp) {
...
static void ldt64_test_setup(pthread_t *cmthreadp, thread_arg_t *cmargp,
  ...
  thunk64_movabs_addr =
      (uintptr_t)thunkit +
      ((uintptr_t)thunk64_movabs - (uintptr_t)compat_mode_trampoline);
  *((uint64_t *)(thunk64_movabs_addr + 2)) = (uint64_t)&long_mode_trampoline;
  ...
  idx = (unsigned)
      cnt; /* Put the desired descriptor in the first available slot */

  /*
   * code_32's address for the purposes of this descriptor is the base mapped
   * address of the thunkit function + the offset of code_32 from
   * compat_mode_trampoline.
   */
  code_addr = (uintptr_t)thunkit +
              ((uintptr_t)code_32 - (uintptr_t)compat_mode_trampoline);
  thunk64_addr = (uintptr_t)thunkit +
                 ((uintptr_t)thunk64 - (uintptr_t)compat_mode_trampoline);

  /* Initialize desired descriptor */
  descs[idx].code.limit00 = (unsigned short)(((code_addr >> 12) + 1) & 0xFFFF);
  descs[idx].code.limit16 =
      (unsigned char)((((code_addr >> 12) + 1) >> 16) & 0xF);
  ...
  if ((err = create_worker_thread(cmargp, (uint32_t)stackAddr, cmthreadp)) !=
      0) {
#ifdef DEBUG
...
#ifdef STANDALONE
static void test_ldt64_with_bsdsig(void)
#else
/*
 * Main test declarations
 */
T_DECL(ldt64_with_bsd_sighandling,
       "Ensures that a 64-bit process can create LDT entries and can execute "
       "code in "
...
#endif
{
  ...
  sysctlbyname("sysctl.proc_translated", &translated, &translated_size, NULL,
               0);
  ...
  ENV_set_ldt_in_sighandler =
      (getenv("LDT_SET_IN_SIGHANDLER") != NULL) ? TRUE : FALSE;
...
#ifdef STANDALONE
static void test_ldt64_with_machexc(void)
#else
