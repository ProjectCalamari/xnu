...
static void *register_sockopt_racer(void *data) {
  int s = *(int *)data;
  struct {
...
static void *register_message_racer(void *data) {
  int s = *(int *)data;
  struct netagent netagent;

  bzero(&netagent, sizeof(netagent));
  while (!finished) {
    setsockopt(s, SYSPROTO_CONTROL, NETAGENT_MESSAGE_TYPE_REGISTER, &netagent,
               sizeof(netagent));
  }
...
static void *unregister_racer(void *data) {
  int s = *(int *)data;
...
static size_t data_available(int sock) {
  int n = 0;
...
T_DECL(netagent_race_infodisc_56244905,
       "Netagent race between register and post event.") {
  int s;
  int evsock;
  pthread_t reg_th;
  pthread_t unreg_th;
  struct kev_request kev_req = {.vendor_code = KEV_VENDOR_APPLE,
  ...
  T_ASSERT_POSIX_SUCCESS(evsock = socket(AF_SYSTEM, SOCK_RAW, SYSPROTO_EVENT),
                         NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(connect(s, (const struct sockaddr *)&sc, sizeof(sc)),
                         NULL);
  T_SETUPEND;

  /* variant 1: */
  /* spin off the racer threads: */
  T_ASSERT_POSIX_ZERO(pthread_create(&reg_th, NULL, register_message_racer, &s),
                      NULL);
  T_ASSERT_POSIX_ZERO(pthread_create(&unreg_th, NULL, unregister_racer, &s),
                      NULL);
  ...
  /* variant 2: */
  /* spin off the racer threads: */
  T_ASSERT_POSIX_ZERO(pthread_create(&reg_th, NULL, register_sockopt_racer, &s),
                      NULL);
  T_ASSERT_POSIX_ZERO(pthread_create(&unreg_th, NULL, unregister_racer, &s),
                      NULL);
