...
T_DECL(socket_poll_close_25786011,
       "Tests an invalid poll call to a socket and then calling close.",
       T_META_LTEPHASE(LTE_POSTINIT), T_META_TAG_VM_PREFERRED) {
  int my_socket, ret;

  my_socket = socket(PF_LOCAL, SOCK_STREAM, 0);
  T_WITH_ERRNO;
  T_ASSERT_TRUE(my_socket > 0, "create socket");
   ...
   * for a socket.
   */
  struct pollfd my_pollfd = {.fd = my_socket, .events = POLLEXTEND};
  ...
  T_WITH_ERRNO;
  T_ASSERT_TRUE(ret == 1, "poll returned %d", ret);
