...
static struct ifbareq *bridge_rt_table_copy(u_int *ret_count);

static void bridge_rt_table_log(struct ifbareq *rt_table, u_int count);

static struct ifbrmne *bridge_mac_nat_entries_copy(u_int *ret_count);

static void bridge_mac_nat_entries_log(struct ifbrmne *entries, u_int count);
...
static void set_ethernet_address(ether_addr_t *eaddr, u_int unit,
                                 u_int addr_index) {
...
static void get_ipv4_address(u_int unit, u_int addr_index, struct in_addr *ip) {
...
static void get_ipv6_ula_address(u_int unit, u_int addr_index,
                                 struct in6_addr *ip) {
  *ip = ula_address;
  /* up to 255 units, 255 addresses */
...
static void get_ipv6_ll_address(const ether_addr_t *mac, struct in6_addr *in6) {
...
static void get_ip_address(uint8_t af, u_int unit, u_int addr_index,
                           union ifbrip *ip) {
  switch (af) {
  case AF_INET:
...
static bool ip_addresses_are_equal(uint8_t af, union ifbrip *ip1,
                                   union ifbrip *ip2) {
  bool equal;
  ...
  case AF_INET:
    ...
    break;
  case AF_INET6:
    equal = IN6_ARE_ADDR_EQUAL(&ip1->ifbrip_addr6, &ip2->ifbrip_addr6);
    break;
  default:
    T_FAIL("unrecognized address family %u", af);
    equal = false;
...
static ether_addr_t ether_external = {{0x80, 0x00, 0x00, 0x00, 0x00, 0x01}};

static inline struct in_addr get_external_ipv4_address(void) {
  struct in_addr ip;
...
static inline void get_external_ip_address(uint8_t af, union ifbrip *ip) {
  switch (af) {
  case AF_INET:
    /* IP 10.1.255.1 */
...
static inline void get_broadcast_ip_address(uint8_t af, union ifbrip *ip) {
  switch (af) {
  case AF_INET:
...
static const char *ether_ntoa_buf(const ether_addr_t *n, char *buf,
                                  int buf_size) {
  char *str;
...
static const char *inet_ptrtop(int af, const void *ptr, char *buf,
                               socklen_t buf_size) {
  union {
    struct in_addr ip;
    struct in6_addr ip6;
...
static __inline__ char *arpop_name(u_int16_t op) {
  switch (op) {
  case ARPOP_REQUEST:
    return "ARP REQUEST";
  case ARPOP_REPLY:
...
static void arp_frame_validate(const struct ether_arp *earp, u_int len,
                               bool dump) {
  ...
  T_QUIET;
  T_ASSERT_GE(len, (u_int)sizeof(*earp), "%s ARP packet size %u need %u",
  ...
  T_LOG("%s type=0x%x proto=0x%x", arpop_name(ntohs(arp_p->ar_op)), arphrd,
        ntohs(arp_p->ar_pro));
  if (arp_p->ar_hln == sizeof(earp->arp_sha)) {
    ether_ntoa_buf((const ether_addr_t *)earp->arp_sha, buf_sender_ether,
                   sizeof(buf_sender_ether));
    ether_ntoa_buf((const ether_addr_t *)earp->arp_tha, buf_target_ether,
                   sizeof(buf_target_ether));
    T_LOG("Sender H/W\t%s", buf_sender_ether);
    T_LOG("Target H/W\t%s", buf_target_ether);
  }
  inet_ptrtop(AF_INET, earp->arp_spa, buf_sender_ip, sizeof(buf_sender_ip));
  inet_ptrtop(AF_INET, earp->arp_tpa, buf_target_ip, sizeof(buf_target_ip));
  T_LOG("Sender IP\t%s", buf_sender_ip);
  T_LOG("Target IP\t%s", buf_target_ip);
  return;
}

static void ip_frame_validate(const void *buf, u_int buf_len, bool dump) {
  ...
  inet_ptrtop(AF_INET, &ip_udp->ip.ip_src, buf_src, sizeof(buf_src));
  inet_ptrtop(AF_INET, &ip_udp->ip.ip_dst, buf_dst, sizeof(buf_dst));
  if (dump) {
    T_LOG("ip src %s dst %s len %u id %d", buf_src, buf_dst, ip_len,
  ...
  T_QUIET;
  T_ASSERT_EQ((u_int)(ip_udp->ip.ip_hl << 2), (u_int)sizeof(struct ip), NULL);
...
static void ip6_frame_validate(const void *buf, u_int buf_len, bool dump) {
  ...
  T_QUIET;
  T_ASSERT_EQ((ip6->ip6_vfc & IPV6_VERSION_MASK), IPV6_VERSION, NULL);
  T_QUIET;
  switch (ip6->ip6_nxt) {
  case IPPROTO_UDP: {
    u_int data_len;
    ...
    if (dump) {
      T_LOG("udp src 0x%x dst 0x%x len %u"
            " csum 0x%x datalen %u",
            ntohs(ip6_udp->udp.uh_sport), ntohs(ip6_udp->udp.uh_dport), udp_len,
            ntohs(ip6_udp->udp.uh_sum), data_len);
    }
    break;
  }
  case IPPROTO_ICMPV6: {
...
static void ethernet_frame_validate(const void *buf, u_int buf_len, bool dump) {
  ...
  ether_ntoa_buf((const ether_addr_t *)&eh_p->ether_dhost, ether_dst,
                 sizeof(ether_dst));
  ether_ntoa_buf((const ether_addr_t *)&eh_p->ether_shost, ether_src,
                 sizeof(ether_src));
  if (dump) {
    T_LOG("ether dst %s src %s type 0x%x", ether_dst, ether_src, ether_type);
  }
  switch (ether_type) {
  case ETHERTYPE_IP:
    ip_frame_validate(eh_p + 1, (u_int)(buf_len - sizeof(*eh_p)), dump);
    break;
  case ETHERTYPE_ARP:
    arp_frame_validate((const struct ether_arp *)(eh_p + 1),
                       (u_int)(buf_len - sizeof(*eh_p)), dump);
    break;
  case ETHERTYPE_IPV6:
    ip6_frame_validate(eh_p + 1, (u_int)(buf_len - sizeof(*eh_p)), dump);
    break;
  default:
...
static u_int ethernet_udp_frame_populate(
    void *buf, size_t buf_len, uint8_t af, const ether_addr_t *src,
    union ifbrip *src_ip, uint16_t src_port, const ether_addr_t *dst,
    union ifbrip *dst_ip, uint16_t dst_port, const void *data, u_int data_len) {
  u_int len;

  switch (af) {
  case AF_INET:
    len = ethernet_udp4_frame_populate(buf, buf_len, src, src_ip->ifbrip_addr,
                                       src_port, dst, dst_ip->ifbrip_addr,
                                       dst_port, data, data_len);
    break;
  case AF_INET6:
    len = ethernet_udp6_frame_populate(buf, buf_len, src, &src_ip->ifbrip_addr6,
                                       src_port, dst, &dst_ip->ifbrip_addr6,
                                       dst_port, data, data_len);
    break;
  default:
    T_FAIL("unrecognized address family %u", af);
...
static u_int ethernet_arp_frame_populate(void *buf, u_int buf_len, uint16_t op,
  ...
  T_QUIET;
  T_ASSERT_GE(buf_len, frame_length, "%s buffer size %u needed %u", __func__,
  ...
  if (target_hw != NULL) {
    bcopy(target_hw, eh_p->ether_dhost, sizeof(eh_p->ether_dhost));
  } else {
    bcopy(&ether_broadcast, eh_p->ether_dhost, sizeof(eh_p->ether_dhost));
  }
...
static uint32_t next_generation(void) { return G_generation++; }

static const void *ethernet_frame_get_udp4_payload(void *buf, u_int buf_len,
...
static const void *ethernet_frame_get_udp6_payload(void *buf, u_int buf_len,
                                                   u_int *ret_payload_length) {
  ether_header_t *eh_p;
  uint16_t ether_type;
  ...
  T_QUIET;
  T_ASSERT_EQ((int)(ip6_udp->ip6.ip6_vfc & IPV6_VERSION_MASK), IPV6_VERSION,
              NULL);
  T_QUIET;
...
static const void *ethernet_frame_get_udp_payload(uint8_t af, void *buf,
                                                  u_int buf_len,
                                                  u_int *ret_payload_length) {
  const void *payload;
  ...
  switch (af) {
  case AF_INET:
    payload = ethernet_frame_get_udp4_payload(buf, buf_len, ret_payload_length);
    break;
  case AF_INET6:
    payload = ethernet_frame_get_udp6_payload(buf, buf_len, ret_payload_length);
    break;
  default:
    T_FAIL("unrecognized address family %u", af);
    payload = NULL;
...
#define MIN_ICMP6_LEN                                                          \
  ((u_int)(sizeof(ether_header_t) + sizeof(struct ip6_hdr) +                   \
...
#define LINKADDR_OPT_LEN                                                       \
  (ALIGN_ND_OPT(sizeof(struct nd_opt_hdr) + sizeof(ether_addr_t)))
#define ETHER_IPV6_LEN (sizeof(*eh_p) + sizeof(*ip6))

static u_int ethernet_nd6_frame_populate(void *buf, u_int buf_len, uint8_t type,
                                         const ether_addr_t *sender_hw,
  ...
  case ND_NEIGHBOR_SOLICIT: {
    ...
    bcopy(target_ip, &nd_ns->nd_ns_target, sizeof(nd_ns->nd_ns_target));
    break;
  }
  case ND_NEIGHBOR_ADVERT: {
    ...
    bcopy(target_ip, &nd_na->nd_na_target, sizeof(nd_na->nd_na_target));
  ...
  bcopy(sender_hw, eh_p->ether_shost, ETHER_ADDR_LEN);
  if (dest_ether != NULL) {
    bcopy(dest_ether, eh_p->ether_dhost, sizeof(eh_p->ether_dhost));
  } else {
...
** Switch port
**/
static void switch_port_check_tx(switch_port_t port) {
  int error;
  struct kevent kev;
  int kq;
...
static void switch_port_send_arp(switch_port_t port, uint16_t op,
                                 const ether_addr_t *sender_hw,
                                 struct in_addr sender_ip,
                                 const ether_addr_t *target_hw,
                                 struct in_addr target_ip) {
  ...
  frame_length = ethernet_arp_frame_populate(&pkt, sizeof(pkt), op, sender_hw,
                                             sender_ip, target_hw, target_ip);
  T_QUIET;
  T_ASSERT_GT(frame_length, 0, "%s: frame_length %u", __func__, frame_length);
  if (G_debug) {
    T_LOG("Port %s -> %s transmitting %u bytes", port->ifname,
          port->member_ifname, frame_length);
  }
  ethernet_frame_validate(&pkt, frame_length, G_debug);
  n = write(port->fd, &pkt, frame_length);
  if (n < 0) {
    T_ASSERT_POSIX_SUCCESS(n, "%s write fd %d failed %ld", port->ifname,
                           port->fd, n);
  }
  T_QUIET;
  T_ASSERT_EQ((u_int)n, frame_length, "%s fd %d wrote %ld", port->ifname,
              port->fd, n);
}

static void switch_port_send_nd6(switch_port_t port, uint8_t type,
                                 const ether_addr_t *sender_hw,
                                 struct in6_addr *sender_ip,
                                 const ether_addr_t *dest_ether,
                                 const ether_addr_t *target_hw,
  ...
  switch_port_check_tx(port);
  frame_length =
      ethernet_nd6_frame_populate(&pkt, sizeof(pkt), type, sender_hw, sender_ip,
                                  dest_ether, target_hw, target_ip);
  T_QUIET;
  T_ASSERT_GT(frame_length, 0, "%s: frame_length %u", __func__, frame_length);
  if (G_debug) {
    T_LOG("Port %s -> %s transmitting %u bytes", port->ifname,
...
static void switch_port_send_udp(switch_port_t port, uint8_t af,
                                 const ether_addr_t *src_eaddr,
                                 union ifbrip *src_ip, uint16_t src_port,
                                 const ether_addr_t *dst_eaddr,
                                 union ifbrip *dst_ip, uint16_t dst_port,
  ...
  /* generate the packet */
  frame_length = ethernet_udp_frame_populate(
      (void *)&pkt, (u_int)sizeof(pkt), af, src_eaddr, src_ip, src_port,
      dst_eaddr, dst_ip, dst_port, payload, payload_length);
  T_QUIET;
  T_ASSERT_GT(frame_length, 0, NULL);
  if (G_debug) {
    T_LOG("Port %s transmitting %u bytes", port->ifname, frame_length);
  }
  ethernet_frame_validate(&pkt, frame_length, G_debug);
  n = write(port->fd, &pkt, frame_length);
  if (n < 0) {
...
static void switch_port_send_udp_addr_index(switch_port_t port, uint8_t af,
                                            u_int addr_index,
                                            const ether_addr_t *dst_eaddr,
                                            union ifbrip *dst_ip,
                                            const void *payload,
  ...
  switch_port_send_udp(port, af, &eaddr, &ip, TEST_SOURCE_PORT, dst_eaddr,
                       dst_ip, TEST_DEST_PORT, payload, payload_length);
}

typedef void(packet_validator)(switch_port_t port, const ether_header_t *eh_p,
                               u_int pkt_len, void *context);
typedef packet_validator *packet_validator_t;

static void switch_port_receive(switch_port_t port, uint8_t af,
                                const void *payload, u_int payload_length,
                                packet_validator_t validator, void *context) {
  ether_header_t *eh_p;
  ssize_t n;
  char *offset;
  ...
  if (n < 0) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(n, "read %s port %d fd %d", port->ifname, port->unit,
                           port->fd);
    return;
  }
  for (offset = port->rx_buf; n > 0;) {
    ...
    T_QUIET;
    T_ASSERT_GE(pkt_len, (u_int)sizeof(*eh_p), "short packet %ld", n);

    /* source shouldn't be broadcast/multicast */
    T_QUIET;
    T_ASSERT_EQ(eh_p->ether_shost[0] & 0x01, 0, "broadcast/multicast source");

    if (G_debug) {
      T_LOG("Port %s [unit %d] [fd %d] Received %u bytes", port->ifname,
            port->unit, port->fd, pkt_len);
    }
    ...
    if (payload != NULL) {
      ...
      p = ethernet_frame_get_udp_payload(af, pkt, pkt_len, &p_len);
      T_QUIET;
      T_ASSERT_NOTNULL(p, "ethernet_frame_get_udp_payload");
      T_QUIET;
      T_ASSERT_EQ(p_len, payload_length, "payload length %u < expected %u",
                  p_len, payload_length);
      T_QUIET;
      T_ASSERT_EQ(bcmp(payload, p, payload_length), 0, "unexpected payload");
    }
...
static void switch_port_log(switch_port_t port) {
  T_LOG("%s [unit %d] [member %s]%s bpf fd %d bufsize %d\n", port->ifname,
        port->unit, port->member_ifname, port->mac_nat ? " [mac-nat]" : "",
...
static switch_port_list_t switch_port_list_alloc(u_int port_count,
                                                 bool mac_nat) {
  switch_port_list_t list;

  list = (switch_port_list_t)calloc(1, switch_port_list_size(port_count));
  ;
...
static void switch_port_list_dealloc(switch_port_list_t list) {
  u_int i;
  switch_port_t port;
...
static errno_t switch_port_list_add_port(switch_port_list_t port_list,
                                         u_int unit, const char *ifname,
                                         u_short if_index,
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(bpf_setif(fd, ifname), "bpf set if %s", ifname);
  T_QUIET;
...
static switch_port_t switch_port_list_find_fd(switch_port_list_t ports,
                                              int fd) {
  u_int i;
  switch_port_t port;
...
static void switch_port_list_log(switch_port_list_t port_list) {
  u_int i;
  switch_port_t port;
...
static switch_port_t switch_port_list_find_member(switch_port_list_t ports,
                                                  const char *member_ifname) {
  u_int i;
  switch_port_t port;
...
static void switch_port_list_check_receive(switch_port_list_t ports, uint8_t af,
                                           const void *payload,
                                           u_int payload_length,
  ...
  for (u = 0, port = ports->list; u < ports->count; u++, port++) {
    port->test_count = 0;
    EV_SET(kev + u, port->fd, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, NULL);
  }

  do {
    n_events = kevent(kq, kev, (int)ports->count, kev, (int)ports->count, &ts);
    T_QUIET;
    ...
    for (i = 0; i < n_events; i++) {
      ...
      port = switch_port_list_find_fd(ports, (int)kev[i].ident);
      T_QUIET;
      T_ASSERT_NE(port, NULL, "port %p fd %d", (void *)port, (int)kev[i].ident);
      switch_port_receive(port, af, payload, payload_length, validator,
                          context);
    }
  } while (n_events != 0);
  close(kq);
...
static bool switch_port_list_verify_rt_table(switch_port_list_t port_list,
                                             bool log) {
  ...
  for (i = 0, ifba = rt_table; i < count; i++, ifba++) {
    ...
    port = switch_port_list_find_member(port_list, ifba->ifba_ifsname);
    T_QUIET;
    ...
    if (!S_cleaning_up) {
      ...
      T_QUIET;
      T_ASSERT_BITS_NOTSET(port->test_address_present, addr_bit,
                           "%s address %u", ifba->ifba_ifsname, addr_index);
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    if (G_debug) {
      T_LOG("%s unit %d [member %s] %u expect %u", port->ifname, port->unit,
            port->member_ifname, port->test_address_count, port->num_addrs);
    }
...
static bool switch_port_list_verify_mac_nat(switch_port_list_t port_list,
                                            bool log) {
  bool all_present = true;
  u_int i;
  u_int count;
  ...
  for (i = 0, scan = entries; i < count; i++, scan++) {
    ...
    addr_index = ea[EA_ADDR_INDEX];
    unit_index = ea[EA_UNIT_INDEX];
    port = switch_port_list_find_member(port_list, scan->ifbmne_ifname);
    T_QUIET;
    T_ASSERT_NOTNULL(port, "switch_port_list_find_member %s",
                     scan->ifbmne_ifname);
    T_QUIET;
    ...
    T_QUIET;
    T_ASSERT_TRUE(
        ip_addresses_are_equal(af, &ip, &scan->ifbmne_ip),
        "mac nat entry IP address %s expected %s",
        inet_ntop(af, &scan->ifbmne_ip_addr, buf_ip1, sizeof(buf_ip1)),
        inet_ntop(af, &ip, buf_ip2, sizeof(buf_ip2)));
    T_QUIET;
    T_ASSERT_BITS_NOTSET(port->test_address_present, addr_bit, "%s address %u",
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    if (port->mac_nat) {
      ...
      T_ASSERT_EQ(port->test_address_count, 0,
                  "mac nat interface %s has %u entries", port->member_ifname,
                  port->test_address_count);
    } else {
      if (G_debug) {
        T_LOG("%s unit %d [member %s] %u expect %u", port->ifname, port->unit,
...
** Basic Bridge Tests
**/
static void send_generation(switch_port_t port, uint8_t af, u_int addr_index,
...
static void check_receive_generation(switch_port_list_t ports, uint8_t af,
                                     uint32_t generation,
                                     packet_validator_t validator,
...
static void validate_source_ether_mismatch(switch_port_t port,
                                           const ether_header_t *eh_p) {
...
static void validate_not_present_dhost(switch_port_t port,
                                       const ether_header_t *eh_p,
  ...
  T_QUIET;
  T_ASSERT_EQ(
      bcmp(eh_p->ether_dhost, &ether_external, sizeof(eh_p->ether_dhost)), 0,
      "%s", __func__);
  port->test_count++;
}

static void validate_broadcast_dhost(switch_port_t port,
                                     const ether_header_t *eh_p,
                                     __unused u_int pkt_len,
                                     __unused void *context) {
  validate_source_ether_mismatch(port, eh_p);
  T_QUIET;
  T_ASSERT_NE((eh_p->ether_dhost[0] & 0x01), 0, "%s", __func__);
  port->test_count++;
}

static void validate_port_dhost(switch_port_t port, const ether_header_t *eh_p,
                                __unused u_int pkt_len,
                                __unused void *context) {
  validate_source_ether_mismatch(port, eh_p);
  T_QUIET;
  T_ASSERT_EQ(eh_p->ether_dhost[EA_UNIT_INDEX], port->unit,
              "wrong dhost unit %d != %d", eh_p->ether_dhost[EA_UNIT_INDEX],
              port->unit);
  port->test_count++;
}

static void check_received_count(switch_port_list_t port_list,
                                 switch_port_t port,
  ...
  for (i = 0, scan = port_list->list; i < port_list->count; i++, scan++) {
    if (scan == port) {
      T_QUIET;
      T_ASSERT_EQ(port->test_count, 0, "unexpected receive on port %d",
                  port->unit);
...
static void unicast_send_all(switch_port_list_t port_list, uint8_t af,
                             switch_port_t port) {
  u_int i;
  switch_port_t scan;
  ...
  for (i = 0, scan = port_list->list; i < port_list->count; i++, scan++) {
    ...
    for (u_int j = 0; j < scan->num_addrs; j++) {
      ...
      switch_port_send_udp_addr_index(port, af, 0, &eaddr, &ip, NULL, 0);
    }
  }
}

static void bridge_learning_test_once(switch_port_list_t port_list, uint8_t af,
                                      packet_validator_t validator,
                                      void *context,
                                      const ether_addr_t *dst_eaddr,
                                      bool retry) {
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    ...
    for (u_int j = 0; j < port->num_addrs; j++) {
      ...
      if (retry) {
        ...
        addr_bit = 1 << j;
        if ((port->test_address_present & addr_bit) != 0) {
          /* already present */
          continue;
        }
        T_LOG("Retry port %s unit %u address %u", port->ifname, port->unit, j);
      }
      generation = next_generation();
      send_generation(port, af, j, dst_eaddr, &dst_ip, generation);

      /* receive across all ports */
      check_receive_generation(port_list, af, generation, validator, context);
...
static void bridge_learning_test(switch_port_list_t port_list, uint8_t af,
                                 packet_validator_t validator, void *context,
                                 const ether_addr_t *dst_eaddr) {
...
#define BROADCAST_MAX_TRIES 20
  for (int try = 1; try < BROADCAST_MAX_TRIES; try++) {
    ...
    if (!retry) {
      T_LOG("%s: %s #ports %u #addrs %u dest %s", __func__, af_get_str(af),
            port_list->count, port_list->list->num_addrs, ntoabuf);
    } else {
      T_LOG("%s: %s #ports %u #addrs %u dest %s (TRY=%d)", __func__,
            af_get_str(af), port_list->count, port_list->list->num_addrs,
            ntoabuf, try);
    }
    bridge_learning_test_once(port_list, af, validator, context, dst_eaddr,
                              retry);
    /*
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    ...
    switch_port_list_check_receive(port_list, af, NULL, 0, validate_port_dhost,
                                   NULL);
...
** MAC-NAT tests
**/
static void mac_nat_check_received_count(switch_port_list_t port_list,
                                         switch_port_t port) {
  u_int i;
  switch_port_t scan;
  ...
  for (i = 0, scan = port_list->list; i < port_list->count; i++, scan++) {
    ...
    T_ASSERT_EQ(scan->test_count, expected,
                "%s [member %s]%s expected %u actual %u", scan->ifname,
                scan->member_ifname, scan->mac_nat ? " [mac-nat]" : "",
                expected, scan->test_count);
  }
}

static void validate_mac_nat(switch_port_t port, const ether_header_t *eh_p,
                             __unused u_int pkt_len, __unused void *context) {
  if (port->mac_nat) {
    bool equal;
...
static void validate_mac_nat_in(switch_port_t port, const ether_header_t *eh_p,
  ...
  T_QUIET;
  T_ASSERT_EQ(eh_p->ether_dhost[EA_UNIT_INDEX], port->unit,
              "dhost unit %u expected %u", eh_p->ether_dhost[EA_UNIT_INDEX],
              port->unit);
  port->test_count++;
}

static void validate_mac_nat_arp_out(switch_port_t port,
                                     const ether_header_t *eh_p, u_int pkt_len,
                                     void *context) {
  ...
  if (port->mac_nat) {
    ...
    T_ASSERT_TRUE(equal, "%s -> %s source address translated",
                  send_port->member_ifname, port->member_ifname);
    /* sender hw must match MAC-NAT interface */
    equal =
        (bcmp(earp->arp_sha, &port->member_mac, sizeof(port->member_mac)) == 0);
    if (!equal) {
      ethernet_frame_validate(eh_p, pkt_len, true);
    }
    T_QUIET;
    ...
    T_ASSERT_EQ(eh_p->ether_shost[EA_UNIT_INDEX], send_port->unit,
                "%s -> %s unit %u expected %u", send_port->member_ifname,
                port->member_ifname, eh_p->ether_shost[EA_UNIT_INDEX],
                send_port->unit);
    /* source hw must match the sender */
    T_QUIET;
    T_ASSERT_EQ(earp->arp_sha[EA_UNIT_INDEX], send_port->unit,
                "%s -> %s unit %u expected %u", send_port->member_ifname,
                port->member_ifname, earp->arp_sha[EA_UNIT_INDEX],
                send_port->unit);
  }
  port->test_count++;
}

static void validate_mac_nat_arp_in(switch_port_t port,
                                    const ether_header_t *eh_p, u_int pkt_len,
                                    void *context) {
  const struct ether_arp *earp;
  switch_port_t send_port = (switch_port_t)context;
  ...
  T_ASSERT_EQ(eh_p->ether_dhost[EA_UNIT_INDEX], port->unit,
              "%s -> %s unit %u expected %u", send_port->member_ifname,
              port->member_ifname, eh_p->ether_dhost[EA_UNIT_INDEX],
              port->unit);
  /* source hw must match the sender */
  T_QUIET;
  T_ASSERT_EQ(earp->arp_tha[EA_UNIT_INDEX], port->unit,
              "%s -> %s unit %u expected %u", send_port->member_ifname,
              port->member_ifname, earp->arp_tha[EA_UNIT_INDEX], port->unit);
  port->test_count++;
}

static void mac_nat_test_arp_out(switch_port_list_t port_list) {
  u_int i;
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    ...
    for (u_int j = 0; j < port->num_addrs; j++) {
      ...
      switch_port_send_arp(port, ARPOP_REQUEST, &eaddr, ip_src, NULL, ip_dst);
      switch_port_list_check_receive(port_list, AF_INET, NULL, 0,
                                     validate_mac_nat_arp_out, port);
      check_received_count(port_list, port, 1);
    }
  }
  T_PASS("%s", __func__);
...
static void mac_nat_send_arp_response(switch_port_t ext_port,
                                      switch_port_t port) {
  struct in_addr ip_src;
  ...
  for (u_int j = 0; j < port->num_addrs; j++) {
    ...
    if (G_debug) {
      T_LOG("Generating ARP destined to %s %s", port->ifname,
            inet_ntoa(ip_dst));
    }
    switch_port_send_arp(ext_port, ARPOP_REPLY, &ether_external, ip_src,
                         &ext_port->member_mac, ip_dst);
  }
}

static void mac_nat_test_arp_in(switch_port_list_t port_list) {
  u_int i;
  switch_port_t port;
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    ...
    switch_port_list_check_receive(port_list, AF_INET, NULL, 0,
                                   validate_mac_nat_arp_in, port_list->list);
...
static void validate_mac_nat_dhcp(switch_port_t port,
                                  const ether_header_t *eh_p, u_int pkt_len,
                                  void *context) {
  u_int dp_flags;
  ...
  if (port->mac_nat) {
    ...
    T_ASSERT_BITS_SET(dp_flags, (u_int)DHCP_FLAGS_BROADCAST,
                      ...
                      send_port->member_ifname, port->member_ifname, dp_flags,
...
static void mac_nat_test_dhcp(switch_port_list_t port_list,
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    ...
    if (G_debug) {
      T_LOG("%s: transmit DHCP packet (member %s)", port->ifname,
            port->member_ifname);
    }
    switch_port_send_udp(port, AF_INET, &eaddr, (union ifbrip *)&ip_src,
                         BOOTP_CLIENT_PORT, ether_dst, (union ifbrip *)&ip_dst,
                         BOOTP_SERVER_PORT, &payload, payload_len);

    switch_port_list_check_receive(port_list, AF_INET, NULL, 0,
                                   validate_mac_nat_dhcp, port);
  ...
  T_PASS("%s %s", __func__, link_layer_unicast ? "unicast" : "broadcast");
}

static void validate_mac_nat_nd6(switch_port_t port,
                                 const struct icmp6_hdr *icmp6, u_int icmp6_len,
                                 uint8_t opt_type, u_int nd_hdr_size,
  ...
  T_ASSERT_EQ(nd_opt->nd_opt_type, opt_type,
              "nd_opt->nd_opt_type 0x%x, opt_type 0x%x", nd_opt->nd_opt_type,
              opt_type);
  T_QUIET;
  ...
  if (port->mac_nat) {
    ...
    equal = (bcmp(linkaddr, &port->member_mac, sizeof(port->member_mac)) == 0);
    T_QUIET;
    T_ASSERT_TRUE(equal, "%s -> %s sender hardware translated",
                  send_port->member_ifname, port->member_ifname);
  } else {
    ...
    T_ASSERT_EQ(linkaddr[EA_UNIT_INDEX], send_port->unit,
                "%s -> %s unit %u expected %u", send_port->member_ifname,
                port->member_ifname, linkaddr[EA_UNIT_INDEX], send_port->unit);
  }
}

static void validate_mac_nat_icmp6_out(switch_port_t port,
                                       const struct icmp6_hdr *icmp6,
                                       u_int icmp6_len,
                                       switch_port_t send_port) {
  switch (icmp6->icmp6_type) {
  case ND_NEIGHBOR_ADVERT:
    validate_mac_nat_nd6(port, icmp6, icmp6_len, ND_OPT_TARGET_LINKADDR,
                         sizeof(struct nd_neighbor_advert), send_port);
    break;
  case ND_NEIGHBOR_SOLICIT:
    validate_mac_nat_nd6(port, icmp6, icmp6_len, ND_OPT_SOURCE_LINKADDR,
                         sizeof(struct nd_neighbor_solicit), send_port);
    break;
  case ND_ROUTER_SOLICIT:
    validate_mac_nat_nd6(port, icmp6, icmp6_len, ND_OPT_SOURCE_LINKADDR,
                         sizeof(struct nd_router_solicit), send_port);
    break;
  default:
...
static void validate_mac_nat_nd6_out(switch_port_t port,
                                     const ether_header_t *eh_p, u_int pkt_len,
                                     void *context) {
  const struct icmp6_hdr *icmp6;
  const struct ip6_hdr *ip6;
...
static void mac_nat_test_nd6_out(switch_port_list_t port_list) {
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    ...
    for (u_int j = 0; j < port->num_addrs; j++) {
      ...
      switch_port_send_nd6(port, ND_NEIGHBOR_SOLICIT, &eaddr,
                           &ip_src.ifbrip_addr6, NULL, NULL,
                           &ip_dst.ifbrip_addr6);
      switch_port_list_check_receive(port_list, AF_INET, NULL, 0,
                                     validate_mac_nat_nd6_out, port);
      check_received_count(port_list, port, 1);
    }
    /* neighbor advert */
    for (u_int j = 0; j < port->num_addrs; j++) {
      ...
      set_ethernet_address(&eaddr, port->unit, j);
      get_ip_address(AF_INET6, port->unit, j, &ip_src);
      switch_port_send_nd6(port, ND_NEIGHBOR_ADVERT, &eaddr,
                           &ip_src.ifbrip_addr6, NULL, &eaddr,
                           &ip_src.ifbrip_addr6);
      switch_port_list_check_receive(port_list, AF_INET, NULL, 0,
                                     validate_mac_nat_nd6_out, port);
      check_received_count(port_list, port, 1);
    ...
    for (u_int j = 0; j < port->num_addrs; j++) {
      ...
      switch_port_send_nd6(port, ND_ROUTER_SOLICIT, &eaddr,
                           &ip_src.ifbrip_addr6, NULL, NULL, NULL);
      switch_port_list_check_receive(port_list, AF_INET, NULL, 0,
                                     validate_mac_nat_nd6_out, port);
      check_received_count(port_list, port, 1);
    }
...
static void mac_nat_send_response(switch_port_t ext_port, uint8_t af,
                                  switch_port_t port) {
  union ifbrip src_ip;
  ...
  for (u_int j = 0; j < port->num_addrs; j++) {
    ...
    switch_port_send_udp(ext_port, af, &ether_external, &src_ip, TEST_DEST_PORT,
                         &ext_port->member_mac, &ip, TEST_SOURCE_PORT, NULL, 0);
  }
}

static void mac_nat_test_ip_once(switch_port_list_t port_list, uint8_t af,
                                 bool retry) {
  union ifbrip dst_ip;
  u_int i;
  switch_port_t port;
  ...
  for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
    ...
    for (u_int j = 0; j < port->num_addrs; j++) {
      ...
      generation = next_generation();
      send_generation(port, af, j, &ether_external, &dst_ip, generation);

      /* receive across all ports */
      check_receive_generation(port_list, af, generation, validate_mac_nat,
                               NULL);
...
static void mac_nat_test_ip(switch_port_list_t port_list, uint8_t af) {
  u_int i;
  switch_port_t port;
  bool verified = false;
...
#define MAC_NAT_MAX_TRIES 20
  for (int try = 1; try < BROADCAST_MAX_TRIES; try++) {
    ...
    if (!retry) {
      T_LOG("%s: #ports %u #addrs %u", __func__, port_list->count,
            port_list->list->num_addrs);
    } else {
      T_LOG("%s: #ports %u #addrs %u destination (TRY=%d)", __func__,
            port_list->count, port_list->list->num_addrs, try);
    }
...
static int bridge_delete_member(const char *bridge, const char *member) {
  struct ifbreq req;
  int ret;
...
static int bridge_member_modify_ifflags(const char *bridge, const char *member,
  ...
  if (need_set) {
    ret = siocdrvspec(bridge, BRDGSIFFLGS, &req, sizeof(req), true);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "BRDGSIFFLGS %s %s 0x%x => 0x%x", bridge,
                           member, flags, req.ifbr_ifsflags);
  }
  return ret;
}

static int bridge_member_modify_mac_nat(const char *bridge, const char *member,
                                        bool enable) {
  return bridge_member_modify_ifflags(bridge, member, IFBIF_MAC_NAT, enable);
}

static int bridge_member_modify_checksum_offload(const char *bridge,
                                                 const char *member,
                                                 bool enable) {
...
#define IFBIF_CHECKSUM_OFFLOAD                                                 \
  0x10000 /* checksum inbound packets,                                         \
           ...
           */
#endif
  return bridge_member_modify_ifflags(bridge, member, IFBIF_CHECKSUM_OFFLOAD,
                                      enable);
}

static struct ifbareq *bridge_rt_table_copy_common(const char *bridge,
                                                   u_int *ret_count) {
  ...
  for (;;) {
    ...
    ret = siocdrvspec(bridge, BRDGRTS, &ifbac, sizeof(ifbac), false);
    T_QUIET;
...
static void bridge_rt_table_log(struct ifbareq *rt_table, u_int count) {
  ...
  for (i = 0, ifba = rt_table; i < count; i++, ifba++) {
    ether_ntoa_buf((const ether_addr_t *)&ifba->ifba_dst, ntoabuf,
                   sizeof(ntoabuf));
    T_LOG("%s %s %lu", ifba->ifba_ifsname, ntoabuf, ifba->ifba_expire);
  }
  return;
}

static struct ifbrmne *bridge_mac_nat_entries_copy_common(const char *bridge,
                                                          u_int *ret_count) {
  char *buf = NULL;
  u_int count = 0;
  int err;
  ...
  T_ASSERT_GE(mnl.ifbml_elsize, (uint16_t)sizeof(struct ifbrmne),
              "mac nat entry size %u minsize %u", mnl.ifbml_elsize,
              (u_int)sizeof(struct ifbrmne));
  if (mnl.ifbml_len == 0) {
    goto done;
  }
...
static void bridge_mac_nat_entries_log(struct ifbrmne *entries, u_int count) {
  ...
  for (i = 0, scan = entries; i < count; i++, scan++) {
    ether_ntoa_buf((const ether_addr_t *)&scan->ifbmne_mac, ntoabuf,
                   sizeof(ntoabuf));
    inet_ntop(scan->ifbmne_af, &scan->ifbmne_ip, ntopbuf, sizeof(ntopbuf));
...
static void bridge_cleanup(const char *bridge, u_int n_ports,
                           bool fail_on_error);
...
static void fake_set_trailers_fcs(bool enable) {
  int error;
  int fcs;
  size_t len;
  int trailer_length;
  ...
  /* set fcs */
  len = sizeof(fake_fcs);
  error = sysctlbyname("net.link.fake.fcs", &fake_fcs, &len, &fcs, sizeof(fcs));
  T_ASSERT_EQ(error, 0, "sysctl net.link.fake.fcs %d", fcs);

  /* set trailer_length */
  len = sizeof(fake_trailer_length);
  error = sysctlbyname("net.link.fake.trailer_length", &fake_trailer_length,
                       &len, &trailer_length, sizeof(trailer_length));
  T_ASSERT_EQ(error, 0, "sysctl net.link.fake.trailer_length %d",
              trailer_length);
}

static void fake_restore_trailers_fcs(void) {
  int error;

  error =
      sysctlbyname("net.link.fake.fcs", NULL, 0, &fake_fcs, sizeof(fake_fcs));
  T_LOG("sysctl net.link.fake.fcs=%d returned %d", fake_fcs, error);
  error = sysctlbyname("net.link.fake.trailer_length", NULL, 0,
                       &fake_trailer_length, sizeof(fake_trailer_length));
  T_LOG("sysctl net.link.fake.trailer_length=%d returned %d",
        fake_trailer_length, error);
}

static void fake_set_bsd_mode(bool enable) {
  int error;
  int bsd_mode;
  size_t len;
  ...
  error = sysctlbyname("net.link.fake.bsd_mode", &fake_bsd_mode, &len,
                       &bsd_mode, sizeof(bsd_mode));
  T_ASSERT_EQ(error, 0, "sysctl net.link.fake.bsd_mode %d", bsd_mode);
}

static void fake_restore_bsd_mode(void) {
  int error;

  error = sysctlbyname("net.link.fake.bsd_mode", NULL, 0, &fake_bsd_mode,
                       sizeof(fake_bsd_mode));
  T_LOG("sysctl net.link.fake.bsd_mode=%d returned %d", fake_bsd_mode, error);
}

static void fake_set_lro(bool enable) {
  int error;
  int lro;

  lro = (enable) ? 1 : 0;
  error = sysctlbyname("net.link.fake.lro", NULL, 0, &lro, sizeof(lro));
  T_ASSERT_EQ(error, 0, "sysctl net.link.fake.lro %d", lro);
}

static void cleanup_common(bool dump_table) {
  if (S_n_ports == 0) {
    return;
  }
  S_cleaning_up = true;
  if (S_port_list != NULL && (S_port_list->mac_nat || dump_table)) {
...
static void sigint_handler(__unused int sig) {
  cleanup_common(false);
  signal(SIGINT, SIG_DFL);
}

static switch_port_list_t bridge_setup(char *bridge, u_int n_ports,
                                       u_int num_addrs, uint8_t setup_flags) {
  u_int addr_index = 1;
  bool attach_stack;
  ...
  if (attach_stack) {
    ...
    addr_index++;
    ifnet_add_ip_address(bridge, bridge_ip_addr, inet_class_c_subnet_mask);
    ifnet_start_ipv6(bridge);
    get_ipv6_ll_address(&bridge_mac, &bridge_ipv6_addr);
    inet_ntop(AF_INET6, &bridge_ipv6_addr, ntopbuf_ip, sizeof(ntopbuf_ip));
  ...
  for (u_int i = 0; i < n_ports; i++) {
    ...
    snprintf(ifname, sizeof(ifname), "%s%d", FETH_NAME, i);
    snprintf(member_ifname, sizeof(member_ifname), "%s%d", FETH_NAME,
             i + n_ports);
    ...
    if (attach_stack) {
      ...
      ifnet_add_ip_address(ifname, ip, inet_class_c_subnet_mask);
      route_add_inet_scoped_subnet(ifname, if_index, ip,
    ...
    if (do_mac_nat) {
      ...
      err = bridge_member_modify_mac_nat(bridge, member_ifname, true);
      if (err != 0) {
        goto done;
      }
    } else if (checksum_offload) {
      err = bridge_member_modify_checksum_offload(bridge, member_ifname, true);
      if (err != 0) {
        goto done;
      }
    }
    /* we'll send/receive on the interface */
    err = switch_port_list_add_port(list, i, ifname, if_index, member_ifname,
                                    num_addrs, do_mac_nat, &ip);
    if (err != 0) {
      goto done;
    }
  }
...
static void bridge_test(packet_validator_t validator, void *context,
                        const ether_addr_t *dst_eaddr, uint8_t af,
                        u_int n_ports, u_int num_addrs) {
#if TARGET_OS_BRIDGE
...
static void bridge_test_mac_nat_ipv4(u_int n_ports, u_int num_addrs) {
#if TARGET_OS_BRIDGE
  T_SKIP("Test uses too much memory");
#else  /* TARGET_OS_BRIDGE */
  switch_port_list_t port_list;
  ...
  signal(SIGINT, sigint_handler);
  port_list = bridge_setup(BRIDGE200, n_ports, num_addrs, SETUP_FLAGS_MAC_NAT);
  if (port_list == NULL) {
    T_FAIL("bridge_setup");
    return;
  }
...
static void bridge_test_mac_nat_ipv6(u_int n_ports, u_int num_addrs,
                                     uint8_t flags) {
#if TARGET_OS_BRIDGE
  T_SKIP("Test uses too much memory");
#else  /* TARGET_OS_BRIDGE */
 ...
 */
static void system_cmd(const char *cmd, bool fail_on_error) {
  pid_t pid = -1;
  int exit_status = 0;
  const char *argv[] = {"/usr/local/bin/bash", "-c", cmd, NULL};
...
static bool executable_is_present(const char *path) {
  struct stat statb = {0};
...
static void cleanup_pf(void) {
  ...
  siocdrvspec(BRIDGE200, BRDGSFILT, &param, sizeof(param), true);
  return;
}

static void block_all_traffic(bool input, const char *infname1,
                              const char *infname2) {
  int ret;
  struct ifbrparam param;
  char command[512];
  ...
  ret = siocdrvspec(BRIDGE200, BRDGSFILT, &param, sizeof(param), true);
  T_ASSERT_POSIX_SUCCESS(ret, "SIOCDRVSPEC(BRDGSFILT %s, 0x%x)", BRIDGE200,
                         param.ifbrp_filter);
...
static void filter_test(uint8_t af) {
#if TARGET_OS_BRIDGE
...
#define PFCTL_PATH "/sbin/pfctl"
    ...
    do {
      ...
      for (i = 0, port = port_list->list; i < port_list->count; i++, port++) {
        ...
        for (u_int j = 0; j < port->num_addrs; j++) {
          ...
          generation = next_generation();
          send_generation(port, af, j, &ether_broadcast, &dst_ip, generation);

          /* receive across all ports */
          check_receive_generation(port_list, af, generation,
                                   validate_broadcast_dhost, NULL);
      ...
      T_PASS("%s broadcast %s %s", __func__,
             blocked ? "blocked" : "not blocked", input ? "input" : "output");
      input = !input;
      cleanup_pf();
  ...
  do {
    do {
      ...
      T_PASS("%s unicast %s %s", __func__, blocked ? "blocked" : "not blocked",
             input ? "input" : "output");
      input = !input;
      cleanup_pf();
    } while (input == false && blocked);
...
static void test_traffic_for_af(switch_port_list_t ports, uint8_t af) {
  u_int i;
  inet_address server;
  int server_if_index;
  ...
  for (i = 0, port = ports->list; i < ports->count; i++, port++) {
    inet_test_traffic(af, &server, server_name, server_if_index, port->ifname,
                      port->if_index);
  }
...
static void bridge_test_transfer(u_int n_ports, uint8_t setup_flags) {
#if TARGET_OS_BRIDGE
  T_SKIP("Test uses too much memory");
#else  /* TARGET_OS_BRIDGE */
  switch_port_list_t port_list;
...
static void lro_test_cleanup(void) {
  ifnet_destroy(BRIDGE200, false);
  ifnet_destroy(FETH0, false);
  fake_set_lro(false);
}

static void sigint_lro_cleanup(__unused int sig) {
  signal(SIGINT, SIG_DFL);
  lro_test_cleanup();
}

static void verify_lro_capability(const char *if_name, bool expected) {
  struct ifreq ifr;
  int result;
  bool lro_enabled;
  ...
  T_ASSERT_EQ(expected, lro_enabled, "%s %s expected %s", __func__, if_name,
              expected ? "enabled" : "disabled");
}

static void bridge_test_lro_disable(void) {
  int err;
...
T_DECL(net_if_bridge_bcast, "bridge broadcast IPv4", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED) {
  bridge_test(validate_broadcast_dhost, NULL, &ether_broadcast, AF_INET, 5, 1);
}

T_DECL(net_if_bridge_bcast_many, "bridge broadcast many IPv4",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  bridge_test(validate_broadcast_dhost, NULL, &ether_broadcast, AF_INET, 5, 20);
}

T_DECL(net_if_bridge_unknown, "bridge unknown host IPv4", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED) {
  bridge_test(validate_not_present_dhost, NULL, &ether_external, AF_INET, 5, 1);
}

T_DECL(net_if_bridge_bcast_v6, "bridge broadcast IPv6", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED) {
  bridge_test(validate_broadcast_dhost, NULL, &ether_broadcast, AF_INET6, 5, 1);
}

T_DECL(net_if_bridge_bcast_many_v6, "bridge broadcast many IPv6",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  bridge_test(validate_broadcast_dhost, NULL, &ether_broadcast, AF_INET6, 5,
              20);
}

T_DECL(net_if_bridge_unknown_v6, "bridge unknown host IPv6",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  bridge_test(validate_not_present_dhost, NULL, &ether_external, AF_INET6, 5,
              1);
}

T_DECL(net_if_bridge_mac_nat_ipv4, "bridge mac nat ipv4", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED) {
  bridge_test_mac_nat_ipv4(5, 10);
}

T_DECL(net_if_bridge_mac_nat_ipv6, "bridge mac nat ipv6", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED, T_META_ENABLED(false /* rdar://133955717 */)) {
  bridge_test_mac_nat_ipv6(5, 10, 0);
}

T_DECL(net_if_bridge_mac_nat_ipv6_trailers, "bridge mac nat ipv6 trailers",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED,
       T_META_ENABLED(false /* rdar://133955717 */)) {
  bridge_test_mac_nat_ipv6(5, 10, SETUP_FLAGS_TRAILERS);
}

T_DECL(net_if_bridge_filter_ipv4, "bridge filter ipv4", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED) {
  filter_test(AF_INET);
}

T_DECL(net_if_bridge_filter_ipv6, "bridge filter ipv6", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED) {
  filter_test(AF_INET6);
}

T_DECL(net_if_bridge_checksum_offload, "bridge checksum offload",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  bridge_test_transfer(2, SETUP_FLAGS_CHECKSUM_OFFLOAD);
}

T_DECL(net_if_bridge_checksum_offload_trailers,
       "bridge checksum offload with trailers+fcs", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED) {
  bridge_test_transfer(2, SETUP_FLAGS_CHECKSUM_OFFLOAD | SETUP_FLAGS_TRAILERS);
}

T_DECL(net_if_bridge_transfer, "bridge transfer", T_META_ASROOT(true)) {
  bridge_test_transfer(2, 0);
}

T_DECL(net_if_bridge_transfer_share_mac, "bridge transfer share member's MAC",
       T_META_ASROOT(true)) {
  bridge_test_transfer(2, SETUP_FLAGS_SHARE_MEMBER_MAC);
}

T_DECL(net_if_bridge_lro_disable, "bridge LRO disable", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED) {
