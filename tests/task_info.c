 ...
 * <rdar://problem/22242021> Add tests to increase code coverage for the
 * task_info API
 * *************************************************************************************
 */
...
T_DECL(task_vm_info, "tests task vm info", T_META_ASROOT(true),
  ...
  err = task_info(mach_task_self(), TASK_VM_INFO_PURGEABLE,
                  (task_info_t)&vm_info, &count);

  T_ASSERT_MACH_SUCCESS(err, "verify task_info call succeeded");

  T_EXPECT_NE(vm_info.virtual_size, 0ULL,
              "task_info return value !=0 for virtual_size\n");

  T_EXPECT_NE(vm_info.phys_footprint, 0ULL,
              "task_info return value !=0 for phys_footprint\n");

  /*
   * Test the REV0 version of TASK_VM_INFO. It should not change the value of
   * phys_footprint.
   */
  ...
  T_EXPECT_EQ(count, TASK_VM_INFO_REV0_COUNT,
              "task_info count(%d) is equal to TASK_VM_INFO_REV0_COUNT", count);

  T_EXPECT_NE(vm_info.virtual_size, 0ULL,
              "task_info --rev0 call does not return 0 for virtual_size");

  T_EXPECT_EQ(
      vm_info.phys_footprint, (unsigned long long)TESTPHYSFOOTPRINTVAL,
      "task_info --rev0 call returned value %llu for vm_info.phys_footprint.  "
      "Expected %u since this value should not be "
      "modified by rev0",
      vm_info.phys_footprint, TESTPHYSFOOTPRINTVAL);

  T_EXPECT_EQ(
      vm_info.min_address, CANARY,
      "task_info --rev0 call returned value 0x%llx for vm_info.min_address. "
      "Expected 0x%llx since this value should not "
      "be modified by rev0",
      vm_info.min_address, CANARY);

  T_EXPECT_EQ(
      vm_info.max_address, CANARY,
      "task_info --rev0 call returned value 0x%llx for vm_info.max_address. "
      "Expected 0x%llx since this value should not "
      "be modified by rev0",
      vm_info.max_address, CANARY);
  ...
  T_EXPECT_EQ(count, TASK_VM_INFO_REV1_COUNT,
              "task_info count(%d) is equal to TASK_VM_INFO_REV1_COUNT", count);

  T_EXPECT_NE(vm_info.virtual_size, 0ULL,
              "task_info --rev1 call does not return 0 for virtual_size");

  T_EXPECT_NE(
      vm_info.phys_footprint, (unsigned long long)TESTPHYSFOOTPRINTVAL,
      "task_info --rev1 call returned value %llu for vm_info.phys_footprint.  "
      "Expected value is anything other than %u "
      "since this value should not be modified by rev1",
      vm_info.phys_footprint, TESTPHYSFOOTPRINTVAL);

  T_EXPECT_EQ(
      vm_info.min_address, CANARY,
      "task_info --rev1 call returned value 0x%llx for vm_info.min_address. "
      "Expected 0x%llx since this value should not "
      "be modified by rev1",
      vm_info.min_address, CANARY);
  ...
  T_EXPECT_EQ(
      vm_info.max_address, CANARY,
      "task_info --rev1 call returned value 0x%llx for vm_info.max_address. "
      "Expected 0x%llx since this value should not "
      "be modified by rev1",
      vm_info.max_address, CANARY);
  ...
  T_EXPECT_EQ(count, TASK_VM_INFO_REV2_COUNT,
              "task_info count(%d) is equal to TASK_VM_INFO_REV2_COUNT\n",
              count);

  T_EXPECT_NE(vm_info.virtual_size, 0ULL,
              "task_info --rev2 call does not return 0 for virtual_size\n");

  T_EXPECT_NE(
      vm_info.phys_footprint, (unsigned long long)TESTPHYSFOOTPRINTVAL,
      "task_info --rev2 call returned value %llu for vm_info.phys_footprint.  "
      "Expected anything other than %u since this "
      "value should be modified by rev2",
      vm_info.phys_footprint, TESTPHYSFOOTPRINTVAL);

  T_EXPECT_NE(vm_info.min_address, CANARY,
              "task_info --rev2 call returned value 0x%llx for "
              "vm_info.min_address. Expected anything other than 0x%llx since "
              "this value should be modified by rev2",
              vm_info.min_address, CANARY);

  T_EXPECT_NE(vm_info.max_address, CANARY,
              "task_info --rev2 call returned value 0x%llx for "
              "vm_info.max_address. Expected anything other than 0x%llx since "
              "this value should be modified by rev2",
              vm_info.max_address, CANARY);
  ...
  T_EXPECT_EQ(count, TASK_VM_INFO_REV4_COUNT,
              "task_info count(%d) is equal to TASK_VM_INFO_REV4_COUNT\n",
              count);

  T_EXPECT_NE(
      vm_info.phys_footprint, (unsigned long long)TESTPHYSFOOTPRINTVAL,
      "task_info --rev4 call returned value %llu for vm_info.phys_footprint.  "
      "Expected anything other than %u since this "
      "value should be modified by rev4",
      vm_info.phys_footprint, TESTPHYSFOOTPRINTVAL);

  T_EXPECT_NE(vm_info.min_address, CANARY,
              "task_info --rev4 call returned value 0x%llx for "
              "vm_info.min_address. Expected anything other than 0x%llx since "
  ...
  T_EXPECT_NE(vm_info.limit_bytes_remaining, CANARY,
              "task_info --rev4 call returned value 0x%llx for "
              "vm_info.limit_bytes_remaining. Expected anything other than "
              "0x%llx since "
              "this value should be modified by rev4",
              vm_info.limit_bytes_remaining, CANARY);
}

T_DECL(host_debug_info, "tests host debug info", T_META_ASROOT(true),
       T_META_LTEPHASE(LTE_POSTINIT)) {
  T_SETUPBEGIN;
  ...
  host = mach_host_self();
  err = host_info(host, HOST_DEBUG_INFO_INTERNAL, (host_info_t)&debug_info,
                  &count);

  T_ASSERT_MACH_SUCCESS(err, "verify host_info call succeeded");
}

T_DECL(task_debug_info, "tests task debug info", T_META_ASROOT(true),
       T_META_LTEPHASE(LTE_POSTINIT)) {
  T_SETUPBEGIN;
  int is_dev = is_development_kernel();
  T_QUIET;
  ...
  err = task_info(mach_task_self(), TASK_DEBUG_INFO_INTERNAL,
                  (task_info_t)&debug_info, &count);

  T_ASSERT_MACH_SUCCESS(err, "verify task_info call succeeded");
}

T_DECL(thread_debug_info, "tests thread debug info", T_META_ASROOT(true),
       T_META_LTEPHASE(LTE_POSTINIT)) {
  T_SETUPBEGIN;
  int is_dev = is_development_kernel();
  T_QUIET;
  ...
  err = thread_info(mach_thread_self(), THREAD_DEBUG_INFO_INTERNAL,
                    (thread_info_t)&debug_info, &count);

  T_ASSERT_MACH_SUCCESS(err, "verify task_info call succeeded");
}

static void do_factorial_task() {
  int number = 20;
  int factorial = 1;
  int i;
  for (i = 1; i <= number; i++) {
...
T_DECL(task_thread_times_info, "tests task thread times info",
       T_META_ASROOT(true), T_META_LTEPHASE(LTE_POSTINIT)) {
  T_SETUPBEGIN;
  int is_dev = is_development_kernel();
  T_QUIET;
  T_ASSERT_TRUE(is_dev, "verify development kernel is running");
  ...
  err = task_info(mach_task_self(), TASK_THREAD_TIMES_INFO,
                  (task_info_t)&thread_times_info_data, &count);
  ...
  err = task_info(mach_task_self(), TASK_THREAD_TIMES_INFO,
                  (task_info_t)&thread_times_info_data_new, &count);

  T_ASSERT_MACH_SUCCESS(err, "verify task_info call succeeded");

  /*
   * The difference is observed to be less than 30 microseconds for user_time
   * and less than 50 microseconds for system_time. This observation was done
   * for over 1000 runs.
   */

  T_EXPECT_FALSE((thread_times_info_data_new.user_time.seconds -
                  thread_times_info_data.user_time.seconds) != 0 ||
                     (thread_times_info_data_new.system_time.seconds -
                      thread_times_info_data.system_time.seconds) != 0,
                 "Tests whether the difference between thread times is greater "
  ...
  T_ASSERT_MACH_ERROR(err, KERN_INVALID_ARGUMENT,
                      "Negative test case: task_info should verify that count "
                      "is at least equal to what is defined in API.");
}

T_DECL(task_absolutetime_info, "tests task absolute time info",
       T_META_ASROOT(true), T_META_LTEPHASE(LTE_POSTINIT)) {
  T_SETUPBEGIN;
  int is_dev = is_development_kernel();
  T_QUIET;
  ...
  err = task_info(mach_task_self(), TASK_ABSOLUTETIME_INFO,
                  (task_info_t)&absolute_time_info_data, &count);
  ...
  err = task_info(mach_task_self(), TASK_ABSOLUTETIME_INFO,
                  (task_info_t)&absolute_time_info_data_new, &count);

  T_ASSERT_MACH_SUCCESS(err, "verify task_info call succeeded");

  user_time_diff = absolute_time_info_data_new.total_user -
                   absolute_time_info_data.total_user;
  system_time_diff = absolute_time_info_data_new.total_system -
                     absolute_time_info_data.total_system;

#if !defined(__arm64__)
  /*
   * On embedded devices the difference is always zero.
   * On non-embedded devices the difference occurs in this range. This was
   * observed over ~10000 runs.
   */

  T_EXPECT_FALSE(user_time_diff < ABSOLUTE_MIN_USER_TIME_DIFF ||
                     system_time_diff < ABSOLUTE_MIN_SYSTEM_TIME_DIFF,
                 "Tests whether the difference between thread times is greater "
  ...
  if (absolute_time_info_data.threads_user <= 0) {
    ...
    T_LOG("User threads time is zero. This should only happen rarely and when "
          "precise_user_time is off");

    err = sysctlbyname("kern.precise_user_kernel_time", &precise_time_val, &len,
                       NULL, 0);
    ...
    T_EXPECT_FALSE(precise_time_val,
                   "user thread time should only be zero when "
                   "precise_user_kernel_time is disabled");
  } else {
    T_PASS(
        "task_info should return non-zero value for user threads time = %llu",
        absolute_time_info_data.threads_user);
  }

#if !defined(__arm64__)
  /*
   * On iOS, system threads are always zero. On OS X this value can be some
   * large positive number. There is no real way to estimate the exact amount.
   */
  T_EXPECT_NE(
      absolute_time_info_data.threads_system, 0ULL,
      "task_info should return non-zero value for system threads time = %llu",
      absolute_time_info_data.threads_system);
#endif
...
T_DECL(task_affinity_tag_info, "tests task_affinity_tag_info",
       T_META_ASROOT(true), T_META_LTEPHASE(LTE_POSTINIT)) {
  T_SETUPBEGIN;
  int is_dev = is_development_kernel();
  T_QUIET;
  ...
  err = task_info(mach_task_self(), TASK_AFFINITY_TAG_INFO,
                  (task_info_t)&affinity_tag_info_data, &count);
   ...
   */
  T_ASSERT_FALSE(affinity_tag_info_data.min != 0 ||
                     affinity_tag_info_data.max != 0,
...
T_DECL(task_flags_info, "tests task_flags_info", T_META_ASROOT(true),
       T_META_LTEPHASE(LTE_POSTINIT)) {
  T_SETUPBEGIN;
  int is_dev = is_development_kernel();
  T_QUIET;
  ...
  err = task_info(mach_task_self(), TASK_FLAGS_INFO,
                  (task_info_t)&flags_info_data, &count);

  T_ASSERT_MACH_SUCCESS(err, "verify task_info call succeeded");

  /* Change for 32-bit arch possibility?*/
  T_ASSERT_EQ(
      (flags_info_data.flags & (unsigned int)(~(TF_LP64 | TF_64B_DATA))), 0U,
...
T_DECL(task_power_info_v2, "tests task_power_info_v2", T_META_ASROOT(true),
       T_META_LTEPHASE(LTE_POSTINIT), T_META_TAG_VM_NOT_ELIGIBLE) {
  T_SETUPBEGIN;
  int is_dev = is_development_kernel();
  T_QUIET;
  T_ASSERT_TRUE(is_dev, "verify development kernel is running");
  ...
  err = task_info(mach_task_self(), TASK_POWER_INFO_V2,
                  (task_info_t)&power_info_data_v2, &count);

  T_ASSERT_MACH_SUCCESS(err, "verify task_info call succeeded");

  T_ASSERT_LE(
      power_info_data_v2.gpu_energy.task_gpu_utilisation, 0ULL,
   ...
   */
  err = task_info(mach_task_self(), TASK_POWER_INFO_V2,
                  (task_info_t)&power_info_data_v2_new, &count);
  T_ASSERT_MACH_SUCCESS(err, "verify task_info call succeeded");
   ...
   */
  T_ASSERT_GT(power_info_data_v2_new.cpu_energy.total_user,
              power_info_data_v2.cpu_energy.total_user,
              "task_info call returns valid user time");
  T_ASSERT_GT(power_info_data_v2_new.cpu_energy.total_system,
              power_info_data_v2.cpu_energy.total_system,
              "task_info call returns valid system time");
#endif

  T_ASSERT_GE(power_info_data_v2.cpu_energy.task_interrupt_wakeups, 1ULL,
              "verify task_info call returns non-zero value for "
              "interrupt_wakeup (ret value = %llu)",
              power_info_data_v2.cpu_energy.task_interrupt_wakeups);

#if !defined(__arm64__)
  if (power_info_data_v2.cpu_energy.task_platform_idle_wakeups != 0) {
    T_LOG("task_info call returned %llu for platform_idle_wakeup",
          power_info_data_v2.cpu_energy.task_platform_idle_wakeups);
  }
#endif
  ...
  T_ASSERT_MACH_ERROR(err, KERN_INVALID_ARGUMENT,
                      "Negative test case: task_info should verify that count "
                      "is at least equal to what is defined in API. Call "
                      "returns errno %d:%s",
                      err, mach_error_string(err));
}

T_DECL(test_task_basic_info_32, "tests TASK_BASIC_INFO_32", T_META_ASROOT(true),
       T_META_LTEPHASE(LTE_POSTINIT), T_META_TAG_VM_PREFERRED) {
  test_task_basic_info(INFO_32);
}

T_DECL(test_task_basic_info_32_2, "tests TASK_BASIC_INFO_32_2",
       T_META_ASROOT(true), T_META_LTEPHASE(LTE_POSTINIT),
...
#if defined(__arm64__)
T_DECL(test_task_basic_info_64i_2, "tests TASK_BASIC_INFO_64_2",
       T_META_ASROOT(true), T_META_LTEPHASE(LTE_POSTINIT),
       T_META_TAG_VM_PREFERRED) {
  test_task_basic_info(INFO_64_2);
}
#else
T_DECL(test_task_basic_info_64, "tests TASK_BASIC_INFO_64", T_META_ASROOT(true),
       T_META_LTEPHASE(LTE_POSTINIT)) {
  test_task_basic_info(INFO_64);
}
#endif /* defined(__arm64__) */

T_DECL(test_mach_task_basic_info, "tests MACH_TASK_BASIC_INFO",
       T_META_ASROOT(true), T_META_LTEPHASE(LTE_POSTINIT)) {
...
#define AFTER 1
  ...
  suspend_count =
      (integer_t)(info_get(kind, GET_SUSPEND_COUNT, info_data[AFTER]));
  ...
  if (dt_waitpid(child_pid, &exit_status, NULL, timeout)) {
    T_ASSERT_MACH_SUCCESS(exit_status,
  ...
  resident_size_diff = info_get(kind, GET_RESIDENT_SIZE, info_data[AFTER]) -
                       info_get(kind, GET_RESIDENT_SIZE, info_data[BEFORE]);
  virtual_size_diff = info_get(kind, GET_VIRTUAL_SIZE, info_data[AFTER]) -
                      info_get(kind, GET_VIRTUAL_SIZE, info_data[BEFORE]);

  /*
   * INFO_32_2 gets the max resident size instead of the current resident size
   * 32 KB tolerance built into test.  The returned value is generally between 0
   * and 16384
   *
   * max resident size is a discrete field in INFO_MACH, so it's handled
   * differently
   */
  if (kind == INFO_32_2) {
    T_EXPECT_EQ(resident_size_diff % 4096, 0ULL,
                "verify task_info returns valid max resident_size");
    T_EXPECT_GE(resident_size_diff, 0ULL,
                "verify task_info returns non-negative max resident_size");
    T_EXPECT_GE(virtual_size_diff, (unsigned long long)PAGE_SIZE,
                "verify task_info returns valid virtual_size");
  } else {
    T_EXPECT_GE(resident_size_diff, (unsigned long long)PAGE_SIZE,
  ...
  if (kind == INFO_MACH) {
    resident_size_diff = info_get(kind, GET_MAX_RES, info_data[AFTER]) -
                         info_get(kind, GET_MAX_RES, info_data[BEFORE]);
    T_EXPECT_EQ(resident_size_diff % 4096, 0ULL,
                "verify task_info returns valid max resident_size");
    T_EXPECT_GE(resident_size_diff, 0ULL,
                "verify task_info returns non-negative max resident_size");
    T_EXPECT_GE(info_get(kind, GET_MAX_RES, info_data[AFTER]),
                info_get(kind, GET_RESIDENT_SIZE, info_data[AFTER]),
                "verify max resident size is greater than or equal to curr "
                "resident size");
  }
  ...
  /*
   * These counters give time for threads that have terminated. We dont have
   * any, so checking for zero.
   */

  time_value_t *user_tv =
      (time_value_t *)(info_get(kind, GET_USER_TIME, info_data[BEFORE]));
  T_EXPECT_EQ((user_tv->seconds + user_tv->microseconds / 1000000), 0,
              "verify task_info shows valid user time");

  time_value_t *sys_tv =
      (time_value_t *)(info_get(kind, GET_SYS_TIME, info_data[BEFORE]));
  T_EXPECT_EQ(sys_tv->seconds + (sys_tv->microseconds / 1000000), 0,
              "verify task_info shows valid system time");
...
T_DECL(test_sigcont_task_suspend_resume,
  ...
  suspend_count =
      (integer_t)(info_get(INFO_MACH, GET_SUSPEND_COUNT, info_data));
  T_ASSERT_EQ(suspend_count, 1,
              "verify task_info shows correct suspend_count (1) (actually user "
              "stop count) ");

  kr = task_info(child_task, TASK_DEBUG_INFO_INTERNAL, (task_info_t)&debug_info,
                 &debug_count);
  T_ASSERT_MACH_SUCCESS(kr, "verify task_info call succeeded");

  debug_suspend_count = debug_info.suspend_count;
  T_ASSERT_EQ(debug_info.suspend_count, 1,
              "verify debug_info shows correct suspend_count(1)");
...
T_DECL(test_sigcont_task_suspend2_resume,
  ...
  T_ASSERT_POSIX_SUCCESS(child_pid, "verify process can be forked %d",
                         child_pid);
  ...
  debug_suspend_count = debug_info.suspend_count;
  T_EXPECT_EQ(debug_suspend_count, 0,
              "verify debug_info shows correct (true) suspend_count(0)");
  ...
  debug_suspend_count = debug_info.suspend_count;
  T_ASSERT_EQ(debug_suspend_count, 1,
              "verify debug_info shows correct (true) suspend_count(1)");
  ...
  T_EXPECT_EQ(
      suspend_count, 1,
      "verify task_info shows correct (user_stop_count) suspend_count (1)");
  ...
  T_EXPECT_EQ(debug_suspend_count, 1,
              "verify debug_info shows correct (true) suspend_count (1)");

  suspend_count =
      (integer_t)(info_get(INFO_MACH, GET_SUSPEND_COUNT, info_data));
  T_ASSERT_EQ(suspend_count, 1,
              "verify task_info shows correct (user_stop_count) suspend_count "
              "(1) after SIG_CONT");
  ...
  T_ASSERT_EQ(signal_no, 0,
              "child should be resumed and no signal should be returned");
  T_ASSERT_EQ(exit_status, 0, "child should exit with 0");
}
