...
static int sock_open_common(int pf, int type) {
  int s;
...
static int sock_open(int type) { return sock_open_common(PF_INET, type); }

static int sock_bind(int s, int port) {
...
static int sockv6_open(int type) { return sock_open_common(PF_INET6, type); }

static int sockv6_bind(int s, int port) {
...
static uint16_t sock_get_port(int sockfd) {
  int error;
  uint16_t p;
...
static void bind_sockets(SocketInfoRef info, const char *msg) {
  for (int i = 0; i < info->socket_count; i++) {
    ...
    if (debug) {
      T_LOG("%s: fd %d port is %d error %d", msg, info->socket_list[i],
            ntohs(port), error);
    }
  }
  return;
...
static void multithreaded_bind_test(bool v6, int socket_count) {
  int error;
  SocketInfo info;
...
static void run_multithreaded_bind_test(int number_of_runs, bool v6,
                                        int socket_count) {
#if TARGET_OS_BRIDGE
...
T_DECL(socket_bind_35685803, "multithreaded bind IPv4 socket as root",
       T_META_ASROOT(false), T_META_CHECK_LEAKS(false),
       T_META_TAG_VM_PREFERRED) {
  run_multithreaded_bind_test(100, false, 100);
}

T_DECL(socket_bind_35685803_root, "multithreaded bind IPv4 socket",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  run_multithreaded_bind_test(100, false, 100);
}

T_DECL(socket_bind_35685803_v6, "multithreaded bind IPv6 socket as root",
       T_META_ASROOT(false), T_META_CHECK_LEAKS(false),
       T_META_TAG_VM_PREFERRED) {
  run_multithreaded_bind_test(100, true, 100);
}

T_DECL(socket_bind_35685803_v6_root, "multithreaded bind IPv6 socket",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  run_multithreaded_bind_test(100, true, 100);
}
