...
T_DECL(test_task_filter_msg_flag,
       "Set the filter msg flag on the task and check if the forked child "
       "inherits it",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.development failed");
  if (dev == 0) {
  ...
  ret = sysctlbyname("kern.task_set_filter_msg_flag", NULL, 0, &new_filter_flag,
                     sizeof(new_filter_flag));
  T_ASSERT_POSIX_SUCCESS(ret, "sysctlbyname");
  ret = sysctlbyname("kern.task_set_filter_msg_flag", &cur_filter_flag,
                     &sysctl_size, NULL, 0);
  T_ASSERT_POSIX_SUCCESS(ret, "sysctlbyname");
  ...
  new_filter_flag = 0;
  ret = sysctlbyname("kern.task_set_filter_msg_flag", &cur_filter_flag,
                     &sysctl_size, &new_filter_flag, sizeof(new_filter_flag));
  T_ASSERT_POSIX_SUCCESS(ret, "sysctlbyname");
  T_ASSERT_EQ(cur_filter_flag, 1, "Task should have filtering on");
