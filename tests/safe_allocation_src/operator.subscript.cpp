...
template <typename RawT, typename QualT> static void tests() {
  // Test the non-const version
  {
    RawT *memory =
        reinterpret_cast<RawT *>(malloc_allocator::allocate(10 * sizeof(RawT)));
  ...
  {
    using Alloc = libkern::safe_allocation<RawT, malloc_allocator,
