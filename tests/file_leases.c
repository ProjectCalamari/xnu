/* compile: xcrun -sdk macosx.internal clang -ldarwintest -o test_file_leases
 * file_leases.c -g -Weverything */
...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vfs.lease"),
              T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("vfs"), T_META_CHECK_LEAKS(false),
...
static void create_test_file(void) {
  ...
  T_ASSERT_POSIX_SUCCESS(mkdir(g_testdir, 0777), "Setup: creating test dir: %s",
                         g_testdir);
...
static void
    __attribute__((noreturn)) run_helpers(const char **helper_test_names,
                                          int num_helpers) {
...
T_HELPER_DECL(open_rdonly_acquire_read_lease_succeed,
              "Open file in O_RDONLY mode and acquire read lease succeeded") {
...
T_HELPER_DECL(
    open_rdonly_acquire_read_lease_EAGAIN,
    "Open file in O_RDONLY mode and acquire read lease failed with EAGAIN") {
  char *testfile = argv[0];
  int err, fd;
...
T_HELPER_DECL(
    open_rdwr_acquire_write_lease_EAGAIN,
    "Open file in O_RDWR mode and acquire write lease failed with EAGAIN") {
  char *testfile = argv[0];
  int err, fd;
...
T_HELPER_DECL(open_rdonly_read_lease_release,
              "Open file in O_RDONLY mode, acquire read lease, and release "
              "lease upon NOTE_LEASE_RELEASE event") {
  ...
  if (err > 0) {
    T_ASSERT_EQ(lease_kevent.fflags, NOTE_LEASE_RELEASE, "Got lease event 0x%x",
                lease_kevent.fflags);
...
T_HELPER_DECL(open_rdonly_write_lease_downgrade,
              "Open file in O_RDONLY mode, acquire a write lease, and "
              "downgrade lease upon NOTE_LEASE_DOWNGRADE event") {
  struct kevent lease_kevent;
  struct timespec kevent_timeout;
  char *testfile = argv[0];
  int err, fd, kq;
...
T_HELPER_DECL(open_rw_write_lease_downgrade,
              "Open file multiple times in O_RDWR mode, acquire a write lease, "
              "and downgrade lease upon NOTE_LEASE_DOWNGRADE event") {
  struct kevent lease_kevent;
  struct timespec kevent_timeout;
  char *testfile = argv[0];
...
T_HELPER_DECL(open_rdonly_read_lease_timedout,
              "Open file in O_RDONLY mode, acquire read lease, and hold lease "
              "beyond lease break timeout upon NOTE_LEASE_RELEASE event",
              T_META_ASROOT(true)) {
  struct kevent lease_kevent;
  struct timespec kevent_timeout;
  uint32_t val, new_val;
...
T_HELPER_DECL(open_rdonly_dir_read_lease,
              "Open directory in O_RDONLY mode, acquire read lease, and "
              "release lease upon NOTE_LEASE_RELEASE event") {
  struct kevent lease_kevent;
  struct timespec kevent_timeout;
  char *testdir = argv[1];
  int err, dir_fd, kq;
...
T_HELPER_DECL(open_rdonly_request_read_range_lock,
              "Open file in O_RDONLY mode and request byte range lock") {
  struct flock lreq;
  char *testfile = argv[0];
  int err, fd;
...
T_HELPER_DECL(file_syscalls_2, "Call file syscalls (mknod)",
              T_META_ASROOT(true)) {
  char destfile[MAXPATHLEN];
  char *testfile = argv[0];
  int err;
 ...
 */
T_DECL(acquire_downgrade_release, "Test acquire, downgrade and release lease",
 ...
 */
T_DECL(open_conflict_1, "Test acquire read lease failure due to open conflicts",
       T_META_ENABLED(TARGET_OS_OSX)) {
 ...
 */
T_DECL(open_conflict_2,
       "Test acquire write lease failure due to open conflicts",
       T_META_ENABLED(TARGET_OS_OSX)) {
 ...
 */
T_DECL(acquire_release_no_lease,
       "Test acquire and release lease when there is no lease is in place",
       T_META_ENABLED(TARGET_OS_OSX)) {
  int err, fd;
 ...
 */
T_DECL(acquire_release_retrieve_non_file,
       "Test acquire, release and retrieve lease on non-regular file",
       T_META_ENABLED(TARGET_OS_OSX)) {
  int err, fd;
  ...
  T_ASSERT_TRUE((err == -1) && (errno == EBADF),
                "Acquire read lease on socket");

  T_WITH_ERRNO;
  err = fcntl(fd, F_SETLEASE, F_WRLCK);
  T_ASSERT_TRUE((err == -1) && (errno == EBADF),
                "Acquire write lease on socket");
 ...
 */
T_DECL(
    retrieve_downgrade_dup_fd,
    "Test retrieve and downgrade lease with duplicated fd created with dup()",
    T_META_ENABLED(TARGET_OS_OSX)) {
  int err, dup_fd, fd;
 ...
 *         parent process.
 */
T_DECL(retrieve_release_fork_fd,
       "Test retrieve and release lease with duplicated fd created with fork()",
       T_META_ENABLED(TARGET_OS_OSX)) {
  pid_t child_pid;
  int err, fd;
 ...
 */
T_DECL(lease_break_release_1,
       "Test lease break release event when file is opened in O_RDWR mode",
       T_META_ENABLED(TARGET_OS_OSX)) {
 ...
 */
T_DECL(lease_break_release_2,
       "Test lease break release event when file is truncated",
       T_META_ENABLED(TARGET_OS_OSX)) {
  const char *helper_test_names[] = {"open_rdonly_read_lease_release",
                                     "truncate"};
 ...
 */
T_DECL(lease_break_release_3,
       "Test lease break release event when byte range lock is requested",
       T_META_ENABLED(TARGET_OS_OSX)) {
  const char *helper_test_names[] = {"open_rdonly_read_lease_release",
                                     "open_rdonly_request_read_range_lock"};
 ...
 */
T_DECL(lease_break_release_4, "Test multiple lease break release events",
       T_META_ENABLED(TARGET_OS_OSX)) {
  const char *helper_test_names[] = {"open_rdonly_read_lease_release",
                                     "open_rdonly_read_lease_release",
                                     "open_rdwr"};
 ...
 */
T_DECL(lease_break_downgrade_1,
       "Test lease break downgrade event with read-only opens",
       T_META_ENABLED(TARGET_OS_OSX)) {
  const char *helper_test_names[] = {"open_rdonly_write_lease_downgrade",
                                     "open_rdonly"};
 ...
 * Result: Process A downgrades the lease and process B's open is unblocked.
 */
T_DECL(lease_break_downgrade_2,
       "Test lease break downgrade event with multiple read-write opens",
       T_META_ENABLED(TARGET_OS_OSX)) {
  const char *helper_test_names[] = {"open_rw_write_lease_downgrade",
                                     "open_rdonly"};
 ...
 */
T_DECL(lease_break_timedout, "Test lease break timedout",
       T_META_ENABLED(TARGET_OS_OSX)) {
  const char *helper_test_names[] = {"open_rdonly_read_lease_timedout",
                                     "open_rdwr"};
 ...
 * Result: Lease operations should succeed as expected.
 */
T_DECL(acquire_release_read_lease_dir, "Test acquire and release read lease",
       T_META_ENABLED(TARGET_OS_OSX)) {
  int err, dir_fd;
 ...
 */
T_DECL(acquire_write_lease_dir, "Test acquire write lease on directory",
       T_META_ENABLED(TARGET_OS_OSX)) {
  int err, dir_fd;
  ...
  T_ASSERT_TRUE((err == -1) && (errno == ENOTSUP),
                "Acquire write lease on directory: %s", g_testdir);
 ...
 */
T_DECL(read_lease_dir_1, "Test directory read leasing and lease break events",
       T_META_ENABLED(TARGET_OS_OSX)) {
  const char *helper_test_names[] = {"open_rdonly_dir_read_lease",
                                     "file_syscalls"};
...
T_DECL(read_lease_dir_2, "Test directory read leasing and lease break events",
       T_META_ENABLED(TARGET_OS_OSX)) {
  const char *helper_test_names[] = {"open_rdonly_dir_read_lease",
                                     "file_syscalls_2"};
