 ...
 */
T_DECL(fcntl_addsig, "Verify that fcntl(F_ADDSIGS) doesn't EFAULT",
       T_META_NAMESPACE("xnu.vfs")) {
  void *blob_space = mmap(NULL, vm_page_size * 3, PROT_READ | PROT_WRITE,
                          MAP_PRIVATE | MAP_ANON, -1, 0);
  T_ASSERT_NE(blob_space, MAP_FAILED, "Blob Region: %p [%zd]", blob_space,
              vm_page_size);

  T_ASSERT_POSIX_SUCCESS(
      munmap((char *)blob_space + (vm_page_size * 2), vm_page_size), NULL);

  size_t blob_size = vm_page_size + 1;
  char *blob_start = ((char *)blob_space) + (vm_page_size * 2) - blob_size;
  fsignatures_t args = {.fs_file_start = 0,
                        .fs_blob_start = blob_start,
                        .fs_blob_size = blob_size};
  ...
  int error = errno;
  T_QUIET;
  T_EXPECT_EQ(result, -1, NULL);
  // EBADEXEC is expected, but not required for success of this test
  T_EXPECT_NE(error, EFAULT, "fcntl: %d (%d:%s)", result, error,
              strerror(error));
}
