...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm.perf"),
              T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("VM"), T_META_CHECK_LEAKS(false),
              T_META_TAG_PERF, T_META_ENABLED(false) /* rdar://84443533 */
);

enum { ALL_ZEROS, MOSTLY_ZEROS, RANDOM, TYPICAL };
...
void freeze_helper_process(void) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sysctlbyname("vm.compressor_compressed_bytes",
                                      &compressed_before, &length, NULL, 0),
                         "failed to query vm.compressor_compressed_bytes");
  length = sizeof(input_before);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sysctlbyname("vm.compressor_input_bytes",
                                      &input_before, &length, NULL, 0),
                         "failed to query vm.compressor_input_bytes");

  T_STAT_MEASURE(compr_time) {
    ret =
        sysctlbyname("kern.memorystatus_freeze", NULL, NULL, &pid, sizeof(pid));
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sysctlbyname("vm.compressor_compressed_bytes",
                                      &compressed_after, &length, NULL, 0),
                         "failed to query vm.compressor_compressed_bytes");
  length = sizeof(input_after);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("vm.compressor_input_bytes", &input_after, &length, NULL, 0),
      "failed to query vm.compressor_input_bytes");
  ...
  dt_stat_add(ratio, (double)(input_after - input_before) /
                         (double)(compressed_after - compressed_before));
...
void run_compressor_test(int size_mb, int page_type) {
  ...
  // This sets the A/B failure threshold at 50% of baseline for
  // compressor_latency
  dt_stat_set_variable((struct dt_stat *)compr_time, kPCFailureThresholdPctVar,
                       50.0);

  signal(SIGUSR2, SIG_IGN);
  ds_decompr = dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR2, 0,
                                      dispatch_get_main_queue());
  T_QUIET;
  T_ASSERT_NOTNULL(ds_decompr, "dispatch_source_create (ds_decompr)");
  ...
  launch_tool_args =
      (char *[]){testpath, "-n", "allocate_pages", "--", sz_str, pt_str, NULL};
  ...
  ds_proc =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, (uintptr_t)pid,
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kill(pid, SIGCONT),
...
T_HELPER_DECL(allocate_pages, "allocates pages to compress") {
  ...
  if (memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_FREEZABLE, getpid(),
                           1, NULL, 0) != KERN_SUCCESS) {
    exit(MEMORYSTATUS_CONTROL_FAILED);
  }

  /* Verify that the state has been set correctly */
  freezable_state = memorystatus_control(
  ...
  dispatch_source_set_event_handler(ds_signal, ^{
    ...
    if (collect_dt_stat_measurements) {
      if (dt_stat_stable(decompr_time)) {
        ...
        dt_stat_mach_time_add(decompr_time,
                              decompr_end_time - decompr_start_time);
      }
    }
...
// Numbers for 10MB and above are fairly reproducible. Anything smaller shows a
// lot of variation.

// Keeping just the 100MB version for iOSMark
#ifndef DT_IOSMARK
T_DECL(compr_10MB_zero, "Compression latency for 10MB - zero pages",
       T_META_ASROOT(true), T_META_SYSCTL_INT(SYSCTL_FREEZE_TO_MEMORY),
       T_META_TAG_VM_NOT_ELIGIBLE) {
  run_compressor_test(10, ALL_ZEROS);
}

T_DECL(compr_10MB_mostly_zero,
       "Compression latency for 10MB - mostly zero pages", T_META_ASROOT(true),
       T_META_SYSCTL_INT(SYSCTL_FREEZE_TO_MEMORY), T_META_TAG_VM_NOT_ELIGIBLE) {
  run_compressor_test(10, MOSTLY_ZEROS);
}

T_DECL(compr_10MB_random, "Compression latency for 10MB - random pages",
       T_META_ASROOT(true), T_META_SYSCTL_INT(SYSCTL_FREEZE_TO_MEMORY),
       T_META_TAG_VM_NOT_ELIGIBLE) {
  run_compressor_test(10, RANDOM);
}

T_DECL(compr_10MB_typical, "Compression latency for 10MB - typical pages",
       T_META_ASROOT(true), T_META_SYSCTL_INT(SYSCTL_FREEZE_TO_MEMORY),
       T_META_TAG_VM_NOT_ELIGIBLE) {
  run_compressor_test(10, TYPICAL);
}

T_DECL(compr_100MB_zero, "Compression latency for 100MB - zero pages",
       T_META_ASROOT(true), T_META_SYSCTL_INT(SYSCTL_FREEZE_TO_MEMORY),
       T_META_TAG_VM_NOT_ELIGIBLE) {
...
T_DECL(compr_100MB_mostly_zero,
       "Compression latency for 100MB - mostly zero pages", T_META_ASROOT(true),
       T_META_SYSCTL_INT(SYSCTL_FREEZE_TO_MEMORY), T_META_TAG_VM_NOT_ELIGIBLE) {
  run_compressor_test(100, MOSTLY_ZEROS);
}

T_DECL(compr_100MB_random, "Compression latency for 100MB - random pages",
       T_META_ASROOT(true), T_META_SYSCTL_INT(SYSCTL_FREEZE_TO_MEMORY),
       T_META_TAG_VM_NOT_ELIGIBLE) {
...
T_DECL(compr_100MB_typical, "Compression latency for 100MB - typical pages",
       T_META_ASROOT(true), T_META_SYSCTL_INT(SYSCTL_FREEZE_TO_MEMORY),
       T_META_TAG_VM_NOT_ELIGIBLE) {
