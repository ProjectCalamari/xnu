...
static void *thread_func(__unused void *arg) {
  lock_a = 1;
  while (lock_b == 0) {
  }
...
T_DECL(mcast_group_race_82820812,
       "Race between multicast group join operations.", T_META_ASROOT(true),
  ...
  for (i = 0; i < ITERATIONS_LIMIT; ++i) {
    ...
    T_ASSERT_POSIX_SUCCESS(bind(fd, (struct sockaddr *)&saddr, sizeof(saddr)),
                           "bind");

    for (j = 0; j < IP_MIN_MEMBERSHIPS - 1; ++j) {
      filler_group.imr_multiaddr.s_addr =
          htonl(ntohl(inet_addr("224.0.0.3")) + j);
      setsockopt(fd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &filler_group,
                 sizeof(filler_group));
    }

    T_ASSERT_POSIX_ZERO(pthread_create(&th, NULL, thread_func, NULL),
                        "pthread_create");
