...
static void *worker_thread_function(void *args) {
  ...
  kr = mach_vm_allocate(mach_task_self(), &addr, VM_SIZE_PER_THREAD,
                        VM_FLAGS_ANYWHERE | VM_PROT_DEFAULT |
                            VM_MAKE_TAG(VM_MEMORY_APPLICATION_SPECIFIC_1));
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_allocate()");
  array = (char *)addr;
  T_QUIET;
  T_EXPECT_NOTNULL(array, "thread %d allocated heap memory to be dirtied",
                   thread_id);
  ...
  /* Phase 1: touch pages */
  T_LOG("thread %d phase 1: dirtying %d heap pages (%d bytes)", thread_id,
        VM_SIZE_PER_THREAD / (int)PAGE_SIZE, VM_SIZE_PER_THREAD);
  ...
  kr = mach_vm_behavior_set(mach_task_self(), addr, VM_SIZE_PER_THREAD,
                            VM_BEHAVIOR_PAGEOUT);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_behavior_set()");
  ...
  T_LOG("thread %d phase 3: faulting pages back in to trigger decompressions",
        thread_id);
  memset(&array[0], 1, VM_SIZE_PER_THREAD);
...
static pthread_t *create_threads(int nthreads, pthread_t *threads,
  ...
  for (i = 0; i < nthreads; i++) {
    targs[i].id = i;
    err = pthread_create(&threads[i], &attr, worker_thread_function,
                         (void *)&targs[i]);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(err, "pthread_create");
  }
...
static void join_threads(int nthreads, pthread_t *threads) {
  ...
  for (i = 0; i < nthreads; i++) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(err, "pthread_join");
  }
}
...
T_DECL(task_vm_info_decompressions,
  ...
  err = sysctlbyname("vm.compressor_mode", &compressor_mode,
                     &compressor_mode_size, NULL, 0);
  if (compressor_mode < 8) {
    T_SKIP("This test requires freezer which is not available on the testing "
           "platform (vm.compressor_mode is set to %d)",
           compressor_mode);
  }
#if TARGET_OS_BRIDGE
  T_SKIP("This test requires freezer which is not available on bridgeOS "
         "(vm.compressor_mode is set to %d)",
         compressor_mode);
#endif
  ...
  err = memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_FREEZABLE,
                             getpid(), (uint32_t)1, NULL, 0);
  T_EXPECT_EQ(KERN_SUCCESS, err, "set pid %d to be freezable", getpid());

  /* Call into kernel to retrieve vm_info and make sure we do not have any
   * decompressions before the test */
  count = TASK_VM_INFO_COUNT;
  err =
      task_info(mach_task_self(), TASK_VM_INFO, (task_info_t)&vm_info, &count);
  T_EXPECT_EQ(count, TASK_VM_INFO_COUNT, "count == TASK_VM_INFO_COUNT: %d",
              count);
  T_EXPECT_EQ_INT(0, err, "task_info(TASK_VM_INFO) returned 0");
  T_EXPECT_EQ_INT(0, vm_info.decompressions,
  ...
  T_EXPECT_EQ(ncpu, atomic_load(&thread_malloc_count),
  ...
  T_QUIET;
  T_EXPECT_EQ_INT(0, err, "task_info(TASK_VM_INFO) returned 0");
  T_EXPECT_EQ(0, vm_info.decompressions,
              "Expected 0 decompressions before compressions");
  ...
  T_EXPECT_EQ(ncpu, atomic_load(&thread_compressed_count),
              "%d threads finished writing to malloc pages\n", ncpu);
  ...
  /* Phase 3: Call into kernel to retrieve vm_info and to get the updated
  ...
  T_EXPECT_EQ(count, TASK_VM_INFO_COUNT, "count == TASK_VM_INFO_COUNT: %d",
              count);
  T_QUIET;
  T_EXPECT_MACH_SUCCESS(kr, "task_info(TASK_VM_INFO)");

  /* Make sure this task has decompressed at least all of the dirtied memory */
  T_EXPECT_GE_INT(vm_info.decompressions, npages,
                  "decompressed %d pages (>= heap pages: %d)",
                  vm_info.decompressions, npages);
