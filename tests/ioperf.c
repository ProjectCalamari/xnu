...
static void io_perf_test_io_init(void) {
  int spawn_ret, pid;
  ...
  T_ASSERT_GE_INT((int)read(rndfd, data_buf, IO_SIZE * 16), 0,
                  "read /dev/urandom");
  close(rndfd);
  ...
  for (int size = 0; size < FILE_SIZE;) {
    T_QUIET;
    T_ASSERT_GE_INT((int)write(fd, data_buf, IO_SIZE * 16), 0,
                    "write test file");
    size += (IO_SIZE * 16);
  }
  close(fd);
...
static pthread_t create_thread(uint32_t thread_id, uint32_t priority,
                               bool fixpri, void *(*start_routine)(void *)) {
  int rv;
...
static void spin_for_duration(uint32_t seconds) {
  ...
  while (mach_absolute_time() < timeout &&
         atomic_load_explicit(&keep_going, memory_order_relaxed)) {
    spin_count++;
  }
}
...
void perform_io(dt_stat_time_t stat) {
  ...
  T_ASSERT_POSIX_ZERO(fcntl(test_file_fd, F_NOCACHE, 1),
                      "fcntl F_NOCACHE enable");
  T_ASSERT_POSIX_ZERO(fcntl(test_file_fd, F_RDAHEAD, 0),
                      "fcntl F_RDAHEAD disable");
  ...
  do {
    T_STAT_MEASURE(stat) { ret = read(test_file_fd, data_buf, IO_SIZE); }
    if (ret == 0) {
      T_QUIET;
...
T_DECL(read_perf, "Sequential Uncached Read Performance", T_META_TYPE_PERF,
       T_META_CHECK_LEAKS(NO), T_META_ASROOT(YES),
  ...
  T_ASSERT_MACH_SUCCESS(mach_timebase_info(&timebase_info),
                        "mach_timebase_info");

  dt_stat_time_t seq_noload =
  ...
  T_ASSERT_MACH_SUCCESS(
      semaphore_create(mach_task_self(), &worker_sem, SYNC_POLICY_FIFO, 0),
      "semaphore_create");

  size_t ncpu_size = sizeof(g_numcpus);
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("hw.ncpu", &g_numcpus, &ncpu_size, NULL, 0),
  ...
  for (uint32_t thread_id = 0; thread_id < n_spinners; thread_id++) {
    threads[thread_id].thread =
        create_thread(thread_id, THR_SPINNER_PRI, true, &spin_thread);
  }
