...
void mach_vm_client(mach_port_t port) {
  ...
  while (1) {
    /* Awaiting the pid/src. addr/size from the server so we know what to remap
     * from where */
    ret = mach_msg(&message->header, MACH_RCV_MSG | MACH_RCV_LARGE, 0,
                   messageSize, port, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
    if (ret == KERN_SUCCESS) {
      if (debug) {
        T_LOG("CLIENT: received info from server... 0x%llx, %lld, 0x%llx, %d - "
              "%d\n",
              message->address, message->size, message->misoffset,
              message->vm_op, message->copy);
      }
      ...
      case VM_OP_REMAP:
        ...
        ret = mach_vm_allocate(mach_task_self(), &tmp, size + 16384,
                               VM_FLAGS_ANYWHERE);
        T_QUIET;
        T_ASSERT_MACH_SUCCESS(ret, "mach_vm_allocate");
        mach_vm_deallocate(mach_task_self(), tmp, size + 16384);

        dest = tmp + 4096;

        ret =
            mach_vm_remap(mach_task_self(), &dest, size, 0,
                          VM_FLAGS_ANYWHERE | VM_FLAGS_RETURN_DATA_ADDR, lport,
                          src, copy, &cur_prot, &max_prot, VM_INHERIT_NONE);

        if (ret) {
          char dstval[64];
          memcpy(dstval, (void *)dest, 64);
          T_LOG("CLIENT: mach_vm_remap FAILED: %s -- src 0x%llx, dest 0x%llx "
                "(%s)\n",
      ...
      case VM_OP_READ_OVERWRITE:
        ...
        if (ret) {
          char dstval[64];
          memcpy(dstval, (void *)dest, 64);
          T_LOG("CLIENT: mach_vm_read_overwrite FAILED: %s -- src 0x%llx, dest "
                "0x%llx (%s)\n",
                mach_error_string(ret), src, dest, dstval);
      ...
      case VM_OP_READ:
        ...
        if (ret) {
          char dstval[64];
          memcpy(dstval, (void *)dest, 64);
          T_LOG("CLIENT: mach_vm_read FAILED: %s -- src 0x%llx, dest 0x%llx "
                "(%s)\n",
                mach_error_string(ret), src, dest, dstval);
      ...
      case VM_OP_MEMENTRY:
        ...
        ret = mach_vm_map(mach_task_self(), &dest, size, 0 /*mask*/,
                          ...
                          memport, 0 /*offset*/, copy, cur_prot, max_prot,
                          VM_INHERIT_NONE);
      ...
      case VM_OP_UNMAP:
        ...
        if (ret) {
          T_LOG("CLIENT: mach_vm_deallocate FAILED: %s -- dest 0x%llx, size "
                "0x%llx\n",
      ...
      if (debug) {
        T_LOG("CLIENT: dest 0x%llx -> 0x%llx (0x%llx), *dest %s\n", dest,
              dest + misoffset, misoffset, dstval);
      ...
      ret = mach_msg(&message->header, MACH_SEND_MSG, message->header.msgh_size,
                     0, MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
      T_QUIET;
      T_ASSERT_MACH_SUCCESS(ret, "CLIENT: mach_msg_send FAILED");
    } else {
      T_QUIET;
      T_ASSERT_MACH_SUCCESS(ret, "CLIENT: mach_msg_rcv FAILED");
    }
  }
}

void mach_server_make_memory_entry(mach_port_t replyPort) {
...
test_different_mementry_mode:
  ...
  if (mementry_pass_idx < 2) {
    ...
    kr = mach_vm_protect(mach_task_self(), (mach_vm_address_t)lsrc,
                         (mach_vm_size_t)getpagesize(), FALSE, VM_PROT_READ);
  ...
  kr = mach_make_memory_entry_64(mach_task_self(), &memsz, lsrc, flags,
                                 &memport, MACH_PORT_NULL);
  if (kr != KERN_SUCCESS) {
    T_LOG(
        "ERROR: mach_make_memory_entry_64 try (%d) failed in Client: (%d) %s\n",
        mementry_pass_idx + 1, kr, mach_error_string(kr));
    server_error_out(replyPort);
  }

  mach_server_contruct_payload(
      &message, lsrc, memport, memsz, misoffset,
      ((flags & MAP_MEM_VM_COPY) == MAP_MEM_VM_COPY) /*copy*/, VM_OP_MEMENTRY);
...
check_again:
  ...
  kr = mach_msg(&message.header, MACH_SEND_MSG, message.header.msgh_size, 0,
  ...
  kr = mach_msg(&reply->header, MACH_RCV_MSG, 0, replySize, replyPort,
  ...
  if (modified_in_server == FALSE) {
    if (strncmp(src_val, dst_val, 64)) {
      T_LOG("FAILED\n");
      T_LOG("(%d) Pre modification mach_make_memory_entry() FAILED: copy(%d) "
            "src_val: %s  dest_val: %s\n",
  ...
  if (modified_in_server == FALSE) {
    ...
    if (perm_changed) {
      kr = mach_vm_protect(mach_task_self(), (mach_vm_address_t)lsrc,
                           (mach_vm_size_t)getpagesize(), FALSE,
...
void mach_server_read(mach_port_t replyPort, int op) {
  ...
  mach_server_contruct_payload(&message, src, MACH_PORT_NULL /* port */, size,
                               0, TRUE /*copy*/, op);
  if (debug) {
...
void mach_server_remap(mach_port_t replyPort) {
  mach_vm_address_t src = 0, lsrc = 0;
  mach_vm_size_t size = TESTSZ;
  kern_return_t kr;
...
remap_again:
  ...
  T_LOG("vm_remap (copy = %s) test...",
        ((remap_copy_pass_idx == 0) ? "FALSE" : "TRUE"));
  ...
  mach_server_contruct_payload(&message, lsrc, MACH_PORT_NULL /* port */,
                               size - 9000, 0, remap_copy_pass_idx /*copy*/,
                               VM_OP_REMAP);
  if (debug) {
    T_LOG("server COPY: Sending 0x%llx, %d, 0x%llx\n", message.address,
          getpid(), message.size);
...
check_again:
  ...
  if (modified_in_server == FALSE) {
    if (strncmp(src_val, dst_val, 64)) {
      T_LOG("Pre modification remap() FAILED: copy(%d) src_val: %s  dest_val: "
            "%s\n",
  ...
  if (remap_copy_pass_idx == 0) {
    ...
    /* Next remap pass to test (copy == TRUE). Send data out again to the other
     * process to remap. */
    goto remap_again;
  }
