...
int mach_server_data_setup(void **buffer) {
  if (0 != access(TEST_TXT_FILE, F_OK)) {
    ...
    while (bytes_written < required_length) {
      bytes_written +=
          (size_t)write(w_fd, &lorem_text[0], (size_t)(lorem_text_length - 1));
      if ((bytes_written + lorem_text_length) > required_length) {
        bytes_written += (size_t)write(
            w_fd, &lorem_text[0], (size_t)(required_length - bytes_written));
        break;
      }
    }
    close(w_fd);
  ...
  *buffer = mmap(NULL, sb.st_size, PROT_READ, MAP_FILE | MAP_PRIVATE, fd, 0);
  ...
  kr = mach_vm_allocate(mach_task_self(), (mach_vm_address_t *)buffer,
                        (mach_vm_size_t)sb.st_size, VM_FLAGS_ANYWHERE);
  assert(kr == KERN_SUCCESS);
#endif
  return 0;
}

void mach_server_data_cleanup(void *buffer, mach_vm_address_t src,
                              mach_vm_size_t size) {
#if MMAP_PATH
  if (buffer) {
...
#else
  mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)buffer,
                     (mach_vm_size_t)sb.st_size);
#endif
...
void mach_server_construct_header(ipc_message_t *message,
...
void mach_server_contruct_payload(ipc_message_t *message, mach_vm_address_t src,
                                  mach_port_t port, mach_vm_size_t size,
                                  mach_vm_offset_t misoffset, boolean_t copy,
                                  int vm_op) {
...
void mach_server_create_allocation(mach_vm_address_t *src, mach_vm_size_t size,
                                   void *buffer) {
  kern_return_t kr = KERN_SUCCESS;
...
void server_error_out(mach_port_t port) {
  /* All done here...*/
  kern_return_t ret;

  mach_msg_size_t messageSize =
      sizeof(ipc_message_t) + sizeof(mach_msg_trailer_t) + 64;
  ...
  ret = mach_msg(&message->header, MACH_SEND_MSG, message->header.msgh_size, 0,
