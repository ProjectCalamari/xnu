...
static void spawn_process(char *action, char *serviceName, char *extraArg,
                          mach_port_t *server_Port, pid_t *serverPid,
                          boolean_t use4k);
...
void spawn_process(char *action, char *serviceName, char *extraArg,
                   mach_port_t *server_Port, pid_t *server_Pid,
                   boolean_t use4k) {
  ...
  if (use4k) {
    ...
    r = sysctlbyname("debug.vm_mixed_pagesize_supported", &supported,
                     &supported_size, NULL, 0);
    if (r == 0 && supported) {
  ...
  if (use4k) {
    T_ASSERT_POSIX_ZERO(
        posix_spawn(&pid, VM_SPAWN_TOOL, &actions, NULL, argv, environ),
        "spawn %s", serviceName);
  } else {
    T_ASSERT_POSIX_ZERO(
        posix_spawn(&pid, buffer, &actions, NULL, argv, environ), "spawn %s",
        serviceName);
  }
  ...
  if (ret != KERN_SUCCESS) {
    printf("ERROR: Failed bootstrap lookup for process with mach service name "
           "'%s': (%d) %s\n",
...
void mach_client() {
  ...
  T_ASSERT_POSIX_ZERO(
      ...
      "create recieve port");
  T_ASSERT_POSIX_ZERO(mach_port_insert_right(mach_task_self(), replyPort,
                                             replyPort,
   ...
   *  mach_msg_size_t replySize = sizeof(ipc_message_t) +
   * sizeof(mach_msg_trailer_t) + 64; ipc_message_t *reply = calloc(1,
   * replySize);
   */
  T_LOG("sending message to %d of size %u", message.header.msgh_remote_port,
...
T_DECL(memory_share_tests,
       "test vm memory sharing between client and server process with "
       "different process PAGE_SIZE",
  ...
  spawn_process("machserver", serviceName, NULL, &serverPort, &serverPid,
                use4k);
  mach_client();
}
 ...
 */
T_DECL_REF(memory_share_tests_4k, memory_share_tests,
           "vm memory sharing with 4k processes", T_META_ENVVAR("USE4K=YES"),
           T_META_ASROOT(true), T_META_ENABLED(false /* rdar://133462123 */));
