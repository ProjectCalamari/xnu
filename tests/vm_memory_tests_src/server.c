...
int main() {
  dispatch_source_t parentSource =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, (uintptr_t)getppid(),
...
#pragma clang diagnostic ignored "-Wdeprecated"
  ret = bootstrap_register2(bootstrap, (char *)serviceName, port,
...
#pragma clang diagnostic pop
  ...
  ret = mach_msg(&message->header, MACH_RCV_MSG, 0, messageSize, port,
                 MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
  if (ret == KERN_SUCCESS) {
    if (MACH_MSGH_BITS_REMOTE(message->header.msgh_bits) ==
        MACH_MSG_TYPE_PORT_SEND) {
      persistentReplyPort = message->header.msgh_remote_port;
      mach_port_mod_refs(mach_task_self(), persistentReplyPort,
