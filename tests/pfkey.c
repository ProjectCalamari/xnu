...
T_GLOBAL_META(T_META_NAMESPACE("xnu.pfkey"), T_META_ASROOT(true),
...
static void pfkey_process_message_test_60822136(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_60822924(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_60822956(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_60679513(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_60768729(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_60769680(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_60822823(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_60822823_1(uint8_t **mhp,
                                                  int pfkey_socket);
static void pfkey_process_message_test_60687183(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_60687183_1(uint8_t **mhp,
                                                  int pfkey_socket);
static void pfkey_process_message_test_60687183_2(uint8_t **mhp,
                                                  int pfkey_socket);
static void pfkey_process_message_test_78944570(uint8_t **mhp,
                                                int pfkey_socket);
static void pfkey_process_message_test_134671927(uint8_t **mhp,
                                                 int pfkey_socket);

static void (*const process_pfkey_message_tests[])(uint8_t **mhp,
                                                   int pfkey_socket) = {
    NULL,
...
static void pfkey_align(struct sadb_msg *msg, uint8_t **mhp) {
  struct sadb_ext *ext;
  int i;
  uint8_t *p;
  ...
  /* validity check */
  T_QUIET;
  T_ASSERT_NOTNULL(msg, "pfkey align msg");
  T_QUIET;
  T_ASSERT_NOTNULL(mhp, "pfkey align mhp");
  ...
  while (p < ep) {
    ext = (void *)p;
    T_QUIET;
    T_ASSERT_GE_PTR((void *)ep, (void *)(p + sizeof(*ext)),
                    "pfkey extension header beyond end of buffer");
    T_QUIET;
    T_ASSERT_GE_ULONG((unsigned long)PFKEY_EXTLEN(ext), sizeof(*ext),
                      "pfkey extension shorter than extension header");
    T_QUIET;
    T_ASSERT_GE_PTR((void *)ep, (void *)(p + PFKEY_EXTLEN(ext)),
                    "pfkey extension length beyond end of buffer");

    T_QUIET;
    T_EXPECT_NULL(mhp[ext->sadb_ext_type],
                  "duplicate extension type %u payload", ext->sadb_ext_type);
  ...
  T_QUIET;
  T_EXPECT_EQ_PTR((void *)ep, (void *)p, "invalid pfkey message length");
  return;
}

static void recv_pfkey_message(int pfkey_socket) {
  uint8_t buffer[8192] __attribute__((aligned(4)));
  struct iovec iovecs[1] = {
      {buffer, sizeof(buffer)},
  };
  struct msghdr msg = {
      NULL, 0, iovecs, sizeof(iovecs) / sizeof(iovecs[0]), NULL, 0, 0,
  };
  ...
  do {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(result = recvmsg(pfkey_socket, &msg, 0), NULL);
...
static void send_pfkey_spd_add_message(int pfkey_socket, uint8_t proto) {
  ...
  struct sadb_address *src_address_payload =
      (struct sadb_address *)(void *)(payload + tlen);
  ...
  T_QUIET;
  T_ASSERT_EQ_INT(inet_pton(AF_INET, TEST_SRC_ADDRESS_IPv4, &src->sin_addr), 1,
                  "src address fail");
  ...
  struct sadb_address *dst_address_payload =
      (struct sadb_address *)(void *)(payload + tlen);
  ...
  T_QUIET;
  T_ASSERT_EQ_INT(inet_pton(AF_INET, TEST_DST_ADDRESS_IPv4, &dst->sin_addr), 1,
                  "dst address fail");
  ...
  struct sadb_lifetime *lifetime_payload =
      (struct sadb_lifetime *)(void *)(payload + tlen);
  ...
  struct sadb_x_policy *policy_payload =
      (struct sadb_x_policy *)(void *)(payload + tlen);
  ...
  msg_payload->sadb_msg_len = PFKEY_UNIT64(tlen);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send spd add");
}

static void send_pfkey_spd_get_message(int pfkey_socket, uint32_t policy_id) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
...
static void send_pfkey_spd_enable_message(int pfkey_socket,
                                          uint32_t policy_id) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send spd enable failed");
}

static void send_pfkey_spd_disable_message(int pfkey_socket,
                                           uint32_t policy_id) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send spd disable failed");
}

static void send_pfkey_spd_delete_message(int pfkey_socket,
                                          uint32_t policy_id) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send spd delete failed");
}

static void send_pfkey_spd_dump_message(int pfkey_socket) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send spd dump failed");
}

static void send_pfkey_flush_sp(int pfkey_socket) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey flush security policies");
}

static void send_pfkey_register(int pfkey_socket) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
...
static void send_pkey_get_spi(int pfkey_socket) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  struct sadb_x_sa2 *sa2_payload =
      (struct sadb_x_sa2 *)(void *)(payload + tlen);
...
static void send_pkey_add_sa(int pfkey_socket, uint32_t spi, const char *src,
                             const char *dst, int family) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  sa2_payload->sa.sadb_sa_flags |=
      (SADB_X_EXT_NATT | SADB_X_EXT_NATT_KEEPALIVE);
  ...
  struct sadb_x_sa2 *sa2_x_payload =
      (struct sadb_x_sa2 *)(void *)(payload + tlen);
  ...
  uint8_t prefixlen = (family == AF_INET) ? (sizeof(struct in_addr) << 3)
  ...
  if (family == AF_INET) {
    ...
    T_QUIET;
    T_ASSERT_EQ_INT(inet_pton(AF_INET, src, &src4->sin_addr), 1,
                    "src address fail");
    ...
    T_QUIET;
    T_ASSERT_EQ_INT(inet_pton(AF_INET6, src, &src6->sin6_addr), 1,
                    "src address fail");
  ...
  if (family == AF_INET) {
    ...
    T_QUIET;
    T_ASSERT_EQ_INT(inet_pton(AF_INET, dst, &dst4->sin_addr), 1,
                    "dst address fail");
    ...
    T_QUIET;
    T_ASSERT_EQ_INT(inet_pton(AF_INET6, dst, &dst6->sin6_addr), 1,
                    "dst address fail");
  ...
  struct sadb_key *encrypt_key_payload =
      (struct sadb_key *)(void *)(payload + tlen);
  ...
  struct sadb_key *auth_key_payload =
      (struct sadb_key *)(void *)(payload + tlen);
  ...
  struct sadb_lifetime *hard_lifetime_payload =
      (struct sadb_lifetime *)(void *)(payload + tlen);
  ...
  struct sadb_lifetime *soft_lifetime_payload =
      (struct sadb_lifetime *)(void *)(payload + tlen);
  soft_lifetime_payload->sadb_lifetime_len =
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send update sa");
}

static void send_pkey_update_sa(int pfkey_socket, uint32_t spi) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
...
static void send_pkey_migrate_sa(int pfkey_socket, uint32_t spi,
                                 const char *src, const char *dst, int family,
                                 const char *migrate_src,
                                 const char *migrate_dst, int migrate_family) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  prefixlen = (migrate_family == AF_INET) ? (sizeof(struct in_addr) << 3)
                                          : (sizeof(struct in6_addr) << 3);

  struct sadb_address *migrate_src_address_payload =
      (struct sadb_address *)(void *)(payload + tlen);
  migrate_src_address_payload->sadb_address_exttype =
      SADB_EXT_MIGRATE_ADDRESS_SRC & 0xffff;
  ...
  if (migrate_family == AF_INET) {
    struct sockaddr_in *migrate_src4 =
        (struct sockaddr_in *)(void *)(payload + tlen);
    T_QUIET;
    T_ASSERT_EQ_INT(inet_pton(AF_INET, migrate_src, &migrate_src4->sin_addr), 1,
                    "migrate src fail");
    migrate_src4->sin_family = AF_INET;
    migrate_src4->sin_len = sizeof(*migrate_src4);
    uint16_t len = sizeof(*migrate_src_address_payload) +
                   PFKEY_ALIGN8(migrate_src4->sin_len);
    ...
    struct sockaddr_in6 *migrate_src6 =
        (struct sockaddr_in6 *)(void *)(payload + tlen);
    T_QUIET;
    T_ASSERT_EQ_INT(inet_pton(AF_INET6, migrate_src, &migrate_src6->sin6_addr),
                    1, "migrate src fail");
    migrate_src6->sin6_family = AF_INET6;
    migrate_src6->sin6_len = sizeof(*migrate_src6);
    uint16_t len = sizeof(*migrate_src_address_payload) +
                   PFKEY_ALIGN8(migrate_src6->sin6_len);
    migrate_src_address_payload->sadb_address_len = PFKEY_UNIT64(len);
    ...
    T_ASSERT_EQ_INT(inet_pton(AF_INET6, migrate_src, &migrate_src6->sin6_addr),
                    1, "migrate src fail");
    migrate_src6->sin6_family = AF_INET6;
    migrate_src6->sin6_len =
        sizeof(*migrate_src6) + 100; // Bad value to trigger exploit
    uint16_t len = sizeof(*migrate_src_address_payload) +
                   PFKEY_ALIGN8(migrate_src6->sin6_len);
    migrate_src_address_payload->sadb_address_len = PFKEY_UNIT64(len);
    tlen += PFKEY_ALIGN8(migrate_src6->sin6_len);
  ...
  struct sadb_address *migrate_dst_address_payload =
      (struct sadb_address *)(void *)(payload + tlen);
  migrate_dst_address_payload->sadb_address_exttype =
      SADB_EXT_MIGRATE_ADDRESS_DST & 0xffff;
  ...
  if (migrate_family == AF_INET) {
    struct sockaddr_in *migrate_dst4 =
        (struct sockaddr_in *)(void *)(payload + tlen);
    T_QUIET;
    T_ASSERT_EQ_INT(inet_pton(AF_INET, migrate_dst, &migrate_dst4->sin_addr), 1,
                    "migrate dst fail");
    migrate_dst4->sin_family = AF_INET;
    migrate_dst4->sin_len = sizeof(*migrate_dst4);
    uint16_t len = sizeof(*migrate_dst_address_payload) +
                   PFKEY_ALIGN8(migrate_dst4->sin_len);
    migrate_dst_address_payload->sadb_address_len = PFKEY_UNIT64(len);
    tlen += PFKEY_ALIGN8(migrate_dst4->sin_len);
  } else if (migrate_family == AF_INET6) {
    struct sockaddr_in6 *migrate_dst6 =
        (struct sockaddr_in6 *)(void *)(payload + tlen);
    T_QUIET;
    T_ASSERT_EQ_INT(inet_pton(AF_INET6, migrate_dst, &migrate_dst6->sin6_addr),
                    1, "migrate dst fail");
    migrate_dst6->sin6_family = AF_INET6;
    migrate_dst6->sin6_len = sizeof(*migrate_dst6);
    uint16_t len = sizeof(*migrate_dst_address_payload) +
                   PFKEY_ALIGN8(migrate_dst6->sin6_len);
    migrate_dst_address_payload->sadb_address_len = PFKEY_UNIT64(len);
    ...
    T_ASSERT_EQ_INT(inet_pton(AF_INET6, migrate_dst, &migrate_dst6->sin6_addr),
                    1, "migrate dst fail");
    migrate_dst6->sin6_family = AF_INET6;
    migrate_dst6->sin6_len =
        sizeof(*migrate_dst6) + 100; // Bad value to trigger exploit
    uint16_t len = sizeof(*migrate_dst_address_payload) +
                   PFKEY_ALIGN8(migrate_dst6->sin6_len);
    migrate_dst_address_payload->sadb_address_len = PFKEY_UNIT64(len);
    tlen += PFKEY_ALIGN8(migrate_dst6->sin6_len);
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send migrate sa");
}

static void send_pfkey_get_sa_stat(int pfkey_socket, uint32_t spi,
  ...
  struct sadb_session_id *session_id_payload =
      (struct sadb_session_id *)(void *)(payload + tlen);
  session_id_payload->sadb_session_id_len =
  ...
  struct sadb_sastat *sadb_stat_payload =
      (struct sadb_sastat *)(void *)(payload + tlen);
  uint16_t length =
      sizeof(*sadb_stat_payload) + PFKEY_ALIGN8(sizeof(struct sastat));
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send get sa stat");
}

static void send_pkey_delete_sa(int pfkey_socket, uint32_t spi) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send delete sa");
}

static void send_pfkey_sa_dump_message(int pfkey_socket) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
  ...
  T_ASSERT_POSIX_SUCCESS(send(pfkey_socket, payload,
                              (size_t)PFKEY_UNUNIT64(msg_payload->sadb_msg_len),
                              0),
                         "pfkey send sa dump failed");
}

static void send_pfkey_flush_sa(int pfkey_socket) {
  uint8_t payload[MCLBYTES] __attribute__((aligned(32)));
  bzero(payload, sizeof(payload));
  uint16_t tlen = 0;
...
static void pfkey_cleanup(void) {
  ...
  if (oldmax != 0) {
    (void)sysctlbyname("kern.ipc.maxsockbuf", NULL, NULL, &oldmax,
                       sizeof(oldmax));
  }
}

static int pfkey_setup_socket(void) {
  int pfkey_socket = -1;
  int bufsiz = 0;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(pfkey_socket = socket(PF_KEY, SOCK_RAW, PF_KEY_V2),
                         NULL);

  if (sysctlbyname("kern.ipc.maxsockbuf", &oldmax, &oldmaxsize, &newmax,
                   sizeof(newmax)) != 0) {
  ...
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(pfkey_socket, SOL_SOCKET, SO_SNDBUF, &bufsiz, sizeof(bufsiz)),
      "pfkey set snd socket buf failed %d", bufsiz);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(pfkey_socket, SOL_SOCKET, SO_RCVBUF, &bufsiz, sizeof(bufsiz)),
      "pfkey set recv socket buf failed %d", bufsiz);

  pfkey_source =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, (uintptr_t)pfkey_socket,
...
static void pfkey_process_message_test_60822136(uint8_t **mhp,
  ...
  if (message->sadb_msg_errno) {
    T_QUIET;
    T_ASSERT_EQ(message->sadb_msg_type, SADB_X_SPDDUMP,
                "SADB error for type %u", message->sadb_msg_type);
    pfkey_cleanup();
    T_END;
  }
  ...
  case SADB_X_SPDADD: {
    struct sadb_x_policy *policy_message =
        (struct sadb_x_policy *)(void *)mhp[SADB_X_EXT_POLICY];
    T_QUIET;
    T_ASSERT_NOTNULL(policy_message, "spd add policy message is NULL");
  ...
  case SADB_X_SPDGET: {
    struct sadb_x_policy *policy_message =
        (struct sadb_x_policy *)(void *)mhp[SADB_X_EXT_POLICY];
    T_QUIET;
    T_ASSERT_NOTNULL(policy_message, "spd get policy message is NULL");
    T_QUIET;
    T_ASSERT_EQ(policy_id, policy_message->sadb_x_policy_id,
                "spd_get: spid mismatch %u != %u", policy_id,
                policy_message->sadb_x_policy_id);
  ...
  case SADB_X_SPDDELETE2: {
    struct sadb_x_policy *policy_message =
        (struct sadb_x_policy *)(void *)mhp[SADB_X_EXT_POLICY];
    T_QUIET;
    T_ASSERT_NOTNULL(policy_message, "spd delete2 policy message is NULL");
    T_QUIET;
    T_ASSERT_EQ(policy_id, policy_message->sadb_x_policy_id,
  ...
  case SADB_X_SPDDUMP: {
    struct sadb_x_policy *policy_message =
        (struct sadb_x_policy *)(void *)mhp[SADB_X_EXT_POLICY];
    T_QUIET;
    T_ASSERT_NOTNULL(policy_message, "spd dump policy message is NULL");
    T_QUIET;
    T_ASSERT_EQ(policy_id, policy_message->sadb_x_policy_id,
...
static void pfkey_process_message_test_60822924(uint8_t **mhp,
  ...
  case SADB_X_SPDENABLE: {
    struct sadb_x_policy *policy_message =
        (struct sadb_x_policy *)(void *)mhp[SADB_X_EXT_POLICY];
    T_QUIET;
    T_ASSERT_NOTNULL(policy_message, "spd enable policy message is NULL");
    T_QUIET;
    T_ASSERT_EQ(policy_id, policy_message->sadb_x_policy_id,
...
static void pfkey_process_message_test_60822956(uint8_t **mhp,
  ...
  case SADB_X_SPDDISABLE: {
    struct sadb_x_policy *policy_message =
        (struct sadb_x_policy *)(void *)mhp[SADB_X_EXT_POLICY];
    T_QUIET;
    T_ASSERT_NOTNULL(policy_message, "spd disable policy message is NULL");
    T_QUIET;
    T_ASSERT_EQ(policy_id, policy_message->sadb_x_policy_id,
...
static void pfkey_process_message_test_60679513(uint8_t **mhp,
  ...
  if (message->sadb_msg_errno) {
    T_QUIET;
    T_ASSERT_EQ(message->sadb_msg_type, SADB_UPDATE, "SADB error for type %u",
                message->sadb_msg_type);
  }
  ...
  case SADB_GETSPI: {
    ...
    T_QUIET;
    T_ASSERT_NOTNULL(sa_message, "sa get spi message is NULL");
  ...
  case SADB_UPDATE: {
    struct sadb_sa *sa_message = (struct sadb_sa *)(void *)mhp[SADB_EXT_SA];
    T_QUIET;
    T_ASSERT_NOTNULL(sa_message, "update sa message is NULL");
    T_QUIET;
    T_ASSERT_EQ(spi, ntohl(sa_message->sadb_sa_spi),
                "sadb update: spi mismatch %u != %u", spi,
                ntohl(sa_message->sadb_sa_spi));
  ...
  case SADB_DELETE: {
    struct sadb_sa *sa_message = (struct sadb_sa *)(void *)mhp[SADB_EXT_SA];
    T_QUIET;
    T_ASSERT_NOTNULL(sa_message, "delete sa message is NULL");
    T_QUIET;
    T_ASSERT_EQ(spi, ntohl(sa_message->sadb_sa_spi),
...
static void pfkey_process_message_test_60768729(uint8_t **mhp,
  ...
  T_QUIET;
  T_ASSERT_EQ(message->sadb_msg_errno, 0, "SADB error for type %u",
              message->sadb_msg_type);
...
static void pfkey_process_message_test_60769680(uint8_t **mhp,
  ...
  T_QUIET;
  T_ASSERT_EQ(message->sadb_msg_errno, 0, "SADB error for type %u error %d",
              message->sadb_msg_type, message->sadb_msg_errno);
...
static void pfkey_process_message_test_60822823(uint8_t **mhp,
  ...
  if (message->sadb_msg_errno != 0) {
    T_QUIET;
    T_ASSERT_EQ(message->sadb_msg_type, SADB_GETSASTAT,
                "SADB error for type %u error %d", message->sadb_msg_type,
                message->sadb_msg_errno);
    T_QUIET;
    T_ASSERT_EQ(message->sadb_msg_errno, EINVAL,
                "SADB error for type %u error %d", message->sadb_msg_type,
                message->sadb_msg_errno);
  ...
  case SADB_ADD: {
    struct sadb_sa *sa_message = (struct sadb_sa *)(void *)mhp[SADB_EXT_SA];
    T_QUIET;
    T_ASSERT_NOTNULL(sa_message, "add sa message is NULL");
    spi = ntohl(sa_message->sadb_sa_spi);
...
static void pfkey_process_message_test_60822823_1(uint8_t **mhp,
  ...
  case SADB_GETSASTAT: {
    struct sadb_session_id *session_id =
        (struct sadb_session_id *)(void *)mhp[SADB_EXT_SESSION_ID];
    T_QUIET;
    T_ASSERT_NOTNULL(session_id, "session id is NULL");
    T_QUIET;
    T_EXPECT_EQ_ULLONG(session_id->sadb_session_id_v[0], 1ULL,
...
static void pfkey_process_message_test_60687183(uint8_t **mhp,
  ...
  case SADB_ADD: {
    ...
    spi = ntohl(sa_message->sadb_sa_spi);
    T_LOG("added sa 0x%x", spi);
    send_pkey_migrate_sa(pfkey_socket, spi, TEST_SRC_ADDRESS_IPv4,
                         TEST_DST_ADDRESS_IPv4, AF_INET,
                         TEST_MIGRATE_SRC_ADDRESS_IPv4,
                         TEST_MIGRATE_DST_ADDRESS_IPv4, AF_INET);
    break;
  }
  case SADB_MIGRATE: {
...
static void pfkey_process_message_test_60687183_1(uint8_t **mhp,
  ...
  case SADB_ADD: {
    ...
    spi = ntohl(sa_message->sadb_sa_spi);
    T_LOG("added sa 0x%x", spi);
    send_pkey_migrate_sa(pfkey_socket, spi, TEST_SRC_ADDRESS_IPv6,
                         TEST_DST_ADDRESS_IPv6, AF_INET6,
                         TEST_MIGRATE_SRC_ADDRESS_IPv6,
                         TEST_MIGRATE_DST_ADDRESS_IPv6, AF_INET6);
    break;
  }
  case SADB_MIGRATE: {
    T_PASS("migrate SA success");
...
static void pfkey_process_message_test_60687183_2(uint8_t **mhp,
  ...
  if (message->sadb_msg_errno != 0) {
    T_QUIET;
    T_ASSERT_EQ(message->sadb_msg_type, SADB_MIGRATE,
                "SADB error for type %u error %d", message->sadb_msg_type,
                message->sadb_msg_errno);
    T_QUIET;
    T_ASSERT_EQ(message->sadb_msg_errno, EINVAL,
  ...
  case SADB_ADD: {
    ...
    send_pkey_migrate_sa(pfkey_socket, spi, TEST_SRC_ADDRESS_IPv6,
                         TEST_DST_ADDRESS_IPv6, AF_INET6,
                         TEST_MIGRATE_SRC_ADDRESS_IPv6,
                         TEST_MIGRATE_DST_ADDRESS_IPv6, AF_CHAOS);
    break;
  }
  case SADB_MIGRATE: {
...
static void pfkey_process_message_test_134671927(uint8_t **mhp,
  ...
  if (message->sadb_msg_type != SADB_EXPIRE &&
      message->sadb_msg_pid != (uint32_t)getpid()) {
    return;
  }
  ...
  case SADB_REGISTER: {
    ...
    send_pkey_add_sa(pfkey_socket, 0x12345678, TEST_SRC_ADDRESS_IPv6,
                     TEST_DST_ADDRESS_IPv6, AF_INET6);
    break;
  }
  case SADB_ADD: {
  ...
  case SADB_EXPIRE: {
    struct sadb_sa *sa_message = (struct sadb_sa *)(void *)mhp[SADB_EXT_SA];
    T_QUIET;
    T_ASSERT_NOTNULL(sa_message, "expire sa message is NULL");
...
static int setup_tcp_server(uint16_t port) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(server_fd = socket(AF_INET, SOCK_STREAM, 0),
  ...
  T_ASSERT_POSIX_SUCCESS(
      ...
      "tcp server bind failed");

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(listen(server_fd, 2), "tcp server listen failed");
  return server_fd;
}

static int setup_loopback_tcp_client(uint16_t server_port) {
  struct sockaddr_in conn_addr = {};
  int client_fd = -1;

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(client_fd = socket(AF_INET, SOCK_STREAM, 0),
                         "tcp client socket creation failed");
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(inet_pton(AF_INET, "127.0.0.1", &conn_addr.sin_addr),
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      connect(client_fd, (struct sockaddr *)&conn_addr, sizeof(conn_addr)),
      "tcp loopback client connect failed");
  return client_fd;
}

static void setup_socket_policy(int socket_fd) {
  uint8_t __attribute__((aligned(4)))
  buf[sizeof(struct sadb_x_policy) + sizeof(struct sadb_x_ipsecrequest) +
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(socket_fd, IPPROTO_IP, IP_IPSEC_POLICY, buf, sizeof(buf)),
...
T_DECL(sadb_policy_dump_60769680,
...
T_DECL(sadb_get_sastat_oob_60769680,
       "security association get sa stat oob read", T_META_TAG_VM_PREFERRED) {
  ...
  send_pfkey_flush_sa(pfkey_socket);
  send_pfkey_flush_sp(pfkey_socket);
  send_pkey_add_sa(pfkey_socket, 0x12345678, TEST_SRC_ADDRESS_IPv4,
                   TEST_DST_ADDRESS_IPv4, AF_INET);
...
T_DECL(sadb_key_migrate_address_ipv4,
       "security association migrate address ipv4", T_META_TAG_VM_PREFERRED) {
...
T_DECL(sadb_key_migrate_address_ipv6,
       "security association migrate address ipv6", T_META_TAG_VM_PREFERRED) {
...
T_DECL(sadb_key_migrate_after_expiry_134671927,
       "security association migrate after expiry", T_META_TAG_VM_PREFERRED) {
