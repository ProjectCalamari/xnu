...
static int my_system(const char *command) {
  ...
  int ret = sysctlbyname("vm.shared_region_pager_count", &after_regions, &size,
                         NULL, 0);
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(ret, "get shared_region_pager_count after");
...
static void cleanup(void) {
  ...
  if (orig_setting == 0) {
    ret = sysctlbyname("vm.vm_shared_region_by_entitlement", NULL, NULL, &off,
                       size_off);
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(ret, "turning sysctl back off");
  }
}
...
T_DECL(sr_entitlement, "shared region by entitlement test") {
  ...
  ret = sysctlbyname("vm.vm_shared_region_by_entitlement", &orig_setting, &size,
                     &on, size_on);
  if (ret != 0) {
    T_SKIP("No pointer authentication support");
  }
  ...
  ret = sysctlbyname("vm.shared_region_pager_count", &before_regions, &size,
                     NULL, 0);
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(ret, "get shared_region_pager_count before");
  T_QUIET;
  T_EXPECT_GE_INT(before_regions, 1, "invalid before number of regions");
  ...
  T_EXPECT_GT_INT(after_regions, before_regions,
