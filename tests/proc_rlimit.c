/*
 * cd $XNU/tests
 * xcrun -sdk macosx.internal/iphoneos.internal make proc_rlimit
 * LDFLAGS="-ldarwintest"
 */
...
static int get_initial_rlimits(void) {
  int err = -1;
  int i;
  ...
  for (i = 0; i < RLIMIT_NLIMITS; i++) {
    ...
    T_QUIET;
    T_EXPECT_EQ(0, err, "getrlimit(%15s, soft: 0x%16llx, hard 0x%16llx) %s",
                RESOURCE_STRING[i], orig_rlimit[i].rlim_cur,
                orig_rlimit[i].rlim_max, err == 0 ? "" : strerror(errno));
  }
  return err;
}

static void print_rlimits(bool initial_limits) {
  int err;
  int i;
  ...
  for (i = 0; i < RLIMIT_NLIMITS; i++) {
    ...
    if (initial_limits) {
      ...
      T_QUIET;
      T_EXPECT_EQ(0, err, "getrlimit(%15s, soft: 0x%16llx, hard 0x%16llx) %s",
                  RESOURCE_STRING[i], lim.rlim_cur, lim.rlim_max,
                  err == 0 ? "" : strerror(errno));
    }
    T_LOG("%35s soft: 0x%16llx hard 0x%16llx", RESOURCE_STRING[i], lim.rlim_cur,
          lim.rlim_max);
  }
}
 ...
 * action:     RAISE/LOWER
 */
static void change_rlimits(int limit_type, rlim_t amount, int action) {
  int err = -1;
  int i;
  ...
  for (i = 0; i < RLIMIT_NLIMITS; i++) {
    ...
    T_EXPECT_EQ(0, err, "getrlimit(%15s, soft: 0x%16llx, hard 0x%16llx) %s",
                RESOURCE_STRING[i], newlim.rlim_cur, newlim.rlim_max,
                err == 0 ? "" : strerror(errno));

    /* Changing soft limit */
    if (limit_type == SOFT_LIMIT) {
      if (action == RAISE) {
        /* Raising soft limits to exceed hard limits is not allowed and we
         * expect to see failure on setrlimit call later */
    ...
    /* Changing hard limit */
      ...
      if (action == RAISE) {
        ...
        /* Raising hard limits is not allowed for normal user and we expect to
         * see failure on setrlimit call later */
        expect_failure = TRUE;
        expect_errno = EPERM;
      } else if (action == LOWER) {
        if (newlim.rlim_max == 0) {
          /* Hard limit might be 0 already, if so skip lowering it (e.g.,
           * RLIMIT_CORE on iOS) */
        } else {
          newlim.rlim_max -= amount;
        }
        /* Soft limit might need to be changed as well since soft cannot be
         * greater than hard  */
    ...
    if (expect_failure) {
      ...
      T_EXPECT_EQ(-1, err,
                  "setrlimit(%15s, soft: 0x%16llx, hard 0x%16llx) failed as "
                  "expected: %s",
                  RESOURCE_STRING[i], newlim.rlim_cur, newlim.rlim_max,
                  strerror(errno));
      T_EXPECT_EQ(expect_errno, errno, "Expect errno %d, errno returned %d",
                  expect_errno, errno);
      continue;
    } else {
      T_EXPECT_EQ(0, err, "setrlimit(%15s, soft: 0x%16llx, hard 0x%16llx) %s",
                  RESOURCE_STRING[i], newlim.rlim_cur, newlim.rlim_max,
                  err == 0 ? "" : strerror(errno));
    }
    ...
    T_EXPECT_EQ(0, err, "getrlimit(%15s, soft: 0x%16llx, hard 0x%16llx) %s",
                RESOURCE_STRING[i], verifylim.rlim_cur, verifylim.rlim_max,
                err == 0 ? "" : strerror(errno));

    /* The kernel forces the hard limit of RLIMIT_NOFILE to be at most
     * maxfileperproc for normal user when changing the hard limit with
     * setrlimit */
    if (i == RLIMIT_NOFILE && limit_type == HARD_LIMIT &&
        newlim.rlim_max > maxfilesperproc) {
      if (newlim.rlim_cur != verifylim.rlim_cur ||
        ...
        T_FAIL("Mismatch limit values %s despite a successful setrlimit call "
               "(setrlimit'd soft 0x%16llx hard 0x%16llx but getrlimit'd soft "
               "0x%16llx hard 0x%16llx)",
               RESOURCE_STRING[i], newlim.rlim_cur, newlim.rlim_max,
               verifylim.rlim_cur, verifylim.rlim_max);
      }
    }
    /* The kernel forces the hard limit of RLIMIT_NPROC to be at most maxproc
       for normal user when changing either soft/hard limit with setrlimit */
...
T_DECL(proc_rlimit, "Test basic functionalities of the getrlimit and setrlimit",
  ...
  /* Use sysctl to query the real limits of RLIMIT_NOFILE/RLIMIT_NPROC for
   * normal user on Apple's systems */
  err = sysctlbyname("kern.maxfilesperproc", &maxfilesperproc,
                     &maxfilesperproc_size, NULL, 0);
  T_EXPECT_EQ_INT(0, err, "maxfilesperproc: %llu", maxfilesperproc);

  err = sysctlbyname("kern.maxprocperuid", &maxprocperuid, &maxprocperuid_size,
                     NULL, 0);
  T_EXPECT_EQ_INT(0, err, "maxprocperuid: %llu", maxprocperuid);

  /* Use sysctl to query the real limits of RLIMIT_NOFILE/RLIMIT_NPROC for super
   * user on Apple's systems (placeholder for adding super user tests) */
  ...
  /* Issue getrlimit syscall to retrieve the initial resource limit values
   * before calling setrlimit */
  err = get_initial_rlimits();
  T_EXPECT_EQ(0, err, "Obtained initial resource values.");

  /* Print out resource limit values to stdout for less-painful triage in case
   * needed */
  ...
  /* Raise soft limits to exceed hard limits (setrlimit should fail, but the
   * darwintest should pass) */
  T_LOG("---------Attempting to raised soft limits by 0x%x to exceed hard "
        "limits---------:",
        LIMIT_DIFF);
  change_rlimits(SOFT_LIMIT, LIMIT_DIFF, RAISE);
   ...
   */
  T_LOG("---------Attempting to raise hard limits by 0x%x---------:",
        LIMIT_DIFF);
  ...
  T_EXPECT_EQ(
      ...
      strerror(errno));
  T_EXPECT_EQ(EINVAL, errno, "Expect errno %d, errno returned %d", EINVAL,
              errno);
