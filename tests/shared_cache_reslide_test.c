...
T_GLOBAL_META(T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("VM"), T_META_OWNER("eperla"),
              T_META_RUN_CONCURRENTLY(true));

#if (__arm64e__) && (TARGET_OS_IOS || TARGET_OS_OSX)
static void *get_current_slide_address(bool reslide) {
  pid_t pid;
  int pipefd[2];
  ...
  if (reslide) {
    T_ASSERT_POSIX_SUCCESS(
        posix_spawnattr_setflags(&attr, _POSIX_SPAWN_RESLIDE),
        "posix_spawnattr_setflags");
  }

  T_ASSERT_POSIX_SUCCESS(pipe(pipefd), "pipe");
  T_ASSERT_POSIX_ZERO(posix_spawn_file_actions_init(&action),
                      "posix_spawn_fileactions_init");
  T_ASSERT_POSIX_ZERO(posix_spawn_file_actions_addclose(&action, pipefd[0]),
                      "posix_spawn_file_actions_addclose");
  T_ASSERT_POSIX_ZERO(posix_spawn_file_actions_adddup2(&action, pipefd[1], 1),
                      "posix_spawn_file_actions_addup2");
  T_ASSERT_POSIX_ZERO(posix_spawn_file_actions_addclose(&action, pipefd[1]),
                      "posix_spawn_file_actions_addclose");
  ...
  T_ASSERT_POSIX_ZERO(
      posix_spawn(&pid, SHARED_CACHE_HELPER, &action, &attr, argvs, envps),
      "helper posix_spawn");
  ...
  T_ASSERT_EQ(WEXITSTATUS(status), EX_OK,
 ...
 */
static char *build_faulting_shared_cache_address(uint8_t flags) {
  uintptr_t fault_address;

  // Grab currently mapped shared cache location and size
  size_t shared_cache_len = 0;
  const void *shared_cache_location =
  ...
  if (flags & TEST_FAULT_WRITE) {
    ...
    fault_address =
        (uintptr_t)shared_cache_location + shared_cache_len + PAGE_SIZE;
  } else {
...
static void induce_crash(volatile char *ptr, uint8_t how_to_crash) {
  pid_t child = fork();
  ...
  if (child == 0) {
    ...
    T_ASSERT_POSIX_SUCCESS(proc_pidinfo(child, PROC_PIDEXITREASONBASICINFO, 1,
    ...
    T_ASSERT_EQ(exit_reason.beri_namespace, OS_REASON_SIGNAL,
                "child should have exited with a signal");

    if (ptr) {
      if (how_to_crash == INDUCE_CRASH_READ) {
        T_ASSERT_EQ_ULLONG(exit_reason.beri_code, (unsigned long long)SIGSEGV,
                           "child should have received SIGSEGV");
      }

      if (how_to_crash == INDUCE_CRASH_WRITE) {
        T_ASSERT_EQ_ULLONG(exit_reason.beri_code, (unsigned long long)SIGBUS,
                           "child should have received SIGBUS");
      }

      T_ASSERT_NE(
          (int)(exit_reason.beri_flags & OS_REASON_FLAG_SHAREDREGION_FAULT), 0,
          "should detect shared cache fault");
    } else {
      T_ASSERT_EQ(
          (int)(exit_reason.beri_flags & OS_REASON_FLAG_SHAREDREGION_FAULT), 0,
          "should not detect shared cache fault");
    }
  }
}
...
static void cleanup_sysctl(void) {
  ...
  if (saved_status == 0) {
    ret = sysctlbyname("vm.vm_shared_region_reslide_aslr", NULL, NULL,
                       &saved_status, sizeof(saved_status));
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(ret, "set shared region resliding back off");
  }
}
...
T_DECL(reslide_sharedcache, "crash induced reslide of the shared cache",
       ...
       T_META_IGNORECRASHES(".*shared_cache_reslide_test.*"),
       T_META_ASROOT(true)) {
#if (__arm64e__) && (TARGET_OS_IOS || TARGET_OS_OSX)
  void *system_address;
  ...
  /* Force resliding on */
  T_ASSERT_POSIX_SUCCESS(sysctlbyname("vm.vm_shared_region_reslide_aslr",
                                      &saved_status, &size, &on, sizeof(on)),
  ...
  T_ASSERT_EQ_PTR(system_address, confirm_address,
                  "system and current addresses should not diverge %p %p",
                  system_address, confirm_address);

  reslide_address = get_current_slide_address(true);
  confirm_address = get_current_slide_address(true);
  T_ASSERT_NE_PTR(system_address, reslide_address,
                  "system and reslide addresses should diverge %p %p",
                  system_address, reslide_address);
  T_ASSERT_EQ_PTR(
      reslide_address, confirm_address,
      "reslide and another reslide (no crash) shouldn't diverge %p %p",
      reslide_address, confirm_address);
  ...
  reslide_address = get_current_slide_address(true);
  T_ASSERT_NE_PTR(system_address, reslide_address,
                  "system and reslide should diverge (after crash) %p %p",
                  system_address, reslide_address);
  T_ASSERT_NE_PTR(
      confirm_address, reslide_address,
      "reslide and another reslide should diverge (after crash) %p %p",
      confirm_address, reslide_address);

  confirm_address = get_current_slide_address(true);
  T_ASSERT_EQ_PTR(
      reslide_address, confirm_address,
      "reslide and another reslide shouldn't diverge (no crash) %p %p",
      reslide_address, confirm_address);
  ...
  induce_crash(ptr, INDUCE_CRASH_READ);
  confirm_address = get_current_slide_address(true);
  T_ASSERT_EQ_PTR(reslide_address, confirm_address,
                  "reslide and another reslide after a non-tracked crash "
                  "shouldn't diverge %p %p",
                  reslide_address, confirm_address);

  /* Ensure we still get the system address */
  confirm_address = get_current_slide_address(false);
  T_ASSERT_EQ_PTR(system_address, confirm_address,
                  "system address and new process without resliding shouldn't "
                  "diverge %p %p",
                  system_address, confirm_address);
  ...
  T_ASSERT_NE_PTR(
      system_address, reslide_address,
      "system and reslide should diverge (after crash, TBI test) %p %p",
      system_address, reslide_address);
  T_ASSERT_NE_PTR(confirm_address, reslide_address,
                  "reslide and another reslide should diverge (after crash, "
                  "TBI test) %p %p",
                  confirm_address, reslide_address);
  ...
  T_ASSERT_NOTNULL(ptr, "faulting on write on %p in the shared region",
                   (void *)ptr);
  confirm_address = get_current_slide_address(true);
  ...
  T_ASSERT_NE_PTR(
      system_address, reslide_address,
      "system and reslide should diverge (after crash, WRITE test) %p %p",
      system_address, reslide_address);
  T_ASSERT_NE_PTR(confirm_address, reslide_address,
                  "reslide and another reslide should diverge (after crash, "
                  "WRITE_TEST) %p %p",
