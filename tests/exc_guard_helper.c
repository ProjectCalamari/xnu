...
/*
 * Global data shared between the code running the block and the exception
 * handler. Ideally this would be thread-local data in the thread running the
 * block, but the exception handler runs on a different thread and can't see it.
 */
...
static pthread_mutex_t exc_guard_helper_request_mutex =
...
static const char *name_for_guard_type(unsigned guard_type) {
...
static size_t exc_guard_helper_exception_handler(
    __unused mach_port_t task, mach_port_t thread, exception_type_t exception,
    mach_exception_data_t codes, __unused uint64_t exception_pc) {
  T_QUIET;
  T_ASSERT_EQ(exception, EXC_GUARD, "exception type");
  T_QUIET;
  T_ASSERT_POSIX_ZERO(pthread_mutex_lock(&exc_guard_helper_request_mutex),
                      "lock");
  ...
  if (thread != exc_guard_helper_request.thread) {
    ...
    if (verbose_exc_helper) {
      T_LOG(
          "exc_guard_helper caught an exception but nobody is waiting for it");
    }
    T_QUIET;
    T_ASSERT_POSIX_ZERO(pthread_mutex_unlock(&exc_guard_helper_request_mutex),
                        "unlock");
    return 0;
  }
  ...
  if (verbose_exc_helper) {
    T_LOG("exc_guard_helper caught EXC_GUARD type %u (%s), flavor %u, "
          ...
          exc_guard_type, name_for_guard_type(exc_guard_type), exc_guard_flavor,
          exc_guard_target, exc_guard_payload,
 ...
 */
static void initialize_exception_handlers(void) {
  if (exc_guard_helper_exc_port == MACH_PORT_NULL) {
    exc_guard_helper_exc_port = create_exception_port(EXC_MASK_GUARD);
    T_QUIET;
    T_ASSERT_NE(exc_guard_helper_exc_port, MACH_PORT_NULL, "exception port");
    repeat_exception_handler(exc_guard_helper_exc_port,
...
void exc_guard_helper_init(void) {
  T_QUIET;
  T_ASSERT_POSIX_ZERO(pthread_mutex_lock(&exc_guard_helper_mutex), "lock");
  initialize_exception_handlers();
  T_QUIET;
  T_ASSERT_POSIX_ZERO(pthread_mutex_unlock(&exc_guard_helper_mutex), "unlock");
}
...
configure_exc_guard_of_type(unsigned int guard_type,
...
unimplemented_guard_type:
   ...
   */
  T_FAIL("guard type %u (%s) is unimplemented in exc_guard_helper", guard_type,
         name_for_guard_type(guard_type));
  T_END;
}

task_exc_guard_behavior_t enable_exc_guard_of_type(unsigned int guard_type) {
  kern_return_t kr;
  ...
  T_ASSERT_MACH_SUCCESS(kr,
                        "exc_guard_helper calling task_get_exc_guard_behavior");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr,
                        "exc_guard_helper calling task_set_exc_guard_behavior "
                        "to enable guard type %u %s",
                        guard_type, name_for_guard_type(guard_type));

  return old_behavior;
}

bool block_raised_exc_guard_of_type(unsigned int guard_type,
...
bool block_raised_exc_guard_of_type_ignoring_translated(
