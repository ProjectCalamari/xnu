...
void create_target_directory(const char *the_targetp) {
  ...
  for (;;) {
    ...
    T_ASSERT_LT(strlen(&my_name[0]) + strlen(the_targetp) + 2,
...
/*
 * create_random_name - creates a file with a random / unique name in the given
 * directory. when do_open is true we create a file else we generate a name that
 * does not exist in the given directory (we do not create anything when do_open
 * is 0). WARNING - caller provides enough space in path buffer for longest
 * possible name. WARNING - assumes caller has appended a trailing '/' on the
 * path passed to us. RAND_MAX is currently 2147483647 (ten characters plus one
 * for a slash)
 */
int create_random_name(char *the_pathp, int do_open) {
  ...
  for (i = 0; i < 1; i++) {
    ...
    T_ASSERT_LT_ULONG((strlen(&my_name[0]) + strlen(the_pathp) + 2),
                      (unsigned long)PATH_MAX,
    ...
    *(myp + 1) = 0x00;
    ...
    if (do_open) {
      ...
      T_EXPECT_TRUE((my_fd != -1 || errno == EEXIST), "open file with name %s",
                    the_pathp);
      ...
      T_EXPECT_TRUE((my_err == 0 || errno == ENOENT),
                    "make sure the name is unique");
...
void remove_target_directory() { rmdir(&g_target_path[0]); }
