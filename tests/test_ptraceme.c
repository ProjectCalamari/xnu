...
T_GLOBAL_META(T_META_NAMESPACE("xnu.spawn"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_OWNER("jainam_shah"), T_META_RUN_CONCURRENTLY(true));

T_DECL(test_ptraceme, "Test that ptraced process is stopped when it execs",
       T_META_ASROOT(true), T_META_ENABLED(TARGET_OS_OSX),
  ...
  if (child_pid == -1) {
    ...
    execl("/bin/echo", "echo",
          "/bin/echo executed - this should not happen before parent has "
          "detached!",
          NULL);
    T_FAIL("execl failed with error: %d: %s", errno, strerror(errno));
  } else {
    /* Parent */
    T_LOG("Parent %d: Calling waitpid(%d, NULL, WUNTRACED)", getpid(),
          child_pid);
    int child_status = 0;
    ...
    T_EXPECT_TRUE(WIFSTOPPED(child_status),
                  "Parent %d: waitpid() indicates that child %d is now stopped "
                  "for tracing",
                  getpid(), child_pid);

    T_LOG("Parent %d: Calling ptrace(PT_DETACH, %d, NULL, 0)", getpid(),
          child_pid);
