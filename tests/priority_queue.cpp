...
static int compare_numbers_descending(const void *a, const void *b) {
...
static void dump_pqueue_entry(priority_queue_entry_sched_t e, int depth) {
...
__unused static void dump_pqueue(struct priority_queue_sched_max *pq) {
...
T_DECL(priority_queue_sched_max, "Basic sched priority queue testing") {
  ...
  qsort(priority_list, PRIORITY_QUEUE_NODES, sizeof(priority_list[0]),
        compare_numbers_descending);
  ...
  for (int i = 0; i < PRIORITY_QUEUE_NODES; i++) {
    key = priority_queue_max_sched_pri(&pq);
    T_ASSERT_EQ(key, priority_list[i], "[%d] priority queue max node removal",
                i);
...
T_DECL(priority_queue_max, "Basic generic priority queue testing") {
  ...
  T_ASSERT_EQ(result->node_key, increase_pri,
              "verify priority_queue_entry_increased() operation");
  ...
  T_ASSERT_TRUE(update_result, "decrease key updated root");
  result = priority_queue_max(&pq, union test_node, e);
  T_ASSERT_EQ(result->node_key, decrease_pri,
              "verify priority_queue_entry_decreased() operation");
...
T_DECL(priority_queue_sched_stable_max,
  ...
  for (int i = 0; i < PRIORITY_QUEUE_NODES; i++) {
    ...
    priority_queue_entry_set_sched_pri(&pq, &node->se, config[i].pri,
                                       config[i].modifier);
    priority_queue_insert(&pq, &node->se);
  }

  /* Sort the local list in descending order */
  qsort_b(config, PRIORITY_QUEUE_NODES, sizeof(struct config),
  ...
  for (int i = 0; i < PRIORITY_QUEUE_NODES; i++) {
    ...
    T_LOG("[%d]: { pri: %2d, modifier: %d, stamp: %lld }\n", i, config[i].pri,
          config[i].modifier, config[i].stamp);
    auto pri = priority_queue_entry_sched_pri(&pq, &node->se);
    T_ASSERT_EQ(pri, config[i].pri, "[%d] priority queue max node removal", i);
