...
T_DECL(mach_port_insert_right, "insert send right for an existing right",
  ...
  T_ASSERT_MACH_SUCCESS(
      retval, "insert a send right for port=[%d] with name=[%d]", port, port);
  ...
  /* 0x7B, lower two bits are 0b11 which is valid gen number, but index is
   * invalid (0) */
  retval = mach_port_insert_right(task, name, port, MACH_MSG_TYPE_MAKE_SEND);
  T_ASSERT_MACH_ERROR(retval, KERN_FAILURE,
  ...
  retval = mach_port_insert_right(task, name, port, MACH_MSG_TYPE_MAKE_SEND);
  T_ASSERT_MACH_ERROR(retval, KERN_INVALID_VALUE,
                      "insert a send right for port=[%d] with name=[%d]", port,
                      name);
  ...
  T_ASSERT_MACH_ERROR(retval, KERN_RIGHT_EXISTS,
