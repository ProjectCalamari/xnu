...
/*
 * rdar://89640053 (Rome 22A201 - panic: assertion failed:
 * RB_EMPTY(&imf->imf_sources))
 */
...
static void print_mreq(const char *opt, struct ip_mreq *mreq) {
  char *imr_multiaddr = strdup(inet_ntoa(mreq->imr_multiaddr));
  char *imr_interface = strdup(inet_ntoa(mreq->imr_interface));

  T_LOG("%s imr_multiaddr: %s imr_interface: %s", opt, imr_multiaddr,
        imr_interface);
...
static void print_mreq_src(const char *opt, struct ip_mreq_source *mreq_src) {
  ...
  T_LOG("%s imr_multiaddr: %s imr_sourceaddr: %s imr_interface: %s", opt,
        imr_multiaddr, imr_sourceaddr, imr_interface);
...
static void test_ip_drop_membership(uint32_t max) {
  ...
  for (uint32_t i = 0; i < max; i++) {
    ...
    if (setsockopt(fd, IPPROTO_IP, IP_ADD_SOURCE_MEMBERSHIP, (void *)&mreq_src,
                   sizeof(mreq_src)) == -1) {
      if (errno == ETOOMANYREFS || errno == EADDRNOTAVAIL || errno == ENOMEM) {
        T_LOG("setsockopt(IPPROTO_IP, IP_ADD_SOURCE_MEMBERSHIP) %s",
              strerror(errno));
        break;
      }
      T_ASSERT_POSIX_SUCCESS(
          -1, "setsockopt(IPPROTO_IP, IP_ADD_SOURCE_MEMBERSHIP)");
    }

    print_mreq("IP_DROP_MEMBERSHIP", &mreq);
    if (setsockopt(fd, 0, IP_DROP_MEMBERSHIP, (void *)&mreq, sizeof(mreq)) ==
        -1) {
