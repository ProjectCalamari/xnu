...
T_DECL(avoid_leaking_KASLR, "rdar://101248992", T_META_TAG_VM_PREFERRED) {
  ...
  *requestp = (struct msg_request_s){
      .header =
          {
              .msgh_bits = MACH_MSGH_BITS_SET(MACH_MSG_TYPE_MAKE_SEND_ONCE, 0,
   ...
   */
  kr = mach_msg(&requestp->header, MACH_SEND_MSG, sizeof(*requestp), 0,
                MACH_PORT_NULL, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
  T_ASSERT_MACH_SUCCESS(kr, "sending message to sync port");

  /*
   * The EV_DISPATCH is required that can pass
   * filt_machport_kqueue_has_turnstile(). The received message will be copied
   * out to replyp. In filt_machport_stash_port(), the value in ext[3] will be
   * set to the mq_port object.
   */
  ...
  assert(MACH_PORT_VALID(replyp->port.name) &&
         replyp->port.disposition == MACH_MSG_TYPE_MOVE_RECEIVE);

  struct kevent_extinfo extinfo;
  int knotes =
      __proc_info(PROC_INFO_CALL_PIDFDINFO, getpid(), PROC_PIDFDKQUEUE_EXTINFO,
                  kq, (user_addr_t)&extinfo, sizeof(extinfo));
