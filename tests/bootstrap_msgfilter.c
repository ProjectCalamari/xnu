...
T_GLOBAL_META(T_META_RUN_CONCURRENTLY(true), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_NAMESPACE("xnu.ipc"), T_META_TAG_VM_PREFERRED);
...
T_DECL(bootstrap_mig_always_filtered,
       "'MIG' messages to bootstrap ports from tasks with filtering should "
       "always be filtered",
  ...
  kern_return_t kr = mach_port_construct(
      mach_task_self(), &port_opts, (uintptr_t)&ctxobj, &test_bootstrap_port);
