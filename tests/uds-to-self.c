 ...
 *  uds-to-self.c
 *
 *  Test for rdar://137498122 (panic when calling unp_get_locks_in_order without
 * checking if the 2 sockets are equal).
 */
...
T_DECL(uds_self_connection, "self-connecting Unix domain sockets") {
  ...
  T_ASSERT_POSIX_SUCCESS(getsockname(fd, (struct sockaddr *)&sun, &solen),
                         NULL);
  T_LOG("socket bound to %s", sun.sun_path);

  T_ASSERT_POSIX_SUCCESS(connect(fd, (struct sockaddr *)&sun, sun.sun_len),
                         NULL);

  T_ASSERT_POSIX_SUCCESS(getpeername(fd, (struct sockaddr *)&sun, &solen),
                         NULL);
  ...
  /*
   * The return value of getsockopt() is not important, what matters is to not
   * panic
   */
  solen = sizeof(xucred);
