...
static const struct rlimit lim_infty = {RLIM_INFINITY, RLIM_INFINITY};

static volatile int stop_looking = 0;

static const struct timespec timeout = {TIMEOUT, 0};
...
static int fork_and_wait_for_segfault() {
  ...
  if (pid == 0) {
    unsigned int *ptr =
        (unsigned int *)0x30; /* Cause a segfault so that we get a coredump */
    *ptr = 0xdeadd00d;
    T_FAIL(
        "Expected segmentation fault on write to invalid memory address 0x30");
  }
...
static void look_for_coredump(const char *format, int pid, int kqfd,
                              struct kevent *kev) {
  int ret = 0;
  int i = 0;
  char buf[BUFFLEN];
...
static void sysctl_enable_coredumps(void) {
  ...
  ret =
      sysctlbyname(coredump_ctl, buf, &oldlen, &enable_core_dump, sizeof(int));
...
T_DECL(proc_core_name_24152432,
       "Tests behavior of core dump when kern.corefile ends in %, e.g., "
       "/cores/core.%",
       T_META_ASROOT(true), T_META_IGNORECRASHES("proc_core_name_24152432.*"),
  ...
  T_WITH_ERRNO;
  T_EXPECT_NE(sig, SIG_ERR, "signal: set sigalrm handler");

  dirp = opendir(dump_dir);
  T_ASSERT_NOTNULL(dirp, "opendir: opening coredump directory");
  dir = dirfd(dirp);
  T_ASSERT_POSIX_SUCCESS(
      dir, "dirfd: getting file descriptor for coredump directory");
  ...
  T_ASSERT_POSIX_SUCCESS(
      ret, "sysctl: set bad core dump location, old value was %s", buf);
  memset(buf, 0, BUFFLEN);
  oldlen = BUFFLEN;
  ...
  ret = sysctlbyname(corefile_ctl, buf, &oldlen, valid_dump_loc,
                     strlen(valid_dump_loc));
  T_ASSERT_POSIX_SUCCESS(
      ret, "sysctl: set valid core dump location, old value was %s", buf);
  memset(buf, 0, BUFFLEN);
