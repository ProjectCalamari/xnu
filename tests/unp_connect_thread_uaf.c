...
static void server_thread1(char *path) {
...
static void server_thread2(char *path) {
  struct sockaddr_un server_sockaddr;
  memset(&server_sockaddr, 0, sizeof(struct sockaddr_un));
  server_sockaddr.sun_family = AF_UNIX;
  strcpy(server_sockaddr.sun_path, path);
...
static void try_to_connect(char *path) {
  struct sockaddr_un server_sockaddr;
  memset(&server_sockaddr, 0, sizeof(struct sockaddr_un));
  server_sockaddr.sun_family = AF_UNIX;
  strcpy(server_sockaddr.sun_path, path);
...
static void test_unp_connect_multithread() {
  int client_sock;
  char path[] = "/tmp/";
  char path1[256];
  char path2[256];
  ...
  for (int i = 0; i < 1024; i++) {
    ...
    T_ASSERT_POSIX_SUCCESS(bind(client_sock,
