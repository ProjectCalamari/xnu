...
T_DECL(proc_info_44873309,
  ...
  retval = proc_pidinfo(pid, PROC_PIDIPCTABLEINFO, 0, (void *)&table_info,
                        (uint32_t)sizeof(table_info));
  T_WITH_ERRNO;
  T_EXPECT_GT(retval, 0, "proc_pidinfo(PROC_PIDIPCTABLEINFO) returned %d",
              retval);
  T_EXPECT_EQ(
      retval, (int)sizeof(table_info),
  ...
  retval = proc_pidinfo(pid, PROC_PIDIPCTABLEINFO, 0, (void *)&table_info2,
                        (uint32_t)sizeof(table_info2));
  T_WITH_ERRNO;
  T_EXPECT_GT(retval, 0, "proc_pidinfo(PROC_PIDIPCTABLEINFO) returned %d",
              retval);
  T_EXPECT_EQ(
      retval, (int)sizeof(table_info2),
      "proc_pidinfo(PROC_PIDIPCTABLEINFO) table_size2 = %u, table_free2 = %u",
      table_info2.table_size, table_info2.table_free);

  T_EXPECT_EQ(table_info.table_free, table_info2.table_free + 1,
              "Comparing the table_free values");
}
