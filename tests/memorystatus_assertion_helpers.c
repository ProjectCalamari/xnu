...
int set_priority(pid_t pid, int32_t priority, uint64_t user_data,
  ...
  if (is_assertion_driven) {
    ...
    if (priority == JETSAM_PRIORITY_IDLE) {
      T_LOG("Relinquish ...assertion... priority(%d) for pid[%d]", priority,
            pid);
    } else {
  ...
  err = memorystatus_control(MEMORYSTATUS_CMD_SET_PRIORITY_PROPERTIES, pid,
                             flag, &mjp, sizeof(mjp));
...
boolean_t check_properties(pid_t pid, int32_t expected_priority,
  ...
  verbose = false;
  (void)get_priority_props(pid, verbose, &actual_priority, &actual_limit_mb,
                           &actual_user_data, &actual_state);
  ...
  /*
   * These tests use well defined limits, so we don't try to handle defaults
   * like a limit of <= 0 which typically applies a system-wide per process
   * limit.
   */

  if ((actual_priority != expected_priority) ||
      (actual_limit_mb != expected_limit_mb) ||
      (actual_user_data != expected_user_data)) {
    /* we have a mismatch */
    T_LOG("%s test failed: %s\n", test, PROP_CHECK_ERROR_STRING);

    if (actual_priority != expected_priority) {
      T_LOG("priority mismatch [actual / expected] [%d / %d]", actual_priority,
            expected_priority);
    }

    if (actual_limit_mb != expected_limit_mb) {
      T_LOG("limit mismatch [actual / expected] [%d / %d]", actual_limit_mb,
            expected_limit_mb);
    }

    if (actual_user_data != expected_user_data) {
      T_LOG("user data mismatch [actual / expected] [0x%llx / 0x%llx]",
            actual_user_data, expected_user_data);
    }
...
int set_memlimits(pid_t pid, int32_t active_limit_mb, int32_t inactive_limit_mb,
  ...
  if (inactive_is_fatal) {
    ...
    mmprops.memlimit_inactive_attr &=
  ...
  err = memorystatus_control(MEMORYSTATUS_CMD_SET_MEMLIMIT_PROPERTIES, pid, 0,
...
boolean_t get_priority_props(pid_t pid, boolean_t verbose, int32_t *priority,
  ...
  int size = memorystatus_control(MEMORYSTATUS_CMD_GET_PRIORITY_LIST, pid, 0,
                                  &entry, sizeof(entry));
  ...
  T_LOG("get_priority_props: pid[%d] limit %d, user_data 0x%llx, priority %d, "
        "state 0x%x",
...
boolean_t verify_assertion_state(uint32_t state,
  ...
  if (actual_assertion_state == expected_assertion_state) {
    ...
    T_FAIL("state 0x%x:  %s but expected %s", state, actual_string,
           expected_string);
    // log_state(state);
    return false; /* failed */
  }
...
static void log_state(uint32_t state) {
  T_LOG("\t%s kMemorystatusSuspended",
        ((state & kMemorystatusSuspended) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusFrozen",
        ((state & kMemorystatusFrozen) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusWasThawed",
        ((state & kMemorystatusWasThawed) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusTracked",
        ((state & kMemorystatusTracked) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusSupportsIdleExit",
        ((state & kMemorystatusSupportsIdleExit) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusDirty",
        ((state & kMemorystatusDirty) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusAssertion",
        ((state & kMemorystatusAssertion) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusActive",
        ((state & kMemorystatusActive) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusRelaunchLow",
        ((state & kMemorystatusRelaunchLow) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusRelaunchMed",
        ((state & kMemorystatusRelaunchMed) ? "IS " : "NOT"));
  T_LOG("\t%s kMemorystatusRelaunchHigh",
        ((state & kMemorystatusRelaunchHigh) ? "IS " : "NOT"));
}
