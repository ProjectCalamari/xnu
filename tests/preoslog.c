 ...
 * To pass the test, one of the entries should match a substring in preoslog
 * buffer.
 */
static const char *g_preoslog_buffer_string[] = {"serial output"};

static boolean_t check_for_substrings(const char *string, size_t len) {
  int i;
  boolean_t res = FALSE;

  for (i = 0; i < (sizeof(g_preoslog_buffer_string) / sizeof(char *)); i++) {
    res = res || strnstr(string, g_preoslog_buffer_string[i], len) == NULL
              ? FALSE
              : TRUE;
  }
...
T_DECL(test_preoslog, "Validate kern.preoslog sysctl has expected log content "
  ...
  if (!is_development_kernel()) {
    ...
    T_ASSERT_POSIX_ERROR(
        ret, ENOENT, " get size kern.preoslog errno==ENOENT on release builds");
    return;
  }
  ...
  T_ASSERT_EQ(header->magic, g_valid_magic,
              "check preoslog header magic - expected %#x, given %#x",
              g_valid_magic, header->magic);
  T_ASSERT_EQ(header->size, size,
              "check preoslog sizes - expected %zu, given %zu", size,
              header->size);
  T_ASSERT_LT(header->offset, header->size - sizeof(*header),
              "check write offset");
