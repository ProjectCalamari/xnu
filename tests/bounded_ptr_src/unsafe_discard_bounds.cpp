...
template <typename T, typename QualT> static void tests() {
...

  {
    ...
    //   |                                                |
    // begin, ptr                                        end
    test_bounded_ptr<QualT> const ptr(array.begin() + 0, array.begin(),
                                      array.end());
  ...
  }
  {
    ...
    //   |        |                                       |
    // begin     ptr                                     end
    test_bounded_ptr<QualT> const ptr(array.begin() + 1, array.begin(),
                                      array.end());
    QualT *raw = ptr.unsafe_discard_bounds();
    _assert(raw == &array[1]);
  }
  {
    ...
    //   |                 |                              |
    // begin              ptr                            end
    test_bounded_ptr<QualT> const ptr(array.begin() + 2, array.begin(),
                                      array.end());
    QualT *raw = ptr.unsafe_discard_bounds();
    _assert(raw == &array[2]);
  }
  {
    ...
    //   |                                   |            |
    // begin                                ptr          end
    test_bounded_ptr<QualT> const ptr(array.begin() + 4, array.begin(),
                                      array.end());
    QualT *raw = ptr.unsafe_discard_bounds();
    _assert(raw == &array[4]);
  }
  ...
  }
  {
    ...
    //   |                          |        |
    // begin                       end      ptr
    test_bounded_ptr<QualT> ptr(array.end() - 1, array.begin(),
                                array.end() - 2);
    QualT *raw = ptr.unsafe_discard_bounds();
    _assert(raw == &array[4]);
  }
  ...
  // would fall outside of the representable memory.
  {
    ...
    QualT *end_of_memory =
        reinterpret_cast<QualT *>(std::numeric_limits<std::uintptr_t>::max());
    ...
    ptr +=
        offset; // now, `base_ + offset_` points outside of representable memory
