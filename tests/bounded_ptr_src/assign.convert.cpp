...
template <typename Stored, typename From, typename To> static void tests() {
  ...
  {
    libkern::bounded_ptr<From, dummy_policy1> from(ptr1, array.begin(),
                                                   array.end());
...
T_DECL(assign_convert, "bounded_ptr.assign.convert", T_META_TAG_VM_PREFERRED) {
  ...
  static_assert(!std::is_assignable_v</*to*/ test_bounded_ptr<Derived>,
                                      /*from*/ test_bounded_ptr<Base>>);
  static_assert(!std::is_assignable_v</*to*/ test_bounded_ptr<DerivedMultiple>,
                                      /*from*/ test_bounded_ptr<Base1>>);
  static_assert(!std::is_assignable_v</*to*/ test_bounded_ptr<DerivedMultiple>,
                                      /*from*/ test_bounded_ptr<Base2>>);
  static_assert(!std::is_assignable_v</*to*/ test_bounded_ptr<Base2>,
                                      /*from*/ test_bounded_ptr<Base1>>);

  // Make sure const-casting away doesn't work
  static_assert(
      !std::is_assignable_v</*to*/ test_bounded_ptr<Derived>,
                            /*from*/ test_bounded_ptr<Derived const>>);

  // Make sure casting to unrelated types doesn't work implicitly
  static_assert(!std::is_assignable_v</*to*/ test_bounded_ptr<char>,
                                      /*from*/ test_bounded_ptr<Derived>>);
  static_assert(!std::is_assignable_v</*to*/ test_bounded_ptr<Unrelated>,
                                      /*from*/ test_bounded_ptr<Derived>>);
  static_assert(!std::is_assignable_v</*to*/ test_bounded_ptr<Base2>,
                                      /*from*/ test_bounded_ptr<Base1>>);
