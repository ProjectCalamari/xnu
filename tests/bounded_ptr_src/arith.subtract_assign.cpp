...
struct tracking_policy {
  ...
  static void trap(char const *msg) { did_trap = true; }
};
bool tracking_policy::did_trap = false;
} // namespace

template <typename T, typename QualT> static void tests() {
  ...
  {
    std::ptrdiff_t sizeof_T =
        sizeof(T); // avoid promotion to unsigned in calculations

    // largest (most positive) n for the number of bytes `n * sizeof(T)` not to
    // overflow ptrdiff_t
    std::ptrdiff_t max_n =
        std::numeric_limits<std::ptrdiff_t>::max() / sizeof_T;

    // smallest (most negative) n for the number of bytes `n * sizeof(T)` not to
    // overflow ptrdiff_t
    std::ptrdiff_t min_n =
        std::numeric_limits<std::ptrdiff_t>::min() / sizeof_T;

    // Overflow with a positive offset
    {
      libkern::bounded_ptr<QualT, tracking_policy> ptr(
  ...
  {
    std::int64_t sizeof_T =
        sizeof(T); // avoid promotion to unsigned in calculations

    // largest (most positive) n for the number of bytes `n * sizeof(T)` not to
    // overflow the int32_t offset
    ...
    // Subtract positive offsets
    {
      ...
      _assert(!tracking_policy::did_trap);
      ptr -= (-min_n % 2);
      _assert(!tracking_policy::did_trap); // offset is now right at its
                                           // negative limit
      ptr -= 1;
      _assert(tracking_policy::did_trap);
    }
