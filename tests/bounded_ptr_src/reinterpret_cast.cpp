...
//  bounded_ptr<T, Policy> reinterpret_pointer_cast(bounded_ptr<U, Policy>
//  const& p) noexcept
//
...
template <typename Stored, typename From, typename To> static void tests() {
...

  {
    ...
    _assert(to.discard_bounds() ==
            reinterpret_cast<To const *>(from.discard_bounds()));
  }

  {
    ...
    test_bounded_ptr<To> to = libkern::reinterpret_pointer_cast<To>(from);
    _assert(to.discard_bounds() ==
            reinterpret_cast<To const volatile *>(from.discard_bounds()));
  }
  ...
  {
    libkern::bounded_ptr<From, non_default_policy> from(
        array.begin(), array.begin(), array.end());
    libkern::bounded_ptr<To, non_default_policy> to =
        libkern::reinterpret_pointer_cast<To>(from);
    _assert(to.discard_bounds() ==
            reinterpret_cast<To const *>(from.discard_bounds()));
  }
}
