...
test_bounded_ptr<void> bounded_malloc(std::size_t size) {
...
void bounded_free(test_bounded_ptr<void> ptr) {
...
T_DECL(example_malloc, "bounded_ptr.example.malloc", T_META_TAG_VM_PREFERRED) {
  ...
  array =
      libkern::reinterpret_pointer_cast<SomeType>(bounded_malloc(alloc_size));

  require(array != nullptr);   // use != nullptr instead of relying on implicit
                               // conversion to bool
