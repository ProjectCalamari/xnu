...
T_DECL(getpriority_setpriority,
  ...
  T_WITH_ERRNO;
  T_ASSERT_FALSE(my_priority == -1 && errno != 0,
                 "Verify getpriority is successful", NULL);

  /* change scheduling priority*/
  my_new_priority = (my_priority == PRIO_MIN) ? (my_priority + 10) : (PRIO_MIN);

  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(setpriority(PRIO_PROCESS, 0, my_new_priority),
  ...
  T_WITH_ERRNO;
  T_ASSERT_FALSE(my_priority == -1 && errno != 0,
                 "Verify getpriority change is successful", NULL);

  T_WITH_ERRNO;
  T_ASSERT_EQ(my_priority, my_new_priority,
              "Verify setpriority correctly set scheduling priority", NULL);

  /* reset scheduling priority */
  T_WITH_ERRNO;
  T_ASSERT_POSIX_SUCCESS(setpriority(PRIO_PROCESS, 0, 0),
                         "Reset scheduling priority", NULL);
}
