...
static const char *qos_env[ENV_VAR_QOS] = {"XNU_TEST_QOS_BO", "XNU_TEST_QOS_QO",
                                           "XNU_TEST_QOS_AO"};
static const char *qos_name_env[ENV_VAR_QOS] = {
...
static pthread_t thread_create_at_qos(qos_class_t qos,
                                      void *(*function)(void *));
static void send(mach_port_t send_port, mach_port_t reply_port,
                 mach_port_t msg_port, qos_class_t qos,
                 mach_msg_option_t options);
static void enable_kevent(uint64_t *workloop_id, unsigned long long port);
static void populate_kevent(struct kevent_qos_s *kev, unsigned long long port);

static void worker_cb(pthread_priority_t __unused priority) {
  T_FAIL("a worker thread was created");
}

static void event_cb(void **__unused events, int *__unused nevents) {
  T_FAIL("a kevent routine was called instead of workloop");
}

static uint32_t get_user_promotion_basepri(void) {
  ...
  kern_return_t kr =
      thread_policy_get(thread_port, THREAD_POLICY_STATE,
                        (thread_policy_t)&thread_policy, &count, &get_default);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "thread_policy_get");
  return thread_policy.thps_user_promotion_basepri;
}

static uint32_t get_thread_iotier(void) {
  mach_msg_type_number_t count = THREAD_POLICY_STATE_COUNT;
  struct thread_policy_state thread_policy;
  boolean_t get_default = FALSE;
  mach_port_t thread_port = pthread_mach_thread_np(pthread_self());
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "thread_policy_get");
  return (thread_policy.effective >> POLICY_EFF_IO_TIER_SHIFT) &
         POLICY_EFF_IO_TIER_MASK;
}

#define EXPECT_QOS_EQ(qos, ...)                                                \
  do {                                                                         \
 ...
 */
static void workloop_cb_test_intransit(uint64_t *workloop_id __unused,
                                       void **eventslist, int *events) {
 ...
 */
static void workloop_cb_test_sync_send(uint64_t *workloop_id __unused,
                                       void **eventslist, int *events) {
 ...
 * WL handler which checks if the servicer thread has correct Qos.
 */
static void workloop_cb_test_kernel_sync_send(uint64_t *workloop_id __unused,
                                              void **eventslist, int *events) {
 ...
 * WL handler which checks if the servicer thread has correct Qos.
 */
static void workloop_cb_test_kernel_async_send(uint64_t *workloop_id __unused,
                                               void **eventslist, int *events) {
  T_LOG("Workloop handler workloop_cb_test_kernel_sync_send called");
  ...
  T_ASSERT_EQ(get_thread_iotier(), USR_THROTTLE_LEVEL_TIER0,
              "Thread IOTier has correct override");
...
/*
 * WL handler which checks the overridden Qos and then enables the knote and
 * checks for the Qos again if that dropped the sync ipc override.
 */
static void workloop_cb_test_sync_send_and_enable(
    uint64_t *workloop_id, struct kevent_qos_s **eventslist, int *events) {
  ...
  /*
   * Check if the override has been dropped, check for priority instead of qos
   * since there will be async qos push.
   */
  reenable_priority = get_user_promotion_basepri();
  T_EXPECT_LT(reenable_priority, override_priority,
              "thread's current override priority %d should be less than "
              "override priority prior to enabling knote %d",
 ...
 * enables the knote and checks for the Qos again that it hasn't dropped the
 * sync ipc override.
 */
static void workloop_cb_test_sync_send_and_enable_handoff(
    uint64_t *workloop_id, struct kevent_qos_s **eventslist, int *events) {
  unsigned override_priority;
  int error;

  T_LOG(
      "Workloop handler workloop_cb_test_sync_send_and_enable_handoff called");
  ...
  error = kevent_id(*workloop_id, &handoff_kev, 1, &handoff_kev, 1, NULL, NULL,
                    KEVENT_FLAG_WORKLOOP | KEVENT_FLAG_ERROR_EVENTS |
                        KEVENT_FLAG_DYNAMIC_KQ_MUST_EXIST);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(error, "kevent_id");
...
/*
 * WL handler receives the first message and checks sync ipc override, then
 * enables the knote and receives 2nd message and checks it sync ipc override.
 */
static int send_two_sync_handler_called = 0;
static void workloop_cb_test_send_two_sync(uint64_t *workloop_id __unused,
                                           struct kevent_qos_s **eventslist,
                                           int *events) {
  T_LOG("Workloop handler workloop_cb_test_send_two_sync called for %d time",
        send_two_sync_handler_called + 1);
  ...
  if (send_two_sync_handler_called == 0) {
    ...
    *events = 1;
    ...
    EXPECT_QOS_EQ(g_expected_qos[ENV_QOS_BEFORE_OVERRIDE],
                  "dispatch_source event handler QoS should be %s",
                  g_expected_qos_name[ENV_QOS_BEFORE_OVERRIDE]);
    *events = 0;
    T_END;
  }
...
static void
workloop_cb_test_two_send_and_destroy(uint64_t *workloop_id __unused,
                                      struct kevent_qos_s **eventslist __unused,
                                      int *events) {
  T_LOG(
      "Workloop handler workloop_cb_test_two_send_and_destroy called %d times",
      two_send_and_destroy_handler + 1);
...
static mach_port_type_t get_reply_port(struct kevent_qos_s *kev) {
  ...
  hdr = (void *)kev->ext[0];
  T_QUIET;
  T_ASSERT_NOTNULL(hdr, "msg hdr");

  reply_port = hdr->msgh_remote_port;
  T_QUIET;
  T_ASSERT_TRUE(MACH_PORT_VALID(reply_port), "reply port valid");
  kr = mach_port_type(mach_task_self(), reply_port, &type);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_type");
  T_QUIET;
  T_ASSERT_TRUE(type & MACH_PORT_TYPE_SEND_ONCE, "send once received");

  return reply_port;
}

static void send_reply(mach_port_t reply_port) {
  kern_return_t kr;
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "server mach_msg");
}
...
static void populate_kevent(struct kevent_qos_s *kev, unsigned long long port) {
  ...
  kev->flags =
      EV_ADD | EV_ENABLE | EV_UDATA_SPECIFIC | EV_DISPATCH | EV_VANISHED;
  kev->fflags = (MACH_RCV_MSG | MACH_RCV_VOUCHER | MACH_RCV_LARGE |
...
static void enable_kevent(uint64_t *workloop_id, unsigned long long port) {
  ...
  error = kevent_id(*workloop_id, &kev, 1, kev_err, 1, NULL, NULL,
                    KEVENT_FLAG_WORKLOOP | KEVENT_FLAG_ERROR_EVENTS |
                        KEVENT_FLAG_DYNAMIC_KQ_MUST_EXIST);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(error, "kevent_id");
 ...
 */
static void workloop_cb_test_sync_send_reply(uint64_t *workloop_id __unused,
                                             struct kevent_qos_s **eventslist,
                                             int *events) {
  ...
  T_QUIET;
  T_ASSERT_EQ_INT(*events, 1, "events received");
  T_QUIET;
  T_ASSERT_EQ_INT((*eventslist)->filter, EVFILT_MACHPORT,
 ...
 */
static void
workloop_cb_test_sync_send_deallocate(uint64_t *workloop_id __unused,
                                      struct kevent_qos_s **eventslist,
                                      int *events) {
  mach_port_t reply_port;
  kern_return_t kr;
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_deallocate");
...
/*
 * WL handler which sends a msg to the client before enabling the event from
 * handler.
 */
static void workloop_cb_test_sync_send_reply_kevent(
    uint64_t *workloop_id, struct kevent_qos_s **eventslist, int *events) {
 ...
 * pthread.
 */
static void workloop_cb_test_sync_send_reply_kevent_pthread(
    uint64_t *workloop_id __unused, struct kevent_qos_s **eventslist,
    int *events) {
  T_LOG("Workloop handler workloop_cb_test_sync_send_reply_kevent_pthread "
        "called");
 ...
 */
static void workloop_cb_test_sync_send_kevent_reply(
    uint64_t *workloop_id, struct kevent_qos_s **eventslist, int *events) {
 ...
 */
static void
workloop_cb_test_sync_send_do_nothing(uint64_t *workloop_id __unused,
                                      struct kevent_qos_s **eventslist,
                                      int *events) {
 ...
 */
static void workloop_cb_test_sync_send_do_nothing_kevent_pthread(
    uint64_t *workloop_id __unused, struct kevent_qos_s **eventslist,
    int *events) {
  T_LOG("Workloop handler workloop_cb_test_sync_send_do_nothing_kevent_pthread "
        "called");
 ...
 */
static void
workloop_cb_test_sync_send_do_nothing_exit(uint64_t *workloop_id __unused,
                                           struct kevent_qos_s **eventslist,
                                           __unused int *events) {
 ...
 */
static void workloop_cb_test_sync_send_reply_kevent_reply_kevent(
    uint64_t *workloop_id __unused, struct kevent_qos_s **eventslist,
    int *events) {
  T_LOG("Workloop handler workloop_cb_test_sync_send_reply_kevent_reply_kevent "
        "called");
 ...
 * second sync sends a msg and reenables kevent after.
 */
static int workloop_cb_test_sync_send_kevent_reply_reply_kevent_handler_called =
    0;
static void workloop_cb_test_sync_send_kevent_reply_reply_kevent(
    uint64_t *workloop_id, struct kevent_qos_s **eventslist, int *events) {
  T_LOG("workloop handler workloop_cb_test_sync_send_kevent_reply_reply_kevent "
        "called");
  ...
  if (workloop_cb_test_sync_send_kevent_reply_reply_kevent_handler_called ==
      0) {
 ...
 */
static void workloop_cb_test_sync_send_kevent_reply_kevent_reply(
    uint64_t *workloop_id, struct kevent_qos_s **eventslist, int *events) {
  T_LOG("workloop handler workloop_cb_test_sync_send_kevent_reply_kevent_reply "
        "called");
 ...
 */
static int workloop_cb_test_sync_send_reply_kevent_kevent_reply_handler_called =
    0;
static void workloop_cb_test_sync_send_reply_kevent_kevent_reply(
    uint64_t *workloop_id, struct kevent_qos_s **eventslist, int *events) {
  T_LOG("workloop handler workloop_cb_test_sync_send_reply_kevent_kevent_reply "
        "called");
  ...
  if (workloop_cb_test_sync_send_reply_kevent_kevent_reply_handler_called ==
      0) {
...
static mach_port_t get_server_port(void) {
  mach_port_t port;
  kern_return_t kr =
      bootstrap_check_in(bootstrap_port, KEVENT_QOS_SERVICE_NAME, &port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "server bootstrap_check_in");
  return port;
}

static void env_set_qos(char **env, qos_class_t qos[], const char *qos_name[],
                        const char *wl_function) {
  int i;
  char *qos_str, *qos_name_str;
  for (i = 0; i < ENV_VAR_QOS; i++) {
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(asprintf(&qos_str, "%s=%d", qos_env[i], qos[i]),
                           NULL);
    T_QUIET;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      asprintf(&env[2 * i], "%s=%s", wl_function_name, wl_function), NULL);
  env[2 * i + 1] = NULL;
}

static void environ_get_qos(qos_class_t qos[], const char *qos_name[],
                            const char **wl_function) {
  char *qos_str;
  char *qos_end;
  int i;
  ...
  for (i = 0; i < ENV_VAR_QOS; i++) {
    ...
    T_QUIET;
    T_ASSERT_NOTNULL(qos_str, "getenv(%s)", qos_env[i]);

    unsigned long qos_l = strtoul(qos_str, &qos_end, 10);
    T_QUIET;
    T_ASSERT_EQ(*qos_end, '\0',
                "getenv(%s) = '%s' should be an "
                "integer",
                qos_env[i], qos_str);

    T_QUIET;
    T_ASSERT_LT(qos_l, (unsigned long)100,
                "getenv(%s) = '%s' should "
                "be less than 100",
                qos_env[i], qos_str);
    ...
    T_QUIET;
    T_ASSERT_NOTNULL(qos_name[i], "getenv(%s)", qos_name_env[i]);
  }
  *wl_function = getenv(wl_function_name);
  T_QUIET;
  T_ASSERT_NOTNULL(*wl_function, "getenv(%s)", wl_function_name);
}

static inline thread_qos_t thread_qos_from_qos_class(qos_class_t cls) {
...
static void send(mach_port_t send_port, mach_port_t reply_port,
  ...
  ret = mach_msg(&send_msg.header, send_opts, send_msg.header.msgh_size, 0,
                 MACH_PORT_NULL, 10000, priority);

  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "client mach_msg");
}

static kern_return_t receive(mach_port_t rcv_port, mach_port_t notify_port) {
  kern_return_t ret = 0;
  ...
  ret = mach_msg(&(rcv_msg.header),
                 MACH_RCV_MSG | MACH_RCV_TIMEOUT | MACH_RCV_SYNC_WAIT, 0,
                 rcv_msg.header.msgh_size, rcv_port, SEND_TIMEOUT_SECS * 1000,
                 notify_port);
...
static void *qos_client_send_to_intransit(void *arg __unused) {
  mach_port_t qos_send_port;
  mach_port_t msg_port;
  mach_port_t special_reply_port;

  kern_return_t kr = bootstrap_look_up(bootstrap_port, KEVENT_QOS_SERVICE_NAME,
                                       &qos_send_port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "client bootstrap_look_up");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "client mach_port_allocate");

  kr = mach_port_insert_right(mach_task_self(), msg_port, msg_port,
                              MACH_MSG_TYPE_MAKE_SEND);

  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "client mach_port_insert_right");
  ...
  /* Send the message with msg port as in-transit port, this msg will not be
   * dequeued */
  ...
  T_LOG("Sent 5 msgs, now trying to send sync ipc message, which will block "
        "with a timeout\n");
  /* Send the message to the in-transit port, it should block and override the
   * rcv's workloop */
...
T_HELPER_DECL(
    qos_client_send_to_intransit_with_thr_pri,
...
static void *qos_sync_rcv(void *arg __unused) {
  mach_port_t qos_send_port;
  mach_port_t special_reply_port;
...
static void thread_wait_to_block(mach_port_t thread_port) {
  ...
  while (1) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "thread_info");
...
T_HELPER_DECL(qos_client_sync_rcv_qos_change,
              "Send messages and syncronously wait for rcv and change qos of "
              "waiting thread") {
  pthread_t rcv_thread;

  rcv_thread = thread_create_at_qos(g_expected_qos[ENV_QOS_BEFORE_OVERRIDE],
                                    qos_sync_rcv);

  T_LOG("Waiting for %d seconds before changing qos of rcv thread",
        SEND_TIMEOUT_SECS);
  sleep(SEND_TIMEOUT_SECS);
  ...
  pthread_override_qos_class_start_np(
      rcv_thread, g_expected_qos[ENV_QOS_AFTER_OVERRIDE], 0);

  sleep(HELPER_TIMEOUT_SECS);
}

static void *qos_client_send_sync_msg_and_test_link(void *arg) {
  mach_port_t qos_send_port;
  mach_port_t special_reply_port;
  ...
  kr = bootstrap_look_up(bootstrap_port, KEVENT_QOS_SERVICE_NAME,
                         &qos_send_port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "client bootstrap_look_up");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_sync_ipc_link_monitoring_start");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_sync_ipc_link_monitoring_stop");
...
static void *qos_client_send_2sync_msg_and_test_link(void *arg) {
  mach_port_t qos_send_port;
  mach_port_t special_reply_port;
  boolean_t in_effect = FALSE;
  kern_return_t kr;
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "receive");
...
T_HELPER_DECL(qos_client_send_2sync_msg_with_link_check_correct_server,
  ...
  pthread_attr_init(&attr);
  pthread_create(&thread, &attr, qos_client_send_2sync_msg_and_test_link,
                 (void *)expected_result);
...
static void *qos_client_send_sync_msg(void *arg __unused) {
  mach_port_t qos_send_port;
  mach_port_t special_reply_port;
...
static void *qos_client_kernel_upcall_send_sync_msg(void *arg __unused) {
  mach_port_t qos_send_port;
  ...
  if (kr == KERN_NOT_SUPPORTED) {
    T_QUIET;
    T_SKIP(
        "QOS Client Kernel Upcall test called on release kernel, skipping\n");
  }

  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "task_test_sync_upcall");
...
T_HELPER_DECL(qos_client_send_kernel_upcall_sync_msg_with_pri,
  ...
  thread_create_at_qos(g_expected_qos[ENV_QOS_AFTER_OVERRIDE],
                       qos_client_kernel_upcall_send_sync_msg);
  sleep(HELPER_TIMEOUT_SECS);
}

static void *qos_client_kernel_upcall_send_async_msg(void *arg __unused) {
  mach_port_t qos_send_port;
   ...
   */
  kr = task_test_async_upcall_propagation(mach_task_self(), qos_send_port,
                                          THREAD_QOS_UTILITY,
                                          USR_THROTTLE_LEVEL_TIER0);
  if (kr == KERN_NOT_SUPPORTED) {
    T_QUIET;
    T_SKIP(
        "QOS Client Kernel Upcall test called on release kernel, skipping\n");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "task_test_async_upcall_propagation");
...
T_HELPER_DECL(qos_iotier_client_send_kernel_upcall_async_msg,
  ...
  thread_create_at_qos(g_expected_qos[ENV_QOS_AFTER_OVERRIDE],
                       qos_client_kernel_upcall_send_async_msg);
  sleep(HELPER_TIMEOUT_SECS);
}

static void *qos_client_send_two_sync_msg_high_qos(void *arg __unused) {
  mach_port_t qos_send_port;
  mach_port_t special_reply_port;
...
static void *qos_client_send_two_sync_msg_low_qos(void *arg __unused) {
  mach_port_t qos_send_port;
  mach_port_t special_reply_port;
...
static void *qos_client_destroy_other_threads_port(void *arg __unused) {
...
T_HELPER_DECL(qos_client_send_two_msg_and_destroy,
              "Send a message with another threads special reply port while "
              "that thread destroys the port") {
  thread_create_at_qos(g_expected_qos[ENV_QOS_AFTER_OVERRIDE],
...
static void *qos_client_send_complex_msg_to_service_port(void *arg __unused) {
  ...
  T_QUIET;
  T_ASSERT_TRUE(MACH_PORT_VALID(tsr_port), "get_thread_special_reply_port");

  conn_port = send_complex_connection_port;

  T_LOG("Sending to the service port with a sync IPC");
  send(svc_port, tsr_port, conn_port, g_expected_qos[ENV_QOS_BEFORE_OVERRIDE],
...
qos_client_send_to_connection_then_service_port(void *arg __unused) {
  ...
  send(conn_port, tsr_port, MACH_PORT_NULL,
       g_expected_qos[ENV_QOS_BEFORE_OVERRIDE], MACH_SEND_PROPAGATE_QOS);
...
static void run_client_server(const char *server_name, const char *client_name,
                              qos_class_t qos[], const char *qos_name[],
                              const char *wl_function) {
...
static void expect_kevent_id_recv(mach_port_t port, qos_class_t qos[],
                                  const char *qos_name[],
                                  const char *wl_function) {
  int r;
  ...
  if (strcmp(wl_function, "workloop_cb_test_intransit") == 0) {
    ...
    T_ASSERT_POSIX_ZERO(
        _pthread_workqueue_init_with_workloop(
            worker_cb, event_cb,
            (pthread_workqueue_function_workloop_t)workloop_cb_test_intransit,
            0, 0),
        NULL);
    ...
    T_ASSERT_POSIX_ZERO(
        _pthread_workqueue_init_with_workloop(
            worker_cb, event_cb,
            (pthread_workqueue_function_workloop_t)workloop_cb_test_sync_send,
            0, 0),
        NULL);
  } else if (strcmp(wl_function, "workloop_cb_test_sync_send_and_enable") ==
             0) {
    T_QUIET;
    ...
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            ...
                            0, 0),
                        NULL);
  } else if (strcmp(wl_function, "workloop_cb_test_two_send_and_destroy") ==
             0) {
    T_QUIET;
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            worker_cb, event_cb,
    ...
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            ...
                            0, 0),
                        NULL);
  } else if (strcmp(wl_function, "workloop_cb_test_sync_send_deallocate") ==
             0) {
    T_QUIET;
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            worker_cb, event_cb,
                            ...
                            0, 0),
                        NULL);
  } else if (strcmp(wl_function, "workloop_cb_test_sync_send_reply_kevent") ==
             0) {
    T_QUIET;
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            worker_cb, event_cb,
    ...
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            ...
                            0, 0),
                        NULL);
  } else if (strcmp(wl_function, "workloop_cb_test_sync_send_kevent_reply") ==
             0) {
    T_QUIET;
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            worker_cb, event_cb,
                            ...
                            0, 0),
                        NULL);
  } else if (strcmp(wl_function, "workloop_cb_test_sync_send_do_nothing") ==
             0) {
    T_QUIET;
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            worker_cb, event_cb,
    ...
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            worker_cb, event_cb,
                            (pthread_workqueue_function_workloop_t)
                                workloop_cb_test_kernel_sync_send,
                            0, 0),
                        NULL);
    ...
    T_ASSERT_POSIX_ZERO(_pthread_workqueue_init_with_workloop(
                            worker_cb, event_cb,
                            (pthread_workqueue_function_workloop_t)
                                workloop_cb_test_kernel_async_send,
                            0, 0),
                        NULL);
  } else {
  ...
  struct kevent_qos_s kev = {
      ...
      .fflags = (MACH_RCV_MSG | MACH_RCV_VOUCHER | MACH_RCV_LARGE |
                 MACH_RCV_LARGE_IDENTITY |
                 MACH_RCV_TRAILER_ELEMENTS(MACH_RCV_TRAILER_AV) |
                 MACH_RCV_TRAILER_TYPE(MACH_MSG_TRAILER_FORMAT_0)),
      .data = 1,
      .qos = (int32_t)_pthread_qos_class_encode(qos[ENV_QOS_QUEUE_OVERRIDE], 0,
                                                0)};

  struct kevent_qos_s kev_err = {0};

  /* Setup workloop for mach msg rcv */
  r = kevent_id(25, &kev, 1, &kev_err, 1, NULL, NULL,
                KEVENT_FLAG_WORKLOOP | KEVENT_FLAG_ERROR_EVENTS);

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(r, "kevent_id");
  T_QUIET;
  T_ASSERT_EQ(r, 0, "no errors returned from kevent_id");
  sleep(HELPER_TIMEOUT_SECS);
}
...
static void *special_reply_port_thread(void *ctxt) {
  kern_return_t ret;
...
#define TEST_QOS(server_name, client_name, name, wl_function_name, qos_bo,     \
                 qos_bo_name, qos_qo, qos_qo_name, qos_ao, qos_ao_name)        \
  T_DECL(server_kevent_id_##name,                                              \
    ...
    const char *qos_name_array[ENV_VAR_QOS] = {qos_bo_name, qos_qo_name,       \
                                               qos_ao_name};                   \
    run_client_server(server_name, client_name, qos_array, qos_name_array,     \
                      wl_function_name);                                       \
  }
/*
 ...
 */
TEST_QOS("server_kevent_id", "qos_get_special_reply_port", special_reply_port,
         "workloop_cb_test_intransit", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default")
 ...
 * sync ipc override.
 */
TEST_QOS("server_kevent_id", "qos_client_send_to_intransit_with_thr_pri",
         transit_IN, "workloop_cb_test_intransit", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_MAINTENANCE, "maintenance", QOS_CLASS_USER_INITIATED,
         "user initiated")
 ...
 */
TEST_QOS("server_kevent_id", "qos_client_send_to_intransit_with_thr_pri",
         transit_UI, "workloop_cb_test_intransit", QOS_CLASS_USER_INITIATED,
         "user initiated", QOS_CLASS_MAINTENANCE, "maintenance",
 ...
 */
TEST_QOS("server_kevent_id", "qos_client_send_sync_and_sync_rcv", rcv_IN,
         "workloop_cb_test_intransit", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_MAINTENANCE, "maintenance", QOS_CLASS_USER_INITIATED,
         "user initiated")
 ...
 * servicer of the workloop gets sync ipc override.
 */
TEST_QOS("server_kevent_id", "qos_client_send_sync_and_sync_rcv", rcv_UI,
         "workloop_cb_test_intransit", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_MAINTENANCE, "maintenance", QOS_CLASS_USER_INTERACTIVE,
         "user interactive with 47 basepri promotion")

/*
 * Test 6: test sending sync ipc message (at IN qos) to port will override the
 * servicer
 *
 ...
 */
TEST_QOS("server_kevent_id", "qos_client_send_sync_msg_with_pri", send_sync_IN,
         "workloop_cb_test_sync_send", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_MAINTENANCE, "maintenance", QOS_CLASS_USER_INITIATED,
         "user initiated")

/*
 * Test 7: test sending sync ipc message (at UI qos) to port will override the
 * servicer
 *
 * Send a message with sync ipc override to a port and check if the servicer
 * of the workloop on other side gets sync ipc override.
 */
TEST_QOS("server_kevent_id", "qos_client_send_sync_msg_with_pri", send_sync_UI,
         "workloop_cb_test_sync_send", QOS_CLASS_MAINTENANCE, "maintenance",
         QOS_CLASS_MAINTENANCE, "maintenance", QOS_CLASS_USER_INTERACTIVE,
         "user initiated with 47 basepri promotion")

/*
 * Test 8: test enabling a knote in workloop handler will drop the sync ipc
 * override of delivered message
 *
 ...
 */
TEST_QOS("server_kevent_id", "qos_client_send_sync_msg_with_pri",
         send_sync_UI_and_enable, "workloop_cb_test_sync_send_and_enable",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_USER_INTERACTIVE, "user initiated with 47 basepri promotion")

/*
 * Test 9: test returning to begin processing drops sync ipc override of
 * delivered message
 *
 * Send a sync ipc message and check if enabling the knote clears the override
 * of the delivered message, but should still have the override of an enqueued
 * message.
 */
TEST_QOS("server_kevent_id", "qos_client_send_two_sync_msg_with_thr_pri",
         send_two_sync_UI, "workloop_cb_test_send_two_sync",
         QOS_CLASS_BACKGROUND, "background", QOS_CLASS_MAINTENANCE,
         "maintenance", QOS_CLASS_USER_INTERACTIVE,
         "user initiated with 47 basepri promotion")
 ...
 */
TEST_QOS("server_kevent_id", "qos_client_send_two_msg_and_destroy",
         send_two_UI_and_destroy, "workloop_cb_test_two_send_and_destroy",
         QOS_CLASS_BACKGROUND, "background", QOS_CLASS_MAINTENANCE,
         "maintenance", QOS_CLASS_USER_INTERACTIVE,
         "user initiated with 47 basepri promotion")
 ...
 */
TEST_QOS("server_kevent_id", "qos_client_send_complex_msg_with_pri",
         send_complex_sync_UI_and_enable,
         "workloop_cb_test_sync_send_and_enable", QOS_CLASS_USER_INITIATED,
         "user initiated", QOS_CLASS_USER_INITIATED, "user initiated",
         QOS_CLASS_USER_INTERACTIVE, "user initiated with 47 basepri promotion")
 ...
 */
TEST_QOS("server_kevent_id", "qos_client_send_complex_msg_with_pri",
         send_complex_sync_UI_and_enable_and_handoff,
         "workloop_cb_test_sync_send_and_enable_handoff",
         QOS_CLASS_USER_INITIATED, "user initiated", QOS_CLASS_USER_INITIATED,
         "user initiated", QOS_CLASS_USER_INTERACTIVE,
         "user initiated with 47 basepri promotion")
 ...
 */
TEST_QOS("server_kevent_id", "qos_client_sync_rcv_qos_change", qos_change_to_IN,
         "workloop_cb_test_intransit", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_MAINTENANCE, "maintenance", QOS_CLASS_USER_INITIATED,
         "user initiated")
 ...
 * Test 14 - 21
 *
 * Test single sync ipc link with server that breaks/preserves the link in
 * different ways.
 */
TEST_QOS("server_kevent_id",
         "qos_client_send_sync_msg_with_link_check_correct_server",
         send_sync_link_correct_server_s, "workloop_cb_test_sync_send_reply",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default")

TEST_QOS("server_kevent_id",
         "qos_client_send_sync_msg_with_link_check_correct_server",
         send_sync_link_correct_server_d,
         "workloop_cb_test_sync_send_deallocate", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default")

TEST_QOS("server_kevent_id",
         "qos_client_send_sync_msg_with_link_check_correct_server",
         send_sync_link_correct_server_sk,
         "workloop_cb_test_sync_send_reply_kevent", QOS_CLASS_DEFAULT,
         "default", QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default")

TEST_QOS("server_kevent_id",
         "qos_client_send_sync_msg_with_link_check_correct_server",
         send_sync_link_correct_server_skp,
         "workloop_cb_test_sync_send_reply_kevent_pthread", QOS_CLASS_DEFAULT,
         "default", QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default")

TEST_QOS("server_kevent_id",
         "qos_client_send_sync_msg_with_link_check_incorrect_server",
         send_sync_link_incorrect_server_ks,
         "workloop_cb_test_sync_send_kevent_reply", QOS_CLASS_DEFAULT,
         "default", QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default")

TEST_QOS("server_kevent_id",
         "qos_client_send_sync_msg_with_link_check_correct_server",
         send_sync_link_correct_server_n,
         "workloop_cb_test_sync_send_do_nothing", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default")

TEST_QOS("server_kevent_id",
         "qos_client_send_sync_msg_with_link_check_incorrect_server",
         send_sync_link_incorrect_server_kp,
         "workloop_cb_test_sync_send_do_nothing_kevent_pthread",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default")
...
TEST_QOS("server_kevent_id",
         "qos_client_send_sync_msg_with_link_check_correct_server",
         send_sync_link_correct_server_e,
         "workloop_cb_test_sync_send_do_nothing_exit", QOS_CLASS_DEFAULT,
         "default", QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default")
 ...
 */
TEST_QOS("server_kevent_id",
         "qos_client_send_2sync_msg_with_link_check_correct_server",
         send_2sync_link_correct_server_sksk,
         "workloop_cb_test_sync_send_reply_kevent_reply_kevent",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default")

TEST_QOS("server_kevent_id",
         "qos_client_send_2sync_msg_with_link_check_incorrect_server",
         send_2sync_link_incorrect_server_kssk,
         "workloop_cb_test_sync_send_kevent_reply_reply_kevent",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default")
...
TEST_QOS("server_kevent_id",
         "qos_client_send_2sync_msg_with_link_check_incorrect_server",
         send_2sync_link_incorrect_server_ksks,
         "workloop_cb_test_sync_send_kevent_reply_kevent_reply",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default")
...
TEST_QOS("server_kevent_id",
         "qos_client_send_2sync_msg_with_link_check_incorrect_server",
         send_2sync_link_incorrect_server_skks,
         "workloop_cb_test_sync_send_reply_kevent_kevent_reply",
         QOS_CLASS_DEFAULT, "default", QOS_CLASS_DEFAULT, "default",
         QOS_CLASS_DEFAULT, "default")

/*
 * Test 26: test sending sync ipc from kernel (at IN qos) to port will override
 * the servicer
 *
 * Do a kernel upcall at IN qos to a port and check if the servicer
 * of the workloop on other side gets sync ipc override.
 */
TEST_QOS("server_kevent_id", "qos_client_send_kernel_upcall_sync_msg_with_pri",
         kernel_send_sync_IN, "workloop_cb_test_kernel_sync_send",
         QOS_CLASS_MAINTENANCE, "maintenance", QOS_CLASS_MAINTENANCE,
         "maintenance", QOS_CLASS_USER_INITIATED, "user initiated")

/*
 * Test 27: test sending async ipc from kernel (at IN qos and Tier 0 iotier) to
 * port will override the servicer
 *
 ...
 */
TEST_QOS("server_kevent_id", "qos_iotier_client_send_kernel_upcall_async_msg",
         kernel_send_async_IN, "workloop_cb_test_kernel_async_send",
         QOS_CLASS_MAINTENANCE, "maintenance", QOS_CLASS_MAINTENANCE,
         "maintenance", QOS_CLASS_UTILITY, "utility")
