...
static void _do_set_crash_behavior_test(char *child_mode, int signal,
                                        uint32_t flags, bool expect_panic) {
  ...
  int ret = sysctlbyname(SYSCTL_CRASH_BEHAVIOR_WOULD_PANIC, NULL, 0,
                         &would_panic, length);
  ...
  if (should_wait) {
    ...
    ret =
        posix_spawnattr_set_crash_behavior_deadline_np(&attrs, deadline, flags);
    T_ASSERT_POSIX_ZERO(
        ret, "posix_spawnattr_set_crash_behavior_deadline_np: %lld", deadline);
  }
  ...
  pid_t child_pid = 0;
  ret = posix_spawn(&child_pid, child_args[0], NULL, &attrs, &child_args[0],
                    environ);
  ...
  if (reason) {
    ret = terminate_with_reason(
        child_pid, OS_REASON_TEST, TEST_REASON_CODE, "Test forcing crash",
        OS_REASON_FLAG_CONSISTENT_FAILURE | OS_REASON_FLAG_NO_CRASH_REPORT);
    T_ASSERT_POSIX_SUCCESS(ret, "terminate_with_reason(%d)", child_pid);
  } else if (reason_signal) {
    ret = terminate_with_reason(
        child_pid, OS_REASON_SIGNAL, SIGABRT, "Test forcing crash with signal",
        OS_REASON_FLAG_CONSISTENT_FAILURE | OS_REASON_FLAG_NO_CRASH_REPORT);
  ...
  ret = sysctlbyname(SYSCTL_CRASH_BEHAVIOR_WOULD_PANIC, &would_panic, &length,
                     NULL, 0);
...
T_DECL(set_crash_behavior_panic_on_nonzero_with_crash,
       ...
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_PREFERRED) {
  _do_set_crash_behavior_test("crash", 0, POSIX_SPAWN_PANIC_ON_NON_ZERO_EXIT,
                              false);
}
...
T_DECL(set_crash_behavior_panic_on_nonzero_with_exit,
       ...
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_PREFERRED) {
  _do_set_crash_behavior_test("exit", 0, POSIX_SPAWN_PANIC_ON_NON_ZERO_EXIT,
                              true);
}
...
T_DECL(set_crash_behavior_panic_on_nonzero_with_success,
       ...
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_PREFERRED) {
  _do_set_crash_behavior_test("success", 0, POSIX_SPAWN_PANIC_ON_NON_ZERO_EXIT,
                              false);
}
...
T_DECL(set_crash_behavior_panic_on_crash_cancelled,
       ...
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_PREFERRED) {
  _do_set_crash_behavior_test("wait", SIGUSR1, POSIX_SPAWN_PANIC_ON_CRASH,
                              false);
}
...
T_DECL(set_crash_behavior_panic_on_crash_sigterm,
       ...
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_PREFERRED) {
  _do_set_crash_behavior_test("spin", SIGTERM, POSIX_SPAWN_PANIC_ON_CRASH,
                              false);
}
...
T_DECL(set_crash_behavior_panic_on_crash_sigkill,
       ...
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_PREFERRED) {
  _do_set_crash_behavior_test("spin", SIGKILL, POSIX_SPAWN_PANIC_ON_CRASH,
                              false);
}
...
T_DECL(set_crash_behavior_panic_on_crash_terminate_with_reason_signal,
       ...
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_PREFERRED) {
  _do_set_crash_behavior_test("reason_signal", 0, POSIX_SPAWN_PANIC_ON_CRASH,
                              true);
}
