 ...
 * If the amount exceeds 1/4 the sytem wide limit, then the corpse is
 * disallowed.
 *
 ...
 */
static void set_memorystatus_vm_map_fork_pidwatch(pid_t pid) {
  uint64_t new_value = (uint64_t)pid;
  size_t new_len = sizeof(new_value);
  int err;

  err = sysctlbyname("kern.memorystatus_vm_map_fork_pidwatch", NULL, NULL,
                     &new_value, new_len);
  T_QUIET;
...
static uint64_t get_memorystatus_vm_map_fork_pidwatch() {
  uint64_t value = 0;
  size_t val_len = sizeof(value);
  int err;

  err = sysctlbyname("kern.memorystatus_vm_map_fork_pidwatch", &value, &val_len,
                     NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      err,
...
/*
 * We want to avoid jetsam giving us bad results, if possible. So check if
 * there's enough memory for the test to run, waiting briefly for some to free
 * up.
 */
static void wait_for_free_mem(int need_mb) {
  int64_t memsize;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "sysctlbyname(hw.memsize...) failed");
  ...
  for (;;) {
    /*
     * memorystatus_level is a percentage of memory available. For example 20
     * means 1/5 of memory. It currently doesn't exist on macOS but neither does
     * jetsam, so pass the test there.
     */
    size = sizeof(memorystatus_level);
    if (sysctlbyname("kern.memorystatus_level", &memorystatus_level, &size,
                     NULL, 0) != 0) {
      return;
    }
    T_QUIET;
    T_ASSERT_LE(memorystatus_level, 100, "memorystatus_level too high");
    T_QUIET;
    T_ASSERT_GT(memorystatus_level, 0, "memorystatus_level negative");

    /*
     * jetsam kicks in at memory status level of 15%, so subtract that much out
     * of what's available.
     */
    ...
    T_LOG("Need %d MB, only %d MB available. sleeping 5 seconds for more to "
          "free. memorystatus_level %d",
          need_mb, (int)(avail / MEGABYTE), memorystatus_level);
    sleep(5);
  }
  T_SKIP("Needed %d MB, but only %d MB available. Skipping test to avoid "
         "jetsam issues.",
 ...
 */
static pid_t spawn_child_process(char *const executable, char *const memlimit,
                                 short flags, int priority, int active_limit_mb,
                                 int inactive_limit_mb) {
  posix_spawnattr_t spawn_attrs;
  int err;
  pid_t child_pid;
  char *const argv_child[] = {executable, "-n",     "child_process",
                              "--",       memlimit, NULL};

  err = posix_spawnattr_init(&spawn_attrs);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "  posix_spawnattr_init() failed");

  err = posix_spawnattr_setflags(&spawn_attrs, POSIX_SPAWN_START_SUSPENDED);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "  posix_spawnattr_setflags() failed");

  err = posix_spawnattr_setjetsam_ext(&spawn_attrs, flags, priority,
                                      active_limit_mb, inactive_limit_mb);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "  posix_spawnattr_setjetsam_ext() failed");

  err = posix_spawn(&child_pid, executable, NULL, &spawn_attrs, argv_child,
                    environ);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "  posix_spawn() failed");

  return child_pid;
}

/*
 * The parent calls this to continue the suspended child, then wait for its
 * result. We collect its resource usage to vefiry the expected amount
 * allocated.
 */
static void test_child_process(pid_t child_pid, int *status,
                               struct rusage *ru) {
  int err = 0;
  pid_t got_pid;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "  kill(%d, SIGCONT) failed", child_pid);

  T_LOG("  waiting for child[%d] to exit", child_pid);

  got_pid = wait4(child_pid, status, 0, ru);
  T_QUIET;
  T_ASSERT_EQ(child_pid, got_pid, "  wait4(%d, ...) returned %d", child_pid,
              got_pid);
}

/*
 * The child process executes this code. The easiest way, with given darwintest
 * infrastructure, it has to return information is via exit status.
 */
...
#define BYTESINEXCESS (2 * MEGABYTE) /* 2 MB - arbitrary */
  ...
  for (bytes_remaining = max_bytes_to_munch; bytes_remaining > 0;
       bytes_remaining -= bytes_this_munch) {
 ...
 * Actual test body.
 */
static void memorystatus_vm_map_fork_parent(int test_variant) {
  ...
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("hw.memsize", &hw_memsize, &size, NULL, 0),
      "read hw.memsize");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "_NSGetExecutablePath(%s, ...)", testpath);

  /*
   * We put the child process in FOREGROUND to try and keep jetsam's hands off
   * it.
   */
  child_pid = spawn_child_process(testpath, memlimit_str, flags,
                                  JETSAM_PRIORITY_FOREGROUND, active_limit_mb,
                                  inactive_limit_mb);
  ...
  if (!WIFEXITED(child_status)) {
    if (WIFSIGNALED(child_status)) {
      ...
      T_SKIP("Child terminated by signal %d test result invalid",
             WTERMSIG(child_status));
    }
    T_SKIP("child did not exit normally (status=%d) test result invalid",
           child_status);
  }
  ...
  T_QUIET;
  T_ASSERT_EQ(exit_val, NORMAL_EXIT, "child exit due to: %s",
              (0 < exit_val && exit_val < NUM_CHILD_EXIT)
                  ? child_exit_why[exit_val]
                  : "unknown");

  /*
   * If the kernel aborted generating a corpse for other reasons, the test is
   * invalid.
   */
  ...
  /*
   * We should always have made it through the vm_map_fork() checks in the
   * kernel for this test.
   */
  T_QUIET;
  T_ASSERT_NE_ULLONG(kernel_pidwatch_val, (uint64_t)child_pid,
                     "child didn't trigger corpse generation");

  T_EXPECT_EQ(kernel_pidwatch_val, expected_pidwatch_val,
              "kernel value 0x%llx - expected 0x%llx", kernel_pidwatch_val,
              expected_pidwatch_val);
}

/*
 * The order of these 2 test functions is important. They will be executed by
 * the test framwork in order.
 *
 * We test "not allowed first", then "allowed". If it were the other way around,
 * the corpse from the "allowed" test would likely cause memory pressure and
 * jetsam would likely kill the "not allowed" test.
 */
T_DECL(memorystatus_vm_map_fork_test_not_allowed,
       "test that corpse generation was not allowed", T_META_ASROOT(true),
       T_META_TAG_VM_PREFERRED, T_META_ENABLED(false /* rdar://133953771 */)) {
