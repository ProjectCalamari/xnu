...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vfs"), T_META_CHECK_LEAKS(false),
...
T_DECL(rename_excl_with_case_variant,
  ...
  T_ASSERT_POSIX_SUCCESS(mkdir(TEST_DIR, 0777), "Setup: creating test dir: %s",
                         TEST_DIR);
  ...
  T_ASSERT_POSIX_SUCCESS(close(fd), "Closing test file1: %s", TEST_FILE1);

  T_WITH_ERRNO;
  fd = open(TEST_FILE2, O_CREAT | O_RDWR, 0666);
  T_ASSERT_TRUE(fd != -1, "Creating test file2: %s", TEST_FILE2);

  T_ASSERT_POSIX_SUCCESS(close(fd), "Closing test file2: %s", TEST_FILE2);
  ...
  if (case_sensitive_vol) {
    ...
    T_ASSERT_POSIX_SUCCESS(
        renamex_np(TEST_FILE1, TEST_FILE1, RENAME_EXCL),
        "Renaming with RENAME_EXCL from source: %s to target: %s", TEST_FILE1,
        TEST_FILE1);
  }

  T_ASSERT_POSIX_SUCCESS(
      renamex_np(TEST_FILE1, TEST_FILE1_UC, RENAME_EXCL),
      "Renaming with RENAME_EXCL from source: %s to target: %s", TEST_FILE1,
      TEST_FILE1_UC);
}
