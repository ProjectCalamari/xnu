...
T_GLOBAL_META(T_META_NAMESPACE("xnu.net"), T_META_ASROOT(true),
              ...
              T_META_ENABLED(!TARGET_OS_BRIDGE), T_META_CHECK_LEAKS(false));

T_DECL(v4mappedv6_join_group,
       "Tests setting IPV6_JOIN_GROUP on an IPv4-mapped IPv6 address") {
  int s;
  ...
  T_ASSERT_POSIX_SUCCESS(
      inet_pton(AF_INET6, "::ffff:127.0.0.1", &sin6.sin6_addr), "inet_pton");
  T_ASSERT_POSIX_SUCCESS(
      connect(s, (const struct sockaddr *)&sin6, sizeof(sin6)), "connect");
  ...
  T_ASSERT_POSIX_FAILURE(
      ...
      EADDRNOTAVAIL, "setsockopt IPV6_JOIN_GROUP");
}
