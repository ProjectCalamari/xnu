...
extern int __microstackshot(char *tracebuf, uint32_t tracebuf_size,
                            uint32_t flags);

T_GLOBAL_META(
    T_META_NAMESPACE("xnu.scheduler"), T_META_RADAR_COMPONENT_NAME("xnu"),
    T_META_RADAR_COMPONENT_VERSION("scheduler"), T_META_OWNER("chimene"),
...
static void check_is_bg(bool wants_bg) {
  kern_return_t kr;
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "task_policy_get(TASK_POLICY_STATE)");
   ...
   */
  T_QUIET;
  T_ASSERT_BITS_SET(policy_state.flags, TASK_IMP_LIVE_DONOR,
                    "test should be live donor enabled");
   ...
   */
  if (wants_bg) {
    T_ASSERT_BITS_SET(policy_state.effective, POLICY_EFF_DARWIN_BG, "%d: is BG",
                      getpid());
  } else {
    T_ASSERT_BITS_NOTSET(policy_state.effective, POLICY_EFF_DARWIN_BG,
                         "%d: is not BG", getpid());
  }
   ...
   */
  if (wants_bg) {
    T_ASSERT_BITS_NOTSET(policy_state.flags, TASK_IMP_DONOR,
                         "%d: is not live donor", getpid());
  } else {
    T_ASSERT_BITS_SET(policy_state.flags, TASK_IMP_DONOR, "%d: is live donor",
                      getpid());
  }
}

static void check_runaway_mode(bool expected_mode) {
  int runaway_mode = getpriority(PRIO_DARWIN_RUNAWAY_MITIGATION, 0);

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(runaway_mode,
                         "getpriority(PRIO_DARWIN_RUNAWAY_MITIGATION)");

  T_LOG("pid %d: runaway mitigation mode is: %d", getpid(), runaway_mode);

  if (expected_mode) {
    T_QUIET;
    T_ASSERT_EQ(runaway_mode, PRIO_DARWIN_RUNAWAY_MITIGATION_ON,
                "should be on");
    check_is_bg(true);
  } else {
    T_QUIET;
    T_ASSERT_EQ(runaway_mode, PRIO_DARWIN_RUNAWAY_MITIGATION_OFF,
                "should be off");
    check_is_bg(false);
  }
}

T_DECL(entitled_runaway_mode,
       "runaway mitigation mode should be settable while entitled") {
  T_LOG("uid: %d", getuid());

  check_runaway_mode(false);

  T_ASSERT_POSIX_SUCCESS(setpriority(PRIO_DARWIN_RUNAWAY_MITIGATION, 0,
                                     PRIO_DARWIN_RUNAWAY_MITIGATION_ON),
                         "setpriority(PRIO_DARWIN_RUNAWAY_MITIGATION, 0, "
                         "PRIO_DARWIN_RUNAWAY_MITIGATION_ON)");

  check_runaway_mode(true);

  T_ASSERT_POSIX_SUCCESS(setpriority(PRIO_DARWIN_RUNAWAY_MITIGATION, 0,
                                     PRIO_DARWIN_RUNAWAY_MITIGATION_OFF),
                         "setpriority(PRIO_DARWIN_RUNAWAY_MITIGATION, 0, "
                         "PRIO_DARWIN_RUNAWAY_MITIGATION_OFF)");
...
T_DECL(entitled_runaway_mode_read_root,
       "runaway mitigation mode should be readable as root",
       T_META_ASROOT(true)) {
  T_LOG("uid: %d", getuid());
...
T_DECL(entitled_runaway_mode_read_notroot,
       "runaway mitigation mode should be readable as not root but entitled",
       T_META_ASROOT(false)) {
  T_LOG("uid: %d", getuid());
...
T_DECL(runaway_mode_child_exit,
       "runaway mitigation mode should disappear when child exits") {
  T_LOG("uid: %d", getuid());
  ...
  if (child_pid == 0) {
    ...
    T_QUIET;
    T_ASSERT_EQ(exit_status, 0, "dt_waitpid: exit_status");
    T_QUIET;
    T_ASSERT_EQ(signum, 0, "dt_waitpid: signum");
  }

  check_runaway_mode(false);
}

T_DECL(runaway_mode_child_set,
       "runaway mitigation mode should be settable on child pid") {
  T_LOG("uid: %d", getuid());
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(pipe(fd), "pipe()");
  ...
  if (child_pid == 0) {
    ...
    T_ASSERT_POSIX_SUCCESS(read(fd[0], buf, sizeof(buf)),
    ...
    T_ASSERT_POSIX_SUCCESS(setpriority(PRIO_DARWIN_RUNAWAY_MITIGATION,
                                       child_pid,
                                       PRIO_DARWIN_RUNAWAY_MITIGATION_ON),
                           "setpriority(PRIO_DARWIN_RUNAWAY_MITIGATION, "
                           "child_pid, PRIO_DARWIN_RUNAWAY_MITIGATION_ON)");
    ...
    T_QUIET;
    T_LOG("Signalling child to continue");
    T_ASSERT_POSIX_SUCCESS(close(fd[1]), "close(fd[1])");
 ...
 */
static size_t ledger_index_for_string(size_t *num_entries, char *string) {
  ...
  *num_entries = (size_t)li.li_entries;
  ...
  free(templateInfo);
  T_QUIET;
  T_ASSERT_TRUE(found, "found %s in ledger", string);
...
static void restore_sfi_state(void) {
  T_LOG("Restoring initial system SFI window %lld, "
        "SFI_CLASS_RUNAWAY_MITIGATION class offtime %lld",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(system_set_sfi_window(0), "system_set_sfi_window(0)");

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sfi_set_class_offtime(SFI_CLASS_RUNAWAY_MITIGATION,
                                               initial_class_offtime),
                         "system_set_sfi_window(%lld)", initial_class_offtime);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(system_set_sfi_window(initial_sfi_window),
...
static void *spin_thread(void *arg) {
  ...
  uint64_t duration =
      spin_seconds * NSEC_PER_SEC * timebase_info.denom / timebase_info.numer;
...
T_DECL(runaway_mode_child_sfi, "runaway mitigation mode should cause SFI") {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(system_get_sfi_window(&initial_sfi_window),
                         "system_get_sfi_window(&initial_sfi_window)");

  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sfi_get_class_offtime(SFI_CLASS_RUNAWAY_MITIGATION,
                                               &initial_class_offtime),
                         "sfi_get_class_offtime(SFI_CLASS_RUNAWAY_MITIGATION, "
                         "&initial_class_offtime)");

  T_LOG("Initial System SFI window %lld, SFI_CLASS_RUNAWAY_MITIGATION class "
        "offtime %lld\n",
        initial_sfi_window, initial_class_offtime);

  size_t num_ledger_entries = 0;
  size_t ledger_index = ledger_index_for_string(&num_ledger_entries,
                                                "SFI_CLASS_RUNAWAY_MITIGATION");
  uint64_t sfi_time_before =
      get_ledger_entry_for_pid(getpid(), ledger_index, num_ledger_entries);
  ...
  T_LOG("Setting custom system SFI window %lld, SFI_CLASS_RUNAWAY_MITIGATION "
        "class offtime %lld",
  ...
  T_ASSERT_POSIX_SUCCESS(
      sfi_set_class_offtime(SFI_CLASS_RUNAWAY_MITIGATION, custom_class_offtime),
      "sfi_set_class_offtime(SFI_CLASS_RUNAWAY_MITIGATION, %lld)",
      custom_class_offtime);
  ...
  uint64_t sfi_time_after =
      get_ledger_entry_for_pid(getpid(), ledger_index, num_ledger_entries);

  T_LOG("Ending accumulated SFI time: %lld\n", sfi_time_after);

  T_ASSERT_LT(sfi_time_before, sfi_time_after,
              "SFI_CLASS_RUNAWAY_MITIGATION SFI time must have increased");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(system_get_sfi_window(&final_sfi_window),
  ...
  /*
   * If the System SFI configuration was changed out from under us during the
   * test, either us or them will be confused.
   */
  T_QUIET;
  T_ASSERT_EQ(
      custom_sfi_window, final_sfi_window,
      "System SFI window should not unexpectedly change during the test");
  T_QUIET;
  T_ASSERT_EQ(
      custom_class_offtime, final_class_offtime,
      "System SFI offtime should not unexpectedly change during the test");
}
...
static bool search_for_self_microstackshot(bool log_details) {
  void *buf = calloc(microstackshot_buf_size, 1);
  T_QUIET;
  T_ASSERT_NOTNULL(buf, "allocate buffer");

  int ret = __microstackshot(buf, microstackshot_buf_size,
                             STACKSHOT_GET_MICROSTACKSHOT);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "microstackshot");

  if (!log_details) {
    T_QUIET;
  }
  T_EXPECT_EQ(*(uint32_t *)buf, (uint32_t)STACKSHOT_MICRO_SNAPSHOT_MAGIC,
              "magic value for microstackshot matches");

  uint32_t magic = STACKSHOT_TASK_SNAPSHOT_MAGIC;

  void *next_tsnap =
      memmem(buf, microstackshot_buf_size, &magic, sizeof(magic));

  void *buf_end = buf + microstackshot_buf_size;

  while (next_tsnap != NULL &&
         next_tsnap + sizeof(struct task_snapshot) < buf_end) {
    ...
    if (tsnap->pid == getpid()) {
      if (log_details) {
        T_LOG("%6d: found self snap: flags 0x%x 0x%llx\n", offset,
              tsnap->ss_flags, tsnap->disk_reads_count);
      }
      found_self = true;

      if (tsnap->disk_reads_count & kTaskRunawayMitigation) {
        T_LOG(
            "%6d: found runaway flag: pid %d, name %s, flags: 0x%x 0x%llx, \n",
            offset, tsnap->pid, (char *)&tsnap->p_comm, tsnap->ss_flags,
            tsnap->disk_reads_count);
        found_flag = true;
      }
    }
...
T_DECL(runaway_mode_microstackshot_flag,
  ...
  int ret = sysctlbyname("kern.microstackshot.pmi_sample_counter", &pmi_counter,
                         &sysctl_size, NULL, 0);
  if (ret == -1 && errno == ENOENT) {
    T_SKIP("no PMI support");
  } else {
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "query PMI counter");
  }
  uint64_t pmi_period;
  sysctl_size = sizeof(pmi_period);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sysctlbyname("kern.microstackshot.pmi_sample_period",
  ...
  T_EXPECT_POSIX_SUCCESS(setpriority(PRIO_DARWIN_RUNAWAY_MITIGATION, 0,
                                     PRIO_DARWIN_RUNAWAY_MITIGATION_ON),
                         "setpriority(PRIO_DARWIN_RUNAWAY_MITIGATION, 0, "
                         "PRIO_DARWIN_RUNAWAY_MITIGATION_ON)");
  ...
  T_LOG("Spinning for %d iterations or %lld*%d cycles or until self-sample is "
        "found\n",
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    T_QUIET;
    T_ASSERT_NE(ru.ri_cycles, (uint64_t)0,
