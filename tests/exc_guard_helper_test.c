...
T_GLOBAL_META(T_META_NAMESPACE("xnu"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("vm"),
              T_META_RUN_CONCURRENTLY(true), T_META_ALL_VALID_ARCHS(true));
 ...
 */
static bool is_hole(mach_vm_address_t query_start, mach_vm_size_t query_size) {
  ...
  kern_return_t kr =
      mach_vm_region_recurse(mach_task_self(), &entry_start, &entry_size,
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_region");
...
static void enable_exc_guard_of_type_and_verify(unsigned int guard_type) {
  struct {
    const char *name;
  ...
  T_QUIET;
  T_ASSERT_TRUE(guard_type < countof(guards) && guards[guard_type].name != NULL,
                "guard type in enable_exc_guard_of_type_and_verify");

  /* disable guard exceptions of this type, then verify that
   * enable_exc_guard_of_type enables them */

  kr = task_get_exc_guard_behavior(mach_task_self(), &disabling_behavior);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "get old behavior");
  disabling_behavior &= ~guards[guard_type].all_mask;
  kr = task_set_exc_guard_behavior(mach_task_self(), disabling_behavior);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "set empty behavior");

  old_behavior = enable_exc_guard_of_type(guard_type);
  T_QUIET;
  T_ASSERT_EQ(old_behavior, disabling_behavior,
              "enable_exc_guard_of_type return value");
  T_QUIET;
  T_ASSERT_FALSE(old_behavior & guards[guard_type].deliver_mask,
                 "%s guard exceptions must not be enabled",
                 guards[guard_type].name);

  kr = task_get_exc_guard_behavior(mach_task_self(), &new_behavior);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "get new behavior");
  T_ASSERT_TRUE(new_behavior & guards[guard_type].deliver_mask,
                "enable_exc_guard_of_type enabled %s guard exceptions",
                guards[guard_type].name);
  T_ASSERT_FALSE(
      new_behavior & guards[guard_type].fatal_mask,
      "enable_exc_guard_of_type set %s guard exceptions to non-fatal",
      guards[guard_type].name);
}

T_DECL(exc_guard_helper_test_vm,
       "test the test helper function block_raised_exc_guard_of_type with VM "
       "guard exceptions") {
...
#define LAST_RETRY 10
  for (int retry_count = 0; retry_count <= LAST_RETRY; retry_count++) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "allocate space");

    /* deallocate the page in the middle; no EXC_GUARD from successful
     * deallocation */
    if (block_raised_exc_guard_of_type(GUARD_TYPE_VIRT_MEMORY, &exc_info, ^{
          kern_return_t kr;
          kr = mach_vm_deallocate(mach_task_self(), allocated + PAGE_SIZE,
                                  PAGE_SIZE);
          T_QUIET;
          T_ASSERT_MACH_SUCCESS(kr, "create hole");
        })) {
      T_FAIL("unexpected guard exception");
    } else {
      T_ASSERT_EQ(exc_info.catch_count, 0,
                  "block_raised_exc_guard_of_type(VM) with no exceptions");
    }
    ...
    bool caught_exception =
        block_raised_exc_guard_of_type(GUARD_TYPE_VIRT_MEMORY, &exc_info, ^{
          ...
          /* deallocate page-hole-page; EXC_GUARD expected from deallocating a
           * hole */
          if (!is_hole(allocated + PAGE_SIZE, PAGE_SIZE)) {
            retry = true; /* somebody allocated inside our unallocated space;
                             retry */
            return;
          }
          kr = mach_vm_deallocate(mach_task_self(), allocated, PAGE_SIZE * 3);
          T_QUIET;
          T_ASSERT_MACH_SUCCESS(kr, "vm_deallocate a hole");

          /* deallocate again, now all holes; EXC_GUARD expected from
           * deallocating a hole */
          ...
          kr = mach_vm_deallocate(mach_task_self(), allocated, PAGE_SIZE * 3);
          T_QUIET;
          T_ASSERT_MACH_SUCCESS(kr, "vm_deallocate a hole again");
    ...
    if (retry) {
      if (retry_count < LAST_RETRY) {
        ...
        T_FAIL("intended unallocated space was repeatedly found to be "
               "allocated, giving up");
      }
    } else if (caught_exception) {
      /* caught an exception as expected: verify what we caught */
      T_ASSERT_EQ(exc_info.catch_count, 2,
                  "block_raised_exc_guard_of_type(VM) with 2 exceptions");
      T_ASSERT_EQ(exc_info.guard_type, GUARD_TYPE_VIRT_MEMORY,
                  "caught exception's type");
      T_ASSERT_EQ(exc_info.guard_flavor, kGUARD_EXC_DEALLOC_GAP,
                  "caught exception's flavor");
      T_ASSERT_EQ(exc_info.guard_payload, allocated + PAGE_SIZE,
                  "caught exception's payload");
      break; /* done retrying */
    } else {
      /* where's the beef? */
...
T_DECL(exc_guard_helper_test_mach_port,
       "test the test helper function block_raised_exc_guard_of_type with Mach "
       "port guard exceptions") {
  kern_return_t kr;
  exc_guard_helper_info_t exc_info;
  mach_port_t port;
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "new port");
  kr = mach_port_insert_right(mach_task_self(), port, port,
                              MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "make send");

  /* add and remove one send right, should succeed */
  if (block_raised_exc_guard_of_type(GUARD_TYPE_MACH_PORT, &exc_info, ^{
        kern_return_t kr;
        kr = mach_port_mod_refs(mach_task_self(), port, MACH_PORT_RIGHT_SEND,
                                +1);
        T_QUIET;
        T_ASSERT_MACH_SUCCESS(kr, "add one send right");
        kr = mach_port_mod_refs(mach_task_self(), port, MACH_PORT_RIGHT_SEND,
                                -1);
        T_QUIET;
        T_ASSERT_MACH_SUCCESS(kr, "remove one send right");
      })) {
    T_FAIL("unexpected guard exception");
  } else {
    T_ASSERT_EQ(exc_info.catch_count, 0,
                "block_raised_exc_guard_of_type(MACH_PORT) with no exceptions");
  }

  /* try to overflow the port's send right count, twice, and catch the
   * exceptions */
  bool caught_exception =
      block_raised_exc_guard_of_type(GUARD_TYPE_MACH_PORT, &exc_info, ^{
        kern_return_t kr;
        unsigned expected_error;
        if (process_is_translated()) {
          expected_error =
              0x1000013; /* KERN_UREFS_OVERFLOW plus another bit? */
        } else {
        ...
        kr = mach_port_mod_refs(mach_task_self(), port, MACH_PORT_RIGHT_SEND,
                                INT32_MAX);
        T_QUIET;
        T_ASSERT_MACH_ERROR(kr, expected_error, "add too many send rights");
        kr = mach_port_mod_refs(mach_task_self(), port, MACH_PORT_RIGHT_SEND,
                                INT32_MAX);
        T_QUIET;
        T_ASSERT_MACH_ERROR(kr, expected_error,
                            "add too many send rights, again");
      });
  if (caught_exception) {
    /* caught an exception as expected: verify what we caught */
    T_ASSERT_EQ(exc_info.catch_count, 2,
                "block_raised_exc_guard_of_type(MACH_PORT) with 2 exceptions");
    T_ASSERT_EQ(exc_info.guard_type, GUARD_TYPE_MACH_PORT,
                "caught exception's type");
    T_ASSERT_EQ(exc_info.guard_flavor, kGUARD_EXC_INVALID_VALUE,
                "caught exception's flavor");
