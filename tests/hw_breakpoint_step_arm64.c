...
#pragma unused(exception_port, thread, task, code, code_count)
  if (exception == EXC_BREAKPOINT ||
      (exception == EXC_CRASH &&
       atomic_load_explicit(&after_kill, memory_order_seq_cst))) {
...
#pragma unused(arg)
  ...
  kret = task_set_exception_ports(
      mach_task_self(), EXC_MASK_CRASH | EXC_MASK_BREAKPOINT, exception_port,
...
static void step_thread(mach_port_name_t task, thread_t thread) {
  kern_return_t kr;
  ...
  kr =
      thread_get_state(thread, ARM_DEBUG_STATE64, (thread_state_t)&dbg, &count);
  ...
  kr = task_resume(task);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "resume target task");

  long err = dispatch_semaphore_wait(sync_sema, SYNC_TIMEOUT);
  T_QUIET;
  T_ASSERT_EQ(err, 0L, "dispatch_semaphore_wait timeout");
}

T_DECL(hw_breakpoint_step,
       "Ensures that a process can be single-stepped using thread_set_state / "
       "ARM_DEBUG_STATE64",
       T_META_ASROOT(true), T_META_OWNER("Samuel Lepetit <slepetit@apple.com>"),
       T_META_TAG_VM_NOT_PREFERRED) {
  kern_return_t kr;
  pthread_t handle_thread;
  sync_sema = dispatch_semaphore_create(0);

  T_ASSERT_POSIX_ZERO(pthread_create(&handle_thread, NULL, exc_handler, NULL),
                      "pthread_create");
  long err = dispatch_semaphore_wait(sync_sema, SYNC_TIMEOUT);
  T_QUIET;
  T_ASSERT_EQ(err, 0L, "dispatch_semaphore_wait timeout");
  ...
  T_EXPECT_POSIX_ZERO(posix_spawn(&pid, args[0], NULL, NULL, args, NULL),
                      "posix_spawn helper");
  ...
  T_QUIET;
  T_ASSERT_TRUE(kr == KERN_SUCCESS, "task_suspend");
