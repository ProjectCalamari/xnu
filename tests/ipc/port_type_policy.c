...
T_GLOBAL_META(T_META_NAMESPACE("xnu.ipc"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("IPC"), T_META_TIMEOUT(10),
...
static void expect_sigkill(void (^fn)(void), const char *format_description,
                           ...) {
  ...
  if (pid == 0) {
    ...
    T_EXPECT_EQ(WTERMSIG(status), SIGKILL, "%s exited with %d, expect SIGKILL",
...
static kern_return_t send_msg(mach_port_t dest_port, mach_msg_header_t *msg,
...
static kern_return_t send_port_descriptor(mach_port_t dest_port,
                                          mach_port_t dsc_port, int disp) {
  ...
  size = (mach_msg_size_t)((char *)&complex_msg.trailer -
                           (char *)&complex_msg.base);
  return send_msg(dest_port, msg, size);
}

static mach_port_t recv_port_descriptor(mach_port_t dst_port) {
  struct msg_complex_port msg;

  kern_return_t kr = mach_msg2(&msg, MACH64_RCV_MSG, MACH_MSG_HEADER_EMPTY, 0,
                               sizeof(msg), dst_port, 0, 0);
...
static mach_port_t get_send_receive_right(void) {
  kern_return_t kr;
  mach_port_t port;
...
static kern_return_t send_ool_port_array(mach_port_t dest_port,
...
static kern_return_t send_ool_port_multiple_arrays(mach_port_t dest_port,
                                                   mach_msg_type_name_t disp) {
 ...
 */
static mach_port_t create_conn_with_port_array_port(void) {
  kern_return_t kr;
  mach_port_t port;
...
static mach_port_t create_exception_port(void) {
  kern_return_t kr;
  mach_port_t port;
...
static mach_port_t create_connection_port(void) {
  kern_return_t kr;
...
static mach_port_t create_reply_port(void) {
  kern_return_t kr;
  mach_port_t port;
...
static mach_port_t create_provisional_reply_port(void) {
  kern_return_t kr;
  mach_port_t port;
...
static mach_port_t create_service_port(void) {
  kern_return_t kr;
  mach_port_t port;
...
static void destruct_generic_port(mach_port_t port) {
  kern_return_t kr;
  ...
  kr = mach_port_destruct(mach_task_self(), port,
                          (type & MACH_PORT_TYPE_SEND) ? -1 : 0, 0);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_destruct");
}
/*
 * Helper functions and types to help making test output nice and readable.
 */
static const char *get_disp_name(mach_msg_type_name_t disp) {
  switch (disp) {
...
static const char *get_notification_name(mach_msg_id_t notification_id) {
 ...
 */
static void
test_disallowed_register_mach_notification(const port_type_desc *port_desc,
  ...
  expect_sigkill(
      ^{
        ...
        (void)mach_port_request_notification(
            mach_task_self(), port, notify_id, 0, notify_port,
            MACH_MSG_TYPE_MAKE_SEND_ONCE, &previous);

        /* Unreachable; ports will be destructed when IPC space is destroyed */
      },
      "%s failed with mach notification %s", port_desc->port_type_name,
 ...
 */
static void test_receive_immovability(const port_type_desc *port_desc) {
  expect_sigkill(
      ^{
...
static void test_send_immovability_move_so(const port_type_desc *port_desc) {
  expect_sigkill(
      ^{
        ...
        kr = mach_port_extract_right(mach_task_self(), port,
                                     MACH_MSG_TYPE_MAKE_SEND_ONCE, &so_right,
                                     &disp);

        T_ASSERT_MACH_SUCCESS(kr, "mach_port_extract_right with %s",
                              port_desc->port_type_name);

        (void)send_port_descriptor(dst_port, so_right,
                                   MACH_MSG_TYPE_MOVE_SEND_ONCE);

        /* Unreachable; ports will be destructed when IPC space is destroyed */
      },
      "%s immovable-send failed with MOVE_SEND_ONCE",
      port_desc->port_type_name);
}

static void test_send_immovability(const port_type_desc *port_desc) {
  expect_sigkill(
      ^{
        mach_msg_type_name_t disp;
      ...
      "%s immovable-send failed extract_right MOVE_SEND",
      port_desc->port_type_name);
...
static void test_ool_port_array(const port_type_desc *port_desc,
                                mach_msg_type_name_t disp) {
  expect_sigkill(
      ^{
      ...
      "sending OOL port array to %s with %s", port_desc->port_type_name,
      get_disp_name(disp));
}
 ...
 */
T_DECL(reply_port_policies, "Reply port policies tests") {
...
T_DECL(immovable_send_port_types, "Port types we expect to be immovable-send") {
  test_send_immovability(&CONNECTION_PORT_DESC);

  test_send_immovability(&SPECIAL_REPLY_PORT_DESC);
}

T_DECL(ool_port_array_policies, "OOL port array policies") {
#if TARGET_OS_VISION
   ...
   */
  test_ool_port_array(&IOT_PORT_DESC, MACH_MSG_TYPE_COPY_SEND);

  test_ool_port_array(&REPLY_PORT_DESC, MACH_MSG_TYPE_COPY_SEND);

  test_ool_port_array(&SPECIAL_REPLY_PORT_DESC, MACH_MSG_TYPE_COPY_SEND);

  test_ool_port_array(&CONNECTION_PORT_DESC, MACH_MSG_TYPE_COPY_SEND);

  test_ool_port_array(&EXCEPTION_PORT_DESC, MACH_MSG_TYPE_COPY_SEND);

  test_ool_port_array(&PROVISIONAL_REPLY_PORT_DESC, MACH_MSG_TYPE_COPY_SEND);

  test_ool_port_array(&TIMER_PORT_DESC, MACH_MSG_TYPE_COPY_SEND);
...
T_DECL(provisional_reply_port, "Provisional reply ports have no restrictions") {
  ...
  T_QUIET;
  T_ASSERT_EQ(prp, recv_port, "recv_port_descriptor send");
...
T_DECL(mktimer_traps, "Test mktimer traps") {
  kern_return_t kr;
  mach_port_t port;
  ...
  for (uint32_t i = 0; i < countof(PORT_TYPE_DESC_ARRAY); ++i) {
    ...
    T_QUIET;
    T_ASSERT_NE(port, MACH_PORT_NULL, "constructing a port type %s",
  ...
  T_QUIET;
  T_ASSERT_NE(port, MACH_PORT_NULL, "constructing a timer (%s)",
