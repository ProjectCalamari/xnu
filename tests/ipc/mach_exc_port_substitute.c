...
T_GLOBAL_META(T_META_NAMESPACE("xnu.ipc"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_RUN_CONCURRENTLY(TRUE), T_META_TAG_VM_NOT_PREFERRED);
...
kern_return_t catch_mach_exception_raise_state_identity(
    __unused mach_port_t exception_port, __unused mach_port_t thread,
    __unused mach_port_t task, __unused exception_type_t type,
    __unused exception_data_t codes, __unused mach_msg_type_number_t code_count,
    __unused int *flavor, __unused thread_state_t in_state,
...
kern_return_t catch_mach_exception_raise_state(
    __unused mach_port_t exception_port, __unused exception_type_t type,
    __unused exception_data_t codes, __unused mach_msg_type_number_t code_count,
    __unused int *flavor, __unused thread_state_t in_state,
    __unused mach_msg_type_number_t in_state_count,
    __unused thread_state_t out_state,
...
kern_return_t catch_mach_exception_raise(
    __unused mach_port_t exception_port, __unused mach_port_t thread,
    mach_port_t task, exception_type_t type, __unused exception_data_t codes,
  ...
  if (cur_test == 0) {
    ...
    kern_return_t kr = mach_port_kobject(mach_task_self(), task, &otype, &addr);
...
T_DECL(
    mach_exc_port_substitute,
    "test read port substition back to control port"
    ...
    T_META_BOOTARGS_SET("amfi_dev_mode_policy=1")) /* Turn off Developer Mode */
{
  ...
  kr = task_set_exception_ports(mach_task_self(), EXC_MASK_BREAKPOINT, exc_port,
...
T_DECL(
    mach_exc_port_substitute_oop,
    "test out of process exception with read port"
    " when dev mode is off",
    T_META_ENABLED(TARGET_OS_IOS), /* Just run on iOS. Some platforms do not
                                      have dev mode */
    T_META_IGNORECRASHES(".*mach_exc_port_substitute.*"),
    T_META_BOOTARGS_SET("amfi_dev_mode_policy=1")) /* Turn off Developer Mode */
{
  ...
  pid = fork();
  T_QUIET;
  T_EXPECT_NE(pid, -1, "fork() should not return -1");
