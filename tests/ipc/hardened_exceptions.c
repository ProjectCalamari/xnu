...
static void bad_access_func(void) {
  T_QUIET;
  T_LOG("Crashing");
  *(void *volatile *)5 = 0;
  T_QUIET;
...
static size_t exc_handler_state_identity_protected(
    task_id_token_t token, uint64_t thread_id, exception_type_t type,
    __unused exception_data_t codes, thread_state_t in_state,
...
#pragma unused(out_state_count)
  *out_state_count = in_state_count;
  ...
  func_pc += 4;
  uint64_t pc_discriminator = ptrauth_blend_discriminator(
      (void *)(unsigned long)signing_key, ptrauth_string_discriminator("pc"));
  func_pc = ptrauth_sign_unauthenticated(func_pc, ptrauth_key_function_pointer,
                                         pc_discriminator);
  ...
  /* Corrupting the thread state should not crash as only the PC is accepted in
   * hardened exceptions */
...
static void thread_register_handler(mach_port_t exc_port,
...
create_hardened_exception_port(const struct mach_exception_options meo,
  ...
  kr = task_register_hardened_exception_handler(
      current_task(), signing_key_local, meo.exceptions_allowed,
...
T_DECL(hardened_exceptions_default,
  ...
  run_exception_handler_behavior64(
      exc_port, NULL, (void *)exc_handler_state_identity_protected,
...
T_DECL(entitled_process_exceptions_disallowed,
       "Test that when you have the special entitlement you may not use the "
       "hardened exception flow, unless you have debugger entitlements",
  ...
  kern_return_t kr = thread_set_exception_ports(
      mach_thread_self(), EXC_MASK_ALL, MACH_PORT_NULL,
