...
T_GLOBAL_META(T_META_NAMESPACE("xnu.ipc"), T_META_RUN_CONCURRENTLY(TRUE),
              T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("IPC"), T_META_TAG_VM_PREFERRED);
...
static const rcv_configs_t rcv_configs[MACH_MSG2_TEST_COUNT] = {
    ...
    {.rcv_mode = MACH_MSG2,
     .rcv_options = MACH64_MSG_VECTOR,
     .rcv_size = 1,
     .expected_kr = MACH_RCV_TOO_LARGE},

    /* Test 4: SV -> CV */
    {.rcv_mode = MACH_MSG2,
     .rcv_options = MACH64_MSG_VECTOR,
     .rcv_size = 2,
     .expected_kr = MACH_MSG_SUCCESS, /* Also need to check expected_aux_size */
     .expected_aux_size = 0,
     .expected_aux = ""},
...
static void *_Nullable do_msg_rcv(void *_Nullable arg) {
  ...
  kr = mach_vm_allocate(mach_task_self(), &data_vec[0].msgv_data,
                        MAX_MESSAGE_SIZE, VM_FLAGS_ANYWHERE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_allocate should succeed");
  data_vec[0].msgv_rcv_size = MAX_MESSAGE_SIZE;

  kr = mach_vm_allocate(mach_task_self(), &data_vec[1].msgv_data,
                        MAX_BUFFER_SIZE, VM_FLAGS_ANYWHERE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_allocate should succeed");
  ...
  for (unsigned int i = 0; i < MACH_MSG2_TEST_COUNT; i++) {
    ...
    if (rcv_configs[i].rcv_mode == MACH_MSG2) {
      kr = mach_msg2((rcv_configs[i].rcv_options & MACH64_MSG_VECTOR)
                     ...
                     MACH_MSG_HEADER_EMPTY, 0, rcv_configs[i].rcv_size,
                     msg_rcv_port, 0, 0);
    } else {
      kr = mach_msg((void *)msg, MACH_RCV_MSG, 0, rcv_configs[i].rcv_size,
                    msg_rcv_port, 0, 0);
    }

    T_LOG("[Test %d] Received a message via mach_msg %d, verifying..", i,
          rcv_configs[i].rcv_mode);

    if (kr != rcv_configs[i].expected_kr) {
      T_FAIL("[Receive] Got unexpected kr %d for test case %d. \
                Expecting: %d",
             kr, i, rcv_configs[i].expected_kr);
    } else {
      if (kr == KERN_SUCCESS) {
        /* verify message proper carries correct data and port */

        T_QUIET;
        T_EXPECT_EQ(msg->data, (uint64_t)MESSAGE_DATA_BYTES,
                    "message should carry correct value");
        T_QUIET;
        T_EXPECT_EQ(msg->header.msgh_remote_port, send_port,
                    "port name should match");
        T_QUIET;
        T_EXPECT_EQ(msg->header.msgh_local_port, msg_rcv_port,
                    "port name should match");
        T_QUIET;
        T_EXPECT_EQ(msg->header.msgh_id, 4141, "ID should match");
        ...
        if (rcv_configs[i].rcv_mode == MACH_MSG2 &&
          ...
          mach_msg_size_t aux_size =
              ((aux_buffer_t *)data_vec[1].msgv_data)->header.msgdh_size;
          ...
          if (aux_size != expected) {
            ...
            if (aux_size > sizeof(mach_msg_aux_header_t)) {
              if (strcmp(content, rcv_configs[i].expected_aux)) {
                T_FAIL(
                    "[Receive] Got unexpected aux content %s for test case %d. \
                                    Expecting: %s",
                    content, i, rcv_configs[i].expected_aux);
              }
            }
          }
        }
...
static void send_msg(send_configs_t configs) {
  kern_return_t kr;
  ...
  for (int i = 0; i < configs.send_count; i++) {
    if (configs.send_mode == MACH_MSG2) {
      if (configs.send_options & MACH64_MSG_VECTOR) {
        ...
        if (configs.aux != NULL) {
          data_vecs[MACH_MSGV_IDX_AUX].msgv_data =
              (mach_vm_address_t)configs.aux;
          data_vecs[MACH_MSGV_IDX_AUX].msgv_send_size = configs.aux_size;
          data_vecs[MACH_MSGV_IDX_AUX].msgv_rcv_size = 0;
          data_count++;
        }

        kr = mach_msg2(data_vecs,
                       MACH64_SEND_MSG | MACH64_SEND_MQ_CALL |
                           configs.send_options,
                       *(configs.msg), data_count, 0, MACH_PORT_NULL, 0, 0);
      } else {
        T_QUIET;
        T_EXPECT_EQ(configs.aux, NULL,
                    "buffer must be NULL for non-vector send");
        kr = mach_msg2(
            configs.msg,
            MACH64_SEND_MSG | MACH64_SEND_MQ_CALL | configs.send_options,
            *(configs.msg), configs.msg_size, 0, MACH_PORT_NULL, 0, 0);
      }
    } else {
...
T_DECL(mach_msg2_interop, "Test mach_msg2 inter-operability") {
  ...
  kr =
      mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &send_port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "port allocation");
  T_LOG("Sending from port 0x%x", send_port);

  kr = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &rcv_port);
  T_LOG("Receiving from port 0x%x", rcv_port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "port allocation");
  kr = mach_port_insert_right(mach_task_self(), rcv_port, rcv_port,
                              MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "insert right");
...
T_DECL(mach_msg2_combined_send_rcv, "Test mach_msg2() combined send/rcv") {
  ...
  kr = mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &sr_port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "port allocation");
  T_LOG("Sending/Receiving from port 0x%x", sr_port);
  kr = mach_port_insert_right(mach_task_self(), sr_port, sr_port,
                              MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "insert right");

  buffer.msg.header.msgh_bits = MACH_MSGH_BITS_SET(
  ...
  aux.header.msgdh_size =
      sizeof(aux_buffer_t) + 0x10; /* set it to wrong size, ignored */
  memcpy(aux.string, buf_string, sizeof(aux.string));
  ...
  kr = mach_msg2(data_vec,
                 MACH64_SEND_MSG | MACH64_SEND_MQ_CALL | MACH64_RCV_MSG |
                     MACH64_MSG_VECTOR,
  ...
  /* Verify content */
  T_EXPECT_EQ(
      (unsigned long)((aux_buffer_t *)data_vec[1].msgv_data)->header.msgdh_size,
  ...
  T_EXPECT_EQ(((aux_buffer_t *)data_vec[1].msgv_data)->header.msgdh_size, 0,
              "Kernel should reset header to 0");
  T_EXPECT_EQ(rcv_buffer.msg.header.msgh_id, 4141,
              "msgh_id in rcv_buffer should match");
}

static void workloop_cb(uint64_t *workloop_id __unused, void **eventslist,
                        int *events __unused) {
  ...
  T_EXPECT_EQ((unsigned long)aux_size, sizeof(aux_buffer_t),
              "aux size should match");
  ...
  if (func != NULL) {
    ...
    T_ASSERT_POSIX_ZERO(
        _pthread_workqueue_init_with_workloop(
            NULL, NULL, (pthread_workqueue_function_workloop_t)func, 0, 0),
        NULL);
  }
...
T_DECL(mach_msg2_kevent_rcv, "Test mach_msg2() receive with kevent workloop") {
  ...
  /* Send the message */
  kr = mach_msg2(data_vec,
                 MACH64_SEND_MSG | MACH64_SEND_MQ_CALL | MACH64_MSG_VECTOR,
