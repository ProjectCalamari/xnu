...
static void expect_sigkill(void (^fn)(void), const char *description) {
  pid_t pid = fork();
  T_QUIET;
...
T_DECL(mach_port_construct_at_most_one,
  ...
  const uint32_t at_most_one_flags[] = {
      MPO_REPLY_PORT,     MPO_CONNECTION_PORT,
      MPO_SERVICE_PORT,   MPO_PROVISIONAL_REPLY_PORT,
      MPO_EXCEPTION_PORT, MPO_CONNECTION_PORT_WITH_PORT_ARRAY};

  for (uint32_t i = 0; i < countof(at_most_one_flags) - 1; ++i) {
    for (uint32_t j = i + 1; j < countof(at_most_one_flags); ++j) {
      mach_port_t port;

      mach_port_options_t opts = {.flags = at_most_one_flags[i] |
                                           at_most_one_flags[j]};

      kern_return_t kr =
          mach_port_construct(mach_task_self(), &opts, 0x0, &port);
      T_ASSERT_MACH_ERROR(kr, KERN_INVALID_ARGUMENT,
                          "mach_port_construct failed for at most one flags");
    }
  }
}
...
T_DECL(mach_port_construct_invalid_arguments_and_values,
  ...
  mach_port_options_t conn_opts = {.flags = MPO_CONNECTION_PORT,
  ...
  T_ASSERT_MACH_SUCCESS(
      kr, "MPO_CONNECTION_PORT succeeds with anonymous service name");
  ...
  mach_port_options_t qlimit_opts = {
      .flags = MPO_QLIMIT, .mpl.mpl_qlimit = MACH_PORT_QLIMIT_MAX + 1};

  kr = mach_port_construct(mach_task_self(), &qlimit_opts, 0x0, &port);
  T_ASSERT_MACH_ERROR(kr, KERN_INVALID_VALUE,
  ...
  for (uint32_t i = 0; i < sizeof(unknown_flags_opts.flags) * CHAR_BIT; ++i) {
    ...
    if (unknown_flags_opts.flags != 0) {
      kr = mach_port_construct(mach_task_self(), &unknown_flags_opts, 0x0,
                               &port);
      T_ASSERT_MACH_ERROR(
...
T_DECL(mach_port_construct_fatal_failure,
  ...
  expect_sigkill(
      ^{
        mach_port_t port;
        mach_port_options_t opts = {.flags =
...
T_DECL(mach_port_construct_kern_denied,
       "mach_port_construct kern defined failures", T_META_TAG_VM_PREFERRED,
       T_META_ENABLED(!TARGET_OS_OSX && !TARGET_OS_BRIDGE)) {
  kern_return_t kr;
  mach_port_t port;
  ...
  T_ASSERT_MACH_ERROR(kr, KERN_DENIED,
                      "MPO_TG_BLOCK_TRACKING failed with KERN_DENIED");
}
