...
T_GLOBAL_META(T_META_NAMESPACE("xnu.spawn"), T_META_RUN_CONCURRENTLY(TRUE),
              T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("spawn"), T_META_TAG_VM_PREFERRED);
...
T_DECL(test_platform_restrictions_entitlements,
       ...
       T_META_CHECK_LEAKS(false), T_META_TAG_VM_NOT_ELIGIBLE,
  ...
  T_EXPECT_EQ_UINT(vers, 2, "Platform restrictions1 bit should not be set");

  T_EXPECT_FALSE(conf->reserved, "reserved bit should not be set");
  T_EXPECT_FALSE(conf->tpro, "TPRO bit should not be set");
  ...
  T_EXPECT_TRUE(space_info.space_policy & 0x400,
                "enhanced V2 bit should be set");

  T_EXPECT_FALSE(space_info.space_policy & 0x100,
                 "enhanced V0 bit should not be set");
  T_EXPECT_FALSE(space_info.space_policy & 0x200,
                 "enhanced V1 bit should not be set");
}
