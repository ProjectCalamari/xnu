...
T_DECL(port_stash_turnstile,
       "stashing knote turnstile on port should take a +1",
...
#define KNOTE_PORT_COUNT 2
  ...
  mach_port_t sync_port = MACH_PORT_NULL,
              knote_port[KNOTE_PORT_COUNT] = {MACH_PORT_NULL};
  kr =
...
#pragma pack(0)
      ...
      .header =
          {
              .msgh_bits = MACH_MSGH_BITS_SET(
                  MACH_MSG_TYPE_MAKE_SEND, MACH_MSG_TYPE_MAKE_SEND_ONCE, 0, 0),
  ...
  kr =
      mach_msg(&(sync_link_msg.header), MACH_SEND_MSG | MACH_SEND_SYNC_OVERRIDE,
