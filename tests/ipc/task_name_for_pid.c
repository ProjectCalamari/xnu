...
T_DECL(task_name_for_pid_entitlement,
       "Test that task_name_for_pid suceeds with entitlement",
  ...
  T_SETUPEND;
  // launchd has root uid/gid so we know that we must be hitting the entitlement
  // check here.
  kr = task_name_for_pid(mach_task_self(), 1, &tname);
  T_ASSERT_MACH_SUCCESS(kr,
                        "task_name_for_pid should succeed on launchd (pid 1)");
