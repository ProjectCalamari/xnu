...
#pragma unused(exception_port, thread_id, task_id_token, exception, codes,     \
               codeCnt)
  T_FAIL("Triggered catch_mach_exception_raise_identity_protected() which "
         "shouldn't happen...");
  __builtin_unreachable();
}
...
kern_return_t catch_mach_exception_raise_state_identity(
    mach_port_t exception_port __unused, mach_port_t thread __unused,
    mach_port_t task __unused, exception_type_t type __unused,
    exception_data_t codes __unused, mach_msg_type_number_t code_count __unused,
  ...
  if (type == EXC_CRASH) {
    T_LOG(
        "Received a crash notification, signaling main thread and returning\n");
  ...
  *out_state_count = in_state_count; /* size of state object in 32-bit words */
  ...
  struct test_user_thread_state_64 *test_state =
      (struct test_user_thread_state_64 *)(void *)out_state;
  ...
  T_QUIET;
  T_ASSERT_NE(userland_diversifier, 0, "Userland diversifier is non zero");
  ...
  arm_thread_state64_set_lr_fptr(*state,
                                 arm_thread_state64_get_lr_fptr(*state));
  ...
  if (reset_diversifier == 0) {
    if (exception_count == 1) {
      ...
      test_state->__opaque_flags |=
          __TEST_USER_THREAD_STATE64_FLAGS_KERNEL_SIGNED_PC;
      T_LOG("Set the kernel signed flag on the thread state");
#else
      T_LOG("Not on arm64, Not doing anything");
#endif
    } else if (exception_count == 2) {
      T_LOG("Not clearing the kernel signed bit, this should be the last "
            "exception");
    } else {
    ...
    if (exception_count == 1) {
      ...
      arm_thread_state64_set_pc_fptr(*state, pc);
      T_LOG("Set the diversifier to zero and signed the pc, this should crash "
            "on return");
#else
      T_LOG("Not on arm64, Not doing anything");
#endif
...
static mach_port_t
create_exception_port_behavior64(exception_mask_t exception_mask,
                                 exception_behavior_t behavior) {
  mach_port_t exc_port = MACH_PORT_NULL;
  mach_port_t task = mach_task_self();
  kern_return_t kr = KERN_SUCCESS;

  if (behavior != EXCEPTION_STATE_IDENTITY &&
      behavior != EXCEPTION_IDENTITY_PROTECTED) {
    T_FAIL("Currently only EXCEPTION_STATE_IDENTITY and "
           "EXCEPTION_IDENTITY_PROTECTED are implemented");
  }
...
static void __unused run_exception_handler(mach_port_t exc_port) {
  ...
  int err = pthread_create(&exc_thread, (pthread_attr_t *)0, exc_server_thread,
                           (void *)(unsigned long long)exc_port);
  T_QUIET;
...
T_DECL(kernel_signed_pac_thread_state,
       "Test that kernel signed thread state given to exception ignores the pc",
  ...
  *(void *volatile *)0 = 0;
  ...
  if (exception_count != expected_exception) {
    T_FAIL("Expected %d exceptions, received %d", expected_exception,
           exception_count);
  } else {
    T_LOG("TEST PASSED");
  }
...
T_DECL(user_signed_pac_thread_state,
       "Test that user signed thread state given to exception works with "
       "correct diversifier",
       T_META_ENABLED(false && TARGET_CPU_ARM64E /* rdar://133955889 */)) {
  mach_port_t exc_port =
      create_exception_port(EXC_MASK_BAD_ACCESS | EXC_MASK_CRASH);
