...
T_GLOBAL_META(T_META_NAMESPACE("xnu.ipc"), T_META_CHECK_LEAKS(false),
              T_META_RUN_CONCURRENTLY(true), T_META_RADAR_COMPONENT_NAME("xnu"),
...
static mach_port_name_t t_port_construct_full(uint32_t mpo_flags,
...
static void t_port_destruct_full(mach_port_name_t *name, uint16_t srights,
...
static mach_port_name_t t_make_sonce(mach_port_name_t port) {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "make-send-once");
  T_QUIET;
  T_ASSERT_EQ(disp, MACH_MSG_TYPE_PORT_SEND_ONCE, "check make-sonce");

  return name;
}

static void t_deallocate_sonce(mach_port_name_t port) {
  kern_return_t kr;

  kr = mach_port_deallocate(mach_task_self(), port);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "dealloc-send-once");
}

static void t_vm_deallocate(void *addr, vm_size_t size) {
  kern_return_t kr;
...
static kern_return_t t_receive(mach_port_name_t name, mach_msg_header_t *msg,
                               mach_msg_size_t size, mach_msg_option64_t opts) {
  opts |= MACH64_RCV_GUARDED_DESC | MACH64_RCV_MSG;
  return mach_msg2(msg, opts, *msg, 0, size, name, 0, 0);
}

__attribute__((overloadable)) static kern_return_t
t_send(mach_port_name_t dest, mach_msg_header_t *msg, void *upto,
...
__attribute__((overloadable)) static kern_return_t
t_send(mach_port_name_t dest, mach_msg_base_t *base, void *upto,
       mach_msg_option64_t opts) {
...
static void t_fill_port(mach_port_name_t dest, size_t n) {
  for (size_t i = 0; i < n; i++) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "send to fill");
  }
}

static vm_size_t t_check_0xff(const void *addr, vm_size_t size) {
...
T_DECL(mach_msg_trailer, "check trailer generation") {
  ...
  kr = task_info(mach_task_self(), TASK_SECURITY_TOKEN, (task_info_t)&sec_token,
                 &count);
  T_ASSERT_MACH_SUCCESS(kr, "task_info(TASK_SECURITY_TOKEN)");

  count = TASK_AUDIT_TOKEN_COUNT;
  kr = task_info(mach_task_self(), TASK_AUDIT_TOKEN, (task_info_t)&audit_token,
                 &count);
  T_ASSERT_MACH_SUCCESS(kr, "task_info(TASK_AUDIT_TOKEN)");

  for (int i = 0; i <= MACH_RCV_TRAILER_LABELS; i++) {
    mach_msg_option64_t topts =
        (mach_msg_option64_t)MACH_RCV_TRAILER_ELEMENTS(i);
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "sending message with trailer %d", i);
    ...
    T_EXPECT_EQ(buf.trailer.msgh_trailer_type, MACH_MSG_TRAILER_FORMAT_0,
                "msgh_trailer_type");
    T_EXPECT_EQ(buf.trailer.msgh_trailer_size, tsize, "msgh_trailer_size");
    if (tsize > offsetof(mach_msg_max_trailer_t, msgh_sender)) {
      T_EXPECT_EQ(
          memcmp(&buf.trailer.msgh_sender, &sec_token, sizeof(sec_token)), 0,
          "msgh_sender");
    }
    if (tsize > offsetof(mach_msg_max_trailer_t, msgh_audit)) {
      T_EXPECT_EQ(
          memcmp(&buf.trailer.msgh_audit, &audit_token, sizeof(audit_token)), 0,
          "msgh_audit");
    }
    if (tsize > offsetof(mach_msg_max_trailer_t, msgh_context)) {
      T_EXPECT_EQ(buf.trailer.msgh_context, DEFAULT_CONTEXT, "msgh_context");
    }
    ...
    T_QUIET;
    T_EXPECT_EQ(
        t_check_0xff((char *)&buf.trailer + tsize,
                     sizeof(buf.trailer) + sizeof(buf.sentinel) - tsize),
        ~0ul, "should be unmodified");
  }
...
static void t_fill_complex_port_msg(struct msg_complex_port *msg,
                                    mach_msg_type_name_t disp,
                                    mach_port_name_t name) {
...
static void t_fill_complex_port_guarded_msg(
    struct msg_complex_guarded_port *msg, mach_msg_type_name_t disp,
    mach_port_name_t name, mach_msg_guard_flags_t flags) {
...
static void t_fill_complex_memory_msg(struct msg_complex_memory *msg,
                                      vm_address_t memory, mach_msg_size_t size,
                                      bool move) {
...
static void t_fill_complex_port_array_msg(struct msg_complex_port_array *msg,
                                          mach_msg_type_name_t disp,
...
static void t_mach_msg_descriptor_port(bool pseudo_receive) {
  ...
  for (size_t i = 0; i < port_dispositions[i]; i++) {
    ...
    if (pseudo_receive) {
      T_ASSERT_MACH_ERROR(kr, MACH_SEND_TIMED_OUT, "pseudo-rcv(disposition:%d)",
                          disp);
    } else {
    ...
    T_ASSERT_EQ((mach_msg_descriptor_type_t)msg.dsc.type,
                MACH_MSG_PORT_DESCRIPTOR, "verify type");
    T_ASSERT_EQ((mach_msg_type_name_t)msg.dsc.disposition, disp,
                "verify disposition");
    if (disp == MACH_MSG_TYPE_PORT_RECEIVE || disp == MACH_PORT_TYPE_SEND) {
...
static void t_mach_msg_descriptor_guarded_port(bool pseudo_receive) {
  mach_port_name_t rcv_name, port;
  kern_return_t kr;
  ...
  for (size_t i = 0; test_flags[i]; i++) {
    ...
    if (test_flags[i] & MACH_MSG_GUARD_FLAGS_UNGUARDED_ON_SEND) {
      ...
      port = t_port_construct_full(MPO_INSERT_SEND_RIGHT | MPO_CONTEXT_AS_GUARD,
                                   1);
    }

    t_fill_complex_port_guarded_msg(&msg, MACH_MSG_TYPE_MOVE_RECEIVE, port,
                                    test_flags[i]);
...
static void t_mach_msg_descriptor_port_array(bool pseudo_receive) {
  ...
  T_ASSERT_EQ(msg.base.body.msgh_descriptor_count, 1u, "verify dsc count");
  T_ASSERT_EQ((mach_msg_descriptor_type_t)msg.dsc.type,
              MACH_MSG_OOL_PORTS_DESCRIPTOR, "verify type");
  T_ASSERT_EQ((mach_msg_type_name_t)msg.dsc.disposition, disp,
              "verify disposition");
...
static void t_mach_msg_descriptor_memory(bool pseudo_receive) {
  mach_port_name_t rcv_name;
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_allocate(1M)");
  ...
  for (size_t n = 0; n < 2; n++) {
    ...
    T_ASSERT_EQ(t_check_0xff(msg.dsc.address, size), ~0ul, "check content");
