...
T_GLOBAL_META(T_META_NAMESPACE("xnu.mach.port_description"),
...
T_DECL(vm_named_entry,
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl(kern.development)");
  ...
  // Describe it
  kr =
      mach_port_kobject_description(mach_task_self(), named_entry, &object_type,
                                    &object_addr, object_description);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_kobject_description()");

  T_LOG("Object Type: %d", object_type);
  T_EXPECT_EQ(object_type, IKOT_NAMED_ENTRY,
              "object has type IKOT_NAMED_ENTRY");

  T_LOG("Object Address: %llu", object_addr);
  if (dev_kern) {
    T_EXPECT_NE(object_addr, 0ull,
                "object address is populated on development kernel");
  } else {
