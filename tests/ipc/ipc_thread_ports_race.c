...
T_GLOBAL_META(T_META_NAMESPACE("xnu.ipc"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("IPC"), T_META_CHECK_LEAKS(false),
              T_META_RUN_CONCURRENTLY(NO), T_META_TAG_VM_PREFERRED);
...
static void *thread_do_nothing(void *arg) { return arg; }

static void thread_creation_bomb_one(void *_ctx __unused, size_t _i __unused) {
  while (!done) {
    pthread_t th[BATCH];
    ...
    for (int i = 0; i < BATCH; i++) {
      ...
      T_QUIET;
      T_ASSERT_EQ(rc, 0, "pthread_create[%d]", i);
    }

    for (int i = 0; i < BATCH && th[i]; i++) {
      int rc = pthread_join(th[i], NULL);
      T_QUIET;
      T_ASSERT_EQ(rc, 0, "pthread_join[%d]", i);
    }
  }
}

static void *thread_creation_bomb(void *arg) {
  done = 0;
...
static void test_race(const char *how, task_t task, pid_t pid) {
...
static void local_test(bool control, task_t tp) {
  pthread_t th;
  int rc;

  rc = pthread_create(&th, NULL, thread_creation_bomb, NULL);
  T_QUIET;
  T_ASSERT_EQ(rc, 0, "started job");

  test_race(control ? "local(ctl)" : "local(read)", tp, 0);

  rc = pthread_join(th, NULL);
  T_QUIET;
  T_ASSERT_EQ(rc, 0, "done");
}

static void fork_child_test(bool control) {
  task_t tp;
  pid_t pid;
  ...
  if (control) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(rc, "task_read_for_pid");
  }
...
T_DECL(thred_ports_termination_race,
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "task_get_special_port(TASK_READ_PORT)");
  local_test(false, tp);
}
