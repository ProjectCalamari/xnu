...
T_DECL(immovable_rights, "Create a port with immovable receive rights") {
  ...
  mach_port_options_t opts = {.flags =
                                  MPO_CONTEXT_AS_GUARD | MPO_IMMOVABLE_RECEIVE};
  kern_return_t kr;
  ...
  kr = mach_port_get_attributes(mach_task_self(), imm_port,
                                ...
                                (mach_port_info_t)&status, &status_size);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_get_attributes");
  ...
  kr = mach_port_guard_with_flags(mach_task_self(), imm_port2, 0x11,
                                  (uint64_t)MPG_IMMOVABLE_RECEIVE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_port_guard_with_flags");
