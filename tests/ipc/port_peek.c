...
T_DECL(mach_port_peek, "Test mach port peeking") {
  ...
  T_QUIET;
  T_ASSERT_EQ(kr, KERN_SUCCESS, "mach_port_allocate");
  ...
  kr = mach_port_peek(mach_task_self(), port, tlrtype, &seqno, &incoming_size,
                      &incoming_id, (void *)&audit_trailer, &size);
...
#define USER_HEADER_SIZE_DELTA                                                 \
  8 /* sizeof(mach_msg_header_t) - sizeof(mach_msg_user_header_t) */
  T_ASSERT_EQ(incoming_size, send_size + USER_HEADER_SIZE_DELTA,
              "Peek must return correct msg size");

  kr = mach_msg(&msg.header, MACH_RCV_MSG, 0, sizeof(msg), port,
