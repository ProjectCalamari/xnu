...
T_DECL(v4_mapped_v6_ops, "v4 mapped v6 sock operations around bind/connect",
  ...
  T_ASSERT_EQ(inet_pton(AF_INET6, "::ffff:c000:201", &local.sin6_addr), 1,
              NULL);
  T_EXPECT_POSIX_FAILURE(
      (ret = bind(s6, (const struct sockaddr *)&local, sizeof(local))),
      EADDRNOTAVAIL, NULL);
  ...
  T_EXPECT_POSIX_SUCCESS(
      connect(s6, (struct sockaddr *)&remote, sizeof(remote)), NULL);
