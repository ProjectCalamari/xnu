...
/*
 * Returns true if the process is translated according to
 * sysctl.proc_translated. For example, Rosetta processes are translated
 * processes.
 */
...
#define XNU_T_META_REQUIRES_DEVELOPMENT_KERNEL                                 \
  T_META_REQUIRES_SYSCTL_EQ("kern.development", 1)
#define XNU_T_META_REQUIRES_RELEASE_KERNEL                                     \
  T_META_REQUIRES_SYSCTL_EQ("kern.development", 0)
