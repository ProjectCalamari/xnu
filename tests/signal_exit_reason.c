...
T_GLOBAL_META(T_META_NAMESPACE("xnu.misc"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("misc"),
              T_META_RUN_CONCURRENTLY(true));

static dispatch_queue_t exit_queue;

static void cleanup(void) { dispatch_release(exit_queue); }

static void dispatch_test(void (^test)(void)) {
...
static bool audit_token_for_pid(pid_t pid, audit_token_t *token) {
  kern_return_t err;
...
static void check_exit_reason(int pid, uint64_t expected_reason_namespace,
                              uint64_t expected_signal) {
  T_LOG("check_exit_reason %d", expected_signal);
  int ret, status;
  struct proc_exitreasonbasicinfo exit_reason;

  ret = proc_pidinfo(pid, PROC_PIDEXITREASONBASICINFO, 1, &exit_reason,
                     PROC_PIDEXITREASONBASICINFOSIZE);
  T_WITH_ERRNO;
  T_QUIET;
  T_ASSERT_EQ(ret, PROC_PIDEXITREASONBASICINFOSIZE,
              "retrieve basic exit reason info");

  waitpid(pid, &status, 0);
  T_QUIET;
  T_EXPECT_FALSE(WIFEXITED(status), "process did not exit normally");
  T_QUIET;
  T_EXPECT_TRUE(WIFSIGNALED(status),
                "process was terminated because of a signal");
  T_QUIET;
  T_EXPECT_EQ(WTERMSIG(status), expected_signal,
              "process should terminate due to signal %llu", expected_signal);

  T_EXPECT_EQ(exit_reason.beri_namespace, expected_reason_namespace,
              "expect OS_REASON_SIGNAL");
  T_EXPECT_EQ(exit_reason.beri_code, expected_signal,
              "expect reason code: %llu", expected_signal);
}

static void wait_collect_exit_reason(int pid, int signal) {
  dispatch_source_t ds_proc = dispatch_source_create(
      DISPATCH_SOURCE_TYPE_PROC, pid, DISPATCH_PROC_EXIT, exit_queue);
...
static void wait_with_timeout_expected(int pid, int seconds) {
  long timeout = 0;
  dispatch_source_t ds_proc = dispatch_source_create(
      DISPATCH_SOURCE_TYPE_PROC, pid, DISPATCH_PROC_EXIT, exit_queue);
  dispatch_semaphore_t sem = dispatch_semaphore_create(0);
  ...
  T_QUIET;
  T_EXPECT_TRUE(timeout != 0, "process exited and was not expected to");
  dispatch_release(ds_proc);
  dispatch_release(sem);
}

static void __test_exit_reason_abort() {
  pid_t child = fork();
  if (child > 0) {
...
static void __test_exit_reason_external_signal(int signal) {
...
static void __test_exit_reason_delegate_signal(int signal) {
  int ret = 0;
  ...
  if (child > 0) {
    ...
    pause();
    // This exit should not hit, but we exit abnormally in case something went
    // wrong
    _exit(-1);
  }
}

static void __test_exit_reason_delegate_terminate() {
  int ret = 0;
  audit_token_t instigator = INVALID_AUDIT_TOKEN_VALUE;
  audit_token_t token = INVALID_AUDIT_TOKEN_VALUE;
  pid_t child = fork();
  ...
  if (child > 0) {
    ...
    T_EXPECT_TRUE(sentsignal == SIGTERM || sentsignal == SIGKILL,
                  "sentsignal retval %d", sentsignal);
...
static void __test_exit_reason_terminate() {
  int ret = 0;
  pid_t child = fork();
  int sentsignal = 0;
  if (child > 0) {
...
static void __test_exit_reason_signal_with_audittoken(int signal) {
  int ret = 0;
  audit_token_t token = INVALID_AUDIT_TOKEN_VALUE;
  pid_t child = fork();
  if (child > 0) {
    ...
    wait_collect_exit_reason(child, signal);
    T_EXPECT_EQ_INT(ret, 0, "expect proc_signal_with_audittoken return: %d",
                    ret);
    // Send signal to the child with its audit token who has exited by now
    ret = proc_signal_with_audittoken(&token, signal);
    T_EXPECT_EQ_INT(ret, ESRCH, "expect no such process return: %d", ret);
...
__test_exit_reason_signal_with_audittoken_fail_bad_token(int signal) {
  ...
  if (child > 0) {
    audit_token_for_pid(child, &token);
    // Send signal to the child with its audit token, modified so pidversion is
    // bad
    token.val[7] += 1;
    ret = proc_signal_with_audittoken(&token, signal);
...
static void null_signal_handler(int sig) {}

static void __test_signal_zombie(void) {
  pid_t child;
  sigset_t set;
  sigset_t oldset;
  int sig = 0, ret = 0;
...
T_DECL(signal_zombie, "signaling a zombie should work",
       T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
...
T_DECL(proc_signal_delegate_success, "proc_signal_delegate should work",
       T_META_TAG_VM_PREFERRED, T_META_ENABLED(false) /* rdar://146369624 */) {
  dispatch_test(^{
...
T_DECL(proc_terminate_success, "proc_terminate should work",
       T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
...
T_DECL(proc_signal_with_audittoken_success,
       "proc_signal_with_audittoken should work", T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
...
T_DECL(proc_signal_with_audittoken_fail_bad_token,
       "proc_signal_with_audittoken should fail with invalid audit token",
       T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
...
T_DECL(proc_delegated_signal_fail_bad_instigator_token,
       "proc_signal_delegated should fail with invalid instigator audit token",
       T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
...
T_DECL(proc_signal_with_audittoken_fail_null_token,
       "proc_signal_with_audittoken should fail with a null audit token",
       T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
...
T_DECL(proc_signal_with_audittoken_fail_bad_signal,
       "proc_signal_with_audittoken should fail with invalid signals",
       T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
...
T_DECL(test_exit_reason_external_signal,
       "tests exit reason for external signals", T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
...
static void *_Nullable pthread_kill_helper(void *_Nullable msg) {
  struct pthread_kill_helper_args *args =
      (struct pthread_kill_helper_args *)msg;
  pthread_kill(*args->pthread, args->signal);
  return NULL;
}

static void __test_exit_reason_pthread_kill_self(int signal) {
  ...
  if (child > 0) {
    ...
    pthread_create(&t, NULL, (void *(*)(void *))pthread_kill_helper,
                   (void *)&args);
    pthread_join(t, NULL);
  }
}

T_DECL(test_exit_reason_pthread_kill_self,
       "tests exit reason for pthread_kill on caller thread",
       T_META_TAG_VM_PREFERRED) {
  dispatch_test(^{
