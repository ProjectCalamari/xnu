...
static void set_da_microstackshot_period(CFNumberRef num) {
...
static void disable_da_microstackshots(void) {
  int64_t zero = 0;
 ...
 */
static void reenable_da_microstackshots(void) {
 ...
 */
static void telemetry_cleanup(void) {
...
static void *thread_spin(__unused void *arg) {
  while (spinning) {
  }
  return NULL;
}

static bool query_pmi_params(unsigned int *pmi_counter, uint64_t *pmi_period) {
...
#define MS_RECORD MACHDBG_CODE(DBG_MACH_STACKSHOT, MICROSTACKSHOT_RECORD)
...
T_DECL(pmi_sampling, "attempt to configure microstackshots on PMI",
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(s, "session create");
  ...
  ktrace_events_single(s, MT_MICROSTACKSHOT,
  ...
  ktrace_events_single_paired(
      ...
      ^(struct trace_point *start, __unused struct trace_point *end) {
        ...
        const uint8_t any_record =
            kPMIRecord | kIORecord | kInterruptRecord | kTimerArmingRecord;
  ...
  T_ASSERT_POSIX_SUCCESS(
      ret, "telemetry syscall succeeded, started microstackshots");

  unsigned int pmi_counter = 0;
  uint64_t pmi_period = 0;
  bool pmi_support = query_pmi_params(&pmi_counter, &pmi_period);
  T_QUIET;
  T_ASSERT_TRUE(pmi_support, "PMI should be supported");
...
#endif // defined(__arm64__)
  T_QUIET;
  T_ASSERT_EQ(pmi_counter, instrs_counter, "PMI on instructions retired");
  T_QUIET;
  T_ASSERT_EQ(pmi_period, INSTRS_PERIOD, "PMI period is set");
...
T_DECL(error_handling,
  ...
  int ret =
      __telemetry(TELEMETRY_CMD_PMI_SETUP, TELEMETRY_PMI_INSTRS, 1, 0, 0, 0);
  T_EXPECT_EQ(ret, -1, "telemetry shouldn't allow PMI every instruction");

  ret = __telemetry(TELEMETRY_CMD_PMI_SETUP, TELEMETRY_PMI_INSTRS, 1000 * 1000,
                    0, 0, 0);
  T_EXPECT_EQ(ret, -1,
              "telemetry shouldn't allow PMI every million instructions");

  ret = __telemetry(TELEMETRY_CMD_PMI_SETUP, TELEMETRY_PMI_CYCLES, 1, 0, 0, 0);
  T_EXPECT_EQ(ret, -1, "telemetry shouldn't allow PMI every cycle");

  ret = __telemetry(TELEMETRY_CMD_PMI_SETUP, TELEMETRY_PMI_CYCLES, 1000 * 1000,
                    0, 0, 0);
  T_EXPECT_EQ(ret, -1, "telemetry shouldn't allow PMI every million cycles");

  ret = __telemetry(TELEMETRY_CMD_PMI_SETUP, TELEMETRY_PMI_CYCLES, UINT64_MAX,
                    0, 0, 0);
...
T_DECL(excessive_sampling,
  ...
  ktrace_events_single(s, START_EVENT, ^(struct trace_point *tp) {
    int error = ktrace_convert_timestamp_to_nanoseconds(s, tp->timestamp,
                                                        &first_timestamp_ns);
    T_QUIET;
  ...
  ktrace_set_completion_handler(s, ^{
    ...
    T_LOG("interrupt record rate: %gHz", interrupt_records / duration_secs);
    T_LOG("I/O record rate: %gHz", io_records / duration_secs);
    T_LOG("timer arm record rate: %gHz", timer_arm_records / duration_secs);
...
T_HELPER_DECL(read_kernel_microstackshots,
  ...
  if (argc < 1) {
    T_ASSERT_FAIL(
        "usage: microstackshot_tests -n read_kernel_microstackshots <file>");
  }
