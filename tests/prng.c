...
static void prng_sanitycheck(uint8_t *buf, size_t buf_size) {
  size_t nblocks = buf_size / BLOCK_SIZE;
  qsort(buf, nblocks, BLOCK_SIZE, cmp);

  for (size_t i = 0; i < nblocks - 1; i += 1) {
    T_QUIET;
    T_ASSERT_NE(memcmp(buf, buf + BLOCK_SIZE, BLOCK_SIZE), 0,
                "duplicate block");
    buf += BLOCK_SIZE;
  }
}

static void prng_getentropy(void *ctx, size_t i) {
...
static void prng_devrandom(void *ctx, size_t i) {
  uint8_t *buf = ((uint8_t *)ctx) + (BUF_SIZE * i);
...
T_DECL(prng_write_random, "Test writing to /dev/random") {
  ...
  T_ASSERT_EQ_INT((int)write(rndfd, entropy, 128), 128,
                  "write 128 bytes to /dev/random");
  T_ASSERT_EQ_INT((int)write(rndfd, entropy, 65), 65,
                  "write 65 bytes to /dev/random");
  T_ASSERT_EQ_INT((int)write(rndfd, entropy, 64), 64,
                  "write 64 bytes to /dev/random");
  T_ASSERT_EQ_INT((int)write(rndfd, entropy, 1), 1,
                  "write 1 byte to /dev/random");
