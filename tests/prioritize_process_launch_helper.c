...
mach_port_t receive(mach_port_t rcv_port, mach_port_t notify_port);
...
static void send(mach_port_t send_port, mach_port_t reply_port,
                 mach_port_t msg_port, mach_msg_option_t options,
  ...
  ret =
      mach_msg(&(send_msg.header),
               ...
               send_msg.header.msgh_size, 0, MACH_PORT_NULL, 10000, 0);
...
mach_port_t receive(mach_port_t rcv_port, mach_port_t notify_port) {
  ...
  ret = mach_msg(&(rcv_msg.header),
                 ...
                 0, rcv_msg.header.msgh_size, rcv_port, 0, notify_port);
...
int main(int argc __attribute__((unused)), char *argv[]) {
  ...
  if (strcmp(argv[1], "EXIT") == 0) {
    ...
    if (kr != KERN_SUCCESS) {
      printf(
          "mach_ports_lookup failed with return value %d and port count %d\n",
          kr, portCnt);
      exit(0);
    }
    ...
    send(send_port, special_reply_port, MACH_PORT_NULL,
         MACH_SEND_SYNC_BOOTSTRAP_CHECKIN, MACH_MSG_TYPE_COPY_SEND);
    sleep(2);
