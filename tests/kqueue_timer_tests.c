 ...
 */
static int do_simple_kevent(struct kevent64_s *kev, uint64_t expected) {
  int ret;
...
static void test_absolute_kevent(int time, int scale) {
  struct timeval tv;
  ...
  EV_SET64(&kev, 1, EVFILT_TIMER, EV_ADD, NOTE_ABSOLUTE | scale, deadline, 0, 0,
           0);
...
static void test_oneshot_kevent(int time, int scale) {
  int ret;
  uint64_t expected = 0;
  struct kevent64_s kev;
  ...
  EV_SET64(&kev, 2, EVFILT_TIMER, EV_ADD | EV_ONESHOT, scale, time, 0, 0, 0);
  ret = do_simple_kevent(&kev, expected);
...
static void test_interval_kevent(int usec) {
  struct kevent64_s kev;
  int ret;
  ...
  for (uint32_t i = 0; i < expected_pops; i++) {
    ...
    gettimeofday(&after, NULL);
    elapsed_usecs =
        (uint64_t)((after.tv_sec - before.tv_sec) * (int64_t)USEC_PER_SEC +
                   (after.tv_usec - before.tv_usec));
  ...
  if (pops > expected_pops + (expected_pops / 20) ||
    ...
    T_FAIL("%s() usec:%d (saw %lld of %lld expected pops)", __func__, usec,
           pops, expected_pops);
    failed++;
  } else {
...
static void test_repeating_kevent(int usec) {
  struct kevent64_s kev;
  int ret;
...
static void test_updated_kevent(int first, int second) {
  struct kevent64_s kev;
  int ret;
...
static void disable_timer_coalescing(void) {
  ...
  kr = task_policy_set(mach_task_self(), TASK_OVERRIDE_QOS_POLICY,
                       (task_policy_t)&qosinfo, TASK_QOS_POLICY_COUNT);
  if (kr != KERN_SUCCESS) {
    T_FAIL("task_policy_set(... TASK_OVERRIDE_QOS_POLICY ...) failed: %d (%s)",
