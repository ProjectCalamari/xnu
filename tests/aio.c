/* compile: xcrun -sdk macosx.internal clang -arch arm64e -arch x86_64
 * -ldarwintest -o test_aio aio.c */
...
T_GLOBAL_META(T_META_NAMESPACE("xnu.file_descriptors.aio"),
...
static void do_init(int num_files, bool enable_nocache) {
  ...
  for (i = 0; i < num_files; i++) {
    ...
    T_QUIET;
    T_ASSERT_NE(g_testfiles[i], NULL, "Allocate path buffer %d size %d", i,
                MAXPATHLEN);

    snprintf(g_testfiles[i], MAXPATHLEN, "%s/%s.%d", tmpdir, AIO_TESTFILE, i);
    ...
    T_QUIET;
    T_ASSERT_NE(g_buffers[i], NULL, "Allocate data buffer %d size %d", i,
                AIO_BUFFER_SIZE);
...
static struct aiocb *init_aiocb(int idx, off_t offset, int lio_opcode) {
  struct aiocb *aiocbp;
...
static int poll_aio_error(struct aiocb *aiocbp) {
  int err;
...
static int wait_for_kevent(int kq, struct kevent64_s *kevent) {
  struct timespec timeout = {.tv_sec = 10, .tv_nsec = 0};

  return kevent64(kq, NULL, 0, kevent, 1, 0, &timeout);
}

static int verify_buffer_data(struct aiocb *aiocbp, uint32_t pattern) {
  char *buf_to_verify;
  int err = 0;
 ...
 */
T_DECL(write_fsync,
       "Test aio_write() and aio_fsync(). Poll for AIO completion.") {
  struct aiocb *aiocbp;
  ssize_t retval;
  int err;
 ...
 */
T_DECL(write_suspend,
       "Test aio_write() and aio_suspend(). Suspend until AIO completion.") {
...
T_DECL(lio_listio_wait, "Test lio_listio() with LIO_WAIT.") {
  ...
  for (i = 0; i < AIO_LIST_MAX; i++) {
    ...
    retval = aio_return(aiocbp);
    T_ASSERT_EQ((int)retval, AIO_BUFFER_SIZE,
                "aio_return() for aiocbp(%d) %p bytes_written 0x%zx", i, aiocbp,
                retval);
  }
}
...
T_DECL(lio_listio_cancel, "Test lio_listio() and aio_cancel().") {
  ...
  for (i = 0; i < AIO_LIST_MAX; i++) {
    ...
    if (err == AIO_NOTCANCELED || err == AIO_ALLDONE) {
      ...
      retval = aio_return(aiocbp);
      T_ASSERT_EQ((int)retval, AIO_BUFFER_SIZE,
                  "aio_return() for aiocbp(%d) %p bytes_written 0x%zx", i,
                  aiocbp, retval);
    } else if (err == AIO_CANCELED) {
      T_WITH_ERRNO;
      retval = aio_return(aiocbp);
      T_ASSERT_EQ((int)retval, -1, "aio_return() for aiocbp(%d) %p", i, aiocbp);
    }
  }
}
 ...
 */
T_DECL(write_read_kevent, "Test aio_write() and aio_read(). Use kevent for AIO "
                          "completion and return status.") {
  struct aiocb *aiocbp;
  ...
  if (err > 0) {
    ...
    T_ASSERT_EQ(kevent.filter, EVFILT_AIO, "kevent.filter %d", kevent.filter);
