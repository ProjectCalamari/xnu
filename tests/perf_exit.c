...
T_GLOBAL_META(T_META_NAMESPACE("xnu.perf"), T_META_ASROOT(true),
              T_META_LTEPHASE(LTE_SINGLEUSER), T_META_TAG_PERF);
#if TARGET_OS_WATCH
...
static struct test_case test_cases[] = {{0, 0},
                                        {0, 10},
                                        {1000000, 0},
#if !TARGET_OS_WATCH
                                        {10000000, 0}
...
static dt_stat_time_t create_stat(int proc_wired_mem, int nthreads) {
...
T_DECL(exit, "exit(2) time from syscall start to end",
       T_META_TIMEOUT(TEST_TIMEOUT), T_META_TAG_VM_NOT_ELIGIBLE) {
  s = create_stat(test_cases[consumer_i].wired_mem,
                  test_cases[consumer_i].threads);
  ...
  ktrace_events_single(session, NEXT_CASE_EVENTID,
  ...
  ktrace_events_single(
      session, (BSDDBG_CODE(DBG_BSD_EXCP_SC, 1) | DBG_FUNC_START),
      ^(ktrace_event_t e) {
        T_QUIET;
        T_ASSERT_LE(e->pid, PID_MAX, "pid %d is valid in start tracepoint",
                    e->pid);
        begin_ts[e->pid] = e->timestamp;
      });

  ktrace_events_single(
      session, (BSDDBG_CODE(DBG_BSD_PROC, BSD_PROC_EXIT) | DBG_FUNC_END),
      ^(ktrace_event_t e) {
        T_ASSERT_LE(e->pid, PID_MAX, "pid %d is valid in end tracepoint",
                    e->pid);
        ...
        T_QUIET;
        T_ASSERT_LE(begin_ts[e->pid], e->timestamp,
        ...
        if (dt_stat_stable(s) && producer_i == consumer_i) {
          dispatch_sync(spawn_queue, ^(void) {
            producer_i++;
            T_ASSERT_POSIX_ZERO(
                kdebug_trace(NEXT_CASE_EVENTID, producer_i, 0, 0, 0),
                "kdebug_trace returns 0");
          });
        }
      });
  ...
  __block void (^spawn_process)(void) = Block_copy(^(void) {
    ...
    T_ASSERT_POSIX_ZERO(bret, "spawned process with pid %d (threads=%s mem=%s)",
                        pid, nthreads_buf, mem_buf);

    bret = waitpid(pid, &status, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(bret, "waited for process %d\n", pid);
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, TEST_TIMEOUT),
