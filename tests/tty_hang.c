 ...
 */
void handle_sigttin(int signal) { return; }
 ...
 * handling
 */
int get_new_session_and_terminal_and_fork_child_to_read(char *pty_name) {
  int sock_fd[2];
  int pty_fd;
  pid_t pid;
  char buf[10];
  ...
  if (pid == 0) { /* child */
    ...
    T_ASSERT_POSIX_SUCCESS(pty_fd_child = open(pty_name, O_RDWR), NULL);
 ...
 * the fork helper unhelpfully hides the pid of the child and in it kills
 * itself. We will instead fork first and wait on the child. If it is able to
 * emerge from the read of the terminal, the test passes and if it doesn't, the
 * test fails. Since the test is testing for a deadlock in proc_exit of the
 * child (caused by a background read in the "grandchild".
 */
void run_test(int do_revoke) {
  int master_fd;
  char *slave_pty;
  pid_t pid;
  ...
  slave_pty = NULL;
  T_ASSERT_POSIX_SUCCESS(master_fd = posix_openpt(O_RDWR | O_NOCTTY), NULL);
...
T_DECL(tty_exit_bgread_hang_test,
       "test for background read hang on ttys with proc exit",
...
/************************** TEST 2 ***********************************/
T_HELPER_DECL(create_new_session_and_revoke_terminal,
              "create_new_session_and_revoke_terminal") {
  run_test(1);
}

T_DECL(tty_revoke_bgread_hang_test,
       "test for background read hang on ttys with revoke",
       T_META_TAG_VM_PREFERRED) {
  dt_helper_t helpers[1];
