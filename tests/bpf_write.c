...
static void do_bpf_write(const char *ifname, u_int ip_len, bool expect_success,
                         u_int write_size_max) {
  int bpf_fd = -1;
  int bdlen = 0;
  u_int payload_len;
  ...
  T_LOG("frame_length %u ip_len %u payload_len %u", frame_length, ip_len,
        payload_len);

  T_ASSERT_GT((size_t)frame_length, (size_t)0,
              "frame_length must greater than zero");
  ...
  /*
   * Note: The following will fail if there is parasitic traffic and that should
   * not happen
   */
  ...
  T_ASSERT_EQ_LONG(nwritten, (long)hp->bh_caplen,
                   "bpf read same size as written");

  T_ASSERT_EQ_INT(bcmp(buffer + hp->bh_hdrlen, pkt, (size_t)nwritten), 0,
                  "bpf read same bytes as written");
...
static void test_bpf_write(u_int data_len, int mtu, bool expect_success,
                           u_int write_size_max) {
  init(mtu);
...
T_DECL(bpf_write_16370_mtu_9000_max_16370,
       "BPF write 16370 feth MTU 9000 max 16370", T_META_TAG_VM_PREFERRED) {
