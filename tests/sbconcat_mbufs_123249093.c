...
T_DECL(sbconcat_mbufs_123249093,
       "test sbconcat_mbufs() overflow radar://123249093") {
  int pair[2] = {-1, -1};
  union {
  ...
  T_ASSERT_POSIX_SUCCESS(socketpair(PF_LOCAL, SOCK_DGRAM, 0, pair),
                         "socketpair");
  T_LOG("pair[0]: %d pair[1]: %d", pair[0], pair[1]);

  T_ASSERT_POSIX_SUCCESS(bind(pair[0], (struct sockaddr *)sun, sun->sun_len),
                         "bind()");

  if (getsockname(pair[0], (struct sockaddr *)sun, &address_len) == 0) {
    T_LOG("getsockname(%d) OK, sun_len: %u sun_path: %s", pair[0], sun->sun_len,
          sun->sun_path);
  } else {
    T_LOG("getsockname(%d) error %s (%d)", pair[0], strerror(errno), errno);
  }
