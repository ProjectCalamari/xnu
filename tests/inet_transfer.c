...
#define set_error_string(__format, ...)                                        \
  do {                                                                         \
    snprintf(error_string, sizeof(error_string), __format, ##__VA_ARGS__);     \
  } while (0)

static void inet_sockaddr_init(inet_sockaddr_t addr, uint8_t af) {
...
static void inet_sockaddr_init_with_endpoint(inet_sockaddr_t addr,
...
static uint8_t inet_sockaddr_get_family(inet_sockaddr_t addr) {
  return addr->sa.sa_family;
}

static void inet_sockaddr_embed_scope(inet_sockaddr_t addr, int if_index) {
...
static bool inet_endpoint_is_valid(inet_endpoint_t node) {
  switch (node->af) {
  case AF_INET:
  case AF_INET6:
    break;
...
static uint8_t proto_get_socket_type(uint8_t proto) {
  return (proto == IPPROTO_UDP) ? SOCK_DGRAM : SOCK_STREAM;
}

static const char *af_get_string(uint8_t af) {
  return (af == AF_INET) ? "AF_INET" : "AF_INET6";
}

static const char *socket_type_get_string(uint8_t type) {
  return (type == SOCK_DGRAM) ? "SOCK_DGRAM" : "SOCK_STREAM";
}

static bool socket_bind_to_interface(const char *msg, int s, uint8_t af,
                                     int if_index) {
  int level;
  int opt;
  ...
  if (setsockopt(s, level, opt, &if_index, sizeof(if_index)) < 0) {
    set_error_string("%s: setsockopt(IP%s_BOUND_IF, %d) %s (%d)", msg,
                     (af == AF_INET) ? "" : "V6", if_index, strerror(errno),
                     errno);
    return false;
  }
  return true;
...
static bool socket_setsockopt_int(const char *msg, int s, int level, int opt,
                                  int val) {
  if (setsockopt(s, level, opt, &val, sizeof(val)) < 0) {
    set_error_string("%s: setsockopt(%d, %d, %d) %s (%d)", msg, level, opt, val,
...
static void inet_socket_init(inet_socket_t sock) {
...
static void inet_socket_close(inet_socket_t sock) {
  if (sock->server_fd >= 0) {
...
static bool inet_socket_init_server(inet_socket_t server,
                                    inet_endpoint_t endpoint,
                                    int server_if_index) {
  int s;
  ...
  if (s < 0) {
    set_error_string("%s: socket(%s, %s) failed %s (%d)", __func__,
                     af_get_string(endpoint->af),
                     socket_type_get_string(socket_type), strerror(errno),
                     errno);
    return false;
  }
  ...
  /* bind to interface */
  if (!socket_bind_to_interface(__func__, s, endpoint->af, server_if_index)) {
    return false;
  }

  /* bind to address */
  if (bind(s, &server->addr.sa, server->addr.sa.sa_len) < 0) {
    set_error_string("%s: bind(%s, %s, port=%d) failed %s (%d)", __func__,
                     af_get_string(endpoint->af),
                     socket_type_get_string(socket_type), endpoint->port,
                     strerror(errno), errno);
    goto failed;
  }
  ...
  if (endpoint->port == 0) {
    ...
    if (getsockname(s, &bound_sa.sa, &bound_sa_len) < 0) {
      set_error_string("%s: getsockname(%s, %s) %s (%d)", __func__,
                       af_get_string(endpoint->af),
                       socket_type_get_string(socket_type), strerror(errno),
                       errno);
      goto failed;
    ...
    if (endpoint->af == AF_INET) {
      port = server->addr.sin.sin_port = bound_sa.sin.sin_port;
    } else {
      port = server->addr.sin6.sin6_port = bound_sa.sin6.sin6_port;
    }
  ...
  if (endpoint->proto == IPPROTO_TCP && listen(s, 1) < 0) {
    set_error_string("%s: listen(%s, 1) failed %s (%d)", __func__,
                     af_get_string(endpoint->af), strerror(errno), errno);
    goto failed;
  }
  server->server_fd = s;
  return true;
...
static bool inet_socket_init_client(inet_socket_t client, int client_if_index,
                                    inet_endpoint_t endpoint) {
  int s;
  uint8_t socket_type = proto_get_socket_type(endpoint->proto);
  ...
  /* bind to interface */
  if (!socket_bind_to_interface(__func__, s, endpoint->af, client_if_index)) {
    goto failed;
  }
  client->sock_fd = s;
  return true;
...
static bool inet_socket_client_connect(inet_socket_t client,
                                       int client_if_index,
                                       inet_endpoint_t endpoint) {
  inet_sockaddr_init_with_endpoint(&client->addr, endpoint);
  inet_sockaddr_embed_scope(&client->addr, client_if_index);
  if (connect(client->sock_fd, &client->addr.sa, client->addr.sa.sa_len) < 0) {
    set_error_string("%s: connect failed %s (%d)", __func__, strerror(errno),
                     errno);
    return false;
  }
  return true;
...
static bool inet_socket_server_accept(inet_socket_t server) {
  ...
  if (new_fd < 0) {
    set_error_string("%s: accept failed %s (%d)", __func__, strerror(errno),
                     errno);
    return false;
  }
...
static void fill_with_random(uint8_t *buf, u_int len) {
  u_int i;
  u_int n;
  uint8_t *p;
  uint32_t random;
...
static bool wait_for_receive(int fd, bool *error) {
  fd_set readfds;
  int n;
  struct timeval tv;
  ...
  *error = false;
  ...
  if (n < 0) {
    *error = true;
    set_error_string("%s: select failed %s (%d)", __func__, strerror(errno),
                     errno);
  }
  return n > 0;
}

static bool send_receive(const char *msg, int send_fd, int recv_fd,
                         const uint8_t *data, uint16_t data_size, bool retry,
                         bool need_connect) {
  ssize_t n;
  uint8_t rbuf[2048];
  inet_sockaddr sa;
  ...
  do {
    ...
    if (n != data_size) {
      set_error_string("%s: %s %d bytes (actual %ld)"
                       " failed %s (%d)",
                       __func__, msg, data_size, n, strerror(errno), errno);
      return false;
    }
#define MAX_TRY 2
    if (retry && !wait_for_receive(recv_fd, &failed)) {
      ...
      try++;
      if (try == MAX_TRY) {
        set_error_string("%s: %s max retry", __func__, msg);
        return false;
      }
      continue;
    }
  ...
  while (total < data_size) {
    if (need_connect) {
      ...
      n = recvfrom(recv_fd, rbuf, sizeof(rbuf), 0, &sa.sa, &sa_len);
    } else {
  ...
  if (total != data_size) {
    set_error_string("%s: %s %d bytes (actual %ld)"
                     " failed %s (%d)",
                     __func__, msg, data_size, total, strerror(errno), errno);
    return false;
  }
  if (need_connect && connect(recv_fd, &sa.sa, sa_len) < 0) {
    set_error_string("%s: %s connect failed %s (%d)", __func__, msg,
                     strerror(errno), errno);
    return false;
  }
  return true;
...
static uint16_range data_sizes[] = {
    {1, 15},     {39, 60},     {79, 100},    {215, 236}, {485, 506},
    {985, 1006}, {1250, 1279}, {1461, 1500}, /* this will result in IP
                                                fragmentation */
    {0, 0}};

static bool inet_transfer_loop(inet_socket_t server, inet_socket_t client) {
  bool need_connect;
  uint8_t buf[2048];
  bool retry;
  uint16_range_t scan;
  ...
  for (scan = data_sizes; scan->start != 0; scan++) {
    for (uint16_t data_size = scan->start; data_size < scan->end; data_size++) {
      bool success;

      /* Client to Server */
      success = send_receive("client send", client->sock_fd, server_fd, buf,
                             data_size, retry, need_connect);
      if (!success) {
        return false;
      }
      ...
      /* Server to Client */
      success = send_receive("server send", server_fd, client->sock_fd, buf,
                             data_size, retry, false);
      if (!success) {
        return false;
      }
    }
...
bool inet_transfer_local(inet_endpoint_t server_endpoint, int server_if_index,
                         int client_if_index) {
  inet_socket client;
  inet_socket server;
  bool success = false;
  ...
  if (!inet_socket_init_server(&server, server_endpoint, server_if_index)) {
    goto failed;
  }
  if (!inet_socket_init_client(&client, client_if_index, server_endpoint)) {
    goto failed;
  }
  if (!inet_socket_client_connect(&client, client_if_index, server_endpoint)) {
    goto failed;
  }
  if (server.proto == IPPROTO_TCP && !inet_socket_server_accept(&server)) {
    goto failed;
  }
...
void inet_test_traffic(uint8_t af, inet_address_t server,
  ...
  for (uint8_t *proto = protos; *proto != 0; proto++) {
    ...
    T_LOG("%s: %s %s server %s client %s", __func__, af_get_str(af),
          ipproto_get_str(*proto), server_ifname, client_ifname);
    endpoint.proto = *proto;
    success = inet_transfer_local(&endpoint, server_if_index, client_if_index);
    T_ASSERT_TRUE(success, "inet_transfer_local(): %s",
                  inet_transfer_error_string());
  }
  return;
}

const char *inet_transfer_error_string(void) { return error_string; }
...
int main(int argc, char *argv[]) {
  ...
  while ((ch = getopt(argc, argv, "i:I:p:s:tu")) != EOF) {
    ...
    case 'p':
      if (endpoint.port != 0) {
        fprintf(stderr, "port specified multiple times\n");
        usage(progname);
      }
    ...
    case 's':
      ...
      if (inet_pton(AF_INET, optarg, &endpoint.addr) == 1) {
        endpoint.af = AF_INET;
      } else if (inet_pton(AF_INET6, optarg, &endpoint.addr) == 1) {
    ...
    case 't':
      if (endpoint.proto != 0) {
        fprintf(stderr, "protocol specified multiple times\n");
        usage(progname);
      }
