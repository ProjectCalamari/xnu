...
T_DECL(settimeofday_29192647,
       "Verify that the syscall settimeofday is effective", T_META_ASROOT(true),
       T_META_CHECK_LEAKS(NO), T_META_LTEPHASE(LTE_POSTINIT)) {
  struct timeval time;
  long new_time;
...
static void get_abs_to_us_scale_factor(uint64_t *numer, uint64_t *denom) {
  ...
  T_QUIET;
  T_ASSERT_EQ(mach_get_times(&old_abstime, NULL, &time), KERN_SUCCESS, NULL);

  old_time_usec =
      (uint64_t)time.tv_sec * USEC_PER_SEC + (uint64_t)time.tv_nsec / 1000;

  sleep(1);

  T_QUIET;
  T_ASSERT_EQ(mach_get_times(&new_abstime, NULL, &time), KERN_SUCCESS, NULL);
  ...
  T_EXPECT_LE_ULLONG(diff, (unsigned long long)ERROR,
                     "Check scale factor time base (%u/%u) delta read usec "
                     "%llu delta converted %llu delta abs %llu",
                     timebaseInfo.numer, timebaseInfo.denom, time_conv1,
                     new_time_usec, new_abstime);
...
T_DECL(adjtime_29192647, "Verify that the syscall adjtime is effective",
       T_META_CHECK_LEAKS(NO), T_META_LTEPHASE(LTE_POSTINIT),
       T_META_ASROOT(true)) {
  struct timespec time;
  struct timeval adj;
...
T_DECL(ntp_adjtime_29192647, "Verify that the syscall ntp_adjtime is effective",
       T_META_CHECK_LEAKS(NO), T_META_LTEPHASE(LTE_POSTINIT),
       T_META_ASROOT(true)) {
  struct timespec time;
  ...
  T_QUIET;
  T_ASSERT_EQ(mach_get_times(&abstime1, NULL, &time), KERN_SUCCESS, NULL);
  ...
  T_QUIET;
  T_ASSERT_EQ(mach_get_times(&abstime2, NULL, &time), KERN_SUCCESS, NULL);
