...
T_DECL(settime_32089962_entitled_root,
  ...
  if (geteuid() != 0) {
    T_SKIP(
        "settime_32089962_entitled_root test requires root privileges to run.");
  }

  /* test settimeofday */
  T_QUIET;
  T_ASSERT_POSIX_ZERO(gettimeofday(&settimeofdaytime, NULL), NULL);
...
T_DECL(settime_32089962_entitled_not_root,
       "Verify that the \"com.apple.settime\" entitlement can allow to change "
       "the time",
  ...
  if (geteuid() == 0) {
    T_SKIP("settime_32089962_entitled_root test requires no root privileges to "
           "run.");
  }
...
T_DECL(settimeofday_29193041_entitled_root,
  ...
  T_EXPECT_GE_LONG(time.tv_sec, new_time,
