...
T_GLOBAL_META(T_META_NAMESPACE("xnu.persona_adoption"),
              T_META_CHECK_LEAKS(false), T_META_RUN_CONCURRENTLY(true),
              T_META_ENABLED(!TARGET_OS_WATCH) // rdar://81809878
);

static uid_t _persona_create(int persona_type, uid_t persona_uid) {
  ...
  snprintf(pinfo.persona_name, MAXLOGNAME, "persona_adoption_test.%s",
           uuid_string);
...
static kern_return_t _persona_try_adopting(uid_t persona_id) {
  struct proc_uniqidentifierinfo uniqidinfo;
  int error = proc_pidinfo(getpid(), PROC_PIDUNIQIDENTIFIERINFO, 0, &uniqidinfo,
                           sizeof(uniqidinfo));
  ...
  mach_voucher_attr_raw_recipe_size_t recipe_size =
      sizeof(mach_voucher_attr_recipe_data_t) + recipe->content_size;
  mach_voucher_attr_raw_recipe_array_t recipes =
      (mach_voucher_attr_raw_recipe_array_t)&voucher_buf[0];
  mach_voucher_t mach_voucher = MACH_VOUCHER_NULL;
  kr = host_create_mach_voucher(mach_host_self(), recipes, recipe_size,
                                &mach_voucher);
  if (kr != 0) {
    return kr;
  }
  ...
  mach_voucher_attr_content_t content_out =
      (mach_voucher_attr_content_t)&voucher_persona;
  mach_voucher_attr_content_size_t content_out_size = sizeof(voucher_persona);
  kr = mach_voucher_attr_command(mach_voucher, MACH_VOUCHER_ATTR_KEY_BANK,
                                 BANK_PERSONA_ID, NULL, 0, content_out,
                                 &content_out_size);
  if (kr != 0) {
    return kr;
  }
...
static void _run_helper_in_persona_cleanup(void) {
...
_run_helper_in_persona(const char *helper_name, int persona_type) {
  ...
  dt_helper_t helper = dt_launchd_helper_plist(
      plist, helper_name, LAUNCH_SYSTEM_DOMAIN, NULL, NULL);

  dt_run_helpers(&helper, 1, 300);
}

T_HELPER_DECL(own_persona_can_be_adopted_impl,
              "own_persona_can_be_adopted helper spawned into persona type "
...
T_DECL(own_persona_can_be_adopted,
       "process spawned into a persona type that prohibits adoption can adopt "
       "own persona",
       T_META_TAG_VM_PREFERRED) {
