 ...
 * - any that are found to be writeable are tested for ability to store at least
 * a single bit
 ...
 * - test that registers that are allowed to be written per-thread are properly
 * context switched
 *
 * And that is why there is a whitelist of regs that we DO intend to be
 * writeable from userspace and we *KNOW* are properly context switched.
 * Registers we *INTEND* userspace to write are in that list. Things like
 * TPIDR_EL0, and others
 *
 */
...
#define DECLARE_TEST(_op0, _op1, _n, _m, _op2)                                 \
  ...
  msr_test_S##_op0##_##_op1##_C##_n##_C##_m##_##_op2(enum msr_op_reason why,   \
                                                     void *param) {            \
    switch (why) {                                                             \
    case msr_get_name:                                                         \
      *(const char **)param = "S" #_op0 "_" #_op1 "_C" #_n "_C" #_m "_" #_op2; \
      break;                                                                   \
                                                                               \
    case msr_read:                                                             \
      *(uint64_t *)param = __builtin_arm_rsr64("S" #_op0 "_" #_op1 "_C" #_n    \
                                               "_C" #_m "_" #_op2);            \
      break;                                                                   \
                                                                               \
    case msr_write:                                                            \
      __builtin_arm_wsr64("S" #_op0 "_" #_op1 "_C" #_n "_C" #_m "_" #_op2,     \
...
#undef DECLARE_TEST
#define DECLARE_TEST(_op0, _op1, _n, _m, _op2)                                 \
  msr_test_S##_op0##_##_op1##_C##_n##_C##_m##_##_op2,
static msr_op tests[] = {
#include "user_msrs.inc"
};
#undef DECLARE_TEST
...
static void msr_test(msr_op op) {
  ...
  if (!readable || !writeable) {
    T_LOG("\t RW needed for more testing. no further testing will be "
          "performed\n");
    return;
  }
  ...
  if (new_val == val) {
    T_LOG("\t reg seems to not take writes (0x%016llx). no further testing "
          "will be performed\n",
          val);
    return;
  }
  T_LOG("\twrote 0x%016llx, saw 0x%016llx\n", (unsigned long long)wrote_val,
  ...
  while (1) {
    ...
    if (time_passed.tv_sec) { // wait one second at most
      T_FAIL("\ttoo long for register to be cleared, last read value was "
             "0x%016llx, expected revert to 0x%016llx!",
             (unsigned long long)new_val, (unsigned long long)val);
      break;
    }
  }
  pthread_mutex_lock(&threads_please_die_lock);
...
static void sig_caught(int signo, siginfo_t *sinfop, void *ucontext) {
  ...
  if ((instr & 0xffd00000) != 0xd5100000) {
    T_ASSERT_FAIL("We did not expect SIGILL on an instr that is not an "
                  "MSR/MRS. [%p] = 0x%08x",
                  (void *)pc, instr);
  }
...
static bool is_release_kernel(void) {
  /*
