...
template <typename T>
static constexpr auto can_call_get_on_temporary(int)
    -> decltype(std::declval<test_shared_ptr<T>>().get(), bool()) {
  return true;
}

template <typename T>
static constexpr auto can_call_get_on_temporary(...) -> bool {
  return false;
}
