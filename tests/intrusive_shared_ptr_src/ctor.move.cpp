...
template <typename Stored, typename From, typename To> static void tests() {
  ...
  // Test with non-null pointers
  {
    ...
    libkern::intrusive_shared_ptr<From, test_policy> from(&obj,
                                                          libkern::retain);
    CHECK(test_policy::retain_count == 1);
    CHECK(from.get() == &obj);

    libkern::intrusive_shared_ptr<To, test_policy> to(
        std::move(from)); // explicit
    CHECK(test_policy::retain_count == 1);
    CHECK(to.get() == &obj);
...
T_DECL(ctor_move, "intrusive_shared_ptr.ctor.move", T_META_TAG_VM_PREFERRED) {
  ...
  static_assert(!std::is_constructible_v</*to*/ test_shared_ptr<Derived>,
                                         /*from*/ test_shared_ptr<Base> &&>);
  static_assert(
      !std::is_constructible_v</*to*/ test_shared_ptr<DerivedMultiple>,
                               /*from*/ test_shared_ptr<Base1> &&>);
  static_assert(
      !std::is_constructible_v</*to*/ test_shared_ptr<DerivedMultiple>,
                               /*from*/ test_shared_ptr<Base2> &&>);
  static_assert(!std::is_constructible_v</*to*/ test_shared_ptr<Base2>,
                                         /*from*/ test_shared_ptr<Base1> &&>);

  // Make sure const-casting away doesn't work
  static_assert(
      !std::is_constructible_v</*to*/ test_shared_ptr<Derived>,
                               /*from*/ test_shared_ptr<Derived const> &&>);

  // Make sure casting to unrelated types doesn't work
  static_assert(!std::is_constructible_v</*to*/ test_shared_ptr<char>,
                                         /*from*/ test_shared_ptr<Derived> &&>);
  static_assert(!std::is_constructible_v</*to*/ test_shared_ptr<Unrelated>,
                                         /*from*/ test_shared_ptr<Derived> &&>);
  static_assert(!std::is_constructible_v</*to*/ test_shared_ptr<Base2>,
                                         /*from*/ test_shared_ptr<Base1> &&>);
  ...
  static_assert(
      !std::is_constructible_v<
          /*to*/ libkern::intrusive_shared_ptr<Derived, dummy_policy<2>>,
          /*from*/ libkern::intrusive_shared_ptr<Derived, dummy_policy<1>> &&>);
}
