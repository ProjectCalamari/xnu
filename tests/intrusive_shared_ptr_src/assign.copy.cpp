...
// Tests for
//  template <typename U>
//  intrusive_shared_ptr& operator=(intrusive_shared_ptr<U, RefcountPolicy>
//  const& other);
//
...
T_DECL(assign_copy, "intrusive_shared_ptr.assign.copy",
  ...
  static_assert(!std::is_assignable_v</*to*/ test_shared_ptr<Derived>,
                                      /*from*/ test_shared_ptr<Base> const &>);
  static_assert(!std::is_assignable_v</*to*/ test_shared_ptr<DerivedMultiple>,
                                      /*from*/ test_shared_ptr<Base1> const &>);
  static_assert(!std::is_assignable_v</*to*/ test_shared_ptr<DerivedMultiple>,
                                      /*from*/ test_shared_ptr<Base2> const &>);
  static_assert(!std::is_assignable_v</*to*/ test_shared_ptr<Base2>,
                                      /*from*/ test_shared_ptr<Base1> const &>);

  // Make sure const-casting away doesn't work
  static_assert(
      !std::is_assignable_v</*to*/ test_shared_ptr<Derived>,
                            /*from*/ test_shared_ptr<Derived const> const &>);

  // Make sure casting to unrelated types doesn't work
  static_assert(
      !std::is_assignable_v</*to*/ test_shared_ptr<char>,
                            /*from*/ test_shared_ptr<Derived> const &>);
  static_assert(
      !std::is_assignable_v</*to*/ test_shared_ptr<Unrelated>,
                            /*from*/ test_shared_ptr<Derived> const &>);
  static_assert(!std::is_assignable_v</*to*/ test_shared_ptr<Base2>,
                                      /*from*/ test_shared_ptr<Base1> const &>);
  ...
  static_assert(
      !std::is_assignable_v<
          ...
          /*from*/ libkern::intrusive_shared_ptr<Derived, dummy_policy<1>> const
              &>);
}
