...
//  intrusive_shared_ptr<To, R> const_pointer_cast(intrusive_shared_ptr<From, R>
//  const& ptr) noexcept;
//
//  template<typename To, typename From, typename R>
//  intrusive_shared_ptr<To, R> const_pointer_cast(intrusive_shared_ptr<From,
//  R>&& ptr) noexcept
//
...
template <typename Stored, typename From, typename To> static void tests() {
  ...
  }
  {
    ...
    tracked_shared_ptr<To> to =
        libkern::const_pointer_cast<To>(std::move(from));
