 ...
 */
static attribute_highlights_t normal_highlights(void) {
...
bad_or_ignored_highlights(vm_entry_attribute_list_t bad_entry_attr,
 ...
 */
static attribute_highlights_t
normal_or_ignored_highlights(vm_entry_attribute_list_t normal_entry_attr,
 ...
 */
static const char *highlight_prefix(attribute_highlight_t highlight) {
 ...
 * It may include ANSI color codes.
 */
static const char *highlight_suffix(attribute_highlight_t highlight __unused) {
  if (use_colors()) {
    return ANSI_UNDIM;
  } else {
    return "";
...
#define HIGHLIGHT(value, attr_path)                                            \
  highlight_prefix(highlights.attr_path ? highlights.highlighting              \
                                        : IgnoredHighlight),                   \
      (value),                                                                 \
      highlight_suffix(highlights.attr_path ? highlights.highlighting          \
                                            : IgnoredHighlight)

/* host_priv port wrappers */

host_priv_t host_priv(void) {
  host_priv_t result;
...
static bool is_power_of_two(mach_vm_size_t n) {
  return n > 0 && (n & (n - 1)) == 0;
}

static bool is_valid_alignment_mask(mach_vm_size_t mask) {
  if (mask == 0) {
    return true;
  }
 ...
 */
static bool is_persistent_vm_behavior(vm_behavior_t behavior) {
  return behavior == VM_BEHAVIOR_DEFAULT || behavior == VM_BEHAVIOR_RANDOM ||
         behavior == VM_BEHAVIOR_SEQUENTIAL || behavior == VM_BEHAVIOR_RSEQNTL;
}

const char *name_for_entry_kind(vm_entry_template_kind_t kind) {
  static const char *kind_name[] = {"END_ENTRIES", "allocation", "hole",
                                    "submap parent"};
  assert(kind < countof(kind_name));
  return kind_name[kind];
}

const char *name_for_kr(kern_return_t kr) {
...
const char *name_for_prot(vm_prot_t prot) {
  assert(prot_contains_all(VM_PROT_ALL /* rwx */, prot));
  /* TODO: uexec? */
  static const char *prot_name[] = {"---", "r--", "-w-", "rw-",
                                    "--x", "r-x", "-wx", "rwx"};
  return prot_name[prot];
}

const char *name_for_inherit(vm_inherit_t inherit) {
...
const char *name_for_behavior(vm_behavior_t behavior) {
...
const char *name_for_share_mode(uint8_t share_mode) {
  assert(share_mode > 0);
...
const char *name_for_bool(boolean_t value) {
  switch (value) {
  case 0:
    return "false";
  case 1:
    return "true";
  default:
    return "true-but-not-1";
  }
}

void clamp_start_end_to_start_end(mach_vm_address_t *const inout_start,
...
void clamp_address_size_to_address_size(mach_vm_address_t *const inout_address,
...
void clamp_address_size_to_checker(mach_vm_address_t *const inout_address,
                                   mach_vm_size_t *const inout_size,
                                   vm_entry_checker_t *checker) {
  clamp_address_size_to_address_size(inout_address, inout_size,
                                     checker->address, checker->size);
}

void clamp_start_end_to_checker(mach_vm_address_t *const inout_start,
                                mach_vm_address_t *const inout_end,
                                vm_entry_checker_t *checker) {
  clamp_start_end_to_start_end(inout_start, inout_end, checker->address,
                               checker_end_address(checker));
}

uint64_t get_object_id_for_address(mach_vm_address_t address) {
...
uint16_t get_user_tag_for_address(mach_vm_address_t address) {
  mach_vm_address_t info_address = address;
  mach_vm_size_t info_size;
  vm_region_submap_info_data_64_t info;
...
uint16_t get_app_specific_user_tag_for_address(mach_vm_address_t address) {
...
static void set_vm_self_region_footprint(bool value) {
  int value_storage = value;
  int error = sysctlbyname("vm.self_region_footprint", NULL, NULL,
                           &value_storage, sizeof(value_storage));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(error, "sysctl(vm.self_region_footprint)");
}

bool __attribute__((overloadable)) get_info_for_address_fast(
    mach_vm_address_t *const inout_address, mach_vm_size_t *const out_size,
    vm_region_submap_info_data_64_t *const out_info, uint32_t submap_depth) {
  kern_return_t kr;
  ...
  kr = mach_vm_region_recurse(mach_task_self(), &actual_address, out_size,
                              &actual_depth, (vm_region_recurse_info_t)out_info,
                              &count);
...
bool __attribute__((overloadable)) get_info_for_address(
    mach_vm_address_t *const inout_address, mach_vm_size_t *const out_size,
    vm_region_submap_info_data_64_t *const out_info, uint32_t submap_depth) {
...
static bool is_mapped(mach_vm_address_t address, uint32_t submap_depth) {
  mach_vm_size_t size;
  vm_region_submap_info_data_64_t info;
  return get_info_for_address_fast(&address, &size, &info, submap_depth);
}

static void dump_region_info(mach_vm_address_t address, mach_vm_size_t size,
  ...
  T_LOG("%sMAPPING   0x%llx..0x%llx (size 0x%llx)%s", submap_prefix, address,
        end, size, suffix);
  T_LOG("%s    %sprotection:     %s%s", submap_prefix,
        HIGHLIGHT(name_for_prot(info->protection), entry.protection_attr));
  T_LOG("%s    %smax protection: %s%s", submap_prefix,
        HIGHLIGHT(name_for_prot(info->max_protection),
                  entry.max_protection_attr));
  T_LOG("%s    %sinheritance:    %s%s", submap_prefix,
        HIGHLIGHT(name_for_inherit(info->inheritance), entry.inheritance_attr));
  T_LOG("%s    %sbehavior:       %s%s", submap_prefix,
        HIGHLIGHT(name_for_behavior(info->behavior), entry.behavior_attr));
  T_LOG("%s    %suser wired count:  %d%s", submap_prefix,
        HIGHLIGHT(info->user_wired_count, entry.user_wired_count_attr));
  T_LOG("%s    %suser tag:       %d%s", submap_prefix,
        HIGHLIGHT(info->user_tag, entry.user_tag_attr));
  T_LOG("%s    %sobject offset:  0x%llx%s", submap_prefix,
        HIGHLIGHT(info->offset, entry.object_offset_attr));
  T_LOG("%s    %sobject id:      0x%llx%s", submap_prefix,
        HIGHLIGHT(info->object_id_full, object.object_id_attr));
  T_LOG("%s    %sref count:      %u%s", submap_prefix,
        HIGHLIGHT(info->ref_count, object.ref_count_attr));
  T_LOG("%s    %sshadow depth:   %hu%s", submap_prefix,
        HIGHLIGHT(info->shadow_depth, object.shadow_depth_attr));
  T_LOG("%s    %spages resident: %u%s", submap_prefix,
        HIGHLIGHT(info->pages_resident, entry.pages_resident_attr));
  T_LOG("%s    %spages shared now private: %u%s", submap_prefix,
        highlight_prefix(IgnoredHighlight), info->pages_shared_now_private,
        highlight_suffix(IgnoredHighlight));
  T_LOG("%s    %spages swapped out: %u%s", submap_prefix,
        highlight_prefix(IgnoredHighlight), info->pages_swapped_out,
        highlight_suffix(IgnoredHighlight));
  T_LOG("%s    %spages dirtied:  %u%s", submap_prefix,
        highlight_prefix(IgnoredHighlight), info->pages_dirtied,
        highlight_suffix(IgnoredHighlight));
  T_LOG("%s    %sexternal pager: %hhu%s", submap_prefix,
        highlight_prefix(IgnoredHighlight), info->external_pager,
        highlight_suffix(IgnoredHighlight));
  T_LOG(
      "%s    %sshare mode:     %s%s", submap_prefix,
      HIGHLIGHT(name_for_share_mode(info->share_mode), entry.share_mode_attr));
  T_LOG("%s    %sis submap:      %s%s", submap_prefix,
        HIGHLIGHT(name_for_bool(info->is_submap), entry.is_submap_attr));
  T_LOG("%s    %ssubmap depth:   %u%s", submap_prefix,
        HIGHLIGHT(submap_depth, entry.submap_depth_attr));
}

static void dump_hole_info(mach_vm_address_t address, mach_vm_size_t size,
                           uint32_t submap_depth,
                           attribute_highlights_t highlights) {
  mach_vm_address_t end = address + size;
  const char *submap_prefix = submap_depth > 0 ? SUBMAP_PREFIX : "";
  ...
  T_LOG("%sHOLE 0x%llx..0x%llx (size 0x%llx)%s", submap_prefix, address, end,
        size, suffix);
  if (submap_depth > 0) {
...
dump_region_info_in_range(mach_vm_address_t range_start,
                          mach_vm_size_t range_size, uint32_t submap_depth,
                          bool recurse, attribute_highlights_t highlights) {
...
static void dump_region_info_for_entry(vm_entry_checker_t *checker,
                                       attribute_highlights_t highlights) {
  /* Try to print at the checker's submap depth only. Don't recurse. */
  dump_region_info_in_range(checker->address, checker->size,
                            checker->submap_depth, false /* recurse */,
                            highlights);
}

void dump_region_info_for_entries(entry_checker_range_t list) {
  /*
  ...
  dump_region_info_in_range(start, end - start, 0 /* submap depth */,
                            true /* recurse */, normal_highlights());
}

/*
 * Count the number of templates in a END_ENTRIES-terminated list.
 */
static unsigned count_entry_templates(const vm_entry_template_t *templates) {
  if (templates == NULL) {
    return 0;
  }
 ...
 */
static unsigned count_object_templates(const vm_object_template_t *templates) {
  if (templates == NULL) {
    return 0;
  }
  for (unsigned count = 0;; count++) {
...
static vm_object_checker_t *object_checker_new(void) {
 ...
 */
static bool object_is_null(vm_object_checker_t *obj_checker) {
  if (obj_checker == NULL) {
    return true;
  }
 ...
 */
static void object_checker_reference(vm_object_checker_t *obj_checker) {
  if (!object_is_null(obj_checker)) {
...
static void
object_checker_deinit(vm_object_checker_t *obj_checker);     /* forward */
 ...
 */
static void object_checker_dereference(vm_object_checker_t *obj_checker) {
  if (!object_is_null(obj_checker)) {
...
static void object_checker_deinit(vm_object_checker_t *obj_checker) {
...
static void object_checker_free(vm_object_checker_t *obj_checker) {
  object_checker_deinit(obj_checker);
  free(obj_checker);
}

vm_object_checker_t *object_checker_clone(vm_object_checker_t *obj_checker) {
 ...
 */
static vm_object_checker_t *
make_anonymous_object_checker(checker_list_t *checker_list,
                              mach_vm_size_t size) {
  vm_object_checker_t *obj_checker = object_checker_new();
  obj_checker->kind = Anonymous;
  obj_checker->verify = vm_object_attributes_with_default(true);
...
static void
checker_list_move_objects_to_parent(checker_list_t *submap_list); /* forward */
 ...
 */
static vm_object_checker_t *
make_submap_object_checker(checker_list_t *checker_list,
                           checker_list_t *submap_checkers) {
  /* address range where the submap is currently mapped */
  mach_vm_address_t submap_start =
      checker_range_start_address(submap_checkers->entries);
  ...
  obj_checker->object_id =
      get_object_id_for_address(submap_start); /* submap_depth==0 */
  obj_checker->object_id_mode = object_has_known_id;
...
static vm_entry_checker_t *checker_new(void) {
  return calloc(sizeof(vm_entry_checker_t), 1);
}

static void checker_free(vm_entry_checker_t *checker) {
  object_checker_dereference(checker->object);
  free(checker);
}

static checker_list_t *checker_list_new(void) {
...
void checker_list_append_object(checker_list_t *list,
 ...
 */
static void checker_list_move_objects_to_parent(checker_list_t *submap_list) {
...
unsigned checker_range_count(entry_checker_range_t entry_range) {
  unsigned count = 0;
  FOREACH_CHECKER(checker, entry_range) { count++; }
  return count;
}
...
mach_vm_size_t checker_range_size(entry_checker_range_t checker_range) {
  return checker_range_end_address(checker_range) -
         checker_range_start_address(checker_range);
}

/*
 * Add a checker to the end of a checker range.
 */
static void checker_range_append(entry_checker_range_t *list,
 ...
 */
static void checker_range_free(entry_checker_range_t range) {
...
static void checker_list_free(checker_list_t *list) {
 ...
 */
static vm_entry_checker_t *checker_clone(vm_entry_checker_t *old) {
...
static void checker_set_pages_resident(vm_entry_checker_t *checker,
                                       mach_vm_size_t pages) {
 ...
 * Includes holes.
 */
static vm_entry_checker_t *checker_nth(vm_entry_checker_t *checkers,
                                       unsigned n) {
 ...
 * Includes holes.
 */
vm_entry_checker_t *checker_list_nth(checker_list_t *list, unsigned n) {
  return checker_nth(list->entries.head, n);
}

static void checker_list_apply_slide(checker_list_t *checker_list,
                                     mach_vm_address_t slide) {
  FOREACH_CHECKER(checker, checker_list->entries) { checker->address += slide; }
}
...
checker_get_and_slide_submap_checkers(vm_entry_checker_t *submap_parent) {
  ...
  submap_checkers->submap_slide =
      submap_parent->address - submap_parent->object_offset;
  checker_list_apply_slide(submap_checkers, submap_checkers->submap_slide);
  return submap_checkers;
}

void unslide_submap_checkers(checker_list_t *submap_checkers) {
  assert(submap_checkers->is_slid);
...
static void checker_tweak_for_vm_region(vm_entry_checker_t *checker,
                                        vm_entry_checker_t *submap_parent) {
  /* clamp checker bounds to the submap window */
  mach_vm_size_t old_size = checker->size;
  clamp_address_size_to_checker(&checker->address, &checker->size,
                                submap_parent);
...
submap_checkers_tweak_for_vm_region(checker_list_t *submap_checkers,
  ...
  submap_checkers->entries = checker_list_find_range_including_holes(
      submap_checkers, submap_parent->address, submap_parent->size);
...
static void cleanup_submap_checkers_untweak(checker_list_tweaks_t *tweaks) {
  ...
  *submap_checkers->entries.head = tweaks->new_head_original_contents;
   ...
   *
   * old_prefix->head..old_prefix->tail <-> head..tail <->
   * old_suffix->head..old_suffix->tail
   */
 ...
 * Adjusts the refcount of the new object checker and (if any) the old object
 * checker. Updates the entry's resident page count if the object has a fill
 * pattern.
 */
void checker_set_object(vm_entry_checker_t *checker,
                        vm_object_checker_t *obj_checker) {
...
void checker_make_shadow_object(checker_list_t *list,
                                vm_entry_checker_t *checker) {
 ...
 */
void checker_set_null_object(checker_list_t *list,
                             vm_entry_checker_t *checker) {
 ...
 * Mimic that here.
 */
uint8_t checker_share_mode(vm_entry_checker_t *checker) {
 ...
 * - TODO: SM_LARGE_PAGE
 */
static unsigned narrow_share_mode(unsigned share_mode) {
  switch (share_mode) {
  case SM_TRUESHARED:
    return SM_SHARED;
 ...
 * after accounting for share mode distinctions that the checkers do not
 * enforce.
 */
static bool same_share_mode(vm_region_submap_info_data_64_t *info,
                            vm_entry_checker_t *checker) {
 ...
 */
#define vm_entry_checker(...) checker_clone(&(vm_entry_checker_t){__VA_ARGS__})

/*
 * Allocate a new checker for an unallocated hole.
 * The new checker is not linked into the list.
 */
static vm_entry_checker_t *make_checker_for_hole(mach_vm_address_t address,
                                                 mach_vm_size_t size) {
  return vm_entry_checker(.address = address, .size = size, .kind = Hole,
                          .verify = vm_entry_attributes_with_default(true));
}

static vm_entry_checker_t *make_checker_for_anonymous_private(
    checker_list_t *list, vm_entry_template_kind_t kind,
    mach_vm_address_t address, mach_vm_size_t size, vm_prot_t protection,
    vm_prot_t max_protection, uint16_t user_tag, bool permanent) {
  ...
  vm_entry_checker_t *checker =
      vm_entry_checker(.kind = kind,
                       ...
                       .behavior = VM_BEHAVIOR_DEFAULT, .permanent = permanent,

                       .user_wired_count = 0, .user_tag = (uint8_t)user_tag,

                       .object_offset = 0, .pages_resident = 0,
                       .needs_copy = false,
...
vm_entry_checker_t *make_checker_for_vm_allocate(checker_list_t *list,
                                                 mach_vm_address_t address,
                                                 mach_vm_size_t size,
                                                 int flags_and_tag) {
  ...
  int ignored_flags = VM_FLAGS_FIXED | VM_FLAGS_ANYWHERE |
                      VM_FLAGS_RANDOM_ADDR | VM_FLAGS_OVERWRITE;
  ...
  return make_checker_for_anonymous_private(list, Allocation, address, size,
                                            VM_PROT_DEFAULT, VM_PROT_ALL, tag,
                                            permanent);
}
...
make_checker_for_shared(checker_list_t *list __unused,
                        vm_entry_template_kind_t kind,
                        mach_vm_address_t address, mach_vm_size_t size,
                        mach_vm_address_t object_offset, vm_prot_t protection,
                        vm_prot_t max_protection, uint16_t user_tag,
                        bool permanent, vm_object_checker_t *obj_checker) {
  // fixme hack: if you ask for protection --x you get r-x
  // fixme arm only?
  if (protection == VM_PROT_EXECUTE) {
    protection = VM_PROT_READ | VM_PROT_EXECUTE;
  ...
  vm_entry_checker_t *checker =
      vm_entry_checker(.kind = kind,
                       ...
                       .object_offset = object_offset, .pages_resident = 0,
                       .needs_copy = false,
 ...
 */
vm_entry_checker_t *
make_checker_for_submap(mach_vm_address_t address, mach_vm_size_t size,
                        mach_vm_address_t object_offset,
                        vm_object_checker_t *submap_object_checker) {
  vm_entry_checker_t *checker =
      vm_entry_checker(.kind = Submap, .address = address, .size = size,
                       .object = NULL, /* set below */
                       ...
                       .user_wired_count = 0, .user_tag = 0, .submap_depth = 0,
                       .object_offset = object_offset, .pages_resident = 0,
                       .needs_copy = false,
 ...
 */
static void
dump_checker_info_with_highlighting(vm_entry_checker_t *checker,
                                    attribute_highlights_t highlights) {
  ...
  T_LOG("%sCHECKER   %s0x%llx%s..%s0x%llx%s %s(size 0x%llx)%s (%s%s)",
        submap_prefix, HIGHLIGHT(checker->address, entry.address_attr),
  ...
  if (checker->kind == Hole) {
    if (checker->submap_depth != 0) {
      ...
      T_LOG("%s    %ssubmap_depth:   %u%s", submap_prefix,
            HIGHLIGHT(checker->submap_depth, entry.submap_depth_attr));
    }
    return;
  }

  T_LOG("%s    %sprotection:     %s%s", submap_prefix,
        HIGHLIGHT(name_for_prot(checker->protection), entry.protection_attr));
  T_LOG("%s    %smax protection: %s%s", submap_prefix,
        HIGHLIGHT(name_for_prot(checker->max_protection),
                  entry.max_protection_attr));
  T_LOG("%s    %sinheritance:    %s%s", submap_prefix,
        HIGHLIGHT(name_for_inherit(checker->inheritance),
                  entry.inheritance_attr));
  T_LOG("%s    %sbehavior:       %s%s", submap_prefix,
        HIGHLIGHT(name_for_behavior(checker->behavior), entry.behavior_attr));
  T_LOG("%s    %suser wired count:  %d%s", submap_prefix,
        HIGHLIGHT(checker->user_wired_count, entry.user_wired_count_attr));
  T_LOG("%s    %suser tag:       %d%s", submap_prefix,
        HIGHLIGHT(checker->user_tag, entry.user_tag_attr));
  T_LOG("%s    %sobject offset:  0x%llx%s", submap_prefix,
        HIGHLIGHT(checker->object_offset, entry.object_offset_attr));

  vm_object_checker_t *obj_checker = checker->object;
  if (object_is_null(obj_checker)) {
    T_LOG("%s    %sobject id:      %d%s", submap_prefix,
          HIGHLIGHT(0, entry.object_attr));
  } else if (obj_checker->object_id_mode == object_is_unknown) {
    T_LOG("%s    %sobject id:      %s%s", submap_prefix,
          HIGHLIGHT("unknown", entry.object_attr));
  } else if (obj_checker->object_id_mode == object_has_unknown_nonnull_id) {
    T_LOG("%s    %sobject id:      %s%s", submap_prefix,
          HIGHLIGHT("unknown, not null", entry.object_attr));
  } else {
    assert(obj_checker->object_id_mode == object_has_known_id);
    T_LOG("%s    %sobject id:      0x%llx%s", submap_prefix,
          HIGHLIGHT(obj_checker->object_id, object.object_id_attr));
    for (vm_object_checker_t *shadow = obj_checker->shadow; shadow;
         shadow = shadow->shadow) {
      T_LOG("%s        %sshadow:         0x%llx%s", submap_prefix,
            HIGHLIGHT(shadow->object_id, object.object_id_attr));
    }
    T_LOG("%s    %sobject size:    0x%llx%s", submap_prefix,
          HIGHLIGHT(obj_checker->size, object.size_attr));
    T_LOG("%s    %sref_count:      %u%s", submap_prefix,
          HIGHLIGHT(object_checker_get_vm_region_ref_count(obj_checker),
                    object.ref_count_attr));
    T_LOG("%s    %sshadow_depth:   %u%s", submap_prefix,
          HIGHLIGHT(object_checker_get_shadow_depth(obj_checker),
                    object.shadow_depth_attr));
    T_LOG("%s    %sself_ref_count: %u%s", submap_prefix,
          HIGHLIGHT(object_checker_get_self_ref_count(obj_checker),
                    object.ref_count_attr));
  }

  T_LOG("%s    %spages resident: %u%s", submap_prefix,
        HIGHLIGHT(checker->pages_resident, entry.pages_resident_attr));
  T_LOG("%s    %sshare mode:     %s%s", submap_prefix,
        HIGHLIGHT(name_for_share_mode(checker_share_mode(checker)),
                  entry.share_mode_attr));
  T_LOG("%s    %sis submap:      %s%s", submap_prefix,
        HIGHLIGHT(name_for_bool(checker_is_submap(checker)),
                  entry.is_submap_attr));
  T_LOG("%s    %ssubmap_depth:   %u%s", submap_prefix,
        HIGHLIGHT(checker->submap_depth, entry.submap_depth_attr));
  T_LOG("%s    %spermanent:      %s%s", submap_prefix,
        HIGHLIGHT(name_for_bool(checker->permanent), entry.permanent_attr));
}

static void dump_checker_info(vm_entry_checker_t *checker) {
  /*
...
void dump_checker_range(entry_checker_range_t list) {
 ...
 */
static void warn_bad_checker(vm_entry_checker_t *checker,
...
static mach_vm_size_t overestimate_size(const vm_entry_template_t templates[],
                                        unsigned count) {
  mach_vm_size_t size = 0;
 ...
 */
static void allocate_arena(mach_vm_size_t arena_size,
  ...
  kr = mach_vm_map(mach_task_self(), &allocated, arena_unaligned_size,
                   0 /* alignment mask */,
                   VM_FLAGS_ANYWHERE | VM_FLAGS_RANDOM_ADDR, 0, 0, 0, 0, 0, 0);
  ...
  if (kr == KERN_NO_SPACE) {
     ...
     */
    kr = mach_vm_map(mach_task_self(), &allocated, arena_unaligned_size,
                     0 /* alignment mask */, VM_FLAGS_ANYWHERE, 0, 0, 0, 0, 0,
                     0);
    if (kr == KERN_SUCCESS) {
  ...
  T_ASSERT_MACH_SUCCESS(kr,
                        "arena allocation "
                        "(size 0x%llx, alignment 0x%llx)",
                        arena_size, arena_alignment_mask);

  if (arena_alignment_mask > PAGE_MASK) {
    /* Align manually within the oversized allocation. */
    mach_vm_address_t aligned =
        (allocated & ~arena_alignment_mask) + arena_alignment_mask + 1;
    ...
    (void)mach_vm_deallocate(mach_task_self(), aligned_end,
                             allocated_end - aligned_end);
...
static void write_fill_pattern(mach_vm_address_t start, mach_vm_size_t size,
                               fill_pattern_t fill_pattern) {
  assert(start % sizeof(uint64_t) == 0);
  if (fill_pattern.mode == Fill) {
    for (mach_vm_address_t c = start; c < start + size; c += sizeof(uint64_t)) {
 ...
 */
static bool verify_fill_pattern(mach_vm_address_t start, mach_vm_size_t size,
                                fill_pattern_t fill_pattern,
  ...
  assert(start % sizeof(uint64_t) == 0);
  if (fill_pattern.mode == Fill) {
    for (mach_vm_address_t c = start; c < end; c += sizeof(uint64_t)) {
...
typedef enum {
  vsto_make_submap = 1, /* make submap from entries in current_map() at
                           start..end, offset ignored */
  vsto_remap_submap =
      2, /* map in current_map() at start..end, from submap address offset */
  vsto_end
} vm_submap_test_op;
...
static void submap_op(vm_submap_test_args *args) {
  int err = sysctlbyname("vm.submap_test_ctl", NULL, NULL, args, sizeof(*args));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "sysctl(vm.submap_test_ctl)");
}

/* Lower address range [start..end) into a submap at that same address. */
static void submapify(mach_vm_address_t start, mach_vm_address_t end) {
 ...
 */
static void remap_submap(mach_vm_address_t submap_base_address,
...
static void
allocate_submap_storage_and_checker(checker_list_t *checker_list,
   ...
   * - disable share mode verification (TODO vm_region says SM_COW, we say
   * SM_PRIVATE)
   * - TODO resolve needs_copy COW and change to COPY_DELAY
   */
  FOREACH_CHECKER(submap_checker, submap_checkers->entries) {
    T_QUIET;
    T_ASSERT_EQ(submap_checker->submap_depth, 0, "nested submaps not allowed");
  ...
  mach_vm_address_t submap_start =
      checker_range_start_address(submap_checkers->entries);
  mach_vm_address_t submap_end =
      checker_range_end_address(submap_checkers->entries);
  assert(submap_start < submap_end);

  /* verify that the submap is bigger than min_size */
  T_QUIET;
  T_ASSERT_GE(submap_end - submap_start, object_scratch->min_size,
   ...
   * This checker stores the entry and object checkers for the submap's
   * contents.
   */
  vm_object_checker_t *obj_checker =
      make_submap_object_checker(checker_list, submap_checkers);
...
static void
allocate_object_storage_and_checker(checker_list_t *checker_list,
                                    const vm_object_template_t *object_tmpl,
                                    object_scratch_t *object_scratch) {
  kern_return_t kr;
  ...
  if (object_scratch->is_private == 1) {
    ...
    object_scratch->checker = make_anonymous_object_checker(checker_list, size);

    write_fill_pattern(address, size, object_tmpl->fill_pattern);
    object_scratch->checker->fill_pattern = object_tmpl->fill_pattern;
  } else {
    T_FAIL("unexpected/unimplemented: object is neither private nor anonymous "
           "nor submap");
  }
}
 ...
 */
static uint8_t choose_user_tag(uint16_t requested_tag, uint8_t *inc) {
 ...
 * override it.
 */
static uint8_t template_real_share_mode(const vm_entry_template_t *tmpl) {
...
static void create_vm_hole(const vm_entry_template_t *tmpl,
  ...
  /* deallocate the hole */
  kr = mach_vm_deallocate(mach_task_self(), dest_address, tmpl->size);
  assert(kr == 0);
...
static void create_vm_submap(const vm_entry_template_t *tmpl,
                             object_scratch_t *object_scratch,
                             mach_vm_address_t dest_address,
                             checker_list_t *checker_list) {
  kern_return_t kr;
  ...
  remap_submap(object_scratch->allocated_address, dest_address, tmpl->size,
               tmpl->offset);
   ...
   */
  checker_range_append(&checker_list->entries,
                       make_checker_for_submap(dest_address, tmpl->size,
                                               tmpl->offset,
                                               object_scratch->checker));
}

__attribute__((overloadable)) checker_list_t *
create_vm_state(const vm_entry_template_t entry_templates[],
  ...
  const vm_object_template_t *end_object_templates =
      &object_templates[object_template_count];
  ...
  for (unsigned i = 0; i < entry_template_count; i++) {
    ...
    T_QUIET;
    T_ASSERT_TRUE(
        prot_contains_all(VM_PROT_READ | VM_PROT_WRITE, tmpl->protection),
        "entry template #%u protection 0x%x exceeds VM_PROT_READ | "
        "VM_PROT_WRITE",
        i, tmpl->protection);

    T_QUIET;
    T_ASSERT_TRUE(prot_contains_all(VM_PROT_ALL, tmpl->max_protection),
                  "entry template #%u max_protection 0x%x exceeds VM_PROT_ALL",
                  i, tmpl->max_protection);

    T_QUIET;
    T_ASSERT_TRUE(
        prot_contains_all(tmpl->max_protection, tmpl->protection),
        "entry template #%u protection exceeds max_protection (%s/%s)", i,
        name_for_prot(tmpl->protection), name_for_prot(tmpl->max_protection));

    /* entry can't be COW and wired at the same time */
    assert(!(tmpl->user_wired_count > 0 &&
             template_real_share_mode(tmpl) == SM_COW));
    ...
    if (tmpl->object) {
      ...
      object_scratch = &new_objects[tmpl->object - start_object_templates];
  ...
  for (unsigned i = 0; i < entry_template_count; i++) {
    ...
    object_scratch_t *object_scratch =
        tmpl->object ? &new_objects[object_tmpl - start_object_templates]
                     : NULL;
    ...
    if (template_real_share_mode(tmpl) == SM_SHARED) {
      ...
      assert(object_scratch->entry_count > 1 &&
             "SM_SHARED entry's object template must be used by at least one "
             "other entry");
    }
  }
  ...
  for (unsigned i = 0; i < object_template_count; i++) {
    ...
    if (object_tmpl->kind == SubmapObject) {
      allocate_submap_storage_and_checker(checker_list, object_tmpl,
                                          object_scratch);
    } else {
      allocate_object_storage_and_checker(checker_list, object_tmpl,
                                          object_scratch);
    }
  }
  ...
  for (unsigned i = 0; i < entry_template_count; i++) {
    ...
    if (template_real_share_mode(tmpl) == SM_SHARED) {
      ...
      kr = mach_vm_map(mach_task_self(), &allocated_address, tmpl->size,
                       0, /* alignment mask */
                       VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE |
                           VM_MAKE_TAG(assigned_tag) | permanent_flag,
      ...
      vm_entry_checker_t *checker = make_checker_for_shared(
          checker_list, tmpl->kind, allocated_address, tmpl->size, tmpl->offset,
          tmpl->protection, tmpl->max_protection, assigned_tag, tmpl->permanent,
          object_scratch->checker);
      ...
      kr = mach_vm_map(mach_task_self(), &allocated_address, tmpl->size,
                       ...
                       0,     /* object offset */
                       false, /* copy */
                       initial_protection, initial_max_protection,
                       VM_INHERIT_DEFAULT);
      assert(kr == 0);
      assert(allocated_address == arena_address);

      vm_entry_checker_t *checker = make_checker_for_anonymous_private(
          checker_list, tmpl->kind, allocated_address, tmpl->size,
      ...
      if (template_real_share_mode(tmpl) == SM_PRIVATE) {
         ...
         */
        vm_object_checker_t *obj_checker = make_anonymous_object_checker(
            checker_list, checker->object_offset + checker->size);
        if (tmpl->object) {
          ...
          kr = mach_vm_behavior_set(mach_task_self(), checker->address,
                                    checker->size, VM_BEHAVIOR_FREE);
          assert(kr == 0);
          kr = mach_vm_behavior_set(mach_task_self(), checker->address,
                                    checker->size, VM_BEHAVIOR_PAGEOUT);
          assert(kr == 0);
        }
        checker_set_object(checker, obj_checker);
      ...
      if (protect_last) {
         ...
         */
        kr = mach_vm_protect(mach_task_self(), allocated_address, tmpl->size,
                             false /*set_max*/, tmpl->protection);
        assert(kr == 0);
        kr = mach_vm_protect(mach_task_self(), allocated_address, tmpl->size,
                             true /*set_max*/, tmpl->max_protection);
        assert(kr == 0);
      }
  ...
  for (unsigned i = 0; i < entry_template_count; i++) {
    ...
    if (tmpl->inheritance != VM_INHERIT_DEFAULT) {
      kr = mach_vm_inherit(mach_task_self(), checker->address, checker->size,
                           tmpl->inheritance);
      assert(kr == 0);
    ...
    if (tmpl->user_wired_count > 0) {
      ...
      for (uint16_t w = 0; w < tmpl->user_wired_count; w++) {
        kr = mach_vm_wire(host_priv(), mach_task_self(), checker->address,
                          checker->size, VM_PROT_READ);
        assert(kr == 0);
      }
    }
  ...
  kr = mach_vm_deallocate(mach_task_self(), arena_address,
                          arena_end - arena_address);
  assert(kr == 0);
...
void create_vm_state_from_config(vm_config_t *config,
                                 checker_list_t **const out_checker_list,
                                 mach_vm_address_t *const out_start_address,
                                 mach_vm_address_t *const out_end_address) {
  checker_list_t *list =
      create_vm_state(config->entry_templates, config->entry_template_count,
  ...
  *out_end_address =
      checker_end_address(list->entries.tail) + config->end_adjustment;
 ...
 */
static void checker_deallocate_allocation(checker_list_t *list,
                                          vm_entry_checker_t *checker) {
  assert(checker->kind == Allocation || checker->kind == Submap);

  kern_return_t kr =
      mach_vm_deallocate(mach_task_self(), checker->address, checker->size);
  assert(kr == 0);
 ...
 */
static void deallocate_vm_allocations(checker_list_t *list) {
  /* not FOREACH_CHECKER due to use-after-free */
...
static void
learn_object_id(checker_list_t *checker_list, vm_object_checker_t *obj_checker,
                uint64_t object_id,
 ...
 */
static bool verify_allocation(checker_list_t *checker_list,
                              vm_entry_checker_t *checker,
                              const char *message) {
  vm_entry_attribute_list_t bad_entry_attr =
  ...
  vm_region_submap_info_data_64_t info;
  if (!get_info_for_address(&actual_address, &actual_size, &info,
                            checker->submap_depth)) {
  ...
  if (actual_address != checker->address || actual_size != checker->size) {
  ...
  if (checker->verify.behavior_attr && info.behavior != checker->behavior) {
  ...
  if (checker->verify.user_tag_attr && info.user_tag != checker->user_tag) {
  ...
  if (checker->verify.object_attr) {
    ...
    unsigned vm_region_ref_count =
        object_checker_get_vm_region_ref_count(obj_checker);
    ...
    if (obj_checker->verify.ref_count_attr &&
      ...
      T_FAIL("%s: wrong object ref count (want %u got %u)", message,
             vm_region_ref_count, info.ref_count);
      bad_entry_attr.object_attr = true;
    ...
    if (obj_checker->verify.shadow_depth_attr &&
      ...
      T_FAIL("%s: wrong object shadow depth (want %u got %u)", message,
    ...
    if (!obj_checker->verify.fill_pattern_attr) {
      ...
      T_LOG("note: %s: can't verify fill pattern of unreadable memory (%s/%s)",
            message, name_for_prot(info.protection),
            name_for_prot(info.max_protection));
    } else {
      ...
      if (!verify_fill_pattern(actual_address, actual_size,
        ...
        T_FAIL("%s: wrong fill at address 0x%llx "
               ...
               message, first_bad_address, obj_checker->fill_pattern.pattern,
  ...
  if (checker->verify.share_mode_attr && !same_share_mode(&info, checker)) {
  ...
  if (checker->verify.pages_resident_attr &&
    ...
    T_FAIL("%s: wrong pages resident count (want %d, got %d)", message,
           checker->pages_resident, info.pages_resident);
 ...
 * T_FAILs and logs details and returns false if it looks wrong.
 */
static bool verify_hole(vm_entry_checker_t *checker, const char *message) {
  bool good = true;
  ...
  if (!good) {
    warn_bad_checker(checker, vm_entry_attributes_with_default(true),
                     vm_object_attributes_with_default(true), message);
  }
  return good;
}

test_result_t verify_vm_state_nested(checker_list_t *checker_list,
                                     bool in_submap, const char *message) {
  bool good = true;

  if (Verbose) {
    T_LOG("*** %s: verifying vm entries %s ***", message,
          in_submap ? "(in submap) " : "");
  }
...
test_result_t verify_vm_state(checker_list_t *checker_list,
                              const char *message) {
 ...
 */
static void
get_expected_errors_for_faults(vm_entry_checker_t *checker,
...
static fill_pattern_t checker_fill_pattern(vm_entry_checker_t *checker) {
...
static bool checker_should_verify_fill_pattern(vm_entry_checker_t *checker) {
  return checker->verify.object_attr && checker->object != NULL &&
 ...
 */
bool verify_checker_faultability(vm_entry_checker_t *checker,
                                 const char *message, bool verify_reads,
                                 bool verify_writes) {
  return verify_checker_faultability_in_address_range(
      checker, message, verify_reads, verify_writes, checker->address,
      checker->size);
}

bool verify_checker_faultability_in_address_range(
  ...
  if (Verbose) {
    ...
    T_LOG("%s: trying %s faults in [0x%llx..0x%llx)", message, faults,
  ...
  get_expected_errors_for_faults(checker, &expected_read_error,
  ...
  for (mach_vm_address_t addr = start; addr < end; addr += PAGE_SIZE) {
    if (verify_reads) {
      ...
      if (enforce_expected_byte && actual_read_error == KERN_SUCCESS &&
...
static test_result_t verify_vm_faultability_nested(checker_list_t *checker_list,
                                                   const char *message,
                                                   bool verify_reads,
                                                   bool verify_writes,
                                                   bool in_submap) {
  ...
  if (Verbose) {
    T_LOG("*** %s: verifying vm faultability %s ***", message,
          in_submap ? "(in submap) " : "");
  }
  ...
  FOREACH_CHECKER(checker, checker_list->entries) {
    ...
    if (checker_is_submap(checker)) {
      ...
      T_QUIET;
      T_ASSERT_FALSE(in_submap, "nested submaps not allowed");
      ...
      good &= verify_vm_faultability_nested(submap_checkers, message,
                                            verify_reads, really_verify_writes,
                                            true /* in_submap */);
    } else {
      good &= verify_checker_faultability(checker, message, verify_reads,
                                          verify_writes);
    }
  }
...
test_result_t verify_vm_faultability(checker_list_t *checker_list,
                                     const char *message, bool verify_reads,
                                     bool verify_writes) {
  return verify_vm_faultability_nested(checker_list, message, verify_reads,
                                       verify_writes, false /* in_submap */);
}

/* Inserts new_left to the left of old_right. */
static void checker_insert_left(vm_entry_checker_t *new_left,
...
static void checker_insert_right(vm_entry_checker_t *old_left,
 ...
 */
static void checker_split_clones(vm_entry_checker_t *left,
...
vm_entry_checker_t *checker_clip_right(checker_list_t *list,
...
vm_entry_checker_t *checker_clip_left(checker_list_t *list,
                                      vm_entry_checker_t *right,
                                      mach_vm_address_t split) {
...
static entry_checker_range_t checker_list_try_find_range_including_holes(
...
entry_checker_range_t checker_list_find_range_including_holes(
    checker_list_t *list, mach_vm_address_t start, mach_vm_size_t size) {
...
entry_checker_range_t checker_list_find_range(checker_list_t *list,
                                              mach_vm_address_t start,
                                              mach_vm_size_t size) {
  entry_checker_range_t result =
      checker_list_find_range_including_holes(list, start, size);

  FOREACH_CHECKER(checker, result) { assert(checker->kind != Hole); }

  return result;
}

vm_entry_checker_t *checker_list_find_checker(checker_list_t *list,
...
vm_entry_checker_t *checker_list_find_allocation(checker_list_t *list,
                                                 mach_vm_address_t addr) {
...
entry_checker_range_t checker_list_find_and_clip(checker_list_t *list,
                                                 mach_vm_address_t start,
                                                 mach_vm_size_t size) {
...
entry_checker_range_t checker_list_find_and_clip_including_holes(
    checker_list_t *list, mach_vm_address_t start, mach_vm_size_t size) {
  mach_vm_address_t end = start + size;
...
static bool can_simplify_kind(vm_entry_checker_t *left,
                              vm_entry_checker_t *right) {
  return (left->kind == Allocation && right->kind == Allocation) ||
         (left->kind == Submap && right->kind == Submap);
}

void checker_simplify_left(checker_list_t *list, vm_entry_checker_t *right) {
  vm_entry_checker_t *left = right->prev;
  if (!left) {
    return;
  }
  if (can_simplify_kind(left, right) && left->protection == right->protection &&
...
void checker_list_simplify(checker_list_t *list, mach_vm_address_t start,
                           mach_vm_size_t size) {
  mach_vm_address_t end = start + size;
  entry_checker_range_t limit =
      checker_list_find_range_including_holes(list, start, size);
  ...
  FOREACH_CHECKER(checker, limit) { checker_simplify_left(list, checker); }
}

void checker_list_replace_range(checker_list_t *list,
                                entry_checker_range_t old_range,
                                entry_checker_range_t new_range) {
  /* old_range and new_range must coincide */
  assert(checker_range_start_address(old_range) ==
         checker_range_start_address(new_range));
  assert(checker_range_end_address(old_range) ==
         checker_range_end_address(new_range));
...
void checker_list_free_range(checker_list_t *list,
  ...
  vm_entry_checker_t *new_hole = make_checker_for_hole(
      checker_range_start_address(range), checker_range_size(range));
...
static bool checker_has_null_vm_object(vm_entry_checker_t *checker) {
  return object_is_null(checker->object);
}

void checker_resolve_null_vm_object(checker_list_t *checker_list,
                                    vm_entry_checker_t *checker) {
...
void checker_fault_for_prot_not_cow(checker_list_t *checker_list,
                                    vm_entry_checker_t *checker,
...
vm_entry_checker_t *checker_list_try_unnest_one_entry_in_submap(
  ...
  {
    /* Find the checker for the entry inside the submap at this parent map
     * address. */
    checker_list_t *submap_checkers DEFER_UNSLIDE =
...
__attribute__((overloadable)) vm_config_t *
...
#define ASSERT_IS_WITHIN(ptr, array, array_count)                              \
  ...
  for (unsigned i = 0; i < result->submap_entry_template_count; i++) {
    ...
    if (tmpl->object) {
       ...
       */
      ASSERT_IS_WITHIN(tmpl->object, submap_object_templates,
                       submap_object_template_count);
      tmpl->object = &result->submap_object_templates[tmpl->object -
                                                      submap_object_templates];
    }
  }
  for (unsigned i = 0; i < result->object_template_count; i++) {
    ...
    ASSERT_IS_WITHIN(tmpl->submap.entries, submap_entry_templates,
    ...
    assert(submap_index + tmpl->submap.entry_count <=
           submap_entry_template_count);

    if (tmpl->submap.objects) {
      ASSERT_IS_WITHIN(tmpl->submap.objects, submap_object_templates,
                       submap_object_template_count);
      ...
      assert(object_index + tmpl->submap.object_count <=
             submap_object_template_count);
    }
  }
  for (unsigned i = 0; i < result->submap_object_template_count; i++) {
    /* no fixups needed inside submap_object_templates, they can't be nested
     * submap objects */
...
static void free_vm_config(vm_config_t *config) {
...
__attribute__((constructor)) static void vm_configurator_init(void) {
  /*
   ...
   */
  guard_entry_template =
      vm_entry_template(.protection = 0, .max_protection = 0,
                        .user_tag = VM_MEMORY_GUARD /* 31 */);
  hole_template = vm_entry_template(.kind = Hole);
  END_ENTRIES = vm_entry_template(.kind = EndEntries);
...
test_result_t test_is_unimplemented(checker_list_t *checker_list __unused,
...
void run_one_vm_test(const char *filename, const char *funcname,
                     const char *testname, configure_fn_t configure_fn,
                     test_fn_t test_fn) {
  vm_config_t *config;
  checker_list_t *checker_list;
  ...
  if (test_fn == NULL) {
    ...
    T_FAIL("test %s.%s not present in test file %s; please write it", funcname,
           testname, short_filename);
    return;
  } else if (test_fn == test_is_unimplemented) {
    /* Test is deliberately not implemented. */
    T_PASS("unimplemented test: %s %s %s", short_filename, funcname, testname);
    return;
  }

  /* Prepare the VM state. */
  config = configure_fn();
  T_LOG("note: starting test: %s %s (%s) ...", funcname, testname,
        config->config_name);

  create_vm_state_from_config(config, &checker_list, &test_fn_start_address,
                              &test_fn_end_address);
  ...
  if (vm_state_start_address != test_fn_start_address ||
    ...
    T_LOG("note: prepared vm state is 0x%llx..0x%llx; calling tested function "
          "on 0x%llx..0x%llx",
          vm_state_start_address, vm_state_end_address, test_fn_start_address,
          test_fn_end_address);
  } else {
    T_LOG("note: prepared vm state is 0x%llx..0x%llx; calling tested function "
          "on the entire range",
  ...
  case TestSucceeded:
    ...
    if (result == TestSucceeded) {
      result = verify_vm_faultability(checker_list, "final faultability check",
                                      true, true);
    }
    if (result == TestSucceeded) {
  ...
  if (result == TestFailed) {
    ...
    T_FAIL("%s %s %s (%s) failed above; run it locally with `env VERBOSE=1 "
           "%s%.*s -n %s %s`",
           short_filename, funcname, testname, config->config_name, arch_cmd,
           exe_name_len, exe_name, funcname, testname);
  } else {
    T_PASS("%s %s %s (%s)", short_filename, funcname, testname,
           config->config_name);
  }
