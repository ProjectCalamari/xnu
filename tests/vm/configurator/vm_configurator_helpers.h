 ...
 */
static inline void
clear_then_set_exc_guard_behavior(task_exc_guard_behavior_t clear,
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "get EXC_GUARD behavior");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "update EXC_GUARD behavior");
}
 ...
 */
static inline void disable_vm_exc_guard(void) {
  clear_then_set_exc_guard_behavior(TASK_EXC_GUARD_VM_ALL, /* clear */
                                    0 /* set */);
}
 ...
 */
static inline void enable_fatal_vm_exc_guard(void) {
  clear_then_set_exc_guard_behavior(TASK_EXC_GUARD_VM_ALL, /* clear */
                                    TASK_EXC_GUARD_VM_DELIVER |
                                        TASK_EXC_GUARD_VM_FATAL /* set */);
}
 ...
 * Halt with T_FAIL if they cannot be enabled.
 */
static inline void enable_non_fatal_vm_exc_guard(void) {
  clear_then_set_exc_guard_behavior(TASK_EXC_GUARD_VM_ALL, /* clear */
 ...
 */
static inline void
checker_perform_successful_vm_deallocate(checker_list_t *checker_list,
 ...
 */
static inline void
checker_perform_vm_deallocate_permanent(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
