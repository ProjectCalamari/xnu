...
void test_90774260(void) {
  ...
  kr = mach_memory_object_memory_entry(mach_host_self(), 1, (vm_size_t)vm_size,
  ...
  if (kr == MIG_BAD_ARGUMENTS) {
    T_LOG("mach_memory_object_memory_entry() returned MIG_BAD_ARGUMENTS "
          "(because broken on arm64_32)");

    memory_object_size_t me_size = vm_size;
    kr = mach_make_memory_entry_64(mach_task_self(), &me_size, 0,
                                   VM_PROT_READ | VM_PROT_WRITE |
                                       MAP_MEM_NAMED_CREATE,
                                   &first_layer_entry_port, MACH_PORT_NULL);
    T_ASSERT_MACH_SUCCESS(kr,
                          "mach_make_memory_entry_64(MAP_MEM_NAMED_CREATE)");
  } else
  ...
  kr = mach_vm_map(mach_task_self(), &first_layer_addr, vm_size, 0, /* mask */
                   VM_FLAGS_ANYWHERE, first_layer_entry_port, 0,    /* offset */
                   0,                                               /* copy */
  ...
  *(int *)first_layer_addr = original_value;
  ...
  kr = mach_make_memory_entry_64(
      mach_task_self(), &me_size, first_layer_addr,
      (VM_PROT_READ | VM_PROT_WRITE | MAP_MEM_VM_COPY), &cow_port, 0);
  T_ASSERT_MACH_SUCCESS(kr, "mach_make_memory_entry_64()");
  T_QUIET;
  T_ASSERT_EQ(me_size, vm_size, "incomplete memory entry");

  // map cow port to a new address in the shared way
  uint64_t shared_addr = 0;
  kr = mach_vm_map(mach_task_self(), &shared_addr, vm_size, 0, /* mask */
  ...
  uint64_t cow_addr = 0;
  kr = mach_vm_map(mach_task_self(), &cow_addr, vm_size, 0, /* mask */
                   VM_FLAGS_ANYWHERE, cow_port, 0,          /* offset */
                   1,                                       /* copy */
                   VM_PROT_READ | VM_PROT_WRITE, VM_PROT_READ | VM_PROT_WRITE,
                   VM_INHERIT_DEFAULT);
