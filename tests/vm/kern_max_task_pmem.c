...
/*
 * Embedded Device having physical memory greater than 600MB should have
 * positive value for kern.max_task_pmem if present. Strategy: Fetch hw.memsize
 * for the device. If hw.memsize > 600MB, and kern.max_task_pmem is present,
 * assert that kern.max_task_pmem is set to value > 0.
 */
T_DECL(kern_max_task_pmem,
       "Embedded platforms should have a positive value for kern.max_task_pmem "
       "when hw.memsize > 600MB",
       T_META_TAG_VM_PREFERRED, T_META_ENABLED(false /* rdar://133954365 */)) {
  ...
  if (hw_memsize <= HW_MEMSIZE_THRESHOLD) {
    T_SKIP("Device has hw.memsize = %lld. Skipping the check for %s",
           hw_memsize, MAX_TASK_PMEM);
  }
  ...
  T_ASSERT_GT_INT(kern_max_task_pmem, 0, "%s should be greater than 0",
                  MAX_TASK_PMEM);
}
