...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("VM"), T_META_OWNER("francois"),
              T_META_CHECK_LEAKS(true), T_META_RUN_CONCURRENTLY(true));

T_DECL(direct_write_wired, "test direct write() to wired file",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd, "ftruncate()");

  map_addr = mmap(NULL, file_size, PROT_READ | PROT_WRITE,
                  MAP_FILE | MAP_SHARED, fd, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(map_addr, "mmap()");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "fcntl(F_NOCACHE)");

  buf = valloc(file_size);
  T_QUIET;
  T_ASSERT_NOTNULL(buf, "valloc()");
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(num_bytes, "write()");
  T_QUIET;
  T_EXPECT_EQ(num_bytes, (ssize_t)file_size, "wrote <file_size> bytes");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(unlink(tmpf), "unlink(%s)", tmpf);
}
...
T_DECL(direct_write_wired_vector_upl,
  ...
  for (w = 0, r = NUM_IOVS - 1; w < NUM_IOVS; w++, r--) {
    ...
    T_ASSERT_POSIX_SUCCESS(
        posix_memalign(&w_iovs[w].iov_base, PAGE_SIZE, w_iovs[w].iov_len),
        "alloc(w_iov_base[%d])", w);
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(
        posix_memalign(&r_iovs[r].iov_base, PAGE_SIZE, r_iovs[r].iov_len),
        "alloc(r_iov_base[%d])", r);
  }
  strlcpy(tmpf, dt_tmpdir(), PATH_MAX);
  strlcat(tmpf, "/wire_direct_write.txt", PATH_MAX);
  T_QUIET;
  T_ASSERT_LT(strlen(tmpf), (unsigned long)PATH_MAX, "path exceeds PATH_MAX");
  fd = open(tmpf, O_RDWR | O_CREAT | O_TRUNC);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd, "open()");
  /* use F_NOCACHE to get direct I/O */
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fcntl(fd, F_NOCACHE, 1), "fcntl(F_NOCACHE)");

  buf = valloc(expected_bytes);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(buf, "valloc()");
  memset(buf, '?', expected_bytes);
  bytes = pwrite(fd, buf, expected_bytes, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(bytes, "pwrite()");
  T_ASSERT_EQ(bytes, expected_bytes, "pwrite() wrote %ld bytes", bytes);
  ...
  T_ASSERT_POSIX_SUCCESS(ret, "mlock(0x%llx)",
                         (uint64_t)((NUM_IOVS / 2) * PAGE_SIZE));

  bytes = pwritev(fd, w_iovs, NUM_IOVS, 0);
  T_ASSERT_EQ(bytes, expected_bytes, "pwritev() wrote %ld bytes", bytes);

  bytes = preadv(fd, r_iovs, NUM_IOVS, 0);
  T_ASSERT_EQ(bytes, expected_bytes, "preadv() read %ld bytes", bytes);
  ...
  for (r = 0; r < NUM_IOVS; r++) {
    ...
    for (r_idx = 0; r_idx < r_cnt; r_idx++) {
      T_QUIET;
      T_ASSERT_EQ(
          r_base[r_idx], w_base[w_idx],
          "r_iovs[%d].iov_base[%d]='%c' == w_iovs[%d].iov_base[%d]='%c'", r,
          r_idx, (unsigned char)r_base[r_idx], w, w_idx,
          (unsigned char)w_base[w_idx]);
      if (++w_idx == w_cnt) {
        w++;
        w_base = w_iovs[w].iov_base;
        w_cnt = (int)w_iovs[w].iov_len;
