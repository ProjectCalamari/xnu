...
static struct child_rc fork_child_test(void (^block)(void)) {
  struct child_rc rc = {};
  pid_t child_pid;
...
static mach_vm_address_t get_permanent_mapping(mach_vm_size_t size) {
  kern_return_t kr;
  ...
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_allocate(%lld, PERMANENT) == %p", size,
                        (void *)addr);
  ...
  *(int *)addr = 42;
  ...
  T_QUIET;
  T_EXPECT_EQ(*(int *)addr, 42, "we can still read what we wrote");
...
T_DECL(permanent_mapping, "check permanent mappings semantics",
  ...
  rc = fork_child_test(^{
     ...
     * we should get an error.
     */
    T_ASSERT_MACH_ERROR(kr2, KERN_NO_SPACE, "mach_vm_protect(VM_PROT_COPY)");
  ...
  rc = fork_child_test(^{
    ...
    addr2 = 0;
    kr2 = mach_vm_allocate(mach_task_self(), &addr2, size, VM_FLAGS_ANYWHERE);
    T_QUIET;
    T_EXPECT_MACH_SUCCESS(kr2, "vm_allocate()");

    remap_addr = addr;
    kr2 = mach_vm_remap(mach_task_self(), &remap_addr, size, 0,
                        VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, mach_task_self(),
                        addr2, TRUE, &cur_prot, &max_prot, VM_INHERIT_DEFAULT);

    /*
     * because the permanent mapping wasn't removed,
     * we should get an error.
     */
    T_ASSERT_MACH_ERROR(kr2, KERN_NO_SPACE, "mach_vm_remap()");
...
T_DECL(vm_tag_describe, "test mach_vm_tag_describe()",
       T_META_TAG_VM_PREFERRED) {
  for (unsigned int i = 0; i <= VM_MEMORY_COUNT; i++) {
    ...
    T_EXPECT_LE(strlen(desc), 24UL,
                "Tag description must be less than 24 characters");
  }
}
