...
static void *protect_thread(__unused void *arg) {
  kern_return_t kr;
  struct context1 *ctx;
  ...
  while (!ctx->done) {
    ...
    kr = vm_protect(mach_task_self(), ctx->e0, ctx->obj_size,
                    FALSE, /* set_maximum */
                    VM_PROT_READ);
    T_QUIET;
    T_EXPECT_MACH_SUCCESS(kr, " vm_protect() RO");
    /* wait a little bit */
    usleep(100);
    ...
    T_QUIET;
    T_EXPECT_MACH_SUCCESS(kr, " vm_protect() RW");
...
T_DECL(unaligned_write_to_cow_bypass, "Test that unaligned copy respects COW",
  ...
  for (start = time(NULL), loops = 0; time(NULL) < start + duration; loops++) {
    ...
    kr = vm_allocate(mach_task_self(), &ctx->e0, ctx->obj_size,
                     VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_MAKE_TAG(241));
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "vm_allocate e0");
    memset((char *)ctx->e0, 'A', ctx->obj_size);
    /* initialize to 'A' */

    /* make a COW copy of e0 */
    e1 = 0;
    kr = vm_read(mach_task_self(), ctx->e0, ctx->obj_size, &e1, &cow_read_size);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "vm_read e0->e1");
    ...
    T_QUIET;
    T_ASSERT_EQ(*(char *)e1, 'A', "COW mapping was modified");
