 ...
 */
static void checker_perform_vm_protect(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size, bool set_max,
                                       vm_prot_t prot) {
 ...
 */
static test_result_t vm_protect_successfully(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size,
                                             vm_prot_t prot) {
  kern_return_t kr;
  ...
  if (kr != 0) {
    T_FAIL("mach_vm_protect(%s) failed (%s)", name_for_prot(prot),
           name_for_kr(kr));
    return TestFailed;
  }
 ...
 */
static test_result_t vm_protect_with_holes(checker_list_t *checker_list,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size) {
  kern_return_t kr;
 ...
 */
static test_result_t vm_protect_beyond_max_prot(checker_list_t *checker_list,
                                                mach_vm_address_t start,
                                                mach_vm_size_t size,
                                                vm_prot_t prot) {
  kern_return_t kr;
  ...
  if (kr != KERN_PROTECTION_FAILURE) {
    T_FAIL("mach_vm_protect(%s which is beyond max) expected %s, got %s\n",
           name_for_prot(prot), name_for_kr(KERN_PROTECTION_FAILURE),
           name_for_kr(kr));
    return TestFailed;
  }
 ...
 */
static test_result_t vm_protect_max_000(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  ...
  results[1] =
      vm_protect_beyond_max_prot(checker_list, start, size, VM_PROT_READ);
  results[2] =
      vm_protect_beyond_max_prot(checker_list, start, size, VM_PROT_WRITE);
  results[3] = vm_protect_beyond_max_prot(checker_list, start, size,
 ...
 */
static test_result_t vm_protect_max_r00(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  test_result_t results[4];
 ...
 */
static test_result_t vm_protect_max_0w0(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  test_result_t results[4];
  ...
  results[1] =
      vm_protect_beyond_max_prot(checker_list, start, size, VM_PROT_READ);
  results[2] =
      vm_protect_successfully(checker_list, start, size, VM_PROT_WRITE);
  results[3] = vm_protect_beyond_max_prot(checker_list, start, size,
                                          VM_PROT_READ | VM_PROT_WRITE);
 ...
 */
static test_result_t vm_protect_max_rw0(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  test_result_t results[4];
  ...
  results[2] =
      vm_protect_successfully(checker_list, start, size, VM_PROT_WRITE);
  results[3] = vm_protect_successfully(checker_list, start, size,
                                       VM_PROT_READ | VM_PROT_WRITE);
 ...
 * VM_PROT_EXEC
 */
static test_result_t vm_protect_max_rwx(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  /* TODO VM_PROT_EXEC */
...
vm_protect_max_r00_or_unnested_submap(checker_list_t *checker_list,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
#if __x86_64__
...
T_DECL(vm_protect, "run vm_protect with various vm configurations") {
  vm_tests_t tests = {
