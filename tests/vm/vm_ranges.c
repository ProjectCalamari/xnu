...
enum { DEFAULT = 0, HEAP, LARGE_FILE };
...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_ENABLED(!TARGET_OS_OSX), T_META_OWNER("mmorran"));

static bool ranges_enabled(void) {
   ...
   */
  return sysctlbyname("vm.vm_map_user_range_default", &range, &range_sz, NULL,
                      0) == 0;
}
...
static struct mach_vm_range get_range(int target_range) {
  int ret = EINVAL;
  struct mach_vm_range range;
  size_t range_sz = sizeof(range);
  ...
  switch (target_range) {
  case DEFAULT:
    ret = sysctlbyname("vm.vm_map_user_range_default", &range, &range_sz, NULL,
                       0);
    T_QUIET;
  ...
  case LARGE_FILE:
    ret = sysctlbyname("vm.vm_map_user_range_large_file", &range, &range_sz,
                       NULL, 0);
    T_QUIET;
...
static task_vm_info_data_t get_vm_task_info(void) {
  task_vm_info_data_t ti;

  mach_msg_type_number_t count = TASK_VM_INFO_COUNT;
  kern_return_t const kr =
      task_info(mach_task_self(), TASK_VM_INFO, (task_info_t)&ti, &count);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "get task_info()");
  return ti;
}

static mach_vm_address_t assert_allocate(mach_vm_address_t dst, int vm_flags) {
  int ret = mach_vm_allocate(mach_task_self(), &dst, ALLOCATION_SIZE, vm_flags);
  T_ASSERT_MACH_SUCCESS(ret, "vm_allocate");
  return dst;
}

static void assert_in_range(struct mach_vm_range range, mach_vm_offset_t addr) {
...
static void assert_in_heap_range(mach_vm_offset_t addr) {
  struct mach_vm_range range = get_range(HEAP);

  assert_in_range(range, addr);
}

static void *assert_mmap(void *addr, size_t sz, int fd, int flags) {
...
static void assert_allocate_eq(mach_vm_address_t dst, int vm_flags) {
  mach_vm_address_t target = dst;

  T_ASSERT_MACH_SUCCESS(
      mach_vm_allocate(mach_task_self(), &target, ALLOCATION_SIZE, vm_flags),
      "vm_allocate");

  T_EXPECT_EQ(target, dst, "target/dst differ");
}

static mach_vm_address_t assert_allocate_in_range(int target_range,
...
static void *assert_mmap_in_range(void *addr, int target_range, size_t sz,
                                  int fd, int flags) {
  struct mach_vm_range range = get_range(target_range);
...
static void ensure_mmap_fails(void *addr, size_t sz, int flags, int fd) {
  void *ret = mmap(addr, sz, VM_PROT_READ | VM_PROT_WRITE, flags, fd, 0);
  T_QUIET;
...
T_DECL(range_allocate_anywhere,
  ...
  assert_allocate_in_range(HEAP, range.min_address - ALLOCATION_SIZE,
                           RANGE_HEAP_FLAGS);
}
...
static void ensure_fixed_mappings_succeed_cross(int heap) {
  ...
  assert_allocate_eq(addr, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE |
                               VM_MEMORY_RANGE_DEFAULT);
  vm_deallocate(mach_task_self(), addr, ALLOCATION_SIZE);

  assert_allocate_eq(addr, VM_FLAGS_FIXED | VM_MEMORY_RANGE_HEAP);
  assert_allocate_eq(addr, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE |
                               VM_MEMORY_RANGE_HEAP);
  vm_deallocate(mach_task_self(), addr, ALLOCATION_SIZE);
}

static void ensure_rogue_fixed_fails(void) {
...
static void ensure_fixed_mapping(void) {
...
T_DECL(range_allocate_fixed,
       "ensure fixed target is honored (even with an incorrect tag)",
       T_META_TAG_VM_PREFERRED) {
  CHECK_RANGES_ENABLED();
...
T_DECL(range_mmap_file, "ensure file is mapped within HEAP range",
  ...
  assert_mmap_in_range(NULL, DEFAULT, ALLOCATION_SIZE, fd,
                       MAP_FILE | MAP_SHARED);
#else
...
T_DECL(range_mmap_alias_tag, "ensure anon mapping with tag is honored",
  ...
  assert_mmap_in_range(NULL, DEFAULT, ALLOCATION_SIZE, VM_MEMORY_RANGE_DEFAULT,
...
T_DECL(range_mmap_with_low_hint,
  ...
  assert_mmap_in_range((void *)target, HEAP, ALLOCATION_SIZE, -1,
                       MAP_ANON | MAP_PRIVATE);
}
...
T_DECL(range_mmap_with_high_hint,
  ...
  void *dst = assert_mmap_in_range((void *)target, HEAP, ALLOCATION_SIZE, -1,
                                   MAP_ANON | MAP_PRIVATE);
...
T_DECL(range_mach_vm_map_with_bad_hint,
   ...
   */
  int ret =
      mach_vm_map(mach_task_self(), &addr, ALLOCATION_SIZE, (mach_vm_offset_t)0,
                  VM_FLAGS_ANYWHERE, MACH_PORT_NULL, (memory_object_offset_t)0,
                  FALSE, VM_PROT_DEFAULT, VM_PROT_ALL, VM_INHERIT_DEFAULT);
  T_QUIET;
  T_EXPECT_EQ(ret, KERN_NO_SPACE, "expected KERN_NO_SPACE");
}
...
T_DECL(range_mach_vm_remap_default,
  ...
  int ret =
      mach_vm_remap(mach_task_self(), &target, ALLOCATION_SIZE,
...
T_DECL(range_mach_vm_remap_heap, "ensure mach_vm_remap remains in same range",
       T_META_TAG_VM_PREFERRED) {
  CHECK_RANGES_ENABLED();
...
static void ensure_range(void) {
  struct mach_vm_range def = get_range(DEFAULT);
  struct mach_vm_range heap = get_range(HEAP);

  T_EXPECT_GT(heap.min_address, def.max_address, "ranges should not overlap");
  T_EXPECT_LE(heap.max_address, MACH_VM_MAX_ADDRESS, "expected max <= %llx",
              MACH_VM_MAX_ADDRESS);
  T_EXPECT_EQ(heap.min_address, heap.min_address & (unsigned long)~0x1FFFFF,
              "expected alignment on 2MB TT boundary");
}

static void ensure_child_range(void) {
  struct mach_vm_range def = get_range(DEFAULT);
  struct mach_vm_range heap = get_range(HEAP);

  T_QUIET;
  T_EXPECT_EQ(def.min_address, parent_default.min_address,
              "expected forked default min to be equal");
  T_QUIET;
  T_EXPECT_EQ(def.max_address, parent_default.max_address,
              "expected forked default max to be equal");
  T_QUIET;
  T_EXPECT_EQ(heap.min_address, parent_heap.min_address,
              "expected forked heap min to be equal");
  T_QUIET;
  T_EXPECT_EQ(heap.max_address, parent_heap.max_address,
...
static bool parse_void_ranges(struct mach_vm_range *void1,
  ...
  T_QUIET;
  T_ASSERT_EQ(*s, '\0', "should be done");
...
T_DECL(create_range, "ensure create ranges kinda works",
  ...
  T_LOG("Ranges are %#llx:%#llx %#llx:%#llx", void1.min_address,
        void1.max_address, void2.min_address, void2.max_address);

#define reset() nranges = 0
...
T_DECL(range_mmap_too_large, "ensure mmap fails when allocation is too large",
       T_META_TAG_VM_PREFERRED) {
...
T_DECL(range_mmap_outside_map_range_fixed,
       "ensure mmap fails when making a fixed allocation beyond VM map max "
       "address",
       T_META_TAG_VM_PREFERRED) {
  // Get VM map min_offset and max_offset
  task_vm_info_data_t const ti = get_vm_task_info();
  ...
  ensure_mmap_fails((void *)target, ALLOCATION_SIZE,
                    MAP_ANON | MAP_PRIVATE | MAP_FIXED, -1);
}

static bool large_file_range_enabled(void) {
  struct mach_vm_range range;
  size_t range_sz = sizeof(range);
   ...
   * or not enabled on our process.
   */
  int const ret = sysctlbyname("vm.vm_map_user_range_large_file", &range,
                               &range_sz, NULL, 0);
  if (ret) {
    T_LOG("vm.vm_map_user_range_large_file errno: %d", errno);
  } else {
    T_LOG("large file range: (%llx, %llx)", range.min_address,
          range.max_address);
  }
  return ret == 0;
}
...
T_DECL(range_mmap_large_file,
       "ensure large file is mapped within LARGE_FILE range",
       T_META_TAG_VM_PREFERRED) {
  ...
  for (i = 0; i < N_ALLOC; ++i) {
    void *p = assert_mmap_in_range(NULL, LARGE_FILE, LARGE_ALLOCATION_SIZE, fd,
                                   MAP_SHARED | MAP_FILE);
    if (p == MAP_FAILED) {
      if (errno != ENOMEM) {
        T_WITH_ERRNO;
        T_LOG("mmap failed: stopped at %u of %d/%llu GB chunks", i + 1, N_ALLOC,
              LARGE_ALLOCATION_SIZE);
      }
      break;
    } else {
    ...
    T_QUIET;
    T_ASSERT_NOTNULL(p, "mmap");
    ptrs[i] = p;
  }

  T_EXPECT_GE_UINT(i, N_ALLOC,
                   "Allocate at least %u/%d %llu-GB chunks of VA space", i,
                   N_ALLOC, (LARGE_ALLOCATION_SIZE / GiB(1)));

  T_LOG("Unmapping memory");
  for (uint32_t j = 0; j < i; ++j) {
    int const res = munmap(ptrs[j], LARGE_ALLOCATION_SIZE);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(res, 0, "munmap");
  }
}
