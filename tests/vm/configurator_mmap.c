...
static void checker_perform_successful_mmap_anon(checker_list_t *checker_list,
...
static test_result_t successful_mmap_anon_fixed(checker_list_t *checker_list,
                                                mach_vm_address_t start,
                                                mach_vm_size_t size) {
  ...
  if (allocated != start) {
    T_FAIL("mmap(ANON | FIXED) returned address 0x%llx (expected 0x%llx)",
           allocated, start);
    return TestFailed;
  }
...
successful_mmap_anon_fixed_with_tag(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size, uint16_t tag) {
  void *ret = mmap((void *)start, size, PROT_READ | PROT_WRITE,
  ...
  if (allocated != start) {
    T_FAIL("mmap(ANON | FIXED, tag) returned address 0x%llx (expected 0x%llx)",
           allocated, start);
    return TestFailed;
  }
...
static test_result_t
successful_mmap_anon_fixed_with_neighbor_tags(checker_list_t *checker_list,
                                              mach_vm_address_t start,
                                              mach_vm_size_t size) {
  uint16_t tag;
  ...
  if (TestFailed ==
      successful_mmap_anon_fixed_with_tag(checker_list, start, size, tag)) {
    return TestFailed;
  }
...
static bool call_mmap_anon_fixed_and_expect_ENOMEM(mach_vm_address_t start,
                                                   mach_vm_size_t size,
                                                   uint16_t tag) {
...
static test_result_t test_permanent_entry_fixed(checker_list_t *checker_list,
                                                mach_vm_address_t start,
                                                mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t
test_permanent_before_permanent_fixed(checker_list_t *checker_list,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t
test_permanent_before_allocation_fixed(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t test_permanent_before_allocation_fixed_rdar144128567(
...
static test_result_t
test_permanent_before_hole_fixed(checker_list_t *checker_list,
...
static test_result_t
test_permanent_after_allocation_fixed(checker_list_t *checker_list,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t
test_permanent_after_hole_fixed(checker_list_t *checker_list,
                                mach_vm_address_t start, mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
    T_LOG("warning: can't work around rdar://143341561 on Rosetta; just "
...
static test_result_t
test_permanent_entry_fixed_with_neighbor_tags(checker_list_t *checker_list,
                                              mach_vm_address_t start,
                                              mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t test_permanent_before_permanent_fixed_with_neighbor_tags(
    checker_list_t *checker_list, mach_vm_address_t start,
    mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t test_permanent_before_allocation_fixed_with_neighbor_tags(
    checker_list_t *checker_list, mach_vm_address_t start,
    mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t test_permanent_before_hole_fixed_with_neighbor_tags(
    checker_list_t *checker_list, mach_vm_address_t start,
    mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t test_permanent_after_allocation_fixed_with_neighbor_tags(
    checker_list_t *checker_list, mach_vm_address_t start,
    mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t
test_permanent_after_hole_fixed_with_neighbor_tags(checker_list_t *checker_list,
                                                   mach_vm_address_t start,
                                                   mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
T_DECL(mmap_anon_fixed_with_neighbor_tags,
  ...
  run_vm_tests("mmap_anon_fixed_with_neighbor_tags", __FILE__, &tests, argc,
               argv);
}
