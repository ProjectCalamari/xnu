...
#define SET_MEMLIMIT_TESTNAME                                                  \
  entitlement_debugging_increased_memory_limit_set_memlimit
#define CONVERT_MEMLIMIT_TESTNAME                                              \
  entitlement_debugging_increased_memory_limit_convert_memlimit_mb
#define LIMIT_WITH_SWAP_TESTNAME                                               \
  entitlement_debugging_increased_memory_limit_with_swap
#define EXTRA_META
#elif ENTITLED
#if BINCOMPAT
#define TESTNAME entitlement_increased_memory_limit_entitled_bincompat
#define SET_MEMLIMIT_TESTNAME                                                  \
  entitlement_increased_memory_limit_set_memlimit_bincompat
#define CONVERT_MEMLIMIT_TESTNAME                                              \
  entitlement_increased_memory_limit_convert_memlimit_mb_bincompat
#define LIMIT_WITH_SWAP_TESTNAME                                               \
  entitlement_increased_memory_limit_with_swap_bincompat
#else /* BINCOMPAT */
...
#define CONVERT_MEMLIMIT_TESTNAME                                              \
  entitlement_increased_memory_limit_convert_memlimit_mb
...
static void reset_old_entitled_max_task_mem(void) {
  int ret;
  size_t size_old_entitled_max_task_pmem = sizeof(old_entitled_max_task_pmem);
  // Use sysctl to change entitled limit
  ret =
      sysctlbyname(MAX_TASK_MEM_ENTITLED, NULL, 0, &old_entitled_max_task_pmem,
...
static pid_t spawn_child_with_memlimit(int32_t memlimit) {
  ...
  args = (char *[]){testpath, "-n", "child", NULL};

  ret = posix_spawnattr_setflags(&attr, POSIX_SPAWN_START_SUSPENDED);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "posix_spawnattr_setflags() failed");
  ret = posix_spawnattr_setjetsam_ext(&attr, 0, JETSAM_PRIORITY_FOREGROUND,
                                      memlimit, memlimit);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "posix_spawnattr_setjetsam_ext");
  ret = posix_spawn(&pid, testpath, NULL, &attr, args, *_NSGetEnviron());
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "posix_spawn() failed");

  return pid;
}

static void resume_child_and_verify_exit(pid_t pid) {
  pid_t rc;
  bool signaled;
  int status, ret;
...
T_DECL(TESTNAME,
       "Verify that entitled processes can allocate up to the entitled memory "
       "limit",
  ...
  ret =
      sysctlbyname(MAX_TASK_MEM, &max_task_pmem, &size_max_task_pmem, NULL, 0);
  T_ASSERT_POSIX_SUCCESS(ret,
                         "call sysctlbyname to get max task physical memory.");

  // Use sysctl to change entitled limit
  entitled_max_task_pmem = max_task_pmem + ENTITLED_EXTRA_MEM;
  ret = sysctlbyname(MAX_TASK_MEM_ENTITLED, &old_entitled_max_task_pmem,
                     &size_old_entitled_max_task_pmem, &entitled_max_task_pmem,
                     size_entitled_max_task_pmem);
  T_ASSERT_POSIX_SUCCESS(
      ret, "call sysctlbyname to set entitled hardware mem size.");
  T_ATEND(reset_old_entitled_max_task_mem);

  if (max_task_pmem >= old_entitled_max_task_pmem) {
    T_SKIP("max_task_pmem (%lld) is larger than entitled value (%lld). "
           "Skipping test on this device.",
           max_task_pmem, old_entitled_max_task_pmem);
  }
  ...
  // Check its memlimt
  ret = memorystatus_control(MEMORYSTATUS_CMD_GET_MEMLIMIT_PROPERTIES, pid, 0,
                             &mmprops, sizeof(mmprops));
...
T_DECL(SET_MEMLIMIT_TESTNAME,
  ...
  T_ASSERT_EQ(mmprops.v1.memlimit_active, entitled_max_task_pmem,
              "active limit");
  T_ASSERT_EQ(mmprops.v1.memlimit_inactive, entitled_max_task_pmem,
              "inactive limit");

  mmprops.v1.memlimit_active = -1;
  mmprops.v1.memlimit_inactive = -1;
  ret = memorystatus_control(MEMORYSTATUS_CMD_SET_MEMLIMIT_PROPERTIES, pid, 0,
                             &mmprops.v1, sizeof(mmprops.v1));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "memorystatus_control");
...
T_DECL(CONVERT_MEMLIMIT_TESTNAME,
  ...
  ret = memorystatus_control(MEMORYSTATUS_CMD_CONVERT_MEMLIMIT_MB, pid,
                             (uint32_t)-1, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "memorystatus_control");
  T_QUIET;
  T_ASSERT_EQ(ret, entitled_max_task_pmem, "got entitled value");

  resume_child_and_verify_exit(pid);
}

T_DECL(LIMIT_WITH_SWAP_TESTNAME,
       "entitled memory limit equals dram size when swap is enabled.",
  ...
  ret = sysctlbyname(MAX_TASK_MEM_ENTITLED, &entitled_max_task_pmem,
                     &size_entitled_max_task_pmem, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret,
                         "call sysctlbyname to get max task physical memory.");
  // Get the dram size. Convert it to MB since entitled_max_task_pmem is in MB
  // too and slight discrepancies can occur.
  ret = sysctlbyname("hw.memsize_physical", &memsize_physical,
                     &size_memsize_physical, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret,
                         "call sysctlbyname to get max task physical memory.");
  uint64_t memsize_physical_mb = (uint64_t)memsize_physical / (1ULL << 20);
  T_QUIET;
  T_ASSERT_EQ((uint64_t)entitled_max_task_pmem, memsize_physical_mb,
              "entitled limit == dram size");
}
#endif /* ENTITLED || ENTITLED_DEBUGGING */
