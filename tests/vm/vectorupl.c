...
T_DECL(vm_vector_upl, "Test for vector UPLs", T_META_TAG_VM_PREFERRED) {
  struct {
    uint64_t base;
    uint32_t len;
  } w_iovs[NUM_IOVS];
  ...
  for (w = 0; w < NUM_IOVS; w++) {
    ...
    void *iov_base;
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(posix_memalign(&iov_base, PAGE_SIZE, w_iovs[w].len),
  ...
  T_ASSERT_POSIX_SUCCESS(sysctlbyname("debug.test.vm_vector_upl", &result, &s,
                                      &addr, sizeof(addr)),
  ...
  /* Validate that the kernel sysctl handler mapped and mutated the page
   * contents as expected. */
  for (w = 0; w < NUM_IOVS; w++) {
    char *iov_base = (char *)w_iovs[w].base;
    for (w_idx = 0; w_idx < w_iovs[w].len; w_idx++) {
      T_QUIET;
      T_ASSERT_EQ(iov_base[w_idx], 'a' + w + 1,
                  "w_iovs[%d].iov_base[%d]='%c' == '%c'", w, w_idx,
                  (unsigned char)iov_base[w_idx], (unsigned char)('a' + w + 1));
    }
  }
