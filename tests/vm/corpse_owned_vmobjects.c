...
T_GLOBAL_META(T_META_NAMESPACE("xnu.memorystatus"),
              T_META_RADAR_COMPONENT_NAME("xnu"), T_META_OWNER("aaron_j_sonin"),
...
#pragma unused(exception_port, thread_id, task_id_token, exception, codes,     \
               codeCnt, flavor, old_state, old_state_count, new_state,         \
               new_state_count)
  T_FAIL("Unsupported catch_mach_exception_raise_state_identity");
  return KERN_NOT_SUPPORTED;
}
...
void verify_owned_vmobjects(task_t task) {
  int ret;
  size_t owned_vmobjects_len;

  ret = sysctlbyname(OWNED_VMOBJECTS_SYSCTL, NULL, &owned_vmobjects_len, &task,
                     sizeof(task));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl " OWNED_VMOBJECTS_SYSCTL);
  T_EXPECT_GT((int)owned_vmobjects_len, 0,
              "owned vmobjects list is populated on %s",
              (task == mach_task_self()) ? "self" : "corpse");
}
...
#pragma unused(thread, task, code, code_count)
  T_QUIET;
  T_EXPECT_TRUE(
      (exception == EXC_CRASH) || (exception == EXC_CORPSE_NOTIFY),
      "catch_mach_exception_raise() catches EXC_CRASH or EXC_CORPSE_NOTIFY");
...
T_HELPER_DECL(i_eat_memory_for_breakfast, "") {
  ...
  ret = mach_vm_allocate(mach_task_self(), &addr, vm_page_size,
                         VM_FLAGS_ANYWHERE | VM_FLAGS_PURGABLE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "allocate purgeable buffer");
  T_QUIET;
  T_ASSERT_NE((int)addr, 0, "purgeable buffer not null");
  verify_owned_vmobjects(mach_task_self());

  /* Allocate and touch all our pages */
  T_LOG("Allocating %d pages...", num_pages);
  buf = mmap(NULL, vm_page_size * num_pages, PROT_READ | PROT_WRITE,
             MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(buf, "mmap");
...
static pid_t launch_child(int num_pages) {
  extern char **environ;
  int ret;
  ...
  char *argv[5] = {testpath, "-n", "i_eat_memory_for_breakfast", num_pages_str,
                   NULL};
  ...
  if ((child_pid = fork()) == 0) {
    ...
    ret = posix_spawnattr_setjetsam_ext(
        &spawn_attrs, POSIX_SPAWN_JETSAM_MEMLIMIT_FATAL,
        JETSAM_PRIORITY_FOREGROUND, TEST_MEMLIMIT_MB, TEST_MEMLIMIT_MB);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "posix_spawnattr_setjetsam_ext");
...
void *exc_thread(void *arg) {
#pragma unused(arg)
  kern_return_t kr;

  while (1) {
    kr = mach_msg_server(mach_exc_server, MACH_MSG_SIZE_RELIABLE,
                         exception_port, 0);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "mach_msg_server");
  }
}
...
T_DECL(corpse_owned_vmobjects, "vm.get_owned_vmobjects sysctl on corpses",
  ...
  task = mach_task_self();
  T_QUIET;
  T_ASSERT_NE(task, MACH_PORT_NULL, "mach_task_self");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "mach_port_allocate");
  ret = mach_port_insert_right(task, exception_port, exception_port,
                               MACH_MSG_TYPE_MAKE_SEND);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "mach_port_insert_right");
  ret = task_set_exception_ports(task, EXC_MASK_CRASH | EXC_MASK_CORPSE_NOTIFY,
                                 exception_port,
                                 EXCEPTION_DEFAULT | MACH_EXCEPTION_CODES, 0);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "task_set_exception_ports");
  ...
  dispatch_semaphore_wait(sync_sema,
                          dispatch_time(DISPATCH_TIME_NOW, 1 * NSEC_PER_SEC));
  T_QUIET;
  T_EXPECT_EQ(caught_crash, true, "Caught EXC_CRASH");
  T_QUIET;
  T_EXPECT_EQ(caught_corpse, true, "Caught EXC_CORPSE_NOTIFY");
}
