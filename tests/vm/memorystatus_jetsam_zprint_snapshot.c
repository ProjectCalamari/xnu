 ...
 *
 * This test validates the ability of the kernel to generate a Jetsam zprint
 * snapshot
 *
 * Main flow:
 * 1. main process sets the kernel variable jzs_trigger_band to 0 to activate a
 * Jetsam zprint snapshot asap
 * 2. main process creates a child process to be Jetsem'd
 * 3. main process Jetsam the child process via memorystatus_control() for the
 * kernel to grab a Jetsam zprint snapshot
 * 4. main process periodically polls on the Jetsam zprint snapshot structures
 * via memorystatus_control()
 *
 * Result:
 *    Test is marked as PASS if the main process is able to obtain Jetsam zprint
 * snapshot structures successfully; marked as FAIL if the test times out before
 * it could obtain Jetsam zprint snapshot structures.
 */
...
static boolean_t jzs_data_available(void) {
  return (jzs_name_buffer && jzs_info_buffer && jzs_meminfo_buffer) ? TRUE
                                                                    : FALSE;
}

static const char *memorystatus_jzs_command_string(uint32_t command) {
  switch (command) {
...
static int collect_jzs_data(mach_zone_name_t **namep, mach_zone_info_t **infop,
                            mach_memory_info_t **memInfop) {
  ...
  for (unsigned int i = 0; i < JZS_COMMAND_COUNT; i++) {
    ...
    if (err == -1) {
      if (errno == EAGAIN) {
        ...
        T_LOG("memorystatus_control(%s ...) size_only failed",
              memorystatus_jzs_command_string(jzs_command));
      }
      return -1;
    }
    jzs_buffer_size = (size_t)
        err; // buffer size returned by memorystatus_control on a success call
    jzs_buffer = malloc(jzs_buffer_size);
    if (jzs_buffer == NULL) {
      T_LOG("malloc(%zu) failed for %s", jzs_buffer_size,
            memorystatus_jzs_command_string(jzs_command));
      return -1;
    }
    ...
    if (err == -1) {
      if (errno == EAGAIN) {
        ...
        T_LOG("memorystatus_control(%s ...) failed to fetch jetsam zprint "
              "snapshot data",
              memorystatus_jzs_command_string(jzs_command));
      }
      return -1;
  ...
  T_LOG("name_size %u, # of names %u", (unsigned)jzs_name_size,
        (unsigned)(jzs_name_size / sizeof(mach_zone_name_t)));
  T_LOG("info_size %u, # of zone infos %u", (unsigned)jzs_info_size,
        (unsigned)(jzs_info_size / sizeof(mach_zone_info_t)));
  T_LOG("meminfo_size %u, # of memory infos %u", (unsigned)jzs_meminfo_size,
        (unsigned)(jzs_meminfo_size / sizeof(mach_memory_info_t)));

  T_LOG("First zone name is '%s'", (*namep)[0].mzn_name);
  T_LOG("First zone count is %u", (unsigned)((*infop)[0].mzi_count));
  T_LOG("First tag size is %u name is '%s'", (unsigned)((*memInfop)[0].size),
        (*memInfop)[0].name);
...
static void compare_live_memory_info(void) {
  unsigned int nameCnt = 0;
  unsigned int infoCnt = 0;
  unsigned int wiredInfoCnt = 0;
  kern_return_t kr;

  kr = mach_memory_info(mach_host_self(), &live_name, &nameCnt, &live_info,
                        &infoCnt, &live_wiredInfo, &wiredInfoCnt);
  T_ASSERT_POSIX_SUCCESS(kr, "live mach_memory_info");

  // Log the live results of mach_memory_info to compare them.
  // Note, the contents won't match because live info is not necessarily
  // redacted, while zprint snapshot is.
  ...
  T_LOG("First tag size is %u name is '%s'", (unsigned)(live_wiredInfo[0].size),
        live_wiredInfo[0].name);
}

static void run_jzs_test(void) {
  pid_t child_pid;
  ...
  do {
    ...
    if (child_pid == 0) {
      ...
      T_LOG("Child process pid %d waiting to be Jetsam'd (sleep for %llu "
            "seconds)",
            getpid(), test_timeout_in_nanoseconds / NSEC_PER_SEC);
    ...
    err = memorystatus_control(MEMORYSTATUS_CMD_TEST_JETSAM, child_pid, 0, NULL,
                               0);
    T_EXPECT_NE_INT(err, -1,
                    " memorystatus_control(MEMORYSTATUS_CMD_TEST_JETSAM, %d, "
                    "...) returned %d",
                    child_pid, err);

    while (0 == waitpid(child_pid, &status, WNOHANG)) {
      if (jzs_data_available() ||
          collect_jzs_data(&jzs_name_buffer, &jzs_info_buffer,
                           &jzs_meminfo_buffer) == 0) {
  ...
  if (jzs_data_available()) {
    T_PASS("Copied jetsam zprint snapshot after %u run%s", retries,
           (retries == 1) ? "" : "s");
  } else {
    T_FAIL("Failed to copy jetsam zprint snapshot after %u run%s", retries,
           (retries == 1) ? "" : "s");
  }
}

T_DECL(memorystatus_jetsam_zprint_snapshot, "Test for zprint snapshot") {
  int err;
  ...
  err = sysctlbyname("kern.jzs_trigger_band", NULL, NULL,
                     &jzs_trigger_band_target, sizeof(jzs_trigger_band_target));
  if (err != 0 && errno == ENOENT) {
    /* No such file or directory, the running kernel doesn't know about jetsam
     * zprint snapshot, skip the test*/
    T_SKIP("The running kernel doesn't know about jetsam zprint snapshot");
  }
  T_ASSERT_POSIX_SUCCESS(
      err, "sysctlbyname(kern.jzs_trigger_band) set jzs_trigger_band to %d",
      jzs_trigger_band_target);
  ...
  T_EXPECT_EQ_INT(1, jzs_data_available(),
                  "Test finished: jzs_data_available()");
