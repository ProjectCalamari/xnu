...
static bool do_successful_vm_deallocate_guarded(mach_vm_address_t start,
                                                mach_vm_size_t size) {
...
static bool do_vm_deallocate_holes_guarded(mach_vm_address_t start,
                                           mach_vm_size_t size) {
  __block kern_return_t kr;
  exc_guard_helper_info_t exc_info;
  bool caught_exception;
  ...
  if (exc_info.guard_flavor != kGUARD_EXC_DEALLOC_GAP) {
    T_EXPECT_EQ(exc_info.guard_flavor, kGUARD_EXC_DEALLOC_GAP,
                "caught exception flavor");
    return false;
  }
...
static test_result_t successful_vm_deallocate(checker_list_t *checker_list,
...
successful_vm_deallocate_guarded(checker_list_t *checker_list,
...
static test_result_t vm_deallocate_holes_guarded(checker_list_t *checker_list,
                                                 mach_vm_address_t start,
                                                 mach_vm_size_t size) {
  checker_perform_successful_vm_deallocate(checker_list, start, size);
...
static test_result_t vm_deallocate_permanent_entry(checker_list_t *checker_list,
                                                   mach_vm_address_t start,
                                                   mach_vm_size_t size) {
  kern_return_t kr;
...
static test_result_t
vm_deallocate_permanent_before_permanent(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
  kern_return_t kr;
...
static test_result_t
vm_deallocate_permanent_before_allocation(checker_list_t *checker_list,
                                          mach_vm_address_t start,
                                          mach_vm_size_t size) {
  kern_return_t kr;
...
static test_result_t
vm_deallocate_permanent_before_hole(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size) {
  kern_return_t kr;
...
static test_result_t
vm_deallocate_permanent_after_allocation(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
  kern_return_t kr;
...
static test_result_t
vm_deallocate_permanent_after_hole(checker_list_t *checker_list,
                                   mach_vm_address_t start,
                                   mach_vm_size_t size) {
  kern_return_t kr;
...
static test_result_t
vm_deallocate_permanent_entry_guarded(checker_list_t *checker_list,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
  checker_perform_vm_deallocate_permanent(checker_list, start, size);
  if (!do_successful_vm_deallocate_guarded(start, size)) {
...
static test_result_t
vm_deallocate_permanent_before_permanent_guarded(checker_list_t *checker_list,
                                                 mach_vm_address_t start,
                                                 mach_vm_size_t size) {
  checker_perform_vm_deallocate_permanent(checker_list, start, size / 2);
  checker_perform_vm_deallocate_permanent(checker_list, start + size / 2,
...
static test_result_t
vm_deallocate_permanent_before_allocation_guarded(checker_list_t *checker_list,
                                                  mach_vm_address_t start,
                                                  mach_vm_size_t size) {
  checker_perform_vm_deallocate_permanent(checker_list, start, size / 2);
  checker_perform_successful_vm_deallocate(checker_list, start + size / 2,
...
static test_result_t
vm_deallocate_permanent_before_hole_guarded(checker_list_t *checker_list,
                                            mach_vm_address_t start,
                                            mach_vm_size_t size) {
  checker_perform_vm_deallocate_permanent(checker_list, start, size / 2);
  /* no changes to checkers in [start + size / 2, start + size) */
...
static test_result_t
vm_deallocate_permanent_after_allocation_guarded(checker_list_t *checker_list,
                                                 mach_vm_address_t start,
                                                 mach_vm_size_t size) {
  checker_perform_successful_vm_deallocate(checker_list, start, size / 2);
  checker_perform_vm_deallocate_permanent(checker_list, start + size / 2,
...
static test_result_t
vm_deallocate_permanent_after_hole_guarded(checker_list_t *checker_list,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size) {
  /* no changes to checkers in [start, start + size / 2) */
  checker_perform_vm_deallocate_permanent(checker_list, start + size / 2,
...
T_DECL(vm_deallocate_guarded,
  ...
  if (isRosetta()) {
    /* Rosetta doesn't deliver VM guard exceptions to the test's exception
     * handler. */
