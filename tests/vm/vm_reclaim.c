...
#define T_META_VM_RECLAIM_ENABLED                                              \
  T_META_SYSCTL_INT("vm.reclaim.max_threshold=268435456")
#define T_META_VM_RECLAIM_DISABLED                                             \
  T_META_SYSCTL_INT("vm.reclaim.max_threshold=0")
#else // !TARGET_OS_IOS
...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm_reclaim"),
              ...
              T_META_RUN_CONCURRENTLY(false), T_META_CHECK_LEAKS(false));

static mach_vm_reclaim_ring_t ringbuffer_init(void) {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_reclaim_ring_allocate()");
  return ringbuffer;
}
...
T_DECL(vm_reclaim_init, "Set up and tear down a reclaim buffer",
  ...
  T_EXPECT_EQ(os_atomic_load(&ringbuffer->head, relaxed), 0ull,
              "head is zeroed");
  T_EXPECT_EQ(os_atomic_load(&ringbuffer->busy, relaxed), 0ull,
              "busy is zeroed");
  T_EXPECT_EQ(os_atomic_load(&ringbuffer->tail, relaxed), 0ull,
              "tail is zeroed");
  size_t expected_len =
      (vm_page_size - offsetof(struct mach_vm_reclaim_ring_s, entries)) /
  ...
  for (unsigned i = 0; i < ringbuffer->len; i++) {
    ...
    T_QUIET;
    T_EXPECT_EQ(entry->address, 0ull, "address is zeroed");
    T_QUIET;
    T_EXPECT_EQ(entry->size, 0u, "size is zeroed");
    T_QUIET;
    T_EXPECT_EQ(entry->behavior, 0, "behavior is zeroed");
  }
}

T_DECL(vm_reclaim_init_fails_when_disabled,
       "Initializing a ring buffer on a system with vm_reclaim disabled should "
       "fail",
  ...
  T_EXPECT_MACH_ERROR(kr, VM_RECLAIM_NOT_SUPPORTED,
                      "mach_vm_reclaim_ring_allocate()");
}

static bool try_cancel(mach_vm_reclaim_ring_t ringbuffer,
                       mach_vm_reclaim_id_t id, mach_vm_address_t addr,
                       mach_vm_size_t size, mach_vm_reclaim_action_t behavior) {
  bool update_accounting;
  mach_vm_reclaim_state_t state;
  kern_return_t kr;
  kr = mach_vm_reclaim_try_cancel(ringbuffer, id, addr, size, behavior, &state,
                                  &update_accounting);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_reclaim_try_cancel()");
  if (update_accounting) {
    kern_return_t tmp_kr = mach_vm_reclaim_update_kernel_accounting(ringbuffer);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(tmp_kr, "mach_vm_reclaim_update_kernel_accounting()");
  }
  return mach_vm_reclaim_is_reusable(state);
}

/*
 * Allocate a buffer of the given size, write val to each byte, and free it via
 * a deferred free call.
 */
...
allocate_and_defer_free(size_t size, mach_vm_reclaim_ring_t ringbuffer,
  ...
  kern_return_t kr = mach_vm_map(
      mach_task_self(), addr, size, 0, VM_FLAGS_ANYWHERE, MEMORY_OBJECT_NULL, 0,
  ...
  kr = mach_vm_reclaim_try_enter(ringbuffer, *addr, size, behavior, &id,
                                 &should_update_kernel_accounting);
  if (should_update_kernel_accounting) {
    kr = mach_vm_reclaim_update_kernel_accounting(ringbuffer);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "mach_vm_reclaim_update_kernel_accounting()");
  }
  return id;
}

static mach_vm_reclaim_id_t
allocate_and_defer_deallocate(size_t size, mach_vm_reclaim_ring_t ringbuffer,
                              unsigned char val,
                              mach_vm_address_t *addr /* OUT */) {
  return allocate_and_defer_free(size, ringbuffer, val, VM_RECLAIM_DEALLOCATE,
                                 addr);
}

T_DECL(vm_reclaim_single_entry,
       "Place a single entry in the buffer and call sync",
       T_META_VM_RECLAIM_ENABLED, T_META_TAG_VM_PREFERRED) {
  static const size_t kAllocationSize = (1UL << 20); // 1MB
  mach_vm_address_t addr;
  mach_vm_reclaim_ring_t ringbuffer = ringbuffer_init();

  mach_vm_reclaim_id_t idx =
      allocate_and_defer_deallocate(kAllocationSize, ringbuffer, 1, &addr);
  T_QUIET;
  T_ASSERT_EQ(idx, 0ULL, "Entry placed at start of buffer");
  mach_vm_reclaim_ring_flush(ringbuffer, 1);
}

static pid_t spawn_helper(char *helper) {
  ...
  launch_tool_args = (char *[]){testpath, "-n", helper, NULL};
...
static int spawn_helper_and_wait_for_exit(char *helper) {
  int status;
  pid_t child_pid, rc;
 ...
 */
static bool check_buffer(mach_vm_address_t addr, size_t size,
 ...
 */
static void read_buffer(mach_vm_address_t addr, size_t size) {
 ...
 * This will likely crash, but if we make it through the entire buffer then
 * segfault on purpose.
 */
static void assert_buffer_has_changed_and_crash(mach_vm_address_t addr,
                                                size_t size,
                                                unsigned char expected) {
  /*
   ...
   * with the value we wrote to it. So scan the buffer. If we segfault it's case
   * #1 and if we see another value it's case #2.
   */
  bool changed = !check_buffer(addr, size, expected);
  T_QUIET;
  T_ASSERT_TRUE(changed, "buffer was re-allocated");
...
static void reuse_reclaimed_entry(mach_vm_reclaim_action_t behavior) {
  kern_return_t kr;
  static const size_t kAllocationSize = (1UL << 20); // 1MB
  mach_vm_address_t addr;
  ...
  mach_vm_reclaim_id_t idx = allocate_and_defer_free(
      kAllocationSize, ringbuffer, kValue, behavior, &addr);
  T_QUIET;
  T_ASSERT_EQ(idx, 0ULL, "Entry placed at start of buffer");
...
T_DECL(vm_reclaim_single_entry_verify_free,
       "Place a single entry in the buffer and call sync",
  ...
  T_QUIET;
  T_ASSERT_TRUE(WIFSIGNALED(status), "Test process crashed.");
  T_QUIET;
  T_ASSERT_EQ(WTERMSIG(status), SIGSEGV,
              "Test process crashed with segmentation fault.");
}
...
T_DECL(vm_reclaim_single_entry_reusable,
  ...
  T_QUIET;
  T_ASSERT_TRUE(WIFEXITED(status), "Test process exited.");
  T_QUIET;
  T_ASSERT_EQ(WEXITSTATUS(status), 0, "Test process exited cleanly.");
}

static void allocate_and_suspend(char *const *argv, bool free_buffer,
                                 bool double_free) {
  kern_return_t kr;
  ...
  const mach_vm_reclaim_count_t kNumEntries =
      (mach_vm_reclaim_count_t)atoi(argv[0]);
  mach_vm_reclaim_count_t capacity;
  kr = mach_vm_reclaim_ring_capacity(ringbuffer, &capacity);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_reclaim_ring_capacity()");
  T_QUIET;
  T_ASSERT_LT(kNumEntries, capacity, "Test does not fill up ringbuffer");

  T_LOG("allocate_and_suspend: Allocating and freeing %u entries...",
        kNumEntries);
  for (size_t i = 0; i < kNumEntries; i++) {
    addr = 0;
    mach_vm_reclaim_id_t idx = allocate_and_defer_deallocate(
        kAllocationSize, ringbuffer, (unsigned char)i, &addr);
    T_QUIET;
    T_ASSERT_EQ(idx, (mach_vm_reclaim_id_t)i, "idx is correct");
  ...
  if (double_free) {
    ...
    mach_vm_reclaim_id_t id = VM_RECLAIM_ID_NULL;
    kr = mach_vm_reclaim_try_enter(ringbuffer, addr, kAllocationSize,
                                   VM_RECLAIM_DEALLOCATE, &id,
                                   &should_update_kernel_accounting);
    T_QUIET;
  ...
  if (free_buffer) {
    mach_vm_size_t buffer_size =
        (size_t)capacity * sizeof(struct mach_vm_reclaim_entry_s) +
        offsetof(struct mach_vm_reclaim_ring_s, entries);
    kr = mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)ringbuffer,
                            buffer_size);
    T_QUIET;
...
static void wait_for_pid_to_be_drained(pid_t child_pid) {
  int val = child_pid;
  int ret;
  size_t len = sizeof(val);
  ret = sysctlbyname("vm.reclaim.wait_for_pid", NULL, NULL, &val, len);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "vm.reclaim.wait_for_pid");
}

static size_t ledger_phys_footprint_index(size_t *num_entries) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "ledger(LEDGER_INFO)");

  T_QUIET;
  T_ASSERT_GT(li.li_entries, (int64_t)0, "num ledger entries is valid");
  *num_entries = (size_t)li.li_entries;
  templateInfo =
      malloc((size_t)li.li_entries * sizeof(struct ledger_template_info));
  T_QUIET;
  T_ASSERT_NOTNULL(templateInfo, "malloc entries");

  footprint_index = 0;
  ret = ledger(LEDGER_TEMPLATE_INFO, (caddr_t)templateInfo,
               (caddr_t)num_entries, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "ledger(LEDGER_TEMPLATE_INFO)");
  ...
  free(templateInfo);
  T_QUIET;
  T_ASSERT_TRUE(found, "found phys_footprint in ledger");
  return footprint_index;
}

static int64_t get_ledger_entry_for_pid(pid_t pid, size_t index,
                                        size_t num_entries) {
  int ret;
  int64_t value;
  ...
  ret = ledger(LEDGER_ENTRY_INFO, (caddr_t)(uintptr_t)pid, (caddr_t)lei,
               (caddr_t)&num_entries);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "ledger(LEDGER_ENTRY_INFO)");
...
static void test_after_background_helper_launches(char *variant, char *arg1,
                                                  dispatch_block_t test_block,
                                                  dispatch_block_t exit_block) {
  char **launch_tool_args;
  char testpath[PATH_MAX];
  uint32_t testpath_buf_size;
  ...
  T_ASSERT_POSIX_ZERO(ret, "_NSGetExecutablePath");
  T_LOG("Executable path: %s", testpath);
  launch_tool_args = (char *[]){testpath, "-n", variant, arg1, NULL};
...
T_DECL(vm_reclaim_full_reclaim_on_suspend,
       "Defer free memory and then suspend.", T_META_ASROOT(true),
       T_META_VM_RECLAIM_ENABLED, T_META_TAG_VM_PREFERRED) {
  test_after_background_helper_launches(
      "allocate_and_suspend", "20",
      ^{
        int ret = 0;
        size_t num_ledger_entries = 0;
        size_t phys_footprint_index =
            ledger_phys_footprint_index(&num_ledger_entries);
        int64_t before_footprint, after_footprint,
            reclaimable_bytes = 20 * (1ULL << 20);
        before_footprint = get_ledger_entry_for_pid(
            child_pid, phys_footprint_index, num_ledger_entries);
        T_QUIET;
        T_EXPECT_GE(before_footprint, reclaimable_bytes,
         ...
         * So we need to call into the kernel to synchronize with the reclaim
         * worker thread.
         */
        wait_for_pid_to_be_drained(child_pid);
        after_footprint = get_ledger_entry_for_pid(
            child_pid, phys_footprint_index, num_ledger_entries);
        T_QUIET;
        T_EXPECT_LE(after_footprint, before_footprint - reclaimable_bytes,
...
T_DECL(vm_reclaim_limit_kills,
       "Deferred reclaims are processed before a limit kill",
       T_META_VM_RECLAIM_ENABLED, T_META_TAG_VM_PREFERRED) {
  int err;
  ...
  err = set_memlimits(getpid(), kMemoryLimit >> 20, kMemoryLimit >> 20, TRUE,
                      TRUE);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(err, "set_memlimits");
  ...
  T_PASS("Was able to allocate and defer free %zu chunks of size %zu bytes "
         "while staying under limit of %zu bytes",
         kNumEntries, kAllocationSize, kMemoryLimit);
}

#if TARGET_OS_IOS && !TARGET_OS_VISION
T_DECL(vm_reclaim_update_reclaimable_bytes_threshold,
       "Kernel reclaims when num_bytes_reclaimable crosses threshold",
  ...
  ret = sysctlbyname("vm.reclaim.max_threshold",
                     &vm_reclaim_reclaimable_max_threshold, &len, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "vm.reclaim.max_threshold");
  kNumEntries = (mach_vm_reclaim_count_t)(vm_reclaim_reclaimable_max_threshold /
                                          kAllocationSize * 1000);
  mach_vm_reclaim_count_t capacity;
  err = mach_vm_reclaim_ring_capacity(ringbuffer, &capacity);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(err, "mach_vm_reclaim_ring_capacity()");
  T_QUIET;
  T_ASSERT_LT(kNumEntries, capacity, "Test does not fill up ringbuffer");
  ...
  for (uint64_t i = 0; i < kNumEntries; i++) {
    mach_vm_reclaim_id_t idx = allocate_and_defer_deallocate(
        kAllocationSize, ringbuffer, (unsigned char)i, &addr);
    T_QUIET;
    T_ASSERT_EQ(idx, i, "idx is correct");
  }

  T_QUIET;
  T_ASSERT_LT(get_ledger_entry_for_pid(getpid(), phys_footprint_index,
                                       num_ledger_entries),
              (int64_t)((kNumEntries)*kAllocationSize),
              "Entries were reclaimed as we crossed threshold");
}
...
T_DECL(vm_reclaim_copyio_buffer_error, "Force a copyio error on the buffer",
       T_META_IGNORECRASHES(".*deallocate_buffer.*"), T_META_VM_RECLAIM_ENABLED,
       T_META_TAG_VM_PREFERRED) {
  int status = spawn_helper_and_wait_for_exit("deallocate_buffer");
  T_QUIET;
  T_ASSERT_TRUE(WIFSIGNALED(status), "Test process crashed.");
  T_QUIET;
  T_ASSERT_EQ(WTERMSIG(status), SIGKILL, "Test process crashed with SIGKILL.");
}
...
T_HELPER_DECL(dealloc_gap, "Put a bad entry in the buffer") {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "task_set_exc_guard_behavior()");
  ...
  kr = mach_vm_reclaim_try_enter(ringbuffer, addr, kAllocationSize,
                                 VM_RECLAIM_DEALLOCATE, &idx,
                                 &should_update_kernel_accounting);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_reclaim_try_enter()");
  T_QUIET;
  T_ASSERT_EQ(idx, 1ULL, "Entry placed at correct index");
...
T_DECL(vm_reclaim_dealloc_gap,
       "Ensure a dealloc gap delivers a fatal exception",
       T_META_IGNORECRASHES(".*dealloc_gap.*"), T_META_VM_RECLAIM_ENABLED,
       T_META_TAG_VM_PREFERRED) {
...
T_HELPER_DECL(allocate_and_suspend_with_dealloc_gap,
  ...
  kern_return_t kr =
      task_set_exc_guard_behavior(mach_task_self(), TASK_EXC_GUARD_ALL);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "task_set_exc_guard_behavior()");
  allocate_and_suspend(argv, false, true);
}

static void vm_reclaim_async_exception(char *variant, char *arg1) {
  test_after_background_helper_launches(
      variant, arg1,
      ^{
        int ret = 0;
      ...
      ^{
        ...
        T_QUIET;
        T_EXPECT_FALSE(exited, "waitpid");
        T_QUIET;
        T_EXPECT_FALSE(status, "Test process crashed.");
        T_QUIET;
        T_EXPECT_EQ(signal, SIGKILL, "Test process crashed with SIGKILL.");
        T_END;
      });
}

T_DECL(vm_reclaim_dealloc_gap_async,
       "Ensure a dealloc gap delivers an async fatal exception",
...
T_DECL(vm_reclaim_copyio_buffer_error_async,
       "Ensure a buffer copyio failure delivers an async fatal exception",
...
T_HELPER_DECL(reuse_freed_entry_fork,
  ...
  T_ASSERT_MACH_ERROR(kr, VM_RECLAIM_RESOURCE_SHORTAGE,
                      "mach_vm_reclaim_ring_allocate() should fail");
  ...
  kr = mach_vm_reclaim_try_enter(buffer_4fork_inherit, addr_4fork_inherit,
                                 allocation_size_4fork_inherit,
                                 VM_RECLAIM_DEALLOCATE, &id, &update);
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_reclaim_try_enter()");
...
T_DECL(inherit_buffer_after_fork,
       "Ensure reclaim buffer is inherited across a fork",
       T_META_IGNORECRASHES(".*vm_reclaim_fork.*"), T_META_VM_RECLAIM_ENABLED,
       T_META_TAG_VM_PREFERRED) {
  ...
  mach_vm_reclaim_id_t idx = allocate_and_defer_deallocate(
      allocation_size_4fork_inherit, buffer_4fork_inherit, value_4fork_inherit,
      &addr_4fork_inherit);
  T_QUIET;
  T_ASSERT_EQ(idx, 0ULL, "Entry placed at start of buffer");
...
T_DECL(reclaim_async_on_repeated_suspend,
       "verify that subsequent suspends are allowed", T_META_VM_RECLAIM_ENABLED,
       T_META_TAG_VM_PREFERRED) {
  ...
  test_after_background_helper_launches(
      ...
      ^{
        ...
        T_LOG("Killing child...");
        T_QUIET;
        T_ASSERT_POSIX_SUCCESS(kill(child_pid, SIGKILL), "kill()");
      },
      ^{
        int status;
        pid_t rc = waitpid(child_pid, &status, 0);
...
T_HELPER_DECL(buffer_init_after_exec, "initialize a ringbuffer after exec") {
  mach_vm_reclaim_ring_t ringbuffer;
  kern_return_t kr = mach_vm_reclaim_ring_allocate(&ringbuffer, 1, 1);
...
T_DECL(reclaim_exec_new_reclaim_buffer,
  ...
  T_ASSERT_POSIX_ZERO(ret, "_NSGetExecutablePath");
  T_LOG("Executable path: %s", testpath);
  launch_tool_args = (char *[]){testpath, "-n", "buffer_init_after_exec", NULL};

  /* Spawn the child process. */
  posix_spawnattr_t spawnattrs;
  posix_spawnattr_init(&spawnattrs);
  posix_spawnattr_setflags(&spawnattrs, POSIX_SPAWN_SETEXEC);
  posix_spawn(&child_pid, testpath, NULL, &spawnattrs, launch_tool_args,
              environ);
  T_ASSERT_FAIL("should not be reached");
}

T_DECL(resize_buffer, "verify that a reclaim buffer may be safely resized",
       T_META_VM_RECLAIM_ENABLED, T_META_TAG_VM_PREFERRED) {
  kern_return_t kr;
  mach_vm_reclaim_ring_t ringbuffer;
  ...
  for (mach_vm_reclaim_count_t i = 0; i < old_capacity; i++) {
    ids[i] =
        allocate_and_defer_deallocate(vm_page_size, ringbuffer, 'A', &addrs[i]);
    T_QUIET;
    T_ASSERT_NE(ids[i], VM_RECLAIM_ID_NULL, "Able to defer deallocation");
  }
  id_tmp =
      allocate_and_defer_deallocate(vm_page_size, ringbuffer, 'X', &addr_tmp);
  ...
  T_ASSERT_EQ(initial_tail, (uint64_t)old_capacity,
              "tail == capacity after fill");

  T_LOG("Resizing buffer to 4x");
  kr = mach_vm_reclaim_ring_resize(ringbuffer, max_len);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "mach_vm_reclaim_ring_resize()");

  // All entries should be reclaimed after resize
  T_EXPECT_EQ(os_atomic_load(&ringbuffer->head, relaxed), initial_tail,
              "head is incremented");
  T_EXPECT_EQ(os_atomic_load(&ringbuffer->busy, relaxed), initial_tail,
              "busy is incremented");
  T_EXPECT_EQ(os_atomic_load(&ringbuffer->tail, relaxed), initial_tail,
              "tail is preserved");
  ...
  for (mach_vm_reclaim_count_t i = 0; i < old_capacity; i++) {
    mach_vm_reclaim_state_t state;
    kr = mach_vm_reclaim_query_state(ringbuffer, ids[i], VM_RECLAIM_DEALLOCATE,
                                     &state);
    bool reclaimed = !(state == VM_RECLAIM_UNRECLAIMED);
    T_QUIET;
    T_EXPECT_TRUE(reclaimed, "Entry is reclaimed after resize");
    bool usable = try_cancel(ringbuffer, ids[i], addrs[i], vm_page_size,
                             VM_RECLAIM_DEALLOCATE);
    T_QUIET;
    T_EXPECT_FALSE(usable, "Entry cannot be re-used after resize");
  }

  T_LOG("Filling resized buffer");
  for (mach_vm_reclaim_count_t i = 0; i < new_capacity; i++) {
    ids[i] =
        allocate_and_defer_deallocate(vm_page_size, ringbuffer, 'B', &addrs[i]);
    T_QUIET;
    T_ASSERT_NE(ids[i], VM_RECLAIM_ID_NULL, "Able to defer deallocation");
  }
  id_tmp =
  ...
  for (mach_vm_reclaim_count_t i = 0; i < new_capacity; i++) {
    bool usable = try_cancel(ringbuffer, ids[i], addrs[i], vm_page_size,
                             VM_RECLAIM_DEALLOCATE);
    T_QUIET;
    T_EXPECT_TRUE(usable, "Entry is available for re-use");
  }
}

T_DECL(resize_after_drain, "resize a buffer after draining it",
       T_META_VM_RECLAIM_ENABLED, T_META_TAG_VM_PREFERRED) {
  int ret;
  ...
  ret = sysctlbyname("vm.reclaim.sampling_period_ns", &sampling_period_ns,
                     &sampling_period_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl(vm.reclaim.sampling_period_ns)");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(err, "mach_vm_reclaim_ring_allocate()");
  ...
  for (mach_vm_reclaim_count_t i = 0; i < 128; i++) {
    ...
    mach_vm_reclaim_id_t id =
        allocate_and_defer_deallocate(vm_page_size, ring, 'A', &addr);
    T_QUIET;
    T_ASSERT_NE(id, VM_RECLAIM_ID_NULL, "Able to defer deallocation");
  }
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl(vm.reclaim.drain_pid)");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "nanosleep()");
...
static void kill_child() { kill(child_pid, SIGKILL); }

kern_return_t mach_vm_deferred_reclamation_buffer_remap(
    task_t source_task, task_t dest_task, mach_vm_address_t addr,
    mach_vm_address_t *addr_u, mach_vm_size_t *size_u);
...
T_DECL(copy_and_query_buffer,
  ...
  for (mach_vm_reclaim_count_t i = 0; i < QUERY_BUFFER_RING_COUNT; i++) {
    ...
    addrs[i] = 0;
    ids[i] = allocate_and_defer_free(sizes[i], self_ring, 'A', actions[i],
                                     &addrs[i]);
    T_QUIET;
    T_ASSERT_NE(ids[i], VM_RECLAIM_ID_NULL, "Able to defer allocation");
  }
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(rr, "query reclaim ring size");
  T_ASSERT_EQ(query_count, QUERY_BUFFER_RING_COUNT,
              "correct reclaim ring query size");

  rr = mach_vm_reclaim_copied_ring_query(&copied_ring, query_buffer,
                                         &query_count);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(rr, "query reclaim ring");
  T_ASSERT_EQ(query_count, QUERY_BUFFER_RING_COUNT, "query count is correct");

  bool all_match = true;
  for (mach_vm_reclaim_count_t i = 0; i < QUERY_BUFFER_RING_COUNT; i++) {
    mach_vm_reclaim_region_t qentry = &query_buffer[i];
    if ((qentry->vmrr_addr != addrs[i]) || (qentry->vmrr_size != sizes[i]) ||
        (qentry->vmrr_behavior != actions[i])) {
      all_match = false;
    }
    T_QUIET;
    T_EXPECT_EQ(qentry->vmrr_addr, addrs[i], "query->vmrr_addr is correct");
    T_QUIET;
    T_EXPECT_EQ(qentry->vmrr_size, sizes[i], "query->vmrr_size is correct");
    T_QUIET;
    T_EXPECT_EQ(qentry->vmrr_behavior, actions[i],
                "query->vmrr_behavior is correct");
  }
  ...
  rr = mach_vm_reclaim_copied_ring_query(&copied_ring, query_buffer,
                                         &query_count);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(rr, "query reclaim ring with small buffer");
  T_ASSERT_EQ(query_count, 5,
              "query reclaim ring with small buffer returns correct size");
