...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm.perf"),
              ...
              T_META_RADAR_COMPONENT_VERSION("VM"), T_META_OWNER("jarrad"),
...
T_DECL(mlock_external, "File-backed wire microbenchmark",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd, "open()");
  buf = mmap(NULL, vmsize, PROT_READ, (MAP_FILE | MAP_SHARED), fd, 0);
  T_QUIET;
  T_ASSERT_NOTNULL(buf, "mmap()");
  ...
  while (!dt_stat_stable(mlock_time)) {
    T_STAT_MEASURE(mlock_time) { ret = mlock(buf, vmsize); }
    T_QUIET;
