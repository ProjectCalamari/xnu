...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_CHECK_LEAKS(false), T_META_ASROOT(YES));
...
T_DECL(read_only_zone_test, "Read-only zalloc test", T_META_TAG_VM_PREFERRED) {
  T_EXPECT_EQ(1ull, run_sysctl_test("zone_ro_basic_test", 0),
              "zone_ro_basic_test");
}
...
T_DECL(zone_gc_stress_test, "stress test for zone_gc",
       T_META_TAG_VM_PREFERRED) {
  T_EXPECT_EQ(1ull, run_sysctl_test("zone_gc_stress_test", 10),
              "zone_gc_stress_test");
}
...
T_DECL(zlog_smoke_test, "check that zlog and zone tagging function at all",
  ...
  if (WIFEXITED(status)) {
    T_LOG("waitpid for %d returned with status %d", pid, WEXITSTATUS(status));
  } else {
    int sig = WTERMSIG(status);
    T_LOG("waitpid for %d killed by signal %d/%s", pid, sig, sys_signame[sig]);
  }
  T_ASSERT_TRUE(WIFEXITED(status) && WEXITSTATUS(status) == 0,
                "zlog exited cleanly");

  /* work around rdar://84948713 */
  T_ASSERT_EQ(dispatch_semaphore_wait(
                  sema, dispatch_time(DISPATCH_TIME_NOW, NSEC_PER_SEC)),
              0L, "found the line we wanted");
