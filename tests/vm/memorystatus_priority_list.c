...
T_GLOBAL_META(T_META_NAMESPACE("xnu.memorystatus"),
              ...
              T_META_RADAR_COMPONENT_VERSION("VM"), T_META_ASROOT(true),
...
get_priority_list(pid_t pid, int call, size_t entry_size, ssize_t *len) {
  ...
  for (i = 0; i < MAX_TRIES; i++) {
    ...
    T_ASSERT_EQ(size % entry_size, 0ul,
                "Priority list size is multiple of struct size");
...
void validate_entry(memorystatus_priority_entry_v2_t *entry, size_t entry_size,
                    pid_t expect_pid) {
  int i;
  if (expect_pid == -1) {
    T_QUIET;
    T_ASSERT_GE(entry->pid, 0, "PID valid");
    T_QUIET;
    T_ASSERT_NE(entry->pid, 0, "kernel_task not in list");
    T_QUIET;
    T_ASSERT_NE(entry->pid, 1, "launchd not in list");
  } else {
    T_QUIET;
    T_ASSERT_EQ(entry->pid, expect_pid, "PID correct");
  }

  if (entry->pid > 1) {
    T_QUIET;
    T_ASSERT_GE(entry->priority, 0, "Entry priority >= 0");
    T_QUIET;
    T_ASSERT_LE(
        entry->priority, JETSAM_PRIORITY_MAX,
        "Entry priority <= JETSAM_PRIORITY_MAX (" STR(JETSAM_PRIORITY_MAX) ")");
  } else {
    T_QUIET;
    T_ASSERT_EQ(entry->priority, JETSAM_PRIORITY_INTERNAL,
                "Entry priority == JETSAM_PRIORITY_INTERNAL (" STR(
                    JETSAM_PRIORITY_INTERNAL) ")");
  }

  if (entry_size == sizeof(memorystatus_priority_entry_v2_t)) {
    boolean_t found_nonzero = false;
    for (i = 0; i < sizeof(entry->_reserved) / sizeof(entry->_reserved[0]);
         i++) {
    ...
    T_QUIET;
    T_ASSERT_FALSE(found_nonzero, "Entry reserved is empty");
  }
}
...
T_DECL(jetsam_priority_list_v2_list, "Jetsam priority list v2 - list") {
  ...
  memorystatus_priority_entry_v2_t *list = get_priority_list(
      0, MEMORYSTATUS_CMD_GET_PRIORITY_LIST_V2, sizeof(*list), &len);
...
void validate_single(pid_t pid) {
  ssize_t len;
  memorystatus_priority_entry_v2_t *list = get_priority_list(
      pid, MEMORYSTATUS_CMD_GET_PRIORITY_LIST_V2, sizeof(*list), &len);

  T_ASSERT_NOTNULL(list, "Getting self priority entry");
  T_ASSERT_EQ(len, sizeof(memorystatus_priority_entry_v2_t),
...
T_DECL(jetsam_priority_list_compat, "Jetsam priority list - v1 compat") {
  ...
  memorystatus_priority_entry_t *list =
      (memorystatus_priority_entry_t *)get_priority_list(
          0, MEMORYSTATUS_CMD_GET_PRIORITY_LIST, sizeof(*list), &len);

  T_ASSERT_NOTNULL(list, "Priority list not null");

  for (i = 0; i < len / sizeof(memorystatus_priority_entry_v2_t); i++) {
    validate_entry((memorystatus_priority_entry_v2_t *)(&list[i]),
                   sizeof(*list), -1);
  }
