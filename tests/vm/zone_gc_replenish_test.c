...
static void *gc_thread_func(__unused void *arg) {
...
static void *alloc_thread_func(__unused void *arg) {
  time_t start = time(NULL);
  size_t n = 0;
...
T_DECL(zone_gc_replenish_test,
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "gc pthread_create");

  ret = pthread_create(&alloc_thread, &attr, alloc_thread_func, NULL);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "alloc pthread_create");
