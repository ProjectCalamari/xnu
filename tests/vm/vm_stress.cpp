...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("VM"), T_META_OWNER("tgal2"));

/** The following are modes that determine the way in which the created objects
 * will be re-mapped to the task's memory. The test behaves as follows according
 * to the chosen policy: RandomPartition - creates a buffer for each (randomly
 * sized) part of each object. Every page of every object will be re-mapped
 * exactly once. OneToMany - creates multiple mappings of the entire object.
 *  Overwrite - same as OneToMany, only that a portion of each mapping's pages
 * will be overwritten, creating double the amount of mappings in total.
...
static uint64_t random_between(uint64_t a, uint64_t b) {
...
public:
  // Member functions:
  int wait_for_status(int runtime_secs) {
  ...
  enum state { paused, running, error, complete };
...
public:
  // Constructor:
  Mapping(uint32_t _id, uint64_t _offset_in_pages, MappingArgs _args,
          uint32_t _fd)
      : id(_id), offset_in_pages(_offset_in_pages), args(_args), fd(_fd),
        lock(std::make_shared<std::shared_mutex>()), src_mapping(std::nullopt),
        is_mapped(false) {
  ...
  static bool compare_by_id(const Mapping &a, const Mapping &b) {
    return a.id < b.id;
  }
  ...
  kern_return_t remap_fixed() {
    kern_return_t kr = mach_vm_remap(
        args.arg_target_task, &args.arg_target_address, args.arg_mapping_size,
        args.arg_mask, VM_FLAGS_OVERWRITE | VM_FLAGS_FIXED, args.arg_src_task,
        args.arg_src_address + offset_in_pages * PAGE_SIZE, args.arg_copy,
        (vm_prot_t *)&(args.arg_cur_protection),
  ...
  int create_mapping() {
    ...
    if (kr != KERN_SUCCESS) {
      throw std::runtime_error(
          "mach_vm_remap failed: " + std::string(mach_error_string(kr)) + "\n");
    }
    return 0;
  }

  void set_src_mapping(Mapping &other) { src_mapping = other; }

  // Operations to be done by the ran threads:

  kern_return_t deallocate_no_lock() {
    is_mapped = false;
    kern_return_t kr = mach_vm_deallocate(
        args.arg_src_task, args.arg_target_address, args.arg_mapping_size);
    return kr;
  }

  bool realloc_no_parent() {
  ...
  bool realloc_with_parent() {
    std::unique_lock<std::shared_mutex> my_unique(*lock, std::defer_lock);
    std::unique_lock<std::shared_mutex> parent_unique(
        *(src_mapping->get().lock), std::defer_lock);
  ...
  bool op_protect() {
    kern_return_t kr = mach_vm_protect(
        mach_task_self(), (mach_vm_address_t)args.arg_target_address,
  ...
  bool op_wire() {
    std::this_thread::sleep_for(std::chrono::microseconds(50));
    uint32_t err = mlock((void *)args.arg_target_address,
                         (num_pages / op_denom) * PAGE_SIZE);
    if (err) {
      return false;
    }
    return true;
  ...
  bool op_write() {
    ...
    for (uint64_t i = 1; i <= num_pages / op_denom; i++) {
      ((char *)args.arg_target_address)[i * PAGE_SIZE - 1] =
          'M'; // M marks it was written via the mapping (for debugging
               // purposes)
    }

    // No need to sync to the file. It will be written when paged-out (which
  ...
  bool op_unwire() {
    uint32_t err = munlock((void *)args.arg_target_address,
                           (num_pages / op_denom) * PAGE_SIZE);
    if (err) {
      return false;
    }
  ...
  bool op_write_direct() {
    std::this_thread::sleep_for(std::chrono::microseconds(50));

    if (!fd) {
      return false; // Return early if no file descriptor (no file-backed
                    // mapping)
    }
    ...
    for (uint64_t i = 1; i <= num_pages / op_denom; i++) {
      ((char *)args.arg_target_address)[i * PAGE_SIZE - 1] =
          'D'; // D marks it was written using op_write_Direct (for debugging
               // purposes)
    }
    ...
    if (lseek(fd, 0, SEEK_SET) == -1) {
      throw std::runtime_error(
          "lseek failed to move cursor to beginning. err=" +
          std::to_string(errno));
    }

    int num_bytes = write(fd, (void *)(args.arg_target_address),
                          (num_pages / op_denom) * PAGE_SIZE);
  ...
  bool op_pageout() {
    if (madvise((void *)args.arg_target_address,
                (num_pages / op_denom) * PAGE_SIZE, MADV_PAGEOUT)) {
      return false;
    }
    return true;
  }

  bool run_op(const std::pair<vm_op, std::string> *op) {
    bool ret = false;
    ret = op->first(this);
  ...
  void create_gap_before() {
    mach_vm_address_t to_dealloc = args.arg_target_address - PAGE_SIZE;
    kern_return_t kr =
        mach_vm_deallocate(mach_task_self(), to_dealloc, PAGE_SIZE);
    if (kr != KERN_SUCCESS) {
      throw std::runtime_error("mach_vm_deallocate failed: " +
                               std::string(mach_error_string(kr)) + "\n");
    }
  }

  void adjust_addresses_and_offset(uint64_t detached_num_pages,
                                   uint64_t detached_size) {
    args.arg_src_address += detached_size;
    args.arg_target_address += detached_size;
    offset_in_pages += detached_num_pages;
  }

  void shrink_size(uint64_t detached_num_pages, uint64_t detached_size) {
    num_pages -= detached_num_pages;
    args.arg_mapping_size -= detached_size;
  }

  /* Fix the wrapper of the mapping after overwriting a part of it, to keep it
   * aligned to real vmmap_entry */
  void fix_overwritten_mapping(uint64_t detached_num_pages) {
  ...
  void print_mapping() {
    T_LOG("\tMAPPING #%2d, from address: %llx, to address: %llx, offset: "
          "%2llu, size: %4llu "
          "pages\n",
          id, args.arg_src_address, args.arg_target_address, offset_in_pages,
          num_pages);
  }
  ...
  void add_child(Mapping *other) { children.emplace_back(other); }

  void print_as_tree(const std::string &prefix = "", bool isLast = true) {
   ...
   * In some cases (MAppingsManager::policy==MappingPolicy::Topology), the
   * source for this mapping is another mapping. This case requires, in certain
   * ops (op_de_re_allocate()), to also hold the source's lock. That means lock
   * is going to be under shared ownership and therefore the locks should be in
   * a shared_ptr.
   */
  uint32_t op_denom =
      1; // tells the various operations what part of num_pages to include.
  ...
  /*
   * The following is disabled due to a deadlock it causes in the kernel too
   * frequently (and we want a running stress test). See rdar://146761078 Once
   * this deadlock is solved, we should uncomment it.
   */
...
public:
  ...
  // Constructor:
  Object(uint32_t _id, uint32_t num_pages) : id(_id), num_pages(num_pages) {}

  // Memeber functions:

  // Creation:

  int open_file_slow_paging() {
  ...
  int open_file() {
    std::string template_str =
        "/tmp/some_file_" + std::to_string(id) + "XXXXXX";
  ...
  int create_source_from_file(bool slow_paging) {
    ...
    src = (mach_vm_address_t)mmap(NULL, num_pages * PAGE_SIZE,
  ...
  int create_source_anon() {
    uint32_t anywhere_flag = TRUE;
    kern_return_t kr = mach_vm_allocate(mach_task_self(), &src,
                                        num_pages * PAGE_SIZE, anywhere_flag);
    if (kr != KERN_SUCCESS) {
  ...
  int create_source(bool is_file, bool slow_paging) {
    if (is_file) {
  ...
  static uint64_t random_object_size(uint64_t obj_size) {
...
public:
  // Constructor:
  MappingsManager(const Object &_obj, MappingPolicy _policy)
  ...
  ~MappingsManager() {
    for (uint32_t i = 0; i < ranges.size(); i++) {
      if (buffers[i]) {
        mach_vm_deallocate(mach_task_self(), (mach_vm_address_t)buffers[i],
                           ranges[i].second - ranges[i].first + 2);
        buffers[i] = nullptr;
      }
    }
  }

  enum topology { chain, star, ternary, random };

  // Member functions:

  std::string topo_to_string() {
    switch (topo) {
    case chain:
      return "chain";
    case star:
  ...
  void create_general_borders(std::vector<uint64_t> &general_borders) {
  ...
  void create_borders(std::vector<uint64_t> &borders) {
  ...
  void convert_borders_to_ranges(std::vector<uint64_t> &borders) {
    for (uint32_t i = 0; i < borders.size() - 1; ++i) {
      ranges.emplace_back(borders[i], borders[i + 1] - 1);
    }
  }

  void make_random_partition() {
  ...
  void create_seq(std::vector<uint32_t> &seq) {
    seq.emplace_back(0);
    for (uint32_t i = 1; i < num_mappings; i++) {
      switch (topo) {
      case chain:
  ...
  void allocate_buffer(uint64_t num_pages_to_alloc) {
    // buffers.emplace_back((char *)malloc((obj.num_pages + 1) * PAGE_SIZE)); //
    // One extra page for a gap
    mach_vm_address_t buff;
    kern_return_t kr = mach_vm_allocate(mach_task_self(), &buff,
                                        num_pages_to_alloc * PAGE_SIZE, TRUE);
    if (kr != KERN_SUCCESS) {
      throw std::runtime_error("Failed to allocate buffer in object #" +
                               std::to_string(obj.id) + "\n");
    }
    buffers.push_back((char *)buff);
  }

  void initialize_partition_buffers() {
    for (auto &range : ranges) {
      allocate_buffer(range.second - range.first + 2);
    }
  }

  MappingArgs initialize_basic_args() {
    MappingArgs args;
  ...
  void map_by_seq(std::vector<uint32_t> &seq) {
  ...
  /* Mode 1 - maps parts of the object to parts of the (only) buffer. Every page
   * is mapped exactly once. */
  void map_by_random_partition() {
    ...
    for (uint32_t i = 0; i < num_mappings; i++) {
      ...
      args.arg_mapping_size =
          (ranges[i].second - ranges[i].first + 1) * PAGE_SIZE;
      mappings.emplace_back(Mapping(i + 1, ranges[i].first, args, obj.fd));
    }
  }

  /* Modes 2,4 - maps the entire object to different buffers (which all have the
   * same size as the object). */
  void map_one_to_many(bool extra) {
    uint32_t num_pages_for_gaps = extra ? 2 : 1;
    MappingArgs args = initialize_basic_args();
    for (uint32_t i = 0; i < num_mappings; i++) {
      allocate_buffer(obj.num_pages + num_pages_for_gaps);
      args.arg_target_address =
          (mach_vm_address_t)(buffers[i] + PAGE_SIZE * num_pages_for_gaps);
      args.arg_mapping_size = obj.num_pages * PAGE_SIZE;
      mappings.emplace_back(Mapping(i + 1, 0, args, obj.fd));
    }
  }

  /* Mode 3 - maps the source object in a certain CoW-topology, based on the
   * given sequence. */
  void map_topo() {
  ...
  void map() {
    ...
    case MappingPolicy::Topology:
      ...
      topo = static_cast<topology>(
          (obj.id - 1) % 4); // Each object (out of every 4 consecutive objects)
                             // will be remapped in a different CoW topology.
      map_topo();
      break;
    default:
      break;
  ...
  void overwrite_mappings() {
    ...
    set_srcs(); // set the src (parent) lock for each newly created mapping to
                // facilitate op_de_re_allocate().
  }
  ...
  bool validate_partition() {
    return validate_sum() && validate_consecutiveness() &&
           validate_start_and_end();
  }

  bool validate_one_to_many() { return validate_all_sizes(); }

  bool validate_user_space() {
    switch (policy) {
    case MappingPolicy::RandomPartition:
  ...
  void set_flags(uint32_t flags) { mpng_flags = flags; }

  void set_is_cow(bool _is_cow) { is_cow = _is_cow; }
  ...
  /**
   * Sets the part to overwrite in case
   * MappingsManager::policy==MappingPolicy::Overwrite. It's the same for all of
   * the mappings and has to be visible outside of the class for logging
   * purposes. Therefore it's static.
   */
  Object obj;
...
public:
  ...
  int create_objects(uint32_t num_objects, uint64_t obj_size,
                     MappingPolicy policy, bool is_file, bool is_cow,
                     bool slow_paging) {
    for (uint32_t i = 1; i <= num_objects; i++) {
      ...
      if (o.create_source(is_file, slow_paging) == 0) {
        ...
        throw std::runtime_error("Error creating source object #" +
                                 std::to_string(i) + "\n");
      }
    }
    return 0;
  ...
  void create_mappings(uint32_t flags, bool is_cow) {
  ...
  bool run_op_on_all_mappings(const std::pair<vm_op, std::string> *op,
                              uint32_t op_idx) {
    for (auto &mngr : managers) {
  ...
  void num2op(std::pair<vm_op, std::string> *op, uint32_t thread_number) {
    op->first = Mapping::ops[thread_number % Mapping::ops.size()].first;
    op->second = Mapping::ops[thread_number % Mapping::ops.size()].second;
  }

  void print_thread_started(uint32_t thread_number, std::string thread_name) {
  ...
  std::future<void> start_thread(uint32_t thread_number) {
    uint32_t op_name_length = 16; // Just the length of the longest op name, for
                                  // nicer printing of op_count
    ...
    std::string operation_name_aligned =
        operation.second; // For nice printing only
    if (operation_name_aligned.length() < op_name_length) {
      operation_name_aligned =
          operation_name_aligned +
          std::string(op_name_length - operation_name_aligned.length(),
                      ' '); // Pad if shorter than op_name_length
    }
    thread_name = operation_name_aligned + " #" +
                  std::to_string(thread_number_remainder + 1);

    print_thread_started(thread_number, thread_name);

    return std::async(
        std::launch::async,
        [this, operation, thread_name,
         thread_number]() { /* lambda: */
                            while (runner.state != TestRuntime::error &&
                                   runner.state != TestRuntime::complete) {
                              if (runner.state == TestRuntime::running) {
                                bool running = this->run_op_on_all_mappings(
                                    &operation,
                                    thread_number % Mapping::ops.size());
                                if (!running) {
                                  break;
                                }
                              }
  ...
  void start_ops(uint32_t num_threads) {
  ...
  void join_threads() {
    for (auto &f : futures) {
      f.get(); // This replaces thread.join() in order to propogate the
               // exceptions raised from non main threads
    }
  }
  ...
  void print_op_counts() {
    for (uint32_t i = 0; i < Mapping::ops.size(); i++) {
      T_LOG("%16s: successes %7d :|: fails: %7d",
            Mapping::ops[i].second.c_str(), op_status_counters[i].success,
            op_status_counters[i].fail);
    }
  }
  ...
  void print_test_result() {
    T_LOG("\ninner validation: OBJECTS AND MAPPINGS APPEAR %s",
          validate() ? "AS EXPECTED" : "*NOT* AS EXPECTED");
  }
...
uint32_t run_test(const TestParams &tp) {
  Memory memory;
  uint32_t status;

  int src_created_successfully =
      memory.create_objects(tp.num_objects, tp.obj_size, tp.policy, tp.is_file,
                            tp.is_cow, tp.slow_paging);
...
void try_catch_test(TestParams &tp) {
  try {
    if (run_test(tp)) {
      T_FAIL("Test failed");
    } else {
...
void print_help() {
  printf("\n\nUsage: <path_to_executable>/vm_stress config -- <mapping_policy> "
         "<num_objects> <obj_size> <runtime_secs> <num_threads> <is_cow> "
         "<is_file> [-s]\n\n");

  printf("  <num_objects>      Number of objects the test will create and work "
         "on\n");
  ...
  printf(
      "  <mapping_policy>   Policy for mapping (part/one_to_many/over/topo)\n");
  printf("  <is_cow>           Copy-on-write flag (0 or 1)\n");
  printf("  <is_file>          File flag (0 or 1)\n\n");
}

void string_to_policy(MappingPolicy &policy, std::string policy_str) {
...
T_DECL(config, "configurable", T_META_ENABLED(false) /* rdar://142726486 */) {
  ...
  if (argc == 0) {
    printf("\n\n\nNo arguments for configurable test, assuming intention was "
           "to skip it.\n\n\n");
  ...
  if (argc != 7 && argc != 8) {
    ...
    printf("Usage: <path_to_executable>/vm_stress config -- <mapping_policy> "
           "<num_objects> <obj_size> <runtime_secs> <num_threads> <is_cow> "
           "<is_file>\nPolicies: part/one_to_many/over/topo\n\n");
    printf("Run \"<path_to_executable>/vm_stress config -- -h\" for more "
           "info\n\n\n");
  ...
  TestParams params = {.num_objects = num_objects,
                       .obj_size = obj_size,
...
T_DECL(vm_stress1, "partitions") {
  TestParams params = {.num_objects = 5,
                       .obj_size = 32,
                       .runtime_secs = 3,
                       .num_threads = 2,
...
T_DECL(vm_stress2, "cow topologies") {
  TestParams params = {.num_objects = 10,
                       .obj_size = 32,
                       .runtime_secs = 4,
                       .num_threads = 4,
...
T_DECL(vm_stress_hole, "Test locking of ranges with holes in them.") {
  uint32_t num_secs = 5;
  uint32_t half_of_num_mappings =
      5; // To ensure num_mappings is an even number.
  ...
  while (std::chrono::steady_clock::now() < end_time) {
    for (uint32_t i = 0; i < half_of_num_mappings * 2; i += 2) {
      if ((err = minherit((void *)mappings[i], 2 * PAGE_SIZE,
                          inheritance % 2)) != 0) {
        break;
      }
    }
