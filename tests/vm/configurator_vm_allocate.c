...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm.configurator"),
 ...
 */
static void checker_perform_successful_vm_allocate(checker_list_t *checker_list,
 ...
 */
static bool is_fixed(int flags) {
  static_assert(VM_FLAGS_FIXED == 0,
                "this test requires VM_FLAGS_FIXED be zero");
...
static bool is_fixed_overwrite(int flags) {
  return is_fixed(flags) && (flags & VM_FLAGS_OVERWRITE);
}
#endif /* workaround_rdar_143341561 */

static bool call_vm_allocate_and_expect_result(mach_vm_address_t start,
                                               mach_vm_size_t size,
                                               int flags_and_tag,
  ...
  bool caught_exception = block_raised_exc_guard_of_type_ignoring_translated(
      GUARD_TYPE_VIRT_MEMORY, &exc_info, ^{
        kr =
  ...
  if (kr != expected_kr) {
    T_EXPECT_MACH_ERROR(kr, expected_kr, "mach_vm_allocate(flags 0x%x)",
                        flags_and_tag);
    return false;
  }
  if (allocated != start) {
    T_FAIL("mach_vm_allocate(flags 0x%x) returned address 0x%llx (expected "
           "0x%llx)",
...
static bool call_vm_allocate_and_expect_success(mach_vm_address_t start,
                                                mach_vm_size_t size,
                                                int flags_and_tag) {
...
static bool call_vm_allocate_and_expect_no_space(mach_vm_address_t start,
                                                 mach_vm_size_t size,
                                                 int flags_and_tag) {
  return call_vm_allocate_and_expect_result(start, size, flags_and_tag,
                                            KERN_NO_SPACE);
...
static test_result_t successful_vm_allocate_fixed(checker_list_t *checker_list,
                                                  mach_vm_address_t start,
                                                  mach_vm_size_t size) {
...
test_permanent_entry_fixed_overwrite(checker_list_t *checker_list,
                                     mach_vm_address_t start,
                                     mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
    T_LOG("warning: can't work around rdar://143341561 on Rosetta; just "
          "passing instead");
...
static test_result_t
test_permanent_before_permanent_fixed_overwrite(checker_list_t *checker_list,
                                                mach_vm_address_t start,
                                                mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t
test_permanent_before_allocation_fixed_overwrite(checker_list_t *checker_list,
                                                 mach_vm_address_t start,
                                                 mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
test_permanent_before_allocation_fixed_overwrite_rdar144128567(
    checker_list_t *checker_list, mach_vm_address_t start,
    mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
    T_LOG("warning: can't work around rdar://143341561 on Rosetta; just "
...
static test_result_t
test_permanent_before_hole_fixed_overwrite(checker_list_t *checker_list,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t
test_permanent_after_allocation_fixed_overwrite(checker_list_t *checker_list,
                                                mach_vm_address_t start,
                                                mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t
test_permanent_after_hole_fixed_overwrite(checker_list_t *checker_list,
                                          mach_vm_address_t start,
                                          mach_vm_size_t size) {
#if workaround_rdar_143341561
  if (isRosetta()) {
...
static test_result_t test_permanent_entry_fixed_overwrite_with_neighbor_tags(
  ...
  if (!call_vm_allocate_and_expect_no_space(
          start, size,
          VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE | VM_MAKE_TAG(tag))) {
    return TestFailed;
  }
...
static test_result_t fixed_no_space(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size) {
...
static test_result_t successful_vm_allocate_fixed_overwrite_with_tag(
    checker_list_t *checker_list, mach_vm_address_t start, mach_vm_size_t size,
    uint16_t tag) {
  if (!call_vm_allocate_and_expect_success(start, size,
...
static test_result_t
successful_vm_allocate_fixed_overwrite(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  return successful_vm_allocate_fixed_overwrite_with_tag(checker_list, start,
                                                         size, 0);
}

static test_result_t successful_vm_allocate_fixed_overwrite_with_neighbor_tags(
    checker_list_t *checker_list, mach_vm_address_t start,
    mach_vm_size_t size) {
  uint16_t tag;
...
T_DECL(vm_allocate_fixed_overwrite_with_neighbor_tags,
  ...
  vm_tests_t tests = {
      ...
      .shared_entry_discontiguous =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .shared_entry_partial =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .shared_entry_pairs =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .shared_entry_x1000 =
      ...
      .protection_single_000_rw0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_single_r00_r00 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_single_r00_rw0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_single_0w0_0w0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_single_0w0_rw0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_single_rw0_rw0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,

      .protection_pairs_000_000 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_000_r00 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_000_0w0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_000_rw0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_r00_000 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_r00_r00 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_r00_0w0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_r00_rw0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_0w0_000 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      ...
      .protection_pairs_rw0_r00 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_rw0_0w0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
      .protection_pairs_rw0_rw0 =
          successful_vm_allocate_fixed_overwrite_with_neighbor_tags,
  };

  run_vm_tests("vm_allocate_fixed_overwrite_with_neighbor_tags", __FILE__,
               &tests, argc, argv);
}
