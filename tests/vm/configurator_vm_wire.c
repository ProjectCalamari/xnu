 ...
 */
static void checker_perform_vm_unwire(checker_list_t *checker_list,
...
checker_perform_vm_unwire_with_holes(checker_list_t *checker_list,
                                     mach_vm_address_t start,
                                     mach_vm_size_t size) {
  entry_checker_range_t limit =
 ...
 */
static void checker_perform_vm_wire(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size, vm_prot_t wire_prot) {
...
static void checker_perform_failed_vm_wire(checker_list_t *checker_list,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size,
                                           vm_prot_t wire_prot) {
  assert(wire_prot != VM_PROT_NONE);
...
successful_vm_wire_read_not_cow(checker_list_t *checker_list,
...
static test_result_t failed_vm_wire_read_not_cow(checker_list_t *checker_list,
                                                 mach_vm_address_t start,
                                                 mach_vm_size_t size) {
  kern_return_t kr;
...
static test_result_t wire_shared_entry(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
...
static test_result_t
wire_shared_entry_discontiguous(checker_list_t *checker_list,
                                mach_vm_address_t start, mach_vm_size_t size) {
  /*
   * two entries each sharing the same object
...
static test_result_t wire_shared_entry_partial(checker_list_t *checker_list,
                                               mach_vm_address_t start,
                                               mach_vm_size_t size) {
  /*
   * two entries each sharing the same object
...
static void checker_make_cow_private(checker_list_t *checker_list,
...
static test_result_t wire_cow_entry(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size) {
...
static test_result_t wire_cow_nocow(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size) {
  vm_entry_checker_t *left_checker = checker_list_nth(checker_list, 0);
  checker_make_cow_private(checker_list, left_checker);
...
static test_result_t wire_nocow_cow(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size) {
  vm_entry_checker_t *left_checker = checker_list_nth(checker_list, 0);
...
static test_result_t wire_cow_unreadable(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
...
static test_result_t wire_cow_unwriteable(checker_list_t *checker_list,
                                          mach_vm_address_t start,
                                          mach_vm_size_t size) {
  vm_entry_checker_t *checker = checker_list_nth(checker_list, 0);
 ...
 */
static test_result_t vm_unwire_holes(checker_list_t *checker_list,
                                     mach_vm_address_t start,
                                     mach_vm_size_t size) {
  ...
  FOREACH_CHECKER(checker, limit) {
    if (checker->kind == Allocation) {
      ...
      kr = mach_vm_wire(host_priv(), mach_task_self(), clipped_address,
                        clipped_size, VM_PROT_READ);
      assert(kr == 0);
      checker_perform_vm_wire(checker_list, clipped_address, clipped_size,
                              VM_PROT_READ);
    }
  }
  ...
  if (kr != expected_kr) {
    T_FAIL("mach_vm_wire(unwire) returned %d (%s), expected %d (%s)\n", kr,
           name_for_kr(kr), expected_kr, name_for_kr(expected_kr));
    return TestFailed;
  }
...
T_DECL(vm_wire, "run vm_wire with various vm configurations") {
  vm_tests_t tests = {
...
T_DECL(vm_unwire, "run vm_unwire with various vm configurations") {
  vm_tests_t tests = {
