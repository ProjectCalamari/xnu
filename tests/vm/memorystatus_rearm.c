...
#pragma unused(exception_port, thread, task, code_count)
  ...
  mach_exception_data_type_t resource =
      EXC_RESOURCE_DECODE_RESOURCE_TYPE(code[0]);
...
void eat_memory(int num_pages) {
  ...
  for (i = 0; i < REARM_TIMES; i++) {
    ...
    T_LOG("Freeing...");
    munmap((void *)((size_t)buf + vm_page_size),
           vm_page_size * (num_pages - 1));

    /* Re-arm EXC_RESOURCE */
    ret = memorystatus_control(MEMORYSTATUS_CMD_REARM_MEMLIMIT, getpid(),
                               MEMORYSTATUS_FLAGS_REARM_ACTIVE |
                                   MEMORYSTATUS_FLAGS_REARM_INACTIVE,
                               NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_ZERO(
        ret, "memorystatus_control(MEMORYSTATUS_CMD_REARM_MEMLIMIT)");
  }
...
T_HELPER_DECL(memory_enjoyer, "") {
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NE(sig, SIG_ERR, "signal(SIGUSR1)");
  dispatch = dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR1, 0,
                                    dispatch_get_main_queue());
  T_QUIET;
  T_ASSERT_NOTNULL(dispatch, "dispatch_source_create");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "signal parent");
...
static pid_t launch_child(int num_pages) {
  ...
  ret =
      posix_spawnattr_setjetsam_ext(&spawn_attrs, 0, JETSAM_PRIORITY_FOREGROUND,
                                    TEST_MEMLIMIT_MB, TEST_MEMLIMIT_MB);
...
static void *exc_handler_thread(void *arg) {
#pragma unused(arg)
  kern_return_t kret;
  ...
  kret = mach_port_insert_right(mach_task_self(), exception_port,
  ...
  kret = task_set_exception_ports(
      mach_task_self(), EXC_MASK_RESOURCE, exception_port,
  ...
  kret = mach_msg_server(mach_exc_server, MACH_MSG_SIZE_RELIABLE,
                         exception_port, 0);
  if (kret != KERN_SUCCESS) {
...
T_DECL(memorylimit_exception_tests, "EXC_RESOURCE re-arming",
  ...
  T_ASSERT_POSIX_ZERO(
      pthread_create(&handle_thread, NULL, exc_handler_thread, NULL),
      "pthread_create");
  dispatch_err = dispatch_semaphore_wait(sync_sema, SEM_TIMEOUT);
  T_QUIET;
  T_ASSERT_EQ(dispatch_err, 0L, "dispatch_semaphore_wait");

  /* Make sure we handle SIGUSR1 */
  sig_ret = signal(SIGUSR1, SIG_IGN);
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NE(sig_ret, SIG_ERR, "signal(SIGUSR1)");

  /*
   * When we receive SIGUSR1 from our child (to indicate that it's ready), send
   * it a SIGUSR1 back to indicate that we're ready (i.e. we've attached to the
   * child). Then, wait for EXC_RESOURCE to happen REARM_TIMES times.
   */
  dispatch = dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR1, 0,
                                    dispatch_get_main_queue());
  T_QUIET;
  ...
  dispatch_source_set_event_handler(dispatch, ^{
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "ptrace");
    ...
    for (int i = 0; i < REARM_TIMES; i++) {
      ...
      T_QUIET;
      T_ASSERT_EQ(dispatch_err, 0L, "Received EXC_RESOURCE");
    }
    T_END;
  });
  dispatch_activate(dispatch);
