...
T_DECL(vm_test_102067976, "Test dangling PTE due to mis-aligned memory entry",
  ...
  kr = mach_memory_object_memory_entry(
      mach_host_self(), 1, (vm_size_t)vm_named_entry_size,
      VM_PROT_READ | VM_PROT_WRITE, MEMORY_OBJECT_NULL, &named_entry);
  if (kr == MIG_BAD_ID) {
  ...
  if (kr == MIG_BAD_ARGUMENTS) {
    T_SKIP("mach_memory_object_memory_entry() returned MIG_BAD_ARGUMENTS "
  ...
  T_QUIET;
  T_ASSERT_NE(named_entry, MACH_PORT_NULL, "named_entry is not null");

  /* map the memory to our space */
  address = 0;
  kr = mach_vm_map(
      mach_task_self(), &address, (-1), /* not 0 but rounds up to 0 */
      0,                                /* mask */
      VM_FLAGS_ANYWHERE, named_entry, vm_map_entry_offset, /* offset */
      FALSE,                                               /* copy */
