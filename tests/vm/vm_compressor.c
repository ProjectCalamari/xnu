...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("VM"), T_META_ASROOT(NO),
              T_META_RUN_CONCURRENTLY(true));

T_DECL(swap_enabled, "Check that Swap is successfully enabled",
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rc, "Failed to query sysctl `vm.swap_enabled`");
  T_EXPECT_EQ(swap_enabled, 1, "Check that vm.swap_enabled is set");
}

T_DECL(host_statistics_q_lens,
       "Check that the compressor queue lens sysctl works",
  ...
  kern_return_t kr =
      host_statistics64(mach_host_self(), HOST_VM_COMPRESSOR_Q_LENS,
                        (host_info64_t)&d, &vm_qlen_count);
  T_ASSERT_MACH_SUCCESS(kr, "host_statistics64(HOST_VM_COMPRESSOR_Q_LENS)");
  T_EXPECT_EQ(vm_qlen_count, (uint32_t)VM_COMPRESSOR_Q_LENS_COUNT,
              "check output is the expected length");
}

size_t read_big_sysctl(const int *mib, size_t mib_len, const char *name,
                       char **buf);
...
size_t read_big_sysctl(const int *mib, size_t mib_len, const char *name,
  ...
  len += 4096; /* allocate a bit extra in case the size changed between the two
                  calls */
  *buf = (char *)malloc(len);
...
size_t get_sysctl_mib(const char *ctl_name, int *mib, size_t mib_sz) {
...
T_DECL(sysctl_c_segments,
       "Check that the sysctl that dumps all the c_segments works correctly",
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1)) {
  ...
  size_t offset = 0;
  T_ASSERT_GE_ULONG(sz, sizeof(uint32_t),
                    "got buffer shorter than the magic value");
  ...
  while (offset < sz) {
    // read next c_segment
    T_QUIET;
    T_ASSERT_LE(offset + sizeof(struct c_segment_info), sz,
                "unexpected offset for c_segment_info");
    const struct c_segment_info *cseg =
        (const struct c_segment_info *)(buf + offset);
    offset += sizeof(struct c_segment_info);
    // read it's slots
    T_QUIET;
    T_ASSERT_LE(offset + cseg->csi_slots_len * sizeof(struct c_slot_info), sz,
                "unexpected offset for c_slot_info");
    offset += cseg->csi_slots_len * sizeof(struct c_slot_info);
  }
}

T_DECL(sysctl_vm_object_dump,
       "Check that the sysctl that dumps the metadata of a vm_object works "
       "correctly",
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1)) {
  int mib[CTL_MAXNAME] = {0};
  size_t mib_len =
      get_sysctl_mib("vm.task_vm_objects_slotmap", mib, CTL_MAXNAME);
  mib[mib_len++] = getpid();

  char *buf = NULL;
  size_t sz =
      read_big_sysctl(mib, mib_len, "vm.compressor_segments.<pid>", &buf);
  ...
  T_ASSERT_LE(offset + sizeof(struct vm_map_info_hdr), sz,
              "vm_map_info_hdr size");
  struct vm_map_info_hdr *hdr = (struct vm_map_info_hdr *)(buf + offset);
  for (int i = 0; i < hdr->vmi_nentries; ++i) {
    T_QUIET;
    T_ASSERT_LE(offset + sizeof(struct vm_map_entry_info), sz,
                "vm_map_entry_info size");
