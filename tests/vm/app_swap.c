...
T_HELPER_DECL(helper, "Dummy helper") { exit(0); }
...
static void continue_child_and_wait_for_exit() {
  int ret, stat;
  /* Resume the child and wait for it to exit. It should exit cleanly. */
  ret = kill(child_pid, SIGCONT);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "failed to send SIGCONT to child process");
  ret = waitpid(child_pid, &stat, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "waitpid");
  T_QUIET;
  T_ASSERT_TRUE(WIFEXITED(stat), "child exited.");
  T_QUIET;
  T_ASSERT_EQ(WEXITSTATUS(stat), 0, "child exited cleanly.");
}
...
static void terminate_and_reap_coalition(uint64_t coalition_id) {
  int ret = 0;
  ret = coalition_terminate(coalition_id, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "coalition_terminate");

  ret = coalition_reap(coalition_id, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "coalition_reap");
}

static void terminate_and_reap_coalitions() {
 ...
 */
static pid_t spawn_coalition_leader(const char *path, char *const *argv,
                                    uint64_t resource_coal_id,
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "posix_spawnattr_init failed with %s", strerror(ret));

  ret = posix_spawnattr_setcoalition_np(
      &attr, jetsam_coal_id, COALITION_TYPE_JETSAM, COALITION_TASKROLE_LEADER);
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "posix_spawnattr_setcoalition_np failed with %s",
              strerror(ret));
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "posix_spawnattr_setflags failed with %s", strerror(ret));

  ret = posix_spawn(&new_pid, path, NULL, &attr, argv, environ);
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "posix_spawn failed with %s", strerror(ret));

  ret = posix_spawnattr_destroy(&attr);
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "posix_spawnattr_destroy failed with %s\n",
              strerror(ret));
  return new_pid;
}

T_DECL(mark_coalition_swappable, "Set coalition is swappable",
       T_META_ASROOT(true), T_META_BOOTARGS_SET("kern.swap_all_apps=1"),
  ...
  char *const args[] = {testpath, "-n", "helper", NULL};
  child_pid = spawn_coalition_leader(testpath, args, resource_coalition_id,
                                     jetsam_coalition_id);

  T_ATEND(continue_child_and_wait_for_exit);

  ret =
      memorystatus_control(MEMORYSTATUS_CMD_GET_PROCESS_COALITION_IS_SWAPPABLE,
                           child_pid, 0, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret,
                         "MEMORYSTATUS_CMD_GET_PROCESS_COALITION_IS_SWAPPABLE");
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "process is not swappable at launch");

  leader_pid = get_coalition_leader(child_pid);

  ret = memorystatus_control(MEMORYSTATUS_CMD_MARK_PROCESS_COALITION_SWAPPABLE,
                             leader_pid, 0, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret,
                         "MEMORYSTATUS_CMD_MARK_PROCESS_COALITION_SWAPPABLE");
  ...
  T_ASSERT_POSIX_SUCCESS(ret,
                         "MEMORYSTATUS_CMD_GET_PROCESS_COALITION_IS_SWAPPABLE");
  T_QUIET;
  T_ASSERT_EQ(ret, 1, "process is swappable");
}
