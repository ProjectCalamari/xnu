...
T_GLOBAL_META(T_META_NAMESPACE("xnu.memorystatus"),
...
enum exit_codes_num { CREATE_LIST(EXIT_CODES_ENUM) };

#define EXIT_CODES_STRING(VAR) #VAR,
static const char *exit_codes_str[] = {CREATE_LIST(EXIT_CODES_STRING)};
...
static int get_vmpage_size(void) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "failed to query vm.pagesize");
  T_QUIET;
  T_ASSERT_GT(vmpage_size, 0, "vm.pagesize is not > 0");
  return vmpage_size;
}
...
/* NB: in_shared_region and get_rprvt are pulled from the memorystatus unit
 * test. We're moving away from those unit tests, so they're copied here.
 */

/* Cribbed from 'top'... */
static int in_shared_region(mach_vm_address_t addr, cpu_type_t type) {
...
static void restore_freezer() {
  int ret;
  T_LOG("Restoring original vm.freeze_enabled value (%d)...", orig_freezer);
  ret = sysctlbyname("vm.freeze_enabled", NULL, NULL, &orig_freezer,
                     sizeof(orig_freezer));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "Enabling freezer via sysctl");
}

static void check_for_and_enable_freezer() {
  int ret, freeze_enabled;
  size_t size = sizeof(freeze_enabled);

  ret = sysctlbyname("vm.freeze_enabled", &freeze_enabled, &size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "Could not find vm.freeze_enabled");

  if (!freeze_enabled) {
    T_LOG("Freezer not enabled, enabling...");
    orig_freezer = freeze_enabled;
    freeze_enabled = 1;
    ret = sysctlbyname("vm.freeze_enabled", NULL, NULL, &freeze_enabled,
...
static unsigned long long get_rprvt(pid_t pid) {
  ...
  mach_vm_size_t pagesize =
      vm_kernel_page_size; // The vm_region page info is reported
  ...
  T_QUIET;
  T_ASSERT_TRUE(kr == KERN_SUCCESS, "Unable to get task_for_pid of child");
  ...
  for (addr = 0;; addr += size) {
    ...
    kr = mach_vm_region(task, &addr, &size, VM_REGION_TOP_INFO,
    ...
    if (in_shared_region(addr, CPU_TYPE_ARM)) {
      ...
      if (split == FALSE && info.share_mode == SM_EMPTY) {
        ...
        kr = mach_vm_region(task, &b_addr, &b_size, VM_REGION_BASIC_INFO_64,
...
void move_to_idle_band(pid_t pid) {
  memorystatus_priority_properties_t props;
  /*
   * Freezing a process also moves it to an elevated jetsam band in order to
   * protect it from idle exits. So we move the child process to the idle band
   * to mirror the typical 'idle app being frozen' scenario.
   */
...
void freeze_helper_process(void) {
  ...
  ret = sysctlbyname("kern.memorystatus_freeze", NULL, NULL, &child_pid,
                     sizeof(child_pid));
  ...
  if (freeze_count % 2) {
    ...
    T_ASSERT_POSIX_SUCCESS(
        sysctlbyname("vm.freeze_enabled", &freeze_enabled, &length, NULL, 0),
        "failed to query vm.freeze_enabled");
    if (freeze_enabled) {
      errno = errno_freeze_sysctl;
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.memorystatus_freeze failed");
    } else {
      /* If freezer is disabled, skip the test. This can happen due to disk
       * space shortage. */
    ...
    T_QUIET;
    T_ASSERT_LT(resident_memory_after, resident_memory_before,
                "Freeze didn't reduce resident memory set");
    if (resident_memory_before > freeze_pages_max) {
      T_QUIET;
      T_ASSERT_LE(resident_memory_before - resident_memory_after,
                  freeze_pages_max,
                  "Freeze pages froze more than the threshold.");
    }
    ret = sysctlbyname("kern.memorystatus_thaw", NULL, NULL, &child_pid,
                       sizeof(child_pid));
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.memorystatus_thaw failed");
  } else {
    T_QUIET;
    T_ASSERT_TRUE(ret != KERN_SUCCESS, "Freeze should have failed");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kill(child_pid, SIGUSR1),
                         "failed to send SIGUSR1 to child process");
}

void run_freezer_test(int num_pages) {
  int ret;
  char sz_str[50];
  ...
  ds_freeze = dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR1, 0,
                                     dispatch_get_main_queue());
  T_QUIET;
  T_ASSERT_NOTNULL(ds_freeze, "dispatch_source_create (ds_freeze)");
  ...
  launch_tool_args =
      (char *[]){testpath, "-n", "allocate_pages", "--", sz_str, NULL};

  /* Spawn the child process. Suspend after launch until the exit proc handler
   * has been set up. */
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(child_pid, "dt_launch_tool");

  ds_proc =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, (uintptr_t)child_pid,
                             DISPATCH_PROC_EXIT, dispatch_get_main_queue());
  T_QUIET;
  T_ASSERT_NOTNULL(ds_proc, "dispatch_source_create (ds_proc)");

  dispatch_source_set_event_handler(ds_proc, ^{
    int status = 0, code = 0;
    pid_t rc = waitpid(child_pid, &status, 0);
    T_QUIET;
    T_ASSERT_EQ(rc, child_pid, "waitpid");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kill(child_pid, SIGCONT),
                         "failed to send SIGCONT to child process");
  dispatch_main();
}

static void allocate_pages(int num_pages) {
  int i, j, vmpgsize;
  char val;
  ...
  dispatch_after(
      ...
      ^{
        /* Signal to the parent that we're done allocating and it's ok to freeze
         * us */
  ...
  signal(SIGUSR1, SIG_IGN);
  ds_signal = dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR1, 0,
                                     dispatch_get_main_queue());
  if (ds_signal == NULL) {
  ...
  dispatch_source_set_event_handler(ds_signal, ^{
    ...
    current_state = memorystatus_control(
        MEMORYSTATUS_CMD_GET_PROCESS_IS_FREEZABLE, getpid(), 0, NULL, 0);
    ...
    printf("[%d] Changing state from %s to %s\n", getpid(),
           (current_state) ? "freezable" : "unfreezable",
           (new_state) ? "freezable" : "unfreezable");
    if (memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_FREEZABLE,
                             getpid(), (uint32_t)new_state, NULL,
                             0) != KERN_SUCCESS) {
...
T_HELPER_DECL(allocate_pages, "allocates pages to freeze",
              T_META_ASROOT(true)) {
  if (argc < 1) {
...
T_DECL(memorystatus_freeze_default_state,
       "Test that the freezer is enabled or disabled as expected by default.",
       T_META_ASROOT(true), T_META_TAG_VM_NOT_PREFERRED) {
  ...
  if (ret != 0) {
    if (expected_freeze_enabled) {
      T_ASSERT_FAIL(
          "Expected freezer enabled but did not find sysctl vm.freeze_enabled");
    } else {
      T_PASS("Did not expect freezer to be enabled and did not find sysctl "
             "vm.freeze_enabled");
    }
  } else {
    T_ASSERT_EQ(freeze_enabled, expected_freeze_enabled,
                "Expected freezer sysctl status %d, got %d",
                expected_freeze_enabled, freeze_enabled);
  }
}

T_DECL(freeze, "VM freezer test", T_META_ENABLED(HAS_FREEZER),
       T_META_ASROOT(true), T_META_TAG_VM_NOT_PREFERRED) {
  check_for_and_enable_freezer();
  run_freezer_test((MEM_SIZE_MB << 20) / get_vmpage_size());
}

static int old_freeze_pages_max = 0;
static void reset_freeze_pages_max(void) {
...
int sysctl_freeze_pages_max(int *new_value) {
  ...
  ret = sysctlbyname("kern.memorystatus_freeze_pages_max", &freeze_pages_max,
                     &size, new_value, size);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret,
                         "Unable to query kern.memorystatus_freeze_pages_max");
  if (!set_end_handler) {
    // Save the original value and instruct darwintest to restore it after the
    // test completes
...
T_DECL(freeze_over_max_threshold, "Max Freeze Threshold is Enforced",
       T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
...
T_HELPER_DECL(frozen_background, "Frozen background process",
  ...
  kern_ret = memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_FREEZABLE,
                                  getpid(), 1, NULL, 0);
  T_QUIET;
  T_ASSERT_EQ(kern_ret, KERN_SUCCESS, "set process is freezable");
...
static void freeze_process(pid_t pid) {
  int ret, freeze_enabled, errno_freeze_sysctl;
  size_t length;
  ...
  if (freeze_enabled) {
    ...
    /* If freezer is disabled, skip the test. This can happen due to disk space
     * shortage. */
    T_LOG("Freeze has been disabled. Terminating early.");
    T_END;
  }
...
static void reset_max_frozen_demotions_daily(void) {
  int sysctl_ret = sysctlbyname("kern.memorystatus_max_freeze_demotions_daily",
                                NULL, NULL, &max_frozen_demotions_daily_default,
                                sizeof(max_frozen_demotions_daily_default));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctl_ret,
      "set kern.memorystatus_max_freeze_demotions_daily to default");
}

static void allow_unlimited_demotions(void) {
  size_t size = sizeof(max_frozen_demotions_daily_default);
  uint32_t new_value = UINT32_MAX;
  int sysctl_ret = sysctlbyname("kern.memorystatus_max_freeze_demotions_daily",
                                &max_frozen_demotions_daily_default, &size,
                                &new_value, sizeof(new_value));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctl_ret, "kern.memorystatus_max_freeze_demotions_daily = UINT32_MAX");
  T_ATEND(reset_max_frozen_demotions_daily);
}

static void memorystatus_assertion_test_demote_frozen(void) {
  /*
   * Test that if we assert a priority on a process, freeze it, and then demote
   * all frozen processes, it does not get demoted below the asserted priority.
  ...
  ds_signal = dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR1, 0,
                                     dispatch_get_main_queue());
  T_QUIET;
  T_ASSERT_NOTNULL(ds_signal, "dispatch_source_create");
  dispatch_source_set_event_handler(ds_signal, ^{
    int sysctl_ret;
    /* Freeze the process, trigger agressive demotion, and check that it hasn't
     * been demoted. */
    freeze_process(child_pid);
    /* Agressive demotion */
    sysctl_ret = sysctlbyname("kern.memorystatus_demote_frozen_processes", NULL,
                              NULL, &demote_value, sizeof(demote_value));
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(
        sysctl_ret,
        "sysctl kern.memorystatus_demote_frozen_processes succeeded");
    /* Check */
    (void)check_properties(child_pid, requestedpriority, inactive_limit_mb, 0x0,
                           ASSERTION_STATE_IS_SET, "Priority was set");
    ...
    (void)check_properties(
        child_pid, JETSAM_PRIORITY_AGING_BAND2, inactive_limit_mb, 0x0,
        ASSERTION_STATE_IS_RELINQUISHED, "Assertion was reqlinquished.");
    /* Kill the child */
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(kill(child_pid, SIGKILL), "Killed child process");
    T_END;
  });
  ...
  (void)check_properties(child_pid, requestedpriority, active_limit_mb, 0x0,
                         ASSERTION_STATE_IS_SET, "Priority was set");
  /* Listen for exit. */
  ds_exit =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, (uintptr_t)child_pid,
  ...
  dispatch_source_set_event_handler(ds_exit, ^{
    ...
    code = WEXITSTATUS(status);
    T_QUIET;
    T_ASSERT_EQ(code, 0, "Child exited cleanly");
    T_END;
  });
...
T_DECL(assertion_test_demote_frozen,
       "demoted frozen process goes to asserted priority.",
...
static unsigned int get_freeze_daily_pages_max(void) {
  unsigned int memorystatus_freeze_daily_mb_max;
  size_t length = sizeof(memorystatus_freeze_daily_mb_max);
  int ret = sysctlbyname("kern.memorystatus_freeze_daily_mb_max",
                         &memorystatus_freeze_daily_mb_max, &length, NULL, 0);
  T_ASSERT_POSIX_SUCCESS(ret, "kern.memorystatus_freeze_daily_mb_max");
  return memorystatus_freeze_daily_mb_max * 1024UL * 1024UL /
         vm_kernel_page_size;
}

static uint64_t get_budget_multiplier(void) {
  uint64_t budget_multiplier = 0;
  size_t size = sizeof(budget_multiplier);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("kern.memorystatus_freeze_budget_multiplier",
                   &budget_multiplier, &size, NULL, 0),
      "get kern.memorystatus_freeze_budget_multiplier");
  return budget_multiplier;
}
static void set_budget_multiplier(uint64_t multiplier) {
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("kern.memorystatus_freeze_budget_multiplier", NULL, NULL,
                   &multiplier, sizeof(multiplier)),
      "set kern.memorystatus_freeze_budget_multiplier");
}

static uint64_t original_budget_multiplier;
static void reset_budget_multiplier(void) {
  set_budget_multiplier(original_budget_multiplier);
}

static unsigned int get_memorystatus_swap_all_apps(void) {
  unsigned int memorystatus_swap_all_apps;
  size_t length = sizeof(memorystatus_swap_all_apps);
  int ret = sysctlbyname("kern.memorystatus_swap_all_apps",
                         &memorystatus_swap_all_apps, &length, NULL, 0);
...
T_DECL(budget_replenishment, "budget replenishes properly",
  ...
  /*
   * Calculate a new budget as if the previous interval expired
   * kTestIntervalSecs ago and we used up its entire budget.
   */
  length = sizeof(kTestIntervalSecs);
  new_budget_ln = sizeof(new_budget);
  ret = sysctlbyname("vm.memorystatus_freeze_calculate_new_budget", &new_budget,
                     &new_budget_ln, &kTestIntervalSecs, length);
  ...
  T_LOG("memorystatus_freeze_daily_pages_max %u",
        memorystatus_freeze_daily_pages_max);
  T_LOG("page_size %lu", page_size);

  /*
   * We're kTestIntervalSecs past a new interval. Which means we are owed
   * kNumSecondsInDay seconds of budget.
   */
  ...
  T_ASSERT_EQ(kTestIntervalSecs, 60 * 60 * 32,
              "kTestIntervalSecs did not change");
  expected_new_budget_pages +=
      ((kTestIntervalSecs * kFixedPointFactor) /
       (kNumSecondsInDay)*memorystatus_freeze_daily_pages_max) /
      kFixedPointFactor;
  ...
  T_LOG("memorystatus_freeze_daily_pages_max after %u",
        memorystatus_freeze_daily_pages_max);

  T_QUIET;
  T_ASSERT_EQ(new_budget, expected_new_budget_pages,
              "Calculate new budget behaves correctly.");
}

static void get_frozen_list(global_frozen_procs_t *frozen_procs) {
  int bytes_written;
  bytes_written = memorystatus_control(MEMORYSTATUS_CMD_FREEZER_CONTROL, 0,
                                       FREEZER_CONTROL_GET_PROCS, frozen_procs,
                                       sizeof(global_frozen_procs_t));
  T_QUIET;
  T_ASSERT_LE((size_t)bytes_written, sizeof(global_frozen_procs_t),
              "Didn't overflow buffer");
  T_QUIET;
  T_ASSERT_GT(bytes_written, 0, "Wrote someting");
}

static bool is_proc_in_frozen_list(pid_t pid, char *name, size_t name_len) {
  bool found = false;
  global_frozen_procs_t *frozen_procs = malloc(sizeof(global_frozen_procs_t));
  T_QUIET;
  T_ASSERT_NOTNULL(frozen_procs, "malloc");
...
static void unset_testing_pid(void) {
  int ret;
  ret = memorystatus_control(MEMORYSTATUS_CMD_SET_TESTING_PID, 0,
                             MEMORYSTATUS_FLAGS_UNSET_TESTING_PID, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, 0, "Drop ownership of jetsam snapshot");
}

static void set_testing_pid(void) {
  int ret;
  ret = memorystatus_control(MEMORYSTATUS_CMD_SET_TESTING_PID, 0,
                             MEMORYSTATUS_FLAGS_SET_TESTING_PID, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "Take ownership of jetsam snapshot");
 ...
 */
static memorystatus_jetsam_snapshot_t *get_jetsam_snapshot(uint32_t flags,
                                                           bool empty_allowed) {
  memorystatus_jetsam_snapshot_t *snapshot = NULL;
  int ret;
  uint32_t size;

  ret = memorystatus_control(MEMORYSTATUS_CMD_GET_JETSAM_SNAPSHOT, 0, flags,
                             NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, 0, "Get jetsam snapshot size");
  size = (uint32_t)ret;
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(snapshot, "Allocate snapshot of size %d", size);

  ret = memorystatus_control(MEMORYSTATUS_CMD_GET_JETSAM_SNAPSHOT, 0, flags,
                             snapshot, size);
  T_QUIET;
  T_ASSERT_GT(size, 0, "Get jetsam snapshot");

  if (((size - sizeof(memorystatus_jetsam_snapshot_t)) /
       sizeof(memorystatus_jetsam_snapshot_entry_t)) != snapshot->entry_count) {
    T_FAIL("Malformed snapshot: %d! Expected %ld + %zd x %ld = %ld\n", size,
           sizeof(memorystatus_jetsam_snapshot_t), snapshot->entry_count,
 ...
 */
static memorystatus_jetsam_snapshot_entry_t *
get_jetsam_snapshot_entry(memorystatus_jetsam_snapshot_t *snapshot, pid_t pid) {
  T_QUIET;
  T_ASSERT_NOTNULL(snapshot, "Got snapshot");
...
static void resume_and_kill_proc(pid_t pid) {
  int ret = pid_resume(pid);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "proc resumed after freeze");
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kill(pid, SIGKILL), "Killed process");
}

static void resume_and_kill_child(void) {
  /* Used for test cleanup. proc might not be suspended so pid_resume might
   * fail. */
  pid_resume(child_pid);
  kill(child_pid, SIGKILL);
}

static dispatch_source_t run_block_after_signal(int sig,
 ...
 */
static void test_after_background_helper_launches(bool exit_with_child,
                                                  const char *variant,
...
T_DECL(get_frozen_procs, "List processes in the freezer",
       T_META_ENABLED(HAS_FREEZER), T_META_TAG_VM_NOT_PREFERRED) {

  check_for_and_enable_freezer();
  test_after_background_helper_launches(true, "frozen_background", ^{
    proc_name_t name;
    /* Place the child in the idle band so that it gets elevated like a typical
     * app. */
    ...
    /* Check */
    T_QUIET;
    T_ASSERT_TRUE(is_proc_in_frozen_list(child_pid, name, sizeof(name)),
                  "Found proc in frozen list");
    T_QUIET;
    T_EXPECT_EQ_STR(name, "memorystatus_freeze_test", "Proc has correct name");
    /* Kill the child */
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(kill(child_pid, SIGKILL), "Killed child process");
    T_END;
...
T_DECL(frozen_to_swap_accounting,
  ...
  test_after_background_helper_launches(true, "frozen_background", ^{
     ...
     * If we don't see any pages get sent to disk before kFreezeToDiskMaxDelay
     * seconds, something is either wrong with the compactor or the accounting.
     */
    for (size_t i = 0; i < kFreezeToDiskMaxDelay / kSnapshotSleepDelay; i++) {
      snapshot =
          get_jetsam_snapshot(MEMORYSTATUS_FLAGS_SNAPSHOT_ON_DEMAND, false);
      child_entry = get_jetsam_snapshot_entry(snapshot, child_pid);
      T_QUIET;
      T_ASSERT_NOTNULL(child_entry, "Found child in snapshot");
    ...
    T_QUIET;
    T_ASSERT_GT(child_entry->jse_frozen_to_swap_pages, 0ULL,
                "child has some pages in swap");
    free(snapshot);
    /* Kill the child */
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(kill(child_pid, SIGKILL), "Killed child process");
...
T_DECL(freezer_snapshot, "App kills are recorded in the freezer snapshot",
  ...
  /* Take ownership of the snapshot to ensure we don't race with another process
  ...
  test_after_background_helper_launches(false, "frozen_background", ^{
    ...
    ret =
        memorystatus_control(MEMORYSTATUS_CMD_TEST_JETSAM, child_pid, 0, 0, 0);
    ...
    child_entry = get_jetsam_snapshot_entry(snapshot, child_pid);
    T_QUIET;
    T_ASSERT_NOTNULL(child_entry, "Child is in freezer snapshot");
    T_QUIET;
    T_ASSERT_EQ(child_entry->killed, (unsigned long long)JETSAM_REASON_GENERIC,
...
T_DECL(freezer_snapshot_consume, "Freezer snapshot is consumed on read",
  ...
  test_after_background_helper_launches(false, "frozen_background", ^{
    ...
    child_entry = get_jetsam_snapshot_entry(snapshot, child_pid);
    T_QUIET;
    T_ASSERT_NOTNULL(child_entry, "Child is in first freezer snapshot");

    snapshot = get_jetsam_snapshot(MEMORYSTATUS_FLAGS_SNAPSHOT_FREEZER, true);
    if (snapshot != NULL) {
      child_entry = get_jetsam_snapshot_entry(snapshot, child_pid);
      T_QUIET;
      T_ASSERT_NULL(child_entry, "Child is not in second freezer snapshot");
    }
...
T_DECL(freezer_snapshot_frozen_state,
  ...
  test_after_background_helper_launches(false, "frozen_background", ^{
    ...
    T_ASSERT_NOTNULL(child_entry, "Child is in freezer snapshot");
    T_QUIET;
    T_ASSERT_TRUE(child_entry->state & kMemorystatusFrozen,
...
T_DECL(freezer_snapshot_thaw_state,
  ...
  test_after_background_helper_launches(false, "frozen_background", ^{
    ...
    T_QUIET;
    T_ASSERT_TRUE(child_entry->state & kMemorystatusWasThawed,
                  "Child entry was thawed");
    T_QUIET;
    T_ASSERT_EQ(child_entry->jse_thaw_count, 1ULL,
                "Child entry's thaw count was incremented");
...
T_HELPER_DECL(check_frozen, "Check frozen state", T_META_ASROOT(true)) {
  ...
  kern_ret = memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_FREEZABLE,
                                  getpid(), 1, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(kern_ret, "set process is freezable");

  /* We should not be frozen yet. */
  is_frozen = memorystatus_control(MEMORYSTATUS_CMD_GET_PROCESS_IS_FROZEN,
                                   getpid(), 0, NULL, 0);
  if (is_frozen == -1) {
  ...
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NE(sig_ret, SIG_ERR, "signal(SIGUSR1, SIG_IGN)");
...
T_DECL(memorystatus_get_process_is_frozen,
...
static void cleanup_memorystatus_freeze_top_process(void) {
  sysctlbyname("kern.memorystatus_freeze_pages_min", NULL, NULL,
               &freeze_pages_min_old, sizeof(freeze_pages_min_old));
  sysctlbyname("kern.memorystatus_freeze_throttle_enabled", NULL, NULL,
               &throttle_enabled_old, sizeof(throttle_enabled_old));
}

/*
 * Disables heuristics that could prevent us from freezing the child via
 * memorystatus_freeze_top_process.
 */
static void memorystatus_freeze_top_process_setup(void) {
  ...
  ret = sysctlbyname("kern.memorystatus_freeze_pages_min",
                     &freeze_pages_min_old, &freeze_pages_min_size,
                     &freeze_pages_min_new, sizeof(freeze_pages_min_new));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "set kern.memorystatus_freeze_pages_min");
  ret = sysctlbyname("kern.memorystatus_freeze_throttle_enabled",
                     &throttle_enabled_old, &throttle_enabled_old_size,
                     &throttle_enabled_new, sizeof(throttle_enabled_new));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "set kern.memorystatus_freeze_throttle_enabled");
  T_ATEND(cleanup_memorystatus_freeze_top_process);
  /* Take ownership of the freezer probabilities for the duration of the test so
   * that we don't race with dasd. */
  set_testing_pid();
}
 ...
 */
static void prepare_proc_for_freezing(pid_t pid) {
...
static void verify_proc_frozen_state(pid_t pid, bool expected) {
  memorystatus_jetsam_snapshot_t *snapshot =
      get_jetsam_snapshot(MEMORYSTATUS_FLAGS_SNAPSHOT_ON_DEMAND, false);
  memorystatus_jetsam_snapshot_entry_t *entry =
      get_jetsam_snapshot_entry(snapshot, pid);
  T_ASSERT_NOTNULL(entry, "%d is in snapshot", pid);
  bool is_frozen = (entry->state & P_MEMSTAT_FROZEN) != 0;
  if (is_frozen != expected) {
    T_LOG("%s frozen state is wrong. Expected %d, got %d. Skip reason: %d. "
          "Jetsam band: %d",
          entry->name, expected, is_frozen, entry->jse_freeze_skip_reason,
          entry->priority);
  }
...
static void verify_proc_is_frozen(pid_t pid) {
  verify_proc_frozen_state(pid, true);
}

static void verify_proc_not_frozen(pid_t pid) {
  verify_proc_frozen_state(pid, false);
}

T_DECL(memorystatus_freeze_top_process,
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.maxproc");
  ret = sysctlbyname("kern.memorystatus_freeze_jetsam_band",
                     &memorystatus_freeze_band, &memorystatus_freeze_band_size,
                     NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.memorystatus_freeze_jetsam_band");
  ...
  test_after_background_helper_launches(true, "frozen_background", ^{
    ...
    size_t buffer_len =
        sizeof(memorystatus_properties_entry_v1_t) * (size_t)maxproc;
    memorystatus_properties_entry_v1_t *properties_list = malloc(buffer_len);
    T_QUIET;
    T_ASSERT_NOTNULL(properties_list, "malloc properties array");
    size_t properties_list_len = 0;
    /* The child needs to age down into the idle band before it's eligible to be
     * frozen. */
    ...
    while (child_band != JETSAM_PRIORITY_IDLE) {
      ...
      bool found = false;
      for (size_t i = 0; i < snapshot->entry_count; i++) {
        memorystatus_jetsam_snapshot_entry_t *snapshot_entry =
            &snapshot->entries[i];
        if (snapshot_entry->priority <= memorystatus_freeze_band &&
            !snapshot_entry->killed) {
          pid_t pid = snapshot_entry->pid;
          memorystatus_properties_entry_v1_t *property_entry =
              &properties_list[properties_list_len++];
      ...
      T_QUIET;
      T_ASSERT_TRUE(found, "Child is in on demand snapshot");
      free(snapshot);
    }
    ret = memorystatus_control(
        MEMORYSTATUS_CMD_GRP_SET_PROPERTIES, 0,
        MEMORYSTATUS_FLAGS_GRP_SET_PROBABILITY, properties_list,
        sizeof(memorystatus_properties_entry_v1_t) * properties_list_len);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "MEMORYSTATUS_FLAGS_GRP_SET_PROBABILITY");
    free(properties_list);
    int val = 1;
    ret = sysctlbyname("vm.memorystatus_freeze_top_process", NULL, NULL, &val,
                       sizeof(val));
...
static void reset_ordered_freeze_mode(void) {
  sysctlbyname("kern.memorystatus_freezer_use_ordered_list", NULL, NULL,
               &use_ordered_list_original, sizeof(use_ordered_list_original));
}

static void reset_ordered_demote_mode(void) {
  sysctlbyname("kern.memorystatus_freezer_use_demotion_list", NULL, NULL,
               &use_demotion_list_original, sizeof(use_demotion_list_original));
}

static void enable_ordered_freeze_mode(void) {
  int ret;
  int val = 1;
  size_t size = sizeof(use_ordered_list_original);
  ret = sysctlbyname("kern.memorystatus_freezer_use_ordered_list",
                     &use_ordered_list_original, &size, &val, sizeof(val));
  T_ASSERT_POSIX_SUCCESS(ret, "kern.memorystatus_freezer_use_ordered_list");
  T_ATEND(reset_ordered_freeze_mode);
}

static void enable_ordered_demote_mode(void) {
  int ret;
  int val = 1;
  size_t size = sizeof(use_demotion_list_original);
  ret = sysctlbyname("kern.memorystatus_freezer_use_demotion_list",
                     &use_demotion_list_original, &size, &val, sizeof(val));
...
construct_child_freeze_entry(memorystatus_properties_freeze_entry_v1 *entry) {
  ...
  memorystatus_jetsam_snapshot_t *snapshot =
      get_jetsam_snapshot(MEMORYSTATUS_FLAGS_SNAPSHOT_ON_DEMAND, false);
  memorystatus_jetsam_snapshot_entry_t *snapshot_entry =
      get_jetsam_snapshot_entry(snapshot, child_pid);
  strncpy(entry->proc_name, snapshot_entry->name, sizeof(entry->proc_name));
  free(snapshot);
}

T_DECL(memorystatus_freeze_top_process_ordered,
       "memorystatus_freeze_top_process chooses the correct process when using "
       "an ordered list",
       T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_NOT_PREFERRED) {
  ...
  test_after_background_helper_launches(true, "frozen_background", ^{
    ...
    ret = memorystatus_control(MEMORYSTATUS_CMD_GRP_SET_PROPERTIES, 0,
                               MEMORYSTATUS_FLAGS_GRP_SET_FREEZE_PRIORITY,
                               entries, sizeof(entries));
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "MEMORYSTATUS_FLAGS_GRP_SET_FREEZE_PRIORITY");
...
static void memorystatus_freeze_top_process_ordered_wrong_pid(
    pid_t (^pid_for_entry)(pid_t)) {
  memorystatus_freeze_top_process_setup();
  enable_ordered_freeze_mode();
  test_after_background_helper_launches(true, "frozen_background", ^{
    int ret, val = 1;
 ...
 */
T_DECL(memorystatus_freeze_top_process_ordered_reused_pid,
       "memorystatus_freeze_top_process is resilient to pid changes",
       T_META_ENABLED(HAS_FREEZER),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1), T_META_ASROOT(true),
       T_META_TAG_VM_NOT_PREFERRED) {
  check_for_and_enable_freezer();
...
T_DECL(memorystatus_freeze_top_process_ordered_wrong_pid,
       "memorystatus_freeze_top_process is resilient to pid changes",
       T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_NOT_PREFERRED) {
...
T_DECL(memorystatus_freeze_demote_ordered,
       "memorystatus_demote_frozen_processes_using_demote_list chooses the "
       "correct process",
       T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_NOT_PREFERRED) {
  ...
  test_after_background_helper_launches(true, "frozen_background", ^{
    ...
    ret = sysctlbyname("kern.memorystatus_freeze_jetsam_band",
                       ...
                       &memorystatus_freeze_band_size, NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "kern.memorystatus_freeze_jetsam_band");
    ...
    ret = memorystatus_control(MEMORYSTATUS_CMD_GRP_SET_PROPERTIES, 0,
                               ...
                               entries, sizeof(entries));
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "MEMORYSTATUS_FLAGS_GRP_SET_DEMOTE_PRIORITY");
    ...
    val = 1;
    ret = sysctlbyname("kern.memorystatus_demote_frozen_processes", NULL, NULL,
                       &val, sizeof(val));
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(
        ret, "sysctl kern.memorystatus_demote_frozen_processes succeeded");
    ...
    T_ASSERT_NOTNULL(child_entry, "Found child in snapshot");
    T_QUIET;
    T_ASSERT_LT(child_entry->priority, memorystatus_freeze_band,
                "child was demoted");
    free(snapshot);

    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(kill(child_pid, SIGKILL), "Killed process");
...
static int memorystatus_freezer_thaw_percentage(void) {
  int val;
  size_t size = sizeof(val);
  int ret = sysctlbyname("kern.memorystatus_freezer_thaw_percentage", &val,
                         &size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      ret, "failed to query kern.memorystatus_freezer_thaw_percentage");
  return val;
}

static void reset_interval(void) {
  ...
  ret = sysctlbyname("kern.memorystatus_freeze_daily_mb_max",
                     &freeze_daily_budget_mb, &size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      ret, "failed to query kern.memorystatus_freeze_daily_mb_max");
  new_budget = (freeze_daily_budget_mb * (1UL << 20) / vm_page_size);
  ret = sysctlbyname("kern.memorystatus_freeze_budget_pages_remaining", NULL,
                     NULL, &new_budget, sizeof(new_budget));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      ret, "failed to set kern.memorystatus_freeze_budget_pages_remaining");
}

static pid_t second_child;
static void cleanup_memorystatus_freezer_thaw_percentage(void) {
  kill(second_child, SIGKILL);
}

T_DECL(memorystatus_freezer_thaw_percentage,
  ...
  check_for_and_enable_freezer();
  /* Take ownership of the freezer probabilities for the duration of the test so
   * that nothing new gets frozen by dasd. */
  set_testing_pid();
  reset_interval();

  /* Spawn one child that will remain frozen throughout the whole test & another
   * that will be thawed. */
  first_signal_block = run_block_after_signal(SIGUSR1, ^{
    ...
    T_QUIET;
    T_ASSERT_EQ(memorystatus_freezer_thaw_percentage(), 0,
                "thaw percentage is still 0 after freeze");
    dispatch_source_cancel(first_signal_block);
    test_after_background_helper_launches(true, "frozen_background", ^{
      reset_interval();
      T_QUIET;
      T_ASSERT_EQ(memorystatus_freezer_thaw_percentage(), 0,
                  "new interval starts with a thaw percentage of 0");
      move_to_idle_band(child_pid);
      ret = pid_suspend(child_pid);
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(ret, "child suspended");
      ...
      T_QUIET;
      T_ASSERT_GT(percentage_after_thaw, 0,
                  "thaw percentage is higher after thaw");
      ...
      T_QUIET;
      T_ASSERT_EQ(memorystatus_freezer_thaw_percentage(), 0,
                  "new interval starts with a 0 thaw percentage");
      ret = pid_resume(child_pid);
      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(ret, "child resumed after freeze");
      T_QUIET;
      T_ASSERT_GT(memorystatus_freezer_thaw_percentage(), 0,
                  "thaw percentage goes back up in new interval");

      T_QUIET;
      T_ASSERT_POSIX_SUCCESS(kill(child_pid, SIGKILL), "failed to kill child");
      T_END;
    });
  });
...
static uint64_t get_budget_pages_remaining(void) {
  uint64_t pages_remaining = 0;
  size_t size = sizeof(pages_remaining);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("kern.memorystatus_freeze_budget_pages_remaining",
                   &pages_remaining, &size, NULL, 0),
...
static void set_budget_pages_remaining(uint64_t pages_remaining) {
  ...
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("kern.memorystatus_freeze_budget_pages_remaining", NULL,
                   NULL, &pages_remaining, sizeof(pages_remaining)),
      "get kern.memorystatus_freeze_budget_pages_remaining");
}

static void set_freeze_enabled(int freeze_enabled) {
  size_t length = sizeof(freeze_enabled);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(
      sysctlbyname("vm.freeze_enabled", NULL, NULL, &freeze_enabled, length),
      "enable vm.freeze_enabled");
}

static void enable_freeze(void) { set_freeze_enabled(1); }

T_DECL(memorystatus_freeze_budget_multiplier,
       "memorystatus_budget_multiplier multiplies budget", T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_NE("kern.memorystatus_freeze_daily_mb_max",
                                 UINT32_MAX),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_ENABLED(HAS_FREEZER),
  ...
  set_budget_multiplier(100);
  T_QUIET;
  T_ASSERT_EQ(get_budget_pages_remaining(), freeze_daily_pages_max,
              "multiplier=100%%");
  set_budget_multiplier(50);
  T_QUIET;
  T_ASSERT_EQ(get_budget_pages_remaining(), freeze_daily_pages_max / 2,
              "multiplier=50%%");
...
T_DECL(memorystatus_freeze_set_dasd_trial_identifiers,
       "set dasd trial identifiers", T_META_ENABLED(HAS_FREEZER),
       T_META_ASROOT(true), T_META_TAG_VM_NOT_PREFERRED) {
#define TEST_STR "freezer-das-trial"
  ...
  strncpy(identifiers.experiment_id, TEST_STR,
          sizeof(identifiers.treatment_id));
  identifiers.deployment_id = 2;
  int ret = memorystatus_control(MEMORYSTATUS_CMD_FREEZER_CONTROL, 0,
                                 FREEZER_CONTROL_SET_DASD_TRIAL_IDENTIFIERS,
                                 &identifiers, sizeof(identifiers));
  T_WITH_ERRNO;
  T_ASSERT_EQ(ret, 0, "FREEZER_CONTROL_SET_DASD_TRIAL_IDENTIFIERS");
}

T_DECL(memorystatus_reset_freezer_state,
       "FREEZER_CONTROL_RESET_STATE kills frozen proccesses",
       T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_NOT_PREFERRED) {
  check_for_and_enable_freezer();
  ...
  test_after_background_helper_launches(false, "frozen_background", ^{
    ...
    ret = memorystatus_control(MEMORYSTATUS_CMD_FREEZER_CONTROL, 0,
                               FREEZER_CONTROL_RESET_STATE, NULL, 0);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "FREEZER_CONRTOL_RESET_STATE");

    /* Verify budget resets to a non-zero value. Some devices may have a
     * configured budget of 0. Skip this assertion if so. */
    uint64_t budget_after_reset = get_budget_pages_remaining();
#if TARGET_OS_XR
    T_QUIET;
    T_ASSERT_EQ(budget_after_reset, 0ULL, "freeze budget after reset == 0");
#else
		T_QUIET; T_ASSERT_GT(budget_after_reset, 0ULL, "freeze budget after reset > 0");
#endif
    /*
     * Verify child has been killed
     * Note that the task termination isn't synchronous with the RESET_STATE
     * call so we may block in waitpid temporarily.
     */
    int stat;
    while (true) {
    ...
    T_QUIET;
    T_ASSERT_TRUE(WIFSIGNALED(stat), "child was signaled");
    T_QUIET;
    T_ASSERT_EQ(WTERMSIG(stat), SIGKILL, "Child received SIGKILL");
...
static void dock_proc(pid_t pid) {
  int ret;
  ret = memorystatus_control(
      MEMORYSTATUS_CMD_ELEVATED_INACTIVEJETSAMPRIORITY_ENABLE, pid, 0, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "dock_proc");
}

T_DECL(memorystatus_freeze_skip_docked,
       "memorystatus_freeze_top_process does not freeze docked processes",
       T_META_ENABLED(HAS_FREEZER && !TARGET_OS_WATCH), T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_NOT_PREFERRED) {
  check_for_and_enable_freezer();
  memorystatus_freeze_top_process_setup();
...
T_HELPER_DECL(corpse_generation, "Generate a large corpse",
              T_META_ASROOT(false)) {
  /*
  ...
  dispatch_source_set_event_handler(ds_signal, ^{
     ...
     * The parent process will try to kill us due to disk space shortage in
     * parallel.
     */
...
T_DECL(memorystatus_disable_freeze_corpse,
       "memorystatus_disable_freeze with parallel corpse creation",
       T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_NOT_PREFERRED) {
  /*
   * In the past, we've had race conditions w.r.t. killing on disk space
   * shortage and corpse generation of frozen processes. This test spwans a
   * frozen helper which generates a large corpse (which should take in the 10s
   * to 100s of m.s. to complete) while the test process triggers disk space
   * kills. We should see that the test process is jetsammed successfully.
   */
  check_for_and_enable_freezer();
  test_after_background_helper_launches(false, "corpse_generation", ^{
    ...
    /*
     * Verify child has been killed
     * Note that the task termination isn't synchronous with the freeze_enabled
     * call so we may block in waitpid temporarily.
     */
    while (true) {
      pid_t wait_p = waitpid(child_pid, &stat, 0);
...
static void unrestrict_coalitions() {
  int ret, val = 1;
  size_t val_size = sizeof(val);
  size_t original_unrestrict_coalitions_size =
      sizeof(original_unrestrict_coalitions_val);
  ret = sysctlbyname("kern.unrestrict_coalitions",
                     &original_unrestrict_coalitions_val,
                     &original_unrestrict_coalitions_size, &val, val_size);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "unrestrict_coalitions");
}

static void reset_unrestrict_coalitions() {
  size_t size = sizeof(original_unrestrict_coalitions_val);
  int ret = sysctlbyname("kern.unrestrict_coalitions", NULL, NULL,
                         &original_unrestrict_coalitions_val, size);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "unrestrict_coalitions");
}

static uint64_t create_coalition(int type) {
  uint64_t id = 0;
  uint32_t flags = 0;
  uint64_t param[2];
  int ret;
...
#define MAX_XPC_SERVICE_FREEZE                                                 \
  10 /* xnu will only freeze 10 XPC services per coalition. */
 ...
 */
static pid_t spawn_coalition_member(const char *path, char *const *argv,
                                    int role, short spawn_flags) {
  int ret;
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "posix_spawnattr_init failed with %s",
                      strerror(ret));

  ret = posix_spawnattr_setcoalition_np(&attr, jetsam_coal,
                                        COALITION_TYPE_JETSAM, role);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "posix_spawnattr_setcoalition_np failed with %s",
                      strerror(ret));
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "posix_spawnattr_setflags failed with %s",
                      strerror(ret));

  ret = posix_spawn(&new_pid, path, NULL, &attr, argv, environ);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "posix_spawn failed with %s", strerror(ret));

  ret = posix_spawnattr_destroy(&attr);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "posix_spawnattr_destroy failed with %s\n",
                      strerror(ret));

  kr = memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_MANAGED, new_pid, 1,
                            NULL, 0);
  T_QUIET;
...
static pid_t get_coalition_leader(pid_t p) {
...
#define p_pid iparam[2]
  ...
  ret = sysctlbyname("kern.coalition_pid_list", pid_list, &pid_list_size,
                     iparam, sizeof(iparam));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.coalition_pid_list");
  T_QUIET;
  T_ASSERT_LE(pid_list_size, kMaxPids * sizeof(int),
              "coalition is small enough");
  ...
  for (size_t i = 0; i < pid_list_size / sizeof(int); i++) {
    ...
    ret = sysctlbyname("kern.coalition_roles", roles, &roles_size, &curr_pid,
                       sizeof(curr_pid));
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "sysctl kern.coalition_roles");
...
void setup_coalitions() {}

void coal_foreach(void (^callback)(pid_t)) {
  for (int i = 0; i < n_coalition_members; i++) {
    callback(coalition_members[i]);
  }
}

void kill_all_coalition_members() {
  int __block ret;
  T_LOG("Killing coalition members...");
  coal_foreach(^(pid_t pid) {
    ret = kill(pid, SIGKILL);
    T_QUIET;
    T_EXPECT_POSIX_SUCCESS(ret, "kill");
  });
}

void spawn_coalition_and_run(dispatch_block_t after_spawn) {
  int ret, __block i = 0;
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(ret, "_NSGetExecutablePath");
  static char *const args[] = {pathbuf, "-n", "frozen_background", NULL};
  ...
  sig_source = run_block_after_signal(SIGUSR1, ^{
    ...
    if (i < n_coalition_members) {
      /* Spawn next child */
      coalition_members[i] =
          spawn_coalition_member(pathbuf, args, COALITION_TASKROLE_XPC, 0);
    } else {
      after_spawn();
    }
  });
  ...
  /* Spawn leader */
  child_pid =
      spawn_coalition_member(pathbuf, args, COALITION_TASKROLE_LEADER, 0);
...
static void kill_all_frozen() {
  /* Disabling and re-enabling the freezer will evict all frozen processes. */
  set_freeze_enabled(0);
  set_freeze_enabled(1);
}

static unsigned int get_used_freezer_slots() {
  int ret;
  unsigned int frozen_procs;
  size_t frozen_procs_size = sizeof(frozen_procs);

  ret = sysctlbyname("kern.memorystatus_freeze_count", &frozen_procs,
                     &frozen_procs_size, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "get kern.memorystatus_freeze_count");

  return frozen_procs;
}

static void setup_coalition_freezing() {
...
static uint32_t get_max_freezer_slots() {
  int ret;
  uint32_t max_freeze_processes = 0;
  size_t max_freeze_processes_len = sizeof(max_freeze_processes);
  ret = sysctlbyname("kern.memorystatus_freeze_processes_max",
                     &max_freeze_processes, &max_freeze_processes_len, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.memorystatus_freeze_processes_max");
...
static void set_max_freezer_slots(uint32_t max_freeze_processes) {
  int ret;
  if (orig_freezer_slots == -1) {
    orig_freezer_slots = get_max_freezer_slots();
    T_ATEND(restore_freezer_slots);
  }
  ret = sysctlbyname("kern.memorystatus_freeze_processes_max", NULL, NULL,
                     &max_freeze_processes, sizeof(max_freeze_processes));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "set kern.memorystatus_freeze_processes_max");
}
...
static void prepare_coalition_for_freezing(void) {
  coal_foreach(^(pid_t pid) {
    prepare_proc_for_freezing(pid);
  });
}

static int freeze_coalition_leader(void) {
  int ret, val = 1;
  memorystatus_properties_freeze_entry_v1 entry;
  construct_child_freeze_entry(&entry);
  ret = memorystatus_control(MEMORYSTATUS_CMD_GRP_SET_PROPERTIES, 0,
                             MEMORYSTATUS_FLAGS_GRP_SET_FREEZE_PRIORITY, &entry,
                             sizeof(entry));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "MEMORYSTATUS_FLAGS_GRP_SET_FREEZE_PRIORITY");
  return sysctlbyname("vm.memorystatus_freeze_top_process", NULL, NULL, &val,
                      sizeof(val));
}
...
T_DECL(
    memorystatus_coalition_freeze,
    "Freezing a coalition leader should freeze all of its XPC service members",
    T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
    T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
    T_META_TAG_VM_NOT_PREFERRED) {
  check_for_and_enable_freezer();
  ...
  kill_all_frozen();
  T_QUIET;
  T_ASSERT_EQ(get_used_freezer_slots(), 0, "No freezer slots used");
  ...
  spawn_coalition_and_run(^{
    ...
    T_ASSERT_EQ(n_coal_frozen, n_coalition_members,
                "All coalition members frozen");
...
T_DECL(memorystatus_coalition_freezer_slot_limit,
       "Exhausting freezer slots and freezing a large coalition should not "
       "exceed slots",
       T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_NOT_PREFERRED) {
  ...
  static_assert(TEST_MAX_FREEZER_SLOTS + N_UNFROZEN_MEMBERS <=
                COAL_MAX_MEMBERS);
  static_assert(TEST_MAX_FREEZER_SLOTS < MAX_XPC_SERVICE_FREEZE);
  ...
  spawn_coalition_and_run(^{
    ...
    T_ASSERT_EQ(n_coal_frozen, n_coalition_members - N_UNFROZEN_MEMBERS,
                "N_UNFROZEN_MEMBERS coalition members remain unfrozen");
...
static void kill_helper() {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kill(helper_pid, SIGUSR2)");
}

T_DECL(memorystatus_two_coalition_freeze,
       "Exhausting freezer slots with one coalition and freezing another "
       "should fail",
       T_META_ENABLED(HAS_FREEZER), T_META_ASROOT(true),
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_TAG_VM_NOT_PREFERRED) {
  ...
  exit_disp =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_PROC, (uintptr_t)helper_pid,
                             DISPATCH_PROC_EXIT, dispatch_get_main_queue());
  dispatch_source_set_event_handler(exit_disp, ^{
    ...
    T_QUIET;
    T_ASSERT_EQ(code, 0, "Helper exited cleanly");
  });
...
T_DECL(do_fastwake_warmup_all, "Test kern.memorystatus_do_fastwake_warmup_all",
       T_META_ASROOT(true),
       T_META_ENABLED(
           false /* rdar://149557081 !TARGET_OS_OSX && !TARGET_OS_BRIDGE */)) {
  int val = 1;
  int ret = sysctlbyname("kern.memorystatus_do_fastwake_warmup_all", NULL, NULL,
                         &val, sizeof(val));
  T_ASSERT_POSIX_SUCCESS(ret,
                         "sysctl(kern.memorystatus_do_fastwake_warmup_all)");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "host_info(HOST_VM_COMPRESSOR_Q_LENS)");

  T_EXPECT_EQ(cstats.qcc_swappedout_count, 0,
              "Zero swapped-out segments after fastwake warmup");
  T_EXPECT_EQ(cstats.qcc_swappedout_sparse_count, 0,
              "Zero sparse swapped-out segments after fastwake warmup");
}
