...
static char *_prot_str[] = {
    ...
    /* 6 */ "-wx",
    /* 7 */ "rwx"};
static char *prot_str(vm_prot_t prot) { return _prot_str[prot & VM_PROT_ALL]; }

static void print_region_info(mach_vm_address_t vmaddr, mach_vm_size_t vmsize,
...
static bool find_first_read_only_mapping(mach_vm_address_t *vmaddr_p,
  ...
  for (vmaddr = 0;; vmaddr += vmsize) {
    ...
    kr = mach_vm_region_recurse(mach_task_self(), &vmaddr, &vmsize, &depth,
                                (vm_region_recurse_info_t)ri, &count);
    T_QUIET;
...
T_DECL(vm_test_linkedit_permanent,
       "Tests that LINKEDIT mapping can't be overwritten",
...
#define ASSERT_UNCHANGED(clip_ok, object_change_ok)                            \
  ...
  T_EXPECT_MACH_SUCCESS(kr, "mach_vm_region_recurse(0x%llx)\n",
                        vmaddr_linkedit);
  print_region_info(vmaddr, vmsize, &ri1, vmaddr_linkedit, vmsize_linkedit,
                    &ri2);
  ...
  kr = mach_vm_allocate(mach_task_self(), &vmaddr_buf, vmsize_linkedit,
                        VM_FLAGS_ANYWHERE);
  T_EXPECT_MACH_SUCCESS(kr, "vm_allocate(0x%llx)", vmsize_linkedit);
  /* copy the data to avoid undue crash */
  memcpy((char *)(uintptr_t)vmaddr_buf, (char *)(uintptr_t)vmaddr_linkedit,
         (size_t)vmsize_linkedit);
  kr =
      mach_vm_write(mach_task_self(), vmaddr_linkedit, /* destination address */
  ...
  T_EXPECT_MACH_SUCCESS(kr, "mach_vm_region_recurse(0x%llx)\n",
                        vmaddr_linkedit);
  print_region_info(vmaddr_linkedit, vmsize_linkedit, &ri1, vmaddr_tmp,
                    vmsize_tmp, &ri2);
  ...
  vmaddr_tmp = vmaddr_linkedit;
  kr = mach_vm_allocate(mach_task_self(), &vmaddr_tmp, vmsize_linkedit,
  ...
  vmaddr_tmp = vmaddr_linkedit;
  kr =
      mach_vm_remap(mach_task_self(), &vmaddr_tmp, vmsize_linkedit, 0,
                    VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, mach_task_self(),
                    vmaddr_buf, TRUE, &cur_prot, &max_prot, VM_INHERIT_DEFAULT);
  T_EXPECT_MACH_ERROR(kr, KERN_NO_SPACE,
                      "vm_remap(OVERWRITE) fails with KERN_NO_SPACE");
  T_EXPECT_EQ(vmaddr_linkedit, vmaddr_tmp, "vmaddr is unchanged");
  /* check protections again */
  depth2 = 0;
  vmaddr_tmp = vmaddr_linkedit;
  ...
  ri1 = ri2;
  T_EXPECT_EQ(ri1.is_submap, 0, "mapping should not be nested");
  kr = mach_vm_protect(mach_task_self(), vmaddr_linkedit, vmsize_linkedit,
                       FALSE, /* set_maximum */
