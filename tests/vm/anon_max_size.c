...
T_DECL(anon_max_size, "Test an ALLOC_MAX_SIZE allocation",
       T_META_NAMESPACE("xnu.vm"), T_META_RADAR_COMPONENT_NAME("xnu"),
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "vm_allocate(0x%llx)", ANON_MAX_SIZE);
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "madvise(MADV_PAGEOUT)");
  ...
  T_QUIET;
  T_ASSERT_EQ(cp[0], 'a', "first page intact");
  T_QUIET;
  T_ASSERT_EQ(cp[vm_size - 1], 'z', "last page intact");
