...
char *_prot_str[] = {
    ...
    /* 6 */ "-wx",
    /* 7 */ "rwx"};
char *prot_str(vm_prot_t prot) { return _prot_str[prot & VM_PROT_ALL]; }

void print_region_info(mach_vm_address_t vmaddr, mach_vm_size_t vmsize,
                       vm_region_submap_short_info_64_t ri,
                       mach_vm_address_t vmaddr2, mach_vm_size_t vmsize2,
                       vm_region_submap_short_info_64_t ri2) {
  T_LOG("   [ 0x%016llx - 0x%016llx ] size 0x%016llx prot 0x%x/0x%x %s/%s "
        "submap %d\n",
        (uint64_t)vmaddr, (uint64_t)(vmaddr + vmsize), (uint64_t)vmsize,
        ri->protection, ri->max_protection, prot_str(ri->protection),
        prot_str(ri->max_protection), ri->is_submap);
  if (ri2) {
    T_LOG("-> [ 0x%016llx - 0x%016llx ] size 0x%016llx prot 0x%x/0x%x %s/%s "
          "submap %d\n",
          (uint64_t)vmaddr2, (uint64_t)(vmaddr2 + vmsize2), (uint64_t)vmsize2,
          ri2->protection, ri2->max_protection, prot_str(ri2->protection),
          prot_str(ri2->max_protection), ri2->is_submap);
  }
}

static bool find_nested_read_only_mapping(mach_vm_address_t *vmaddr_p,
  ...
  for (vmaddr_sub = 0;; vmaddr_sub += vmsize_sub) {
    ...
    kr = mach_vm_region_recurse(mach_task_self(), &vmaddr_sub, &vmsize_sub,
                                &depth, (vm_region_recurse_info_t)ri, &count);
    T_QUIET;
...
T_DECL(vm_test_shreg_ro,
       "Tests that read-only shared-region mappings can't be overwritten",
...
#define ASSERT_UNCHANGED(clip_ok)                                              \
  do {                                                                         \
    if (clip_ok) {                                                             \
      T_EXPECT_GE(vmaddr_tmp, vmaddr, "vmaddr clipped 0x%llx -> 0x%llx",       \
                  (uint64_t)vmaddr, (uint64_t)vmaddr_tmp);                     \
      T_EXPECT_LE(vmsize_tmp, vmsize, "vmsize clipped 0x%llx -> 0x%llx",       \
                  (uint64_t)vmsize, (uint64_t)vmsize_tmp);                     \
    } else {                                                                   \
      T_EXPECT_EQ(vmaddr_tmp, vmaddr, "vmaddr unchanged 0x%llx -> 0x%llx",     \
                  (uint64_t)vmaddr, (uint64_t)vmaddr_tmp);                     \
      T_EXPECT_EQ(vmsize_tmp, vmsize, "vmsize unchanged 0x%llx -> 0x%llx",     \
                  (uint64_t)vmsize, (uint64_t)vmsize_tmp);                     \
    }                                                                          \
    T_EXPECT_LE(ri2.protection, VM_PROT_READ, "should not become writable");   \
  ...
  depth1 = 0;
  count = VM_REGION_SUBMAP_SHORT_INFO_COUNT_64;
  kr = mach_vm_region_recurse(mach_task_self(), &vmaddr, &vmsize, &depth1,
                              (vm_region_recurse_info_t)&ri1, &count);
  T_EXPECT_MACH_SUCCESS(kr, "mach_vm_region_recurse(0x%llx)\n", vmaddr_sub);
  ...
  kr = mach_vm_allocate(mach_task_self(), &vmaddr_tmp, vmsize_sub,
  ...
  vmaddr_tmp = vmaddr_sub;
  count = VM_REGION_SUBMAP_SHORT_INFO_COUNT_64;
  kr = mach_vm_region_recurse(mach_task_self(), &vmaddr_tmp, &vmsize_tmp,
                              &depth2, (vm_region_recurse_info_t)&ri2, &count);
  T_EXPECT_MACH_SUCCESS(kr, "mach_vm_region_recurse(0x%llx)\n", vmaddr_sub);
  ...
  vmaddr_tmp = vmaddr_sub;
  kr =
      mach_vm_remap(mach_task_self(), &vmaddr_tmp, vmsize_sub, 0,
                    VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, mach_task_self(),
                    vmaddr_sub, TRUE, &cur_prot, &max_prot, VM_INHERIT_DEFAULT);
  ...
  ri1 = ri2;
  T_EXPECT_EQ(ri1.is_submap, 1, "mapping should be nested");
  kr = mach_vm_protect(mach_task_self(), vmaddr_sub, vmsize_sub,
                       FALSE, /* set_maximum */
  ...
  kr = mach_vm_allocate(mach_task_self(), &vmaddr_tmp, vmsize_sub,
                        VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE);
  T_EXPECT_MACH_ERROR(kr, KERN_NO_SPACE,
                      "vm_allocate(OVERWRITE) fails with KERN_NO_SPACE");
  ...
  kr = mach_vm_allocate(mach_task_self(), &vmaddr_buf, vmsize_sub,
                        VM_FLAGS_ANYWHERE);
  T_EXPECT_MACH_SUCCESS(kr, "vm_allocate(0x%llx)", vmsize_sub);
  /* copy the data to avoid undue crash */
  memcpy((char *)(uintptr_t)vmaddr_buf, (char *)(uintptr_t)vmaddr_sub,
         (size_t)vmsize_sub);
  kr = mach_vm_write(mach_task_self(), vmaddr_sub, /* destination address */
  ...
  kr = mach_vm_protect(mach_task_self(), vmaddr_sub, vmsize_sub,
                       ...
                       VM_PROT_COPY | VM_PROT_READ | VM_PROT_WRITE);
  T_EXPECT_MACH_ERROR(kr, KERN_PROTECTION_FAILURE,
                      "vm_protect(0x%llx,0x%llx,VM_PROT_COPY) fails with "
...
T_DECL(shared_region_x86_writable,
       "Tests shared region PROT_WRITE is permitted on Intel only, and fails "
  ...
  kern_return_t kr;
  kr = mach_vm_protect(mach_task_self(), vmaddr_sub, vmsize_sub, FALSE,
