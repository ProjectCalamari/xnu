 ...
 */
static test_result_t successful_mincore_nested(checker_list_t *checker_list,
  ...
  for (mach_vm_size_t page_index = 0; page_index < page_count; page_index++) {
    ...
    if (checker != NULL && checker->kind == Submap) {
      ...
      test_result_t result =
          successful_mincore_nested(submap_checkers, page_address, PAGE_SIZE);
    ...
    if (checker == NULL) {
      ...
      T_QUIET;
      T_EXPECT_EQ((page_info & ~MINCORE_ANONYMOUS), 0,
...
static test_result_t successful_mincore(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
...
T_DECL(mincore, "run mincore with various vm configurations") {
  vm_tests_t tests = {
