...
static int get_kill_counts(uint32_t *buffer, size_t buffer_size, int band,
                           int flags) {
  return memorystatus_control(MEMORYSTATUS_CMD_GET_KILL_COUNTS, band, flags,
                              buffer, buffer_size);
}
...
static void spawn_and_jetsam(int32_t band) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_ZERO(error, "spawn child");
  prop.priority = band;
  prop.user_data = 0;
  error = memorystatus_control(MEMORYSTATUS_CMD_SET_PROCESS_IS_MANAGED, child,
                               1, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(error, "set child managed");
  error = memorystatus_control(MEMORYSTATUS_CMD_SET_PRIORITY_PROPERTIES, child,
                               MEMORYSTATUS_SET_PRIORITY_ASSERTION, &prop,
                               sizeof(prop));
  T_QUIET;
  T_ASSERT_POSIX_ZERO(error, "set child priority");
  error = memorystatus_control(MEMORYSTATUS_CMD_TEST_JETSAM, child, 0, NULL, 0);
  T_QUIET;
  T_ASSERT_POSIX_ZERO(error, "jetsam child");
}

#define N_TEST_BANDS 5
// Insert at head to skip idle aging
int32_t test_bands[N_TEST_BANDS] = {JETSAM_PRIORITY_IDLE_HEAD,
                                    JETSAM_PRIORITY_BACKGROUND, 35,
                                    JETSAM_PRIORITY_MAIL, 45};
int32_t expected_bands[N_TEST_BANDS] = {JETSAM_PRIORITY_IDLE,
                                        JETSAM_PRIORITY_BACKGROUND, 35,
                                        JETSAM_PRIORITY_MAIL, 45};
...
T_DECL(memorystatus_kill_counts, "jetsam kill counts", T_META_ASROOT(true)) {
  int i, j;
  ...
  for (i = 0; i < N_TEST_BANDS; i++) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_NOTNULL(buffers[i], "malloc()");
    for (j = 0; j < proc_counts[i]; j++) {
      spawn_and_jetsam(test_bands[i]);
    }
  }

  void (^get_all_kill_counts)(uint32_t **, int) =
      ^(uint32_t **buffers, int flags) {
        int i, error;
        for (i = 0; i < N_TEST_BANDS; i++) {
          error = get_kill_counts(buffers[i], BUFFER_SIZE, expected_bands[i],
                                  flags);
          T_ASSERT_POSIX_ZERO(error, "get kill counts (band %d)",
                              expected_bands[i]);
        }
      };

  /* Query for size */
  void (^check_buffer)(uint32_t **, bool) = ^(uint32_t **buffers,
                                              bool expect_missing) {
    int i;
    bool missing_proc;
    ...
    for (i = 0; i < N_TEST_BANDS; i++) {
      ...
      if (!expect_missing) {
        T_QUIET;
        T_EXPECT_LE(proc_counts[i], count,
                    "Children in band %d found in kill list.", test_bands[i]);
      }
    }
