...
static void cleanup_ripe_age(void) {
...
T_DECL(compression_sweep,
  ...
  T_QUIET;
  T_ASSERT_GT(sweep_period_secs, compressed_page_target_age_secs,
              "should sleep longer than target age");
  const int sweep_count = 3;

  vm_statistics64_data_t vm_stat_before;
  unsigned int count = HOST_VM_INFO64_COUNT;
  kern_return_t kret = host_statistics64(
      mach_host_self(), HOST_VM_INFO64, (host_info64_t)&vm_stat_before, &count);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kret, "host_statistics64");
  T_QUIET;
  T_ASSERT_EQ(count, HOST_VM_INFO64_COUNT, "host_statistics64 size");
  ...
  // rdar://71454311 (Compression sweep swap outs are flaky, should induce
  // compressions)
  T_MAYFAIL;
