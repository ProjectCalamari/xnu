...
static void willneed_tear_down(void) {
  int ret;
  kern_return_t kr;
...
T_DECL(madv_willneed, "test madvise(MADV_WILLNEED)") {
  ...
  kr = vm_allocate(mach_task_self(), &vmaddr_4willneed, vmsize_4willneed,
  ...
  fbaddr_4willneed = mmap(NULL, vmsize_4willneed, PROT_READ,
                          (MAP_FILE | MAP_SHARED), fd_4willneed, 0);
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "mincore()");
  for (size_t i = 0; i < kNumPages_4willneed; i++) {
    T_QUIET;
    T_ASSERT_BITS_SET(vec[i], MINCORE_ANONYMOUS,
                      "pre-madvise pages should be anonymous");
    T_QUIET;
    T_ASSERT_BITS_NOTSET(vec[i], MINCORE_INCORE,
                         "pre-madvise pages must not be resident");
    T_QUIET;
    T_ASSERT_BITS_NOTSET(vec[i], MINCORE_REFERENCED,
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "madvise(MADV_WILLNEED)");
  ...
  for (size_t i = 0; i < kNumPages_4willneed; i++) {
    ...
    T_ASSERT_BITS_NOTSET(vec[i], MINCORE_REFERENCED,
                         "pre-madvise pages must not be referenced");
    T_QUIET;
    T_ASSERT_BITS_NOTSET(vec[i], MINCORE_MODIFIED,
