...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              T_META_RADAR_COMPONENT_VERSION("VM"), T_META_TAG_VM_PREFERRED);
...
T_DECL(memory_entry_page_counts,
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(err, "mach_make_memory_entry()");
  T_QUIET;
  T_ASSERT_NE(memory_entry, MACH_PORT_NULL, "memory entry is non-null");

  T_LOG("Mapping memory entry");
  mach_vm_address_t addr;
  err = mach_vm_map(mach_task_self(), &addr, size, 0, VM_FLAGS_ANYWHERE,
                    memory_entry, 0, FALSE, VM_PROT_DEFAULT, VM_PROT_DEFAULT,
                    VM_INHERIT_NONE);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(err, "mach_vm_map()");

  T_LOG("Querying page counts");
  ret = mach_memory_entry_get_page_counts(memory_entry, &resident, &dirty,
                                          &swapped);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "mach_memory_entry_get_page_counts()");
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "madvise()");
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(err, "mach_vm_deallocate()");

  err = mach_port_deallocate(mach_task_self(), memory_entry);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(err, "mach_port_deallocate()");
}
