...
static void run_compress_benchmark(const test_args_t *args) {
  ...
  double compressor_throughput = 0, decompressor_throughput = 0,
         compression_ratio = 0;
  while (total_compressor_time < args->ta_duration_seconds * NSEC_PER_SEC) {
    ...
    int ret = sysctlbyname("kern.perf_compressor", &sysctl_data, &len,
                           &sysctl_data, sizeof(sysctl_data));
    if (ret < 0) {
    ...
    if (sysctl_data.bytes_processed != args->ta_buffer_size) {
      fprintf(stderr,
              "WARNING: Failed to compress the whole buffer. Only compressed "
              "%llu bytes out of %llu bytes\n",
  ...
  compressor_throughput =
      (double)total_bytes_compressed / total_compressor_time * NSEC_PER_SEC;
  printf("bytes_compressed=%llu, compressor_bytes_used=%llu\n",
         total_bytes_compressed, total_compressor_bytes_used);
  compression_ratio =
      (double)total_bytes_compressed / total_compressor_bytes_used;
  if (total_decompressor_time != 0) {
    decompressor_throughput =
        (double)total_bytes_compressed / total_decompressor_time * NSEC_PER_SEC;
  }
  printf("-----Results-----\n");
...
static void parse_arguments(int argc, const char **argv, test_args_t *args) {
...
static void print_help(const char **argv) {
  fprintf(stderr,
          "%s: [-v] <test-variant> allocation-type duration_seconds "
          "buffer_size_mb\n",
          argv[0]);
  fprintf(stderr, "\ntest variants:\n");
  fprintf(stderr, "	%s	Measure compressor throughput.\n",
          kCompressArgument);
  fprintf(stderr,
          "	%s	Measure compressor and decompressor throughput.\n",
