...
static void zba_populate_any(vm_address_t addr, vm_size_t size) {
  int rc = mprotect((void *)addr, size, PROT_READ | PROT_WRITE);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(rc, "mprotect");
}

static void zba_populate_nope(vm_address_t addr, vm_size_t size) {
#pragma unused(addr, size)
  T_FAIL("Trying to extend the storage");
  T_END;
}

static void zba_test_allow_extension(void) {
  zba_test_info.zbats_populate = zba_populate_any;
}

static void zba_test_disallow_extension(void) {
  zba_test_info.zbats_populate = zba_populate_nope;
}

static void zba_test_setup(void) {
  kern_return_t kr;
  int rc;
...
T_DECL(zone_buddy_allocator_encodings, "test the buddy allocator formulas",
  ...
  for (uint32_t o = ZBA_MAX_ORDER + 1; o-- > 0;) {
    for (vm_address_t pos = 0; pos < ZBA_CHUNK_SIZE; pos += ZBA_GRANULE << o) {
      ...
      T_QUIET;
      T_ASSERT_LT(node, 8 * sizeof(bits), "fits in bitfield: %zd", pos);
      T_QUIET;
      T_ASSERT_EQ(0, bits[node / 8] & (1 << (node % 8)), "never seen");
...
T_DECL(zone_buddy_allocator, "test the zone bits setup",
  ...
  for (pos = zba_chunk_header_size(0); pos < ZBA_CHUNK_SIZE;
       pos += ZBA_GRANULE) {
    T_QUIET;
    T_ASSERT_EQ(base + pos, zba_alloc(0, false), "alloc");
  ...
  base += ZBA_CHUNK_SIZE;
  for (pos = zba_chunk_header_size(1); pos < ZBA_CHUNK_SIZE;
       pos += ZBA_GRANULE) {
    T_QUIET;
    T_ASSERT_EQ(base + pos, zba_alloc(0, false), "alloc");
    *(uint64_t *)(base + pos) = ~0ull;
