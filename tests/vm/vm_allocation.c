...
static suite_t *create_suite(const char *name, int numoftests, test_fn_t set_up,
                             unit_test_t *tests, test_fn_t tear_down) {
  suite_t *suite = (suite_t *)malloc(sizeof(suite_t));
  T_QUIET;
  T_WITH_ERRNO;
  T_ASSERT_NOTNULL(suite, "malloc()");
...
static void destroy_suite(suite_t *suite) { free(suite); }
...
static void log_test_info(unit_test_t *unit_test, unsigned test_num) {
  if (unit_test->expected_signal) {
    logr("[BEGIN] #%04d: %s, SIGNAL(%d) expected", test_num, unit_test->name,
         unit_test->expected_signal);
  } else {
    logr("[BEGIN] #%04d: %s", test_num, unit_test->name);
  }
}

static void log_test_result(unit_test_t *unit_test, boolean_t test_passed,
                            unsigned test_num) {
  logr("[%s] #%04d: %s", test_passed ? "PASS" : "FAIL", test_num,
       unit_test->name);
}
...
void set_expected_signal(int signal) { _expected_signal = signal; }

int get_expected_signal() { return _expected_signal; }
...
static boolean_t child_terminated_normally(int child_status) {
  ...
  if (WIFEXITED(child_status)) {
    ...
    if (exit_status) {
      T_LOG("Child process unexpectedly exited with code %d.", exit_status);
    } else if (!_expected_signal) {
      normal_exit = TRUE;
    } else {
      T_LOG("Child process unexpectedly exited with zero, "
            "where signal %d is expected.",
            _expected_signal);
    }
    ...
    if (signal == _expected_signal ||
        (_expected_signal == -1 &&
         (signal == SIGBUS || signal == SIGSEGV || signal == SIGKILL))) {
      if (_quietness <= 0) {
      ...
      T_LOG("Child process unexpectedly died with signal %d.", signal);
    }
  } else {
...
static boolean_t child_test_passed(suite_t *suite, unit_test_t *unit_test) {
  int test_status;
  ...
  pid_t test_pid = fork();
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(test_pid, "fork()");
  if (!test_pid) {
   ...
   */
  if ((get_expected_signal() == 0) && (unit_test->expected_signal != 0)) {
...
#define run_suite(set_up, tests, tear_down, ...)                               \
  _run_suite((sizeof(tests) / sizeof(tests[0])), (set_up), (tests),            \
             (tear_down), __VA_ARGS__)
...
void _run_suite(int numoftests, test_fn_t set_up, UnitTests tests,
  ...
  va_start(ap, format);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(vasprintf(&name, format, ap), "vasprintf()");
  va_end(ap);
...
/* Logging verbosity. */
void set_quietness(int value) { _quietness = value; }

int get_quietness() { return _quietness; }
...
void log_aggregated_results() {
  ...
  T_ASSERT_EQ(results.passed_tests, results.numoftests,
              "%ju passed of total %ju tests", results.passed_tests,
              results.numoftests);
}
...
#define MACH_VM_ADDRESS_T(address)                                             \
...
#define assert_mach_return(kr, expected_kr, format, ...)                       \
  do {                                                                         \
    ...
    if ((kr) != (expected_kr)) {                                               \
      T_QUIET;                                                                 \
      T_ASSERT_MACH_ERROR(kr, expected_kr, format, ##__VA_ARGS__);             \
    }                                                                          \
  } while (0)
...
#define assert_mach_success(kr, format, ...)                                   \
  do {                                                                         \
    ...
    if (kr != KERN_SUCCESS) {                                                  \
      T_QUIET;                                                                 \
      T_ASSERT_MACH_SUCCESS(kr, format, ##__VA_ARGS__);                        \
    }                                                                          \
  } while (0)
...
#define assert_mach_failure(kr, format, ...)                                   \
  do {                                                                         \
    ...
    if (kr == KERN_SUCCESS) {                                                  \
      T_QUIET;                                                                 \
      T_ASSERT_NE(kr, KERN_SUCCESS, format, ##__VA_ARGS__);                    \
    }                                                                          \
  } while (0)

/* Determine if TASK_EXC_GUARD_VM_FATAL is enabled for task */
static boolean_t get_task_exc_guard_vm_fatal(void) {
  task_exc_guard_behavior_t behavior;

  assert_mach_success(task_get_exc_guard_behavior(mach_task_self(), &behavior),
...
/* Memory allocators */
typedef kern_return_t (*allocate_fn_t)(vm_map_t, mach_vm_address_t *,
                                       mach_vm_size_t, int);

/*
 * Remember any pre-reserved fixed address, which needs to be released prior to
 * an allocation.
 */
...
/*
 * If trying to allocate at a fixed address, we need to do the delayed
 * deallocate first.
 */
static void check_fixed_address(mach_vm_address_t *address,
                                mach_vm_size_t size) {
  if (fixed_vm_address != 0 && fixed_vm_address <= *address &&
...
kern_return_t wrapper_mach_vm_allocate(vm_map_t map, mach_vm_address_t *address,
                                       mach_vm_size_t size, int flags) {
  check_fixed_address(address, size);
  return mach_vm_allocate(map, address, size, flags);
}

kern_return_t wrapper_mach_vm_map(vm_map_t map, mach_vm_address_t *address,
                                  mach_vm_size_t size, int flags) {
  check_fixed_address(address, size);
  return mach_vm_map(map, address, size, (mach_vm_offset_t)0, flags,
                     MACH_PORT_NULL, (memory_object_offset_t)0, FALSE,
                     VM_PROT_DEFAULT, VM_PROT_ALL, VM_INHERIT_DEFAULT);
}

/* Should have the same behavior as when mask is zero. */
kern_return_t wrapper_mach_vm_map_4kB(vm_map_t map, mach_vm_address_t *address,
                                      mach_vm_size_t size, int flags) {
  check_fixed_address(address, size);
  return mach_vm_map(map, address, size, (mach_vm_offset_t)0xFFF, flags,
                     MACH_PORT_NULL, (memory_object_offset_t)0, FALSE,
                     VM_PROT_DEFAULT, VM_PROT_ALL, VM_INHERIT_DEFAULT);
}

kern_return_t wrapper_mach_vm_map_2MB(vm_map_t map, mach_vm_address_t *address,
                                      mach_vm_size_t size, int flags) {
  check_fixed_address(address, size);
  return mach_vm_map(map, address, size, (mach_vm_offset_t)0x1FFFFF, flags,
                     MACH_PORT_NULL, (memory_object_offset_t)0, FALSE,
                     VM_PROT_DEFAULT, VM_PROT_ALL, VM_INHERIT_DEFAULT);
}

kern_return_t memory_entry(mach_vm_size_t *size, mach_port_t *object_handle) {
  mach_vm_size_t original_size = *size;
  kern_return_t kr;

  kr = mach_make_memory_entry_64(
      mach_task_self(), size, (memory_object_offset_t)0,
      (MAP_MEM_NAMED_CREATE | VM_PROT_ALL), object_handle, 0);
  if (kr != KERN_SUCCESS) {
    return kr;
  }
  T_QUIET;
  T_ASSERT_EQ(*size, round_page(original_size),
              ...
              (uintmax_t)*size, (uintmax_t)*size, (uintmax_t)original_size,
              (uintmax_t)original_size);
  return KERN_SUCCESS;
}

kern_return_t wrapper_mach_vm_map_named_entry(vm_map_t map,
  ...
  check_fixed_address(address, size);
  kr = mach_vm_map(map, address, size, (mach_vm_offset_t)0, flags,
                   object_handle, (memory_object_offset_t)0, FALSE,
                   VM_PROT_DEFAULT, VM_PROT_ALL, VM_INHERIT_DEFAULT);
  assert_mach_success(mach_port_deallocate(mach_task_self(), object_handle),
...
static struct {
  ...
  const char *description;
    ...
    {wrapper_mach_vm_map_4kB, "mach_vm_map() "
                              "(4 kB address alignment)"},
    {wrapper_mach_vm_map_2MB, "mach_vm_map() "
                              "(2 MB address alignment)"},
    {wrapper_mach_vm_map_named_entry, "mach_vm_map() (named "
                                      "entry, zero mask)"},
};
...
static struct {
  ...
  const char *description;
    ...
    {VM_FLAGS_ANYWHERE, "unspecified"},
    {VM_FLAGS_FIXED, "fixed"},
};
...
static int numofalignments =
    sizeof(address_alignments) / sizeof(*address_alignments);
...
static struct {
  ...
  const char *description;
    ...
    {VMCOPY_MODIFY_SHARED_COPIED, "modify vm_copy source's shared "
                                  "or copied from/to region"},
};
static int numofvmcopyactions =
    sizeof(vmcopy_actions) / sizeof(vmcopy_actions[0]);
...
static void set_allocator(allocate_fn_t allocate) { _allocator = allocate; }

static allocate_fn_t get_allocator() { return _allocator; }

static void set_vm_size(mach_vm_size_t size) { _vm_size = size; }

static mach_vm_size_t get_vm_size() { return _vm_size; }

static void set_address_flag(int flag) { _address_flag = flag; }

static int get_address_flag() { return _address_flag; }

static void set_address_alignment(boolean_t alignment) {
  _address_alignment = alignment;
}

static boolean_t get_address_alignment() { return _address_alignment; }

static void set_vm_address(mach_vm_address_t address) { _vm_address = address; }

static mach_vm_address_t get_vm_address() { return _vm_address; }

static void set_already_deallocated_vm_page(mach_vm_address_t address) {
  _already_deallocated_vm_page = address;
}

static mach_vm_address_t get_already_deallocated_vm_page() {
  return _already_deallocated_vm_page;
}

static void set_buffer_size(mach_vm_size_t size) { _buffer_size = size; }

static mach_vm_size_t get_buffer_size() { return _buffer_size; }

static void set_buffer_address(mach_vm_address_t address) {
  _buffer_address = address;
}

static mach_vm_address_t get_buffer_address() { return _buffer_address; }

static void set_buffer_offset(int offset) { _buffer_offset = offset; }

static int get_buffer_offset() { return _buffer_offset; }

static void set_already_deallocated_buffer_page(mach_vm_address_t address) {
  _already_deallocated_buffer_page = address;
}

static mach_vm_address_t get_already_deallocated_buffer_page() {
  return _already_deallocated_buffer_page;
}

static void set_vmcopy_post_action(int action) { _vmcopy_post_action = action; }

static int get_vmcopy_post_action() { return _vmcopy_post_action; }
...
static test_info_t test_info[] = {{"allocate", &flag_run_allocate_test},
...
static void process_options(test_option_t options) {
  test_info_t *tp;
 ...
 * memory range. */
mach_vm_size_t aligned_size(mach_vm_address_t address, mach_vm_size_t size) {
...
static inline void assert_aligned_address(mach_vm_address_t address) {
  T_QUIET;
  T_ASSERT_EQ((address & get_mask()), 0ull,
...
static inline void assert_trunc_address(mach_vm_address_t address,
                                        mach_vm_address_t trunc_address) {
  T_QUIET;
  T_ASSERT_EQ(trunc_address, (address & ~get_mask()),
              "Address "
              "0x%jx is unexpectedly not truncated to address 0x%jx.",
              (uintmax_t)address, (uintmax_t)trunc_address);
}

static inline void assert_address_value(mach_vm_address_t address,
  ...
  if (MACH_VM_ADDRESS_T(address) != marker) {
    T_ASSERT_FAIL("Address 0x%jx unexpectedly has value 0x%jx, "
                  "instead of 0x%jx.",
                  (uintmax_t)address, (uintmax_t)MACH_VM_ADDRESS_T(address),
                  (uintmax_t)marker);
  }
}

void assert_allocate_return(mach_vm_address_t *address, mach_vm_size_t size,
                            int address_flag, kern_return_t expected_kr) {
  assert_mach_return(
      get_allocator()(mach_task_self(), address, size, address_flag),
      expected_kr, "Allocator");
}

void assert_allocate_success(mach_vm_address_t *address, mach_vm_size_t size,
                             int address_flag) {
  assert_allocate_return(address, size, address_flag, KERN_SUCCESS);
}

void assert_deallocate_return(mach_vm_address_t address, mach_vm_size_t size,
                              kern_return_t expected_kr) {
  assert_mach_return(mach_vm_deallocate(mach_task_self(), address, size),
                     expected_kr, "mach_vm_deallocate()");
}
...
void assert_read_return(mach_vm_address_t address, mach_vm_size_t size,
                        vm_offset_t *data, mach_msg_type_number_t *data_size,
                        kern_return_t expected_kr) {
  assert_mach_return(
      mach_vm_read(mach_task_self(), address, size, data, data_size),
      expected_kr, "mach_vm_read()");
}

void assert_read_success(mach_vm_address_t address, mach_vm_size_t size,
                         vm_offset_t *data, mach_msg_type_number_t *data_size) {
  assert_read_return(address, size, data, data_size, KERN_SUCCESS);
  T_QUIET;
  T_ASSERT_EQ((mach_vm_size_t)*data_size, size,
              "Returned buffer size 0x%jx "
              "(%ju) is unexpectedly different from source size 0x%jx "
              "(%ju).",
              (uintmax_t)*data_size, (uintmax_t)*data_size, (uintmax_t)size,
              (uintmax_t)size);
}

void assert_write_return(mach_vm_address_t address, vm_offset_t data,
                         mach_msg_type_number_t data_size,
                         kern_return_t expected_kr) {
  assert_mach_return(mach_vm_write(mach_task_self(), address, data, data_size),
                     expected_kr, "mach_vm_write()");
}

void assert_write_success(mach_vm_address_t address, vm_offset_t data,
                          mach_msg_type_number_t data_size) {
  assert_write_return(address, data, data_size, KERN_SUCCESS);
}

void assert_allocate_copy_return(mach_vm_address_t source, mach_vm_size_t size,
                                 mach_vm_address_t *dest,
                                 kern_return_t expected_kr) {
  assert_allocate_success(dest, size, VM_FLAGS_ANYWHERE);
  assert_mach_return(mach_vm_copy(mach_task_self(), source, size, *dest),
                     expected_kr, "mach_vm_copy()");
}
void assert_allocate_copy_success(mach_vm_address_t source, mach_vm_size_t size,
                                  mach_vm_address_t *dest) {
  assert_allocate_copy_return(source, size, dest, KERN_SUCCESS);
}

void assert_copy_return(mach_vm_address_t source, mach_vm_size_t size,
                        mach_vm_address_t dest, kern_return_t expected_kr) {
  assert_mach_return(mach_vm_copy(mach_task_self(), source, size, dest),
                     expected_kr, "mach_vm_copy()");
}

void assert_copy_success(mach_vm_address_t source, mach_vm_size_t size,
                         mach_vm_address_t dest) {
  assert_copy_return(source, size, dest, KERN_SUCCESS);
}

void assert_copy_failure(mach_vm_address_t source, mach_vm_size_t size,
                         mach_vm_address_t dest) {
  assert_mach_failure(mach_vm_copy(mach_task_self(), source, size, dest),
                      "mach_vm_copy()");
}
...
void filter_addresses_do_else(address_filter_t filter, boolean_t reversed,
...
void no_action(mach_vm_address_t i, mach_vm_address_t value) {}

void read_zero(mach_vm_address_t i, mach_vm_address_t value) {
  assert_address_value(i, 0);
}

void verify_address(mach_vm_address_t i, mach_vm_address_t value) {
  assert_address_value(i, value);
}

void write_address(mach_vm_address_t i, mach_vm_address_t value) {
  MACH_VM_ADDRESS_T(i) = value;
}

/* Various patterns. */
boolean_t empty(mach_vm_address_t i) { return FALSE; }

boolean_t checkerboard(mach_vm_address_t i) {
  return !((i / vm_address_size) & 0x1);
}

boolean_t page_ends(mach_vm_address_t i) {
...
void set_up_allocator() {
  T_QUIET;
  T_ASSERT_TRUE(allocators_idx >= 0 && allocators_idx < numofallocators,
...
mach_vm_address_t get_fixed_address(mach_vm_size_t size) {
  /* mach_vm_map() starts looking for an address at 0x0. */
  mach_vm_address_t address = 0x0;

  /*
   * The tests seem to have some funky off by one allocations. To avoid
   * problems, we'll bump anything non-zero to have at least an extra couple
   * pages.
   */
  if (size != 0) {
  ...
  /*
   * Keep the memory allocated, otherwise the logv()/printf() activity sprinkled
   * in these tests can cause malloc() to use the desired range and tests will
   * randomly fail. The allocate routines will do the delayed vm_deallocate() to
   * free the fixed memory just before allocation testing in the wrapper.
   */
  T_QUIET;
  T_ASSERT_EQ(fixed_vm_address, 0ull, "previous fixed address not used");
  T_QUIET;
  T_ASSERT_EQ(fixed_vm_size, 0ull, "previous fixed size not used");
...
void set_up_vm_address(mach_vm_size_t size) {
  T_QUIET;
  T_ASSERT_TRUE(flags_idx >= 0 && flags_idx < numofflags,
                "Invalid address_flags[] index: %d.", flags_idx);
  T_QUIET;
  T_ASSERT_TRUE(alignments_idx >= 0 && alignments_idx < numofalignments,
  ...
  if (!(get_address_flag() & VM_FLAGS_ANYWHERE)) {
    ...
    logv("Looking for fixed %saligned address for allocation "
         "of 0x%jx (%ju) byte%s...",
         aligned ? "" : "un", (uintmax_t)size, (uintmax_t)size,
         (size == 1) ? "" : "s");
    ...
    logv("Found %saligned fixed address 0x%jx.", aligned ? "" : "un",
         (uintmax_t)address);
  } else {
...
void set_up_vm_size() {
  T_QUIET;
  T_ASSERT_TRUE(sizes_idx >= 0 && sizes_idx < numofsizes,
                "Invalid vm_sizes[] index: %d.", sizes_idx);
  set_vm_size(vm_sizes[sizes_idx].size);
}

void set_up_buffer_size() {
  T_QUIET;
  T_ASSERT_TRUE(buffer_sizes_idx >= 0 && buffer_sizes_idx < numofsizes,
                "Invalid vm_sizes[] index: %d.", buffer_sizes_idx);
  set_buffer_size(vm_sizes[buffer_sizes_idx].size);
}

void set_up_buffer_offset() {
  T_QUIET;
  T_ASSERT_TRUE(offsets_idx >= 0 && offsets_idx < numofoffsets,
                "Invalid buffer_offsets[] index: %d.", offsets_idx);
  set_buffer_offset(buffer_offsets[offsets_idx].offset);
}

void set_up_vmcopy_action() {
  T_QUIET;
  T_ASSERT_TRUE(vmcopy_action_idx >= 0 &&
                    vmcopy_action_idx < numofvmcopyactions,
                "Invalid vmcopy_actions[] index: %d.", vmcopy_action_idx);
  set_vmcopy_post_action(vmcopy_actions[vmcopy_action_idx].action);
}

void set_up_allocator_and_vm_size() {
  set_up_allocator();
  set_up_vm_size();
}

void set_up_vm_variables() {
  set_up_vm_size();
  set_up_vm_address(get_vm_size());
}

void set_up_allocator_and_vm_variables() {
  set_up_allocator();
  set_up_vm_variables();
}

void set_up_buffer_variables() {
  set_up_buffer_size();
  set_up_buffer_offset();
}

void set_up_copy_shared_mode_variables() { set_up_vmcopy_action(); }

/*******************************/
/* Allocation set up functions */
/*******************************/

static void log_allocation(mach_vm_size_t size, int flags,
                           mach_vm_address_t address, const char *message) {
  if (flags & VM_FLAGS_ANYWHERE) {
    logv("Allocating 0x%jx (%ju) byte%s %s...", (uintmax_t)size,
         (uintmax_t)size, (size == 1) ? "" : "s", message);
  } else {
    logv("Allocating 0x%jx (%ju) byte%s at address 0x%jx %s...",
         (uintmax_t)size, (uintmax_t)size, (size == 1) ? "" : "s",
         (uintmax_t)address, message);
  }
}

/* Allocate VM region of given size. */
void allocate(mach_vm_size_t size) {
  ...
  logv("Memory of rounded size 0x%jx (%ju) allocated at "
       "address 0x%jx.",
       (uintmax_t)round_page(size), (uintmax_t)round_page(size),
       (uintmax_t)address);
  ...
  if (!(flag & VM_FLAGS_ANYWHERE)) {
    ...
    logv("Address 0x%jx is correctly truncated to allocated "
         "address 0x%jx.",
...
void allocate_buffer(mach_vm_size_t buffer_size) {
  ...
  logv("Memory of rounded size 0x%jx (%ju) allocated at "
       "address 0x%jx.",
       (uintmax_t)round_page(buffer_size), (uintmax_t)round_page(buffer_size),
       (uintmax_t)data);
  data += get_buffer_offset();
  T_QUIET;
  T_ASSERT_EQ((mach_vm_address_t)(vm_offset_t)data, data,
              "Address 0x%jx "
...
void set_up_vm_variables_and_allocate() {
  set_up_vm_variables();
  allocate(get_vm_size());
}

void set_up_allocator_and_vm_variables_and_allocate() {
  set_up_allocator();
  set_up_vm_variables_and_allocate();
}

void set_up_vm_variables_and_allocate_extra_page() {
  set_up_vm_size();
...
void set_up_buffer_variables_and_allocate_extra_page() {
...
void set_up_vm_and_buffer_variables_allocate_for_writing() {
...
void set_up_vm_and_buffer_variables_allocate_for_copying() {
...
void deallocate_range(mach_vm_address_t address, mach_vm_size_t size) {
  logv("Deallocating 0x%jx (%ju) byte%s at address 0x%jx...", (uintmax_t)size,
       (uintmax_t)size, (size == 1) ? "" : "s", (uintmax_t)address);
 ...
 */
void deallocate_range_except_page(
    mach_vm_address_t address, mach_vm_size_t size,
  ...
  if (already_deallocated_address != 0) {
    ...
    mach_vm_address_t already_deallocated_end =
        already_deallocated_address + vm_page_size;
...
void deallocate() {
  deallocate_range_except_page(get_vm_address(), get_vm_size(),
 ...
 * start addresses. */
void deallocate_extra_page() {
  /* Set the address and size to their original allocation
   *  values. */
  deallocate_range_except_page(mach_vm_trunc_page(get_vm_address()),
                               get_vm_size() + 1,
                               get_already_deallocated_vm_page());
  set_already_deallocated_vm_page(0x0);
}
...
void deallocate_vm_and_buffer() {
  deallocate_range_except_page(mach_vm_trunc_page(get_vm_address()),
                               get_vm_size() + 1,
                               get_already_deallocated_vm_page());
  set_already_deallocated_vm_page(0x0);

  deallocate_range_except_page(mach_vm_trunc_page(get_buffer_address()),
 ...
 */
void deallocate_vm_page_early(mach_vm_address_t address) {
  ...
  T_QUIET;
  T_ASSERT_EQ(get_already_deallocated_vm_page(), 0ull,
              "deallocate_vm_page_early can only be used once per test");
  T_QUIET;
  T_ASSERT_EQ(address, mach_vm_trunc_page(address),
              "deallocate_vm_page_early address must be page aligned");
  T_QUIET;
  T_ASSERT_TRUE(
      address >= vm_start && address + vm_page_size <= vm_end,
 ...
 */
void deallocate_buffer_page_early(mach_vm_address_t address) {
  mach_vm_address_t buffer_start = mach_vm_trunc_page(get_buffer_address());
  mach_vm_address_t buffer_end = mach_vm_round_page(
      buffer_start + get_buffer_size() + get_buffer_offset());
  T_QUIET;
  T_ASSERT_EQ(get_already_deallocated_buffer_page(), 0ull,
  ...
  T_QUIET;
  T_ASSERT_TRUE(
      address >= buffer_start && address + vm_page_size <= buffer_end,
...
void read_deallocate() {
  ...
  logv("Reading 0x%jx (%ju) byte%s at address 0x%jx...", (uintmax_t)size,
       (uintmax_t)size, (size == 1) ? "" : "s", (uintmax_t)address);
  assert_read_success(address, size, &read_address, &read_size);
  logv("Memory of size 0x%jx (%ju) read into buffer of "
       "address 0x%jx.",
  ...
  T_QUIET;
  T_ASSERT_EQ((vm_offset_t)(mach_vm_address_t)read_address, read_address,
              "Address 0x%jx unexpectedly overflows to 0x%jx when cast "
              "as mach_vm_address_t type.",
              (uintmax_t)read_address,
              (uintmax_t)(mach_vm_address_t)read_address);
  T_QUIET;
  T_ASSERT_EQ((mach_msg_type_number_t)(mach_vm_size_t)read_size, read_size,
              "Size 0x%jx (%ju) unexpectedly overflows to 0x%jx (%ju) "
              "when cast as mach_vm_size_t type.",
              (uintmax_t)read_size, (uintmax_t)read_size,
              (uintmax_t)(mach_vm_size_t)read_size,
              (uintmax_t)(mach_vm_size_t)read_size);
...
void set_up_vm_variables_allocate_read_deallocate() {
  set_up_vm_variables_and_allocate_extra_page();
  read_deallocate();
}
...
void write_buffer() {
  ...
  mach_msg_type_number_t buffer_size =
      (mach_msg_type_number_t)get_buffer_size();

  logv("Writing buffer of address 0x%jx and size 0x%jx (%ju), on "
       "memory at address 0x%jx...",
       (uintmax_t)data, (uintmax_t)buffer_size, (uintmax_t)buffer_size,
       (uintmax_t)address);
...
void set_up_vm_and_buffer_variables_allocate_write() {
  set_up_vm_and_buffer_variables_allocate_for_writing();
  write_buffer();
}
...
void copy_deallocate(void) {
  ...
  logv("Copying 0x%jx (%ju) byte%s at address 0x%jx...", (uintmax_t)size,
       (uintmax_t)size, (size == 1) ? "" : "s", (uintmax_t)source);
  assert_allocate_copy_success(source, size, &dest);
  logv("Memory of size 0x%jx (%ju) copy into region of "
       "address 0x%jx.",
   ...
   *  setting the global address from the buffer's. */
  T_QUIET;
  T_ASSERT_EQ((mach_vm_address_t)(vm_offset_t)dest, dest,
              "Address 0x%jx unexpectedly overflows to 0x%jx when cast "
              "as mach_vm_address_t type.",
...
void copy_region() {
  ...
  logv("Copying memory region of address 0x%jx and size 0x%jx (%ju), on "
       "memory at address 0x%jx...",
...
void set_up_vm_variables_allocate_copy_deallocate() {
  set_up_vm_variables_and_allocate_extra_page();
  copy_deallocate();
}
 ...
 * into the destination memory. */
void set_up_source_and_dest_variables_allocate_copy() {
...
void set_up_vm_variables_allocate_protect(vm_prot_t protection,
                                          const char *protection_name) {
  set_up_vm_variables_and_allocate_extra_page();
  mach_vm_size_t size = get_vm_size();
  mach_vm_address_t address = get_vm_address();

  logv("Setting %s-protection on 0x%jx (%ju) byte%s at address "
       "0x%jx...",
       protection_name, (uintmax_t)size, (uintmax_t)size,
       (size == 1) ? "" : "s", (uintmax_t)address);
  assert_mach_success(
      mach_vm_protect(mach_task_self(), address, size, FALSE, protection),
      "mach_vm_protect()");
  logv("Region %s-protected.", protection_name);
}

void set_up_vm_variables_allocate_readprotect() {
  set_up_vm_variables_allocate_protect(VM_PROT_WRITE, "read");
}

void set_up_vm_variables_allocate_writeprotect() {
...
void test_nonzero_address_iff_nonzero_size() {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();

  T_QUIET;
  T_ASSERT_TRUE((address && size) || (!address && !size),
                "Address 0x%jx is unexpectedly %szero.", (uintmax_t)address,
                address ? "non" : "");
  logv("Address 0x%jx is %szero as expected.", (uintmax_t)address,
       size ? "non" : "");
}

/* Allocated address is aligned. */
void test_aligned_address() {
  mach_vm_address_t address = get_vm_address();
...
void verify_pattern(address_filter_t filter, boolean_t reversed,
                    mach_vm_address_t address, mach_vm_size_t size,
                    const char *pattern_name) {
  logv("Verifying %s pattern on region of address 0x%jx "
       "and size 0x%jx (%ju)...",
       pattern_name, (uintmax_t)address, (uintmax_t)size, (uintmax_t)size);
  filter_addresses_do_else(filter, reversed, address, size, verify_address,
                           read_zero, address);
  logv("Pattern verified.");
}

void write_pattern(address_filter_t filter, boolean_t reversed,
                   mach_vm_address_t address, mach_vm_size_t size,
                   const char *pattern_name) {
  logv("Writing %s pattern on region of address 0x%jx "
       "and size 0x%jx (%ju)...",
       pattern_name, (uintmax_t)address, (uintmax_t)size, (uintmax_t)size);
  filter_addresses_do_else(filter, reversed, address, size, write_address,
                           no_action, address);
  logv("Pattern written.");
}

void write_and_verify_pattern(address_filter_t filter, boolean_t reversed,
                              mach_vm_address_t address, mach_vm_size_t size,
                              const char *pattern_name) {
  logv("Writing and verifying %s pattern on region of "
...
void test_zero_filled() {
  verify_pattern(empty, FALSE, mach_vm_trunc_page(get_vm_address()),
                 aligned_size(get_vm_address(), get_vm_size()), "zero-filled");
}

void test_write_address_filled() {
  write_and_verify_pattern(empty, TRUE, get_vm_address(),
                           round_page(get_vm_size()), "address-filled");
}

void test_write_checkerboard() {
  write_and_verify_pattern(checkerboard, FALSE, get_vm_address(),
                           round_page(get_vm_size()), "checkerboard");
}

void test_write_reverse_checkerboard() {
  write_and_verify_pattern(checkerboard, TRUE, get_vm_address(),
                           round_page(get_vm_size()), "reverse checkerboard");
}

void test_write_page_ends() {
  write_and_verify_pattern(page_ends, FALSE, get_vm_address(),
                           round_page(get_vm_size()), "page ends");
}

void test_write_page_interiors() {
  write_and_verify_pattern(page_ends, TRUE, get_vm_address(),
                           round_page(get_vm_size()), "page interiors");
}
...
void test_reallocate_pages() {
  ...
  logv("Reallocating pages in allocated region of address 0x%jx "
       "and size 0x%jx (%ju)...",
  ...
  logv("Returned expected error at each page: %s.",
       mach_error_string(KERN_NO_SPACE));
}

/* Allocating in VM_MAP_NULL fails. */
void test_allocate_in_null_map() {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  int flag = get_address_flag();

  log_allocation(size, flag, address, "in NULL VM map");
  assert_mach_return(get_allocator()(VM_MAP_NULL, &address, size, flag),
                     MACH_SEND_INVALID_DEST, "Allocator");
  logv("Returned expected error: %s.",
       mach_error_string(MACH_SEND_INVALID_DEST));
}

/* Allocating with non-user flags fails. */
void test_allocate_with_kernel_flags() {
  allocate_fn_t allocator = get_allocator();
  vm_map_t this_task = mach_task_self();
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  ...
  int bad_flag, i;
  kern_return_t kr;
  int valid_flags = VM_FLAGS_USER_ALLOCATE | VM_FLAGS_USER_MAP |
                    VM_FLAGS_USER_REMAP | VM_FLAGS_ALIAS_MASK;
  ...
  logv("Returned expected error with each invalid flag: %s.",
 ...
 *  arguments. */
void test_mach_vm_map_protection_inheritance_error() {
  kern_return_t kr;
  vm_map_t my_task = mach_task_self();
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  vm_map_offset_t mask = (get_allocator() == wrapper_mach_vm_map ||
                          get_allocator() == wrapper_mach_vm_map_named_entry)
                             ? (mach_vm_offset_t)0
                             : (mach_vm_offset_t)get_mask();
  int flag = get_address_flag();
  mach_port_t object_handle = MACH_PORT_NULL;
  vm_prot_t cur_protections[] = {VM_PROT_DEFAULT,
                                 (VM_PROT_ALL | VM_PROT_ALLEXEC) + 1,
                                 ~VM_PROT_IS_MASK, INT_MAX};
  vm_prot_t max_protections[] = {VM_PROT_ALL,
                                 (VM_PROT_ALL | VM_PROT_ALLEXEC) + 1,
                                 ~VM_PROT_IS_MASK, INT_MAX};
  vm_inherit_t inheritances[] = {VM_INHERIT_DEFAULT, VM_INHERIT_LAST_VALID + 1,
                                 UINT_MAX};
  int i, j, k;

  if (get_allocator() == wrapper_mach_vm_map_named_entry) {
    assert_mach_success(memory_entry(&size, &object_handle),
                        "mach_make_memory_entry_64()");
  }
  log_allocation(size, flag, address,
                 "with various invalid protection/inheritance arguments");
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      for (k = 0; k < 3; k++) {
        ...
        kr = mach_vm_map(my_task, &address, size, mask, flag, object_handle,
                         (memory_object_offset_t)0, FALSE, cur_protections[i],
                         max_protections[j], inheritances[k]);
        assert_mach_return(kr, KERN_INVALID_ARGUMENT,
                           ...
                           cur_protections[i], max_protections[j],
                           inheritances[k]);
      }
    }
  }
  logv("Returned expected error in each case: %s.",
       mach_error_string(KERN_INVALID_ARGUMENT));
}

/* mach_vm_map() with unspecified address fails if the starting
 *  address overflows when rounded up to a boundary value. */
void test_mach_vm_map_large_mask_overflow_error() {
  ...
  logv("Allocating 0x%jx (%ju) byte%s at an unspecified address "
       "starting at 0x%jx with mask 0x%jx...",
       (uintmax_t)size, (uintmax_t)size, (size == 1) ? "" : "s",
       (uintmax_t)address, (uintmax_t)mask);
  assert_mach_return(
      mach_vm_map(mach_task_self(), &address, size, mask, VM_FLAGS_ANYWHERE,
                  MACH_PORT_NULL, (memory_object_offset_t)0, FALSE,
                  VM_PROT_DEFAULT, VM_PROT_ALL, VM_INHERIT_DEFAULT),
...
void allocate_edge_size(mach_vm_address_t *address, mach_vm_size_t size,
                        kern_return_t expected_kr) {
...
void test_allocate_zero_size() {
  mach_vm_address_t address = 0x0;
  /* mach_vm_map() cannot allocate 0 bytes at an unspecified
   *  address, see 8003930. Other allocators succeed. */
  kern_return_t kr_expected = (get_allocator() != wrapper_mach_vm_allocate)
...
void test_allocate_invalid_large_size() {
  ...
  if (get_allocator() != wrapper_mach_vm_map_named_entry) {
     ...
     *  - 4 kb. */
    mach_port_t object_handle = MACH_PORT_NULL;
    logv("Creating named entry of 0x%jx (%ju) bytes...", (uintmax_t)size,
         (uintmax_t)size);
    assert_mach_return(mach_make_memory_entry_64(
                           mach_task_self(), &size, (memory_object_offset_t)0,
                           (MAP_MEM_NAMED_CREATE | VM_PROT_ALL), &object_handle,
                           0),
 ...
 * page-aligned value. */
void test_allocate_overflowing_size() {
  mach_vm_address_t address = 0x0;

  allocate_edge_size(&address, (mach_vm_size_t)UINTMAX_MAX,
                     KERN_INVALID_ARGUMENT);
}
...
void test_allocate_at_zero() {
  mach_vm_address_t address = 0x0;
  mach_vm_size_t size = get_vm_size();

  kern_return_t kr_expected = size ? KERN_INVALID_ADDRESS
                              : (get_allocator() != wrapper_mach_vm_allocate)
                                  ? KERN_INVALID_ARGUMENT
                                  : KERN_SUCCESS;

  logv("Allocating 0x%jx (%ju) byte%s at address 0x0...", (uintmax_t)size,
       (uintmax_t)size, (size == 1) ? "" : "s");
...
void test_allocate_2MB_boundary_unaligned_page_aligned_address() {
  mach_vm_size_t size = get_vm_size();

  mach_vm_address_t address =
      get_fixed_address(size + vm_page_size) + vm_page_size;
  logv("Found 2 MB boundary-unaligned, page aligned address "
       "0x%jx.",
       (uintmax_t)address);
  ...
  kern_return_t kr_expected =
      (!size && get_allocator() != wrapper_mach_vm_allocate)
          ? KERN_INVALID_ARGUMENT
      : (get_allocator() == wrapper_mach_vm_map_2MB) ? KERN_NO_SPACE
                                                     : KERN_SUCCESS;
  logv("Allocating 0x%jx (%ju) byte%s at address 0x%jx...", (uintmax_t)size,
       (uintmax_t)size, (size == 1) ? "" : "s", (uintmax_t)address);
  assert_allocate_return(&address, size, VM_FLAGS_FIXED, kr_expected);
  logv("Returned expected value: %s.", mach_error_string(kr_expected));
  if (kr_expected == KERN_SUCCESS) {
...
void test_allocate_page_with_highest_address_hint() {
  /* Highest valid page-aligned address. */
  mach_vm_address_t address = (mach_vm_size_t)UINTMAX_MAX - vm_page_size + 1;

  logv("Allocating one page with unspecified address, but hint at "
       "0x%jx...",
       (uintmax_t)address);
  if (get_allocator() == wrapper_mach_vm_allocate) {
    ...
    assert_allocate_return(&address, vm_page_size, VM_FLAGS_ANYWHERE,
                           KERN_NO_SPACE);
...
void test_allocate_first_fit_pages() {
  allocate_fn_t allocator = get_allocator();
  ...
  logv("Looking for first fit address for allocating one "
       "page...");
  ...
  logv("Looking for next higher first fit address for allocating "
       "one page...");
...
void access_deallocated_range_address(mach_vm_address_t address,
                                      const char *position) {
  logv("Will deallocate and read from %s 0x%jx of deallocated range...",
       position, (uintmax_t)address);
  deallocate();
...
void test_access_deallocated_range_start() {
  access_deallocated_range_address(get_vm_address(), "start");
}

/* Middle of deallocated range is inaccessible. */
void test_access_deallocated_range_middle() {
  access_deallocated_range_address(
      get_vm_address() + (round_page(get_vm_size()) >> 1), "middle");
}

/* End of deallocated range is inaccessible. */
void test_access_deallocated_range_end() {
  access_deallocated_range_address(
      round_page(get_vm_size()) - vm_address_size + get_vm_address(), "end");
}
 ...
 * rounding up. */
void test_deallocate_suicide() {
  mach_vm_address_t address = 0x0;
  mach_vm_size_t size = (mach_vm_size_t)UINTMAX_MAX - vm_page_size + 1;

  logv("Deallocating 0x%jx (%ju) bytes at address 0x%jx...", (uintmax_t)size,
       (uintmax_t)size, (uintmax_t)address);
...
void test_write_pattern_deallocate_reallocate_zero_filled() {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();

  write_pattern(page_ends, FALSE, address, size, "page ends");
  logv("Deallocating, then Allocating 0x%jx (%ju) byte%s at address 0x%jx...",
       (uintmax_t)size, (uintmax_t)size, (size == 1) ? "" : "s",
       (uintmax_t)address);
  deallocate();
...
void test_deallocate_zero_size_ranges() {
  int i;
  kern_return_t kr;
  vm_map_t this_task = mach_task_self();
...
void test_deallocate_wrapped_around_ranges() {
  int i;
  kern_return_t kr;
  vm_map_t this_task = mach_task_self();
  struct {
  ...
  logv("Deallocating various memory ranges wrapping around the "
       "address space...");
  ...
  logv("Returned expected error on each range: %s.",
       mach_error_string(KERN_INVALID_ARGUMENT));
}

/* Deallocating in VM_MAP_NULL fails. */
void test_deallocate_in_null_map() {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  ...
  logv("Deallocating 0x%jx (%ju) byte%s at address 0x%jx in NULL VM map...",
       (uintmax_t)size, (uintmax_t)size, (size == 1) ? "" : "s",
       (uintmax_t)address);
  assert_mach_return(mach_vm_deallocate(VM_MAP_NULL, address, size),
                     MACH_SEND_INVALID_DEST, "mach_vm_deallocate()");
  logv("Returned expected error: %s.",
       mach_error_string(MACH_SEND_INVALID_DEST));
}
...
void test_read_address_offset() {
  ...
  if (size < vm_page_size * 2 || get_address_alignment()) {
    ...
    T_QUIET;
    T_ASSERT_EQ(((address - 1) & (vm_page_size - 1)), 0ull,
                "Buffer "
                "address 0x%jx does not have the expected boundary "
                "offset of 1.",
                (uintmax_t)address);
    logv("Buffer address 0x%jx has the expected boundary "
         "offset of 1.",
         (uintmax_t)address);
  }
}
...
void test_read_null_map() {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  vm_offset_t read_address;
  mach_msg_type_number_t read_size;

  logv("Reading 0x%jx (%ju) byte%s at address 0x%jx in NULL VM "
       "map...",
       (uintmax_t)size, (uintmax_t)size, (size == 1) ? "" : "s",
       (uintmax_t)address);
  assert_mach_return(
      mach_vm_read(VM_MAP_NULL, address, size, &read_address, &read_size),
...
void test_read_partially_deallocated_range() {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  ...
  logv("Deallocating a mid-range page at address 0x%jx...",
       (uintmax_t)mid_point);
...
void test_read_partially_unreadable_range() {
  ...
  kern_return_t kr_expected = (size < vm_page_size * 2)
                                  ? KERN_INVALID_ADDRESS
                                  : KERN_PROTECTION_FAILURE;

  logv("Read-protecting a mid-range page at address 0x%jx...",
       (uintmax_t)mid_point);
  assert_mach_success(mach_vm_protect(mach_task_self(), mid_point, vm_page_size,
                                      FALSE, VM_PROT_WRITE),
                      "mach_vm_protect()");
...
void read_edge_size(mach_vm_size_t size, kern_return_t expected_kr) {
  int i;
  kern_return_t kr;
  vm_map_t this_task = mach_task_self();
  mach_vm_address_t addresses[] = {
  ...
  logv("Reading 0x%jx (%ju) bytes at various addresses...", (uintmax_t)size,
       (uintmax_t)size);
  for (i = 0; i < numofaddresses; i++) {
...
void test_read_wrapped_around_ranges() {
  ...
  struct {
    ...
    mach_vm_size_t size;
      ...
      {(mach_vm_address_t)(UINTMAX_MAX - UINT_MAX + 1),
       (mach_vm_size_t)UINT_MAX},
      {(mach_vm_address_t)(UINTMAX_MAX - UINT_MAX + vm_page_size),
       (mach_vm_size_t)(UINT_MAX - vm_page_size + 1)},
      {(mach_vm_address_t)UINTMAX_MAX - vm_page_size + 1, vm_page_size},
      {(mach_vm_address_t)UINTMAX_MAX, 1},
  };
  int numofranges = sizeof(ranges) / sizeof(ranges[0]);
  ...
  logv("Reading various memory ranges wrapping around the "
       "address space...");
  for (i = 0; i < numofranges; i++) {
    kr = mach_vm_read(this_task, ranges[i].address, ranges[i].size,
...
void write_read_verify_pattern(address_filter_t filter, boolean_t reversed,
                               const char *pattern_name) {
  mach_vm_address_t address = get_vm_address();
  ...
  logv("Verifying %s pattern on buffer of "
       "address 0x%jx and size 0x%jx (%ju)...",
       pattern_name, (uintmax_t)read_address, (uintmax_t)read_size,
       (uintmax_t)read_size);
  filter_addresses_do_else(filter, reversed, read_address, read_size,
                           verify_address, read_zero, address);
  logv("Pattern verified on destination buffer.");
}

void test_read_address_filled() {
  write_read_verify_pattern(empty, TRUE, "address-filled");
}

void test_read_checkerboard() {
  write_read_verify_pattern(checkerboard, FALSE, "checkerboard");
}

void test_read_reverse_checkerboard() {
...
void test_write_null_map() {
  ...
  logv("Writing buffer of address 0x%jx and size 0x%jx (%ju), on "
       ...
       (uintmax_t)data, (uintmax_t)buffer_size, (uintmax_t)buffer_size,
       (uintmax_t)address);
  assert_mach_return(mach_vm_write(VM_MAP_NULL, address, data, buffer_size),
                     MACH_SEND_INVALID_DEST, "mach_vm_write()");
  logv("Returned expected error: %s.",
       mach_error_string(MACH_SEND_INVALID_DEST));
}

/* Writing 0 bytes always succeeds. */
void test_write_zero_size() {
  set_buffer_size(0);
  write_buffer();
}
...
void test_write_partially_deallocated_buffer() {
  mach_vm_address_t address = get_vm_address();
  vm_offset_t data = (vm_offset_t)get_buffer_address();
  mach_msg_type_number_t buffer_size =
      (mach_msg_type_number_t)get_buffer_size();
  mach_vm_address_t buffer_mid_point =
      (mach_vm_address_t)mach_vm_trunc_page(data + buffer_size / 2);

  logv("Deallocating a mid-range buffer page at address "
       "0x%jx...",
  ...
  logv("Returned expected error: %s.",
       mach_error_string(MACH_SEND_INVALID_MEMORY));
}

/* Writing a partially read-protected buffer fails. */
void test_write_partially_unreadable_buffer() {
  mach_vm_address_t address = get_vm_address();
  vm_offset_t data = (vm_offset_t)get_buffer_address();
  mach_msg_type_number_t buffer_size =
      (mach_msg_type_number_t)get_buffer_size();
  ...
  logv("Read-protecting a mid-range buffer page at address "
       "0x%jx...",
       (uintmax_t)buffer_mid_point);
  assert_mach_success(mach_vm_protect(mach_task_self(), buffer_mid_point,
                                      vm_page_size, FALSE, VM_PROT_WRITE),
                      "mach_vm_protect()");
  logv("Page read-protected.");
...
void test_write_on_partially_deallocated_range() {
  mach_vm_address_t address = get_vm_address();
  ...
  logv("Deallocating the first destination page at address "
       "0x%jx...",
       (uintmax_t)start);
...
void test_write_on_partially_unwritable_range() {
  mach_vm_address_t address = get_vm_address();
  mach_vm_address_t start = mach_vm_trunc_page(address);
  vm_offset_t data = (vm_offset_t)get_buffer_address();
  mach_msg_type_number_t buffer_size =
  ...
  kern_return_t kr_expected = (buffer_size < vm_page_size * 2)
                                  ? KERN_INVALID_ADDRESS
                                  : KERN_PROTECTION_FAILURE;

  logv("Write-protecting the first destination page at address "
       "0x%jx...",
       (uintmax_t)start);
  assert_mach_success(mach_vm_protect(mach_task_self(), start, vm_page_size,
                                      FALSE, VM_PROT_READ),
                      "mach_vm_protect()");
...
void test_zero_filled_write() {
  verify_pattern(empty, FALSE, mach_vm_trunc_page(get_vm_address()),
                 round_page(get_vm_size() + 1), "zero-filled");
  verify_pattern(empty, FALSE, mach_vm_trunc_page(get_buffer_address()),
                 round_page(get_buffer_size() + get_buffer_offset()),
                 "zero-filled");
}
...
void pattern_write(address_filter_t filter, boolean_t reversed,
                   const char *pattern_name) {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  ...
  logv("Verifying %s pattern on destination of "
       "address 0x%jx and size 0x%jx (%ju)...",
       pattern_name, (uintmax_t)address, (uintmax_t)buffer_size,
       (uintmax_t)size);
  filter_addresses_do_else(filter, reversed, address, buffer_size,
                           verify_address, read_zero, buffer_address);
  logv("Pattern verified on destination.");
}

void test_address_filled_write() {
  pattern_write(empty, TRUE, "address-filled");
}

void test_checkerboard_write() {
  pattern_write(checkerboard, FALSE, "checkerboard");
}

void test_reverse_checkerboard_write() {
...
void test_copy_null_map() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_address_t dest = get_buffer_address();
  mach_msg_type_number_t size = (mach_msg_type_number_t)get_buffer_size();

  logv("Copying buffer of address 0x%jx and size 0x%jx (%ju), on "
       "memory at address 0x%jx in NULL VM MAP...",
       (uintmax_t)dest, (uintmax_t)size, (uintmax_t)size, (uintmax_t)source);
  assert_mach_return(mach_vm_copy(VM_MAP_NULL, source, size, dest),
                     MACH_SEND_INVALID_DEST, "mach_vm_copy()");
  logv("Returned expected error: %s.",
       mach_error_string(MACH_SEND_INVALID_DEST));
}

void copy_edge_size(mach_vm_size_t size, kern_return_t expected_kr) {
  int i;
  kern_return_t kr;
  vm_map_t this_task = mach_task_self();
  mach_vm_address_t addresses[] = {
  ...
  logv("Allocating 0x%jx (%ju) byte%s...", (uintmax_t)size, (uintmax_t)size,
       (size == 1) ? "" : "s");
  assert_allocate_success(&dest, 4096, VM_FLAGS_ANYWHERE);
  logv("Copying 0x%jx (%ju) bytes at various addresses...", (uintmax_t)size,
       (uintmax_t)size);
  for (i = 0; i < numofaddresses; i++) {
  ...
  logv("mach_vm_copy() returned expected value in each case: "
       "%s.",
       mach_error_string(expected_kr));
...
void test_copy_zero_size() { copy_edge_size(0, KERN_SUCCESS); }

/* Copying 4GB or higher always fails. */
void test_copy_invalid_large_size() {
  copy_edge_size((mach_vm_size_t)UINT_MAX - 1, KERN_INVALID_ADDRESS);
}

/* Reading a range wrapped around the address space fails. */
void test_copy_wrapped_around_ranges() {
  int i;
  kern_return_t kr;
  vm_map_t this_task = mach_task_self();
  struct {
  ...
  logv("Copying various memory ranges wrapping around the "
       "address space...");
  for (i = 0; i < numofranges; i++) {
...
void write_copy_verify_pattern(address_filter_t filter, boolean_t reversed,
                               const char *pattern_name) {
  mach_vm_address_t source = get_vm_address();
 ...
 * zero-filled after writing. */
void test_zero_filled_copy_dest() {
  verify_pattern(empty, FALSE, mach_vm_trunc_page(get_vm_address()),
                 round_page(get_vm_size() + 1), "zero-filled");
  verify_pattern(empty, FALSE, mach_vm_trunc_page(get_buffer_address()),
                 round_page(get_buffer_size() + get_buffer_offset()),
...
void test_copy_partially_deallocated_range() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_size_t size = get_vm_size();
...
void test_copy_partially_unreadable_range() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  mach_vm_address_t mid_point = mach_vm_trunc_page(source + size / 2);
  mach_vm_address_t dest = 0;
   ...
   *  returning different errors, see 8182239. */
  kern_return_t kr_expected =
      (size < vm_page_size) ? KERN_INVALID_ADDRESS : KERN_PROTECTION_FAILURE;
...
void test_copy_dest_partially_deallocated_region() {
  mach_vm_address_t dest = get_vm_address();
  mach_vm_address_t source = get_buffer_address();
  mach_msg_type_number_t size = (mach_msg_type_number_t)get_buffer_size();
  mach_vm_address_t source_mid_point =
      (mach_vm_address_t)mach_vm_trunc_page(dest + size / 2);

  logv("Deallocating a mid-range source page at address "
       "0x%jx...",
       (uintmax_t)source_mid_point);
  deallocate_vm_page_early(source_mid_point);
  logv("Page deallocated.");

  logv("Copying region of address 0x%jx and size 0x%jx (%ju), on "
       "memory at address 0x%jx...",
       (uintmax_t)source, (uintmax_t)size, (uintmax_t)size, (uintmax_t)dest);
...
void test_copy_source_partially_deallocated_region() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_address_t dest = get_buffer_address();
  mach_msg_type_number_t size = (mach_msg_type_number_t)get_buffer_size();
  mach_vm_address_t source_mid_point =
      (mach_vm_address_t)mach_vm_trunc_page(source + size / 2);
...
void test_copy_source_partially_unreadable_region() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_address_t dest = get_buffer_address();
  mach_msg_type_number_t size = (mach_msg_type_number_t)get_buffer_size();
  mach_vm_address_t mid_point =
      (mach_vm_address_t)mach_vm_trunc_page(source + size / 2);
  kern_return_t kr = (size < vm_page_size * 2) ? KERN_INVALID_ADDRESS
                                               : KERN_PROTECTION_FAILURE;
  ...
  logv("Copying region at address 0x%jx and size 0x%jx (%ju), on "
       "memory at address 0x%jx...",
       (uintmax_t)source, (uintmax_t)size, (uintmax_t)size, (uintmax_t)dest);
...
void test_copy_dest_partially_unwriteable_region() {
  mach_vm_address_t dest = get_vm_address();
  mach_vm_address_t source = get_buffer_address();
  mach_msg_type_number_t size = (mach_msg_type_number_t)get_buffer_size();
  mach_vm_address_t mid_point =
  ...
  // The type of failure is not guaranteed to be consistent between
  // architectures, so we just make sure it fails.
  assert_copy_failure(source, size, dest);
  logv("Returned expected error.");
}

/* Copying on partially deallocated memory fails. */
void test_copy_source_on_partially_deallocated_range() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_address_t dest = get_buffer_address();
  mach_vm_address_t start = mach_vm_trunc_page(source);
  mach_msg_type_number_t size = (mach_msg_type_number_t)get_buffer_size();

  logv("Deallocating the first source page at address "
       "0x%jx...",
       (uintmax_t)start);
  deallocate_vm_page_early(start);
  logv("Page deallocated.");
...
/* Copying on partially deallocated memory fails. */
void test_copy_dest_on_partially_deallocated_range() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_address_t dest = get_buffer_address();
...
void test_copy_dest_on_partially_unwritable_range() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_address_t dest = get_buffer_address();
  mach_vm_address_t start = mach_vm_trunc_page(dest);
  mach_msg_type_number_t size = (mach_msg_type_number_t)get_buffer_size();
...
void test_copy_source_on_partially_unreadable_range() {
  mach_vm_address_t source = get_vm_address();
  mach_vm_address_t dest = get_buffer_address();
  mach_vm_address_t start = mach_vm_trunc_page(source);
  mach_msg_type_number_t size = (mach_msg_type_number_t)get_buffer_size();
  ...
  logv("Read-protecting the first destination page at address "
       "0x%jx...",
       (uintmax_t)start);
  assert_mach_success(mach_vm_protect(mach_task_self(), start, vm_page_size,
                                      FALSE, VM_PROT_WRITE),
...
void test_zero_filled_extended() {
  verify_pattern(empty, FALSE, mach_vm_trunc_page(get_vm_address()),
                 round_page(get_vm_size() + 1), "zero-filled");
}
...
void test_zero_filled_readprotect() {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();

  logv("Setting read access on 0x%jx (%ju) byte%s at address 0x%jx...",
       (uintmax_t)size, (uintmax_t)size, (size == 1) ? "" : "s",
       (uintmax_t)address);
  assert_mach_success(
      mach_vm_protect(mach_task_self(), address, size, FALSE, VM_PROT_DEFAULT),
      "mach_vm_protect()");
  logv("Region has read access.");
  test_zero_filled_extended();
}

void verify_protection(vm_prot_t protection, const char *protection_name) {
  mach_vm_address_t address = get_vm_address();
  mach_vm_size_t size = get_vm_size();
  mach_vm_size_t original_size = size;
  ...
  logv("Verifying %s-protection on region of address 0x%jx and "
       "size 0x%jx (%ju) with mach_vm_region()...",
       protection_name, (uintmax_t)address, (uintmax_t)size, (uintmax_t)size);
  assert_mach_success(mach_vm_region(mach_task_self(), &address, &size,
                                     VM_REGION_BASIC_INFO_64,
                                     (vm_region_info_t)&info, &count, &unused),
                      "mach_vm_region()");
  if (original_size) {
    T_QUIET;
    T_ASSERT_EQ((info.protection & protection), 0,
                "Region "
    ...
    T_QUIET;
    T_ASSERT_NE(info.protection & protection, 0,
                "Region is "
...
void test_verify_readprotection() { verify_protection(VM_PROT_READ, "read"); }

void test_verify_writeprotection() {
...
void access_readprotected_range_address(mach_vm_address_t address,
                                        const char *position) {
  logv("Reading from %s 0x%jx of read-protected range...", position,
       (uintmax_t)address);
  mach_vm_address_t bad_value = MACH_VM_ADDRESS_T(address);
  T_ASSERT_FAIL("Unexpectedly read value 0x%jx at address 0x%jx. "
...
void test_access_readprotected_range_start() {
  access_readprotected_range_address(mach_vm_trunc_page(get_vm_address()),
                                     "start");
}

/* Middle of read-protected range is inaccessible. */
void test_access_readprotected_range_middle() {
  mach_vm_address_t address = get_vm_address();
  access_readprotected_range_address(
      mach_vm_trunc_page(address) + (aligned_size(address, get_vm_size()) >> 1),
      "middle");
}

/* End of read-protected range is inaccessible. */
void test_access_readprotected_range_end() {
  access_readprotected_range_address(
      round_page(get_vm_address() + get_vm_size()) - vm_address_size, "end");
}

/* Addresses in write-protected range are unwritable. */
void write_writeprotected_range_address(mach_vm_address_t address,
                                        const char *position) {
  logv("Writing on %s 0x%jx of write-protected range...", position,
       (uintmax_t)address);
  MACH_VM_ADDRESS_T(address) = 0x0;
...
void test_write_writeprotected_range_start() {
  write_writeprotected_range_address(mach_vm_trunc_page(get_vm_address()),
                                     "start");
}

/* Middle of write-protected range is unwritable. */
void test_write_writeprotected_range_middle() {
  mach_vm_address_t address = get_vm_address();
  write_writeprotected_range_address(
      mach_vm_trunc_page(address) + (aligned_size(address, get_vm_size()) >> 1),
      "middle");
}

/* End of write-protected range is unwritable. */
void test_write_writeprotected_range_end() {
  write_writeprotected_range_address(
      round_page(get_vm_address() + get_vm_size()) - vm_address_size, "end");
}
...
void protect_zero_size(vm_prot_t protection, const char *protection_name) {
  int i;
  kern_return_t kr;
  vm_map_t this_task = mach_task_self();
  mach_vm_address_t addresses[] = {
...
void test_readprotect_zero_size() { protect_zero_size(VM_PROT_READ, "Read"); }

void test_writeprotect_zero_size() {
  protect_zero_size(VM_PROT_WRITE, "Write");
}

/* Protecting a range wrapped around the address space fails. */
void protect_wrapped_around_ranges(vm_prot_t protection,
                                   const char *protection_name) {
  int i;
  kern_return_t kr;
  vm_map_t this_task = mach_task_self();
  ...
  logv("%s-protecting various memory ranges wrapping around the "
       "address space...",
       protection_name);
  for (i = 0; i < numofranges; i++) {
    kr = mach_vm_protect(this_task, ranges[i].address, ranges[i].size, FALSE,
                         protection);
    assert_mach_return(kr, KERN_INVALID_ARGUMENT,
...
void test_readprotect_wrapped_around_ranges() {
  protect_wrapped_around_ranges(VM_PROT_READ, "Read");
}

void test_writeprotect_wrapped_around_ranges() {
...
void assert_share_mode(mach_vm_address_t address, unsigned share_mode,
                       const char *share_mode_name) {}

/* Do the vm_copy() and verify its success. */
void assert_vmcopy_success(vm_address_t src, vm_address_t dst,
  ...
  assert_mach_success(kr,
                      ...
                      "unexpectedly failed.",
                      (uintmax_t)src, (uintmax_t)dst, (uintmax_t)size,
                      (uintmax_t)size);
  logv("Copy (mach_vm_copy()) was successful as expected.");
}

void write_region(mach_vm_address_t address, mach_vm_size_t start) {
  mach_vm_size_t size = get_vm_size();

  filter_addresses_do_else(empty, FALSE, address, size, write_address,
                           write_address, start);
}

void verify_region(mach_vm_address_t address, mach_vm_address_t start) {
  mach_vm_size_t size = get_vm_size();

  filter_addresses_do_else(empty, FALSE, address, size, verify_address,
                           verify_address, start);
}

/* Perform the post vm_copy() action and verify its results. */
void modify_one_and_verify_all_regions(vm_address_t src, vm_address_t dst,
  ...
  case VMCOPY_MODIFY_SHARED_COPIED:
    ...
    logv("Modifying: shared/copied%s...",
         shared ? " (shared with source region)" : "");
  ...
  verify_region(src, (VMCOPY_MODIFY_SRC == action ||
                      (shared && VMCOPY_MODIFY_SHARED_COPIED == action))
                         ? 1
                         : 0);
  logv("destination... ");
  ...
  if (shared_copied) {
    logv("shared/copied... ");
    verify_region(shared_copied, (VMCOPY_MODIFY_SHARED_COPIED == action ||
                                  (shared && VMCOPY_MODIFY_SRC == action))
                                     ? 1
                                     : 0);
  }
...
void test_vmcopy_shared_source() {
  mach_vm_size_t size = get_vm_size();
  ...
  assert_mach_success(
      mach_vm_inherit(mach_task_self(), src, size, VM_INHERIT_SHARE),
      "mach_vm_inherit()");
  ...
  if (pid == 0) {
    ...
    verify_region(shared, (VMCOPY_MODIFY_SHARED_COPIED == action ||
                           VMCOPY_MODIFY_SRC == action)
                              ? 1
                              : 0);
    logv("Verification was successful as expected.");
  } else {
...
void test_vmcopy_copied_from_source() {
  mach_vm_size_t size = get_vm_size();
  ...
  assert_mach_success(mach_vm_copy(mach_task_self(), copied, size, src),
                      "mach_vm_copy()");
...
void test_vmcopy_copied_to_source() {
  mach_vm_size_t size = get_vm_size();
  mach_vm_address_t src, dst, copied;
  ...
  assert_mach_success(mach_vm_copy(mach_task_self(), src, size, copied),
                      "mach_vm_copy()");
...
void test_vmcopy_trueshared_source() {
  mach_vm_size_t size = get_vm_size();
  ...
  assert_mach_success(mach_make_memory_entry_64(mach_task_self(), &size,
                                                (memory_object_offset_t)shared,
                                                cur_protect, &mem_obj,
                                                (mem_entry_name_port_t)NULL),
                      "mach_make_memory_entry_64()");
  assert_mach_success(mach_vm_map(mach_task_self(), &src, size, 0, TRUE,
                                  mem_obj, 0, FALSE, cur_protect, max_protect,
                                  VM_INHERIT_NONE),
                      "mach_vm_map()");
...
void test_vmcopy_private_aliased_source() {
  mach_vm_size_t size = get_vm_size();
  mach_vm_address_t src = 0x0, dst, shared;
  vm_prot_t cur_protect = (VM_PROT_READ | VM_PROT_WRITE);
  vm_prot_t max_protect = (VM_PROT_READ | VM_PROT_WRITE);
  ...
  assert_mach_success(
      mach_vm_remap(mach_task_self(), &src, size, 0, TRUE, mach_task_self(),
                    shared, FALSE, &cur_protect, &max_protect, VM_INHERIT_NONE),
      "mach_vm_remap()");
...
void run_allocate_test_suites() {
  UnitTests allocate_main_tests = {
      {"Allocated address is nonzero iff size is nonzero",
       test_nonzero_address_iff_nonzero_size, 0},
      ...
      {"Write and verify reverse checkerboard pattern",
       test_write_reverse_checkerboard, 0},
  ...
  UnitTests allocate_argument_error_tests = {
      ...
      {"Allocate super-page with incompatible flags",
       test_allocate_superpage_with_incompatible_flags, 0},
  };
  UnitTests allocate_fixed_size_tests = {
      ...
      {"Allocate a page with highest address hint",
       test_allocate_page_with_highest_address_hint, 0},
      {"Allocate two pages and verify first fit strategy",
       test_allocate_first_fit_pages, 0},
  };
  ...
  UnitTests mach_vm_map_large_mask_overflow_error_test = {
      {"mach_vm_map() with large address mask",
       test_mach_vm_map_large_mask_overflow_error, 0},
  };
  ...
  for (allocators_idx = 0; allocators_idx < numofallocators; allocators_idx++) {
    for (sizes_idx = 0; sizes_idx < numofsizes; sizes_idx++) {
      for (flags_idx = 0; flags_idx < numofflags; flags_idx++) {
        for (alignments_idx = 0; alignments_idx < numofalignments;
             alignments_idx++) {
          ...
          if ((flags_idx == ANYWHERE && alignments_idx == UNALIGNED) ||
              (allocators_idx != MACH_VM_ALLOCATE && sizes_idx == ZERO_BYTES &&
               flags_idx == FIXED)) {
            continue;
          }
          run_suite(set_up_allocator_and_vm_variables,
                    allocate_argument_error_tests, do_nothing,
                    ...
                    (flags_idx == ANYWHERE)
                        ? ""
                        : address_alignments[alignments_idx].description,
                    vm_sizes[sizes_idx].description,
          ...
          if (allocators_idx != MACH_VM_ALLOCATE) {
            run_suite(set_up_allocator_and_vm_variables,
                      mach_vm_map_protection_inheritance_error_test, do_nothing,
          ...
          if (allocators_idx == MACH_VM_ALLOCATE || sizes_idx != ZERO_BYTES) {
            run_suite(set_up_allocator_and_vm_variables_and_allocate,
                      allocate_main_tests, deallocate,
                      "%s main "
      ...
      run_suite(set_up_allocator_and_vm_size, allocate_address_error_tests,
                do_nothing,
                "%s address "
    ...
    run_suite(set_up_allocator, allocate_fixed_size_tests, do_nothing,
              "%s fixed size allocation tests",
              allocators[allocators_idx].description);
    run_suite(set_up_allocator, allocate_invalid_large_size_test, do_nothing,
              "%s invalid large size allocation test",
              allocators[allocators_idx].description);
  }
  /* mach_vm_map() only large mask overflow tests. */
  for (sizes_idx = 0; sizes_idx < numofsizes; sizes_idx++) {
    run_suite(set_up_vm_size, mach_vm_map_large_mask_overflow_error_test,
              do_nothing,
...
void run_deallocate_test_suites() {
  ...
  UnitTests deallocate_edge_case_tests = {
      ...
      {"Deallocate wrapped around memory ranges",
       test_deallocate_wrapped_around_ranges, 0},
  };
    ...
    for (flags_idx = 0; flags_idx < numofflags; flags_idx++) {
      for (alignments_idx = 0; alignments_idx < numofalignments;
        ...
        if (sizes_idx != ZERO_BYTES) {
          run_suite(set_up_vm_variables_and_allocate,
                    access_deallocated_memory_tests, do_nothing,
        ...
        if (vm_sizes[sizes_idx].size == 0) {
          ...
          deallocate_reallocate_tests[0].expected_signal =
              _expected_vm_exc_guard_signal;
        }
        run_suite(set_up_vm_variables_and_allocate, deallocate_reallocate_tests,
                  do_nothing,
  ...
  run_suite(do_nothing, deallocate_edge_case_tests, do_nothing,
            "Edge case deallocation tests");
  run_suite(do_nothing, deallocate_suicide_test, do_nothing,
            "Whole address space deallocation test");
}

void run_read_test_suites() {
  UnitTests read_main_tests = {
      {"Read address is nonzero iff size is nonzero",
       test_nonzero_address_iff_nonzero_size, 0},
  ...
  UnitTests read_inaccessible_tests = {
      {"Read partially deallocated memory",
       test_read_partially_deallocated_range, 0},
      {"Read partially read-protected memory",
       test_read_partially_unreadable_range, 0},
  };
  ...
  for (sizes_idx = 0; sizes_idx < numofsizes; sizes_idx++) {
    for (flags_idx = 0; flags_idx < numofflags; flags_idx++) {
      for (alignments_idx = 0; alignments_idx < numofalignments;
        ...
        run_suite(set_up_vm_variables_allocate_read_deallocate, read_main_tests,
                  deallocate,
                  "mach_vm_read() "
                  ...
                  (uintmax_t)vm_sizes[sizes_idx].size,
                  (uintmax_t)vm_sizes[sizes_idx].size);
        run_suite(set_up_vm_variables_and_allocate_extra_page,
                  read_pattern_tests, deallocate,
                  ...
                  (uintmax_t)vm_sizes[sizes_idx].size);
        run_suite(set_up_vm_variables_and_allocate_extra_page,
                  read_null_map_test, deallocate_extra_page,
        ...
        if (sizes_idx != ZERO_BYTES) {
          run_suite(set_up_vm_variables_and_allocate_extra_page,
                    read_inaccessible_tests, deallocate_extra_page,
  ...
  run_suite(do_nothing, read_edge_case_tests, do_nothing,
            "mach_vm_read() fixed size tests");
}

void run_write_test_suites() {
  ...
  UnitTests write_edge_case_tests = {
      {"Write into NULL VM map", test_write_null_map, 0},
      {"Write zero size", test_write_zero_size, 0},
  };
  UnitTests write_inaccessible_tests = {
      {"Write partially deallocated buffer",
       test_write_partially_deallocated_buffer, 0},
      {"Write partially read-protected buffer",
       test_write_partially_unreadable_buffer, 0},
      {"Write on partially deallocated range",
       test_write_on_partially_deallocated_range, 0},
      {"Write on partially write-protected range",
       test_write_on_partially_unwritable_range, 0},
  };
  ...
  for (sizes_idx = 0; sizes_idx < numofsizes; sizes_idx++) {
    for (flags_idx = 0; flags_idx < numofflags; flags_idx++) {
      for (alignments_idx = 0; alignments_idx < numofalignments;
           alignments_idx++) {
        for (buffer_sizes_idx = 0; buffer_sizes_idx < numofsizes;
             buffer_sizes_idx++) {
          for (offsets_idx = 0; offsets_idx < numofoffsets; offsets_idx++) {
            ...
            run_suite(set_up_vm_and_buffer_variables_allocate_for_writing,
                      write_edge_case_tests, deallocate_vm_and_buffer,
                      ...
                      (uintmax_t)vm_sizes[sizes_idx].size,
                      (uintmax_t)vm_sizes[sizes_idx].size,
                      vm_sizes[buffer_sizes_idx].description,
                      (uintmax_t)vm_sizes[buffer_sizes_idx].size,
                      (uintmax_t)vm_sizes[buffer_sizes_idx].size,
                      buffer_offsets[offsets_idx].offset);
            /* A zero size buffer is always accessible. */
            if (buffer_sizes_idx != ZERO_BYTES) {
              run_suite(set_up_vm_and_buffer_variables_allocate_for_writing,
                        write_inaccessible_tests, deallocate_vm_and_buffer,
                        "mach_vm_write() inaccessibility tests, "
            ...
            run_suite(set_up_vm_and_buffer_variables_allocate_write,
                      write_main_tests, deallocate_vm_and_buffer,
                      ...
                      buffer_offsets[offsets_idx].offset);
            run_suite(set_up_vm_and_buffer_variables_allocate_for_writing,
                      write_pattern_tests, deallocate_vm_and_buffer,
...
void run_protect_test_suites() {
  UnitTests readprotection_main_tests = {
      {"Read-protect, read-allow and verify zero-filled memory",
       test_zero_filled_readprotect, 0},
      {"Verify that region is read-protected iff size is "
       "nonzero",
       test_verify_readprotection, 0},
  };
  UnitTests access_readprotected_memory_tests = {
      {"Read start of read-protected range",
       test_access_readprotected_range_start, SIGBUS},
      {"Read middle of read-protected range",
       test_access_readprotected_range_middle, SIGBUS},
  ...
  UnitTests write_writeprotected_memory_tests = {
      {"Write at start of write-protected range",
       test_write_writeprotected_range_start, SIGBUS},
      {"Write in middle of write-protected range",
       test_write_writeprotected_range_middle, SIGBUS},
      {"Write at end of write-protected range",
       test_write_writeprotected_range_end, SIGBUS},
  };
  UnitTests protect_edge_case_tests = {
      ...
      {"Read-protect wrapped around memory ranges",
       test_readprotect_wrapped_around_ranges, 0},
      {"Write-protect wrapped around memory ranges",
       test_writeprotect_wrapped_around_ranges, 0},
  };
  ...
  for (sizes_idx = 0; sizes_idx < numofsizes; sizes_idx++) {
    for (flags_idx = 0; flags_idx < numofflags; flags_idx++) {
      for (alignments_idx = 0; alignments_idx < numofalignments;
        ...
        run_suite(set_up_vm_variables_allocate_readprotect,
                  readprotection_main_tests, deallocate_extra_page,
                  ...
                  (uintmax_t)vm_sizes[sizes_idx].size,
                  (uintmax_t)vm_sizes[sizes_idx].size);
        run_suite(set_up_vm_variables_allocate_writeprotect,
                  writeprotection_main_tests, deallocate_extra_page,
        ...
        if (sizes_idx != ZERO_BYTES) {
           ...
           *  error. */
          run_suite(set_up_vm_variables_allocate_readprotect,
                    access_readprotected_memory_tests, deallocate_extra_page,
           ...
           *  error. */
          run_suite(set_up_vm_variables_allocate_writeprotect,
                    write_writeprotected_memory_tests, deallocate_extra_page,
  ...
  run_suite(do_nothing, protect_edge_case_tests, do_nothing,
            "Edge case protection tests");
}

void run_copy_test_suites() {
  /* Copy tests */
  ...
  UnitTests copy_inaccessible_tests = {
      {"Copy source partially deallocated region",
       test_copy_source_partially_deallocated_region, 0},
      /* XXX */
      {"Copy destination partially deallocated region",
       test_copy_dest_partially_deallocated_region, 0},
      {"Copy source partially read-protected region",
       test_copy_source_partially_unreadable_region, 0},
      /* XXX */
      {"Copy destination partially write-protected region",
       test_copy_dest_partially_unwriteable_region, 0},
      {"Copy source on partially deallocated range",
       test_copy_source_on_partially_deallocated_range, 0},
      {"Copy destination on partially deallocated range",
       test_copy_dest_on_partially_deallocated_range, 0},
      {"Copy source on partially read-protected range",
       test_copy_source_on_partially_unreadable_range, 0},
      {"Copy destination on partially write-protected range",
       test_copy_dest_on_partially_unwritable_range, 0},
  };
  ...
  UnitTests copy_shared_mode_tests = {
      ...
      {"Copy using a private aliased source",
       test_vmcopy_private_aliased_source, 0},
  };
  ...
  for (vmcopy_action_idx = 0; vmcopy_action_idx < numofvmcopyactions;
       vmcopy_action_idx++) {
    run_suite(set_up_copy_shared_mode_variables, copy_shared_mode_tests,
              do_nothing, "Copy shared mode tests, %s",
      ...
      for (alignments_idx = 0; alignments_idx < numofalignments;
        ...
        for (buffer_sizes_idx = 0; buffer_sizes_idx < numofsizes;
          ...
          for (offsets_idx = 0; offsets_idx < numofoffsets; offsets_idx++) {
            ...
            run_suite(set_up_vm_and_buffer_variables_allocate_for_copying,
                      copy_edge_case_tests, deallocate_vm_and_buffer,
            ...
            if (buffer_sizes_idx != ZERO_BYTES) {
              run_suite(set_up_vm_and_buffer_variables_allocate_for_copying,
                        copy_inaccessible_tests, deallocate_vm_and_buffer,
                        "mach_vm_copy() inaccessibility tests, "
                        "%s%s address, %s size: 0x%jx (%ju), buffer "
                        "%s size: 0x%jx (%ju), buffer boundary "
                        "offset: %d",
            ...
            run_suite(set_up_source_and_dest_variables_allocate_copy,
                      copy_main_tests, deallocate_vm_and_buffer,
                      ...
                      buffer_offsets[offsets_idx].offset);
            run_suite(set_up_source_and_dest_variables_allocate_copy,
                      copy_pattern_tests, deallocate_vm_and_buffer,
...
void perform_test_with_options(test_option_t options) {
...
T_DECL(vm_test_deallocate, "Deallocate VM unit test",
       T_META_ENABLED(!TARGET_OS_BRIDGE), /* disabled on bridgeOS due to
                                             failures, rdar://137493917 */
