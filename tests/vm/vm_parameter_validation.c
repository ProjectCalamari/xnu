...
#define GOLDEN_FILES_VERSION                                                   \
  "vm_parameter_validation_golden_images_a2474e92.tar.xz"
...
T_GLOBAL_META(T_META_NAMESPACE("xnu.vm"), T_META_RADAR_COMPONENT_NAME("xnu"),
              ...
              T_META_RUN_CONCURRENTLY(
                  false), /* tests should be concurrency-safe now, but keep this
                             in case concurrent tests would provoke timeouts */
 ...
 * When vm_parameter_validation runs, it calls
 * assets/vm_parameter_validation/decompress.sh, which detects the platform and
 * decompresses the corresponding user and kern golden files.
 *
 * Any return code mismatch is reported as a failure, printing test name and
 * iteration. New tests not present in the 'golden' list will run but they are
 * also reported as a failure.
 *
 * There are two environment variable flags that makes development work easier
 * and can temporarily disable golden list testing.
 *
 * SKIP_TESTS
 ...
 *
 * DUMP_RESULTS
 * When running with DUMP_RESULTS set, the test will print all the returned
 * values (as opposed to only the failing ones). To pretty-print this output use
 * the python script: DUMP_RESULTS=1 vm_parameter_validation |
 * tools/format_vm_parameter_validation.py
 */
...
static void set_up_guard_page(void) {
  /*
...
static int munmap_helper(void *ptr, size_t size) {
...
// We disable EXC_GUARD if possible. If we can't, we disable those tests
// instead.
static bool EXC_GUARD_ENABLED = true;

static int call_munlock(void *start, size_t size) {
  int err = munlock(start, size);
  return err ? errno : 0;
}

static int call_mlock(void *start, size_t size) {
...
static kern_return_t call_munmap(MAP_T map __unused, mach_vm_address_t start,
                                 mach_vm_size_t size) {
  int err = __munmap((void *)start, (size_t)size);
  return err ? errno : 0;
}

static int call_mremap_encrypted(void *start, size_t size) {
  int err = mremap_encrypted(start, size, CRYPTID_NO_ENCRYPTION, /*cputype=*/0,
                             /*cpusubtype=*/0);
  return err ? errno : 0;
}
...
static mach_port_t make_a_mem_object(mach_vm_size_t size) {
  mach_port_t out_handle;
  kern_return_t kr = mach_memory_object_memory_entry_64(
      mach_host_self(), 1, size, VM_PROT_READ | VM_PROT_WRITE, 0, &out_handle);
  assert(kr == 0);
  return out_handle;
}

static mach_port_t make_a_mem_entry(vm_size_t size) {
  mach_port_t port;
  memory_object_size_t s = (memory_object_size_t)size;
  kern_return_t kr = mach_make_memory_entry_64(
      mach_host_self(), &s, (memory_object_offset_t)0,
      MAP_MEM_NAMED_CREATE | MAP_MEM_LEDGER_TAGGED, &port, MACH_PORT_NULL);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "allocate memory entry");
  return port;
}

static inline void check_mach_memory_entry_outparam_changes(
    kern_return_t *kr, mach_port_t out_handle, mach_port_t saved_handle) {
  if (*kr != KERN_SUCCESS) {
...
// U64: mach_make_memory_entry_64(64) (mach_make_memory_entry is the same MIG
// message) U32: mach_make_memory_entry(32), mach_make_memory_entry_64(64),
// _mach_make_memory_entry(64) (each is a unique MIG message)
#define IMPL(FN, T)                                                            \
  static kern_return_t call_##FN##__start_size__memonly(MAP_T map, T start,    \
                                                        T size) {              \
    ...
    kern_return_t kr = FN(map, &io_size, start, VM_PROT_READ | MAP_MEM_ONLY,   \
                          &out_handle, memobject);                             \
    if (kr == 0) {                                                             \
    ...
    return kr;                                                                 \
                                                                               ...
                                                                               \
  static kern_return_t call_##FN##__start_size__namedcreate(MAP_T map,         \
                                                            T start, T size) { \
    mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);            \
    T io_size = size;                                                          \
    mach_port_t invalid_value = UNLIKELY_INITIAL_MACH_PORT;                    \
    mach_port_t out_handle = invalid_value;                                    \
    kern_return_t kr =                                                         \
        FN(map, &io_size, start, VM_PROT_READ | MAP_MEM_NAMED_CREATE,          \
           &out_handle, memobject);                                            \
    if (kr == 0) {                                                             \
      (void)mach_port_deallocate(mach_task_self(), out_handle);                \
    }                                                                          \
    ...
    return kr;                                                                 \
                                                                               ...
                                                                               \
  static kern_return_t call_##FN##__start_size__copy(MAP_T map, T start,       \
                                                     T size) {                 \
    mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);            \
    T io_size = size;                                                          \
    mach_port_t invalid_value = UNLIKELY_INITIAL_MACH_PORT;                    \
    mach_port_t out_handle = invalid_value;                                    \
    kern_return_t kr =                                                         \
        FN(map, &io_size, start, VM_PROT_READ | MAP_MEM_VM_COPY, &out_handle,  \
           memobject);                                                         \
    if (kr == 0) {                                                             \
      (void)mach_port_deallocate(mach_task_self(), out_handle);                \
    }                                                                          \
    ...
    return kr;                                                                 \
                                                                               ...
                                                                               \
  static kern_return_t call_##FN##__start_size__share(MAP_T map, T start,      \
                                                      T size) {                \
    mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);            \
    T io_size = size;                                                          \
    mach_port_t invalid_value = UNLIKELY_INITIAL_MACH_PORT;                    \
    mach_port_t out_handle = invalid_value;                                    \
    kern_return_t kr =                                                         \
        FN(map, &io_size, start, VM_PROT_READ | MAP_MEM_VM_SHARE, &out_handle, \
           memobject);                                                         \
    if (kr == 0) {                                                             \
      (void)mach_port_deallocate(mach_task_self(), out_handle);                \
    }                                                                          \
    ...
    return kr;                                                                 \
                                                                               ...
                                                                               \
  static kern_return_t call_##FN##__start_size__namedreuse(MAP_T map, T start, \
                                                           T size) {           \
    mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);            \
    T io_size = size;                                                          \
    mach_port_t invalid_value = UNLIKELY_INITIAL_MACH_PORT;                    \
    mach_port_t out_handle = invalid_value;                                    \
    kern_return_t kr =                                                         \
        FN(map, &io_size, start, VM_PROT_READ | MAP_MEM_NAMED_REUSE,           \
           &out_handle, memobject);                                            \
    if (kr == 0) {                                                             \
      (void)mach_port_deallocate(mach_task_self(), out_handle);                \
    }                                                                          \
    ...
    return kr;                                                                 \
                                                                               ...
                                                                               \
  static kern_return_t call_##FN##__vm_prot(MAP_T map, T start, T size,        \
                                            vm_prot_t prot) {                  \
    mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);            \
    T io_size = size;                                                          \
    mach_port_t invalid_value = UNLIKELY_INITIAL_MACH_PORT;                    \
    mach_port_t out_handle = invalid_value;                                    \
    kern_return_t kr = FN(map, &io_size, start, prot, &out_handle, memobject); \
    if (kr == 0) {                                                             \
      (void)mach_port_deallocate(mach_task_self(), out_handle);                \
    }                                                                          \
    (void)mach_port_deallocate(mach_task_self(), memobject);                   \
...
static inline void check_mach_memory_object_memory_entry_outparam_changes(
    kern_return_t *kr, mach_port_t out_handle, mach_port_t saved_out_handle) {
  if (*kr != KERN_SUCCESS) {
...
#define IMPL(FN)                                                               \
  static kern_return_t call_##FN##__size(MAP_T map __unused,                   \
                                         mach_vm_size_t size) {                \
    kern_return_t kr;                                                          \
    mach_port_t invalid_value = UNLIKELY_INITIAL_MACH_PORT;                    \
    mach_port_t out_entry = invalid_value;                                     \
    kr = FN(mach_host_self(), 1, size, VM_PROT_READ | VM_PROT_WRITE, 0,        \
            &out_entry);                                                       \
    if (kr == 0) {                                                             \
      (void)mach_port_deallocate(mach_task_self(), out_entry);                 \
    }                                                                          \
    check_mach_memory_object_memory_entry_outparam_changes(&kr, out_entry,     \
                                                           invalid_value);     \
    return kr;                                                                 \
  }                                                                            \
  static kern_return_t call_##FN##__vm_prot(                                   \
      MAP_T map __unused, mach_vm_size_t size, vm_prot_t prot) {               \
    kern_return_t kr;                                                          \
    mach_port_t invalid_value = UNLIKELY_INITIAL_MACH_PORT;                    \
    mach_port_t out_entry = invalid_value;                                     \
...
static inline void check_vm_read_outparam_changes(kern_return_t *kr,
                                                  mach_vm_size_t size,
                                                  mach_vm_size_t requested_size,
...
static kern_return_t call_mach_vm_read(MAP_T map, mach_vm_address_t start,
                                       mach_vm_size_t size) {
...
#if TEST_OLD_STYLE_MACH
static kern_return_t call_vm_read(MAP_T map, vm_address_t start,
                                  vm_size_t size) {
  vm_offset_t out_addr = UNLIKELY_INITIAL_ADDRESS;
  mach_msg_type_number_t out_size = UNLIKELY_INITIAL_SIZE;
...
static kern_return_t call_mach_vm_read_list(MAP_T map, mach_vm_address_t start,
                                            mach_vm_size_t size) {
...
#if TEST_OLD_STYLE_MACH
static kern_return_t call_vm_read_list(MAP_T map, vm_address_t start,
                                       vm_size_t size) {
...
static inline void
check_vm_read_overwrite_outparam_changes(kern_return_t *kr, mach_vm_size_t size,
                                         mach_vm_size_t requested_size) {
  if (*kr == KERN_SUCCESS) {
    if (size != requested_size) {
      *kr = OUT_PARAM_BAD;
...
static kern_return_t __unused call_mach_vm_read_overwrite__ssz(
    MAP_T map, mach_vm_address_t start, mach_vm_address_t start_2,
    mach_vm_size_t size) {
  mach_vm_size_t out_size;
  kern_return_t kr =
      mach_vm_read_overwrite(map, start, size, start_2, &out_size);
  check_vm_read_overwrite_outparam_changes(&kr, out_size, size);
  return kr;
}

static kern_return_t call_mach_vm_read_overwrite__src(MAP_T map,
                                                      mach_vm_address_t src,
                                                      mach_vm_size_t size) {
  mach_vm_size_t out_size;
  allocation_t dst SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  kern_return_t kr =
      mach_vm_read_overwrite(map, src, size, dst.addr, &out_size);
  check_vm_read_overwrite_outparam_changes(&kr, out_size, size);
  return kr;
}

static kern_return_t call_mach_vm_read_overwrite__dst(MAP_T map,
                                                      mach_vm_address_t dst,
                                                      mach_vm_size_t size) {
  mach_vm_size_t out_size;
  allocation_t src SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  kern_return_t kr =
      mach_vm_read_overwrite(map, src.addr, size, dst, &out_size);
  check_vm_read_overwrite_outparam_changes(&kr, out_size, size);
  return kr;
}

#if TEST_OLD_STYLE_MACH
static kern_return_t __unused call_vm_read_overwrite__ssz(MAP_T map,
                                                          vm_address_t start,
                                                          vm_address_t start_2,
                                                          vm_size_t size) {
  vm_size_t out_size;
...
static kern_return_t call_vm_read_overwrite__src(MAP_T map, vm_address_t src,
                                                 vm_size_t size) {
  vm_size_t out_size;
  allocation_t dst SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  kern_return_t kr =
      vm_read_overwrite(map, src, size, (vm_address_t)dst.addr, &out_size);
  check_vm_read_overwrite_outparam_changes(&kr, out_size, size);
  return kr;
}

static kern_return_t call_vm_read_overwrite__dst(MAP_T map, vm_address_t dst,
                                                 vm_size_t size) {
  vm_size_t out_size;
  allocation_t src SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  kern_return_t kr =
      vm_read_overwrite(map, (vm_address_t)src.addr, size, dst, &out_size);
  check_vm_read_overwrite_outparam_changes(&kr, out_size, size);
  return kr;
}
#endif

static kern_return_t __unused call_mach_vm_copy__ssz(MAP_T map,
                                                     mach_vm_address_t start,
                                                     mach_vm_address_t start_2,
                                                     mach_vm_size_t size) {
  kern_return_t kr = mach_vm_copy(map, start, size, start_2);
  return kr;
}

static kern_return_t call_mach_vm_copy__src(MAP_T map, mach_vm_address_t src,
                                            mach_vm_size_t size) {
  allocation_t dst SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
...
static kern_return_t call_mach_vm_copy__dst(MAP_T map, mach_vm_address_t dst,
                                            mach_vm_size_t size) {
  allocation_t src SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
...
#if TEST_OLD_STYLE_MACH
static kern_return_t __unused call_vm_copy__ssz(MAP_T map,
                                                mach_vm_address_t start,
                                                mach_vm_address_t start_2,
                                                mach_vm_size_t size) {
  kern_return_t kr =
      vm_copy(map, (vm_address_t)start, (vm_size_t)size, (vm_address_t)start_2);
  return kr;
}

static kern_return_t call_vm_copy__src(MAP_T map, vm_address_t src,
                                       vm_size_t size) {
  allocation_t dst SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
...
static kern_return_t call_vm_copy__dst(MAP_T map, vm_address_t dst,
                                       vm_size_t size) {
  allocation_t src SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
...
static kern_return_t __unused call_mach_vm_write__ssz(MAP_T map,
                                                      mach_vm_address_t start,
                                                      mach_vm_address_t start_2,
                                                      mach_vm_size_t size) {
  kern_return_t kr = mach_vm_write(map, start, (vm_offset_t)start_2,
                                   (mach_msg_type_number_t)size);
  return kr;
}

static kern_return_t call_mach_vm_write__src(MAP_T map, mach_vm_address_t start,
                                             mach_vm_size_t size) {
  allocation_t dst SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  kern_return_t kr = mach_vm_write(map, dst.addr, (vm_offset_t)start,
                                   (mach_msg_type_number_t)size);
  return kr;
}

static kern_return_t call_mach_vm_write__dst(MAP_T map, mach_vm_address_t start,
                                             mach_vm_size_t size) {
  allocation_t src SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  kern_return_t kr = mach_vm_write(map, start, (vm_offset_t)src.addr,
                                   (mach_msg_type_number_t)size);
  return kr;
}

#if TEST_OLD_STYLE_MACH
static kern_return_t __unused call_vm_write__ssz(MAP_T map,
                                                 mach_vm_address_t start,
                                                 mach_vm_address_t start_2,
                                                 mach_vm_size_t size) {
  kern_return_t kr = vm_write(map, (vm_address_t)start, (vm_offset_t)start_2,
                              (mach_msg_type_number_t)size);
  return kr;
}

static kern_return_t call_vm_write__src(MAP_T map, vm_address_t start,
                                        vm_size_t size) {
  allocation_t dst SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  kern_return_t kr = vm_write(map, (vm_address_t)dst.addr, start,
                              (mach_msg_type_number_t)size);
  return kr;
}

static kern_return_t call_vm_write__dst(MAP_T map, vm_address_t start,
                                        vm_size_t size) {
  allocation_t src SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  kern_return_t kr =
      vm_write(map, start, (vm_offset_t)src.addr, (mach_msg_type_number_t)size);
  return kr;
}
#endif
...
#define IMPL(FN, T, FLAVOR, PROT)                                              \
  static kern_return_t call_##FN##__##FLAVOR(MAP_T map, T start, T size) {     \
    mach_port_t host_priv = HOST_PRIV_NULL;                                    \
    kern_return_t kr = host_get_host_priv_port(mach_host_self(), &host_priv);  \
    assert(kr ==                                                               \
           0); /* host priv port on macOS requires entitlements or root */     \
...
typedef kern_return_t (*map_fn_t)(
...
static kern_return_t call_map_fn__allocate_fixed(map_fn_t fn, MAP_T map,
                                                 mach_vm_address_t start,
                                                 mach_vm_size_t size) {
  mach_vm_address_t out_addr = start;
  kern_return_t kr =
      fn(map, &out_addr, size, 0, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, 0, 0, 0,
         0, 0, VM_INHERIT_NONE);
  // fixed-overwrite with pre-existing allocation, don't deallocate
  return kr;
}

static kern_return_t call_map_fn__allocate_fixed_copy(map_fn_t fn, MAP_T map,
                                                      mach_vm_address_t start,
                                                      mach_vm_size_t size) {
  mach_vm_address_t out_addr = start;
  kern_return_t kr =
      fn(map, &out_addr, size, 0, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, 0, 0,
         true, 0, 0, VM_INHERIT_NONE);
  // fixed-overwrite with pre-existing allocation, don't deallocate
  return kr;
}

static kern_return_t call_map_fn__allocate_anywhere(
    map_fn_t fn, MAP_T map, mach_vm_address_t start_hint, mach_vm_size_t size) {
  mach_vm_address_t out_addr = start_hint;
  kern_return_t kr = fn(map, &out_addr, size, 0, VM_FLAGS_ANYWHERE, 0, 0, 0, 0,
                        0, VM_INHERIT_NONE);
  if (kr == 0) {
...
static kern_return_t call_map_fn__memobject_fixed(map_fn_t fn, MAP_T map,
                                                  mach_vm_address_t start,
                                                  mach_vm_size_t size) {
  mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);
  mach_vm_address_t out_addr = start;
  kern_return_t kr = fn(
      map, &out_addr, size, 0, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, memobject,
...
static kern_return_t call_map_fn__memobject_fixed_copy(map_fn_t fn, MAP_T map,
                                                       mach_vm_address_t start,
                                                       mach_vm_size_t size) {
  mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);
  mach_vm_address_t out_addr = start;
  kern_return_t kr = fn(
      map, &out_addr, size, 0, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, memobject,
      KB16, true, VM_PROT_DEFAULT, VM_PROT_DEFAULT, VM_INHERIT_DEFAULT);
  (void)mach_port_deallocate(mach_task_self(), memobject);
  // fixed-overwrite with pre-existing allocation, don't deallocate
  return kr;
}

static kern_return_t call_map_fn__memobject_anywhere(
    map_fn_t fn, MAP_T map, mach_vm_address_t start_hint, mach_vm_size_t size) {
  mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);
  mach_vm_address_t out_addr = start_hint;
  kern_return_t kr =
      fn(map, &out_addr, size, 0, VM_FLAGS_ANYWHERE, memobject, KB16, false,
...
static kern_return_t helper_call_map_fn__memobject__ssoo(
    map_fn_t fn, MAP_T map, int flags, bool copy, mach_vm_address_t start,
    mach_vm_size_t size, vm_object_offset_t offset, mach_vm_size_t obj_size) {
  mach_port_t memobject = make_a_mem_object(obj_size);
  mach_vm_address_t out_addr = start;
  kern_return_t kr = fn(map, &out_addr, size, 0, flags, memobject, offset, copy,
                        VM_PROT_DEFAULT, VM_PROT_DEFAULT, VM_INHERIT_DEFAULT);
...
static kern_return_t call_map_fn__memobject_fixed__start_size_offset_object(
    map_fn_t fn, MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
    vm_object_offset_t offset, mach_vm_size_t obj_size) {
  return helper_call_map_fn__memobject__ssoo(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, false, start, size, offset,
      obj_size);
}
...
call_map_fn__memobject_fixed_copy__start_size_offset_object(
    ...
    vm_object_offset_t offset, mach_vm_size_t obj_size) {
  return helper_call_map_fn__memobject__ssoo(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, true, start, size, offset,
      obj_size);
}

static kern_return_t call_map_fn__memobject_anywhere__start_size_offset_object(
    map_fn_t fn, MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
    vm_object_offset_t offset, mach_vm_size_t obj_size) {
  return helper_call_map_fn__memobject__ssoo(fn, map, VM_FLAGS_ANYWHERE, false,
                                             start, size, offset, obj_size);
}

static kern_return_t
help_call_map_fn__allocate__inherit(map_fn_t fn, MAP_T map, int flags,
                                    bool copy, mach_vm_address_t start,
                                    mach_vm_size_t size, vm_inherit_t inherit) {
  mach_vm_address_t out_addr = start;
  kern_return_t kr = fn(map, &out_addr, size, 0, flags, 0, KB16, copy,
                        VM_PROT_DEFAULT, VM_PROT_DEFAULT, inherit);
  deallocate_if_not_fixed_overwrite(kr, map, out_addr, size, flags);
  return kr;
}

static kern_return_t call_map_fn__allocate_fixed__inherit(
    map_fn_t fn, MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
    vm_inherit_t inherit) {
  return help_call_map_fn__allocate__inherit(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, false, start, size,
      inherit);
}

static kern_return_t call_map_fn__allocate_fixed_copy__inherit(
    map_fn_t fn, MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
    vm_inherit_t inherit) {
  return help_call_map_fn__allocate__inherit(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, true, start, size, inherit);
}

static kern_return_t call_map_fn__allocate_anywhere__inherit(
    map_fn_t fn, MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
    vm_inherit_t inherit) {
  return help_call_map_fn__allocate__inherit(fn, map, VM_FLAGS_ANYWHERE, false,
                                             start, size, inherit);
}

static kern_return_t help_call_map_fn__memobject__inherit(
    map_fn_t fn, MAP_T map, int flags, bool copy, mach_vm_address_t start,
    mach_vm_size_t size, vm_inherit_t inherit) {
  mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);
  mach_vm_address_t out_addr = start;
  kern_return_t kr = fn(map, &out_addr, size, 0, flags, memobject, KB16, copy,
                        VM_PROT_DEFAULT, VM_PROT_DEFAULT, inherit);
  deallocate_if_not_fixed_overwrite(kr, map, out_addr, size, flags);
  (void)mach_port_deallocate(mach_task_self(), memobject);
  return kr;
...
static kern_return_t call_map_fn__memobject_fixed__inherit(
    map_fn_t fn, MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
    vm_inherit_t inherit) {
  return help_call_map_fn__memobject__inherit(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, false, start, size,
      inherit);
}

static kern_return_t call_map_fn__memobject_fixed_copy__inherit(
    map_fn_t fn, MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
    vm_inherit_t inherit) {
  return help_call_map_fn__memobject__inherit(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, true, start, size, inherit);
...
static kern_return_t call_map_fn__memobject_anywhere__inherit(
    map_fn_t fn, MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
    vm_inherit_t inherit) {
  return help_call_map_fn__memobject__inherit(fn, map, VM_FLAGS_ANYWHERE, false,
                                              start, size, inherit);
}

static kern_return_t call_map_fn__allocate__flags(map_fn_t fn, MAP_T map,
                                                  mach_vm_address_t *start,
                                                  mach_vm_size_t size,
                                                  int flags) {
  kern_return_t kr = fn(map, start, size, 0, flags, 0, KB16, false,
                        VM_PROT_DEFAULT, VM_PROT_DEFAULT, VM_INHERIT_DEFAULT);
  deallocate_if_not_fixed_overwrite(kr, map, *start, size, flags);
  return kr;
}

static kern_return_t call_map_fn__allocate_copy__flags(map_fn_t fn, MAP_T map,
                                                       mach_vm_address_t *start,
                                                       mach_vm_size_t size,
                                                       int flags) {
  kern_return_t kr = fn(map, start, size, 0, flags, 0, KB16, false,
...
static kern_return_t call_map_fn__memobject__flags(map_fn_t fn, MAP_T map,
                                                   mach_vm_address_t *start,
                                                   mach_vm_size_t size,
                                                   int flags) {
  mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);
  kern_return_t kr = fn(map, start, size, 0, flags, memobject, KB16, false,
                        VM_PROT_DEFAULT, VM_PROT_DEFAULT, VM_INHERIT_DEFAULT);
  deallocate_if_not_fixed_overwrite(kr, map, *start, size, flags);
  (void)mach_port_deallocate(mach_task_self(), memobject);
  return kr;
...
static kern_return_t
call_map_fn__memobject_copy__flags(map_fn_t fn, MAP_T map,
                                   mach_vm_address_t *start,
                                   mach_vm_size_t size, int flags) {
  mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);
  kern_return_t kr = fn(map, start, size, 0, flags, memobject, KB16, true,
                        VM_PROT_DEFAULT, VM_PROT_DEFAULT, VM_INHERIT_DEFAULT);
  deallocate_if_not_fixed_overwrite(kr, map, *start, size, flags);
  (void)mach_port_deallocate(mach_task_self(), memobject);
  return kr;
...
static kern_return_t
help_call_map_fn__allocate__prot_pairs(map_fn_t fn, MAP_T map, int flags,
                                       bool copy, vm_prot_t cur,
                                       vm_prot_t max) {
  mach_vm_address_t out_addr = 0;
  kern_return_t kr = fn(map, &out_addr, KB16, 0, flags, 0, KB16, copy, cur, max,
                        VM_INHERIT_DEFAULT);
  deallocate_if_not_fixed_overwrite(kr, map, out_addr, KB16, flags);
  return kr;
}

static kern_return_t call_map_fn__allocate_fixed__prot_pairs(map_fn_t fn,
                                                             MAP_T map,
                                                             vm_prot_t cur,
                                                             vm_prot_t max) {
  return help_call_map_fn__allocate__prot_pairs(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, false, cur, max);
}

static kern_return_t
call_map_fn__allocate_fixed_copy__prot_pairs(map_fn_t fn, MAP_T map,
                                             vm_prot_t cur, vm_prot_t max) {
  return help_call_map_fn__allocate__prot_pairs(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, true, cur, max);
}

static kern_return_t call_map_fn__allocate_anywhere__prot_pairs(map_fn_t fn,
                                                                MAP_T map,
                                                                vm_prot_t cur,
                                                                vm_prot_t max) {
  return help_call_map_fn__allocate__prot_pairs(fn, map, VM_FLAGS_ANYWHERE,
                                                false, cur, max);
}

static kern_return_t
help_call_map_fn__memobject__prot_pairs(map_fn_t fn, MAP_T map, int flags,
                                        bool copy, vm_prot_t cur,
                                        vm_prot_t max) {
  mach_port_t memobject = make_a_mem_object(TEST_ALLOC_SIZE + 1);
  mach_vm_address_t out_addr = 0;
  kern_return_t kr = fn(map, &out_addr, KB16, 0, flags, memobject, KB16, copy,
...
static kern_return_t call_map_fn__memobject_fixed__prot_pairs(map_fn_t fn,
                                                              MAP_T map,
                                                              vm_prot_t cur,
                                                              vm_prot_t max) {
  return help_call_map_fn__memobject__prot_pairs(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, false, cur, max);
}

static kern_return_t
call_map_fn__memobject_fixed_copy__prot_pairs(map_fn_t fn, MAP_T map,
                                              vm_prot_t cur, vm_prot_t max) {
  return help_call_map_fn__memobject__prot_pairs(
      fn, map, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, true, cur, max);
}
...
static kern_return_t
call_map_fn__memobject_anywhere__prot_pairs(map_fn_t fn, MAP_T map,
                                            vm_prot_t cur, vm_prot_t max) {
  return help_call_map_fn__memobject__prot_pairs(fn, map, VM_FLAGS_ANYWHERE,
                                                 false, cur, max);
}
...
#define IMPL_MAP_FN_START_SIZE(map_fn, instance)                               \
  static kern_return_t call_##map_fn##__##instance(                            \
      MAP_T map, mach_vm_address_t start, mach_vm_size_t size) {               \
    return call_map_fn__##instance(map_fn, map, start, size);                  \
  }

#define IMPL_MAP_FN_HINT_SIZE(map_fn, instance)                                \
  static kern_return_t call_##map_fn##__##instance(                            \
      MAP_T map, mach_vm_address_t start_hint, mach_vm_size_t size) {          \
    return call_map_fn__##instance(map_fn, map, start_hint, size);             \
  }

#define IMPL_MAP_FN_START_SIZE_OFFSET_OBJECT(map_fn, instance)                 \
  static kern_return_t                                                         \
  call_##map_fn##__##instance##__start_size_offset_object(                     \
      MAP_T map, mach_vm_address_t start, mach_vm_size_t size,                 \
      vm_object_offset_t offset, mach_vm_size_t obj_size) {                    \
    return call_map_fn__##instance##__start_size_offset_object(                \
        map_fn, map, start, size, offset, obj_size);                           \
  }

#define IMPL_MAP_FN_START_SIZE_INHERIT(map_fn, instance)                       \
  static kern_return_t call_##map_fn##__##instance##__inherit(                 \
      MAP_T map, mach_vm_address_t start, mach_vm_size_t size,                 \
      vm_inherit_t inherit) {                                                  \
    return call_map_fn__##instance##__inherit(map_fn, map, start, size,        \
                                              inherit);                        \
  }

#define IMPL_MAP_FN_START_SIZE_FLAGS(map_fn, instance)                         \
  static kern_return_t call_##map_fn##__##instance##__flags(                   \
      MAP_T map, mach_vm_address_t *start, mach_vm_size_t size, int flags) {   \
    return call_map_fn__##instance##__flags(map_fn, map, start, size, flags);  \
  }

#define IMPL_MAP_FN_PROT_PAIRS(map_fn, instance)                               \
  static kern_return_t call_##map_fn##__##instance##__prot_pairs(              \
      MAP_T map, vm_prot_t cur, vm_prot_t max) {                               \
...
static kern_return_t
mach_vm_map_wrapped(vm_map_t target_task, mach_vm_address_t *address,
  ...
  kern_return_t kr =
      mach_vm_map(target_task, &addr, size, mask, flags, object, offset, copy,
...
#if TEST_OLD_STYLE_MACH
static kern_return_t
vm_map_64_retyped(vm_map_t target_task, mach_vm_address_t *address,
                  mach_vm_size_t size, mach_vm_offset_t mask, int flags,
                  mem_entry_name_port_t object, memory_object_offset_t offset,
                  boolean_t copy, vm_prot_t cur_protection,
                  vm_prot_t max_protection, vm_inherit_t inheritance) {
  vm_address_t addr = (vm_address_t)*address;
  kern_return_t kr = vm_map_64(
      target_task, &addr, (vm_size_t)size, (vm_address_t)mask, flags, object,
      (vm_offset_t)offset, copy, cur_protection, max_protection, inheritance);
  check_mach_vm_map_outparam_changes(&kr, addr, (vm_address_t)*address, flags,
                                     target_task);
  *address = addr;
  return kr;
}
...
static kern_return_t
vm_map_retyped(vm_map_t target_task, mach_vm_address_t *address,
               mach_vm_size_t size, mach_vm_offset_t mask, int flags,
               mem_entry_name_port_t object, memory_object_offset_t offset,
               boolean_t copy, vm_prot_t cur_protection,
               vm_prot_t max_protection, vm_inherit_t inheritance) {
...
// mmap
// Directly calling this symbol lets us hit the syscall directly instead of the
// libsyscall wrapper.
void *__mmap(void *addr, size_t len, int prot, int flags, int fildes,
             off_t off);

// We invert MAP_UNIX03 in the flags. This is because by default libsyscall
// intercepts calls to mmap and adds MAP_UNIX03. That means MAP_UNIX03 should be
// the default for most of our tests, and we should only test without MAP_UNIX03
// when we explicitly want to.
void *mmap_wrapper(void *addr, size_t len, int prot, int flags, int fildes,
                   off_t off) {
  flags ^= MAP_UNIX03;
...
static bool unsigned_code_is_disallowed(void) {
  if (isRosetta()) {
    return false;
  }
  ...
  if (0 == sysctlbyname("security.mac.amfi.unsigned_code_policy", &out_value,
                        &io_size, NULL, 0)) {
    return out_value;
  }
...
static int maybe_hide_mmap_failure(int ret, int prot, int fd) {
...
static kern_return_t help_call_mmap__vm_prot(MAP_T map __unused, int flags,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size,
                                             vm_prot_t prot) {
  int fd = -1;
...
static kern_return_t help_call_mmap__kernel_flags(MAP_T map __unused,
                                                  int mmap_flags,
                                                  mach_vm_address_t start,
                                                  mach_vm_size_t size,
                                                  int kernel_flags) {
  void *rv = mmap_wrapper((void *)start, (size_t)size, VM_PROT_DEFAULT,
                          mmap_flags, kernel_flags, 0);
  if (rv == MAP_FAILED) {
    return errno;
  } else {
    assert(0 == munmap_helper(rv, size));
...
static kern_return_t
help_call_mmap__dst_size_fileoff(MAP_T map __unused, int flags,
                                 mach_vm_address_t dst, mach_vm_size_t size,
                                 mach_vm_address_t fileoff) {
  int fd = -1;
  if (!(flags & MAP_ANON)) {
    fd = get_fd();
  }
  void *rv = mmap_wrapper((void *)dst, (size_t)size, VM_PROT_DEFAULT, flags, fd,
                          (off_t)fileoff);
  if (rv == MAP_FAILED) {
    return errno;
  } else {
...
static kern_return_t help_call_mmap__start_size(MAP_T map __unused, int flags,
                                                mach_vm_address_t start,
                                                mach_vm_size_t size) {
  int fd = -1;
  if (!(flags & MAP_ANON)) {
  ...
  void *rv =
      mmap_wrapper((void *)start, (size_t)size, VM_PROT_DEFAULT, flags, fd, 0);
  if (rv == MAP_FAILED) {
    return errno;
  } else {
    assert(0 == munmap_helper(rv, size));
...
static kern_return_t help_call_mmap__offset_size(MAP_T map __unused, int flags,
  ...
  void *rv = mmap_wrapper((void *)0, (size_t)size, VM_PROT_DEFAULT, flags, fd,
                          (off_t)offset);
  if (rv == MAP_FAILED) {
    return errno;
  } else {
...
#define IMPL_ONE_FROM_HELPER(type, variant, flags, ...)                        \
  static kern_return_t __attribute__((used)) call_mmap##__##variant##__##type( \
      MAP_T map, mach_vm_address_t start,                                      \
      mach_vm_size_t size DROP_COMMAS(__VA_ARGS__)) {                          \
    return help_call_mmap__##type(map, flags, start,                           \
                                  size DROP_TYPES(__VA_ARGS__));               \
  }

// call functions

#define IMPL_FROM_HELPER(type, ...)                                            \
  IMPL_ONE_FROM_HELPER(type, file_private, MAP_FILE | MAP_PRIVATE,             \
                       ##__VA_ARGS__)                                          \
  IMPL_ONE_FROM_HELPER(type, anon_private, MAP_ANON | MAP_PRIVATE,             \
                       ##__VA_ARGS__)                                          \
  IMPL_ONE_FROM_HELPER(type, file_shared, MAP_FILE | MAP_SHARED,               \
                       ##__VA_ARGS__)                                          \
  IMPL_ONE_FROM_HELPER(type, anon_shared, MAP_ANON | MAP_SHARED,               \
                       ##__VA_ARGS__)                                          \
  IMPL_ONE_FROM_HELPER(type, file_private_codesign,                            \
                       MAP_FILE | MAP_PRIVATE | MAP_RESILIENT_CODESIGN,        \
                       ##__VA_ARGS__)                                          \
  IMPL_ONE_FROM_HELPER(type, file_private_media,                               \
                       MAP_FILE | MAP_PRIVATE | MAP_RESILIENT_MEDIA,           \
                       ##__VA_ARGS__)                                          \
  IMPL_ONE_FROM_HELPER(type, nounix03_private,                                 \
                       MAP_FILE | MAP_PRIVATE | MAP_NOT_UNIX03, ##__VA_ARGS__) \
  IMPL_ONE_FROM_HELPER(type, fixed_private,                                    \
                       MAP_FILE | MAP_PRIVATE | MAP_FIXED, ##__VA_ARGS__)
...
IMPL_ONE_FROM_HELPER(kernel_flags, anon_private, MAP_ANON | MAP_PRIVATE, int,
                     kernel_flags)
IMPL_ONE_FROM_HELPER(kernel_flags, anon_shared, MAP_ANON | MAP_SHARED, int,
                     kernel_flags)

static kern_return_t call_mmap__mmap_flags(MAP_T map __unused,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size,
                                           int mmap_flags) {
  int fd = -1;
...
static kern_return_t
call_mach_memory_entry_ownership__ledger_tag(MAP_T map __unused,
                                             int ledger_tag) {
  mach_port_t mementry = make_a_mem_entry(TEST_ALLOC_SIZE + 1);
  kern_return_t kr =
      mach_memory_entry_ownership(mementry, mach_task_self(), ledger_tag, 0);
  (void)mach_port_deallocate(mach_task_self(), mementry);
  return kr;
}

static kern_return_t
call_mach_memory_entry_ownership__ledger_flag(MAP_T map __unused,
                                              int ledger_flag) {
  mach_port_t mementry = make_a_mem_entry(TEST_ALLOC_SIZE + 1);
  kern_return_t kr = mach_memory_entry_ownership(
      mementry, mach_task_self(), VM_LEDGER_TAG_DEFAULT, ledger_flag);
  (void)mach_port_deallocate(mach_task_self(), mementry);
  return kr;
}
...
kern_return_t short_circuit_deallocator(MAP_T map, start_size_trial_t trial) {
  ...
  if (!vm_sanitize_range_overflows_strict_zero(trial.start, trial.size,
...
static kern_return_t call_mach_vm_deallocate(MAP_T map, mach_vm_address_t start,
                                             mach_vm_size_t size) {
...
#if TEST_OLD_STYLE_MACH
static kern_return_t call_vm_deallocate(MAP_T map, mach_vm_address_t start,
                                        mach_vm_size_t size) {
...
static kern_return_t call_mach_vm_allocate__flags(MAP_T map,
                                                  mach_vm_address_t *start,
                                                  mach_vm_size_t size,
                                                  int flags) {
  mach_vm_address_t saved_start = *start;
  kern_return_t kr = mach_vm_allocate(map, start, size, flags);
  check_mach_vm_allocate_outparam_changes(&kr, *start, size, saved_start, flags,
                                          map);
  return kr;
}

static kern_return_t
call_mach_vm_allocate__start_size_fixed(MAP_T map, mach_vm_address_t *start,
                                        mach_vm_size_t size) {
  mach_vm_address_t saved_start = *start;
  kern_return_t kr = mach_vm_allocate(map, start, size, VM_FLAGS_FIXED);
  check_mach_vm_allocate_outparam_changes(&kr, *start, size, saved_start,
                                          VM_FLAGS_FIXED, map);
  return kr;
}

static kern_return_t
call_mach_vm_allocate__start_size_anywhere(MAP_T map, mach_vm_address_t *start,
                                           mach_vm_size_t size) {
  mach_vm_address_t saved_start = *start;
  kern_return_t kr = mach_vm_allocate(map, start, size, VM_FLAGS_ANYWHERE);
  check_mach_vm_allocate_outparam_changes(&kr, *start, size, saved_start,
                                          VM_FLAGS_ANYWHERE, map);
  return kr;
}

static kern_return_t call_mach_vm_inherit(MAP_T map, mach_vm_address_t start,
                                          mach_vm_size_t size) {
  kern_return_t kr = mach_vm_inherit(map, start, size, VM_INHERIT_NONE);
  return kr;
}
#if TEST_OLD_STYLE_MACH
static kern_return_t call_vm_inherit(MAP_T map, vm_address_t start,
                                     vm_size_t size) {
...
static int call_minherit(void *start, size_t size) {
  int err = minherit(start, size, VM_INHERIT_SHARE);
  return err ? errno : 0;
}

static kern_return_t call_mach_vm_inherit__inherit(MAP_T map,
                                                   mach_vm_address_t start,
                                                   mach_vm_size_t size,
                                                   vm_inherit_t value) {
...
static int call_minherit__inherit(void *start, size_t size, int value) {
  int err = minherit(start, size, value);
  return err ? errno : 0;
}

static kern_return_t call_mach_vm_protect__start_size(MAP_T map,
                                                      mach_vm_address_t start,
                                                      mach_vm_size_t size) {
  kern_return_t kr =
      mach_vm_protect(map, start, size, 0, VM_PROT_READ | VM_PROT_WRITE);
  return kr;
}
static kern_return_t call_mach_vm_protect__vm_prot(MAP_T map,
                                                   mach_vm_address_t start,
                                                   mach_vm_size_t size,
                                                   vm_prot_t prot) {
...
#if TEST_OLD_STYLE_MACH
static kern_return_t call_vm_protect__start_size(MAP_T map, vm_address_t start,
                                                 vm_size_t size) {
  kern_return_t kr =
      vm_protect(map, start, size, 0, VM_PROT_READ | VM_PROT_WRITE);
  return kr;
}
static kern_return_t call_vm_protect__vm_prot(MAP_T map, vm_address_t start,
...
static int call_mprotect__start_size(void *start, size_t size) {
  int err = __mprotect(start, size, PROT_READ | PROT_WRITE);
  return err ? errno : 0;
}

static int call_mprotect__vm_prot(void *start, size_t size, int prot) {
...
#if TEST_OLD_STYLE_MACH
static kern_return_t
call_vm_behavior_set__start_size__default(MAP_T map, vm_address_t start,
                                          vm_size_t size) {
...
static kern_return_t
call_vm_behavior_set__start_size__can_reuse(MAP_T map, vm_address_t start,
                                            vm_size_t size) {
  kern_return_t kr = vm_behavior_set(map, start, size, VM_BEHAVIOR_CAN_REUSE);
  return kr;
}

static kern_return_t call_vm_behavior_set__vm_behavior(MAP_T map,
                                                       vm_address_t start,
                                                       vm_size_t size,
...
extern int __shared_region_map_and_slide_2_np(
    uint32_t files_count, const struct shared_file_np *files,
    uint32_t mappings_count,
    const struct shared_file_mapping_slide_np *mappings);

static int
maybe_hide_shared_region_map_failure(int ret, uint32_t files_count,
...
static int call_shared_region_map_and_slide_2_np_child(
    uint32_t files_count, const struct shared_file_np *files,
    uint32_t mappings_count,
    const struct shared_file_mapping_slide_np *mappings) {
  int err = __shared_region_map_and_slide_2_np(files_count, files,
                                               mappings_count, mappings);
  return err ? maybe_hide_shared_region_map_failure(errno, files_count, files,
                                                    mappings_count)
             : 0;
}
...
void *thread_func(void *args) {
  ...
  int err = call_shared_region_map_and_slide_2_np_child(
      files_count, files, mappings_count, mappings);
...
static int call_shared_region_map_and_slide_2_np_in_thread(
    uint32_t files_count, const struct shared_file_np *files,
    uint32_t mappings_count,
    const struct shared_file_mapping_slide_np *mappings) {
  // From vm/vm_shared_region.c: After a chroot(), the calling process keeps
  // using its original shared region [...] But its children will use a
  // different shared region [...]
  if (chroot(".") < 0) {
    return BUSTED;
  }
...
static int call_madvise__start_size(void *start, size_t size) {
  int err = madvise(start, size, MADV_NORMAL);
  return err ? errno : 0;
}

static int call_madvise__vm_advise(void *start, size_t size, int advise) {
  int err = madvise(start, size, advise);
  return err ? errno : 0;
}

static int call_mach_vm_msync__start_size(MAP_T map, mach_vm_address_t start,
                                          mach_vm_size_t size) {
  kern_return_t kr = mach_vm_msync(map, start, size, VM_SYNC_ASYNCHRONOUS);
  return kr;
}

static int call_mach_vm_msync__vm_sync(MAP_T map, mach_vm_address_t start,
                                       mach_vm_size_t size, vm_sync_t sync) {
  kern_return_t kr = mach_vm_msync(map, start, size, sync);
  return kr;
}

#if TEST_OLD_STYLE_MACH
static int call_vm_msync__start_size(MAP_T map, vm_address_t start,
                                     vm_size_t size) {
  kern_return_t kr = vm_msync(map, start, size, VM_SYNC_ASYNCHRONOUS);
  return kr;
}

static int call_vm_msync__vm_sync(MAP_T map, vm_address_t start, vm_size_t size,
                                  vm_sync_t sync) {
...
static int call_msync__start_size(void *start, size_t size) {
  int err = __msync(start, size, MS_SYNC);
  return err ? errno : 0;
}

static int call_msync__vm_msync(void *start, size_t size, int msync_value) {
...
static int call_msync_nocancel__start_size(void *start, size_t size) {
  int err = __msync_nocancel(start, size, MS_SYNC);
  return err ? errno : 0;
}

static int call_msync_nocancel__vm_msync(void *start, size_t size,
                                         int msync_value) {
  int err = __msync_nocancel(start, size, msync_value);
  return err ? errno : 0;
}

static void check_mach_vm_machine_attribute_outparam_changes(
    kern_return_t *kr, vm_machine_attribute_val_t value,
    vm_machine_attribute_val_t saved_value) {
  if (value != saved_value) {
    *kr = OUT_PARAM_BAD;
  }
}

static int call_mach_vm_machine_attribute__start_size(MAP_T map,
                                                      mach_vm_address_t start,
                                                      mach_vm_size_t size) {
  vm_machine_attribute_val_t value = MATTR_VAL_GET;
  vm_machine_attribute_val_t initial_value = value;
  kern_return_t kr =
      mach_vm_machine_attribute(map, start, size, MATTR_CACHE, &value);
  check_mach_vm_machine_attribute_outparam_changes(&kr, value, initial_value);
  return kr;
}

static int call_mach_vm_machine_attribute__machine_attribute(
    MAP_T map, mach_vm_address_t start, mach_vm_size_t size,
...
#if TEST_OLD_STYLE_MACH
static int call_vm_machine_attribute__start_size(MAP_T map, vm_address_t start,
                                                 vm_size_t size) {
  vm_machine_attribute_val_t value = MATTR_VAL_GET;
  vm_machine_attribute_val_t initial_value = value;
  kern_return_t kr =
      vm_machine_attribute(map, start, size, MATTR_CACHE, &value);
  check_mach_vm_machine_attribute_outparam_changes(&kr, value, initial_value);
  return kr;
}

static int
call_vm_machine_attribute__machine_attribute(MAP_T map, vm_address_t start,
                                             vm_size_t size,
                                             vm_machine_attribute_t attr) {
  vm_machine_attribute_val_t value = MATTR_VAL_GET;
  vm_machine_attribute_val_t initial_value = value;
...
static int call_mach_vm_purgable_control__address__get(MAP_T map,
                                                       mach_vm_address_t addr) {
  int state = INVALID_PURGABLE_STATE;
  int initial_state = state;
  kern_return_t kr =
      mach_vm_purgable_control(map, addr, VM_PURGABLE_GET_STATE, &state);
  check_mach_vm_purgable_control_outparam_changes(&kr, state, initial_state,
                                                  VM_PURGABLE_GET_STATE);
  return kr;
}

static int
call_mach_vm_purgable_control__address__purge_all(MAP_T map,
                                                  mach_vm_address_t addr) {
  int state = INVALID_PURGABLE_STATE;
  int initial_state = state;
  kern_return_t kr =
      mach_vm_purgable_control(map, addr, VM_PURGABLE_PURGE_ALL, &state);
  check_mach_vm_purgable_control_outparam_changes(&kr, state, initial_state,
                                                  VM_PURGABLE_PURGE_ALL);
  return kr;
}

static int call_mach_vm_purgable_control__purgeable_state(
    MAP_T map, mach_vm_address_t addr, vm_purgable_t control, int state) {
  int initial_state = state;
...
#if TEST_OLD_STYLE_MACH
static int call_vm_purgable_control__address__get(MAP_T map,
                                                  vm_address_t addr) {
  int state = INVALID_PURGABLE_STATE;
  int initial_state = state;
  kern_return_t kr =
      vm_purgable_control(map, addr, VM_PURGABLE_GET_STATE, &state);
  check_mach_vm_purgable_control_outparam_changes(&kr, state, initial_state,
                                                  VM_PURGABLE_GET_STATE);
  return kr;
}

static int call_vm_purgable_control__address__purge_all(MAP_T map,
                                                        vm_address_t addr) {
  int state = INVALID_PURGABLE_STATE;
  int initial_state = state;
  kern_return_t kr =
      vm_purgable_control(map, addr, VM_PURGABLE_PURGE_ALL, &state);
  check_mach_vm_purgable_control_outparam_changes(&kr, state, initial_state,
                                                  VM_PURGABLE_PURGE_ALL);
  return kr;
}

static int call_vm_purgable_control__purgeable_state(MAP_T map,
                                                     vm_address_t addr,
...
static void check_mach_vm_region_recurse_outparam_changes(
    kern_return_t *kr, void *info, void *saved_info, size_t info_size,
    natural_t depth, natural_t saved_depth, mach_vm_address_t addr,
    mach_vm_address_t saved_addr, mach_vm_size_t size,
    mach_vm_size_t saved_size) {
  if (*kr == KERN_SUCCESS) {
    ...
    if (depth != saved_depth || addr != saved_addr || size != saved_size ||
        memcmp(info, saved_info, info_size) != 0) {
      *kr = OUT_PARAM_BAD;
    }
  }
}

static kern_return_t call_mach_vm_region_recurse(MAP_T map,
                                                 mach_vm_address_t addr) {
  ...
  kern_return_t kr =
      mach_vm_region_recurse(map, &addr_cpy, &size_out, &depth,
                             (vm_region_recurse_info_t)&info, &count);
  check_mach_vm_region_recurse_outparam_changes(
      &kr, &info, &saved_info, sizeof(info), depth, saved_depth, addr, addr_cpy,
      size_out, saved_size);
...
#if TEST_OLD_STYLE_MACH
static kern_return_t call_vm_region_recurse(MAP_T map, vm_address_t addr) {
  ...
  kern_return_t kr = vm_region_recurse(map, &addr_cpy, &size_out, &depth,
                                       (vm_region_recurse_info_t)&info, &count);

  check_mach_vm_region_recurse_outparam_changes(
      &kr, &info, &saved_info, sizeof(info), depth, saved_depth, addr_cpy, addr,
      size_out, saved_size);

  return kr;
}

static kern_return_t call_vm_region_recurse_64(MAP_T map, vm_address_t addr) {
  vm_region_submap_info_data_64_t info;
  info.inheritance = INVALID_INHERIT;
  vm_region_submap_info_data_64_t saved_info = info;
  ...
  kern_return_t kr =
      vm_region_recurse_64(map, &addr_cpy, &size_out, &depth,
                           (vm_region_recurse_info_t)&info, &count);
...
static kern_return_t call_mach_vm_page_info(MAP_T map, mach_vm_address_t addr) {
  ...
  kern_return_t kr =
      mach_vm_page_info(map, addr, flavor, (vm_page_info_t)&info, &count);
  check_mach_vm_page_info_outparam_changes(&kr, info, saved_info, count,
                                           saved_count);
  return kr;
}

static void check_mach_vm_page_query_outparam_changes(kern_return_t *kr,
                                                      int disposition,
                                                      int saved_disposition,
                                                      int ref_count) {
  if (*kr == KERN_SUCCESS) {
    /*
...
static kern_return_t call_mach_vm_page_query(MAP_T map,
                                             mach_vm_address_t addr) {
...
#if TEST_OLD_STYLE_MACH
static kern_return_t call_vm_map_page_query(MAP_T map, vm_address_t addr) {
  int disp = INVALID_DISPOSITION_VALUE, ref = 0;
  int saved_disposition = disp;
...
static void check_mach_vm_page_range_query_outparam_changes(
    kern_return_t *kr, mach_vm_size_t out_count, mach_vm_size_t in_count) {
  if (out_count != in_count) {
    *kr = OUT_PARAM_BAD;
  }
}

static kern_return_t call_mach_vm_page_range_query(MAP_T map,
                                                   mach_vm_address_t start,
                                                   mach_vm_size_t size) {
  ...
  kern_return_t kr =
      mach_vm_allocate(map, &prq_buf, prq_buf_size + KB16, VM_FLAGS_ANYWHERE);
  assert(kr == 0);
  ...
  // yes, EFAULT as a kern_return_t because mach_vm_page_range_query returns
  // copyio's error
  if (kr == EFAULT) {
    bool bad = false;
...
static int call_mincore(void *start, size_t size) {
  ...
  char *mincore_buf = 0;
  mincore_buf = mmap(NULL, mincore_buf_size + KB16, PROT_READ | PROT_WRITE,
...
static vm_map_kernel_flags_trials_t *generate_mmap_kernel_flags_trials() {
...
static results_t *test_mmap_with_allocated_vm_map_kernel_flags_t(
    kern_return_t (*func)(MAP_T map, mach_vm_address_t src, mach_vm_size_t size,
                          int flags),
    const char *testname) {
  MAP_T map SMART_MAP;
  ...
  results_t *results =
      alloc_results(testname, eSMART_MMAP_KERNEL_FLAGS_TRIALS, trials->count);
...
static results_t *test_unix_with_allocated_vm_inherit_t(unix_with_inherit_fn fn,
                                                        const char *testname) {
  MAP_T map SMART_MAP;
  allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  vm_inherit_trials_t *trials SMART_VM_INHERIT_TRIALS();
  results_t *results =
      alloc_results(testname, eSMART_VM_INHERIT_TRIALS, trials->count);

  for (unsigned i = 0; i < trials->count; i++) {
    vm_inherit_trial_t trial = trials->list[i];
    int ret =
        fn((void *)(uintptr_t)base.addr, (size_t)base.size, (int)trial.value);
    append_result(results, ret, trial.name);
  }
  return results;
}
...
static results_t *test_unix_with_allocated_vm_msync_t(unix_with_msync_fn fn,
                                                      const char *testname) {
  MAP_T map SMART_MAP;
  allocation_t base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  vm_msync_trials_t *trials SMART_VM_MSYNC_TRIALS();
  results_t *results =
      alloc_results(testname, eSMART_VM_MSYNC_TRIALS, trials->count);
...
static results_t *test_unix_with_allocated_aligned_vm_advise_t(
    unix_with_advise_fn fn, mach_vm_size_t align_mask, const char *testname) {
  MAP_T map SMART_MAP;
  allocation_t base SMART_ALLOCATE_ALIGNED_VM(map, TEST_ALLOC_SIZE, align_mask,
                                              VM_PROT_DEFAULT);
  vm_advise_trials_t *trials SMART_VM_ADVISE_TRIALS();
  results_t *results =
      alloc_results(testname, eSMART_VM_ADVISE_TRIALS, trials->count);
...
// Rosetta userspace intercepts shared_region_map_and_slide_2_np calls and this
// Rosetta wrapper function doesn't have the necessary checks to support invalid
// input arguments. Skip these trials intead of crashing the test.
static bool shared_region_map_and_slide_would_crash(
    shared_region_map_and_slide_2_trial_t *trial) {
...
typedef int (*unix_shared_region_map_and_slide_2_np)(
    uint32_t files_coun, const struct shared_file_np *files,
    uint32_t mappings_count,
    const struct shared_file_mapping_slide_np *mappings);

static results_t *test_unix_shared_region_map_and_slide_2_np(
    unix_shared_region_map_and_slide_2_np func, const char *testname) {
  uint64_t dyld_fp = (uint64_t)get_dyld_fd();
  shared_region_map_and_slide_2_trials_t *trials
      SMART_SHARED_REGION_MAP_AND_SLIDE_2_TRIALS(dyld_fp);
  results_t *results =
      alloc_results(testname, eSMART_SHARED_REGION_MAP_AND_SLIDE_2_TRIALS,
                    dyld_fp, trials->count);
  ...
  for (unsigned i = 0; i < trials->count; i++) {
    ...
    if (isRosetta() && shared_region_map_and_slide_would_crash(&trial)) {
      ...
      ret = func(trial.files_count, trial.files, trial.mappings_count,
                 trial.mappings);
    }
    append_result(results, ret, trial.name);
  }
...
static results_t *test_dst_size_fileoff(
    kern_return_t (*func)(MAP_T map, mach_vm_address_t dst, mach_vm_size_t size,
                          mach_vm_address_t fileoff),
    const char *testname) {
  MAP_T map SMART_MAP;
  src_dst_size_trials_t *trials SMART_FILEOFF_DST_SIZE_TRIALS();
  results_t *results =
      alloc_results(testname, eSMART_FILEOFF_DST_SIZE_TRIALS, trials->count);
...
allocate_for_mmap_fixed(MAP_T map, mach_vm_address_t trial_dst,
                        mach_vm_size_t trial_size, bool trial_dst_is_absolute,
                        bool trial_size_is_absolute, mach_vm_address_t *out_dst,
                        mach_vm_size_t *out_size, bool *out_panic,
                        bool *out_slide) {
  *out_panic = false;
  *out_slide = false;
  ...
  if (trial_dst_is_absolute && trial_size_is_absolute) {
    ...
    *out_size = trial_size;
    kern_return_t kr =
        mach_vm_allocate(map, out_dst, *out_size, VM_FLAGS_FIXED);
    if (kr == KERN_NO_SPACE) {
    ...
    *out_size = TEST_ALLOC_SIZE;
    kern_return_t kr =
        mach_vm_allocate(map, out_dst, *out_size, VM_FLAGS_ANYWHERE);
    if (kr != 0) {
...
static __attribute__((overloadable)) void
allocate_for_mmap_fixed(MAP_T map, start_size_trial_t trial,
                        mach_vm_address_t *out_dst, mach_vm_size_t *out_size,
                        bool *out_panic, bool *out_slide) {
  allocate_for_mmap_fixed(map, trial.start, trial.size, trial.start_is_absolute,
                          trial.size_is_absolute, out_dst, out_size, out_panic,
                          out_slide);
}
static __attribute__((overloadable)) void
allocate_for_mmap_fixed(MAP_T map, src_dst_size_trial_t trial,
                        mach_vm_address_t *out_dst, mach_vm_size_t *out_size,
                        bool *out_panic, bool *out_slide) {
  allocate_for_mmap_fixed(map, trial.dst, trial.size, trial.dst_is_absolute,
                          !trial.size_is_dst_relative, out_dst, out_size,
                          out_panic, out_slide);
}
...
static results_t *test_fixed_dst_size_fileoff(
    kern_return_t (*func)(MAP_T map, mach_vm_address_t dst, mach_vm_size_t size,
                          mach_vm_address_t fileoff),
    const char *testname) {
  MAP_T map SMART_MAP;
  ...
  for (unsigned i = 0; i < trials->count; i++) {
    ...
    allocate_for_mmap_fixed(map, trial, &dst_alloc, &dst_size, &should_panic,
                            &should_slide_trial);
    if (should_panic) {
...
// Like test_mach_with_allocated_start_size, but specialized for
// mmap(MAP_FIXED). See test_fixed_dst_size_fileoff for more.
static results_t *
test_fixed_dst_size(kern_return_t (*func)(MAP_T map, mach_vm_address_t dst,
                                          mach_vm_size_t size),
                    const char *testname) {
  MAP_T map SMART_MAP;
  start_size_trials_t *trials SMART_START_SIZE_TRIALS(0); // no base addr
  results_t *results =
      alloc_results(testname, eSMART_START_SIZE_TRIALS, 0, trials->count);
  for (unsigned i = 0; i < trials->count; i++) {
...
static results_t *test_allocated_src_allocated_dst_size(
    kern_return_t (*func)(MAP_T map, mach_vm_address_t src, mach_vm_size_t size,
                          mach_vm_address_t dst),
    const char *testname) {
  /*
   ...
   */
  MAP_T map SMART_MAP;
  allocation_t src_base SMART_ALLOCATE_VM(map, TEST_ALLOC_SIZE,
                                          VM_PROT_DEFAULT);
  allocation_t dst_base SMART_ALLOCATE_VM_AFTER(
      map, src_base.addr, TEST_ALLOC_SIZE, VM_PROT_DEFAULT);
  assert(src_base.addr < dst_base.addr);
  src_dst_size_trials_t *trials SMART_SRC_DST_SIZE_TRIALS();
  results_t *results =
      alloc_results(testname, eSMART_SRC_DST_SIZE_TRIALS, trials->count);
...
static void disable_exc_guard() {
  T_SETUPBEGIN;

  // Disable EXC_GUARD for the duration of the test.
  // We restore it at the end.
  kern_return_t kr =
      task_get_exc_guard_behavior(mach_task_self(), &saved_exc_guard_behavior);
  assert(kr == 0);
...
static void restore_exc_guard() {
  // restore process's EXC_GUARD handling
  (void)task_set_exc_guard_behavior(mach_task_self(), saved_exc_guard_behavior);
}

static int set_disable_vm_sanitize_telemetry_via_sysctl(uint32_t val) {
  int ret = sysctlbyname("debug.disable_vm_sanitize_telemetry", NULL, NULL,
                         &val, sizeof(uint32_t));
  if (ret != 0) {
...
static int disable_vm_sanitize_telemetry(void) {
  return set_disable_vm_sanitize_telemetry_via_sysctl(1);
}

static int reenable_vm_sanitize_telemetry(void) {
...
#define FILL_TRIALS_NAMES(results, trials)                                     \
  {                                                                            \
    unsigned t_count = ((trials)->count < (results)->count)                    \
                           ? (trials)->count                                   \
                           : (results)->count;                                 \
...
static void fill_golden_trials(uint64_t trialsargs[static TRIALSARGUMENTS_SIZE],
                               results_t *results) {
  ...
  case eSMART_VM_TAG_TRIALS: {
    // special case, trails (vm_tag_trials_values) depend on data only available
    // on KERNEL
  ...
  case eSMART_START_SIZE_OFFSET_OBJECT_TRIALS: {
    start_size_offset_object_trials_t *trials
    SMART_START_SIZE_OFFSET_OBJECT_TRIALS();
    FILL_TRIALS_NAMES(results, trials);
  }
  ...
  case eSMART_VM_PURGEABLE_AND_STATE_TRIALS: {
    vm_purgeable_and_state_trials_t *trials
    SMART_VM_PURGEABLE_AND_STATE_TRIALS();
    FILL_TRIALS_NAMES(results, trials);
  }
  ...
  case eSMART_SHARED_REGION_MAP_AND_SLIDE_2_TRIALS: {
    shared_region_map_and_slide_2_trials_t *trials
        SMART_SHARED_REGION_MAP_AND_SLIDE_2_TRIALS(trialsargs0);
    FILL_TRIALS_NAMES(results, trials);
  }
  ...
  default:
    T_FAIL("New formula %u, args %llu %llu, update fill_golden_trials, "
           "testname: %s\n",
...
static results_t *test_name_to_golden_results(const char *testname) {
...
static void dump_results_list(results_t *res_list[], uint32_t res_num_tests) {
...
static void dump_kernel_results_list() {
  testprintf("======\n");
...
static int populate_golden_results(const char *filename) {
  FILE *file;
  ...
  kern_trialname_generation =
      strnstr(filename, "kern_golden_image", strlen(filename)) != NULL;
  ...
  while (fgets(line, MAX_LINE_LENGTH, file) != NULL) {
    ...
    if (strncmp(line, TESTNAME_DELIMITER, strlen(TESTNAME_DELIMITER)) == 0) {
      ...
      in_test = TRUE;
    } else if (in_test && strncmp(line, TRIALSFORMULA_DELIMITER,
                                  strlen(TRIALSFORMULA_DELIMITER)) == 0) {
      sscanf(line, "%*s %s %*s %llu,%llu,%llu", trial_formula, &trial_args[0],
             &trial_args[1], &trial_page_size);
      formula = trialsformula_from_string(trial_formula);
    } else if (in_test && strncmp(line, RESULTCOUNT_DELIMITER,
                                  strlen(RESULTCOUNT_DELIMITER)) == 0) {
      ...
      results = alloc_results(test_name, formula, trial_args,
                              TRIALSARGUMENTS_SIZE, num_results);
      assert(results);
      results->count = num_results;
      fill_golden_trials(trial_args, results);
      golden_list[num_tests++] = results;
      // T_LOG("num_tests %u, testname %s, count: %u", num_tests,
      // results->testname, results->count);
    } else if (in_test && strncmp(line, TESTRESULT_DELIMITER,
                                  strlen(TESTRESULT_DELIMITER)) == 0) {
      ...
      if (result_ret == OUT_PARAM_BAD) {
        ...
        T_FAIL("Out parameter violation in test %s - %s\n", results->testname,
               results->list[result_number].name);
      }
    } else {
...
static void clean_golden_results() {
  for (uint32_t x = 0; x < num_tests; ++x) {
    if (golden_list[x]->tested_count == 0) {
      T_LOG("WARN: Test %s found in golden file but no test with that name was "
            "run\n",
...
static void clean_kernel_results() {
...
// buffer to output userspace golden file results (using same size as the kern
// buffer)
...
void goldenprintf(const char *format, ...) {
...
// Read existing golden file and print its contents in verbose format (like
// dump_results). Controlled by DUMP_GOLDEN_IMAGE.
static bool dump_golden = FALSE;
// Run tests as tests (i.e. emit TS_{PASS/FAIL}), enabled unless golden image
// generation is true.
...
static void read_env() {
  ...
  should_test_results =
      (getenv("SKIP_TESTS") == NULL) && !dump_golden && !generate_golden;
}
...
static unsigned binary_search(result_t *list, unsigned count,
                              const result_t *trial) {
...
static inline bool trial_name_equals(const result_t *a, const result_t *b) {
  // NB: strlen match need to handle cases where a shorter 'bname' would match a
  // longer 'aname'.
...
static const result_t *get_golden_result(results_t *golden_results,
                                         const result_t *trial,
                                         unsigned trial_idx) {
  ...
  qsort(golden_results->list, golden_results->count, sizeof(result_t),
        compare_names);
  unsigned g_idx =
      binary_search(golden_results->list, golden_results->count, trial);
...
static void test_results(results_t *golden_results, results_t *results) {
  bool passed = TRUE;
    ...
    if (golden_result) {
      if (results->list[i].ret == ACCEPTABLE) {
        ...
        T_FAIL("%s RESULT %d (expected %d), %s\n", results->testname,
               results->list[i].ret, golden_result->ret, results->list[i].name);
        passed = FALSE;
      }
    } else {
      /*
      ...
      T_FAIL("%s NEW RESULT %d, %s -- %s\n", results->testname,
             results->list[i].ret, results->list[i].name, suggestion);
      passed = FALSE;
    }
  }
  ...
  if (acceptable_count > acceptable_max) {
    T_LOG("%s %u more RESULT ACCEPTABLE trials not logged\n", results->testname,
          acceptable_count - acceptable_max);
  }
  if (passed) {
    T_PASS("%s passed\n", results->testname);
  }
...
static results_t *process_results(results_t *results) {
  results_t *golden_results = NULL;
  ...
  if (should_test_results) {
    ...
    if (golden_results) {
      ...
      T_FAIL(
          "New test %s found, update golden list to allow return code testing",
          results->testname);
...
T_DECL(vm_parameter_validation_user, "parameter validation for userspace calls",
       T_META_SPAWN_TOOL(DECOMPRESS), T_META_SPAWN_TOOL_ARG("user"),
  ...
  T_LOG("dump %d, golden %d, dump_golden %d, test %d\n", dump, generate_golden,
        dump_golden, should_test_results);
  ...
  if (generate_golden && unsigned_code_is_disallowed()) {
    ...
    T_FAIL("Can't generate golden files with SIP enabled. Disable SIP and try "
           "again.\n");
    return;
  }

  if ((dump_golden || should_test_results) &&
      populate_golden_results(GOLDEN_FILE)) {
   ...
   * The memory entry test functions use macros to generate each flavor of
   * memory entry function. This is partially becauseof many entrypoints
   * (mach_make_memory_entry/mach_make_memory_entry_64/mach_make_memory_entry)
   * and partially because many flavors of each function are called
   * (copy/memonly/share/...).
   */

  // Mach start/size with both old-style and new-style types
  // (co-located so old and new can be compared more easily)
#define RUN_NEW(fn, name)                                                      \
  dealloc_results(process_results(                                             \
      test_mach_with_allocated_start_size(fn, name " (start/size)")))
#if TEST_OLD_STYLE_MACH
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(process_results(                                             \
      test_oldmach_with_allocated_start_size(fn, name " (start/size)")))
#define RUN_OLD64(fn, name) RUN_NEW(fn, name)
#else
#define RUN_OLD(fn, name)                                                      \
  do {                                                                         \
  } while (0)
#define RUN_OLD64(fn, name)                                                    \
  do {                                                                         \
  } while (0)
#endif
  // mach_make_memory_entry has up to three entry points on U32, unlike other
  // functions that have two
  RUN_NEW(call_mach_make_memory_entry_64__start_size__copy,
          "mach_make_memory_entry_64 (copy)");
  RUN_OLD(call_mach_make_memory_entry__start_size__copy,
          "mach_make_memory_entry (copy)");
  RUN_OLD64(call__mach_make_memory_entry__start_size__copy,
            "_mach_make_memory_entry (copy)");
  RUN_NEW(call_mach_make_memory_entry_64__start_size__memonly,
          "mach_make_memory_entry_64 (mem_only)");
  RUN_OLD(call_mach_make_memory_entry__start_size__memonly,
          "mach_make_memory_entry (mem_only)");
  RUN_OLD64(call__mach_make_memory_entry__start_size__memonly,
            "_mach_make_memory_entry (mem_only)");
  RUN_NEW(call_mach_make_memory_entry_64__start_size__namedcreate,
          "mach_make_memory_entry_64 (named_create)");
  RUN_OLD(call_mach_make_memory_entry__start_size__namedcreate,
          "mach_make_memory_entry (named_create)");
  RUN_OLD64(call__mach_make_memory_entry__start_size__namedcreate,
            "_mach_make_memory_entry (named_create)");
  RUN_NEW(call_mach_make_memory_entry_64__start_size__share,
          "mach_make_memory_entry_64 (share)");
  RUN_OLD(call_mach_make_memory_entry__start_size__share,
          "mach_make_memory_entry (share)");
  RUN_OLD64(call__mach_make_memory_entry__start_size__share,
            "_mach_make_memory_entry (share)");
  RUN_NEW(call_mach_make_memory_entry_64__start_size__namedreuse,
          "mach_make_memory_entry_64 (named_reuse)");
  RUN_OLD(call_mach_make_memory_entry__start_size__namedreuse,
          "mach_make_memory_entry (named_reuse)");
  RUN_OLD64(call__mach_make_memory_entry__start_size__namedreuse,
            "_mach_make_memory_entry (named_reuse)");
#undef RUN_NEW
#undef RUN_OLD
#undef RUN_OLD64

#define RUN(fn, name)                                                          \
  dealloc_results(process_results(test_mach_with_size(fn, name " (size)")))
  RUN(call_mach_memory_object_memory_entry_64__size,
      "mach_memory_object_memory_entry_64");
  RUN(call_replacement_mach_memory_object_memory_entry__size,
      "mach_memory_object_memory_entry");
#undef RUN

#define RUN_NEW(fn, name)                                                      \
  dealloc_results(process_results(                                             \
      test_mach_with_allocated_vm_prot_t(fn, name " (vm_prot_t)")))
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(process_results(                                             \
      test_oldmach_with_allocated_vm_prot_t(fn, name " (vm_prot_t)")))
#define RUN_OLD64(fn, name) RUN_NEW(fn, name)
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(test_mach_vm_prot(fn, name " (vm_prot_t)")))
  RUN(call_mach_memory_object_memory_entry_64__vm_prot,
      "mach_memory_object_memory_entry_64");
  RUN(call_replacement_mach_memory_object_memory_entry__vm_prot,
      "mach_memory_object_memory_entry");
#undef RUN

#define RUN(fn, name)                                                          \
  dealloc_results(                                                             \
      process_results(test_mach_with_ledger_tag(fn, name " (ledger tag)")))
  RUN(call_mach_memory_entry_ownership__ledger_tag,
      "mach_memory_entry_ownership");
#undef RUN

#define RUN(fn, name)                                                          \
  dealloc_results(                                                             \
      process_results(test_mach_with_ledger_flag(fn, name " (ledger flag)")))
  RUN(call_mach_memory_entry_ownership__ledger_flag,
      "mach_memory_entry_ownership");
#undef RUN
...
#define RUN(fn, name)                                                          \
  dealloc_results(                                                             \
      process_results(test_mach_allocation_func_with_start_size(fn, name)))
  RUN(call_mach_vm_allocate__start_size_fixed,
      "mach_vm_allocate (fixed) (realigned start/size)");
  RUN(call_mach_vm_allocate__start_size_anywhere,
      "mach_vm_allocate (anywhere) (hint/size)");
#undef RUN

#define RUN(fn, name)                                                          \
  dealloc_results(                                                             \
      process_results(test_mach_allocation_func_with_vm_map_kernel_flags_t(    \
          fn, name " (vm_map_kernel_flags_t)")))
  RUN(call_mach_vm_allocate__flags, "mach_vm_allocate");
#undef RUN

  dealloc_results(process_results(test_deallocator(
      call_mach_vm_deallocate, "mach_vm_deallocate (start/size)")));
#if TEST_OLD_STYLE_MACH
  dealloc_results(process_results(
      test_deallocator(call_vm_deallocate, "vm_deallocate (start/size)")));
#endif

#define RUN(fn, name)                                                          \
  dealloc_results(process_results(test_deallocator(fn, name " (start/size)")))
...
#undef RUN
   ...
   * The macros are used both for function generation (see IMPL_ONE_FROM_HELPER)
   * and to call all of those. This was written this way to further avoid lots
   * of code duplication, as the map/remap functions have many different
   * parameter combinations we want to test.
   */

  // map tests

#define RUN_START_SIZE(fn, name)                                               \
  dealloc_results(process_results(test_mach_with_allocated_start_size(         \
      fn, name " (realigned start/size)")))
#define RUN_HINT_SIZE(fn, name)                                                \
  dealloc_results(process_results(                                             \
      test_mach_with_allocated_start_size(fn, name " (hint/size)")))
#define RUN_PROT_PAIR(fn, name)                                                \
  dealloc_results(                                                             \
      process_results(test_mach_vm_prot_pair(fn, name " (prot_pairs)")))
#define RUN_INHERIT(fn, name)                                                  \
  dealloc_results(process_results(                                             \
      test_mach_with_allocated_vm_inherit_t(fn, name " (vm_inherit_t)")))
#define RUN_FLAGS(fn, name)                                                    \
  dealloc_results(                                                             \
      process_results(test_mach_allocation_func_with_vm_map_kernel_flags_t(    \
          fn, name " (vm_map_kernel_flags_t)")))
#define RUN_SSOO(fn, name)                                                     \
  dealloc_results(                                                             \
      process_results(test_mach_with_allocated_start_size_offset_object(       \
          fn, name " (start/size/offset/object)")))

#define RUN_ALL(fn, name)                                                      \
  RUN_START_SIZE(call_##fn##__allocate_fixed,                                  \
                 #name " (allocate fixed overwrite)");                         \
  RUN_START_SIZE(call_##fn##__allocate_fixed_copy,                             \
                 #name " (allocate fixed overwrite copy)");                    \
  RUN_START_SIZE(call_##fn##__memobject_fixed,                                 \
                 #name " (memobject fixed overwrite)");                        \
  RUN_START_SIZE(call_##fn##__memobject_fixed_copy,                            \
                 #name " (memobject fixed overwrite copy)");                   \
  RUN_HINT_SIZE(call_##fn##__allocate_anywhere, #name " (allocate anywhere)"); \
  RUN_HINT_SIZE(call_##fn##__memobject_anywhere,                               \
                #name " (memobject anywhere)");                                \
  RUN_PROT_PAIR(call_##fn##__allocate_fixed__prot_pairs,                       \
                #name " (allocate fixed overwrite)");                          \
  RUN_PROT_PAIR(call_##fn##__allocate_fixed_copy__prot_pairs,                  \
                #name " (allocate fixed overwrite copy)");                     \
  RUN_PROT_PAIR(call_##fn##__allocate_anywhere__prot_pairs,                    \
                #name " (allocate anywhere)");                                 \
  RUN_PROT_PAIR(call_##fn##__memobject_fixed__prot_pairs,                      \
                #name " (memobject fixed overwrite)");                         \
  RUN_PROT_PAIR(call_##fn##__memobject_fixed_copy__prot_pairs,                 \
                #name " (memobject fixed overwrite copy)");                    \
  RUN_PROT_PAIR(call_##fn##__memobject_anywhere__prot_pairs,                   \
                #name " (memobject anywhere)");                                \
  RUN_INHERIT(call_##fn##__allocate_fixed__inherit,                            \
              #name " (allocate fixed overwrite)");                            \
  RUN_INHERIT(call_##fn##__allocate_fixed_copy__inherit,                       \
              #name " (allocate fixed overwrite copy)");                       \
  RUN_INHERIT(call_##fn##__allocate_anywhere__inherit,                         \
              #name " (allocate anywhere)");                                   \
  RUN_INHERIT(call_##fn##__memobject_fixed__inherit,                           \
              #name " (memobject fixed overwrite)");                           \
  RUN_INHERIT(call_##fn##__memobject_fixed_copy__inherit,                      \
              #name " (memobject fixed overwrite copy)");                      \
  RUN_INHERIT(call_##fn##__memobject_anywhere__inherit,                        \
              #name " (memobject anywhere)");                                  \
  ...
  RUN_SSOO(call_##fn##__memobject_fixed__start_size_offset_object,             \
           #name " (memobject fixed overwrite)");                              \
  RUN_SSOO(call_##fn##__memobject_fixed_copy__start_size_offset_object,        \
           #name " (memobject fixed overwrite copy)");                         \
  RUN_SSOO(call_##fn##__memobject_anywhere__start_size_offset_object,          \
           #name " (memobject anywhere)");
...
#define RUN_HELPER(harness, fn, variant, type, type_name, name)                \
  dealloc_results(process_results(harness(FN_NAME(fn, variant, type), #name    \
                                          " (" #variant ") (" type_name ")")))
#define RUN_SRC_SIZE(fn, variant, type_name, name)                             \
  RUN_HELPER(test_mach_with_allocated_start_size, fn, variant, src_size,       \
             type_name, name)
#define RUN_DST_SIZE(fn, variant, type_name, name)                             \
  RUN_HELPER(test_mach_with_allocated_start_size, fn, variant, dst_size,       \
             type_name, name)
#define RUN_PROT_PAIRS(fn, variant, name)                                      \
  RUN_HELPER(test_mach_with_allocated_vm_prot_pair, fn, variant, prot_pairs,   \
             "prot_pairs", name)
#define RUN_INHERIT(fn, variant, name)                                         \
  RUN_HELPER(test_mach_with_allocated_vm_inherit_t, fn, variant, inherit,      \
             "inherit", name)
#define RUN_FLAGS(fn, variant, name)                                           \
  RUN_HELPER(test_mach_with_allocated_vm_map_kernel_flags_t, fn, variant,      \
             flags, "flags", name)
#define RUN_SRC_DST_SIZE(fn, dst, variant, type_name, name)                    \
  RUN_HELPER(test_allocated_src_##dst##_dst_size, fn, variant, src_dst_size,   \
             type_name, name)
...
#define RUN(fn, name)                                                          \
  dealloc_results(                                                             \
      process_results(test_mmap_with_allocated_vm_map_kernel_flags_t(          \
          fn, name " (kernel flags)")))
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(                                             \
      test_mach_with_allocated_mmap_flags(fn, name " (mmap flags)")))
...
#define RUN(fn, name)                                                          \
  ...
  RUN(call_mmap__file_private_codesign__start_size,
      "mmap (file private codesign)");
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(test_fixed_dst_size(fn, name " (dst/"        \
                                                               "size)")))
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(                                             \
      test_mach_with_allocated_start_size(fn, name " (offset/size)")))
  ...
  RUN(call_mmap__file_private_codesign__offset_size,
      "mmap (file private codesign)");
...
#define RUN(fn, name)                                                          \
  dealloc_results(                                                             \
      process_results(test_dst_size_fileoff(fn, name " (hint/size/fileoff)")))
  ...
  RUN(call_mmap__file_private_codesign__dst_size_fileoff,
      "mmap (file private codesign)");
  RUN(call_mmap__file_private_media__dst_size_fileoff,
      "mmap (file private media)");
  RUN(call_mmap__nounix03_private__dst_size_fileoff, "mmap (no unix03)");
#undef RUN

#define RUN(fn, name)                                                          \
  dealloc_results(process_results(                                             \
      test_fixed_dst_size_fileoff(fn, name " (dst/size/fileoff)")))
...
#define RUN(fn, name)                                                          \
  ...
  RUN(call_mmap__file_private_codesign__vm_prot,
      "mmap (file private codesign)");
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(                                             \
      test_unix_with_allocated_start_size(fn, name " (start/size)")))
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(                                             \
      test_unix_with_allocated_vm_inherit_t(fn, name " (vm_inherit_t)")))
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(                                             \
      test_unix_with_allocated_vm_prot_t(fn, name " (vm_prot_t)")))
...
#endif
  RUN_NEW(call_mach_vm_behavior_set__start_size__default,
          "mach_vm_behavior_set (VM_BEHAVIOR_DEFAULT)");
  RUN_OLD(call_vm_behavior_set__start_size__default,
          "vm_behavior_set (VM_BEHAVIOR_DEFAULT)");
#undef RUN_NEW
#undef RUN_OLD

#define RUN_NEW(fn, name)                                                      \
  dealloc_results(process_results(test_mach_with_allocated_aligned_start_size( \
      fn, alignment_for_can_reuse, name " (start/size)")))
#if TEST_OLD_STYLE_MACH
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_oldmach_with_allocated_aligned_start_size(          \
          fn, alignment_for_can_reuse, name " (start/size)")))
#else
#define RUN_OLD(fn, name)                                                      \
  do {                                                                         \
...
#endif
  RUN_NEW(call_mach_vm_behavior_set__start_size__can_reuse,
          "mach_vm_behavior_set (VM_BEHAVIOR_CAN_REUSE)");
  RUN_OLD(call_vm_behavior_set__start_size__can_reuse,
          "vm_behavior_set (VM_BEHAVIOR_CAN_REUSE)");
#undef RUN_NEW
#undef RUN_OLD

#define RUN_NEW(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_mach_with_allocated_aligned_vm_behavior_t(          \
          fn, alignment_for_can_reuse, name " (vm_behavior_t)")))
#if TEST_OLD_STYLE_MACH
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_oldmach_with_allocated_aligned_vm_behavior_t(       \
          fn, alignment_for_can_reuse, name " (vm_behavior_t)")))
#else
#define RUN_OLD(fn, name)                                                      \
  do {                                                                         \
...
#define RUN(fn, name)                                                          \
  dealloc_results(                                                             \
      process_results(test_unix_with_allocated_aligned_vm_advise_t(            \
          fn, alignment_for_can_reuse, name " (vm_advise_t)")))
...
#define RUN_NEW(fn, name)                                                      \
  dealloc_results(process_results(                                             \
      test_mach_with_allocated_vm_sync_t(fn, name " (vm_sync_t)")))
#if TEST_OLD_STYLE_MACH
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(process_results(                                             \
      test_oldmach_with_allocated_vm_sync_t(fn, name " (vm_sync_t)")))
#else
#define RUN_OLD(fn, name)                                                      \
  do {                                                                         \
  } while (0)
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(                                             \
      test_unix_with_allocated_vm_msync_t(fn, name " (msync flags)")))
...
#endif
  RUN_NEW(call_mach_vm_machine_attribute__start_size,
...
#define RUN_NEW(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_mach_with_allocated_vm_machine_attribute_t(         \
          fn, name " (machine_attribute_t)")))
#if TEST_OLD_STYLE_MACH
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_oldmach_with_allocated_vm_machine_attribute_t(      \
          fn, name " (machine_attribute_t)")))
#else
#define RUN_OLD(fn, name)                                                      \
  do {                                                                         \
...
#endif
  RUN_NEW(call_mach_vm_machine_attribute__machine_attribute,
          "mach_vm_machine_attribute");
...
#define RUN_NEW(fn, name)                                                      \
  dealloc_results(process_results(                                             \
      test_mach_with_allocated_purgeable_addr(fn, name " (addr)")))
#if TEST_OLD_STYLE_MACH
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(process_results(                                             \
      test_oldmach_with_allocated_purgeable_addr(fn, name " (addr)")))
#else
#define RUN_OLD(fn, name)                                                      \
  do {                                                                         \
  } while (0)
#endif
  RUN_NEW(call_mach_vm_purgable_control__address__get,
          "mach_vm_purgable_control (get)");
  RUN_OLD(call_vm_purgable_control__address__get, "vm_purgable_control (get)");

  RUN_NEW(call_mach_vm_purgable_control__address__purge_all,
          "mach_vm_purgable_control (purge all)");
  RUN_OLD(call_vm_purgable_control__address__purge_all,
          "vm_purgable_control (purge all)");
#undef RUN_NEW
#undef RUN_OLD
#define RUN_NEW(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_mach_with_allocated_purgeable_and_state(            \
          fn, name " (purgeable and state)")))
#if TEST_OLD_STYLE_MACH
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_oldmach_with_allocated_purgeable_and_state(         \
          fn, name " (purgeable and state)")))
#else
#define RUN_OLD(fn, name)                                                      \
  do {                                                                         \
...
#endif
  RUN_NEW(call_mach_vm_purgable_control__purgeable_state,
...
#define RUN_NEW(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_mach_with_allocated_addr(fn, name " (addr)")))
#if TEST_OLD_STYLE_MACH
#define RUN_OLD(fn, name)                                                      \
  dealloc_results(                                                             \
      process_results(test_oldmach_with_allocated_addr(fn, name " (addr)")))
#else
#define RUN_OLD(fn, name)                                                      \
  do {                                                                         \
  } while (0)
...
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(test_unix_shared_region_map_and_slide_2_np(  \
      fn, name " (files/mappings)")))
  RUN(call_shared_region_map_and_slide_2_np_child,
      "shared_region_map_and_slide_2_np");
  RUN(call_shared_region_map_and_slide_2_np_in_thread,
      "different thread shared_region_map_and_slide_2_np");
#undef RUN

#if 0
#define RUN(fn, name)                                                          \
  dealloc_results(process_results(                                             \
      test_mach_vm_deferred_reclamation_buffer_init(fn, name)))
...
out:
  ...
  if (generate_golden) {
    if (!out_param_bad_count || (dump && !should_test_results)) {
      // Print after verified there is not OUT_PARAM_BAD results before
      // printing, or user explicitly set DUMP_RESULTS=1 GENERATE_GOLDEN_IMAGE=1
...
static int out_bad_param_in_kern_golden_results(char *kern_buffer) {
  ...
  if (out_param_bad_match) {
    T_FAIL("Out parameter violation return code (%s) found in results, "
           "aborting.\n",
           out_param_bad_str);
    return 1;
  }
...
static int populate_kernel_results(char *kern_buffer) {
  char *line = NULL;
  char *test_name = NULL;
  ...
  while (line != NULL) {
    if (strncmp(line, TESTNAME_DELIMITER, strlen(TESTNAME_DELIMITER)) == 0) {
      ...
      uint32_t num_kern_results =
          (uint32_t)strtoul(s_num_kern_results, NULL, 10);
      kern_results =
          alloc_results(test_name, eUNKNOWN_TRIALS, num_kern_results);
      kern_list[num_kern_tests++] = kern_results;
    } else if (in_test && strncmp(line, TESTCONFIG_DELIMITER,
                                  strlen(TESTCONFIG_DELIMITER)) == 0) {
    ...
    } else if (strncmp(line, KERN_FAILURE_DELIMITER,
                       strlen(KERN_FAILURE_DELIMITER)) == 0) {
      /*
...
static int64_t run_sysctl_test(const char *t, int64_t value) {
  char name[1024];
  int64_t result = 0;
...
T_DECL(vm_parameter_validation_kern, "parameter validation for kext/xnu calls",
       T_META_SPAWN_TOOL(DECOMPRESS), T_META_SPAWN_TOOL_ARG("kern"),
       T_META_SPAWN_TOOL_ARG(TMP_DIR),
       T_META_SPAWN_TOOL_ARG(GOLDEN_FILES_VERSION),
       T_META_SPAWN_TOOL_ARG(GOLDEN_FILES_ARCH)) {
  if (disable_vm_sanitize_telemetry() != 0) {
  ...
  int64_t result =
      run_sysctl_test("vm_parameter_validation_kern_v2", (int64_t)&args);
  ...
  if (generate_golden) {
    if (!out_bad_param_in_kern_golden_results(output) ||
        (dump && !should_test_results)) {
      // Print after verified there is not OUT_PARAM_BAD results before
      // printing, or user explicitly set DUMP_RESULTS=1 GENERATE_GOLDEN_IMAGE=1
      printf("%s", output);
    }
