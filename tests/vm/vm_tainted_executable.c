...
T_DECL(vm_tainted_executable, "Test that a tainted executable gets killed",
       T_META_TAG_VM_PREFERRED, T_META_IGNORECRASHES(".*hell0.*")) {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd1, "open(./hello)");
  fd2 = mkstemp(tmp_path);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fd2, "mkstemp(%s)", tmp_path);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fstat(fd1, &fs), NULL);
  fsize = (size_t)fs.st_size;
  mapaddr1 = mmap(NULL, fsize, PROT_READ, MAP_FILE | MAP_PRIVATE, fd1, 0);
  T_QUIET;
  T_ASSERT_NOTNULL(mapaddr1, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(write(fd2, mapaddr1, fsize), NULL);
  ...
  for (;;) {
    char zero = '0';
    big_cp = memmem(big_cp, (size_t)(big_ep - big_cp), little_cp, little_len);
    if (big_cp == NULL) {
      break;
    }
    T_LOG("found string at offset 0x%llx", (off_t)(big_cp - big_sp));
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(pwrite(fd2, &zero, 1, (big_cp - big_sp + 4)), NULL);
    big_cp += little_len;
  }
  /* make the new binary "r-x" */
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(fchmod(fd2, S_IRUSR | S_IXUSR), NULL);
  /* cleanup */
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(close(fd1), NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(close(fd2), NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(munmap(mapaddr1, fsize), NULL);
  T_SETUPEND;
