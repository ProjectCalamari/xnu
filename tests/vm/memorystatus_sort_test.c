...
#define NUM_PER_ROLE                                                           \
  3 /* Number of procs per role in coalition (besides leader) */
#define NUM_PROCS_IN_COALITION                                                 \
  (NUM_PER_ROLE * (COALITION_NUM_TASKROLES - 1) + 1)
#define NUM_COALITIONS 3

#define COAL_ORDER_NUM_PIDS                                                    \
  (NUM_PROCS_IN_COALITION + COALITION_NUM_TASKROLES - 1)
typedef struct {
  pid_t pids[NUM_PROCS_IN_COALITION]; // An array of pids in this coalition.
                                      // Owned by this struct.
  pid_t
      expected_order[COAL_ORDER_NUM_PIDS]; // An array of pids in this coalition
 ...
 */
static pid_t launch_proc_in_coalition(uint64_t *coalition_ids, int role,
                                      int num_pages);

static void bufprint(char **buf, size_t *size, const char *fmt, ...) {
  va_list list;
  int n_written;
...
static char *pids_str(pid_t *pids, int n_pids) {
  int i;
 ...
 */
static void sort_and_verify(unsigned int prio,
                            memorystatus_jetsam_sort_order_t order,
                            pid_t *expected_order, size_t expected_order_len) {
  ...
  memcpy(original_expected_order, expected_order,
         sizeof(pid_t) * expected_order_len);
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(ret, "Band sorted and order copied out");
  ...
  T_EXPECT_TRUE(in_order, "Band in correct order when sorted in order (%d)",
                order);
...
static void random_order(int *arr, int size) {
  int i, a, b, s;
  for (i = 0; i < size; i++) {
    arr[i] = i;
  }
...
static void add_coalition_to_order(pid_t *order, coalition_info_t *coal,
                                   int coal_idx) {
  int order_idx = coal_idx * (COAL_ORDER_NUM_PIDS + 1);
  memcpy(&order[order_idx], &coal->expected_order,
         sizeof(coal->expected_order));
  if (coal_idx != 0) {
    order[order_idx - 1] = -1;
  }
}
 ...
 */
#define COALS_EXPECTED_ORDER_LEN                                               \
  ((COAL_ORDER_NUM_PIDS * NUM_COALITIONS) + (NUM_COALITIONS - 1))
T_DECL(memorystatus_sort_coalitions_footprint,
       "Sort coalitions by leader footprint", T_META_ASROOT(true),
  ...
  /* Sort by leader footprint and verify coalitions are sorted by leader
   * footprint */
  sort_and_verify(JETSAM_PRIORITY_FOREGROUND, JETSAM_SORT_FOOTPRINT,
                  expected_order, COALS_EXPECTED_ORDER_LEN);
...
T_DECL(memorystatus_sort_coalitions_lru, "Sort coalitions by leader LRU",
  ...
  sort_and_verify(JETSAM_PRIORITY_FOREGROUND, JETSAM_SORT_LRU, expected_order,
                  COALS_EXPECTED_ORDER_LEN);
...
#define kNumChildren 3
  ...
  /*
   * The expected sort order of the children in the order that they were
   * launched. Used to construct the expected_order pid array. Note that procs
   * should be sorted in descending footprint order.
   */
  ...
  // After we spawn the children, they'll signal that they've touched their
  // pages.
  res = signal(SIGUSR1, SIG_IGN);
  T_WITH_ERRNO;
  T_ASSERT_NE(res, SIG_ERR, "SIG_IGN SIGUSR1");
  ds_allocated = dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, SIGUSR1, 0,
                                        dispatch_get_main_queue());
  T_QUIET;
  T_ASSERT_NOTNULL(ds_allocated, "dispatch_source_create (ds_allocated)");

  dispatch_source_set_event_handler(ds_allocated, ^{
    if (num_children < kNumChildren) {
      pid =
          launch_proc_in_coalition(NULL, 0, kChildrenFootprints[num_children]);
      ...
      sort_and_verify(kJetsamBand, JETSAM_SORT_FOOTPRINT_NOCOAL, expected_order,
                      kNumChildren);
      T_END;
    }
  });
...
static pid_t launch_proc_in_coalition(uint64_t *coalition_ids, int role,
  ...
  extern char **environ;
  T_QUIET;
  T_ASSERT_LT(num_children + 1, (size_t)kMaxChildrenProcs,
  ...
  T_QUIET;
  T_ASSERT_LE((size_t)ret, sizeof(num_pages_str),
...
static void init_coalition(coalition_info_t *coalition, size_t leader_fp) {
  ...
  for (int i = 0; i < COALITION_NUM_TYPES; i++) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_ZERO(ret, "coalition_create");
  }

  sigemptyset(&set);
  ret = sigaddset(&set, SIGUSR1);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "sigaddset(SIGUSR1)");
  ...
  for (size_t i = 0; i < NUM_PROCS_IN_COALITION; i++) {
    ...
    ret = sigwait(&set, &sig);
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ret, "sigwait");
    T_QUIET;
    T_ASSERT_EQ(sig, SIGUSR1, "sigwait == SIGUSR1");
...
static void place_proc_in_band(pid_t pid, int band) {
  ...
  props.priority = band;
  props.user_data = 0;
  ret = memorystatus_control(MEMORYSTATUS_CMD_SET_PRIORITY_PROPERTIES, pid, 0,
                             &props, sizeof(props));
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "move proc to band");
}

static void place_coalition_in_band(const coalition_info_t *coalition,
                                    int band) {
  for (size_t i = 0; i < NUM_PROCS_IN_COALITION; i++) {
...
static void cleanup_children(void) {
  ...
  for (size_t i = 0; i < num_children; i++) {
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(exited_pid, "waitpid");
    T_QUIET;
    T_ASSERT_TRUE(WIFSIGNALED(status), "proc was signaled.");
    T_QUIET;
    T_ASSERT_EQ(WTERMSIG(status), SIGKILL, "proc was killed");
  }
}

static bool has_unrestrict_coalitions() {
  int ret, val;
  size_t val_sz;
...
static void unrestrict_coalitions() {
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.unrestrict_coalitions <- 1");
}

static void restrict_coalitions() {
  int ret, val = 0;
  size_t val_sz;
  val_sz = sizeof(val);
  ret = sysctlbyname("kern.unrestrict_coalitions", NULL, 0, &val, val_sz);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "kern.unrestrict_coalitions <- 0");
}
