 ...
 */
__attribute__((overloadable)) static bool
...
/*
 * Return true if [start, start + size) is an unallocated hole at submap depth
 * 0. Unallocate space inside a submap does not count.
 */
__attribute__((overloadable, used)) static bool is_hole(mach_vm_address_t start,
                                                        mach_vm_size_t size) {
 ...
 */
static void assert_hole_checker_and_entry(vm_entry_checker_t *checker,
 ...
 */
static void assert_allocation_checker(vm_entry_checker_t *checker,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
  assert(start % PAGE_SIZE == 0);
  assert(size % PAGE_SIZE == 0);
 ...
 */
static void
assert_allocation_entry(mach_vm_address_t start, mach_vm_size_t size,
                        uint32_t submap_depth,
                        vm_region_submap_info_data_64_t *const out_info) {
  mach_vm_address_t entry_start = start;
  mach_vm_size_t entry_size;
  assert(
      get_info_for_address(&entry_start, &entry_size, out_info, submap_depth));
 ...
 * Returns the vm_region output for the memory.
 */
static void assert_allocation_checker_and_entry(
    vm_entry_checker_t *checker, mach_vm_address_t start, mach_vm_size_t size,
    vm_region_submap_info_data_64_t *const out_info) {
 ...
 */
static void assert_allocation_and_hole_pattern(checker_list_t *checker_list,
                                               const char *pattern) {
  ...
  for (size_t i = 0; i < strlen(pattern); i++) {
    ...
    switch (pattern[i]) {
    case '#':
      assert_allocation_checker_and_entry(checker, entry_address,
                                          DEFAULT_ENTRY_SIZE, &info);
      break;
    case '.':
      assert_hole_checker_and_entry(checker, entry_address, DEFAULT_ENTRY_SIZE);
      break;
    default:
      T_ASSERT_FAIL("pattern character '%c' is neither '#' nor '.'",
                    pattern[i]);
      break;
    }
  }
...
static void assert_checker_and_entry_protection_equals(
    vm_entry_checker_t *checker, vm_region_submap_info_data_64_t *info,
    int protection, int max_protection) {
 ...
 */
static void destructively_assert_permanent_checker_and_entry(
    vm_entry_checker_t *checker, mach_vm_address_t start, mach_vm_size_t size) {
  assert(start % PAGE_SIZE == 0);
  assert(size % PAGE_SIZE == 0);
 ...
 */
static void destructively_assert_nonpermanent_checker_and_entry(
    checker_list_t *list, vm_entry_checker_t *checker, mach_vm_address_t start,
    mach_vm_size_t size) {
  assert(start % PAGE_SIZE == 0);
  assert(size % PAGE_SIZE == 0);
 ...
 */
static void assert_submap_checker_and_entry(
    vm_entry_checker_t *checker, mach_vm_address_t start, mach_vm_size_t size,
    vm_region_submap_info_data_64_t *const out_info) {
  assert(start % PAGE_SIZE == 0);
  assert(size % PAGE_SIZE == 0);
  ...
  mach_vm_address_t entry_start = start;
  mach_vm_size_t entry_size;
  assert(get_info_for_address(&entry_start, &entry_size, out_info,
                              checker->submap_depth));
  assert(entry_start == start);
  assert(entry_size == size);
}

static test_result_t test_single_entry_1(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
...
static test_result_t test_single_entry_2(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
  vm_entry_checker_t *checker = checker_list_nth(checker_list, 0);
  vm_region_submap_info_data_64_t info;
  ...
  assert_allocation_checker_and_entry(checker, start, DEFAULT_ENTRY_SIZE,
                                      &info);

  return TestSucceeded;
}

static test_result_t test_single_entry_3(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
  vm_entry_checker_t *checker = checker_list_nth(checker_list, 0);
  vm_region_submap_info_data_64_t info;
  ...
  assert_allocation_checker_and_entry(
      checker, start - DEFAULT_PARTIAL_ENTRY_SIZE, DEFAULT_ENTRY_SIZE, &info);

  return TestSucceeded;
}

static test_result_t test_single_entry_4(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
  vm_entry_checker_t *checker = checker_list_nth(checker_list, 0);
  vm_region_submap_info_data_64_t info;
  ...
  assert_allocation_checker_and_entry(checker,
                                      start - DEFAULT_PARTIAL_ENTRY_SIZE / 2,
                                      DEFAULT_ENTRY_SIZE, &info);

  return TestSucceeded;
}

static test_result_t test_multiple_entries_1(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
...
static test_result_t test_multiple_entries_2(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "###");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_multiple_entries_3(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "############"
                                                   "############"
...
static test_result_t test_multiple_entries_4(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "###");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_multiple_entries_5(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "###");
  assert(start == checker_range_start_address(checker_list->entries) +
...
static test_result_t test_multiple_entries_6(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "###");
  assert(start == checker_range_start_address(checker_list->entries) +
...
static test_result_t test_some_holes_1(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, ".#");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_2(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, ".###");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_3(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, ".#");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_4(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, ".###");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_5(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "#.");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_6(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "###.");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_7(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "#.");
  assert(start == checker_range_start_address(checker_list->entries) +
...
static test_result_t test_some_holes_8(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "###.");
  assert(start == checker_range_start_address(checker_list->entries) +
...
static test_result_t test_some_holes_9(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "#.#");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_10(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "#.#.#");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_11(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "##.##.##");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_some_holes_12(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "###.###.###");
  assert(start == checker_range_start_address(checker_list->entries));
...
static test_result_t test_all_holes_1(checker_list_t *checker_list,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "#.#");
  assert(start == checker_range_start_address(checker_list->entries) +
...
static test_result_t test_all_holes_2(checker_list_t *checker_list,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, "#.");
  assert(start == checker_range_start_address(checker_list->entries) +
...
static test_result_t test_all_holes_3(checker_list_t *checker_list,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, ".#");
  assert(start == checker_range_start_address(checker_list->entries) +
...
static test_result_t test_all_holes_4(checker_list_t *checker_list,
                                      mach_vm_address_t start,
                                      mach_vm_size_t size) {
  assert_allocation_and_hole_pattern(checker_list, ".");
  assert(start == checker_range_start_address(checker_list->entries) +
...
static test_result_t test_null_entry(checker_list_t *checker_list,
                                     mach_vm_address_t start,
                                     mach_vm_size_t size) {
  vm_entry_checker_t *checker = checker_list_nth(checker_list, 0);
  vm_region_submap_info_data_64_t info;
...
static test_result_t test_nonresident_entry(checker_list_t *checker_list,
                                            mach_vm_address_t start,
                                            mach_vm_size_t size) {
  vm_entry_checker_t *checker = checker_list_nth(checker_list, 0);
  vm_region_submap_info_data_64_t info;
...
static test_result_t test_resident_entry(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
  vm_entry_checker_t *checker = checker_list_nth(checker_list, 0);
  vm_region_submap_info_data_64_t info;
...
static void test_one_shared_pair(checker_list_t *checker_list,
  ...
  assert_allocation_checker_and_entry(left_checker, left_entry_start, size,
                                      &left_info);
  assert_allocation_checker_and_entry(right_checker, right_entry_start, size,
                                      &right_info);
...
static test_result_t test_shared_entry(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
...
test_shared_entry_discontiguous(checker_list_t *checker_list,
                                mach_vm_address_t start, mach_vm_size_t size) {
  /*
   * right entry's object offset begins
   * after the left entry's range ends
   */
  test_one_shared_pair(checker_list, start, start + size, size,
                       DEFAULT_ENTRY_SIZE);
  return TestSucceeded;
}

static test_result_t test_shared_entry_partial(checker_list_t *checker_list,
                                               mach_vm_address_t start,
                                               mach_vm_size_t size) {
  /*
   * right entry's object offset begins
...
static test_result_t test_shared_entry_pairs(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  /*
   * two shared pairs
...
static test_result_t test_shared_entry_x1000(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  /* many entries, all of which share the same object */

  entry_checker_range_t limit =
      checker_list_find_range(checker_list, start, size);
...
/* common code for two-shared-entry tests */
static test_result_t test_cow_entry(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size) {
  /*
  ...
  assert_allocation_checker_and_entry(right_checker, start + size, size,
                                      &right_info);
...
static test_result_t test_cow_unreferenced(checker_list_t *checker_list,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size) {
  /*
...
static test_result_t test_cow_nocow_common(
    vm_entry_checker_t *cow_checker, vm_entry_checker_t *plain_checker,
    mach_vm_address_t cow_start_address, mach_vm_address_t plain_start_address,
    mach_vm_size_t entry_size) {
  /* two entries: one is COW, one is not COW */
  vm_region_submap_info_data_64_t cow_info, plain_info;
  assert_allocation_checker_and_entry(cow_checker, cow_start_address,
                                      entry_size, &cow_info);
  assert_allocation_checker_and_entry(plain_checker, plain_start_address,
                                      entry_size, &plain_info);
...
static test_result_t test_cow_nocow(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size) {
  /*
  ...
  return test_cow_nocow_common(cow_checker, plain_checker, cow_start,
                               plain_start, size / 2);
}

static test_result_t test_nocow_cow(checker_list_t *checker_list,
                                    mach_vm_address_t start,
                                    mach_vm_size_t size) {
  /*
...
static test_result_t test_cow_unreadable(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
  /*
...
static test_result_t test_cow_unwriteable(checker_list_t *checker_list,
                                          mach_vm_address_t start,
                                          mach_vm_size_t size) {
  /*
...
static test_result_t test_permanent_entry(checker_list_t *checker_list,
                                          mach_vm_address_t start,
                                          mach_vm_size_t size) {
...
static test_result_t
test_permanent_before_permanent(checker_list_t *checker_list,
                                mach_vm_address_t start, mach_vm_size_t size) {
  destructively_assert_permanent_checker_and_entry(
      checker_list_nth(checker_list, 0), start, size / 2);
  destructively_assert_permanent_checker_and_entry(
      checker_list_nth(checker_list, 1), start + size / 2, size / 2);

  return TestSucceeded;
}

static test_result_t
test_permanent_before_allocation(checker_list_t *checker_list,
                                 mach_vm_address_t start, mach_vm_size_t size) {
  destructively_assert_permanent_checker_and_entry(
      checker_list_nth(checker_list, 0), start, size / 2);
  destructively_assert_nonpermanent_checker_and_entry(
      checker_list, checker_list_nth(checker_list, 1), start + size / 2,
      size / 2);
  assert_hole_checker_and_entry(checker_list_nth(checker_list, 2), start + size,
                                DEFAULT_ENTRY_SIZE);
  return TestSucceeded;
}

static test_result_t
test_permanent_before_allocation_2(checker_list_t *checker_list,
                                   mach_vm_address_t start,
                                   mach_vm_size_t size) {
  vm_region_submap_info_data_64_t info;
  destructively_assert_permanent_checker_and_entry(
  ...
  destructively_assert_nonpermanent_checker_and_entry(
      checker_list, checker_list_nth(checker_list, 1), start + size / 2,
      size / 2);
  assert_allocation_checker_and_entry(checker_list_nth(checker_list, 2),
                                      start + size, DEFAULT_ENTRY_SIZE, &info);

  return TestSucceeded;
}

static test_result_t test_permanent_before_hole(checker_list_t *checker_list,
                                                mach_vm_address_t start,
                                                mach_vm_size_t size) {
  destructively_assert_permanent_checker_and_entry(
      checker_list_nth(checker_list, 0), start, size / 2);
  assert_hole_checker_and_entry(checker_list_nth(checker_list, 1),
                                start + size / 2, size / 2);
...
static test_result_t
test_permanent_after_allocation(checker_list_t *checker_list,
                                mach_vm_address_t start, mach_vm_size_t size) {
  destructively_assert_nonpermanent_checker_and_entry(
      checker_list, checker_list_nth(checker_list, 0), start, size / 2);
  destructively_assert_permanent_checker_and_entry(
      checker_list_nth(checker_list, 1), start + size / 2, size / 2);
...
static test_result_t test_permanent_after_hole(checker_list_t *checker_list,
                                               mach_vm_address_t start,
                                               mach_vm_size_t size) {
  assert_hole_checker_and_entry(checker_list_nth(checker_list, 0), start,
                                size / 2);
  destructively_assert_permanent_checker_and_entry(
      checker_list_nth(checker_list, 1), start + size / 2, size / 2);
...
static test_result_t
test_single_submap_single_entry_common(checker_list_t *checker_list,
                                       mach_vm_address_t start,
                                       mach_vm_size_t size) {
  vm_region_submap_info_data_64_t info;
...
static test_result_t
test_single_submap_single_entry(checker_list_t *checker_list,
                                mach_vm_address_t start, mach_vm_size_t size) {
  return test_single_submap_single_entry_common(checker_list, start, size);
}

static test_result_t
test_single_submap_single_entry_first_pages(checker_list_t *checker_list,
                                            mach_vm_address_t start,
                                            mach_vm_size_t size) {
  ...
  return test_single_submap_single_entry_common(checker_list, submap_start,
                                                submap_size);
}

static test_result_t
test_single_submap_single_entry_last_pages(checker_list_t *checker_list,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size) {
...
static test_result_t
test_single_submap_single_entry_middle_pages(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
...
static test_result_t test_single_submap_oversize_entry_common(
    checker_list_t *checker_list, mach_vm_address_t parent_start,
    mach_vm_size_t parent_size, mach_vm_address_t parent_offset,
    mach_vm_size_t submap_size) {
  vm_region_submap_info_data_64_t parent_info, content_info;

  vm_entry_checker_t *submap_parent = checker_list_nth(checker_list, 0);
  assert_submap_checker_and_entry(submap_parent, parent_start, parent_size,
                                  &parent_info);
   ...
   */
  vm_entry_checker_t *submap_content = checker_list_nth(submap_checkers, 0);
  assert_allocation_checker(submap_content, parent_start - parent_offset,
                            submap_size);
  assert(submap_content->submap_depth == 1);
  assert_allocation_entry(parent_start, parent_size, 1 /* submap_depth */,
                          &content_info);
...
static test_result_t
test_single_submap_oversize_entry_at_start(checker_list_t *checker_list,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size) {
  /*
   ...
   */
  return test_single_submap_oversize_entry_common(checker_list, start, size,
                                                  size /* parent_offset */,
                                                  size * 2 /* submap_size */);
}

static test_result_t
test_single_submap_oversize_entry_at_end(checker_list_t *checker_list,
                                         mach_vm_address_t start,
                                         mach_vm_size_t size) {
  /*
   * parent map:   [start, start+size]
...
static test_result_t
test_single_submap_oversize_entry_at_both(checker_list_t *checker_list,
                                          mach_vm_address_t start,
                                          mach_vm_size_t size) {
  /*
   * parent map:            [start, start+size]
   ...
   */
  return test_single_submap_oversize_entry_common(checker_list, start, size,
                                                  size / 2 /* parent_offset */,
                                                  size * 2 /* submap_size */);
}

static test_result_t test_submap_before_allocation_common(
  ...
  vm_entry_checker_t *submap_parent = checker_list_nth(checker_list, 0);
  assert_submap_checker_and_entry(submap_parent, start, size / 2,
                                  &submap_parent_info);

  vm_region_submap_info_data_64_t allocation_info;
  vm_entry_checker_t *allocation = checker_list_nth(checker_list, 1);
  assert_allocation_checker_and_entry(allocation, start + size / 2, size / 2,
                                      &allocation_info);
  ...
  assert_allocation_entry(start, size / 2, 1 /* submap_depth */,
                          &submap_content_info);
  assert_checker_and_entry_protection_equals(
      submap_content, &submap_content_info, submap_protection,
      submap_protection);

  return TestSucceeded;
}

static test_result_t test_submap_before_allocation(checker_list_t *checker_list,
                                                   mach_vm_address_t start,
                                                   mach_vm_size_t size) {
  return test_submap_before_allocation_common(checker_list, start, size,
                                              VM_PROT_READ | VM_PROT_WRITE);
}

static test_result_t
test_submap_before_allocation_ro(checker_list_t *checker_list,
                                 mach_vm_address_t start, mach_vm_size_t size) {
  return test_submap_before_allocation_common(checker_list, start, size,
                                              VM_PROT_READ);
}

static test_result_t test_submap_after_allocation_common(
    checker_list_t *checker_list, mach_vm_address_t start, mach_vm_size_t size,
    int submap_protection) {
  vm_region_submap_info_data_64_t allocation_info;
  vm_entry_checker_t *allocation = checker_list_nth(checker_list, 0);
  assert_allocation_checker_and_entry(allocation, start, size / 2,
                                      &allocation_info);

  vm_region_submap_info_data_64_t submap_parent_info;
  vm_entry_checker_t *submap_parent = checker_list_nth(checker_list, 1);
  assert_submap_checker_and_entry(submap_parent, start + size / 2, size / 2,
                                  &submap_parent_info);
  ...
  assert_allocation_entry(start + size / 2, size / 2, 1 /* submap_depth */,
                          &submap_content_info);
  assert_checker_and_entry_protection_equals(
      submap_content, &submap_content_info, submap_protection,
      submap_protection);
...
static test_result_t test_submap_after_allocation(checker_list_t *checker_list,
                                                  mach_vm_address_t start,
                                                  mach_vm_size_t size) {
  return test_submap_after_allocation_common(checker_list, start, size,
                                             VM_PROT_READ | VM_PROT_WRITE);
}

static test_result_t
test_submap_after_allocation_ro(checker_list_t *checker_list,
                                mach_vm_address_t start, mach_vm_size_t size) {
  return test_submap_after_allocation_common(checker_list, start, size,
                                             VM_PROT_READ);
}
...
static test_result_t
test_submap_before_hole_common(checker_list_t *checker_list,
                               mach_vm_address_t start, mach_vm_size_t size,
                               int submap_protection) {
  vm_region_submap_info_data_64_t submap_parent_info;
  vm_entry_checker_t *submap_parent = checker_list_nth(checker_list, 0);
  ...
  assert_hole_checker_and_entry(hole, start + size / 2, size / 2);
...
static test_result_t test_submap_before_hole(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  return test_submap_before_hole_common(checker_list, start, size,
                                        VM_PROT_READ | VM_PROT_WRITE);
}

static test_result_t test_submap_before_hole_ro(checker_list_t *checker_list,
                                                mach_vm_address_t start,
                                                mach_vm_size_t size) {
  return test_submap_before_hole_common(checker_list, start, size,
                                        VM_PROT_READ);
...
static test_result_t test_submap_after_hole_common(checker_list_t *checker_list,
                                                   mach_vm_address_t start,
                                                   mach_vm_size_t size,
                                                   int submap_protection) {
  vm_entry_checker_t *hole = checker_list_nth(checker_list, 0);
  assert_hole_checker_and_entry(hole, start, size / 2);
...
static test_result_t test_submap_after_hole(checker_list_t *checker_list,
                                            mach_vm_address_t start,
                                            mach_vm_size_t size) {
  return test_submap_after_hole_common(checker_list, start, size,
                                       VM_PROT_READ | VM_PROT_WRITE);
}

static test_result_t test_submap_after_hole_ro(checker_list_t *checker_list,
                                               mach_vm_address_t start,
                                               mach_vm_size_t size) {
  return test_submap_after_hole_common(checker_list, start, size, VM_PROT_READ);
}
 ...
 * all of default size.
 */
static void assert_submap_allocation_submap(checker_list_t *checker_list,
                                            mach_vm_address_t start,
                                            mach_vm_size_t size) {
  vm_region_submap_info_data_64_t info;
  ...
  assert_submap_checker_and_entry(checker, start + offset, DEFAULT_ENTRY_SIZE,
                                  &info);
  ...
  assert_allocation_checker_and_entry(checker, start + offset,
                                      DEFAULT_ENTRY_SIZE, &info);
...
static test_result_t test_submap_allocation_submap_one_entry_common(
    checker_list_t *checker_list, mach_vm_address_t start, mach_vm_size_t size,
    int submap_protection) {
...
static test_result_t
test_submap_allocation_submap_one_entry(checker_list_t *checker_list,
                                        mach_vm_address_t start,
                                        mach_vm_size_t size) {
  return test_submap_allocation_submap_one_entry_common(
      checker_list, start, size, VM_PROT_READ | VM_PROT_WRITE);
}

static test_result_t
test_submap_allocation_submap_one_entry_ro(checker_list_t *checker_list,
                                           mach_vm_address_t start,
                                           mach_vm_size_t size) {
  return test_submap_allocation_submap_one_entry_common(checker_list, start,
                                                        size, VM_PROT_READ);
}

static test_result_t test_submap_allocation_submap_two_entries_common(
    checker_list_t *checker_list, mach_vm_address_t start, mach_vm_size_t size,
    int submap_protection) {
  /* parent map is submap - allocation - submap */
  assert_submap_allocation_submap(checker_list, start, size);
...
static test_result_t
test_submap_allocation_submap_two_entries(checker_list_t *checker_list,
                                          mach_vm_address_t start,
                                          mach_vm_size_t size) {
  return test_submap_allocation_submap_two_entries_common(
      checker_list, start, size, VM_PROT_READ | VM_PROT_WRITE);
}

static test_result_t
test_submap_allocation_submap_two_entries_ro(checker_list_t *checker_list,
                                             mach_vm_address_t start,
                                             mach_vm_size_t size) {
  return test_submap_allocation_submap_two_entries_common(checker_list, start,
                                                          size, VM_PROT_READ);
}

static test_result_t test_submap_allocation_submap_three_entries_common(
    checker_list_t *checker_list, mach_vm_address_t start, mach_vm_size_t size,
    int submap_protection) {
  /* parent map is submap - allocation - submap */
  assert_submap_allocation_submap(checker_list, start, size);
...
static test_result_t
test_submap_allocation_submap_three_entries(checker_list_t *checker_list,
                                            mach_vm_address_t start,
                                            mach_vm_size_t size) {
  return test_submap_allocation_submap_three_entries_common(
      checker_list, start, size, VM_PROT_READ | VM_PROT_WRITE);
}

static test_result_t
test_submap_allocation_submap_three_entries_ro(checker_list_t *checker_list,
                                               mach_vm_address_t start,
                                               mach_vm_size_t size) {
  return test_submap_allocation_submap_three_entries_common(checker_list, start,
                                                            size, VM_PROT_READ);
}

static void assert_protection(mach_vm_address_t address, vm_prot_t prot,
                              bool check_max, uint32_t submap_depth) {
  ...
  if (check_max) {
    T_QUIET;
    T_ASSERT_EQ(prot, info.max_protection, "entry max protection");
  } else {
    T_QUIET;
    T_ASSERT_EQ(prot, info.protection, "entry protection");
  }
}

static test_result_t test_protection_single_common(checker_list_t *checker_list,
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(checker, "checker");
  T_QUIET;
  T_ASSERT_EQ(checker->protection, prot, "checker protection");
  T_QUIET;
  T_ASSERT_EQ(checker->max_protection, max, "checker max protection");
...
static test_result_t test_protection_pair_common(checker_list_t *checker_list,
                                                 mach_vm_address_t address,
                                                 vm_prot_t left_prot,
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(left_checker, "checker");
  T_QUIET;
  T_ASSERT_EQ(left_checker->protection, left_prot, "left entry protection");
  T_QUIET;
  T_ASSERT_EQ(right_checker->protection, right_prot, "right entry protection");

  assert_protection(left_checker->address, left_prot, false /* check max */,
                    0 /* submap depth */);
  assert_protection(right_checker->address, right_prot, false /* check max */,
                    0 /* submap depth */);
...
static test_result_t
test_protection_single_000_000(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start, VM_PROT_NONE,
                                       VM_PROT_NONE);
}

static test_result_t
test_protection_single_000_r00(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start, VM_PROT_NONE,
                                       VM_PROT_READ);
...
static test_result_t
test_protection_single_000_0w0(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start, VM_PROT_NONE,
                                       VM_PROT_WRITE);
...
static test_result_t
test_protection_single_000_rw0(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start, VM_PROT_NONE,
                                       VM_PROT_READ | VM_PROT_WRITE);
...
static test_result_t
test_protection_single_r00_r00(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start, VM_PROT_READ,
                                       VM_PROT_READ);
}

static test_result_t
test_protection_single_r00_rw0(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start, VM_PROT_READ,
                                       VM_PROT_READ | VM_PROT_WRITE);
...
static test_result_t
test_protection_single_0w0_0w0(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start, VM_PROT_WRITE,
                                       VM_PROT_WRITE);
}

static test_result_t
test_protection_single_0w0_rw0(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start, VM_PROT_WRITE,
                                       VM_PROT_READ | VM_PROT_WRITE);
...
static test_result_t
test_protection_single_rw0_rw0(checker_list_t *checker_list,
                               mach_vm_address_t start,
                               mach_vm_size_t size __unused) {
  return test_protection_single_common(checker_list, start,
                                       VM_PROT_READ | VM_PROT_WRITE,
                                       VM_PROT_READ | VM_PROT_WRITE);
}

static test_result_t
test_protection_pair_000_000(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_NONE,
                                     VM_PROT_NONE);
}

static test_result_t
test_protection_pair_000_r00(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_NONE,
                                     VM_PROT_READ);
...
static test_result_t
test_protection_pair_000_0w0(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_NONE,
                                     VM_PROT_WRITE);
...
static test_result_t
test_protection_pair_000_rw0(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_NONE,
                                     VM_PROT_READ | VM_PROT_WRITE);
...
static test_result_t
test_protection_pair_r00_000(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_READ,
                                     VM_PROT_NONE);
}

static test_result_t
test_protection_pair_r00_r00(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_READ,
                                     VM_PROT_READ);
...
static test_result_t
test_protection_pair_r00_0w0(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_READ,
                                     VM_PROT_WRITE);
...
static test_result_t
test_protection_pair_r00_rw0(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_READ,
                                     VM_PROT_READ | VM_PROT_WRITE);
...
static test_result_t
test_protection_pair_0w0_000(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_WRITE,
                                     VM_PROT_NONE);
}

static test_result_t
test_protection_pair_0w0_r00(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_WRITE,
                                     VM_PROT_READ);
...
static test_result_t
test_protection_pair_0w0_0w0(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_WRITE,
                                     VM_PROT_WRITE);
...
static test_result_t
test_protection_pair_0w0_rw0(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start, VM_PROT_WRITE,
                                     VM_PROT_READ | VM_PROT_WRITE);
...
static test_result_t
test_protection_pair_rw0_000(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
...
static test_result_t
test_protection_pair_rw0_r00(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(
...
static test_result_t
test_protection_pair_rw0_0w0(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(
...
static test_result_t
test_protection_pair_rw0_rw0(checker_list_t *checker_list,
                             mach_vm_address_t start,
                             mach_vm_size_t size __unused) {
  return test_protection_pair_common(checker_list, start,
                                     VM_PROT_READ | VM_PROT_WRITE,
                                     VM_PROT_READ | VM_PROT_WRITE);
}
