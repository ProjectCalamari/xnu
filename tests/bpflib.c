...
PRIVATE_EXTERN int bpf_set_timeout(int fd, struct timeval *tv_p) {
  return ioctl(fd, BIOCSRTIMEOUT, tv_p);
}

PRIVATE_EXTERN int bpf_get_blen(int fd, int *blen) {
  return ioctl(fd, BIOCGBLEN, blen);
}

PRIVATE_EXTERN int bpf_set_blen(int fd, int blen) {
  return ioctl(fd, BIOCSBLEN, &blen);
}

PRIVATE_EXTERN int bpf_set_header_complete(int fd, u_int header_complete) {
  return ioctl(fd, BIOCSHDRCMPLT, &header_complete);
}

PRIVATE_EXTERN int bpf_set_see_sent(int fd, u_int see_sent) {
  return ioctl(fd, BIOCSSEESENT, &see_sent);
}

PRIVATE_EXTERN int bpf_dispose(int bpf_fd) {
  if (bpf_fd >= 0) {
    return close(bpf_fd);
  }
  return 0;
...
PRIVATE_EXTERN int bpf_set_traffic_class(int fd, int tc) {
  return ioctl(fd, BIOCSETTC, &tc);
}

#ifdef BIOCSDIRECTION
PRIVATE_EXTERN int bpf_set_direction(int fd, u_int direction) {
  return ioctl(fd, BIOCSDIRECTION, &direction);
}

PRIVATE_EXTERN int bpf_get_direction(int fd, u_int *direction) {
  return ioctl(fd, BIOCGDIRECTION, direction);
}

#endif /* BIOCSDIRECTION */

PRIVATE_EXTERN int bpf_new(void) {
  char bpfdev[256];
  int i;
  int fd = -1;
...
PRIVATE_EXTERN int bpf_setif(int fd, const char *en_name) {
  struct ifreq ifr;
...
PRIVATE_EXTERN int bpf_set_immediate(int fd, u_int value) {
  return ioctl(fd, BIOCIMMEDIATE, &value);
}

PRIVATE_EXTERN int bpf_filter_receive_none(int fd) {
...
PRIVATE_EXTERN int bpf_arp_filter(int fd, int type_offset, int type,
                                  u_int pkt_size) {
  struct bpf_insn insns[] = {
...
PRIVATE_EXTERN int bpf_set_write_size_max(int fd, u_int value) {
#ifdef BIOCSWRITEMAX
...
PRIVATE_EXTERN int bpf_get_write_size_max(int fd, u_int *value) {
#ifdef BIOCGWRITEMAX
...
PRIVATE_EXTERN int bpf_set_batch_write(int fd, u_int value) {
...
PRIVATE_EXTERN int bpf_get_batch_write(int fd, u_int *value) {
...
void bpf_read_continuously(int fd, u_int blen) {
  int n;
