...
static mach_port_t server_checkin(void) {
  mach_port_t mp;
  kern_return_t kr;

  kr = bootstrap_check_in(bootstrap_port, SERVICE_NAME, &mp);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "bootstrap_check_in");
  return mp;
}

static mach_port_t server_lookup(void) {
  mach_port_t mp;
  kern_return_t kr;

  kr = bootstrap_look_up(bootstrap_port, SERVICE_NAME, &mp);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "bootstrap_look_up");
  return mp;
}

static mach_port_t make_sr_port(void) {
...
static void destroy_port(mach_port_t port, bool receive, int srights) {
  kern_return_t kr;
  ...
  if (srights) {
    kr = mach_port_mod_refs(mach_task_self(), port, MACH_PORT_RIGHT_SEND,
                            -srights);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "srights -= %d", srights);
  }
  if (receive) {
    kr =
        mach_port_mod_refs(mach_task_self(), port, MACH_PORT_RIGHT_RECEIVE, -1);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(kr, "receive -= 1");
  }
}

static void send_port(mach_msg_id_t id, mach_port_t dest, mach_port_t right,
                      mach_msg_type_name_t disp) {
  struct one_port_msg msg = {
      .header =
          {
              .msgh_remote_port = dest,
              .msgh_bits = MACH_MSGH_BITS_SET(MACH_MSG_TYPE_COPY_SEND, 0,
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "send(%d)", id);
}

#pragma mark - basic test about right deduplication

static mach_port_t receive_port(mach_msg_id_t expected_id, mach_port_t rcv_port,
                                mach_msg_type_name_t expected_disp) {
  struct one_port_msg msg = {};
  kern_return_t kr;

  T_LOG("waiting for message %d", expected_id);
  kr = mach_msg(&msg.header, MACH_RCV_MSG, 0, sizeof(msg), rcv_port, 0, 0);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(kr, "receive(%d)", expected_id);
  T_QUIET;
  T_ASSERT_EQ(msg.header.msgh_id, expected_id, "message id matches");
  T_QUIET;
  T_ASSERT_NE(msg.header.msgh_bits & MACH_MSGH_BITS_COMPLEX, 0,
              "message is complex");
  T_QUIET;
  T_ASSERT_EQ(msg.body.msgh_descriptor_count, 1, "message has one right");
  T_QUIET;
  T_ASSERT_EQ((mach_msg_type_name_t)msg.port_descriptor.disposition,
              expected_disp, "port has right disposition");
