...
static void *listener_thread(void *unused) {
  ...
  while (1) {
    n = recvfrom(sockfd, buf, 5, 0, (struct sockaddr *)&clientaddr, &clientlen);
    if (n < 0) {
...
T_DECL(
    v4mappedv6_recvpktinfo,
    "Test setting and using IPV6_RECVPKTINFO on a v4-mapped-v6 address socket",
  ...
  int Option = 0;
  LOGSOCKOPT(
      setsockopt(sockfd, IPPROTO_IPV6, IPV6_V6ONLY, &Option, sizeof(Option)));

  Option = 1;
  LOGSOCKOPT(setsockopt(sockfd, IPPROTO_IPV6, IPV6_RECVPKTINFO, &Option,
                        sizeof(Option)));
  ...
  struct in_addr v4addr = {.s_addr = htonl(INADDR_LOOPBACK)};
  ...
  ret = connect(sockfd, (const struct sockaddr *)&remote_addr,
                sizeof(remote_addr));
...
void do_kqueue(int kq, int sockfd) {
  ...
  for (int i = 0; i < nev; i++) {
    T_LOG("[kevent] ident = %zd, filter = %hu, flags = %hd, fflags = %d, data "
          "= %zd, udata = %p\n",
          evList[i].ident, evList[i].filter, evList[i].flags, evList[i].fflags,
          evList[i].data, evList[i].udata);
