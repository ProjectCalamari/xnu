...
static void check_game_mode(bool expected_mode) {
...
T_DECL(entitled_game_mode_read_notroot,
       "game mode bit should be readable as not root but entitled",
       T_META_ASROOT(false)) {
  T_LOG("uid: %d", getuid());
...
static struct coalinfo_debuginfo get_coal_debuginfo(uint64_t coal_id,
  ...
  T_LOG("coal(%s): %lld, game count %d, flags 0x%x (group 0x%llx, rec %d, "
        "focal: %d, nonfocal %d)",
        prefix, coal_id, coaldebuginfo.game_task_count,
...
static void check_game_mode_count(uint32_t expected_count) {
...
#if TARGET_OS_OSX
    T_ASSERT_EQ(coaldebuginfo_res.game_task_count, expected_count,
                "should have game in res coalition");
    T_QUIET;
    T_ASSERT_EQ(coaldebuginfo_jet.game_task_count, 0,
                "should not have game in jet coalition");
#else
    T_QUIET;
    T_ASSERT_EQ(coaldebuginfo_res.game_task_count, 0,
                "should not have game in res coalition");
    T_ASSERT_EQ(coaldebuginfo_jet.game_task_count, expected_count,
                "should have game in jet coalition");
#endif
...
T_DECL(entitled_game_mode_check_count,
       "game mode bit should affect coalition thread group flags",
...
T_DECL(game_mode_double_set_and_child_exit,
       "game mode bit on parent should stay when game mode child exits",
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_REQUIRES_SYSCTL_EQ("kern.thread_groups_supported", 1)) {
  T_LOG("uid: %d", getuid());
  skip_if_unsupported();
...
T_DECL(game_mode_double_set_and_child_unset,
       "game mode bit on parent should stay when game mode child unsets",
       T_META_REQUIRES_SYSCTL_EQ("kern.development", 1),
       T_META_REQUIRES_SYSCTL_EQ("kern.thread_groups_supported", 1)) {
  T_LOG("uid: %d", getuid());
  skip_if_unsupported();
