...
static inline unsigned char *get_guarded_page(void) {
  unsigned char *p =
      mmap(NULL, 3 * PAGE_SIZE, PROT_NONE, MAP_SHARED | MAP_ANON, 0, 0);
  p += PAGE_SIZE;
  mprotect(p, PAGE_SIZE, PROT_READ | PROT_WRITE);
  return p;
}

static inline void free_guarded_page(unsigned char *p) {
 ...
 */
extern int memcmp_zero_ptr_aligned(const void *s, size_t n);
