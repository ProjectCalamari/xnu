...
static void xfsz_signal_handler(__unused int signo) { sigcount++; }

static void fsize_test(bool use_fd) {
  struct rlimit rlim;
  int fd, ret;
  ...
  if (use_fd) {
    ...
    T_EXPECT_POSIX_FAILURE(ret, EFBIG,
                           "ftruncate() with length RLIMIT_FSIZE + 1");
  } else {
    ret = truncate(TMP_FILE_PATH, FSIZE_CUR + 1);
    T_EXPECT_POSIX_FAILURE(ret, EFBIG,
                           "truncate() with length RLIMIT_FSIZE + 1");
  }
...
T_DECL(ftruncate_fsize,
       "ftruncate() should fail with EFBIG and send SIGXFSZ signal when length "
       "> RLIMIT_FSIZE",
...
T_DECL(truncate_fsize,
       "truncate() should fail with EFBIG and send SIGXFSZ signal when length "
       "> RLIMIT_FSIZE",
       T_META_TAG_VM_PREFERRED) {
  fsize_test(false);
}
