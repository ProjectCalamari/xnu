...
static void check_fixed_counts(struct thsc_cpi counts[2]) {
  T_QUIET;
  T_EXPECT_GT(counts[0].tcpi_instructions, UINT64_C(0),
...
T_DECL(core_fixed_task, "check that task counting is working",
  ...
  T_ASSERT_MACH_SUCCESS(kr, "task_inspect(... TASK_INSPECT_BASIC_COUNTS ...)");
...
T_DECL(core_fixed_kdebug, "check that the kdebug macros for monotonic work",
  ...
  ktrace_events_single_paired(
      s, KDBG_EVENTID(DBG_MONOTONIC, DBG_MT_TMPCPU, 0x3fff),
  ...
  T_ASSERT_POSIX_ZERO(
      ktrace_start(s, dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0)),
      NULL);

  r = sysctlbyname("kern.monotonic.kdebug_test", NULL, NULL, &set, sizeof(set));
...
static void *spin_thread_self_counts(__unused void *arg) {
...
static void *spin_task_inspect(__unused void *arg) {
  task_t task = mach_task_self();
  uint64_t counts[2] = {0};
  unsigned int size = 0;
  while (true) {
     ...
     */
    T_QUIET;
    T_ASSERT_LT(counts[0], 1ULL << 63, "check for valid count entry 1");
    T_QUIET;
    T_ASSERT_LT(counts[1], 1ULL << 63, "check for valid count entry 2");
  }
}
...
T_DECL(core_fixed_stack_leak_race,
  ...
  int ncpus = 0;
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(sysctlbyname("hw.logicalcpu_max", &ncpus,
                                      &(size_t){sizeof(ncpus)}, NULL, 0),
                         "get number of CPUs");
  T_QUIET;
  T_ASSERT_GT(ncpus, 0, "got non-zero number of CPUs");
  pthread_t *threads = calloc((unsigned long)ncpus, sizeof(*threads));

  T_QUIET;
  T_ASSERT_NOTNULL(threads, "allocated space for threads");
  ...
  for (int i = 0; i < ncpus; i++) {
    ...
    T_ASSERT_POSIX_ZERO(
        pthread_create(&threads[i], NULL,
                       i & 1 ? spin_task_inspect : spin_thread_self_counts,
                       NULL),
        NULL);
  }
...
static void perf_sysctl_deltas(const char *sysctl_name, const char *stat_name) {
  uint64_t deltas[2];
  size_t deltas_size;
  int r;
  ...
  dt_stat_t instrs = dt_stat_create("instructions", "%s_instrs", stat_name);
...
T_DECL(perf_core_fixed_task,
       "test the performance of fixed task counter access", T_META_ASROOT(true),
