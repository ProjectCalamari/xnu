 ...
 * rdar://84355745
 *
 * The test allows to create a custom ICMP reply, and to send only a portion of
 * it.
 *
 * To reproduce rdar://84355745, the test creates an ICMP "host unreachable"
 * packet that contains a TCP header, and sends the first 28 bytes of the ICMP
 * payload (48 including the outer IP header).
 *
 *  +-----+-----+-----+-----+
 *  | IP  | ICMP| IP  | TCP |
 *  +-----+-----+-----+-----+
...
static uint16_t checksum_buffer(uint16_t *buf, size_t len) {
  unsigned long sum = 0;
  while (len > 1) {
    sum += *buf++;
    len -= 2;
...
static void icmp4_pcb_print(icmp4_pcb_t pcb) {
  if (pcb == NULL) {
  ...
  fprintf(
      ...
      "        dport=%hu [%hu]\n"
      "        cksum=%hu\n",
      pcb->id, pcb->id, pcb->seq, pcb->err, pcb->syserr, pcb->txlen,
      pcb->icmp_hdr_len,
      ...
      (uint16_t)(pcb->inner_ip_hdr == NULL
                     ? -1
                     : IP_VHL_HL(pcb->inner_ip_hdr->ip_vhl) << 2),
      (uint16_t)(pcb->inner_ip_hdr == NULL ? -1 : pcb->inner_ip_hdr->ip_sum),
      pcb->inner_tcp_hdr_len,
      (uint16_t)(pcb->inner_tcp_hdr == NULL ? -1
                                            : pcb->inner_tcp_hdr->th_sport),
      (uint16_t)(pcb->inner_tcp_hdr == NULL
                     ? -1
                     : ntohs(pcb->inner_tcp_hdr->th_sport)),
      (uint16_t)(pcb->inner_tcp_hdr == NULL ? -1
                                            : pcb->inner_tcp_hdr->th_dport),
      (uint16_t)(pcb->inner_tcp_hdr == NULL
                     ? -1
                     : ntohs(pcb->inner_tcp_hdr->th_dport)),
      (uint16_t)(pcb->inner_tcp_hdr == NULL ? -1 : pcb->inner_tcp_hdr->th_sum));
}

static void icmp4_pcb_init(icmp4_pcb_t pcb) {
  memset(pcb, 0, sizeof(struct icmp4_pcb));
}

static void icmp4_pcb_close(icmp4_pcb_t pcb) {
  if (pcb->fd != -1) {
    close(pcb->fd);
    pcb->fd = -1;
  }
...
static int icmp4_pcb_open(icmp4_pcb_t pcb, struct in_addr *local,
...
static size_t icmp4_pcb_get_payload_len(icmp4_pcb_t pcb) {
  return pcb->icmp_hdr_len + pcb->inner_ip_hdr_len + pcb->inner_ip_hdr_len;
}

static size_t icmp4_pcb_set_payload(icmp4_pcb_t pcb, struct icmp *icmp_in,
                                    struct ip *ip_in, struct tcphdr *tcp_in) {
  ...
  pcb->inner_tcp_hdr =
      (struct tcphdr *)(ptr + pcb->icmp_hdr_len + pcb->inner_ip_hdr_len);
  ...
  pcb->inner_tcp_hdr->th_sum =
      checksum_buffer((uint16_t *)pcb->inner_tcp_hdr, pcb->inner_tcp_hdr_len);
  ...
  memcpy(&(pcb->inner_ip_hdr->ip_src), &(pcb->in4addr_local),
         sizeof(struct in_addr));
  memcpy(&(pcb->inner_ip_hdr->ip_dst), &(pcb->in4addr_remote),
         sizeof(struct in_addr));
  pcb->inner_ip_hdr->ip_sum =
      checksum_buffer((uint16_t *)pcb->inner_ip_hdr, pcb->inner_ip_hdr_len);
  ...
  pcb->icmp_hdr->icmp_cksum =
      checksum_buffer((uint16_t *)pcb->icmp_hdr, sizeof(struct icmp));

  return icmp4_pcb_get_payload_len(pcb);
}

static int icmp4_pcb_send_unreach(icmp4_pcb_t pcb, size_t maxlen) {
...
static void icmp4_pcb_assert_payload_correct(icmp4_pcb_t pcb, size_t maxlen) {
  if (pcb == NULL) {
    return;
  }
  ...
  T_ASSERT_LE(ICMP_MINLEN, icmplen,
              "ICMP payload smaller than minimal ICMP len");

  T_ASSERT_GE(icmplen, ICMP_ADVLENMIN,
              "ICMP payload smaller than minimal advertised ICMP len");
  ...
  T_ASSERT_GE(icmplen, icmp_advlen,
              "ICMP payload smaller than advertised ICMP len");

  // validate inner IP header length (ip_icmp.c:568)
  T_ASSERT_GE(inner_ip_hdr_len, sizeof(struct ip),
              "IP payload smaller than IP header length");

  // validate that the TCP header is outside of maxlen
  size_t tcp_hdr_offset =
      (size_t)((uint8_t *)(pcb->inner_tcp_hdr) - (uint8_t *)(pcb->icmp_hdr));
...
T_DECL(icmp_send_malformed_packet_1, "ICMP packet with malformed TCP header") {
  ...
  T_ASSERT_GT(
      icmp4_pcb_set_payload(&pcb, &icmp_payload, &ip_payload, &tcp_payload), 0L,
      NULL);
