...
static void thread_create_at_qos(qos_class_t qos, void *(*function)(void *));
static uint64_t nanoseconds_to_absolutetime(uint64_t nanoseconds);
static int sched_create_load_at_qos(qos_class_t qos, void **load_token);
static int sched_terminate_load(void *load_token) __unused;
static void do_work(int num);
static void dispatch_sync_cancel(mach_port_t owner_thread,
...
static void sched_mti_init(void) { mach_timebase_info(&sched_mti); }
uint64_t nanoseconds_to_absolutetime(uint64_t nanoseconds) {
  pthread_once(&sched_mti_once_control, sched_mti_init);

  return (uint64_t)(nanoseconds *
                    (((double)sched_mti.denom) / ((double)sched_mti.numer)));
}
...
static int sched_create_load_at_qos(qos_class_t qos, void **load_token) {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "pthread_attr_init");

  if (&pthread_attr_set_qos_class_np) {
    ret = pthread_attr_set_qos_class_np(&attr, qos, 0);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(ret, "pthread_attr_set_qos_class_np");
  }

  context = calloc(1, sizeof(*context));
  if (context == NULL) {
    T_QUIET;
    T_LOG("calloc returned error");
    return ENOMEM;
  }
  ...
  for (i = 0; i < nthreads; i++) {
    ret =
        pthread_create(&context->threads[i], &attr, sched_load_thread, context);
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(ret, "pthread_create");
    T_QUIET;
    T_LOG("%s: Created thread %d (%p)\n", __FUNCTION__, i,
          (void *)context->threads[i]);
  }

  ret = pthread_attr_destroy(&attr);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "pthread_attr_destroy");
...
static void *sched_load_thread(void *arg) {
...
static int sched_terminate_load(void *load_token) {
  int ret;
  int i;
  ...
  for (i = 0; i < context->thread_count; i++) {
    ...
    T_QUIET;
    T_ASSERT_MACH_SUCCESS(ret, "pthread_join");
  }
...
static uint32_t get_thread_base_priority(void) {
  kern_return_t kr;
  mach_port_t thread_port = pthread_mach_thread_np(pthread_self());
...
static uint32_t register_port_options(void) {
...
static void register_port(uint64_t wlid, mach_port_t port) {
  int r;
  ...
  r = kevent_id(wlid, &kev, 1, &kev_err, 1, NULL, NULL,
 ...
 */
static void workloop_cb_test_intransit(uint64_t *workloop_id, void **eventslist,
                                       int *events) {
  ...
  T_LOG("Workloop handler %s called. Received message on 0x%llx", __func__,
        *workloop_id);
  ...
  T_QUIET;
  T_ASSERT_EQ(*events, 1, "should have one event");

  T_EXPECT_REQUESTED_QOS_EQ(QOS_CLASS_MAINTENANCE,
  ...
  case LISTENER_WLID:
    ...
    T_QUIET;
    T_ASSERT_FALSE(got_peer, "Should not have seen peer yet");
    got_peer = true;
    break;
  ...
  if (*workloop_id == LISTENER_WLID) {
    ...
    *events = 0;
    ...
    /*
     * Destroying the message will send a send-once notification for reply port,
     * once the send-once notification is consumed (by the waiting thread), only
     * then the actual send-once right is destroyed and only then the push will
     * go away.
     */
    ...
    T_EXPECT_EFFECTIVE_QOS_EQ(QOS_CLASS_MAINTENANCE,
                              "dispatch_source event handler QoS should be "
                              "QOS_CLASS_MAINTENANCE after destroying message");
    T_EXPECT_LE(get_user_promotion_basepri(), 0u,
                "dispatch_source event handler should not be overridden after "
                "destroying message");
    T_EXPECT_LE(get_thread_base_priority(), 4u,
                "dispatch_source event handler should have base pri at 4 or "
                "less after destroying message");
  }
}

static void run_client_server(const char *server_name,
...
static void send(mach_port_t send_port, mach_port_t reply_port,
      ...
      .header =
          {
              ...
              .msgh_bits = MACH_MSGH_BITS_SET(
                  ...
                  MACH_MSG_TYPE_MOVE_SEND, MACH_MSGH_BITS_COMPLEX),
  ...
  ret = mach_msg(&(send_msg.header),
                 MACH_SEND_MSG | MACH_SEND_TIMEOUT | MACH_SEND_OVERRIDE |
                     (test_noimportance ? MACH_SEND_NOIMPORTANCE : 0) | options,
...
static mach_msg_id_t receive(mach_port_t rcv_port, mach_port_t notify_port) {
  struct {
    mach_msg_header_t header;
    mach_msg_body_t body;
    mach_msg_port_descriptor_t port_descriptor;
  ...
  kern_return_t kr;
  kr = mach_msg(&(rcv_msg.header), MACH_RCV_MSG | MACH_RCV_SYNC_WAIT, 0,
...
static void thread_wait_to_boost(mach_port_t thread_port,
                                 mach_port_t yield_thread, int priority) {
  thread_extended_info_data_t extended_info;
  kern_return_t kr;
...
static void dispatch_sync_wait(mach_port_t owner_thread,
                               qos_class_t promote_qos) {
  struct kevent_qos_s kev_err[] = {{0}};
  uint32_t fflags = 0;
  uint64_t mask = 0;
  ...
  r = kevent_id(30, kev, 1, kev_err, 1, NULL, NULL,
                KEVENT_FLAG_WORKLOOP | KEVENT_FLAG_ERROR_EVENTS);
  T_QUIET;
...
static void *thread_at_sixty(void *arg __unused) {
  ...
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "pthread_getschedparam");

  param.sched_priority = 60;

  ret = pthread_setschedparam(pthread_self(), policy, &param);
  T_QUIET;
  T_ASSERT_MACH_SUCCESS(ret, "pthread_setschedparam");
...
static void *thread_at_ui(void *arg __unused) {
...
static void *thread_at_in(void *arg __unused) {
...
static void *thread_at_default(void *arg __unused) {
  ...
  T_LOG("Thread at Default priority trying to wait on dispatch sync for "
        "maintenance thread");
...
static void *thread_at_maintenance(void *arg __unused) {
  ...
  kern_return_t kr = bootstrap_look_up(
      bootstrap_port, TURNSTILE_MULTIHOP_SERVICE_NAME, &service_port);
  T_QUIET;
  ...
  /* Become the dispatch sync owner, dispatch_sync_owner will be set in
   * dispatch_sync_wait function */
  ...
  T_ASSERT_EQ(message_id, MACH_NOTIFY_SEND_ONCE,
...
T_HELPER_DECL(three_ulock_sync_ipc_hop_noimportance,
              "Create chain of 4 threads with 3 ulocks and 1 no-importance "
              "sync IPC at different qos") {
...
T_HELPER_DECL(
  ...
  T_ASSERT_POSIX_ZERO(
      _pthread_workqueue_init_with_workloop(
          worker_cb, event_cb,
          (pthread_workqueue_function_workloop_t)workloop_cb_test_intransit, 0,
          0),
      NULL);
...
#define TEST_MULTIHOP(server_name, client_name, name)                          \
  T_DECL(server_kevent_id_##name, "Event delivery using a kevent_id",          \
...
/*
 * Test 1: Test multihop priority boosting with ulocks, dispatch sync and sync
 * IPC.
 *
 * Create thread's at different Qos and acquire a ulock and block on next
 * ulock/dispatch sync creating a sync chain. The last hop the chain is blocked
 * on Sync IPC.
 */
TEST_MULTIHOP("server_kevent_id", "three_ulock_sync_ipc_hop",
              three_ulock_sync_ipc_hop)

TEST_MULTIHOP("server_kevent_id", "three_ulock_sync_ipc_hop_noimportance",
              three_ulock_sync_ipc_hop_noimportance)

/*
 * Test 2: Test multihop priority boosting with ulocks, dispatch sync and sync
 * IPC.
 *
 * Create thread's at different Qos and acquire a ulock and block on next
 * ulock/dispatch sync creating a sync chain. The last hop the chain is blocked
 * on Sync IPC. Before the last priority 60 thread blocks on ulock, it also
 * starts spinforeverd at priority 31.
 */
TEST_MULTIHOP_SPIN("server_kevent_id", "three_ulock_sync_ipc_hop",
                   three_ulock_sync_ipc_hop_spin)
