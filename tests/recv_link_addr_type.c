...
static void cleanup(void) { network_interface_pair_list_destroy(S_feth_pairs); }
...
T_DECL(ip_recv_link_addr_type, "IP_RECV_LINK_ADDR_TYPE") {
  ...
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(receive_fd, SOL_SOCKET, SO_BINDTODEVICE, ifname1, solen),
      NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(getsockopt(receive_fd, IPPROTO_IP,
                                    IP_RECV_LINK_ADDR_TYPE, &optval, &solen),
                         NULL);
  T_LOG("IP_RECV_LINK_ADDR_TYPE default: %d", optval);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(setsockopt(receive_fd, IPPROTO_IP,
                                    IP_RECV_LINK_ADDR_TYPE, &optval, solen),
                         NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(setsockopt(receive_fd, IPPROTO_IP, IP_ADD_MEMBERSHIP,
                                    &mreq, sizeof(mreq)),
                         NULL);

  struct timeval timeo = {.tv_sec = 1, .tv_usec = 0};
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(receive_fd, SOL_SOCKET, SO_RCVTIMEO, &timeo, sizeof(timeo)),
      NULL);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(setsockopt(receive_fd, IPPROTO_IP, IP_RECVDSTADDR,
  ...
  T_ASSERT_POSIX_SUCCESS(
      bind(receive_fd, (struct sockaddr *)&sin, sizeof(struct sockaddr_in)),
      NULL);

  solen = sizeof(struct sockaddr_in);
  T_ASSERT_POSIX_SUCCESS(
      getsockname(receive_fd, (struct sockaddr *)&sin, &solen), NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sender_fd, SOL_SOCKET, SO_BINDTODEVICE, ifname2, solen), NULL);
  ...
  optval = 1;
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sender_fd, SOL_SOCKET, SO_BROADCAST, &optval, sizeof(optval)),
      NULL);

  /*
   * Send unicast, broadcast and multicast a few times to allow for ARP to do
   * its job
   */
  for (int i = 0; i < 3; i++) {
    str = "unicast";
    ...
    sin.sin_port = port;
    T_ASSERT_POSIX_SUCCESS(retval =
                               sendto(sender_fd, str, strlen(str) + 1, 0,
  ...
  while (true) {
    ...
    for (cmsg = CMSG_FIRSTHDR(&recvmsghdr); cmsg != NULL;
      ...
      if (cmsg->cmsg_level == IPPROTO_IP &&
          cmsg->cmsg_type == IP_RECV_LINK_ADDR_TYPE) {
        addr_type = *(int *)CMSG_DATA(cmsg);
      }
    }
    T_LOG("received packet to: %s address type: %d", inet_ntoa(addr),
          addr_type);
...
T_DECL(ipv6_recv_link_addr_type, "IPV6_RECV_LINK_ADDR_TYPE") {
  ...
  T_LOG("feth one: %s index: %u ip: %s ip6: %s",
        S_feth_pairs->list->one.if_name, S_feth_pairs->list->one.if_index,
        inet_ntoa(S_feth_pairs->list->one.ip), addrstr);

  inet_ntop(AF_INET6, &S_feth_pairs->list->two.ip6, addrstr, sizeof(addrstr));
  T_LOG("feth one: %s index: %u ip: %s ip6: %s",
        S_feth_pairs->list->two.if_name, S_feth_pairs->list->two.if_index,
        inet_ntoa(S_feth_pairs->list->two.ip), addrstr);
  ...
  optval = 1;
  T_ASSERT_POSIX_SUCCESS(setsockopt(receive_fd, IPPROTO_IPV6, IPV6_V6ONLY,
                                    &optval, sizeof(optval)),
                         NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(getsockopt(receive_fd, IPPROTO_IPV6,
                                    IPV6_RECV_LINK_ADDR_TYPE, &optval, &solen),
                         NULL);
  T_LOG("IPV6_RECV_LINK_ADDR_TYPE default: %d", optval);

  optval = 1;
  T_ASSERT_POSIX_SUCCESS(setsockopt(receive_fd, IPPROTO_IPV6,
                                    IPV6_RECV_LINK_ADDR_TYPE, &optval, solen),
                         NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(setsockopt(receive_fd, IPPROTO_IPV6, IPV6_JOIN_GROUP,
                                    &mreq, sizeof(mreq)),
                         NULL);
  ...
  optval = 1;
  T_ASSERT_POSIX_SUCCESS(setsockopt(receive_fd, IPPROTO_IPV6, IPV6_RECVPKTINFO,
                                    &optval, sizeof(optval)),
                         NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(
      bind(receive_fd, (struct sockaddr *)&sin6, sizeof(struct sockaddr_in6)),
      NULL);

  solen = sizeof(struct sockaddr_in6);
  T_ASSERT_POSIX_SUCCESS(
      getsockname(receive_fd, (struct sockaddr *)&sin6, &solen), NULL);
  ...
  T_ASSERT_POSIX_SUCCESS(setsockopt(sender_fd, IPPROTO_IPV6, IPV6_MULTICAST_IF,
                                    &optval, sizeof(optval)),
                         NULL);

  optval = IPV6_DEFHLIM;
  T_ASSERT_POSIX_SUCCESS(setsockopt(sender_fd, IPPROTO_IPV6,
  ...
  /*
   * Send unicast, broadcast and multicast a few times to allow for ND to do its
   * job
   */
  for (int i = 0; i < 3; i++) {
    str = "unicast";
    ...
    T_ASSERT_POSIX_SUCCESS(retval =
                               sendto(sender_fd, str, strlen(str) + 1, 0,
                                      (struct sockaddr *)&sin6, sin6.sin6_len),
                           NULL);
  ...
  while (true) {
    ...
    for (cmsg = CMSG_FIRSTHDR(&recvmsghdr); cmsg != NULL;
      ...
      if (cmsg->cmsg_level == IPPROTO_IPV6 &&
          cmsg->cmsg_type == IPV6_RECV_LINK_ADDR_TYPE) {
        addr_type = *(int *)CMSG_DATA(cmsg);
      }
    }
