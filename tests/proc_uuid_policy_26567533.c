...
T_DECL(proc_uuid_policy_26567533, "Tests passing a NULL uuid in (uap->uuid).",
       T_META_LTEPHASE(LTE_POSTINIT), T_META_TAG_VM_PREFERRED) {
  int i, ret;
  uuid_t null_uuid;
  memset(null_uuid, 0, sizeof(uuid_t));

  uint32_t policy_flags[] = {PROC_UUID_POLICY_FLAGS_NONE, PROC_UUID_NO_CELLULAR,
  ...
  for (i = 0; i < NUM_PROC_UUID_POLICY_FLAGS; i++) {
    ...
    /* Since UUID is null, this call should fail with errno = EINVAL. */
    ret = proc_uuid_policy(PROC_UUID_POLICY_OPERATION_REMOVE, null_uuid,
                           sizeof(uuid_t), policy_flags[i]);
