...
static void do_recv(int sk, char *buf, size_t size) {
  do {
    struct iovec iov[1];
    struct msghdr msg;
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(poll(pfd, 1, -1), "poll");
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(res, "recvmsg");
    buf += res;
    size -= (size_t)res;
  } while (size);
}
...
static void do_send(int sk, char *buf, size_t size) {
  do {
    struct iovec iov[1];
    struct msghdr msg;
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(res, "poll");
    if (res == 0) {
      continue;
    }
    ...
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(res_sendmsg, "sendmsg");
    buf += res_sendmsg;
    size -= (size_t)res_sendmsg;
  } while (size);
}

static void cfg_sk(int sk) {
  int newSndBufSz = MSG1_LEN * 2;
  socklen_t optLen = sizeof(newSndBufSz);
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sk, SOL_SOCKET, SO_SNDBUF, &newSndBufSz, optLen),
      "setsockopt");
  newSndBufSz = MSG1_LEN * 2;
  optLen = sizeof(newSndBufSz);
  T_ASSERT_POSIX_SUCCESS(
      setsockopt(sk, SOL_SOCKET, SO_RCVBUF, &newSndBufSz, optLen),
      "setsockopt");
  int flags = fcntl(sk, F_GETFL, 0);
  T_ASSERT_POSIX_SUCCESS(flags, "fcntl");
  T_ASSERT_POSIX_SUCCESS(fcntl(sk, F_SETFL, flags | O_NONBLOCK), "fcntl");
}

T_DECL(uipc_sosendcheck, "Tests the UNIX Domain poll filter",
       T_META_CHECK_LEAKS(false), T_META_TAG_VM_PREFERRED) {
  int s[2];
  T_ASSERT_POSIX_SUCCESS(socketpair(AF_UNIX, SOCK_STREAM, 0, s), "socketpair");
  cfg_sk(s[0]);
  cfg_sk(s[1]);
  char *buf = malloc(MSG1_LEN);
