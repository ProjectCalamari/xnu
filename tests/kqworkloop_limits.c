...
T_GLOBAL_META(T_META_NAMESPACE("xnu.kevent"), T_META_RUN_CONCURRENTLY(TRUE),
...
static int spawn_child_process_with_limits(int soft_limit, int hard_limit,
                                           int test_num) {
...
T_DECL(test_kqworkloop_soft_and_hard_limit,
       "Allocate kqworkloops with soft and hard limit",
       T_META_IGNORECRASHES(".*kqworkloop_limits_client.*"),
       T_META_CHECK_LEAKS(false), T_META_TAG_VM_PREFERRED,
...
void setup_server_args(struct args *args) {
...
T_DECL(test_kqworkloop_hard_limit_with_resource_notify_port,
  ...
  kern_return_t kr = mach_msg_server_once(resource_notify_server, 4096,
                                          resource_notify_port, 0);
...
kern_return_t receive_kqworkloops_violation(
    __unused mach_port_t receiver, __unused proc_name_t procname,
    __unused pid_t pid, __unused mach_timespec_t timestamp,
    __unused int64_t observed_kqworkloops, __unused int64_t kqworkloops_allowed,
    __unused mach_port_t fatal_port, __unused resource_notify_flags_t flags) {
  T_LOG("Received a notification on the resource notify port");
  T_LOG("kqworkloops_allowed = %lld, kqworkloops observed = %lld",
        kqworkloops_allowed, observed_kqworkloops);
  if (fatal_port) {
