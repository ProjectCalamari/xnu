...
static int ifnet_destroy(int s, const char *ifname, bool fail_on_error) {
  int err;
  struct ifreq ifr;
...
static int ifnet_set_flags(int s, const char *ifname, uint16_t flags_set,
  ...
  if (flags_before == flags_after) {
    ...
    /* issue the ioctl */
    T_QUIET;
    T_ASSERT_POSIX_SUCCESS(ioctl(s, SIOCSIFFLAGS, &ifr), "SIOCSIFFLAGS %s 0x%x",
                           ifr.ifr_name, (uint16_t)ifr.ifr_flags);
  }
  return ret;
}

static int ifnet_create(int s, char *ifname, size_t ifname_size) {
  int error = 0;
  struct ifreq ifr;
  ...
  for (int i = 0; i < SIFCREATE_RETRY; i++) {
    if (ioctl(s, SIOCIFCREATE, &ifr) < 0) {
      error = errno;
      T_LOG("SIOCSIFCREATE %s: %s", ifname, strerror(error));
      if (error == EBUSY) {
...
static size_t snprint_dottedhex(char *str, size_t strsize, const void *data,
...
static void print_sockaddr_dl(const char *pre, const struct sockaddr *sa,
                              const char *post) {
  char nbuffer[256];
  char abuffer[256];
  char sbuffer[256];
  ...
  snprint_dottedhex(abuffer, sizeof(abuffer), sdl.sdl_data + sdl.sdl_nlen,
                    sdl.sdl_alen);
  snprint_dottedhex(sbuffer, sizeof(sbuffer),
                    sdl.sdl_data + sdl.sdl_nlen + sdl.sdl_alen, sdl.sdl_slen);

  T_LOG("%ssdl_len %u sdl_family %u sdl_index %u sdl_type %u sdl_nlen %u (%s) "
        "sdl_alen %u (%s) sdl_slen %u (%s)%s",
        pre != NULL ? pre : "", sdl.sdl_len, sdl.sdl_family, sdl.sdl_index,
        sdl.sdl_type, sdl.sdl_nlen, nbuffer, sdl.sdl_alen, abuffer,
        sdl.sdl_slen, sbuffer, post != NULL ? post : "");
}

static void print_sockaddr_in(const char *pre, const struct sockaddr *sa,
                              const char *post) {
  char abuffer[256];
  char zbuffer[256];
  ...
  snprint_dottedhex(zbuffer, sizeof(zbuffer), sin.sin_zero,
                    sizeof(sin.sin_zero));

  T_LOG("%ssin_len %u sin_family %u sin_port %u sin_addr %s sin_zero %s%s",
        pre != NULL ? pre : "", sin.sin_len, sin.sin_family,
        htons(sin.sin_port), abuffer, zbuffer, post != NULL ? post : "");
}

static void print_sockaddr_in6(const char *pre, const struct sockaddr *sa,
                               const char *post) {
  char abuffer[256];
  ...
  T_LOG("%ssin6_len %u sin6_family %u sin6_port %u sin6_flowinfo %u sin6_addr "
        "%s sin6_scope_id %u%s",
        pre != NULL ? pre : "", sin6.sin6_len, sin6.sin6_family,
        htons(sin6.sin6_port), sin6.sin6_flowinfo, abuffer, sin6.sin6_scope_id,
        post != NULL ? post : "");
}

static void print_sockaddr(const char *pre, const struct sockaddr *sa,
                           const char *post) {
  char buffer[256];
  ...
  T_LOG("%ssa_len %u sa_family %u sa_data %s%s", pre != NULL ? pre : "",
        sa->sa_len, sa->sa_family, buffer, post != NULL ? post : "");
}

#define ROUNDUP(a, size) (((a) & ((size) - 1)) ? (1 + ((a) | (size - 1))) : (a))

#define NEXT_SA(p)                                                             \
  (struct sockaddr *)((caddr_t)p +                                             \
                      (p->sa_len ? ROUNDUP(p->sa_len, sizeof(u_int32_t))       \
                                 : sizeof(u_long)))

static size_t get_rti_info(int addrs, struct sockaddr *sa,
...
static void print_address(const char *pre, const struct sockaddr *sa,
                          const char *post, u_char asFamily) {
  if (sa == NULL) {
    T_LOG("%s(NULL)%s", pre != NULL ? pre : "", post != NULL ? post : "");
    return;
  }
  if (sa->sa_len == 0) {
    T_LOG("%ssa_len 0%s", pre != NULL ? pre : "", post != NULL ? post : "");
    return;
  }
  if (sa->sa_len == 1) {
    T_LOG("%ssa_len 1%s", pre != NULL ? pre : "", post != NULL ? post : "");
    return;
  }
...
static void print_rti_info(struct sockaddr *rti_info[]) {
  struct sockaddr *sa;
  u_char asFamily = 0;
...
static void print_rt_iflist2(const char *label) {
  size_t len;
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(sysctl(mib, 6, NULL, &len, NULL, 0),
                         "sysctl NET_RT_IFLIST2");

  T_QUIET;
  T_ASSERT_NOTNULL(buf = calloc(1, len), "rt_if_list_buf calloc(1, %zd)", len);

  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(sysctl(mib, 6, buf, &len, NULL, 0),
                         "sysctl NET_RT_IFLIST2");
  ...
  for (next = buf; next < lim; next += ifmsg->ifm_msglen) {
    ...
    if (ifmsg->ifm_type == RTM_IFINFO2) {
      ...
      (void)get_rti_info(ifam->ifam_addrs, (struct sockaddr *)(ifam + 1),
                         rti_info);
...
static int check_rt_if_list_for_pattern(const char *label,
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(pattern_buf = calloc(1, pattern_size),
                   "pattern_buf calloc(1, %zd)", pattern_size);
  ...
  T_QUIET;
  T_ASSERT_NOTNULL(rt_if_list_buf = calloc(1, len),
                   "rt_if_list_buf calloc(1, %zd)", len);

  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(sysctl(mib, 6, rt_if_list_buf, &len, NULL, 0),
                         "sysctl NET_RT_IFLIST");
...
static bool find_unused_pattern_in_rt_if_list(unsigned char *pattern,
                                              size_t pattern_size) {
...
static const char *ioc_str(unsigned long cmd) {
  switch (cmd) {
#define X(a)                                                                   \
  case a:                                                                      \
    return #a;
...
static struct ioc_ifreq ioc_list[] = {
    ...
    {SIOCDIFADDR, sizeof(struct sockaddr_in), AF_INET6, "10.2.3.1",
     EADDRNOTAVAIL},
    {SIOCDIFADDR, sizeof(struct sockaddr_in6), AF_INET, "10.2.3.1",
     EADDRNOTAVAIL},
    {SIOCDIFADDR, sizeof(struct sockaddr_in), AF_INET, "10.2.3.1",
     EADDRNOTAVAIL},
    ...
    {SIOCDIFADDR, sizeof(struct sockaddr_in6), AF_INET, "10.2.3.4",
     EADDRNOTAVAIL},

    {SIOCSIFADDR, sizeof(struct sockaddr_in), AF_INET, "10.2.3.5", 0},
    {SIOCDIFADDR, sizeof(struct sockaddr_in), AF_INET, "0.0.0.0",
     EADDRNOTAVAIL},
...
static void test_sioc_ifr_bounds(struct ioc_ifreq *ioc_ifreq, int s,
                                 const char *ifname) {
  struct ifreq ifr = {0};
  unsigned char pattern;
  ...
  T_LOG("TEST CASE: %s ioctl(%s, sa_len %u, sa_family %u, %s) -> %d", __func__,
        ioc_str(ioc_ifreq->ioc_cmd), ioc_ifreq->salen, ioc_ifreq->safamily,
        ioc_ifreq->sastr, ioc_ifreq->error);
  ...
  if (ioc_ifreq->error == 0) {
    T_EXPECT_POSIX_SUCCESS(retval = ioctl(s, ioc_ifreq->ioc_cmd, &ifr),
                           "%s, %s: retval %d", ioc_str(ioc_ifreq->ioc_cmd),
                           ioc_ifreq->sastr, retval);
  } else {
    T_EXPECT_POSIX_FAILURE(retval = ioctl(s, ioc_ifreq->ioc_cmd, &ifr),
                           ioc_ifreq->error, "%s, %s: retval %d errno %s",
                           ioc_str(ioc_ifreq->ioc_cmd), ioc_ifreq->sastr,
                           retval, strerror(errno));
  }

  T_EXPECT_EQ(check_rt_if_list_for_pattern("test_sioc_ifr_bounds", pattern,
                                           PATTERN_SIZE),
              0, "pattern should not be found");

  fflush(stdout);
  fflush(stderr);
}

T_DECL(sioc_ifr_bounds,
       "test bound checks on struct ifreq addresses passed to interface ioctls",
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(s = socket(AF_INET, SOCK_DGRAM, 0), "socket");
...
static struct ioc_ifra ioc_ifra_list[] = {
    {.description = "fully formed",
     ...
     .addr_fam = AF_INET,
     .addr_str = "10.1.1.1",
     .broad_len = sizeof(struct sockaddr_in),
     .broad_fam = AF_INET,
     .broad_str = "1.1.1.255",
     .mask_len = sizeof(struct sockaddr_in),
     .mask_fam = AF_INET,
    ...
    {.description = "ifra mask len 13",
     ...
     .mask_str = "255.255.255.0",
     .error = 0},
    {.description = NULL, .error = -1}};

T_DECL(sioc_ifra_addr_bounds,
       "test bound checks on socket address passed to interface ioctls",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  int s = -1;
  ...
  for (ioc_ifra = ioc_ifra_list; ioc_ifra->error != -1; ioc_ifra++) {
    ...
    T_LOG("");
    T_LOG("TEST CASE: %s, ioctl(SIOCAIFADDR, %s)", ioc_ifra->description,
          ioc_ifra->addr_str != NULL ? ioc_ifra->addr_str : "");
    ...
    if (ioc_ifra->error == 0) {
      T_EXPECT_POSIX_SUCCESS(retval = ioctl(s, SIOCAIFADDR, &ifra),
                             "SIOCAIFADDR retval %d", retval);
    } else {
      T_EXPECT_POSIX_FAILURE(retval = ioctl(s, SIOCAIFADDR, &ifra), EINVAL,
                             "SIOCAIFADDR retval %d, %s", retval,
                             strerror(errno));
    }

    T_EXPECT_EQ(check_rt_if_list_for_pattern("after ioctl SIOCAIFADDR", pattern,
                                             PATTERN_SIZE),
                0, "pattern should not be found");
  }
...
T_DECL(sioc_ifr_dstaddr_leak,
  ...
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(getifaddrs(&ifap), "getifaddrs");
  ...
  T_EXPECT_POSIX_SUCCESS(ioctl(s, SIOCSIFDSTADDR, &ifr),
  ...
  T_EXPECT_EQ(check_rt_if_list_for_pattern("AFTER", pattern, PATTERN_SIZE), 0,
...
static struct ioc_ifreq ioc_list_config[] = {
    ...
    {SIOCSIFADDR, sizeof(struct sockaddr_in), AF_INET, "255.255.255.255",
     EINVAL},
    ...
    {SIOCAIFADDR_IN6, sizeof(struct sockaddr_in6), AF_INET6,
     "ff33:40:fd25:549c:675b::1", EINVAL},
    {SIOCAIFADDR_IN6, sizeof(struct sockaddr_in6), AF_INET6, "::ffff:0a0a:0a0a",
     EINVAL},
    {SIOCAIFADDR_IN6, sizeof(struct sockaddr_in6), AF_INET6, "::0a0a:0a0a",
     EINVAL},

    {0, 0, 0, "", -1},
};

static void test_sioc_ifr_addr_config_v4(struct ioc_ifreq *ioc_ifreq, int s,
                                         const char *ifname) {
  struct ifreq ifr = {0};
  struct sockaddr_in *sin;
...
static void test_sioc_ifr_addr_config_v6(struct ioc_ifreq *ioc_ifreq, int s,
                                         const char *ifname) {
  struct in6_aliasreq ifra = {
      {0}, {0}, {0},
      {0}, 0,   {0, 0, ND6_INFINITE_LIFETIME, ND6_INFINITE_LIFETIME}};
  struct sockaddr_in6 *sin6 = (struct sockaddr_in6 *)(void *)&ifra.ifra_addr;
  struct sockaddr_in6 *mask6 =
      (struct sockaddr_in6 *)(void *)&ifra.ifra_prefixmask;
  T_LOG("");
  T_LOG("TEST CASE: %s ioctl(%s, sa_len %u, sa_family %u, %s) -> %d", __func__,
        ioc_str(ioc_ifreq->ioc_cmd), ioc_ifreq->salen, ioc_ifreq->safamily,
        ioc_ifreq->sastr, ioc_ifreq->error);
  ...
  T_ASSERT_EQ(inet_pton(AF_INET6, "ffff:ffff:ffff:ffff::", &mask6->sin6_addr),
              1, NULL);
  ...
  if (ioc_ifreq->error == 0) {
    ...
    T_EXPECT_POSIX_FAILURE(retval = ioctl(s, ioc_ifreq->ioc_cmd, &ifra),
                           ioc_ifreq->error, "%s, %s: retval %d errno %s",
                           ioc_str(ioc_ifreq->ioc_cmd), ioc_ifreq->sastr,
                           retval, strerror(errno));
  }
...
T_DECL(sioc_ifr_addr_config,
       "test failure cases for interface address configuration ioctls",
       T_META_ASROOT(true), T_META_TAG_VM_PREFERRED) {
  int s = -1;
  int s6 = -1;
  char ifname[IFNAMSIZ];
  ...
  T_EXPECT_POSIX_SUCCESS(s = socket(AF_INET, SOCK_DGRAM, 0), "socket");
  T_QUIET;
  T_EXPECT_POSIX_SUCCESS(s6 = socket(AF_INET6, SOCK_DGRAM, 0), "socket");
...
static int get_inet_dgram_socket(void) {
  if (S_socket >= 0) {
    return S_socket;
  }
...
static void close_inet_dgram_socket(void) {
  if (S_socket >= 0) {
    close(S_socket);
    S_socket = -1;
  }
...
static int get_inet6_dgram_socket(void) {
  if (S_socket_6 >= 0) {
    return S_socket_6;
  }
...
static void close_inet6_dgram_socket(void) {
  if (S_socket_6 >= 0) {
    close(S_socket_6);
    S_socket_6 = -1;
  }
...
static void test_proto_attach_cleanup(void) {
  if (ifname[0] != '\0') {
    int s;
...
static void test_proto_attach_atend(void) {
...
static void test_proto_attach(void) {
  ...
  T_ASSERT_POSIX_SUCCESS(error, "SIOCPROTOATTACH %s succeeded", ifr.ifr_name);

  /* second proto attach should fail with EEXIST */
  error = ioctl(s, SIOCPROTOATTACH, &ifr);
  T_ASSERT_POSIX_FAILURE(error, EEXIST, "SIOCPROTOATTACH %s failed as expected",
                         ifr.ifr_name);
  ...
  T_ASSERT_POSIX_SUCCESS(error, "SIOCPROTOATTACH_IN6 %s succeeded",
                         ifr.ifr_name);

  /* second proto attach should fail with EEXIST */
  error = ioctl(s6, SIOCPROTOATTACH_IN6, &ifr);
  T_ASSERT_POSIX_FAILURE(
      error, EEXIST, "SIOCPROTOATTACH_IN6 %s failed as expected", ifr.ifr_name);
  test_proto_attach_cleanup();
}

T_DECL(sioc_proto_attach, "test protocol attachment", T_META_ASROOT(true),
 ...
 */
T_DECL(sioc_print_all, "print all socket interface ioctls",
       T_META_ENABLED(false), T_META_TAG_VM_PREFERRED) {
#define X(a)                                                                   \
  T_LOG("%-32s 0x%08lx GROUP %3lu BASECMD %3ld LEN %4lu\n", #a, a,             \
        (a & 0x00000ff00) >> 8, a & 0x000000ff, IOCPARM_LEN(a));
  SIOC_LIST
#undef X
}
