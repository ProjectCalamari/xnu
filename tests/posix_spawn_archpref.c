...
static void run_test(const char *name, cpu_type_t type, cpu_subtype_t subtype) {
  int ret, pid;
  ...
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "%s: posix_spawnattr_init", name);

  ret = posix_spawnattr_setarchpref_np(&spawnattr,
                                       sizeof(cpuprefs) / sizeof(cpuprefs[0]),
                                       cpuprefs, subcpuprefs, NULL);
  T_QUIET;
  T_ASSERT_POSIX_SUCCESS(ret, "%s: posix_spawnattr_setarchpref_np", name);
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, pid, "%s: child pid", name);

  T_QUIET;
  T_ASSERT_EQ(WIFEXITED(wait_ret), 1,
              "%s: child process should have called exit()", name);

  if (subtype != CPU_SUBTYPE_ANY) {
    T_ASSERT_EQ(WEXITSTATUS(wait_ret), subtype,
  ...
  T_QUIET;
  T_ASSERT_EQ(ret, 0, "%s: posix_spawnattr_destroy", name);
}

T_DECL(posix_spawn_archpref,
       "verify posix_spawn_setarchpref_np can select slices",
       T_META_TAG_VM_PREFERRED) {
#if defined(__x86_64__)
  run_test("x86_64", CPU_TYPE_X86_64, CPU_SUBTYPE_X86_64_ALL);
#endif /* defined(__x86_64__) */
       /* macOS does not build a arm64 slice */
#if defined(__arm64__) && defined(__LP64__) && !TARGET_OS_OSX
