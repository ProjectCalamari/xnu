...
/*
 * Test to validate that _COMM_PAGE_CPU_QUIESCENT_COUNTER ticks at least once
 * per second
 *
...
T_DECL(test_quiescent_counter,
       "Validate that _COMM_PAGE_CPU_QUIESCENT_COUNTER increments",
...
T_DECL(test_quiescent_counter,
  ...
  rv = sysctlbyname("kern.cpu_checkin_interval", &cpu_checkin_min_interval,
                    &value_size, NULL, 0);
  ...
  T_ASSERT_GT(cpu_checkin_min_interval, 0,
              "kern.cpu_checkin_interval should be > 0");

  COMM_PAGE_SLOT_TYPE(uint64_t)
  commpage_addr = COMM_PAGE_SLOT(uint64_t, CPU_QUIESCENT_COUNTER);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    T_ASSERT_GT(counter, last_counter,
                "_COMM_PAGE_CPU_QUIESCENT_COUNTER must monotonically increase "
