...
kern_return_t
memory_backing_aware_buffer_stage_outproc(struct kdp_output_stage *stage,
                                          unsigned int request, char *corename,
                                          uint64_t length, void *panic_data);

static kern_return_t kosf_outproc_mock(__unused struct kdp_output_stage *stage,
...
T_GLOBAL_META(T_META_NAMESPACE("xnu.unit.panic_path_test"),
              T_META_RADAR_COMPONENT_NAME("xnu"), T_META_OWNER("e_zisman"),
              T_META_RUN_CONCURRENTLY(false));

T_DECL(xnu_osfmk_kdp_memory_backing_aware_buffer_stage_outproc,
  ...
  struct {
    ...
    kern_return_t expected;
      ...
      {.test_name = "not-normal memory flow, 1-byte reads",
       ...
       .panic_data_length =
           sizeof(panic_data) -
           1, // ensure length of panic data is not aligned to 4 bytes.
       .expected = KERN_SUCCESS},
  };

  T_MOCK_SET_RETVAL(kvtophys, pmap_paddr_t,
                    0x12345678); // arbitrary value; isn't used anyways since we
                                 // mock pmap_cache_attributes.

  for (size_t i = 0; i < sizeof(test_cases) / sizeof(test_cases[0]); i++) {
    T_MOCK_SET_RETVAL(pmap_cache_attributes, unsigned int,
                      test_cases[i].pmap_cache_attributes_retval);

    T_EXPECT_EQ(
        test_cases[i].expected,
        memory_backing_aware_buffer_stage_outproc(
            &stage1, KDP_DATA, "corename", sizeof(panic_data), panic_data),
